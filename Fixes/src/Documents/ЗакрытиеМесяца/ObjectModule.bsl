...
&Вместо("КорректировкаСписанияУчетЗатрат")
Процедура AzRyFix_КорректировкаСписанияУчетЗатрат(ВидОперации, ТаблицаОшибок, Отказ)
	
	Если Не AzRyИспользоватьСтаруюВерсиюЗакрытияМесяца Тогда		
		ПродолжитьВызов(ВидОперации, ТаблицаОшибок, Отказ);
		Возврат;
	КонецЕсли;	
...
&Вместо("РассчитатьФактическуюСебестоимостьВыпуска")
Процедура AzRyFix_РассчитатьФактическуюСебестоимостьВыпуска(Отказ, ВидОперации, ТаблицаОшибок)
	
	Если Не AzRyИспользоватьСтаруюВерсиюЗакрытияМесяца Тогда		
		ПродолжитьВызов(Отказ, ВидОперации, ТаблицаОшибок);
		Возврат;
	КонецЕсли;	
