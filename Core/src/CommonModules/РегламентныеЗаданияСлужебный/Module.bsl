...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("ka='ბლოკირების მოხსნა ასევე, შეიძლება მოგვიანებით, შემდეგ ნაწილში <b>ადმინისტრირება - მხარდაჭერა და მომსახურება</b>.';ru='Блокировку также можно снять позднее в разделе <b>Администрирование - Поддержка и обслуживание</b>.'"));
...
Процедура ВызватьИсключениеЕслиНетПраваАдминистрирования() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ka='Нарушение прав доступа.';ru='Нарушение прав доступа.'");
		КонецЕсли;
	Иначе
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание) Экспорт
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელით გაშვება: %1';ru='Запуск вручную: %1'"), ПредставлениеРегламентногоЗадания(Задание));
...
Функция ТекстНеОпределено() Экспорт
	...
	Возврат НСтр("ka='<გაურკვეველია>';ru='<не определено>'");
...
Процедура ОтменитьФоновоеЗадание(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='ფონური დავალება სერვერზე ვერ მოიძებნა.';ru='Фоновое задание не найдено на сервере.'");
	КонецЕсли;
	
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Активно Тогда
		ВызватьИсключение НСтр("ka='დავალების შესრულება არ ხდება, მისი შეცვლა არ შეიძლება.';ru='Задание не выполняется, его нельзя отменить.'");
	КонецЕсли;
...
Процедура УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='გარე რესურსებთან მუშაობა დაბლოკილია';ru='Работа с внешними ресурсами заблокирована'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы = Ложь, ПоказатьФорму = Ложь) Экспорт
	...
	Если РазделениеВключено <> ПараметрыБлокировки.РазделениеВключено Тогда
		...
		ТекстСообщения = НСтр("ka='საინფორმაციო ბაზა დანართიდან ინტერნეტში გადაადგილდა.';ru='Информационная база была перемещена из приложения в Интернете.'");
	...
	Если ЭтоФайловаяБаза <> ПараметрыБлокировки.ЭтоФайловаяБаза Тогда
		ТекстСообщения = ?(ЭтоФайловаяБаза, НСтр("ka='საინფორმაციო ბაზამ მუშაობის კლიენტ-სერვერული რეჟიმიდან გადაინაცვლა ფაილურ რეჟიმში.';ru='Информационная база была перемещена из клиент-серверного режима работы в файловый.'"),
			НСтр("ka='საინფორმაციო ბაზამ მუშაობის ფაილური რეჟიმიდან გადაინაცვლა კლიენტ-სერვერულ რეჟიმში.';ru='Информационная база была перемещена из файловый режима работы в клиент-серверный.'"));
		УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
		Возврат Истина;
	КонецЕсли;
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საინფორმაციო ბაზის კატალოგში შემოწმების ფაილი %1 არ არის.';ru='В каталоге информационной базы отсутствует файл проверки %1.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИдентификаторБазы <> ПараметрыБлокировки.ИдентификаторБазы Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შემოწმების ფაილ %1-ში საინფორმაციო ბაზის იდენტიფიკატორი მიმდინარე ბაზაში არსებულ იდენტიფიკატორს არ შეესაბამება.';ru='Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
