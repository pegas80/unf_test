...
Функция ВыполнитьТестовоеПодключениеКСайту(НастройкиПодключения, ТекстСообщения) Экспорт
	...
	Если НЕ ПолучитьНастройкиПодключения(ПараметрыПодключения, ОписаниеОшибки) Тогда
		...
		ТекстСообщения = НСтр("ka='შეცდომა საიტთან დაკავშირების პარამეტრების მიღებისას.';ru='Ошибка при получении параметров подключения к сайту.';en='Error retrieving settings for connecting to the site.'") + Символы.ПС + ОписаниеОшибки;
		Возврат Ложь;
	...
	Если Соединение = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='შეცდომა საიტთან შეერთების დაყენებისას.';ru='Ошибка при установке соединения с сайтом.';en='Error installing the site connection.'") + Символы.ПС + ОписаниеОшибки;
		Возврат Ложь;
	...
	Если Успешно Тогда
		...
		ТекстСообщения = НСтр("ka='საიტთან შეერთება წარმატებით არის დაყენებული.';ru='Соединение с сайтом успешно установлено.';en='A compound with the site is successfully established.'");
		
	Иначе
		
		ТекстСообщения = НСтр("ka='შეერთების დაყენება ვერ მოხერხდა.';ru='Не удалось установить соединение.';en='Unable to establish a connection.'") + Символы.ПС + ОписаниеОшибки;
...
Процедура ЗаданиеВыполнитьОбмен(КодУзлаОбмена) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(УзелОбмена) Тогда
		...
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			УзелОбмена,
			НСтр("ka='კოდთან გაცვლის კვანძი ნაპოვნი არ არის';ru='Не найден узел обмена с кодом';en='Unable to find a node with code sharing'") + " " + КодУзлаОбмена);
	...
	Если УзелОбмена.ПометкаУдаления Тогда
		...
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			УзелОбмена.Метаданные(),
			УзелОбмена,
			НСтр("ka='გაცვლის გამართვა მონიშნულია წასაშლელად. გაცვლა გაუქმებულია.';ru='Настройка обмена помечена на удаление. Обмен отменен.';en='Setting up an exchange marked for deletion. Exchange canceled.'"));
	...
	Если УзелОбмена.ВыгружатьНаСайт Тогда
		...
		Если Не ВыполнитьТестовоеПодключениеКСайту(СтруктураПодключения, ТекстСообщения) Тогда			
			ЗаписьЖурналаРегистрации("Обмен с сайтами",
				...
				УзелОбмена.Метаданные(),
				УзелОбмена,
				ТекстСообщения + НСтр("ka='გაცვლა გაუქმებულია.';ru=' Обмен отменен.'"));
	...
	ВыполнитьОбмен(УзелОбмена, НСтр("ka='ფონური გაცვლა';ru='Фоновый обмен';en='background exchange'"));
...
Процедура ВыполнитьОбмен(УзелОбмена, РежимЗапускаОбмена, ВыгружатьТолькоИзменения = Истина) Экспорт
	...
	Если ОсновныеПараметры.ОбменТоварами Тогда 
		...
		СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДата()) + " " + НСтр("ka='საქონლის გადმოტვირთვის გაშვება';ru='Запуск выгрузки товаров';en='Running unloading goods'");
...
Функция ВыгрузитьКаталогПредложенияНаСайт(Параметры, СтрокаТаблицыИнформации)
	...
	Если Успешно Тогда 
		...
		СтрокаТаблицыИнформации.Описание = 
			...
			+ ТекущаяДата() + НСтр("ka='საქონელი წარმატებით გადმოიტვირთა საიტზე.';ru=' Товары успешно выгружены на сайт.'")
			+ ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС + НСтр("ka='დამატებითი ინფორმაცია გადმოტვირთვის შესახებ:';ru='Дополнительная информация о выгрузке:';en='For more information about unloading:'") + Символы.ПС + ОписаниеОшибки);
		
	Иначе
		
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ ТекущаяДата() + НСтр("ka='საიტზე გადმოტვირთვა შეცდომებით დასრულდა.';ru=' Выгрузка на сайт завершилась с ошибками.'") + Символы.ПС + ОписаниеОшибки;
	...
	Исключение
		...
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание
			+ Символы.ПС
			+ ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось очистить каталог обмена: ';ka='გაცვლის კატალოგის გასუფთავება ვერ მოხერხდა:'")
...
Функция ВыгрузитьКлассификаторИКаталог(Параметры, ИмяФайлаКаталога, ПакетCML)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(СтруктураРезультата.ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать XML-файл классификатора товаров на диск: ';ka='ვერ მოხერხდა საქონლების კლასიფიკატორის XML-ფაილის დისკზე ჩაწერა:'")
...
Функция ВыгрузитьФайл(ДанныеФайла, Параметры, ОписаниеОшибки, ВладелецКартинок=Неопределено) Экспорт
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ვერ მოხერხდა ნომენკლატურა %2-ის ფაილ %1-ის მონაცემთა მიღება';ru='Не удалось получить данные файла %1 номенклатуры %2';en='Failed to get the data file% 1% 2 Nomenclature'"),
					ДанныеФайла.Файл,
					ВладелецКартинок));
	...
	Иначе
		...
		Исключение
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				ОписаниеИсключительнойОшибки(НСтр("ru='Выгрузка файла номенклатуры: ';ka='ნომენკლატურის ფაილის გადმოტვირთვა:'")
					+ ВладелецКартинок));
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ОписаниеИсключительнойОшибки(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='კატალოგ %1-ის შექმნა ვერ მოხერხდა. ნომენკლატურა: %2';ru='Не удалось создать каталог %1. Номенклатура: %2';en='Unable to create directory% 1. Nomenclature: 2%'"),
				КаталогФайла,
				ВладелецКартинок))
		);
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ОписаниеИсключительнойОшибки(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ვერ მოხერხდა ფაილ %1-ის ჩაწერა დისკზე. ნომენკლატურა: %2';ru='Не удалось записать файл %1 на диск. Номенклатура: %2';en='Could not write the file% 1 to disk. Nomenclature: 2%'"),
				ПолноеИмяФайла,
...
Функция ВыгрузитьПакетПредложений(Параметры, ИмяФайлаПрайса, ПакетCML)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(СтруктураРезультата.ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать XML-файл пакета предложений на диск: ';ka='ვერ მოხერხდა შეთავაზების პაკეტის XML-ფაილის ჩაწერა დისკზე:'")
...
Процедура ПодготовитьИтоговуюИнформациюПоВыгрузкеТоваров(ТаблицаКаталогов, СтрокаТаблицыИнформации, ВыгруженоОбъектов)
	...
	СтрокаТаблицыИнформации.Описание = 
		СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ ТекущаяДата() + НСтр("ka='საქონლის გადმოტვირთვის ფაილების ფორმირება დასრულებულია.';ru=' Завершено формирование файлов выгрузки товаров'");
	...
	СтрокаТаблицыИнформации.Описание = 
		СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ НСтр("ru='Выгружено товаров: ';ka='გადმოტვირთულია საქონელი:'") + СтруктураРезультата.ВыгруженоТоваров + Символы.ПС
		+ НСтр("ru='предложений: ';ka='წინადადებებს:'") + СтруктураРезультата.ВыгруженоПредложений + Символы.ПС
		+ НСтр("ru='картинок: ';ka='სურათები:'") + СтруктураРезультата.ВыгруженоКартинок + Символы.ПС;
	
	Для Каждого Каталог Из ТаблицаКаталогов Цикл
		
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ НСтр("ru='В том числе для каталога ';ka='მათ შორის კატალოგისთვის'") + Каталог.Каталог + ":" + Символы.ПС 
			+ НСтр("ru='товаров: ';ka='საქონელი:'") + Каталог.СтруктураРезультата.ВыгруженоТоваров + Символы.ПС
			+ НСтр("ru='предложений: ';ka='წინადადებებს:'") + Каталог.СтруктураРезультата.ВыгруженоПредложений + Символы.ПС
			+ НСтр("ru='картинок: ';ka='სურათები:'") + Каталог.СтруктураРезультата.ВыгруженоКартинок + Символы.ПС;
		...
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ НСтр("ru='Ошибки в процессе выгрузки каталога ';ka='შეცდომები კატალოგის გადმოტვირთვის პროცესში'") + Каталог.Каталог + ":"
...
Процедура ЗафиксироватьЗавершениеВыгрузкиНоменклатуры(СтрокаТаблицыИнформации, Результат)
	...
	СтрокаТаблицыИнформации.Описание = СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ ДатаОкончания + " " + НСтр("ka='საქონლების გადმოტვირთვა დასრულებულია';ru='Завершена выгрузка товаров';en='Complete unloading of goods'");
...
Функция ЗагрузитьЗаказыССайта(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Если НЕ ВыполнитьАвторизациюДляСоединения(Соединение, Параметры.НастройкиПодключения, ОтветСервера, ОписаниеОшибки, ТипСоединения) Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='შეკვეთების ჩატვირთვა არ შესრულებულა.';ru='Загрузка заказов не выполнена.';en='Download orders not executed.'"));
		Возврат Ложь;
	...
	Если ПустаяСтрока(СтрокаXML) Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='სერვერიდან ჩატვირთულ მონაცემთა წაკითხვა ვერ მოხერხდა.';ru='Не удалось прочитать данные, загруженные с сервера.';en='Could not read data downloaded from the server.'"));
		Возврат Ложь;
	...
	Если НЕ ЗагрузитьЗаказы(СтрокаXML, СтруктураСтатистики, Параметры, ОписаниеОшибки) Тогда 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, 
			НСтр("ka='სერვერიდან ჩატვირთული დოკუმენტების დამუშავება ვერ მოხერხდა.';ru='Не удалось обработать документы, загруженные с сервера.';en='Unable to process the documents downloaded from the server.'"));
	...
	Если ОтветСервера = Неопределено Тогда 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ka='შეკვეთების ჩატვირთვა არ შესრულებულა.';ru='Загрузка заказов не выполнена.';en='Download orders not executed.'"));
		Возврат Ложь;
...
Функция ЗагрузитьЗаказыИзФайла(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Если НЕ Файл.Существует()
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ჩატვირთვის ფაილი ვერ მოიძებნა';ru='Файл загрузки не найден';en='Download File not found'") + ": " + Параметры.ФайлЗагрузки);
		Возврат Ложь;
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, СтруктураСтатистики, Параметры, ОписаниеОшибки) Экспорт
	...
	ИначеЕсли НЕ ОбъектXDTOСодержитСвойство(ЗаказыXDTO, "Документ") Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='დასამუშავებლად მონაცემები არ არის.';ru='Нет документов для обработки.';en='No documents for processing.'"));
		Возврат Истина;
	...
	Если МассивОтмененныхЗаказов.Количество() > 0 Тогда
		...
		Для Каждого ДокументСсылка Из МассивОтмененныхЗаказов Цикл
			...
			Исключение
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось отменить заказ: ';ka='შეკვეთის გაუქმება ვერ მოხერხდა:'")
						+ ДокументСсылка));
...
Функция ПолучитьЗаказыXDTO(СтрокаXML, Параметры, ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='XML-ის წაკითხვა ვერ მოხერხდა';ru='Не удалось прочитать XML';en='Failed read the XML'")));
...
Функция СоздатьОбновитьЗаказы(ДокументыXDTO, РанееЗагруженныеЗаказы, СтруктураСтатистики, Параметры, ОписаниеОшибки)
	...
	Для каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если НЕ ОбъектXDTOСодержитСвойство(ДокументXDTO, "ХозОперация")
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='შეცდომა კვანძის მნიშვნელობაში <Документ>.<ХозОперация> XML დოკუმენტის (%1)';ru='Ошибка в значении узла <Документ>.<ХозОперация> документа XML (%1)';en='The error in the value of the node <Document>. <Hozoperatsy> Document XML (% 1)'"),
		...
		Если НЕ ОбъектXDTOСодержитСвойство(ДокументXDTO, "Валюта")
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='შეცდომა კვანძის <Документ> მნიშვნელობაში. XML დოკუმენტის <Валюта>. ვალუტა უნდა იქნას შევსებული.';ru='Ошибка в значении узла <Документ>.<Валюта> документа XML. Валюта должна быть заполнена.';en='The error in the value of the node <Document>. <Currency> XML documents. Currency should be filled.'"));
		...
		Если НайденнаСтрока <> Неопределено Тогда
			...
			Если ДокументОбъект.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					Строка(ДокументОбъект.Ссылка) + НСтр("ka='გამოტოვებულია შემდეგი მიზეზის გამო:';ru=' пропущен по причине:'") + Символы.ПС 
					+ НСтр("ka='Статус заказа - ""Завершен"".';ru='Статус заказа - ""Завершен"".';en='Order status - ""Completed"".'"));
		...
		Если ЕстьСсылкиНаДокументыОплатыОтгрузки Тогда
			...
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 გამოტოვებულია, ვინაიდან არსებობს შეკვეთის საფუძველზე შეყვანილი დოკუმენტები.';ru='%1 пропущен, т.к. существуют документы, введенные на основании заказа.';en='% 1 is omitted, because there are documents introduced by the order.'"),
			...
			Если ОбновитьСоздатьДополнительныеСведенияЗаказа(ДокументОбъект.Ссылка, СвойстваЗаказа, ОписаниеОшибки) Тогда
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='შეკვეთის თვისებები %1 განახლებულია.';ru='Свойства заказа %1 обновлены.';en='Properties Order% 1 updated.'"),
					ДокументОбъект.Ссылка);
		...
		Иначе
			...
			Исключение
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ka='ვერ მოხერხდა ჩაწერა შეკვეთისა #';ru='Не удалось записать заказ №';en='Could not write the order number'") + ДокументОбъект.Номер));
...
Функция ОбновитьСоздатьДополнительныеСведенияЗаказа(ЗаказПокупателя, СвойстваЗаказа, ОписаниеОшибки)
	...
	Для Каждого СвойствоЗаказа Из МассивСоздатьОбновить Цикл
		...
		Если Свойство = Неопределено Тогда
			...
			СвойствоОбъект.Комментарий = НСтр("ka='შექმნილია ავტომატურად საიტიდან შეკვეთის ჩატვირთვისას';ru='Создано автоматически при загрузке заказа с сайта';en='Created automatically when ordering from the website'");
...
Функция ЗаполнитьЗаказДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваЗаказа, Параметры, ОписаниеОшибки)
	...
	Если НЕ ИдентифицироватьКонтрагента(ДокументОбъект, ДокументXDTO, Параметры, ОписаниеОшибки) Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ru='Не удалось идентифицировать контрагента для заказа номер: ';ka='Не удалось идентифицировать контрагента для заказа номер: '") + ДокументXDTO.Номер);
	КонецЕсли;
...
Функция ИдентифицироватьКонтрагента(ДокументОбъект, ДокументXDTO, Параметры, ОписаниеОшибки)
	...
	Иначе
		...
		Если ПустаяСтрока(Наименование) Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ka='კონტრაგენტის სახელწოდება არ არის შევსებული!';ru='Наименование контрагента не заполнено!';en='name of the counterparty is not filled!'"));
		...
		Если Параметры.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.Наименование Тогда
			...
			ТекстСообщенияНайденоНесколькоКонтрагентов = НСтр("ru=' по наименованию: ';ka='დასახელების მიხედვით:'") + Наименование;
		...
		ИначеЕсли Параметры.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.ИННКПП Тогда	
			...
			Если ПустаяСтрока(ИНН) Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки, 
					НСтр("ka='კონტრაგენტის გსნ არ არის შევსებული!';ru='ИНН контрагента не заполнен!';en='INN counterparty is empty!'"));
			...
			ТекстСообщенияНайденоНесколькоКонтрагентов = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გსნ-ის მიხედვით: %1, КПП: %2';ru=' по ИНН: %1, КПП: %2'"),
					ИНН, КПП);
		...
		Если НЕ ЗначениеЗаполнено(КонтрагентСсылка) Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ka='კონტრაგენტის პოვნა ან შექმნა ვერ მოხერხდა!';ru='Не удалось найти или создать контрагента!';en='Unable to find or create a counterpart!'"));
...
Процедура ЗаполнитьДанныеСправочниковПодчиненныхКонтрагенту(ДокументОбъект, КонтрагентОбъект, КонтрагентXDTO, ОписаниеОшибки)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Представители") И КонтрагентXDTO.Представители <> Неопределено Тогда
		...
		Для каждого ПредставительXDTO Из МассивПредставителейXDTO Цикл 
			...
			Элемент.Комментарий = НСтр("ru='Создан автоматически при импорте заказа с сайта ';ka='შექმნილია ავტომატურად, საიტიდან შეკვეთის იმპორტისას'") + ТекущаяДата();
			Элемент.Записать();
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "РасчетныеСчета")
		...
		Для каждого РасчетныйСчетXDTO Из МассивРасчетныхСчетовXDTO Цикл
			...
			Если НЕ ЗначениеЗаполнено(Валюта) Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось определить валюту банковского счета: ';ka='ვერ მოხერხდა საბანკო ანგარიშის ვალუტის დადგენა:'")
					+ НомерСчета));
...
Функция НайтиСоздатьХарактеристикуНоменклатуры(ТоварXDTO, ЭтоУслуга, Номенклатура, Параметры, ОписаниеОшибки) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ru='Характеристика номенклатуры не найдена по уникальному идентификатору: ';ka='ნომენკლატურის მახასიათებელი უნიკალური იდენტიფიკატორით ვერ მოიძებნა:'") + ИдХарактеристики);
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось выполнить поиск характеристики номенклатуры по уникальному идентификатору: ';ka='ნომენკლატურის მახასიათებლის ძებნის შესრულება ვერ მოხერხდა უნიკალური იდენტიფიკატორის მიხედვით:'") + ИдХарактеристики));
	...
	Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Ид") И СвойствоXDTOЗаполнено(ТоварXDTO.Ид) Тогда
		...
		Если СтрДлина(ИдНоменклатуры)=СтрДлина(Новый УникальныйИдентификатор) Тогда //Уникальный идентификатор 1С
			Попытка
				...
				Иначе
					ДобавитьОписаниеОшибки(ОписаниеОшибки,
					НСтр("ru='Номенклатура не найдена по уникальному идентификатору: ';ka='უნიკალური იდენტიფიკატორის მიხედვით ნომენკლატურა ვერ მოიძებნა:'") + ИдНоменклатуры);
				КонецЕсли;
				
			Исключение
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
				ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось выполнить поиск номенклатуры по уникальному идентификатору: ';ka='ვერ მოხერხდა ნომენკლატურის ძებნა უნიკალური იდენტიფიკატორით:'") + ТоварXDTO.Ид));
			КонецПопытки;
...
Функция ВыгрузитьЗаказыВФайл(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать XML-файл на диск: ';ka='ვერ მოხერხდა XML-ფაილის ჩაწერა დისკზე:'")
...
Процедура ЗаписатьИнформациюПоЗаказамВТаблицуИнформации(ТаблицаИнформации, ДатаНачала, Действие, Успех, СтруктураСтатистики, ОписаниеОшибки);
	...
	Если Действие = Перечисления.ДействияПриОбмене.ЗагрузкаДанных Тогда
		...
		Описание = Строка(ДатаНачала) + " " + НСтр("ka='შეკვეთების ჩატვირთვის გაშვება';ru='Запуск загрузки заказов';en='orders download Run'")
			+ Символы.ПС + НСтр("ru='Обработано: ';ka='დამუშავებულია:'") + СтруктураСтатистики.ОбработаноНаЗагрузке
			+ Символы.ПС + НСтр("ru='Загружено: ';ka='ჩატვირთულია:'") + СтруктураСтатистики.Загружено.Количество();
		
		ВывестиСписокДокументовДляПротокола(Описание, СтруктураСтатистики.Загружено);
		
		Описание = Описание
			+ Символы.ПС + НСтр("ru='Пропущено: ';ka='გამოტოვებულია:'") + СтруктураСтатистики.Пропущено.Количество();
		
		ВывестиСписокДокументовДляПротокола(Описание, СтруктураСтатистики.Пропущено);
		
		Описание = Описание
			+ Символы.ПС + НСтр("ru='Обновлено: ';ka='განახლებულია:'") + СтруктураСтатистики.Обновлено.Количество();
		
		ВывестиСписокДокументовДляПротокола(Описание, СтруктураСтатистики.Обновлено);
		
		Описание = Описание
			+ Символы.ПС + НСтр("ru='Создано: ';ka='შექმნილია:'") + СтруктураСтатистики.Создано.Количество();
		...
		Описание = Описание
			...
			+ " "
			+ НСтр("ka='შეკვეთების ჩატვირთვა დასრულებულია';ru='Завершена загрузка заказов';en='Completed orders download'");
			
	Иначе
		
		Описание = Строка(ДатаНачала) + " " + НСтр("ka='შეკვეთების გადმოტვირთვის გაშვება';ru='Запуск выгрузки заказов';en='Running orders unloading'")
			+ Символы.ПС + НСтр("ru='Выгружено: ';ka='გადმოტვირთულია:'") + СтруктураСтатистики.Выгружено.Количество();
		...
		Описание = Описание
			...
			+ СтрокаТаблицыИнформации.ДатаОкончания
			+ " "
			+ НСтр("ka='შეკვეთების გადმოტვირთვა დასრულებულია';ru='Завершена выгрузка заказов';en='Unloading completed orders'");
	...
	СтрокаТаблицыИнформации.Описание = СтрокаТаблицыИнформации.Описание
		+ Символы.ПС + НСтр("ka='დამატებითი ინფორმაცია:';ru='Дополнительная информация:';en='Additional Information:'") + Символы.ПС + ОписаниеОшибки;
...
Функция ПолучитьСтруктуруОсновныхПараметровОбмена() Экспорт
	...
	ОсновныеПараметры.Вставить("НаименованиеНалога", НСтр("ka='დღგ';ru='НДС';en='VAT'"));
...
Функция УстановитьСоединениеССервером(ПараметрыПодключения, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვერ %1:%2-თან შეერთების დაყენება ვერ მოხერხდა. შეამოწმეთ სერვერის მისამართის სისწორე, პორტი, მომხმარებლის სახელი და პაროლი.';ru='Не удалось установить соединение с сервером %1:%2. Проверьте правильность адреса сервера, порт, имя пользователя и пароль.';en='Failed to connect to server% 1% 2. Check that the server address, port, username and password.'"),
				ПараметрыПодключения.Сервер,
				ПараметрыПодключения.Порт)
			)
		);
...
Функция ВыполнитьАвторизациюДляСоединения(Соединение, ПараметрыПодключения, ОтветСервера, ОписаниеОшибки, ТипСоединения = "catalog") Экспорт
	...
	Если Соединение = Неопределено Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ავტორიზაცია არ არის შესრულებული.';ru='Авторизация не выполнена.';en='Authorization is not performed.'"));
		Возврат Ложь;
	...
	Если СокрЛП(НРег(СтрПолучитьСтроку(ОтветСервера, 1))) <> "success" Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ავტორიზაცია არ არის შესრულებული.';ru='Авторизация не выполнена.';en='Authorization is not performed.'")
			+ Символы.ПС 
			+ НСтр("ka='სერვერთან შეერთების დაყენება ვერ მოხერხდა. შეამოწმეთ მომხმარებლის სახელი და პაროლი.';ru='Не удалось установить соединение с сервером. Проверьте имя пользователя и пароль.';en='Could not connect to the server. Check your username and password.'"));
...
Функция ВыгрузитьНаСайт(Параметры,
	...
	Если НЕ Успешно Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='საიტზე გადმოტვირთვა არ შესრულდა.';ru='Выгрузка на сайт не выполнена.';en='Unloading to the site failed.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если СтрЧислоСтрок(ОтветСервера) <> 2 Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='საიტზე გადმოტვირთვა არ შესრულდა.';ru='Выгрузка на сайт не выполнена.';en='Unloading to the site failed.'") + Символы.ПС + НСтр("ka='სერვერის პასუხის წაკითხვა ვერ მოხერხდა. გაცვლის პარამეტრები არ არის მიღებული.';ru='Не удалось прочитать ответ сервера. Параметры обмена не получены.';en='Unable to read the response from the server. not received communication parameters.'"));
		Возврат Ложь;
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='გაცვლის პარამეტრების მიღების შეცდომა (ფაილის ზომის შეზღუდვა)!';ru='Ошибка получения параметров обмена (ограничение размера файла)!';en='Failed to get the exchange parameters (file size limit)!'")));
	...
	Для Каждого ТекФайл Из СписокФайловДляОтправки цикл
		...
		Если ОтветСервера = Неопределено Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ka='ვერ მოხერხდა სერვერის პასუხის მიღება. ფაილი არ არის გაგზავნილი.';ru='Не удалось получить ответ сервера. Файл не отправлен.';en=""Could not get the server's response. The file is not sent.""") + " (" + ТекФайл.Значение + ").");
		...
		Если СостояниеОбмена = НастройкиПодключения.ОтветСервера_АварийноеЗавершениеТекущейОперации Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ka='შეცდომა მოხდა სერვერის მხარეს. ფაილი არ გაგზავნილა.';ru='Произошла ошибка на стороне сервера. Файл не отправлен.';en='An error occurred on the server side. The file is not sent.'") + " (" + ТекФайл.Значение + ")."
				+ Символы.ПС + НСтр("ru='Ответ сервера: ';ka='სერვერის პასუხი:'") + Символы.ПС + ОтветСервера);
		...
		ИначеЕсли СостояниеОбмена = НастройкиПодключения.ОтветСервера_УспешноеЗавершениеТекущейОперации Тогда
			...
			Если СтрЧислоСтрок(ОтветСервера) > 1 Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					НСтр("ka='მიღებულია სეანსის წარმატებული დასრულების გაფართოებული სტატუსი';ru='Получен расширенный статус успешного завершения сеанса.';en='Get an advanced status of the successful completion of the session.'")
					+ Символы.ПС + НСтр("ru='Ответ сервера: ';ka='სერვერის პასუხი:'") + Символы.ПС + ОтветСервера);
		...
		Иначе
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ka='შეცდომა მოხდა სერვერის მხარეს. არ არის მიღებული ოპერაციის დასრულების სტატუსი. ფაილი არ არის გაგზავნილი.';ru='Произошла ошибка на стороне сервера. Не получен статус завершения операции. Файл не отправлен.';en='An error occurred on the server side. Not received the status of the operation is complete. The file is not sent.'") + " (" + ТекФайл.Значение + ")."
				+ Символы.ПС + НСтр("ru='Ответ сервера: ';ka='სერვერის პასუხი:'") + Символы.ПС + ОтветСервера);
			...
			Пока ИмпортПродолжается Цикл
				...
				Если ОтветСервера = Неопределено Тогда 
					...
					ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
						+ НСтр("ka='ვერ მოხერხდა გაცვლის პროცესის მიმდინარე მდგომარეობის მიღება. გაცვლის მონაცემები გაგზავნილია, მაგრამ არ არის ჩატვირთული.';ru='Не удалось получить текущее состояние процесса обмена. Данные обмена отправлены, но не загружены.';en='Failed to get the current status of the exchange process. Data exchange sent, but not loaded.'"));
					
				ИначеЕсли СтрЧислоСтрок(ОтветСервера) = 0 Тогда
					
					Успешно = Ложь;
					ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
						+ НСтр("ka='გაცვლის პროცესის მიმდინარე მდგომარეობის შესახებ მონაცემთა წაკითხვა ვერ მოხერხდა. გაცვლის მონაცემები გაგზავნილია, მაგრამ არ არის ჩატვირთული.';ru='Не удалось прочитать данные о текущем состоянии процесса обмена. Данные обмена отправлены, но не загружены.';en='Unable to read the current state of the data exchange process. Data exchange sent, but not loaded.'"));
				...
				Иначе
					...
					Если СостояниеОбмена = НастройкиПодключения.ОтветСервера_АварийноеЗавершениеТекущейОперации Тогда
						...
						Успешно = Ложь;
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
							+ НСтр("ka='შეცდომა მოხდა სერვერის მხარეს.';ru='Произошла ошибка на стороне сервера.';en='An error occurred on the server side.'")
							+ Символы.ПС + НСтр("ka='სერვერის პასუხი';ru='Ответ сервера';en='server response'") + ": " + Символы.ПС + ОтветСервера);
					...
					Иначе
						...
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
							+ НСтр("ka='შეცდომა მოხდა სერვერის მხარეს. მიღებულია იმპორტის უცნობი სტატუსი.';ru='Произошла ошибка на стороне сервера. Получен неизвестный статус импорта.';en='An error occurred on the server side. An unknown status of the import.'")
							+ Символы.ПС + НСтр("ka='სერვერის პასუხი';ru='Ответ сервера';en='server response'") + ": " + Символы.ПС + ОтветСервера);
...
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ПараметрыЗапроса="", Заголовки="", ОписаниеОшибки)
	...
	Если ФайлОтвета.Существует() Тогда
		...
		Если ТекстОтвета.КоличествоСтрок()>0 Тогда
			...
			КонецЕсли;
		Иначе
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ფაილის გაგზავნა სერვერზე: მიღებულია სერვერის ცარიელი პასუხი.';ru='Отправка файла на сервер: Получен пустой ответ сервера.';en='Sending a file to the server: Received empty response from the server.'"));
		КонецЕсли;
		
	Иначе
		
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ფაილის გაგზავნა სერვერზე: სერვერის პასუხი არ არის მიღებული.';ru='Отправка файла на сервер: Ответ сервера не получен.';en='Sending a file to the server: The server response is received.'")); 
	...
	Исключение
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			,
			НСтр("ka='დროებითი ფაილის წაშლა ვერ მოხერხდა';ru='Не удалось удалить временный файл';en='Could not delete the temporary file'") + " " + ИмяФайлаОтвета);
	КонецПопытки;
...
Функция ПодготовитьZIPАрхивы(КаталогНаДиске, ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='ვერ მოხერხდა zip-არქივის ჩაწერა დისკზე!';ru='Не удалось записать zip-архив на диск!';en='Could not write zip-file to disk!'")));
		Возврат НовыйСписокФайлов;
...
Функция ПолучитьДанныеССервера(Соединение, ПараметрыЗапроса = "", Заголовки = "", ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='სერვერისნ მონაცემთა მიღება ვერ მოხერხდა. შეამოწმეთ სერვერის მისამართის სისწორე, მომხმარებლის სახელი და პაროლი,';ru='Не удалось получить данные с сервера. Проверьте правильность адреса сервера, порт, имя пользователя и пароль,';en='Unable to get data from the server. Check that the server address, port, username and password,'")
				+ Символы.ПС
				+ НСтр("ka='აგრეთვე ინტერნეტთან შეერთების გამართვები.';ru='а также настройки подключения к Интернет.';en='as well as setting up an Internet connection.'")));
		
	КонецПопытки;
	
	Если HTTPОтветСервера<>Неопределено И HTTPОтветСервера.КодСостояния<>200 Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Получение данных с сервера: Ответ сервера: ';ka='Получение данных с сервера: Ответ сервера: '"+HTTPОтветСервера.КодСостояния));
	КонецЕсли;
	...
	Если ФайлОтвета.Существует() Тогда
		...
		Иначе
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ka='მონაცემთა მიღება სერვერიდან: მიღებულია სერვერის ცარიელი პასუხი.';ru='Получение данных с сервера: Получен пустой ответ сервера.';en='Retrieving data from the server: Received empty response from the server.'"));
		КонецЕсли;
		
	Иначе
		
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ka='მონაცემთა მიღება სერვერიდან: სერვერის პასუხი არ არის მიღებული.';ru='Получение данных с сервера: Ответ сервера не получен.';en='Getting data from the server: The server response is received.'"));
...
Процедура ВывестиСписокДокументовДляПротокола(Описание, МассивДокументов)
	...
	Описание = Описание + ". " + НСтр("ka='დოკუმენტების სია:';ru='Список документов:';en='List of documents:'");
	...
	Для Каждого Док Из МассивДокументов Цикл
		...
		Описание = Описание + Символы.ПС + Символы.НПП + Символы.НПП
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='№ %1 %2 -დან (№ %3 %4 -დან საიტზე)';ru='№ %1 от %2 (№ %3 от %4 на сайте)';en='№% 1% 2 from (№% 3% 4 from the site)'"),
				Док.Номер,
				Док.Дата,
				НомерЗаказаНаСайте,
				ДатаЗаказаНаСайте);
...
Функция РаспаковатьZIPАрхив(СтрокаZIP, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='შეკვეთებიანი არქივის განფუთვა ვერ მოხერხდა!';ru='Не удалось распаковать архив с заказами!';en='Unable to unzip the archive with the orders!'")));
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать архив с заказами: ';ka='ვერ მოხერხდა შეკვეთებიანი არქივის ჩაწერა:'")
				+ ИмяФайла));
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось распаковать архив с заказами: ';ka='შეკვეთებიანი არქივის განფუთვა ვერ მოხერხდა:'")
				+ ИмяФайла));
...
Функция ОписаниеИсключительнойОшибки(ТекстНачалаСообщения = "", ТекстОкончанияСообщения = "") Экспорт
	...
	ТекстСообщения = НСтр("ru='Произошла ошибка: ';ka='შეცდომა მოხდა:'")
...
Функция РазобратьАдресСайта(НастройкиПодключения, ОписаниеОшибки)
	...
	Если НЕ ПустаяСтрока(АдресСайта) Тогда
		...
		Исключение
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось получить номер порта: ';ka='ვერ მოხერხდა პორტის ნომრის მიღება:'")
					+ ПортСтрока + Символы.ПС
					+ НСтр("ka='შეამოწმეთ საიტის მისამართის შეყვანის სისწორე.';ru='Проверьте правильность ввода адреса сайта.';en='Check the correct input website address.'")));
...
Функция НадписьВсеГруппы() Экспорт
	Возврат "(" + НСтр("ka='ყველა';ru='Все';en='Everything'") + ")";
КонецФункции

Функция НадписьНетГруппы() Экспорт
	Возврат НСтр("ka='<არ არის ჯგუფი>';ru='<Нет группы>';en='<No Group>'");
КонецФункции
...
Процедура ВыполнитьОбменИнтерактивно(ПараметрыОбмена, АдресРезультата = Неопределено) Экспорт
	...
	РежимЗапускаОбмена = НСтр("ka='ინტერაქტიული გაცვლა';ru='Интерактивный обмен';en='interactive exchange'");
