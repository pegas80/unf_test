...
Функция ВыполнитьТестовоеПодключениеКСайту(НастройкиПодключения, ТекстСообщения) Экспорт
	...
	Если Соединение = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='შეცდომა საიტთან შეერთების დაყენებისას.';ru='Ошибка при установке соединения с сайтом.'") + Символы.ПС + ОписаниеОшибки;
		Возврат Ложь;
	...
	Если Успешно Тогда
		...
		ТекстСообщения = НСтр("ka='საიტთან შეერთება წარმატებით არის დაყენებული.';ru='Соединение с сайтом успешно установлено.'");
...
Процедура ЗаданиеВыполнитьОбмен(КодУзлаОбмена) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(УзелОбмена) Тогда
		...
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			УзелОбмена,
			НСтр("ka='კოდთან გაცვლის კვანძი ნაპოვნი არ არის';ru='Не найден узел обмена с кодом'") + " " + КодУзлаОбмена);
	...
	Если УзелОбмена.ПометкаУдаления Тогда
		...
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			УзелОбмена.Метаданные(),
			УзелОбмена,
			НСтр("ka='გაცვლის გამართვა მონიშნულია წასაშლელად. გაცვლა გაუქმებულია.';ru='Настройка обмена помечена на удаление. Обмен отменен.'"));
	...
	ВыполнитьОбмен(УзелОбмена, НСтр("ka='ფონური გაცვლა';ru='Фоновый обмен'"));
...
Процедура ВыполнитьОбмен(УзелОбмена, РежимЗапускаОбмена, ВыгружатьТолькоИзменения = Истина) Экспорт
	...
	Если ОсновныеПараметры.ОбменТоварами Тогда 
		...
		СтрокаТаблицыИнформации.Описание = Строка(ТекущаяДата()) + " " + НСтр("ka='საქონლის გადმოტვირთვის გაშვება';ru='Запуск выгрузки товаров'");
...
Функция ВыгрузитьКаталогПредложенияНаСайт(Параметры, СтрокаТаблицыИнформации)
	...
	Если Успешно Тогда 
		...
		СтрокаТаблицыИнформации.Описание = 
			...
			+ ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС + НСтр("ka='დამატებითი ინფორმაცია გადმოტვირთვის შესახებ:';ru='Дополнительная информация о выгрузке:'") + Символы.ПС + ОписаниеОшибки);
...
Функция ВыгрузитьФайл(ДанныеФайла, Параметры, ОписаниеОшибки, ВладелецКартинок=Неопределено) Экспорт
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ვერ მოხერხდა ნომენკლატურა %2-ის ფაილ %1-ის მონაცემთა მიღება';ru='Не удалось получить данные файла %1 номенклатуры %2'"),
					ДанныеФайла.Файл,
					ВладелецКартинок));
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ОписаниеИсключительнойОшибки(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='კატალოგ %1-ის შექმნა ვერ მოხერხდა. ნომენკლატურა: %2';ru='Не удалось создать каталог %1. Номенклатура: %2'"),
				КаталогФайла,
				ВладелецКартинок))
		);
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ОписаниеИсключительнойОшибки(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ვერ მოხერხდა ფაილ %1-ის ჩაწერა დისკზე. ნომენკლატურა: %2';ru='Не удалось записать файл %1 на диск. Номенклатура: %2'"),
				ПолноеИмяФайла,
...
Процедура ЗафиксироватьЗавершениеВыгрузкиНоменклатуры(СтрокаТаблицыИнформации, Результат)
	...
	СтрокаТаблицыИнформации.Описание = СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ ДатаОкончания + " " + НСтр("ka='საქონლების გადმოტვირთვა დასრულებულია';ru='Завершена выгрузка товаров'");
...
Функция ЗагрузитьЗаказыССайта(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Если НЕ ВыполнитьАвторизациюДляСоединения(Соединение, Параметры.НастройкиПодключения, ОтветСервера, ОписаниеОшибки, ТипСоединения) Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='შეკვეთების ჩატვირთვა არ შესრულებულა.';ru='Загрузка заказов не выполнена.'"));
		Возврат Ложь;
	...
	Если ПустаяСтрока(СтрокаXML) Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='სერვერიდან ჩატვირთულ მონაცემთა წაკითხვა ვერ მოხერხდა.';ru='Не удалось прочитать данные, загруженные с сервера.'"));
		Возврат Ложь;
	...
	Если НЕ ЗагрузитьЗаказы(СтрокаXML, СтруктураСтатистики, Параметры, ОписаниеОшибки) Тогда 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, 
			НСтр("ka='სერვერიდან ჩატვირთული დოკუმენტების დამუშავება ვერ მოხერხდა.';ru='Не удалось обработать документы, загруженные с сервера.'"));
	...
	Если ОтветСервера = Неопределено Тогда 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ka='შეკვეთების ჩატვირთვა არ შესრულებულა.';ru='Загрузка заказов не выполнена.'"));
		Возврат Ложь;
...
Функция ЗагрузитьЗаказыИзФайла(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Если НЕ Файл.Существует()
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ჩატვირთვის ფაილი ვერ მოიძებნა';ru='Файл загрузки не найден'") + ": " + Параметры.ФайлЗагрузки);
		Возврат Ложь;
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, СтруктураСтатистики, Параметры, ОписаниеОшибки) Экспорт
	...
	ИначеЕсли НЕ ОбъектXDTOСодержитСвойство(ЗаказыXDTO, "Документ") Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='დასამუშავებლად მონაცემები არ არის.';ru='Нет документов для обработки.'"));
		Возврат Истина;
...
Функция ПолучитьЗаказыXDTO(СтрокаXML, Параметры, ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='XML-ის წაკითხვა ვერ მოხერხდა';ru='Не удалось прочитать XML'")));
...
Функция СоздатьОбновитьЗаказы(ДокументыXDTO, РанееЗагруженныеЗаказы, СтруктураСтатистики, Параметры, ОписаниеОшибки)
	...
	Для каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если НЕ ОбъектXDTOСодержитСвойство(ДокументXDTO, "ХозОперация")
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='შეცდომა კვანძის მნიშვნელობაში <Документ>.<ХозОперация> XML დოკუმენტის (%1)';ru='Ошибка в значении узла <Документ>.<ХозОперация> документа XML (%1)'"),
		...
		Если НЕ ОбъектXDTOСодержитСвойство(ДокументXDTO, "Валюта")
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='შეცდომა კვანძის <Документ> მნიშვნელობაში. XML დოკუმენტის <Валюта>. ვალუტა უნდა იქნას შევსებული.';ru='Ошибка в значении узла <Документ>.<Валюта> документа XML. Валюта должна быть заполнена.'"));
		...
		Если НайденнаСтрока <> Неопределено Тогда
			...
			Если ДокументОбъект.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					...
					+ НСтр("ka='Статус заказа - ""Завершен"".';ru='Статус заказа - ""Завершен"".'"));
		...
		Если ЕстьСсылкиНаДокументыОплатыОтгрузки Тогда
			...
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 გამოტოვებულია, ვინაიდან არსებობს შეკვეთის საფუძველზე შეყვანილი დოკუმენტები.';ru='%1 пропущен, т.к. существуют документы, введенные на основании заказа.'"),
			...
			Если ОбновитьСоздатьДополнительныеСведенияЗаказа(ДокументОбъект.Ссылка, СвойстваЗаказа, ОписаниеОшибки) Тогда
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='შეკვეთის თვისებები %1 განახლებულია.';ru='Свойства заказа %1 обновлены.'"),
					ДокументОбъект.Ссылка);
		...
		Иначе
			...
			Исключение
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ka='ვერ მოხერხდა ჩაწერა შეკვეთისა #';ru='Не удалось записать заказ №'") + ДокументОбъект.Номер));
...
Функция ОбновитьСоздатьДополнительныеСведенияЗаказа(ЗаказПокупателя, СвойстваЗаказа, ОписаниеОшибки)
	...
	Для Каждого СвойствоЗаказа Из МассивСоздатьОбновить Цикл
		...
		Если Свойство = Неопределено Тогда
			...
			СвойствоОбъект.Комментарий = НСтр("ka='შექმნილია ავტომატურად საიტიდან შეკვეთის ჩატვირთვისას';ru='Создано автоматически при загрузке заказа с сайта'");
...
Функция ИдентифицироватьКонтрагента(ДокументОбъект, ДокументXDTO, Параметры, ОписаниеОшибки)
	...
	Иначе
		...
		Если ПустаяСтрока(Наименование) Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ka='კონტრაგენტის სახელწოდება არ არის შევსებული!';ru='Наименование контрагента не заполнено!'"));
		...
		ИначеЕсли Параметры.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.ИННКПП Тогда	
			...
			Если ПустаяСтрока(ИНН) Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки, 
					НСтр("ka='კონტრაგენტის გსნ არ არის შევსებული!';ru='ИНН контрагента не заполнен!'"));
		...
		Если НЕ ЗначениеЗаполнено(КонтрагентСсылка) Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ka='კონტრაგენტის პოვნა ან შექმნა ვერ მოხერხდა!';ru='Не удалось найти или создать контрагента!'"));
...
Процедура ЗаписатьИнформациюПоЗаказамВТаблицуИнформации(ТаблицаИнформации, ДатаНачала, Действие, Успех, СтруктураСтатистики, ОписаниеОшибки);
	...
	Если Действие = Перечисления.ДействияПриОбмене.ЗагрузкаДанных Тогда
		...
		Описание = Строка(ДатаНачала) + " " + НСтр("ka='შეკვეთების ჩატვირთვის გაშვება';ru='Запуск загрузки заказов'")
		...
		Описание = Описание
			...
			+ " "
			+ НСтр("ka='შეკვეთების ჩატვირთვა დასრულებულია';ru='Завершена загрузка заказов'");
			
	Иначе
		
		Описание = Строка(ДатаНачала) + " " + НСтр("ka='შეკვეთების გადმოტვირთვის გაშვება';ru='Запуск выгрузки заказов'")
		...
		Описание = Описание
			...
			+ СтрокаТаблицыИнформации.ДатаОкончания
			+ " "
			+ НСтр("ka='შეკვეთების გადმოტვირთვა დასრულებულია';ru='Завершена выгрузка заказов'");
	...
	СтрокаТаблицыИнформации.Описание = СтрокаТаблицыИнформации.Описание
		+ Символы.ПС + НСтр("ka='დამატებითი ინფორმაცია:';ru='Дополнительная информация:'") + Символы.ПС + ОписаниеОшибки;
...
Функция ПолучитьСтруктуруОсновныхПараметровОбмена() Экспорт
	...
	ОсновныеПараметры.Вставить("НаименованиеНалога", НСтр("ka='დღგ';ru='НДС'"));
...
Функция УстановитьСоединениеССервером(ПараметрыПодключения, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვერ %1:%2-თან შეერთების დაყენება ვერ მოხერხდა. შეამოწმეთ სერვერის მისამართის სისწორე, პორტი, მომხმარებლის სახელი და პაროლი.';ru='Не удалось установить соединение с сервером %1:%2. Проверьте правильность адреса сервера, порт, имя пользователя и пароль.'"),
				ПараметрыПодключения.Сервер,
				ПараметрыПодключения.Порт)
			)
		);
...
Функция ВыполнитьАвторизациюДляСоединения(Соединение, ПараметрыПодключения, ОтветСервера, ОписаниеОшибки, ТипСоединения = "catalog") Экспорт
	...
	Если Соединение = Неопределено Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ავტორიზაცია არ არის შესრულებული.';ru='Авторизация не выполнена.'"));
		Возврат Ложь;
	...
	Если СокрЛП(НРег(СтрПолучитьСтроку(ОтветСервера, 1))) <> "success" Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ავტორიზაცია არ არის შესრულებული.';ru='Авторизация не выполнена.'")
			+ Символы.ПС 
			+ НСтр("ka='სერვერთან შეერთების დაყენება ვერ მოხერხდა. შეამოწმეთ მომხმარებლის სახელი და პაროლი.';ru='Не удалось установить соединение с сервером. Проверьте имя пользователя и пароль.'"));
...
Функция ВыгрузитьНаСайт(Параметры,
	...
	Если НЕ Успешно Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='საიტზე გადმოტვირთვა არ შესრულდა.';ru='Выгрузка на сайт не выполнена.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если СтрЧислоСтрок(ОтветСервера) <> 2 Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='საიტზე გადმოტვირთვა არ შესრულდა.';ru='Выгрузка на сайт не выполнена.'") + Символы.ПС + НСтр("ka='სერვერის პასუხის წაკითხვა ვერ მოხერხდა. გაცვლის პარამეტრები არ არის მიღებული.';ru='Не удалось прочитать ответ сервера. Параметры обмена не получены.'"));
		Возврат Ложь;
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='გაცვლის პარამეტრების მიღების შეცდომა (ფაილის ზომის შეზღუდვა)!';ru='Ошибка получения параметров обмена (ограничение размера файла)!'")));
	...
	Для Каждого ТекФайл Из СписокФайловДляОтправки цикл
		...
		Если ОтветСервера = Неопределено Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ka='ვერ მოხერხდა სერვერის პასუხის მიღება. ფაილი არ არის გაგზავნილი.';ru='Не удалось получить ответ сервера. Файл не отправлен.'") + " (" + ТекФайл.Значение + ").");
		...
		Если СостояниеОбмена = НастройкиПодключения.ОтветСервера_АварийноеЗавершениеТекущейОперации Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ka='შეცდომა მოხდა სერვერის მხარეს. ფაილი არ გაგზავნილა.';ru='Произошла ошибка на стороне сервера. Файл не отправлен.'") + " (" + ТекФайл.Значение + ")."
		...
		ИначеЕсли СостояниеОбмена = НастройкиПодключения.ОтветСервера_УспешноеЗавершениеТекущейОперации Тогда
			...
			Если СтрЧислоСтрок(ОтветСервера) > 1 Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					НСтр("ka='მიღებულია სეანსის წარმატებული დასრულების გაფართოებული სტატუსი';ru='Получен расширенный статус успешного завершения сеанса.'")
					+ Символы.ПС + НСтр("ru='Ответ сервера: ';ka='სერვერის პასუხი:'") + Символы.ПС + ОтветСервера);
		...
		Иначе
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ka='შეცდომა მოხდა სერვერის მხარეს. არ არის მიღებული ოპერაციის დასრულების სტატუსი. ფაილი არ არის გაგზავნილი.';ru='Произошла ошибка на стороне сервера. Не получен статус завершения операции. Файл не отправлен.'") + " (" + ТекФайл.Значение + ")."
				+ Символы.ПС + НСтр("ru='Ответ сервера: ';ka='სერვერის პასუხი:'") + Символы.ПС + ОтветСервера);
			...
			Пока ИмпортПродолжается Цикл
				...
				Если ОтветСервера = Неопределено Тогда 
					...
					ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
						+ НСтр("ka='ვერ მოხერხდა გაცვლის პროცესის მიმდინარე მდგომარეობის მიღება. გაცვლის მონაცემები გაგზავნილია, მაგრამ არ არის ჩატვირთული.';ru='Не удалось получить текущее состояние процесса обмена. Данные обмена отправлены, но не загружены.'"));
					
				ИначеЕсли СтрЧислоСтрок(ОтветСервера) = 0 Тогда
					
					Успешно = Ложь;
					ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
						+ НСтр("ka='გაცვლის პროცესის მიმდინარე მდგომარეობის შესახებ მონაცემთა წაკითხვა ვერ მოხერხდა. გაცვლის მონაცემები გაგზავნილია, მაგრამ არ არის ჩატვირთული.';ru='Не удалось прочитать данные о текущем состоянии процесса обмена. Данные обмена отправлены, но не загружены.'"));
				...
				Иначе
					...
					Если СостояниеОбмена = НастройкиПодключения.ОтветСервера_АварийноеЗавершениеТекущейОперации Тогда
						...
						Успешно = Ложь;
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
							+ НСтр("ka='შეცდომა მოხდა სერვერის მხარეს.';ru='Произошла ошибка на стороне сервера.'")
							+ Символы.ПС + НСтр("ka='სერვერის პასუხი';ru='Ответ сервера'") + ": " + Символы.ПС + ОтветСервера);
					...
					Иначе
						...
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
							+ НСтр("ka='შეცდომა მოხდა სერვერის მხარეს. მიღებულია იმპორტის უცნობი სტატუსი.';ru='Произошла ошибка на стороне сервера. Получен неизвестный статус импорта.'")
							+ Символы.ПС + НСтр("ka='სერვერის პასუხი';ru='Ответ сервера'") + ": " + Символы.ПС + ОтветСервера);
...
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ПараметрыЗапроса="", Заголовки="", ОписаниеОшибки)
	...
	Если ФайлОтвета.Существует() Тогда
		...
		Если ТекстОтвета.КоличествоСтрок()>0 Тогда
			...
			КонецЕсли;
		Иначе
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ფაილის გაგზავნა სერვერზე: მიღებულია სერვერის ცარიელი პასუხი.';ru='Отправка файла на сервер: Получен пустой ответ сервера.'"));
		КонецЕсли;
		
	Иначе
		
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ფაილის გაგზავნა სერვერზე: სერვერის პასუხი არ არის მიღებული.';ru='Отправка файла на сервер: Ответ сервера не получен.'")); 
	...
	Исключение
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			,
			НСтр("ka='დროებითი ფაილის წაშლა ვერ მოხერხდა';ru='Не удалось удалить временный файл'") + " " + ИмяФайлаОтвета);
	КонецПопытки;
...
Функция ПодготовитьZIPАрхивы(КаталогНаДиске, ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='ვერ მოხერხდა zip-არქივის ჩაწერა დისკზე!';ru='Не удалось записать zip-архив на диск!'")));
		Возврат НовыйСписокФайлов;
...
Функция ПолучитьДанныеССервера(Соединение, ПараметрыЗапроса = "", Заголовки = "", ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='სერვერისნ მონაცემთა მიღება ვერ მოხერხდა. შეამოწმეთ სერვერის მისამართის სისწორე, მომხმარებლის სახელი და პაროლი,';ru='Не удалось получить данные с сервера. Проверьте правильность адреса сервера, порт, имя пользователя и пароль,'")
				+ Символы.ПС
				+ НСтр("ka='აგრეთვე ინტერნეტთან შეერთების გამართვები.';ru='а также настройки подключения к Интернет.'")));
	...
	Если ФайлОтвета.Существует() Тогда
		...
		Иначе
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ka='მონაცემთა მიღება სერვერიდან: მიღებულია სერვერის ცარიელი პასუხი.';ru='Получение данных с сервера: Получен пустой ответ сервера.'"));
		КонецЕсли;
		
	Иначе
		
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ka='მონაცემთა მიღება სერვერიდან: სერვერის პასუხი არ არის მიღებული.';ru='Получение данных с сервера: Ответ сервера не получен.'"));
...
Процедура ВывестиСписокДокументовДляПротокола(Описание, МассивДокументов)
	...
	Описание = Описание + ". " + НСтр("ka='დოკუმენტების სია:';ru='Список документов:'");
	...
	Для Каждого Док Из МассивДокументов Цикл
		...
		Описание = Описание + Символы.ПС + Символы.НПП + Символы.НПП
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='№ %1 %2 -დან (№ %3 %4 -დან საიტზე)';ru='№ %1 от %2 (№ %3 от %4 на сайте)'"),
				Док.Номер,
				Док.Дата,
				НомерЗаказаНаСайте,
				ДатаЗаказаНаСайте);
...
Функция РаспаковатьZIPАрхив(СтрокаZIP, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ka='შეკვეთებიანი არქივის განფუთვა ვერ მოხერხდა!';ru='Не удалось распаковать архив с заказами!'")));
		...
		Исключение
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось получить номер порта: ';ka='ვერ მოხერხდა პორტის ნომრის მიღება:'")
					...
					+ НСтр("ka='შეამოწმეთ საიტის მისამართის შეყვანის სისწორე.';ru='Проверьте правильность ввода адреса сайта.'")));
...
Функция НадписьВсеГруппы() Экспорт
	Возврат "(" + НСтр("ka='ყველა';ru='Все'") + ")";
КонецФункции

Функция НадписьНетГруппы() Экспорт
	Возврат НСтр("ka='<არ არის ჯგუფი>';ru='<Нет группы>'");
КонецФункции
...
Процедура ВыполнитьОбменИнтерактивно(ПараметрыОбмена, АдресРезультата = Неопределено) Экспорт
	...
	РежимЗапускаОбмена = НСтр("ka='ინტერაქტიული გაცვლა';ru='Интерактивный обмен'");
