...
"';ka='УстановитьСостояниеНовостейДляПользователей, начало работы."
"МассивПользователей:"
"	%МассивПользователей%"
"СтруктураОтборов:"
"	%СтруктураОтборов%"
...
"'");
	...
	Если (ТипЗнч(МассивПользователей) <> ТипМассив)
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%МассивПользователей%", НСтр("ka='ყველას მიხედვით';ru='По всем';en='for all'"));
	Иначе
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтруктураОтборов%", НСтр("ka='Передано значения типа, отличного от Структуры';ru='Передано значения типа, отличного от Структуры';en='Submitted type value different from that of'"));
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначенияСостояний%", НСтр("ka='Передано значения типа, отличного от Структуры';ru='Передано значения типа, отличного от Структуры';en='Submitted type value different from that of'"));
	КонецЕсли;
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისმდგომარეობისდაყენებამომხმარებლებისთვის';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';en='News. Service and regulations. UstanovitSostoyanieNovosteyDlyaPolzovateley'"), // ИдентификаторШага
...
"';ka='УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: при работе в модели сервиса запускать получение новостей вручную можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"Состояния новостей можно установить только для пользователей с ролью ЧтениеНовостей или ПолныеПрава';ka='შესრულების შეცდომა: ისეთი მომხმარებლები არ არის, რომლებისთვის დასაყენებელი იქნება ახალი ამბების მდგომარეობა."
"ახალი ამბების მდგომარეობის დაყენება შეიძლება მხოლოდ იმ მომხმარებლებისთვის, რომლებსაც გააჩნიათ როლი ახალიამბებისწაკითხვა ან სრულიუფლებები'");
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისმდგომარეობისდაყენებამომხმარებლებისთვის';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';en='News. Service and regulations. UstanovitSostoyanieNovosteyDlyaPolzovateley'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
"';ka='УстановитьСостояниеНовостейДляПользователей, завершение."
"Обработано пользователей: %1"
"Обработано новостей: %2"
"'"),
...
"Возможные имена ключей совпадают с наименованиями ресурсов регистра сведений СостоянияНовостей';ka='მომხმარებლებისთვისახალიამბებისმდგომარეობისდაყენება, შესრულების შეცდომა: შევსების მნიშვნელობებად გადაცემულია ""სტრუქტურა""-სგან განსხვავებული ტიპის მნიშვნელობა, ან შევსების გამართვები შევსებული არ არის (სტრუქტურის არცერთი გასაღები არ არის მითითებული)."
"გასაღებების შესაძლებელი სახელები ემთხვევა ინფორმაციის რეგისტრ ახალიამბებისმდგომარეობები-ს რესურსების სახელწოდებებს'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისმდგომარეობისდაყენებამომხმარებლებისთვის';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';en='News. Service and regulations. UstanovitSostoyanieNovosteyDlyaPolzovateley'") , // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
"';ka='Не рекомендуется давать возможность чтения новостей внешнему пользователю (%ТекущийВнешнийПользователь%)"
"Подключены следующие роли:"
"	- ЧтениеНовостей         - %ЧтениеНовостей%"
"	- РедактированиеНовостей - %РедактированиеНовостей%"
"	- АдминистраторСистемы   - %АдминистраторСистемы%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:Новости.Разное';ru='БИП:Новости.Разное';en='BIP: Novosti.Raznoe'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სხვადასხვა. მომხმარებლებთან მუშაობის შეცდომა';ru='Новости. Разное. Ошибка работы с пользователями';en='News. Miscellanea. Error working with users'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"news.webits.1c.ru",
		443,
		НСтр("ka='ახალი ამბების მიღების ვებ-სერვისები';ru='Веб-сервисы получения новостей';en='Web services receive news'"));
	...
	Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
		...
		Если ГдеДвоеточие > 0 Тогда
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон(
					НСтр("ru='Попытка %1 / %2."
"Ошибка определения порта из имени сервера %3 (%4).';ka='მცდელობა %1 / %2."
"პორტის დადგენის შეცდომა სერვერის სახელიდან %3 (%4).'"),
					ПопыткаСоединения,
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей';en='BIP: Novosti.Zagruzka news'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ჩატვირთვა. ახალი ამბების სერვერთან შეერთება. პარამეტრების დადგენის შეცდომა';ru='Новости. Загрузка новостей. Соединение с сервером новостей. Ошибка определения параметров';en='News. Loading news. The connection to the news server. The error in determining the parameters'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"პორტი        = %5'"),
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей';en='BIP: Novosti.Zagruzka news'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. ახალი ამბების ჩატვირთვა. ახალი ამბების სერვერთან შეერთება';ru='Новости. Загрузка новостей. Соединение с сервером новостей';en='News. Loading news. The connection to the news server'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"%5';ka='მცდელობა %1 / %2:"
"ახალი ამბების მისაღებად სერვერ %3 (%4)-თან შეერთების შეცდომა შემდეგი მიზეზით:"
"%5'"),
				ПопыткаСоединения,
				НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
				ИмяСервера,
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей';en='BIP: Novosti.Zagruzka news'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. ახალი ამბების ჩატვირთვა. ახალი ამბების სერვერთან შეერთება. შეერთების შეცდომა';ru='Новости. Загрузка новостей. Соединение с сервером новостей. Ошибка соединения';en='News. Loading news. The connection to the news server. Connection error'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
#Область ЗаполнениеШаблонов_Новости_КатегорииНовостейИнтервалыВерсий
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					Символы.Таб + НСтр("ka='Набор записей пустой.';ru='Набор записей пустой.';en='A set of blank records.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_РассчитанныеОтборыПоНовостям_Пользовательские
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					НСтр("ka='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.';ru='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.';en='The service model for the system administrator can not get user settings or selections.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_ПоставляемыеДанные
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ПоставляемыеДанные%", Символы.Таб + НСтр("ka='Не найдено в поставляемых данных.';ru='Не найдено в поставляемых данных.';en='Not found in the supplied data.'"));
			КонецЕсли;
...
#КонецОбласти
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОНовостях%", НСтр("ka='Не найдено ни одной новости с таким UIN';ru='Не найдено ни одной новости с таким UIN';en='No news with UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ЛентыНовостей
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОЛентахНовостей%", НСтр("ka='Не найдено ни одной ленты новостей для новости с таким UIN';ru='Не найдено ни одной ленты новостей для новости с таким UIN';en='No news feeds for news with UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	Иначе
		ПредставлениеНастройкиНовостей = СтрШаблон(НСтр("ka='Настройки новостей необрабатываемого типа %1';ru='Настройки новостей необрабатываемого типа %1';en='News settings unhandled type% 1'"), ТипЗнч(НастройкиНовостей));
	КонецЕсли;

	РезультатДанныеОПрограммномОкружении_Константы = СтрШаблон(
		НСтр("ka='Константа.НастройкиНовостей:"
"%1"
"РазрешенаРаботаСНовостями: %2"
"РазрешенаРаботаСНовостямиЧерезИнтернет: %3';ru='Константа.НастройкиНовостей:"
"%1"
"РазрешенаРаботаСНовостями: %2"
...
"	Разделение данных:"
"		%ТекущийПользователь().РазделениеДанных%"
"';ka='Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
"Доступно использование разделенных данных: %ДоступноИспользованиеРазделенныхДанных()%"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ПараметрыСеанса
	...
	Иначе
		ПредставлениеПараметрыОкруженияБИП_Новости = СтрШаблон(НСтр("ka='Параметры окружения подсистемы новостей необрабатываемого типа %1';ru='Параметры окружения подсистемы новостей необрабатываемого типа %1';en='environment subsystem parameters news unhandled type% 1'"), ТипЗнч(ПараметрыОкруженияБИП_Новости));
	КонецЕсли;

	РезультатДанныеОПрограммномОкружении_ПараметрыСеанса = СтрШаблон(
		НСтр("ru='Параметры окружения подсистемы новостей:"
"%1';ka='Параметры окружения подсистемы новостей:"
"%1'"),
...
"Данные компьютера: %7';ka='Имя конфигурации: %1"
"Версия конфигурации: %2"
"Версия подсистемы БИП: %3"
"Версия платформы: %4"
"Операционная система: %5"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	Если РегламентноеЗадание = Неопределено Тогда
		ПредставлениеРасписания = НСтр("ka='Регламентное задание не найдено';ru='Регламентное задание не найдено';en='Scheduled jobs found'");
		РегламентноеЗадание_Использование = Ложь;
		ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ka='Никогда не выполнялось';ru='Никогда не выполнялось';en='Never satisfied'");
	Иначе
		...
		Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание) = ТипФоновоеЗадание Тогда
			...
			Иначе
				РегламентноеЗадание_СтруктураПоследнегоЗадания.Вставить("ПодробноеПредставлениеОшибки", НСтр("ka='Выполнено без ошибок';ru='Выполнено без ошибок';en='Completed without error'"));
			КонецЕсли;
		...
		Иначе
			ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ka='არ იყო შესრულებული';ru='Не выполнялось';en='were not satisfied'");
		КонецЕсли;
	КонецЕсли;
...
"Использование: %2"
"Последнее задание:"
"%3';ka='Расписание регламентного задания:"
"%1"
"Использование: %2"
"Последнее задание:"
"%3'"),
...
Функция НайтиНовости(СтруктураПараметровПоиска) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		Если НЕ ПустаяСтрока(лкСтрокаПоиска) Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = НСтр("ka='ახალიამბებისდამუშავება.ახალიამბებისძებნა: ახალი ამბების ძებნის შეცდომა მოხდა, ძებნის სტრიქონი [%СтрокаПоиска%] შემდეგი მიზეზით: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%';ru='ОбработкаНовостей.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%СтрокаПоиска%] по причине: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%';en='ObrabotkaNovostey.NaytiNovosti: news search error, search string [% Searchbox%] due to:% PodrobnoePredstavlenieOshibki (InformatsiyaObOshibke)%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:Новости.Разное';ru='БИП:Новости.Разное';en='BIP: Novosti.Raznoe'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სხვადასხვა. სრულტექსტიანი ძებნის შეცდომა';ru='Новости. Разное. Ошибка полнотекстового поиска';en='News. Miscellanea. Error-text search'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"';ka='Начало выполнения регламентного задания"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. ახალი ამბების ყველა გამართვა. დასაწყისი';ru='Новости. Все обновления новостей. Начало';en='News. All news updates. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. დასასრული';ru='Новости. Все обновления новостей. Конец';en='News. All news updates. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			НСтр("ka='სარეგლამენტო დავალების შესრულების იძულებითი დასრულება: საინფორმაციო ბაზა მონოპოლიურ რეჟიმში მუშაობს.';ru='Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.';en='Terminating perform scheduled tasks: information database running in exclusive mode.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		ЕстьОшибки = Истина;
...
"Время начала (мс): %ТНачало%"
"';ka='Получение и обработка новостей для лент новостей: %ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей)%"
"Время начала (мс): %ТНачало%"
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ლენტის მიხედვით ახალი ამბების მიღება და დამუშავება. დასაწყისი';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Начало';en='News. All news updates. Receiving and processing of news on news feeds. Start'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"';ka='Завершение получения и обработки новостей для лент новостей"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ლენტების მიხედვით ახალი ამბების მიღება და დამუშავება. დასასრული';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Конец';en='News. All news updates. Receiving and processing of news on news feeds. the end'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ლენტებით ახალი ამბების მიღება და დამუშავება. დასამუშავებლად ახალი ამბების ლენტების არარსებობა';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Отсутствие лент новостей для обработки';en='News. All news updates. Receiving and processing of news on news feeds. The lack of news feeds for processing'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ka='სარეგლამენტო განახლებისთვის ახალი ამბების ლენტები არ არის';ru='Нет лент новостей для регламентного обновления';en='No Feed for scheduled updates'"), // Комментарий
					ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='1. ახალი ამბების ლენტებიდან ახალი ამბების მიღება და დამუშავება (განზომილების სავალდებულო შევსებით ახალიამბებისლენტი)';ru='1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)';en='1. Receipt and processing of the news in the news feeds (with the required filling LentaNovostey measurement)'"),
					ТШаг,
...
"Время начала (мс): %ТНачало%"
"';ka='Обновление классификаторов через интернет, начало"
"Время начала (мс): %ТНачало%"
"'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
						НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. კლასიფიკატორების განახლება. დასაწყისი';ru='Новости. Все обновления новостей. Обновление классификаторов. Начало';en='News. All news updates. Update of classifiers. Start'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Обновление классификаторов через интернет, завершение"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
						НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. კლასიფიკატორების განახლება. დასასრული';ru='Новости. Все обновления новостей. Обновление классификаторов. Конец';en='News. All news updates. Update of classifiers. the end'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						Т1Длительность, // Данные
						ТекстСообщения, // Комментарий
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. კლასიფიკატორების განახლება. ინტერნეტით მუშაობა გამორთულია';ru='Новости. Все обновления новостей. Обновление классификаторов. Отключена работа через интернет';en='News. All news updates. Update of classifiers. Disable via the Internet'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ka='ინტერნეტით კლასიფიკატორების განახლება გამორთულია';ru='Обновление классификаторов через интернет отключено';en='Update of classifiers via the Internet is disabled'"), // Комментарий
					ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='2. კლასიფიკატორების განახლება';ru='2. Обновление классификаторов';en='2. Updating the classifiers'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"Время начала (мс): %ТНачало%"
"';ka='Автоудаление новостей, начало"
"Время начала (мс): %ТНачало%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ავტოწაშლა. დასაწყისი';ru='Новости. Все обновления новостей. Автоудаление новостей. Начало';en='News. All news updates. News Auto Delete. Start'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Автоудаление новостей, завершение."
"Результат выполнения:"
"%РезультатВыполнения%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ავტოწაშლა. დასასრული';ru='Новости. Все обновления новостей. Автоудаление новостей. Конец';en='News. All news updates. News Auto Delete. the end'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='3. ახალი ამბების ავტოწაშლა';ru='3. Автоудаление новостей';en='3. Auto Erase News'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"Время начала (мс): %ТНачало%"
"';ka='Обновление периодических свойств, начало"
"Время начала (мс): %ТНачало%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. პერიოდული თვისებების განახლება. დასაწყისი';ru='Новости. Все обновления новостей. Обновление периодических свойств. Начало';en='News. All news updates. Periodic update of properties. Start'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Обновление периодических свойств, завершение."
"Результат выполнения:"
"%РезультатВыполнения%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა გამართვა. პერიოდული თვისებების განახლება. დასასრული';ru='Новости. Все обновления новостей. Обновление периодических свойств. Конец';en='News. All news updates. Periodic update of properties. the end'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='4. პერიოდული თვისებების განახლება (მნიშვნელოვნება, აქტუალურობა)';ru='4. Обновление периодических свойств (важность, актуальность)';en='4. Update periodic properties (the importance, urgency)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"Время начала (мс): %ТНачало%"
"';ka='Обновление привязок к метаданным, начало"
"Время начала (мс): %ТНачало%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. მეტამონაცემებზე მიბმულების განახლება. დასაწყისი';ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Начало';en='News. All news updates. Updating metadata bindings. Start'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Обновление привязок к метаданным, завершение."
"Результат выполнения:"
"%РезультатВыполнения%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. მეტამონაცემებზე მიბმულების განახლება. დასასრული';ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Конец';en='News. All news updates. Updating metadata bindings. the end'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='5. მეტამონაცემთა მიბმულების განახლება (+ მნიშვნელოვნება)';ru='5. Обновление привязок к метаданным (+ важность)';en='5. Update the bindings metadata (+ important)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"Время начала (мс): %ТНачало%"
"';ka='Проверка версии платформы, начало"
"Время начала (мс): %ТНачало%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. პლატფორმის ვერსიის შემოწმება. დასაწყისი';ru='Новости. Все обновления новостей. Проверка версии платформы. Начало';en='News. All news updates. Check platform version. Start'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Проверка версии платформы, завершение."
"Результат выполнения:"
"%РезультатВыполнения%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей';en='BIP: Novosti.Vse news updates'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. პლატფორმის ვერსიის შემოწმება. დასასრული';ru='Новости. Все обновления новостей. Проверка версии платформы. Конец';en='News. All news updates. Check platform version. the end'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='6. პლატფორმის ვერსიის შემოწმება';ru='6. Проверка версии платформы';en='6. Check platform version'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"По шагам:"
"%РезультатПоШагам%"
"';ka='Завершение выполнения регламентного задания"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
""
...
"	Разделение данных:"
"		%ТекущийПользователь().РазделениеДанных%"
"';ka='Выполнение регламентного задания"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისწაშლა_ყოველდღიურად. წაშლის დასაწყისი';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. Начало удаления';en='News. Service and regulations. UdalitNovosti_Ezhednevno. Start removing'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"КатегорииНовостейИнтервалыВерсийНеобработанные: %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%';ka='შეკრებილია მონაცემები წასაშლელად:"
"ახალი ამბების (წასაშლელად): %ДанныеДляУдаления.НовостиДляУдаления.Количество()%"
"Новостей (для установки пометки удаления): %ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%"
"ახალიამბებისკატეგორიებიუბრალოდაუმუშავებლები: %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%"
"ახალიამბებისკატეგორიებიინტერვალებივერსიებიდაუმუშავებლები: %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისწაშლა_ყოველდღიურად. წასაშლელად მონაცემთა შეკრების შემდეგ';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления';en='News. Service and regulations. UdalitNovosti_Ezhednevno. After collecting the data to remove'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		ТекстСообщения = НСтр("ka='წაშლის შედეგი: %РезультатВыполнения%';ru='Результат удаления: %РезультатВыполнения%';en='Removal Results:% RezultatVypolneniya%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისწაშლა_ყოველდღიურად. წაშლის დასრულება';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. Окончание удаления';en='News. Service and regulations. UdalitNovosti_Ezhednevno. remove End'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისპერიოდულითვისებებისგანახლება. დასაწყისი';ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей. Начало';en='News. Service and regulations. ObnovleniePeriodicheskihSvoystvNovostey. Start'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		Если НЕ Результат.Пустой() Тогда
			...
			РезультатВыполнения = НСтр("ka='პერიოდული თვისებების გასაახლებლად მოიძებნა %ВсегоСтрок% ახალი ამბავი';ru='Найдено %ВсегоСтрок% новостей для обновления периодических свойств';en='Found% VsegoStrok% for news updates periodic properties'");
			...
			КонецЦикла;
		Иначе
			РезультатВыполнения = НСтр("ka='გასაახლებლად მონაცემები ვერ მოიძებნა';ru='Не найдено данных для обновления';en='Found data to be updated'");
		КонецЕсли;
		...
		ТекстСообщения = НСтр("ka='პერიოდული რეკვიზიტების დამუშავების შედეგი: %РезультатВыполнения%';ru='Результат обработки периодических реквизитов: %РезультатВыполнения%';en='The result of processing periodic requisites:% RezultatVypolneniya%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისპერიოდულითვისებებისგანახლება. დასასრული';ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей. Конец';en='News. Service and regulations. ObnovleniePeriodicheskihSvoystvNovostey. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. მეტამონაცემებზემიბმულებისგანახლება. დასაწყისი';ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным. Начало';en='News. Service and regulations. ObnovleniePrivyazokKMetadannym. Start'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		Если НЕ Результат.Пустой() Тогда
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			ВсегоСтрок = Выборка.Количество();
			РезультатВыполнения = НСтр("ka='მოიძებნა %ВсегоСтрок% ახალი ამბავი მეტამონაცემთა მიერთებულების გასაახლებლად';ru='Найдено %ВсегоСтрок% новостей для обновления привязок к метаданным';en='Found%% VsegoStrok news update bindings metadata'");
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ВсегоСтрок%", ВсегоСтрок);
			Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ka='მეტამონაცემებზე მიბმულების დამუშავების შედეგი: %РезультатВыполнения%';ru='Результат обработки привязок к метаданным: %РезультатВыполнения%';en='The result of processing the metadata bindings:% RezultatVypolneniya%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. მეტამონაცემებზემიბმულებისგანახლება. დასასრული';ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным. Конец';en='News. Service and regulations. ObnovleniePrivyazokKMetadannym. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';ka='ПолучитьИОбработатьНовостиПоЛентамНовостей, при работе в модели сервиса запускать получение новостей вручную можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Ошибка выполнения';en='News. Service and regulations. PoluchitIObrabotatNovostiPoLentamNovostey. Runtime error'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
	...
	// Запись в журнал регистрации
	Т0Начало = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ИдентификаторШага = НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. დასაწყისი';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Начало';en='News. Service and regulations. PoluchitIObrabotatNovostiPoLentamNovostey. Start'");
	ТекстСообщения = НСтр("ru='Начало загрузки и обработки новостей"
"Время начала (мс): %ТНачало%"
"';ka='Начало загрузки и обработки новостей"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
"';ka='ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей"
"Передан параметр с количеством значений: %МассивЛентНовостей.Количество()%"
"Результат выполнения запроса: Пустой"
"Возможно, список лент новостей пустой, или у всех лент новостей установлен признак ЛокальнаяЛентаНовостей (что означает, что по ней не надо загружать новости), или у всех лент новостей установлена пометка удаления."
"'");
...
"недостаточно прав для выполнения операции загрузки новостей.';ka='ახალიამბებისლენტისმიხედვითახალიამბებისმიღებადადამუშავება, მომხმარებელს"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"არასაკმარისი უფლებები აქვს ახალი ამბების ჩატვირთვის ოპერაციების შესასრულებლად.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%", ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ());
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Ошибка выполнения';en='News. Service and regulations. PoluchitIObrabotatNovostiPoLentamNovostey. Runtime error'") , // ИдентификаторШага
...
"'");
	...
	Т0Конец = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Т0Длительность = Т0Конец - Т0Начало;
	ИдентификаторШага = НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიმაბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. დასასრული';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Конец';en='News. Service and regulations. PoluchitIObrabotatNovostiPoLentamNovostey. the end'");
...
"По шагам:"
"%РезультатПоШагам%"
"';ka='Окончание загрузки и обработки новостей"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
""
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';ka='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტულისიებისგანახლებაახალიამბებისთვის_ყოველდღიურად. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Ошибка выполнения';en='News. Service and regulations. ObnovitStandartnyeSpiskiDlyaNovostey_Ezhednevno. Runtime error'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"	Разделение данных:"
"		%ТекущийПользователь().РазделениеДанных%"
"';ka='Выполнение регламентного задания (начало)"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები, სერვისი და რეგლამენტი. ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად. დასაწყისი';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Начало';en='News. Service and regulations. ObnovitStandartnyeSpiskiDlyaNovostey_Ezhednevno. Start'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Результат выполнения:"
"	%Результат%"
"';ka='Выполнение регламентного задания (конец)"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად. დასასრული';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Конец';en='News. Service and regulations. ObnovitStandartnyeSpiskiDlyaNovostey_Ezhednevno. the end'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად. შესრულების გაფრთხილება';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Предупреждение выполнения';en='News. Service and regulations. ObnovitStandartnyeSpiskiDlyaNovostey_Ezhednevno. warning execution'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
...
"Их можно обновлять из метаданных (при обновлении конфигурации), либо вручную.';ka='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, включен режим [Работа только с локальными новостями],"
"поэтому никакие классификаторы не обновляются с серверов 1С."
...
"поэтому все механизмы новостного центра отключены.';ka='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, отключен режим [Разрешена работа с новостями],"
...
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"недостаточно прав для выполнения операции загрузки стандартных списков.';ka='ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად, მომხმარებელს"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"არასაკმარისი უფლებები აქვს სტანდარტული სიების ჩატვირთვის ოპერაციების შესასრულებლად.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%", ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ());
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტულისიებისგანახლებაახალიამბებისთვის_ყოველდღიურად. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Ошибка выполнения';en='News. Service and regulations. ObnovitStandartnyeSpiskiDlyaNovostey_Ezhednevno. Runtime error'"), // ИдентификаторШага
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';ka='ПересчитатьНеобработанныеКатегорииНовостей, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Ошибка выполнения';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Runtime error'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
	...
	Если (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейПростые))
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 1/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 1/4';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Doing step 1/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, ახალი ამბების კატეგორიების გადავსების დამუშავების შესრულების დასაწყისი.';ru='ПересчитатьНеобработанныеКатегорииНовостей, начало выполнения обработки перезаполнения категорий новостей.';en='PereschitatNeobrabotannyeKategoriiNovostey, commencement of refilling categories of news processing.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 2/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 2/4';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Performing step 2/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, არასწორი ჩანაწერების წაშლა, რომელთა დამუშავება შეუძლებელია.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление неправильных записей, которые невозможно обработать.';en='PereschitatNeobrabotannyeKategoriiNovostey, removal of invalid records that can not be processed.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
" - УИННовости: %ТекущаяСтрока.УИННовости%';ka='ჩანაწერი წაშლილია ინფორმაციის რეგისტრიდან ახალიამბებისკატეგორიებიუბრალოდაუმუშავებლები გასაღებებით:"
" - ახალიამბებისკატეგორიასტრიქონი: %ТекущаяСтрока.КатегорияНовостейСтрока%"
" - ახალიამბებისკატეგორიებისმნიშვნელობასტრიქონი: %ТекущаяСтрока.ЗначениеКатегорииНовостейСтрока%"
" - შერჩევის პირობა: %ТекущаяСтрока.УсловиеОтбора%"
" - ახალიამბებისლენტი: %ТекущаяСтрока.ЛентаНовостей%"
" - ახალიამბისდუი: %ТекущаяСтрока.УИННовости%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. უბრალო კატეგორიების არასწორი ჩანაწერების წაშლა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Удаление неправильных записей простых категорий';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Removing invalid entries simple categories'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
" - ЛентаНовостей: %ТекущаяСтрока.ЛентаНовостей%"
" - УИННовости: %ТекущаяСтрока.УИННовости%';ka='ჩანაწერი წაშლილია ინფორმაციის რეგისტრიდან ახალიამბებისკატეგორიებიინტერვალებივერსიებიდაუმუშავებლები გასაღებებით:"
" - ახალიამბებისკატეგორიასტრიქონი: %ТекущаяСтрока.КатегорияНовостейСтрока%"
" - პროდუქტი: %ТекущаяСтрока.Продукт%"
" - ვერსიადან: %ТекущаяСтрока.ВерсияОТ%"
" - ვერსიამდე: %ТекущаяСтрока.ВерсияДО%"
" - ახალიამბებისლენტი: %ТекущаяСтрока.ЛентаНовостей%"
" - დუიახალიამბები: %ТекущаяСтрока.УИННовости%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТекущаяСтрока.КатегорияНовостейСтрока%", ТекущаяСтрока.КатегорияНовостейСтрока);
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. ვერსიების ინტერვალების არასწორი ჩანაწერების წაშლა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Удаление неправильных записей интервалов версий';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Removing invalid entries versions intervals'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 3/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 3/4';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Performing step 3/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, არასწორი ჩანაწერების წაშლა, რომელთა დამუშავება შეუძლებელია.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление неправильных записей, которые невозможно обработать.';en='PereschitatNeobrabotannyeKategoriiNovostey, removal of invalid records that can not be processed.'"), // Комментарий
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 4/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 4/4';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Performing step 4/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, ახალ ამბებზე მიუერთებელი ჩანაწერების წაშლა.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление записей, не привязанных к новостям.';en='PereschitatNeobrabotannyeKategoriiNovostey, delete records, not tied to the news.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება დასრულებულია';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение завершено';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. execution is completed'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, დამუშავება დასრულებულია.';ru='ПересчитатьНеобработанныеКатегорииНовостей, обработка окончена.';en='PereschitatNeobrabotannyeKategoriiNovostey, the processing is finished.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации

	Иначе
		ТекстСообщения = НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, у пользователя"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"недостаточно прав для выполнения операции обработка категорий новостей.';ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, მომხმარებელს"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"არასაკმარისი უფლებები აქვს ახალი ამბების კატეგორიების დამუშავების ოპერაციის შესასრულებლად.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%", ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ());
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Ошибка выполнения';en='News. Service and regulations. PereschitatNeobrabotannyeKategoriiNovostey. Runtime error'"), // ИдентификаторШага
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';ka='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისმიხედვითშერჩევებისგადათვლა_იშვიათადცვალებადი. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям_РедкоМеняющиеся. Ошибка выполнения';en='News. Service and regulations. PereschitatOtboryPoNovostyam_RedkoMenyayuschiesya. Runtime error'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
"';ka='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, не заполнены значения плана видов характеристик КатегорииНовостей:"
"КатегорияВерсияПродукта: %КатегорияВерсияПродукта%"
"КатегорияВерсияПлатформы: %КатегорияВерсияПлатформы%"
"Возможно, они еще не обновились с сервера при обновлении классификаторов, а также не были заполнены в макете СтандартныеЗначения плана видов характеристик КатегорииНовостей"
"'");
...
"Подробности:"
"%3';ka='Ошибка получения значения функциональной опции %1 для категории %2."
"Подробности:"
"%3'"),
						...
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
							НСтр("ka='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие';en='News. Service and regulations. PereschitatOtboryPoNovostyam. Are common'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
							Метаданные.Справочники.Новости, // ОбъектМетаданных
							, // Данные
							ТекстСообщения, // Комментарий
...
Процедура ПроверитьИзменениеВерсииПлатформы(РезультатВыполнения = "") Экспорт
	...
	Если МонопольныйРежим() Тогда
		...
		//  то будут ошибки при всех попытках записи данных в базу.
		// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме новостей.
		РезультатВыполнения = НСтр("ka='მონოპოლურ რეჟიმში პლატფორმის ვერსიის შემოწმება არ ხორციელდება';ru='В монопольном режиме проверка версии платформы не осуществляется';en='In single mode platform version check is carried out'");
		Возврат;
	КонецЕсли;
...
""
"Версия конфигурации в этой обработке не проверяется, а пересчитывается принудительно в модуле обновления конфигурации, а также после каждого получения новостей.';ka='პლატფორმის შენახული ვერსია: %СохраненнаяВерсияПлатформы%"
"პლატფორმის მიმდინარე ვერსია: %ТекущаяВерсияПлатформы%"
"%ТребуетсяОбновлениеРедкоменяющихсяОтборов%"
""
"ამ დამუშავებაში კონფიგურაციის ვერსიის შემოწმება არ ხდება, არამედ კონფიგურაციის განახლების მოდულში იძულებით გადაითვლება, ასევე, ახალი ამბების ყოველი მიღების შემდეგ.'");
	...
	УстановитьПривилегированныйРежим(Истина);
			...
			Если МенеджерЗаданийАктивен = Истина Тогда
				Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
					...
					МассивФоновыхЗаданий.Добавить(ФоновыеЗадания.Выполнить(
						...
						"", // Ключ
						СтрШаблон(
							НСтр("ka='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';ru='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';en='ObrabotkaNovostey.PereschitatOtboryPoNovostyam_RedkoMenyayuschiesya due to changes in platform version from% 1 to% 2'"),
							лкПоследняяВерсия,
							лкТекущаяВерсия)));
					// Последняя версия платформы сохранится в "ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся".
				КонецЕсли;
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ka='იშვიათად ცვალებადი შერჩევები გადაითვლება (გაშვებულია ფონური დავალება)';ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)';en='Rarely changing selections will be recalculated (running a background job)'"));
			Иначе
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ka='იშვიათად ცვალებადი შერჩევები გადაითვლება (აშკარა გაშვება ახალიამბებისმიხედვითშერჩევებისგადათვლა_იშვიათადცვალებადი()';ru='Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())';en='Rarely changing selections will be counted (a clear run PereschitatOtboryPoNovostyam_RedkoMenyayuschiesya ())'"));
		...
		Иначе
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%СохраненнаяВерсияПлатформы%", лкПоследняяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТекущаяВерсияПлатформы%", лкТекущаяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ka='იშვიათად ცვალებადი შერჩევები დარჩება როგორც არის';ru='Редко меняющиеся отборы останутся как есть';en='Rarely changes the selection will remain as is'"));
		КонецЕсли;
...
"Время начала (мс): %ТНачало%"
"';ka='Начало проверки лент новостей на актуальность"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისარააქტიურილენტებისძებნადადამუშავება. დასაწყისი';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Начало';en='News. Service and regulations. NaytiIObrabotatNeaktivnyeLentyNovostey. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ЛентаНовостейЧастотаОбновления = 0 Тогда
					лкЧастотаОбновления = НСтр("ka='მექანიკურად (0)';ru='вручную (0)';en='manually (0)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 1 Тогда
					лкЧастотаОбновления = НСтр("ka='один раз в день (1)';ru='один раз в день (1)';en='once a day (1)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 2 Тогда
					лкЧастотаОбновления = НСтр("ka='каждый час (2)';ru='каждый час (2)';en='every hour (2)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 3 Тогда
					лкЧастотаОбновления = НСтр("ka='каждые 15 минут (3)';ru='каждые 15 минут (3)';en='every 15 minutes (3)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 4 Тогда
					лкЧастотаОбновления = НСтр("ka='каждую минуту (4)';ru='каждую минуту (4)';en='every minute (4)'");
				Иначе
					лкЧастотаОбновления = СтрШаблон(
						НСтр("ka='არ არის განსაზღვრული (%1)';ru='неопределено (%1)';en='Undefined (1%)'"),
						Строка(Выборка.ЛентаНовостейЧастотаОбновления));
				КонецЕсли;

				РезультатВыполнения_ЛентаНовостей = НСтр("ka='ახალი ამბების ლენტი %ЛентаНовостей%, ახალი ამბების ლენტის განახლების სიხშირე: %ЧастотаОбновления%, უკანასკნელი წარმატებული ჩატვირთვის თარიღი: %ДатаЗагрузкиПоследнихНовостей%, წარუმატებლობა: %КоличествоНеудачПриЗагрузкеНовостей%, ჩატვირთვის ბოლო მცდელობა: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%';ru='Лента новостей %ЛентаНовостей%, частота обновления ленты новостей: %ЧастотаОбновления%, дата последней успешной загрузки: %ДатаЗагрузкиПоследнихНовостей%, неудач: %КоличествоНеудачПриЗагрузкеНовостей%, последняя попытка загрузки: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%';en='Newsline% LentaNovostey%, news feed update frequency:% ChastotaObnovleniya%, date of the last successful boot:% DataZagruzkiPoslednihNovostey% failure:% KolichestvoNeudachPriZagruzkeNovostey%, the latest attempt to download:% DataPosledneyPopytkiZagruzkiNovostey%. %Act%'");
				...
				РезультатВыполнения_ЛентаНовостей = СтрЗаменить(
					...
					?(Выборка.ДатаЗагрузкиПоследнихНовостей = '00010101',
						НСтр("ka='(არასდროს)';ru='(никогда)';en='(never)'"),
				...
				Если Выборка.ЛентаНовостейАктивна = Истина Тогда
					...
					Если СтруктураНовости.Ссылка.Пустая() Тогда
						...
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი აქტიურია, არააქტიურობის შესახებ ახალი ამბები არ არის';ru='Лента новостей активна, новости о неактивности нет';en='Newsline is active, there is no news of inactivity'"));
					ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
						...
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი აქტიურია, არააქტიურობის შესახებ ახალი ამბავი უკვე მონიშნულია წასაშლელად';ru='Лента новостей активна, новость о неактивности уже была помечена на удаление';en='Newsline is active, the news of inactivity has been tagged for deletion'"));
					Иначе
						// Пометить найденную новость на удаление.
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						НовостьОбъект.УстановитьПометкуУдаления(Истина);
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი აქტიურია, არააქტიურობის შესახებ ახალი ამბავი წასაშლელად მოინიშნება';ru='Лента новостей активна, новость о неактивности будет помечена на удаление';en='Newsline is active, the news of inactivity will be marked for deletion'"));
					КонецЕсли;
				Иначе
					// Найти новость и обновить. Если ее нет - создать и заполнить.
					Если СтруктураНовости.Ссылка.Пустая() Тогда
						НовостьОбъект = Справочники.Новости.СоздатьЭлемент();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი არააქტიურია, არააქტიურობის შესახებ ახალი ამბავი შეიქმნება';ru='Лента новостей НЕактивна, новость о неактивности будет создана';en='Newsline inactive, the news of inactivity will be created'"));
					Иначе
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი. არააქტიურია, სიახლე არააქტიურობის შესახებ განახლდება';ru='Лента новостей НЕактивна, новость о неактивности будет обновлена';en='Newsline inactive, the news of inactivity will be updated'"));
					КонецЕсли;
					...
					НовостьОбъект.Наименование                       = НСтр("ka='ახალი ამბების ლენტი არააქტიურია';ru='Лента новостей неактивна';en='Newsline inactive'");
					НовостьОбъект.Подзаголовок                       = НСтр("ka='ახალი ამბების ლენტი არააქტიურია. რეკომენდებულია ავტოგანახლებიდან მისი ამოღება';ru='Лента новостей неактивна. Рекомендуется исключить ее из автообновления';en='Newsline is inactive. It is recommended to exclude it from the auto-update'");
...
"Подробно:"
"%ИнформацияОбОшибке%"
"';ka='Произошла ошибка при создании / обновлении новости о неактивности ленты новостей %ЛентаНовостей%"
"Подробно:"
"%ИнформацияОбОшибке%"
"'");
							...
							ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
								НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისარააქტიურილენტებისძებნადადამუშავება. შეცდომა';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Ошибка';en='News. Service and regulations. NaytiIObrabotatNeaktivnyeLentyNovostey. Mistake'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения, // Комментарий
						...
						КонецПопытки;
					Иначе
						РезультатВыполнения_ЛентаНовостей = РезультатВыполнения_ЛентаНовостей + Символы.ПС + НСтр("ka='Отменено создание / изменение новости про отсутствие активности для ленты новостей';ru='Отменено создание / изменение новости про отсутствие активности для ленты новостей';en='Canceled create / edit news about the lack of activity for newsfeeds'");
					КонецЕсли;
				КонецЕсли;
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Окончание проверки лент новостей на актуальность."
"Лента новостей считается НЕактуальной, если по ней было более 10 неудачных попыток получения новостей и последняя удачная попытка была более суток назад."
""
"Результат выполнения:"
"%РезультатВыполнения%"
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისარააქტიურილენტებისძებნადადამუშავება. დასასრული';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Конец';en='News. Service and regulations. NaytiIObrabotatNeaktivnyeLentyNovostey. the end'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
"в параметре Использовать передано значение неправильного типа: %1';ka='ახალიამბებისდამუშავება.სარეგლამენტოდავალებებისგამოყენებისშეცვლა: პარამეტრ ""გამოყენება""-ში გადაცემულია არასწორი ტიპის მნიშვნელობა: %1'"),
				ТипЗнч(Использовать));
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სარეგლამენტოდავალებებისგამოყენებისცვლილება';ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';en='News. Service and regulations. IzmenenieIspolzovaniyaReglamentnyhZadany'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ka='წინასწარ განსაზღვრული სარეგლამენტო დავალება ახალიამბებისყველაგანახლება ვერ მოიძებნა.';ru='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.';en='We found predefined scheduled jobs VseObnovleniyaNovostey.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სარეგლამენტოდავალებებისგამოყენებისცვლილება';ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';en='News. Service and regulations. IzmenenieIspolzovaniyaReglamentnyhZadany'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
Процедура УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, РезультатВыполнения)
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Если (ДанныеДляУдаления.Свойство("НовостиДляУдаления"))
			...
			ТекстСообщения = НСтр("ka='არის წასაშლელად მონიშნული ახალი ამბები (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). მათი წაშლა უნდა მოხდეს ხელით, შემდეგნაირად, ყველა ფუნქცია / სტანდარტულები / მონიშნული ობიექტების წაშლა.';ru='Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.';en='There are marked for deletion news (% DannyeDlyaUdaleniya.NovostiDlyaUdaleniya.Kolichestvo ()% of news). They must be removed manually through all of the function / Accessories / Deleting marked objects.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='წასაშლელად მონიშნული ახალი ამბები არ არის';ru='Нет помеченных на удаление новостей';en='No marked for deletion news'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления"))
			...
			Для Каждого ТекущаяНовость Из ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ka='ყველა მოძველებული ახაი ამბავი წარმატებით არის წასაშლელად მონიშნული (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';ru='Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';en='All older news successfully marked for deletion (DannyeDlyaUdaleniya.NovostiDlyaUstanovkiPometkiUdaleniya.Kolichestvo% ()% of news).'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='წასაშლელად მოსანიშნი მოძველებული ახალი ამბები არ არის.';ru='Нет устаревших новостей, которые надо пометить на удаление.';en='No outdated news, which should mark removal.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ka='წაიშალა %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% დაუმუშავებელი უბრალო კატეგორია.';ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% необработанных простых категорий.';en='It was removed DannyeDlyaUdaleniya.KategoriiNovosteyProstyeNeobrabotannye.Kolichestvo% ()% of untreated simple categories.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='უბრალო დაუმუშავებელი კატეგორიები ვერ მოიძებნა.';ru='Простых необработанных категорий не было найдено.';en='Simple rough categories were found.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ka='წაიშალა %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% ვერსიის ინტერვალების დაუმუშავებელი კატეგორია.';ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% необработанных категорий интервалов версий.';en='It was removed DannyeDlyaUdaleniya.KategoriiNovosteyIntervalyVersiyNeobrabotannye.Kolichestvo% ()% of untreated categories versions intervals.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='ვერსიის ინტერვალების დაუმუშავებელი კატეგორიები ვერ მოიძებნა.';ru='Необработанных категорий интервалов версий не было найдено.';en='Rough categories intervals versions have been found.'");
		КонецЕсли;
...
"Время начала (мс): %ТНачало%"
"';ka='Начало загрузки всех стандартных значений"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტული მნიშვნელობების ჩატვირთვა. ყველა სტანდარტული მნიშვნელობა. დასაწყისი';ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения. Начало';en='News. Service and regulations. Loading standard values. All standard values. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Окончание загрузки всех стандартных значений"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент';en='BIP: Novosti.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტული მნიშვნელობების ჩატვირთვა. ყველა სტანდარტული მნიშვნელობა. დასასრული';ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения. Конец';en='News. Service and regulations. Loading standard values. All standard values. the end'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ka='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';ru='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';en='It starts at the first start, in general data. Installation constants loading default values, resets registers information (shared data).'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ka='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';ru='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';en='It starts at the first start, for each data field. Setting user preferences (divided data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';ru='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';en='Runs during the transition to the new version, in the general data. Installation constants loading default values, resets registers information (shared data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';en='Runs during the transition to any new version for each data field. Setting user preferences (divided data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';ru='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';en='Runs during the transition to the new version for each data field. Setting user preferences (divided data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
"Время начала (мс): %ТНачало%"
"';ka='Начало процедуры обновления. Первый запуск (общие данные)"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск. Начало';en='News. Updating of information security. Total information. First start. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"%Результат%';ka='აღმოჩენილია ქვესისტემა მომხმარებლებისინტერნეტმხარდაჭერა.ახალიამბები-ს დანერგვის შეცდომები."
"დანერგვის აღმოჩენილი შეცდომები:"
"%Результат%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Результат%", Результат);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
					НСтр("ru='Новости. Обновление ИБ. Проверка внедрения';en='News. Updating of information security. Check implementation'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Время начала (мс): %ТНачало%"
"';ka='Начало процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%"
"Время начала (мс): %ТНачало%"
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Начало';en='News. Updating of information security. Total information. Overrides. First start. Start'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Конец';en='News. Updating of information security. Total information. Overrides. First start. the end'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='ქვესისტემა მომხმარებლებისინტერნეტმხარდაჭერა.ახალი ამბები არ არსებობს. პირველი გაშვების დამუშავება არ შესრულებულა.';ru='Подсистема ИнтернетПоддержкаПользователей.Новости не существует. Обработка первого запуска не выполнена.';en='InternetPodderzhkaPolzovateley.Novosti subsystem does not exist. Processing the first run is not performed.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск. Конец';en='News. Updating of information security. Total information. First start. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Конец процедуры обновления. Первый запуск (общие данные)"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
...
"Время начала (мс): %ТНачало%"
"';ka='Начало процедуры обновления. Первый запуск (область данных)"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Начало';en='News. Updating of information security. OblastDannyh. First start. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ТекстСообщения = НСтр("ka='ახალ ამბებთან მუშაობას გარე მომხმარებლისთვის მხარდაჭერა არ გააჩნია. განახლების დამამუშავებელი გამოტოვებულია.';ru='Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.';en='Work with news is not supported to an external user. Processor upgrade skipped.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец';en='News. Updating of information security. OblastDannyh. First start. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Начало';en='News. Updating of information security. OblastDannyh. Overrides. First start. Start'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Конец';en='News. Updating of information security. OblastDannyh. Overrides. First start. the end'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Конец процедуры обновления. Первый запуск (область данных)"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
...
"Время начала (мс): %ТНачало%"
"';ka='Начало процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (общие данные)"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%", ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки());
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию. Начало';en='News. Updating of information security. Total information. PereytiNaVersiyu. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Начало';en='News. Updating of information security. Total information. Overrides. PereytiNaVersiyu. Start'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Конец';en='News. Updating of information security. Total information. Overrides. PereytiNaVersiyu. the end'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='ქვესისტემა მომხმარებლებისინტერნეტმხარდაჭერა.ახალი ამბები არ არსებობს. განახლების დამუშავება არ შესრულებულა.';ru='Подсистема ИнтернетПоддержкаПользователей.Новости не существует. Обработка обновления не выполнена.';en='InternetPodderzhkaPolzovateley.Novosti subsystem does not exist. updates are not processing is performed.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию. Конец';en='News. Updating of information security. Total information. PereytiNaVersiyu. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (общие данные)"
"Разрешена работа с новостями = %лкРазрешенаРаботаСНовостями%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Начало';en='News. Updating of information security. Total information. PereytiNaVersiyu_HHH. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало';en='News. Updating of information security. Total information. Overrides. PereytiNaVersiyu_HHH. Start'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец';en='News. Updating of information security. Total information. Overrides. PereytiNaVersiyu_HHH. the end'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Конец';en='News. Updating of information security. Total information. PereytiNaVersiyu_HHH. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"Время начала (мс): %ТНачало%"
"';ka='Начало процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область данных)"
"Время начала (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%", ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки());
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию. Начало';en='News. Updating of information security. OblastDannyh. PereytiNaVersiyu. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию. Конец';en='News. Updating of information security. OblastDannyh. PereytiNaVersiyu. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Начало';en='News. Updating of information security. OblastDannyh. Overrides. PereytiNaVersiyu. Start'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';ka='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область данных)"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Конец';en='News. Updating of information security. OblastDannyh. Overrides. PereytiNaVersiyu. the end'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Начало';en='News. Updating of information security. OblastDannyh. PereytiNaVersiyu_HHH. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Конец';en='News. Updating of information security. OblastDannyh. PereytiNaVersiyu_HHH. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало';en='News. Updating of information security. OblastDannyh. Overrides. PereytiNaVersiyu_HHH. Start'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: Novosti.Obnovlenie IB'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец';en='News. Updating of information security. OblastDannyh. Overrides. PereytiNaVersiyu_HHH. the end'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
...
"%1"
""
"';ka='Ошибки настройки общего реквизита ОбластьДанныхОсновныеДанные:"
"%1"
""
"'"),
				РезультатЛокальный);
...
"%1"
""
"';ka='Ошибки настройки общего реквизита ОбластьДанныхВспомогательныеДанные:"
"%1"
""
"'"),
				РезультатЛокальный);
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='უფლებების გამართვის შეცდომები როლ სისტემისადმინისტრატორი-სთვის :';ru='Ошибки настройки прав для роли АдминистраторСистемы:';en='Rights setting error for the role AdministratorSistemy:'")
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='როლ სრულიუფლებები-სთვის უფლებების გამართვის შეცდომები:';ru='Ошибки настройки прав для роли ПолныеПрава:';en='rights setting error for the role PolnyePrava:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"';ka='Не настроен идентификатор конфигурации."
"Для его настройки необходимо внести изменения в общий модуль"
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
"для строки условия ProductName."
"В противном случае новости для этой конфигурации приходить НЕ будут."
...
"';ka='Неправильно настроен идентификатор конфигурации."
"Для его настройки необходимо внести изменения в общий модуль"
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
"для строки условия ProductName."
"Идентификатор конфигурации должен состоять из английских букв, цифр, знака минуса и знака подчеркивания."
"'");
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='კონფიგურაციის კოდის გამართვის შეცდომები:';ru='Ошибки настройки кода конфигурации:';en='settings configuration error code:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
"для строки условия ProductVersion.';ka='კონფიგურაციის ვერსია არასწორად არის გამართული (რიცხვების რაოდენობა 4-სგან განსხვავდება)."
"მეტამონაცემებში, ველ ""ვერსია""-ში უნდა იყოს ა.ბ.გ.დ ტიპის (წერტილით გაყოფილი ოთხი რიცხვი) მონაცემები."
"თუ რაიმე მიზეზით მეტამონაცემებში ვერსია მიცემულია ა.ბ.გ.დ-სგან განსხვავებულ ფორმატში, მაშინ აუცილებელია ცვლილებების შეტანა საერთო მოდულში ახალიამბებისდამუშავებახელახლაგანსაზღვრებადი.წინასწარგანსაზღვრულიკატეგორიისმნიშვნელობისმიღება, პირობა ProductVersion-ის სტრიქონისთვის.'")
	Иначе
...
"Список неправильных символов: %НайденныеНеправильныеСимволы%';ka='კონფიგურაციის ვერსია არასწორად არის გამართული (არის არასწორი სიმბოლოები)."
"მეტამონაცემებში, ველ ვერსია-ში უნდა იყოს ა.ბ.გ.დ სახეობის მონაცემები (წერტილით გაყოფილი ოთხი რიცხვი)."
"თუ რაიმე მიზეზის გამო, მეტამონაცემებში ვერსია მიცემულია ა.ბ.გ.დ-სგან განსხვავებულ ფორმატში, მაშინ საჭიროა ცვლილებების შეტანა საერთო მოდულში ახალიამბებისდამუშავებახელახლაგანსაზღვრებადი.წინასწარგანსაზღვრულიკატეგორიისმნიშვნელობისმიღება, პირობა ProductVersion-ის სტრიქონისთვის."
"არასწორი სიმბოლოების სია: %НайденныеНеправильныеСимволы%'");
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='Ошибки настройки версии конфигурации:';ru='Ошибки настройки версии конфигурации:';en='configuration settings version error:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"Также допустимо, что у %ПолноеИмяМетаданных% установлено использование Авто, но тогда у общего реквизита %ИмяОбщегоРеквизита% должно быть установлено Автоиспользование в %ПравильноеЗначениеИспользования%.';ka='გამოყენება არასწორად არის დაყენებული %ПолноеИмяМетаданных%-ისთვის საერთო რეკვიზიტ %ИмяОбщегоРеквизита%-ისთვის. დაყენებულია: %НайденныйЭлемент.Использование%, მოსალოდნელია: %ПравильноеЗначениеИспользования%."
"ასევე დასაშვებია, რომ Также допустимо, что у %ПолноеИмяМетаданных%-ზე დაყენებულია გამოყენება ""ავტო"", მაგრამ მაშინ საერთო რეკვიზიტ %ИмяОбщегоРеквизита%-ს დაყენებული უნდა ჰქონდეს ""ავტოგამოყენება"" %ПравильноеЗначениеИспользования%-ში.'");
		...
		КонецЕсли;
	Иначе
		ТекущийРезультат = НСтр("ka='მეტამონაცემთა მოთხოვნილი ობიექტი ვერ მოიძებნა: %ПолноеИмяМетаданных%';ru='Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%';en='Unable to find the required metadata object:% PolnoeImyaMetadannyh%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("ka='როლ %Роль%-ში %ПолноеИмяМетаданных%-ისთვის უფლება %Право% არასწორად არის დაყენებული. დაყენებულია: %ТекущееЗначениеПрава%, მოსალოდნელია: %ПравильноеЗначениеПрава%';ru='Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%';en='Improperly installed right% Right% as a% role% to% PolnoeImyaMetadannyh%. Installs:% TekuscheeZnacheniePrava% expected:% PravilnoeZnacheniePrava%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
