...
"'");
	...
	Если (ТипЗнч(МассивПользователей) <> ТипМассив)
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%МассивПользователей%", НСтр("ka='ყველას მიხედვით';ru='По всем'"));
	Иначе
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтруктураОтборов%", НСтр("ka='Передано значения типа, отличного от Структуры';ru='Передано значения типа, отличного от Структуры'"));
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначенияСостояний%", НСтр("ka='Передано значения типа, отличного от Структуры';ru='Передано значения типа, отличного от Структуры'"));
	КонецЕсли;
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისმდგომარეობისდაყენებამომხმარებლებისთვის';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей'"), // ИдентификаторШага
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:Новости.Разное';ru='БИП:Новости.Разное'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სხვადასხვა. მომხმარებლებთან მუშაობის შეცდომა';ru='Новости. Разное. Ошибка работы с пользователями'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"news.webits.1c.ru",
		443,
		НСтр("ka='ახალი ამბების მიღების ვებ-სერვისები';ru='Веб-сервисы получения новостей'"));
...
"პორტის დადგენის შეცდომა სერვერის სახელიდან %3 (%4).'"),
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ჩატვირთვა. ახალი ამბების სერვერთან შეერთება. პარამეტრების დადგენის შეცდომა';ru='Новости. Загрузка новостей. Соединение с сервером новостей. Ошибка определения параметров'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"პორტი        = %5'"),
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. ახალი ამბების ჩატვირთვა. ახალი ამბების სერვერთან შეერთება';ru='Новости. Загрузка новостей. Соединение с сервером новостей'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"%5'"),
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. ახალი ამბების ჩატვირთვა. ახალი ამბების სერვერთან შეერთება. შეერთების შეცდომა';ru='Новости. Загрузка новостей. Соединение с сервером новостей. Ошибка соединения'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
#Область ЗаполнениеШаблонов_Новости_КатегорииНовостейИнтервалыВерсий
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					Символы.Таб + НСтр("ka='Набор записей пустой.';ru='Набор записей пустой.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_РассчитанныеОтборыПоНовостям_Пользовательские
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					НСтр("ka='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.';ru='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_ПоставляемыеДанные
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ПоставляемыеДанные%", Символы.Таб + НСтр("ka='Не найдено в поставляемых данных.';ru='Не найдено в поставляемых данных.'"));
			КонецЕсли;
...
#КонецОбласти
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОНовостях%", НСтр("ka='Не найдено ни одной новости с таким UIN';ru='Не найдено ни одной новости с таким UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ЛентыНовостей
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОЛентахНовостей%", НСтр("ka='Не найдено ни одной ленты новостей для новости с таким UIN';ru='Не найдено ни одной ленты новостей для новости с таким UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	Иначе
		ПредставлениеНастройкиНовостей = СтрШаблон(НСтр("ka='Настройки новостей необрабатываемого типа %1';ru='Настройки новостей необрабатываемого типа %1'"), ТипЗнч(НастройкиНовостей));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ПараметрыСеанса
	...
	Иначе
		ПредставлениеПараметрыОкруженияБИП_Новости = СтрШаблон(НСтр("ka='Параметры окружения подсистемы новостей необрабатываемого типа %1';ru='Параметры окружения подсистемы новостей необрабатываемого типа %1'"), ТипЗнч(ПараметрыОкруженияБИП_Новости));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	Если РегламентноеЗадание = Неопределено Тогда
		ПредставлениеРасписания = НСтр("ka='Регламентное задание не найдено';ru='Регламентное задание не найдено'");
		РегламентноеЗадание_Использование = Ложь;
		ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ka='Никогда не выполнялось';ru='Никогда не выполнялось'");
	Иначе
		...
		Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание) = ТипФоновоеЗадание Тогда
			...
			Иначе
				РегламентноеЗадание_СтруктураПоследнегоЗадания.Вставить("ПодробноеПредставлениеОшибки", НСтр("ka='Выполнено без ошибок';ru='Выполнено без ошибок'"));
			КонецЕсли;
		...
		Иначе
			ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ka='არ იყო შესრულებული';ru='Не выполнялось'");
		КонецЕсли;
	КонецЕсли;
...
Функция НайтиНовости(СтруктураПараметровПоиска) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		Если НЕ ПустаяСтрока(лкСтрокаПоиска) Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = НСтр("ka='ახალიამბებისდამუშავება.ახალიამბებისძებნა: ახალი ამბების ძებნის შეცდომა მოხდა, ძებნის სტრიქონი [%СтрокаПоиска%] შემდეგი მიზეზით: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%';ru='ОбработкаНовостей.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%СтрокаПоиска%] по причине: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:Новости.Разное';ru='БИП:Новости.Разное'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სხვადასხვა. სრულტექსტიანი ძებნის შეცდომა';ru='Новости. Разное. Ошибка полнотекстового поиска'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. ახალი ამბების ყველა გამართვა. დასაწყისი';ru='Новости. Все обновления новостей. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. დასასრული';ru='Новости. Все обновления новостей. Конец'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			НСтр("ka='სარეგლამენტო დავალების შესრულების იძულებითი დასრულება: საინფორმაციო ბაზა მონოპოლიურ რეჟიმში მუშაობს.';ru='Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		ЕстьОшибки = Истина;
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ლენტის მიხედვით ახალი ამბების მიღება და დამუშავება. დასაწყისი';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Начало'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ლენტების მიხედვით ახალი ამბების მიღება და დამუშავება. დასასრული';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ლენტებით ახალი ამბების მიღება და დამუშავება. დასამუშავებლად ახალი ამბების ლენტების არარსებობა';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Отсутствие лент новостей для обработки'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ka='სარეგლამენტო განახლებისთვის ახალი ამბების ლენტები არ არის';ru='Нет лент новостей для регламентного обновления'"), // Комментарий
					ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='1. ახალი ამბების ლენტებიდან ახალი ამბების მიღება და დამუშავება (განზომილების სავალდებულო შევსებით ახალიამბებისლენტი)';ru='1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)'"),
					ТШаг,
...
"'");
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
						НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. კლასიფიკატორების განახლება. დასაწყისი';ru='Новости. Все обновления новостей. Обновление классификаторов. Начало'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
"'");
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
						НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. კლასიფიკატორების განახლება. დასასრული';ru='Новости. Все обновления новостей. Обновление классификаторов. Конец'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						Т1Длительность, // Данные
						ТекстСообщения, // Комментарий
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. კლასიფიკატორების განახლება. ინტერნეტით მუშაობა გამორთულია';ru='Новости. Все обновления новостей. Обновление классификаторов. Отключена работа через интернет'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ka='ინტერნეტით კლასიფიკატორების განახლება გამორთულია';ru='Обновление классификаторов через интернет отключено'"), // Комментарий
					ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='2. კლასიფიკატორების განახლება';ru='2. Обновление классификаторов'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ავტოწაშლა. დასაწყისი';ru='Новости. Все обновления новостей. Автоудаление новостей. Начало'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. ახალი ამბების ავტოწაშლა. დასასრული';ru='Новости. Все обновления новостей. Автоудаление новостей. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='3. ახალი ამბების ავტოწაშლა';ru='3. Автоудаление новостей'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. პერიოდული თვისებების განახლება. დასაწყისი';ru='Новости. Все обновления новостей. Обновление периодических свойств. Начало'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა გამართვა. პერიოდული თვისებების განახლება. დასასრული';ru='Новости. Все обновления новостей. Обновление периодических свойств. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='4. პერიოდული თვისებების განახლება (მნიშვნელოვნება, აქტუალურობა)';ru='4. Обновление периодических свойств (важность, актуальность)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. მეტამონაცემებზე მიბმულების განახლება. დასაწყისი';ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Начало'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. მეტამონაცემებზე მიბმულების განახლება. დასასრული';ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='5. მეტამონაცემთა მიბმულების განახლება (+ მნიშვნელოვნება)';ru='5. Обновление привязок к метаданным (+ важность)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. პლატფორმის ვერსიის შემოწმება. დასაწყისი';ru='Новости. Все обновления новостей. Проверка версии платформы. Начало'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.ახალი ამბების ყველა განახლება';ru='БИП:Новости.Все обновления новостей'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. ახალი ამბების ყველა განახლება. პლატფორმის ვერსიის შემოწმება. დასასრული';ru='Новости. Все обновления новостей. Проверка версии платформы. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ka='6. პლატფორმის ვერსიის შემოწმება';ru='6. Проверка версии платформы'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისწაშლა_ყოველდღიურად. წაშლის დასაწყისი';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. Начало удаления'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"ახალიამბებისკატეგორიებიინტერვალებივერსიებიდაუმუშავებლები: %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისწაშლა_ყოველდღიურად. წასაშლელად მონაცემთა შეკრების შემდეგ';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		ТекстСообщения = НСтр("ka='წაშლის შედეგი: %РезультатВыполнения%';ru='Результат удаления: %РезультатВыполнения%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისწაშლა_ყოველდღიურად. წაშლის დასრულება';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. Окончание удаления'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისპერიოდულითვისებებისგანახლება. დასაწყისი';ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей. Начало'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		Если НЕ Результат.Пустой() Тогда
			...
			РезультатВыполнения = НСтр("ka='პერიოდული თვისებების გასაახლებლად მოიძებნა %ВсегоСтрок% ახალი ამბავი';ru='Найдено %ВсегоСтрок% новостей для обновления периодических свойств'");
			...
			КонецЦикла;
		Иначе
			РезультатВыполнения = НСтр("ka='გასაახლებლად მონაცემები ვერ მოიძებნა';ru='Не найдено данных для обновления'");
		КонецЕсли;
		...
		ТекстСообщения = НСтр("ka='პერიოდული რეკვიზიტების დამუშავების შედეგი: %РезультатВыполнения%';ru='Результат обработки периодических реквизитов: %РезультатВыполнения%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისპერიოდულითვისებებისგანახლება. დასასრული';ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей. Конец'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. მეტამონაცემებზემიბმულებისგანახლება. დასაწყისი';ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным. Начало'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		Если НЕ Результат.Пустой() Тогда
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			ВсегоСтрок = Выборка.Количество();
			РезультатВыполнения = НСтр("ka='მოიძებნა %ВсегоСтрок% ახალი ამბავი მეტამონაცემთა მიერთებულების გასაახლებლად';ru='Найдено %ВсегоСтрок% новостей для обновления привязок к метаданным'");
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ВсегоСтрок%", ВсегоСтрок);
			Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ka='მეტამონაცემებზე მიბმულების დამუშავების შედეგი: %РезультатВыполнения%';ru='Результат обработки привязок к метаданным: %РезультатВыполнения%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. მეტამონაცემებზემიბმულებისგანახლება. დასასრული';ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным. Конец'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Ошибка выполнения'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
	...
	// Запись в журнал регистрации
	Т0Начало = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ИдентификаторШага = НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. დასაწყისი';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Начало'");
...
"'");
	...
	Т0Конец = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Т0Длительность = Т0Конец - Т0Начало;
	ИдентификаторШага = НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიმაბებისლენტებისმიხედვითახალიამბებისმიღებადადამუშავება. დასასრული';ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Конец'");
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტულისიებისგანახლებაახალიამბებისთვის_ყოველდღიურად. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Ошибка выполнения'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები, სერვისი და რეგლამენტი. ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად. დასაწყისი';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Начало'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად. დასასრული';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისთვისსტანდარტულისიებისგანახლება_ყოველდღიურად. შესრულების გაფრთხილება';ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Предупреждение выполнения'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Ошибка выполнения'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
	...
	Если (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейПростые))
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 1/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 1/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, ახალი ამბების კატეგორიების გადავსების დამუშავების შესრულების დასაწყისი.';ru='ПересчитатьНеобработанныеКатегорииНовостей, начало выполнения обработки перезаполнения категорий новостей.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 2/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 2/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, არასწორი ჩანაწერების წაშლა, რომელთა დამუშავება შეუძლებელია.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление неправильных записей, которые невозможно обработать.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
" - ახალიამბისდუი: %ТекущаяСтрока.УИННовости%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. უბრალო კატეგორიების არასწორი ჩანაწერების წაშლა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Удаление неправильных записей простых категорий'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
" - დუიახალიამბები: %ТекущаяСтрока.УИННовости%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
					НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. ვერსიების ინტერვალების არასწორი ჩანაწერების წაშლა';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Удаление неправильных записей интервалов версий'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 3/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 3/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, არასწორი ჩანაწერების წაშლა, რომელთა დამუშავება შეუძლებელია.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление неправильных записей, которые невозможно обработать.'"), // Комментарий
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება, ნაბიჯი 4/4';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 4/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, ახალ ამბებზე მიუერთებელი ჩანაწერების წაშლა.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление записей, не привязанных к новостям.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა. შესრულება დასრულებულია';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение завершено'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ka='ახალიამბებისდაუმუშავებელიკატეგორიებისგადათვლა, დამუშავება დასრულებულია.';ru='ПересчитатьНеобработанныеКатегорииНовостей, обработка окончена.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისმიხედვითშერჩევებისგადათვლა_იშვიათადცვალებადი. შესრულების შეცდომა';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям_РедкоМеняющиеся. Ошибка выполнения'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
"%3'"),
						...
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
							НСтр("ka='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
							Метаданные.Справочники.Новости, // ОбъектМетаданных
							, // Данные
							ТекстСообщения, // Комментарий
...
Процедура ПроверитьИзменениеВерсииПлатформы(РезультатВыполнения = "") Экспорт
	...
	Если МонопольныйРежим() Тогда
		...
		//  то будут ошибки при всех попытках записи данных в базу.
		// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме новостей.
		РезультатВыполнения = НСтр("ka='მონოპოლურ რეჟიმში პლატფორმის ვერსიის შემოწმება არ ხორციელდება';ru='В монопольном режиме проверка версии платформы не осуществляется'");
		Возврат;
	КонецЕсли;
...
"ამ დამუშავებაში კონფიგურაციის ვერსიის შემოწმება არ ხდება, არამედ კონფიგურაციის განახლების მოდულში იძულებით გადაითვლება, ასევე, ახალი ამბების ყოველი მიღების შემდეგ.'");
	...
	УстановитьПривилегированныйРежим(Истина);
			...
			Если МенеджерЗаданийАктивен = Истина Тогда
				Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
					...
					МассивФоновыхЗаданий.Добавить(ФоновыеЗадания.Выполнить(
						...
						"", // Ключ
						СтрШаблон(
							НСтр("ka='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';ru='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2'"),
							лкПоследняяВерсия,
							лкТекущаяВерсия)));
					// Последняя версия платформы сохранится в "ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся".
				КонецЕсли;
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ka='იშვიათად ცვალებადი შერჩევები გადაითვლება (გაშვებულია ფონური დავალება)';ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)'"));
			Иначе
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ka='იშვიათად ცვალებადი შერჩევები გადაითვლება (აშკარა გაშვება ახალიამბებისმიხედვითშერჩევებისგადათვლა_იშვიათადცვალებადი()';ru='Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
		...
		Иначе
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%СохраненнаяВерсияПлатформы%", лкПоследняяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТекущаяВерсияПлатформы%", лкТекущаяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ka='იშვიათად ცვალებადი შერჩევები დარჩება როგორც არის';ru='Редко меняющиеся отборы останутся как есть'"));
		КонецЕсли;
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისარააქტიურილენტებისძებნადადამუშავება. დასაწყისი';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ЛентаНовостейЧастотаОбновления = 0 Тогда
					лкЧастотаОбновления = НСтр("ka='მექანიკურად (0)';ru='вручную (0)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 1 Тогда
					лкЧастотаОбновления = НСтр("ka='один раз в день (1)';ru='один раз в день (1)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 2 Тогда
					лкЧастотаОбновления = НСтр("ka='каждый час (2)';ru='каждый час (2)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 3 Тогда
					лкЧастотаОбновления = НСтр("ka='каждые 15 минут (3)';ru='каждые 15 минут (3)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 4 Тогда
					лкЧастотаОбновления = НСтр("ka='каждую минуту (4)';ru='каждую минуту (4)'");
				Иначе
					лкЧастотаОбновления = СтрШаблон(
						НСтр("ka='არ არის განსაზღვრული (%1)';ru='неопределено (%1)'"),
				...
				Если Выборка.ЛентаНовостейАктивна = Истина Тогда
					...
					Если СтруктураНовости.Ссылка.Пустая() Тогда
						...
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი აქტიურია, არააქტიურობის შესახებ ახალი ამბები არ არის';ru='Лента новостей активна, новости о неактивности нет'"));
					ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
						...
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი აქტიურია, არააქტიურობის შესახებ ახალი ამბავი უკვე მონიშნულია წასაშლელად';ru='Лента новостей активна, новость о неактивности уже была помечена на удаление'"));
					Иначе
						// Пометить найденную новость на удаление.
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						НовостьОбъект.УстановитьПометкуУдаления(Истина);
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი აქტიურია, არააქტიურობის შესახებ ახალი ამბავი წასაშლელად მოინიშნება';ru='Лента новостей активна, новость о неактивности будет помечена на удаление'"));
					КонецЕсли;
				Иначе
					// Найти новость и обновить. Если ее нет - создать и заполнить.
					Если СтруктураНовости.Ссылка.Пустая() Тогда
						НовостьОбъект = Справочники.Новости.СоздатьЭлемент();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი არააქტიურია, არააქტიურობის შესახებ ახალი ამბავი შეიქმნება';ru='Лента новостей НЕактивна, новость о неактивности будет создана'"));
					Иначе
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ka='ახალი ამბების ლენტი. არააქტიურია, სიახლე არააქტიურობის შესახებ განახლდება';ru='Лента новостей НЕактивна, новость о неактивности будет обновлена'"));
					КонецЕсли;
					...
					НовостьОбъект.Наименование                       = НСтр("ka='ახალი ამბების ლენტი არააქტიურია';ru='Лента новостей неактивна'");
					НовостьОбъект.Подзаголовок                       = НСтр("ka='ახალი ამბების ლენტი არააქტიურია. რეკომენდებულია ავტოგანახლებიდან მისი ამოღება';ru='Лента новостей неактивна. Рекомендуется исключить ее из автообновления'");
...
"'");
							...
							ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
								НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისარააქტიურილენტებისძებნადადამუშავება. შეცდომა';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Ошибка'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения, // Комментарий
						...
						КонецПопытки;
					Иначе
						РезультатВыполнения_ЛентаНовостей = РезультатВыполнения_ЛентаНовостей + Символы.ПС + НСтр("ka='Отменено создание / изменение новости про отсутствие активности для ленты новостей';ru='Отменено создание / изменение новости про отсутствие активности для ленты новостей'");
					КонецЕсли;
				КонецЕсли;
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. ახალიამბებისარააქტიურილენტებისძებნადადამუშავება. დასასრული';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
"в параметре Использовать передано значение неправильного типа: %1';ka='ახალიამბებისდამუშავება.სარეგლამენტოდავალებებისგამოყენებისშეცვლა: პარამეტრ ""გამოყენება""-ში გადაცემულია არასწორი ტიპის მნიშვნელობა: %1'"),
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
				НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სარეგლამენტოდავალებებისგამოყენებისცვლილება';ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ka='წინასწარ განსაზღვრული სარეგლამენტო დავალება ახალიამბებისყველაგანახლება ვერ მოიძებნა.';ru='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სარეგლამენტოდავალებებისგამოყენებისცვლილება';ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
Процедура УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, РезультатВыполнения)
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Если (ДанныеДляУдаления.Свойство("НовостиДляУдаления"))
			...
			ТекстСообщения = НСтр("ka='არის წასაშლელად მონიშნული ახალი ამბები (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). მათი წაშლა უნდა მოხდეს ხელით, შემდეგნაირად, ყველა ფუნქცია / სტანდარტულები / მონიშნული ობიექტების წაშლა.';ru='Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='წასაშლელად მონიშნული ახალი ამბები არ არის';ru='Нет помеченных на удаление новостей'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления"))
			...
			Для Каждого ТекущаяНовость Из ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ka='ყველა მოძველებული ახაი ამბავი წარმატებით არის წასაშლელად მონიშნული (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';ru='Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='წასაშლელად მოსანიშნი მოძველებული ახალი ამბები არ არის.';ru='Нет устаревших новостей, которые надо пометить на удаление.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ka='წაიშალა %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% დაუმუშავებელი უბრალო კატეგორია.';ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% необработанных простых категорий.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='უბრალო დაუმუშავებელი კატეგორიები ვერ მოიძებნა.';ru='Простых необработанных категорий не было найдено.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ka='წაიშალა %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% ვერსიის ინტერვალების დაუმუშავებელი კატეგორია.';ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% необработанных категорий интервалов версий.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ka='ვერსიის ინტერვალების დაუმუშავებელი კატეგორიები ვერ მოიძებნა.';ru='Необработанных категорий интервалов версий не было найдено.'");
		КонецЕсли;
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტული მნიშვნელობების ჩატვირთვა. ყველა სტანდარტული მნიშვნელობა. დასაწყისი';ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ახალი ამბები.სერვისი და რეგლამენტი';ru='БИП:Новости.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტული მნიშვნელობების ჩატვირთვა. ყველა სტანდარტული მნიშვნელობა. დასასრული';ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ka='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';ru='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ka='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';ru='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';ru='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
...
"'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='ქვესისტემა მომხმარებლებისინტერნეტმხარდაჭერა.ახალი ამბები არ არსებობს. პირველი გაშვების დამუშავება არ შესრულებულა.';ru='Подсистема ИнтернетПоддержкаПользователей.Новости не существует. Обработка первого запуска не выполнена.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
...
"'");
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ТекстСообщения = НСтр("ka='ახალ ამბებთან მუშაობას გარე მომხმარებლისთვის მხარდაჭერა არ გააჩნია. განახლების დამამუშავებელი გამოტოვებულია.';ru='Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
...
"'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='ქვესისტემა მომხმარებლებისინტერნეტმხარდაჭერა.ახალი ამბები არ არსებობს. განახლების დამუშავება არ შესრულებულა.';ru='Подсистема ИнтернетПоддержкаПользователей.Новости не существует. Обработка обновления не выполнена.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. განახლება ИБ';ru='БИП:Новости.Обновление ИБ'"), // ИмяСобытия
...
"'"),
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='უფლებების გამართვის შეცდომები როლ სისტემისადმინისტრატორი-სთვის :';ru='Ошибки настройки прав для роли АдминистраторСистемы:'")
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='როლ სრულიუფლებები-სთვის უფლებების გამართვის შეცდომები:';ru='Ошибки настройки прав для роли ПолныеПрава:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"'");
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='კონფიგურაციის კოდის გამართვის შეცდომები:';ru='Ошибки настройки кода конфигурации:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"არასწორი სიმბოლოების სია: %НайденныеНеправильныеСимволы%'");
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ka='Ошибки настройки версии конфигурации:';ru='Ошибки настройки версии конфигурации:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"ასევე დასაშვებია, რომ Также допустимо, что у %ПолноеИмяМетаданных%-ზე დაყენებულია გამოყენება ""ავტო"", მაგრამ მაშინ საერთო რეკვიზიტ %ИмяОбщегоРеквизита%-ს დაყენებული უნდა ჰქონდეს ""ავტოგამოყენება"" %ПравильноеЗначениеИспользования%-ში.'");
		...
		КонецЕсли;
	Иначе
		ТекущийРезультат = НСтр("ka='მეტამონაცემთა მოთხოვნილი ობიექტი ვერ მოიძებნა: %ПолноеИмяМетаданных%';ru='Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("ka='როლ %Роль%-ში %ПолноеИмяМетаданных%-ისთვის უფლება %Право% არასწორად არის დაყენებული. დაყენებულია: %ТекущееЗначениеПрава%, მოსალოდნელია: %ПравильноеЗначениеПрава%';ru='Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
