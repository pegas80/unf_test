...
"ВыгрузкаЗагрузкаУзловПлановОбменов.ПередВыгрузкойОбъекта()!';ka='მეტამონაცემთა ობიექტი %1 არ შეიძლება დამუშავდეს შემდეგი დამამუშავებლით გაცვლისგეგმებისკვანძებისგადმოტვირთვაჩატვირთვა.ობიექტისგადმოტვირთვამდე()!'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
Процедура ПередЗагрузкойОбъекта(Контейнер, Объект, Артефакты, Отказ) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком"
"ВыгрузкаЗагрузкаУзловПлановОбменов.ПередЗагрузкойОбъекта()!';ka='მეტამონაცემთა ობიექტი %1 ვერ დამუშავდება შემდეგი დამამუშავებლით გაცვლისგეგმებისკვანძებისგადმოტვირთვაჩატვირთვა.ობიექტისჩატვირთვამდე()!'", Метаданные.ОсновнойЯзык.КодЯзыка),
			Объект.Метаданные().ПолноеИмя()
		);
