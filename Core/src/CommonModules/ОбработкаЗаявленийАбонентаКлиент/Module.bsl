...
Процедура ОбновитьСтатусыЗаявленийАбонентов(
	...
	Если ЗаявленияАбонентов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Отсутствуют отправленные заявления';ru='Отсутствуют отправленные заявления'"));
		Возврат;
	КонецЕсли;
...
Процедура СообщитьОбОтсуствииИнтернета(ДополнительныеПараметры)
	...
	ОписаниеОшибки 		= НСтр("ka='Не удалось обратиться к серверу обновлений';ru='Не удалось обратиться к серверу обновлений'");
...
Процедура СообщитьОбОтсуствииИзмененияВСтатусеИПолучитьОтветНаСледующееЗаявление(ДополнительныеПараметры)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Статус заявления %1 от %2 не изменился';ru='Статус заявления %1 от %2 не изменился'"),
