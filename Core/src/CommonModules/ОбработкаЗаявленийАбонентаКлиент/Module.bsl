...
Процедура ОбновитьСтатусыЗаявленийАбонентов(
	...
	Если ЗаявленияАбонентов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Отсутствуют отправленные заявления';ru='Отсутствуют отправленные заявления';en='There are no posted statements'"));
		Возврат;
	КонецЕсли;
...
Процедура СообщитьОбОтсуствииИнтернета(ДополнительныеПараметры)
	...
	ОписаниеОшибки 		= НСтр("ka='Не удалось обратиться к серверу обновлений';ru='Не удалось обратиться к серверу обновлений';en='Failed to contact the update server'");
...
Процедура СообщитьОбОтсуствииИзмененияВСтатусеИПолучитьОтветНаСледующееЗаявление(ДополнительныеПараметры)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Статус заявления %1 от %2 не изменился';ru='Статус заявления %1 от %2 не изменился';en='The status of the application% 1 from% 2 is not changed'"),
