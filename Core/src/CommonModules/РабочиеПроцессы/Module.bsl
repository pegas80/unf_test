...
Процедура ОбработатьПравилаРабочихПроцессов(ДанныеКОбработке) Экспорт
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ka='სამუშაო პროცესების წესების დამუშავების სტარტი. დამუშავებაში არსებული წყაროების რაოდენობა: %1';ru='Старт обработки правил рабочих процессов. Количество источников к обработке: %1';en='Start processing workflow rules. Number of sources for processing: 1%'"),
...
"для источника: ""%2""';ka='სამუშაო პროცესის წესის მიხედვით მოქმედებების შესრულების სტარტი: ""%1"", შემდეგი წყაროსთვის: ""%2""'"), Выборка.Правило, Выборка.Источник);
			ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
			...
			ТекстСообщения = СтрШаблон(НСтр("ka='სამუშაო პროცესის წესის მიხედვით მოქმედებების შესრულების დასრულება: ""%1"", შემდეგი წყაროსთვის: ""%2""';ru='Завершение выполнения действий по правилу рабочего процесса: ""%1"","
"для источника: ""%2""'"), Выборка.Правило, Выборка.Источник);
	...
	ТекстСообщения = НСтр("ka='სამუშაო პროცესების წესების დამუშავების დასრულება';ru='Завершение обработки правил рабочих процессов';en='Completion of the processing workflow rules'");
	ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
		Метаданные.Справочники.ПравилаРабочегоПроцесса, , ТекстСообщения);
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='სამუშაო პროცესის წესები';ru='Правила рабочего процесса';en='Terms workflow'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
