	...
	Если СтруктураПараметров.ПроверитьИНН Тогда
		...
		Если СтруктураВозврата.ОшибокПоИННнет Тогда
			...
			Если ЭтоЮрЛицо = Неопределено Тогда
				...
				СтруктураВозврата.ИННВведенКорректно = Ложь;
				СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტის სახეობა უცნობია. მიუთითეთ კონტრაგენტის სახეობა';ru='Неизвестен вид контрагента. Укажите вид контрагента';en='Unknown type of counterparty. Specify the type of counterparty'"),,ЦветВыделенияНекорректногоЗначения);
			...
			Если СтруктураВозврата.ОшибокПоИННнет Тогда
				...
				Если  ЭтоЮрЛицо И ДлинаИНН <> 10 Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='ИНН юридического лица должен состоять из 10 цифр';ru='ИНН юридического лица должен состоять из 10 цифр';en='TIN of the legal entity must be 10 digits'"),,ЦветВыделенияНекорректногоЗначения);
					
					ТекстДляНекорректногоИНН = НСтр("ka='%1"
"გსნ 10 ციფრს არ შეიცავს';ru='%1"
"ИНН содержит не 10 цифр'");
				...
				ИначеЕсли НЕ ЭтоЮрЛицо  И ДлинаИНН <> 12 Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='ფიზიკური პირის გსნ 12 ციფრისგან უნდა შედგებოდეს';ru='ИНН физического лица должен состоять из 12 цифр';en='INN natural person shall consist of 12 digits'"),,ЦветВыделенияНекорректногоЗначения);
					
					ТекстДляНекорректногоИНН = НСтр("ka='%1"
"გსნ 12 ციფრს არ შეიცავს';ru='%1"
"ИНН содержит не 12 цифр'");
				...
				ИначеЕсли Лев(ИНН, 2) = "00" Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='გსნ-ის პირველი ორი ციფრი არ შეიძლება, რომ ""00"" იყოს';ru='Первые две цифры ИНН не могут быть ""00""';en='The first two digits of the TIN can not be ""00""'"),,ЦветВыделенияНекорректногоЗначения);
					
					ТекстДляНекорректногоИНН = НСтр("ka='%1"
"გსნ-ის პირველი ორი ციფრი არ შეიძლება, რომ ""00"" იყოს';ru='%1"
"Первые две цифры ИНН не могут быть ""00""'");
				...
				Если СтруктураВозврата.ОшибокПоИННнет Тогда
					...
					Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
						...
						СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='გსნ მხოლოდ ციფრებისგან უდა შედგებოდეს';ru='ИНН должен включать только цифры';en='INN should include only numbers'"),,ЦветВыделенияНекорректногоЗначения);
						
						ТекстДляНекорректногоИНН = НСтр("ka='%1"
"გსნ არა მხოლოდ ციფრებს შეიცავს';ru='%1"
					...
					Если СтруктураВозврата.ОшибокПоИННнет Тогда
						...
						Если ЭтоЮрЛицо Тогда
							...
							Если КонтрольныйРазряд <> Число(Сред(ИНН, 10, 1)) или КонтрольнаяСумма = 0 Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='იურიდიული პირის გსნ არაკორექტულად არის შეყვანილი';ru='ИНН юридического лица введен некорректно';en='TIN of the legal entity is invalid'"),,ЦветВыделенияНекорректногоЗначения);
								
								ТекстДляНекорректногоИНН = НСтр("ka='%1"
"გსნ არ შეესაბამება ფორმატს';ru='%1"
"ИНН не соответствует формату'");
						...
						Иначе
							...
							Если КонтрольныйРазряд11 <> Число(Сред(ИНН,11,1)) ИЛИ КонтрольныйРазряд12 <> Число(Сред(ИНН,12,1)) или (КонтрольнаяСумма11 = 0 и КонтрольнаяСумма12 = 0) Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='ფიზიკური პირის გსნ არაკორექტულად არის შეყვანილი';ru='ИНН физического лица введен некорректно';en='VAT number of a natural person is invalid'"),,ЦветВыделенияНекорректногоЗначения);
	...
	Если СтруктураПараметров.ПроверитьКПП Тогда
		...
		Если СтруктураПараметров.СтранаРегистрации = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
			...
			Если ЭтоЮрЛицо = Неопределено Тогда
				...
				СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტის სახეობა უცნობია. მიუთითეთ კონტრაგენტის სახეობა';ru='Неизвестен вид контрагента. Укажите вид контрагента';en='Unknown type of counterparty. Specify the type of counterparty'"),,ЦветВыделенияНекорректногоЗначения);
			...
			Если СтруктураВозврата.ОшибокПоКППнет Тогда
				...
				Если СтруктураВозврата.ОшибокПоКППнет Тогда
					...
					Если ДлинаКПП <> 9 Тогда
						...
						СтруктураВозврата.НадписьПоясненияНекорректногоКПП  = Новый ФорматированнаяСтрока(НСтр("ka='აამკ უნდა შეიცავდეს 9 ციფრს';ru='КПП должен содержать 9 цифр';en='Transmission should contain nine digits'"),,ЦветВыделенияНекорректногоЗначения);
						
						ТекстДляНекорректногоКПП = НСтр("ka='%1"
"აამკ შეიცავს 9 ციფრს';ru='%1"
"КПП содержит не 9 цифр'");
					...
					Если СтруктураВозврата.ОшибокПоКППнет Тогда
						...
						Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КПП) Тогда
							...
							СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ka='აამკ-ში მხოლოდ ციფრები უნდა შედიოდეს';ru='КПП должен включать только цифры';en='CPR should include only numbers'"),,ЦветВыделенияНекорректногоЗначения);
							
							ТекстДляНекорректногоКПП = НСтр("ka='%1"
"აამკ არა მხოლოდ ციფრებს შეიცავს';ru='%1"
"КПП содержит не только цифры'");
						...
						Если СтруктураВозврата.ОшибокПоКППнет Тогда
							...
							Если НЕ ПризнакГоловногоПодразделения и НЕ ПризнакОбособленногоПодразделения Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ka='აამკ არ შეესაბამება ფორმატს';ru='КПП не соответствует формату';en='PPC does not fit'"),,ЦветВыделенияНекорректногоЗначения);
								
								ТекстДляНекорректногоКПП = НСтр("ka='%1"
"აამკ არ შეესაბამება ფორმატს';ru='%1"
...
Функция ПреобразоватьНомерДляОтправкиSMS(знач Номер) Экспорт
	...
	Иначе
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Российские номера телефонов должны начинаться на +7 (или 8)';en='Russian phone numbers must begin at 7 (or 8)'");
		Возврат Результат;
	КонецЕсли;
	...
	Иначе
		Результат.НомерКорректен = Ложь;
		Результат.СообщениеОбОшибке = НСтр("ru='Российские номера телефонов содержат 10 цифр';en='Russian phone numbers contain 10 digits'");
	КонецЕсли;
...
Функция ОснованияПлатежа(ВидПеречисления, Период = Неопределено) Экспорт
	...
	Иначе
		ОснованияПлатежа.Добавить(НезаполненноеЗначение(), НСтр("ka='0 - მნიშვნელობის შევსება არ ხდება';ru='0 - значение не заполняется';en='0 - value is not filled'"));
	КонецЕсли;
...
Функция ТипыПлатежа(ВидПеречисления, Период = Неопределено) Экспорт
	...
	Иначе
		ТипыПлатежа.Добавить(НезаполненноеЗначение(), НСтр("ka='0 - მნიშვნელობის შევსება არ ხდება';ru='0 - значение не заполняется';en='0 - value is not filled'"));
	КонецЕсли;
...
Функция НомерСчетаКорректен(НомерСчета, КодБанка, ЯвляетсяБанкомРФ, ТекстСообщения = "") Экспорт
	...
	Если ЯвляетсяБанкомРФ Тогда
		...
		Если Не ПроверитьДлинуНомераСчета(НомерСчета) Тогда
			ТекстСообщения = НСтр("ru='Номер счета должен состоять из 20 цифр';en='account number must be 20 digits'");
		ИначеЕсли Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(НомерСчета) Тогда
			ТекстСообщения = НСтр("ru='В номере счета должны быть только цифры';en='The account number must be only numbers'");
		Иначе
	...
	Иначе
		...
		Если Не ПроверитьРазрешенныеСимволыСчета(НомерСчета) Тогда
			ТекстСообщения = НСтр("ka='Разрешены только буквы от A до Z и цифры';ru='Разрешены только буквы от A до Z и цифры';en='Allowed only letters from A to Z and digits'");
		Иначе
			НомерСчетаКорректен = Истина;
		КонецЕсли;
