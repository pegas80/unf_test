	...
	Если СтруктураПараметров.ПроверитьИНН Тогда
		...
		Если СтруктураВозврата.ОшибокПоИННнет Тогда
			...
			Если ЭтоЮрЛицо = Неопределено Тогда
				...
				СтруктураВозврата.ИННВведенКорректно = Ложь;
				СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტის სახეობა უცნობია. მიუთითეთ კონტრაგენტის სახეობა';ru='Неизвестен вид контрагента. Укажите вид контрагента'"),,ЦветВыделенияНекорректногоЗначения);
			...
			Если СтруктураВозврата.ОшибокПоИННнет Тогда
				...
				Если  ЭтоЮрЛицо И ДлинаИНН <> 10 Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='ИНН юридического лица должен состоять из 10 цифр';ru='ИНН юридического лица должен состоять из 10 цифр'"),,ЦветВыделенияНекорректногоЗначения);
...
"ИНН содержит не 10 цифр'");
				...
				ИначеЕсли НЕ ЭтоЮрЛицо  И ДлинаИНН <> 12 Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='ფიზიკური პირის გსნ 12 ციფრისგან უნდა შედგებოდეს';ru='ИНН физического лица должен состоять из 12 цифр'"),,ЦветВыделенияНекорректногоЗначения);
...
"ИНН содержит не 12 цифр'");
				...
				ИначеЕсли Лев(ИНН, 2) = "00" Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='გსნ-ის პირველი ორი ციფრი არ შეიძლება, რომ ""00"" იყოს';ru='Первые две цифры ИНН не могут быть ""00""'"),,ЦветВыделенияНекорректногоЗначения);
...
"Первые две цифры ИНН не могут быть ""00""'");
				...
				Если СтруктураВозврата.ОшибокПоИННнет Тогда
					...
					Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
						...
						СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='გსნ მხოლოდ ციფრებისგან უდა შედგებოდეს';ru='ИНН должен включать только цифры'"),,ЦветВыделенияНекорректногоЗначения);
					...
					Если СтруктураВозврата.ОшибокПоИННнет Тогда
						...
						Если ЭтоЮрЛицо Тогда
							...
							Если КонтрольныйРазряд <> Число(Сред(ИНН, 10, 1)) или КонтрольнаяСумма = 0 Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='იურიდიული პირის გსნ არაკორექტულად არის შეყვანილი';ru='ИНН юридического лица введен некорректно'"),,ЦветВыделенияНекорректногоЗначения);
...
"ИНН не соответствует формату'");
						...
						Иначе
							...
							Если КонтрольныйРазряд11 <> Число(Сред(ИНН,11,1)) ИЛИ КонтрольныйРазряд12 <> Число(Сред(ИНН,12,1)) или (КонтрольнаяСумма11 = 0 и КонтрольнаяСумма12 = 0) Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ka='ფიზიკური პირის გსნ არაკორექტულად არის შეყვანილი';ru='ИНН физического лица введен некорректно'"),,ЦветВыделенияНекорректногоЗначения);
	...
	Если СтруктураПараметров.ПроверитьКПП Тогда
		...
		Если СтруктураПараметров.СтранаРегистрации = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
			...
			Если ЭтоЮрЛицо = Неопределено Тогда
				...
				СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტის სახეობა უცნობია. მიუთითეთ კონტრაგენტის სახეობა';ru='Неизвестен вид контрагента. Укажите вид контрагента'"),,ЦветВыделенияНекорректногоЗначения);
			...
			Если СтруктураВозврата.ОшибокПоКППнет Тогда
				...
				Если СтруктураВозврата.ОшибокПоКППнет Тогда
					...
					Если ДлинаКПП <> 9 Тогда
						...
						СтруктураВозврата.НадписьПоясненияНекорректногоКПП  = Новый ФорматированнаяСтрока(НСтр("ka='აამკ უნდა შეიცავდეს 9 ციფრს';ru='КПП должен содержать 9 цифр'"),,ЦветВыделенияНекорректногоЗначения);
...
"КПП содержит не 9 цифр'");
					...
					Если СтруктураВозврата.ОшибокПоКППнет Тогда
						...
						Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КПП) Тогда
							...
							СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ka='აამკ-ში მხოლოდ ციფრები უნდა შედიოდეს';ru='КПП должен включать только цифры'"),,ЦветВыделенияНекорректногоЗначения);
...
"КПП содержит не только цифры'");
						...
						Если СтруктураВозврата.ОшибокПоКППнет Тогда
							...
							Если НЕ ПризнакГоловногоПодразделения и НЕ ПризнакОбособленногоПодразделения Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ka='აამკ არ შეესაბამება ფორმატს';ru='КПП не соответствует формату'"),,ЦветВыделенияНекорректногоЗначения);
...
Функция ОснованияПлатежа(ВидПеречисления, Период = Неопределено) Экспорт
	...
	Иначе
		ОснованияПлатежа.Добавить(НезаполненноеЗначение(), НСтр("ka='0 - მნიშვნელობის შევსება არ ხდება';ru='0 - значение не заполняется'"));
	КонецЕсли;
...
Функция ТипыПлатежа(ВидПеречисления, Период = Неопределено) Экспорт
	...
	Иначе
		ТипыПлатежа.Добавить(НезаполненноеЗначение(), НСтр("ka='0 - მნიშვნელობის შევსება არ ხდება';ru='0 - значение не заполняется'"));
	КонецЕсли;
...
Функция НомерСчетаКорректен(НомерСчета, КодБанка, ЯвляетсяБанкомРФ, ТекстСообщения = "") Экспорт
	...
	Иначе
		...
		Если Не ПроверитьРазрешенныеСимволыСчета(НомерСчета) Тогда
			ТекстСообщения = НСтр("ka='Разрешены только буквы от A до Z и цифры';ru='Разрешены только буквы от A до Z и цифры'");
		Иначе
			НомерСчетаКорректен = Истина;
		КонецЕсли;
