...
Процедура ПоказатьПанельОтчетов(ПутьКПодсистеме, ПараметрыВыполненияКоманды, Удалить_Заголовок = "") Экспорт
	...
	Если ТипЗнч(ПараметрыВыполненияКоманды) = Тип("Структура")
		И ПараметрыВыполненияКоманды.Свойство("ТекущийРазделСсылка")
		Тогда
	
		ФормаПараметры.Вставить("ТекущийРазделСсылка", ПараметрыВыполненияКоманды.ТекущийРазделСсылка);	
...
Процедура ОткрытьДиалогРазмещенияВариантовВРазделах(МассивВариантов, ДополнительныеПараметры = Неопределено, Владелец = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивВариантов) <> Тип("Массив") ИЛИ МассивВариантов.Количество() < 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ანგარიშგებების ვარიანტები, რომელთა განთავსება განყოფილებებში აუცილებელია.';ru='Выберите варианты отчетов, которые необходимо разместить в разделах.';en='Select versions of the reports, which should be placed in the sections.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьДиалогСбросаНастроекПользователей(МассивВариантов, Владелец = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивВариантов) <> Тип("Массив") ИЛИ МассивВариантов.Количество() < 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ანგარიშგების ვარიანტები, რომლებისთვის აუცილებელია სამომხმარებლო გამართვების ჩამოყრა.';ru='Выберите варианты отчетов, для которых необходимо сбросить пользовательские настройки.';en='Select versions of the reports for which the user settings must be reset.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьДиалогСбросаНастроекРазмещения(МассивВариантов, Владелец = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивВариантов) <> Тип("Массив") ИЛИ МассивВариантов.Количество() < 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ პროგრამის ანგარიშგებების ვარიანტები, რომლებისთვის აუცილებელია განთავსების გამართვების ჩამოყრა.';ru='Выберите варианты отчетов программы, для которых необходимо сбросить настройки размещения.';en='Select the options for the program reports for which you need to reset your network settings.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуОтчета(Знач ФормаВладелец, Знач Вариант, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыОткрытия.ТипОтчета) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ში ანგარიშგების ტიპი არ არის დადგენილი';ru='Не определен тип отчета в %1';en='Not determined the type of report to% 1'"), "ВариантыОтчетовКлиент.ОткрытьФормуОтчета");
	КонецЕсли;
	...
	ИначеЕсли ПараметрыОткрытия.ТипОтчета = "Дополнительный" Тогда
		...
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='გარე ანგარიშგების ვარიანტის გახსნა შეიძლება მხოლოდ ანგარიშგების ფორმიდან.';ru='Вариант внешнего отчета можно открыть только из формы отчета.';en='Optionally external report can only be opened from the report form.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыОткрытия.ИмяОтчета) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგების სახელი %1-ში არ არის განსაზღვრული';ru='Не определено имя отчета в %1';en='Not defined name for the report in the 1%'"), "ВариантыОтчетовКлиент.ОткрытьФормуОтчета");
	КонецЕсли;
...
Процедура РедактироватьМногострочныйТекст(ФормаИлиОбработчик, ТекстРедактирования, ВладелецРеквизита, ИмяРеквизита, Знач Заголовок = "") Экспорт
	...
	Если ПустаяСтрока(Заголовок) Тогда
		Заголовок = НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'");
	КонецЕсли;
