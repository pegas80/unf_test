...
Функция НайтиПеречисление(Знач ИмяПеречисления, ПредставлениеПеречисления) Экспорт
	...
	Если ИмяПрикладногоПеречисления = Неопределено Тогда // не задано соответствие
		ШаблонСообщения = НСтр("ka='გამოყენებითი გადაწყვეტილების კოდში აუცილებელია შესაბამისობის მითითება ჩამონათვალისთვის %1.';ru='В коде прикладного решения необходимо указать соответствие для перечисления %1.'");
...
Функция СтавкаНДСИзПредставления(ПредставлениеСтавкиНДС) Экспорт
	...
	Если СтрСтавкаНДС = Неопределено ИЛИ СтрНайти(НСтр("ka='დრგ-ის გარეშე';ru='БЕЗ НДС'"), ВРег(СтрСтавкаНДС)) > 0 Тогда
		ЗначениеНДС = НСтр("ka='დღგ-ის გარეშე';ru='без НДС'");
	Иначе
...
Функция ПредставлениеОснованияДляВидаЭД(ВидЭД) Экспорт
	...
	СоответствиеВидовЭДДокументамИБ.Вставить(Перечисления.ВидыЭД.ПроизвольныйЭД, НСтр("ka='Исходящий произвольный документ';ru='Исходящий произвольный документ'"));
	...
	Если ПредставлениеОснования = Неопределено Тогда // не задано соответствие
		ШаблонСообщения = НСтр("ka='В переопределяемом модуле прикладного решения необходимо указать представление документаИБ(основания) и хоз. операции для вида ЭД %1.';ru='В переопределяемом модуле прикладного решения необходимо указать представление документаИБ(основания) и хоз. операции для вида ЭД %1.'");
...
Функция ТЗСопоставленияПолейАдресаФНС_CML()
	...
	НоваяСтрока.ПредставлениеCML = НСтр("ka='საფოსტო ინდექსი';ru='Почтовый индекс'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "КодРегион";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='რეგიონი';ru='Регион'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "НаселПункт";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Город";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='ქალაქი';ru='Город'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Улица";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='ქუჩა';ru='Улица'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Дом";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='სახლი';ru='Дом'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Корпус";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='კორპუსი';ru='Корпус'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Кварт";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='ბინა';ru='Квартира'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "КодСтр";
	НоваяСтрока.ПредставлениеCML = НСтр("ka='ქვეყანა';ru='Страна'");
