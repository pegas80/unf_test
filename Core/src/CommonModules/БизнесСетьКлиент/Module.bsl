...
Процедура ОтправитьЧерезБизнесСетьПродолжить(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ИначеЕсли Не ВозможнаОтправка Тогда
		ТекстВопроса = НСтр("ka='ორგანიზაცია ""%1"" სერვის 1С:ბიზნეს-ქსელზე არ არის შეერთებული. შევუერთოთ ეხლა?';ru='Организация ""%1"" не подключена к сервису 1С:Бизнес-сеть. Подключить сейчас?';en=""The organization '% 1' is not connected to the service of 1C: Business Network. Connect now?""");
	...
	ИначеЕсли Результат.КодСостояния = 404 Тогда
		Если ЗначениеЗаполнено(Ссылка) Тогда
			Если УчастникЯвляетсяОрганизацией Тогда
				...
				ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
			Иначе
				ТекстВопроса = НСтр("ka='კონტრაგენტი ""%1"" არ არის შეერთებული სერვის 1С:ბიზნეს-ქსელთან."
"გავაგზავნოთ შეერთების მოწვევა?';ru='Контрагент ""%1"" не подключен к сервису 1С:Бизнес-сеть."
"Направить приглашение подключения?'");
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ka='სერვის 1C:ბიზნეს-ქსელში მონაწილე არ არის რეგისტრირებული.';ru='Участник не зарегистрирован в сервисе 1С:Бизнес-сеть.';en='Member is not registered when 1C: Business Network.'"));
		КонецЕсли;
		Возврат;
	Иначе
		Возврат;
...
Процедура ЗагрузитьДокументВИБ(Контекст, СопоставлятьНоменклатуруПередЗаполнениемДокумента, Отказ, ОбновитьСтруктуруРазбора = Ложь)
	...
	ТекстОповещения	= НСтр("ka='ატვირთვა შესრულებულია';ru='Загрузка выполнена.';en='Download complete.'");
	ТекстПояснения	= НСтр("ka='დოკუმენტი ჩატვირთულია სერვის 1C:ბიზნეს-ქსელით.';ru='Загружен документ через сервис 1С:Бизнес-сеть.';en='Document is loaded through the service of 1C: Business Network.'");
