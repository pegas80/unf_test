...
Процедура ОтправитьЧерезБизнесСетьПродолжить(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ИначеЕсли Не ВозможнаОтправка Тогда
		ТекстВопроса = НСтр("ka='ორგანიზაცია ""%1"" სერვის 1С:ბიზნეს-ქსელზე არ არის შეერთებული. შევუერთოთ ეხლა?';ru='Организация ""%1"" не подключена к сервису 1С:Бизнес-сеть. Подключить сейчас?'");
...
"Направить приглашение подключения?'");
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ka='სერვის 1C:ბიზნეს-ქსელში მონაწილე არ არის რეგისტრირებული.';ru='Участник не зарегистрирован в сервисе 1С:Бизнес-сеть.'"));
		КонецЕсли;
		Возврат;
	Иначе
		Возврат;
...
Процедура ЗагрузитьДокументВИБ(Контекст, СопоставлятьНоменклатуруПередЗаполнениемДокумента, Отказ, ОбновитьСтруктуруРазбора = Ложь)
	...
	ТекстОповещения	= НСтр("ka='ატვირთვა შესრულებულია';ru='Загрузка выполнена.'");
	ТекстПояснения	= НСтр("ka='დოკუმენტი ჩატვირთულია სერვის 1C:ბიზნეს-ქსელით.';ru='Загружен документ через сервис 1С:Бизнес-сеть.'");
