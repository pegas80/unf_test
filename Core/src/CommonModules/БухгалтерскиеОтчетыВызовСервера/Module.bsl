...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ВыводитьПолностью Тогда
		...
		Исключение
			...
			КонецЦикла;
			ТекстСообщения = НСтр("ka='ანგარიშგება არ დაფორმირდა! %1';ru='Отчет не сформирован! %1'");
...
Процедура ОбработкаПроверкиЗаполнения(ОтчетОбъект, Отказ, Проверки = Неопределено) Экспорт
	...
	Если Проверки.Свойство("ВыборПоказателя") И Проверки.ВыборПоказателя Тогда
		...
		Если КоличествоПоказателей = 0 И НаборПоказателей.Количество() > 0 Тогда
			ТекстСообщения = НСтр("ka='არ არის არჩეული მაჩვენებელი(ები)';ru='Не выбран показатель(и)'");
	...
	Если Проверки.Свойство("СписокВидовСубконто") И Проверки.СписокВидовСубконто Тогда
		...
		Если ОтчетОбъект.СписокВидовСубконто.Количество() > МаксКоличествоСубконто Тогда
			ТекстСообщения = НСтр("ka='არჩეულია ძალიან ბევრი სუბკონტოს სახეობა. დასაშვებია მაქსიმუმ %1';ru='Выбрано слишком много видов субконто, максимально допустимо %1'");
...
Процедура ОбработкаПроверкиЗаполненияОтборов(ОтчетОбъект, Отказ) Экспорт
	...
	Для каждого Отбор Из ОтборыВерхнегоУровня Цикл
		...
		ТекстСообщения = НСтр("ka='ველი სუბკონტოს რეკვიზიტებით ""%1"" არ შეიძლება გამოყენებული იყოს დაჯგუფურ შერჩევაში სხვა ველებთან ერთად.';ru='Поле с реквизитами субконто ""%1"" нельзя использовать в групповом отборе с иными полями.'");
...
Функция ПолучитьПараметрыРасшифровкиОтчета(Адрес, ИдентификаторОбъекта, Расшифровка) Экспорт
	...
	Если ИдентификаторОбъекта = "АнализСубконто"
		...
		ПредставлениеДляОСВПоСчету            = НСтр("ka='ОСВ по счету %Счет%';ru='ОСВ по счету %Счет%'");
		ПредставлениеДляКарточкиСчета         = НСтр("ka='Карточка счета %Счет%';ru='Карточка счета %Счет%'");
		ПредставлениеДляАнализаСчета          = НСтр("ka='Анализ счета %Счет%';ru='Анализ счета %Счет%'");
		ПредставлениеДляОборотыСчета          = НСтр("ka='Обороты счета %Счет%';ru='Обороты счета %Счет%'");
		ПредставлениеДляОборотыСчетаПоМесяцам = НСтр("ka='Обороты счета %Счет% по месяцам';ru='Обороты счета %Счет% по месяцам'");
		ПредставлениеДляОборотыСчетаПоДням    = НСтр("ka='Обороты счета %Счет% по дням';ru='Обороты счета %Счет% по дням'");
		ПредставлениеДляКарточкиСубконто      = НСтр("ka='Карточка субконто';ru='Карточка субконто'");
		ПредставлениеДляОтчетаПоПроводкам     = НСтр("ka='Отчет по проводкам';ru='Отчет по проводкам'");
		ПредставлениеОткрытьОбъект 			  = НСтр("ka='Открыть ""%Значение%""';ru='Открыть ""%Значение%""'");
	...
	Если ПоляТекущейРасшифровки.Количество() > 0
		...
		ИЛИ ПоляТекущейРасшифровки.Количество() = 1) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЗначениеПоля)) Тогда
				ВозможныеРасшифровки.Добавить(ПоляТекущейРасшифровки[0].Значение, СтрШаблон(НСтр("ka='გახსნა ""%1""';ru='Открыть ""%1""'"), ПоляТекущейРасшифровки[0].Значение));
			КонецЕсли;
...
Функция ОписаниеОтборовОтчета(ПараметрыОтчета, КомпоновщикНастроек)
	...
	Иначе
		...
		Если ПараметрыОтчета.Свойство("Подразделение") И ЗначениеЗаполнено(ПараметрыОтчета.Подразделение) Тогда
			ТекстПодразделение = СтрШаблон(НСтр("ka='განყოფილება უდრის ""%1""';ru='Подразделение Равно ""%1""'"), ПараметрыОтчета.Подразделение);
		КонецЕсли;
...
Процедура ПодставитьИменаСубконто(ФормаОтчета, ТекстОтбор)
	...
	ИмяПоляПрефикс = НСтр("ka='Субконто';ru='Субконто'");	
	Если СтрНайти(ТекстОтбор, ИмяПоляПрефикс) = 0
	 Или Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета.Отчет, "Счет") Тогда
		Возврат;
	КонецЕсли;
	ИмяПоляПриставка = НСтр("ka='Кор';ru='Кор'");
