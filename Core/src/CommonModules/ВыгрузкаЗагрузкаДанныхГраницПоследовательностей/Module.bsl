...
Процедура ПередВыгрузкойТипа(Контейнер, Сериализатор, ОбъектМетаданных, Отказ) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='მეტამონაცემთა ობიექტი %1 არ შეიძლება დამუშავდეს შემდეგი დამამუშავებლით თანმიმდევრულობისსაზღვრებისმონაცემთაგადმოტვირთვაჩატვირთვა.ობიექტისგადმოტვირთვამდე()!';ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхГраницПоследовательностей.ПередВыгрузкойОбъекта()!';en='Metadata object% 1 can not be processed by the handler VygruzkaZagruzkaDannyhGranitsPosledovatelnostey.PeredVygruzkoyObekta ()!'"),
