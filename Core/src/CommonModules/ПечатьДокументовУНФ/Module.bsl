...
Функция ПредставлениеЗаголовкаНДС(СуммаНДС, СуммаВключаетНДС, ЧастичнаяОплата) Экспорт
	...
	Если ЧастичнаяОплата Тогда
		...
		ТекстНДС = ТекстНДС + НСтр("ru=' оплаты';ka=' оплаты'");
	...
	ИначеЕсли СуммаВключаетНДС Тогда
		...
		ЗаголовокИтоговНДС = СтрШаблон(НСтр("ru='მათ შორის %1:';ka='მათ შორის %1:'"), ТекстНДС);
		
	Иначе
		
		ЗаголовокИтоговНДС = СтрШаблон(НСтр("ka='Сумма %1:';ru='Сумма %1:';en='Amount% 1:'"), ТекстНДС);
...
Функция ПредставлениеОснованияПечати(ОснованиеПечатиСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ОснованиеПечатиСсылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		Возврат Нстр("ka='ხელშეკრულება: ';ru='Договор: '") + Строка(ОснованиеПечатиСсылка.Наименование);
...
Функция ПолучитьОбластьБезопасно(Макет, ИмяОбласти, ПредставлениеОбласти, Ошибки) Экспорт
	...
	Если Макет.Области.Найти(ИмяОбласти) = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='ВНИМАНИЕ! Не обнаружена область макета %1. Возможно используется пользовательский макет.';ru='ВНИМАНИЕ! Не обнаружена область макета %1. Возможно используется пользовательский макет.';en='ATTENTION! Not found the layout area% 1. Perhaps using a custom layout.'");
...
Функция ПолучитьПлашкуПодписиБезопастно(ОбластьМакета, ИмяПлашки, ПредставлениеПодписи, Ошибки) ЭКспорт
	...
	Если ПлашкаПодписи = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='ВНИМАНИЕ! Нет места для подписи %1. Возможно используется пользовательский макет.';ru='ВНИМАНИЕ! Нет места для подписи %1. Возможно используется пользовательский макет.';en='ATTENTION! There is no place for the signature% 1. Perhaps using a custom layout.'");
...
Функция ИтоговаяСтрока(КоличествоПозиций, СуммаДокумента, ВалютаДокумента) Экспорт
	...
	Возврат СтрШаблон(
		НСтр("ka='სულ არის %2 ოდენობის %1 დასახელება';ru='Всего наименований %1, на сумму %2';en='All kinds of 1%, amounting to 2%'"),
		КоличествоПозиций,
