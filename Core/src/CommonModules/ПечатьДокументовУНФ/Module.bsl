...
Функция ПредставлениеЗаголовкаНДС(СуммаНДС, СуммаВключаетНДС, ЧастичнаяОплата) Экспорт
	...
	Если СуммаНДС = 0 Тогда
		...
		ЗаголовокИтоговНДС = НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)'");
	...
	Иначе
		...
		ЗаголовокИтоговНДС = СтрШаблон(НСтр("ka='Сумма %1:';ru='Сумма %1:'"), ТекстНДС);
...
Функция ПолучитьОбластьБезопасно(Макет, ИмяОбласти, ПредставлениеОбласти, Ошибки) Экспорт
	...
	Если Макет.Области.Найти(ИмяОбласти) = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='ВНИМАНИЕ! Не обнаружена область макета %1. Возможно используется пользовательский макет.';ru='ВНИМАНИЕ! Не обнаружена область макета %1. Возможно используется пользовательский макет.'");
...
Функция ПолучитьПлашкуПодписиБезопастно(ОбластьМакета, ИмяПлашки, ПредставлениеПодписи, Ошибки) ЭКспорт
	...
	Если ПлашкаПодписи = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='ВНИМАНИЕ! Нет места для подписи %1. Возможно используется пользовательский макет.';ru='ВНИМАНИЕ! Нет места для подписи %1. Возможно используется пользовательский макет.'");
...
Функция ШаблонТекстаПредупрежденияПравкиМакета() Экспорт
	...
	Возврат НСтр("ka='ყურადღება! შესაძლებელია, რომ გამოიყენებოდეს სამომხმარებლო მაკეტი. ანგარიშების ბეჭდვის საშტატო მექანიზმი შეიძლება არაკორექტულად მუშაობდეს.';ru='ВНИМАНИЕ! Возможно используется пользовательский макет. Штатный механизм печати счетов может работать некоректно.'");
...
Функция ИтоговаяСтрока(КоличествоПозиций, СуммаДокумента, ВалютаДокумента) Экспорт
	...
	Возврат СтрШаблон(
		НСтр("ka='სულ არის %2 ოდენობის %1 დასახელება';ru='Всего наименований %1, на сумму %2'"),
		КоличествоПозиций,
