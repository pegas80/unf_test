...
Процедура ФормаОтчетаОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ТипРасшифровки = "РасшифровкаРегламентногоЗадания" Тогда
		...
		ВариантРасшифровки.Добавить("СведенияОРегламентномЗадании", НСтр("ka='ინფორმაცია სარეგლამენტო დავალების შესახებ';ru='Сведения о регламентном задании'"));
		ВариантРасшифровки.Добавить("ОткрытьЖурналРегистрации", НСтр("ka='რეგისტრაციის ჟურნალზე გადასვლა';ru='Перейти к журналу регистрации'"));
...
Процедура РезультатОбработкаРасшифровкиЗавершение(ВыбранныйВариант, ПараметрыОбработчика) Экспорт
	...
	Если Действие = "СведенияОРегламентномЗадании" Тогда
		...
		Для Каждого ТочкаДиаграммыГанта Из СписокТочек Цикл
			...
			Если ТочкаДиаграммыГанта.Значение = НСтр("ka='ფონური დავალებები';ru='Фоновые задания'") Тогда
				Продолжить;
			КонецЕсли;
