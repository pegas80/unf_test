...
Процедура ФормаОтчетаОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ТипРасшифровки = "РасшифровкаРегламентногоЗадания" Тогда
		...
		ВариантРасшифровки.Добавить("СведенияОРегламентномЗадании", НСтр("ka='ინფორმაცია სარეგლამენტო დავალების შესახებ';ru='Сведения о регламентном задании';en='For information about scheduled jobs'"));
		ВариантРасшифровки.Добавить("ОткрытьЖурналРегистрации", НСтр("ka='რეგისტრაციის ჟურნალზე გადასვლა';ru='Перейти к журналу регистрации';en='Go to the journal registration'"));
...
Процедура РезультатОбработкаРасшифровкиЗавершение(ВыбранныйВариант, ПараметрыОбработчика) Экспорт
	...
	Если Действие = "СведенияОРегламентномЗадании" Тогда
		...
		Для Каждого ТочкаДиаграммыГанта Из СписокТочек Цикл
			...
			Если ТочкаДиаграммыГанта.Значение = НСтр("ka='ფონური დავალებები';ru='Фоновые задания';en='background jobs'") Тогда
				Продолжить;
			КонецЕсли;
