...
Функция ФлагНастройкиВопросаОДолгойСинхронизации(ЗначениеФлага = Неопределено, ОписаниеНастройки = Неопределено) Экспорт
	...
	ОписаниеНастройки.Вставить("Представление", НСтр("ka='ხანგრძლივი სინქრონიზაციის შესახებ გაფრთხილების ჩვენება';ru='Показывать предупреждение о длительной синхронизации';en='Show warning about long-term synchronization'"));
...
Функция ИмяФайлаПакетаУстановки() Экспорт
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა.zip';ru='Автономная работа.zip';en='Autonomous rabota.zip'");
...
Функция МониторАвтономныхРабочихМест() Экспорт
	...
	Для Каждого НастройкаСинхронизации Из НастройкиСинхронизации Цикл
		...
		Иначе
			НастройкаСинхронизации.ПредставлениеДатыСинхронизации = НСтр("ka='არ იყო შესრულებული';ru='не выполнялась';en='I have not been performed'");
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииСозданиеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა. ავტონომიური სამუშაო ადგილის შექმნა';ru='Автономная работа.Создание автономного рабочего места';en='Autonomous rabota.Sozdanie standalone workstation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა. ავტონომიური სამუშაო ადგილის წაშლა';ru='Автономная работа.Удаление автономного рабочего места';en='Autonomous rabota.Udalenie standalone workstation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция НаименованиеАвтономногоРабочегоМестаПоУмолчанию()
	...
	Результат = НСтр("ka='ავტონომიური მუშაობა - %1';ru='Автономная работа - %1';en='Stand-alone operation - 1%'");
...
Функция ДопустимыеСимволыПрефиксаАвтономногоРабочегоМеста()
	...
	Возврат НСтр("ka='აბგდევზთიკლმნოპჟრსტუფქღყშჩცძწჭხჯჰაბგდევზთიკლმნოპჟრსტუფქღყშჩცძწჭხჯჰ';ru='АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя';en='ABVGDEZhZIKLMNOPRSTUFHTsChShEYuYaabvgdezhziklmnoprstufhtschsheyuya'"); // 54 символа
...
Процедура СинхронизироватьДанныеСПриложениемВИнтернете() Экспорт
	...
	Если Не ЭтоАвтономноеРабочееМесто() Тогда
		...
		ПодробноеПредставлениеОшибкиДляЖурналаРегистрации =
			НСтр("ka='ეს საინფორმაციო ბაზა არ არის ავტონომიური სამუშაო ადგილი. მონაცემთა სინქრონიზაცია გაუქმებულია.';ru='Эта информационная база не является автономным рабочим местом. Синхронизация данных отменена.';en='This information base is not a stand-alone workstation. Synchronization data has been canceled.'",
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ka='ინტერნეტში დანართიან მონაცემთა სინქრონიზაციის პროცესში შეცდომები წარმოიშვა (იხ. რეგისტრაციის ჟურნალი).';ru='В процессе синхронизации данных с приложением в Интернете возникли ошибки (см. журнал регистрации).';en='In the process of data synchronization with the application on the Internet any errors (see. Log).'");
	КонецЕсли;
...
"в файловом режиме работы информационной базы.';ka='ავტონომიური სამუშაო ადგილის პირველი გაშვება უნდა შესრულდეს საინფორმაციო ბაზის მუშაობის ფაილურ რეჟიმში.'");
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыИзНачальногоОбраза()
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='შეიძლება, საინფორმაციო ბაზა გახსნილია კონფიგურატორის რეჟიმში. დაასრულეთ კონფიგურატორის მუშაობა და გაიმეორეთ პროგრამის გაშვება.';ru='Возможно, информационная база открыта в режиме конфигуратора."
"Завершите работу конфигуратора и повторите запуск программы.'");
	...
	Попытка
		...
		Если Пользователь = Неопределено Тогда
			ВызватьИсключение НСтр("ka='Идентификация пользователя не выполнена."
"Возможно, справочник пользователей не включен в состав плана обмена автономной работы.';ru='Идентификация пользователя не выполнена."
"Возможно, справочник пользователей не включен в состав плана обмена автономной работы.'");
		...
		Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
			НСтр("ka='ავტონომიური სამუშაო ადგილის საწყისი გამართვა.';ru='Начальная настройка автономного рабочего места.';en='Initial Setup standalone workstation.'"));
...
Функция ПолучитьПараметрыИзНачальногоОбраза()
	...
	Если ПустаяСтрока(СтрокаXML) Тогда
		ВызватьИсключение НСтр("ka='ავტონომიურ სამუშაო ადგილს არ გადაეცა გამართვები. ავტონომიურ სამუშაო ადგილთან მუშაობა შეუძლებელია.';ru='В автономное рабочее место не были переданы настройки."
...
Функция СчитатьДанныеВСтруктуру(ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ka='Ошибка чтения XML';ru='Ошибка чтения XML';en='XML reading error'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииСинхронизацияДанных()
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა. მონაცემთა სინქრონიზაცია';ru='Автономная работа.Синхронизация данных';en='Autonomous data rabota.Sinhronizatsiya'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
