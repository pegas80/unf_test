...
Функция ФлагНастройкиВопросаОДолгойСинхронизации(ЗначениеФлага = Неопределено, ОписаниеНастройки = Неопределено) Экспорт
	...
	ОписаниеНастройки.Вставить("Представление", НСтр("ka='ხანგრძლივი სინქრონიზაციის შესახებ გაფრთხილების ჩვენება';ru='Показывать предупреждение о длительной синхронизации'"));
...
Функция ИмяФайлаПакетаУстановки() Экспорт
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა.zip';ru='Автономная работа.zip'");
...
Функция МониторАвтономныхРабочихМест() Экспорт
	...
	Для Каждого НастройкаСинхронизации Из НастройкиСинхронизации Цикл
		...
		Иначе
			НастройкаСинхронизации.ПредставлениеДатыСинхронизации = НСтр("ka='არ იყო შესრულებული';ru='не выполнялась'");
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииСозданиеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა. ავტონომიური სამუშაო ადგილის შექმნა';ru='Автономная работа.Создание автономного рабочего места'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა. ავტონომიური სამუშაო ადგილის წაშლა';ru='Автономная работа.Удаление автономного рабочего места'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция НаименованиеАвтономногоРабочегоМестаПоУмолчанию()
	...
	Результат = НСтр("ka='ავტონომიური მუშაობა - %1';ru='Автономная работа - %1'");
...
Функция ДопустимыеСимволыПрефиксаАвтономногоРабочегоМеста()
	...
	Возврат НСтр("ka='აბგდევზთიკლმნოპჟრსტუფქღყშჩცძწჭხჯჰაბგდევზთიკლმნოპჟრსტუფქღყშჩცძწჭხჯჰ';ru='АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя'"); // 54 символа
...
Процедура СинхронизироватьДанныеСПриложениемВИнтернете() Экспорт
	...
	Если Не ЭтоАвтономноеРабочееМесто() Тогда
		...
		ПодробноеПредставлениеОшибкиДляЖурналаРегистрации =
			НСтр("ka='ეს საინფორმაციო ბაზა არ არის ავტონომიური სამუშაო ადგილი. მონაცემთა სინქრონიზაცია გაუქმებულია.';ru='Эта информационная база не является автономным рабочим местом. Синхронизация данных отменена.'",
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ka='ინტერნეტში დანართიან მონაცემთა სინქრონიზაციის პროცესში შეცდომები წარმოიშვა (იხ. რეგისტრაციის ჟურნალი).';ru='В процессе синхронизации данных с приложением в Интернете возникли ошибки (см. журнал регистрации).'");
	КонецЕсли;
...
"Возможно, справочник пользователей не включен в состав плана обмена автономной работы.'");
		...
		Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
			НСтр("ka='ავტონომიური სამუშაო ადგილის საწყისი გამართვა.';ru='Начальная настройка автономного рабочего места.'"));
...
Функция СчитатьДанныеВСтруктуру(ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ka='Ошибка чтения XML';ru='Ошибка чтения XML'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииСинхронизацияДанных()
	...
	Возврат НСтр("ka='ავტონომიური მუშაობა. მონაცემთა სინქრონიზაცია';ru='Автономная работа.Синхронизация данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
