...
Функция ПредставлениеНалоговогоПериода(НалоговыйПериод) Экспорт
	...
	Если ОписаниеПериода.Периодичность = ПлатежПоКонкретнойДате() Тогда
		Шаблон = НСтр("ka='по сроку уплаты [Дата]';ru='по сроку уплаты [Дата]';en='for payment date [Date]'");
	...
	ИначеЕсли ОписаниеПериода.Периодичность = ПериодичностьГод() Тогда
		Шаблон = НСтр("ka='за [Год] год';ru='за [Год] год';en='for the [year]'");
	ИначеЕсли ОписаниеПериода.Периодичность = ПериодичностьПолугодие() Тогда
		Шаблон = НСтр("ka='за [НомерПериода] полугодие [Год] года';ru='за [НомерПериода] полугодие [Год] года';en='for [NomerPerioda] half [year]'");
	ИначеЕсли ОписаниеПериода.Периодичность = ПериодичностьКвартал() Тогда
		Шаблон = НСтр("ka='за [НомерПериода] квартал [Год] года';ru='за [НомерПериода] квартал [Год] года';en='for [NomerPerioda] quarter, [year]'");
	ИначеЕсли ОписаниеПериода.Периодичность = ПериодичностьМесяц() Тогда
		Шаблон = НСтр("ka='за [ИмяМесяца] [Год] года';ru='за [ИмяМесяца] [Год] года';en='for [ImyaMesyatsa] [year]'");
