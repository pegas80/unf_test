	...
	Если НЕ ЭтоДекларация
		...
		ИначеЕсли Форма.РеквизитыПроверкиКонтрагентов.Свойство("ЭтоДекларацияПоНДС") Тогда
			...
			Если ДополнительныеПараметры.Свойство("ЭтоЗапускПроверкиКомандой") Тогда
				ПоказатьПредупреждение(, НСтр("ka='Для проверки контрагентов необходимо заполнить отчет.';ru='Для проверки контрагентов необходимо заполнить отчет.';en='To check the contractors must complete a report.'"));
			КонецЕсли;
...
Процедура ПоказатьРезультатПроверкиСразу(Форма)
	...
	ИначеЕсли НЕ Форма.РеквизитыПроверкиКонтрагентов.ЕстьДоступКВебСервисуФНС Тогда
		ПоказатьПредупреждение(, НСтр("ka='საგადასახადო სამსახურის სერვისის მიერ კონტრაგენტების შემოწმება არ შესრულდა სერვისთან შეერთების შეცდომის გამო';ru='Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису';en='Checking counterparties FTS service is not executed due to an error while connecting to the service'"));
	ИначеЕсли Форма.РеквизитыПроверкиКонтрагентов.КоличествоНекорректныхКонтрагентов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нет ошибок проверки контрагентов сервисом ФНС';ru='Нет ошибок проверки контрагентов сервисом ФНС';en='No error checking contractors FTS service'"));
	КонецЕсли;
