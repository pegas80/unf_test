	...
	Пока Выборка.Следующий() Цикл
		Если ТипЗнч(Выборка.ДокументПотомок) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда
			...
			Исключение
				...
				ВидОперации = НСтр("ka='საფუძველ-დოკუმენტ ""%1""-ის ჩაწერა დოკუმენტ ""%2""-ში';ru='запись документа-основания ""%1"" в документ ""%2""'");
		...
		Для каждого ЭлектронныйДокумент Из СтруктураПакетаЭД.МассивЭД Цикл
			...
			Если ЭлектронныйДокумент.Зашифрован Тогда
				...
				Если ПарольДоступаКЗакрытомуКлючу = Неопределено Тогда
					ШаблонСообщения = НСтр("ka='სერთიფიკატზე პაროლის მიღება ვერ მოხერხდა: %1';ru='Не удалось получить пароль к сертификату: %1'");
				...
				Исключение
					ШаблонСообщения = НСтр("ka='ედ-ის პაკეტის გაშიფრვა: %1';ru='Расшифровка пакета ЭД: %1'");
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
...
"%2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураПакетаЭД.ПакетЭДСсылка,
				ОписаниеОшибки);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД'"), ТекстСообщения,
				ТекстСообщения);
		...
		Для каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
			...
			Если НЕ ЗначениеЗаполнено(Подпись.АдресЭД) Тогда
				ОписаниеОшибки = НСтр("ka='საპასუხო ხელმოწერის შესამოწმებლად საწყისი მონაცემები ვერ მოიძებნა.';ru='Не удалось найти исходные данные для проверки ответной подписи.'");
				Прервать;
			КонецЕсли;
			...
			Исключение
				ШаблонСообщения = НСтр("ka='სერთიფიკატების მიღება ხელმოწერიდან: %1';ru='Получение сертификатов из подписи: %1'");
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Прервать;
			КонецПопытки;
	...
	Для каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			Исключение
				...
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის არქივის განფუთვა';ru='Распаковка архива пакета ЭД'"), ТекстОшибки,
					ТекстСообщения);
		...
		Исключение
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД'"), ТекстОшибки,
				ТекстСообщения);
...
"- полученный пакет не предназначен для текущей ИБ (первичный титул был сформирован в другой базе/копии текущей базы).'");
				...
				Если ЭлектронныйДокумент.ТипДанных = "СлужебныйЭД" Тогда
					...
					Если Не ПервичныйЭДНайден Тогда
						ТекстСообщения = НСтр("ka='სამსახურებრივი ელექტრონული დოკუმენტისთვის მფლობელი ვერ მოიძებნა.';ru='Не удалось найти владельца для служебного электронного документа.'");
					...
					Если СтруктураФайлаЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.Ошибка Тогда
						...
						ШаблонСообщения = НСтр("ka='ელექტრონული დოკუმენტ %1-ის გადაცემისას შეცდომა წარმოიშვა, მიმღები %2.';ru='Возникла ошибка при передаче электронного документа %1, получатель %2.'");
...
"შეთანხმება ელექტრონული დოკუმენტების გაცვლის თაობაზე - %4.'");
						...
						ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ელექტრონული დოკუმენტის გადაცემა';ru='Передача электронного документа'"),
			...
			Для Каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
				...
				Если НЕ ЗначениеЗаполнено(Подпись.СсылкаНаЭДПрисоединенныйФайл) Тогда
					ТекстСообщения = НСтр("ka='ხელმოწერის შესანახად ელექტრონული დოკუმენტი ვერ მოიძებნა.';ru='Не удалось найти электронный документ для сохранения подписи.'");
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
...
Процедура ДобавитьПодпись(ПрисоединенныйФайл, ДанныеПодписи) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ka='ელექტრონული დოკუმენტ %1-ის ხელმოწერის შეცდომა';ru='Ошибка подписи электронного документа %1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ka='ეხ-ის ხელმოწერის დაყენება';ru='установка подписи ЭП'");
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивЭДКОбновлениюСтатуса) Экспорт
	...
	Для Каждого ПрисоединенныйФайл Из МассивЭДКОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='ელექტრონული დოკუმენტ %1-ის ხელმოწერის შეცდომა';ru='Ошибка подписи электронного документа %1'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
			Операция = НСтр("ka='ეხ-ის ხელმოწერის დაყენება';ru='установка подписи ЭП'");
		...
		Попытка
			...
			ИначеЕсли ТипОтправки =  Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				...
				Если КоличествоОтправленных <> КоличествоКОтправке Тогда
					ШаблонСообщения = НСтр("ka='ედბ-ის ოპერატორისთვის გაგზავნა ვერ მოხერხდა ""%1"".';ru='Не удалось отправить оператору ЭДО ""%1"".'");
...
"%3'");
			...
			ШаблонВидаОперации = НСтр("ka='ედ-ის პაკეტის გაგზავნა გამართვის მიხედვით: %1, გაცვლის მეთოდი %2';ru='отправка пакета ЭД по настройке: %1, способ обмена %2'");
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Если НастройкаЭДСуществует(СсылкаНаВладельца, ПараметрыЭД) Тогда
			Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоЧтенияЭД(Ложь) Тогда
				ШаблонСостоянияЭД = НСтр("ka='%1 ""ედბ-ის წასაკითხად არასაკმარისი უფლებებია""';ru='%1 «Недостаточно прав для чтения ЭДО»'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Результат) Тогда
					Гиперссылка = Ложь;
					Результат = НСтр("ka='ЭДО არ არის დაწყებული';ru='ЭДО не начат'");
				КонецЕсли;
				...
				Если СостояниеСоглашения = Перечисления.СостоянияСоглашенийЭД.ПроверкаТехническойСовместимости Тогда
					ШаблонСостоянияЭД = НСтр("ka='%1 (ტექნიკური თავსებადობის შემოწმება)';ru='%1 (проверка технической совместимости)'");
					Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(Результат) Тогда
				ШаблонСостоянияЭД = НСтр("ka='%1 (ედბ-ის გამართვა არ არის შეერთებული)';ru='%1 (настройка ЭДО не подключена)'");
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				Гиперссылка = Истина;
			Иначе
...
Процедура ЗаполнитьТекстПриглашенияКЭДО(ТекстПриглашения, ПараметрыЭД, СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями
		...
		ТекстПриглашения = НСтр("ka='მიმღებ ორგანიზაციასთან ედბ-ის მოქმედი გამართვა არ არის';ru='Нет действующей настройки ЭДО с организацией-получателем'");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЭД.Организация) И ЗначениеЗаполнено(ПараметрыЭД.Контрагент)
		...
		ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1 მოვიწვიოთ ელექტრონული დოკუმენტების გაცვლაზე 1C-ედბ-ში';ru='Пригласить %1 к обмену электронными документами в 1С-ЭДО'");
		...
		Если Результат[0].Пустой() Тогда
			Если Результат[1].Пустой() Тогда // нет профиля обмена через Такском
				...
				ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-ის შეერთება ედბ-ზე';ru='Подключить ""%1"" к ЭДО'");
				...
				Если Выборка.Следующий()
					...
					ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-თან შესაძლებელია ელექტრონული დოკუმენტების გაცვლა';ru='С ""%1"" возможен обмен электронными документами'");
					...
					Если ИмяРеализацияТоваровУслуг <> Неопределено
						...
						ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-თან ელექტრონული დოკუმენტების გაცვლის დაწყება ეხლა შეიძლება';ru='Обмен электронными документами с ""%1"" можно начать прямо сейчас'");
					ИначеЕсли ИмяПоступлениеТоваровУслуг <> Неопределено
						...
						ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-ისგან ელექტრონული დოკუმენტების მიღება, და არა ხელით შეყვანა, პირდაპირ ეხლავე შეიძლება';ru='Получать электронные документы от ""%1"", а не вводить руками, можно прямо сейчас'");
					КонецЕсли;
...
Процедура ОпределитьСтатусыПодписей(ЭД, ВыводитьСообщения = Ложь) Экспорт
	...
	Для каждого СтрокаЭП Из ОбъектЭД.ЭлектронныеПодписи Цикл
		...
		Исключение
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='ელექტრონული ხელმოწერის შემოწმება';ru='проверка электронной подписи'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		...
		Если ПолучаемДанные Тогда 
			...
			ИначеЕсли СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ka='არქივიდან ედ CML-ის ამოღება ხელმოწერისას';ru='Извлечение ЭД CML из архива при подписи'"), ТекстОшибки, ТекстСообщения);
				КонецПопытки;
				АрхивЧтение.Закрыть();
			...
			Если СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის არქივის ჩამოყალიბება ხელმოწერის დროს';ru='Формирование архива ЭД при подписи'"),
...
Функция НастройкиЭДОИСертификатыАвторизации(Знач ПрофилиНастроекЭДО, Знач ЭлектронныеДокументы = Неопределено,
	...
	Если ТипЗнч(СтМассивовСтруктурСертификатов) = Тип("Структура") Тогда
		...
		Если ВыполнятьКриптооперацииНаСервере И НЕ ЗначениеЗаполнено(МассивОтпечатковСервер) И ИспользоватьЭП Тогда
			...
			Исключение
				ВидОперации = НСтр("ka='სერთიფიკატების მონაცემთა მიღება';ru='Получение данных сертификатов'");
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если ТипЗнч(СтруктураСертификата) = Тип("Структура") Тогда
		...
		ИначеЕсли СтруктураСертификата.Свойство("МаркерЗашифрованный", МаркерЗашифрованный)
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				Операция = НСтр("ka='მარკერის გაშიფრვა';ru='Расшифровка маркера'");
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СвойстваСертификата(АдресДанныхСертификата) Экспорт
	...
	Если ВыбранныйСертификат=Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერთიფიკატი არ არის ნაპოვნი';ru='Сертификат не найден'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыЕстьИдея() Экспорт
	...
	ШаблонКонфигурации = НСтр("ka='%1 (%2)';ru='%1 (%2)'");
...
Функция СостояниеВерсииЭД(СсылкаНаВладельца)
	...
	Если СтруктураВерсииЭД.Свойство("СостояниеВерсииЭД") Тогда
		...
		Если ДобавитьПричинуЗакрытия Тогда
			...
			Причина = СтрЗаменить(НСтр("ka=', მიზეზი: %1';ru=', причина: %1'"), "%1", ?(ЗначениеЗаполнено(Комментарий), Комментарий, НСтр("ka='არ არის მითითებული';ru='не указана'")));
...
Функция ПолучитьТекстСводногоСостоянияЭД(СсылкаНаВладельца)
	...
	Если ТаблицаДанных.Количество() > 0 Тогда
		...
		Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
			ТекстСводногоСостояния = НСтр("ka='ედბ იძულებით დაიხურა';ru='ЭДО закрыт принудительно'");
		Иначе
			Если ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				ТекстСНашейСтороны = НСтр("ka='ჩვენი მხრიდან';ru='с нашей стороны'");
			КонецЕсли;
			Если ЗначениеЗаполнено(Строка.ДействияСоСтороныДругогоУчастника)
				...
				ТекстСоСтороныДругогоУчастника = НСтр("ka='სხვა მონაწილეების მხრიდან';ru='со стороны других участников'");
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекстСНашейСтороны) 
				Или ЗначениеЗаполнено(ТекстСоСтороныДругогоУчастника) Тогда
				
				ТекстСводногоСостояния = НСтр("ka='საჭიროა მოქმედებები';ru='Требуются действия'")+ " " + ТекстСНашейСтороны
			...
			ИначеЕсли ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
					ТекстСводногоСостояния = НСтр("ka='ედბ დასრულებულია';ru='ЭДО завершен'");
				ИначеЕсли Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
					ТекстСводногоСостояния = НСтр("ka='ედბ დასრულდა შესწორებით';ru='ЭДО завершен с исправлением'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
"%2'");
		...
		ШаблонСообщения = НСтр("ka='შეცდომა ელ.დოკუმენტების გაგზავნისას ედბ-ის გამართვების პროფილის მიხედვით: %1, გაცვლის მეთოდი: %2.';ru='Ошибка при отправке эл.документов по профилю настроек ЭДО: %1, способ обмена: %2.'"); 
		...
		ВидОперации = НСтр("ka='ელ. დოკუმენტების გაგზავნა';ru='Отправка эл.документов'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							ВидОперации,
							ТекстОшибки,
							ТекстСообщения);
...
Функция СформироватьТемуПисьма(Отправитель, Получатель)
	...
	Возврат НСтр("ka='ელ.დოკუმენტების გაცვლა:';ru='Обмен эл.документами:'")+ " " + Отправитель + ?(ЗначениеЗаполнено(Получатель), " -> " + Получатель, "");
...
"%2'");
		...
		ВидОперации = НСтр("ka='ხელმოწერების შემოწმების შედეგების შენახვა';ru='Сохранение результатов проверки подписей'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПодписатьЭДОпределеннымСертификатом(ДобавленныеФайлы,
	...
	Если КриптоСертификат <> Неопределено Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='სერთიფიკატის შემოწმება კორექტულობაზე';ru='проверка сертификата на корректность'"),
											  ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
											  ТекстСообщения);
			Для Каждого НеПодписанныйЭД Из ФайлыНаПодпись Цикл
				МассивНеПодписанныхЭД.Добавить(НеПодписанныйЭД);
		...
		Для Каждого ТекДокумент Из ФайлыНаПодпись Цикл
			...
			СтруктураДанныхПодписи = Новый Структура("Сертификат, ПарольПользователя, Комментарий", КриптоСертификат, ПараметрыСертификата.ПарольПользователя,
				НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа'"));
			Попытка
...
"%2';ka='სერვერზე ხელმოწერის შეცდომა. აუცილებელია კრიპტოგრაფიის სერთიფიკატის ტესტის ჩატარება: %1-ისთვის. %2'");
				...
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ka='ეხ-ის ხელმოწერის დაყენება';ru='установка подписи ЭП'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
						ТекстСообщения);
	...
	Для Каждого СсылкаНаЭД Из МассивЭД Цикл
		...
		Если ЭтоИзвещение Тогда
			...
			Если НЕ (ПараметрыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДОЭСФ
				...
				ШаблонСообщения = НСтр("ka='ედ %1 ტიპისთვის არ არის გათვალისწინებული მიღების თაობაზე შეტყობინების ჩამოყალიბება.';ru='Для типа ЭД %1 не предусмотрено формирование Извещения о получении.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
		...
		ИначеЕсли СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				Если НЕ ЭлектронноеВзаимодействиеСлужебный.ВозможноИзвлечьФайлы(АрхивЧтение, ПапкаДляРаспаковки) Тогда
					ТекстСообщения = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ПолучитьСообщениеОбОшибке("006");
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД'"), ТекстОшибки, ТекстСообщения);
				ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ПапкаДляРаспаковки);
				Возврат "";
			КонецПопытки;
