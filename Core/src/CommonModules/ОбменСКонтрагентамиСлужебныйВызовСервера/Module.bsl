...
"организация %1, "
"контрагент %2, "
"договор %3.';ka='ედბ-ის გამართვების დადგენა ვერ მოხერხდა შემდეგი პარამეტრებისთვის: "
"ორგანიზაცია %1, "
"კონტრაგენტი %2, "
"ხელშეკრულება %3.'");
	...
	Пока Выборка.Следующий() Цикл
		Если ТипЗнч(Выборка.ДокументПотомок) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда
			...
			Исключение
				...
				ВидОперации = НСтр("ka='საფუძველ-დოკუმენტ ""%1""-ის ჩაწერა დოკუმენტ ""%2""-ში';ru='запись документа-основания ""%1"" в документ ""%2""';en='Recording paper-base ""%1"" in the document ""%2""'");
		...
		Для каждого ЭлектронныйДокумент Из СтруктураПакетаЭД.МассивЭД Цикл
			...
			Если ЭлектронныйДокумент.Зашифрован Тогда
				...
				Если ПарольДоступаКЗакрытомуКлючу = Неопределено Тогда
					ШаблонСообщения = НСтр("ka='სერთიფიკატზე პაროლის მიღება ვერ მოხერხდა: %1';ru='Не удалось получить пароль к сертификату: %1';en='Could not get the password to the certificate% 1'");
				...
				Исключение
					ШаблонСообщения = НСтр("ka='ედ-ის პაკეტის გაშიფრვა: %1';ru='Расшифровка пакета ЭД: %1';en='Explanation packet ED: 1%'");
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
...
"%2.';ka='პაკეტ %1-ის განფუთვისას შეცდომა წარმოიშვა:"
"%2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураПакетаЭД.ПакетЭДСсылка,
				ОписаниеОшибки);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД';en='Unpacking ED Pack'"), ТекстСообщения,
				ТекстСообщения);
		...
		Для каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
			...
			Если НЕ ЗначениеЗаполнено(Подпись.АдресЭД) Тогда
				ОписаниеОшибки = НСтр("ka='საპასუხო ხელმოწერის შესამოწმებლად საწყისი მონაცემები ვერ მოიძებნა.';ru='Не удалось найти исходные данные для проверки ответной подписи.';en='Could not find the original data to verify the signature of the response.'");
				Прервать;
			КонецЕсли;
			...
			Исключение
				ШаблонСообщения = НСтр("ka='სერთიფიკატების მიღება ხელმოწერიდან: %1';ru='Получение сертификатов из подписи: %1';en='Preparation of certificate signature: 1%'");
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Прервать;
			КонецПопытки;
	...
	Для каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			Исключение
				...
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის არქივის განფუთვა';ru='Распаковка архива пакета ЭД';en='Unpacking ED Pack'"), ТекстОшибки,
					ТекстСообщения);
		...
		Исключение
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД';en='Unpacking ED Pack'"), ТекстОшибки,
				ТекстСообщения);
...
"- полученный пакет не предназначен для текущей ИБ (первичный титул был сформирован в другой базе/копии текущей базы).';ka='Для полученного ответного титула (%1) не найден исходящий ЭД."
"Возможные причины:"
"- первичный титул был удален из текущей базы;"
"- полученный пакет не предназначен для текущей ИБ (первичный титул был сформирован в другой базе/копии текущей базы).'");
				...
				Если ЭлектронныйДокумент.ТипДанных = "СлужебныйЭД" Тогда
					...
					Если Не ПервичныйЭДНайден Тогда
						ТекстСообщения = НСтр("ka='სამსახურებრივი ელექტრონული დოკუმენტისთვის მფლობელი ვერ მოიძებნა.';ru='Не удалось найти владельца для служебного электронного документа.';en='Unable to find the owner for the service of the electronic document.'");
					...
					Если СтруктураФайлаЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.Ошибка Тогда
						...
						ШаблонСообщения = НСтр("ka='ელექტრონული დოკუმენტ %1-ის გადაცემისას შეცდომა წარმოიშვა, მიმღები %2.';ru='Возникла ошибка при передаче электронного документа %1, получатель %2.';en='An error in the transmission of an electronic document 1%, 2% recipient.'");
...
"Соглашение об обмене электронными документами - %4.';ka='ელექტრონული დოკუმენტ %1-ის გადაცემისას წარმოიშვა შეცდომა: %2."
"საინფორმაციო ბაზის დოკუმენტი (ელექტრონული დოკუმენტის მფლობელი) - %3;"
"შეთანხმება ელექტრონული დოკუმენტების გაცვლის თაობაზე - %4.'");
						...
						ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ელექტრონული დოკუმენტის გადაცემა';ru='Передача электронного документа';en='The transfer of the electronic document'"),
			...
			Для Каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
				...
				Если НЕ ЗначениеЗаполнено(Подпись.СсылкаНаЭДПрисоединенныйФайл) Тогда
					ТекстСообщения = НСтр("ka='ხელმოწერის შესანახად ელექტრონული დოკუმენტი ვერ მოიძებნა.';ru='Не удалось найти электронный документ для сохранения подписи.';en='Unable to find an electronic document to save the signature.'");
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
...
"%2';ka='ედ-ის პაკეტის განფუთვისას: %1 შეცდომა წარმოიშვა:"
"%2'");
...
Процедура ДобавитьПодпись(ПрисоединенныйФайл, ДанныеПодписи) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ka='ელექტრონული დოკუმენტ %1-ის ხელმოწერის შეცდომა';ru='Ошибка подписи электронного документа %1';en='Error electronic document signature% 1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ka='ეხ-ის ხელმოწერის დაყენება';ru='установка подписи ЭП';en='Installation EP signatures'");
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивЭДКОбновлениюСтатуса) Экспорт
	...
	Для Каждого ПрисоединенныйФайл Из МассивЭДКОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='ელექტრონული დოკუმენტ %1-ის ხელმოწერის შეცდომა';ru='Ошибка подписи электронного документа %1';en='Error electronic document signature% 1'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
			Операция = НСтр("ka='ეხ-ის ხელმოწერის დაყენება';ru='установка подписи ЭП';en='Installation EP signatures'");
		...
		Попытка
			...
			ИначеЕсли ТипОтправки =  Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				...
				Если КоличествоОтправленных <> КоличествоКОтправке Тогда
					ШаблонСообщения = НСтр("ka='ედბ-ის ოპერატორისთვის გაგზავნა ვერ მოხერხდა ""%1"".';ru='Не удалось отправить оператору ЭДО ""%1"".';en='Failed to send EDI operator ""%1"".'");
...
"%3';ka='პაკეტის გაგზავნის შეცდომა გამართვის მიხედვით: %1, გაცვლის მეთოდი: %2"
"%3'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				РеквизитыПакетаЭД.НастройкаЭДО, РеквизитыПакетаЭД.СпособОбменаЭД, КраткоеПредставлениеОшибки);
			ШаблонВидаОперации = НСтр("ka='ედ-ის პაკეტის გაგზავნა გამართვის მიხედვით: %1, გაცვლის მეთოდი %2';ru='отправка пакета ЭД по настройке: %1, способ обмена %2';en='Sending ED package for setting up: 1%, 2% exchange method'");
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Если НастройкаЭДСуществует(СсылкаНаВладельца, ПараметрыЭД) Тогда
			Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоЧтенияЭД(Ложь) Тогда
				ШаблонСостоянияЭД = НСтр("ka='%1 ""ედბ-ის წასაკითხად არასაკმარისი უფლებებია""';ru='%1 «Недостаточно прав для чтения ЭДО»';en='% 1 ""You are not authorized to read EDI""'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Результат) Тогда
					Гиперссылка = Ложь;
					Результат = НСтр("ka='ЭДО არ არის დაწყებული';ru='ЭДО не начат';en='EDI is not started'");
				КонецЕсли;
				...
				Если СостояниеСоглашения = Перечисления.СостоянияСоглашенийЭД.ПроверкаТехническойСовместимости Тогда
					ШаблонСостоянияЭД = НСтр("ka='%1 (ტექნიკური თავსებადობის შემოწმება)';ru='%1 (проверка технической совместимости)';en='1% (test technical compatibility)'");
					Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(Результат) Тогда
				ШаблонСостоянияЭД = НСтр("ka='%1 (ედბ-ის გამართვა არ არის შეერთებული)';ru='%1 (настройка ЭДО не подключена)';en='1% (EDM setting is not connected)'");
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				Гиперссылка = Истина;
			Иначе
...
Процедура ЗаполнитьТекстПриглашенияКЭДО(ТекстПриглашения, ПараметрыЭД, СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями
		...
		ТекстПриглашения = НСтр("ka='მიმღებ ორგანიზაციასთან ედბ-ის მოქმედი გამართვა არ არის';ru='Нет действующей настройки ЭДО с организацией-получателем';en='No current EDI configuration with the organization of the recipient'");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЭД.Организация) И ЗначениеЗаполнено(ПараметрыЭД.Контрагент)
		...
		ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1 მოვიწვიოთ ელექტრონული დოკუმენტების გაცვლაზე 1C-ედბ-ში';ru='Пригласить %1 к обмену электронными документами в 1С-ЭДО';en='Invite% 1 to the exchange of electronic documents in the 1S-EDM'");
		...
		Если Результат[0].Пустой() Тогда
			Если Результат[1].Пустой() Тогда // нет профиля обмена через Такском
				...
				ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-ის შეერთება ედბ-ზე';ru='Подключить ""%1"" к ЭДО';en=""Connect '% 1' to the EDT""");
				...
				Если Выборка.Следующий()
					...
					ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-თან შესაძლებელია ელექტრონული დოკუმენტების გაცვლა';ru='С ""%1"" возможен обмен электронными документами';en='With the ""%1"" can exchange electronic documents'");
					...
					Если ИмяРеализацияТоваровУслуг <> Неопределено
						...
						ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-თან ელექტრონული დოკუმენტების გაცვლის დაწყება ეხლა შეიძლება';ru='Обмен электронными документами с ""%1"" можно начать прямо сейчас';en='Exchange of electronic documents with the ""%1"" you can start now!'");
					ИначеЕсли ИмяПоступлениеТоваровУслуг <> Неопределено
						...
						ШаблонТекстаНавигационнойСсылки = НСтр("ka='%1-ისგან ელექტრონული დოკუმენტების მიღება, და არა ხელით შეყვანა, პირდაპირ ეხლავე შეიძლება';ru='Получать электронные документы от ""%1"", а не вводить руками, можно прямо сейчас';en='Receive electronic documents from ""%1"", instead of typing hands, you can now!'");
					КонецЕсли;
				...
				КонецЕсли;
			Иначе
				ТекстПриглашения = НСтр("ru='Настройте ЭДО с контрагентом %1';en='Adjust EDC counterparty 1%'");
...
Процедура ОпределитьСтатусыПодписей(ЭД, ВыводитьСообщения = Ложь) Экспорт
	...
	Для каждого СтрокаЭП Из ОбъектЭД.ЭлектронныеПодписи Цикл
		...
		Исключение
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='ელექტრონული ხელმოწერის შემოწმება';ru='проверка электронной подписи';en='electronic signature verification'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		...
		Если ПолучаемДанные Тогда 
			...
			ИначеЕсли СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ka='არქივიდან ედ CML-ის ამოღება ხელმოწერისას';ru='Извлечение ЭД CML из архива при подписи';en='Removing ED CML from the archive when signing'"), ТекстОшибки, ТекстСообщения);
				КонецПопытки;
				АрхивЧтение.Закрыть();
			...
			Если СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის არქივის ჩამოყალიბება ხელმოწერის დროს';ru='Формирование архива ЭД при подписи';en='Formation archive ED with signatures'"),
			...
			Если ТекСтрока.СтатусЭД = Перечисления.СтатусыЭД.Сформирован 
				...
				Исключение
					...
					Если ЗначениеЗаполнено(ТекстСообщения) Тогда
						Операция = НСтр("ru='Утверждение ЭД';en='ED Adoption'");
		...
		Если ТЗ_Сертификатов.Количество() = 0 И ЗначениеЗаполнено(ЭД) Тогда
			...
			Если ДоступныеСертификаты[0].Количество > 0 Тогда
				...
				Если Не ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
					ШаблонСообщения = НСтр("ka='ედბ-ის გამართვების პროფილში: %1"
"ეხ-ის არცერთი ხელმისაწვდომი სერთიფიკატი არ მოიძებნა.';ru='В профиле настроек ЭДО: %1"
...
"Проверьте настройки криптографии.';ka='ელექტრონულ დოკუმენტზე ხელმოწერის შეცდომა. შეამოწმეთ კრიპტოგრაფიის გამართვები.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				
			ИначеЕсли ДопПараметры.Свойство("ОшибкаНастройкиСертификата", ОшибкаНастройкиСертификата)
				И ОшибкаНастройкиСертификата Тогда
				ТекстСообщения = НСтр("ru='Ошибка подписания электронного документа."
"Проверьте настройки сертификатов.';ka='ელექტრონულ დოკუმენტზე ხელმოწერის შეცდომა. შეამოწმეთ სერთიფიკატების გამართვები.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция НастройкиЭДОИСертификатыАвторизации(Знач ПрофилиНастроекЭДО, Знач ЭлектронныеДокументы = Неопределено,
	...
	Если ТипЗнч(СтМассивовСтруктурСертификатов) = Тип("Структура") Тогда
		...
		Если ВыполнятьКриптооперацииНаСервере И НЕ ЗначениеЗаполнено(МассивОтпечатковСервер) И ИспользоватьЭП Тогда
			...
			Исключение
				ВидОперации = НСтр("ka='სერთიფიკატების მონაცემთა მიღება';ru='Получение данных сертификатов';en='Preparation of these certificates'");
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если ТипЗнч(СтруктураСертификата) = Тип("Структура") Тогда
		...
		ИначеЕсли СтруктураСертификата.Свойство("МаркерЗашифрованный", МаркерЗашифрованный)
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				Операция = НСтр("ka='მარკერის გაშიფრვა';ru='Расшифровка маркера';en='deciphering marker'");
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СвойстваСертификата(АдресДанныхСертификата) Экспорт
	...
	Если ВыбранныйСертификат=Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერთიფიკატი არ არის ნაპოვნი';ru='Сертификат не найден';en='Certificate can not be found'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыЕстьИдея() Экспорт
	...
	ШаблонКонфигурации = НСтр("ka='%1 (%2)';ru='%1 (%2)';en='%12)'");
...
Функция СостояниеВерсииЭД(СсылкаНаВладельца)
	...
	Если СтруктураВерсииЭД.Свойство("СостояниеВерсииЭД") Тогда
		...
		Если СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
			СостояниеВерсии = НСтр("ka='ედბ დასრულებულია';ru='ЭДО завершен';en='EDI is completed'");
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
			СостояниеВерсии = НСтр("ka='ედბ დასრულდა შესწორებით';ru='ЭДО завершен с исправлением';en='EDC completed correction'");
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.Аннулирован Тогда
			СостояниеВерсии = НСтр("ka='ЭДО ანულირებულია';ru='ЭДО аннулирован';en='EDO canceled'");
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
			СостояниеВерсии = НСтр("ka='ედბ იძულებით დაიხურა';ru='ЭДО закрыт принудительно';en='EDO closed forcibly'");
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.Отклонен Тогда
			СостояниеВерсии = НСтр("ka='ედბ დახურულია გადახრით';ru='ЭДО закрыт с отклонением';en='EDI is closed with a deviation'");
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.НеСформирован 
			...
			СостояниеВерсии = НСтр("ru='ЭДО не начат';en='EDI is not started'");
		Иначе
		...
		Если ДобавитьПричинуЗакрытия Тогда
			...
			Причина = СтрЗаменить(НСтр("ka=', მიზეზი: %1';ru=', причина: %1';en='Reason: 1%'"), "%1", ?(ЗначениеЗаполнено(Комментарий), Комментарий, НСтр("ka='არ არის მითითებული';ru='не указана';en='not specified'")));
...
Функция ПолучитьТекстСводногоСостоянияЭД(СсылкаНаВладельца)
	...
	Если ТаблицаДанных.Количество() > 0 Тогда
		...
		Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
			ТекстСводногоСостояния = НСтр("ka='ედბ იძულებით დაიხურა';ru='ЭДО закрыт принудительно';en='EDO closed forcibly'");
		Иначе
			Если ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				ТекстСНашейСтороны = НСтр("ka='ჩვენი მხრიდან';ru='с нашей стороны';en='from our side'");
			КонецЕсли;
			Если ЗначениеЗаполнено(Строка.ДействияСоСтороныДругогоУчастника)
				...
				ТекстСоСтороныДругогоУчастника = НСтр("ka='სხვა მონაწილეების მხრიდან';ru='со стороны других участников';en='the other parties'");
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекстСНашейСтороны) 
				Или ЗначениеЗаполнено(ТекстСоСтороныДругогоУчастника) Тогда
				
				ТекстСводногоСостояния = НСтр("ka='საჭიროა მოქმედებები';ru='Требуются действия';en='action is required'")+ " " + ТекстСНашейСтороны
			...
			ИначеЕсли ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
					ТекстСводногоСостояния = НСтр("ka='ედბ დასრულებულია';ru='ЭДО завершен';en='EDI is completed'");
				ИначеЕсли Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
					ТекстСводногоСостояния = НСтр("ka='ედბ დასრულდა შესწორებით';ru='ЭДО завершен с исправлением';en='EDC completed correction'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
"%2';ka='შეცდომა ედბ-ის გამართვების პროფილით %1 ელექტრონული ფოსტის სერვერზე შეტყობინების გაგზავნისას."
"%2'");
		...
		ШаблонСообщения = НСтр("ka='შეცდომა ელ.დოკუმენტების გაგზავნისას ედბ-ის გამართვების პროფილის მიხედვით: %1, გაცვლის მეთოდი: %2.';ru='Ошибка при отправке эл.документов по профилю настроек ЭДО: %1, способ обмена: %2.';en='Error sending el.dokumentov on profile of EDM settings:% 1, the exchange method:% 2.'"); 
		...
		ВидОперации = НСтр("ka='ელ. დოკუმენტების გაგზავნა';ru='Отправка эл.документов';en='sending el.dokumentov'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							ВидОперации,
							ТекстОшибки,
							ТекстСообщения);
...
Функция СформироватьТемуПисьма(Отправитель, Получатель)
	...
	Возврат НСтр("ka='ელ.დოკუმენტების გაცვლა:';ru='Обмен эл.документами:';en='Exchange el.dokumentami:'")+ " " + Отправитель + ?(ЗначениеЗаполнено(Получатель), " -> " + Получатель, "");
...
Процедура СохранитьРезультатыПроверкиПодписей(ЭД, МассивРезультатов) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ka='ობიექტ %1-ის ბლოკირება ვერ მოხერხდა"
"%2';ru='Не удалось заблокировать объект %1"
"%2'");
		
		ВидОперации = НСтр("ka='ხელმოწერების შემოწმების შედეგების შენახვა';ru='Сохранение результатов проверки подписей';en='Saving the signature verification results'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПодписатьЭДОпределеннымСертификатом(ДобавленныеФайлы,
	...
	Если КриптоСертификат <> Неопределено Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='სერთიფიკატის შემოწმება კორექტულობაზე';ru='проверка сертификата на корректность';en='certificate validations'"),
											  ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
											  ТекстСообщения);
			Для Каждого НеПодписанныйЭД Из ФайлыНаПодпись Цикл
				МассивНеПодписанныхЭД.Добавить(НеПодписанныйЭД);
		...
		Для Каждого ТекДокумент Из ФайлыНаПодпись Цикл
			...
			СтруктураДанныхПодписи = Новый Структура("Сертификат, ПарольПользователя, Комментарий", КриптоСертификат, ПараметрыСертификата.ПарольПользователя,
				НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа';en='The signing of the electronic document'"));
			Попытка
...
"%2';ka='სერვერზე ხელმოწერის შეცდომა. აუცილებელია კრიპტოგრაფიის სერთიფიკატის ტესტის ჩატარება: %1-ისთვის. %2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СертификатПодписи,
					КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ka='ეხ-ის ხელმოწერის დაყენება';ru='установка подписи ЭП';en='Installation EP signatures'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
						ТекстСообщения);
...
"Необходимо указать сетевой каталог для обмена.';ka='აღნიშნული კატალოგი %1 არ შეიძლება, რომ გამოყენებული იქნას გასაცვლელად, ვინაიდან იგი სერვერიდან მიუწვდომელია. აუცილებელია ქსელური კატალოგის მითითება გასაცვლელად.'");
...
Процедура УстановитьОтветственногоЭД(Знач СписокОбъектов, Знач НовыйОтветственный, КоличествоОбработанныхЭД,
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось установить ответственного электронного документа (%1). %2';en='Unable to establish a responsible electronic document (% 1). 2%'"), 
			Выборка.Ссылка,
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для Каждого СсылкаНаЭД Из МассивЭД Цикл
		...
		Если ЭтоИзвещение Тогда
			...
			Если НЕ (ПараметрыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДОЭСФ
				...
				ШаблонСообщения = НСтр("ka='ედ %1 ტიპისთვის არ არის გათვალისწინებული მიღების თაობაზე შეტყობინების ჩამოყალიბება.';ru='Для типа ЭД %1 не предусмотрено формирование Извещения о получении.';en='For 1% of ED formation provided on receipt notices.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
		...
		ИначеЕсли СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				Если НЕ ЭлектронноеВзаимодействиеСлужебный.ВозможноИзвлечьФайлы(АрхивЧтение, ПапкаДляРаспаковки) Тогда
					ТекстСообщения = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ПолучитьСообщениеОбОшибке("006");
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД';en='Unpacking ED Pack'"), ТекстОшибки, ТекстСообщения);
				ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ПапкаДляРаспаковки);
				Возврат "";
			КонецПопытки;
