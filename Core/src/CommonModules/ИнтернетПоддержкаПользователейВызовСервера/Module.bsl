...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვისთან შეერთებისას';ru='Ошибка при подключении к сервису Интернет-поддержки';en='Error connecting to the service online support'");
		КонецЕсли;
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვერიდან მონაცემთა მიღებისას.';ru='Ошибка при получении данных с сервера Интернет-поддержки.';en='Error retrieving information from the Internet Information Server Support.'");
		КонецЕсли;
...
Процедура ЗаписатьОбщиеПараметрыИПП(Знач ОбщиеПараметры) Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ka='ინტერნეტ-მხარდაჭერის პარამეტრების ჩასაწერად არასაკმარისი უფლებებია.';ru='Недостаточно прав для записи параметров Интернет-поддержки.';en='Insufficient permissions to write online support options.'");
	КонецЕсли;
...
Функция ДопПараметрыОтправкиСообщения(Знач ДопПараметры) Экспорт
	...
	Вложения.Добавить(
		Новый Структура(
			"Представление, Текст",
			НСтр("ka='ტექნიკური ინფორმაცია. .txt';ru='Техническая информация.txt';en='Technical informatsiya.txt'"),
...
Функция ШаблонТекстаСообщения(ДопПараметры)
	...
	Возврат НСтр("ka='გამარჯობათ!"
""
"%msgtxt"
""
"პროგრამული პროდუქტის სარეგისტრაციო ნომერი: <მიუთითეთ სარეგ. ნომერი>;"
"ორგანიზაცია: <მიუთითეთ ორგანიზაციის სახელწოდება>."
"პატივისცემით,"
".';ru='Здравствуйте!"
""
"%msgtxt"
...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры)
	...
	Если ИмяПрограммы = "Unknown" Тогда
		ИмяПрограммы = НСтр("ka='<არ არის შევსებული>';ru='<Не заполнено>';en='<Not filled>'");
	КонецЕсли;
...
"Вид приложения: %8;"
"Режим: %9;';ka='ტექნიკური ინფორმაცია პროგრამის შესახებ:"
"პროგრამის სახელწოდება: %1"
"პროგრამის სახელი: %2;"
"პროგრამის ვერსია: %3;"
"მომწოდებელი: %4;"
"პლატფორმა 1С:საწარმოს ვერსია: %5;"
"ინტერნეტ-მხარდაჭერის ბიბლიოთეკის ვერსია: %6;"
"სტანდარტული ქვესისტემების ბიბლიოთეკის ვერსია: %7;"
"დანართის ტიპი: %8;"
"რეჟიმი: %9;'");
	...
	Результат =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ВидПриложения,
			?(ЭтоФайловаяИБ, НСтр("ka='ფაილური';ru='Файловый';en='File'"), НСтр("ka='ჩრდილოეთის';ru='Серверный';en='server'")));
...
"Права для подключения Интернет-поддержки: %3;';ka='სრული უფლებები: %1;"
"ადმინისტრატორის უფლებები: %2;"
"უფლებები ინტერნეტ-მხარდაჭერის შესაერთებლად: %3;'"),
			?(Пользователи.ЭтоПолноправныйПользователь(, Ложь, Ложь), НСтр("ka='есть';ru='есть';en='there is'"), НСтр("ka='нет';ru='нет';en='not'")),
			?(Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь), НСтр("ka='есть';ru='есть';en='there is'"), НСтр("ka='нет';ru='нет';en='not'")),
			?(ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП(), НСтр("ka='есть';ru='есть';en='there is'"), НСтр("ka='нет';ru='нет';en='not'")));
	
	// Личные данные:
	Результат = Результат
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ლოგ-ინი ინტერნეტ-მხარდაჭერის შესაერთებლად: %1;"
"პაროლი ინტერნეტ-მხარდაჭერის შესაერთებლად: %2;"
"პროგრამული პროდუქტიის რეგისტრაციის ნომერი: %3;';ru='Логин для подключения Интернет-поддержки: %1;"
"Пароль для подключения Интернет-поддержки: %2;"
"Регистрационный номер программного продукта: %3;'"),
			ДопПараметры.Логин,
			?(ДопПараметры.ПарольЗаполнен, НСтр("ka='ევსებულია';ru='заполнен';en='full'"), НСтр("ka='არ არის შევსებული';ru='не заполнен';en='is empty'")),
	...
	Результат = Результат
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ინტერნეტ-მხარდაჭერის სერვერებთან შეერთების გამართვები:"
" ტაიმაუტი: %1;"
" შეერთების დაყენება 1С:საწარმოს სერვერზე: %2;"
" დომენური ზონა: %3';ru='Настройки соединения с серверами Интернет-поддержки:"
"	Таймаут: %1;"
	...
	Если ДоступнаРаботаСНастройкамиКлиентаЛицензирования() Тогда
		...
		Если Не ПустаяСтрока(ИДКонфигурации) Тогда
			Результат = Результат + Символы.ПС
				+ НСтр("ka='ლიცენზირების კლიენტის სახელი:';ru='Имя клиента лицензирования:';en='Client Name of licensing:'") + " "
				+ КлиентЛицензирования.ИмяКлиентаЛицензирования()
				+ Символы.ПС + НСтр("ka='კონფიგურაციის იდენტიფიკატორი:';ru='Идентификатор конфигурации:';en='Configuration ID:'")
