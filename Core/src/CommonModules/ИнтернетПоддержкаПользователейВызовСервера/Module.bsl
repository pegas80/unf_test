...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვისთან შეერთებისას';ru='Ошибка при подключении к сервису Интернет-поддержки'");
		КонецЕсли;
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვერიდან მონაცემთა მიღებისას.';ru='Ошибка при получении данных с сервера Интернет-поддержки.'");
		КонецЕсли;
...
Процедура ЗаписатьОбщиеПараметрыИПП(Знач ОбщиеПараметры) Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ka='ინტერნეტ-მხარდაჭერის პარამეტრების ჩასაწერად არასაკმარისი უფლებებია.';ru='Недостаточно прав для записи параметров Интернет-поддержки.'");
	КонецЕсли;
...
Функция ДопПараметрыОтправкиСообщения(Знач ДопПараметры) Экспорт
	...
	Вложения.Добавить(
		Новый Структура(
			"Представление, Текст",
			НСтр("ka='ტექნიკური ინფორმაცია. .txt';ru='Техническая информация.txt'"),
...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры)
	...
	Если ИмяПрограммы = "Unknown" Тогда
		ИмяПрограммы = НСтр("ka='<არ არის შევსებული>';ru='<Не заполнено>'");
	КонецЕсли;
...
"რეჟიმი: %9;'");
	...
	Результат =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ВидПриложения,
			?(ЭтоФайловаяИБ, НСтр("ka='ფაილური';ru='Файловый'"), НСтр("ka='ჩრდილოეთის';ru='Серверный'")));
...
"უფლებები ინტერნეტ-მხარდაჭერის შესაერთებლად: %3;'"),
			?(Пользователи.ЭтоПолноправныйПользователь(, Ложь, Ложь), НСтр("ka='есть';ru='есть'"), НСтр("ka='нет';ru='нет'")),
			?(Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь), НСтр("ka='есть';ru='есть'"), НСтр("ka='нет';ru='нет'")),
			?(ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП(), НСтр("ka='есть';ru='есть'"), НСтр("ka='нет';ru='нет'")));
...
"Регистрационный номер программного продукта: %3;'"),
			ДопПараметры.Логин,
			?(ДопПараметры.ПарольЗаполнен, НСтр("ka='ევსებულია';ru='заполнен'"), НСтр("ka='არ არის შევსებული';ru='не заполнен'")),
	...
	Если ДоступнаРаботаСНастройкамиКлиентаЛицензирования() Тогда
		...
		Если Не ПустаяСтрока(ИДКонфигурации) Тогда
			Результат = Результат + Символы.ПС
				+ НСтр("ka='ლიცენზირების კლიენტის სახელი:';ru='Имя клиента лицензирования:'") + " "
				+ КлиентЛицензирования.ИмяКлиентаЛицензирования()
				+ Символы.ПС + НСтр("ka='კონფიგურაციის იდენტიფიკატორი:';ru='Идентификатор конфигурации:'")
