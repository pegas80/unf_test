...
Процедура ДобавитьСообщениеВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди, СообщениеОбмена) Экспорт
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მობილურ კლიენტთან გაცვლა. შეტყობინების დამატება გაცვლის შეტყობინებების რიგში';ru='Обмен с мобильным клиентом.Добавление сообщения в очередь сообщений обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ВызватьИсключение(НСтр("ka='მონაცემთა გაგზავნის შესრულება ვერ მოხერხდა. დაწვრილებით, იხ. საინფორმაციო ბაზის რეგისტრაციის ჟურნალში.';ru='Не удалось выполнить отправку данных. Подробности см. в Журнале регистрации информационной базы.'"));
...
Процедура ПроверитьОчередьСообщенийОбмена(УзелОбмена, Знач НомерПринятого) Экспорт
	...
	Пока ВыборкаСообщенийОбмена.Следующий() Цикл
		...
		ИначеЕсли ВыборкаСообщенийОбмена.НомерСообщения > НомерСообщенияОчереди Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მობილურ კლიენტთან გაცვლა. გაცვლის შეტყობინებების რიგის შემოწმება';ru='Обмен с мобильным клиентом.Проверка очереди сообщений обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				ВыборкаСообщенийОбмена.МобильныйКлиент,
				НСтр("ka='დარღვეულია გაცვლის შეტყობინებების თანმიმდევრობა.';ru='Нарушен порядок следования сообщений обмена.'"));
