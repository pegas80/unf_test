...
Функция ДвоичныеДанныеПрисоединенногоФайла(Знач СсылкаНаОбъект) Экспорт
	...
	Если НЕ МассивФайлов.Количество() Тогда
		ТекстОшибки = НСтр("ka='ობიექტის მიერთებული ფაილი არ არის აღმოჩენილი: %1';ru='Не обнаружен присоединенный файл объекта: %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, СсылкаНаОбъект);
		Операция = НСтр("ka='მიერთებული ფაილის ორობითი მონაცემების მიღება';ru='Получение двоичных данных присоединенного файла'");
...
Функция ВерсияАктуальнойВК(ИмяВнешнегоМодуля) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		НаименованиеЗадания = НСтр("ka='1С:დირექტბანკი გარე კომპონენტის განახლება (%1).';ru='1С:ДиректБанк: Обновление внешней компоненты (%1).'");
...
Функция СкачатьВКНаСервере(Знач УникальныйИдентификатор, Знач URL) Экспорт
	...
	Попытка
		НазваниеЗадания = НСтр("ka='1С:ДиректБанк: Скачивание внешней компоненты с сервера банка';ru='1С:ДиректБанк: Скачивание внешней компоненты с сервера банка'");
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно = Ложь;
		Текст = НСтр("ka='Не удалось скачать внешний модуль с сервера банка.';ru='Не удалось скачать внешний модуль с сервера банка.'");
		ВидОперации = НСтр("ka='Скачивание внешней компоненты с сервера банка';ru='Скачивание внешней компоненты с сервера банка'");
...
Функция ИнформацияОВКБанка(АдресВнешнейКомпоненты) Экспорт
	...
	Если Не НайденаИнформация Тогда
		Операция = НСтр("ka='Чтение информации о файле внешнего модуля.';ru='Чтение информации о файле внешнего модуля.'");
		ТекстОшибки = НСтр("ka='გარე კომპონენტის არქივში ფაილი INFO.XML არ არის';ru='В архиве внешней компоненты отсутствует файл INFO.XML'");
		...
		Если ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
			...
			Если ДействующиеПараметрыОбмена.НастройкаОбменаДействует
				...
				Если Не ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					Гиперссылка = Ложь;
					ТекстСостоянияЭД = НСтр("ka='ЭДО არ არის დაწყებული';ru='ЭДО не начат'");
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					ШаблонСостоянияЭД = НСтр("ka='%1 (გაცვლის გამართვა უმოქმედოა)';ru='%1 (настройка обмена недействительна)'");
				...
				Иначе
					Если ДействующиеПараметрыОбмена.НастройкаОбменаДействует
						...
						ТекстСостоянияЭД = НСтр("ka='ელექტრონული დოკუმენტის მოცემულ სახეობას ბანკი არ იღებს';ru='Данный вид электронного документа банк не принимает'");
					Иначе
						ТекстСостоянияЭД = НСтр("ka='Прямой обмен с банком не подключен';ru='Прямой обмен с банком не подключен'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
			ТекстСостоянияЭД = НСтр("ka='Прямой обмен с банками отключен';ru='Прямой обмен с банками отключен'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьСообщениеОбмена(СообщениеОбмена, Знач Реквизиты) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='Изменение параметров сообщения';ru='Изменение параметров сообщения'");
...
"Причина: %2'");
	...
	Если Реквизиты.Свойство("УдалитьЭП") Тогда
		...
		Исключение
			ВидОперации = НСтр("ka='Удаление электронных подписей';ru='Удаление электронных подписей'");
...
"%1'");
			...
			ВидОперации = НСтр("ka='Получение служебных данных электронного документа';ru='Получение служебных данных электронного документа'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивСообщенийОбменаКОбновлениюСтатуса) Экспорт
	...
	Для Каждого СообщениеОбмена Из МассивСообщенийОбменаКОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='Ошибка подписи электронного документа в сообщении обмена %1';ru='Ошибка подписи электронного документа в сообщении обмена %1'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", СообщениеОбмена);
			Операция = НСтр("ka='Установка подписи ЭП';ru='Установка подписи ЭП'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%1'");
		...
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ka='Сохранение результата проверки электронных подписей';ru='Сохранение результата проверки электронных подписей'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ДобавитьПодпись(Знач СообщениеОбмена, Знач Подпись, Знач Сертификат, Знач ДанныеПроверкиПодписи = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ka='Ошибка сохранения подписи электронного документа %1';ru='Ошибка сохранения подписи электронного документа %1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ka='Сохранение данных электронной подписи';ru='Сохранение данных электронной подписи'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбновитьСтатусыДокументовПакетаЭДО(Знач ПакетОбменСБанками, Знач НовыйСтатусПакетаЭД, Знач СтруктураПараметров, СообщенияОбмена = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='Смена статуса пакета ЭДО и обновление статусов ЭД';ru='Смена статуса пакета ЭДО и обновление статусов ЭД'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетовЭДО Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ОтменитьТранзакцию();
				ШаблонВидаОперации = НСтр("ka='Отправка пакета в банк по настройке: %1';ru='Отправка пакета в банк по настройке: %1'");
...
"%2.'");
			...
			ШаблонВидаОперации = НСтр("ka='Отправка пакета по настройке: %1';ru='Отправка пакета по настройке: %1'");
			ВидОперации = СтрШаблон(ШаблонВидаОперации, НастройкаОбмена);
...
Функция СтруктураСертификата(Знач ДвоичныеДанныеСертификата) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატის ჩატვირთვა ბანკის გასაღებიდან';ru='Загрузка сертификата с банковского ключа'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПрисоединенныйФайл(СообщениеОбмена) Экспорт
	...
	Иначе
		...
		ТекстОшибки = НСтр("ka='Не обнаружен присоединенный файл сообщения %1';ru='Не обнаружен присоединенный файл сообщения %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, СообщениеОбмена);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция МаркерБанкаПоСертификату(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата, ВерсияФормата = Неопределено, ПробнаяОперация = Ложь, НастройкаОбмена = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'");
...
"%2'");
		...
		Иначе
			ТекстОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
		ОписаниеОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Попытка
		...
		Если НЕ ResultBank.Success = Неопределено Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ka='Получен некорректный ответ банковского сервиса';ru='Получен некорректный ответ банковского сервиса'");
...
"Файл ответа банка: %1'");
		...
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ИмяФайлаРезультата);
	Исключение
		ТекстСообщения = НСтр("ka='Ошибка чтения ответа банка.';ru='Ошибка чтения ответа банка.'");
		ОписаниеОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'");
...
Процедура ПолучитьЭДИзБанкаАсинхронныйОбмен(Знач НастройкаОбмена, Знач Параметры, ДанныеВозврата) Экспорт
	...
	Иначе
		Шаблон = НСтр("ka='Ошибка аутентификации на сервисе по адресу (%1)';ru='Ошибка аутентификации на сервисе по адресу (%1)'");
...
"%2'");
		...
		Иначе
			ТекстОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
		ВидОперации = НСтр("ka='Получение списка новых пакетов из банка.';ru='Получение списка новых пакетов из банка.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура СформироватьЗапросЗонд(Знач НастройкаОбмена, Знач МассивОтпечатковСертификатов, НовоеСообщениеОбмена, НастройкиОбмена) Экспорт
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='ედ-ის ჩამოყალიბება';ru='Формирование ЭД'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	НазваниеДокумента = НСтр("ka='მოთხოვნა-ზონდი';ru='Запрос-зонд'");
...
Процедура ОбработатьОтветБанкаПослеОтправкиДокументовЧерезВнешнююОбработку(Знач ПакетыЭДО, Знач ДанныеОтвета, ОтправленныеДокументы) Экспорт
	...
	Для Каждого Ответ Из ДанныеОтвета Цикл
		...
		Если Не ЗначениеЗаполнено(СообщениеОбмена) Тогда
			ТекстСообщения = НСтр("ka='Получены некорректные данные из банка.';ru='Получены некорректные данные из банка.'");
			ТекстОшибки = НСтр("ka='Не найдено исходное сообщение обмена по идентификатору: %1.';ru='Не найдено исходное сообщение обмена по идентификатору: %1.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Ответ.Ключ);
			ВидОперации = НСтр("ka='Чтение данных ответа банка';ru='Чтение данных ответа банка'");
...
Процедура СохранитьСхемыДанных(Знач НастройкаОбмена, Знач МассивСообщений, Знач МассивСхемДанных) Экспорт
	...
	Для Индекс = 0 По КоличествоСообщений - 1 Цикл
		...
		СтруктураРеквизитов.Вставить("Представление", НСтр("ka='მონაცემების სქემა';ru='Схема данных'"));
...
Процедура ОбработатьИзвещениеСОшибкойСбербанк(Знач Ответ, Знач ИдентификаторСобытия, ТекстСообщения) Экспорт
	...
	Исключение
		ЧтениеXML.Закрыть();
		ТекстСообщения = НСтр("ka='ბანკიდან მიღებული მონაცემების წაკითხვისას შეცდომა წარმოიშვა';ru='Возникла ошибка при чтении полученных данных из банка'");
...
"Текст ошибки: %2'");
		...
		ВидОперации = НСтр("ka='Чтение полученных данных из банка';ru='Чтение полученных данных из банка'");
...
"Описание: %3'");
	Если ТипЗнч(ЭД.Errors.Error) = Тип("СписокXDTO") Тогда
		Для Каждого Error Из ЭД.Errors.Error Цикл
			...
			ВидОперации = НСтр("ka='Отправка данных в банк';ru='Отправка данных в банк'");
...
Процедура УдалитьИдентификаторЗапроса(Знач НастройкаОбмена, Знач Идентификатор) Экспорт
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='Удаление идентификатора запроса банка';ru='Удаление идентификатора запроса банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
	КонецПопытки;
...
Функция ТекстЗапросаНочнойВыписки(НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации, ЭП, СертификатПодписи) Экспорт
	...
	Попытка
		...
		Если ДанныеСертификата = Неопределено Тогда
			ВызватьИсключение НСтр("ka='Не найден сертификат установленной подписи';ru='Не найден сертификат установленной подписи'");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			ВызватьИсключение НСтр("ka='Ошибка формирования запроса ночной выписки';ru='Ошибка формирования запроса ночной выписки'");
		КонецЕсли;
...
"Идентификатор события: %1'");
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ka='Чтение полученных данных из банка';ru='Чтение полученных данных из банка'"), ТекстОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
		Возврат;
	КонецПопытки;
...
Функция ПакетXMLСбербанка(Знач СообщениеОбмена, Знач НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации) Экспорт
	...
	Попытка
		...
		ИначеЕсли ЭД.Тип() = ОбменСБанкамиСлужебный.ТипЗначенияCML(ФабрикаXDTO, ПространствоИмен, "StmtReqType") Тогда
			...
			Request.Проверить();
		Иначе
			ВызватьИсключение НСтр("ka='Неизвестный формат файла';ru='Неизвестный формат файла'");
		КонецЕсли;
		ОбъектXML.Закрыть();
	...
	Исключение
		ОбъектXML.Закрыть();
		ТекстОшибки = НСтр("ka='ბანკის მონაცემთა პაკეტის შედგენისას შეცდომა წარმოიშვა';ru='Возникла ошибка при формировании пакета данных банка'");
...
Функция МассивЗапросовСостоянийОбработкиДокументов(НастройкаОбмена, ВидЭД) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Операция = НСтр("ka='Формирование запроса статуса электронного документа';ru='Формирование запроса статуса электронного документа'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ЗапросВыпискиСбербанк(Знач НастройкаОбмена, Знач ДатаНачала, Знач ДатаОкончания, Знач НомерСчета, ФормироватьПринудительно = Ложь, ГотовыеВыписки = Неопределено) Экспорт
	...
	ИмяФайла = НСтр("ka='%1-დან %2-ის ჩათვლით პერიოდის ამონაწერის მოთხოვნა';ru='Запрос выписки за период с %1 по %2'");
	ИмяФайла = СтрШаблон(
		...
		Если ПЭД = Неопределено Тогда
			...
			Если Выборка.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку Тогда
				...
				Если Не ЗначениеЗаполнено(СлужебноеСообщениеБанка) Тогда
					ТекстСообщения = НСтр("ka='Невозможно сформировать пакет банка';ru='Невозможно сформировать пакет банка'");
					ВидОперации = НСтр("ka='Формирование пакета ЭД';ru='Формирование пакета ЭД'");
					ТекстОшибки = НСтр("ka='Не найдены дополнительные данные для электронного документа';ru='Не найдены дополнительные данные для электронного документа'");
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ДанныеДляАутентификацииПоСертификату(ДанныеСертификата, ВерсияФормата)
	...
	Операция = НСтр("ka='Формирование электронного документа.';ru='Формирование электронного документа.'");
...
Функция ЗапросВыпискиCMLXDTO(ДатаНачала, ДатаКонца, МассивБанковскихСчетов, Банк)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ka='ედ-ის ჩამოყალიბება';ru='Формирование ЭД'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция МассивЗапросовВыписки(НастройкаОбмена, Организация, Банк, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	ШаблонСообщения = НСтр("ka='%1-დან %2-ის ჩათვლით ამონაწერის მოთხოვნა';ru='Запрос выписки с %1 по %2'");
...
Функция МассивЗапросовВыпискиAsync(НастройкаОбмена, ИдентификаторОрганизации, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
		...
		Шаблон = НСтр("ka='%1-დან %2-ის ჩათვლით ამონაწერის მოთხოვნა';ru='Запрос выписки с %1 по %2'");
...
Функция ДанныеДляОтправкиВБанк(Ссылки, ПрограммаБанка)
	...
	Пока ВыборкаНастройкиОбмена.Следующий() Цикл
		...
		Иначе
			ШаблонСообщения = НСтр("ka='Не указаны сертификаты в настройке обмена с банком %1';ru='Не указаны сертификаты в настройке обмена с банком %1'");
			ТекстСообщения = СтрШаблон(
...
Процедура СохранитьВыпискуСбербанка(ЭД, НастройкаОбмена, МассивВыписок, СообщениеВладелец = Неопределено)
	...
	Для Каждого ВыпискаXDTO Из ЭД.Statements.Statement Цикл
		...
		Если ОбменСБанкамиСлужебный.ФайлИзменен(ФабрикаXDTO, ВыпискаXDTO, ВремФайл) Тогда
			ВызватьИсключение НСтр("ka='Обнаружена подмена файла.';ru='Обнаружена подмена файла.'");
		КонецЕсли;
		...
		ПредставлениеДокумента = НСтр("ka='ბანკის ამონაწერი შემდეგი პერიოდისთვის';ru='Выписка банка за период с'") + " " + Формат(ДатаНачала, "ДЛФ=D")
			+ " " + НСтр("ka='მიხედვით';ru='по'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
	...
	Для Каждого Тикет Из ЭД.Tickets.Ticket Цикл
		...
		Если Выборка.Следующий() Тогда
			...
			Если НЕ ЗначениеЗаполнено(Выборка.Квитанция) Тогда
				НазваниеЭД = НСтр("ka='კვიტანცია   დან';ru='Квитанция от'")+ " " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT");
			...
			Иначе
				...
				Представление = НСтр("ka='კვიტანცია   დან';ru='Квитанция от'") + " " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT");
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	Пока Результат.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ka='დოკუმენტ %1-სთვის უკვე არის აქტუალური ელექტრონული დოკუმენტი.';ru='Для документа %1 уже есть актуальный электронный документ.'");
...
Функция ОтправитьЗапросЗондНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, МассивСообщений) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='სატესტო მოთხოვნის გაგზავნა ბანკში';ru='Отправка тестового запроса в банк'");
...
Функция ПолучитьИзвещениеОСостоянииЭДАсинхронноНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, МассивСообщений = Неопределено) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='ელექტრონული დოკუმენტის მდგომარეობის შესახებ შეტყობინების მიღება';ru='Получение извещения о состоянии электронного документа'");
	ПараметрыВыполнения = Новый Массив;
	ПараметрыВыполнения.Добавить(ПараметрыЗапроса);
	ПараметрыВыполнения.Добавить(АдресХранилища);
...
Функция СведенияОВнешнейОбработке(ИмяВнешнегоМодуля) Экспорт
	...
	Попытка
		...
		Возврат СтруктураВозврата;
	Исключение
		Операция = НСтр("ka='გარე დამუშავების ობიექტის შექმნა.';ru='Создание объекта внешней обработки.'");
