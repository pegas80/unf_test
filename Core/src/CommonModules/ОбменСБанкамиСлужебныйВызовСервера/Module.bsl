...
Функция ДвоичныеДанныеПрисоединенногоФайла(Знач СсылкаНаОбъект) Экспорт
	...
	Если НЕ МассивФайлов.Количество() Тогда
		ТекстОшибки = НСтр("ka='ობიექტის მიერთებული ფაილი არ არის აღმოჩენილი: %1';ru='Не обнаружен присоединенный файл объекта: %1';en='Not detected object attached file:% 1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, СсылкаНаОбъект);
		Операция = НСтр("ka='მიერთებული ფაილის ორობითი მონაცემების მიღება';ru='Получение двоичных данных присоединенного файла';en='Obtaining binary data attached file'");
...
Функция ПолученоИзвещениеПоЗапросу(СообщениеЗапрос, ЕстьОшибка) Экспорт
	...
	Если ЗначениеЗаполнено(Извещение) Тогда 
		...
		Если РеквизитыСообщенияОбмена.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросЗонд
			...
			ЕстьОшибка = Истина;
			ТекстСообщения = НСтр("ka='მოთხოვნა უარყოფილია. უარყოფის მიზეზი: %1';ru='Запрос отклонен."
...
Функция ВерсияАктуальнойВК(ИмяВнешнегоМодуля) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		НаименованиеЗадания = НСтр("ka='1С:დირექტბანკი გარე კომპონენტის განახლება (%1).';ru='1С:ДиректБанк: Обновление внешней компоненты (%1).';en='1C: DirektBank: Update the outer components (1%).'");
...
Функция СкачатьВКНаСервере(Знач УникальныйИдентификатор, Знач URL) Экспорт
	...
	Попытка
		НазваниеЗадания = НСтр("ka='1С:ДиректБанк: Скачивание внешней компоненты с сервера банка';ru='1С:ДиректБанк: Скачивание внешней компоненты с сервера банка';en='1C: DirektBank: Download external components from the bank server'");
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно = Ложь;
		Текст = НСтр("ka='Не удалось скачать внешний модуль с сервера банка.';ru='Не удалось скачать внешний модуль с сервера банка.';en='Unable to download an external module to the bank server.'");
		ВидОперации = НСтр("ka='Скачивание внешней компоненты с сервера банка';ru='Скачивание внешней компоненты с сервера банка';en='Download external components from the bank server'");
...
Функция ИнформацияОВКБанка(АдресВнешнейКомпоненты) Экспорт
	...
	Если Не НайденаИнформация Тогда
		Операция = НСтр("ka='Чтение информации о файле внешнего модуля.';ru='Чтение информации о файле внешнего модуля.';en='Reading the information on the external module file.'");
...
"Загрузите актуальную версию внешнего модуля.';ka='При подключении внешнего модуля произошла ошибка."
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		...
		Если ТЗ_Сертификатов.Количество() = 0 И ЗначениеЗаполнено(СообщениеОбмена) Тогда
			...
			Если ДоступныеСертификаты[0].Количество > 0 Тогда
				...
				ШаблонСообщения = НСтр("ka='გაცვლის გამართვაში: %1 ეხ-ის არცერთი ხელმისაწვდომი სერთიფიკატი არ მოიძებნა.';ru='В настройке обмена: %1"
		...
		Если ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
			...
			Если ДействующиеПараметрыОбмена.НастройкаОбменаДействует
				...
				Если Не ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					Гиперссылка = Ложь;
					ТекстСостоянияЭД = НСтр("ka='ЭДО არ არის დაწყებული';ru='ЭДО не начат';en='EDI is not started'");
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					ШаблонСостоянияЭД = НСтр("ka='%1 (გაცვლის გამართვა უმოქმედოა)';ru='%1 (настройка обмена недействительна)';en='1% (exchange invalid setting)'");
				...
				Иначе
					Если ДействующиеПараметрыОбмена.НастройкаОбменаДействует
						...
						ТекстСостоянияЭД = НСтр("ka='ელექტრონული დოკუმენტის მოცემულ სახეობას ბანკი არ იღებს';ru='Данный вид электронного документа банк не принимает';en='This type of electronic document bank does not accept'");
					Иначе
						ТекстСостоянияЭД = НСтр("ka='Прямой обмен с банком не подключен';ru='Прямой обмен с банком не подключен';en='Direct exchange is not connected with the bank'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
			ТекстСостоянияЭД = НСтр("ka='Прямой обмен с банками отключен';ru='Прямой обмен с банками отключен';en='Direct exchange with the banks off'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьСообщениеОбмена(СообщениеОбмена, Знач Реквизиты) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='Изменение параметров сообщения';ru='Изменение параметров сообщения';en='Change message options'");
		ТекстОшибки = НСтр("ka='Не удалось заблокировать сообщение обмена: %1"
"Причина: %2';ru='Не удалось заблокировать сообщение обмена: %1"
"Причина: %2'");
	...
	Если Реквизиты.Свойство("УдалитьЭП") Тогда
		...
		Исключение
			ВидОперации = НСтр("ka='Удаление электронных подписей';ru='Удаление электронных подписей';en='Removal of electronic signatures'");
			ТекстОшибки = НСтр("ka='Не удалось заблокировать присоединенный файл: %1"
"Причина: %2';ru='Не удалось заблокировать присоединенный файл: %1"
"Причина: %2'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = СтрШаблон(
...
"Причина: %2"
"Операция не выполнена.';ka='Не удалось удалить подписи присоединенного файла: %1"
"Причина: %2"
"Операция не выполнена.'");
			ТекстСообщения = СтрШаблон(
...
Функция СтруктураСодержимогоСообщенияОбмена(СообщениеОбмена) Экспорт
	...
	Если Выборка.Количество() > 0 И Выборка.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку
		...
		Если Не ЗначениеЗаполнено(СлужебныйЭД) Тогда
			ТекстСообщения = НСтр("ka='Не обнаружены служебные данные электронного документа."
"Операция не выполнена.';ru='Не обнаружены служебные данные электронного документа."
"Операция не выполнена.'");
			ТекстОшибки = НСтр("ka='Не обнаружены служебные данные для сообщения обмена:"
"%1';ru='Не обнаружены служебные данные для сообщения обмена:"
"%1'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, СообщениеОбмена);
			ВидОперации = НСтр("ka='Получение служебных данных электронного документа';ru='Получение служебных данных электронного документа';en='Preparation of service data of the electronic document'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивСообщенийОбменаКОбновлениюСтатуса) Экспорт
	...
	Для Каждого СообщениеОбмена Из МассивСообщенийОбменаКОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='Ошибка подписи электронного документа в сообщении обмена %1';ru='Ошибка подписи электронного документа в сообщении обмена %1';en='Error electronic document signature in the message exchange 1%'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", СообщениеОбмена);
			Операция = НСтр("ka='Установка подписи ЭП';ru='Установка подписи ЭП';en='Installing the EP signatures'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%1';ka='При сохранении результата проверки электронных подписей произошла ошибка:"
"%1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки);

		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ka='Сохранение результата проверки электронных подписей';ru='Сохранение результата проверки электронных подписей';en='Store the results of verification of electronic signatures'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Документ не обработан, так как содержит невалидные подписи.';ka='Обработка сообщения обмена %1."
...
Процедура ДобавитьПодпись(Знач СообщениеОбмена, Знач Подпись, Знач Сертификат, Знач ДанныеПроверкиПодписи = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ka='Ошибка сохранения подписи электронного документа %1';ru='Ошибка сохранения подписи электронного документа %1';en='Error saving electronic document signature% 1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ka='Сохранение данных электронной подписи';ru='Сохранение данных электронной подписи';en='Saving data of an electronic signature'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбновитьСтатусыДокументовПакетаЭДО(Знач ПакетОбменСБанками, Знач НовыйСтатусПакетаЭД, Знач СтруктураПараметров, СообщенияОбмена = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='Смена статуса пакета ЭДО и обновление статусов ЭД';ru='Смена статуса пакета ЭДО и обновление статусов ЭД';en='Changing EDI package status and update the status of ED'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетовЭДО Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ОтменитьТранзакцию();
				ШаблонВидаОперации = НСтр("ka='Отправка пакета в банк по настройке: %1';ru='Отправка пакета в банк по настройке: %1';en='Sending a package for setting up the bank: 1%'");
...
"%2.';ka='Ошибка отправки пакета по настройке: %1"
"%2.'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтрШаблон(
				ШаблонСообщения, НастройкаОбмена, КраткоеПредставлениеОшибки);
			ШаблонВидаОперации = НСтр("ka='Отправка пакета по настройке: %1';ru='Отправка пакета по настройке: %1';en='Sending setup package: 1%'");
			ВидОперации = СтрШаблон(ШаблонВидаОперации, НастройкаОбмена);
...
Функция СтруктураСертификата(Знач ДвоичныеДанныеСертификата) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატის ჩატვირთვა ბანკის გასაღებიდან';ru='Загрузка сертификата с банковского ключа';en='Download a certificate from the bank key'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТестСообщения = НСтр("ru='Не удалось прочитать файл сертификата, операция прервана.';en='Could not read the certificate file, the operation is aborted.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Проверьте настройки обмена или обратитесь к администратору.';ka='Сертификат подписи недоступен для текущего пользователя."
"Проверьте настройки обмена или обратитесь к администратору.'");
	...
	Пока ВыборкаСертификаты.Следующий() Цикл
		Если ВыборкаСертификаты.ДействителенДо < ТекущаяДатаСеанса() Тогда
			ШаблонСообщения = НСтр("ka='სერთიფიკატის მოქმედების ვადა დასრულდა: %1"
"მიიღეთ ახალი სერთიფიკატი';ru='Закончился срок действия сертификата: %1"
...
Функция ПрисоединенныйФайл(СообщениеОбмена) Экспорт
	...
	Иначе
		...
		ТекстОшибки = НСтр("ka='Не обнаружен присоединенный файл сообщения %1';ru='Не обнаружен присоединенный файл сообщения %1';en='Not found attached message file% 1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, СообщениеОбмена);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция МаркерБанкаПоСертификату(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата, ВерсияФормата = Неопределено, ПробнаяОперация = Ложь, НастройкаОбмена = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'");
...
"Код ошибки: %1."
"%2';ka='Ошибка аутентификации на сервере банка."
"Код ошибки: %1."
"%2'");
...
"Проверьте настройки обмена с сервисом 1С:ДиректБанк или обратитесь в техподдержку банка.';ka='Не удалось пройти процесс аутентификации. Получен некорректный ответ банка."
...
"Полученный файл: %1"
"Подробная информация об ошибке: %2';ka='Не удалось прочитать файл, полученный из банка."
"Полученный файл: %1"
...
"Код ошибки: %1."
"%2';ka='Ошибка отправки данных аутентификации на сервер банка."
"Код ошибки: %1."
"%2'");
			ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
		Иначе
			ТекстОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
		ОписаниеОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Попытка
		...
		Если НЕ ResultBank.Success = Неопределено Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ka='Получен некорректный ответ банковского сервиса';ru='Получен некорректный ответ банковского сервиса';en='Received an invalid response to banking services'");
				ТекстОшибки = НСтр("ru='Ответ банка содержит некорректные данные."
"Файл ответа банка: %1';ka='Ответ банка содержит некорректные данные."
"Файл ответа банка: %1'");
		...
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ИмяФайлаРезультата);
	Исключение
		ТекстСообщения = НСтр("ka='Ошибка чтения ответа банка.';ru='Ошибка чтения ответа банка.';en='Reading bank response error.'");
		ОписаниеОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'");
		ШаблонОшибки = НСтр("ru='При чтении ответа банка произошла ошибка."
"Файл ответа: %1"
"Ошибка: %2';ka='При чтении ответа банка произошла ошибка."
"Файл ответа: %1"
"Ошибка: %2'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПолучитьЭДИзБанкаАсинхронныйОбмен(Знач НастройкаОбмена, Знач Параметры, ДанныеВозврата) Экспорт
	...
	Иначе
		Шаблон = НСтр("ka='Ошибка аутентификации на сервисе по адресу (%1)';ru='Ошибка аутентификации на сервисе по адресу (%1)';en='Authentication failed on service address (% 1)'");
...
"Код ошибки: %1."
"%2';ka='При получении списка новых документов из банка произошла ошибка."
"Код ошибки: %1."
"%2'");
				ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
			КонецЕсли;
		Иначе
			ТекстОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
		ВидОперации = НСтр("ka='Получение списка новых пакетов из банка.';ru='Получение списка новых пакетов из банка.';en='Get a list of new packages from the bank.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Код ошибки: %1."
"%2';ka='При получении нового документа из банка произошла ошибка."
"Код ошибки: %1."
"%2'");
				ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
			Иначе
...
"Код ошибки: %1."
"%2';ka='Ошибка отправки одноразового пароля."
"Код ошибки: %1."
"%2'");
			ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
		Иначе
...
Процедура СформироватьЗапросЗонд(Знач НастройкаОбмена, Знач МассивОтпечатковСертификатов, НовоеСообщениеОбмена, НастройкиОбмена) Экспорт
	...
	НазваниеДокумента = НСтр("ka='მოთხოვნა-ზონდი';ru='Запрос-зонд';en='Request for probe'");
...
Процедура ОбработатьОтветБанкаПослеОтправкиДокументовЧерезВнешнююОбработку(Знач ПакетыЭДО, Знач ДанныеОтвета, ОтправленныеДокументы) Экспорт
	...
	Для Каждого Ответ Из ДанныеОтвета Цикл
		...
		Если Не ЗначениеЗаполнено(СообщениеОбмена) Тогда
			ТекстСообщения = НСтр("ka='Получены некорректные данные из банка.';ru='Получены некорректные данные из банка.';en='Received incorrect data from the bank.'");
			ТекстОшибки = НСтр("ka='Не найдено исходное сообщение обмена по идентификатору: %1.';ru='Не найдено исходное сообщение обмена по идентификатору: %1.';en='It found the original message of the exchange identifier:% 1.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Ответ.Ключ);
			ВидОперации = НСтр("ka='Чтение данных ответа банка';ru='Чтение данных ответа банка';en='Reading data bank response'");
...
Процедура СохранитьСхемыДанных(Знач НастройкаОбмена, Знач МассивСообщений, Знач МассивСхемДанных) Экспорт
	...
	Для Индекс = 0 По КоличествоСообщений - 1 Цикл
		...
		СтруктураРеквизитов.Вставить("Представление", НСтр("ka='მონაცემების სქემა';ru='Схема данных';en='data scheme'"));
...
Процедура ОбработатьИзвещениеСОшибкойСбербанк(Знач Ответ, Знач ИдентификаторСобытия, ТекстСообщения) Экспорт
	...
	Исключение
		ЧтениеXML.Закрыть();
		ТекстСообщения = НСтр("ka='ბანკიდან მიღებული მონაცემების წაკითხვისას შეცდომა წარმოიშვა';ru='Возникла ошибка при чтении полученных данных из банка';en='An error occurred while reading the data from the bank'");
		ТекстОшибки = НСтр("ka='ბანკიდან მიღებული მონაცემების წაკითხვისას შეცდომა წარმოიშვა."
"მოვლენის იდენტიფიკატორი: %1"
"შეცდომის ტექსტი: %2';ru='Возникла ошибка при чтении полученных данных из банка"
"Идентификатор события: %1"
"Текст ошибки: %2'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(
			ТекстОшибки, ИдентификаторСобытия, ПодробноеПредставлениеОшибки);
		ВидОперации = НСтр("ka='Чтение полученных данных из банка';ru='Чтение полученных данных из банка';en='Reading the data from the bank'");
...
"Код: %1."
"Тип: %2"
"Описание: %3';ka='Получена ошибка из банка."
"Код: %1."
"Тип: %2"
"Описание: %3'");
	Если ТипЗнч(ЭД.Errors.Error) = Тип("СписокXDTO") Тогда
		Для Каждого Error Из ЭД.Errors.Error Цикл
			...
			ВидОперации = НСтр("ka='Отправка данных в банк';ru='Отправка данных в банк';en='Sending data to the bank'");
...
Процедура УдалитьИдентификаторЗапроса(Знач НастройкаОбмена, Знач Идентификатор) Экспорт
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='Удаление идентификатора запроса банка';ru='Удаление идентификатора запроса банка';en='Removing bank request ID'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
	КонецПопытки;
...
Функция ТекстЗапросаНочнойВыписки(НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации, ЭП, СертификатПодписи) Экспорт
	...
	Попытка
		...
		Если ДанныеСертификата = Неопределено Тогда
			ВызватьИсключение НСтр("ka='Не найден сертификат установленной подписи';ru='Не найден сертификат установленной подписи';en='certificate installed signatures not found'");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			ВызватьИсключение НСтр("ka='Ошибка формирования запроса ночной выписки';ru='Ошибка формирования запроса ночной выписки';en='Error formation of night discharge request'");
		КонецЕсли;
...
Процедура ОбработатьОтветСбербанка(Знач Ответ, Знач НастройкаОбмена, МассивНовыхСообщений, Знач Идентификатор, Знач ИдентификаторСобытия, ЕстьОшибка) Экспорт
	...
	Исключение
		ЧтениеXML.Закрыть();
		ТекстСообщения = НСтр("ka='ბანკიდან მიღებული მონაცემების წაკითხვისას შეცდომა წარმოიშვა მოვლენის იდენტიფიკატორი: %1';ru='Возникла ошибка при чтении полученных данных из банка"
"Идентификатор события: %1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ИдентификаторСобытия);
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ka='Чтение полученных данных из банка';ru='Чтение полученных данных из банка';en='Reading the data from the bank'"), ТекстОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
		Возврат;
	КонецПопытки;
...
Функция ПакетXMLСбербанка(Знач СообщениеОбмена, Знач НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации) Экспорт
	...
	Попытка
		...
		ИначеЕсли ЭД.Тип() = ОбменСБанкамиСлужебный.ТипЗначенияCML(ФабрикаXDTO, ПространствоИмен, "StmtReqType") Тогда
			...
			Request.Проверить();
		Иначе
			ВызватьИсключение НСтр("ka='Неизвестный формат файла';ru='Неизвестный формат файла';en='Unknown file format'");
		КонецЕсли;
		ОбъектXML.Закрыть();
	...
	Исключение
		ОбъектXML.Закрыть();
		ТекстОшибки = НСтр("ka='ბანკის მონაცემთა პაკეტის შედგენისას შეცდომა წარმოიშვა';ru='Возникла ошибка при формировании пакета данных банка';en='An error occurred during the formation of the bank data packet'");
...
Функция МассивЗапросовСостоянийОбработкиДокументов(НастройкаОбмена, ВидЭД) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Операция = НСтр("ka='Формирование запроса статуса электронного документа';ru='Формирование запроса статуса электронного документа';en='Formation of an electronic document status request'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ЗапросВыпискиСбербанк(Знач НастройкаОбмена, Знач ДатаНачала, Знач ДатаОкончания, Знач НомерСчета, ФормироватьПринудительно = Ложь, ГотовыеВыписки = Неопределено) Экспорт
	...
	ИмяФайла = НСтр("ka='%1-დან %2-ის ჩათვლით პერიოდის ამონაწერის მოთხოვნა';ru='Запрос выписки за период с %1 по %2';en='Query statements for the period from% 1 to% 2'");
	ИмяФайла = СтрШаблон(
		...
		Если ПЭД = Неопределено Тогда
			...
			Если Выборка.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку Тогда
				...
				Если Не ЗначениеЗаполнено(СлужебноеСообщениеБанка) Тогда
					ТекстСообщения = НСтр("ka='Невозможно сформировать пакет банка';ru='Невозможно сформировать пакет банка';en='It is impossible to form a bank package'");
					ВидОперации = НСтр("ka='Формирование пакета ЭД';ru='Формирование пакета ЭД';en='Formation ED Pack'");
					ТекстОшибки = НСтр("ka='Не найдены дополнительные данные для электронного документа';ru='Не найдены дополнительные данные для электронного документа';en='Found additional data to the electronic document'");
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ДанныеДляАутентификацииПоСертификату(ДанныеСертификата, ВерсияФормата)
	...
	Операция = НСтр("ka='Формирование электронного документа.';ru='Формирование электронного документа.';en='Formation of an electronic document.'");
...
Функция МассивЗапросовВыписки(НастройкаОбмена, Организация, Банк, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	ШаблонСообщения = НСтр("ka='%1-დან %2-ის ჩათვლით ამონაწერის მოთხოვნა';ru='Запрос выписки с %1 по %2';en='Query statements from% 1 to% 2'");
...
Функция МассивЗапросовВыпискиAsync(НастройкаОбмена, ИдентификаторОрганизации, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
		...
		Шаблон = НСтр("ka='%1-დან %2-ის ჩათვლით ამონაწერის მოთხოვნა';ru='Запрос выписки с %1 по %2';en='Query statements from% 1 to% 2'");
...
Функция ДанныеДляОтправкиВБанк(Ссылки, ПрограммаБанка)
	...
	Пока ВыборкаНастройкиОбмена.Следующий() Цикл
		...
		Иначе
			ШаблонСообщения = НСтр("ka='Не указаны сертификаты в настройке обмена с банком %1';ru='Не указаны сертификаты в настройке обмена с банком %1';en='Certificates are not listed in the configuration exchange with the bank 1%'");
			ТекстСообщения = СтрШаблон(
...
Процедура СохранитьВыпискуСбербанка(ЭД, НастройкаОбмена, МассивВыписок, СообщениеВладелец = Неопределено)
	...
	Для Каждого ВыпискаXDTO Из ЭД.Statements.Statement Цикл
		...
		Если ОбменСБанкамиСлужебный.ФайлИзменен(ФабрикаXDTO, ВыпискаXDTO, ВремФайл) Тогда
			ВызватьИсключение НСтр("ka='Обнаружена подмена файла.';ru='Обнаружена подмена файла.';en='Found substitute file.'");
		КонецЕсли;
		...
		ПредставлениеДокумента = НСтр("ka='ბანკის ამონაწერი შემდეგი პერიოდისთვის';ru='Выписка банка за период с';en='A bank statement for the period from'") + " " + Формат(ДатаНачала, "ДЛФ=D")
			+ " " + НСтр("ka='მიხედვით';ru='по';en='by'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
	...
	Для Каждого Тикет Из ЭД.Tickets.Ticket Цикл
		...
		Если Выборка.Следующий() Тогда
			...
			Если НЕ ЗначениеЗаполнено(Выборка.Квитанция) Тогда
				НазваниеЭД = НСтр("ka='კვიტანცია   დან';ru='Квитанция от';en='receipt of'")+ " " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT");
			...
			Иначе
				...
				Представление = НСтр("ka='კვიტანცია   დან';ru='Квитанция от';en='receipt of'") + " " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT");
...
"%1.';ka='При формировании электронного документа произошла ошибка."
"%1.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Исключение
		ТекстСообщения = НСтр("ru='При формировании электронного документа произошла ошибка."
"Обратитесь к администратору.';ka='При формировании электронного документа произошла ошибка."
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	Пока Результат.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ka='დოკუმენტ %1-სთვის უკვე არის აქტუალური ელექტრონული დოკუმენტი.';ru='Для документа %1 уже есть актуальный электронный документ.';en='To document% 1 already has a current electronic document.'");
...
"Не требуется выполнения действий с электронным документом.';ka='დამუშავება %1. ელექტრონულ დოკუმენტთან მოქმედებების შესრულება საჭირო არ არის.'");
...
Функция ОтправитьЗапросЗондНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, МассивСообщений) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='სატესტო მოთხოვნის გაგზავნა ბანკში';ru='Отправка тестового запроса в банк';en='Sending a test request to the bank'");
...
Функция ПолучитьИзвещениеОСостоянииЭДАсинхронноНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, МассивСообщений = Неопределено) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='ელექტრონული დოკუმენტის მდგომარეობის შესახებ შეტყობინების მიღება';ru='Получение извещения о состоянии электронного документа';en='Receiving notice of a state of the electronic document'");
	ПараметрыВыполнения = Новый Массив;
	ПараметрыВыполнения.Добавить(ПараметрыЗапроса);
	ПараметрыВыполнения.Добавить(АдресХранилища);
...
Функция СведенияОВнешнейОбработке(ИмяВнешнегоМодуля) Экспорт
	...
	Попытка
		...
		Возврат СтруктураВозврата;
	Исключение
		Операция = НСтр("ka='გარე დამუშავების ობიექტის შექმნა.';ru='Создание объекта внешней обработки.';en='Creating external processing facility.'");
