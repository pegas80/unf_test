...
"';ru='При копировании были очищены виды деятельности в разделе лицензий.'");
				Сообщение.Сообщить();
...
"изменяется двойным щелчком, клавишей DEL или действием ""Удалить"".'");
	...
	Поле.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
	Поле.Подсказка = НСтр("ka='სტრიქონის ნომერი';ru='Номер строки.'");
	...
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		...
		Если Прав(Результат, 4) = ".tmp" Тогда
			...
			Исключение
			    ...
			    ЗаписьЖурналаРегистрации(НСтр("ka='Запись документа Выгрузка регламентированного отчета.';ru='Запись документа Выгрузка регламентированного отчета.'"),
		...
		РегламентированнаяОтчетность.ОбработатьСобытие1СОтчетности(
					НСтр("ka='Регламентированный отчет. Выгрузка';ru='Регламентированный отчет. Выгрузка'"), ДокументСсылкаАлкоОтчет);
		Возврат;
