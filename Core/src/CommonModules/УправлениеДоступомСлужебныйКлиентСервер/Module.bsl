...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа, ДобавитьКоличествоЗначений = Истина) Экспорт
	...
	Если Форма.ЭтоПрофильГруппДоступа Тогда
		Если КоличествоЗначений = 0 Тогда
			ЧислоИПредмет = НСтр("ka='არ არის დანიშნულები';ru='не назначены'");
		Иначе
			ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
					"Л = ru_RU", НСтр("ka='მნიშვნლობა,მნიშვნელობები,მნიშვნელობებს,,,,,,0';ru='значение,значения,значений,,,,,,0'"));
		КонецЕсли;
	...
	Если КоличествоЗначений = 0 Тогда
		Представление = ?(ОписаниеВидаДоступа.ВсеРазрешены,
			НСтр("ka='ყველა ნებადართულია, გამონაკლისების გარეშე';ru='Все разрешены, без исключений'"),
			НСтр("ka='ყველა აკრძალულია, გამონაკლისის გარეშე';ru='Все запрещены, без исключений'"));
	Иначе
		ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
				"Л = ru_RU", НСтр("ka='значения,значений,значений,,,,,,0';ru='значения,значений,значений,,,,,,0'"));
		
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ОписаниеВидаДоступа.ВсеРазрешены,
				НСтр("ka='ყველა ნებადართულია, გარდა %1';ru='Все разрешены, кроме %1'"),
				НСтр("ka='ყველა აკრძალულია, გარდა %1';ru='Все запрещены, кроме %1'")),
			ЧислоИПредмет);
	КонецЕсли;
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ВидДоступа = Форма.ВидДоступаПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ka='აკრძალული მნიშვნელობები (%1) - მიმდინარე მომხმარებელი ყოველთვის ნებადართულია';ru='Запрещенные значения (%1) - текущий пользователь всегда разрешен'"),
				НСтр("ka='ნებადართული მნიშვნელობები (%1) - მიმდინარე მომხმარებელი ყოველთვის ნებადართულია';ru='Разрешенные значения (%1) - текущий пользователь всегда разрешен'") );
		
		ИначеЕсли ТекущиеДанные.ВидДоступа = Форма.ВидДоступаВнешниеПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ka='აკრძალული მნიშVნელობები (%1) - მიმდინარე გარე მომხმარებელი ყოველთვის ნებადართულია';ru='Запрещенные значения (%1) - текущий внешний пользователь всегда разрешен'"),
				НСтр("ka='ნებადართული მნიშვნელობები (%1) - მიმდინარე გარე მომხმარებელი ყოველთვის ნებადართულია';ru='Разрешенные значения (%1) - текущий внешний пользователь всегда разрешен'") );
		Иначе
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ka='აკრძალული მნიშVნელობები (%1)';ru='Запрещенные значения (%1)'"),
				НСтр("ka='ნებადართული მნიშვნელობები (%1)';ru='Разрешенные значения (%1)'") );
		КонецЕсли;
	...
	Если Форма.ТекущиеТипыВыбираемыхЗначений.Количество() = 0 Тогда
		Форма.ТекущиеТипыВыбираемыхЗначений.Добавить(Неопределено, НСтр("ka='დაუდგენელია';ru='Неопределено'"));
	КонецЕсли;
	...
	Пока НЕ Отказ И ИндексВидаДоступа >= 0 Цикл
		...
		Если СтрокаВидаДоступа.ВидДоступа = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ka='დაშვების ტიპი არ არის არჩეული.';ru='Вид доступа не выбран.'"),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				НСтр("ka='დაშვების ტიპი სტრიქონ %1-ში არჩეული არ არის.';ru='Вид доступа в строке %1 не выбран.'"),
		...
		Если Параметры.Назначение <> Неопределено
		  И Не ВидДоступаСоответствуетНазначениюПрофиля(СтрокаВидаДоступа.ВидДоступа, НазначениеПрофиля) Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაშვების სახეობა ""%1"" პროფილის დანიშნულებას არ შეესაბამება.';ru='Вид доступа ""%1"" не соответствует назначению профиля.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში დაშვების სახეობა ""%1"" პროფილის დანიშნულებას არ შეესაბამება.';ru='Вид доступа ""%1"" в строке %%1 не соответствует назначению профиля.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Если НайденныеВидыДоступа.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაშვების სახეობა ""%1"" მეორდება.';ru='Вид доступа ""%1"" повторяется.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში დაშვების სახეობა ""%1"" მეორდება.';ru='Вид доступа ""%1"" в строке %%1 повторяется.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Пока НЕ Отказ И ИндексЗначенияДоступа >= 0 Цикл
			...
			Если СтрокаЗначенияДоступа.ЗначениеДоступа = Неопределено Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					НСтр("ka='მნიშვნელობა არ არის არჩეული.';ru='Значение не выбрано.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ka='მნიშვნელობა სტრიქონ %1-ში არ არის არჩეული.';ru='Значение в строке %1 не выбрано.'"),
			...
			Если НайденныеЗначения.Количество() > 1 Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					Параметры.ПутьКТаблицам + "ЗначенияДоступа[%1].ЗначениеДоступа",
					НСтр("ka='მნიშვნელობა მეორდება.';ru='Значение повторяется.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ka='მნიშვნელობა სტრიქონ %1-ში მეორდება.';ru='Значение в строке %1 повторяется.'"),
					Параметры.ЗначенияДоступа.Индекс(СтрокаЗначенияДоступа));
				Отказ = Истина;
				Прервать;
			КонецЕсли;
