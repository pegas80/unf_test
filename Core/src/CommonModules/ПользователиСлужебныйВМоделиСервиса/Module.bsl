	...
	Если ДоступноИзменениеПользователей() Тогда
		...
		Иначе
			...
			Если ЕстьПравоДобавленияПользователей() Тогда
				Возврат ДействияСНовымПользователемСервиса();
			Иначе
				ВызватьИсключение НСтр("ka='ახალი მომხმარებლების დასამატებლად დაშვების უფლებები არასაკმარისია';ru='Недостаточно прав доступа для добавления новых пользователей'");
			КонецЕсли;
...
Процедура ПередНачаломОбработкиПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.ВнешниеПользователи")
	   И ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение НСтр("ka='სერვისის მოდელში გარე მომხმარებლებს მხარდაჭერა არ გააჩნია.';ru='Внешние пользователи не поддерживаются в модели сервиса.'");
	КонецЕсли;
	...
	Для каждого ПраваДоступаОбъектаXDTO Из ПраваДоступаОбъектовXDTO.Item Цикл
		...
		ИначеЕсли ПраваДоступаОбъектаXDTO.Object.Тип() = ТипКИПользователя Тогда
			...
			Если ВидКИ = Неопределено Тогда
				ШаблонСообщения = НСтр("ka='მიღებულია საკონტაქტო ინფორმაციის უცნობი ტიპი: %1';ru='Получен неизвестный вид контактной информации: %1'");
			...
			КонецЦикла;
		Иначе
			ШаблонСообщения = НСтр("ka='მიღებულია დაშვების ობიექტების უცნობი ტიპი: %1';ru='Получен неизвестный тип объектов доступа: %1'");
