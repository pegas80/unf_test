...
Функция НовыйПредставленияИндексов() Экспорт
	...
	Результат.Вставить("Заголовок", Новый ФорматированнаяСтрока(НСтр("ka='1СПАРК რისკები';ru='1СПАРК Риски';en='1SPARK Risks'")));
...
Процедура ОтобразитьИндексыСПАРК(
	...
	Если РезультатИндексыКонтрагента.СостояниеЗагрузкиДанных = ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ЗапущеноФоновоеЗадание") Тогда
		...
		Форма.Элементы.КартинкаОжиданиеЗагрузкиИндексовСПАРКРиски.Подсказка = НСтр("ka='Выполняется получение данных из Интернет.';ru='Выполняется получение данных из Интернет.';en='Retrieves data from the Internet.'");
	ИначеЕсли РезультатИндексыКонтрагента.СостояниеЗагрузкиДанных = ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ОтменаФоновогоЗадания") Тогда
		...
		Форма.Элементы.КартинкаОжиданиеЗагрузкиИндексовСПАРКРиски.Подсказка = НСтр("ka='Получение данных из Интернет отменено.';ru='Получение данных из Интернет отменено.';en='Getting data from the Internet has been canceled.'");
	ИначеЕсли РезультатИндексыКонтрагента.СостояниеЗагрузкиДанных = ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ОшибкаФоновогоЗадания") Тогда
		Форма.Элементы.КартинкаОжиданиеЗагрузкиИндексовСПАРКРиски.Видимость = Истина;
		Форма.Элементы.КартинкаОжиданиеЗагрузкиИндексовСПАРКРиски.Картинка = БиблиотекаКартинок.Предупреждение;
		Форма.Элементы.КартинкаОжиданиеЗагрузкиИндексовСПАРКРиски.Подсказка = НСтр("ka='Ошибка получения данных из Интернет.';ru='Ошибка получения данных из Интернет.';en='Error retrieving data from the Internet.'");
...
#Область ВыводДанных_Однострочный
		...
		ИначеЕсли РезультатИндексыКонтрагента.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеизвестныйИНН") Тогда
			...
			МассивСтрок.Добавить(НСтр("ka='Нет информации о контрагенте';ru='Нет информации о контрагенте';en='No information about the counterparty'"));
		...
		ИначеЕсли РезультатИндексыКонтрагента.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НекорректныйИНН") Тогда
			...
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ka='Нет информации о контрагенте';ru='Нет информации о контрагенте';en='No information about the counterparty'"),
					,
		...
		Иначе // Пустое поле - ошибок нет.
			...
			Если РезультатИндексыКонтрагента.СостояниеВыводаДанных = ПредопределенноеЗначение("Перечисление.СостоянияВыводаИндексовСПАРКРиски.ВКэшеНетДанных") Тогда
				...
				Если РезультатИндексыКонтрагента.СостояниеЗагрузкиДанных = ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ЗапущеноФоновоеЗадание") Тогда
					ТекстОшибки = НСтр("ka='მონაცემების მიღება...';ru='Получение данных...';en='Retrieving data ...'");
					ЦветТекстаОшибки = Неопределено; // Авто
				ИначеЕсли РезультатИндексыКонтрагента.СостояниеЗагрузкиДанных = ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ОтменаФоновогоЗадания") Тогда
					ТекстОшибки = НСтр("ka='Ошибка получения данных (слишком медленное соединение или отменено администратором)';ru='Ошибка получения данных (слишком медленное соединение или отменено администратором)';en='Receive error (too slow connection or administrator canceled)'");
				ИначеЕсли РезультатИндексыКонтрагента.СостояниеЗагрузкиДанных = ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ОшибкаФоновогоЗадания") Тогда
					ТекстОшибки = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных';en='Error receiving data'");
				Иначе // Загрузка завершена или не осуществлялась.
					ТекстОшибки = НСтр("ka='Ошибка получения данных (данные не получены)';ru='Ошибка получения данных (данные не получены)';en='Error receiving data (data not available)'");
				КонецЕсли;
			...
			ИначеЕсли РезультатИндексыКонтрагента.СостояниеВыводаДанных = ПредопределенноеЗначение("Перечисление.СостоянияВыводаИндексовСПАРКРиски.НеправильныйИНН") Тогда
				ЕстьОшибкаПолученияДанных = Истина;
				ТекстОшибки = НСтр("ka='Нет информации о контрагенте';ru='Нет информации о контрагенте';en='No information about the counterparty'");
			...
			Иначе
				ЕстьОшибкаПолученияДанных = Истина;
				ТекстОшибки = НСтр("ka='განუსაზღვრელი შეცდომა';ru='Неопределенная ошибка';en='Unspecified error'");
			КонецЕсли;
			...
			Иначе
				...
				Если (РезультатИндексыКонтрагента.Активен <> Истина) Тогда
					МассивСтрок.Добавить(
						Новый ФорматированнаяСтрока(
							СтрШаблон(
								НСтр("ka='Контрагент прекратил деятельность %1';ru='Контрагент прекратил деятельность %1';en='Counterparty discontinued operations 1%'"),
				...
				Иначе
					...
					Иначе
						Если (РезультатИндексыКонтрагента.ИндексДолжнойОсмотрительности >= 0)
							...
							МассивСтрок.Добавить(
								Новый ФорматированнаяСтрока(
									НСтр("ka='Индекс должной осмотрительности:';ru='Индекс должной осмотрительности:';en='Index due diligence:'")));
		...
		Если ЕстьОшибкаПодключения = Истина Тогда
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru='1СПАРК Риски: оценка надежности контрагентов. ';ka='1СПАРК Риски: оценка надежности контрагентов. '")));
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ka='დაწვრილებით';ru='Подробнее';en='More'"),
					,
					,
					,
					"SPARK:About"));
...
#Область ВыводДанных_Многострочный
		...
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ka='1СПАРК რისკები';ru='1СПАРК Риски';en='1SPARK Risks'"),
			...
			Иначе
				...
				Иначе
					...
					Если (РезультатИндексыКонтрагента.ИндексФинансовогоРиска >= 0)
						...
						МассивСтрок.Добавить(
							Новый ФорматированнаяСтрока(
								НСтр("ka='Индекс финансового риска:';ru='Индекс финансового риска:';en='financial risk index:'")));
						МассивСтрок.Добавить(Символы.НПП);
						МассивСтрок.Добавить(
							Новый ФорматированнаяСтрока(
								СтрШаблон(
					...
					Если (РезультатИндексыКонтрагента.ИндексПлатежнойДисциплины >= 0)
						...
						МассивСтрок.Добавить(
							Новый ФорматированнаяСтрока(
								НСтр("ka='Индекс платежной дисциплины:';ru='Индекс платежной дисциплины:';en='payment discipline index:'")));
						МассивСтрок.Добавить(Символы.НПП);
						МассивСтрок.Добавить(
							Новый ФорматированнаяСтрока(
								СтрШаблон(
		...
		Если ЕстьОшибкаПодключения = Истина Тогда
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru='Оценка надежности контрагентов. ';ka='Оценка надежности контрагентов. '")));
			МассивСтрок.Добавить(Символы.ПС);
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ka='Подробнее о сервисе';ru='Подробнее о сервисе';en='More about the service'"),
					,
					,
					,
					"SPARK:About"));
