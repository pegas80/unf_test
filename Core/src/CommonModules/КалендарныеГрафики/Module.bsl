...
Функция ДатыПоКалендарю(Знач ГрафикРаботы, Знач ДатаОт, Знач МассивДней, Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь, ВызыватьИсключение = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(ГрафикРаботы) Тогда
		Если ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("ka='სამუშაო გრაფიკი ან საწარმოო კალენდარი არ არის მითითებული.';ru='Не указан график работы или производственный календарь.';en='Unknown schedule or factory calendar.'");
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПоКалендарю = Неопределено Тогда
			СообщениеОбОшибке = НСтр("ka='საწარმოო კალენდარი ""%1"" თარიღ %2-დან სამუშაო დღეების აღნიშნული რაოდენობით არ არის შევსებული.';ru='Производственный календарь «%1» не заполнен с даты %2 на указанное количество рабочих дней.';en='Factory calendar ""%1"" is not filled with the date of 2% on a specified number of working days.'");
			Если ВызыватьИсключение Тогда
...
Функция РазностьДатПоКалендарю(Знач ГрафикРаботы, Знач ДатаНачала, Знач ДатаОкончания, ВызыватьИсключение = Истина) Экспорт
	...
	Если Результат.Пустой() Тогда
		Если ВызыватьИсключение Тогда
			СообщениеОбОшибке = НСтр("ka='მუშაობის გრაფიკი ""%1"" %2 პერიოდისთვის არ არის შევსებული.';ru='График работы «%1» не заполнен на период %2.';en='Schedule ""%1"" is empty for a period of 2%.'");
...
Функция ДатыБлижайшихРабочихДней(График, НачальныеДаты, ПолучатьПредшествующие = Ложь, ВызыватьИсключение = Истина, ИгнорироватьНезаполненностьГрафика = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе 
			...
			КонецЕсли;
			Если ВызыватьИсключение Тогда
				ТекстСообщения = НСтр("ka='თარიღ %1-ისთვის უახლოესი სამუშაო თარიღის დადგენა შეუძლებელია, შეიძლება, რომ მუშაობის გრაფიკი არ არის შევსებული.';ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, график работы не заполнен.';en='Unable to determine the next business date to the date of 1% may work schedule is empty.'");
...
Функция РасписанияРаботыНаПериод(Графики, ДатаНачала, ДатаОкончания) Экспорт
	...
	ВызватьИсключение НСтр("ka='ქვესისტემა ""მუშაობის გრაფიკები"" არ არის აღმოჩენილი.';ru='Подсистема «Графики работы» не обнаружена.';en='The subsystem ""Schedules"" was not found.'");
