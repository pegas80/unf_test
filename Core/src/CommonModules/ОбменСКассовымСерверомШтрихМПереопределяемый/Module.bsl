...
Процедура ОбработатьРезультатПодключенияКассы(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		...
		Исключение
			СообщениеОбОшибке = СообщениеОбОшибке + СтрШаблон(НСтр("ka='При подключении ошибочно обработан ККМ: %1';ru='При подключении ошибочно обработан ККМ: %1';en='When connecting erroneously processed CMC% 1'"),
		...
		Если ИнформацияОПодключении.ОсвободитьЛицензию И ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Если НЕ ОтменаРегистрации Тогда
				СообщениеОбОшибке = СообщениеОбОшибке +
					СтрШаблон(НСтр("ka='Не освобождена лицензия для ККМ с регистрационным номером: %1';ru='Не освобождена лицензия для ККМ с регистрационным номером: %1';en='Not available for license with CMC registration number:% 1'"),
						МенеджерЗаписиКассы.ИдентификаторТарификации) + Символы.ПС;
				ЕстьОшибка = Истина;
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ka='Освобождена лицензия %1 на сервере лицензий';ru='Освобождена лицензия %1 на сервере лицензий';en='Freed license% 1 on the license server'"), МенеджерЗаписиКассы.ИдентификаторТарификации);
...
Процедура ОбработатьРезультатОчисткиТоваров(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		Попытка
			...
			КассаОбъект.КассаККМ = КассовыйАппарат;
		Исключение
			СообщениеОбОшибке = СообщениеОбОшибке + СтрШаблон(НСтр("ka='При очистке товаров ошибочно обработан ККМ: %1';ru='При очистке товаров ошибочно обработан ККМ: %1';en='When cleaning products erroneously processed CMC% 1'", КодЯзыка),
				КассовыйАппарат) + Символы.ПС;
			ЕстьОшибки = Истина;
			Возврат;
		КонецПопытки;
		...
		Если ИнформацияОПодключении.ОсвободитьЛицензию И ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Если НЕ ОтменаРегистрации Тогда
				СообщениеОбОшибке = СообщениеОбОшибке +
					СтрШаблон(НСтр("ka='Не освобождена лицензия для ККМ с регистрационным номером: %1';ru='Не освобождена лицензия для ККМ с регистрационным номером: %1';en='Not available for license with CMC registration number:% 1'", КодЯзыка),
						КассаОбъект.ИдентификаторТарификации) + Символы.ПС;
				ЕстьОшибка = Истина;
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ka='Освобождена лицензия %1 на сервере лицензий';ru='Освобождена лицензия %1 на сервере лицензий';en='Freed license% 1 on the license server'",
...
Процедура ОбработатьРезультатРегистрацииТоваров(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		Попытка
			...
			КассаОбъект.КассаККМ = КассовыйАппарат;
		Исключение
			СообщениеОбОшибке = СообщениеОбОшибке + СтрШаблон(НСтр("ka='При регистрации товаров ошибочно обработан ККМ: %1';ru='При регистрации товаров ошибочно обработан ККМ: %1';en='Upon registration of the goods wrongly handled CMC: 1%'", КодЯзыка),
				КассовыйАппарат) + Символы.ПС;
			ЕстьОшибки = Истина;
			Возврат;
		КонецПопытки;
...
Процедура ОбработатьРезультатПроверкиЗагрузкиТоваров(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		Попытка
			...
			КассаОбъект.КассаККМ = КассовыйАппарат;
		Исключение
			СообщениеОбОшибке = СообщениеОбОшибке + СтрШаблон(НСтр("ka='При проверке выгрузки товаров ошибочно обработан ККМ: %1';ru='При проверке выгрузки товаров ошибочно обработан ККМ: %1';en='When checking the unloading of goods erroneously interpreted by CMC: 1%'", КодЯзыка),
				КассовыйАппарат) + Символы.ПС;
			ЕстьОшибки = Истина;
			Возврат;
		КонецПопытки;
		...
		КонецЕсли;
	Иначе
		СообщениеОбОшибке = СообщениеОбОшибке + СтрШаблон(НСтр("ka='При проверке выгрузки ошибочно обработан ККМ: %1';ru='При проверке выгрузки ошибочно обработан ККМ: %1';en='When checking the discharging incorrectly processed CMC% 1'", КодЯзыка),
			КассовыйАппарат) + Символы.ПС;
		ЕстьОшибка = Истина;
	КонецЕсли;
...
Функция УзелТовары() Экспорт
	...
	Если Не ЗначениеЗаполнено(ГлавныйУзелОбмена.Наименование) Тогда
		...
		ГлавныйУзелОбъект.Наименование = НСтр("ka='Центральный (товары и услуги)';ru='Центральный (товары и услуги)';en='Central (goods and services)'", КодЯзыка);
		Попытка
	...
	Если УзелТовары.Пустая() Тогда
		...
		УзелТовары.Наименование =
			НСтр("ka='Обмен ценами (товары и услуги)';ru='Обмен ценами (товары и услуги)';en='Exchange prices (goods and services)'", КодЯзыка);
		Попытка
			УзелТовары.Записать();
...
Процедура ВыполнитьЗагрузкуЗакрытияКассовойСмены(ИнформацияПоДокументу, КассовыйАппарат, ЕстьОшибки, СообщениеОбОшибке, СозданныйДокумент = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(ОтчетОРозничныхПродажах) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОтчетОРозничныхПродажах.Ссылка) Тогда
			ЕстьОшибки = Истина;
			СообщениеОбОшибке = НСтр("ka='Не найдено открытие кассовой смены';ru='Не найдено открытие кассовой смены';en='Opening cash change found'");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьКассовуюСмену(КассовыйАппарат, НомерСмены, ДатаСмены, ИдентификаторДокумента, ЕстьОшибки, СообщениеОбОшибке)
	...
	Иначе
		ЕстьОшибки = Ложь;
		СообщениеОбОшибке = НСтр("ka='Проверьте настройки кассы ККМ.';ru='Проверьте настройки кассы ККМ.';en='Check the settings of POS cash register.'");
...
Функция ПолучитьЭквайринговыйТерминалПоУмолчанию(ПолученныеДанные)
	...
	Иначе
		ВызватьИсключение(НСтр("ka='Не найден основной договор только что созданного контрагента.';ru='Не найден основной договор только что созданного контрагента.';en='Not found the main contract of the newly created company.'"));
	КонецЕсли;
