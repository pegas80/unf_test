	...
	Если (ПараметрыЦентраМониторинга.ВключитьЦентрМониторинга ИЛИ ПараметрыЦентраМониторинга.ЦентрОбработкиИнформацииОПрограмме) И ЭтоГлавныйУзел() Тогда 
		Если ПараметрыЦентраМониторинга.РегистрироватьДампы И ДатаЗапуска >= ПараметрыЦентраМониторинга.РегистрацияДамповСледующееФормирование Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - დამპების რეგისტრაცია';ru='Центр мониторинга - регистрация дампов'",
		...
		Если ПараметрыЦентраМониторинга.РегистрироватьБизнесСтатистику И ДатаЗапуска >= ПараметрыЦентраМониторинга.БизнесСтатистикаСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - სტატისტიკის ოპერაციების რეგისტრაცია';ru='Центр мониторинга - регистрация операций статистики'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		Если (ПараметрыЦентраМониторинга.РегистрироватьСтатистикуКонфигурации ИЛИ ПараметрыЦентраМониторинга.РегистрироватьНастройкуКонфигурации) И ДатаЗапуска >= ПараметрыЦентраМониторинга.СтатистикаКонфигурацииСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - კონფიგურაციის სტატისტიკის შეკრება';ru='Центр мониторинга - собрать статистику конфигурации'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		Если ДатаЗапуска >= ПараметрыЦентраМониторинга.ОтправкаДанныхСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - გასაგზავნად პაკეტის ჩამოყალიბება';ru='Центр мониторинга - сформировать пакет для отправки'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - მონიტორინგის მონაცემების გაგზავნა';ru='Центр мониторинга - отправить данные мониторинга'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
...
Процедура АгрегироватьЗамерыОперацийСтатистики(ТекущаяДата)
	...
	Если ОбработатьЗаписиДо > ГраницаАгрегирования Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. სტატისტიკის ოპერაციების გაზომვების აგრეგირება';ru='Центр мониторинга.Агрегировать замеры операций статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
			ВызватьИсключение Ошибка;
		КонецПопытки;
	КонецЕсли;
...
Процедура УдалитьУстаревшиеДанныеОперацийСтатистики()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. სტატისტიკის ოპერაციების მოძველებული მონაცემების წაშლა';ru='Центр мониторинга.Удалить устаревшие данные операций статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если РегистрыСведений.ОбластиСтатистики.СобиратьСтатистикуКонфигурации(ОбластьДанныхСтрока) Тогда
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. კონფიგურაციის სტატისტიკა ხელახლა განსაზღვრებადი';ru='Центр мониторинга.Статистика конфигурации переопределяемая'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,
						,,
						НСтр("ka='სეანსის განცალკევების დაყენება ვერ მოხერხდა. მონაცემთა არეალი';ru='Не удалось установить разделение сеанса.Область данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " = " + Формат(Выборка.ОбластьДанных, "ЧГ=0")
...
Процедура СформироватьПакетДляОтправки()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. პაკეტის შედგენა გასაგზავნად';ru='Центр мониторинга.Формирование пакета для отправки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыОтправки(Параметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. გაგზავნის პარამეტრების დაყენება';ru='Центр мониторинга.Установка параметров отправки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыЦентраМониторинга(НовыеПараметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. მონიტორინგის ცენტრის პარამეტრების დაყენება';ru='Центр мониторинга.Установка параметров центра мониторинга'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция УдалитьПараметрыЦентраМониторинга()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. მონიტორინგის ცენტრის პარამეტრების წაშლა';ru='Центр мониторинга.Удалить параметры центра мониторинга'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция НайтиКаталогДампов(КаталогНастроек) 
	...
	Если Файл.Существует() Тогда
		Попытка
			...
			КонецЦикла;
		Исключение
			Сообщение = НСтр("ka='ტექნოლოგიური ჟურნალის გამართვების ფაილის წაკითხვის შეცდომა';ru='Ошибка чтения файла настроек технологического журнала'");
			Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
			Сообщение = Сообщение + НСтр("ka='Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';ru='Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.'");
			
			ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი';ru='Центр мониторинга'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
		...
		Если ПараметрыДампов <> Неопределено Тогда
			Если НЕ ПараметрыДампов.Свойство("location") ИЛИ НЕ ПараметрыДампов.Свойство("create") ИЛИ НЕ ПараметрыДампов.Свойство("type") Тогда
				Сообщение = НСтр("ka='Ошибка секции сбора дампов в файле настроек технологического журнала';ru='Ошибка секции сбора дампов в файле настроек технологического журнала'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + НСтр("ka='Регистрация дампов не возможна. Удалите файл или восстановите настройки.';ru='Регистрация дампов не возможна. Удалите файл или восстановите настройки.'");
				ЧтениеXML.Закрыть();
	...
	ЧтениеXML.Закрыть();
		...
		Если ВРЕГ(ПервыйДочерний.ИмяУзла) = "CONFIG" Тогда
			...
			Попытка
				...
				ЗаписьXML.Закрыть();
			Исключение
				Сообщение = НСтр("ka='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';ru='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				
				ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი - დამპების რეგისტრაცია';ru='Центр мониторинга - регистрация дампов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
...
Функция СоздатьФайлНастроекСбораДампов(ПутьКаталога)
	...
	Попытка
		...
		ЗаписьXML.Закрыть();
	Исключение
		Сообщение = НСтр("ka='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';ru='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.'");
...
Функция ПолучитьКаталогНастроекТехнологическогоЖурнала()
	...
	Пока ПоискКаталога = Истина Цикл
		...
		Если КаталогиНастроек.Найти(ПутьКаталога) <> Неопределено Тогда
			...
			КаталогНастроек.ОписаниеОшибки = НСтр("ka='აღმოჩენილია ციკლური ბმული';ru='Обнаружена циклическая ссылка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Иначе
				...
				Иначе
					КаталогНастроек.Путь = "";
					КаталогНастроек.Существует = Ложь;
					КаталогНастроек.ОписаниеОшибки = НСтр("ka='კატალოგში გამართვების კონფიგურაციის ფაილი ვერ მოიძებნა';ru='Не найден файл конфигурации настроек в каталоге'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ПутьКаталога;
...
Функция ПолучитьКаталогИзФайлаКонфигурацииНастроек(ФайлКонфигурацииНастроек)
	...
	Иначе
		...
		КаталогНастроек.Существует = Ложь;
		КаталогНастроек.ОписаниеОшибки = НСтр("ka='ფაილში სექცია ConfLocation-ი ვერ მოიძებნა';ru='Не найдена секция ConfLocation в файле'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ФайлКонфигурацииНастроек.ПолноеИмя;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииЦентрМониторингаУдалениеДампа()
	Возврат НСтр("ka='მონიტორინგის ცენტრი. დამპის წაშლა';ru='Центр мониторинга.Удаление дампа'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

Функция СобытиеЖурналаРегистрацииЦентрМониторингаРазобратьБуферОперацийСтатистики()
	Возврат НСтр("ka='მონიტორინგის ცენტრი. სტატისტიკის ოპერაციების ბუფერის გარჩევა';ru='Центр мониторинга.Разобрать буфер операций статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
#КонецОбласти
