...
Функция ВыполнитьПроизвольнуюПроверку(Организация, ПараметрыКонтроля, ДатаНачала, ДатаОкончания, ОграниченияПроверки = Неопределено, СохранятьПротокол = Истина, ПолныйРезультат = Ложь) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მონაცემთა შემოწმება. მონაცემთა შემოწმების შეცდომა';ru='Проверка данных.Ошибка проверки данных'"),
...
Функция ВыполнитьПовторнуюПроверкуПараметраКонтроля(ПараметрКонтроля, Организация, СобытиеКалендаря = Неопределено, ДатаНачала, ДатаОкончания, ОргиниальноеДеревоРезультата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მონაცემთა შემოწმება.მონაცემთა განმეორებითი შემოწმების შეცდომა';ru='Проверка данных.Ошибка повторной проверки данных'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ВыполнитьПроверкуСведенийПоОрганизации(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока.Описание = НСтр("ka='გადამხდელის რეკვიზიტები';ru='Реквизиты Налогоплательщика'");
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Если ПустаяСтрока(ФИОФизическихЛиц.Фамилия) И (ОграниченияПроверки.Найти("Фамилия")<>Неопределено ИЛИ ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФИОФизическихЛиц.Фамилия",НСтр("ka='გვარი არ არის მითითებული';ru='Не указана фамилия'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(ФИОФизическихЛиц.Имя) И (ОграниченияПроверки.Найти("Имя")<>Неопределено ИЛИ ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФИОФизическихЛиц.Имя",НСтр("ka='სახელი არ არის მითითებული';ru='Не указано имя'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(ФИОФизическихЛиц.Отчество) И (ОграниченияПроверки.Найти("Отчество")<>Неопределено ИЛИ ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСПредупреждениями.Вставить("ФИОФизическихЛиц.Отчество",НСтр("ka='მამის სახელი არ არის მითითებული';ru='Не указано отчество'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.НаименованиеПолное) И (ОграниченияПроверки.Найти("НаименованиеПолное")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.НаименованиеПолное",НСтр("ka='მეწარმის იურიდიული სახელწოდება არ არის მითითებული';ru='Не указано юридическое название предпринимателя'"));
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицо.Пол) И (ОграниченияПроверки.Найти("Пол")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФизическоеЛицо.Пол",НСтр("ka='სქესი არ არის მითითებული';ru='Не указан пол'"));
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицо.ДатаРождения) И (ОграниченияПроверки.Найти("ДатаРождения")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФизическоеЛицо.ДатаРождения",НСтр("ka='დაბადების თარიღი არ არის მითითებული';ru='Не указана дата рождения'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.ИНН) И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ИНН",НСтр("ka='ს.კ / პ.ნ არ არის მითითებული';ru='Не указан ИНН'"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(СокрЛП(Организация.ИНН), Ложь, "") И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ИНН",НСтр("ka='გსნ-ის არაკორექტული მნიშვნელობა';ru='Некорректное значение ИНН'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.ОГРН) И (ОграниченияПроверки.Найти("ОГРНИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ОГРН",НСтр("ka='ძსსნ არ არის მითითებული';ru='Не указан ОГРН'"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(СокрЛП(Организация.ОГРН), Ложь, "") И (ОграниченияПроверки.Найти("ОГРНИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ОГРН",НСтр("ka='ინდმეწარმის ძსსნ-ის არაკორექტული მნიშვნელობა';ru='Некорректное значение ОГРНИП'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.КодПоОКПО) И (ОграниченияПроверки.Найти("КодПоОКПО")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КодПоОКПО",НСтр("ka='სოკ-ის კოდი არ არის მითითებული';ru='Не указан Код по ОКПО'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(Организация.РегистрационныйНомерПФР_ИП) И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР_ИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР_ИП",НСтр("ka='საპენსიო ფონდში სარეგისტრაციო ნომერი არ არის მითითებული (საკუთარი)';ru='Не указан Регистрационный номер в ПФР (на себя)'"));
		ИначеЕсли СтрДлина(СокрЛП(Организация.РегистрационныйНомерПФР_ИП))<> 14 И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР_ИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР_ИП",НСтр("ka='საპენსიო ფონდში სარეგისტრაციო ნომრის (საკუთარი) არაკორექტული მნიშვნელობა';ru='Некорректное значение Регистрационного номера в ПФР (на себя)'"));
		КонецЕсли;
		
	Иначе
		
		Если ПустаяСтрока(Организация.Наименование) И (ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.Наименование",НСтр("ka='პროგრამაში ორგანიზაციის წარდგენა არ არის მითითებული';ru='Не указано представление организации в программе'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.НаименованиеПолное) И (ОграниченияПроверки.Найти("НаименованиеПолное")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.НаименованиеПолное",НСтр("ka='ორგანიზაციის იურიდიული სახელწოდება არ არის მითითებული';ru='Не указано юридическое название организации'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(Организация.КПП) И (ОграниченияПроверки.Найти("КПП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КПП",НСтр("ka='აამკ არ არის მითითებული';ru='Не указан КПП'"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(СокрЛП(Организация.КПП), "") И (ОграниченияПроверки.Найти("КПП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КПП",НСтр("ka='აამკ-ის არაკორექტული მნიშვნელობა';ru='Некорректное значение КПП'"));
		КонецЕсли;
		...
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(СокрЛП(Организация.ОГРН), Истина, "") И (ОграниченияПроверки.Найти("ОГРН")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ОГРН",НСтр("ka='ძსსნ-ის არაკორექტული მნიშვნელობა';ru='Некорректное значение ОГРН'"));
		КонецЕсли;
		...
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.КодПоОКПОСоответствуетТребованиям(СокрЛП(Организация.КодПоОКПО), Истина, "") И (ОграниченияПроверки.Найти("КодПоОКПО")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КодПоОКПО",НСтр("ka='კოდის არაკორექტული მნიშვნელობა სოკ-ის მიხედვით';ru='Некорректное значение кода по ОКПО'"));
		КонецЕсли;
	...
	Если ПустаяСтрока(Организация.НаименованиеПлательщикаПриПеречисленииНалогов) И (ОграниченияПроверки.Найти("НаименованиеПлательщикаПриПеречисленииНалогов")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.НаименованиеПлательщикаПриПеречисленииНалогов",НСтр("ka='ბიუჯეტში გადასახადების გადახდისას ორგანიზაციის სახელწოდება არ არის მითითებული';ru='Не указано наименование организации при оплате налогов в бюджет'"));
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Организация.КассаПоУмолчанию) И (ОграниченияПроверки.Найти("КассаПоУмолчанию")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КассаПоУмолчанию",НСтр("ka='ძირითადი სალარო არ არის მითითებული';ru='Не указана основная касса'"));
	КонецЕсли;
	...
	Если ПустаяСтрока(Организация.СвидетельствоСерияНомер) И (ОграниченияПроверки.Найти("СвидетельствоСерияНомер")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СвидетельствоСерияНомер",НСтр("ka='მოწმობის ნომერი არ არის მითითებული';ru='Не указан номер свидетельства'"));
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Организация.СвидетельствоДатаВыдачи) И (ОграниченияПроверки.Найти("СвидетельствоДатаВыдачи")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СвидетельствоДатаВыдачи",НСтр("ka='მოწმობის გაცემის თარიღი არ არის მითითებული';ru='Не указан дата выдачи свидетельства'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодПоОКАТО) И (ОграниченияПроверки.Найти("КодПоОКАТО")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодПоОКАТО",НСтр("ka='ატოკ-ის მიხედვით კოდი არ არის მითითებული';ru='Не указан Код по ОКАТО'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодПоОКТМО) И (ОграниченияПроверки.Найти("КодПоОКТМО")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодПоОКТМО",НСтр("ka='მუნიციპალური ტერიტორიების კლასიფიკატორის მიხედვით კოდი არ არის მითითებული';ru='Не указан Код по ОКТМО'"));
	КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("КодНалоговогоОрганаПолучателя")<>Неопределено) Тогда
		Если ПустаяСтрока(Организация.КодНалоговогоОрганаПолучателя) Тогда
			ПоляСОшибками.Вставить("Объект.КодНалоговогоОрганаПолучателя",НСтр("ka='საგადასახადო სამსახურის ინსპექციის კოდი არ არის მითითებული';ru='Не указан Код ИФНС'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("НаименованиеИФНС")<>Неопределено) Тогда
		...
		Если Не ЗначениеЗаполнено(ГосОрган.Ссылка) Тогда
			ПоляСОшибками.Вставить("ПлатежныеРеквизитыФНСПредставление",НСтр("ka='საგადასახადო ორგანოს სახელწოდება არ არის მითითებული';ru='Не указано наименование налогового органа'"));
		КонецЕсли;
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОКОПФ) И (ОграниченияПроверки.Найти("КодОКОПФ")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКОПФ",НСтр("ka='საორგანიზაციო-სამართლებრივი ფორმების კლასიფიკატორის კოდი არ არის მითითებული';ru='Не указан Код ОКОПФ'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОКФС) И (ОграниченияПроверки.Найти("КодОКФС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКФС",НСтр("ka='საკუთრების ფორმების კლასიფიკატორის კოდი არ არის მითითებული';ru='Не указан Код ОКФС'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОКВЭД) И (ОграниченияПроверки.Найти("КодОКВЭД")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКВЭД",НСтр("ka='ემსკ-ის კოდი არ არის მითითებული';ru='Не указан Код ОКВЭД'"));
	ИначеЕсли НЕ ПроверкаДанныхКлиентСервер.КодОКВЭДСоответствуетТребованиям(Организация.КодОКВЭД) И (ОграниченияПроверки.Найти("КодОКВЭД")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКВЭД",НСтр("ka='ემსკ-ის მიხედვით საქმიანობის ტიპის კოდის არაკორექტული მნიშვნელობა';ru='Некорректное значение кода вида деятельности по ОКВЭД'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.СтраховойНомерПФР) И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СтраховойНомерПФР",НСтр("ka='საპენსიო ფონდის სადაზღვევო ნომერი არ არის მითითებული';ru='Не указан страховой номер ПФР'"));
	ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СокрЛП(Организация.СтраховойНомерПФР),"") И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СтраховойНомерПФР",НСтр("ka='საპენსიო ფონდის სადაზღვევო ნომრის არაკორექტული მნიშვნელობა';ru='Некорректное значение страхового номера ПФР'"));
	КонецЕсли;
	...
	Если ПустаяСтрока(Организация.РегистрационныйНомерПФР) И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР",НСтр("ka='საპენსიო ფონდში დამსაქმებლად სარეგისტრაციო ნომერი არ არის მითითებული';ru='Не указан Регистрационный номер в ПФР в качестве работодателя'"));
	ИначеЕсли НЕ ПроверкаДанныхКлиентСервер.РегНомерПФРСоответствуетТребованиям(СокрЛП(Организация.РегистрационныйНомерПФР)) И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР",НСтр("ka='საპენსიო ფონდში სარეგ. ნომრის მნიშვნელობა არ შეესაბამება შაბლონს:""XXX-XXX-XXXXXX"" (X - არის ნებისმიერი ციფრი)!';ru='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра)!'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОрганаПФР) И (ОграниченияПроверки.Найти("КодОрганаПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОрганаПФР",НСтр("ka='საპენსიო ფონდის ორგანოს კოდი არ არის მითითებული';ru='Не указан Код органа ПФР'"));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Организация.ДатаРегистрации) И (ОграниченияПроверки.Найти("ДатаРегистрации")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.ДатаРегистрации",НСтр("ka='რეგისტრაციის თარიღი არ არის მითითებული';ru='Не указана дата регистрации'"));
	КонецЕсли;
	
	
	Если Не ЗначениеЗаполнено(Организация.КодОрганаФСС) И (ОграниченияПроверки.Найти("КодОрганаФСС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОрганаФСС",НСтр("ka='სდფ-ის ორგანოს კოდი არ არის მითითებული';ru='Не указан код органа ФСС'"));
	КонецЕсли;
	...
	Если ПустаяСтрока(Организация.РегистрационныйНомерФСС) И (ОграниченияПроверки.Найти("РегистрационныйНомерФСС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерФСС",НСтр("ka='სდფ-ში სარეგისტრაციო ნომერი არ არის მითითებული';ru='Не указан Регистрационный номер в ФСС'"));
	ИначеЕсли СтрДлина(СокрЛП(Организация.РегистрационныйНомерФСС))<> 10 И (ОграниченияПроверки.Найти("РегистрационныйНомерФСС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерФСС",НСтр("ka='სოცდაზღვევის ფონდში სარეგისტრაციო ნომრის არაკორექტული მნიშვნელობა';ru='Некорректное значение Регистрационного номера в ФСС'"));
	КонецЕсли;
	
	Если (ОграниченияПроверки.Найти("ЮрАдрес")<>Неопределено) Тогда
		Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
			ТекстОшибки = НСтр("ka='იურიდიული მისამართი არ არის მითითებული (ჩაწერის მიხედვით მისამართი)';ru='Не указан юридический адрес (Адрес по прописке)'")
		Иначе
			ТекстОшибки = НСтр("ka='იურიდიული მისამართი არ არის მითითებული';ru='Не указан юридический адрес'");
		КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("Телефон")<>Неопределено) Тогда
		...
		Если Строки.Количество() = 0 Тогда
			ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ka='საკონტაქტო ტელეფონი არ არის მითითებული';ru='Не указан контактный телефон'"));
		Иначе
			Если ПустаяСтрока(Строки[0].Представление) Тогда
				ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ka='საკონტაქტო ტელეფონი არ არის მითითებული';ru='Не указан контактный телефон'"));
			КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("EmailОрганизации")<>Неопределено) Тогда
		...
		Если Строки.Количество() = 0 Тогда
			ПоляСОшибками.Вставить("ПредставлениеКИ_1",НСтр("ka='e-mail-ი არ არის მითითებული';ru='Не указана e-mail'"));
		Иначе
			Если ПустаяСтрока(Строки[0].Представление) Тогда
				ПоляСОшибками.Вставить("ПредставлениеКИ_1",НСтр("ka='e-mail-ი არ არის მითითებული';ru='Не указан e-mail'"));
			КонецЕсли;
		КонецЕсли;
	...
	Если ПоляСОшибками.Количество() > 0 Тогда
		...
		Строка.Описание = НСтр("ka='სავალდებულო რეკვიზიტები არ არის შევსებული';ru='Не заполнены необходимые реквизиты'");
	...
	Если ПоляСПредупреждениями.Количество() > 0 Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='ზოგიერთ ველთან დაკავშირებით პრობლემები არის';ru='Есть проблемы с некоторыми полями'");
	...
	Если ПоляСОшибками.Количество() = 0 И ПоляСПредупреждениями.Количество() = 0 Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='ძირითადი რეკვიზიტები შევსებულია';ru='Основные реквизиты заполнены'");
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		Если (ОграниченияПроверки.Найти("Руководитель")<>Неопределено) Тогда
			...
			Если ЗначениеЗаполнено(Организация.ПодписьРуководителя) Тогда
				...
				Если ДанныеФЛ.Количество() > 0 Тогда
					...
					Если ПустаяСтрока(ФИО.Фамилия) ИЛИ ПустаяСтрока(ФИО.Имя) Тогда
						Строка = КорневаяСтрока.Строки.Добавить();
						Строка.Описание = НСтр("ka='ხელმძღვანელის - ფიზიკური პირის გვარი-სახელი არ არის მითითებული';ru='Не указано ФИО руководителя - физического лица'");
						Строка.РезультатКонтроля = 2;
						...
						ОшибочныеПоля.Вставить("Объект.Наименование",НСтр("ka='გვარი-სახელი არ არის მითითებული';ru='Не указано ФИО'"));
						Строка.Расшифровка.ПараметрыРасшифровки = ОшибочныеПоля;
					Иначе
						Строка = КорневаяСтрока.Строки.Добавить();
						Строка.Описание = НСтр("ka='ორგანიზაციის ხელმძღვანელი დადგენილია';ru='Руководитель организации установлен'");
						Строка.РезультатКонтроля = 0;
						Строка.ПараметрКонтроля = "СведенияОбОрганизации";
					КонецЕсли;
				Иначе
					...
					Строка = КорневаяСтрока.Строки.Добавить();
					Строка.Описание = НСтр("ka='ფიზიკური პირის გვარი-სახელი არ არის მითითებული';ru='Не указано ФИО физического лица'");
					Строка.РезультатКонтроля = 2;
					Строка.ПараметрКонтроля = "СведенияОбОрганизации";
			...
			Иначе
				...
				Строка = КорневаяСтрока.Строки.Добавить();
				Строка.Описание = НСтр("ka='ორგანიზაციის ხელმძღვანელი არ არის მიცემული';ru='Не задан руководитель организации'");
				Строка.РезультатКонтроля = 2;
				Строка.ПараметрКонтроля = "СведенияОбОрганизации";
		...
		Если ЗначениеЗаполнено(Организация.ПодписьГлавногоБухгалтера) Тогда
				...
				Если ДанныеФЛ.Количество() > 0 Тогда
					...
					Если ПустаяСтрока(ФИО.Фамилия) ИЛИ ПустаяСтрока(ФИО.Имя) Тогда
						Строка = КорневаяСтрока.Строки.Добавить();
						Строка.Описание = НСтр("ka='მთავარი ბუღალტერის - ფიზიკური პირის გვარ-სახელი არ არის მითითებული';ru='Не указано ФИО главного бухгалтера - физического лица'");
						Строка.РезультатКонтроля = 2;
						Строка.ПараметрКонтроля = "СведенияОбОрганизации";
					...
					Иначе
						Строка = КорневаяСтрока.Строки.Добавить();
						Строка.Описание = НСтр("ka='ორგანიზაციის მთავარი ბუღალტერი დადგენილია';ru='Главный бухгалтер организации установлен'");
						Строка.РезультатКонтроля = 0;
						Строка.ПараметрКонтроля = "СведенияОбОрганизации";
					КонецЕсли;
			...
			Иначе
				...
				Строка = КорневаяСтрока.Строки.Добавить();
				Строка.Описание = НСтр("ka='ორგანიზაციის მთავარი ბუღალტერი არ არის მიცემული';ru='Не задан главный бухгалтер организации'");
				Строка.РезультатКонтроля = 2;
				Строка.ПараметрКонтроля = "СведенияОбОрганизации";
...
Процедура ВыполнитьПроверкуПараметровУчета(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='აღრიცხვის პარამეტრების გამართვები';ru='Настройки параметров учета'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если НЕ РегламентированнаяОтчетностьУСН.ПараметрыУСНЗаполнены(Организация, ДатаПроверки) Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='აღრიცხვის პარამეტრები არ არის შევსებული';ru='Параметры учета не заполнены'");
		Строка.РезультатКонтроля = 2;
...
Процедура ВыполнитьПроверкуБанковскихСчетов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='საბანკო ანგარიშები';ru='Банковские счета'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если ЗначениеЗаполнено(ОсновнойБанковскийСчет) Тогда
		Строка.Описание = НСтр("ka='ძირითადი საბანკო ანგარიში მიცემულია';ru='Основной банковский счет задан'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
	Иначе
		ПоляСПредупреждениями = Новый Соответствие;
		ПоляСПредупреждениями.Вставить("Объект.БанковскийСчетПоУмолчанию", НСтр("ka='ძირითადი საბანკო ანგარიში არ არის მიცემული';ru='Не задан основной банковский счет'"));
		
		Строка.Описание = НСтр("ka='ძირითადი საბანკო ანგარიში არ არის მიცემული';ru='Основной банковский счет не задан'");
		Строка.РезультатКонтроля = 1;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
		Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
		Строка.Расшифровка.ТипРасшифровки = "Объектная";
	...
	Если Результат[1].Пустой() Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='საბანკო ანგარიშები არ არის';ru='Нет банковских счетов'");
		Строка.РезультатКонтроля = 1;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
		Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
	...
	ИначеЕсли Результат[0].Пустой() Тогда
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='საბანკო ანგარიშების გახსნის თარიღები მიცემულია';ru='Даты открытия банковских счетов заданы'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
	Иначе
		Пока Выборка.Следующий() Цикл
			...
			ПоляСПредупреждениями = Новый Соответствие;
			ПоляСПредупреждениями.Вставить("Объект.ДатаОткрытия", НСтр("ka='ანგარიშის გახსნის თარიღი არ არის მითითებული «';ru='Не указана дата открытия счета «'")+Выборка.ПредставлениеБанкСчета+"»");
			
			Строка.Описание = НСтр("ka='ანგარიშის გახსნის თარიღი არ არის მითითებული «';ru='Не указана дата открытия счета «'")+ Выборка.ПредставлениеБанкСчета + "»";
			Строка.РезультатКонтроля = 1;
			Строка.ПараметрКонтроля = "БанковскиеСчета";
			Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
			Строка.Расшифровка.ТипРасшифровки = "Объектная";
...
Процедура ВыполнитьПроверкуНачальныхОстатков(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='საწყისი ნაშთები';ru='Начальные остатки'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Не ЗначениеЗаполнено(ДатаРегистрации) Или (ДатаРегистрации >= ГодСдачиОтчетности) Тогда
		...
		Строка.Описание = НСтр("ka='საწყისი ნაშთები შევსებულია';ru='Начальные остатки заполнены'");
		Строка.РезультатКонтроля = 0;
	...
	Иначе
		Если МассивРезультатов[1].Пустой() Тогда
			...
			Если ВыборкаВНО.Количество() = 0 Тогда
				...
				Строка.Описание = НСтр("ka='საწყისი ნაშთები არ არის შევსებული';ru='Начальные остатки не заполнены'");
				Строка.РезультатКонтроля = 1;
				Строка.ПараметрКонтроля = "НачальныеОстатки";
				Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
				Строка.Расшифровка.ТипРасшифровки = "Объектная";
...
Процедура ВыполнитьПроверкуКонтрольПроведенных(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='გაუტარებელი დოკუმენტების კონტროლი';ru='Контроль непроведенных документов'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='გაუტარებელი დოკუმენტები არ არის';ru='Непроведенных документов нет'");
		Строка.РезультатКонтроля = 0;
	...
	Иначе
		...
		Если ТаблицаРезультата.Количество() = 11 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = НСтр("ka='არის გაუტარებელი დოკუმენტები. ანგარიშგება ახდენს ამ დოკუმენტების იგნორირებას. თუ ისინი არ არის საჭირო, მაშინ მონიშნეთ წასაშლელად.';ru='Есть непроведенные документы. Эти документы игнорируются отчетностью. Если они не нужны, то пометьте их на удаление'");
			Строка.РезультатКонтроля = 1;
			Строка.ПараметрКонтроля = "КонтрольПроведенных";
			Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
			Строка.Расшифровка.ТипРасшифровки = "Списочная";
		...
		Иначе
			...
			Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
				...
				Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='დოკუმენტი ""%1"" არ არის გატარებული და წასაშლელად მონიშნული';ru='Документ «%1» не проведен и не помечен на удаление'"),
...
Процедура ВыполнитьПроверкуНаличияПоказателейЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='Показатели ЕНВД';ru='Показатели ЕНВД'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		ПоляСОшибкам.Вставить("Команда.НастроитьЕНВД",НСтр("ka='მშეგ-ის მიხედვით საქმიანობის აქტუალური ტიპები არ არის';ru='Нет актуальных видов деятельности по ЕНВД'"));
		
		Строка = КорневаяСтрока.Строки.Добавить();
		
		Строка.Описание = НСтр("ka='მშეგ-ის მიხედვით საქმიანობის აქტუალური ტიპები არ არის';ru='Нет актуальных видов деятельности по ЕНВД'");
	...
	Иначе
		...
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='მშეგ-ის მაჩვენებლები შევსებულია';ru='Показатели ЕНВД заполнены'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "НаличиеПоказателейЕНВД";
...
Процедура ВыполнитьПроверкуЗаполненияПоказателейЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	Для Каждого Строка Из ДокументЕНВД.ПоказателиЕНВД Цикл
		...
		Если Строка.ЗначениеФизическогоПоказателя1 = 0 И Строка.ВыработкаДней1 > 0 Тогда
			ПоляСОшибками.Вставить("Объект.ПоказателиЕНВД["+(Строка.НомерСтроки-1)+"].ЗначениеФизическогоПоказателя1", НСтр("ka='ფიზ. მაჩვენებელი არ არის';ru='Нет физ. показателя'"));
		КонецЕсли;
		
		Если Строка.ЗначениеФизическогоПоказателя2 = 0  И Строка.ВыработкаДней2 > 0 Тогда
			ПоляСОшибками.Вставить("Объект.ПоказателиЕНВД["+(Строка.НомерСтроки-1)+"].ЗначениеФизическогоПоказателя2", НСтр("ka='ფიზ. მაჩვენებელი არ არის';ru='Нет физ. показателя'"));
		КонецЕсли;
		
		Если Строка.ЗначениеФизическогоПоказателя3 = 0  И Строка.ВыработкаДней3 > 0 Тогда
			ПоляСОшибками.Вставить("Объект.ПоказателиЕНВД["+(Строка.НомерСтроки-1)+"].ЗначениеФизическогоПоказателя3", НСтр("ka='ფიზ. მაჩვენებელი არ არის';ru='Нет физ. показателя'"));
		КонецЕсли;
	...
	Если ПоляСОшибками.Количество() > -0 Тогда
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='მშეგ-ის მიხედვით ზოგიერთი მაჩვენებელი არ არის შევსებული';ru='Не заполнены некоторые показатели по ЕНВД'");
		Строка.РезультатКонтроля = 2;
	...
	Иначе
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='მშეგ-ის მაჩვენებლები არაკორექტულად არის შევსებული';ru='Показатели ЕНВД заполнены корректно'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "ЗаполнениеПоказателейЕНВД";
	КонецЕсли;
...
Процедура КорректностьЗаполненияВидовДеятельностиЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='საქმიანობის ტიპების კორექტულობა მშეგ';ru='Корректность видов деятельности ЕНВД'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='საქმიანობის ტიპები მშეგ არაკორექტულად არის შევსებული';ru='Виды деятельности ЕНВД заполнены корректно'");
		Строка.РезультатКонтроля = 0;
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Код010203 И Выборка.КоличествоОкун = 0 И Год(ДатаНачала) < 2015 Тогда
				ПоляСОшибкам.Вставить("СсылкаЗаполненияОКУН",НСтр("ka='საქმიანობის ტიპების კოდებისთვის 01, 02, 03 აუცილებელია მოსახლეობის მომსახურების კლასიფიკატორის კოდები';ru='Для кодов видов деятельности 01, 02, 03 необходимы коды ОКУН'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.КодНалоговогоОрганаПолучателя) Тогда
				ПоляСОшибкам.Вставить("Объект.КодНалоговогоОрганаПолучателя",НСтр("ka='მიმღები საგადასახადო ორგანოს კოდი არ არის შევსებული';ru='Не заполнен код налогового органа получателя'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.СтавкаЕНВД) Тогда
				ПоляСОшибкам.Вставить("Объект.СтавкаЕНВД",НСтр("ka='მშეგ-ის განაკვეთი არ არის შევსებული';ru='Не заполнена ставка ЕНВД'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.КодПоОКАТО) Тогда
				ПоляСОшибкам.Вставить("Объект.КодПоОКАТО",НСтр("ka='მუნიციპალური ტერიტორიების კლასიფიკატორის მიხედვით კოდი არ არის შევსებული';ru='Не заполнен код по ОКТМО'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.АдресОсуществленияДеятельности) Тогда
				ПоляСОшибкам.Вставить("КонтактнаяИнформацияПоле0",НСтр("ka='საქმიანობის განხორციელების ადგილი არ არის შევსებული';ru='Не заполнено место осуществления деятельности'"));
			КонецЕсли;
			...
			Если ПоляСОшибкам.Количество() > 0 Тогда
				...
				Строка.Описание = НСтр("ka='საქმიანობის ტიპს «';ru='У вида деятельности «'") + Выборка.Ссылка + НСтр("ka='» не заполнены реквизиты';ru='» не заполнены реквизиты'");
			...
			Если ПоляСПредупреждениями.Количество() > 0 Тогда
				...
				Строка.Описание = НСтр("ka='საქმიანობის ტიპს «';ru='У вида деятельности «'") + Выборка.Ссылка + НСтр("ka='» есть проблемы с некоторыми полями';ru='» есть проблемы с некоторыми полями'");
...
Процедура ВыполнитьПроверкуСведенийПоСотрудникам(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='თანამშრომლების რეკვიზიტები';ru='Реквизиты сотрудников'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Пока ВыборкаСотрудники.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ФизЛицо) Тогда
			Если ПустаяСтрока(ВыборкаСотрудники.Фамилия) И (ОграниченияПроверки.Найти("Фамилия")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("Наименование",НСтр("ka='გვარი არ არის მითითებული';ru='Не указана фамилия'"));
			КонецЕсли;
			
			Если ПустаяСтрока(ВыборкаСотрудники.Имя) И (ОграниченияПроверки.Найти("Имя")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("Наименование",НСтр("ka='სახელი არ არის მითითებული';ru='Не указано имя'"));
			КонецЕсли;
			
			Если ПустаяСтрока(ВыборкаСотрудники.Отчество) И (ОграниченияПроверки.Найти("Отчество")<>Неопределено) Тогда
				ПоляСПредупреждениями.Вставить("Наименование",НСтр("ka='მამის სახელი არ არის მითითებული';ru='Не указано отчество'"));
			КонецЕсли;
			
			Если ПустаяСтрока(ФизЛицо.ИНН) И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.ИНН",НСтр("ka='ს.კ / პ.ნ არ არის მითითებული';ru='Не указан ИНН'"));
			ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(СокрЛП(ФизЛицо.ИНН), Ложь, "") И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.ИНН",НСтр("ka='გსნ-ის არაკორექტული მნიშვნელობა';ru='Некорректное значение ИНН'"));
			КонецЕсли;
			...
			Если ПустаяСтрока(ФизЛицо.СтраховойНомерПФР) И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.СтраховойНомерПФР",НСтр("ka='საპენსიო ფონდის სადაზღვევო ნომერი არ არის მითითებული';ru='Не указан страховой номер ПФР'"));
			ИначеЕсли Не РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СокрЛП(ФизЛицо.СтраховойНомерПФР),"") И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.СтраховойНомерПФР",НСтр("ka='საპენსიო ფონდში სადაზღვევო ნომრის არაკორექტული მნიშვნელობა';ru='Некорректное значение Страхового номера в ПФР'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ФизЛицо.Гражданство) И (ОграниченияПроверки.Найти("Гражданство")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.Гражданство",НСтр("ka='მოქალაქეობა არ არის მითითებული';ru='Не указано гражданство'"));
			КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("АдресПоПрописке")<>Неопределено) Тогда
				...
				Если Строки.Количество() = 0 Тогда
					ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ka='ჩაწერის მიხედვით მისამართი არ არის მითითებული';ru='Не указан адрес по прописке'"));
				Иначе
					Если ПустаяСтрока(Строки[0].Представление) Тогда
						ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ka='ჩაწერის მიხედვით მისამართი არ არის მითითებული';ru='Не указан адрес по прописке'"));
					КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("Индекс")<>Неопределено) Тогда
				...
				Строки = ФизЛицо.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.АдресФизЛицаПоПрописке));
				Если Строки.Количество() = 0 Тогда
					ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ka='ჩაწერის მიხედვით მისამართის ინდექსი არ არის მითითებული';ru='Не указан индекс адреса по прописке'"));
				Иначе
					...
					ИначеЕсли СтрДлина(СокрЛП(СтруктураАдреса.Индекс))<> 6 Тогда
						ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ka='ჩაწერის მიხედვით მისამართის ინდექსი არაკორექტულად არის შევსებული';ru='Некорректно заполнен индекс адреса по прописке'"));
					КонецЕсли;
				КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("Телефон")<>Неопределено) Тогда
				...
				Если Строки.Количество() = 0 Тогда
					ПоляСОшибками.Вставить("Физлицо",НСтр("ka='საკონტაქტო ტელეფონი არ არის მითითებული';ru='Не указан контактный телефон'"));
				Иначе
					Если ПустаяСтрока(Строки[0].Представление) Тогда
						ПоляСОшибками.Вставить("Физлицо",НСтр("ka='საკონტაქტო ტელეფონი არ არის მითითებული';ru='Не указан контактный телефон'"));
					КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("УдостоверениеЛичности")<>Неопределено) Тогда
				...
				Если ДокументУдЛичности = Неопределено Тогда
					ПоляСОшибками.Вставить("КлючДокументФизЛица.Серия", НСтр("ka='პირადობის დამადასტურებელი დოკუმენტი არ არის მითითებული';ru='Не указан документ, удостоверяющий личность'"));
				Иначе
					
					Если ПустаяСтрока(ДокументУдЛичности.Серия) Тогда
						ПоляСОшибками.Вставить("КлючДокументФизЛица.Серия", НСтр("ka='პირადობის დამადასტურებელი დოკუმენტის სერია არ არის მითითებული';ru='Не указана серия документа, удостоверяющего личность'"));
					КонецЕсли;
					
					Если ПустаяСтрока(ДокументУдЛичности.Номер) Тогда
						ПоляСОшибками.Вставить("КлючДокументФизЛица.Номер", НСтр("ka='პირადობის დამადასტურებელი დოკუმენტის ნომერი არ არის მითითებული';ru='Не указан номер документа, удостоверяющего личность'"));
					КонецЕсли;
			...
			КонецЕсли;
		Иначе
			ПоляСОшибками.Вставить("Физлицо",НСтр("ka='Не заполнено физическое лицо';ru='Не заполнено физическое лицо'"));
		КонецЕсли;
		...
		Если ПоляСОшибками.Количество() > 0 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელ ""%1""-ს სავალდებულო რეკვიზიტები არ აქვს შევსებული';ru='У сотрудника «%1» не заполнены необходимые реквизиты'"),
		...
		Если ПоляСПредупреждениями.Количество() > 0 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელ ""%1""-ს ზოგიერთ ველთან დაკავშირებით პრობლემები აქვს';ru='У сотрудника «%1» есть проблемы с некоторыми полями'"),
				ВыборкаСотрудники.СотрудникПредставление);
		...
		Если ПоляСОшибками.Количество() = 0 И ПоляСПредупреждениями.Количество() = 0 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელ ""%1""-თან დაკავშირებით ძირითადი რეკვიზიტები შევსებულია';ru='Основные реквизиты по сотруднику «%1» заполнены'"),
				ВыборкаСотрудники.СотрудникПредставление);
			Строка.РезультатКонтроля = 0;
			Строка.ПараметрКонтроля = "СведенияПоСотрудникам";
...
Процедура ВыполнитьПроверкуТарифаСтраховыхВзносов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='სადაზღვევო შენატანების ტარიფის გამართვები';ru='Настройки тарифа страховых взносов'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если НЕ РегламентированнаяОтчетностьУСН.ТарифыСтраховыхВзносовЗаполнены(Организация, ДатаПроверки) Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='სადაზღვევო შენატანების ტარიფები არ არის შევსებული';ru='Тарифы страховых взносов не заполнены'");
		Строка.РезультатКонтроля = 2;
...
Процедура ВыполнитьПроверкуТорговыхТочек(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='სავაჭრო წერტილები';ru='Торговые точки'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		ПоляСОшибкам = Новый Соответствие;
		ПоляСОшибкам.Вставить("Команда.ТорговыеТочки",НСтр("ka='აქტუალური სავაჭრო წერტილები არ არის';ru='Нет актуальных торговых точек'"));
		
		Строка = КорневаяСтрока.Строки.Добавить();
		
		Строка.Описание = НСтр("ka='აქტუალური სავაჭრო წერტილები არ არის';ru='Нет актуальных торговых точек'");
	...
	Иначе
		...
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ka='სავაჭრო წერტილების მაჩვენებლები შევსებულია';ru='Показатели торговых точек заполнены'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "СведенияПоТорговымТочкам";
...
Процедура ВыполнитьПроверкуТорговыхТочекРозничнойПродажиАлкоголя(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ka='ალკოჰოლის საცალო ვაჭრობის სავაჭრო წერტილები';ru='Торговые точки розничной продажи алкоголя'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		Строка.Описание = НСтр("ka='ესასს-თან შეერთებული სავაჭრო წერტილები არ არის';ru='Нет торговых точек, подключенных к ЕГАИС'");
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			Если Не ЗначениеЗаполнено(Выборка.Представление) Тогда
				ЕстьОшибка = Истина;
				ПоляСОшибками = Новый Соответствие;
				ПоляСОшибками.Вставить("ПредставлениеКИ_1", НСтр("ka='ფაქტობრივი მისამართი არ არის მითითებული';ru='Не указан фактический адрес'"));
				
				Строка = КорневаяСтрока.Строки.Добавить();
				Строка.Описание = НСтр("ka='სავაჭრო წერტილის მისამართი არ არის მითითებული «';ru='Не указан адрес торговой точки «'")+ Выборка.Наименование + "»";
				Строка.РезультатКонтроля = 2;
				Строка.ПараметрКонтроля = "ТорговыеТочкиРозничнойПродажиАлкоголя";
				Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
				Строка.Расшифровка.ТипРасшифровки = "Объектная";
		...
		Если Не ЕстьОшибка Тогда
			КорневаяСтрока.РезультатКонтроля = 0;
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = НСтр("ka='ალკოჰოლის საცალო ვაჭრობის სავაჭრო წერტილები შევსებულია';ru='Торговые точки розничной продажи алкоголя заполнены'");
			Строка.РезультатКонтроля = 0;
			Строка.ПараметрКонтроля = "ТорговыеТочкиРозничнойПродажиАлкоголя";
		КонецЕсли;
	КонецЕсли;
