...
Процедура ПриСозданииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация", ПоложениеЗаголовкаКИ = "",
	...
	Для Каждого ОбъектКонтактнойИнформации Из КонтактнаяИнформация Цикл
		...
		Иначе
			...
			Исключение
				...
				НСтр("ka='საკონტაქტო ინფორმაციის არაკორექტული ფორმატი.';ru='Некорректный формат контактной информации.'"), ,
	...
	Если Не ОтложеннаяИнициализация 
		...
		Описание = НСтр("ka='საკონტაქტო ინფორმაციის დამატებითი ველის დამატება';ru='Добавить дополнительное поле контактной информации'");
		...
		Кнопка.Заголовок = НСтр("ka='დამატება';ru='Добавить'");
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если СтрокаТаблицы.ЭтоРеквизитТабличнойЧасти Тогда
			...
			Для Каждого СтрокаТабличнойЧастиФормы Из ТабличнаяЧастьФормы Цикл
				...
				Если ОбязательноеЗаполнение И ПустаяСтрока(Представление) Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი ""%1"" არ არის შევსებული.';ru='Поле ""%1"" не заполнено.'"), ВидИнформации.Наименование),,Поле);
		...
		Иначе
			...
			Если ВидИнформации <> ПредыдущийВид И ОбязательноеЗаполнение И ПустаяСтрока(Представление)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი ""%1"" არ არის შევსებული.';ru='Поле ""%1"" не заполнено.'"), ВидИнформации.Наименование),,, ИмяРеквизита);
				ТекущийУровеньОшибок = 2;
...
Функция СтранаАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ka='ქვეყნის დადგენა შეუძლებელია, მოსალოდნელია მისამართი.';ru='Невозможно определить страну, ожидается адрес.'");
	КонецЕсли;
...
Функция РегионАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Адрес = XDTOАдрес.Состав;
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ka='საქართველოს რაიონის დადგენა შეუძლებელია, მოსალოდნელია მისამართი.';ru='Невозможно определить субъекта РФ, ожидается адрес.'");
	КонецЕсли;
...
Функция ГородАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Адрес = XDTOАдрес.Состав;
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ka='ქალაქის დადგენა შეუძლებელია, მოსალოდნელია მისამართი.';ru='Невозможно определить город, ожидается адрес.'");
	КонецЕсли;
...
Функция ДоменАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	ВызватьИсключение НСтр("ka='დომენის დადგენა შეუძლებელია, მოსალოდნელია ელექტრონული ფოსტა ან ვებ-ბმული.';ru='Невозможно определить домен, ожидается электронная почта или веб-ссылка.'");	
КонецФункции
...
Функция НомерТелефонаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	ВызватьИсключение НСтр("ka='ნომრის დადგენა შეუძლებელია, მოსალოდნელია ტელეფონი ან ფაქსი.';ru='Невозможно определить номер, ожидается телефона или факс.'");
КонецФункции
...
Процедура СоздатьВТКонтактнаяИнформация(МенеджерВременныхТаблиц, МассивОбъектов, ТипыКИ = Неопределено, ВидыКИ = Неопределено, Дата = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивОбъектов) <> Тип("Массив") ИЛИ МассивОбъектов.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='არასწორი მნიშვნელობა საკონტაქტო ინფორმაციის მფლობელების მასივისთვის.';ru='Неверное значение для массива владельцев контактной информации.'");
	КонецЕсли;
	...
	Для каждого ОбъектСКонтактнойИнформацией Из ОбъектыСГруппировкойПоТипам Цикл
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.Найти("КонтактнаяИнформация") = Неопределено Тогда
			ВызватьИсключение  МетаданныеОбъекта.Имя + " " + НСтр("ka='не содержит контактную информацию.';ru='не содержит контактную информацию.'");
		КонецЕсли;
...
Процедура КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам) Экспорт
	...
	Для каждого ВидКонтактнойИнформации Из ВидыКонтактнойИнформации Цикл
		...
		Если ИнформацияПоКолонкам.Найти(ИмяКолонки, "ИмяКолонки") = Неопределено Тогда
			...
			СтрокаИнфоПроКолонки.Группа = НСтр("ka='საკონტაქტო ინფორმაცია';ru='Контактная информация'");
...
Функция ПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	Если ИнформацияОВидеКИ.Тип <> Перечисления.ТипыКонтактнойИнформации.Другое Тогда
		...
		Кнопка.Заголовок = НСтр("ka='კომენტარის შეყვანა';ru='Ввести комментарий'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Подсказка = НСтр("ka='კომენტარის შეყვანა';ru='Ввести комментарий'");
	...
	Если ИнформацияОВидеКИ.ХранитьИсториюИзменений Тогда
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ka='ცვლილებების ისტორია...';ru='История изменений...'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ИсторияИзменений;
		Команда.Подсказка = НСтр("ka='აჩვენებს საკონტაქტო ინფორმაციის ცვლილების ისტორიას';ru='Показывает историю изменения контактной информации'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ka='მისამართი იანდექს.რუკებზე';ru='Адрес на Яндекс.Картах'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ЯндексКарты;
		Команда.Подсказка = НСтр("ka='იანდექს-რუკების რუკებზე აჩვენებს მისამართს';ru='Показывает адрес на картах Яндекс.Карты'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ka='მისამართი Google Maps-ზე';ru='Адрес на Google Maps'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.GoogleMaps;
		Команда.Подсказка = НСтр("ka='მისამართს აჩვენებს Google Maps-ის რუკაზე';ru='Показывает адрес на карте Google Maps'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
...
Процедура ПунктКонтекстногоМенюПеремещения(ПредыдущийЭлемент, Форма, Направление, ИмяЭлементаДляРазмещения)
	...
	Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
	Если Направление > 0 Тогда 
		ТекстКоманды = НСтр("ka='ზემოთ გადაადგილება';ru='Переместить вверх'");
		Кнопка.Картинка = БиблиотекаКартинок.ПереместитьВверх;
	Иначе
		ТекстКоманды = НСтр("ka='ქვემოთ გადაადგილება';ru='Переместить вниз'");
	...
	Если АдресЭП <> Неопределено И АдресЭП.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
		Попытка
			...
			Если Результат.Количество() > 1 Тогда
				...
				СтрокаОшибки = НСтр("ka='დასაშვებია ელექტრონული ფოსტის მხოლოდ ერთი მისამართის შეყვანა';ru='Допускается ввод только одного адреса электронной почты'");
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			Элемент.Заголовок = НСтр("ka='შევსება';ru='Заполнить'");
			Команда.Подсказка = НСтр("ka='მისამართის შევსება სხვა ველიდან';ru='Заполнить адрес из другого поля'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница Тогда
			...
			Элемент.Заголовок = НСтр("ka='გადასვლა';ru='Перейти'");
			Команда.Подсказка = НСтр("ka='ბმულის მიხედვით გადასვლა';ru='Перейти по ссылке'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
			...
			Элемент.Заголовок = НСтр("ka='წერილის დაწერა';ru='Написать письмо'");
			Команда.Подсказка = НСтр("ka='წერილის დაწერა';ru='Написать письмо'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			Если Форма.ПараметрыКонтактнойИнформации[ИмяЭлементаДляРазмещения].МожноОтправлятьSMS Тогда
					Элемент.Заголовок = НСтр("ka='დავურეკოთ ან SMS გავუგზავნოთ';ru='Позвонить или отправить SMS'");
					Команда.Подсказка = НСтр("ka='დავურეკოთ ან SMS გავუგზავნოთ';ru='Позвонить или отправить SMS'");
					Команда.Картинка = БиблиотекаКартинок.ПозвонитьИлиОтправитьSMS;
			Иначе
				Элемент.Заголовок = НСтр("ka='Позвонить';ru='Позвонить'");
				Команда.Подсказка = НСтр("ka='ტელეფონზე დარეკვა';ru='Позвонить по телефону'");
				Команда.Картинка = БиблиотекаКартинок.Позвонить;
			КонецЕсли;
			
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Skype Тогда
			Элемент.Заголовок = НСтр("ka='Skype';ru='Skype'");
			Команда.Подсказка = НСтр("ka='Skype';ru='Skype'");
