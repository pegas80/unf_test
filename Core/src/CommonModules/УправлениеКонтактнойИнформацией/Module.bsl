...
Процедура ПриСозданииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация", ПоложениеЗаголовкаКИ = "",
	...
	Для Каждого ОбъектКонтактнойИнформации Из КонтактнаяИнформация Цикл
		...
		Иначе
			...
			Исключение
				...
				НСтр("ka='საკონტაქტო ინფორმაციის არაკორექტული ფორმატი.';ru='Некорректный формат контактной информации.';en='Incorrect contact information format.'"), ,
	...
	Если Не ОтложеннаяИнициализация 
		...
		Описание = НСтр("ka='საკონტაქტო ინფორმაციის დამატებითი ველის დამატება';ru='Добавить дополнительное поле контактной информации';en='Add additional contact information field'");
		...
		Кнопка.Заголовок = НСтр("ka='დამატება';ru='Добавить';en='Add'");
	...
	Иначе
		...
		Если ОтложеннаяИнициализация
			...
			И Форма.Элементы.Найти("КонтактнаяИнформацияЗаглушка") = Неопределено Тогда
				...
				ПредупреждениеОРазмещении = НСтр("ka='საკონტაქტო ინფორმაციის ასასახად აუცილებელია ჯგუფ ""%1""-ის განთავსება არა პირველ ელემენტად (ნებისმიერი სხვა ჯგუფის შემდეგ) ჯგუფ ""%2""-ში (მენიუ კიდევ - ფორმის შეცვლა).';ru='Для отображения контактной информации необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).';en='To display the contact information necessary to place group ""%1"" is not the first element (after any other group) in the group ""%2"" (Another menu - Change shape).'");
...
"   • в открывшейся форме ""Настройка формы"" в меню Еще выбрать пункт ""Установить стандартные настройки"".';ka='ასევე, შეიძლება ფორმის სტანდარტული გამართვების დაყენება:"
"   • მენიუ ""კიდევ""-ში ავირჩიოთ პუნქტი ""ფორმის შეცვლა""...;"
"   • გახსნილ ფორმა ""ფორმის გამართვა""-ში მენიუ ""კიდევ""-ში ავირჩიოთ პუნქტი ""სტანდარტული გამართვების დაყენება"".'");
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если СтрокаТаблицы.ЭтоРеквизитТабличнойЧасти Тогда
			...
			Для Каждого СтрокаТабличнойЧастиФормы Из ТабличнаяЧастьФормы Цикл
				...
				Если ОбязательноеЗаполнение И ПустаяСтрока(Представление) Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი ""%1"" არ არის შევსებული.';ru='Поле ""%1"" не заполнено.';en='The ""%1"" is not filled.'"), ВидИнформации.Наименование),,Поле);
		...
		Иначе
			...
			Если ВидИнформации <> ПредыдущийВид И ОбязательноеЗаполнение И ПустаяСтрока(Представление)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი ""%1"" არ არის შევსებული.';ru='Поле ""%1"" не заполнено.';en='The ""%1"" is not filled.'"), ВидИнформации.Наименование),,, ИмяРеквизита);
				ТекущийУровеньОшибок = 2;
...
Функция СтранаАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ka='ქვეყნის დადგენა შეუძლებელია, მოსალოდნელია მისამართი.';ru='Невозможно определить страну, ожидается адрес.';en='It is impossible to identify the country will address.'");
	КонецЕсли;
...
Функция РегионАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Адрес = XDTOАдрес.Состав;
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ka='საქართველოს რაიონის დადგენა შეუძლებელია, მოსალოდნელია მისამართი.';ru='Невозможно определить субъекта РФ, ожидается адрес.';en='Unable to determine the entity of the Russian Federation, will address.'");
	КонецЕсли;
...
Функция ГородАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Адрес = XDTOАдрес.Состав;
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ka='ქალაქის დადგენა შეუძლებელია, მოსალოდნელია მისამართი.';ru='Невозможно определить город, ожидается адрес.';en='Unable to determine the city, address expected.'");
	КонецЕсли;
...
Функция ДоменАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	ВызватьИсключение НСтр("ka='დომენის დადგენა შეუძლებელია, მოსალოდნელია ელექტრონული ფოსტა ან ვებ-ბმული.';ru='Невозможно определить домен, ожидается электронная почта или веб-ссылка.';en='Unable to determine the domain, will e-mail or web link.'");	
КонецФункции
...
Функция НомерТелефонаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	ВызватьИсключение НСтр("ka='ნომრის დადგენა შეუძლებელია, მოსალოდნელია ტელეფონი ან ფაქსი.';ru='Невозможно определить номер, ожидается телефона или факс.';en='Unable to determine the number expected to phone or fax.'");
КонецФункции
...
Процедура СоздатьВТКонтактнаяИнформация(МенеджерВременныхТаблиц, МассивОбъектов, ТипыКИ = Неопределено, ВидыКИ = Неопределено, Дата = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивОбъектов) <> Тип("Массив") ИЛИ МассивОбъектов.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='არასწორი მნიშვნელობა საკონტაქტო ინფორმაციის მფლობელების მასივისთვის.';ru='Неверное значение для массива владельцев контактной информации.';en='Invalid value for array owners contact information.'");
	КонецЕсли;
	...
	Для каждого ОбъектСКонтактнойИнформацией Из ОбъектыСГруппировкойПоТипам Цикл
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.Найти("КонтактнаяИнформация") = Неопределено Тогда
			ВызватьИсключение  МетаданныеОбъекта.Имя + " " + НСтр("ka='не содержит контактную информацию.';ru='не содержит контактную информацию.';en='It does not contain contact information.'");
		КонецЕсли;
...
Процедура КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам) Экспорт
	...
	Для каждого ВидКонтактнойИнформации Из ВидыКонтактнойИнформации Цикл
		...
		Если ИнформацияПоКолонкам.Найти(ИмяКолонки, "ИмяКолонки") = Неопределено Тогда
			...
			СтрокаИнфоПроКолонки.Группа = НСтр("ka='საკონტაქტო ინფორმაცია';ru='Контактная информация';en='Contact Information'");
...
Функция ПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	Если ИнформацияОВидеКИ.Тип <> Перечисления.ТипыКонтактнойИнформации.Другое Тогда
		...
		Кнопка.Заголовок = НСтр("ka='კომენტარის შეყვანა';ru='Ввести комментарий';en='Enter your comment'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Подсказка = НСтр("ka='კომენტარის შეყვანა';ru='Ввести комментарий';en='Enter your comment'");
	...
	Если ИнформацияОВидеКИ.ХранитьИсториюИзменений Тогда
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ka='ცვლილებების ისტორია...';ru='История изменений...';en='Revision history of ...'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ИсторияИзменений;
		Команда.Подсказка = НСтр("ka='აჩვენებს საკონტაქტო ინფორმაციის ცვლილების ისტორიას';ru='Показывает историю изменения контактной информации';en='Shows the history of contact information changes'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ka='მისამართი იანდექს.რუკებზე';ru='Адрес на Яндекс.Картах';en='Address On Yandex.Maps'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ЯндексКарты;
		Команда.Подсказка = НСтр("ka='იანდექს-რუკების რუკებზე აჩვენებს მისამართს';ru='Показывает адрес на картах Яндекс.Карты';en='Shows the address On Yandex.Maps cards'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ka='მისამართი Google Maps-ზე';ru='Адрес на Google Maps';en='Address on Google Maps'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.GoogleMaps;
		Команда.Подсказка = НСтр("ka='მისამართს აჩვენებს Google Maps-ის რუკაზე';ru='Показывает адрес на карте Google Maps';en='Shows the address on Google Maps'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
...
Процедура ПунктКонтекстногоМенюПеремещения(ПредыдущийЭлемент, Форма, Направление, ИмяЭлементаДляРазмещения)
	...
	Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
	Если Направление > 0 Тогда 
		ТекстКоманды = НСтр("ka='ზემოთ გადაადგილება';ru='Переместить вверх';en='move up'");
		Кнопка.Картинка = БиблиотекаКартинок.ПереместитьВверх;
	Иначе
		ТекстКоманды = НСтр("ka='ქვემოთ გადაადგილება';ru='Переместить вниз';en='move down'");
	...
	Если АдресЭП <> Неопределено И АдресЭП.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
		Попытка
			...
			Если Результат.Количество() > 1 Тогда
				...
				СтрокаОшибки = НСтр("ka='დასაშვებია ელექტრონული ფოსტის მხოლოდ ერთი მისამართის შეყვანა';ru='Допускается ввод только одного адреса электронной почты';en='You can enter only one email address'");
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			Элемент.Заголовок = НСтр("ka='შევსება';ru='Заполнить';en='Insert'");
			Команда.Подсказка = НСтр("ka='მისამართის შევსება სხვა ველიდან';ru='Заполнить адрес из другого поля';en='Fill in the address of another field'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница Тогда
			...
			Элемент.Заголовок = НСтр("ka='გადასვლა';ru='Перейти';en='Go'");
			Команда.Подсказка = НСтр("ka='ბმულის მიხედვით გადასვლა';ru='Перейти по ссылке';en='To link'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
			...
			Элемент.Заголовок = НСтр("ka='წერილის დაწერა';ru='Написать письмо';en='Write a letter'");
			Команда.Подсказка = НСтр("ka='წერილის დაწერა';ru='Написать письмо';en='Write a letter'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			Если Форма.ПараметрыКонтактнойИнформации[ИмяЭлементаДляРазмещения].МожноОтправлятьSMS Тогда
					Элемент.Заголовок = НСтр("ka='დავურეკოთ ან SMS გავუგზავნოთ';ru='Позвонить или отправить SMS';en='Call or send SMS'");
					Команда.Подсказка = НСтр("ka='დავურეკოთ ან SMS გავუგზავნოთ';ru='Позвонить или отправить SMS';en='Call or send SMS'");
					Команда.Картинка = БиблиотекаКартинок.ПозвонитьИлиОтправитьSMS;
			Иначе
				Элемент.Заголовок = НСтр("ka='Позвонить';ru='Позвонить';en='call'");
				Команда.Подсказка = НСтр("ka='ტელეფონზე დარეკვა';ru='Позвонить по телефону';en='Make a phone call'");
				Команда.Картинка = БиблиотекаКартинок.Позвонить;
			КонецЕсли;
			
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Skype Тогда
			Элемент.Заголовок = НСтр("ka='Skype';ru='Skype';en='Skype'");
			Команда.Подсказка = НСтр("ka='Skype';ru='Skype';en='Skype'");
...
Функция КонтактнаяИнформацияXMLПоПредставлениюГрузия(Представление) Экспорт
	
	Возврат "<КонтактнаяИнформация xmlns=""http://www.v8.1c.ru/ssl/contactinfo"" xmlns:xs=""http://www.w3.org/2001/XMLSchema"" xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" Представление=""" + Представление +"""><Комментарий/><Состав xsi:type=""Адрес"" Страна=""საქართველო""><Состав xsi:type=""xs:string"">" + Представление +"</Состав></Состав></КонтактнаяИнформация>";
