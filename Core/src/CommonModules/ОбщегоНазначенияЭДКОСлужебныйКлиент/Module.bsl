...
Процедура УстановитьViPNetCSP(ОповещениеОЗавершении, ВладелецФормы) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ka='ViPNet CSP-ის დაყენება შესაძლებელია მხოლოდ Windows-ის ოჯახის საოპერაციო სისტემებზე.';ru='Установка ViPNet CSP возможно только на операционных системах семейства Windows.'");
		ПоказатьПредупреждение(, ОписаниеОшибки,, НСтр("ka='ViPNet CSP-ის დაყენება';ru='Установка ViPNet CSP'"));
	...
	Если Результат.Выполнено Тогда
		Если Результат.СистемнаяИнформация.ВерсияОС = "10.0" Тогда // Windows 10, Windows Server 2016
			ОписаниеОшибки = СтрШаблон(
								НСтр("ka='ViPNet CSP-ის მუშაობას საოპერაციო სისტემა %1-ზე მხარდაჭერა არ გააჩნია.';ru='Работа ViPNet CSP на операционной системе %1 не поддерживается.'"),
		...
		Если Не ЗначениеЗаполнено(ПараметрыДистрибутива) Тогда
			ОписаниеОшибки = НСтр("ka='სერვისი დროებით მიუწვდომელია. მიმართეთ მხარდაჭერის სამსახურს ან მოგვიანებით სცადეთ.';ru='Сервис временно недоступен. Обратитесь в службу поддержки или повторите попытку позже.'");
			ПоказатьПредупреждение(, ОписаниеОшибки);
			РезультатВыполнения = Новый Структура;
			РезультатВыполнения.Вставить("Выполнено", Ложь);
			РезультатВыполнения.Вставить("ОписаниеОшибки", ОписаниеОшибки);
		...
		Параметры = Новый Структура("ПоясняющийТекст", НСтр("ka='მიმდინარეობს პროგრამის ჩატვირთვა...';ru='Выполняется загрузка программы...'"));
...
Процедура УстановитьViPNetCSPПослеПолученияПодтвержденияНаУстановку(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		РезультатВыполнения = Новый Структура;
		РезультатВыполнения.Вставить("Выполнено", Ложь);
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ka='მომხმარებელმა უარყო ViPNet CSP-ის დაყენება';ru='Пользователь отклонил установку ViPNet CSP'"));
...
Процедура УстановитьViPNetCSPПослеСохраненияСкрипта(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		РезультатВыполнения = Новый Структура;
		РезультатВыполнения.Вставить("Выполнено", Ложь);
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ka='შეცდომა ViPNet CSP-ის დაყენებისას';ru='Ошибка при установке ViPNet CSP'"));
...
Процедура УстановитьViPNetCSPПослеЗавершенияУстановки(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		Если ЗначениеЗаполнено(Результат.КодВозврата) Тогда
			РезультатВыполнения = Новый Структура;
			РезультатВыполнения.Вставить("Выполнено", Ложь);
			РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ka='ViPNet CSP-ის დაყენება არ არის შესრულებული';ru='Установка ViPNet CSP не выполнена'"));
