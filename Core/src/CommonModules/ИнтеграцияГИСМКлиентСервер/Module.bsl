...
Функция ПредставлениеПодписи(СертификатКриптографии, Сообщение, ВыводитьГиперссылки, СтруктураЦветаСтиля) Экспорт
	...
	СтрокаПодписано = Новый ФорматированнаяСтрока(НСтр("ka='ხელმოწერილია';ru='Подписано';en='Signed'"));
	...
	Если Сообщение.ПодписьВерна Тогда
		СтрокаПодписьВерна = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ka='Подпись верна (Дата проверки: %1).';ru='Подпись верна (Дата проверки: %1).';en='true Signature (Date of Inspection:% 1).'"), Формат(Сообщение.ДатаПроверкиПодписи, НСтр("ka='ДФ=dd.MM.yyyy';ru='ДФ=dd.MM.yyyy';en='ДФ=dd.MM.yyyy'"))));
	Иначе
		Если ЗначениеЗаполнено(Сообщение.ДатаПроверкиПодписи) Тогда
			СтрокаПодписьВерна = Новый ФорматированнаяСтрока(
				СтрШаблон(НСтр("ka='Подпись не верна (Дата проверки: %1).';ru='Подпись не верна (Дата проверки: %1).';en='Signature is not true (Date of Inspection:% 1).'"), Формат(Сообщение.ДатаПроверкиПодписи, НСтр("ka='ДФ=dd.MM.yyyy';ru='ДФ=dd.MM.yyyy';en='ДФ=dd.MM.yyyy'"))),,
				СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ);
		Иначе
			СтрокаПодписьВерна = Новый ФорматированнаяСтрока(
				НСтр("ka='Проверка подписи не выполнялась.';ru='Проверка подписи не выполнялась.';en='Signature verification is not performed.'"),,
				СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ);
		КонецЕсли;
	КонецЕсли;
	
	СтрокаПроверить = Новый ФорматированнаяСтрока(НСтр("ka='შემოწმება';ru='Проверить';en='Check'"),,СтруктураЦветаСтиля.ЦветГиперссылкиГИСМ,,"ПроверитьПодпись");
...
Процедура ДобавитьТекстОшибкиНеЗаполненGLNОрганизации(СообщениеXML, GLN, Организация) Экспорт
	...
	Если Не ЗначениеЗаполнено(GLN) Тогда
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='ორგანიზაცია %1-ის GLN არ არის შევსებული.';ru='Не заполнен GLN организации %1.';en='Not filled GLN organization 1%.'"), Организация));
	Иначе
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Указан не корректный GLN организации %1.';ru='Указан не корректный GLN организации %1.';en='Unknown is not a valid GLN organization 1%.'"), Организация));
	КонецЕсли;
...
Процедура ДобавитьТекстОшибкиУказанНеКорректныйКППОрганизации(СообщениеXML, КПП, Организация) Экспорт
	...
	ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Указан не корректный КПП организации %1.';ru='Указан не корректный КПП организации %1.';en='Unknown is not a valid checkpoint organization 1%.'"), Организация));
...
Процедура ДобавитьТекстОшибкиНеЗаполненоПолноеНаименованиеКонтрагента(СообщениеXML, Контрагент) Экспорт
	...
	ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Не заполнено полное наименование контрагента %1.';ru='Не заполнено полное наименование контрагента %1.';en='Not filled full name of the counterparty 1%.'"), Контрагент));
...
Процедура ДобавитьТекстОшибкиНеЗаполненGLNПартнера(СообщениеXML, GLN, Партнер) Экспорт
	...
	Если Не ЗначениеЗаполнено(GLN) Тогда
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='პარტნიორ %-ის GLN არ არის შევსებული.';ru='Не заполнен GLN партнера %1.';en='Not filled GLN partner% 1.'"), Партнер));
	Иначе
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Указан не корректный GLN партнера %1.';ru='Указан не корректный GLN партнера %1.';en='Unknown is not a valid GLN partner% 1.'"), Партнер));
	КонецЕсли;
	...
	Исключение
		...
		#Если НЕ ТонкийКлиент Тогда
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ka='Ошибка отправки POST-запроса к серверу ГИСМ 195.149.110.235';ru='Ошибка отправки POST-запроса к серверу ГИСМ 195.149.110.235';en='Error while sending the POST request to the server-GISM 195,149,110,235'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		#КонецЕсли
	КонецПопытки;
