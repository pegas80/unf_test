...
Функция РеквизитыПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	РеквизитыПредпринимателя.НаименованиеПолное = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ინდივიდუალური მეწარმე %1';ru='Индивидуальный предприниматель %1'"),
...
Функция РеквизитыЮридическихЛицПоНаименованию(Знач Наименование, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки     = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სახელწოდება - %1:';ru='Наименование -  %1:'"), Наименование)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
			...
			Если СтрокаИзменения.СвСвид <> Неопределено Тогда
				Для каждого СтрокаСвидетельства Из СтрокаИзменения.СвСвид Цикл
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='გაცემულია მოწმობა %1 %2';ru='Выдано свидетельство %1 %2'"), 
...
Процедура ЗаполнитьРеквизитыПенсионногоФонда(ОбъектXDTO, Реквизиты)
	...
	Если ОбъектXDTO.СвРегПФ <> Неопределено Тогда
		...
		РегистрационныйНомерПФР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-%2-%3';ru='%1-%2-%3'"), 
		...
		Если ОбъектXDTO.СвРегПФ.СвОргПФ <> Неопределено Тогда
			...
			КодОрганаПФР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-%2';ru='%1-%2'"), 
		...
		Для каждого ПроверкиЗаМесяц Из ОбъектXDTO.InspectionsUnregular294 Цикл
			Если ПроверкиЗаМесяц.InspectionUnregular294 <> Неопределено Тогда
				...
				Для каждого СтрокаПроверки Из ПроверкиЗаМесяц.InspectionUnregular294 Цикл
					...
					НоваяСтрока.Длительность = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='с %1 по %2';ru='с %1 по %2'"), 
					...
					Если Завершена Тогда
						...
						Если КоличествоНарушений = 0 Тогда
							НоваяСтрока.Результат = НСтр("ka='დარღვევები არ არის გამოვლენილი';ru='Нарушений не выявлено'");
						Иначе
							НоваяСтрока.Нарушения.Сортировать("НомерНарушения");
							НоваяСтрока.Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გამოვლენილია %1';ru='Выявлено %1'"),
								СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
									КоличествоНарушений, НСтр("ka='დარღვევა,დარღვევები,დარღვევები';ru='нарушение,нарушения,нарушений'")));
						КонецЕсли;
				...
				Для каждого СтрокаПроверки Из ПроверкиЗаМесяц.InspectionRegular294 Цикл
					...
					Если ЗначениеЗаполнено(СтрокаПроверки.DURATION_SEC_II) Тогда
						Длительность = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='%1 სამუშაო დღე';ru='%1 рабочих дней'"), СтрокаПроверки.DURATION_SEC_II);
					ИначеЕсли ЗначениеЗаполнено(СтрокаПроверки.DURATION_SEC_I) Тогда
						Длительность = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='%1 სამუშაო საათი';ru='%1 рабочих часов'"), СтрокаПроверки.DURATION_SEC_I);
					Иначе
						Длительность = "";
					КонецЕсли;
					НоваяСтрока.ДатаПроверки = ДатаНачала;
					Если ЗначениеЗаполнено(Длительность) Тогда
						НоваяСтрока.Длительность = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='%1-დან, %2';ru='с %1, %2'"), Формат(ДатаНачала, "ДЛФ=D"), Длительность);
					Иначе
						НоваяСтрока.Длительность = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='1%-დან';ru='с %1'"), Формат(ДатаНачала, "ДЛФ=D"));
					КонецЕсли;
					НоваяСтрока.КонтролирующийОрган = СокрЛП(СтрокаПроверки.KO_NAME);
...
Процедура ОбработатьОшибкуСервиса(ОбъектXDTO, ОписаниеОшибки, ИмяМетода, СтруктураРеквизитов)
	...
	Если ОписаниеОшибкиВРег = ВРег("НеУказаныПараметрыАутентификации") Тогда
		...
		ОписаниеОшибки = НСтр("ka='Не указаны логин и пароль для доступа к интернет-поддержке';ru='Не указаны логин и пароль для доступа к интернет-поддержке'");
	...
	ИначеЕсли ОписаниеОшибкиВРег = ВРег("НеУказанПароль") Тогда
		...
		ТекстСобытия   = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
		ОписаниеОшибки = НСтр("ka='ინტერნეტ-მხარდაჭერასთან დასაშვებად პაროლი არ არის მითითებული';ru='Не указан пароль для доступа к интернет-поддержке'");
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-11:") > 0 
		...
		ТекстСобытия   = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
		ОписаниеОшибки = НСтр("ka='სერვისი 1C:კონტრაგენტი არ არის შეერთებული';ru='Не подключен сервис 1С:Контрагент'");
		
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, """STATUS"":401") > 0 
		ИЛИ СтрНайти(ОписаниеОшибкиВРег, "SERVER-9:") > 0 Тогда
		
		ТекстОшибки  = НСтр("ka='Неверно указаны логин и пароль для доступа к интернет-поддержке';ru='Неверно указаны логин и пароль для доступа к интернет-поддержке'");
		ТекстСобытия = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-1:") > 0 Тогда
		...
		ИначеЕсли ИмяМетода = "getCorporationRequisitesByNameAndAddress" Тогда
			ТекстОшибки = НСтр("ka='იურიდიული პირის სახელწოდება არ არის მითითებული';ru='Не указано наименование юридического лица'");
		ИначеЕсли ИмяМетода = "getInspectionsByInn" Тогда
			ТекстОшибки = НСтр("ka='შესამოწმებელი პირის გსნ არ არის მითითებული';ru='Не указан ИНН проверяемого лица'");	
		КонецЕсли;
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-2:") > 0 Тогда
		...
		ИначеЕсли ИмяМетода = "getInspectionsByInn" Тогда
			ТекстОшибки = НСтр("ka='შესამოწმებელი პირის გსნ 10 ან 12 ციფრისგან უნდა შედგებოდეს';ru='ИНН проверяемого лица должен состоять из 10 или 12 цифр'");
		КонецЕсли;
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-3:") > 0 Тогда
		...
		Если ИмяМетода = "getCorporationRequisitesByINN" 
			...
			ИЛИ ИмяМетода = "checkCorporationTrustability" Тогда
			ТекстОшибки = НСтр("ka='იურიდიული პირის გსნ მხოლოდ ციფრებს უნდა შეიცავდეს';ru='ИНН юридического лица должен содержать только цифры'");
		ИначеЕсли ИмяМетода = "getEntrepreneurRequisitesByINN" 
			ИЛИ ИмяМетода = "checkPersonTrustabilityByInn" Тогда
			ТекстОшибки = НСтр("ka='მეწარმის გსნ მხოლოდ ციფრებს უნდა შეიცავდეს';ru='ИНН предпринимателя должен содержать только цифры'");
		ИначеЕсли ИмяМетода = "getInspectionsByInn" Тогда
			ТекстОшибки = НСтр("ka='შესამოწმებელი პირის გსნ მხოლოდ ციფრებისგან უნდა შედგებოდეს';ru='ИНН проверяемого лица должен содержать только цифры'");
		КонецЕсли;
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-7:") > 0 Тогда
		
		ТекстОшибки  = НСтр("ka='Превышен лимит количества вызовов сервиса за один день';ru='Превышен лимит количества вызовов сервиса за один день'");
		ТекстСобытия = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-8:") > 0 Тогда
		
		ТекстОшибки  = НСтр("ka='Отсутствует действующий договор ИТС';ru='Отсутствует действующий договор ИТС'");
		ТекстСобытия = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ОписаниеОшибки)
		...
		Если ИмяМетода = "getCorporationRequisitesByINN" 
			...
			ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გსნ %1-ის მიხედვით რეკვიზიტების შესავსებად მონაცემები ვერ მოიძებნა';ru='Не удалось найти данные для заполнения реквизитов по ИНН %1'"),
				СтруктураРеквизитов.ИНН);
		ИначеЕсли ИмяМетода = "getCorporationRequisitesByNameAndAddress" Тогда
			ТекстОшибки  = НСтр("ka='სახელწოდების მიხედვით ძებნის შესრულება ვერ მოხერხდა';ru='Не удалось выполнить поиск по наименованию'");
		ИначеЕсли ИмяМетода = "getInspectionsByInn" Тогда
			ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გსნ %1-ის მიხედვით შემოწმებების შესახებ მონაცემები ვერ მოიძებნა';ru='Не удалось найти данные о проверках по ИНН %1'"),
				СтруктураРеквизитов.ИНН);
		ИначеЕсли ИмяМетода = "checkCorporationTrustability" Тогда
			ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გსნ %1-ით იურიდიული პირის შესახებ მონაცემთა მოძებნა ვერ მოხერხდა';ru='Не удалось найти данные о юридическом лице с ИНН %1'"),
				СтруктураРеквизитов.ИНН);
		ИначеЕсли ИмяМетода = "checkPersonTrustabilityByInn" Тогда
			ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გსნ %1-ის მქონე მეწარმის შესახებ მონაცემები ვერ მოიძებნა';ru='Не удалось найти данные о предпринимателе с ИНН %1'"),
				СтруктураРеквизитов.ИНН);
		КонецЕсли;
		ТекстСобытия   = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
