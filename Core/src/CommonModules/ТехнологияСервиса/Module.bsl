...
"""1С:Библиотека стандартных подсистем"" версии не младше %1!';ka='В конфигурацию не внедрена библиотека ""1С:Библиотека стандартных подсистем""."
"Без внедрения этой библиотеки конфигурация не может использоваться в модели сервиса."
""
"Для использования этой конфигурации в модели сервиса требуется внедрить библиотеку"
"""1С:Библиотека стандартных подсистем"" версии не младше %1!'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, ТребуемаяВерсияБСП()) < 0 Тогда
			...
			ВызватьИсключение СтрШаблон(
				НСтр("ka='ბიბლიოთეკის მიმდინარე ვერსია ""1С:სერვისის ტექნოლოგიების ბიბლიოთეკა""-თი სერვისის მოდელში კონფიგურაციის გამოსაყენებლად, საჭიროა ბიბლიოთეკის გამოყენებადი ვერსია ""1С:სტანდარტული ქვესისტემების ბიბლიოთეკა""-ს განახლება!"
""
"გამოყენებადი ვერსია: %1, საჭიროა ვერსია არანაკლებ %2!';ru='Для использования конфигурации в модели сервиса с текущей версией библиотеки"
...
Функция ПолучитьОписаниеОшибкиWebСервиса(ИнформацияОбОшибке) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='web-სერვისის ოპერაციის შესრულება';ru='Выполнение операции web-сервиса';en='Operation web-service'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , ,
