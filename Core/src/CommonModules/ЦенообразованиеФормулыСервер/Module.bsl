...
Процедура СформироватьНовыйИдентификаторВидаЦен(Идентификатор, ВидЦенНаименование, ВидЦенВладелец = "") Экспорт
	...
	Если ПустаяСтрока(Идентификатор) Тогда
		...
		Идентификатор = НСтр("ka='ИдентификаторВидаЦен';ru='ИдентификаторВидаЦен';en='IdentifikatorVidaTsen'");
...
Процедура ПроверитьФормулу(Ошибки, Формула) Экспорт
	...
	Если СтрЧислоВхождений(ТекстФормулы, ЦенообразованиеФормулыСервер.СтрокаНачалаОперанда()) <> СтрЧислоВхождений(ТекстФормулы, ЦенообразованиеФормулыСервер.СтрокаКонцаОперанда()) Тогда
		...
		ТекстОшибки = НСтр("ka='გახსნილი ოპერანდების რაოდენობა დახურული ოპერანდების რაოდენობის ტოლი არ არის.';ru='Количество открытых операндов не равно количеству закрытых.';en='The number of open operands are not equal to the number of closed.'");
	...
	Если СтрЧислоВхождений(ТекстФормулы, "(") <> СтрЧислоВхождений(ТекстФормулы, ")") Тогда
		...
		ТекстОшибки = НСтр("ka='ღია ფრჩხილების რაოდენობა დახურული ფრჩხილების რაოდენობის ტოლი არ არის.';ru='Количество открытых скобок не равно количеству закрытых.';en='The number of open brackets is not equal to the number of closed.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Формула", ТекстОшибки, "");
	...
	Для каждого Строка Из ТаблицаОперандов Цикл
		...
		Если НЕ ЗначениеЗаполнено(Строка.ВидЦен) Тогда
			...
			Если НЕ ЭтоЗарезервированныйОперанд(Строка.Операнд) Тогда
				...
				ТекстОшибки = НСтр("ka='ოპერანდი %1 არ არის ამოცნობილი. შეამოწმეთ ფორმულის დაწერის სისწორე.';ru='Не распознан операнд %1."
...
"Подробное описание: ';ka='При расчете возникли ошибки. Проверьте правильность написания формулы."
