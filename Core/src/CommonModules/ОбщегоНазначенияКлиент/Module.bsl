...
Функция ПроверитьТипПараметраКоманды(Знач Параметр, Знач ОжидаемыйТип) Экспорт
	...
	Если НЕ Результат Тогда
		ПоказатьПредупреждение(,НСтр("ka='მოქმედება არ შეიძლება შესრულდეს არჩეული ელემენტისთვის.';ru='Действие не может быть выполнено для выбранного элемента.'"));
	КонецЕсли;
...
//    ТекстСообщения = НСтр("ka='დოკუმენტის ამოსაბეჭდად აუცილებელია ფაილებთან მუშაობის გაფართოების დაყენება.';ru='Для печати документа необходимо установить расширение работы с файлами.'");
...
#Если Не ВебКлиент Тогда
	...
	Если КодВозврата = Неопределено Или КодВозврата > 0 Тогда
		...
		ТекстСообщения = НСтр("ka='შეცდომა კომპონენტ comcntr-ის რეგისტრაციისას.';ru='Ошибка при регистрации компоненты comcntr.'") + Символы.ПС
			+ НСтр("ka='შეცდომის კოდი regsvr32:';ru='Код ошибки regsvr32:'") + " " + КодВозврата;
			
		Если КодВозврата = 5 Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("ka='დაშვების უფლებები არასაკმარისია.';ru='Недостаточно прав доступа.'");
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ka='კომპონენტ comcntr-ის რეგისტრაცია';ru='Регистрация компоненты comcntr'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), "Ошибка", ТекстСообщения);
		...
		ПоказатьПредупреждение(,ТекстСообщения + Символы.ПС + НСтр("ka='დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Подробности см. в Журнале регистрации.'"));
...
Процедура ПоказатьПодтверждениеЗакрытияФормы(Знач ОповещениеСохранитьИЗакрыть, Отказ, 
	...
	Если ЗавершениеРаботы Тогда
		Если ТекстПредупрежденияПриЗавершении = "" Тогда // передан параметр из ПередЗакрытием
			ТекстПредупрежденияПриЗавершении = НСтр("ka='Данные были изменены. Все изменения будут потеряны.';ru='Данные были изменены. Все изменения будут потеряны.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьФормуРедактированияКомментария(Знач МногострочныйТекст, Знач ФормаВладелец, Знач ИмяРеквизита, 
	...
	ЗаголовокФормы = ?(Заголовок <> Неопределено, Заголовок, НСтр("ka='კომენტარი';ru='Комментарий'"));
...
Процедура ПроверитьРасширениеРаботыСФайламиПодключеноЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		ТекстСообщения = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ვებ-კლიენტ 1C:საწარმოსთვის გაფართოება არ არის დაყენებული.';ru='Действие недоступно, так как не установлено расширение для веб-клиента 1С:Предприятие.'")
	КонецЕсли;
...
Процедура ПодтвердитьЗакрытиеФормы() Экспорт
	...
	Если ПустаяСтрока(Параметры.ТекстПредупреждения) Тогда
		ТекстВопроса = НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?'");
	Иначе
