...
Функция ШаблонТемы() Экспорт
	Возврат НСтр("ka=""[НаименованиеРассылки]  [ДатаВыполнения(ДЛФ='D')]-ის"";ru=""[НаименованиеРассылки] от [ДатаВыполнения(ДЛФ='D')]""");
КонецФункции
...
Функция ШаблонИмениАрхива() Экспорт
	...
	Возврат НСтр("ka=""[НаименованиеРассылки]_[ДатаВыполнения(ДФ='yyyy-MM-dd')]"";ru=""[НаименованиеРассылки]_[ДатаВыполнения(ДФ='yyyy-MM-dd')]""");
КонецФункции
...
Функция СписокВариантовЗаполненияРасписаний() Экспорт
	...
	СписокВариантов.Добавить(1, НСтр("ka='ყოველდღე';ru='Каждый день'"));
	СписокВариантов.Добавить(2, НСтр("ka='ყოველი მეორე დღე';ru='Каждый второй день'"));
	СписокВариантов.Добавить(3, НСтр("ka='ყოველი მეოთხე დღე';ru='Каждый четвертый день'"));
	СписокВариантов.Добавить(4, НСтр("ka='სამუშაო დღეებში';ru='По будням'"));
	СписокВариантов.Добавить(5, НСтр("ka='დასვენების დღეებში';ru='По выходным'"));
	СписокВариантов.Добавить(6, НСтр("ka='ორშაბათობით';ru='По понедельникам'"));
	СписокВариантов.Добавить(7, НСтр("ka='პარასკევობით';ru='По пятницам'"));
	СписокВариантов.Добавить(8, НСтр("ka='კვირაობით';ru='По воскресеньям'"));
	СписокВариантов.Добавить(9, НСтр("ka='თვის პირველ დღეს';ru='В первый день месяца'"));
	СписокВариантов.Добавить(10, НСтр("ka='თვის ბოლო დღეს';ru='В последний день месяца'"));
	СписокВариантов.Добавить(11, НСтр("ka='ყოველი კვარტალი ათ რიცხვში';ru='Каждый квартал десятого числа'"));
	СписокВариантов.Добавить(12, НСтр("ka='სხვა...';ru='Другое...'"));
...
Функция СтрокаСообщенийПользователю(МассивОшибок = Неопределено, ДобавитьДежурнуюФразу = Истина) Экспорт
	...
	Если ВсеОшибки <> "" И ДобавитьДежурнуюФразу Тогда
		ВсеОшибки = ВсеОшибки + Отступ + "---" + Отступ + НСтр("ka='დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Подробности см. в журнале регистрации.'");
	КонецЕсли;
...
Функция ПредставлениеСпособовДоставки(ПараметрыДоставки) Экспорт
	Префикс = НСтр("ka='შედეგი';ru='Результат'");
	ТекстПредставления = "";
	Суффикс = "";
	...
	Если Не ПараметрыДоставки.ТолькоУведомить Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ka='и';ru='и'")) 
		+ " "
		+ НСтр("ka='გაგზავნილია ფოსტით (იხ. დანართები)';ru='отправлен по почте (см. вложения)'");
	...
	Если ПараметрыДоставки.ВыполненаВПапку Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ka='и';ru='и'")) 
		+ " "
		+ НСтр("ka='მიწოდებულია საქაღალდეში';ru='доставлен в папку'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаВСетевойКаталог Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ka='и';ru='и'")) 
		+ " "
		+ НСтр("ka='მიწოდებულია ქსელურ კატალოგში';ru='доставлен в сетевой каталог'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаНаFTP Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ka='и';ru='и'")) 
		+ " "
		+ НСтр("ka='მიწოდებულია FTP რესურსზე';ru='доставлен на FTP ресурс'")
		+ " ";
