...
Процедура ПриВыполненииДоставкиВПапку(ПараметрыДоставки, Вложения) Экспорт
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ანგარიშგებების დაგზავნა %1 %2-გან';ru='Рассылка отчетов %1 от %2';en='reports subscribe% 1% 2 from'"),
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПутьКФайлу) И ЗначениеЗаполнено(Выборка.Том) Тогда
			...
			Если СуществующийФайл.Количество() = 0 Тогда
				...
				НесуществующийФайл.СтатусПроверки = НСтр("ka='დისკზე არსებულ ტომში მონაცემები არ არის';ru='Отсутствуют данные в томе на диске';en='There are no data on the disk volume'");
			...
			Иначе
				...
				СуществующийФайл[0].СтатусПроверки = НСтр("ka='ერთიანი მონაცემები';ru='Целостные данные';en='holistic data'");
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка) Экспорт
	...
	Попытка
		...
		ТекстКомментария = НСтр("ka='ფაილი წაიშალა უსარგებლო ფაილების გასუფთავებისას.';ru='Файл удален при очистке ненужных файлов.';en='File removed by cleaning junk files.'")
	...
	Исключение
		...
		Если Не ПустаяСтрока(ИмяФайлаСПутем) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='უსარგებლო ფაილების გასუფთავება';ru='Очистка ненужных файлов';en='Cleaning of unnecessary files'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов(ВозможныеПрава) Экспорт
	...
	Право.Имя           = "Чтение";
	Право.Заголовок     = НСтр("ka='წაკითხვა';ru='Чтение';en='Reading'");
	Право.Подсказка     = НСтр("ka='საქაღალდეების და ფაილების წაკითხვა';ru='Чтение папок и файлов';en='Reading files and folders'");
	...
	Право.Заголовок     = НСтр("ka='ფოლდერის შეცვლა';ru='Изменение"
"папок'");
	Право.Подсказка     = НСтр("ka='ფაილების საქაღალდეების დამატება, ცვლილება და წაშლის ნიშნული';ru='Добавление, изменение и"
"пометка удаления папок файлов'");
	...
	Право.Заголовок     = НСтр("ka='ფაილების შეცვლა';ru='Изменение"
"файлов'");
	Право.Подсказка     = НСтр("ka='Изменение файлов в папке';ru='Изменение файлов в папке';en='Changing the files in a folder'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
	// Права для стандартных шаблонов ограничений доступа.
	...
	Право.Заголовок     = НСтр("ka='ფაილების დამატება';ru='Добавление"
"файлов'");
	Право.Подсказка     = НСтр("ka='Добавление файлов в папку';ru='Добавление файлов в папку';en='Adding files to a folder'");
	// Права, требуемые для этого права.
...
"удаления';ka='Пометка"
"удаления'");
	Право.Подсказка     = НСтр("ka='Пометка удаления файлов в папке';ru='Пометка удаления файлов в папке';en='Marking delete files in the folder'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("ИзменениеФайлов");
...
"правами';ka='Управление"
"правами'");
	Право.Подсказка     = НСтр("ka='Управление правами папки';ru='Управление правами папки';en='Rights Management folder'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='რედაქტირებადი ფაილები';ru='Редактируемые файлы';en='editable files'");
...
Функция Кодировки() Экспорт
	...
	СписокКодировок.Добавить("ibm852",       НСтр("ka='IBM852 (ცენტრალური ევროპის DOS)';ru='IBM852 (Центральноевропейская DOS)';en='IBM852 (Central European DOS)'"));
	СписокКодировок.Добавить("ibm866",       НСтр("ka='IBM866 (რუსული ანბანი DOS)';ru='IBM866 (Кириллица DOS)';en='IBM866 (DOS Cyrillic)'"));
	СписокКодировок.Добавить("iso-8859-1",   НСтр("ka='ISO-8859-1 (დასავლეთ ევროპული ISO)';ru='ISO-8859-1 (Западноевропейская ISO)';en='ISO-8859-1 (Western European ISO)'"));
	СписокКодировок.Добавить("iso-8859-2",   НСтр("ka='ISO-8859-2 (ცენტრალური ევროპის ISO)';ru='ISO-8859-2 (Центральноевропейская ISO)';en='ISO-8859-2 (Central European ISO)'"));
	СписокКодировок.Добавить("iso-8859-3",   НСтр("ka='ISO-8859-3 (ლათინური ანბანი 3 ISO)';ru='ISO-8859-3 (Латиница 3 ISO)';en='ISO-8859-3 (Latin 3 ISO)'"));
	СписокКодировок.Добавить("iso-8859-4",   НСтр("ka='ISO-8859-4 (ბალტიური ISO)';ru='ISO-8859-4 (Балтийская ISO)';en='ISO-8859-4 (Baltic ISO)'"));
	СписокКодировок.Добавить("iso-8859-5",   НСтр("ka='ISO-8859-5 (რუსული ანბანი ISO)';ru='ISO-8859-5 (Кириллица ISO)';en='ISO-8859-5 (ISO Cyrillic)'"));
	СписокКодировок.Добавить("iso-8859-7",   НСтр("ka='ISO-8859-7 (ბერძნული ISO)';ru='ISO-8859-7 (Греческая ISO)';en='ISO-8859-7 (Greek ISO)'"));
	СписокКодировок.Добавить("iso-8859-9",   НСтр("ka='ISO-8859-9 (თურქული ISO)';ru='ISO-8859-9 (Турецкая ISO)';en='ISO-8859-9 (Turkish ISO)'"));
	СписокКодировок.Добавить("iso-8859-15",  НСтр("ka='ISO-8859-15 (ლათინური ანბანი 9 ISO)';ru='ISO-8859-15 (Латиница 9 ISO)';en='ISO-8859-15 (Latin 9 ISO)'"));
	СписокКодировок.Добавить("koi8-r",       НСтр("ka='KOI8-R (რუსული ანბანი KOI8-R)';ru='KOI8-R (Кириллица KOI8-R)';en='KOI8-R (Cyrillic KOI8-R)'"));
	СписокКодировок.Добавить("koi8-u",       НСтр("ka='KOI8-U (რუსული ანბანი KOI8-U)';ru='KOI8-U (Кириллица KOI8-U)';en='KOI8-U (KOI8-U Cyrillic)'"));
	СписокКодировок.Добавить("us-ascii",     НСтр("ka='US-ASCII (США)';ru='US-ASCII (США)';en='US-ASCII (USA)'"));
	СписокКодировок.Добавить("utf-8",        НСтр("ka='UTF-8 (უნიკოდი UTF-8)';ru='UTF-8 (Юникод UTF-8)';en='UTF-8 (UTF-8, Unicode)'"));
	СписокКодировок.Добавить("windows-1250", НСтр("ka='Windows-1250 (ცენტრალური ევროპის Windows)';ru='Windows-1250 (Центральноевропейская Windows)';en='Windows-1250 (Central European Windows)'"));
	СписокКодировок.Добавить("windows-1251", НСтр("ka='Windows-1251 (რუსული ანბანი Windows)';ru='windows-1251 (Кириллица Windows)';en='windows-1251 (Windows Cyrillic)'"));
	СписокКодировок.Добавить("windows-1252", НСтр("ka='Windows-1252 (დასავლეთ ევროპული Windows)';ru='Windows-1252 (Западноевропейская Windows)';en='Windows-1252 (Western European Windows)'"));
	СписокКодировок.Добавить("windows-1253", НСтр("ka='Windows-1253 (ბერძნული Windows)';ru='Windows-1253 (Греческая Windows)';en='Windows-1253 (Greek Windows)'"));
	СписокКодировок.Добавить("windows-1254", НСтр("ka='Windows-1254 (თურქული Windows)';ru='Windows-1254 (Турецкая Windows)';en='Windows-1254 (Turkish Windows)'"));
	СписокКодировок.Добавить("windows-1257", НСтр("ka='Windows-1257 (ბალტიური Windows)';ru='Windows-1257 (Балтийская Windows)';en='Windows-1257 (Baltic Windows)'"));
...
Функция СобытиеЖурналаРегистрацииДляОбмена() 
	...
	Возврат НСтр("ka='ფაილები. მონაცემთა გაცვლის დროს ფაილის გაგზავნა ვერ მოხერხდა';ru='Файлы.Не удалось отправить файл при обмене данными';en='Fayly.Ne able to send data exchange file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
