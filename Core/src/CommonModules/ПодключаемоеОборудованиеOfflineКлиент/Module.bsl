...
Процедура АсинхронныйВыгрузитьТоварыВОборудованиеOffline(ТипОборудования, МассивУстройств, ТекстСообщения = "", ПоказыватьПредупреждение = Истина, ОповещениеОВыполнении, ТолькоИзмененные = Истина) Экспорт
	...
	Состояние(НСтр("ka='მიმდინარეობს მოწყობილობაში საქონლის გადმოტვირთვა Offline...';ru='Выполняется выгрузка товаров в оборудование Offline...'"));
	...
	Для каждого ИдентификаторУстройства Из МассивУстройств Цикл
		...
		Если СтруктураДанные.Данные.Количество() = 0 Тогда
			...
			Если СтруктураДанные.КоличествоНеВыгруженныхСтрокСОшибками = 0 Тогда
				ОписаниеОшибок = СформироватьОписаниеОшибкиДляУстройства(ИдентификаторУстройства, Неопределено, ОписаниеОшибок, НСтр("ka='გადმოსატვირთად მონაცემები არ არის!';ru='Нет данных для выгрузки!'"));
			Иначе
				ОписаниеОшибок = СформироватьОписаниеОшибкиДляУстройства(ИдентификаторУстройства, Неопределено, ОписаниеОшибок, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მონაცემები არ გადმოიტვირთა. აღმოჩენილია შეცდომა: %1';ru='Данные не выгружены. Обнаружено ошибок: %1'"), СтруктураДанные.КоличествоНеВыгруженныхСтрокСОшибками));
			КонецЕсли;
			Продолжить;
		КонецЕсли;
...
Процедура АсинхронныйОчиститьТоварыВОборудованиеOffline(ТипОборудования, МассивУстройств, ТекстСообщения = "", ПоказыватьПредупреждение = Истина, ОповещениеОВыполнении) Экспорт
	...
	Состояние(НСтр("ka='მოწყობილობაში მიმდინარეობს საქონლის გასუფთავება Offline...';ru='Выполняется очистка товаров в оборудовании Offline...'"));
...
Процедура АсинхронныйЗагрузитьОтчетОРозничныхПродажах(МассивУстройств, ТекстСообщения = "", ПоказыватьПредупреждение = Истина, ОповещениеОВыполнении) Экспорт
	...
	Состояние(НСтр("ka='ხორციელდება საცალო გაყიდვების შესახებ ანგარიშგებების ჩატვირთვა სსა-დან Offline...';ru='Выполняется загрузка отчетов о розничных продажах из ККМ Offline...'"));
...
Функция СформироватьОписаниеОшибкиДляУстройства(ИдентификаторУстройства, ВыходныеПараметры, ОписаниеОшибок, ОписаниеОшибки)
	...
	Возврат ОписаниеОшибок
	      + Символы.ПС
	      + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომების აღწერილობა მოწყობილობა %1-ისთვის: %2';ru='Описание ошибок для устройства %1: %2'"), ИдентификаторУстройства, ОписаниеОшибки)
...
Процедура АсинхронныйВыгрузитьТоварыВОборудованиеOfflineФрагмент(Параметры)
	...
	Если Параметры.ТребуетсяВыполнить > 0 Тогда
		...
		Если Параметры.Выполнено = Параметры.ТребуетсяВыполнить Тогда
			ТекстСообщения = НСтр("ka='საქონელი წარმატებით გადმოიტვირთა!';ru='Товары успешно выгружены!'");
		ИначеЕсли Параметры.Выполнено > 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საქონელი წარმატებით გადმოიტვირთა %2-დან %1 მოწყობილობისთვის.';ru='Товары успешно выгружены для %1 устройств из %2.'"), Параметры.Выполнено, Параметры.ТребуетсяВыполнить) + Параметры.ОписаниеОшибок;
		Иначе
			ТекстСообщения = НСтр("ka='საქონლის გადმოტვირთვა ვერ მოხერხდა:';ru='Выгрузить товары не удалось:'") + Параметры.ОписаниеОшибок;
		КонецЕсли;
	...
	Иначе
		Если Параметры.ПоказыватьПредупреждение Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ka='აუცილებელია მოწყობილობის არჩევა, რომლისთვის საჭიროა საქონლის გადმოტვირთვა.';ru='Необходимо выбрать устройство, для которого требуется выгрузить товары.'"), 10);
		КонецЕсли;
	КонецЕсли;
...
Процедура АсинхронныйОчиститьТоварыВОборудованиеOfflineФрагмент(Параметры)
	...
	Если Параметры.ТребуетсяВыполнить > 0 Тогда
		...
		Если Параметры.Выполнено = Параметры.ТребуетсяВыполнить Тогда
			ТекстСообщения = НСтр("ka='საქონელი წარმატებით გასუფთავდა!';ru='Товары успешно очищены!'");
		ИначеЕсли Параметры.Выполнено > 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საქონელი წარმატებით გასუფთავდა %2-დან %1 მოწყობილობისთვის. %3';ru='Товары успешно очищены для %1 устройств из %2. %3'"), Параметры.Выполнено, Параметры.ТребуетсяВыполнить, Параметры.ОписаниеОшибок);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საქონლის გასუფთავება ვერ მოხერხდა: %1';ru='Очистить товары не удалось: %1'"), Параметры.ОписаниеОшибок);
		КонецЕсли;
	...
	Иначе
		Если Параметры.ПоказыватьПредупреждение Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ka='აუცილებელია მოწყობილობის არჩევა, რომლისთვის საჭიროა საქონლების გასუფთავება.';ru='Необходимо выбрать устройство, для которого требуется очистить товары.'"), 10);
		КонецЕсли;
	КонецЕсли;
...
Процедура АсинхронныйЗагрузитьОтчетОРозничныхПродажахФрагмент(Параметры)
	...
	Если Параметры.ТребуетсяВыполнить > 0 Тогда
		...
		Если Параметры.Выполнено = Параметры.ТребуетсяВыполнить Тогда
			ТекстСообщения = НСтр("ka='საცალო გაყიდვების შესახებ ანგარიშგებები წარმატებით ჩაიტვირთა!';ru='Отчеты о розничных продажах успешно загружены!'");
		ИначеЕсли Параметры.Выполнено > 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საცალო გაყიდვების შესახებ ანგარიშგებები წარმატებით არის ჩატვირთული %2-დან %1 მოწყობილობისთვის.';ru='Успешно загружены отчеты о розничных продажах для %1 устройств из %2.'"), Параметры.Выполнено, Параметры.ТребуетсяВыполнить) + Параметры.ОписаниеОшибок;
		Иначе
			ТекстСообщения = НСтр("ka='საცალო გაყიდვების შესახებ ანგარიშგებების ჩატვირთვა ვერ მოხერხდა:';ru='Отчеты о розничных продажах загрузить не удалось:'") + Параметры.ОписаниеОшибок;
		КонецЕсли;
