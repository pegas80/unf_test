			...
			Если ФайлВременный.Существует() Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(
						НСтр("ka='მიწოდებული მონაცემები. ფაილების წაშლა მოცულობაში';ru='Поставляемые данные.Удаление файлов в томе'",
...
Функция ПоставляемыеДанныеИзКэша(Знач СсылкаИлиИдентификатор) Экспорт
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მისაწოდებელი მონაცემები.ფაილის მიღება ტომიდან';ru='Поставляемые данные.Получение файла из тома'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
...
Функция ПолучитьОписаниеДанных(Знач Дескриптор) Экспорт
	...
	Если ТипЗнч(Дескриптор) = Тип("ОбъектXDTO") Тогда
		...
		Описание = Описание + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka=', დამატებულია: %1 (%2), რეკომენდებულია ჩატვირთვა: %3 (%2)';ru=', добавлен: %1 (%2), рекомендовано загрузить: %3 (%2)'"), 
	...
	Иначе
		...
		Описание = Описание + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka=', დამატებულია: %1 (%2)';ru=', добавлен: %1 (%2)'"), 
