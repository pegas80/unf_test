			...
			Если ФайлВременный.Существует() Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(
						НСтр("ka='მიწოდებული მონაცემები. ფაილების წაშლა მოცულობაში';ru='Поставляемые данные.Удаление файлов в томе';en='Dannye.Udalenie supplied files in the volume'",
...
Функция ПоставляемыеДанныеИзКэша(Знач СсылкаИлиИдентификатор) Экспорт
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მისაწოდებელი მონაცემები.ფაილის მიღება ტომიდან';ru='Поставляемые данные.Получение файла из тома';en='Dannye.Poluchenie supplied from the volume file'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
...
""
"Файл: ""%1.%2"".';ka='ფაილის გახსნის შეცდომა: სერვერზე ფაილი ვერ მოიძებნა. მიმართეთ ადმინისტრატორს."
""
"ფაილი: ""%1.%2"".'"),
...
Функция ПолучитьОписаниеДанных(Знач Дескриптор) Экспорт
	...
	Если ТипЗнч(Дескриптор) = Тип("ОбъектXDTO") Тогда
		...
		Описание = Описание + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka=', დამატებულია: %1 (%2), რეკომენდებულია ჩატვირთვა: %3 (%2)';ru=', добавлен: %1 (%2), рекомендовано загрузить: %3 (%2)';en='Is added: 1% (2%), recommended load: 3% (2%)'"), 
	...
	Иначе
		...
		Описание = Описание + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka=', დამატებულია: %1 (%2)';ru=', добавлен: %1 (%2)';en='Is added: 1% (2%)'"), 
