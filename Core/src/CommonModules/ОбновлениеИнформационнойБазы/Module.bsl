...
Процедура ЗаписатьОбъект(Знач Объект, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, 
	...
	Если ДокументРежимЗаписи <> Неопределено Тогда
		Если ТипЗнч(ДокументРежимЗаписи) <> Тип("РежимЗаписиДокумента") Тогда
			ТекстИсключения = НСтр("ka='პარამეტრ დოკუმენტიჩაწერისრეჟიმი-ს არასწორი ტიპი';ru='Неправильный тип параметра ДокументРежимЗаписи'");
...
Процедура ОтметитьВыполнениеОбработки(Данные, ДополнительныеПараметры = Неопределено, Очередь = Неопределено) Экспорт
	...
	Если (ТипЗнч(Данные) = Тип("Массив")
		...
		ТекстИсключения = НСтр("ka='პროცედურა საინფორმაციობაზისგანახლება.დამუშავებისშესრულებისაღნიშვნა-ში გადაცემულია ცარიელი მასივი. დამუშავების შესრულების აღნიშვნა შეუძლებელია.';ru='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки передан пустой массив. Не возможно отметить выполнение обработки.'");
		ВызватьИсключение ТекстИсключения;
...
Функция МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипДанных = Тип("ДанныеФормыСтруктура") Тогда
		...
		Иначе
			ТекстИсключения = НСтр("ka='პროცედურა საინფორმაციობაზისგანახლება.მეტამონაცემებიდამონაცემთამიხედვითშერჩევა არ შეიძლება, რომ ამ ფორმისთვის იქნას გამოყენებული.';ru='Процедура ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным не может быть использована для этой формы.'");
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = НСтр("ka='მეტამონაცემთა ამ ტიპისთვის ანალიზს მხარდაჭერა არ გააჩნია შემდეგ ფუნქციაში საინფორმაციობაზისგანახლება.მეტამონაცემებიდამონაცემთამიხედვითშერჩევა.';ru='Для этого типа метаданных не поддерживается анализ в функции ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоДвижения Тогда
		...
		Если ДополнительныеПараметры.ОтметитьВсеРегистраторы Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ka='რეგისტრის ყველა რეგისტრატორის რეგისტრაციისთვის აუცილებელია პარამეტრ ""მონაცემები""-ში მეტამონაცემთაობიექტი:დოკუმენტი ან დოკუმენტიბმული-ს გადაცემა.';ru='Для регистрации всех регистраторов регистра необходимо в параметре ""Данные"" передать ОбъектМетаданных:Документ или ДокументСсылка.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Иначе
			Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
				ТекстИсключения = НСтр("ka='მთლიანად, მეტამონაცემთა ობიექტის განახლებაზე რეგისტრაციას მხარდაჭერა არ გააჩნია. საჭიროა კონკრეტული მონაცემების განახლება.';ru='Не поддерживается регистрация к обновлению целиком объекта метаданных. Нужно обновлять конкретные данные.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ka='გადაცემულია პარამეტრ ""სრულისახელიობიექტისმეტამონაცემები""-ს არასწორი ტიპი შემდეგ ფუნქციაში საინფორმაციობაზისგანახლება.დასამუშავებლადმონაცემებიარის';ru='Передан неправильный тип параметра ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	...
	Для каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл 
		...
		Иначе
			ТекстИсключения = НСтр("ka='მეტამონაცემთა ტიპ ""%МетаданныеОбъекта%""-ისთვის ფუნქცია საინფორმაციობაზისგანახლება.დასამუშავებლადმონაცემებიარის-ში შემოწმებას მხარდაჭერა არ გააჩნია';ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
	...
	Иначе
		...
		Если ДополнительныеИсточникиСсылки.Количество() > 0 Тогда
			...
			Если ПолноеИмяОбъекта = Неопределено Тогда
				ТекстИсключения = НСтр("ka='ფუნქცია %ИмяФункции%-ის მოხმობის შეცდომა: დოკუმენტის სახელი არ არის გადაცემული, მაგრამ გადაცემულია მონაცემთა დამატებითი წყაროები.';ru='Ошибка вызова функции %ИмяФункции%: не передано имя документа, но переданы дополнительные источники данных.'");
