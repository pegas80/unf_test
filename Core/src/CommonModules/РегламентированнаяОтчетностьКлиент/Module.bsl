...
Функция ОсновнаяФормаОрганизацияНеЗаполненаВывестиТекст() Экспорт
	...
	Возврат НСтр("ka='ორგანიზაცია არ არის არჩეული.';ru='Не выбрана организация.';en='Not selected organization.'");
...
Функция ПриЗаписиРегламентированногоОтчетаНаКлиенте(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინახება %1...';ru='Сохраняется %1...';en='Saved% 1 ...'"), Форма.Заголовок), , , БиблиотекаКартинок.Записать);
...
Процедура МеханизмыОчисткиРегламентированныхОтчетов(Форма, РежимОчистки = Неопределено, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	Если СтрНайти(РежимОчистки, "ОчиститьТекущийРаздел") > 0 Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстВопроса);
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Форма", Форма);
...
Процедура Очистить(Форма, ВыполняемоеОповещение = Неопределено, ЗадаватьВопросПередОчисткой = Истина, Знач ТекстВопроса = "") Экспорт
	...
	Если ЗадаватьВопросПередОчисткой Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстВопроса), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
"иначе измененные данные будут утеряны.';ka='Данные отчета были изменены."
"Перед завершением работы рекомендуется записать отчет,"
"иначе измененные данные будут утеряны.'");
		...
		ПоказатьВопрос(Оповещение, НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?';en='The data have been changed. I save changes?'"), РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да);
...
"Это связано с тем, что индивидуальные предприниматели не обязаны сдавать бухгалтерскую отчетность (пп. 5 п. 1 ст. 23 НК РФ).';ka='Отчет не может быть выгружен."
"Выгрузить этот отчет могут только организации."
"Для индивидуальных предпринимателей выгрузка не предусмотрена форматом ФНС."
...
"Это связано с тем, что индивидуальные предприниматели не обязаны сдавать бухгалтерскую отчетность (пп. 5 п. 1 ст. 23 НК РФ).';ka='Отчет не может быть отправлен."
"Отправить этот отчет могут только организации."
"Для индивидуальных предпринимателей отправка не предусмотрена форматом ФНС."
"Это связано с тем, что индивидуальные предприниматели не обязаны сдавать бухгалтерскую отчетность (пп. 5 п. 1 ст. 23 НК РФ).'"));
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ДистрибутивКомпонентыПечатиМашиночитаемыхФорм Цикл
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось выгрузить шаблон машиночитаемой формы ""%1"" во временный каталог!%2';ru='Не удалось выгрузить шаблон машиночитаемой формы ""%1"" во временный каталог!%2';en='Unable to unload the machine-readable form template ""%1"" into a temporary directory! 2%'"), Элемент.Представление, Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЦикла;
	...
	Если ТаксДокПринт = Неопределено Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='დაყენება';ru='Установить';en='establish'"));
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для формирования машиночитаемой формы необходимо установить%1библиотечный модуль печати двухмерного штрихкода PDF417.';en='For the formation of a machine-readable form, you must install% 1bibliotechny printing module two-dimensional bar code PDF417.'"), Символы.ПС);
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеУстановитьКомпонентуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ УстановитьКомпонентуПечатиМашиночитаемыхФорм(Истина, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВременныйКаталог) Тогда
			ПоказатьПредупреждение(,НСтр("ka='მანქანასაკითხავი ფორმის ჩამოყალიბება შეწყვეტილია: სისტემაში არ არის დაყენებული დამხმარე კომპონენტა.';ru='Формирование машиночитаемой формы прервано: в системе не установлена вспомогательная компонента.';en='Forming machine readable form is interrupted: in the system is not installed auxiliary component.'"));
			Возврат;
		Иначе
			ТаксДокПринт = СоздатьОбъектДляФормированияМашичитаемойФормы();
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПодключитьРасширениеРаботыСФайламиПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВебБраузер)
		...
		ПоказатьПредупреждение(,НСтр("ka='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 в Веб-клиенте поддерживается только в браузере Internet Explorer.';ru='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 в Веб-клиенте поддерживается только в браузере Internet Explorer.';en='Attention! Print blank form with a two-dimensional bar code PDF417 the Web client is only supported in the browser Internet Explorer.'"));
		Возврат;
	КонецЕсли;
...
"Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';ka='На титульном листе не указано наименование налогового органа, в который представляется декларация."
...
Функция УстановитьКомпонентуПечатиМашиночитаемыхФорм(ЖдатьЗавершения, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог = "") Экспорт
	...
	Исключение
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось сохранить дистрибутив компоненты печати машиночитаемых форм во временный файл!%1';ru='Не удалось сохранить дистрибутив компоненты печати машиночитаемых форм во временный файл!%1';en='Failed to save print distribution components of machine-readable form in a temporary file!% 1'"), Символы.ПС + ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
	...
	#Если ВебКлиент Тогда
		Если НЕ РезультатПолученияФайлов Тогда
			...
			ПоказатьПредупреждение(,НСтр("ka='Установка компоненты отменена.';ru='Установка компоненты отменена.';en='Installing components canceled.'"));
	...
	Исключение
		УдалитьВременныйФайлДистрибутива(ВремФайлДистрибутива);
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მანქანასაკითხავი ფორმების ბეჭდვის კომპონენტის დაყენების მცდელობისას მოხდა შეცდომა!%1';ru='При попытке установки компоненты печати машиночитаемых форм произошла ошибка!%1';en='When attempting to install the components of the print machine-readable error occurred!% 1'"), Символы.ПС + ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОбработатьОшибкуМодуляПечатиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ეხლა, გაშვებული იქნება ბეჭდვის კომპონენტის დაყენების პროგრამა.%1შეიძლება, რომ დაყენების მსვლელობაში საჭირო გახდეს ""1С:საწარმო""-ს მუშაობის სეანსის დასრულება.%2გავაგრძელოთ?';ru='Сейчас будет запущена программа установки компоненты печати.%1Возможно, в ходе установки потребуется завершение сеанса работы ""1С:Предприятие"".%2Продолжить?';en='Who will run the installation program printing components 1Vozmozhno%, during the installation will require the completion of a session of ""1C: Enterprise""..% 2Prodolzhit?'"), Символы.ПС, Символы.ПС);
...
Функция НапечататьОтчеты(СписокСсылокНаРеглОтчеты, ВидПечати = "ПоказатьБланк") Экспорт
	...
	Для Каждого Стр Из СписокСсылокНаРеглОтчеты Цикл
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1. ანგარიშგების ამობეჭდვა ვერ მოხერხდა! სავარაუდოდ, ანგარიშგების მიერ არ არის გათვალიწინებული ამობეჭდვის შესაძლებლობა ფორმის გაუხსნელად.';ru='%1. Не удалось напечатать отчет! Вероятно, отчетом не предусмотрена возможность печати без открытия формы.';en='%one. Unable to print the report! Probably, the report does not provide the ability to print without opening form.'"), ПредставлениеДокументаРеглОтч);
...
Функция ИнициализацияОтчета(Док, ПредставлениеДокументаРеглОтч = Неопределено) Экспорт
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';ru='Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';en='An old revision of the report form is not supported by the current configuration version.'");
		Сообщение.Сообщить();
...
Функция ВыгрузитьОтчеты(СписокСсылокНаРеглОтчеты, КаталогСохранения = Неопределено, СообщенияОбОшибках = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина, ВыгружатьСОшибками = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, УникальныйИДФормы = Неопределено) Экспорт
	...
	Попытка
		...
		Если РезультатПроверки = 1 Тогда
			...
			ПоказатьПредупреждение(,НСтр("ka='არჩეულ ანგარიშგებებს განსხვავებული რეკვიზიტები ""ორგანიზაცია"" გააჩნიათ. პაკეტური გადმოტვირთვა შეუძლებელია!';ru='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна!';en='We selected different details of the report ""Organization"". Batch upload is not possible!'"));
			
		ИначеЕсли РезультатПроверки = 2 Тогда
			
			ПоказатьПредупреждение(,НСтр("ka='არჩეულ ანგარიშგებებს განსხვავებული შედგენის პერიოდების დასრულების თარიღები აქვთ. საპაკეტო გადმოტვირთვა შეუძლებელია!';ru='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна!';en='We selected reports vary the date of the drawing up periods. Batch upload is not possible!'"));
...
Процедура ВыбратьИУстановитьТерриториальныйОрганФНС(Форма, СтандартнаяОбработка, ИмяТитульногоЛиста, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='ინსპექციის ასარჩევად საჭიროა შესაბამისი ჩანაწერის წარმოება ცნობარ ""საგადასახადო ორგანოში რეგისტრაციები""-ში.';ru='Для выбора инспекции необходимо завести соответствующую запись в справочнике ""Регистрации в налоговом органе"".';en='To select the inspections necessary to make the appropriate entry in the ""Registration with the tax authority.""'");
...
Процедура ПроверитьКонтрольныеСоотношенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ДополнительныеПараметры.НеОткрыватьПриОтсутствииОшибок = Ложь Тогда 
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='В отчете не обнаружены ошибки по соотношениям показателей."
"Вывести форму для просмотра соотношений?';ru='В отчете не обнаружены ошибки по соотношениям показателей."
...
Процедура ПриНажатииНаКнопкуОтправкиЧерезПредставителяВРегламентированномОтчете(Форма, КонтролирующийОрган = "ФНС") Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Иначе // все, кроме документа РегламентированныйОтчет
			ПоказатьПредупреждение(,НСтр("ka='Необходимо записать отчет перед отправкой.';ru='Необходимо записать отчет перед отправкой.';en='You must write the report before sending it.'"));
		КонецЕсли;
	Иначе 
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ОткрытьФормуПрисоединенныеФайлы(Результат, ДопПараметры) Экспорт
	...
	Если ТипЗнч(ДопПараметры.ВладелецФайла)
		...
		ТекстПредупреждения = НСтр("ka='Присоединение файлов к электронным представлениям регламентированных отчетов не предусмотрено';ru='Присоединение файлов к электронным представлениям регламентированных отчетов не предусмотрено';en='Attaching files to the electronic submission of reports provided by the regulated'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		...
		Если ТипЗнч(ДопПараметры.ВладелецФайла) <> Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если РегламентированнаяОтчетностьВызовСервера.ВозможноПрисоединениеФайловКОбъекту(
				...
				Если НЕ ПараметрыПроверки.СтандартнаяОбработка Тогда
					...
					Иначе
						ТекстПредупреждения = НСтр("ka='Присоединение файлов к отчетам данного вида из формы списка отчетов не предусмотрено';ru='Присоединение файлов к отчетам данного вида из формы списка отчетов не предусмотрено';en='Attaching files to the records of this species from the list of report forms provided by'");
					КонецЕсли;
			...
			Иначе
				...
				ТекстПредупреждения = НСтр("ka='Присоединение файлов к отчетам данного вида не предусмотрено';ru='Присоединение файлов к отчетам данного вида не предусмотрено';en='Attaching files to the records of this type is not provided'");
				ПоказатьПредупреждение(, ТекстПредупреждения);
...
Процедура ОткрытьИсториюОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='Недостаточно прав для использования модуля документооборота!';ru='Недостаточно прав для использования модуля документооборота!';en='Not have permission to use the Workflow module!'"));
		Возврат;
	КонецЕсли;
	...
	Если Источник = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='Объект не записан.';ru='Объект не записан.';en='The object is not recorded.'"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьАвтоматическиСформированнуюФормуРеглОтчета(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, СсылкаНаСохрРеглОтчет = Неопределено, КорректирующийОтчет = Ложь)
	...
	Если ТекФорма = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ანგარიშგების გახსნა ვერ მოხერხდა!';ru='Не удалось открыть отчет!';en='Unable to open the report!'");
		Сообщение.Сообщить();
...
Процедура ОбработатьОшибкуМодуляПечати(ОписаниеОшибки, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог)
	...
	ИначеЕсли КодОшибки = "0x8004003C" Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Отсутствует шаблон для формирования машиночитаемой формы!%1Возможно, необходимо зарегистрировать в информационной базе внешний шаблон.%2Показать список зарегистрированных шаблонов?';ru='Отсутствует шаблон для формирования машиночитаемой формы!%1Возможно, необходимо зарегистрировать в информационной базе внешний шаблон.%2Показать список зарегистрированных шаблонов?';en='There is no template for the formation of a machine-readable form! 1Vozmozhno%, you need to register in the database of the external template.% 2Pokazat list of registered templates?'"),
			Символы.ПС, Символы.ПС);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ka='შეცდომა მანქანაწასაკითხი ფორმის ჩამოყალიბებისას';ru='Ошибка при формировании машиночитаемой формы';en='Error in the formation of a machine-readable form'"));
		Возврат;
	...
	ПоказатьВопрос(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстПредупреждения),
				РежимДиалогаВопрос.ДаНет,
				,
				,
				НСтр("ka='შეცდომა მანქანაწასაკითხი ფორმის ჩამოყალიბებისას';ru='Ошибка при формировании машиночитаемой формы';en='Error in the formation of a machine-readable form'"));
...
"Открыть справочную информацию с описанием особенностей настроек веб-браузера?';ka='Формирование бланка формы прервано. Не удалось подключить вспомогательную"
"компоненту для формирования бланков форм с двухмерным штрихкодом PDF417."
"Возможно, браузер не настроен в соответствии с особенностями настройки веб-браузера."
