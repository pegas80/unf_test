...
Функция ОсновнаяФормаОрганизацияНеЗаполненаВывестиТекст() Экспорт
	...
	Возврат НСтр("ka='ორგანიზაცია არ არის არჩეული.';ru='Не выбрана организация.'");
...
Функция ПриЗаписиРегламентированногоОтчетаНаКлиенте(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინახება %1...';ru='Сохраняется %1...'"), Форма.Заголовок), , , БиблиотекаКартинок.Записать);
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ДистрибутивКомпонентыПечатиМашиночитаемыхФорм Цикл
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось выгрузить шаблон машиночитаемой формы ""%1"" во временный каталог!%2';ru='Не удалось выгрузить шаблон машиночитаемой формы ""%1"" во временный каталог!%2'"), Элемент.Представление, Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЦикла;
	...
	Если ТаксДокПринт = Неопределено Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='დაყენება';ru='Установить'"));
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеУстановитьКомпонентуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ УстановитьКомпонентуПечатиМашиночитаемыхФорм(Истина, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВременныйКаталог) Тогда
			ПоказатьПредупреждение(,НСтр("ka='მანქანასაკითხავი ფორმის ჩამოყალიბება შეწყვეტილია: სისტემაში არ არის დაყენებული დამხმარე კომპონენტა.';ru='Формирование машиночитаемой формы прервано: в системе не установлена вспомогательная компонента.'"));
			Возврат;
		Иначе
			ТаксДокПринт = СоздатьОбъектДляФормированияМашичитаемойФормы();
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПодключитьРасширениеРаботыСФайламиПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВебБраузер)
		...
		ПоказатьПредупреждение(,НСтр("ka='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 в Веб-клиенте поддерживается только в браузере Internet Explorer.';ru='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 в Веб-клиенте поддерживается только в браузере Internet Explorer.'"));
		Возврат;
	КонецЕсли;
...
Функция УстановитьКомпонентуПечатиМашиночитаемыхФорм(ЖдатьЗавершения, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог = "") Экспорт
	...
	Исключение
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось сохранить дистрибутив компоненты печати машиночитаемых форм во временный файл!%1';ru='Не удалось сохранить дистрибутив компоненты печати машиночитаемых форм во временный файл!%1'"), Символы.ПС + ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
	...
	#Если ВебКлиент Тогда
		Если НЕ РезультатПолученияФайлов Тогда
			...
			ПоказатьПредупреждение(,НСтр("ka='Установка компоненты отменена.';ru='Установка компоненты отменена.'"));
	...
	Исключение
		УдалитьВременныйФайлДистрибутива(ВремФайлДистрибутива);
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მანქანასაკითხავი ფორმების ბეჭდვის კომპონენტის დაყენების მცდელობისას მოხდა შეცდომა!%1';ru='При попытке установки компоненты печати машиночитаемых форм произошла ошибка!%1'"), Символы.ПС + ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
...
Функция ИнициализацияОтчета(Док, ПредставлениеДокументаРеглОтч = Неопределено) Экспорт
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';ru='Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.'");
		Сообщение.Сообщить();
...
Функция ВыгрузитьОтчеты(СписокСсылокНаРеглОтчеты, КаталогСохранения = Неопределено, СообщенияОбОшибках = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина, ВыгружатьСОшибками = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, УникальныйИДФормы = Неопределено) Экспорт
	...
	Попытка
		...
		Если РезультатПроверки = 1 Тогда
			...
			ПоказатьПредупреждение(,НСтр("ka='არჩეულ ანგარიშგებებს განსხვავებული რეკვიზიტები ""ორგანიზაცია"" გააჩნიათ. პაკეტური გადმოტვირთვა შეუძლებელია!';ru='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна!'"));
			
		ИначеЕсли РезультатПроверки = 2 Тогда
			
			ПоказатьПредупреждение(,НСтр("ka='არჩეულ ანგარიშგებებს განსხვავებული შედგენის პერიოდების დასრულების თარიღები აქვთ. საპაკეტო გადმოტვირთვა შეუძლებელია!';ru='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна!'"));
...
Процедура ВыбратьИУстановитьТерриториальныйОрганФНС(Форма, СтандартнаяОбработка, ИмяТитульногоЛиста, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='ინსპექციის ასარჩევად საჭიროა შესაბამისი ჩანაწერის წარმოება ცნობარ ""საგადასახადო ორგანოში რეგისტრაციები""-ში.';ru='Для выбора инспекции необходимо завести соответствующую запись в справочнике ""Регистрации в налоговом органе"".'");
...
Процедура ПриНажатииНаКнопкуОтправкиЧерезПредставителяВРегламентированномОтчете(Форма, КонтролирующийОрган = "ФНС") Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Иначе // все, кроме документа РегламентированныйОтчет
			ПоказатьПредупреждение(,НСтр("ka='Необходимо записать отчет перед отправкой.';ru='Необходимо записать отчет перед отправкой.'"));
		КонецЕсли;
	Иначе 
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ОткрытьФормуПрисоединенныеФайлы(Результат, ДопПараметры) Экспорт
	...
	Если ТипЗнч(ДопПараметры.ВладелецФайла)
		...
		ТекстПредупреждения = НСтр("ka='Присоединение файлов к электронным представлениям регламентированных отчетов не предусмотрено';ru='Присоединение файлов к электронным представлениям регламентированных отчетов не предусмотрено'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		...
		Если ТипЗнч(ДопПараметры.ВладелецФайла) <> Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если РегламентированнаяОтчетностьВызовСервера.ВозможноПрисоединениеФайловКОбъекту(
				...
				Если НЕ ПараметрыПроверки.СтандартнаяОбработка Тогда
					...
					Иначе
						ТекстПредупреждения = НСтр("ka='Присоединение файлов к отчетам данного вида из формы списка отчетов не предусмотрено';ru='Присоединение файлов к отчетам данного вида из формы списка отчетов не предусмотрено'");
					КонецЕсли;
			...
			Иначе
				...
				ТекстПредупреждения = НСтр("ka='Присоединение файлов к отчетам данного вида не предусмотрено';ru='Присоединение файлов к отчетам данного вида не предусмотрено'");
				ПоказатьПредупреждение(, ТекстПредупреждения);
...
Процедура ОткрытьИсториюОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='Недостаточно прав для использования модуля документооборота!';ru='Недостаточно прав для использования модуля документооборота!'"));
		Возврат;
	КонецЕсли;
	...
	Если Источник = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='Объект не записан.';ru='Объект не записан.'"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьАвтоматическиСформированнуюФормуРеглОтчета(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, СсылкаНаСохрРеглОтчет = Неопределено, КорректирующийОтчет = Ложь)
	...
	Если ТекФорма = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ანგარიშგების გახსნა ვერ მოხერხდა!';ru='Не удалось открыть отчет!'");
		Сообщение.Сообщить();
...
Процедура ОбработатьОшибкуМодуляПечати(ОписаниеОшибки, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог)
	...
	ИначеЕсли КодОшибки = "0x8004003C" Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ka='შეცდომა მანქანაწასაკითხი ფორმის ჩამოყალიბებისას';ru='Ошибка при формировании машиночитаемой формы'"));
		Возврат;
	...
	ПоказатьВопрос(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстПредупреждения),
				...
				,
				,
				НСтр("ka='შეცდომა მანქანაწასაკითხი ფორმის ჩამოყალიბებისას';ru='Ошибка при формировании машиночитаемой формы'"));
