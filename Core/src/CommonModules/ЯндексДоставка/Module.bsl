...
"Удостоверьтесь, что все поля адреса доставки корректно заполнены.'");
	...
	ИначеЕсли ПараметрыПоискаСпособаДоставки.СпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
		ВызватьИсключение НСтр("ka='Выбран неподходящий способ доставки.';ru='Выбран неподходящий способ доставки.'");
	КонецЕсли;
...
Функция ЗаданиеПередатьЗаказы(Знач ТаблицаЗаказов, Знач АдресХранилища) Экспорт
	...
	НСтр("ka='Неверный тип параметра, ожидается ""ТаблицаЗначений.';ru='Неверный тип параметра, ожидается ""ТаблицаЗначений.'"),
	...
	Для Каждого ТекСтрока Из ТаблицаЗаказов Цикл
		...
		Если ТекСтрока.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
			...
			НСтр("ka='Завершенный заказ не может быть передан в Яндекс.Доставку.';ru='Завершенный заказ не может быть передан в Яндекс.Доставку.'"));
		Иначе
...
Процедура ОбновитьСостоянияИДанныеЗаказов(Знач ДанныеЗаказовИзЯндексДоставки, Знач СвойстваЗаказов, Знач СтатусыСостояния, Знач ОбновленныеЗаказы, Знач НастройкаПодключения)
	...
	Для Каждого ТекДанныеЗаказа Из ДанныеЗаказовИзЯндексДоставки Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='Обновление состояния заказов по данным Яндекс.Доставки';ru='Обновление состояния заказов по данным Яндекс.Доставки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
