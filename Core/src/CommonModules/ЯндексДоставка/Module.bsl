...
"Удостоверьтесь, что все поля адреса отправления корректно заполнены.';ka='Адрес отправления указан неверно."
...
"Удостоверьтесь, что все поля адреса доставки корректно заполнены.';ka='Адрес доставки указан неверно."
"Удостоверьтесь, что все поля адреса доставки корректно заполнены.'");
	...
	ИначеЕсли ПараметрыПоискаСпособаДоставки.СпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
		ВызватьИсключение НСтр("ka='Выбран неподходящий способ доставки.';ru='Выбран неподходящий способ доставки.';en='Chosen the wrong method of delivery.'");
	КонецЕсли;
...
Процедура ЗаданиеЗагрузкаДанныхИзЯндексДоставки(Ключ, АдресХранилища = Неопределено) Экспорт
	...
	Если НастройкаПодключения = Неопределено Тогда
		ВызватьИсключение
		СтрШаблон(
		НСтр("ru='Отсутствует настройка подключения к Яндекс.Доставке для организации ""%1""';ka='Отсутствует настройка подключения к Яндекс.Доставке для организации ""%1""'"),
...
Функция ЗаданиеПередатьЗаказы(Знач ТаблицаЗаказов, Знач АдресХранилища) Экспорт
	...
	НСтр("ka='Неверный тип параметра, ожидается ""ТаблицаЗначений.';ru='Неверный тип параметра, ожидается ""ТаблицаЗначений.';en='Wrong type of parameter expected ""TablitsaZnacheny.'"),
	...
	Для Каждого ТекСтрока Из ТаблицаЗаказов Цикл
		...
		Если ТекСтрока.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
			...
			НСтр("ka='Завершенный заказ не может быть передан в Яндекс.Доставку.';ru='Завершенный заказ не может быть передан в Яндекс.Доставку.';en='The completed order can not be transferred to Yandeks.Dostavku.'"));
		Иначе
...
Функция РассчитатьСтоимостьДляМагазина(ВариантДоставкиЗаказа, СуммаЗаказа, ОбъявленнаяЦенность) Экспорт
	...
	Результат.СтоимостьДляМагазинаПредставление = СтрШаблон(НСтр("ru='%1 р.';ka='%1 р.'"), Результат.СтоимостьДляМагазина);
...
"%2';ka='Яндекс сообщил об ошибке %1"
"%2'"),
...
Процедура ОбновитьСостоянияИДанныеЗаказов(Знач ДанныеЗаказовИзЯндексДоставки, Знач СвойстваЗаказов, Знач СтатусыСостояния, Знач ОбновленныеЗаказы, Знач НастройкаПодключения)
	...
	Для Каждого ТекДанныеЗаказа Из ДанныеЗаказовИзЯндексДоставки Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='Обновление состояния заказов по данным Яндекс.Доставки';ru='Обновление состояния заказов по данным Яндекс.Доставки';en='Updating the status of orders according Yandeks.Dostavki'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
