...
Функция ОплатитьПлатежнойКартой(ОбъектДрайвера, Параметры, ПараметрыПодключения, Сумма, НомерКарты, ВыходныеПараметры) Экспорт
	...
	Исключение
		...
		Если (Найти(ТекстОшибки, НСтр("ka='ფაქტობრივ პარამეტრებს';ru='фактических параметров';en='actual parameters'")) > 0) Тогда
			ТекстОшибки = НСтр("ru='Ошибка вызова метода библиотеки ""SBRFCOM""."
"Неверное количество параметров метода ""ОплатитьПлатежнойКартой""."
"Укажите в форме настройке оборудования другую версию библиотеки ""SBRFCOM"".';ka='ბიბლიოთეკა ""SBRFCOM""-ის მეთოდის მოხმობის შეცდომა."
"მეთოდ ""საგადახდობარათითგადახდა""-ს პარამეტრების არასწორი რაოდენობა."
"მოწყობილობის გამართვის ფორმაში მიუთითეთ ბიბლიოთეკა ""SBRFCOM""-ის სხვა ვერსია.'");
		КонецЕсли;
		ВыходныеПараметры.Добавить(999);
...
"Укажите в форме настройке оборудования другую версию библиотеки ""SBRFCOM"".';ka='ბიბლიოთეკა ""SBRFCOM""-ის მეთოდის მოხმობის შეცდომა."
"მეთოდ ""საგადახდობარათითგადახდისდაბრუნება""-ს პარამეტრების არასწორი რაოდენობა."
"მოწყობილობის გამართვის ფორმაში მიუთითეთ ბიბლიოთეკა ""SBRFCOM""-ის სხვა ვერსია.'");
		КонецЕсли;
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстОшибки);
...
Функция ПолучитьСтрокиСлипЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, СлипЧек, ВыходныеПараметры);
	...
	Для Индекс = 1 По ОбъектДрайвера.КоличествоСтрокСлипЧека Цикл
		...
		Иначе
			...
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(НСтр("ka='შეცდომა სლიპ-ჩეკის სტრიქონების მიღებისას.';ru='Ошибка при получении строк слип-чека.';en='Error retrieving lines slip-check.'"));
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Если Результат Тогда
		...
		ВыходныеПараметры.Добавить(НСтр("ka='ბარათის ტესტური წაკითხვა წარმატებით შესრულდა.';ru='Тестовое чтение карты успешно выполнено.';en='Test card reading successful.'"));
	Иначе
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить("");
		ОбъектДрайвера.ПолучитьОшибку(ВыходныеПараметры[1]);
