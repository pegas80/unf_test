...
Функция НайтиСоздатьСчетНаОплатуПоставщика(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда // получены изменения по существующему документу
			...
			Если СсылкаНаВладельца.Проведен Тогда 
				ВызватьИсключение НСтр("ka='ედ-ის საფუძველზე შევსება შესაძლებელია მხოლოდ გაუტარებელი დოკუმენტისთვის';ru='Заполнение на основании ЭД возможно только для непроведенного документа'");
			КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='შევსება ედ-ის საფუძველზე';ru='Заполнение на основании ЭД'"),
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СведенияОбУчастнике.СвидетельствоСерияНомер) Тогда
			Свидетельство = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='%2-ის მოწმობა %%1';ru='Свидетельство №%1 от %2'"),
...
Функция СоздатьПерезаполнитьНоменклатуруПоставщика(СтрокаОбъекта, ДеревоРазбора) Экспорт
	...
	Попытка
		НовЭл.Записать();
	Исключение
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ცნობარ %1-ის ელემენტის შექმნა.';ru='Создание элемента справочника %1.'"),
...
Функция ОпределитьТипДокументаОснованияДляЗаголовка(ВидОперации, ИсправляемыйДокументРеализации) Экспорт
	...
	Если СоглашениеКСоглашению Тогда
		ЗаголовокДокументаОснования = НСтр("ka='შეთანხმებისთვის';ru='к соглашению'");
	ИначеЕсли ТипЗнч(ИсправляемыйДокументРеализации) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		ЗаголовокДокументаОснования = НСтр("ka='მომსახურებების გაწევის აქტისადმი';ru='к акту об оказании услуг'");
	Иначе
		ЗаголовокДокументаОснования = НСтр("ka='ზედნადებისთვის';ru='к накладной'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='ელექტრონული დოკუმენტების ჩატვირთვა';ru='Загрузка электронных документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
