...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	КлючОсновногоВариантаОформления = НСтр("ka='კაპიკებით';ru='С копейками'");
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		КлючВариантаОформления = НСтр("ka='კაპიკების გარეშე';ru='Без копеек'");
		ОтчетыУНФ.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0");
		
		КлючВариантаОформления = НСтр("ka='ათასებში';ru='В тысячах'");
...
Процедура ДобавитьВариантыОформленияКоличества(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	КлючОсновногоВариантаОформления = НСтр("ka='არ გასწორდეს';ru='Не выравнивать'");
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		КлючВариантаОформления = НСтр("ka='სამ ნიშნამდე გათანაბრება მძიმის (,) შემდეგ';ru='Выравнивать до 3-х знаков после запятой'");
...
Процедура ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных) Экспорт
	...
	Если НЕ Поле=Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='წელიწადის თვე';ru='Месяц года'");
			ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата));
			ВычисляемоеПоле.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ka='ДФ=ММММ';ru='ДФ=ММММ'"));
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='წლის დღე და თვე';ru='День и месяц года'");
			ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата));
			ВычисляемоеПоле.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ka=""ДФ='д ММММ'"";ru=""ДФ='д ММММ'""")); 
			ВычисляемоеПоле.ОграничениеИспользования.Условие = Истина;
		КонецЕсли; 
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='წლის კვირა';ru='Неделя года'");
			ВычисляемоеПоле.ОграничениеИспользования.Условие = Истина;
		КонецЕсли; 
	КонецЕсли;
	...
	Если Поле<>Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='ტეგი';ru='Тег'");
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='სეგმენტი';ru='Сегмент'");
			ВычисляемоеПоле.ОграничениеИспользования.Группировка = Истина;
			ВычисляемоеПоле.ОграничениеИспользования.Поле = Истина;
			ВычисляемоеПоле.ОграничениеИспользования.Порядок = Истина;
		КонецЕсли; 
...
Процедура СтандартизироватьСхему(СхемаКомпоновкиДанных) Экспорт
	...
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодСекунда", НСтр("ka='წამი';ru='Секунда'"), НСтр("ka=""ДФ='дд.ММ.гггг ЧЧ:мм:сс'"";ru=""ДФ='дд.ММ.гггг ЧЧ:мм:сс'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодМинута", НСтр("ka='წუთი';ru='Минута'"), НСтр("ka=""ДФ='дд.ММ.гггг ЧЧ:мм'"";ru=""ДФ='дд.ММ.гггг ЧЧ:мм'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодЧас", НСтр("ka='Час';ru='Час'"), НСтр("ka=""ДФ='дд.ММ.гггг ЧЧ:00'"";ru=""ДФ='дд.ММ.гггг ЧЧ:00'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДень", НСтр("ka='დღე';ru='День'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодНеделя", НСтр("ka='კვირის დასაწყისი';ru='Начало недели'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДекада", НСтр("ka='დეკადის დასაწყისი';ru='Начало декады'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодМесяц", НСтр("ka='თვე';ru='Месяц'"), НСтр("ka=""ДФ='МММ гггг'"";ru=""ДФ='МММ гггг'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодКвартал", НСтр("ka='კვარტალი';ru='Квартал'"), НСтр("ka=""ДФ='к ''кв.'' гггг'"";ru=""ДФ='к ''кв.'' гггг'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодПолугодие", НСтр("ka='ნახევარი წლის  დასაწყისი';ru='Начало полугодия'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодГод", НСтр("ka='წელი';ru='Год'"), НСтр("ka='ДФ=гггг';ru='ДФ=гггг'"));
	...
	Для каждого Показатель Из СписокПоказателей Цикл
		...
		Для каждого ПолеИтога Из СхемаКомпоновкиДанных.ПоляИтога Цикл
			...
			Если НастройкиОтчета.ДополнительныеСвойства.ПоказыватьАбсолютноеИзменение И Показатель.Пометка Тогда
				...
				ПараметрыФормирования.СтруктураПолейСравнения.Вставить(ИмяПоля);
				ВычисляемоеПоле.Выражение = Показатель.Значение;
				ВычисляемоеПоле.Заголовок = НСтр("ka='სხვაობა';ru='Разница'");
				Ресурс = СхемаКомпоновкиДанных.ПоляИтога.Добавить();
				Ресурс.ПутьКДанным = ВычисляемоеПоле.ПутьКДанным;
				Ресурс.Группировки.Очистить();
				Для каждого ИмяГруппировки Из ПолеИтога.Группировки Цикл
			...
			Если НастройкиОтчета.ДополнительныеСвойства.ПоказыватьОтносительноеИзменение И Показатель.Пометка Тогда
				...
				ПараметрыФормирования.СтруктураПолейСравнения.Вставить(ИмяПоля);
				ВычисляемоеПоле.Выражение = Показатель.Значение;
				ВычисляемоеПоле.Заголовок = НСтр("ka='%';ru='%'");
	...
	Для каждого Рисунок Из ДокументРезультат.Рисунки Цикл
		...
		Для ИндексСерии = 0 По Диаграмма.Серии.Количество() - 1 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Серия.Значение) Тогда
				Серия.Значение = НСтр("ka='<არ არის მითითებული>';ru='<Не указано>'")
			КонецЕсли; 
...
Функция ТаблицаПериодов(НачалоПериода, КонецПериода, ПараметрыОтчета) Экспорт
	...
	Пока ТекущаяДата <= КонецПериода Цикл
		...
		Иначе
			...
			НСтр("ka='პარამეტრ ""პერიოდულობა""-ს დამამუშავებელი არ არის = ""%1"".';ru='Отсутствует обработчик параметра Периодичность = ""%1"".'"),
			Периодичность);
		КонецЕсли;
...
"%2';ka='შეცდომა დამატებითი ანგარიშგება ""%1""-ის ჩაწერისას: %2'"),
		...
		НСтр("ka='დამატებითი ანგარიშგების ვარიანტების ჩაწერა';ru='Запись вариантов дополнительного отчета'"),
...
Процедура ОбновитьНастройкиВарианта(Вариант, СтруктураНастроек, ЗаменятьСуществующие = Ложь)
	...
	Если СтруктураНастроек.Свойство("СвязанныеПоля") И (ОбновитьПривязки ИЛИ ЗаменятьСуществующие) Тогда
		Для каждого ОписаниеПоля Из СтруктураНастроек.СвязанныеПоля Цикл
			...
			Исключение
				ТекстСообщения = СтрШаблон(
				НСтр("ka='მოდულ ""%2""-ში დაკავშირებული ველ ""%1""-ის ტიპის არასწორი აღწერილობა, ვარიანტი ""%3""';ru='Неверное описание типа связанного поля ""%1"" в модуле ""%2"", вариант ""%3""'"), 
				ОписаниеПоля.Объект,
...
Процедура СоздатьФильтрПределы(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.Заголовок = ?(Прав(ИмяРеквизита,2)="От", НСтр("ka='(საიდან, ვისგან)';ru='от'"), НСтр("ka='до';ru='до'"));
		Элемент.РастягиватьПоГоризонтали = Истина;
...
Процедура СоздатьФильтрТумблер(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если ТипЗнч(Значения)=Тип("Массив") Тогда
			Элемент.СписокВыбора.Добавить(Стр.ТипЗначения.ПривестиЗначение(Неопределено), НСтр("ka='ყველა';ru='Все'"));
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если Стр.Тип="Фильтр" Тогда
			...
			Если ОписаниеТиповСодержитИерархическийТип(Стр.ТипЗначения) Тогда
				...
				Кнопка.Подсказка = НСтр("ka='გამოიყენება ელემენტების არჩევა';ru='Используется выбор элементов'");
...
Процедура ДобавитьПолеВыбораПериодаПланирования(НастройкиСКД, Форма, Обязательный = Истина) Экспорт
	...
	Если Стр=Неопределено Тогда
		...
		Стр.Заголовок = НСтр("ka='დაგეგმვის სცენარი';ru='Сценарий планирования'");
