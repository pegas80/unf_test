...
"ИмяОтчета = ""%1"", КлючВарианта = ""%2"".';ka='გადაცემული პარამეტრების მიხედვით ანგარიშგების ვარიანტის დადგენა ვერ მოხერხდა:"
"ანგარიშგებისსახელი = ""%1"", ვარიანტისგასაღები = ""%2"".'"),
...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	КлючОсновногоВариантаОформления = НСтр("ka='კაპიკებით';ru='С копейками';en='With penny'");
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		КлючВариантаОформления = НСтр("ka='კაპიკების გარეშე';ru='Без копеек';en='without kopecks'");
		ОтчетыУНФ.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0");
		
		КлючВариантаОформления = НСтр("ka='ათასებში';ru='В тысячах';en='thousands'");
...
Процедура ДобавитьВариантыОформленияКоличества(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	КлючОсновногоВариантаОформления = НСтр("ka='არ გასწორდეს';ru='Не выравнивать';en='Do not align'");
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		КлючВариантаОформления = НСтр("ka='სამ ნიშნამდე გათანაბრება მძიმის (,) შემდეგ';ru='Выравнивать до 3-х знаков после запятой';en='Leveling up to 3 decimal places'");
...
Процедура ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных) Экспорт
	...
	Если НЕ Поле=Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='წელიწადის თვე';ru='Месяц года';en='month year'");
			ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата));
			ВычисляемоеПоле.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ka='ДФ=ММММ';ru='ДФ=ММММ';en='ДФ=ММММ'"));
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='წლის დღე და თვე';ru='День и месяц года';en='Day and month of the year'");
			ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата));
			ВычисляемоеПоле.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ka=""ДФ='д ММММ'"";ru=""ДФ='д ММММ'"";en=""DF = 'd MMMM'""")); 
			ВычисляемоеПоле.ОграничениеИспользования.Условие = Истина;
		КонецЕсли; 
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='წლის კვირა';ru='Неделя года';en='week of the year'");
			ВычисляемоеПоле.ОграничениеИспользования.Условие = Истина;
		КонецЕсли; 
	КонецЕсли;
	...
	Если Поле<>Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='ტეგი';ru='Тег';en='tag'");
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ka='სეგმენტი';ru='Сегмент';en='Segment'");
			ВычисляемоеПоле.ОграничениеИспользования.Группировка = Истина;
			ВычисляемоеПоле.ОграничениеИспользования.Поле = Истина;
			ВычисляемоеПоле.ОграничениеИспользования.Порядок = Истина;
		КонецЕсли; 
...
Процедура СтандартизироватьСхему(СхемаКомпоновкиДанных) Экспорт
	...
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодСекунда", НСтр("ka='წამი';ru='Секунда';en='Second'"), НСтр("ka=""ДФ='дд.ММ.гггг ЧЧ:мм:сс'"";ru=""ДФ='дд.ММ.гггг ЧЧ:мм:сс'"";en=""DF = 'DD.MM.YYYY_HH: mm: ss'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодМинута", НСтр("ka='წუთი';ru='Минута';en='Minute'"), НСтр("ka=""ДФ='дд.ММ.гггг ЧЧ:мм'"";ru=""ДФ='дд.ММ.гггг ЧЧ:мм'"";en=""DF = 'DD.MM.YYYY_HH: mm'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодЧас", НСтр("ka='Час';ru='Час';en='Hour'"), НСтр("ka=""ДФ='дд.ММ.гггг ЧЧ:00'"";ru=""ДФ='дд.ММ.гггг ЧЧ:00'"";en=""DF = 'DD.MM.YYYY_HH: 00'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДень", НСтр("ka='დღე';ru='День';en='Day'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг';en='DF = DD.MM.YYYY'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодНеделя", НСтр("ka='კვირის დასაწყისი';ru='Начало недели';en='Beginning of the week'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг';en='DF = DD.MM.YYYY'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДекада", НСтр("ka='დეკადის დასაწყისი';ru='Начало декады';en='The beginning of the decade'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг';en='DF = DD.MM.YYYY'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодМесяц", НСтр("ka='თვე';ru='Месяц';en='Month'"), НСтр("ka=""ДФ='МММ гггг'"";ru=""ДФ='МММ гггг'"";en=""DF = 'MMM yyyy'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодКвартал", НСтр("ka='კვარტალი';ru='Квартал';en='Quarter'"), НСтр("ka=""ДФ='к ''кв.'' гггг'"";ru=""ДФ='к ''кв.'' гггг'"";en=""DF = 'k' 'sq.' Yyyy '"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодПолугодие", НСтр("ka='ნახევარი წლის  დასაწყისი';ru='Начало полугодия';en='Beginning six months'"), НСтр("ka='ДФ=дд.ММ.гггг';ru='ДФ=дд.ММ.гггг';en='DF = DD.MM.YYYY'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодГод", НСтр("ka='წელი';ru='Год';en='Year'"), НСтр("ka='ДФ=гггг';ru='ДФ=гггг';en='ДФ=гггг'"));
	...
	Для каждого Показатель Из СписокПоказателей Цикл
		...
		Для каждого ПолеИтога Из СхемаКомпоновкиДанных.ПоляИтога Цикл
			...
			Если НастройкиОтчета.ДополнительныеСвойства.ПоказыватьАбсолютноеИзменение И Показатель.Пометка Тогда
				...
				ПараметрыФормирования.СтруктураПолейСравнения.Вставить(ИмяПоля);
				ВычисляемоеПоле.Выражение = Показатель.Значение;
				ВычисляемоеПоле.Заголовок = НСтр("ka='სხვაობა';ru='Разница';en='Difference'");
				Ресурс = СхемаКомпоновкиДанных.ПоляИтога.Добавить();
				Ресурс.ПутьКДанным = ВычисляемоеПоле.ПутьКДанным;
				Ресурс.Группировки.Очистить();
				Для каждого ИмяГруппировки Из ПолеИтога.Группировки Цикл
			...
			Если НастройкиОтчета.ДополнительныеСвойства.ПоказыватьОтносительноеИзменение И Показатель.Пометка Тогда
				...
				ПараметрыФормирования.СтруктураПолейСравнения.Вставить(ИмяПоля);
				ВычисляемоеПоле.Выражение = Показатель.Значение;
				ВычисляемоеПоле.Заголовок = НСтр("ka='%';ru='%';en='%'");
	...
	Для каждого Рисунок Из ДокументРезультат.Рисунки Цикл
		...
		Для ИндексСерии = 0 По Диаграмма.Серии.Количество() - 1 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Серия.Значение) Тогда
				Серия.Значение = НСтр("ka='<არ არის მითითებული>';ru='<Не указано>';en='<Not Specified>'")
			КонецЕсли; 
...
Функция ТаблицаПериодов(НачалоПериода, КонецПериода, ПараметрыОтчета) Экспорт
	...
	Пока ТекущаяДата <= КонецПериода Цикл
		...
		Иначе
			...
			НСтр("ka='პარამეტრ ""პერიოდულობა""-ს დამამუშავებელი არ არის = ""%1"".';ru='Отсутствует обработчик параметра Периодичность = ""%1"".';en='No handler parameter Frequency = ""%1"".'"),
			Периодичность);
		КонецЕсли;
...
"%2';ka='შეცდომა დამატებითი ანგარიშგება ""%1""-ის ჩაწერისას: %2'"),
		Источник.Наименование,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		
		ЗаписьЖурналаРегистрации(
		НСтр("ka='დამატებითი ანგარიშგების ვარიანტების ჩაწერა';ru='Запись вариантов дополнительного отчета';en='Recording options for an additional report'"),
...
"%2';ka='დამატებითი გარე ანგარიშგება ""%1""-ის განახლება ვერ მოხერხდა შემდეგი მიზეზით:"
"%2'"), 
...
"%2';ka='შეცდომა ანგარიშგება ""%1""-ში დამამუშავებელ ""ანგარიშგებისგამართვებისდადგენისას"" მოხმობისას შემდეგი მიზეზით:"
"%2'"), 
				ВыборкаОтчеты.Отчет, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбновитьНастройкиВарианта(Вариант, СтруктураНастроек, ЗаменятьСуществующие = Ложь)
	...
	Если СтруктураНастроек.Свойство("СвязанныеПоля") И (ОбновитьПривязки ИЛИ ЗаменятьСуществующие) Тогда
		Для каждого ОписаниеПоля Из СтруктураНастроек.СвязанныеПоля Цикл
			...
			Исключение
				ТекстСообщения = СтрШаблон(
				НСтр("ka='მოდულ ""%2""-ში დაკავშირებული ველ ""%1""-ის ტიპის არასწორი აღწერილობა, ვარიანტი ""%3""';ru='Неверное описание типа связанного поля ""%1"" в модуле ""%2"", вариант ""%3""';en='Invalid field type declaration associated ""%1"" module ""%2"" variant ""%3""'"), 
				ОписаниеПоля.Объект,
...
Процедура СоздатьФильтрПределы(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.Заголовок = ?(Прав(ИмяРеквизита,2)="От", НСтр("ka='(საიდან, ვისგან)';ru='от';en='From'"), НСтр("ka='до';ru='до';en='before'"));
		Элемент.РастягиватьПоГоризонтали = Истина;
...
Процедура СоздатьФильтрТумблер(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если ТипЗнч(Значения)=Тип("Массив") Тогда
			Элемент.СписокВыбора.Добавить(Стр.ТипЗначения.ПривестиЗначение(Неопределено), НСтр("ka='ყველა';ru='Все';en='Everything'"));
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если Стр.Тип="Фильтр" Тогда
			...
			Если ОписаниеТиповСодержитИерархическийТип(Стр.ТипЗначения) Тогда
				...
				Кнопка.Подсказка = НСтр("ka='გამოიყენება ელემენტების არჩევა';ru='Используется выбор элементов';en='Use the selection elements'");
...
Процедура ДобавитьПолеВыбораПериодаПланирования(НастройкиСКД, Форма, Обязательный = Истина) Экспорт
	...
	Если Стр=Неопределено Тогда
		...
		Стр.Заголовок = НСтр("ka='დაგეგმვის სცენარი';ru='Сценарий планирования';en='planning scenario'");
