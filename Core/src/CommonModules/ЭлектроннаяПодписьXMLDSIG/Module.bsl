...
Функция ТипКриптопровайдера(МенеджерКриптографии)
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Не удалось определить тип криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1';en='Unable to determine the type of CSP 1%'"), ИмяКриптопровайдера);
	КонецЕсли;
