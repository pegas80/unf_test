...
Процедура ВыполнитьКоманду(Знач Форма, Знач ИмяКоманды) Экспорт
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		...
		Если ПустаяСтрока(Параметры.НомерТелефона) Тогда
			Если МожноОтправлятьSMS Тогда
				ТекстПредупреждения = НСтр("ka='ზარის განსახორციელებლად ან SMS-ის გასაგზავნად საჭიროა ტელეფონის ნომრის შეყვანა.';ru='Для совершения звонка или отправки SMS требуется ввести номер телефона.';en='To make a call or send an SMS you must enter a phone number.'");
			Иначе
				ТекстПредупреждения = НСтр("ka='Для совершения звонка требуется ввести номер телефона.';ru='Для совершения звонка требуется ввести номер телефона.';en='To make a call you want to enter a phone number.'");
			КонецЕсли;
			ПоказатьПредупреждение(, ТекстПредупреждения);
		ИначеЕсли МожноОтправлятьSMS Тогда
			Список = Новый СписокЗначений;
			Список.Добавить("Позвонить", НСтр("ka='Позвонить';ru='Позвонить';en='call'"),, БиблиотекаКартинок.Позвонить);
			Список.Добавить("ОтправитьSMS", НСтр("ka='სმს-ის გაგზავნა..';ru='Отправить SMS...';en='Send SMS ...'"),, БиблиотекаКартинок.ОтправитьSMS);
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Skype") Тогда
		...
		Список = Новый СписокЗначений;
		Список.Добавить("Позвонить", НСтр("ka='Позвонить';ru='Позвонить';en='call'"));
...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საკონტაქტო ინფორმაციის თაობაზე წერილის შედგენა ტიპ ""%1""-ით არ შეიძლება';ru='Нельзя создать письмо по контактной информацию с типом ""%1""';en='You can not create a letter using the contact information with the type ""%1""'"), ТипИнформации);
	КонецЕсли;
	
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("ka='წერილის გასაგზავნად აუცილებელია ელექტრონული ფოსტის მისამართის შეყვანა.';ru='Для отправки письма необходимо ввести адрес электронной почты.';en='To send a letter you need to enter an email address.'"));
		Возврат;
	КонецЕсли;
...
Процедура СоздатьSMS(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = "") Экспорт
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
		ВызватьИсключение НСтр("ka='SMS-ის გაგზავნა არ არის ხელმისაწვდომი.';ru='Отправка SMS недоступна.';en='Sending SMS is not available.'");
	КонецЕсли;
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='SMS-ის გაგზავნა ""%1"" ტიპის საკონტაქტო ინფორმაციის მიხედვით არ შეიძლება';ru='Нельзя отправитьSMS по контактной информацию с типом ""%1""';en='You can not otpravitSMS of contact information with the type ""%1""'"), ТипИнформации);
	КонецЕсли;
	
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("ka='SMS-ის გასაგზავნად საჭიროა ტელეფონის ნომრის შეყვანა.';ru='Для отправки SMS необходимо ввести номер телефона.';en='To send an SMS you must enter a phone number.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиПоВебСсылке(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.ВебСтраница") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 ტიპის მქონე საკონტაქტო ინფორმაციის თაობაზე ბმულის გახსნა არ შეიძლება';ru='Нельзя открыть ссылку по контактной информации с типом ""%1""';en='You can not open the link for contact information with the type ""%1""'"), ТипИнформации);
	КонецЕсли;
	...
	Если ТипЗнч(АдресСсылки) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ka='ბმულის მიღების შეცდომა, საკონტაქტო ინფორმაციის არასწორი ტიპი';ru='Ошибка получения ссылки, неверный тип контактной информации';en='Error getting link, the wrong type of contact information'");
	КонецЕсли;
...
Процедура ПослеВыбораИзМенюSkype(ВыбранныйЭлемент, Параметры) Экспорт
	...
	#Если НЕ ВебКлиент Тогда
		Если ПустаяСтрока(ПрограммаТелефонииУстановлена("skype")) Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ka='Skype-ით ზარის განსახორციელებლად საჭიროა პროგრამის დაყენება.';ru='Для совершения звонка по Skype требуется установить программу.';en='To make a call on Skype you want to install the program.'"));
			Возврат;
		КонецЕсли;
	#КонецЕсли
	...
	ТекстСообщения = НСтр("ka='Skype-ს გასაშვებად საჭიროა ფაილებთან მუშაობის გაფართოების დაყენება.';ru='Для запуска Skype необходимо установить расширение работы с файлами.';en='To start Skype you must install the extension to work with files.'");
...
Процедура ПозвонитьПоТелефону(НомерТелефона)
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ДоступноеИмяПротокола = Неопределено Тогда
			СтрокаСПредупреждением = Новый ФорматированнаяСтрока(
					НСтр("ka='ზარის განსახორციელებლად საჭიროა ტელეფონიის პროგრამის დაყენება, მაგალითად';ru='Для совершения звонка требуется установить программу телефонии, например';en='To make a call you want to install telephony program, for example'"),
	...
	ТекстСообщения = НСтр("ka='ზარის განსახორციელებლად აუცილებელია ფაილებთან მუშაობის გაფართოების დაყენება.';ru='Для совершения звонка необходимо установить расширение работы с файлами.';en='To make a call you need to install the extension to work with files.'");
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Оповещение, ТекстСообщения);
...
Процедура ЗаполнитьАдрес(Знач Форма, Знач ИмяРеквизита, Знач НайденнаяСтрока, Знач Результат)
	...
	Если АдресаДляЗаполнения.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='იმისთვის, რომ ერთი და იგივე მისამართი რამდენიმიჯერ არ შევიყვანოთ, შეიძლება ამ ველში იმ მნიშვნელობის კოპირება, რომელიც უწინ მეზობელ ველში იყო შეყვანილი.';ru='Для того чтобы не вводить один и тот же адрес несколько раз, можно скопировать в это поле значение, ранее введенное в соседнем поле.';en='In order to avoid entering the same address several times, it can be copied to this field value, previously entered in the adjacent field.'")
			,, НСтр("ka='მისამართის კოპირება';ru='Копирование адреса';en='copy address'"));
		Возврат;
	КонецЕсли;
...
Процедура ВвестиКомментарий(Знач Форма, Знач ИмяРеквизита, Знач НайденнаяСтрока, Знач Результат)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Комментарий, 
		НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'"));
КонецПроцедуры
...
Функция ПобитовоеИ_Доп(БинарныйМассив1, БинарныйМассив2)
	Результат = Новый Массив;
...
Функция ПобитовоеИли_Доп(БинарныйМассив1, БинарныйМассив2)
	Результат = Новый Массив;
	Для Индекс = 0 По БинарныйМассив1.Количество()-1 Цикл
