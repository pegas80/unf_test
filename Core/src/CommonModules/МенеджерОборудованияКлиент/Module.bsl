...
Процедура ВыполнитьНастройкуОборудования(Идентификатор, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დრაივერის გამართვის ფორმის ინიციალიზაციის შეცდომა მოხდა.';ru='Произошла ошибка инициализации формы настройки драйвера.';en='An initialization error driver settings form.'")); 
	КонецЕсли;
...
Процедура ВыполнитьНастройкуОборудования_Завершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Иначе
			СообщениеОбОшибке = НСтр("ka='მოწყობილობის პარამეტრების შენახვა ვერ მოხერხდა.';ru='Не удалось сохранить параметры устройства.';en='Unable to save the parameters of the device.'");
...
Процедура ПредложитьВыбратьУстройство(ОповещениеВыбора, ТипОборудования, ТекстЗаголовкаВыбора, 
	...
	Если Не ОбновитьРабочееМестоКлиента() Тогда
		ТекстСообщения = НСтр("ka='წინასწარ აუცილებელია მიმდინარე სეანსის შესაერთებელი მოწყობილობის სამუშაო ადგილის არჩევა.';ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';en='You must first choose a workplace connected equipment current session.'");
	...
	Иначе
		//Малышев 2024-08-08 - ONESOFT-3343 (--- убрать устройство с баллами
		ЗапроситьОтметкуЭлементов = (ТекстЗаголовкаВыбора = НСтр("ru = 'Выберите эквайринговый терминал'; ka = 'აირჩიეთ ეკვაირინგის ტერმინალი'; en = 'Select acquiring terminal'")); 
			
		СписокУстройств = Новый СписокЗначений();
		Для Каждого Устройства Из СписокДоступныхУстройств Цикл

			//Малышев 2024-08-08 - ONESOFT-3343 (--- убрать устройство с баллами
			Если ЗапроситьОтметкуЭлементов
				и Устройства.Параметры.Свойство("СкрытьИзСпискаВыбора") 
				и Устройства.Параметры.СкрытьИзСпискаВыбора = Истина Тогда
				Продолжить;
			КонецЕсли;	
			//Малышев 2024-08-08 - ONESOFT-3343 --)
		...
		Иначе
			...
			Если ЗапроситьОтметкуЭлементов Тогда 
			//Если ТекстЗаголовкаВыбора = НСтр("ru = 'Выберите эквайринговый терминал'; ka = 'აირჩიეთ ეკვაირინგის ტერმინალი'; en = 'Select acquiring terminal'") Тогда 
				//СписокУстройств.ПоказатьВыборЭлемента(ОписаниеОповещения, ТекстЗаголовкаВыбора);
				Для Каждого Строк Из СписокУстройств Цикл 
					Строк.Пометка = Истина;
				КонецЦикла;
				СписокУстройств.ПоказатьОтметкуЭлементов(ОписаниеОповещения, ТекстЗаголовкаВыбора);
			Иначе
				СписокУстройств.ПоказатьВыборЭлемента(ОписаниеОповещения, ТекстЗаголовкаВыбора);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПредложитьВыбратьУстройствоЗавершение(Результат, Параметры) Экспорт
	...
	Если Параметры.СледующееОповещение <> Неопределено Тогда
		...
		Если ТипЗнч(Результат) = Тип("СписокЗначений") Тогда 
			...
			Для Каждого Строк Из Результат Цикл 
				Если Строк.Пометка Тогда 
					//МассивТерминалов.Добавить(Строк.Значение);
					Идентификатор = ?(Строк = Неопределено, Неопределено, Строк.Значение);
					ВыполнитьОбработкуОповещения(Параметры.СледующееОповещение, Идентификатор);
				КонецЕсли;
			КонецЦикла;
			//ВыполнитьОбработкуОповещения(Параметры.СледующееОповещение, МассивТерминалов);
		Иначе
			Идентификатор = ?(Результат = Неопределено, Неопределено, Результат.Значение);
			ВыполнитьОбработкуОповещения(Параметры.СледующееОповещение, Идентификатор);
		КонецЕсли;
...
Процедура НачатьВыборФайлаДрайвера(ОповещениеПриВыборе) Экспорт 
	...
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ დრაივერის ფაილი';ru='Выберите файл драйвера';en='Select the driver file'");
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='დრაივერის ფაილი';ru='Файл драйвера';en='driver file'") + ?(МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент(), "(*.zip)|*.zip", "(*.zip, *.exe)| *.zip; *.exe");  
...
Процедура НачатьПодключениеОборудования(ОповещениеПриПодключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если Не Результат Тогда
		Если ОповещениеПриПодключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='წინასწარ აუცილებელია მიმდინარე სეანსის შესაერთებელი მოწყობილობის სამუშაო ადგილის არჩევა.';ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';en='You must first choose a workplace connected equipment current session.'");
	...
	Если ТипЗнч(ИдентификаторУстройства) = Тип("Массив") Тогда 
		Для каждого ЭлемИдн Из ИдентификаторУстройства Цикл 
			МассОборудования = МенеджерОборудованияКлиентПовтИсп.ПолучитьСписокОборудования(ТипыПО, ЭлемИдн);
			Для Каждого ЭлемОборуд Из МассОборудования Цикл 
				СписокОборудования.Добавить(ЭлемОборуд);
			КонецЦикла;
		КонецЦикла;
	Иначе
		СписокОборудования = МенеджерОборудованияКлиентПовтИсп.ПолучитьСписокОборудования(ТипыПО, ИдентификаторУстройства);
		...
		Для Каждого Устройство Из СписокОборудования Цикл
			...
			Если ПодключенноеУстройство = Неопределено Тогда // Если устройство не было подключено ранее.
				...
				Если ОбработчикДрайвераМодуль = Неопределено Тогда
					...
					Если ОповещениеПриПодключении <> Неопределено Тогда
						ОписаниеОшибки = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.';en='Unable to connect the driver handler.'");
					...
					Иначе
						...
						Если ОбъектДрайвера = Неопределено Тогда
							Если ОповещениеПриПодключении <> Неопределено Тогда
								...
								ОписаниеОшибки = НСтр("ka='%Наименование%: მოწყობილობის დრაივერის ჩატვირთვა ვერ მოხერხდა."
"შეამოწმეთ, არის თუ არა დრაივერი კორექტულად დაყენებული და სისტემაში რეგისტრირებული.';ru='%Наименование%: Не удалось загрузить драйвер устройства."
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
						...
						Иначе
							...
							Если Результат Тогда
								...
								Если ВыходныеПараметры.Количество() >= 2 Тогда
									...
									Если ВыходныеПараметры.Количество() >= 5 Тогда 
										НовоеПодключение.Вставить("мRequest", ВыходныеПараметры[2]);
										НовоеПодключение.Вставить("мМассив",  ВыходныеПараметры[3]);
										НовоеПодключение.Вставить("млЭС",  	  ВыходныеПараметры[4]);
									КонецЕсли;	
								...
								Если ОповещениеПриПодключении <> Неопределено Тогда
									ОписаниеОшибки = НСтр("ka='შეცდომები არ არის.';ru='Ошибок нет.';en='There are no errors.'");
							...
							Иначе
								...
								Если ОповещениеПриПодключении <> Неопределено Тогда
									ОписаниеОшибки = НСтр("ka='ვერ მოხერხდა მოწყობილობის შეერთება ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';en='Unable to connect the device ""% name%"":% OpisanieOshibki% (% KodOshibki%)'");
									ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , НовоеПодключение.Наименование);
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено И ОповещениеПриПодключении <> Неопределено Тогда
		ОписаниеОшибки =  НСтр("ka='შერჩეული მოწყობილობის გამოყენება შეერთებისთვის შეუძლებელია. მიუთითეთ სხვა მოწყობილობა.';ru='Выбранное устройство не может использоваться для подключения. Укажите другое устройство.';en='The selected device can not be used for the connection. Specify a different device.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриПодключении, РезультатВыполнения);
	ИначеЕсли ТипыПО = Неопределено И ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки =  НСтр("ka='შესაერთებლად ხელმისაწვდომი მოწყობილობა არ არის.';ru='Нет доступного оборудования для подключения.';en='No equipment available to connect.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриПодключении, РезультатВыполнения);
	КонецЕсли;
...
Процедура НачатьПодключениеОборудования_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		// Сообщим пользователю о том, что не удалось подключить устройство.
		Если Параметры.ОповещениеПриПодключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки%';en='Unable to connect the device ""% name%"":% OpisanieOshibki%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , Параметры.НовоеПодключение.Наименование);
						...
						Иначе
							...
							Иначе
								...
								Если НЕ Результат Тогда
									// Сообщим пользователю о том, что не удалось подключить устройство.
									Если ОповещениеПриОтключении <> Неопределено Тогда
										ОписаниеОшибки = НСтр("ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки%';en='When you unplug the device ""% name%"" error:%% OpisanieOshibki'");
										ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , ПодключенноеУстройство.Наименование);
										ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
										РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
										ВыполнитьОбработкуОповещения(ОповещениеПриОтключении, РезультатВыполнения);
...
Процедура НачатьОтключениеОборудование_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		// Сообщим пользователю о том, что не удалось подключить устройство.
		Если Параметры.ОповещениеПриОтключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='მოწყობილობა ""%Наименование%""-ის გამორთვისას შეცდომა მოხდა.';ru='При отключении устройства ""%Наименование%"" произошла ошибка.';en='When you unplug the device ""% name%"" An error has occurred.'");
...
Процедура НачатьОтключениеВсегоОборудованияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='მოწყობილობის გამორთვისას შეცდომა მოხდა: ""%ОписаниеОшибки%"".';ru='При отключении оборудования произошла ошибка: ""%ОписаниеОшибки%"".';en='When you disconnect the equipment error occurred: ""% OpisanieOshibki%.""'");
			...
			Если ПодключенноеУстройство = Неопределено Тогда // Если устройство не было подключено ранее.
				...
				Если ОбъектДрайвера = Неопределено Тогда
					...
					ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
								   + НСтр("ka='%Наименование%: მოწყობილობის დრაივერის ჩატვირთვა ვერ მოხერხდა."
"შეამოწმეთ, არის თუ არა დრაივერი კორექტულად დაყენებული და სისტემაში რეგისტრირებული.';ru='%Наименование%: Не удалось загрузить драйвер устройства."
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
				...
				Если ОбработчикДрайвераМодуль = Неопределено Тогда
					// Сообщить об ошибке, что не удалось загрузить драйвер.
					ОписаниеОшибки = ОписаниеОшибки +  НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.';en='Unable to connect the driver handler.'");
					КонечныйРезультат = Ложь;
					Продолжить;
				Иначе
				...
				Иначе
					// Сообщим пользователю о том, что не удалось подключить устройство.
					ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
								   + НСтр("ka='ვერ მოხერხდა მოწყობილობის შეერთება ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';en='Unable to connect the device ""% name%"":% OpisanieOshibki% (% KodOshibki%)'");
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , Устройство.Наименование);
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%"     , ВыходныеПараметры[0]);
				КонецЕсли;
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='შერჩეული მოწყობილობის გამოყენება შეერთებისთვის შეუძლებელია. მიუთითეთ სხვა მოწყობილობა.';ru='Выбранное устройство не может использоваться для подключения."
			...
			Если КлиентПодключения <> Неопределено  И (ТипыПО = Неопределено Или ТипыПО.Найти(ПодключенноеУстройство.ТипОборудованияИмя) <> Неопределено)
			   И (ИдентификаторУстройства = Неопределено  Или ПодключенноеУстройство.Ссылка = ИдентификаторУстройства)Тогда
				 ...
				 Если ПодключенноеУстройство.КоличествоПодключенных = 1 Тогда
					...
					Если ПодключенноеУстройство.ОбъектДрайвера = Неопределено Тогда
						// Сообщить об ошибке, что не удалось загрузить драйвер.
						ОписаниеОшибки = НСтр("ka='""%Наименование%"": მოწყობილობის დრაივერი ვერ ჩაიტვირთა."
"შეამოწმეთ, დრაივერი კორექტულად არის თუ არა დაყენებული და სისტემაში რეგისტრირებული.';ru='""%Наименование%"": Не удалось загрузить драйвер устройства."
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%", ПодключенноеУстройство.Наименование);
						КонечныйРезультат = Ложь;
						Продолжить;
					...
					Если НЕ Результат Тогда
						ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
									   + НСтр("ka='მოწყობილობა ""%Наименование%"" გამორთვისას მოხდა შეცდომა: %ОписаниеОшибки% (%КодОшибки%)';ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки% (%КодОшибки%)';en='When you unplug the device ""% name%"" error:% OpisanieOshibki% (% KodOshibki%)'");
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%", ПодключенноеУстройство.Наименование);
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%", ВыходныеПараметры[0]);
					Иначе
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
		...
		Иначе
			...
			Если ОбработчикДрайвераМодуль = Неопределено Тогда
				ТекстСообщения = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.';en='Unable to connect the driver handler.'");
				ВыходныеПараметры = Новый Массив();
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(ТекстСообщения);
				ВыходныеПараметры.Добавить(НСтр("ka='არ არის დაყენებული';ru='Не установлен';en='Not installed'"));
			Иначе
				// Принудительно заменяем асинхронный обработчик на синхронный, так как работам в синхронном режиме.
				Если ОбработчикДрайвераМодуль = ПодключаемоеОборудованиеУниверсальныйДрайверАсинхронноКлиент Тогда
					ОбработчикДрайвераМодуль = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
	...
	Иначе
		...
		ВыходныеПараметры = Новый Массив();
		ТекстСообщения = НСтр("ka='მოწყობილობა არ არის ჩართული. ოპერაციის შესრულების წინ მოწყობილობა უნდა ჩაირთოს.';ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.';en='The apparatus is not connected. Before operation the device must be connected.'");
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
	КонецЕсли;
...
Функция ВыполнитьДополнительнуюКоманду(Команда, ВходныеПараметры, ВыходныеПараметры, Идентификатор, Параметры) Экспорт
	...
	Если ПодключенноеУстройство = Неопределено Тогда
		...
		Если ОбъектДрайвера = Неопределено Тогда
			// Сообщить об ошибке, что не удалось загрузить драйвер.
			ТекстСообщения = НСтр("ka='ვერ მოხერხდა მოწყობილობის დრაივერის შეერთება. შეამოწმეთ, კორექტულად თუ არის დრაივერი დაყენებული და დარეგისტრირებული სისტემაში.';ru='Не удалось загрузить драйвер устройства."
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
			ВыходныеПараметры = Новый Массив();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(ТекстСообщения);
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='მოწყობილობა ჩართულია. ოპერაციის შესრულების წინ მოწყობილობა გამორთული უნდა იყოს.';ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.';en='The device is connected. Before operation the device must be turned off.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
		ВыходныеПараметры.Добавить(НСтр("ka='დაყენებულია';ru='Установлен';en='Set'"));
...
Процедура ПодключитьОборудованиеЗавершениеПоУмолчанию(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='მოწყობილობის შეერთებისას შეცდომა მოხდა:""%ОписаниеОшибки%"".';ru='При подключении оборудования произошла ошибка:""%ОписаниеОшибки%"".';en='When connected to equipment error occurred: ""% OpisanieOshibki%.""'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%" , РезультатВыполнения.ОписаниеОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
"Файл ""%Файл%"" в макете не найден.';ka='მაკეტში დისტრიბუტივიდან დრაივერის დაყენების შეცდომა."
"ფაილი ""%Файл%"" მაკეტში არ მოიძებნა.'");
...
"Необходимый файл в архиве не найден.';ka='დისტრიბუტივიდან არქივში დრაივერის დაყენების შეცდომა."
"საჭირო ფაილი არქივში ვერ მოიძებნა.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки); 
...
Процедура НачатьПодключениеРасширенияРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Не Подключено И ДополнительныеПараметры.ПредлагатьУстановку Тогда
		...
		ТекстСообщения = НСтр("ka='მუშაობის გასაგრძელებლად აუცილებელია ვებ-კლიენტ ""1С:საწარმო""-სთვის გაფართოების დაყენება. დავაყენოთ?';ru='Для продолжении работы необходимо установить расширение для веб-клиента ""1С:Предприятие"". Установить?';en='To continue, you must install the extension for the Web client ""1C: Enterprise"". Set?'");
		...
		Попытка
			...
			Иначе
				...
				Иначе
					...
					КонецЕсли;
				КонецЕсли;
				Если ProgID2 = "BDO_DatecsDP25.Datecs" Тогда
					ОбъектДрайвера = Новый COMОбъект(ProgID2);					
				Иначе					
...
Процедура УстановитьДрайвер(Идентификатор, ОповещениеИзДистрибутиваПриЗавершении = Неопределено, ОповещениеИзАрхиваПриЗавершении = Неопределено) Экспорт
	...
	Попытка  
		Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
			...
			#Если ВебКлиент Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოცემული დრაივერი ვებ-კლიენტში მუშაობას მხარს არ უჭერს.';ru='Данный драйвер не поддерживает работу в веб-клиенте.';en='This driver does not support the Web client.'")); 
			#Иначе
				Если МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент() Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოცემული დრაივერის დაყენება და გამოყენება Linux-ის გარემოში ვერ მოხდება.';ru='Данный драйвер не может быть установлен и использован в среде Linux.';en='This driver can not be installed and used in a Linux environment.'")); 
					Возврат;
				КонецЕсли;
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დრაივერის დაყენებისას შეცდომა მოხდა.';ru='Произошла ошибка при установке драйвера.';en='An error occurred while installing the driver.'")); 
	КонецПопытки;  
...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, Идентификатор, Команда, ВходныеПараметры) Экспорт
	...
	Если ПодключенноеУстройство <> Неопределено Тогда
		...
		Если ОбработчикДрайвераМодуль = Неопределено Или ОбъектДрайвера = Неопределено Тогда
			// Сообщить об ошибке, что не удалось загрузить драйвер.
			ТекстОшибки = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ან დრაივერის ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера или загрузить драйвер.';en='Unable to connect the processor driver or download a driver.'");
	...
	Иначе
		// Сообщить об ошибке, что устройство не подключено.
		ТекстОшибки = НСтр("ka='მოწყობილობა არ არის ჩართული. ოპერაციის შესრულების წინ მოწყობილობა უნდა ჩაირთოს.';ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.';en='The apparatus is not connected. Before operation the device must be connected.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании();
		РезультатВыполнения.Результат = Ложь;
		РезультатВыполнения.ОписаниеОшибки = ТекстОшибки; 
		ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатВыполнения);
...
Процедура НачатьВыполнениеДополнительнойКоманды(ОповещениеПриЗавершении, Команда, ВходныеПараметры, Идентификатор, Параметры) Экспорт
	...
	Иначе
		// Сообщить об ошибке, что устройство подключено.
		ТекстОшибки = НСтр("ka='მოწყობილობა ჩართულია. ოპერაციის შესრულების წინ მოწყობილობა გამორთული უნდა იყოს.';ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.';en='The device is connected. Before operation the device must be turned off.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
...
Процедура НачатьВыполнениеДополнительнойКоманды_Завершение(ОбъектДрайвера, ПараметрыКоманды) Экспорт
	...
	Если ОбъектДрайвера = Неопределено Тогда
		// Сообщить об ошибке, что не удалось загрузить драйвер.
		ТекстОшибки = НСтр("ka='ვერ მოხერხდა მოწყობილობის დრაივერის შეერთება. შეამოწმეთ, კორექტულად თუ არის დრაივერი დაყენებული და დარეგისტრირებული სისტემაში.';ru='Не удалось загрузить драйвер устройства."
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстОшибки);
	...
	Иначе
		...
		Если ОбработчикДрайвераМодуль = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.';en='Unable to connect the driver handler.'");
			ВыходныеПараметры = Новый Массив();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(ТекстОшибки);
			ВыходныеПараметры.Добавить(НСтр("ka='არ არის დაყენებული';ru='Не установлен';en='Not installed'"));
		...
		Иначе
			...
			времПараметрыПодключения.Вставить("ИДУстройства", "");  //Малышев 2025-09-25 ONESOFT-4381
			Если ОбработчикДрайвераМодуль.ПоддержкаАсинхронногоРежима() Тогда
...
Функция ОбработатьСобытиеОтУстройства(ОписаниеСобытия, ОписаниеОшибки = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(глПодключаемоеОборудование) Тогда
		Возврат Результат;
	КонецЕсли;
	// Олег Литвиненко <---
...
Процедура НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ფისკალური მოწყობილობა არ არის არჩეული ან შეერთებული.';ru='Фискальное устройство не выбрано или не подключено.';en='Fiscal device is not selected or is not connected.'");
	...
	Иначе
		...
		Если ТребуетсяПроверкаАктивностиСмены И НЕ КассовыеСменыВызовСервера.ПоследняяСменаАктивна(ИдентификаторУстройства) Тогда
			ОписаниеОшибки = НСтр("ru='Кассовая смена не открыта или истекла.';en='Cash change is not open or expired.'");
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
			Возврат;
		КонецЕсли;
...
Процедура НачатьВыполнениеКоманды_ПослеПодтвержденияЗакрытияСмены(Результат, Параметры) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='На фискальном устройстве смена не закрыта';ru='На фискальном устройстве смена не закрыта';en='In fiscal device change is not closed'");
...
Процедура НачатьВыполнениеКоманды_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='მოწყობილობის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении оборудования произошла ошибка: %ДополнительноеОписание%';en='When connected to equipment error occurred:% DopolnitelnoeOpisanie%'");
...
Процедура НачатьВыполнениеКоманды_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='შეცდომები არ არის.';ru='Ошибок нет.';en='There are no errors.'");
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ka='ოპერაციის შესრულებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При выполнении операции произошла ошибка: %ДополнительноеОписание%';en='In an operation error:%% DopolnitelnoeOpisanie'");
...
Процедура НачатьФормированиеОтчетаОТекущемСостоянииРасчетов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ka='აირჩიეთ ფისკალური მოწყობილობა';ru='Выберите фискальное устройство';en='Select a fiscal device'"),
			НСтр("ka='ფისკალური მოწყობილობა არ არის შეერთებული.';ru='Фискальное устройство не подключено.';en='Fiscal device is not connected.'"), 
			НСтр("ka='ფისკალური მოწყობილობა არ არის არჩეული.';ru='Фискальное устройство не выбрано.';en='Fiscal device is not selected.'"),
			Истина);
	Иначе
...
Процедура НачатьЗакрытиеСменыНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено, КассоваяСмена = Неопределено) Экспорт
	...
	Если КассоваяСмена = Неопределено Тогда
		...
		Если ИдентификаторУстройства = Неопределено Тогда
			...
			ПараметрыВыбора.Вставить("Заголовок", НСтр("ru='Выберите кассовую смену для закрытия';en='Select cash change to close'"));
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					Если ОповещениеПриЗавершении <> Неопределено Тогда
						ТекстСообщения = НСтр("ru='Кассовая смена была закрыта ранее';en='Cash change was closed earlier'");
						РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ТекстСообщения, ИдентификаторУстройства);
						ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатОперации);
					КонецЕсли;
...
Процедура НачатьФискализациюЧекаНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено,
	...
	Иначе
		...
		Если ОповещениеПриЗавершении.ДополнительныеПараметры.Свойство("ДанныеОплаты") 
			И ОповещениеПриЗавершении.ДополнительныеПараметры.ДанныеОплаты.Свойство("Наличными")  
			И ОповещениеПриЗавершении.ДополнительныеПараметры.ДанныеОплаты.Наличными > 0 ТОгда
			
			ВходныеПараметры  = Неопределено;
			ВыходныеПараметры = Неопределено;
			ПараметрыУстройства = МенеджерОборудованияКлиентПовтИсп.ПолучитьДанныеУстройства(ИдентификаторУстройства).Параметры;
			
			Если ПараметрыУстройства.Свойство("P_OpenCashDrawerAfterDocumentCashOnly") и ПараметрыУстройства.P_OpenCashDrawerAfterDocumentCashOnly Тогда
				//Сообщить("Сумма налички " + ОповещениеПриЗавершении.ДополнительныеПараметры.ДанныеОплаты.Наличными + ". Открываем ящик");
				ОповещениеПослеОткрытия = Новый ОписаниеОповещения("ТестУстройстваЗавершение", ЭтотОбъект);
				МенеджерОборудованияКлиент.НачатьВыполнениеДополнительнойКоманды(ОповещениеПослеОткрытия, "OpenCashDrawer", ВходныеПараметры, ИдентификаторУстройства, ПараметрыУстройства);
			КонецЕсли;
		КонецЕсли;
		//Малышев 2025-09-25 ONESOFT-4381 --)
...
&НаКлиенте
Процедура ТестУстройстваЗавершение(РезультатВыполнения, Параметры) Экспорт  //Малышев 2025-09-25 ONESOFT-4381
...
Процедура НачатьПечатьТекста(ОповещениеПриЗавершении, УникальныйИдентификатор, Текст, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	//+++ Дмгг
	Если ОповещениеПриЗавершении.ДополнительныеПараметры <>Неопределено и //Малышев 2024-08-01
		ОповещениеПриЗавершении.ДополнительныеПараметры.Свойство("ПараметрыОперации") Тогда 
		Если ОповещениеПриЗавершении.ДополнительныеПараметры.ПараметрыОперации.Свойство("ЭтоПечатьОшибки") Тогда
			Контекст.Вставить("ЭтоПечатьОшибки"      , Истина);
		КонецЕсли;
	КонецЕсли;
	//--- Дмгг
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ka='აირჩიეთ საბეჭდი მოწყობილობა';ru='Выберите печатающее устройство';en='Select the printing device'"), 
			НСтр("ka='საბეჭდი მოწყობილობა არ არის შეერთებული.';ru='Печатающее устройство не подключено.';en='The printing apparatus is not connected.'"), 
			НСтр("ka='საბეჭდი მოწყობილობა არ არის არჩეული.';ru='Печатающее устройство не выбрано.';en='The printing device is not selected.'"), 
			Истина);
	Иначе
		НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Контекст);
	КонецЕсли;
...
Функция СформироватьТексНефискальногоЧека(ШиринаСтроки, ТипДокумента, ВходныеПараметры, ДополнительныйТекст = Неопределено) Экспорт
	...
	Если ТипДокумента = 0 Тогда
		...
		ТипыЧеков.Вставить(1, НСтр("ka='გაყიდვა';ru='ПРОДАЖА';en='SALE'"));       
		ТипыЧеков.Вставить(2, НСтр("ka='დაბრუნება';ru='ВОЗВРАТ';en='RETURN'"));  
		ТипыЧеков.Вставить(3, НСтр("ru='ПОКУПКА';en='PURCHASE'"));  
		ТипыЧеков.Вставить(4, НСтр("ru='ВОЗВРАТ ПОКУПКИ';en='RETURN OF PURCHASE'"));  
	...
	ИначеЕсли ТипДокумента = 1 Тогда
		ТипЧека = НСтр("ru='ОТКРЫТИЕ СМЕНЫ';en='OPENING OF CHANGE'");  
	ИначеЕсли ТипДокумента = 2 Тогда
		ТипЧека = НСтр("ka='შეტანა';ru='ВНЕСЕНИЕ';en='MAKING'");  
	ИначеЕсли ТипДокумента = 3 Тогда
		ТипЧека = НСтр("ka='ამოღება';ru='ВЫЕМКА';en='extraction'");  
	ИначеЕсли ТипДокумента = 4 Тогда
		ТипЧека = НСтр("ka='ანგარიშგება დაფარვის გარეშე';ru='ОТЧЕТ БЕЗ ГАШЕНИЯ';en='Report without slaking'");  
	ИначеЕсли ТипДокумента = 5 Тогда
		ТипЧека = НСтр("ka='ანგარიშგება დაფარვით';ru='ОТЧЕТ С ГАШЕНИЕМ';en='REPORT with cancel'");  
	КонецЕсли;
	...
	Если ПечатьКлише Тогда
		...
		НомерКассы = ?(ОбщиеПараметры.Свойство("НомерКассы") И НЕ ПустаяСтрока(ОбщиеПараметры.НомерКассы), 
			НСтр("ka='სალარო';ru='КАССА';en='CASHBOX'") + Символы.НПП + ОбщиеПараметры.НомерКассы, "");
	
		ОрганизацияИНН = ?(ОбщиеПараметры.Свойство("ОрганизацияИНН") И НЕ ПустаяСтрока(ОбщиеПараметры.ОрганизацияИНН), 
			НСтр("ka='ჩეკი №';ru='ЧЕК №';en='CHECK number'") + Символы.НПП + ОбщиеПараметры.ОрганизацияИНН, "");
		...
		НомерЧека = ?(ОбщиеПараметры.Свойство("НомерЧека") И НЕ ПустаяСтрока(ОбщиеПараметры.НомерЧека),
			НСтр("ka='ჩეკი №';ru='ЧЕК №';en='CHECK number'") + ОбщиеПараметры.НомерЧека, "");
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если Окр(Количество * Цена, 2) > Сумма Тогда
					ТекстСтроки = ТекстСтроки + ПостроитьПоле(НСтр("ka='ფასდაკლება =';ru='СКИДКА =';en='DISCOUNT ='") + Символы.НПП
									+ Формат(Количество * Цена - Сумма, ФорматЧисла) + " ", ШиринаСтроки, , Ложь)  + Символы.ПС;
				ИначеЕсли Окр(Количество * Цена, 2) < Сумма Тогда
					ТекстСтроки = ТекстСтроки + ПостроитьПоле(НСтр("ka='დანამატი =';ru='НАДБАВКА =';en='SURCHARGE ='") + Символы.НПП
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПоле(НСтр("ka='ჯამი =';ru='ИТОГ =';en='TOTAL ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаЧека, ФорматЧисла) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		Если СуммаНаличнойОплаты > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='ნაღდი ფულით =';ru='НАЛИЧНЫМИ =';en='CASH ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаНаличнойОплаты, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты1 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='საგადახდო ბარათით =';ru='ПЛАТ.КАРТОЙ =';en='PLAT.KARTOY ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты1, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты2 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='კრედიტით =';ru='КРЕДИТОМ =';en='CREDIT ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты2, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты3 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='სერთიფიკატით =';ru='СЕРТИФИКАТОМ =';en='CERTIFICATE ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты3, ФорматЧисла) + Символы.ПС;
		КонецЕсли;          
		
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПоле(НСтр("ka='ჩაბარება =';ru='СДАЧА =';en='DELIVERY ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаОплаты - СуммаЧека, ФорматЧисла) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		Кассир = ?(ОбщиеПараметры.Свойство("Кассир"), ОбщиеПараметры.Кассир, "");
		Текст = Текст + ПостроитьПоле(Символы.НПП + НСтр("ka='მოლარე';ru='КАССИР';en='CASHIER'") + ":" + Символы.НПП + Кассир, ШиринаСтроки) + Символы.ПС;                        
		Текст = Текст + ПостроитьПоле(НСтр("ka='ხელმოწერა';ru='ПОДПИСЬ';en='SIGNATURE'") + ":", ШиринаСтроки, "_") + Символы.ПС + Символы.ПС;
...
Функция СформироватьШаблонИнфоКвитанции(Параметры, ШиринаСтроки, ПечатьШтрихкода = Истина) Экспорт
	...
	ОрганизацияИНН = ?(НЕ ПустаяСтрока(Параметры.ИНН), НСтр("ka='ს.კ / პ.ნ:';ru='ИНН:';en='INN:'") + Параметры.ИНН, "");
	ОрганизацияКПП = ?(НЕ ПустаяСтрока(Параметры.КПП), НСтр("ka='КПП:';ru='КПП:';en='Transmission:'") + Параметры.КПП, "");
	...
	НомерКассы = ?(НЕ ПустаяСтрока(Параметры.НомерКассы), НСтр("ka='КАССА:';ru='КАССА:';en='CASHBOX:'") + Параметры.НомерКассы, "");
	НомерСмены = ?(НЕ ПустаяСтрока(Параметры.НомерСмены), НСтр("ka='СМЕНА:';ru='СМЕНА:';en='CHANGE:'") + Формат(Параметры.НомерСмены, "ЧГ=0"), 0);
	...
	НомерЧека = ?(НЕ ПустаяСтрока(Параметры.НомерЧека), НСтр("ka='ЧЕК:';ru='ЧЕК:';en='CHECK:'") + Параметры.НомерЧека, "");
	ДатаВремя = НСтр("ka='ДАТА:';ru='ДАТА:';en='DATE:'") + Формат(ТекущаяДата(), "ДФ=""дд.ММ.гггг ЧЧ:мм""");
...
Функция ПолучитьШаблонАтрибутовЧекаДляОтсылки(РегистрационныйНомерККТ, ЗаводскойНомерФН, ФискальныйПризнак, СуммаЧека, АдресСайтаПроверки, ДатаСеанса) Экспорт 
	...
	ТекстСообщения  = НСтр("ru='ККТ№:';en='KKT№:'") + РегистрационныйНомерККТ + ";" + Символы.НПП + 
		НСтр("ru='СУММА:';en='TOTAL:'") + Формат(СуммаЧека, "ЧРД=,;ЧЦ=10;ЧДЦ=2;ЧН=0,00;ЧГ=0") + ";" + Символы.НПП + 
		Формат(ДатаСеанса, "ДФ:""дд.ММ.гггг ЧЧ:мм""") + ";" + Символы.НПП + 
		НСтр("ru='ФН№:';en='FN№:'") + ЗаводскойНомерФН + ";" + Символы.НПП + 
		НСтр("ru='ФПД:';en='PF:'") + ФискальныйПризнак + ";" + Символы.НПП + 
...
Процедура НачатьПодключенияЭквайринговогоТерминала(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройстваЭТ = Неопределено, ИдентификаторУстройстваПУ = Неопределено) Экспорт
	...
	Если Контекст.ИдентификаторУстройстваЭТ = Неопределено
		//--------------------------------------------------------------------------
		// © Шишка В.В. [12 октября 2021]  НАЧАЛО
		//И ТипЗнч(Контекст.ОповещениеПриЗавершении.ДополнительныеПараметры.ОповещениеПриЗавершении.Модуль) = Тип("УправляемаяФорма")
		//И Контекст.ОповещениеПриЗавершении.ДополнительныеПараметры.ОповещениеПриЗавершении.Модуль.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаОплаты"
		//И НЕ Контекст.ОповещениеПриЗавершении.ДополнительныеПараметры.ОповещениеПриЗавершении.Модуль.ИспользоватьНовуюФормуОплат	
		// © Шишка В.В. [12 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ЭквайринговыйТерминал",
			НСтр("ka='აირჩიეთ ეკვაირინგის ტერმინალი';ru='Выберите эквайринговый терминал';en='Select acquiring terminal'"), 
			НСтр("ka='ეკვაირინგის ტერმინალი არ არის შეერთებული.';ru='Эквайринговый терминал не подключен.';en='Acquiring the terminal is not connected.'"),
			НСтр("ka='ეკვაირინგის ტერმინალი არ არის არჩეული';ru='Эквайринговый терминал не выбран.';en='Acquiring the terminal has not been selected.'"), 
			Истина);
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьУстройствоЗавершение(ИдентификаторУстройстваЭТ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваЭТ = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ეკვაირინგის ტერმინალი არ არის არჩეული';ru='Эквайринговый терминал не выбран.';en='Acquiring the terminal has not been selected.'");
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		// Ошибка подключения.
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ტერმინალის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении терминала произошла ошибка: %ДополнительноеОписание%';en='An error occurred when connecting the terminal:% DopolnitelnoeOpisanie%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПечатьНаТерминалеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ПечатьКвитанцийНаТерминале Тогда
			...
			КонецЕсли;
		Иначе   
			ПечатноеУстройствоЭквТерминала = ОбщегоНазначенияВызовСервера.ПоискПУ_ВЭквайреновомТерминале(Параметры.ИдентификаторУстройстваЭТ);//Справочники.ЭквайринговыеТерминалы.НайтиПоРеквизиту("ПодключаемоеОборудование", Параметры.ИдентификаторУстройстваЭТ)
			//Если Параметры.ИдентификаторУстройстваПУ = Неопределено Тогда
			Если Параметры.ИдентификаторУстройстваПУ = Неопределено И ПечатноеУстройствоЭквТерминала = Неопределено Тогда
				ПоддерживаемыеТипыВО = Новый Массив();
				ПоддерживаемыеТипыВО.Добавить("ФискальныйРегистратор");
				ПоддерживаемыеТипыВО.Добавить("ПринтерЧеков");
				ПоддерживаемыеТипыВО.Добавить("ККТ");
				...
				ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
					НСтр("ka='აირჩიეთ საბეჭდი მოწყობილობა';ru='Выберите печатающее устройство';en='Select the printing device'"), НСтр("ka='საბეჭდი მოწყობილობა არ არის შეერთებული.';ru='Печатающее устройство не подключено.';en='The printing apparatus is not connected.'"), НСтр("ka='საბეჭდი მოწყობილობა არ არის არჩეული.';ru='Печатающее устройство не выбрано.';en='The printing device is not selected.'"), Истина);
			///+++ Дмгг		
			ИначеЕсли ПечатноеУстройствоЭквТерминала <> Неопределено Тогда	
				НачатьПодключенияЭквайринговогоТерминала_ВыбратьПечатающегоУстройстваЗавершение(ПечатноеУстройствоЭквТерминала, Параметры);	
			Иначе
	...
	Иначе
		...
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ტერმინალის გამოყენებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При использовании терминала произошла ошибка: %ДополнительноеОписание%';en='When using the terminal error has occurred:% DopolnitelnoeOpisanie%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьПечатающегоУстройстваЗавершение(ИдентификаторУстройстваПУ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваПУ = Неопределено Тогда
		НачатьОтключениеОборудованиеПоИдентификатору(, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройстваЭТ);
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='საბეჭდი მოწყობილობა არ არის არჩეული.';ru='Печатающее устройство не выбрано.';en='The printing device is not selected.'");
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПечатающееУстройствоПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		НачатьОтключениеОборудованиеПоИдентификатору(, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройстваЭТ);
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='საბეჭდი მოწყობილობის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении печатающего устройство произошла ошибка: %ДополнительноеОписание%';en='An error occurred while connecting the printing device:% DopolnitelnoeOpisanie%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
//// Выполнить краткий отчет на эквайринговом терминале.
// Если эквайринговый терминал не поддерживает печать квитанций на терминале, для печати подключается печатающее устройство.
//
Процедура НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройстваЭТ = Неопределено, ИдентификаторУстройстваПУ = Неопределено) Экспорт
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОповещениеПриЗавершении" , ОповещениеПриЗавершении);
	Контекст.Вставить("УникальныйИдентификатор" , УникальныйИдентификатор);
	
	Оповещение = Новый ОписаниеОповещения("НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале_ПодключениеЗавершение", ЭтотОбъект, Контекст);
...
Процедура НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Если РезультатПодключения.Результат Тогда  
		...
		НачатьВыполнениеКоманды(Оповещение, РезультатПодключения.ИдентификаторУстройстваЭТ, "КраткийОтчет", ВходныеПараметры);
	Иначе
		// Ошибка подключения.
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ტერმინალის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении терминала произошла ошибка: %ДополнительноеОписание%';en='An error occurred when connecting the terminal:% DopolnitelnoeOpisanie%'");
...
Процедура НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале_ВыполнениеКомандыЗавершение(РезультатВыполнения, Параметры) Экспорт
	
	Попытка
		ТекстСлипЧека = РезультатВыполнения.ВыходныеПараметры[0][1];
	Исключение
		//G.Didebashvili
		ТекстСлипЧека = РезультатВыполнения.ВыходныеПараметры[0].СлипОтХоста;
	КонецПопытки;
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если Не Параметры.ПечатьКвитанцийНаТерминале И Не Параметры.ИдентификаторУстройстваПУ = Неопределено И Не ПустаяСтрока(ТекстСлипЧека) Тогда
			Оповещение = Новый ОписаниеОповещения("НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале_ПечатьЗавершение", ЭтотОбъект, Параметры);
...
Процедура НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале_ПечатьЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='სლიპ-ჩეკის ამობეჭდვისას შეცდომა წარმოიშვა: %ДополнительноеОписание%';ru='При печати слип чека возникла ошибка: %ДополнительноеОписание%';en='When you print a receipt slip error occurred:% DopolnitelnoeOpisanie%'");
...
Процедура НачатьВыполнениеОперацииНаЭквайринговомТерминале_ВыполнениеКомандыЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		...
		Попытка
			//შეცდომის ჩეკის ბეჭდვა ტერმინალიდან	- Start
			ПараметрыОперации = Параметры.ПараметрыОперации;
			Если Параметры.Свойство("БезВозвращаемыхПараметров") Или РезультатВыполнения.ВыходныеПараметры.Количество()= 1 Тогда
				ПараметрыОперации.НомерКарты     = "";
				ПараметрыОперации.СсылочныйНомер = "";
			...
			Если Не Параметры.ПечатьКвитанцийНаТерминале И Не Параметры.ИдентификаторУстройстваПУ = Неопределено И Не ПустаяСтрока(ПараметрыОперации.ТекстСлипЧека) Тогда
				ПараметрыПред = Параметры;
				Параметры.ПараметрыОперации.Вставить("ЭтоПечатьОшибки", Истина);  //+++ Дмгг
				Оповещение = Новый ОписаниеОповещения("НачатьВыполнениеОперацииНаЭквайринговомТерминале_ПечатьЗавершение", ЭтотОбъект, Параметры);
				НачатьПечатьТекста(Оповещение, Параметры.УникальныйИдентификатор, ПараметрыОперации.ТекстСлипЧека, Параметры.ИдентификаторУстройстваПУ);
				
				Параметры = ПараметрыПред;
				//შეცდომის ჩეკის ბეჭდვა ტერმინალიდან	- Start  			
			КонецЕсли;
			//შეცდომის ჩეკის ბეჭდვა ტერმინალიდან	- End
		Исключение
		КонецПопытки;
...
Процедура НачатьВыводИнформацииНаДисплейПокупателя(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, СтрокиТекста = Неопределено) Экспорт
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено И ОповещениеПриЗавершении <> Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='მყიდველის შეერთებული დისპლეები არ არის.';ru='Нет подключенных дисплеев покупателя.';en='No connection customer displays.'");
...
Процедура НачатьОперациюНаДисплейПокупателя_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ka='მყიდველის დისპლეის გამოყენებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При использовании дисплея покупателя произошла ошибка: %ДополнительноеОписание%';en='When using a customer display error has occurred:% DopolnitelnoeOpisanie%'");
...
Процедура НачатьЗагрузкуДанныеИзТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, СворачиватьДанные = Ложь) Экспорт
	...
	ПредложитьВыбратьУстройство(Оповещение, "ТерминалСбораДанных", НСтр("ka='აირჩიეთ მონაცემთა შეკრების ტერმინალი';ru='Выберите терминал сбора данных';en='Select a data collection terminal'"), НСтр("ka='მონაცემთა შეკრების ტერმინალი არ არის შეერთებული.';ru='Терминал сбора данных не подключен.';en='data collection terminal is not connected.'"), ,Истина);
...
Процедура НачатьЗагрузкуДанныеИзТСД_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='მონაცემთა შეკრების ტერმინალი არ არის შეერთებული.';ru='Терминал сбора данных не подключен.';en='data collection terminal is not connected.'");
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
"%ОписаниеОшибки%';ka='მონაცემთა შეკრების ტერმინალიდან მონაცემთა ჩატვირთვისას შეცდომა მოხდა. %ОписаниеОшибки%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(РезультатВыполнения.Результат, ОписаниеОшибки, Параметры.ИдентификаторУстройства);  
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьВыгрузкуДанныеВТСД_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = ""; 
	Иначе
		ОписаниеОшибки = НСтр("ka='მონაცემთა შეკრების ტერმინალში მონაცემთა გადმოტვირთვისას შეცდომა მოხდა: %ОписаниеОшибки%';ru='При выгрузке данных в терминал сбора данных произошла ошибка: %ОписаниеОшибки%';en='When uploading data to the data collection terminal error:%% OpisanieOshibki'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьОчисткуДанныеВТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ТерминалСбораДанных",
			НСтр("ka='აირჩიეთ მონაცემთა შეკრების ტერმინალი';ru='Выберите терминал сбора данных';en='Select a data collection terminal'"), НСтр("ka='მონაცემთა შეკრების ტერმინალი არ არის შეერთებული.';ru='Терминал сбора данных не подключен.';en='data collection terminal is not connected.'"), ,Истина);
	Иначе
...
Процедура НачатьОчисткуДанныеВТСД_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = ""; 
	Иначе
		ОписаниеОшибки = НСтр("ka='მონაცემთა შეკრების ტერმინალში მონაცემთა გასუფთავებისას შეცდომა მოხდა: %ОписаниеОшибки%';ru='При очистке данных в терминале сбора данных произошла ошибка: %ОписаниеОшибки%';en='When cleaning the data in the data collection terminal error:%% OpisanieOshibki'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьПолученияВесаСЭлектронныхВесов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ЭлектронныеВесы",
			НСтр("ka='აირჩიეთ ელექტრონული სასწორი';ru='Выберите электронные весы';en='Select electronic scales'"), 
			НСтр("ka='ელექტრონული სასწორი არ არის შეერთებული.';ru='Электронные весы не подключены.';en='Electronic scales are not connected.'"), 
			НСтр("ka='ელექტრონული სასწორი არ არის არჩეული.';ru='Электронные весы не выбраны.';en='Electronic scales are selected.'"),
			Истина);
	Иначе
...
Процедура НачатьПолученияВесаСЭлектронныхВесов_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='ელექტრონული სასწორი არ არის არჩეული.';ru='Электронные весы не выбраны.';en='Electronic scales are selected.'");
...
Процедура НачатьПолученияВесаСЭлектронныхВесов_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		Вес = Неопределено;
		ОписаниеОшибки = НСтр("ka='ელექტრონული სასწორის გამოყენებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При использовании электронных весов произошла ошибка: %ДополнительноеОписание%';en='When using an electronic scale error:%% DopolnitelnoeOpisanie'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки);
		КонецЕсли;
...
Процедура НачатьПечатьЭтикеток(ОповещениеПриЗавершенииПечати, УникальныйИдентификатор, ОписаниеЭтикеткиВXML, ДанныеДляПечати, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда	
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ПринтерЭтикеток",
			НСтр("ka='აირჩიეთ ეტიკეტების პრინტერი';ru='Выберите принтер этикеток';en='Select Label Printer'"), 
			НСтр("ka='ეტიკეტების პრინტერი არ არის შეერთებული.';ru='Принтер этикеток не подключен.';en='label printer is not connected.'"), 
			НСтр("ka='ეტიკეტების პრინტერი არ არის არჩეული.';ru='Принтер этикеток не выбран.';en='Label printer is selected.'"), 
			Истина);
	Иначе
...
Процедура НачатьПечатьЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ეტიკეტების პრინტერი არ არის არჩეული.';ru='Принтер этикеток не выбран.';en='Label printer is selected.'");
...
"%ОписаниеОшибки%"
"Печать прервана.';ka='ეტიკეტის მაკეტის დამუშავებისას შეცდომა მოხდა."
"%ОписаниеОшибки%"
"ამობეჭდვა შეწყვეტილია.'");
...
"%ОписаниеОшибки%';ka='მოწყობილობის შეერთებისას შეცდომა მოხდა. %ОписаниеОшибки%'");
...
Процедура НачатьРедактированиеМакета(ОповещениеПриЗавершенииРедактирования, ОписаниеЭтикеткиВXML, АдресХранилищаСКД) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='ეტიკეტების რედაქტორის გამოყენება შეუძლებელია! ფუნქციურობას მხარდაჭერა არ გააჩნია.';ru='Использование редактора этикеток невозможно! Функциональность не поддерживается.';en='Using the label editor is impossible! The functionality is not supported.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецПопытки;
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток(ОповещениеПриЗавершении, УникальныйИдентификатор, ТаблицаВыгрузкиТоваров, ИдентификаторУстройства = Неопределено, ЧастичнаяВыгрузка = Ложь, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ТаблицаВыгрузкиТоваров.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ka='გადმოსატვირთად მონაცემები არ არის!';ru='Нет данных для выгрузки!';en='There is no data to upload!'");
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ВесыСПечатьюЭтикеток",
			НСтр("ka='აირჩიეთ ეტიკეტების მბეჭდავი სასწორი';ru='Выберите весы с печатью этикеток';en='Select the scales with label printing'"),
			НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის შეერთებული.';ru='Весы с печатью этикеток не подключены.';en='Scales with label printing are not connected.'"), 
			НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის არჩეული.';ru='Весы с печатью этикеток не выбраны.';en='Scales with label printing is selected.'"), 
			Истина);
	Иначе
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის არჩეული.';ru='Весы с печатью этикеток не выбраны.';en='Scales with label printing is selected.'");
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток_ФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		...
		НачатьПодключениеОборудованиеПоИдентификатору(ОписаниеОповещения, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройства);
	Иначе
		ТекстСообщения = НСтр("ka='ვებ-კლიენტ ""1C:საწარმო""-სთვის დაყენებული გაფართოების გარეშე მოცემული ოპერაცია მიუწვდომელია.';ru='Данная операция не доступна без установленного расширения для веб-клиента ""1С:Предприятие"".';en='This operation is not available without a set extensions for the Web client ""1C: Enterprise"".'");
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		Если Параметры.СледующееОповещение <> Неопределено Тогда
...
"%ОписаниеОшибки%';ka='მოწყობილობაში მონაცემთა გადმოტვირთვისას შეცდომა მოხდა. %ОписаниеОшибки%'");
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის არჩეული.';ru='Весы с печатью этикеток не выбраны.';en='Scales with label printing is selected.'");
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='При подключении устройства произошла ошибка. |%ОписаниеОшибки%';en='When connecting a device error has occurred. |% OpisanieOshibki%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатПодключения.ОписаниеОшибки);
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ka='შეცდომები არ არის.';ru='Ошибок нет.';en='There are no errors.'");
	Иначе
		ОписаниеОшибки = НСтр("ru='При очистке данных в оборудование произошла ошибка. |%ОписаниеОшибки%';en='When cleaning the data in the equipment error. |% OpisanieOshibki%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьВыгрузкуНастроекВККМOffline(ОповещениеПриВыгрузкеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено,
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ККМOffline",
			НСтр("ka='აირჩიეთ სსა Offline';ru='Выберите ККМ Offline';en='Select CMC Offline'"), НСтр("ka='სსა Offline არ არის შეერთებული.';ru='ККМ Offline не подключены.';en='CMC Offline are not connected.'"));
	Иначе
...
Процедура НачатьВыгрузкуНастроекВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს გამართვების გადმოტვირთვა სსა-ში Offline…';ru='Выполняется выгрузка настроек в ККМ Offline...';en='Unloading is performed in CMC settings Offline ...'")); 
...
"%ОписаниеОшибки%';ka='მოწყობილობაში მონაცემთა გადმოტვირთვისას შეცდომა მოხდა. %ОписаниеОшибки%'");
			...
			Иначе
				ТекстСообщения = НСтр("ka='მონაცემები წარმატებით გადმოიტვირთა.';ru='Данные выгружены успешно.';en='Data uploaded successfully.'");
			КонецЕсли;
...
Процедура НачатьОчисткуТоваровВККМOffline(ОповещениеПриОчисткеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Истина) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ККМOffline",
			НСтр("ka='აირჩიეთ სსა Offline';ru='Выберите ККМ Offline';en='Select CMC Offline'"), 
			НСтр("ka='სსა Offline არ არის შეერთებული.';ru='ККМ Offline не подключены.';en='CMC Offline are not connected.'"),
			НСтр("ru='ККМ Offline не не выбрано.';en='CMC Offline is not selected.'"), 
...
Процедура НачатьОчисткуТоваровВККМOfflineЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ТекстСообщения = НСтр("ka='მოწყობილობა არ არის არჩეული.';ru='Оборудование не выбрано.';en='The equipment is not selected.'");
		Если Параметры.СледующееОповещение <> Неопределено Тогда
...
Процедура НачатьОчисткуТоваровВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს საქონლის გასუფთავება სსა-ში Offline...';ru='Выполняется очистка товаров в ККМ Offline...';en='Clears goods CMC Offline ...'"));
...
"%ОписаниеОшибки%';ka='მოწყობილობაში მონაცემთა გასუფთავებისას შეცდომა მოხდა. %ОписаниеОшибки%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
			Иначе
				ТекстСообщения = НСтр("ka='მონაცემთა წაშლა წარმატებით დასრულდა.';ru='Очистка данных успешно завершена.';en='Data cleaning is successfully completed.'");
			КонецЕсли;
...
Процедура НачатьВыгрузкуДанныеВККМOffline(ОповещениеПриВыгрузкеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено,
	...
	Иначе
		...
		Если МенеджерОборудованияВызовСервера.ПроверитьДублированиеКодов(РасширеннаяВыгрузка, ГруппыТоваров, Товары, ДублирующиесяКоды) Тогда
			...
			ТекстСообщения = НСтр("ka='გადმოტვირთვა შეუძლებელია! ხორციელდება საქონლის შემდეგი კოდების დუბლირება: %1';ru='Выгрузка невозможна! Дублируются следующие коды товаров: %1';en='Unloading is not possible! The following items are duplicated codes:% 1'");
...
Процедура НачатьВыгрузкуДанныеВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Параметры.РасширеннаяВыгрузка И НЕ ВремРасширеннаяВыгрузка Тогда
		ТекстСообщения = НСтр("ka='გაფართოებული გადმოტვირთვის ოპერაცია არჩეული დრაივერისთვის ხელმისაწვდომი არ არის.';ru='Операция расширенной выгрузки не доступна для выбранного драйвера.';en='Operation extended discharge is not available for the selected driver.'");
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		Если Параметры.СледующееОповещение <> Неопределено Тогда
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს საქონლის გადმოტვირთვა სსა-ში Offline...';ru='Выполняется выгрузка товаров в ККМ Offline...';en='Unloading of goods is performed in CMC Offline ...'"));
...
Процедура НачатьЗагрузкуОтчетаОРозничныхПродажахИзККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Параметры.РасширеннаяЗагрузка И НЕ ВремРасширеннаяЗагрузка Тогда
		ТекстСообщения = НСтр("ka='არჩეული დრაივერისთვის გაფართოებული ჩატვირთვის ოპერაცია მიუწვდომელია.';ru='Операция расширенной загрузки не доступна для выбранного драйвера.';en='Operation Extended Download is not available for the selected driver.'");
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		Если Параметры.СледующееОповещение <> Неопределено Тогда
	...
	Если Результат Тогда
		Состояние(НСтр("ka='მიმდინარეობს საქონლის ჩატვირთვა სსა-დან Offline...';ru='Выполняется загрузка товаров из ККМ Offline...';en='Loading goods from CMC Offline ...'"));
...
"%ОписаниеОшибки%';ka='სსა Offline-დან მონაცემთა ჩატვირთვისას შეცდომა მოხდა."
"%ОписаниеОшибки%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
				ТаблицаТоваров = Неопределено;
			Иначе
				ТаблицаТоваров = ВыходныеПараметры[0];
				ТекстСообщения = НСтр("ka='მონაცემები წარმატებით ჩაიტვირთა.';ru='Данные загружены успешно.';en='The data is loaded successfully.'");
			КонецЕсли;
...
Процедура НачатьЗагрузкуОтчетаОРозничныхПродажахИзККМOfflineВыполнитьКомандуЗавершение(РезультатКоманды, Параметры) Экспорт
	...
	Если НЕ РезультатКоманды.Результат Тогда
		ТекстСообщения = НСтр("ru='При загрузке данных из ККМ Offline произошла ошибка. %ОписаниеОшибки%';en='When loading data from CMC Offline error occurred. % OpisanieOshibki%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", РезультатКоманды.ВыходныеПараметры[1]);
		ТаблицаТоваров = Неопределено;
	Иначе
...
Процедура НачатьОткрытиеСессииСчитывателяRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	ИначеЕсли  ОповещениеПриЗавершении <> Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Нет подключенных RFID считывателей.';en='No connected RFID readers.'");
		РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатОперации);
	КонецЕсли;
...
Процедура НачатьОткрытиеСессииСчитывателяRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ka='RFID წამკითხველთან მუშაობისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При работе со считывателем RFID произошла ошибка: %ДополнительноеОписание%';en='When working with the RFID reader error occurred:% DopolnitelnoeOpisanie%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьЗаписьДанныхВМеткуRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства, ПараметрыЗаписи, Таймаут = 0) Экспорт 
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "СчитывательRFID",
			НСтр("ka='აირჩიეთ RFID წამკითხველი';ru='Выберите RFID считыватель';en='Select RFID Reader'"), НСтр("ru='Нет подключенных RFID считывателей.';en='No connected RFID readers.'"));
	Иначе
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='RFID წამკითხველი არ არის შეერთებული.';ru='RFID считыватель  не подключен.';en='RFID reader is connected.'");
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании();
			РезультатОперации.Результат = Ложь;
			РезультатОперации.ОписаниеОшибки = ОписаниеОшибки;
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ОписаниеОшибки = ""; 
	Если НЕ РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ka='RFID წამკითხველთან მუშაობისას შეცდომა მოხდა: %ОписаниеОшибки%';ru='При работе со считывателем RFID произошла ошибка: %ОписаниеОшибки%';en='When working with the RFID reader error occurred:% OpisanieOshibki%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
	...
	#Иначе
		...
		Для Каждого Файл Из ПомещаемыеФайлы Цикл
			...
			Исключение
				...
				ТекстСообщения =  НСтр("ka='ფაილ %ИмяФайла%-ის წაკითხვისას შეცდომა მოხდა';ru='При чтении файла %ИмяФайла% произошла ошибка';en='When reading the file% filename% An error occurred'");
...
Процедура ПолучитьСодержаниеТекстовыхФайловЗавершение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Тогда
		...
		РезультатЧтенияФайлов.ТекстОшибки = НСтр("ka='უცნობი შეცდომა სერვერზე ფაილების გადაცემისას.';ru='Неизвестная ошибка при передаче файлов на сервер.';en='An unknown error occurred when transferring files to the server.'");
...
Процедура НачатьВыполнениеКоманды_ВыбратьСменуЗавершение(СтруктураПараметровСмены, Параметры) Экспорт
	...
	Если СтруктураПараметровСмены = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Кассовая смена не выбрана';en='Cash change is not selected'");
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
