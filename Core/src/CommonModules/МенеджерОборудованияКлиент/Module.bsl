...
Процедура ВыполнитьНастройкуОборудования(Идентификатор, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დრაივერის გამართვის ფორმის ინიციალიზაციის შეცდომა მოხდა.';ru='Произошла ошибка инициализации формы настройки драйвера.'")); 
	КонецЕсли;
...
Процедура ВыполнитьНастройкуОборудования_Завершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Иначе
			СообщениеОбОшибке = НСтр("ka='მოწყობილობის პარამეტრების შენახვა ვერ მოხერხდა.';ru='Не удалось сохранить параметры устройства.'");
...
Процедура ПредложитьВыбратьУстройство(ОповещениеВыбора, ТипОборудования, ТекстЗаголовкаВыбора, 
	...
	Если Не ОбновитьРабочееМестоКлиента() Тогда
		ТекстСообщения = НСтр("ka='წინასწარ აუცილებელია მიმდინარე სეანსის შესაერთებელი მოწყობილობის სამუშაო ადგილის არჩევა.';ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.'");
...
Процедура НачатьВыборФайлаДрайвера(ОповещениеПриВыборе) Экспорт 
	...
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ დრაივერის ფაილი';ru='Выберите файл драйвера'");
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='დრაივერის ფაილი';ru='Файл драйвера'") + ?(МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент(), "(*.zip)|*.zip", "(*.zip, *.exe)| *.zip; *.exe");  
...
Процедура НачатьПодключениеОборудования(ОповещениеПриПодключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если Не Результат Тогда
		Если ОповещениеПриПодключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='წინასწარ აუცილებელია მიმდინარე სეანსის შესაერთებელი მოწყობილობის სამუშაო ადგილის არჩევა.';ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.'");
		...
		Для Каждого Устройство Из СписокОборудования Цикл
			...
			Если ПодключенноеУстройство = Неопределено Тогда // Если устройство не было подключено ранее.
				...
				Если ОбработчикДрайвераМодуль = Неопределено Тогда
					...
					Если ОповещениеПриПодключении <> Неопределено Тогда
						ОписаниеОшибки = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.'");
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
						...
						Иначе
							...
							Если Результат Тогда
								...
								Если ОповещениеПриПодключении <> Неопределено Тогда
									ОписаниеОшибки = НСтр("ka='შეცდომები არ არის.';ru='Ошибок нет.'");
							...
							Иначе
								...
								Если ОповещениеПриПодключении <> Неопределено Тогда
									ОписаниеОшибки = НСтр("ka='ვერ მოხერხდა მოწყობილობის შეერთება ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)'");
									ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , НовоеПодключение.Наименование);
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено И ОповещениеПриПодключении <> Неопределено Тогда
		ОписаниеОшибки =  НСтр("ka='შერჩეული მოწყობილობის გამოყენება შეერთებისთვის შეუძლებელია. მიუთითეთ სხვა მოწყობილობა.';ru='Выбранное устройство не может использоваться для подключения. Укажите другое устройство.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриПодключении, РезультатВыполнения);
	ИначеЕсли ТипыПО = Неопределено И ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки =  НСтр("ka='შესაერთებლად ხელმისაწვდომი მოწყობილობა არ არის.';ru='Нет доступного оборудования для подключения.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриПодключении, РезультатВыполнения);
	КонецЕсли;
...
Процедура НачатьОтключениеОборудование_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		// Сообщим пользователю о том, что не удалось подключить устройство.
		Если Параметры.ОповещениеПриОтключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='მოწყობილობა ""%Наименование%""-ის გამორთვისას შეცდომა მოხდა.';ru='При отключении устройства ""%Наименование%"" произошла ошибка.'");
...
Процедура НачатьОтключениеВсегоОборудованияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='მოწყობილობის გამორთვისას შეცდომა მოხდა: ""%ОписаниеОшибки%"".';ru='При отключении оборудования произошла ошибка: ""%ОписаниеОшибки%"".'");
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
				...
				Если ОбработчикДрайвераМодуль = Неопределено Тогда
					// Сообщить об ошибке, что не удалось загрузить драйвер.
					ОписаниеОшибки = ОписаниеОшибки +  НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.'");
					КонечныйРезультат = Ложь;
					Продолжить;
				Иначе
				...
				Иначе
					// Сообщим пользователю о том, что не удалось подключить устройство.
					ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
								   + НСтр("ka='ვერ მოხერხდა მოწყობილობის შეერთება ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)'");
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , Устройство.Наименование);
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%"     , ВыходныеПараметры[0]);
				КонецЕсли;
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
					...
					Если НЕ Результат Тогда
						ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
									   + НСтр("ka='მოწყობილობა ""%Наименование%"" გამორთვისას მოხდა შეცდომა: %ОписаниеОшибки% (%КодОшибки%)';ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки% (%КодОшибки%)'");
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%", ПодключенноеУстройство.Наименование);
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%", ВыходныеПараметры[0]);
					Иначе
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
		...
		Иначе
			...
			Если ОбработчикДрайвераМодуль = Неопределено Тогда
				ТекстСообщения = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.'");
				ВыходныеПараметры = Новый Массив();
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(ТекстСообщения);
				ВыходныеПараметры.Добавить(НСтр("ka='არ არის დაყენებული';ru='Не установлен'"));
			Иначе
				// Принудительно заменяем асинхронный обработчик на синхронный, так как работам в синхронном режиме.
				Если ОбработчикДрайвераМодуль = ПодключаемоеОборудованиеУниверсальныйДрайверАсинхронноКлиент Тогда
					ОбработчикДрайвераМодуль = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;
	...
	Иначе
		...
		ВыходныеПараметры = Новый Массив();
		ТекстСообщения = НСтр("ka='მოწყობილობა არ არის ჩართული. ოპერაციის შესრულების წინ მოწყობილობა უნდა ჩაირთოს.';ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.'");
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
	КонецЕсли;
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='მოწყობილობა ჩართულია. ოპერაციის შესრულების წინ მოწყობილობა გამორთული უნდა იყოს.';ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
		ВыходныеПараметры.Добавить(НСтр("ka='დაყენებულია';ru='Установлен'"));
...
Процедура ПодключитьОборудованиеЗавершениеПоУмолчанию(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='მოწყობილობის შეერთებისას შეცდომა მოხდა:""%ОписаниеОшибки%"".';ru='При подключении оборудования произошла ошибка:""%ОписаниеОшибки%"".'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%" , РезультатВыполнения.ОписаниеОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура НачатьПодключениеРасширенияРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Не Подключено И ДополнительныеПараметры.ПредлагатьУстановку Тогда
		...
		ТекстСообщения = НСтр("ka='მუშაობის გასაგრძელებლად აუცილებელია ვებ-კლიენტ ""1С:საწარმო""-სთვის გაფართოების დაყენება. დავაყენოთ?';ru='Для продолжении работы необходимо установить расширение для веб-клиента ""1С:Предприятие"". Установить?'");
...
Процедура УстановитьДрайвер(Идентификатор, ОповещениеИзДистрибутиваПриЗавершении = Неопределено, ОповещениеИзАрхиваПриЗавершении = Неопределено) Экспорт
	...
	Попытка  
		Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
			...
			#Если ВебКлиент Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოცემული დრაივერი ვებ-კლიენტში მუშაობას მხარს არ უჭერს.';ru='Данный драйвер не поддерживает работу в веб-клиенте.'")); 
			#Иначе
				Если МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент() Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოცემული დრაივერის დაყენება და გამოყენება Linux-ის გარემოში ვერ მოხდება.';ru='Данный драйвер не может быть установлен и использован в среде Linux.'")); 
					Возврат;
				КонецЕсли;
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დრაივერის დაყენებისას შეცდომა მოხდა.';ru='Произошла ошибка при установке драйвера.'")); 
	КонецПопытки;  
...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, Идентификатор, Команда, ВходныеПараметры) Экспорт
	...
	Если ПодключенноеУстройство <> Неопределено Тогда
		...
		Если ОбработчикДрайвераМодуль = Неопределено Или ОбъектДрайвера = Неопределено Тогда
			// Сообщить об ошибке, что не удалось загрузить драйвер.
			ТекстОшибки = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ან დრაივერის ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера или загрузить драйвер.'");
	...
	Иначе
		// Сообщить об ошибке, что устройство не подключено.
		ТекстОшибки = НСтр("ka='მოწყობილობა არ არის ჩართული. ოპერაციის შესრულების წინ მოწყობილობა უნდა ჩაირთოს.';ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании();
		РезультатВыполнения.Результат = Ложь;
		РезультатВыполнения.ОписаниеОшибки = ТекстОшибки; 
		ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатВыполнения);
...
Процедура НачатьВыполнениеДополнительнойКоманды(ОповещениеПриЗавершении, Команда, ВходныеПараметры, Идентификатор, Параметры) Экспорт
	...
	Иначе
		// Сообщить об ошибке, что устройство подключено.
		ТекстОшибки = НСтр("ka='მოწყობილობა ჩართულია. ოპერაციის შესრულების წინ მოწყობილობა გამორთული უნდა იყოს.';ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.'");
	...
	Иначе
		...
		Если ОбработчикДрайвераМодуль = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ka='დრაივერის დამამუშავებლის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить обработчик драйвера.'");
			ВыходныеПараметры = Новый Массив();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(ТекстОшибки);
			ВыходныеПараметры.Добавить(НСтр("ka='არ არის დაყენებული';ru='Не установлен'"));
...
Процедура НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ფისკალური მოწყობილობა არ არის არჩეული ან შეერთებული.';ru='Фискальное устройство не выбрано или не подключено.'");
...
Процедура НачатьВыполнениеКоманды_ПослеПодтвержденияЗакрытияСмены(Результат, Параметры) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='На фискальном устройстве смена не закрыта';ru='На фискальном устройстве смена не закрыта'");
...
Процедура НачатьВыполнениеКоманды_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='მოწყობილობის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении оборудования произошла ошибка: %ДополнительноеОписание%'");
...
Процедура НачатьВыполнениеКоманды_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='შეცდომები არ არის.';ru='Ошибок нет.'");
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ka='ოპერაციის შესრულებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При выполнении операции произошла ошибка: %ДополнительноеОписание%'");
...
Процедура НачатьФормированиеОтчетаОТекущемСостоянииРасчетов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ka='აირჩიეთ ფისკალური მოწყობილობა';ru='Выберите фискальное устройство'"),
			НСтр("ka='ფისკალური მოწყობილობა არ არის შეერთებული.';ru='Фискальное устройство не подключено.'"), 
			НСтр("ka='ფისკალური მოწყობილობა არ არის არჩეული.';ru='Фискальное устройство не выбрано.'"),
			Истина);
	Иначе
...
Процедура НачатьПечатьТекста(ОповещениеПриЗавершении, УникальныйИдентификатор, Текст, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ka='აირჩიეთ საბეჭდი მოწყობილობა';ru='Выберите печатающее устройство'"), 
			НСтр("ka='საბეჭდი მოწყობილობა არ არის შეერთებული.';ru='Печатающее устройство не подключено.'"), 
			НСтр("ka='საბეჭდი მოწყობილობა არ არის არჩეული.';ru='Печатающее устройство не выбрано.'"), 
			Истина);
	Иначе
		НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Контекст);
	КонецЕсли;
...
Функция СформироватьТексНефискальногоЧека(ШиринаСтроки, ТипДокумента, ВходныеПараметры, ДополнительныйТекст = Неопределено) Экспорт
	...
	Если ТипДокумента = 0 Тогда
		...
		ТипыЧеков.Вставить(1, НСтр("ka='გაყიდვა';ru='ПРОДАЖА'"));       
		ТипыЧеков.Вставить(2, НСтр("ka='დაბრუნება';ru='ВОЗВРАТ'"));  
	...
	ИначеЕсли ТипДокумента = 2 Тогда
		ТипЧека = НСтр("ka='შეტანა';ru='ВНЕСЕНИЕ'");  
	ИначеЕсли ТипДокумента = 3 Тогда
		ТипЧека = НСтр("ka='ამოღება';ru='ВЫЕМКА'");  
	ИначеЕсли ТипДокумента = 4 Тогда
		ТипЧека = НСтр("ka='ანგარიშგება დაფარვის გარეშე';ru='ОТЧЕТ БЕЗ ГАШЕНИЯ'");  
	ИначеЕсли ТипДокумента = 5 Тогда
		ТипЧека = НСтр("ka='ანგარიშგება დაფარვით';ru='ОТЧЕТ С ГАШЕНИЕМ'");  
	КонецЕсли;
	...
	Если ПечатьКлише Тогда
		...
		НомерКассы = ?(ОбщиеПараметры.Свойство("НомерКассы") И НЕ ПустаяСтрока(ОбщиеПараметры.НомерКассы), 
			НСтр("ka='სალარო';ru='КАССА'") + Символы.НПП + ОбщиеПараметры.НомерКассы, "");
	
		ОрганизацияИНН = ?(ОбщиеПараметры.Свойство("ОрганизацияИНН") И НЕ ПустаяСтрока(ОбщиеПараметры.ОрганизацияИНН), 
			НСтр("ka='ჩეკი №';ru='ЧЕК №'") + Символы.НПП + ОбщиеПараметры.ОрганизацияИНН, "");
		...
		НомерЧека = ?(ОбщиеПараметры.Свойство("НомерЧека") И НЕ ПустаяСтрока(ОбщиеПараметры.НомерЧека),
			НСтр("ka='ჩეკი №';ru='ЧЕК №'") + ОбщиеПараметры.НомерЧека, "");
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если Окр(Количество * Цена, 2) > Сумма Тогда
					ТекстСтроки = ТекстСтроки + ПостроитьПоле(НСтр("ka='ფასდაკლება =';ru='СКИДКА ='") + Символы.НПП
									+ Формат(Количество * Цена - Сумма, ФорматЧисла) + " ", ШиринаСтроки, , Ложь)  + Символы.ПС;
				ИначеЕсли Окр(Количество * Цена, 2) < Сумма Тогда
					ТекстСтроки = ТекстСтроки + ПостроитьПоле(НСтр("ka='დანამატი =';ru='НАДБАВКА ='") + Символы.НПП
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПоле(НСтр("ka='ჯამი =';ru='ИТОГ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаЧека, ФорматЧисла) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		Если СуммаНаличнойОплаты > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='ნაღდი ფულით =';ru='НАЛИЧНЫМИ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаНаличнойОплаты, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты1 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='საგადახდო ბარათით =';ru='ПЛАТ.КАРТОЙ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты1, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты2 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='კრედიტით =';ru='КРЕДИТОМ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты2, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты3 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ka='სერთიფიკატით =';ru='СЕРТИФИКАТОМ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты3, ФорматЧисла) + Символы.ПС;
		КонецЕсли;          
		
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПоле(НСтр("ka='ჩაბარება =';ru='СДАЧА ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаОплаты - СуммаЧека, ФорматЧисла) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		Кассир = ?(ОбщиеПараметры.Свойство("Кассир"), ОбщиеПараметры.Кассир, "");
		Текст = Текст + ПостроитьПоле(Символы.НПП + НСтр("ka='მოლარე';ru='КАССИР'") + ":" + Символы.НПП + Кассир, ШиринаСтроки) + Символы.ПС;                        
		Текст = Текст + ПостроитьПоле(НСтр("ka='ხელმოწერა';ru='ПОДПИСЬ'") + ":", ШиринаСтроки, "_") + Символы.ПС + Символы.ПС;
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			ИначеЕсли ПозицияЧека.Свойство("Штрихкод") Тогда
				...
				Если НЕ ОбщийМодульОборудования.ПечатьШтрихкода(ОбъектДрайвера, Параметры, ПараметрыПодключения,
					...
					Текст = НСтр("ka='<შტრიხკოდი %ТипШтрихКода% არ არის ამობეჭდილი>';ru='<Штрихкод %ТипШтрихКода% не распечатан>'");
...
Функция СформироватьШаблонИнфоКвитанции(Параметры, ШиринаСтроки, ПечатьШтрихкода = Истина) Экспорт
	...
	ОрганизацияИНН = ?(НЕ ПустаяСтрока(Параметры.ИНН), НСтр("ka='ს.კ / პ.ნ:';ru='ИНН:'") + Параметры.ИНН, "");
	ОрганизацияКПП = ?(НЕ ПустаяСтрока(Параметры.КПП), НСтр("ka='КПП:';ru='КПП:'") + Параметры.КПП, "");
	...
	НомерКассы = ?(НЕ ПустаяСтрока(Параметры.НомерКассы), НСтр("ka='КАССА:';ru='КАССА:'") + Параметры.НомерКассы, "");
	НомерСмены = ?(НЕ ПустаяСтрока(Параметры.НомерСмены), НСтр("ka='СМЕНА:';ru='СМЕНА:'") + Формат(Параметры.НомерСмены, "ЧГ=0"), 0);
	...
	НомерЧека = ?(НЕ ПустаяСтрока(Параметры.НомерЧека), НСтр("ka='ЧЕК:';ru='ЧЕК:'") + Параметры.НомерЧека, "");
	ДатаВремя = НСтр("ka='ДАТА:';ru='ДАТА:'") + Формат(ТекущаяДата(), "ДФ=""дд.ММ.гггг ЧЧ:мм""");
...
Процедура НачатьПодключенияЭквайринговогоТерминала(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройстваЭТ = Неопределено, ИдентификаторУстройстваПУ = Неопределено) Экспорт
	...
	Если Контекст.ИдентификаторУстройстваЭТ = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ЭквайринговыйТерминал",
			НСтр("ka='აირჩიეთ ეკვაირინგის ტერმინალი';ru='Выберите эквайринговый терминал'"), 
			НСтр("ka='ეკვაირინგის ტერმინალი არ არის შეერთებული.';ru='Эквайринговый терминал не подключен.'"),
			НСтр("ka='ეკვაირინგის ტერმინალი არ არის არჩეული';ru='Эквайринговый терминал не выбран.'"), 
			Истина);
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьУстройствоЗавершение(ИдентификаторУстройстваЭТ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваЭТ = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ეკვაირინგის ტერმინალი არ არის არჩეული';ru='Эквайринговый терминал не выбран.'");
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		// Ошибка подключения.
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ტერმინალის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении терминала произошла ошибка: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПечатьНаТерминалеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Иначе
			Если Параметры.ИдентификаторУстройстваПУ = Неопределено Тогда
				...
				ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
					НСтр("ka='აირჩიეთ საბეჭდი მოწყობილობა';ru='Выберите печатающее устройство'"), НСтр("ka='საბეჭდი მოწყობილობა არ არის შეერთებული.';ru='Печатающее устройство не подключено.'"), НСтр("ka='საბეჭდი მოწყობილობა არ არის არჩეული.';ru='Печатающее устройство не выбрано.'"), Истина);
			Иначе
	...
	Иначе
		...
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ტერმინალის გამოყენებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При использовании терминала произошла ошибка: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьПечатающегоУстройстваЗавершение(ИдентификаторУстройстваПУ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваПУ = Неопределено Тогда
		НачатьОтключениеОборудованиеПоИдентификатору(, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройстваЭТ);
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='საბეჭდი მოწყობილობა არ არის არჩეული.';ru='Печатающее устройство не выбрано.'");
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПечатающееУстройствоПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		НачатьОтключениеОборудованиеПоИдентификатору(, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройстваЭТ);
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='საბეჭდი მოწყობილობის შეერთებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При подключении печатающего устройство произошла ошибка: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьВыполнениеСверкиИтоговНаЭквайринговомТерминале_ПечатьЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='სლიპ-ჩეკის ამობეჭდვისას შეცდომა წარმოიშვა: %ДополнительноеОписание%';ru='При печати слип чека возникла ошибка: %ДополнительноеОписание%'");
...
Процедура НачатьВыводИнформацииНаДисплейПокупателя(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, СтрокиТекста = Неопределено) Экспорт
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено И ОповещениеПриЗавершении <> Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='მყიდველის შეერთებული დისპლეები არ არის.';ru='Нет подключенных дисплеев покупателя.'");
...
Процедура НачатьОперациюНаДисплейПокупателя_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ka='მყიდველის დისპლეის გამოყენებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При использовании дисплея покупателя произошла ошибка: %ДополнительноеОписание%'");
...
Процедура НачатьЗагрузкуДанныеИзТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, СворачиватьДанные = Ложь) Экспорт
	...
	ПредложитьВыбратьУстройство(Оповещение, "ТерминалСбораДанных", НСтр("ka='აირჩიეთ მონაცემთა შეკრების ტერმინალი';ru='Выберите терминал сбора данных'"), НСтр("ka='მონაცემთა შეკრების ტერმინალი არ არის შეერთებული.';ru='Терминал сбора данных не подключен.'"), ,Истина);
...
Процедура НачатьЗагрузкуДанныеИзТСД_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='მონაცემთა შეკრების ტერმინალი არ არის შეერთებული.';ru='Терминал сбора данных не подключен.'");
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьВыгрузкуДанныеВТСД_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = ""; 
	Иначе
		ОписаниеОшибки = НСтр("ka='მონაცემთა შეკრების ტერმინალში მონაცემთა გადმოტვირთვისას შეცდომა მოხდა: %ОписаниеОшибки%';ru='При выгрузке данных в терминал сбора данных произошла ошибка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьОчисткуДанныеВТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ТерминалСбораДанных",
			НСтр("ka='აირჩიეთ მონაცემთა შეკრების ტერმინალი';ru='Выберите терминал сбора данных'"), НСтр("ka='მონაცემთა შეკრების ტერმინალი არ არის შეერთებული.';ru='Терминал сбора данных не подключен.'"), ,Истина);
	Иначе
...
Процедура НачатьОчисткуДанныеВТСД_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = ""; 
	Иначе
		ОписаниеОшибки = НСтр("ka='მონაცემთა შეკრების ტერმინალში მონაცემთა გასუფთავებისას შეცდომა მოხდა: %ОписаниеОшибки%';ru='При очистке данных в терминале сбора данных произошла ошибка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьПолученияВесаСЭлектронныхВесов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ЭлектронныеВесы",
			НСтр("ka='აირჩიეთ ელექტრონული სასწორი';ru='Выберите электронные весы'"), 
			НСтр("ka='ელექტრონული სასწორი არ არის შეერთებული.';ru='Электронные весы не подключены.'"), 
			НСтр("ka='ელექტრონული სასწორი არ არის არჩეული.';ru='Электронные весы не выбраны.'"),
			Истина);
	Иначе
...
Процедура НачатьПолученияВесаСЭлектронныхВесов_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='ელექტრონული სასწორი არ არის არჩეული.';ru='Электронные весы не выбраны.'");
...
Процедура НачатьПолученияВесаСЭлектронныхВесов_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		Вес = Неопределено;
		ОписаниеОшибки = НСтр("ka='ელექტრონული სასწორის გამოყენებისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При использовании электронных весов произошла ошибка: %ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки);
		КонецЕсли;
...
Процедура НачатьПечатьЭтикеток(ОповещениеПриЗавершенииПечати, УникальныйИдентификатор, ОписаниеЭтикеткиВXML, ДанныеДляПечати, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда	
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ПринтерЭтикеток",
			НСтр("ka='აირჩიეთ ეტიკეტების პრინტერი';ru='Выберите принтер этикеток'"), 
			НСтр("ka='ეტიკეტების პრინტერი არ არის შეერთებული.';ru='Принтер этикеток не подключен.'"), 
			НСтр("ka='ეტიკეტების პრინტერი არ არის არჩეული.';ru='Принтер этикеток не выбран.'"), 
			Истина);
	Иначе
...
Процедура НачатьПечатьЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ეტიკეტების პრინტერი არ არის არჩეული.';ru='Принтер этикеток не выбран.'");
...
Процедура НачатьРедактированиеМакета(ОповещениеПриЗавершенииРедактирования, ОписаниеЭтикеткиВXML, АдресХранилищаСКД) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='ეტიკეტების რედაქტორის გამოყენება შეუძლებელია! ფუნქციურობას მხარდაჭერა არ გააჩნია.';ru='Использование редактора этикеток невозможно! Функциональность не поддерживается.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецПопытки;
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток(ОповещениеПриЗавершении, УникальныйИдентификатор, ТаблицаВыгрузкиТоваров, ИдентификаторУстройства = Неопределено, ЧастичнаяВыгрузка = Ложь, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ТаблицаВыгрузкиТоваров.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ka='გადმოსატვირთად მონაცემები არ არის!';ru='Нет данных для выгрузки!'");
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ВесыСПечатьюЭтикеток",
			НСтр("ka='აირჩიეთ ეტიკეტების მბეჭდავი სასწორი';ru='Выберите весы с печатью этикеток'"),
			НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის შეერთებული.';ru='Весы с печатью этикеток не подключены.'"), 
			НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის არჩეული.';ru='Весы с печатью этикеток не выбраны.'"), 
			Истина);
	Иначе
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის არჩეული.';ru='Весы с печатью этикеток не выбраны.'");
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток_ФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		...
		НачатьПодключениеОборудованиеПоИдентификатору(ОписаниеОповещения, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройства);
	Иначе
		ТекстСообщения = НСтр("ka='ვებ-კლიენტ ""1C:საწარმო""-სთვის დაყენებული გაფართოების გარეშე მოცემული ოპერაცია მიუწვდომელია.';ru='Данная операция не доступна без установленного расширения для веб-клиента ""1С:Предприятие"".'");
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		Если Параметры.СледующееОповещение <> Неопределено Тогда
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ka='ეტიკეტების მბეჭდავი სასწორი არ არის არჩეული.';ru='Весы с печатью этикеток не выбраны.'");
...
Процедура НачатьВыгрузкуНастроекВККМOffline(ОповещениеПриВыгрузкеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено,
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ККМOffline",
			НСтр("ka='აირჩიეთ სსა Offline';ru='Выберите ККМ Offline'"), НСтр("ka='სსა Offline არ არის შეერთებული.';ru='ККМ Offline не подключены.'"));
	Иначе
...
Процедура НачатьВыгрузкуНастроекВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს გამართვების გადმოტვირთვა სსა-ში Offline…';ru='Выполняется выгрузка настроек в ККМ Offline...'")); 
...
"%ОписаниеОшибки%';ka='მოწყობილობაში მონაცემთა გადმოტვირთვისას შეცდომა მოხდა. %ОписаниеОшибки%'");
			...
			Иначе
				ТекстСообщения = НСтр("ka='მონაცემები წარმატებით გადმოიტვირთა.';ru='Данные выгружены успешно.'");
			КонецЕсли;
...
Процедура НачатьОчисткуТоваровВККМOffline(ОповещениеПриОчисткеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Истина) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ККМOffline",
			НСтр("ka='აირჩიეთ სსა Offline';ru='Выберите ККМ Offline'"), 
			НСтр("ka='სსა Offline არ არის შეერთებული.';ru='ККМ Offline не подключены.'"),
...
Процедура НачатьОчисткуТоваровВККМOfflineЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ТекстСообщения = НСтр("ka='მოწყობილობა არ არის არჩეული.';ru='Оборудование не выбрано.'");
		Если Параметры.СледующееОповещение <> Неопределено Тогда
...
Процедура НачатьОчисткуТоваровВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს საქონლის გასუფთავება სსა-ში Offline...';ru='Выполняется очистка товаров в ККМ Offline...'"));
...
"%ОписаниеОшибки%';ka='მოწყობილობაში მონაცემთა გასუფთავებისას შეცდომა მოხდა. %ОписаниеОшибки%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
			Иначе
				ТекстСообщения = НСтр("ka='მონაცემთა წაშლა წარმატებით დასრულდა.';ru='Очистка данных успешно завершена.'");
			КонецЕсли;
...
Процедура НачатьВыгрузкуДанныеВККМOffline(ОповещениеПриВыгрузкеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено,
	...
	Иначе
		...
		Если МенеджерОборудованияВызовСервера.ПроверитьДублированиеКодов(РасширеннаяВыгрузка, ГруппыТоваров, Товары, ДублирующиесяКоды) Тогда
			...
			ТекстСообщения = НСтр("ka='გადმოტვირთვა შეუძლებელია! ხორციელდება საქონლის შემდეგი კოდების დუბლირება: %1';ru='Выгрузка невозможна! Дублируются следующие коды товаров: %1'");
...
Процедура НачатьВыгрузкуДанныеВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Параметры.РасширеннаяВыгрузка И НЕ ВремРасширеннаяВыгрузка Тогда
		ТекстСообщения = НСтр("ka='გაფართოებული გადმოტვირთვის ოპერაცია არჩეული დრაივერისთვის ხელმისაწვდომი არ არის.';ru='Операция расширенной выгрузки не доступна для выбранного драйвера.'");
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		Если Параметры.СледующееОповещение <> Неопределено Тогда
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს საქონლის გადმოტვირთვა სსა-ში Offline...';ru='Выполняется выгрузка товаров в ККМ Offline...'"));
...
Процедура НачатьЗагрузкуОтчетаОРозничныхПродажахИзККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Параметры.РасширеннаяЗагрузка И НЕ ВремРасширеннаяЗагрузка Тогда
		ТекстСообщения = НСтр("ka='არჩეული დრაივერისთვის გაფართოებული ჩატვირთვის ოპერაცია მიუწვდომელია.';ru='Операция расширенной загрузки не доступна для выбранного драйвера.'");
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		Если Параметры.СледующееОповещение <> Неопределено Тогда
	...
	Если Результат Тогда
		Состояние(НСтр("ka='მიმდინარეობს საქონლის ჩატვირთვა სსა-დან Offline...';ru='Выполняется загрузка товаров из ККМ Offline...'"));
...
"%ОписаниеОшибки%'");
			...
			Иначе
				...
				ТекстСообщения = НСтр("ka='მონაცემები წარმატებით ჩაიტვირთა.';ru='Данные загружены успешно.'");
			КонецЕсли;
...
Процедура НачатьОткрытиеСессииСчитывателяRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ka='RFID წამკითხველთან მუშაობისას შეცდომა მოხდა: %ДополнительноеОписание%';ru='При работе со считывателем RFID произошла ошибка: %ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьЗаписьДанныхВМеткуRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства, ПараметрыЗаписи, Таймаут = 0) Экспорт 
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "СчитывательRFID",
			НСтр("ka='აირჩიეთ RFID წამკითხველი';ru='Выберите RFID считыватель'"), НСтр("ru='Нет подключенных RFID считывателей.'"));
	Иначе
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='RFID წამკითხველი არ არის შეერთებული.';ru='RFID считыватель  не подключен.'");
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании();
			РезультатОперации.Результат = Ложь;
			РезультатОперации.ОписаниеОшибки = ОписаниеОшибки;
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ОписаниеОшибки = ""; 
	Если НЕ РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ka='RFID წამკითხველთან მუშაობისას შეცდომა მოხდა: %ОписаниеОшибки%';ru='При работе со считывателем RFID произошла ошибка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
	...
	#Иначе
		...
		Для Каждого Файл Из ПомещаемыеФайлы Цикл
			...
			Исключение
				...
				ТекстСообщения =  НСтр("ka='ფაილ %ИмяФайла%-ის წაკითხვისას შეცდომა მოხდა';ru='При чтении файла %ИмяФайла% произошла ошибка'");
...
Процедура ПолучитьСодержаниеТекстовыхФайловЗавершение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Тогда
		...
		РезультатЧтенияФайлов.ТекстОшибки = НСтр("ka='უცნობი შეცდომა სერვერზე ფაილების გადაცემისას.';ru='Неизвестная ошибка при передаче файлов на сервер.'");
