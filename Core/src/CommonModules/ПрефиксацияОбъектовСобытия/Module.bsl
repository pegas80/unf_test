...
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУникальныйПрефиксДляНумерацииДокументов") Тогда
	
		Отказ = Ложь;
		УстановитьУникальныйПрефикс(Источник, Префикс, Отказ);
		Если Не Отказ Тогда
...
Функция РеквизитОрганизацияДоступен(Объект)
	...
	Если   (ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта)
		...
		Если РеквизитОрганизация = Неопределено Тогда
			...
			СтрокаСообщения = НСтр("ka='მეტამონაცემთა ობიექტ %1-სთვის არ არის განსაზღვრული რეკვიზიტი სახელით %2.';ru='Для объекта метаданных %1 не определен реквизит с именем %2.';en='For metadata object% 1 is not defined props named% 2.'");
...
Процедура УстановитьУникальныйПрефикс(Источник, Префикс, Отказ)
	
	Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(Источник.Ссылка)) Тогда
	
		МетаданныеДокумента = Источник.Метаданные();
		СсылкаНаДокумент = Документы[МетаданныеДокумента.Имя].ПустаяСсылка();
		ДанныеПрефикса = ПрефиксацияОбъектовПовтИсп.ПолучитьДанныеПрефиксовДокумента(СсылкаНаДокумент);
		Если НЕ ЗначениеЗаполнено(ДанныеПрефикса.ПрефиксДокумента) Тогда
		...
		СимвКом = ПолучитьФункциональнуюОпцию("ПрефиксыОрганизаций", Новый Структура("Организация", Источник.Организация));
		СимвДокумента = ДанныеПрефикса.ПрефиксДокумента;
		ПрефиксГода = Формат(Источник.Дата, "ДФ=yy");
		ПрефиксМесяца = Формат(Источник.Дата, "ДФ=MM");
		
		ШаблонПрефикса = "[COM][GS][YY][MM]";
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[COM]", СимвКом);
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[GS]", СимвДокумента);
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[YY]", ?(ДанныеПрефикса.ГодВПрефиксе, ПрефиксГода, ""));
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[MM]", ?(ДанныеПрефикса.МесяцВПрефиксе, ПрефиксМесяца, ""));
