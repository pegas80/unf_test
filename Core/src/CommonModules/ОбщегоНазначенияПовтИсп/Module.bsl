...
Функция ПолучитьПреобразованиеXSLИзОбщегоМакета(Знач ИмяОбщегоМакета) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='XSL-ის მიღება';ru='Получение XSL'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция РазделенныеОбъектыМетаданных(Знач Разделитель) Экспорт
	...
	Если МетаданныеОбщегоРеквизита = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საერთო რეკვიზიტი %1 კონფიგურაციაში ვერ მოიძებნა!';ru='Общий реквизит %1 не обнаружен в конфигурации!'"), Разделитель);
	КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საერთო რეკვიზიტ %1-ისთვის მონაცემთა დაყოფა არ გამოიყენება!';ru='Для общего реквизита %1 не используется разделение данных!'"), Разделитель);
	...
	Для Каждого МетаданныеПоследовательности Из Метаданные.Последовательности Цикл
		...
		Если МетаданныеПоследовательности.Документы.Количество() = 0 Тогда
			...
			ШаблонСообщения = НСтр("ka='თანმიმდევრობა %1-ში არ არის ჩართული არცერთი დოკუმენტი.';ru='В последовательность %1 не включено ни одного документа.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, МетаданныеПоследовательности.Имя);
			ЗаписьЖурналаРегистрации(НСтр("ka='მეტამონაცემთა განცალკევებული ობიექტების მიღება';ru='Получение разделенных объектов метаданных'", 
	...
	Для Каждого МетаданныеЖурналаДокументов Из Метаданные.ЖурналыДокументов Цикл
		...
		Если МетаданныеЖурналаДокументов.РегистрируемыеДокументы.Количество() = 0 Тогда
			...
			ШаблонСообщения = НСтр("ka='ჟურნალ %1-ში არ არის ჩართული არცერთი დოკუმენტი.';ru='В журнал %1 не включено ни одного документа.'");
...
Функция ДанныеКэшаВерсий(Знач Идентификатор, Знач ТипДанных, Знач ПараметрыПолучения, Знач ИспользоватьУстаревшиеДанные = Истина) Экспорт
	...
	Если ТребуетсяВыполнитьОбновление Тогда
		...
		Иначе
			...
			НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვერსიების ქეშის განახლება. ჩანაწერის იდენტიფიკატორი %1. მონაცემთა ტიპი %2.';ru='Обновление кэша версий. Идентификатор записи %1. Тип данных %2.'"),
				Идентификатор,
				ТипДанных);
...
Функция ОпцииИнтерфейса() Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ka='ინტერფაისის აწყობა';ru='Настройка интерфейса'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
