...
Функция ПолучитьПреобразованиеXSLИзОбщегоМакета(Знач ИмяОбщегоМакета) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='XSL-ის მიღება';ru='Получение XSL';en='Getting XSL'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция РазделенныеОбъектыМетаданных(Знач Разделитель) Экспорт
	...
	Если МетаданныеОбщегоРеквизита = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საერთო რეკვიზიტი %1 კონფიგურაციაში ვერ მოიძებნა!';ru='Общий реквизит %1 не обнаружен в конфигурации!';en='Total Props 1% is not found in the database!'"), Разделитель);
	КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საერთო რეკვიზიტ %1-ისთვის მონაცემთა დაყოფა არ გამოიყენება!';ru='Для общего реквизита %1 не используется разделение данных!';en='For general props% 1 is not used data sharing!'"), Разделитель);
	...
	Для Каждого МетаданныеПоследовательности Из Метаданные.Последовательности Цикл
		...
		Если МетаданныеПоследовательности.Документы.Количество() = 0 Тогда
			...
			ШаблонСообщения = НСтр("ka='თანმიმდევრობა %1-ში არ არის ჩართული არცერთი დოკუმენტი.';ru='В последовательность %1 не включено ни одного документа.';en='In sequence 1% are not included any documents.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, МетаданныеПоследовательности.Имя);
			ЗаписьЖурналаРегистрации(НСтр("ka='მეტამონაცემთა განცალკევებული ობიექტების მიღება';ru='Получение разделенных объектов метаданных';en='Getting separated metadata objects'", 
	...
	Для Каждого МетаданныеЖурналаДокументов Из Метаданные.ЖурналыДокументов Цикл
		...
		Если МетаданныеЖурналаДокументов.РегистрируемыеДокументы.Количество() = 0 Тогда
			...
			ШаблонСообщения = НСтр("ka='ჟურნალ %1-ში არ არის ჩართული არცერთი დოკუმენტი.';ru='В журнал %1 не включено ни одного документа.';en='In the journal% 1 does not include any document.'");
...
Функция ДанныеКэшаВерсий(Знач Идентификатор, Знач ТипДанных, Знач ПараметрыПолучения, Знач ИспользоватьУстаревшиеДанные = Истина) Экспорт
	...
	Если ТребуетсяВыполнитьОбновление Тогда
		...
		Иначе
			...
			НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვერსიების ქეშის განახლება. ჩანაწერის იდენტიფიკატორი %1. მონაცემთა ტიპი %2.';ru='Обновление кэша версий. Идентификатор записи %1. Тип данных %2.';en='Updating cache versions. Record Identifier 1%. Data Type 2%.'"),
				Идентификатор,
				ТипДанных);
...
"Тип данных: %2';ka='შეცდომა ვერსიის ქეშის განახლებისას. მონაცემები არ არის მიღებული."
"ჩანაწერის იდენტიფიკატორი: %1"
"მონაცემთა ტიპი: %2'");
...
Функция ОпцииИнтерфейса() Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ka='ინტერფაისის აწყობა';ru='Настройка интерфейса';en='Customize interface'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"%1';ka='ინტერფეისის ოპციის მიღებისას შეცდომა მოხდა:"
"%1'"),
