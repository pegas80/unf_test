...
Функция ПолучитьСообщениеОбОшибке(КодОшибки, СтороннееОписаниеОшибки = "") Экспорт
	...
	ШаблонСообщения = НСтр("ka='შეცდომის კოდი %1. %2';ru='Код ошибки %1. %2';en='Error Code% 1. 2%'");
...
Процедура ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, ТекстСообщения = "", КодСобытия = 2, СсылкаНаОбъект = Неопределено) Экспорт
	...
	Если ЭтоПолноправныйПользователь И ЗначениеЗаполнено(ПодробныйТекстОшибки) И НЕ ПустаяСтрока(ТекстСообщения)
		...
		ТекстСообщения = ТекстСообщения + Символы.ПС
			+ НСтр("ka='დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Подробности см. в журнале регистрации.';en='For details, see. In the log.'");
	КонецЕсли;
	...
	КонецЕсли;
	ТекстОшибки = СтрШаблон(НСтр("ka='ოპერაციის შესრულება: %1"
...
Функция ТекстСообщенияОНеобходимостиНастройкиСистемы(ВидОперации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекстСообщения) Тогда
		Если ВРег(ВидОперации) = "РАБОТАСЭД" Тогда
			ТекстСообщения = НСтр("ka='ელექტრონულ დოკუმენტებთან სამუშაოდ აუცილებელია სისტემის გამართვებში ელექტრონული დოკუმენტების გაცვლის გამოყენების ჩართვა.';ru='Для работы с электронными документами необходимо"
"в настройках системы включить использование обмена электронными документами.'");
		ИначеЕсли ВРег(ВидОперации) = "ПОДПИСАНИЕЭД" Тогда
			ТекстСообщения = НСтр("ka='ედ-ის ხელმოწერის შესაძლებლობისთვის აუცილებელია სისტემის გამართვებში ელექტრონული ციფრული ხელმოწერების გამოყენების ოპციის ჩართვა.';ru='Для возможности подписания ЭД необходимо"
"в настройках системы включить опцию использования электронных цифровых подписей.'");
		ИначеЕсли ВРег(ВидОперации) = "НАСТРОЙКАКРИПТОГРАФИИ" Тогда
			ТекстСообщения = НСтр("ka='კრიპტოგრაფიის გამართვის შესაძლებლობისთვის აუცილებელია სისტემის გამართვებში ელექტრონული ციფრული ხელმოწერების გამოყენების ოპციის ჩართვა.';ru='Для возможности настройки криптографии необходимо "
"в настройках системы включить опцию использования электронных цифровых подписей.'");
		ИначеЕсли ВРег(ВидОперации) = "РАБОТАСБАНКАМИ" Тогда
			ТекстСообщения = НСтр("ka='ბანკებთან ედ-ების გაცვლის შესაძლებლობისთვის, აუცილებელია სისტემის გამართვებში ბანკებთან პირდაპირი ურთიერთქმედების გამოყენების ოპციის ჩართვა.';ru='Для возможности обмена ЭД с банками необходимо "
"в настройках системы включить опцию использования прямого взаимодействия с банками.'");
		Иначе
			ТекстСообщения = НСтр("ka='Операция не может быть выполнена. Не выполнены необходимые настройки системы.';ru='Операция не может быть выполнена. Не выполнены необходимые настройки системы.';en='The operation can not be performed. Not made the necessary system settings.'");
		КонецЕсли;
	КонецЕсли;
...
" - изменить размещение каталога временных файлов в процедуре ""ОбменСКонтрагентамиПереопределяемый.ТекущийКаталогВременныхФайлов"".';ka='Невозможно извлечь файлы из архива. Путь к файлам архива должен быть короче 256 символов."
"Возможные способы устранения ошибки:"
" - в настройках операционной системы, в переменных среды, изменить путь к временным файлам;"
" - изменить размещение каталога временных файлов в процедуре ""ОбменСКонтрагентамиПереопределяемый.ТекущийКаталогВременныхФайлов"".'"));
	// Коды ошибок 1С
	СообщенияОшибок.Вставить("0", НСтр("ka='მოთხოვნაში არსებული ერთ-ერთი ხელმოწერა უცნობ პირს ეკუთვნის.';ru='Одна из имеющихся в запросе подписей принадлежит неизвестному лицу.';en='One available in a query signatures belongs to an unknown person.'"));
	СообщенияОшибок.Вставить("2", НСтр("ka='ერთ-ერთი ხელმოწერა არასწორია';ru='Одна из подписей неверна';en='One of the signatures were invalid'"));
	СообщенияОшибок.Вставить("3", НСтр("ka='წარმოდგენილი უნდა იყოს ორი სხვადასხვა ხელმოწერა.';ru='Должны быть представлены две разные подписи.';en='two different signatures should be submitted.'"));
	СообщенияОшибок.Вставить("4", НСтр("ru='Неверный тип содержимого: двоичный.';en='Invalid content type: Binary.'"));
	СообщенияОшибок.Вставить("5", НСтр("ka='შემოთავაზებული უნდა იყოს თუნდ ერთი ხელმოწერა.';ru='Должна быть предоставлена хотя бы одна подпись.';en='It must be given at least one signature.'"));
	СообщенияОшибок.Вставить("6", НСтр("ka='ყველა ხელმოწერა არ განსხვავდება.';ru='Не все подписи отличаются.';en='Not all signs are different.'"));
	СообщенияОшибок.Вставить("7", НСтр("ka='ყველა პრეფიქსი არ უზრუნველყოფს ოპერაციისთვის აუცილებელ უფლებამოსილების დონეს.';ru='Все подписи не обеспечивают уровень полномочий, необходимых для операции.';en='All signatures do not provide the level of authority required for the operation.'"));
	СообщенияОшибок.Вставить("8", НСтр("ka='Один из подписантов неизвестен.';ru='Один из подписантов неизвестен.';en='One of the signatories is unknown.'"));
	СообщенияОшибок.Вставить("9", НСтр("ka='სატრანსპორტო მიმოსვლის ტიპის შემცველობა არის არასწორი, მოსალოდნელია: application/xml.';ru='Содержимое типа транспортного сообщения является неправильным, ожидается: application/xml.';en='Content type of transportation is wrong, it is expected: application / xml.'"));
	СообщенияОшибок.Вставить("10", НСтр("ka='საქმიანი შეტყობინების შემცველობა არასწორია, მოსალოდნელია: application/xml.';ru='Содержимое типа делового сообщения неверно, ожидается: application/xml.';en='Content type of business communication is wrong, it is expected: application / xml.'"));
	СообщенияОшибок.Вставить("11", НСтр("ka='ყველა ხელმოწერა არ შეესაბამება ერთი და იგივე კლიენტს.';ru='Не все подписи соответствуют одному и тому же клиенту.';en='Not all the signatures correspond to one and the same client.'"));
	СообщенияОшибок.Вставить("12", НСтр("ka='მოთხოვნაში არსებული ყველა ხელმოწერა საკმარისია იმისათვის, რომ მიღებული იქნას მოთხოვნილ ანგარიშთან დაშვების უფლება.';ru='Всех имеющихся в запросе подписей недостаточно для того, чтобы получить право на доступ к запрашиваемому счету.';en='All available in the query is not enough signatures to qualify for access to the requested account.'"));
	СообщенияОшибок.Вставить("13", НСтр("ka='HTTP მოთხოვნა URL არასწორია. მხარდაჭერილია მხოლოდ რესურსების და მდგომარეობის მოთხოვნები.';ru='HTTP запрос URL неверный. Поддерживаются только запросы ресурсов и состояния.';en='HTTP request URL is incorrect. Only supported resources and status requests.'"));
	СообщенияОшибок.Вставить("14", НСтр("ka='სატრანსპორტო კონტეინერის შემოწმების შეცდომა.';ru='Ошибка проверки транспортного контейнера.';en='Error checking of the shipping container.'"));
	СообщенияОшибок.Вставить("15", НСтр("ru='Ошибка проверки контейнера бизнес данных."
"Необходимо обратиться в тех.поддержку банка';ka='Ошибка проверки контейнера бизнес данных."
"Необходимо обратиться в тех.поддержку банка'"));
	СообщенияОшибок.Вставить("16", НСтр("ka='ანგარიშის ამონაწერში საკმაოდ მცირე საწყისი თარიღი არის.';ru='В выписке счета слишком малая начальная дата.';en='The statement of account is too small start date.'"));
	СообщенияОшибок.Вставить("17", НСтр("ka='ანგარიშის ამონაწერში საკმაოდ დიდი საბოლოო თარიღია.';ru='В выписке счета слишком большая конечная дата.';en='The statement of account is too big end date.'"));
	СообщенияОшибок.Вставить("18", НСтр("ka='დოკუმენტის არასწორი თარიღი.';ru='Неверная дата документа.';en='Incorrect date of the document.'"));
	СообщенияОшибок.Вставить("19", НСтр("ka='უნებართვო ინსტრუქცია';ru='Неразрешенная инструкция.';en='Unauthorized instruction.'"));
	СообщенияОшибок.Вставить("21", НСтр("ru='Неразрешенная инструкция.';en='Unauthorized instruction.'"));
	
	СообщенияОшибок.Вставить("100", НСтр("ka='კრიპტოგრაფიის მენეჯერის შექმნა კომპიუტერზე ვერ მოხერხდა.';ru='Не удалось создать менеджер криптографии на компьютере.';en='Failed to create cryptography manager on the computer.'"));
	СообщенияОшибок.Вставить("101", НСтр("ru='Сертификат не найден в хранилище сертификатов на компьютере.';en='The certificate is not found in the certificate store on your computer.'"));
	СообщенияОшибок.Вставить("102", НСтр("ka='სერთიფიკატი უმოქმედოა.';ru='Сертификат не действителен.';en='The certificate is not valid.'"));
	СообщенияОшибок.Вставить("103", НСтр("ka='კომპიუტერზე დაშიფრვის/გაშიფრვის ოპერაციის შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции шифрования/расшифровки на компьютере.';en='Unable to perform the operation on the computer encryption / decryption.'"));
	СообщенияОшибок.Вставить("104", НСтр("ka='კომპიუტერზე ეხ-ის შედგენის/შემოწმების ოპერაციების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции формирования/проверки ЭП на компьютере.';en='Unable to perform an operation of generating / checking EP on the computer.'"));
	СообщенияОшибок.Вставить("105", НСтр("ka='კომპიუტერში სერთიფიკატების საცავში ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов в хранилище сертификатов на компьютере.';en='No certificates available in the certificate store on your computer.'"));
	
	СообщенияОшибок.Вставить("110", НСтр("ka='სერვერზე კრიპტოგრაფიის მენეჯერის შექმნა ვერ მოხერხდა.';ru='Не удалось создать менеджер криптографии на сервере.';en='Failed to create cryptography manager on the server.'"));
	СообщенияОшибок.Вставить("111", НСтр("ka='Сертификат не найден в хранилище сертификатов на сервере.';ru='Сертификат не найден в хранилище сертификатов на сервере.';en='The certificate is not found in the certificate store on the server.'"));
	СообщенияОшибок.Вставить("112", НСтр("ka='სერთიფიკატი უმოქმედოა.';ru='Сертификат не действителен.';en='The certificate is not valid.'"));
	СообщенияОшибок.Вставить("113", НСтр("ka='სერვერზე დაშიფრვის/გაშიფრვის ოპერაციების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции шифрования/расшифровки на сервере.';en='Unable to perform the operation encryption / decryption on the server.'"));
	СообщенияОшибок.Вставить("114", НСтр("ka='სერვერზე ეხ-ის ჩამოყალიბების/შემოწმების ოპერაციების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции формирования/проверки ЭП на сервере.';en='Unable to perform an operation of generating / checking EP server.'"));
	СообщенияОшибок.Вставить("115", НСтр("ka='სერვერზე არსებულ სერთიფიკატების საცავში ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов в хранилище сертификатов на сервере.';en='No certificates available in the certificate store on the server.'"));
	
	СообщенияОшибок.Вставить("106", НСтр("ka='Версия платформы 1С ниже ""8.2.17"".';ru='Версия платформы 1С ниже ""8.2.17"".';en='platform version of 1C below ""8.2.17"".'"));
	СообщенияОшибок.Вставить("107", НСтр("ka='გაცვლის კატალოგების შექმნა ვერ მოხერხდა.';ru='Не удалось создать каталоги обмена.';en='Could not create an exchange directories.'"));
	
	СообщенияОшибок.Вставить("121", НСтр("ru='Не удалось соединиться с FTP сервером.';en='Could not connect to the FTP server.'"));
	СообщенияОшибок.Вставить("122", НСтр("ka='კატალოგის შექმნა შეუძლებელია, ვინაიდან FTP რესურსზე არსებობს ასეთი სახელის მქონე ფაილი.';ru='Невозможно создать каталог, так как на FTP ресурсе существует файл с таким именем.';en='Unable to create directory as an FTP resource exists with the file name.'"));
	СообщенияОшибок.Вставить("123", НСтр("ka='კატალოგის შექმნა შეუძლებელია.';ru='Невозможно создать каталог.';en='Unable to create directory.'"));
	СообщенияОшибок.Вставить("124", НСтр("ru='Невозможно открыть каталог.';en='Unable to open the folder.'"));
	СообщенияОшибок.Вставить("125", НСтр("ka='FTP რესურსზე ფაილების ძებნისას შეცდომა მოხდა.';ru='Произошла ошибка при поиске файлов на FTP ресурсе.';en='An error occurred while searching for files on an FTP share.'"));
	СообщенияОшибок.Вставить("126", НСтр("ka='Различаются данные записанного, а затем прочитанного тестового файла в каталоге.';ru='Различаются данные записанного, а затем прочитанного тестового файла в каталоге.';en='Different data is written, and then read the test file in the directory.'"));
	СообщенияОшибок.Вставить("127", НСтр("ka='კატალოგში ფაილის ჩაწერა ვერ მოხერხდა.';ru='Не удалось записать файл в каталог.';en='Could not write the file to the directory.'"));
	СообщенияОшибок.Вставить("128", НСтр("ru = 'Не удалось прочитать файл в каталоге."));
	СообщенияОшибок.Вставить("129", НСтр("ka='Не удалось удалить файл.';ru='Не удалось удалить файл.';en='Unable to delete the file.'"));
	...
	СообщенияОшибок.Вставить("0106", НСтр("ka='ხელშეკრულებაში მითითებულია არაკორექტული ფორმატის ორგანიზაციის იდენტიფიკატორი.';ru='В соглашении указан идентификатор организации некорректного формата.';en='The agreement specifies the ID of the organization malformed.'")); // Неверный формат идентификатора отправителя (название параметра?) в файле card.xml 400 0106
	...
	// Асинхронный режим
	СообщенияОшибок.Вставить("0202", НСтр("ka='ხელშეკრულებაში მითითებულია კონტრაგენტის იდენტიფიკატორი, რომელიც არ არის ტაქსკომში რეგისტრირებული.';ru='В соглашении указан идентификатор контрагента не зарегистрированный в Такском.';en='The agreement counterparty identifier specified is not registered in Taxcom.'")); // Получатель с указанным идентификатором не зарегистрирован 0202
