...
Функция ПолучитьСообщениеОбОшибке(КодОшибки, СтороннееОписаниеОшибки = "") Экспорт
	...
	ШаблонСообщения = НСтр("ka='შეცდომის კოდი %1. %2';ru='Код ошибки %1. %2'");
...
Процедура ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, ТекстСообщения = "", КодСобытия = 2, СсылкаНаОбъект = Неопределено) Экспорт
	...
	Если ЭтоПолноправныйПользователь И ЗначениеЗаполнено(ПодробныйТекстОшибки) И НЕ ПустаяСтрока(ТекстСообщения)
		...
		ТекстСообщения = ТекстСообщения + Символы.ПС
			+ НСтр("ka='დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Подробности см. в журнале регистрации.'");
	КонецЕсли;
...
"в настройках системы включить опцию использования прямого взаимодействия с банками.'");
		Иначе
			ТекстСообщения = НСтр("ka='Операция не может быть выполнена. Не выполнены необходимые настройки системы.';ru='Операция не может быть выполнена. Не выполнены необходимые настройки системы.'");
		КонецЕсли;
	КонецЕсли;
...
" - изменить размещение каталога временных файлов в процедуре ""ОбменСКонтрагентамиПереопределяемый.ТекущийКаталогВременныхФайлов"".'"));
	// Коды ошибок 1С
	СообщенияОшибок.Вставить("0", НСтр("ka='მოთხოვნაში არსებული ერთ-ერთი ხელმოწერა უცნობ პირს ეკუთვნის.';ru='Одна из имеющихся в запросе подписей принадлежит неизвестному лицу.'"));
	СообщенияОшибок.Вставить("2", НСтр("ka='ერთ-ერთი ხელმოწერა არასწორია';ru='Одна из подписей неверна'"));
	СообщенияОшибок.Вставить("3", НСтр("ka='წარმოდგენილი უნდა იყოს ორი სხვადასხვა ხელმოწერა.';ru='Должны быть представлены две разные подписи.'"));
	СообщенияОшибок.Вставить("4", НСтр("ru='Неверный тип содержимого: двоичный.'"));
	СообщенияОшибок.Вставить("5", НСтр("ka='შემოთავაზებული უნდა იყოს თუნდ ერთი ხელმოწერა.';ru='Должна быть предоставлена хотя бы одна подпись.'"));
	СообщенияОшибок.Вставить("6", НСтр("ka='ყველა ხელმოწერა არ განსხვავდება.';ru='Не все подписи отличаются.'"));
	СообщенияОшибок.Вставить("7", НСтр("ka='ყველა პრეფიქსი არ უზრუნველყოფს ოპერაციისთვის აუცილებელ უფლებამოსილების დონეს.';ru='Все подписи не обеспечивают уровень полномочий, необходимых для операции.'"));
	СообщенияОшибок.Вставить("8", НСтр("ka='Один из подписантов неизвестен.';ru='Один из подписантов неизвестен.'"));
	СообщенияОшибок.Вставить("9", НСтр("ka='სატრანსპორტო მიმოსვლის ტიპის შემცველობა არის არასწორი, მოსალოდნელია: application/xml.';ru='Содержимое типа транспортного сообщения является неправильным, ожидается: application/xml.'"));
	СообщенияОшибок.Вставить("10", НСтр("ka='საქმიანი შეტყობინების შემცველობა არასწორია, მოსალოდნელია: application/xml.';ru='Содержимое типа делового сообщения неверно, ожидается: application/xml.'"));
	СообщенияОшибок.Вставить("11", НСтр("ka='ყველა ხელმოწერა არ შეესაბამება ერთი და იგივე კლიენტს.';ru='Не все подписи соответствуют одному и тому же клиенту.'"));
	СообщенияОшибок.Вставить("12", НСтр("ka='მოთხოვნაში არსებული ყველა ხელმოწერა საკმარისია იმისათვის, რომ მიღებული იქნას მოთხოვნილ ანგარიშთან დაშვების უფლება.';ru='Всех имеющихся в запросе подписей недостаточно для того, чтобы получить право на доступ к запрашиваемому счету.'"));
	СообщенияОшибок.Вставить("13", НСтр("ka='HTTP მოთხოვნა URL არასწორია. მხარდაჭერილია მხოლოდ რესურსების და მდგომარეობის მოთხოვნები.';ru='HTTP запрос URL неверный. Поддерживаются только запросы ресурсов и состояния.'"));
	СообщенияОшибок.Вставить("14", НСтр("ka='სატრანსპორტო კონტეინერის შემოწმების შეცდომა.';ru='Ошибка проверки транспортного контейнера.'"));
...
"Необходимо обратиться в тех.поддержку банка'"));
	СообщенияОшибок.Вставить("16", НСтр("ka='ანგარიშის ამონაწერში საკმაოდ მცირე საწყისი თარიღი არის.';ru='В выписке счета слишком малая начальная дата.'"));
	СообщенияОшибок.Вставить("17", НСтр("ka='ანგარიშის ამონაწერში საკმაოდ დიდი საბოლოო თარიღია.';ru='В выписке счета слишком большая конечная дата.'"));
	СообщенияОшибок.Вставить("18", НСтр("ka='დოკუმენტის არასწორი თარიღი.';ru='Неверная дата документа.'"));
	СообщенияОшибок.Вставить("19", НСтр("ka='უნებართვო ინსტრუქცია';ru='Неразрешенная инструкция.'"));
	СообщенияОшибок.Вставить("21", НСтр("ru='Неразрешенная инструкция.'"));
	
	СообщенияОшибок.Вставить("100", НСтр("ka='კრიპტოგრაფიის მენეჯერის შექმნა კომპიუტერზე ვერ მოხერხდა.';ru='Не удалось создать менеджер криптографии на компьютере.'"));
	СообщенияОшибок.Вставить("101", НСтр("ru='Сертификат не найден в хранилище сертификатов на компьютере.'"));
	СообщенияОшибок.Вставить("102", НСтр("ka='სერთიფიკატი უმოქმედოა.';ru='Сертификат не действителен.'"));
	СообщенияОшибок.Вставить("103", НСтр("ka='კომპიუტერზე დაშიფრვის/გაშიფრვის ოპერაციის შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции шифрования/расшифровки на компьютере.'"));
	СообщенияОшибок.Вставить("104", НСтр("ka='კომპიუტერზე ეხ-ის შედგენის/შემოწმების ოპერაციების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции формирования/проверки ЭП на компьютере.'"));
	СообщенияОшибок.Вставить("105", НСтр("ka='კომპიუტერში სერთიფიკატების საცავში ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов в хранилище сертификатов на компьютере.'"));
	
	СообщенияОшибок.Вставить("110", НСтр("ka='სერვერზე კრიპტოგრაფიის მენეჯერის შექმნა ვერ მოხერხდა.';ru='Не удалось создать менеджер криптографии на сервере.'"));
	СообщенияОшибок.Вставить("111", НСтр("ka='Сертификат не найден в хранилище сертификатов на сервере.';ru='Сертификат не найден в хранилище сертификатов на сервере.'"));
	СообщенияОшибок.Вставить("112", НСтр("ka='სერთიფიკატი უმოქმედოა.';ru='Сертификат не действителен.'"));
	СообщенияОшибок.Вставить("113", НСтр("ka='სერვერზე დაშიფრვის/გაშიფრვის ოპერაციების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции шифрования/расшифровки на сервере.'"));
	СообщенияОшибок.Вставить("114", НСтр("ka='სერვერზე ეხ-ის ჩამოყალიბების/შემოწმების ოპერაციების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить операции формирования/проверки ЭП на сервере.'"));
	СообщенияОшибок.Вставить("115", НСтр("ka='სერვერზე არსებულ სერთიფიკატების საცავში ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов в хранилище сертификатов на сервере.'"));
	
	СообщенияОшибок.Вставить("106", НСтр("ka='Версия платформы 1С ниже ""8.2.17"".';ru='Версия платформы 1С ниже ""8.2.17"".'"));
	СообщенияОшибок.Вставить("107", НСтр("ka='გაცვლის კატალოგების შექმნა ვერ მოხერხდა.';ru='Не удалось создать каталоги обмена.'"));
	
	СообщенияОшибок.Вставить("121", НСтр("ru='Не удалось соединиться с FTP сервером.'"));
	СообщенияОшибок.Вставить("122", НСтр("ka='კატალოგის შექმნა შეუძლებელია, ვინაიდან FTP რესურსზე არსებობს ასეთი სახელის მქონე ფაილი.';ru='Невозможно создать каталог, так как на FTP ресурсе существует файл с таким именем.'"));
	СообщенияОшибок.Вставить("123", НСтр("ka='კატალოგის შექმნა შეუძლებელია.';ru='Невозможно создать каталог.'"));
	СообщенияОшибок.Вставить("124", НСтр("ru='Невозможно открыть каталог.'"));
	СообщенияОшибок.Вставить("125", НСтр("ka='FTP რესურსზე ფაილების ძებნისას შეცდომა მოხდა.';ru='Произошла ошибка при поиске файлов на FTP ресурсе.'"));
	СообщенияОшибок.Вставить("126", НСтр("ka='Различаются данные записанного, а затем прочитанного тестового файла в каталоге.';ru='Различаются данные записанного, а затем прочитанного тестового файла в каталоге.'"));
	СообщенияОшибок.Вставить("127", НСтр("ka='კატალოგში ფაილის ჩაწერა ვერ მოხერხდა.';ru='Не удалось записать файл в каталог.'"));
	СообщенияОшибок.Вставить("128", НСтр("ru = 'Не удалось прочитать файл в каталоге."));
	СообщенияОшибок.Вставить("129", НСтр("ka='Не удалось удалить файл.';ru='Не удалось удалить файл.'"));
	...
	СообщенияОшибок.Вставить("0106", НСтр("ka='ხელშეკრულებაში მითითებულია არაკორექტული ფორმატის ორგანიზაციის იდენტიფიკატორი.';ru='В соглашении указан идентификатор организации некорректного формата.'")); // Неверный формат идентификатора отправителя (название параметра?) в файле card.xml 400 0106
	...
	// Асинхронный режим
	СообщенияОшибок.Вставить("0202", НСтр("ka='ხელშეკრულებაში მითითებულია კონტრაგენტის იდენტიფიკატორი, რომელიც არ არის ტაქსკომში რეგისტრირებული.';ru='В соглашении указан идентификатор контрагента не зарегистрированный в Такском.'")); // Получатель с указанным идентификатором не зарегистрирован 0202
