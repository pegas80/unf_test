...
Процедура НастроитьОбменШаг1(Сообщение, Отправитель)
	...
	Попытка
		...
		Если Не ПустаяСтрока(КодЭтогоУзла)
			...
			СтрокаСообщения = НСтр("ka='ამ დანართში წინასწარგანსაზღვრული კვანძის კოდი ""%1"" არ შეესაბამება მოსალოდნელს ""%2"". გაცვლის გეგმა: %3';ru='Код предопределенного узла в этом приложении ""%1"" не соответствует ожидаемому ""%2"". План обмена: %3'");
...
Функция КорреспондентОбмена(Знач ИмяПланаОбмена, Знач Код)
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		СтрокаСообщения = НСтр("ka='გაცვლის გეგმის კვანძი ვერ მოიძებნა; გაცვლის გეგმის სახელი %1; კვანძის კოდი %2';ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2'");
