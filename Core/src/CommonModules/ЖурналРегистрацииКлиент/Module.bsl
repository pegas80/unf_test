...
//     НСтр("ka='ინტერნეტთან დაკავშირება შეუძლებელია განახლებების შესამოწმებლად.';ru='Невозможно подключиться к сети Интернет для проверки обновлений.';en='Unable to connect to the Internet to check for updates.'"));
//
...
Процедура ОткрытьДанныеДляПросмотра(ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.Данные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='რეგისტრაციის ჟურნალის ეს ჩანაწერი არ არის დაკავშირებული მონაცემებთან (იხ. სვეტი ""მონაცემები"")';ru='Эта запись журнала регистрации не связана с данными (см. колонку ""Данные"")';en='This log entry is not linked to the data (see. The column ""Data"")'"));
		Возврат;
	КонецЕсли;
...
"%1';ka='რეგისტრაციის ჟურნალის ეს ჩანაწერი დაკავშირებულია მონაცემებთან, მაგრამ მათი გამოსახვა შეუძლებელია."
"%1'");
		Если ТекущиеДанные.Событие = "_$Data$_.Delete" Тогда 
			// это - событие удаления
			ТекстПредупреждения =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, НСтр("ka='მონაცემები წაშლილია საინფორმაციო ბაზიდან';ru='Данные удалены из информационной базы';en='Data removed from the knowledge base'"));
		Иначе
			ТекстПредупреждения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, НСтр("ka='შეიძლება, მონაცემები წაშლილია საინფორმაციო ბაზიდან';ru='Возможно, данные удалены из информационной базы';en='The data may be removed from the knowledge base'"));
		КонецЕсли;
