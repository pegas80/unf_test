...
//     НСтр("ka='ინტერნეტთან დაკავშირება შეუძლებელია განახლებების შესამოწმებლად.';ru='Невозможно подключиться к сети Интернет для проверки обновлений.'"));
//
...
Процедура ОткрытьДанныеДляПросмотра(ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.Данные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='რეგისტრაციის ჟურნალის ეს ჩანაწერი არ არის დაკავშირებული მონაცემებთან (იხ. სვეტი ""მონაცემები"")';ru='Эта запись журнала регистрации не связана с данными (см. колонку ""Данные"")'"));
		Возврат;
	КонецЕсли;
...
"%1'");
		Если ТекущиеДанные.Событие = "_$Data$_.Delete" Тогда 
			...
			ТекстПредупреждения =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, НСтр("ka='მონაცემები წაშლილია საინფორმაციო ბაზიდან';ru='Данные удалены из информационной базы'"));
		Иначе
			ТекстПредупреждения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, НСтр("ka='შეიძლება, მონაცემები წაშლილია საინფორმაციო ბაზიდან';ru='Возможно, данные удалены из информационной базы'"));
		КонецЕсли;
