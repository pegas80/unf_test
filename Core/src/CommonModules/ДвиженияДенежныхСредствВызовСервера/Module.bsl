...
Функция ЗаголовокНадписиОстатковДС(СчетКасса, Организация, ОстатокДенежныхСредств = 0) Экспорт
	...
	КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='ნაშთი';ru='Остаток'") + " ", КрупныйШрифт));
...
Функция ПолучитьСписокВидовОперацийПоступленияДСБанк() Экспорт
	...
	СписокОпераций.Добавить(Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратДенегОтСотрудника, НСтр("ka='ზედმეტად გადახდილი ხელფასის უკან დაბრუნება';ru='Возврат денег от сотрудника'")); //+++ Дмгг ВозвратЗПОтСотрузника
...
Функция ПолучитьСписокВидовОперацийРасходДСБанк() Экспорт
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуЗарплата.Получить() Тогда
		...
		СписокОпераций.Добавить(Перечисления.ВидыОперацийРасходСоСчета.Зарплата, НСтр("ka='ხელფასი უწყისის მიხედვით';ru='Зарплата по ведомости'")); // Kuzmich, 23.02.2019,Зарплата по ведомости в разрезе сотрудников
		СписокОпераций.Добавить(Перечисления.ВидыОперацийРасходСоСчета.ЗарплатаВРазрезеСотрудников, НСтр("ka='ხელფასი თანამშრმლის ჭრილში უწყისის მიხედვით';ru='Зарплата по ведомости в разрезе сотрудников'")); // Kuzmich, 23.02.2019,Зарплата по ведомости в разрезе сотрудников
	КонецЕсли;
	...
	СписокОпераций.Добавить(Перечисления.ВидыОперацийРасходСоСчета.ПенсионномуФонду, НСТР("ka='საპენსიო ფონდში';ru='Пенсионному фонду'")); // Паганини ПФ
