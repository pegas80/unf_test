...
//		ПараметрыКоманды.СопровождающийТекст = НСтр("ka='სრულდება ბრძანება...';ru='Выполняется команда...'");
...
Процедура ВыполнитьКомандуВФоне(Знач ИдентификаторКоманды, Знач ПараметрыКоманды, Знач Обработчик) Экспорт
	...
	Если Не ЗначениеЗаполнено(СопровождающийТекст) Тогда
		СопровождающийТекст = НСтр("ka='ბრძანება სრულდება.';ru='Команда выполняется.'");
	КонецЕсли;
...
Функция ВыполнитьНазначаемуюКомандуНаКлиенте(Форма, ИмяЭлемента) Экспорт
	...
	Если Объект.Ссылка.Пустая() Или Форма.Модифицированность Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ბრძანება ""%1""-ის შესასრულებლად საჭიროა მონაცემთა ჩაწერა.';ru='Для выполнения команды ""%1"" необходимо записать данные.'"),
			ВыполняемаяКоманда.Представление);
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ჩავიწეროთ და გავაგრძელოთ';ru='Записать и продолжить'"));
...
Процедура ПоказатьОповещениеПриВыполненииКоманды(ВыполняемаяКоманда)
	Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='ბრძანება სრულდება...';ru='Команда выполняется...'"), , ВыполняемаяКоманда.Представление);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	ТекстСообщения = НСтр("ka='ფაილში გარე დამუშავების (ანგარიშგების) გადმოსატვირთად რეკომენდებულია ვებ-კლიენტ 1C:საწარმოსთვის გაფართოების დაყენება.';ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента 1С:Предприятие.'");
...
Процедура ВыгрузитьВФайлЗавершение(Подключено, ПараметрыВыгрузки) Экспорт
	...
	ДиалогСохраненияФайла.Заголовок = НСтр("ka='ფაილის მითითება';ru='Укажите файл'");
