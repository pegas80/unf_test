...
//		ПараметрыКоманды.СопровождающийТекст = НСтр("ka='სრულდება ბრძანება...';ru='Выполняется команда...';en='The command ...'");
...
Процедура ВыполнитьКомандуВФоне(Знач ИдентификаторКоманды, Знач ПараметрыКоманды, Знач Обработчик) Экспорт
	...
	Если Не ЗначениеЗаполнено(СопровождающийТекст) Тогда
		СопровождающийТекст = НСтр("ka='ბრძანება სრულდება.';ru='Команда выполняется.';en='The command is executed.'");
	КонецЕсли;
...
Функция ВыполнитьНазначаемуюКомандуНаКлиенте(Форма, ИмяЭлемента) Экспорт
	...
	Если Объект.Ссылка.Пустая() Или Форма.Модифицированность Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ბრძანება ""%1""-ის შესასრულებლად საჭიროა მონაცემთა ჩაწერა.';ru='Для выполнения команды ""%1"" необходимо записать данные.';en='you must write the data to perform the ""%1"" command.'"),
			ВыполняемаяКоманда.Представление);
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ჩავიწეროთ და გავაგრძელოთ';ru='Записать и продолжить';en='Record and continue'"));
...
Процедура ПоказатьОповещениеПриВыполненииКоманды(ВыполняемаяКоманда)
	Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='ბრძანება სრულდება...';ru='Команда выполняется...';en='The command is executed ...'"), , ВыполняемаяКоманда.Представление);
	КонецЕсли;
КонецПроцедуры
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если ФормаОбработки = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგებისთვის ან დამუშავებისთვის ""%1"" ძირითადი ფორმა არ არის დანიშნული, ან ძირითადი ფორმა არ არის განკუთვნილი ჩვეულებრივ დანართში გასაშვებად. ბრძანება ""%2"" არ შეიძლება, რომ შესრულებული იქნას.';ru='Для отчета или обработки ""%1"" не назначена основная форма,"
...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	ТекстСообщения = НСтр("ka='ფაილში გარე დამუშავების (ანგარიშგების) გადმოსატვირთად რეკომენდებულია ვებ-კლიენტ 1C:საწარმოსთვის გაფართოების დაყენება.';ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента 1С:Предприятие.';en='it is recommended to install the extension for Web client 1C for discharging external processing (report) in the file: Enterprise.'");
...
Процедура ВыгрузитьВФайлЗавершение(Подключено, ПараметрыВыгрузки) Экспорт
	...
	ДиалогСохраненияФайла.Заголовок = НСтр("ka='ფაილის მითითება';ru='Укажите файл';en='specify file'");
