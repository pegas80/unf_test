...
Процедура ОбновитьДанныеРеглОтчета(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, СсылкаНаСохрРеглОтчет) Экспорт
	...
	Если ИмяФормыОтчета = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ka='ანგარიშგების მონაცემთა განახლება ვერ მოხერხდა!';ru='Не удалось обновить данные отчета!';en='Failed to update the report data!'");
		Сообщение.Сообщить();
...
Функция СформироватьАвтоматическиРеглОтчет(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, КорректирующийОтчет = Ложь,
	...
	Если ИмяФормыОтчета = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ანგარიშგების შედგენა ვერ მოხერხდა!';ru='Не удалось сформировать отчет!';en='Unable to generate a report!'");
		Сообщение.Сообщить();
...
Функция ВыгрузитьРегламентированныйОтчет(СохраненныйОтчет) Экспорт
	...
	Если ТипЗнч(СохраненныйОтчет) <> Тип("ДокументСсылка.РегламентированныйОтчет")
		...
		ДобавитьОшибкуФормированияРеглОтчета(Результат,
			НСтр("ka='Неверное значение сохраненного регламентированного отчета для выгрузки';ru='Неверное значение сохраненного регламентированного отчета для выгрузки';en='Invalid value stored regulated reports to unload'"));
	...
	Если НЕ ВыгрузкаОтчетаПоддерживается(РеквизитыОтчета.ИсточникОтчета, РеквизитыОтчета.ВыбраннаяФорма) Тогда
		...
		ДобавитьОшибкуФормированияРеглОтчета(Результат,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Выгрузка отчета %1 за %2 возможна только в форме, автоматическая выгрузка не поддерживается';ru='Выгрузка отчета %1 за %2 возможна только в форме, автоматическая выгрузка не поддерживается';en='Unloading report% 1% 2 for the only possible form of automatic unloading is not supported'"),
...
Функция ЗаписатьРеглОтчетУСНФормаОтчета2015Кв1(ПараметрыЗаписи)
	...
	Если СохраняемыйРеглОтчет <> Неопределено Тогда
		...
		Иначе
			...
			ДобавитьОшибкуФормированияРеглОтчета(ПараметрыЗаписи,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Неверный тип входящего параметра (%1) при автоматическом формировании декларации по УСН';ru='Неверный тип входящего параметра (%1) при автоматическом формировании декларации по УСН';en='Invalid type input parameter (1%) in the automatic formation declaration USN'"),
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"),
			РегламентированнаяОтчетностьКлиентСервер.СформироватьТекстСообщения(ОписаниеОшибки()));
