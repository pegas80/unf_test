...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
		ПоказатьПредупреждение(, НСтр("ka='არ არის არჩეული ფაილი, რომელიც ხელმოსაწერია.';ru='Не выбран файл, который нужно подписать.';en='Do not select the file you want to sign.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		ПоказатьПредупреждение(, НСтр("ka='ელექტრონული ხელმოწერების დამატებას მხარდაჭერა არ გააჩნია.';ru='Добавление электронных подписей не поддерживается.';en='Adding digital signatures is not supported.'"));
		Возврат;
	КонецЕсли;
...
"в настройках программы использование электронных подписей.';ka='ელექტრონული ხელმოწერის დასამატებლად, პროგრამის გამართვებში ჩართეთ ელექტრონული ხელმოწერების გამოყენება.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Исключение
			...
			ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='აღწერილობა=""%1""';ru='Описание=""%1""';en='Description = ""%1""'"),
