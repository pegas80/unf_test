	...
	Для Каждого ЭлементСценария Из СценарийПримененияРазрешений Цикл
		...
		Если ЭлементСценария.Операция = ВидыОпераций.Создание Тогда
			...
			Если АдминистрированиеКластераКлиентСервер.ПрофильБезопасностиСуществует(ПараметрыАдминистрированияКластера, ЭлементСценария.Профиль) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უსაფრთხოების პროფილი %1 უკვე არის სერვერების კლასტერში. უსაფრთხოების პროფილში გამართვები გამოიცვლება...';ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...';en='The safety profile of% 1 is already present in the server cluster. Settings in the safety profile will be replaced ...'"), ЭлементСценария.Профиль));
		...
		ИначеЕсли ЭлементСценария.Операция = ВидыОпераций.Удаление Тогда
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერში უსაფრთხოების პროფილი %1 არ არის. შეიძლება, უსაფრთხოების პროფილი მანამდე იქნა წაშლილი...';ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...';en='The safety profile of% 1 does not exist in the server cluster. Possibly, the safety profile was deleted earlier ...'"), ЭлементСценария.Профиль));
...
Процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ka='ნებართვების აწყობა';ru='Настройка разрешений';en='Setting permissions'"),,
			НСтр("ka='სერვერების კლასტერში უსაფრთხოების პროფილების გამართვებში შეტანილია ცვლილებები.';ru='Внесены изменения в настройки профилей безопасности в кластере серверов.';en='Changes in the security profile settings in a server cluster.'"));
