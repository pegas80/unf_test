	...
	Для Каждого ЭлементСценария Из СценарийПримененияРазрешений Цикл
		...
		Если ЭлементСценария.Операция = ВидыОпераций.Создание Тогда
			...
			Если АдминистрированиеКластераКлиентСервер.ПрофильБезопасностиСуществует(ПараметрыАдминистрированияКластера, ЭлементСценария.Профиль) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უსაფრთხოების პროფილი %1 უკვე არის სერვერების კლასტერში. უსაფრთხოების პროფილში გამართვები გამოიცვლება...';ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...'"), ЭлементСценария.Профиль));
		...
		ИначеЕсли ЭлементСценария.Операция = ВидыОпераций.Удаление Тогда
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერში უსაფრთხოების პროფილი %1 არ არის. შეიძლება, უსაფრთხოების პროფილი მანამდე იქნა წაშლილი...';ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...'"), ЭлементСценария.Профиль));
...
Процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ka='ნებართვების აწყობა';ru='Настройка разрешений'"),,
			НСтр("ka='სერვერების კლასტერში უსაფრთხოების პროფილების გამართვებში შეტანილია ცვლილებები.';ru='Внесены изменения в настройки профилей безопасности в кластере серверов.'"));
