...
Процедура ПолучитьУникальныйИдентификаторАбонента(
	...
	Если ТипЗнч(СертификатЭП) <> Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования")
		...
		ВызватьИсключение НСтр("ka='Некорректное значение параметра ""СертификатЭП"".';ru='Некорректное значение параметра ""СертификатЭП"".'");
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатАбонента(
	...
	ИначеЕсли ТипЗнч(ИдентификаторАбонента) <> Тип("Строка") Или ПустаяСтрока(ИдентификаторАбонента) Тогда
		ВызватьИсключение НСтр("ka='Некорректное значение параметра ""ИдентификаторАбонента"".';ru='Некорректное значение параметра ""ИдентификаторАбонента"".'");
	КонецЕсли;
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если Не ЗначениеЗаполнено(СертификатЭП) Тогда
		...
		ТекстОшибки = НСтр("ka='""Сертификат абонента""';ru='""Сертификат абонента""'");
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ka='и';ru='и'") + " ";
		КонецЕсли;
		
		ТекстОшибки = ТекстОшибки + НСтр("ka='""Организация""';ru='""Организация""'");
		КоличествоОшибок = КоличествоОшибок + 1;
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоОшибок = 1 Тогда
			ДопТекст = НСтр("ka='გთხოვთ, შეავსოთ ველი';ru='Пожалуйста, заполните поле'") + " ";
		Иначе
			ДопТекст = НСтр("ka='გთხოვთ, შეავსოთ ველი';ru='Пожалуйста, заполните поля'") + " ";
		КонецЕсли;
		...
		Если ВариантБизнесПроцесса = "taxcomGetID" Тогда
			ТекстСообщения = ТекстСообщения
				+ " " + НСтр("ka='ედ-ის გაცვლის მონაწილის უნიკალური იდენტიფიკატორის მიღებამდე';ru='до получения уникального идентификатора участника обмена ЭД'");
		ИначеЕсли ВариантБизнесПроцесса = "taxcomPrivat" Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("ka='პირად კაბინეტში გადასვლამდე';ru='до перехода в личный кабинет'");
		КонецЕсли;
...
"Подробнее см. в журнале регистрации.'");
	...
	Если ПараметрыДляРасшифрования.markerED = Неопределено Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)';ru='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)'"));
...
"%1'"),
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиент.ЗавершитьБизнесПроцесс(КонтекстВзаимодействия);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)';ru='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)'"));
		ПоказатьПредупреждение(, СообщениеОбОшибкеДляПользователя);
		Возврат;
	...
	ОписаниеДанных.Вставить("Операция", НСтр("ka='Проверка пароля сертификата';ru='Проверка пароля сертификата'"));
...
Процедура ПоказатьПричинуОтклоненияЗаявкиЭДО(КонтекстВзаимодействия) Экспорт
	...
	Если НЕ ПустаяСтрока(ОписаниеПричины.КодОшибки) Тогда
		ТекстСообщения = НСтр("ka='Код ошибки: %1';ru='Код ошибки: %1'");
	...
	Если ПустаяСтрока(ОписаниеПричины.ТекстОшибки) Тогда
		ТекстСообщения = НСтр("ka='Неизвестная ошибка. Обратитесь в службу техподдержки.';ru='Неизвестная ошибка. Обратитесь в службу техподдержки.'");
	КонецЕсли;
