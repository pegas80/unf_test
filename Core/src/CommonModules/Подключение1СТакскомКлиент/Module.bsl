...
Процедура ПолучитьУникальныйИдентификаторАбонента(
	...
	Если ТипЗнч(СертификатЭП) <> Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования")
		...
		ВызватьИсключение НСтр("ka='Некорректное значение параметра ""СертификатЭП"".';ru='Некорректное значение параметра ""СертификатЭП"".';en='Incorrect parameter value ""SertifikatEP"".'");
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатАбонента(
	...
	ИначеЕсли ТипЗнч(ИдентификаторАбонента) <> Тип("Строка") Или ПустаяСтрока(ИдентификаторАбонента) Тогда
		ВызватьИсключение НСтр("ka='Некорректное значение параметра ""ИдентификаторАбонента"".';ru='Некорректное значение параметра ""ИдентификаторАбонента"".';en='Incorrect parameter value ""IdentifikatorAbonenta"".'");
	КонецЕсли;
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если Не ЗначениеЗаполнено(СертификатЭП) Тогда
		...
		ТекстОшибки = НСтр("ka='""Сертификат абонента""';ru='""Сертификат абонента""';en='""Certificate user""'");
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ka='и';ru='и';en='and'") + " ";
		КонецЕсли;
		
		ТекстОшибки = ТекстОшибки + НСтр("ka='""Организация""';ru='""Организация""';en='""Organization""'");
		КоличествоОшибок = КоличествоОшибок + 1;
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоОшибок = 1 Тогда
			ДопТекст = НСтр("ka='გთხოვთ, შეავსოთ ველი';ru='Пожалуйста, заполните поле';en='Please fill in this field'") + " ";
		Иначе
			ДопТекст = НСтр("ka='გთხოვთ, შეავსოთ ველი';ru='Пожалуйста, заполните поля';en='Please fill in the fields'") + " ";
		КонецЕсли;
		...
		Если ВариантБизнесПроцесса = "taxcomGetID" Тогда
			ТекстСообщения = ТекстСообщения
				+ " " + НСтр("ka='ედ-ის გაცვლის მონაწილის უნიკალური იდენტიფიკატორის მიღებამდე';ru='до получения уникального идентификатора участника обмена ЭД';en='to obtain a unique identifier ED exchange participant'");
		ИначеЕсли ВариантБизнесПроцесса = "taxcomPrivat" Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("ka='პირად კაბინეტში გადასვლამდე';ru='до перехода в личный кабинет';en='to go to your personal cabinet'");
		КонецЕсли;
...
"Продолжить?';ka='ედ-ის გაცვლის მონაწილის უნიკალური იდენტიფიკატორის მიღება ხელმისაწვდომი იქნება მომხმარებელთა ინტერნეტ-მხარდაჭერის სერვისთან შეერთების და ხელშეკრულებაში მითითებული აბონენტის სერთიფიკატის მფლობელის ნამდვილობის შემოწმების შემდეგ. გავაგრძელოთ?'");
		Иначе
			ТекстОповещенияПользователя =
			НСтр("ka='ხელშეკრულებაში ახალი სერთიფიკატის დამატება განხორციელდება მომხმარებლების ინტერნეტ-მხარდაჭერის სერვისთან შეერთების და ხელშეკრულებაში მითითებული აბონენტის სერთიფიკატის მფლობელის ნამდვილობის შემოწმების შემდეგ. გავაგრძელოთ?';ru='Добавление нового сертификата в соглашение будет выполнено"
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?'");
		КонецЕсли;
	...
	ИначеЕсли ВариантБизнесПроцесса = "taxcomPrivat" Тогда
		...
		ТекстОповещенияПользователя = 
		НСтр("ka='ედ-ის გაცვლის მონაწილის პირად კაბინეტში შესვლა ხელმისაწვდომი იქნება მომხმარებელთა ინტერნეტ-მხარდაჭერის სერვისთან შეერთების და ხელშეკრულებაში მითითებული აბონენტის სერთიფიკატის მფლობელის ნამდვილობის შემოწმების შემდეგ. გავაგრძელოთ?';ru='Вход в личный кабинет участника обмена ЭД будет доступен"
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?'");
...
"Указан неверный вариант бизнес-процесса (%1).';ka='Ошибка встраивания механизма Интернет-поддержки пользователей."
...
"Подробнее см. в журнале регистрации.';ka='Ошибка при проверке пароля сертификата."
"Подробнее см. в журнале регистрации.'");
	...
	Если ПараметрыДляРасшифрования.markerED = Неопределено Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)';ru='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)';en='Error checking the authenticity of the certificate owner. No authentication token (markerED)'"));
...
"%1';ka='Ошибка при проверке подлинности владельца сертификата. Не удалось получить двоичные данные маркера (markerED)."
"%1'"),
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиент.ЗавершитьБизнесПроцесс(КонтекстВзаимодействия);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)';ru='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)';en='Error checking the authenticity of the certificate owner. Not specified certificate (IDCertificateED, IDCertificateED_Dop)'"));
		ПоказатьПредупреждение(, СообщениеОбОшибкеДляПользователя);
		Возврат;
	...
	ОписаниеДанных.Вставить("Операция", НСтр("ka='Проверка пароля сертификата';ru='Проверка пароля сертификата';en='Checking certificate password'"));
...
Процедура ПоказатьПричинуОтклоненияЗаявкиЭДО(КонтекстВзаимодействия) Экспорт
	...
	Если НЕ ПустаяСтрока(ОписаниеПричины.КодОшибки) Тогда
		ТекстСообщения = НСтр("ka='Код ошибки: %1';ru='Код ошибки: %1';en='Error code:% 1'");
	...
	Если ПустаяСтрока(ОписаниеПричины.ТекстОшибки) Тогда
		ТекстСообщения = НСтр("ka='Неизвестная ошибка. Обратитесь в службу техподдержки.';ru='Неизвестная ошибка. Обратитесь в службу техподдержки.';en='Unknown error. Please contact technical support.'");
	КонецЕсли;
...
""
"- отпечаток сертификата: %1';ka='Параметры участника обмена ЭД:"
""
"- отпечаток сертификата: %1'");
	...
	Если НЕ ПустаяСтрока(КодОшибки) Тогда
		...
		СтрДанныхОшибки = Символы.ПС + НСтр("ka='- код ошибки: %1,"
"- описание ошибки: %2';ru='- код ошибки: %1,"
"- описание ошибки: %2'");
