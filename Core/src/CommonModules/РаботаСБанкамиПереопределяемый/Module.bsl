...
Процедура ВосстановитьЭлементИзОбщихДанных(Знач Форма) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ka='საერთო მონაცემებიდან აღდგენა';ru='Восстановление из общих данных'"), 
...
Процедура ОбработатьФлагРучногоИзменения(Знач Форма)
	...
	Если Форма.РучноеИзменение = Неопределено Тогда
		...
		Иначе
			Форма.ТекстРучногоИзменения = НСтр("ka='ელემენტი შეიქმნა ხელის რეჟიმში. ავტომატური განახლება შეუძლებელია.';ru='Элемент создан вручную. Автоматическое обновление не возможно.'");
		КонецЕсли;
	...
	ИначеЕсли Форма.РучноеИзменение = Истина Тогда
		Форма.ТекстРучногоИзменения = НСтр("ka='ელემენტის ავტომატური განახლება უარყოფილია.';ru='Автоматическое обновление элемента отключено.'");
		...
		КонецЕсли;
	Иначе
		Форма.ТекстРучногоИзменения = НСтр("ka='ელემენტის განახლება ავტომატურად ხდება.';ru='Элемент обновляется автоматически.'");
		...
		Если НЕ ПустаяСтрока(ПараметрыОбъект.РодительКод) И НЕ ЗначениеЗаполнено(ПараметрыОбъект.Родитель) Тогда
			...
			Если НЕ ЗначениеЗаполнено(БанкОбъект.Родитель) Тогда
				ИмяСобытия = ?(ИмяСобытия = "",
					НСтр("ka='შერჩევა კლასიფიკატორიდან';ru='Подбор из классификатора'"), ИмяСобытия);
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ბსკ %1-ის მქონე ელემენტის მშობლის მიღება ვერ მოხერხდა';ru='Не смогли получить родителя у элемента с БИК %1'"), СокрЛП(ПараметрыОбъект.Код));
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ka='ბანკების კლასიფიკატორი';ru='Классификатор банков'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
