...
"%2';ka='Не удалось записать группу (регион) %1."
"%2'");
...
"%1';ka='Не удалось записать элемент."
"%1'");
			ТекстСообщения    = СтрШаблон(
				ШаблонСообщения,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ВосстановитьЭлементИзОбщихДанных(Знач Форма) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ka='საერთო მონაცემებიდან აღდგენა';ru='Восстановление из общих данных';en='Recovery of common data'"), 
		...
		Если НЕ ПустаяСтрока(ПараметрыОбъект.РодительКод) И НЕ ЗначениеЗаполнено(ПараметрыОбъект.Родитель) Тогда
			...
			Если НЕ ЗначениеЗаполнено(БанкОбъект.Родитель) Тогда
				ИмяСобытия = ?(ИмяСобытия = "",
					НСтр("ka='შერჩევა კლასიფიკატორიდან';ru='Подбор из классификатора';en='Selection of the classifier'"), ИмяСобытия);
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ბსკ %1-ის მქონე ელემენტის მშობლის მიღება ვერ მოხერხდა';ru='Не смогли получить родителя у элемента с БИК %1';en='Not able to get the parent of an element with the BIC 1%'"), СокрЛП(ПараметрыОбъект.Код));
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ka='ბანკების კლასიფიკატორი';ru='Классификатор банков';en='Qualifier banks'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
