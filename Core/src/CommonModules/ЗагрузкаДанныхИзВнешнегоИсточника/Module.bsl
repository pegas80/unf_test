...
Процедура ЗаполнитьДеревоГруппыИПолей(ТаблицаПолейЗагрузки, ГруппыИПоля, ПолноеИмяОбъектаЗаполнения, ЭтоЗагрузкаВТЧ)
	...
	Для каждого СтрокаТаблицы Из ТаблицаГруппПолей Цикл
		...
		//Малышев 2024-07-15 ONESOFT-3315
		Если НоваяСтрокаПервогоУровня.ИмяГруппыПолей = "Номенклатура" Тогда
			НоваяСтрокаПервогоУровня.ПредставлениеПоля = НСтр("ru='Номенклатура';ka='ნომენკლატურა';en='Items'");	
		ИначеЕсли НоваяСтрокаПервогоУровня.ИмяГруппыПолей = "Заказ" Тогда
			НоваяСтрокаПервогоУровня.ПредставлениеПоля = НСтр("ru='Заказ';ka='შეკვეთა';en='Order'");	
		ИначеЕсли НоваяСтрокаПервогоУровня.ИмяГруппыПолей = "Документ" Тогда
			НоваяСтрокаПервогоУровня.ПредставлениеПоля = НСтр("ru='Документ';ka='დოკუმენტი';en='Document'");	
		ИначеЕсли НоваяСтрокаПервогоУровня.ИмяГруппыПолей = "Счет" Тогда
			НоваяСтрокаПервогоУровня.ПредставлениеПоля = НСтр("ru='Счет';ka='ინვოისი';en='Invoice'");	
...
Процедура ЗаполнитьРасшифровкиВТабличномДокументе(ТабличныйДокумент, КоличествоКолонок, НастройкиЗагрузкиДанных) Экспорт
	...
	Расшифровка.Добавить("Не загружать", "აირჩიეთ ჩასატვირთი ველი");
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл
		...
		ОбластьПриемник.Текст 				= "აირჩიეთ ჩასატვირთი ველი";
		...
		ОбластьКолонка.ШиринаКолонки = 35.75;
