...
Функция МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2() Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ИмяСобытия = ИмяСобытияТарификации() + НСтр("ka='МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2';ru='МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2';en='MenedzherServisaPodderzhivaetUpravlenieTarifamiVPrilozhenii_1_0_1_2'");
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_ОбработчикОбновления() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ka='Регистрация тарифицируемых услуг. Обработчик обновления';ru='Регистрация тарифицируемых услуг. Обработчик обновления';en='Register chargeable services. processor upgrade'");
	...
	Попытка
		...
		ТекстИнформации = НСтр("ka='Услуги успешно отправлены в Менеджер сервиса';ru='Услуги успешно отправлены в Менеджер сервиса';en='Services successfully sent to the service manager'");
		ЗаписьЖурналаРегистрации(ИмяСобытия, 
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ka='Регистрация тарифицируемых услуг. Регламентное задание';ru='Регистрация тарифицируемых услуг. Регламентное задание';en='Register chargeable services. Scheduled task'");
...
Процедура ЗапроситьЛицензииУникальныхУслуг_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ka='Запрос лицензий уникальных услуг. Регламентное задание';ru='Запрос лицензий уникальных услуг. Регламентное задание';en='Query unique services licenses. Scheduled task'");
	...
	Попытка
		...
		ТекстИнформации = НСтр("ka='წარმატება';ru='Успех.';en='Success.'");
		ЗаписьЖурналаРегистрации(ИмяСобытия, 
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Функция ИмяСобытияТарификации() Экспорт
	...
	Возврат НСтр("ka='Управление тарифами в модели сервиса.';ru='Управление тарифами в модели сервиса.';en='Tariff management in the service model.'");
...
Процедура ПроверитьСтруктуруПоставщиковУслуг(ПоставщикиУслуг)
	...
	Для Каждого Поставщик Из ПоставщикиУслуг Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru='Имеется несколько поставщиков услуг с идентифкатором: ';ka='Имеется несколько поставщиков услуг с идентифкатором: '") + Поставщик.Идентификатор;
		КонецЕсли;
	КонецЦикла;
