...
Функция ДеревоРазбораВыпискиБанка(ТекстДляРазбора) Экспорт
	...
	Пока Импорт_ТекущаяСтрока <= Импорт_КоличествоСтрок Цикл
		...
		Если Лев(ВРег(СокрЛП(Стр)), 14) = "СЕКЦИЯДОКУМЕНТ" Тогда
			...
			Иначе
				...
				ТекстОшибки = НСтр("ka='დარღვეულია იმპორტის ფაილის სტრუქტურა, სტრიქონი %1 : %2';ru='Нарушена структура файла импорта, строка %1 : %2'");
		...
		ИначеЕсли Лев(ВРег(СокрЛП(Стр)), 14) = "СЕКЦИЯРАСЧСЧЕТ" Тогда
			...
			Если НЕ СчетЗагружен Тогда
				Сообщение = Новый СообщениеПользователю;
				ТекстСообщения = НСтр("ka='Нарушена структура файла импорта в секции описания расчетного счета. Строка: %1';ru='Нарушена структура файла импорта в секции описания расчетного счета. Строка: %1'");
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																	ТекстСообщения,
																	Импорт_ТекущаяСтрока - 1);
				Сообщение.Сообщить();
		...
		ИначеЕсли Лев(ВРег(СокрЛП(Стр)), 10) = "КОНЕЦФАЙЛА" Тогда
			Если НЕ Импорт_ПризнакОбмена Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='В файле импорта отсутствует признак обмена ""1CClientBankExchange"".';ru='В файле импорта отсутствует признак обмена ""1CClientBankExchange"".'");
				Сообщение.Сообщить();
				Возврат Неопределено;
			КонецЕсли;
	...
	Если НЕ НайденКонецФайла Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Файл загрузки не соответствует стандарту (не найдена секция КонецФайла).';ru='Файл загрузки не соответствует стандарту (не найдена секция КонецФайла).'");
		Сообщение.Сообщить();
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПриСозданииФормыПроверкаСертификата(Сертификат, ДополнительныеПроверки, ПараметрыДополнительныхПроверок, СтандартныеПроверки) Экспорт
	...
	Если Результат.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
		...
		НовСтрока.Представление = НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных'");
...
"საჭიროა PIN-ი.'");
		...
		НовСтрока.Представление = НСтр("ka='შექმნილი ხელმოწერის შემოწმება';ru='Проверка созданной подписи'");
...
Функция ПолучитьДатуИзСтроки(Знач ДатаСтрокой, ТекстОшибки)
	...
	Если ПозицияТочки = 0 Тогда
		ТекстОшибки = НСтр("ka='თარიღიანი სტრიქონის არასწორი ფორმატი';ru='Неверный формат строки с датой'");
		Возврат Неопределено;
	КонецЕсли;
	...
	Пока Лев(ВРег(СокрЛП(СтрокаРазбора)), 14) <> "КОНЕЦДОКУМЕНТА" Цикл
		...
		Если РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Тогда
			...
			Если Не ПустаяСтрока(ТекстОшибки) ИЛИ ЕстьОшибка Тогда
				Сообщение = Новый СообщениеПользователю;
				Шаблон = НСтр("ka='საგადახდო დოკუმენტის არასწორი რეკვიზიტი, სტრიქონი %1 : %2';ru='Неверный реквизит платежного документа, строка %1 : %2'");
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																			Шаблон,
																			Импорт_ТекущаяСтрока - 1,
																			СтрокаРазбора);
		...
		Иначе
			...
			Сообщение = Новый СообщениеПользователю;
			Шаблон = НСтр("ka='დარღვეულია საგადახდო დოკუმენტის სტრუქტურა, სტრიქონი %1 : %2';ru='Нарушена структура платежного документа, строка %1 : %2'");
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																			Шаблон,
																			Импорт_ТекущаяСтрока - 1,
																			СтрокаРазбора);
...
Функция ЗагрузитьСекциюРасчСчета(СекцияРасчСчет, Импорт_ТекущаяСтрока, Импорт_КоличествоСтрок, Импорт_ТекстДляРазбора, СтруктураСчета, ПоляДат, ПоляЧисел)
	...
	Пока РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Цикл
		Если СтруктураСчета.Свойство(Тег, НазваниеРеквизита) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстОшибки = НСтр("ka='არის შეცდომა';ru='Есть ошибка'");
		КонецЕсли;
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			Сообщение = Новый СообщениеПользователю;
			ШаблонСообщения = НСтр("ka='არასწორი რეკვიზიტი საანგარიშსწორებო ანგარიშის აღწერილობის სექციაში, სტრიქონი %1 : %2';ru='Неверный реквизит в секции описания расчетного счета, строка %1 : %2'");
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																	ШаблонСообщения,
																	Импорт_ТекущаяСтрока - 1,
																	СтрокаРазбора);
...
Процедура ЗагрузитьСтрокуЗаголовка(ЗначениеXDTO, УсловияОтбора, ТекстСтрокиЗаголовка, ТекущаяСтрока, Поля, Отказ)
	...
	Если ЕстьОшибка Тогда
		Сообщение = Новый СообщениеПользователю;
		ТекстСообщения = НСтр("ka='სათაურის არასწორი რეკვიზიტი, სტრიქონი %1: %2';ru='Неверный реквизит заголовка, строка %1 : %2'");
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																ТекстСообщения,
																ТекущаяСтрока - 1,
...
Процедура ПеренестиДанныеОбменаСБанками(Параметры) Экспорт
	...
	Попытка
		...
		СоглашениеЭДОбъект.Комментарий = НСтр("ka='##ედბ-ის გამართვა მონიშნულია წასაშლელად ავტომატურად განახლებისას.';ru='##Настройка ЭДО помечена на удаление автоматически при обновлении.'");
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Операция = НСтр("ka='ბანკებთან პირდაპირი გაცვლის ქვესისტემის განახლება';ru='Обновление подсистемы прямого обмена с банками'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='ბანკებთან პირდაპირი გაცვლის ქვესისტემის განახლებისას შეცდომა მოხდა';ru='При обновлении подсистемы прямого обмена с банками произошла ошибка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ПредставлениеЭД(Наименование, ВидЭД, СсылкаНаОбъект)
	...
	ШаблонПредставленияЭД = НСтр("ka='%1 %2 %3-ის';ru='%1 %2 от %3'");
