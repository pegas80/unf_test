...
Процедура ОткрытьФормуНастройкиСканирования() Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ТекстСообщения = НСтр("ka='Сканирование поддерживается только в клиенте под управлением ОС Windows.';ru='Сканирование поддерживается только в клиенте под управлением ОС Windows.';en='Scanning is only supported on the client running on Windows.'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ТекстСообщения = НСтр("ka='ვებ-კლიენტში სკანირებას მხარდაჭერა არ გააჩნია.';ru='Сканирование не поддерживается в веб-клиенте.';en='Scanning is not supported on the Web client.'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
