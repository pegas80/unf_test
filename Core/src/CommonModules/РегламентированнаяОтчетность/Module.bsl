...
Функция КоличествоОбъектовНеЗаписанныхВРегистр(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='Проверка полноты данных журнала регламентированной отчетности';ru='Проверка полноты данных журнала регламентированной отчетности';en='Check the completeness of the data log regulatory reporting'");
...
Функция ДобавлениеОбъектовОтсутствующихВРегистре(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='Заполнение данных журнала регламентированной отчетности';ru='Заполнение данных журнала регламентированной отчетности';en='Filling the data log regulatory reporting'");
...
Процедура ВыполнитьДобавлениеВРегистр(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		НСтр("ka='Заполнение данных журнала регламентированной отчетности';ru='Заполнение данных журнала регламентированной отчетности';en='Filling the data log regulatory reporting'"), 
...
Процедура ОбработатьСобытие1СОтчетности(ИмяСобытия, СсылкаНаИсточник = Неопределено, ДопПараметрСтруктура = Неопределено) Экспорт
	...
	Если ИмяСобытия = НСтр("ka='Регламентированный отчет. Создание';ru='Регламентированный отчет. Создание';en='Regulated report. Creature'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКСозданиюРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ka='Регламентированный отчет. Выгрузка';ru='Регламентированный отчет. Выгрузка';en='Regulated report. Unloading'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКВыгрузкеРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ka='Регламентированный отчет. Проверка выгрузки';ru='Регламентированный отчет. Проверка выгрузки';en='Regulated report. Check discharge'") Тогда
...
Функция РеглОтчеты(Наим, НеЗапоминатьВКэше = Ложь) Экспорт
	...
	Если НЕ ПризнакИспользованияВнешнегоОтчетаСУчетомКэша(НайденныйЭлемент, НеЗапоминатьВКэше) ИЛИ НайденныйЭлемент.ВнешнийОтчетХранилище = Неопределено Тогда // используется внутренний отчет
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება %1 ვერ მოიძებნა!';ru='Не найден отчет %1!';en='Unable to find the report% 1!'"), СокрП(Наим));
			Сообщение.Сообщить();
			Возврат Неопределено;
	...
	Иначе // используется внешний отчет
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გარე ანგარიშგება %1-ის გაშვება ვერ მოხერხდა!';ru='Не удалось запустить внешний отчет %1!';en='Unable to start the external account% 1!'"), СокрП(Наим));
			Сообщение.Сообщить();
...
Функция ЗаблокироватьДокумент(мСохраненныйДок)
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტის დაბლოკვა ვერ ხერხდება!%1შეიძლება, რომ დოკუმენტი უკვე გახსნილია სხვა მომხმარებლის მიერ!';ru='Не удается заблокировать документ!%1Возможно, документ уже открыт другим пользователем!';en='Unable to lock the document! 1Vozmozhno%, the document has already been opened by another user!'"), Символы.ПС);
...
Функция ПродолжитьПриНесоответствииВерсийФорм(Знач ВерсияФормы, Знач мВерсияФормы) Экспорт
	...
	Если ДатамВерсияФормы < ДатаВерсияФормы Тогда
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შენახული მონაცემები არ შეიძლება გახსნილი იქნას ანგარიშგების მიმდინარე ვერსიის მიერ!%1ანგარიშგების ვერსია - %2მონაცემთა ვერსია-დან - %3-დან';ru='Сохраненные данные не могут быть открыты текущей версией отчета!%1Версия отчета - от %2Версия данных - от %3';en='Saved data can not be opened by the current version of the report of the report 1Versiya% -% of 2Versiya data - from 3%'"), Символы.ПС, Формат(ДатамВерсияФормы, "ДЛФ=DD; ДП='Пустая дата'") + Символы.ПС, Формат(ДатаВерсияФормы,  "ДЛФ=DD; ДП='Пустая дата'"));
...
Функция ВывестиСведенияСлужебнойЧасти30(Текст, Организация = Неопределено, Знач ДатаДок = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач КодИнсп = Неопределено, ТипИнф = "ОТЧЕТНОСТЬНП", ВерсияФормата = "3.00") Экспорт
	...
	Если ПустаяСтрока(ОргИНН) Тогда
		...
		Сообщение.Текст = НСтр("ka='Невозможно определить ИНН организации! Выгрузка прервана!';ru='Невозможно определить ИНН организации! Выгрузка прервана!';en='Unable to determine INN organization! Unloading is interrupted!'");
	...
	Если ПустаяСтрока(КодИНСП) Тогда
		...
		Сообщение.Текст = НСтр("ka='საგადასახადო ინსპექციის კოდის დადგენა შეუძლებელია! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить код налоговой инспекции! Выгрузка прервана!';en='Unable to determine the code tax office! Unloading is interrupted!'");
...
Функция ВывестиСведенияОбОтправителе30(Текст, Организация = Неопределено, Знач ОргНазв = Неопределено, Знач ОргТелефон = Неопределено, Знач ОргБухгалтер = Неопределено, ВыгрузкаДекларацииПоАлкоголю = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИНН) Тогда
		...
		Сообщение.Текст = НСтр("ka='გადამხდელის ტიპის დადგენა შეუძლებელია! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить вид налогоплательщика! Выгрузка прервана!';en='It is impossible to determine the type of taxpayer! Unloading is interrupted!'");
	...
	Если ПустаяСтрока(ОргНазв) Тогда
		...
		Сообщение.Текст = НСтр("ka='ორგანიზაციის სრული სახელწოდების დადგენა შეუძლებელია! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить полное название организации! Выгрузка прервана!';en='It is impossible to determine the full name of the organization! Unloading is interrupted!'");
	...
	Если КатегорияНП = 1 И ПустаяСтрока(ОргБухгалтер) Тогда
		...
		Сообщение.Текст = НСтр("ka='Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!';ru='Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!';en='Unable to determine the chief accountant of the company name! Unloading is interrupted!'");
	...
	Иначе
		...
		Если ПустаяСтрока(ФИООтпрФЛ) Тогда
			...
			Сообщение.Текст = НСтр("ka='შეუძლებელია მთავარი ბუღალტრის გსმს-ის და ინდმეწარმის გსმს-ის დადგენა! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить ФИО главного бухгалтера и ФИО индивидуального предпринимателя! Выгрузка прервана!';en='Unable to determine the name of the chief accountant and the name of the individual entrepreneur! Unloading is interrupted!'");
	...
	Для Каждого Стр Из ТП Цикл
		...
		Если БылаЦифра Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ნომრის გარდაქმნის შეცდომა დაბოლოებიდან: %1 === %2';ru='Ошибки преобразования номера из окончания: %1 === %2';en='Conversion error number from the closure% 1% 2 ==='"), Корень, Номер);
...
Процедура ПереносДанныхЭДОПриОбновлении10158() Экспорт
	...
	ТекстСообщения = НСтр("ka='Обновление данных информационной базы по ЭДО с КО';ru='Обновление данных информационной базы по ЭДО с КО';en='Update data information base on EDI with KO'");
...
Процедура ОбновитьРеквизитВидОбменаСКонтролирующимиОрганамиСправочникаОрганизации() Экспорт
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Попытка
            ...
            Объект.Записать();
        Исключение
            ТекстСообщения = НСтр("ka='ელექტრონული დოკუმენტბრუნვა მაკონტროლებელ ორგანოებთან';ru='Электронный документооборот с контролирующими органами';en='Electronic document management with regulatory authorities'");
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ბაზის განახლება';ru='Обновление информационной базы';en='Update the information base'");
...
Функция ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении(ОбъектРегОтчет, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СокрЛП(ОбъектРегОтчет.НаименованиеОтчета) = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если СокрЛП(ОбъектРегОтчет.ВыбраннаяФорма) = "ФормаОтчета2016Кв1" Тогда
			...
			Описание = НСтр("ka='Отчет сохранен в конфигурации предыдущей версии с неуникальными значениями показателя ""Уникальный номер"" в Разделе 3.1 и(или) неуникальными сочетаниями значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2.';ru='Отчет сохранен в конфигурации предыдущей версии с неуникальными значениями показателя ""Уникальный номер"" в Разделе 3.1 и(или) неуникальными сочетаниями значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2.';en='The report is saved in the configuration of the previous version with the non-unique values ​​of the index ""Unique number"" in Section 3.1 and (or) non-unique combinations of values ​​of the parameters ""unique number"" and ""serial number"" in Section 3.2.'");
			...
			Описание = НСтр("ka='В отчете, сохраненном в конфигурации предыдущей версии, не соблюдена последовательность в нумерации значений показателя ""Уникальный номер"" в Разделе 3.1 и(или) сочетаний значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2 и(или) Разделе 3.3.';ru='В отчете, сохраненном в конфигурации предыдущей версии, не соблюдена последовательность в нумерации значений показателя ""Уникальный номер"" в Разделе 3.1 и(или) сочетаний значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2 и(или) Разделе 3.3.';en='The report saved in a previous version of the configuration, non-consecutive numbering index values ​​""Unique number"" in Section 3.1 and (or) a combination of indicator values ​​""unique number"" and ""serial number"" in Section 3.2 and (or) Section 3.3.'");
