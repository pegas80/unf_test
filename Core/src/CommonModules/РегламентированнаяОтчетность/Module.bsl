...
Функция КоличествоОбъектовНеЗаписанныхВРегистр(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='Проверка полноты данных журнала регламентированной отчетности';ru='Проверка полноты данных журнала регламентированной отчетности'");
...
Функция ДобавлениеОбъектовОтсутствующихВРегистре(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='Заполнение данных журнала регламентированной отчетности';ru='Заполнение данных журнала регламентированной отчетности'");
...
Процедура ВыполнитьДобавлениеВРегистр(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		НСтр("ka='Заполнение данных журнала регламентированной отчетности';ru='Заполнение данных журнала регламентированной отчетности'"), 
...
Процедура ОбработатьСобытие1СОтчетности(ИмяСобытия, СсылкаНаИсточник = Неопределено, ДопПараметрСтруктура = Неопределено) Экспорт
	...
	Если ИмяСобытия = НСтр("ka='Регламентированный отчет. Создание';ru='Регламентированный отчет. Создание'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКСозданиюРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ka='Регламентированный отчет. Выгрузка';ru='Регламентированный отчет. Выгрузка'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКВыгрузкеРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ka='Регламентированный отчет. Проверка выгрузки';ru='Регламентированный отчет. Проверка выгрузки'") Тогда
...
Функция РеглОтчеты(Наим, НеЗапоминатьВКэше = Ложь) Экспорт
	...
	Если НЕ ПризнакИспользованияВнешнегоОтчетаСУчетомКэша(НайденныйЭлемент, НеЗапоминатьВКэше) ИЛИ НайденныйЭлемент.ВнешнийОтчетХранилище = Неопределено Тогда // используется внутренний отчет
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება %1 ვერ მოიძებნა!';ru='Не найден отчет %1!'"), СокрП(Наим));
			Сообщение.Сообщить();
			Возврат Неопределено;
	...
	Иначе // используется внешний отчет
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გარე ანგარიშგება %1-ის გაშვება ვერ მოხერხდა!';ru='Не удалось запустить внешний отчет %1!'"), СокрП(Наим));
			Сообщение.Сообщить();
...
Функция ЗаблокироватьДокумент(мСохраненныйДок)
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტის დაბლოკვა ვერ ხერხდება!%1შეიძლება, რომ დოკუმენტი უკვე გახსნილია სხვა მომხმარებლის მიერ!';ru='Не удается заблокировать документ!%1Возможно, документ уже открыт другим пользователем!'"), Символы.ПС);
...
Функция ВывестиСведенияСлужебнойЧасти30(Текст, Организация = Неопределено, Знач ДатаДок = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач КодИнсп = Неопределено, ТипИнф = "ОТЧЕТНОСТЬНП", ВерсияФормата = "3.00") Экспорт
	...
	Если ПустаяСтрока(ОргИНН) Тогда
		...
		Сообщение.Текст = НСтр("ka='Невозможно определить ИНН организации! Выгрузка прервана!';ru='Невозможно определить ИНН организации! Выгрузка прервана!'");
	...
	Если ПустаяСтрока(КодИНСП) Тогда
		...
		Сообщение.Текст = НСтр("ka='საგადასახადო ინსპექციის კოდის დადგენა შეუძლებელია! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить код налоговой инспекции! Выгрузка прервана!'");
...
Функция ВывестиСведенияОбОтправителе30(Текст, Организация = Неопределено, Знач ОргНазв = Неопределено, Знач ОргТелефон = Неопределено, Знач ОргБухгалтер = Неопределено, ВыгрузкаДекларацииПоАлкоголю = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИНН) Тогда
		...
		Сообщение.Текст = НСтр("ka='გადამხდელის ტიპის დადგენა შეუძლებელია! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить вид налогоплательщика! Выгрузка прервана!'");
	...
	Если ПустаяСтрока(ОргНазв) Тогда
		...
		Сообщение.Текст = НСтр("ka='ორგანიზაციის სრული სახელწოდების დადგენა შეუძლებელია! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить полное название организации! Выгрузка прервана!'");
	...
	Если КатегорияНП = 1 И ПустаяСтрока(ОргБухгалтер) Тогда
		...
		Сообщение.Текст = НСтр("ka='Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!';ru='Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!'");
	...
	Иначе
		...
		Если ПустаяСтрока(ФИООтпрФЛ) Тогда
			...
			Сообщение.Текст = НСтр("ka='შეუძლებელია მთავარი ბუღალტრის გსმს-ის და ინდმეწარმის გსმს-ის დადგენა! გადმოტვირთვა შეწყვეტილია!';ru='Невозможно определить ФИО главного бухгалтера и ФИО индивидуального предпринимателя! Выгрузка прервана!'");
	...
	Для Каждого Стр Из ТП Цикл
		...
		Если БылаЦифра Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ნომრის გარდაქმნის შეცდომა დაბოლოებიდან: %1 === %2';ru='Ошибки преобразования номера из окончания: %1 === %2'"), Корень, Номер);
...
Процедура ПереносДанныхЭДОПриОбновлении10158() Экспорт
	...
	ТекстСообщения = НСтр("ka='Обновление данных информационной базы по ЭДО с КО';ru='Обновление данных информационной базы по ЭДО с КО'");
...
Процедура ОбновитьРеквизитВидОбменаСКонтролирующимиОрганамиСправочникаОрганизации() Экспорт
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Попытка
            ...
            Объект.Записать();
        Исключение
            ТекстСообщения = НСтр("ka='ელექტრონული დოკუმენტბრუნვა მაკონტროლებელ ორგანოებთან';ru='Электронный документооборот с контролирующими органами'");
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ბაზის განახლება';ru='Обновление информационной базы'");
