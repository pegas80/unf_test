...
Функция ИспользуемаяВерсияСервисаУправляющегоПриложения(Знач ПарольПользователя = "") Экспорт
	...
	Если ПоддерживаемыеВерсии.Найти(ИспользуемаяВерсия) = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='კორესპონდენტი %1-ის მიერ ინტერფეისი %2 არ არის მხარდაჭერილი!';ru='Корреспондент %1 не поддерживает интерфейс %2!'"),
...
Процедура ЗавершитьСеансыОбластиДанных(Знач НомераСеансов, Знач ПарольПользователя) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='დანართის მმართველის მიმდინარე ვერსიის მიერ დანართებიდან სეანსის დასრულებას მხარდაჭერა არ გააჩნია. აუცილებელია მმართველი დანართის განახლება.';ru='Текущая версия управляющего приложения не поддерживает завершение сеанса из приложений. Необходимо обновить управляющее приложение.'");
