...
Процедура ПередНачаломРаботыКлиента(ТребуетсяОбновлениеИнтерфейса = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ka='აქტიური ტარიფების მიღება';ru='Получение активных тарифов';en='Preparation of active tariffs'"), УровеньЖурналаРегистрации.Ошибка, , , ?(ЗначениеЗаполнено(ПодробноеОписание), ПодробноеОписание, Строка(СписокТарифиов)));
	КонецПопытки;
...
Функция ТекстОшибкиПодключения()
	...
	Возврат(НСтр("ka='სერვისის მენეჯერთან შეერთება შეუძლებელია.';ru='Не возможно подключиться к Менеджеру сервиса.';en='It is not possible to connect to the Service Manager.'"));
...
Функция ЕстьАктивныйТарифМобильногоУНФ(Знач ОтветСтрокой)
	...
	Если ПустаяСтрока(ОтветСтрокой) Тогда
		ВызватьИсключение(НСтр("ka='ტარიფებთან დაკავშირებით სერვისის მენეჯერის ცარიელი პასუხი.';ru='Пустой ответ Менеджера сервиса по тарифам.';en='Blank Reply Manager service on tariffs.'"));
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ka='Мобильное приложение.Установка даты последней синхронизации в регламентном задании';ru='Мобильное приложение.Установка даты последней синхронизации в регламентном задании';en='Mobile prilozhenie.Ustanovka last synchronization date of scheduled jobs'"),
