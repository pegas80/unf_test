...
Процедура ПередНачаломРаботыКлиента(ТребуетсяОбновлениеИнтерфейса = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ka='აქტიური ტარიფების მიღება';ru='Получение активных тарифов'"), УровеньЖурналаРегистрации.Ошибка, , , ?(ЗначениеЗаполнено(ПодробноеОписание), ПодробноеОписание, Строка(СписокТарифиов)));
	КонецПопытки;
...
Функция ТекстОшибкиПодключения()
	...
	Возврат(НСтр("ka='სერვისის მენეჯერთან შეერთება შეუძლებელია.';ru='Не возможно подключиться к Менеджеру сервиса.'"));
...
Функция ЕстьАктивныйТарифМобильногоУНФ(Знач ОтветСтрокой)
	...
	Если ПустаяСтрока(ОтветСтрокой) Тогда
		ВызватьИсключение(НСтр("ka='ტარიფებთან დაკავშირებით სერვისის მენეჯერის ცარიელი პასუხი.';ru='Пустой ответ Менеджера сервиса по тарифам.'"));
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ka='Мобильное приложение.Установка даты последней синхронизации в регламентном задании';ru='Мобильное приложение.Установка даты последней синхронизации в регламентном задании'"),
