...
Процедура ПроверитьНаличиеИнвентаризации(ОрганизацияЕГАИС, ТекстОшибкиСклад, ТекстОшибкиТорговыйЗал) Экспорт
	...
	МассивСтроки.Добавить(?(ТипСклада = Перечисления.ТипыСтруктурныхЕдиниц.Склад, НСтр("ka='საწყობზე';ru='на складе';en='stock'"), НСтр("ru='в торговом зале';en='the trading floor'")));
	...
	Если РезультатЗапроса.Пустой() Тогда
		МассивСтроки = Новый Массив;
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='საქონლის ინვენტარიზაცია';ru='Инвентаризация товаров';en='inventory of goods'"),,,, "ОткрытьСписокИнвентаризаций"));
		МассивСтроки.Добавить(" ");
		МассивСтроки.Добавить(ПредставлениеСклада);
		МассивСтроки.Добавить(" ");
		МассивСтроки.Добавить(НСтр("ka='არ გატარებულა.';ru='не проводилась.';en='It was not carried out.'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НачалоДня(Выборка.Дата) < НачалоДня(ДобавитьМесяц(ТекущаяДатаСеанса(), -1)) Тогда
			...
			МассивСтроки.Добавить(ПредставлениеСклада);
			МассивСтроки.Добавить(" ");
			МассивСтроки.Добавить(НСтр("ka='проводилась больше месяца назад.';ru='проводилась больше месяца назад.';en='carried out more than a month ago.'"));
			МассивСтроки.Добавить(Символы.ПС);
			МассивСтроки.Добавить(НСтр("ka='Рекомендуется провести повторную инвентаризацию.';ru='Рекомендуется провести повторную инвентаризацию.';en='It is recommended to re-inventory.'"));
