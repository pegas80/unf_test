...
Процедура АнализДоступностиФайловКлассификатораВКаталогеЗавершение(ИндексРегиона, ДополнительныеПараметры) Экспорт
	...
	Иначе // окончание цикла
		...
		Для Каждого КлючЗначение Из Представления Цикл
			СообщениеОбОшибке = НСтр("ka='რაიონ ""%1""-ისთვის მონაცემთა ფაილი ""%2"" ვერ მოიძებნა';ru='Для региона ""%1"" не найден файл данных ""%2""'") + Символы.ПС;
			СообщениеОбОшибке = СообщениеОбОшибке + НСтр("ka='აქტუალური სამისამართო ცნობების ჩატვირთვა შეიძლება შემდეგ მისამართზე http://its.1c.ru/download/fias';ru='Актуальные адресные сведения можно загрузить по адресу http://its.1c.ru/download/fias'");
