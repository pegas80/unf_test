...
Функция СтруктураСклоненияЧерезЗапросКСервису(Представление, ПоказыватьСообщения) 
	...
	Если Результат <> Неопределено Тогда
		...
		Если Не ПустаяСтрока(СообщениеОбОшибке) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='ბრუნების ვებ-სერვისის მოხმობა';ru='Вызов веб-сервиса склонения'", КодОсновногоЯзыка), УровеньЖурналаРегистрации.Предупреждение, , ,СообщениеОбОшибке);
		КонецЕсли;	
		
	Иначе
		
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ЗаписьЖурналаРегистрации(НСтр("ka='ბრუნების ვებ-სერვისის მოხმობა';ru='Вызов веб-сервиса склонения'", КодОсновногоЯзыка), УровеньЖурналаРегистрации.Ошибка, , ,СообщениеОбОшибке);
		
		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეცდომა უღლების სერვისის მოხმობისას. მიმართეთ ადმინისტრატორს.';ru='Ошибка при вызове сервиса склонения. Обратитесь к администратору.'") + Символы.ПС + НСтр("ka='Техническая информация:';ru='Техническая информация:'") + Символы.ПС + СообщениеОбОшибке);
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты("ОбщийМакет.КомпонентаСклоненияФИО", 
		НСтр("ka='გსმს-ის საბრუნად ფუნქციის გამოსაყენებლად.';ru='Для использования функций по склонению ФИО.'")));
