...
Процедура ЗавершитьРаботуПользователей() Экспорт
	...
	Если ТекущийМомент < ВремяНачалаБлокировки Тогда
		ТекстСообщения = НСтр("ka='მომხმარებელთა მუშაობის ბლოკირება დაგეგმილია %1-სთვის.';ru='Блокировка работы пользователей запланирована на %1.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ВремяНачалаБлокировки);
		ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების მუშაობის დასრულება';ru='Завершение работы пользователей'"), 
	...
	Исключение
		СоединенияИБКлиент.УстановитьПризнакРаботаПользователейЗавершается(Ложь);
			ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების მუშაობის დასრულება';ru='Завершение работы пользователей'"),
			"e1cib/app/Обработка.БлокировкаРаботыПользователей", 
			НСтр("ka='სეანსების დასრულება არ არის შესრულებული. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Завершение сеансов не выполнено. Подробности см. в Журнале регистрации.'"), БиблиотекаКартинок.Предупреждение32);
	...
	ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების მუშაობის დასრულება';ru='Завершение работы пользователей'"),
		"e1cib/app/Обработка.БлокировкаРаботыПользователей", 
		НСтр("ka='სეანსები წარმატებით დასრულდა';ru='Завершение сеансов выполнено успешно'"), БиблиотекаКартинок.Информация32);
	Оповестить("ЗавершениеРаботыПользователей", Новый Структура("Статус,КоличествоСеансов", "Готово", КоличествоСеансов));
	СоединенияИБКлиент.ЗавершитьРаботуЭтогоСеанса();
