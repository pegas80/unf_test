...
"%1';ka='Ошибка создания менеджер криптографии (%2, %3, %4) по причине:"
"%1'"),
			СписокОшибок,
			ВходящийКонтекст.ДоступныеКриптопровайдеры[ВходящийКонтекст.ТекущийКриптопровайдер].Имя, 
			ВходящийКонтекст.ДоступныеКриптопровайдеры[ВходящийКонтекст.ТекущийКриптопровайдер].Путь, 
...
Процедура СоздатьМенеджерКриптографииПослеПолученияСпискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			СписокОшибок = НСтр("ka='Менеджер криптографии успешно инициализирован с указанными параметрами криптопровайдера, но криптопровайдер не найден в системе.';ru='Менеджер криптографии успешно инициализирован с указанными параметрами криптопровайдера, но криптопровайдер не найден в системе.';en='Cryptography manager has been initialized with the specified parameters CSP, cryptographic but is not found in the system.'");		
...
Процедура ПоказатьСертификатПослеПоиска(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			...
			СписокСертификатов.ПоказатьВыборЭлемента(Оповещение, НСтр("ka='Выберите сертификат для детального просмотра';ru='Выберите сертификат для детального просмотра';en='Select the certificate for a detailed view'"));
		КонецЕсли;
	Иначе
...
Процедура ОтобразитьОдинСертификатВПолеВвода(ДополнительныеПараметры)
	...
	Если ТипЗнчСертификат <> Тип("Структура") И ТипЗнчСертификат <> Тип("Соответствие") Тогда
		...
		Иначе
			...
			Результат.Вставить("ОписаниеОшибки", 	НСтр("ka='Не установлена компонента работы с криптографией';ru='Не установлена компонента работы с криптографией';en='Component is not installed with cryptography'"));
...
Процедура ОтобразитьОдинСертификатВПолеВводаПослеПолученияСвойствСертификата(Результат, ВходящиеПараметры) Экспорт
	...
	Иначе
		...
		ПолеВвода.РасширеннаяПодсказка.Заголовок	= Новый ФорматированнаяСтрока(
			НСтр("ka='Сертификат не зарегистрирован в системном хранилище сертификатов';ru='Сертификат не зарегистрирован в системном хранилище сертификатов';en='The certificate is not registered in the system certificate store'"),,
			Новый Цвет(255, 0, 0));
...
Процедура ОтобразитьОдинСертификатВПолеВводаПоСвойствамСертификата(Знач СвойстваСертифката, ВходящиеПараметры)
	...
	Если ТекДата > СвойстваСертифката.ДействителенПо Тогда
		...
		Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ka='Срок действия сертификата истек';ru='Срок действия сертификата истек';en='The certificate has expired'"),,
			Новый Цвет(255, 0, 0));
	...
	ИначеЕсли ТекДата < СвойстваСертифката.ДействителенС Тогда
		...
		Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ka='Срок действия сертификата еще не наступил';ru='Срок действия сертификата еще не наступил';en='The certificate has not yet come'"),,
			Новый Цвет(255, 0, 0));
...
Процедура ОтобразитьНесколькоСертификатовВПолеВводаПослеПолученияСвойствСертификатов(Результат, ВходящиеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ОшибкиЕсть Тогда
			ПолеВвода.ЦветТекста = Новый Цвет(255, 0, 0);
			Подсказка = Новый ФорматированнаяСтрока(
				НСтр("ka='Один или несколько сертификатов недействительны';ru='Один или несколько сертификатов недействительны';en='One or more certificates are invalid'"),,
				Новый Цвет(255, 0, 0));
			ПолеВвода.РасширеннаяПодсказка.Заголовок = Подсказка;
			ПолеВвода.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
...
"не зарегистрированы в системном хранилище сертификатов';ka='Обратите внимание, один или несколько сертификатов "
...
Функция ПредставлениеЧислаСертификатов(ЧислоСертификатов)
	...
	Если ЧислоСертификатов = 0 Тогда
		Результат = НСтр("ka='Сертификат не выбран';ru='Сертификат не выбран';en='The certificate is not selected'");
	ИначеЕсли ЧислоСертификатов = 1 Тогда
		Результат = НСтр("ka='1 სერტიფიკატი';ru='1 сертификат';en='1 certificate'");
	ИначеЕсли ЧислоСертификатов >= 2 И ЧислоСертификатов <= 4 Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერტიფიკატის %1';ru='%1  сертификата';en='% 1 certificate'"), ЧислоСертификатов);
	Иначе // считаем, что больше 20 сертификатов не будет, иначе следует дописать
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერტიფიკატების %1';ru='%1  сертификатов';en='% 1 certificates'"), ЧислоСертификатов);
	КонецЕсли;
...
Процедура РасшифроватьPKCS7(ОповещениеОЗавершении, ЗашифрованныйФайл, ИмяФайлаИлиРасширениеИлиАдрес, Сертификат, ВыводитьСоообщения, МенеджерКриптографии) Экспорт
	...
	Если Сертификат = Неопределено Или СертификатВЗащищенномХранилищеНаСервере(Сертификат) Тогда
		...
		Если НЕ ЭтоАдресВоВременномХранилище(ЗашифрованныйФайл) И ТипЗнч(ЗашифрованныйФайл)<>Тип("ДвоичныеДанные") Тогда
			...
			ОшибкаТекстом = НСтр("ru='Для электронной подписи в облаке данные могут быть переданы как двоичные данные или как адрес в хранилище';en='For electronic signature in a cloud data can be transmitted as binary data or address storage'");
...
Функция ОписаниеОшибкиНеУдалосьПодключитьВнешнююКомпонентуКриптографии()
	...
	Возврат НСтр("ka='Не удалось подключить внешнюю компоненту для работы с криптографией.';ru='Не удалось подключить внешнюю компоненту для работы с криптографией.';en='Failed to connect an external component to work with cryptography.'");

КонецФункции

Функция ОписаниеОшибкиНеУдалосьСоздатьМенеджерКриптографии()
	
	Возврат НСтр("ka='Не удалось создать объект для работы с криптографией. В системе не обнаружен криптопровайдер.';ru='Не удалось создать объект для работы с криптографией. В системе не обнаружен криптопровайдер.';en='Failed to create object for cryptography. The system is not detected crypto.'");
	
КонецФункции

Функция ОписаниеОшибкиОбщийТекстПриОшибкахВМенеджереКриптографии()
	
	Возврат НСтр("ka='Ошибка внешней компоненты для работы с криптографией.';ru='Ошибка внешней компоненты для работы с криптографией.';en='Error external components to work with cryptography.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьСформироватьЗапросНаСертификат()
	
	Возврат НСтр("ka='კრიპტოგრაფიული შეცდომა: მაკონტროლებელი ორგანოებისთვის სერთიფიკატზე მოთხოვნის ჩამოყალიბება ვერ მოხერხდა.';ru='Криптографическая ошибка: не удалось сформировать запрос на сертификат для контролирующих органов.';en='Cryptographic Error: Unable to generate a certificate request for the regulatory authorities.'");
...
"Возможно выбран не верный закрытый ключ.';ka='Не удалось привязать сертификат к закрытому ключу."
...
Функция ОписаниеОшибкиНеУдалосьУстановитьЛичныйСертификат()
	...
	Возврат НСтр("ka='პირადი სერთიფიკატის დადგენა ვერ მოხერხდა.';ru='Не удалось установить личный сертификат.';en='Unable to establish a personal certificate.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьВыгрузитьСертификат()
	
	Возврат НСтр("ka='Не удалось выгрузить сертификат.';ru='Не удалось выгрузить сертификат.';en='Unable to upload the certificate.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьИмпортироватьСертификат()
	
	Возврат НСтр("ka='Не удалось зарегистрировать сертификат в системном хранилище.';ru='Не удалось зарегистрировать сертификат в системном хранилище.';en='Failed to register the certificate in the system storage.'");
...
Функция ОписаниеОшибкиНеУдалосьПодписатьФайл()
	...
	Возврат НСтр("ru = 'Не удалось подписать файл по причине:
                  |%1'; ka = 'Не удалось подписать файл по причине:
                  |%1'");
...
Функция ОписаниеОшибкиНеУдалосьПроверитьПодпись()
	...
	Возврат НСтр("ka='Не удалось проверить подпись.';ru='Не удалось проверить подпись.';en='Unable to verify the signature.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьЗашифроватьФайл()

	Возврат НСтр("ka='Не удалось зашифровать файл.';ru='Не удалось зашифровать файл.';en='Failed to encrypt the file.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПодписатьИЗашифроватьФайл()

	Возврат НСтр("ka='Не удалось подписать и зашифровать файл.';ru='Не удалось подписать и зашифровать файл.';en='Unable to sign and encrypt the file.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьРасшифроватьФайл()

	Возврат НСтр("ka='Не удалось расшифровать файл.';ru='Не удалось расшифровать файл.';en='Unable to decrypt the file.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПолучитьСвойстваПодписи()

	Возврат НСтр("ka='Не удалось получить свойства подписи.';ru='Не удалось получить свойства подписи.';en='Could not get the properties of the signature.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПолучитьХешФайла()
	
	Возврат НСтр("ka='Не удалось получить хеш файла.';ru='Не удалось получить хеш файла.';en='Failed to get the hash file.'");
...
Функция ОписаниеОшибкиНеУдалосьНайтиСертификат(Сертификат = Неопределено)
	...
	ОписаниеОшибки = НСтр("ka='Не удалось найти сертификат <%1>  в системном хранилище.';ru='Не удалось найти сертификат <%1>  в системном хранилище.';en='Unable to find the certificate <% 1> in the system storage.'");
...
Функция ОписаниеОшибкиНеВведенПарольДоступаКЗакрытомуКлючу()
	...
	Возврат НСтр("ka='Операция прервана: требуется ввести пароль доступа к закрытому ключу!';ru='Операция прервана: требуется ввести пароль доступа к закрытому ключу!';en='Operation aborted: A password is required to access the private key!'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьОткрытьСертификат()

	Возврат НСтр("ka='სერთიფიკატის გახსნა ვერ მოხერხდა.';ru='Не удалось открыть сертификат.';en='Unable to open the certificate.'");
...
Функция ПолучитьСписокХранилищСертификатов(ХранилищеСертификатов)
	...
	ИначеЕсли ТипЗнч(ХранилищеСертификатов) = Тип("Структура") Тогда
		...
		КонецЕсли;
	Иначе
		ВызватьИсключение(НСтр("ka='Параметр <ХранилищеСертификатов> должен иметь тип Структура, Массив или Неопределено.';ru='Параметр <ХранилищеСертификатов> должен иметь тип Структура, Массив или Неопределено.';en='The <HranilischeSertifikatov> must be of type structure array or Undefined.'"));
	КонецЕсли;
...
"%2"
"по причине:"
"%3';ka='Ошибка при вызове ""%1"" менеджера криптографии."
"по причине:"
"%2"
"по причине:"
"%3'"),
	...
	Иначе
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при вызове ""%1"" менеджера криптографии."
"по причине:"
"%2';ka='Ошибка при вызове ""%1"" менеджера криптографии."
"по причине:"
"%2'"),
...
Процедура МенеджерКриптографииПриОшибкеПолученияЗначенияСвойстваСписокОшибок(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	ОписаниеОшибки = НСтр("ka='Ошибка чтения свойства ""СписокОшибок"" менеджера криптографии.';ru='Ошибка чтения свойства ""СписокОшибок"" менеджера криптографии.';en='Error reading the properties ""SpisokOshibok"" cryptography manager.'");
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ka='Электронный документооборот с контролирующими органами. Менеджер криптографии';ru='Электронный документооборот с контролирующими органами. Менеджер криптографии';en='Electronic document management with regulatory authorities. cryptography manager'");
...
Функция ЗаполнитьПараметрыКриптографии(ДоступныеКриптопровайдеры, ОписаниеОшибки)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПутьМодуляКриптографии) Тогда
			ОписаниеОшибки = НСтр("ka='Не указан путь модуля криптографии в настройках программы.';ru='Не указан путь модуля криптографии в настройках программы.';en='Unknown path cryptographic module in the program settings.'");
			Возврат Ложь;
		КонецЕсли;            
...
Функция ОписаниеОшибкиНеУказаныКлючевыеПараметрыСертификатаДляПоиска()
	...
	Возврат НСтр("ka='Не указаны ключевые параметры сертификата для поиска.';ru='Не указаны ключевые параметры сертификата для поиска.';en='Not specified certificate key parameters for the search.'");
