...
Процедура СоздатьМенеджерКриптографииПослеПолученияСпискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			СписокОшибок = НСтр("ka='Менеджер криптографии успешно инициализирован с указанными параметрами криптопровайдера, но криптопровайдер не найден в системе.';ru='Менеджер криптографии успешно инициализирован с указанными параметрами криптопровайдера, но криптопровайдер не найден в системе.'");		
...
Процедура ПоказатьСертификатПослеПоиска(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			...
			СписокСертификатов.ПоказатьВыборЭлемента(Оповещение, НСтр("ka='Выберите сертификат для детального просмотра';ru='Выберите сертификат для детального просмотра'"));
		КонецЕсли;
	Иначе
...
Процедура ОтобразитьОдинСертификатВПолеВвода(ДополнительныеПараметры)
	...
	Если ТипЗнчСертификат <> Тип("Структура") И ТипЗнчСертификат <> Тип("Соответствие") Тогда
		...
		Иначе
			...
			Результат.Вставить("ОписаниеОшибки", 	НСтр("ka='Не установлена компонента работы с криптографией';ru='Не установлена компонента работы с криптографией'"));
...
Процедура ОтобразитьОдинСертификатВПолеВводаПослеПолученияСвойствСертификата(Результат, ВходящиеПараметры) Экспорт
	...
	Иначе
		...
		ПолеВвода.РасширеннаяПодсказка.Заголовок	= Новый ФорматированнаяСтрока(
			НСтр("ka='Сертификат не зарегистрирован в системном хранилище сертификатов';ru='Сертификат не зарегистрирован в системном хранилище сертификатов'"),,
			Новый Цвет(255, 0, 0));
...
Процедура ОтобразитьОдинСертификатВПолеВводаПоСвойствамСертификата(Знач СвойстваСертифката, ВходящиеПараметры)
	...
	Если ТекДата > СвойстваСертифката.ДействителенПо Тогда
		...
		Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ka='Срок действия сертификата истек';ru='Срок действия сертификата истек'"),,
			Новый Цвет(255, 0, 0));
	...
	ИначеЕсли ТекДата < СвойстваСертифката.ДействителенС Тогда
		...
		Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ka='Срок действия сертификата еще не наступил';ru='Срок действия сертификата еще не наступил'"),,
			Новый Цвет(255, 0, 0));
...
Процедура ОтобразитьНесколькоСертификатовВПолеВводаПослеПолученияСвойствСертификатов(Результат, ВходящиеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ОшибкиЕсть Тогда
			ПолеВвода.ЦветТекста = Новый Цвет(255, 0, 0);
			Подсказка = Новый ФорматированнаяСтрока(
				НСтр("ka='Один или несколько сертификатов недействительны';ru='Один или несколько сертификатов недействительны'"),,
				Новый Цвет(255, 0, 0));
			ПолеВвода.РасширеннаяПодсказка.Заголовок = Подсказка;
			ПолеВвода.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
...
Функция ПредставлениеЧислаСертификатов(ЧислоСертификатов)
	...
	Если ЧислоСертификатов = 0 Тогда
		Результат = НСтр("ka='Сертификат не выбран';ru='Сертификат не выбран'");
	ИначеЕсли ЧислоСертификатов = 1 Тогда
		Результат = НСтр("ka='1 სერტიფიკატი';ru='1 сертификат'");
	ИначеЕсли ЧислоСертификатов >= 2 И ЧислоСертификатов <= 4 Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერტიფიკატის %1';ru='%1  сертификата'"), ЧислоСертификатов);
	Иначе // считаем, что больше 20 сертификатов не будет, иначе следует дописать
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერტიფიკატების %1';ru='%1  сертификатов'"), ЧислоСертификатов);
	КонецЕсли;
...
Функция ОписаниеОшибкиНеУдалосьПодключитьВнешнююКомпонентуКриптографии()
	...
	Возврат НСтр("ka='Не удалось подключить внешнюю компоненту для работы с криптографией.';ru='Не удалось подключить внешнюю компоненту для работы с криптографией.'");

КонецФункции

Функция ОписаниеОшибкиНеУдалосьСоздатьМенеджерКриптографии()
	
	Возврат НСтр("ka='Не удалось создать объект для работы с криптографией. В системе не обнаружен криптопровайдер.';ru='Не удалось создать объект для работы с криптографией. В системе не обнаружен криптопровайдер.'");
	
КонецФункции

Функция ОписаниеОшибкиОбщийТекстПриОшибкахВМенеджереКриптографии()
	
	Возврат НСтр("ka='Ошибка внешней компоненты для работы с криптографией.';ru='Ошибка внешней компоненты для работы с криптографией.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьСформироватьЗапросНаСертификат()
	
	Возврат НСтр("ka='კრიპტოგრაფიული შეცდომა: მაკონტროლებელი ორგანოებისთვის სერთიფიკატზე მოთხოვნის ჩამოყალიბება ვერ მოხერხდა.';ru='Криптографическая ошибка: не удалось сформировать запрос на сертификат для контролирующих органов.'");
...
Функция ОписаниеОшибкиНеУдалосьУстановитьЛичныйСертификат()
	...
	Возврат НСтр("ka='პირადი სერთიფიკატის დადგენა ვერ მოხერხდა.';ru='Не удалось установить личный сертификат.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьВыгрузитьСертификат()
	
	Возврат НСтр("ka='Не удалось выгрузить сертификат.';ru='Не удалось выгрузить сертификат.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьИмпортироватьСертификат()
	
	Возврат НСтр("ka='Не удалось зарегистрировать сертификат в системном хранилище.';ru='Не удалось зарегистрировать сертификат в системном хранилище.'");
...
Функция ОписаниеОшибкиНеУдалосьПроверитьПодпись()
	...
	Возврат НСтр("ka='Не удалось проверить подпись.';ru='Не удалось проверить подпись.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьЗашифроватьФайл()

	Возврат НСтр("ka='Не удалось зашифровать файл.';ru='Не удалось зашифровать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПодписатьИЗашифроватьФайл()

	Возврат НСтр("ka='Не удалось подписать и зашифровать файл.';ru='Не удалось подписать и зашифровать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьРасшифроватьФайл()

	Возврат НСтр("ka='Не удалось расшифровать файл.';ru='Не удалось расшифровать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПолучитьСвойстваПодписи()

	Возврат НСтр("ka='Не удалось получить свойства подписи.';ru='Не удалось получить свойства подписи.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПолучитьХешФайла()
	
	Возврат НСтр("ka='Не удалось получить хеш файла.';ru='Не удалось получить хеш файла.'");
...
Функция ОписаниеОшибкиНеУдалосьНайтиСертификат(Сертификат = Неопределено)
	...
	ОписаниеОшибки = НСтр("ka='Не удалось найти сертификат <%1>  в системном хранилище.';ru='Не удалось найти сертификат <%1>  в системном хранилище.'");
...
Функция ОписаниеОшибкиНеВведенПарольДоступаКЗакрытомуКлючу()
	...
	Возврат НСтр("ka='Операция прервана: требуется ввести пароль доступа к закрытому ключу!';ru='Операция прервана: требуется ввести пароль доступа к закрытому ключу!'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьОткрытьСертификат()

	Возврат НСтр("ka='სერთიფიკატის გახსნა ვერ მოხერხდა.';ru='Не удалось открыть сертификат.'");
...
Функция ПолучитьСписокХранилищСертификатов(ХранилищеСертификатов)
	...
	ИначеЕсли ТипЗнч(ХранилищеСертификатов) = Тип("Структура") Тогда
		...
		КонецЕсли;
	Иначе
		ВызватьИсключение(НСтр("ka='Параметр <ХранилищеСертификатов> должен иметь тип Структура, Массив или Неопределено.';ru='Параметр <ХранилищеСертификатов> должен иметь тип Структура, Массив или Неопределено.'"));
	КонецЕсли;
...
Процедура МенеджерКриптографииПриОшибкеПолученияЗначенияСвойстваСписокОшибок(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	ОписаниеОшибки = НСтр("ka='Ошибка чтения свойства ""СписокОшибок"" менеджера криптографии.';ru='Ошибка чтения свойства ""СписокОшибок"" менеджера криптографии.'");
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ka='Электронный документооборот с контролирующими органами. Менеджер криптографии';ru='Электронный документооборот с контролирующими органами. Менеджер криптографии'");
...
Функция ЗаполнитьПараметрыКриптографии(ДоступныеКриптопровайдеры, ОписаниеОшибки)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПутьМодуляКриптографии) Тогда
			ОписаниеОшибки = НСтр("ka='Не указан путь модуля криптографии в настройках программы.';ru='Не указан путь модуля криптографии в настройках программы.'");
			Возврат Ложь;
		КонецЕсли;            
...
Функция ОписаниеОшибкиНеУказаныКлючевыеПараметрыСертификатаДляПоиска()
	...
	Возврат НСтр("ka='Не указаны ключевые параметры сертификата для поиска.';ru='Не указаны ключевые параметры сертификата для поиска.'");
