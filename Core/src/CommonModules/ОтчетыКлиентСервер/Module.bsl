...
//               //	Команда.Заголовок = НСтр("ka='ჩემი ბრძანება';ru='Моя команда...';en='My team...'");
//               //	
...
Функция ПредставлениеЭлементаУсловногоОформления(ЭлементКД) Экспорт
	...
	Если ПредставлениеОформления = "" Тогда
		ПредставлениеОформления = НСтр("ka='არ არის გაფორმებული';ru='Не оформлять';en='Do not draw'");
	КонецЕсли;
	
	ПредставлениеПолей = Строка(ЭлементКД.Поля);
	Если ПредставлениеПолей = "" Тогда
		ПредставлениеПолей = НСтр("ka='ყველა ველი';ru='Все поля';en='All fields'");
	Иначе
		ПредставлениеПолей = НСтр("ka='ველები:';ru='Поля:';en='fields:'") + " " + ПредставлениеПолей;
	КонецЕсли;
	...
	Иначе
		Разделитель = "; ";
		ПредставлениеОтбора = НСтр("ka='პირობა:';ru='Условие:';en='Condition:'") + " " + ПредставлениеОтбора;
	КонецЕсли;
...
Функция ПредставлениеЦвета(Цвет)
	...
	ИначеЕсли Цвет.Вид = ВидЦвета.Абсолютный Тогда
		...
		Если Представление = "0, 0, 0" Тогда
			Представление = НСтр("ka='შავი';ru='Черный';en='The black'");
		ИначеЕсли Представление = "255, 255, 255" Тогда
			Представление = НСтр("ka='თეთრი';ru='Белый';en='White'");
		КонецЕсли;
	ИначеЕсли Цвет.Вид = ВидЦвета.АвтоЦвет Тогда
		Представление = НСтр("ka='ავტო';ru='Авто';en='Auto'");
	Иначе
		Представление = "";
	КонецЕсли;
	Возврат Представление;
...
Функция ПараметрыКопирования(ТипЭлемента, Коллекция)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ელემენტების კოპირებას ""%1"" მხარდაჭერა არ გააჩნია';ru='Копирование элементов ""%1"" не поддерживается';en='Copy Items ""%1"" not supported'"),
			ТипЭлемента);
...
Функция СписокВыбораВидовСравнения(ОписаниеТипов) Экспорт
	...
	Если ИнформацияОТипах.ОграниченнойДлины Тогда
		...
		Если ИнформацияОТипах.СодержитОбъектныеТипы Тогда
			...
			Список.Добавить(ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии); // НСтр("ka='სიაში, დაქვემდებარებულების ჩათვლით';ru='В списке включая подчиненные';en='The list including subordinates'")
			Список.Добавить(ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии); // НСтр("ka='არა სიაში დაქვემდებარებულების ჩათვლით';ru='Не в списке включая подчиненные';en='Not on the list including subordinates'").
			
			Список.Добавить(ВидСравненияКомпоновкиДанных.ВИерархии); // НСтр("ka='ჯგუფში';ru='В группе';en='In a group'")
			Список.Добавить(ВидСравненияКомпоновкиДанных.НеВИерархии); // НСтр("ka='არა ჯგუფში';ru='Не в группе';en='Not in the group'")
