...
//               //	Команда.Заголовок = НСтр("ka='ჩემი ბრძანება';ru='Моя команда...'");
//               //	
...
Функция ПредставлениеЭлементаУсловногоОформления(ЭлементКД) Экспорт
	...
	Если ПредставлениеОформления = "" Тогда
		ПредставлениеОформления = НСтр("ka='არ არის გაფორმებული';ru='Не оформлять'");
	КонецЕсли;
	
	ПредставлениеПолей = Строка(ЭлементКД.Поля);
	Если ПредставлениеПолей = "" Тогда
		ПредставлениеПолей = НСтр("ka='ყველა ველი';ru='Все поля'");
	Иначе
		ПредставлениеПолей = НСтр("ka='ველები:';ru='Поля:'") + " " + ПредставлениеПолей;
	КонецЕсли;
	...
	Иначе
		Разделитель = "; ";
		ПредставлениеОтбора = НСтр("ka='პირობა:';ru='Условие:'") + " " + ПредставлениеОтбора;
	КонецЕсли;
...
Функция ПредставлениеЦвета(Цвет)
	...
	ИначеЕсли Цвет.Вид = ВидЦвета.Абсолютный Тогда
		...
		Если Представление = "0, 0, 0" Тогда
			Представление = НСтр("ka='შავი';ru='Черный'");
		ИначеЕсли Представление = "255, 255, 255" Тогда
			Представление = НСтр("ka='თეთრი';ru='Белый'");
		КонецЕсли;
	ИначеЕсли Цвет.Вид = ВидЦвета.АвтоЦвет Тогда
		Представление = НСтр("ka='ავტო';ru='Авто'");
	Иначе
		Представление = "";
	КонецЕсли;
	Возврат Представление;
...
Функция ПараметрыКопирования(ТипЭлемента, Коллекция)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ელემენტების კოპირებას ""%1"" მხარდაჭერა არ გააჩნია';ru='Копирование элементов ""%1"" не поддерживается'"),
			ТипЭлемента);
...
Функция СписокВыбораВидовСравнения(ОписаниеТипов) Экспорт
	...
	Если ИнформацияОТипах.ОграниченнойДлины Тогда
		...
		Если ИнформацияОТипах.СодержитОбъектныеТипы Тогда
			...
			Список.Добавить(ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии); // НСтр("ka='სიაში, დაქვემდებარებულების ჩათვლით';ru='В списке включая подчиненные'")
			Список.Добавить(ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии); // НСтр("ka='არა სიაში დაქვემდებარებულების ჩათვლით';ru='Не в списке включая подчиненные'").
			
			Список.Добавить(ВидСравненияКомпоновкиДанных.ВИерархии); // НСтр("ka='ჯგუფში';ru='В группе'")
			Список.Добавить(ВидСравненияКомпоновкиДанных.НеВИерархии); // НСтр("ka='არა ჯგუფში';ru='Не в группе'")
