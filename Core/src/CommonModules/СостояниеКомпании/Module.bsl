...
Процедура ИнтерактивноПодготовитьОтчетДляПросмотраНаЭкране(Знач ПараметрыЗадания, Знач АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Подготовка отчета ""Состояние компании"" для просмотра на экране';ru='Подготовка отчета ""Состояние компании"" для просмотра на экране'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ИнтерактивноОтправитьОтчетПоЭлектроннойПочте(Знач ПараметрыЗадания, Знач АдресХранилища) Экспорт
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='Отправка отчета ""Состояние компании"" по электронной почте';ru='Отправка отчета ""Состояние компании"" по электронной почте'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		СообщитьОбОшибкеДлительногоЗадания(ИнформацияОбОшибке);
...
Функция АвтоНаименованиеНастройки() Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='კომპანიის მდგომარეობის ანგარიშგება %1';ru='Сводный отчет о состоянии компании %1'"), КоличествоНастроек);
...
Процедура ЗаполнитьСодержимоеСекции(Знач НастройкаСекции, Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач ОписанияВсехСекций)
	...
	Если Не ОписанияВсехСекций.Свойство(НастройкаСекции.ТипСекции) Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Указан недопустимый ключ секции: ""%1"".';ru='Указан недопустимый ключ секции: ""%1"".'"), НастройкаСекции.ТипСекции);
	КонецЕсли;
...
Функция КлючФильтра(ТекФильтр)
	...
	НСтр("ka='Для поля ""%1"" указан неподдерживаемый вид сравнения ""%2"".';ru='Для поля ""%1"" указан неподдерживаемый вид сравнения ""%2"".'"),
	ТекФильтр.Путь,
...
Процедура Подключаемый_СекцияДеньги(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции)
	...
	ВывестиЗаголовокПоМестамХранения(ТабличныйДокумент, ДеньгиВКассах, НСтр("ka='სალაროებში';ru='В кассах'"), ТаблицаВалют);
	ВывестиЗаголовокПоМестамХранения(ТабличныйДокумент, ДеньгиНаСчетах, НСтр("ka='ანგარიშებზე';ru='На счетах'"), ТаблицаВалют);
	ВывестиЗаголовокПоМестамХранения(ТабличныйДокумент, ДеньгиУПодотчетников, НСтр("ka='ქვეპასუხისმგებლებთან';ru='У подотчетников'"), ТаблицаВалют);
...
Процедура Подключаемый_СекцияДебиторкаДолгиПокупателей(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции)
	...
	Если ДЗадолженностьОстальные <> 0 Тогда
		...
		ОбластьСтрокаКонтрагент.Параметры.Контрагент = СтрШаблон(НСтр("ka='დარჩენილი(%!) ';ru='Остальные (%1)'"), Счетчик - 10);
...
Процедура Подключаемый_СекцияПродажиПокупатели(Знач ТабличныйДокумент, Знач Организация, Знач ПериодОтчета, Знач НастройкаСекции)
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		Иначе
			ОбластьПокупатель.Параметры.Покупатель = НСтр("ka='საცალო მყიდველი';ru='Розничный покупатель'");
		КонецЕсли;
	...
	Если ПокупателиОстаток > 0 Тогда
		...
		ПокупателиОстаток, НСтр("ka='покупатель,покупателя,покупателей';ru='покупатель,покупателя,покупателей'"));
