...
Процедура ЗавершитьОбновление(Знач РезультатОбновления, Знач ЭлектроннаяПочта, Знач ИмяАдминистратораОбновления) Экспорт
	...
	ТекстСообщения = НСтр("ka='გარე სკრიპტიდან განახლების დასრულება.';ru='Завершение обновления из внешнего скрипта.';en='Completion of the renovation of the external script.'");
	...
	Если Не ЕстьПраваНаУстановкуОбновления() Тогда
		ТекстСообщения = НСтр("ka='კონფიგურაციის განახლების დასასრულებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для завершения обновления конфигурации.';en='Not have permission to complete the configuration updates.'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями")
		...
		Попытка
			...
			ТекстСообщения = НСтр("ka='შეტყობინება განახლების თაობაზე წარმატებით გაიგზავნა ელექტრონული ფოსტის შემდეგ მისამართზე:';ru='Уведомление об обновлении успешно отправлено на адрес электронной почты:';en='An update notification is successfully sent to the email address:'")
				+ " " + ЭлектроннаяПочта;
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,,ТекстСообщения);
		Исключение
			ТекстСообщения = НСтр("ka='შეცდომა ელექტრონული ფოსტით წერილის გაგზავნისას:';ru='Ошибка при отправке письма электронной почты:';en='Error sending letters to e-mail:'")
...
Процедура ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, Знач АдресНазначения, Знач УспешноеОбновление)
	...
	Тема = ? (УспешноеОбновление, НСтр("ka='კონფიგურაციის წარმატებული განახლება ""%1"", ვერსია ""%2""';ru='Успешное обновление конфигурации ""%1"", версия %2';en='Successful configuration update ""%1"" version of% 2'"), 
		НСтр("ka='კონფიგურაცია ""%1""-ის განახლების შეცდომა, ვერსია %2';ru='Ошибка обновления конфигурации ""%1"", версия %2';en='update the configuration error ""%1"" version of% 2'"));
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Тема, Метаданные.КраткаяИнформация, Метаданные.Версия);
	
	Подробности = ?(УспешноеОбновление, НСтр("ka='კონფიგურაციის განახლება წარმატებით დასრულდა.';ru='Обновление конфигурации завершено успешно.';en='configuration update is completed successfully.'"), 
		НСтр("ka='კონფიგურაციის განახლებისას შეცდომა მოხდა. დაწვრილებითი ინფორმაცია ჩაწერილია რეგისტრაციის ჟურნალში.';ru='При обновлении конфигурации произошли ошибки. Подробности записаны в журнал регистрации.';en='When updating a configuration error occurred. Details are recorded in the log.'"));
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1"
""
"კონფიგურაცია: %2"
"ვერსია: %3"
"შეერთების სტრიქონი: %4';ru='%1"
""
"Конфигурация: %2"
"Версия: %3"
"Строка соединения: %4'"),
