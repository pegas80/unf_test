...
Процедура ЗавершитьОбновление(Знач РезультатОбновления, Знач ЭлектроннаяПочта, Знач ИмяАдминистратораОбновления) Экспорт
	...
	ТекстСообщения = НСтр("ka='გარე სკრიპტიდან განახლების დასრულება.';ru='Завершение обновления из внешнего скрипта.'");
	...
	Если Не ЕстьПраваНаУстановкуОбновления() Тогда
		ТекстСообщения = НСтр("ka='კონფიგურაციის განახლების დასასრულებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для завершения обновления конфигурации.'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями")
		...
		Попытка
			...
			ТекстСообщения = НСтр("ka='შეტყობინება განახლების თაობაზე წარმატებით გაიგზავნა ელექტრონული ფოსტის შემდეგ მისამართზე:';ru='Уведомление об обновлении успешно отправлено на адрес электронной почты:'")
				+ " " + ЭлектроннаяПочта;
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,,ТекстСообщения);
		Исключение
			ТекстСообщения = НСтр("ka='შეცდომა ელექტრონული ფოსტით წერილის გაგზავნისას:';ru='Ошибка при отправке письма электронной почты:'")
...
Процедура ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, Знач АдресНазначения, Знач УспешноеОбновление)
	...
	Тема = ? (УспешноеОбновление, НСтр("ka='კონფიგურაციის წარმატებული განახლება ""%1"", ვერსია ""%2""';ru='Успешное обновление конфигурации ""%1"", версия %2'"), 
		НСтр("ka='კონფიგურაცია ""%1""-ის განახლების შეცდომა, ვერსია %2';ru='Ошибка обновления конфигурации ""%1"", версия %2'"));
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Тема, Метаданные.КраткаяИнформация, Метаданные.Версия);
	
	Подробности = ?(УспешноеОбновление, НСтр("ka='კონფიგურაციის განახლება წარმატებით დასრულდა.';ru='Обновление конфигурации завершено успешно.'"), 
		НСтр("ka='კონფიგურაციის განახლებისას შეცდომა მოხდა. დაწვრილებითი ინფორმაცია ჩაწერილია რეგისტრაციის ჟურნალში.';ru='При обновлении конфигурации произошли ошибки. Подробности записаны в журнал регистрации.'"));
