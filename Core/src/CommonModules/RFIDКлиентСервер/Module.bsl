	...
	Если ЗначениеЗаполнено(GTIN)
		...
		Если ОбрабатываемаяСтрока.ЗаполненRFIDTID Тогда
			...
			Иначе
				...
				ТекстСообщения = НСтр("ka='არაპერსონიფიცირებული მარკირებისას საკონტროლო საიდენტიფიკაციო ნომერი უნდა იყვეს გენერირებული TID RFID-ით ნიშნულით. გენერირებისას მოხდა შეცდომა % შეცდომის ტექსტი% მიმართეთ ადმინისტრატორს.';ru='При маркировке непесонифицированными КиЗ номер серии должен быть сгенирирован по TID RFID-метки. При генерации произошла ошибка: %ТекстОшибки%. Обратитесь к администратору.';en='When marking nepesonifitsirovannymi Keyes series number must be on sgenirirovan TID RFID-tags. When generating the error occurred:% TekstOshibki%. Contact your administrator.'");
