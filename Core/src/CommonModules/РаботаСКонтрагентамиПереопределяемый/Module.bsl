...
Процедура ЗаполнитьОписаниеДанныхПрограммы(ТаблицаОписания) Экспорт
	...
	НоваяСтрока.Заголовок  = НСтр("ka='დასახელება';ru='Наименование';en='Name'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Реквизиты";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ka='რეკვიზიტები';ru='Реквизиты';en='Requisites'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "ЮридическийАдрес";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ka='იურიდიული მისამართი';ru='Юридический адрес';en='Legal address'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Телефон";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ka='ტელეფონი';ru='Телефон';en='Phone'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Email";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ka='e-mail';ru='E-mail';en='E-mail'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "КонтактныеЛица";
	НоваяСтрока.ЭтоТаблица = Истина;
	НоваяСтрока.Заголовок  = НСтр("ka='საკონტაქტო პირები';ru='Контактные лица';en='Contact faces'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "БанковскиеСчета";
	НоваяСтрока.ЭтоТаблица = Истина;
	НоваяСтрока.Заголовок  = НСтр("ka='საბანკო ანგარიშები';ru='Банковские счета';en='bank accounts'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Родитель";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ka='ჯგუფი';ru='Группа';en='Group'");
...
#Область ЗаполнениеРеквизитовКонтрагента
	...
	СтруктураРеквизиты.Представление = СтрШаблон(НСтр("ka='ИНН %1';ru='ИНН %1';en='1% TIN'"), Реквизиты.ИНН);
	...
	Если Реквизиты.ВидКонтрагента = Перечисления.ВидыКонтрагентов.ЮридическоеЛицо Или Реквизиты.ВидКонтрагента = Перечисления.ВидыКонтрагентов.ГосударственныйОрган Тогда
		...
		СтруктураРеквизиты.Описание = СтрШаблон(НСтр("ka='КПП %1';ru='КПП %1';en='CAT 1%'"), Реквизиты.КПП);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.КоррСчет) Тогда
			Представление = СтрШаблон(
				НСтр("ka='საანგარიშსწორებო ანგარიში %1, ბსკ %2, საკორ. ანგარიში %3';ru='р/с %1, БИК %2, корр. счет %3';en='p /% 1,% 2 BIK, corr. score 3%'"),
				Выборка.НомерСчета,
				Выборка.БИК,
				Выборка.КоррСчет);
		Иначе
			Представление = СтрШаблон(
				НСтр("ka='საანგარიშსწ/ანგ %1, ბსკ %2';ru='р/с %1, БИК %2';en='p /% 1,% 2 BIC'"),
				Выборка.НомерСчета,
				Выборка.БИК);
			КонецЕсли;
		...
		Если ИнформацияОСвязях.Свойство("ЮридическийАдрес") И ЗначениеЗаполнено(ИнформацияОСвязях.ЮридическийАдрес.Представление) Тогда
			...
			Если ВывестиПримечание Тогда
				СтруктураЮридическийАдрес.ПримечаниеПредставление = СтрШаблон(
					НСтр("ka='იპესრ-ის მონაცემებით:"
"%1';ru='По данным ЕГРЮЛ:"
"%1'"),
