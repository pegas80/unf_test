...
Функция ВыгрузитьТовары(Параметры, Товары, ЧастичнаяВыгрузка, ВыходныеПараметры, РасширеннаяВыгрузка) 
	...
	Если ЧастичнаяВыгрузка И НЕ МожноВыгружатьТовары(Параметры) Тогда
		...
		ВыходныеПараметры.Добавить(999);
		ОписаниеОшибки = НСтр("ka='გადმოტვირთვის გაკეთება არ შეიძლება. წინა გადმოტვირთვის საქონელი ჯერ არ არის მიღებული სსა-offline';ru='Нельзя сделать выгрузку. Товары предыдущей выгрузки еще не были получены ККМ-offline'");
	...
	Если Результат Тогда
		...
		Исключение
			...
			ВыходныеПараметры.Добавить(999);
			ОписаниеОшибки = НСтр("ka='საქონლის ფაილის ჩაწერა ვერ მოხერხდა შემდეგ მისამართზე: %Адрес%';ru='Не удалось записать файл товаров по адресу: %Адрес%'");
...
Функция ЗагрузитьОтчет(Параметры, ВыходныеПараметры) 
	...
	Исключение
		ВыходныеПараметры.Добавить(999);
		ОписаниеОшибки = НСтр("ka='ანგარიშგების ფაილის წაკითხვა შემდეგ მისამართზე ვერ მოხერხდა: %Адрес%';ru='Не удалось прочитать файл отчета по адресу: %Адрес%'");
...
Функция ОтчетЗагружен(Параметры, ВыходныеПараметры) 
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='ანგარიშგება %1-ის მონიშვნისას შეცდომა მოხდა. ანგარიშგების დამუშავება არ არის დაფიქსირებული!';ru='При пометке отчета %1 произошла ошибка. Обработка отчета не зафиксирована!'");
...
Функция ТестУстройства(Параметры, ВыходныеПараметры) 
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = НСтр("ka='Файл базы товаров не указан.';ru='Файл базы товаров не указан.'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		...
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='ანგარიშგების ფაილი არ არის მითითებული.';ru='Файл отчета не указан.'") 
	КонецЕсли;
...
Процедура ПроверкаОбработанностиФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успешно Тогда
		...
		Если СтрДлина(Строка) = 1 И Найти(Строка,"#") > 0 Тогда
			...
			ОписаниеОповещения.ДополнительныеПараметры.ВыходныеПараметры.Добавить(НСтр("ka='გადმოტვირთვის გაკეთება არ შეიძლება. წინა გადმოტვირთვის საქონელი ჯერ არ არის მიღებული სსა-offline';ru='Нельзя сделать выгрузку. Товары предыдущей выгрузки еще не были получены ККМ-offline'"));
...
Процедура ВыгрузкаНастроекЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ka='საქონლის ფაილის ჩაწერა ვერ მოხერხდა შემდეგ მისამართზე: %Адрес%';ru='Не удалось записать файл товаров по адресу: %Адрес%'");
...
Процедура ПроверкаСуществованияФайлаПриЗагрузкеОтчетаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВыходныеПараметры.Добавить(999);
		ОписаниеОшибки = НСтр("ka='ფაილი %Адрес% არ არსებობს!';ru='Файл %Адрес% не существует!'");
		ВыходныеПараметры.Добавить(СтрЗаменить(ОписаниеОшибки, "%Адрес%", ИмяФайла));
...
Функция ЗаполнитьТранзакцию(Строка, Транзакция, ТекстОшибки)
	...
	Попытка
		...
		ПолеОшибки = Нстр("ka='ტრანზაქციის ნომერი (1)';ru='Номер транзакции (1)'");
		
		// Номер транзакции.
		Транзакция.Вставить("Номер", Число(СтрПолучитьСтроку(Строка, 1)));
		
		ПолеОшибки = Нстр("ka='ტრანზაქციის თარიღი და დრო (2,3)';ru='Дата и время транзакции (2,3)'");
	...
	Исключение
		...
		ТекстОшибки = НСтр("ka='ფაილის არასწორი ფორმატი. ველის ამოცნობა შეუძლებელია: %1%';ru='Неверный формат файла. Невозможно распознать поле: %1%'");
...
Функция УдалитьТранзакциюИзЧека(Смена, Транзакция, ТекстОшибки)
	...
	Иначе
		...
		Если НЕ Результат Тогда
			ТекстОшибки = НСтр("ka='ოპერაცია შეწყვეტილია. შეცდომა ტრანზაქცია №%1%-ის ჩატვირთვისას!';ru='Операция прервана. Ошибка при загрузке транзакции №%1%!'");
...
Функция ЗакрытьЧек(Смена, НомерЧека, ТекстОшибки)
	...
	Если ТекущийЧек = Неопределено Тогда
		ТекстОшибки = НСтр("ka='ფაილის არასწორი ფორმატი. ჩეკის მოძებნა შეუძლებელია #%1%';ru='Неверный формат файла. Невозможно найти чек №%1%'");
...
Функция РазобратьРасширенныйОтчет(ТекстовыйДокумент, ВыходныеПараметры)
	...
	ИначеЕсли Строка = "@" Тогда
		ТекстОшибки = НСтр("ka='ოპერაცია შეწყვეტილია! ანგარიშგება უკვე ჩაიტვირთა.';ru='Операция прервана! Отчет уже был загружен.'");
		Результат = Ложь;
	Иначе
		ТекстОшибки = НСтр("ka='ჩატვირთვა შეუძლებელია! ანგარიშგების უცნობი ფორმატი.';ru='Загрузка невозможна! Неизвестный формат отчета.'");
		Результат = Ложь;
	КонецЕсли;
...
Функция РазобратьОтчет(ТекстовыйДокумент, ВыходныеПараметры)
	...
	Если Строка = "#" Тогда
		...
		Пока Истина Цикл
			...
			Попытка
				...
				ПолеОшибки = Нстр("ka='ტრანზაქციის თარიღი (2,3)';ru='Дата транзакции (2,3)'");
				...
				ПолеОшибки = Нстр("ka='სსა ნომერი (5)';ru='Номер ККМ (5)'");
				НомерККМ        = Число(НомерККМСтр);
				ПолеОшибки = Нстр("ka='ჩეკის ნომერი (6)';ru='Номер чека (6)'");
				НомерЧека       = Число(НомерЧекаСтр);
			Исключение
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ka='ფაილის არასწორი ფორმატი. ველის ამოცნობა შეუძლებელია.';ru='Неверный формат файла. Невозможно распознать поле:'") + Символы.НПП + ПолеОшибки);
				Результат = Ложь;
				Прервать;
			КонецПопытки;
			...
			Если ТипТранзакции = 1 ИЛИ ТипТранзакции = 11 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ka='საქონლის კოდი (8)';ru='Код товара (8)'");
					...
					ПолеОшибки = Нстр("ka='სექცია (9)';ru='Секция (9)'");
					Секция     = Число(СекцияСтр);
					ПолеОшибки = Нстр("ka='საქონლის ფასი (10)';ru='Цена товара (10)'");
					Цена       = Число(ЦенаСтр);
					ПолеОшибки = Нстр("ka='საქონლის რაოდენობა (11)';ru='Количество товара (11)'");
					Количество = Число(КоличествоСтр);
					ПолеОшибки = Нстр("ka='თანხა (12)';ru='Сумма (12)'");
			...
			ИначеЕсли ТипТранзакции = 2 Тогда
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ka='შეცდომა სტორნო';ru='Ошибка сторно'"));
				Результат = Ложь;
				Прервать;
			...
			ИначеЕсли ТипТранзакции = 12 Тогда
				...
				Исключение
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ka='ფაილის არასწორი ფორმატი. ველის გარდაქმნა რიცხვზე შეუძლებელია:';ru='Неверный формат файла. Невозможно преобразовать к числу поле:'") + Символы.НПП + ПолеОшибки);
					Результат = Ложь;
					Прервать;
				КонецПопытки;
			...
			ИначеЕсли ТипТранзакции = 3 Или ТипТранзакции = 4 Тогда
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ka='დაბრუნების შეცდომა';ru='Ошибка возврата'"));
				Результат = Ложь;
				Прервать;
			...
			ИначеЕсли ТипТранзакции = 15 Или ТипТранзакции = 17 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ka='სექცია (9)';ru='Секция (9)'");
					Секция     = Число(СекцияСтр);
					ПолеОшибки = Нстр("ka='ფასდაკლება (11)';ru='Скидка (11)'");
				...
				Иначе
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ka='პოზიციაზე ჯამური ფასდაკლების შეცდომა';ru='Ошибка итоговой скидки на позицию'"));
					Результат = Ложь;
					Прервать;
				КонецЕсли;
			...
			ИначеЕсли ТипТранзакции = 16 Или ТипТранзакции = 18 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ka='სექცია (9)';ru='Секция (9)'");
					Секция     = Число(СекцияСтр);
					ПолеОшибки = Нстр("ka='დამატება(11)';ru='Надбавка (11)'");
				...
				Иначе
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ka='პოზიციაზე ჯამური წანამატის შეცდომა';ru='Ошибка итоговой надбавки на позицию'"));
					Результат = Ложь;
					Прервать;
				КонецЕсли;
			...
			ИначеЕсли ТипТранзакции = 80 Тогда
				...
				Попытка
					ПолеОшибки = Нстр("ka='პოზიციის ტრანზაქცია (11)';ru='Транзакция позиции (11)'");
					ТранзакцияПозиции      = Число(ТранзакцияПозицииСтр);
					ПолеОшибки = Нстр("ka='დაბრუნებადი ჩეკის ნომერი (12)';ru='Номер возвращаемого чека (12)'");
				...
				Иначе
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ka='ჩეკის ნომრის მიხედვით დაბრუნების შეცდომა';ru='Ошибка возврата по номеру чека'"));
					Результат = Ложь;
					Прервать;
				КонецЕсли;
			...
			ИначеЕсли ТипТранзакции = 35 Тогда
				...
				Попытка
					ПолеОшибки  = Нстр("ka='ფასდაკლების თანხა (11)';ru='Сумма скидки (11)'");
			...
			ИначеЕсли ТипТранзакции = 36 Тогда
				...
				Попытка
					ПолеОшибки    = Нстр("ka='დანამატის თანხა (11)';ru='Сумма надбавки (11)'");
			...
			ИначеЕсли ТипТранзакции = 37 Тогда
				...
				Попытка
					ПолеОшибки = Нстр("ka='ფასდაკლების პროცენტი (11)';ru='Процент скидки (11)'");
			...
			ИначеЕсли ТипТранзакции = 38 Тогда
				...
				Попытка
					ПолеОшибки = Нстр("ka='დანამატის პროცენტი (11)';ru='Процент надбавки (11)'");
	...
	ИначеЕсли Строка = "@" Тогда
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='ოპერაცია შეწყვეტილია! ანგარიშგება უკვე ჩაიტვირთა!';ru='Операция прервана. Отчет уже был загружен!'"));
		Результат = Ложь;
	Иначе
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='მონაცემთა არასწორი ფორმატი ან მონაცემები არ არის.';ru='Неверный формат данных или данные отсутствуют.'"));
		Результат = Ложь;
	КонецЕсли;
...
Функция СформироватьФайлНастроек(СтруктураНастроек, ТекстовыйФайл, ТекстОшибки)
	...
	Если Налоги.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ka='გამართვების გადმოტვირთვა შეუძლებელია. გადასახადის განაკვეთების სია ცარიელია!';ru='Выгрузка настроек невозможна. Список ставок налога пуст!'");
		Результат = Ложь;
	Иначе
		...
		Если КомбинацииНалогов.Количество() = 0 Тогда
			...
			Если Налоги.Количество() > 1 Тогда
				ТекстОшибки = НСтр("ka='გამართვების გადმოტვირთვა შეუძლებელია. საქონელზე რამდენიმე გადასახადი ირიცხება, აუცილებელია გადასახადების კომბინაციების შევსება!';ru='Выгрузка настроек невозможна. На товары начисляется несколько налогов, необходимо заполнить комбинации налогов!'");
				Результат = Ложь;
			Иначе
		...
		Иначе
			...
			Если Результат Тогда
				...
				Для Каждого ТекКомбинация Из КомбинацииНалогов Цикл
					...
					Текст = НСтр("ka='Группа №';ru='Группа №'") + Код;
