...
Функция ПодготовитьСообщениеПолученияКвитанцииКПередаче(КонвертSOAP, Реквизиты) Экспорт
	...
	Исключение
		...
		ЗаголовокТекстаОшибки = СтрШаблон(
			НСтр("ka='Ошибка при подготовке к передаче сообщения по документу %1';ru='Ошибка при подготовке к передаче сообщения по документу %1'"),
			Реквизиты.Документ);
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ГИСМ';ru='ГИСМ'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОписаниеОперации(Описание, ДокументСсылка, НомерВерсии = Неопределено) Экспорт
	...
	ИначеЕсли НомерВерсии = Неопределено И ДокументСсылка <> Неопределено Тогда
		Возврат СтрШаблон(НСтр("ka='%1 по документу ""%2""';ru='%1 по документу ""%2""'"), Описание, ДокументСсылка);
	ИначеЕсли НомерВерсии <> Неопределено И ДокументСсылка = Неопределено Тогда
		Возврат СтрШаблон(НСтр("ka='%1. ვერსია %2';ru='%1. Версия %2'"), Описание, НомерВерсии);
	Иначе
		Возврат СтрШаблон(НСтр("ka='%1 по документу ""%2"". Версия %3';ru='%1 по документу ""%2"". Версия %3'"), Описание, ДокументСсылка, НомерВерсии);
	КонецЕсли;
...
Функция КатегорииОпераций() Экспорт
	...
	ПолучениеСпискаВходящихДокументов.Вставить(
		...
		НСтр("ka='Получение списка входящих документов';ru='Получение списка входящих документов'"));
	
	ПередачаДанных = Новый Соответствие;
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанных,
		НСтр("ka='მონაცემების გადაცემა';ru='Передача данных'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхОтгрузкаМаркированныхТоваров,
		НСтр("ka='Передача сведений об отгрузке маркированных товаров';ru='Передача сведений об отгрузке маркированных товаров'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхИмпортМаркированнойПродукции,
		НСтр("ka='Передача сведений об импорте маркированной продукции на территорию РФ';ru='Передача сведений об импорте маркированной продукции на территорию РФ'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаАннулирования,
		НСтр("ka='ანულირების გადაცემა';ru='Передача аннулирования'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки,
		НСтр("ka='Передача запрос на закрытие заявки';ru='Передача запрос на закрытие заявки'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаПодтверждения, 
		НСтр("ka='Передача подтверждения поступления КиЗ';ru='Передача подтверждения поступления КиЗ'"));
	
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхЗаявкаНаВыпускКиЗ,
		НСтр("ka='Передача заявки на выпуск КиЗ';ru='Передача заявки на выпуск КиЗ'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВозвратОтРозничногоПокупателя,
		НСтр("ka='Передача сведений о возврате товара от розничного покупателя';ru='Передача сведений о возврате товара от розничного покупателя'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхРозничнаяПродажа,
		НСтр("ka='Передача сведений о КиЗ, реализованных в розницу';ru='Передача сведений о КиЗ, реализованных в розницу'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВыбраковкаКиЗ,
		НСтр("ka='Передача сведений о выбраковке неиндивидуализированных КиЗ';ru='Передача сведений о выбраковке неиндивидуализированных КиЗ'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхСписаниеКиЗ,
		НСтр("ka='Передача сведений о списанных КиЗ';ru='Передача сведений о списанных КиЗ'"));
		
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхСписаниеТоваров,
		НСтр("ka='Передача сведений о списании маркированной продукции';ru='Передача сведений о списании маркированной продукции'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхМаркировкаТоваров,
		НСтр("ka='Передача сведений о маркированной продукции';ru='Передача сведений о маркированной продукции'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхПеремаркировкаТоваров,
		НСтр("ka='Передача сведений о перемаркированной продукции';ru='Передача сведений о перемаркированной продукции'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВозвратФизическомуЛицуКомиссия,
		НСтр("ka='Передача сведений о возврате маркированной продукции физическому лицу (комиссия)';ru='Передача сведений о возврате маркированной продукции физическому лицу (комиссия)'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхОтгрузкаЗаПределыЕАЭС,
		НСтр("ka='Передача сведений об экспорте маркированной продукции за пределы ЕАЭС';ru='Передача сведений об экспорте маркированной продукции за пределы ЕАЭС'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхОтгрузкаЭкспортЕАЭС,
		НСтр("ka='Передача сведений об экспорте маркированной продукции в страны ЕАЭС';ru='Передача сведений об экспорте маркированной продукции в страны ЕАЭС'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВвозМаркированнойПродукцииИзЕАЭС,
		НСтр("ka='Передача сведений о ввозе маркированной продукции из стран ЕАЭС на территорию РФ';ru='Передача сведений о ввозе маркированной продукции из стран ЕАЭС на территорию РФ'"));
	
	ТекстПолучениеКвитанции = НСтр("ka='კვიტანციის მიღება';ru='Получение квитанции'");
...
Функция ПоддерживаемыеТипыДокументов() Экспорт
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Уведомление об изменении статуса заявки на выпуск КиЗ';ru='Уведомление об изменении статуса заявки на выпуск КиЗ'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Уведомление о выпущенных КиЗ';ru='Уведомление о выпущенных КиЗ'"));
	ПоддерживаемыеТипыДокументов.Вставить(12, ОписаниеТипаДокумента);
	
	ОписаниеТипаДокумента = Новый Структура;
	ОписаниеТипаДокумента.Вставить("Операция", Перечисления.ОперацииОбменаГИСМ.ПолучениеТоваров);
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='შეტყობინება მარკირებული საქონლების შემოსვლის შესახებ';ru='Уведомление о поступлении маркированных товаров'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Аннулирование уведомления об отгрузке маркированных товаров';ru='Аннулирование уведомления об отгрузке маркированных товаров'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Подтверждение получения маркированных товаров';ru='Подтверждение получения маркированных товаров'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Закрытие уведомления об отгрузке маркированных товаров';ru='Закрытие уведомления об отгрузке маркированных товаров'"));
	ПоддерживаемыеТипыДокументов.Вставить(33, ОписаниеТипаДокумента);
	
	ОписаниеТипаДокумента = Новый Структура;
	ОписаниеТипаДокумента.Вставить("Операция", Перечисления.ОперацииОбменаГИСМ.ПолучениеТоваров);
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Уведомление о поступлении маркированных товаров незарегистрированному покупателю';ru='Уведомление о поступлении маркированных товаров незарегистрированному покупателю'"));
	...
	Если ВерсииСхемОбмена.Клиент <> НоваяВерсияСхемОбмена
		...
		Если СоответствиеВерсийСхемОбмена().Получить(НоваяВерсияСхемОбмена) <> Неопределено Тогда
			...
			ВозвращаемоеЗначение.ТекстСообщения = СтрШаблон(
				НСтр("ka='Версия Формата обмена с ГИСМ обновлена до %2.';ru='Версия Формата обмена с ГИСМ обновлена до %2.'"),
...
Функция ОбъектXDTOПоИмениСвойства(ОбъектXDTOРодитель, ИмяСвойства, Версия = Неопределено) Экспорт
	...
	Если СвойствоXDTO = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Свойство %1 отсутствует в составе типов информационной системы маркировки.';ru='Свойство %1 отсутствует в составе типов информационной системы маркировки.'"), ИмяСвойства);
	Иначе
...
Процедура ЗаполнитьСписокВыбораДальнейшееДействие(СписокВыбора, ВсеТребующиеДействия, ВсеТребующиеОжидания) Экспорт
	...
	СписокВыбора.Добавить("ВсеТребующиеДействия", НСтр("ka='ყველა მოთხოვნილი მოქმედება';ru='Все требующие действия'"));
	СписокВыбора.Добавить("ВсеТребующиеОжидания", НСтр("ka='Все требующие ожидания';ru='Все требующие ожидания'"));
	СписокВыбора.Добавить("ВсеТребующиеДействияИлиОжидания", НСтр("ka='Все требующие действия или ожидания';ru='Все требующие действия или ожидания'"));
...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ИмяСписка = "Список", ИмяПоляДата = "Дата") Экспорт
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты сегодня: ""09:46""';ru='Представление даты сегодня: ""09:46""'");
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты до начала этого дня: ""10.06.2012""';ru='Представление даты до начала этого дня: ""10.06.2012""'");
	ЭлементУсловногоОформления.Представление = ПредставлениеЭлемента;
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты после начала этого дня: ""10.06.2012""';ru='Представление даты после начала этого дня: ""10.06.2012""'");
	ЭлементУсловногоОформления.Представление = ПредставлениеЭлемента;
...
Процедура ОтразитьНеЗаказаннуюНоменклатуруВСтроке(Выборка, Объект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			ТекстОписанияПроблемы = НСтр("ka='""%1"" отстуствует в списке заказанных КиЗ.';ru='""%1"" отстуствует в списке заказанных КиЗ.'");
...
Процедура ОтразитьНеНайденнуюНоменклатуруВСтроке(Выборка, Объект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НайденнаяСтрока.ЕстьПроблема = Истина;
			НайденнаяСтрока.ОписаниеПроблемы = НСтр("ka='Для данной строки в информационной базе не найдена позиция номенклатуры.';ru='Для данной строки в информационной базе не найдена позиция номенклатуры.'");
		КонецЦикла;
...
Функция ПолноеИмяСсылочногоТипаПоДаннымОпределяемогоТипа(ИмяОпределяемогоТипа, ДолженБытьУказанТолькоОдинТип = Истина) Экспорт
	...
	Если ДолженБытьУказанТолькоОдинТип И ТипыЗаданныеВОпределяемомТипе.Количество() > 1 Тогда
		ВызватьИсключение(СтрШаблон(НСтр("ka='В определяемом типе ""%1"" должен быть указан только один тип.';ru='В определяемом типе ""%1"" должен быть указан только один тип.'"), МетаданныеОпределяемогоТипа.Имя));
	КонецЕсли;
	...
	Если МетаданныеНоменклатура = Неопределено Тогда
		ВызватьИсключение(СтрШаблон(НСтр("ka='В определяемом типе ""%1"" должен быть указан ссылочный тип.';ru='В определяемом типе ""%1"" должен быть указан ссылочный тип.'"), МетаданныеОпределяемогоТипа.Имя));
	Иначе
...
Процедура УстановитьУсловноеОформлениеСтатусДальнейшееДействиеГИСМ(УсловноеОформление,
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<не требуется>';ru='<не требуется>'"));
...
Процедура УстановитьУсловноеОформлениеНомерГИСМ(УсловноеОформление,
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<არ არის მიღებული>';ru='<не получен>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаНеТребуетВниманияГИСМ);
...
Функция ОбработатьЗагрузкуДокумента(ТекстВходящегоСообщенияXML, ТекстИсходящегоСообщенияXML, ДанныеДляПолученияДокумента, ВозвращаемоеЗначение = Неопределено) Экспорт
	...
	Иначе
		...
		Попытка
			...
			Иначе
				ВызватьИсключение СтрШаблон(НСтр("ka='Неизвестный тип загружаемого документа action_id %1';ru='Неизвестный тип загружаемого документа action_id %1'"), action_id);
			КонецЕсли;
		...
		Исключение
			...
			Иначе
				НаименованиеДокумента = НСтр("ka='არამხარდეჭირილი დოკუმენტი';ru='Неподдерживаемый документ'");
			КонецЕсли;
...
"%2'"),
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='Статус обработки %1 не поддерживается в данной версии';ru='Статус обработки %1 не поддерживается в данной версии'"), ДанныеДокумента.order_status);
	КонецЕсли;
	
	Если ДанныеДокумента.reason = 1 Тогда
		Описание = НСтр("ka='არ არის ИНН/КПП';ru='Отсутствуют ИНН/КПП'");
	ИначеЕсли ДанныеДокумента.reason = 2 Тогда
		Описание = НСтр("ka='По указанным ИНН/КПП не найден контрагент';ru='По указанным ИНН/КПП не найден контрагент'");
	ИначеЕсли ДанныеДокумента.reason = 3 Тогда
		Описание = НСтр("ka='Отсутствуют реквизиты договора';ru='Отсутствуют реквизиты договора'");
	ИначеЕсли ДанныеДокумента.reason = 4 Тогда
		Описание = НСтр("ka='Истекли 5 рабочих дней для исправления некорректных реквизитов заявления';ru='Истекли 5 рабочих дней для исправления некорректных реквизитов заявления'");
	ИначеЕсли ДанныеДокумента.reason = 5 Тогда
		Описание = НСтр("ka='Не заполнен реквизит заявления (с указанием конкретного реквизита)';ru='Не заполнен реквизит заявления (с указанием конкретного реквизита)'");
	ИначеЕсли ДанныеДокумента.reason = 6 Тогда
		Описание = НСтр("ka='Значение реквизита заявления не найдено в справочнике (с указанием конкретного реквизита)';ru='Значение реквизита заявления не найдено в справочнике (с указанием конкретного реквизита)'");
	ИначеЕсли ДанныеДокумента.reason = 7 Тогда
		Описание = НСтр("ka='Не найден договор с указанными реквизитами';ru='Не найден договор с указанными реквизитами'");
	ИначеЕсли ДанныеДокумента.reason = 8 Тогда
		Описание = НСтр("ka='დაბრუნებულია ტვირთგადამზიდის მიერ';ru='Возвращено грузоперевозчиком'");
	ИначеЕсли ДанныеДокумента.reason = 9 Тогда
		Описание = НСтр("ka='დაბრუნებულია შემკვეთის მიერ';ru='Возвращено заказчиком'");
	ИначеЕсли ДанныеДокумента.reason = Неопределено Тогда
		Описание = "";
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='Причина изменения статуса %1 не поддерживается в данной версии';ru='Причина изменения статуса %1 не поддерживается в данной версии'"), ДанныеДокумента.reason);
	КонецЕсли;
...
"%2'"),
	...
	Если ДанныеДокумента.emits.by_sign_type.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='Данные о выпущенных КиЗ не загружены';ru='Данные о выпущенных КиЗ не загружены'");
	КонецЕсли;
...
Функция ЗагрузитьУведомлениеОПоступленииМаркированныхТоваров(ДанныеДокумента, ДанныеОтправителя, НомерДокумента, Сообщения)
	...
	Если ВерсияНеМенееТребуемой(Сообщения.ВерсияВходящегоСообщенияXML, "2.41") Тогда
		...
		Если ДанныеДокумента.order_details.sign.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ka='Данные о поступивших КиЗ не загружены';ru='Данные о поступивших КиЗ не загружены'");
		КонецЕсли;
...
Функция ЗагрузитьПодтверждениеПолученияМаркированныхТоваров(ДанныеДокумента, Сообщения)
	...
	ЗаголовокТекстаОшибки = НСтр("ka='Ошибка при загрузке подтверждения клиентом получения маркированных товаров:';ru='Ошибка при загрузке подтверждения клиентом получения маркированных товаров:'");
...
"%2'"),
	...
	Если ВерсияНеМенееТребуемой(Сообщения.ВерсияВходящегоСообщенияXML, "2.41") Тогда
		...
		Если ДанныеДокумента.details.sign.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ka='Номера КиЗ для подтверждения не загружены';ru='Номера КиЗ для подтверждения не загружены'");
		КонецЕсли;
