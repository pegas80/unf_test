...
Функция ПодготовитьСообщениеПолученияКвитанцииКПередаче(КонвертSOAP, Реквизиты) Экспорт
	...
	Исключение
		...
		ЗаголовокТекстаОшибки = СтрШаблон(
			НСтр("ka='Ошибка при подготовке к передаче сообщения по документу %1';ru='Ошибка при подготовке к передаче сообщения по документу %1';en='Error messages when preparing for the transfer of the document% 1'"),
			Реквизиты.Документ);
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ГИСМ';ru='ГИСМ';en='GISM'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОписаниеОперации(Описание, ДокументСсылка, НомерВерсии = Неопределено) Экспорт
	...
	ИначеЕсли НомерВерсии = Неопределено И ДокументСсылка <> Неопределено Тогда
		Возврат СтрШаблон(НСтр("ka='%1 по документу ""%2""';ru='%1 по документу ""%2""';en=""1% on the document '% 2'"""), Описание, ДокументСсылка);
	ИначеЕсли НомерВерсии <> Неопределено И ДокументСсылка = Неопределено Тогда
		Возврат СтрШаблон(НСтр("ka='%1. ვერსია %2';ru='%1. Версия %2';en='%one. Version% 2'"), Описание, НомерВерсии);
	Иначе
		Возврат СтрШаблон(НСтр("ka='%1 по документу ""%2"". Версия %3';ru='%1 по документу ""%2"". Версия %3';en=""1% on the document '% 2'. Version 3%"""), Описание, ДокументСсылка, НомерВерсии);
	КонецЕсли;
...
Функция КатегорииОпераций() Экспорт
	...
	ПолучениеСпискаВходящихДокументов.Вставить(
		...
		НСтр("ka='Получение списка входящих документов';ru='Получение списка входящих документов';en='Getting a list of incoming documents'"));
	
	ПередачаДанных = Новый Соответствие;
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанных,
		НСтр("ka='მონაცემების გადაცემა';ru='Передача данных';en='Data transfer'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхОтгрузкаМаркированныхТоваров,
		НСтр("ka='Передача сведений об отгрузке маркированных товаров';ru='Передача сведений об отгрузке маркированных товаров';en='Transmission of information on the shipment marked goods'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхИмпортМаркированнойПродукции,
		НСтр("ka='Передача сведений об импорте маркированной продукции на территорию РФ';ru='Передача сведений об импорте маркированной продукции на территорию РФ';en='Transfer of information on imports of marked products to the Russian Federation'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаАннулирования,
		НСтр("ka='ანულირების გადაცემა';ru='Передача аннулирования';en='Transfer cancellation'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки,
		НСтр("ka='Передача запрос на закрытие заявки';ru='Передача запрос на закрытие заявки';en='Transfer request to the application closing'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаПодтверждения, 
		НСтр("ka='Передача подтверждения поступления КиЗ';ru='Передача подтверждения поступления КиЗ';en='The transfer confirmation receipt Keyes'"));
	
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхЗаявкаНаВыпускКиЗ,
		НСтр("ka='Передача заявки на выпуск КиЗ';ru='Передача заявки на выпуск КиЗ';en='Transfer requests for the release of Keys'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВозвратОтРозничногоПокупателя,
		НСтр("ka='Передача сведений о возврате товара от розничного покупателя';ru='Передача сведений о возврате товара от розничного покупателя';en='Transmission of information on the return of the goods from the retail buyer'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхРозничнаяПродажа,
		НСтр("ka='Передача сведений о КиЗ, реализованных в розницу';ru='Передача сведений о КиЗ, реализованных в розницу';en='Transfer of information about Keyes sold at retail'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВыбраковкаКиЗ,
		НСтр("ka='Передача сведений о выбраковке неиндивидуализированных КиЗ';ru='Передача сведений о выбраковке неиндивидуализированных КиЗ';en='Communication of culling unascertained Keyes'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхСписаниеКиЗ,
		НСтр("ka='Передача сведений о списанных КиЗ';ru='Передача сведений о списанных КиЗ';en='Communication of decommissioned Keyes'"));
		
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхСписаниеТоваров,
		НСтр("ka='Передача сведений о списании маркированной продукции';ru='Передача сведений о списании маркированной продукции';en='Transfer of information to cancel the marked products'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхМаркировкаТоваров,
		НСтр("ka='Передача сведений о маркированной продукции';ru='Передача сведений о маркированной продукции';en='Transmission of information on labeled products'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхПеремаркировкаТоваров,
		НСтр("ka='Передача сведений о перемаркированной продукции';ru='Передача сведений о перемаркированной продукции';en='Transfer of information about re-label products'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВозвратФизическомуЛицуКомиссия,
		НСтр("ka='Передача сведений о возврате маркированной продукции физическому лицу (комиссия)';ru='Передача сведений о возврате маркированной продукции физическому лицу (комиссия)';en='Transmission of information on the return of an individual labeled products (Commission)'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхОтгрузкаЗаПределыЕАЭС,
		НСтр("ka='Передача сведений об экспорте маркированной продукции за пределы ЕАЭС';ru='Передача сведений об экспорте маркированной продукции за пределы ЕАЭС';en='Transfer of information about exporting labeled products outside the EAEC'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхОтгрузкаЭкспортЕАЭС,
		НСтр("ka='Передача сведений об экспорте маркированной продукции в страны ЕАЭС';ru='Передача сведений об экспорте маркированной продукции в страны ЕАЭС';en='Transfer of information about exporting labeled products to the countries of the EAEC'"));
	ПередачаДанных.Вставить(
		Перечисления.ОперацииОбменаГИСМ.ПередачаДанныхВвозМаркированнойПродукцииИзЕАЭС,
		НСтр("ka='Передача сведений о ввозе маркированной продукции из стран ЕАЭС на территорию РФ';ru='Передача сведений о ввозе маркированной продукции из стран ЕАЭС на территорию РФ';en='Transmission of information on the import of labeled products from the countries of the EAEC to the territory of the Russian Federation'"));
	
	ТекстПолучениеКвитанции = НСтр("ka='კვიტანციის მიღება';ru='Получение квитанции';en='obtain receipts'");
...
Функция ПоддерживаемыеТипыДокументов() Экспорт
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Уведомление об изменении статуса заявки на выпуск КиЗ';ru='Уведомление об изменении статуса заявки на выпуск КиЗ';en='Notification of change in the status of the application release Keyes'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Уведомление о выпущенных КиЗ';ru='Уведомление о выпущенных КиЗ';en='Notice of the issued Keyes'"));
	ПоддерживаемыеТипыДокументов.Вставить(12, ОписаниеТипаДокумента);
	
	ОписаниеТипаДокумента = Новый Структура;
	ОписаниеТипаДокумента.Вставить("Операция", Перечисления.ОперацииОбменаГИСМ.ПолучениеТоваров);
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='შეტყობინება მარკირებული საქონლების შემოსვლის შესახებ';ru='Уведомление о поступлении маркированных товаров';en='Notification to the marked goods'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Аннулирование уведомления об отгрузке маркированных товаров';ru='Аннулирование уведомления об отгрузке маркированных товаров';en='Cancellation notice of the shipment marked goods'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Подтверждение получения маркированных товаров';ru='Подтверждение получения маркированных товаров';en='Acknowledgment of receipt of the marked goods'"));
	...
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Закрытие уведомления об отгрузке маркированных товаров';ru='Закрытие уведомления об отгрузке маркированных товаров';en='Closure notice of the shipment marked goods'"));
	ПоддерживаемыеТипыДокументов.Вставить(33, ОписаниеТипаДокумента);
	
	ОписаниеТипаДокумента = Новый Структура;
	ОписаниеТипаДокумента.Вставить("Операция", Перечисления.ОперацииОбменаГИСМ.ПолучениеТоваров);
	ОписаниеТипаДокумента.Вставить("Описание", НСтр("ka='Уведомление о поступлении маркированных товаров незарегистрированному покупателю';ru='Уведомление о поступлении маркированных товаров незарегистрированному покупателю';en='Notification to the marked goods unregistered customers'"));
...
"До выполнения обновления загрузка данных из ГИСМ будет выполнятся не в полном объеме.';ka='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)."
"Необходимо выполнить обновление конфигурации."
	...
	Если ВерсииСхемОбмена.Клиент <> НоваяВерсияСхемОбмена
		...
		Если СоответствиеВерсийСхемОбмена().Получить(НоваяВерсияСхемОбмена) <> Неопределено Тогда
			...
			ВозвращаемоеЗначение.ТекстСообщения = СтрШаблон(
				НСтр("ka='Версия Формата обмена с ГИСМ обновлена до %2.';ru='Версия Формата обмена с ГИСМ обновлена до %2.';en='Format version with GISM exchange upgraded to 2%.'"),
...
Функция ОбъектXDTOПоИмениСвойства(ОбъектXDTOРодитель, ИмяСвойства, Версия = Неопределено) Экспорт
	...
	Если СвойствоXDTO = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Свойство %1 отсутствует в составе типов информационной системы маркировки.';ru='Свойство %1 отсутствует в составе типов информационной системы маркировки.';en='Property% 1 is not a part of types of information labeling system.'"), ИмяСвойства);
	Иначе
...
Процедура ЗаполнитьСписокВыбораДальнейшееДействие(СписокВыбора, ВсеТребующиеДействия, ВсеТребующиеОжидания) Экспорт
	...
	СписокВыбора.Добавить("ВсеТребующиеДействия", НСтр("ka='ყველა მოთხოვნილი მოქმედება';ru='Все требующие действия';en='All require action'"));
	СписокВыбора.Добавить("ВсеТребующиеОжидания", НСтр("ka='Все требующие ожидания';ru='Все требующие ожидания';en='All require waiting'"));
	СписокВыбора.Добавить("ВсеТребующиеДействияИлиОжидания", НСтр("ka='Все требующие действия или ожидания';ru='Все требующие действия или ожидания';en='All require action or expectations'"));
...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ИмяСписка = "Список", ИмяПоляДата = "Дата") Экспорт
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты сегодня: ""09:46""';ru='Представление даты сегодня: ""09:46""';en='Submission date today: ""9:46""'");
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты до начала этого дня: ""10.06.2012""';ru='Представление даты до начала этого дня: ""10.06.2012""';en='Submission date to the beginning of this day: ""10.06.2012""'");
	ЭлементУсловногоОформления.Представление = ПредставлениеЭлемента;
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты после начала этого дня: ""10.06.2012""';ru='Представление даты после начала этого дня: ""10.06.2012""';en='Submission date after the start of the day: ""10.06.2012""'");
	ЭлементУсловногоОформления.Представление = ПредставлениеЭлемента;
...
Процедура ОтразитьНеЗаказаннуюНоменклатуруВСтроке(Выборка, Объект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			ТекстОписанияПроблемы = НСтр("ka='""%1"" отстуствует в списке заказанных КиЗ.';ru='""%1"" отстуствует в списке заказанных КиЗ.';en='""%1"" in the absence of the user list, ordered Keyes.'");
...
Процедура ОтразитьНеНайденнуюНоменклатуруВСтроке(Выборка, Объект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			НайденнаяСтрока.ЕстьПроблема = Истина;
			НайденнаяСтрока.ОписаниеПроблемы = НСтр("ka='Для данной строки в информационной базе не найдена позиция номенклатуры.';ru='Для данной строки в информационной базе не найдена позиция номенклатуры.';en='For a given line in the information database is found position nomenclature.'");
		КонецЦикла;
...
Функция ПолноеИмяСсылочногоТипаПоДаннымОпределяемогоТипа(ИмяОпределяемогоТипа, ДолженБытьУказанТолькоОдинТип = Истина) Экспорт
	...
	Если ДолженБытьУказанТолькоОдинТип И ТипыЗаданныеВОпределяемомТипе.Количество() > 1 Тогда
		ВызватьИсключение(СтрШаблон(НСтр("ka='В определяемом типе ""%1"" должен быть указан только один тип.';ru='В определяемом типе ""%1"" должен быть указан только один тип.';en='The defined type ""%1"" should be only one type is specified.'"), МетаданныеОпределяемогоТипа.Имя));
	КонецЕсли;
	...
	Если МетаданныеНоменклатура = Неопределено Тогда
		ВызватьИсключение(СтрШаблон(НСтр("ka='В определяемом типе ""%1"" должен быть указан ссылочный тип.';ru='В определяемом типе ""%1"" должен быть указан ссылочный тип.';en=""The defined type '% 1' must be a reference type."""), МетаданныеОпределяемогоТипа.Имя));
	Иначе
...
Процедура УстановитьУсловноеОформлениеСтатусДальнейшееДействиеГИСМ(УсловноеОформление,
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<не требуется>';ru='<не требуется>';en='<не требуется>'"));
...
Процедура УстановитьУсловноеОформлениеНомерГИСМ(УсловноеОформление,
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<არ არის მიღებული>';ru='<не получен>';en='<Not received>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаНеТребуетВниманияГИСМ);
...
Функция ОбработатьЗагрузкуДокумента(ТекстВходящегоСообщенияXML, ТекстИсходящегоСообщенияXML, ДанныеДляПолученияДокумента, ВозвращаемоеЗначение = Неопределено) Экспорт
	...
	Иначе
		...
		Попытка
			...
			Иначе
				ВызватьИсключение СтрШаблон(НСтр("ka='Неизвестный тип загружаемого документа action_id %1';ru='Неизвестный тип загружаемого документа action_id %1';en='Unknown type of document that is loaded action_id% 1'"), action_id);
			КонецЕсли;
		...
		Исключение
			...
			Иначе
				НаименованиеДокумента = НСтр("ka='არამხარდეჭირილი დოკუმენტი';ru='Неподдерживаемый документ';en='unsupported document'");
			КонецЕсли;
...
"%4';ka='При загрузке документа %5 (action_id=%1) с output_id=%2 возникла ошибка:"
"Текст ошибки: %3"
"Данные документа доступны для просмотра и повторной загрузке в списке ""Очередь получения данных из ГИСМ"""
"Текст сообщения SOAP:"
"%4'"),
...
"%2';ka='При загрузке уведомления об изменении состояния обработки эмитентом заявки на выпуск КиЗ возникла ошибка:"
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2'"),
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='Статус обработки %1 не поддерживается в данной версии';ru='Статус обработки %1 не поддерживается в данной версии';en='Processing Status 1% is not supported in this version'"), ДанныеДокумента.order_status);
	КонецЕсли;
	
	Если ДанныеДокумента.reason = 1 Тогда
		Описание = НСтр("ka='არ არის ИНН/КПП';ru='Отсутствуют ИНН/КПП';en='No INN / KPP'");
	ИначеЕсли ДанныеДокумента.reason = 2 Тогда
		Описание = НСтр("ka='По указанным ИНН/КПП не найден контрагент';ru='По указанным ИНН/КПП не найден контрагент';en='For these INN / KPP not found counterparty'");
	ИначеЕсли ДанныеДокумента.reason = 3 Тогда
		Описание = НСтр("ka='Отсутствуют реквизиты договора';ru='Отсутствуют реквизиты договора';en='There are no details of the contract'");
	ИначеЕсли ДанныеДокумента.reason = 4 Тогда
		Описание = НСтр("ka='Истекли 5 рабочих дней для исправления некорректных реквизитов заявления';ru='Истекли 5 рабочих дней для исправления некорректных реквизитов заявления';en='Expired on 5 working days to correct the incorrect application details'");
	ИначеЕсли ДанныеДокумента.reason = 5 Тогда
		Описание = НСтр("ka='Не заполнен реквизит заявления (с указанием конкретного реквизита)';ru='Не заполнен реквизит заявления (с указанием конкретного реквизита)';en='Not filled with props statements (with specification of props)'");
	ИначеЕсли ДанныеДокумента.reason = 6 Тогда
		Описание = НСтр("ka='Значение реквизита заявления не найдено в справочнике (с указанием конкретного реквизита)';ru='Значение реквизита заявления не найдено в справочнике (с указанием конкретного реквизита)';en='Meaning props statements found in the directory (with specification of props)'");
	ИначеЕсли ДанныеДокумента.reason = 7 Тогда
		Описание = НСтр("ka='Не найден договор с указанными реквизитами';ru='Не найден договор с указанными реквизитами';en='Unable to find an agreement with the said details'");
	ИначеЕсли ДанныеДокумента.reason = 8 Тогда
		Описание = НСтр("ka='დაბრუნებულია ტვირთგადამზიდის მიერ';ru='Возвращено грузоперевозчиком';en='returned freight carrier'");
	ИначеЕсли ДанныеДокумента.reason = 9 Тогда
		Описание = НСтр("ka='დაბრუნებულია შემკვეთის მიერ';ru='Возвращено заказчиком';en='returned to the customer'");
	ИначеЕсли ДанныеДокумента.reason = Неопределено Тогда
		Описание = "";
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='Причина изменения статуса %1 не поддерживается в данной версии';ru='Причина изменения статуса %1 не поддерживается в данной версии';en='The reason for changing the status of 1% is not supported in this version'"), ДанныеДокумента.reason);
	КонецЕсли;
...
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2';ka='При загрузке уведомления о выпуске КиЗ эмитентом возникла ошибка:"
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2'"),
			ДанныеДокумента.emit_order_id,
	...
	Если ДанныеДокумента.emits.by_sign_type.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='Данные о выпущенных КиЗ не загружены';ru='Данные о выпущенных КиЗ не загружены';en='Data on issued Keyes is not loaded'");
	КонецЕсли;
...
Функция ЗагрузитьУведомлениеОПоступленииМаркированныхТоваров(ДанныеДокумента, ДанныеОтправителя, НомерДокумента, Сообщения)
	...
	Если ВерсияНеМенееТребуемой(Сообщения.ВерсияВходящегоСообщенияXML, "2.41") Тогда
		...
		Если ДанныеДокумента.order_details.sign.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ka='Данные о поступивших КиЗ не загружены';ru='Данные о поступивших КиЗ не загружены';en='Data about incoming Keys are not loaded'");
		КонецЕсли;
...
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2';ka='При загрузке аннулирования клиентом уведомления об отгрузке возникла ошибка:"
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2'"),
...
Функция ЗагрузитьПодтверждениеПолученияМаркированныхТоваров(ДанныеДокумента, Сообщения)
	...
	ЗаголовокТекстаОшибки = НСтр("ka='Ошибка при загрузке подтверждения клиентом получения маркированных товаров:';ru='Ошибка при загрузке подтверждения клиентом получения маркированных товаров:';en='Error loading client acknowledgment receipt marked goods:'");
...
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2';ka='При загрузке подтверждения клиентом получения маркированных товаров возникла ошибка:"
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2'"),
			ДанныеДокумента.move_order_id,
	...
	Если ВерсияНеМенееТребуемой(Сообщения.ВерсияВходящегоСообщенияXML, "2.41") Тогда
		...
		Если ДанныеДокумента.details.sign.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ka='Номера КиЗ для подтверждения не загружены';ru='Номера КиЗ для подтверждения не загружены';en='Keyes Non-confirmation is not loaded'");
		КонецЕсли;
...
"Текст входящего сообщения SOAP:"
"%4';ka='При загрузке подтверждения клиентом получения маркированных товаров по заявке %1 возникла ошибка:"
"В документе %2 в табличной части ""Товары"" не найден товар с номером КиЗ %3."
"Текст входящего сообщения SOAP:"
"%4'"),
					ДанныеДокумента.move_order_id,
...
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2';ka='При загрузке закрытия клиентом уведомления об отгрузке возникла ошибка:"
"Не найдена заявка с id %1."
"Текст входящего сообщения SOAP:"
"%2'"),
			ДанныеДокумента.move_order_id,
...
"Текст входящего сообщения SOAP:"
"%3';ka='При загрузке номера уведомления об отгрузке незарегистрированному покупателю возникла ошибка:"
"Не найден документ-основание с номером документа %1 для контрагента с ИНН %2"
"Текст входящего сообщения SOAP:"
"%3'"),
...
"Текст ошибки: %1';ka='При выполнении обмена с ГИСМ возникла ошибка:"
