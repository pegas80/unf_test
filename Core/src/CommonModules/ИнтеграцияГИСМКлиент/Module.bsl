...
Процедура ПодтвердитьПолучениеДляДокументовИзСписка(ДинамическийСписок) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ka='В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.';ru='В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.'"));
...
Процедура ПолучитьGLNПоИННКПП(Форма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнено поле ""Контрагент""';ru='Не заполнено поле ""Контрагент""'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Данные.ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнен ИНН контрагента';ru='Не заполнен ИНН контрагента'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	...
	Если Данные = Неопределено Тогда
		...
		Если Результат.ИнформацияОВерсиях <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Результат.ИнформацияОВерсиях.ТекстСообщения) Тогда
				...
				Иначе
					ПоказатьОповещениеПользователя(Результат.ИнформацияОВерсиях.ТекстСообщения,,НСтр("ka='Версия обмена обновлена';ru='Версия обмена обновлена'"), БиблиотекаКартинок.Информация32);
				КонецЕсли;
			КонецЕсли;
...
Процедура Подписать(Сообщения, Организация, ОповещениеПриЗавершении) Экспорт
	...
	Если КоличествоСообщений = 1 Тогда
		ЗаголовокДанных = НСтр("ka='შეტყობინება';ru='Сообщение'");
	Иначе
		ЗаголовокДанных = НСтр("ka='შეტყობინებები';ru='Сообщения'");
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='გაცვლა მარკირების სისტემასთან';ru='Обмен с ГИСМ'"));
...
"электронной подписи, связанных с организацией %2, необходимых для обмена с ГИСМ.'"),
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", СтрШаблон(НСтр("ka='ჩვენება (%1)';ru='Показать (%1)'"), КоличествоСообщений));
...
Функция СообщенияПоОрганизациям(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
			ЗаголовокСообщения = СтрШаблон(НСтр("ka='Сообщение ""%1"" не подготовлено к отправке в ГИСМ:';ru='Сообщение ""%1"" не подготовлено к отправке в ГИСМ:'"), Сообщение.Описание) + Символы.ПС;
...
Процедура ПодготовитьСообщенияКПередачеДляДокументовИзСписка(ВходящиеДанные, Операция, ДинамическийСписок = Неопределено) Экспорт
	...
	Если ВходящиеДанные.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='ბრძანება არ შეიძლება შესრულდეს მითითებული ობიექტისთვის.';ru='Команда не может быть выполнена для указанного объекта.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьСообщениеКПередаче(ДокументСсылка, Операция) Экспорт
	...
	Если Сообщения = Неопределено И Операция <> Неопределено Тогда
		ПоказатьПредупреждение(,СтрШаблон(НСтр("ka='По операции ""%1"" передача данных в ГИСМ не требуется';ru='По операции ""%1"" передача данных в ГИСМ не требуется'"), Операция));
	КонецЕсли;
	Если Сообщения = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьСообщениеКПередаче_ПриЗавершенииОперацииПодписи(Сообщения, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Если Не Выполнено Тогда
			...
			ТекстЗаголовка = НСтр("ka='При подготовке сообщения к передаче в ГИСМ возникла ошибка:';ru='При подготовке сообщения к передаче в ГИСМ возникла ошибка:'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстЗаголовка + Символы.ПС + ЭлементДанных.ТекстОшибки);
			
		Иначе
			
			ТекстЗаголовка = НСтр("ka='Сообщение подготовлено к передаче в ГИСМ';ru='Сообщение подготовлено к передаче в ГИСМ'");
			Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаДанных") Тогда
				ТекстСообщения = НСтр("ka='Для документа %Документ% подготовлено к отправке сообщение передачи данных';ru='Для документа %Документ% подготовлено к отправке сообщение передачи данных'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
				ТекстСообщения = НСтр("ka='Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки';ru='Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
				ТекстСообщения = НСтр("ka='Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров';ru='Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров'");
			КонецЕсли;
...
Процедура ОповеститьПользователяОВыполненнойОперации(СписокДокументов, КоличествоОбработанных, КоличествоВсего, Операция) Экспорт
	...
	ТекстЗаголовка = НСтр("ka='Подготовка сообщения к передаче в ГИСМ';ru='Подготовка сообщения к передаче в ГИСМ'");
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
			ТекстСообщения = НСтр("ka='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке'");
		ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
			ТекстСообщения = НСтр("ka='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке'");
		Иначе
			ТекстСообщения = НСтр("ka='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке'");
		КонецЕсли;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='Подготовлено сообщений: 0.';ru='Подготовлено сообщений: 0.'");
...
Процедура ОповеститьПользователяОРезультатахВыполненияОбмена(Изменения) Экспорт
	...
	ТекстЗаголовка = НСтр("ka='Выполнение обмена с ГИСМ';ru='Выполнение обмена с ГИСМ'");
	...
	Если СтатусыДокументов.Количество() = 0 Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ka='Документов с измененным статусом: %1.';ru='Документов с измененным статусом: %1.'"), СтатусыДокументов.Количество());
	...
	ИначеЕсли СтатусыДокументов.Количество() = 1 Тогда
		...
		Для Каждого КлючИЗначение Из СтатусыДокументов Цикл
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Для документа %1 изменен статус ГИСМ: %2.';ru='Для документа %1 изменен статус ГИСМ: %2.'"),
				КлючИЗначение.Ключ,
