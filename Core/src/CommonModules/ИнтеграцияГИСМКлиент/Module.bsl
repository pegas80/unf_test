...
Процедура ПодтвердитьПолучениеДляДокументовИзСписка(ДинамическийСписок) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ka='В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.';ru='В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.';en='In these lines there are no documents for which acknowledgment of receipt Keyes.'"));
...
Процедура ПолучитьGLNПоИННКПП(Форма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнено поле ""Контрагент""';ru='Не заполнено поле ""Контрагент""';en='Do not fill in the ""Counterparty""'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Данные.ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнен ИНН контрагента';ru='Не заполнен ИНН контрагента';en='Not filled INN counterparty'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	...
	Если Данные = Неопределено Тогда
		...
		Если Результат.ИнформацияОВерсиях <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Результат.ИнформацияОВерсиях.ТекстСообщения) Тогда
				...
				Иначе
					ПоказатьОповещениеПользователя(Результат.ИнформацияОВерсиях.ТекстСообщения,,НСтр("ka='Версия обмена обновлена';ru='Версия обмена обновлена';en='exchange version is updated'"), БиблиотекаКартинок.Информация32);
				КонецЕсли;
			КонецЕсли;
...
Процедура Подписать(Сообщения, Организация, ОповещениеПриЗавершении) Экспорт
	...
	Если КоличествоСообщений = 1 Тогда
		ЗаголовокДанных = НСтр("ka='შეტყობინება';ru='Сообщение';en='Message'");
	Иначе
		ЗаголовокДанных = НСтр("ka='შეტყობინებები';ru='Сообщения';en='messages'");
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='გაცვლა მარკირების სისტემასთან';ru='Обмен с ГИСМ';en='Обмен с ГИСМ'"));
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(
				НСтр("ka='В информационной базе для пользователя %1 не найдено зарегистрированных сертификатов"
"электронной подписи, связанных с организацией %2, необходимых для обмена с ГИСМ.';ru='В информационной базе для пользователя %1 не найдено зарегистрированных сертификатов"
"электронной подписи, связанных с организацией %2, необходимых для обмена с ГИСМ.'"),
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", СтрШаблон(НСтр("ka='ჩვენება (%1)';ru='Показать (%1)';en='Display (1%)'"), КоличествоСообщений));
...
Функция СообщенияПоОрганизациям(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
			ЗаголовокСообщения = СтрШаблон(НСтр("ka='Сообщение ""%1"" не подготовлено к отправке в ГИСМ:';ru='Сообщение ""%1"" не подготовлено к отправке в ГИСМ:';en='Message ""%1"" is not prepared to be sent to GISM:'"), Сообщение.Описание) + Символы.ПС;
...
Процедура ПодготовитьСообщениеКПередаче(ДокументСсылка, Операция) Экспорт
	...
	Если Сообщения = Неопределено И Операция <> Неопределено Тогда
		ПоказатьПредупреждение(,СтрШаблон(НСтр("ka='По операции ""%1"" передача данных в ГИСМ не требуется';ru='По операции ""%1"" передача данных в ГИСМ не требуется';en='By the operation ""%1"" data transmission is required in GISM'"), Операция));
	КонецЕсли;
	Если Сообщения = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьСообщениеКПередаче_ПриЗавершенииОперацииПодписи(Сообщения, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Если Не Выполнено Тогда
			...
			ТекстЗаголовка = НСтр("ka='При подготовке сообщения к передаче в ГИСМ возникла ошибка:';ru='При подготовке сообщения к передаче в ГИСМ возникла ошибка:';en='When a message is to be transferred to GISM error occurred:'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстЗаголовка + Символы.ПС + ЭлементДанных.ТекстОшибки);
			
		Иначе
			
			ТекстЗаголовка = НСтр("ka='Сообщение подготовлено к передаче в ГИСМ';ru='Сообщение подготовлено к передаче в ГИСМ';en='A message prepared for transfer to the GISM'");
			Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаДанных") Тогда
				ТекстСообщения = НСтр("ka='Для документа %Документ% подготовлено к отправке сообщение передачи данных';ru='Для документа %Документ% подготовлено к отправке сообщение передачи данных';en='For the document% Document% prepared for sending data message'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
				ТекстСообщения = НСтр("ka='Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки';ru='Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки';en='For the document% Document% prepared to send a message to close the applications'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
				ТекстСообщения = НСтр("ka='Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров';ru='Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров';en='For the document% Document% prepared to send confirmation of receipt of goods'");
			КонецЕсли;
...
Процедура ОповеститьПользователяОВыполненнойОперации(СписокДокументов, КоличествоОбработанных, КоличествоВсего, Операция) Экспорт
	...
	ТекстЗаголовка = НСтр("ka='Подготовка сообщения к передаче в ГИСМ';ru='Подготовка сообщения к передаче в ГИСМ';en='Prepare messages for transfer to GISM'");
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
			ТекстСообщения = НСтр("ka='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке';en='For KolichestvoObrabotannyh%% of%% KolichestvoVsego selected in the list of documents prepared for the closing of the application sent'");
		ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
			ТекстСообщения = НСтр("ka='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке';en='For KolichestvoObrabotannyh%% of%% KolichestvoVsego selected in the list of confirmation of receipt of the documents prepared for shipment'");
		Иначе
			ТекстСообщения = НСтр("ka='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке';en='For KolichestvoObrabotannyh%% of%% KolichestvoVsego selected in the list of documents prepared for sending data'");
		КонецЕсли;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='Подготовлено сообщений: 0.';ru='Подготовлено сообщений: 0.';en='Prepared Posts: 0.'");
...
Процедура ОповеститьПользователяОРезультатахВыполненияОбмена(Изменения) Экспорт
	...
	ТекстЗаголовка = НСтр("ka='Выполнение обмена с ГИСМ';ru='Выполнение обмена с ГИСМ';en='Performing with GISM exchange'");
	...
	Если СтатусыДокументов.Количество() = 0 Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ka='Документов с измененным статусом: %1.';ru='Документов с измененным статусом: %1.';en='Documents with the changed status:% 1.'"), СтатусыДокументов.Количество());
	...
	ИначеЕсли СтатусыДокументов.Количество() = 1 Тогда
		...
		Для Каждого КлючИЗначение Из СтатусыДокументов Цикл
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Для документа %1 изменен статус ГИСМ: %2.';ru='Для документа %1 изменен статус ГИСМ: %2.';en='To document% 1 changed status GISM:% 2.'"),
				КлючИЗначение.Ключ,
...
"Проверьте, что для сертификатов электронной подписи указаны организации с заполненным GLN.';ka='В информационной базе для пользователя %1 не найдено зарегистрированных сертификатов"
"электронной подписи, необходимых для обмена с ГИСМ."
