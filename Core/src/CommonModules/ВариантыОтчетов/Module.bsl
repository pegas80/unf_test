...
Функция ИспользуемыеРазделы() Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Иначе
			ШаблонЗаголовка = НСтр("ka='განყოფილება ""%1""-ის ანგარიშგებები';ru='Отчеты раздела ""%1""'");
		КонецЕсли;
		...
		Если РазделМетаданные = ИдентификаторНачальнойСтраницы Тогда
			...
			Строка.Представление = НСтр("ka='საწყისი გვერდი';ru='Начальная страница'");
		Иначе
...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%2""-ისთვის ვარიანტი ""%1"" ვერ მოიძებნა.';ru='Вариант ""%1"" не найден для отчета ""%2"".'"),
			КлючВарианта,
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ОшибкаПоВарианту(
					Неопределено,
					НСтр("ka='სისტემური საცავიდან ანგარიშგებების სამომხმარებლო გამართვების არჩევის პროცესში შეცდომა წარმოიშვა:';ru='В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:'")
					+ Символы.ПС
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგების ვარიანტების პირდაპირი კონვერტაცია';ru='Прямая конвертация вариантов отчетов'");
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибкаПоВарианту(
				Неопределено,
				НСтр("ka='სტანდარტული საცავიდან ანგარიშგებების ვარიანტების შერჩევის პროცესში შეცდომა წარმოიშვა:';ru='В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგებების ვარიანტების კონვერტაციის დასრულება';ru='Завершить конвертацию вариантов отчетов'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Иначе
			ОшибкаПоВарианту(
				...
				НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტი ""%1"": ვერ მოიძებნა ავტორი ""%3""';ru='Вариант ""%1"" отчета ""%2"": не найден автор ""%3""'"),
		...
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მეთოდ ""%1""-ში პარამეტრები ""%2"" არ არის მითითებული.';ru='В методе ""%1"" не указаны параметры ""%2"".'"),
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეთოდ ""%1""-ში პარამეტრი ""%2"" არ არის მითითებული.';ru='В методе ""%1"" не указан параметр ""%2"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%1"" ჩაწერილია როგორც გარე, და მისი პროგრამიდან შეერთება არ შეიძლება';ru='Отчет ""%1"" записан как внешний и не может быть подключен из программы'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		Если ПолучатьМетаданные Тогда
			...
			Если Результат.Метаданные = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ანგარიშგება ""%1"" პროგრამაში ვერ მოიძებნა';ru='Отчет ""%1"" не найден в программе'"),
					Результат.Имя);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დამატებითი ანგარიშგების ვარიანტების ჩასაწერად დაშვების უფლებები არასაკმარისია ""%1"".';ru='Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვარიანტ ""%1""-ის გასახსნელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для открытия варианта ""%1"".'"), Строка(ВариантСсылка));
		КонецЕсли;
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Не Подключение.Успех Тогда
		Результат.ТекстОшибки = НСтр("ka='ანგარიშგების შედგენა ვერ მოხერხდა:';ru='Не удалось сформировать отчет:'") + Символы.ПС + Подключение.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если ПроверятьЗаполнение Тогда
		...
		Если Не ПроверкаПройдена Тогда
			Результат.ТекстОшибки = НСтр("ka='ანგარიშგებამ შევსების შემოწმება ვერ გაიარა:';ru='Отчет не прошел проверку заполнения:'");
	...
	Исключение
		ТекстОшибки = НСтр("ka='Не удалось определить используемые таблицы:';ru='Не удалось определить используемые таблицы:'");
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = НСтр("ka='ანგარიშგება ""%1""-ის სქემის წაკითხვა ვერ მოხერხდა:';ru='Не удалось прочитать схему отчета ""%1"":'");
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ka='ანგარიშგება ""%1""-ის ვარიანტების სიის წაკითხვა ვერ მოხერხდა სქემიდან:';ru='Не удалось прочитать список вариантов отчета ""%1"" из схемы:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ПредупреждениеПоВарианту(Неопределено, ТекстОшибки);
				КонецПопытки;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ka='მენეჯერის მოდულიდან ანგარიშგება ""%1""-ის ვარიანტების სიის წაკითხვა ვერ მოხერხდა:';ru='Не удалось прочитать список вариантов отчета ""%1"" из модуля менеджера:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტ ""%1""-ის გამართვების წაკითხვა ვერ მოხერხდა:';ru='Не удалось прочитать настройки варианта ""%1"" отчета ""%2"":'"),
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(ДеревоВариантов, Отчет, ВыводитьОтчетыВместоВариантов)
	...
	Иначе
		...
		Если СтрокаОтчет = Неопределено Тогда
			ПредупреждениеПоВарианту(Неопределено, НСтр("ka='ანგარიშგება ""%1"" ქვესისტემასთან არ არის შეერთებული.';ru='Отчет ""%1"" не подключен к подсистеме.'"), Отчет.Имя);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	СтрокаОтчет.ГруппироватьПоОтчету = ВыводитьОтчетыВместоВариантов;
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ПредупреждениеПоВарианту(
				Неопределено,
				НСтр("ka='ანგარიშგება ""%1"" პროგრამაში ვერ მოიძებნა, იგი აღნიშნული იქნება როგორც გარე.';ru='Отчет ""%1"" не найден в программе, он будет значиться как внешний.'"),
				ОтчетПолноеИмя);
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგება ""%1""-სთვის დაშვების უფლებები არასაკმარისია.';ru='Недостаточно прав доступа к отчету ""%1"".'"),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%1""-სთვის შეუძლებელია ტიპის განსაზღვრა (არ არის დადგენილი პრეფიქსი).';ru='Для отчета ""%1"" невозможно определить тип (не установлен префикс).'"),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
...
Функция ГлобальныеНастройки() Экспорт
	...
	Результат.Поиск.Вставить("ПодсказкаВвода", НСтр("ka='ანგარიშგების სახელწოდება, ველი ან ავტორი';ru='Наименование, поле или автор отчета'"));
...
Процедура ЗапускПроцедуры(ПредставлениеПроцедуры)
	ИнформацияПоВарианту(Неопределено, НСтр("ka='პროცედურა ""%1""-ის გაშვება.';ru='Запуск процедуры ""%1"".'"), ПредставлениеПроцедуры);
КонецПроцедуры
...
Процедура ЗавершениеПроцедуры(ПредставлениеПроцедуры, Изменено = Неопределено)
	Текст = НСтр("ka='პროცესურის დასრულება ""%1"".';ru='Завершение процедуры ""%1"".'");
	Если Изменено <> Неопределено Тогда
		Текст = Текст + " " + НСтр("ka='შეცვლილია %2 ობიექტი.';ru='Изменено %2 объектов.'");
	КонецЕсли;
...
Процедура ПерейтиКРедакции21() Экспорт
	ПредставлениеПроцедуры = НСтр("ka='რედაქცია 2.1-ზე გადასვლა';ru='Перейти к редакции 2.1'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგებების წინასწარ განსაზღვრული ვარიანტების ბმულების შევსება';ru='Заполнить ссылки предопределенных вариантов отчетов'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგებებში სწრაფი გამართვების რაოდენობის შემცირება';ru='Сокращение количества быстрых настроек в отчетах'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტი ""%1"":';ru='Вариант ""%1"" отчета ""%2"":'")
			+ Символы.ПС + НСтр("ka='სამომხმარებლოს სწრაფი გამართვების რაოდენობის შემცირებისას შეცდომა წარმოიშვა:';ru='При уменьшении количества быстрых настроек пользовательского возникла ошибка:'")
	...
	Если Ошибок > 0 И НомерПопытки <= 30 Тогда
		...
		ТекстОшибки = ПредставлениеПроцедуры + ":"
			+ Символы.ПС + НСтр("ka='%1 ანგარიშგების სწრაფი გამართვების რაოდენობის შემცირება ვერ მოხერხდა.';ru='Не удалось уменьшить количество быстрых настроек %1 отчетов.'");
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики)
	...
	Кэш.Вставить("Уточнение", ?(Кэш.Режим = "ОбщиеДанныеКонфигурации", НСтр("ka='კონფიგურაციის მეტამონაცემები';ru='метаданные конфигурации'"), НСтр("ka='გაფართოებების მეტამონაცემები';ru='метаданные расширений'")));
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное)
	...
	Кэш.Вставить("Уточнение", НРег(ОтчетыКлиентСервер.ИмяВПредставление(Режим)));
	Кэш.Уточнение = Кэш.Уточнение + ", " + ?(Полное, НСтр("ka='სრული';ru='полное'"), НСтр("ka='ცვლილებების მიხედვით';ru='по изменениям'"));
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ძებნის ინდექსის განახლება (%1)';ru='Обновление индекса поиска (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПримечаниеПоВарианту(Выборка.Ссылка, НСтр("ka='სქემის ინდექსაცია';ru='Индексация схемы'"));
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если Найденные.Количество() = 0 Тогда
				ТекстОшибки = НСтр("ka='ანგარიშგება ""%2""-ისთვის ვარიანტი ""%1"" ვერ მოიძებნა';ru='Вариант ""%1"" не найден для отчета ""%2""'");
				ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგებების ვარიანტების გასაღებების განახლება (%1)';ru='Обновление ключей вариантов отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='წინასწარ განსაზღვრულების გამართვების განახლება (%1)';ru='Обновление настроек предопределенных (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьСоставОбласти(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგებების ვარიანტების განახლება (%1)';ru='Обновление вариантов отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოშორებული ანგარიშგებების ვარიანტების წაშლა (%1)';ru='Удаление вариантов удаленных отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1"" (გახსნა)';ru='Отчет ""%1"" (открытие)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1"" (შედგენა)';ru='Отчет ""%1"" (формирование)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1"" (გამართვები)';ru='Отчет ""%1"" (настройки)'"), АктуальноеНаименование);
КонецПроцедуры
...
Функция СписокГлобальныхКлючевыхОпераций()
	...
	СписокОпераций.Добавить("ПанельОтчетов.Открытие",       НСтр("ka='ანგარიშგებების პანელი (გახსნა)';ru='Панель отчетов (открытие)'"));
	СписокОпераций.Добавить("ПанельОтчетов.Поиск",          НСтр("ka='ანგარიშგებების პანელი (ნახვის რეჟიმში ძებნა)';ru='Панель отчетов (поиск в режиме просмотра)'"));
	СписокОпераций.Добавить("ПанельОтчетов.РежимНастройки", НСтр("ka='ანგარიშგებების პანელი (გამართვის რეჟიმი)';ru='Панель отчетов (режим настройки)'"));
	СписокОпераций.Добавить("СписокОтчетов.Открытие", НСтр("ka='ანგარიშების სია (გახსნა)';ru='Список отчетов (открытие)'"));
	СписокОпераций.Добавить("СписокОтчетов.Поиск",    НСтр("ka='ანგარიშგებების სია (ძებნა)';ru='Список отчетов (поиск)'"));
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("ka='განუცალკევებელი ქეშის ჩაწერა კონსტანტაში';ru='Запись неразделенного кэша в константу'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений(Кэш)
	...
	ПредставлениеПроцедуры = НСтр("ka='გაფართოების ვერსიების რეგისტრის ჩანაწერი';ru='Запись регистра версий расширений'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
"Наименования полей или Наименования параметров и отборов.'"),
		...
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ НСтр("ka='დაწვრილებით - იხ. მოდულ ""ანგარიშგებებისვარიანტებიხელახლაგანსაზღვრებადი""-ს პროცედურა ""ანგარიშგებებისვარიანტებისგამართვა"".';ru='Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			Текст = НСтр("ka='ანგარიშგება ""%1""-ის წინასწარ განსაზღვრული ვარიანტის გამართვების წაკითხვა ვერ მოხერხდა.';ru='Не удалось прочитать настройки предопределенного варианта отчета ""%1"".'");
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	НастройкиКД = ВариантОбъект.Настройки.Получить();
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = НСтр("ka='აღმოჩენილია ანგარიშგება ""%2""-ის სამომხმარებლო ვარიანტ ""%1""-ის ცარიელი გამართვები.';ru='Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".'");
		ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
	...
	Если ОтчетыСНастройками.Найти(ОтчетСсылка) <> Неопределено Тогда
		Если ОтчетОбъект = Неопределено Тогда
			...
			Если Подключение.Успех Тогда
				ОтчетОбъект = Подключение.Объект;
			Иначе
				Текст = НСтр("ka='ანგარიშგება ""%1""-ის გამართვების მიღება ვერ მოხერხდა:';ru='Не удалось получить настройки отчета ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки;
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				...
				"НЕОПРЕДЕЛЕНО");
		Иначе
			Запрос.УстановитьПараметр("НаименованиеРабочегоСтола", НСтр("ka='საწყისი გვერდი';ru='Начальная страница'"));
		КонецЕсли;
