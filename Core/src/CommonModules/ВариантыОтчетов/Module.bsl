...
Функция ИспользуемыеРазделы() Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Иначе
			ШаблонЗаголовка = НСтр("ka='განყოფილება ""%1""-ის ანგარიშგებები';ru='Отчеты раздела ""%1""';en='Reports of ""%1""'");
		КонецЕсли;
		...
		Если РазделМетаданные = ИдентификаторНачальнойСтраницы Тогда
			...
			Строка.Представление = НСтр("ka='საწყისი გვერდი';ru='Начальная страница';en='start page'");
		Иначе
...
"Проверьте свойство ""Хранилище вариантов"" в свойствах отчета.';ka='ანგარიშგება ""%1""-ის აღწერილობის მიღების შეცდომა: იგი არ არის შეერთებული ქვესისტემა ""%2""-ზე."
"ანგარიშგების თვისებებში შეამოწმეთ თვისება ""ვარიანტების საცავი"".'"),
			Строка(Отчет),
...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%2""-ისთვის ვარიანტი ""%1"" ვერ მოიძებნა.';ru='Вариант ""%1"" не найден для отчета ""%2"".';en='Option ""%1"" could not be found for the report ""2%"".'"),
			КлючВарианта,
...
"Поиск отчетов временно недоступен.';ka='პროგრამაში გათვალისწინებული ანგარიშგებების ძებნის ინდექსის სრული განახლება. ანგარიშგებების ძებნა დროებით მიუწვდომელია.'");
	КонецЕсли;
...
"Поиск отчетов временно недоступен.';ka='მომხმარებლის მიერ შენახული ანგარიშგებების ძებნის ინდექსის სრული განახლება. ანგარიშგებების ძებნა დროებით მიუწვდომელია.'");
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректный тип параметра ""Отчет""';en='Invalid parameter type ""Report""'");
	КонецЕсли;
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ОшибкаПоВарианту(
					Неопределено,
					НСтр("ka='სისტემური საცავიდან ანგარიშგებების სამომხმარებლო გამართვების არჩევის პროცესში შეცდომა წარმოიშვა:';ru='В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:';en='The error occurred during the sample customizations reports from the system store:'")
					+ Символы.ПС
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგების ვარიანტების პირდაპირი კონვერტაცია';ru='Прямая конвертация вариантов отчетов';en='Direct conversion reporting options'");
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибкаПоВарианту(
				Неопределено,
				НСтр("ka='სტანდარტული საცავიდან ანგარიშგებების ვარიანტების შერჩევის პროცესში შეცდომა წარმოიშვა:';ru='В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:';en='The error occurred during the sample version of the report from the standard repository:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგებების ვარიანტების კონვერტაციის დასრულება';ru='Завершить конвертацию вариантов отчетов';en='Complete conversion reporting options'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Иначе
			ОшибкаПоВарианту(
				...
				НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტი ""%1"": ვერ მოიძებნა ავტორი ""%3""';ru='Вариант ""%1"" отчета ""%2"": не найден автор ""%3""';en='Option ""%1"" report ""%2"": not found the author ""%3""'"),
...
"Поиск отчетов временно недоступен.';ka='ანგარიშგებების ძებნის ინდექსის განახლება, რომლებიც პროგრამაში არის გათვალისწინებული. ანგარიშგებების ძებნა დროებით მიუწვდომელია.'");
	КонецЕсли;
...
"в пользовательских отчетах до 2 шт.';ka='სამომხმარებლო ანგარიშგებებში 2 ცალამდე ამცირებს სწრაფი გამართვების რაოდენობას'");
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
		...
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მეთოდ ""%1""-ში პარამეტრები ""%2"" არ არის მითითებული.';ru='В методе ""%1"" не указаны параметры ""%2"".';en='In the method ""%1"" parameter is specified, ""%2"".'"),
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вариант ""%1"" (ключ ""%2"") не найден в схеме отчета ""%3"".';en='Option ""%1"" (key ""%2"") was not found in the report schema ""%3"".'"),
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეთოდ ""%1""-ში პარამეტრი ""%2"" არ არის მითითებული.';ru='В методе ""%1"" не указан параметр ""%2"".';en='In the method of ""%1"" Unknown parameter ""%2"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%1"" ჩაწერილია როგორც გარე, და მისი პროგრამიდან შეერთება არ შეიძლება';ru='Отчет ""%1"" записан как внешний и не может быть подключен из программы';en='Report ""%1"" is recorded as an external and can not be connected from the program'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		Если ПолучатьМетаданные Тогда
			...
			Если Результат.Метаданные = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ანგარიშგება ""%1"" პროგრამაში ვერ მოიძებნა';ru='Отчет ""%1"" не найден в программе';en='Report ""%1"" was not found in the program'"),
					Результат.Имя);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დამატებითი ანგარიშგების ვარიანტების ჩასაწერად დაშვების უფლებები არასაკმარისია ""%1"".';ru='Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".';en='Not enough privileges to write an additional report options ""%1"".'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვარიანტ ""%1""-ის გასახსნელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для открытия варианта ""%1"".';en='Not have permission to open the option ""%1"".'"), Строка(ВариантСсылка));
		КонецЕсли;
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Не Подключение.Успех Тогда
		Результат.ТекстОшибки = НСтр("ka='ანგარიშგების შედგენა ვერ მოხერხდა:';ru='Не удалось сформировать отчет:';en='Unable to generate a report:'") + Символы.ПС + Подключение.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если ПроверятьЗаполнение Тогда
		...
		Если Не ПроверкаПройдена Тогда
			Результат.ТекстОшибки = НСтр("ka='ანგარიშგებამ შევსების შემოწმება ვერ გაიარა:';ru='Отчет не прошел проверку заполнения:';en='The report has been inspected filling:'");
	...
	Исключение
		ТекстОшибки = НСтр("ka='Не удалось определить используемые таблицы:';ru='Не удалось определить используемые таблицы:';en='Unable to determine the use of the table:'");
	...
	Если СсылкиВариантов.Количество() > 0 Тогда
		...
		Для Каждого Ссылка Из СсылкиВариантов Цикл
			...
			Если ЗначенияРеквизитов = Неопределено Тогда
				СтрокаТаблицы.Представление = НСтр("ru='<Недостаточно прав для работы с вариантом отчета>';en='<Insufficient permissions to work with the version of the report>'");
			Иначе
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не НайденВПрограмме Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" не найден в программе>';en='<Report ""%1"" was not found in the program>'"),
					ИмяОтчета);
			...
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" отключен в настройках программы>';en='<Report ""%1"" is disabled in the settings>'"),
					ИмяОтчета);
			Иначе
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = НСтр("ka='ანგარიშგება ""%1""-ის სქემის წაკითხვა ვერ მოხერხდა:';ru='Не удалось прочитать схему отчета ""%1"":';en='Unable to read the report schema ""%1"":'");
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ka='ანგარიშგება ""%1""-ის ვარიანტების სიის წაკითხვა ვერ მოხერხდა სქემიდან:';ru='Не удалось прочитать список вариантов отчета ""%1"" из схемы:';en='Unable to read the report, a list of ""%1"" from the scheme:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ПредупреждениеПоВарианту(Неопределено, ТекстОшибки);
				КонецПопытки;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ka='მენეჯერის მოდულიდან ანგარიშგება ""%1""-ის ვარიანტების სიის წაკითხვა ვერ მოხერხდა:';ru='Не удалось прочитать список вариантов отчета ""%1"" из модуля менеджера:';en='Unable to read the report, a list of ""%1"" of the management module:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტ ""%1""-ის გამართვების წაკითხვა ვერ მოხერხდა:';ru='Не удалось прочитать настройки варианта ""%1"" отчета ""%2"":';en='Unable to read the setting option ""%1"" report ""%2"":'"),
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(ДеревоВариантов, Отчет, ВыводитьОтчетыВместоВариантов)
	...
	Иначе
		...
		Если СтрокаОтчет = Неопределено Тогда
			ПредупреждениеПоВарианту(Неопределено, НСтр("ka='ანგარიშგება ""%1"" ქვესისტემასთან არ არის შეერთებული.';ru='Отчет ""%1"" не подключен к подсистеме.';en='Report ""%1"" is not connected to the subsystem.'"), Отчет.Имя);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	СтрокаОтчет.ГруппироватьПоОтчету = ВыводитьОтчетыВместоВариантов;
...
"так же числится как старое имя ""%4"" (актуальное имя ""%5"").';ka='ანგარიშგება ""%1""-ის ვარიანტის სახელის ცვლილების რეგისტრაციის შეცდომა:"
"ვარიანტის აქტუალური სახელი ""%2"" (ძველი სახელი ""%3"")"
"ასევე ირიცხება, როგორც ძველი სახელი ""%4"" (აქტუალური სახელი ""%5"").'"),
...
"варианта отчета ""%4"" (актуальное имя ""%5"").';ka='ანგარიშგების ვარიანტის სახელ ""%1""-ის ცვლილების რეგისტრაციის შეცდომა: ვარიანტის ძველი სახელია ""%2"" (აქტუალური სახელი ""%3"") ასევე ირიცხება ანგარიშგების ვარიანტ ""%4""-ის ძველ სახელად (აქტუალური სახელი ""%5"").'"),
				Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта,
				Изменение.АктуальноеИмяВарианта,
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ПредупреждениеПоВарианту(
				Неопределено,
				НСтр("ka='ანგარიშგება ""%1"" პროგრამაში ვერ მოიძებნა, იგი აღნიშნული იქნება როგორც გარე.';ru='Отчет ""%1"" не найден в программе, он будет значиться как внешний.';en='Report ""%1"" was not found in the program, it will be listed as External.'"),
				ОтчетПолноеИмя);
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგება ""%1""-სთვის დაშვების უფლებები არასაკმარისია.';ru='Недостаточно прав доступа к отчету ""%1"".';en='It is not enough to the report permissions ""%1"".'"),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%1""-სთვის შეუძლებელია ტიპის განსაზღვრა (არ არის დადგენილი პრეფიქსი).';ru='Для отчета ""%1"" невозможно определить тип (не установлен префикс).';en='it is impossible to determine the type (no prefix set) for the report ""%1"".'"),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
...
Функция ГлобальныеНастройки() Экспорт
	...
	Результат.Поиск.Вставить("ПодсказкаВвода", НСтр("ka='ანგარიშგების სახელწოდება, ველი ან ავტორი';ru='Наименование, поле или автор отчета';en='Name of the field or the author of the report'"));
...
Процедура ЗапускПроцедуры(ПредставлениеПроцедуры)
	ИнформацияПоВарианту(Неопределено, НСтр("ka='პროცედურა ""%1""-ის გაშვება.';ru='Запуск процедуры ""%1"".';en='Running the procedure ""%1"".'"), ПредставлениеПроцедуры);
КонецПроцедуры
...
Процедура ЗавершениеПроцедуры(ПредставлениеПроцедуры, Изменено = Неопределено)
	Текст = НСтр("ka='პროცესურის დასრულება ""%1"".';ru='Завершение процедуры ""%1"".';en='Completing the procedure ""%1"".'");
	Если Изменено <> Неопределено Тогда
		Текст = Текст + " " + НСтр("ka='შეცვლილია %2 ობიექტი.';ru='Изменено %2 объектов.';en='2% Modified objects.'");
	КонецЕсли;
...
Процедура ПерейтиКРедакции21() Экспорт
	ПредставлениеПроцедуры = НСтр("ka='რედაქცია 2.1-ზე გადასვლა';ru='Перейти к редакции 2.1';en='Go to the edition 2.1'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგებების წინასწარ განსაზღვრული ვარიანტების ბმულების შევსება';ru='Заполнить ссылки предопределенных вариантов отчетов';en='Fill links predefined reporting options'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("ka='ანგარიშგებებში სწრაფი გამართვების რაოდენობის შემცირება';ru='Сокращение количества быстрых настроек в отчетах';en='Reducing the number of quick settings in reports'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტი ""%1"":';ru='Вариант ""%1"" отчета ""%2"":';en='Option ""%1"" report ""%2"":'")
			+ Символы.ПС + НСтр("ka='სამომხმარებლოს სწრაფი გამართვების რაოდენობის შემცირებისას შეცდომა წარმოიშვა:';ru='При уменьшении количества быстрых настроек пользовательского возникла ошибка:';en='As the number of quick settings of the user error occurred:'")
	...
	Если Ошибок > 0 И НомерПопытки <= 30 Тогда
		...
		ТекстОшибки = ПредставлениеПроцедуры + ":"
			+ Символы.ПС + НСтр("ka='%1 ანგარიშგების სწრაფი გამართვების რაოდენობის შემცირება ვერ მოხერხდა.';ru='Не удалось уменьшить количество быстрых настроек %1 отчетов.';en='Failed to reduce the number of quick settings% 1 records.'");
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики)
	...
	Кэш.Вставить("Уточнение", ?(Кэш.Режим = "ОбщиеДанныеКонфигурации", НСтр("ka='კონფიგურაციის მეტამონაცემები';ru='метаданные конфигурации';en='configuration metadata'"), НСтр("ka='გაფართოებების მეტამონაცემები';ru='метаданные расширений';en='metadata extensions'")));
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное)
	...
	Кэш.Вставить("Уточнение", НРег(ОтчетыКлиентСервер.ИмяВПредставление(Режим)));
	Кэш.Уточнение = Кэш.Уточнение + ", " + ?(Полное, НСтр("ka='სრული';ru='полное';en='full'"), НСтр("ka='ცვლილებების მიხედვით';ru='по изменениям';en='on changes'"));
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ძებნის ინდექსის განახლება (%1)';ru='Обновление индекса поиска (%1)';en='Updating the search index (% 1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПримечаниеПоВарианту(Выборка.Ссылка, НСтр("ka='სქემის ინდექსაცია';ru='Индексация схемы';en='Indexing scheme'"));
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если Найденные.Количество() = 0 Тогда
				ТекстОшибки = НСтр("ka='ანგარიშგება ""%2""-ისთვის ვარიანტი ""%1"" ვერ მოიძებნა';ru='Вариант ""%1"" не найден для отчета ""%2""';en='Option ""%1"" could not be found for the report ""%2""'");
				ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგებების ვარიანტების გასაღებების განახლება (%1)';ru='Обновление ключей вариантов отчетов (%1)';en='Key update reporting options (1%)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='წინასწარ განსაზღვრულების გამართვების განახლება (%1)';ru='Обновление настроек предопределенных (%1)';en='Updating the settings of predefined (1%)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьСоставОбласти(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგებების ვარიანტების განახლება (%1)';ru='Обновление вариантов отчетов (%1)';en='Update report options (% 1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოშორებული ანგარიშგებების ვარიანტების წაშლა (%1)';ru='Удаление вариантов удаленных отчетов (%1)';en='Removal remote reporting options (1%)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1"" (გახსნა)';ru='Отчет ""%1"" (открытие)';en='Report ""%1"" (open)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1"" (შედგენა)';ru='Отчет ""%1"" (формирование)';en='Report ""%1"" (formation)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1"" (გამართვები)';ru='Отчет ""%1"" (настройки)';en='Report ""%1"" (settings)'"), АктуальноеНаименование);
КонецПроцедуры
...
Функция СписокГлобальныхКлючевыхОпераций()
	...
	СписокОпераций.Добавить("ПанельОтчетов.Открытие",       НСтр("ka='ანგარიშგებების პანელი (გახსნა)';ru='Панель отчетов (открытие)';en='Panel reports (opening)'"));
	СписокОпераций.Добавить("ПанельОтчетов.Поиск",          НСтр("ka='ანგარიშგებების პანელი (ნახვის რეჟიმში ძებნა)';ru='Панель отчетов (поиск в режиме просмотра)';en='Panel reports (search in playback mode)'"));
	СписокОпераций.Добавить("ПанельОтчетов.РежимНастройки", НСтр("ka='ანგარიშგებების პანელი (გამართვის რეჟიმი)';ru='Панель отчетов (режим настройки)';en='Reports Panel (Setup Mode)'"));
	СписокОпераций.Добавить("СписокОтчетов.Открытие", НСтр("ka='ანგარიშების სია (გახსნა)';ru='Список отчетов (открытие)';en='The list of reports (opening)'"));
	СписокОпераций.Добавить("СписокОтчетов.Поиск",    НСтр("ka='ანგარიშგებების სია (ძებნა)';ru='Список отчетов (поиск)';en='The list of reports (search)'"));
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("ka='განუცალკევებელი ქეშის ჩაწერა კონსტანტაში';ru='Запись неразделенного кэша в константу';en='Record undivided cache in constant'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений(Кэш)
	...
	ПредставлениеПроцедуры = НСтр("ka='გაფართოების ვერსიების რეგისტრის ჩანაწერი';ru='Запись регистра версий расширений';en='Recording versions extensions register'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ПроиндексироватьСодержимоеСхемы(ВариантОбъект) Экспорт
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტ ""%1""-ისთვის ძებნის გამართვები არ არის შევსებული: ველების სახელწოდებები ან პარამეტრების და შერჩევების სახელწოდებები.';ru='Для варианта ""%1"" отчета ""%2"" не заполнены настройки поиска:"
"Наименования полей или Наименования параметров и отборов.'"),
			ВариантОбъект.КлючВарианта,
			ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ НСтр("ka='დაწვრილებით - იხ. მოდულ ""ანგარიშგებებისვარიანტებიხელახლაგანსაზღვრებადი""-ს პროცედურა ""ანგარიშგებებისვარიანტებისგამართვა"".';ru='Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".';en='Learn more - see the procedure ""NastroitVariantyOtchetov"" module ""VariantyOtchetovPereopredelyaemy."".'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			Текст = НСтр("ka='ანგარიშგება ""%1""-ის წინასწარ განსაზღვრული ვარიანტის გამართვების წაკითხვა ვერ მოხერხდა.';ru='Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';en='Unable to read the configuration of a predetermined version of the report ""%1"".'");
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	НастройкиКД = ВариантОбъект.Настройки.Получить();
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = НСтр("ka='აღმოჩენილია ანგარიშგება ""%2""-ის სამომხმარებლო ვარიანტ ""%1""-ის ცარიელი გამართვები.';ru='Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".';en='Found empty customize the user option ""%1"" report ""2%"".'");
		ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
	...
	Если ОтчетыСНастройками.Найти(ОтчетСсылка) <> Неопределено Тогда
		Если ОтчетОбъект = Неопределено Тогда
			...
			Если Подключение.Успех Тогда
				ОтчетОбъект = Подключение.Объект;
			Иначе
				Текст = НСтр("ka='ანგარიშგება ""%1""-ის გამართვების მიღება ვერ მოხერხდა:';ru='Не удалось получить настройки отчета ""%1"":';en='Could not get a report setting ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки;
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				...
				"НЕОПРЕДЕЛЕНО");
		Иначе
			Запрос.УстановитьПараметр("НаименованиеРабочегоСтола", НСтр("ka='საწყისი გვერდი';ru='Начальная страница';en='start page'"));
		КонецЕсли;
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр("ka='ანგარიშგება შეიძლება არაკორექტულ მონაცემებს შეიცავდეს, ვინაიდან პროგრამის ახალ ვერსიაზე გადასვლა არ დასრულებულა. თუ ანგარიშგება დიდი ხნით მიუწვდომელია, მაშინ აუცილებლად უნდა მიმართოთ ადმინისტრატორს.';ru='Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, необходимо обратиться к администратору.';en='The report may contain incorrect data, as not completed the transition to the new version of the program. If the report is not available for a long time, you need to contact the administrator.'");
КонецФункции
