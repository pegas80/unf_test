Функция ЕстьОтрицательныеОстатки(СсылкаНаДокумент, Организация, СтруктурнаяЕдиница) Экспорт
	...
	Если НачалоМесяца(СсылкаНаДокумент.Дата) < НачалоМесяца(ТекущаяДатаСеанса()) Тогда
		...
		Если НЕ МРезультатов[0].Пустой() И НЕ МРезультатов[1].Пустой() Тогда
			Возврат СтрЗаменить(НСтр("ka='Есть остатки < 0 на %КонецМесяца% и на текущую дату';ru='Есть остатки < 0 на %КонецМесяца% и на текущую дату'"), "%КонецМесяца%", Формат(КонецМесяца(СсылкаНаДокумент.Дата), "ДЛФ=D"));
		ИначеЕсли НЕ МРезультатов[0].Пустой() Тогда
			Возврат НСтр("ka='Есть остатки < 0 на текущую дату';ru='Есть остатки < 0 на текущую дату'");
		ИначеЕсли НЕ МРезультатов[1].Пустой() Тогда
			Возврат СтрЗаменить(НСтр("ka='Есть остатки < 0 на %КонецМесяца%';ru='Есть остатки < 0 на %КонецМесяца%'"), "%КонецМесяца%", Формат(КонецМесяца(СсылкаНаДокумент.Дата), "ДЛФ=D"));
		Иначе
			Возврат "";
		КонецЕсли;
	Иначе
