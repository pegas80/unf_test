...
Функция ОткрытьКассовуюСмену(КассаККМ, ОписаниеОшибки = "", ТекстПереходящегоОстатка = "", КассоваяСмена = Неопределено) Экспорт
	...
	Если СтруктураСостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ka='С момента открытия кассовой смены истекло более 24 часов.';ru='С момента открытия кассовой смены истекло более 24 часов.'");
	...
	Иначе
		...
		Если НоваяКассоваяСмена.ПроверитьЗаполнение() Тогда
			...
			Если СуммаОстатка <> 0 Тогда
				ТекстПереходящегоОстатка = НСтр("ka='Сумма остатка в кассе: %1 руб.';ru='Сумма остатка в кассе: %1 руб.'");
		...
		Иначе
			Результат = Ложь;
			ОписаниеОшибки = НСтр("ka='Проверьте настройки кассы ККМ.';ru='Проверьте настройки кассы ККМ.'");
		КонецЕсли;
...
Функция СоздатьОтчетОРозничныхПродажах(КассоваяСмена, ОписаниеОшибки = "") Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = НСтр("ka='შეამოწმეთ საცალო საწყობის და სსა-ის სალაროს გასწორებები.';ru='Проверьте настройки розничного склада и кассы ККМ.'");
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.Статус = Перечисления.СтатусыКассовойСмены.Открыта Тогда
			...
			Если Дата - Выборка.НачалоКассовойСмены < 86400 Тогда
				СменаОткрыта = Истина;
			Иначе
				ОписаниеОшибки = НСтр("ka='სალაროს ცვლის გახსნიდან ამოიწურა 24 საათზე მეტი. აუცილებელია სალაროს ცვლის დახურვა';ru='С момента открытия кассовой смены истекло более 24 часов. Необходимо выполнить закрытие кассовой смены'");
				СменаОткрыта = Ложь;
			КонецЕсли;
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.Статус) Тогда
			...
			Если Выборка.ОкончаниеКассовойСмены >= Дата И Выборка.НачалоКассовойСмены <= Дата Тогда
				СменаОткрыта = Истина;
			Иначе
				ОписаниеОшибки = НСтр("ka='ცვლა არ არის გახსნილი';ru='Смена не открыта'");
				СменаОткрыта = Ложь;
			КонецЕсли;
