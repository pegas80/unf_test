...
Функция СоединениеССерверомИнтернета(ПараметрыСоединения)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронная подпись в модели сервиса.Соединение с сервером интернета';ru='Электронная подпись в модели сервиса.Соединение с сервером интернета';en='Electronic signature in servisa.Soedinenie model Internet server'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ВызватьHTTPМетод(HTTPМетод, URL, ПараметрыЗапроса, СоответствиеПолейОтвета, Заголовки = Неопределено)
	...
	Иначе
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронная подпись в модели сервиса.Менеджер сервиса криптографии';ru='Электронная подпись в модели сервиса.Менеджер сервиса криптографии';en='Electronic signature in servisa.Menedzher cryptography service model'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,, Ответ.ПолучитьТелоКакСтроку());
		Результат.Вставить("Выполнено", Ложь);
		Результат.Вставить("КодОшибки", "НеизвестнаяОшибка");
		Результат.Вставить("ОписаниеОшибки", НСтр("ka='სერვისი დროებით მიუწვდომელია. მიმართეთ მხარდაჭერის სამსახურს ან მოგვიანებით სცადეთ.';ru='Сервис временно недоступен. Обратитесь в службу поддержки или повторите попытку позже.';en='Service is temporarily unavailable. Please contact customer support or try again later.'"));
	КонецЕсли;
