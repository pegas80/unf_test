...
Процедура ТестНастроекСПроверкойСертификата(Сертификат, ВладелецФормы, ПроверитьАвторизацию,
	...
	ЗаголовокФормы = НСтр("ka='სერთიფიკატ %1-ის შემოწმება';ru='Проверка сертификата %1';en='Checking certificate% 1'");
...
Процедура ОбработатьОтклонениеАннулированиеЭД(СсылкаНаЭД, ПараметрыЭД, ОтклонитьАннулирование = Ложь) Экспорт
	...
	Если ПараметрыЭД.Отклонить Тогда
		...
		Если ОтклонитьАннулирование Тогда
			Заголовок = НСтр("ka='მიუთითეთ ანულირების შესახებ წინადადების უარყოფის მიზეზები';ru='Укажите причины отклонения предложения об аннулировании';en='Identify the reasons for rejection of the proposal to annul'");
		Иначе
			
			Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоСчетФактура(СсылкаНаЭД) Тогда
				Заголовок = НСтр("ka='მიუთითეთ დაზუსტების თაობაზე მოთხოვნის ტექსტი';ru='Укажите текст запроса на уточнение';en='Specify the query text to clarify'");
			Иначе
	...
	Иначе
		...
		Заголовок = НСтр("ka='მიუთითეთ დოკუმენტის ანულირების მიზეზები';ru='Укажите причины аннулирования документа';en='Identify the reasons for cancellation of the document'");
	КонецЕсли;
	...
	Если ВходящаяСтруктура.Свойство("СтруктСтруктурМассивовЭДИСертификатов", СтруктСтруктурМассивовЭДИСертификатов)
		...
		Для Каждого Элемент Из СтруктСтруктурМассивовЭДИСертификатов Цикл
			...
			Если Структура.Свойство("СоответствиеЭдИДд", СоответствиеЭдИДд)
				...
				Иначе
					ПредставлениеЭД = НСтр("ka='Электронные документы (%1)';ru='Электронные документы (%1)';en='Electronic documents (1%)'");
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ТекстЗаголовка = НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами';en='Exchange of electronic documents'");
...
Процедура ОтправитьПолучитьЭДВыполнить(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ka='მიმდინარეობს მოწვევების შესახებ ინფორმაციის მიღება. მოიცადეთ...';ru='Выполняется получение информации о приглашениях. Подождите...';en='Retrieves information about referrals. Wait...'");
	Состояние(НСтр("ka='მიღება';ru='Получение.';en='Getting.'"), , ТекстСообщения);
	...
	ТекстСообщения = НСтр("ka='მიმდინარეობს ელექტრონული დოკუმენტების პაკეტის გაგზავნა და მიღება. მოიცადეთ...';ru='Выполняется отправка и получение пакетов электронных документов. Подождите...';en='You are sending and receiving packets of electronic documents. Wait...'");
	Состояние(НСтр("ka='გაგზავნა და მიღება.';ru='Отправка и получение.';en='Sending and receiving.'"), , ТекстСообщения);
	...
	ЗаголовокОповещения = НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами';en='Exchange of electronic documents'");
...
Процедура ПолучитьМаркерПродолжить(Отпечатки, ДополнительныеПараметры) Экспорт
	...
	Если СвойстваСертификата.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ka='ხელმისაწვდომი სერთიფიკატები არ არის. ოპერაცია %1-ის შესრულება შეწყვეტილია.';ru='Нет доступных сертификатов. Выполнение операции %1 прервано.';en='No certificates available. Operation% 1 has been interrupted.'");
	...
	Если ЗашифрованныеДанные = Неопределено Тогда
		...
		ПредставлениеОперации = ДополнительныеПараметры.ПредставлениеОперации;
		ТекстСообщения = НСтр("ka='Ошибка получения данных от сервиса ЭДО. Выполнение операции %1 прервано.';ru='Ошибка получения данных от сервиса ЭДО. Выполнение операции %1 прервано.';en='Error retrieving data from the EDI service. Operation% 1 has been interrupted.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ПредставлениеОперации);
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",   НСтр("ka='ედბ-ის სერვისში ავთენტიფიკაცია';ru='Аутентификация в сервисе ЭДО';en='Authentication EDI service'"));
	ОписаниеДанных.Вставить("ОтборСертификатов",   МассивСертификатов);
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклНачало(Контекст)
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='ელექტრონული დოკუმენტის გაშიფრვა';ru='Расшифровка электронного документа';en='Decoding of an electronic document'"));
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклПослеРасшифровкиЭД(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не РезультатВыполнения.Успех Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ka='პაკეტიდან ელექტრონული დოკუმენტის გაშიფრვა ვერ მოხერხდა.';ru='Не удалось расшифровать электронный документ из пакета.';en='Unable to decrypt the electronic document from the package.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеОшибкиВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ШаблонСообщения = НСтр("ka='სერთიფიკატის გადმოტვირთვა: %1';ru='Выгрузка сертификата: %1';en='Uploading certificate% 1'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИнформацияОбОшибке);
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеВыгрузкиСертификата(ДанныеСертификата, Контекст) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Контекст.Подпись.АдресЭД) Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ka='საპასუხო ხელმოწერის შესამოწმებლად საწყისი მონაცემები ვერ მოიძებნა.';ru='Не удалось найти исходные данные для проверки ответной подписи.';en='Could not find the original data to verify the signature of the response.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
"%2.'");
	...
	ШаблонОповещения = НСтр("ka='განფუთულია პაკეტი: %1.';ru='Распаковано пакетов: %1.';en='Unzip the package:% 1.'");
...
"%1"
"произошла ошибка: %2';ka='ელექტრონული დოკუმენტის ხელმოწერის შემოწმებისას:"
"%1 "
"შეცდომა მოხდა: %2'");
...
Процедура ОткрытьЭДДляПросмотраПослеПолученияОтпечатков(Отпечатки, Параметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяФормыЭлектронногоДокумента) Тогда
		ВызватьИсключение НСтр("ru='Электронный документ не предназначен для просмотра.';en='An electronic document is not intended for viewing.'");
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(СоответствиеДанныхПакетов) = Тип("Соответствие") Тогда
		Если СоответствиеДанныхПакетов.Количество() > Параметры.ИндексПеребораПакетов Тогда
			...
			Если Параметры.ИндексПеребораДанных + 1 < МассивДанныхЭД.Количество() Тогда
				...
				Исключение
					ВидОперации = НСтр("ka='მონაცემების დაშიფვრა';ru='Шифрование данных';en='data encryption'");
					ТекстСообщения = НСтр("ka='მონაცემთა დაშიფრვისას შეცდომა მოხდა: %1';ru='При шифровании данных произошла ошибка: %1';en='If you encrypt data error occurred:% 1'");
...
Процедура ОповеститьПользователяОСменеОтветственного(Ответственный, КоличествоВсего, КоличествоОбработанных) Экспорт
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		ТекстСообщения = НСтр("ka='გამოყოფილი ელ.დოკუმენტების %КоличествоВсего%-დან %КоличествоОбработанных%-სთვის დაყენებულია პასუხისმგებელი';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных эл.документов"
"установлен ответственный ""%Ответственный%""'");
		...
		ТекстЗаголовка = НСтр("ka='პასუხისმგებელი ""%Ответственный%"" დაყენებულია';ru='Ответственный ""%Ответственный%"" установлен';en='Responsible ""%% responsible"" is set'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%Ответственный%", Ответственный);
		ПоказатьОповещениеПользователя(ТекстЗаголовка, , ТекстСообщения, БиблиотекаКартинок.Информация32);
		
	Иначе
		
		ТекстСообщения = НСтр("ka='პასუხისმგებელი ""%პასუხისმგებელი%"" არ არის დადგენილი არცერთი ელ.დოკუმენტისთვის.';ru='Ответственный ""%Ответственный%"" не установлен ни для одного эл.документа.';en='Responsible ""%% responsible"" is not set for any el.dokumenta.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ответственный%", Ответственный);
		ТекстЗаголовка = НСтр("ka='პასუხისმგებელი ""%Ответственный%"" არ არის დაყენებული';ru='Ответственный ""%Ответственный%"" не установлен';en='Responsible ""%% responsible"" is not set'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%Ответственный%", Ответственный);
		ПоказатьОповещениеПользователя(ТекстЗаголовка,, ТекстСообщения, БиблиотекаКартинок.Информация32);
...
Процедура СформироватьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрКоманды.Количество() > 1 Тогда
		...
		ТекстВопроса = НСтр("ka='გამოყოფილი ელემენტებისთვის შევქმნათ ელექტრონული დოკუმენტები?';ru='Создать электронные документы для выделенных элементов?';en='Create electronic documents for selected items?'");
