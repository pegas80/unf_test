...
Процедура ПроверитьКонтрагентовПослеВключенияПроверкиФоновоеЗадание() Экспорт
	...
	Попытка
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='კონტრაგენტების შემოწმება';ru='Проверка контрагентов';en='Checking counterparties'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. შეუმოწმებელი კონტრაგენტების დამუშავების ფონური დავალება ჩართვის შემდეგ';ru='Проверка контрагентов.Фоновое задание по обработке непроверенных контрагентов после включения';en='Check kontragentov.Fonovoe task processing untested counterparties after the'", КодОсновногоЯзыка),
...
Процедура УстановитьВидимостьИЗаголовокПредупрежденияПроТестовыйРежим(Элемент) Экспорт
	...
	Иначе
		...
		Элемент.Заголовок = 
			НСтр("ka='ამჟამად, საგადასახადო სამსახურის ვებ-სერვისი სატესტო რეჟიმში მუშაობს.';ru='В настоящее время веб-сервис ФНС работает в тестовом режиме.';en='Currently, the Federal Tax Service web service running in test mode.'");
	КонецЕсли;
...
Процедура УстановитьНадписиВПанелиОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტების შემოწმება საგადასახადო სამსახურის მონაცემებით წარმატებით შესრულდა';ru='Проверка контрагентов по данным ФНС выполнена успешно';en='Checking counterparties according FTS successful'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Есть некорректные контрагенты.
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриНекорректныхКонтрагентах");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='საგადასახადო სამსახურის მონაცემებით აღმოჩენილია უმოქმედო კონტრაგენტები';ru='Обнаружены недействующие контрагенты по данным ФНС';en='Found inactive counterparts according FNS'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Выполняется проверка
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриПроверкеВПроцессеВыполнения");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='საგადასახადო სამსახურის მონაცემთა მიხედვით მიმდინარეობს კონტრაგენტების შემოწმება';ru='Выполняется проверка контрагентов согласно данным ФНС';en='Checks counterparts according to FNS'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Нет доступа к сервису
	ЭлементФормы = Форма.Элементы.Найти("ТекстНетДоступаКСервису");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტების შემოწმება ვერ მოხერხდა: საგადასახადო სამსახურის სერვისი დროებით მიუწვდომელია';ru='Не удалось произвести проверку контрагентов: сервис ФНС временно недоступен';en='Failed to check contractors: FTS service is temporarily unavailable'"), СсылкаНаИнструкцию);
	КонецЕсли;
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		Попытка
		 	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентовВДокументе", 
				Параметры, , НСтр("ka='კონტრაგენტების შემოწმება დოკუმენტში';ru='Проверка контрагентов в документе';en='Checking counterparties in the document'"));
		...
		Исключение
			...
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. ფონური დავალება დოკუმენტში';ru='Проверка контрагентов.Фоновое задание в документе';en='Check kontragentov.Fonovoe assignment in the document'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПроверитьКонтрагентовВОтчете(Форма, СтруктураПараметров) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='კონტრაგენტების შემოწმება ანგარიშგებაში';ru='Проверка контрагентов в отчете';en='Checking counterparties report'");
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Исключение
		...
		НСтр("ka='კონტრაგენტების შემოწმება. კონტრაგენტების მდგომარეობის გასუფთავება';ru='Проверка контрагентов.Очистка состояний контрагентов';en='Check kontragentov.Ochistka state contractors'", КодОсновногоЯзыка), 
...
Процедура УдалитьРезультатПроверкиКонтрагентовПоДокументам(Документы = Неопределено) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(
		НСтр("ka='კონტრაგენტების შემოწმება. დოკუმენტების მდგომარეობის გასუფთავება';ru='Проверка контрагентов.Очистка состояний документов';en='Check kontragentov.Ochistka state documents'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"До завершения выполнения существование контрагента может быть определено не для всех контрагентов';ka='ახორციელებს საგადასახადო სამსახურის ბაზაში კონტრაგენტების არსებობის შემოწმებას. შესრულების დამთავრებამდე, კონტრაგენტის არსებობა შეიძლება ყველა კონტრაგენტისთვის არ იყოს განსაზღვრული'");
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. შეცდომა კონტრაგენტის ბარათიდან შემოწმებისას';ru='Проверка контрагентов.Ошибка при проверке из карточки контрагента';en='Check kontragentov.Oshibka when checking out a company card'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. ვებ-სერვისთან დაშვების შეცდომა';ru='Проверка контрагентов.Ошибка доступа к веб-сервису';en='Check kontragentov.Oshibka access to web service'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ОтветСервисаФНС(Прокси, WSЗапрос)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება.NdsRequest2';ru='Проверка контрагентов.NdsRequest2';en='Check kontragentov.NdsRequest2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДобавитьКонтрагентаВЗапросКСервису(WSЗапрос, Прокси, ДанныеКонтрагента)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. მონაცემთა შედგენა საგადასახადო სამსახურის სერვისთან მიმართვისთვის';ru='Проверка контрагентов.Формирование данных для обращения к сервису ФНС';en='Check kontragentov.Formirovanie Information to access the FTS service'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки; 
	...
	Для ИндексТекущегоКонтрагента = МинимальныйНомерКонтрагента По МаксимальныйНомерКонтрагента Цикл 
		...
		Если ДанныеКонтрагента.Состояние <> Перечисления.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных Тогда
			...
			Исключение
				...
				КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
				ЗаписьЖурналаРегистрации(
				НСтр("ka='კონტრაგენტების შემოწმება. ვებ-სერვისის პასუხის დამუშავება';ru='Проверка контрагентов.Обработка ответа от веб-сервиса';en='Check kontragentov.Obrabotka response from the web service'", КодОсновногоЯзыка), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='კონტრაგენტების შემოწმება. რეგისტრში კონტრაგენტების შემოწმების შედეგების ჩაწერა';ru='Проверка контрагентов.Запись результатов проверки контрагентов в регистр';en='Checking results kontragentov.Zapis counterparts in test register'", КодОсновногоЯзыка), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентовВДокументеВРегистр(СохраняемыйРезультат)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		НСтр("ka='კონტრაგენტების შემოწმება. რეგისტრში დოკუმენტების შემოწმების შედეგების ჩაწერა';ru='Проверка контрагентов.Запись результатов проверки документов в регистр';en='Check kontragentov.Zapis documents verify the results in the register'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция НовоеПолеКартинки(Форма, ТаблицаФормы, Родитель, ПолеСКонтрагентом)
	...
	НовоеПоле.Заголовок					= НСтр("ka='საგადასახადო სამსახურის მონაცემთა მიხედვით შეცდომა არის';ru='Есть ошибка по данным ФНС';en='There is an error according to the Federal Tax Service'");
