...
Процедура ПроверитьКонтрагентовПослеВключенияПроверкиФоновоеЗадание() Экспорт
	...
	Попытка
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='კონტრაგენტების შემოწმება';ru='Проверка контрагентов'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. შეუმოწმებელი კონტრაგენტების დამუშავების ფონური დავალება ჩართვის შემდეგ';ru='Проверка контрагентов.Фоновое задание по обработке непроверенных контрагентов после включения'", КодОсновногоЯзыка),
...
Процедура УстановитьВидимостьИЗаголовокПредупрежденияПроТестовыйРежим(Элемент) Экспорт
	...
	Иначе
		...
		Элемент.Заголовок = 
			НСтр("ka='ამჟამად, საგადასახადო სამსახურის ვებ-სერვისი სატესტო რეჟიმში მუშაობს.';ru='В настоящее время веб-сервис ФНС работает в тестовом режиме.'");
	КонецЕсли;
...
Процедура УстановитьНадписиВПанелиОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტების შემოწმება საგადასახადო სამსახურის მონაცემებით წარმატებით შესრულდა';ru='Проверка контрагентов по данным ФНС выполнена успешно'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Есть некорректные контрагенты.
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриНекорректныхКонтрагентах");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='საგადასახადო სამსახურის მონაცემებით აღმოჩენილია უმოქმედო კონტრაგენტები';ru='Обнаружены недействующие контрагенты по данным ФНС'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Выполняется проверка
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриПроверкеВПроцессеВыполнения");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='საგადასახადო სამსახურის მონაცემთა მიხედვით მიმდინარეობს კონტრაგენტების შემოწმება';ru='Выполняется проверка контрагентов согласно данным ФНС'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Нет доступа к сервису
	ЭлементФормы = Форма.Элементы.Найти("ТекстНетДоступаКСервису");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='კონტრაგენტების შემოწმება ვერ მოხერხდა: საგადასახადო სამსახურის სერვისი დროებით მიუწვდომელია';ru='Не удалось произвести проверку контрагентов: сервис ФНС временно недоступен'"), СсылкаНаИнструкцию);
	КонецЕсли;
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		Попытка
		 	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентовВДокументе", 
				Параметры, , НСтр("ka='კონტრაგენტების შემოწმება დოკუმენტში';ru='Проверка контрагентов в документе'"));
		...
		Исключение
			...
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. ფონური დავალება დოკუმენტში';ru='Проверка контрагентов.Фоновое задание в документе'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПроверитьКонтрагентовВОтчете(Форма, СтруктураПараметров) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='კონტრაგენტების შემოწმება ანგარიშგებაში';ru='Проверка контрагентов в отчете'");
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Исключение
		...
		НСтр("ka='კონტრაგენტების შემოწმება. კონტრაგენტების მდგომარეობის გასუფთავება';ru='Проверка контрагентов.Очистка состояний контрагентов'", КодОсновногоЯзыка), 
...
Процедура УдалитьРезультатПроверкиКонтрагентовПоДокументам(Документы = Неопределено) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(
		НСтр("ka='კონტრაგენტების შემოწმება. დოკუმენტების მდგომარეობის გასუფთავება';ru='Проверка контрагентов.Очистка состояний документов'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. შეცდომა კონტრაგენტის ბარათიდან შემოწმებისას';ru='Проверка контрагентов.Ошибка при проверке из карточки контрагента'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. ვებ-სერვისთან დაშვების შეცდომა';ru='Проверка контрагентов.Ошибка доступа к веб-сервису'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ОтветСервисаФНС(Прокси, WSЗапрос)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება.NdsRequest2';ru='Проверка контрагентов.NdsRequest2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДобавитьКонтрагентаВЗапросКСервису(WSЗапрос, Прокси, ДанныеКонтрагента)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. მონაცემთა შედგენა საგადასახადო სამსახურის სერვისთან მიმართვისთვის';ru='Проверка контрагентов.Формирование данных для обращения к сервису ФНС'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки; 
	...
	Для ИндексТекущегоКонтрагента = МинимальныйНомерКонтрагента По МаксимальныйНомерКонтрагента Цикл 
		...
		Если ДанныеКонтрагента.Состояние <> Перечисления.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных Тогда
			...
			Исключение
				...
				КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
				ЗаписьЖурналаРегистрации(
				НСтр("ka='კონტრაგენტების შემოწმება. ვებ-სერვისის პასუხის დამუშავება';ru='Проверка контрагентов.Обработка ответа от веб-сервиса'", КодОсновногоЯзыка), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='კონტრაგენტების შემოწმება. რეგისტრში კონტრაგენტების შემოწმების შედეგების ჩაწერა';ru='Проверка контрагентов.Запись результатов проверки контрагентов в регистр'", КодОсновногоЯзыка), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентовВДокументеВРегистр(СохраняемыйРезультат)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		НСтр("ka='კონტრაგენტების შემოწმება. რეგისტრში დოკუმენტების შემოწმების შედეგების ჩაწერა';ru='Проверка контрагентов.Запись результатов проверки документов в регистр'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция НовоеПолеКартинки(Форма, ТаблицаФормы, Родитель, ПолеСКонтрагентом)
	...
	НовоеПоле.Заголовок					= НСтр("ka='საგადასახადო სამსახურის მონაცემთა მიხედვით შეცდომა არის';ru='Есть ошибка по данным ФНС'");
