...
Процедура УстановитьПоставляемоеРасширениеВОбластьДанных(Знач ОписаниеИнсталляции, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='გადმოცემული გაფართოებები.  ინიცირებულია მონაცემთა არიალში გადმოცემული გაფართოებების დაყენება';ru='Поставляемые расширения.Инициирована установка поставляемого расширения в область данных'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемоеРасширение) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. დაყენება მონაცემთა არიალში.';ru='Поставляемые расширения.Установка в область данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. დაყენება მონაცემთა არიალში გადადებულია.';ru='Поставляемые расширения.Установка в область данных отложена'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемоеРасширениеИзОбластиДанных(Знач ПоставляемоеРасширение) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გაუქმება მონაცემთა არიალიდან';ru='Поставляемые расширения.Удаление из области данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемоеРасширение,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გაუქმები შეცდომა მონაცემთა არიალიდან.';ru='Поставляемые расширения.Ошибка удаления из области данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиРасширенияВОбластьДанных(Знач ПоставляемоеРасширение, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. მონაცემთა არიალში დაყენების შეცდომა.';ru='Поставляемые расширения.Ошибка установки в область данных'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,
		ПоставляемоеРасширение,
...
Процедура АктуализацияИспользуемогоРасширения(Знач ИдентификаторИспользуемогоРасширения) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='Расширение с идентификатором %1 не является поставляемым!';ru='Расширение с идентификатором %1 не является поставляемым!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Если НЕ Загружать Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='გადმოცემული გაფართოებები. გადმოცემული გაფართოების ჩატვირთვა გაუქმებულია.';ru='Поставляемые расширения.Загрузка поставляемого расширения отменена'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,,
				НСтр("ka='მიწოდებადი გაფართოება არათავსებადია არსებულ კონფიგურაციასთან';ru='Поставляемое расширение несовместимо с данной конфигурацией'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Совместимость);
...
Процедура ЗапроситьОбновлениеРасширений() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='Запрос обновлений расширений';ru='Запрос обновлений расширений'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбработатьПоставляемыеРасширения(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გადმოცემული გაფართოების ჩატვირთვა';ru='Поставляемые расширения.Загрузка поставляемого расширения'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ka='Инициирована загрузка поставляемого расширения';ru='Инициирована загрузка поставляемого расширения'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
	...
	Если ПоставляемоеРасширение.ИдентификаторВерсии <> ОписаниеПоставляемогоРасширения.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გადმოცემული გაფართოების ჩატვირთვა დასრულებულია.';ru='Поставляемые расширения.Загрузка поставляемого расширения завершена'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,,
			ПоставляемоеРасширение.Ссылка,
			НСтр("ka='Завершена загрузка поставляемого расширения';ru='Завершена загрузка поставляемого расширения'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
		...
		Для каждого ИнсталляцияРасширения из ИспользуемыеРасширения Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. დაგეგმილია გამოყენებული გაფართოების აქტუალიზება.';ru='Поставляемые расширения.Запланирована актуализация используемого расширения'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
				НСтр("ka='მონაცემების არე:';ru='Область данных:'") + ИнсталляцияРасширения.ОбластьДанных);
		...
		Для каждого ЭлементОчереди из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='გადმოცემული გაფართოებები. დაგეგმილია მონაცემთა არიალის  გადმოცემული გაფართოებების გადავადებული  დაყენება ';ru='Поставляемые расширения.Запланирована отложенная установка поставляемого расширения в область данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
