...
"доступно только для сеансов с установленным разделением данных!';ka='Использование функции РасширенияВМоделиСервиса.ПоставляемоеРасширение()"
...
"доступно только для сеансов с установленным разделением данных!';ka='Использование функции РасширенияВМоделиСервиса.ИспользуемоеРасширение()"
"доступно только для сеансов с установленным разделением данных!'");
...
Процедура УстановитьПоставляемоеРасширениеВОбластьДанных(Знач ОписаниеИнсталляции, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='გადმოცემული გაფართოებები.  ინიცირებულია მონაცემთა არიალში გადმოცემული გაფართოებების დაყენება';ru='Поставляемые расширения.Инициирована установка поставляемого расширения в область данных';en='Supplied rasshireniya.Initsiirovana installation supplied expansion in the data area'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемоеРасширение) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. დაყენება მონაცემთა არიალში.';ru='Поставляемые расширения.Установка в область данных';en='Rasshireniya.Ustanovka supplied to the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. დაყენება მონაცემთა არიალში გადადებულია.';ru='Поставляемые расширения.Установка в область данных отложена';en='Rasshireniya.Ustanovka supplied to the data area postponed'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемоеРасширениеИзОбластиДанных(Знач ПоставляемоеРасширение) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გაუქმება მონაცემთა არიალიდან';ru='Поставляемые расширения.Удаление из области данных';en='Rasshireniya.Udalenie supplied from the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемоеРасширение,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გაუქმები შეცდომა მონაცემთა არიალიდან.';ru='Поставляемые расширения.Ошибка удаления из области данных';en='Supplied rasshireniya.Oshibka remove from the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиРасширенияВОбластьДанных(Знач ПоставляемоеРасширение, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. მონაცემთა არიალში დაყენების შეცდომა.';ru='Поставляемые расширения.Ошибка установки в область данных';en='Supplied rasshireniya.Oshibka installation in the data area'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,
		ПоставляемоеРасширение,
...
Процедура АктуализацияИспользуемогоРасширения(Знач ИдентификаторИспользуемогоРасширения) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='Расширение с идентификатором %1 не является поставляемым!';ru='Расширение с идентификатором %1 не является поставляемым!';en='Extension with ID% 1 is not supplied!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Если НЕ Загружать Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='გადმოცემული გაფართოებები. გადმოცემული გაფართოების ჩატვირთვა გაუქმებულია.';ru='Поставляемые расширения.Загрузка поставляемого расширения отменена';en='Supplied rasshireniya.Zagruzka supplied expansion canceled'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,,
				НСтр("ka='მიწოდებადი გაფართოება არათავსებადია არსებულ კონფიგურაციასთან';ru='Поставляемое расширение несовместимо с данной конфигурацией';en='Supplied expansion compatible with this configuration'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Совместимость);
...
Процедура ЗапроситьОбновлениеРасширений() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='Запрос обновлений расширений';ru='Запрос обновлений расширений';en='Request for extension updates'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбработатьПоставляемыеРасширения(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გადმოცემული გაფართოების ჩატვირთვა';ru='Поставляемые расширения.Загрузка поставляемого расширения';en='Supplied rasshireniya.Zagruzka supplied extension'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ka='Инициирована загрузка поставляемого расширения';ru='Инициирована загрузка поставляемого расширения';en='Initiated by loading the supplied extension'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
	...
	Если ПоставляемоеРасширение.ИдентификаторВерсии <> ОписаниеПоставляемогоРасширения.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. გადმოცემული გაფართოების ჩატვირთვა დასრულებულია.';ru='Поставляемые расширения.Загрузка поставляемого расширения завершена';en='Supplied rasshireniya.Zagruzka supplied extension completed'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,,
			ПоставляемоеРасширение.Ссылка,
			НСтр("ka='Завершена загрузка поставляемого расширения';ru='Завершена загрузка поставляемого расширения';en='Completed loading the supplied extension'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
		...
		Для каждого ИнсталляцияРасширения из ИспользуемыеРасширения Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='გადმოცემული გაფართოებები. დაგეგმილია გამოყენებული გაფართოების აქტუალიზება.';ru='Поставляемые расширения.Запланирована актуализация используемого расширения';en='Supplied rasshireniya.Zaplanirovana used updated extensions'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
				НСтр("ka='მონაცემების არე:';ru='Область данных:';en='Data Field:'") + ИнсталляцияРасширения.ОбластьДанных);
		...
		Для каждого ЭлементОчереди из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='გადმოცემული გაფართოებები. დაგეგმილია მონაცემთა არიალის  გადმოცემული გაფართოებების გადავადებული  დაყენება ';ru='Поставляемые расширения.Запланирована отложенная установка поставляемого расширения в область данных';en='Supplied rasshireniya.Zaplanirovana delayed installation of the supplied expansion in the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
