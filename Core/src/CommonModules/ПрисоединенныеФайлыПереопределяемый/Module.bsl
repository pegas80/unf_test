...
Процедура ПриСозданииНаСервереПрисоединенныйФайл(Знач Форма) Экспорт
	...
	Форма.Заголовок = НСтр("ka='მიერთებული ფაილი';ru='Присоединенный файл'");
	...
	Декорация.Заголовок = НСтр("ka='ფაილის ბარათზე გადასასვლელად დააჭირეთ ჰიპერბმულს';ru='Для того, чтобы перейти к карточке файла, нажмите на гиперссылку'");
	
	Кнопка = Форма.Элементы.Добавить("ПерейтиКФормеФайла1", Тип("КнопкаФормы"));
	Кнопка.Вид        = ВидКнопкиФормы.Гиперссылка;
	Кнопка.Заголовок  = НСтр("ka='ფაილის ფორმაზე გადასვლა';ru='Перейти к форме файла'");
...
Процедура ВызватьИсключениеОткрытияФормы(Форма) Экспорт
	...
	ВызватьИсключение НСтр("ka='ფორმის დამოუკიდებლად გამოყენება არ არის გათვალისწინებული.';ru='Самостоятельное использование формы не предусмотрено.'");
