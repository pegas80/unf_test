	...
	Если ИДОтчета = "РегламентированныйОтчетУСН" Тогда
		Если ИДРедакцииОтчета = "ФормаОтчета2009Кв1" Тогда
			Если Не ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
				ВызватьИсключение НСтр("ka='Не заполнен реквизит отчета Организация при авторасчете декларации по УСН';ru='Не заполнен реквизит отчета Организация при авторасчете декларации по УСН'");
			КонецЕсли;
	...
	Если ДокументыПерсУчета.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ТипЗнч(ЗапрашиваемыеДанные.Ссылка) = Тип("СправочникСсылка.КомплектыОтчетностиПерсучета") Тогда
				...
				ЗапрашиваемыеДанные.Наименование = НСтр("ka='Комплект форм квартальной отчетности в ПФР';ru='Комплект форм квартальной отчетности в ПФР'");
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОпределитьТаблицуОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания) Экспорт
	...
	ОписаниеДокументаОтчетности.ГруппаВДереве = НСтр("ka='Отчетность по физлицам';ru='Отчетность по физлицам'");
...
//  Команда.Подсказка = НСтр("ka='Показать дополнительную информацию';ru='Показать дополнительную информацию'");
...
//  Кнопка.Заголовок = НСтр("ka='Инфо';ru='Инфо'");
