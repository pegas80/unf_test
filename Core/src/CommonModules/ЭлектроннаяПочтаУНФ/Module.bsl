...
Функция ОтправитьПочтовоеСообщение(Знач ПараметрыПисьма, Знач УчетнаяЗапись) Экспорт
	...
	Если ТипЗнч(УчетнаяЗапись) <> Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты")
		...
		ВызватьИсключение НСтр("ka='სააღრიცხვო ჩანაწერი შეუვსებელია ან არასწორად შევსებულია.';ru='Учетная запись не заполнена или заполнена неправильно.'");
	КонецЕсли;
...
Процедура ЗагрузитьПочтуПоКомандеПользователя(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='ელექტრონული ფოსტის შეტყობინებების სათაურების ჩატვირთვა';ru='Загрузка заголовков сообщений электронной почты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьСодержаниеСообщения(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Параметры.УчетнаяЗапись) Тогда
			ВызватьИсключение НСтр("ka='დავალება ""შეტყობინებისშემცველობისჩატვირთვა()""-სთვის სააღრიცხვო ჩანაწერი არ არის მითითებული.';ru='Не указана учетная запись для задания ""ЗагрузитьСодержаниеСообщения()"".'");
		КонецЕсли;
...
"Повторно подключите учетную запись Gmail.'");
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='ელექტრონული ფოსტის შეტყობინების შემცველობის ჩატვირთვა';ru='Загрузка содержания сообщения электронной почты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		СообщитьОбОшибкеДлительногоЗадания(ИнформацияОбОшибке);
...
Функция ЗаголовокПрогресса(Знач УчетнаяЗапись, Знач ОтборПоКонтакту) Экспорт
	...
	Если ЗначениеЗаполнено(ОтборПоКонтакту) Тогда
		Возврат СтрШаблон(НСтр("ka='მიმდინარეობს მიმოწერის ჩატვირთვა ""%1""-დან ""%2""-ში';ru='Загружается переписка с ""%1"" из ""%2""'"), ОтборПоКонтакту, УчетнаяЗапись);
	Иначе
		Возврат СтрШаблон(НСтр("ka='Получение почты ""%1""';ru='Получение почты ""%1""'"), УчетнаяЗапись);
	КонецЕсли;
...
Процедура ОбработатьПараметрыПисьма(ПараметрыПисьма, УчетнаяЗапись)
	...
	Если ПараметрыПисьма = Неопределено Тогда
		ВызватьИсключение НСтр("ka='არ არის მითითებული გაგზავნის პარამეტრები.';ru='Не заданы параметры отправки.'");
	КонецЕсли;
	...
	Если ТипЗнчКому = Неопределено И ТипЗнчКопии = Неопределено И ТипЗнчСлепыеКопии = Неопределено Тогда
		ВызватьИсключение НСтр("ka='მითითებული არ არის არცერთი მიმღები.';ru='Не указано ни одного получателя.'");
	КонецЕсли;
...
Функция ЗагрузитьЗаголовкиСообщенийИнтернетПочты(УчетнаяЗапись, РежимЗагрузки, Знач Пользователь, Знач ОтборПоКонтакту = Неопределено)
	...
	Для Каждого ТекСтрока Из ТаблицаИнтернетПочтовыхСообщений Цикл
		...
		СтрШаблон(НСтр("ka='%1: ჩატვირთულია %3-დან %2';ru='%1: загружено %2 из %3'"),
		ЗаголовокПрогресса,
...
Функция ЗаписатьСобытие(ИнтернетПочтовоеСообщение, ПочтовыйЯщик, Идентификатор, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СобытияПоИдентификатору) Тогда
			...
			НСтр("ka='შესაძლებელია, რომ ფოსტის ჩატვირთვის რამდენიმე დავალება იყოს გაშვებული: შეტყობინება იდენტიფიკატორ %1-ით უკვე ჩაწერილია.';ru='Возможно, запущено несколько заданий загрузки почты: сообщение с идентификатором %1 уже записано.'"),
			Идентификатор);
...
Функция ПротоколИнтернетПочтыIMAP(УчетнаяЗапись)
	...
	ВызватьИсключение НСтр("ka='Подключение к серверу входящей почты может быть выполнено только по протоколу IMAP.';ru='Подключение к серверу входящей почты может быть выполнено только по протоколу IMAP.'");
