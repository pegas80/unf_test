	...
	Для Каждого ТекущийТип Из ТипыЗначений Цикл
		...
		Если Свойства = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ტიპი ""%1"" დაშვების მნიშვნელობების ტიპს არ წარმოადგენს';ru='Тип ""%1"" не является типом значений доступа'"), Строка(ТекущийТип));
		КонецЕсли;
	...
	Попытка
		...
		Если Объект.Пользователи.Найти(Администратор, "Пользователь") = Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='დაშვების მართვა.დაშვების ჯგუფ ""ადმინისტრატორები""-ს ავტომატური ცვლილება';ru='Управление доступом.Автоматическое изменение группы доступа Администраторы'",
				...
				Если СтарыеРоли[Строка.Роль] = Неопределено Тогда
					...
					Иначе
						...
						Для каждого Профиль Из Профили Цикл
							ЗаписьЖурналаРегистрации(
								НСтр("ka='Управление доступом.Роль не найдена в метаданных';ru='Управление доступом.Роль не найдена в метаданных'",
								     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнениеДанныхДляОграниченияДоступа(КоличествоДанных = 0, ТолькоКэшРеквизиты = Ложь, ЕстьИзменения = Неопределено) Экспорт
	...
	Если КоличествоДанных < 10000 Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ka='წვდომის მართვა.მონაცემთა შევსება დაშვების შესაზღუდავად';ru='Управление доступом.Заполнение данных для ограничения доступа'",
				 ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			НСтр("ka='დაშვების შესაზღუდავად მონაცემთა შევსება დასრულებულია.';ru='Завершено заполнение данных для ограничения доступа.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	...
	Иначе
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			НСтр("ka='შესრულებულია მონაცემთა ნაწილის ჩაწერა წვდომის შეზღუდვისთვის.';ru='Выполнена запись части данных для ограничения доступа.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	КонецЕсли;
...
Процедура ПриИзмененииОграниченияДоступаНаУровнеЗаписей(ОграничениеДоступаНаУровнеЗаписейВключено) Экспорт
	...
	Если ОграничениеДоступаНаУровнеЗаписейВключено Тогда
		...
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			НСтр("ka='დაშვების შესაზღუდავად მონაცემთა შევსება დაწყებულია.';ru='Начато заполнение данных для ограничения доступа.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
...
Процедура ОбновитьНаборыЗаписей(Знач Данные, ЕстьИзменения) Экспорт
	...
	Если Данные.НовыеЗаписиСодержатТолькоРазличия Тогда
		...
		Если Данные.ИмяПервогоИзмерения = Неопределено Тогда
			ВызватьИсключение
				НСтр("ka='არასწორი პარამეტრები პროცედურაში ჩანაწერებისკრებულებისგანახლება.';ru='Некорректные параметры в процедуре ОбновитьНаборыЗаписей.'");
		Иначе
...
Функция СвойстваПрофиляОткрытиеВнешнихОтчетовИОбработок()
	...
	Результат.Наименование = НСтр("ka='გარე ანგარიშგებების და დამუშავებების გახსნა';ru='Открытие внешних отчетов и обработок'", Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	Если ТипЗнч(Пользователь) <> Тип("СправочникСсылка.Пользователи")
	   И ТипЗнч(Пользователь) <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ОжидаемыеТипы, 
			?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, НСтр("ka='დაუდგენელია';ru='Неопределено'")),
...
"დასაშვები ტიპები: სტრუქტურა, დაუდგენელია.'"),
	...
	Для каждого КлючИЗначение Из Параметры Цикл
		Если Не ВсеПараметры.Свойство(КлючИЗначение.Ключ) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მითითებულია არარსებული პარამეტრი %1';ru='Указан несуществующий параметр %1'"),
				КлючИЗначение.Ключ);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОбязательныеПараметры) Тогда
		...
		Для каждого КлючИЗначение Из ОбязательныеПараметры Цикл
			Если Не Параметры.Свойство(КлючИЗначение.Ключ) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='სავალდებულო პარამეტრი %1 არ არის მითითებული';ru='Не указан обязательный параметр %1'"),
					КлючИЗначение.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПредставленияВсеРазрешеныВФорме(Форма, ЭтоПрофиль)
	...
	Если ЭтоПрофиль Тогда
		...
		Строка.Представление = НСтр("ka='ყველა აკრძალულია, გამოაკლისები ინიშნება დაშვების ჯგუფებში';ru='Все запрещены, исключения назначаются в группах доступа'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВначалеВсеРазрешены";
		Строка.Представление = НСтр("ka='ყველა ნებადართულია, გამონაკლისები ინიშნება დაშვების ჯგუფებში';ru='Все разрешены, исключения назначаются в группах доступа'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеЗапрещены";
		Строка.Представление = НСтр("ka='ყველა აკრძალულია, გამონაკლისები ინიშნება პროფილში';ru='Все запрещены, исключения назначаются в профиле'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеРазрешены";
		Строка.Представление = НСтр("ka='ყველა ნებადართულია, გამონაკლისები ინიშნება პროფილში';ru='Все разрешены, исключения назначаются в профиле'");
	Иначе
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеЗапрещены";
		Строка.Представление = НСтр("ka='ყველა აკრძალულია';ru='Все запрещены'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеРазрешены";
		Строка.Представление = НСтр("ka='ყველა ნებადართულია';ru='Все разрешены'");
	КонецЕсли;
