	...
	Для Каждого ТекущийТип Из ТипыЗначений Цикл
		...
		Если Свойства = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ტიპი ""%1"" დაშვების მნიშვნელობების ტიპს არ წარმოადგენს';ru='Тип ""%1"" не является типом значений доступа';en='Type ""%1"" is not an access type values'"), Строка(ТекущийТип));
		КонецЕсли;
...
Процедура ПослеЗаписиАдминистратораПриАвторизации(Комментарий) Экспорт
	...
	Комментарий =
		НСтр("ka='შესრულებულია გაშვება როლ ""სრული უფლებები""-ს მქონე მომხმარებლის სახელით, რომელიც არ არის რეგისტრირებული მომხმარებლების სიაში."
"მომხმარებლების სიაში შესრულებულია ავტომატური რეგისტრაცია."
"მომხმარებელი დამატებულია დაშვების ჯგუფ ""ადმინისტრატორები""-ში."
""
"მომხმარებლების უფლებების გასამართავად და სიის საწარმოებლად განკუთვნილია სია ""მომხმარებლები"", ამისთვის 1С:საწარმოს კონფიგურირების რეჟიმის გამოყენება არ არის საჭირო.';ru='Выполнен запуск от имени пользователя с ролью ""Полные права"","
...
Процедура ПриОпределенииТекстаВопросаПередЗаписьюПервогоАдминистратора(ТекстВопроса) Экспорт
	...
	ТекстВопроса =
		НСтр("ka='პროგრამის მომხმარებელთა სიას ემატება პირველი მომხმარებელი, ამიტომ, იგი ავტომატურად იქნება შეყვანილი დაშვების ჯგუფ ადმინისტრატორებში. გავაგრძელოთ?';ru='В список пользователей программы добавляется первый пользователь,"
...
"%2';ka='მომხმარებელი ""%1"" დამატებულია დაშვების ჯგუფ ""ადმინისტრატორები""-ში შემდეგი მიზეზით:"
"%2'"),
		Администратор,
		Уточнение);
	...
	Попытка
		...
		Если Объект.Пользователи.Найти(Администратор, "Пользователь") = Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='დაშვების მართვა.დაშვების ჯგუფ ""ადმინისტრატორები""-ს ავტომატური ცვლილება';ru='Управление доступом.Автоматическое изменение группы доступа Администраторы';en='dostupom.Avtomaticheskoe change Access Control Administrators group'",
				...
				Если СтарыеРоли[Строка.Роль] = Неопределено Тогда
					...
					Иначе
						...
						Для каждого Профиль Из Профили Цикл
							ЗаписьЖурналаРегистрации(
								НСтр("ka='Управление доступом.Роль не найдена в метаданных';ru='Управление доступом.Роль не найдена в метаданных';en='dostupom.Rol control is not found in the metadata'",
								     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"не найдена в метаданных.';ka='При обновлении ролей пользователя ""%1"""
"роль ""%2"""
"профиля групп доступа ""%3"""
...
Процедура ПроверитьНаличиеПользователяИБВГруппеДоступаАдминистраторы(ПользователиГруппы, ОписаниеОшибки) Экспорт
	...
	Если НЕ НайденДействующийАдминистратор Тогда
		ОписаниеОшибки =
			НСтр("ka='დაშვების ჯგუფ ადმინისტრატორებში უნდა იყოს თუნდ ერთი მომხმარებელი, რომლისთვის ნებადართული იქნება პროგრამაში შესვლა.';ru='В группе доступа Администраторы"
...
Процедура ЗаполнениеДанныхДляОграниченияДоступа(КоличествоДанных = 0, ТолькоКэшРеквизиты = Ложь, ЕстьИзменения = Неопределено) Экспорт
	...
	Если КоличествоДанных < 10000 Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ka='წვდომის მართვა.მონაცემთა შევსება დაშვების შესაზღუდავად';ru='Управление доступом.Заполнение данных для ограничения доступа';en='dostupom.Zapolnenie management data to limit access'",
				 ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			НСтр("ka='დაშვების შესაზღუდავად მონაცემთა შევსება დასრულებულია.';ru='Завершено заполнение данных для ограничения доступа.';en='Completed filling data to limit access.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	...
	Иначе
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			НСтр("ka='შესრულებულია მონაცემთა ნაწილის ჩაწერა წვდომის შეზღუდვისთვის.';ru='Выполнена запись части данных для ограничения доступа.';en='When recording of the data to limit access.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	КонецЕсли;
...
"У объекта типа ""%1"""
"не найдена табличная часть ""Наборы значений доступа"".';ka='Неверные параметры."
"У объекта типа ""%1"""
...
"""Записать наборы значений доступа"".';ka='Неверные параметры."
"Тип объекта ""%1"""
"не найден в подписке на события"
...
"не задан тип %1';ka='შეცდომა მოხდა დაშვების მნიშვნელობების კრებულების ჩაწერისას: ინფორმაციის რეგისტრში დაშვებისმნიშვნელობებისკრებულები გნზომილება ობიექტში არ არის მიცემული ტიპი %1'"),
...
Процедура ПриИзмененииОграниченияДоступаНаУровнеЗаписей(ОграничениеДоступаНаУровнеЗаписейВключено) Экспорт
	...
	Если ОграничениеДоступаНаУровнеЗаписейВключено Тогда
		...
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			НСтр("ka='დაშვების შესაზღუდავად მონაცემთა შევსება დაწყებულია.';ru='Начато заполнение данных для ограничения доступа.';en='Started filling the data to limit access.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
...
"""%1"".';ka='შეცდომა პროცედურის შესრულებისას დაშვებისმართვა.შერჩევისპირობისდაყენებამოთხოვნაში()."
""
"პარამეტრში შერჩევისპირობისპარამეტრისსახელიველისსახელი ვერ მოიძებნა გამყოფი (ორი წერტილი) ფორმატის შემდეგ სტრიქონში ""<პირობის პარამეტრის სახელი>:<ველის სახელი>"""
"""%1"".'"),
				ТекущаяСтрока);
		КонецЕсли;
...
Процедура ОбновитьНаборыЗаписей(Знач Данные, ЕстьИзменения) Экспорт
	...
	Если Данные.НовыеЗаписиСодержатТолькоРазличия Тогда
		...
		Если Данные.ИмяПервогоИзмерения = Неопределено Тогда
			ВызватьИсключение
				НСтр("ka='არასწორი პარამეტრები პროცედურაში ჩანაწერებისკრებულებისგანახლება.';ru='Некорректные параметры в процедуре ОбновитьНаборыЗаписей.';en='Incorrect settings in ObnovitNaboryZapisey procedure.'");
		Иначе
...
""
"Неверный параметр ОписаниеОбъектаМетаданных.';ka='შეცდომა საერთო მოდულ დაშვებისმართვასამსახურებრივი-ის ფუნქცია მეტამონაცემთაობიექტისცარიელიბმული-ში."
""
"არასწორი პარამეტრი მეტამონაცემთაობიექტისაღწერილობა.'");
	КонецЕсли;
...
"""%1"".';ka='შეცდომა საერთო მოდულ წვდომისმართვასამსახურებრივი-ს ფუნქცია მეტამონაცემთაობიექტისცარიელიბმული-ს ფუნქციაში."
""
"მეტამონაცემთა ობიექტ ""%1""-სთვის ცარიელი ბმულის მიღება ვერ მოხერხდა.'"),
...
""
"В тексте запроса не найдена строка ""&ПодстановкаПоляВидИзмененияСтроки"".';ka='შეცდომა მოდულის დაშვებისმართვასამსახურებრივი პარამეტრის მნიშვნელობაში განზომილებებისარჩევისმოთხოვნისტექსტი."
""
"მოთხოვნის ტექსტში ვერ მოიძებნა სტრიქონი ""&ПодстановкаПоляВидИзмененияСтроки"".'");
	КонецЕсли;
...
"процедуры ТекстЗапросаВыбораИзменений модуля УправлениеДоступомСлужебный."
""
"В тексте запроса не найдена строка ""&ПодстановкаПоляВидИзмененияСтроки"".';ka='შეცდომა მოდულის დაშვებისმართვასამსახურებრივი პროცედურის ცვლილებებისარჩევისმოთხოვნისტექსტი პარამეტრის მნიშვნელობაში ახლებისარჩევისმოთხოვნისტექსტი."
""
"მოთხოვნის ტექსტში ვერ მოიძებნა სტრიქონი ""&ПодстановкаПоляВидИзмененияСтроки"".'");
	КонецЕсли;
...
"""%1"" возникла ошибка:"
""
"%2';ka='ობიექტის დაშვების მნიშვნელობების დამოკიდებული კრებულის განახლებისას"
"""%1"" წარმოიშვა შეცდომა:"
""
"%2'"),
...
Функция СвойстваПрофиляОткрытиеВнешнихОтчетовИОбработок()
	...
	Результат.Наименование = НСтр("ka='გარე ანგარიშგებების და დამუშავებების გახსნა';ru='Открытие внешних отчетов и обработок';en='Opening of external reports and treatments'", Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	Если ТипЗнч(Пользователь) <> Тип("СправочникСсылка.Пользователи")
	   И ТипЗнч(Пользователь) <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ОжидаемыеТипы, 
			?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, НСтр("ka='დაუდგენელია';ru='Неопределено';en='Undefined'")),
...
"Допустимые типы: Структура, Неопределено.';ka='თვისებების კრებულის არაკორექტული ტიპი ""%1""."
"დასაშვები ტიპები: სტრუქტურა, დაუდგენელია.'"),
	...
	Для каждого КлючИЗначение Из Параметры Цикл
		Если Не ВсеПараметры.Свойство(КлючИЗначение.Ключ) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მითითებულია არარსებული პარამეტრი %1';ru='Указан несуществующий параметр %1';en='A nonexistent parameter 1%'"),
				КлючИЗначение.Ключ);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОбязательныеПараметры) Тогда
		...
		Для каждого КлючИЗначение Из ОбязательныеПараметры Цикл
			Если Не Параметры.Свойство(КлючИЗначение.Ключ) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='სავალდებულო პარამეტრი %1 არ არის მითითებული';ru='Не указан обязательный параметр %1';en='Unknown required parameter% 1'"),
					КлючИЗначение.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
"""Записать наборы значений доступа"".';ka='არასწორი პარამეტრები. ობიექტის ტიპი ""%1"" მოვლენების გამოწერებში ვერ მოიძებნა ""წვდომის მნიშვნელობების კრებულების ჩაწერა"".'"),
			ТипЗначенияОбъект);
	КонецЕсли;
	...
	Если ВозможныеТипыОбъектов.Получить(ТипЗнч(СсылкаНаОбъект)) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи наборов значений доступа:"
"в регистре сведений НаборыЗначенийДоступа в измерении Объект"
"не задан тип ""%1"".';ka='შეცდომა დაშვების მნიშვნელობების კრებულის ჩაწერისას: ინფორმაციის რეგისტრ დაშვებისმნიშვნელობებისკრებულები-ში ზომილობა ობიექტი-ში არ არის მიცემული ტიპი ""%1"".'"),
			СсылкаНаОбъект.Метаданные().ПолноеИмя());
	КонецЕсли;
...
"Тип объекта ""%1"""
"не найден в подписках на события"
"""Заполнить наборы значений доступа табличных частей"".';ka='არასწორი პარამეტრები. ობიექტის ტიპი ""%1"" მოვლენების გამოწერებში ვერ მოიძებნა ""ცხრილების დაშვების მნიშვნელობების კრებულების შევსება"".'"),
					ТипЗначенияОбъект);
			КонецЕсли;
...
"Тип объекта ""%1"""
"не найден в подписке на события"
"""Записать зависимые наборы значений доступа"".';ka='არასწორი პარამეტრები. ობიექტის ტიპი ""%1"" ვერ მოიძებნა მოვლენების ""დაშვების მნიშვნელობების დამოკიდებული კრებულების ჩაწერა"" ხელმოწერაში.'"),
			ТипЗначенияОбъект);
	КонецЕсли;
...
Процедура ЗаписатьПользователяПриОбновленииРолей(ПользовательСсылка, ПользовательИБ, БылиПолныеПрава, ПарольПользователяСервиса)
	...
	Если ПарольПользователяСервиса = Неопределено Тогда
		...
		ВызватьИсключение
			НСтр("ka='ადმინისტრაციული დაშვების შესაცვლელად საჭიროა სერვისის მომხმარებლის პაროლი."
""
"ოპერაცია შეიძლება შესრულდეს მხოლოდ ინტერაქტიულად.';ru='Для изменения административного доступа"
...
""
"Допустимо только 2 значения: ""1"" и ""-1"".';ka='შეცდომა საერთო მოდულის დაშვებისმართვასამსახურებრივი პროცედურაში ჩანაწერებისკრებულებისგანახლება."
""
"პარამეტრ ახალიჩანაწერები-ს არასწორი მნიშვნელობა - სვეტი სტრიქონისშეცვლისტიპი შეიცავს დაუშვებელ მნიშვნელობას ""%1""."
""
"დასაშვებია მხოლოდ 2 მნიშვნელობა: ""1"" и ""-1"".'"),
...
"в значении параметра НовыеЗаписи.';ka='შეცდომა საერთო მოდულის დაშვებისმართვასამსახურებრივი პროცედურაში ჩანაწერებისკრებულებისგანახლება."
""
"პარამეტრ ახალიჩანაწერები-ს მნიშვნელობის სტრიქონში მოთხოვნილი ვერ მოიძებნა.'");
...
Процедура ЗаполнитьТаблицуПредставленияВсеРазрешеныВФорме(Форма, ЭтоПрофиль)
	...
	Если ЭтоПрофиль Тогда
		...
		Строка.Представление = НСтр("ka='ყველა აკრძალულია, გამოაკლისები ინიშნება დაშვების ჯგუფებში';ru='Все запрещены, исключения назначаются в группах доступа';en='All prohibited, exceptions are assigned to access groups'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВначалеВсеРазрешены";
		Строка.Представление = НСтр("ka='ყველა ნებადართულია, გამონაკლისები ინიშნება დაშვების ჯგუფებში';ru='Все разрешены, исключения назначаются в группах доступа';en='All allowed, exceptions are assigned to access groups'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеЗапрещены";
		Строка.Представление = НСтр("ka='ყველა აკრძალულია, გამონაკლისები ინიშნება პროფილში';ru='Все запрещены, исключения назначаются в профиле';en='All prohibited, exceptions are assigned to profile'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеРазрешены";
		Строка.Представление = НСтр("ka='ყველა ნებადართულია, გამონაკლისები ინიშნება პროფილში';ru='Все разрешены, исключения назначаются в профиле';en='All allowed, exceptions are assigned to profile'");
	Иначе
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеЗапрещены";
		Строка.Представление = НСтр("ka='ყველა აკრძალულია';ru='Все запрещены';en='All prohibited'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеРазрешены";
		Строка.Представление = НСтр("ka='ყველა ნებადართულია';ru='Все разрешены';en='All resolved'");
	КонецЕсли;
