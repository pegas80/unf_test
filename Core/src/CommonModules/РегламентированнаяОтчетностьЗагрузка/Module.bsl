...
Функция ЗагрузкаОтчетаИзФайла(ФормаОтчета, ПараметрыОтчета, ДанныеДляЗагрузки, ДеревоДляЗагрузки) Экспорт
	...
	Если НЕ ЭтоАдресВременногоХранилища(ОсновнойФайл.АдресДанных) Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось открыть файл выгрузки!';ru='Не удалось открыть файл выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТипЗнч(НастройкиЗагрузки) <> Тип("Структура")
		...
		СообщениеОбОшибке.ТекстСообщения = СтрШаблон(
			НСтр("ka='Возможность загрузки из файла отчета ""%1"" за выбранный период не поддерживается.';ru='Возможность загрузки из файла отчета ""%1"" за выбранный период не поддерживается.'"),
	...
	Если ТипЗнч(ДеревоДляЗагрузки) <> Тип("ДеревоЗначений") И ТипЗнч(ДеревоДляЗагрузки) <> Тип("Структура") Тогда
		СообщениеОбОшибке.Способ = "Предупреждение";
		СообщениеОбОшибке.ТекстСообщения = СтрШаблон(
			НСтр("ka='Возможность загрузки из файла отчета ""%1"" не реализована.';ru='Возможность загрузки из файла отчета ""%1"" не реализована.'"),
			ДанныеДляЗагрузки.ВидОтчета);
		Возврат Ложь;
	КонецЕсли;
		...
		Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ЭтоФайлВыгрузкиРеестраНДС(ОсновнойФайл.ИмяФайла) Тогда
			Если ТипЗнч(ДеревоДляЗагрузки) = Тип("Структура") Тогда
				...
				Если ВыбранноеДеревоДляЗагрузки = Неопределено Тогда
					СообщениеОбОшибке.Способ = "Предупреждение";
					СообщениеОбОшибке.ТекстСообщения = НСтр("ka='Не определена схема для загрузки реестра НДС из файла.';ru='Не определена схема для загрузки реестра НДС из файла.'");
					Возврат Ложь;
				КонецЕсли;
	...
	ИначеЕсли ДанныеДляЗагрузки.ТипДокумента = Перечисления.ТипыОтправляемыхДокументов.КомплектОтчетностиПФР Тогда
		...
		СообщениеОбОшибке.Способ = "Предупреждение";
		СообщениеОбОшибке.ТекстСообщения = НСтр("ka='Возможность загрузки из файла отчетов ПФР не поддерживается.';ru='Возможность загрузки из файла отчетов ПФР не поддерживается.'");
		Возврат Ложь;
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Не удалось заполнить схему данных из файла! %1';ru='Не удалось заполнить схему данных из файла! %1'"), ОписаниеОшибки));
		ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка данных регламентированного отчета';ru='Загрузка данных регламентированного отчета'"),
	...
	Исключение
		ОписаниеОшибки = ИнформацияОбОшибке().Описание;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Не удалось заполнить и сохранить документ отчета! %1';ru='Не удалось заполнить и сохранить документ отчета! %1'"), ОписаниеОшибки));
		ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка данных регламентированного отчета';ru='Загрузка данных регламентированного отчета'"),
			УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);
		Возврат Ложь;
	КонецПопытки;
...
Процедура ПолучитьДеревоДляЗагрузкиПоФормату(ФормаОтчета, ДеревоДляЗагрузки, Настройки, ВерсияФормата)
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Не удалось получить схему загрузки для версии данных %1. Используется схема по умолчанию.';ru='Не удалось получить схему загрузки для версии данных %1. Используется схема по умолчанию.'"), ВерсияФормата));
		ЗаписьЖурналаРегистрации(НСтр("ka='Получение схемы для загрузки';ru='Получение схемы для загрузки'"),
...
Процедура ЗаполнитьДаннымиДерево(ДеревоДляЗагрузки, ИмяФайла);
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='Ошибка чтения XML: %1.';ru='Ошибка чтения XML: %1.'"), ИнформацияОбОшибке().Описание);
	КонецПопытки;
	...
	Пока ОбъектЧтениеXML.Прочитать() Цикл
		...
		Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если НЕ ЭлементНайден Тогда
				ТекстСообщения = СтрШаблон(НСтр("ka='В схеме загрузки не найден узел с именем ""%1""!';ru='В схеме загрузки не найден узел с именем ""%1""!'"), ИмяТекущегоЭлемента);
...
Функция ВсеЭлементыИАтрибутыУзла(Узел, Атрибуты = Неопределено)
	...
	Для Каждого ПодчиненныйУзел Из Узел.Строки Цикл
		...
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ka='В схеме загрузки не распознан тип узла ""%1""!';ru='В схеме загрузки не распознан тип узла ""%1""!'"), ПодчиненныйУзел.Код);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьДаннымиСохраненныйДокумент(СохраненныйДок, ДеревоДляЗагрузки, ПараметрыОтчета, ДанныеДляЗагрузки, ТекстыПравилЗагрузки)
	...
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ka='Ошибка записи заполненного документа отчета: %1.';ru='Ошибка записи заполненного документа отчета: %1.'"),
...
Процедура ЗаполнитьПоказательДокументаИзУзлаДерева(Узел, ДанныеОтчета, ПараметрыОтчета, НомерСтраницы = 0, НомерСтроки = 0)
	...
	Если ТипЗнч(НомерСтраницы) = Тип("Массив") Тогда // раздел многоуровневый (многостраничный)
		...
		Если ДанныеОтчета.ДанныеМногоуровневыхРазделов = Неопределено Тогда
			ВызватьИсключение СтрШаблон(НСтр("ka='Не найдены данные многоуровневой части документа отчета. Ошибка в разделе: ""%1"".';ru='Не найдены данные многоуровневой части документа отчета. Ошибка в разделе: ""%1"".'"), Раздел);
		КонецЕсли;
		...
		Если ТипЗнч(МногоуровневыйРаздел) <> Тип("ДеревоЗначений") ИЛИ МногоуровневыйРаздел.Строки.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='В данных документа отчета не найден многоуровневый раздел ""%1"" для показателя ""%2"".';ru='В данных документа отчета не найден многоуровневый раздел ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
		...
		Если ВетвьДерева = МногоуровневыйРаздел Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='В данных документа отчета не найдена страница многоуровневого раздела ""%1"" для показателя ""%2"".';ru='В данных документа отчета не найдена страница многоуровневого раздела ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли НомерСтраницы > 0 Тогда // раздел многостраничный
		...
		Если ДанныеОтчета.ДанныеМногостраничныхРазделов = Неопределено Тогда
			ВызватьИсключение СтрШаблон(НСтр("ka='Не найдены данные многостраничной части документа отчета. Ошибка в разделе: ""%1"".';ru='Не найдены данные многостраничной части документа отчета. Ошибка в разделе: ""%1"".'"), Раздел);
		КонецЕсли;
		...
		Если ТипЗнч(МногостраничныйРаздел) <> Тип("ТаблицаЗначений") ИЛИ МногостраничныйРаздел.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='В данных документа отчета не найден многостраничный раздел ""%1"" для показателя ""%2"".';ru='В данных документа отчета не найден многостраничный раздел ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
	...
	Иначе // обычный (одностраничный) раздел
		...
		Если ТипЗнч(СтраницаРаздела) <> Тип("Структура") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='В данных документа отчета не найден раздел ""%1"" для показателя ""%2"".';ru='В данных документа отчета не найден раздел ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='В данных раздела ""%1"" документа отчета не найден показатель ""%2"" многострочной части.';ru='В данных раздела ""%1"" документа отчета не найден показатель ""%2"" многострочной части.'"),
				Раздел, Узел.Ключ));
		КонецЕсли;
	КонецЕсли;
		...
		Если НастройкиЗагрузки.Свойство("ИмяФайлаНастроек", ИмяФайлаНастроек) И ЗначениеЗаполнено(ИмяФайлаНастроек) Тогда
			Если ОбщегоНазначенияПовтИсп.РежимРаботыПрограммы().ЭтоАдминистраторПрограммы = Истина Тогда
				...
				Попытка
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ka='Используются обработчики из файла настроек загрузки ""%1"".';ru='Используются обработчики из файла настроек загрузки ""%1"".'"), ИмяФайлаНастроек));
				Исключение
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ka='Не найден файл настроек загрузки ""%1"": %2.';ru='Не найден файл настроек загрузки ""%1"": %2.'"), ИмяФайлаНастроек,
						РегламентированнаяОтчетностьКлиентСервер.СформироватьТекстСообщения(ИнформацияОбОшибке().Описание)));
...
Функция УзелУдовлетворяетВыражениюПредиката(Узел, МаксИндекс, Индекс, Знач Предикат)
	...
	Если НЕ ПустаяСтрока(Предикат) Тогда
		...
		Исключение
			...
			ОписаниеОшибки = ИнформацияОбОшибке().Описание;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Не удалось вычислить выражение XPath для дерева загрузки! %1';ru='Не удалось вычислить выражение XPath для дерева загрузки! %1'"), ОписаниеОшибки));
			ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка данных регламентированного отчета';ru='Загрузка данных регламентированного отчета'"),
				УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);
