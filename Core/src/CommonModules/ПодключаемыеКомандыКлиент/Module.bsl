...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяРаботаСФайлами Тогда
		...
		ТекстСообщения = НСтр("ka='Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.';ru='Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.'");
		...
		Если ИнформацияОДокументах.КоличествоНеПроведенных > 0 Тогда
			Если ИнформацияОДокументах.ЕстьПравоПроведенияХотяБыОдногоНепроведенного Тогда
				Если ИнформацияОДокументах.КоличествоНеПроведенных = 1 Тогда
					ТекстВопроса = НСтр("ka='Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?';ru='Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?'");
				Иначе
					ТекстВопроса = НСтр("ka='Для выполнения команды необходимо предварительно провести документы. Выполнить проведение документов и продолжить?';ru='Для выполнения команды необходимо предварительно провести документы. Выполнить проведение документов и продолжить?'");
				КонецЕсли;
			...
			Иначе
				Если ИнформацияОДокументах.КоличествоНеПроведенных = 1 Тогда
					ТекстПредупреждения = НСтр("ka='Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.';ru='Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.'");
				Иначе
					ТекстПредупреждения = НСтр("ka='Для выполнения команды необходимо предварительно провести документы. Недостаточно прав для проведения документов.';ru='Для выполнения команды необходимо предварительно провести документы. Недостаточно прав для проведения документов.'");
				КонецЕсли;
...
Процедура ПродолжитьВыполнениеКомандыПослеПодтверждениеПроведения(Ответ, Контекст) Экспорт
	...
	ШаблонСообщения = НСтр("ka='დოკუმენტი %1 არ არის გატარებული: %2';ru='Документ %1 не проведен: %2'");
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		ТекстДиалога = НСтр("ka='ვერ მოხერხდა ერთი ან რამდენიმე დოკუმენტის გატარება.';ru='Не удалось провести один или несколько документов.'");
		
		КнопкиДиалога = Новый СписокЗначений;
		Если Контекст.МассивСсылок.Количество() = 0 Тогда
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='ОК';ru='ОК'"));
		Иначе
			ТекстДиалога = ТекстДиалога + " " + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?'");
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='გაგრძელება';ru='Продолжить'"));
