...
"в настройках системы включить опцию использования электронных цифровых подписей.'");
	ИначеЕсли ВРег(ВидОперации) = "РАБОТАСБАНКАМИ" Тогда
			ТекстСообщения = НСтр("ka='ბანკებთან ედ-ის გაცვლის შესაძლებლობისთვის, აუცილებელია პროგრამის გამართვებში ბანკებთან პირდაპირი გაცვლის გამოყენების ოპციის ჩართვა.';ru='Для возможности обмена ЭД с банками необходимо "
"в настройках программы включить опцию использования прямого обмена с банками.'");
	Иначе
		ТекстСообщения = НСтр("ka='ოპერაცია ვერ შესრულდება. პროგრამის სავალდებულო გამართვები არ არის შესრულებული.';ru='Операция не может быть выполнена. Не выполнены необходимые настройки программы.';en='The operation can not be performed. Do not make the necessary settings of the program.'");
	КонецЕсли;
...
Процедура ПроверитьГотовностьИсточников(ДокументыМассив, ФормаИсточник = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ka='დოკუმენტ ""%1""-ის გამოტანა არ ხდება.';ru='Документ ""%1"" не выставляется.';en='The document ""%1"" is not exposed.'");
	...
	Иначе
		Если КоличествоНепроведенныхДокументов = 1 Тогда
			Текст = НСтр("ka='ედ-ის ჩამოყალიბებამდე საჭიროა დოკუმენტის გატარება.';ru='Перед формированием ЭД документ необходимо провести.';en='Before the formation of the ED document will be required.'");
		Иначе
			Текст = НСтр("ka='Перед формированием ЭД документы необходимо провести.';ru='Перед формированием ЭД документы необходимо провести.';en='Before the formation of ED documents necessary to carry out.'");
		КонецЕсли;
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	
	ШаблонСообщения = НСтр("ka='დოკუმენტი %1 არ არის გატარებული.';ru='Документ %1 не проведен.';en='% Document 1 is not carried out.'");
