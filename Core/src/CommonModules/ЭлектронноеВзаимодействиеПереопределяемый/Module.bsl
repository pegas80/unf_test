...
"в настройках программы включить опцию использования прямого обмена с банками.'");
	Иначе
		ТекстСообщения = НСтр("ka='ოპერაცია ვერ შესრულდება. პროგრამის სავალდებულო გამართვები არ არის შესრულებული.';ru='Операция не может быть выполнена. Не выполнены необходимые настройки программы.'");
	КонецЕсли;
...
Процедура ПроверитьГотовностьИсточников(ДокументыМассив, ФормаИсточник = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ka='დოკუმენტ ""%1""-ის გამოტანა არ ხდება.';ru='Документ ""%1"" не выставляется.'");
	...
	Иначе
		Если КоличествоНепроведенныхДокументов = 1 Тогда
			Текст = НСтр("ka='ედ-ის ჩამოყალიბებამდე საჭიროა დოკუმენტის გატარება.';ru='Перед формированием ЭД документ необходимо провести.'");
		Иначе
			Текст = НСтр("ka='Перед формированием ЭД документы необходимо провести.';ru='Перед формированием ЭД документы необходимо провести.'");
		КонецЕсли;
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	
	ШаблонСообщения = НСтр("ka='დოკუმენტი %1 არ არის გატარებული.';ru='Документ %1 не проведен.'");
