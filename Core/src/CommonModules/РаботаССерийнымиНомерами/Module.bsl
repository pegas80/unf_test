		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы[ИмяПоляНоменклатура].ИспользоватьСерийныеНомера Тогда
				...
				Если ОтборСерийныеНомера.Количество() <> СтрокаЗапасыКоличество Тогда
					...
					ТекстСообщения = НСтр("ka='Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.';ru='Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.'");
...
Процедура ВыполнитьКонтрольДублей(Отказ, Знач Запасы, Знач СерийныеНомера, Знач вхФорма, ИмяПоляКлючСвязи)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='სტრიქონ %НомерСтроки%-ში მითითებული სერიული ნომერი ""%СерийныйНомер% მითითებულია განმეორებით.';ru='Серийный номер ""%СерийныйНомер%"", указанный в строке %НомерСтроки%, указан повторно.'"
			);
	...
	Если ИспользоватьСерийныеНомераОстатки() = Истина Тогда
		...
		Если Объект.Номенклатура.ИспользоватьСерийныеНомера Тогда
			...
			Если Объект.СерийныеНомера.Количество() <> ОбъектКоличество Тогда
				ТекстСообщения = НСтр("ka='სერიული ნომრების რიცხვი განსხვადება დოკუმენტში არსებული რაოდენობისგან.';ru='Число серийных номеров отличается от количества в документе.'");
				ТекстСообщения = ТекстСообщения + НСтр("ka=' სერიული ნომერი - %ЧислоНомеров%, საჭიროა %КоличествоВСтроке%';ru=' Серийных номеров - %ЧислоНомеров%, нужно %КоличествоВСтроке%'");
