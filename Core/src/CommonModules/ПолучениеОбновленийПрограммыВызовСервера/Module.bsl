...
"<br />Используемая сейчас версия: %3'"),
		?(ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена,
			НСтр("ka='აუცილებელი';ru='необходимо'"),
			НСтр("ka='რეკომუნდებულია';ru='рекомендуется'")),
...
Функция НачатьПроверкуНаличияОбновления() Экспорт
	...
	Попытка
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить(
			...
			ПараметрыФЗ,
			,
			НСтр("ka='განახლებების არსებობის შემოწმება';ru='Проверка наличия обновлений'"));
	Исключение
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='განახლებების არსებობის ფონური შემოწმების შეცდომა. ფონური დავალების შესრულება ვერ მოხერხდა. %1';ru='Ошибка фоновой проверки наличия обновлений. Не удалось выполнить фоновое задание. %1'"),
...
Функция РезультатЗаданияПроверкиНаличияОбновлений(Знач ИДЗадания) Экспорт
	...
	Исключение
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='განახლებების არსებობის ფონური შემოწმება. შეცდომა დავალებაზე მიმართვისას. %1';ru='Фоновая проверка наличия обновлений. Ошибка при обращении к заданию. %1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат -1;
	КонецПопытки;
	...
	Если Задание = Неопределено Тогда
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='განახლებების არსებობის ფონური შემოწმება. აღნიშნული იდენტიფიკატორის დავალება ვერ მოიძებნა.';ru='Фоновая проверка наличия обновлений. Задание с указанным идентификатором не найдено.'"));
		Возврат -1;
	КонецЕсли;
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Завершено Тогда
		...
		Исключение
			ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='განახლებების არსებობის ფონური შემოწმება. შეცდომა დავალების შესრულების შედეგის მიღებისას. %1';ru='Фоновая проверка наличия обновлений. Ошибка при получении результата выполнения задания. %1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
			Возврат -1;
		КонецПопытки;
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='განახლებების არსებობის ფონური შემოწმება. დავალება დასრულებულია ავარიულად. %1';ru='Фоновая проверка наличия обновлений. Задание завершено аварийно. %1'"),
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='განახლებების არსებობის ფონური შემოწმება. დავალება გაუქმებულია ადმინისტრატორის მიერ.';ru='Фоновая проверка наличия обновлений. Задание отменено администратором.'"));
		Возврат -1;
