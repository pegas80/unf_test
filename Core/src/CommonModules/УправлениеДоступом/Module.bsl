...
Функция ЕстьПраво(Право, СсылкаНаОбъект, Пользователь = Неопределено) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ცხრილ ""%1""-სთვის შესაძლებელი უფლებების აღწერილობა ვერ მოიძებნა';ru='Не найдено описание возможных прав для таблицы ""%1""'"),
	...
	Если ОписаниеПрава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ცხრილ ""%2""-სთვის უფლება ""%1""-ის აღწერილობა ვერ მოიძებნა';ru='Не найдено описание права ""%1"" для таблицы ""%2""'"),
...
"доступа у реквизита ГруппаДоступа.'"),
	...
	Если ГруппыЗначенийДляИзменения.Количество() = 0
	   И СозданиеНового Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დასამატებლად საჭიროა ნებადართული ""%1"".';ru='Для добавления требуются разрешенные ""%1"".'"),
...
//	ОписаниеПрофиля.Наименование  = НСтр("ka='მომხმარებელი';ru='Пользователь'", Метаданные.ОсновнойЯзык.КодЯзыка);
