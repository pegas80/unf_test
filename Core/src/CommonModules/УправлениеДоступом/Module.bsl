...
"может быть зависимость только от права Чтения.';ka='შეცდომა მოდულ ""დაშვებისმართვა""-ს ფუნქცია ""არისროლი""-ში."
"დაშვების მნიშვნელობების კრებულში მითითებულია დაშვების ტიპი ცხრილის ""ცვლილებისუფლება"" იდენტიფიკატორ ""%1""-ით."
"როლის შემოწმების შეზღუდვაში (როგორც დამატებითი უფლების) შეიძლება იყოს დამოკიდებულება მხოლოდ'"),
...
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';ka='შეცდომა მოდულ დაშვებისმართვა-ს ფუნქცია არისროლი-ში."
"დაშვების მნიშვნელობების კრებული დაშვების ცნობილ სახეობა ""%1""-ს შეიცავს, რომლის მითითება არ არის საჭირო."
""
"მისათითებელია მხოლოდ დაშვების სპეციალური სახეობები ""წაკითხვისუფლება"", ""შეცვლისუფლება"", თუ ისინი გამოიყენება.'"),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
"Набор значений доступа содержит неизвестный вид доступа ""%1"".';ka='შეცდომა მოდულ წვდომისმართვა-ს ფუნქცია არისროლი-ში. დაშვების მნიშვნელობების კრებული დაშვების უცნობ სახეობა ""%1""-ს შეიცავს.'"),
				Строка.ВидДоступа);
		КонецЕсли;
...
Функция ЕстьПраво(Право, СсылкаНаОбъект, Пользователь = Неопределено) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ცხრილ ""%1""-სთვის შესაძლებელი უფლებების აღწერილობა ვერ მოიძებნა';ru='Не найдено описание возможных прав для таблицы ""%1""';en='We found a description of possible rights for the table ""%1""'"),
	...
	Если ОписаниеПрава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ცხრილ ""%2""-სთვის უფლება ""%1""-ის აღწერილობა ვერ მოიძებნა';ru='Не найдено описание права ""%1"" для таблицы ""%2""';en='We found the right description, ""%1"" on table ""%2""'"),
...
"общего модуля УправлениеДоступом.';ka='შეცდომა საერთო მოდულ ქვდომისმართვა-ს პროცედურა დაშვებისმნიშვნელობისფორმისშექმნისას-ში.'");
...
"%1';ka='პარამეტრი ახლისშექმნა არ არის მითითებული, ხოლო ფორმა ""ობიექტი.ბმული""-ს რეკვიზიტიდან ავტომატური შევსება მიუწვდომელია შემდეგი მიზეზით:"
...
"из реквизита формы ""Объект.Ссылка"" недоступно по причине:"
"%1';ka='პარამეტრი მნიშვნელობისტიპი არ არის მითითებული, ხოლო ფორმა ""ობიექტი.ბმული""-ის რეკვიზიტიდან ავტომატური შევსება მიუწვდომელია შემდეგი მიზეზით:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
...
"%1';ka='პარამეტრი ""რეკვიზიტი"" არ არის მითითებული, ხოლო ფორმა ""ობიექტი.დაშვებისჯგუფი""-ს რეკვიზიტიდან ავტომატური შევსება მიუწვდომელია შემდეგი მიზეზით:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
...
"%2';ka='პარამეტრ ""რეკვიზიტი""-ში მითითებული ფორმა ""%1""-ის რეკვიზიტის მნიშვნელობის მიღება ვერ მოხერხდა შემდეგი მიზეზით:"
...
"%2'"), Реквизит, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если ТипЗнч(ГруппаЗначенийДоступа) <> СвойстваГрупп.Тип Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='""%1"" ტიპის დაშვების მნიშვნელობებისთვის გამოიყენება დაშვების სახეობა ""%2"" მნიშვნელობების ტიპით ""%3"", რომელიც მიცემულია ხელახლა განსაზღვრებად მოდულში."
"მაგრამ ეს ტიპი არ ემთხვევა ტიპ ""%4""-ს რეკვიზიტ დაშვებისჯგუფი-თან დაშვების მნიშვნელობის ფორმაში.';ru='Для значений доступа типа ""%1"""
...
"доступа у реквизита ГруппаДоступа.'"),
	...
	Если ГруппыЗначенийДляИзменения.Количество() = 0
	   И СозданиеНового Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დასამატებლად საჭიროა ნებადართული ""%1"".';ru='Для добавления требуются разрешенные ""%1"".';en='To add a permission is required ""%1"".'"),
...
"общего модуля УправлениеДоступом.';ka='Ошибка в процедуре ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа"
...
"Значение параметра Список ""%1"" не является динамическим списком.';ka='საერთო მოდულ დაშვებისმართვა-ს პროცედურა დინამიკურისიისშერჩევებისგამართვა-ს მოხმობის შეცდომა."
"პარამეტრ სია ""%1""-ის მნიშვნელობა დინამიკურ სიას არ წარმოადგენს.'"), Строка(Список));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не указана основная таблица.';ka='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не указана основная таблица.'");
	...
	Если Не Список.ПроизвольныйЗапрос Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не установлен флажок ""Произвольный запрос"".';ka='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
...
"""%1"".';ka='საერთო მოდულ წვდომისმართვა-ს პროცედურა დინამიკურისიისშერჩევებისგამართვა-ს მოხმობის შეცდომა."
"გადაცემულ დინამიკურ სიაზე ძირითადი ცხრილის ფსევდონიმი ვერ მოიძებნა"
...
"""Записать зависимые наборы значений доступа"".';ka='Неверные параметры."
"Тип объекта ""%1"""
"не найден ни в одной из подписок на события"
"""Записать наборы значений доступа"","
"""Записать зависимые наборы значений доступа"".'"),
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ""%2"""
"დაშვების მნიშვნელობების კრებული არ ჩამოაყალიბა შემდეგი მიზეზით:"
"%3';ru='%1 ""%2"""
...
"%3'"),
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ""%2"" ჩამოაყალიბა დაშვების მნიშვნელობების ცარიელი კრებული.';ru='%1 ""%2"""
...
//	ОписаниеПрофиля.Наименование  = НСтр("ka='მომხმარებელი';ru='Пользователь';en='User'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
"%1.';ka='Ошибка в параметрах процедуры ПереименоватьПравоВНастройкахПравОбъектов"
"общего модуля УправлениеДоступом."
""
"После обновления будут повторяться настройки следующих новых имен прав:"
"%1.'"),
...
""
"Указывать требуется только специальные виды доступа"
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';ka='Объект ""%1"" сформировал набор значений доступа,"
"содержащий известный вид доступа ""%2"", который не требуется указывать."
""
"Указывать требуется только специальные виды доступа"
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.'"),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект ""%1"" сформировал набор значений доступа,"
"содержащий неизвестный вид доступа ""%2"".';ka='Объект ""%1"" сформировал набор значений доступа,"
...
Функция СвойстваГруппЗначенияДоступа(ТипЗначенияДоступа, ЗаголовокОшибки)
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ტიპის დაშვების მნიშვნელობებისთვის დაშვების მნიშვნელობების ჯგუფები არ გამოიყენება.';ru='Для значений доступа типа ""%1"""
