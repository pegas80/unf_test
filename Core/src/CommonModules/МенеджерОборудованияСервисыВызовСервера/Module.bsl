...
Процедура ПометитьОбработкуДокумента(Отказ, ПолноеИмяФайла, ТипДокумента, Сообщение, ВерсияФорматаОбмена) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='ფაილ-ანგარიშგების წაკითხვისას შეცდომა მოხდა. ანგარიშგების ფორმატი გაცვლის ფორმატის ვერსიას არ შეესაბამება.';ru='При чтении файла-отчета произошла ошибка. Формат отчета не соответствует версии формата обмена.';en='When reading the report file error occurred. Report format does not match the version of the exchange format.'") 
	...
	Если ЗначениеЗаполнено(Данные.Обработан) Тогда
		Отказ = Истина;
		Сообщение = НСтр("ka='ანგარიშგება უწინ დამუშავდა. მონიშვნა არ განხორციელებულა.';ru='Отчет был обработан ранее. Пометка не производилась.';en='The report has been treated previously. Mark was made.'");
	Иначе
...
Процедура ЗаполнитьСтруктуруДокумента(СтруктураДокумента, ТипДокумента, XMLТекст, ВерсияФорматаОбмена) Экспорт
	...
	Если ТипДокумента = "SalesReport" Тогда
		...
		Если ВерсияФорматаФайла <> ВерсияФорматаОбмена Тогда
			ТекстСообщения = НСтр("ka='ფაილ-ანგარიშგების წაკითხვისას შეცდომა მოხდა. ანგარიშგების ფორმატი გაცვლის ფორმატის ვერსიას არ შეესაბამება.';ru='При чтении файла-отчета произошла ошибка. Формат отчета не соответствует версии формата обмена.';en='When reading the report file error occurred. Report format does not match the version of the exchange format.'") + Символы.ПС + ОписаниеОшибки();
			СтруктураДокумента.Отказ = Истина;
			СтруктураДокумента.СообщениеОбОшибке = ТекстСообщения;
			Возврат;
		КонецЕсли;
