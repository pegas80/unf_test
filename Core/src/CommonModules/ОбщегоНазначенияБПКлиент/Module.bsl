...
Процедура УстановитьОсновнуюОрганизацию(ОсновнаяОрганизация, ПоказыватьОповещение = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ОсновнаяОрганизация) И ПоказыватьОповещение Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Установлена основная организация:';en='The basic organization:'"),
...
Функция ПроверитьВозможностьПечатиЧека(Форма) Экспорт
	...
	Если НЕ Форма.Объект.Проведен
		...
		ПоказатьПредупреждение( , НСтр("ru='Операция возможна только после проведения документа';en='Operation is possible only after the document'"));
		ПечататьЧек = Ложь;
...
Процедура ПредупредитьОНеобходимостиОбновленияПрограммы(ПараметрыРаботыКлиента) Экспорт
	...
	Если ПараметрыРаботыКлиента.ЭтоАдминистраторСистемы И ПараметрыРаботыКлиента.ЭтоГлавныйУзел Тогда
		...
		#Если ВебКлиент Тогда
			Параметры.Вставить("ТекстСообщения", ТекстСообщения + НСтр("ru='Обновление конфигурации недоступно в веб-клиенте, рекомендуется открыть программу в тонком клиенте.';en='Updating the configuration is not available in the web client, it is recommended to open the program to the thin client.'"));
			ОткрытьФорму("ОбщаяФорма.НерекомендуемаяВерсияКонфигурации", Параметры);
			Возврат;
		#КонецЕсли
		
		Параметры.Вставить("ТекстСообщения", ТекстСообщения + НСтр("ru='Вы можете обновить конфигурацию прямо сейчас.';en='You can update the configuration now.'"));
		Параметры.Вставить("ОткрытьОбновлениеПрограммы", Истина);
	Иначе
		Параметры.Вставить("ТекстСообщения", ТекстСообщения + НСтр("ru='Для обновления конфигурации обратитесь к ответственному за обновление.';en='To update the configuration, refer to the obligation to update.'"));
	КонецЕсли;
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Или Не ПараметрыПриложения[ИмяПараметра] Тогда
		...
		Если Не РеквизитыОрганизацииЗаполнены Тогда
			...
			Если Результат.ДоступноРедактированиеРеквизитовОрганизации Тогда
				...
				СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Заполнить сейчас';en='Populate Now'"));
				СписокКнопок.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Позже';en='Later'"));
				ТекстВопроса = НСтр("ru='Для продолжения работы заполните свои реквизиты';en='To continue fill in your details'");
				
			Иначе
				
				СписокКнопок.Добавить(КодВозвратаДиалога.ОК);
				ТекстВопроса = НСтр("ru='Не заполнены реквизиты организации. Обратитесь к администратору.';en='Not filled with details of the organization. Contact your administrator.'");
...
Процедура ОповеститьОбИзмененииСтатусовДокументов(Форма, СписокДокументов, СтатусДокумента, ДополнительныйСтатус = Неопределено) Экспорт
	...
	Иначе
		...
		ПредставлениеОписания = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			Нстр("ru=';1% документ;;1% документа;1% документов;1% документа';en='1% 1% ;; paper document; 1% of the documents, the document 1%'"), КоличествоДокументов);
...
Процедура ОткрытьФормуНастройкиСинхронизацииСКалендаремGoogle() Экспорт
	...
	//	ТекстВопроса = НСтр("ru='Для возможности синхронизации задач с календарем Google необходимо сначала включить регулярное обновление списка задач в программе, продолжить?';en='To be able to sync tasks with Google calendar, you must first enable regular updating of the list of tasks in the program to continue?'");
	//	
	...
	//	ПоказатьПредупреждение(, НСтр("ru='Для возможности синхронизации задач с календарем Google необходимо сначала включить регулярное обновление списка задач в программе, обратитесь для этого к администратору.';en='To be able to sync tasks with Google calendar, you must first enable regular updating of the list of tasks in the program, contact your administrator to do it.'"));
...
Функция ВыделенныеДокументы(Форма)
	...
	Если СписокДокументов.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Команда не может быть выполнена для указанного объекта';en='Command can not be executed for the indicated object'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПоказатьСообщениеОСтарыхРеквизитахНалоговой() Экспорт
	...
	ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ru='Изменились реквизиты уплаты налогов и взносов';en='Changed details of taxes and fees'");
