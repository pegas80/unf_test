...
Функция ВерсииИнтерфейсаТекущейИБ(Знач ИмяИнтерфейса) Экспорт
	...
	Если Результат = Неопределено Или Результат.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიმდინარე საინფორმაციო ბაზის მიერ ინტერფეისი %1 მხარდაჭერილი არ არის.';ru='Текущая информационная база не поддерживает интерфейс %1.';en='Current information base does not support the interface% 1.'"), ИмяИнтерфейса);
	Иначе
		Возврат Результат;
	КонецЕсли;
