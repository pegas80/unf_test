		...
		Если Движение.Количество() > 0 Тогда
			...
			Если НЕ ПравоДоступа("Изменение", МетаданныеНабора) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Нарушение прав доступа: %1';en='An access violation:% 1'"),
...
Функция ПолучитьПаспортныеДанныеСтрокой(Организация, ФизЛицо) Экспорт
	...
	//Иначе
	//	Возврат НСтр("ru='Отсутствуют данные об удостоверении личности.';en='There are no data on the identity card.'");
	//КонецЕсли;
...
Функция ДанныеФизЛица(Организация, ФизЛицо, ДатаСреза, ФИОКратко = Истина) Экспорт
	...
	Возврат Неопределено; //УчетЗарплаты.ДанныеФизическихЛиц(Организация, ФизЛицо, ДатаСреза, ФИОКратко);
...
Функция ДанныеФизЛиц(Организация, ФизЛицо, ДатаСреза) Экспорт
	...
	Возврат Неопределено; //УчетЗарплаты.ДанныеФизическихЛиц(Организация, ФизЛицо, ДатаСреза, Истина, Истина);
...
Функция ПредставлениеДокументаПриПроведении(Документ) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперации", МетаданныеДокумента) Тогда
		ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Проведение документа: %1 (%2)';en='Conducting document: 1% (2%)'"), СокрЛП(Документ), Документ.ВидОперации);
	Иначе
		ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Проведение документа: %1';en='Conducting document% 1'"), СокрЛП(Документ));
	КонецЕсли;
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	Для каждого ИмяПараметра Из СписокПараметров Цикл
		...
		Иначе
			...
			ТекстСообщения = НСТр("ru='Не удалось определить значение параметра организации:';en='Unable to determine the value of the organization:'") + " " + ИмяПараметра;
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);

			ЗаписьЖурналаРегистрации(
				НСтр("ru='Формирование сведений об организации';en='Formation of company information'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,
...
Функция ПроверитьВыводТабличногоДокумента(ТабДокумент, ВыводимыеОбласти, РезультатПриОшибке = Истина) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Проверка вывода на печать';en='Check the print output'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПодобратьВероятнуюДолжностьОтветственногоЛица(ОтветственноеЛицо) Экспорт
	...
	Возврат Неопределено; //ОтветственныеЛицаБП.ПодобратьВероятнуюДолжностьОтветственногоЛица(ОтветственноеЛицо);
...
Процедура ЗаполнитьСписокОрганизаций(ЭлементПолеОрганизация, СоответствиеОрганизаций) Экспорт
	...
	|	ИЗ
	|		Справочник.Организации КАК Организации) КАК НаборОрганизаций
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВключатьОбособленныеПодразделения Тогда
			ОрганизацияПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 с обособленными подразделениями';en='1% isolated divisions'"),
...
Функция ДоступенПростойИнтерфейс() Экспорт
	...
	Возврат ОбщегоНазначенияБП.ДоступенПростойИнтерфейс();
...
Процедура УстановитьСтандартныйИнтерфейсДляВсехпользователей() Экспорт
	...
	Если ВсеПользователи.Количество() > 0 Тогда 
		Для Каждого ПользовательИБ Из ВсеПользователи Цикл
			УстановитьСтандартныйИнтерфейс(ПользовательИБ.Имя);
		КонецЦикла;
	Иначе
...
Функция СобытиеОбработкаДвижений()
	...
	Возврат НСТр("ru='Обработка движений';en='Processing movements'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьТекстВопросаПриУдаленииСубконто(Знач СтруктураПараметровУчета, Знач СнятьПризнакУчетаПоПодразделениям = Ложь) Экспорт
	...
	ВидыДействий.Вставить("Действие", НСтр("ru='Будут удалены следующие субконто: %1';en='subkonto following are removed: 1%'"));
	ВидыДействий.Вставить("Количественный", НСтр("ru='Будут очищены количественные обороты по следующим субконто: %1';en='They will be cleared in the following quantitative Turnover subkonto: 1%'"));
	ВидыДействий.Вставить("Суммовой", НСтр("ru='Будут очищены суммовые обороты по следующим субконто: %1';en='Are cleared accrual turnovers subkonto follows: 1%'"));
	ВидыДействий.Вставить("ТолькоОбороты", НСтр("ru='Будут очищены остатки по следующим субконто: %1';en='balances following subkonto are cleared% 1'"));
	ВидыДействий.Вставить("Валютный", НСтр("ru='Будут очищены валютные обороты по следующим субконто: %1';en='They are cleared for the following momentum exchange subkonto: 1%'"));
...
Процедура СообщитьОЦиклическойСсылкеВОбъекте(МетаданныеОбъекта, ЭлементДанных)
	...
	Если ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) Тогда
		ВидОбъекта = НСтр("ru='В справочнике';en='The handbook'");
	ИначеЕсли ОбщегоНазначения.ЭтоПланВидовХарактеристик(МетаданныеОбъекта) Тогда
		ВидОбъекта = НСтр("ru='В плане видов характеристик';en='In terms of characteristics of the species'");
	КонецЕсли;
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 %2 элемент ""%3"" в поле ""%4"" содержит циклическую ссылку на самого себя. Необходимо указать правильную группу.';en='% 1% 2 element ""3%"" in the ""4%"" contains a cyclic reference to itself. You must specify the correct group.'"),
		ВидОбъекта,
		МетаданныеОбъекта.Синоним,
		ЭлементДанных,
		МетаданныеОбъекта.СтандартныеРеквизиты.Родитель.Синоним);
	ЗаписьЖурналаРегистрации(
		НСтр("ru='ЦиклическаяСсылка';en='TsiklicheskayaSsylka'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УстановитьИнтерфейсТакси() Экспорт
