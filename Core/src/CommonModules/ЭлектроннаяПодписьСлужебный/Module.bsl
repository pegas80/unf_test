...
Функция МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено) Экспорт
	...
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე მონაცემთა ხელმოწერა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось подписать данные на сервере %1 по причине:';en='Not able to sign data on the server% 1 for a reason:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე ხელმოწერის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить подпись на сервере %1 по причине:';en='Unable to verify the signature on the server% 1 for a reason:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე მონაცემთა დაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось зашифровать данные на сервере %1 по причине:';en='Failed to encrypt the data on the server% 1 for a reason:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე მონაცემთა გაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось расшифровать данные на сервере %1 по причине:';en='Not able to decipher the data on the server% 1 for a reason:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე სერთიფიკატის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить сертификат на сервере %1 по причине:';en='Unable to verify the certificate on the server% 1 for a reason:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე სერთიფიკატების მიღება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить сертификаты на сервере %1 по причине:';en='Failed to get the certificates on the server% 1 for a reason:'");
...
"პარამეტრ ოპერაციას არასწორი მნიშვნელობა ""%1"".'"), Операция);
	...
	Иначе
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось выполнить операцию на сервере %1 по причине:';en='Unable to perform the operation on the server% 1 for a reason:'");
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Программа) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='პროგრამა %1 არ არის ხელმისაწვდომი ან არ არის დაყენებული.';ru='Программа %1 не доступна или не установлена.';en='% 1 program is not available or is not installed.'"), Программа);
	Иначе
		ТекстОшибки = НСтр("ka='არცერთი პროგრამა არ არის ხელმისაწვდომი ან არ არის დაყენებული.';ru='Ни одна из программ не доступна или не установлена.';en='Neither of the programs is not available or is not installed.'");
	КонецЕсли;
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.';en='Contact your administrator.'");
	КонецЕсли;
...
"%1"
"';ka='Сертификат не найден на сервере по причине:"
"%1"
"'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ka='სერთიფიკატი სერვერზე ვერ მოიძებნა.';ru='Сертификат не найден на сервере.';en='The certificate is not found on the server.'");
	КонецЕсли;
		
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.';en='Contact your administrator.'")
	КонецЕсли;
...
Процедура ПроверитьУникальностьПредставления(Представление, СертификатСсылка, Поле, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстСообщения = НСтр("ka='ასეთი წარდგენის სერთიფიკატი უკვე არსებობს.';ru='Сертификат с таким представлением уже существует.';en='Certificate with representation already exists.'");
...
Функция СертификатИзДвоичныхДанных(ДанныеСертификата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронная подпись.Удаление временного файла';ru='Электронная подпись.Удаление временного файла';en='Electronic podpis.Udalenie temporary file'",
...
Функция ПроверитьПодпись(Знач КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии) Экспорт
	...
	Иначе
		...
		Если ПодписьВерна Тогда
			ВызватьИсключение НСтр("ka='Подпись не верна (SignatureValue корректно, отличается DigestValue).';ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';en='Signature is not true (SignatureValue correctly characterized DigestValue).'")
		Иначе
			ВызватьИсключение НСтр("ka='Подпись не верна (SignatureValue не корректно.';ru='Подпись не верна (SignatureValue не корректно.';en='Signature is not true (SignatureValue not correct.'");
		КонецЕсли;
...
Функция ОбъектВнешнейКомпонентыXMLDSig()
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Не удалось подключить внешнюю компоненту XMLDSig.';ru='Не удалось подключить внешнюю компоненту XMLDSig.';en='Failed to connect an external component XMLDSig.'");
	КонецЕсли;
...
Функция ТипКриптопровайдера(МенеджерКриптографии)
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось определить тип криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1';en='Unable to determine the type of CSP 1%'"), ИмяКриптопровайдера);
	КонецЕсли;
...
Функция GetSignOIDFromCert(ОбъектКомпоненты, СертификатКриптографииBase64)
	...
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';en='Error calling the GetSignOIDFromCert components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';en='Error calling the GetSignOIDFromCert components XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция C14N(ОбъектКомпоненты, КонвертSOAP, XPath)
	...
	Попытка
		КаноникализированныйТекстXML = ОбъектКомпоненты.C14N(
			...
			XPath);
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода C14N компоненты XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.';en='Error calling the C14N components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если КаноникализированныйТекстXML = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода C14N компоненты XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.';en='Error calling the C14N components XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Hash(ОбъектКомпоненты, КаноникализированныйТекстXML, OIDАлгоритмаХеширования, ТипКриптопровайдера)
	...
	Попытка
		DigestValue = ОбъектКомпоненты.Hash(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Hash компоненты XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.';en='Error calling the Hash components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если DigestValue = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Hash компоненты XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.';en='Error calling the Hash components XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Sign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, СертификатКриптографии, ПарольДоступаКЗакрытомуКлючу)
	...
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Sign компоненты XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.';en='Error calling the ingredients Sign XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если SignatureValue = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Sign компоненты XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.';en='Error calling the ingredients Sign XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция VerifySign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, SignatureValue, СертификатКриптографииBase64, ТипКриптопровайдера)
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.VerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода VerifySign компоненты XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';en='Error calling the VerifySign components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если ПодписьВерна = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода VerifySign компоненты XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';en='Error calling the VerifySign components XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
"%2';ka='საინფორმაციო ბაზიდან სერთიფიკატ ""%1""-ის მონაცემთა მიღებისას შეცდომა წარმოიშვა:"
"%2'"),
			Сертификат,
