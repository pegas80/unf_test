...
Функция МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено) Экспорт
	...
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე მონაცემთა ხელმოწერა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось подписать данные на сервере %1 по причине:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე ხელმოწერის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить подпись на сервере %1 по причине:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე მონაცემთა დაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось зашифровать данные на сервере %1 по причине:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე მონაცემთა გაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось расшифровать данные на сервере %1 по причине:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე სერთიფიკატის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить сертификат на сервере %1 по причине:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე სერთიფიკატების მიღება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить сертификаты на сервере %1 по причине:'");
...
"პარამეტრ ოპერაციას არასწორი მნიშვნელობა ""%1"".'"), Операция);
	...
	Иначе
		ЗаголовокОшибки = НСтр("ka='სერვერ %1-ზე ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось выполнить операцию на сервере %1 по причине:'");
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Программа) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='პროგრამა %1 არ არის ხელმისაწვდომი ან არ არის დაყენებული.';ru='Программа %1 не доступна или не установлена.'"), Программа);
	Иначе
		ТекстОшибки = НСтр("ka='არცერთი პროგრამა არ არის ხელმისაწვდომი ან არ არის დაყენებული.';ru='Ни одна из программ не доступна или не установлена.'");
	КонецЕсли;
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.'");
	КонецЕсли;
...
"'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ka='სერთიფიკატი სერვერზე ვერ მოიძებნა.';ru='Сертификат не найден на сервере.'");
	КонецЕсли;
		
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.'")
	КонецЕсли;
...
Процедура ПроверитьУникальностьПредставления(Представление, СертификатСсылка, Поле, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстСообщения = НСтр("ka='ასეთი წარდგენის სერთიფიკატი უკვე არსებობს.';ru='Сертификат с таким представлением уже существует.'");
...
Функция СертификатИзДвоичныхДанных(ДанныеСертификата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронная подпись.Удаление временного файла';ru='Электронная подпись.Удаление временного файла'",
...
Функция ПроверитьПодпись(Знач КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии) Экспорт
	...
	Иначе
		...
		Если ПодписьВерна Тогда
			ВызватьИсключение НСтр("ka='Подпись не верна (SignatureValue корректно, отличается DigestValue).';ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).'")
		Иначе
			ВызватьИсключение НСтр("ka='Подпись не верна (SignatureValue не корректно.';ru='Подпись не верна (SignatureValue не корректно.'");
		КонецЕсли;
...
Функция ОбъектВнешнейКомпонентыXMLDSig()
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Не удалось подключить внешнюю компоненту XMLDSig.';ru='Не удалось подключить внешнюю компоненту XMLDSig.'");
	КонецЕсли;
...
Функция ТипКриптопровайдера(МенеджерКриптографии)
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось определить тип криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1'"), ИмяКриптопровайдера);
	КонецЕсли;
...
Функция GetSignOIDFromCert(ОбъектКомпоненты, СертификатКриптографииBase64)
	...
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция C14N(ОбъектКомпоненты, КонвертSOAP, XPath)
	...
	Попытка
		КаноникализированныйТекстXML = ОбъектКомпоненты.C14N(
			...
			XPath);
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода C14N компоненты XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если КаноникализированныйТекстXML = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода C14N компоненты XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Hash(ОбъектКомпоненты, КаноникализированныйТекстXML, OIDАлгоритмаХеширования, ТипКриптопровайдера)
	...
	Попытка
		DigestValue = ОбъектКомпоненты.Hash(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Hash компоненты XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если DigestValue = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Hash компоненты XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Sign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, СертификатКриптографии, ПарольДоступаКЗакрытомуКлючу)
	...
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Sign компоненты XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если SignatureValue = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода Sign компоненты XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция VerifySign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, SignatureValue, СертификатКриптографииBase64, ТипКриптопровайдера)
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.VerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ka='Ошибка вызова метода VerifySign компоненты XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если ПодписьВерна = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Ошибка вызова метода VerifySign компоненты XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
