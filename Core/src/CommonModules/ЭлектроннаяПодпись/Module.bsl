...
Процедура УдалитьСтрокуПодписи(ПодписанныйОбъект, ИндексСтроки)
	...
	Если ПодписанныйОбъект.ЭлектронныеПодписи.Количество() < ИндексСтроки + 1 Тогда
		ВызватьИсключение НСтр("ka='სტრიქონი წარწერებით ვერ იქნა ნაპოვნი.';ru='Строка с подписью не найдена.';en='String signature is not found.'");
	КонецЕсли;
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда 
		Если СтрокаТабличнойЧасти.УстановившийПодпись <> Пользователи.ТекущийПользователь() Тогда
			ВызватьИсключение НСтр("ka='ხელმოწერის წასაშლელად საკმარისი უფლება არ არის.';ru='Недостаточно прав на удаление подписи.';en='Not have permission to delete the signature.'");
		КонецЕсли;
	КонецЕсли;
