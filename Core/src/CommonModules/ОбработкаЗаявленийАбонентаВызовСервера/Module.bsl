...
Функция ОбновитьРеквизитыЗаявления(Документ, РеквизитыДокумента) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Обработка заявлений.Обновление статуса';ru='Обработка заявлений.Обновление статуса'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбменССерверомПолучитьОтвет(ДокументЗаявление, ЭтоВызовИзМастера, ТекстОшибокДляМастера)
	...
	Если Сервис = Неопределено Тогда
		Результат.Вставить("СтатусКомментарий", НСтр("ka='Не удалось соединиться с сервером';ru='Не удалось соединиться с сервером'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если кодРезультата = "0" Тогда
		...
		Иначе
			...
			СтатусКомментарий = НСтр("ka='При получении ответа возникла ошибка: ответ нечитаем';ru='При получении ответа возникла ошибка: ответ нечитаем'");
	...
	ИначеЕсли Кодрезультата = "1" Тогда
		...
		СтатусКомментарий = НСтр("ka='Заявление еще не обработано сервером, попробуйте позже.';ru='Заявление еще не обработано сервером, попробуйте позже.'");
		Результат.Вставить("СтатусКомментарий", СтатусКомментарий);
...
Функция РегистрацияРазобратьОтвет(ДокументЗаявление, РезультатОбмена, ЭтоВызовИзМастера, ТекстОшибокДляМастера)
		...
		Для Счетчик = 0 по Архив1.Элементы.Количество() - 1 Цикл
			...
			Если Архив1.Элементы[Счетчик].расширение = "bin" Тогда
				...
				Иначе
					...
					ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СообщитьИлиПередатьВМастерОшибку(
									НСтр("ka='განცხადება უარყოფილია';ru='Заявление отклонено'"),
									ЭтоВызовИзМастера,
									ТекстОшибокДляМастера, 
									не ЭтоВызовИзМастера);
	...
	Исключение
		...
		СтатусКомментарий = НСтр("ka='Ошибка. Не удалось разобрать ответ сервера.';ru='Ошибка. Не удалось разобрать ответ сервера.'");
