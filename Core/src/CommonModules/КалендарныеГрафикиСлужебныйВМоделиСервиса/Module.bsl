...
Процедура РаспространитьДанныеПроизводственныхКалендарейПоГрафикамРаботы(Знач УсловияОбновления, 
	...
	Для каждого ОбластьДанных Из ОбластиДляОбновления Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. კალენდარული გრაფიკები';ru='Поставляемые данные.Календарные графики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция НачалоЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. კალენდარული გრაფიკები';ru='Поставляемые данные.Календарные графики'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მონაცემთა ფაილის არასწორი ფორმატი. მოსალოდნელია ელემენტის დაწყება %1';ru='Неверный формат файла данных. Ожидается начало элемента %1'"), Имя));
		Возврат Ложь;
	КонецЕсли;
...
Функция КонецЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.КонецЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. კალენდარული გრაფიკები';ru='Поставляемые данные.Календарные графики'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მონაცემთა ფაილის ფორმატის არასწორი ფორმატი. მოსალოდნელის ელემენტის დასასრული %1';ru='Неверный формат файла данных. Ожидается конец элемента %1'"), Имя));
		Возврат Ложь;
	КонецЕсли;
