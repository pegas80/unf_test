...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეორე პარამეტრის არასწორი ტიპი რეკვიზიტები: %1';ru='Неверный тип второго параметра Реквизиты: %1'"),
...
Функция НайтиКонтрагентов(СтрокаПоиска, СписокКонтрагентов)
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		Возврат НСтр("ka='ძალიან ბევრი შედეგია, დააზუსტეთ მოთხოვნა:';ru='Слишком много результатов, уточните запрос.'");
	КонецЕсли;

	Если СписокПоиска.ПолноеКоличество() = 0 Тогда
		Возврат НСтр("ka='არაფერი მოიძებნა';ru='Ничего не найдено'");
	КонецЕсли;
	...
	Пока ЕстьСледующаяПорция Цикл
		...
		Для СчетчикЭлементов = 0 По КонечнаяПозиция Цикл
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.КонтактныеЛица Тогда
				...
				ШаблонОснования = НСтр("ka='მოიძებნა: საკონტაქტო პირი ""%1"" - %2';ru='Найдено: Контактное лицо ""%1"" - %2'");
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.ФизическиеЛица Тогда
				...
				Если ТаблицаНайденныхКонтрагентов <> Неопределено Тогда
					...
					Для каждого СтрокаТаблицы Из ТаблицаНайденныхКонтрагентов Цикл
						ШаблонОснования = НСтр("ka='მოიძებნა: საკონტაქტო პირის ""%2"" - %3 ფიზიკური პირი ""%1""';ru='Найдено: Физическое лицо ""%1"" контактного лица ""%2"" - %3'");
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.ДоговорыКонтрагентов Тогда
				...
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования =  НСтр("ka='მოიძებნა: ხელშეკრულება ""%1"" - %2';ru='Найдено: Договор ""%1"" - %2'");
				Основание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОснования, Элемент.Значение, Элемент.Описание);
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.БанковскиеСчета Тогда
				...
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования =  НСтр("ka='მოიძებნა: საბანკო ანგარიში ""%1"" - %2';ru='Найдено: Банковский счет ""%1"" - %2'");
				Основание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОснования, Элемент.Значение, Элемент.Описание);
	...
	Если ВРег(Метаданные.Имя) = ВРег("УправлениеНебольшойФирмойБазовая") Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.КоличествоОрганизаций > 2 Тогда
				Отказ = Истина;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ограничение базовой версии. В информационной базе могут быть введены только две организации.';ru='Ограничение базовой версии. В информационной базе могут быть введены только две организации.'"));
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьПаспортныеДанныеСтрокой(СтруктураДанных) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураДанных.ФизЛицо) Тогда
		Возврат НСтр("ka='პირადობის დამადასტურებელი მონაცემები არ არის.';ru='Отсутствуют данные об удостоверении личности.'");
	КонецЕсли; 
	...
	Иначе
		...
		Если НЕ (НЕ ЗначениеЗаполнено(ДатаВыдачи)
			...
			СтрокаПаспортныеДанные = НСтр("ka='%ВидДокумента% სერია: %Серия%, № %Номер%, გაცემულია; %ДатаВыдачи% წელს, %КемВыдан%; № ქვეგანყ. %НомерПодразделения%';ru='%ВидДокумента% Серия: %Серия%, № %Номер%, Выдан: %ДатаВыдачи% года, %КемВыдан%; № подр. %НомерПодразделения%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:'"));
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыОприходование(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='Попытка оприходовать номенклатуру с нулевой стоимостью.';ru='Попытка оприходовать номенклатуру с нулевой стоимостью.'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ШаблонСообщения = НСтр("ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,';ru='Номенклатура: %НоменклатураХарактеристикаПартия%,'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПоставщиками(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მომწოდებლებთან ანგარიშსწორებების დაფიქსირების შესაძლებლობა არ არის';ru='Нет возможности зафиксировать расчеты с поставщиками'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		...
		Иначе
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				Если ВыборкаЗаписей.СуммаНепогашеннойЗадолженности = 0 Тогда
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მომწოდებლისადმი დავალიანება უკვე მთლიანად გადახდილია.';ru='%ПредставлениеКонтрагента% - задолженность перед поставщиком по документу уже полностью оплачена.';en='% PredstavlenieKontragenta% - payable to suppliers have been paid in full through the document.'"
					);
				Иначе
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПокупателями(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მყიდველებთან ანგარიშსწორებების დაფიქსირების შესაძლებლობა არ არის';ru='Нет возможности зафиксировать расчеты с покупателями'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
...
"Остаток задолженности покупателя: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				Если ВыборкаЗаписей.СуммаНепогашенныхАвансов = 0 Тогда
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მყიდველისგან მიღებული ავანსები უკვე მთლიანად ჩათვლილია სასაქონლო დოკუმენტებში.';ru='%ПредставлениеКонтрагента% - полученные по документу авансы от покупателя уже полностью зачтены в товарных документах.';en='% PredstavlenieKontragenta% - advances received the document from the buyer is already fully utilized in commercial documents.'"
					);
				Иначе
...
"Остаток незачтенных авансов: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.'"
		...
		Иначе
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				Если ВыборкаЗаписей.СуммаНепогашеннойЗадолженности = 0 Тогда
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მყიდველის დავალიანება უკვე მთლიანად გადახდილია.';ru='%ПредставлениеКонтрагента% - задолженность покупателя по документу уже полностью оплачена.';en='% PredstavlenieKontragenta% - payable by the buyer to the document has already been paid in full.'"
					);
				Иначе
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыПереданныеВРемонте(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ШаблонСообщения = НСтр("ka='ნომენკლატურა: %НоменклатураХарактеристика%';ru='Номенклатура: %НоменклатураХарактеристика%'");
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	Если НЕ (ЮрФизЛицо.Метаданные().Реквизиты.Найти("ЮридическоеФизическоеЛицо") = Неопределено) Тогда
		...
		ИначеЕсли ИмяСправочника = "Организации" Тогда
			...
			Сведения.Свидетельство = НСтр("ka='მოწმობა';ru='свидетельство'") + " " + Данные.СвидетельствоСерияНомер + " " + НСтр("ka='(საიდან, ვისგან)';ru='от'") + " " + Формат(Данные.СвидетельствоДатаВыдачи, "ДЛФ=D");
...
Функция ПроверитьВыводТабличногоДокумента(ТабДокумент, ВыводимыеОбласти, РезультатПриОшибке = Истина)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСТр("ka='მიმდინარე პრინტერის შესახებ ინფორმაციის მიღება შეუძლებელია (შეიძლება სისტემაში არცერთი პრინტერი არ არის დაყენებული)';ru='Невозможно получить информацию о текущем принтере (возможно, в системе не установлено ни одного принтера)'"),
...
Процедура УстановитьТекстПроСчетФактуру(ФормаДокумента, Полученный = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(НайденныйСчетФактура) Тогда
		ТекстПроСчетФактуру = НСтр("ka='№ %Номер% %Дата% წლიდან';ru='№ %Номер% от %Дата% г.'");
...
Функция ПроверитьВыпискуСчетовФактур(ИсточникиПечати, ДокументРеализации, СообщенияПользователю, ВидОперации = Неопределено)
	...
	Если СчетФактураВыданный = Неопределено Тогда
		...
		Попытка 
			...
			ТекстСообщения = НСтр("ka='შედგენილია დაქვემდებარებული დოკუმენტი %2-ის ანგარიშფაქტურა # %1.';ru='Сформирован подчиненный документ Cчет-фактура № %1 от %2.'");
...
Процедура УстановитьСтандартныеНастройкиПодбора(ТекущийПользователь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекущийПользователь) Тогда
		...
		Нстр("ka='მიუთითებელია მომხმარებელი, რომლისთვის ხდება გადარჩევის გამართვების დაყენება სტანდარტულად.';ru='Неуказан пользователь, для которого устанавливаются настройки подбора по умолчанию.'")
		);
...
Процедура ЗаполнитьТемуТекстОтправки(ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм)
	...
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2 %3';ru='%1 %2 %3'"),
		ПредставлениеЗаголовкаДокумента,
		ПредставлениеДляКого,
		ПредставлениеОтКого
		);
...
Функция ОчиститьДанныеВБазе() Экспорт
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение(НСтр("ka='ოპერაციის შესასრულებლად უფლებები არასაკმარისია';ru='Не достаточно прав для выполнения операции'"));
	КонецЕсли;
	...
	Исключение
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მონოპოლიური რეჟიმის დაყენება ვერ მოხერხდა (%1)';ru='Не удалось установить монопольный режим (%1)'"),
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა წაშლა';ru='Удаление данных'"), 
...
Функция ПолучитьКалендарьПоПроизводственномуКалендарюРФ() Экспорт
	...
	Если ПроизводственныйКалендарьРФ = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='საქართველოს საწარმოო კალენდრის საფუძველზე ორგანიზაციებისთვის მუშაობის გრაფიკების მონაცემთა შევსება ვერ მოხერხდა.';ru='Неудалось заполнить данные граффиков работы для организации на основании производственного календаря РФ.'"), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыПоЭквайрингу(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Иначе
		ЗаголовокОшибки = НСтр("ka='Информация:';ru='Информация:'");
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.СуммаВалОстаток < 0 Или ВыборкаЗаписей.КомиссияВалОстаток < 0 Тогда
			Если ПоказатьСообщениеОбОшибки Тогда
				...
				ЗаголовокОшибки = НСтр("ka='შეცდომა:';ru='Ошибка:'");
				ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='საგადახდო ბარათებით ანგარიშსწორების დაფიქსირების შესაძლებლობა არ არის';ru='Нет возможности зафиксировать расчеты по платежным картам'");
				СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
			КонецЕсли;
...
Функция ОтказСнятьФункциональнаяУчетВалютныхОпераций() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ka='ბაზაში შემოღებულია ეროვნულისგან განსხვავებული ვალუტები. გამართვის შეცვლა შეუძლებელია!';ru='В базе заведены валюты, отличные от национальной. Изменение настройки невозможно!'");
	КонецЕсли;
...
Функция ОтказСнятьИспользоватьОбменССайтом() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ka='პროგრამაში გამართულია და გამოიყენება საიტებთან გაცვლა! გასწორების გამორთვა შეუძლებელია!';ru='В программе настроен и используется обмен с сайтами! Отключение настройки невозможно!'");
	КонецЕсли;
...
Функция ОтказИзменитьВалютаУчета() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстСообщения = НСтр("ka='ბაზაში არის მოძრაობები დაგროვების ""ჯამური"" რეგისტრებით! აღრიცხვის ვალუტის შეცვლა აკრძალულია!';ru='В базе есть движения по ""суммовым"" регистрам накопления! Изменение валюты учета запрещено!'");	
...
Функция ОтказСнятиеУчетПоНесколькимСкладам() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ძირითადისგან განსხვავებული საწყობები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются склады, отличные от основного! Снятие опции запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетВРазличныхЕдиницахИзмерения() Экспорт
	...
	Пока ВыборкаЕдиницыИзмерения.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='დანართებში დოკუმენტები შეყვანილია სამომხმარებლო საზომ ერთეულებში! ოპციის მოხსნა აკრძალულია!';ru='В приложении введены документы в пользовательских единицах измерения! Снятие опции запрещено!'");
			Прервать;
...
Функция ОтказСнятьФункциональнаяОпцияПередачаЗапасовНаОтветхранение() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""გადაცემული მარაგები"" შეიცავს ინფორმაციას საპასუხისმგებლო შენახვაზე გადაცემის შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче на ответхранение! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияПриемЗапасовНаОтветхранение() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""მიღებული მარაგები"" შეიცავს ინფორმაციას საპასუხისმგებლო შენახვაზე მიღების შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме на ответхранение! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьОрдерныйСклад() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის საორდერო საწყობები! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют ордерные склады! Снятие флага запрещено!'");
	...
	Если НЕ РезультатЗапросаЗапасыКПоступлениюНаСклады.Пустой()
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='ბაზაში არის მოძრაობები საორდერო საწყობის მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют движения по ордерному складу! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияРезервированиеЗапасов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზა შეიცავს ინფორმაციას რეზერვების შესახებ! ალმის მოხსნა აკრძალულია!';ru='В базе содержится информация о резервах! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетЦенКонтрагентов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის კონტრაგენტების ხელშეკრულებები შემომავალი ფასების დაყენებული სახეობებით! ალმის მოხსნა აკრძალულია!';ru='В базе содержатся договоры контрагентов с установлинными видами входящих цен! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетПоЯчейкам() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზა შეიცავს მოძრაობებს უჯრედების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе содержатся движения по ячейкам! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьХарактеристики() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები მახასიათებლების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть движения по характеристикам! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПартии() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები პარტიების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть движения по партиям! Снятие флага запрещено!'");
	...
	Если ПолучитьФункциональнуюОпцию("ПриемТоваровНаКомиссию") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ჩართულია ოპცია ""საკომისიოდ საქონლის მიღება"" (ნაწილი გაყიდვები)! ალმის მოხსნა აკრძალულია!';ru='Включена опция ""Прием товаров на комиссию"" (раздел Продажи)! Снятие флага запрещено!'");
	...
	Если ПолучитьФункциональнуюОпцию("ПереработкаДавальческогоСырья") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ოპცია ""გამცემის ნედლეულის გადამუშავება"" (ნაწილი წარმოება) ჩართულია! ალმის მოხსნა აკრძალულია!';ru='Включена опция ""Переработка давальческого сырья"" (раздел Производство)! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСерийныеНомера() Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ka='ბაზაში არის ნაშთი სერიული ნომრების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть остатки по серийным номерам! Снятие флага запрещено!'");
	КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияПередачаСырьяВПереработку() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""გადაცემული მარაგები"" შეიცავს ინფორმაციას გადამუშავებაზე გადაცემის შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче в переработку! Снятие флага запрещено!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСкидкиНаценки() Экспорт
	...
	Пока ВыборкаВидыСкидокНаценок.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ფასდაკლებათა, ფასნამატთა სახეობები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются виды скидок, наценок! Снятие опции запрещено!'");
			Прервать;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='დოკუმენტებში გამოიყენება ფასდაკლებები და ფასნამატები! ოპციის მოხსნა აკრძალულია!';ru='В документах используются скидки и наценки! Снятие опции запрещено!'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДисконтныеКарты") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ჩართულია ოპცია ""დისკონტური ბარათების გამოყენება""! ალმის მოხსნა აკრძალულია!';ru='Включена опция ""Использовать дисконтные карты""! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияПередачаТоваровНаКомиссию() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""გადაცემული მარაგები"" შეიცავს ინფორმაციას საკომისიოდ გადაცემის შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче на комиссию! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияПриемТоваровНаКомиссию() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""მიღებული მარაგები"" შეიცავს ინფორმაციას საკომისიოდ მიღების შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме на комиссию! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетРозничныхПродаж() Экспорт
	...
	Если ВыборкаЗапроса.Следующий()
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები ან ობიექტები, რომლებიც განეკუთვნება საცალო გაყიდვების ოპერაციების აღრიცხვას! ალმის მოხსნა აკრძალულია!';ru='В базе есть движения или объекты, относящиеся к учету операций розничных продаж! Снятие флага запрещено!'");
	...
	Пока ВыборкаНаправленияДеятельности.Следующий() Цикл
		...
		Если ВыборкаНаправленияДеятельности.Ссылка <> Справочники.НаправленияДеятельности.ОсновноеНаправление
			...
			Если ТаблицаСсылок.Количество() > 0 Тогда
				...
				ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ძირითადისგან განსხვავებული მოღვაწეობის მიართულებები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются направления деятельности, отличные от основного! Снятие опции запрещено!'");
				Прервать;
...
Функция ОтказСнятиеУчетПоНесколькимПодразделениям() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ძირითადისგან განსხვავებული ქვეგანყოფილებები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются подразделения, отличные от основного! Снятие опции запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетВнеоборотныхАктивов() Экспорт
	...
	Если Отказ Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები გრძელვადიანი აქტივების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют движения по внеоборотным активам! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуРаботы() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""შეკვეთა-განწესი""! ალამ ""სამუშაოები""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ - наряд""! Снятие флага ""Работы"" запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуПроизводство() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""საწარმოო შეკვეთა""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ на производство""! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""წარმოება""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Производство""! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""დანახარჯების განაწილება""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Распределение затрат""! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სანარდო განწესი""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Сдельный наряд""! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[12].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ხელფასის დარიცხვა"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანხარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Начисление зарплаты"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[13].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""გადასახადების დარიცხვა"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Начисление налогов"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[14].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ოპერაცია"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Операция"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!'");
	...
	Если НЕ МассивРезультатов[15].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სხვა დანახარჯები (ხარჯები)"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Прочие затраты (расходы)"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!'");
...
Функция ПроверитьДвиженияПоРегистрамПодсистемыПроизводство()
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის მოძრაობები რეგისტრ ""მარაგების"" მიხედვით, სადაც აღრიცხვის დათვლას გააჩნია ტიპი არაპირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют движения по регистру ""Запасы"", где счет учета имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияПереработкаДавальческогоСырья() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""მიღებული მარაგები"" შეიცავს ინფორმაციას გადამუშავებაზე მიღების შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме в переработку! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьТехоперации() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის ინფორმაცია სამუშაო ცენტრების ჩატვირთვის შესახებ ან ""სანარდო განწესის"" ტიპის დოკუმენტები! ალმის მოხსნა აკრძალულია!';ru='В базе присутствует информация о загрузке рабочих центров или документы вида ""Сдельный наряд""! Снятие флага запрещено!'");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ბაზაში არის ნომენკლატურა ტიპით ""ოპერაცია""! ალმის მოხსნა აკრძალულია!';ru='В базе присутствует номенклатура с типом ""Операция""! Снятие флага запрещено!'");
...
Функция ПроверитьДвиженияПоРегистрамПодсистемыЗарплата() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის მოძრაობები რეგისტრ ""დარცხვები და დაკავებების"" მიხედვით! ალამ ""ხელფასი""-ს აკრძალულია!';ru='В информационной базе присутствуют движения по регистру ""Начисления и удержания""! Снятие флага ""Зарплата"" запрещено!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის მოძრაობები რეგისტრ ""პერსონალთან ანგარიშსწორების"" მიხედვით! ალამ ""ხელფასის"" მოხსნა აკრძალულია!';ru='В информационной базе присутствуют движения по регистру ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуЗарплата() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ხელფასის დარიცხვა""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Начисление зарплаты""! Снятие флага ""Зарплата"" запрещено!'");
	...
	Если НЕ МассивРезультатов[6].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არიან თანამშრომლები დასაქმების ტიპით ""შეთავსება""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага ""Зарплата"" запрещено!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСовместительство() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არიან თანამშრომლები, რომელთა დასაქმების ტიპია ""შეთავსება""! ალმის მოხსნა აკრძალულია!';ru='В базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияВестиУчетНалогаНаДоходыИВзносов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის ცნობარ ""დარიცხვების და დაკავების სახეობათა"" ელემენტები ტიპით ""ბეგარა""! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага запрещено!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьДисконтныеКарты() Экспорт
	...
	Пока ВыборкаДисконтныеКарты.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება დისკონტური ბარათები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются дисконтные карты! Снятие опции запрещено!'");
			Прервать;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьАвтоматическиеСкидкиНаценки() Экспорт
	...
	Пока ВыборкаАвтоматическиеСкидки.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ავტომატური ფასდაკლებების, ფასნამატების სახეობები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются виды автоматических скидок, наценок! Снятие опции запрещено!'");
			Прервать;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='დოკუმენტებში გამოიყენება ავტომატური ფასდაკლებები და ფასნამატები! ოპციების მოხსნა აკრძაქლულია!';ru='В документах используются автоматические скидки и наценки! Снятие опции запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияРазноситьОплатуОтЭквайрераПоЭквайринговымОперациям() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("КассовыйМетодУчетаДоходовИРасходов") Тогда
		ТекстОшибки = НСтр("ka='როდესაც დაყენებულია ოპცია ""შემოსავლების და ხარჯების აღრიცხვის სალარო მეთოდი"", ოპციის მოხსნა აკრძალულია!';ru='При установленной опции ""Кассовый метод учета доходов и расходов"" снятие опции запрещено!'");
	КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьЗаказыВРозничнойТорговле() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საცალო დოკუმენტებში გამოიყენება მყიდველთა შეკვეთები! ოპციის მოხსნა აკრძალულია!';ru='В розничных документах используются заказы покупателей! Снятие опции запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьРемонты() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის ნაშთი რემონტში მყოფი საქონლის მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть остатки по товарам в ремонте! Снятие флага запрещено!'");
...
Функция ОтказСнятьИспользоватьНесколькоОрганизаций() Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстОшибки = НСтр("ka='ძირითადის გარდა, ბაზაში არის სხვა ორგანიზაციებიც. ოპციის გამოსართავად, საჭიროა ძირითადის გარდა, ყველას წაშლა.';ru='В базе есть организации помимо основной. Для выключения опции необходимо удалить все кроме основной.'");
	КонецЕсли;
...
Процедура УстановитьАдресСервисаЦентраМониторинга(НовоеЗначение) Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ka='დერვისის მისამართი';ru='Адрес сервиса'") + " "
			+ НовоеЗначение + " "
			+ НСтр("ka='პროგრამის გამოყენების შესახებ ანგარიშგებების გასაგზავნად ვებ-სერვისის დასაშვებ მისამართს არ წარმოადგენს.';ru='не является допустимым адресом веб-сервиса для отправки отчетов об использовании программы.'"); 
...
Процедура ПроверитьДанныеЕГАИС(ТаблицаТоваров, ДанныеЕГАИСДостаточны, ДокументОбъект) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ka='ესასს-ში გადასაცემად მონაცემები არასაკმარისია:';ru='Не хватает данных для передачи в ЕГАИС:'"),
		ДокументОбъект);
	
	ДополнительныйСимвол = "";
	Пока Выборка.Следующий() Цикл
		
		ТекстТекущейОшибки = НСтр("ka='სტრიქონი #%1';ru='Строка №%1:'");
		...
		Если Выборка.ОшибкаШтрихкода Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='შტრიხკოდი';ru='штрихкод'");
		КонецЕсли;
		Если Выборка.ОшибкаОбъема Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='პროდუქციის ერთეულის მოცულობა';ru='объем единицы продукции'");
		КонецЕсли;
		Если Выборка.ОшибкаМарки Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='აქციზური მარკა';ru='акцизная марка'");
		КонецЕсли;
		Если Выборка.ОшибкаКрепости Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='სიმაგრე';ru='крепость'");
		КонецЕсли;
		Если Выборка.ОшибкаКодаВидаПродукции Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='ალკოჰოლური პროდუქციის სახეობის კოდი';ru='код вида алкогольной продукции'");
		КонецЕсли;
...
Процедура УстановитьОтображаниеПодменюПечати(ГруппаФормыПечать) Экспорт
	...
	ГруппаФормыПечать.Подсказка		= НСтр("ka='ამობეჭდვა წინასწარი ნახვით';ru='Печать с предварительным просмотром'");
	ГруппаФормыПечать.Заголовок		= НСтр("ka='ბეჭდვა';ru='Печать'");
...
Функция КомандаПечатаетсяВСерверномКонтексте(знач ОписаниеКомандыПечати) Экспорт
	...
	Если ОписаниеКомандыПечати.Представление = НСтр("ka='Настраиваемый комплект документов';ru='Настраиваемый комплект документов'") Тогда
		Возврат Ложь;
	КонецЕсли;
