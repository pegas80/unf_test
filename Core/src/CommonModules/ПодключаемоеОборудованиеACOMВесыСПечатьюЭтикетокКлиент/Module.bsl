...
Функция ВыгрузитьТовары(ОбъектДрайвера, Параметры, ПараметрыПодключения, Товары, ЧастичнаяВыгрузка, ВыходныеПараметры) 
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის გადმოტვირთვა ეტიკეტების მბეჭდავ სასწორში...';ru='Выполняется выгрузка товаров в весы с печатью этикеток...';en='Unloading of goods is carried out in the scales with label printing ...'")); 
...
Функция ОчиститьТоварыВВесах(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) 
	...
	ВыходныеПараметры.Добавить(999);
	ВыходныеПараметры.Добавить(НСтр("ka='მოცემული სასწორის მიერ საქონლის ავტომატური გასუფთავება არ არის მხარდაჭერილი. სასწორში საქონლის გასასუფთავებლად გაუშვით სასწორში მონაცემთა ჩატვირთვის დანართი და დააჭირეთ ღილაკ ""სასწორში PLU-ს გასუფთავება""-ს';ru='Данные весы не поддерживают автоматическую очистку товаров."
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) 
	...
	Если ПустаяСтрока(ВремБазаТоваров) Тогда
		Результат = Ложь;
		ТекстОшибки = НСтр("ka='Файл базы товаров не указан.';ru='Файл базы товаров не указан.';en='base catalog file is not specified.'");
	КонецЕсли;
