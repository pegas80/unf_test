...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	Если РеквизитыСХранилищемЗначений = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='მეტამონაცემთა ობიექტი %1 ვერ დამუშავდება შემდეგი დამამუშავებლით მნიშვნელობებისსაცავებისმონაცემთაგადმოტვირთვაჩატვირთვა.ობიექტისგადმოტვირთვამდე()!';ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхХранилищЗначений.ПередВыгрузкойОбъекта()!';en='Metadata object% 1 can not be processed by the handler VygruzkaZagruzkaDannyhHranilischZnacheniy.PeredVygruzkoyObekta ()!'"),
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='Неожиданный объект метаданных: %1!';ru='Неожиданный объект метаданных: %1!';en='Unexpected object metadata:% 1!'"),
			ОбъектМетаданных.ПолноеИмя);
...
Процедура ПередЗагрузкойКонстанты(Контейнер, Объект, Артефакт)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='მფლობელის ტიპი {%1}%2 მეტამონაცემთა ობიექტ %3-ისთვის არ უნდა იქნას გამოყენებული!';ru='Тип владельца {%1}%2 не должен использоваться для объекта метаданных %3!';en='Type of owner} {% 1% 2 should not be used for metadata object% 3!'"),
...
Процедура ЗагрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='გადმოტვირთვის კონტეინერში მნიშვნელობების საცავის მონაცემთა განთავსების მოულოდნელი ტიპი: {%1}%2!';ru='Неожиданный тип размещения данных хранилища значений в контейнере выгрузки: {%1}%2!';en='Unexpected Guest accommodation data storage values ​​in the discharge container: {1}%% 2!'"),
