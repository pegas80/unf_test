...
Функция СообщениеЗапросаGLN(Организация, ИНН, КПП) Экспорт
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение GLN по ИНН %1, КПП %2';ru='Получение GLN по ИНН %1, КПП %2';en='Preparation of GLN INN 1%, 2% CPT'"), ИНН, КПП);
	Иначе
		СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение GLN по ИНН %1';ru='Получение GLN по ИНН %1';en='Preparation of GLN INN 1%'"), ИНН);
	КонецЕсли;
...
Функция СообщенияКПередачеЗапросыСписковВходящихДокументовИКвитанцийXML() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
		СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение входящих документов организации %1';ru='Получение входящих документов организации %1';en='Receiving incoming documents of the organization 1%'"), Выборка.Организация);
...
Функция СообщенияКПередачеПолучениеКвитанций(GLN) Экспорт
	...
	Пока ДанныеДляПолученияКвитанцииИзОчереди.Следующий() Цикл
		...
		СообщениеXML.Описание = СтрШаблон(
			НСтр("ka='Получение квитанции по операции %1 документа %2 по организации %3';ru='Получение квитанции по операции %1 документа %2 по организации %3';en='Preparation of receipt of a transaction document 1% 2% 3% on the organization'"),
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если Категории.ПолучениеСпискаВходящихДокументов.Получить(Сообщение.Операция) <> Неопределено Тогда
			...
			Для Каждого ДанныеДляПолученияДокумента Из РезультатПередачиИОбработки.РезультатОбработкиОтвета.Документы Цикл
				...
				СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
				СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение документа ""%1"" по организации %2';ru='Получение документа ""%1"" по организации %2';en='Receiving a document ""%1"" on the organization of 2%'"), ТипДокумента.Описание, Сообщение.Организация);
...
Функция ПередатьДанные(ИсходящееСообщение, СертификатыДляПодписанияНаСервере = Неопределено) Экспорт
	...
	Если ОбновитьСостояниеПодтверждения Тогда
		...
		Исключение
			...
			ВозвращаемоеЗначение.РезультатОбработкиОтвета.ТекстОшибки = СтрШаблон(НСтр("ka='Не удалось заблокировать документ %1.';ru='Не удалось заблокировать документ %1.';en='Unable to lock the document% 1.'"), ДокументСсылка);
			Возврат ВозвращаемоеЗначение;
		КонецПопытки;
...
"Текст ошибки: %2';ka='При передаче сообщения по документу %1 возникла ошибка:"
"Текст ошибки: %2'"),
				ДокументСсылка,
...
"%1"
"По причине:"
"%2';ka='Данные не переданы в ГИСМ:"
"%1"
"По причине:"
...
"Текст ошибки: %2';ka='При загрузке квитанции о фиксации по документу %1 возникла ошибка:"
"Текст ошибки: %2'"),
				ДокументСсылка,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"Переданные данные: %1"
"По причине:"
"%2';ka='Квитанция о фиксации не получена:"
"Переданные данные: %1"
"По причине:"
"%2'"), ОписаниеОперации, РезультатОтправкиЗапроса.ТекстОшибки));
		Иначе
...
Функция ЗапросGLNПоИННКППXML(СообщениеXML, ИНН, КПП = Неопределено) Экспорт
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ka='Указан не верный ИНН.';ru='Указан не верный ИНН.';en='Set not correct TIN.'");
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ka='Указан не верный КПП.';ru='Указан не верный КПП.';en='Unknown is not a true PPC.'");
		КонецПопытки;
	КонецЕсли;
...
Функция ЗапросКвитанцииОФиксацииПоСсылкеXML(Ссылка, Операция) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("Описание", СтрШаблон(НСтр("ka='Запрос квитанции о фиксации документа %1';ru='Запрос квитанции о фиксации документа %1';en='Request receipt of fixing the document% 1'"), Ссылка));
	ВозвращаемоеЗначение.Вставить("СообщенияXML", Новый Массив);
...
"Текст ошибки: %2';ka='При подготовке к передаче сообщения по документу %1 возникла ошибка:"
"Текст ошибки: %2'"),
			Реквизиты.Документ,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ДобавитьЗаписьВПротоколОбмена(КонвертSOAP, Реквизиты) Экспорт
	...
	Если Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Исходящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ka='Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:';ru='Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:';en='Error checking of an electronic signature is sent in GISM message% 1:'"), ПрисоединенныйФайл);
	ИначеЕсли Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Входящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ka='Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:';ru='Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:';en='Error checking the electronic signature of the downloaded GISM message% 1:'"), ПрисоединенныйФайл);
	КонецЕсли;
...
"Текст ошибки: %1';ka='При разборе XML ответа на запрос получения квитанции из ГИСМ возникла ошибка:"
		...
		Если ОбъектXDTO.result = Неопределено Тогда
			...
			Если ОбъектXDTO.request_query_error <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ka='Квитанция получена. Ошибка: Неверный запрос."
"Дополнительная информация: Код ошибки: %1. Текст ошибки: %2.';ru='Квитанция получена. Ошибка: Неверный запрос."
...
"%1.';ka='При получения квитанции из ГИСМ произошел системный сбой:"
"%1.'"),
					ОписаниеОшибки);
...
"Дополнительная информация: Код ошибки: %1. Текст ошибки: %2.'"),
				...
				ИначеЕсли ОбъектXDTO.result.system_failure <> Неопределено Тогда
					...
					ВозвращаемоеЗначение.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Ошибка;
					ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
						НСтр("ka='Квитанция получена. Ошибка: Системный сбой."
"Дополнительная информация: %1.';ru='Квитанция получена. Ошибка: Системный сбой."
				...
				ИначеЕсли ОбъектXDTO.result.reject <> Неопределено Тогда
					...
					Если ОбъектXDTO.result.reject.denied_action <> Неопределено Тогда
						...
						Если ОписаниеТипаДокумента <> Неопределено Тогда
							ОписаниеОшибки = СтрШаблон(НСтр("ka='Недостаточно прав на выполнение операции ""%1""';ru='Недостаточно прав на выполнение операции ""%1""';en='Insufficient permissions to perform the operation ""%1""'"), ОписаниеТипаДокумента.Описание);
						Иначе
							ОписаниеОшибки = СтрШаблон(НСтр("ka='Недостаточно прав на выполнение операции (action id: %1)';ru='Недостаточно прав на выполнение операции (action id: %1)';en='Insufficient permissions to perform the operation (action id:% 1)'"), ТипДокумента);
						КонецЕсли;
					...
					ВозвращаемоеЗначение.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Отклонено;
					ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
						НСтр("ka='Квитанция получена. Ошибка: Отказ на проводку запроса."
"Дополнительная информация: %1.';ru='Квитанция получена. Ошибка: Отказ на проводку запроса."
"Дополнительная информация: %1.'"),
						ОписаниеОшибки);
				...
				ИначеЕсли ОбъектXDTO.result.moderation <> Неопределено Тогда
					...
					ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
						НСтр("ka='Запрос ожидает рассмотрения и утверждения."
"Дополнительная информация: %1.';ru='Запрос ожидает рассмотрения и утверждения."
"Дополнительная информация: %1.'"),
						ОписаниеОшибки);
...
"Текст ошибки: %1';ka='При разборе объекта XDTO ответа на запрос получения квитанции из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';ka='При разборе XML ответа на запрос передачи данных в ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Код ошибки: %1. Текст ошибки: %2.';ka='При передаче данных в ГИСМ возникла ошибка"
...
"Дополнительная информация: %1.';ka='При передаче данных в ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки);
...
"Текст ошибки: %1';ka='При разборе объекта XDTO ответа на запрос передачи данных в ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';ka='При разборе XML ответа на запрос получения документа из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
	...
	Попытка
		...
		Если ОбъектXDTO.item_body = Неопределено Тогда
			...
			Если ДанныеДляПолученияДокумента = Неопределено Тогда
				...
				ДанныеДляПолученияДокумента.Вставить("action_id", НСтр("ka='<не прочитано>';ru='<не прочитано>';en='<Not read>'"));
				ДанныеДляПолученияДокумента.Вставить("output_id", НСтр("ka='<не прочитано>';ru='<не прочитано>';en='<Not read>'"));
				
				НаименованиеДокумента = НСтр("ka='<не прочитано>';ru='<не прочитано>';en='<Not read>'");
...
"Код ошибки: %1. Текст ошибки: %2.';ka='При получении документа %5 (action_id=%3, output_id=%4) из ГИСМ возникла ошибка:"
"Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки,
...
"Дополнительная информация: %1.';ka='При получении документа %4 (action_id=%2, output_id=%3) из ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки,
					ДанныеДляПолученияДокумента.action_id,
					ДанныеДляПолученияДокумента.output_id,
...
"Текст ошибки: %1';ka='При разборе объекта XDTO ответа на запрос получения документа из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';ka='При разборе XML ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Код ошибки: %1. Текст ошибки: %2.';ka='При получении списка новых документов из ГИСМ возникла ошибка:"
"Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
...
"Дополнительная информация: %1.';ka='При получении списка новых документов из ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки);
...
"Текст ошибки: %1';ka='При разборе объекта XDTO ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';ka='При разборе XML ответа на запрос получения данных реестра из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Код ошибки: %1. Текст ошибки: %2.';ka='При получении данных реестра из ГИСМ возникла ошибка:"
"Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
...
"Дополнительная информация: %1.';ka='При получении данных реестра из ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки);
...
"Текст ошибки: %1';ka='При разборе объекта XDTO ответа на запрос получения данных реестра из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
Функция ОтправитьЗапрос(ТекстПодписанногоИсходящегоСообщенияXML, ВариантЗапроса) Экспорт
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(РезультатОтправки.КодСостояния) Тогда
			...
			ЗаголовокОшибки = НСтр("ka='HTTP-запрос не отправлен.';ru='HTTP-запрос не отправлен.';en='HTTP-request is not sent.'");
		Иначе
			ТекстСообщенияXMLОтправлен = Истина;
			ЗаголовокОшибки = СтрШаблон(НСтр("ka='Код состояния HTTP: %1.';ru='Код состояния HTTP: %1.';en='HTTP status code:% 1.'"), РезультатОтправки.КодСостояния);
		КонецЕсли;
