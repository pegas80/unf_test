...
Функция СообщениеЗапросаGLN(Организация, ИНН, КПП) Экспорт
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение GLN по ИНН %1, КПП %2';ru='Получение GLN по ИНН %1, КПП %2'"), ИНН, КПП);
	Иначе
		СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение GLN по ИНН %1';ru='Получение GLN по ИНН %1'"), ИНН);
	КонецЕсли;
...
Функция СообщенияКПередачеЗапросыСписковВходящихДокументовИКвитанцийXML() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
		СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение входящих документов организации %1';ru='Получение входящих документов организации %1'"), Выборка.Организация);
...
Функция СообщенияКПередачеПолучениеКвитанций(GLN) Экспорт
	...
	Пока ДанныеДляПолученияКвитанцииИзОчереди.Следующий() Цикл
		...
		СообщениеXML.Описание = СтрШаблон(
			НСтр("ka='Получение квитанции по операции %1 документа %2 по организации %3';ru='Получение квитанции по операции %1 документа %2 по организации %3'"),
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если Категории.ПолучениеСпискаВходящихДокументов.Получить(Сообщение.Операция) <> Неопределено Тогда
			...
			Для Каждого ДанныеДляПолученияДокумента Из РезультатПередачиИОбработки.РезультатОбработкиОтвета.Документы Цикл
				...
				СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
				СообщениеXML.Описание = СтрШаблон(НСтр("ka='Получение документа ""%1"" по организации %2';ru='Получение документа ""%1"" по организации %2'"), ТипДокумента.Описание, Сообщение.Организация);
...
Функция ПередатьДанные(ИсходящееСообщение, СертификатыДляПодписанияНаСервере = Неопределено) Экспорт
	...
	Если ОбновитьСостояниеПодтверждения Тогда
		...
		Исключение
			...
			ВозвращаемоеЗначение.РезультатОбработкиОтвета.ТекстОшибки = СтрШаблон(НСтр("ka='Не удалось заблокировать документ %1.';ru='Не удалось заблокировать документ %1.'"), ДокументСсылка);
			Возврат ВозвращаемоеЗначение;
		КонецПопытки;
...
Функция ЗапросGLNПоИННКППXML(СообщениеXML, ИНН, КПП = Неопределено) Экспорт
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ka='Указан не верный ИНН.';ru='Указан не верный ИНН.'");
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ka='Указан не верный КПП.';ru='Указан не верный КПП.'");
		КонецПопытки;
	КонецЕсли;
...
Функция ЗапросКвитанцииОФиксацииПоСсылкеXML(Ссылка, Операция) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("Описание", СтрШаблон(НСтр("ka='Запрос квитанции о фиксации документа %1';ru='Запрос квитанции о фиксации документа %1'"), Ссылка));
	ВозвращаемоеЗначение.Вставить("СообщенияXML", Новый Массив);
...
Функция ДобавитьЗаписьВПротоколОбмена(КонвертSOAP, Реквизиты) Экспорт
	...
	Если Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Исходящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ka='Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:';ru='Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:'"), ПрисоединенныйФайл);
	ИначеЕсли Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Входящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ka='Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:';ru='Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:'"), ПрисоединенныйФайл);
	КонецЕсли;
				...
				ИначеЕсли ОбъектXDTO.result.reject <> Неопределено Тогда
					...
					Если ОбъектXDTO.result.reject.denied_action <> Неопределено Тогда
						...
						Если ОписаниеТипаДокумента <> Неопределено Тогда
							ОписаниеОшибки = СтрШаблон(НСтр("ka='Недостаточно прав на выполнение операции ""%1""';ru='Недостаточно прав на выполнение операции ""%1""'"), ОписаниеТипаДокумента.Описание);
						Иначе
							ОписаниеОшибки = СтрШаблон(НСтр("ka='Недостаточно прав на выполнение операции (action id: %1)';ru='Недостаточно прав на выполнение операции (action id: %1)'"), ТипДокумента);
						КонецЕсли;
	...
	Попытка
		...
		Если ОбъектXDTO.item_body = Неопределено Тогда
			...
			Если ДанныеДляПолученияДокумента = Неопределено Тогда
				...
				ДанныеДляПолученияДокумента.Вставить("action_id", НСтр("ka='<не прочитано>';ru='<не прочитано>'"));
				ДанныеДляПолученияДокумента.Вставить("output_id", НСтр("ka='<не прочитано>';ru='<не прочитано>'"));
				
				НаименованиеДокумента = НСтр("ka='<не прочитано>';ru='<не прочитано>'");
...
Функция ОтправитьЗапрос(ТекстПодписанногоИсходящегоСообщенияXML, ВариантЗапроса) Экспорт
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(РезультатОтправки.КодСостояния) Тогда
			...
			ЗаголовокОшибки = НСтр("ka='HTTP-запрос не отправлен.';ru='HTTP-запрос не отправлен.'");
		Иначе
			ТекстСообщенияXMLОтправлен = Истина;
			ЗаголовокОшибки = СтрШаблон(НСтр("ka='Код состояния HTTP: %1.';ru='Код состояния HTTP: %1.'"), РезультатОтправки.КодСостояния);
		КонецЕсли;
