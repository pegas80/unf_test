...
Функция УстановитьУсловноеОформлениеОтмененногоЗаказа(УсловноеОформлениеКД,
	...
	ЭлементУсловногоОформления.Представление	= НСтр("ka='Отмененные заказы зачеркивать';ru='Отмененные заказы зачеркивать';en='Canceled orders to cross out'");
...
Функция УстановитьУсловноеОформлениеСостоянияЗавершен(УсловноеОформлениеКД,
	...
	ЭлементУсловногоОформления.РежимОтображения	= РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
	ЭлементУсловногоОформления.Представление	= НСтр("ka='Состояние ""Завершен"" жирным шрифтом';ru='Состояние ""Завершен"" жирным шрифтом';en='State ""Completed"" in bold'");
...
Процедура УстановитьУсловноеОформлениеПоЦветамСостояний(УсловноеОформлениеКД,
	...
	Пока ВыборкаСостояний.Следующий() Цикл
		...
		ЭлементУсловногоОформления.РежимОтображения	= РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
		ЭлементУсловногоОформления.Представление	= НСтр("ka='Оформление в цвет состояния';ru='Оформление в цвет состояния';en='Making color status'");
...
Процедура ЗаполнитьСписокВыбораЗавершенияЗаказа(СписокВыбора) Экспорт
	...
	КонецЦикла;
	СписокВыбора.Добавить("ПустаяСсылка", НСтр("ka='არ არის დასრულებული';ru='Не завершен';en='Not finished'"));
...
Процедура ПроверитьОчиститьИсториюСостояний(ЗаказСсылка, ПредыдущееСостояние, ТекущееСостояние, ВидЗаказа = Неопределено) Экспорт
	...
	//Если ПорядокПредыдущего = Неопределено Или ПорядокТекущего >= ПорядокПредыдущего Тогда //+++ Дмгг, баг из-за того что неопределено сравингивается с числом по виду >=
	Если ПорядокПредыдущего = Неопределено Или (ПорядокТекущего <> НЕОПРЕДЕЛЕНО И ПорядокТекущего >= ПорядокПредыдущего) Тогда //+++ Дмгг
		Возврат;
	КонецЕсли;
