...
Процедура УстановитьФлагАктивностиВОбласти(Знач ОбластьДанных = Неопределено, Знач Состояние = Истина)
	...
	Если ОбластьДанных = Неопределено Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ka='განუცალკევებელი სეანსიდან პროცედურა აქტიურობისალმისდაყენებაარეალში-ს მოხმობისას პარამეტრი მონაცემთაარეალი სავალდებულოა!';ru='При вызове процедуры УстановитьФлагАктивностиВОбласти из неразделенного сеанса параметр ОбластьДанных является обязательным!'");
		КонецЕсли;
	Иначе
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей()
			...
			ВызватьИсключение(НСтр("ka='არეალის მონაცემებთან მუშაობა აკრძალულია  მიმდინარეს გარდა';ru='Запрещено работать с данными области кроме текущей'"));
...
Процедура СозданиеКопий(Знач ПараметрыСоздания, Знач СостояниеСоздания) Экспорт
	...
	Если НЕ ТребуетсяСоздание Тогда
		ЗаписьЖурналаРегистрации(
			СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ka='შექმნილის გამოტოვება';ru='Пропуск создания'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა არეალის სარეზერვო ასლის შექმნა. დროებითი ფაილის წაშლა ვერ მოხერხდა';ru='Создание резервной копии области данных.Не удалось удалить временный файл'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		ЗаписатьСобытиеВЖурнал(
			НСтр("ka='შექმნა:';ru='Создание'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			ПараметрыСобытия);
			
	Исключение
		
		ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა არეალის სარეზერვო ასლის შექმნა';ru='Создание резервной копии области данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Попытка
...
"%3';ka='არეალ %1-ის სარეზერვო ასლის შექმნისას შეცდომა მოხდა. მცდელობის ნომერი: %2 შემდეგი მიზეზით: %3'");
		...
		Событие = НСтр("ka='შექმნის იტერაციის შეცდომა';ru='Ошибка итерации создания'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
...
"%3'");
		...
		Событие = НСтр("ka='შექმნის შეცდომა';ru='Ошибка создания'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ВыгрузитьОбластьВХранилищеМС(Знач ПараметрыВыгрузки, АдресХранилища = Неопределено) Экспорт
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ka='დაშვების უფლებების დარღვევა';ru='Нарушение прав доступа'"));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ka='დანართების სარეზერვო კოპირება';ru='Резервное копирование приложений'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
