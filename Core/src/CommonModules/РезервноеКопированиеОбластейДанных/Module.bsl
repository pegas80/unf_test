...
Процедура УстановитьФлагАктивностиВОбласти(Знач ОбластьДанных = Неопределено, Знач Состояние = Истина)
	...
	Если ОбластьДанных = Неопределено Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ka='განუცალკევებელი სეანსიდან პროცედურა აქტიურობისალმისდაყენებაარეალში-ს მოხმობისას პარამეტრი მონაცემთაარეალი სავალდებულოა!';ru='При вызове процедуры УстановитьФлагАктивностиВОбласти из неразделенного сеанса параметр ОбластьДанных является обязательным!';en='When the procedure is called UstanovitFlagAktivnostiVOblasti of undivided session option OblastDannyh is a must!'");
		КонецЕсли;
	Иначе
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей()
			...
			ВызватьИсключение(НСтр("ka='არეალის მონაცემებთან მუშაობა აკრძალულია  მიმდინარეს გარდა';ru='Запрещено работать с данными области кроме текущей';en='Do not work with the data of the area except for the current'"));
...
Процедура СозданиеКопий(Знач ПараметрыСоздания, Знач СостояниеСоздания) Экспорт
	...
	Если НЕ ТребуетсяСоздание Тогда
		ЗаписьЖурналаРегистрации(
			СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ka='შექმნილის გამოტოვება';ru='Пропуск создания';en='skipping creation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა არეალის სარეზერვო ასლის შექმნა. დროებითი ფაილის წაშლა ვერ მოხერხდა';ru='Создание резервной копии области данных.Не удалось удалить временный файл';en='Create a backup area dannyh.Ne to delete the temporary file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		ЗаписатьСобытиеВЖурнал(
			НСтр("ka='შექმნა:';ru='Создание';en='Creature'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			ПараметрыСобытия);
			
	Исключение
		
		ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა არეალის სარეზერვო ასლის შექმნა';ru='Создание резервной копии области данных';en='Create a backup of the data area'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Попытка
...
"Номер попытки: %2"
"По причине:"
"%3';ka='არეალ %1-ის სარეზერვო ასლის შექმნისას შეცდომა მოხდა. მცდელობის ნომერი: %2 შემდეგი მიზეზით: %3'");
		Уровень = УровеньЖурналаРегистрации.Предупреждение;
		Событие = НСтр("ka='შექმნის იტერაციის შეცდომა';ru='Ошибка итерации создания';en='Error creating iteration'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		ШаблонКомментария = НСтр("ru='При создании резервной копии области %1 произошла невосстановимая ошибка."
"Номер попытки: %2"
"По причине:"
"%3';ka='არეალ %1-ის სარეზერვო ასლის შექმნისას აღუდგენელი შეცდომა მოხდა."
"მცდელობის ნომერი: %2"
"შემდეგი მიზეზით:"
"%3'");
		Уровень = УровеньЖурналаРегистрации.Ошибка;
		Событие = НСтр("ka='შექმნის შეცდომა';ru='Ошибка создания';en='error creating'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ВыгрузитьОбластьВХранилищеМС(Знач ПараметрыВыгрузки, АдресХранилища = Неопределено) Экспорт
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ka='დაშვების უფლებების დარღვევა';ru='Нарушение прав доступа';en='Access Violation'"));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ka='დანართების სარეზერვო კოპირება';ru='Резервное копирование приложений';en='Backup applications'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"%1';ka='შეცდომა სარეზერვო კოპირების გამართვების მიღებისას: %1'");
...
"%1';ka='შეცდომა სარეზერვო კოპირების გამართვების შენახვისას: %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СообщениеОбОшибке);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
