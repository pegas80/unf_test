...
Процедура ОбновлениеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ka='ППД ინდექსის განახლება';ru='Обновление индекса ППД'"), Ложь, Истина);
...
Процедура СлияниеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ka='ППД-ის ინდექსის შერწყმა';ru='Слияние индекса ППД'"), Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ДатаАктуальностиИндекса > ТекущаяДата() Тогда
		Интервал = НСтр("ka='ერთ დღეზე ნაკლები';ru='менее одного дня'");
	Иначе
		// Исключение, должна использоваться ТекущаяДата().
	...
	Дело.Представление  = НСтр("ka='სრულტექსტიანი ძებნის ინდექსი მოძველდა';ru='Индекс полнотекстового поиска устарел'");
	...
	Дело.Подсказка      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ბოლო განახლებები %1 ადრე';ru='Последнее обновление %1 назад'"), Интервал);
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	ЗаписьЖурнала(Неопределено, НСтр("ka='პროცედურა ""%1""-ის გაშვება.';ru='Запуск процедуры ""%1"".'"), , ПредставлениеПроцедуры);
	
	Попытка
		ПолнотекстовыйПоиск.ОбновитьИндекс(РазрешитьСлияние, Порциями);
		ЗаписьЖурнала(Неопределено, НСтр("ka='პროცედურა ""%1""-ის წარმატებული დასრულება.';ru='Успешное завершение процедуры ""%1"".'"), , ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(Неопределено, НСтр("ka='პროცედურის შესრულების შეცდომა ""%1"":';ru='Ошибка выполнения процедуры ""%1"":'"), ИнформацияОбОшибке(), ПредставлениеПроцедуры);
	КонецПопытки;
...
Процедура ЗаписьЖурнала(УровеньЖурнала, КомментарийСПараметрами, ИнформацияОбОшибке = Неопределено, Параметр = Неопределено)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='სრულტექსტიანი ინდექსირება';ru='Полнотекстовое индексирование'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
