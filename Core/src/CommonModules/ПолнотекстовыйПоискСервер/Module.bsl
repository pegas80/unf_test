...
Процедура ОбновлениеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ka='ППД ინდექსის განახლება';ru='Обновление индекса ППД';en='Updating the PPD index'"), Ложь, Истина);
...
Процедура СлияниеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ka='ППД-ის ინდექსის შერწყმა';ru='Слияние индекса ППД';en='Merge PPD index'"), Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ДатаАктуальностиИндекса > ТекущаяДата() Тогда
		Интервал = НСтр("ka='ერთ დღეზე ნაკლები';ru='менее одного дня';en='less than one day'");
	Иначе
		// Исключение, должна использоваться ТекущаяДата().
	...
	Дело.Представление  = НСтр("ka='სრულტექსტიანი ძებნის ინდექსი მოძველდა';ru='Индекс полнотекстового поиска устарел';en='full-text search index is outdated'");
	...
	Дело.Подсказка      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ბოლო განახლებები %1 ადრე';ru='Последнее обновление %1 назад';en='Last updated% 1 ago'"), Интервал);
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	ЗаписьЖурнала(Неопределено, НСтр("ka='პროცედურა ""%1""-ის გაშვება.';ru='Запуск процедуры ""%1"".';en='Running the procedure ""%1"".'"), , ПредставлениеПроцедуры);
	
	Попытка
		ПолнотекстовыйПоиск.ОбновитьИндекс(РазрешитьСлияние, Порциями);
		ЗаписьЖурнала(Неопределено, НСтр("ka='პროცედურა ""%1""-ის წარმატებული დასრულება.';ru='Успешное завершение процедуры ""%1"".';en='Successful completion of the procedure ""%1"".'"), , ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(Неопределено, НСтр("ka='პროცედურის შესრულების შეცდომა ""%1"":';ru='Ошибка выполнения процедуры ""%1"":';en='Runtime Error ""%1"" procedure:'"), ИнформацияОбОшибке(), ПредставлениеПроцедуры);
	КонецПопытки;
...
Процедура ЗаписьЖурнала(УровеньЖурнала, КомментарийСПараметрами, ИнформацияОбОшибке = Неопределено, Параметр = Неопределено)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='სრულტექსტიანი ინდექსირება';ru='Полнотекстовое индексирование';en='Full-text indexing'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
