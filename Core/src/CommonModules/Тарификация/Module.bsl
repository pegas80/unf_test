...
Функция УслугаПоИдентификаторуИИдентификаторуПоставщика(Знач ИдентификаторУслуги, Знач ИдентификаторПоставщика, ВызыватьИсключение = Истина) Экспорт
	...
	Иначе
		ШаблонОшибки = НСтр("ka='Идентификатор поставщика: %1, Идентификатор услуги: %2';ru='Идентификатор поставщика: %1, Идентификатор услуги: %2'");
...
Функция ЗарегистрированаЛицензияУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, КонтекстЛицензии = "") Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Проверка наличия лицензии уникальной услуги из приложения';ru='Проверка наличия лицензии уникальной услуги из приложения'");
...
Функция ЗанятьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, ИдентификаторОперации, КонтекстЛицензии = "") Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Заем лицензии уникальной услуги из приложения';ru='Заем лицензии уникальной услуги из приложения'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
		...
		Если Ответ.КодСостояния = 200 Тогда
			...
			ЗаписатьИнформацию(ИмяСобытия, ЗаголовокКомментария, НСтр("ka='წარმატება';ru='Успех'"));
			Возврат ОтветЗанятияУслуги;
...
Функция ОсвободитьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, ИдентификаторОперации, КодОбластиДанных = Неопределено,
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Освобождение лицензии уникальной услуги из приложения';ru='Освобождение лицензии уникальной услуги из приложения'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
		...
		ИначеЕсли Ответ.КодСостояния = 410 Тогда
			ЗаписатьИнформацию(ИмяСобытия, ЗаголовокКомментария, НСтр("ka='Не найдена лицензия';ru='Не найдена лицензия'"));
			СнятьРазделительСеанса(КодОбластиДанных);
			Возврат Ложь;
		ИначеЕсли Ответ.КодСостояния = 400 Тогда
			ВызватьИсключение СтруктураОтветаHTTP.ErrorMessage;
...
Функция ЗанятьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Заем лицезний лимитированной услуги из приложения';ru='Заем лицезний лимитированной услуги из приложения'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			,
			КоличествоЛицензий,
...
Функция ОсвободитьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Освобождение лицезний лимитированной услуги из приложения';ru='Освобождение лицезний лимитированной услуги из приложения'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
...
Функция ПодтвердитьОперацию(ИдентификаторОперации) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='ოპერაციის ფიქსაცია';ru='Фиксация операции'");
...
Функция ОтменитьОперацию(ИдентификаторОперации) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='ოპერაციის გაუქმება';ru='Отмена операции'");
		ЗаголовокКомментария = НСтр("ru='Идентификатор операции: ';ka='Идентификатор операции: '") + Строка(ИдентификаторОперации);
...
Функция КоличествоЛицензийУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Получение количества свободных лицензий уникальной услуги из приложения';ru='Получение количества свободных лицензий уникальной услуги из приложения'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
...
Функция КоличествоЛицензийЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Получение количества доступных лицензий лимитированной услуги из приложения';ru='Получение количества доступных лицензий лимитированной услуги из приложения'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			,
			КоличествоЛицензий,
...
Функция ИмяСобытияЖР() Экспорт
	...
	Возврат ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ka='Операции с лицензиями.';ru='Операции с лицензиями.'");
...
Функция ОбновитьПоставщикаУслуг(Знач Код, Знач Наименование, Знач Идентификатор, Знач ПометкаУдаления) Экспорт
	...
	Иначе
		...
		Если Код = Неопределено Тогда 
			ВызватьИсключение НСтр("ka='При обновлении поставщика поле ""Код"" должно быть установлено';ru='При обновлении поставщика поле ""Код"" должно быть установлено'");
		КонецЕсли;
	КонецЕсли;
...
Функция РезультатПредопределенныхПроверокОперацийСЛицензиями() Экспорт
	...
	Если Не Результат Тогда
		ЗаголовокКомментария = НСтр("ka='Менеджер сервиса не поддерживает версию тарификации ""1.0.1.2""';ru='Менеджер сервиса не поддерживает версию тарификации ""1.0.1.2""'");
...
Функция СтруктураОтветаHTTP(ОтветСтрокой)
	...
	Если ПустаяСтрока(ОтветСтрокой) Тогда
		ВызватьИсключение(НСтр("ka='Пустое тело ответа системы тарификации';ru='Пустое тело ответа системы тарификации'"));
	КонецЕсли;
