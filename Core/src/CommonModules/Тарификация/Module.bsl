...
Функция УслугаПоИдентификаторуИИдентификаторуПоставщика(Знач ИдентификаторУслуги, Знач ИдентификаторПоставщика, ВызыватьИсключение = Истина) Экспорт
	...
	Иначе
		ШаблонОшибки = НСтр("ka='Идентификатор поставщика: %1, Идентификатор услуги: %2';ru='Идентификатор поставщика: %1, Идентификатор услуги: %2';en='Vendor ID:% 1, service ID:% 2'");
...
Функция ЗарегистрированаЛицензияУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, КонтекстЛицензии = "") Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Проверка наличия лицензии уникальной услуги из приложения';ru='Проверка наличия лицензии уникальной услуги из приложения';en='Check for unique service license from the app'");
...
Функция ЗанятьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, ИдентификаторОперации, КонтекстЛицензии = "") Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Заем лицензии уникальной услуги из приложения';ru='Заем лицензии уникальной услуги из приложения';en='The loan is a unique service of the license application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
		...
		Если Ответ.КодСостояния = 200 Тогда
			...
			ЗаписатьИнформацию(ИмяСобытия, ЗаголовокКомментария, НСтр("ka='წარმატება';ru='Успех';en='Success'"));
			Возврат ОтветЗанятияУслуги;
		ИначеЕсли Ответ.КодСостояния = 400 Тогда
			ВызватьИсключение СтруктураОтветаHTTP.ErrorMessage;
		Иначе
			ВызватьИсключение НСтр("ru='Код возврата веб-сервера: ';ka='Код возврата веб-сервера: '") + Строка(Ответ.КодСостояния);
		КонецЕсли;
...
Функция ОсвободитьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, ИдентификаторОперации, КодОбластиДанных = Неопределено,
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Освобождение лицензии уникальной услуги из приложения';ru='Освобождение лицензии уникальной услуги из приложения';en='Exemption unique service license from the app'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
		...
		ИначеЕсли Ответ.КодСостояния = 410 Тогда
			ЗаписатьИнформацию(ИмяСобытия, ЗаголовокКомментария, НСтр("ka='Не найдена лицензия';ru='Не найдена лицензия';en='License not found'"));
			СнятьРазделительСеанса(КодОбластиДанных);
			Возврат Ложь;
		ИначеЕсли Ответ.КодСостояния = 400 Тогда
			ВызватьИсключение СтруктураОтветаHTTP.ErrorMessage;
...
Функция ЗанятьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Заем лицезний лимитированной услуги из приложения';ru='Заем лицезний лимитированной услуги из приложения';en='Loan litsezny limited services from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			,
			КоличествоЛицензий,
...
Функция ОсвободитьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Освобождение лицезний лимитированной услуги из приложения';ru='Освобождение лицезний лимитированной услуги из приложения';en='Exemption litsezny limited services from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
...
Функция ПодтвердитьОперацию(ИдентификаторОперации) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='ოპერაციის ფიქსაცია';ru='Фиксация операции';en='fixation surgery'");
		ЗаголовокКомментария = НСтр("ru='Идентификатор операции: ';ka='Идентификатор операции: '") + Строка(ИдентификаторОперации);
...
Функция ОтменитьОперацию(ИдентификаторОперации) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='ოპერაციის გაუქმება';ru='Отмена операции';en='To cancel the operation'");
		ЗаголовокКомментария = НСтр("ru='Идентификатор операции: ';ka='Идентификатор операции: '") + Строка(ИдентификаторОперации);
...
Функция КоличествоЛицензийУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Получение количества свободных лицензий уникальной услуги из приложения';ru='Получение количества свободных лицензий уникальной услуги из приложения';en='Get the number of free licenses of unique services from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
...
Функция КоличествоЛицензийЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ka='Получение количества доступных лицензий лимитированной услуги из приложения';ru='Получение количества доступных лицензий лимитированной услуги из приложения';en='Getting the number of available licenses limited services from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			,
			КоличествоЛицензий,
...
Функция ИмяСобытияЖР() Экспорт
	...
	Возврат ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ka='Операции с лицензиями.';ru='Операции с лицензиями.';en='Operations with licenses.'");
...
Функция ОбновитьПоставщикаУслуг(Знач Код, Знач Наименование, Знач Идентификатор, Знач ПометкаУдаления) Экспорт
	...
	Иначе
		...
		Если Код = Неопределено Тогда 
			ВызватьИсключение НСтр("ka='При обновлении поставщика поле ""Код"" должно быть установлено';ru='При обновлении поставщика поле ""Код"" должно быть установлено';en='When updating a vendor specific field ""Code"" must be set'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТипУслугиПоИдентификатору(Знач ИдентификаторТипаУслуги) Экспорт
	...
	ВызватьИсключение НСтр("ru='Неизвестный код услуги: ';ka='Неизвестный код услуги: '") + ИдентификаторТипаУслуги;
...
Функция РезультатПредопределенныхПроверокОперацийСЛицензиями() Экспорт
	...
	Если Не Результат Тогда
		ЗаголовокКомментария = НСтр("ka='Менеджер сервиса не поддерживает версию тарификации ""1.0.1.2""';ru='Менеджер сервиса не поддерживает версию тарификации ""1.0.1.2""';en='Service Manager does not support charging version ""1.0.1.2""'");
...
Функция СтруктураОтветаHTTP(ОтветСтрокой)
	...
	Если ПустаяСтрока(ОтветСтрокой) Тогда
		ВызватьИсключение(НСтр("ka='Пустое тело ответа системы тарификации';ru='Пустое тело ответа системы тарификации';en='Blank body charging system response'"));
	КонецЕсли;
