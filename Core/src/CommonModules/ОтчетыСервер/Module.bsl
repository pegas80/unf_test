...
Процедура ЗаполнитьПредставлениеНастройки(СвойстваНастройки, НастройкаВариантаКД, ПользовательскаяНастройкаКД, ДоступнаяНастройкаКД)
	...
	ИначеЕсли ЗначениеЗаполнено(СвойстваНастройки.Подтип) Тогда
		Если СвойстваНастройки.Подтип = "ДиаграммаСерии" Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='სერიები';ru='Серии';en='series'");
		ИначеЕсли СвойстваНастройки.Подтип = "ДиаграммаТочки" Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='წერტილები';ru='Точки';en='points'");
		ИначеЕсли СвойстваНастройки.Подтип = "ТаблицаСтроки" Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='Строки';ru='Строки';en='lines'");
		ИначеЕсли СвойстваНастройки.Подтип = "ТаблицаКолонки" Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='სვეტები';ru='Колонки';en='loudspeakers'");
		ИначеЕсли СвойстваНастройки.Подтип = "Отчет" Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='ანგარიშგება';ru='Отчет';en='Report'");
		Иначе
			ПредставлениеПоУмолчанию = Строка(СвойстваНастройки.Подтип);
		КонецЕсли;
	ИначеЕсли СвойстваНастройки.Тип = "Отбор" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='შერჩევა';ru='Отбор';en='Selection'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Порядок" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='დახარისხება';ru='Сортировка';en='Sorting'");
	ИначеЕсли СвойстваНастройки.Тип = "ВыбранныеПоля" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='ველები';ru='Поля';en='fields'");
	ИначеЕсли СвойстваНастройки.Тип = "УсловноеОформление" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='გაფორმება';ru='Оформление';en='Registration'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Группировка"
		...
		Если ПустаяСтрока(ПредставлениеПоУмолчанию) Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='<Детальные записи>';ru='<Детальные записи>';en='<Detailed records>'");
		КонецЕсли;
	ИначеЕсли СвойстваНастройки.Тип = "Таблица" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='ცხრილი';ru='Таблица';en='Table'");
	ИначеЕсли СвойстваНастройки.Тип = "Диаграмма" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='დიაგრამა';ru='Диаграмма';en='Diagram'");
	ИначеЕсли СвойстваНастройки.Тип = "НастройкиВложенногоОбъекта" Тогда
		...
		Если ПустаяСтрока(ПредставлениеПоУмолчанию) Тогда
			ПредставлениеПоУмолчанию = НСтр("ka='ჩადებული დაჯგუფება';ru='Вложенная группировка';en='nested grouping'");
		КонецЕсли;
	ИначеЕсли СвойстваНастройки.Тип = "СтруктураНастроек" Тогда
		ПредставлениеПоУмолчанию = НСтр("ka='სტრუქტურა';ru='Структура';en='Structure'");
	Иначе
...
//+++ Дмгг
Функция ПолучитьСловоИзТекста(Знач Строка, Знач Разделитель, НомерСловаПоПорядку = 1) Экспорт
	...
	ИначеЕсли НомерСловаПоПорядку = 1 Тогда  
	...
	СтрокаОстаток = Строка;
	Для Сч = 1 По НомерСловаПоПорядку Цикл 
		
		Позиция = Найти(СтрокаОстаток, Разделитель);
		Если Позиция = 0 Тогда
			СтрокаДоРазделителя = СтрокаОстаток;
			Прервать;
		Иначе
			СтрокаДоРазделителя = Лев(СтрокаОстаток, Позиция);
			СтрокаОстаток = СтрЗаменить(СтрокаОстаток,СтрокаДоРазделителя,"");
		КонецЕсли;
