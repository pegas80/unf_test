...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
	...
	Если ИспользуетсяFTPПротокол Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ka='სერვერ %1-თან FTP-შეერთების დამყარება ვერ მოხერხდა:';ru='Не удалось установить FTP-соединение с сервером %1:';en='Unable to install FTP-connection to the server% 1:'") + Символы.ПС + "%2";
...
"%2';ka='სერვერ %1-თან HTTP-შეერთების დამყარება ვერ მოხერხდა:"
"%2'");
		...
		Иначе
			...
			Если HTTPОтвет.КодСостояния = 301 Или HTTPОтвет.КодСостояния = 302 Тогда
				...
				Если КоличествоПеренаправлений > 20 Тогда
					ВызватьИсключение НСтр("ka='Обнаружена циклическая переадресация.';ru='Обнаружена циклическая переадресация.';en='Detected redirect loop.'");
				КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ka='Не удалось получить файл %1 с сервера:';ru='Не удалось получить файл %1 с сервера:';en='Unable to retrieve file% 1 from the server:'") + Символы.ПС + "%2";
	...
	Иначе
		ВызватьИсключение НСтр("ka='Не указано место для сохранения файла.';ru='Не указано место для сохранения файла.';en='Unknown location to save the file.'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='ინტერნეტიდან ფაილების მიღება';ru='Получение файлов из Интернета';en='Getting files from the Internet'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
