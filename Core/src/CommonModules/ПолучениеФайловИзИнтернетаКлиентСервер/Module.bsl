...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
	...
	Если ИспользуетсяFTPПротокол Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ka='სერვერ %1-თან FTP-შეერთების დამყარება ვერ მოხერხდა:';ru='Не удалось установить FTP-соединение с сервером %1:'") + Символы.ПС + "%2";
...
"%2'");
		...
		Иначе
			...
			Если HTTPОтвет.КодСостояния = 301 Или HTTPОтвет.КодСостояния = 302 Тогда
				...
				Если КоличествоПеренаправлений > 20 Тогда
					ВызватьИсключение НСтр("ka='Обнаружена циклическая переадресация.';ru='Обнаружена циклическая переадресация.'");
				КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ka='Не удалось получить файл %1 с сервера:';ru='Не удалось получить файл %1 с сервера:'") + Символы.ПС + "%2";
	...
	Иначе
		ВызватьИсключение НСтр("ka='Не указано место для сохранения файла.';ru='Не указано место для сохранения файла.'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='ინტერნეტიდან ფაილების მიღება';ru='Получение файлов из Интернета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
