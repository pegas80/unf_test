...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	СтрокаСообщения = НСтр("ka='მონაცემთა გაცვლის პროცესის დასაწყისი შემდეგი კვანძისთვის %1';ru='Начало процесса обмена данными для узла %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СтруктураЗаголовкаГиперссылкиМонитораПроблем(Узлы = Неопределено) Экспорт
	...
	Если Количество > 0 Тогда
		...
		Заголовок = НСтр("ka='გაფრთხილება (%1)';ru='Предупреждения (%1)'");
	...
	Иначе
		...
		Заголовок = НСтр("ka='გაფრთხილებები არ არის';ru='Предупреждений нет'");
...
Функция ПредставлениеДатыСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		Возврат НСтр("ka='სინქრონიზაცია არ შესრულებულა.';ru='Синхронизация не выполнялась.'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უკანასკნელი სინქრონიზაცია: %1';ru='Последняя синхронизация: %1'"), ОтносительнаяДатаСинхронизации(ДатаСинхронизации));
КонецФункции
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		...
		Возврат НСтр("ka='არასდროს';ru='Никогда'");
	...
	ИначеЕсли Интервал < 60 * 5 Тогда // 5 мин
		...
		Результат = НСтр("ka='ეხლა';ru='Сейчас'");
		
	ИначеЕсли Интервал < 60 * 15 Тогда // 15 мин
		
		Результат = НСтр("ka='5 წუთის წინ';ru='5 минут назад'");
		
	ИначеЕсли Интервал < 60 * 30 Тогда // 30 мин
		
		Результат = НСтр("ka='15 წუთის წინ';ru='15 минут назад'");
		
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		
		Результат = НСтр("ka='30 წუთის წინ';ru='30 минут назад'");
		
	ИначеЕсли Интервал < 60 * 60 * 2 Тогда // 2 часа
		
		Результат = НСтр("ka='1 საათის უკან';ru='1 час назад'");
		
	ИначеЕсли Интервал < 60 * 60 * 3 Тогда // 3 часа
		
		Результат = НСтр("ka='2 საათის წინ';ru='2 часа назад'");
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დღეს, 51';ru='Сегодня, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გუშინ,%1';ru='Вчера, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გუშინწინ, %1';ru='Позавчера, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
...
Функция НомерОбластиИзКодаУзлаПланаОбмена(Знач КодУзла) Экспорт
	...
	Если ТипЗнч(КодУзла) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ka='ნომერ [1] პარამეტრის არასწორი ტიპი.';ru='Неправильный тип параметра номер [1].'");
	КонецЕсли;
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მონაცემთა გაცვლა. პრიორიტეტული მონაცემების ჩატვირთვა';ru='Обмен данными.Загрузка приоритетных данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаписатьДанныеДляОбновленияВФайл(Параметры, Данные, ВидДанных, ПолноеИмяОбъекта = "") Экспорт
	...
	Если Параметры.ЗаписьИзмененийДляПодчиненногоУзлаРИБСФильтрами = Неопределено Тогда
		ТекстИсключения = НСтр("ka='დამუშავებაში არასწორად არის ორგანიზებული დამუშავებზე მონაცემთა რეგისტრაციის პარამეტრებთან მუშაობა.';ru='В обработчике неправильно организована работа с параметрами регистрации данных к обработке.'");
...
Процедура ОбновитьРегламентныеЗаданияСценариевОбменовДанными() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ka='შეცდომა სარეგლამენტო დავალების განახლებისას გაცვლის მონაცემთა სცენარისთვის.';ru='Ошибка при обновлении регламентного задания для сценария обмена данными.'");
		КонецЕсли;
...
Процедура РассчитатьПроцентВыгрузки(КоличествоВыгруженных, КоличествоОбъектовКВыгрузке) Экспорт
	...
	Если КоличествоОбъектовКВыгрузке = 0 Или КоличествоВыгруженных > КоличествоОбъектовКВыгрузке Тогда
		...
		Шаблон = НСтр("ka='Обработано: %1 объектов.';ru='Обработано: %1 объектов.'");
	...
	Иначе
		...
		Шаблон = НСтр("ka='Обработано: %1 из %2 объектов.';ru='Обработано: %1 из %2 объектов.'");
...
Процедура РассчитатьПроцентЗагрузки(КоличествоЗагруженных, КоличествоОбъектовКЗагрузке, РазмерФайлаСообщенияОбмена) Экспорт
	...
	Если РазмерФайлаСообщенияОбмена <> 0 Тогда
		Шаблон = НСтр("ka='Размер сообщения %1 МБ';ru='Размер сообщения %1 МБ'");
...
Функция FTPИмяСервераИПуть(Знач СтрокаПодключения) Экспорт
	...
	Если (ВРег(Лев(СтрокаПодключения, 6)) <> "FTP://"
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';ru='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
	...
	Если ПараметрыПодключения.Количество() < 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='FTP-რესურსთან შეერთების სტრიქონში სერვერის სახელი არ არის მითითებული: ""%1""';ru='В строке подключения к FTP-ресурсу не указано имя сервера: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
...
Функция ВыбратьИзменения(Знач Узел, Знач НомерСообщения, Знач ФильтрВыборки = Неопределено) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა ცვლილების ამოკრება აქტიურ ტრანზაქციაში აკრძალულია.';ru='Выборка изменений данных запрещена в активной транзакции.'");
	КонецЕсли;
...
Функция НастройкиПланаОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ЗаголовокФормыПомощника = НСтр("ka='მონაცემთა სინქრონიზაცია %Программа%-სთან (გამართვა)';ru='Синхронизация данных с %Программа% (настройка)'");
...
Процедура ПроверитьВозможностьВыполненияОбменов() Экспорт
	...
	ИначеЕсли ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
		...
		ВызватьИсключение НСтр("ka='საინფორმაციო ბაზა იმყოფება განახლების მდგომარეობაში.';ru='Информационная база находится в состоянии обновления.'");
...
Процедура ПроверитьИспользованиеОбменаДанными() Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		ТекстСообщения = НСтр("ka='Синхронизация данных запрещена администратором.';ru='Синхронизация данных запрещена администратором.'");
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект) Экспорт
	...
	Для Каждого СтрокаДвижения Из ТаблицаДвижений Цикл
		...
		Если Не ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ТекстИсключения = НСтр("ka='Нарушение прав доступа: %1';ru='Нарушение прав доступа: %1'");
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ka='მონაცემთა სინქრონიზაცია სხვა პროგრამებთან';ru='Синхронизация данных с другими программами'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСообщения = НСтр("ka='მონაცემთა გაცვლის პროცესის დასაწყისი გასწორების მიხედვით %1';ru='Начало процесса обмена данными по настройке %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбменДаннымиПоРегламентномуЗаданию(КодСценарияОбмена) Экспорт
	...
	Если Не ЗначениеЗаполнено(КодСценарияОбмена) Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა გაცვლის სცენარი მიცემული არ არის.';ru='Не задан сценарий обмена данными.'");
	КонецЕсли;
	...
	Иначе
		СтрокаСообщения = НСтр("ka='მონაცემთა გაცვლის სცენარი კოდით %1 ვერ მოიძებნა.';ru='Сценарий обмена данными с кодом %1 не найден.'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталог(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, ВыводитьСообщения = Истина) Экспорт
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("ka='გაცვლის მიმოსვლის ტრანსპორტის დამუშავების ინიციალიზაციისას წარმოიშვა შეცდომები.';ru='При инициализации обработки транспорта сообщений обмена возникли ошибки.'");
	...
	Если СтруктураНастроекОбмена.РезультатВыполненияОбмена <> Неопределено Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("ka='გაცვლის შეტყობინების მიღებისას შეცდომები წარმოიშვა!';ru='При получении сообщений обмена возникли ошибки.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,,,, Отказ);
		КонецЕсли;
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондента(Отказ, УзелИнформационнойБазы, ВыводитьСообщения = Истина) Экспорт
	...
	Исключение
		Если ВыводитьСообщения Тогда
			Сообщение = НСтр("ka='გაცვლის განხორციელება ვერ მოხერხდა: %1';ru='Не удалось произвести обмен: %1'");
	...
	Если Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			...
			Сообщение = НСтр("ka='მონაცემთა გადმოტვირთვა ვერ მოხერხდა: %1';ru='Не удалось выгрузить данные: %1'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Если Прокси = Неопределено Тогда
		...
		Отказ = Истина;
		Сообщение = НСтр("ka='შეცდომა მეორე საინფორმაციო ბაზასთან შეერთების დაყენებისას: %1';ru='Ошибка при установке подключения ко второй информационной базе: %1'");
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ka='მონაცემთა გადმოტვირთვისას შეცდომები წარმოიშვა მეორე საინფორმაციო ბაზაში: %1';ru='При выгрузке данных возникли ошибки во второй информационной базе: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
	...
	Если ДлительнаяОперация Тогда
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ka='ბაზა-კორესპონდენტისგან მონაცემთა მიღების მოლოდინი...';ru='Ожидание получения данных от базы-корреспондента...'",
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ka='შეცდომები წარმოიშვა გაცვლის შეტყობინების მიღებისას ფაილების გადაცემის სერვისიდან: %1';ru='Возникли ошибки при получении сообщения обмена из сервиса передачи файлов: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
	...
	Исключение
		Отказ = Истина;
		Сообщение = НСтр("ka='გაცვლის შეტყობინების მიღებისას შეცდომები წარმოიშვა: %1';ru='При получении сообщения обмена возникли ошибки: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	Если ПараметрыОбмена.УзелИнформационнойБазы = Неопределено Тогда
		...
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая()
			...
			ИначеЕсли ОшибкаПерехода Тогда
				СтрокаСообщенияОбОшибке = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლის შესრულება ვერ მოხდა.';ru='Не удалось выполнить переход на синхронизацию данных через универсальный формат.'");
				ВызватьИсключение СтрокаСообщенияОбОшибке;
			КонецЕсли;
		КонецЕсли;
		
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая() Тогда
			СтрокаСообщенияОбОшибке = НСтр("ka='გაცვლის გეგმის კვანძი %1 კოდით %2 ვერ მოიძებნა.';ru='Узел плана обмена %1 с кодом %2 не найден.'");
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		СтрокаСообщенияОбОшибке = НСтр("ka='შეცდომა მონაცემთა გაცვლის პროცესის ინიციალიზაციისას.';ru='Ошибка при инициализации процесса обмена данными.'");
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена);
		ВызватьИсключение СтрокаСообщенияОбОшибке;
	КонецЕсли;
		...
		Если СтруктураНастроекОбмена.ИспользоватьПередачуБольшогоОбъемаДанных Тогда
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) И НЕ Отказ Тогда
				...
				Попытка
					...
					Если ПараметрыОбмена.ДлительнаяОперация Тогда
						ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ka='ბაზა-კორესპონდენტში მონაცემთა ჩატვირთვის მოლოდინი...';ru='Ожидание загрузки данных в базе-корреспонденте...'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
						Возврат;
					КонецЕсли;
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыПоВнешнемуСоединению(Отказ, УзелИнформационнойБазы,
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
			...
			ТекстСообщения = НСтр("ka='აუცილებელია ბაზა-კორესპონდენტში უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა.';ru='Необходим переход на синхронизацию данных через универсальный формат в базе-корреспонденте.'");
			...
			Если СтруктураПараметров.Ошибка Тогда
				ЕстьОшибки = Истина;
				НСтрока = НСтр("ka='შეცდომა უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლისას: %1. გაცვლა გაუქმებულია.';ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.'",
			...
			ИначеЕсли РезультатПерехода = Неопределено Тогда
				ЕстьОшибки = Истина;
				СтрокаСообщенияОбОшибке = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლას მხარდაჭერა არ გააჩნია';ru='Переход на синхронизацию данных через универсальный формат не выполнен'");
			КонецЕсли;
			Если ЕстьОшибки Тогда
				...
				Возврат;
			Иначе
				Сообщение = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა წარმატებით დასრულდა.';ru='Переход на синхронизацию данных через универсальный формат завершен успешно.'");
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		Если НЕ ЭтоПланОбменаXDTO Тогда
			...
			Если ПравилаКонвертацииОбъектов = Неопределено Тогда
				...
				НСтрока = НСтр("ka='მეორე საინფორმაციო ბაზაში კონვერტაციის წესები მიცემული არ არის გაცვლის გეგმისთვის %1. გაცვლა გაუქმებულია.';ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Исключение
				ЗаписьЖурналаРегистрацииОбменаДанными(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მეორე საინფორმაციო ბაზაში შეცდომა წარმოიშვა: %1';ru='Возникла ошибка во второй информационной базе: %1'"),
...
Процедура ПередЧтениемСообщенияОбмена(Знач Получатель, СообщениеОбмена, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		...
		Если ТипЗнч(СохраненноеСообщениеОбмена) = Тип("ДвоичныеДанные") Тогда
			...
			ЗаписатьСобытиеПолученияДанных(Получатель, НСтр("ka='გაცვლის შეტყობინება მიღებულია ქეშიდან.';ru='Сообщение обмена получено из кэша.'"));
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Если ЧтениеСообщения.Отправитель <> УзелИнформационнойБазы Тогда // Сообщение предназначено не для этого узла.
		...
		ЗаписьЖурналаРегистрации(КлючСообщенияЖурналаРегистрации, УровеньЖурналаРегистрации.Ошибка,
			УзелИнформационнойБазы.Метаданные(), УзелИнформационнойБазы, НСтр("ka='გაცვლის შეტყობინება შეიცავს მონაცემებს საინფორმაციო ბაზის სხვა კვანძისთვის.';ru='Сообщение обмена содержит данные для другого узла информационной базы.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		//
		Возврат;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა. Web-სერვისსთან მიერთების დაყენება';ru='Обмен данными.Установка подключения к web-сервису'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა. წესების ჩატვირთვა';ru='Обмен данными.Загрузка правил'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა. მონაცემთა გაცვლის შექმნა';ru='Обмен данными.Создание обмена данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა. დროებითი ფაილის წაშლა';ru='Обмен данными.Удаление временного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилищаВСервисе(Знач ИдентификаторФайла, Знач УзелИнформационнойБазы, Знач РазмерЧасти = 1024, Знач ПараметрыАутентификации = Неопределено) Экспорт
	...
	Иначе
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ინტერნეტიდან გაცვლის შეტყობინების მიღების დაწყება (ფაილის ნაწილების რაოდენობა %1).';ru='Начало получения сообщения обмена из Интернета (количество частей файла %1).'"),
		...
		Если Разархиватор.Элементы.Количество() = 0 Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииУдалениеВременногоФайла(),
					УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
			ВызватьИсключение(НСтр("ka='არქივის ფაილი მონაცემებს არ შეიცავს.';ru='Файл архива не содержит данных.'"));
		КонецЕсли;
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ინტერნეტიდან გაცვლის შეტყობინების მიღების დასრულება (გაცვლის შეკუმშული შეტყობინების ზომა %1 მბ).';ru='Окончание получения сообщения обмена из Интернета (размер сжатого сообщения обмена %1 Мб).'"),
...
Процедура ПриПолученииФайлаИзХранилища(Знач ИдентификаторФайла, ИмяФайла)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Описание = НСтр("ka='ფაილი იდენტიფიკატორ %1-ით ვერ მოიძებნა.';ru='Файл с идентификатором %1 не обнаружен.'");
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ПредставлениеИсключения = НСтр("ka='დაშვების უფლებების დარღვევა!';ru='Нарушение прав доступа!'");
		ТекстИсключения = НСтр("ka='დაშვების უფლებების დარღვევა!';ru='Нарушение прав доступа!'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными'");
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='არ არის მიცემული საინფორმაციო ბაზის კვანძი, რომელთან უნდა განხორციელდეს ინფორმაციის გაცვლა. გაცვლა გაუქმებულია.';ru='Не задан узел информационной базы с которым нужно производить обмен информацией. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ВидТранспортаОбмена) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='გაცვლის ტრანსპორტის ტიპი არ არის მიცემული. გაცვლა გაუქმებულია.';ru='Не задан вид транспорта обмена. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ДействиеПриОбмене) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='მითითებული არ არის, თუ რომელი მოქმედების შესრულება ხორციელდება (გადმოტვირთვა / ჩატვირთვა). გაცვლა გაუქმებულია.';ru='Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='საინფორმაციო ბაზის კვანძი მონიშნულია წასაშლელად. გაცვლა გაუქმებულია.';ru='Узел информационной базы помечен на удаление. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='საინფორმაციო ბაზის მიმდინარე კვანძთან მონაცემთა გაცვლის ორგანიზება არ შეიძლება. გაცვლა გაუქმებულია.';ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли ПустаяСтрока(СтруктураНастроекОбмена.УзелИнформационнойБазыКод)
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='გაცვლის ერთ-ერთ კვანძს გააჩნია ცარიელი კოდი. გაცვლა გაუქმებულია.';ru='Один из узлов обмена имеет пустой код. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='გადმოტვირთვის გასამართად გარე დამუშავების ფაილი არ არსებობს. გაცვლა გაუქმებულია.';ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковЗагрузки Тогда
		...
		Если Не ФайлОбработкиЗагрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='გარე დამუშავების ფაილი ჩატვირთვის გასამართად არ არსებობს. გაცვლა გაუქმებულია.';ru='Файл внешней обработки для отладки загрузки не существует. Обмен отменен.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура УстановитьПравилаОбменаВыгрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ka='არ არის მითითებული კონვერტაციის წესები გაცვლის გეგმა %1-ისთვის. მონაცემთა გადმოტვირთვა გაუქმებულია.';ru='Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ka='გაცვლის გეგმა %1-ისთვის კონვერტაციის წესები არ არის მიცემული. მონაცემთა ჩატვირთვა გაუქმებულია.';ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(
	...
	Если Не ПустаяСтрока(СокрЛП(ТекстСообщения)) Тогда
		...
		ПричинаОшибки = " " + НСтр("ka='%1-ის მიზეზით.';ru='По причине %1.'");
...
Процедура ПроверитьВозможностьАдминистрированияОбменов() Экспорт
	...
	Если Не ЕстьПраваНаАдминистрированиеОбменов() Тогда
		...
		ВызватьИсключение НСтр("ka='მონაცემთა სინქრონიზაციის ადმინისტრირებისთვის არასაკმარისი უფლებებია.';ru='Недостаточно прав для администрирования синхронизации данных.'");
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда
		СтрокаСообщения = НСтр("ka='%1, %2';ru='%1, %2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		СтрокаСообщения = НСтр("ka='%1, %2; დამუშავებულია ობიექტი: %3';ru='%1, %2; Объектов обработано: %3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция РаспаковатьZipФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ka='შეცდომა არქივის ფაილების განფუთვისას: %1 კატალოგში: %2';ru='Ошибка при распаковке файлов архива: %1 в каталог: %2'");
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МаскаУпаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ka='შეცდომა არქივის ფაილების შეფუთვისას: %1 კატალოგიდან: %2';ru='Ошибка при запаковке файлов архива: %1 из каталог: %2'");
...
Функция ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("ka='მონაცემთა გაცვლა.[გაცვლისგეგმისსახელი].[მოქმედებაგაცვლისას]';ru='Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьОбязательныеРеквизитыФормы(Форма, Знач Реквизиты)
	...
	Если ОтсутствующиеРеквизиты.Количество() > 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კვანძის გამართვის ფორმის სავალდებულო რეკვიზიტები არ არის: %1';ru='Отсутствуют обязательные реквизиты формы настройки узла: %1'"),
...
Процедура ВнешнееСоединениеОбновитьНастройкиОбменаДанными(Знач ИмяПланаОбмена, Знач КодУзла, Знач ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		Сообщение = НСтр("ka='გაცვლის გეგმის კვანძი ვერ მოიძებნა; გაცვლის გეგმის სახელი %1; კვანძის კოდი %2';ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2'");
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ, СтруктураНастроек, ВидТранспорта, СообщениеОбОшибке = "", НовыйПароль = Неопределено) Экспорт
	...
	Если Не ОбработкаОбъект.ПодключениеУстановлено() Тогда
		...
		ДополнительноеСообщение = НСтр("ka='შეცდომის შესახებ ტექნიკური ინფორმაცია იხ. რეგისტრაციის ჟურნალში';ru='Техническую информацию об ошибке см. в журнале регистрации.'");
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='გაცვლის მიმოსვლის ტრანსპორტი';ru='Транспорт сообщений обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("ka='სხვა პროგრამაზე შესაერთებლად მითითებულ მომხმარებელს უნდა დაენიშნოს როლი ""სისტემის ადმინისტრატორი"" და ""სრული უფლებები""';ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""'");
	...
	Иначе
		...
		Если СостояниеНеДопустимо Тогда
			Результат.ПодробноеОписаниеОшибки = НСтр("ka='სხვა პროგრამა განახლების მდგომარეობაში იმყოფება.';ru='Другая программа находится в состоянии обновления.'");
			Результат.КраткоеОписаниеОшибки   = Результат.ПодробноеОписаниеОшибки;
			Результат.Соединение = Неопределено;
		КонецЕсли;
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент, Знач СтруктураНастроек, СообщениеПользователю = "") Экспорт
	...
	СобытиеЖурналаРегистрации = НСтр("ka='მონაცემთა გაცვლა. შეერთების შემოწმება';ru='Обмен данными.Проверка подключения'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Для Каждого ЗаписьПланаОбмена Из Результат Цикл
		...
		Если ПравилаОбменаЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-ისთვის მიმდინარეობს მონაცემთა კონვერტაციის წესების განახლება';ru='Выполняется обновление правил конвертации данных для плана обмена %1'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
		...
		Если ПравилаРегистрацииЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-სთვის მიმდინარეობს მონაცემთა რეგისტრაციის წესების განახლება';ru='Выполняется обновление правил регистрации данных для плана обмена %1'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
				УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა გაცვლის წესების განახლებისას შეცდომები წარმოიშვა (იხ. რეგისტრაციის ჟურნალი).';ru='При обновлении правил обмена данными возникли ошибки (см. Журнал регистрации).'");
	КонецЕсли;
	...
	Если Файл.Существует() И Файл.ЭтоФайл() Тогда
		...
		Если МаксимальныйДопустимыйРазмерСообщения <> 0 Тогда
			...
			Если РазмерПакета > МаксимальныйДопустимыйРазмерСообщения Тогда
				...
				СтрокаСообщения = НСтр("ka='გამავალი პაკეტის ძემამ შეადგინა %1 კბაიტი, რაც აღემატება დასაშვებ ზღვარს %2 კბაიტს.';ru='Размер исходящего пакета составил %1 Кбайт, что превышает допустимое ограничение %2 Кбайт.'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Если УзелИнформационнойБазы <> Неопределено Тогда
			...
			Попытка
				...
				Если Отказ Тогда
					ВызватьИсключение НСтр("ka='მთავარი კვანძიდან მონაცემთა მიღება შეცდომებით დასრულდა.';ru='Получение данных из главного узла завершилось с ошибками.'");
				КонецЕсли;
			Исключение
				УстановитьПривилегированныйРежим(Истина);
				УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='გაფრთხილებები სინქრონიზაციის დროს';ru='Предупреждения при синхронизации'");
...
Процедура ПриЗаполненииСпискаТекущихДелНеобходимоОбновление(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Истина;
		Дело.Представление  = НСтр("ka='პროგრამის ვერსიის განახლება';ru='Обновить версию программы'");
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("ka='განახლების დაყენება';ru='Установка обновления'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
...
Процедура ПриЗаполненииСпискаТекущихДелПроверитьСовместимостьСТекущейВерсией(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='გაცვლის წესები';ru='Правила обмена'");
		...
		Если ГруппаДела = Неопределено Тогда
			...
			ГруппаДела.Представление = НСтр("ka='შესაბამისობის შემოწმება';ru='Проверить совместимость'");
...
Процедура УстановитьСообщениеОбменаДаннымиИзГлавногоУзла(СообщениеОбмена, ГлавныйУзел) Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел, НСтр("ka='გაცვლის შეტყობინება ჩაწერილია ქეშში.';ru='Сообщение обмена записано в кэш.'"));
...
Процедура ОчиститьСообщениеОбменаДаннымиИзГлавногоУзла() Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел(), НСтр("ka='გაცვლის შეტყობინება ქეშიდან წაშლილია.';ru='Сообщение обмена удалено из кэша.'"));
...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ka='მონაცემთა სინქრონიზაციის გამართვა';ru='Настройка синхронизация данных'"));
	Результат.Вставить("Родительный", НСтр("ka='მონაცემთა სინქრონიზაციის გამართვები';ru='Настройки синхронизации данных'"));
...
Функция ИнтерактивноеИзменениеВыгрузки(Знач УзелИнформационнойБазы, Знач АдресХранилищаФормы, Знач ЕстьСценарийУзла=Неопределено) Экспорт
	...
	ПараметрыСценарияДополнения.ВариантБезДополнения.Вставить("Пояснение", 
		НСтр("ka='გაიგზავნება მხოლოდ მონაცემები საერთო გამართვების თანახმად.';ru='Будут отправлены только данные согласно общим настройкам.'")); 
	...
	ПараметрыСценарияДополнения.ВариантВсеДокументы.Вставить("Пояснение",
		НСтр("ka='დამატებით გაიგზავნება შერჩევის პირობების დამაკმაყოფილებელი პერიოდის ყველა დოკუმენტი.';ru='Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.'")); 
	...
	ПараметрыСценарияДополнения.ВариантПроизвольныйОтбор.Вставить("Пояснение",
		НСтр("ka='დამატებით გაიგზავნება მონაცემები შერჩევის თანახმად.';ru='Дополнительно будут отправлены данные согласно отбору.'")); 
	...
	ПараметрыСценарияДополнения.ВариантДополнительно.Вставить("Пояснение",
		НСтр("ka='გაიგზავნება გამართვების დამატებითი მონაცემები.';ru='Будут отправлены дополнительные данные по настройкам.'")); 
...
Функция ПредставлениеОтбораДополненияВыгрузки(Знач Период, Знач Отбор, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	Если ПустаяСтрока(ОтборСтрокой) Тогда
		Если ОписаниеПустогоОтбора=Неопределено Тогда
			ОтборСтрокой = НСтр("ka='ყველა ობიექტი';ru='Все объекты'");
		Иначе
...
Функция ПредставлениеДетальногоДополненияВыгрузки(Знач ДополнительнаяРегистрация, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	ИначеЕсли ОписаниеПустогоОтбора=Неопределено Тогда
		Возврат НСтр("ka='დამატებითი მონაცემები არ არის არჩეული';ru='Дополнительные данные не выбраны'");
...
Функция ДополнениеВыгрузкиИмяАвтоСохраненияНастроек() Экспорт
	Возврат НСтр("ka='უკანასკნელი გაგზავნა (ინახება ავტომატურად)';ru='Последняя отправка (сохраняется автоматически)'");
КонецФункции
