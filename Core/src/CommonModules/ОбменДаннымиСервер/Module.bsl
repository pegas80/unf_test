...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	СтрокаСообщения = НСтр("ka='მონაცემთა გაცვლის პროცესის დასაწყისი შემდეგი კვანძისთვის %1';ru='Начало процесса обмена данными для узла %1';en='Starting the process of data exchange node 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СтруктураЗаголовкаГиперссылкиМонитораПроблем(Узлы = Неопределено) Экспорт
	...
	Если Количество > 0 Тогда
		...
		Заголовок = НСтр("ka='გაფრთხილება (%1)';ru='Предупреждения (%1)';en='Cards (1%)'");
	...
	Иначе
		...
		Заголовок = НСтр("ka='გაფრთხილებები არ არის';ru='Предупреждений нет';en='no warning'");
...
"Реквизит ""%1"".';ka='სტანდარტული მნიშვნელობებისთვის მონაცემთა შემადგენელ ტიპს მხარდაჭერა არ გააჩნია. რეკვიზიტი ""%1"".'");
			...
			Если Не ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) Тогда
				...
				СтрокаСообщения = НСтр("ka='მნიშვნელობების არჩევას სტანდარტულად მხარდაჭერა გააჩნია მხოლოდ ცნობარებისთვის. რეკვიზიტი ""%1"".';ru='Выбор значений по умолчанию поддерживается только для справочников."
"Реквизит ""%1"".'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Реквизит.ПолноеИмя());
				ВызватьИсключение СтрокаСообщения;
			КонецЕсли;
...
Функция ПредставлениеДатыСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		Возврат НСтр("ka='სინქრონიზაცია არ შესრულებულა.';ru='Синхронизация не выполнялась.';en='Never synced.'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უკანასკნელი სინქრონიზაცია: %1';ru='Последняя синхронизация: %1';en='Last sync:% 1'"), ОтносительнаяДатаСинхронизации(ДатаСинхронизации));
КонецФункции
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		...
		Возврат НСтр("ka='არასდროს';ru='Никогда';en='Never'");
	...
	ИначеЕсли Интервал < 60 * 5 Тогда // 5 мин
		...
		Результат = НСтр("ka='ეხლა';ru='Сейчас';en='Now'");
		
	ИначеЕсли Интервал < 60 * 15 Тогда // 15 мин
		
		Результат = НСтр("ka='5 წუთის წინ';ru='5 минут назад';en='5 minutes ago'");
		
	ИначеЕсли Интервал < 60 * 30 Тогда // 30 мин
		
		Результат = НСтр("ka='15 წუთის წინ';ru='15 минут назад';en='15 minutes ago'");
		
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		
		Результат = НСтр("ka='30 წუთის წინ';ru='30 минут назад';en='30 minutes ago'");
		
	ИначеЕсли Интервал < 60 * 60 * 2 Тогда // 2 часа
		
		Результат = НСтр("ka='1 საათის უკან';ru='1 час назад';en='1 hour ago'");
		
	ИначеЕсли Интервал < 60 * 60 * 3 Тогда // 3 часа
		
		Результат = НСтр("ka='2 საათის წინ';ru='2 часа назад';en='2 hours before'");
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დღეს, 51';ru='Сегодня, %1';en='Today, the 1%'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გუშინ,%1';ru='Вчера, %1';en='Yesterday, 1%'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გუშინწინ, %1';ru='Позавчера, %1';en='The day before yesterday, 1%'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
...
Функция НомерОбластиИзКодаУзлаПланаОбмена(Знач КодУзла) Экспорт
	...
	Если ТипЗнч(КодУзла) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ka='ნომერ [1] პარამეტრის არასწორი ტიპი.';ru='Неправильный тип параметра номер [1].';en='Wrong type of parameter number [1].'");
	КонецЕсли;
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მონაცემთა გაცვლა. პრიორიტეტული მონაცემების ჩატვირთვა';ru='Обмен данными.Загрузка приоритетных данных';en='Exchange dannymi.Zagruzka priority data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ВызватьИсключение
			НСтр("ru='Ошибка загрузки приоритетных данных из сообщения обмена."
"См. подробности в журнале регистрации.';ka='Ошибка загрузки приоритетных данных из сообщения обмена."
"См. подробности в журнале регистрации.'");
	...
	Если Отказ Тогда
		...
		Если КонфигурацияИзменена() Тогда
			ВызватьИсключение
				НСтр("ka='მთავარი კვანძიდან მიღებული პროგრამის ცვლილებები ჩატვირთულია."
"დაასრულეთ პროგრამის მუშაობა. კონფიგურატორში გახსენით პროგრამა და შეასრულეთ ბრძანება ""მონაცემთა ბაზის კონფიგურაციის განახლება"" (F7)""."
""
"ამის შემდეგ, გაუშვით პროგრამა.';ru='Загружены изменения программы, полученные из главного узла."
...
Процедура ЗаписатьДанныеДляОбновленияВФайл(Параметры, Данные, ВидДанных, ПолноеИмяОбъекта = "") Экспорт
	...
	Если Параметры.ЗаписьИзмененийДляПодчиненногоУзлаРИБСФильтрами = Неопределено Тогда
		ТекстИсключения = НСтр("ka='დამუშავებაში არასწორად არის ორგანიზებული დამუშავებზე მონაცემთა რეგისტრაციის პარამეტრებთან მუშაობა.';ru='В обработчике неправильно организована работа с параметрами регистрации данных к обработке.';en='The processor is organized correctly work with the parameters registration data to be processed.'");
...
Процедура ОбновитьРегламентныеЗаданияСценариевОбменовДанными() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ka='შეცდომა სარეგლამენტო დავალების განახლებისას გაცვლის მონაცემთა სცენარისთვის.';ru='Ошибка при обновлении регламентного задания для сценария обмена данными.';en='Error updating scheduled jobs for data exchange scenarios.'");
		КонецЕсли;
...
Процедура РассчитатьПроцентВыгрузки(КоличествоВыгруженных, КоличествоОбъектовКВыгрузке) Экспорт
	...
	Если КоличествоОбъектовКВыгрузке = 0 Или КоличествоВыгруженных > КоличествоОбъектовКВыгрузке Тогда
		...
		Шаблон = НСтр("ka='Обработано: %1 объектов.';ru='Обработано: %1 объектов.';en='Processed% 1 objects.'");
	...
	Иначе
		...
		Шаблон = НСтр("ka='Обработано: %1 из %2 объектов.';ru='Обработано: %1 из %2 объектов.';en='Processed% 1% 2 objects.'");
...
Процедура РассчитатьПроцентЗагрузки(КоличествоЗагруженных, КоличествоОбъектовКЗагрузке, РазмерФайлаСообщенияОбмена) Экспорт
	...
	Если РазмерФайлаСообщенияОбмена <> 0 Тогда
		Шаблон = НСтр("ka='Размер сообщения %1 МБ';ru='Размер сообщения %1 МБ';en='The size of the message% 1 MB'");
...
Функция FTPИмяСервераИПуть(Знач СтрокаПодключения) Экспорт
	...
	Если (ВРег(Лев(СтрокаПодключения, 6)) <> "FTP://"
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';ru='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';en='The connection string to the FTP-resource does not conform to the format: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
	...
	Если ПараметрыПодключения.Количество() < 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='FTP-რესურსთან შეერთების სტრიქონში სერვერის სახელი არ არის მითითებული: ""%1""';ru='В строке подключения к FTP-ресурсу не указано имя сервера: ""%1""';en='In line connection to the FTP-resource, not a server name: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
...
Функция ВыбратьИзменения(Знач Узел, Знач НомерСообщения, Знач ФильтрВыборки = Неопределено) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა ცვლილების ამოკრება აქტიურ ტრანზაქციაში აკრძალულია.';ru='Выборка изменений данных запрещена в активной транзакции.';en='Sampling of these changes is prohibited in the active transaction.'");
	КонецЕсли;
...
Функция НастройкиПланаОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ЗаголовокФормыПомощника = НСтр("ka='მონაცემთა სინქრონიზაცია %Программа%-სთან (გამართვა)';ru='Синхронизация данных с %Программа% (настройка)';en='Data synchronization with the% Program% (setting)'");
...
Процедура ПроверитьВозможностьВыполненияОбменов() Экспорт
	...
	Если Не Пользователи.РолиДоступны("ВыполнениеСинхронизацииДанных") Тогда
		...
		ВызватьИсключение НСтр("ru='Недостаточно прав для синхронизации данных.';en='Not have permission to synchronize data.'");
	...
	ИначеЕсли ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
		...
		ВызватьИсключение НСтр("ka='საინფორმაციო ბაზა იმყოფება განახლების მდგომარეობაში.';ru='Информационная база находится в состоянии обновления.';en='The information base is up to date.'");
...
Процедура ПроверитьИспользованиеОбменаДанными() Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		ТекстСообщения = НСтр("ka='Синхронизация данных запрещена администратором.';ru='Синхронизация данных запрещена администратором.';en='Data synchronization is prohibited by the administrator.'");
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект) Экспорт
	...
	Для Каждого СтрокаДвижения Из ТаблицаДвижений Цикл
		...
		Если Не ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ТекстИсключения = НСтр("ka='Нарушение прав доступа: %1';ru='Нарушение прав доступа: %1';en='An access violation:% 1'");
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ka='მონაცემთა სინქრონიზაცია სხვა პროგრამებთან';ru='Синхронизация данных с другими программами';en='Synchronize data with other programs'", Метаданные.ОсновнойЯзык.КодЯзыка);
	ОписаниеПрофиля.Описание =
		НСтр("ka='დამატებით ენიშნება იმ მომხმარებლებს, რომლებისთვის ხელმისაწვდომი უნდა იყოს სხვა პროგრამებთან მონაცემთა მონიტორინგის და სინქრონიზაციის საშუალებები.';ru='Дополнительно назначается тем пользователям, которым должны быть доступны средства"
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСообщения = НСтр("ka='მონაცემთა გაცვლის პროცესის დასაწყისი გასწორების მიხედვით %1';ru='Начало процесса обмена данными по настройке %1';en='Start the data exchange process for setting up 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбменДаннымиПоРегламентномуЗаданию(КодСценарияОбмена) Экспорт
	...
	Если Не ЗначениеЗаполнено(КодСценарияОбмена) Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა გაცვლის სცენარი მიცემული არ არის.';ru='Не задан сценарий обмена данными.';en='Do not set the script data exchange.'");
	КонецЕсли;
	...
	Иначе
		СтрокаСообщения = НСтр("ka='მონაცემთა გაცვლის სცენარი კოდით %1 ვერ მოიძებნა.';ru='Сценарий обмена данными с кодом %1 не найден.';en='data exchange scenario with code% 1 was not found.'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталог(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, ВыводитьСообщения = Истина) Экспорт
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("ka='გაცვლის მიმოსვლის ტრანსპორტის დამუშავების ინიციალიზაციისას წარმოიშვა შეცდომები.';ru='При инициализации обработки транспорта сообщений обмена возникли ошибки.';en='When the initialization processing Transport exchange any error messages.'");
	...
	Если СтруктураНастроекОбмена.РезультатВыполненияОбмена <> Неопределено Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("ka='გაცვლის შეტყობინების მიღებისას შეცდომები წარმოიშვა!';ru='При получении сообщений обмена возникли ошибки.';en='When you receive a message exchange any errors.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,,,, Отказ);
		КонецЕсли;
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондента(Отказ, УзелИнформационнойБазы, ВыводитьСообщения = Истина) Экспорт
	...
	Исключение
		Если ВыводитьСообщения Тогда
			Сообщение = НСтр("ka='გაცვლის განხორციელება ვერ მოხერხდა: %1';ru='Не удалось произвести обмен: %1';en='Unable to make an exchange: 1%'");
	...
	Если Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			...
			Сообщение = НСтр("ka='მონაცემთა გადმოტვირთვა ვერ მოხერხდა: %1';ru='Не удалось выгрузить данные: %1';en='Unable to upload the data:% 1'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Если Прокси = Неопределено Тогда
		...
		Отказ = Истина;
		Сообщение = НСтр("ka='შეცდომა მეორე საინფორმაციო ბაზასთან შეერთების დაყენებისას: %1';ru='Ошибка при установке подключения ко второй информационной базе: %1';en='An error occurred while establishing the connection to the second information database% 1'");
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ka='მონაცემთა გადმოტვირთვისას შეცდომები წარმოიშვა მეორე საინფორმაციო ბაზაში: %1';ru='При выгрузке данных возникли ошибки во второй информационной базе: %1';en='When unloading data errors occurred in the second information database% 1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
	...
	Если ДлительнаяОперация Тогда
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ka='ბაზა-კორესპონდენტისგან მონაცემთა მიღების მოლოდინი...';ru='Ожидание получения данных от базы-корреспондента...';en='Waiting for receiving data from the base-correspondent ...'",
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ka='შეცდომები წარმოიშვა გაცვლის შეტყობინების მიღებისას ფაილების გადაცემის სერვისიდან: %1';ru='Возникли ошибки при получении сообщения обмена из сервиса передачи файлов: %1';en='An error occurred while getting messages from the exchange file transfer service:% 1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
	...
	Исключение
		Отказ = Истина;
		Сообщение = НСтр("ka='გაცვლის შეტყობინების მიღებისას შეცდომები წარმოიშვა: %1';ru='При получении сообщения обмена возникли ошибки: %1';en='When you receive a message exchange errors occurred:% 1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	Если ПараметрыОбмена.УзелИнформационнойБазы = Неопределено Тогда
		...
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая()
			...
			ИначеЕсли ОшибкаПерехода Тогда
				СтрокаСообщенияОбОшибке = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლის შესრულება ვერ მოხდა.';ru='Не удалось выполнить переход на синхронизацию данных через универсальный формат.';en='Unable to switch to synchronize data through a universal format.'");
				ВызватьИсключение СтрокаСообщенияОбОшибке;
			КонецЕсли;
		КонецЕсли;
		
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая() Тогда
			СтрокаСообщенияОбОшибке = НСтр("ka='გაცვლის გეგმის კვანძი %1 კოდით %2 ვერ მოიძებნა.';ru='Узел плана обмена %1 с кодом %2 не найден.';en='Node sharing plan% 1% 2 with code not found.'");
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		СтрокаСообщенияОбОшибке = НСтр("ka='შეცდომა მონაცემთა გაცვლის პროცესის ინიციალიზაციისას.';ru='Ошибка при инициализации процесса обмена данными.';en='Error initializing the data exchange process.'");
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена);
		ВызватьИсключение СтрокаСообщенияОбОшибке;
	КонецЕсли;
		...
		Если СтруктураНастроекОбмена.ИспользоватьПередачуБольшогоОбъемаДанных Тогда
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) И НЕ Отказ Тогда
				...
				Попытка
					...
					Если ПараметрыОбмена.ДлительнаяОперация Тогда
						ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ka='ბაზა-კორესპონდენტში მონაცემთა ჩატვირთვის მოლოდინი...';ru='Ожидание загрузки данных в базе-корреспонденте...';en='Waiting for the download data in the correspondent ...'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
						Возврат;
					КонецЕсли;
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыПоВнешнемуСоединению(Отказ, УзелИнформационнойБазы,
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
			...
			ТекстСообщения = НСтр("ka='აუცილებელია ბაზა-კორესპონდენტში უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა.';ru='Необходим переход на синхронизацию данных через универсальный формат в базе-корреспонденте.';en='Requires transfer to synchronize data via a universal format based correspondent.'");
			...
			Если СтруктураПараметров.Ошибка Тогда
				ЕстьОшибки = Истина;
				НСтрока = НСтр("ka='შეცდომა უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლისას: %1. გაცვლა გაუქმებულია.';ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';en='Error while switching to synchronize data through a universal format:% 1. Exchange canceled.'",
			...
			ИначеЕсли РезультатПерехода = Неопределено Тогда
				ЕстьОшибки = Истина;
				СтрокаСообщенияОбОшибке = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლას მხარდაჭერა არ გააჩნია';ru='Переход на синхронизацию данных через универсальный формат не выполнен';en='Daylight data synchronization via a universal format is not formed'");
			КонецЕсли;
			Если ЕстьОшибки Тогда
				...
				Возврат;
			Иначе
				Сообщение = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა წარმატებით დასრულდა.';ru='Переход на синхронизацию данных через универсальный формат завершен успешно.';en='Transition to synchronize data through a universal format has been successfully completed.'");
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		Если НЕ ЭтоПланОбменаXDTO Тогда
			...
			Если ПравилаКонвертацииОбъектов = Неопределено Тогда
				...
				НСтрока = НСтр("ka='მეორე საინფორმაციო ბაზაში კონვერტაციის წესები მიცემული არ არის გაცვლის გეგმისთვის %1. გაცვლა გაუქმებულია.';ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';en='Do not set the conversion rules for the second information base for the exchange of 1% of the plan. Exchange canceled.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Исключение
				ЗаписьЖурналаРегистрацииОбменаДанными(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მეორე საინფორმაციო ბაზაში შეცდომა წარმოიშვა: %1';ru='Возникла ошибка во второй информационной базе: %1';en='There was an error in the second information database% 1'"),
...
Процедура ПередЧтениемСообщенияОбмена(Знач Получатель, СообщениеОбмена, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		...
		Если ТипЗнч(СохраненноеСообщениеОбмена) = Тип("ДвоичныеДанные") Тогда
			...
			ЗаписатьСобытиеПолученияДанных(Получатель, НСтр("ka='გაცვლის შეტყობინება მიღებულია ქეშიდან.';ru='Сообщение обмена получено из кэша.';en='Message exchange is obtained from the cache.'"));
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Если ЧтениеСообщения.Отправитель <> УзелИнформационнойБазы Тогда // Сообщение предназначено не для этого узла.
		...
		ЗаписьЖурналаРегистрации(КлючСообщенияЖурналаРегистрации, УровеньЖурналаРегистрации.Ошибка,
			УзелИнформационнойБазы.Метаданные(), УзелИнформационнойБазы, НСтр("ka='გაცვლის შეტყობინება შეიცავს მონაცემებს საინფორმაციო ბაზის სხვა კვანძისთვის.';ru='Сообщение обмена содержит данные для другого узла информационной базы.';en='exchange message contains data for other node information base.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		//
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка Тогда
			ВызватьИсключение(НСтр("ru='В процессе получения данных возникли ошибки.';en='In the process of data acquisition errors occurred.'"));
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა. Web-სერვისსთან მიერთების დაყენება';ru='Обмен данными.Установка подключения к web-сервису';en='Exchange dannymi.Ustanovka connect to the web-service'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა. წესების ჩატვირთვა';ru='Обмен данными.Загрузка правил';en='Exchange rules dannymi.Zagruzka'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა. მონაცემთა გაცვლის შექმნა';ru='Обмен данными.Создание обмена данными';en='Exchange dannymi.Sozdanie data exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა. დროებითი ფაილის წაშლა';ru='Обмен данными.Удаление временного файла';en='Exchange dannymi.Udalenie temporary file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными';en='Data exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилищаВСервисе(Знач ИдентификаторФайла, Знач УзелИнформационнойБазы, Знач РазмерЧасти = 1024, Знач ПараметрыАутентификации = Неопределено) Экспорт
	...
	Иначе
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ინტერნეტიდან გაცვლის შეტყობინების მიღების დაწყება (ფაილის ნაწილების რაოდენობა %1).';ru='Начало получения сообщения обмена из Интернета (количество частей файла %1).';en='Start receiving the message exchange from the Internet (the number of parts of the file% 1).'"),
		...
		Если Разархиватор.Элементы.Количество() = 0 Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииУдалениеВременногоФайла(),
					УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
			ВызватьИсключение(НСтр("ka='არქივის ფაილი მონაცემებს არ შეიცავს.';ru='Файл архива не содержит данных.';en='The archive file contains no data.'"));
		КонецЕсли;
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ინტერნეტიდან გაცვლის შეტყობინების მიღების დასრულება (გაცვლის შეკუმშული შეტყობინების ზომა %1 მბ).';ru='Окончание получения сообщения обмена из Интернета (размер сжатого сообщения обмена %1 Мб).';en='End of receipt of the message exchange from the Internet (the size of the compressed message sharing% 1 MB).'"),
...
Процедура ПриПолученииФайлаИзХранилища(Знач ИдентификаторФайла, ИмяФайла)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Описание = НСтр("ka='ფაილი იდენტიფიკატორ %1-ით ვერ მოიძებნა.';ru='Файл с идентификатором %1 не обнаружен.';en='File ID% 1 was not found.'");
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ПредставлениеИсключения = НСтр("ka='დაშვების უფლებების დარღვევა!';ru='Нарушение прав доступа!';en='Access Violation!'");
		ТекстИсключения = НСтр("ka='დაშვების უფლებების დარღვევა!';ru='Нарушение прав доступа!';en='Access Violation!'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными';en='Data exchange'");
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='არ არის მიცემული საინფორმაციო ბაზის კვანძი, რომელთან უნდა განხორციელდეს ინფორმაციის გაცვლა. გაცვლა გაუქმებულია.';ru='Не задан узел информационной базы с которым нужно производить обмен информацией. Обмен отменен.';en='Do not set the node information base with which to exchange information. Exchange canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ВидТранспортаОбмена) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='გაცვლის ტრანსპორტის ტიპი არ არის მიცემული. გაცვლა გაუქმებულია.';ru='Не задан вид транспорта обмена. Обмен отменен.';en='Do not set the form of exchange of transport. Exchange canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ДействиеПриОбмене) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='მითითებული არ არის, თუ რომელი მოქმედების შესრულება ხორციელდება (გადმოტვირთვა / ჩატვირთვა). გაცვლა გაუქმებულია.';ru='Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.';en='Not Specified performed action (upload / download). Exchange canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='საინფორმაციო ბაზის კვანძი მონიშნულია წასაშლელად. გაცვლა გაუქმებულია.';ru='Узел информационной базы помечен на удаление. Обмен отменен.';en='Node information database is marked for deletion. Exchange canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='საინფორმაციო ბაზის მიმდინარე კვანძთან მონაცემთა გაცვლის ორგანიზება არ შეიძლება. გაცვლა გაუქმებულია.';ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.';en='You can not exchange data with the current node information base. Exchange canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли ПустаяСтрока(СтруктураНастроекОбмена.УзелИнформационнойБазыКод)
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='გაცვლის ერთ-ერთ კვანძს გააჩნია ცარიელი კოდი. გაცვლა გაუქმებულია.';ru='Один из узлов обмена имеет пустой код. Обмен отменен.';en='One of the exchange nodes has an empty code. Exchange canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='გადმოტვირთვის გასამართად გარე დამუშავების ფაილი არ არსებობს. გაცვლა გაუქმებულია.';ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';en='external processing file for debugging discharge does not exist. Exchange canceled.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковЗагрузки Тогда
		...
		Если Не ФайлОбработкиЗагрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='გარე დამუშავების ფაილი ჩატვირთვის გასამართად არ არსებობს. გაცვლა გაუქმებულია.';ru='Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';en='external processing file for debugging boot does not exist. Exchange canceled.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура УстановитьПравилаОбменаВыгрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ka='არ არის მითითებული კონვერტაციის წესები გაცვლის გეგმა %1-ისთვის. მონაცემთა გადმოტვირთვა გაუქმებულია.';ru='Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';en='Do not set the conversion rules for the exchange of 1% of the plan. Upload data canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ka='გაცვლის გეგმა %1-ისთვის კონვერტაციის წესები არ არის მიცემული. მონაცემთა ჩატვირთვა გაუქმებულია.';ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';en='Do not set the conversion rules for the exchange of 1% of the plan. Loading data canceled.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(
	...
	Если Не ПустаяСтрока(СокрЛП(ТекстСообщения)) Тогда
		...
		ПричинаОшибки = " " + НСтр("ka='%1-ის მიზეზით.';ru='По причине %1.';en='Due to the% 1.'");
...
"Возможно не заполнены все реквизиты, обязательные к заполнению.';ka='სხვა საინფორმაციო ბაზიდან მიღებული დოკუმენტ %1-ის გატარება ვერ მოხერხდა.%2 შეიძლება, რომ სავალდებულოდ შესავსები ყველა რეკვიზიტი არ არის შევსებული.'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуЗаписиОбъекта(Объект, УзелОбмена, ТекстИсключения) Экспорт
	...
	СтрокаСообщения = НСтр("ka='სხვა საინფორმაციო ბაზიდან მიღებული ობიექტ %1-ის ჩაწერა ვერ მოხერხდა.%2 შეიძლება, რომ სავალდებულოდ შესავსები ყველა რეკვიზიტი არ არის შევსებული.';ru='Не удалось записать объект %1, полученный из другой информационной базы.%2"
...
Процедура ПроверитьВозможностьАдминистрированияОбменов() Экспорт
	...
	Если Не ЕстьПраваНаАдминистрированиеОбменов() Тогда
		...
		ВызватьИсключение НСтр("ka='მონაცემთა სინქრონიზაციის ადმინისტრირებისთვის არასაკმარისი უფლებებია.';ru='Недостаточно прав для администрирования синхронизации данных.';en='Not have permission to administer the data synchronization.'");
...
"Для синхронизации данных через прямое подключение требуется использовать ОС Windows.';ka='მონაცემთა სინქრონიზაცია საოპერაციო სისტემა Linux-ის მართვის ქვეშ მყოფ სერვერზე პირდაპირი შეერთებით მიუწვდომელია."
"პირდაპირი შეერთებით მონაცემთა სინქრონიზაციისთვის, საჭირო არის საოპერაციო სისტემა Windows-ის გამოყენება.'");
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда
		СтрокаСообщения = НСтр("ka='%1, %2';ru='%1, %2';en='%12'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		СтрокаСообщения = НСтр("ka='%1, %2; დამუშავებულია ობიექტი: %3';ru='%1, %2; Объектов обработано: %3';en='%12; Objects processed: 3%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция РаспаковатьZipФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ka='შეცდომა არქივის ფაილების განფუთვისას: %1 კატალოგში: %2';ru='Ошибка при распаковке файлов архива: %1 в каталог: %2';en='Error extracting archive file% 1 in directory% 2'");
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МаскаУпаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ka='შეცდომა არქივის ფაილების შეფუთვისას: %1 კატალოგიდან: %2';ru='Ошибка при запаковке файлов архива: %1 из каталог: %2';en='Error while zapakovke archive files:% 1 of the catalog: 2%'");
...
Функция ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("ka='მონაცემთა გაცვლა.[გაცვლისგეგმისსახელი].[მოქმედებაგაცვლისას]';ru='Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]';en='Data exchange. [ImyaPlanaObmena]. [DeystviePriObmene]'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьОбязательныеРеквизитыФормы(Форма, Знач Реквизиты)
	...
	Если ОтсутствующиеРеквизиты.Количество() > 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კვანძის გამართვის ფორმის სავალდებულო რეკვიზიტები არ არის: %1';ru='Отсутствуют обязательные реквизиты формы настройки узла: %1';en='There are no mandatory details form node configuration:% 1'"),
...
Процедура ВнешнееСоединениеОбновитьНастройкиОбменаДанными(Знач ИмяПланаОбмена, Знач КодУзла, Знач ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		Сообщение = НСтр("ka='გაცვლის გეგმის კვანძი ვერ მოიძებნა; გაცვლის გეგმის სახელი %1; კვანძის კოდი %2';ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2';en='Unable to find a node sharing plan; the name of the sharing plan for 1%; node code 2%'");
...
Процедура ВыполнитьПроверкуНаличияПравилОбменаЗагруженныхИзФайла(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Если Не Результат.Пустой() Тогда
		...
		СтрокаСообщения = НСтр("ka='გაცვლის გეგმებისთვის %1 გამოიყენება ფაილიდან ჩატვირთული გაცვლის წესები."
"ეს წესები, შეიძლება პროგრამის ახალ ვერსიასთან არათავსებადი იყოს."
"პროგრამასთან მუშაობისას შეცდომების შესაძლებელი წარმოშობის გასაფრთხილებლად, რეკომენდებულია ფაილიდან გაცვლის წესების აქტუალიზება.';ru='Для планов обмена %1 используются правила обмена, загруженные из файла."
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ, СтруктураНастроек, ВидТранспорта, СообщениеОбОшибке = "", НовыйПароль = Неопределено) Экспорт
	...
	Если Не ОбработкаОбъект.ПодключениеУстановлено() Тогда
		...
		ДополнительноеСообщение = НСтр("ka='შეცდომის შესახებ ტექნიკური ინფორმაცია იხ. რეგისტრაციის ჟურნალში';ru='Техническую информацию об ошибке см. в журнале регистрации.';en='Technical information about the error message. In the log.'");
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='გაცვლის მიმოსვლის ტრანსპორტი';ru='Транспорт сообщений обмена';en='Transport exchange messages'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("ka='სხვა პროგრამაზე შესაერთებლად მითითებულ მომხმარებელს უნდა დაენიშნოს როლი ""სისტემის ადმინისტრატორი"" და ""სრული უფლებები""';ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';en='The user specified to connect to another program must be assigned the role of ""system administrator"" and ""full rights""'");
	...
	Иначе
		...
		Если СостояниеНеДопустимо Тогда
			Результат.ПодробноеОписаниеОшибки = НСтр("ka='სხვა პროგრამა განახლების მდგომარეობაში იმყოფება.';ru='Другая программа находится в состоянии обновления.';en='Another program is updated.'");
			Результат.КраткоеОписаниеОшибки   = Результат.ПодробноеОписаниеОшибки;
			Результат.Соединение = Неопределено;
		КонецЕсли;
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент, Знач СтруктураНастроек, СообщениеПользователю = "") Экспорт
	...
	СобытиеЖурналаРегистрации = НСтр("ka='მონაცემთა გაცვლა. შეერთების შემოწმება';ru='Обмен данными.Проверка подключения';en='Exchange dannymi.Proverka connection'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Для Каждого ЗаписьПланаОбмена Из Результат Цикл
		...
		Если ПравилаОбменаЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-ისთვის მიმდინარეობს მონაცემთა კონვერტაციის წესების განახლება';ru='Выполняется обновление правил конвертации данных для плана обмена %1';en='Are updating data conversion rules for the exchange of 1% Plan'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
		...
		Если ПравилаРегистрацииЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-სთვის მიმდინარეობს მონაცემთა რეგისტრაციის წესების განახლება';ru='Выполняется обновление правил регистрации данных для плана обмена %1';en='It is updating the registration rules for exchange of data% 1 plan'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
				УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა გაცვლის წესების განახლებისას შეცდომები წარმოიშვა (იხ. რეგისტრაციის ჟურნალი).';ru='При обновлении правил обмена данными возникли ошибки (см. Журнал регистрации).';en='When you update the data exchange rules any errors (see. The log).'");
	КонецЕсли;
	...
	Если Файл.Существует() И Файл.ЭтоФайл() Тогда
		...
		Если МаксимальныйДопустимыйРазмерСообщения <> 0 Тогда
			...
			Если РазмерПакета > МаксимальныйДопустимыйРазмерСообщения Тогда
				...
				СтрокаСообщения = НСтр("ka='გამავალი პაკეტის ძემამ შეადგინა %1 კბაიტი, რაც აღემატება დასაშვებ ზღვარს %2 კბაიტს.';ru='Размер исходящего пакета составил %1 Кбайт, что превышает допустимое ограничение %2 Кбайт.';en='Outgoing packet size was 1% KB, which exceeds the allowable limit% 2 Kbytes.'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Если УзелИнформационнойБазы <> Неопределено Тогда
			...
			Попытка
				...
				Если Отказ Тогда
					ВызватьИсключение НСтр("ka='მთავარი კვანძიდან მონაცემთა მიღება შეცდომებით დასრულდა.';ru='Получение данных из главного узла завершилось с ошибками.';en='Obtaining data from the master node fails.'");
				КонецЕсли;
			Исключение
				УстановитьПривилегированныйРежим(Истина);
				УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
...
"Синхронизация отменена.';ka='აღმოჩენილია მონაცემთა ბაზის კონფიგურაციაზე დაბრუნება. სინქრონიზაცია გაუქმებულია.'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Функция РазличаютсяВерсииКорреспондента(ИмяПланаОбмена, КлючСообщенияЖурналаРегистрации, ВерсияВЭтойПрограмме,
	...
	Если ЗначениеЗаполнено(ВерсияВЭтойПрограмме) И ЗначениеЗаполнено(ВерсияВДругойПрограмме)
		...
		Если ВерсияВЭтойПрограммеБезНомераСборки <> ВерсияВДругойПрограммеБезНомераСборки Тогда
			...
			ШаблонСообщения = НСтр("ka='მონაცემთა სინქრონიზაცია შეიძლება არაკორექტულად შესრულდეს, ვინაიდან ამ პროგრამის კონვერტაციის წესებში არსებული პროგრამის ვერსია ""%1"" (%2) სხვა პროგრამის კონვერტაციის წესებში არსებული ვერსია %3-ისგან განსხვავდება. დარწმუნდით იმაში, რომ ჩატვირთულია აქტუალური წესები, რომლებიც ორივე პროგრამის გამოსაყენებელი ვერსიების შესაფერისია.';ru='Синхронизация данных может быть выполнена некорректно, т.к. версия программы ""%1"" (%2) в правилах конвертации этой программы отличается от версии %3 в правилах конвертации в другой программе. Убедитесь, что загружены актуальные правила, подходящие для используемых версий обеих программ.';en='Data synchronization can be performed correctly, since ""%1"" version of the program (2%) in the conversion rules of the program is different from the version of 3% in the rules of conversion in another program. Make sure that the current rules are loaded suitable for the version of both programs.'");
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='გაფრთხილებები სინქრონიზაციის დროს';ru='Предупреждения при синхронизации';en='Cards synchronization'");
...
Процедура ПриЗаполненииСпискаТекущихДелНеобходимоОбновление(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Истина;
		Дело.Представление  = НСтр("ka='პროგრამის ვერსიის განახლება';ru='Обновить версию программы';en='Update version'");
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("ka='განახლების დაყენება';ru='Установка обновления';en='Installing the update'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
...
Процедура ПриЗаполненииСпискаТекущихДелПроверитьСовместимостьСТекущейВерсией(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='გაცვლის წესები';ru='Правила обмена';en='rules of exchange'");
		...
		Если ГруппаДела = Неопределено Тогда
			...
			ГруппаДела.Представление = НСтр("ka='შესაბამისობის შემოწმება';ru='Проверить совместимость';en='check compatibility'");
...
Процедура УстановитьСообщениеОбменаДаннымиИзГлавногоУзла(СообщениеОбмена, ГлавныйУзел) Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел, НСтр("ka='გაცვლის შეტყობინება ჩაწერილია ქეშში.';ru='Сообщение обмена записано в кэш.';en='Message exchange is recorded in the cache.'"));
...
Процедура ОчиститьСообщениеОбменаДаннымиИзГлавногоУзла() Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел(), НСтр("ka='გაცვლის შეტყობინება ქეშიდან წაშლილია.';ru='Сообщение обмена удалено из кэша.';en='Message exchange is removed from the cache.'"));
...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ka='მონაცემთა სინქრონიზაციის გამართვა';ru='Настройка синхронизация данных';en='Configure Data Synchronization'"));
	Результат.Вставить("Родительный", НСтр("ka='მონაცემთა სინქრონიზაციის გამართვები';ru='Настройки синхронизации данных';en='data synchronization settings'"));
...
Функция ИнтерактивноеИзменениеВыгрузки(Знач УзелИнформационнойБазы, Знач АдресХранилищаФормы, Знач ЕстьСценарийУзла=Неопределено) Экспорт
	...
	ПараметрыСценарияДополнения.ВариантБезДополнения.Вставить("Пояснение", 
		НСтр("ka='გაიგზავნება მხოლოდ მონაცემები საერთო გამართვების თანახმად.';ru='Будут отправлены только данные согласно общим настройкам.';en='Once the data will be sent according to a general configuration.'")); 
	...
	ПараметрыСценарияДополнения.ВариантВсеДокументы.Вставить("Пояснение",
		НСтр("ka='დამატებით გაიგზავნება შერჩევის პირობების დამაკმაყოფილებელი პერიოდის ყველა დოკუმენტი.';ru='Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.';en='Additionally, all documents will be sent during the period that satisfy the selection criteria.'")); 
	...
	ПараметрыСценарияДополнения.ВариантПроизвольныйОтбор.Вставить("Пояснение",
		НСтр("ka='დამატებით გაიგზავნება მონაცემები შერჩევის თანახმად.';ru='Дополнительно будут отправлены данные согласно отбору.';en='Additional data will be sent according to the selection.'")); 
	...
	ПараметрыСценарияДополнения.ВариантДополнительно.Вставить("Пояснение",
		НСтр("ka='გაიგზავნება გამართვების დამატებითი მონაცემები.';ru='Будут отправлены дополнительные данные по настройкам.';en='They will be sent additional information about the settings.'")); 
...
Функция ПредставлениеОтбораДополненияВыгрузки(Знач Период, Знач Отбор, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	Если ПустаяСтрока(ОтборСтрокой) Тогда
		Если ОписаниеПустогоОтбора=Неопределено Тогда
			ОтборСтрокой = НСтр("ka='ყველა ობიექტი';ru='Все объекты';en='All objects'");
		Иначе
...
Функция ПредставлениеДетальногоДополненияВыгрузки(Знач ДополнительнаяРегистрация, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	ИначеЕсли ОписаниеПустогоОтбора=Неопределено Тогда
		Возврат НСтр("ka='დამატებითი მონაცემები არ არის არჩეული';ru='Дополнительные данные не выбраны';en='Additional data is not selected'");
...
Функция ДополнениеВыгрузкиИмяАвтоСохраненияНастроек() Экспорт
	Возврат НСтр("ka='უკანასკნელი გაგზავნა (ინახება ავტომატურად)';ru='Последняя отправка (сохраняется автоматически)';en='Last reported (saved automatically)'");
КонецФункции
