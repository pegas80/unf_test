...
Функция НовыйОбъектДвоичныеДанные(ТихийРежим = Истина)
	...
	Исключение
		...
		Если НЕ ТихийРежим Тогда
			ТекстСообщения = НСтр("ka='Не удалось создать объект для работы с двоичными данными.';ru='Не удалось создать объект для работы с двоичными данными.'");
...
Функция ПодключитьВК(ПутьВК, ТихийРежим = Истина)
	...
	Если НЕ КодВозврата И НЕ ТихийРежим Тогда
		ТекстСообщения = НСтр("ka='Не удалось подключить внешнюю компоненту для работы с криптографией.';ru='Не удалось подключить внешнюю компоненту для работы с криптографией.'");
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ТекстСообщения);
	КонецЕсли;
...
Функция ЗаголовокГиперссылкиЗаявления(ФормаОрганизации, Организация, Отказ, СтандартнаяОбработка) Экспорт
	...
	ЗаголовокПервичногоЗаявления = НСтр("ka='Заявление на подключение к 1С-Отчетности';ru='Заявление на подключение к 1С-Отчетности'");
	ЗаголовокВторичногоЗаявления = НСтр("ka='Заявление на продление 1С-Отчетности или изменение реквизитов';ru='Заявление на продление 1С-Отчетности или изменение реквизитов'");
...
Процедура СоздатьЗаписьВРегистреЖурналОтправокВКонтролирующиеОрганы(ЗначенияЗаполнения) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронный документооборот с контролирующими органами. Журнал отправок';ru='Электронный документооборот с контролирующими органами. Журнал отправок'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОпределитьСтатусИСостояниеСдачиОтчетности(Ссылка, Организация, ВидКонтролирующегоОргана, СостояниеСдачиОтчетности, СтатусОтправки) Экспорт
	...
	Если ОбъектОтправляетсяВКонтролирующиеОрганы(Ссылка) Тогда
		...
		Если ТекущееСостояние = Неопределено ИЛИ ТекущееСостояние.ТекущийЭтапОтправки = Неопределено Тогда
			СтатусОтправки 				= НСтр("ka='არ არის გაგზავნილი';ru='Не отправлено'");
...
Процедура ЗаполнитьРегистрДокументыПоТребованиюФНС(Параметры) Экспорт
	...
	Пока ОбработаноОбъектов < 1000 И Не Параметры.ОбработкаЗавершена Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Электронный документооборот с контролирующими органами. Документы по требованию ФНС';ru='Электронный документооборот с контролирующими органами. Документы по требованию ФНС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ОбменССерверомСоздатьСервис(
	...
	Исключение
		...
		Если ВыводитьСообщения Тогда
			...
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СообщитьИлиПередатьВМастерОшибку(НСтр("ka='რეგისტრაციის სერვისთან შეერთების შეცდომა!';ru='Ошибка соединения с сервисом регистрации!'"), 
							ЭтоВызовИзМастера, 
							ТекстОшибокДляМастера, 
							Истина);
		КонецЕсли;
...
Функция ОбновитьСвойстваЦиклаОбмена(ЦиклОбмена, Предмет = Неопределено) Экспорт
	...
	Если ОбъектЦиклОбмена.Модифицированность() Тогда
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მშობლის ციკლის თვისებების განახლება ვერ მოხერხდა %1%2';ru='Не удалось обновить свойства родительского цикла обмена %1%2'"),
...
Функция ПолучитьНадписьВремяОбновления(ДатаПоследнегоОбновления) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаПоследнегоОбновления) Тогда
		Возврат НСтр("ka='Обновлено только что';ru='Обновлено только что'");
	КонецЕсли;
	...
	ИначеЕсли ВременнаяРазница < 15 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено только что';ru='Обновлено только что'");
	ИначеЕсли ВременнаяРазница < 30 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено 15 минут назад';ru='Обновлено 15 минут назад'");		
	ИначеЕсли ВременнаяРазница < 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено полчаса назад';ru='Обновлено полчаса назад'");		
	ИначеЕсли ВременнаяРазница < 2 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено час назад';ru='Обновлено час назад'");		
	ИначеЕсли ВременнаяРазница < 24 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено несколько часов назад';ru='Обновлено несколько часов назад'");		
	ИначеЕсли ВременнаяРазница < 48 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='განახლებულია გუშინ';ru='Обновлено вчера'");		
	Иначе
		НадписьВремяОбновления = НСтр("ka='Обновлено несколько дней назад';ru='Обновлено несколько дней назад'");
	КонецЕсли;
...
Функция НадписьНовыхНет(Группа)
	...
	Если Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения
		...
		Возврат НСтр("ka='ახლები არ არის';ru='Новых нет'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда
		Возврат НСтр("ka='დაუსრულებული არ არის';ru='Незавершенных нет'");
	КонецЕсли;
...
Функция ЗаголовокНедавноПросмотренных(Группа)
	...
	Если Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения Тогда
		Возврат НСтр("ka='Недавно полученные сообщения';ru='Недавно полученные сообщения'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ОбработанныеЗапросы Тогда
		Возврат НСтр("ka='Недавно обработанные запросы';ru='Недавно обработанные запросы'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ЗавершенныеОтправки Тогда
		Возврат НСтр("ka='Недавно завершенные отправки';ru='Недавно завершенные отправки'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда
		Возврат "";
	КонецЕсли;
...
Процедура ОтослатьПисьмоПартнеру(Заявление) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ka='Электронный документооборот с контролирующими органами.Ошибка обращения к методу SendLetterToPartner веб-сервису';ru='Электронный документооборот с контролирующими органами.Ошибка обращения к методу SendLetterToPartner веб-сервису'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
