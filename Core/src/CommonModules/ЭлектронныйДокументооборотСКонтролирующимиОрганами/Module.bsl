...
Функция НовыйОбъектДвоичныеДанные(ТихийРежим = Истина)
	...
	Исключение
		...
		Если НЕ ТихийРежим Тогда
			ТекстСообщения = НСтр("ka='Не удалось создать объект для работы с двоичными данными.';ru='Не удалось создать объект для работы с двоичными данными.';en='Failed to create object for working with binary data.'");
...
Функция ПодключитьВК(ПутьВК, ТихийРежим = Истина)
	...
	Если НЕ КодВозврата И НЕ ТихийРежим Тогда
		ТекстСообщения = НСтр("ka='Не удалось подключить внешнюю компоненту для работы с криптографией.';ru='Не удалось подключить внешнюю компоненту для работы с криптографией.';en='Failed to connect an external component to work with cryptography.'");
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ТекстСообщения);
	КонецЕсли;
...
Функция ЗаголовокГиперссылкиЗаявления(ФормаОрганизации, Организация, Отказ, СтандартнаяОбработка) Экспорт
	...
	ЗаголовокПервичногоЗаявления = НСтр("ka='Заявление на подключение к 1С-Отчетности';ru='Заявление на подключение к 1С-Отчетности';en='Application for connection to 1C-Accounting'");
	ЗаголовокВторичногоЗаявления = НСтр("ka='Заявление на продление 1С-Отчетности или изменение реквизитов';ru='Заявление на продление 1С-Отчетности или изменение реквизитов';en='Application for extension of 1C-Accounting or change details'");
...
Процедура СоздатьЗаписьВРегистреЖурналОтправокВКонтролирующиеОрганы(ЗначенияЗаполнения) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронный документооборот с контролирующими органами. Журнал отправок';ru='Электронный документооборот с контролирующими органами. Журнал отправок';en='Electronic document management with regulatory authorities. magazine shipments'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОпределитьСтатусИСостояниеСдачиОтчетности(Ссылка, Организация, ВидКонтролирующегоОргана, СостояниеСдачиОтчетности, СтатусОтправки) Экспорт
	...
	Если ОбъектОтправляетсяВКонтролирующиеОрганы(Ссылка) Тогда
		...
		Если ТекущееСостояние = Неопределено ИЛИ ТекущееСостояние.ТекущийЭтапОтправки = Неопределено Тогда
			СтатусОтправки 				= НСтр("ka='არ არის გაგზავნილი';ru='Не отправлено';en='Not sent'");
...
Процедура ЗаполнитьРегистрДокументыПоТребованиюФНС(Параметры) Экспорт
	...
	Пока ОбработаноОбъектов < 1000 И Не Параметры.ОбработкаЗавершена Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Электронный документооборот с контролирующими органами. Документы по требованию ФНС';ru='Электронный документооборот с контролирующими органами. Документы по требованию ФНС';en='Electronic document management with regulatory authorities. Documents FTS request'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ОбменССерверомСоздатьСервис(
	...
	Исключение
		...
		Если ВыводитьСообщения Тогда
			...
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СообщитьИлиПередатьВМастерОшибку(НСтр("ka='რეგისტრაციის სერვისთან შეერთების შეცდომა!';ru='Ошибка соединения с сервисом регистрации!';en='Error connecting to the service registering!'"), 
							ЭтоВызовИзМастера, 
							ТекстОшибокДляМастера, 
							Истина);
		КонецЕсли;
...
Функция ОбновитьСвойстваЦиклаОбмена(ЦиклОбмена, Предмет = Неопределено) Экспорт
	...
	Если ОбъектЦиклОбмена.Модифицированность() Тогда
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მშობლის ციკლის თვისებების განახლება ვერ მოხერხდა %1%2';ru='Не удалось обновить свойства родительского цикла обмена %1%2';en='Unable to update the properties of the parent cycle sharing% 1% 2'"),
...
Функция ПолучитьНадписьВремяОбновления(ДатаПоследнегоОбновления) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаПоследнегоОбновления) Тогда
		Возврат НСтр("ka='Обновлено только что';ru='Обновлено только что';en='Updated seconds ago'");
	КонецЕсли;
	...
	ИначеЕсли ВременнаяРазница < 15 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено только что';ru='Обновлено только что';en='Updated seconds ago'");
	ИначеЕсли ВременнаяРазница < 30 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено 15 минут назад';ru='Обновлено 15 минут назад';en='Updated 15 minutes ago'");		
	ИначеЕсли ВременнаяРазница < 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено полчаса назад';ru='Обновлено полчаса назад';en='UPDATED half an hour ago'");		
	ИначеЕсли ВременнаяРазница < 2 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено час назад';ru='Обновлено час назад';en='Updated hours ago'");		
	ИначеЕсли ВременнаяРазница < 24 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='Обновлено несколько часов назад';ru='Обновлено несколько часов назад';en='Updated hours ago'");		
	ИначеЕсли ВременнаяРазница < 48 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ka='განახლებულია გუშინ';ru='Обновлено вчера';en='Updated yesterday'");		
	Иначе
		НадписьВремяОбновления = НСтр("ka='Обновлено несколько дней назад';ru='Обновлено несколько дней назад';en='Updated days ago'");
	КонецЕсли;
...
Функция НадписьНовыхНет(Группа)
	...
	Если Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения
		...
		Возврат НСтр("ka='ახლები არ არის';ru='Новых нет';en='There are no new ones'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда
		Возврат НСтр("ka='დაუსრულებული არ არის';ru='Незавершенных нет';en='no unfinished'");
	КонецЕсли;
...
Функция ЗаголовокНедавноПросмотренных(Группа)
	...
	Если Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения Тогда
		Возврат НСтр("ka='Недавно полученные сообщения';ru='Недавно полученные сообщения';en='Recently received messages'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ОбработанныеЗапросы Тогда
		Возврат НСтр("ka='Недавно обработанные запросы';ru='Недавно обработанные запросы';en='Recently processed requests'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ЗавершенныеОтправки Тогда
		Возврат НСтр("ka='Недавно завершенные отправки';ru='Недавно завершенные отправки';en='Recently completed sending'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда
		Возврат "";
	КонецЕсли;
...
Функция КодРегистрацииПродуктаВМоделиСервиса(АдресХранилища) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru='Электронный документооборот с контролирующими органами.Ошибка обращения к методу GetRegistrationCode веб-сервису ';ka='Электронный документооборот с контролирующими органами.Ошибка обращения к методу GetRegistrationCode веб-сервису '", 
...
Процедура ОтослатьПисьмоПартнеру(Заявление) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ka='Электронный документооборот с контролирующими органами.Ошибка обращения к методу SendLetterToPartner веб-сервису';ru='Электронный документооборот с контролирующими органами.Ошибка обращения к методу SendLetterToPartner веб-сервису';en='Electronic document management with controlling organami.Oshibka reference to the method SendLetterToPartner web service'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
