...
Функция ОтправитьSMS(НомераПолучателей, Текст, ИмяОтправителя, Логин, Пароль) Экспорт
	...
	Если ПустаяСтрока(СтрокаПолучателей) Или ПустаяСтрока(Текст) Тогда
		Результат.ОписаниеОшибки = НСтр("ka='შეტყობინების არასწორი პარამეტრები';ru='Неверные параметры сообщения'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИмяФайлаОтвета) Тогда
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки + НСтр("ka='დაკავშირება არ არის დაყენებული';ru='Соединение не установлено'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ОтветСервера = "100" Тогда
		Для Каждого НомерПолучателя Из СтрРазделить(СтрокаПолучателей, ",", Ложь) Цикл
			...
			Если ИдентификаторСообщения = Неопределено Тогда
				Результат.ОписаниеОшибки = НСтр("ka='Ответ сервера не распознан';ru='Ответ сервера не распознан'");
				Прервать;
			КонецЕсли;
...
"%1 (код ошибки: %2)'"), ОписаниеОшибкиПолученияСтатуса(КодСостояния), КодСостояния, ИдентификаторСообщения);
		ЗаписьЖурналаРегистрации(НСтр("ka='SMS-ის გაგზავნა';ru='Отправка SMS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОписанияОшибок()
	...
	ОписанияОшибок.Вставить("200", НСтр("ka='Авторизация не выполнена: неверный api_id.';ru='Авторизация не выполнена: неверный api_id.'"));
	ОписанияОшибок.Вставить("201", НСтр("ka='Не достаточно средств на лицевом счету.';ru='Не достаточно средств на лицевом счету.'"));
	ОписанияОшибок.Вставить("202", НСтр("ka='Неправильно указан получатель.';ru='Неправильно указан получатель.'"));
	ОписанияОшибок.Вставить("203", НСтр("ka='Нет текста сообщения.';ru='Нет текста сообщения.'"));
	ОписанияОшибок.Вставить("204", НСтр("ka='Имя отправителя не согласовано с провайдером (SMS.RU).';ru='Имя отправителя не согласовано с провайдером (SMS.RU).'"));
	ОписанияОшибок.Вставить("205", НСтр("ka='Сообщение слишком длинное (превышает 8 SMS).';ru='Сообщение слишком длинное (превышает 8 SMS).'"));
	ОписанияОшибок.Вставить("206", НСтр("ka='Достигнут дневной лимит на отправку сообщений.';ru='Достигнут дневной лимит на отправку сообщений.'"));
	ОписанияОшибок.Вставить("207", НСтр("ka='На этот номер (или один из номеров) нельзя отправлять сообщения, либо указано более 100 номеров в списке получателей';ru='На этот номер (или один из номеров) нельзя отправлять сообщения, либо указано более 100 номеров в списке получателей'"));
	ОписанияОшибок.Вставить("208", НСтр("ka='Параметр time указан неправильно.';ru='Параметр time указан неправильно.'"));
	ОписанияОшибок.Вставить("209", НСтр("ka='Номер получателя (или один из номеров) в стоп-листе (см. в личном кабинете на сайте).';ru='Номер получателя (или один из номеров) в стоп-листе (см. в личном кабинете на сайте).'"));
	ОписанияОшибок.Вставить("210", НСтр("ka='Используется GET, где необходимо использовать POST.';ru='Используется GET, где необходимо использовать POST.'"));
	ОписанияОшибок.Вставить("211", НСтр("ka='Метод не найден.';ru='Метод не найден.'"));
	ОписанияОшибок.Вставить("212", НСтр("ka='Неверная кодировка текста сообщения (необходимо использовать UTF-8).';ru='Неверная кодировка текста сообщения (необходимо использовать UTF-8).'"));
	ОписанияОшибок.Вставить("220", НСтр("ka='Сервис временно недоступен.';ru='Сервис временно недоступен.'"));
	ОписанияОшибок.Вставить("230", НСтр("ka='Сообщение не принято к отправке: достигнут дневной лимит сообщений на один номер (60 шт).';ru='Сообщение не принято к отправке: достигнут дневной лимит сообщений на один номер (60 шт).'"));
	ОписанияОшибок.Вставить("300", НСтр("ka='Авторизация не выполнена: token устарел (истек срок действия, либо изменился IP отправителя.';ru='Авторизация не выполнена: token устарел (истек срок действия, либо изменился IP отправителя.'"));
	ОписанияОшибок.Вставить("301", НСтр("ka='Авторизация не выполнена: логин или пароль указаны неверно.';ru='Авторизация не выполнена: логин или пароль указаны неверно.'"));
	ОписанияОшибок.Вставить("302", НСтр("ka='Авторизация не выполнена: пользователь авторизован, но аккаунт не подтвержден (пользователь не ввел код, присланный в регистрационной SMS.)';ru='Авторизация не выполнена: пользователь авторизован, но аккаунт не подтвержден (пользователь не ввел код, присланный в регистрационной SMS.)'"));
...
Функция ОписаниеОшибкиОтправки(КодОшибки)
	...
	Если ТекстСообщения = Неопределено Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Сообщение не отправлено (код ошибки: %1).';ru='Сообщение не отправлено (код ошибки: %1).'"), КодОшибки);
	КонецЕсли;
...
Функция ОписаниеОшибкиПолученияСтатуса(КодОшибки)
	...
	ТекстСообщения = ОписанияОшибок[КодОшибки];
	Если ТекстСообщения = Неопределено Тогда
		ТекстСообщения = НСтр("ka='Отказ выполнения операции';ru='Отказ выполнения операции'");
	КонецЕсли;
	Возврат ТекстСообщения;
КонецФункции
...
Функция ВыполнитьЗапрос(ИмяМетода, ПараметрыЗапроса)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='SMS-ის გაგზავნა';ru='Отправка SMS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция Разрешения() Экспорт
	...
	Адрес = "sms.ru";
	Порт = Неопределено;
	Описание = НСтр("ka='Отправка SMS через ""SMS.RU"".';ru='Отправка SMS через ""SMS.RU"".'");
