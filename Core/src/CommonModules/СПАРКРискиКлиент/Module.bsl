...
Процедура ОтобразитьИндексыСПАРК(
	...
	Если УслугаПодключена <> Истина Тогда
		...
		РезультатИндексыКонтрагента.Вставить("СообщениеОбОшибке", НСтр("ka='Услуга не подключена.';ru='Услуга не подключена.';en='The service is not connected.'"));
...
Процедура ПоказатьЗначенияИндексовКонтрагента(ИндексыСПАРКРиски, ВладелецФормы = Неопределено) Экспорт
	...
	Если ИндексыСПАРКРиски = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нет информации для отображения.';ru='Нет информации для отображения.';en='There is no information to display.'"));
		Возврат;
	ИначеЕсли ИндексыСПАРКРиски.ПодлежитПроверке <> Истина Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.';ru='Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.';en='The selected contractor is not subject to review when 1SPARK risks.'"));
		Возврат;
	КонецЕсли;
