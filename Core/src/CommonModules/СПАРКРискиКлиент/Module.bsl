...
Процедура ОтобразитьИндексыСПАРК(
	...
	Если УслугаПодключена <> Истина Тогда
		...
		РезультатИндексыКонтрагента.Вставить("СообщениеОбОшибке", НСтр("ka='Услуга не подключена.';ru='Услуга не подключена.'"));
...
Процедура ПоказатьЗначенияИндексовКонтрагента(ИндексыСПАРКРиски, ВладелецФормы = Неопределено) Экспорт
	...
	Если ИндексыСПАРКРиски = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нет информации для отображения.';ru='Нет информации для отображения.'"));
		Возврат;
	ИначеЕсли ИндексыСПАРКРиски.ПодлежитПроверке <> Истина Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.';ru='Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.'"));
		Возврат;
	КонецЕсли;
