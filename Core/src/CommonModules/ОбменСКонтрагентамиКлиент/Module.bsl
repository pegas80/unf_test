...
Процедура ОповеститьОНовыхДокументахЭДО() Экспорт
	...
	НавигационнаяСсылка = НавигационнаяСсылкаТекущиеДелаЭДО();
		ЗаголовокОповещения = НСтр("ka='სერვისი ЭДО';ru='Сервис ЭДО'");
		ТекстОповещения = НСтр("ka='ახალი დოკუმენტების მიღება';ru='Получить новые документы'");
...
"Записать?'"));
				...
				ШаблонСообщения = НСтр("ka='დოკუმენტი შეცვლილია. ელექტრონული დოკუმენტის ჩამოსაყალიბებლად აუცილებელია %1';ru='Документ изменен. Для формирования электронного документа его необходимо %1'");
				...
				ПоказатьВопрос( Обработчик, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.Отмена,
					НСтр("ka='დოკუმენტი შეიცვალა.';ru='Документ изменен.'"));
...
Процедура ЗаписатьВФорме(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если Проведен Тогда
			...
			Исключение
				ПоказатьПредупреждение(, НСтр("ka='Операция не выполнена.';ru='Операция не выполнена.'"));
				Отказ = Истина;
			КонецПопытки;
		Иначе
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ka='%1. ელექტრონული დოკუმენტი ვერ მოიძებნა.';ru='%1. Электронный документ не найден.'");
...
Процедура ЗакрытьПринудительноЭДО(МассивВладельцевЭД) Экспорт
	...
	ПричинаЗакрытия = "";
	ПоказатьВводСтроки(Обработчик, ПричинаЗакрытия, НСтр("ka='Укажите причину закрытия документооборота';ru='Укажите причину закрытия документооборота'"),,Истина);
...
"Операция возможна только для непроведенных документов.'");
	...
	Если СоответствиеВладельцевИЭД.Количество() = 0 Тогда
		Для Каждого ТекущийДокумент Из МассивСсылок Цикл
			Шаблон = НСтр("ka='Электронный документ для %1 не найден';ru='Электронный документ для %1 не найден'");
	...
	Если МассивИзмененныхВладельцев.Количество() > 0 Тогда
		...
		Если МассивИзмененныхВладельцев.Количество() = 1 Тогда
			ТекстСостоянияВывод = НСтр("ka='დოკუმენტი გადავსებულია.';ru='Документ перезаполнен.'");
		Иначе
			ТекстСостоянияВывод = НСтр("ka='დოკუმენტები გადავსებულია (%1).';ru='Документы перезаполнены (%1).'");
...
Процедура ЗакрытьПринудительноРезультатВводаСтроки(ПричинаЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПричинаЗакрытия) Тогда
		...
		ТекстСообщения = НСтр("ka='Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.';ru='Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	ТекстОповещения = НСтр("ka='Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)';ru='Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", КоличествоОбработанныхЭД);
	ПоказатьОповещениеПользователя(НСтр("ka='დოკუმენტების დამუშავება';ru='Обработка документов'"), , ТекстОповещения);
...
Процедура ОбновитьКартинкуЭДО(Форма, КартинкаЭДО, МестоРазмещенияКоманд)
	...
	Если Не КартинкаЭДО = БиблиотекаКартинок.ЭмблемаСервиса1СЭДО Тогда
		ПодменюЭДО.Подсказка = НСтр("ka='სერვის 1C-ედბ-ში არის ახალი დოკუმენტები';ru='В сервисе 1С-ЭДО есть новые документы'");
	Иначе
		ПодменюЭДО.Подсказка = НСтр("ka='ბრძანებები ЭДО';ru='Команды ЭДО'");
	КонецЕсли;
