...
Процедура ПроверитьКонтрагентаПриИзменении(ПараметрыЗапуска) Экспорт  
	...
	Попытка
		...
		ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентаФоновоеЗадание", 
			Параметры, ПараметрыЗапуска.ИНН + " " + ПараметрыЗапуска.КПП, НСтр("ka='კონტრაგენტის შემოწმება';ru='Проверка контрагента';en='Check counterparty'"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='კონტრაგენტების შემოწმება. შემოწმების გაშვება კონტრაგენტების რეკვიზიტების შეცვლისას';ru='Проверка контрагентов.Запуск проверки при изменении реквизитов контрагентов';en='Check kontragentov.Zapusk check when changing counterparty details'", КодОсновногоЯзыка),
...
Функция РезультатПроверкиПараметровДоступа() Экспорт
	...
	Если ПроверкаКонтрагентов.ЕстьДоступКВебСервисуФНС() Тогда
		...
		ТекстПредупреждения = НСтр("ka='ვებ-სერვისთან წვდომის შემოწმება წარმატებით იქნა გავლილი';ru='Проверка доступа к веб-сервису успешно пройдена';en='Verifying access to the web service has successfully passed'");
		
	Иначе
		
		ПредупреждениеПроОтсутствиеДоступа = Новый ФорматированнаяСтрока(НСтр("ka='ვებ-სერვისთან წვდომა არ არის';ru='Доступ к веб-сервису отсутствует';en='Access to the web service is not'"));
		
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
			ПереходКНастройкамПроксиСервера = Новый ФорматированнаяСтрока(
				НСтр("ka='პროქსი-სერვერის პარამეტრების გამართვა';ru='Настроить параметры прокси-сервера';en='Configure proxy settings'"),,,, "e1cib/command/ОбщаяКоманда.ПереходКНастройкамДоступаКВебСервису");
		Иначе
