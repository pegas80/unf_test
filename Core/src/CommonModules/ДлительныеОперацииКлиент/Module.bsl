...
Процедура ПроверитьПараметрыОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении, Знач ПараметрыОжидания)
	...
	Если ПараметрыОжидания <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.Проверить(ПараметрыОжидания.Интервал = 0 Или ПараметрыОжидания.Интервал >= 1, 
			НСтр("ka='პარამეტრი მოლოდინისპარამეტრები.ინტერვალი უნდა იყოს 1-ის ტოლი ან მასზე მეტი';ru='Параметр ПараметрыОжидания.Интервал должен быть больше или равен 1';en='ParametryOzhidaniya.Interval parameter should be greater than or equal to 1'"),
		...
		ОбщегоНазначенияКлиентСервер.Проверить(Не (ПараметрыОжидания.ОповещениеОПрогрессеВыполнения <> Неопределено И ПараметрыОжидания.ВыводитьОкноОжидания), 
			НСтр("ka='თუ პარამეტრი მოლოდინისპარამეტრები.მოლოდინისფანჯრისგამოტანა დაყენებულია ""ჭეშმარიტება""-ში, მაშინ პარამეტრ მოლოდინისპარამეტრები.შესრულებისპროგრესისშესახებშეტყობინება-ს მხარდაჭერა არ გააჩნია';ru='Если параметр ПараметрыОжидания.ВыводитьОкноОжидания установлен в Истина, то параметр ПараметрыОжидания.ОповещениеОПрогрессеВыполнения не поддерживается';en='If the ParametryOzhidaniya.VyvoditOknoOzhidaniya set to True, the option is not supported ParametryOzhidaniya.OpoveschenieOProgresseVypolneniya'"),
			"ДлительныеОперацииКлиент.ОжидатьЗавершение");
	КонецЕсли;
