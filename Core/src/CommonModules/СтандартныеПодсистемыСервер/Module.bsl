...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ПолноеИмяРеквизита = "Список.Дата", ИмяЭлемента = "Дата") Экспорт
	...
	ЭлементОформления.Представление = НСтр("ka='Представление даты: ""10.06.2012""';ru='Представление даты: ""10.06.2012""';en='Submission date: ""10.06.2012""'");
	...
	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Представление = НСтр("ka='Представление даты сегодня: ""09:46""';ru='Представление даты сегодня: ""09:46""';en='Submission date today: ""9:46""'");
	...
	Если ВключитьВСостав.Количество() <> 0
		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			ОписаниеИсключения1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-ის შემადგენლობაში უნდა შედიოდეს მეტამონაცემთა შემდეგი ობიექტები: %2';ru='В состав плана обмена %1 должны входить следующие объекты метаданных: %2';en='The structure of the exchange 1% of the plan should include the following metadata objects: 2%'"),
				ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			ОписаниеИсключения2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმის %1 შემადგენლობაში არ უნდა შედიოდეს მეტამონაცემთა შემდეგი ობიექტები: %2';ru='В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2';en='The structure of the exchange 1% should not be metadata objects must include a plan: 2%'"),
				ИмяПланаОбмена,
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			ОписаниеИсключения3 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-ის შემადგენლობაში ავტორეგისტრაციის დაყენებული ნიშან-თვისების ობიექტები არ უნდა იყოს. საჭიროა ავტორეგისტრაციის აკრძალვა მეტამონაცემთა შემდეგი ობიექტებისთვის: %2';ru='В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации."
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ПараметрыКлиента.Количество() = 0
	   И ТекущийРежимЗапуска() <> Неопределено Тогда
		...
		ВызватьИсключение НСтр("ka='Не заполнены параметры клиента на сервере.';ru='Не заполнены параметры клиента на сервере.';en='Not filled client settings on the server.'");
	КонецЕсли;
...
"после обновления параметров работы программы.';ka='Недопустимое обращение к необновленным параметрам работы программы"
"(например, к некоторым параметрам сеанса)."
""
"Необходимо перенести вызов прикладного кода"
...
"- Версия конфигурации не предусматривает обновление без установки монопольного режима.';ka='Невозможно выполнить обновление информационной базы:"
"- Невозможно установить монопольный режим"
"- Версия конфигурации не предусматривает обновление без установки монопольного режима.'");
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ИмяСобытия = НСтр("ka='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';en='Object Identifiers metadannyh.Trebuetsya download critical changes'",
				...
				ТекстОшибки =
					НСтр("ka='საინფორმაციო ბაზა არ შეიძლება განახლდეს მთავარ კვანძში არსებული პრობლემის გამო:"
"- მთავარი კვანძი არაკორექტულად იქნა განახლებული (შეიძლება, რომ არ გაზრდილა კონფიგურაციის ვერსიის ნომერი, რის გამოც, არ მოხდა ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ს შევსება);"
"- ან პრიორიტეტულ მონაცემთა (ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ს ელემენტები) გადმოსატვირთად გაუქმება მოხდა."
""
"აუცილებელია მთავარი კვანძის განახლების განმეორებით შესრულება, პრიორიტეტულ მონაცემთა გადმოსატვირთად რეგისტრირება და მონაცემთა სინქრონიზაციის გამეორება:"
"- მთავარ კვანძში გაუშვით პროგრამა პარამეტრით /საინფორმაციობაზისგანახლებისგაშვება;"
...
"%1'");
				...
				Если ВыполнитьЗагрузку Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ka=' - შემდეგ გაიმეორეთ მონაცემთა სინქრონიზაცია ამ საინფორმაცი ბაზასთან (თავდაპირველად, მთავარ კვანძში, შემდეგ, ამ საინფორმაციო ბაზაში, ხელახლა გაშვების შემდეგ)';ru='- затем повторите синхронизацию данных с этой информационной базой"
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Объект.Метаданные().ПолноеИмя(),
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ka='Регистрация изменений для разделенных объектов не поддерживается.';ru='Регистрация изменений для разделенных объектов не поддерживается.';en='Registration of changes for shared objects is not supported.'");
		КонецЕсли;
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	Возврат Символы.ПС + Символы.ПС 
		+ НСтр("ka='შემმუშავებლისთვის: შეიძლება, საჭირო არის დამხმარე მონაცემების განახლება, რომლებიც გავლენას ახდენენ პროგრამის მუშაობაზე. განახლების შესასრულებლად შეიძლება:"
"- გამოვიყენოთ გარე დამუშავება ""შემმუშავებლის ინსტრუმენტები: დამხმარე მონაცემების განახლება"","
"- ან გავუშვათ პროგრამა 1С:საწარმოს საბრძანებო სტრიქონის პარამეტრით ""/საინფორმაციობაზისგანახლებისგაშვება"","
"- ან გავზარდოთ კონფიგურაციის ვერსიის ნომერი იმისათვის, რომ მომდევნო გაშვებისას შესრულდეს საინფორმაციო ბაზის მონაცემთა განახლების პროცედურები.';ru='Для разработчика: возможно требуется обновить вспомогательные данные,"
...
Функция ОбработчикиСобытий() Экспорт
	...
	Если ОбязательныеСобытияБезОбработчиков.Количество() > 0 Тогда
		...
		ВызватьИсключение НСтр("ka='სავალდებულო მოვლენებისთვის დამამუშავებლები არ არის განსაზღვრული. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Для обязательных событий не определены обработчики."
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("ka='Недопустимо создавать предопределенный элемент помеченный на удаление.';ru='Недопустимо создавать предопределенный элемент помеченный на удаление.';en='It is unacceptable to create a predefined element is marked for deletion.'");
	Иначе
...
"""%1"".';ka='Недопустимо помечать на удаление предопределенный элемент:"
"""%1"".'"),
...
"""%1"".';ka='Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:"
"""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
"""%1"".';ka='Недопустимо удалять предопределенный элемент"
"""%1"".'"),
		Строка(Источник.Ссылка));
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონფიგურაცია ""%1""-ის ჩაშენებული ენის ვარიანტს მხარდაჭერა არ გააჩნია."
"აუცილებელია ენის ვარიანტ ""%2""-ის გამოყენება.';ru='Вариант встроенного языка конфигурации ""%1"" не поддерживается."
"Необходимо использовать вариант языка ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='გასაშვებად საჭიროა პლატფორმა 1С:საწარმოს ვერსია %1 ან უფრო მაღალი.';ru='Для запуска необходима версия платформы 1С:Предприятие %1 или выше.';en='To run the required version of the 1C: Enterprise 1% or higher.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ka='Не заполнено свойство конфигурации Версия.';ru='Не заполнено свойство конфигурации Версия.';en='Not filled configuration property version.'");
	Иначе
		Попытка
			НулеваяВерсия = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Метаданные.Версия, "0.0.0.0") = 0;
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='კონფიგურაცია ""ვერსია""-ს თვისება არასწორად არის შევსებული: ""%1""."
"სწორი ფორმატია, მაგალითად: ""1.2.3.45"".';ru='Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Правильный формат, например: ""1.2.3.45"".'"),
				Метаданные.Версия);
		КонецПопытки;
		Если НулеваяВерсия Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Версия не может быть нулевой.';ru='Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Версия не может быть нулевой.'"),
	...
	Если (Метаданные.ОсновныеРоли.Количество() <> 2 И Метаданные.ОсновныеРоли.Количество() <> 3)
	 ...
	 Или Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		ВызватьИсключение
			НСтр("ka='კონფიგურაციაში თვისება ძირითადიროლები-ში სტანდარტული როლი სისტემისადმინისტრატორი და სრულიუფლებები არ არის მითითებული ან მითითებულია ზედმეტი როლები.';ru='В конфигурации в свойстве ОсновныеРоли не указаны стандартные роли"
...
""
"Событие ""%1"""
"уже добавлено.';ka='Ошибка при подготовке списка событий."
""
"Событие ""%1"""
"уже добавлено.'"),
				Событие);
...
"""%1""."
""
"Ошибка в имени модуля ""%2"".';ka='Ошибка при подготовке обработчиков события"
"""%1""."
""
...
"""%1""."
""
"Модуль ""%2"" уже добавлен.';ka='Ошибка при подготовке обработчиков события"
"""%1""."
""
...
"""%3"".';ka='Ошибка при подготовке обработчиков события"
"""%1""."
""
"Обработчик ""%2"" уже добавлен для события"
"""%3"".'"),
