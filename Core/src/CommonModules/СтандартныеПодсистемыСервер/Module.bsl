...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ПолноеИмяРеквизита = "Список.Дата", ИмяЭлемента = "Дата") Экспорт
	...
	ЭлементОформления.Представление = НСтр("ka='Представление даты: ""10.06.2012""';ru='Представление даты: ""10.06.2012""'");
	...
	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Представление = НСтр("ka='Представление даты сегодня: ""09:46""';ru='Представление даты сегодня: ""09:46""'");
	...
	Если ВключитьВСостав.Количество() <> 0
		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			ОписаниеИсключения1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმა %1-ის შემადგენლობაში უნდა შედიოდეს მეტამონაცემთა შემდეგი ობიექტები: %2';ru='В состав плана обмена %1 должны входить следующие объекты метаданных: %2'"),
				ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			ОписаниеИсключения2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაცვლის გეგმის %1 შემადგენლობაში არ უნდა შედიოდეს მეტამონაცემთა შემდეგი ობიექტები: %2';ru='В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2'"),
				ИмяПланаОбмена,
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ПараметрыКлиента.Количество() = 0
	   И ТекущийРежимЗапуска() <> Неопределено Тогда
		...
		ВызватьИсключение НСтр("ka='Не заполнены параметры клиента на сервере.';ru='Не заполнены параметры клиента на сервере.'");
	КонецЕсли;
	...
	Если ИменаПараметров <> "" Тогда
		...
		Если ТребуетсяОбновление Тогда
			...
			Если ТекущийРежимЗапуска() = Неопределено Тогда
				ВызватьИсключение
					НСтр("ka='პროგრამაში შესვლა დროებით შეუძლებელია ახალი ვერსიით განახლების გამო.';ru='Вход в программу временно невозможен в связи с обновлением на новую версию.'");
			Иначе
				ВызватьИсключение
...
"- Версия конфигурации не предусматривает обновление без установки монопольного режима.'");
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ИмяСобытия = НСтр("ka='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения'",
...
" (сначала в главном узле, затем в этой информационной базе после перезапуска).'"));
				Иначе
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ka=' - შემდეგ გაიმეორეთ დაქვემდებარებული კვანძის შექმნა.';ru='- затем повторите создание подчиненного узла.'"));
				КонецЕсли;
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			ВызватьИсключение НСтр("ka='გაყოფილ რეჟიმში განუყოფელ მონაცემთა ცვლილებების რეგისტრაცია.';ru='Регистрация изменений неразделенных данных в разделенном режиме.'");
		КонецЕсли;
		...
		Если Не ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных("ПланОбмена." + ИмяПланаОбмена,
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ka='გაცვლის განუყოფელი გეგმებისთვის ცვლილებების რეგისტრაციას მხარდაჭერა არ გააჩნია.';ru='Регистрация изменений для неразделенных планов обмена не поддерживается.'");
		КонецЕсли;
		
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Объект.Метаданные().ПолноеИмя(),
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ka='Регистрация изменений для разделенных объектов не поддерживается.';ru='Регистрация изменений для разделенных объектов не поддерживается.'");
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("ka='პროგრამის მუშაობის შესაძლებლობისთვის.';ru='Для возможности работы программы.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное        = Истина;
	Дело.Представление = НСтр("ka='მუშაობის სიჩქარე შემცირებულია';ru='Скорость работы снижена'");
	Дело.Форма         = "Обработка.РекомендацияПоПовышениюСкоростиРаботы.Форма.Форма";
	Дело.Владелец      = НСтр("ka='პროგრამის მუშაობის სიჩქარე';ru='Скорость работы программы'");
...
Процедура ОчиститьКаталогВременныхФайлов(ПутьККаталогу) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='სტანდარტული ქვესისტემები';ru='Стандартные подсистемы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დროებითი ფაილების კატალოგის გასუფთავების შეცდომა ""%1"":%2';ru='Ошибка очистки каталога временных файлов ""%1"":%2'"),
				ПутьККаталогу,
...
Функция ДобавитьЭлементУсловногоОформления(Форма, Инструкция) Экспорт
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если РежимОтладки И Форма.Элементы.Найти(ИмяЭлемента) = Неопределено Тогда
			Шаблон = НСтр("ka='პირობითი გაფორმების დამატების შეცდომა: ელემენტი ""%1"" ვერ მოიძებნა.';ru='Ошибка добавления условного оформления: Не найден элемент ""%1"".'");
...
Функция ОбработчикиСобытий() Экспорт
	...
	Если ОбязательныеСобытияБезОбработчиков.Количество() > 0 Тогда
		ИмяСобытия  = НСтр("ka='მოვლენების დამმუშავაბელი';ru='Обработчики событий'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
		Комментарий = НСтр("ka='შემდეგი სავალდებულო მოვლენებისთვის დაუდგენელია დამამუშავებლები:';ru='Для следующих обязательных событий не определены обработчики:'")
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("ka='Недопустимо создавать предопределенный элемент помеченный на удаление.';ru='Недопустимо создавать предопределенный элемент помеченный на удаление.'");
	Иначе
...
"Необходимо использовать вариант языка ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='გასაშვებად საჭიროა პლატფორმა 1С:საწარმოს ვერსია %1 ან უფრო მაღალი.';ru='Для запуска необходима версия платформы 1С:Предприятие %1 или выше.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ka='Не заполнено свойство конфигурации Версия.';ru='Не заполнено свойство конфигурации Версия.'");
	Иначе
		Попытка
