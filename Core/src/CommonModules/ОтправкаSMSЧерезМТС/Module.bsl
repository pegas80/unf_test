...
Функция ОтправитьSMS(НомераПолучателей, Текст, ИмяОтправителя, Логин, Знач Пароль) Экспорт
	...
	Если ИдентификаторыПолучателей.Количество() > 0 Тогда
		...
		Исключение
			...
			Результат.ОписаниеОшибки = НСтр("ka='Не удалось отправить SMS';ru='Не удалось отправить SMS'")
...
Функция Разрешения() Экспорт
	...
	Порт = Неопределено;
	Описание = НСтр("ka='SMS-ის გაგზავნა მობილური ოპერატორით.';ru='Отправка SMS через МТС.'");
