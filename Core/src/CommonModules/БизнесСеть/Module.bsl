...
Процедура ВыполнитьКомандуРубрикатора(ИмяКоманды, ПараметрыКоманды, Результат, Отказ) Экспорт
	...
	Исключение
		ТекстОшибки = нСтр("ka='სერვერთან შეერთება არ არსებობს';ru='Отсутствует соединение с сервером'");
	...
	Попытка
		...
		Иначе
			ВызватьИсключение НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა.';ru='Ошибка выполнения команды сервиса.'");
		КонецЕсли;
	Исключение
		ТекстСообщения = НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა';ru='Ошибка выполнения команды сервиса:'") + " " + ИнформацияОбОшибке().Описание;
	...
	Если Результат.КодСостояния >= 400 И Не Результат.КодСостояния = 404 Тогда
		ТекстСообщения = НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა (%1).';ru='Ошибка выполнения команды сервиса (%1).'");
	...
	Если Отказ И Не ПараметрыКоманды.Свойство("БлокироватьСообщенияОбОшибках") Тогда
		...
		Если ТекстСообщения = "" Тогда
			ТекстСообщения = НСтр("ka='სერვის 1C:ბიზნეს-ქსელში ბრძანების შესრულების შეცდომა.';ru='Ошибка выполнения команды в сервисе 1С:Бизнес-сеть.'");
		КонецЕсли;
		
		ПодробныйТекстОшибки = ТекстСообщения + Символы.ПС + СтрШаблон(НСтр("ka='მისამართი: %1.';ru='Адрес: %1.'"), ПараметрыВызова.Адрес);
		Если ТипЗнч(ОбъектJSON) = Тип("Структура") Тогда
			...
			Если Не ПустаяСтрока(ТекстОшибкиСервиса) Тогда
				ПодробныйТекстОшибки = ПодробныйТекстОшибки	
					+ Символы.ПС + СтрШаблон(НСтр("ka='დაწვრილებით: %1';ru='Подробности: %1.'"), ТекстОшибкиСервиса);
			КонецЕсли;
		КонецЕсли;
		
		ВидОперации = СтрШаблон(НСтр("ka='ბრძანება ""%1""';ru='Команда ""%1""'"), ИмяКоманды);
...
Функция ВидыДокументовСервиса() Экспорт
	...
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.АктИсполнитель,    НСтр("ka='შესრულებული სამუშაოების აქტი';ru='Акт выполненных работ'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.АктНаПередачуПрав, НСтр("ka='უფლებების გადაცემის აქტი';ru='Акт передачи прав'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ОтветНаЗаказ,      НСтр("ka='მყიდველის შეკვეთა';ru='Заказ покупателя'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ЗаказТовара,       НСтр("ka='შეკვეთა მომწოდებელს';ru='Заказ поставщику'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель,
		НСтр("ka='ღირებულების კორქტირება';ru='Корректировка стоимости'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ТОРГ12Продавец,    НСтр("ka='სასაქონლო ზედნადები';ru='Товарная накладная'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара,
		НСтр("ka='ანგარიშგება საკომისიო საქონლის გაყიდვის თაობაზე';ru='Отчет о продажах комиссионного товара'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара,
		НСтр("ka='ანგარიშგება საკომისიო საქონლის ჩამოწერის თაობაზე';ru='Отчет о списании комиссионного товара'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ПрайсЛист,         НСтр("ka='პრაის-ლისტი';ru='Прайс-лист'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.СчетНаОплату,      НСтр("ka='გადახდის მოთხოვნა';ru='Счет на оплату'"));
...
Процедура ПодключитьОрганизации(МассивОрганизаций, Отказ, ПараметрыКоманды, ТребуетсяОбновитьИнтерфейс) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(ДанныеСервиса) = Тип("Массив") Тогда
			Если ДанныеСервиса.Количество() = 0 Тогда
				Отказ = Истина;
				ВызватьИсключение НСтр("ka='სერვის 1C:ბიზნეს-ქსელის აბონენტი ვერ მოიძებნა. საჭიროა სერვისთან ორგანიზაციის განმეორებითი შეერთება.';ru='Абонент сервиса 1С:Бизнес-сеть не найден. Требуется повторное подключение организации к сервису.'");
			Иначе
	...
	Если ТекущийАбонент = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Ошибка получения администратора абонента.';ru='Ошибка получения администратора абонента.'"),,,,Отказ);
		Возврат;
	КонецЕсли;
	...
	Для каждого ОрганизацияМассива Из МассивОрганизаций Цикл
		...
		Если Результат.КодСостояния = 200 И ТипЗнч(ДанныеСервиса) = Тип("Массив") И ДанныеСервиса[0] = Истина Тогда
			...
			Если Результат.КодСостояния <> 200 Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='ორგანიზაცია ""%1""-ის შეერთების შეცდომა.';ru='Ошибка подключения организации ""%1"".'");
...
"свяжитесь со службой технической поддержки.'");
				ВидОперации = НСтр("ka='ორგანიზაციის ჩართვა';ru='Подключение организации'");
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации,
...
Процедура ОтключитьОрганизации(МассивОрганизаций, РежимОтключенияВСервисе, Отказ, ТребуетсяОбновитьИнтерфейс) Экспорт
	...
	Если МассивОрганизаций.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ka='Команда не может быть выполнена.';ru='Команда не может быть выполнена.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьПараметрыВызоваКомандыРубрикатора(ИмяКоманды,
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка инициализации сервиса';ru='Ошибка инициализации сервиса'"),,,, Отказ);
...
#Область БизнесСеть
	...
	Если ИмяКоманды = "ПолучениеКлюча" Тогда
		...
		ИначеЕсли ПараметрыКоманды.Права = "bn_user" Тогда 
			...
			Иначе
				ТекстСообщения = НСтр("ka='Текущий пользователь не зарегистрирован в сервисе 1С:Бизнес-сеть.';ru='Текущий пользователь не зарегистрирован в сервисе 1С:Бизнес-сеть.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			КонецЕсли;
	...
	ИначеЕсли ИмяКоманды = "ПолучитьРеквизитыУчастника" Тогда
		...
		Если Не ЗначениеЗаполнено(ПараметрыОрганизации.ИНН) Тогда
			Если ПараметрыКоманды.Свойство("Ссылка") И ЗначениеЗаполнено(ПараметрыКоманды.Ссылка) Тогда
				ТестСообщения = НСтр("ka='Не заполнено свойство ""ИНН"" для ""%1"".';ru='Не заполнено свойство ""ИНН"" для ""%1"".'");
				ТестСообщения = СтрШаблон(ТестСообщения, ПараметрыКоманды.Ссылка);
			Иначе
				ТестСообщения = НСтр("ka='Не заполнено свойство ""ИНН"".';ru='Не заполнено свойство ""ИНН"".'");
			КонецЕсли;
	...
	ИначеЕсли ИмяКоманды = "ОтправкаПриглашения" Тогда
		...
		Если Не ЗначениеЗаполнено(ПараметрыКоманды.ЭлектроннаяПочта) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана адрес электронной почты контрагента';ru='Не указана адрес электронной почты контрагента'")
	...
	ИначеЕсли ИмяКоманды = "ОбновитьСтатус" Тогда
		...
		Если ПараметрыКоманды.Свойство("МассивДанных") Тогда
			...
			Для каждого ЭлементМассива Из ПараметрыКоманды.МассивДанных Цикл
				...
				СтруктураСтатуса.Вставить("metaData", НСтр("ka='ჩატვირთვის თარიღი';ru='Дата загрузки:'") + ТекущаяДатаСеанса());
			...
			Для каждого ЭлементМассива Из ПараметрыКоманды.МассивДанных Цикл
				...
				Если ВРег(ПараметрыКоманды.Статус) = ВРег("Отклонен") Тогда 
					Если ЗначениеЗаполнено(ЭлементМассива.Ссылка) Тогда
						СтруктураСтатуса.Вставить("documentGuid", Строка(ЭлементМассива.Ссылка.УникальныйИдентификатор()));
					КонецЕсли;
					СтруктураСтатуса.Вставить("metaData", НСтр("ka='უარყოფის თარიღი:';ru='Дата отклонения:'") + ТекущаяДатаСеанса());
...
Процедура ПолучитьНовыйКлюч(Соединение, ПараметрыСоединения, Аутентификация, Отказ)
	...
	Если СтруктураКлюча <> Неопределено Тогда
		...
		Попытка
			...
			HTTPОтвет = Соединение.ОтправитьДляОбработки(HTTPЗапрос);
		Исключение
			ТекстСообщения = НСтр("ka='Ошибка подключения к сервису 1С:Бизнес-сеть:';ru='Ошибка подключения к сервису 1С:Бизнес-сеть:'") 
	...
	Если HTTPОтвет.КодСостояния <> 200 Тогда
		Отказ = Истина;
		ТекстСообщения = СтрШаблон(НСтр("ka='Ошибка аутентификации в сервисе 1С:Бизнес-сеть. Код ошибки %1.';ru='Ошибка аутентификации в сервисе 1С:Бизнес-сеть. Код ошибки %1.'"),
...
Функция СформироватьДанныеРегистрацииАбонента(ПараметрыКоманды, ПараметрыСоединения, Отказ)
	...
	Если Не Пользователи.РолиДоступны("АдминистрированиеАбонентаБизнесСеть") Тогда
		ТекстОшибки = НСтр("ka='Недостаточно прав доступа для выполнения операции в сервисе 1С:Бизнес-сеть';ru='Недостаточно прав доступа для выполнения операции в сервисе 1С:Бизнес-сеть'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,,Отказ);
		Возврат "";
	КонецЕсли;
...
Процедура СоздатьОбъектыИБ(ПараметрыЗагрузки, Отказ)
	...
	Если НЕ МожноЗагрузитьЭДВида(ПараметрыЗагрузки.ВидЭД) Тогда
		ТекстСообщения = НСтр("ka='Не поддерживается загрузка электронных документов вида ""%1"".';ru='Не поддерживается загрузка электронных документов вида ""%1"".'");
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыЗагрузки.Контрагент) Тогда
		ТекстСообщения = НСтр("ka='კონტრაგენტი არ არის მითითებული.';ru='Не указан контрагент.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
	...
	Если ПараметрыЗагрузки.РежимЗаполненияДокумента Тогда
		Если Не ЗначениеЗаполнено(ПараметрыЗагрузки.ДокументИБ) Тогда
			ТекстСообщения = НСтр("ka='შესავსებად დოკუმენტი არ არის მითითებული.';ru='Не указан документ для заполнения.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		КонецЕсли;
