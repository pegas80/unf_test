...
Процедура ПроверитьКонтрагентовВОтчете(Форма, ДополнительныеПараметры) Экспорт
	...
	НаименованиеЗадания = НСтр("ka='Проверка контрагентов в отчетах по НДС';ru='Проверка контрагентов в отчетах по НДС';en='Checking counterparties VAT reports'");
...
Функция СсылкаНаИнструкцию() Экспорт
	...
	// Предлагаем заменить стандартное поведение
	Результат = Новый ФорматированнаяСтрока(НСтр("ka='დაწვრილებით, შემოწმების შესახებ';ru='Подробнее о проверке';en='Learn how to check'"),,,, "e1cib/app/Обработка.ИнструкцияПоИспользованиюПроверкиКонтрагента");
