...
Процедура УстановитьРежимКомандногоИнтерфейса(Режим) Экспорт
	...
	Константы.ПредпринимательВСервисеСтандартныйИнтерфейс.Установить(
		...
		И НЕ Константы.ИнтерфейсИнтеграцииСБанком.Получить());
	
	ОтображатьПодсистемуУчетаЗарплатыИКадров = Ложь; //УчетЗарплаты.ИспользуетсяПодсистемаУчетаЗарплатыИКадров();
	...
	ИнтеграцияСБанком = Константы.ИнтеграцияСБанком.Получить();
	
	Константы.ОплатаПолныйИнтерфейс.Установить(ЭтоБизнесСтарт() И НЕ ИнтеграцияСБанком И РежимТакси);
	Константы.ОплатаПростойИнтерфейс.Установить(ЭтоБизнесСтарт() И НЕ ИнтеграцияСБанком И РежимТаксиПростой);
...
Функция ЭтоБизнесСтарт() Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
		...
		Возврат СтрНайти(ВРег(ПараметрЗапускаПриложения), "БИЗНЕССТАРТ") > 0;
	КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Ложь Тогда //ТарификацияБП.УстановитьРазрешенУчетРегулярнойДеятельности(Истина) Тогда
		Параметры.Вставить("ОбновитьИнтерфейс", Истина);
	КонецЕсли;
	Параметры.Вставить("РазрешенУчетРегулярнойДеятельности", Ложь); //ТарификацияБПВызовСервераПовтИсп.РазрешенУчетРегулярнойДеятельности());
	...
	Если ЭтоБизнесСтарт() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		
		// Проверка окончания подписки
		ПараметрыПроцедуры = Новый Структура;
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
		ПараметрыВыполнения.ОжидатьЗавершение = 0;
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Проверка окончания подписки';en='Checking the end of the subscription'");
		...
		// Проверка доступности стандартного интерфейса приложения
		УстановитьПривилегированныйРежим(Истина);
		ИнтеграцияСБанком = Константы.ИнтеграцияСБанком.Получить();
		УстановитьПривилегированныйРежим(Ложь);
		Если ИнтеграцияСБанком Тогда
			
			СтандартныйИнтерфейс = ЭтоСтандартныйИнтерфейс();
			РазрешенСтандартныйИнтерфейс = Истина;//ТарификацияБПВызовСервераПовтИсп.РазрешенСтандартныйИнтерфейс();
			
			Если СтандартныйИнтерфейс И НЕ РазрешенСтандартныйИнтерфейс Тогда
				Параметры.Вставить("ЗаблокироватьПриложение", Истина);
			КонецЕсли;
			
			Если НЕ СтандартныйИнтерфейс И РазрешенСтандартныйИнтерфейс Тогда
...
Функция ЭтоИнтерфейсИнтеграцииСБанком() Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ИнтерфейсИнтеграцииСБанком")
		И (ОбщегоНазначения.РазделениеВключено() Или ОбщегоНазначенияКлиентСервер.РежимОтладки());
...
Функция ЭтоСтандартныйИнтерфейс() Экспорт
	...
	Возврат Константы.ИнтерфейсТакси.Получить()
		...
		ИЛИ КОнстанты.ИнтерфейсВерсии82.Получить();
	...
	Если Пользователи.ЭтоПолноправныйПользователь(,Истина) Тогда
		...
		Если СообщитьОРекомендуемойВерсииПлатформы Тогда
			...
			Параметры.Вставить("ТекстСообщенияОРекомендуемойВерсииПлатформы",
					НСтр("ru='Для работы с версией 3.0.44 конфигурации ""Бухгалтерии предприятия"" необходимо использовать версию платформы 8.3.8';en='you must use version 8.3.8 platform to work with version 3.0.44 configuration ""Company Accounts""'"));
...
Функция ПолучитьЗначениеФункциональности(Знач ИмяФункциональности) Экспорт
	...
	Если ИмяФункциональности = ВРег("ПользовательУправляетСчетамиУчета") Тогда
		Результат = Истина; //СчетаУчетаВДокументахВызовСервераПовтИсп.ПользовательУправляетСчетамиУчета();
	ИначеЕсли ИмяФункциональности = ВРег("ЭтоБазоваяВерсияКонфигурации") Тогда
		Результат = Ложь; //ОбщегоНазначенияБП.ЭтоБазоваяВерсияКонфигурации();
	ИначеЕсли ИмяФункциональности = ВРег("ЭтоПрофВерсияКонфигурации") Тогда
		Результат = Истина; //ОбщегоНазначенияБП.ЭтоПрофВерсияКонфигурации();
	ИначеЕсли ИмяФункциональности = ВРег("ЭтоКорпВерсияКонфигурации") Тогда
		Результат = Истина; //ОбщегоНазначенияБП.ЭтоКорпВерсияКонфигурации();
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Описание изменений системы';en='Description of the system changes'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция СобытиеЖурналаРегистрацииПараметрыУчета() Экспорт
	...
	Возврат НСтр("ru='Настройка параметров учета';en='Configuring account settings'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())
...
Процедура ДобавитьВПротоколИзменениеСубконто(ПротоколИзменений, ВидСубконто, Изменение)
	...
	Если Изменение = 1 Тогда
		ШаблонТекста = НСтр("ru='Добавлено субконто ""%1""';en='Added subkonto ""%1""'");
	ИначеЕсли Изменение = -1 Тогда
		ШаблонТекста = НСтр("ru='Удалено субконто ""%1""';en='Subkonto Removed ""%1""'");
	Иначе
		ШаблонТекста = НСтр("ru='Изменено субконто ""%1""';en='Changed subkonto ""%1""'");
	КонецЕсли;
...
Процедура ДобавитьВПротоколИзменениеПризнакаУчета(ПротоколИзменений, ВидСубконто, ПризнакУчета, Изменение)
	...
	Если Изменение = 1 Тогда
		ШаблонТекста = НСтр("ru='У субконто ""%1"" установлен вид учета %2';en='In subkonto ""%1"" is set to the type of accounting 2%'");
	ИначеЕсли Изменение = -1 Тогда
		ШаблонТекста = НСтр("ru='У субконто ""%1"" снят вид учета %2';en='In subkonto ""%1"" has been removed type of accounting 2%'");
	Иначе
		ШаблонТекста = НСтр("ru='У субконто ""%1"" изменен вид учета %2';en='In subkonto ""%1"" modified form of the account% 2'");
	КонецЕсли;
...
Функция ПолучитьСтруктуруПризнаковУчетаСубконто()
	...
	ПризнакиУчета.Вставить("Количественный", НСтр("ru='Количественный';en='Quantitative'"));
	ПризнакиУчета.Вставить("Суммовой", НСтр("ru='Суммовой';en='accrual'"));
	ПризнакиУчета.Вставить("ТолькоОбороты", НСтр("ru='Только обороты';en='speed only'"));
	ПризнакиУчета.Вставить("Валютный", НСтр("ru='Валютный';en='Monetary'"));
	...
	Иначе
		...
		Если ПолучитьФункциональнуюОпцию("ИнтерфейсТаксиПростой") Тогда
			...
			ХранилищеОбщихНастроек.Сохранить(ВРег("ДатаСменыИнтерфейса"),,
				ТекущаяДатаСеанса());
		ИначеЕсли ПолучитьФункциональнуюОпцию("ИнтерфейсИнтеграцииСБанком") Тогда
			УстановитьРежимКомандногоИнтерфейса("ИнтерфейсТакси");
			ХранилищеОбщихНастроек.Сохранить(ВРег("ДатаСменыИнтерфейса"),,
				ТекущаяДатаСеанса());
		КонецЕсли;
...
Процедура ЗаполнитьКонстантуПредпринимательВСервисеПриОбновлении() Экспорт
	...
	МенеджерЗначения.Значение = ОбщегоНазначения.РазделениеВключено() И СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации();
...
Функция ДоступенПростойИнтерфейс() Экспорт
	...
	Возврат ЭтоБизнесСтарт();
	
КонецФункции

Процедура УстановитьОпциюУстановкаМобильногоПриложения() Экспорт
	
	ПолныйИнтерфейс = ПолучитьФункциональнуюОпцию("ИнтерфейсТакси")
		ИЛИ ПолучитьФункциональнуюОпцию("ИнтерфейсВерсия82")
...
Процедура ОтключитьНачальнуюСтраницуПользователя(ИмяПользователя) Экспорт
	...
	ОписаниеНастроек.Представление = НСтр("ka='საწყისი გვერდი';ru='Начальная страница';en='start page'");
...
Процедура УстановитьВидимостьКолонокДополнительнойИнформации(Форма) Экспорт
	...
	ОтображатьСтатусыДокументов = Ложь; //ПолучитьФункциональнуюОпцию("ОтображатьСтатусыДокументовВСписках");
	
	СтруктураРеквизита = Новый Структура("ОтображатьСтатусыДокументов", ОтображатьСтатусыДокументов);
	ЗаполнитьЗначенияСвойств(Форма, СтруктураРеквизита);
	
	Элементы = Форма.Элементы;
	КолонкиСписка = Форма.Список.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы;
	
	// Регистр сведений "НаличиеПрисоединенныхФайлов".
	Если КолонкиСписка.Найти("ЕстьФайлы") <> Неопределено Тогда
		
		Элементы.ЕстьФайлы.Видимость = ОтображатьСтатусыДокументов;
		
	КонецЕсли;
	
	// Регистр сведений "ДокументыСОшибкамиПроверкиКонтрагентов".
	Если КолонкиСписка.Найти("СодержитНекорректныхКонтрагентов") <> Неопределено Тогда
	...
	Если КолонкиСписка.Найти("Подписан") <> Неопределено Тогда // используется только основной статус Подписан
		...
		ЕстьПравоИзмененияСтатусовДокументов = Ложь;//СтатусыДокументов.ПравоИзмененияСтатусовДокументов(ОбъектМетаданных);
		ЕстьПравоЧтенияСтатусовДокументов = Ложь; //СтатусыДокументов.ПравоЧтенияСтатусовДокументов();
		...
		Элементы.Подписан.Видимость = ОтображатьСтатусыДокументов И ЕстьПравоЧтенияСтатусовДокументов;
		
	ИначеЕсли КолонкиСписка.Найти("Оригинал") <> Неопределено Тогда // используется только основной статус ОригиналПолучен
		...
		Элементы.УстановитьСтатусОригиналПолучен.Видимость   = ЕстьПравоИзмененияСтатусовДокументов И ОтображатьСтатусыДокументов;
		Элементы.УстановитьСтатусОригиналНеПолучен.Видимость = ЕстьПравоИзмененияСтатусовДокументов И ОтображатьСтатусыДокументов;
		...
		Элементы.ИзменитьСтатус.Видимость = ЕстьПравоИзмененияСтатусовДокументов И Не ОтображатьСтатусыДокументов;
		Элементы.Оригинал.Видимость = ЕстьПравоЧтенияСтатусовДокументов И ОтображатьСтатусыДокументов;
		
	ИначеЕсли КолонкиСписка.Найти("Оплата") <> Неопределено Тогда // используются и основной, и дополнительный статусы
		
		Элементы.Оплата.Видимость = ОтображатьСтатусыДокументов;
		...
		Если КолонкиСписка.Найти("Отгрузка") <> Неопределено Тогда // для покупателя
			
			Элементы.Отгрузка.Видимость = ОтображатьСтатусыДокументов;
		...
		ИначеЕсли КолонкиСписка.Найти("Поступление") <> Неопределено Тогда // для поставщика
			
			Элементы.Поступление.Видимость = ОтображатьСтатусыДокументов;
			Если ОтображатьСтатусыДокументов Тогда
	...
	Если КолонкиСписка.Найти("СчетФактура") <> Неопределено Тогда
		...
		Элементы.СчетФактура.Видимость = ОтображатьСтатусыДокументов И ОтображатьСчетФактуруВСписках;
	...
	Если КолонкиСписка.Найти("СостояниеВерсииЭД") <> Неопределено Тогда
		Элементы.СостояниеВерсииЭД.Видимость = ОтображатьСтатусыДокументов;
	КонецЕсли;
	
	// Регистр сведений "СрокиОплатыДокументов".
	Если КолонкиСписка.Найти("СрокОплаты") <> Неопределено Тогда
		
		Элементы.СрокОплаты.Видимость = ОтображатьСтатусыДокументов;
	...
	Если КолонкиСписка.Найти("Состояние") <> Неопределено Тогда
		
		Элементы.Состояние.Видимость = ОтображатьСтатусыДокументов;
