...
Функция СформироватьСообщениеИОтправить(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон.ПредназначенДляSMS Тогда
		...
		Если Сообщение.Получатель.Количество() = 0 Тогда
			Результат.ОписаниеОшибки  = НСтр("ka='Для отправки сообщения необходимо ввести номер телефонов получателей.';ru='Для отправки сообщения необходимо ввести номер телефонов получателей.';en='To send a message you must enter the number of the recipients phone.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
			...
			Иначе
				Результат.ОписаниеОшибки = НСтр("ka='Сообщение SMS не может быть отправлено сразу.';ru='Сообщение SMS не может быть отправлено сразу.';en='SMS message can not be sent immediately.'");
			КонецЕсли;
			Возврат Результат;
		КонецЕсли;
	Иначе
		Сообщение = СформироватьСообщение(ПараметрыОтправки);
		Если Сообщение.Получатель.Количество() = 0 Тогда
			Результат.ОписаниеОшибки  = НСтр("ka='Сообщение не может быть отправлено сразу, т.к необходимо ввести адрес электронной почты.';ru='Сообщение не может быть отправлено сразу, т.к необходимо ввести адрес электронной почты.';en='The message can not be sent immediately, because you need to enter an email address.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			Иначе
				Результат.ОписаниеОшибки  = НСтр("ka='Сообщение не может быть отправлено сразу.';ru='Сообщение не может быть отправлено сразу.';en='The message can not be sent immediately.'");
				Возврат Результат;
			КонецЕсли;
		КонецЕсли
	КонецЕсли;
...
Процедура ОпределитьСписокРеквизитовИВложений(СведенияОШаблоне, ПараметрыШаблона)
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.ПолноеИмяТипаНазначения) Тогда
		...
		СсылкаНаОбъект.Представление = НСтр("ka='ბმული';ru='Ссылка на';en='Link to'") + " """ + Представление + """";
	...
	Для каждого ПроизвольныйПараметр Из ПараметрыШаблона.Параметры Цикл
		...
		Если ПроизвольныйПараметр.Значение.ОписаниеТипа.Типы().Количество() > 0 Тогда
			...
			Иначе
				Префикс = НСтр("ka='თავისუფალი';ru='Произвольные';en='arbitrary'");
...
Функция ВидыШаблонов() Экспорт
	...
	ТипыШаблонов.Добавить("Email", НСтр("ka='ელექტრონული წერილი';ru='Электронного письма';en='email'"));
	ТипыШаблонов.Добавить("SMS", НСтр("ka='სმს შეტყობინებები';ru='Сообщения SMS';en='SMS messages'"));
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Владелец, Сведения, ИмяФайла, Размер, КоличествоПустыхИменВоВложениях = 0) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Если ПустаяСтрока(ИмяБезРасширения) Тогда
			...
			НСтр("ka='Вложение без имени';ru='Вложение без имени';en='Attachment with no name'") + ?(КоличествоПустыхИменВоВложениях = 0, ""," " + Строка(КоличествоПустыхИменВоВложениях + 1));
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						...
						УровеньЖурналаРегистрации.Ошибка,,, НСтр("ka='Ошибка при создании внешний печатной формы.';ru='Ошибка при создании внешний печатной формы.';en='Error creating the appearance of the printing plate.'"));
					Прервать;
				КонецПопытки;
...
Функция ОпределитьОбщиеРеквизиты()
	...
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ТекущаяДата", НСтр("ka='მიმდინარე თარიღი';ru='Текущая дата';en='The current date'"), Новый ОписаниеТипов("Дата"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ЗаголовокСистемы", НСтр("ka='სისტემის სათაური';ru='Заголовок системы';en='Title system'"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ТекущийПользователь", НСтр("ka='მიმდინარე მომხმარებელი';ru='Текущий пользователь';en='Current user'"), Новый ОписаниеТипов("СправочникСсылка.Пользователи"));
...
Процедура ЗаполнитьРеквизитыПоСКД(Реквизиты, Предмет, ПараметрыШаблона) Экспорт
	...
	Если ЕстьНезаполненныйПараметр Тогда
		ВызватьИсключение НСтр("ka='Не удалось сформировать сообщение, т.к. отсутствуют необходимые параметры.';ru='Не удалось сформировать сообщение, т.к. отсутствуют необходимые параметры.';en='It failed to generate a message, as There are no required parameters.'");
	КонецЕсли;
...
Функция ЗаголовокОбщиеРеквизиты() Экспорт
	Возврат НСтр("ka='საერთო რეკვიზიტები';ru='Общие реквизиты';en='General details'");
КонецФункции
...
Функция ИмяУзлаОбщихРеквизитов() Экспорт
	Возврат НСтр("ka='ОбщиеРеквизиты';ru='ОбщиеРеквизиты';en='ObschieRekvizity'");
КонецФункции
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ka='Формирование шаблона сообщений';ru='Формирование шаблона сообщений';en='Formation of the message template'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
