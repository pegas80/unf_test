...
Функция СформироватьСообщениеИОтправить(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон.ПредназначенДляSMS Тогда
		...
		Если Сообщение.Получатель.Количество() = 0 Тогда
			Результат.ОписаниеОшибки  = НСтр("ka='Для отправки сообщения необходимо ввести номер телефонов получателей.';ru='Для отправки сообщения необходимо ввести номер телефонов получателей.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
			...
			Иначе
				Результат.ОписаниеОшибки = НСтр("ka='Сообщение SMS не может быть отправлено сразу.';ru='Сообщение SMS не может быть отправлено сразу.'");
			КонецЕсли;
			Возврат Результат;
		КонецЕсли;
	Иначе
		Сообщение = СформироватьСообщение(ПараметрыОтправки);
		Если Сообщение.Получатель.Количество() = 0 Тогда
			Результат.ОписаниеОшибки  = НСтр("ka='Сообщение не может быть отправлено сразу, т.к необходимо ввести адрес электронной почты.';ru='Сообщение не может быть отправлено сразу, т.к необходимо ввести адрес электронной почты.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			Иначе
				Результат.ОписаниеОшибки  = НСтр("ka='Сообщение не может быть отправлено сразу.';ru='Сообщение не может быть отправлено сразу.'");
				Возврат Результат;
			КонецЕсли;
		КонецЕсли
	КонецЕсли;
...
Процедура ОпределитьСписокРеквизитовИВложений(СведенияОШаблоне, ПараметрыШаблона)
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.ПолноеИмяТипаНазначения) Тогда
		...
		СсылкаНаОбъект.Представление = НСтр("ka='ბმული';ru='Ссылка на'") + " """ + Представление + """";
	...
	Для каждого ПроизвольныйПараметр Из ПараметрыШаблона.Параметры Цикл
		...
		Если ПроизвольныйПараметр.Значение.ОписаниеТипа.Типы().Количество() > 0 Тогда
			...
			Иначе
				Префикс = НСтр("ka='თავისუფალი';ru='Произвольные'");
...
Функция ВидыШаблонов() Экспорт
	...
	ТипыШаблонов.Добавить("Email", НСтр("ka='ელექტრონული წერილი';ru='Электронного письма'"));
	ТипыШаблонов.Добавить("SMS", НСтр("ka='სმს შეტყობინებები';ru='Сообщения SMS'"));
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Владелец, Сведения, ИмяФайла, Размер, КоличествоПустыхИменВоВложениях = 0) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Если ПустаяСтрока(ИмяБезРасширения) Тогда
			...
			НСтр("ka='Вложение без имени';ru='Вложение без имени'") + ?(КоличествоПустыхИменВоВложениях = 0, ""," " + Строка(КоличествоПустыхИменВоВложениях + 1));
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						...
						УровеньЖурналаРегистрации.Ошибка,,, НСтр("ka='Ошибка при создании внешний печатной формы.';ru='Ошибка при создании внешний печатной формы.'"));
					Прервать;
				КонецПопытки;
...
Функция ОпределитьОбщиеРеквизиты()
	...
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ТекущаяДата", НСтр("ka='მიმდინარე თარიღი';ru='Текущая дата'"), Новый ОписаниеТипов("Дата"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ЗаголовокСистемы", НСтр("ka='სისტემის სათაური';ru='Заголовок системы'"));
	ДобавитьОбщийРеквизит(ОбщиеРеквизитыСтроки, "ТекущийПользователь", НСтр("ka='მიმდინარე მომხმარებელი';ru='Текущий пользователь'"), Новый ОписаниеТипов("СправочникСсылка.Пользователи"));
...
Функция ЗаголовокОбщиеРеквизиты() Экспорт
	Возврат НСтр("ka='საერთო რეკვიზიტები';ru='Общие реквизиты'");
КонецФункции
...
Функция ИмяУзлаОбщихРеквизитов() Экспорт
	Возврат НСтр("ka='ОбщиеРеквизиты';ru='ОбщиеРеквизиты'");
КонецФункции
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ka='Формирование шаблона сообщений';ru='Формирование шаблона сообщений'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
