...
Процедура ОбновитьПользователя(ЗначенияРеквизитов, ПараметрыРаботыКлиента)
	...
	Иначе
		...
		ПользовательОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора", НСтр("ka='Создание первого администратора.';ru='Создание первого администратора.';en='Creation of the first administrator.'"));
...
Процедура ОбновитьЗаписатьОрганизации(ЗначенияРеквизитов, ПараметрыРаботыКлиента)
	...
	Для каждого СтрокаОрганизации Из ТаблицаОрганизаций Цикл
		...
		Если ОрганизацияОбъект = Неопределено Тогда
			...
			ОрганизацияОбъект.СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДС("НДС18");
