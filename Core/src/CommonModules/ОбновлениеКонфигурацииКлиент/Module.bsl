...
Функция ЗаголовокСозданияРезервнойКопии(Параметры) Экспорт
	...
	Если Параметры.СоздаватьРезервнуюКопию = 0 Тогда
		Результат = НСтр("ka='არ შევქმნათ საინფორმაციო ბაზის სარეზერვო ასლი';ru='Не создавать резервную копию ИБ';en='Do not create a backup copy of the IB'");
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 1 Тогда
		Результат = НСтр("ka='საინფორმაციო ბაზის დროებითი სარეზერვო ასლის შექმნა';ru='Создавать временную резервную копию ИБ';en='Create a temporary backup IB'");
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		Результат = НСтр("ka='საინფორმაციო ბაზის სარეზერვო ასლის შექმნა';ru='Создавать резервную копию ИБ';en='Create a backup copy of the IB'");
	КонецЕсли;
	
	Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда
		Результат = Результат + " " + НСтр("ka='და უკუგორების შესრულება არასაშტატო სიტუაციაში';ru='и выполнять откат при нештатной ситуации';en='and perform a rollback if abnormality'");
	Иначе
		Результат = Результат + " " + НСтр("ka='და არ შევასრულოთ წაგორება არასაშტატო სიტუაციაში';ru='и не выполнять откат при нештатной ситуации';en='and not roll back when the contingency'");
	КонецЕсли;
...
Функция ВозможнаУстановкаОбновления(Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоФайловаяБаза И Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("ka='მიუთითეთ არსებული კატალოგი საინფორმაციო ბაზის სარეზერრვო ასლის შესანახად';ru='Укажите существующий каталог для сохранения резервной копии ИБ.';en='Specify an existing folder to save the backup information security.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.РежимОбновления = 0 Тогда // Обновить сейчас
		...
		Если ЭтоФайловаяБаза И ОбновлениеКонфигурацииВызовСервера.НаличиеАктивныхСоединений(ПараметрыПриложения[ИмяПараметра]) Тогда
			ПоказатьПредупреждение(, НСтр("ka='კონფიგურაციის განახლების გაგრძელება შეუძლებელია, ვინაიდან საინფორმაციო ბაზასთან ყველა შეერთებები დასრულებული არ არის.';ru='Невозможно продолжить обновление конфигурации, так как не завершены все соединения с информационной базой.';en='Unable to continue the configuration update, since not all connection information base completed.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если Параметры.ВыслатьОтчетНаПочту
			...
			ПоказатьПредупреждение(, НСтр("ka='მიუთითეთ ელექტრონული ფოსტის დასაშვები მისამართი.';ru='Укажите допустимый адрес электронной почты.';en='Please enter a valid email address.'"));
			Возврат Ложь;
		КонецЕсли;
		...
		Если Не ЗапланироватьОбновлениеКонфигурации(Параметры, ПараметрыАдминистрирования) Тогда
			ПоказатьПредупреждение(, НСтр("ka='Не удалось запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.';ru='Не удалось запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.';en='Failed to schedule the configuration update. the error information stored in the log.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапуститьСкриптОбновления(Параметры, ПараметрыАдминистрирования)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ka='მიმდინარეობს კონფიგურაციის განახლების პროცედურა:';ru='Выполняется процедура обновления конфигурации:';en='Procedure is performed configuration updates:'") + " " + ИмяГлавногоФайлаСкрипта);
...
Функция ДатаОбновленияУказанаВерно(Параметры)
	...
	Если Параметры.ДатаВремяОбновления < ТекущаяДата Тогда
		ТекстСообщения = НСтр("ka='კონფიგურაციის განახლება შეიძლება დაიგეგმოს მხოლოდ მომავალი თარიღისთვის';ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';en='Update configuration can be scheduled only at a future date and time.'");
	ИначеЕсли Параметры.ДатаВремяОбновления > ДобавитьМесяц(ТекущаяДата, 1) Тогда
		ТекстСообщения = НСтр("ka='კონფიგურაციის განახლება შეიძლება დაიგეგმოს არა უგვიანეს ერთი თვისა მიმდინარე თარიღთან შედარებით.';ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';en='Update configuration can be scheduled no later than a month from the current date.'");
	КонецЕсли;
...
Функция УдалитьЗадачуПланировщикаScheduledJob(КодЗадачи)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='შეცდომა მგეგმავის ამოცანის წაშლისას: %1';ru='Ошибка при удалении задачи планировщика: %1';en='Error deleting the task scheduler: 1%'");
	...
	Если Не Результат Тогда // Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx.
		...
		Возврат Результат;
	КонецЕсли;
	ТекстСообщения = НСтр("ka='დამგეგმის ამოცანა წარმატებით წაისალა (ამოცანის კოდი: %1).';ru='Задача планировщика успешно удалена (код задачи: %1).';en='Task Scheduler successfully removed (problem code:% 1).'");
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
...
Функция СоздатьЗадачуПланировщикаScheduledJob(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომა დამგეგმავის ამოცანის შექმნისას: %1';ru='Ошибка при создании задачи планировщика: %1';en='Error creating task scheduler: 1%'"), ОписаниеОшибки());
	...
	Если КодОшибки <> 0 Тогда // Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომა დამგეგმავის ამოცანის შექმნისას: %1';ru='Ошибка при создании задачи планировщика: %1';en='Error creating task scheduler: 1%'"), КодОшибки);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ТекстСообщения);
		Возврат 0;
	КонецЕсли;
	
	ТекстСообщения = НСтр("ka='დამგეგმის ამოცანა წარმატებით დაიგეგმა (ბრძანება: %1; თარიღი: %2; ამოცანის კოდი: %3).';ru='Задача планировщика успешно запланирована (команда: %1; дата: %2; код задачи: %3).';en='Scheduler task successfully scheduled (command: 1%; date: 2%; task code: 3%).'");
...
Функция СоздатьЗадачуПланировщикаScheduleService(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Попытка
		...
		ОписаниеЗадачи.RegistrationInfo.Description = НСтр("ka='კონფიგურაცია 1C:საწარმოს განახლება';ru='Обновление конфигурации 1С:Предприятие';en='Update configuration 1C: Enterprise'");
...
Функция ИмяЗадачиScheduleService(Знач КодЗадачи)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კონფიგურაციის განახლება (%1)';ru='Обновление конфигурации (%1)';en='configuration update (1%)'"), Формат(КодЗадачи, "ЧГ=0"));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ka='კონფიგურაციის განახლება';ru='Обновление конфигурации';en='update configuration'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
#Если НЕ ВебКлиент Тогда
	...
	Если НастройкиОбновления.КонфигурацияИзменена Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='კონფიგურაციის განახლება';ru='Обновление конфигурации';en='update configuration'"),
			"e1cib/app/Обработка.УстановкаОбновлений",
			НСтр("ka='კონფიგურაცია განსხვავდება საინფორმაციო ბაზის ძირითადი კონფიგურაციისგან.';ru='Конфигурация отличается от основной конфигурации информационной базы.';en='The configuration is different from the basic configuration of the information base.'"), 
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса"] = Истина Тогда
		...
		ПараметрыПредупреждения.ТекстФлажка  = НСтр("ka='კონფიგურაციის განახლების დაყენება';ru='Установить обновление конфигурации';en='Set configuration update'");
		ПараметрыПредупреждения.ТекстПредупреждения  = НСтр("ka='Запланирована установка обновления';ru='Запланирована установка обновления';en='Schedule Updates'");
