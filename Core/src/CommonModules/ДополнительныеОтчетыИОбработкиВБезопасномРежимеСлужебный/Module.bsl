...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ka='დამატებითი ანგარიშგება ან დამუშავება';ru='Дополнительный отчет или обработка';en='An additional report or processing'"));
	Результат.Вставить("Родительный", НСтр("ka='დამატებით ანგარიშგებას ან დამუშავებას';ru='Дополнительного отчета или обработки';en='Additional statement or processing'"));
...
Функция ВыполнитьСценарийБезопасногоРежима(Знач КлючСессии, Знач Сценарий, Знач ИсполняемыйОбъект, ПараметрыВыполнения, СохраняемыеПараметры = Неопределено, ОбъектыНазначения = Неопределено) Экспорт
	...
	Для Каждого ШагСценария Из Сценарий Цикл
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოქმედების უცნობი ტიპი სცენარის ეტაპისთვის: %1';ru='Неизвестный вид действия для этапа сценария: %1';en='Unknown action for the phase of the script:% 1'"), ШагСценария.ВидДействия);
		...
		Для Каждого ПараметрМетода Из ПараметрыМетода Цикл
			...
			Иначе
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უცნობი პარამეტრი სცენარის ეტაპისთვის: %1';ru='Неизвестный параметр для этапа сценария: %1';en='Unknown option to stage the script:% 1'"), ПараметрМетода.Вид);
...
Функция СформироватьПодробноеОписаниеРазрешения(Знач ВидРазрешения, Знач ПараметрыРазрешения) Экспорт
	...
	Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"<P><FONT size=2>%1%2</FONT></P>", РасшифровкаРазрешения, ?(
			ЕстьПараметры,
			" " + НСтр("ka='შემდეგი შეზღუდვებით:';ru='со следующими ограничениями';en='with the following restrictions'") + ":",
			"."));
...
Функция ТекстИсключенияРазрешениеНеПредоставлено(Знач КлючСессии, Знач ТипТребуемогоРазрешения)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ანგარიშგებისთვის ან დამუშავებისთვის %1 არ არის მიცემული ნებართვა {%2}%3!';ru='Дополнительному отчету или обработке %1 не предоставлено разрешение {%2}%3!';en='Additional reports or processing of% 1 is not granted permission {2}% 3%!'"),
...
Функция ТекстИсключенияРазрешениеНеПредоставленоДляОграничителя(Знач КлючСессии, Знач ТипТребуемогоРазрешения, Знач ПроверяемоеОграничение, Знач Ограничитель)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დამატებითი ანგარიშგებისთვის ან დამუშავებისთვის %1 ნებართვა არ არის მიცემული {%2}%3 %5-ის ტოლი შემზღუდავის მნიშვნელობისას %4!';ru='Для дополнительного отчета или обработки %1"
...
Функция ТекстИсключенияНеУстановленОбязательныйОграничитель(Знач КлючСессии, Знач ТипТребуемогоРазрешения, Знач ПроверяемоеОграничение)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დამატებითი ანგარიშგებისთვის ან დამუშავებისთვის %1 ნებართვის წარდგენისას (%2)%3 სავალდებულო შემზღუდველი არ იქნა მითითებული!';ru='Для дополнительного отчета или обработки %1"
