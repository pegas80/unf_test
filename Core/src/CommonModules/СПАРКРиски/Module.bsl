...
Функция ИндексыСПАРККонтрагента(Контрагент, ПризнакЗагрузкиДанных = Ложь) Экспорт
	...
	Если УслугаПодключена <> Истина Тогда
		...
		Результат.Вставить("СообщениеОбОшибке", НСтр("ka='Услуга не подключена.';ru='Услуга не подключена.'"));
			...
			Если СПАРКРискиКлиентСервер.ИННСоответствуетТребованиям(КонтрагентИНН, ТекстОшибки) Тогда
				...
				Если ТипЗнч(ПризнакЗагрузкиДанных) = ТипУникальныйИдентификатор Тогда // Задание было запущено ранее.
					...
					Иначе
						...
						Результат.Вставить("ТекстОшибки",             НСтр("ka='Не найдено фоновое задание';ru='Не найдено фоновое задание'"));
						Результат.Вставить("СостояниеВыводаДанных",   Перечисления.СостоянияВыводаИндексовСПАРКРиски.ВКэшеНетДанных);
				...
				Иначе
					...
					Если (ПризнакЗагрузкиДанных = Истина) Тогда
						...
						ПараметрыВыполнения.Вставить("НаименованиеФоновогоЗадания",
							СтрШаблон(
								НСтр("ka='Загрузка индексов СПАРК Риски для контрагента %1 с ИНН %2';ru='Загрузка индексов СПАРК Риски для контрагента %1 с ИНН %2'"),
								Контрагент,
								КонтрагентИНН));
				...
				ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Выполнено" Тогда
					...
					Иначе
						Результат.Вставить("ВидОшибки",                Перечисления.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка);
						Результат.Вставить("ТекстОшибки",              НСтр("ka='Ошибка получения индексов СПАРК Риски';ru='Ошибка получения индексов СПАРК Риски'"));
				...
				ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Отменено" Тогда
					Результат.Вставить("ТекстОшибки",                  НСтр("ka='Выполнение фонового задания отменено администратором';ru='Выполнение фонового задания отменено администратором'"));
					Результат.Вставить("СостояниеВыводаДанных",        Перечисления.СостоянияВыводаИндексовСПАРКРиски.ПустаяСсылка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Начальное заполнение.';ru='1СПАРК Риски. Начальное заполнение.'");
	...
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.7.11';ru='1СПАРК Риски. Переход на версию 2.1.7.11'");
	...
	Обработчик.ОбщиеДанные         = Истина;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.8.15';ru='1СПАРК Риски. Переход на версию 2.1.8.15'");
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на новую версию';ru='1СПАРК Риски. Переход на новую версию'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.7.10';ru='1СПАРК Риски. Переход на версию 2.1.7.10'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.9.2';ru='1СПАРК Риски. Переход на версию 2.1.9.2'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"assessment.1c.ru",
		443,
		НСтр("ka='Сервис 1СПАРК Риски (зона ru)';ru='Сервис 1СПАРК Риски (зона ru)'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"assessment.1c.eu",
		443,
		НСтр("ka='Сервис 1СПАРК Риски (зона eu)';ru='Сервис 1СПАРК Риски (зона eu)'"));
	НовыеРазрешения.Добавить(Разрешение);
		...
		Если Выборка = Неопределено Тогда
			...
			НаименованиеДела =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='1СПАРК Риски. Критичных событий мониторинга %1';ru='1СПАРК Риски. Критичных событий мониторинга %1'"),
					КоличествоСобытий);
...
Процедура ПриФормированииСпискаУслуг(Услуги) Экспорт
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='Индикаторы риска для компаний';ru='Индикаторы риска для компаний'"));
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='კომპანიების მონიტორინგი';ru='Мониторинг компаний'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
	
	НоваяУслуга = Новый Структура;
	НоваяУслуга.Вставить("Идентификатор", СПАРКРискиКлиентСервер.ИдентификаторУслугиСправкиПоКомпаниям());
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='Справки по компаниям';ru='Справки по компаниям'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='1СПАРК რისკები';ru='1СПАРК Риски'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПодключаемыеКомандыФормыКонтрагента(ЭтоФормаОбъекта)
	...
	Если ЭтоФормаОбъекта И ИспользованиеРазрешено("ПостановкаНаМониторинг;") Тогда
		...
		СтрКоманды.Синоним = НСтр("ka='Включить мониторинг событий';ru='Включить мониторинг событий'");
	КонецЕсли;
	
	СтрКоманды = Результат.Добавить();
	СтрКоманды.Имя     = "СПАРКРиски_ОткрытьСписокСправокКонтрагента";
	СтрКоманды.Синоним = НСтр("ka='ცნობები';ru='Справки'");
	
	СтрКоманды = Результат.Добавить();
	СтрКоманды.Имя     = "СПАРКРиски_СобытияМониторингаПоКонтрагенту";
	СтрКоманды.Синоним = НСтр("ka='მონიტორინგის შემთხვევები';ru='События мониторинга'");
...
Процедура УстановитьСостояниеСправки(СправкаСсылка, Состояние) Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЧтениеДанныхСПАРКРиски") Тогда
		ВызватьИсключение НСтр("ka='არასაკმარისი უფლებები.';ru='Недостаточно прав.'");
	КонецЕсли;
...
"Задание не выполнено.'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиОбщее';ru='Выполнение ВсеОбновленияСПАРКРискиОбщее'"));
		...
		Если МонопольныйРежим() Тогда
			...
			ОписаниеРезультата = НСтр("ka='Информационная база запущена в монопольном режиме.';ru='Информационная база запущена в монопольном режиме.'");
			ЕстьОшибки = Истина;
...
#Область Шаг_ЗагрузкаИндексовСПАРК
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиОбщее, Загрузка индексов СПАРК';ru='Выполнение ВсеОбновленияСПАРКРискиОбщее, Загрузка индексов СПАРК'"));
...
#Область ПостановкаНаМониторинг
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиОбщее, постановка контрагентов на мониторинг';ru='Выполнение ВсеОбновленияСПАРКРискиОбщее, постановка контрагентов на мониторинг'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (общее)';ru='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (общее)'"), // ИдентификаторШага
...
"Задание не выполнено.'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиРазделенное';ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		ИмяСобытияЖурналаРегистрации(), // ИмяСобытия
		НСтр("ka='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (разделенное)';ru='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (разделенное)'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОчиститьРезультатыПостановкиНаМониторинг_2_1_9_2() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Очистка результатов постановки на мониторинг в области данных при переходе на версию 2.1.9.2.';ru='Очистка результатов постановки на мониторинг в области данных при переходе на версию 2.1.9.2.'"));
	СПАРКРискиМониторингСобытий.ОчиститьРезультатыПостановкиНаМониторинг();
