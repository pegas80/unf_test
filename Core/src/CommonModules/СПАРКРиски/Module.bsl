...
Функция ИндексыСПАРККонтрагента(Контрагент, ПризнакЗагрузкиДанных = Ложь) Экспорт
	...
	Если УслугаПодключена <> Истина Тогда
		...
		Результат.Вставить("СообщениеОбОшибке", НСтр("ka='Услуга не подключена.';ru='Услуга не подключена.';en='The service is not connected.'"));
		Результат.Вставить("ИнформацияОбОшибке", СтрШаблон(
			НСтр("ru='Не удалось загрузить индексы 1СПАРК Риски."
"Услуга с идентификатором %1 не подключена.';ka='Не удалось загрузить индексы 1СПАРК Риски."
			...
			Если СПАРКРискиКлиентСервер.ИННСоответствуетТребованиям(КонтрагентИНН, ТекстОшибки) Тогда
				...
				Если ТипЗнч(ПризнакЗагрузкиДанных) = ТипУникальныйИдентификатор Тогда // Задание было запущено ранее.
					...
					Иначе
						...
						Результат.Вставить("ТекстОшибки",             НСтр("ka='Не найдено фоновое задание';ru='Не найдено фоновое задание';en='We found a background job'"));
						Результат.Вставить("СостояниеВыводаДанных",   Перечисления.СостоянияВыводаИндексовСПАРКРиски.ВКэшеНетДанных);
				...
				Иначе
					...
					Если (ПризнакЗагрузкиДанных = Истина) Тогда
						...
						ПараметрыВыполнения.Вставить("НаименованиеФоновогоЗадания",
							СтрШаблон(
								НСтр("ka='Загрузка индексов СПАРК Риски для контрагента %1 с ИНН %2';ru='Загрузка индексов СПАРК Риски для контрагента %1 с ИНН %2';en='Loading index SPARK risks to a company with 1% 2% TIN'"),
								Контрагент,
								КонтрагентИНН));
				...
				ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Выполнено" Тогда
					...
					Иначе
						Результат.Вставить("ВидОшибки",                Перечисления.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка);
						Результат.Вставить("ТекстОшибки",              НСтр("ka='Ошибка получения индексов СПАРК Риски';ru='Ошибка получения индексов СПАРК Риски';en='Failed to get the index SPARK Risks'"));
				...
				ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Отменено" Тогда
					Результат.Вставить("ТекстОшибки",                  НСтр("ka='Выполнение фонового задания отменено администратором';ru='Выполнение фонового задания отменено администратором';en='Run a background job is canceled by the administrator'"));
					Результат.Вставить("СостояниеВыводаДанных",        Перечисления.СостоянияВыводаИндексовСПАРКРиски.ПустаяСсылка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Начальное заполнение.';ru='1СПАРК Риски. Начальное заполнение.';en='1SPARK risks. Initial filling.'");
	...
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.7.11';ru='1СПАРК Риски. Переход на версию 2.1.7.11';en='1SPARK risks. Transition to version 2.1.7.11'");
	...
	Обработчик.ОбщиеДанные         = Истина;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.8.15';ru='1СПАРК Риски. Переход на версию 2.1.8.15';en='1SPARK risks. Transition to version 2.1.8.15'");
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на новую версию';ru='1СПАРК Риски. Переход на новую версию';en='1SPARK risks. The transition to the new version'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.7.10';ru='1СПАРК Риски. Переход на версию 2.1.7.10';en='1SPARK risks. Transition to version 2.1.7.10'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='1СПАРК Риски. Переход на версию 2.1.9.2';ru='1СПАРК Риски. Переход на версию 2.1.9.2';en='1SPARK risks. Transition to version 2.1.9.2'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"assessment.1c.ru",
		443,
		НСтр("ka='Сервис 1СПАРК Риски (зона ru)';ru='Сервис 1СПАРК Риски (зона ru)';en='Service 1SPARK Risks (zone ru)'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"assessment.1c.eu",
		443,
		НСтр("ka='Сервис 1СПАРК Риски (зона eu)';ru='Сервис 1СПАРК Риски (зона eu)';en='Service 1SPARK Risks (eu zone)'"));
	НовыеРазрешения.Добавить(Разрешение);
		...
		Если Выборка = Неопределено Тогда
			...
			НаименованиеДела =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='1СПАРК Риски. Критичных событий мониторинга %1';ru='1СПАРК Риски. Критичных событий мониторинга %1';en='1SPARK risks. Critical event monitoring% 1'"),
					КоличествоСобытий);
...
Процедура ПриФормированииСпискаУслуг(Услуги) Экспорт
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='Индикаторы риска для компаний';ru='Индикаторы риска для компаний';en='Indicators of risk for companies'"));
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='კომპანიების მონიტორინგი';ru='Мониторинг компаний';en='monitoring companies'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
	
	НоваяУслуга = Новый Структура;
	НоваяУслуга.Вставить("Идентификатор", СПАРКРискиКлиентСервер.ИдентификаторУслугиСправкиПоКомпаниям());
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='Справки по компаниям';ru='Справки по компаниям';en='Information on a company'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
...
"общего модуля СПАРКРискиПереопределяемый.';ka='Не удалось определить свойства справочника Контрагенты."
"Не заполнена реализация метода ПриОпределенииСвойствСправочниковКонтрагентов()"
...
"ПриОпределенииСвойствСправочниковКонтрагентов() общего модуля СПАРКРискиПереопределяемый.';ka='Неизвестный тип справочника контрагентов (%1). Описание справочника отсутствует в методе"
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='1СПАРК რისკები';ru='1СПАРК Риски';en='1SPARK Risks'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПодключаемыеКомандыФормыКонтрагента(ЭтоФормаОбъекта)
	...
	Если ЭтоФормаОбъекта И ИспользованиеРазрешено("ПостановкаНаМониторинг;") Тогда
		...
		СтрКоманды.Синоним = НСтр("ka='Включить мониторинг событий';ru='Включить мониторинг событий';en='Enable monitoring of events'");
	КонецЕсли;
	
	СтрКоманды = Результат.Добавить();
	СтрКоманды.Имя     = "СПАРКРиски_ОткрытьСписокСправокКонтрагента";
	СтрКоманды.Синоним = НСтр("ka='ცნობები';ru='Справки';en='information'");
	
	СтрКоманды = Результат.Добавить();
	СтрКоманды.Имя     = "СПАРКРиски_СобытияМониторингаПоКонтрагенту";
	СтрКоманды.Синоним = НСтр("ka='მონიტორინგის შემთხვევები';ru='События мониторинга';en='monitoring Events'");
...
Процедура УстановитьСостояниеСправки(СправкаСсылка, Состояние) Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЧтениеДанныхСПАРКРиски") Тогда
		ВызватьИсключение НСтр("ka='არასაკმარისი უფლებები.';ru='Недостаточно прав.';en='Enough rights.'");
	КонецЕсли;
...
"Задание не выполнено.';ka='Проверка надежности контрагентов в сервисе 1СПАРК Риски отключена."
"Задание не выполнено.'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиОбщее';ru='Выполнение ВсеОбновленияСПАРКРискиОбщее';en='Performing VseObnovleniyaSPARKRiskiObschee'"));
		...
		Если МонопольныйРежим() Тогда
			...
			ОписаниеРезультата = НСтр("ka='Информационная база запущена в монопольном режиме.';ru='Информационная база запущена в монопольном режиме.';en='Information base launched in exclusive mode.'");
			ЕстьОшибки = Истина;
...
#Область Шаг_ЗагрузкаИндексовСПАРК
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиОбщее, Загрузка индексов СПАРК';ru='Выполнение ВсеОбновленияСПАРКРискиОбщее, Загрузка индексов СПАРК';en='Performing VseObnovleniyaSPARKRiskiObschee, load index SPARK'"));
...
"Дата планируемого запуска: %3';ka='Время выполнения шага %1 еще не наступило."
"Текущая дата сеанса: %2"
...
#Область ПостановкаНаМониторинг
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиОбщее, постановка контрагентов на мониторинг';ru='Выполнение ВсеОбновленияСПАРКРискиОбщее, постановка контрагентов на мониторинг';en='Performing VseObnovleniyaSPARKRiskiObschee, staging contractors to monitor'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (общее)';ru='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (общее)';en='SPARK risks. Service and regulations. All updates to SPARK risks (total)'"), // ИдентификаторШага
...
"Задание не выполнено.'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиРазделенное';ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное';en='Performing VseObnovleniyaSPARKRiskiRazdelennoe'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		ИмяСобытияЖурналаРегистрации(), // ИмяСобытия
		НСтр("ka='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (разделенное)';ru='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (разделенное)';en='SPARK risks. Service and regulations. All updates to SPARK Risks (divided)'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"Не определено значение поля ПараметрыЗаполнения.ЗапросСвойствКонтрагентов.';ka='Не заполнена реализация переопределяемого метода"
"СПАРКРискиПереопределяемый.ПараметрыНачальногоЗаполненияДанных1СПАРКРиски()."
...
Процедура ОбновлениеИнформационнойБазы_ОчиститьРезультатыПостановкиНаМониторинг_2_1_9_2() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Очистка результатов постановки на мониторинг в области данных при переходе на версию 2.1.9.2.';ru='Очистка результатов постановки на мониторинг в области данных при переходе на версию 2.1.9.2.';en='Purification by monitoring performances of the results in the data during the transition to version 2.1.9.2.'"));
	СПАРКРискиМониторингСобытий.ОчиститьРезультатыПостановкиНаМониторинг();
