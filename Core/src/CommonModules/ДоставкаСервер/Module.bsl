...
Процедура ОбновитьПараметрыРасчета(ПараметрыРасчетов, ЗначенияФормулы, ФормулаСтоимости, ФормулаСебестоимости, Отказ) Экспорт
	...
	Если ОшибкаВФормуле Тогда
		...
		НСтр("ka='Ошибка составления формулы расчета стоимости';ru='Ошибка составления формулы расчета стоимости'"),
		"Объект.ФормулаСтоимости");
		Отказ = Истина;
	КонецЕсли;
	ОшибкаВФормуле = Ложь;
	...
	Если ОшибкаВФормуле Тогда
		...
		НСтр("ka='Ошибка составления формулы расчета себестоимости';ru='Ошибка составления формулы расчета себестоимости'"), 
		"Объект.ФормулаСебестоимости");
		Отказ = Истина;  
	КонецЕсли;
	Если Отказ Тогда
	...
	Если НЕ РезультатЗапроса.Получить(3).Пустой() Тогда
		...
		Ошибки, 
		"Объект", 
		НСтр("ka='В формулах используются несуществующие параметры';ru='В формулах используются несуществующие параметры'"),
		"Объект");
		ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(Ошибки);
		Возврат;
	КонецЕсли;
...
Функция СтоимостьДоставки(Контекст, СтруктураПараметров) Экспорт
	...
	Если СтруктураРеквизитов.ВариантУчета=Перечисления.ВариантыУчетаДоставки.БесплатнаяДоставка Тогда
		ТекстСообщения = СтрШаблон(НСтр("ka='Службой ""%1"" доставка выполняется бесплатно';ru='Службой ""%1"" доставка выполняется бесплатно'"), СлужбаДоставки);
	...
	ИначеЕсли СтруктураРеквизитов.ВариантУчета=Перечисления.ВариантыУчетаДоставки.ВозмещениеСтоимости Тогда
		...
		Если ПустаяСтрока(Формула) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ka='Не задана формула расчета себестоимости доставки для службы ""%1""';ru='Не задана формула расчета себестоимости доставки для службы ""%1""'",), СлужбаДоставки);
	...
	ИначеЕсли СтруктураРеквизитов.ВариантУчета=Перечисления.ВариантыУчетаДоставки.ДоставкаСОплатой Тогда
		...
		Если ПустаяСтрока(Формула) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ka='Не задана формула расчета стоимости доставки для службы ""%1""';ru='Не задана формула расчета стоимости доставки для службы ""%1""'",), СлужбаДоставки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(СлужбаДоставки, ТекстСообщения, , , "ФормулаСтоимости");
			Возврат 0;
		КонецЕсли; 
	Иначе
		ТекстСообщения = СтрШаблон(НСтр("ka='Не задан вариант учета для службы ""%1""';ru='Не задан вариант учета для службы ""%1""'"), СлужбаДоставки);
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(СлужбаДоставки, ТекстСообщения, , , "ВариантУчета");
		Возврат 0;
	КонецЕсли; 
...
Функция ВозможностьПечатиДоставки(Шапка) Экспорт
	...
	Если Не ЗначениеЗаполнено(Шапка.НоменклатураДоставки) Тогда
		...
		НСтр("ka='Не заполнена услуга доставки. Печать невозможна';ru='Не заполнена услуга доставки. Печать невозможна'"),
		Шапка.Ссылка,
...
Процедура ЗаполнитьДеревоПараметровРасчета(ПараметрыРасчетов, ТолькоЧисловые = Ложь) Экспорт
	...
	СтрокаРодитель.Идентификатор = НСтр("ka='შეკვეთა';ru='Заказ'");
	...
	СтрокаРодитель = ПараметрыРасчетов.ПолучитьЭлементы().Добавить();
	СтрокаРодитель.Идентификатор = НСтр("ka='კონტრაგენტი';ru='Контрагент'");
...
Функция РассчитатьПоФормуле(Контекст, Формула, ДополнительныеПараметры)
	...
	Исключение
		ТекстСообщения = НСтр("ka='Не удалось рассчитать параметры доставки. Возможно, формула содержит ошибку или не заполнены показатели.';ru='Не удалось рассчитать параметры доставки. Возможно, формула содержит ошибку или не заполнены показатели.'");
...
Функция ПодсказкаДляАдресаОтправления(Субъект)
	...
	Строки.Добавить(НСтр("ka='В качестве адреса отправления используется фактический адрес';ru='В качестве адреса отправления используется фактический адрес'"));
	Строки.Добавить(" ");
	
	Если ТипЗнч(Субъект) = Тип("СправочникСсылка.Организации") Тогда
		Строки.Добавить(НСтр("ka='ორგანიზაციები';ru='организации'"));
	ИначеЕсли ТипЗнч(Субъект) = Тип("СправочникСсылка.СтруктурныеЕдиницы") Тогда
		Строки.Добавить(НСтр("ka='склада';ru='склада'"));
	КонецЕсли;
	...
	Если ТипЗнч(Субъект) = Тип("СправочникСсылка.СтруктурныеЕдиницы") Тогда
		...
		Строки.Добавить(НСтр("ka='Выбрать склад можно на вкладке ""Дополнительно"".';ru='Выбрать склад можно на вкладке ""Дополнительно"".'"));
	КонецЕсли;
