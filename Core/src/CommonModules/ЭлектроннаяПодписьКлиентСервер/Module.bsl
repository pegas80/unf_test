...
Функция ПредставлениеСертификата(Сертификат, Отчество = Ложь, СрокДействия = Истина) Экспорт
	...
	Возврат ПредставлениеСубъекта(Сертификат, Отчество) + ", "
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka=' %1-დე';ru='до %1'"), Формат(Сертификат.ДатаОкончания, "ДФ=MM.yyyy"));
...
Функция ЗаполнитьОписаниеДанныхСертификата(Таблица, Сертификат) Экспорт
	...
	Если ДляПодписи И ДляШифрования Тогда
		Назначение = НСтр("ka='მონაცემთა ხელმოწერა, მონაცემთა დაშიფრვა';ru='Подписание данных, Шифрование данных'");
		
	ИначеЕсли ДляПодписи Тогда
		Назначение = НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных'");
	Иначе
		Назначение = НСтр("ka='მონაცემების დაშიფვრა';ru='Шифрование данных'");
	КонецЕсли;
	
	Таблица.Очистить();
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("ka='ვიზეა გაცემული:';ru='Кому выдан:'");
	Строка.Значение = СокрЛП(СтруктураСертификата.КомуВыдан);
	
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("ka='ვინ გასცა';ru='Кем выдан:'");
	Строка.Значение = СокрЛП(СтруктураСертификата.КемВыдан);
	
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("ka='ძალაშია   -მდე:';ru='Действителен до:'");
	Строка.Значение = Формат(СтруктураСертификата.ДействителенДо, "ДЛФ=D");
	
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("ka='დანიშნულება:';ru='Назначение:'");
