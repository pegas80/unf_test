...
Функция ПараметрыИсходящегоЗапроса(ВидДокумента) Экспорт
	...
	Иначе 
		ВызватьИсключение НСтр("ka='Для переданного вида документа параметры не предусмотрены.';ru='Для переданного вида документа параметры не предусмотрены.';en='Transferred to the document type There are no parameters.'");
...
Функция СформироватьHTTPЗапрос(ТранспортныйМодуль, ДанныеЗапроса) Экспорт
	...
	#Если Клиент Тогда
		Состояние(НСтр("ka='Обращение к транспортному модулю ЕГАИС...';ru='Обращение к транспортному модулю ЕГАИС...';en='Appeal to the transport module EGAIS ...'"));
	#КонецЕсли
	...
	Исключение
		...
		#Если НЕ ТонкийКлиент Тогда
			ТекстОшибки = НСтр("ka='Ошибка формирования запроса в УТМ по адресу %1';ru='Ошибка формирования запроса в УТМ по адресу %1';en='Error generating a request in UTM at 1%'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", ДанныеЗапроса.АдресЗапроса);
			ЗаписьЖурналаРегистрации(НСтр("ka='ЕГАИС';ru='ЕГАИС';en='ЕГАИС'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
Функция ПрочитатьОтветМодуляЕГАИС(КодСостояния, ОтветМодуляЕГАИС, РазбиратьОтвет) Экспорт
	...
	Если КодСостояния = 504 Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Нет связи с УТМ.';ru='Нет связи с УТМ.';en='No connection to the RTU.'");
	Иначе
		Если КодСостояния <> 200 Тогда
			Результат.ОписаниеОшибки = НСтр("ka='Сервер отклонил запрос:';ru='Сервер отклонил запрос:';en='The server refuses a request:'");
		КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ТекстСообщения = НСтр("ka='პარამეტრ ვერსიისსტრიქონი1-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1';en='Wrong format StrokaVersii1 parameter:% 1'");
	...
	Если Версия2.Количество() <> 4 Тогда
		ТекстСообщения = НСтр("ka='პარამეტრ ვერსიისსტრიქონი2-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1';en='Wrong format StrokaVersii2 parameter:% 1'");
