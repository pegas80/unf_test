...
Процедура ДобавитьУсловиеФильтра(Фильтр, Знач Свойство, Знач ВидСравненияЗначений, Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='ფილტრის აღწერილობა არაკორექტულად არის მიცემული!';ru='Некорректно задано описание фильтра!';en='Ill-defined description of the filter!'");
...
Функция ПроверитьУсловияФильтра(Знач ПроверяемыйОбъект, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='ფილტრი არაკორექტულად არის მიცემული!';ru='Некорректно задан фильтр!';en='Incorrectly set the filter!'");
...
Функция МенеджерАдминистрирования(Знач ПараметрыАдминистрирования)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეერთების უცნობი ტიპი: %1!';ru='Неизвестный тип подключения: %1!';en='Unknown connection type:% 1!'"), ПараметрыАдминистрирования.ТипПодключения);
