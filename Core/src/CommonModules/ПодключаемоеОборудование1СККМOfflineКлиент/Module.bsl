			...
			#Если ВебКлиент Тогда
				...
				Если НЕ Отказ Тогда
					...
					Если Не ОбработанностьПакетов Тогда
						СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='წინა გადმოტვირთვის ყველა ფაილი არ არის დამუშავებული';ru='Не все файлы предыдущей выгрузки обработаны';en='Not all the files of the previous discharge treated'"));
						Возврат Ложь;
					КонецЕсли;
				КонецЕсли;
			#Иначе
				...
				Исключение
					...
					СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ფაილის წაკითხვისას შეცდომა მოდა.';ru='При чтении файла произошла ошибка.';en='An error occurred while reading the file.'"));
					Возврат Ложь;
				КонецПопытки;
...
Процедура ОповещениеПоискФайловЗагрузкаОтчета(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если НайденныеФайлы.Количество()= 0 Тогда
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ანგარიშგების ფაილი ვერ მოიძებნა';ru='Файл отчета не найден';en='Report File Not Found'"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗаписатьПрайсЛист(ИмяФайла, СтруктураПрайсЛиста, Результат, ВыходныеПараметры, ВерсияФорматаОбмена)
	...
	Исключение
		Результат = Ложь;
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ფაილის ჩაწერა ვერ მოხერხდა.';ru='Не удалось записать файл.';en='Failed to write file.'"));
	КонецПопытки;
...
Функция ТестУстройства(Параметры, ВыходныеПараметры)
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = НСтр("ka='გადმოტვირთვის კატალოგი არ არის მითითებული';ru='Каталог выгрузки не указан';en='Catalog of unloading Unknown'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		...
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='პრაის-ლისტის ფაილის სახელი არ არის მითითებული';ru='Имя файла прайс-листа не указано';en='Filename pricelist Unknown'") 
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='ჩატვირთვის კატალოგი არ არის მითითებული';ru='Каталог загрузки не указан';en='Download catalog Unknown'") 
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='ანგარიშგების ფაილის სახელი არ არის მითითებული';ru='Имя файла отчета не указано';en='the report file name is not specified'") 
	КонецЕсли;
	...
	Если ВремПараметр=Неопределено Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='პაკეტში არსებული ელემენტების რაოდენობა არ არის მითითებული';ru='Не указано количество элементов в пакете';en='Not Specified number of elements in the package'") 
	КонецЕсли;
	...
	Если Результат Тогда
		Если Лев(Параметры.ИмяЗагружаемогоФайла, СтрДлина(Параметры.ИмяФайлаПрайсЛиста)) = Параметры.ИмяФайлаПрайсЛиста Тогда
			Результат = Ложь;
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='გადმოსატვირთი და ჩასატვირთი ფაილების სახელები ერთმანეთს არ უნდა კვეთდეს.';ru='Имена выгружаемого и загружаемого файлов не должны пересекаться.';en='Names upload and download files should not be crossed.'"); 
		КонецЕсли;
	КонецЕсли;
...
Функция ОбработатьФайлОтчет(Отказ, СодержимоеОтчета, ВыходныеПараметры, ВерсияФорматаОбмена)
	...
	Если ЗначениеЗаполнено(СтруктураДокумента.Обработан) Тогда
		...
		Отказ = Истина;
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ანგარიშგება უწინ დამუშავდა.';ru='Отчет был обработан ранее.';en='The report has been treated previously.'"));
...
Процедура ЗаписатьТекстовыйДокумент(ИмяФайла, Содержимое, ВыходныеПараметры, Отказ)
	...
	Попытка
		ТекстовыйДокумент.Записать(ИмяФайла, КодировкаТекста.UTF8);
	Исключение
		ТекстСообщения =  НСтр("ka='ფაილ %ИмяФайла%-ის ჩაწერის მცდელობისას შეცდომა მოხდა.';ru='При попытке записи файла %ИмяФайла% произошла ошибка';en='% Filename% file when trying to write error occurred'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ИмяФайла);
