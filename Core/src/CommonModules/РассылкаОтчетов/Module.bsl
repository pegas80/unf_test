...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ka='ანგარიშგებების დაგზავნა. ამოქმედება მოთხოვნის მიხედვით';ru='Рассылка отчетов. Запуск по требованию';en='Reports Newsletter. Running on demand'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не Рассылка.Подготовлена
		...
		Если Не Рассылка.Подготовлена Тогда
			Причина = Причина + Символы.ПС + НСтр("ka='დაგზავნა არ არის მომზადებული';ru='Рассылка не подготовлена';en='Newsletter not prepared'");
		КонецЕсли;
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + НСтр("ka='დაგზავნა მონიშნულია წასაშლელად';ru='Рассылка помечена на удаление';en='Subscribe tagged for deletion'");
		КонецЕсли;
		
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ka='დასრულება';ru='Завершение';en='completion'"), СокрЛП(Причина));
		Возврат Ложь;
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='ფორმატები სტანდარტულად არ არის დაყენებული.';ru='Не установлены форматы по умолчанию.';en='Do not set the default formats.'");
	КонецЕсли;
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ka='მიწოდების მეთოდი არ არის არჩეული.';ru='Не выбран способ доставки.';en='Do not choose the delivery method.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		НСтр("ka='ინიციალიზებული დაგზავნა %1, ავტორი: %2';ru='Инициализирована рассылка %1, автор: %2';en='Initialized by sending 1% author% 2'"), , 
		"'"+ Строка(Рассылка) +"'", 
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		ТекстЖурнала = НСтр("ka='ანგარიშგება %1 ყალიბდება';ru='Отчет %1 формируется';en='Report 1% formed'");
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + НСтр("ka='(სამომხმარებლო გამართვები არ არის მიცემული)';ru='(пользовательские настройки не заданы)';en='(User settings are not set)'");
		КонецЕсли;
		...
		Попытка
			...
			КонецЕсли;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание,
				НСтр("ka='ანგარიშგება %1 წარმატებით არის შედგენილი';ru='Отчет %1 успешно сформирован';en='Report% 1 was successfully formed'"), ,
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
			НомерОтчета = НомерОтчета + 1;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ka='ანგარიშგება %1 არ არის შედგენილი';ru='Отчет %1 не сформирован:';en='Report% 1 has not been established:'"), ИнформацияОбОшибке(),
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
		КонецПопытки;
	КонецЦикла;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ka='არცერთი ანგარიშგება არ არის შენახული';ru='Не сохранено ни одного отчета';en='Not saved a single report'"), ,);
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ka='ანგარიშგებების გაგზავნა მიმღებ %1-ისთვის ვერ მოხერხდა:';ru='Не удалось отправить отчеты получателю %1:';en='Unable to send reports to the recipient% 1:'"), ИнформацияОбОшибке(), 
	...
	Если РассылкаВыполнена Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ka='დაგზავნა შესრულებულია';ru='Рассылка выполнена';en='Subscribe performed'"));
	Иначе
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ka='დაგზავნა არ არის შესრულებული';ru='Рассылка не выполнена';en='Subscribe not performed'"));
	КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Если Всего = 0 Тогда
		ТекстСообщения = НСтр("ka='არჩეული ჯგუფები ანგარიშგებების დაგზავნებს არ შეიცავს.';ru='Выбранные группы не содержат рассылок отчетов.';en='Selected groups do not contain reports mailings.'");
	ИначеЕсли Всего <= 5 Тогда
		ТекстСообщения = "";
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			Если Не СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" არ არის მომზადებული.';ru='Рассылка ""%1"" не подготовлена.';en='Newsletter ""%1"" is not prepared.'");
			ИначеЕсли Не СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" არ არის შესრულებული.';ru='Рассылка ""%1"" не выполнена.';en='Newsletter ""%1"" is not executed.'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" შეცდომებით შესრულდა.';ru='Рассылка ""%1"" выполнена с ошибками.';en='Newsletter ""%1"" is unsuccessful.'");
			Иначе
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" შესრულებულია.';ru='Рассылка ""%1"" выполнена.';en='Newsletter ""%1"" is executed.'");
			КонецЕсли;
...
"Выполнено: %3"
"С ошибками: %4"
"Не выполнено: %5';ka='მომზადებულია დაგზავნა: %1 %2-დან"
"შესრულებულია: %3"
"შეცდომებით: %4"
"არ არის შესრულებული: %5'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru='Отключает неиспользуемые регламентные задания рассылок отчетов.';en='Disables unnecessary scheduled jobs reports mailings.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='პრობლემები ანგარიშგებების დაგზავნასთან';ru='Проблемы с рассылками отчетов';en='Problems with sending reports'");
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КомандаСоздать.Заголовок = НСтр("ka='ანგარიშგებების დაგზავნის შექმნა...';ru='Создать рассылку отчетов...';en='Create reports newsletter ...'");
	КомандаСоздать.Подсказка = НСтр("ka='ანგარიშგებების ახალი დაგზავნის შექმნა და მასში ანგარიშგების დამატება მიმდინარე გამართვებითურთ.';ru='Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.';en='Create a new e-mail reports, and add to it a report of the current settings.'");
	...
	КомандаПрисоединить.Заголовок = НСтр("ka='ანგარიშგებების არსებულ დაგზავნაში ჩართვა...';ru='Включить в существующую рассылку отчетов...';en='Incorporated into the existing newsletter reports ...'");
	КомандаПрисоединить.Подсказка = НСтр("ka='მიმდინარე გამართვებითურთ ანგარიშგების მიერთება ანგარიშგებების არსებულ დაგზავნაზე.';ru='Присоединить отчет с текущими настройками к существующей рассылке отчетов.';en='Attach the report to the current settings for an existing report distribution.'");
	...
	Подменю.Заголовок = НСтр("ka='დაგზავნის გაგზავნა';ru='Отправить рассылку';en='Send e-mail'");
...
Процедура АктуализироватьСписокРегламентныхЗаданий(Параметры) Экспорт
	...
	Пока РассылкиОтчетов.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось обработать регламентное задание рассылки отчетов: %1 по причине: %2';en='Failed to process scheduled jobs report distribution:% 1 for a reason:% 2'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре АктуализироватьСписокРегламентныхЗаданий не удалось обработать некоторые регламентные задания рассылки отчетов (пропущены): %1';en='AktualizirovatSpisokReglamentnyhZadany procedure failed to process some routine maintenance tasks for delivery of reports (skipped):% 1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.РассылкиОтчетов,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Процедура АктуализироватьСписокРегламентныхЗаданий обработала очередную порцию рассылки отчетов: %1';en='AktualizirovatSpisokReglamentnyhZadany procedure has processed the next portion of report delivery: 1%'"),
					ОбъектовОбработано));
	КонецЕсли;
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия = НСтр("ka='ანგარიშგებების დაგზავნა. ამოქმედება განრიგის მიხედვით';ru='Рассылка отчетов. Запуск по расписанию';en='Reports Newsletter. Scheduled start'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	ШаблонСообщенияОбОшибке = НСтр("ka='მიმღებების სია ""%1""-ის ჩამოყალიბებისას შეცდომა წარმოიშვა:';ru='При формировании списка получателей ""%1"" возникла ошибка:';en='In the formation of the list of recipients ""%1"" An error has occurred:'");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%1""-ის გამართვების შეერთების და ჩატვირთვის დროს შეცდომა წარმოიშვა.';ru='При подключении и загрузке настроек отчета ""%1"" возникла ошибка.';en='When you connect and download a report setting ""%1"" An error has occurred.'"),
...
"Подробнее см. в процедуре ОпределитьИсключаемыеОтчеты модуля РассылкаОтчетовПереопределяемый.';ka='ანგარიშგება ""%1""-ის მიერ დაგზავნა არ არის მხარდაჭერილი."
"დაწვრილებით, იხ. მოდულ ანგარიშგებებისდაგზავნახელახლაგანსაზღვრებადი-ს პროცედურა გამოსარიცხიანგარიშგებებისდადგენა-ში.'"),
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Не ПараметрыОтчета.Свойство("Инициализирован") Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			НСтр("ka='ანგარიშგება ""%1"" არ არის ინიციალიზებული';ru=""Отчет '%1' не инициализирован"";en='Report ""%1"" is not initialized'"), ,
			Строка(ПараметрыОтчета.Отчет));
		Возврат Результат;
	КонецЕсли;
	...
	Если Не Формирование.Успех Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='ანგარიშგება ""%1""';ru='Отчет ""%1"":';en='Report ""%1"":'"), Формирование.ТекстОшибки,
			Строка(ПараметрыОтчета.Отчет));
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	Результат = Ложь;
	ШаблонСообщенияОбОшибке = НСтр("ka='ანგარიშგებების მიწოდების შეცდომა';ru='Ошибка доставки отчетов';en='Error report delivery'");
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = НСтр("ka='ელექტრონული ფოსტით დაგზავნის შესახებ შეტყობინების გაგზავნა შეუძლებელია:';ru='Невозможно отправить уведомление о рассылке по электронной почте:';en='Unable to send to send notifications by e-mail:'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = НСтр("ka='ელექტრონული ფოსტით ანგარიშგების გაგზავნა შეუძლებელია:';ru='Невозможно отправить отчет по электронной почте:';en='Unable to send reports by e-mail:'");
	...
	Если Рассылка.ИспользоватьСетевойКаталог Тогда
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогWindows) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("ka='Windows-ის სერვერიდან ანგარიშგებების გამოსაქვეყნებელი ქსელური კატალოგი.';ru='Сетевой каталог для публикации отчетов с сервера Windows.';en='Online catalog for publishing reports from a Windows server.'"));
		...
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогLinux) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("ka='ქსელური კატალოგი Linux-ის სერვერიდან ანგარიშგებების გამოსაქვეყნებლად.';ru='Сетевой каталог для публикации отчетов с сервера Linux.';en='Online catalog for publishing reports from a Linux server.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Если Рассылка.ИспользоватьFTPРесурс Тогда
		Если ЗначениеЗаполнено(Рассылка.FTPСервер) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				Рассылка.FTPПорт,
				НСтр("ka='FTP რესურსი ანგარიშგებების გამოსაქვეყნებლად.';ru='FTP ресурс для публикации отчетов.';en='FTP resource for publishing reports.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Возврат Разрешения;
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = НСтр("ka='ანგარიშგება %1-ის ფორმატ %2-ში ჩაწერის შეცდომა:';ru='Ошибка записи отчета %1 в формат %2:';en='Report Error writing% 1% 2 in format:'");
			
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ЗаголовокОшибки, НСтр("ka='ფორმატს მხარდაჭერა არ გააჩნია';ru='Формат не поддерживается';en='The format is not supported'"),
					"'"+ СтрокаОтчет.Ключ +"'", 
					"'"+ ПараметрыФормата.Имя +"'");
				Продолжить;
			КонецЕсли;
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокОшибки + Символы.ПС + НСтр("ka='ფაილი %3 ვერ მოიძებნა.';ru='Файл %3 не найден.';en='% 3 File not found.'"), ,
				"'"+ СтрокаОтчет.Ключ +"'", 
				"'"+ ПараметрыФормата.Имя +"'",
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если Недоступные.Количество() > 0 Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='В рассылке есть недоступные отчеты (%1):%2';en='The newsletter has inaccessible reports (% 1):% 2'"), ,
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.Папка) Тогда
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ka='საქაღალდე არ არის შევსებული, საქაღალდეში მიწოდება გამორთულია';ru='Папка не заполнена, доставка в папку отключена';en='The folder is not filled in the folder delivery is disabled'"));
		Иначе
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
				МодульРаботаСФайламиСлужебный = ОбщегоНазначения.ОбщийМодуль("РаботаСФайламиСлужебный");
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					НСтр("ru='Недостаточно прав для создания файлов в папке ""%1"".';en='Insufficient permissions to create files in the folder ""%1"".'"), ,
					ПредставлениеПапки);
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			Если ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) Тогда
				ЗначениеПодстановки = НСтр("ka='Linux';ru='Linux';en='Linux'");
			ИначеЕсли ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогLinux) Тогда
				ЗначениеПодстановки = НСтр("ka='Windows';ru='Windows';en='Windows'");
			Иначе
				ЗначениеПодстановки = НСтр("ka='Windows-ი და Linux-ი';ru='Windows и Linux';en='Windows and Linux'");
			КонецЕсли;
			
			ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ka='ქსელური კატალოგი %1 არ არის არჩეული, ქსელურ კატალოგში მიწოდება გამორთულია';ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена';en='The online catalog% 1 is not selected, delivery to a network directory is disabled'"), ,
				ЗначениеПодстановки);
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс И Не ЗначениеЗаполнено(ПараметрыДоставки.Сервер) Тогда
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='FTP სერვერი არ არის შევსებული, FTP რესურსზე საქაღალდეში მიწოდება გამორთულია';ru='FTP сервер не заполнен, доставка в папку на FTP ресурс отключена';en='FTP server is empty, the delivery to the folder on the FTP resource is disabled'"));
	КонецЕсли;
	
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту И Не ЗначениеЗаполнено(ПараметрыДоставки.УчетнаяЗапись) Тогда
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='სააღრიცხვო ჩანაწერი არ არის არჩეული, ელექტრონული ფოსტით მიწოდება გამორთულია';ru='Учетная запись не выбрана, доставка по электронной почте отключена';en='The account is not selected, an e-mail delivery is disabled'"));
	КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если Не ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ka='პერსონალიზებული დაგზავნა შეიძლება გაიგზავნოს მხოლოდ ელექტრონული ფოსტით';ru='Персонализированная рассылка может быть отправлена только по электронной почте';en='Personalized mailing can be sent by email only'"));
			Возврат Ложь;
		КонецЕсли;
		ПараметрыДоставки.ИспользоватьПапку          = Ложь;
		ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить
			...
			И Не ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ka='ელექტრონული ფოსტით შეტყობინების გამოყენება შესაძლებელია მხოლოდ მიწოდების სხვა მეთოდებთან ერთობლივად';ru='Использование уведомлений по электронной почте возможно только совместно с другими способами доставки';en='Using the e-mail notification is only possible in conjunction with other delivery methods'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		Если ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя = 
				...
				+ Символы.ПС 
				+ Символы.ПС
				+ НСтр("ka='ანგარიშგებების ფაილები შეფუთულია არქივში';ru='Файлы отчетов запакованы в архив';en='Report files are archived'")
				+ " ";
		КонецЕсли;
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если Не ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='ინფორმაციის გამოსატანად მომხმარებელ %1-ს არასაკმარისი უფლებები გააჩნია';ru='У пользователя %1 недостаточно прав на вывод информации';en='User% 1 has insufficient permissions to display information'"), ,
