...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ka='ანგარიშგებების დაგზავნა. ამოქმედება მოთხოვნის მიხედვით';ru='Рассылка отчетов. Запуск по требованию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не Рассылка.Подготовлена
		...
		Если Не Рассылка.Подготовлена Тогда
			Причина = Причина + Символы.ПС + НСтр("ka='დაგზავნა არ არის მომზადებული';ru='Рассылка не подготовлена'");
		КонецЕсли;
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + НСтр("ka='დაგზავნა მონიშნულია წასაშლელად';ru='Рассылка помечена на удаление'");
		КонецЕсли;
		
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ka='დასრულება';ru='Завершение'"), СокрЛП(Причина));
		Возврат Ложь;
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='ფორმატები სტანდარტულად არ არის დაყენებული.';ru='Не установлены форматы по умолчанию.'");
	КонецЕсли;
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ka='მიწოდების მეთოდი არ არის არჩეული.';ru='Не выбран способ доставки.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		НСтр("ka='ინიციალიზებული დაგზავნა %1, ავტორი: %2';ru='Инициализирована рассылка %1, автор: %2'"), , 
		"'"+ Строка(Рассылка) +"'", 
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		ТекстЖурнала = НСтр("ka='ანგარიშგება %1 ყალიბდება';ru='Отчет %1 формируется'");
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + НСтр("ka='(სამომხმარებლო გამართვები არ არის მიცემული)';ru='(пользовательские настройки не заданы)'");
		КонецЕсли;
		...
		Попытка
			...
			КонецЕсли;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание,
				НСтр("ka='ანგარიშგება %1 წარმატებით არის შედგენილი';ru='Отчет %1 успешно сформирован'"), ,
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
			НомерОтчета = НомерОтчета + 1;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ka='ანგარიშგება %1 არ არის შედგენილი';ru='Отчет %1 не сформирован:'"), ИнформацияОбОшибке(),
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
		КонецПопытки;
	КонецЦикла;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ka='არცერთი ანგარიშგება არ არის შენახული';ru='Не сохранено ни одного отчета'"), ,);
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ka='ანგარიშგებების გაგზავნა მიმღებ %1-ისთვის ვერ მოხერხდა:';ru='Не удалось отправить отчеты получателю %1:'"), ИнформацияОбОшибке(), 
	...
	Если РассылкаВыполнена Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ka='დაგზავნა შესრულებულია';ru='Рассылка выполнена'"));
	Иначе
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ka='დაგზავნა არ არის შესრულებული';ru='Рассылка не выполнена'"));
	КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Если Всего = 0 Тогда
		ТекстСообщения = НСтр("ka='არჩეული ჯგუფები ანგარიშგებების დაგზავნებს არ შეიცავს.';ru='Выбранные группы не содержат рассылок отчетов.'");
	ИначеЕсли Всего <= 5 Тогда
		ТекстСообщения = "";
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			Если Не СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" არ არის მომზადებული.';ru='Рассылка ""%1"" не подготовлена.'");
			ИначеЕсли Не СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" არ არის შესრულებული.';ru='Рассылка ""%1"" не выполнена.'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" შეცდომებით შესრულდა.';ru='Рассылка ""%1"" выполнена с ошибками.'");
			Иначе
				ШаблонСообщения = НСтр("ka='დაგზავნა ""%1"" შესრულებულია.';ru='Рассылка ""%1"" выполнена.'");
			КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='პრობლემები ანგარიშგებების დაგზავნასთან';ru='Проблемы с рассылками отчетов'");
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КомандаСоздать.Заголовок = НСтр("ka='ანგარიშგებების დაგზავნის შექმნა...';ru='Создать рассылку отчетов...'");
	КомандаСоздать.Подсказка = НСтр("ka='ანგარიშგებების ახალი დაგზავნის შექმნა და მასში ანგარიშგების დამატება მიმდინარე გამართვებითურთ.';ru='Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.'");
	...
	КомандаПрисоединить.Заголовок = НСтр("ka='ანგარიშგებების არსებულ დაგზავნაში ჩართვა...';ru='Включить в существующую рассылку отчетов...'");
	КомандаПрисоединить.Подсказка = НСтр("ka='მიმდინარე გამართვებითურთ ანგარიშგების მიერთება ანგარიშგებების არსებულ დაგზავნაზე.';ru='Присоединить отчет с текущими настройками к существующей рассылке отчетов.'");
	...
	Подменю.Заголовок = НСтр("ka='დაგზავნის გაგზავნა';ru='Отправить рассылку'");
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия = НСтр("ka='ანგარიშგებების დაგზავნა. ამოქმედება განრიგის მიხედვით';ru='Рассылка отчетов. Запуск по расписанию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	ШаблонСообщенияОбОшибке = НСтр("ka='მიმღებების სია ""%1""-ის ჩამოყალიბებისას შეცდომა წარმოიშვა:';ru='При формировании списка получателей ""%1"" возникла ошибка:'");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%1""-ის გამართვების შეერთების და ჩატვირთვის დროს შეცდომა წარმოიშვა.';ru='При подключении и загрузке настроек отчета ""%1"" возникла ошибка.'"),
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Не ПараметрыОтчета.Свойство("Инициализирован") Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			НСтр("ka='ანგარიშგება ""%1"" არ არის ინიციალიზებული';ru=""Отчет '%1' не инициализирован"""), ,
			Строка(ПараметрыОтчета.Отчет));
		Возврат Результат;
	КонецЕсли;
	...
	Если Не Формирование.Успех Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='ანგარიშგება ""%1""';ru='Отчет ""%1"":'"), Формирование.ТекстОшибки,
			Строка(ПараметрыОтчета.Отчет));
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	Результат = Ложь;
	ШаблонСообщенияОбОшибке = НСтр("ka='ანგარიშგებების მიწოდების შეცდომა';ru='Ошибка доставки отчетов'");
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = НСтр("ka='ელექტრონული ფოსტით დაგზავნის შესახებ შეტყობინების გაგზავნა შეუძლებელია:';ru='Невозможно отправить уведомление о рассылке по электронной почте:'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = НСтр("ka='ელექტრონული ფოსტით ანგარიშგების გაგზავნა შეუძლებელია:';ru='Невозможно отправить отчет по электронной почте:'");
	...
	Если Рассылка.ИспользоватьСетевойКаталог Тогда
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогWindows) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("ka='Windows-ის სერვერიდან ანგარიშგებების გამოსაქვეყნებელი ქსელური კატალოგი.';ru='Сетевой каталог для публикации отчетов с сервера Windows.'"));
		...
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогLinux) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("ka='ქსელური კატალოგი Linux-ის სერვერიდან ანგარიშგებების გამოსაქვეყნებლად.';ru='Сетевой каталог для публикации отчетов с сервера Linux.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Если Рассылка.ИспользоватьFTPРесурс Тогда
		Если ЗначениеЗаполнено(Рассылка.FTPСервер) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				Рассылка.FTPПорт,
				НСтр("ka='FTP რესურსი ანგარიშგებების გამოსაქვეყნებლად.';ru='FTP ресурс для публикации отчетов.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Возврат Разрешения;
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = НСтр("ka='ანგარიშგება %1-ის ფორმატ %2-ში ჩაწერის შეცდომა:';ru='Ошибка записи отчета %1 в формат %2:'");
			
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ЗаголовокОшибки, НСтр("ka='ფორმატს მხარდაჭერა არ გააჩნია';ru='Формат не поддерживается'"),
					"'"+ СтрокаОтчет.Ключ +"'", 
					"'"+ ПараметрыФормата.Имя +"'");
				Продолжить;
			КонецЕсли;
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокОшибки + Символы.ПС + НСтр("ka='ფაილი %3 ვერ მოიძებნა.';ru='Файл %3 не найден.'"), ,
				"'"+ СтрокаОтчет.Ключ +"'", 
				"'"+ ПараметрыФормата.Имя +"'",
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.Папка) Тогда
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ka='საქაღალდე არ არის შევსებული, საქაღალდეში მიწოდება გამორთულია';ru='Папка не заполнена, доставка в папку отключена'"));
		Иначе
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
				МодульРаботаСФайламиСлужебный = ОбщегоНазначения.ОбщийМодуль("РаботаСФайламиСлужебный");
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			Если ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) Тогда
				ЗначениеПодстановки = НСтр("ka='Linux';ru='Linux'");
			ИначеЕсли ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогLinux) Тогда
				ЗначениеПодстановки = НСтр("ka='Windows';ru='Windows'");
			Иначе
				ЗначениеПодстановки = НСтр("ka='Windows-ი და Linux-ი';ru='Windows и Linux'");
			КонецЕсли;
			
			ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ka='ქსელური კატალოგი %1 არ არის არჩეული, ქსელურ კატალოგში მიწოდება გამორთულია';ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена'"), ,
				ЗначениеПодстановки);
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс И Не ЗначениеЗаполнено(ПараметрыДоставки.Сервер) Тогда
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='FTP სერვერი არ არის შევსებული, FTP რესურსზე საქაღალდეში მიწოდება გამორთულია';ru='FTP сервер не заполнен, доставка в папку на FTP ресурс отключена'"));
	КонецЕсли;
	
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту И Не ЗначениеЗаполнено(ПараметрыДоставки.УчетнаяЗапись) Тогда
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='სააღრიცხვო ჩანაწერი არ არის არჩეული, ელექტრონული ფოსტით მიწოდება გამორთულია';ru='Учетная запись не выбрана, доставка по электронной почте отключена'"));
	КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если Не ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ka='პერსონალიზებული დაგზავნა შეიძლება გაიგზავნოს მხოლოდ ელექტრონული ფოსტით';ru='Персонализированная рассылка может быть отправлена только по электронной почте'"));
			Возврат Ложь;
		КонецЕсли;
		ПараметрыДоставки.ИспользоватьПапку          = Ложь;
		ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить
			...
			И Не ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ka='ელექტრონული ფოსტით შეტყობინების გამოყენება შესაძლებელია მხოლოდ მიწოდების სხვა მეთოდებთან ერთობლივად';ru='Использование уведомлений по электронной почте возможно только совместно с другими способами доставки'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		Если ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя = 
				...
				+ Символы.ПС 
				+ Символы.ПС
				+ НСтр("ka='ანგარიშგებების ფაილები შეფუთულია არქივში';ru='Файлы отчетов запакованы в архив'")
				+ " ";
		КонецЕсли;
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если Не ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ka='ინფორმაციის გამოსატანად მომხმარებელ %1-ს არასაკმარისი უფლებები გააჩნია';ru='У пользователя %1 недостаточно прав на вывод информации'"), ,
