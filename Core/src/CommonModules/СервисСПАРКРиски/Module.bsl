...
Процедура ЗагрузитьИндексыКонтрагентовВФоне(Параметры, АдресРезультата) Экспорт
	...
	ДлительныеОперации.СообщитьПрогресс(
		100,
		НСтр("ka='СервисСПАРКРиски.ЗагрузитьИндексыКонтрагентовВФоне завершено';ru='СервисСПАРКРиски.ЗагрузитьИндексыКонтрагентовВФоне завершено';en='ServisSPARKRiski.ZagruzitIndeksyKontragentovVFone completed'"),
...
#Область ПроверкаПодключенияИнтернетПоддержки
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ЗагрузитьИндексыКонтрагентов, ПередВызовомОперацииСервиса';ru='Выполнение ЗагрузитьИндексыКонтрагентов, ПередВызовомОперацииСервиса';en='Performing ZagruzitIndeksyKontragentov, PeredVyzovomOperatsiiServisa'"));
		...
		Если НЕ Результат.ВидОшибки.Пустая() Тогда
			...
			ОписаниеРезультата = СтрШаблон(
				НСтр("ka='Ошибка выполнения: %1';ru='Ошибка выполнения: %1';en='Runtime Error:% 1'"),
				Результат.ВидОшибки);
...
#Область СозданиеТаблицыЗначений
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ЗагрузитьИндексыКонтрагентов, СозданиеТаблицыЗначений';ru='Выполнение ЗагрузитьИндексыКонтрагентов, СозданиеТаблицыЗначений';en='Performing ZagruzitIndeksyKontragentov, SozdanieTablitsyZnacheny'"));
...
#Область ДополнительныеПараметрыВызоваОперацииСервиса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ЗагрузитьИндексыКонтрагентов, ДополнительныеПараметрыВызоваОперацииСервиса';ru='Выполнение ЗагрузитьИндексыКонтрагентов, ДополнительныеПараметрыВызоваОперацииСервиса';en='Performing ZagruzitIndeksyKontragentov, DopolnitelnyeParametryVyzovaOperatsiiServisa'"));
...
#Область ВызовВебСервиса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ЗагрузитьИндексыКонтрагентов, ВызовВебСервиса';ru='Выполнение ЗагрузитьИндексыКонтрагентов, ВызовВебСервиса';en='Performing ZagruzitIndeksyKontragentov, VyzovVebServisa'"));

	ОписаниеРезультата = СтрШаблон(
		НСтр("ka='Вызов метода веб-сервиса для %1 записей (пачками).';ru='Вызов метода веб-сервиса для %1 записей (пачками).';en='Method call a web service for% 1 records (batches).'"),
	...
	Для Каждого ТекущийКонтрагент Из МассивОписанийКонтрагентов Цикл
		...
		Если РазмерЗапроса >= 100 Тогда
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ Символы.ПС
				+ СтрШаблон(
					НСтр("ka='Обработка пачки %1: %2 мс; %3';ru='Обработка пачки %1: %2 мс; %3';en='Treatment pack% 1% 2 ms; 3%'"),
					НомерПачки,
					ТКонец - ТНачало,
					?(Результат.ВидОшибки.Пустая(),
						"",
						СтрШаблон(
							НСтр("ka='Вид ошибки: %1';ru='Вид ошибки: %1';en='Error type:% 1'"),
							Результат.ВидОшибки)));
	...
	Если РазмерЗапроса > 0 Тогда
		...
		ОписаниеРезультата = ОписаниеРезультата
			+ Символы.ПС
			+ СтрШаблон(
				НСтр("ka='Обработка пачки %1 (последней): %2 мс; %3';ru='Обработка пачки %1 (последней): %2 мс; %3';en='Treatment pack 1% (last)% 2 ms; 3%'"),
				НомерПачки,
				ТКонец - ТНачало,
				?(Результат.ВидОшибки.Пустая(),
					"",
...
#Область ЗаполнениеГрадациямиИндексовИРеквизитами
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ЗагрузитьИндексыКонтрагентов, ЗаполнениеГрадациямиИндексовИРеквизитами';ru='Выполнение ЗагрузитьИндексыКонтрагентов, ЗаполнениеГрадациямиИндексовИРеквизитами';en='Performing ZagruzitIndeksyKontragentov, ZapolnenieGradatsiyamiIndeksovIRekvizitami'"));

	ОписаниеРезультата = СтрШаблон(
		НСтр("ka='Обработка результата веб-сервиса для %1 записей.';ru='Обработка результата веб-сервиса для %1 записей.';en='Web service processing result for% 1 records.'"),
		МассивОписанийКонтрагентов.Количество());
	...
	ОписаниеРезультата = ОписаниеРезультата
		+ Символы.ПС
		+ СтрШаблон(
			НСтр("ka='Подготовка массива контрагентов (%1 элементов): %2 мс';ru='Подготовка массива контрагентов (%1 элементов): %2 мс';en='Preparation array counterparts (1% components)% 2 ms'"),
	...
	ОписаниеРезультата = ОписаниеРезультата
		+ Символы.ПС
		+ СтрШаблон(
			НСтр("ka='Получение реквизитов для массива контрагентов (%1 элементов): %2 мс';ru='Получение реквизитов для массива контрагентов (%1 элементов): %2 мс';en='Preparation details to counterparties array (1% components)% 2 ms'"),
			МассивКонтрагентов.Количество(),
			ТКонец - ТНачало);
	...
	ОписаниеРезультата = ОписаниеРезультата
		+ Символы.ПС
		+ СтрШаблон(
			НСтр("ka='Заполнение результата веб-сервиса (%1 элементов) реквизитами и градациями индексов: %2 мс';ru='Заполнение результата веб-сервиса (%1 элементов) реквизитами и градациями индексов: %2 мс';en='Filling result web service (1% cells) of the essential elements and gradation index:% 2ms'"),
...
#Область ДозаполнениеРезультатаИСохранениеВКэше
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ЗагрузитьИндексыКонтрагентов, ДозаполнениеРезультатаИСохранениеВКэше';ru='Выполнение ЗагрузитьИндексыКонтрагентов, ДозаполнениеРезультатаИСохранениеВКэше';en='Performing ZagruzitIndeksyKontragentov, DozapolnenieRezultataISohranenieVKeshe'"));

	ОписаниеРезультата = НСтр("ka='Дозаполнение результата веб-сервиса.';ru='Дозаполнение результата веб-сервиса.';en='Afterfilling result of a web service.'");
		...
		Если (ТипЗнч(Результат.ЗначенияИндексов) = ТипТаблицаЗначений) И (Результат.ЗначенияИндексов.Количество() > 0) Тогда
			...
			Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтрШаблон(
						НСтр("ka='Идентификаторы типов событий, подготовка списка из %1 в %2 элементов: %3 мс';ru='Идентификаторы типов событий, подготовка списка из %1 в %2 элементов: %3 мс';en='Identifiers types of events, to prepare a list of 1% to 2% of elements:% 3ms'"),
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтрШаблон(
						НСтр("ka='Идентификаторы типов событий, загрузка списка новых событий (%1 элементов): %2 мс';ru='Идентификаторы типов событий, загрузка списка новых событий (%1 элементов): %2 мс';en='Identifiers event types, downloading the list of new events (% 1 cells)% 2 ms'"),
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ Символы.ПС
				+ СтрШаблон(
					НСтр("ka='Заполнение списка событий в результате (%1 элементов): %2 мс';ru='Заполнение списка событий в результате (%1 элементов): %2 мс';en='Filling event list resulting from the (1% components)% 2 ms'"),
					ЛокальныйЗначенияИндексов.Количество(),
					ТКонец - ТНачало);
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ Символы.ПС
				+ СтрШаблон(
					НСтр("ka='Сохранение результата (%1 элементов) в кэше: %2 мс';ru='Сохранение результата (%1 элементов) в кэше: %2 мс';en='Store the results of (1% of elements) in the cache% 2 ms'"),
					МассивОписанийКонтрагентов.Количество(),
					ТКонец - ТНачало);
...
"';ka='СервисСПАРКРиски.ЗагрузитьИндексыКонтрагентов, результат."
"Длительность (мс): %1"
"Результат выполнения:"
"%2"
""
...
"'"),
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ka='СПАРК Риски. Отладка. ЗагрузитьИндексыКонтрагентов';ru='СПАРК Риски. Отладка. ЗагрузитьИндексыКонтрагентов';en='SPARK risks. Debugging. ZagruzitIndeksyKontragentov'"), // ИдентификаторШага
...
Функция НачатьПодготовкуСправок(СписокИНН) Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЗапросНовойСправкиСПАРКРиски", , Ложь) Тогда
		...
		Результат.СообщениеОбОшибке  = НСтр("ka='Недостаточно прав для запроса новой справки 1СПАРК Риски.';ru='Недостаточно прав для запроса новой справки 1СПАРК Риски.';en='Insufficient permissions for the new reference 1SPARK Risks request.'");
...
"Некорректное значение параметра СписокИНН. Количество элементов списка превышает 100.';ka='Исключительная ситуация при вызове метода СПАРК.ВключитьМониторингСобытийПоКонтрагентам()."
...
"Обратитесь к администратору.';ka='Недостаточно прав для работы с сервисом 1СПАРК Риски."
"Обратитесь к администратору.'");
		Результат.ИнформацияОбОшибке = НСтр("ka='Недостаточно прав для работы с сервисом 1СПАРК Риски.';ru='Недостаточно прав для работы с сервисом 1СПАРК Риски.';en='Not have permission to work with the service 1SPARK risks.'");
	...
	Если ЗаполнитьДанныеАутентификации И Результат.ВидОшибки.Пустая() Тогда
		...
		Если ДанныеАутентификации = Неопределено Тогда
			...
			Результат.СообщениеОбОшибке  = НСтр("ka='Интернет-поддержка пользователей не подключена.';ru='Интернет-поддержка пользователей не подключена.';en='Internet-user support is not connected.'");
			Результат.ИнформацияОбОшибке = Результат.СообщениеОбОшибке;
...
"%2';ka='Не удалось вызвать операцию %1. Ошибка аутентификации."
"%2'"),
					URLОперации,
			...
			ИначеЕсли ИмяСвойства = "results" Тогда
				...
				ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения <> ТипЗначенияJSON.НачалоМассива Тогда
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Некорректное значение свойства results. Ожидается тип значения Массив; Фактический тип значения: %1.';ru='Некорректное значение свойства results. Ожидается тип значения Массив; Фактический тип значения: %1.';en='Incorrect value of the results. value type array is expected; The actual type of value: 1%.'"),
...
Процедура ПрочитатьСобытияМониторинга(ЧтениеОтвета, Результат, ИНН)
	...
	ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения <> ТипЗначенияJSON.НачалоМассива Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Некорректное значение свойства results[].events. Ожидается тип значения Массив; Фактический тип значения: %1.';ru='Некорректное значение свойства results[].events. Ожидается тип значения Массив; Фактический тип значения: %1.';en='Incorrect value of the results []. Events. value type array is expected; The actual type of value: 1%.'"),
			Строка(ЧтениеОтвета.ТипТекущегоЗначения));
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура ПроверитьДоступностьСервиса(РезультатОперации)
	...
	ИначеЕсли РезультатПроверки.ИмяОшибки = "ServerError" Тогда
		...
		РезультатОперации.СообщениеОбОшибке =
			НСтр("ka='Сервис временно недоступен.';ru='Сервис временно недоступен.';en='Service is temporarily unavailable.'")
			+ Символы.ПС
	...
	Иначе
		...
		РезультатОперации.СообщениеОбОшибке =
			НСтр("ka='Не удалось подключиться к сервису.';ru='Не удалось подключиться к сервису.';en='Unable to connect to the service.'")
			+ Символы.ПС
			+ РезультатПроверки.СообщениеОбОшибке;
		
	КонецЕсли;
	
	РезультатОперации.ИнформацияОбОшибке = НСтр("ka='Не удалось проверить доступность сервиса.';ru='Не удалось проверить доступность сервиса.';en='Failed to check service availability.'")
...
Процедура ПриОшибкеВызоваОперацииСервиса(РезультатЗагрузкиСодержимого, URLОперации, Результат)
	...
	ШапкаИнформацииОбОшибке =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Ошибка при вызове операции %1 сервиса СПАРК.';ru='Ошибка при вызове операции %1 сервиса СПАРК.';en='Error when calling% 1 service operations SPARK.'"),
			URLОперации);
	...
	Если ЭтоОбрабатываемыйКодОшибкиСервиса(РезультатЗагрузкиСодержимого.КодСостояния) Тогда
		...
		Исключение
			ПодвалСообщенияЖурналаРегистрации =
				НСтр("ka='Не удалось обработать описание ошибки.';ru='Не удалось обработать описание ошибки.';en='Unable to handle the error.'")
	...
	Если РезультатЗагрузкиСодержимого.КодСостояния = 402 Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ka='Требуется оплата сервиса.';ru='Требуется оплата сервиса.';en='Payment is required in service.'");
		КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния = 403 Тогда
		...
		Иначе
			...
			Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена;
			Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
				Результат.СообщениеОбОшибке = НСтр("ka='Неверный логин или пароль пользователя Интернет-поддержки.';ru='Неверный логин или пароль пользователя Интернет-поддержки.';en='Invalid username or password of the user of Internet support.'");
			КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния = 404 Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ka='Запрошенные данные не найдены.';ru='Запрошенные данные не найдены.';en='Requested data not found.'");
		КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодОшибки = "ConnectError" Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ka='Не удалось подключиться к сервису.';ru='Не удалось подключиться к сервису.';en='Unable to connect to the service.'")
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодОшибки = "ClientError" Тогда
		...
		Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ОшибкаПодключения;
		Результат.СообщениеОбОшибке = НСтр("ka='Не удалось получить данные из сервиса СПАРК.';ru='Не удалось получить данные из сервиса СПАРК.';en='Unable to obtain data from the service SPARK.'")
			+ Символы.ПС
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодОшибки = "ServerError" Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ka='В сервисе возникли неполадки.';ru='В сервисе возникли неполадки.';en='The service has a problem.'")
			+ Символы.ПС
			+ ?(ПустаяСтрока(Результат.СообщениеОбОшибке),
				РезультатЗагрузкиСодержимого.СообщениеОбОшибке,
				Результат.СообщениеОбОшибке);
	...
	Иначе
		...
		Результат.СообщениеОбОшибке = НСтр("ka='Ошибка при получении данных из сервиса 1СПАРК Риски.';ru='Ошибка при получении данных из сервиса 1СПАРК Риски.';en='Error retrieving data from the service 1SPARK risks.'")
			+ Символы.ПС + РезультатЗагрузкиСодержимого.СообщениеОбОшибке;
...
"Тело ответа: %3';ka='Ошибка при вызове операции %1 сервиса СПАРК."
"Ошибка при обработке ответа сервиса. %2"
"Тело ответа: %3'"),
		URLОперации,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
	...
	Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса;
	Результат.СообщениеОбОшибке = НСтр("ka='Некорректный ответ сервиса.';ru='Некорректный ответ сервиса.';en='Incorrect answer service.'");
...
Функция ЗначениеСвойстваJSON(ЧтениеОбъектаJSON, ЗначениеПоУмолчанию = Неопределено)
	...
	Иначе
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось прочитать значение свойства %1. Некорректный тип значения свойства (%2).';ru='Не удалось прочитать значение свойства %1. Некорректный тип значения свойства (%2).';en='Unable to read the value of 1%. Incorrect value type property (2%).'"),
			ИмяСвойства,
