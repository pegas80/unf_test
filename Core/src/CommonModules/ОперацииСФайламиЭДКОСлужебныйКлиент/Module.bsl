...
Процедура ОткрытьФайлПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ka='Не удалось открыть файл.';ru='Не удалось открыть файл.'");
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ОписаниеОшибки);
	КонецЕсли;
...
Процедура СохранитьФайлы(Знач СохраняемыеФайлы, КаталогСохранения, ВыводитьСообщения) Экспорт
	...
	Если ТипЗнч(СохраняемыеФайлы) = Тип("Массив") И СохраняемыеФайлы.Количество() = 0 Тогда
		...
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(НСтр("ka='Отсутствуют файлы для сохранения';ru='Отсутствуют файлы для сохранения'"));
		Возврат;
	КонецЕсли;
...
Процедура СохранитьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Если Подключено Тогда
		...
		Если ТребуетсяПоказатьДиалогВыбораКаталога Тогда
			...
			Диалог.Заголовок = НСтр("ka='Выберите каталог для сохранения';ru='Выберите каталог для сохранения'");
...
Процедура СохранитьФайлыПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ВыводитьСообщения Тогда
		...
		Если КоличествоСохраняемыхФайлов <> 0 Тогда
			...
			СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгружено файлов: %1.';ru='Выгружено файлов: %1.'"), КоличествоСохраненныхФайлов);
			...
			Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
				Если ОписаниеФайла.Размер > ВходящийКонтекст.Параметры.МаксимальныйРазмерФайла Тогда
					ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
						СтрШаблон(НСтр("ka='Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.';ru='Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.'"),
							ОписаниеФайла.Имя,
		...
		Если ВходящийКонтекст.Параметры.Свойство("ДопустимыеТипыФайлов")
			...
			Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
				Если ДопустимыеТипыФайлов.Найти(ОписаниеФайла.Расширение) = Неопределено Тогда
					ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
						СтрШаблон(НСтр("ka='Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".';ru='Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".'"),
...
Функция ОписаниеОшибкиНеПолученКаталогВременныхФайлов()
	...
	Возврат НСтр("ka='Не удалось получить каталог временных файлов на клиенте.';ru='Не удалось получить каталог временных файлов на клиенте.'");

КонецФункции

Функция ОписаниеОшибкиНеУдалосьСоздатьКаталог()
	
	Возврат НСтр("ka='Не удалось создать каталог на клиенте.';ru='Не удалось создать каталог на клиенте.'");
...
Функция ОписаниеОшибкиНеУдалосьСоздатьОбъектДвоичныеДанные()
	...
	Возврат НСтр("ka='Не удалось создать объект для работы с двоичными данными.';ru='Не удалось создать объект для работы с двоичными данными.'");
...
Функция ОписаниеОшибкиОшибкаЧтенияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка чтения файла %1.';ru='Ошибка чтения файла %1.'"), ИмяФайла);
...
Функция ОписаниеОшибкиОшибкаСохраненияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Во время сохранения файла %1 возникла ошибка.';ru='Во время сохранения файла %1 возникла ошибка.'"), ИмяФайла);	
...
Функция ОписаниеОшибкиФайлНеСуществует(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка обращения к файлу %1 - файл не существует.';ru='Ошибка обращения к файлу %1 - файл не существует.'"), ИмяФайла);	
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ka='Электронный документооборот с контролирующими органами. Операции с файлами';ru='Электронный документооборот с контролирующими органами. Операции с файлами'");
