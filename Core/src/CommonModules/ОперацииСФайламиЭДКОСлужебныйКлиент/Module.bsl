...
"расширение для работы с файлами не подключено.';ka='Не удалось открыть файл %1, т.к."
...
Процедура ОткрытьФайлПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ka='Не удалось открыть файл.';ru='Не удалось открыть файл.';en='Unable to open file.'");
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ОписаниеОшибки);
	КонецЕсли;
...
"Возможно файл уже открыт.';ka='Не удалось открыть файл %1."
...
Процедура ЗапуститьПриложениеНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ka='Не удалось запустить приложение,"
"т.к. расширение для работы с файлами не подключено.';ru='Не удалось запустить приложение,"
...
Процедура СохранитьФайлы(Знач СохраняемыеФайлы, КаталогСохранения, ВыводитьСообщения) Экспорт
	...
	Если ТипЗнч(СохраняемыеФайлы) = Тип("Массив") И СохраняемыеФайлы.Количество() = 0 Тогда
		...
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(НСтр("ka='Отсутствуют файлы для сохранения';ru='Отсутствуют файлы для сохранения';en='There are no files to be saved'"));
		Возврат;
	КонецЕсли;
...
Процедура СохранитьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Если Подключено Тогда
		...
		Если ТребуетсяПоказатьДиалогВыбораКаталога Тогда
			...
			Диалог.Заголовок = НСтр("ka='Выберите каталог для сохранения';ru='Выберите каталог для сохранения';en='Select a directory to save'");
		...
		Исключение
			...
			Если ВходящийКонтекст.ВыводитьСообщения Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Не удалось выгрузить на диск файл "
"%1!"
"%2';ru='Не удалось выгрузить на диск файл "
"%1!"
	...
	Если НомерТекущегоФайла <= ЧислоФайлов - 1 Тогда
		...
		Если НЕ ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
			Если СтрДлина(Имя) > 260 Тогда
				ПоказатьПредупреждение(, НСтр("ka='Длина пути одного или нескольких файлов превышает 260 символов."
"Файлы не могут быть сохранены."
"Выберите другой каталог сохранения файлов.';ru='Длина пути одного или нескольких файлов превышает 260 символов."
"Файлы не могут быть сохранены."
"Выберите другой каталог сохранения файлов.'"));
	...
	Иначе
		...
		Если ВходящийКонтекст.Свойство("ЗапроситьПодтверждениеОбновитьФайлы") Тогда
			...
			ТекстВопроса = НСтр("ka='В выбранном каталоге присутствуют файлы с такими же именами."
"Продолжить сохранение с заменой файлов?';ru='В выбранном каталоге присутствуют файлы с такими же именами."
...
Процедура СохранитьФайлыПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ВыводитьСообщения Тогда
		Если ПолученныеФайлы = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не удалось выгрузить файлы';en='Failed to upload files'"));
			Возврат;
		КонецЕсли;
		...
		Если КоличествоСохраняемыхФайлов <> 0 Тогда
			...
			СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгружено файлов: %1.';ru='Выгружено файлов: %1.';en='Unloaded file:% 1.'"), КоличествоСохраненныхФайлов);
			
			Если КоличествоСохраненныхФайлов <> КоличествоСохраняемыхФайлов Тогда
				СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1"
"Не удалось выгрузить: %2.';ru='%1"
			...
			Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
				Если ОписаниеФайла.Размер > ВходящийКонтекст.Параметры.МаксимальныйРазмерФайла Тогда
					ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
						СтрШаблон(НСтр("ka='Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.';ru='Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.';en='File Size ""%1"" (% 2) does not meet the requirements: the maximum file size - 3%.'"),
							ОписаниеФайла.Имя,
		...
		Если ВходящийКонтекст.Параметры.Свойство("ДопустимыеТипыФайлов")
			...
			Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
				Если ДопустимыеТипыФайлов.Найти(ОписаниеФайла.Расширение) = Неопределено Тогда
					ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
						СтрШаблон(НСтр("ka='Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".';ru='Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".';en='file type ""%1"" does not meet the requirements: Permitted file types - ""2%"".'"),
...
Функция ОписаниеОшибкиНеПолученКаталогВременныхФайлов()
	...
	Возврат НСтр("ka='Не удалось получить каталог временных файлов на клиенте.';ru='Не удалось получить каталог временных файлов на клиенте.';en='Could not get temporary file directory on the client.'");

КонецФункции

Функция ОписаниеОшибкиНеУдалосьСоздатьКаталог()
	
	Возврат НСтр("ka='Не удалось создать каталог на клиенте.';ru='Не удалось создать каталог на клиенте.';en='Unable to create a directory on the client.'");
...
"Дальнейшее выполнение операции невозможно.';ka='Расширение для работы с файлами не подключено. "
...
Функция ОписаниеОшибкиНеУдалосьСоздатьОбъектДвоичныеДанные()
	...
	Возврат НСтр("ka='Не удалось создать объект для работы с двоичными данными.';ru='Не удалось создать объект для работы с двоичными данными.';en='Failed to create object for working with binary data.'");
...
Функция ОписаниеОшибкиОшибкаЧтенияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка чтения файла %1.';ru='Ошибка чтения файла %1.';en='Error reading file% 1.'"), ИмяФайла);
...
Функция ОписаниеОшибкиОшибкаСохраненияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Во время сохранения файла %1 возникла ошибка.';ru='Во время сохранения файла %1 возникла ошибка.';en='While saving the file% 1 An error occurred.'"), ИмяФайла);	
...
"%1"
"по причине:"
"%2';ka='Не удалось удалить файл или каталог"
"%1"
"по причине:"
"%2'");
...
Функция ОписаниеОшибкиФайлНеСуществует(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка обращения к файлу %1 - файл не существует.';ru='Ошибка обращения к файлу %1 - файл не существует.';en='Error handling of the file% 1 - the file does not exist.'"), ИмяФайла);	
...
"%2';ka='Ошибка при вызове ""%1"" объекта для работы с файлами."
"%2'"),
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ka='Электронный документооборот с контролирующими органами. Операции с файлами';ru='Электронный документооборот с контролирующими органами. Операции с файлами';en='Electronic document management with regulatory authorities. File operations'");
...
Процедура ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ИмяМетода, Контекст, ВозможноПродолжениеБезУстановки)
	...
	ТекстСообщения = НСтр("ka='Для работы подсистемы документооборота с контролирующими органами необходимо установить расширение работы с файлами.';ru='Для работы подсистемы документооборота с контролирующими органами необходимо установить расширение работы с файлами.';en='To work workflow subsystem with the supervisory authorities is necessary to install the extension to work with files.'");
