...
Процедура ДобавитьСообщение(Комментарий, Ссылка, Форма) Экспорт
	...
	Если (Ссылка.Пустая() Или Форма.Модифицированность) Тогда
		ТекстВопроса = НСтр("ka='კომენტარის დასატოვებლად საჭიროა ცვლილებების ჩაწერა. გავაგრძელოთ?';ru='Чтобы оставлять комментарии, нужно записать изменения. Продолжить?';en='To post comments, you need to save the changes. Proceed?'");
	...
	Если ИмяСобытия=ИмяСобытияЗаписьПрисоединенногоФайла() Тогда
		Если Параметр.Свойство("ВладелецФайла") И Параметр.ВладелецФайла = Ссылка Тогда
			...
			Если Параметр.Свойство("ЭтоНовый") И Параметр.ЭтоНовый=Истина Тогда
				ОбсужденияВызовСервера.ДобавитьСообщение(НСтр("ru='Присоединен файл ';ka='Присоединен файл '")+ТекстИсточник, Ссылка, Истина);
			Иначе
				ОбсужденияВызовСервера.ДобавитьСообщение(НСтр("ru='Записан файл ';ka='Записан файл '")+ТекстИсточник, Ссылка,Истина);
			КонецЕсли;
...
Процедура ОбработатьНажатие(Форма, Элемент, ДанныеСобытия, СтандартнаяОбработка) Экспорт
	...
	Если ДанныеСобытия.Свойство("Href") И ЗначениеЗаполнено(ДанныеСобытия.Href) Тогда
		...
		Иначе
			//--------------------------------------------------------------------------
			// © Шишка В.В. [10 ноября 2021]  НАЧАЛО
			//Это откровенный костыль
			//Из - за невыясненных остоятельств не открывается навигационная ссылка в глобальном формате вида:
			// e1c:/filev/<Буква диска>/<Путь к базе>/e1cib/data/...
			...
			Исключение
				...
				ПозицияЛокальная = СтрНайти(ДанныеСобытия.Href, "e1cib/data/");
				
				ЛокальнаяСсылка = Сред(ДанныеСобытия.Href, ПозицияЛокальная);
				ПерейтиПоНавигационнойСсылке(ЛокальнаяСсылка);
				
			КонецПопытки;	
			// © Шишка В.В. [10 ноября 2021]  КОНЕЦ
			//--------------------------------------------------------------------------
		КонецЕсли; 
	КонецЕсли; 
