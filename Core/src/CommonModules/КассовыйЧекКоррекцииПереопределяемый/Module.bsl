...
Процедура ПодготовитьИПроверитьПечатьЧека(Объект,
	...
	Если ЗначениеЗаполнено(Объект.КассаККМ)  Тогда
		...
		ТекстОшибки = НСтр("ka='Кассовая смена не открыта!';ru='Кассовая смена не открыта!';en='Cash change is not open!'");
		...
		КонецЕсли;
	Иначе
		ТекстОшибки = НСтр("ka='Не выбрана касса ККМ!';ru='Не выбрана касса ККМ!';en='Not selected POS cash register!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			,
			"Объект.КассаККМ",
...
Процедура ИзменитьФормуДокумента(Форма)
	...
	НовоеПоле.Заголовок					= НСтр("ka='სურათი დოკუმენტის მდგომარეობა';ru='Картинка состояния документа';en='document state image'");
