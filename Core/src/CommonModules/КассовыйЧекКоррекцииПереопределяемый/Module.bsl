...
Процедура ПодготовитьИПроверитьПечатьЧека(Объект,
	...
	Если ЗначениеЗаполнено(Объект.КассаККМ)  Тогда
		...
		ТекстОшибки = НСтр("ka='Кассовая смена не открыта!';ru='Кассовая смена не открыта!'");
		...
		КонецЕсли;
	Иначе
		ТекстОшибки = НСтр("ka='Не выбрана касса ККМ!';ru='Не выбрана касса ККМ!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			,
			"Объект.КассаККМ",
...
Процедура ИзменитьФормуДокумента(Форма)
	...
	НовоеПоле.Заголовок					= НСтр("ka='სურათი დოკუმენტის მდგომარეობა';ru='Картинка состояния документа'");
