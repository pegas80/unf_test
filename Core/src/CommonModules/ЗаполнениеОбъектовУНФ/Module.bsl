...
Процедура ВызватьОбработчикПередЗаполнением(СтратегияЗаполнения, ДокументОбъект, ДанныеЗаполнения)
	...
	Если ТипЗнч(СтратегияЗаполнения) <> Тип("Соответствие") Тогда
		ВызватьИсключение НСтр("ka='პარამეტრ ""შევსებისდამამუშავებელი""-ს არაკორექტული ტიპი: მოსალოდნელია ""სტრიქონი"" ან ""შესაბამისობა"".';ru='Некорректный тип параметра ""ОбработчикЗаполнения"": ожидается Строка или Соответствие.'");
	КонецЕсли;
...
Процедура ДополнитьОснованиеПечати(ЗначенияИзНастроек, ДокументОбъект)
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Договор) Тогда
		...
		"ОснованиеПечати",
		СтрШаблон(
		НСтр("ka='ხელშეკრულება: %1';ru='Договор: %1'"),
...
Процедура РазыменоватьПоляКонтрагента(РазыменованныеПоля)
	...
	Если ДанныеКонтрагента.ЭтоГруппа Тогда
		ВызватьИсключение НСтр("ka='კონტრაგენტების ჯგუფის არჩევა არ შეიძლება.';ru='Нельзя выбирать группу контрагентов.'");
	КонецЕсли;
...
Функция ПолучитьСостояниеЗаказаПокупателя(знач ВидЗаказа = Неопределено) Экспорт
	...
	Если ИспользуютсяВидыЗаказов И Не ЗначениеЗаполнено(ВидЗаказа) Тогда
		ВызватьИсключение НСтр("ka='Не указан вид заказа для заполнения состояния заказа покупателя';ru='Не указан вид заказа для заполнения состояния заказа покупателя'");
	КонецЕсли;
...
Функция ПолучитьСостояниеЗаказНаряда(знач ВидЗаказа = Неопределено) Экспорт
	...
	Если ИспользуютсяВидыЗаказов И Не ЗначениеЗаполнено(ВидЗаказа) Тогда
		ВызватьИсключение НСтр("ka='Не указан вид заказа для заполнения состояния заказ-наряда';ru='Не указан вид заказа для заполнения состояния заказ-наряда'");
	КонецЕсли;
