...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление = НСтр("ka='ელექტრონული ფოსტის სააღრიცხვო ჩანაწერები';ru='Учетные записи электронной почты'");
...
Процедура ПроверитьВозможностьОтправкиИПолученияЭлектроннойПочты(УчетнаяЗапись, СообщениеОбОшибке, ДополнительноеСообщение) Экспорт
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляОтправки Тогда
		...
		Исключение
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომა გამავალი ფოსტის გამართვებში: %1';ru='Ошибка в настройках исходящей почты: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		Если Не НастройкиУчетнойЗаписи.ИспользоватьДляПолучения Тогда
			ДополнительноеСообщение = Символы.ПС + НСтр("ka='(ელექტრონული შეტყობინებების გაგზავნის შემოწმება შესრულებულია)';ru='(Выполнена проверка отправки электронных сообщений.)'");
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляПолучения Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = СообщениеОбОшибке + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომა შემომავალი ფოსტის გამართვებში: %1';ru='Ошибка в настройках входящей почты: %1'"),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) );
		КонецПопытки;
		Если Не НастройкиУчетнойЗаписи.ИспользоватьДляОтправки Тогда
			ДополнительноеСообщение = Символы.ПС + НСтр("ka='(ელექტრონული შეტყობინებების მიღების შემოწმება შესრულებულია)';ru='(Выполнена проверка получения электронных сообщений.)'");
		КонецЕсли;
	КонецЕсли;
