...
"Google Chrome и Mozilla Firefox не поддерживается.';ka='Выполнение команды ""%1"" в браузерах"
"Google Chrome и Mozilla Firefox не поддерживается.'");
	Иначе
		ТекстПредупреждения = НСтр("ka='ბრძანება ""%1""-ის შესასრულებლად აუცილებელია ვებ-კლიენტ 1С:საწარმოსთვის გაფართოების დაყენება.';ru='Для выполнения команды ""%1"" необходимо"
...
Процедура КорректноеИмяФайла(ИмяФайла, УдалятьНекорректныеСимволы = Ложь) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ფაილის სახელწოდება არ უნდა შეიცავდეს შემდეგ სიმბოლოებს: %1';ru='В имени файла не должно быть следующих символов: %1';en='The file name should be the following characters:% 1'"), СтрИсключения);
		...
		Если Параметры.РазмерВХранилищеФайлов <> 0
		   И Параметры.РазмерВХранилищеФайлов <> Параметры.РазмерВРабочемКаталоге Тогда
			...
			Параметры.Вставить("Заголовок",
				НСтр("ka='Размер файла отличается';ru='Размер файла отличается';en='File size is different'"));
...
"открыть существующий без обновления?';ka='Размер файла в рабочем каталоге и в хранилище файлов отличается."
""
"Взять файл из хранилища файлов и заменить им существующий или"
"открыть существующий без обновления?'"));
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			...
			Параметры.Вставить("Заголовок", НСтр("ka='ფაილების საცავში ახალი ფაილი';ru='В хранилище файлов новый файл';en='The new file storage file'"));
...
""
"Взять файл из хранилища файлов и заменить им существующий или"
"открыть существующий без обновления?';ka='Файл в хранилище файлов, отмеченный как занятый для редактирования,"
"имеет более позднюю дату изменения (новее), чем в рабочем каталоге."
""
"Взять файл из хранилища файлов и заменить им существующий или"
"открыть существующий без обновления?'"));
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Иначе
			...
			Параметры.Вставить("Заголовок", НСтр("ka='სამუშაო კატალოგში ახალი ფაილი';ru='В рабочем каталоге новый файл';en='In the working directory of the new file'"));
...
"из хранилища файлов c потерей изменений и открыть?';ka='სამუშაო კატალოგში ფაილს ცვლილების (ახალი) უფრო გვიანი თარიღი აქვს, ვიდრე ფაილების საცავში. შეიძლება, იგი შეცვლილია."
""
"გავხსნათ არსებული ფაილი თუ შევცვალოთ იგი ფაილით ფაილების საცავიდან, ცვლილებების დაკარგვით და გავხსნათ?'"));
		КонецЕсли;
	КонецЕсли;
...
"%1';ka='მომხმარებლის მონაცემთა სამუშაო კატალოგის მიღება ვერ მოხერხდა შემდეგი მიზეზით:"
...
Процедура ОбработатьФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПолученныеФайлы) <> Тип("Массив") Или ПолученныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ka='ფაილის მიღება გაუქმდა.';ru='Получение файла было отменено.';en='Fetching file was canceled.'"), , Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПомещенныеФайлы) <> Тип("Массив") Или ПомещенныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ka='ფაილის მოთავსება გაუქმდა.';ru='Помещение файла было отменено.';en='file room was canceled.'"), , Контекст);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьКомпоненту(ОбработчикРезультата) Экспорт
	...
	Если ПроинициализироватьКомпоненту() Тогда
		Состояние(НСтр("ka='სკანირების კომპონენტი უკვე დაყენებულია.';ru='Компонента сканирования уже установлена.';en='Scanning the component is already installed.'"));
		ВернутьРезультат(ОбработчикРезультата, Истина);
		Возврат;
	КонецЕсли;
