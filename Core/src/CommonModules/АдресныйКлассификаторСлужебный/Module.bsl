...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("ka='სამისამართო კლასიფიკატორი ჯერ არ ჩატვირთულა.';ru='Адресный классификатор еще не загружался.';en='Address qualifier is not yet loaded.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("ka='სამისამართო კლასიფიკატორი დღეს ჩაიტვირთა.';ru='Адресный классификатор был загружен сегодня.';en='The address classifier was loaded today.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = НСтр("ka='სამისამართო კლასიფიკატორი გუშინ ჩაიტვირთა.';ru='Адресный классификатор был загружен вчера.';en='The address classifier was loaded yesterday.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = НСтр("ka='სამისამართო კლასიფიკატორი გუშინწინ ჩაიტვირთა.';ru='Адресный классификатор был загружен позавчера.';en='The address classifier was loaded the day before yesterday.'");
			
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სამისამართო კლასიფიკატორი %1-ის წინ ჩაიტვირთა.';ru='Адресный классификатор был загружен %1 назад.';en='Addressable classifier was loaded ago 1%.'"),
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("ka='შეარჩიეთ რეგიონი';ru='Выберите регион';en='Choose region'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("ka='შეარჩიეთ ოლქი';ru='Выберите округ';en='Select a district'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ რაიონი';ru='Выберите район';en='Select area'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ ქალაქი';ru='Выберите город';en='Select a city'");
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ შიდასაქალაქო რაიონი';ru='Выберите внутригородской район';en='Select the inner city'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ დასახლებული პუნქტი';ru='Выберите населенный пункт';en='Select a town'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ ქუჩა';ru='Выберите улицу';en='Select a street'");
	ИначеЕсли Уровень = 90 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ დამატებითი ელემენტი';ru='Выберите дополнительный элемент';en='Select additional element'");
	ИначеЕсли Уровень = 91 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ დაქვემდებარებული ელემენტი';ru='Выберите подчиненный элемент';en='Select the subordinate element'");
	ИначеЕсли Уровень = -1 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ ორიენტირი';ru='Выберите ориентир';en='Select benchmark'");
	КонецЕсли;
...
Функция ЕстьСписокДомов(ИдентификаторАдресногоОбъекта) Экспорт
	...
	ИначеЕсли Источник = "Сервис1С" Тогда
		...
		Порция = Сервис.Autocomplete(ИдентификаторАдресногоОбъекта, 0, НСтр("ka='ДОМ=';ru='ДОМ=';en='HOUSE ='"), 1, КодЯзыка, Метаданные.Имя);
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Если ОповещатьОПрогрессе Тогда
			ТекстОсталосьЗагрузить = ?(ВсегоСубъектов - НомерПоПорядку > 0, " (" + НСтр("ka='დარჩენილია';ru='осталось';en='left'")
			+ " " + Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=") + ")", "");
			ИнформацияОЗагрузке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка региона ""%1 - %2""';ru='Загрузка региона ""%1 - %2""';en='Download the region ""% 1 -% 2 '''"),
...
"%1 по причине: %2';ka='დროებითი ფაილ %1-ის წაშლა ვერ მოხერხდა შემდეგი მიზეზით: %2'"), ПолноеИмяФайла, КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки
...
Процедура ЗаполнитьВерсиюПоставщикаДанныхВнутр(Результат)
	Результат.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სქბ %1';ru='БСП %1';en='1% BSP'"), СтандартныеПодсистемыСервер.ВерсияБиблиотеки());
КонецПроцедуры
	...
	Иначе
		...
		ИначеЕсли Уровень = 91 Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|";
		Иначе
			ВызватьИсключение НСтр("ka='სამისამართო ობიექტის არაკორექტული დონის მოთხოვნა';ru='Запрос некорректного уровня адресного объекта';en='Query incorrect address object level'");
		КонецЕсли;
...
Процедура ОпределитьИдентификаторыАдресаСервис1С(Результат, АдресДляПроверки)
	...
	Иначе
		ВызватьИсключение НСтр("ka='Сведения о контактной информации были повреждены или некорректно заполнены';ru='Сведения о контактной информации были повреждены или некорректно заполнены';en='Information about the contact details have been damaged or incorrectly filled'");
	КонецЕсли;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ТипЗнч(Адрес) <> Тип("ОбъектXDTO") Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Адрес", НСтр("ka='Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.';ru='Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.';en='Unable to verify the address due to incorrect information. Check the correctness of the address.'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(СубъектРФ.Наименование) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ka='მისამართის სუბიექტი არ არის მითითებული';ru='Не указан субъект РФ адреса';en='Unknown address the subject of the Russian Federation'") );
		Возврат Результат;
	ИначеЕсли Уровни.Найти(1) = Неопределено Тогда
		// В уровнях должно быть указано всегда.
		ВызватьИсключение НСтр("ka='დონეებში მისამართის შესამოწმებლად საქართველოს რაიონის დონე არ არის მითითებული.';ru='В уровнях для проверки адреса не указан уровень субъекта РФ.';en='The levels for the test address is not specified level of RF subjects.'");
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Регион ""%1"" не существует';ru='Регион ""%1"" не существует';en='Region ""%1"" does not exist'"), Адрес.СубъектРФ));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ka='Отсутствуют адресные сведения по региону';ru='Отсутствуют адресные сведения по региону';en='No address information for the region'") + ": " + Адрес.СубъектРФ + ".");
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе';ru='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе';en='RF subject (region) ""% ​​1"" was not found in the address qualifier'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Округ ""%1"" отсутствует в адресном классификаторе';ru='Округ ""%1"" отсутствует в адресном классификаторе';en='District ""%1"" is missing in the address qualifier'"), Округ.Наименование + " " + Округ.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Район ""%1"" отсутствует в адресном классификаторе';ru='Район ""%1"" отсутствует в адресном классификаторе';en='Area ""%1"" is missing in the address qualifier'"), Район.Наименование + " " + Район.Сокращение));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქი ""%1"" სამისამართო კლასიფიკატორში არ არის.';ru='Город ""%1"" отсутствует в адресном классификаторе';en='City ""%1"" is missing in the address qualifier'"), Город.Наименование + " " + Город.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შიდასაქალაქო რაიონი ""%1"" სამისამართო კლასიფიკატორში არ არის';ru='Внутригородской район ""%1"" отсутствует в адресном классификаторе';en='Inner city areas ""%1"" is missing in the address qualifier'"),
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასახლებული პუნქტი ""%1"" სამისამართო კლასიფიკატორში არ არის';ru='Населенный пункт ""%1"" отсутствует в адресном классификаторе';en='Settlement ""%1"" is missing in the address qualifier'"),
		...
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Дополнительная территория ""%1""отсутствует в адресном классификаторе';ru='Дополнительная территория ""%1""отсутствует в адресном классификаторе';en='Additional area ""%1"" is missing in the address qualifier'"),
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе';ru='Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе';en='Element additional territory ""%1"" is missing in the address qualifier'"),
		...
		Если ИсторияОбъекта.Количество() > 0 Тогда 
			Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Улица ""%1"" была переименована в ""%2""';ru='Улица ""%1"" была переименована в ""%2""';en='Street ""%1"" was renamed ""%2""'"), СтрокаОбъект.Значение, СтрокаОбъект.Представление));
			КонецЦикла;
		Иначе
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Улица ""%1"" отсутствует в адресном классификаторе';ru='Улица ""%1"" отсутствует в адресном классификаторе';en='Street ""%1"" is missing in the address qualifier'"), Улица.Наименование + " " + Улица.Сокращение));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			...
			Если НеточныеСовпадения.Количество() = 1 Тогда
				Подсказка = ", " + НСтр("ka='მაგრამ არის';ru='но есть';en='but there'") + """" + НеточныеСовпадения[0]+ """";
			ИначеЕсли НеточныеСовпадения.Количество() > 1 Тогда
				...
				Если НеточныеСовпадения.Количество() = 2 Тогда
					Подсказка = НСтр("ka='მაგრამ არის ""%1"" ან ""%2""';ru='но есть ""%1"" или ""%2""';en='but there is ""%1"" or ""%2""'");
					ВтороеЗначение =  НеточныеСовпадения[1];
				Иначе
					Подсказка = НСтр("ka='მაგრამ არის ""%1"" და სხვა ვარიანტები (%2)';ru='но есть ""%1"" и другие варианты(%2)';en='but there is ""1%"" and other variants (2%)'");
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='""%1"" отсутствует в адресном классификаторе';ru='""%1"" отсутствует в адресном классификаторе';en='""%1"" is absent in address qualifier'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)) + Подсказка);
		КонецЕсли;
	...
	Если НЕ ИндексКорректный И ЗначениеЗаполнено(Индекс) И ОшибкиПроверки.Количество() = 0 Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სამისამართო კლასიფიკატორში ინდექსი ""%1"" არ შეესაბამება შეყვანილ მისამართს';ru='Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу';en='""%1"" in the address index qualifier does not match the entered address'"), Индекс));
	КонецЕсли;
...
Функция СформироватьОписаниеЗдания(НазваниеВладения, НомерВладения, НомерКорпуса, НазваниеСтроения, НомерСтроения, ДополнительныеПараметры = Неопределено)
	...
	Если ЗначениеЗаполнено(НомерКорпуса) Тогда
		КорпусИСтроение = ", "+ НСтр("ka='корпус';ru='корпус';en='housing'") + " " + НомерКорпуса;
	КонецЕсли;
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("2060", НСтр("ka='Этаж';ru='Этаж';en='Floor'"),   3, 6));
	...
	// Уточняющие объекты
	Результат.Добавить(СтрокаОбъектаАдресации("10100000", НСтр("ka='საფოსტო ინდექსი';ru='Почтовый индекс';en='Postcode'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10200000", НСтр("ka='სამისამართო წერტილი';ru='Адресная точка';en='Address point'")));
	...
	Результат.Добавить(СтрокаОбъектаАдресации("10400000", НСтр("ka='საგზაო ქსელის, დამატებითი სამისამართო ელემენტის დაგეგმვითი სტრუქტურის ელემენტი';ru='Элемент улично-дорожной сети, планировочной структуры дополнительного адресного элемента';en='The element of the road network, additional address planning structure element'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10500000", НСтр("ka='სამრეწველო ზონა';ru='Промышленная зона';en='Industrial Zone'")));
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Сервис интернет-поддержки пользователей не подключен.';ru='Сервис интернет-поддержки пользователей не подключен.';en='Service online support is not connected users.'");
	КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		Описание.ПодробноеПредставлениеОшибки = НСтр("ka='Информация об ошибке отсутствует';ru='Информация об ошибке отсутствует';en='error information is not'");
		Возврат Описание;
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ka='სამისამართო კლასიფიკატორის განახლებების ჩატვირთვა.';ru='Загрузка обновлений адресного классификатора.';en='Download updates the address of the classifier.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = НСтр("ka='სამისამართო კლასიფიკატორი მოძველდა';ru='Адресный классификатор устарел';en='The address classifier is outdated'");
			Дело.Количество     = 0;
			Дело.Подсказка      = НСтр("ka='მისამართების ავტოგადარჩევა და კორექტულობის შემოწმება დროებით მიუწვდომელია.';ru='Автоподбор и проверка корректности адресов временно недоступны.';en='Autofit and checking the correctness of addresses temporarily unavailable.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("ka='აუცილებელია განახლებების არსებობის შემოწმება.';ru='Необходимо проверить наличие обновлений.';en='You need to check for updates.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონის შესახებ ინფორმაციის აქტუალიზაცია ""%1 - %2"" (დარჩა %3) ...';ru='Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...';en='Updating of information on the region, ""% 1 -% 2"" (% 3 remaining) ...'"),
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Ошибка поиска типа диапазона для %1';ru='Ошибка поиска типа диапазона для %1';en='range-type search error for% 1'"), Описание);
	КонецЕсли;
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("ka='Респ';ru='Респ';en='rep'") Тогда
		Возврат НСтр("ka='Республика';ru='Республика';en='Republic'") + " " + Наименование;
	ИначеЕсли Сокращение = НСтр("ka='обл';ru='обл';en='region'") Тогда
		Возврат Наименование + " " + НСтр("ka='область';ru='область';en='region'");
	ИначеЕсли Сокращение = НСтр("ka='г';ru='г';en='g'") Тогда
		Возврат НСтр("ka='წ.';ru='г.';en='city'") + " " + Наименование;
	КонецЕсли;
...
Функция ИдентификаторДома(СтрокаСоСпискомДомов, СведенияОДоме)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "h" ИЛИ  Строка = "H" Тогда
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(ОписаниеДома.Дом) ИЛИ ОписаниеДома.Дом = НСтр("ka='არა';ru='НЕТ';en='NOT'") Тогда
...
Функция ИзвлечьСписокДомовСервис1С(ИдентификаторРодитель, СтрокаПоиска, ВариантыДомов)
	...
	Порция = Сервис.Autocomplete(ИдентификаторРодитель, 0, НСтр("ka='ДОМ=';ru='ДОМ=';en='HOUSE ='") + СтрокаПоиска, 2000, КодЯзыка, Метаданные.Имя);
	...
	Для Каждого Строка Из Список Цикл
		...
		Для каждого Дом Из МассивДомов Цикл
			...
			Если ТипАдресногоОбъекта = НСтр("ka='სახლი';ru='ДОМ';en='HOUSE'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ka='ВЛАДЕНИЕ';ru='ВЛАДЕНИЕ';en='OWNERSHIP'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ka='ДОМОВЛАДЕНИЕ';ru='ДОМОВЛАДЕНИЕ';en='DWELLING'") Тогда
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		ТекстПроСтроения = НСтр("ka='корпус';ru='корпус';en='housing'") + " " + ОписаниеДома.Корпус;
...
Функция НаименованиеВладенийИСтроений()
	...
	Результат.Владения.Вставить(0, НСтр("ka='სახლი';ru='Дом';en='House'"));
	Результат.Строения.Вставить(0, НСтр("ka='Строение';ru='Строение';en='Structure'"));
	...
	Для Каждого КодСубъекта Из КодыСубъектовРФ Цикл
		...
		Если Не ЗагруженныйФайл.Статус Тогда
			...
			ВызватьИсключение ЗагруженныйФайл.СообщениеОбОшибке + Символы.ПС
				+ НСтр("ka='შესაძლებელი მიზეზები:"
"• არაკორექტულად არის შეყვანილი ან არ არის შეყვანილი ლოგ-ინი და პაროლი;"
"• ინტერნეტთან შეერთება არ არის;"
"• ვებ-კვანძზე გაუმართაობები წარმოიშვა;"
"• ბრანდმაუერი ან სხვა შუალედური პროგრამული უზრუნველყოფა (ანტივირუსები და მისთ.) ბლოკავს ინტერნეტთან პროგრამის შეერთების მცდელობას;"
"• ინტერნეტთან შეერთება ხორციელდება პროქსი-სერვერით, მაგრამ მისი პარამეტრები პროგრამაში არ არის მიცემული.';ru='Возможные причины:"
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Очистка региона ""%1 - %2"" (осталось %3) ...';ru='Очистка региона ""%1 - %2"" (осталось %3) ...';en='Cleaning region ""% 1 -% 2"" (% 3 remaining) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция РазрешенияБезопасностиОбновления() Экспорт
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ka='Проверка обновления адресного классификатора.';ru='Проверка обновления адресного классификатора.';en='Verifying Update address classifier.'")));
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ka='სამისამართო კლასიფიკატორის მონაცემთა ჩატვირთვა.';ru='Загрузка данных адресного классификатора.';en='Loading data address classifier.'")));
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილნ""%1""-ში ჩანაწერების სია ვერ მოიძებნა ტიპ %2-ში';ru='В файле ""%1"" не найден список записей в типе  %2';en='The file ""%1"" not found a list of entries in the type 2%'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილ ""%1""-ში ძირეული კვანძი %2 ვერ მოიძებნა';ru='В файле ""%1"" не найден корневой узел %2';en='The file ""%1"" not found the root node% 2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
