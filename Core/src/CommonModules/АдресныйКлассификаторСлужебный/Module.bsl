...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("ka='სამისამართო კლასიფიკატორი ჯერ არ ჩატვირთულა.';ru='Адресный классификатор еще не загружался.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("ka='სამისამართო კლასიფიკატორი დღეს ჩაიტვირთა.';ru='Адресный классификатор был загружен сегодня.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = НСтр("ka='სამისამართო კლასიფიკატორი გუშინ ჩაიტვირთა.';ru='Адресный классификатор был загружен вчера.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = НСтр("ka='სამისამართო კლასიფიკატორი გუშინწინ ჩაიტვირთა.';ru='Адресный классификатор был загружен позавчера.'");
			
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სამისამართო კლასიფიკატორი %1-ის წინ ჩაიტვირთა.';ru='Адресный классификатор был загружен %1 назад.'"),
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("ka='შეარჩიეთ რეგიონი';ru='Выберите регион'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("ka='შეარჩიეთ ოლქი';ru='Выберите округ'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ რაიონი';ru='Выберите район'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ ქალაქი';ru='Выберите город'");
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ შიდასაქალაქო რაიონი';ru='Выберите внутригородской район'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ დასახლებული პუნქტი';ru='Выберите населенный пункт'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ ქუჩა';ru='Выберите улицу'");
	ИначеЕсли Уровень = 90 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ დამატებითი ელემენტი';ru='Выберите дополнительный элемент'");
	ИначеЕсли Уровень = 91 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ დაქვემდებარებული ელემენტი';ru='Выберите подчиненный элемент'");
	ИначеЕсли Уровень = -1 Тогда
		Результат.Заголовок = НСтр("ka='აირჩიეთ ორიენტირი';ru='Выберите ориентир'");
	КонецЕсли;
...
Функция ЕстьСписокДомов(ИдентификаторАдресногоОбъекта) Экспорт
	...
	ИначеЕсли Источник = "Сервис1С" Тогда
		...
		Порция = Сервис.Autocomplete(ИдентификаторАдресногоОбъекта, 0, НСтр("ka='ДОМ=';ru='ДОМ='"), 1, КодЯзыка, Метаданные.Имя);
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Если ОповещатьОПрогрессе Тогда
			ТекстОсталосьЗагрузить = ?(ВсегоСубъектов - НомерПоПорядку > 0, " (" + НСтр("ka='დარჩენილია';ru='осталось'")
			+ " " + Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=") + ")", "");
			ИнформацияОЗагрузке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка региона ""%1 - %2""';ru='Загрузка региона ""%1 - %2""'"),
...
Процедура ЗаполнитьВерсиюПоставщикаДанныхВнутр(Результат)
	Результат.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სქბ %1';ru='БСП %1'"), СтандартныеПодсистемыСервер.ВерсияБиблиотеки());
КонецПроцедуры
	...
	Иначе
		...
		ИначеЕсли Уровень = 91 Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|";
		Иначе
			ВызватьИсключение НСтр("ka='სამისამართო ობიექტის არაკორექტული დონის მოთხოვნა';ru='Запрос некорректного уровня адресного объекта'");
		КонецЕсли;
...
Процедура ОпределитьИдентификаторыАдресаСервис1С(Результат, АдресДляПроверки)
	...
	Иначе
		ВызватьИсключение НСтр("ka='Сведения о контактной информации были повреждены или некорректно заполнены';ru='Сведения о контактной информации были повреждены или некорректно заполнены'");
	КонецЕсли;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ТипЗнч(Адрес) <> Тип("ОбъектXDTO") Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Адрес", НСтр("ka='Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.';ru='Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(СубъектРФ.Наименование) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ka='მისამართის სუბიექტი არ არის მითითებული';ru='Не указан субъект РФ адреса'") );
		Возврат Результат;
	ИначеЕсли Уровни.Найти(1) = Неопределено Тогда
		// В уровнях должно быть указано всегда.
		ВызватьИсключение НСтр("ka='დონეებში მისამართის შესამოწმებლად საქართველოს რაიონის დონე არ არის მითითებული.';ru='В уровнях для проверки адреса не указан уровень субъекта РФ.'");
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Регион ""%1"" не существует';ru='Регион ""%1"" не существует'"), Адрес.СубъектРФ));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ka='Отсутствуют адресные сведения по региону';ru='Отсутствуют адресные сведения по региону'") + ": " + Адрес.СубъектРФ + ".");
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе';ru='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Округ ""%1"" отсутствует в адресном классификаторе';ru='Округ ""%1"" отсутствует в адресном классификаторе'"), Округ.Наименование + " " + Округ.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Район ""%1"" отсутствует в адресном классификаторе';ru='Район ""%1"" отсутствует в адресном классификаторе'"), Район.Наименование + " " + Район.Сокращение));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქი ""%1"" სამისამართო კლასიფიკატორში არ არის.';ru='Город ""%1"" отсутствует в адресном классификаторе'"), Город.Наименование + " " + Город.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შიდასაქალაქო რაიონი ""%1"" სამისამართო კლასიფიკატორში არ არის';ru='Внутригородской район ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასახლებული პუნქტი ""%1"" სამისამართო კლასიფიკატორში არ არის';ru='Населенный пункт ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Дополнительная территория ""%1""отсутствует в адресном классификаторе';ru='Дополнительная территория ""%1""отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе';ru='Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ИсторияОбъекта.Количество() > 0 Тогда 
			Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Улица ""%1"" была переименована в ""%2""';ru='Улица ""%1"" была переименована в ""%2""'"), СтрокаОбъект.Значение, СтрокаОбъект.Представление));
			КонецЦикла;
		Иначе
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Улица ""%1"" отсутствует в адресном классификаторе';ru='Улица ""%1"" отсутствует в адресном классификаторе'"), Улица.Наименование + " " + Улица.Сокращение));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			...
			Если НеточныеСовпадения.Количество() = 1 Тогда
				Подсказка = ", " + НСтр("ka='მაგრამ არის';ru='но есть'") + """" + НеточныеСовпадения[0]+ """";
			ИначеЕсли НеточныеСовпадения.Количество() > 1 Тогда
				...
				Если НеточныеСовпадения.Количество() = 2 Тогда
					Подсказка = НСтр("ka='მაგრამ არის ""%1"" ან ""%2""';ru='но есть ""%1"" или ""%2""'");
					ВтороеЗначение =  НеточныеСовпадения[1];
				Иначе
					Подсказка = НСтр("ka='მაგრამ არის ""%1"" და სხვა ვარიანტები (%2)';ru='но есть ""%1"" и другие варианты(%2)'");
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='""%1"" отсутствует в адресном классификаторе';ru='""%1"" отсутствует в адресном классификаторе'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)) + Подсказка);
		КонецЕсли;
	...
	Если НЕ ИндексКорректный И ЗначениеЗаполнено(Индекс) И ОшибкиПроверки.Количество() = 0 Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სამისამართო კლასიფიკატორში ინდექსი ""%1"" არ შეესაბამება შეყვანილ მისამართს';ru='Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу'"), Индекс));
	КонецЕсли;
...
Функция СформироватьОписаниеЗдания(НазваниеВладения, НомерВладения, НомерКорпуса, НазваниеСтроения, НомерСтроения, ДополнительныеПараметры = Неопределено)
	...
	Если ЗначениеЗаполнено(НомерКорпуса) Тогда
		КорпусИСтроение = ", "+ НСтр("ka='корпус';ru='корпус'") + " " + НомерКорпуса;
	КонецЕсли;
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("1010", НСтр("ka='სახლი';ru='Дом'"),          1, 1, 2));
	Результат.Добавить(СтрокаОбъектаАдресации("1020", НСтр("ka='მფლობელობა';ru='Владение'"),     1, 2, 1));
	Результат.Добавить(СтрокаОбъектаАдресации("1030", НСтр("ka='სახლის მფლობელობა';ru='Домовладение'"), 1, 3, 3));
	
	Результат.Добавить(СтрокаОбъектаАдресации("1050", НСтр("ka='კორპუსი';ru='Корпус'"),     2, 1));
	Результат.Добавить(СтрокаОбъектаАдресации("1060", НСтр("ka='Строение';ru='Строение'"),   2, 2, 1));
	Результат.Добавить(СтрокаОбъектаАдресации("1080", НСтр("ka='ლიტერა';ru='Литера'"),     2, 3, 3));
	Результат.Добавить(СтрокаОбъектаАдресации("1070", НСтр("ka='შენობა';ru='Сооружение'"), 2, 4, 2));
	Результат.Добавить(СтрокаОбъектаАдресации("1040", НСтр("ka='უბანი';ru='Участок'"),    2, 5));
	
	Результат.Добавить(СтрокаОбъектаАдресации("2010", НСтр("ka='ბინა';ru='Квартира'"),  3, 1));
	Результат.Добавить(СтрокаОбъектаАдресации("2030", НСтр("ka='Офис';ru='Офис'"),      3, 2));
	Результат.Добавить(СтрокаОбъектаАдресации("2040", НСтр("ka='Бокс';ru='Бокс'"),      3, 3));
	Результат.Добавить(СтрокаОбъектаАдресации("2020", НСтр("ka='სათავსო';ru='Помещение'"), 3, 4));
	Результат.Добавить(СтрокаОбъектаАдресации("2050", НСтр("ka='ოთახი';ru='Комната'"),   3, 5));
	Результат.Добавить(СтрокаОбъектаАдресации("2060", НСтр("ka='Этаж';ru='Этаж'"),   3, 6));
	//  Наши сокращения для поддержки обратной совместимости при парсинге.
	Результат.Добавить(СтрокаОбъектаАдресации("2010", НСтр("ka='кв.';ru='кв.'"),       3, 6));
	Результат.Добавить(СтрокаОбъектаАдресации("2030", НСтр("ka='оф.';ru='оф.'"),       3, 7));
	
	// Уточняющие объекты
	Результат.Добавить(СтрокаОбъектаАдресации("10100000", НСтр("ka='საფოსტო ინდექსი';ru='Почтовый индекс'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10200000", НСтр("ka='სამისამართო წერტილი';ru='Адресная точка'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10300000", НСтр("ka='მებაღეთა ამხანაგობა';ru='Садовое товарищество'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10400000", НСтр("ka='საგზაო ქსელის, დამატებითი სამისამართო ელემენტის დაგეგმვითი სტრუქტურის ელემენტი';ru='Элемент улично-дорожной сети, планировочной структуры дополнительного адресного элемента'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10500000", НСтр("ka='სამრეწველო ზონა';ru='Промышленная зона'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10600000", НСтр("ka='გარაჟების სამშენებლო კოოპერატივი';ru='Гаражно-строительный кооператив'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10700000", НСтр("ka='ტერიტორია';ru='Территория'")));
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Сервис интернет-поддержки пользователей не подключен.';ru='Сервис интернет-поддержки пользователей не подключен.'");
	КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		Описание.ПодробноеПредставлениеОшибки = НСтр("ka='Информация об ошибке отсутствует';ru='Информация об ошибке отсутствует'");
		Возврат Описание;
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ka='სამისამართო კლასიფიკატორის განახლებების ჩატვირთვა.';ru='Загрузка обновлений адресного классификатора.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = НСтр("ka='სამისამართო კლასიფიკატორი მოძველდა';ru='Адресный классификатор устарел'");
			Дело.Количество     = 0;
			Дело.Подсказка      = НСтр("ka='მისამართების ავტოგადარჩევა და კორექტულობის შემოწმება დროებით მიუწვდომელია.';ru='Автоподбор и проверка корректности адресов временно недоступны.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("ka='აუცილებელია განახლებების არსებობის შემოწმება.';ru='Необходимо проверить наличие обновлений.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონის შესახებ ინფორმაციის აქტუალიზაცია ""%1 - %2"" (დარჩა %3) ...';ru='Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...'"),
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Ошибка поиска типа диапазона для %1';ru='Ошибка поиска типа диапазона для %1'"), Описание);
	КонецЕсли;
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("ka='Респ';ru='Респ'") Тогда
		Возврат НСтр("ka='Республика';ru='Республика'") + " " + Наименование;
	ИначеЕсли Сокращение = НСтр("ka='обл';ru='обл'") Тогда
		Возврат Наименование + " " + НСтр("ka='область';ru='область'");
	ИначеЕсли Сокращение = НСтр("ka='г';ru='г'") Тогда
		Возврат НСтр("ka='წ.';ru='г.'") + " " + Наименование;
	КонецЕсли;
...
Функция ИдентификаторДома(СтрокаСоСпискомДомов, СведенияОДоме)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "h" ИЛИ  Строка = "H" Тогда
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(ОписаниеДома.Дом) ИЛИ ОписаниеДома.Дом = НСтр("ka='არა';ru='НЕТ'") Тогда
...
Функция ИзвлечьСписокДомовСервис1С(ИдентификаторРодитель, СтрокаПоиска, ВариантыДомов)
	...
	Порция = Сервис.Autocomplete(ИдентификаторРодитель, 0, НСтр("ka='ДОМ=';ru='ДОМ='") + СтрокаПоиска, 2000, КодЯзыка, Метаданные.Имя);
	...
	Для Каждого Строка Из Список Цикл
		...
		Для каждого Дом Из МассивДомов Цикл
			...
			Если ТипАдресногоОбъекта = НСтр("ka='სახლი';ru='ДОМ'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ka='ВЛАДЕНИЕ';ru='ВЛАДЕНИЕ'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ka='ДОМОВЛАДЕНИЕ';ru='ДОМОВЛАДЕНИЕ'") Тогда
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		ТекстПроСтроения = НСтр("ka='корпус';ru='корпус'") + " " + ОписаниеДома.Корпус;
...
Функция НаименованиеВладенийИСтроений()
	...
	Результат.Владения.Вставить(0, НСтр("ka='სახლი';ru='Дом'"));
	Результат.Строения.Вставить(0, НСтр("ka='Строение';ru='Строение'"));
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Очистка региона ""%1 - %2"" (осталось %3) ...';ru='Очистка региона ""%1 - %2"" (осталось %3) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция РазрешенияБезопасностиОбновления() Экспорт
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ka='Проверка обновления адресного классификатора.';ru='Проверка обновления адресного классификатора.'")));
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ka='სამისამართო კლასიფიკატორის მონაცემთა ჩატვირთვა.';ru='Загрузка данных адресного классификатора.'")));
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილნ""%1""-ში ჩანაწერების სია ვერ მოიძებნა ტიპ %2-ში';ru='В файле ""%1"" не найден список записей в типе  %2'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილ ""%1""-ში ძირეული კვანძი %2 ვერ მოიძებნა';ru='В файле ""%1"" не найден корневой узел %2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
