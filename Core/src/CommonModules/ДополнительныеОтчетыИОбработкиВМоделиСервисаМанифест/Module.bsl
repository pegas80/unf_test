...
Функция СформироватьМанифест(Знач ОбъектОбработки, Знач ОбъектВерсии, Знач ВариантыОтчета = Неопределено, Знач РасписанияКоманд = Неопределено, Знач РазрешенияОбработки = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების ტიპს %1 სერვისის მოდელში მხარდაჭერა არ გააჩნია!';ru='Вид дополнительных отчетов и обработок %1 не поддерживается в модели сервиса!'"),
			ОбъектВерсии.Вид);
	КонецЕсли;
	...
	Если ОбъектВерсии.Команды.Количество() > 0 Тогда
		...
		Для Каждого ОписаниеКоманды Из ОбъектВерсии.Команды Цикл
			...
			Иначе
				ВызватьИсключение СтрШаблон(НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების გაშვების მეთოდი %1 სერვისის მოდელში არ არის მხარდაჭერილი!';ru='Способ запуска дополнительных отчетов и обработок %1 не поддерживается в модели сервиса!'"),
