		...
		Если НЕ ВерсияСсылка.Том.Пустая() Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ფაილები. ფაილის გახსნა';ru='Файлы.Открытие файла';en='Fayly.Otkrytie file'",
...
Функция СоздатьФайл(Знач Владелец, Знач СведенияОФайле)
	...
	ИначеЕсли Не ПустаяСтрока(СведенияОФайле.АдресВременногоХранилищаФайла) Тогда 
		//ПолучитьИзВременногоХранилища(СведенияОФайле.АдресВременногоХранилищаФайла)  
		Файл.ТекстХранилище = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(СведенияОФайле.АдресВременногоХранилищаФайла));
	КонецЕсли;
...
Функция СохранитьИзмененияИОсвободитьФайл(ДанныеФайла, СведенияОФайле,
	...
	Если Не ДанныеФайлаТекущие.РедактируетТекущийПользователь Тогда
		ВызватьИсключение НСтр("ka='ფაილი არ არის დაკავებული მიმდინარე მომხმარებლის მიერ';ru='Файл не занят текущим пользователем';en='The file is not occupied by the current user'");
	КонецЕсли;
...
Функция ИмпортФайловСформироватьОтчет(МассивИменФайловСОшибками) Экспорт
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("ka='შემდეგი ფაილების ჩატვირთვა ვერ მოხერხდა:';ru='Не удалось загрузить следующие файлы:';en='Failed to download the following files:'");
