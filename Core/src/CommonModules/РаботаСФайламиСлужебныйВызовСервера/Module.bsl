		...
		Если НЕ ВерсияСсылка.Том.Пустая() Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ფაილები. ფაილის გახსნა';ru='Файлы.Открытие файла'",
...
Функция СохранитьИзмененияИОсвободитьФайл(ДанныеФайла, СведенияОФайле,
	...
	Если Не ДанныеФайлаТекущие.РедактируетТекущийПользователь Тогда
		ВызватьИсключение НСтр("ka='ფაილი არ არის დაკავებული მიმდინარე მომხმარებლის მიერ';ru='Файл не занят текущим пользователем'");
	КонецЕсли;
...
Функция ИмпортФайловСформироватьОтчет(МассивИменФайловСОшибками) Экспорт
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("ka='შემდეგი ფაილების ჩატვირთვა ვერ მოხერხდა:';ru='Не удалось загрузить следующие файлы:'");
