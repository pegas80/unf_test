...
Процедура ПроверитьДокументыПроведены(ДокументыМассив, ДокументыПроведены, ФормаИсточник = Неопределено) Экспорт
	...
	Если КоличествоНепроведенныхДокументов > 0 Тогда
		...
		Если КоличествоНепроведенныхДокументов = 1 Тогда
			ТекстВопроса = НСтр("ka='დოკუმენტის ელექტრონული ვერსიის შესადგენად, აუცილებელია მისი წინასწარ გატარება. შევასრულოთ დოკუმენტის გატარება და გავაგრძელოთ?';ru='Для того чтобы сформировать электронную версию документа, его необходимо предварительно провести. Выполнить проведение документа и продолжить?';en='In order to create an electronic version of the document, it must be performed first. Run holding a document and continue?'");
		Иначе
			ТекстВопроса = НСтр("ka='დოკუმენტების ელექტრონული ვერსიების შესადგენად, საჭიროა მათი წინასწარ გატარება. შევასრულოთ დოკუმენტების გატარება და გავაგრძელოთ?';ru='Для того чтобы сформировать электронные версии документов, их необходимо предварительно провести. Выполнить проведение документов и продолжить?';en='In order to generate electronic versions of the documents, they must be performed first. Run holding documents and proceed?'");
		КонецЕсли;
		...
		ШаблонСообщения = НСтр("ka='დოკუმენტი %1 არ არის გატარებული: %2 ედ-ის შედგენა შეუძლებელია.';ru='Документ %1 не проведен: %2 Формирование ЭД невозможно.';en='1% Document not held% 2 Formation ED impossible.'");
