...
Процедура ПроверитьДокументыПроведены(ДокументыМассив, ДокументыПроведены, ФормаИсточник = Неопределено) Экспорт
	...
	Если КоличествоНепроведенныхДокументов > 0 Тогда
		...
		Если КоличествоНепроведенныхДокументов = 1 Тогда
			ТекстВопроса = НСтр("ka='დოკუმენტის ელექტრონული ვერსიის შესადგენად, აუცილებელია მისი წინასწარ გატარება. შევასრულოთ დოკუმენტის გატარება და გავაგრძელოთ?';ru='Для того чтобы сформировать электронную версию документа, его необходимо предварительно провести. Выполнить проведение документа и продолжить?'");
		Иначе
			ТекстВопроса = НСтр("ka='დოკუმენტების ელექტრონული ვერსიების შესადგენად, საჭიროა მათი წინასწარ გატარება. შევასრულოთ დოკუმენტების გატარება და გავაგრძელოთ?';ru='Для того чтобы сформировать электронные версии документов, их необходимо предварительно провести. Выполнить проведение документов и продолжить?'");
		КонецЕсли;
		...
		ШаблонСообщения = НСтр("ka='დოკუმენტი %1 არ არის გატარებული: %2 ედ-ის შედგენა შეუძლებელია.';ru='Документ %1 не проведен: %2 Формирование ЭД невозможно.'");
