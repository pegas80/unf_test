...
Функция СобытиеЖурналаРегистрации(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ka='Обновление информации о направлениях сдачи отчетности';ru='Обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებობის გაგზავნა';ru='Отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ka='Получение результатов отправки отчетности';ru='Получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюНачалаРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ka='Начато обновление информации о направлениях сдачи отчетности';ru='Начато обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ka='Начата отправка отчетности';ru='Начата отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ka='Начато получение результатов отправки отчетности';ru='Начато получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюОкончанияРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებულობის ჩაბარების მიმართულებების შესახებ ინფორმაციის განახლება დასრულებულია';ru='Завершено обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებულობის გაგზავნა დასრულებულია';ru='Завершена отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებულობის გაგზავნის შედეგების მიღება დასრულებულია';ru='Завершено получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция СсылкаПоСтрокеСтатуса(СтрокаСтатуса, ОписаниеОшибки = "")
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერის პასუხის მიხედვით ანგარიშგებულობის ჩაბარების სტატუსის დადგენა ვერ მოხერხდა (%1)';ru='Не удалось определить статус сдачи отчетности по ответу сервера (%1)'"), ТекстИзФайла);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПроверитьПередРегистрациейЗаявкиНаОтправкуОтчетности(Отчет, КонтролирующийОрган = "ФНС", ТекстВопроса = "") Экспорт
	...
	Если НЕ НаправлениеСдачиОтчетностиОткрыто(Организация, ТипНаправления, КодНаправления) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ორგანიზაციის თაობაზე ანგარიშგებულობის ჩაბარების მიმართულება დახურულია. გაგზავნა შეუძლებელია.';ru='Направление сдачи отчетности по организации закрыто. Отправка невозможна.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗарегистрироватьОтправкуОтчетностиВСправочнике(ДанныеПакета, ИмяФайлаПакета, Отчет) Экспорт
	...
	Если ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФНС Тогда
		...
		ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНаправления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код инспекции-получателя отчета должен состоять строго из четырех цифр.';ru='Код инспекции-получателя отчета должен состоять строго из четырех цифр.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФСГС Тогда
		...
		ИЛИ Сред(КодНаправления, 3, 1) <> "-" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XX, где X - любая цифра.';ru='Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XX, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	Иначе
	...
	Если МетаданныеОтчета = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Неизвестный вид отчета.';ru='Неизвестный вид отчета.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВидОтчета) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ცნობარ ""რეგლამენტირებული ანგარიშგებები""-ში %1 ტიპის ანგარიშგების განსაზღვრება ვერ მოიძებნა.';ru='В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.'"), ИсточникОтчета));
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьСведенияИзУправляющегоПриложения()
	...
	Если НЕ ЗначениеЗаполнено(ВебАдрес) Тогда
		ЗаписьЖурналаРегистрации(СобытиеЖурнала,
								...
								Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности,
								,
								НСтр("ka='Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.';ru='Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция СформироватьФайлПакета(
	...
	Для Инд = 0 По КороткиеИменаФайловВыгрузки.Количество() - 1 Цикл
		...
		Иначе
			Если НЕ ЗаписатьТекстВФайл(ЭлТекстВыгрузки, ПолноеИмяФайлаВыгрузки, ЭлКодировкаТекстаВыгрузки) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.';ru='Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если КонтролирующийОрган = "ФНС" Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.';ru='Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ИзменитьФайлВыгрузкиПриОтправкеЧерезУполномоченногоПредставителя(ТекстВыгрузки, ИмяФайлаВыгрузки, Доверенность, КодНО, ДатаПериода = '00010101')
	...
	Если нрег(РасширениеФайла) = ".xml" Тогда // формат xml - меняем ИдФайл и имя файла
		...
		Если КомпонентыИмени.Количество() < 5 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.';ru='Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.'"));
			Возврат Ложь;
		КонецЕсли;
