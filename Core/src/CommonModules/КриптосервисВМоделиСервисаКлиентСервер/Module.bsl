...
Функция ЗамаскироватьНомерТелефона(Знач НомерТелефона) Экспорт
	...
	Если МассивСимволов.Количество() = 12 Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение(НСтр("ka='ტელეფონის ნომრის გარდაქმნის შეცდომა.';ru='Ошибка преобразования номера телефона.'"));
	КонецЕсли;
...
Функция НомерТелефона(Знач НомерТелефона) Экспорт 
	...
	Иначе
		ВызватьИсключение(НСтр("ka='ფორმატ +79999999999-ში ტელეფონის ნომრის გარდაქმნის შეცდომა.';ru='Ошибка преобразования номера телефона к формату +79999999999.'"));
	КонецЕсли;
