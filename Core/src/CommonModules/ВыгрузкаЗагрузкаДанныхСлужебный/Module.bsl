...
Процедура ЗагрузитьДанныеИзКаталога(Знач КаталогЗагрузки, Знач ПараметрыЗагрузки) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.2.19.0") < 0
		...
		ВызватьИсключение
			НСтр("ka='მონაცემთა ჩატვირთვის შესასრულებლად საჭიროა ტექნოლოგიური პლატფორმა ""1С:საწარმო""-ს განახლება."
"ვერსია 8.2-ისთვის აუცილებელია რელიზ 8.2.19-ის (ან უფრო ახლის) გამოყენება."
"ვერსია 8.3-ისთვის აუცილებელია რელიზ 8.3.4-ის (ან უფრო ახლის) გამოყენება.';ru='Для выполнения загрузки данных требуется обновить технологическую платформу ""1С:Предприятие""."
...
"Для версии 8.3 необходимо использовать релиз 8.3.4 (или более новый).'");
	...
	Если Не ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(ИнформацияОВыгрузке) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='ფაილიდან მონაცემთა ჩატვირთვა შეუძლებელია, ვინაიდან ფაილი გადმოიტვირთა სხვა კონფიგურაციიდან (ფაილი გადმოიტვირთა კონფიგურაცია %1-იდან და არ შეიძლება, რომ კონფიგურაცია %2-ში ჩაიტვირთოს)';ru='Невозможно загрузить данные из файла, т.к. файл был выгружен из другой конфигурации (файл выгружен из конфигурации %1 и не может быть загружен в конфигурацию %2)';en='Unable to load data from a file, as file has been unloaded from the other configuration (configuration file unloaded from 1% and can not be loaded in the configuration of 2%)'"),
	...
	Если Не ВыгрузкаВАрхивеСовместимаСТекущейВерсиейКонфигурации(ИнформацияОВыгрузке) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='ფაილიდან მონაცემთა ჩატვირთვა შეუძლებელია, ვინაიდან ფაილი კონფიგურაციის სხვა ვერსიიდან გადმოიტვირთა (ფაილი გადმოიტვირთა ვერსია %1-ის კონფიგურაციიდან და არ შეიძლება, რომ ვერსია %2-ის კონფიგურაციაში ჩაიტვირთოს)';ru='Невозможно загрузить данные из файла, т.к. файл был выгружен из другой версии конфигурации (файл выгружен из конфигурации версии %1 и не может быть загружен в конфигурацию версии %2)';en='Unable to load data from a file, as file has been unloaded from the other versions of configuration (configuration file unloaded from version 1% and can not be loaded into the configuration version% 2)'"),
...
Функция XMLТипСсылки(Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='შეცდომა ობიექტ %1-ისთვის ბმულის XMLტიპის დადგენისას: ობიექტი არ არის მიმთითებელი!';ru='Ошибка при определении XMLТипа ссылки для объекта %1: объект не является ссылочным!';en='Error in determining XMLTipa reference object% 1: The object is not a reference!'"),
