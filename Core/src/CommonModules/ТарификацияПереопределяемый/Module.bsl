...
Процедура ВыполнитьАктивациюЛицензииКоличествоКасс(ИмяЛицензии, ЛицензияАктивирована) Экспорт
	...
	Если Не ЗначениеЗаполнено(КассовыйАппарат) Тогда
		...
		Если Не Отмена Тогда
			Сообщение = СтрШаблон(НСтр("ka='Не освобождена лицензия для ККМ с регистрационным номером: %1';ru='Не освобождена лицензия для ККМ с регистрационным номером: %1'", КодЯзыка),
				ИмяЛицензии);
			ЗаписьЖурналаРегистрации(НСтр("ka='ლიცენზიის დეაქტივიზირება';ru='Деактивация лицензии.'",КодЯзыка),
...
Функция ОграничениеКоличествоКассПоТарифу() Экспорт
	...
	ТарифноеОграничение.Вставить("Наименование", НСтр("ka='Количество подключенных ККМ (по рег. номеру)';ru='Количество подключенных ККМ (по рег. номеру)'",
...
Функция СсылкаНаЛичныйКабинетАбонента() Экспорт
	...
	СсылкаНаЛичныйКабинет = ИнформационныйЦентрСервер.КонтекстнаяСсылкаПоИдентификатору(НСтр("ka='ЛичныйКабинет';ru='ЛичныйКабинет'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ПроверкаИспользованияЛицензииКоличествоКасс(ИдентификаторЛицензии, ТарифноеОграничение = Неопределено)
	...
	ИмяСобытия = Тарификация.ИмяСобытияЖР() + НСтр("ka='Проверка наличия лицензии уникальной услуги из приложения';ru='Проверка наличия лицензии уникальной услуги из приложения'");
