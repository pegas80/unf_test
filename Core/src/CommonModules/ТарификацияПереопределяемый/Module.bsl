...
Процедура ВыполнитьАктивациюЛицензииКоличествоКасс(ИмяЛицензии, ЛицензияАктивирована) Экспорт
	...
	Если Не ЗначениеЗаполнено(КассовыйАппарат) Тогда
		...
		Если Не Отмена Тогда
			Сообщение = СтрШаблон(НСтр("ka='Не освобождена лицензия для ККМ с регистрационным номером: %1';ru='Не освобождена лицензия для ККМ с регистрационным номером: %1';en='Not available for license with CMC registration number:% 1'", КодЯзыка),
				ИмяЛицензии);
			ЗаписьЖурналаРегистрации(НСтр("ka='ლიცენზიის დეაქტივიზირება';ru='Деактивация лицензии.';en='Deactivating a license.'",КодЯзыка),
...
Функция ОграничениеКоличествоКассПоТарифу() Экспорт
	...
	ТарифноеОграничение.Вставить("Наименование", НСтр("ka='Количество подключенных ККМ (по рег. номеру)';ru='Количество подключенных ККМ (по рег. номеру)';en='The number of connected CMC (according reg. Number)'",
...
Функция СсылкаНаЛичныйКабинетАбонента() Экспорт
	...
	СсылкаНаЛичныйКабинет = ИнформационныйЦентрСервер.КонтекстнаяСсылкаПоИдентификатору(НСтр("ka='ЛичныйКабинет';ru='ЛичныйКабинет';en='Personal Area'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ПроверкаИспользованияЛицензииКоличествоКасс(ИдентификаторЛицензии, ТарифноеОграничение = Неопределено)
	...
	ИмяСобытия = Тарификация.ИмяСобытияЖР() + НСтр("ka='Проверка наличия лицензии уникальной услуги из приложения';ru='Проверка наличия лицензии уникальной услуги из приложения';en='Check for unique service license from the app'");
