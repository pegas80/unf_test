...
Процедура ЗапланироватьОбновлениеОбластейДанных(Знач БлокироватьОбласти = Истина, Знач СообщениеБлокировки = "") Экспорт
	...
	Если ПустаяСтрока(СообщениеБлокировки) Тогда
		...
		Если ПустаяСтрока(СообщениеБлокировки) Тогда
			СообщениеБлокировки = НСтр("ka='განახლების შესასრულებლად სისტემა დაბლოკილია.';ru='Система заблокирована для выполнения обновления.';en='The system is locked to perform the update.'");
		КонецЕсли;
	КонецЕсли;
		...
		Если ИдентификаторБиблиотеки = Метаданные.Имя Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НЕ Выборка.Обновлена Тогда
					...
					ШаблонКомментария = НСтр("ka='კონფიგურაციის ვერსიის განახლება შესრულდა ბიბლიოთეკის ვერსიის განახლებამდე %1';ru='Обновление версии конфигурации было выполнено до обновления версии библиотеки %1';en='Updating the configuration revision was carried out to upgrade the library% 1'");
				...
				Если ОписаниеПланаОбновленияБиблиотеки = Неопределено Тогда
					...
					ШаблонКомментария = НСтр("ka='ბიბლიოთეკა %1-ის განახლების გეგმა ვერ მოიძებნა';ru='Не найден план обновления библиотеки %1';en='Not found renovation plan of the library% 1'");
					ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, Выборка.Имя);
					ЗаписьЖурналаРегистрации(
						ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
						УровеньЖурналаРегистрации.Ошибка,
...
"Требуется план обновления на версию %2, найден план для обновления на версию %3';ka='აღმოჩენილია ბიბლიოთეკა %1-ის განახლების არაკორექტული გეგმა"
"საჭიროა ვერსია %2-ით განახლების გეგმა, მოიძებნა ვერსია %3-ით განახლების გეგმა'");
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если ОбновлениеИнформационнойБазыСлужебный.НеобходимоВыполнитьОбновление(Метаданные.Версия, ВерсияОбщихДанных) Тогда
			Сообщение = НСтр("ka='საინფორმაციო ბაზის განახლების საერთო ნაწილი არ შესრულებულა. მიმართეთ ადმინისტრატორს.';ru='Не выполнена общая часть обновления информационной базы."
	...
	Если КлючЗаписи <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ka='Обновление области данных';ru='Обновление области данных';en='Updating the data area'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение(НСтр("ka='Ошибка обновления области данных. Запись версий области данных заблокирована.';ru='Ошибка обновления области данных. Запись версий области данных заблокирована.';en='Update failed data area. Record version of the data area is locked.'"));
		КонецПопытки;
	КонецЕсли;
	Возврат КлючЗаписи;
	...
	Иначе
		...
		Если Не МонопольныйРежим() Тогда
			...
			Исключение
				...
				СтрокаСообщения = НСтр("ka='მონოპოლიური რეჟიმის დაყენება ვერ მოხერხდა. შეცდომის აღწერილობა: %1';ru='Не удалось установить монопольный режим. Описание ошибки: %1';en='Unable to establish a monopoly regime. Description of error:% 1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
