...
Процедура ЗапланироватьОбновлениеОбластейДанных(Знач БлокироватьОбласти = Истина, Знач СообщениеБлокировки = "") Экспорт
	...
	Если ПустаяСтрока(СообщениеБлокировки) Тогда
		...
		Если ПустаяСтрока(СообщениеБлокировки) Тогда
			СообщениеБлокировки = НСтр("ka='განახლების შესასრულებლად სისტემა დაბლოკილია.';ru='Система заблокирована для выполнения обновления.'");
		КонецЕсли;
	КонецЕсли;
		...
		Если ИдентификаторБиблиотеки = Метаданные.Имя Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НЕ Выборка.Обновлена Тогда
					...
					ШаблонКомментария = НСтр("ka='კონფიგურაციის ვერსიის განახლება შესრულდა ბიბლიოთეკის ვერსიის განახლებამდე %1';ru='Обновление версии конфигурации было выполнено до обновления версии библиотеки %1'");
				...
				Если ОписаниеПланаОбновленияБиблиотеки = Неопределено Тогда
					...
					ШаблонКомментария = НСтр("ka='ბიბლიოთეკა %1-ის განახლების გეგმა ვერ მოიძებნა';ru='Не найден план обновления библиотеки %1'");
					ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, Выборка.Имя);
					ЗаписьЖурналаРегистрации(
						ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
						УровеньЖурналаРегистрации.Ошибка,
	...
	Если КлючЗаписи <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ka='Обновление области данных';ru='Обновление области данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение(НСтр("ka='Ошибка обновления области данных. Запись версий области данных заблокирована.';ru='Ошибка обновления области данных. Запись версий области данных заблокирована.'"));
		КонецПопытки;
	КонецЕсли;
	Возврат КлючЗаписи;
	...
	Иначе
		...
		Если Не МонопольныйРежим() Тогда
			...
			Исключение
				...
				СтрокаСообщения = НСтр("ka='მონოპოლიური რეჟიმის დაყენება ვერ მოხერხდა. შეცდომის აღწერილობა: %1';ru='Не удалось установить монопольный режим. Описание ошибки: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
