...
Функция ПредставлениеОперацииПолученияНастроек()
	...
	Возврат НСтр("ka='მონაცემთა სინქრონიზაციის გამართვების მიღება სერვისის მენეჯერიდან.';ru='Получение настроек синхронизации данных из Менеджера сервиса.';en='Receiving settings synchronization from the Manager service.'");
	
КонецФункции

Функция ПредставлениеВключениеСинхронизации()
	
	Возврат НСтр("ka='სერვისის მენეჯერში მონაცემთა სინქრონიზაციის ჩართვა.';ru='Включение синхронизации данных в Менеджере сервиса.';en='Inclusion of the synchronization data in the service manager.'");
	
КонецФункции

Функция ПредставлениеОтключениеСинхронизации()
	
	Возврат НСтр("ka='სერვისის მენეჯერში მონაცემთა სინქრონიზაციის გამორთვა.';ru='Отключение синхронизации данных в Менеджере сервиса.';en='Disabling the data synchronization manager service.'");
	
КонецФункции

Функция ПредставлениеВыполнениеСинхронизации()
	
	Возврат НСтр("ka='მონაცემთა სინქრონიზაციის შესრულება მომხმარებლის მოთხოვნით.';ru='Выполнение синхронизации данных по запросу пользователя.';en='Performing data synchronization upon user request.'");
