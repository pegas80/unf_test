...
Функция ПредставлениеОперацииПолученияНастроек()
	...
	Возврат НСтр("ka='მონაცემთა სინქრონიზაციის გამართვების მიღება სერვისის მენეჯერიდან.';ru='Получение настроек синхронизации данных из Менеджера сервиса.'");
	
КонецФункции

Функция ПредставлениеВключениеСинхронизации()
	
	Возврат НСтр("ka='სერვისის მენეჯერში მონაცემთა სინქრონიზაციის ჩართვა.';ru='Включение синхронизации данных в Менеджере сервиса.'");
	
КонецФункции

Функция ПредставлениеОтключениеСинхронизации()
	
	Возврат НСтр("ka='სერვისის მენეჯერში მონაცემთა სინქრონიზაციის გამორთვა.';ru='Отключение синхронизации данных в Менеджере сервиса.'");
	
КонецФункции

Функция ПредставлениеВыполнениеСинхронизации()
	
	Возврат НСтр("ka='მონაცემთა სინქრონიზაციის შესრულება მომხმარებლის მოთხოვნით.';ru='Выполнение синхронизации данных по запросу пользователя.'");
