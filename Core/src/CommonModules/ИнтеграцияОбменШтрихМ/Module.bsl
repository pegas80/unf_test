...
Функция ЗагрузитьДанныеОбменаИз1СКасса(ИмяФайла) Экспорт
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ЗаписьЖурналаРегистрации(
			...
			НСтр("ka='Ошибка чтения сообщения загрузки. Неверный формат сообщения.';ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.'"));
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
			...
			"Загрузка1СКасса.Загрузка1СКасса",
			УровеньЖурналаРегистрации.Ошибка,,, 
			 НСтр("ka='В ходе выполнения операции возникли ошибки';ru='В ходе выполнения операции возникли ошибки'") + ": " + Символы.ПС + КомпонентыОбмена.СтрокаСообщенияОбОшибке);
		Возврат Ложь;
	КонецЕсли;
...
Функция ОтправитьПисьмоОНовомАбонентеИз1СКасса() Экспорт
	...
	Попытка
		...
		Если ПустаяСтрока(АдресПочты) Тогда
			ИмяСобытия = НСтр("ka='Загрузка1СКасса.Загрузка1СКасса';ru='Загрузка1СКасса.Загрузка1СКасса'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				
			Комментарий = НСтр("ka='Не установлен адрес электронной почты получателя уведомления о новом пользователе из 1С:Кассы';ru='Не установлен адрес электронной почты получателя уведомления о новом пользователе из 1С:Кассы'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Если НЕ Результат Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ka='Абонент не принадлежит Штрих-М';ru='Абонент не принадлежит Штрих-М'"));
