...
Процедура ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ka='დაწყებულია ვალუტების კურსის სარეგლამენტო ჩატვირთვა';ru='Начата регламентная загрузка курсов валют'"));
	...
	Если ПриЗагрузкеВозниклиОшибки Тогда
		ЗаписьЖурналаРегистрации(
			...
			, 
			,
			НСтр("ka='ვალუტის კურსების ჩატვირთვის სარეგლამენტო დავალებისას წარმოიშვა შეცდომები';ru='Во время регламентного задания загрузки курсов валют возникли ошибки'"));
		ВызватьИсключение НСтр("ka='Загрузка курсов не выполнена. Подробности см. в журнале регистрации.';ru='Загрузка курсов не выполнена. Подробности см. в журнале регистрации.'");
	Иначе
		ЗаписьЖурналаРегистрации(
			ИмяСобытия,
			...
			,
			,
			НСтр("ka='დასრულებულია ვალუტის კურსების სარეგლამენტო ჩატვირთვა.';ru='Завершена регламентная загрузка курсов валют.'"));
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='ვალუტის კურსები მოძველდა';ru='Курсы валют устарели'");
		Дело.Важное         = Истина;
...
Функция Разрешения()
	...
	Порт = Неопределено;
	Описание = НСтр("ka='ვალუტების კურსის ჩატვირთვა ინტერნეტიდან.';ru='Загрузка курсов валют из Интернета.'");
	Разрешения.Добавить( 
...
Процедура ОбновитьКурсВалюты(Параметры, АдресРезультата) Экспорт
	...
	ИначеЕсли СпособУстановкиКурса = Перечисления.СпособыУстановкиКурсаВалюты.РасчетПоФормуле Тогда
		...
		Если ОсновныеВалюты.Количество() = 0 Тогда
			ТекстОшибки = НСтр("ka='ფორმულაში გამოყენებული უნდა იქნას თუნდაც ერთი ძირითადი ვალუტა.';ru='В формуле должна быть использована хотя бы одна основная валюта.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ka='Отключает ошибочно включенную загрузку курсов валюты ""Российский рубль (643)"" из интернета.';ru='Отключает ошибочно включенную загрузку курсов валюты ""Российский рубль (643)"" из интернета.'");
...
Процедура ОбновитьСведенияОВалюте937() Экспорт
	...
	Если Не Валюта.Пустая() Тогда
		...
		Валюта.НаименованиеПолное = НСтр("ka='ბოლივარი';ru='Боливар'");
...
Функция ЗагрузитьКурсВалютыИзФайла(Знач Валюта, Знач ПутьКФайлу, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки)
	...
	Если ЧислоЗагружаемыхДнейВсего <> ЧислоЗагруженныхДней Тогда
		Если ЧислоЗагруженныхДней = 0 Тогда
			ПояснениеОЗагрузке = НСтр("ka='ვალუტის კურსები %1 (%2) არ არის ჩატვირთული. მონაცემები არ არის.';ru='Курсы валюты %1 (%2) не загружены. Нет данных.'");
		Иначе
			ПояснениеОЗагрузке = НСтр("ka='ვალუტა %1 (%2)-ის თაობაზე ყველა კურსი არ არის ჩატვირთული.';ru='Загружены не все курсы по валюте %1 (%2).'");
		КонецЕсли;
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации()
	Возврат НСтр("ka='ვალუტები. ვალუტების კურსის ჩატვირთვა';ru='Валюты.Загрузка курсов валют'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗагрузитьАктуальныйКурсГрузия() Экспорт
	...
	ИмяСобытия = НСтр("ka='ვალუტები. ვალუტების კურსის ჩატვირთვა';ru='Валюты.Загрузка курсов валют'");
