...
"%ИнформацияОбОшибке%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПолныеПрава%", ?(ПолныеПрава = Истина, НСтр("ka='ხელმისაწვდომია სრული უფლებები';ru='доступны полные права'"), НСтр("ka='роль РедакторНовостей';ru='роль РедакторНовостей'")));
				...
				Если НЕ НайденныйПользователь.Пустая() Тогда
					Результат.Добавить(
						Новый Структура("Пользователь, ПользовательПредставление, ПолныеПрава",
							...
							?(ПолныеПрава = Истина,
								НайденныйПользователь.Наименование + " " + НСтр("ka='(სრული უფლებები)';ru='(полные права)'"),
								НайденныйПользователь.Наименование + " " + НСтр("ka='(სიახლეების რედატორი)';ru='(редактор новостей)'")),
							ПолныеПрава));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьОписаниеОтборов(ЛентаНовостей, Пользователь) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если ствПользовательскиеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ka='მომხმარებლის მიერ გამართული შერჩევები (შეცვლა შეიძლება):';ru='Отборы, настроенные пользователем (можно изменить):'") + Символы.ПС;
			Для Каждого ТекущееПредставлениеОтбора Из ствПользовательскиеОтборы Цикл
				...
				Если МассивЗначений.Количество() = 0 Тогда
					ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + "  " + ТекущееПредставлениеОтбора.Ключ + НСтр("ka=': * (ნებისმიერი მნიშვნელობა)';ru=': * (любое значение)'") + Символы.ПС;
		...
		Если ствАдминистративныеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ka='ადმინისტრატორის მიერ გამართული შერჩევები (შეცვლა არ შეიძლება):';ru='Отборы, настроенные администратором (нельзя изменить):'") + Символы.ПС;
		...
		Если ствАвтоматическиЗаполняемыеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ka='ავტომატურად შევსებადი შერჩევები (შეცვლა არ შეიძლება):';ru='Автоматически заполняемые отборы (нельзя изменить):'") + Символы.ПС;
		...
		Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
			ПредставлениеВсехОтборовПодробно = НСтр("ka='შერჩევები არ არის გამართული';ru='Отборы не настроены'");
		КонецЕсли;

	Иначе
		ПредставлениеВсехОтборовПодробно = НСтр("ka='ამ ლენტზე შერჩევების გამართვა არ ხდება';ru='По этой ленте отборы не настраиваются'");
	КонецЕсли;
