...
"Не рекомендуется разрешать редактирование из-за риска рассогласования данных.'"),
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='რედაქტირების უფლება';ru='Разрешить редактирование'"));
	...
	Иначе
		Если Параметры.МассивСсылок.Количество() = 1 Тогда
			ПоказатьОповещениеПользователя(НСтр("ka='Редактирование реквизитов разрешено';ru='Редактирование реквизитов разрешено'"),
				ПолучитьНавигационнуюСсылку(Параметры.МассивСсылок[0]), Параметры.МассивСсылок[0]);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Разрешено редактирование реквизитов объектов (%1)';ru='Разрешено редактирование реквизитов объектов (%1)'"),
				Параметры.МассивСсылок.Количество());
