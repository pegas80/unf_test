...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ПараметрыРаботыКлиента.ЭтоАвтономноеРабочееМесто Тогда
		...
		Если ПараметрыРаботыКлиента.СинхронизироватьДанныеСПриложениемВИнтернетеПриНачалеРаботы Тогда
			...
			ПоказатьОповещениеПользователя(НСтр("ka='ავტონომიური მუშაობა';ru='Автономная работа';en='Stand-alone operation'"), "e1cib/app/Обработка.ВыполнениеОбменаДанными",
				НСтр("ka='რეკომენდებულია მონაცემთა სინქრონიზება დანართთან ინტერნეტში.';ru='Рекомендуется синхронизировать данные с приложением в Интернете.';en='It is recommended to synchronize the data with the application on the Internet.'"), БиблиотекаКартинок.Информация32);
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьСинхронизироватьДанныеСПриложениемВИнтернетеПриЗавершенииСеанса"] = Истина
		...
		ПараметрыПредупреждения.РасширеннаяПодсказка = НСтр("ka='ზოგიერთ შემთხვევაში, მონაცემთა სინქრონიზაციას შეიძლება ხანგრძლივი დრო დასჭირდეს:"
" - კავშირგაბმულობის ნელი არხი;"
" - მონაცემთა დიდი მოცულობა;"
" - ხელმისაწვდომია დანართის განახლება ინტერნეტში.';ru='В некоторых случаях синхронизация данных может занять длительное время:"
" - медленный канал связи;"
" - большой объем данных;"
" - доступно обновление приложения в Интернете.'");

		ПараметрыПредупреждения.ТекстФлажка = НСтр("ka='მონაცემთა სინქრონიზება ინტერნეტში დანართით';ru='Синхронизировать данные с приложением в Интернете';en='Synchronize data with applications on the Internet'");
