...
Функция ПредставлениеСтандартногоПериода(СтандартныйПериод, ВидПериода) Экспорт
	...
	Если СтандартныйПериод.Вариант = ВариантСтандартногоПериода.Месяц Тогда
		Возврат НСтр("ka='გასული თვის ამავე თარიღიდან';ru='С такой же даты прошлого месяца'");
...
Функция СписокВычисляемыхПериодов(ВидПериода) Экспорт
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Месяц") Тогда
		...
		СписокПериодов.Добавить(ВариантСтандартногоПериода.Месяц, НСтр("ka='გასული თვის ამავე თარიღიდან';ru='С такой же даты прошлого месяца'"));
...
Функция СписокФиксированныхПериодов(Знач НачалоПериода, ВидПериода) Экспорт
	...
	НавигационныйПунктРанееПредставление = НСтр("ka='წინად..';ru='Ранее...'");
	НавигационныйПунктПозжеПредставление = НСтр("ka='მოგვიანებით...';ru='Позже...'");
	...
	Если ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.День") Тогда
		...
		Для Счетчик = 1 По 7 Цикл
			СписокПериодов.Добавить(Период, Формат(Период, НСТР("ka=""Л=ka; ДФ='dd MMMM yyyy, dddd'"";ru=""ДФ='dd MMMM yyyy, dddd'""")) + ?(Период = Сегодня, " - " + НСтр("ka='დღეს';ru='сегодня'"), ""));
			Период = Период + 86400;
		КонецЦикла;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Неделя") Тогда
		...
		Для Счетчик = 0 По 6 Цикл
			...
			ПредставлениеПериода = Формат(Период, "ДФ=dd.MM") + " - " + Формат(КонецПериода, "ДЛФ=D") + " (" + НеделяГода(КонецПериода) + " " + НСтр("ka='წლის კვირა';ru='неделя года'") + ")";
			Если Период = ТекущееНачалоНедели Тогда
				ПредставлениеПериода = ПредставлениеПериода + " - " + НСтр("ka='ეს კვირა';ru='эта неделя'");
			КонецЕсли;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Декада") Тогда
		...
		СтрокаДекада = НСтр("ka='დეკადა';ru='декада'");
		...
		Для Декада = НачальнаяДекада По КонечнаяДекада Цикл
			...
			ДекадаВМесяце = ДекадаВГоду - (МесяцВГоду-1)*3 + 1;
			Период = Дата(Год, МесяцВГоду, (ДекадаВМесяце - 1) * 10 + 1);
			Представление = Формат(Период, НСТР("ka=""Л=ka;ДФ='MMMM yyyy'"";ru=""ДФ='MMMM yyyy'""")) + ", " + Лев("III", ДекадаВМесяце) + " " + СтрокаДекада + ?(Декада = ТекущаяДекадаАбсолютно, " - " + НСтр("ka='ეს დეკადა';ru='эта декада'"), "");
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Месяц") Тогда
		...
		Для Месяц = НачальныйМесяц По КонечныйМесяц Цикл
			...
			МесяцВГоду = Месяц - Год * 12;
			Период = Дата(Год, МесяцВГоду, 1);
			СписокПериодов.Добавить(Период, Формат(Период, НСтр("ka=""Л=ka;ДФ='MMMM yyyy'"";ru=""ДФ='MMMM yyyy'""")) + ?(Год = ТекущийГод И ТекущийМесяц = Месяц, " - " + НСтр("ka='ეს თვე';ru='этот месяц'"), ""));
		КонецЦикла;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Квартал") Тогда
		...
		СтрокаКвартал = НСтр("ka='კვარტალი';ru='квартал'");
		...
		Для Квартал = НачальныйКвартал По КонечныйКвартал Цикл
			...
			МесяцВГоду = (КварталВГоду-1)*3 + 1;
			Период = Дата(Год, МесяцВГоду, 1);
			Представление = ?(КварталВГоду = 4, "IV", Лев("III", КварталВГоду)) + " " + СтрокаКвартал + " " + Формат(Период, "ДФ='yyyy'") + ?(Квартал = ТекущийКварталАбсолютно, " - " + НСтр("ka='ეს კვარტალი';ru='этот квартал'"), "");
			СписокПериодов.Добавить(Период, Представление);
		КонецЦикла;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Полугодие") Тогда
		...
		СтрокаПолугодие = НСтр("ka='ნახევარი წელი';ru='полугодие'");
		...
		Для Полугодие = НачальноеПолугодие По КонечноеПолугодие Цикл
			...
			МесяцВГоду = (ПолугодиеВГоду-1)*6 + 1;
			Период = Дата(Год, МесяцВГоду, 1);
			Представление = Лев("II", ПолугодиеВГоду) + " " + СтрокаПолугодие + " " + Формат(Период, "ДФ='yyyy'") + ?(Полугодие = ТекущееПолугодиеАбсолютно, " - " + НСтр("ka='არ ნახევარი წელია';ru='это полугодие'"), "");
			СписокПериодов.Добавить(Период, Представление);
		КонецЦикла;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Год") Тогда
		...
		Для Год = НачальныйГод По КонечныйГод Цикл
			СписокПериодов.Добавить(Дата(Год, 1, 1), Формат(Год, "ЧГ=") + ?(Год = ТекущийГод, " - " + НСтр("ka='ეს წელი';ru='этот год'"), ""));
		КонецЦикла;
