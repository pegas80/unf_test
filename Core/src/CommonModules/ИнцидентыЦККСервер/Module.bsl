...
Процедура СоздатьЗаписьТипа(Знач СловарьТипов, Знач Описание) Экспорт
	...
	Если ТипЗнч(СловарьТипов)<>Тип("Соответствие") Тогда		
		ВызватьИсключение НСтр("ka='პარამეტრ ""ტიპებისლექსიკონი""-ს უნდა ჰქონდეს ტიპი ""შესაბამისობა"".';ru='Параметр СловарьТипов должен иметь тип Соответствие.'");
	КонецЕсли;
	
	Если ТипЗнч(Описание)<>Тип("Структура") Тогда		
		ВызватьИсключение НСтр("ka='პარამეტრ ""აღწერილობა""-ს უნდა ჰქონდეს ტიპი ""სტრუქტურა"".';ru='Параметр Описание должен иметь тип Структура.'");
	КонецЕсли;
	...
	Если ПустаяСтрока(Описание.ТипИнцидента) Тогда
		ВызватьИсключение НСтр("ka='ინციდენტისტიპი არ არის მითითებული';ru='ТипИнцидента не указан'");
	КонецЕсли;
	...
	Если УровеньИнцидента<>"Информация" И 
		...
		ВызватьИсключение НСтр("ka='ინციდენტის დონის არასწორი მნიშვნელობა';ru='Неверное значение уровня инцидента'");
	КонецЕсли;
...
Процедура ОткрытьИнцидент(Знач ТипИнцидента, Знач КодИнцидента, Знач ТекстСообщения, Знач Асинхронно = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТипИнцидента) Тогда	
		ВызватьИсключение НСтр("ka='ინციდენტის ტიპი არ არის მითითებული';ru='Не указан тип инцидента'"); 
	КонецЕсли;
	Если ПустаяСтрока(КодИнцидента) Тогда	
		ВызватьИсключение НСтр("ka='ინციდენტის კოდი არ არის მითითებული';ru='Не указан код инцидента'"); 
	КонецЕсли;
	Если ПустаяСтрока(ТекстСообщения) Тогда	
		ВызватьИсключение НСтр("ka='ინციდენტის შეტყობინება არ არის მითითებული';ru='Не указано сообщение инцидента'"); 
	КонецЕсли;
	...
	ВызватьИсключение СтрШаблон(
		НСтр("ka='ინციდენტის ტიპი %1 არ არის რეგისტრირებული';ru='Тип инцидента %1 не зарегистрирован'"), 
		ТипИнцидента);
...
Процедура ОтослатьСтатистику(Знач ИдентификаторСчетчика, Знач ЗначениеСчетчика, Знач Данные = неопределено, Знач ДатаСчетчика = неопределено) Экспорт
	...
	Если Результат <> "OK" Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ინტეგრაციაЦКК';ru='ИнтеграцияЦКК'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ka='შეცდომა ხკც-ში მრიცხველის გადაცემისას';ru='Ошибка при передаче счетчика в ЦКК'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(КомандаЗапроса, Данные)
	...
	Если Ответ.КодСостояния<>200 Тогда
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка,,, 
			СтрШаблон(
			НСтр("ka='ხარისხის კონტროლის ცენტრის ინციდენტების სერვისის ბრძანება %1-ის შეცდომა (კოდი %3): %2';ru='Ошибка (код %3) команды %1 сервиса инцидентов ЦКК: %2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОтослатьПоказателиДлиныОчередейЗаданий() 
	...
	Иначе
		ВызватьИсключение НСтр("ka='კლასტერის და საინფორმაციო ბაზის სახელის დადგენა შეუძლებელია შეერთების სტრიქონიდან';ru='Невозможно определить имя кластера и информационной базы из строки подключения'");
	КонецЕсли;
