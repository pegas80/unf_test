...
Процедура СоздатьЗаписьТипа(Знач СловарьТипов, Знач Описание) Экспорт
	...
	Если ТипЗнч(СловарьТипов)<>Тип("Соответствие") Тогда		
		ВызватьИсключение НСтр("ka='პარამეტრ ""ტიპებისლექსიკონი""-ს უნდა ჰქონდეს ტიპი ""შესაბამისობა"".';ru='Параметр СловарьТипов должен иметь тип Соответствие.';en='SlovarTipov parameter must be of type Value.'");
	КонецЕсли;
	
	Если ТипЗнч(Описание)<>Тип("Структура") Тогда		
		ВызватьИсключение НСтр("ka='პარამეტრ ""აღწერილობა""-ს უნდა ჰქონდეს ტიპი ""სტრუქტურა"".';ru='Параметр Описание должен иметь тип Структура.';en='Description Parameter must have a type of structure.'");
	КонецЕсли;
	...
	Если ПустаяСтрока(Описание.ТипИнцидента) Тогда
		ВызватьИсключение НСтр("ka='ინციდენტისტიპი არ არის მითითებული';ru='ТипИнцидента не указан';en='TipIntsidenta Unknown'");
	КонецЕсли;
	...
	Если УровеньИнцидента<>"Информация" И 
		...
		ВызватьИсключение НСтр("ka='ინციდენტის დონის არასწორი მნიშვნელობა';ru='Неверное значение уровня инцидента';en='Invalid value of the incident level'");
	КонецЕсли;
...
Процедура ОткрытьИнцидент(Знач ТипИнцидента, Знач КодИнцидента, Знач ТекстСообщения, Знач Асинхронно = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТипИнцидента) Тогда	
		ВызватьИсключение НСтр("ka='ინციდენტის ტიპი არ არის მითითებული';ru='Не указан тип инцидента';en='Unknown type of incident'"); 
	КонецЕсли;
	Если ПустаяСтрока(КодИнцидента) Тогда	
		ВызватьИсключение НСтр("ka='ინციდენტის კოდი არ არის მითითებული';ru='Не указан код инцидента';en='Unknown incident code'"); 
	КонецЕсли;
	Если ПустаяСтрока(ТекстСообщения) Тогда	
		ВызватьИсключение НСтр("ka='ინციდენტის შეტყობინება არ არის მითითებული';ru='Не указано сообщение инцидента';en='Unknown message incident'"); 
	КонецЕсли;
	...
	ВызватьИсключение СтрШаблон(
		НСтр("ka='ინციდენტის ტიპი %1 არ არის რეგისტრირებული';ru='Тип инцидента %1 не зарегистрирован';en='1% of incident type is not registered'"), 
		ТипИнцидента);
...
Процедура ОтослатьСтатистику(Знач ИдентификаторСчетчика, Знач ЗначениеСчетчика, Знач Данные = неопределено, Знач ДатаСчетчика = неопределено) Экспорт
	...
	Если Результат <> "OK" Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ინტეგრაციაЦКК';ru='ИнтеграцияЦКК';en='IntegratsiyaTsKK'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ka='შეცდომა ხკც-ში მრიცხველის გადაცემისას';ru='Ошибка при передаче счетчика в ЦКК';en='Transmission error counter CCC'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(КомандаЗапроса, Данные)
	...
	Если Ответ.КодСостояния<>200 Тогда
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка,,, 
			СтрШаблон(
			НСтр("ka='ხარისხის კონტროლის ცენტრის ინციდენტების სერვისის ბრძანება %1-ის შეცდომა (კოდი %3): %2';ru='Ошибка (код %3) команды %1 сервиса инцидентов ЦКК: %2';en='Error (code% 3)% 1 service team CCC incidents: 2%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОтослатьПоказателиДлиныОчередейЗаданий() 
	...
	Иначе
		ВызватьИсключение НСтр("ka='კლასტერის და საინფორმაციო ბაზის სახელის დადგენა შეუძლებელია შეერთების სტრიქონიდან';ru='Невозможно определить имя кластера и информационной базы из строки подключения';en='Unable to determine the cluster name and the information base of the connecting line'");
	КонецЕсли;
