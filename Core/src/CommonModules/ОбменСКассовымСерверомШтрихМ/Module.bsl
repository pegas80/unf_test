...
Процедура ПолучитьКассовыеДокументыДляКассовыхАппаратов(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		Если Настройки.НетПодключенныхКасс Тогда
			СообщениеОбОшибке = НСтр("ka='Нет подключенных ККМ.';ru='Нет подключенных ККМ.';en='No connection CMC.'", КодЯзыка);
			УровеньЖурнала = УровеньЖурналаРегистрации.Информация;
		Иначе
			СообщениеОбОшибке = НСтр("ka='Не настроен обмен с сервером.';ru='Не настроен обмен с сервером.';en='It does not set up an exchange with the server.'", КодЯзыка);
			ЕстьОшибки = Истина;
			УровеньЖурнала = УровеньЖурналаРегистрации.Предупреждение;
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Документы для кассовых аппаратов.';ru='Документы для кассовых аппаратов.';en='Documents for cash registers.'", КодЯзыка),
...
Процедура ВыполнитьОчисткуТоваровНаКассовыхАппаратах(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		...
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Очистка всех товаров на кассовых аппаратах.';ru='Очистка всех товаров на кассовых аппаратах.';en='Cleaning of all goods to the cash registers.'", КодЯзыка),
				УровеньЖурнала,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПодключениеКассовыхАппратов(ИнформацияПоКассовымАппаратам, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если Режим = 1 Тогда
		...
		ЗаголовокЖурнала = НСтр("ka='Подключение кассовых аппаратов.';ru='Подключение кассовых аппаратов.';en='Connecting the cash register.'", КодЯзыка);
	ИначеЕсли Режим = 2 Тогда
		Настройки.Вставить("ТипМетода", "cashregister_remove_from_agent");
		ЗаголовокЖурнала = НСтр("ka='Отключение кассовых аппаратов.';ru='Отключение кассовых аппаратов.';en='Disabling the cash registers.'", КодЯзыка);
	Иначе
		Настройки.Вставить("ТипМетода", "cashregister_change_agent_node_id");
		ЗаголовокЖурнала = НСтр("ka='Переподключение кассовых аппаратов.';ru='Переподключение кассовых аппаратов.';en='Reconnect the cash registers.'", КодЯзыка);
	КонецЕсли;
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		...
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Подключение кассовых аппаратов.';ru='Подключение кассовых аппаратов.';en='Connecting the cash register.'", КодЯзыка),
				УровеньЖурнала,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		...
		Если ЕстьОшибки Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Получение идентификатора области.';ru='Получение идентификатора области.';en='Receiving area identifier.'", КодЯзыка),
		...
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ka='Получение идентификатора области.';ru='Получение идентификатора области.';en='Receiving area identifier.'", КодЯзыка),
				...
				НСтр("ka='Не установлен идентификатор области.';ru='Не установлен идентификатор области.';en='Do not set the identifier field.'", КодЯзыка));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьВыгрузкуТоваров(ТоварыИКассы, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		...
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров на кассовых аппаратах.';ru='Выгрузка товаров на кассовых аппаратах.';en='Unloading of goods at the cash register.'", КодЯзыка),
				УровеньЖурнала,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если Соединение.HTTPСоединение = Неопределено Тогда
		...
		ЕстьОшибки = Истина;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров. Соединение с сервером.';ru='Выгрузка товаров. Соединение с сервером.';en='Unloading of goods. Connection to the server.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ТоварыИКассы Цикл
		...
		Для каждого Строка Из ИнформацияПоКассовымАппаратам Цикл
			...
			Если ЗарегистрированнаяКасса = Неопределено Тогда
				ЕстьОшибки = Истина;
				Если ВестиЛог Тогда
					ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров. Зарегистрированные кассы.';ru='Выгрузка товаров. Зарегистрированные кассы.';en='Unloading of goods. Registered offices.'", КодЯзыка),
						УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
				КонецЕсли;
				Возврат;
			Иначе
		...
		Если ЕстьОшибки Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров. Регистрация товаров.';ru='Выгрузка товаров. Регистрация товаров.';en='Unloading of goods. Registration of goods.'", КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
			ЕстьОшибки = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьСтатусЗагрузкиТоваровНаКассовыхАппаратах(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	Если Соединение.HTTPСоединение = Неопределено Тогда
		...
		ЕстьОшибки = Истина;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Проверка актуальности товаров.';ru='Проверка актуальности товаров.';en='Check the relevance of the goods.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция ЗарегистрированнаяКассаНаСервере(ИнформацияОКассе, Создавать, СообщениеОбОшибке, ОбластьДанных = 0) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ЗарегистрированнаяКасса) Тогда
		...
		Иначе
			СообщениеОбОшибке = СтрШаблон(НСтр("ru='Не найдена зарегистрированная касса с рег. номером: %1 ';ka='Не найдена зарегистрированная касса с рег. номером: %1 '",
...
Функция УзелОбменаШтрих() Экспорт
	...
	Если Не ЗначениеЗаполнено(ГлавныйУзелОбмена.Наименование) Тогда
		...
		ГлавныйУзелОбъект.Наименование = НСтр("ka='ცენტრალური';ru='Центральный';en='Central'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Попытка
	...
	Если УзелОбменаШтрихСсылка.Пустая() Тогда
		...
		УзелОбменаШтрихОбъект.Наименование =
			НСтр("ka='Настройка обмена с кассовым сервером Штрих-М';ru='Настройка обмена с кассовым сервером Штрих-М';en='Setting up an exchange with a cash register server Bar-M'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Установка соедиенения.';ka='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Установка соедиенения.'", КодЯзыка),
	...
	Если НЕ ЗначениеЗаполнено(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		...
		Если Отказ Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Получение идентификатора области';ka='Регламентное задание ""Обмен данными с сервером Штрих-М""."
...
"Получение идентификатора области'", КодЯзыка),
	...
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Проверка принадлежности касс к текущей области.';ka='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Проверка принадлежности касс к текущей области.'", КодЯзыка),
	...
	Если НЕ ЕстьПодключенныеКассы(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Проверка регистрации касс в текущей области.';ka='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Проверка регистрации касс в текущей области.'", КодЯзыка),
			УровеньЖурналаРегистрации.Предупреждение,,,
			НСтр("ka='Нет ни одной зарегистрированной кассы в области.';ru='Нет ни одной зарегистрированной кассы в области.';en='No one registered offices in the region.'", КодЯзыка));
		Возврат;
	КонецЕсли;
	...
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Регламентное задание ""Обмен данными с сервером Штрих-М""."
"Получение кассовых документов.';ka='Регламентное задание ""Обмен данными с сервером Штрих-М""."
...
Процедура ОбработатьОтвет_КассовыеДокументы(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (кассовые документы).';ru='Ошибка формата ответного сообщения (кассовые документы).';en='the reply message format error (cash documents).'",
	...
	Если НЕ ИмяРеквизитаОтвета = "fetch_documents_response" Тогда
		...
		Иначе
			СообщениеОбОшибке = НСтр("ka='Ошибка записи кассового аппарата.';ru='Ошибка записи кассового аппарата.';en='cash register write error.'",
...
"с одним регистрационным кодом %1 и одним идентификатором области Штрих %2.';ka='Более одной зарегистрированной кассы,"
...
"Не найдены реквизиты документа';ka='Ошибка при обмене документом doc_id: %1"
...
Процедура ОбработатьОтвет_РегистрацияКасс(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (регистрация касс).';ru='Ошибка формата ответного сообщения (регистрация касс).';en='Format error response message (registration offices).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатПодключенияКасс(КассыДляРегистрации, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ka='При регистрации ошибочно обработан ККМ: %1';ru='При регистрации ошибочно обработан ККМ: %1';en='When registering wrongly processed CMC% 1'", КодЯзыка),
		...
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ka='При регистрации ошибочно обработан регистрационный номер ККМ: %1';ru='При регистрации ошибочно обработан регистрационный номер ККМ: %1';en='When registering erroneously interpreted by CMC registration number:% 1'",
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Подключение ККМ.';ru='Обмен Штрих. Подключение ККМ.';en='Exchange Bar. Connecting the CMC.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_ОчисткаТоваров(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (удаление товаров).';ru='Ошибка формата ответного сообщения (удаление товаров).';en='Format error response message (removal of goods).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_ПроверкаАктуальностиТоваров(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		ОбработатьРезультатПроверкаАктуальностиТоваров(ДополнительныеПараметры,
			Отказ, СообщениеОбОшибке);
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (проверка актуальности товаров).';ru='Ошибка формата ответного сообщения (проверка актуальности товаров).';en='Format error response message (check the relevance of the goods).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_РегистрацияТоваров(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		ОбработатьРезультатРегистрацииТоваров(ДополнительныеПараметры,
			Отказ, СообщениеОбОшибке);
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (регистрация товаров).';ru='Ошибка формата ответного сообщения (регистрация товаров).';en='the reply message format error (registration goods).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатОчисткиТоваров(ДополнительныеПараметры, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Попытка
				КассаОбъект = СтрокаКассы.Ссылка.ПолучитьОбъект();
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ru='При очистке товаров ошибочно обработан ККМ с заводским номером: %1';en='When cleaning products mistakenly treated with CMC serial number: 1%'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если КассаОбъект = Неопределено Тогда
				Сообщение = Сообщение + СтрШаблон(НСтр("ru='При очистке товаров ошибочно обработан ККМ с заводским номером: %1. Объект не найден.';en='When cleaning products mistakenly treated with CMC serial number: 1%. Object not found.'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецЕсли;
			...
			КонецЕсли;
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ru='При очистке товаров ошибочно обработан ККМ с заводским номером: %1. Не найдена ссылка на объект.';en='When cleaning products mistakenly treated with CMC serial number: 1%. Found object reference.'",
				КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьОшибка Тогда
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Очистка товаров';ru='Обмен Штрих. Очистка товаров';en='Exchange Bar. Cleaning products'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатПроверкаАктуальностиТоваров(Кассы, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из Кассы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Попытка
				КассаОбъект = СтрокаКассы.Ссылка.ПолучитьОбъект();
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ru='При проверке товаров ошибочно обработан ККМ с заводским номером: %1';en='When checking goods mistakenly treated with CMC serial number: 1%'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если КассаОбъект = Неопределено Тогда
				Сообщение = Сообщение + СтрШаблон(НСтр("ru='При проверке товаров ошибочно обработан ККМ с заводским номером: %1. Объект не найден.';en='When checking goods erroneously interpreted by CMC with the serial number:% 1. Object not found.'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецЕсли;
			...
			КонецЕсли;
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ru='При проверке товаров ошибочно обработан ККМ с заводским номером: %1. Не найдена ссылка на объект.';en='When checking goods erroneously interpreted by CMC with the serial number:% 1. Found object reference.'",
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Проверка актуальности товаров.';ru='Обмен Штрих. Проверка актуальности товаров.';en='Exchange Bar. Check the relevance of the goods.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатРегистрацииТоваров(ДополнительныеПараметры, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Попытка
				КассаОбъект = СтрокаКассы.Ссылка.ПолучитьОбъект();
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ru='При регистрации товаров ошибочно обработан ККМ с заводским номером: %1';en='When registering the goods handled wrongly CMC with serial number: 1%'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если КассаОбъект = Неопределено Тогда
				Сообщение = Сообщение + СтрШаблон(НСтр("ru='При регистрации товаров ошибочно обработан ККМ с заводским номером: %1. Объект не найден.';en='When registering the goods handled wrongly CMC with serial number: 1%. Object not found.'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецЕсли;
			...
			КонецЕсли;
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ru='При регистрации товаров ошибочно обработан ККМ с заводским номером: %1. Не найдена ссылка на объект.';en='When registering the goods handled wrongly CMC with serial number: 1%. Found object reference.'",
				КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьОшибка Тогда
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Регистрация товаров.';ru='Обмен Штрих. Регистрация товаров.';en='Exchange Bar. Registration of goods.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_НовыйИдентификаторОбласти(Результат, Отказ, СообщениеОбОшибке)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (Новый идентификатор области).';ru='Ошибка формата ответного сообщения (Новый идентификатор области).';en='Format error response message (New ID field).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтветСервера(Результат, Настройки, Отказ, СообщениеОбОшибке)
	...
	Если Результат.КодСостояния >= 400 И Результат.КодСостояния < 500 Тогда
		СообщениеОбОшибке = НСтр("ka='Код статуса больше 4XX, ошибка запроса. Код статуса: %1';ru='Код статуса больше 4XX, ошибка запроса. Код статуса: %1';en='4XX status code greater, query error. Status Code:% 1'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 500 И Результат.КодСостояния < 600 Тогда
		СообщениеОбОшибке = НСтр("ka='Код статуса больше 5XX, ошибка сервера. Код статуса: %1';ru='Код статуса больше 5XX, ошибка сервера. Код статуса: %1';en='5XX status code greater, server error. Status Code:% 1'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 300 И Результат.КодСостояния < 400 Тогда
		СообщениеОбОшибке = НСтр("ka='Код статуса больше 3XX, Перенаправление. Код статуса: %1';ru='Код статуса больше 3XX, Перенаправление. Код статуса: %1';en='status code greater 3XX, Redirect. Status Code:% 1'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Если Результат.КодСостояния = 302 Тогда
			...
			Иначе
				СообщениеОбОшибке = НСтр("ka='მუდმივი გადამისამართება. სტატუსის კოდი: %1';ru='Постоянное перенаправление. Код статуса: %1';en='A permanent redirect. Status Code:% 1'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				Отказ = Истина;
			КонецЕсли;
		Иначе
	...
	Если ОбъектJSON.Свойство("response_error") Тогда
		...
		Если НЕ КодРезультата = 0 Тогда
			...
			Если КодОшибки = 1 Тогда
				СообщениеОбОшибке = НСтр("ka='Несуществующий идентификатор сервиса.';ru='Несуществующий идентификатор сервиса.';en='Non-existent service ID.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 2 Тогда
				СообщениеОбОшибке = НСтр("ka='არასწორი ხელმოწერა.';ru='Неверная подпись.';en='Invalid signature.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 3 Тогда
				СообщениеОбОшибке = НСтр("ka='Неверный тип запроса.';ru='Неверный тип запроса.';en='Invalid request type.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 4 Тогда
				СообщениеОбОшибке = НСтр("ka='Неверный формат запроса.';ru='Неверный формат запроса.';en='Invalid request format.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 5 Тогда
				СообщениеОбОшибке = НСтр("ka='სერვერის შეცდომა.';ru='Ошибка сервера.';en='Server error.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			Иначе
				СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения.';ru='Ошибка формата ответного сообщения.';en='Error response message format.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			КонецЕсли;
