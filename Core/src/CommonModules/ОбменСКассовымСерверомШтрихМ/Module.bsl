...
Процедура ПолучитьКассовыеДокументыДляКассовыхАппаратов(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		Если Настройки.НетПодключенныхКасс Тогда
			СообщениеОбОшибке = НСтр("ka='Нет подключенных ККМ.';ru='Нет подключенных ККМ.'", КодЯзыка);
			УровеньЖурнала = УровеньЖурналаРегистрации.Информация;
		Иначе
			СообщениеОбОшибке = НСтр("ka='Не настроен обмен с сервером.';ru='Не настроен обмен с сервером.'", КодЯзыка);
			ЕстьОшибки = Истина;
			УровеньЖурнала = УровеньЖурналаРегистрации.Предупреждение;
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Документы для кассовых аппаратов.';ru='Документы для кассовых аппаратов.'", КодЯзыка),
...
Процедура ВыполнитьОчисткуТоваровНаКассовыхАппаратах(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		...
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Очистка всех товаров на кассовых аппаратах.';ru='Очистка всех товаров на кассовых аппаратах.'", КодЯзыка),
				УровеньЖурнала,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПодключениеКассовыхАппратов(ИнформацияПоКассовымАппаратам, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если Режим = 1 Тогда
		...
		ЗаголовокЖурнала = НСтр("ka='Подключение кассовых аппаратов.';ru='Подключение кассовых аппаратов.'", КодЯзыка);
	ИначеЕсли Режим = 2 Тогда
		Настройки.Вставить("ТипМетода", "cashregister_remove_from_agent");
		ЗаголовокЖурнала = НСтр("ka='Отключение кассовых аппаратов.';ru='Отключение кассовых аппаратов.'", КодЯзыка);
	Иначе
		Настройки.Вставить("ТипМетода", "cashregister_change_agent_node_id");
		ЗаголовокЖурнала = НСтр("ka='Переподключение кассовых аппаратов.';ru='Переподключение кассовых аппаратов.'", КодЯзыка);
	КонецЕсли;
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		...
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Подключение кассовых аппаратов.';ru='Подключение кассовых аппаратов.'", КодЯзыка),
				УровеньЖурнала,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		...
		Если ЕстьОшибки Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Получение идентификатора области.';ru='Получение идентификатора области.'", КодЯзыка),
		...
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ka='Получение идентификатора области.';ru='Получение идентификатора области.'", КодЯзыка),
				...
				НСтр("ka='Не установлен идентификатор области.';ru='Не установлен идентификатор области.'", КодЯзыка));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьВыгрузкуТоваров(ТоварыИКассы, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		...
		КонецЕсли;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров на кассовых аппаратах.';ru='Выгрузка товаров на кассовых аппаратах.'", КодЯзыка),
				УровеньЖурнала,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если Соединение.HTTPСоединение = Неопределено Тогда
		...
		ЕстьОшибки = Истина;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров. Соединение с сервером.';ru='Выгрузка товаров. Соединение с сервером.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ТоварыИКассы Цикл
		...
		Для каждого Строка Из ИнформацияПоКассовымАппаратам Цикл
			...
			Если ЗарегистрированнаяКасса = Неопределено Тогда
				ЕстьОшибки = Истина;
				Если ВестиЛог Тогда
					ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров. Зарегистрированные кассы.';ru='Выгрузка товаров. Зарегистрированные кассы.'", КодЯзыка),
						УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
				КонецЕсли;
				Возврат;
			Иначе
		...
		Если ЕстьОшибки Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка товаров. Регистрация товаров.';ru='Выгрузка товаров. Регистрация товаров.'", КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
			ЕстьОшибки = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьСтатусЗагрузкиТоваровНаКассовыхАппаратах(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	Если Соединение.HTTPСоединение = Неопределено Тогда
		...
		ЕстьОшибки = Истина;
		Если ВестиЛог Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Проверка актуальности товаров.';ru='Проверка актуальности товаров.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция УзелОбменаШтрих() Экспорт
	...
	Если Не ЗначениеЗаполнено(ГлавныйУзелОбмена.Наименование) Тогда
		...
		ГлавныйУзелОбъект.Наименование = НСтр("ka='ცენტრალური';ru='Центральный'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Попытка
	...
	Если УзелОбменаШтрихСсылка.Пустая() Тогда
		...
		УзелОбменаШтрихОбъект.Наименование =
			НСтр("ka='Настройка обмена с кассовым сервером Штрих-М';ru='Настройка обмена с кассовым сервером Штрих-М'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Проверка регистрации касс в текущей области.'", КодЯзыка),
			...
			НСтр("ka='Нет ни одной зарегистрированной кассы в области.';ru='Нет ни одной зарегистрированной кассы в области.'", КодЯзыка));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьОтвет_КассовыеДокументы(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (кассовые документы).';ru='Ошибка формата ответного сообщения (кассовые документы).'",
	...
	Если НЕ ИмяРеквизитаОтвета = "fetch_documents_response" Тогда
		...
		Иначе
			СообщениеОбОшибке = НСтр("ka='Ошибка записи кассового аппарата.';ru='Ошибка записи кассового аппарата.'",
...
Процедура ОбработатьОтвет_РегистрацияКасс(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (регистрация касс).';ru='Ошибка формата ответного сообщения (регистрация касс).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатПодключенияКасс(КассыДляРегистрации, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ka='При регистрации ошибочно обработан ККМ: %1';ru='При регистрации ошибочно обработан ККМ: %1'", КодЯзыка),
		...
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ka='При регистрации ошибочно обработан регистрационный номер ККМ: %1';ru='При регистрации ошибочно обработан регистрационный номер ККМ: %1'",
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Подключение ККМ.';ru='Обмен Штрих. Подключение ККМ.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_ОчисткаТоваров(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (удаление товаров).';ru='Ошибка формата ответного сообщения (удаление товаров).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_ПроверкаАктуальностиТоваров(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		ОбработатьРезультатПроверкаАктуальностиТоваров(ДополнительныеПараметры,
			Отказ, СообщениеОбОшибке);
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (проверка актуальности товаров).';ru='Ошибка формата ответного сообщения (проверка актуальности товаров).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_РегистрацияТоваров(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		ОбработатьРезультатРегистрацииТоваров(ДополнительныеПараметры,
			Отказ, СообщениеОбОшибке);
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (регистрация товаров).';ru='Ошибка формата ответного сообщения (регистрация товаров).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатОчисткиТоваров(ДополнительныеПараметры, Отказ, СообщениеОбОшибке)
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Очистка товаров';ru='Обмен Штрих. Очистка товаров'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатПроверкаАктуальностиТоваров(Кассы, Отказ, СообщениеОбОшибке)
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Проверка актуальности товаров.';ru='Обмен Штрих. Проверка актуальности товаров.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьРезультатРегистрацииТоваров(ДополнительныеПараметры, Отказ, СообщениеОбОшибке)
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='Обмен Штрих. Регистрация товаров.';ru='Обмен Штрих. Регистрация товаров.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_НовыйИдентификаторОбласти(Результат, Отказ, СообщениеОбОшибке)
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения (Новый идентификатор области).';ru='Ошибка формата ответного сообщения (Новый идентификатор области).'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтветСервера(Результат, Настройки, Отказ, СообщениеОбОшибке)
	...
	Если Результат.КодСостояния >= 400 И Результат.КодСостояния < 500 Тогда
		СообщениеОбОшибке = НСтр("ka='Код статуса больше 4XX, ошибка запроса. Код статуса: %1';ru='Код статуса больше 4XX, ошибка запроса. Код статуса: %1'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 500 И Результат.КодСостояния < 600 Тогда
		СообщениеОбОшибке = НСтр("ka='Код статуса больше 5XX, ошибка сервера. Код статуса: %1';ru='Код статуса больше 5XX, ошибка сервера. Код статуса: %1'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Отказ = Истина;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 300 И Результат.КодСостояния < 400 Тогда
		СообщениеОбОшибке = НСтр("ka='Код статуса больше 3XX, Перенаправление. Код статуса: %1';ru='Код статуса больше 3XX, Перенаправление. Код статуса: %1'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Если Результат.КодСостояния = 302 Тогда
			...
			Иначе
				СообщениеОбОшибке = НСтр("ka='მუდმივი გადამისამართება. სტატუსის კოდი: %1';ru='Постоянное перенаправление. Код статуса: %1'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				Отказ = Истина;
			КонецЕсли;
		Иначе
	...
	Если ОбъектJSON.Свойство("response_error") Тогда
		...
		Если НЕ КодРезультата = 0 Тогда
			...
			Если КодОшибки = 1 Тогда
				СообщениеОбОшибке = НСтр("ka='Несуществующий идентификатор сервиса.';ru='Несуществующий идентификатор сервиса.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 2 Тогда
				СообщениеОбОшибке = НСтр("ka='არასწორი ხელმოწერა.';ru='Неверная подпись.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 3 Тогда
				СообщениеОбОшибке = НСтр("ka='Неверный тип запроса.';ru='Неверный тип запроса.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 4 Тогда
				СообщениеОбОшибке = НСтр("ka='Неверный формат запроса.';ru='Неверный формат запроса.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ИначеЕсли КодОшибки = 5 Тогда
				СообщениеОбОшибке = НСтр("ka='სერვერის შეცდომა.';ru='Ошибка сервера.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			Иначе
				СообщениеОбОшибке = НСтр("ka='Ошибка формата ответного сообщения.';ru='Ошибка формата ответного сообщения.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			КонецЕсли;
