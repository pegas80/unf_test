...
Функция ИзменитьПорядокЭлементов(Ссылка, УстановленныеОтборы, ОтображениеСписком, Вверх) Экспорт
	...
	Если Информация.ЕстьРодитель И ОтображениеСписком И Не УстановленныеОтборы.ЕстьОтборПоРодителю Тогда
		Возврат НСтр("ka='გადაადგილების წინ აუცილებელია გამოსახულების დაყენება ხის ან იერარქიული სიის სახით!';ru='Перед перемещением необходимо установить отображение в виде дерева или иерархического списка!'");
	КонецЕсли;
	...
	Если Информация.ЕстьВладелец И НЕ УстановленныеОтборы.ЕстьОтборПоВладельцу Тогда
		Возврат НСтр("ka='გადაადგილების წინ აუცილებელია მფლობელის მიხედვით შერჩევის დაყენება!';ru='Перед перемещением необходимо установить отбор по владельцу!'");
	КонецЕсли;
...
Функция КПППоКомбинацииОрганизацииИКодаИФНС(Организация, КодИФНС) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეუძლებელია აამკ-ის დადგენა ორგანიზაცია %1-სთვის და';ru='Невозможно определить КПП для организации %1 и ИФНС с кодом %2!'"), Организация, КодИФНС);
	...
	Для Каждого МакетОтчета Из МакетыОтчета Цикл
		...
		И Лев(нрег(МакетОтчета.Имя), СтрДлина(ПрефиксИмениМакетаШаблона)) = нрег(ПрефиксИмениМакетаШаблона) Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მანქანასაკითხავი ფორმის შაბლონის მიღება ვერ მოხერხდა ""%1""!%2';ru='Не удалось получить шаблон машиночитаемой формы ""%1""!%2'"), ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მანქანასაკითხავი ფორმის შაბლონის მიღება ვერ მოხერხდა ""%1""!%2';ru='Не удалось получить шаблон машиночитаемой формы ""%1""!%2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
...
Функция ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(АдресПараметровПроксиВоВременномХранилище = "", ПолучитьОпределенияВебСервиса = Истина, ЭтоВызовИзВебКлиента = Ложь) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Невозможно подключиться к веб-сервису формирования машиночитаемых бланков.';ru='Невозможно подключиться к веб-сервису формирования машиночитаемых бланков.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(АдресТочкиПодключенияВебСервиса) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не заполнен адрес точки подключения веб-сервиса!';ru='Не заполнен адрес точки подключения веб-сервиса!'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось получить подключение к веб-сервису!';ru='Не удалось получить подключение к веб-сервису!'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
...
Функция ВывестиМашиночитаемуюФормуЧерезВебСервис(ИмяФормыОтчета, НаименованиеИФНС, ФайлВыгрузкиВоВременномХранилище, ПараметрыСформированногоОтчета, АдресПараметровПроксиВоВременномХранилище) Экспорт
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.';ru='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
	...
	Если НЕ ЭтоАдресВременногоХранилища(АдресФайлаВыгрузкиВоВременномХранилище) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось получить файл выгрузки отчета на сервере!';ru='Не удалось получить файл выгрузки отчета на сервере!'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьМашиночитаемуюФормуСВебСервиса(ПараметрыСформированногоОтчета) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.';ru='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.'");
	...
	Если ОбъектОтчет.Метаданные().Макеты.Найти(ИмяМакета) <> Неопределено Тогда
		...
		Если КоличествоСтрок > 0 Тогда
			...
			Если ОбластьНастройки <> Неопределено Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ka='რეგლამენტირებული ანგარიშგებობა';ru='Регламентированная отчетность'"), УровеньЖурналаРегистрации.Ошибка, , , ПутьКФорме + Символы.ПС + ОписаниеОшибки());
				КонецПопытки;
				ЧтениеJSON.Закрыть();
			КонецЕсли;
		КонецЕсли;
	...
	Если ТипЗнч(СвойстваВидаОтчета) = Тип("Структура") Тогда
		Если СвойстваВидаОтчета.Свойство("ВерсияФормата") Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка регламентированной отчетности';ru='Загрузка регламентированной отчетности'"), УровеньЖурналаРегистрации.Ошибка, , , ИсточникОтчета + Символы.ПС + ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьСохраненныйДокумент(ДокументСсылка) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Ошибка при удалении документа: ""%1"".';ru='Ошибка при удалении документа: ""%1"".'"), Строка(ДокументСсылка));
		Сообщение.Сообщить();
	КонецПопытки;
КонецПроцедуры
...
Процедура ОтразитьВЖурналеОтчетов(ПараметрыОтчета) Экспорт
	...
	Исключение
		...
		Если ОшибкаУстановкиБлокировки Тогда
			Возврат;
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ka='Журнал отчетов (статусы): ошибка записи';ru='Журнал отчетов (статусы): ошибка записи'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ФП = "Ложь"
		   И ДатаНачалаОП = НачалоГода(ДатаНачалаОП) Тогда
			...
			Если СтрНайти(ПредставлениеФинПериода, "1 квартал") > 0 Тогда
				...
				ПредставлениеФинПериода = СтрЗаменить(ПредставлениеФинПериода, НСтр("ka='1 კვარტალი';ru='1 квартал'"), НСтр("ka='Январь - Март';ru='Январь - Март'"));
...
Функция ПроверитьИВыборочноОбновитьШаблоныМашиночитаемыхФормНаВебСервисе(ПрефиксИмениШаблона, ШаблоныМашиночитаемыхФорм, Прокси)
	...
	Если ШаблоныМашиночитаемыхФорм.Количество() > 0 Тогда
		...
		Если Прокси.UploadTemplates(WSСписокШаблонов) <> Истина Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Не удалось обновить шаблон машиночитаемой формы на веб-сервисе';ru='Не удалось обновить шаблон машиночитаемой формы на веб-сервисе'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
...
Функция ПроверитьКСНаСервере(УникальныйИдентификатор, АдресХранилища, ИдентификаторЗадания, РезультатПроверкиКС, АдресВозвращаемыхПараметровДляПроверкиВФоне) Экспорт 
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		НСтр("ka='Выполняется проверка контрольных соотношений. Пожалуйста, подождите...';ru='Выполняется проверка контрольных соотношений. Пожалуйста, подождите...'"));
