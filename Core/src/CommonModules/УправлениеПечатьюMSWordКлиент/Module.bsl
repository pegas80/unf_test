...
#КонецЕсли
	...
	Исключение
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		ВызватьИсключение(НСтр("ka='შეცდომა შაბლონის ფაილის გახსნისას.';ru='Ошибка при открытии файла шаблона.';en='Error when you open the template file.'") + Символы.ПС 
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ka='ბეჭდვა';ru='Печать';en='Print'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

Процедура НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке)
#Если ВебКлиент Тогда
	ТекстУточнения = НСтр("ka='ვებით მუშაობისას საჭიროა ბროუზერი Internet Explorer საოპერაციო სისტემა Windows-ის მართვით. იხ. აგრეთვე დოკუმენტაციის თავი ""ვებ-ბროუზერების გამართვა ვებ-კლიენტში სამუშაოდ""';ru='При работе через веб требуется браузер Internet Explorer под управлением операционной системы Windows. См. также главу документации ""Настройка веб-браузеров для работы в веб-клиенте""';en='When working through a web browser requires Internet Explorer running Windows operating system. See also chap. Documentation ""Setting up the Web browser for the Web client""'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось сформировать печатную форму: %1. "
"Для вывода печатных форм в формате Microsoft Word требуется, чтобы на компьютере был установлен пакет Microsoft Office. %2';ka='ბეჭდური ფორმის ჩამოყალიბება ვერ მოხერხდა: %1. Microsoft Word-ის ფორმატში ბეჭდური ფორმების გამოსატანდ საჭიროა, რომ კომპიუტერზე დაყენებული იყოს Microsoft Office-ის პაკეტი. %2'"),
