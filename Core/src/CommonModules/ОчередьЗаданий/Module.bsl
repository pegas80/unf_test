...
Функция ПолучитьЗадания(Знач Отбор) Экспорт
	...
	Если ПустаяСтрока(ТекстЗапроса) Тогда
		ВызватьИсключение НСтр("ka='შერჩევის არაკორექტული მნიშვნელობა - არცერთი ცნობარი არ არის აღმოჩენილი, რომლიდან დავალებები შერჩევის პირობებს დააკმაყოფილებს!';ru='Некорректное значение отбора - не обнаружено на одного справочника, задания из которого подходили бы под условия в отборе!';en='Incorrect selection of value - not found on a directory, the task of which would be suited to the conditions in the selection!'");
	КонецЕсли;
...
Функция ДобавитьЗадание(ПараметрыЗадания) Экспорт
	...
	Если НЕ ПараметрыЗадания.Свойство("ИмяМетода") Тогда
		ВызватьИсключение(НСтр("ka='დავალების სავალდებულო პარამეტრი არ არის მიცემული მეთოდისსახელი';ru='Не задан обязательный параметр задания ИмяМетода';en='Do not set a required parameter setting methodName'"));
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(Задание.Шаблон) Тогда
			...
			Для каждого КлючИЗначение Из ПараметрыЗадания Цикл
				...
				Если Не ОписаниеПараметра.Шаблон Тогда
					ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %1-ით რიგის დავალება შექმნილია შაბლონის საფუძველზე. დადგენილი შაბლონის დავალებების პარამეტრ %2-ის შეცვლა აკრძალულია.';ru='Задание очереди с идентификатором %1 создано на основе шаблона."
"Изменение параметра %2 заданий с установленным шаблоном запрещено.'");
	...
	Попытка
		...
		Если Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
			ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %1-ით დავალება შესაცვლელად ვერ მოიძებნა. მონაცემთა არეალი: %2';ru='Задание с идентификатором %1 к изменению не найдено. Область данных: %2';en='Task ID% 1 change not found. The data area: 2%'");
...
Процедура УдалитьЗадание(Идентификатор) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
				...
				ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
		Если ЗначениеЗаполнено(Задание.Шаблон) Тогда
			ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %1-ით რიგის დავალება შექმნილია შაბლონის საფუძველზე. დადგენილი შაბლონის დავალებების წაშლა აკრძალულია.';ru='Задание очереди с идентификатором %1 создано на основе шаблона."
"Удаление заданий с установленным шаблоном запрещено.'");
...
Функция ШаблонПоИмени(Знач Имя) Экспорт
	...
	Если Результат.Пустой() Тогда
		ШаблонСообщения = НСтр("ka='Не найден шаблон задания с именем %1';ru='Не найден шаблон задания с именем %1';en='Not found job template named% 1'");
...
Функция ПолучитьТекстИсключенияДублированиеЗаданийСОдинаковымКлючом() Экспорт
	...
	Возврат НСтр("ka=""ველ 'გასაღები'-ს ერთნაირი მნიშვნელობის დავალებების დუბლირება დაუშვებელია."";ru=""Дублирование заданий с одинаковым значения поля 'Ключ' не допустимо."";en=""Duplicating tasks with the same values ​​of the field 'key' is not admissible.""");
...
Процедура ПроверитьПараметрыЗадания(Параметры, Режим)
	...
	Если ТипЗнч(Параметры) <> Тип("Структура") Тогда
		ШаблонСообщения = НСтр("ka='Передан недопустимый тип набора параметров задания - %1';ru='Передан недопустимый тип набора параметров задания - %1';en='Transmitted invalid type set job parameters - 1%'");
	...
	Для каждого КлючИЗначение Из Параметры Цикл
		...
		Если ОписаниеПараметра = Неопределено 
			...
			ШаблонСообщения = НСтр("ka='Передан недопустимый параметр задания - %1';ru='Передан недопустимый параметр задания - %1';en='Transmitted invalid reference parameter - 1%'");
		...
		Если Отбор И ТипЗнч(КлючИЗначение.Значение) = Тип("Массив") Тогда
			...
			Для каждого ОписаниеОтбора Из КлючИЗначение.Значение Цикл
				Если ТипЗнч(ОписаниеОтбора) <> Тип("Структура") Тогда
					ШаблонСообщения = НСтр("ka='Передан недопустимый тип %1 в коллекции описания отбора %2';ru='Передан недопустимый тип %1 в коллекции описания отбора %2';en='Passed an invalid type% 1 in the selection of collection% 2'");
...
"Отсутствует свойство %2.';ka='Передано недопустимое описание отбора в коллекции описания отбора %1."
"Отсутствует свойство %2.'");
				...
				Если ВидыСравнения.Получить(ОписаниеОтбора.ВидСравнения) = Неопределено Тогда
					ШаблонСообщения = НСтр("ka='Передан недопустимый вид сравнения в описании отбора в коллекции описания отбора %1';ru='Передан недопустимый вид сравнения в описании отбора в коллекции описания отбора %1';en='Passed an invalid type of comparison in describing the selection in the selection of collection% 1'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КлючИЗначение.Ключ);
					ВызватьИсключение(ТекстСообщения);
				КонецЕсли;
...
"Для вида сравнения %3 ожидается тип Массив.';ka='Передан недопустимый тип %1 в описании отбора в коллекции описания отбора %2."
"Для вида сравнения %3 ожидается тип Массив.'");
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если Параметры.Свойство("ОбластьДанных") Тогда
			Если Параметры.ОбластьДанных <> ОбщегоНазначения.ЗначениеРазделителяСеанса() Тогда
				ВызватьИсключение(НСтр("ka='მოცემულ სეანსში დაუშვებელია მონაცემებთან მიმართვა მონაცემთა სხვა არეალიდან!';ru='В данном сеансе недопустимо обращение к данным из другой области данных!';en='This session is unacceptable accessing data from another area of ​​the data!'"));
			КонецЕсли;
		Иначе
	...
	Если Параметры.Свойство("ЗапланированныйМоментЗапуска")
		...
		ШаблонСообщения = НСтр("ka='Передано недопустимое значение %1 параметра задания %2';ru='Передано недопустимое значение %1 параметра задания %2';en='Transmitted invalid value assignment% 1% 2 Parameter'");
...
Процедура ПроверитьЗначениеНаСоответствиеОписаниюПараметра(Знач Значение, Знач ОписаниеПараметра)
	...
	Если НЕ ОписаниеПараметра.Тип.СодержитТип(ТипЗнч(Значение)) Тогда
		ШаблонСообщения = НСтр("ka='Передан недопустимый тип %1 параметра задания %2';ru='Передан недопустимый тип %1 параметра задания %2';en='Transmitted invalid type% 1% 2 reference parameter'");
...
Процедура ПроверитьНаличиеРегистрацииОбработчикаЗадания(Знач ИмяМетода)
	...
	Если ОчередьЗаданийСлужебныйПовтИсп.СоответствиеИменМетодовПсевдонимам().Получить(ВРег(ИмяМетода)) = Неопределено Тогда
		ШаблонСообщения = НСтр("ka='Не зарегистрирован псевдоним метода %1 для использования в качестве обработчика задания очереди.';ru='Не зарегистрирован псевдоним метода %1 для использования в качестве обработчика задания очереди.';en='Not registered alias method% 1 for use as a queue handler tasks.'");
...
Функция ОписаниеЗаданияПоИдентификатору(Знач Идентификатор)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) ИЛИ Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
		ШаблонСообщения = НСтр("ka='Передано недопустимое значение %1 параметра задания Идентификатор';ru='Передано недопустимое значение %1 параметра задания Идентификатор';en='Transmitted invalid value% 1 reference parameter identifier'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Идентификатор);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
	...
	Если Задания.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ka='რიგის დავალება იდენტიფიკატორ %1-ით ვერ მოიძებნა';ru='Задание очереди с идентификатором %1 не найдено';en='Queues ID% 1 found'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Идентификатор);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
...
Функция ПолучитьТекстИсключенияПолученияДанныхДругойОбласти()
	...
	Возврат НСтр("ka='მოცემულ სეანსში დაუშვებელია მონაცემებთან მიმართვა მონაცემთა სხვა არეალიდან!';ru='В данном сеансе недопустимо обращение к данным из другой области данных!';en='This session is unacceptable accessing data from another area of ​​the data!'");
