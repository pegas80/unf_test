...
Процедура ИзменитьСлужебногоПользователяОблачнаяТелефония(Включить = Ложь) Экспорт
	...
	Если ПользовательИзСправочника = Неопределено И Включить Тогда
		...
		ОписаниеПользователяИБ.ПолноеИмя = НСтр("ka='Служебный пользователь облачной телефонии';ru='Служебный пользователь облачной телефонии';en='Call a user of cloud telephony'");
...
Функция АдресОбратногоВызоваОблачнаяТелефония() Экспорт
	...
	Если ОбщиеНастройкиТелефонии.Свойство("АдресРесурсаАнонимнойПубликации")
		...
		ВызватьИсключение НСтр("ka='Необходимо заполнить параметры публикаций http-сервисов';ru='Необходимо заполнить параметры публикаций http-сервисов';en='It is necessary to fill the parameters of publications http-service'");
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='ტელეფონია';ru='Телефония';en='Telephony'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если Выборка.Следующий() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				...
				Выборка.Событие,
				НСтр("ka='Не удалось установить время завершения телефонного звонка. Объект заблокирован.';ru='Не удалось установить время завершения телефонного звонка. Объект заблокирован.';en='Failed to set the completion of the phone call. The object is locked.'")
			);
			Возврат;
		КонецПопытки;
