...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ФиксированныеПараметрыРезервногоКопированияИБ.ПроведеноВосстановление Тогда
		ТекстОповещения = НСтр("ka='მონაცემთა აღდგენა წარმატებით ჩატარდა.';ru='Восстановление данных проведено успешно.'");
		ПоказатьОповещениеПользователя(НСтр("ka='მონაცემები აღდგენილია.';ru='Данные восстановлены.'"), , ТекстОповещения);
	КонецЕсли;
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПараметрыПредупреждения.ТекстФлажка = НСтр("ka='სარეზერვო კოპირების შესრულება';ru='Выполнить резервное копирование'");
	ПараметрыПредупреждения.Приоритет = 50;
	ПараметрыПредупреждения.ТекстПредупреждения = НСтр("ka='Не выполнено резервное копирование при завершении работы.';ru='Не выполнено резервное копирование при завершении работы.'");
...
Процедура ПровестиРезервноеКопирование()
	...
	Кнопки.Добавить("Отложить", НСтр("ka='15 წუთით გადადება';ru='Отложить на 15 минут'"));
...
"Выполнить резервное копирование сейчас?'"),
		Кнопки, 30, "Да", НСтр("ka='სარეზერვო კოპირება განრიგის მიხედვით';ru='Резервное копирование по расписанию'"), "Да");
	...
	Если ПараметрыУдаления.ТипОграничения <> "ХранитьВсе" И КаталогХранения <> Неопределено Тогда
		...
		Исключение
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
				НСтр("ka='სარეზერვო ასლებიანი კატალოგის გასუფთავების გატარება ვერ მოხერხდა.';ru='Не удалось провести очистку каталога с резервными копиями.'") + Символы.ПС 
...
Процедура ПроверитьРезервноеКопированиеИБ(Параметры)
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование'"),
			"e1cib/command/ОбщаяКоманда.ПоказатьРезультатРезервногоКопирования",
			НСтр("ka='სარეზერვო კოპირება წარმატებით ჩატარდა';ru='Резервное копирование проведено успешно'"), БиблиотекаКартинок.Информация32);
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	Если ВариантОповещения = "Просрочено" Тогда
		...
		ТекстПояснения = НСтр("ka='ავტომატური სარეზერვო კოპირება არ შესრულდა';ru='Автоматическое резервное копирование не было выполнено.'"); 
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование'"),
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ТекстПояснения = НСтр("ka='რეკომენდებულია საინფორმაციო ბაზის სარეზერვო კოპირების გამართვა.';ru='Рекомендуется настроить резервное копирование информационной базы.'"); 
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ბაზის სარეზერვო კოპირება';ru='Резервное копирование информационной базы'",
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	#Если НЕ ВебКлиент Тогда
		...
		СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации", НСтр("ka='საინფორმაციო ბაზის სარეზერვო კოპირება';ru='Резервное копирование ИБ'"));
