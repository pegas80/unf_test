...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ФиксированныеПараметрыРезервногоКопированияИБ.ПроведеноВосстановление Тогда
		ТекстОповещения = НСтр("ka='მონაცემთა აღდგენა წარმატებით ჩატარდა.';ru='Восстановление данных проведено успешно.';en='Data recovery performed successfully.'");
		ПоказатьОповещениеПользователя(НСтр("ka='მონაცემები აღდგენილია.';ru='Данные восстановлены.';en='The data restored.'"), , ТекстОповещения);
	КонецЕсли;
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПараметрыПредупреждения.ТекстФлажка = НСтр("ka='სარეზერვო კოპირების შესრულება';ru='Выполнить резервное копирование';en='Run backups'");
	ПараметрыПредупреждения.Приоритет = 50;
	ПараметрыПредупреждения.ТекстПредупреждения = НСтр("ka='Не выполнено резервное копирование при завершении работы.';ru='Не выполнено резервное копирование при завершении работы.';en='Do not be backed up during shutdown.'");
...
Процедура ПровестиРезервноеКопирование()
	...
	Кнопки.Добавить("Отложить", НСтр("ka='15 წუთით გადადება';ru='Отложить на 15 минут';en='Postponed for 15 minutes'"));
	
	ОписаниеОповещение = Новый ОписаниеОповещения("ПровестиРезервноеКопированиеЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещение, НСтр("ka='განრიგის მიხედვით სარეზერვო კოპირების შესასრულებლად ყველაფერი მხად არის."
"ეხლა შევასრულოთ სარეზერვო კოპირება?';ru='Все готово для выполнения резервного копирования по расписанию."
"Выполнить резервное копирование сейчас?'"),
		Кнопки, 30, "Да", НСтр("ka='სარეზერვო კოპირება განრიგის მიხედვით';ru='Резервное копирование по расписанию';en='Scheduled Backup'"), "Да");
	...
	Если ПараметрыУдаления.ТипОграничения <> "ХранитьВсе" И КаталогХранения <> Неопределено Тогда
		...
		Исключение
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
				НСтр("ka='სარეზერვო ასლებიანი კატალოგის გასუფთავების გატარება ვერ მოხერხდა.';ru='Не удалось провести очистку каталога с резервными копиями.';en='Unable to carry out cleaning the directory with backups.'") + Символы.ПС 
...
Процедура ПроверитьРезервноеКопированиеИБ(Параметры)
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование';en='Backup'"),
			"e1cib/command/ОбщаяКоманда.ПоказатьРезультатРезервногоКопирования",
			НСтр("ka='სარეზერვო კოპირება წარმატებით ჩატარდა';ru='Резервное копирование проведено успешно';en='Backup successfully done'"), БиблиотекаКартинок.Информация32);
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	Если ВариантОповещения = "Просрочено" Тогда
		...
		ТекстПояснения = НСтр("ka='ავტომატური სარეზერვო კოპირება არ შესრულდა';ru='Автоматическое резервное копирование не было выполнено.';en='Automatic backup was not met.'"); 
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование';en='Backup'"),
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ТекстПояснения = НСтр("ka='რეკომენდებულია საინფორმაციო ბაზის სარეზერვო კოპირების გამართვა.';ru='Рекомендуется настроить резервное копирование информационной базы.';en='It is recommended to set up backup information base.'"); 
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование';en='Backup'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ბაზის სარეზერვო კოპირება';ru='Резервное копирование информационной базы';en='Backing up the knowledge base'",
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	#Если НЕ ВебКлиент Тогда
		...
		СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации", НСтр("ka='საინფორმაციო ბაზის სარეზერვო კოპირება';ru='Резервное копирование ИБ';en='Backup IB'"));
