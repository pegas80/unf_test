...
Функция ОписаниеРесурсаИнтернетПоддержки() Экспорт
	...
	Результат.Вставить("Описание", НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей';en='Internet-user support'"));
	Возврат Результат;
...
Функция ПредставлениеРазмераФайла(Знач Размер) Экспорт
	...
	Если Размер < 1024 Тогда
		Возврат Формат(Размер, "ЧДЦ=1") + " " + НСтр("ka='ბაიტი';ru='байт';en='byte'");
	ИначеЕсли Размер < 1024 * 1024 Тогда
		Возврат Формат(Размер / 1024, "ЧДЦ=1") + " " + НСтр("ka='კბ';ru='КБ';en='KB'");
	ИначеЕсли Размер < 1024 * 1024 * 1024 Тогда
		Возврат Формат(Размер / (1024 * 1024), "ЧДЦ=1") + " " + НСтр("ka='მბ';ru='МБ';en='MB'");
	Иначе
		Возврат Формат(Размер / (1024 * 1024 * 1024), "ЧДЦ=1") + " " + НСтр("ka='გბ';ru='ГБ';en='GB'");
	КонецЕсли;
		...
		Исключение
			УстановитьОписаниеОшибки(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Не удалось загрузить содержимое (%1). %2';ru='Не удалось загрузить содержимое (%1). %2';en='Failed to load the content (% 1). 2%'"),
					URL,
		...
		Если Ответ.КодСостояния = 301 // 301 Moved Permanently
			...
			Если КоличествоПеренаправлений > МаксКолвоПеренаправлений Тогда
				УстановитьОписаниеОшибки(
					Результат,
					"ServerError",
					НСтр("ka='გადამისამართების რაოდენობა გადამეტებულია.';ru='Превышено количество перенаправлений.';en='Exceeded the number of redirects.'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='სერვერის შეცდომა ფაილის მიღებისას (%1). გადამისამართების რაოდენობა გადაჭარბებულია (%2).';ru='Ошибка сервера при получении файла (%1). Превышено количество перенаправлений (%2).';en='Server Error retrieving file (% 1). Exceeded the number of redirections (2%).'"),
						URL,
			...
			Иначе
				...
				Если Location = Неопределено Тогда
					УстановитьОписаниеОшибки(
						Результат,
						"ServerError",
						НСтр("ka='არაკორექტული გადამისამართება.';ru='Некорректное перенаправление.';en='Incorrect redirection.'"),
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='სერვერის შეცდომა (%1) ფაილის მიღებისას (%2). არაკორექტული გადამისამართება, პასუხ ""Location""-ის HTTP-სათაური არ არის.';ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, отсутствует HTTP-заголовок ответа ""Location"".';en='Server Error (% 1) when retrieving a file (2%). Incorrect redirect, no HTTP-header ""Location"" response.'"),
							Ответ.КодСостояния,
							URL),
						Перенаправления);
					Возврат Результат;
				Иначе
					...
					Если ПустаяСтрока(Location) Тогда
						УстановитьОписаниеОшибки(
							...
							НСтр("ka='არაკორექტული გადამისამართება.';ru='Некорректное перенаправление.';en='Incorrect redirection.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='სერვერის შეცდომა (%1) ფაილის მიღებისას (%2). არაკორექტული გადამისამართება, პასუხის ცარიელი HTTP-სათაური ""Location"".';ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, пустой HTTP-заголовок ответа ""Location"".';en='Server Error (% 1) when retrieving a file (2%). Incorrect redirect, an empty HTTP-response header ""Location"".'"),
								Ответ.КодСостояния,
								URL),
							Перенаправления);
						Возврат Результат;
					...
					Если ВыполненныеПеренаправления.Получить(Location) <> Неопределено Тогда
						УстановитьОписаниеОшибки(
							Результат,
							"ServerError",
							НСтр("ka='ციკლური გადამიმართება.';ru='Циклическое перенаправление.';en='Cyclic redirection.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='სერვერის შეცდომა (%1) ფაილის მიღებისას (%2). ციკლური გადამისამართება (%3).';ru='Ошибка сервера (%1) при получении файла (%2). Циклическое перенаправление (%3).';en='Server Error (% 1) when retrieving a file (2%). Cyclic redirection (3%).'"),
								Ответ.КодСостояния,
								URL,
								Location),
							Перенаправления);
	...
	Если Ответ.КодСостояния < 200 Или Ответ.КодСостояния >= 300 Тогда
		...
		Если Ответ.КодСостояния = 407 Тогда
			...
			УстановитьОписаниеОшибки(
				Результат,
				"ConnectError",
				НСтр("ka='პროქსი-სერვერზე ავთენტიფიკაციის შეცდომა.';ru='Ошибка аутентификации на прокси-сервере.';en='Authentication failed on the proxy server.'"),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка клиента (%1) при выполнении запроса к ресурсу (%2)."
"Тело ответа: %3';ka='კლიენტის შეცდომა (%1) რესურსისთვის მოთხოვნის შესრულებისას (%2)."
"პასუხის სხეული: %3'"),
					Ответ.КодСостояния,
					URL,
		...
		ИначеЕсли Ответ.КодСостояния < 200
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Некорректный ответ сервера (%1).';ru='Некорректный ответ сервера (%1).';en='Incorrect server response (1%).'"),
					Ответ.КодСостояния),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка сервера при получении файла (%1). Некорректный (неподдерживаемый) ответ (%2)."
"Тело ответа: %3';ka='სერვერის შეცდომა ფაილის მიღებისას (%1). არაკორექტული (არამხარდაჭერილი) პასუხი (%2)."
"პასუხის სხეული: %3'"),
					URL,
					Ответ.КодСостояния,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
		...
		ИначеЕсли Ответ.КодСостояния >= 400 И Ответ.КодСостояния < 500 Тогда
			...
			УстановитьОписаниеОшибки(
				...
				"ClientError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Ошибка (%1) при выполнении запроса к ресурсу.';ru='Ошибка (%1) при выполнении запроса к ресурсу.';en='Error (1%) when the request to the resource.'"),
...
"პასუხის სხეული: %3'"),
		...
		Иначе
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Код ошибки: %1.';ru='Код ошибки: %1.';en='Error code:% 1.'"),
					Строка(Ответ.КодСостояния)),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка сервера (%1) при обработке запроса к ресурсу (%2)."
"Тело ответа: %3';ka='სერვერის შეცდომა (%1) რესურსისადმი მოთხოვნის დამუშავებისას (%2)."
"პასუხის სხეული: %3'"),
					Ответ.КодСостояния,
					URL,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
...
Функция ПользовательскоеПредставлениеИнтервалаВерсий(ВерсияОТ, ВерсияДО) Экспорт
	...
	Если (ВерсияОТ = "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда
		Результат = НСтр("ka='ნებისმიერი ვერსია';ru='Любая версия';en='Any version'");
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда
		...
		Иначе // Интервал версий
			Результат = СтрШаблон(
				НСтр("ka='ინტერვალი  %1...%2';ru='Интервал %1...%2';en='Interval 1% ... 2%'"),
	...
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда // От версии и выше
		Результат = СтрШаблон(
			НСтр("ka='От %1 и выше';ru='От %1 и выше';en='From 1% and above'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияОТ, Истина));
	ИначеЕсли (ВерсияОТ = "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда // До версии включительно
		Результат = СтрШаблон(
			НСтр("ka='До версии %1 включительно';ru='До версии %1 включительно';en='Prior to version% 1 inclusive'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияДО, Истина));
...
Функция ПредставлениеДняНедели(НомерДняНедели) Экспорт
	...
	Результат = СтрШаблон(
		НСтр("ka='Неопределено (%1)';ru='Неопределено (%1)';en='Undefined (1%)'"),
		НомерДняНедели);

	Если НомерДняНедели = 1 Тогда
		Результат = НСтр("ka='ორშაბათი';ru='Понедельник';en='Monday'");
	ИначеЕсли НомерДняНедели = 2 Тогда
		Результат = НСтр("ka='სამშაბათი';ru='Вторник';en='Tuesday'");
	ИначеЕсли НомерДняНедели = 3 Тогда
		Результат = НСтр("ka='ოთხშაბათი';ru='Среда';en='Wednesday'");
	ИначеЕсли НомерДняНедели = 4 Тогда
		Результат = НСтр("ka='ხუთშაბათი';ru='Четверг';en='Thursday'");
	ИначеЕсли НомерДняНедели = 5 Тогда
		Результат = НСтр("ka='პარასკევი';ru='Пятница';en='Friday'");
	ИначеЕсли НомерДняНедели = 6 Тогда
		Результат = НСтр("ka='შაბათი';ru='Суббота';en='Saturday'");
	ИначеЕсли НомерДняНедели = 7 Тогда
		Результат = НСтр("ka='კვირა დღე';ru='Воскресенье';en='Sunday'");
	КонецЕсли;
...
"%1';ka='Ошибка при получении xml-представления объекта ХДТО:"
"%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:БазовыйФункционал.Отладка';ru='БИП:БазовыйФункционал.Отладка';en='BIP: BazovyyFunktsional.Otladka'"), // ИмяСобытия.
				НСтр("ka='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';ru='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';en='BazovyyFunktsional. Debugging. PredstavlenieObektaHDTO'"), // ИдентификаторШага.
...
"%2';ka='При чтении файла с именем %1 произошла ошибка:"
"%2'"),
			ПолноеИмяФайла,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ПроверитьЛогинИПароль(Логин, Пароль, Знач НастройкиСоединенияССерверами) Экспорт
	...
	ИначеЕсли РезультатПолученияФайла.КодСостояния = 403 Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ka='არასწორი ლოგ-ინი ან პაროლი.';ru='Неверный логин или пароль.';en='Wrong login or password.'");
...
"Повторите попытку позже.';ka='ლოგ-ინის და პაროლის შეყვანის მცდელობის რაოდენობა გადაჭარბებულია. მოგვიანებით სცადეთ.'");

	Иначе

		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვის %1-ში ლოგ-ინის და პაროლის შემოწმება ვერ მოხერხდა. %2';ru='Не удалось проверить логин и пароль в сервисе %1. %2';en='Failed to check the username and password when 1%. 2%'"),
				URLСервиса,
...
Функция СлужебнаяТикетАутентификации(
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა ავთენტიფიკაციის სერვისზე შეერთებისას.';ru='Ошибка при подключении к сервису аутентификации.';en='Error connecting to the authentication service.'")
		...
		Результат.СообщениеОбОшибке = НСтр("ka='შეცდომა სერვისთან შეერთებისას.';ru='Ошибка при подключении к сервису.';en='Error connecting to the service.'")
			+ Символы.ПС
			+ ПроксиСервиса.СообщениеОбОшибке
			+ Символы.ПС
			+ НСтр("ka='დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Подробнее см. в журнале регистрации.';en='For details, see. In the log.'");
		Возврат Результат;
	...
	Иначе
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა ავთენტიფიკაციის ტიკეტის მიღებისას.';ru='Ошибка при получении тикета аутентификации.';en='Error obtaining authentication ticket.'")
...
Функция СервисАутентификации_getTicket(
	...
	Если Не ПустаяСтрока(Результат.КодОшибки) Тогда
		...
		Если СтрНайти(Результат.ИнформацияОбОшибке, "IncorrectLoginOrPasswordApiException") > 0 Тогда
			Результат.КодОшибки         = "НеверныйЛогинИлиПароль";
			Результат.ИнформацияОбОшибке = НСтр("ka='არასწორი ლოგ-ინი ან პაროლი (IncorrectLoginOrPasswordApiException).';ru='Неверный логин или пароль (IncorrectLoginOrPasswordApiException).';en='Invalid username or password (IncorrectLoginOrPasswordApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ka='არასწორი ლოგ-ინი ან პაროლი.';ru='Неверный логин или пароль.';en='Wrong login or password.'");

		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TooManyFailedAttemptsApiException") > 0 Тогда
			Результат.КодОшибки = "ПревышеноКоличествоПопыток";
			Результат.ИнформацияОбОшибке =
				НСтр("ka='ავთენტიფიკაციის მცდელობის რაოდენობა გადაჭარბებულია (TooManyFailedAttemptsApiException).';ru='Превышено количество попыток аутентификации (TooManyFailedAttemptsApiException).';en='Exceeded the number of authentication attempts (TooManyFailedAttemptsApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ka='ავთენტიფიკაციის მცდელობის რაოდენობა გადაჭარბებულია.';ru='Превышено количество попыток аутентификации.';en='Exceeded the number of authentication attempts.'");

		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TicketApiException") > 0 Тогда
			Результат.КодОшибки = "ОшибкаСервиса";
			Результат.ИнформацияОбОшибке = НСтр("ka='ავთენტიფიკაციის სერვისის უცნობი შეცდომა (TicketApiException).';ru='Неизвестная ошибка сервиса аутентификации (TicketApiException).';en='Unknown error authentication service (TicketApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ka='სერვისის შიდა შეცდომა.';ru='Внутренняя ошибка сервиса.';en='The internal service error.'")
	...
	Иначе
		...
		Если ОписаниеОтветаSOAP.Ошибка Тогда
			...
			Результат.СообщениеОбОшибке = НСтр("ka='სერვისის შეცდომა.';ru='Ошибка сервиса.';en='service error.'")
				+ Символы.ПС + ОписаниеОтветаSOAP.СообщениеОбОшибке;
			Результат.ИнформацияОбОшибке =
				НСтр("ka='ავთენტიფიკაციის სერვისის ოპერაციის getTicket() მოხმობის შეცდომა.';ru='Ошибка вызова операции getTicket() сервиса аутентификации.';en='The call getTicket operation () authentication service.'") + " "
				+ ОписаниеОтветаSOAP.ИнформацияОбОшибке
				+ Символы.ПС + НСтр("ka='პასუხის ტანი:';ru='Тело ответа:';en='the response body:'") + " " + Лев(ОписаниеОтвета.ТелоОтвета, 1024 * 5);
			Возврат Результат;
		КонецЕсли;
...
"%1';ka='Не удалось отправить данные сообщения в службу технической поддержки."
"Не удалось проверить доступность сервиса отправки сообщений."
"%1'"),
				РезультатПроверки.ИнформацияОбОшибке));
...
"%1'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки."
"%1';ka='ტექნიკური მხარდაჭერის სამსახურში შეტყობინების მონაცემთა გაგზავნა ვერ მოხერხდა."
"%1'"),
...
"%1"
"Тело ответа: %2';ka='ტექნიკური მხარდაჭერის სამსახურში შეტყობინების მონაცემთა გაგზავნა ვერ მოხერხდა."
"შეცდომა სერვისის პასუხის დამუშავებისას."
"შეტყობინებების გაგზავნის სერვისის არაკორექტული პასუხი."
"%1"
"პასუხის სხეული: %2'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Некорректный ответ сервиса отправки сообщений.';ka='ტექნიკური მხარდაჭერის სამსახურში შეტყობინების მონაცემთა გაგზავნა ვერ მოხერხდა."
"შეტყობინებების გაგზავნის სერვისის არაკორექტული პასუხი.'");
...
"Ответ сервиса: %1';ka='შეტყობინების მონაცემთა ტექნიკურ მხარდაჭერაში გაგზავნა ვერ მოხერხდა."
"შეცდომა ტექ. მხარდაჭერაში შეტყობინების გადაცემისას."
"სერვისმა შეგვატყობინა შეცდომის შესახებ."
"სერვისის პასუხი: %1'"),
			Лев(РезультатОтправки.Содержимое, 1024)));
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Ошибка при передаче сообщения в тех. поддержку."
"Подробнее см. в журнале регистрации.';ka='შეტყობინების მონაცემთა გაგზავნა ტექნიკური მხარდაჭერის სამსახურში ვერ მოხერხდა."
"შეცდომა ტექ. მხარდაჭერაში შეტყობინების გადაცემისას."
"დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.'");
...
"Подробнее см. в журнале регистрации.';ka='1C:სტთ-ის პორტალზე შესვლის შეცდომა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.'");
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьОписаниеОшибки(Результат, КодОшибки, СообщениеОбОшибке, ИнформацияОбОшибке, Перенаправления)
	...
	Если КодОшибки = "ConnectError" Тогда
		ДопСообщение = НСтр("ka='Ошибка при подключении к серверу.';ru='Ошибка при подключении к серверу.';en='Error connecting to the server.'");

	ИначеЕсли КодОшибки = "ServerError" Тогда
		ДопСообщение = НСтр("ka='На сервере возникла внутренняя ошибка при обработке запроса.';ru='На сервере возникла внутренняя ошибка при обработке запроса.';en='On the server encountered an internal error while processing your request.'");

	ИначеЕсли КодОшибки = "ClientError" Тогда
		ДопСообщение = НСтр("ka='არაკორექტული მოთხოვნა.';ru='Некорректный запрос.';en='Invalid request.'");

	ИначеЕсли КодОшибки = "InternalError" Тогда
		ДопСообщение = НСтр("ka='შიდა შეცდომა.';ru='Внутренняя ошибка.';en='Internal error.'");

	ИначеЕсли КодОшибки = "LoginError" Тогда
		ДопСообщение = НСтр("ka='სერვერზე ავთენტიფიკაციის შეცდომა.';ru='Ошибка аутентификации на сервере.';en='Authentication failed on the server.'");
	...
	Если Перенаправления.Количество() > 0 Тогда
		Результат.ИнформацияОбОшибке = Результат.ИнформацияОбОшибке + Символы.ПС
			+ НСтр("ka='გადამისამართებები:';ru='Перенаправления:';en='redirects:'") + Символы.ПС
...
Функция НовыйОпределенияСервиса(
	...
	Если НЕ ПустаяСтрока(ОписаниеWSDL.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა WSDL-აღწერილობის მიღებისას.';ru='Ошибка при получении WSDL-описания.';en='Error retrieving WSDL-description.'") + " "
	...
	Исключение
		...
		Результат.КодОшибки          = "ServerError";
		Результат.СообщениеОбОшибке  = НСтр("ka='სერვისის არაკორექტული WSDL-აღწერილობა.';ru='Некорректное WSDL-описание сервиса.';en='Incorrect WSDL-description service.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა WSDL-აღწერილობის დამუშავებისას %1. %2';ru='Ошибка при обработке WSDL-описания %1. %2';en='Error processing WSDL-description% 1. 2%'"),
				МестоположениеWSDL,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если УзлыДирективыИмпорта.Количество() > 0 Тогда
		...
		Для каждого УзелИмпорта Из УзлыДирективыИмпорта Цикл
			...
			Если ПустаяСтрока(Location) Тогда
				...
				Результат.СообщениеОбОшибке  = НСтр("ka='სერვისის არაკორექტული WSDL-აღწერილობა.';ru='Некорректное WSDL-описание сервиса.';en='Incorrect WSDL-description service.'");
				Результат.ИнформацияОбОшибке =
					НСтр("ka='დირექტივა import-ისთვის location-ი არ არის.';ru='Отсутствует location для директивы import.';en='No location for the directive import.'");
				Возврат Результат;
			КонецЕсли;
	...
	Для каждого УзелТипов Из УзлыОписанияТипов Цикл
		...
		Для каждого УзелСхемы Из УзлыСхем Цикл
			...
			Исключение
				...
				Результат.ИнформацияОбОшибке =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='შეცდომა WSDL-აღწერილობის დამუშავებისას %1. მონაცემთა სქემის აგების შეცდომა. %2';ru='Ошибка при обработке WSDL-описания %1. Ошибка построения схемы данных. %2';en='Error processing WSDL-description% 1. Error building data schema. 2%'"),
						МестоположениеWSDL,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат Результат;
	...
	Иначе
		...
		Иначе
			...
			Исключение
				...
				Результат.ИнформацияОбОшибке =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='შეცდომა WSDL-აღწერილობა %1-ის დამუშავებისას. შეცდომა სერვისის ფაბრიკის შექმნისას. %2';ru='Ошибка при обработке WSDL-описания %1. Ошибка при создании фабрики XDTO сервиса. %2';en='Error processing WSDL-description% 1. Error creating factory XDTO service. 2%'"),
						МестоположениеWSDL,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат Результат;
...
Функция НовыйПроксиВебСервиса(
	...
	Если НЕ ПустаяСтрока(ОпределениеСервиса.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა ვებ-სერვისის აღწერილობის შექმნისას.';ru='Ошибка при создании описания веб-сервиса.';en='Error creating describe a web service.'") + " "
	...
	Если НайденныйПорт = Неопределено Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ka='სერვისის განსაზღვრება ვერ მოიძებნა.';ru='Не найдено определение сервиса.';en='We found the service definition.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при создании описания веб-сервиса %1. Определение сервиса не найдено."
"URI пространства имен: %2;"
"Имя сервиса: %3;"
"Имя точки подключения: %4';ka='შეცდომა ვებ-სერვის %1-ის აღწერილობის შექმნისას. სერვისის განსაზღვრება ვერ მოიძებნა."
"სახელების სივრცის URI: %2;"
"სერვისის სახელი: %3;"
"შეერთების წერტილის სახელი: %4'"),
				МестоположениеWSDL,
				URIПространстваИмен,
				ИмяСервиса,
				ИмяТочкиПодключения);
...
Процедура СлужебнаяПроверитьURLДоступен(
	...
	Исключение
		ИмяОшибки = "Unknown";
		СообщениеОбОшибке = НСтр("ka='უცნობი შეცდომა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.';en='Unknown error. For details, see. In the log.'");
...
"%1';ka='Неизвестная ошибка при проверке доступности URL."
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	...
	Если Результат = Неопределено Тогда
		...
		Если ПараметрыИнтернетПоддержки.РаботаВМоделиСервиса Тогда
			...
			Результат = Новый Структура("Действие,Сообщение",
				"ПоказатьСообщение",
				НСтр("ka='სერვისის მოდელში მუშაობისას მომხმარებლების ინტერნეტ-მხარდაჭერის გამოყენება მიუწვდომელია.';ru='Использование Интернет-поддержки пользователей недоступно при работе в модели сервиса.';en='Using Internet-user support is not available when working in the service model.'"));

		ИначеЕсли НЕ ПараметрыИнтернетПоддержки.ЗапускРазрешен Тогда

			Результат = Новый Структура("Действие,Сообщение",
				"ПоказатьСообщение",
				НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერის გასაშვებად არასაკმარისი უფლებებია. მიმართეთ ადმინისტრატორს.';ru='Недостаточно прав для запуска Интернет-поддержки пользователей. Обратитесь к администратору.';en='Enough rights to launch Internet-user support. Contact your administrator.'"));
...
Функция НовыйОписаниеСервисаИПП(МестоположениеWSDL = Неопределено, Таймаут = -1) Экспорт
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		ТекстИсключения = НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვისზე შეერთებისას.';ru='Ошибка при подключении к сервису Интернет-поддержки.';en='Error connecting to the service online support.'") + " "
			+ ПроксиСервиса.ИнформацияОбОшибке;
...
"Не удалось определить тип корневого свойства isConfigurationSupported.';ka='შეცდომა სერვისის ოპერაცია isConfigurationSupported-ის მოხმობისას (%1)."
"ძირეული თვისების ტიპის დადგენა ვერ მოხერხდა isConfigurationSupported.'"),
			"%1",
	...
	Если НЕ ПустаяСтрока(ОписаниеОтветаSOAP.КодОшибки) Тогда
		ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომა სერვისის ოპერაცია isConfigurationSupported-ის მოხმობისას (%1):';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1):';en='Error isConfigurationSupported service call operation (1%):'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ " " + ОписаниеОтветаSOAP.ИнформацияОбОшибке;
		ВызватьИсключение ТекстИсключения;
	...
	Если ТипОбъекта = Неопределено Тогда
		ТекстИсключения = СтрЗаменить(НСтр("ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1)."
"Не удалось определить тип корневого свойства isConfigurationSupportedResponse.';ka='შეცდომა სერვისის ოპერაცია isConfigurationSupported-ის მოხმობისას (%1)."
"ძირეული თვისების ტიპის დადგენა ვერ მოხერხდა isConfigurationSupportedResponse.'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ОписаниеОтветаSOAP.Ошибка Тогда
		...
		ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომეა სერვისის ოპერაცია isConfigurationSupported-ის მოხმობისას (%1).';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1).';en='Error isConfigurationSupported service call operation (1%).'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ Символы.ПС
			+ ОписаниеОтветаSOAP.ИнформацияОбОшибке
			+ Символы.ПС + Символы.ПС
			+ НСтр("ka='მოთხოვნის ტანი:';ru='Тело запроса:';en='Request body:'")
			+ Символы.ПС
			+ Лев(ТекстКонверта, 1024 * 5)
			+ НСтр("ka='პასუხის ტანი:';ru='Тело ответа:';en='the response body:'")
			+ Символы.ПС
...
Функция СервисИПП_process(ПараметрыЗапроса, ОписаниеСервисаИПП, Таймаут = -1)
	...
	Если НЕ ПустаяСтрока(ОписаниеОтветаSOAP.КодОшибки) Тогда
		ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომა სერვისის ოპერაცია process-ის მოხმობისას (%1):';ru='Ошибка при вызове операции process сервиса (%1):';en='Error process service call operation (1%):'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ " " + ОписаниеОтветаSOAP.ИнформацияОбОшибке;
		ВызватьИсключение ТекстИсключения;
...
"Не удалось определить тип корневого свойства processResponse.';ka='შეცდომა სერვისის ოპერაცია process-ის მოხმობისას (%1)."
"ძირეული თვისების ტიპის დადგენა ვერ მოხერხდა processResponse.'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Функция ПрочитатьОтветВКонвертеSOAP(ТелоОтвета, ОписаниеСервисаИПП, ТипЗначения)
	...
	Исключение
		...
		Результат.СообщениеОбОшибке  = НСтр("ka='პასუხის არასწორი ფორმატი.';ru='Неверный формат ответа.';en='Invalid response format.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პასუხის არასწორი ფორმატი SOAP. %1';ru='Неверный формат ответа SOAP. %1';en='Invalid SOAP response format. %one'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если ЧтениеОтвета.ТипУзла = ТипУзлаXML.НачалоЭлемента
		...
		Результат.Ошибка = Истина;
		Результат.СообщениеОбОшибке  = НСтр("ka='SOAP-სერვერის შეცდომა.';ru='Ошибка SOAP-сервера.';en='SOAP-server error.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='SOAP-სერვერის შეცდომა. %1';ru='Ошибка SOAP-сервера. %1';en='SOAP-server error. %one'"),
...
Функция ОписаниеИсключенияSOAPВСтроку(ИсключениеSOAP)
	...
	Если НЕ ПустаяСтрока(ИсключениеSOAP.FaultActor) Тогда
		Результат = Результат + ?(ПустаяСтрока(Результат), "", Символы.ПС + НСтр("ka='შეცდომის წყარო:';ru='Источник ошибки:';en='Error Source:'") + " ")
...
Функция СтруктурироватьОтветСервера(
	...
	Исключение
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ka='უცნობი შეცდომა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Неизвестная ошибка. См. подробности в журнале регистрации.';en='Unknown error. See. Details in the log.'");
...
Функция СтруктурироватьСообщениеИлиВопросПользователю(КомандаСервера)
	...
	Если НЕ СтруктураКоманды.Свойство("Заголовок") ИЛИ ПустаяСтрока(СтруктураКоманды.Заголовок) Тогда
		СтруктураКоманды.Вставить("Заголовок", НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей';en='Internet-user support'"));
	КонецЕсли;
...
Функция ПараметрыВнутреннейФормы(ИндексФормы, МестоЗапуска)
	...
	ИначеЕсли ИндексФормы = "bh1" Тогда
		ПараметрыФормы.Вставить("Заголовок", НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей';en='Internet-user support'"));
...
Функция СтруктурироватьЗаписьАдресногоКлассификатора(КомандаСервера)
		...
		Для каждого Параметр Из ПараметрыСписокСтран Цикл
			...
			Если ИмяТекПараметра = "country" Тогда
				...
				Если ИдентификаторСтраны <> Неопределено Тогда
					...
					СписокРегионовСтраны.Вставить(0, "-1", НСтр("ka='<არ არის არჩეული>';ru='<не выбран>';en='<Not Set>'"));
	...
	СписокСтран.Вставить(0, "-1", НСтр("ka='<არ არის არჩეული>';ru='<не выбрана>';en='<Not selected>'"));
...
Функция СтруктурироватьЗаписьОрганизацийПользователя(КомандаСервера)
	...
	СписокОрганизаций.Вставить(0, "-1", НСтр("ka='<добавить новую организацию>';ru='<добавить новую организацию>';en='<Add a new organization>'"));
...
Процедура ДобавитьКомандыСервиса(
	...
	Если ПередаваемыеПараметрыЗапроса <> Неопределено Тогда
		...
		Для каждого ПередаваемыйПараметр Из ПередаваемыеПараметрыЗапроса Цикл
			...
			Иначе
				...
				Исключение
					ИнфОшибка = ИнформацияОбОшибке();
					ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომა გადასაცემ მონაცემთა გარდაქმნისას. %1';ru='Ошибка при преобразовании передаваемых данных. %1';en='Error converting data transmitted. %one'"),
						"%1",
						ПодробноеПредставлениеОшибки(ИнфОшибка));
					ВызватьИсключение ТекстИсключения;
				КонецПопытки;
	...
	Если МассивСтруктурыКоманд = Неопределено ИЛИ МассивСтруктурыКоманд.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='სერვერის ცარიელი პასუხი.';ru='Пустой ответ сервера.';en='Empty server response.'");
	КонецЕсли;
