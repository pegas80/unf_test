...
Функция ОписаниеРесурсаИнтернетПоддержки() Экспорт
	...
	Результат.Вставить("Описание", НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей'"));
	Возврат Результат;
...
Функция ПредставлениеРазмераФайла(Знач Размер) Экспорт
	...
	Если Размер < 1024 Тогда
		Возврат Формат(Размер, "ЧДЦ=1") + " " + НСтр("ka='ბაიტი';ru='байт'");
	ИначеЕсли Размер < 1024 * 1024 Тогда
		Возврат Формат(Размер / 1024, "ЧДЦ=1") + " " + НСтр("ka='კბ';ru='КБ'");
	ИначеЕсли Размер < 1024 * 1024 * 1024 Тогда
		Возврат Формат(Размер / (1024 * 1024), "ЧДЦ=1") + " " + НСтр("ka='მბ';ru='МБ'");
	Иначе
		Возврат Формат(Размер / (1024 * 1024 * 1024), "ЧДЦ=1") + " " + НСтр("ka='გბ';ru='ГБ'");
	КонецЕсли;
		...
		Исключение
			УстановитьОписаниеОшибки(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Не удалось загрузить содержимое (%1). %2';ru='Не удалось загрузить содержимое (%1). %2'"),
					URL,
		...
		Если Ответ.КодСостояния = 301 // 301 Moved Permanently
			...
			Если КоличествоПеренаправлений > МаксКолвоПеренаправлений Тогда
				УстановитьОписаниеОшибки(
					Результат,
					"ServerError",
					НСтр("ka='გადამისამართების რაოდენობა გადამეტებულია.';ru='Превышено количество перенаправлений.'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='სერვერის შეცდომა ფაილის მიღებისას (%1). გადამისამართების რაოდენობა გადაჭარბებულია (%2).';ru='Ошибка сервера при получении файла (%1). Превышено количество перенаправлений (%2).'"),
						URL,
			...
			Иначе
				...
				Если Location = Неопределено Тогда
					УстановитьОписаниеОшибки(
						Результат,
						"ServerError",
						НСтр("ka='არაკორექტული გადამისამართება.';ru='Некорректное перенаправление.'"),
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='სერვერის შეცდომა (%1) ფაილის მიღებისას (%2). არაკორექტული გადამისამართება, პასუხ ""Location""-ის HTTP-სათაური არ არის.';ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, отсутствует HTTP-заголовок ответа ""Location"".'"),
							Ответ.КодСостояния,
							URL),
						Перенаправления);
					Возврат Результат;
				Иначе
					...
					Если ПустаяСтрока(Location) Тогда
						УстановитьОписаниеОшибки(
							...
							НСтр("ka='არაკორექტული გადამისამართება.';ru='Некорректное перенаправление.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='სერვერის შეცდომა (%1) ფაილის მიღებისას (%2). არაკორექტული გადამისამართება, პასუხის ცარიელი HTTP-სათაური ""Location"".';ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, пустой HTTP-заголовок ответа ""Location"".'"),
								Ответ.КодСостояния,
								URL),
							Перенаправления);
						Возврат Результат;
					...
					Если ВыполненныеПеренаправления.Получить(Location) <> Неопределено Тогда
						УстановитьОписаниеОшибки(
							Результат,
							"ServerError",
							НСтр("ka='ციკლური გადამიმართება.';ru='Циклическое перенаправление.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='სერვერის შეცდომა (%1) ფაილის მიღებისას (%2). ციკლური გადამისამართება (%3).';ru='Ошибка сервера (%1) при получении файла (%2). Циклическое перенаправление (%3).'"),
								Ответ.КодСостояния,
								URL,
								Location),
							Перенаправления);
	...
	Если Ответ.КодСостояния < 200 Или Ответ.КодСостояния >= 300 Тогда
		...
		Если Ответ.КодСостояния = 407 Тогда
			...
			УстановитьОписаниеОшибки(
				Результат,
				"ConnectError",
				НСтр("ka='პროქსი-სერვერზე ავთენტიფიკაციის შეცდომა.';ru='Ошибка аутентификации на прокси-сервере.'"),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"პასუხის სხეული: %3'"),
		...
		ИначеЕсли Ответ.КодСостояния < 200
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Некорректный ответ сервера (%1).';ru='Некорректный ответ сервера (%1).'"),
					Ответ.КодСостояния),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"პასუხის სხეული: %3'"),
		...
		ИначеЕсли Ответ.КодСостояния >= 400 И Ответ.КодСостояния < 500 Тогда
			...
			УстановитьОписаниеОшибки(
				...
				"ClientError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Ошибка (%1) при выполнении запроса к ресурсу.';ru='Ошибка (%1) при выполнении запроса к ресурсу.'"),
...
"პასუხის სხეული: %3'"),
		...
		Иначе
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Код ошибки: %1.';ru='Код ошибки: %1.'"),
					Строка(Ответ.КодСостояния)),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПользовательскоеПредставлениеИнтервалаВерсий(ВерсияОТ, ВерсияДО) Экспорт
	...
	Если (ВерсияОТ = "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда
		Результат = НСтр("ka='ნებისმიერი ვერსია';ru='Любая версия'");
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда
		...
		Иначе // Интервал версий
			Результат = СтрШаблон(
				НСтр("ka='ინტერვალი  %1...%2';ru='Интервал %1...%2'"),
	...
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда // От версии и выше
		Результат = СтрШаблон(
			НСтр("ka='От %1 и выше';ru='От %1 и выше'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияОТ, Истина));
	ИначеЕсли (ВерсияОТ = "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда // До версии включительно
		Результат = СтрШаблон(
			НСтр("ka='До версии %1 включительно';ru='До версии %1 включительно'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияДО, Истина));
...
Функция ПредставлениеДняНедели(НомерДняНедели) Экспорт
	...
	Результат = СтрШаблон(
		НСтр("ka='Неопределено (%1)';ru='Неопределено (%1)'"),
		НомерДняНедели);

	Если НомерДняНедели = 1 Тогда
		Результат = НСтр("ka='ორშაბათი';ru='Понедельник'");
	ИначеЕсли НомерДняНедели = 2 Тогда
		Результат = НСтр("ka='სამშაბათი';ru='Вторник'");
	ИначеЕсли НомерДняНедели = 3 Тогда
		Результат = НСтр("ka='ოთხშაბათი';ru='Среда'");
	ИначеЕсли НомерДняНедели = 4 Тогда
		Результат = НСтр("ka='ხუთშაბათი';ru='Четверг'");
	ИначеЕсли НомерДняНедели = 5 Тогда
		Результат = НСтр("ka='პარასკევი';ru='Пятница'");
	ИначеЕсли НомерДняНедели = 6 Тогда
		Результат = НСтр("ka='შაბათი';ru='Суббота'");
	ИначеЕсли НомерДняНедели = 7 Тогда
		Результат = НСтр("ka='კვირა დღე';ru='Воскресенье'");
	КонецЕсли;
...
"%1'"),
			...
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:БазовыйФункционал.Отладка';ru='БИП:БазовыйФункционал.Отладка'"), // ИмяСобытия.
				НСтр("ka='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';ru='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО'"), // ИдентификаторШага.
...
Функция ПроверитьЛогинИПароль(Логин, Пароль, Знач НастройкиСоединенияССерверами) Экспорт
	...
	ИначеЕсли РезультатПолученияФайла.КодСостояния = 403 Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ka='არასწორი ლოგ-ინი ან პაროლი.';ru='Неверный логин или пароль.'");
...
"Повторите попытку позже.';ka='ლოგ-ინის და პაროლის შეყვანის მცდელობის რაოდენობა გადაჭარბებულია. მოგვიანებით სცადეთ.'");
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვის %1-ში ლოგ-ინის და პაროლის შემოწმება ვერ მოხერხდა. %2';ru='Не удалось проверить логин и пароль в сервисе %1. %2'"),
				URLСервиса,
...
Функция СлужебнаяТикетАутентификации(
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა ავთენტიფიკაციის სერვისზე შეერთებისას.';ru='Ошибка при подключении к сервису аутентификации.'")
		...
		Результат.СообщениеОбОшибке = НСтр("ka='შეცდომა სერვისთან შეერთებისას.';ru='Ошибка при подключении к сервису.'")
			+ Символы.ПС
			+ ПроксиСервиса.СообщениеОбОшибке
			+ Символы.ПС
			+ НСтр("ka='დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Подробнее см. в журнале регистрации.'");
		Возврат Результат;
	...
	Иначе
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა ავთენტიფიკაციის ტიკეტის მიღებისას.';ru='Ошибка при получении тикета аутентификации.'")
...
Функция СервисАутентификации_getTicket(
	...
	Если Не ПустаяСтрока(Результат.КодОшибки) Тогда
		...
		Если СтрНайти(Результат.ИнформацияОбОшибке, "IncorrectLoginOrPasswordApiException") > 0 Тогда
			Результат.КодОшибки         = "НеверныйЛогинИлиПароль";
			Результат.ИнформацияОбОшибке = НСтр("ka='არასწორი ლოგ-ინი ან პაროლი (IncorrectLoginOrPasswordApiException).';ru='Неверный логин или пароль (IncorrectLoginOrPasswordApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ka='არასწორი ლოგ-ინი ან პაროლი.';ru='Неверный логин или пароль.'");

		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TooManyFailedAttemptsApiException") > 0 Тогда
			Результат.КодОшибки = "ПревышеноКоличествоПопыток";
			Результат.ИнформацияОбОшибке =
				НСтр("ka='ავთენტიფიკაციის მცდელობის რაოდენობა გადაჭარბებულია (TooManyFailedAttemptsApiException).';ru='Превышено количество попыток аутентификации (TooManyFailedAttemptsApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ka='ავთენტიფიკაციის მცდელობის რაოდენობა გადაჭარბებულია.';ru='Превышено количество попыток аутентификации.'");

		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TicketApiException") > 0 Тогда
			Результат.КодОшибки = "ОшибкаСервиса";
			Результат.ИнформацияОбОшибке = НСтр("ka='ავთენტიფიკაციის სერვისის უცნობი შეცდომა (TicketApiException).';ru='Неизвестная ошибка сервиса аутентификации (TicketApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ka='სერვისის შიდა შეცდომა.';ru='Внутренняя ошибка сервиса.'")
	...
	Иначе
		...
		Если ОписаниеОтветаSOAP.Ошибка Тогда
			...
			Результат.СообщениеОбОшибке = НСтр("ka='სერვისის შეცდომა.';ru='Ошибка сервиса.'")
				+ Символы.ПС + ОписаниеОтветаSOAP.СообщениеОбОшибке;
			Результат.ИнформацияОбОшибке =
				НСтр("ka='ავთენტიფიკაციის სერვისის ოპერაციის getTicket() მოხმობის შეცდომა.';ru='Ошибка вызова операции getTicket() сервиса аутентификации.'") + " "
				+ ОписаниеОтветаSOAP.ИнформацияОбОшибке
				+ Символы.ПС + НСтр("ka='პასუხის ტანი:';ru='Тело ответа:'") + " " + Лев(ОписаниеОтвета.ТелоОтвета, 1024 * 5);
			Возврат Результат;
		КонецЕсли;
...
Процедура УстановитьОписаниеОшибки(Результат, КодОшибки, СообщениеОбОшибке, ИнформацияОбОшибке, Перенаправления)
	...
	Если КодОшибки = "ConnectError" Тогда
		ДопСообщение = НСтр("ka='Ошибка при подключении к серверу.';ru='Ошибка при подключении к серверу.'");

	ИначеЕсли КодОшибки = "ServerError" Тогда
		ДопСообщение = НСтр("ka='На сервере возникла внутренняя ошибка при обработке запроса.';ru='На сервере возникла внутренняя ошибка при обработке запроса.'");

	ИначеЕсли КодОшибки = "ClientError" Тогда
		ДопСообщение = НСтр("ka='არაკორექტული მოთხოვნა.';ru='Некорректный запрос.'");

	ИначеЕсли КодОшибки = "InternalError" Тогда
		ДопСообщение = НСтр("ka='შიდა შეცდომა.';ru='Внутренняя ошибка.'");

	ИначеЕсли КодОшибки = "LoginError" Тогда
		ДопСообщение = НСтр("ka='სერვერზე ავთენტიფიკაციის შეცდომა.';ru='Ошибка аутентификации на сервере.'");
	...
	Если Перенаправления.Количество() > 0 Тогда
		Результат.ИнформацияОбОшибке = Результат.ИнформацияОбОшибке + Символы.ПС
			+ НСтр("ka='გადამისამართებები:';ru='Перенаправления:'") + Символы.ПС
...
Функция НовыйОпределенияСервиса(
	...
	Если НЕ ПустаяСтрока(ОписаниеWSDL.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა WSDL-აღწერილობის მიღებისას.';ru='Ошибка при получении WSDL-описания.'") + " "
	...
	Исключение
		...
		Результат.КодОшибки          = "ServerError";
		Результат.СообщениеОбОшибке  = НСтр("ka='სერვისის არაკორექტული WSDL-აღწერილობა.';ru='Некорректное WSDL-описание сервиса.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა WSDL-აღწერილობის დამუშავებისას %1. %2';ru='Ошибка при обработке WSDL-описания %1. %2'"),
				МестоположениеWSDL,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если УзлыДирективыИмпорта.Количество() > 0 Тогда
		...
		Для каждого УзелИмпорта Из УзлыДирективыИмпорта Цикл
			...
			Если ПустаяСтрока(Location) Тогда
				...
				Результат.СообщениеОбОшибке  = НСтр("ka='სერვისის არაკორექტული WSDL-აღწერილობა.';ru='Некорректное WSDL-описание сервиса.'");
				Результат.ИнформацияОбОшибке =
					НСтр("ka='დირექტივა import-ისთვის location-ი არ არის.';ru='Отсутствует location для директивы import.'");
				Возврат Результат;
			КонецЕсли;
	...
	Для каждого УзелТипов Из УзлыОписанияТипов Цикл
		...
		Для каждого УзелСхемы Из УзлыСхем Цикл
			...
			Исключение
				...
				Результат.ИнформацияОбОшибке =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='შეცდომა WSDL-აღწერილობის დამუშავებისას %1. მონაცემთა სქემის აგების შეცდომა. %2';ru='Ошибка при обработке WSDL-описания %1. Ошибка построения схемы данных. %2'"),
						МестоположениеWSDL,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат Результат;
	...
	Иначе
		...
		Иначе
			...
			Исключение
				...
				Результат.ИнформацияОбОшибке =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='შეცდომა WSDL-აღწერილობა %1-ის დამუშავებისას. შეცდომა სერვისის ფაბრიკის შექმნისას. %2';ru='Ошибка при обработке WSDL-описания %1. Ошибка при создании фабрики XDTO сервиса. %2'"),
						МестоположениеWSDL,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат Результат;
...
Функция НовыйПроксиВебСервиса(
	...
	Если НЕ ПустаяСтрока(ОпределениеСервиса.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ka='შეცდომა ვებ-სერვისის აღწერილობის შექმნისას.';ru='Ошибка при создании описания веб-сервиса.'") + " "
	...
	Если НайденныйПорт = Неопределено Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ka='სერვისის განსაზღვრება ვერ მოიძებნა.';ru='Не найдено определение сервиса.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура СлужебнаяПроверитьURLДоступен(
	...
	Исключение
		ИмяОшибки = "Unknown";
		СообщениеОбОшибке = НСтр("ka='უცნობი შეცდომა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.'");
	...
	Если Результат = Неопределено Тогда
		...
		Если ПараметрыИнтернетПоддержки.РаботаВМоделиСервиса Тогда
			...
			Результат = Новый Структура("Действие,Сообщение",
				"ПоказатьСообщение",
				НСтр("ka='სერვისის მოდელში მუშაობისას მომხმარებლების ინტერნეტ-მხარდაჭერის გამოყენება მიუწვდომელია.';ru='Использование Интернет-поддержки пользователей недоступно при работе в модели сервиса.'"));

		ИначеЕсли НЕ ПараметрыИнтернетПоддержки.ЗапускРазрешен Тогда

			Результат = Новый Структура("Действие,Сообщение",
				"ПоказатьСообщение",
				НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერის გასაშვებად არასაკმარისი უფლებებია. მიმართეთ ადმინისტრატორს.';ru='Недостаточно прав для запуска Интернет-поддержки пользователей. Обратитесь к администратору.'"));
...
Функция НовыйОписаниеСервисаИПП(МестоположениеWSDL = Неопределено, Таймаут = -1) Экспорт
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		ТекстИсключения = НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვისზე შეერთებისას.';ru='Ошибка при подключении к сервису Интернет-поддержки.'") + " "
			+ ПроксиСервиса.ИнформацияОбОшибке;
...
"ძირეული თვისების ტიპის დადგენა ვერ მოხერხდა isConfigurationSupported.'"),
	...
	Если НЕ ПустаяСтрока(ОписаниеОтветаSOAP.КодОшибки) Тогда
		ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომა სერვისის ოპერაცია isConfigurationSupported-ის მოხმობისას (%1):';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1):'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ " " + ОписаниеОтветаSOAP.ИнформацияОбОшибке;
		ВызватьИсключение ТекстИсключения;
...
"ძირეული თვისების ტიპის დადგენა ვერ მოხერხდა isConfigurationSupportedResponse.'"),
	...
	Если ОписаниеОтветаSOAP.Ошибка Тогда
		...
		ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომეა სერვისის ოპერაცია isConfigurationSupported-ის მოხმობისას (%1).';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1).'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ Символы.ПС
			+ ОписаниеОтветаSOAP.ИнформацияОбОшибке
			+ Символы.ПС + Символы.ПС
			+ НСтр("ka='მოთხოვნის ტანი:';ru='Тело запроса:'")
			+ Символы.ПС
			+ Лев(ТекстКонверта, 1024 * 5)
			+ НСтр("ka='პასუხის ტანი:';ru='Тело ответа:'")
			+ Символы.ПС
...
Функция СервисИПП_process(ПараметрыЗапроса, ОписаниеСервисаИПП, Таймаут = -1)
	...
	Если НЕ ПустаяСтрока(ОписаниеОтветаSOAP.КодОшибки) Тогда
		ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომა სერვისის ოპერაცია process-ის მოხმობისას (%1):';ru='Ошибка при вызове операции process сервиса (%1):'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ " " + ОписаниеОтветаSOAP.ИнформацияОбОшибке;
		ВызватьИсключение ТекстИсключения;
...
Функция ПрочитатьОтветВКонвертеSOAP(ТелоОтвета, ОписаниеСервисаИПП, ТипЗначения)
	...
	Исключение
		...
		Результат.СообщениеОбОшибке  = НСтр("ka='პასუხის არასწორი ფორმატი.';ru='Неверный формат ответа.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პასუხის არასწორი ფორმატი SOAP. %1';ru='Неверный формат ответа SOAP. %1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если ЧтениеОтвета.ТипУзла = ТипУзлаXML.НачалоЭлемента
		...
		Результат.Ошибка = Истина;
		Результат.СообщениеОбОшибке  = НСтр("ka='SOAP-სერვერის შეცდომა.';ru='Ошибка SOAP-сервера.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='SOAP-სერვერის შეცდომა. %1';ru='Ошибка SOAP-сервера. %1'"),
...
Функция ОписаниеИсключенияSOAPВСтроку(ИсключениеSOAP)
	...
	Если НЕ ПустаяСтрока(ИсключениеSOAP.FaultActor) Тогда
		Результат = Результат + ?(ПустаяСтрока(Результат), "", Символы.ПС + НСтр("ka='შეცდომის წყარო:';ru='Источник ошибки:'") + " ")
...
Функция СтруктурироватьОтветСервера(
	...
	Исключение
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ka='უცნობი შეცდომა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Неизвестная ошибка. См. подробности в журнале регистрации.'");
...
Функция СтруктурироватьСообщениеИлиВопросПользователю(КомандаСервера)
	...
	Если НЕ СтруктураКоманды.Свойство("Заголовок") ИЛИ ПустаяСтрока(СтруктураКоманды.Заголовок) Тогда
		СтруктураКоманды.Вставить("Заголовок", НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей'"));
	КонецЕсли;
...
Функция ПараметрыВнутреннейФормы(ИндексФормы, МестоЗапуска)
	...
	ИначеЕсли ИндексФормы = "bh1" Тогда
		ПараметрыФормы.Вставить("Заголовок", НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей'"));
...
Функция СтруктурироватьЗаписьАдресногоКлассификатора(КомандаСервера)
		...
		Для каждого Параметр Из ПараметрыСписокСтран Цикл
			...
			Если ИмяТекПараметра = "country" Тогда
				...
				Если ИдентификаторСтраны <> Неопределено Тогда
					...
					СписокРегионовСтраны.Вставить(0, "-1", НСтр("ka='<არ არის არჩეული>';ru='<не выбран>'"));
	...
	СписокСтран.Вставить(0, "-1", НСтр("ka='<არ არის არჩეული>';ru='<не выбрана>'"));
...
Функция СтруктурироватьЗаписьОрганизацийПользователя(КомандаСервера)
	...
	СписокОрганизаций.Вставить(0, "-1", НСтр("ka='<добавить новую организацию>';ru='<добавить новую организацию>'"));
...
Процедура ДобавитьКомандыСервиса(
	...
	Если ПередаваемыеПараметрыЗапроса <> Неопределено Тогда
		...
		Для каждого ПередаваемыйПараметр Из ПередаваемыеПараметрыЗапроса Цикл
			...
			Иначе
				...
				Исключение
					ИнфОшибка = ИнформацияОбОшибке();
					ТекстИсключения = СтрЗаменить(НСтр("ka='შეცდომა გადასაცემ მონაცემთა გარდაქმნისას. %1';ru='Ошибка при преобразовании передаваемых данных. %1'"),
						"%1",
						ПодробноеПредставлениеОшибки(ИнфОшибка));
					ВызватьИсключение ТекстИсключения;
				КонецПопытки;
	...
	Если МассивСтруктурыКоманд = Неопределено ИЛИ МассивСтруктурыКоманд.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='სერვერის ცარიელი პასუხი.';ru='Пустой ответ сервера.'");
	КонецЕсли;
