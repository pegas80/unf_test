...
Процедура РазместитьНаФормеКомандыЭДО(Форма, МестоРазмещенияКомандПоУмолчанию, МодульПодсистемы, НаправлениеЭД = Неопределено, ТолькоВМенюЕще = Ложь) Экспорт
	...
	Если МестоРазмещенияКомандПоУмолчанию <> Неопределено Тогда
		...
		Если МодульПодсистемы = "ОбменСБанками" Тогда
			МестоРазмещенияКомандПоУмолчанию.Заголовок = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк'");
		КонецЕсли;
	КонецЕсли;
	...
	Если МестоРазмещенияКомандПоУмолчанию = Неопределено Тогда
		...
		Если МодульПодсистемы = "ОбменСБанками" Тогда
			ПодменюЭДО.Заголовок = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк'");
		Иначе
			ПодменюЭДО.Заголовок = НСтр("ka='ედბ';ru='ЭДО'");
		КонецЕсли;
...
Процедура ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(ОписаниеСобытия, КодСобытия, УровеньВажности = Неопределено,
	...
	Если КодСобытия = 0 Тогда Подсистема = НСтр("ka='საერთო ქვესისტემა';ru='Общая подсистема'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 1 Тогда Подсистема = НСтр("ka='ბანკებთან გაცვლა';ru='Обмен с банками'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 2 Тогда Подсистема = НСтр("ka='კონტრაგენტებთან გაცვლა';ru='Обмен с контрагентами'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 3 Тогда Подсистема = НСтр("ka='საიტებთან გაცვლა';ru='Обмен с сайтами'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 4 Тогда Подсистема = НСтр("ka='რეგლამენტური დავალებები';ru='Регламентные задания'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 5 Тогда Подсистема = НСтр("ka='ბიზნეს-ქსელი';ru='Бизнес-сеть'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 6 Тогда Подсистема = НСтр("ka='Интеграция с Яндекс Кассой';ru='Интеграция с Яндекс Кассой'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Подсистема = НСтр("ka='საერთო ქვესისტემა';ru='Общая подсистема'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	...
	ИмяСобытия = СтрЗаменить(НСтр("ka='ელექტრონული ურთიერთქმედება.%1';ru='Электронное взаимодействие.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		"%1", Подсистема);
...
Процедура СообщитьПользователюОНарушенииПравДоступа() Экспорт
	...
	ТекстСообщения = НСтр("ka='Нарушение прав доступа.';ru='Нарушение прав доступа.'");
...
Функция РасшифровкаКодаСостоянияHTTP(КодСостояния) Экспорт
	...
	СоответствиеКодов.Вставить(400, НСтр("ka='კლიენტის მოთხოვნაში სერვერმა აღმოაჩინა სინტაქსისური შეცდომა.';ru='Сервер обнаружил в запросе клиента синтаксическую ошибку.'"));
	СоответствиеКодов.Вставить(401, НСтр("ka='ავთენტიფიკაციის არასწორი მონაცემები.';ru='Неверные данные аутентификации.'"));
	СоответствиеКодов.Вставить(403, НСтр("ka='კლიენტს რესურსთან დაშვება არ გააჩნია.';ru='У клиента нет доступа к ресурсу.'"));
	СоответствиеКодов.Вставить(404, НСтр("ka='აღნიშნული URI-ის მიხედვით სერვერზე რესურსი არ არის.';ru='На сервере нет ресурса по указанному URI.'"));
	СоответствиеКодов.Вставить(405, НСтр("ka='კლიენტის მიერ მითითებული მეთოდის გამოყენება მიმდინარე რესურსზე არ შეიძლება.';ru='Указанный клиентом метод нельзя применить к текущему ресурсу.'"));
	СоответствиеКодов.Вставить(406, НСтр("ka='მოთხოვნილი URI სათაურში გადაცემულ მახასიათებლებს ვერ დააკმაყოფილებს.';ru='Запрошенный URI не может удовлетворить переданным в заголовке характеристикам.'"));
	СоответствиеКодов.Вставить(407, НСтр("ka='სერვერთან დასაშვებად საჭიროა ავთენტიფიკაცია პროქსი-სერვერისთვის.';ru='Для доступа к серверу требуется аутентификация для прокси-сервера.'"));
	СоответствиеКодов.Вставить(408, НСтр("ka='სერვერის მიერ კლიენტისგან გადაცემის დრო ამოიწურა.';ru='Время ожидания сервером передачи от клиента истекло.'"));
	СоответствиеКодов.Вставить(409, НСтр("ka='მოთხოვნა ვერ შესრულდება რესურსთან კონფლიქტური მიმართვის გამო.';ru='Запрос не может быть выполнен из-за конфликтного обращения к ресурсу.'"));
	СоответствиеКодов.Вставить(410, НСтр("ka='Ресурс был удален и теперь недоступен.';ru='Ресурс был удален и теперь недоступен.'"));
	СоответствиеКодов.Вставить(411, НСтр("ka='Не указан объем передаваемых данных в заголовке.';ru='Не указан объем передаваемых данных в заголовке.'"));
	СоответствиеКодов.Вставить(412, НСтр("ka='მოთხოვნის სათაურის არცერთი პირობითი ველი არ შესრულებულა.';ru='Ни одно из условных полей заголовка запроса не было выполнено.'"));
	СоответствиеКодов.Вставить(413, НСтр("ka='სერვერმა უარი თქვა მოთხოვნის დამუშავებაზე მოთხოვნის სხეულის საკმაოდ დიდი ზომის გამო.';ru='Сервер отказался обработать запрос по причине слишком большого размера тела запроса.'"));
	СоответствиеКодов.Вставить(414, НСтр("ka='სერვერს არ შეუძლია მოთხოვნის დამუშავება მითითებული საკმაოდ გრძელი URL-ის გამო.';ru='Сервер не может обработать запрос из-за слишком длинного указанного URL.'"));
	СоответствиеКодов.Вставить(415, НСтр("ka='სერვერი უარს ამბობს მოცემული მეთოდით მონაცემთა აღნიშნულ ტიპთან მუშაობაზე.';ru='Сервер отказывается работать с указанным типом данных при данном методе.'"));
	СоответствиеКодов.Вставить(422, НСтр("ka='არის ლოგიკური შეცდომა, რომლის გამო შეუძლებელია ოპერაციის ჩატარება.';ru='Имеется логическая ошибка, из-за которой невозможно произвести операцию.'"));
	СоответствиеКодов.Вставить(423, НСтр("ka='მოთხოვნიდან მიზნობრივი რესურსი მის მიმართ აღნიშნული მეთოდის გამოყენებისგან დაბლოკილია.';ru='Целевой ресурс из запроса заблокирован от применения к нему указанного метода.'"));
	СоответствиеКодов.Вставить(426, НСтр("ka='კლიენტისთვის საჭიროებაზე განაახლეთ პროტოკოლი.';ru='Клиенту на необходимость обновить протокол.'"));
	СоответствиеКодов.Вставить(429, НСтр("ka='კლიენტმა მოკლე დროში სცადა საკმაოდ ბევრი მოთხოვნის გაგზავნა.';ru='Клиент попытался отправить слишком много запросов за короткое время.'"));
	СоответствиеКодов.Вставить(431, НСтр("ka='სათაურების დასაშვები სიგრძე გადამეტებულია.';ru='Превышена допустимая длина заголовков.'"));
	СоответствиеКодов.Вставить(434, НСтр("ka='მოთხოვნილი მისამართი მიუწვდომელია.';ru='Запрашиваемый адрес недоступен.'"));
	СоответствиеКодов.Вставить(449, НСтр("ka='მიღებულია არა საკმარისი ინფორმაცია';ru='Поступило недостаточно информации.'"));
	СоответствиеКодов.Вставить(451, НСтр("ka='იურიდიული მიზეზების გამო რესურსთან დაშვება დახურულია.';ru='Доступ к ресурсу закрыт по юридическим причинам.'"));
	
	СоответствиеКодов.Вставить(500, НСтр("ka='სერვერის შიდა შეცდომა';ru='Внутренняя ошибка сервера.'"));
	СоответствиеКодов.Вставить(501, НСтр("ka='მოთხოვნის დასამუშავებლად საჭირო შესაძლებლობები სერვერის მიერ არ არის მხარდაჭერილი.';ru='Сервер не поддерживает возможностей, необходимых для обработки запроса.'"));
	СоответствиеКодов.Вставить(502, НСтр("ka='სერვერმა, რომელიც რაბის ან პროქსი-სერვერის როლს თამაშობს, ზემდგომი სერვერისგან უმოქმედო საპასუხო შეტყობინება მიიღო.';ru='Сервер, выступая в роли шлюза или прокси-сервера, получил недействительное ответное сообщение от вышестоящего сервера.'"));
	СоответствиеКодов.Вставить(503, НСтр("ka='ტექნიკური მიზეზების გამო სერვერს დროებით არ გააჩნია მოთხოვნების დამუშავების შესაძლებლობა.';ru='Сервер временно не имеет возможности обрабатывать запросы по техническим причинам.'"));
	СоответствиеКодов.Вставить(504, НСтр("ka='შლუზის ან პროქსი-სერვერის როლში მყოფი სერვერი არ დაელოდა ზემდგომი სერვერის პასუხს მიმდინარე მოთხოვნის დასასრულებლად.';ru='Сервер в роли шлюза или прокси-сервера не дождался ответа от вышестоящего сервера для завершения текущего запроса.'"));
	СоответствиеКодов.Вставить(505, НСтр("ka='მოთხოვნაში აღნიშნულ HTTP პროტოკოლის ვერსიას სერვერის მხარდაჭერა არ გააჩნია.';ru='Сервер не поддерживает указанную в запросе версию протокола HTTP.'"));
	СоответствиеКодов.Вставить(507, НСтр("ka='მიმდინარე მოთხოვნის შესასრულებლად არ არის საკმარისი ადგილი.';ru='Не хватает места для выполнения текущего запроса.'"));
	СоответствиеКодов.Вставить(510, НСтр("ka='სერვერზე არ არის გაფართოება, რომლის გამოყენებაც კლიენტს სურს.';ru='На сервере отсутствует расширение, которое желает использовать клиент.'"));
	СоответствиеКодов.Вставить(511, НСтр("ka='აუცილებელია პროვაიდერის ქსელში ავტორიზაცია.';ru='Необходимо авторизоваться в сети провайдера.'"));
...
Процедура УдалитьВременныеФайлы(Путь, Маска = Неопределено) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='Удаление временного файла.';ru='Удаление временного файла.'");
...
Функция ЗначениеРеквизитаВДереве(ДеревоДанных, ПолныйПуть, СообщатьОбОшибке = Истина) Экспорт
	...
	ИначеЕсли СообщатьОбОшибке Тогда
		ТекстСообщения = НСтр("ka='მონაცემთა ხეში ველი ვერ მოიძებნა სრული გზით <%1>';ru='Не найдено поле в дереве данных по полному пути <%1>'");
	...
	Для Каждого Строка Из ДеревоДанных.Строки Цикл
		...
		Если Строка.Обязательное = "Да" И Не ЗначениеЗаполнено(Строка.Значение)
			...
			Если ЭтоТаблица Тогда
				ШаблонОшибки = НСтр("ka='ცხრილის სავალდებულო ველი არ არის შევსებული: ""%1"". სტრიქონ %2-ში';ru='Не заполнено обязательное поле таблицы: ""%1."" в строке %2'");
				ТекстОшибки = ТекстОшибки
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПрикладноеПредставление, НомерСтроки)
							+ Символы.ПС;
			Иначе
				ШаблонОшибки = НСтр("ka='სავალდებულო ველი არ არის შევსებული: ""%1"".';ru='Не заполнено обязательное поле: ""%1.""'");
				ТекстОшибки = ТекстОшибки
...
Функция ТаблицыРеквизитовОбъектовОдинаковые(Таблица1, Таблица2, РеквизитыОбъекта, ДопПараметры = Неопределено)
	...
	Если НЕ ПризнакСовпадения И ЗначениеЗаполнено(ДопПараметры) Тогда
		...
		Если ИмяТабличнойЧасти = "Шапка" Тогда
			...
			НовСтрокаДереваМесто.Место = НСтр("ka='ქუდის რეკვიზიტები';ru='Реквизиты шапки'");
		...
		Иначе
			НовСтрокаДереваМесто = СтрокаДереваСравнения.Строки.Добавить();
			НовСтрокаДереваМесто.Место = СтрШаблон(НСтр("ka='Табличная часть <%1>';ru='Табличная часть <%1>'"), ИмяТабличнойЧасти);
			НовСтрокаДереваРеквизита = НовСтрокаДереваМесто.Строки.Добавить();
