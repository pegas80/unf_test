...
Процедура РазместитьНаФормеКомандыЭДО(Форма, МестоРазмещенияКомандПоУмолчанию, МодульПодсистемы, НаправлениеЭД = Неопределено, ТолькоВМенюЕще = Ложь) Экспорт
	...
	Если МестоРазмещенияКомандПоУмолчанию <> Неопределено Тогда
		...
		Если МодульПодсистемы = "ОбменСБанками" Тогда
			МестоРазмещенияКомандПоУмолчанию.Заголовок = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк';en='1C: DirektBank'");
		КонецЕсли;
	КонецЕсли;
	...
	Если МестоРазмещенияКомандПоУмолчанию = Неопределено Тогда
		...
		Если МодульПодсистемы = "ОбменСБанками" Тогда
			ПодменюЭДО.Заголовок = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк';en='1C: DirektBank'");
		Иначе
			ПодменюЭДО.Заголовок = НСтр("ka='ედბ';ru='ЭДО';en='EDO'");
		КонецЕсли;
...
Процедура ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(ОписаниеСобытия, КодСобытия, УровеньВажности = Неопределено,
	...
	Если КодСобытия = 0 Тогда Подсистема = НСтр("ka='საერთო ქვესისტემა';ru='Общая подсистема';en='Total subsystem'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 1 Тогда Подсистема = НСтр("ka='ბანკებთან გაცვლა';ru='Обмен с банками';en='Exchange with the banks'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 2 Тогда Подсистема = НСтр("ka='კონტრაგენტებთან გაცვლა';ru='Обмен с контрагентами';en='Exchange with counterparts'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 3 Тогда Подсистема = НСтр("ka='საიტებთან გაცვლა';ru='Обмен с сайтами';en='Exchange with sites'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 4 Тогда Подсистема = НСтр("ka='რეგლამენტური დავალებები';ru='Регламентные задания';en='scheduled jobs'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 5 Тогда Подсистема = НСтр("ka='ბიზნეს-ქსელი';ru='Бизнес-сеть';en='Business Network'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 6 Тогда Подсистема = НСтр("ka='Интеграция с Яндекс Кассой';ru='Интеграция с Яндекс Кассой';en='Integration with Yandex Cashier'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Подсистема = НСтр("ka='საერთო ქვესისტემა';ru='Общая подсистема';en='Total subsystem'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	...
	ИмяСобытия = СтрЗаменить(НСтр("ka='ელექტრონული ურთიერთქმედება.%1';ru='Электронное взаимодействие.%1';en='Electron interaction.% 1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		"%1", Подсистема);
...
Функция РасшифровкаКодаСостоянияHTTP(КодСостояния) Экспорт
	...
	СоответствиеКодов.Вставить(400, НСтр("ka='კლიენტის მოთხოვნაში სერვერმა აღმოაჩინა სინტაქსისური შეცდომა.';ru='Сервер обнаружил в запросе клиента синтаксическую ошибку.';en='The server found in the client request syntax error.'"));
	СоответствиеКодов.Вставить(401, НСтр("ka='ავთენტიფიკაციის არასწორი მონაცემები.';ru='Неверные данные аутентификации.';en='Incorrect authentication data.'"));
	СоответствиеКодов.Вставить(403, НСтр("ka='კლიენტს რესურსთან დაშვება არ გააჩნია.';ru='У клиента нет доступа к ресурсу.';en='The client does not have access to the resource.'"));
	СоответствиеКодов.Вставить(404, НСтр("ka='აღნიშნული URI-ის მიხედვით სერვერზე რესურსი არ არის.';ru='На сервере нет ресурса по указанному URI.';en='On the server, there is no resource with the specified URI.'"));
	СоответствиеКодов.Вставить(405, НСтр("ka='კლიენტის მიერ მითითებული მეთოდის გამოყენება მიმდინარე რესურსზე არ შეიძლება.';ru='Указанный клиентом метод нельзя применить к текущему ресурсу.';en='Specified by the client method can not be applied to the current resource.'"));
	СоответствиеКодов.Вставить(406, НСтр("ka='მოთხოვნილი URI სათაურში გადაცემულ მახასიათებლებს ვერ დააკმაყოფილებს.';ru='Запрошенный URI не может удовлетворить переданным в заголовке характеристикам.';en='Requested URI can not meet the characteristics transmitted in the header.'"));
	СоответствиеКодов.Вставить(407, НСтр("ka='სერვერთან დასაშვებად საჭიროა ავთენტიფიკაცია პროქსი-სერვერისთვის.';ru='Для доступа к серверу требуется аутентификация для прокси-сервера.';en='To access the server requires authentication for the proxy server.'"));
	СоответствиеКодов.Вставить(408, НСтр("ka='სერვერის მიერ კლიენტისგან გადაცემის დრო ამოიწურა.';ru='Время ожидания сервером передачи от клиента истекло.';en='wait time server from the client has expired.'"));
	СоответствиеКодов.Вставить(409, НСтр("ka='მოთხოვნა ვერ შესრულდება რესურსთან კონფლიქტური მიმართვის გამო.';ru='Запрос не может быть выполнен из-за конфликтного обращения к ресурсу.';en='The request could not be completed due to a conflict referring to the resource.'"));
	СоответствиеКодов.Вставить(410, НСтр("ka='Ресурс был удален и теперь недоступен.';ru='Ресурс был удален и теперь недоступен.';en='The resource has been removed and is now available.'"));
	СоответствиеКодов.Вставить(411, НСтр("ka='Не указан объем передаваемых данных в заголовке.';ru='Не указан объем передаваемых данных в заголовке.';en='Unknown amount of data transmitted in the header.'"));
	СоответствиеКодов.Вставить(412, НСтр("ka='მოთხოვნის სათაურის არცერთი პირობითი ველი არ შესრულებულა.';ru='Ни одно из условных полей заголовка запроса не было выполнено.';en='None of the conditional request header fields has not been fulfilled.'"));
	СоответствиеКодов.Вставить(413, НСтр("ka='სერვერმა უარი თქვა მოთხოვნის დამუშავებაზე მოთხოვნის სხეულის საკმაოდ დიდი ზომის გამო.';ru='Сервер отказался обработать запрос по причине слишком большого размера тела запроса.';en='The server refused to process the request due to too large request body.'"));
	СоответствиеКодов.Вставить(414, НСтр("ka='სერვერს არ შეუძლია მოთხოვნის დამუშავება მითითებული საკმაოდ გრძელი URL-ის გამო.';ru='Сервер не может обработать запрос из-за слишком длинного указанного URL.';en='The server can not process the request due to too long a specified URL.'"));
	СоответствиеКодов.Вставить(415, НСтр("ka='სერვერი უარს ამბობს მოცემული მეთოდით მონაცემთა აღნიშნულ ტიპთან მუშაობაზე.';ru='Сервер отказывается работать с указанным типом данных при данном методе.';en='The server refuses to work with said data type in this method.'"));
	СоответствиеКодов.Вставить(422, НСтр("ka='არის ლოგიკური შეცდომა, რომლის გამო შეუძლებელია ოპერაციის ჩატარება.';ru='Имеется логическая ошибка, из-за которой невозможно произвести операцию.';en='There is a logic error, because of which it is impossible to perform the operation.'"));
	СоответствиеКодов.Вставить(423, НСтр("ka='მოთხოვნიდან მიზნობრივი რესურსი მის მიმართ აღნიშნული მეთოდის გამოყენებისგან დაბლოკილია.';ru='Целевой ресурс из запроса заблокирован от применения к нему указанного метода.';en='Target resource of the request is blocked by the application thereto of said method.'"));
	СоответствиеКодов.Вставить(426, НСтр("ka='კლიენტისთვის საჭიროებაზე განაახლეთ პროტოკოლი.';ru='Клиенту на необходимость обновить протокол.';en='The need to update the client protocol.'"));
	СоответствиеКодов.Вставить(429, НСтр("ka='კლიენტმა მოკლე დროში სცადა საკმაოდ ბევრი მოთხოვნის გაგზავნა.';ru='Клиент попытался отправить слишком много запросов за короткое время.';en='The client tried to send too many requests in a short time.'"));
	СоответствиеКодов.Вставить(431, НСтр("ka='სათაურების დასაშვები სიგრძე გადამეტებულია.';ru='Превышена допустимая длина заголовков.';en='Exceed the permissible length of the header.'"));
	СоответствиеКодов.Вставить(434, НСтр("ka='მოთხოვნილი მისამართი მიუწვდომელია.';ru='Запрашиваемый адрес недоступен.';en='The requested address is not available.'"));
	СоответствиеКодов.Вставить(449, НСтр("ka='მიღებულია არა საკმარისი ინფორმაცია';ru='Поступило недостаточно информации.';en='Received enough information.'"));
	СоответствиеКодов.Вставить(451, НСтр("ka='იურიდიული მიზეზების გამო რესურსთან დაშვება დახურულია.';ru='Доступ к ресурсу закрыт по юридическим причинам.';en='Access to the resource is closed for legal reasons.'"));
	
	СоответствиеКодов.Вставить(500, НСтр("ka='სერვერის შიდა შეცდომა';ru='Внутренняя ошибка сервера.';en='Internal Server Error.'"));
	СоответствиеКодов.Вставить(501, НСтр("ka='მოთხოვნის დასამუშავებლად საჭირო შესაძლებლობები სერვერის მიერ არ არის მხარდაჭერილი.';ru='Сервер не поддерживает возможностей, необходимых для обработки запроса.';en='The server does not support the features required to process the request.'"));
	СоответствиеКодов.Вставить(502, НСтр("ka='სერვერმა, რომელიც რაბის ან პროქსი-სერვერის როლს თამაშობს, ზემდგომი სერვერისგან უმოქმედო საპასუხო შეტყობინება მიიღო.';ru='Сервер, выступая в роли шлюза или прокси-сервера, получил недействительное ответное сообщение от вышестоящего сервера.';en='A server acting as a gateway or proxy server received an invalid response message from the upstream server.'"));
	СоответствиеКодов.Вставить(503, НСтр("ka='ტექნიკური მიზეზების გამო სერვერს დროებით არ გააჩნია მოთხოვნების დამუშავების შესაძლებლობა.';ru='Сервер временно не имеет возможности обрабатывать запросы по техническим причинам.';en='The server is temporarily unable to process requests for technical reasons.'"));
	СоответствиеКодов.Вставить(504, НСтр("ka='შლუზის ან პროქსი-სერვერის როლში მყოფი სერვერი არ დაელოდა ზემდგომი სერვერის პასუხს მიმდინარე მოთხოვნის დასასრულებლად.';ru='Сервер в роли шлюза или прокси-сервера не дождался ответа от вышестоящего сервера для завершения текущего запроса.';en='Server as a gateway or proxy and did not wait for a response from an upstream server to complete the current request.'"));
	СоответствиеКодов.Вставить(505, НСтр("ka='მოთხოვნაში აღნიშნულ HTTP პროტოკოლის ვერსიას სერვერის მხარდაჭერა არ გააჩნია.';ru='Сервер не поддерживает указанную в запросе версию протокола HTTP.';en='The server does not support this request in the HTTP protocol version.'"));
	СоответствиеКодов.Вставить(507, НСтр("ka='მიმდინარე მოთხოვნის შესასრულებლად არ არის საკმარისი ადგილი.';ru='Не хватает места для выполнения текущего запроса.';en='There is not enough space to perform the current request.'"));
	СоответствиеКодов.Вставить(510, НСтр("ka='სერვერზე არ არის გაფართოება, რომლის გამოყენებაც კლიენტს სურს.';ru='На сервере отсутствует расширение, которое желает использовать клиент.';en='On the server, there is no extension, which wants to use the client.'"));
	СоответствиеКодов.Вставить(511, НСтр("ka='აუცილებელია პროვაიდერის ქსელში ავტორიზაცია.';ru='Необходимо авторизоваться в сети провайдера.';en='You need to log in to the network provider.'"));
...
Процедура УдалитьВременныеФайлы(Путь, Маска = Неопределено) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='Удаление временного файла.';ru='Удаление временного файла.';en='Delete temporary files.'");
...
Функция ЗначениеРеквизитаВДереве(ДеревоДанных, ПолныйПуть, СообщатьОбОшибке = Истина) Экспорт
	...
	ИначеЕсли СообщатьОбОшибке Тогда
		ТекстСообщения = НСтр("ka='მონაცემთა ხეში ველი ვერ მოიძებნა სრული გზით <%1>';ru='Не найдено поле в дереве данных по полному пути <%1>';en='We found the field in the data tree by the full path <% 1>'");
	...
	Для Каждого Строка Из ДеревоДанных.Строки Цикл
		...
		Если Строка.Обязательное = "Да" И Не ЗначениеЗаполнено(Строка.Значение)
			...
			Если ЭтоТаблица Тогда
				ШаблонОшибки = НСтр("ka='ცხრილის სავალდებულო ველი არ არის შევსებული: ""%1"". სტრიქონ %2-ში';ru='Не заполнено обязательное поле таблицы: ""%1."" в строке %2';en='Missing required field in the table: ""%1"". in row 2%'");
				ТекстОшибки = ТекстОшибки
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПрикладноеПредставление, НомерСтроки)
							+ Символы.ПС;
			Иначе
				ШаблонОшибки = НСтр("ka='სავალდებულო ველი არ არის შევსებული: ""%1"".';ru='Не заполнено обязательное поле: ""%1.""';en='Missing required field ""%1"".'");
				ТекстОшибки = ТекстОшибки
...
Функция ТаблицыРеквизитовОбъектовОдинаковые(Таблица1, Таблица2, РеквизитыОбъекта, ДопПараметры = Неопределено)
	...
	Если НЕ ПризнакСовпадения И ЗначениеЗаполнено(ДопПараметры) Тогда
		...
		Если ИмяТабличнойЧасти = "Шапка" Тогда
			...
			НовСтрокаДереваМесто.Место = НСтр("ka='ქუდის რეკვიზიტები';ru='Реквизиты шапки';en='Imprint caps'");
		...
		Иначе
			НовСтрокаДереваМесто = СтрокаДереваСравнения.Строки.Добавить();
			НовСтрокаДереваМесто.Место = СтрШаблон(НСтр("ka='Табличная часть <%1>';ru='Табличная часть <%1>';en='Tabular section <% 1>'"), ИмяТабличнойЧасти);
			НовСтрокаДереваРеквизита = НовСтрокаДереваМесто.Строки.Добавить();
