...
Процедура ВыполнитьМетодОбъекта(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Некорректное значение параметра ИмяМетода (%1) в РаботаВБезопасномРежиме.ВыполнитьМетодОбъекта';ru='Некорректное значение параметра ИмяМетода (%1) в РаботаВБезопасномРежиме.ВыполнитьМетодОбъекта';en='Incorrect methodName parameter value (1%) in RabotaVBezopasnomRezhime.VypolnitMetodObekta'"), ИмяМетода);
	КонецПопытки;
...
Процедура ПроверитьИмяМетодаКонфигурации(Знач ИмяМетода) Экспорт
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации';ru='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации';en='Wrong format methodName parameter (passed value: ""%1"") in RabotaVBezopasnomRezhime.ProveritImyaMetodaKonfiguratsii'"), ИмяМетода);
	КонецЕсли;
...
"Не найден общий модуль ""%2"".';ka='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Не найден общий модуль ""%2"".'"),
	...
	Если ЧастиИмени.Количество() = 3 Тогда
		...
		Если Менеджер = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Не найден менеджер объекта ""%2"".';ka='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Не найден менеджер объекта ""%2"".'"),
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='მეთოდის უსაფრთხო შესრულება';ru='Безопасное выполнение метода';en='Safe execution method'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Имя метода ""%2"" не соответствует требованиям образования имен процедур и функций.';ka='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
...
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';ka='სეანსის პარამეტრების დაყენების დამამუშავებლების შესრულება შეუძლებელია შემდეგი მიზეზის გამო: 1С:საწარმოს სერვერების კლასტერში უსაფრთხოების პროფილი %1 არ არის, ან მისთვის უსაფრთხოების პროფილად უსაფრთხოების რეჟიმის გამოყენება აკრძალულია."
""
"პროგრამის შრომისუნარიანობის აღსადგენად, საჭიროა კლასტერის კონსოლით უსაფრთხოების პროფილის გამოყენების გამორთვა და კონფიგურაციის ინტერფეისის დახმარებით უსაფრთხოების პროფილების ხელახლა გამართვა (შესაბამისი ბრძანებები მდებარეობს პროგრამის გამართვების ნაწილში).'"),
...
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';ka='სეანსის პარამეტრების დაყენების დამამუშავებლების შესრულება შეუძლებელია შემდეგი მიზეზით: უსაფრთხოების პროფილი %1 პრივილეგირებული რეჟიმის დაყენების ნებართვას არ შეიცავს. შეიძლება, რომ კლასტერის კონსოლით მოხდა მისი რედაქტირება."
""
"პროგრამის შრომისუნარიანობის აღსადგენად, საჭიროა კლასტერის კონსოლით უსაფრთხოების პროფილის გამოყენების გამორთვა და კონფიგურაციის ინტერფეისის დახმარებით უსაფრთხოების პროფილების ხელახლა გამართვა (შესაბამისი ბრძანებები პროგრამის გამართვების ნაწილში მდებარეობს).'"),
				ПрофильИнформационнойБазы);
...
""
"Возможно, для информационной базы через консоль кластера был установлен профиль безопасности, не допускающий выполнения внешних модулей без установки безопасного режима. В этом случае для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).При этом программа будет автоматически корректно настроена на использование совместно с включенными профилями безопасности.';ka='სეანსის პარამეტრების დაყენების დამამუშავებლების შესრულება შეუძლებელია შემდეგი მიზეზით: %1."
""
"შეიძლება, საინფორმაციო ბაზისთვის კლასტერის კონსოლით დაყენებული იქნა უსაფრთხოების პროფილი, რომელიც არ უშვებს გარე მოდულების შესრულებას უსაფრთხო რეჟიმის დაუყენებლად. ამ შემთხვევაში, პროგრამის შრომისუნარიანობის აღსადგენად, საჭიროა კლასტერის კონსოლით უსაფრთხოების პროფილის გამოყენების გამორთვა და უსაფრთხოების პროფილების ხელახლა გამართვა კონფიგურაციის ინტერფეისის მეშვეობით (შესაბამისი ბრძანებები მდებარეობს პროგრამის გამართვების ნაწილში). ამასთან, პროგრამა ავტომატურად, კორექტულად იქნება გამართული უსაფრთხოების ჩართულ პროფილებთან ერთობლივად გამოსაყენებლად.'"),
...
"--------------------------------------------------------------------------------------------"
""
"Запуск программы будет невозможен.';ka='სეანსის პარამეტრების დაყენებისას შეცდომა მოხდა:"
""
"--------------------------------------------------------------------------------------------"
"%1"
"--------------------------------------------------------------------------------------------"
""
"პროგრამის გაშვება შეუძლებელი იქნება.'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='სეანსის პარამეტრების დაყენება';ru='Установка параметров сеанса';en='The session setup parameters'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ობიექტ ""%1""-ისთვის მენეჯერის მიღება ვერ მოხერხდა';ru='Не удалось получить менеджер для объекта ""%1""';en='Failed to get the manager for the project ""%1""'"), Имя);
