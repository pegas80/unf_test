...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты) Экспорт
	...
	Если Ссылка = Неопределено Тогда //2024-10-23
		Возврат Новый Структура;
	КонецЕсли;	
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მეორე პარამეტრის არასწორი ტიპი რეკვიზიტები: %1';ru='Неверный тип второго параметра Реквизиты: %1';en='Wrong type of second parameter Details: 1%'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
Функция ПровестиДокументы(Документы) Экспорт
	...
	Для Каждого ДокументСсылка Из Документы Цикл
		...
		Если ДокументОбъект.ПроверитьЗаполнение() Тогда
			...
			КонецПопытки;
		Иначе
			ПредставлениеОшибки = НСтр("ka='დოკუმენტის ველები შეუვსებელია.';ru='Поля документа не заполнены.';en='Document fields are empty.'");
		КонецЕсли;
...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "") Экспорт
	...
	Для Каждого Строка Из МестаИспользования Цикл
		...
		ИначеЕсли МетаКонстанты.Содержит(Мета) Тогда
			Представление = Мета.Представление() + " (" + НСтр("ka='კონსტანტა';ru='константа';en='constant'") + ")";
			
		ИначеЕсли МетаПоследовательности.Содержит(Мета) Тогда
			Представление = Мета.Представление() + " (" + НСтр("ka='თანმიმდევრობა';ru='последовательность';en='sequence'") + ")";
...
Функция ПрочитатьXMLВТаблицу(Знач XML) Экспорт
	...
	Если Не Чтение.Прочитать() Тогда
		ВызватьИсключение НСтр("ka='ცარიელი XML';ru='Пустой XML';en='empty XML'");
	ИначеЕсли Чтение.Имя <> "Items" Тогда
		ВызватьИсключение НСтр("ka='შეცდომა XML-ის სტრუქტურაში';ru='Ошибка в структуре XML';en='The error in the XML structure'");
	КонецЕсли;
	...
	Если Не СравниваютсяМассивы Тогда
		...
		Если ПустаяСтрока(ИменаКолонок) Тогда
			Если ТипыОсобыхКоллекций.Найти(ТипЗнч(КоллекцияСтрок1)) <> Неопределено
				...
				КонецЕсли;
			Иначе
				ТекстИсключения = НСтр("ka='%1 ტიპის კოლექციისთვის აუცილებელია იმ ველების სახელების მითითება, რომელთა მიხედვით ხორციელდება შედარება';ru='Для коллекции типа %1 необходимо указать имена полей, по которым производится сравнение';en='For a collection of type% 1 must specify the names of the fields for which comparison'");
...
Функция ИдентификаторCOMСоединителя(Знач ИмяCOMСоединителя) Экспорт
	...
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 კლასისთვის CLSID არ არის მიცემული';ru='На задан CLSID для класса %1';en='On CLSID specified for Class 1%'"), ИмяCOMСоединителя);
...
Функция ПредметСтрокой(СсылкаНаПредмет) Экспорт
	...
	Если СсылкаНаПредмет = Неопределено Или СсылкаНаПредмет.Пустая() Тогда
		Результат = НСтр("ka='მიცემული არ არის';ru='не задан';en='not specified'");
...
Функция СоздатьWSПрокси(ПереданныеПараметры) Экспорт
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		...
		Попытка
			WSПроксиPing.Ping();
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='WSპროქსი';ru='WSПрокси';en='WSProksi'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ВыполнитьЗапросВнеТранзакции(Знач Запрос) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение(НСтр("ka='ტრანზაქცია აქტიურია. მოთხოვნის შესრულება ტრანზაქციის გარეშე შეუძლებელია.';ru='Транзакция активна. Выполнение запроса вне транзакции невозможно.';en='The transaction is active. Fulfilling the request transaction is not possible.'"));
	КонецЕсли;
...
Процедура ПриНачалеВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено) Экспорт
	...
	Если СтандартныеПодсистемыСервер.НеобходимоОбновлениеПараметровРаботыПрограммы() Тогда
		ВызватьИсключение
			НСтр("ka='პროგრამაში შესვლა დროებით შეუძლებელია ახალი ვერსიით განახლების გამო."
"განახლების დროისთვის რეკომენდებულია სარეგლამენტო დავალებების შესრულების აკრძალვა.';ru='Вход в программу временно невозможен в связи с обновлением на новую версию."
"Рекомендуется запрещать выполнение регламентных заданий на время обновления.'");
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   ...
	   И ЗначениеЗаполнено(Константы.ГлавныйУзел.Получить()) Тогда
		...
		ВызватьИсключение
			НСтр("ka='პროგრამაში შესვლა დროებით შეუძლებელია მთავარ კვანძთან კავშირის აღდგენამდე."
"რეკომენდებულია, რომ აღდგენის პროცესში აიკრძალოს სარეგლამენტო დავალებების შესრულება.';ru='Вход в программу временно невозможен до восстановления связи с главным узлом."
...
"Выполнение прервано.';ka='სარეგლამენტო დავალება ფუნქციური ოპციებით მიუწვდომელია ან პროგრამის მიმდინარე რეჟიმში მუშაობას მხარს არ უჭერს. შესრულება შეწყვეტილია.'");
		КонецЕсли;
...
Функция ИнтервалВремениСтрокой(ВремяНачала, ВремяОкончания = Неопределено) Экспорт
	...
	ИначеЕсли ВремяНачала > ВремяОкончания Тогда
		ВызватьИсключение НСтр("ka='ინტერვალის დასრულების თარიღი არ შეიძლება, რომ დაწყების თარიღზე ნაკლები იყოს.';ru='Дата окончания интервала не может быть меньше даты начала.';en='The end date of the interval can not be less than the start date.'");
	КонецЕсли;
	...
	Если ВеличинаИнтервалаВДнях > 365 Тогда
		ОписаниеИнтервала = НСтр("ka='თვეზე მეტი';ru='более месяца';en='more than a month'");
	...
	Иначе
		ОписаниеИнтервала = НСтр("ka='ერთ დღეზე ნაკლები';ru='менее одного дня';en='less than one day'");
	КонецЕсли;
...
Функция ДеревоМетаданныхКонфигурации(Отбор = Неопределено) Экспорт
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("Константы",               НСтр("ka='კონსტანტები';ru='Константы';en='constants'"),                 БиблиотекаКартинок.Константа,              БиблиотекаКартинок.Константа,                    КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Справочники",             НСтр("ka='ცნობარები';ru='Справочники';en='References'"),               БиблиотекаКартинок.Справочник,             БиблиотекаКартинок.Справочник,                   КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Документы",               НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'"),                 БиблиотекаКартинок.Документ,               БиблиотекаКартинок.ДокументОбъект,               КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовХарактеристик", НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик';en='Plans species'"), БиблиотекаКартинок.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристикОбъект, КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыСчетов",             НСтр("ka='ანგარიშეგების გეგმები';ru='Планы счетов';en='Charts of accounts'"),              БиблиотекаКартинок.ПланСчетов,             БиблиотекаКартинок.ПланСчетовОбъект,             КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовРасчета",       НСтр("ka='ანგარიშსწორებების ტიპების გეგმები';ru='Планы видов расчета';en='Plans of calculation types'"),       БиблиотекаКартинок.ПланВидовРасчета,       БиблиотекаКартинок.ПланВидовРасчетаОбъект,       КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыСведений",        НСтр("ka='ცნობების რეგისტრები';ru='Регистры сведений';en='registers information'"),         БиблиотекаКартинок.РегистрСведений,        БиблиотекаКартинок.РегистрСведений,              КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыНакопления",      НСтр("ka='დაგროვების რეგისტრები';ru='Регистры накопления';en='accumulation registers'"),       БиблиотекаКартинок.РегистрНакопления,      БиблиотекаКартинок.РегистрНакопления,            КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыБухгалтерии",     НСтр("ka='ბუღალტერიის რეგისტრები';ru='Регистры бухгалтерии';en='accounting registers'"),      БиблиотекаКартинок.РегистрБухгалтерии,     БиблиотекаКартинок.РегистрБухгалтерии,           КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыРасчета",         НСтр("ka='ანგარიშსწორების რეგისტრები';ru='Регистры расчета';en='registers calculation'"),          БиблиотекаКартинок.РегистрРасчета,         БиблиотекаКартинок.РегистрРасчета,               КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("БизнесПроцессы",          НСтр("ka='ბიზნეს-პროცესები';ru='Бизнес-процессы';en='Business processes'"),           БиблиотекаКартинок.БизнесПроцесс,          БиблиотекаКартинок.БизнесПроцессОбъект,          КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Задачи",                  НСтр("ka='ამოცანები';ru='Задачи';en='Tasks'"),                    БиблиотекаКартинок.Задача,                 БиблиотекаКартинок.ЗадачаОбъект,                 КоллекцииОбъектовМетаданных);
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მეტამონაცემთა ობიექტის უცნობი ტიპი ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';en='Unknown type of metadata object ""%1""'"), ПолноеИмя);
...
Функция ОбъектПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если МассивСтрок.Количество() >= 2 Тогда
		...
		Имя = МассивСтрок[1];
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ka='ანგარიშგების ან დამუშავების არაკორექტული სრული სახელი ""%1"".';ru='Некорректное полное имя отчета или обработки ""%1"".';en='Incorrect report or the full name of the processing ""%1"".'"), "%1", ПолноеИмя);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ka='""%1"" არ არის ანგარიშგება ან დამუშავება.';ru='""%1"" не является отчетом или обработкой.';en='""%1"" is not a report or treatment.'"), "%1", ПолноеИмя);
	КонецЕсли;
КонецФункции
...
Функция ВидОбъектаПоТипу(Тип) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრის მნიშვნელობის არასწორი ტიპი';ru='Неверный тип значения параметра (%1)';en='Invalid type parameter value (1%)'"), Строка(Тип));
...
Процедура ЗаблокироватьИБ(Знач ПроверитьОтсутствиеДругихСеансов = Истина) Экспорт
	...
	Иначе
		...
		Иначе
			ВызватьИсключение(НСтр("ka='ქვესისტემა ""მუშაობა სერვისის მოდელში"" მიუწვდომელია';ru='Подсистема ""Работа в модели сервиса"" не доступна';en='Subsystem ""Employment in the service model"" is not available'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьВерсииИнтерфейса(Знач Адрес, Знач Пользователь, Знач Пароль = Неопределено, Знач Интерфейс = Неопределено) Экспорт
	...
	Если Не ПараметрыПодключения.Свойство("URL") 
		...
		ВызватьИсключение(НСтр("ka='სერვისის URL მიცემული არ არის.';ru='Не задан URL сервиса.';en='Do not set the service URL.'"));
	КонецЕсли;
...
Функция ПолучитьВерсииИнтерфейсаЧерезВнешнееСоединение(ВнешнееСоединение, Знач ИмяИнтерфейса) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ka='ქვესისტემების ინტერფეისების ვერსიონირება კორეპსონდენტის მიერ მხარდაჭერილი არ არის. შეცდომის აღწერილობა: %1';ru='Корреспондент не поддерживает версионирование интерфейсов подсистем."
"Описание ошибки: %1'");
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Получение версий интерфейса';ru='Получение версий интерфейса';en='Obtaining interface versions'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"параметр должен содержать ссылку; передано значение: %3 (тип %4).';ka='პარამეტრ %1-ის დაუშვებელი მნიშვნელობა %2-ში. "
"პარამეტრი უნდა შეიცავდეს ბმულს; გადაცემულია მნიშვნელობა: %3 (ტიპი %4).'"),
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Общий модуль ""%1"" не найден.';ru='Общий модуль ""%1"" не найден.';en='Total module ""%1"" not found.'"), Имя);
	КонецЕсли;
...
Функция Просклонять(Знач ФИО, Падеж, Результат, Пол = Неопределено) Экспорт
	...
	ВызватьИсключение НСтр("ka='საერთოდანიშნულების.ბრუნება-ს მოსახმობად აუცილებელია ქვესისტემა ""ობიექტების წარდგენის ბრუნება"".';ru='Для вызова ОбщегоНазначения.Просклонять необходима подсистема «Склонение представлений объектов».';en='To call ObschegoNaznacheniya.Prosklonyat need subsystem ""Declination representations of objects.""'");
...
Функция ПодготовитьДанныеКэшаВерсий(Знач ТипДанных, Знач ПараметрыПолучения) Экспорт
	...
	Иначе
		ШаблонТекста = НСтр("ka='ვერსიის ქეშის მონაცემთა უცნობი ტიპი: %1';ru='Неизвестный тип данных кэша версий: %1';en='Unknown type Cache versions: 1%'");
...
Функция WSОпределения(Знач АдресWSDL, Знач ИмяПользователя, Знач Пароль, Знач Таймаут = 10) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='WSDL-ის მიღება';ru='Получение WSDL';en='Getting WSDL'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль, Знач Таймаут)
	...
	Если НЕ ОписаниеФайла.Статус Тогда
		ВызватьИсключение(НСтр("ka='Ошибка получения файла описания web-сервиса:';ru='Ошибка получения файла описания web-сервиса:';en='Error getting file describing web-service:'") + Символы.ПС + ОписаниеФайла.СообщениеОбОшибке)
	КонецЕсли;
...
"%1';ka='Ошибка получения файла описания web-сервиса:"
"В полученном файле не содержится ни одного описания сервиса."
""
"Возможно, адрес файла описания указан неверно:"
"%1'");
...
"Данные типа ""%1"" не могут быть зафиксированы.';ka='შეცდომა საერთო მოდულის საერთოდანიშნულების ფუნქციაში ფიქსირებულიმონაცემები."
"""%1"" ტიპის მონაცემები არ შეიძლება დაფიქსირებული იყოს.'"),
		Строка(ТипДанных) );
...
"либо для него не поддерживается получение модуля менеджера.';ka='Объект метаданных ""%1"" не найден,"
...
Функция РазметитьМестаИспользования(Знач ПараметрыВыполнения, Знач Ссылка, Знач ЦелеваяСсылка, Знач ТаблицаПоиска)
	...
	Для Каждого МестоИспользования Из Результат.МестаИспользования Цикл
		...
		Иначе
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ში ბმულების შეცვლას მხარდაჭერა არ გააჩნია.';ru='Замена ссылок в ""%1"" не поддерживается.';en='Replacing references to ""%1"" is not supported.'"), Информация.ПолноеИмя);
	...
	Попытка
		Если ВнутренняяТранзакция Тогда
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კონტანტა %1-ის დაბლოკვა ვერ მოხერხდა';ru='Не удалось заблокировать константу %1';en='Unable to lock the constant 1%'"), 
					ПредставлениеДанных);
		...
		Исключение
			...
			Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ის ჩაწერა ვერ მოხერხდა შემდეგი მიზეზით: %2';ru='Не удалось записать %1 по причине: %2';en='Could not write% 1 due to:% 2'"), 
		...
		Если ВнутренняяТранзакция Тогда
			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ობიექტ ""%1""-ის დაბლოკვა ვერ მოხერხდა:"
"%2';ru='Не удалось заблокировать объект ""%1"":"
"%2'"),
					ПредставлениеДанных,
		...
		Исключение
			СостояниеОперации = "ОшибкаЗаписи";
			ОписаниеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ის ჩაწერა ვერ მოხერხდა შემდეგი მიზეზით: %2';ru='Не удалось записать %1 по причине: %2';en='Could not write% 1 due to:% 2'"), 
				ПредставлениеДанных, ОписаниеОшибки);
			ВызватьИсключение;
		КонецПопытки;
	...
	Попытка
		...
		Если ВнутренняяТранзакция Тогда
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось заблокировать набор %1';en='Unable to lock a set of 1%'"), 
					ПредставлениеДанных);
				СостояниеОперации = "ОшибкаБлокировки";
				ВызватьИсключение;
			КонецПопытки;
	...
	Попытка
		...
		Для Каждого Ссылка Из Удаляемые Цикл
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				ТекстОшибки = НСтр("ka='წაშლის შეცდომა';ru='Ошибка удаления';en='Failed to delete'")
					+ Символы.ПС
...
Процедура ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, ТаблицаПовторногоПоиска)
	...
	Для Каждого Строка Из ТаблицаПовторногоПоиска Цикл
		...
		ЗарегистрироватьОшибкуЗамены(Результат, Ссылка, 
			...
			НСтр("ka='გამოყენების ყველა ადგილი არ არის შეცვლილი. შეიძლება, რომ გამოყენების ადგილები დამატებული იქნა ან შეიცვალა სხვა მომხმარებლის მიერ.';ru='Заменены не все места использования. Возможно места использования были добавлены или изменены другим пользователем.';en='Replaced, not all of use. Perhaps the use of space has been added or changed by another user.'")));
	КонецЦикла;
...
Функция ОписаниеИзмеренийНабора(Знач Мета, Кэш)
	...
	Если Метаданные.РегистрыСведений.Содержит(Мета) Тогда
		...
		Если МетаПериод = Периодичность.ПозицияРегистратора Тогда
			...
			ДанныеИзмерения.Представление = НСтр("ka='რეგისტრატორი';ru='Регистратор';en='recorder'");
		...
		ИначеЕсли МетаПериод = Периодичность.Год Тогда
			...
			ДанныеИзмерения.Представление = НСтр("ka='პერიოდი';ru='Период';en='Period'");
...
Процедура ЗаписатьОбъект(Знач Объект, Знач ПараметрыЗаписи)
	...
	Если ТестРеквизитов.Иерархический Или ТестРеквизитов.ВидыСубконто <> Неопределено Тогда 
		...
		Если Объект.Родитель = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='""%1""-ის ჩაწერისას წარმოიქმნება ციკლური ბმული იერარქიაში.';ru='При записи ""%1"" возникает циклическая ссылка в иерархии.';en='When recording ""%1"" occurs in the circular reference hierarchy.'"),
				Строка(Объект));
			КонецЕсли;
	...
	Для Каждого МетаВладелец Из ТестРеквизитов.Владельцы Цикл
		...
		Если Объект.Владелец = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='""%1""-ის ჩაწერისას წარმოიშვება ციკლური ბმული დაქვემდებარებაში.';ru='При записи ""%1"" возникает циклическая ссылка в подчинении.';en='When recording ""%1"" occurs in the circular reference command.'"),
				Строка(Объект));
		КонецЕсли;
		Прервать;
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	Возврат НСтр("ka='ბმულების ძებნა და წაშლა';ru='Поиск и удаление ссылок';en='Find and delete links'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗаменитьСсылкуОднойТранзакцией(Результат, Знач Дубль, Знач ПараметрыВыполнения, Знач ТаблицаПоиска)
	...
	Исключение
		...
		Если СостояниеОперации = "ОшибкаБлокировки" Тогда
			...
			Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გამოყენების ყველა ადგილის დაბლოკვა ვერ მოხერხდა %1';ru='Не удалось заблокировать все места использования %1:';en='Unable to block all use of space% 1:'") 
...
Процедура ЗарегистрироватьОшибкуВТаблицу(Результат, Дубль, Оригинал, Данные, Информация, ТипОшибки, ИнформацияОбОшибке)
	...
	Если ТипОшибки = "БлокировкаДляРегистра" Тогда
		НовыйШаблон = НСтр("ka='რედაქტირების დაწყება ვერ მოხერხდა %1: %2';ru='Не удалось начать редактирование %1: %2';en='Failed to start editing% 1% 2'");
		Ошибка.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "БлокировкаДляУдаленияДубля" Тогда
		НовыйШаблон = НСтр("ka='წაშლის დაწყება ვერ მოხერხდა: %2';ru='Не удалось начать удаление: %2';en='Unable to start the removal of% 2'");
		Ошибка.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "УдалитьНаборДубля" Тогда
		НовыйШаблон = НСтр("ka='დუბლის შესახებ ინფორმაციის გასუფთავება ვერ მოხერხდა %1-ში:%2';ru='Не удалось очистить сведения о дубле в %1: %2';en='Unable to clear information about duplicate in% 1% 2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	ИначеЕсли ТипОшибки = "ЗаписатьНаборОригинала" Тогда
		НовыйШаблон = НСтр("ka='ინფორმაციის განახლება %1-ში ვერ მოხერხდა: %2';ru='Не удалось обновить сведения в %1: %2';en='Failed to update the information in% 1% 2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	Иначе
	...
	НовыйШаблон = НовыйШаблон + Символы.ПС + Символы.ПС + НСтр("ka='დაწვრილებით, იხილეთ რეგისტრაციის ჟურნალში.';ru='Подробности в журнале регистрации.';en='The details in the log.'");
...
Функция ВычислитьЗначениеДанныхПоПути(Знач Данные, Знач ПутьДанных)
	...
	Для Каждого ИмяРеквизита Из Имена Цикл
		...
		Если Тест[ИмяРеквизита] = Неопределено Тогда
			...
			ЗаполнитьЗначенияСвойств(Тест, Результат);
			Если Тест[ИмяРеквизита] = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ობიექტ %2-ისთვის %1-ის გამოთვლის შეცდომა. რეკვიზიტი %3 ვერ მოიძებნა';ru='Ошибка вычисления %1 для объекта %2. Реквизит %3 не найден';en='Error computation% 1% 2 for the object. Props 3% can not be found'"),
...
//+++ Дмгг
Процедура УстановитьФильтрСпискаДокументовПоОсновномуСкладу(Список, ИмяДокумента = "") Экспорт
	...
	ЗначениеОсновногоСклада = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "ДоступныеСклады");
	МассивСкладов = ЗначениеОсновногоСклада.СписокСкладов.ВыгрузитьКолонку("Склад");
	
	Если Список = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Если ИмяДокумента <> "ПеремещениеЗапасов" И ИмяДокумента <> "ВнутреннийЗаказ"  Тогда 
	...
	ИначеЕсли ИмяДокумента = "ЗаказПокупателя" Тогда 
		ЭлементОтбора = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	    ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СтруктурнаяЕдиницаПродажи");
	    ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
	    ЭлементОтбора.Использование = Истина;
	    ЭлементОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	    ЭлементОтбора.ПравоеЗначение = МассивСкладов;
	Иначе
		...
		ЭлементОтбора = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СтруктурнаяЕдиницаПолучатель");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
		ЭлементОтбора.Использование = Истина;
		ЭлементОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
		ЭлементОтбора.ПравоеЗначение = МассивСкладов;
...
//+++Lazare
Функция ПолучитьГруппыНоменклатурыИзНастроекПользователя(настройка) Экспорт
		
	Пользователь 	  = Пользователи.ТекущийПользователь();	
	ЗначениеНастроики = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, настройка);
	МассивГрупп		  = ЗначениеНастроики.СписокГрупп.ВыгрузитьКолонку("Группа");
	
	Возврат МассивГрупп ;
	
КонецФункции

Функция ЕстьТТН(Источник) Экспорт
	...
	|ГДЕ
	|	РС_ЭлектроннаяНакладная.ПометкаУдаления = ЛОЖЬ
	|	И &Источник В (РС_ЭлектроннаяНакладная.ДокументыОснования.ДокументОснование)
	|	И (РС_ЭлектроннаяНакладная.СтатусТН = ЗНАЧЕНИЕ(Перечисление.РС_СтатусыТН.Активная)
	|			ИЛИ РС_ЭлектроннаяНакладная.СтатусТН = ЗНАЧЕНИЕ(Перечисление.РС_СтатусыТН.Завершеная))";    
...
//Малышев 2026-06-26 - сервисная для отладки
//Пареметры
//   ИсточникТаблиц - то, что содержит МенеджерВременныхТаблиц. Запрос или структура, например из проведения документов
//   ИмяВременнойТаблицы - возврат одной конкретной таблицы - не сделано
Функция ПолучитьВременныеТаблицыИсточника(ИсточникТаблиц, ЗаписатьТаблицуПоПути = "") Экспорт
	ИтоговаяСтруктура = Новый Структура;
	
	МенеджерВременныхТаблиц = Неопределено;
	
	ИндексСтроки = 0;
	Если ТипЗнч(ИсточникТаблиц) = Тип("Запрос") Тогда
		Если ИсточникТаблиц.МенеджерВременныхТаблиц<>Неопределено Тогда
			МенеджерВременныхТаблиц = ИсточникТаблиц.МенеджерВременныхТаблиц; 
			
		КонецЕсли;
		
		ИмяКлюча = "п" + Формат(ИндексСтроки,"ЧЦ=2; ЧН=; ЧВН=; ЧГ=") + "_ПАРАМЕТРЫ_"+Формат(ИсточникТаблиц.Параметры.Количество(),"ЧН=; ЧВН=; ЧГ=");
		ИтоговаяСтруктура.Вставить(ИмяКлюча, ИсточникТаблиц.Параметры);
	
		
	ИначеЕсли ТипЗнч(ИсточникТаблиц) = Тип("Структура") Тогда
		МенеджерВременныхТаблиц = ИсточникТаблиц.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	КонецЕсли;	
	
   	ИндексСтроки = 0;
	Если МенеджерВременныхТаблиц<>Неопределено Тогда
		Для каждого СтрТаблицы из ИсточникТаблиц.МенеджерВременныхТаблиц.Таблицы Цикл 
			ДанныеТаблицы = СтрТаблицы.ПолучитьДанные();
			Если ДанныеТаблицы <> Неопределено Тогда
				Структураданных = Новый Структура;
				Структураданных.Вставить("Индекс", ИндексСтроки);
				Структураданных.Вставить("Данные", ДанныеТаблицы.Выгрузить());
				Структураданных.Вставить("КоличествоСтрок", Структураданных.Данные.Количество());
				Структураданных.Вставить("ИмяВременнойТаблицы", СтрТаблицы.ПолноеИмя);
				ИмяКлюча = "т" + Формат(ИндексСтроки,"ЧЦ=2; ЧН=; ЧВН=; ЧГ=") + "_" + СтрТаблицы.ПолноеИмя + "_"+Формат(Структураданных.Данные.Количество(),"ЧН=; ЧВН=; ЧГ=");
				ИтоговаяСтруктура.Вставить(ИмяКлюча, Структураданных);
			КонецЕсли;          
	...
	Если ТипЗнч(ИсточникТаблиц) = Тип("РезультатЗапроса") Тогда
		Структураданных = Новый Структура;
		Структураданных.Вставить("Индекс", ИндексСтроки+1);
		Структураданных.Вставить("Данные", ИсточникТаблиц.Выгрузить());
		Структураданных.Вставить("КоличествоСтрок", Структураданных.Данные.Количество());
		Структураданных.Вставить("ИмяВременнойТаблицы", "РЕЗУЛЬТАТ_ЗАПРОСА_0");
		ИмяКлюча = "к" + Формат(ИндексСтроки,"ЧЦ=2; ЧН=; ЧВН=") + "_РЕЗУЛЬТАТ_ЗАПРОСА_0_"+Формат(Структураданных.Данные.Количество(),"ЧН=; ЧВН=");
		ИтоговаяСтруктура.Вставить(ИмяКлюча, Структураданных);
	КонецЕсли;
	
		
	
	Если ЗаписатьТаблицупоПути<>"" Тогда 
		ТекущаяСтрока = 1; 
		Макет = Новый ТабличныйДокумент;
		
		Если ТипЗнч(ИсточникТаблиц) = Тип("Запрос") Тогда
			Макет.Область(ТекущаяСтрока, 2).Текст = "Параметры запроса";
			Макет.Область(ТекущаяСтрока, 2).Шрифт = Новый Шрифт(Макет.Область(ТекущаяСтрока, 2).Шрифт,,15,Истина);
			ТекущаяСтрока = ТекущаяСтрока + 1;
				
			Макет.НачатьГруппуСтрок("1", Истина);
			Макет.Область(ТекущаяСтрока, 2).Текст = "Ключ";		
		   	Макет.Область(ТекущаяСтрока, 2).ЦветФона = WebЦвета.СветлоСерый;
			Макет.Область(ТекущаяСтрока, 3).Текст = "Значение";		
		   	Макет.Область(ТекущаяСтрока, 3).ЦветФона = WebЦвета.СветлоСерый;
			Макет.Область(ТекущаяСтрока, 4).Текст = "Тип";		
		   	Макет.Область(ТекущаяСтрока, 4).ЦветФона = WebЦвета.СветлоСерый;
			ТекущаяСтрока = ТекущаяСтрока + 1;
				
			Для каждого ДанныеПараметров из ИсточникТаблиц.Параметры Цикл
				Макет.Область(ТекущаяСтрока, 2).Текст = ДанныеПараметров.Ключ;		
			   	Макет.Область(ТекущаяСтрока, 3).Текст = ДанныеПараметров.Значение;		
				Макет.Область(ТекущаяСтрока, 4).Текст = ТипЗнч(ДанныеПараметров.Значение);		
				ТекущаяСтрока = ТекущаяСтрока + 1;
			КонецЦикла;
		...
		Для каждого ДанныеТаблицы из ИтоговаяСтруктура Цикл
			
			Если лев(ДанныеТаблицы.Ключ,1)<>"т" Тогда
				Продолжить;
			КонецЕсли;	
				
			Макет.Область(ТекущаяСтрока, 1).Текст = Формат(ДанныеТаблицы.Значение.Индекс,"ЧН=");		
			Макет.Область(ТекущаяСтрока, 1).Шрифт = Новый Шрифт(Макет.Область(ТекущаяСтрока, 2).Шрифт,,12,Истина);
			Макет.Область(ТекущаяСтрока, 2).Текст = ДанныеТаблицы.Значение.ИмяВременнойТаблицы;
			Макет.Область(ТекущаяСтрока, 2).Шрифт = Новый Шрифт(Макет.Область(ТекущаяСтрока, 2).Шрифт,,15,Истина);
			
			Макет.Область(ТекущаяСтрока, 8).Текст = ДанныеТаблицы.Значение.КоличествоСтрок;		
			Макет.Область(ТекущаяСтрока, 8).Шрифт = Новый Шрифт(Макет.Область(ТекущаяСтрока, 2).Шрифт,,15,Истина);
			Макет.Область(ТекущаяСтрока, 9).Текст = "- Количество:";		
		    Макет.Область(ТекущаяСтрока, 9).Шрифт = Новый Шрифт(Макет.Область(ТекущаяСтрока, 2).Шрифт,,15,Истина);
			
			Макет.НачатьГруппуСтрок("1", Ложь);
			
			ТекущаяСтрока = ТекущаяСтрока + 1;
			Таб = ДанныеТаблицы.Значение.Данные;
			ТекКолонка = 1;
			Макет.Область(ТекущаяСтрока, ТекКолонка).Текст = "Индекс";		
		   	Макет.Область(ТекущаяСтрока, ТекКолонка).ЦветФона = WebЦвета.СветлоСерый;
			ТекКолонка = ТекКолонка + 1;
			
			
			Для каждого СтрКолонки из Таб.Колонки Цикл
				Макет.Область(ТекущаяСтрока, ТекКолонка).Текст = СтрКолонки.Имя;		
		   		Макет.Область(ТекущаяСтрока, ТекКолонка).ЦветФона = WebЦвета.СветлоСерый;
				//Макет.Область(ТекущаяСтрока, ТекКолонка).ГраницаСверху = истина;
				//Макет.Область(ТекущаяСтрока, ТекКолонка).ГраницаСправа = истина;
				ТекКолонка = ТекКолонка + 1;
			КонецЦикла;
			ТекущаяСтрока = ТекущаяСтрока + 1;
			...
			Для каждого строкаТаблицы из Таб Цикл
				ТекКолонка = 1;
				Макет.Область(ТекущаяСтрока, ТекКолонка).Текст = НомерСтрокиТаблицы;		
		   		ТекКолонка = ТекКолонка + 1;
				Для каждого СтрКолонки из Таб.Колонки Цикл
					Макет.Область(ТекущаяСтрока, ТекКолонка).Текст = строкаТаблицы[СтрКолонки.Имя];		
		   			ТекКолонка = ТекКолонка + 1;
				КонецЦикла;
		...
		КонецЦикла;
		Макет.Записать(ЗаписатьТаблицупоПути);
		Сообщить("Записан файл " + ЗаписатьТаблицупоПути);
	КонецЕсли;
