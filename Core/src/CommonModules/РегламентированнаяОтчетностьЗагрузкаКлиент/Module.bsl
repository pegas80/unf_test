...
Процедура ЗагрузитьИзФайлаОтчетПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Не предусмотрена загрузка данных в отчет ""%1"".';ru='Не предусмотрена загрузка данных в отчет ""%1"".'"), ПредставлениеДокументаРеглОтч);
			Сообщение.Сообщить();
			Возврат;
		КонецПопытки;
	...
	Если ФормаОтчета = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось получить заполненную по умолчанию форму отчета ""%1"". %2';ru='Не удалось получить заполненную по умолчанию форму отчета ""%1"". %2'"), ПредставлениеДокументаРеглОтч, ОписаниеОшибки);
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	ТекстКомментария = НСтр("ka='Данные отчета загружены из файла.';ru='Данные отчета загружены из файла.'");
	...
	Если НовыйСохраненныйДок = Неопределено ИЛИ НовыйСохраненныйДок.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось создать документ отчета для загрузки данных из файла в ""%1"".';ru='Не удалось создать документ отчета для загрузки данных из файла в ""%1"".'"), ПредставлениеДокументаРеглОтч);
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если Результат.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС") Тогда
		...
		Если ФормаКонсолидацииОтчетностиПоНДС = Неопределено Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Не удалось загрузить данные с помощью обработки ""Консолидация отчетности по НДС"".';ru='Не удалось загрузить данные с помощью обработки ""Консолидация отчетности по НДС"".'");
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
		...
		Если Результат.Свойство("СообщениеОбОшибке", СообщениеОбОшибке) И СообщениеОбОшибке <> Неопределено Тогда
			Если ТипЗнч(СообщениеОбОшибке) = Тип("Структура") Тогда
				...
				Иначе
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Загрузка данных из файла в ""%1"" не выполнена.';ru='Загрузка данных из файла в ""%1"" не выполнена.'"), ПредставлениеДокументаРеглОтч);
					Сообщение.Сообщить();
				КонецЕсли;
			Иначе
				Сообщение = Новый СообщениеПользователю;
...
Процедура ПолучениеФайловДляЗагрузкиНачало(Адрес, ФайлыДляЗагрузки = "", ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстСообщения = НСтр("ka='Для загрузки файлов электронных представлений необходимо установить расширение работы с файлами.';ru='Для загрузки файлов электронных представлений необходимо установить расширение работы с файлами.'");
		...
		Если ЭтоФайлВыгрузкиУведомленияОКонтролируемыхСделках
		   И ОсновнойФайлНаДиске.Размер > МаксимальныйРазмер Тогда
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ka='საგადასახადო სამსახურის მოთხოვნის თანახმად, 5.01 ფორმატის ფაილი, რომელიც %2 მბ-ზე მეტი არის, დაყოფილი უნდა იქნას ფორმატ 5.02-ის ფაილებად.%1ჩავტვირთოთ ფაილი გაცალკევებით?';ru='Согласно требованиям ФНС файл формата 5.01 больше %2 Мб должен быть разбит на файлы формата 5.02.%1Загрузить файл с разбивкой?'"),
				Символы.ПС,
				Строка(РазмерВМб));
		...
		Иначе
			...
			Если ФормаЭлектронногоПредставления = Неопределено Тогда
				...
				Сообщение.Текст = СтрШаблон(
					НСтр("ka='Не удалось открыть форму электронного представления для файла: ""%1"".';ru='Не удалось открыть форму электронного представления для файла: ""%1"".'"),
...
Функция ПолучитьДиалогВыбораФайла()
	...
	ДиалогВыбора.Заголовок = НСтр("ka='Выберите файл(ы) для загрузки';ru='Выберите файл(ы) для загрузки'");
	ДиалогВыбора.МножественныйВыбор = Истина;
	ДиалогВыбора.ПроверятьСуществованиеФайла = Истина;
	ДиалогВыбора.Фильтр = НСтр("ka='ყველა ფაილი';ru='Все файлы'") + " (*.*)|*.*";
...
Функция ПолучитьЗаполненнуюПоУмолчаниюФормуОтчета(Знач ПолноеИмяФормыОтчета, ПараметрыФормыОтчета)
	...
	ИначеЕсли ПараметрыФормыОтчета = "Форма отчета не найдена" Тогда
		ПоказатьПредупреждение(, НСтр("ka='Форма отчета не найдена в выбранном периоде!';ru='Форма отчета не найдена в выбранном периоде!'"));
		Возврат Неопределено;
	...
	Если (ПараметрыФормыОтчета.Организация = Неопределено ИЛИ ПараметрыФормыОтчета.Организация.Пустая())
	   И (ПараметрыФормыОтчета.Свойство("ОрганизацияЮЛ"))Тогда
		Если ПараметрыФормыОтчета.ОрганизацияЮЛ = Неопределено ИЛИ ПараметрыФормыОтчета.ОрганизацияЮЛ.Пустая() Тогда
			ПоказатьПредупреждение(, НСтр("ka='Не удалось подобрать организацию для формы отчета!';ru='Не удалось подобрать организацию для формы отчета!'"));
			Возврат Неопределено;
		Иначе
	...
	Если ВыбраннаяФормаОтчета = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не удалось получить заполненную по умолчанию форму отчета!';ru='Не удалось получить заполненную по умолчанию форму отчета!'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция РеквизитОбъектаЕслиСуществует(Объект, ИмяРеквизита)
	...
	Если НЕ РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Объект, ИмяРеквизита) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не найдено свойство ""%1"" объекта ""%2"".';ru='Не найдено свойство ""%1"" объекта ""%2"".'"), ИмяРеквизита, Строка(Объект));
