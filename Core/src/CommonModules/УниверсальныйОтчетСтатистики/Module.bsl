...
Процедура ПодготовитьДанныеОтчетаПоШаблону(КонтекстФормы, ДанныеШаблонаОтчета, Отказ = Ложь) Экспорт 
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის ტექსტის მიღება ვერ მოხერხდა:%1';ru='Не удалось получить текст XML-шаблона:%1'"), Символы.ПС + ОписаниеОшибки());
		Сообщение.Сообщить();
