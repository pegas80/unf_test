...
Функция ВыгрузитьДанныеВАрхив(Знач ПараметрыВыгрузки) Экспорт
	...
	Если Не ПроверитьНаличиеПрав() Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა გადმოსატვირთად არასაკმარისი დაშვების უფლებებია!';ru='Недостаточно прав доступа для выгрузки данных!';en='It is not enough access rights to download data!'");
	КонецЕсли;
...
Процедура ЗагрузитьДанныеИзАрхива(Знач ИмяАрхива, Знач ПараметрыЗагрузки) Экспорт
	...
	Если Не ПроверитьНаличиеПрав() Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა ჩასატვირთად არასაკმარისი დაშვების უფლებებია!';ru='Недостаточно прав доступа для загрузки данных!';en='It is not enough access rights to download data!'");
	КонецЕсли;
...
Функция ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(Знач ИмяАрхива) Экспорт
	...
	Попытка
		...
		Если ЭлементОписанияВыгрузки = Неопределено Тогда
			ВызватьИсключение НСтр("ka='გადმოტვირთვის ფაილში არ არის ფაილი DumpInfo.xml!';ru='В файле выгрузки отсутствует файл DumpInfo.xml!';en='The file upload file is missing DumpInfo.xml!'");
		КонецЕсли;
