...
Процедура ЗаписатьДанныеВRFID(Результат, Параметры) Экспорт
	...
	Если ДанныеСерии = Неопределено Тогда
		ТекстСообщения = НСтр("ka='Выберите строку, по которой нужно записать метку и положите соотвествующую метку на устройство.';ru='Выберите строку, по которой нужно записать метку и положите соотвествующую метку на устройство.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		НужноЗаписатьМетку = Ложь;
	ИначеЕсли Не ДанныеСерии.НужноЗаписатьМетку Тогда
		ТекстСообщения = НСтр("ka='По текущей строке не нужно записывать метку.';ru='По текущей строке не нужно записывать метку.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		НужноЗаписатьМетку = Ложь;
	ИначеЕсли  НЕ ЗначениеЗаполнено(ДанныеСерии.НомерКИЗГИСМ) Тогда
		ТекстСообщения = НСтр("ka='Перед записью метки, укажите номер КиЗ.';ru='Перед записью метки, укажите номер КиЗ.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		НужноЗаписатьМетку = Ложь;
	ИначеЕсли Истина
		И Не ЗначениеЗаполнено(ДанныеСерии.НомерГИСМ) Тогда
		ТекстСообщения = НСтр("ka='Перед записью метки, укажите номер серии.';ru='Перед записью метки, укажите номер серии.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		НужноЗаписатьМетку = Ложь;
	КонецЕсли;	
...
Процедура ОтработатьТаймаутОжиданияСчитыванияМетки(Форма) Экспорт
	...
	ТекстСообщения = НСтр("ka='Истекло время ожидания чтения RFID-метки. Поднесите RFID-метку к считывателю и повторите попытку.';ru='Истекло время ожидания чтения RFID-метки. Поднесите RFID-метку к считывателю и повторите попытку.'");
...
Процедура ЗавершениеОперацииСчитывателяRFID(РезультатВыполнения, ПараметрыОперацииСчитывателяRFID) Экспорт
	...
	Если Не РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='Операция считывателя RFID завершилась с ошибкой:""%ОписаниеОшибки%"".';ru='Операция считывателя RFID завершилась с ошибкой:""%ОписаниеОшибки%"".'");
