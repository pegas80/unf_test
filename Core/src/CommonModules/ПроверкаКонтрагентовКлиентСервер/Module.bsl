...
Функция ПодсказкаВДокументе(ПараметрыПрорисовки, СостояниеПроверки) Экспорт
	...
	РекламаСервиса = Новый ФорматированнаяСтрока(
		НСтр("ka='შეამოწმეთ კონტრაგენტი ქვემენიუ ""კიდევ""-ში არსებული ღილაკით';ru='Проверьте контрагента по кнопке в подменю Еще'"));
	...
	ИначеЕсли СостояниеПроверки = ПредопределенноеЗначение("Перечисление.СостоянияПроверкиКонтрагентов.ПроверкаВПроцессе") Тогда
		...
		Подстроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='საგადასახადო სამსახურის მონაცემთა მიხედვით მიმდინარეობს კონტრაგენტების შემოწმება';ru='Выполняется проверка контрагентов согласно данным ФНС'")));
...
"საგადასახადო სამსახურის სერვისი დროებით მიუწვდომელია'")));
	...
	ИначеЕсли СостояниеПроверки = ПредопределенноеЗначение("Перечисление.СостоянияПроверкиКонтрагентов.ПроверкаВыполнена")
		...
		Если НЕ КонтрагентЗаполнен Тогда
			...
			Подстроки.Добавить(НСтр("ka='საგადასახადო სამსახურის ბაზაში კონტრაგენტის შემოწმება არ შესრულებულა: კონტრაგენტი არ არის შევსებული';ru='Проверка контрагента по базе ФНС не выполнена: не заполнен контрагент'"));
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентНеПодлежитПроверке") Тогда
			...
			Подстроки.Добавить(НСтр("ka='საგადასახადო სამსახურის ბაზაში კონტრაგენტის შემოწმება არ შესრულებულა: შემოწმებას ექვემდებარებიან მხოლოდ საქართველოს კონტრაგენტები';ru='Проверка контрагента по базе ФНС не выполнена: проверке подлежат только российские контрагенты'"));
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.ПустойИННИлиКПП") Тогда
			...
			Подстроки.Добавить(НСтр("ka='გსნ ან აამკ არ არის შევსებული';ru='Не заполнен ИНН или КПП'"));
		...
		ИначеЕсли ПроверкаКонтрагентовКлиентСерверПовтИсп.ЭтоСостояниеНедействующегоКонтрагента(СостояниеКонтрагента) Тогда
			...
			Если КонтрагентовНесколько Тогда
				// Выводим обобщенно
				Подстроки.Добавить(НСтр("ka='საგადასახადო სამსახურის მონაცემებით აღმოჩენილია უმოქმედო კონტრაგენტები';ru='Обнаружены недействующие контрагенты по данным ФНС'"));
			Иначе
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных") Тогда
			...
			Подстроки.Добавить(НСтр("ka='გსნ-ის, აამკ-ის ან დოკუმენტის თარიღის შევსებაში აღმოჩენილია შეცდომები';ru='Обнаружены ошибки в заполнении ИНН, КПП или даты документа'"));
			ЦветФона = ЦветНекорректногоКонтрагента;
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентЕстьВБазеФНС") Тогда
			...
			Если КонтрагентовНесколько Тогда
				// Выводим обобщенно 
				Подстроки.Добавить(НСтр("ka='კონტრაგენტების შემოწმება საგადასახადო სამსახურის მონაცემებით წარმატებით შესრულდა';ru='Проверка контрагентов по данным ФНС выполнена успешно'"));
			Иначе
				// Выводим конкретное состояние.
				Подстроки.Добавить(Строка(СостояниеКонтрагента));
			КонецЕсли;
...
Функция СтрокаСРезультатомПроверкиКонтрагента(Форма)
	...
	Если ЭтоФизическоеЛицо 
		...
		СтрокаСРезультатом = НСтр("ka='არ მოქმედებს';ru='Не действует'");
	Иначе
