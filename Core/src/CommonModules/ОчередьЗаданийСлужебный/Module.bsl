		...
		Пока Выборка.Следующий() Цикл 
			Попытка
				...
				Если ЗначениеЗаполнено(Выборка.Шаблон)
					...
					ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %1-ით რიგის დავალების შაბლონი ვერ მოიძებნა';ru='На найден шаблон задания очереди с идентификатором %1'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Выборка.Шаблон);
					ЗаписьЖурналаРегистрации(НСтр("ka='დავალებების რიგი. შესრულება';ru='Очередь заданий.Выполнение'", 
...
"%3'");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='სარეგლამენტო დავალებების რიგი. შეცდომების დამამუშავებლის შეცდომა';ru='Очередь регламентных заданий.Ошибка обработчика ошибок'", 
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
"Уровень рекурсии: %2'");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='სარეგლამენტო დავალებების რიგი. შეცდომების დამუშავების დავალებების მოხსნა';ru='Очередь регламентных заданий.Снятие заданий обработки ошибок'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Если ТранзакцияАктивна() Тогда
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებელ %1-ის შესრულების სტარტამდე არის აქტიური ტრანზაქციები!';ru='Перед стартом выполнения обработчика %1 есть активные транзакции!'"),
				ИмяМетода);
			
		ЗаписьЖурналаРегистрации(НСтр("ka='სარეგლამენტო დავალებების რიგი. შესრულება';ru='Очередь регламентных заданий.Выполнение'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
	...
	Попытка
		...
		Если ТранзакцияАктивна() Тогда
			...
			ШаблонСообщения = НСтр("ka='დამამუშავებლის შესრულების დასრულებისას %1 ტრანზაქცია არ დახურულა';ru='По завершении выполнения обработчика %1 не была закрыта транзакция'");
		...
		Если Не(РазделительУстановлен) И ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებელ %1-ის შესრულების დასრულებისას სეანსის დაყოფა არ გამორთულა!';ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса!'"),
				ИмяМетода);
		...
		ИначеЕсли РазделительУстановлен И ЗначениеРазделителя <> РаботаВМоделиСервиса.ЗначениеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებელ %1-ის შესრულების დასრულებისას სეანსის გამყოფის მნიშვნელობა შეიცვალა!';ru='По завершении выполнения обработчика %1 было изменено значение разделителя сеанса!'"),
				ИмяМетода);
...
Функция ПолучитьПараметрыОбработчикаОшибок(Знач Задание,Знач ИнформацияОбОшибкеВыполненияЗадания = Неопределено)
	...
	Если Результат.ОбработчикСуществует Тогда
		...
		Если ИнформацияОбОшибкеВыполненияЗадания = Неопределено Тогда
			...
			ИнформацияОбОшибкеВыполненияЗадания = КонструкторИнформацииОбОшибке(НСтр("ka='დავალება დასრულდა უცნბი შეცდომით, რომელიც შეიძლება გამოწვეული იყოს სამუშაო პროცესის ვარდნით.';ru='Задание завершилось с неизвестной ошибкой, возможно вызванной падением рабочего процесса.'"));
...
Процедура ЗаписьКонтроляВыполнения(Знач ИмяСобытия, Знач ЗаписьЗадания, Знач Комментарий = "")
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ОписаниеМетода.Шаблон) Тогда
			...
			ИмяМетода = НСтр("ka='შაბლონი';ru='Шаблон:'") + ОписаниеМетода.Шаблон;
...
Процедура ВыполнитьОбработчикЗадания(Шаблон, Псевдоним, Параметры)
	...
	Если ИмяМетода = Неопределено Тогда
		ШаблонСообщения = НСтр("ka='მეთოდი %1 მოსახმობად არ არის ნებადართული დავალებების რიგით.';ru='Метод %1 не разрешен к вызову через очередь заданий.'");
...
Процедура ЗапланироватьЗадание(Знач Выборка)
	...
	Если Выборка.ОбластьДанных <> -1 Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
			    НСтр("ka='სავალებების რიგი. დაგეგმვა';ru='Очередь заданий.Планирование'",
			    ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			    УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
	...
	Попытка
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные().ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
			...
			Если ЗначениеЗаполнено(Задание.Шаблон)
				...
				ШаблонСообщения = НСтр("ka='რიგის დავალების შაბლონი ვერ მოიძებნა %1';ru='На найден шаблон задания очереди  %1'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Задание.Шаблон);
				
				ЗаписьЖурналаРегистрации(НСтр("ka='სავალებების რიგი. დაგეგმვა';ru='Очередь заданий.Планирование'", 
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда
			...
			ЗаписьКонтроляВыполнения(НСтр("ka='სარეგლამენტო დავალებების რიგი. დასრულებულია შეცდომებით';ru='Очередь регламентных заданий.Завершено с ошибками'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Выборка.Идентификатор, 
				НСтр("ka='შემსრულებელი დავალება იძულებით დასრულდა';ru='Исполняющее задание было принудительно завершено'"));
			...
			Иначе // Нет расписания
				...
				Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
					...
					Если ЗначениеЗаполнено(Задание.Шаблон) Тогда // Задание по шаблону без расписания.
						...
						ШаблонСообщения = НСтр("ka='რიგის დავალებების შაბლონისთვის %1 განრიგი ვერ მოიძებნა';ru='Для шаблон заданий очереди  %1 не найдено расписание'");
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Задание.Шаблон);
						ЗаписьЖурналаРегистрации(НСтр("ka='სავალებების რიგი. დაგეგმვა';ru='Очередь заданий.Планирование'", 
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
							УровеньЖурналаРегистрации.Ошибка,
...
Процедура ВыполнитьЗаданиеОчереди(Знач Ссылка, Знач ИсполняющееФоновоеЗадание, 
	...
	Попытка
		ЗаписьКонтроляВыполнения(НСтр("ka='სარეგლამენტო დავალებების რიგი.სტარტი';ru='Очередь регламентных заданий.Старт'", 
		...
		ЗаписьКонтроляВыполнения(НСтр("ka='სარეგლამენტო დავალებების რიგი. დასრულებულია წარმატებით';ru='Очередь регламентных заданий.Завершено успешно'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка);
