		...
		Пока Выборка.Следующий() Цикл 
			Попытка
				...
				Если ЗначениеЗаполнено(Выборка.Шаблон)
					...
					ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %1-ით რიგის დავალების შაბლონი ვერ მოიძებნა';ru='На найден шаблон задания очереди с идентификатором %1';en='Found on the queue job template ID% 1'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Выборка.Шаблон);
					ЗаписьЖурналаРегистрации(НСтр("ka='დავალებების რიგი. შესრულება';ru='Очередь заданий.Выполнение';en='turn zadaniy.Vypolnenie'", 
...
"По причине:"
"%3';ka='შეცდომა შეცდომების დამამუშავებლის შესრულებისას"
"მეთოდის ფსევდონიმი: %1"
"შეცდომის დამამუშავებლის მეთოდი: %2"
"შემდეგი მიზეზით:"
"%3'");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='სარეგლამენტო დავალებების რიგი. შეცდომების დამამუშავებლის შეცდომა';ru='Очередь регламентных заданий.Ошибка обработчика ошибок';en='Queue error handler routine zadaniy.Oshibka'", 
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура СнятьЗаданияОбработкиОшибок(Знач ПараметрыВызова, Знач ИнформацияОбОшибке = Неопределено, Знач СчетчикРекурсии = 1) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ШаблонКомментария = НСтр("ka='შესრულდა დამამუშავებელი დავალებების მოხსნა."
"მეთოდის ფსევდონიმი: %1"
"რეკურსიის დონე: %2';ru='Был выполнен обработчик снятие заданий."
"Псевдоним метода: %1"
"Уровень рекурсии: %2'");
			ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария,
				ПараметрыОбработчикаОшибок.ИмяМетодаЗадания,
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='სარეგლამენტო დავალებების რიგი. შეცდომების დამუშავების დავალებების მოხსნა';ru='Очередь регламентных заданий.Снятие заданий обработки ошибок';en='Of all scheduled tasks zadaniy.Snyatie error handling'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Если ТранзакцияАктивна() Тогда
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებელ %1-ის შესრულების სტარტამდე არის აქტიური ტრანზაქციები!';ru='Перед стартом выполнения обработчика %1 есть активные транзакции!';en='Before the start of execution of the handler% 1 is an active transaction!'"),
				ИмяМетода);
			
		ЗаписьЖурналаРегистрации(НСтр("ka='სარეგლამენტო დავალებების რიგი. შესრულება';ru='Очередь регламентных заданий.Выполнение';en='Queue routine zadaniy.Vypolnenie'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
	...
	Попытка
		...
		Если ТранзакцияАктивна() Тогда
			...
			ШаблонСообщения = НСтр("ka='დამამუშავებლის შესრულების დასრულებისას %1 ტრანზაქცია არ დახურულა';ru='По завершении выполнения обработчика %1 не была закрыта транзакция';en='On completion handler% 1 was not closed transaction'");
		...
		Если Не(РазделительУстановлен) И ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებელ %1-ის შესრულების დასრულებისას სეანსის დაყოფა არ გამორთულა!';ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса!';en='On completion handler% 1 has not been turned off separation of the session!'"),
				ИмяМетода);
		...
		ИначеЕсли РазделительУстановлен И ЗначениеРазделителя <> РаботаВМоделиСервиса.ЗначениеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებელ %1-ის შესრულების დასრულებისას სეანსის გამყოფის მნიშვნელობა შეიცვალა!';ru='По завершении выполнения обработчика %1 было изменено значение разделителя сеанса!';en='On completion handler% 1 was changed to the session separator value!'"),
				ИмяМетода);
...
Функция ПолучитьПараметрыОбработчикаОшибок(Знач Задание,Знач ИнформацияОбОшибкеВыполненияЗадания = Неопределено)
	...
	Если Результат.ОбработчикСуществует Тогда
		...
		Если ИнформацияОбОшибкеВыполненияЗадания = Неопределено Тогда
			...
			ИнформацияОбОшибкеВыполненияЗадания = КонструкторИнформацииОбОшибке(НСтр("ka='დავალება დასრულდა უცნბი შეცდომით, რომელიც შეიძლება გამოწვეული იყოს სამუშაო პროცესის ვარდნით.';ru='Задание завершилось с неизвестной ошибкой, возможно вызванной падением рабочего процесса.';en='Task completed with an unknown error, possibly due to the fall of the working process.'"));
...
Процедура ЗаписьКонтроляВыполнения(Знач ИмяСобытия, Знач ЗаписьЗадания, Знач Комментарий = "")
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ОписаниеМетода.Шаблон) Тогда
			...
			ИмяМетода = НСтр("ka='შაბლონი';ru='Шаблон:';en='Template:'") + ОписаниеМетода.Шаблон;
...
Процедура ВыполнитьОбработчикЗадания(Шаблон, Псевдоним, Параметры)
	...
	Если ИмяМетода = Неопределено Тогда
		ШаблонСообщения = НСтр("ka='მეთოდი %1 მოსახმობად არ არის ნებადართული დავალებების რიგით.';ru='Метод %1 не разрешен к вызову через очередь заданий.';en='The method% 1 is not allowed to call through the job queue.'");
...
Процедура ЗапланироватьЗадание(Знач Выборка)
	...
	Если Выборка.ОбластьДанных <> -1 Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
			    НСтр("ka='სავალებების რიგი. დაგეგმვა';ru='Очередь заданий.Планирование';en='turn zadaniy.Planirovanie'",
			    ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			    УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
	...
	Попытка
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные().ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
			...
			Если ЗначениеЗаполнено(Задание.Шаблон)
				...
				ШаблонСообщения = НСтр("ka='რიგის დავალების შაბლონი ვერ მოიძებნა %1';ru='На найден шаблон задания очереди  %1';en='Found on the job template queue% 1'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Задание.Шаблон);
				
				ЗаписьЖурналаРегистрации(НСтр("ka='სავალებების რიგი. დაგეგმვა';ru='Очередь заданий.Планирование';en='turn zadaniy.Planirovanie'", 
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда
			...
			ЗаписьКонтроляВыполнения(НСтр("ka='სარეგლამენტო დავალებების რიგი. დასრულებულია შეცდომებით';ru='Очередь регламентных заданий.Завершено с ошибками';en='Queue routine zadaniy.Zaversheno erroneously'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Выборка.Идентификатор, 
				НСтр("ka='შემსრულებელი დავალება იძულებით დასრულდა';ru='Исполняющее задание было принудительно завершено';en='Executing a task force has been completed'"));
			...
			Иначе // Нет расписания
				...
				Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
					...
					Если ЗначениеЗаполнено(Задание.Шаблон) Тогда // Задание по шаблону без расписания.
						...
						ШаблонСообщения = НСтр("ka='რიგის დავალებების შაბლონისთვის %1 განრიგი ვერ მოიძებნა';ru='Для шаблон заданий очереди  %1 не найдено расписание';en='For job queues template% 1 is not found schedule'");
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Задание.Шаблон);
						ЗаписьЖурналаРегистрации(НСтр("ka='სავალებების რიგი. დაგეგმვა';ru='Очередь заданий.Планирование';en='turn zadaniy.Planirovanie'", 
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
							УровеньЖурналаРегистрации.Ошибка,
...
Процедура ВыполнитьЗаданиеОчереди(Знач Ссылка, Знач ИсполняющееФоновоеЗадание, 
	...
	Попытка
		ЗаписьКонтроляВыполнения(НСтр("ka='სარეგლამენტო დავალებების რიგი.სტარტი';ru='Очередь регламентных заданий.Старт';en='Queue routine zadaniy.Start'", 
		...
		ЗаписьКонтроляВыполнения(НСтр("ka='სარეგლამენტო დავალებების რიგი. დასრულებულია წარმატებით';ru='Очередь регламентных заданий.Завершено успешно';en='Queue routine zadaniy.Zaversheno successfully'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка);
