...
Функция ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке)
	...
	Исключение
		ТекстСообщенияОбОшибке = НСтр("ka='მაკეტის XML-აღწერილობის წაკითხვისას შეცდომა მოხდა.';ru='При чтении XML-описания макета произошла ошибка.'");
		Результат = Ложь;
	КонецПопытки;
...
Функция ПолучитьОписаниеМакета(Источник, СообщениеОбОшибке, Результат) Экспорт
	...
	Пока ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке) Цикл
		...
		Если ЧтениеXML.Имя="Formatting" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда			
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ka='ელემენტ Formatting-ის ატრიბუტ Width-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Width элемента Formatting произошла ошибка.'");
				Возврат Ложь;				
			КонецЕсли;
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ka='ელემენტ Formatting-ის ატრიბუტ Height-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Height элемента Formatting произошла ошибка.'");
				Возврат Ложь;				
			КонецЕсли;
		...
		ИначеЕсли (ЧтениеXML.Имя="Text" ИЛИ ЧтениеXML.Имя="Barcode" ИЛИ ЧтениеXML.Имя="Image" ИЛИ ЧтениеXML.Имя="UserData") И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ka='ელემენტ UserData-ის ატრიბუტ FieldName-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута FieldName элемента UserData произошла ошибка.'");
				Возврат Ложь;				
			КонецЕсли;
			...
			Если ПрочитатьАтрибут(ЧтениеXML, "TypeFill", ЗначениеАтрибута) Тогда
				...
				Если ЗначениеАтрибута=Неопределено Тогда					
					СообщениеОбОшибке = НСтр("ka='ელემენტ UserData-ის ატრიბუტ TypeFill-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута TypeFill элемента UserData произошла ошибка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Если ЗначениеАтрибута="Parameter" Тогда					
					...
					Если ПрочитатьАтрибут(ЧтениеXML, "Value", ЗначениеАтрибута) Тогда
						...
						Если ЗначениеАтрибута=Неопределено Тогда							
							СообщениеОбОшибке = НСтр("ka='ელემენტ UserData-ის ატრიბუტ Value-ის ფაილის მიღებისას შეცდომა მოხდა.';ru='При чтении файла атрибута Value элемента UserData произошла ошибка.'");
							Возврат Ложь;							
						Иначе							
			...
			Если НовоеПоле.Тип<>"UserData" Тогда				
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ Left-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Left элемента Text произошла ошибка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ Top-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Top элемента Text произошла ошибка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტის ფაილ Width-ის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Width элемента Text произошла ошибка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ Height-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Height элемента Text произошла ошибка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Если НовоеПоле.Тип="Text" Тогда
					...
					Иначе
						СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ FontSize-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута FontSize элемента Text произошла ошибка.'");
						Возврат Ложь;
					КонецЕсли;
				...
				ИначеЕсли НовоеПоле.Тип="Barcode" Тогда
					...
					Иначе
						СообщениеОбОшибке = НСтр("ka='ელემენტ Barcode-ის ატრიბუტ Type-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Type элемента Barcode произошла ошибка.'");
						Возврат Ложь;
					КонецЕсли;
...
Функция ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода) Экспорт
	...
	Если ВнешняяКомпонента = Неопределено Тогда
		ВызватьИсключение НСтр("ka='შტრიხკოდის ბეჭდვის გარე კომპონენტის შეერთების შეცდომა!';ru='Ошибка подключения внешней компоненты печати штрихкода!'");
	КонецЕсли;
