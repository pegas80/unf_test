...
Процедура ДокументыПерсУчетаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если СтруктураОбъекта.ОтчетныйПериод <> Неопределено И (Не ЗначениеЗаполнено(СтруктураОбъекта.ОтчетныйПериод)) И (Не Источник.ПометкаУдаления) Тогда
		ТекстСообщения = НСтр("ka='არ არის შევსებული ველის მნიშვნელობა საანგარიშგებო პერიოდი';ru='Не заполнено значение поля отчетный период'");
...
Процедура ЗаполнитьАдрес(НаборЗаписейАдрес, СтрокаАдрес, СписокСтран, ФорматНеструктурированныйАдрес, ФорматИностранныйАдрес, ЗаментяБуквуЕ = Ложь) Экспорт
	...
	Если ЗаПределамиРФ = Истина Тогда
		Если АдресСписком.Страна = "РОССИЯ" Тогда
			...
			ТекстПредупреждения = Нстр("ka='შენახულია როგორც არასტრუქტურირებული მისამართი.';ru='сохранен как неструктурированный адрес.'");
		Иначе
			// Иностранный адрес
...
Функция ПредставлениеОтчетногоПериода(Период, Кратко = Ложь) Экспорт
	...
	ИначеЕсли Месяц(Период) <= 6  Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წლის 1 ნახევარწელიწადი';ru='1 полугодие %1 г.'"), Формат(Период, "ДФ=ггг"));
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წლის მეორე ნახევარი';ru='2 полугодие %1 г.'"), Формат(Период, "ДФ=ггг"));
	КонецЕсли;
