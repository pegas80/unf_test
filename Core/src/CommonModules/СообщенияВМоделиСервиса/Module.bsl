...
Процедура ОшибкаНеизвестноеИмяКанала(Знач КаналСообщений) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეტყობინების არხის უცნობი სახელი %1';ru='Неизвестное имя канала сообщений %1'"), КаналСообщений);
...
Процедура ЗаписатьСобытиеНачалоОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. დამუშავების დასაწყისი';ru='Сообщения в модели сервиса.Начало обработки'",
...
Процедура ЗаписатьСобытиеОкончаниеОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. დამუშავების დასრულება';ru='Сообщения в модели сервиса.Окончание обработки'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
...
Процедура ДоставитьБыстрыеСообщения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение(НСтр("ka='სწრაფი შეტყობინებების მიტანა შეუძლებელია ტრანზაქციაში';ru='Доставка быстрых сообщений невозможна в транзакции'"));
	КонецЕсли;
	...
	Попытка
		ФоновыеЗадания.Выполнить(ИмяМетодаЗадания, , КлючЗадания, НСтр("ka='სწრაფი შეტყობინებების მიტანა';ru='Доставка быстрых сообщений'"))
	Исключение
		// Дополнительная обработка исключения не требуется
		// ожидаемое исключение - дублирование задания с таким же ключом.
		ЗаписьЖурналаРегистрации(НСтр("ka='სწრაფი შეტყობინებების მიტანა';ru='Доставка быстрых сообщений'",
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
			Если ОбщегоНазначения.ЗначениеРазделителяСеанса() <> Сообщение.Body.Zone Тогда
				ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. შეტყობინების გაგზავნა';ru='Сообщения в модели сервиса.Отправка сообщения'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				ШаблонОшибки = НСтр("ka='შეცდომა შეტყობინების გაგზავნისას. მონაცემთა არეალი %1 არ ემთხვევა მიმდინარეს (%2).';ru='Ошибка при отправке сообщения. Область данных %1 не совпадает с текущей (%2).'");
...
Процедура ПриОтправкеСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. გაგზავნა';ru='Сообщения в модели сервиса.Отправка'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Процедура ПриПолученииСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. მიღება';ru='Сообщения в модели сервиса.Получение'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Функция ПредставлениеСообщенияДляЖурнала(Знач Сообщение)
	...
	Шаблон = НСтр("ka='არხი: %1';ru='Канал: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
		Шаблон = НСтр("ka='მონაცემთა არეალი: %1';ru='Область данных: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ТранслироватьСообщениеВВерсиюКорреспондентаПриНеобходимости(Сообщение, Знач ИнформацияПодключения, Знач ПредставлениеПолучателя)
	...
	Если Не ИнформацияПодключения.Свойство("URL") 
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საინფორმაციო ბაზა %1-თან შეტყობინებების გაცვლის სერვისის URL არ არის მიცემული';ru='Не задан URL сервиса обмена сообщениями с информационной базой %1'"), ПредставлениеПолучателя);
	КонецЕсли;
	...
	Если ВерсияКорреспондента = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კორესპონდენტი %1 მხარს არ უჭერს მიმდინარე საინფორმაციო ბაზის მიერ მხარდაჭერილ ინტერფეისის შეტყობინებების ვერსიების %2 მიღებას!';ru='Корреспондент %1 не поддерживает получение версий сообщений интерфейса %2, поддерживаемых текущей информационной базой!'"),
