...
Процедура ОшибкаНеизвестноеИмяКанала(Знач КаналСообщений) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეტყობინების არხის უცნობი სახელი %1';ru='Неизвестное имя канала сообщений %1';en='Unknown name of the channel message% 1'"), КаналСообщений);
...
Процедура ЗаписатьСобытиеНачалоОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. დამუშავების დასაწყისი';ru='Сообщения в модели сервиса.Начало обработки';en='Posts in the model servisa.Nachalo processing'",
...
Процедура ЗаписатьСобытиеОкончаниеОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. დამუშავების დასრულება';ru='Сообщения в модели сервиса.Окончание обработки';en='Posts in servisa.Okonchanie processing model'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
...
Процедура ДоставитьБыстрыеСообщения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение(НСтр("ka='სწრაფი შეტყობინებების მიტანა შეუძლებელია ტრანზაქციაში';ru='Доставка быстрых сообщений невозможна в транзакции';en='instant message delivery is not possible in the transaction'"));
	КонецЕсли;
	...
	Попытка
		ФоновыеЗадания.Выполнить(ИмяМетодаЗадания, , КлючЗадания, НСтр("ka='სწრაფი შეტყობინებების მიტანა';ru='Доставка быстрых сообщений';en='Delivery fast communications'"))
	Исключение
		// Дополнительная обработка исключения не требуется
		// ожидаемое исключение - дублирование задания с таким же ключом.
		ЗаписьЖурналаРегистрации(НСтр("ka='სწრაფი შეტყობინებების მიტანა';ru='Доставка быстрых сообщений';en='Delivery fast communications'",
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
			Если ОбщегоНазначения.ЗначениеРазделителяСеанса() <> Сообщение.Body.Zone Тогда
				ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. შეტყობინების გაგზავნა';ru='Сообщения в модели сервиса.Отправка сообщения';en='Posts in servisa.Otpravka model posts'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				ШаблонОшибки = НСтр("ka='შეცდომა შეტყობინების გაგზავნისას. მონაცემთა არეალი %1 არ ემთხვევა მიმდინარეს (%2).';ru='Ошибка при отправке сообщения. Область данных %1 не совпадает с текущей (%2).';en='Error sending message. % 1 data region does not coincide with the current (2%).'");
...
Процедура ПриОтправкеСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. გაგზავნა';ru='Сообщения в модели сервиса.Отправка';en='Posts in servisa.Otpravka model'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Процедура ПриПолученииСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='შეტყობინებები სერვისის მოდელში. მიღება';ru='Сообщения в модели сервиса.Получение';en='Posts in servisa.Poluchenie model'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Функция ПредставлениеСообщенияДляЖурнала(Знач Сообщение)
	...
	Шаблон = НСтр("ka='არხი: %1';ru='Канал: %1';en='Channel: 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Шаблон = НСтр("ka='სათაური: %1';ru='Заголовок:"
"%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
		Шаблон = НСтр("ka='მონაცემთა არეალი: %1';ru='Область данных: %1';en='Data Area% 1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ТранслироватьСообщениеВВерсиюКорреспондентаПриНеобходимости(Сообщение, Знач ИнформацияПодключения, Знач ПредставлениеПолучателя)
	...
	Если ИнтерфейсСообщения = Неопределено Тогда
		ВызватьИсключение НСтр("ka='გასაგზავნი შეტყობინების ინტერფეისის დადგენა ვერ მოხერხდა: შეტყობინებაში გამოყენებული არცერთი ტიპისთვის არ არის რეგისტრირებული ინტერფეისის დამამუშავებელი!';ru='Не удалось определить интерфейс отправляемого сообщения: ни для одного из типов, используемых в сообщении, не зарегистрирован обработчик интерфейса!';en='Unable to determine the interface message to be sent: for any of the types used in the message is not registered handler interface!'");
	КонецЕсли;
	
	Если Не ИнформацияПодключения.Свойство("URL") 
			Или Не ЗначениеЗаполнено(ИнформацияПодключения.URL) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საინფორმაციო ბაზა %1-თან შეტყობინებების გაცვლის სერვისის URL არ არის მიცემული';ru='Не задан URL сервиса обмена сообщениями с информационной базой %1';en='Not specified URL messaging service with the information base 1%'"), ПредставлениеПолучателя);
	КонецЕсли;
	...
	Если ВерсияКорреспондента = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კორესპონდენტი %1 მხარს არ უჭერს მიმდინარე საინფორმაციო ბაზის მიერ მხარდაჭერილ ინტერფეისის შეტყობინებების ვერსიების %2 მიღებას!';ru='Корреспондент %1 не поддерживает получение версий сообщений интерфейса %2, поддерживаемых текущей информационной базой!';en='Correspondent% 1 does not support the version of the interface receive posts 2%, supported by the current information base!'"),
