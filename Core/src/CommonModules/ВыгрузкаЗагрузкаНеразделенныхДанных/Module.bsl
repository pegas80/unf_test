...
Процедура ПередВыгрузкойТипа(Контейнер, Сериализатор, ОбъектМетаданных, Отказ) Экспорт
	...
	Если Не ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
		ВызватьИсключение НСтр("ka='ბმულების გამოცვლა ხელმისაწვდომია მხოლოდ მიმთითებელ მონაცემებში';ru='Замена ссылок доступна только в ссылочных данных';en='Replacement of links available only in the reference data'");
	КонецЕсли;
...
Процедура ПослеВыгрузкиОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты) Экспорт
	...
	Если Контейнер.ДополнительныеСвойства.ОбщиеДанныеТребующиеСопоставленияСсылок.Найти(ОбъектМетаданных) <> Неопределено Тогда
		...
		Если Не ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
			ВызватьИсключение НСтр("ka='ბმულების გამოცვლა ხელმისაწვდომია მხოლოდ მიმთითებელ მონაცემებში';ru='Подмена ссылок доступна только в ссылочных данных';en='The substitution of references is available in the reference data'");
		КонецЕсли;
...
Процедура КонтрольИспользованияСсылокНаНеразделенныеДанныеВРазделенных() Экспорт
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='კონფიგურაციის მეტამონაცემთა სტრუქტურაში აღმოჩენილია შეცდომები: %1';ru='Обнаружены ошибки в структуре метаданных конфигурации: %1';en='Errors found in the configuration metadata structure: 1%'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ТекстОшибки);
...
Процедура ПроверитьПоляЕстественногоКлюча(Знач ОбъектМетаданных, Знач ПоляЕстественногоКлюча)
	...
	Если ПоляЕстественногоКлюча = Неопределено Или ПоляЕстественногоКлюча.Количество() = 0 Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='მონაცემთა ტიპ %1-ისთვის ბმულების გამოსაცვლელად ბუნებრივი გასაღებები არ არის მითითებული. შეამოწმეთ დამამუშავებელი ლოკალურვერსიაშიჩასატვირთავიტიპებისდადგენისას.';ru='Для типа данных %1 не указаны естественные ключи для замены ссылок."
...
Процедура КонтрольСсылокНаНеразделенныеДанныеПриВыгрузке(Контейнер, Объект, ПоляДляКонтроляСсылокНаНеразделенныеДанные)
	...
	Для Каждого ПолеДляКонтроляСсылокНаНеразделенныеДанные Из ПоляДляКонтроляСсылокНаНеразделенныеДанные Цикл
		...
		Если СтруктураИмениОбъекта[0] <> СтруктураИмениПоля[0] Или СтруктураИмениОбъекта[1] <> СтруктураИмениПоля[1] Тогда
			...
			ВызватьИсключение НСтр("ka='Некорректный кэш контроля неразделенных данных при выгрузке!';ru='Некорректный кэш контроля неразделенных данных при выгрузке!';en='Invalid cache control unshared data when unloading!'");
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ka='Объект метаданных %1 не поддерживается!';ru='Объект метаданных %1 не поддерживается!';en='Metadata object% 1 is not supported!'", Метаданные.ОсновнойЯзык.КодЯзыка),
				ПолноеИмяОбъектаМетаданных);
		КонецЕсли;
...
"---------------------------------------------------------------------------------------------------------------------------"
"%5"
"---------------------------------------------------------------------------------------------------------------------------';ka='Объект метаданных %1 включен в перечень объектов, для которых не требуется сопоставление ссылок при выгрузке / загрузке"
"данных (в переопределяемой процедуре "
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхНеТребующихСопоставлениеСсылокПриЗагрузке(),"
"но при этом для него не обеспечивается требования отсутствия несопоставляемых ссылок при выгрузке."
""
