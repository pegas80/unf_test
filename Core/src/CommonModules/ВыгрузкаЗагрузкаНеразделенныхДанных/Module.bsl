...
Процедура ПередВыгрузкойТипа(Контейнер, Сериализатор, ОбъектМетаданных, Отказ) Экспорт
	...
	Если Не ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
		ВызватьИсключение НСтр("ka='ბმულების გამოცვლა ხელმისაწვდომია მხოლოდ მიმთითებელ მონაცემებში';ru='Замена ссылок доступна только в ссылочных данных'");
	КонецЕсли;
...
Процедура ПослеВыгрузкиОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты) Экспорт
	...
	Если Контейнер.ДополнительныеСвойства.ОбщиеДанныеТребующиеСопоставленияСсылок.Найти(ОбъектМетаданных) <> Неопределено Тогда
		...
		Если Не ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
			ВызватьИсключение НСтр("ka='ბმულების გამოცვლა ხელმისაწვდომია მხოლოდ მიმთითებელ მონაცემებში';ru='Подмена ссылок доступна только в ссылочных данных'");
		КонецЕсли;
...
Процедура КонтрольИспользованияСсылокНаНеразделенныеДанныеВРазделенных() Экспорт
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='კონფიგურაციის მეტამონაცემთა სტრუქტურაში აღმოჩენილია შეცდომები: %1';ru='Обнаружены ошибки в структуре метаданных конфигурации: %1'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ТекстОшибки);
...
Процедура КонтрольСсылокНаНеразделенныеДанныеПриВыгрузке(Контейнер, Объект, ПоляДляКонтроляСсылокНаНеразделенныеДанные)
	...
	Для Каждого ПолеДляКонтроляСсылокНаНеразделенныеДанные Из ПоляДляКонтроляСсылокНаНеразделенныеДанные Цикл
		...
		Если СтруктураИмениОбъекта[0] <> СтруктураИмениПоля[0] Или СтруктураИмениОбъекта[1] <> СтруктураИмениПоля[1] Тогда
			...
			ВызватьИсключение НСтр("ka='Некорректный кэш контроля неразделенных данных при выгрузке!';ru='Некорректный кэш контроля неразделенных данных при выгрузке!'");
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ka='Объект метаданных %1 не поддерживается!';ru='Объект метаданных %1 не поддерживается!'", Метаданные.ОсновнойЯзык.КодЯзыка),
				ПолноеИмяОбъектаМетаданных);
		КонецЕсли;
