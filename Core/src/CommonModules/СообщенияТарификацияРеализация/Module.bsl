	...
	Попытка
		...
		Для Каждого УслугаXDTO Из ОбщиеДанныеПоТарификации.Services Цикл 
			...
			Если ПоставщикУслуги.Пустая() Тогда 
				ВызватьИсключение НСтр("ru='Не найде поставщик услуги с кодом: ';ka='Не найде поставщик услуги с кодом: '") + УслугаXDTO.ServiceProviderID;
			КонецЕсли;
...
Процедура ОбновитьДанныеАбонентаПоТарификацииВПриложении(Знач ДанныеАбонентаПоТарификации) Экспорт
	...
	ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ka='Обновление данных по тарификации в области данных';ru='Обновление данных по тарификации в области данных';en='Update billing data in the data area'");
	...
	Попытка
		...
		Для Каждого ПодпискаНаУслугу Из ДанныеАбонентаПоТарификации.ServiceSubscriptions Цикл
			...
			Иначе
				ШаблонТекстаОшибки = НСтр("ka='Не найдена услуга с идентификатором ""%1"" и идентификатором поставщика ""%2""';ru='Не найдена услуга с идентификатором ""%1"" и идентификатором поставщика ""%2""';en='Found service with ID ""%1"" and the ID provider ""%2""'");
...
Процедура УдалитьДанныеПоТарификацииВПриложениеАбонента(Знач УдаляемыеПодписки) Экспорт
	...
	Для Каждого Подписка Из УдаляемыеПодписки.SubscriptionsID Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ИнформацияПоОшибке = НСтр("ru='Проблема с удалением данных по подписке: ';ka='Проблема с удалением данных по подписке: '") + 
				Строка(УдаляемыеПодписки.SubscriptionsID) +
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ka='Удаление данных по подписке';ru='Удаление данных по подписке';en='Removing subscription data'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура ОбновитьДанныеПоЛицензиям(ОбновляемыеЛицензии) Экспорт
	...
	Исключение
		...
		ИнформацияПоОшибке = ИнформацияОбОшибке();
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ka='Обновление данных по лицензиям';ru='Обновление данных по лицензиям';en='Update license data'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура УдалитьДанныеПоЛицензиям(УдаляемыеЛицензии) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ИнформацияПоОшибке = ИнформацияОбОшибке();
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ka='Удаление данных по лицензиям';ru='Удаление данных по лицензиям';en='Removal of license data'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
