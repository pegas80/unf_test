...
//		НСтр("ka='შეტყობინება შეცდომის შესახებ.';ru='Сообщение об ошибке.';en='an error message.'"), ,
...
//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
//		НСтр("ka='შეტყობინება შეცდომის შესახებ.';ru='Сообщение об ошибке.';en='an error message.'"), ОбъектИнформационнойБазы, "Ответственный",,Отказ);
//
// 4. Для вывода сообщения по ссылке на объект информационной базы.
//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
//		НСтр("ka='შეტყობინება შეცდომის შესახებ.';ru='Сообщение об ошибке.';en='an error message.'"), Ссылка, , , Отказ);
//
...
//                    например, НСтр("ka='მომხმარებელი არ არის არჩეული.';ru='Пользователь не выбран.';en='This user has not selected.'").
//
...
//                    свойством ГруппаОшибок, например, НСтр("ka='სტრიქონ %1-ში მომხმარებელი არ არის არჩეული.';ru='Пользователь в строке %1 не выбран.';en='User% 1 line is not selected.'").
//
...
Функция ТекстОшибкиЗаполнения(ВидПоля = "Поле", ВидСообщения = "Заполнение",
	...
	Если ВРег(ВидПоля) = "ПОЛЕ" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Поле ""%1"" не заполнено';en='The ""%1"" filled'");
...
"%4'");
	...
	ИначеЕсли ВРег(ВидПоля) = "СПИСОК" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("ka='სია ""%3""-ში არცერთი სტრიქონი არ არის შეტანილი';ru='Не введено ни одной строки в список ""%3""';en='Do not put a single line in the list of ""%3""'");
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
	...
	Для Каждого Элемент Из Источник Цикл
		Если Заменять <> Истина И Приемник.Свойство(Элемент.Ключ) Тогда
			Если Заменять = Ложь Тогда
				Продолжить;
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Пересечение ключей источника и приемника: ""%1"".';ru='Пересечение ключей источника и приемника: ""%1"".';en='The intersection of source and receiver key ""%1"".'"), Элемент.Ключ);
			КонецЕсли
		КонецЕсли;
...
Процедура УстановитьСостояниеПоляТабличногоДокумента(ПолеТабличногоДокумента, Состояние = "НеИспользовать") Экспорт
	...
	Если ТипЗнч(ПолеТабличногоДокумента) = Тип("ПолеФормы") 
		...
		ИначеЕсли ВРег(Состояние) = "НЕАКТУАЛЬНОСТЬ" Тогда
			...
			ОтображениеСостояния.Картинка                       = Новый Картинка;
			ОтображениеСостояния.Текст                          = НСтр("ka='ანგარიშგება არ არის ფორმირებული. ანგარიშგების მისაღებად დააჭირეთ ""ფორმირება""-ს.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';en='The report has not been formed. Click ""Create"" to obtain a report.'");;
		...
		Иначе
			ВызватьИсключение(НСтр("ka=""პარამეტრის დაუშვებელი მნიშვნელობა (პარამეტრი ნომერი '2')"";ru=""Недопустимое значение параметра (параметр номер '2')"";en=""Invalid parameter value ( '2' option number)"""));
		КонецЕсли;
	Иначе
		ВызватьИсключение(НСтр("ka=""პარამეტრის დაუშვებელი მნიშვნელობა (პარამეტრი ნომერი '1')"";ru=""Недопустимое значение параметра (параметр номер '1')"";en=""Invalid parameter value ( '1' option number)"""));
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='პარამეტრ ვერსიისსტრიქონი1-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1';en='Wrong format StrokaVersii1 parameter:% 1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	    	НСтр("ka='პარამეტრ ვერსიისსტრიქონი2-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1';en='Wrong format StrokaVersii2 parameter:% 1'"), СтрокаВерсии2);
	КонецЕсли;
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		КраткоеОписаниеОшибки = НСтр("ka='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';ru='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';en='Direct connection to the database is not available on the server running Linux.'");
	#Иначе
		ПодключениеНедоступно = ЭтоLinuxКлиент() Или ЭтоOSXКлиент();
		КраткоеОписаниеОшибки = НСтр("ka='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';ru='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';en='Direct connection to the database is only available on the client running on Windows.'");
	#КонецЕсли
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='სხვა პროგრამასთან დაკავშირება ვერ მოხერხდა: %1';ru='Не удалось подключится к другой программе: %1';en='Failed to connect to another program: 1%'");
	...
	Если ФайловыйВариантРаботы Тогда
		...
		Если ПустаяСтрока(Параметры.КаталогИнформационнойБазы) Тогда
			СтрокаСообщенияОбОшибке = НСтр("ka='საინფორმაციო ბაზის კატალოგის ადგილმდებარეობა მიცემული არ არის.';ru='Не задано месторасположение каталога информационной базы.';en='Do not set the location information database directory.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Параметры.ИмяСервера1СПредприятия) Или ПустаяСтрока(Параметры.ИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			СтрокаСообщенияОбОшибке = НСтр("ka='არ არის მიცემული შეერთების აუცილებელი პარამეტრები: ""სერვერის სახელი"", ""საინფორმაციო ბაზის სახელი სერვერზე"".';ru='Не заданы обязательные параметры подключения: ""Имя сервера""; ""Имя информационной базы на сервере"".';en='Do not set the required connection settings, ""Server Name""; ""Information database name on the server.""'");
			ОшибкаПроверкиЗаполнения = Истина;
		КонецЕсли;
	...
	Исключение
		Информация = ИнформацияОбОшибке();
		СтрокаСообщенияОбОшибке = НСтр("ka='სხვა პროგრამასთან შეერთება ვერ მოხერხდა: %1';ru='Не удалось подключиться к другой программе: %1';en='Unable to connect to another program: 1%'");
	...
	Для Каждого СтрокаАдреса Из МассивАдресов Цикл
		...
		Если АдресОпределен Тогда 
			Если ЕстьНедопустимыеСимволыВПсевдониме Тогда
				ОписаниеОшибки = НСтр("ka='წარდგენა შეიცავს დაუშვებელ სიმბოლოებს';ru='Представление содержит недопустимые символы';en='Presentation contains invalid characters'");
			КонецЕсли;
		Иначе
			Если ЕстьПризнакНаличияАдресаВСтроке Тогда 
				ОписаниеОшибки = НСтр("ka='ელექტრონული ფოსტის მისამართი შეცდომებს შეიცავს';ru='Адрес электронной почты содержит ошибки';en='E-mail address contains an error'");
			Иначе
				ОписаниеОшибки = НСтр("ka='სტრიქონი არ შეიცავს ელექტრონული ფოსტის მისამართს';ru='Строка не содержит адреса электронной почты';en='The string does not contain e-mail addresses'");
			КонецЕсли;
		КонецЕсли;	
...
Функция РазобратьСтрокуСПочтовымиАдресами(Знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	СообщениеНедопустимыеСимволы = НСтр("ka='ელექტრონული ფოსტა ""%2""-ის მისამართში არსებული დაუშვებელი სიმბოლო ""%1""';ru='Недопустимый символ ""%1"" в адресе электронной почты ""%2""';en='Invalid character ""%1"" in the email address ""%2""'");
	СообщениеНеверныйФорматПочтовогоАдреса = НСтр("ka='ელექტრონული ფოსტა ""%1""-ის არაკორექტული მისამართი';ru='Некорректный адрес электронной почты ""%1""';en='Invalid e-mail address ""%1""'");
...
Процедура Проверить(Знач Условие, Знач Сообщение = "", Знач КонтекстПроверки = "") Экспорт
	...
	Если Условие <> Истина Тогда
		Если ПустаяСтрока(Сообщение) Тогда
			ТекстИсключения = НСтр("ka='დაუშვებელი ოპერაცია';ru='Недопустимая операция';en='Illegal operation'"); // Assertion failed
		Иначе
			ТекстИсключения = Сообщение;
		КонецЕсли;
		Если Не ПустаяСтрока(КонтекстПроверки) Тогда
			ТекстИсключения = ТекстИсключения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ში';ru='в %1';en='in 1'"), КонтекстПроверки);
		КонецЕсли;
...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(ТипЗнч(ИмяПроцедурыИлиФункции) = Тип("Строка"), 
		НСтр("ka='პარამეტრ პროცედურისანფუნქციისსახელი-ს მნიშვნელობა დაუშვებელია';ru='Недопустимо значение параметра ИмяПроцедурыИлиФункции';en='Unacceptable value parameter ImyaProtseduryIliFunktsii'"), Контекст);
	Проверить(ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ka='დაუშვებელია პარამეტრ პარამეტრისსახელი-ს მნიშვნელობა';ru='Недопустимо значение параметра ИмяПараметра';en='Unacceptable value parameter ParameterName'"), Контекст);
		
	ЭтоОписаниеТипов = ТипЗнч(ОжидаемыеТипы) = Тип("ОписаниеТипов");
	Проверить(ЭтоОписаниеТипов Или ТипЗнч(ОжидаемыеТипы) = Тип("Тип"), 
		НСтр("ka='დაუშვებელია პარამეტრის მნიშვნელობა მოსალოდნელიტიპები';ru='Недопустимо значение параметра ОжидаемыеТипы';en='Unacceptable value parameter OzhidaemyeTipy'"), Контекст);
...
"В структуре ожидалось свойство %3 (тип %4).';ka='პარამეტრ %1-ის (სტრუქტურა) დაუშვებელი მნიშვნელობა %2-ში. "
"სტრუქტურაში მოსალოდნელი იყო თვისება %3 (ტიპი %4).'");
		НедопустимоеСвойство = НСтр("ru='Недопустимое значение свойства %1 в параметре %2 (Структура) в %3. "
"Ожидалось: %4; передано значение: %5 (тип %6).';ka='თვისების დაუშვებელი მნიშვნელობა %1 პარამეტრ %2-ში (სტრუქტურა) %3-ში. "
"მოსალოდნელი იყო: %4; გადაცემულია მნიშვნელობა: %5 (ტიპი %6).'");
		Для каждого Свойство Из ОжидаемыеТипыСвойств Цикл
			...
			Проверить(ЭтоКорректныйТип, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НедопустимоеСвойство, 
				...
				?(ЗначениеСвойства <> Неопределено, ЗначениеСвойства, НСтр("ka='დაუდგენელია';ru='Неопределено';en='Undefined'")),
...
Функция ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	...
	НедопустимыйСимволВИмениАдресата = НСтр("ka='დაუშვებელი სომბოლო ადრესატის მისამართში.';ru='Недопустимый символ в имени адресата.';en=""Invalid character in the recipient's name.""");
	НедопустимыйСимволВПочтовомАдресе = НСтр("ka='დაუშვებელი სომბოლო საფოსტო მოსამართში.';ru='Недопустимый символ в почтовом адресе.';en='Invalid character in the email address.'");
	СимволыОбрамления = "<>[]";
