...
//		НСтр("ka='შეტყობინება შეცდომის შესახებ.';ru='Сообщение об ошибке.'"), ,
...
//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
//		НСтр("ka='შეტყობინება შეცდომის შესახებ.';ru='Сообщение об ошибке.'"), ОбъектИнформационнойБазы, "Ответственный",,Отказ);
//
// 4. Для вывода сообщения по ссылке на объект информационной базы.
//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
//		НСтр("ka='შეტყობინება შეცდომის შესახებ.';ru='Сообщение об ошибке.'"), Ссылка, , , Отказ);
//
...
//                    например, НСтр("ka='მომხმარებელი არ არის არჩეული.';ru='Пользователь не выбран.'").
//
...
//                    свойством ГруппаОшибок, например, НСтр("ka='სტრიქონ %1-ში მომხმარებელი არ არის არჩეული.';ru='Пользователь в строке %1 не выбран.'").
//
...
Функция ТекстОшибкиЗаполнения(ВидПоля = "Поле", ВидСообщения = "Заполнение",
	...
	Если ВРег(ВидПоля) = "ПОЛЕ" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Поле ""%1"" не заполнено'");
...
"%4'");
	...
	ИначеЕсли ВРег(ВидПоля) = "КОЛОНКА" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("ka='სია ""%3""-ის სტრიქონ %2-ში სვეტი ""%1"" არ არის შევსებული';ru='Не заполнена колонка ""%1"" в строке %2 списка ""%3""'");
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
...
"%4'");
	...
	ИначеЕсли ВРег(ВидПоля) = "СПИСОК" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("ka='სია ""%3""-ში არცერთი სტრიქონი არ არის შეტანილი';ru='Не введено ни одной строки в список ""%3""'");
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
	...
	Для Каждого Элемент Из Источник Цикл
		Если Заменять <> Истина И Приемник.Свойство(Элемент.Ключ) Тогда
			Если Заменять = Ложь Тогда
				Продолжить;
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Пересечение ключей источника и приемника: ""%1"".';ru='Пересечение ключей источника и приемника: ""%1"".'"), Элемент.Ключ);
			КонецЕсли
		КонецЕсли;
...
Процедура УстановитьСостояниеПоляТабличногоДокумента(ПолеТабличногоДокумента, Состояние = "НеИспользовать") Экспорт
	...
	Если ТипЗнч(ПолеТабличногоДокумента) = Тип("ПолеФормы") 
		...
		ИначеЕсли ВРег(Состояние) = "НЕАКТУАЛЬНОСТЬ" Тогда
			...
			ОтображениеСостояния.Картинка                       = Новый Картинка;
			ОтображениеСостояния.Текст                          = НСтр("ka='ანგარიშგება არ არის ფორმირებული. ანგარიშგების მისაღებად დააჭირეთ ""ფორმირება""-ს.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.'");;
		...
		Иначе
			ВызватьИсключение(НСтр("ka=""პარამეტრის დაუშვებელი მნიშვნელობა (პარამეტრი ნომერი '2')"";ru=""Недопустимое значение параметра (параметр номер '2')"""));
		КонецЕсли;
	Иначе
		ВызватьИсключение(НСтр("ka=""პარამეტრის დაუშვებელი მნიშვნელობა (პარამეტრი ნომერი '1')"";ru=""Недопустимое значение параметра (параметр номер '1')"""));
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='პარამეტრ ვერსიისსტრიქონი1-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	    	НСтр("ka='პარამეტრ ვერსიისსტრიქონი2-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		КраткоеОписаниеОшибки = НСтр("ka='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';ru='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.'");
	#Иначе
		ПодключениеНедоступно = ЭтоLinuxКлиент() Или ЭтоOSXКлиент();
		КраткоеОписаниеОшибки = НСтр("ka='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';ru='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.'");
	#КонецЕсли
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='სხვა პროგრამასთან დაკავშირება ვერ მოხერხდა: %1';ru='Не удалось подключится к другой программе: %1'");
	...
	Если ФайловыйВариантРаботы Тогда
		...
		Если ПустаяСтрока(Параметры.КаталогИнформационнойБазы) Тогда
			СтрокаСообщенияОбОшибке = НСтр("ka='საინფორმაციო ბაზის კატალოგის ადგილმდებარეობა მიცემული არ არის.';ru='Не задано месторасположение каталога информационной базы.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Параметры.ИмяСервера1СПредприятия) Или ПустаяСтрока(Параметры.ИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			СтрокаСообщенияОбОшибке = НСтр("ka='არ არის მიცემული შეერთების აუცილებელი პარამეტრები: ""სერვერის სახელი"", ""საინფორმაციო ბაზის სახელი სერვერზე"".';ru='Не заданы обязательные параметры подключения: ""Имя сервера""; ""Имя информационной базы на сервере"".'");
			ОшибкаПроверкиЗаполнения = Истина;
		КонецЕсли;
	...
	Исключение
		Информация = ИнформацияОбОшибке();
		СтрокаСообщенияОбОшибке = НСтр("ka='სხვა პროგრამასთან შეერთება ვერ მოხერხდა: %1';ru='Не удалось подключиться к другой программе: %1'");
	...
	Для Каждого СтрокаАдреса Из МассивАдресов Цикл
		...
		Если АдресОпределен Тогда 
			Если ЕстьНедопустимыеСимволыВПсевдониме Тогда
				ОписаниеОшибки = НСтр("ka='წარდგენა შეიცავს დაუშვებელ სიმბოლოებს';ru='Представление содержит недопустимые символы'");
			КонецЕсли;
		Иначе
			Если ЕстьПризнакНаличияАдресаВСтроке Тогда 
				ОписаниеОшибки = НСтр("ka='ელექტრონული ფოსტის მისამართი შეცდომებს შეიცავს';ru='Адрес электронной почты содержит ошибки'");
			Иначе
				ОписаниеОшибки = НСтр("ka='სტრიქონი არ შეიცავს ელექტრონული ფოსტის მისამართს';ru='Строка не содержит адреса электронной почты'");
			КонецЕсли;
		КонецЕсли;	
...
Функция РазобратьСтрокуСПочтовымиАдресами(Знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	СообщениеНедопустимыеСимволы = НСтр("ka='ელექტრონული ფოსტა ""%2""-ის მისამართში არსებული დაუშვებელი სიმბოლო ""%1""';ru='Недопустимый символ ""%1"" в адресе электронной почты ""%2""'");
	СообщениеНеверныйФорматПочтовогоАдреса = НСтр("ka='ელექტრონული ფოსტა ""%1""-ის არაკორექტული მისამართი';ru='Некорректный адрес электронной почты ""%1""'");
...
Процедура Проверить(Знач Условие, Знач Сообщение = "", Знач КонтекстПроверки = "") Экспорт
	...
	Если Условие <> Истина Тогда
		Если ПустаяСтрока(Сообщение) Тогда
			ТекстИсключения = НСтр("ka='დაუშვებელი ოპერაცია';ru='Недопустимая операция'"); // Assertion failed
		Иначе
			ТекстИсключения = Сообщение;
		КонецЕсли;
		Если Не ПустаяСтрока(КонтекстПроверки) Тогда
			ТекстИсключения = ТекстИсключения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ში';ru='в %1'"), КонтекстПроверки);
		КонецЕсли;
...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(ТипЗнч(ИмяПроцедурыИлиФункции) = Тип("Строка"), 
		НСтр("ka='პარამეტრ პროცედურისანფუნქციისსახელი-ს მნიშვნელობა დაუშვებელია';ru='Недопустимо значение параметра ИмяПроцедурыИлиФункции'"), Контекст);
	Проверить(ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ka='დაუშვებელია პარამეტრ პარამეტრისსახელი-ს მნიშვნელობა';ru='Недопустимо значение параметра ИмяПараметра'"), Контекст);
		
	ЭтоОписаниеТипов = ТипЗнч(ОжидаемыеТипы) = Тип("ОписаниеТипов");
	Проверить(ЭтоОписаниеТипов Или ТипЗнч(ОжидаемыеТипы) = Тип("Тип"), 
		НСтр("ka='დაუშვებელია პარამეტრის მნიშვნელობა მოსალოდნელიტიპები';ru='Недопустимо значение параметра ОжидаемыеТипы'"), Контекст);
...
"მოსალოდნელი იყო: %4; გადაცემულია მნიშვნელობა: %5 (ტიპი %6).'");
		Для каждого Свойство Из ОжидаемыеТипыСвойств Цикл
			...
			Проверить(ЭтоКорректныйТип, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НедопустимоеСвойство, 
				...
				?(ЗначениеСвойства <> Неопределено, ЗначениеСвойства, НСтр("ka='დაუდგენელია';ru='Неопределено'")),
...
Функция ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	...
	НедопустимыйСимволВИмениАдресата = НСтр("ka='დაუშვებელი სომბოლო ადრესატის მისამართში.';ru='Недопустимый символ в имени адресата.'");
	НедопустимыйСимволВПочтовомАдресе = НСтр("ka='დაუშვებელი სომბოლო საფოსტო მოსამართში.';ru='Недопустимый символ в почтовом адресе.'");
	СимволыОбрамления = "<>[]";
