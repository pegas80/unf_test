...
"Необходимо для загружаемого файла назначить уникальный идентификатор.';ka='ჩასატვირთი ფაილის იდენტიფიკატორი არ არის მითითებული. მეთოდის შემდგომი შესრულება შეუძლებელია. ჩასატვირთი ფაილისთვის აუცილებელია უნიკალური იდენტიფიკატორის დანიშვნა.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЗагружаемаяЧастьФайла)
		И ТипЗнч(ЗагружаемаяЧастьФайла) <> Тип("ДвоичныеДанные") Тогда
		СообщениеОбОшибке = НСтр("ka='მეთოდი ვერ შესრულდება, ვინაიდან გადაცემული მონაცემები მონაცემთა მისაღებად ტიპს არ შეესაბამება.';ru='Метод не может быть выполнен, т.к. переданные данные не соответствуют типу для получения данных.';en='The method can not be executed because transmitted data do not match the type for the data.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
...
Функция ВыгрузитьЧастьФайла(ИдентификаторФайла, НомерВыгружаемойЧастиФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Часть файла с указанным номером не найдена.';ru='Часть файла с указанным номером не найдена.';en='part of the file with the specified number is not found.'");
	КонецЕсли;
...
Функция ПодготовитьФайлДляЗагрузки(ИдентификаторФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %1-ით გადაცემის სესიის არცერთი ფრაგმენტი არ მოიძებნა."
"უნდა დავრწმუნდეთ იმაში, რომ პროგრამის გამართვებში მიცემულია პარამეტრები ""დროებითი ფაილების კატალოგი Linux-ისთვის"" და ""დროებითი ფაილების კატალოგი Windows-ისთვის"".';ru='Не найден ни один фрагмент сессии передачи с идентификатором %1."
...
"""Каталог временных файлов для Linux"" и ""Каталог временных файлов для Windows"".'");
	...
	Если Разархиватор.Элементы.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ka='არქივის ფაილი მონაცემებს არ შეიცავს.';ru='Файл архива не содержит данных.';en='The archive file contains no data.'");
		ВызватьИсключение(СообщениеОбОшибке);
...
Процедура ЗагрузитьДанныеXDTOВИнформационнуюБазу(ПараметрыWEBСервиса, СообщениеОбОшибке) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ka='შეცდომა მონაცემების ჩატვირთვისას: %1';ru='Ошибка при загрузке данных: %1';en='Error loading% 1'");
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления() Экспорт
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ka='Синхронизация данных временно недоступна в связи с обновлением приложения.';ru='Синхронизация данных временно недоступна в связи с обновлением приложения.';en='Data synchronization is temporarily unavailable due to application updates.'");
...
Функция СформироватьКлючСообщенияЖР(НаправлениеОбмена, ПараметрыWEBСервиса)
	...
	Если НаправлениеОбмена = "Получение" Тогда
		ШаблонКлючаСообщения = НСтр("ka='მონაცემთა ჩატვირთვა Web-სერვის %1-ით';ru='Загрузка данных через Web-сервис %1';en='Loading data via Web-service 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		ШаблонКлючаСообщения = НСтр("ka='მონაცემთა გადმოტვირთვა Web-სერვის %1-ით';ru='Выгрузка данных через Web-сервис %1';en='Upload data via Web-service 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
