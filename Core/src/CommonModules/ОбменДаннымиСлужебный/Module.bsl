...
"Необходимо для загружаемого файла назначить уникальный идентификатор.';ka='ჩასატვირთი ფაილის იდენტიფიკატორი არ არის მითითებული. მეთოდის შემდგომი შესრულება შეუძლებელია. ჩასატვირთი ფაილისთვის აუცილებელია უნიკალური იდენტიფიკატორის დანიშვნა.'");
	...
	Если Не ЗначениеЗаполнено(ЗагружаемаяЧастьФайла)
		...
		СообщениеОбОшибке = НСтр("ka='მეთოდი ვერ შესრულდება, ვინაიდან გადაცემული მონაცემები მონაცემთა მისაღებად ტიპს არ შეესაბამება.';ru='Метод не может быть выполнен, т.к. переданные данные не соответствуют типу для получения данных.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
...
Функция ВыгрузитьЧастьФайла(ИдентификаторФайла, НомерВыгружаемойЧастиФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		СообщениеОбОшибке = НСтр("ka='Часть файла с указанным номером не найдена.';ru='Часть файла с указанным номером не найдена.'");
	КонецЕсли;
...
"""Каталог временных файлов для Linux"" и ""Каталог временных файлов для Windows"".'");
	...
	Если Разархиватор.Элементы.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ka='არქივის ფაილი მონაცემებს არ შეიცავს.';ru='Файл архива не содержит данных.'");
		ВызватьИсключение(СообщениеОбОшибке);
...
Процедура ЗагрузитьДанныеXDTOВИнформационнуюБазу(ПараметрыWEBСервиса, СообщениеОбОшибке) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ka='შეცდომა მონაცემების ჩატვირთვისას: %1';ru='Ошибка при загрузке данных: %1'");
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления() Экспорт
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ka='Синхронизация данных временно недоступна в связи с обновлением приложения.';ru='Синхронизация данных временно недоступна в связи с обновлением приложения.'");
...
Функция СформироватьКлючСообщенияЖР(НаправлениеОбмена, ПараметрыWEBСервиса)
	...
	Если НаправлениеОбмена = "Получение" Тогда
		ШаблонКлючаСообщения = НСтр("ka='მონაცემთა ჩატვირთვა Web-სერვის %1-ით';ru='Загрузка данных через Web-сервис %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		ШаблонКлючаСообщения = НСтр("ka='მონაცემთა გადმოტვირთვა Web-სერვის %1-ით';ru='Выгрузка данных через Web-сервис %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
