...
Функция ТекущаяНастройкаРезервногоКопирования() Экспорт
	...
	Если НастройкиРезервногоКопирования = Неопределено Тогда
		Возврат НСтр("ka='სარეზერვო კოპირების გასამართავად აუცილებელია მიმართოთ ადმინისტრატორს.';ru='Для настройки резервного копирования необходимо обратиться к администратору.'");
	КонецЕсли;
	
	ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება არ არის გამართული, საინფორმაციო ბაზას მონაცემთა დაკარგვის საფრთხე ემუქრება.';ru='Резервное копирование не настроено, информационная база подвергается риску потери данных.'");
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Если НастройкиРезервногоКопирования.ВыполнятьАвтоматическоеРезервноеКопирование Тогда
			...
			Если НастройкиРезервногоКопирования.ВариантВыполнения = "ПриЗавершенииРаботы" Тогда
				ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება რეგულარულად ხორციელდება მუშაობის დასრულებისას.';ru='Резервное копирование выполняется регулярно при завершении работы.'");
			ИначеЕсли НастройкиРезервногоКопирования.ВариантВыполнения = "ПоРасписанию" Тогда // По расписанию
				...
				Если Не ПустаяСтрока(Расписание) Тогда
					ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება რეგულარულად ხორციელდება განრიგის მიხედვით: %1';ru='Резервное копирование выполняется регулярно по расписанию: %1'");
		...
		Иначе
			...
			Если НастройкиРезервногоКопирования.РезервноеКопированиеНастроено Тогда
				ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება არ ხორციელდება (ორგანიზებულია გარეშე პროგრამებით).';ru='Резервное копирование не выполняется (организовано сторонними программами).'");
			КонецЕсли;
	...
	Иначе
		...
		ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირების შესრულება არ ხდება (ორგანიზებულია მბმს-ის საშუალებებით).';ru='Резервное копирование не выполняется (организовано средствами СУБД).'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если Не ОтключеноУведомлениеОНастройкеРезервногоКопирования Тогда
			...
			Дело.Представление  = НСтр("ka='სარეზერვო კოპირების გამართვა';ru='Настроить резервное копирование'");
			Дело.Важное         = Истина;
		...
		Если Не ОтключеноУведомлениеОВыполненииРезервногоКопирования Тогда
			...
			Дело.Представление  = НСтр("ka='სარეზერვო კოპირება არ შესრულებულა';ru='Резервное копирование не выполнено'");
			Дело.Важное         = Истина;
