...
Функция ТекущаяНастройкаРезервногоКопирования() Экспорт
	...
	Если НастройкиРезервногоКопирования = Неопределено Тогда
		Возврат НСтр("ka='სარეზერვო კოპირების გასამართავად აუცილებელია მიმართოთ ადმინისტრატორს.';ru='Для настройки резервного копирования необходимо обратиться к администратору.';en='To back up the settings you need to contact the administrator.'");
	КонецЕсли;
	
	ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება არ არის გამართული, საინფორმაციო ბაზას მონაცემთა დაკარგვის საფრთხე ემუქრება.';ru='Резервное копирование не настроено, информационная база подвергается риску потери данных.';en='Backup is not configured, the information base is exposed to the risk of data loss.'");
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Если НастройкиРезервногоКопирования.ВыполнятьАвтоматическоеРезервноеКопирование Тогда
			...
			Если НастройкиРезервногоКопирования.ВариантВыполнения = "ПриЗавершенииРаботы" Тогда
				ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება რეგულარულად ხორციელდება მუშაობის დასრულებისას.';ru='Резервное копирование выполняется регулярно при завершении работы.';en='Backup is done regularly during shutdown.'");
			ИначеЕсли НастройкиРезервногоКопирования.ВариантВыполнения = "ПоРасписанию" Тогда // По расписанию
				...
				Если Не ПустаяСтрока(Расписание) Тогда
					ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება რეგულარულად ხორციელდება განრიგის მიხედვით: %1';ru='Резервное копирование выполняется регулярно по расписанию: %1';en='Backup is done regularly on a schedule: 1%'");
		...
		Иначе
			...
			Если НастройкиРезервногоКопирования.РезервноеКопированиеНастроено Тогда
				ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირება არ ხორციელდება (ორგანიზებულია გარეშე პროგრამებით).';ru='Резервное копирование не выполняется (организовано сторонними программами).';en='Backup is not performed (hosted by third-party software).'");
			КонецЕсли;
	...
	Иначе
		...
		ТекущаяНастройка = НСтр("ka='სარეზერვო კოპირების შესრულება არ ხდება (ორგანიზებულია მბმს-ის საშუალებებით).';ru='Резервное копирование не выполняется (организовано средствами СУБД).';en='Backup is not satisfied (organized database means).'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если Не ОтключеноУведомлениеОНастройкеРезервногоКопирования Тогда
			...
			Дело.Представление  = НСтр("ka='სარეზერვო კოპირების გამართვა';ru='Настроить резервное копирование';en='Adjust backup'");
			Дело.Важное         = Истина;
		...
		Если Не ОтключеноУведомлениеОВыполненииРезервногоКопирования Тогда
			...
			Дело.Представление  = НСтр("ka='სარეზერვო კოპირება არ შესრულებულა';ru='Резервное копирование не выполнено';en='Backup is not satisfied'");
			Дело.Важное         = Истина;
