...
Процедура ОбработатьСообщение(Знач КаналСообщений, Знач ТелоСообщения, Знач Отправитель) Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. შეტყობინების დამუშავების შეცდომა';ru='Поставляемые данные.Ошибка обработки сообщения'", 
...
Процедура ОбработатьНовыйДескриптор(Знач Дескриптор) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მისაწოდებელი მონაცემები. ხელმისაწვდომია ახალი მონაცემები';ru='Поставляемые данные.Доступны новые данные'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньЖурналаРегистрации.Информация, ,
		, ?(Загружать, НСтр("ka='რიგში დამატებულია ჩატვირთვის დავალება.';ru='В очередь добавлено задание на загрузку.'"), НСтр("ka='მონაცემთა ჩატვირთვა არ არის საჭირო.';ru='Загрузка данных не требуется.'"))
...
Процедура ЗагрузитьДанные(Знач ДескрипторXML) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. XML-თან მუშაობის შეცდომა';ru='Поставляемые данные.Ошибка работы с XML'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, ,
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. მონაცემთა ჩატვირთვა';ru='Поставляемые данные.Загрузка данных'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньЖурналаРегистрации.Информация, ,
		, НСтр("ka='ჩატვირთვა დაწყებულია';ru='Загрузка начата'") + Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор));
	...
	Если ЗначениеЗаполнено(Дескриптор.FileGUID) Тогда
		...
		Если ИмяФайлаВыгрузки = Неопределено Тогда
			ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. მონაცემთა ჩატვირთვა';ru='Поставляемые данные.Загрузка данных'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Информация, ,
				, НСтр("ka='ფაილი არ შეიძლება, რომ ჩაიტვირთოს';ru='Файл не может быть загружен'") + Символы.ПС 
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. მონაცემთა ჩატვირთვა';ru='Поставляемые данные.Загрузка данных'", 
		...
		, НСтр("ka='ჩატვირთვა წარმატებით შესრულდა';ru='Загрузка успешно выполнена'") + Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор));
	...
	Для каждого Обработчик Из ПолучитьОбработчики(Дескриптор.DataType) Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. დამუშავების შეცდომა';ru='Поставляемые данные.Ошибка обработки'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка, ,
				, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
				+ Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор)
				+ Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებლის კოდი: %1';ru='Код обработчика: %1'"), Обработчик.КодОбработчика));
	...
	Если ИмяФайлаВыгрузки <> Неопределено Тогда
		...
		Если ФайлВременный.Существует() Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. მონაცემთა ჩატვირთვა';ru='Поставляемые данные.Загрузка данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
	...
	Если ТранзакцияАктивна() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. დამუშავების შეცდომა';ru='Поставляемые данные.Ошибка обработки'", 
			...
			,
			, 
			НСтр("ka='დამამუშავებლის შესრულების დასრულებისას ტრანზაქცია არ დახურულა';ru='По завершении выполнения обработчика не была закрыта транзакция'")
				 + Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор));
	...
	Если БылиОшибки Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. დამუშავების შეცდომა';ru='Поставляемые данные.Ошибка обработки'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ka='მონაცემთა დამუშავება გაშვებული იქნება განმეორებით, დამამუშავებლის შეცდომის გამო.';ru='Обработка данных будет запущена повторно из-за ошибки обработчика.'")
			 + Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор));
	Иначе
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. მონაცემთა ჩატვირთვა';ru='Поставляемые данные.Загрузка данных'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Информация, ,
			, НСтр("ka='ახალი მონაცემები დამუშავებულია';ru='Новые данные обработаны'") + Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор));
...
Процедура УведомитьОбОтменеОбработки(Знач Обработчик, Знач Дескриптор)
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. დამუშავების გაუქმება';ru='Поставляемые данные.Отмена обработки'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Информация, ,
			, ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор)
			+ Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამამუშავებლის კოდი: %1';ru='Код обработчика: %1'"), Обработчик.КодОбработчика));
...
Функция ПолучитьФайлИзХранилища(Знач Дескриптор)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='მოსაწოდებელი მონაცემები. საცავის შეცდომა';ru='Поставляемые данные.Ошибка хранилища'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, ,
			, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			+ Символы.ПС + ПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор));
