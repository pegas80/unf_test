...
Процедура СоздатьКонтрагентаПоРеквизитам(СтруктураРеквизитов, Отказ) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ცნობარ ""კონტრაგენტები""-ს ახალი ელემენტის ჩაწერა';ru='Запись нового элемента справочника Контрагенты';en='Recording a new element Contractors directory'"),
...
"Подробности см. в Журнале регистрации.';ka='შეცდომა ცნობარ ""კონტრაგენტები""-ს ახალი ელემენტის ჩაწერისას"
...
Процедура ВыполнитьКонтрольРеквизитовДокументов(МассивСсылокНаОбъект, ТекстСообщения, Отказ) Экспорт
	...
	Для каждого Ссылка Из МассивСсылокНаОбъект Цикл
		...
		ИначеЕсли ИмяМетаданных <> Ссылка.Метаданные().Имя Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='დოკუმენტების სხვადასხვა სახეობისთვის ოპერაცია შეუძლებელია';ru='Операция невозможна для разных видов документов';en='Operation is not possible for different types of documents'");
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Если Выборка.Количество() > 1 Тогда
		Отказ = Истина;
		Шаблон = НСтр("ka='ოპერაცია შეუძლებელია. დოკუმენტების რეკვიზიტები განსხვავდება (%1)';ru='Операция невозможна. Отличаются реквизиты документов (%1)';en='Operation is not possible. Different requisites of documents (% 1)'");
