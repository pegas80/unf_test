...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	// Kuzmich, 12.09.2023, Добавление параметра расчетов Норма часов сотрудника
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия      = "*";
	Обработчик.Процедура   = "ОбновлениеИнформационнойБазыУНФ.ДобавлениеПараметраРасчетовИВидаНачисленийПоГрафикуСотрудника";
	Обработчик.ОбщиеДанные = Истина;
	// Kuzmich, 12.09.2023, Добавление параметра расчетов Норма часов сотрудника
	
	//Малышев 2024-10-18 ONESOFT-3521 (--
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия      = "*";
	Обработчик.Процедура   = "ОбновлениеИнформационнойБазыУНФ.ПереносПроектовОтправителейВПроектыВПеремещениях";
	Обработчик.ОбщиеДанные = Истина;
	//Малышев 2024-11-18 ONESOFT-3521 --)
...
Процедура ЗаполнитьПараметрыРасчетовИВидыНачислений()
	...
	//+++ Дмгг, 26.11.2025, ONESOFT-4497
	// Норма часов по графику работы
	Если НЕ УправлениеНебольшойФирмойСервер.ПараметрРасчетаСуществует("НормаЧасовПоГрафикуРаботы") Тогда
		
		ПараметрНормаЧасовПоГрафикуРаботы = Справочники.ПараметрыРасчетов.СоздатьЭлемент();
		ПараметрНормаЧасовПоГрафикуРаботы.Наименование 		 = "Норма часов по графику работы";
		ПараметрНормаЧасовПоГрафикуРаботы.Идентификатор 	 = "НормаЧасовПоГрафикуРаботы";
		ПараметрНормаЧасовПоГрафикуРаботы.ПроизвольныйЗапрос = Истина;
		ПараметрНормаЧасовПоГрафикуРаботы.ЗадаватьЗначениеПриРасчетеЗП = Ложь;
		НовыйПараметрЗапроса 						 = ПараметрНормаЧасовПоГрафикуРаботы.ПараметрыЗапроса.Добавить();
		НовыйПараметрЗапроса.Имя 					 = "Организация";
		НовыйПараметрЗапроса.Представление 			 = "Организация";
		НовыйПараметрЗапроса 						 = ПараметрНормаЧасовПоГрафикуРаботы.ПараметрыЗапроса.Добавить();
		НовыйПараметрЗапроса.Имя 					 = "ПериодРегистрации";
		...
		ПараметрРасчетовНормаДней.Запрос 			 = 
		"ВЫБРАТЬ
		|	СУММА(РАЗНОСТЬДАТ(РегистрСведенийГрафикиРаботы.ВремяНачала, ДОБАВИТЬКДАТЕ(РегистрСведенийГрафикиРаботы.ВремяОкончания, СЕКУНДА, 1), ЧАС)) КАК КоличествоЧвсов,
		|	МАКСИМУМ(РегистрСведенийСотрудники.Период) КАК Период
		|ПОМЕСТИТЬ ВременнаяТаблицаДанныеСотрудника
		|ИЗ
		|	РегистрСведений.Сотрудники.СрезПоследних(&ПериодРегистрации, ) КАК РегистрСведенийСотрудники
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботы КАК РегистрСведенийГрафикиРаботы
		|		ПО РегистрСведенийСотрудники.Период <= РегистрСведенийГрафикиРаботы.ВремяНачала
		|			И РегистрСведенийСотрудники.ГрафикРаботы = РегистрСведенийГрафикиРаботы.ГрафикРаботы
		|ГДЕ
		|	РегистрСведенийСотрудники.Организация = &Организация
		|	И РегистрСведенийСотрудники.Сотрудник = &Сотрудник
		|	И РегистрСведенийГрафикиРаботы.ВремяНачала >= РегистрСведенийСотрудники.Период
		|	И РегистрСведенийГрафикиРаботы.ВремяНачала МЕЖДУ НАЧАЛОПЕРИОДА(&ПериодРегистрации, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ПериодРегистрации, МЕСЯЦ)
		|
		|СГРУППИРОВАТЬ ПО
		|	РегистрСведенийГрафикиРаботы.ВремяНачала
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВременнаяТаблицаДанныеСотрудника.КоличествоЧвсов) КАК КоличествоЧвсов
		|ИЗ
		|	ВременнаяТаблицаДанныеСотрудника КАК ВременнаяТаблицаДанныеСотрудника";
		
		ПараметрНормаЧасовПоГрафикуРаботы.Записать();
...
Процедура ЗаполнитьДокументыФизическихЛицПоКлассификаторуМВД(СтруктураПараметров = Неопределено) Экспорт
	...
	//ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ПаспортГР,				"",		"99", "საქართველოს პასპორტი");        Rajak
...
#Область Обработчики_Общие

// Kuzmich, 12.09.2023, Добавление параметра расчетов Норма часов сотрудника
Процедура ДобавлениеПараметраРасчетовИВидаНачисленийПоГрафикуСотрудника(парам = Неопределено) Экспорт
	
	// Норма часов сотрудника
	Если НЕ УправлениеНебольшойФирмойСервер.ПараметрРасчетаСуществует("НормаЧасовСотрудника") Тогда
		
		ПараметрРасчетовНормаЧасовСотрудника = Справочники.ПараметрыРасчетов.СоздатьЭлемент();
		ПараметрРасчетовНормаЧасовСотрудника.Наименование 	  = "Норма часов сотрудника";
		ПараметрРасчетовНормаЧасовСотрудника.Идентификатор 	  = "НормаЧасовСотрудника";
		ПараметрРасчетовНормаЧасовСотрудника.ПроизвольныйЗапрос = Истина;
		ПараметрРасчетовНормаЧасовСотрудника.ЗадаватьЗначениеПриРасчетеЗП = Ложь;
		НовыйПараметрЗапроса 						 = ПараметрРасчетовНормаЧасовСотрудника.ПараметрыЗапроса.Добавить();
		НовыйПараметрЗапроса.Имя 					 = "Организация";
		НовыйПараметрЗапроса.Представление 			 = "Организация";
		НовыйПараметрЗапроса 						 = ПараметрРасчетовНормаЧасовСотрудника.ПараметрыЗапроса.Добавить();
		НовыйПараметрЗапроса.Имя 					 = "ПериодРегистрации";
		...
		НовыйПараметрЗапроса.Имя 					 = "Сотрудник";
		НовыйПараметрЗапроса.Представление 			 = "Сотрудник";
		ПараметрРасчетовНормаЧасовСотрудника.Запрос 			 = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	КалендарныеГрафики.ДатаГрафика КАК ДатаГрафика
		|ПОМЕСТИТЬ ВТ_ДниМесяца
		|ИЗ
		|	РегистрСведений.КалендарныеГрафики КАК КалендарныеГрафики
		|ГДЕ
		|	КалендарныеГрафики.ДатаГрафика МЕЖДУ НАЧАЛОПЕРИОДА(&ПериодРегистрации, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ПериодРегистрации, МЕСЯЦ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ДниМесяца.ДатаГрафика КАК ДатаГрафика,
		|	МАКСИМУМ(РегистрСведенийСотрудники.Период) КАК Период
		|ПОМЕСТИТЬ ВТ_ДниМесяца_РегистрСведенийСотрудникиПериод
		|ИЗ
		|	РегистрСведений.Сотрудники КАК РегистрСведенийСотрудники
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДниМесяца КАК ВТ_ДниМесяца
		|		ПО РегистрСведенийСотрудники.Период <= ВТ_ДниМесяца.ДатаГрафика
		|ГДЕ
		|	РегистрСведенийСотрудники.Организация = &Организация
		|	И РегистрСведенийСотрудники.Сотрудник = &Сотрудник
		|	И ВТ_ДниМесяца.ДатаГрафика >= РегистрСведенийСотрудники.Период
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ДниМесяца.ДатаГрафика
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТ_ДниМесяца_РегистрСведенийСотрудникиПериод.ДатаГрафика КАК ДатаГрафика,
		|	РегистрСведенийСотрудники.ГрафикРаботы.Календарь КАК ГрафикРаботыСотрудника,
		|	РАЗНОСТЬДАТ(ГрафикиРаботыПериоды.ВремяНачала, ГрафикиРаботыПериоды.ВремяОкончания, МИНУТА) / 60 КАК РабочихЧасов
		|ПОМЕСТИТЬ ВТ_ГрафикиСотрудника
		|ИЗ
		|	ВТ_ДниМесяца_РегистрСведенийСотрудникиПериод КАК ВТ_ДниМесяца_РегистрСведенийСотрудникиПериод
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Сотрудники КАК РегистрСведенийСотрудники
		|		ПО (РегистрСведенийСотрудники.Период = ВТ_ДниМесяца_РегистрСведенийСотрудникиПериод.Период)
		|			И (РегистрСведенийСотрудники.Организация = &Организация)
		|			И (РегистрСведенийСотрудники.Сотрудник = &Сотрудник)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГрафикиРаботы.Периоды КАК ГрафикиРаботыПериоды
		|		ПО (РегистрСведенийСотрудники.ГрафикРаботы = ГрафикиРаботыПериоды.Ссылка)
		|			И (ДЕНЬНЕДЕЛИ(ВТ_ДниМесяца_РегистрСведенийСотрудникиПериод.ДатаГрафика) = ГрафикиРаботыПериоды.НомерДня)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВТ_ГрафикиСотрудника.РабочихЧасов) КАК НормаЧасовСотрудника
		|ИЗ
		|	РегистрСведений.КалендарныеГрафики КАК КалендарныеГрафики
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ГрафикиСотрудника КАК ВТ_ГрафикиСотрудника
		|		ПО КалендарныеГрафики.Календарь = ВТ_ГрафикиСотрудника.ГрафикРаботыСотрудника
		|			И КалендарныеГрафики.ДатаГрафика = ВТ_ГрафикиСотрудника.ДатаГрафика
		|ГДЕ
		|	КалендарныеГрафики.Год = ГОД(&ПериодРегистрации)
		|	И КалендарныеГрафики.ДатаГрафика МЕЖДУ НАЧАЛОПЕРИОДА(&ПериодРегистрации, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ПериодРегистрации, МЕСЯЦ)
		|	И КалендарныеГрафики.ДеньВключенВГрафик";
		ПараметрРасчетовНормаЧасовСотрудника.Записать();
		
	КонецЕсли;
	
	// Вид начислений Оклад по часам сотрудника
	НаименованиеНачисленийПочасоваяОплатаСотрадника = "თანამშრომელის საათობრივი ანაზღაურება";
	Если НЕ ЗначениеЗаполнено(Справочники.ВидыНачисленийИУдержаний.НайтиПоНаименованию(НаименованиеНачисленийПочасоваяОплатаСотрадника,ИСТИНА)) Тогда
		
		// Группа Начисление
		ГруппаНачисление = Справочники.ВидыНачисленийИУдержаний.НайтиПоНаименованию("Начисления",ИСТИНА);
		Если НЕ ЗначениеЗаполнено(ГруппаНачисление) Тогда
			ГруппаНачислениеОбъект 			 = Справочники.ВидыНачисленийИУдержаний.СоздатьГруппу();
			ГруппаНачислениеОбъект.Наименование = "Начисления";
			ГруппаНачислениеОбъект.Записать();
			
			ГруппаНачисление = ГруппаНачислениеОбъект.Ссылка;
		КонецЕсли;
		
		СчетЗатрат = ПланыСчетов.Управленческий.НайтиПоКоду("7410");
		СчетЗатратРегл = ПланыСчетов.Хозрасчетный.НайтиПоКоду("7410");
		
		НовоеНачисление 					= Справочники.ВидыНачисленийИУдержаний.СоздатьЭлемент();
		НовоеНачисление.Родитель 			= ГруппаНачисление;
		НовоеНачисление.Наименование 		= НаименованиеНачисленийПочасоваяОплатаСотрадника;
		НовоеНачисление.Тип 				= Перечисления.ТипыНачисленийИУдержаний.Начисление;
		НовоеНачисление.СчетЗатрат			= ?(ЗначениеЗаполнено(СчетЗатрат),СчетЗатратРегл,ПланыСчетов.Управленческий.КосвенныеЗатраты);
		НовоеНачисление.СчетЗатратРегл		= СчетЗатратРегл;
		НовоеНачисление.Формула 			= "[ТарифнаяСтавка] * [ОтработаноЧасов] / [НормаЧасовСотрудника]";
		НовоеНачисление.КодДоходаНДФЛ		= Справочники.КодыДоходовНДФЛ.Код2000;
		НовоеНачисление.ВидДоходаПоСтраховымВзносам = Справочники.ВидыДоходовПоСтраховымВзносам.ОблагаетсяЦеликом;
		НовоеНачисление.Записать();
...
//Малышев 2024-11-18 ONESOFT-3521 (--
Процедура ПереносПроектовОтправителейВПроектыВПеремещениях(парам = Неопределено) Экспорт
	...
	|	ГДЕ
	|		ПеремещениеЗапасов.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.пустаяСсылка)
	|		И ПеремещениеЗапасов.ПроектОтправитель <> ЗНАЧЕНИЕ(Справочник.Проекты.пустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ПеремещениеЗапасов.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.пустаяСсылка)
	|		И ПеремещениеЗапасов.ПроектОтправитель <> ЗНАЧЕНИЕ(Справочник.Проекты.пустаяСсылка)) КАК Т
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Ссылка
	|
	...
	Если не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Док.ПроектОтправитель <> Справочники.Проекты.ПустаяСсылка()
				и Док.Проект = Справочники.Проекты.ПустаяСсылка()
				Тогда
				Док.Проект = Док.ПроектОтправитель;
				Док.ПроектОтправитель = Справочники.Проекты.ПустаяСсылка();
			КонецЕсли;
			
			Для Каждого СтрокаДокумента из Док.Запасы Цикл
				Если СтрокаДокумента.ПроектОтправитель <> Справочники.Проекты.ПустаяСсылка()
					и СтрокаДокумента.Проект = Справочники.Проекты.ПустаяСсылка() 
					Тогда
						СтрокаДокумента.Проект = СтрокаДокумента.ПроектОтправитель;
						СтрокаДокумента.ПроектОтправитель = Справочники.Проекты.ПустаяСсылка();
				КонецЕсли;	
			КонецЦикла;	
