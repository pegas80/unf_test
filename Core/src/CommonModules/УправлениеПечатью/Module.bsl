...
Функция МакетПечатнойФормы(ПолныйПутьКМакету) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მაკეტი ""%1"" ვერ მოიძებნა. ოპერაცია შეწყვეტილია.';ru='Макет ""%1"" не найден. Операция прервана.';en='Layout ""%1"" not found. Operation aborted.'"), ПолныйПутьКМакету);
	КонецЕсли;
...
Функция ДанныеQRКода(QRСтрока, УровеньКоррекции, Размер) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='QR-კოდების ფორმირება';ru='Формирование QR-кода';en='Formation of the QR-code'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='საბეჭდი ფორმების მაკეტები';ru='Макеты печатных форм';en='Layouts of printed forms'");
...
Функция Разрешения()
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты(
		"ОбщийМакет.КомпонентаПечатиQRКода", НСтр("ka='QR კოდების ბეჭდვა.';ru='Печать QR кодов.';en='Print QR codes.'")));
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Для Каждого ОписаниеПечатнойФормы Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბეჭდური ფორმა ""%1""-ისთვის ეგზემპლარების რაოდენობა არ არის მიცემული.';ru='Не задано количество экземпляров для печатной формы ""%1"".';en='Do not set the number of copies for the printing form ""%1"".'"),
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბეჭდური ფორმა ""%1""-ის ჩამოყალიბებისას შეცდომა წარმოიშვა. მიმართეთ ადმინისტრატორს.';ru='При формировании печатной формы ""%1"" возникла ошибка. Обратитесь к администратору.';en='An error occurred during the formation of the printed form ""%1"". Contact your administrator.'"), ИмяМакета);
...
Процедура ДобавитьКомандыПечати(Форма, КомандыПечати, Знач МестоРазмещенияКоманд = Неопределено)
	...
	Иначе
		Если Не ОднаКомандаПечати Тогда
			...
			ПодменюПечать.Заголовок = НСтр("ka='ბეჭდვა';ru='Печать';en='Print'");
	...
	КомандаФормы.Заголовок = НСтр("ru='(нет печатных форм)';en='(No printed forms)'");
	КомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
	КомандаФормы.Отображение = ОтображениеКнопки.КартинкаИТекст;
...
Функция СтрокаОбязательныхРеквизитов(ДанныеДокумента, ТекстСообщения)
	...
	Для Каждого Элемент Из ОбязательныеРеквизиты Цикл
		Если Не ЗначениеЗаполнено(ДанныеДокумента[Элемент.Ключ]) Тогда
			ТекстСообщения = НСтр("ka='სავალდებულო რეკვიზიტი არ არის შევსებული: %1';ru='Не заполнен обязательный реквизит: %1';en='Do not fill in the required props: 1%'");
...
"""%2""';ka='დოკუმენტ %1-ისთვის QR-კოდის შექმნა შეუძლებელია"
"სავალდებულო რეკვიზიტების სტრიქონი 300 სიმბოლოზე ნაკლები უნდა იყოს:"
"""%2""'");
...
Функция КомпонентаФормированияQRКода(Отказ)
	...
	ТекстОшибки = НСтр("ka='QR-კოდის გენერაციისთვის გარე კომპონენტის შეერთება ვერ მოხერხდა';ru='Не удалось подключить внешнюю компоненту для генерации QR-кода';en='Failed to connect external components to generate QR-code'");
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბეჭდვა %1 არ შესრულებულა: არჩეული ბეჭდური ფორმა მიუწვდომელია.';ru='Печать %1 не выполнена: выбранная печатная форма недоступна.';en='% 1 Printing is not performed: the selected printing plate unavailable.'"), Объект);
