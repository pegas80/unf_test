...
Функция СоседнийЭлемент(ПеремещаемыйЭлемент, Список, Знач Направление)
	...
	Если Не ПоляОтбораВСпискеДоступных(КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.Элементы, КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.ДоступныеПоляОтбора.Элементы) Тогда
		ВызватьИсключение НСтр("ka='ელემენტის გადასაადგილებლად აუცილებელია ყველა შერჩევის გამორთვა სიაში.';ru='Для перемещения элемента необходимо отключить все отборы в списке.';en='To move an item you want to disable all the selections in the list.'");
	КонецЕсли;
...
Функция ПроверитьВозможностьПеремещения(Ссылка, Список, ОтображениеСписком)
	...
	Если Не ПараметрыДоступа.Доступность Тогда
		Возврат НСтр("ka='ელემენტების თანმიმდევრობის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения порядка элементов.';en='Not have permission to change the order of items.'");
	КонецЕсли;
	...
	Для Каждого ЭлементГруппировки Из Список.КомпоновщикНастроек.ПолучитьНастройки().Структура Цикл
		Если ЭлементГруппировки.Использование Тогда
			Возврат НСтр("ka='ელემენტების რიგითობის შესაცვლელად აუცილებელია ყველა დაჯგუფების გამორთვა.';ru='Для изменения порядка элементов необходимо отключить все группировки.';en='To change the order of items you need to disable all groupings.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если Информация.ЕстьРодитель И ОтображениеСписком И Не СписокСодержитОтборПоРодителю(Список) Тогда
		Возврат НСтр("ka='ელემენტების რიგითობის შესაცვლელად აუცილებელია ნახვის რეჟიმი ""ხე"" ან ""იერარქიული სია"".';ru='Для изменения порядка элементов необходимо установить режим просмотра ""Дерево"" или ""Иерархический список"".';en='To change the order of the elements necessary to establish the view ""tree"" or ""hierarchical list.""'");
	КонецЕсли;
	...
	Если Информация.ЕстьВладелец И Не СписокСодержитОтборПоВладельцу(Список) Тогда
		Возврат НСтр("ka='ელემენტების თანმიმდევრობის შესაცვლელად, აუცილებელია ველ ""მფლობელის"" მიხედვით შერჩევის დაყენება.';ru='Для изменения порядка элементов необходимо установить отбор по полю ""Владелец"".';en='you need to install the selection of the field to change the order of items ""Owner"".'");
	КонецЕсли;
	...
	Если Информация.ЕстьГруппы Тогда
		...
		Если ЭтоГруппа И Не Информация.ДляГрупп Или Не ЭтоГруппа И Не Информация.ДляЭлементов Тогда
			Возврат НСтр("ka='არჩეული ელემენტის გადაადგილება არ შეიძლება.';ru='Выбранный элемент нельзя перемещать.';en='The selected item can not be moved.'");
		КонецЕсли;
	КонецЕсли;
