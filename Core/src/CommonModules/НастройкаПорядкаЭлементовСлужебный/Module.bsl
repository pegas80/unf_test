...
Функция СоседнийЭлемент(ПеремещаемыйЭлемент, Список, Знач Направление)
	...
	Если Не ПоляОтбораВСпискеДоступных(КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.Элементы, КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.ДоступныеПоляОтбора.Элементы) Тогда
		ВызватьИсключение НСтр("ka='ელემენტის გადასაადგილებლად აუცილებელია ყველა შერჩევის გამორთვა სიაში.';ru='Для перемещения элемента необходимо отключить все отборы в списке.'");
	КонецЕсли;
...
Функция ПроверитьВозможностьПеремещения(Ссылка, Список, ОтображениеСписком)
	...
	Если Не ПараметрыДоступа.Доступность Тогда
		Возврат НСтр("ka='ელემენტების თანმიმდევრობის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения порядка элементов.'");
	КонецЕсли;
	...
	Для Каждого ЭлементГруппировки Из Список.КомпоновщикНастроек.ПолучитьНастройки().Структура Цикл
		Если ЭлементГруппировки.Использование Тогда
			Возврат НСтр("ka='ელემენტების რიგითობის შესაცვლელად აუცილებელია ყველა დაჯგუფების გამორთვა.';ru='Для изменения порядка элементов необходимо отключить все группировки.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если Информация.ЕстьРодитель И ОтображениеСписком И Не СписокСодержитОтборПоРодителю(Список) Тогда
		Возврат НСтр("ka='ელემენტების რიგითობის შესაცვლელად აუცილებელია ნახვის რეჟიმი ""ხე"" ან ""იერარქიული სია"".';ru='Для изменения порядка элементов необходимо установить режим просмотра ""Дерево"" или ""Иерархический список"".'");
	КонецЕсли;
	...
	Если Информация.ЕстьВладелец И Не СписокСодержитОтборПоВладельцу(Список) Тогда
		Возврат НСтр("ka='ელემენტების თანმიმდევრობის შესაცვლელად, აუცილებელია ველ ""მფლობელის"" მიხედვით შერჩევის დაყენება.';ru='Для изменения порядка элементов необходимо установить отбор по полю ""Владелец"".'");
	КонецЕсли;
	...
	Если Информация.ЕстьГруппы Тогда
		...
		Если ЭтоГруппа И Не Информация.ДляГрупп Или Не ЭтоГруппа И Не Информация.ДляЭлементов Тогда
			Возврат НСтр("ka='არჩეული ელემენტის გადაადგილება არ შეიძლება.';ru='Выбранный элемент нельзя перемещать.'");
		КонецЕсли;
	КонецЕсли;
