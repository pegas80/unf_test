...
Процедура Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры.Вставить("Операция",            НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных'"));
...
Процедура Зашифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("ka='მონაცემების დაშიფვრა';ru='Шифрование данных'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ka='მონაცემები';ru='Данные'"));
...
Процедура Расшифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("ka='მონაცემების გაშფვრა';ru='Расшифровка данных'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ka='მონაცემები';ru='Данные'"));
	СерверныеПараметры.Вставить("ОтборСертификатов");
...
Процедура ИнформироватьОПодписанииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь, ИзФайла = Ложь) Экспорт
	...
	Если ИзФайла Тогда
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("ka='ფაილებიდან დამატებულია ხელმოწერები:';ru='Добавлены подписи из файлов:'");
		Иначе
			ТекстСообщения = НСтр("ka='დამატებულია ხელმოწერა ფაილიდან:';ru='Добавлена подпись из файла:'");
		КонецЕсли;
	Иначе
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("ka='დაყენებული ხელმოწერები:';ru='Установлены подписи:'");
		Иначе
			ТекстСообщения = НСтр("ka='დაყენებულია ხელმოწერა:';ru='Установлена подпись:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнформироватьОШифрованииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("ka='შესრულებულია გაშიფვრა:';ru='Выполнено шифрование:'");
...
Процедура ИнформироватьОРасшифровкеОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("ka='შესრულებულია გაშიფრვა:';ru='Выполнена расшифровка:'");
