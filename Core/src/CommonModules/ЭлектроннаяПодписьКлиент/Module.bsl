...
Процедура Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры.Вставить("Операция",            НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных';en='Signing data'"));
...
Процедура Зашифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("ka='მონაცემების დაშიფვრა';ru='Шифрование данных';en='data encryption'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ka='მონაცემები';ru='Данные';en='Data'"));
...
Процедура Расшифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("ka='მონაცემების გაშფვრა';ru='Расшифровка данных';en='decrypting data'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ka='მონაცემები';ru='Данные';en='Data'"));
	СерверныеПараметры.Вставить("ОтборСертификатов");
...
Процедура ИнформироватьОПодписанииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь, ИзФайла = Ложь) Экспорт
	...
	Если ИзФайла Тогда
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("ka='ფაილებიდან დამატებულია ხელმოწერები:';ru='Добавлены подписи из файлов:';en='Added a signature from file:'");
		Иначе
			ТекстСообщения = НСтр("ka='დამატებულია ხელმოწერა ფაილიდან:';ru='Добавлена подпись из файла:';en='Added a signature from a file:'");
		КонецЕсли;
	Иначе
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("ka='დაყენებული ხელმოწერები:';ru='Установлены подписи:';en='Set the signature:'");
		Иначе
			ТекстСообщения = НСтр("ka='დაყენებულია ხელმოწერა:';ru='Установлена подпись:';en='Installed signature:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнформироватьОШифрованииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("ka='შესრულებულია გაშიფვრა:';ru='Выполнено шифрование:';en='Encrypt:'");
...
Процедура ИнформироватьОРасшифровкеОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("ka='შესრულებულია გაშიფრვა:';ru='Выполнена расшифровка:';en='Made transcript:'");
