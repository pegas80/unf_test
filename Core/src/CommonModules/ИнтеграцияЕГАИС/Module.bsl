...
Функция СправочникВидовАлкогольнойПродукции() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Определено более одного справочника видов алкогольной продукции.';ru='Определено более одного справочника видов алкогольной продукции.'");
	КонецЕсли;
...
Функция ПреобразоватьВXDTOОрганизацию_v2(Организация, ОрганизацияРФ, ТекстОшибки)
	...
	Если РеквизитыОрганизации.ТипОрганизации.Пустая() Тогда
		ТекстОшибки = НСтр("ka='Для организации %1 не заполнен тип.';ru='Для организации %1 не заполнен тип.'");
	...
	Если ОрганизацияРФ И РеквизитыОрганизации.ТипОрганизации <> Перечисления.ТипыОрганизацийЕГАИС.ЮридическоеЛицоРФ
		...
		ТекстОшибки = НСтр("ka='Организация %1 не зарегистрирована на территории РФ.';ru='Организация %1 не зарегистрирована на территории РФ.'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Организация);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция ПреобразоватьВXDTOАктПодтвержденияТТН(ВидДокумента, ДокументСсылка, ТекстОшибки)
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС") Тогда
		ДанныеДляВыгрузки = Документы.ТТНИсходящаяЕГАИС.ИнициализироватьДанныеДокументаДляВыгрузки(ДокументСсылка, ВидДокумента);
	Иначе
		ТекстОшибки = НСтр("ka='Для документа %1 не предусмотрена выгрузка акта подтверждения.';ru='Для документа %1 не предусмотрена выгрузка акта подтверждения.'");
...
Функция ОбработатьВходящиеДокументы(МассивДокументов, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтруктураДокумента Из МассивДокументов Цикл
		...
		Исключение
			ТекстОшибки = НСтр("ka='შემომავალი დოკუმენტ %1-ის წაკითხვა ვერ მოხერხდა';ru='Не удалось прочитать входящий документ %1'");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='ЕГАИС';ru='ЕГАИС'", Метаданные.ОсновнойЯзык.КодЯзыка),
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ActInventoryInformBReg = Неопределено
			...
			Если РезультатОперации.Результат Тогда
				РезультатОбработкиДокумента.КомментарийЕГАИС = ?(РезультатОперации.ПолученОтказ, РезультатОперации.ОписаниеОшибки, НСтр("ka='ცნობები დარეგისტრირებულია.';ru='Справки зарегистрированы.'"));
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.WayBillAct = Неопределено
			...
			Если РезультатОперации.Результат Тогда
				РезультатОбработкиДокумента.КомментарийЕГАИС = ?(РезультатОперации.ПолученОтказ, РезультатОперации.ОписаниеОшибки, НСтр("ka='Загружен акт подтверждения ТТН.';ru='Загружен акт подтверждения ТТН.'"));
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ConfirmTicket = Неопределено Тогда
			...
			Если РезультатОперации.Результат Тогда
				РезультатОбработкиДокумента.КомментарийЕГАИС = ?(РезультатОперации.ПолученОтказ, РезультатОперации.ОписаниеОшибки, НСтр("ka='Обработано подтверждение акта расхождений.';ru='Обработано подтверждение акта расхождений.'"));
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.Ticket = Неопределено Тогда
			...
			Иначе
				РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ka='Не удалось определить формат ответа на запрос %1';ru='Не удалось определить формат ответа на запрос %1'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyClient = Неопределено
			...
			Если СписокОрганизацийXDTO.Количество() = 0 Тогда
				РезультатОбработкиДокумента.ПолученОтказ = Истина;
				РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ka='Отсутствуют данные для загрузки.';ru='Отсутствуют данные для загрузки.'");
			Иначе
...
"Загружено элементов: %1.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyFormA = Неопределено
			...
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ka='Загружена справка 1.';ru='Загружена справка 1.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyFormB = Неопределено
			...
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ka='Загружена справка 2.';ru='Загружена справка 2.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyRests = Неопределено
			...
			Если НЕ ДанныеЗапроса = Неопределено И ЗначениеЗаполнено(ДанныеЗапроса.ДокументОснование) Тогда
				...
				РезультатОбработкиДокумента.ДокументОснование = ДанныеЗапроса.ДокументОснование;
			Иначе
				ТекстОшибки = НСтр("ka='Не найден исходящий документ запроса остатков.';ru='Не найден исходящий документ запроса остатков.'");
			КонецЕсли;
			
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ka='Загружены остатки по складу.';ru='Загружены остатки по складу.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyRestsShop_v2 = Неопределено Тогда
			...
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ka='Загружены остатки по торговому залу.';ru='Загружены остатки по торговому залу.'");
			РезультатОбработкиДокумента.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ОтветОстаткиВТорговомЗале;
			
			Результат.Вставить(СтруктураДокумента.АдресЗапроса, РезультатОбработкиДокумента);
			
		Иначе
			РезультатОбработкиДокумента.ОписаниеОшибки = НСтр("ka='Не удалось определить формат входящего документа %1';ru='Не удалось определить формат входящего документа %1'");
...
Функция ЗарегистрироватьСправкиПоАктуПостановкиНаБаланс(КвитанцияXDTO, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(КвитанцияXDTO.Header.Identity) Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Не заполнен идентификатор акта.';ru='Не заполнен идентификатор акта.'");
	...
	Исключение
		Результат.ОписаниеОшибки = НСтр("ka='Некорректный идентификатор акта %1.';ru='Некорректный идентификатор акта %1.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если АктОбъект = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Неизвестный идентификатор акта %1.';ru='Неизвестный идентификатор акта %1.'");
		Результат.ОписаниеОшибки = СтрЗаменить(Результат.ОписаниеОшибки, "%1", КвитанцияXDTO.Header.Identity);
		Результат.ПолученОтказ = Истина;
		Возврат Результат;
	КонецЕсли;
...
Функция ЗагрузитьПодтверждениеАктаРасхожденийТТН(КвитанцияXDTO, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(КвитанцияXDTO.Header.WBRegId) Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Не заполнен идентификатор ТТН.';ru='Не заполнен идентификатор ТТН.'");
		Результат.ПолученОтказ = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТТН) Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Не найдена входящая ТТН с идентификатором %1.';ru='Не найдена входящая ТТН с идентификатором %1.'");
...
Функция ЗагрузитьАктПодтвержденияТТН(АктXDTO, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(ТТН) Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Не найдена исходящая ТТН с идентификатором %1.';ru='Не найдена исходящая ТТН с идентификатором %1.'");
...
Функция ПредставлениеОшибкиXDTO(ПредставлениеОшибки, ЧтениеXML, Глубина = Неопределено)
	...
	Если СтрНайти(ПредставлениеОшибки, НСтр("ka='Ошибка проверки данных XDTO';ru='Ошибка проверки данных XDTO'")) = 0 Тогда
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			Если НЕ ЗначениеЗаполнено(ЗначениеПоля) Тогда
				ПредставлениеОшибки = НСтр("ka='Не заполнено значение поля ""%1"" (%2)';ru='Не заполнено значение поля ""%1"" (%2)'");
			...
			Иначе
				ПредставлениеОшибки = НСтр("ka='Некорректное значение ""%1"" поля ""%2"" (%3)';ru='Некорректное значение ""%1"" поля ""%2"" (%3)'");
		...
		Иначе
			...
			Пока ИндексСтроки > 0 Цикл
				...
				Если ПредыдущееПоле.Обязательное Тогда
					ПредставлениеОшибки = НСтр("ka='Отсутствует обязательное поле ""%1"" (%2)';ru='Отсутствует обязательное поле ""%1"" (%2)'");
...
Процедура СформироватьИдентификаторОбъектаРИБПередЗаписью(Источник, Отказ) Экспорт
	...
	Иначе
		...
		Если ПредыдущийКод <> Источник[ИмяРеквизита] Тогда
			Отказ = Истина;
			СтрокаСообщения = НСтр("ka='Изменение реквизита %1 для существующих элементов запрещено.';ru='Изменение реквизита %1 для существующих элементов запрещено.'");
...
Функция ЗаписатьОбъект(Объект, ТекстОшибки, РежимЗаписи = Неопределено)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ТекстОшибки = НСтр("ka='Ошибка записи %1.';ru='Ошибка записи %1.'");
		
		ПредставлениеОбъекта = "";
		Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(Объект.Ссылка)) Тогда
			ПредставлениеОбъекта = ПредставлениеОбъекта + НСтр("ka='ცნობარის ელემენტი';ru='элемента справочника'");
