...
"%2';ka='დროებითი ფაილ ""%1""-ის წაშლა ვერ მოხერხდა:"
"%2'"),
			ВременныйФайл,
...
Функция ПолучитьФайлИзИнтернета(Знач КлючСессии, Знач URL, Знач Порт = 0, Знач ИмяПользователя = "", Знач Пароль = "", Знач Таймаут = 20,Знач ЗащищенноеСоединение = Неопределено, Знач ПассивноеСоединение = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		ВызватьИсключение НСтр("ka='მოცემულ კონფიგურაციაში შემდეგი ფუნქციის მოხმობა არ არის მხარდაჭერილი დამატებითიანგარიშგებებიდადამუშავებებიუსაფრთხორეჟიმშიფაილისმიღებაინტერნეტიდან!';ru='В данной конфигурации не поддерживается вызов функции ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПолучитьФайлИзИнтернета!';en='This configuration is not supported by the call DopolnitelnyeOtchetyIObrabotkiVBezopasnomRezhime.PoluchitFaylIzInterneta function!'");
	КонецЕсли;
	...
	Если ВРег(Протокол) = "FTP" Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ka='შეცდომა მოხდა FTP-შეერთების შექმნისას სერვერთან %1:';ru='Ошибка при создании FTP-соединения с сервером %1:';en='Error creating FTP-connections to the server% 1:'") + Символы.ПС + "%2";
	...
	Иначе
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			СообщениеОбОшибке = НСтр("ka='შეცდომა მოხდა HTTP-შეერთების შექმნისას სერვერთან %1:';ru='Ошибка при создании HTTP-соединения с сервером %1:';en='Error while creating a HTTP-connection to the server% 1:'") + Символы.ПС + "%2";
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ka='შეცდომა ფაილის მიღებისას სერვერიდან %1:';ru='Ошибка при получении файла с сервера %1:';en='Error retrieving file from the server% 1:'") + Символы.ПС + "%2";
...
Функция ПередатьФайлВИнтернет(Знач КлючСессии, Знач АдресДвоичныхДанных, Знач URL, Знач Порт = 0, Знач ИмяПользователя = "", Знач Пароль = "", Знач Таймаут = 20,Знач ЗащищенноеСоединение = Неопределено, Знач ПассивноеСоединение = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		ВызватьИсключение НСтр("ka='მოცემულ კონფიგურაციაში არ არის მხარდაჭერილი შემდეგი ფუნქციის მოხმობა დამატებითიანგარიშგებებიდადამუშავებებიუსაფრთხორეჟიმში.ფაილისგადაცემაინტერნეტში!';ru='В данной конфигурации не поддерживается вызов функции ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПередатьФайлВИнтернет!';en='This configuration is not supported by the call DopolnitelnyeOtchetyIObrabotkiVBezopasnomRezhime.PeredatFaylVInternet function!'");
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ka='შეცდომა სერვერ %1-ზე ფაილის გადაცემისას:';ru='Ошибка при передаче файла на сервер %1:';en='Error when transferring files to the server% 1:'") + Символы.ПС + "%2";
...
Процедура ПроверитьКорректностьИмениМетодаВнешнегоОбъекта(Знач ИмяМетода)
	...
	Для Каждого ЗапрещенныйСимвол Из ЗапрещенныеСимволы Цикл
		...
		Если СтрНайти(ИмяМетода, ЗапрещенныйСимвол) > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='1% COM-ობიექტისთვის ან გარე კომპონენტის ობიექტისთვის მეთოდის კორექტულ სახელს არ წარმოადგენს!';ru='%1 не является корректным именем метода для COM-объекта или объекта внешней компоненты!';en='% 1 is not a valid method name for the COM-object or objects of the external components!'"), ИмяМетода);
...
"из сценария!';ka='საერთო მოდულის ფუნქციის არაკორექტული მოხმობა დამატებითიანგარიშგებებიდადამუშავებებიუსაფრთხორეჟიმში! მოცემული მოდულის ექსპორტირებადი ფუნქციების უსაფრთხო რეჟიმში გამოსაყენებლად მოხმობილი უნდა იქნას მხოლოდ სცენარიდან!'");
