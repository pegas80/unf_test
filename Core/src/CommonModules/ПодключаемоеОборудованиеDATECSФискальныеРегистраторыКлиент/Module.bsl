...
Функция ПечатьШтрихкода(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТипШтрихКода, ШтрихКод, ВыходныеПараметры) Экспорт
	...
	СтрокаТекста = НСтр("ka='შტრიხკოდი:';ru='ШТРИХКОД:';en='BARCODE:'") + ШтрихКод; 
	Результат = ОбъектДрайвера.НапечататьНефискСтроку(ПараметрыПодключения.ИДУстройства, СтрокаТекста);
	Если НЕ Результат Тогда
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
...
Функция ОткрытьДенежныйЯщик(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Исключение
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='ბრძანება ""ფულის უჯრის გახსნა""-ს მოცემული დრაივერის მიერ მხარდაჭერა არ გააჩნია.';ru='Команда ""Открыть денежный ящик"" не поддерживается данным драйвером.';en='The ""Open cash drawer"" is not supported by this driver.'"));
		Возврат Результат;
	КонецПопытки;
