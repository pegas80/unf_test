...
Функция ПроверитьКорректностьАдресаЭлектроннойПочты(Адрес) Экспорт
	...
	Для Каждого ПроверенныйАдрес Из РезультатПроверки Цикл
		Если Не ЗначениеЗаполнено(ПроверенныйАдрес.Адрес)
			...
			ТекстОшибки = НСтр("ka='მითითებულია ელექტრონული ფოსტის არაკორექტული მისამართი.';ru='Указан некорректный адрес электронной почты.';en='Set incorrect e-mail address.'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
