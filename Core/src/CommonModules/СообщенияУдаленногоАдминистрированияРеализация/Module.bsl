...
Процедура ПодготовитьОбластьДанных(Знач КодОбластиДанных, Знач ИзВыгрузки, Знач Вариант, Знач ИдентификаторВыгрузки) Экспорт
	...
	Попытка
		Если НЕ ЗначениеЗаполнено(Константы.РежимИспользованияИнформационнойБазы.Получить()) Тогда
			ТекстСообщения = НСтр("ka='საინფორმაციო ბაზის მუშაობის რეჟიმი დადგენილი არ არის';ru='Не установлен режим работы информационной базы';en='Do not set mode information base'");
		...
		Если МенеджерЗаписи.Выбран() Тогда
			Если МенеджерЗаписи.Статус = Перечисления.СтатусыОбластейДанных.Удалена Тогда
				ШаблонСообщения = НСтр("ka='მონაცემთა არეალი %1 წაშლილია';ru='Область данных %1 удалена';en='Data Area% 1 removed'");
			...
			ИначеЕсли МенеджерЗаписи.Статус = Перечисления.СтатусыОбластейДанных.КУдалению Тогда
				ШаблонСообщения = НСтр("ka='მონაცემთა არეალი %1 წაშლის პროცესში არის';ru='Область данных %1 в процессе удаления';en='Data Area% 1 during the removal'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КодОбластиДанных);
				ВызватьИсключение(ТекстСообщения);
			ИначеЕсли МенеджерЗаписи.Статус = Перечисления.СтатусыОбластейДанных.Новая Тогда
				ШаблонСообщения = НСтр("ka='მონაცემთა არეალი %1 გამოსაყენებლად მომზადების პროცესში იმყოფება';ru='Область данных %1 в процессе подготовки к использованию';en='Data Area% 1 in preparation for the use of'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КодОбластиДанных);
				ВызватьИсключение(ТекстСообщения);
			ИначеЕсли МенеджерЗаписи.Статус = Перечисления.СтатусыОбластейДанных.Используется Тогда
				ШаблонСообщения = НСтр("ka='მონაცემთა არეალი %1 გამოიყენება.';ru='Область данных %1 используется.';en='% Data area 1 is used.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КодОбластиДанных);
				ВызватьИсключение(ТекстСообщения);
			КонецЕсли;
		КонецЕсли;
...
Процедура УдалитьОбластьДанных(Знач КодОбластиДанных) Экспорт
	...
	Попытка
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			ШаблонСообщения = НСтр("ka='მონაცემთა არეალი %1 არ არსებობს.';ru='Область данных %1 не существует.';en='Area% data 1 does not exist.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КодОбластиДанных);
			ВызватьИсключение(ТекстСообщения);
		КонецЕсли;
	...
	Попытка
		...
		Для каждого КлючИЗначение Из Параметры Цикл
			...
			Если СтрокаПараметра = Неопределено Тогда
				ШаблонСообщения = НСтр("ka='პარამეტრ %1-ის უცნობი სახელი';ru='Не известное имя параметра %1';en='Not well-known name of the parameter% 1'");
			...
			ИначеЕсли СтрокаПараметра.ЗапретЗаписи Тогда
				ШаблонСообщения = НСтр("ka='პარამეტრი %1 შეიძლება გამოყენებული იქნას მხოლოდ წასაკითხად';ru='Параметр %1 может использоваться только для чтения';en='The% 1 can be used for read-only'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КлючИЗначение.Ключ);
				ВызватьИсключение(ТекстСообщения);
			КонецЕсли;
	...
	Попытка
		...
		Для каждого ОписаниеПользователя Из Параметры.UsersList.Item Цикл
			...
			ПользовательОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("ka='მონაცემთა არეალის ადმინისტრატორის შექმნა სერვისის მენეჯერიდან.';ru='Создание администратора области данных из менеджера сервиса.';en='Creating the data area of ​​the service manager administrator.'"));
...
Функция ЯзыкПоКоду(Знач КодЯзыка)
	...
	Если ЗначениеЗаполнено(КодЯзыка) Тогда
		...
		ШаблонСообщения = НСтр("ka='ენის არამხარდაჭერილი კოდი: %1';ru='Неподдерживаемый код языка: %1';en='Unsupported language code:% 1'");
...
"Ошибка: %2';ka='მითითებულია ელექტრონული ფოსტის არაკორექტული მისამართი: %1 შეცდომა: %2'");
...
Функция ПолучитьПользователяОбластиПоИдентификаторуПользователяСервиса(Знач ИдентификаторПользователяСервиса)
	...
	Если Результат.Пустой() Тогда
		ШаблонСообщения = НСтр("ka='მომხმარებელი სერვისის მომხმარებლის იდენტიფიკატორით ვერ მოიძებნა %1';ru='Не найден пользователь с идентификатором пользователя сервиса %1';en='Unable to find a user with the user ID service 1%'");
...
Функция ПолучитьПользователяИБПоПользователюОбластиДанных(Знач ПользовательОбластиДанных)
	...
	Если ПользовательИБ = Неопределено Тогда
		ШаблонСообщения = НСтр("ka='მონაცემთა არეალის მომხმარებლისთვის იდენტიფიკატორით %1 არ არსებობს საინფორმაციო ბაზის მომხმარებელი';ru='Для пользователя области данных с идентификатором %1 не существует пользователя информационной базы';en='For the user data area with the identifier% 1 does not exist user information database'");
	...
	Если ПолучитьЧасовойПоясИнформационнойБазы() <> ЧасовойПояс Тогда
		...
		Иначе
			...
			Исключение
				...
				ШаблонСообщения = НСтр("ka='სასაათო სარტყელ ""%1""-ის დასაყენებლად მონაცემთა არეალის დაბლოკვა ვერ მოხერხდა';ru='Не удалось заблокировать область данных для установки часового пояса ""%1""';en='It failed to block a section of data to set the time zone ""%1""'");
...
Функция СобытиеЖурналаРегистрацииУдаленноеАдминистрирование()
	...
	Возврат НСтр("ka='მოშორებული ადმინისტრირება სერვისის მოდელში. პარამეტრების დაყენება';ru='Удаленное администрирование в модели сервиса.Установить параметры';en='Remote administration in the model parameters servisa.Ustanovit'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
