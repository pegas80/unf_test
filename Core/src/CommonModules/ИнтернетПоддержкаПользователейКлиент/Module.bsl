...
Процедура ПодключитьИнтернетПоддержкуПользователей(
	...
	Если Не ИнтернетПоддержкаПользователейКлиентСервер.ДоступноПодключениеИнтернетПоддержки() Тогда
		...
		ПоказатьПредупреждение(
			ОповещениеАвторизацияНедоступна,
			НСтр("ka='ინტერნეტ-მხარდაჭერის შესაერთებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для подключения Интернет-поддержки.';en='Not have permission to connect online support.'"));
		Возврат;
...
"%1';ka='Не удалось подключить обработчик запроса настроек клиента лицензирования."
"%1'"),
...
Процедура СтартоватьМеханизмИзМеню(СтартовыеПараметры = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.МониторИнтернетПоддержки") Тогда
		...
		ВызватьИсключение НСтр("ka='ჩაშენების შეცდომა. ქვესისტემა ""ინტერნეტ-მხარდაჭერის მონიტორი"" არ არის.';ru='Ошибка встраивания. Отсутствует подсистема ""Монитор Интернет-поддержки"".';en='embedding error. No subsystem ""Monitor Internet support.""'");
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
		...
		ВызватьИсключение НСтр("ka='ჩაშენების შეცდომა. ქვესისტემა ""1C-ტაქსკომის შეერთება"" არ არის.';ru='Ошибка встраивания. Отсутствует подсистема ""Подключение 1С-Такском"".';en='embedding error. No subsystem ""Connecting 1C Taxcom"".'");
...
Процедура ОткрытьВебСтраницуСДополнительнымиПараметрами(
	...
	Если НеобходимаАвторизации
		...
		Тикет = "";
		Состояние(, , НСтр("ka='Переход на Портал 1С:ИТС';ru='Переход на Портал 1С:ИТС';en='Daylight portal 1C: ITS'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(
	...
	Если Получатель <> "webIts" И Получатель <> "taxcom" И Получатель <> "backup" Тогда
		ВызватьИсключение НСтр("ka='შეტყობინების უცნობი მიმღები.';ru='Неизвестный получатель сообщения.';en='Unknown recipient of the message.'");
	КонецЕсли;
	...
	Состояние(, , НСтр("ka='ტექნიკური მხარდაჭერის სამსახურში შეტყობინების მომზადება';ru='Подготовка сообщения в службу технической поддержки';en='Preparation posts Technical Support'"));
...
Процедура ВыполнитьСценарий(
	...
	Исключение
		...
		ПараметрыФормы.Вставить("ОписаниеОшибки",
			НСтр("ka='უცნობი შეცდომა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.';en='Unknown error. For details, see. In the log.'"));
...
Процедура ОбработкаКомандСервиса(
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки = НСтр("ka='არადამუშავებადი გამონაკლისი მოხდა:';ru='Произошло необрабатываемое исключение:';en='An unhandled exception occurred:'")
			+ " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если КонтекстОбработчика.ПроизошлаОшибка Тогда
		...
		ИначеЕсли КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение"
			...
			ПоказатьПредупреждение(,
				КонтекстОбработчика.ПользовательскоеОписаниеОшибки,
				,
				НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей';en='Internet-user support'"));
			Возврат;
	...
	ИначеЕсли КонтекстОбработчика.ВыполнитьОстановку Тогда
		...
		Если КонтекстОбработчика.Свойство("ОписаниеПричиныОстановки") Тогда
			ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა';ru='Интернет-поддержка пользователей';en='Internet-user support'"),
				,
	...
	Иначе
		...
		ОбщегоНазначенияКлиент.ПроверитьРасширениеРаботыСФайламиПодключено(
			Новый ОписаниеОповещения(
				...
				ЭтотОбъект,
				ПараметрыСообщения),
			НСтр("ka='გასაგრძელებლად აუცილებელია გაფართოების დაყენება ფაილებთან სამუშაოდ.';ru='Для продолжения необходимо установить расширение для работы с файлами.';en='To continue, you must install the extension for the files.'"));
...
Процедура ПриОтправкеДанныхСообщенияВТехПоддержку(РезультатОтправки, ПараметрыСообщения)
	...
	Если Результат Тогда
		...
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ЗаголовокОкна", НСтр("ka='შეტყობინების გაგზავნა ტექნიკური მხარდაჭერის სამსახურში';ru='Отправка сообщения в службу технической поддержки';en='Sending a message to the technical support service'"));
...
Функция ВидПриложения()
	...
	#Если ВебКлиент Тогда
	Возврат НСтр("ka='ვებ-კლიენტი';ru='Веб-клиент';en='Web Client'");
	#ИначеЕсли ТолстыйКлиентОбычноеПриложение Тогда
	Возврат НСтр("ka='მსხვილი კლიენტი, ჩვეულებრივი დანართი';ru='Толстый клиент, обычное приложение';en='Fat client, a regular application'");
	#ИначеЕсли ТолстыйКлиентУправляемоеПриложение Тогда
	Возврат НСтр("ka='მსხვილი კლიენტი, მართვადი დანართი';ru='Толстый клиент, управляемое приложение';en='Thick Client, managed application'");
	#ИначеЕсли ТонкийКлиент Тогда
	Возврат НСтр("ka='თხელი კლიენტი';ru='Тонкий клиент';en='thin client'");
	#Иначе
	Возврат "";
	#КонецЕсли
...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვისზე შეერთებისას.';ru='Ошибка при подключении к сервису Интернет-поддержки.';en='Error connecting to the service online support.'");
		КонецЕсли;
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("СоздатьЗаписьВЖурналеРегистрации");
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки = НСтр("ka='შეცდომა ინტერნეტ-მხარდაჭერის სერვერიდან მონაცემთა მიღებისას.';ru='Ошибка при получении данных с сервера Интернет-поддержки.';en='Error retrieving information from the Internet Information Server Support.'");
		КонецЕсли;
...
Функция СформироватьПараметрыОткрытияФормы(КСКонтекст, ИмяОткрываемойФормы)
	...
	Если ИмяОткрываемойФормы = "ОбщаяФорма.ПодключениеИнтернетПоддержки" Тогда
		...
		ПараметрыНовойФормы.Вставить("ЗаголовокКнопкиОК", НСтр("ka='შესვლა';ru='Войти';en='To come in'"));
...
Процедура ОстановитьМеханизм(КонтекстВзаимодействия, СтруктураКоманды)
	...
	Если СокрЛП(Строка(КодОшибки)) <> "0" И КодОшибки <> Неопределено Тогда
		...
		ПараметрыФормы.Вставить("ПодробноеОписаниеОшибки",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვისის შეცდომა (%1).';ru='Ошибка сервиса (%1).';en='service error (% 1).'"),
				Строка(КодОшибки)));
...
Процедура ОтобразитьСостояниеОбращениеКСервису()
	...
	Состояние(
		,
		,
		НСтр("ka='პასუხის მოლოდინი მომხმარებელთა ინტერნეტ-მხარდაჭერის სერვერიდან';ru='Ожидание ответа от сервера Интернет-поддержки пользователей';en='Waiting for a response from the server support Internet users'"),
		БиблиотекаКартинок.ИнтернетПоддержкаПользователей);
...
"Продолжить?';ka='ლოგ-ინ %1-ის მქონე მომხმარებლის შეერთება ინტერნეტ-მხარდაჭერაზე შეწყდება. ახალი შეერთებისთვის, საჭიროა ლოგ-ინის და პაროლის ხელახლა შეყვანა."
"გავაგრძელოთ?'");
