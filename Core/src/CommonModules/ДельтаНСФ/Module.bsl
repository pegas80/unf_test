...
Функция DoSendReceive(SoapAction, SoapArgs, Отказ = Ложь, ВремяОжидания = 30) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ka='ინფორმაციის გადაცემის შეცდომა! შეამოწმეთ აწყობის პარამეტრების შევსება.';ru='Ошибка передачи информации! Проверьте правильность заполнения настроек.'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат Ложь;
	КонецПопытки;	
	Если Не res Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='კავშირის შეცდომა!';ru='Ошибка связи!'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат Ложь;
	...
	Если  posstr1<1 Или posstr2<1 Или posstr1 +  lenstr1 = posstr2 Тогда
		//Сообщение = Новый СообщениеПользователю;
		//Сообщение.Текст = SoapAction + НСтр("ka=': მიღებულია მცდარი კონტენტი.';ru=': Получен Ошибочный Контент.'");
		//Сообщение.Сообщить();
		Отказ = Истина;
...
Функция ПроверкиПередСоеденением(Организация = Неопределено, su, sp, ПроверкаНаДекларирование = Ложь) Экспорт
	...
	Если ПроверкаНаДекларирование Тогда
		...
		Если Не (СервисЭлектронныхСФ Или Декларирование) Тогда
			СтрокаСообщения = НСтр("ka='ელექტრონული ანგარიშ-ფაქტურებისა და დეკლარირების სერვისი გამორთულია';ru='Настройки сервис электронных счет-фактур и декларирования выключены'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
			Возврат Ложь;
		КонецЕсли;
	Иначе
		Если не СервисЭлектронныхСФ Тогда
			СтрокаСообщения = НСтр("ka='ელექტრონული ანგარიშ-ფაქტურების სერვისი გამორთულია';ru='Сервис электронных счет-фактур выключен'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если не ЗначениеЗаполнено(ОсновнойПользовательНалоговогоСервера) Тогда
		СтрокаСообщения = НСтр("ka='არ არის შევსებული მომხმარებლის ""საგადასახადო სერვერის ძირითადი მომხმარებელი""';ru='В настройках пользователя не заполнен ""Основной пользователь налогового сервера""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
	
	Если не ОсновнойПользовательНалоговогоСервера.Активирован Тогда
		СтрокаСообщения = НСтр("ka='საგადასახადო სერვერის მომხმარებელი არ არის გააქტიურებული';ru='Пользователь налогового сервера не активирован'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;

	Если не ЗначениеЗаполнено(ОсновнойПользовательНалоговогоСервера.Пароль) Тогда
		СтрокаСообщения = НСтр("ka='საგადასახადო სერვერის მომხმარებელის პაროლი არ არის შევსებული';ru='Пароль пользователя налогового сервера не заполнен'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
	
	Если не ЗначениеЗаполнено(ОсновнойПользовательНалоговогоСервера.ИП) Тогда
		СтрокаСообщения = НСтр("ka='საგადასახადო სერვერის მომხმარებელის IP არ არის შევსებული';ru='IP пользователя налогового сервера не заполнен'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;

	Если Организация <> Неопределено и СписокОрганизаций.НайтиПоЗначению(ОсновнаяОрганизация) = Неопределено Тогда
		СтрокаСообщения = НСтр("ka='საგადასახადო სერვერის ძირითადი მომხმარებელის ორგანიზაცია არ შეესაბამება ორგანიზაციას';ru='Организация основного пользователя налогового сервера не совпадает с организацией'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
	
	Если не ЗначениеЗаполнено(ОсновнаяОрганизация.ПользовательСинхронизацииСФ) Тогда
		СтрокаСообщения = НСтр("ka='ორგანიზაციაში არ არის შევსებული ""საგადასახადო სერვერის მომხმარებელი""';ru='У организации не заполнен ""Пользователь налогового сервера""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
		
	Если не ЗначениеЗаполнено(ОсновнаяОрганизация.ПарольСинхронизацииСФ) Тогда
		СтрокаСообщения = НСтр("ka='ორგანიზაციაში არ არის შევსებული ""საგადასახადო სერვერის პაროლი""';ru='У организации не заполнен ""Пароль налогового сервера""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
	
    Если не ЗначениеЗаполнено(ОсновнаяОрганизация.ИНН) Тогда
		СтрокаСообщения = НСтр("ka='ორგანიზაციის საიდენტიფიკაციო კოდი არ არის შევსებული';ru='ИНН организации не заполнен'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверкаСтатусаСФ(user_id, su, sp, invois_id, k_id = Неопределено, dec_status = Неопределено, seq_num_s = Неопределено, Первичная = Ложь) Экспорт
	...
	Если ДельтаНСФ.get_invoice(user_id, invois_id, su, sp, f_series, f_number, operation_dt, reg_dt, seller_un_id, buyer_un_id, overhead_no, overhead_dt, status, seq_num_s, seq_num_b, k_id, r_un_id, k_type, b_s_user_id, dec_status) <> "true" Тогда
		Если Первичная Тогда
			СтрокаПервичная = НСтр("ka='პირველადი ანგარიშ-ფაქტურა';ru='Первичная счет-фактура'");
		Иначе
			СтрокаПервичная = НСтр("ka='ანგარიშ-ფაქტურა';ru='Счет-фактура'");
		КонецЕсли;
...
Функция ПолучитьТипКорректировоки(Тип) Экспорт
	...
	Если Тип = "1" Тогда
		Возврат Перечисления.СтатусыКорректировочнойСчетФактуры.ОтмененаОперация;
	ИначеЕсли Тип = "2" Тогда
		Возврат Перечисления.СтатусыКорректировочнойСчетФактуры.ИзмененВидОперации;
	ИначеЕсли Тип = "3" Тогда
		Возврат Перечисления.СтатусыКорректировочнойСчетФактуры.ИзмененаСуммаОперации;
	ИначеЕсли Тип = "4" Тогда
		Возврат Перечисления.СтатусыКорректировочнойСчетФактуры.ВозвратТовара;
	Иначе
		Возврат Перечисления.СтатусыКорректировочнойСчетФактуры.ПустаяСсылка();
	КонецЕсли;
...
Функция ПроверкаНаСоответствиеКорректировки(НомерОснования, Корректировочная, СтрокаСообщения = Неопределено) Экспорт
	...
	Если (НомерОснования = "-1" и Корректировочная) или (НомерОснования <> "-1" и не Корректировочная) Тогда
		Если СтрокаСообщения  = Неопределено Тогда
			СтрокаСообщения = НСтр("ka='ანგარიშ-ფაქტურის და ელექტრონული ანგარიშ-ფაქტურის კორექტირების სტატუსი არ შეესაბამება ერთმანეთს';ru='Статусы корректировок счет фактуры и электронной счет-фактуры не совпадают'");
		КонецЕсли;	
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;	
	Иначе
...
Функция ПолучениеТипаКорректировки(СтатусКорректировочнойСчетФактуры) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтатусКорректировочнойСчетФактуры) Тогда
		СтрокаСообщения = НСтр("ka='არ არის შევსებული ""კორექტირების ანგარიშ-ფაქტურის სტატუსი""';ru='Не заполнен ""Статус корректировочной счет-фактуры""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	ИначеЕсли СтатусКорректировочнойСчетФактуры = Перечисления.СтатусыКорректировочнойСчетФактуры.ОтмененаОперация Тогда
		Возврат "1";
	ИначеЕсли СтатусКорректировочнойСчетФактуры = Перечисления.СтатусыКорректировочнойСчетФактуры.ИзмененВидОперации Тогда
		Возврат "2";
	ИначеЕсли СтатусКорректировочнойСчетФактуры = Перечисления.СтатусыКорректировочнойСчетФактуры.ИзмененаСуммаОперации Тогда
		Возврат "3";
	ИначеЕсли СтатусКорректировочнойСчетФактуры = Перечисления.СтатусыКорректировочнойСчетФактуры.ВозвратТовара Тогда
		Возврат "4";
	КонецЕсли;		
...
Функция ПолученияСтатусаКорректировки(user_id, su, sp, invois_id, НомерСтроки = Неопределено) Экспорт
	...
	Если ДельтаНСФ.get_invoice(user_id, invois_id, su, sp, f_series, f_number, operation_dt, reg_dt, seller_un_id, buyer_un_id, overhead_no, overhead_dt, status, seq_num_s, seq_num_b, k_id, r_un_id, k_type, b_s_user_id, dec_status) <> "true" Тогда
		СтрокаСообщения = ?(НомерСтроки = Неопределено, "", НСтр("ru = 'В строке номер "+НомерСтроки+": '; ka = 'სტრიქონში ნომერ "+НомерСтроки+": '")) + НСтр("ka='შეცდომა ანგარიშ-ფაქტურის მიღებისას';ru='Ошибка при получении счет-фактуры'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
        Возврат Ложь;
	КонецЕсли;
...
Функция ПолучениеДанныхССервераИПечать(ТабДок, Организация, НомОнлНСФ, Корректировочная = Неопределено, СтрокаСНомеромСтроки = "",ИзМоейКоманди=ложь) Экспорт
	...
	Если не ЗначениеЗаполнено(invois_id) Тогда
		СтрокаСообщения = СтрокаСНомеромСтроки + НСтр("ka='არ არის შევსებული ""ონლაინ დოკუმენტის ID""';ru='Не заполнен ""ID онлайн счет-фактуры""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);
		Возврат Ложь;
	КонецЕсли;
	...
	Если chek(su, sp, user_id, sua) <> "true" Тогда
		СтрокаСообщения = СтрокаСНомеромСтроки + НСтр("ka='საგადასახადო სერვერთან ავტორიზაციის შეცდომა';ru='Ошибка авторизации с сервером налоговой службы'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения,,,,);	
		Возврат Ложь; 
	Конецесли;
	...
	Если ИзМоейКоманди Тогда
		ОбластьШапки.Параметры.МесяцОп = Формат(ДельтаНСФ.ПереводВДату(operation_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='MMMM'"));
        ОбластьШапки.Параметры.ГодОп = Формат(ДельтаНСФ.ПереводВДату(operation_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='yyyy'"));
		ОбластьШапки.Параметры.МесяцРег = Формат(ДельтаНСФ.ПереводВДату(reg_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='MMMM'"));
        ОбластьШапки.Параметры.ГодРег = Формат(ДельтаНСФ.ПереводВДату(reg_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='yyyy'"));
        ОбластьШапки.Параметры.Число = Формат(ДельтаНСФ.ПереводВДату(reg_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='dd'"));
		ОбластьШапки.Параметры.Статус = ПолучитьСтатус(status);

	Иначе
		ОбластьШапки.Параметры.Месяц = Формат(ДельтаНСФ.ПереводВДату(operation_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='MMMM yyyy'"));
		ОбластьШапки.Параметры.Дата = Формат(ДельтаНСФ.ПереводВДату(reg_dt), (НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ='dd MMMM yyyy'"));
