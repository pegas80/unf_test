...
Процедура СообщитьОбОшибке(Знач ТекстСообщения, Отказ, Заголовок = "", Знач Статус = Неопределено, ВызыватьИсключение = Истина) Экспорт
	...
	#Если Сервер Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='რეგლამენტირებული ანგარიშგებობა';ru='Регламентированная отчетность'"), УровеньЖурналаРегистрации.Ошибка, , , Заголовок + Символы.ПС + ТекстСообщения);
