...
Процедура УстановитьБлокировкуСеансовОбластиДанных(Параметры, Знач ПоМестномуВремени = Истина, Знач ОбластьДанных = -1) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ИначеЕсли ОбластьДанных <> ОбщегоНазначения.ЗначениеРазделителяСеанса() Тогда
			ВызватьИсключение НСтр("ka='გამყოფების გამოყენებადი მნიშვნელობების სეანსიდან მონაცემთა იმ არეალის სეანსების ბლოკირების დაყენება არ შეიძლება, რომელიც სეანსში გამოყენებულისგან განსხვავდება!';ru='Из сеанса с используемыми значениями разделителей нельзя установить блокировку сеансов области данных, отличной от используемой в сеансе!';en='From the session with the values ​​of separators can not be used to set the lock session data area different from that used in session!'");
		КонецЕсли;
		
	Иначе
		
		Если ОбластьДанных = -1 Тогда
			ВызватьИсключение НСтр("ka='მონაცემთა არეალის სეანსებზე ბლოკირების დაყენება შეუძლებელია - მონაცემთა არეალი არ არის მითითებული!';ru='Невозможно установить блокировку сеансов области данных - не указана область данных!';en='It is impossible to install a lock session data field - not specified area of ​​data!'");
		КонецЕсли;
...
Функция СообщениеОНеотключенныхСеансах() Экспорт
	...
	Сообщение = НСтр("ka='სეანსების გამორთვა ვერ მოხერხდა:';ru='Не удалось отключить сеансы:';en='Unable to disable sessions:'");
...
Функция ИнформацияОБлокирующихСеансах(ТекстСообщения = "") Экспорт
	...
	Если ИмеютсяБлокирующиеСеансы Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='არის პროგრამასთან მუშაობის აქტიური სეანსები, რომლებიც არ შეიძლება, რომ იძულებით დასრულდეს:"
"%1"
"%2';ru='Имеются активные сеансы работы с программой,"
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущийРежим.Конец) Тогда
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-დან %2-ის ჩათვლით პერიოდისთვის';ru='на период с %1 по %2';en='% for the period from 1 to 2%'"), ТекущийРежим.Начало, ТекущийРежим.Конец);
	Иначе
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='1%-დან';ru='с %1';en='with 1%'"), ТекущийРежим.Начало);
	КонецЕсли;
	Если ЗначениеЗаполнено(ТекущийРежим.Сообщение) Тогда
		ПричинаБлокировки = НСтр("ka='მიზეზი:';ru='по причине:';en='because of:'") + Символы.ПС + ТекущийРежим.Сообщение;
	Иначе
		ПричинаБлокировки = НСтр("ka='სარეგლამენტო სამუშაოების ჩასატარებლად';ru='для проведения регламентных работ';en='for scheduled maintenance'");
	КонецЕсли;
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დანართის ადმინისტრატორის მიერ დაყენებულია მომხმარებლების მუშაობის ბლოკირება %1 %2."
""
"დანართი დროებით მიუწვდომელია.';ru='Администратором приложения установлена блокировка работы пользователей %1 %2."
""
"Приложение временно недоступно.'"), ПериодБлокировки, ПричинаБлокировки);
	...
	Если Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დანართის ადმინისტრატორის მიერ დაყენებულია მომხმარებელთა მუშაობის ბლოკირება %1 %2."
""
"შევიდეთ დაბლოკილ დანართში?';ru='Администратором приложения установлена блокировка работы пользователей %1 %2."
""
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ПараметрыБлокировки.Установлена Тогда
		Если ТекущаяДатаСеанса < ПараметрыБлокировки.Начало Тогда
			Если ПараметрыБлокировки.Конец <> Дата(1, 1, 1) Тогда
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაგეგმილია %1-დან %2-ის ჩათვლით';ru='Запланирована с %1 по %2';en='Planned% 1 to% 2'"),
					Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"), Формат(ПараметрыБлокировки.Конец, "ДЛФ=DT"));
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაგეგმილია %1-ით';ru='Запланирована с %1';en='Scheduled from 1%'"), Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"));
			КонецЕсли;
			Важность = Ложь;
		ИначеЕсли ПараметрыБлокировки.Конец <> Дата(1, 1, 1) И ТекущаяДатаСеанса > ПараметрыБлокировки.Конец И ПараметрыБлокировки.Начало <> Дата(1, 1, 1) Тогда
			Важность = Ложь;
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='არ მოქმედებს (ვადა ამოიწურა %1)';ru='Не действует (истек срок %1)';en='Not Applicable (expired% 1)'"), Формат(ПараметрыБлокировки.Конец, "ДЛФ=DT"));
		Иначе
			Если ПараметрыБлокировки.Конец <> Дата(1, 1, 1) Тогда
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='с %1 по %2';ru='с %1 по %2';en='% 1% 2'"),
					Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"), Формат(ПараметрыБлокировки.Конец, "ДЛФ=DT"));
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='1%-დან';ru='с %1';en='with 1%'"), 
					Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"));
			КонецЕсли;
			Важность = Истина;
		КонецЕсли;
	Иначе
		Сообщение = НСтр("ka='არ მოქმედებს';ru='Не действует';en='It does not work'");
		Важность = Ложь;
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='მომხმარებელთა მუშაობის ბლოკირება';ru='Блокировка работы пользователей';en='Blocking user experience'");
...
Функция СформироватьСообщениеБлокировки(Знач Сообщение, Знач КодРазрешения) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='%1"
"მომხმარებლებისთვის მუშაობის ნებართვის მისაცემად, შეიძლება დანართის გახსნა პარამეტრით მომხმარებელთამუშაობისნებართვა. მაგალითად:"
"http://<სერვერის ვებ-მისამართი>/?C=მომხმარებელთამუშაობისნებართვა';ru='%1"
...
"http://<веб-адрес сервера>/?C=РазрешитьРаботуПользователей'");
	...
	Иначе
		ТекстСообщения = ТекстСообщения + НСтр("ka='%1"
"მომხმარებელთა მუშაობის ნებართვისთვის, ისარგებლეთ სერვერების კლასტერის კონსოლით ან ჩართეთ ""1С:საწარმო"" შემდეგი პარამეტრებით:"
"ENTERPRISE %2 /მომხმარებელთამუშაობისნებართვა /UC%3';ru='%1"
...
"ENTERPRISE %2 /CРазрешитьРаботуПользователей /UC%3'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СоединенияИБКлиентСервер.ТекстДляАдминистратора(),
			СтрокаПутиКИнформационнойБазе, НСтр("ka='<დაშვების კოდი>';ru='<код разрешения>';en='<Authorization code>'"));
	КонецЕсли;
