...
Процедура ВыбратьФайл1СБухфон(ОповещениеОЗакрытии, ПутьКФайлу = "") Экспорт
	...
	ТекстПредложения = НСтр("ka='Для выбора приложения необходимо установить расширение работы с файлами.';ru='Для выбора приложения необходимо установить расширение работы с файлами.';en='To select the applications you need to install the extension to work with files.'");
...
Процедура ВыбратьФайл1СБухфонПослеУстановкиРасширения(Подключено, ДополнительныеПараметры) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='Выберите исполняемый файл приложения';ru='Выберите исполняемый файл приложения';en=""Select the application's executable file""");
	...
	Фильтр = НСтр("ka='Файл приложения (*.exe)|*.exe';ru='Файл приложения (*.exe)|*.exe';en='application file (* .exe) | * .exe'");
...
Процедура Запустить1СБухфон() Экспорт
	...
	ТекстСообщения = НСтр("ka='Для запуска приложения необходимо установить расширение работы с файлами.';ru='Для запуска приложения необходимо установить расширение работы с файлами.';en='To run the application, you must install the extension to work with files.'");
