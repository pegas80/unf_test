...
Процедура ВыбратьФайл1СБухфон(ОповещениеОЗакрытии, ПутьКФайлу = "") Экспорт
	...
	ТекстПредложения = НСтр("ka='Для выбора приложения необходимо установить расширение работы с файлами.';ru='Для выбора приложения необходимо установить расширение работы с файлами.'");
...
Процедура ВыбратьФайл1СБухфонПослеУстановкиРасширения(Подключено, ДополнительныеПараметры) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='Выберите исполняемый файл приложения';ru='Выберите исполняемый файл приложения'");
	...
	Фильтр = НСтр("ka='Файл приложения (*.exe)|*.exe';ru='Файл приложения (*.exe)|*.exe'");
...
Процедура Запустить1СБухфон() Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ПоказатьПредупреждение(,НСтр("ka='Для работы с приложением необходима операционная система Microsoft Windows.';ru='Для работы с приложением необходима операционная система Microsoft Windows.'"));
		Возврат
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("Запустить1СБухфонПослеУстановкиРасширения", ЭтотОбъект);
	ТекстСообщения = НСтр("ka='Для запуска приложения необходимо установить расширение работы с файлами.';ru='Для запуска приложения необходимо установить расширение работы с файлами.'");
