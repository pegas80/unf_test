...
"Обратитесь к администратору.';ka='ბანკთან პირდაპირი გაცვლის გამართვისთვის არასაკმარისი უფლებებია."
...
Процедура ПолучитьВыпискуБанка(НастройкаОбмена, ДатаНачала, Знач ДатаОкончания, Владелец, НомерСчета = Неопределено, ОткрыватьФормуУточненияПериода = Ложь) Экспорт
	...
	Если ДатаНачала > ТекущаяДатаСеанса Тогда
		СообщениеТекст = НСтр("ka='Дата начала периода запроса не должна быть позднее текущей даты.';ru='Дата начала периода запроса не должна быть позднее текущей даты.';en='Start date of the request period should not be later than the current date.'");
	...
	Если ДатаОкончания > КонецДня(ТекущаяДатаСеанса) Тогда
		СообщениеТекст = НСтр("ka='Дата конца периода запроса не должна быть позднее текущей даты.';ru='Дата конца периода запроса не должна быть позднее текущей даты.';en='the end of the inquiry period, the date should not be later than the current date.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "КонецПериода");
		Возврат;
	КонецЕсли;
	
	Если ДатаНачала > ДатаОкончания Тогда
		СообщениеТекст = НСтр("ka='Дата начала периода запроса не должна быть больше даты конца периода.';ru='Дата начала периода запроса не должна быть больше даты конца периода.';en='Start date of the request period should not be more than the end date of the period.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "НачалоПериода");
		Возврат;
	КонецЕсли;
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ТекстСообщения = НСтр("ka='გაცვლის გამართვა უმოქმედოა, ოპერაცია შეუძლებელია';ru='Настройка обмена недействительна, операция невозможна';en='sharing setting is ineffective, surgery is not possible'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
		...
		Если МинимальнаяДата > ДатаНачала Тогда
			Если ОткрыватьФормуУточненияПериода Тогда
				...
				Возврат;
			Иначе
				СообщениеТекст = НСтр("ka='Период запроса выписки не должен превышать 14 дней.';ru='Период запроса выписки не должен превышать 14 дней.';en='discharge request period should not exceed 14 days.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "КонецПериода");
				Возврат;
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли ГотовыеВыписки.Количество() Тогда
			...
			ТекстВопроса = НСтр("ka='ბაზაში უკვე არის აღნიშნული პერიოდისთვის ბანკის ამონაწერები. ამონაწერები ჩავტვირთოთ ბაზიდან თუ ახლები მივიღოთ ბანკიდან?';ru='В базе уже есть выписки банка за указанный период."
"Загрузить выписки из базы или получить новые из банка?'");
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(Истина, НСтр("ka='ბაზიდან ჩატვირთვა';ru='Загрузить из базы';en='Download from the database'"));
			Кнопки.Добавить(Ложь, НСтр("ka='Получить из банка';ru='Получить из банка';en='Get out of the bank'"));
			Заголовок = НСтр("ka='ამონაწერის მიღება';ru='Получение выписки';en='obtaining statements'");
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ka='%1. Актуальный электронный документ не найден.';ru='%1. Актуальный электронный документ не найден.';en='%one. Date the electronic document was not found.'");
...
Процедура ПроверитьУстановкуПодписиСбербанк(Оповещение, Сертификат, Контекст)
	...
	Если Не ЗначениеЗаполнено(НастройкаОбмена) Тогда
		ОписаниеОшибки = НСтр("ka='Не найдена действующая настройка обмена с сервисом 1С:ДиректБанка с данным сертификатом';ru='Не найдена действующая настройка обмена с сервисом 1С:ДиректБанка с данным сертификатом';en='Found operating exchange configuration with the service of 1C: DirektBanka with this certificate'");
