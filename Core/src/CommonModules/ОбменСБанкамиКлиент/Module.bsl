...
Процедура ПолучитьВыпискуБанка(НастройкаОбмена, ДатаНачала, Знач ДатаОкончания, Владелец, НомерСчета = Неопределено, ОткрыватьФормуУточненияПериода = Ложь) Экспорт
	...
	Если ДатаНачала > ТекущаяДатаСеанса Тогда
		СообщениеТекст = НСтр("ka='Дата начала периода запроса не должна быть позднее текущей даты.';ru='Дата начала периода запроса не должна быть позднее текущей даты.'");
	...
	Если ДатаОкончания > КонецДня(ТекущаяДатаСеанса) Тогда
		СообщениеТекст = НСтр("ka='Дата конца периода запроса не должна быть позднее текущей даты.';ru='Дата конца периода запроса не должна быть позднее текущей даты.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "КонецПериода");
		Возврат;
	КонецЕсли;
	
	Если ДатаНачала > ДатаОкончания Тогда
		СообщениеТекст = НСтр("ka='Дата начала периода запроса не должна быть больше даты конца периода.';ru='Дата начала периода запроса не должна быть больше даты конца периода.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "НачалоПериода");
		Возврат;
	КонецЕсли;
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ТекстСообщения = НСтр("ka='გაცვლის გამართვა უმოქმედოა, ოპერაცია შეუძლებელია';ru='Настройка обмена недействительна, операция невозможна'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
		...
		Если МинимальнаяДата > ДатаНачала Тогда
			Если ОткрыватьФормуУточненияПериода Тогда
				...
				Возврат;
			Иначе
				СообщениеТекст = НСтр("ka='Период запроса выписки не должен превышать 14 дней.';ru='Период запроса выписки не должен превышать 14 дней.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "КонецПериода");
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
"Загрузить выписки из базы или получить новые из банка?'");
			...
			Кнопки.Добавить(Истина, НСтр("ka='ბაზიდან ჩატვირთვა';ru='Загрузить из базы'"));
			Кнопки.Добавить(Ложь, НСтр("ka='Получить из банка';ru='Получить из банка'"));
			Заголовок = НСтр("ka='ამონაწერის მიღება';ru='Получение выписки'");
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ka='%1. Актуальный электронный документ не найден.';ru='%1. Актуальный электронный документ не найден.'");
...
Процедура ПроверитьУстановкуПодписиСбербанк(Оповещение, Сертификат, Контекст)
	...
	Если Не ЗначениеЗаполнено(НастройкаОбмена) Тогда
		ОписаниеОшибки = НСтр("ka='Не найдена действующая настройка обмена с сервисом 1С:ДиректБанка с данным сертификатом';ru='Не найдена действующая настройка обмена с сервисом 1С:ДиректБанка с данным сертификатом'");
