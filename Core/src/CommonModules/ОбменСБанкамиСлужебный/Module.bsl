...
Процедура ПодключитьВнешнююОбработку(Знач АдресФайла, ИмяОбъекта) Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ka='გარე დამუშავების შეერთება';ru='Подключение внешней обработки'");
...
Функция ПараметрыВК(ПутьКФайлуИнформации) Экспорт
	...
	Исключение
		Операция = НСтр("ka='გარე კომპონენტის შესახებ ინფორმაციის ფაილის წაკითხვა.';ru='Чтение файла информации о внешней компоненте.'");
		ТекстСообщения = НСтр("ka='გარე კომპონენტის შესახებ ინფორმაციის წაკითხვისას შეცდომა მოხდა.';ru='При чтении информации о внешней компоненте произошла ошибка.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ИнформацияОВКБанка(АдресВнешнейКомпоненты) Экспорт
	...
	Исключение
		Операция = НСтр("ka='გარე მოდულის ფაილის წაკითხვა.';ru='Чтение файла внешнего модуля.'");
		ТекстСообщения = НСтр("ka='გარე მოდულის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла внешнего модуля произошла ошибка.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	...
	Если Не НайденаИнформация Тогда
		Операция = НСтр("ka='გარე მოდულის ფაილის შესახებ ინფორმაციის წაკითხვა';ru='Чтение информации о файле внешнего модуля'");
		...
		ТекстСообщения = НСтр("ka='აღნიშნული ფაილი გარე მოდულს არ წარმოადგენს';ru='Указанный файл не является внешним модулем'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	...
	Исключение
		Операция = НСтр("ka='გარე მოდულის ფაილის განფუთვა.';ru='Распаковка файла внешнего модуля.'");
		ТекстСообщения = НСтр("ka='При распаковки файла внешнего модуля произошла ошибка.';ru='При распаковки файла внешнего модуля произошла ошибка.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ВремКаталог);
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ВремФайл);
...
"Внешний идентификатор: %2'");
			...
			ВидОперации = НСтр("ka='Сохранение сообщения обмена в информационной базе.';ru='Сохранение сообщения обмена в информационной базе.'");
...
Процедура ОтправитьВБанк(Настройки, Данные, Результат, ТекстОшибки) Экспорт
	...
	Иначе
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			Шаблон = НСтр("ka='ბანკში დოკუმენტის გაგზავნისას შეცდომა მოხდა (%1). %2';ru='При отправке документа в банк произошла ошибка (%1). %2'");
...
Процедура ОтправитьЗапросВыпискиВБанк(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ВидОперации = НСтр("ka='ამონაწერის მოთხოვნის გაგზავნა ბანკში';ru='Отправка запроса выписки в банк'");
...
"%2'");
		...
		Иначе
			ТекстСообщения = НСтр("ka='ბანკიდან მიღებული ფაილის წაკითხვა შეუძლებელია, ვინაიდან იგი შეცდომას შეიცავს';ru='Файл, полученный из банка невозможно прочитать так как он содержит ошибку'");
		КонецЕсли;
...
"პაკეტის ფაილი: %3'");
		...
		ВидОперации = НСтр("ka='Чтение ЭД.';ru='Чтение ЭД.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"შეცდომიანი ფაილი: %3'");
					...
					Операция = НСтр("ka='პაკეტის განფუთვა';ru='Распаковка пакета'");
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"პაკეტის ფაილი: %3'");
				...
				Если ЗначениеЗаполнено(СообщениеЭДО) Тогда
					...
					НазваниеДокумента = НСтр("ka='უწყება ელექტრონული დოკუმენტის მდგომარეობის შესახებ';ru='Извещение о состоянии электронного документа'");
						...
						Иначе
							...
							Если StatusDocNotice.Result.Error.Code = "2207" Тогда
								...
								РеквизитыДляИзменения.Вставить(
									"ПричинаОтклонения", НСтр("ka='Документ отсутствует в информационной базе банка.';ru='Документ отсутствует в информационной базе банка.'"));
					...
					КонецЕсли;
				Иначе
					ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის მდგომარეობის შესახებ უწყების წაკითხვა';ru='Чтение извещения о состоянии электронного документа'");
					ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
...
"შეცდომიანი ფაილი: %3'");
				...
				Если ЗначениеЗаполнено(ИдентификаторЗапроса) Тогда
					...
					Если НЕ ЗначениеЗаполнено(СообщениеЗапрос) Тогда
						ВидОперации = НСтр("ka='პაკეტის განფუთვა';ru='Распаковка пакета'");
						ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
...
"ამონაწერის მოთხოვნა ვერ მოიძებნა იდენტიფიკატორის მიხედვით: %2'");
				...
				Если ЗначениеЗаполнено(СообщениеЗапрос) Тогда
					...
					НазваниеДокумента = СтрЗаменить(
						НазваниеЗапроса, НСтр("ka='ამონაწერის მოთხოვნა';ru='Запрос выписки'"), НСтр("ka='ბანკის ამონაწერი პერიოდის მანძილზე';ru='Выписка банка за период'"));
				Иначе
					НазваниеДокумента = НСтр("ka='%1-ის ბანკის ამონაწერი';ru='Выписка банка от %1'");
			...
			ИначеЕсли Document.dockind = "20" Тогда // Подтверждение открытия счетов по зарплатному проекту
				...
				НазваниеДокумента = НСтр("ka='სახელფასო პროექტის მიხედვით ანგარიშების გახსნის დადასტურება';ru='Подтверждение открытия счетов по зарплатному проекту'");
			...
			ИначеЕсли Document.dockind = "22" Тогда // Подтверждение зачисления денежных средств на счета сотрудников
				...
				НазваниеДокумента = НСтр("ka='თანამშრომლების ანგარიშებზე ფულადი სახსრების ჩარიცხვის დადასტურება';ru='Подтверждение зачисления денежных средств на счета сотрудников'");
...
Процедура СохранитьПолученныйПакет(НастройкаОбмена, ИмяФайла, ДанныеВозврата) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='ბანკიდან მიღებულია არაკორექტული მონაცემები. პრობლემის გადასაჭრელად მიმართეთ თქვენს ბანკს.';ru='Получены некорректные данные из банка. Для решения проблемы обратитесь в свой банк.'");
...
"Текст ошибки: %2.'");
		...
		ВидОперации = НСтр("ka='ბანკიდან მიღებული ელექტრონული დოკუმენტის წაკითხვა.';ru='Чтение полученного из банка электронного документа.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ГотовыеКРаспаковкеПакетыЭД(Знач ПакетыЭД) Экспорт
	...
	Пока ВыборкаПакеты.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(ВыборкаПакеты.Ссылка) Тогда
			Операция = НСтр("ka='პაკეტის განფუთვა';ru='Распаковка пакета'");
			ТекстСообщения = НСтр("ka='მიერთებული ფაილი არ არის პაკეტისთვის: %1';ru='Нет присоединенного файла для пакета: %1'");
		...
		Если ВыборкаПакеты.Статус <> Перечисления.СтатусыПакетовЭД.КРаспаковке Тогда
			...
			ШаблонСообщения = НСтр("ka='პაკეტ %1-ის სტატუსი განსხვავდება მნიშვნელობა ""განსაფუთად""-ისგან.';ru='Статус пакета %1 отличен от значения ""К распаковке"".'");
...
Процедура ПрочитатьИдентификаторыПакетов(НастройкаОбмена, ИмяФайла, МассивИдентификаторов, ЕстьОшибка) Экспорт
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='При чтении данных, полученных из банка, произошла ошибка.';ru='При чтении данных, полученных из банка, произошла ошибка.'");
...
"Подробное представление ошибки: %2'");
		...
		ВидОперации = НСтр("ka='ახალი დოკუმენტების სიის წაკითხვა';ru='Чтение списка новых документов'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
		ОбъектXML.Закрыть();
	...
	Если НЕ ResultBank.Success = Неопределено Тогда
		...
		Иначе
			Операция = НСтр("ka='ბანკის პასუხის წაკითხვა.';ru='Чтение ответа банка.'");
...
"პასუხის ფაილი: %1'");
			...
			ТекстСообщения = НСтр("ka='ბანკიდან მიღებულია არაკორექტული მონაცემები';ru='Получены некорректные данные из банка'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%1'");
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='დოკუმენტების მიღების უკანასკნელი თარიღის შენახვა';ru='Сохранение последней даты получения документов'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура СоздатьПакетОбменСБанками(НастройкаОбмена, МассивСообщений, ПакетОбменСБанками) Экспорт
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='ედბ-ის პაკეტის შედგენა';ru='формирование пакета ЭДО'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			...
			Для Каждого СтрокаПодписи Из МассивПодписей Цикл
				...
				Иначе
					...
					Если СвойстваСертификата = Неопределено Тогда
						ВызватьИсключение НСтр("ka='სერთიფიკატის თვისებების წაკითხვის შეცდომა';ru='Ошибка чтения свойств сертификата'");
					КонецЕсли;
...
Процедура ОбработатьОтветИзБанка(ОтветБанка, ОтправленноеСообщениеОбмена, ПолученноеСообщениеОбмена = Неопределено, ЕстьОшибка = Неопределено) Экспорт
	...
	Иначе
		...
		НаименованиеВыписки = СтрЗаменить(НаименованиеЗапроса, НСтр("ka='ამონაწერის მოთხოვნა';ru='Запрос выписки'"), НСтр("ka='ბანკის ამონაწერი პერიოდის მანძილზე';ru='Выписка банка за период'"));
...
"Необходимо проверить компьютер антивирусной программой.'");
		ПодробноеПредставлениеОшибки = ТекстСообщения + Символы.ПС + НСтр("ka='Путь к файлу: %1';ru='Путь к файлу: %1'");
		ПодробноеПредставлениеОшибки = СтрШаблон(ПодробноеПредставлениеОшибки, ИмяФайла);
		ВидОперации = НСтр("ka='Проверка неизменности данных временного файла.';ru='Проверка неизменности данных временного файла.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СформироватьСообщенияОбмена(МассивОбъектов, ПараметрыОбмена) Экспорт
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если (СтруктураОбмена.СтруктураЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение
			...
			ПараметрыСообщения = Новый Структура;
			ПараметрыСообщения.Вставить("Представление", НСтр("ka='მონაცემების სქემა';ru='Схема данных'"));
...
Процедура УстановитьСостояниеПриЗаписиОбъектаИБ(Источник, Отказ) Экспорт
	...
	Если НЕ Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ka='არსებობს აქტუალური ელექტრონული დოკუმენტი. დოკუმენტის საკვანძო რეკვიზიტების რედაქტირება აკრძალულია.';ru='Существует актуальный электронный документ. Запрещено редактирование ключевых реквизитов документа.'");
...
Функция НайтиСоздатьСертификатВнешнегоМодуля(ДанныеСертификата, Организация, ПрограммаБанка, Параметры) Экспорт
	...
	ШаблонНаименования = НСтр("ka='%1, %2-მდე';ru='%1, до %2'");
...
Процедура ЗагрузитьСписокDirectBank(ЗапускИзРегламентногоЗадания = Истина) Экспорт
	...
	Если Результат.Статус Тогда
		...
		Исключение
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='სერვის 1С:დირექტბანკით გაცვლის მხარდამჭერი ბანკების სიის ფაილის წაკითხვა.';ru='Чтение файла списка банков, поддерживающих обмен через сервис 1С:ДиректБанк.'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
"%2'");
	...
	ИначеЕсли ЗначениеЗаполнено(Результат.СообщениеОбОшибке) Тогда
		...
		ВидОперации = НСтр("ka='ბანკების სიის ჩატვირთვა, რომელთა მიერ მხარდაჭერილია გაცვლა DirectBank-ი';ru='Загрузка списка банков, поддерживающих обмен DirectBank'");
...
"%2'");
		...
		Шаблон = НСтр("ka='ელექტრონული ურთიერთქმედება.ბანკებთან გაცვლა';ru='Электронное взаимодействие.Обмен с банками'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СоздатьНастройкуОбмена(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='გაცვლის გამართვის შექმნის შეცდომა: %1';ru='Ошибка создания настройки обмена: %1'");
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='Создание настройки обмена';ru='Создание настройки обмена'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Обратитесь в свой банк.'");
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = НСтр("ka='Не найден банк по реквизитам: %1%2';ru='Не найден банк по реквизитам: %1%2'");
		Если СтруктураПоискаБанка.Свойство("Код") Тогда
			ТекстБИК = Символы.ПС + НСтр("ka='БИК: %1';ru='БИК: %1'");
...
"Версия: %3.'");
	...
	Если ЗаписыватьОбъект Тогда
		Попытка
			Объект.Записать();
		Исключение
			ТекстСообщения = НСтр("ka='Возникла ошибка при создании настройки обмена с сервисом 1С:ДиректБанк.';ru='Возникла ошибка при создании настройки обмена с сервисом 1С:ДиректБанк.'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='Создание настройки обмена с сервисом 1С:ДиректБанк.';ru='Создание настройки обмена с сервисом 1С:ДиректБанк.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Неопределено;
		КонецПопытки
...
Функция XDTOДанныеФайлаНастроек(АдресФайла) Экспорт
	...
	Попытка
		...
		ЭД.Проверить();
	Исключение
		ТекстСообщения = НСтр("ka='გამართვების წაკითხვისას შეცდომა წარმოიშვა.';ru='Возникла ошибка при чтении настроек.'");
...
Функция ОпределитьНастройкиОбменаЭДПоИсточнику(Источник, ВыводитьСообщения, МассивОтпечатковСертификатов, СообщениеОбмена = Неопределено, ВидЭД = Неопределено, ФлагДействующиеНастройкиОбмена = Истина) Экспорт
	...
	Если ВыводитьСообщения И ДействующиеПараметрыОбмена.НастройкаОбменаДействует
		...
		ТекстСообщения = НСтр("ka='Данный вид электронного документа банк не принимает.';ru='Данный вид электронного документа банк не принимает.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	Исключение
		...
		Попытка
			...
			Если ДанныеСертификата = Неопределено Тогда
				ВызватьИсключение НСтр("ka='Некорректное содержимое сертификата криптографии.';ru='Некорректное содержимое сертификата криптографии.'");
			КонецЕсли;
			ТекСертификат = Новый СертификатКриптографии(ДанныеСертификата);
		Исключение
			ТекстСообщения = НСтр("ka='ფაილიდან სერთიფიკატის მონაცემთა წაკითხვისას შეცდომა წარმოიშვა.';ru='Возникла ошибка при чтении данных сертификата из файла.'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='Чтение настроек ЭДО из файла.';ru='Чтение настроек ЭДО из файла.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Неопределено;
		КонецПопытки;
...
Функция ПрочитатьФайлПоСхеме(ИмяФайла, ДеревоРазбора, НовыйЭД, ПоказыватьОшибки = Ложь)
	...
	Исключение
		...
		Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) ИЛИ ПоказыватьОшибки Тогда
			ШаблонСообщения = НСтр("ka='ფაილ %1-დან მონაცემთა წაკითხვისას შეცდომა წარმოიშვა: %2.';ru='Возникла ошибка при чтении данных из файла %1: %2.'");
...
Процедура ПрочитатьЗапросВыпискиAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	ТекстБанк = " " + НСтр("ka='ბანკში';ru='в банке'") + " ";
	Если НЕ DataRequest.Bank.Name = Неопределено Тогда
		ТекстБанк = ТекстБанк + DataRequest.Bank.Name + ", ";
	КонецЕсли;
	ТекстБанк = ТекстБанк + НСтр("ka='ბანკის კოდი';ru='БИК:'") + " " + DataRequest.Bank.BIC;
...
Функция ПредставлениеЭД(ВидЭД, СсылкаНаОбъект, НомерДокумента)
	...
	Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение
		...
		ШаблонПредставленияЭД = НСтр("ka='%1 № %2  %3 -დან';ru='%1 № %2 от %3'");
	Иначе
		ШаблонПредставленияЭД = НСтр("ka='%1 %2 %3';ru='%1 %2 %3'");
	КонецЕсли;
...
Процедура СообщитьОбОтсутствииНастройкиОбмена(ПараметрыЭД, Источник)
	...
	Для Каждого СтрокаПараметров Из ТаблицаПараметров Цикл
		Если ЗначениеЗаполнено(СтрокаПараметров.Значение) Тогда
			Текст = Текст + Символы.ПС + НСтр("ka='<%1>: %2';ru='<%1>: %2'");
...
Функция СформироватьНовыйПакетОбменСБанками(СтруктураПараметров)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Операция = НСтр("ka='გაცვლის პაკეტის შექმნა';ru='Создание пакета обмена'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
