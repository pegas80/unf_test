...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Прототип.Представление = НСтр("ka='პროგრამის ოპტიმიზირება';ru='Оптимизировать программу';en='optimize program'");
	Прототип.Подсказка     = НСтр("ru='Ускорить проведение документов и формирование отчетов."
"Обязательная ежемесячная процедура, может занять некоторое время.';ka='დოკუმენტების გატარების და ანგარიშგებების შედგენის დაჩქარება."
"სავალდებულო ყოველთვიური პროცედურა, რომელსაც შეიძლება გარკვეული დრო დაჭირდეს.'");
...
Функция РасписаниеЗаданияПоУмолчанию(Знач Имя)
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დავალების უცნობი სახელი %1';ru='Неизвестное имя задания %1';en='Unknown job name% 1'"), Имя);
