...
Функция ПоместитьВХранилище(Ссылка, ИдентификаторФормы) Экспорт
	...
	Если НЕ ДополнительныеОтчетыИОбработки.ВозможнаВыгрузкаОбработкиВФайл(Ссылка) Тогда
		ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების ფაილების გადმოსატვირთად არასაკმარისი უფლებებია';ru='Недостаточно прав для выгрузки файлов дополнительных отчетов и обработок'");
	КонецЕсли;
...
Функция ЗапуститьДлительнуюОперацию(Знач УникальныйИдентификатор, Знач ПараметрыКоманды) Экспорт
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დამატებითი ანგარიშგების ან დამუშავების შესრულება ""%1"", ბრძანების სახელი ""%2""';ru='Выполнение дополнительного отчета или обработки ""%1"", имя команды ""%2""'"),
