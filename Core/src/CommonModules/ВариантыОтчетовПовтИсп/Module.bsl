...
Функция ПодсистемыТекущегоПользователя() Экспорт
	...
	КорневаяСтрока.Представление = НСтр("ka='ყველა ნაწილი';ru='Все разделы';en='All Forums'");
...
Процедура ДобавитьПодсистемыТекущегоПользователя(РодительСтрока, РодительМетаданные = Неопределено, РазделСсылка = Неопределено)
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		Если ЭтоНачальнаяСтраница Тогда
			...
			СтрокаДерева.Представление = НСтр("ka='საწყისი გვერდი';ru='Начальная страница';en='start page'");
		Иначе
		...
		Если ХранилищеЗначения = Неопределено Тогда
			Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
				...
				Если Не ПривилегированныйРежим() Тогда
					ВызватьИсключение НСтр("ka='გაფართოებების დამხმარე მონაცემთა განახლება ვერ მოხერხდა. მიმართეთ ადმინისტრატორს.';ru='Не удалось обновить вспомогательные данные расширений. Обратитесь к администратору.';en='Failed to update the assistance data extensions. Contact your administrator.'");
				КонецЕсли;
			КонецЕсли;
