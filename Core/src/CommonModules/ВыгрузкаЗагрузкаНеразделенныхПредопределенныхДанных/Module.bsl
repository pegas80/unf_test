		...
		Если ПараметрыВыгрузки.ВыгружаемыеТипы.Найти(ОбъектМетаданных) <> Неопределено Тогда
			...
			Для Каждого ИмяПоля Из ПравилоКонтроля.Значение Цикл
				...
				Иначе
					...
					ВызватьИсключение СтрШаблон(НСтр("ka='Объект метаданных %1 не поддерживается!';ru='Объект метаданных %1 не поддерживается!';en='Metadata object% 1 is not supported!'"), ОбъектМетаданных.ПолноеИмя());
...
Процедура ВызватьИсключениеНеУдалосьОпределитьПоле(Знач ИмяПоля, Знач ИмяОбъекта)
	...
	ВызватьИсключение СтрШаблон(НСтр("ka='Не удалось построить запрос получения значения поля %1 объекта метаданных %2!';ru='Не удалось построить запрос получения значения поля %1 объекта метаданных %2!';en='Unable to get the value of a request to build a field% 1% metadata object 2!'"),
		ИмяПоля, ИмяОбъекта
	);
