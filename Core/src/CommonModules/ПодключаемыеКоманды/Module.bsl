...
"требуется явно указывать параметр ""%2""';ka='В формах внешних отчетов и обработок"
"при вызове процедуры ""%1"""
...
Процедура ПроверитьИмяВидаКоманд(ИмяВида)
	...
	Исключение
		ТекстОшибки = НСтр("ka='Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';ru='Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';en='type the name of the teams ""%1"" does not satisfy the requirements of naming variables.'");
...
Процедура ВывестиКоманды_2_4(Форма, Команды, ПараметрыРазмещения)
	...
	Если КомандаЗаглушка = Неопределено Тогда
		...
		КомандаЗаглушка.Заголовок = НСтр("ka='(нет)';ru='(нет)';en='(not)'");
	КонецЕсли;
...
"Подробнее см. документацию БСП.';ka='Не внедрена обязательная подсистема ""Подключаемые отчеты и обработки""."
...
"%2';ka='При определении команд, подключенных к форме ""%1"", возникла ошибка:"
"%2'"),
				ИмяФормы,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				НСтр("ka='შესაერთებელი ბრძანებები';ru='Подключаемые команды';en='connected team'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ЗначениеЗаполнено(ОписаниеКоманды.Обработчик) Тогда
			...
			Если КоличествоПодстрок = 2 Тогда
				...
				Если ОбъектМетаданныхОбщийМодуль = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Общий модуль ""%1"" не найден.';ru='Общий модуль ""%1"" не найден.';en='Total module ""%1"" not found.'"),
						ИмяМодуля);
				КонецЕсли;
