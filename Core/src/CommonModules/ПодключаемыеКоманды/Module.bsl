...
Процедура ПроверитьИмяВидаКоманд(ИмяВида)
	...
	Исключение
		ТекстОшибки = НСтр("ka='Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';ru='Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.'");
...
Процедура ВывестиКоманды_2_4(Форма, Команды, ПараметрыРазмещения)
	...
	Если КомандаЗаглушка = Неопределено Тогда
		...
		КомандаЗаглушка.Заголовок = НСтр("ka='(нет)';ru='(нет)'");
	КонецЕсли;
...
"%2'"),
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='შესაერთებელი ბრძანებები';ru='Подключаемые команды'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ЗначениеЗаполнено(ОписаниеКоманды.Обработчик) Тогда
			...
			Если КоличествоПодстрок = 2 Тогда
				...
				Если ОбъектМетаданныхОбщийМодуль = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Общий модуль ""%1"" не найден.';ru='Общий модуль ""%1"" не найден.'"),
						ИмяМодуля);
				КонецЕсли;
