...
Функция ПолучитьКластер(СоединениеСАгентомСервера, Знач ПортКластера, Знач ИмяАдминистратораКластера, Знач ПарольАдминистратораКластера)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სამუშაო სერვერ %1-ზე კლასტერი %2 ვერ მოიძებნა';ru='На рабочем сервере %1 не найден кластер %2'"),
...
Функция СоединениеСРабочимПроцессом(COMСоединитель, СоединениеСАгентомСервера, Кластер)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერში %1:%2 აქტიური სამუშაო პროცესები ვერ მოიძებნა.';ru='В кластере серверов %1:%2 не найдено активных рабочих процессов.'"),
		Кластер.HostName,
...
Функция ПолучитьОписаниеИБ(СоединениеСАгентомСервера, Кластер, Знач ИмяВКластере)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერში %1:%2 საინფორმაციო ბაზა ""%3"" ვერ მოიძებნა!';ru='В кластере серверов %1:%2 не найдена информационная база ""%3""!'"),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяВКластере);
	...
	Для Каждого ИнформационнаяБаза Из СоединениеСРабочимПроцессом.GetInfoBases() Цикл
		...
		Если ИнформационнаяБаза.Name = ИмяВКластере Тогда
			...
			Если Не ЗначениеЗаполнено(ИнформационнаяБаза.DBMS) Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 საინფორმაციო ბაზის ადმინისტრატორის არასწორი სახელი და პაროლი სერვერების კლასტერში %2:%3 (სახელი: ""%4"").';ru='Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").'"),
					ИмяВКластере,
					Кластер.HostName, 
					Кластер.MainPort,
					ИмяАдминистратораИБ);
...
Функция ПолучитьПрофильБезопасности(СоединениеСАгентомСервера, Кластер, ИмяПрофиля)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერში %1:%2 უსაფრთხოების პროფილი ""%3"" ვერ მოიძებნა!';ru='В кластере серверов %1:%2 не найден профиль безопасности ""%3""!'"),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяПрофиля);
