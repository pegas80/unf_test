	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საგადასახადო ორგანოს კოდი %1:';ru='Код налогового органа %1:'"), РеквизитыНалоговогоОргана.Код)
...
Функция РеквизитыОтделенияФССПоКоду(Знач КодОтделенияФСС) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სოცდაზღვევის ფონდის განყოფილების კოდი %1:';ru='Код отделения ФСС %1:'"), РеквизитыФСС.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Функция РеквизитыОтделенияПФРПоКоду(Знач КодОтделенияПФР) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საპენსიო ფონდის განყოფილების კოდი %1:';ru='Код отделения ПФР %1:'"), РеквизитыПФР.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбновитьДанныеГосударственногоОргана(Реквизиты) Экспорт
	...
	Иначе
		...
		НаименованиеРодителя = НСтр("ka='სახელმიფო ორგანოები';ru='Государственные органы'");
		Запрос = Новый Запрос();
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სახელმწიფო ორგანო %1-ის შენახვის შეცდომა:';ru='Ошибка сохранения государственного органа %1:'"), Реквизиты.Код)
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			...
			Если ЗначениеЗаполнено(Реквизиты) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Реквизиты.ОписаниеОшибки) Тогда
					...
					Если Реквизиты.ИНН <> ПлатежныеРеквизиты.ИНН Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='ИНН';ru='ИНН'");
					КонецЕсли;
					
					Если Реквизиты.КПП <> ПлатежныеРеквизиты.КПП Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='КПП';ru='КПП'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.ПолучательПлатежа <> ПлатежныеРеквизиты.ПолучательПлатежа Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='გადახდის მიმღები';ru='Получатель платежа'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.РасчетныйСчет <> ПлатежныеРеквизиты.РасчетныйСчет Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='ანგარიშსწორების ანგარიში';ru='Расчетный счет'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.БИК <> ПлатежныеРеквизиты.БИК Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='ბანკის კოდი';ru='БИК'");
					КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, СтруктураРеквизитов)
	...
	ИначеЕсли Найти(ОписаниеОшибки, "SERVER-1") > 0 Тогда
		ТекстОшибки  = НСтр("ka='Не указан код государственного органа';ru='Не указан код государственного органа'");
	...
	ИначеЕсли Найти(ОписаниеОшибки, "SERVER-3") > 0 Тогда
		ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მონაცემები კოდ %1-ის მქონე სახელმწიფო ორგანოს შესახებ ვერ მოიძებნა';ru='Данные о государственном органе с кодом %1 не найдены'"),
	...
	ИмяСобытия = НСтр("ka='Сервис данных гос_органов.';ru='Сервис данных гос_органов.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
...
Функция НаименованиеПлательщикаПолучателяПоУмолчанию(ВладелецСчета, БанковскийСчет) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваБанковскогоСчета.БанкРасчетов)
		...
		Наименование = СокрЛП(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ს/ა %2  %3 %4-ში';ru='%1 р/с %2 в %3 %4'"),
