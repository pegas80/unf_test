	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საგადასახადო ორგანოს კოდი %1:';ru='Код налогового органа %1:';en='the tax authority code of% 1:'"), РеквизитыНалоговогоОргана.Код)
...
Функция РеквизитыОтделенияФССПоКоду(Знач КодОтделенияФСС) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სოცდაზღვევის ფონდის განყოფილების კოდი %1:';ru='Код отделения ФСС %1:';en='Branch Code FSS% 1:'"), РеквизитыФСС.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Функция РеквизитыОтделенияПФРПоКоду(Знач КодОтделенияПФР) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საპენსიო ფონდის განყოფილების კოდი %1:';ru='Код отделения ПФР %1:';en='Branch Code FIU% 1:'"), РеквизитыПФР.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбновитьДанныеГосударственногоОргана(Реквизиты) Экспорт
	...
	Иначе
		...
		НаименованиеРодителя = НСтр("ka='სახელმიფო ორგანოები';ru='Государственные органы';en='state authorities'");
		Запрос = Новый Запрос();
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სახელმწიფო ორგანო %1-ის შენახვის შეცდომა:';ru='Ошибка сохранения государственного органа %1:';en='Failed to save the state body% 1:'"), Реквизиты.Код)
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			...
			Если ЗначениеЗаполнено(Реквизиты) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Реквизиты.ОписаниеОшибки) Тогда
					...
					Если Реквизиты.ИНН <> ПлатежныеРеквизиты.ИНН Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='ИНН';ru='ИНН';en='INN'");
					КонецЕсли;
					
					Если Реквизиты.КПП <> ПлатежныеРеквизиты.КПП Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='КПП';ru='КПП';en='PPC'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.ПолучательПлатежа <> ПлатежныеРеквизиты.ПолучательПлатежа Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='გადახდის მიმღები';ru='Получатель платежа';en='Payment receiver'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.РасчетныйСчет <> ПлатежныеРеквизиты.РасчетныйСчет Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='ანგარიშსწორების ანგარიში';ru='Расчетный счет';en='Payment account'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.БИК <> ПлатежныеРеквизиты.БИК Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ka='ბანკის კოდი';ru='БИК';en='BIC'");
					КонецЕсли;
...
"указаны верно';ka='საგადახდო რეკვიზიტები სწორად არის მითითებული'"));
					Иначе
						...
						Результат.Вставить("РезультатПроверки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='შეიძლება, რომ საგადახდო რეკვიზიტები არასწორად არის მითითებული. რეკვიზიტებში მოიძებნა სხვაობა: %1';ru='Возможно, платежные реквизиты указаны не верно."
"Найдены отличия в реквизитах: %1'"), СписокРасхождений));
				...
				ИначеЕсли Реквизиты.ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
					...
					Результат.Вставить("РезультатПроверки", НСтр("ka='საგადახდო რეკვიზიტების შესამოწმებლად აუცილებელია მომხმარებლების ინტერნეტ-მხარდაჭერაზე შეერთება.';ru='Для проверки платежных реквизитов"
...
Функция ПредставлениеПлатежныхРеквизитовГосударственногоОргана(ГосударственныйОрган) Экспорт
	...
	Иначе
		Возврат НСтр("ka='<არ არის შევსებული>';ru='<Не заполнено>';en='<Not filled>'");
	КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, СтруктураРеквизитов)
	...
	ИначеЕсли Найти(ОписаниеОшибки, "SERVER-1") > 0 Тогда
		ТекстОшибки  = НСтр("ka='Не указан код государственного органа';ru='Не указан код государственного органа';en='Unknown state agency code'");
	...
	ИначеЕсли Найти(ОписаниеОшибки, "SERVER-3") > 0 Тогда
		ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მონაცემები კოდ %1-ის მქონე სახელმწიფო ორგანოს შესახებ ვერ მოიძებნა';ru='Данные о государственном органе с кодом %1 не найдены';en='Data of the public authority with the code% 1 not found'"),
	...
	ИмяСобытия = НСтр("ka='Сервис данных гос_органов.';ru='Сервис данных гос_органов.';en='Service data gos_organov.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
...
Функция НаименованиеПлательщикаПолучателяПоУмолчанию(ВладелецСчета, БанковскийСчет) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваБанковскогоСчета.БанкРасчетов)
		...
		Наименование = СокрЛП(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ს/ა %2  %3 %4-ში';ru='%1 р/с %2 в %3 %4';en='1% p / s in 2% 3% 4%'"),
