...
Функция ИнформацияОДоступномОбновленииВызовОперации(
	...
	Если ИмяТекущейПрограммы = "Unknown" Тогда
		...
		Результат.Сообщение = НСтр("ka='Неверные параметры подключения к сервису.';ru='Неверные параметры подключения к сервису.'");
...
Функция НовыйКонтекстПолученияИУстановкиОбновлений(Параметры) Экспорт
	...
	Результат.Вставить("ТекущееДействие",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Подготовка к получению обновления...';ru='Подготовка к получению обновления...'"),
			КоличествоФайлов));
...
Процедура СоздатьКаталогиДляПолученияОбновления(Обновление, Контекст) Экспорт
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Ошибка при создании каталога для сохранения дистрибутива конфигурации (%1).';ru='Ошибка при создании каталога для сохранения дистрибутива конфигурации (%1).'"),
		...
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось создать каталог %1 для сохранения дистрибутива конфигурации. %2';ru='Не удалось создать каталог %1 для сохранения дистрибутива конфигурации. %2'"),
			ВременныйКаталогОбновленийКонфигурации,
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Ошибка при создании каталога (%1).';ru='Ошибка при создании каталога (%1).'"),
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось создать каталог %1. %2';ru='Не удалось создать каталог %1. %2'"),
			КаталогИндексаФайлов,
			КраткоеПредставлениеОшибки(ИнфОшибка));
		Возврат;
	...
	Если Обновление.ФорматФайлаОбновления <> "zip" Тогда
		...
		Исключение
			...
			СообщениеЖурнала =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Ошибка при создании каталога дистрибутива конфигурации (%1).';ru='Ошибка при создании каталога дистрибутива конфигурации (%1).'"),
	...
	Если Обновление.ФорматФайлаОбновления = "zip" Тогда
		...
		Если Не ФайлСуществует(Обновление.ПолноеИмяCFUФайлаВКаталогеДистрибутивов, Ложь) Тогда
			...
			Контекст.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Некорректный файл дистрибутива %1. Отсутствует файл обновления конфигурации %2.';ru='Некорректный файл дистрибутива %1. Отсутствует файл обновления конфигурации %2.'"),
			...
			Контекст.Сообщение = НСтр("ka='Дистрибутив не содержит файл обновления конфигурации.';ru='Дистрибутив не содержит файл обновления конфигурации.'");
			Возврат;
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='პროგრამის განახლებების მიღება';ru='Получение обновлений программы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
