...
Функция ПервоеЗначениеКонтактнойИнформацииОбъектаПоТипу(Ссылка, ТипКонтактнойИнформации) Экспорт
	...
	Если СтрНачинаетсяС(ПолноеИмя , НСтр("ka='ცნობარი';ru='Справочник';en='Directory'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ka='ცნობარი';ru='Справочник';en='Directory'") + Ссылка.Метаданные().Имя;
	ИначеЕсли СтрНачинаетсяС(ПолноеИмя , НСтр("ka='დოკუმენტი';ru='Документ';en='Document'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ka='დოკუმენტი';ru='Документ';en='Document'") + Ссылка.Метаданные().Имя;
	КонецЕсли;
...
"До завершения обработки некоторая контактная информация может отображаться некорректно.';ka='საკონტაქტო ინფორმაციის ცნობების განახლება."
"დამუშავების დასრულებამდე ზოგიერთი საკონტაქტო ინფორმაცია შეიძლება არაკორექტულად იქნას გამოსახული.'");
...
"До завершения обработки данные документов будут отображаться некорректно.';ka='Заполнение значения поля ""Другое"" у видов контактной информации."
...
Процедура ОбновитьСуществующиеСтраныМира() Экспорт
	...
	Для Каждого СтрокаКлассификатора Из Справочники.СтраныМира.ТаблицаКлассификатора() Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მსოფლიოს ქვეყნა %1-ის (კოდი %2) ჩაწერის შეცდომა კლასიფიკატორის განახლებისას, %3';ru='Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3';en='of the world record error% 1 (code% 2) for updating classifier, 3%'"),
...
Функция ПроверитьАдресВXML(АдресВXML, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если НЕ ЭтоСтрокаXML(АдресВXML) Тогда
		...
		РезультатПроверки.СписокОшибок.Добавить("ФорматАдреса", НСтр("ka='მისამართის არაკორექტული ფორმატი';ru='Некорректный формат адреса';en='Invalid address format'"));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საკონტაქტო ინფორმაცია';ru='Контактная информация';en='Contact Information'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗаполнитьСписокАвтоподбораИзТаблицы(Результат, ТабличныеДанные, ЭтоПодборУлицы)
	...
	Для Каждого Строка Из ТабличныеДанные Цикл
		...
		Если Строка.Неактуален Тогда
			...
			Предупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მისამართი ""%1"" არააქტუალურია.';ru='Адрес ""%1"" неактуален.';en='Address ""%1"" is irrelevant.'"), Строка.Представление);
		Иначе
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Исключение
			...
			Если ТипЗнч(ОжидаемыйВид) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საკონტაქტო ინფორმაციის არაკორექტული XML ფორმატი ""%1""-ისთვის, ველების მნიშვნელობები გასუფთავებული იქნა.';ru='Некорректный формат XML контактной информации для ""%1"", значения полей были очищены.';en='Malformed XML contact information for the ""%1"", the values ​​of the fields have been cleared.'"),
					Строка(ОжидаемыйВид));
			Иначе
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის არაკორექტული XML ფორმატი, ველების მნიშვნელობები გასუფთავებული იქნა.';ru='Некорректный формат XML контактной информации, значения полей были очищены.';en='Malformed XML contact information, the field values ​​have been cleared.'");
			КонецЕсли;
		КонецПопытки;
		...
		Если ТекстОшибки = Неопределено Тогда
			...
			Если ОжидаемыйТип = ПеречислениеАдрес И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია მისამართი';ru='Ошибка десериализации контактной информации, ожидается адрес';en='Error deserializing contact information, is expected to address'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеАдресЭлектроннойПочты И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ელექტრონული ფოსტის მისამართი';ru='Ошибка десериализации контактной информации, ожидается адрес электронной почты';en='Error deserializing contact information, e-mail address will'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ვებ-გვერდი';ru='Ошибка десериализации контактной информации, ожидается веб-страница';en='Error deserializing contact information, it is expected a web page'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеТелефон И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "НомерТелефона") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ტელეფონი';ru='Ошибка десериализации контактной информации, ожидается телефон';en='Error deserializing contact information, the phone will'");
			...
			ИначеЕсли ОжидаемыйТип = ПеречислениеSkype И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Skype") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია Skype';ru='Ошибка десериализации контактной информации, ожидается Skype';en='Error deserializing contact details, will Skype'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеДругое И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Прочее") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ""სხვა""';ru='Ошибка десериализации контактной информации, ожидается ""другое""';en='Error deserializing contact information, will the ""other""'");
			КонецЕсли;
		КонецЕсли;
...
"т.к. обязательное поле тип не заполнено.';ka='ცნობები საკონტაქტო ინფორმაციის სახეობა %1-ის შესახებ დაზიანდა ან არაკორექტულად არის შევსებული, ვინაიდან სავალდებულო ველი ""ტიპი"" არ არის შევსებული.'");
...
Функция ДесериализацияПрочейКонтактнойИнформации(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	ИначеЕсли ОжидаемыйТип <> Неопределено Тогда
		ВызватьИсключение НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია სხვა ტიპი';ru='Ошибка десериализации контактной информации, ожидается другой тип';en='Error deserializing contact information, another type expected'");
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если ТипЗнч(АдресРФ) <> Тип("ОбъектXDTO") Или АдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		Если ФлагиПроверки.АдресТолькоРоссийский Тогда
			ТекстОшибки = НСтр("ka='მისამართი უნდა იყოს მხოლოდ საქართველოსი.';ru='Адрес должен быть только российским.';en='The address must be only Russian.'");
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресРФ) Тогда
		...
		Если ФлагиПроверки.ОбязательноеЗаполнение Тогда
			...
			ТекстОшибки = НСтр("ka='მისამართი არ არის შევსებული.';ru='Адрес не заполнен.';en='Address is not complete.'");
	...
	Если РезультатГруппами Тогда
		...
		ИначеЕсли КоличествоОшибок > 0 Тогда
			...
			ВсеОшибки.Добавить(ГруппаОшибокАдреса(НаименованиеГруппыОшибки,
				НСтр("ka='მისამართის ნაწილები სამისამართო კლასიფიკატორს არ შეესაბამება:';ru='Части адреса не соответствуют адресному классификатору:';en='Part of the address do not match the address classifier:'")));
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Иначе
		...
		Если XTDOКонтактная.Представление <> Представление Тогда
			Результат.Добавить(ГруппаОшибокАдреса("ПредставлениеНеСоответствуетНаборуПолей",
				НСтр("ka='მისამართი არ შეესაბამება ველების კრებულში არსებულ მნიშვნელობებს.';ru='Адрес не соответствует значениям в наборе полей.';en='Address does not match the values ​​in a set of fields.'")));
			ДобавитьОшибкуЗаполненияАдреса(Результат[0], "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მისამართის წარდგენა საკონტაქტო ინფორმაციის სახეობისთვის ""%1"" მისამართში არსებული მონაცემებისგან განსხვავდება.';ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.';en='Submission address for contact information type ""%1"" is different from the data in the address.'"),
				Строка(ВидИнформации.Наименование)));
		КонецЕсли;
	КонецЕсли;
	
	НеЗаполненыОбязательныеПоля = ГруппаОшибокАдреса("НеЗаполненыОбязательныеПоля",
		НСтр("ka='სავალდებულო ველები არ არის შევსებული:';ru='Не заполнены обязательные поля:';en='Missing required field:'"));
	Результат.Добавить(НеЗаполненыОбязательныеПоля);
	
	НеУказаныСокращенияПолей = ГруппаОшибокАдреса("НеУказаныСокращенияПолей",
		НСтр("ka='შემოკლება არ არის მითითებული ველებისთვის:';ru='Не указано сокращение для полей:';en='Unknown reduction for the fields:'"));
	Результат.Добавить(НеУказаныСокращенияПолей);
	
	НедопустимыеСимволыПолей = ГруппаОшибокАдреса("НедопустимыеСимволыПолей",
		НСтр("ka='დაუშვებელი სიმბოლოები მოიძებნა შემდეგ ველებში:';ru='Найдены недопустимые символы в полях:';en='Found invalid characters in the fields of:'"));
	Результат.Добавить(НедопустимыеСимволыПолей);
	
	НеСоответствуетДлинаПолей = ГруппаОшибокАдреса("НеСоответствуетДлинаПолей",
		НСтр("ka='დადგენილს არ შეესაბამება ველების სიგრძე:';ru='Не соответствует установленной длина полей:';en='Does not meet the established field length:'"));
	...
	Если ПустаяСтрока(Индекс) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса(),
			НСтр("ka='საფოსტო ინდექსი არ არის მითითებული.';ru='Не указан почтовый индекс.';en='Missing postal code.'"), "Индекс");
	КонецЕсли;
	...
	Если ПустаяСтрока(Регион) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "СубъектРФ",
			НСтр("ka='რეგიონი არ არის მითითებული.';ru='Не указан регион.';en='Unknown region.'"), "Регион");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Регион)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "СубъектРФ",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონ ""%1""-ის სახელწოდებაში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии региона ""%1"".';en='Unknown reduction in the region''s name ""%1"".'"), Регион), НСтр("ka='რეგიონი';ru='Регион';en='Region'"));
		КонецЕсли;
		Если СтрДлина(Регион) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონი ""%1""-ის სახელწოდება 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название региона ""%1"" должно быть короче 50 символов.';en='the region the name ""%1"" must be shorter than 50 characters.'"),Регион), НСтр("ka='რეგიონი';ru='Регион';en='Region'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონ ""%1""-ის სახელწოდებაში არ არის რუსული სიმბოლოები.';ru='В названии региона ""%1"" есть не кириллические символы.';en='In the name of the region ""%1"" is not a Cyrillic characters.'"), Регион), НСтр("ka='რეგიონი';ru='Регион';en='Region'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Район)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რაიონ ""%1""-ის სახელწოდებაში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии района ""%1"".';en='Unknown reduction in the area the name ""%1"".'"), Район), НСтр("ka='რაიონი';ru='Район';en='District'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რაიონის სახელწოდება ""%1"" 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название района ""%1"" должно быть короче 50 символов.';en='District name ""%1"" must be shorter than 50 characters.'"), Район), НСтр("ka='რაიონი';ru='Район';en='District'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რაიონ ""%1""-ის სახელწოდებაში არ არის რუსული სიმბოლოები.';ru='В названии района ""%1"" есть не кириллические символы.';en='The district name ""%1"" is not a Cyrillic characters.'"), Район), НСтр("ka='რაიონი';ru='Район';en='District'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Город)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქის სახელწოდება ""%1""-ში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии города ""%1"".';en='Unknown reduction in the city''s name ""%1"".'"), Город), НСтр("ka='ქალაქი';ru='Город';en='Town'"));
		КонецЕсли;
		Если СтрДлина(Город) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქის სახელწოდება ""%1"" 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название города ""%1"" должно быть короче 50 символов.';en='City Name ""%1"" must be shorter than 50 characters.'"), Город), НСтр("ka='ქალაქი';ru='Город';en='Town'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქ ""%1""-ის სახელწოდება არაქართულ სიმბოლოებს შეიცავს.';ru='В названии города ""%1"" есть не кириллические символы.';en='In the city the title of ""%1"" is not a Cyrillic characters.'"), Город), НСтр("ka='ქალაქი';ru='Город';en='Town'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(НаселенныйПункт)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასახლებული პუნქტის სახელწოდება ""%1""-ში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии населенного пункта ""%1"".';en='Unknown reduction in the locality name ""%1"".'"), НаселенныйПункт),
					НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
		Если СтрДлина(НаселенныйПункт) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უკანასკნელი პუნქტის ""%1""-ის სახელწოდება 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название населенного пункта ""%1"" должно быть короче 50 символов.';en='The name of the item ""%1"" must be shorter than 50 characters.'"), НаселенныйПункт),
				НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასახლებული პუნქტის სახელწოდება ""%1""-ში არის არაქართული სიმბოლოები.';ru='В названии населенного пункта ""%1"" есть не кириллические символы.';en='The name of the village ""%1"" is not a Cyrillic characters.'"), НаселенныйПункт),
					НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Улица)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქუჩის სახელწოდება ""%1""-ში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии улицы ""%1"".';en='Unknown reduction in street name ""%1"".'"), Улица), НСтр("ka='ქუჩა';ru='Улица';en='Street'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქუჩის სახელწოდება ""%1"" 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название улицы ""%1"" должно быть короче 50 символов.';en='street name ""%1"" must be shorter than 50 characters.'"), Улица), НСтр("ka='ქუჩა';ru='Улица';en='Street'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქუჩის სახელწოდება ""%1""-ში არის არაქართული სიმბოლოები.';ru='В названии улицы ""%1"" есть не кириллические символы.';en='The street name ""%1"" is not a Cyrillic characters.'"), Улица), НСтр("ka='ქუჩა';ru='Улица';en='Street'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(ДополнительныйЭлемент)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებით ელემენტ ""%1""-ზე შემოკლება არ არის მითითებული.';ru='Не указано сокращение у дополнительного элемента ""%1"".';en='Unknown reduction in the additional element ""%1"".'"), ДополнительныйЭлемент),
				НСтр("ka='ქუჩა';ru='Улица';en='Street'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ელემენტ ""%1""-ის სახელწოდება 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название дополнительного элемента ""%1"" должно быть короче 50 символов.';en='Title additive element ""%1"" should be shorter than 50 characters.'"), ДополнительныйЭлемент),
				НСтр("ka='ДополнительныйЭлемент';ru='ДополнительныйЭлемент';en='DopolnitelnyyElement'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(ДополнительныйЭлемент, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ელემენტ ""%1""-ის სახელწოდებაში არაქართული სიმბოლოები არის.';ru='В названии дополнительного элемента ""%1"" есть не кириллические символы.';en='The title of the additional item ""%1"" is not a Cyrillic characters.'"), ДополнительныйЭлемент),
				НСтр("ka='ДополнительныйЭлемент';ru='ДополнительныйЭлемент';en='DopolnitelnyyElement'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Индекс) Тогда
		...
		Если СтрДлина(Индекс) <> 6 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Индекс) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				НСтр("ka='საფოსტო ინდექსი უნდა შედგებოდეს 6 ციფრისგან';ru='Почтовый индекс должен состоять из 6 цифр.';en='Postal code should consist of 6 digits.'"),
				НСтр("ka='ინდექსი';ru='Индекс';en='Index'"));
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ДанныеЗдания Из ЗданияПомещения.Здания Цикл
		Если СтрДлина(ДанныеЗдания.Значение) > 12 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеЗдания.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение поля ""%1"" должно быть короче 12 символов.';en='The field value ""1%"" should be shorter than 12 characters.'"), ДанныеЗдания.Тип), ДанныеЗдания.Тип);
		КонецЕсли;
	КонецЦикла;
	Для Каждого ДанныеПомещения Из ЗданияПомещения.Помещения Цикл
		Если СтрДлина(ДанныеПомещения.Значение) > 12 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеПомещения.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение поля ""%1"" должно быть короче 12 символов.';en='The field value ""1%"" should be shorter than 12 characters.'"), ДанныеПомещения.Тип), ДанныеПомещения.Тип);
		КонецЕсли;
	КонецЦикла;
	...
	Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Район) Тогда
		Если НазванияГородовФедеральногоЗначения().Найти(ВРег(Регион)) = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Город",
				НСтр("ka='ქალაქი შეიძლება, რომ არ იყოს მითითებული მხოლოდ რეგიონში - ცენტრალური მნიშვნელობის ქალაქში.';ru='Город может быть не указан только в регионе - городе федерального значения.';en='The city can be specified only in the region - the city of federal significance.'"),
				НСтр("ka='ქალაქი';ru='Город';en='Town'"));
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "НаселПункт",
				НСтр("ka='დასახლებული პუნქტი შეიძლება, რომ არ იქნას მითითებული მხოლოდ რეგიონში - ცენტრალური მნიშვნელობის ქალაქში.';ru='Населенный пункт может быть не указан только в регионе - городе федерального значения.';en='Settlement can be specified only in the region - the city of federal significance.'"),
				НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресРФ) Тогда
		...
		Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Улица) И НЕ ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Улица",
				НСтр("ka='ქალაქი ან დასახლებული პუნქტი უნდა შეიცავდეს ქუჩის სახელს.';ru='Город или населенный пункт должен содержать название улицы.';en='City or town must include the name of the street.'"), 
				НСтр("ka='ქუჩა';ru='Улица';en='Street'"));
		КонецЕсли;
...
Функция ИсключатьПроверкуУлицыВАдресе(Знач АдресРФ)
	...
	Если ВРег(СокрЛП(АдресРФ.СубъектРФ)) = НСтр("ka='მოსკოვი ქ';ru='МОСКВА Г';en='MOSCOW F'") И ВРег(СокрЛП(АдресРФ.Город)) = НСтр("ka='ქ. ზელენოგრადი';ru='ЗЕЛЕНОГРАД Г';en='Zelenograd G'") Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საკონტაქტო ინფორმაციის სახეობა ""%1""-ზე სავალდებულო ველი ""ტიპი"" არ არის შევსებული.';ru='Не заполонено обязательное поле тип у вида контактной информации ""%1"".';en='Not flooded Obligatory field type in the type of contact information ""%1"".'"),
...
" Проверка корректности адреса доступна только для российских адресов';ka='საკონტაქტო ინფორმაციის სახეობა %1-ზე მისამართის შემოწმების გამართვები არაკორექტულად არის შევსებული."
"მისამართის კორექტულობის შემოწმება ხელმისაწვდომია მხოლოდ საქართველოს მისამართებისთვის'"), Строка(ВидКонтактнойИнформации.Наименование));
...
" Не допускается возможность ввода нескольких значений контактной информации при включенной истории хранения изменений.';ka='საკონტაქტო ინფორმაციის სახეობა %1-ზე მისამართის გამართვები არაკორექტულად არის შევსებული."
" ცვლილებების შენახვის ჩართული ისტორიის დროს დაუშვებელია საკონტაქტო ინფორმაციის რამდენიმე მნიშვნელობის შეყვანის შესაძლებლობა.'"),
						Строка(ВидКонтактнойИнформации.Наименование));
		КонецЕсли;
	КонецЕсли;
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип <> Неопределено Тогда
		Если ОжидаемыйТип <> Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			ВызватьИсключение НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია მისამართი';ru='Ошибка десериализации контактной информации, ожидается адрес';en='Error deserializing contact information, is expected to address'");
		КонецЕсли;
	КонецЕсли;
...
Функция ДесериализацияТелефонаФакса(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ტელეფონი ან ფაქსი';ru='Ошибка десериализации контактной информации, ожидается телефон или факс';en='Error deserializing contact information, is expected to phone or fax'");
	КонецЕсли;
...
Функция РазличияКонтактнойИнформации(Знач Данные1, Знач Данные2) Экспорт
	...
	Если ТипКонтактнойИнформации <> ДанныеКИ2.ТипКонтактнойИнформации Тогда
		...
		СтрокаРезультата[Колонки.Добавить("Описание").Имя]  = НСтр("ka='საკონტაქტო ინფორმაციის სხვადასხვა ტიპები';ru='Различные типы контактной информации';en='Various types of contact information'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПривестиКонтактнуюИнформациюXML(Знач Данные) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='გარდაქმნის პარამეტრების შეცდომა, საკონტაქტო ინფორმაციის ტიპი არ არის დადგენილი';ru='Ошибка параметров преобразования, не определен тип контактной информации';en='Error converting the parameters are not defined by the type of contact information'");
