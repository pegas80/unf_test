...
Функция ПервоеЗначениеКонтактнойИнформацииОбъектаПоТипу(Ссылка, ТипКонтактнойИнформации) Экспорт
	...
	Если СтрНачинаетсяС(ПолноеИмя , НСтр("ka='ცნობარი';ru='Справочник'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ka='ცნობარი';ru='Справочник'") + Ссылка.Метаданные().Имя;
	ИначеЕсли СтрНачинаетсяС(ПолноеИмя , НСтр("ka='დოკუმენტი';ru='Документ'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ka='დოკუმენტი';ru='Документ'") + Ссылка.Метаданные().Имя;
	КонецЕсли;
...
Процедура ОбновитьСуществующиеСтраныМира() Экспорт
	...
	Для Каждого СтрокаКлассификатора Из Справочники.СтраныМира.ТаблицаКлассификатора() Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მსოფლიოს ქვეყნა %1-ის (კოდი %2) ჩაწერის შეცდომა კლასიფიკატორის განახლებისას, %3';ru='Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3'"),
...
Функция ПроверитьАдресВXML(АдресВXML, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если НЕ ЭтоСтрокаXML(АдресВXML) Тогда
		...
		РезультатПроверки.СписокОшибок.Добавить("ФорматАдреса", НСтр("ka='მისამართის არაკორექტული ფორმატი';ru='Некорректный формат адреса'"));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საკონტაქტო ინფორმაცია';ru='Контактная информация'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗаполнитьСписокАвтоподбораИзТаблицы(Результат, ТабличныеДанные, ЭтоПодборУлицы)
	...
	Для Каждого Строка Из ТабличныеДанные Цикл
		...
		Если Строка.Неактуален Тогда
			...
			Предупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მისამართი ""%1"" არააქტუალურია.';ru='Адрес ""%1"" неактуален.'"), Строка.Представление);
		Иначе
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Исключение
			...
			Если ТипЗнч(ОжидаемыйВид) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საკონტაქტო ინფორმაციის არაკორექტული XML ფორმატი ""%1""-ისთვის, ველების მნიშვნელობები გასუფთავებული იქნა.';ru='Некорректный формат XML контактной информации для ""%1"", значения полей были очищены.'"),
					Строка(ОжидаемыйВид));
			Иначе
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის არაკორექტული XML ფორმატი, ველების მნიშვნელობები გასუფთავებული იქნა.';ru='Некорректный формат XML контактной информации, значения полей были очищены.'");
			КонецЕсли;
		КонецПопытки;
		...
		Если ТекстОшибки = Неопределено Тогда
			...
			Если ОжидаемыйТип = ПеречислениеАдрес И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია მისამართი';ru='Ошибка десериализации контактной информации, ожидается адрес'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеАдресЭлектроннойПочты И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ელექტრონული ფოსტის მისამართი';ru='Ошибка десериализации контактной информации, ожидается адрес электронной почты'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ვებ-გვერდი';ru='Ошибка десериализации контактной информации, ожидается веб-страница'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеТелефон И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "НомерТелефона") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ტელეფონი';ru='Ошибка десериализации контактной информации, ожидается телефон'");
			...
			ИначеЕсли ОжидаемыйТип = ПеречислениеSkype И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Skype") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია Skype';ru='Ошибка десериализации контактной информации, ожидается Skype'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеДругое И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Прочее") Тогда
				ТекстОшибки = НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ""სხვა""';ru='Ошибка десериализации контактной информации, ожидается ""другое""'");
			КонецЕсли;
		КонецЕсли;
...
Функция ДесериализацияПрочейКонтактнойИнформации(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	ИначеЕсли ОжидаемыйТип <> Неопределено Тогда
		ВызватьИсключение НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია სხვა ტიპი';ru='Ошибка десериализации контактной информации, ожидается другой тип'");
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если ТипЗнч(АдресРФ) <> Тип("ОбъектXDTO") Или АдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		Если ФлагиПроверки.АдресТолькоРоссийский Тогда
			ТекстОшибки = НСтр("ka='მისამართი უნდა იყოს მხოლოდ საქართველოსი.';ru='Адрес должен быть только российским.'");
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресРФ) Тогда
		...
		Если ФлагиПроверки.ОбязательноеЗаполнение Тогда
			...
			ТекстОшибки = НСтр("ka='მისამართი არ არის შევსებული.';ru='Адрес не заполнен.'");
	...
	Если РезультатГруппами Тогда
		...
		ИначеЕсли КоличествоОшибок > 0 Тогда
			...
			ВсеОшибки.Добавить(ГруппаОшибокАдреса(НаименованиеГруппыОшибки,
				НСтр("ka='მისამართის ნაწილები სამისამართო კლასიფიკატორს არ შეესაბამება:';ru='Части адреса не соответствуют адресному классификатору:'")));
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Иначе
		...
		Если XTDOКонтактная.Представление <> Представление Тогда
			Результат.Добавить(ГруппаОшибокАдреса("ПредставлениеНеСоответствуетНаборуПолей",
				НСтр("ka='მისამართი არ შეესაბამება ველების კრებულში არსებულ მნიშვნელობებს.';ru='Адрес не соответствует значениям в наборе полей.'")));
			ДобавитьОшибкуЗаполненияАдреса(Результат[0], "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მისამართის წარდგენა საკონტაქტო ინფორმაციის სახეობისთვის ""%1"" მისამართში არსებული მონაცემებისგან განსხვავდება.';ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.'"),
				Строка(ВидИнформации.Наименование)));
		КонецЕсли;
	КонецЕсли;
	
	НеЗаполненыОбязательныеПоля = ГруппаОшибокАдреса("НеЗаполненыОбязательныеПоля",
		НСтр("ka='სავალდებულო ველები არ არის შევსებული:';ru='Не заполнены обязательные поля:'"));
	Результат.Добавить(НеЗаполненыОбязательныеПоля);
	
	НеУказаныСокращенияПолей = ГруппаОшибокАдреса("НеУказаныСокращенияПолей",
		НСтр("ka='შემოკლება არ არის მითითებული ველებისთვის:';ru='Не указано сокращение для полей:'"));
	Результат.Добавить(НеУказаныСокращенияПолей);
	
	НедопустимыеСимволыПолей = ГруппаОшибокАдреса("НедопустимыеСимволыПолей",
		НСтр("ka='დაუშვებელი სიმბოლოები მოიძებნა შემდეგ ველებში:';ru='Найдены недопустимые символы в полях:'"));
	Результат.Добавить(НедопустимыеСимволыПолей);
	
	НеСоответствуетДлинаПолей = ГруппаОшибокАдреса("НеСоответствуетДлинаПолей",
		НСтр("ka='დადგენილს არ შეესაბამება ველების სიგრძე:';ru='Не соответствует установленной длина полей:'"));
	...
	Если ПустаяСтрока(Индекс) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса(),
			НСтр("ka='საფოსტო ინდექსი არ არის მითითებული.';ru='Не указан почтовый индекс.'"), "Индекс");
	КонецЕсли;
	...
	Если ПустаяСтрока(Регион) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "СубъектРФ",
			НСтр("ka='რეგიონი არ არის მითითებული.';ru='Не указан регион.'"), "Регион");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Регион)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "СубъектРФ",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონ ""%1""-ის სახელწოდებაში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии региона ""%1"".'"), Регион), НСтр("ka='რეგიონი';ru='Регион'"));
		КонецЕсли;
		Если СтрДлина(Регион) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონი ""%1""-ის სახელწოდება 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название региона ""%1"" должно быть короче 50 символов.'"),Регион), НСтр("ka='რეგიონი';ru='Регион'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონ ""%1""-ის სახელწოდებაში არ არის რუსული სიმბოლოები.';ru='В названии региона ""%1"" есть не кириллические символы.'"), Регион), НСтр("ka='რეგიონი';ru='Регион'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Район)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რაიონ ""%1""-ის სახელწოდებაში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии района ""%1"".'"), Район), НСтр("ka='რაიონი';ru='Район'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რაიონის სახელწოდება ""%1"" 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название района ""%1"" должно быть короче 50 символов.'"), Район), НСтр("ka='რაიონი';ru='Район'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რაიონ ""%1""-ის სახელწოდებაში არ არის რუსული სიმბოლოები.';ru='В названии района ""%1"" есть не кириллические символы.'"), Район), НСтр("ka='რაიონი';ru='Район'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Город)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქის სახელწოდება ""%1""-ში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии города ""%1"".'"), Город), НСтр("ka='ქალაქი';ru='Город'"));
		КонецЕсли;
		Если СтрДлина(Город) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქის სახელწოდება ""%1"" 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название города ""%1"" должно быть короче 50 символов.'"), Город), НСтр("ka='ქალაქი';ru='Город'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქალაქ ""%1""-ის სახელწოდება არაქართულ სიმბოლოებს შეიცავს.';ru='В названии города ""%1"" есть не кириллические символы.'"), Город), НСтр("ka='ქალაქი';ru='Город'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(НаселенныйПункт)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასახლებული პუნქტის სახელწოდება ""%1""-ში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии населенного пункта ""%1"".'"), НаселенныйПункт),
					НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт'"));
		КонецЕсли;
		Если СтрДлина(НаселенныйПункт) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უკანასკნელი პუნქტის ""%1""-ის სახელწოდება 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название населенного пункта ""%1"" должно быть короче 50 символов.'"), НаселенныйПункт),
				НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასახლებული პუნქტის სახელწოდება ""%1""-ში არის არაქართული სიმბოლოები.';ru='В названии населенного пункта ""%1"" есть не кириллические символы.'"), НаселенныйПункт),
					НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Улица)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქუჩის სახელწოდება ""%1""-ში შემოკლება არ არის მითითებული.';ru='Не указано сокращение в названии улицы ""%1"".'"), Улица), НСтр("ka='ქუჩა';ru='Улица'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქუჩის სახელწოდება ""%1"" 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название улицы ""%1"" должно быть короче 50 символов.'"), Улица), НСтр("ka='ქუჩა';ru='Улица'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ქუჩის სახელწოდება ""%1""-ში არის არაქართული სიმბოლოები.';ru='В названии улицы ""%1"" есть не кириллические символы.'"), Улица), НСтр("ka='ქუჩა';ru='Улица'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(ДополнительныйЭлемент)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებით ელემენტ ""%1""-ზე შემოკლება არ არის მითითებული.';ru='Не указано сокращение у дополнительного элемента ""%1"".'"), ДополнительныйЭлемент),
				НСтр("ka='ქუჩა';ru='Улица'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ელემენტ ""%1""-ის სახელწოდება 50 სიმბოლოზე მოკლე უნდა იყოს.';ru='Название дополнительного элемента ""%1"" должно быть короче 50 символов.'"), ДополнительныйЭлемент),
				НСтр("ka='ДополнительныйЭлемент';ru='ДополнительныйЭлемент'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(ДополнительныйЭлемент, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ელემენტ ""%1""-ის სახელწოდებაში არაქართული სიმბოლოები არის.';ru='В названии дополнительного элемента ""%1"" есть не кириллические символы.'"), ДополнительныйЭлемент),
				НСтр("ka='ДополнительныйЭлемент';ru='ДополнительныйЭлемент'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Индекс) Тогда
		...
		Если СтрДлина(Индекс) <> 6 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Индекс) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				НСтр("ka='საფოსტო ინდექსი უნდა შედგებოდეს 6 ციფრისგან';ru='Почтовый индекс должен состоять из 6 цифр.'"),
				НСтр("ka='ინდექსი';ru='Индекс'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Район) Тогда
		Если НазванияГородовФедеральногоЗначения().Найти(ВРег(Регион)) = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Город",
				НСтр("ka='ქალაქი შეიძლება, რომ არ იყოს მითითებული მხოლოდ რეგიონში - ცენტრალური მნიშვნელობის ქალაქში.';ru='Город может быть не указан только в регионе - городе федерального значения.'"),
				НСтр("ka='ქალაქი';ru='Город'"));
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "НаселПункт",
				НСтр("ka='დასახლებული პუნქტი შეიძლება, რომ არ იქნას მითითებული მხოლოდ რეგიონში - ცენტრალური მნიშვნელობის ქალაქში.';ru='Населенный пункт может быть не указан только в регионе - городе федерального значения.'"),
				НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресРФ) Тогда
		...
		Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Улица) И НЕ ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Улица",
				НСтр("ka='ქალაქი ან დასახლებული პუნქტი უნდა შეიცავდეს ქუჩის სახელს.';ru='Город или населенный пункт должен содержать название улицы.'"), 
				НСтр("ka='ქუჩა';ru='Улица'"));
		КонецЕсли;
...
Функция ИсключатьПроверкуУлицыВАдресе(Знач АдресРФ)
	...
	Если ВРег(СокрЛП(АдресРФ.СубъектРФ)) = НСтр("ka='მოსკოვი ქ';ru='МОСКВА Г'") И ВРег(СокрЛП(АдресРФ.Город)) = НСтр("ka='ქ. ზელენოგრადი';ru='ЗЕЛЕНОГРАД Г'") Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საკონტაქტო ინფორმაციის სახეობა ""%1""-ზე სავალდებულო ველი ""ტიპი"" არ არის შევსებული.';ru='Не заполонено обязательное поле тип у вида контактной информации ""%1"".'"),
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип <> Неопределено Тогда
		Если ОжидаемыйТип <> Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			ВызватьИсключение НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია მისამართი';ru='Ошибка десериализации контактной информации, ожидается адрес'");
		КонецЕсли;
	КонецЕсли;
...
Функция ДесериализацияТелефонаФакса(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("ka='საკონტაქტო ინფორმაციის დესერიალიზაციის შეცდომა, მოსალოდნელია ტელეფონი ან ფაქსი';ru='Ошибка десериализации контактной информации, ожидается телефон или факс'");
	КонецЕсли;
...
Функция РазличияКонтактнойИнформации(Знач Данные1, Знач Данные2) Экспорт
	...
	Если ТипКонтактнойИнформации <> ДанныеКИ2.ТипКонтактнойИнформации Тогда
		...
		СтрокаРезультата[Колонки.Добавить("Описание").Имя]  = НСтр("ka='საკონტაქტო ინფორმაციის სხვადასხვა ტიპები';ru='Различные типы контактной информации'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПривестиКонтактнуюИнформациюXML(Знач Данные) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='გარდაქმნის პარამეტრების შეცდომა, საკონტაქტო ინფორმაციის ტიპი არ არის დადგენილი';ru='Ошибка параметров преобразования, не определен тип контактной информации'");
