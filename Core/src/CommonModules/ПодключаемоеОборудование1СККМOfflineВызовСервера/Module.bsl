...
Функция ПакетыОбработаны(Выгрузка, Пакеты, ВыходныеПараметры, ВерсияФорматаОбмена) Экспорт
	...
	Для Каждого ТекПакет Из Пакеты Цикл
		...
		Исключение
			СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='დამუშავებულობაზე ფაილის შემოწმებისას შეცდომა მოხდა.';ru='При проверке файла на обработанность произошла ошибка.'"));
			Возврат Неопределено;
		КонецПопытки;
...
Процедура ПометитьОтчетКакОбработанный(ТекстОтчета, ВыходныеПараметры, Отказ, ВерсияФорматаОбмена) Экспорт
	...
	Если ЗначениеЗаполнено(ЗагружаемыеОтчеты.Обработан) Тогда
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ანგარიშგება უკვე მონიშნულია როგორც დამუშავებული';ru='Отчет уже помечен как обработанный'"));
		Отказ = Истина;
	КонецЕсли;
...
Функция СоздатьЧтениеXML(ТекстПакета, ЧтениеXML, ВыходныеПараметры)
	...
	Попытка
		ЧтениеXML.Прочитать();
	Исключение
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ფაილის წაკითხვისას შეცდომა მოხდა';ru='При чтении файла произошла ошибка'"));
		Результат = Ложь;
	КонецПопытки;
...
Функция ОбъектXDTOВТекстXML(ОбъектXDTO, ТекстXML, ВыходныеПараметры)
	...
	Исключение
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ka='ფაილის შედგენისას შეცდომა წარმოიშვა.';ru='При формировании файла произошла ошибка.'"));
		Возврат Ложь;
	КонецПопытки;
