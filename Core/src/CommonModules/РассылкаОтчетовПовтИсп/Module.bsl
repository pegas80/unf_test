...
Функция ТекстОшибкиПроверкиПраваДобавления() Экспорт
	Если Не ПравоДоступа("Вывод", Метаданные) Тогда
		Возврат НСтр("ka='ინფორმაციის გამოტანაზე უფლებები არ არის.';ru='Нет прав на вывод информации.';en='No rights to display information.'");
	КонецЕсли;
	Если Не ПравоДоступа("Изменение", Метаданные.Справочники.РассылкиОтчетов) Тогда
		Возврат НСтр("ka='ანგარიშგებების დასაგზავნად უფლებები არ არის.';ru='Нет прав на рассылки отчетов.';en='No rights for report delivery.'");
	КонецЕсли;
	Если Не РаботаСПочтовымиСообщениями.ДоступнаОтправкаПисем() Тогда
		Возврат НСтр("ka='წერილების გასაგზავნად უფლებები არ არის ან ხელმისაწვდომი სააღრიცხვო ჩანაწერები არ არის.';ru='Нет прав на отправку писем или нет доступных учетных записей.';en='No rights to send letters or not available account.'");
	КонецЕсли;
	Возврат "";
КонецФункции
