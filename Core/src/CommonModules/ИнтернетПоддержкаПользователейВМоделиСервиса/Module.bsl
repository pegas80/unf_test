...
Функция ТикетАутентификацииНаПорталеПоддержкиВНеразделенномСеансе(ВладелецТикета, ОбластьДанных) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ka='Получение тикета недоступно в разделенном сеансе.';ru='Получение тикета недоступно в разделенном сеансе.'");
	КонецЕсли;
...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета, ОбластьДанных = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ka='Недоступно при работе в локальном режиме.';ru='Недоступно при работе в локальном режиме.'");
	КонецЕсли;
	...
	ИначеЕсли ОбластьДанных = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Не заполнено значение параметра ""ОбластьДанных""';ru='Не заполнено значение параметра ""ОбластьДанных""'");
	Иначе
...
"Получен код состояния 404 при обращении к ресурсу %1.'"),
		...
		Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			Результат.СообщениеОбОшибке = НСтр("ka='Ошибка аутентификации (404).';ru='Ошибка аутентификации (404).'");
		Иначе
			Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.'");
		КонецЕсли;
				...
				Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
					Результат.СообщениеОбОшибке =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Ошибка аутентификации (%1).';ru='Ошибка аутентификации (%1).'"),
							РезультатОперации.КодСостояния);
				Иначе
					Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.'");
				КонецЕсли;
...
"Тело запроса: %5'"),
			...
			Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
				Результат.СообщениеОбОшибке =
					НСтр("ka='Ошибка аутентификации. Некорректный ответ сервиса.';ru='Ошибка аутентификации. Некорректный ответ сервиса.'");
			Иначе
				Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.'");
			КонецЕсли;
...
"Тело запроса: %5'"),
		...
		Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			Результат.СообщениеОбОшибке =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Неизвестная ошибка сервиса аутентификации (%1).';ru='Неизвестная ошибка сервиса аутентификации (%1).'"),
					РезультатОперации.КодСостояния);
		Иначе
			Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.'");
		КонецЕсли;
