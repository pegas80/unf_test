...
Функция ТикетАутентификацииНаПорталеПоддержкиВНеразделенномСеансе(ВладелецТикета, ОбластьДанных) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ka='Получение тикета недоступно в разделенном сеансе.';ru='Получение тикета недоступно в разделенном сеансе.';en='Getting the ticket is not available in a shared session.'");
	КонецЕсли;
...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета, ОбластьДанных = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ka='Недоступно при работе в локальном режиме.';ru='Недоступно при работе в локальном режиме.';en='Not available when operating in Local Mode.'");
	КонецЕсли;
	...
	ИначеЕсли ОбластьДанных = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Не заполнено значение параметра ""ОбластьДанных""';ru='Не заполнено значение параметра ""ОбластьДанных""';en='Not filled value ""OblastDannyh"" parameter'");
	Иначе
...
"Получен код состояния 404 при обращении к ресурсу %1.';ka='В Менеджере сервиса отсутствует сервис тикетов аутентификации."
"Получен код состояния 404 при обращении к ресурсу %1.'"),
		...
		Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			Результат.СообщениеОбОшибке = НСтр("ka='Ошибка аутентификации (404).';ru='Ошибка аутентификации (404).';en='Authentication failed (404).'");
		Иначе
			Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.';en='Authentication failed.'");
		КонецЕсли;
...
"Код состояния: %2;"
"Сообщение: %3"
"Тело запроса: %4';ka='Не удалось получить тикет аутентификации в Менеджере сервиса (%1)."
"Код состояния: %2;"
"Сообщение: %3"
"Тело запроса: %4'"),
					URLСервиса,
				...
				Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
					Результат.СообщениеОбОшибке =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Ошибка аутентификации (%1).';ru='Ошибка аутентификации (%1).';en='Authentication failed (% 1).'"),
							РезультатОперации.КодСостояния);
				Иначе
					Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.';en='Authentication failed.'");
				КонецЕсли;
...
"Код состояния: %3;"
"Тело ответа: %4"
"Тело запроса: %5';ka='Не удалось получить тикет аутентификации в Менеджере сервиса (%1)."
"Некорректный ответ Менеджера сервиса."
"Ошибка при обработке ответа Менеджера сервиса:"
"%2"
"Код состояния: %3;"
...
"Тело запроса: %5'"),
			...
			Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
				Результат.СообщениеОбОшибке =
					НСтр("ka='Ошибка аутентификации. Некорректный ответ сервиса.';ru='Ошибка аутентификации. Некорректный ответ сервиса.';en='Authentication failed. Incorrect answer service.'");
			Иначе
				Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.';en='Authentication failed.'");
			КонецЕсли;
...
"%2';ka='Не удалось получить тикет аутентификации в Менеджере сервиса (%1)."
"Не удалось подключиться к Менеджеру сервиса."
"%2'"),
			URLСервиса,
...
"Тело запроса: %5'"),
		...
		Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			Результат.СообщениеОбОшибке =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Неизвестная ошибка сервиса аутентификации (%1).';ru='Неизвестная ошибка сервиса аутентификации (%1).';en='Unknown error authentication service (1%).'"),
					РезультатОперации.КодСостояния);
		Иначе
			Результат.СообщениеОбОшибке = НСтр("ka='აუტენტიფიკაციის შეცდომა';ru='Ошибка аутентификации.';en='Authentication failed.'");
		КонецЕсли;
