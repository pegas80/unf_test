...
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Если НЕ ВсеСеансыУдалены Тогда
		...
		ВызватьИсключение НСтр("ka='სეანსების წაშლა ვერ მოხერხდა.';ru='Не удалось удалить сеансы.';en='Failed to delete sessions.'");
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Если НЕ ВсеСоединенияРазорваны Тогда
		...
		ВызватьИсключение НСтр("ka='შეერთებების გაწყვეტა ვერ მოხერხდა.';ru='Не удалось разорвать соединения.';en='Unable to break the connection.'");
...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერში %1 უსაფრთხოების პროფილი %2 არ არის რეგისტრირებული!';ru='В кластере серверов %1 не зарегистрирован профиль безопасности %2!';en='In a cluster of servers% 1 was not registered with the safety profile of 2%!'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერ %1-ში უსაფრთხოების პროფილი %2 უკვე რეგისტრირებულია!';ru='В кластере серверов %1 уже зарегистрирован профиль безопасности %2!';en='In a cluster of servers% 1 is already registered with the safety profile of 2%!'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Функция ИдентификаторИнформационнойБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერ %1-ში საინფორმაციო ბაზა %2 არ არის რეგისტრირებული!';ru='В кластере серверов %1 не зарегистрирована информационная база %2!';en='In a cluster of servers% 1 is not registered data base% 2!'"), ИдентификаторКластера, ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
Функция ИдентификаторКластера(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერების კლასტერი პორტით %1 არ არის აღმოჩენილი';ru='Не обнаружен кластер серверов с портом %1';en='Not detected a cluster of servers with the port% 1'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
...
Функция ВыполнитьКоманду(Знач Шаблон, Знач ПараметрыАдминистрированияКластера, Знач ЗначенияПараметров = Неопределено)
	...
	#Если Сервер Тогда
		...
		Если БезопасныйРежим() <> Ложь Тогда
			ВызватьИсключение НСтр("ka='კლასტერის ადმინისტრირება უსაფრთხო რეჟიმში შეუძლებელია!';ru='Администрирование кластера невозможно в безопасном режиме!';en='Cluster Administration impossible in safe mode!'");
		КонецЕсли;
		
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			ВызватьИсключение НСтр("ka='სერვისის მოდელში გამოყენებითი საინფორმაციო ბაზის მიერ კლასტერის ადმინისტრირების ფუნქციის შესრულება დაუშვებელია!';ru='В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера!';en='The service model is unacceptable execution of the application cluster information database administration functions!'");
		КонецЕсли;
...
"- Для компьютеров с ОС Linux требуется установить пакет 1c-enterprise83-server*.';ka='Невозможно выполнить операцию администрирования кластера серверов по причине: файл %1 не найден!"
""
"Для администрирования кластера через сервер администрирования (ras) требуется установить на данном"
"компьютере клиент сервера администрирования (rac)."
"Для его установки:"
