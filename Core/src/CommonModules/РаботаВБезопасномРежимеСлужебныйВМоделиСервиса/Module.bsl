...
Функция РежимИсполненияВнешнегоМодуля(Знач ВнешнийМодуль) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='ფუნქცია არ არის განკუთვნილი საინფორმაციო ბაზაში მოსახმობად, რომელშიც მონაცემთა არეალების მიხედვით გაყოფა გამორთულია!';ru='Функция не предназначена для вызова в информационной базе, в которой выключено разделение по областям данных!';en='The function is not intended to call in the information database, which is off the division data fields!'");
	КонецЕсли;
...
Функция КлючРазрешения(Знач Разрешение) Экспорт
	...
	Если СтрДлина(Ключ) > 32 Тогда
		ВызватьИсключение НСтр("ka='გასაღების სიგრძის გადამეტება';ru='Превышение длины ключа';en='Excess length of the key'");
	КонецЕсли;
...
Функция СтрокаТаблицыРазрешений(Знач ТаблицаРазрешений, Знач Отбор, Знач ДобавитьПриОтсутствии = Истина) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='სტრიქონების უნიკალურობის დარღვევა შერჩევის მიხედვით ნებართვების ცხრილში %1';ru='Нарушение уникальности строк в таблице разрешений по отбору %1';en='Violation of unique rows in the table permits selection of% 1'"),
...
Процедура ЗаблокироватьРегистрыПредоставленныхРазрешений(Знач ПрограммныйМодуль = Неопределено, Знач ЗаблокироватьРежимыПодключенияВнешнихМодулей = Истина) Экспорт
	...
	Если Не ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='ტრანზაქცია არ არის აქტიური!';ru='Транзакция не активна!';en='The transaction is not active!'");
	КонецЕсли;
...
Процедура ДобавитьРазрешение(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля, Знач ТипВладельца, Знач ИдентификаторВладельца, Знач КлючРазрешения, Знач Разрешение, Знач ДополнениеРазрешения = Неопределено) Экспорт
	...
	Если Менеджер.Выбран() Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ka='საკვანძო ველების მიხედვით ნებართვების დუბლირება:"
"- პროგრამულიმოდულისტიპი: %1"
"- პროგრამულიმოდულისიდენტიფიკატორი: %2"
"- მფლობელისტიპი: %3"
"- მფლობელისიდენტიფიკატორი: %4"
"- ნებართვისგასაღები: %5.';ru='Дублирование разрешений по ключевым полям:"
...
"- ТипВладельца: %3"
"- ИдентификаторВладельца: %4"
"- КлючРазрешения: %5.';ka='Коллизия разрешений по ключам:"
"- ТипПрограммногоМодуля: %1"
"- ИдентификаторПрограммногоМодуля: %2"
"- ТипВладельца: %3"
"- ИдентификаторВладельца: %4"
...
"- ТипВладельца: %3"
"- ИдентификаторВладельца: %4"
"- КлючРазрешения: %5.';ka='არარსებული ნებართვის წაშლის მცდელობა:"
"- პროგრამულიმოდულისტიპი: %1"
"- პროგრამულიმოდულისიდენტიფიკატორი: %2"
"- მფლობელისტიპი: %3"
"- მფლობელისიდენტიფიკატორი: %4"
...
Функция РезультатОбработкиПакета(Знач ИдентификаторПакета) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ka='მოთხოვნების პაკეტი %1 ვერ მოიძებნა';ru='Не найден пакет запросов %1';en='Not found request packet% 1'"), ИдентификаторПакета);
...
Функция СостояниеОбработкиПакета(Знач ИдентификаторПакета)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='მოთხოვნების პაკეტი %1 ვერ მოიძებნა';ru='Не найден пакет запросов %1';en='Not found request packet% 1'"), ИдентификаторПакета);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтандартнаяОбработка Тогда
			...
			Иначе
				...
				ВызватьИсключение СтрШаблон(
					НСтр("ka='პროგრამული მოდული არ არის სერიალიზებული გასაღების მიხედვით:"
"- ტიპი: %1"
"- იდენტიფიკატორი: %2';ru='Не сериализован программный модуль по ключу:"
"- Тип: %1"
"- Идентификатор: %2'"),
					Выборка.ТипПрограммногоМодуля,
					Выборка.ИдентификаторПрограммногоМодуля);
