...
Функция РежимИсполненияВнешнегоМодуля(Знач ВнешнийМодуль) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='ფუნქცია არ არის განკუთვნილი საინფორმაციო ბაზაში მოსახმობად, რომელშიც მონაცემთა არეალების მიხედვით გაყოფა გამორთულია!';ru='Функция не предназначена для вызова в информационной базе, в которой выключено разделение по областям данных!'");
	КонецЕсли;
...
Функция КлючРазрешения(Знач Разрешение) Экспорт
	...
	Если СтрДлина(Ключ) > 32 Тогда
		ВызватьИсключение НСтр("ka='გასაღების სიგრძის გადამეტება';ru='Превышение длины ключа'");
	КонецЕсли;
...
Функция СтрокаТаблицыРазрешений(Знач ТаблицаРазрешений, Знач Отбор, Знач ДобавитьПриОтсутствии = Истина) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='სტრიქონების უნიკალურობის დარღვევა შერჩევის მიხედვით ნებართვების ცხრილში %1';ru='Нарушение уникальности строк в таблице разрешений по отбору %1'"),
...
Процедура ЗаблокироватьРегистрыПредоставленныхРазрешений(Знач ПрограммныйМодуль = Неопределено, Знач ЗаблокироватьРежимыПодключенияВнешнихМодулей = Истина) Экспорт
	...
	Если Не ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='ტრანზაქცია არ არის აქტიური!';ru='Транзакция не активна!'");
	КонецЕсли;
...
Функция РезультатОбработкиПакета(Знач ИдентификаторПакета) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ka='მოთხოვნების პაკეტი %1 ვერ მოიძებნა';ru='Не найден пакет запросов %1'"), ИдентификаторПакета);
...
Функция СостояниеОбработкиПакета(Знач ИдентификаторПакета)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='მოთხოვნების პაკეტი %1 ვერ მოიძებნა';ru='Не найден пакет запросов %1'"), ИдентификаторПакета);
