...
Функция Параметры() Экспорт
	...
	Если НЕ СохраненныеПараметры.Свойство("НазначениеРолей") Тогда
		ПредставлениеПараметра = НСтр("ka='ხელმიუწვდომელი როლები';ru='Недоступные роли';en='unavailable role'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ВсеРоли") Тогда
		ПредставлениеПараметра = НСтр("ka='ყველა როლი';ru='Все роли';en='All roles'");
	КонецЕсли;
...
"მომხმარებლების მუშაობის პარამეტრი არ არის შევსებული:"
"""%1"".'")
...
Функция НедоступныеРоли(Назначение = "ДляПользователей", Сервис = Неопределено) Экспорт
	...
	ПроверитьНазначение(Назначение,
		НСтр("ka='შეცდომა საერთო მოდულ მომხმარებლებისამსახურებრივიგანმშესრ-ის ფუნქცია მიუწვდომელიროლები-ში';ru='Ошибка в функции НедоступныеРоли общего модуля ПользователиСлужебныйПовтИсп.';en='Error in function module PolzovateliSluzhebnyyPovtIsp NedostupnyeRoli general.'"));
...
Функция ДеревоРолей(ПоПодсистемам = Истина, Назначение = "ДляПользователей") Экспорт
	...
	ПроверитьНазначение(Назначение,
		НСтр("ka='შეცდომა საერთო მოდულ მომხმარებლებისამსახურებრივიგანმშესრ-ის ფუნქცია როლებისხე-ში';ru='Ошибка в функции ДеревоРолей общего модуля ПользователиСлужебныйПовтИсп.';en='Error in function module PolzovateliSluzhebnyyPovtIsp DerevoRoley general.'"));
...
"- ""СовместноДляПользователейИВнешнихПользователей"".';ka='პარამეტრი ""დანიშნულება"" ""%1"" არაკორექტულად არის მითითებული."
""
"დასაშვებია მხოლოდ შემდეგი მნიშვნელობები:"
"- ""ადმინისტრატორებისთვის"","
"- ""მომხმარებლებისთვის"","
"- ""გარემომხმარებლებისთვის"","
"- ""ერთობლივადმომხმარებლებისთვისგარემომხმარებლებისთვის"".'"),
			Назначение);
	КонецЕсли;
