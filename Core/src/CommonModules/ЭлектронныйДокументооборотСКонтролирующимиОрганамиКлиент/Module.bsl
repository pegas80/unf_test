...
Процедура ОткрытьФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='ელექტრონულ დოკუმენტბრუნვასთან შეერთება შეუძლებელია: უფლებები არასაკმარისია.';ru='Подключение к электронному документообороту невозможно: недостаточно прав.';en='Connection to the electronic document can not be: are not allowed.'"));
...
Процедура ОткрытьФормуСотрудникаНаРеквизите(ОповещениеОЗавершении, ФормаВладелец, Сотрудник, ЗначениеРеквизита, ИмяРеквизита) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		ИначеЕсли ИмяРеквизита = "АдресРегистрации" Тогда
			...
			Параметры.Вставить("Заголовок",               НСтр("ka='მისამართის შეყვანა';ru='Ввод адреса';en='Enter address'"));
		...
		ИначеЕсли ИмяРеквизита = "Телефон" Тогда
			Параметры = Новый Структура;
			Параметры.Вставить("Заголовок",               НСтр("ka='ტელეფონის შეყვანა';ru='Ввод телефона';en='Enter phone'"));
			Параметры.Вставить("Представление", 		   ЗначениеРеквизита);
...
Процедура ОтправитьПояснение(
	...
	Если НЕ СведенияОЗаполнении.ЕстьСтрокиДляОтправки Тогда
		Если СведенияОЗаполнении.ЕстьИзмененияПоНДС Тогда
			ТекстПредупреждения = НСтр("ka='Все строки, в которых указан ответ, содержат изменения в сумме начисленного НДС."
"Отправка пояснения невозможна, внесите исправления в учетные данные и сформируйте корректирующую декларацию.';ru='Все строки, в которых указан ответ, содержат изменения в сумме начисленного НДС."
...
"Отправка пояснения невозможна.';ka='Нет ни одной строки, в которой указан ответ."
"Отправка пояснения невозможна.'");
		КонецЕсли;
		ПоказатьПредупреждение( , ТекстПредупреждения, , НСтр("ka='Невозможно отправить пояснение';ru='Невозможно отправить пояснение';en='It is impossible to send an explanation'"));
		Возврат;
	КонецЕсли;
	...
	Если СведенияОЗаполнении.ЕстьИзмененияПоНДС Тогда
		МассивСообщений.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='ყურადღება!';ru='Внимание!';en='Attention!'"), , Новый Цвет(225, 40, 40)));
...
"Строки, содержащие изменения по начисленному НДС, в пояснение включены не будут.';ru='"
"Изменилась сумма начисленного НДС. "
"Внесите исправления в учетные данные и сформируйте корректирующую декларацию."
"Строки, содержащие изменения по начисленному НДС, в пояснение включены не будут.'")));
	...
	Если СведенияОЗаполнении.ЕстьНеОтвеченныеСтроки Тогда
		...
		КонецЕсли;
		МассивСообщений.Добавить(Новый ФорматированнаяСтрока(
								НСтр("ka='В ФНС будут отправлены только те строки пояснения, по которым указан ответ."
"Уточнение по остальным строкам требования можно будет отправить позже в другом пояснении.';ru='В ФНС будут отправлены только те строки пояснения, по которым указан ответ."
"Уточнение по остальным строкам требования можно будет отправить позже в другом пояснении.'")));
	...
	Если МассивСообщений.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, Новый ФорматированнаяСтрока(МассивСообщений), Кнопки, , , НСтр("ka='Отправка пояснения';ru='Отправка пояснения';en='sending an explanation'"));
	Иначе
...
Процедура ПроверитьВладельцаПриОткрытииПрисоединенныхФайловИзСпискаОтчетов(ПараметрыПроверки) Экспорт
	...
	Если ТипЗнч(ПараметрыПроверки.Владелец) = Тип("СправочникСсылка.МакетыПенсионныхДел") Тогда
		...
		ПараметрыПроверки.ТекстПредупреждения = НСтр("ru='Присоединение файлов к ""Макету пенсионного дела"" должно выполняться из основной формы.';en='Attaching files to the ""Layout pension business"" must be carried out from the main form.'");
	КонецЕсли;
