...
Процедура ОткрытьФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='ელექტრონულ დოკუმენტბრუნვასთან შეერთება შეუძლებელია: უფლებები არასაკმარისია.';ru='Подключение к электронному документообороту невозможно: недостаточно прав.'"));
...
Процедура ОткрытьФормуСотрудникаНаРеквизите(ОповещениеОЗавершении, ФормаВладелец, Сотрудник, ЗначениеРеквизита, ИмяРеквизита) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		ИначеЕсли ИмяРеквизита = "АдресРегистрации" Тогда
			...
			Параметры.Вставить("Заголовок",               НСтр("ka='მისამართის შეყვანა';ru='Ввод адреса'"));
		...
		ИначеЕсли ИмяРеквизита = "Телефон" Тогда
			Параметры = Новый Структура;
			Параметры.Вставить("Заголовок",               НСтр("ka='ტელეფონის შეყვანა';ru='Ввод телефона'"));
			Параметры.Вставить("Представление", 		   ЗначениеРеквизита);
...
"Отправка пояснения невозможна.'");
		КонецЕсли;
		ПоказатьПредупреждение( , ТекстПредупреждения, , НСтр("ka='Невозможно отправить пояснение';ru='Невозможно отправить пояснение'"));
		Возврат;
	КонецЕсли;
	...
	Если СведенияОЗаполнении.ЕстьИзмененияПоНДС Тогда
		МассивСообщений.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='ყურადღება!';ru='Внимание!'"), , Новый Цвет(225, 40, 40)));
...
"Уточнение по остальным строкам требования можно будет отправить позже в другом пояснении.'")));
	...
	Если МассивСообщений.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, Новый ФорматированнаяСтрока(МассивСообщений), Кнопки, , , НСтр("ka='Отправка пояснения';ru='Отправка пояснения'"));
	Иначе
