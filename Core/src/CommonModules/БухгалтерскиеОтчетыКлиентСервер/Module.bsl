...
Функция ПолучитьПредставлениеПериода(НачалоПериода = '00010101', КонецПериода = '00010101', ТолькоДаты  = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(КонецПериода) Тогда 
		Если КонецПериода >= НачалоПериода Тогда
			ТекстПериод = ?(ТолькоДаты, "", " " + НСтр("ru='за';en='per'")+ " ") 
				+ ПредставлениеПериода(НачалоДня(НачалоПериода), КонецДня(КонецПериода), Нстр("ka='Л=ka; ФП = Истина';ru='ФП = Истина';en='OP = True'"));
		Иначе
			ТекстПериод = "";
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(НачалоПериода) И Не ЗначениеЗаполнено(КонецПериода) Тогда
		ТекстПериод = ?(ТолькоДаты, "", " " + НСтр("ru='за';en='per'")+ " ") 
			+ ПредставлениеПериода(НачалоДня(НачалоПериода), КонецДня(Дата(3999, 11, 11)), Нстр("ka='Л=ka; ФП = Истина';ru='ФП = Истина';en='OP = True'"));
...
Функция ПолучитьНаименованиеЗаданияВыполненияОтчета(Форма, ИмяОтчета = "") Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Выполнение отчета: %1';en='Execution report:% 1'");
...
Процедура ИзменитьЗаголовокКнопкиПанельНастроек(Кнопка, ВидимостьПанелиНастроек) Экспорт
	...
	Если ВидимостьПанелиНастроек Тогда
		Кнопка.Заголовок = НСтр("ru='Скрыть настройки';en='Hide settings'");
	Иначе
		Кнопка.Заголовок = НСтр("ru='Показать настройки';en='Display settings'");
	КонецЕсли;
...
Функция ОтчетПоддерживаетУниверсальныеНастройки(ИмяОтчета) Экспорт
	...
	//СписокОтчетов.Добавить("КарточкаСубконто"); // Kuzmich, 16.01.2022, Добавление отчетов КарточкаСубконто,ОборотыМеждуСубконто,ОборотыСубконто
	//СписокОтчетов.Добавить("КарточкаСчета"); // Kuzmich, 16.01.2022, Добавление отчетов КарточкаСубконто,ОборотыМеждуСубконто,ОборотыСубконто
	СписокОтчетов.Добавить("ОборотноСальдоваяВедомостьРегл");
	СписокОтчетов.Добавить("ОборотноСальдоваяВедомостьПоСчету");
	//СписокОтчетов.Добавить("ОборотыМеждуСубконто"); // Kuzmich, 16.01.2022, Добавление отчетов КарточкаСубконто,ОборотыМеждуСубконто,ОборотыСубконто
	...
	СписокОтчетов.Добавить("ОборотыСубконто"); // Kuzmich, 17.01.2022, Добавление отчетов КарточкаСубконто,ОборотыМеждуСубконто,ОборотыСубконто
