...
Функция ЗаголовокНадписиВзаиморасчетов(Контрагент, СуммаДолга = 0) Экспорт
	...
	Если Сумма < 0 Тогда
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='ჩვენი ვალი';ru='Мы должны'") + " ", КрупныйШрифт));
		Сумма = -Сумма;
	Иначе
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='დავალიანება ჩვენს მიმართ';ru='Должен нам'") + " ", КрупныйШрифт));
	КонецЕсли;
