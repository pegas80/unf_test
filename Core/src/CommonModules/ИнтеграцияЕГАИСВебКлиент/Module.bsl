...
Процедура ФормированиеHTTPЗапроса_ПослеПодключенияКомпоненты(Подключено, Контекст) Экспорт
	...
	Если Подключено Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ka='Ошибка инициализации компоненты HTTP-запросов.';ru='Ошибка инициализации компоненты HTTP-запросов.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
"Перейти к установке данной компоненты?'");
	...
	Иначе
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.'"));
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПередУстановкойКомпоненты(РезультатВопроса, Контекст) Экспорт
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Не установлена компонента HTTP-запросов.';ru='Не установлена компонента HTTP-запросов.'"));
	Иначе
...
Процедура ФормированиеHTTPЗапроса_ПослеУстановкиПараметровПрокси(РезультатВыполнения, Параметры, Контекст) Экспорт
	...
	Если НЕ РезультатВыполнения Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Ошибка установки настроек прокси-сервера.';ru='Ошибка установки настроек прокси-сервера.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПослеДобавленияЗаголовка(РезультатВыполнения, Параметры, Контекст) Экспорт
	...
	Если НЕ РезультатВыполнения Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Ошибка добавления заголовка запроса.';ru='Ошибка добавления заголовка запроса.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПослеДобавленияЗаголовков(Контекст)
	...
	Иначе
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Неверный тип HTTP-запроса.';ru='Неверный тип HTTP-запроса.'"));
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_Завершение(КодСостояния, Параметры, Контекст) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
		ТекстОшибки = НСтр("ka='Ошибка при выполнении %1-запроса по адресу %2';ru='Ошибка при выполнении %1-запроса по адресу %2'");
