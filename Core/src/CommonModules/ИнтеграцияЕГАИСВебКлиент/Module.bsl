...
Процедура ФормированиеHTTPЗапроса_ПослеПодключенияКомпоненты(Подключено, Контекст) Экспорт
	...
	Если Подключено Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ka='Ошибка инициализации компоненты HTTP-запросов.';ru='Ошибка инициализации компоненты HTTP-запросов.';en='Error initializing the HTTP-requests component.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
"Перейти к установке данной компоненты?';ka='Для поддержки интеграции с ЕГАИС необходима установка компоненты HTTP-запросов."
...
Процедура ФормированиеHTTPЗапроса_ПередУстановкойКомпоненты(РезультатВопроса, Контекст) Экспорт
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Не установлена компонента HTTP-запросов.';ru='Не установлена компонента HTTP-запросов.';en='Do not set the HTTP-requests component.'"));
	Иначе
...
Процедура ФормированиеHTTPЗапроса_ПослеУстановкиПараметровПрокси(РезультатВыполнения, Параметры, Контекст) Экспорт
	...
	Если НЕ РезультатВыполнения Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Ошибка установки настроек прокси-сервера.';ru='Ошибка установки настроек прокси-сервера.';en='Failed to set the proxy settings.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПослеДобавленияЗаголовка(РезультатВыполнения, Параметры, Контекст) Экспорт
	...
	Если НЕ РезультатВыполнения Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Ошибка добавления заголовка запроса.';ru='Ошибка добавления заголовка запроса.';en='Error adding a request header.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПослеДобавленияЗаголовков(Контекст)
	...
	Иначе
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Неверный тип HTTP-запроса.';ru='Неверный тип HTTP-запроса.';en='Wrong type HTTP-request.'"));
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_Завершение(КодСостояния, Параметры, Контекст) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
		ТекстОшибки = НСтр("ka='Ошибка при выполнении %1-запроса по адресу %2';ru='Ошибка при выполнении %1-запроса по адресу %2';en='Execution Error% 1-request at 2%'");
