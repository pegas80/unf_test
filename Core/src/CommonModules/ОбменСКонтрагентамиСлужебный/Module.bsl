...
Процедура ОбменСКонтрагентамиВладелецПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ka='არსებობს ელექტრონული დოკუმენტი. დოკუმენტის საკვანძო რეკვიზიტების შეცვლა აკრძალულია.';ru='Существует электронный документ. Изменение ключевых реквизитов документа запрещено.';en='There is an electronic document. Changing the key details of the document is prohibited.'");
...
Процедура ЗаполнитьЗначениеРеквизитаОрганизацияПриОдноФирменномУчете(Источник, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ИспользуетсяНесколькоОрганизаций И ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", Источник.Метаданные())
		...
		Если Не ЗначениеЗаполнено(Источник.Организация) Тогда
			ВызватьИсключение НСтр("ka='Не удалось определить организацию.';ru='Не удалось определить организацию.';en='Unable to determine the organization.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РегламентноеПолучениеНовыхЭД() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение
			НСтр("ka='სერვისის რეჟიმში სარეგლამენტო დავალების მუშაობა არ არის გათვალისწინებული.';ru='Работа регламентного задания в режиме сервиса не предусмотрена.';en='Work scheduled jobs are not available in the service mode.'");
	КонецЕсли;
	...
	Текст = НСтр("ka='ახალი ელ.დოკუმენტების მიღებით დაწყებულია სარეგლამენტო დავალება.';ru='Начато регламентное задание по получению новых эл.документов.';en='Initiated routine task of getting new el.dokumentov.'");
	...
	Попытка
		...
		ШаблонСообщения = НСтр("ka='სარეგლამენტო დავალება დასრულებულია."
"მიღებულია პაკეტი: %1. განფუთულია: %2.';ru='Закончено регламентное задание."
"Получено пакетов: %1. Распаковано: %2.'");
	...
	Исключение
		...
		Операция = НСтр("ka='ელექტრონული დოკუმენტების სარეგლამენტო მიღება';ru='Регламентное получение электронных документов';en='Routine receive electronic documents'");
		
		ШаблонСообщения = НСтр("ka='შესრულების პროცესში შეცდომა მოხდა: %1';ru='При процессе выполнении произошла ошибка: %1';en='If an error was encountered:% 1'");
...
Процедура РегламентнаяОтправкаОформленныхЭД() Экспорт
	...
	Текст = НСтр("ka='დაწყებულია გაფორმებული ელ.დოკუმენტების გაგზავნის სარეგლამენტო დავალება.';ru='Начато регламентное задание по отправке оформленных эл.документов.';en='Initiated routine task of sending decorated el.dokumentov.'");
	...
	Попытка
		...
		Текст = НСтр("ka='სარეგლამენტო დავალება დასრულებულია. გაგზავნილია პაკეტი: %КоличествоПакетов%.';ru='Закончено регламентное задание. Отправлено пакетов: %КоличествоПакетов%.';en='Finished scheduled jobs. Sent packets:% KolichestvoPaketov%.'");
		Текст = СтрЗаменить(Текст, "%КоличествоПакетов%", КоличествоПакетов);
		ЭлектронноеВзаимодействиеСлужебный.ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(Текст, 4, УровеньЖурналаРегистрации.Информация);
	Исключение
		ОписаниеОшибки = НСтр("ka='გაფორმებული ელ.დოკუმენტების სარეგლამენტო გაგზავნისას შეცდომა წარმოიშვა. დამატებითი აღწერილობა: %ДополнительноеОписание%';ru='Во время регламентной отправки оформленных эл.документов произошла ошибка."
...
Процедура ПроверитьНаличиеНовыхЭД() Экспорт
	...
	ВидОперации = НСтр("ka='ახალი ელექტრონული დოკუმენტების შესახებ ინფორმაციის მიღება';ru='Получение информации о новых электронных документах';en='Getting information about the new electronic documents'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა.';ru='Ошибка выполнения команды сервиса.';en='Error executing service commands.'");
		...
		Если Не Ответ.КодСостояния = 200 Тогда
			...
			ШаблонСообщения = НСтр("ka='სერვისის პასუხის კოდი %1';ru='Код ответа сервиса %1';en='Answer Service Code% 1'");
...
"%1';ka='ტესტი. FTP შეერთების დაყენების შემოწმება. %1'");
	Иначе
		ШаблонСообщения = "%1";
	КонецЕсли;
...
"%1';ka='გამავალი დოკუმენტების კატალოგის შემოწმება."
"%1'");
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, РезультатТеста);
	ТекстСообщения = НСтр("ka='პროფილ %1-ის მიხედვით გაცვლის ტესტი.';ru='Тест обмена по профилю %1.';en='sharing test the profile% 1.'") + " " + ТекстСообщения;
...
"%1';ka='გამავალი დოკუმენტების კატალოგში ფაილების ჩაწერის და წაკითხვის შემოწმება."
"%1'");
...
"%1';ka='შემომავალი დოკუმენტების კატალოგის შემოწმება."
"%1'");
	Попытка
...
"%1';ka='შემომავალი დოკუმენტების კატალოგში ფაილების ჩაწერის და წაკითხვის შემოწმება."
"%1'");
		ПроверитьФайл(ШаблонСообщения, FTPСоединение, ТекстОшибки);
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			РезультатТеста = ТекстОшибки;
...
Функция СформироватьНовыйПакетЭД(СтруктураПараметров) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის ახალი პაკეტის შექმნა';ru='создание нового ПакетаЭД';en='creation of a new PaketaED'"),
																					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ОбновитьСтатусыДокументовПакетаЭД(ПакетЭД, НовыйСтатусПакетаЭД, ДатаИзменения) Экспорт
	...
	Если ПакетЭД.ЭлектронныеДокументы.Количество() = 0 Тогда
		...
		ШаблонОшибки = НСтр("ka='დოკუმენტი %1 არ გაგზავნილა. დაწვრილებით, იხ, რეგისტრაციის ჟურნალში';ru='Документ %1 не был отправлен. Подробнее см. Журнал регистрации';en='% Document 1 has not been sent. For details, see. The log'");
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
					ПакетЭД );
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ka='ედ-ის პაკეტის გაგზავნა და ედ-ის სტატუსების განახლება';ru='Отправка пакета ЭД и обновление статусов ЭД';en='Sending a package ED and ED status update'"),
			ТекстОшибки,
			ТекстСообщения);
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ka='ედ-ის პაკეტის სტატუსის შეცვლა და ედ-ის სტატუსების განახლება';ru='смена статуса пакета ЭД и обновление статусов ЭД';en='ED change package status and update the status of ED'"), ТекстОшибки, ТекстСообщения);
	КонецПопытки;
...
Функция ПолучитьПредставлениеЭД(СсылкаНаФайлЭД) Экспорт
	...
	Иначе
		...
		ИначеЕсли РеквизитыЭД.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			ТекстЗаголовка = НСтр("ru='Произвольный документ %1 от %2';en='An arbitrary document by% 1% 2'");
		...
		ИначеЕсли РеквизитыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД
			...
			Представление = НСтр("ru='Информация продавца №';en='Information seller number'") + " " + РеквизитыЭД.НомерДокументаОтправителя + ТекстДаты;
		Иначе
			Представление = "" + РеквизитыЭД.ВидЭД + " № " + РеквизитыЭД.НомерДокументаОтправителя + ТекстДаты
			+ ТекстВерсии;
		КонецЕсли;
...
Функция ОпределитьПредставлениеЭД(ВидЭД, СтруктураПараметров) Экспорт
	...
	Если ВидЭД <> Перечисления.ВидыЭД.СчетФактура И ВидЭД <> Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		ШаблонТекстаВерсии = НСтр("ka=' (ვერსია %1)';ru='(версия %1)';en='(Version% 1)'");
	...
	Если СтруктураПараметров.Свойство("ТипЭД", ТипЭД)
		...
		Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
			...
			ТекстВидЭД = НСтр("ka='მყიდველის ინფორმაცია';ru='Информация покупателя';en=""buyer's information""");
		...
		ИначеЕсли ЭтоИзвещениеОПолучении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ka='უწყება მიღების თაობაზე';ru='Извещение о получении';en='Notice of receipt'");
		ИначеЕсли ЭтоПодтверждение(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'");
			
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДПЭСФ
				ИЛИ ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДП Тогда
				ТекстВидЭД = ТекстВидЭД + " " + НСтр("ru='даты получения';en='date of receipt'");
			Иначе
				ТекстВидЭД = ТекстВидЭД + " " + НСтр("ru='даты отправки';en='date of dispatch'");
			КонецЕсли;
			
		ИначеЕсли ЭтоУведомлениеОбУточнении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ka='ანულირების წინადადება';ru='Предложение об аннулировании';en='The proposal for cancellation'");
	...
	Иначе
		...
		Если ВидЭД = Перечисления.ВидыЭД.СчетФактура Тогда
			ТекстВидЭД = НСтр("ka='Счет-фактура';ru='Счет-фактура';en='Счет-фактура'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД Тогда
				ТекстВидЭД = НСтр("ka='გამყიდველის ინფორმაცია';ru='Информация продавца';en='Seller information'");
			КонецЕсли;
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
			ТекстВидЭД = НСтр("ka='საკორექტირებო ანგარიშფაქტურა';ru='Корректировочный счет-фактура';en='The correction invoice'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.КСЧФДИСУКД Тогда
				ТекстВидЭД = НСтр("ka='გამყიდველის ინფორმაცია';ru='Информация продавца';en='Seller information'");
			КонецЕсли;
			
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ТОРГ12 Тогда
			ТекстВидЭД = НСтр("ka='სასაქონლო ზედნადები (ТОРГ-12)';ru='Товарная накладная (ТОРГ-12)';en='Waybill (bargaining-12)'");
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктВыполненныхРабот Тогда
			ТекстВидЭД = НСтр("ka='აქტი';ru='Акт';en='Act'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктНаПередачуПрав Тогда
			ТекстВидЭД = НСтр("ka='აქტი უფლებების გადასაცემად';ru='Акт на передачу прав';en='Act on the transfer of rights'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД Тогда
				ТекстВидЭД = НСтр("ka='გამყიდველის ინფორმაცია';ru='Информация продавца';en='Seller information'");
			КонецЕсли;
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.СчетНаОплату Тогда
			ТекстВидЭД = НСтр("ka='ანგარიში';ru='Счет';en='Score'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗаказТовара Тогда
			ТекстВидЭД = НСтр("ka='საქონლის შეკვეთა';ru='Заказ товара';en='order goods'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтветНаЗаказ Тогда
			ТекстВидЭД = НСтр("ka='პასუხი შეკვეთაზე';ru='Ответ на заказ';en='Order Response'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПрайсЛист Тогда
			ТекстВидЭД = НСтр("ka='პრაის-ლისტი';ru='Прайс-лист';en='Price list'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
			ТекстВидЭД = НСтр("ka='საქონლის კატალოგი';ru='Каталог товаров';en='Catalog'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ka='ანგარიშგება საკომისიო საქონლის გაყიდვის თაობაზე';ru='Отчет о продажах комиссионного товара';en='Report about sales commission goods'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ka='ანგარიშგება საკომისიო საქონლის ჩამოწერის თაობაზე';ru='Отчет о списании комиссионного товара';en='Report on the write-off of commission goods'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ka='ინტერკამპანის გადაცემა';ru='Передача Интеркампани';en='Transfer Interkampani'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ВозвратТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ka='თავისუფალი დოკუმენტი';ru='Произвольный документ';en='Custom document'");
...
"Документ не обработан, так как содержит невалидные подписи.';ka='ელექტრონული დოკუმენტის დამუშავება %1."
"დოკუმენტი არ არის დამუშავებული, ვინაიდან არავალიდურ ხელმოწერებს შეიცვას.'");
...
"%1';ka='შეცდომა ელექტრონული ფოსტის სერვერიდან შეტყობინების მიღებისას. %1'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ТекстОшибки);
			
			ТекстСообщения = НСтр("ka='შეცდომა ახალი ელ.დოკუმენტების მიღებისას.';ru='Ошибка при получении новых эл.документов.';en='Error obtaining new el.dokumentov.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ka='ახალი ელ.დოკუმენტების მიღება';ru='Получение новых эл.документов';en='Getting new el.dokumentov'"), ТекстОшибки, ТекстСообщения);
			Продолжить;
		КонецПопытки;
		...
		Для Каждого Сообщение Из НаборСообщений Цикл
			...
			Если СтрНайти(Сообщение.Тема, НСтр("ru='Подтверждение получения пакета электронных документов.';en='Acknowledgment of receipt of package of electronic documents.'")) Тогда
			...
			Для Каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
				...
				Если СтруктураЭД.Свойство(КлючЗначение.Ключ, Значение) Тогда
					Если НЕ ЗначениеЗаполнено(Значение) Тогда
						Текст = НСтр("ru='Значение параметра <%1> не заполнено.';en='The parameter value <1%> to be answered.'");
					...
					КонецЕсли;
				Иначе
					Текст = НСтр("ru='Не передан обязательный параметр <%1>.';en='Not passed a required parameter <% 1>.'");
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КлючЗначение.Ключ);
					ТекстОшибки = ?(ЗначениеЗаполнено(ТекстОшибки), ТекстОшибки + Символы.ПС + Текст, Текст);
				КонецЕсли;
			КонецЦикла;
...
"ИНН: %3"
"КПП: %4"
"Статус: %5.';ka='გაცვლა ედბ-ის გამართვების პროფილით %1."
"ბაზაში კონტრაგენტი არ მოიძებნა:"
"სახელწოდება: %2"
"გსნ: %3"
"აამკ: %4"
"სტატუსი: %5.'");
				...
				Иначе
					ОписаниеОшибки = ?(ЗначениеЗаполнено(НовыеПриглашения.ОписаниеОшибки),
						НовыеПриглашения.ОписаниеОшибки, НСтр("ka='შეცდომა';ru='Ошибка';en='Mistake'"));
...
"ИНН: %5"
"КПП: %6"
"Статус: %7.';ka='გაცვლა ედბ-ის გამართვების პროფილით %1."
"%2 მოწვევაში კონტრაგენტისთვის:"
"სახელწოდება: %3"
"ეხ-ის მისამართი: %4"
"გსნ: %5"
"აამკ: %6"
"სტატუსი: %7.'");
			...
			Если НовыеПриглашения.СтатусУчастника = Перечисления.СтатусыУчастниковОбменаЭД.Присоединен Тогда
				...
				Если Выборка.Следующий() Тогда
					...
					НастройкаЭДО.Комментарий           = НСтр("ka='##ავტომატურად მოხსნილია სტატუსი ""მიერთებულია"". უკვე არსებობს ედბ-ის გამართვა არჩეული კონტრაგენტის მიხედვით.';ru='##Автоматически снят статус ""присоединен"". Уже существует настройка ЭДО по выбранному контрагенту.';en='## automatically removed the status ""connected"". There is already setting the EDM on selected counterparty.'");
				КонецЕсли;
			...
			ТекстОписанияОшибки = "";
			Если Ошибка Тогда
				ШаблонОписанияОшибки = НСтр("ka='%1. გაიმეორეთ გაგზავნა.';ru='%1. Повторите отправку.';en='%one. Resend.'");
		...
		Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
			...
			Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					КонецЕсли;
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД';en='Unpacking ED Pack'"),
...
Функция КорректноеИмяФайла(Знач СтрИмяФайла, УдалятьНекорректные = Ложь) Экспорт
	...
	Если Не Результат Тогда
		Текст = НСтр("ka='ფაილის სახელში არ უნდა იყოს შემდეგი სიმბოლოები: %1"
"ფაილის სახელი: %2';ru='В имени файла не должно быть следующих символов: %1"
"Имя файла: %2'");
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если СтруктураОбмена.Свойство("МассивФайлов") И СтруктураОбмена.МассивФайлов.Количество() > 0 Тогда
			...
			Если Не АдресАрхива = Неопределено Тогда
				...
				ПараметрыФайла.Вставить("Автор", Пользователи.АвторизованныйПользователь());
				ПараметрыФайла.Вставить("ВладелецФайлов", НовыйЭлектронныйДокумент); 
				ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ka='დამატებითი ფაილები';ru='Дополнительные файлы';en='additional files'"));
...
Функция ЗагрузитьСтрокуXMLВДеревоЗначений(СтрокаXML, ОписаниеОшибки = Неопределено, ЧтениеXML = Неопределено, Знач ТекущийУзел = Неопределено) Экспорт
	...
	Если ПерваяИтерация Тогда
		...
		Исключение
			ОписаниеОшибки = НСтр("ru='Ошибка разбора XML:';en='XML Parsing Error:'") + " " + ИнформацияОбОшибке().Описание + ".";
			Возврат Неопределено;
		КонецПопытки;
	КонецЕсли;
...
Функция БилетНаСайтПоддержки(ПоказыватьОшибки = Истина) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если Не МодульТарификация.ЗарегистрированаЛицензияБезлимитнойУслуги(
			...
			ТекстЗаголовкаСообщения = НСтр("ru='При аунтификациии на портале 1С:ИТС возникла ошибка:';en='When auntifikatsiii portal 1C: ITS error occurred:'");
			ОписаниеОшибки = НСтр("ru='Услуга ""Обмен электронными документами"" не подключена.';en='Service ""electronic document exchange"" is not connected.'");
			ТекстСообщения = "";
...
Процедура ОтправитьРегистрационныйПакет1СЭДО(Знач Файл, ПараметрыАутентификации, ЕстьОшибки) Экспорт
	...
	Исключение
		...
		ШаблонЗаголовкаСообщения = НСтр("ka='რეგისტრაციის შესახებ განცხადების გაგზავნისას: %1 სერვის 1С-ედბ-ზე შეცდომა წარმოიშვა:';ru='При отправке заявления о регистрации: %1 на сервисе 1С-ЭДО возникла ошибка:';en='When you send an application for registration: 1% on 1C-service EDC error occurred:'");
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			Если МассивПараметровОшибки.Количество() >= 7 Тогда
				...
				ШаблонОшибки = НСтр("ka='%1: %2.';ru='%1: %2.';en='%12.'");
...
Функция ПолучитьФайлОтветаОтСервераЭДО(АдресРесурса, ПараметрЗапросаОтветаОтСервера = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстЗаголовкаСообщения = НСтр("ka='სერვის 1C-ედბ-სთან მუშაობისას შეცდომა წარმოიშვა:';ru='При работе с сервисом 1С-ЭДО возникла ошибка:';en='When working with 1C-EDI service error occurred:'");
...
Функция ЕстьДоступКВебСервису1CЭДО(ВыводитьСообщения = Ложь) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ka='%1"
"სერვისი 1С-ედბ მიუწვდომელია. მოგვიანებით სცადეთ.';ru='%1"
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт
	...
	Исключение
		...
		Операция = НСтр("ka='1С-ედბ-სთან კონტრაგენტის შეერთების შემოწმება კონტრაგენტის ფორმიდან.';ru='Проверка подключения контрагента к 1С-ЭДО из формы контрагента.';en='Check the connection to the counterparty 1C-EDI form of counterparty.'");
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор)
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='სერვის 1C-ედბ-ზე კონტრაგენტების მდგომარეობის შემოწმებისას შეცდომა წარმოიშვა:';ru='При проверке состояния контрагентов на сервисе 1С-ЭДО возникла ошибка:';en='When checking the status of the service contractors 1C-EDM error occurred:'");
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		Операция = НСтр("ka='1C-ედბ-სთან კონტრაგენტების შეერთების შემოწმების შედეგების ჩაწერა რეგისტრში.';ru='Запись результатов проверки подключения контрагентов к 1С-ЭДО в регистр.';en='Recording connection test results counterparties to 1C-EDM in the register.'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробныйТекстОшибки);
	КонецПопытки;
...
Функция УведомленияПодключены(ИдентификаторОрганизации, Соединение = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ka='შეტყობინებების მდგომარეობის შესახებ ინფორმაციის მიღება';ru='Получение информации о состоянии уведомлений';en='Getting information about the status of notifications'");
...
Процедура ОбработатьПодтверждениеПолучения(Сообщение, ЭтоФайл = Ложь)
	...
	Иначе
		Если СтрНайти(Сообщение.Тема, НСтр("ru='Подтверждение получения пакета электронных документов';en='Acknowledgment of receipt of package of electronic documents'")) > 0 Тогда
...
Процедура ОтправитьПодтверждениеПоПакету(ПакетЭД, РесурсОтправителя, АдресОтправителя, ТипОтправки)
	...
	Если ЗначениеЗаполнено(ВыборкаПрисоединенныхФайлов) И ВыборкаПрисоединенныхФайлов.Следующий() Тогда
		Если ТипОтправки = Перечисления.СпособыОбменаЭД.ЧерезЭлектроннуюПочту Тогда
			...
			ПараметрыОтправки.Вставить("Тема",     НСтр("ru='Подтверждение получения пакета электронных документов:';en='Acknowledgment of receipt of package of electronic documents:'") + " " + ИмяПакетаЭД);
...
"%1';ka='შეცდომა ელექტრონული ფოსტის სერვერზე შეტყობინების გაგზავნისას. %1'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ТекстОшибки);
				
				ТекстСообщения = НСтр("ka='შეცდომა ელ.დოკუმენტების გაგზავნისას.';ru='Ошибка при отправке эл.документов.';en='Error sending el.dokumentov.'");
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ელ. დოკუმენტების გაგზავნა';ru='Отправка эл.документов';en='sending el.dokumentov'"),
				ТекстОшибки,
				ТекстСообщения);
			КонецПопытки;
...
Функция НеобходимоПолучить(Сообщение, СтруктураПараметров)
	...
	Если Сообщение.Вложения.Количество() = 1 Тогда 
		...
		Если СтрНайти(Сообщение.Тема, НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'")) Тогда
			Возврат Истина;
		КонецЕсли;
		
		Если СтрНайти(Сообщение.Тема, НСтр("ka='ელ.დოკუმენტების გაცვლა:';ru='Обмен эл.документами:';en='Exchange el.dokumentami:'")) Тогда
...
Функция НеобходимоПолучитьДвоичныеДанные(ДвоичныеДанные, ИмяФайла, СтруктураПараметров)
	...
	Если НЕ ФайлКарточки = Неопределено Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='ფაილ %1-დან მონაცემთა წაკითხვისას შეცდომა წარმოიშვა: %2.';ru='Возникла ошибка при чтении данных из файла %1: %2.';en='An error occurred while reading data from the file% 1% 2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ФайлКарточки.ПолноеИмя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='Чтение ЭД.';ru='Чтение ЭД.';en='ED reading.'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ТекстСообщения);
		КонецПопытки;
...
Процедура ОбновитьСтатусыЭД(МассивДокументов, ПризнакПодписи)
	...
	Для Каждого ЭлектронныйДокумент Из МассивДокументов Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის მომზადება გასაგზავნად';ru='подготовка к отправке ЭД';en='Preparation for sending ED'"),
																						ТекстОшибки,
																						ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗаписатьДатуПолучения(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის მიღების თარიღის ჩანაწერი';ru='запись даты получения ЭД';en='the date of receipt of the record of ED'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ЗаписатьДатуОтправки(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის გაგზავნის თარიღის ჩანაწერი';ru='запись даты отправки ЭД';en='Record the date of dispatch of ED'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Функция СоздатьПакетЭД(РеквизитыДокумента, МассивЭД, ТекстСообщения = "")
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='ედ-ის პაკეტის ჩამოყალიბება';ru='формирование пакета ЭД';en='forming package ED'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ПрисоединенныйФайл(ВладелецФайла, ТипЭлементаВерсииЭД = Неопределено,СообщатьОбОшибке = Ложь) Экспорт
	...
	ИначеЕсли СообщатьОбОшибке Тогда
		...
		ТекстОшибки = НСтр("ru='Не обнаружен присоединенный файл документа %1';en='Not found attached document file% 1'");
...
Функция СписокОперацийВидаЭД(ВидЭД, ДобавлятьПредопределенныеСпособы = Ложь, Префикс = "") Экспорт 
	...
	Если ДобавлятьПредопределенныеСпособы Тогда
		Если СписокОпераций.Количество() = 0 Тогда
			СписокОпераций.Добавить("Автоматически", НСтр("ka='ავტომატურად';ru='Автоматически';en='Automatically'"),Истина);
		КонецЕсли;
		
		СписокОпераций.Добавить("Вручную", НСтр("ru='Вручную в форме электронного документа';en='Hand in the form of an electronic document'"));
	КонецЕсли;
...
Функция ПолучитьОжидаемоеДействиеКонтрагента(Строка,ВладелецФайла)
	...
	Если Строка.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		Если Строка.ВидЭД = Перечисления.ВидыЭД.Подтверждение И НЕ ЗначениеЗаполнено(Строка.Ссылка)
			...
			ВозвращаемоеЗначение = НСтр("ka='ედბ-ის ოპერატორის დადასტურებები';ru='Подтверждения оператора ЭДО';en='Confirmation EDI operator'");
		ИначеЕсли (Строка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИПУУЭСФ
			...
			ВозвращаемоеЗначение = НСтр("ka='უწყისები მიღების თაობაზე';ru='Извещения о получении';en='the receipt of the notice'");
		КонецЕсли;
		
	Иначе
		Если Строка.СтатусЭД = Перечисления.СтатусыЭД.ПереданОператору Тогда
			
			ВозвращаемоеЗначение = НСтр("ka='გაგზავნის დადასტურება';ru='Подтверждения отправки';en='sending confirmation'");
		...
		ИначеЕсли Строка.СтатусЭД = Перечисления.СтатусыЭД.Отправлен Тогда
			...
			ВозвращаемоеЗначение = НСтр("ka='მიწოდების დადასტურება';ru='Подтверждения о доставке';en='Delivery confirmation'");
		КонецЕсли;
	КонецЕсли;
...
"и наличие доступа к каталогу временных файлов.';ka='შეამოწმეთ რუსული ენის მხარდაჭერა საოპერაციო სისტემის რეგიონულ გამართვებში non-Unicode programs-ისთვის და დროებითი ფაილების კატალოგთან წვდომის არსებობა.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='არქივის ფაილის ჩაწერა დისკზე';ru='запись файла архива на диск';en='archive file recording to disk'"),
																					ТекстОшибки,
																					ТекстСообщения);
...
Функция ВерсияСхемыФНС_2016() Экспорт
	Возврат НСтр("ka='საგადასახადო სამსახური 5.01 (2016 წლიდან)';ru='ФНС 5.01 (с 2016г.)';en='FTS 5.01 (s 2016).'");
КонецФункции
...
Процедура ПрочитатьИННКППИзФайла(ИННКПП, СтрокаXML)
	...
	Попытка
		...
		ЧтениеXML.УстановитьСтроку(СтрокаXML);
	Исключение
		ОписаниеОшибки = СтрШаблон(НСтр("ru='Ошибка разбора XML: %1.';en='XML Parsing Error:% 1.'"), ИнформацияОбОшибке().Описание);
		Возврат;
	КонецПопытки;
...
Процедура СообщитьОбОтсутствииСоглашенияЭД(ПараметрыЭД, Источник)
		...
		Если Порядок > 0 Тогда
			...
			Если ЭтоСчетФактураВыданный Тогда
				Если НРег(ТекПараметр.Ключ) = НРег("ЭтоСчетФактураВыданный") Тогда
					СтрокаПараметров.Ключ = НСтр("ka='გაცვლის მეთოდი';ru='Способ обмена';en='exchange process'");
					СтрокаПараметров.Значение = НСтр("ru='через оператора ЭДО';en='through EDI operator'");
				ИначеЕсли НРег(ТекПараметр.Ключ) = НРег("Контрагент") Тогда
	...
	Для Каждого СтрокаПараметров Из ТаблицаПараметров Цикл
		Если ЗначениеЗаполнено(СтрокаПараметров.Значение) Тогда
			Текст = Текст + Символы.ПС + НСтр("ka='<%1>: %2';ru='<%1>: %2';en='<% 1>% 2'");
