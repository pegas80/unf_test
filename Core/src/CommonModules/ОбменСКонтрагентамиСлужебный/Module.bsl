...
Процедура ОбменСКонтрагентамиВладелецПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ka='არსებობს ელექტრონული დოკუმენტი. დოკუმენტის საკვანძო რეკვიზიტების შეცვლა აკრძალულია.';ru='Существует электронный документ. Изменение ключевых реквизитов документа запрещено.'");
...
Процедура ЗаполнитьЗначениеРеквизитаОрганизацияПриОдноФирменномУчете(Источник, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ИспользуетсяНесколькоОрганизаций И ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", Источник.Метаданные())
		...
		Если Не ЗначениеЗаполнено(Источник.Организация) Тогда
			ВызватьИсключение НСтр("ka='Не удалось определить организацию.';ru='Не удалось определить организацию.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РегламентноеПолучениеНовыхЭД() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение
			НСтр("ka='სერვისის რეჟიმში სარეგლამენტო დავალების მუშაობა არ არის გათვალისწინებული.';ru='Работа регламентного задания в режиме сервиса не предусмотрена.'");
	КонецЕсли;
	...
	Текст = НСтр("ka='ახალი ელ.დოკუმენტების მიღებით დაწყებულია სარეგლამენტო დავალება.';ru='Начато регламентное задание по получению новых эл.документов.'");
...
"Получено пакетов: %1. Распаковано: %2.'");
	...
	Исключение
		...
		Операция = НСтр("ka='ელექტრონული დოკუმენტების სარეგლამენტო მიღება';ru='Регламентное получение электронных документов'");
		
		ШаблонСообщения = НСтр("ka='შესრულების პროცესში შეცდომა მოხდა: %1';ru='При процессе выполнении произошла ошибка: %1'");
...
Процедура РегламентнаяОтправкаОформленныхЭД() Экспорт
	...
	Текст = НСтр("ka='დაწყებულია გაფორმებული ელ.დოკუმენტების გაგზავნის სარეგლამენტო დავალება.';ru='Начато регламентное задание по отправке оформленных эл.документов.'");
	...
	Попытка
		...
		Текст = НСтр("ka='სარეგლამენტო დავალება დასრულებულია. გაგზავნილია პაკეტი: %КоличествоПакетов%.';ru='Закончено регламентное задание. Отправлено пакетов: %КоличествоПакетов%.'");
...
Процедура ПроверитьНаличиеНовыхЭД() Экспорт
	...
	ВидОперации = НСтр("ka='ახალი ელექტრონული დოკუმენტების შესახებ ინფორმაციის მიღება';ru='Получение информации о новых электронных документах'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა.';ru='Ошибка выполнения команды сервиса.'");
		...
		Если Не Ответ.КодСостояния = 200 Тогда
			...
			ШаблонСообщения = НСтр("ka='სერვისის პასუხის კოდი %1';ru='Код ответа сервиса %1'");
...
"%1'");
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, РезультатТеста);
	ТекстСообщения = НСтр("ka='პროფილ %1-ის მიხედვით გაცვლის ტესტი.';ru='Тест обмена по профилю %1.'") + " " + ТекстСообщения;
...
Функция СформироватьНовыйПакетЭД(СтруктураПараметров) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის ახალი პაკეტის შექმნა';ru='создание нового ПакетаЭД'"),
																					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ОбновитьСтатусыДокументовПакетаЭД(ПакетЭД, НовыйСтатусПакетаЭД, ДатаИзменения) Экспорт
	...
	Если ПакетЭД.ЭлектронныеДокументы.Количество() = 0 Тогда
		...
		ШаблонОшибки = НСтр("ka='დოკუმენტი %1 არ გაგზავნილა. დაწვრილებით, იხ, რეგისტრაციის ჟურნალში';ru='Документ %1 не был отправлен. Подробнее см. Журнал регистрации'");
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
					ПакетЭД );
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ka='ედ-ის პაკეტის გაგზავნა და ედ-ის სტატუსების განახლება';ru='Отправка пакета ЭД и обновление статусов ЭД'"),
			ТекстОшибки,
			ТекстСообщения);
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ka='ედ-ის პაკეტის სტატუსის შეცვლა და ედ-ის სტატუსების განახლება';ru='смена статуса пакета ЭД и обновление статусов ЭД'"), ТекстОшибки, ТекстСообщения);
	КонецПопытки;
...
Функция ОпределитьПредставлениеЭД(ВидЭД, СтруктураПараметров) Экспорт
	...
	Если ВидЭД <> Перечисления.ВидыЭД.СчетФактура И ВидЭД <> Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		ШаблонТекстаВерсии = НСтр("ka=' (ვერსია %1)';ru='(версия %1)'");
	...
	Если СтруктураПараметров.Свойство("ТипЭД", ТипЭД)
		...
		Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
			...
			ТекстВидЭД = НСтр("ka='მყიდველის ინფორმაცია';ru='Информация покупателя'");
		...
		ИначеЕсли ЭтоИзвещениеОПолучении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ka='უწყება მიღების თაობაზე';ru='Извещение о получении'");
		ИначеЕсли ЭтоПодтверждение(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ka='დადასტურება';ru='Подтверждение'");
		...
		ИначеЕсли ЭтоУведомлениеОбУточнении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ka='ანულირების წინადადება';ru='Предложение об аннулировании'");
	...
	Иначе
		...
		Если ВидЭД = Перечисления.ВидыЭД.СчетФактура Тогда
			ТекстВидЭД = НСтр("ka='Счет-фактура';ru='Счет-фактура'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД Тогда
				ТекстВидЭД = НСтр("ka='გამყიდველის ინფორმაცია';ru='Информация продавца'");
			КонецЕсли;
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
			ТекстВидЭД = НСтр("ka='საკორექტირებო ანგარიშფაქტურა';ru='Корректировочный счет-фактура'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.КСЧФДИСУКД Тогда
				ТекстВидЭД = НСтр("ka='გამყიდველის ინფორმაცია';ru='Информация продавца'");
			КонецЕсли;
			
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ТОРГ12 Тогда
			ТекстВидЭД = НСтр("ka='სასაქონლო ზედნადები (ТОРГ-12)';ru='Товарная накладная (ТОРГ-12)'");
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктВыполненныхРабот Тогда
			ТекстВидЭД = НСтр("ka='აქტი';ru='Акт'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктНаПередачуПрав Тогда
			ТекстВидЭД = НСтр("ka='აქტი უფლებების გადასაცემად';ru='Акт на передачу прав'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД Тогда
				ТекстВидЭД = НСтр("ka='გამყიდველის ინფორმაცია';ru='Информация продавца'");
			КонецЕсли;
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.СчетНаОплату Тогда
			ТекстВидЭД = НСтр("ka='ანგარიში';ru='Счет'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗаказТовара Тогда
			ТекстВидЭД = НСтр("ka='საქონლის შეკვეთა';ru='Заказ товара'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтветНаЗаказ Тогда
			ТекстВидЭД = НСтр("ka='პასუხი შეკვეთაზე';ru='Ответ на заказ'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПрайсЛист Тогда
			ТекстВидЭД = НСтр("ka='პრაის-ლისტი';ru='Прайс-лист'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
			ТекстВидЭД = НСтр("ka='საქონლის კატალოგი';ru='Каталог товаров'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ka='ანგარიშგება საკომისიო საქონლის გაყიდვის თაობაზე';ru='Отчет о продажах комиссионного товара'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ka='ანგარიშგება საკომისიო საქონლის ჩამოწერის თაობაზე';ru='Отчет о списании комиссионного товара'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ka='ინტერკამპანის გადაცემა';ru='Передача Интеркампани'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ВозвратТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ka='თავისუფალი დოკუმენტი';ru='Произвольный документ'");
...
"%1';ka='შეცდომა ელექტრონული ფოსტის სერვერიდან შეტყობინების მიღებისას. %1'");
			...
			ТекстСообщения = НСтр("ka='შეცდომა ახალი ელ.დოკუმენტების მიღებისას.';ru='Ошибка при получении новых эл.документов.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ka='ახალი ელ.დოკუმენტების მიღება';ru='Получение новых эл.документов'"), ТекстОшибки, ТекстСообщения);
			Продолжить;
		КонецПопытки;
...
"სტატუსი: %5.'");
				...
				Иначе
					ОписаниеОшибки = ?(ЗначениеЗаполнено(НовыеПриглашения.ОписаниеОшибки),
						НовыеПриглашения.ОписаниеОшибки, НСтр("ka='შეცდომა';ru='Ошибка'"));
...
"სტატუსი: %7.'");
			...
			Если НовыеПриглашения.СтатусУчастника = Перечисления.СтатусыУчастниковОбменаЭД.Присоединен Тогда
				...
				Если Выборка.Следующий() Тогда
					...
					НастройкаЭДО.Комментарий           = НСтр("ka='##ავტომატურად მოხსნილია სტატუსი ""მიერთებულია"". უკვე არსებობს ედბ-ის გამართვა არჩეული კონტრაგენტის მიხედვით.';ru='##Автоматически снят статус ""присоединен"". Уже существует настройка ЭДО по выбранному контрагенту.'");
				КонецЕсли;
			...
			ТекстОписанияОшибки = "";
			Если Ошибка Тогда
				ШаблонОписанияОшибки = НСтр("ka='%1. გაიმეორეთ გაგზავნა.';ru='%1. Повторите отправку.'");
		...
		Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
			...
			Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					КонецЕсли;
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის პაკეტის განფუთვა';ru='Распаковка пакета ЭД'"),
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если СтруктураОбмена.Свойство("МассивФайлов") И СтруктураОбмена.МассивФайлов.Количество() > 0 Тогда
			...
			Если Не АдресАрхива = Неопределено Тогда
				...
				ПараметрыФайла.Вставить("Автор", Пользователи.АвторизованныйПользователь());
				ПараметрыФайла.Вставить("ВладелецФайлов", НовыйЭлектронныйДокумент); 
				ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ka='დამატებითი ფაილები';ru='Дополнительные файлы'"));
...
Процедура ОтправитьРегистрационныйПакет1СЭДО(Знач Файл, ПараметрыАутентификации, ЕстьОшибки) Экспорт
	...
	Исключение
		...
		ШаблонЗаголовкаСообщения = НСтр("ka='რეგისტრაციის შესახებ განცხადების გაგზავნისას: %1 სერვის 1С-ედბ-ზე შეცდომა წარმოიშვა:';ru='При отправке заявления о регистрации: %1 на сервисе 1С-ЭДО возникла ошибка:'");
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			Если МассивПараметровОшибки.Количество() >= 7 Тогда
				...
				ШаблонОшибки = НСтр("ka='%1: %2.';ru='%1: %2.'");
...
Функция ПолучитьФайлОтветаОтСервераЭДО(АдресРесурса, ПараметрЗапросаОтветаОтСервера = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстЗаголовкаСообщения = НСтр("ka='სერვის 1C-ედბ-სთან მუშაობისას შეცდომა წარმოიშვა:';ru='При работе с сервисом 1С-ЭДО возникла ошибка:'");
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт
	...
	Исключение
		...
		Операция = НСтр("ka='1С-ედბ-სთან კონტრაგენტის შეერთების შემოწმება კონტრაგენტის ფორმიდან.';ru='Проверка подключения контрагента к 1С-ЭДО из формы контрагента.'");
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор)
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ka='სერვის 1C-ედბ-ზე კონტრაგენტების მდგომარეობის შემოწმებისას შეცდომა წარმოიშვა:';ru='При проверке состояния контрагентов на сервисе 1С-ЭДО возникла ошибка:'");
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		Операция = НСтр("ka='1C-ედბ-სთან კონტრაგენტების შეერთების შემოწმების შედეგების ჩაწერა რეგისტრში.';ru='Запись результатов проверки подключения контрагентов к 1С-ЭДО в регистр.'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробныйТекстОшибки);
	КонецПопытки;
...
Функция УведомленияПодключены(ИдентификаторОрганизации, Соединение = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ka='შეტყობინებების მდგომარეობის შესახებ ინფორმაციის მიღება';ru='Получение информации о состоянии уведомлений'");
...
"%1';ka='შეცდომა ელექტრონული ფოსტის სერვერზე შეტყობინების გაგზავნისას. %1'");
				...
				ТекстСообщения = НСтр("ka='შეცდომა ელ.დოკუმენტების გაგზავნისას.';ru='Ошибка при отправке эл.документов.'");
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ელ. დოკუმენტების გაგზავნა';ru='Отправка эл.документов'"),
				ТекстОшибки,
				ТекстСообщения);
			КонецПопытки;
...
Функция НеобходимоПолучить(Сообщение, СтруктураПараметров)
	...
	Если Сообщение.Вложения.Количество() = 1 Тогда 
		...
		Если СтрНайти(Сообщение.Тема, НСтр("ka='დადასტურება';ru='Подтверждение'")) Тогда
			Возврат Истина;
		КонецЕсли;
		
		Если СтрНайти(Сообщение.Тема, НСтр("ka='ელ.დოკუმენტების გაცვლა:';ru='Обмен эл.документами:'")) Тогда
...
Функция НеобходимоПолучитьДвоичныеДанные(ДвоичныеДанные, ИмяФайла, СтруктураПараметров)
	...
	Если НЕ ФайлКарточки = Неопределено Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='Чтение ЭД.';ru='Чтение ЭД.'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ТекстСообщения);
		КонецПопытки;
...
Процедура ПроверитьФайл(ШаблонСообщения, FTPСоединение, ТекстОшибки)
	...
	ТестоваяСтрока = НСтр("ka='სატესტო სტრიქონი 1С: საწარმო';ru='Тестовая строка 1С: Предприятие'");
	...
	Если НЕ СтрокаРезультата = ТестоваяСтрока Тогда
		ШаблонСообщения = НСтр("ka='%1 %2.';ru='%1 %2.'");
...
Процедура ОбновитьСтатусыЭД(МассивДокументов, ПризнакПодписи)
	...
	Для Каждого ЭлектронныйДокумент Из МассивДокументов Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის მომზადება გასაგზავნად';ru='подготовка к отправке ЭД'"),
																						ТекстОшибки,
																						ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗаписатьДатуПолучения(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის მიღების თარიღის ჩანაწერი';ru='запись даты получения ЭД'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ЗаписатьДатуОтправки(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის გაგზავნის თარიღის ჩანაწერი';ru='запись даты отправки ЭД'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Функция СоздатьПакетЭД(РеквизитыДокумента, МассивЭД, ТекстСообщения = "")
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='ედ-ის პაკეტის ჩამოყალიბება';ru='формирование пакета ЭД'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СписокОперацийВидаЭД(ВидЭД, ДобавлятьПредопределенныеСпособы = Ложь, Префикс = "") Экспорт 
	...
	Если ДобавлятьПредопределенныеСпособы Тогда
		Если СписокОпераций.Количество() = 0 Тогда
			СписокОпераций.Добавить("Автоматически", НСтр("ka='ავტომატურად';ru='Автоматически'"),Истина);
		КонецЕсли;
...
Функция ПолучитьОжидаемоеДействиеКонтрагента(Строка,ВладелецФайла)
	...
	Если Строка.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		Если Строка.ВидЭД = Перечисления.ВидыЭД.Подтверждение И НЕ ЗначениеЗаполнено(Строка.Ссылка)
			...
			ВозвращаемоеЗначение = НСтр("ka='ედბ-ის ოპერატორის დადასტურებები';ru='Подтверждения оператора ЭДО'");
		ИначеЕсли (Строка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИПУУЭСФ
			...
			ВозвращаемоеЗначение = НСтр("ka='უწყისები მიღების თაობაზე';ru='Извещения о получении'");
		КонецЕсли;
		
	Иначе
		Если Строка.СтатусЭД = Перечисления.СтатусыЭД.ПереданОператору Тогда
			
			ВозвращаемоеЗначение = НСтр("ka='გაგზავნის დადასტურება';ru='Подтверждения отправки'");
		...
		ИначеЕсли Строка.СтатусЭД = Перечисления.СтатусыЭД.Отправлен Тогда
			...
			ВозвращаемоеЗначение = НСтр("ka='მიწოდების დადასტურება';ru='Подтверждения о доставке'");
		КонецЕсли;
	КонецЕсли;
...
"и наличие доступа к каталогу временных файлов.';ka='შეამოწმეთ რუსული ენის მხარდაჭერა საოპერაციო სისტემის რეგიონულ გამართვებში non-Unicode programs-ისთვის და დროებითი ფაილების კატალოგთან წვდომის არსებობა.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='არქივის ფაილის ჩაწერა დისკზე';ru='запись файла архива на диск'"),
																					ТекстОшибки,
																					ТекстСообщения);
...
Функция ВерсияСхемыФНС_2016() Экспорт
	Возврат НСтр("ka='საგადასახადო სამსახური 5.01 (2016 წლიდან)';ru='ФНС 5.01 (с 2016г.)'");
КонецФункции
		...
		Если Порядок > 0 Тогда
			...
			Если ЭтоСчетФактураВыданный Тогда
				Если НРег(ТекПараметр.Ключ) = НРег("ЭтоСчетФактураВыданный") Тогда
					СтрокаПараметров.Ключ = НСтр("ka='გაცვლის მეთოდი';ru='Способ обмена'");
