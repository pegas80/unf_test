...
Функция ПолучитьДвоичныеДанныеФайла(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილის ორობითი მონაცემები რეგისტრ მიერთებულიფაილები-ში არ არის"
""
"ფაილის ბმული: ""%1"".';ru='Двоичные данные файла отсутствуют в регистре ПрисоединенныеФайлы"
""
"Ссылка на файл: ""%1"".'"),
			ПолучитьНавигационнуюСсылку(ПрисоединенныйФайл));
		ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. ფაილის გახსნა';ru='Файлы.Открытие файла';en='Fayly.Otkrytie file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ПредставлениеВладельцаФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მიერთებულია %1 : %2-ზე';ru='Присоединен к %1 : %2';en='Connected to% 1,% 2'"),
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. ფაილის მიღება ტომიდან';ru='Файлы.Получение файла из тома';en='Fayly.Poluchenie file from the volume'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Справочники[ПрисоединенныйФайл.Метаданные().Имя],
				ПрисоединенныйФайл,
				СообщениеОбОшибке);
...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიერთებული ფაილი ""%1"" (%2) ვერ მოიძებნა';ru='Не найден присоединенный файл ""%1"" (%2)';en='Not found attached file ""%1"" (% 2)'"),
...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებული ფაილის დამატებისას.';ru='Ошибка при добавлении присоединенного файла.';en='Error when adding an attached file.'");
	...
	Если НоваяСсылкаНаФайл = Неопределено Тогда
		ИмяСправочника = ПрисоединенныеФайлыСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, "", ЗаголовокОшибки,
			НСтр("ka='ამ შემთხვევაში, პარამეტრი ""ფაილისახალიბმული"" უნდა იყოს მითითებული.';ru='В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.';en='In this case, the ""NovayaSsylkaNaFayl"" must be specified.'"));
...
"Ссылка на новый файл не заполнена.';ka='შეცდომა მიერთებული ფაილის დამატებისას. ბმული ახალ ფაილზე არ არის შევსებული.'");
		КонецЕсли;
...
"%2';ka='შეცდომა მიერთებული ფაილის დამატებისას ""%1"":"
"%2'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ფაილები. მიერთებული ფაილის დამატება';ru='Файлы.Добавление присоединенного файла';en='Fayly.Dobavlenie attached file'",
...
Функция НоваяСсылкаНаФайл(ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებულ ფაილზე ახალი ბმულის მიღებისას.';ru='Ошибка при получении новой ссылки на присоединенный файл.';en='Error getting a new reference to the attached file.'");
...
Функция ПолучитьИмяФормыОбъектаПрисоединенныхФайловПоВладельцу(Знач ВладелецФайлов) Экспорт
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებული ფაილის ფორმის სახელის მიღებისას.';ru='Ошибка при получении имени формы присоединенного файла.';en='Error getting form name of the attached file.'");
	ОкончаниеОшибки = НСтр("ka='ამ შემთხვევაში, ფორმის მიღება შეუძლებელია.';ru='В этом случае получение формы невозможно.';en='In this case, it is impossible to obtain the form.'");
...
"в присоединенные файлы подсистемы Присоединенные файлы.';ka='შეცდომა ქვესისტემა ფაილებთან მუშაობა-ს მიერთებული ფაილების ქვესისტემა მიერთებული ფაილები-ს მიერთებულ ფაილებში კონვერტაციის დროს.'");
	...
	Если Не Источник.ЭтоНовый() Тогда
		...
		Если ИзмененаПометкаУдаления И ЗначениеЗаполнено(Источник.Редактирует) Тогда
			...
			Если Источник.Редактирует = Пользователи.АвторизованныйПользователь() Тогда
				...
				ТекстОшибки = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი ""%1"" რედაქტირებისთვის არის დაკავებული.';ru='Действие недоступно, так как файл ""%1"" занят для редактирования.';en='Action is not available because the file ""%1"" is not available for editing.'");
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Источник.Наименование);
				
			Иначе
				ТекстОшибки = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი ""%1"" მომხმარებელ %2-ის მიერ რედაქტირებისთვის არის დაკავებული.';ru='Действие недоступно, так как файл ""%1"" занят для редактирования"
"пользователем %2.'");
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='შეცდომა მიერთებული ფაილების წასაშლელად მონიშვნისას.';ru='Ошибка при пометке на удаление присоединенных файлов.';en='An error occurred while tagging to delete attachments.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
	...
	Для каждого ОписаниеИмениСправочника Из ИменаСправочников Цикл
		...
		Пока Выборка.Следующий() Цикл
			Если Источник.ПометкаУдаления И ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='""%1"" არ შეიძლება წაიშალოს, ვინაიდან შეიცავს მიერთებულ ფაილ ""%2""-ს, რომელიც რედაქტირებისთვის არის დაკავებული.';ru='""%1"" не может быть удален,"
