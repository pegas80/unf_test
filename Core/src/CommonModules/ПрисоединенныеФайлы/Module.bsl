...
"Ссылка на файл: ""%1"".'"),
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. ფაილის გახსნა';ru='Файлы.Открытие файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ПредставлениеВладельцаФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მიერთებულია %1 : %2-ზე';ru='Присоединен к %1 : %2'"),
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. ფაილის მიღება ტომიდან';ru='Файлы.Получение файла из тома'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Справочники[ПрисоединенныйФайл.Метаданные().Имя],
				ПрисоединенныйФайл,
				СообщениеОбОшибке);
...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიერთებული ფაილი ""%1"" (%2) ვერ მოიძებნა';ru='Не найден присоединенный файл ""%1"" (%2)'"),
...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებული ფაილის დამატებისას.';ru='Ошибка при добавлении присоединенного файла.'");
	...
	Если НоваяСсылкаНаФайл = Неопределено Тогда
		ИмяСправочника = ПрисоединенныеФайлыСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, "", ЗаголовокОшибки,
			НСтр("ka='ამ შემთხვევაში, პარამეტრი ""ფაილისახალიბმული"" უნდა იყოს მითითებული.';ru='В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.'"));
...
"%2'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ფაილები. მიერთებული ფაილის დამატება';ru='Файлы.Добавление присоединенного файла'",
...
Функция НоваяСсылкаНаФайл(ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებულ ფაილზე ახალი ბმულის მიღებისას.';ru='Ошибка при получении новой ссылки на присоединенный файл.'");
...
Функция ПолучитьИмяФормыОбъектаПрисоединенныхФайловПоВладельцу(Знач ВладелецФайлов) Экспорт
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებული ფაილის ფორმის სახელის მიღებისას.';ru='Ошибка при получении имени формы присоединенного файла.'");
	ОкончаниеОшибки = НСтр("ka='ამ შემთხვევაში, ფორმის მიღება შეუძლებელია.';ru='В этом случае получение формы невозможно.'");
...
"""%1"".'"),
		...
		Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='ფაილები. ფაილის ჩაწერის შეცდომა საინფორმაციო ბაზის განახლებისას';ru='Файлы.Ошибка записи файла при обновлении ИБ'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				Источник.Ссылка,
	...
	Если Не Источник.ЭтоНовый() Тогда
		...
		Если ИзмененаПометкаУдаления И ЗначениеЗаполнено(Источник.Редактирует) Тогда
			...
			Если Источник.Редактирует = Пользователи.АвторизованныйПользователь() Тогда
				...
				ТекстОшибки = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი ""%1"" რედაქტირებისთვის არის დაკავებული.';ru='Действие недоступно, так как файл ""%1"" занят для редактирования.'");
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='შეცდომა მიერთებული ფაილების წასაშლელად მონიშვნისას.';ru='Ошибка при пометке на удаление присоединенных файлов.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
