...
Процедура ПриИзмененииСтатусаОбработкиДокумента(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС") Тогда
		...
		Если НовыйСтатус = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ПринятАктОтказа Тогда
			...
			Исключение
				ТекстОшибки = НСтр("ka='მიღებულ ზედნადებ %1-ზე წაშლის ნიშნულის დაყენება ვერ მოხერხდა';ru='Не удалось установить пометку удаления не принятой накладной %1';en='Unable to establish a mark removal is not received the invoice% 1'");
...
Процедура ЗаполнитьТоварыДокументаЕГАИСНаОснованииДанныхЗаполнения(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='Нет данных для заполнения по основанию ""%ДокументОснование%"".';ru='Нет данных для заполнения по основанию ""%ДокументОснование%"".';en='No data to fill the base ""DokumentOsnovanie%%"".'");
