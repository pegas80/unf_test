		...
		Если НайденноеДействие <> Неопределено Тогда
			...
			ИначеЕсли НайденноеДействие.Значение = "Предупреждение" Тогда
				...
				Если ТипЗнч(Текст) = Тип("ЭлементСпискаЗначений") Тогда
					...
					Иначе
						Заголовок = НСтр("ka='ინფორმაცია';ru='Информация'");
					КонецЕсли;
...
"%3'"),
					...
					ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:Новости.Клиент';ru='БИП:Новости.Клиент'"), // ИмяСобытия
						НСтр("ka='ახალი ამბები. კლიენტი. ახალიამბებისდამუშავებაკლიენტი. სანავიგაციობმულისდამუშავება';ru='Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки'"), // ИдентификаторШага
...
"%3'"),
	...
	Иначе
		ТекстСообщения = СтрШаблон(
			НСтр("ka='Действие %1 с параметрами %2 не обрабатывается.';ru='Действие %1 с параметрами %2 не обрабатывается.'"),
			Действие,
			СписокПараметров);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:Новости.Клиент';ru='БИП:Новости.Клиент'"), // ИмяСобытия
...
"%2'"),
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:Новости.Клиент';ru='БИП:Новости.Клиент'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. კლიენტი. ახალიამბებისდამუშავებაკლიენტი. ინტერნეტბმულზეგადასვლა';ru='Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'"), // ИдентификаторШага
			"Ошибка", // УровеньЖурналаРегистрации строкой
			"", // ОбъектМетаданных строкой
