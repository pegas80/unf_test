		...
		Если НайденноеДействие <> Неопределено Тогда
			...
			ИначеЕсли НайденноеДействие.Значение = "Предупреждение" Тогда
				...
				Если ТипЗнч(Текст) = Тип("ЭлементСпискаЗначений") Тогда
					...
					Иначе
						Заголовок = НСтр("ka='ინფორმაცია';ru='Информация';en='Information'");
					КонецЕсли;
...
"%3';ka='Произошла ошибка при обработке действия %1 с параметрами %2."
"Подробная информация об ошибке:"
"%3'"),
						Действие,
						СписокПараметров,
					...
					ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:Новости.Клиент';ru='БИП:Новости.Клиент';en='BIP: Novosti.Klient'"), // ИмяСобытия
						НСтр("ka='ახალი ამბები. კლიენტი. ახალიამბებისდამუშავებაკლიენტი. სანავიგაციობმულისდამუშავება';ru='Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки';en='News. Client. ObrabotkaNovosteyKlient. ObrabotkaNavigatsionnoySsylki'"), // ИдентификаторШага
...
"%3'"),
	...
	Иначе
		ТекстСообщения = СтрШаблон(
			НСтр("ka='Действие %1 с параметрами %2 не обрабатывается.';ru='Действие %1 с параметрами %2 не обрабатывается.';en='Action% 1% 2 with parameters not processed.'"),
			Действие,
			СписокПараметров);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:Новости.Клиент';ru='БИП:Новости.Клиент';en='BIP: Novosti.Klient'"), // ИмяСобытия
...
"Подробная информация об ошибке:"
"%2';ka='Произошла ошибка при переходе по ссылке %1."
"Подробная информация об ошибке:"
"%2'"),
			ИнтернетСсылка,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:Новости.Клиент';ru='БИП:Новости.Клиент';en='BIP: Novosti.Klient'"), // ИмяСобытия
			НСтр("ka='ახალი ამბები. კლიენტი. ახალიამბებისდამუშავებაკლიენტი. ინტერნეტბმულზეგადასვლა';ru='Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке';en='News. Client. ObrabotkaNovosteyKlient. PereytiPoInternetSsylke'"), // ИдентификаторШага
			"Ошибка", // УровеньЖурналаРегистрации строкой
			"", // ОбъектМетаданных строкой
