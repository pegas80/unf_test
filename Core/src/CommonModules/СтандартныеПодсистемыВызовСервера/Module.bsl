...
Функция ЗаписатьОшибкуВЖурналРегистрацииПриЗапускеИлиЗавершении(ПрекратитьРаботу, Знач Событие, Знач ТекстОшибки) Экспорт
	...
	Если Событие = "Запуск" Тогда
		ИмяСобытия = НСтр("ka='პრპგრამის ამოქმედება';ru='Запуск программы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Если ПрекратитьРаботу Тогда
			НачалоОписанияОшибки = НСтр("ka='პროგრამის გაშვებისას გამონაკლისი სიტუაცია წარმოიქმნა. პროგრამის გაშვება ავარიულად დასრულდა.';ru='Возникла исключительная ситуация при запуске программы. Запуск программы аварийно завершен.'");
		Иначе
			НачалоОписанияОшибки = НСтр("ka='პროგრამის გაშვებისას საგანგებო სიტუაცია წარმოიქმნა.';ru='Возникла исключительная ситуация при запуске программы.'");
		КонецЕсли;
	Иначе
		ИмяСобытия = НСтр("ka='პროგრამის დასრულება';ru='Завершение программы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		НачалоОписанияОшибки = НСтр("ka='პროგრამის დასრულებისას გამონაკლისი სიტუაცია წარმოიშვა.';ru='Возникла исключительная ситуация при завершении программы.'");
	КонецЕсли;
