...
Функция ОбработатьПараметрыЗапуска(Знач ЗначениеПараметраЗапуска, Знач ПараметрыЗапуска)
	...
	Если ЗначениеПараметраЗапуска = ВРег("РазрешитьРаботуПользователей") Тогда
		...
		Если НЕ СоединенияИБВызовСервера.РазрешитьРаботуПользователей() Тогда
			ТекстСообщения = НСтр("ka='გაშვების პარამეტრი მომხმარებელთამუშაობისდაშვება არ არის დამუშავებული. საინფორმაციო ბაზის ადმინისტრირების უფლება არ არის.';ru='Параметр запуска РазрешитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';en='RazreshitRabotuPolzovateley startup parameter is not fulfilled. No administration rights to the knowledge base.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СоединенияИБКлиентСервер.СобытиеЖурналаРегистрации(),,
			НСтр("ka='შესრულდა გაშვება პარამეტრ ""მომხმარებლებისმუშაობისნებართვა""-თი. პროგრამის მუშაობა დასრულდება.';ru='Выполнен запуск с параметром ""РазрешитьРаботуПользователей"". Работа программы будет завершена.';en='I launched a ""RazreshitRabotuPolzovateley"" parameter. The work program will be completed.'"), ,Истина);
	...
	ИначеЕсли ЗначениеПараметраЗапуска = ВРег("ЗавершитьРаботуПользователей") Тогда
		...
		Если НЕ СоединенияИБВызовСервера.УстановитьБлокировкуСоединений() Тогда
			ТекстСообщения = НСтр("ka='გაშვების პარამეტრი მომხმარებელთამუშაობისდასრულება არ არის დამუშავებული. საინფორმაციო ბაზის ადმინისტრირების უფლება არ არის.';ru='Параметр запуска ЗавершитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';en='ZavershitRabotuPolzovateley startup parameter is not fulfilled. No administration rights to the knowledge base.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
...
Процедура ЗавершитьРаботуЭтогоСеанса(ВыводитьВопрос = Истина) Экспорт
	...
	ТекстСообщения = НСтр("ka='პროგრამასთან მომხმარებელთა მუშაობა დასრულდება. დავასრულოთ ამ სეანსის მუშაობა?';ru='Работа пользователей с программой запрещена. Завершить работу этого сеанса?';en='Users work with the program is prohibited. To complete the work of this session?'");
	Заголовок = НСтр("ka='მიმდინარე სეანსის მუშაობის დასრულება';ru='Завершение работы текущего сеанса';en='Completing the current session'");
...
Процедура ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры, Неопределен) Экспорт
	...
	Если Не ПустаяСтрока(ТекстВопроса) Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='შესვლა';ru='Войти';en='To come in'"));
		Если ПараметрыКлиента.ВозможноСнятьБлокировку Тогда
			Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='ბლოკირების მოხსნა და შესვლა';ru='Снять блокировку и войти';en='Unlock and enter'"));
		КонецЕсли;
...
Процедура ЗадатьВопросПриЗавершенииРаботы(ТекстСообщения) Экспорт
	...
	ТекстВопроса = НСтр("ka='%1 დასრულდეს სამუშაო?';ru='%1"
"Завершить работу?'");
