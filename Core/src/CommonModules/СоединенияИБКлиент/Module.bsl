...
Функция ОбработатьПараметрыЗапуска(Знач ЗначениеПараметраЗапуска, Знач ПараметрыЗапуска)
	...
	Если ЗначениеПараметраЗапуска = ВРег("РазрешитьРаботуПользователей") Тогда
		...
		Если НЕ СоединенияИБВызовСервера.РазрешитьРаботуПользователей() Тогда
			ТекстСообщения = НСтр("ka='გაშვების პარამეტრი მომხმარებელთამუშაობისდაშვება არ არის დამუშავებული. საინფორმაციო ბაზის ადმინისტრირების უფლება არ არის.';ru='Параметр запуска РазрешитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СоединенияИБКлиентСервер.СобытиеЖурналаРегистрации(),,
			НСтр("ka='შესრულდა გაშვება პარამეტრ ""მომხმარებლებისმუშაობისნებართვა""-თი. პროგრამის მუშაობა დასრულდება.';ru='Выполнен запуск с параметром ""РазрешитьРаботуПользователей"". Работа программы будет завершена.'"), ,Истина);
	...
	ИначеЕсли ЗначениеПараметраЗапуска = ВРег("ЗавершитьРаботуПользователей") Тогда
		...
		Если НЕ СоединенияИБВызовСервера.УстановитьБлокировкуСоединений() Тогда
			ТекстСообщения = НСтр("ka='გაშვების პარამეტრი მომხმარებელთამუშაობისდასრულება არ არის დამუშავებული. საინფორმაციო ბაზის ადმინისტრირების უფლება არ არის.';ru='Параметр запуска ЗавершитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
...
Процедура ЗавершитьРаботуЭтогоСеанса(ВыводитьВопрос = Истина) Экспорт
	...
	ТекстСообщения = НСтр("ka='პროგრამასთან მომხმარებელთა მუშაობა დასრულდება. დავასრულოთ ამ სეანსის მუშაობა?';ru='Работа пользователей с программой запрещена. Завершить работу этого сеанса?'");
	Заголовок = НСтр("ka='მიმდინარე სეანსის მუშაობის დასრულება';ru='Завершение работы текущего сеанса'");
...
Процедура ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры, Неопределен) Экспорт
	...
	Если Не ПустаяСтрока(ТекстВопроса) Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='შესვლა';ru='Войти'"));
		Если ПараметрыКлиента.ВозможноСнятьБлокировку Тогда
			Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='ბლოკირების მოხსნა და შესვლა';ru='Снять блокировку и войти'"));
		КонецЕсли;
