	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ka='ფაილები. ტომში ფაილების წაშლა გაცვლისას';ru='Файлы.Удаление файлов в томе при обмене'",
...
Функция ДобавитьФайлВТом(ДвоичныеДанныеИлиПуть, ВремяИзмененияУниверсальное, ИмяБезРасширения, Расширение,
	...
	Если Выборка.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='ფაილის განსათავსებლად არცერთი ტომი არ არის.';ru='Нет ни одного тома для размещения файла.'");
	КонецЕсли;
			...
			Если СсылкаНаТом.МаксимальныйРазмер <> 0 Тогда
				...
				Если НовыйРазмер > СсылкаНаТом.МаксимальныйРазмер Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='გადამეტებულია ტომის მაქსიმალური ზომა (%1 მბ).';ru='Превышен максимальный размер тома (%1 Мб).'"),
...
"%1'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='ფაილები დამატებული ფაილები';ru='Файлы.Добавление файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура КриптографияПриСозданииФормыНаСервере(Форма, ЭтоФормаСписка = Истина, ТолькоКартинкаСтрок = Ложь) Экспорт
	...
	Если ЭлектронноеПодписание И Шифрование Тогда
		Заголовок = НСтр("ka='ელექტრონული ხელმოწერა და დაშიფრვა';ru='Электронная подпись и шифрование'");
		Подсказка = НСтр("ka='ელექტრონული ხელმოწერის არსებობა დასაშიფრად';ru='Наличие электронной подписи или шифрования'");
		Картинка  = БиблиотекаКартинок["ПодписанЗашифрованЗаголовок"];
	ИначеЕсли ЭлектронноеПодписание Тогда
		Заголовок = НСтр("ka='ელექტრონული ხელმოწერა';ru='Электронная подпись'");
		Подсказка = НСтр("ka='ელექტრონული ხელმოწერის არსებობა';ru='Наличие электронной подписи'");
		Картинка  = БиблиотекаКартинок["ПодписанЭП"];
	Иначе // Шифрование
		Заголовок = НСтр("ka='დაშიფრვა';ru='Шифрование'");
		Подсказка = НСтр("ka='დაშიფვრის არსებობა';ru='Наличие шифрования'");
...
"в формате UNC (\\servername\resource)';ka='გზა ფაილურ ბაზასთან უნდა იყოს UNC-ის ფორმატში (\\servername\resource)'"),
	...
	Если ПустаяСтрока(ПолноеИмяФайловойБазы) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მიუთითეთ ფაილური ბაზის სრული სახელი (ფაილი 1cv8.1cd)';ru='Укажите полное имя файловой базы (файл 1cv8.1cd)'"),,
			"ПолноеИмяФайловойБазыWindows");
		Результат.ДанныеПодготовлены = Ложь;
	ИначеЕсли Результат.ДанныеПодготовлены Тогда
		...
		Если ЕстьФайлыВТомах Тогда
			Если ПустаяСтрока(ПутьКАрхивуСФайламиТомов) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მიუთითეთ ტომების ფაილებიანი არქივის სრული სახელი (файл *.zip)';ru='Укажите полное имя архива с файлами томов (файл *.zip)'"),, 
					"ПутьКАрхивуСФайламиТомовWindows");
				Результат.ДанныеПодготовлены = Ложь;
			Иначе
...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='ფაილები. ტექსტის ამოღება';ru='Файлы.Извлечение текста'",
		     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		НСтр("ka='ტექსტის სარეგლამენტო ამოღება დაწყებულია';ru='Начато регламентное извлечения текста'"));
...
"""%2"".'"),
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ka='ტექსტის რეგლამენტური ამოკრეფვა დამთავრებულია';ru='Закончено регламентное извлечение текста'"));
...
Процедура ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		ШаблонОшибки = НСтр("ka='შეცდომა HTTP 1С : %1 %2-ში';ru='Ошибка HTTP 1С : %1 в %2'");
...
Процедура ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	ТекстСобытия = НСтр("ka='ფაილის ჩატვირთვა სერვერიდან: %1';ru='Загрузка файла с сервера: %1'");
	...
	Если АдресЗагруженногоФайла <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='ფაილი ჩატვირთულია სერვერიდან: %1';ru='Загружен файл с сервера: %1'");
	Иначе
		ТекстСообщения = НСтр("ka='ფაილის ჩატვირთვა ვერ მოხერხდა სერვერიდან: %1';ru='Не удалось загрузить файл с сервера:  %1'");
	КонецЕсли;
...
Процедура ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСообщения, ПараметрыСинхронизации)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='ფაილების სინქრონიზაცია ღრუბლოვან სერვისთან:';ru='Синхронизация файлов с облачным сервисом:'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Информация,
					,
					ПараметрыСинхронизации.УчетнаяЗапись,
					ТекстСообщения);
			...
			Если НайденныйPropstat<>Неопределено Тогда
				Для каждого ДочернийУзелPropstat Из НайденныйPropstat.ДочерниеУзлы Цикл
					Если ДочернийУзелPropstat.ЛокальноеИмя = "quota-available-bytes" Тогда
						...
						ИнформацияОСвободномМесте = НСтр("ka='თავისუფალი ადგილი (ბაიტი): %1(%2 მბ)';ru='Свободное место (байт): %1(%2 Мб)'");
					...
					ИначеЕсли ДочернийУзелPropstat.ЛокальноеИмя = "quota-used-bytes" Тогда
						...
						ИнформацияОЗанятомМесте = НСтр("ka='დაკავებულია (ბაიტი): %1(%2 მბ)';ru='Занято (байт): %1(%2 Мб)'");
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			Если НайденныйHref = Неопределено Тогда
				ТекстОшибки = НСтр("ka='სერვერის პასუხის შეცდომა: %1-ში HREF-ი ვერ მოიძებნა';ru='Ошибка ответа от сервера: не найден HREF в %1'");
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href") = Неопределено) Тогда
				...
				Если ПодсистемаРаботаСФайламиСуществует Тогда
					...
					Если НЕ ПроверитьВозможностьUID1C(СтрокаДереваФайлов.Href,Строка(Новый УникальныйИдентификатор), ПараметрыСинхронизации) Тогда
						ТекстСобытия = НСтр("ka='ფაილის დამატებითი თვისებების შენახვა შეუძლებელია, იგი ჩაიტვირთება: %1';ru='Невозможно сохранение дополнительных свойств файла, он не будет загружен: %1'");
					...
					Попытка
						...
						ТекстСобытия = НСтр("ka='საქაღალდე ჩატვირთულია სერვერიდან: %1';ru='Загружена папка с сервера:  %1'");
...
Процедура СинхронизироватьФайлыСОблачнымСервисом(УчетнаяЗапись)
	...
	ТекстСобытия = НСтр("ka='ღრუბლოვან სერვისთან ფაილების სინქრონიზაციის დასაწყისი.';ru='Начало синхронизации файлов с облачным сервисом.'");
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			ТекстОшибки = НСтр("ka='სერვერიდან ფაილების სტრუქტურის ჩატვირთვისას შეცდომა მოხდა, სინქრონიზაცია არ შესრულებულა.';ru='При загрузке структуры файлов с сервера произошла ошибка, синхронизация не выполнена.'");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
			...
			Попытка
				...
				Если СозданНовыйВБазе И НЕ УдаленВБазе Тогда
					...
					ТекстСобытия = НСтр("ka='შექმნილია ობიექტი ღრუბლოვან სერვისში %1';ru='Создан объект в облачном сервисе %1'");
				...
				ИначеЕсли УдаленНаСервере И НЕ УдаленВБазе Тогда
					...
					ТекстСобытия = НСтр("ka='წაშლილია ბაზაში: %1';ru='Удалено в базе: %1'");
					ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации);
				...
				ИначеЕсли (ИзмененВБазе ИЛИ ИзмененНаСервере) И НЕ (УдаленВБазе ИЛИ УдаленНаСервере) Тогда
					...
					Если ИзмененНаСервере И НЕ ИзмененВБазе Тогда
						...
						СтрокаТаблицы.Обработан = Истина;
						ТекстСобытия = НСтр("ka='ობიექტი განახლებულია ბაზაში: %1';ru='Обновлен объект в базе: %1'");
						ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации);
	...
	ТекстСобытия = НСтр("ka='ღრუბლოვან სერვისთან ფაილების სინქრონიზაცია დასრულებულია';ru='Завершена синхронизация файлов с облачным сервисом'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
...
Процедура ВыполнитьПроверкуПодключения(УчетнаяЗапись, РезультатПроверки) Экспорт 
	...
	ТекстСобытия = НСтр("ka='დაწყებულია ფაილების სინქრონიზაციის შემოწმება';ru='Начата проверка синхронизации файлов'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
	
	ПрочитатьПараметрыКаталога(РезультатПроверки,АдресСервера, ПараметрыСинхронизации);
	
	ТекстСобытия = НСтр("ka='ფაილების სინქრონიზაციის შემოწმება დასრულებულია';ru='Завершена проверка синхронизации файлов'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
...
Процедура ОсвободитьЗахваченныеФайлыУчетнойЗаписи(УчетнаяЗапись = Неопределено)
	...
	ТекстСобытия = НСтр("ka='ღრუბლოვანი სერვისის მიერ შეპყრობილი ფაილების გათავისუფლების დასაწყისი.';ru='Начало освобождения файлов, захваченных облачным сервисом.'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
	...
	Попытка
		...
		Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
			...
			Попытка
				...
				ТекстСобытия = НСтр("ka='ობიექტი წაშლილია ღრუბლოვან სერვისში %1';ru='Удален объект в облачном сервисе %1'");
	...
	ТекстСобытия = НСтр("ka='ღრუბლოვანი სერვისის მიერ შეპყრობილი ფაილების გათავისუფლება დასრულებულია';ru='Завершено освобождения файлов, захваченных облачным сервисом'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
