...
Функция ПересчитатьПоКурсу(Сумма, ПараметрыТекущегоКурса, ПараметрыНовогоКурса) Экспорт
	...
	Если ПараметрыТекущегоКурса.Курс = 0
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ვალუტა %1-ში გადათვლისას თანხა %2 დაყენებულია ნულოვან მნიშვნელობაში, ვინაიდან ვალუტის კურსი არ არის მიცემული.';ru='При пересчете в валюту %1 сумма %2 установлена в нулевое значение, т.к. курс валюты не задан.';en='For conversion into the currency amount% 1% 2 set to zero, since the exchange rate is not specified.'"), 
