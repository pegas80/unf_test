...
Функция ДобавитьПодменюПросмотраСпискаНовостей(
	...
	Если (ТипЗнч(ЭлементКоманднаяПанель) = Тип("ГруппаФормы")) Тогда
		...
		Если ЭлементПодменюНовости = Неопределено Тогда // Еще подменю не создавали
			...
			ЭлементПодменюНовости.Заголовок = НСтр("ka='ახალი ამბები';ru='Новости';en='news'");
		...
		Если (ТипЗнч(ЭлементПодменюНовости) = Тип("ГруппаФормы")) Тогда
			...
			Если (ТипЗнч(ТаблицаНовостей) = Тип("ДанныеФормыКоллекция")
				...
				Если (ТаблицаНовостей.Количество() > 0) Тогда
					...
					ПодменюПостоянныхНовостей.Заголовок = НСтр("ka='მუდმივი ახალი ამბების სიის ქვემენიუ';ru='Подменю списка постоянных новостей';en='Submenu regular news list'");
					...
					ПодменюНовостей.Заголовок = НСтр("ka='ახალი ამბების სიის ქვემენიუ';ru='Подменю списка новостей';en='Submenu news list'");
					...
					ПунктМеню.Заголовок  = НСтр("ka='ამ თემის ყველა ახალი ამბავი';ru='Все новости по этой теме';en='All news on this topic'");
...
Функция ДобавитьКнопкуПросмотраСпискаНовостей(
	...
	Если (ТипЗнч(ЭлементКоманднаяПанель) = Тип("ГруппаФормы")) Тогда
		...
		КомандаНовость.Подсказка   = НСтр("ka='დააჭირეთ, რომ გაიხსნას კონტექსტური ახალი ამბების სია';ru='Нажмите для открытия списка контекстных новостей';en='Click to open the contextual news list'");

		КнопкаНовостей = Форма.Элементы.Добавить("КнопкаНовостей", Тип("КнопкаФормы"), ЭлементКоманднаяПанель);
		КнопкаНовостей.Заголовок             = НСтр("ka='ახალი ამბები';ru='Новости';en='news'");
	...
	Результат = 
		НРег(
			ЧислоПрописью(
				КоличествоНовостей,
				"НП=Истина; НД=Ложь;",
				НСтр("ka='ახალი ამბავი,ახალი ამბები,ახალ ამბებს,ж,,,,,0';ru='новость,новости,новостей,ж,,,,,0';en='news, news, news, Well ,,,,, 0'")));
