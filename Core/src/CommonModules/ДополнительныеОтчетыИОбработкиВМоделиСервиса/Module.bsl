...
Функция ПоставляемаяОбработка(ИспользуемаяОбработка) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ka='შემდეგი ფუნქციის გამოყენება დამატებითიანგარიშგებებიდადამუშავებებისერვისისმოდელში.მოსაწოდებელიდამუშავება() ხელმისაწვდომია მხოლოდ მონაცემთა დაყენებული დაყოფის მქონე სეანსებიდან!';ru='Использование функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ПоставляемаяОбработка()"
...
Функция ИспользуемаяОбработка(ПоставляемаяОбработка) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ka='შემდეგი ფუნქციის გამოყენება დამატებითიანგარიშგებებიდადამუშავებებისერვისისმოდელში.გამოყენებულიდამუშავება() ხელმისაწვდომია მხოლოდ მონაცემთა დაყენებული დაყოფის მქონე სეანსებიდან!';ru='Использование функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ИспользуемаяОбработка()"
"доступно только сеансов с установленным разделением данных!'");
...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, Знач СправочникиИДокументы, Знач НастройкиРасположенияКоманд, Знач ВариантыДополнительногоОтчета, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. ინიცირებულია მონაცემთა სფეროში მიწოდებული დამუშავების დაყენება.';ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';en='The included reports and obrabotki.Initsiirovana processing unit is supplied to the data area'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемаяОбработка) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაყენება მონაცემთა არიალში';ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных';en='The included reports and obrabotki.Ustanovka in the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაყენება მონაცემთა არიალში გადადებულია.';ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';en='The included reports and obrabotki.Ustanovka in the data postponed'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მონაცემთა არიალიდან წაშლა';ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных';en='Supplied obrabotki.Udalenie reports and data from the field'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. გაუქმების შეცდომა მონაცემთა სფეროდან .';ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';en='The included reports and obrabotki.Oshibka removal of the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиДополнительнойОбработкиВОбластьДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაყენების შეცდომა მონაცემთა არიალში.';ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';en='The included reports and obrabotki.Oshibka installation in the data area'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		ПоставляемаяОбработка,
	...
	Если ЭтоПоставляемаяОбработка(Источник.Ссылка) Тогда
		...
		Для Каждого КонтролируемыйРеквизит Из КонтролируемыеРеквизиты Цикл
			...
			Если ИсходныйРеквизит <> РезультирующийРеквизит Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ka='სერვისის მენეჯერის დამატებითი დამუშავებების კატალოგიდან მიღებული დამატებითი დამუშავება %2-ისთვის რეკვიზიტ %1-ის მნიშვნელობის შეცვლის დაუშვებელი მცდელობა!';ru='Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, полученной из каталога дополнительных обработок менеджера сервиса!';en='Invalid attempt to change the value of props 1% to 2% additional processing, obtained from the directory of additional treatments Manager service!'"), 
...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Или Ссылка = Справочники.ДополнительныеОтчетыИОбработки.ПустаяСсылка() Тогда
			...
			ВызватьИсключение НСтр("ka='მოთხოვნილია არარსებული დამატებითი დამუშავების შეერთება!';ru='Запрошено подключение несуществующей дополнительной обработки!';en='Requested a non-existent connection further processing!'");
...
Процедура ПриСозданииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если Не СтандартнаяОбработка Тогда
		...
		Если ИмяОбработки = Неопределено Тогда
			ВызватьИсключение НСтр("ka='მოთხოვნილია არარსებული დამატებითი დამუშავების ობიექტის შექმნა!';ru='Запрошено создание объекта несуществующей дополнительной обработки!';en='Requested object creation defunct additional treatment!'");
		КонецЕсли;
...
Процедура ПриПолученииРазрешенийСессииБезопасногоРежима(Знач КлючСессии, ОписанияРазрешений, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон(НСтр("ka='გაშვების გასაღებ %1-ისთვის მოსაწოდებელი დამუშავება არ არის აღმოჩენილი!';ru='Не обнаружена поставляемая обработка для ключа запуска %1!';en='delivered treatment is not found to run the% 1 key!'"),
				Строка(КлючСессии));
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='დამატებითი დამუშავება იდენტიფიკატორ %1-ით მოწოდებადი არ არის!';ru='Дополнительная обработка с идентификатором %1 не является поставляемой!';en='Additional treatment with ID% 1 is not supplied!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მიწოდებული დამუშავების ჩატვირთვა გაუქმებულია.';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';en='The included reports and obrabotki.Zagruzka delivered canceled processing'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				,
				НСтр("ka='მიწოდებადი დამუშავება არათავსებადია არსებულ კონფიგურაციასთან';ru='Поставляемая обработка несовместима с данной конфигурацией';en='Delivered processing is incompatible with this configuration'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Совместимость);
...
Процедура ЗапроситьОбновлениеДополнительныхОтчетовИОбработок() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების განახლებების მოთხოვნა';ru='Запрос обновлений дополнительных отчетов и обработок';en='Update request additional reports and treatments'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მიწოდებული დამუშავების ჩატვირთვა .';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки';en='Supplied obrabotki.Zagruzka reports and processing of supplied'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		НСтр("ka='ინიცირებულია მოსაწოდებელი დამუშავების ჩატვირთვა';ru='Инициирована загрузка поставляемой обработки';en='Downloading processing is initiated supplied'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Манифест);
	...
	Если ПоставляемаяОбработка.ВерсияGUID <> ОписаниеПоставляемойОбработки.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მიწოდებული დამუშავების ჩატვირთვა დასრულებულია.';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';en='The included reports and obrabotki.Zagruzka supplied complete processing'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			ПоставляемаяОбработка.Ссылка,
			НСтр("ka='მოსაწოდებელი დამუშავების ჩატვირთვა დასრულებულია';ru='Завершена загрузка поставляемой обработки';en='Loading processing is completed supplied'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Манифест);
		...
		Для Каждого ИнсталляцияОбработки Из ИспользуемыеОбработки Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაგეგმილია მოწოდებული დამუშავების მოწყობილობის აქტუალიზება.';ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';en='Supplied obrabotki.Zaplanirovana reports and updating data supplied processing'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
				НСтр("ka='მონაცემების არე:';ru='Область данных:';en='Data Field:'") + ИнсталляцияОбработки.ОбластьДанных);
		...
		Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მოწოდებული დამატებითი ანგარიშები და დამუშავებები.  დაგეგმილია მონაცემთა სფეროში მიწოდებული დამუშავების გადადებული დაყენება';ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';en='Supplied obrabotki.Zaplanirovana reports and delayed processing unit is supplied to the data area'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
				НСтр("ka='მონაცემების არე:';ru='Область данных:';en='Data Field:'") + ЭлементОчереди.ОбластьДанных);
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			Если Не ПолучитьФункциональнуюОпцию("НезависимоеИспользованиеДополнительныхОтчетовИОбработокВМоделиСервиса") Тогда
				...
				ВызватьИсключение НСтр("ka='Этот дополнительный отчет или обработка не может быть использован в сервисе!';ru='Этот дополнительный отчет или обработка не может быть использован в сервисе!';en='This additional report or treatment can not be used when!'");
...
Процедура ПроверкаВозможностиВыполненияПоставляемойОбработки(Знач ИспользуемаяОбработка)
	...
	Если ПараметрыПубликацииИспользуемойОбработки.Публикация = Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена Тогда
		ВызватьИсключение НСтр("ka='თქვენს დანართში დამატებითი დამუშავების გამოყენება აკრძალულია! დახმარებისთვის მიმართეთ მომხმარებელს, რომელსაც მოცემულ დანართში ადმინისტრატორის უფლებები გააჩნია.';ru='Использование дополнительной обработки в вашем приложении запрещено! Обратитесь за помощью к пользователю, обладающего правами администратора в данном приложении.';en='The use of additional treatment is prohibited in your application! Get help from a user with administrator rights in this application.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
		...
		Если ПараметрыПубликацииИспользуемойОбработки.Версия <> ПараметрыПубликацииПоставляемойОбработки.Версия Тогда
			ВызватьИсключение НСтр("ka='დამატებითი დამუშავების გამოყენება დროებით მიუწვდომელია მისი განახლების შესრულების გამო. მოცემულ პროცესს შეიძლება რამდენიმე წუთი დაჭირდეს. შექმნილი უხერხულობისთვის ბოდიშს გიხდით.';ru='Использование дополнительной обработки временно недоступно по причине выполнения ее обновления. Данный процесс может занять несколько минут. Приносим извинения на доставленные неудобства.';en='The use of additional treatment is temporarily unavailable due to the implementation of its renewal. This process can take several minutes. We apologize to any inconvenience caused.'");
		КонецЕсли;
