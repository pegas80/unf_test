...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, Знач СправочникиИДокументы, Знач НастройкиРасположенияКоманд, Знач ВариантыДополнительногоОтчета, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. ინიცირებულია მონაცემთა სფეროში მიწოდებული დამუშავების დაყენება.';ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемаяОбработка) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაყენება მონაცემთა არიალში';ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაყენება მონაცემთა არიალში გადადებულია.';ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მონაცემთა არიალიდან წაშლა';ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. გაუქმების შეცდომა მონაცემთა სფეროდან .';ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиДополнительнойОбработкиВОбластьДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაყენების შეცდომა მონაცემთა არიალში.';ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		ПоставляемаяОбработка,
...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Или Ссылка = Справочники.ДополнительныеОтчетыИОбработки.ПустаяСсылка() Тогда
			...
			ВызватьИсключение НСтр("ka='მოთხოვნილია არარსებული დამატებითი დამუშავების შეერთება!';ru='Запрошено подключение несуществующей дополнительной обработки!'");
...
Процедура ПриСозданииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если Не СтандартнаяОбработка Тогда
		...
		Если ИмяОбработки = Неопределено Тогда
			ВызватьИсключение НСтр("ka='მოთხოვნილია არარსებული დამატებითი დამუშავების ობიექტის შექმნა!';ru='Запрошено создание объекта несуществующей дополнительной обработки!'");
		КонецЕсли;
...
Процедура ПриПолученииРазрешенийСессииБезопасногоРежима(Знач КлючСессии, ОписанияРазрешений, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон(НСтр("ka='გაშვების გასაღებ %1-ისთვის მოსაწოდებელი დამუშავება არ არის აღმოჩენილი!';ru='Не обнаружена поставляемая обработка для ключа запуска %1!'"),
				Строка(КлючСессии));
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='დამატებითი დამუშავება იდენტიფიკატორ %1-ით მოწოდებადი არ არის!';ru='Дополнительная обработка с идентификатором %1 не является поставляемой!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მიწოდებული დამუშავების ჩატვირთვა გაუქმებულია.';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				,
				НСтр("ka='მიწოდებადი დამუშავება არათავსებადია არსებულ კონფიგურაციასთან';ru='Поставляемая обработка несовместима с данной конфигурацией'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Совместимость);
...
Процедура ЗапроситьОбновлениеДополнительныхОтчетовИОбработок() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების განახლებების მოთხოვნა';ru='Запрос обновлений дополнительных отчетов и обработок'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მიწოდებული დამუშავების ჩატვირთვა .';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		НСтр("ka='ინიცირებულია მოსაწოდებელი დამუშავების ჩატვირთვა';ru='Инициирована загрузка поставляемой обработки'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Манифест);
	...
	Если ПоставляемаяОбработка.ВерсияGUID <> ОписаниеПоставляемойОбработки.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. მიწოდებული დამუშავების ჩატვირთვა დასრულებულია.';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			ПоставляемаяОбработка.Ссылка,
			НСтр("ka='მოსაწოდებელი დამუშავების ჩატვირთვა დასრულებულია';ru='Завершена загрузка поставляемой обработки'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Манифест);
		...
		Для Каждого ИнсталляцияОбработки Из ИспользуемыеОбработки Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული დამატებითი ანგარიშები და დამუშავებები. დაგეგმილია მოწოდებული დამუშავების მოწყობილობის აქტუალიზება.';ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
				НСтр("ka='მონაცემების არე:';ru='Область данных:'") + ИнсталляцияОбработки.ОбластьДанных);
		...
		Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მოწოდებული დამატებითი ანგარიშები და დამუშავებები.  დაგეგმილია მონაცემთა სფეროში მიწოდებული დამუშავების გადადებული დაყენება';ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
				НСтр("ka='მონაცემების არე:';ru='Область данных:'") + ЭлементОчереди.ОбластьДанных);
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			Если Не ПолучитьФункциональнуюОпцию("НезависимоеИспользованиеДополнительныхОтчетовИОбработокВМоделиСервиса") Тогда
				...
				ВызватьИсключение НСтр("ka='Этот дополнительный отчет или обработка не может быть использован в сервисе!';ru='Этот дополнительный отчет или обработка не может быть использован в сервисе!'");
