...
Функция ПодготовленныеОбработчикиКлиентскогоСобытия(Событие, Служебное = Ложь, ПерваяПопытка = Истина)
	...
	Если Обработчики = Неопределено Тогда
		Если Служебное Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტური სამსახურებრივი მოვლენა ""%1"" ვერ მოიძებნა.';ru='Не найдено клиентское служебное событие ""%1"".';en='client service event is not found ""%1"".'"), Событие);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტური მოვლენა ""%1"" ვერ მოიძებნა.';ru='Не найдено клиентское событие ""%1"".';en='client event is not found ""%1"".'"), Событие);
		КонецЕсли;
	КонецЕсли;
