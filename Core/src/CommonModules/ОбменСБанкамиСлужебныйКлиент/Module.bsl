...
Процедура ЗавершитьТестПодписиСертификатаСбербанк(ПодписьВерна, Параметры) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		Параметры.Контекст.ОписаниеОшибки = НСтр("ka='ხელმოწერის შემოწმებისას შეცდომა მოხდა';ru='При проверке подписи произошла ошибка';en='When signature verification error occurred'");
	ИначеЕсли НЕ ПодписьВерна Тогда
		Параметры.Контекст.ОписаниеОшибки = НСтр("ka='დაყენებული ხელმოწერა არასწორია';ru='Установленная подпись неверна';en='Installed the signature is invalid'");
	КонецЕсли;
...
Процедура ПослеПолученияСпискаБизнесСистемСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		ТекстСообщения = НСтр("ka='ბიზნეს სისტემების სიის მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении списка бизнес систем.';en='An error occurred while retrieving the list of business systems.'");
		ТекстОшибки = НСтр("ka='გარე კომპონენტა VpnKey-TLS-მა ბიზნეს სისტემების სიის მიღებისას დააბრუნა შეცდომის კოდი %1';ru='Внешняя компонента VpnKey-TLS при получении списка бизнес систем вернула код ошибки %1';en='External VpnKey-TLS component in the preparation of business systems, the list of returned error code% 1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатВызова);
		Операция = НСтр("ka='ბიზნეს სისტემების სიის მიღება.';ru='Получение списка бизнес систем.';en='Getting business systems list.'");
...
Процедура ОбработатьОшибкуПолученияСпискаБизнесСистемНаТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	Операция = НСтр("ka='Получение списка бизнес систем на аппаратном устройстве.';ru='Получение списка бизнес систем на аппаратном устройстве.';en='Getting a list of business systems on the hardware device.'");
	ТекстСообщения = НСтр("ka='Не удалось получить список бизнес систем с аппаратного устройства';ru='Не удалось получить список бизнес систем с аппаратного устройства';en='Unable to get a business system with a list of hardware devices'");
...
Процедура ПроверитьУстановкуПодписиПослеАутентификацииНаТокенеСбербанк(Успех, Параметры) Экспорт
	...
	Если Не Успех Тогда
		ОписаниеОшибки = НСтр("ka='ბანკის გასაღებზე ავთენტიფიცირება ვერ მოხერხდა';ru='Не удалось аутентифицироваться на банковском ключе';en='Failed to authenticate the bank key'");
...
"Код ошибки: ДО-%1"
"%2';ka='საბანკო სერთიფიკატების მიღების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		...
		Операция = НСтр("ka='საბანკო სერთიფიკატების მიღება';ru='Получение банковских сертификатов';en='Obtaining bank certificates'");
	...
	Если СертификатыНаУстройстве.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='ბანკის გასაღებზე სერთიფიკატები არ არის.';ru='На банковском ключе отсутствуют сертификаты.';en='On the way there are no banking certificates.'");
...
"Код ошибки: ДО-%1"
"%2';ka='PIN-კოდის დაყენების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		...
		Операция = НСтр("ka='PIN-კოდის დაყენება ';ru='Установка PIN-кода';en='Setting the PIN-code'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
...
Процедура ПолучитьВыпискуЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, Параметры) Экспорт
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			КонецЦикла
		Иначе
			ТекстСообщения = НСтр("ka='ოპერაციის შესასრულებლად აუცილებელია საბანკო გასაღების შეერთება კომპიუტერთან';ru='Для выполнения операции необходимо подключить банковский ключ к компьютеру';en='The operation requires bank connected to a computer key'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат
		КонецЕсли;
		...
		ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка';en='Authentication Resource Bank'");
		Если Соответствие.Количество() > 0 Тогда
			...
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("ka='კომპიუტერზე შეერთებულია ბანკის არაშესაბამისი გასაღები.';ru='К компьютеру подключен не подходящий банковский ключ';en='Connected to the computer is not the right key bank'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			Если НЕ ПарольУстановлен Тогда
				...
				Иначе
					...
					ПолучитьПарольКСертификату(ОООЗ, Соответствие, НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка';en='Authentication Resource Bank'"));
			...
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("ka='კომპიუტერზე არ არის შეერთებული ბანკის გასაღები გაცვლის გამართვის მიხედვით მონაცემთა გასაგზავნად: %1';ru='К компьютеру не подключен банковский ключ для отправки данных по настройке обмена: %1';en='Connected to your computer to send the bank key for setting the exchange of data:% 1'");
		...
		Для Каждого СообщениеОбмена Из МассивСообщенийОбмена Цикл
			...
			Для Каждого СтрокаЭП Из СтруктураСодержимогоСообщенияОбмена.Подписи Цикл
				...
				Исключение
					...
					ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
					ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
					Операция = НСтр("ka='ხელმოწერის შემოწმება';ru='Проверка подписи';en='signature verification'");
					ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Код ошибки: ДО-%1"
"%2';ka='სერთიფიკატის პაროლის დაყენების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='სერთიფიკატის პაროლის დაყენება';ru='Установка пароля сертификата';en='Installing the certificate password'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
"Код ошибки: ДО-%1"
"%2';ka='დაყენებული PIN-კოდის შემოწმების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='დაყენებული PIN-კოდის არსებობის შემოწმება';ru='Проверка наличия установленного PIN-кода';en='Checking for PIN-code set'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Ложь;
...
"Код ошибки: ДО-%1"
"%2';ka='მონაცემთა გაგზავნის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='მონაცემთა გაგზავნა';ru='Отправка данных';en='sending data'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
	КонецПопытки;
...
Процедура НачатьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры) Экспорт
	...
	РезультатТеста = НСтр("ka='გავლილია წარმატებით.';ru='Пройден успешно.';en='Passed.'");
	...
	Если ВнешнийПодключаемыйМодуль = Неопределено Тогда
		ТекстСообщения = НСтр("ka='ტესტი არ არის გავლილი.';ru='Тест не пройден.';en='Fail.'");
	...
	Для Каждого Элемент Из ДоступныеСертификаты Цикл
		...
		ТекстСообщения = НСтр("ka='სერთიფიკატის შემოწმება: %1';ru='Проверка сертификата: %1';en='Checking certificate% 1'");
		...
		ПолучитьПарольКСертификату(Оповещение, Соответствие, НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка';en='Authentication Resource Bank'"));
		Возврат;
	КонецЦикла;
	
	Если ДоступныеСертификаты.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='შემოწმება სრულად არ ჩატარებულა, ვინაიდან გაცვლის გამართვაში ხელმოწერის სერთიფიკატები არ არის';ru='Проверка проведена не полностью, т.к. в настройке обмена отсутствуют сертификаты подписи';en='Checking is not fully carried out, as to set up an exchange no signing certificates'");
		СообщитьПользователю(ТекстСообщения, ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	
	// Блок проверки отправки тестового запроса.
	ОписаниеТеста = НСтр("ka='ტესტი. სატესტო მოთხოვნის გაგზავნა.';ru='Тест. Отправка тестового запроса.';en='Test. Sending a test request.'");
...
"Код ошибки: ДО-%1"
"%2';ka='სატესტო მოთხოვნის გაგზავნის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='სატესტო მოთხოვნის გაგზავნა';ru='Отправка тестового запроса';en='Sending a test request'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Код ошибки: ДО-%1"
"%2';ka='Ошибка при поиске подключенных аппаратных устройств."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='Поиск аппаратных устройств.';ru='Поиск аппаратных устройств.';en='Search hardware devices.'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Неопределено;
...
"Убедитесь, что устройство подключено к компьютеру и повторите операцию';ka='Не найдено ни одного аппаратного устройства."
		...
		Если НЕ ПодключаемыйМодуль.УстановитьСоединение(СертификатXML) Тогда
			...
			Если ТребуетсяРасширеннаяАутентификация Тогда
				Если ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
					ВызватьИсключение НСтр("ka='გაფართოებული ავთენტიფიკაციის მეთოდები არ არის დადგენილი.';ru='Не определены способы расширенной аутентификации.';en='Not defined Extensible Authentication methods.'");
				КонецЕсли;
				Если НЕ ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
					ВызватьИсключение НСтр("ka='გაფართოებული ავთენტიფიკაცია SMS-ით არ არის მხარდაჭერილი.';ru='Расширенная аутентификация по SMS не поддерживается.';en='Extended authentication via SMS is not supported.'");
				КонецЕсли;
			...
			Иначе
				ВызватьИсключение НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.';en='Connection installation failed.'");
			КонецЕсли;
		Иначе
			ВыполнитьОбработкуОповещения(Оповещение, Истина);
		КонецЕсли;
...
"Код ошибки: ДО-%1"
"%2';ka='შეერთების დაყენების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='შეერთების დაყენება';ru='Установка соединения';en='Establishing a connection'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
...
"Код ошибки: ДО-%1"
"%2';ka='PIN-კოდის შეყვანის აუცილებლობის შემოწმების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='PIN-კოდის შეყვანის აუცილებლობის შემოწმება';ru='Проверка необходимости ввода PIN-кода';en='Check the need to enter the PIN-code'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Ошибка = Истина;
...
"Причина: %1';ka='ელექტრონული ხელმოწერა არასწორია."
"მიზეზი: %1'");
...
"Проверьте настройки криптографии или обратитесь к администратору.';ka='ელექტრონული დოკუმენტის ხელმოწერების შემოწმება ვერ მოხერხდა."
"შეამოწმეთ კრიპტოგრაფიის გამართვები ან მიმართეთ ადმინისტრატორს.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
"Код ошибки: ДО-%1"
"%2';ka='სერთიფიკატის მონაცემთა მიღების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='სერთიფიკატის მონაცემთა მიღება';ru='Получение данных сертификата';en='Obtaining the certificate data'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Неопределено;
...
"Код ошибки: ДО-%1"
"%2';ka='მონაცემთა სქემის მიღების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
				ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ka='მონაცემთა სქემის მიღება';ru='Получение схемы данных';en='Preparation data schema'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
					Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
				ПрерватьПодписание = Истина;
...
"Код ошибки: ДО-%1"
"%2';ka='დოკუმენტებზე ხელმოწერის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
				ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ka='დოკუმენტების ხელმოწერა';ru='Подписание документов';en='Signing of documents'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
									Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
				ПрерватьПодписание = Истина;
...
Процедура ПродолжитьТестНастройкиОбменаПослеВводаПароляКСертификатуЧерезДополнительнуюОбработку(ПараметрВозврата, Параметры) Экспорт
	...
	Если ПараметрВозврата = Неопределено ИЛИ Параметры.СоотвСертификатовИИхСтруктур.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='სერთიფიკატისთვის პაროლი არ არის შეყვანილი. ტესტი შეწყვეტილია.';ru='Не введен пароль для сертификата."
"Тест прерван.'");
		СообщитьПользователю(ТекстСообщения, ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	...
	ОписаниеТеста = НСтр("ka='Тест. Чтение данных сертификата.';ru='Тест. Чтение данных сертификата.';en='Test. certificate data reading.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ДанныеСертификата = ДанныеСертификатаЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, СертификатXML);
	Если ДанныеСертификата = Неопределено Тогда
		Возврат;
	...
	ОписаниеТеста = НСтр("ru='Тест. Проверка наличия PIN-кода на аппаратном устройстве.';en='Test. Checking for PIN-code on a hardware device.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ЕстьОшибка = Ложь;
	ТребуетсяПИН = ТребуетсяУстановкаPINКодаХранилищаЧерезДополнительнуюОбработку(
	...
	Если ТребуетсяПИН Тогда
		ОписаниеТеста = НСтр("ru='Тест. Установка PIN-кода на аппаратном устройстве.';en='Test. Setting the PIN-code on a hardware device.'");
		СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
		ПараметрыФормы = Новый Структура;
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработкуПослеАутентификации(АутентификацияВыполнена, Параметры) Экспорт
	...
	ОписаниеТеста = НСтр("ka='Тест. Установка подписи.';ru='Тест. Установка подписи.';en='Test. Setting a signature.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	...
	Исключение
		...
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='ხელმოწერის დაყენება';ru='Установка подписи';en='installation captions'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат;
	...
	ОписаниеТеста = НСтр("ka='Тест. Проверка подписи.';ru='Тест. Проверка подписи.';en='Test. signature verification.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	Попытка
	...
	Если НЕ ПодписьВалидна Тогда
		СообщитьПользователю(НСтр("ka='Подпись не валидна';ru='Подпись не валидна';en='The signature is not valid'"), ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	СообщитьПользователю(РезультатТеста, ИдентификаторНазначения);
...
Процедура НачатьПроцессПолученияВыпискиЧерезВК(НастройкаОбмена, ДатаНачала, ДатаОкончания, НомерСчета, Владелец) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			...
			ТекстСообщения = НСтр("ka='დოკუმენტის ხელმოსაწერად შესაფერისი სერთიფიკატი ვერ მოიძებნა ამონაწერის მოთხოვნა';ru='Не найден подходящий сертификат для подписи документа Запрос выписки';en='Unable to find a suitable certificate to sign the document discharge request'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			ВидОперации = НСтр("ka='Подписание электронных документов';ru='Подписание электронных документов';en='The signing of electronic documents'");
...
Процедура ПослеОшибкиВключенияЖурналированияВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ჟურნალირების ჩართვისას შეცდომა მოხდა.';ru='При включении журналирования произошла ошибка.';en='When you turn on logging error.'");
	ВидОперации = НСтр("ka='ჟირნალირების ჩართვა.';ru='Включение журналирования.';en='Enable logging.'");
...
Процедура ПослеПолученияИнформацииОбОшибкеВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ШаблонОшибки = ДополнительныеПараметры.ТекстСообщения + Символы.ПС + НСтр("ka='შეცდომის კოდი: ВК-%1"
"%2';ru='Код ошибки: ВК-%1"
"%2'");
...
Процедура ПослеПопыткиПодключенияВнешнейКомпоненты(Результат, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка подключения внешнего модуля банка.';en='Error connecting an external unit of the bank.'");
			Операция = НСтр("ru='Подключение внешней компоненты банка';en='Connecting an external component of the Bank'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		КонецПопытки;
...
Процедура ПослеПодключенияВнешнейКомпоненты(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		Попытка
			...
			ЗакэшироватьПодключаемыйМодуль(Параметры.НазваниеВК, ПодключаемыйМодуль);
		Исключение
			ТекстСообщения = НСтр("ka='ბანკის გარე კომპონენტის შეერთების შეცდომა.';ru='Ошибка подключения внешней компоненты банка.';en='Error connecting an external component of the Bank.'");
			Операция = НСтр("ka='ბანკის გარე მოდულის შეერთება';ru='Подключение внешнего модуля банка';en='Connecting an external bank module'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		КонецПопытки;
...
Функция ТекстСообщенияСбербанк(Тикет)
	...
	Если Тикет = "00000000-0000-0000-0000-000000000000" Тогда
		ТекстСообщения = НСтр("ka='ბანკის სერვერი დროებით მიუწვდომელია. განმეორებით სცადეთ ან მიმართეთ საკუთარ ბანკს.';ru='Сервер банка временно недоступен. Повторите попытку или обратитесь в свой банк.';en=""the bank's server is temporarily unavailable. Please try again or contact your bank.""");
...
Процедура НайтиБизнесСистемуПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НомерБизнесСистемы = Неопределено Тогда
		ТекстСообщения = НСтр("ka='საბანკო გასაღებზე ბიზნეს სისტემა ვერ მოიძებნა. აუცილებელია TLS VPN Key-ის მუშაობის შემოწმება.';ru='Не найдена бизнес система на банковском ключе."
"Необходимо проверить работу TLS VPN Key.'");
		Операция = НСтр("ka='Поиск бизнес системы на электронном ключе.';ru='Поиск бизнес системы на электронном ключе.';en='Search business systems on the electronic key.'");
		ШаблонОшибки = НСтр("ka='ელექტრონულ გასაღებზე ბიზნეს-სისტემა %1 ვერ მოიძებნა."
"დაბრუნების სიის შემცველობა: %2';ru='На электронном ключе не найдена бизнес-система %1."
...
Процедура ОбработатьОшибкуПослеПредъявленияПинаНаТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ВидОперации = НСтр("ru='Предъявление кода доступа на аппаратном устройстве';en='Presentation of the access code on a hardware device,'");
...
Процедура ПолучитьИнформациюОТокенеСбербанк(Оповещение, ПодключаемыйМодуль)
	...
	Исключение
		Операция = НСтр("ru='Получение информации об аппаратном устройстве.';en='Receiving information about the hardware device.'");
...
Процедура ОбработатьОшибкуПолученияИнформацииОТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ШаблонОшибки = НСтр("ru = 'При получении информации об аппаратном устройстве произошла ошибка.
						|%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение информации об аппаратном устройстве.';en='Receiving information about the hardware device.'");
...
Процедура ОбработатьРезультатАсинхроннойПодписиСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		Операция = НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных';en='Signing data'");
		ТекстОшибки = НСтр("ru='При подписании электронного документа произошла ошибка"
"Внешний модуль VpnKey-TLS при подписании вернул код ошибки %1';ka='При подписании электронного документа произошла ошибка"
...
"%1';ka='მონაცემთა ხელმოწერისას შეცდომა მოხდა."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='მონაცემთა ხელმოწერა.';ru='Подпись данных.';en='Information Signature.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
	Результат = Новый Структура;
	Результат.Вставить("Успех", Ложь);
...
Процедура ПослеПолученияДанныхСертификатаСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		ТекстСообщения = НСтр("ka='სერთიფიკატის მონაცემთა მიღებისას შეცდომა მოხდა.';ru='При получении данных сертификата произошла ошибка.';en='Upon receipt of the certificate data error has occurred.'");
		ТекстОшибки = НСтр("ka='სერთიფიკატის მიღებისას გარე მოდულ VpnKey-TLS-მა დააბრუნა შეცდომის კოდი';ru='Внешний модуль VpnKey-TLS при получении сертификата вернул код ошибки';en='External module VpnKey-TLS certificate when we receive the returned error code'") + РезультатВызова;
		Операция = НСтр("ka='Получение сертификата криптографии.';ru='Получение сертификата криптографии.';en='Getting cryptographic certificate.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
...
Процедура ОбработатьОшибкуПослеПолученияДанныхСертификатаСТокенаСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ШаблонОшибки = НСтр("ru = 'При получении сертификата с аппаратного устройства произошла ошибка.
						|%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение данных сертификата с аппаратного устройства.';en='Preparation of certificate data from a hardware device.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияДанныхСертификата, Неопределено);
...
Процедура ПроверитьУстановкуПодписиПолученияСертификатовСТокенаСбербанк(Результат, Параметры) Экспорт
	...
	Для Каждого КлючЗначение Из СоответствиеСертификатов Цикл
		...
		Исключение
			ТекстСообщения = НСтр("ka='სერთიფიკატის წაკითხვის შეცდომა.';ru='Ошибка чтения сертификата.';en='Error reading certificate.'");
		...
		Если СтруктураСертификата.Отпечаток = РеквизитыПроверяемогоСертификата.Отпечаток Тогда
			...
			Представление = НСтр("ka='სატესტო სტრიქონი';ru='Тестовая строка';en='test line'");
	...
	ТекстСообщения = НСтр("ru='Сертификат не найден на аппаратном устройстве.';en='Certificate can not be found on the hardware device.'");
...
"Внешний модуль VpnKey-TLS при получении списка доступных сертификатов вернул код ошибки: %1';ka='სერთიფიკატების სიის მიღებისას ბანკის გასაღებზე შეცდომა მოხდა."
"ხელმისაწვდომი სერთიფიკატების სიის მიღებისას გარე მოდულ VpnKey-TLS-მა დააბრუნა შეცდომის კოდი: %1'");
		ТекстОшибки = СтрШаблон(ШаблонОшибки, РезультатВызова);
		Операция = НСтр("ka='Подписание электронного документа.';ru='Подписание электронного документа.';en='The signing of the electronic document.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
		ОчиститьДанныеАвторизацииСбербанк();
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстОшибки);
	...
	Если СоответствиеСертификатов.Количество() Тогда
		...
		Возврат;
	Иначе
		ТекстСообщения = НСтр("ru='Не найден ни один сертификат на аппаратном устройстве.';en='Unable to find any certificate to the hardware device.'");
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
	КонецЕсли;
...
Процедура ОбработатьОшибкуПолученияСпискаИдентификаторовСертификатов(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ru='Получение списка сертификатов с аппаратного устройства.';en='Obtaining a list of certificates with the hardware device.'");
	ТекстСообщения = НСтр("ru='Не удалось получить информацию о сертификатах с аппаратного устройства.';en='Unable to get information about certificates from a hardware device.'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
	Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
...
Процедура ПодобратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Исключение
			...
			Операция = НСтр("ka='Чтение данных сертификата.';ru='Чтение данных сертификата.';en='certificate data reading.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	ЗаголовокФормыВыбора = НСтр("ka='აირჩიეთ ხელმოწერის სერთიფიკატი';ru='Выберите сертификат подписи';en='Select the signing certificate'");
...
Процедура ОбработатьОшибкуПослеПроверкиПодписиНаТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ШаблонОшибки = НСтр("ru = 'При аутентификации на аппаратном устройстве произошла ошибка.
						|%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Предъявление данных аутентификации на аппаратном устройстве.';en='Presentation data authentication hardware device.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
"Внешний модуль VpnKey-TLS при подписании вернул код ошибки %1'");
	...
	ИначеЕсли Состояние = "FAILED" ИЛИ Состояние = "REJECTED" Тогда
		Операция = НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных';en='Signing data'");
		ТекстОшибки = НСтр("ru='При подписании электронного документа произошла ошибка"
"Код состояния: %1';ka='При подписании электронного документа произошла ошибка"
"Код состояния: %1'");
...
"%1';ka='ხელმოწერის მონაცემთა მიღებისას შეცდომა მოხდა:"
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='ხელმოწერის მონაცემთა მიღება';ru='Получение данных подписи';en='Preparation signature data'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
Процедура ПодписатьТестовыйЗапросПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	ПодключаемыйМодуль = ПодключеннаяВнешняяКомпонентаБанка("VPNKeyTLS");
	Представление = НСтр("ka='სატესტო მოთხოვნა';ru='Тестовый запрос';en='test request'");
...
Процедура ОтправитьТестовыйЗапросПослеПроверкиПодписиСбербанк(ПодписьВерна, Параметры) Экспорт
	...
	Если Не ПодписьВерна Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Установленная подпись неверна.';en='Installed the signature is invalid.'"));
		Результат = Новый Структура("Успех", Ложь);
		ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеТестаНастройки, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПослеОтправкиЗапросаВСбербанк(РезультатВызова, ПараметрыВызова, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(КодОшибки) ИЛИ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВидОперации = НСтр("ka='მოთხოვნის გაგზავნა ბანკში.';ru='Отправка запроса в банк';en='Sending a request to the bank'");
		ТекстСообщения = НСтр("ru='Нет связи с сервером банка."
"Повторите операцию позже или обратитесь в техподдержку банка';ka='ბანკის სერვერთან არ არის კავშირი. ოპერაცია მოგვიანებით გაიმეორეთ ან მიმართეთ ბანკის ტექდახმარებას'");
		ШаблонОшибки = НСтр("ru='Произошла ошибка сетевого взаимодействия."
"Код ошибки: %1"
"Текст ошибки: %2';ka='მოხდა ქსელური ურთიერთქმედების შეცდომა."
"შეცდომის კოდი: %1"
"შეცდომის ტექსტი: %2'");
	...
	Если РезультатВызова = "<!--REQUESTID_DUBLIC-->" Тогда
		ТекстОшибки = НСтр("ru='Документ с таким идентификатором есть в информационной базе банка. Повторная отправка невозможна.';en=""Document with this ID is in the bank's information base. Resending is not possible.""");
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстОшибки);
		ВыполнитьОбработкуОповещения(Параметры.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
"%1';ka='ბანკში მოთხოვნის გაგზავნისას შეცდომა მოხდა."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='მოთხოვნის გაგზავნა ბანკში.';ru='Отправка запроса в банк.';en='Sending a request to the bank.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
...
Процедура ПослеПолученияДокументаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(КодОшибки) ИЛИ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВидОперации = НСтр("ka='დოკუმენტების მიღება ბანკიდან.';ru='Получение документов из банка.';en='Receiving documents from the bank.'");
		ШаблонОшибки = НСтр("ru='Произошла ошибка сетевого взаимодействия."
"Код ошибки: %1."
"Текст ошибки: %2';ka='მოხდა ქსელური ურთიერთქმედების შეცდომა."
"შეცდომის კოდი: %1."
"შეცდომის ტექსტი: %2'");
		ТекстЗаписи = СтрШаблон(ШаблонОшибки, КодОшибки, ТекстОшибки);
		ТекстСообщения = НСтр("ru='Нет связи с сервером банка."
"Повторите операцию позже или обратитесь в техподдержку банка.';ka='ბანკის სერვერთან კავშირი არ არის. მოგვიანებით გაიმეორეთ ოპერაცია ან მიმართეთ ბანკის ტექმხარდაჭერას.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%1';ka='ბანკიდან დოკუმენტების მიღებისას შეცდომა მოხდა. %1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='დოკუმენტების მიღება ბანკიდან.';ru='Получение документов из банка.';en='Receiving documents from the bank.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
...
Процедура ОтправитьЗапросВыпискиСбербанк(Оповещение, НастройкаОбмена, ЗапросВыписки) Экспорт
	...
	ВидОперации = НСтр("ka='ბანკის ამონაწერის მოთხოვნა';ru='Запрос выписки банка';en='Request bank statements'");
...
Процедура ПослеУстановкиКаналаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВызова = 0 Тогда
		Операция = НСтр("ka='Установка виртуального канала';ru='Установка виртуального канала';en='Installing a virtual channel'");
		ТекстСообщения = НСтр("ru='Не удалось установить связь с сервером."
"Необходимо проверить работу TLS VPN Key.';ka='სერვერთან კავშირის დამყარება ვერ მოხერხდა."
"აუცილებელია TLS VPN Key-ის მუშაობის შემოწმება.'");
		ТекстОшибки = НСтр("ka='ვირტუალური არხის დაყენებისას კომპონენტა AddIn.Bicrypt-მა შეცდომის კოდი დააბრუნა';ru='Компонента AddIn.Bicrypt при установке виртуального канала вернула код ошибки';en='AddIn.Bicrypt component during the installation of the virtual channel has returned an error code'")
							+ " " + РезультатВызова;
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ОчиститьДанныеАвторизацииСбербанк();
	КонецЕсли;
...
Процедура ОбработатьОшибкуУстановкиКаналаНаТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ШаблонОшибки = НСтр("ru = 'При установке канала на аппаратном устройстве произошла ошибка.
						|%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Установка канала на аппаратном устройстве.';en='Installation channel hardware device.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
"%1';ka='ბანკის გარე მოდულის ვერსიის მიღებისას შეცდომა მოხდა: %1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='ბანკის გარე კომპონენტის ვერსიის მიღება';ru='Получение версии внешней компоненты банка';en='Obtaining version foreign bank components'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
...
Процедура ПолучитьПараметрыТокенаПослеТихогоСтартаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		...
		ТекстОшибки = НСтр("ru='Внешний модуль VpnKey-TLS при запуске Start.exe вернул код ошибки %1';en='External module VpnKey-TLS Start.exe at startup returned error code 1%'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатВызова);
		Операция = НСтр("ru='Запуск клиента аппаратного устройства.';en='Running the client hardware device.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ОчиститьДанныеАвторизацииСбербанк();
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаТокене, Ложь);
		Возврат;
...
"%1';ka='ბანკის პროგრამის გაშვებისას შეცდომა მოხდა:"
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='პროგრამის გაშვება Start.exe';ru='Запуск программы Start.exe';en='Launch Start.exe program'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаТокене, Ложь);
...
Процедура ОбработатьРезультатАутентификацииНаТокенеСбербанка(НастройкаОбмена, РезультатАвторизации, ОповещениеПослеАутентификацииНаТокене)
	...
	Если Не (РезультатАвторизации = 0) Тогда
		Если РезультатАвторизации = 28 Тогда
			ШаблонСообщения = НСтр("ka='PIN%1 დაბლოკილია';ru='PIN%1 заблокирован';en='1 blocked PIN%'");
		...
		ИначеЕсли РезультатАвторизации = 27 Тогда
			ШаблонСообщения = НСтр("ka='PUK%1 დაბლოკილია';ru='PUK%1 заблокирован';en='1 blocked PUK%'");
			ТекстСообщения = СтрШаблон(
								ШаблонСообщения, ЗначениеИзКэшаСбербанк("НомерКонтейнера"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли РезультатАвторизации = 25 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მომხმარებელი დაბლოკილია';ru='Пользователь заблокирован';en='the user is blocked'"));
		ИначеЕсли РезультатАвторизации = 29 ИЛИ РезультатАвторизации = 30 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Неверные данные аутентификации';en='Incorrect authentication data'"));
		Иначе
			ОчиститьСообщения();
			ТекстСообщения = НСтр("ru = 'Не удалось выполнить аутентификацию на аппаратном устройстве.
										|Необходимо выполнить его перезапуск'");
			ТекстОшибки = НСтр("ru='Внешний модуль VpnKey-TLS при авторизации на аппаратном устройстве вернул код ошибки: %1';en='External module VpnKey-TLS authentication when the hardware device has returned an error code: 1%'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатАвторизации);
			Операция = НСтр("ka='ტოკენზე ავტორიზაცია';ru='Авторизация на токене';en='Authorization token'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ПослеПолученияОтпечатковВыполнитьОбмен(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если МассивСертификатов.Количество() Тогда
		...
		ОписаниеДанных.Вставить("Операция", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'"));
...
"Проверьте настройки прямого обмена с банком или обратитесь к администратору';ka='ბანკის სერვერზე ავთენტიფიკაციისთვის ხელმისაწვდომი სერთიფიკატები არ არის."
"შეამოწმეთ ბანკთან პირდაპირი გაცვლის გამართვები ან მიმართეთ ადმინისტრატორს'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		МассивСертификатов = Новый Массив;
		...
		Если МассивСертификатов.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერზე ავთენტიფიკაციისთვის კომპიუტერზე სერთიფიკატები ვერ მოიძებნა."
"სერთიფიკატები დააყენეთ საოპერაციო სისტემის სერთიფიკატების პირად საცავში.';ru='На компьютере не найдены сертификаты для аутентификации на сервере банка."
...
Процедура ВыполнитьОбменСБанками(Результат, Параметры) Экспорт
	...
	Если НЕ Параметры.Свойство("НастройкиОбмена", НастройкиОбмена)
		...
		ЗаголовокОповещения = НСтр("ru='Обмен с сервисом 1С:ДиректБанк';en='Exchange with the service of 1C: DirektBank'");
		ШаблонОповещения = НСтр("ka='გაგზავნილია პაკეტი: (%1), მიღებულია პაკეტი: (%2).';ru='Отправлено пакетов: (%1), получено пакетов: (%2).';en='Sent packages (1%) received packets (2%).'");
...
Процедура ВыполнитьОбменСБанком(Параметры)
	...
	ТекстСообщения = НСтр("ka='Отправка пакетов электронных документов в банк. Подождите...';ru='Отправка пакетов электронных документов в банк. Подождите...';en='Sending packages of electronic documents to the bank. Wait...'");
	Состояние(НСтр("ka='Отправка.';ru='Отправка.';en='Sending.'"), , ТекстСообщения);
...
Процедура ВыполнитьСинхронизациюПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	Если ОтправлятьЗапросНовыхДокументов Тогда
		...
		ПодключаемыйМодуль = ПодключеннаяВнешняяКомпонентаБанка("VPNKeyTLS");
		Представление = НСтр("ka='ახალი დოკუმენტების მოთხოვნა';ru='Запрос новых документов';en='Request for new documents'");
		Параметры.Вставить("ИдентификаторЗапроса", ИдентификаторЗапроса);
		Параметры.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
...
Процедура ПослеПолученияОтпечатковПроверитьСвязь(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			ИЛИ НЕ НастройкиОбмена.СертификатДоступен Тогда
			ТекстСообщения = НСтр("ka='დოკუმენტ ""მოთხოვნა-ზონდი""-ზე ხელმოსაწერად შესაბამისი სერთიფიკატი ვერ მოიძებნა';ru='Не найден подходящий сертификат для подписи документа Запрос-зонд';en='Unable to find a suitable certificate to sign a document request probe'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			СтруктураРезультата = Новый Структура("Успех", Ложь);
			ВыполнитьОбработкуОповещения(Параметры.ОбработчикПослеТестаНастройки, СтруктураРезультата);
			Возврат;
		...
		Операция = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа';en='The signing of the electronic document'");
		ОписаниеДанных = Новый Структура;
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ТекстЗаголовка = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк';en='1C: DirektBank'");
...
Процедура ПолучитьДанныеАутентификации(НастройкаОбмена, ОписаниеОповещенияОЗакрытии, ЭтоТест = Ложь) Экспорт
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ВидОперации", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'"));
...
Процедура ОбработатьОшибкуПолученияСертификатаИзПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ka='სერთიფიკატის მიღება ხელმოწერიდან';ru='Получение сертификата из подписи';en='Obtain a certificate from a signature'");
	ТекстСообщения = НСтр("ka='ხელმოწერიდან სერთიფიკატის მიღება ვერ მოხერხდა';ru='Не удалось получить сертификат из подписи';en='Unable to get a certificate from a signature'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
...
Процедура ОбработатьОшибкуВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ka='ხელმოწერის სერთიფიკატის გადმოტვირთვა';ru='Выгрузка сертификата подписи';en='signing certificate Unloading'");
	ТекстСообщения = НСтр("ka='მიღებული ხელმოწერის სერთიფიკატის გადმოტვირთვა ვერ მოხერხდა';ru='Не удалось выгрузить сертификат полученной подписи';en='Failed to upload the certificate obtained signatures'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеПолученияСертификатов, Неопределено);
...
"Код ошибки: ДО-%1"
"%2';ka='დამატებითი დამუშავების ინიციალიზაციის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
			ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
			Операция = НСтр("ka='გარე დამუშავების ინიციალიზაცია';ru='Инициализация внешней обработки';en='Initialize external processing'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Ложь;
...
"Код ошибки: ДО-%1"
"%2';ka='ინიციალიზაციის დასრულების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
				ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ka='გარე დამუშავების ინიციალიზაციის დასრულება';ru='Завершение инициализации внешней обработки';en='Completion of the initialization external processing'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
					Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
				Возврат Ложь;
...
Процедура ОбработатьОтклонениеЭД(СообщениеОбмена, ОписаниеОповещения) Экспорт
	...
	Заголовок = НСтр("ka='მიუთითეთ დოკუმენტის უარყოფის მიზეზები';ru='Укажите причины отклонения документа';en='Identify the reasons for rejection of the document'");
...
Процедура ОбработатьОтклонениеЭДЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат <> Неопределено Тогда
		ТекстСообщения = НСтр("ru='Причина не указана, действие отменено.';en='The reason is not specified, the action is canceled.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПослеОпределенияСтатусаОчереднойПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ДополнительныеПараметры.РезультатПроверки.Добавить(СтруктураЗаписи);
	ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ka='ხელმოწერის შემოწმება';ru='Проверка подписи';en='signature verification'");
		...
		Для Каждого Элемент Из СертификатыСообщенийОбмена Цикл
			...
			Если Структура.Свойство("СоответствиеСообщенийОбменаИДД", СоответствиеСообщенийОбменаИДД)
				...
				Если СоответствиеСообщенийОбменаИДД.Количество() = 1 Тогда
					Операция = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа';en='The signing of the electronic document'");
				Иначе
					Операция = НСтр("ka='Подписание электронных документов';ru='Подписание электронных документов';en='The signing of electronic documents'");
				КонецЕсли;
				...
				ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ka='დოკუმენტი';ru='Документ';en='Document'"));
				ОписаниеДанных.Вставить("БезПодтверждения",    Истина);
				...
				Иначе
					ПредставлениеСообщенияОбмена = НСтр("ka='Электронные документы (%1)';ru='Электронные документы (%1)';en='Electronic documents (1%)'");
...
Процедура ТестНастройкиОбменаЧерезДополнительнуюОбработку(НастройкаОбмена, ИдентификаторНазначения)
	...
	ОписаниеТеста = НСтр("ka='Тест. Проверка наличия внешней обработки для обмена с банком.';ru='Тест. Проверка наличия внешней обработки для обмена с банком.';en='Test. Check for an external data processor to the bank exchange.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ВнешняяОбработкаПодключена = ОбменСБанкамиСлужебныйВызовСервера.ПодключитьАктуальнуюВнешнююОбработку(
		РеквизитыНастройкиОбмена.ИмяВнешнегоМодуля, АдресФайла);
	Если Не ЗначениеЗаполнено(АдресФайла) Тогда
		ТекстСообщения = НСтр("ru='В информационной базе отсутствует внешний модуль обмена с банком.';en='The information base with no external bank exchange module.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	Если Не ВнешняяОбработкаПодключена Тогда
		Возврат;
	...
	ОписаниеТеста = НСтр("ka='Тест. Инициализация служебного интерфейса.';ru='Тест. Инициализация служебного интерфейса.';en='Test. Initialization of the service interface.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры)
	...
	ОписаниеТеста = НСтр("ka='Тест. Аутентификация на ресурсе банка.';ru='Тест. Аутентификация на ресурсе банка.';en=""Test. Authentication on the Bank's resources.""");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка';en='Authentication Resource Bank'");
	Попытка
...
"Код ошибки: ДО-%1"
"%2';ka='ბანკის რესურსზე ავტორიზაციის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка';en='Authentication Resource Bank'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		Возврат;
	...
	ОписаниеТеста = НСтр("ka='Тест. Установка соединения с банком.';ru='Тест. Установка соединения с банком.';en='Test. Establishing a connection with the bank.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
Процедура ПодписатьЗапросыВыписок(Результат, ПараметрыОбработки)
	...
	ПредставлениеНабора = НСтр("ka='ამონაწერების მოთხოვნები (%1)';ru='Запросы выписок (%1)';en='Request extracts (1%)'");
...
Процедура ПослеПолученияВерсииВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВызова = ДополнительныеПараметры.ОжидаемаяВерсия Тогда
		...
		#Иначе
			ТекстСообщения = НСтр("ru='Отличаются версии используемого и загруженного внешнего модуля."
"Перезапустите программу.';ka='Отличаются версии используемого и загруженного внешнего модуля."
...
Процедура ОбработатьОшибкуПолученияВерсииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='გარე კომპონენტის ვერსიის მიღება.';ru='Получение версии внешней компоненты.';en='Obtaining version of external components.'");
	ТекстСообщения = НСтр("ka='გარე მოდულის ვერსიის მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении версии внешнего модуля.';en='An error occurred while getting the version of the plug.'");
...
Процедура ПолучитьПарольАутентификацииПослеОпределенияСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'") + " "; // Для запроса пароля сертификата
...
Процедура ОбработатьОшибкуПроверкиНеобходимостиУстановкиПароляВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ავთენტიფიკაციის დროს შეცდომა მოხდა.';ru='При аутентификации произошла ошибка.';en='An error occurred while authenticating.'");
	ВидОперации = НСтр("ka='პაროლის დაყენების აუცილებლობის შემოწმება.';ru='Проверка необходимости установки пароля.';en='Check the password setup required.'");
...
Процедура ОбработатьОшибкуПолученияХранилищКлючейЭП(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Получение информации об аппаратных устройствах.';en='Obtaining information of hardware devices.'");
	ТекстСообщения = НСтр("ru='При поиске аппаратных устройств, подключенных к компьютеру, произошла ошибка.';en='When looking for hardware devices connected to the computer, an error occurred.'");
...
Процедура ПродолжитьОтправкуПлатежныхДокументовПоНастройкамиПослеПолученияНовыхДокументовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат.Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ka='ახალი დოკუმენტების მიღება ბანკიდან.';ru='Получение новых документов из банка.';en='Getting the new documents from the bank.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбработатьОшибкуПроверкиДополненияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='დოკუმენტზე ხელმოწერისას შეცდომა მოხდა.';ru='При подписании документа произошла ошибка.';en='An error occurred while signing the document.'");
	ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის დამატებით შევსების შემოწმება.';ru='Проверка дополнения электронного документа.';en='Check the electronic document supplements.'");
...
Процедура ОбработатьОшибкуДополненияЭДЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='დოკუმენტზე დამატებების გაკეთებისას შეცდომა მოხდა.';ru='При дополнении документа произошла ошибка.';en='an error occurred while adding the document.'");
	ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის დამატება.';ru='Дополнение электронного документа.';en='Addition of an electronic document.'");
...
Процедура ПослеПолученияДанныхСертификатовСКлючаВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДоступныеСертификаты.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='В настройке обмена с сервисом 1С:ДиректБанк нет ни одного доступного сертификата.';en='In setting up the exchange with the service of 1C: DirektBank No certificate available.'");
	...
	Если ДанныеВыбора.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='საბანკო გასაღებზე ვერ მოიძებნა ბანკთან გაცვლის გასწორებაში მითითებული სერთიფიკატები.';ru='На банковском ключе не найдены сертификаты, указанные в настройке обмена с банком.';en='The bank key is not found certificates referred to in setting a bank exchange.'");
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеТестаНастройки, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если НастройкиОбмена.Подписывать Тогда
		ВидОперации = НСтр("ka='Подписание электронного документа.';ru='Подписание электронного документа.';en='The signing of the electronic document.'");
		Оповещение = Новый ОписаниеОповещения(
...
Процедура ОбработатьОшибкуОтправкиПакетаЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ბანკში მონაცემთა გაგზავნისას შეცდომა მოხდა.';ru='При отправке данных в банк произошла ошибка.';en='When sending data to a bank error occurred.'");
	ВидОперации = НСтр("ka='ელექტრონული დოკუმენტების პაკეტების გაგზავნა.';ru='Отправка пакетов электронных документов.';en='Sending packages of electronic documents.'");
...
Процедура ПослеОтправкиПакетаЧерезВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		ВидОперации = НСтр("ka='დოკუმენტის გაგზავნა ბანკში';ru='Отправка документа в банк';en='Sending documents to the bank'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбработатьОшибкуПолученияСпискаПакетов(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ბანკიდან დოკუმენტების პაკეტების სიის მიღების შეცდომა.';ru='Ошибка получения списка пакетов документов из банка.';en='Failed to get a list of packages of documents from the bank.'");
	ВидОперации = НСтр("ka='პაკეტების სიის მიღება.';ru='Получение списка пакетов.';en='Getting list of packages.'");
...
Процедура ОбработатьОшибкуПолученияПакетаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ბანკიდან დოკუმენტების პაკეტის მიღების შეცდომა.';ru='Ошибка получения пакета документов из банка.';en='Failure to get a package of documents from the bank.'");
	ВидОперации = НСтр("ka='ბანკიდან პაკეტის მიღება.';ru='Получение пакета из банка.';en='Getting a package from the bank.'");
...
Процедура ОбработатьОшибкуПроверкиПодписиЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При проверке подписи документа произошла ошибка.';en='When checking the signature of the document error.'");
	ВидОперации = НСтр("ru='Проверка подписи электронного документа.';en='Check the electronic document signatures.'");
...
Процедура ОбработатьОшибкуПодготовкиПодписиЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='При подготовки подписи электронного документа произошла ошибка.';ru='При подготовки подписи электронного документа произошла ошибка.';en='An error occurred while preparing an electronic document signatures.'");
	ВидОперации = НСтр("ka='Подготовка подписи электронного документа.';ru='Подготовка подписи электронного документа.';en='Preparing the signature of an electronic document.'");
...
Процедура ОбработатьОшибкуПодписанияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ელექტრონულ დოკუმენტზე ხელმოწერისას შეცდომა მოხდა.';ru='При подписании электронного документа произошла ошибка.';en='When signing an electronic document the error occurred.'");
	ВидОперации = НСтр("ka='Подписание электронного документа.';ru='Подписание электронного документа.';en='The signing of the electronic document.'");
	ПолучитьИнформациюОбОшибкеВК(ДополнительныеПараметры.ОповещениеПослеПодписанияЭД,
		ДополнительныеПараметры.ПодключаемыйМодуль, ВидОперации, ТекстОшибки);
...
Процедура ОбработатьОшибкуРасширеннойАутентификации(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='SMS-ით ავთენტიფიკაციის შეცდომა.';ru='Ошибка аутентификации по SMS.';en='Error by SMS authentication.'");
	ВидОперации = НСтр("ka='ერთჯერადი პაროლის გაგზავნა SMS-დან.';ru='Отправка одноразового пароля из SMS.';en='Sending of SMS one-time password.'");
...
Процедура ОбработатьОшибкуОтправкиПароляПоSMSВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='SMS-ით ერთჯერადი პაროლის გაგზავნის შეცდომა.';ru='Ошибка отправки одноразового пароля по SMS.';en='Error sending one-time password by SMS.'");
	ВидОперации = НСтр("ka='ერთჯერადი პაროლის გაგზავნა SMS-ით.';ru='Отправка одноразового пароля в SMS.';en='Sending a one-time password via SMS.'");
	ПолучитьИнформациюОбОшибкеВК(ДополнительныеПараметры.ОповещениеПослеУстановкиСоединения,
		ДополнительныеПараметры.ПодключаемыйМодуль, ВидОперации, ТекстСообщения);
...
Процедура ОбработатьОшибкуУстановкиСоединенияВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='ბანკთან შეერთების დაყენებისას შეცდომა მოხდა.';ru='При установке соединения с банком произошла ошибка.';en='When a connection is established with a bank error.'");
	ВидОперации = НСтр("ka='ბანკთან შეერთების დაყენება.';ru='Установка соединения с банком.';en='Establishing a connection with the bank.'");
...
Процедура ОбработатьОшибкуДополненияДанныхКлючаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='ელექტრონულ გასაღებთან მუშაობისას შეცდომა მოხდა.';ru='При работе с электронным ключем произошла ошибка.';en='When working with an electronic key error.'");
	ВидОперации = НСтр("ka='გასაღების მონაცემების დამატებით შევსება.';ru='Дополнение данных ключа.';en='Addition key data.'");
...
Процедура ОбработатьОшибкуУстановкиПароляВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='პაროლის დაყენების შეცდომა.';ru='Ошибка установки пароля.';en='setting a password error.'");
	ВидОперации = НСтр("ka='გასაღების პაროლის დაყენება.';ru='Установка пароля ключа.';en='Setting the key password.'");
...
Процедура ВыбратьКлючПослеПолученияХранилищКлючейЭП(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Количество() > 1 Тогда
		...
		ЗаголовокВыбора = НСтр("ka='აირჩიეთ ელექტრონული გასაღები:';ru='Выберите электронный ключ:';en='Select the electronic key:'");
...
Процедура ОбработатьОшибкуПроверкиНеобходимостиПИНВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='პინ-კოდის დაყენების აუცილებლობის შემოწმება.';ru='Проверка необходимости установки ПИН-кода.';en='Check the need to install PIN.'");
	ТекстСообщения = НСтр("ka='პინ-კოდის საჭიროების შემოწმებისას შეცდომა მოხდა.';ru='При проверке необходимости ПИН-кода произошла ошибка.';en='When checking the necessary PIN error occurred.'");
...
Процедура ОбработатьОшибкуУстановкиПинВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Установка PIN кода.';en='Setting the PIN code.'");
	ТекстСообщения = НСтр("ka='PIN კოდის დაყენებისას შეცდომა მოხდა.';ru='При установки PIN кода произошла ошибка.';en='When the installation error occurred PIN.'");
...
Процедура ОбработатьОшибкуПолученияДанныхСертификатаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='ეხ-ის გასაღების მონაცემთა მიღება';ru='Получение данных ключа ЭП.';en='Preparation signature key data.'");
	ТекстСообщения = НСтр("ka='ეხ-ის გასაღების მონაცემთა მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении данных ключа ЭП.';en='An error occurred while retrieving data signature key.'");
...
Процедура ПослеПолученияКлючейЭПВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ МассивДвоичныхДанныхСертификатов.Количество() Тогда
		ТекстСообщения = НСтр("ka='ბანკის გასაღებზე სერთიფიკატები არ არის';ru='Отсутствуют сертификаты на банковском ключе';en='There are no certificates in the bank key'");
...
Процедура ОбработатьОшибкуПолученияКлючейВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='გასაღებების მიღება';ru='Получение ключей';en='keys'");
	ТекстСообщения = НСтр("ka='გასაღებების მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении ключей.';en='An error occured when getting the keys.'");
...
Процедура ОбработатьОшибкуИнициализацииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='გარე მოდულის ინიციალიზაციის დროს შეცდომა მოხდა.';ru='При инициализации внешнего модуля произошла ошибка.';en='An error occurred while initializing the external module.'");
	ВидОперации = НСтр("ka='გარე კომპონენტის ინიციალიზაცია.';ru='Инициализация внешней компоненты.';en='Initialization of external components.'");
...
"Код ошибки: ДО-%1"
"%2';ka='SMS ავთენტიფიკაციის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ДополнительныеПараметры.ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='SMS ავთენტიფიკაცია';ru='SMS аутентификация';en='SMS authentication'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиОдноразовогоПароля, Ложь);
...
"Код ошибки: ДО-%1"
"%2';ka='SMS-ით ავთენტიფიკაციის გაშვების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(
...
"Код ошибки: ДО-%1"
"%2';ka='სერთიფიკატის ჩატვირთვისას შეცდომა მოხდა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ДополнительныеПараметры.ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Получение дополнительных данных сертификата';en='Obtaining additional certificate data'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияСертификата, Неопределено);
