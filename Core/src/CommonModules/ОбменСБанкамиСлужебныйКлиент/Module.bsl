...
Процедура ЗавершитьТестПодписиСертификатаСбербанк(ПодписьВерна, Параметры) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		Параметры.Контекст.ОписаниеОшибки = НСтр("ka='ხელმოწერის შემოწმებისას შეცდომა მოხდა';ru='При проверке подписи произошла ошибка'");
	ИначеЕсли НЕ ПодписьВерна Тогда
		Параметры.Контекст.ОписаниеОшибки = НСтр("ka='დაყენებული ხელმოწერა არასწორია';ru='Установленная подпись неверна'");
	КонецЕсли;
...
Процедура ПослеПолученияСпискаБизнесСистемСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		ТекстСообщения = НСтр("ka='ბიზნეს სისტემების სიის მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении списка бизнес систем.'");
		ТекстОшибки = НСтр("ka='გარე კომპონენტა VpnKey-TLS-მა ბიზნეს სისტემების სიის მიღებისას დააბრუნა შეცდომის კოდი %1';ru='Внешняя компонента VpnKey-TLS при получении списка бизнес систем вернула код ошибки %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатВызова);
		Операция = НСтр("ka='ბიზნეს სისტემების სიის მიღება.';ru='Получение списка бизнес систем.'");
...
Процедура ОбработатьОшибкуПолученияСпискаБизнесСистемНаТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	Операция = НСтр("ka='Получение списка бизнес систем на аппаратном устройстве.';ru='Получение списка бизнес систем на аппаратном устройстве.'");
	ТекстСообщения = НСтр("ka='Не удалось получить список бизнес систем с аппаратного устройства';ru='Не удалось получить список бизнес систем с аппаратного устройства'");
...
Процедура ПроверитьУстановкуПодписиПослеАутентификацииНаТокенеСбербанк(Успех, Параметры) Экспорт
	...
	Если Не Успех Тогда
		ОписаниеОшибки = НСтр("ka='ბანკის გასაღებზე ავთენტიფიცირება ვერ მოხერხდა';ru='Не удалось аутентифицироваться на банковском ключе'");
...
"%2'");
		...
		Операция = НСтр("ka='საბანკო სერთიფიკატების მიღება';ru='Получение банковских сертификатов'");
	...
	Если СертификатыНаУстройстве.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='ბანკის გასაღებზე სერთიფიკატები არ არის.';ru='На банковском ключе отсутствуют сертификаты.'");
...
"%2'");
		...
		Операция = НСтр("ka='PIN-კოდის დაყენება ';ru='Установка PIN-кода'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
...
Процедура ПолучитьВыпискуЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, Параметры) Экспорт
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			КонецЦикла
		Иначе
			ТекстСообщения = НСтр("ka='ოპერაციის შესასრულებლად აუცილებელია საბანკო გასაღების შეერთება კომპიუტერთან';ru='Для выполнения операции необходимо подключить банковский ключ к компьютеру'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат
		КонецЕсли;
		...
		ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка'");
		Если Соответствие.Количество() > 0 Тогда
			...
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("ka='კომპიუტერზე შეერთებულია ბანკის არაშესაბამისი გასაღები.';ru='К компьютеру подключен не подходящий банковский ключ'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			Если НЕ ПарольУстановлен Тогда
				...
				Иначе
					...
					ПолучитьПарольКСертификату(ОООЗ, Соответствие, НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка'"));
			...
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("ka='კომპიუტერზე არ არის შეერთებული ბანკის გასაღები გაცვლის გამართვის მიხედვით მონაცემთა გასაგზავნად: %1';ru='К компьютеру не подключен банковский ключ для отправки данных по настройке обмена: %1'");
		...
		Для Каждого СообщениеОбмена Из МассивСообщенийОбмена Цикл
			...
			Для Каждого СтрокаЭП Из СтруктураСодержимогоСообщенияОбмена.Подписи Цикл
				...
				Исключение
					...
					ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
					ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
					Операция = НСтр("ka='ხელმოწერის შემოწმება';ru='Проверка подписи'");
					ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='სერთიფიკატის პაროლის დაყენება';ru='Установка пароля сертификата'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='დაყენებული PIN-კოდის არსებობის შემოწმება';ru='Проверка наличия установленного PIN-кода'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Ложь;
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='მონაცემთა გაგზავნა';ru='Отправка данных'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
	КонецПопытки;
...
Процедура НачатьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры) Экспорт
	...
	РезультатТеста = НСтр("ka='გავლილია წარმატებით.';ru='Пройден успешно.'");
	...
	Если ВнешнийПодключаемыйМодуль = Неопределено Тогда
		ТекстСообщения = НСтр("ka='ტესტი არ არის გავლილი.';ru='Тест не пройден.'");
	...
	Для Каждого Элемент Из ДоступныеСертификаты Цикл
		...
		ТекстСообщения = НСтр("ka='სერთიფიკატის შემოწმება: %1';ru='Проверка сертификата: %1'");
		...
		ПолучитьПарольКСертификату(Оповещение, Соответствие, НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка'"));
		Возврат;
	КонецЦикла;
	
	Если ДоступныеСертификаты.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='შემოწმება სრულად არ ჩატარებულა, ვინაიდან გაცვლის გამართვაში ხელმოწერის სერთიფიკატები არ არის';ru='Проверка проведена не полностью, т.к. в настройке обмена отсутствуют сертификаты подписи'");
		СообщитьПользователю(ТекстСообщения, ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	
	// Блок проверки отправки тестового запроса.
	ОписаниеТеста = НСтр("ka='ტესტი. სატესტო მოთხოვნის გაგზავნა.';ru='Тест. Отправка тестового запроса.'");
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='სატესტო მოთხოვნის გაგზავნა';ru='Отправка тестового запроса'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='Поиск аппаратных устройств.';ru='Поиск аппаратных устройств.'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Неопределено;
		...
		Если НЕ ПодключаемыйМодуль.УстановитьСоединение(СертификатXML) Тогда
			...
			Если ТребуетсяРасширеннаяАутентификация Тогда
				Если ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
					ВызватьИсключение НСтр("ka='გაფართოებული ავთენტიფიკაციის მეთოდები არ არის დადგენილი.';ru='Не определены способы расширенной аутентификации.'");
				КонецЕсли;
				Если НЕ ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
					ВызватьИсключение НСтр("ka='გაფართოებული ავთენტიფიკაცია SMS-ით არ არის მხარდაჭერილი.';ru='Расширенная аутентификация по SMS не поддерживается.'");
				КонецЕсли;
			...
			Иначе
				ВызватьИсключение НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.'");
			КонецЕсли;
		Иначе
			ВыполнитьОбработкуОповещения(Оповещение, Истина);
		КонецЕсли;
...
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='შეერთების დაყენება';ru='Установка соединения'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='PIN-კოდის შეყვანის აუცილებლობის შემოწმება';ru='Проверка необходимости ввода PIN-кода'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Ошибка = Истина;
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='სერთიფიკატის მონაცემთა მიღება';ru='Получение данных сертификата'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Неопределено;
...
"%2'");
				ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ka='მონაცემთა სქემის მიღება';ru='Получение схемы данных'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
					Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
				ПрерватьПодписание = Истина;
...
"%2'");
				ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ka='დოკუმენტების ხელმოწერა';ru='Подписание документов'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
									Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
				ПрерватьПодписание = Истина;
...
"Тест прерван.'");
	...
	ОписаниеТеста = НСтр("ka='Тест. Чтение данных сертификата.';ru='Тест. Чтение данных сертификата.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ДанныеСертификата = ДанныеСертификатаЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, СертификатXML);
	Если ДанныеСертификата = Неопределено Тогда
		Возврат;
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработкуПослеАутентификации(АутентификацияВыполнена, Параметры) Экспорт
	...
	ОписаниеТеста = НСтр("ka='Тест. Установка подписи.';ru='Тест. Установка подписи.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	...
	Исключение
		...
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='ხელმოწერის დაყენება';ru='Установка подписи'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат;
	...
	ОписаниеТеста = НСтр("ka='Тест. Проверка подписи.';ru='Тест. Проверка подписи.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	Попытка
	...
	Если НЕ ПодписьВалидна Тогда
		СообщитьПользователю(НСтр("ka='Подпись не валидна';ru='Подпись не валидна'"), ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	СообщитьПользователю(РезультатТеста, ИдентификаторНазначения);
...
Процедура НачатьПроцессПолученияВыпискиЧерезВК(НастройкаОбмена, ДатаНачала, ДатаОкончания, НомерСчета, Владелец) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			...
			ТекстСообщения = НСтр("ka='დოკუმენტის ხელმოსაწერად შესაფერისი სერთიფიკატი ვერ მოიძებნა ამონაწერის მოთხოვნა';ru='Не найден подходящий сертификат для подписи документа Запрос выписки'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			ВидОперации = НСтр("ka='Подписание электронных документов';ru='Подписание электронных документов'");
...
Процедура ПослеОшибкиВключенияЖурналированияВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ჟურნალირების ჩართვისას შეცდომა მოხდა.';ru='При включении журналирования произошла ошибка.'");
	ВидОперации = НСтр("ka='ჟირნალირების ჩართვა.';ru='Включение журналирования.'");
...
Процедура ПослеПодключенияВнешнейКомпоненты(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		Попытка
			...
			ЗакэшироватьПодключаемыйМодуль(Параметры.НазваниеВК, ПодключаемыйМодуль);
		Исключение
			ТекстСообщения = НСтр("ka='ბანკის გარე კომპონენტის შეერთების შეცდომა.';ru='Ошибка подключения внешней компоненты банка.'");
			Операция = НСтр("ka='ბანკის გარე მოდულის შეერთება';ru='Подключение внешнего модуля банка'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		КонецПопытки;
...
Функция ТекстСообщенияСбербанк(Тикет)
	...
	Если Тикет = "00000000-0000-0000-0000-000000000000" Тогда
		ТекстСообщения = НСтр("ka='ბანკის სერვერი დროებით მიუწვდომელია. განმეორებით სცადეთ ან მიმართეთ საკუთარ ბანკს.';ru='Сервер банка временно недоступен. Повторите попытку или обратитесь в свой банк.'");
...
"Необходимо проверить работу TLS VPN Key.'");
		Операция = НСтр("ka='Поиск бизнес системы на электронном ключе.';ru='Поиск бизнес системы на электронном ключе.'");
...
Процедура ОбработатьРезультатАсинхроннойПодписиСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		Операция = НСтр("ka='მონაცემების ხელმოწერა';ru='Подписание данных'");
...
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='მონაცემთა ხელმოწერა.';ru='Подпись данных.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
	Результат = Новый Структура;
	Результат.Вставить("Успех", Ложь);
...
Процедура ПослеПолученияДанныхСертификатаСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		ТекстСообщения = НСтр("ka='სერთიფიკატის მონაცემთა მიღებისას შეცდომა მოხდა.';ru='При получении данных сертификата произошла ошибка.'");
		ТекстОшибки = НСтр("ka='სერთიფიკატის მიღებისას გარე მოდულ VpnKey-TLS-მა დააბრუნა შეცდომის კოდი';ru='Внешний модуль VpnKey-TLS при получении сертификата вернул код ошибки'") + РезультатВызова;
		Операция = НСтр("ka='Получение сертификата криптографии.';ru='Получение сертификата криптографии.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
...
Процедура ПроверитьУстановкуПодписиПолученияСертификатовСТокенаСбербанк(Результат, Параметры) Экспорт
	...
	Для Каждого КлючЗначение Из СоответствиеСертификатов Цикл
		...
		Исключение
			ТекстСообщения = НСтр("ka='სერთიფიკატის წაკითხვის შეცდომა.';ru='Ошибка чтения сертификата.'");
		...
		Если СтруктураСертификата.Отпечаток = РеквизитыПроверяемогоСертификата.Отпечаток Тогда
			...
			Представление = НСтр("ka='სატესტო სტრიქონი';ru='Тестовая строка'");
...
"ხელმისაწვდომი სერთიფიკატების სიის მიღებისას გარე მოდულ VpnKey-TLS-მა დააბრუნა შეცდომის კოდი: %1'");
		...
		Операция = НСтр("ka='Подписание электронного документа.';ru='Подписание электронного документа.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
		ОчиститьДанныеАвторизацииСбербанк();
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстОшибки);
...
Процедура ПодобратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Исключение
			...
			Операция = НСтр("ka='Чтение данных сертификата.';ru='Чтение данных сертификата.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	ЗаголовокФормыВыбора = НСтр("ka='აირჩიეთ ხელმოწერის სერთიფიკატი';ru='Выберите сертификат подписи'");
...
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='ხელმოწერის მონაცემთა მიღება';ru='Получение данных подписи'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
Процедура ПодписатьТестовыйЗапросПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	ПодключаемыйМодуль = ПодключеннаяВнешняяКомпонентаБанка("VPNKeyTLS");
	Представление = НСтр("ka='სატესტო მოთხოვნა';ru='Тестовый запрос'");
...
Процедура ПослеОтправкиЗапросаВСбербанк(РезультатВызова, ПараметрыВызова, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(КодОшибки) ИЛИ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВидОперации = НСтр("ka='მოთხოვნის გაგზავნა ბანკში.';ru='Отправка запроса в банк'");
...
Процедура ПослеПолученияДокументаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(КодОшибки) ИЛИ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВидОперации = НСтр("ka='დოკუმენტების მიღება ბანკიდან.';ru='Получение документов из банка.'");
		ШаблонОшибки = НСтр("ru='Произошла ошибка сетевого взаимодействия."
"Код ошибки: %1."
"Текст ошибки: %2';ka='მოხდა ქსელური ურთიერთქმედების შეცდომა."
"შეცდომის კოდი: %1."
...
Процедура ОтправитьЗапросВыпискиСбербанк(Оповещение, НастройкаОбмена, ЗапросВыписки) Экспорт
	...
	ВидОперации = НСтр("ka='ბანკის ამონაწერის მოთხოვნა';ru='Запрос выписки банка'");
...
Процедура ПослеУстановкиКаналаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВызова = 0 Тогда
		Операция = НСтр("ka='Установка виртуального канала';ru='Установка виртуального канала'");
...
"აუცილებელია TLS VPN Key-ის მუშაობის შემოწმება.'");
		ТекстОшибки = НСтр("ka='ვირტუალური არხის დაყენებისას კომპონენტა AddIn.Bicrypt-მა შეცდომის კოდი დააბრუნა';ru='Компонента AddIn.Bicrypt при установке виртуального канала вернула код ошибки'")
							+ " " + РезультатВызова;
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ОчиститьДанныеАвторизацииСбербанк();
	КонецЕсли;
...
"%1';ka='ბანკის გარე მოდულის ვერსიის მიღებისას შეცდომა მოხდა: %1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='ბანკის გარე კომპონენტის ვერსიის მიღება';ru='Получение версии внешней компоненты банка'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
...
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ka='პროგრამის გაშვება Start.exe';ru='Запуск программы Start.exe'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаТокене, Ложь);
...
Процедура ОбработатьРезультатАутентификацииНаТокенеСбербанка(НастройкаОбмена, РезультатАвторизации, ОповещениеПослеАутентификацииНаТокене)
	...
	Если Не (РезультатАвторизации = 0) Тогда
		Если РезультатАвторизации = 28 Тогда
			ШаблонСообщения = НСтр("ka='PIN%1 დაბლოკილია';ru='PIN%1 заблокирован'");
		...
		ИначеЕсли РезультатАвторизации = 27 Тогда
			ШаблонСообщения = НСтр("ka='PUK%1 დაბლოკილია';ru='PUK%1 заблокирован'");
			ТекстСообщения = СтрШаблон(
								ШаблонСообщения, ЗначениеИзКэшаСбербанк("НомерКонтейнера"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли РезультатАвторизации = 25 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მომხმარებელი დაბლოკილია';ru='Пользователь заблокирован'"));
		...
		Иначе
			...
			Операция = НСтр("ka='ტოკენზე ავტორიზაცია';ru='Авторизация на токене'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ПослеПолученияОтпечатковВыполнитьОбмен(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если МассивСертификатов.Количество() Тогда
		...
		ОписаниеДанных.Вставить("Операция", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'"));
...
Процедура ВыполнитьОбменСБанками(Результат, Параметры) Экспорт
	...
	Если НЕ Параметры.Свойство("НастройкиОбмена", НастройкиОбмена)
		...
		ШаблонОповещения = НСтр("ka='გაგზავნილია პაკეტი: (%1), მიღებულია პაკეტი: (%2).';ru='Отправлено пакетов: (%1), получено пакетов: (%2).'");
...
Процедура ВыполнитьОбменСБанком(Параметры)
	...
	ТекстСообщения = НСтр("ka='Отправка пакетов электронных документов в банк. Подождите...';ru='Отправка пакетов электронных документов в банк. Подождите...'");
	Состояние(НСтр("ka='Отправка.';ru='Отправка.'"), , ТекстСообщения);
...
Процедура ВыполнитьСинхронизациюПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	Если ОтправлятьЗапросНовыхДокументов Тогда
		...
		ПодключаемыйМодуль = ПодключеннаяВнешняяКомпонентаБанка("VPNKeyTLS");
		Представление = НСтр("ka='ახალი დოკუმენტების მოთხოვნა';ru='Запрос новых документов'");
		Параметры.Вставить("ИдентификаторЗапроса", ИдентификаторЗапроса);
		Параметры.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
...
Процедура ПослеПолученияОтпечатковПроверитьСвязь(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			ИЛИ НЕ НастройкиОбмена.СертификатДоступен Тогда
			ТекстСообщения = НСтр("ka='დოკუმენტ ""მოთხოვნა-ზონდი""-ზე ხელმოსაწერად შესაბამისი სერთიფიკატი ვერ მოიძებნა';ru='Не найден подходящий сертификат для подписи документа Запрос-зонд'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			СтруктураРезультата = Новый Структура("Успех", Ложь);
			ВыполнитьОбработкуОповещения(Параметры.ОбработчикПослеТестаНастройки, СтруктураРезультата);
			Возврат;
		...
		Операция = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа'");
		ОписаниеДанных = Новый Структура;
...
Процедура ОтправитьИПолучитьДокументыВБанк(ДанныеАутентификации, Параметры)
	...
	ЗаголовокОповещения = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк'");
	ПоказатьОповещениеПользователя(ЗаголовокОповещения, , ТекстОповещения);
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ТекстЗаголовка = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк'");
...
Процедура ПолучитьДанныеАутентификации(НастройкаОбмена, ОписаниеОповещенияОЗакрытии, ЭтоТест = Ложь) Экспорт
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ВидОперации", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'"));
...
Процедура ОбработатьОшибкуПолученияСертификатаИзПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ka='სერთიფიკატის მიღება ხელმოწერიდან';ru='Получение сертификата из подписи'");
	ТекстСообщения = НСтр("ka='ხელმოწერიდან სერთიფიკატის მიღება ვერ მოხერხდა';ru='Не удалось получить сертификат из подписи'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
...
Процедура ОбработатьОшибкуВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ka='ხელმოწერის სერთიფიკატის გადმოტვირთვა';ru='Выгрузка сертификата подписи'");
	ТекстСообщения = НСтр("ka='მიღებული ხელმოწერის სერთიფიკატის გადმოტვირთვა ვერ მოხერხდა';ru='Не удалось выгрузить сертификат полученной подписи'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеПолученияСертификатов, Неопределено);
...
"%2'");
			ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
			Операция = НСтр("ka='გარე დამუშავების ინიციალიზაცია';ru='Инициализация внешней обработки'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Ложь;
...
"%2'");
				ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ka='გარე დამუშავების ინიციალიზაციის დასრულება';ru='Завершение инициализации внешней обработки'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
					Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
				Возврат Ложь;
...
Процедура ОбработатьОтклонениеЭД(СообщениеОбмена, ОписаниеОповещения) Экспорт
	...
	Заголовок = НСтр("ka='მიუთითეთ დოკუმენტის უარყოფის მიზეზები';ru='Укажите причины отклонения документа'");
...
Процедура ПослеОпределенияСтатусаОчереднойПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ДополнительныеПараметры.РезультатПроверки.Добавить(СтруктураЗаписи);
	ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ka='ხელმოწერის შემოწმება';ru='Проверка подписи'");
		...
		Для Каждого Элемент Из СертификатыСообщенийОбмена Цикл
			...
			Если Структура.Свойство("СоответствиеСообщенийОбменаИДД", СоответствиеСообщенийОбменаИДД)
				...
				Если СоответствиеСообщенийОбменаИДД.Количество() = 1 Тогда
					Операция = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа'");
				Иначе
					Операция = НСтр("ka='Подписание электронных документов';ru='Подписание электронных документов'");
				КонецЕсли;
				...
				ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ka='დოკუმენტი';ru='Документ'"));
				ОписаниеДанных.Вставить("БезПодтверждения",    Истина);
				...
				Иначе
					ПредставлениеСообщенияОбмена = НСтр("ka='Электронные документы (%1)';ru='Электронные документы (%1)'");
...
Процедура ТестНастройкиОбменаЧерезДополнительнуюОбработку(НастройкаОбмена, ИдентификаторНазначения)
	...
	ОписаниеТеста = НСтр("ka='Тест. Проверка наличия внешней обработки для обмена с банком.';ru='Тест. Проверка наличия внешней обработки для обмена с банком.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ВнешняяОбработкаПодключена = ОбменСБанкамиСлужебныйВызовСервера.ПодключитьАктуальнуюВнешнююОбработку(
	...
	ОписаниеТеста = НСтр("ka='Тест. Инициализация служебного интерфейса.';ru='Тест. Инициализация служебного интерфейса.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры)
	...
	ОписаниеТеста = НСтр("ka='Тест. Аутентификация на ресурсе банка.';ru='Тест. Аутентификация на ресурсе банка.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка'");
	Попытка
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='ავთენტიფიკაცია ბანკის რესურსზე';ru='Аутентификация на ресурсе банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		Возврат;
	...
	ОписаниеТеста = НСтр("ka='Тест. Установка соединения с банком.';ru='Тест. Установка соединения с банком.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
Процедура ПодписатьЗапросыВыписок(Результат, ПараметрыОбработки)
	...
	ПредставлениеНабора = НСтр("ka='ამონაწერების მოთხოვნები (%1)';ru='Запросы выписок (%1)'");
...
Процедура ОбработатьОшибкуПолученияВерсииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='გარე კომპონენტის ვერსიის მიღება.';ru='Получение версии внешней компоненты.'");
	ТекстСообщения = НСтр("ka='გარე მოდულის ვერსიის მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении версии внешнего модуля.'");
...
Процедура ПолучитьПарольАутентификацииПослеОпределенияСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВидОперации = НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'") + " "; // Для запроса пароля сертификата
...
Процедура ОбработатьОшибкуПроверкиНеобходимостиУстановкиПароляВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ავთენტიფიკაციის დროს შეცდომა მოხდა.';ru='При аутентификации произошла ошибка.'");
	ВидОперации = НСтр("ka='პაროლის დაყენების აუცილებლობის შემოწმება.';ru='Проверка необходимости установки пароля.'");
...
Процедура ПродолжитьОтправкуПлатежныхДокументовПоНастройкамиПослеПолученияНовыхДокументовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат.Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ka='ახალი დოკუმენტების მიღება ბანკიდან.';ru='Получение новых документов из банка.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбработатьОшибкуПроверкиДополненияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='დოკუმენტზე ხელმოწერისას შეცდომა მოხდა.';ru='При подписании документа произошла ошибка.'");
	ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის დამატებით შევსების შემოწმება.';ru='Проверка дополнения электронного документа.'");
...
Процедура ОбработатьОшибкуДополненияЭДЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='დოკუმენტზე დამატებების გაკეთებისას შეცდომა მოხდა.';ru='При дополнении документа произошла ошибка.'");
	ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის დამატება.';ru='Дополнение электронного документа.'");
...
Процедура ПослеПолученияДанныхСертификатовСКлючаВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеВыбора.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='საბანკო გასაღებზე ვერ მოიძებნა ბანკთან გაცვლის გასწორებაში მითითებული სერთიფიკატები.';ru='На банковском ключе не найдены сертификаты, указанные в настройке обмена с банком.'");
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеТестаНастройки, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если НастройкиОбмена.Подписывать Тогда
		ВидОперации = НСтр("ka='Подписание электронного документа.';ru='Подписание электронного документа.'");
		Оповещение = Новый ОписаниеОповещения(
...
Процедура ОбработатьОшибкуОтправкиПакетаЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ბანკში მონაცემთა გაგზავნისას შეცდომა მოხდა.';ru='При отправке данных в банк произошла ошибка.'");
	ВидОперации = НСтр("ka='ელექტრონული დოკუმენტების პაკეტების გაგზავნა.';ru='Отправка пакетов электронных документов.'");
...
Процедура ПослеОтправкиПакетаЧерезВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		ВидОперации = НСтр("ka='დოკუმენტის გაგზავნა ბანკში';ru='Отправка документа в банк'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбработатьОшибкуПолученияСпискаПакетов(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ბანკიდან დოკუმენტების პაკეტების სიის მიღების შეცდომა.';ru='Ошибка получения списка пакетов документов из банка.'");
	ВидОперации = НСтр("ka='პაკეტების სიის მიღება.';ru='Получение списка пакетов.'");
...
Процедура ОбработатьОшибкуПолученияПакетаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ბანკიდან დოკუმენტების პაკეტის მიღების შეცდომა.';ru='Ошибка получения пакета документов из банка.'");
	ВидОперации = НСтр("ka='ბანკიდან პაკეტის მიღება.';ru='Получение пакета из банка.'");
...
Процедура ОбработатьОшибкуПодготовкиПодписиЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='При подготовки подписи электронного документа произошла ошибка.';ru='При подготовки подписи электронного документа произошла ошибка.'");
	ВидОперации = НСтр("ka='Подготовка подписи электронного документа.';ru='Подготовка подписи электронного документа.'");
...
Процедура ОбработатьОшибкуПодписанияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='ელექტრონულ დოკუმენტზე ხელმოწერისას შეცდომა მოხდა.';ru='При подписании электронного документа произошла ошибка.'");
	ВидОперации = НСтр("ka='Подписание электронного документа.';ru='Подписание электронного документа.'");
	ПолучитьИнформациюОбОшибкеВК(ДополнительныеПараметры.ОповещениеПослеПодписанияЭД,
		ДополнительныеПараметры.ПодключаемыйМодуль, ВидОперации, ТекстОшибки);
...
Процедура ОбработатьОшибкуРасширеннойАутентификации(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='SMS-ით ავთენტიფიკაციის შეცდომა.';ru='Ошибка аутентификации по SMS.'");
	ВидОперации = НСтр("ka='ერთჯერადი პაროლის გაგზავნა SMS-დან.';ru='Отправка одноразового пароля из SMS.'");
...
Процедура ОбработатьОшибкуОтправкиПароляПоSMSВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='SMS-ით ერთჯერადი პაროლის გაგზავნის შეცდომა.';ru='Ошибка отправки одноразового пароля по SMS.'");
	ВидОперации = НСтр("ka='ერთჯერადი პაროლის გაგზავნა SMS-ით.';ru='Отправка одноразового пароля в SMS.'");
	ПолучитьИнформациюОбОшибкеВК(ДополнительныеПараметры.ОповещениеПослеУстановкиСоединения,
		ДополнительныеПараметры.ПодключаемыйМодуль, ВидОперации, ТекстСообщения);
...
Процедура ОбработатьОшибкуУстановкиСоединенияВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='ბანკთან შეერთების დაყენებისას შეცდომა მოხდა.';ru='При установке соединения с банком произошла ошибка.'");
	ВидОперации = НСтр("ka='ბანკთან შეერთების დაყენება.';ru='Установка соединения с банком.'");
...
Процедура ОбработатьОшибкуДополненияДанныхКлючаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='ელექტრონულ გასაღებთან მუშაობისას შეცდომა მოხდა.';ru='При работе с электронным ключем произошла ошибка.'");
	ВидОперации = НСтр("ka='გასაღების მონაცემების დამატებით შევსება.';ru='Дополнение данных ключа.'");
...
Процедура ОбработатьОшибкуУстановкиПароляВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ka='პაროლის დაყენების შეცდომა.';ru='Ошибка установки пароля.'");
	ВидОперации = НСтр("ka='გასაღების პაროლის დაყენება.';ru='Установка пароля ключа.'");
...
Процедура ВыбратьКлючПослеПолученияХранилищКлючейЭП(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Количество() > 1 Тогда
		...
		ЗаголовокВыбора = НСтр("ka='აირჩიეთ ელექტრონული გასაღები:';ru='Выберите электронный ключ:'");
...
Процедура ОбработатьОшибкуПроверкиНеобходимостиПИНВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='პინ-კოდის დაყენების აუცილებლობის შემოწმება.';ru='Проверка необходимости установки ПИН-кода.'");
	ТекстСообщения = НСтр("ka='პინ-კოდის საჭიროების შემოწმებისას შეცდომა მოხდა.';ru='При проверке необходимости ПИН-кода произошла ошибка.'");
...
Процедура ОбработатьОшибкуУстановкиПинВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ka='PIN კოდის დაყენებისას შეცდომა მოხდა.';ru='При установки PIN кода произошла ошибка.'");
...
Процедура ОбработатьОшибкуПолученияДанныхСертификатаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='ეხ-ის გასაღების მონაცემთა მიღება';ru='Получение данных ключа ЭП.'");
	ТекстСообщения = НСтр("ka='ეხ-ის გასაღების მონაცემთა მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении данных ключа ЭП.'");
...
Процедура ПослеПолученияКлючейЭПВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ МассивДвоичныхДанныхСертификатов.Количество() Тогда
		ТекстСообщения = НСтр("ka='ბანკის გასაღებზე სერთიფიკატები არ არის';ru='Отсутствуют сертификаты на банковском ключе'");
...
Процедура ОбработатьОшибкуПолученияКлючейВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='გასაღებების მიღება';ru='Получение ключей'");
	ТекстСообщения = НСтр("ka='გასაღებების მიღებისას შეცდომა მოხდა.';ru='Произошла ошибка при получении ключей.'");
...
Процедура ОбработатьОшибкуИнициализацииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='გარე მოდულის ინიციალიზაციის დროს შეცდომა მოხდა.';ru='При инициализации внешнего модуля произошла ошибка.'");
	ВидОперации = НСтр("ka='გარე კომპონენტის ინიციალიზაცია.';ru='Инициализация внешней компоненты.'");
...
"%2'");
		...
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='SMS ავთენტიფიკაცია';ru='SMS аутентификация'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиОдноразовогоПароля, Ложь);
