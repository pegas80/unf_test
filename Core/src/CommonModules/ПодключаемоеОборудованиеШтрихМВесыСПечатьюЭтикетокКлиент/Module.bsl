...
Функция ОтключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Исключение
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='მოწყობილობის გამორთვის შეცდომა';ru='Ошибка отключения устройства';en='Error disabling device'"));
    	Результат = Ложь;	
	КонецПопытки;
	...
	Если Результат Тогда
		Если Не ЧастичнаяВыгрузка Тогда
			...
			Если ОбъектДрайвера.ОчиститьБазуТоваровИСообщений() <> 0 Тогда
				ВыходныеПараметры.Очистить();
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ka='საქონლის ბაზის გადუფთავება ვერ მოხერხდა.';ru='Не удалось очистить базу товаров.';en='Failed to clear the database of products.'") + ОбъектДрайвера.ОписаниеРезультата);
				Результат = Ложь;	
			КонецЕсли;
		КонецЕсли;
	...
	Если Результат Тогда	             
		...
		Если Результат  Тогда
			...
			Для каждого ТекущийЭлемент Из Товары Цикл
				...
				// Нехорошо конечно так делать, но главное, чтобы работало...
				Попытка
					ЭтоШтучный=ЛОЖЬ;
					Если ТекущийЭлемент.Свойство("Штучный",ЭтоШтучный) Тогда
						Если ЭтоШтучный Тогда
							Результат = Выгрузить(ОбъектДрайвера, ВыходныеПараметры, ТекущийЭлемент.PLU, КодТовараВрем,
						НаименованиеТовара, ТекущийЭлемент.Цена, СрокХраненияВрем,1);
						Иначе
							Результат = Выгрузить(ОбъектДрайвера, ВыходныеПараметры, ТекущийЭлемент.PLU, КодТовараВрем,
							НаименованиеТовара, ТекущийЭлемент.Цена, СрокХраненияВрем);
						КонецЕсли;
				...
				Исключение
					Результат = Выгрузить(ОбъектДрайвера, ВыходныеПараметры, ТекущийЭлемент.PLU, КодТовараВрем,
						НаименованиеТовара, ТекущийЭлемент.Цена, СрокХраненияВрем);
				КонецПопытки;
				//Kuzmich, Проблемы с выгрузкой штучного товара
				...
				Состояние(НСтр("ka='საქონლის გადმოტვირთვა...';ru='Выгрузка товаров...';en='Unloading the goods ...'"), Окр(ТекущийПроцент));
...
//
Функция Выгрузить(ОбъектДрайвера, ВыходныеПараметры, PLU, КодТовара, Наименование, Цена, СрокХранения, ТекТипТовара=0)//Kuzmich, 13.02.2019, Проблемы с выгрузкой штучного товара
	
	Результат = Истина;
	ВремНаименование = СокрЛП(Наименование);
	ОбъектДрайвера.НомерПЛУ = PLU;  // Kuzmich, ПЛУ товара в весах
	...
	ОбъектДрайвера.КодТовара        = Число(КодТовара); // Kuzmich, Код товара в весах, она должна быть в штрихкоде
	ОбъектДрайвера.НомерИзображения = 0;
	ОбъектДрайвера.КодРОСТЕСТ       = " ";
	//ОбъектДрайвера.ТипТовара        = 0;
	ОбъектДрайвера.ТипТовара 		= ТекТипТовара; //Kuzmich, 13.02.2019, Проблемы с выгрузкой штучного товара
	ОбъектДрайвера.ТипПрефиксаШК 	= 0; // Kuzmich, 13.02.2019, проблемы с передачей данных
...
Функция ОчиститьТоварыВВесах(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის გასუფთავება ეტიკეტების მბეჭდავ სასწორში...';ru='Выполняется очистка товаров в весах с печатью этикеток...';en='Clears the goods on the scales with label printing ...'"));
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Исключение
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='ამოუცნობი შეცდომა.';ru='Не опознанная ошибка.';en='Does not recognize the error.'"));
   		Результат = Ложь;	
	КонецПопытки;
