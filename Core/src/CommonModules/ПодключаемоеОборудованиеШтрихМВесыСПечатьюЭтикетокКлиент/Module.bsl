...
Функция ОтключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Исключение
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='მოწყობილობის გამორთვის შეცდომა';ru='Ошибка отключения устройства'"));
    	Результат = Ложь;	
	КонецПопытки;
	...
	Если Результат Тогда
		Если Не ЧастичнаяВыгрузка Тогда
			...
			Если ОбъектДрайвера.ОчиститьБазуТоваровИСообщений() <> 0 Тогда
				ВыходныеПараметры.Очистить();
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ka='საქონლის ბაზის გადუფთავება ვერ მოხერხდა.';ru='Не удалось очистить базу товаров.'") + ОбъектДрайвера.ОписаниеРезультата);
				Результат = Ложь;	
			КонецЕсли;
		КонецЕсли;
	...
	Если Результат Тогда	             
		...
		Если Результат  Тогда
			...
			Для каждого ТекущийЭлемент Из Товары Цикл
				...
				Состояние(НСтр("ka='საქონლის გადმოტვირთვა...';ru='Выгрузка товаров...'"), Окр(ТекущийПроцент));
...
Функция ОчиститьТоварыВВесах(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის გასუფთავება ეტიკეტების მბეჭდავ სასწორში...';ru='Выполняется очистка товаров в весах с печатью этикеток...'"));
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Исключение
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ka='ამოუცნობი შეცდომა.';ru='Не опознанная ошибка.'"));
   		Результат = Ложь;	
	КонецПопытки;
