...
Функция ВыполнитьТрансляцию(Знач ИсходныйОбъект, Знач ОписаниеИсходнойВерсии, Знач ОписаниеРезультирующейВерсии) Экспорт
	...
	Если ЦепочкаТрансляцииИнтерфейса = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონფიგურაციაში არ არის რეგისტრირებული ვერსია %1-დან ვერსია %2-ში ტრანსლაციის დამამუშავებელი!';ru='В конфигурации не зарегистрирован обработчик трансляции из версии %1 в версию %2!';en='The configuration is not registered handler broadcasts from version to version% 1% 2!'"),
...
Функция ТранслироватьОбъект(Знач Объект, Знач ПравилаТрансляцииИнтерфейсов)
	...
	Если ЦепочкаТрансляцииИнтерфейса = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='პაკეტისთვის {%1} ტრანსლაციის დამამუშავებლის დადგენა ვერ მოხერხდა, მოცემული თვისების დასამუშავებლად სტანდარტული ტრანსლაციის შესრულება შეუძლებელია';ru='Не удалось определить обработчик трансляции для пакета {%1}, невозможно выполнение стандартной трансляции для обработки данного свойства';en='Unable to determine the broadcast handler for the package% {1} can not perform a standard broadcast for the treatment of a given property'"), 
...
Функция СтандартнаяОбработка(Знач Объект, Знач ПакетРезультирующегоОбъекта, Знач ПравилаТрансляцииИнтерфейсов)
	...
	Иначе
		...
		Если ТипРезультирующегоОбъекта = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 ტიპის ტრანსლაციის სტანდარტული დამუშავების შესრულება პაკეტ %2-ში ვერ მოხერხდა: ტიპი %1 პაკეტ %2-ში არ არსებობს!';ru='Не удалось выполнить стандартную обработку трансляции типа %1 в пакет %2: тип %1 не существует в пакете %2!';en='Failed to broadcast standard treatment of type% 1% 2 in the package: type% 1 does not exist in the package% 2!'"),
	...
	Для Каждого Свойство Из ТипРезультирующегоОбъекта.Свойства Цикл
		...
		Если СвойствоОригинала = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 ტიპის %2 ტიპში კონვერტაციის სტანდარტული დამუშავების შესრულება ვერ მოხერხდა: თვისება %3 არ არის დადგენილი %1 ტიპისთვის!';ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %1!';en='Failed to convert the standard treatment of type% 1% 2 Type:% 3 property is not defined for type% 1!'"),
				"{" + ТипИсходногоОбъекта.URIПространстваИмен + "}" + ТипИсходногоОбъекта.Имя,
	...
	Для Каждого Свойство Из ТипИсходногоОбъекта.Свойства Цикл
		...
		Если ТранслируемоеСвойство = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 ტიპის %2 ტიპში კონვერტაციის სტანდარტული დამუშავების შესრულება ვერ მოხერხდა: თვისება %3 არ არის დადგენილი %2 ტიპისთვის!';ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %2!';en='Failed to convert the standard treatment of type% 1% 2 Type:% 3 property is not defined for type% 2!'"),
				"{" + ТипИсходногоОбъекта.URIПространстваИмен + "}" + ТипИсходногоОбъекта.Имя,
				"{" + ТипРезультирующегоОбъекта.URIПространстваИмен + "}" + ТипРезультирующегоОбъекта.Имя,
				Свойство.ЛокальноеИмя);
		КонецЕсли;
