...
Функция ОтработатьПолучениеФайла(Сообщение) Экспорт
	...
	Событие = Событие + НСтр("ka='Запрос на передачу файла';ru='Запрос на передачу файла';en='Request for file transfer'"); 
...
"%5"
""
"Ответ прикладного обработчика: %6';ka='Обработан поступивший запрос на передачу файла. Параметры запроса:"
"Call_ID: %1"
"File_ID: %2"
"Sender: %3"
"Recipient: %4"
...
Функция ОтработатьКвитанциюПередачиФайла(Сообщение) Экспорт
	...
	Если ТелоСообщения.Error Тогда
		...
		Ответ = РаспределенноеВыполнениеКомандПереопределяемый.ОбработатьОшибкуПередачиФайла(ИдентификаторВызова, 
 		...
 		Событие = Событие + НСтр("ka='Ошибка передачи файла';ru='Ошибка передачи файла';en='Error file transfer'"); 
...
"%5"
""
"Ответ прикладного обработчика: %6';ka='При передаче файла возникла ошибка. Параметры запроса:"
"Call_ID: %1"
"Sender: %2"
"Recipient: %3"
"CallCode: %4"
...
"Ответ прикладного обработчика: %6'", Локаль); 
	...
	Иначе
		...
		Событие = Событие + НСтр("ka='Квитанция о передаче файла';ru='Квитанция о передаче файла';en='The receipt of the file transfer'"); 
...
"CallCode: %4"
""
"Ответ прикладного обработчика: %5';ka='Обработана поступившая квитанция на передачу файла. Параметры запроса:"
"Call_ID: %1"
"Sender: %2"
"Recipient: %3"
"CallCode: %4"
...
Функция ОтработатьПрямойВызов(Сообщение) Экспорт
	...
	Событие = РаспределенноеВыполнениеКоманд.ИмяСобытияЖурналаРегистрации() + ".";
	Событие = Событие + НСтр("ka='Задание поставлено в очередь';ru='Задание поставлено в очередь';en='Setting queued'", Локаль); 
	Комментарий = НСтр("ru='В очередь поставлено выполнение команды %1 (обработка %2, вызов %3).';en='The queued execution command 1% (treatment 2%, 3% challenge).'", Локаль);
