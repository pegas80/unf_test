...
Функция ОтработатьПолучениеФайла(Сообщение) Экспорт
	...
	Событие = Событие + НСтр("ka='Запрос на передачу файла';ru='Запрос на передачу файла'"); 
...
Функция ОтработатьКвитанциюПередачиФайла(Сообщение) Экспорт
	...
	Если ТелоСообщения.Error Тогда
		...
		Ответ = РаспределенноеВыполнениеКомандПереопределяемый.ОбработатьОшибкуПередачиФайла(ИдентификаторВызова, 
 		...
 		Событие = Событие + НСтр("ka='Ошибка передачи файла';ru='Ошибка передачи файла'"); 
...
"Ответ прикладного обработчика: %6'", Локаль); 
	...
	Иначе
		...
		Событие = Событие + НСтр("ka='Квитанция о передаче файла';ru='Квитанция о передаче файла'"); 
...
Функция ОтработатьПрямойВызов(Сообщение) Экспорт
	...
	Событие = РаспределенноеВыполнениеКоманд.ИмяСобытияЖурналаРегистрации() + ".";
	Событие = Событие + НСтр("ka='Задание поставлено в очередь';ru='Задание поставлено в очередь'", Локаль); 
