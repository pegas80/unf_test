...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Иначе
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("ka='გამყოფები არ არის დაყენებული';ru='Не установлены разделители';en='Not installed spacers'"), ПараметрыКлиента.ПодробнаяИнформация);
	КонецЕсли;
...
Процедура ОткрытьСписокАктивныхПользователей(ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			НСтр("ka='აქტიურ მომხმარებელთა სიის გასახსნელად გადადით მენიუში"
"ყველა ფუნქცია - სტანდარტულები - აქტიური მომხმარებლები.';ru='Для того чтобы открыть список активных пользователей, перейдите в меню"
...
#КонецЕсли
	...
	Если Предупреждения.Количество() = 0 Тогда
		Если Не ПараметрКлиента("ЗапрашиватьПодтверждениеПриЗавершенииПрограммы") Тогда
			Возврат;
		КонецЕсли;
		ТекстПредупреждения = НСтр("ka='დავამთავროთ პროგრამასთან მუშაობა?';ru='Завершить работу с программой?';en='To complete the work with the program?'");
		Отказ = Истина;
	Иначе
		Отказ = Истина;
...
Процедура ПоказатьПредупреждениеИПродолжить(Параметры, ОписаниеПредупреждения) Экспорт
	...
	Если Параметры.Отказ Тогда
		Кнопки.Добавить("Завершить", НСтр("ka='მუშაობის დასრულება';ru='Завершить работу';en='To finish work'"));
...
Процедура ВывестиПредупреждение(Форма, ПараметрыПредупреждения, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Контекст.Подробно) И Контекст.Подробно <> Контекст.Текст Тогда
		Кнопки.Добавить(1, НСтр("ka='დაწვრილებით...';ru='Подробнее...';en='More ...'"));
	КонецЕсли;
	Если ТипЗнч(Форма) = Тип("УправляемаяФорма") И ЗначениеЗаполнено(Контекст.ПутьКРеквизитуФормы) Тогда
		Кнопки.Добавить(2, НСтр("ka='რეკვიზიტზე გადასვლა';ru='Перейти к реквизиту';en='Go to the prop'"));
	КонецЕсли;
	...
	Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
...
Процедура ПоказатьПодробнуюИнформацию(Обработчик, Текст, Заголовок = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НастройкиДиалога.Заголовок) Тогда
		НастройкиДиалога.Заголовок = НСтр("ka='დაწვრილებით';ru='Подробнее';en='More'");
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(0, НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
...
Процедура ИнтерактивнаяОбработкаПроверкиВерсииПлатформыПриЗапуске(Параметры, Неопределен) Экспорт
	...
	Если ПараметрыКлиента.РаботаВПрограммеЗапрещена Тогда
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения = НСтр("ka='პროგრამაში შესვლა შეუძლებელია. აუცილებელია 1С:საწარმოს ვერსიის წინასწარ განახლება.';ru='Вход в программу невозможен."
"Необходимо предварительно обновить версию платформы 1С:Предприятие.'");
		Иначе
			ТекстСообщения = НСтр("ka='პროგრამაში შესვლა შეუძლებელია. პლატფორმა 1С:საწარმოს ვერსიის გასაახლებლად აუცილებელია, რომ მიმართოთ ადმინისტრატორს.';ru='Вход в программу невозможен."
...
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';ka='რეკომენდებულია პროგრამის მუშაობის დასრულება და პლატფორმა 1С:საწარმოს ვერსიის განახლება. წინააღმდეგ შემთხვევაში, პროგრამის ზოგიერთი შესაძლებლობა მიუწვდომელი იქნება ან არაკორექტულად იმუშავებს.'");
		Иначе
			ТекстСообщения = 
...
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';ka='რეკომენდებულია პროგრამის მუშაობის დასრულება და ადმინისტრატორთან მიმართვა პლატფორმა 1С:საწარმოს ვერსიის გასაახლებლად. წინააღმდეგ შემთხვევაში, პროგრამის ზოგიერთი შესაძლებლობა მიუწვდომელი იქნება ან არაკორექტულად იმუშავებს.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнтерактивнаяОбработкаВосстановленияСвязиСГлавнымУзлом(Параметры, Неопределен) Экспорт
	...
	Если ПараметрыКлиента.ВосстановитьСвязьСГлавнымУзлом = Ложь Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ka='მთავარ კვანძთან კავშირის აღდგენამდე პროგრამაში შესვლა დროებით შეუძლებელია. დაწვრილებითი ინფორმაციისთვის მიმართეთ ადმინისტრატორს.';ru='Вход в программу временно невозможен до восстановления связи с главным узлом."
...
"Выберите объект метаданных.';ka='მეტამონაცემთა ობიექტების ჯგუფის არჩევა შეუძლებელია. აირჩიეთ მეტამონაცემთა ობიექტი.'"));
		КонецЕсли;
	КонецЕсли;
...
"дополнительным параметром необходимо указать значение разделителя.';ka='გაშვების პარამეტრის მითითებისას მონაცემთაარეალშიშესვლა, დამატებით პარამეტრად აუცილებელია გამყოფის მნიშვნელობის მითითება.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение
			НСтр("ka='გამყოფის მნიშვნელობა პარამეტრ მონაცემთაარეალშიშესვლა-ში უნდა იყოს რიცხვი.';ru='Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.';en='Value separator VoytiVOblastDannyh parameter must be a number.'");
	КонецПопытки;
...
Процедура ОткрытьПрикладнуюФормуПредупреждения(Параметры, ОбработкаОтвета, ПредупреждениеПользователю, ИмяФормы, ПараметрыФормы)
	...
	Если ДействиеГиперссылка.Свойство("ПрикладнаяФормаПредупреждения", Форма) Тогда
		...
		Если ДействиеГиперссылка.Свойство("ПараметрыПрикладнойФормыПредупреждения", ПараметрыФормы) Тогда
			...
			ПараметрыФормы.Вставить("ЗаголовокКнопкиДа",  НСтр("ka='დასრულება';ru='Завершить';en='To complete'"));
			ПараметрыФормы.Вставить("ЗаголовокКнопкиНет", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
Процедура ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	ТекстПредупреждения = НачалоОписанияОшибки + Символы.ПС
		+ НСтр("ka='შეცდომის შესახებ ტექნიკური ინფორმაცია ჩაწერილია რეგისტრაციის ჟურნალში.';ru='Техническая информация об ошибке записана в журнал регистрации.';en='Technical information about the error recorded in the log.'")
...
Процедура ПоказатьПомещениеФайлаПриПодключенииРасширенияРаботыСФайлами(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыДиалога.Свойство("Режим") Тогда
		...
		Если Режим = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
			ВызватьИсключение НСтр("ka='კატალოგის არჩევა არ არის მხარდაჭერილი';ru='Выбор каталога не поддерживается';en='Selecting the catalog is not supported'");
		КонецЕсли;
	Иначе
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Заголовок = НСтр("ka='მუშაობის სიჩქარე შემცირებულია';ru='Скорость работы снижена';en='The speed of the reduced'");
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Рекомендуется увеличить"
" объем памяти до %1 Гб.';ka='რეკომენდებულია მეხსიერების მოცულობის გაზრდა %1 გბ-მდე.'"), РекомендуемыйОбъем);
