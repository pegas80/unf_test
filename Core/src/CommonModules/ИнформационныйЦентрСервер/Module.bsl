...
Функция ПолучитьИмяСобытияДляЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ცენტრი';ru='Информационный центр';en='Information centre'", ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка());
...
Функция ПолучитьПроксиИнформационногоЦентра() Экспорт
	...
	Если Не ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ВызватьИсключение(НСтр("ka='სერვისის მენეჯერთან შეერთება შეუძლებელია.';ru='Не возможно подключиться к Менеджеру сервиса.';en='It is not possible to connect to the Service Manager.'"));
	КонецЕсли;
...
Функция ПолучитьПроксиНовостейСлужбыПоддержки() Экспорт
	...
	Если Не ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ВызватьИсключение(НСтр("ka='ინფორმაციის მიღების სერვისი ხელმისაწვდომია მხოლოდ სერვისის მოდულში.';ru='Сервис новостей доступен только в модели сервиса.';en='News service is only available in the service model.'"));
	КонецЕсли;
...
Функция ТекстВыводаИнформацииОбОшибкеВЦентреИдей() Экспорт 
	...
	Возврат НСтр("ka='იდეის ცენტრი დროებით მიუწვდომელია.""გთხოვთ, მოგვიანებით სცადოთ.';ru='Центр идей временно не доступен.""Пожалуйста, повторите попытку позже.';en='Center ideas are temporarily not available. ""Please try again later.'")
...
Функция СформироватьЗаголовокКомментариев(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='კომენტარები: %1';ru='Комментарии: %1';en='Comments: 1%'"), ПредставлениеИдеи.CommentsCount);
...
Функция СформироватьЗаголовокДатыРеализации(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='რეალიზაციის გეგმიური თარიღი: %1';ru='Плановая дата реализации: %1';en='Planned date of implementation:% 1'"), ПредставлениеИдеи.PlanMadeDatePresentation);
...
Функция СформироватьДатуОтклонения(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='უარყოფილია: %1';ru='Отклонено: %1';en='Declined% 1'"), Формат(ПредставлениеИдеи.ClosingDate, "ДЛФ=DD"));
...
Функция СформироватьДатуРеализации(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='რეალიზებულია: %1';ru='Реализовано: %1';en='Implemented:% 1'"), Формат(ПредставлениеИдеи.ClosingDate, "ДЛФ=DD"));
...
Функция ТекстВыводаИнформацииОбОшибкеВСлужбеПоддержки() Экспорт
	...
	Возврат НСтр("ka='მხარდაჭერის სამსახური დროებით მიუწვდომელია. გთხოვთ, მოგვიანებით სცადეთ.';ru='Служба поддержки временно не доступна.""Пожалуйста, повторите попытку позже.';en='Support service is not available temporarily. ""Please try again later.'")
...
Функция СформироватьШаблонТекстаВТехПоддержку() Экспорт
	...
	Шаблон = НСтр("ka='გამარჯობათ!""<p/>""<p/>კურსორისპოზიცია""<p/>""პატივისცემით, %1.';ru='Здравствуйте!""<p/>""<p/>ПозицияКурсора""<p/>""С уважением, %1.';en='Hello! ""<P />"" <p /> PozitsiyaKursora ""<p />"" With respect, 1%.'");
...
Функция СформироватьXMLСТехническимиПараметрами(ДополнительныеПараметры = Неопределено) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='საინფორმაციო ცენტრი. წერილის გაგწავნა ტექნიკური დახმარების ცენტრში. ვერ მოხერხდა ტექნიკური პარამეტრების დროებითი ფაილის წაშლა.';ru='Информационный центр. Отправка сообщения в техподдержку. Не удалось удалить временный файл технических параметров.';en='Information centre. Sending a message to support. Could not delete the temporary file of technical parameters.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура СформироватьГруппыВывода(Форма, ТаблицаСсылок, ГруппаФормы, КоличествоГрупп, КоличествоСсылокВГруппе, ВыводитьСсылкуВсе)
	...
	Если ВыводитьСсылкуВсе Тогда
		...
		Элемент.Заголовок               = НСтр("ka='ყველა';ru='Все';en='Everything'");
