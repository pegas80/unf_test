...
Функция ПолучитьИмяСобытияДляЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ცენტრი';ru='Информационный центр'", ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка());
...
Функция ПолучитьПроксиИнформационногоЦентра() Экспорт
	...
	Если Не ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ВызватьИсключение(НСтр("ka='სერვისის მენეჯერთან შეერთება შეუძლებელია.';ru='Не возможно подключиться к Менеджеру сервиса.'"));
	КонецЕсли;
...
Функция ПолучитьПроксиНовостейСлужбыПоддержки() Экспорт
	...
	Если Не ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ВызватьИсключение(НСтр("ka='ინფორმაციის მიღების სერვისი ხელმისაწვდომია მხოლოდ სერვისის მოდულში.';ru='Сервис новостей доступен только в модели сервиса.'"));
	КонецЕсли;
...
Функция ТекстВыводаИнформацииОбОшибкеВЦентреИдей() Экспорт 
	...
	Возврат НСтр("ka='იდეის ცენტრი დროებით მიუწვდომელია.""გთხოვთ, მოგვიანებით სცადოთ.';ru='Центр идей временно не доступен.""Пожалуйста, повторите попытку позже.'")
...
Функция СформироватьЗаголовокКомментариев(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='კომენტარები: %1';ru='Комментарии: %1'"), ПредставлениеИдеи.CommentsCount);
...
Функция СформироватьЗаголовокДатыРеализации(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='რეალიზაციის გეგმიური თარიღი: %1';ru='Плановая дата реализации: %1'"), ПредставлениеИдеи.PlanMadeDatePresentation);
...
Функция СформироватьДатуОтклонения(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='უარყოფილია: %1';ru='Отклонено: %1'"), Формат(ПредставлениеИдеи.ClosingDate, "ДЛФ=DD"));
...
Функция СформироватьДатуРеализации(Знач ПредставлениеИдеи) Экспорт
	...
	Возврат СтрШаблон(НСтр("ka='რეალიზებულია: %1';ru='Реализовано: %1'"), Формат(ПредставлениеИдеи.ClosingDate, "ДЛФ=DD"));
...
Функция ТекстВыводаИнформацииОбОшибкеВСлужбеПоддержки() Экспорт
	...
	Возврат НСтр("ka='მხარდაჭერის სამსახური დროებით მიუწვდომელია. გთხოვთ, მოგვიანებით სცადეთ.';ru='Служба поддержки временно не доступна.""Пожалуйста, повторите попытку позже.'")
...
Функция СформироватьШаблонТекстаВТехПоддержку() Экспорт
	...
	Шаблон = НСтр("ka='გამარჯობათ!""<p/>""<p/>კურსორისპოზიცია""<p/>""პატივისცემით, %1.';ru='Здравствуйте!""<p/>""<p/>ПозицияКурсора""<p/>""С уважением, %1.'");
...
Процедура СформироватьГруппыВывода(Форма, ТаблицаСсылок, ГруппаФормы, КоличествоГрупп, КоличествоСсылокВГруппе, ВыводитьСсылкуВсе)
	...
	Если ВыводитьСсылкуВсе Тогда
		...
		Элемент.Заголовок               = НСтр("ka='ყველა';ru='Все'");
