...
Процедура ДополнитьПолучателей(Объект, Узлы) Экспорт
	...
	Для Каждого Элемент Из Узлы Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ka='გაცვლის გეგმის [ИмяПланаОбмена] შემადგენლობისთვის მიუთითებელია ობიექტის [ПолноеИмя] რეგისტრაცია';ru='Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]';en='For the composition of the communications plan [ImyaPlanaObmena] is not specified object registration [PathName]'");
	...
	Иначе
		...
		Если РежимВыгрузкиОбъекта = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
			...
			Если ЭтоНовыйОбъект Тогда
				ВызватьИсключение НСтр("ka='ბმულით გადმოსატვირთი ჩაუწერელი დოკუმენტების რეგისტრაცია არ არის მხარდაჭერილი.';ru='Регистрация незаписанных объектов, выгружаемых по ссылке, не поддерживается.';en='Register unfixed objects, unloaded the link is not supported.'");
			КонецЕсли;
...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ВызватьИсключение НСтр("ka='მონაცემთა სინქრონიზაციის სახელწოდების და კოდის შეცვლა დაუშვებელია.';ru='Изменение наименования и кода синхронизации данных недопустимо.';en='Changing the name and code synchronization data is unacceptable.'");
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Иначе
				...
				Если ЭтоРазделенныеДанные Тогда
					ВызватьИсключение НСтр("ka='გაყოფილ მონაცემთა განუყოფელ რეჟიმში ცვლილებების რეგისტრაცია.';ru='Регистрация изменений разделенных данных в неразделенном режиме.';en='Registration of changes in the undivided data separated mode.'");
				КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა გაცვლა. ობიექტების რეგისტრაციის წესები';ru='Обмен данными.Правила регистрации объектов';en='Exchange objects registration dannymi.Pravila'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ობიექტის მოდიფიცირებულობის დადგენის შეცდომა: %1';ru='Ошибка определения модифицированности объекта: %1';en=""The error in determining the object's modification: 1%"""),
...
"Описание ошибки:"
"%2';ka='ობიექტების რეგისტრაციის წესების შესრულების შეცდომა გაცვლის გეგმა %1-ისთვის. შეცდომის აღწერა: %2'"),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ПолучитьЗначенияСвойствДляСсылки(Ссылка, СвойстваОбъекта, Знач СвойстваОбъектаСтрокой, Знач ОбъектМетаданныхИмя) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ka='შეცდომა ბმულის თვისებების მიღებისას. მოთხოვნის შეცდომა: [ОписаниеОшибки]';ru='Ошибка при получении свойств ссылки. Ошибка выполнения запроса: [ОписаниеОшибки]';en='Error getting link Properties. Error executing the query: [OpisanieOshibki]'");
...
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, Знач ТекстЗапроса, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага, Знач Выгрузка = Ложь) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ka='შეცდომა მიმღებთა კვანძების სიის მიღებისას. მოთხოვნის შეცდომა: [ОписаниеОшибки]';ru='Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: [ОписаниеОшибки]';en='Error retrieving the list of recipients of nodes. Error executing the query: [OpisanieOshibki]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтрокаСообщения;
	КонецПопытки;
...
Функция НаборЗаписейПоТипу(ОбъектМетаданных)
	...
	Иначе
		...
		СтрокаСообщения = НСтр("ka='მეტამონაცემთა ობიექტ %1-სთვის არ არის გათვალისწინებული ჩანაწერების კრებული.';ru='Для объекта метаданных %1 не предусмотрено набора записей.';en='For metadata object% 1 does not provide a set of records.'");
...
"[ЗначениеКонстанты]"
"// {Окончание алгоритма}"
"';ka='Ошибка алгоритма вычисления значения константы:"
"План обмена: [ИмяПланаОбмена]"
"Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]"
"Алгоритм:"
...
"План обмена: [ИмяПланаОбмена];"
"Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';ka='შეცდომა დამამუშავებლის შესრულებისას: ""[ИмяОбработчика]""; გაცვლის გეგმა: [ИмяПланаОбмена]; მეტამონაცემთა ობიექტი: [ОбъектМетаданныхИмя] შეცდომის აღწერილობა: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ka='დამუშავების წინ';ru='Перед обработкой';en='before treatment'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
...
"Описание ошибки: [Описание]';ka='შეცდომა დამამუშავებლის შესრულებისას: ""[ИмяОбработчика]""; გაცვლის გეგმა: [ИмяПланаОбмена]; მეტამონაცემების ობიექტი: [ОбъектМетаданныхИмя]¶შეცდომის აღწერა: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ka='დამუშავებისას';ru='При обработке';en='when processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
Процедура ВыполнитьОбработчикПРОПриОбработкеДополнительный(Отказ, ПРО, Объект, ТекстЗапроса, ПараметрыЗапроса, ИспользоватьКэш, ДополнительныеСвойства = Неопределено)
	...
	Если ПРО.ЕстьОбработчикПриОбработкеДополнительный Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';ka='შეცდომა დამამუშავებლის შესრულებისას: ""[ИмяОбработчика]""; გაცვლის გეგმა: [ИмяПланаОбмена]; მეტამონაცემების ობიექტი: [ОбъектМетаданныхИмя]¶შეცდომის აღწერა: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ka='დამუშავებისას (დამატებითი)';ru='При обработке (дополнительный)';en='When processing (optional)'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
Процедура ВыполнитьОбработчикПРОПослеОбработки(ПРО, Отказ, Объект, ОбъектМетаданных, Получатели, Знач Выгрузка)
	...
	Если ПРО.ЕстьОбработчикПослеОбработки Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';ka='შეცდომა დამამუშავებლის შესრულებისას: ""[ИмяОбработчика]""; გაცვლის გეგმა: [ИмяПланаОбмена]; მეტამონაცემების ობიექტი: [ОбъектМетаданныхИмя]¶შეცდომის აღწერა: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ka='დამუშავების შემდეგ';ru='После обработки';en='after processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
Функция ДанныеРазличаются(Данные1, Данные2, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='მიცემულია დაუშვებელი პარამეტრის [1] მნიშვნელობა მეთოდის ОбщегоНазначения.ЗначенияСвойствИзменены.';ru='Задано недопустимое значение параметра [1] метода ОбщегоНазначения.ЗначенияСвойствИзменены.';en='Set to an invalid value [1] ObschegoNaznacheniya.ZnacheniyaSvoystvIzmeneny method.'");
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			Если СсылкаСуществует Тогда
				...
				Если ЗаписатьОбъект Тогда
					Комментарий = НСтр("ka='წინა ვერსია (კონფლიქტის ავტომატური გადაწყვეტა).';ru='Предыдущая версия (автоматическое разрешение конфликта).';en='Previous version (automatic conflict resolution).'");
				Иначе
					Комментарий = НСтр("ka='მიმდინარე ვერსია (კონფლიქტის ავტომატური გადაწყვეტა).';ru='Текущая версия (автоматическое разрешение конфликта).';en='Current version (automatic conflict resolution).'");
				КонецЕсли;
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ka='მიმდინარე ვერსია (კონფლიქტის ავტომატური გადაწყვეტა).';ru='Текущая версия (автоматическое разрешение конфликта).';en='Current version (automatic conflict resolution).'"));
			Иначе
				СведенияОВерсииОбъекта = Новый Структура;
				СведенияОВерсииОбъекта.Вставить("АвторВерсии", Отправитель);
...
Процедура ЗарегистрироватьЗапретЗагрузкиДанныхПоДате(ЭлементДанных, Отправитель, СообщениеОбОшибке)
	...
	Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель.Ссылка) И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементДанных.Метаданные()) Тогда
		...
		Если СсылкаСуществует Тогда
			...
			Комментарий = НСтр("ka='ვერსია შექმნილია მონაცემთა სინქრონიზაციის დროს.';ru='Версия создана при синхронизации данных.';en='Version is created during data synchronization.'");
		...
		Иначе
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='%1 აკრძალულ პერიოდში ჩატვირთვა აკრძალულია.%2%2%3';ru='%1 запрещено загружать в запрещенный период.%2%2%3';en='% 1 is not allowed to load in a prohibited period.% 2% 2% 3'");
...
Процедура ЗарегистрироватьПредупреждениеОКоллизииВЖурналеРегистрации(Объект, МетаданныеОбъекта, ЗаписатьОбъект, ЭтоСсылочныйТип)
	...
	Если ЗаписатьОбъект Тогда
		...
		ТекстПредупрежденияЖР = НСтр("ka='წარმოიშვა ობიექტების ცვლილების კონფლიქტი. ამ საინფორმაციო ბაზის ობიექტი შეიცვალა მეორე საინფორმაციო ბაზის ობიექტის ვერსიით.';ru='Возник конфликт изменений объектов."
"Объект этой информационной базы был заменен версией объекта из второй информационной базы.'");
		
	Иначе
		
		ТекстПредупрежденияЖР = НСтр("ka='წარმოიშვა ობიექტების ცვლილების კონფლიქტი. მეორე საინფორმაციო ბაზიდან ობიექტი არ იქნა მიღებული. ამ საინფორმაციო ბაზის ობიექტი არ არის შეცვლილი.';ru='Возник конфликт изменений объектов."
...
Функция ПолучитьДанныеОбъектаСтрокойДоИзменения(Объект, МетаданныеОбъекта, ЭтоСсылочныйТип, СсылкаСуществует, ОбъектВБазе)
	...
	Если ЭтоСсылочныйТип Тогда
		...
		Иначе
			...
			СтрокаОбъекта = НСтр("ka='ობიექტი წაშლილია';ru='Объект удален';en='object deleted'");
