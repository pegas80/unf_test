...
"установлен для нескольких внешних пользователей.';ka='შეცდომა მონაცემთა ბაზაში: ავტორიზაციის ობიექტი ""%1"" (%2) დაყენებულია რამდენიმე გარე მომხმარებლისთვის.'"),
				ОбъектАвторизации,
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в базе данных:"
"Для внешнего пользователя ""%1"" не задан объект авторизации.';ka='შეცდომა მონაცემთა ბაზაში: გარე მომხმარებელ %1-სთვის ავტორიზაციის ობიექტი ვერ მოიძებნა.'"),
			ВнешнийПользователь);
	КонецЕсли;
