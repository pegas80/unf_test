...
Функция ПолноеИмяСлужебногоПользователя(Знач Идентификатор = Неопределено) Экспорт
	...
	Результат = НСтр("ka='<Служебный пользователь %1>';ru='<Служебный пользователь %1>';en='<Call of the user% 1>'");
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().РазделениеДанных.Количество() = 0 Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Если Не ПользовательЗарегистрированКакНеразделенный(ИдентификаторПользователя) Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ka='იდენტიფიკატორ %1-ით მომხმარებელი არ არის რეგისტრირებული როგორც განუცალკევებელი!';ru='Пользователь с идентификатором %1 не зарегистрирован в качестве неразделенного!';en='User ID% 1 is not registered as an undivided!'"),
...
Функция ТекстИсключенияЗаписьНеразделенныхПользователейЗапрещена()
	...
	Возврат НСтр("ka='განუყოფელი მომხმარებლების ჩაწერა გამყოფების ჩართულად გამოყენებისას აკრძალულია.';ru='Запись неразделенных пользователей"
