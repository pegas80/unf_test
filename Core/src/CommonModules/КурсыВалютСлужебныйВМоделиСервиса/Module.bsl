...
Процедура ЗагрузитьКурсы() Экспорт
	...
	Если Дескрипторы.Descriptor.Количество() < 1 Тогда
		ВызватьИсключение(НСтр("ka='სერვისის მენეჯერში არ არის ტიპ ""ვალუტისკურსები""-ს მონაცემები';ru='В менеджере сервиса отсутствуют данные вида ""КурсыВалют""'"));
	КонецЕсли;
...
Процедура ОбработатьПоставляемыеКурсыЗаДень(Знач Дескриптор, Знач ПутьКФайлу)
	...
	Если ДатаКурсов = "" Тогда
		ВызватьИсключение НСтр("ka='ტიპ ""ვალუტისკურსებიდღისთვის"" მონაცემები არ შეიცავს მახასიათებლებს ""თარიღი"". კურსების განახლება შეუძლებელია.';ru='Данные вида ""КурсыВалютЗаДень"" не содержат характеристики ""Дата"". Обновление курсов невозможно.'"); 
	КонецЕсли;
	...
	Для каждого ОбластьДанных Из ОбластиДляОбновления Цикл
		...
		Попытка
			...
			Пока ВыборкаВалюты.Следующий() Цикл
				...
				Иначе
					Комментарий = НСтр("ka='Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.';ru='Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.'"); 
					Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Комментарий, ВыборкаВалюты.Код, ДатаКурсов);
					ИмяСобытия = НСтр("ka='მიწოდებული მონაცემები. ვალუტის კურსის ჩატვირთვის გაუქმება';ru='Поставляемые данные.Отмена загрузки курсов валюты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული მონაცემები. ვალუტის კურსის გავრცელება მონაცემთა სფეროზე';ru='Поставляемые данные.Распространение курсов валют по областям данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
