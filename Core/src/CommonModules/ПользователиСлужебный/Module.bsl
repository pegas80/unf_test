	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы И РолиАдминистратораДоступны() Тогда
			ИнтеграцияСтандартныхПодсистем.ПриСозданииАдминистратора(НайденныйПользователь,
				НСтр("ka='ავტორიზაციის დროს აღმოჩნდა, რომ მომხმარებელს გააჩნია ადმინისტრატორის როლები.';ru='При авторизации у пользователя найдены роли администратора.'"));
		КонецЕсли;
		Возврат РезультатУстановкиПараметровСеанса(РегистрироватьВЖурнале);
	КонецЕсли;
...
"режим конфигурирования 1С:Предприятия для этого использовать не следует.'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='მომხმარებლები. ადმინისტრატორი რეგისტრირებულია ცნობარ ადმინისტრატორებში.';ru='Пользователи.Администратор зарегистрирован в справочнике Пользователи'",
...
Функция СвойстваНеуказанногоПользователя() Экспорт
	...
	Свойства.Вставить("ПолноеИмяДляПоиска", НСтр("ka='<მითითებული არ არის>';ru='<Не указан>'"));
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление          = НСтр("ka='მომხმარებლები';ru='Пользователи'");
	...
	ВидДоступа.Представление          = НСтр("ka='გარე მომხმარებლები';ru='Внешние пользователи'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='არაკორექტული ცნობები მომხმარებლების შესახებ';ru='Некорректные сведения о пользователях'");
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если Не ДействияСПользователемСервиса.ИзменениеПароля Тогда
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვისი: მომხმარებელ ""%1""-ის პაროლის შესაცვლელად არასაკმარისი უფლებებია.';ru='Сервис: Недостаточно прав для изменения пароля пользователя ""%1"".'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьДействительностьПользователя")
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელი ""%1"" უმოქმედოა.';ru='Пользователь ""%1"" недействителен.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ის სააღრიცხვო ჩანაწერი ვერ მოიძებნა.';ru='Не найдена учетная запись пользователя ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   И Не УровеньДоступа.НастройкиДляВхода Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ის პაროლის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения пароля пользователя ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПользовательИБ <> Неопределено И ПользовательИБ.ЗапрещеноИзменятьПароль Тогда
		Если УровеньДоступа.НастройкиДляВхода Тогда
			Если ДополнительныеПараметры.Свойство("УчитыватьСвойствоЗапрещеноИзменятьПароль") Тогда
				ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='დაყენებულია მომხმარებელ ""%1""-ის პაროლის შეცვლის აკრძალვა.';ru='Установлен запрет изменения пароля пользователя ""%1"".'"), Пользователь));
				Возврат Ложь;
			КонецЕсли;
		Иначе
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"მიმართეთ ადმინისტრატორს.'"), Пользователь));
	...
	ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='პაროლის შეცვლა შესაძლებელი იქნება მხოლოდ %1-ის შემდეგ.';ru='Пароль можно будет сменить только через %1.'"), ЧислоИПредмет));
	...
	Если ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   ...
	   И (ВызовИзФормыСменаПароля Или СтарыйПароль <> Неопределено) Тогда
		...
		Если Не Параметры.СтарыйПарольСовпадает Тогда
			Возврат НСтр("ka='ძველი პაროლი არასწორად არის მითითებული.';ru='Старый пароль указан неверно.'");
		КонецЕсли;
	КонецЕсли;
...
"задать настройки входа пользователей в настройках программы.'"),
	...
	Попытка
		...
		Если ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода
		   ...
		   И ИспользованныеПароли.Найти(СохраняемоеЗначениеПароля) <> Неопределено Тогда
			...
			ТекстОшибки = НСтр("ka='ახალი პაროლი უწინ გამოიყენებოდა.';ru='Новый пароль использовался ранее.'");
	...
	Исключение
		...
		Если ВызовИзФормыСменаПароля Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მომხმარებლები. პაროლის შეცვლის შეცდომა';ru='Пользователи.Ошибка смены пароля'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура НачатьОбработкуПользователяИБ(ПользовательОбъект,
	...
	ПараметрыОбработки.Вставить("ТекстСообщенияНедостаточноПрав",
		НСтр("ka='საინფორმაციო ბაზის მომხმარებლის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения пользователя информационной базы.'"));
	...
	Если ОписаниеПользователяИБ.Свойство("Имя") Тогда
		...
		Если ПустаяСтрока(Имя) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='შეუვსებელია სახელი (შესასვლელად).';ru='Не заполнено Имя (для входа).'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='სახელი (შესასვლელად) აღემატება 64 სიმბოლოს.';ru='Имя (для входа) превышает 64 символа.'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='სახელი (შესასვლელად) შეიცავს აკრძალულ სიმბოლოს "":"".';ru='Имя (для входа) содержит запрещенный символ "":"".'"),
				,
				"Имя",
				,
				Отказ);
			   ...
			   И ПользовательИБ.УникальныйИдентификатор
				...
				Если НайденныйПользователь = Неопределено
				 ИЛИ НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
					...
					ТекстОшибки = НСтр("ka='სახელი (შესასვლელად) უკვე დაკავებულია.';ru='Имя (для входа) уже занято.'");
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='სახელი (შესასვლელად) უკვე დაკავებულია მომხმარებელ ""%1""-სთვის.';ru='Имя (для входа) уже занято для пользователя ""%1"".'"),
...
"""%1""-ის როლების განახლებისას გარე მომხმარებელთა ჯგუფ ""%3""-ის როლი ""%2"" მეტამონაცემებში ვერ მოიძებნა.'"),
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მომხმარებლები. მეტამონაცემებში როლი ვერ მოიძებნა';ru='Пользователи.Роль не найдена в метаданных'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Функция ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации,
	...
	Если Результат Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='უკვე არსებობს ობიექტ ""%1""-თან დაკავშირებული გარე მომხმარებელი.';ru='Уже существует внешний пользователь, связанный с объектом ""%1"".'"),
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Если МассивПеремещенныхПользователей.Количество() = 0 И МассивНеПеремещенныхПользователей.Количество() = 0 Тогда
		Если МассивПользователей.Количество() = 1 Тогда
			ТекстСообщения = НСтр("ka='მომხმარებელი ""%1"" უკვე შეყვანილია ჯგუფ ""%2""-ში';ru='Пользователь ""%1"" уже включен в группу ""%2"".'");
			ИмяПеремещаемогоПользователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивПользователей[0], "Наименование");
		Иначе
			ТекстСообщения = НСтр("ka='ყველა არჩეული მომხმარებელი უკვე შეყვანილია ჯგუფ ""%2""-ში.';ru='Все выбранные пользователи уже включены в группу ""%2"".'");
	...
	Если КоличествоПользователей = 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи
		 Или ГруппаПриемник = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			...
			СтрокаДействие = НСтр("ka='ჯგუფიდან გარიცხულია';ru='исключен из группы'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ka='გადაადგილებულია ჯგუფში';ru='перемещен в группу'");
		Иначе
			СтрокаДействие = НСтр("ka='ჩართულია ჯგუფში';ru='включен в группу'");
		КонецЕсли;
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"Л = ru_RU", НСтр("ka='მომხმარებელი,მომხმარებელს,მომხმარებლებს,,,,,,0';ru='пользователь,пользователя,пользователей,,,,,,0'"));
		
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи Тогда
			СтрокаДействие = НСтр("ka='გარიცხულია ჯგუფიდან';ru='исключены из группы'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ka='გადაადგილებულია ჯგუფში';ru='перемещены в группу'");
		Иначе
			СтрокаДействие = НСтр("ka='ჩართულია ჯგუფში';ru='включены в группу'");
		КонецЕсли;
		СообщениеПользователю = НСтр("ka='%1 %2 ""%3""';ru='%1 %2 ""%3""'");
	КонецЕсли;
...
Функция ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
	...
	Возврат НСтр("ka='ავტორიზაცია არ არის შესრულებული. სისტემის მუშაობა დასრულდება.';ru='Авторизация не выполнена. Работа системы будет завершена.'")
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	ЗаголовокОшибки = НСтр("ka='სეანს მიმდინარემომხმარებელი-ს პარამეტრის დაყენება ვერ მოხერხდა.';ru='Не удалось установить параметр сеанса ТекущийПользователь.'") + Символы.ПС;
	...
	Если СведенияОПользователе.ТекущийПользователь        = Неопределено
	 Или СведенияОПользователе.ТекущийВнешнийПользователь = Неопределено Тогда
		...
		Возврат ЗаголовокОшибки + ТекстСообщенияПользовательНеНайденВСправочнике(
			...
			+ НСтр("ka='მომხმარებლის ძებნისას შიდა შეცდომა წარმოიშვა.';ru='Возникла внутренняя ошибка при поиске пользователя.'");
	КонецЕсли;
...
Функция НайтиТекущегоПользователяВСправочнике()
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
			Возврат НСтр("ka='გარე მომხმარებლები გამორთულია.';ru='Внешние пользователи отключены.'");
		КонецЕсли;
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მომხმარებლები.პროგრამაში შესვლის შეცდომა';ru='Пользователи.Ошибка входа в программу'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	Для Каждого ОписаниеРоли Из РолиДляПроверки Цикл
		...
		Если НазначениеРолей.ТолькоДляАдминистраторовСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ სისტემის ადმინისტრატორებისთვის)';ru='""%1"" (предназначена только для администраторов системы)'");
		
		ИначеЕсли РазделениеВключено
		        И НазначениеРолей.ТолькоДляПользователейСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			
			ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ სისტემის მომხმარებლებისთვის)';ru='""%1"" (предназначена только для пользователей системы)'");
		Иначе
			...
			Если НазначениеРолей.ТолькоДляВнешнихПользователей.Получить(ИмяРоли) <> Неопределено Тогда
				ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ გარე მომხმარებლებისთვის)';ru='""%1"" (предназначена только для внешних пользователей)'");
				
			Иначе // Это внешний пользователь.
				ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ მომხმარებლებისთვის)';ru='""%1"" (предназначена только для пользователей)'");
			КонецЕсли;
		КонецЕсли;
	...
	ИмяСобытия = НСтр("ka='მომხმარებლები. შეცდომა საინფორმაციო ბაზის მომხმარებლებისთვის როლების დაყენებისას';ru='Пользователи.Ошибка при установке ролей пользователю ИБ'",
	     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"%2.';ka='მომხმარებელ %1-ისთვის მიუწვდომელი როლების დანიშვნის მცდელობა: %2.'"),
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ისთვის შეუძლებელია მიუწვდომელი როლის წაშლა.';ru='Пользователю ""%1"" невозможно удалить недоступную роль.'"),
			ПользовательИБ.ПолноеИмя);
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ისთვის შეუძლებელია მიუწვდომელი როლების წაშლა.';ru='Пользователю ""%1"" невозможно удалить недоступные роли.'"),
			ПользовательИБ.ПолноеИмя);
	Иначе
...
"Обратитесь к администратору.'");
		Если ПриЗапуске Тогда
			...
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მომხმარებლები.პროგრამაში შესვლის შეცდომა';ru='Пользователи.Ошибка входа в программу'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მომხმარებლები. უკანასკნელი აქტიურობის თარიღის განახლების შეცდომა';ru='Пользователи.Ошибка обновления даты последней активности'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, МинимальнаяДлинаПароля, СложныйПароль)
	...
	Если СтрДлина(Пароль) < МинимальнаяДлинаПароля Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ახალი პაროლის სიმბოლოების რაოდენობა %1-ზე ნაკლები არ უნდა იყოს.';ru='Количество символов нового пароля должно быть не менее %1.'"),
	...
	Если СложныйПароль И Не ПарольОтвечаетТребованиямСложности(Пароль) Тогда
		Возврат НСтр("ka='პაროლი არ პასუხობს სირთულის მოთხოვნებს.';ru='Пароль не отвечает требованиям сложности.'")
			+ Символы.ПС + Символы.ПС
...
"%2'");
			КонецЕсли;
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მომხმარებლები. პროგრამაში შესვლის ავტომატური აკრძალვის შეცდომა';ru='Пользователи.Ошибка автоматического запрещения входа в программу'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
