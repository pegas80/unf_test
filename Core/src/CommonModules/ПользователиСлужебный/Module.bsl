...
Процедура УстановкаПараметровСеанса(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить параметр сеанса ТекущийПользователь по причине:""""%1"".""""Обратитесь к администратору.'; ka = 'სეანს მიმდინარემომხმარებელი-ს პარამეტრის დადგენა ვერ მოხერხდა შემდეგი მიზეზით:""""%1"".""""მიმართეთ ადმინისტრატორს.'; en = 'The session parameter CurrentUser could not be set for reason: """"%1"".""""Contact your administrator.'"),
...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы И РолиАдминистратораДоступны() Тогда
			ИнтеграцияСтандартныхПодсистем.ПриСозданииАдминистратора(НайденныйПользователь,
				НСтр("ka='ავტორიზაციის დროს აღმოჩნდა, რომ მომხმარებელს გააჩნია ადმინისტრატორის როლები.';ru='При авторизации у пользователя найдены роли администратора.';en='When logging the user found the administrator role.'"));
		КонецЕсли;
		Возврат РезультатУстановкиПараметровСеанса(РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Если Не РолиАдминистратораДоступны() Тогда
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
			НСтр("ru = 'Запуск от имени пользователя с правом Администрирование невозможен,""так как он не зарегистрирован в списке пользователей.""""Для ведения списка и настройки прав пользователей предназначен список Пользователи,""режим конфигурирования 1С:Предприятия для этого использовать не следует.'; ka = 'უფლება ადმინისტრირების მქონე მომხმარებლის სახელით გაშვება შეუძლებელია, ვინაიდან იგი არ არის რეგისტრირებული მომხმარებელთა სიაში.""""სიის საწარმოებლად და მომხმარებელთა უფლებების გასამართად განკუთვნილია სია მომხმარებლები, ამისათვის კონფიგურირების რეჟიმ 1С:საწარმოს გამოყენება საჭირო არ არის.'; en = 'Running as a user with the Administration right is impossible, ""since he is not registered in the list of users."" ""The Users list is intended for maintaining the list and setting up user rights,"" the 1C:Enterprise configuration mode should not be used for this.'"),
			, РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке,
			НСтр("ru = 'Не удалось выполнить автоматическую регистрацию администратора в списке по причине:""""%1"".""""Для ведения списка и настройки прав пользователей предназначен список Пользователи,""режим конфигурирования 1С:Предприятия для этого использовать не следует.'; ka = 'სიაში ადმინისტრატორის ავტომატური რეგისტრაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:""""%1"".""""სიის საწარმოებლად და მომხმარებლების უფლებების გასამართად განკუთვნილია სია ""მომხმარებლები"", ამისთვის 1С:საწარმოს კონფიგურირების რეჟიმის გამოყენება არ არის საჭირო.'; en = 'The automatic registration of the administrator in the list failed due to the following reason: """"%1"".""""The Users list is intended for maintaining the list and setting up user rights,""1C:Enterprise configuration mode should not be used for this.'"),
			РегистрироватьВЖурнале);
	КонецПопытки;
	
	Комментарий =
		НСтр("ru = 'Выполнен запуск от имени пользователя с ролью ""Полные права"",""который не зарегистрирован в списке пользователей.""Выполнена автоматическая регистрация в списке пользователей.""""Для ведения списка и настройки прав пользователей предназначен список Пользователи,""режим конфигурирования 1С:Предприятия для этого использовать не следует.'; ka = 'განხორციელდა გაშვება როლ ""სრული უფლებები""-ს მქონე მომხმარებლის სახელით, რომელიც მომხმარებლების სიაში არ არის რეგისტრირებული.""მომხმარებლების სიაში შესრულებულია ავტომატური რეგისტრაცია.""""სიის საწარმოებლად და მომხმარებლების უფლებების გასამართად განკუთვნილია სია ""მომხმარებლები"", ამისთვის 1С:საწარმოს კონფიგურირების რეჟიმის გამოყენება არ არის საჭირო.'; en = 'Launched on behalf of a user with the role ""Full rights"",""which is not registered in the list of users.""Automatic registration in the list of users was performed.""""The Users list is intended for maintaining the list and setting up user rights,""1C:Enterprise configuration mode is used for this do not do it.'");
	
	ИнтеграцияСтандартныхПодсистем.ПослеЗаписиАдминистратораПриАвторизации(Комментарий);
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Пользователи.Администратор зарегистрирован в справочнике Пользователи'; ka = 'მომხმარებლები. ადმინისტრატორი რეგისტრირებულია ცნობარ ადმინისტრატორებში.'; en = 'Users. The administrator is registered in the Users directory'",
...
Процедура УстановитьНачальныеНастройки(Знач ИмяПользователя, Пользователь = Неопределено) Экспорт
	...
	Попытка
		ВерсияЧислом = Число(СоставВерсии[0] + СоставВерсии[1] + СоставВерсии[2]);
	Исключение
		ВерсияЧислом = 0;
	КонецПопытки;
...
Функция ОшибкаНедостаточноПравДляВходаВПрограмму(РегистрироватьВЖурнале = Истина) Экспорт
	...
	Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
		НСтр("ru = 'Недостаточно прав для входа в программу.""""Обратитесь к администратору.'; ka = 'პროგრამაში შესასვლელად არასაკმარისი უფლებებია.""""მიმართეთ ადმინისტრატორს.'; en = 'Insufficient rights to log into the program.""""Contact your administrator.'"),
		, РегистрироватьВЖурнале);
...
Процедура ОбработатьИнтерфейсРолей(Действие, Параметры) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей()""Неверное значение параметра Действие: ""%1"".'; ka = 'შეცდომა პროცედურაში მომხმარებლებისამსახურებრივი.როლებისინტერფეისისდამუშავება()""პარამეტრ მოქმედება-ს არასწორი მნიშვნელობა: ""%1"".'; en = 'Error in the procedure UsersService.ProcessRoleInterface()""Invalid value for the Action parameter: ""%1"".'"),
			Действие);
	КонецЕсли;
...
Функция СвойстваНеуказанногоПользователя() Экспорт
	...
	Свойства.Вставить("ПолноеИмяДляПоиска", НСтр("ka='<მითითებული არ არის>';ru='<Не указан>';en='<Not Set>'"));
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление          = НСтр("ka='მომხმარებლები';ru='Пользователи';en='users'");
	...
	ВидДоступа.Представление          = НСтр("ka='გარე მომხმარებლები';ru='Внешние пользователи';en='external users'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='არაკორექტული ცნობები მომხმარებლების შესახებ';ru='Некорректные сведения о пользователях';en='Incorrect information about users'");
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если Не ДействияСПользователемСервиса.ИзменениеПароля Тогда
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვისი: მომხმარებელ ""%1""-ის პაროლის შესაცვლელად არასაკმარისი უფლებებია.';ru='Сервис: Недостаточно прав для изменения пароля пользователя ""%1"".';en='Service: Not enough rights for the user to change the password ""%1"".'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьДействительностьПользователя")
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელი ""%1"" უმოქმედოა.';ru='Пользователь ""%1"" недействителен.';en='User ""%1"" is invalid.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ის სააღრიცხვო ჩანაწერი ვერ მოიძებნა.';ru='Не найдена учетная запись пользователя ""%1"".';en='Found a user account ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   И Не УровеньДоступа.НастройкиДляВхода Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ის პაროლის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения пароля пользователя ""%1"".';en='Insufficient permissions for the user to change the password ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПользовательИБ <> Неопределено И ПользовательИБ.ЗапрещеноИзменятьПароль Тогда
		Если УровеньДоступа.НастройкиДляВхода Тогда
			Если ДополнительныеПараметры.Свойство("УчитыватьСвойствоЗапрещеноИзменятьПароль") Тогда
				ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='დაყენებულია მომხმარებელ ""%1""-ის პაროლის შეცვლის აკრძალვა.';ru='Установлен запрет изменения пароля пользователя ""%1"".';en='Prohibits the user to change the password ""%1"".'"), Пользователь));
				Возврат Ложь;
			КонецЕсли;
		Иначе
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установлен запрет изменения пароля пользователя ""%1"".""Обратитесь к администратору.'; ka = 'დაყენებულია მომხმარებლის პაროლის ""%1"" შეცვლის აკრძალვა.""მიმართეთ ადმინისტრატორს.'; en = 'Changing the password for user ""%1"" is prohibited.""Contact your administrator.'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ЧислоИПредмет = Формат(КоличествоДней, "ЧГ=") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоДней,
			"Л = ru_RU", НСтр("ru = 'день,дня,дней,,,,,,0'; ka = 'დღე, დღის, დღეები,,,,,,0'; en = 'day, for day, days ,,,,,, 0'"));
	
	ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Пароль можно будет сменить только через %1.'; ka = 'პაროლის შეცვლა შესაძლებელი იქნება მხოლოდ %1-ის შემდეგ.'; en = 'The password can be changed only by %1.'"), ЧислоИПредмет));
...
Функция ОбработатьНовыйПароль(Параметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   ...
	   И (ВызовИзФормыСменаПароля Или СтарыйПароль <> Неопределено) Тогда
		...
		Если Не Параметры.СтарыйПарольСовпадает Тогда
			Возврат НСтр("ka='ძველი პაროლი არასწორად არის მითითებული.';ru='Старый пароль указан неверно.';en='The old password is incorrect.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1""""Ограничение установлено в конфигураторе в меню ""Администрирование"" пункт ""Параметры информационной базы ..."".""Следует очистить минимальную длину и требование сложности пароля в конфигураторе и""задать настройки входа пользователей в настройках программы.'; ka = '%1""""შეზღუდვა დაყენებულია კონფიგურატორში მენიუ ""ადმინისტრირება""-ში პუნქტი ""საინფორმაციო ბაზის პარამეტრები..."".""საჭიროა პაროლის მინიმალური სიგრძის და სირთულის მოთხოვნის გასუფთავება კონფიგურატორში და პროგრამის გამართვებში მომხმარებლების შესვლის გამართვების მიცემა.'; en = '%1""""The restriction is set in the configurator in the ""Administration"" menu, item ""Infobase parameters..."".""You should clear the minimum length and password complexity requirement in the configurator and""set user login settings in the program settings.'"),
			ТекстОшибки);
	КонецЕсли;
	...
	Попытка
		...
		Если ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода
		   ...
		   И ИспользованныеПароли.Найти(СохраняемоеЗначениеПароля) <> Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru = 'Новый пароль использовался ранее.'; ka = 'ახალი პაროლი მანამდე გამოიყენებოდა.'; en = 'A new password has been used previously.'");
	...
	Исключение
		...
		Если ВызовИзФормыСменаПароля Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пользователи.Ошибка смены пароля'; ka = 'მომხმარებლები. პაროლის შეცვლის შეცდომა'; en = 'Users. Password change error'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось сменить пароль пользователя ""%1"" по причине:""%2'; ka = 'მომხმარებელ ""%1""-ის პაროლის შეცვლა ვერ მოხერხდა შემდეგი მიზეზით: %2'; en = 'Failed to change password for user ''%1'' due to reason: ''%2'''"),
...
Функция ПодсказкаДляНовогоПароля() Экспорт
	...
	Возврат
		НСтр("ru = 'Надежный пароль:
              ...
              |- не совпадает с именем (для входа).'; ka = 'ძლიერი პაროლი:
              |- აქვს მინიმუმ 7 სიმბოლო;
              |- შეიცავს 4-დან 3 ტიპის სიმბოლოს: დიდ ასოებს
              |   ასოები, მცირე ასოები, რიცხვები, სპეციალური სიმბოლოები;
              |- არ ემთხვევა სახელს (შესვლისთვის).'; en = 'Strong password:
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ИмяПользователя())
			   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
				...
				ВременныйАдминистраторИБ.Имя = НСтр("ru = 'Временный первый администратор'; ka = 'დროებითი პირველი ადმინისტრატორი'; en = 'Since the first administrator'")
			...
			ВременныйПользовательИБ.Имя = НСтр("ru = 'Временный пользователь'; ka = 'დროებითი მომხმარებელი'; en = 'a temporary user'")
				+ " (" + Строка(Новый УникальныйИдентификатор) + ")";
...
Процедура НачатьОбработкуПользователяИБ(ПользовательОбъект,
	...
	ПараметрыОбработки.Вставить("ТекстСообщенияНедостаточноПрав",
		НСтр("ka='საინფორმაციო ბაზის მომხმარებლის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения пользователя информационной базы.';en='Not have permission to change the user information database.'"));
	...
	Если НЕ ОписаниеПользователяИБ.Свойство("Действие") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""В параметре ОписаниеПользователяИБ не указано свойство Действие.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას.""პარამეტრ საინფორმაციო ბაზის მომხმარებლის აღწერილობაში არ არის მითითებული თვისება მოქმედება.'; en = 'Error while writing user ""%1."" The Action property is not specified in the IB User Description parameter.'"),
	...
	Если ОписаниеПользователяИБ.Действие <> "Записать"
	   И ОписаниеПользователяИБ.Действие <> "Удалить" Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""В параметре ОписаниеПользователяИБ указано""неверное значение ""%2"" свойства Действие.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას.""პარამეტრ საინფორმაციო ბაზის მომხმარებლის აღწერილობაში მითითებულია თვისება მოქმედება-ს არასწორი მნიშვნელობა ""%2"".'; en = 'An error occurred while writing user ""%1"". The IB User Description parameter specified an ""incorrect value ""%2"" of the Action property.'"),
			ПользовательОбъект.Ссылка,
	...
	Если ОписаниеПользователяИБ.Действие = "Записать"
	   ...
	   И ОписаниеПользователяИБ.УникальныйИдентификатор
		...
		Если ПараметрыОбработки.СтарыйПользовательИБСуществует Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Нельзя сопоставить пользователя ИБ с пользователем в справочнике,""с которым уже сопоставлен другой пользователем ИБ.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. სბ-ის მომხმარებლის შეპირისპირება ცნობარში არსებულ მომხმარებელთან, რომელთანაც უკვე შეპირისპირებულია სბ-ის სხვა მომხმარებელი, არ შეიძლება.'; en = 'Error when writing user ""%1"". ""It is not possible to map an information security user to a user in the directory,"" to which another information security user is already mapped.'"),
		...
		Если ПользовательПоИдентификаторуСуществует(
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Нельзя сопоставить пользователя ИБ с этим пользователем в справочнике,""так как он уже сопоставлен с другим пользователем в справочнике""""%2"".'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. სბ-ის მომხმარებლის შეპირისპირება ცნობარში ამ მომხმარებელთან არ შეიძლება, ვინაიდან იგი უკვე შეპირისპირებულია სხვა მომხმარებელთან ცნობარ ""%2""-ში.'; en = 'Error when writing user ""%1"". ""It is not possible to map the information security user to this user in the directory,"" since he is already mapped to another user in the directory ""%2"".'"),
	...
	Если НЕ ЗначениеЗаполнено(ТекущийПользовательИБ.Имя)
	   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
		...
		Если ТипЗнч(ОписаниеПользователяИБ) = Тип("Структура") Тогда
			...
			Если ЗапретРедактированияРолей()
				...
				Если ВРег(Пользователи.РольАдминистратораСистемы().Имя) = ВРег("ПолныеПрава") Тогда
					Текст = НСтр("ru = 'В список пользователей программы добавляется первый пользователь,""поэтому ему автоматически будет назначена роль ""Полные права"".""Продолжить?'; ka = 'პროგრამის მომხმარებელთა სიას ემატება პირველი მომხმარებელი, ამიტომ, მისთვის ავტომატურად დაინიშნება როლი ""სრული უფლებები"". გავაგრძელოთ?'; en = 'The first user is added to the list of program users,"" so he will automatically be assigned the ""Full Rights"" role."" Continue?'");
				Иначе
					Текст = НСтр("ru = 'В список пользователей программы добавляется первый пользователь, поэтому ему""автоматически будут назначены роли ""Администратор системы"" и ""Полные права"".""Продолжить?'; ka = 'პროგრამის მომხმარებლების სიას დაემატება პირველი მომხმარებელი, ამიტომ მას, ავტომატურად დაენიშნება როლი ""სისტემის ადმინისტრატორი"" და სრული უფლებები"" ში.""გავაგრძელოთ?'; en = 'The first user is added to the list of program users, so he will be automatically assigned the “System Administrator” and “Full Rights” roles.” Continue?'");
				КонецЕсли;
		...
		Иначе
			...
			Текст = НСтр("ru = 'Первый пользователь информационной базы должен иметь полные права.""Внешний пользователь не может быть полноправным.""Сначала создайте администратора в справочнике Пользователи.'; ka = 'საინფორმაციო ბაზის პირველ მომხმარებელს სრული უფლებები უნდა ჰქონდეს. გარე მომხმარებელი არ შეიძლება, რომ სრულუფლებიანი იყოს. თავდაპირველად, ცნობარ მომხმარებლებში შექმენით ადმინისტრატორი.'; en = 'The first user of the infobase must have full rights. ""An external user cannot have full rights."" First, create an administrator in the Users directory.'");
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОписаниеПользователяИБ.Свойство("Имя") Тогда
		...
		Если ПустаяСтрока(Имя) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='შეუვსებელია სახელი (შესასვლელად).';ru='Не заполнено Имя (для входа).';en='Do not fill in the name (login).'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='სახელი (შესასვლელად) აღემატება 64 სიმბოლოს.';ru='Имя (для входа) превышает 64 символа.';en='Name (login) exceeds 64 characters.'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='სახელი (შესასვლელად) შეიცავს აკრძალულ სიმბოლოს "":"".';ru='Имя (для входа) содержит запрещенный символ "":"".';en='Name (login) contains illegal characters "":"".'"),
				,
				"Имя",
				,
				Отказ);
			   ...
			   И ПользовательИБ.УникальныйИдентификатор
				...
				Если НайденныйПользователь = Неопределено
				 ИЛИ НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
					...
					ТекстОшибки = НСтр("ka='სახელი (შესასვლელად) უკვე დაკავებულია.';ru='Имя (для входа) уже занято.';en='Name (login) is already taken.'");
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='სახელი (შესასვლელად) უკვე დაკავებულია მომხმარებელ ""%1""-სთვის.';ru='Имя (для входа) уже занято для пользователя ""%1"".';en='Name (login) already occupied for the user ""%1"".'"),
	...
	Если ОписаниеПользователяИБ.Свойство("ПользовательОС") Тогда
		...
		Если Не ПустаяСтрока(ОписаниеПользователяИБ.ПользовательОС)
		   И Не СтандартныеПодсистемыСервер.ЭтоУчебнаяПлатформа() Тогда
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Пользователь ОС должен быть в формате""""\\ИмяДомена\ИмяПользователя"".'; ka = 'საოპერაციო სისტემის მომხმარებელი უნდა იყოს შემდეგ ფორმატში """"\\დომენისსახელი\მომხმარებლისსახელი"".'; en = 'The OS user must be in the format ""\\DomainName\UserName"".'"),
					,
					"ПользовательОС",
					,
					Отказ);
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При обновлении ролей внешнего пользователя""""%1""""роль ""%2""""группы внешних пользователей ""%3""""не найдена в метаданных.'; ka = 'გარე მომხმარებელ""""%1""-ის როლების განახლებისას გარე მომხმარებელთა ჯგუფ ""%3""-ის როლი ""%2"" მეტამონაცემებში ვერ მოიძებნა.'; en = 'When updating external user roles ""%1"" the role ""%2"" of the external user group ""%3"" was not found in the metadata.'"),
				СокрЛП(Выборка.ВнешнийПользователь.Наименование),
				Выборка.Роль,
				Строка(Выборка.ГруппаВнешнихПользователей));
			
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пользователи.Роль не найдена в метаданных'; ka = 'მომხმარებლები. მეტამონაცემებში როლი ვერ მოიძებნა'; en = 'Users.Role not found in metadata'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Функция ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации,
	...
	Если Результат Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='უკვე არსებობს ობიექტ ""%1""-თან დაკავშირებული გარე მომხმარებელი.';ru='Уже существует внешний пользователь, связанный с объектом ""%1"".';en='Already there is an external user associated with the object ""%1"".'"),
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Если МассивПеремещенныхПользователей.Количество() = 0 И МассивНеПеремещенныхПользователей.Количество() = 0 Тогда
		Если МассивПользователей.Количество() = 1 Тогда
			ТекстСообщения = НСтр("ka='მომხმარებელი ""%1"" უკვე შეყვანილია ჯგუფ ""%2""-ში';ru='Пользователь ""%1"" уже включен в группу ""%2"".';en='User ""%1"" already included in the group ""2%"".'");
			ИмяПеремещаемогоПользователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивПользователей[0], "Наименование");
		Иначе
			ТекстСообщения = НСтр("ka='ყველა არჩეული მომხმარებელი უკვე შეყვანილია ჯგუფ ""%2""-ში.';ru='Все выбранные пользователи уже включены в группу ""%2"".';en=""All selected users are already included in the group '% 2'.""");
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			Если ТипПользователяСовпадаетСГруппой Тогда
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Пользователь ""%1"" не может быть включен в группу ""%2"",""т.к. у группы стоит признак ""Все пользователи заданного типа"".'; ka = 'მომხმარებელი ""%1"" არ შეიძლება, რომ შეყვანილი იქნას ჯგუფ ""%2""-ში, ვინაიდან ჯგუფს დაყენებული აქვს ნიშან-თვისება ""მოცემული ტიპის ყველა მომხმარებელი"".'; en = 'User ""%1"" cannot be included in group ""%2"" because the group has the ""All users of the specified type"" attribute.'"),
					Предмет, НаименованиеГруппы) + Символы.ПС;
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Пользователь ""%1"" не может быть включен в группу ""%2"",""т.к. в состав ее участников входят только %3.'; ka = 'მომხმარებელი ""%1"" არ შეიძლება, რომ შეყვანილი იქნას ჯგუფ ""%2""-ში, ვინაიდან მის მონაწილეთა შემადგენლობაში შედიან მხოლოდ %3.'; en = 'User ""%1"" cannot be included in group ""%2"" because its members include only %3.'"),
		...
		Иначе
			...
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не все пользователи могут быть включены в группу ""%1"",""т.к. в состав ее участников входят только %2""или у группы стоит признак ""Все пользователи заданного типа"".'; ka = 'ყველა მომხმარებელი არ შეიძლება, რომ შეყვანილი იქნას ჯგუფ ""%1""-ში, ვინაიდან მის შემადგენლობაში შედიან მხოლოდ %2, ან ჯგუფს დაყენებული აქვს ნიშან-თვისება ""მოცემული ტიპის ყველა მომხმარებელი"".'; en = 'Not all users can be included in the group ""%1"" because its members include only %2, or the group has the ""All users of the specified type"" attribute.'"),
				НаименованиеГруппы,
	...
	Если КоличествоПользователей = 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи
		 Или ГруппаПриемник = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			...
			СтрокаДействие = НСтр("ka='ჯგუფიდან გარიცხულია';ru='исключен из группы';en='expelled from the group'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ka='გადაადგილებულია ჯგუფში';ru='перемещен в группу';en='It moved into the group'");
		Иначе
			СтрокаДействие = НСтр("ka='ჩართულია ჯგუფში';ru='включен в группу';en='included in the group'");
		КонецЕсли;
		
		СообщениеПользователю = НСтр("ru = '""%1"" %2 ""%3""'; ka = '""%1"" %2 ""%3""'; en = '""%1"" %2 ""%3""'");
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"Л = ru_RU", НСтр("ru = 'пользователь,пользователя,пользователей,,,,,,0'; ka = 'მომხმარებელი,მომხმარებელს,მომხმარებლებს,,,,,,0'; en = 'user,for user, users 0 ,,,,,,'"));
		
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи Тогда
			СтрокаДействие = НСтр("ka='გარიცხულია ჯგუფიდან';ru='исключены из группы';en='excluded from group'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ka='გადაადგილებულია ჯგუფში';ru='перемещены в группу';en='moved into the group'");
		Иначе
			СтрокаДействие = НСтр("ka='ჩართულია ჯგუფში';ru='включены в группу';en='included in the group'");
		КонецЕсли;
		СообщениеПользователю = НСтр("ru = '%1 %2 ""%3""'; ka = '%1 %2 ""%3""'; en = '%1 %2 ""%3""'");
	КонецЕсли;
...
Функция ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
	...
	Возврат НСтр("ka='ავტორიზაცია არ არის შესრულებული. სისტემის მუშაობა დასრულდება.';ru='Авторизация не выполнена. Работа системы будет завершена.';en='Authorization is not performed. System operation is completed.'")
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось установить параметр сеанса ТекущийПользователь.'; ka = 'სეანს მიმდინარემომხმარებელი-ს პარამეტრის დაყენება ვერ მოხერხდა.'; en = 'Failed to set the CurrentUser session parameter.'") + Символы.ПС;
	...
	Если СведенияОПользователе.ТекущийПользователь        = Неопределено
	 Или СведенияОПользователе.ТекущийВнешнийПользователь = Неопределено Тогда
		...
		Возврат ЗаголовокОшибки + ТекстСообщенияПользовательНеНайденВСправочнике(
			...
			+ НСтр("ka='მომხმარებლის ძებნისას შიდა შეცდომა წარმოიშვა.';ru='Возникла внутренняя ошибка при поиске пользователя.';en='An internal error occurred when a user searches.'");
	КонецЕсли;
...
Функция ТекстСообщенияТекущийПользовательНедоступенВСеансеБезРазделителей()
	...
	Возврат
		НСтр("ru = 'Недопустимое получение параметра сеанса ТекущийПользователь""в сеансе без указания всех разделителей.'; ka = 'სეანსის პარამეტრის დაუშვებელი მიღება მიმდინარემომხმარებელი სეანსში არის ყველა გამყოფის მიუთითებლად.'; en = 'It is illegal to retrieve the session parameter ""CurrentUser"" in a session without specifying all delimiters.'");
...
Функция НайтиТекущегоПользователяВСправочнике()
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
			Возврат НСтр("ka='გარე მომხმარებლები გამორთულია.';ru='Внешние пользователи отключены.';en='External users are disconnected.'");
		КонецЕсли;
...
Функция ТекстСообщенияПользовательНеНайденВСправочнике(ИмяПользователя)
	...
	Если ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
		ШаблонСообщенияОбОшибке =
			НСтр("ru = 'Пользователь ""%1"" не найден в справочниках""""Пользователи"" и ""Внешние пользователи"".""""Обратитесь к администратору.'; ka = 'მომხმარებელი ""%1"" ცნობარ ""მომხმარებლები""-ში და ""გარე მომხმარებლები""-ში  ვერ მოიძებნა.""""მიმართეთ ადმინისტრატორს.'; en = 'User ""%1"" was not found in the ""Users"" and ""External Users"" directories."" ""Contact your administrator.'");
	Иначе
		ШаблонСообщенияОбОшибке =
			НСтр("ru = 'Пользователь ""%1"" не найден в справочнике ""Пользователи"".""""Обратитесь к администратору.'; ka = 'ცნობარ ""მომხმარებლები""-ში მომხმარებელი %1 ვერ მოიძებნა.""""მიმართეთ ადმინისტრატორს.'; en = 'User ""%1"" was not found in the ""Users"" directory.""""Contact your administrator.'");
	КонецЕсли;
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Пользователи.Ошибка входа в программу'; ka = 'მომხმარებლები.პროგრამაში შესვლის შეცდომა'; en = 'Users. Error logging into the program'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	Для Каждого ОписаниеРоли Из РолиДляПроверки Цикл
		...
		Если НазначениеРолей.ТолькоДляАдминистраторовСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ სისტემის ადმინისტრატორებისთვის)';ru='""%1"" (предназначена только для администраторов системы)';en='""%1"" (intended only for system administrators)'");
		
		ИначеЕсли РазделениеВключено
		        И НазначениеРолей.ТолькоДляПользователейСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			
			ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ სისტემის მომხმარებლებისთვის)';ru='""%1"" (предназначена только для пользователей системы)';en='""%1"" (intended only for users of the system)'");
		Иначе
			...
			Если НазначениеРолей.ТолькоДляВнешнихПользователей.Получить(ИмяРоли) <> Неопределено Тогда
				ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ გარე მომხმარებლებისთვის)';ru='""%1"" (предназначена только для внешних пользователей)';en='""%1"" (intended for external users only)'");
				
			Иначе // Это внешний пользователь.
				ТекстШаблона = НСтр("ka='""%1"" (განკუთვნილია მხოლოდ მომხმარებლებისთვის)';ru='""%1"" (предназначена только для пользователей)';en='""%1"" (intended for users only)'");
			КонецЕсли;
		КонецЕсли;
	...
	ИмяСобытия = НСтр("ru = 'Пользователи.Ошибка при установке ролей пользователю ИБ'; ka = 'მომხმარებლები. შეცდომა საინფორმაციო ბაზის მომხმარებლებისთვის როლების დაყენებისას'; en = 'Users. Error when setting roles for an IB user'",
	     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если РежимПроверки = "ПриЗапуске" Тогда
		Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 Тогда
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Попытка входа пользователя %1 с недоступной ролью:""%2.'; ka = 'მიუწვდომელი როლის მქონე მომხმარებელ %1-ის შესვლის მცდელობა:""%2.'; en = 'Attempted login for user %1 with inaccessible role: ""%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		Иначе
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Попытка входа пользователя %1 с недоступными ролями:""%2.'; ka = 'მიუწვდომელი როლების მქონე მომხმარებელ %1-ის შესვლის მცდელობა:""%2.'; en = 'Attempted login for user %1 with unavailable roles:""%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		КонецЕсли;
		...
		ТекстСообщенияВхода =
			НСтр("ru = 'Невозможно выполнить вход из-за наличия недоступных ролей.""Обратитесь к администратору.'; ka = 'შესვლის შესრულება შეუძლებელია მიუწვდომელი როლების არსებობის გამო. მიმართეთ ადმინისტრატორს.'; en = 'You cannot sign in because there are inaccessible roles.""Contact your administrator.'");
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка удалить пользователю %1 недоступную роль""""%2"".'; ka = 'მომხმარებელ %1-ისთვის მიუწვდომელი როლ ""%2""-ის წაშლის მცდელობა.'; en = 'An attempt was made to remove the inaccessible role """"%2"" for user %1.'"),
			ПользовательИБ.ПолноеИмя, УдаляемыеНедоступныеРоли);
			
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка удалить пользователю %1 недоступные роли:""%2.'; ka = 'მომხმარებელ %1-ისთვის მიუწვდომელი როლების წაშლის მცდელობა:""%2.'; en = 'An attempt was made to remove inaccessible roles for user %1: ""%2.'"),
			ПользовательИБ.ПолноеИмя, УдаляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка назначить пользователю %1 недоступную роль:""%2.'; ka = 'მომხმარებელ %1-ისთვის მიუწვდომელი როლის დანიშვნის მცდელობა:""%2.'; en = 'An attempt was made to assign user %1 an unavailable role: ""%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
			
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка назначить пользователю %1 недоступные роли:""%2.'; ka = 'მომხმარებელ %1-ისთვის მიუწვდომელი როლების დანიშვნის მცდელობა: %2.'; en = 'An attempt was made to assign unavailable roles to user %1: ""%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ისთვის შეუძლებელია მიუწვდომელი როლის წაშლა.';ru='Пользователю ""%1"" невозможно удалить недоступную роль.';en='User ""%1"" can not be deleted inaccessible role.'"),
			ПользовательИБ.ПолноеИмя);
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-ისთვის შეუძლებელია მიუწვდომელი როლების წაშლა.';ru='Пользователю ""%1"" невозможно удалить недоступные роли.';en='User ""%1"" can not be deleted inaccessible role.'"),
			ПользовательИБ.ПолноеИмя);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователю ""%1"" невозможно назначить недоступную роль""%2.'; ka = 'მომხმარებელ ""%1""-ისთვის შეუძლებელია მიუწვდომელი როლ %2-ის დანიშვნა.'; en = 'User ''%1'' cannot be assigned to the unavailable role ''%2''.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователю ""%1"" невозможно назначить недоступные роли:""%2.'; ka = 'მომხმარებელ ""%1""-ისთვის შეუძლებელია მიუწვდომელი როლების დანიშვნა:""%2.'; en = 'User ""%1"" cannot be assigned to the unavailable roles: ""%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
...
Функция ТребуетсяСменитьПароль(ОписаниеОшибки = "", ПриЗапуске = Ложь, РегистрироватьВЖурнале = Истина)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ШаблонТекстаОшибки =
			НСтр("ru = 'Не удалось записать сведения о текущем пользователе по причине:""%1""""Обратитесь к администратору.'; ka = 'მიმდინარე მომხმარებლის შესახებ ინფორმაციის ჩაწერა ვერ მოხერხდა, რადგან: ""% 1""""დაუკავშირდით თქვენს ადმინისტრატორს.'; en = 'Failed to record information about the current user because: ""%1""""Contact your administrator.'");
		Если ПриЗапуске Тогда
			...
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Пользователи.Ошибка входа в программу'; ka = 'მომხმარებლები.პროგრამაში შესვლის შეცდომა'; en = 'Users.Error entry into the program'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Пользователи.Ошибка обновления даты последней активности'; ka = 'მომხმარებლები. უკანასკნელი აქტიურობის თარიღის განახლების შეცდომა'; en = 'USers.Error renovation last activity date'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, МинимальнаяДлинаПароля, СложныйПароль)
	...
	Если СтрДлина(Пароль) < МинимальнаяДлинаПароля Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Количество символов нового пароля должно быть не менее %1.'; ka = 'ახალი პაროლის სიმბოლოების რაოდენობა %1-ზე ნაკლები არ უნდა იყოს.'; en = 'Number of new characters of the password must be at least %1.'"),
	...
	Если СложныйПароль И Не ПарольОтвечаетТребованиямСложности(Пароль) Тогда
		Возврат НСтр("ka='პაროლი არ პასუხობს სირთულის მოთხოვნებს.';ru='Пароль не отвечает требованиям сложности.';en='Password does not meet complexity requirements.'")
			+ Символы.ПС + Символы.ПС
...
Функция ПарольОтвечаетТребованиямСложности(Пароль)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить отвечает ли пароль требованиям сложности по причине:""%1'; ka = 'შემდეგი მიზეზით ვერ მოხერხდა იმის შემოწმება, პასუხობს თუ არა პაროლი სირთულის მოთხოვნებს:""%1'; en = 'Failed to check whether the password meets the complexity requirements due to: ""%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
Процедура ПроверитьИзмененияРеквизитовПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи")
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Реквизит Служебный не допускается изменять в подписках на события.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. მოვლენებზე ხელმოწერებში რეკვიზიტ სამსახურებრივი-ს შეცვლა არ არის დაშვებული.'; en = 'Error when writing user ""%1"". ""The Service attribute cannot be changed in event subscriptions.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	
	Если БлокируемыеРеквизиты.Подготовлен <> ПользовательОбъект.Подготовлен Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Реквизит Подготовлен не допускается изменять в подписках на события.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. რეკვიზიტ მომზადებულია-ს შეცვლა მოვლენების გამოწერებში დაუშვებელია.'; en = 'Error when writing user ""%1"". ""The Prepared attribute is not allowed to be changed in event subscriptions.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	
	Если АвтоРеквизиты.ИдентификаторПользователяИБ <> ПользовательОбъект.ИдентификаторПользователяИБ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Реквизит ИдентификаторПользователяИБ не допускается изменять.""Обновление реквизита выполняется автоматически.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. რეკვიზიტის საინფორმაციობაზისმომხმარებლისიდენტიფიკატორი შეცვლა დაუშვებელია. რეკვიზიტის განახლება ხორციელდება ავტომატურად.'; en = 'Error when writing user ""%1"". ""The IB User Identifier attribute is not allowed to be changed."" The attribute is updated automatically.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ,
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Реквизит СвойстваПользователяИБ не допускается изменять.""Обновление реквизита выполняется автоматически.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. რეკვიზიტი სბმომხმარებლისთვისებები-ს შეცვლა დაუშვებელია. რეკვიზიტის განახლება ავტომატურად ხორციელდება.'; en = 'Error when writing user ""%1"". ""The IB User Properties attribute is not allowed to be changed."" The attribute is updated automatically.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если СтарыйПользователь.ПометкаУдаления = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Нельзя помечать на удаление пользователя, которому разрешен вход в программу.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. პროგრამაში შესვლის ნებართვის მქონე მომხმარებლის წასაშლელად მონიშვნა არ შეიძლება.'; en = 'Error while writing user ""%1"". ""Cannot mark for deletion a user who is allowed to log into the program.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если СтарыйПользователь.Недействителен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Нельзя пометить недействительным пользователя, которому разрешен вход в программу.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. პროგრამაში შესვლის ნებართვის მქონე მომხმარებლის უმოქმედოდ მონიშვნა არ შეიძლება.'; en = 'Error writing user ''%1''."" A user who is allowed to log into the program cannot be marked invalid.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если СтарыйПользователь.Подготовлен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".""Нельзя пометить подготовленным пользователя, которому разрешен вход в программу.'; ka = 'შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. მომხმარებლის, რომლისთვის ნებადართულია პროგრამაში შესვლა, მომზადებულად მონიშვნა არ შეიძლება.'; en = 'There was an error writing user ""%1."" It is not possible to mark as prepared a user who is allowed to log into the program.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Если Выборка.СрокДействияИстек Тогда
				ШаблонКомментария =
					НСтр("ru = 'Не удалось снять пользователю ""%1"" признак""""Вход в программу разрешен"" в связи с окончанием срока действия по причине:""%2'; ka = 'მომხმარებელ ""%1""-ისთვის ნიშან-თვისება ""პროგრამაში შესვლა აკრძალულია""-ს მოხსნა ვერ მოხერხდა შემდეგი მიზეზით მოქმედების ვადის დასრულების გამო:""%2'; en = 'It was not possible to remove the user ""%1"" attribute ""Login to the program is allowed"" due to the expiration of the validity period for the reason: ""%2'");
			Иначе
				ШаблонКомментария =
					НСтр("ru = 'Не удалось снять пользователю ""%1"" признак""""Вход в программу разрешен"" в связи с отсутствием работы""в программе более установленного срока по причине:""%2'; ka = 'მომხმარებელ ""%1""-ისთვის ნიშან-თვისება ""პროგრამაში შესვლა ნებადართულია""-ს მოხსნა ვერ მოხერხდა პროგრამაში დადგენილ ვადაზე მეტად მუშაობის არარსებობის გამო შემდეგი მიზეზით:""%2'; en = 'It was not possible to clear the user ""%1"" attribute ""Entry into the program is allowed"" due to the lack of work"" in the program for more than the established period due to the reason: ""%2'");
			КонецЕсли;
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пользователи.Ошибка автоматического запрещения входа в программу'; ka = 'მომხმარებლები. პროგრამაში შესვლის ავტომატური აკრძალვის შეცდომა'; en = 'Users. Error automatically prohibiting entry into the program'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
