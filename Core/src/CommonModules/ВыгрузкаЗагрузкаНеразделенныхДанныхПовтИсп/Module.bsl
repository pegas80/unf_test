...
"поле объекта не обнаружено!';ka='ველი %1 არ შეიძლება, რომ ობიექტ %2-ის ბუნებრივი გასაღების ველად იქნას გამოყენებული:"
"ობიექტის ველი არ არის აღმოჩენილი!'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
"значений типа ХранилищеЗначения в качестве полей естественного ключа не поддерживается!';ka='ველი %1 არ შეიძლება, რომ ობიექტ %2-ის ბუნებრივი გასაღების ველად იქნას გამოყენებული:"
"მნიშვნელობისსაცავი-ს ტიპის მნიშვნელობების გამოყენება ბუნებრივი გასაღების ველებად არ არის მხარდაჭერილი!'", Метаданные.ОсновнойЯзык.КодЯзыка),
							ПолеЕстественногоКлюча,
							ТипОбщегоКлассификатора.ПолноеИмя());
...
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке()';ka='ველი %1 არ შეიძლება გამოყენებული იქნას ობიექტ %2-ის ბუნებრივი გასაღების ველად:"
"ველის ტიპად შეიძლება გამოყენებული იქნას ობიექტი %3, რომელიც არ არის შეყვანილი საერთო მონაცემთა კრებულში შემდეგი, ხელახლა განსაზღვრებადი პროცედურით მონაცემთაგადმოტვირთვაჩატვირთვახელახლაგანსაზღვრებადი.საერთომონაცემთატიპებისშევსებისასრომლებიცმხარსუჭერენბმულებისშეპირისპირებასჩატვირთვისას()'", Метаданные.ОсновнойЯзык.КодЯзыка),
							ПолеЕстественногоКлюча,
							ТипОбщегоКлассификатора.ПолноеИмя(),
...
Процедура ВызватьИсключениеПриНаличииВРазделенныхДанныхСсылокНаНеразделенныеБезПоддержкиСопоставленияСсылок(Знач ОбъектМетаданных, Знач ИмяПоля, Знач МетаданныеСсылки, Знач ПриВыгрузке)
	...
	Если ОбщегоНазначенияБТС.ЭтоКонстанта(ОбъектМетаданных) Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ka='განცალკევებული კონსტანტა %1-ის მნიშვნელობად გამოიყენება ბმულები განუცალკევებელ ობიექტ %2-ზე';ru='В качестве значения разделенной константы %1 используются ссылки на"
"неразделенный объект %2'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	ИначеЕсли ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ka='განცალკევებული ობიექტ %2-ის რეკვიზიტ %1-ის მნიშვნელობად გამოიყენება ბმულები განუცალკევებელ ობიექტზე %3';ru='В качестве значения реквизита %1 разделенного объекта %2 используются ссылки на"
"неразделенный объект %3'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	ИначеЕсли ОбщегоНазначенияБТС.ЭтоНаборЗаписей(ОбъектМетаданных) Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ka='ჩანაწერების განცალკევებული კრებულის %2 გაზომვის, რესურსის ან რეკვიზიტის %1 მნიშვნელობად გამოიყენება ბმულები განუცალკევებელ ობიექტზე %3';ru='В качестве значения измерения, ресурса или реквизита %1 разделенного набора записей %2 используются ссылки на"
"неразделенный объект %3'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ИмяПоля,
			ОбъектМетаданных.ПолноеИмя(),
			МетаданныеСсылки.ПолноеИмя());
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='Неожиданный объект метаданных: %1!';ru='Неожиданный объект метаданных: %1!';en='Unexpected object metadata:% 1!'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ОбъектМетаданных.ПолноеИмя());
...
"но при выгрузке была диагностирована попытка выгрузки ссылки на неразделенный объект).';ka='(в качестве типа значения для объекта установлен составной тип данных,"
"который может содержать ссылки как на разделенные данные, так и на неразделенные,"
...
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке().';ka='ამასთან, განუცალკევებელი ობიექტი %1 არ არის შეყვანილი საერთო მონაცემთა ტიპების შემადგენლობაში, რომლებისთვის შეიძლება ბმულების შეპირისპირების შესრულება გადმოტვირთვის და ჩატვირთვის დროს."
"მოცემული სიტუაცია დაუშვებელია, ვინაიდან, გადმოტვირთული მონაცემების ჩატვირთვისას სხვა სბ-ში ჩაიტვირთება ""გაცვეთილი"" ბმულები ობიექტ %1-ზე."
""
"სიტუაციის გამოსასწორებლად, საჭიროა ობიექტ %1-ისთვის ველების დასადგენი მექანიზმის რეალიზება, რომლებიც ერთმნიშვნელოვნად განსაზღვრავენ ობიექტის ბუნებრივ გასაღებს, და ობიექტ %1-ის შეყვანა საერთო მონაცემთა ტიპების შემადგენლობაში, რომლებისთვის შეიძლება ბმულების შეპირისპირების შესრულება გადმოტვირთვის და ჩატვირთვის დროს, მეტამონაცემთა ობიექტ %1-ის მითითებით შემდეგ პროცედურაში მონაცემებისგადმოტვირთვაჩატვირთვახელახლაგანსაზღვრებადი.საერთომონაცემებისტიპებისშევსებისასრომლებიცმხარსუჭერენბმულებისშეპირისპირებასჩატვირთვისას().'", Метаданные.ОсновнойЯзык.КодЯзыка),
		МетаданныеСсылки.ПолноеИмя());
