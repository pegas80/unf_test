...
Процедура ВыполнитьОбработчикВПривилегированномРежиме(Значение, Знач СтрокаОбработчика) Экспорт
	...
	Если ТекущийРежимЗапуска() = РежимЗапускаКлиентскогоПриложения.УправляемоеПриложение Тогда
		ВызватьИсключение НСтр("ka='მეთოდს არ გააჩნია მართვადი დანართის რეჟიმში მხარდაჭერა.';ru='Метод не поддерживается в режиме управляемого приложения.'");
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ka='სერვისის მოდელში მუშაობისას მეთოდს მხარდაჭერა არ გააჩნია.';ru='Метод не поддерживается при работе в модели сервиса.'");
	КонецЕсли;
