...
Функция ОбновитьСобытияМониторинга() Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЧтениеДанныхСПАРКРиски") Тогда
		ВызватьИсключение НСтр("ka='ოპერაციის შესასრულებლად უფლებები არასაკმარისია.';ru='Недостаточно прав для выполнения операции.'");
	КонецЕсли;
	
	СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(НСтр("ka='Обновление событий мониторинга 1СПАРК Риски.';ru='Обновление событий мониторинга 1СПАРК Риски.'"));
...
"Услуга с идентификатором %1 не подключена.'"),
	...
	Если СписокИНН = Неопределено Тогда
		...
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(НСтр("ka='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - полный.';ru='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - полный.'"));
	...
	Иначе
		...
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - для списка ИНН (%1).';ru='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - для списка ИНН (%1).'"),
...
"%1'"),
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось загрузить новые типы событий. %1';ru='Не удалось загрузить новые типы событий. %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбновитьТипыСобытийИзПоставляемыхДанных(Дескриптор, ПутьКФайлу) Экспорт
	...
	Если Не РезультатВызова.ВидОшибки.Пустая() Тогда
		ВидОшибки = РезультатВызова.ВидОшибки;
		СПАРКРиски.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='Не удалось обновить типы событий из поставляемых данных.';ru='Не удалось обновить типы событий из поставляемых данных.'") + Символы.ПС
	...
	Если ТекущиеТипыВИнформационнойБазе.Количество() = РезультатВызова.ТипыСобытий.Количество() Тогда
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ka='Обновление типов событий мониторинга 1СПАРК Риски не требуется.';ru='Обновление типов событий мониторинга 1СПАРК Риски не требуется.'"));
		Возврат;
	КонецЕсли;
...
"Услуга с идентификатором %1 не подключена.'"),
	...
	СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Отправка запросов постановки контрагентов на мониторинг.';ru='Отправка запросов постановки контрагентов на мониторинг.'"));
...
"Длительность: %3'"),
	...
	СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Обработка результатов постановки на мониторинг.';ru='Обработка результатов постановки на мониторинг.'"));
...
Процедура ЗаполнитьИПоставитьКонтрагентовНаМониторингЗадание(КонтекстВыполненияПоШагам) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Заполнить контрагентов на мониторинге';ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Заполнить контрагентов на мониторинге'"));
...
"Запланированное время начала: %3'"),
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Поставить контрагентов на мониторинг';ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Поставить контрагентов на мониторинг'"));
