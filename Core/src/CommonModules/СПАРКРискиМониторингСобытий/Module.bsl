...
Функция ОбновитьСобытияМониторинга() Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЧтениеДанныхСПАРКРиски") Тогда
		ВызватьИсключение НСтр("ka='ოპერაციის შესასრულებლად უფლებები არასაკმარისია.';ru='Недостаточно прав для выполнения операции.';en='Insufficient permissions to perform the operation.'");
	КонецЕсли;
	
	СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(НСтр("ka='Обновление событий мониторинга 1СПАРК Риски.';ru='Обновление событий мониторинга 1СПАРК Риски.';en='Update monitoring events 1SPARK risks.'"));
...
"Интернет-поддержка пользователей не подключена.';ka='Не удалось загрузить события 1СПАРК Риски."
...
"Услуга с идентификатором %1 не подключена.';ka='Не удалось загрузить события мониторинга 1СПАРК Риски."
"Услуга с идентификатором %1 не подключена.'"),
	...
	Если СписокИНН = Неопределено Тогда
		...
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(НСтр("ka='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - полный.';ru='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - полный.';en='Load monitoring events 1SPARK risks. Download option - full.'"));
	...
	Иначе
		...
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - для списка ИНН (%1).';ru='Загрузка событий мониторинга 1СПАРК Риски. Вариант загрузки - для списка ИНН (%1).';en='Load monitoring events 1SPARK risks. Download option - for a list of VAT (1%).'"),
...
"Записано событий в информационную базу: %3.';ka='Получены события мониторинга 1СПАРК Риски (%1)."
"Идентификатор последнего события: %2."
...
"%2';ka='Не удалось записать описание типа события с идентификатором %1."
"%2'"),
...
"%1';ka='Получены неизвестные идентификаторы типов событий:"
"%1'"),
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось загрузить новые типы событий. %1';ru='Не удалось загрузить новые типы событий. %1';en='Failed to load new types of events. %one'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбновитьТипыСобытийИзПоставляемыхДанных(Дескриптор, ПутьКФайлу) Экспорт
	...
	Если Не РезультатВызова.ВидОшибки.Пустая() Тогда
		ВидОшибки = РезультатВызова.ВидОшибки;
		СПАРКРиски.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ka='Не удалось обновить типы событий из поставляемых данных.';ru='Не удалось обновить типы событий из поставляемых данных.';en='Failed to update event types from the supplied data.'") + Символы.ПС
	...
	Если ТекущиеТипыВИнформационнойБазе.Количество() = РезультатВызова.ТипыСобытий.Количество() Тогда
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ka='Обновление типов событий мониторинга 1СПАРК Риски не требуется.';ru='Обновление типов событий мониторинга 1СПАРК Риски не требуется.';en='Update types of monitoring events 1SPARK risks is required.'"));
		Возврат;
	КонецЕсли;
...
"Интернет-поддержка пользователей не подключена.';ka='Не удалось поставить контрагентов на мониторинг в сервисе 1СПАРК Риски."
"Интернет-поддержка пользователей не подключена.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не ИнтернетПоддержкаПользователей.УслугаПодключена(ИдентификаторУслуги) Тогда
		...
		СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось поставить контрагентов на мониторинг в сервисе 1СПАРК Риски."
"Услуга с идентификатором %1 не подключена.';ka='Не удалось поставить контрагентов на мониторинг в сервисе 1СПАРК Риски."
"Услуга с идентификатором %1 не подключена.'"),
				ИдентификаторУслуги));
		Возврат;
	КонецЕсли;
	...
	СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Отправка запросов постановки контрагентов на мониторинг.';ru='Отправка запросов постановки контрагентов на мониторинг.';en='Sending a staging request contractors to monitor.'"));
...
"Количество ИНН: %2;"
"Длительность: %3';ka='Завершена отправка запросов на постановку контрагентов на мониторинг."
"Количество запросов: %1;"
"Количество ИНН: %2;"
"Длительность: %3'"),
			ОтправленоЗапросов,
	...
	СПАРКРиски.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Обработка результатов постановки на мониторинг.';ru='Обработка результатов постановки на мониторинг.';en='Processing of the results of monitoring performances.'"));
...
"Длительность: %3';ka='Завершена обработка результатов постановки на мониторинг."
"Проверено идентификаторов запросов: %1;"
"Отправлено запросов: %2;"
"Длительность: %3'"),
				КоличествоИдентификаторовЗапросов,
...
"%1';ka='Ошибка при записи результатов постановки на мониторинг."
"%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		КонецПопытки;
...
"%1';ka='Ошибка при записи идентификаторов запросов постановки на мониторинг."
"%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		КонецПопытки;
...
Процедура ЗаполнитьИПоставитьКонтрагентовНаМониторингЗадание(КонтекстВыполненияПоШагам) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Заполнить контрагентов на мониторинге';ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Заполнить контрагентов на мониторинге';en='Performing VseObnovleniyaSPARKRiskiRazdelennoe, Fill in the monitoring of counterparties'"));
...
"Начало задания: %2"
"Запланированное время начала: %3';ka='Время выполнения шага %1 еще не наступило."
"Начало задания: %2"
"Запланированное время начала: %3'"),
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ka='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Поставить контрагентов на мониторинг';ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное, Поставить контрагентов на мониторинг';en='Performing VseObnovleniyaSPARKRiskiRazdelennoe, Put counterparties monitoring'"));
