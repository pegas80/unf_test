...
Функция СформироватьДекларациюПоЕНВД(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ka='მშეგ-ის მიხედვით დეკლარაციის შედგენა, კალენდრის შესაბამისი მოვლენის გარეშე შეუძლებელია';ru='Формирование декларации по ЕНВД невозможно без соответствующего события календаря';en='Formation of the Declaration on UTII impossible without a corresponding calendar events'");
	КонецЕсли;
