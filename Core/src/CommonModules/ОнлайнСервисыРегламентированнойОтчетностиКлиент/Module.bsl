...
Процедура ПродолжитьПослеПроверкиПодключенияКИППЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли Результат.Выполнено И НЕ Результат.ВыполнениеРазрешено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нет доступа к интернет-поддержке пользователя - внешние формы не будут загружаться автоматически.';ru='Нет доступа к интернет-поддержке пользователя - внешние формы не будут загружаться автоматически.'"));
	ИначеЕсли НЕ Результат.Выполнено Тогда
		ПоказатьПредупреждение(, НСтр("ka='მომხმარებლის ინტერნეტ-მხარდაჭერის წვდომის შემოწმებისას უცნობი შეცდომა წარმოიშვა.';ru='При проверке доступа к интернет-поддержке пользователя возникла неизвестная ошибка.'"));
	КонецЕсли;
...
Процедура ПопытатьсяПерейтиПоНавигационнойСсылке(Ссылка) Экспорт
	...
	Исключение
		...
		ПоказатьПредупреждение(, НСтр("ka='Не удалось перейти по указанной ссылке!';ru='Не удалось перейти по указанной ссылке!'"));
