...
Функция РассчитатьЕдиныйНалог(Организация, ДатаДокументаОбработкиСобытия, СобытиеКалендаря) Экспорт
	...
	Состояние(
		НСтр("ka='გათვლილი';ru='Рассчитано';en='calculated'"),
		,
		НСтр("ka='ერთიანი გადასახადი გამოანგარიშებულია';ru='Единый налог рассчитан';en='A single tax is calculated'"),
...
Функция РассчитатьАвансовыйПлатежПоУСН(Организация, ДатаДокументаОбработкиСобытия,СобытиеКалендаря) Экспорт
	...
	Состояние(
		НСтр("ka='გათვლილი';ru='Рассчитано';en='calculated'"),
		,
		НСтр("ka='საავანსო გადახდა გამოთვლილია';ru='Авансовый платеж рассчитан';en='Advance payment is calculated'"),
		БиблиотекаКартинок.Рассчет32);
...
Процедура ОткрытьФормуРегламентированногоОтчетаПоСобытию(Организация = Неопределено,СобытиеКалендаря) Экспорт
	...
	Если ДокументРегламентированнойОтчетности = Неопределено  Тогда
		ПоказатьПредупреждение(,НСтр("ka='შენახული ანგარიშგება არ არის. შეადგინეთ ანგარიშგება.';ru='Нет сохраненного отчета. Сформируйте отчет.';en='No saved report. Generate the report.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьБанковскийСчетДляУплатыНалога(ОповещениеВыбораСчета, Организация, Форма = Неопределено, ЭлементФормы = Неопределено) Экспорт
	...
	Если СпсБанкСчетов.Количество() = 0 Тогда
		...
		СпсОтвет.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ანგარიშების ჩვენება';ru='Показать счета';en='Show accounts'"));
		СпсОтвет.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
		
		оп = Новый ОписаниеОповещения("ОповещениеВопросаПоказаСпискаСчетов", ЭтотОбъект, Новый Структура("Организация", Организация));
		ПоказатьВопрос(оп, НСтр("ka='შესაბამისი საბანკო ანგარიშები ვერ მოიძებნა';ru='Не найдены подходящие банковские счета';en='suitable bank accounts not found'"), СпсОтвет);
	...
	Иначе
		...
		СпсБанкСчетов.ПоказатьВыборЭлемента(оп, НСтр("ka='რომელი ჩეკიდან გავატაროთ გადახდა';ru='С какого счета провести оплату';en='With which account to make payment'"));
		#Иначе
