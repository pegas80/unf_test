...
Функция РассчитатьЕдиныйНалог(Организация, ДатаДокументаОбработкиСобытия, СобытиеКалендаря) Экспорт
	...
	Состояние(
		НСтр("ka='გათვლილი';ru='Рассчитано'"),
		,
		НСтр("ka='ერთიანი გადასახადი გამოანგარიშებულია';ru='Единый налог рассчитан'"),
...
Функция РассчитатьАвансовыйПлатежПоУСН(Организация, ДатаДокументаОбработкиСобытия,СобытиеКалендаря) Экспорт
	...
	Состояние(
		НСтр("ka='გათვლილი';ru='Рассчитано'"),
		,
		НСтр("ka='საავანსო გადახდა გამოთვლილია';ru='Авансовый платеж рассчитан'"),
		БиблиотекаКартинок.Рассчет32);
...
Процедура ОткрытьФормуРегламентированногоОтчетаПоСобытию(Организация = Неопределено,СобытиеКалендаря) Экспорт
	...
	Если ДокументРегламентированнойОтчетности = Неопределено  Тогда
		ПоказатьПредупреждение(,НСтр("ka='შენახული ანგარიშგება არ არის. შეადგინეთ ანგარიშგება.';ru='Нет сохраненного отчета. Сформируйте отчет.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьБанковскийСчетДляУплатыНалога(ОповещениеВыбораСчета, Организация, Форма = Неопределено, ЭлементФормы = Неопределено) Экспорт
	...
	Если СпсБанкСчетов.Количество() = 0 Тогда
		...
		СпсОтвет.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ანგარიშების ჩვენება';ru='Показать счета'"));
		...
		ПоказатьВопрос(оп, НСтр("ka='შესაბამისი საბანკო ანგარიშები ვერ მოიძებნა';ru='Не найдены подходящие банковские счета'"), СпсОтвет);
	...
	Иначе
		...
		СпсБанкСчетов.ПоказатьВыборЭлемента(оп, НСтр("ka='რომელი ჩეკიდან გავატაროთ გადახდა';ru='С какого счета провести оплату'"));
		#Иначе
