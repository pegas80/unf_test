...
Функция ПрочитанныеДанныеОтвета(Ответ)
	...
	Если ЗначениеЗаполнено(ТелоОтвета) Тогда
		...
		Исключение
			...
			Событие = ИмяСобытия() + "." + НСтр("ka='Ошибка чтения JSON';ru='Ошибка чтения JSON'", Локаль);
...
Функция ИмяСобытия()
	...
	Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
	Возврат НСтр("ka='Программный интерфейс сервиса';ru='Программный интерфейс сервиса'", Локаль);
