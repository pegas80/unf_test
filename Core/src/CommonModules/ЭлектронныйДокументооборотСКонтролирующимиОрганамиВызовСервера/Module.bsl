...
Процедура ИзменитьЗначениеВФорме1СОтчетность(Ссылка, ИмяПоля, Знач НовоеЗначение) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Не удалось изменить свойство в таблице форме 1С-Отчетности';ru='Не удалось изменить свойство в таблице форме 1С-Отчетности'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура УдалитьВыборCSPИзВременныхНастроек() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Удаление сохраненных настроек мастера подключения к 1С-Отчетности';ru='Удаление сохраненных настроек мастера подключения к 1С-Отчетности'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция НаименованиеТОГС(КодОрганаФСГС) Экспорт
	...
	Возврат НСтр("ka='<Не выбран код территориального органа>';ru='<Не выбран код территориального органа>'");
...
Функция ПолучитьДанныеСотрудника(ВидВладельцаЭЦП, ДанныеОрганизации, Сотрудник) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='Электронный документооборот с контролирующими органами. Не удалось получить сведения о сотруднике';ru='Электронный документооборот с контролирующими органами. Не удалось получить сведения о сотруднике'");
