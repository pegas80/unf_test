...
Функция НаименованиеПодсистемы(КодЯзыка) Экспорт
	Возврат НСтр("ka='ანგარიშგებათა ვარიანტები';ru='Варианты отчетов';en='reporting options'", ?(КодЯзыка = Неопределено, ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(), КодЯзыка));
КонецФункции

// Представление группы важности.
Функция ПредставлениеСмТакже() Экспорт
	Возврат НСтр("ka='იხ.აგრეთვე';ru='См. также';en='see also'");
КонецФункции 

// Представление группы важности.
Функция ПредставлениеВажный() Экспорт
	Возврат НСтр("ka='მნიშვნელოვანი';ru='Важный';en='Important'");
КонецФункции
...
Функция РазложитьСтрокуПоискаВМассивСлов(СтрокаПоиска) Экспорт
	...
	Для НомерСимвола = 1 По ДлинаСтроки Цикл
		...
		ИначеЕсли ОткрытаКавычка
			...
			//Паганини_
		    Или (КодСимвола >= 4304 И КодСимвола <= 4336) // Грузинский
			//_Паганини
...
Функция ТекстОшибки(Номер, Реквизит1 = Неопределено, Реквизит2 = Неопределено, Реквизит3 = Неопределено) Экспорт
	Если Номер = 1 Или Номер = 2 Тогда
		Если Номер = 1 Тогда
			ТекстОшибки = НСтр("ka='ანგარიშგება ""%1"" შეერთებულია ძირითად ფორმაზე, მაგრამ არ არის შეერთებული გამართვების ფორმაზე.';ru='Отчет ""%1"" подключен к основной форме, но не подключен к форме настроек.';en='Report ""%1"" is connected to the base form, but not connected to the form settings.'");
		Иначе
			ТекстОшибки = НСтр("ka='ანგარიშგება ""%1"" ძირითად ფორმაზე არ არის შეერთებული, მაგრამ შეერთებულია გამართვების ფორმაზე.';ru='Отчет ""%1"" не подключен к основной форме, но подключен к форме настроек.';en='Report ""%1"" is not connected to the main form, but is connected to the form settings.'");
		КонецЕсли;
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ka='დაწვრილებით, იხ. ქვესისტემის დანერგვის დოკუმენტაციაში.';ru='Подробнее см. в документации по внедрению подсистемы.';en='For details, see. In the documentation for the implementation of the subsystem.'");
