...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სახელ %1-ით შსო ვერ მოიძებნა';ru='Не найдено ПКО с именем %1';en='PKO found with the name% 1'"), Имя);
...
Процедура ВыгрузкаОбъектаВыборки(КомпонентыОбмена, Объект, ПравилоОбработки = Неопределено) Экспорт
	...
	Если ТипЗнч(Объект) <> Тип("Структура") Тогда
		...
		Исключение
			ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Событие: %1."
"Объект: %2."
""
"%3.';ru='Событие: %1."
"Объект: %2."
...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თვისების უცნობი ტიპი <%1>. ობიექტის ტიპი: %2';ru='Неизвестный тип свойства <%1>. Тип объекта: %2';en='Unknown property type <% 1>. Object type: 2%'"), Свойство.Имя, Строка(ТипXDTO));
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.';ru='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.';en='Error forming XDTO object: Property type <% 1>. Building Name: <% 2>.'"), ТипСвойства, Свойство.Имя)
...
"Тип источника: %1"
"Тип приемника: %2';ka='XML-სქემის შეცდომა. მიმღებისთვის მიცემული უნდა იყოს ერთი თვისება."
"წყაროს ტიპი: %1"
"მიმღების ტიპი: %2'"),
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ka='მონაცემთა გაცვლის ბლოკირება არ შეიძლება, რომ აქტიურ ტრანზაქციაში იქნას დაყენებული.';ru='Блокировка на обмен данными не может быть установлена в активной транзакции.';en='Lock on the data exchange can not be set in an active transaction.'");
		КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='შეცდომა მონაცემების ჩატვირთვისას: %1';ru='Ошибка при загрузке данных: %1';en='Error loading% 1'");
...
"обновление настройки синхронизации данных. Необходимо:';ka='მონაცემების მიღება წყაროსგან, რომელშიც მონაცემთა სინქრონიზაციის გამართვის განახლება არ არის შესრულებული. აუცილებელია:'")
					+ Символы.ПС + НСтр("ka='1) მონაცემთა განმეორებითი სინქრონიზაციის შესრულება რაღაც დროის შემდეგ.';ru='1) Выполнить повторную синхронизацию данных через некоторое время.';en='1) Re-run data synchronization after some time.'")
					+ Символы.ПС + НСтр("ka='2) მონაცემთა სინქრონიზაციის შესრულება წყაროს მხარეს, ამის შემდეგ, ამ საინფორმაციო ბაზაში განმეორებით შევასრულოთ მონაცემთა სინქრონიზაცია.';ru='2) Выполнить синхронизацию данных на стороне источника, после этого "
"повторно выполнить синхронизацию данных в этой информационной базе.'")
					+ Символы.ПС + НСтр("ka='(1 - ტრანსპორტის სახეობა ""ინტერნეტით""-სთვის, 2 - ტრანსპორტის სახეობა ""სხვა""-სთვის)';ru='(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)';en='(1 - for transport mode via the Internet, 2 - mode of transport to another)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если МенеджерПланаОбмена.ФорматОбмена() <> ФорматОбмена.БазовыйФормат Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გაცვლის შეტყობინების ფორმატი <%1> გაცვლის გეგმის ფორმატ <%2>-ს არ შეესაბამება.';ru='Формат сообщения обмена <%1> не соответствует формату плана обмена <%2>.';en='The format of message exchange <% 1> does not fit into the plan of exchange <% 2>.'"),
			...
			Если ВерсииФорматаОбменаМассив(КомпонентыОбмена.УзелКорреспондента).Найти(ФорматОбмена.Версия) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გაცვლის შეტყობინების ფორმატის ვერსიას <%1> მხარდაჭერა არ გააჩნია.';ru='Версия формата сообщения обмена <%1> не поддерживается.';en='format version of message exchange <% 1> is not supported.'"),
					ФорматОбмена.Версия);
			КонецЕсли;
			...
			Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
				...
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
				                            НСтр("ka='გაცვლის ფორმატის ვერსიის ნომერი შეიცვალა.';ru='Изменен номер версии формата обмена.';en='Modified versions of the exchange format number.'"),,Ложь,,, Истина);
			КонецЕсли;
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='დაცალკავებულ რეჟიმში არა დაცალკავებული % მონაცემების შეცვლის მცდელობა';ru='Попытка изменение неразделенных данных (%1) в разделенном режиме.';en='Attempting to change undivided data (1%) in the split mode.'");
...
Функция МенеджерОбменаВерсииФормата(Знач УзелИнформационнойБазы, Знач ВерсияФормата) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონვერტაციის მენეჯერი არ არის გარკვეული გაცვლის ფორმატის ვერსიისთვის <%1>.';ru='Не определен Менеджер конвертации для версии формата обмена <%1>.';en='The manager is not defined conversion to version interchange format <% 1>.'"),
			ВерсияФормата);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრ ვერსიისსტრიქონი1-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1';en='Wrong format StrokaVersii1 parameter:% 1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრ ვერსიისსტრიქონი2-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1';en='Wrong format StrokaVersii2 parameter:% 1'"), СтрокаВерсии2);
	КонецЕсли;
...
"Задано более одного правила конвертации для типа источника <%1>.';ka='წინასწარ განსაზღვრული მონაცემების კონვერტაციის წესების შეცდომა."
"წყაროს ტიპ <%1>-ისთვის მიცემულია კონვერტაციის ერთ წესზე მეტი.'"),
				Строка(Тип));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1>.';ka='წინასწარ განსაზღვრული მონაცემების კონვერტაციის შეცდომა. წყაროს ტიპ <%1>-ისთვის კონვერტაციის წესი არ არის განსაზღვრული.'"),
			Строка(Тип));
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка правил конвертации предопределенных данных."
"Задано более одного правила конвертации для типа источника <%1> и типа приемника <%2>.';ka='წინასწარ განსაზღვრული მონაცემების კონვერტაციის წესების შეცდომა."
"წყაროს ტიპ <%1>-ისთვის და მიმღების ტიპ <%2>-ისთვის მიცემულია კონვერტაციის ერთ წესზე მეტი.'"),
				Строка(Тип),
				Строка(ТипXDTO));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1> и типа приемника <%2>.';ka='წინასწარგანსაზღვრულ მონაცემთა კონვერტაციის წესების შეცდომა."
"კონვერტაციის წესი არ არის დადგენილი წყაროს ტიპ <%1>-ისთვის და მიმღების ტიპ <%2>-ისთვის.'"),
			Строка(Тип),
			Строка(ТипXDTO));
...
Функция ЗначениеПеречисленияXDTO(Знач КонвертацииЗначений, Знач Значение)
	...
	Если ЗначениеXDTO = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='წინასწარ განსაზღვრული მნიშვნელობისთვის კონვერტაციის წესი ვერ მოიძებნა."
"წყაროს მნიშვნელობის ტიპი: <%1>"
"წყაროს მნიშვნელობა: <%2>';ru='Не найдено правило конвертации для значения предопределенных данных."
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка чтения объекта XDTO, имя свойства: <%1>.';en='read XDTO object error, the name of the property: <% 1>.'"), ИмяСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка конвертации свойства объекта XDTO, имя свойства: <%2>.';en='Error converting properties XDTO object name of the property: <% 2>.'"), ЗначениеСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, КомпонентыОбмена, ТипЗначенияОбъектаИБ)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Если  КоличествоСсылокПодходящегоТипа > 1 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='უნიკალური იდენტიფიკატორ <%1>-ისთვის და კვანძ <%1>-ისთვის დანიშნულია რამდენიმე ბმული.';ru='Для уникального идентификатора <%1> и узла <%2> назначено несколько ссылок.';en='For a unique identifier <% 1> and node <% 2> is assigned a few links.'"),
...
Функция ПОДПоТипуСсылкиXDTO(КомпонентыОбмена, ТипСсылкиXDTO, ВозвращатьПустоеЗначение = Ложь)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ბმულის ტიპ XDTO-სთვის ПОД არ მოიძებნა."
"ბმულის ტიპი XDTO: %1"
"შეცდომის აღწერილობა: %2';ru='Не найдено ПОД для типа ссылки XDTO."
"Тип ссылки XDTO: %1"
"Описание ошибки: %2'"),
...
Функция ПОДПоТипуОбъектаXDTO(КомпонентыОбмена, ТипОбъектаXDTO, ВозвращатьПустоеЗначение = Ложь)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ობიექტის ტიპ XDTO-სთვის ПОД არ მოიძებნა ."
"ობიექტის ტიპი XDTO: %1"
"შეცდომის აღწერილობა: %2';ru='Не найдено ПОД для типа объекта XDTO."
"Тип объекта XDTO: %1"
"Описание ошибки: %2'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სახელ %1-ით ПОД ვერ მოიძებნა';ru='Не найдено ПОД с именем %1';en='Not found under the name% 1'"), Имя);
		...
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %3-ის №%2';ru='%1 №%2 от %3';en='№% 1% 2% 3 from'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1, %2';ru='%1, %2';en='%12'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
...
Функция ВерсииФорматаОбмена(Знач УзелИнформационнойБазы)
	...
	Если ВерсииФорматаОбмена.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='გაცვლის ფორმატის ვერსიები არ არის მიცემული."
"გაცვლის გეგმის სახელი: %1"
"პროცედურა: გაცვლისფორმატისვერსიებისმიღება(<გაცვლისფორმატისვერსიები>)';ru='Не заданы версии формата обмена."
"Имя плана обмена: %1"
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გაცვლის ფორმატის ვერსიის არაკანონიკური წარდგენა: <%1>.';ru='Неканоническое представление версии формата обмена: <%1>.';en='Noncanonical representation version interchange format: <% 1>.'"), Версия);
	КонецЕсли;
