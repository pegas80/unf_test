...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სახელ %1-ით შსო ვერ მოიძებნა';ru='Не найдено ПКО с именем %1'"), Имя);
...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თვისების უცნობი ტიპი <%1>. ობიექტის ტიპი: %2';ru='Неизвестный тип свойства <%1>. Тип объекта: %2'"), Свойство.Имя, Строка(ТипXDTO));
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.';ru='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.'"), ТипСвойства, Свойство.Имя)
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ka='მონაცემთა გაცვლის ბლოკირება არ შეიძლება, რომ აქტიურ ტრანზაქციაში იქნას დაყენებული.';ru='Блокировка на обмен данными не может быть установлена в активной транзакции.'");
		КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='შეცდომა მონაცემების ჩატვირთვისას: %1';ru='Ошибка при загрузке данных: %1'");
...
"обновление настройки синхронизации данных. Необходимо:';ka='მონაცემების მიღება წყაროსგან, რომელშიც მონაცემთა სინქრონიზაციის გამართვის განახლება არ არის შესრულებული. აუცილებელია:'")
					+ Символы.ПС + НСтр("ka='1) მონაცემთა განმეორებითი სინქრონიზაციის შესრულება რაღაც დროის შემდეგ.';ru='1) Выполнить повторную синхронизацию данных через некоторое время.'")
...
"повторно выполнить синхронизацию данных в этой информационной базе.'")
					+ Символы.ПС + НСтр("ka='(1 - ტრანსპორტის სახეობა ""ინტერნეტით""-სთვის, 2 - ტრანსპორტის სახეობა ""სხვა""-სთვის)';ru='(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если МенеджерПланаОбмена.ФорматОбмена() <> ФорматОбмена.БазовыйФормат Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გაცვლის შეტყობინების ფორმატი <%1> გაცვლის გეგმის ფორმატ <%2>-ს არ შეესაბამება.';ru='Формат сообщения обмена <%1> не соответствует формату плана обмена <%2>.'"),
			...
			Если ВерсииФорматаОбменаМассив(КомпонентыОбмена.УзелКорреспондента).Найти(ФорматОбмена.Версия) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გაცვლის შეტყობინების ფორმატის ვერსიას <%1> მხარდაჭერა არ გააჩნია.';ru='Версия формата сообщения обмена <%1> не поддерживается.'"),
					ФорматОбмена.Версия);
			КонецЕсли;
			...
			Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
				...
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
				                            НСтр("ka='გაცვლის ფორმატის ვერსიის ნომერი შეიცვალა.';ru='Изменен номер версии формата обмена.'"),,Ложь,,, Истина);
			КонецЕсли;
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='დაცალკავებულ რეჟიმში არა დაცალკავებული % მონაცემების შეცვლის მცდელობა';ru='Попытка изменение неразделенных данных (%1) в разделенном режиме.'");
...
Функция МенеджерОбменаВерсииФормата(Знач УзелИнформационнойБазы, Знач ВерсияФормата) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონვერტაციის მენეჯერი არ არის გარკვეული გაცვლის ფორმატის ვერსიისთვის <%1>.';ru='Не определен Менеджер конвертации для версии формата обмена <%1>.'"),
			ВерсияФормата);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრ ვერსიისსტრიქონი1-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრ ვერსიისსტრიქონი2-ის არასწორი ფორმატი: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გაცვლის ფორმატის არაკანონიკური სახელი <%1>';ru='Неканоническое имя формата обмена <%1>'"), ФорматОбмена);
	КонецЕсли;
...
Функция НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, КомпонентыОбмена, ТипЗначенияОбъектаИБ)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Если  КоличествоСсылокПодходящегоТипа > 1 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='უნიკალური იდენტიფიკატორ <%1>-ისთვის და კვანძ <%1>-ისთვის დანიშნულია რამდენიმე ბმული.';ru='Для уникального идентификатора <%1> и узла <%2> назначено несколько ссылок.'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სახელ %1-ით ПОД ვერ მოიძებნა';ru='Не найдено ПОД с именем %1'"), Имя);
		...
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %3-ის №%2';ru='%1 №%2 от %3'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1, %2';ru='%1, %2'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გაცვლის ფორმატის ვერსიის არაკანონიკური წარდგენა: <%1>.';ru='Неканоническое представление версии формата обмена: <%1>.'"), Версия);
	КонецЕсли;
