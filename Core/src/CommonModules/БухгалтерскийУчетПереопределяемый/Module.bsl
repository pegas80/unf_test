...
Функция ВедетсяКадровыйУчет() Экспорт
	...
	Возврат Ложь; //УчетЗарплаты.ВедетсяКадровыйУчет();
...
Функция ВедетсяПроизводственнаяДеятельность(Организация, Период) Экспорт
 	...
 	Возврат Ложь; //ПолучитьФункциональнуюОпцию("ВедетсяПроизводственнаяДеятельность") И УчетнаяПолитика.ВедетсяПроизводственнаяДеятельность(Организация, Период);
...
Функция ИспользоватьВалютныйУчет() Экспорт
	...
	Возврат Истина; //ПолучитьФункциональнуюОпцию("ИспользоватьВалютныйУчет");
...
Функция ПолучитьСведенияОСпискеНоменклатуры(Знач СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	//			РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыОбъекта.ДоговорКонтрагента, "ПредъявляетНДС,УчетАгентскогоНДС,Владелец,ВидАгентскогоДоговора");
	//			Если НЕ РеквизитыДоговора.ПредъявляетНДС
	//				И НЕ РеквизитыДоговора.УчетАгентскогоНДС 
	//				ИЛИ РеквизитыДоговора.ВидАгентскогоДоговора = Перечисления.ВидыАгентскихДоговоров.РеализацияТоваров Тогда
	//				
	//				Если РеквизитыДоговора.ВидАгентскогоДоговора = Перечисления.ВидыАгентскихДоговоров.РеализацияТоваров Тогда 
	//					СведенияОНоменклатуре.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;
	//				ИначеЕсли УчетНДС.КонтрагентРезидентТаможенногоСоюза(РеквизитыДоговора.Владелец) Тогда
...
Функция ПолучитьБанковскийСчетПоУмолчанию(ЮрФизЛицо, Валюта = Неопределено) Экспорт
	...
	Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации")
		...
		БанковскийСчет = Справочники.БанковскиеСчета.ПустаяСсылка();// ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЮрФизЛицо, "ОсновнойБанковскийСчет");
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникОбъект.Организации")
		...
		БанковскийСчет = Справочники.БанковскиеСчета.ПустаяСсылка();//ЮрФизЛицо.ОсновнойБанковскийСчет;
...
Функция СведенияОЮрФизЛице(Компания, Период = '00010101', Знач БанковскийСчет = Неопределено, ПлатежВБюджет = Ложь) Экспорт
	...
	Если Реквизиты.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо
		...
		Реквизиты.Свидетельство = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='свидетельство %1 от %2';en='certificate% 1% 2 from'"),
...
Функция ПолучитьЗначениеПоУмолчанию(Настройка, ИмяПользователяИБ = Неопределено) Экспорт
	...
	ИначеЕсли НастройкаВРег = ВРег("ОсновнойСклад") Тогда
		Возврат Справочники.СтруктурныеЕдиницы.ПустаяСсылка();// Склады.ПолучитьСкладПоУмолчанию(ИмяПользователяИБ);
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыПартийТоваровКомитентов(ТаблицаТоварыСПартиями, Реквизиты, Отказ) Экспорт
	...
	Для каждого СтрокаПартии Из ТаблицаТоварыСПартиями Цикл
		...
		Если СтрокаПараметровПартии <> Неопределено Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаПартии.СчетРасчетовСКомитентом) Тогда
				Если НЕ ЗначениеЗаполнено(СчетаРасчетов.СчетРасчетовСКомитентом) Тогда
					ТекстОшибки = НСтр("ru='При списании комиссионных товаров не определен счет расчетов с комитентом по партии ""%Партия%""';en='On derecognition of a commission of goods is not determined by a calculation by the principal party ""Party%%""'");
		...
		ИначеЕсли НЕ Отказ Тогда // не выводим ошибку если ранее уже была ошибка (например в подборе партий комиссионных товаров)
			...
			ТекстОшибки = НСтр("ru='При списании комиссионных товаров не определены параметры партии ""%Партия%""';en='On derecognition of commission goods parameters Party ""%% Party"" is not defined'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Партия%", СтрокаПартии.Партия);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Реквизиты.Регистратор, , , Отказ);
...
Функция НоваяПодпись()
	...
	Подпись = НСтр("ru='С уважением%1';en='With respect% 1'");
