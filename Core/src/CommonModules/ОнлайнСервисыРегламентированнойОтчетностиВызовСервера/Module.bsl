...
Функция ЗапуститьФоновоеЗаданиеАктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(УникальныйИдентификаторФормы, МожноЗагружатьВнешниеФормыОтчетов = Ложь) Экспорт
	...
	Возврат ДлительныеОперации.ЗапуститьВыполнениеВФоне(
				...
				НСтр("ka='Производится обновление служебной информации из Интернет...';ru='Производится обновление служебной информации из Интернет...';en='Is updated service information from the Internet ...'"));
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ka='Не удалось распаковать архив с внешней формой отчета.';ru='Не удалось распаковать архив с внешней формой отчета.';en='Unable to unzip the file to the external form of the report.'");
		ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка внешних регламентированных отчетов';ru='Загрузка внешних регламентированных отчетов';en='Loading external regulated reports'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Если СвойстваМанифеста["ИДКонфигурации"] <> СвойстваКонфигурации["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации';ru='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации';en='Do not pass inspection for compliance with configuration identifier in the identifier of the current report configuration'");
	...
	Если СвойстваМанифеста["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.';ru='Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.';en='Do not pass inspection for compliance with the version in the version of the report of the current configuration.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
"Описание:"
"""%1""';ka='Получение объекта отчета по ссылке."
"Описание:"
"""%1""'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Внешний отчет ""%1"" вер. ""%2"" отключен.';ru='Внешний отчет ""%1"" вер. ""%2"" отключен.';en='External Report ""%1"" ver. ""%2"" is disabled.'"), СвойстваМанифеста["Имя"], СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ka='Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов';ru='Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов';en='Validation fails to compare versions of the manifesto and the selected stored records'");
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете';ru='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете';en='Do not pass inspection for compliance with the identifier configuration ID in the current configuration of a connected record'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов';ru='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов';en='Do not pass inspection for compliance with the current version and the connected reports'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВРег(СвойстваОтчета["ИсточникОтчета"]) <> ВРег(СвойстваКонфигурации["ИсточникОтчета"]) Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов';ru='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов';en='Do not pass inspection for compliance with the names of the current metadata and connected reports'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ka='Не пройдена проверка на сравнение версий выбранного и хранимого отчетов';ru='Не пройдена проверка на сравнение версий выбранного и хранимого отчетов';en='Not passed check and compare versions of the selected stored records'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ТекстИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Внешний отчет вер. ""%1"" успешно загружен.';ru='Внешний отчет вер. ""%1"" успешно загружен.';en='External report ver. ""%1"" has been successfully uploaded.'"), СвойстваОтчета["КраткаяВерсияВнешнегоОтчета"]);
		
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Информация, ТекстИнформации);
		
	Исключение
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Запись объекта отчета с новыми данными."
"Описание:"
"""%1""';ru='Запись объекта отчета с новыми данными."
"Описание:"
"""%1""'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
...
Функция ПрочитатьМанифест(КаталогРаспаковки)
	...
	Если Чтение.ТипУзла <> ТипУзлаXML.НачалоЭлемента ИЛИ Чтение.Имя <> "Info" Тогда
		ВызватьИсключение(НСтр("ka='XML-ის წაკითხვის შეცდომა. ფაილის არასწორი ფორმატი. მოსალოდნელია ელემენტ Info-ს დაწყება.';ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Info.';en='XML reading error. Invalid file format. Start of Info element.'"));
	КонецЕсли;
	
	Если НЕ Чтение.Прочитать() Тогда
		ВызватьИсключение(НСтр("ka='XML-ის წაკითხვის შეცდომა. აღმოჩენილია ფაილის დასრულება.';ru='Ошибка чтения XML. Обнаружено завершение файла.';en='XML reading error. Found file completion.'"));
	КонецЕсли;
...
Функция ФайлОтчетаИзАрхива(КаталогРаспаковки)
	...
	Если НайденныеФайлы.Количество() = 0 Тогда
		ВызватьИсключение(НСтр("ka='Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".';ru='Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".';en='Failed to get the report. Archive does not contain a file with the extension ""* .erf"".'"));
	Иначе
...
Процедура ЗаписатьВЖурналРегистрации(Уровень, ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка внешних регламентированных отчетов';ru='Загрузка внешних регламентированных отчетов';en='Loading external regulated reports'"), Уровень,,, ТекстОшибки);
