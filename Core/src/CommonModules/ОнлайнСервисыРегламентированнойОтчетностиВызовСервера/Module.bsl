...
Функция ЗапуститьФоновоеЗаданиеАктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(УникальныйИдентификаторФормы, МожноЗагружатьВнешниеФормыОтчетов = Ложь) Экспорт
	...
	Возврат ДлительныеОперации.ЗапуститьВыполнениеВФоне(
				...
				НСтр("ka='Производится обновление служебной информации из Интернет...';ru='Производится обновление служебной информации из Интернет...'"));
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ka='Не удалось распаковать архив с внешней формой отчета.';ru='Не удалось распаковать архив с внешней формой отчета.'");
		ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка внешних регламентированных отчетов';ru='Загрузка внешних регламентированных отчетов'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Если СвойстваМанифеста["ИДКонфигурации"] <> СвойстваКонфигурации["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации';ru='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации'");
	...
	Если СвойстваМанифеста["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.';ru='Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
"""%1""'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Внешний отчет ""%1"" вер. ""%2"" отключен.';ru='Внешний отчет ""%1"" вер. ""%2"" отключен.'"), СвойстваМанифеста["Имя"], СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ka='Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов';ru='Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов'");
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете';ru='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов';ru='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВРег(СвойстваОтчета["ИсточникОтчета"]) <> ВРег(СвойстваКонфигурации["ИсточникОтчета"]) Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов';ru='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ka='Не пройдена проверка на сравнение версий выбранного и хранимого отчетов';ru='Не пройдена проверка на сравнение версий выбранного и хранимого отчетов'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ТекстИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Внешний отчет вер. ""%1"" успешно загружен.';ru='Внешний отчет вер. ""%1"" успешно загружен.'"), СвойстваОтчета["КраткаяВерсияВнешнегоОтчета"]);
...
Функция ПрочитатьМанифест(КаталогРаспаковки)
	...
	Если Чтение.ТипУзла <> ТипУзлаXML.НачалоЭлемента ИЛИ Чтение.Имя <> "Info" Тогда
		ВызватьИсключение(НСтр("ka='XML-ის წაკითხვის შეცდომა. ფაილის არასწორი ფორმატი. მოსალოდნელია ელემენტ Info-ს დაწყება.';ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Info.'"));
	КонецЕсли;
	
	Если НЕ Чтение.Прочитать() Тогда
		ВызватьИсключение(НСтр("ka='XML-ის წაკითხვის შეცდომა. აღმოჩენილია ფაილის დასრულება.';ru='Ошибка чтения XML. Обнаружено завершение файла.'"));
	КонецЕсли;
...
Функция ФайлОтчетаИзАрхива(КаталогРаспаковки)
	...
	Если НайденныеФайлы.Количество() = 0 Тогда
		ВызватьИсключение(НСтр("ka='Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".';ru='Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".'"));
	Иначе
...
Процедура ЗаписатьВЖурналРегистрации(Уровень, ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка внешних регламентированных отчетов';ru='Загрузка внешних регламентированных отчетов'"), Уровень,,, ТекстОшибки);
