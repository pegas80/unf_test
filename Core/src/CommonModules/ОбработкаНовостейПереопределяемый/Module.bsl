...
//   НоваяСтрока.Наименование = НСтр("ka='როგორ განვაახლოთ კონფიგურაცია';ru='Как обновить конфигурацию';en='How to update the configuration'");
//   НоваяСтрока.Ссылка = Справочники.Новости.НайтиПоРеквизиту("УИННовости", "123456"); // Новость должна присутствовать в справочнике!
//   НоваяСтрока = Форма.Новости.Добавить();
//   НоваяСтрока.Наименование = НСтр("ka='როგორ მოვახდინოთ ავტოგანახლების გამართვა';ru='Как настроить автообновление';en='How to configure auto-updates'");
...
"	Отказ: %3';ka='Устанавливаем для нового пользователя признак ОповещениеВключено = Ложь."
"Параметры:"
"	Наименование: %1"
"	Это новый пользователь: %2"
"	Отказ: %3'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Начало';ru='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Начало';en='News. Overrides. ObrabotatZapisPolzovatelya. Start'"), // ИмяСобытия
	...
	ТекстСообщения = НСтр("ka='Завершаем для нового пользователя установку признака ОповещениеВключено = Ложь.';ru='Завершаем для нового пользователя установку признака ОповещениеВключено = Ложь.';en='Completing the new user setup feature OpoveschenieVklyucheno = False.'");
	ЗаписьЖурналаРегистрации(
		НСтр("ka='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Конец';ru='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Конец';en='News. Overrides. ObrabotatZapisPolzovatelya. the end'"), // ИмяСобытия
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
