...
Функция ОтправитьSMS(НомераПолучателей, Знач Текст, ИмяОтправителя = Неопределено, ПеревестиВТранслит = Ложь) Экспорт
	...
	Если Не НастройкаОтправкиSMSВыполнена() Тогда
		Результат.ОписаниеОшибки = НСтр("ka='SMS-ის გასაგზავნად პროვაიდერის გამართვები არასწორად არის მიცემული.';ru='Неверно заданы настройки провайдера для отправки SMS.';en='Set incorrectly provider settings to send SMS.'");
		Возврат Результат;
	КонецЕсли;
...
"ожидается тип ""Структура"", передан тип ""%1""';ka='Неверный тип значения в коллекции Результат.ОтправленныеСообщения:"
