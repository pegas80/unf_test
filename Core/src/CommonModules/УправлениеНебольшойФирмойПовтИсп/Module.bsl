...
//
Функция ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь = Неопределено, Настройка, ПустоеЗначение = Неопределено) Экспорт
...
//"Закройте и заново запустите программу.';ka='Не найдена ставка %БезНДС%. Откройте список ставок НДС (Компания - Все справочники - Ставки НДС) и проверьте,"
//"что у ставки %БезНДС% значение ставки = 0 и установлен флаг %НеОблагается%."
...
//	
//	Возврат Неопределено;
Возврат Справочники.СтавкиНДС.БезНДС;//_Паганини
...
Функция ПолучитьСтавкуНДСНоль() Экспорт
	...
	//
	//Возврат Неопределено;
	Возврат Справочники.СтавкиНДС.НДС0//_Паганини
...
//
Функция ПолучитьСтавкуНДС(ИмяПредопределенного) Экспорт
	...
	//
	//Возврат Неопределено;
	Возврат Справочники.СтавкиНДС[ИмяПредопределенного];
	//_Паганини
...
Функция ПолучитьПредопределенноеЗначениеСправочника(ИмяСправочника, ИмяПредопределеного) Экспорт
	ЗнСправочника = Справочники[ИмяСправочника][ИмяПредопределеного];
	Возврат ЗнСправочника;
КонецФункции

Функция ПолучитьПустуюСсылкуСправочника(ИмяСправочника) Экспорт
	ЗнСправочника = Справочники[ИмяСправочника].ПустаяСсылка();
	Возврат ЗнСправочника;
КонецФункции

Функция ПолучитьЗначениеСправочникаПоНаименованию(ИмяСправочника, Наименование) Экспорт
	ЗнСправочника = Справочники[ИмяСправочника].НайтиПоНаименованию(Наименование);
	Возврат ЗнСправочника;
КонецФункции

Функция ПолучитьПредопределенноеЗначениеСчета(ИмяПредопределеного) Экспорт
	ЗнСчета = ПланыСчетов.Хозрасчетный[ИмяПредопределеного];
	Возврат ЗнСчета;
КонецФункции
...
Функция ПолучитьОсновнойБанковскийСчетКонтрагента(ИмяКонтрагента) Экспорт
	...
	//|	Контрагенты.БанковскийСчетПоУмолчанию.Ссылка КАК БанковскийСчет
	//|ИЗ
	//|	Справочник.Контрагенты КАК Контрагенты
	//|ГДЕ
	//|	НЕ Контрагенты.ПометкаУдаления
	//|	И Контрагенты.БанковскийСчетПоУмолчанию <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка)
	//|	И Контрагенты.Ссылка = &Контрагент";
	//
	//Запрос.УстановитьПараметр("Контрагент",ИмяКонтрагента );
	//ТаблицаПоиска = Запрос.Выполнить().Выгрузить();
	//
	//Для Каждого Ст Из  ТаблицаПоиска  Цикл 	
	//	РезультатПоиска = Ст.БанковскийСчет.Ссылка ;
	//КонецЦикла;
	//		
	//
...
//++lyskov 19.02.2023
// todo_lysp Перенести в ПовторноеИспользованиеСервер.

// Возвращает значения ресурсов учетной политики переданной организации на даннный период.
// Рекомендуется в качестве периода передавать начало года искомой даты, поскольку учетная политика хранится
// с годовой периодичностью. Это позволит избежать дублирования одинаковых данных в кэше повторно используемых значений.
//
// Параметры:
//  Организация - СправочникСсылка.Организации - организация учетной политики.
//  Период - Дата - период данных учетной политики (рекомендуется передавать начало года).
//
// Возвращаемое значение:
//   ФиксированннаяСтруктура - значения ресурсов учетной политики.
//
Функция УчетнаяПолитикаОрганизации(Организация, Период) Экспорт
	
	СрезРегистра = РегистрыСведений.УчетнаяПолитикаРегл.СрезПоследних(
		Период,
		Новый Структура("Организация", Организация));
	
	Если СрезРегистра.Количество() = 0 Тогда
		Возврат Новый ФиксированнаяСтруктура;
	КонецЕсли;
	
	ЗначенияУчетнойПолитики = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СрезРегистра[0]);
	
	Возврат Новый ФиксированнаяСтруктура(ЗначенияУчетнойПолитики);
