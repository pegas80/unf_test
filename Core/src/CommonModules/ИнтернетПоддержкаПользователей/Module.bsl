...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецТикета) Тогда
		ВызватьИсключение НСтр("ka='პარამეტრ ""ტიკეტისმფლობელი""-ს მნიშვნელობა არ არის შევსებული';ru='Не заполнено значение параметра ""ВладелецТикета""';en='Not filled with the value of ""VladeletsTiketa""'");
	КонецЕсли;
	...
	Если ДанныеАутентификации = Неопределено Тогда
		...
		РезультатПолученияТикета.Вставить("СообщениеОбОшибке" , НСтр("ka='არასწორი ლოგ-ინი ან პაროლი.';ru='Неверный логин или пароль.';en='Wrong login or password.'"));
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"webits.1c.ru",
			443,
			НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა (ზონა.ru)';ru='Интернет-поддержка пользователей (зона ru)';en='Internet-user support (zone ru)'"));
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"login.1c.ru",
			443,
			НСтр("ka='ავთენტიფიკაციის სერვისები (ზონა ru)';ru='Сервисы аутентификации (зона ru)';en='Authentication Services (zone ru)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"login.1c.eu",
			443,
			НСтр("ka='ავთენტიფიკაციის სერვისები (ზონა eu)';ru='Сервисы аутентификации (зона eu)';en='Authentication Services (eu zone)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			443,
			НСтр("ka='ტექნიკური მხარდაჭერის სამსახურის სერვისები (ზონა ru)';ru='Сервисы службы технической поддержки (зона ru)';en='Services Technical Support (zone ru)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			443,
			НСтр("ka='ტექნიკური მხარდაჭერის სამსახურის სერვისები (ზონა eu)';ru='Сервисы службы технической поддержки (зона eu)';en='Services Technical Support (eu Area)'"));
		НовыеРазрешения.Добавить(Разрешение);
...
Процедура ПриВключенииРазделенияПоОбластямДанных() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ka='Обработка включения разделения по областям данных.';ru='Обработка включения разделения по областям данных.';en='Processing enable separation of data fields.'"));
...
Функция ПостащикУслугПортал1СИТСПриФормированииСпискаУслуг(ПоставщикиУслуг) Экспорт
	...
	ПоставщикПортал1СИТС.Вставить("Наименование" , НСтр("ka='პორტალი  1С:ИТС';ru='Портал 1С:ИТС';en='Portal 1C: ITS'"));
	...
	Иначе
		...
		Если ТребуетсяРазделение Тогда
			Если ЗначениеРазделителя = Неопределено Тогда
				ВызватьИсключение НСтр("ka='Не заполнено значение параметра ""ЗначениеРазделителя"".';ru='Не заполнено значение параметра ""ЗначениеРазделителя"".';en='Not filled with the value of ""ZnachenieRazdelitelya"".'");
			КонецЕсли;
		...
		Если Не Результат Тогда
			ЗаписатьИнформациюВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Услуга с идентификатором %1 не подключена.';ru='Услуга с идентификатором %1 не подключена.';en='The service identifier% 1 not connected.'"),
					ИдентификаторУслуги));
		КонецЕсли;
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
	...
	Если ЛокальныйЗаписатьСообщениеВЖурналРегистрации = Истина Тогда
		...
		КомментарийЛокальный = НСтр("ka='ნაბიჯის იდენტიფიკატორი: %ИдентификаторШага%"
"%Комментарий%';ru='Идентификатор шага: %ИдентификаторШага%"
"%Комментарий%'");
				...
				Если ТипЗнч(ПараметрыФайлаВыгрузки) = ТипСтруктура Тогда
					Если (ПараметрыФайлаВыгрузки.Свойство("Архивировать"))
						...
						Архив = Новый ЗаписьZipФайла(
							...
							, // Без пароля
							СтрШаблон(
								НСтр("ka='Выгрузка журнала регистраций библиотеки БИП за период %1...%2';ru='Выгрузка журнала регистраций библиотеки БИП за период %1...%2';en='Unloading magazine registrations BIP library for the period% 1% 2 ...'"),
...
"Возможно, отсутствуют события за указанный период (%1...%2).';ka='Файл с выгрузкой событий журнала регистрации не существует."
...
"%1';ka='Не удалось экспортировать события журнала регистрации в файл по причине:"
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ka='БазовыйФункционал. Отладка. Экспорт журнала регистрации';ru='БазовыйФункционал. Отладка. Экспорт журнала регистрации';en='BazovyyFunktsional. Debugging. Export log'"), // ИдентификаторШага
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейОшибка()
	...
	Возврат НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა.შეცდომა';ru='Интернет-поддержка пользователей.Ошибка';en='Internet support polzovateley.Oshibka'",
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация()
	...
	Возврат НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერა. ინფორმაცია';ru='Интернет-поддержка пользователей.Информация';en='Internet support polzovateley.Informatsiya'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ka='Запускается при первом запуске, в общих данных. Установка общих данных.';ru='Запускается при первом запуске, в общих данных. Установка общих данных.';en='It starts at the first start, in general data. Setting the common data.'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ka='Запускается при первом запуске, для каждой области данных. Установка разделенных данных.';ru='Запускается при первом запуске, для каждой области данных. Установка разделенных данных.';en='It starts at the first start, for each data field. Installing the cut data.'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на новую версию, в общих данных. Установка общих данных.';ru='Запускается при переходе на новую версию, в общих данных. Установка общих данных.';en='Runs during the transition to the new version, in the general data. Setting the common data.'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ka='Запускается при переходе на любую новую версию, для каждой области данных. Установка разделенных данных.';ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка разделенных данных.';en='Runs during the transition to any new version for each data field. Installing the cut data.'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск';ru='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск';en='Performing ObnovlenieInformatsionnoyBazy_ObschieDannye_PervyyZapusk'"));
...
"'"),
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';en='Service and regulations. ObnovlenieInformatsionnoyBazy. Total information. First start'"), // ИдентификаторШага
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск';ru='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск';en='Performing ObnovlenieInformatsionnoyBazy_OblastDannyh_PervyyZapusk'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск, Переопределение';ru='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск, Переопределение';en='Performing ObnovlenieInformatsionnoyBazy_OblastDannyh_PervyyZapusk, Override'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("ka='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';en='Service and regulations. ObnovlenieInformatsionnoyBazy. OblastDannyh. First start'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию';ru='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию';en='Performing ObnovlenieInformatsionnoyBazy_ObschieDannye_PereytiNaVersiyu'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск, Основные действия, Проверка внедрения';ru='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск, Основные действия, Проверка внедрения';en='Performing ObnovlenieInformatsionnoyBazy_ObschieDannye_PervyyZapusk, Key actions, introduction Testing'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("ka='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';en='Service and regulations. ObnovlenieInformatsionnoyBazy. OblastDannyh. PereytiNaVersiyu'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию';ru='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию';en='Performing ObnovlenieInformatsionnoyBazy_OblastDannyh_PereytiNaVersiyu'"));
...
"Необходима версия библиотеки стандартных подсистем не ниже версии 2.3.3.69';ka='Ошибка внедрения библиотеки Интернет-поддержки пользователей."
