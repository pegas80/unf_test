	...
	Если ТипЗнч(СтруктураРасписания) = ТипСтруктура Тогда
		...
		Если СтруктураРасписания.РасписаниеВключено = Истина Тогда
			...
			Если СтруктураРасписания.Вариант = "Ежедневно_ОдинРазВДень" Тогда
				...
				Результат =
					Результат
					+ СтрШаблон(НСтр("ka='#Ежедневно в %1#';ru='#Ежедневно в %1#';en='# Every day #% 1'"),
			...
			Если СтруктураРасписания.Вариант = "Ежедневно_НесколькоРазВДень" Тогда
				...
				Если ЭлементСтруктуры.КоличествоЧасовПовтора = 1 Тогда
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#Ежедневно с %1 до %2 каждый час#';ru='#Ежедневно с %1 до %2 каждый час#';en='# Daily from 1% to 2% each hour #'"),
							Формат(ЭлементСтруктуры.ВремяНачала, "ДФ=HH:mm"),
							Формат(ЭлементСтруктуры.ВремяОкончания, "ДФ=HH:mm"));
				Иначе
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#Ежедневно с %1 до %2 каждые %3#';ru='#Ежедневно с %1 до %2 каждые %3#';en='# Daily from 1% to 2% # 3% each'"),
							Формат(ЭлементСтруктуры.ВремяНачала, "ДФ=HH:mm"),
							Формат(ЭлементСтруктуры.ВремяОкончания, "ДФ=HH:mm"),
			...
			Если СтруктураРасписания.Вариант = "Еженедельно" Тогда
				...
				Если ВРег(Вариант) = ВРег("Кратко") Тогда
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#еженедельно в %1#';ru='#еженедельно в %1#';en='# Weekly 1% #'"),
							Формат(ЭлементСтруктуры.Время, "ДФ=HH:mm"));
				Иначе // Подробно.
					СписокДнейНедели = "";
					...
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#В %1 по дням недели: %2#';ru='#В %1 по дням недели: %2#';en='# The% 1 per day: 2% #'"),
							Формат(ЭлементСтруктуры.Время, "ДФ=HH:mm"),
							СписокДнейНедели);
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураРасписания.Вариант = "Ежемесячно_ПоДням" Тогда
				...
				Если ВРег(Вариант) = ВРег("Кратко") Тогда
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#в определенные числа месяца в %1#';ru='#в определенные числа месяца в %1#';en='# In certain day of the month in% 1 #'"),
							Формат(ЭлементСтруктуры.Время, "ДФ=HH:mm"));
				Иначе // Подробно.
					СписокДнейМесяца = "";
					Для Каждого ТекущийДеньМесяца Из ЭлементСтруктуры.ДниМесяца Цикл
						Если (ТекущийДеньМесяца = 32) ИЛИ (ТекущийДеньМесяца = 99) Тогда
							СписокДнейМесяца = СписокДнейМесяца + "#" + НСтр("ka='თვის ბოლო დღე';ru='последний день месяца';en='the last day of the month'") + "#";
						Иначе
					...
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#В %1 по числам месяца: %2#';ru='#В %1 по числам месяца: %2#';en='#% 1 in the number of months: 2% #'"),
							Формат(ЭлементСтруктуры.Время, "ДФ=HH:mm"),
							СписокДнейМесяца);
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураРасписания.Вариант = "Ежемесячно_ПоДнямНедели" Тогда
				...
				Если ВРег(Вариант) = ВРег("Кратко") Тогда
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#в определенные дни недель в пределах месяца в %1#';ru='#в определенные дни недель в пределах месяца в %1#';en='# On certain days of the week within the month at 1% #'"),
							Формат(ЭлементСтруктуры.Время, "ДФ=HH:mm"));
				Иначе // Подробно.
					Если ЭлементСтруктуры.НомерНедели = 1 Тогда
						НомерНеделиСтрокой = НСтр("ka='პირველი';ru='первой';en='first'");
					ИначеЕсли ЭлементСтруктуры.НомерНедели = 2 Тогда
						НомерНеделиСтрокой = НСтр("ka='მეორე';ru='второй';en='second'");
					ИначеЕсли ЭлементСтруктуры.НомерНедели = 3 Тогда
						НомерНеделиСтрокой = НСтр("ka='მესამე';ru='третьей';en='third'");
					ИначеЕсли ЭлементСтруктуры.НомерНедели = 4 Тогда
						НомерНеделиСтрокой = НСтр("ka='მეოთხე';ru='четвертой';en='fourth'");
					ИначеЕсли ЭлементСтруктуры.НомерНедели = 5 Тогда
						НомерНеделиСтрокой = НСтр("ka='ბოლო';ru='последней';en='the last'");
					КонецЕсли;
					...
					Результат =
						Результат
						+ СтрШаблон(НСтр("ka='#В %1 по дням недели: %2 каждой %3 недели#';ru='#В %1 по дням недели: %2 каждой %3 недели#';en='# The% 1 per day:% 2% every 3 weeks #'"),
							Формат(ЭлементСтруктуры.Время, "ДФ=HH:mm"),
							СписокДнейНедели,
							НомерНеделиСтрокой);
				КонецЕсли;
		...
		Иначе
			...
			Если ВРег(Вариант) = ВРег("Кратко") Тогда
				Результат =
					Результат
					+ НСтр("ka='გამორთულია';ru='отключено';en='disabled'");
			Иначе // Подробно.
				Результат =
					Результат
					+ НСтр("ka='Выполнение по-расписанию отключено.';ru='Выполнение по-расписанию отключено.';en='on-schedule execution disabled.'");
			КонецЕсли;
...
Функция ПолучитьПредставлениеРазмера(Размер) Экспорт
	...
	Если Размер < 1*1024*1024 Тогда // До 1 Мб - в байтах
		Результат = СтрШаблон(
			НСтр("ka='%1 ბაიტი';ru='%1 байт';en='% 1 byte'"),
			Формат(Размер, "ЧЦ=20; ЧДЦ=; ЧРГ=' '; ЧН=0; ЧГ=3,0"));
	Иначе // В Мегабайтах
		Результат = СтрШаблон(
			НСтр("ka='%1 მბაიტი';ru='%1 Мбайт';en='% 1 MB'"),
...
Функция ЕстьЗапрещенныеСимволыДляКоманднойСтроки(СтрокаДляПроверки, ТекстОшибки = Неопределено) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ka='Найдены запрещенные символы: %1';ru='Найдены запрещенные символы: %1';en='Found invalid characters:% 1'"),
			СписокНайденныхЗапрещенныхСимволов);
	КонецЕсли;
...
Функция ПолучитьСтруктуруСуффиксовИдентификаторовИБ() Экспорт
	...
	Результат.Вставить("РучнаяКопия",
			Новый Структура("Суффикс, Описание",
				"_Manual",
				НСтр("ru=' (вручную)';ka='(მექანიკურად)'")));
