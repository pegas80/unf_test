...
Функция ВыполнитьОбновлениеИнформационнойБазы(ПараметрыОбновления) Экспорт
	...
	Если НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		Сообщение = НСтр("ka='პროგრამის ვერსიის გასაახლებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для обновления версии программы.'");
...
"Будет выполнено обновление информационной базы.'"),
	...
	Иначе
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მიმდინარეობს მონაცემთა დაწყებითი შევსება ვერსია ""%1""-მდე.';ru='Выполняется начальное заполнение данных до версии ""%1"".'"),
			ВерсияМетаданных);
	КонецЕсли;
	...
	Сообщение = НСтр("ka='ახალი ვერსიით პროგრამის გასაახლებლად შესრულდება დამამუშავებლები: %1';ru='Для обновления программы на новую версию будут выполнены обработчики: %1'");
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Обновление информационной базы на версию ""%1"" выполнено успешно.';ru='Обновление информационной базы на версию ""%1"" выполнено успешно.'"), ВерсияМетаданных);
	ЗаписатьИнформацию(Сообщение);
...
"Ожидалось: %3; передано значение: %4 (тип %5).';ka='პარამეტრ %1-ის დაუშვებელი მნიშვნელობა %2-ში. მოსალოდნელი იყო: %3; გადაცემულია მნიშვნელობა: %4 (ტიპი %5).'"),
			...
			НСтр("ka='დაწყებითიშევსება, ვერსიისგანახლება ან სხვაპროგრამიდანგადასვლა';ru='НачальноеЗаполнение, ОбновлениеВерсии или ПереходСДругойПрограммы'"), 
...
Процедура ЗаписатьВремяВыполненияОбновления(ВремяНачалаОбновления, ВремяОкончанияОбновления) Экспорт
	...
	ПродолжительностьЧасы = ?(Часы = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 სთ';ru='%1 час'"), Часы));
	ПродолжительностьМинуты = ?(Минуты = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 мин';ru='%1 мин'"), Минуты));
	ПродолжительностьСекунды = ?(Секунды = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წამი';ru='%1 сек'"), Секунды));
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				ШаблонСообщения = НСтр("ka='Не найден план обновления конфигурации %1 с версии %2 на версию %3';ru='Не найден план обновления конфигурации %1 с версии %2 на версию %3'");
			Иначе
				ШаблонСообщения = НСтр("ka='Не найден план обновления библиотеки %1 с версии %2 на версию %3';ru='Не найден план обновления библиотеки %1 с версии %2 на версию %3'");
			КонецЕсли;
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ka='ხორციელდება საინფორმაციო ბაზის განახლების აუცილებელი პროცედურები.';ru='Выполняются обязательные процедуры обновления информационной базы.'");
		Иначе
			НоваяВерсияИБ = Версия.Версия;
			Если ТекущаяВерсияИБ = "0.0.0.0" Тогда
				Сообщение = НСтр("ka='მიმდინარეობს მონაცემთა საწყისი შევსება.';ru='Выполняется начальное заполнение данных.'");
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვერსია %1-დან ვერსია %2-ზე ხორციელდება საინფორმაციო ბაზის განახლება.';ru='Выполняется обновление информационной базы с версии %1 на версию %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიმდინარეობს ბიბლიოთეკა %3-ის მონაცემთა %1 ვერსიიდან %2 ვერსიაზე განახლება.';ru='Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.'"), 
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ka='შესრულებულია საინფორმაციო ბაზის განახლების აუცილებელი პროცედურები';ru='Выполнены обязательные процедуры обновления информационной базы.'");
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საინფორმაციო ბაზის განახლება შესრულებულია %1 ვერსიიდან ვერსია %2-ზე.';ru='Выполнено обновление информационной базы с версии %1 на версию %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შესრულებულია ბიბლიოთეკა %3-ის მონაცემთა განახლება %1 ვერსიიდან %2 ვერსიაზე.';ru='Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
...
Функция ОбновитьИнформационнуюБазуВФоне(УникальныйИдентификаторФормы, БлокировкаИБ) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Фоновое обновление информационной базы';ru='Фоновое обновление информационной базы'");
...
Процедура ПерейтиСДругойПрограммы()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ka='При работе в модели сервиса переход с другой программы не предусмотрен.';ru='При работе в модели сервиса переход с другой программы не предусмотрен.'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ბაზის განახლება';ru='Обновление информационной базы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПротокол() Экспорт
	...
	Возврат СобытиеЖурналаРегистрации() + "." + НСтр("ka='შესრულების პროტოკოლი';ru='Протокол выполнения'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='პროგრამის განახლება არ დასრულებულა';ru='Обновление программы не завершено'");
...
"так как он выполняется только при переходе на версию %2'");
		...
		ИначеЕсли ВыполняемыйОбработчик = Неопределено Тогда
			ТекстИсключения = НСтр("ka='Отключаемый обработчик обновления %1 не существует';ru='Отключаемый обработчик обновления %1 не существует'");
...
Процедура ПроверитьВложеннуюТранзакцию(ТранзакцияАктивнаНаНачалоВыполнения, НазваниеОбработчика)
	...
	ИмяСобытия = СобытиеЖурналаРегистрации() + "." + НСтр("ka='დამამუშავებლების შესრულება';ru='Выполнение обработчиков'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		Если ПустаяСтрока(Обработчик.Версия) Тогда
			...
			Если Обработчик.НачальноеЗаполнение <> Истина Тогда
				ОписаниеОшибки = НСтр("ka='У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.';ru='У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.'");
			КонецЕсли;
...
"свойство НачальноеЗаполнение.'");
		...
		Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда
			ЗаголовокОшибки = НСтр("ka='Ошибка в свойстве обработчика обновления конфигурации';ru='Ошибка в свойстве обработчика обновления конфигурации'");
		Иначе
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Ошибка в свойстве обработчика обновления библиотеки %1 версии %2';ru='Ошибка в свойстве обработчика обновления библиотеки %1 версии %2'"),
...
Процедура ЗаписатьИнформациюОХодеОбновления(Обработчик, ХодВыполненияОбработчиков, ВФоне)
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Сообщение = НСтр("ka='მიმდინარეობს შესრულება განახლების დამამაუშავებელი %1 (%2 %3-დან).';ru='Выполняется обработчик обновления %1 (%2 из %3).'");
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗапуститьПроцедуруОбработкиДанных(ОбработчикОбновления, ПараметрыОбновления)
	...
	Попытка
		СообщениеОЗапущенномОбработчике = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიმდინარეობს განახლების პროცედურა ""%1"".';ru='Выполняется процедура обновления ""%1"".'"), ИмяОбработчика);
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим И Не ПропуститьПроверкуОбработанныхДанных Тогда
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				Если ЧислоПопыток >= 2 Тогда
					ТекстИсключения = НСтр("ka='მონაცემთა დამუშავების პროცედურა დაიციკლა. შესრულება შეწყვეტილია.';ru='Произошло зацикливание процедуры обработки данных. Выполнение прервано.'");
					ВызватьИсключение ТекстИсключения;
				Иначе
