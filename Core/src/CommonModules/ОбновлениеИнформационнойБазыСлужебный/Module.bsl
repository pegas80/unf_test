...
Функция ВыполнитьОбновлениеИнформационнойБазы(ПараметрыОбновления) Экспорт
	...
	Если НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		Сообщение = НСтр("ka='პროგრამის ვერსიის გასაახლებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для обновления версии программы.';en='Insufficient privileges to update the version of the program.'");
	...
	Если РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონფიგურაციის სახელი შეიცვალა ""%1""-ით. შესრულდება სხვა პროგრამიდან გადასვლა.';ru='Изменилось имя конфигурации на ""%1""."
"Будет выполнен переход с другой программы.'"),
			Метаданные.Имя);
	ИначеЕсли РежимОбновленияДанных = "ОбновлениеВерсии" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შეიცვალა კონფიგურაციის ვერსიის ნომერი: ""%1""-დან ""%2""-ზე. მოხდება საინფორმაციო ბაზის განახლება.';ru='Изменился номер версии конфигурации: с ""%1"" на ""%2""."
"Будет выполнено обновление информационной базы.'"),
			ВерсияДанных, ВерсияМетаданных);
	Иначе
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მიმდინარეობს მონაცემთა დაწყებითი შევსება ვერსია ""%1""-მდე.';ru='Выполняется начальное заполнение данных до версии ""%1"".';en='Performed the initial filling of the data to version ""%1"".'"),
			ВерсияМетаданных);
	КонецЕсли;
	...
	Сообщение = НСтр("ka='ახალი ვერსიით პროგრამის გასაახლებლად შესრულდება დამამუშავებლები: %1';ru='Для обновления программы на новую версию будут выполнены обработчики: %1';en='To upgrade to the new version will be performed handlers:% 1'");
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Обновление информационной базы на версию ""%1"" выполнено успешно.';ru='Обновление информационной базы на версию ""%1"" выполнено успешно.';en='Update information database to version ""%1"" was successful.'"), ВерсияМетаданных);
	ЗаписатьИнформацию(Сообщение);
...
"Ожидалось: %3; передано значение: %4 (тип %5).';ka='პარამეტრ %1-ის დაუშვებელი მნიშვნელობა %2-ში. მოსალოდნელი იყო: %3; გადაცემულია მნიშვნელობა: %4 (ტიპი %5).'"),
			"РежимОбновленияДанных", "ПриОпределенииРежимаОбновленияДанных", 
			НСтр("ka='დაწყებითიშევსება, ვერსიისგანახლება ან სხვაპროგრამიდანგადასვლა';ru='НачальноеЗаполнение, ОбновлениеВерсии или ПереходСДругойПрограммы';en='NachalnoeZapolnenie, ObnovlenieVersii or PerehodSDrugoyProgrammy'"), 
...
Функция ИнформационнаяБазаЗаблокированаДляОбновления(УчитыватьПривилегированныйРежим = Истина) Экспорт
	...
	СообщениеАдминистраторуСистемы =
		НСтр("ka='პროგრამაში შესვლა დროებით შეუძლებელია ახალი ვერსიით განახლების გამო. პროგრამის ვერსიის განახლების დასასრულებლად, საჭიროა ადმინისტრაციული უფლებები (როლი ""სისტემის ადმინისტრატორი"" და ""სრული უფლებები"").';ru='Вход в программу временно невозможен в связи с обновлением на новую версию."
...
"(роли ""Администратор системы"" и ""Полные права"").'");
	...
	Если НеобходимоОбновлениеНеразделенныхДанныхИнформационнойБазы() Тогда
		...
		СообщениеАдминистраторуОбластиДанных =
			НСтр("ka='დანართში შესვლა დროებით შეუძლებელია ახალი ვერსიით განახლების გამო. დაწვრილებითი ინფორმაციისთვის, მიმართეთ სერვისის ადმინისტრატორს.';ru='Вход в приложение временно невозможен в связи с обновлением на новую версию."
...
"Обратитесь к администратору сервиса за подробностями.'");
	Иначе
		...
		Сообщение =
			НСтр("ka='პროგრამაში შესვლა დროებით შეუძლებელია ახალი ვერსიით განახლების გამო. დაწვრილებითი ინფორმაციისთვის, მიმართეთ ადმინისტრატორს.';ru='Вход в программу временно невозможен в связи с обновлением на новую версию."
...
Процедура ЗаписатьВремяВыполненияОбновления(ВремяНачалаОбновления, ВремяОкончанияОбновления) Экспорт
	...
	ПродолжительностьЧасы = ?(Часы = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 სთ';ru='%1 час';en='%1 hour'"), Часы));
	ПродолжительностьМинуты = ?(Минуты = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 мин';ru='%1 мин';en='%1 minute'"), Минуты));
	ПродолжительностьСекунды = ?(Секунды = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წამი';ru='%1 сек';en='% 1 seconds'"), Секунды));
...
"ознакомиться с документацией на ИТС (http://its.1c.ru/db/bspdoc)';ka='მიწოდების ფაილი 1С:სტანდარტული ქვესისტემების ბიბლიოთეკა შაბლონის მიხედვით საინფორმაციო ბაზების შესაქმნელად არ არის განკუთვნილი. გამოყენებამდე, აუცილებელია სტთ-ზე დოკუმენტაციასთან გაცნობა (http://its.1c.ru/db/bspdoc)'");
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				ШаблонСообщения = НСтр("ka='Не найден план обновления конфигурации %1 с версии %2 на версию %3';ru='Не найден план обновления конфигурации %1 с версии %2 на версию %3';en='Not found renovation plan configuration% 1 version% 2 to version% 3'");
			Иначе
				ШаблонСообщения = НСтр("ka='Не найден план обновления библиотеки %1 с версии %2 на версию %3';ru='Не найден план обновления библиотеки %1 с версии %2 на версию %3';en='Not found a plan of renovation of the library% 1 version% 2 to version% 3'");
			КонецЕсли;
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ka='ხორციელდება საინფორმაციო ბაზის განახლების აუცილებელი პროცედურები.';ru='Выполняются обязательные процедуры обновления информационной базы.';en='Performed the required procedures for updating the information base.'");
		Иначе
			НоваяВерсияИБ = Версия.Версия;
			Если ТекущаяВерсияИБ = "0.0.0.0" Тогда
				Сообщение = НСтр("ka='მიმდინარეობს მონაცემთა საწყისი შევსება.';ru='Выполняется начальное заполнение данных.';en='Performed the initial data to fill.'");
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვერსია %1-დან ვერსია %2-ზე ხორციელდება საინფორმაციო ბაზის განახლება.';ru='Выполняется обновление информационной базы с версии %1 на версию %2.';en='Are updating the information base from version to version% 1% 2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიმდინარეობს ბიბლიოთეკა %3-ის მონაცემთა %1 ვერსიიდან %2 ვერსიაზე განახლება.';ru='Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.';en='Performed data library update from version% 3% 1 version% 2.'"), 
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ka='შესრულებულია საინფორმაციო ბაზის განახლების აუცილებელი პროცედურები';ru='Выполнены обязательные процедуры обновления информационной базы.';en='Made mandatory procedure for updating the information base.'");
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საინფორმაციო ბაზის განახლება შესრულებულია %1 ვერსიიდან ვერსია %2-ზე.';ru='Выполнено обновление информационной базы с версии %1 на версию %2.';en='Upgraded information database from version to version% 1% 2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შესრულებულია ბიბლიოთეკა %3-ის მონაცემთა განახლება %1 ვერსიიდან %2 ვერსიაზე.';ru='Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.';en='Upgraded data library% 3 with version% 1 version% 2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
...
Функция ОбновитьИнформационнуюБазуВФоне(УникальныйИдентификаторФормы, БлокировкаИБ) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Фоновое обновление информационной базы';ru='Фоновое обновление информационной базы';en='Background updating the information base'");
...
""
"Подробности ошибки:"
"%1';ka='Невозможно выполнить обновление информационной базы:"
"- Невозможно установить монопольный режим"
"- Версия конфигурации не предусматривает обновление без установки монопольного режима"
""
"Подробности ошибки:"
...
Процедура ПерейтиСДругойПрограммы()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ka='При работе в модели сервиса переход с другой программы не предусмотрен.';ru='При работе в модели сервиса переход с другой программы не предусмотрен.';en='When working in the service transition model with other programs is not provided.'");
	КонецЕсли;
...
"""%1"""
"произошла ошибка:"
"""%2"".';ka='При вызове обработчика перехода с другой программы"
"""%1"""
"произошла ошибка:"
"""%2"".'"),
				ИмяОбработчика,
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ka='საინფორმაციო ბაზის განახლება';ru='Обновление информационной базы';en='Update the information base'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПротокол() Экспорт
	...
	Возврат СобытиеЖурналаРегистрации() + "." + НСтр("ka='შესრულების პროტოკოლი';ru='Протокол выполнения';en='perform protocol'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"""%1"".';ka='განახლების გადადებული დამამუშავებლის მონაცემთა შევსების პროცედურა არ არის მითითებული ""%1"".'"),
...
"не должен быть установлен признак ""МонопольныйРежим"".';ka='У отложенного обработчика ""%1"""
...
""
"Данные свойства не могут одновременно принимать значение ""Истина"".';ka='გადადებულ დამამუშავებელ ""%1""-ს არაკორექტულად აქვს შევსებული თვისებების მნიშვნელობები:"
" - ""მხოლოდმთავარკვანძშიგაშვება"""
" - ""გაშვებაგსბდაქვემდებარებულკვანძშიცფილტრებიანად""."
""
"მოცემულ თვისებებს არ შეუძლიათ, რომ ერთდროულად მიიღონ მნიშვნელობა ""ჭეშმარიტება"".'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
""
"Данное свойство не может принимать значение ""Истина"" у отложенного обработчика.';ka='გადადებულ დამამუშავებელ ""%1""-ს მითითებული აქვს თვისება ""საერთომონაცემები""-ს დაუშვებელი მნიშვნელობა."
""
"მოცემული თვისება გადადებულ დამამუშავებელზე ვერ შეძლებს მნიშვნელობა ""ჭეშმარიტება""-ს მიღებას.'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='პროგრამის განახლება არ დასრულებულა';ru='Обновление программы не завершено';en='software update has not been completed'");
...
Функция ВозможноОперативноеОбновление(ПроверяемыеИтерацииОбновления = Неопределено) Экспорт
	...
	Если ПроцедурыОбработчиков.Количество() <> 0 Тогда
		ТекстСообщения = НСтр("ru='Следующие обработчики не поддерживают обновление без установки монопольного режима:';en='The following handlers do not support the upgrade without installing the single-user mode:'");
...
"так как он выполняется только при переходе на версию %2';ka='Обработчик обновления %1 не может быть отключен, "
"так как он выполняется только при переходе на версию %2'");
		...
		ИначеЕсли ВыполняемыйОбработчик = Неопределено Тогда
			ТекстИсключения = НСтр("ka='Отключаемый обработчик обновления %1 не существует';ru='Отключаемый обработчик обновления %1 не существует';en='Switchable processor upgrade% 1 does not exist'");
...
"""%1"""
"произошла ошибка:"
"""%2"".';ka='При вызове обработчика обновления:"
"""%1"""
"произошла ошибка:"
"""%2"".'"),
			ИмяОбработчика,
...
Процедура ПроверитьВложеннуюТранзакцию(ТранзакцияАктивнаНаНачалоВыполнения, НазваниеОбработчика)
	...
	ИмяСобытия = СобытиеЖурналаРегистрации() + "." + НСтр("ka='დამამუშავებლების შესრულება';ru='Выполнение обработчиков';en='Performing handlers'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"При активных транзакциях, открытых выше по стеку, исключение также необходимо пробрасывать выше по стеку.';ka='Ошибка выполнения обработчика обновления %1:"
"Обработчиком обновления было поглощено исключение при активной внешней транзакции."
"При активных транзакциях, открытых выше по стеку, исключение также необходимо пробрасывать выше по стеку.'");
		...
		Иначе
			ШаблонКомментария = НСтр("ru='Ошибка выполнения обработчика обновления %1:"
"Обработчиком обновления была закрыта лишняя транзакция, открытая ранее (выше по стеку).';ka='Ошибка выполнения обработчика обновления %1:"
"Обработчиком обновления была закрыта лишняя транзакция, открытая ранее (выше по стеку).'");
	...
	Иначе
		Если ТранзакцияАктивна() Тогда
			ШаблонКомментария = НСтр("ru='Ошибка выполнения обработчика обновления %1:"
"Открытая внутри обработчика обновления транзакция осталась активной (не была закрыта или отменена).';ka='Ошибка выполнения обработчика обновления %1:"
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		Если ПустаяСтрока(Обработчик.Версия) Тогда
			...
			Если Обработчик.НачальноеЗаполнение <> Истина Тогда
				ОписаниеОшибки = НСтр("ka='У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.';ru='У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.';en='At the handler is empty property or property NachalnoeZapolnenie version.'");
			КонецЕсли;
...
"свойство ВыполнятьВГруппеОбязательных.';ka='У обработчика не правильно заполнено свойство Приоритет или"
...
"Допустимое значение: ""Монопольно"", ""Отложенно"", ""Оперативно"".';ka='У обработчика ""%1"" не правильно заполнено свойство РежимВыполнения."
...
"свойство НачальноеЗаполнение.';ka='У обработчика не правильно заполнено свойство Опциональный или"
"свойство НачальноеЗаполнение.'");
		...
		Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда
			ЗаголовокОшибки = НСтр("ka='Ошибка в свойстве обработчика обновления конфигурации';ru='Ошибка в свойстве обработчика обновления конфигурации';en='Error in property handler configuration updates'");
		Иначе
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Ошибка в свойстве обработчика обновления библиотеки %1 версии %2';ru='Ошибка в свойстве обработчика обновления библиотеки %1 версии %2';en='in the property handler error updating library version% 1% 2'"),
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("ka='(%1).';ru='(%1).';en='(%one).'") + Символы.ПС
			+ Символы.ПС
			+ ОписаниеОшибки,
			Обработчик.Процедура);
...
Процедура ЗаписатьИнформациюОХодеОбновления(Обработчик, ХодВыполненияОбработчиков, ВФоне)
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Сообщение = НСтр("ka='მიმდინარეობს შესრულება განახლების დამამაუშავებელი %1 (%2 %3-დან).';ru='Выполняется обработчик обновления %1 (%2 из %3).';en='Running handler updates 1% (2% from 3%).'");
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗапуститьПроцедуруОбработкиДанных(ОбработчикОбновления, ПараметрыОбновления)
	...
	Попытка
		СообщениеОЗапущенномОбработчике = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიმდინარეობს განახლების პროცედურა ""%1"".';ru='Выполняется процедура обновления ""%1"".';en='Perform the upgrade procedure ""%1"".'"), ИмяОбработчика);
...
"Выполнение прервано для предотвращения зацикливания механизма обработки данных.';ka='განახლების პროცედურის გაშვების დასაშვები რაოდენობა გადამეტებულია. შესრულება შეწყვეტილია მონაცემთა დამუშავების მექანიზმის დაციკლების თავიდან ასაცილებლად.'");
...
"Выполнение прервано из-за явной ошибки в коде обработчика.';ka='განახლების დამამუშავებელმა არ მოახდინა პარამეტრ დამუშავებადასრულებულია-ს ინიციალიზება."
"შესრულება შეწყვეტილია დამამუშავებლის კოდში აშკარა შეცდომის გამო.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим И Не ПропуститьПроверкуОбработанныхДанных Тогда
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				Если ЧислоПопыток >= 2 Тогда
					ТекстИсключения = НСтр("ka='მონაცემთა დამუშავების პროცედურა დაიციკლა. შესრულება შეწყვეტილია.';ru='Произошло зацикливание процедуры обработки данных. Выполнение прервано.';en='It happened loop data processing procedures. Execution terminated.'");
					ВызватьИсключение ТекстИсключения;
				Иначе
...
"заполнен список блокируемых объектов, но не задано свойство ""ПроцедураПроверки"".';ka='გადადებულ დამამუშავებელ ""%1""-ს შევსებული აქვს ბლოკირებადი ობიექტების სია, მაგრამ თვისება ""შემოწმებისპროცედურა"" არ არის მიცემული.'");
	...
	Для Каждого СтрокаБиблиотека Из СведенияОбОбновлении.ДеревоОбработчиков.Строки Цикл
		...
		Для Каждого СтрокаВерсия Из СтрокаБиблиотека.Строки Цикл
			...
			Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
				...
				Попытка
					Сообщение = НСтр("ka='მიმდინარეობს განახლების გადადებული დამამუშავებლის"
"""%2"" "
"მონაცემთა შევსების პროცედურა"
"""%1"".';ru='Выполняется процедура заполнения данных"
"""%1"""
"отложенного обработчика обновления"
"""%2"".'");
...
"""%2"""
"произошла ошибка:"
"""%3"".';ka='განახლების გადადებული დამამუშავებელ "
"""%2""-ის "
"მონაცემთა შევსების პროცედურა "
"""%1""-ის მოხმობისას "
"შეცდომა მოხდა:"
