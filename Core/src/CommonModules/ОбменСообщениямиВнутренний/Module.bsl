				...
				Если МодульРаботаВМоделиСервиса.ОбластьДанныхЗаблокирована(ОбластьДанных) Тогда
					...
					Иначе
						ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Не удалось выполнить обмен сообщениями по причине: область данных %1 заблокирована!';ru='Не удалось выполнить обмен сообщениями по причине: область данных %1 заблокирована!';en='Unable to exchange messages for a reason:% 1 data area is locked!'"),
							ОбластьДанных);
					КонецЕсли;
				КонецЕсли;
...
"совпадающих со значениями разделителей сеанса.';ka='სისტემის შეტყობინებების რიგის დამუშავება გაშვებულია გამყოფების დაყენებული მნიშვნელობების სეანსიდან. დამუშავება განხორციელდება მხოლოდ იმ შეტყობინებებისთვის, რომლებიც შენახულია განცალკევებულ ცნობარში, გამყოფების მნიშვნელობების მქონე ელემენტებში, რომლებიც ემთხვევა სეანსის გამყოფების მნიშვნელობებს.'"));
        ...
        Попытка
            ...
            Попытка
                ...
                Если СчетчикПопыток > 3 Тогда
                    Если НЕ ЗначениеЗаполнено(ОбъектСообщения.ПодробноеПредставлениеОшибки) Тогда
                        ОбъектСообщения.ПодробноеПредставлениеОшибки = НСтр("ka='შეტყობინების დამუშავების მცდელობის რაოდენობა ამოწურულია.';ru='Исчерпано количество попыток обработки сообщения.';en='Exceeded the number of attempts to process the message.'");
                    КонецЕсли;
            ...
            Если ОбъектСообщения.Заблокировано Тогда
                ...
                ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЭтойПодсистемы(),
                        ...
                        СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                            НСтр("ka='შეტყობინების დამუშავების მცდელობის რაოდენობა ამოწურულია %1';ru='Исчерпано количество попыток обработки сообщения %1';en='Exhausted the number of message processing attempts to 1%'"),
            ...
            Попытка
                ...
                Если Выборка.КонечнаяТочкаЗаблокирована Тогда
                    ОбъектСообщения.Заблокировано = Истина;
                    ВызватьИсключение НСтр("ka='დაბლოკილი საბოლოო წერტილისგან მიღებული შეტყობინების დამუშავების მცდელობა.';ru='Попытка обработки сообщения, полученного от заблокированной конечной точки.';en='Attempting to process the message received from the endpoint is locked.'");
                КонецЕсли;

                Если НайденныеСтроки.Количество() = 0 Тогда
                    ОбъектСообщения.Заблокировано = Истина;
                    ВызватьИсключение НСтр("ka='შეტყობინებისთვის არ არის დანიშნული დამამუშავებელი.';ru='Не назначен обработчик для сообщения.';en='Not assigned a handler for the message.'");
                КонецЕсли;
                ...
                Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
                    ...
                    Если ТранзакцияАктивна() Тогда
                        ...
                        КонецЦикла;
                        ОбъектСообщения.Заблокировано = Истина;
                        ВызватьИсключение НСтр("ka='შეტყობინების დამამუშავებელში ტრანზაქცია არ დაფიქსირებულა.';ru='В обработчике сообщения не была зафиксирована транзакция.';en='The message handler has not been committed transactions.'");
                    КонецЕсли;
            ...
            Исключение
                ...
                ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЭтойПодсистемы(),
                        УровеньЖурналаРегистрации.Ошибка,,,
                        СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                            НСтр("ka='შეტყობინების დამუშავების შეცდომა %1: %2';ru='Ошибка обработки сообщения %1: %2';en='message processing error% 1% 2'"),
            ...
            Если ОбрабатыватьСообщенияВНеразделенныхДанных И ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
                ...
                ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    НСтр("ka='არხის შეტყობინების %1 დამუშავების შემდეგ, სეანსის დაყოფა არ ჩართულა!';ru='После обработки сообщения канала %1 не было выключено разделение сеанса!';en='After processing the message channel% 1 has not been turned off separation of the session!'"),
...
Процедура УстановитьВедущуюКонечнуюТочкуНаСторонеПолучателя(КодЭтойКонечнойТочки, КодВедущейКонечнойТочки) Экспорт
	...
	Если ПланыОбмена.ОбменСообщениями.НайтиПоКоду(КодЭтойКонечнойТочки) <> ЭтотУзел() Тогда
		СтрокаСообщенияОбОшибке = НСтр("ka='მიცემულია საბოლოო წერტილთან მიერთების არასწორი პარამეტრები. მიერთების პარამეტრები მიუთითებენ სხვა საბოლოო წერტილს.';ru='Заданы неверные параметры подключения к конечной точке. Параметры подключения указывают на другую конечную точку.';en='Are incorrect connection settings to the endpoint. Connection settings point to another endpoint.'");
		СтрокаСообщенияОбОшибкеДляЖурналаРегистрации = НСтр("ka='მიცემულია ბოლო წერტილთან მიერთების არასწორი პარამეტრები. მიერთების პარამეტრები მიუთითებენ სხვა ბოლო წერტილს.';ru='Заданы неверные параметры подключения к конечной точке."
"Параметры подключения указывают на другую конечную точку.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Попытка
		...
		Если УзелКонечнойТочки.Пустая() Тогда
			...
			ВызватьИсключение НСтр("ka='Конечная точка в базе-корреспонденте не обнаружена.';ru='Конечная точка в базе-корреспонденте не обнаружена.';en='Endpoint is not found in the database correspondent.'");
	...
	Попытка
		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Если ТипЗнч(Данные) = Тип("УдалениеОбъекта") Тогда
				...
				ВызватьИсключение НСтр("ka='Передача объекта УдалениеОбъекта через механизм быстрых сообщений не поддерживается!';ru='Передача объекта УдалениеОбъекта через механизм быстрых сообщений не поддерживается!';en='Transfer object UdalenieObekta through instant messaging mechanism is not supported!'");
...
Функция СобытиеЖурналаРегистрацииПодключениеКонечнойТочки() Экспорт
	...
	Возврат НСтр("ka='Обмен сообщениями.Подключение конечной точки';ru='Обмен сообщениями.Подключение конечной точки';en='Exchange soobscheniyami.Podklyuchenie endpoint'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУстановкаВедущейКонечнойТочки() Экспорт
	...
	Возврат НСтр("ka='Обмен сообщениями.Установка ведущей конечной точки';ru='Обмен сообщениями.Установка ведущей конечной точки';en='Exchange soobscheniyami.Ustanovka leading endpoint'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииЭтойПодсистемы() Экспорт
	...
	Возврат НСтр("ka='შეტყობინებების გაცვლა';ru='Обмен сообщениями';en='Message exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьПодключениеКонечнойТочкиНаСторонеОтправителя(Отказ,
	...
	Если Не УзелКонечнойТочки.Пустая() Тогда
		Отказ = Истина;
		СтрокаСообщенияОбОшибке = НСтр("ka='Конечная точка уже подключена к информационной базе; наименование точки: %1';ru='Конечная точка уже подключена к информационной базе; наименование точки: %1';en='Endpoint is already connected to the database; point name: 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
