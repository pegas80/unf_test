...
Процедура ВыполнитьКомандуДополнительнойОбработки(ИдентификаторОбработки, ИдентификаторКоманды, ИдентификаторОперации) Экспорт
	...
	Событие = Событие + НСтр("ka='Начало выполнения команды';ru='Начало выполнения команды'"); 
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			...
			Событие = ИмяСобытияЖурналаРегистрации() + ".";
			Событие = Событие + НСтр("ka='Завершение выполнения команды';ru='Завершение выполнения команды'"); 
		...
		Исключение
			...
			Событие = НСтр("ka='Ошибка выполнения команды';ru='Ошибка выполнения команды'"); 
			Комментарий = НСтр("ka='ბრძანების შესრულებისას წარმოიშვა შეცდომა"
"';ru='При выполнении команды возникла ошибка'"); 
	...
	Иначе
		ОшибкаВыполнения = Истина;
		Событие = НСтр("ka='Обработка не найдена';ru='Обработка не найдена'"); 
		Комментарий = НСтр("ka='Не удалось найти внешнюю обработку по идентификатору';ru='Не удалось найти внешнюю обработку по идентификатору'"); 
...
Функция ВыполнитьПередачуФайлаПриложению(ИмяФайла, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ Указатель.Существует() Тогда
		Комментарий = НСтр("ka='Файл с указанным именем не найден на файловой системе.';ru='Файл с указанным именем не найден на файловой системе.'", Локаль); 
	...
	Если НЕ ЗначениеЗаполнено(КодПолучателя) И НЕ ТипЗнч(КодПолучателя) = Тип("Число") Тогда
		Комментарий = НСтр("ka='Параметр КодПолучателя не содержит валидный код области данных.';ru='Параметр КодПолучателя не содержит валидный код области данных.'", Локаль); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Процедура ВыслатьКвитанциюПередачиФайла(ИдентификаторВызова, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторВызова) ИЛИ НЕ ТипЗнч(ИдентификаторВызова) = Тип("УникальныйИдентификатор") Тогда
		Комментарий = НСтр("ka='Параметр ИдентификаторВызова не содержит валидный идентификатор.';ru='Параметр ИдентификаторВызова не содержит валидный идентификатор.'", Локаль); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();	
	Возврат НСтр("ka='Распределенное выполнение команд';ru='Распределенное выполнение команд'", Локаль); 
