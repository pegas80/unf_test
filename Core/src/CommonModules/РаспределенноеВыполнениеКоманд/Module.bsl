...
Процедура ВыполнитьКомандуДополнительнойОбработки(ИдентификаторОбработки, ИдентификаторКоманды, ИдентификаторОперации) Экспорт
	...
	Событие = Событие + НСтр("ka='Начало выполнения команды';ru='Начало выполнения команды';en='Start command'"); 
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			...
			Событие = ИмяСобытияЖурналаРегистрации() + ".";
			Событие = Событие + НСтр("ka='Завершение выполнения команды';ru='Завершение выполнения команды';en='Completion of the command'"); 
		...
		Исключение
			...
			Событие = НСтр("ka='Ошибка выполнения команды';ru='Ошибка выполнения команды';en='Runtime Error Team'"); 
			Комментарий = НСтр("ka='ბრძანების შესრულებისას წარმოიშვა შეცდომა"
"';ru='При выполнении команды возникла ошибка';en='The command was a mistake'"); 
	...
	Иначе
		ОшибкаВыполнения = Истина;
		Событие = НСтр("ka='Обработка не найдена';ru='Обработка не найдена';en='Treatment Found'"); 
		Комментарий = НСтр("ka='Не удалось найти внешнюю обработку по идентификатору';ru='Не удалось найти внешнюю обработку по идентификатору';en='Unable to find an external processing by ID'"); 
...
Функция ВыполнитьПередачуФайлаПриложению(ИмяФайла, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ Указатель.Существует() Тогда
		Комментарий = НСтр("ka='Файл с указанным именем не найден на файловой системе.';ru='Файл с указанным именем не найден на файловой системе.';en='The file with the specified name is not found on the file system.'", Локаль); 
	...
	Если НЕ ЗначениеЗаполнено(КодПолучателя) И НЕ ТипЗнч(КодПолучателя) = Тип("Число") Тогда
		Комментарий = НСтр("ka='Параметр КодПолучателя не содержит валидный код области данных.';ru='Параметр КодПолучателя не содержит валидный код области данных.';en='KodPoluchatelya parameter does not contain valid data area code.'", Локаль); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Процедура ВыслатьКвитанциюПередачиФайла(ИдентификаторВызова, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторВызова) ИЛИ НЕ ТипЗнч(ИдентификаторВызова) = Тип("УникальныйИдентификатор") Тогда
		Комментарий = НСтр("ka='Параметр ИдентификаторВызова не содержит валидный идентификатор.';ru='Параметр ИдентификаторВызова не содержит валидный идентификатор.';en='IdentifikatorVyzova parameter does not contain a valid ID.'", Локаль); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();	
	Возврат НСтр("ka='Распределенное выполнение команд';ru='Распределенное выполнение команд';en='Distributed command execution'", Локаль); 
