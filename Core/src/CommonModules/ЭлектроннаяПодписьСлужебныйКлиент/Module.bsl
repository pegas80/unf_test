...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ფაილი სერტიფიკატის შესანახად';ru='Выберите файл для сохранения сертификата'");
	Диалог.Фильтр    = НСтр("ka='სერთიფიკატების ფაილები (*.cer)|*.cer|ყველა ფაილი (*.*)|*.*';ru='Файлы сертификатов (*.cer)|*.cer|Все файлы (*.*)|*.*'");
...
Процедура СохранитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ka='სერთიფიკატი შენახულია ფაილში:';ru='Сертификат сохранен в файл:'"),,
...
Процедура СохранитьЗапросНаСертификатПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ფაილი სერთიფიკატის მოთხოვნის შესანახად';ru='Выберите файл для сохранения запроса на сертификат'");
	Диалог.Фильтр    = НСтр("ka='სერთიფიკატების ფაილები (*.p10)|*.p10|ყველა ფაილი (*.*)|*.*';ru='Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*'");
	Диалог.МножественныйВыбор = Ложь;
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		ЕстьПолученныеФайлы = Истина;
		ПоказатьОповещениеПользователя(НСтр("ka='სერთიფიკატის მოთხოვნა შენახულია ფაილში:';ru='Запрос на сертификат сохранен в файл:'"),,
			ПолученныеФайлы[0].Имя);
	КонецЕсли;
...
Процедура СохранитьПодписьПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ელექტრონული ხელმოწერების ფაილები (*.%1)|*.%1|ყველა ფაილი (*.*)|*.*';ru='Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*'"),
		РасширениеДляФайловПодписи);
	
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ფაილი ხელმოწერის შესანახად';ru='Выберите файл для сохранения подписи'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='ელექტრონული ხელმოწერა შენახულია ფაილში.';ru='Электронная подпись сохранена в файл:'"),,
		ПолученныеФайлы[0].Имя);
...
"%1'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა.';ru='Сертификат не найден на компьютере.'");
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ka='ხელმოწერის მონაცემებში სერთიფიკატი ვერ მოიძებნა.';ru='Сертификат не найден в данных подписи.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("ka='(სერვერზე)';ru='(на сервере)'") + Символы.ПС
			+ ОписаниеОшибки + " " + НСтр("ka='(კლიენტზე)';ru='(на клиенте)'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ka='მონაცემთა ხელმოწერა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось подписать данные по причине:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ka='ხელმოწერის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить подпись по причине:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ka='მონაცემთა დაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось зашифровать данные по причине:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ka='მონაცემთა გაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось расшифровать данные по причине:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ka='სერთიფიკატის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить сертификат по причине:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ka='სერთიფიკატების მიღება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить сертификаты по причине:'");
...
"პარამეტრ ოპერაციას არასწორი მნიშვნელობა ""%1"".'"), Операция);
	Иначе
		ЗаголовокОшибки = НСтр("ka='ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось выполнить операцию по причине:'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст)
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ka='არცერთი პროგრამის გამოყენება არ არის გათვალისწინებული.';ru='Не предусмотрено использование ни одной программы.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
		...
		Если Не СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.'");
		КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Программа %1 не доступна или не установлена на компьютере.';ru='Программа %1 не доступна или не установлена на компьютере.'"),
				Контекст.Программа);
		Иначе
			ТекстОшибки = НСтр("ka='Ни одна из программ не доступна или не установлена на компьютере.';ru='Ни одна из программ не доступна или не установлена на компьютере.'");
		КонецЕсли;
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ სერთიფიკატის ფაილი (მხოლოდ დასაშიფრად)';ru='Выберите файл сертификата (только для шифрования)'");
	Диалог.Фильтр = НСтр("ka='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';ru='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ka='სერთიფიკატის ფაილი უნდა იყოს შემდეგ ფორმატში DER X.509,  ოპერაცია შეწყვეტილია.';ru='Файл сертификата должен быть в формате DER X.509, операция прервана.'"));
		Возврат;
	КонецЕсли;
	
	Если Не Форма.Открыта() Тогда
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("ka='გახსნა';ru='Открыть'"));
		Кнопки.Добавить("Отмена",  НСтр("ka='გაუქმება';ru='Отмена'"));
		ПоказатьВопрос(
			...
			НСтр("ka='Сертификат уже добавлен.';ru='Сертификат уже добавлен.'"), Кнопки);
	КонецЕсли;
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Текст = НСтр("ka='Сертификаты, которые могут быть использованы для этой операции ограничены.';ru='Сертификаты, которые могут быть использованы для этой операции ограничены.'");
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("ka='Следующие разрешенные сертификаты доступны для выбора:';ru='Следующие разрешенные сертификаты доступны для выбора:'");
	...
	Если ЗначениеЗаполнено(НедоступныеСертификаты) Тогда
		Если ЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ka='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:'");
			Иначе
				Заголовок = НСтр("ka='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';ru='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ka='Следующие разрешенные сертификаты не найдены на компьютере:';ru='Следующие разрешенные сертификаты не найдены на компьютере:'");
			Иначе
				Заголовок = НСтр("ka='Ни один из следующих разрешенных сертификатов не найден на компьютере:';ru='Ни один из следующих разрешенных сертификатов не найден на компьютере:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура СохранитьДанныеВместеСПодписью(ОписаниеДанных, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ka='მონაცემები';ru='Данные'"));
...
Процедура СохранитьДанныеВместеСПодписьюПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Текст = НСтр("ka='Папка с файлами:';ru='Папка с файлами:'") + Символы.ПС;
	Текст = Текст + Контекст.ПутьКФайлам;
	Текст = Текст + Символы.ПС + Символы.ПС;
	
	Текст = Текст + НСтр("ka='ფაილები:';ru='Файлы:'") + Символы.ПС;
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig Тогда
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция)
					+ Символы.ПС + НСтр("ka='Внешняя компонента XMLDSig может использоваться только для подписания.';ru='Внешняя компонента XMLDSig может использоваться только для подписания.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
		...
		Если Контекст.НаСторонеКлиента Тогда
			...
			Исключение
				...
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
					НСтр("ka='При записи подписи возникла ошибка:';ru='При записи подписи возникла ошибка:'")
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиПодписи(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ka='При записи подписи возникла ошибка:';ru='При записи подписи возникла ошибка:'") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ka='При записи зашифрованных данных возникла ошибка:';ru='При записи зашифрованных данных возникла ошибка:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ka='При записи расшифрованных данных возникла ошибка:';ru='При записи расшифрованных данных возникла ошибка:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	 ...
	 Или ТипЗнч(Результат.Данные) <> Тип("ДвоичныеДанные")
	   ...
	   И (Не ЭтоXMLDSig) Тогда
		...
		Если ТипЗнч(Результат) <> Тип("Структура") Или Не Результат.Свойство("ОписаниеОшибки") Тогда
			Ошибка = Новый Структура("ОписаниеОшибки", НСтр("ka='Некорректный тип данных.';ru='Некорректный тип данных.'"));
		Иначе
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("ka='Передача данных отменена пользователем.';ru='Передача данных отменена пользователем.'"));
	Иначе
	...
	Если Подключено Тогда
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ka='Ошибка инициализации компоненты XMLDSig.';ru='Ошибка инициализации компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Выполнено Тогда
			...
			Иначе
				ЗавершитьОперациюСОшибкой(
					Контекст,
					НСтр("ka='Не установлен режим работы компоненты XMLDSig.';ru='Не установлен режим работы компоненты XMLDSig.'"));
			КонецЕсли;
...
"Перейти к установке данной компоненты?'");
	...
	Иначе
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Операция невозможна. Требуется установка компоненты XMLDSig.';ru='Операция невозможна. Требуется установка компоненты XMLDSig.'"));
...
Процедура ПередУстановкойКомпоненты(РезультатВопроса, Контекст) Экспорт
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.'"));
...
Процедура НачатьПодписаниеСообщенияSOAP(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		...
		ЗавершитьОперациюСОшибкой(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Не удалось определить тип криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1'"), ИнформацияМодуляКриптографии.Имя)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"';ru='При выполнении метода GetSignOIDFromCert произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если OIDАлгоритмаПодписи <> Контекст.ДанныеАлгоритмаПодписания.OIDАлгоритмаПодписи Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';ru='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.'"),
	...
	Иначе
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ka='Ошибка вызова метода C14N компоненты XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"';ru='При выполнении метода C14N произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода Hash компоненты XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"';ru='При выполнении метода Hash произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
"';ru='При выполнении метода C14N произошла ошибка:'"), Контекст);
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода Sign компоненты XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"';ru='При выполнении метода Sign произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
"';ru='При выполнении метода C14N произошла ошибка:'"), Контекст);
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода VerifySign компоненты XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"';ru='При выполнении метода VerifySign произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Подпись не верна (SignatureValue не корректно.';ru='Подпись не верна (SignatureValue не корректно.'"));
...
"';ru='При выполнении метода Hash произошла ошибка:'"), Контекст);
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Подпись не верна (SignatureValue корректно, отличается DigestValue).';ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).'"));
...
Процедура НачатьПолучениеТекстаОшибки(НачалоОписанияТекстаОшибки, Контекст)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода GetLastError компоненты XMLDSig.';ru='Ошибка вызова метода GetLastError компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
