...
"Некорректное значение параметра ДанныеСертификата ""%1"".';ka='შეცდომა საერთო მოდულ ელექტრონულიხელმოწერაკლიენტი-ს პროცედურა სერთიფიკატისგახსნა-ს მოხმობისას: პარამეტრ სერთიფიკატისმონაცემები-ს არაკორექტული მნიშვნელობა ""%1"".'"),
...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ფაილი სერტიფიკატის შესანახად';ru='Выберите файл для сохранения сертификата';en='Select the file to store the certificate'");
	Диалог.Фильтр    = НСтр("ka='სერთიფიკატების ფაილები (*.cer)|*.cer|ყველა ფაილი (*.*)|*.*';ru='Файлы сертификатов (*.cer)|*.cer|Все файлы (*.*)|*.*';en='Certificate Files (* .cer) | * .cer | All files (* *.) | * *.'");
...
Процедура СохранитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ka='სერთიფიკატი შენახულია ფაილში:';ru='Сертификат сохранен в файл:';en='The certificate is stored in the file:'"),,
...
Процедура СохранитьЗапросНаСертификатПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ფაილი სერთიფიკატის მოთხოვნის შესანახად';ru='Выберите файл для сохранения запроса на сертификат';en='Select the file to store the certificate request'");
	Диалог.Фильтр    = НСтр("ka='სერთიფიკატების ფაილები (*.p10)|*.p10|ყველა ფაილი (*.*)|*.*';ru='Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*';en='certificate files (* .p10) | * .p10 | All files (* *.) | * *.'");
	Диалог.МножественныйВыбор = Ложь;
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		ЕстьПолученныеФайлы = Истина;
		ПоказатьОповещениеПользователя(НСтр("ka='სერთიფიკატის მოთხოვნა შენახულია ფაილში:';ru='Запрос на сертификат сохранен в файл:';en='The certificate request is saved in a file:'"),,
			ПолученныеФайлы[0].Имя);
	КонецЕсли;
...
Процедура СохранитьПодписьПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ელექტრონული ხელმოწერების ფაილები (*.%1)|*.%1|ყველა ფაილი (*.*)|*.*';ru='Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*';en='Files electronic signatures (*% 1.) |. *% 1 | All files (* *.) | * *.'"),
		РасширениеДляФайловПодписи);
	
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ფაილი ხელმოწერის შესანახად';ru='Выберите файл для сохранения подписи';en='Select the file to store the captions'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='ელექტრონული ხელმოწერა შენახულია ფაილში.';ru='Электронная подпись сохранена в файл:';en='An electronic signature is stored in the file:'"),,
		ПолученныеФайлы[0].Имя);
...
"%1';ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა შემდეგი მიზეზით:"
"%1'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა.';ru='Сертификат не найден на компьютере.';en='Certificate can not be found on your computer.'");
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ka='ხელმოწერის მონაცემებში სერთიფიკატი ვერ მოიძებნა.';ru='Сертификат не найден в данных подписи.';en='Certificate can not be found in the data label.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("ka='(სერვერზე)';ru='(на сервере)';en='(on server)'") + Символы.ПС
			+ ОписаниеОшибки + " " + НСтр("ka='(კლიენტზე)';ru='(на клиенте)';en='(On the client)'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("ru='Требуется программа электронной подписи и шифрования';en='Requires software electronic signature and encryption'");
	Операция       = Контекст.Операция;
	
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ka='მონაცემთა ხელმოწერა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось подписать данные по причине:';en='Unable to sign the data is due to:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ka='ხელმოწერის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить подпись по причине:';en='Unable to verify the signature for a reason:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ka='მონაცემთა დაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось зашифровать данные по причине:';en='Could not encrypt data due to:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ka='მონაცემთა გაშიფრვა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось расшифровать данные по причине:';en='Unable to decrypt the data because of:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ka='სერთიფიკატის შემოწმება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось проверить сертификат по причине:';en='Unable to verify the certificate due to:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ka='სერთიფიკატების მიღება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить сертификаты по причине:';en='Unable to obtain certificates for a reason:'");
...
"Неверное значение параметра Операция ""%1"".';ka='შეცდომა ფუნქცია კრიპტოგრაფიისმენეჯერი-ში."
"პარამეტრ ოპერაციას არასწორი მნიშვნელობა ""%1"".'"), Операция);
	Иначе
		ЗаголовокОшибки = НСтр("ka='ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось выполнить операцию по причине:';en='The operation failed due to:'");
	КонецЕсли;
	...
	Если Не Подключено Тогда
		ТекстОшибки =
			НСтр("ka='ინტერნეტის მიმომხილველში საჭიროა გაფართოების დაყენება ელექტრონულ ხელმოწერასთან და დაშიფრვასთან სამუშაოდ.';ru='В обозреватель интернет требуется установить расширение"
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст)
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ka='არცერთი პროგრამის გამოყენება არ არის გათვალისწინებული.';ru='Не предусмотрено использование ни одной программы.';en='It is not provided by the use of a single program.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
		...
		Если Не СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.';en='Contact your administrator.'");
		КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Программа %1 не доступна или не установлена на компьютере.';ru='Программа %1 не доступна или не установлена на компьютере.';en='The program% 1 is not available or is not installed on the computer.'"),
				Контекст.Программа);
		Иначе
			ТекстОшибки = НСтр("ka='Ни одна из программ не доступна или не установлена на компьютере.';ru='Ни одна из программ не доступна или не установлена на компьютере.';en='Neither of the programs is not available or is not installed on the computer.'");
		КонецЕсли;
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ სერთიფიკატის ფაილი (მხოლოდ დასაშიფრად)';ru='Выберите файл сертификата (только для шифрования)';en='Select the certificate file (for encryption only)'");
	Диалог.Фильтр = НСтр("ka='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';ru='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';en='Certificate X.509 (* .cer; * crt.) | * .Cer;. * Crt | All files (* *.) | * *.'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ka='სერთიფიკატის ფაილი უნდა იყოს შემდეგ ფორმატში DER X.509,  ოპერაცია შეწყვეტილია.';ru='Файл сертификата должен быть в формате DER X.509, операция прервана.';en='The certificate file must be in DER X.509 format, the operation is aborted.'"));
		Возврат;
	КонецЕсли;
	
	Если Не Форма.Открыта() Тогда
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("ka='გახსნა';ru='Открыть';en='Open'"));
		Кнопки.Добавить("Отмена",  НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
		ПоказатьВопрос(
			...
			НСтр("ka='Сертификат уже добавлен.';ru='Сертификат уже добавлен.';en='The certificate has already been added.'"), Кнопки);
	КонецЕсли;
...
Процедура ПоказатьОшибкуОбращенияКПрограмме(ЗаголовокФормы, ЗаголовокОшибки, ОшибкаНаКлиенте, ОшибкаНаСервере,
	...
	Если ТипЗнч(ОшибкаНаКлиенте) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ka='პროცედურა პროგრამასთანმიმართვისშეცდომისჩვენება-სთვის მითითებულია პარამეტრ კლიენტზეშეცდომა-ს არაკორექტული ტიპი.';ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме"
"указан некорректный тип параметра ОшибкаНаКлиенте.'");
	КонецЕсли;
	
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ka='პროცედურა პროგრამასთანმიმართვისშეცდომისჩვენება-სთვის მითითებულია პარამეტრ სერვერზეშეცდომა-ს არაკორექტული ტიპი.';ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме"
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Текст = НСтр("ka='Сертификаты, которые могут быть использованы для этой операции ограничены.';ru='Сертификаты, которые могут быть использованы для этой операции ограничены.';en='Certificates, which can be used for this operation is limited.'");
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("ka='Следующие разрешенные сертификаты доступны для выбора:';ru='Следующие разрешенные сертификаты доступны для выбора:';en='The following authorized certificates are available for selection:'");
	...
	Если ЗначениеЗаполнено(НедоступныеСертификаты) Тогда
		Если ЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ka='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';en='The following authorized certificates are not found on the computer or on the server:'");
			Иначе
				Заголовок = НСтр("ka='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';ru='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';en='None of these authorized certificates can not be found either on the computer or on the server:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ka='Следующие разрешенные сертификаты не найдены на компьютере:';ru='Следующие разрешенные сертификаты не найдены на компьютере:';en='The following authorized certificates found on your computer:'");
			Иначе
				Заголовок = НСтр("ka='Ни один из следующих разрешенных сертификатов не найден на компьютере:';ru='Ни один из следующих разрешенных сертификатов не найден на компьютере:';en='None of these authorized certificates is not found on the computer:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПодписьИзФайлаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Требуется программа электронной подписи и шифрования';en='Requires software electronic signature and encryption'"),
...
Процедура СохранитьДанныеВместеСПодписью(ОписаниеДанных, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ka='მონაცემები';ru='Данные';en='Data'"));
...
Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи файла возникла ошибка:';en='An error occurred when writing a file:'") + Символы.ПС + Результат.ОписаниеОшибки);
		
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Не удалось сохранить подписи вместе с файлом';en='Could not save signature with the file'"), "", Ошибка, Новый Структура);
		Возврат;
...
Процедура СохранитьДанныеВместеСПодписьюПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Текст = НСтр("ka='Папка с файлами:';ru='Папка с файлами:';en='File Folder:'") + Символы.ПС;
	Текст = Текст + Контекст.ПутьКФайлам;
	Текст = Текст + Символы.ПС + Символы.ПС;
	
	Текст = Текст + НСтр("ka='ფაილები:';ru='Файлы:';en='files:'") + Символы.ПС;
...
"необходимо установить расширение для работы с файлами.';ka='Перед установкой расширения для работы с электронной подписью и шифрованием"
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig Тогда
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция)
					+ Символы.ПС + НСтр("ka='Внешняя компонента XMLDSig может использоваться только для подписания.';ru='Внешняя компонента XMLDSig может использоваться только для подписания.';en='External XMLDSig component can only be used for signing.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
		...
		Если Контекст.НаСторонеКлиента Тогда
			...
			Исключение
				...
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
					НСтр("ka='При записи подписи возникла ошибка:';ru='При записи подписи возникла ошибка:';en='When recording a signature error occurred:'")
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиПодписи(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ka='При записи подписи возникла ошибка:';ru='При записи подписи возникла ошибка:';en='When recording a signature error occurred:'") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ka='При записи зашифрованных данных возникла ошибка:';ru='При записи зашифрованных данных возникла ошибка:';en='When writing the encrypted data error occurred:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ka='При записи расшифрованных данных возникла ошибка:';ru='При записи расшифрованных данных возникла ошибка:';en='decrypted data error occurred when writing:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	 ...
	 Или ТипЗнч(Результат.Данные) <> Тип("ДвоичныеДанные")
	   ...
	   И (Не ЭтоXMLDSig) Тогда
		...
		Если ТипЗнч(Результат) <> Тип("Структура") Или Не Результат.Свойство("ОписаниеОшибки") Тогда
			Ошибка = Новый Структура("ОписаниеОшибки", НСтр("ka='Некорректный тип данных.';ru='Некорректный тип данных.';en='Incorrect data type.'"));
		Иначе
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("ka='Передача данных отменена пользователем.';ru='Передача данных отменена пользователем.';en='Data transfer canceled by the user.'"));
	Иначе
	...
	Если Подключено Тогда
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ka='Ошибка инициализации компоненты XMLDSig.';ru='Ошибка инициализации компоненты XMLDSig.';en='Error initializing XMLDSig components.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Выполнено Тогда
			...
			Иначе
				ЗавершитьОперациюСОшибкой(
					Контекст,
					НСтр("ka='Не установлен режим работы компоненты XMLDSig.';ru='Не установлен режим работы компоненты XMLDSig.';en='It is not set to work XMLDSig components.'"));
			КонецЕсли;
...
"Перейти к установке данной компоненты?';ka='Для подписания XML необходима установка компоненты XMLDSig."
"Перейти к установке данной компоненты?'");
	...
	Иначе
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Операция невозможна. Требуется установка компоненты XMLDSig.';ru='Операция невозможна. Требуется установка компоненты XMLDSig.';en='Operation is not possible. Requires installation XMLDSig components.'"));
...
Процедура ПередУстановкойКомпоненты(РезультатВопроса, Контекст) Экспорт
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';en='Operation is not possible. Requires installation of components for HTTP-requests.'"));
...
Процедура НачатьПодписаниеСообщенияSOAP(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		...
		ЗавершитьОперациюСОшибкой(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Не удалось определить тип криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1';en='Unable to determine the type of CSP 1%'"), ИнформацияМодуляКриптографии.Имя)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';en='Error calling the GetSignOIDFromCert components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияGetSignOIDFromCert(OIDАлгоритмаПодписи, Параметры, Контекст) Экспорт
	...
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ka='GetSignOIDFromCert მეთოდის შესრულებისას მოხდა შეცდომა:"
"';ru='При выполнении метода GetSignOIDFromCert произошла ошибка:';en='An error occurred while performing GetSignOIDFromCert method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если OIDАлгоритмаПодписи <> Контекст.ДанныеАлгоритмаПодписания.OIDАлгоритмаПодписи Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';ru='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';en='Certificate signature cryptography algorithm differs from the algorithm 1%.'"),
	...
	Иначе
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ka='Ошибка вызова метода C14N компоненты XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.';en='Error calling the C14N components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияC14N_ПодписываемыйТег(КаноникализированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КаноникализированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ka='C14N მეთოდის შესრულებისას მოხდა შეცდომა:"
"';ru='При выполнении метода C14N произошла ошибка:';en='In carrying out the method C14N error occurred:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода Hash компоненты XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.';en='Error calling the Hash components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияHash_ПодписываемыйТег(DigestValue, Параметры, Контекст) Экспорт
	...
	Если DigestValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ka='Hash მეთოდის შესრულებისას მოხდა შეცდომა:"
"';ru='При выполнении метода Hash произошла ошибка:';en='When the Hash method, an error occurred:'"), Контекст);
		Возврат;
	КонецЕсли;
...
"';ru='При выполнении метода C14N произошла ошибка:';en='In carrying out the method C14N error occurred:'"), Контекст);
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода Sign компоненты XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.';en='Error calling the ingredients Sign XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияSign(SignatureValue, Параметры, Контекст) Экспорт
	...
	Если SignatureValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ka='Sign  მეთოდის შესრულებისას მოხდა შეცდომა:"
"';ru='При выполнении метода Sign произошла ошибка:';en='When the Sign method of error:'"), Контекст);
		Возврат;
	КонецЕсли;
...
"';ru='При выполнении метода C14N произошла ошибка:';en='In carrying out the method C14N error occurred:'"), Контекст);
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода VerifySign компоненты XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';en='Error calling the VerifySign components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ka='VerifySign მეთოდის შესრულებისას მოხდა შეცდომა:"
"';ru='При выполнении метода VerifySign произошла ошибка:';en='An error occurred while performing VerifySign method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Подпись не верна (SignatureValue не корректно.';ru='Подпись не верна (SignatureValue не корректно.';en='Signature is not true (SignatureValue not correct.'"));
...
"';ru='При выполнении метода Hash произошла ошибка:';en='When the Hash method, an error occurred:'"), Контекст);
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Подпись не верна (SignatureValue корректно, отличается DigestValue).';ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';en='Signature is not true (SignatureValue correctly characterized DigestValue).'"));
...
Процедура НачатьПолучениеТекстаОшибки(НачалоОписанияТекстаОшибки, Контекст)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ka='Ошибка вызова метода GetLastError компоненты XMLDSig.';ru='Ошибка вызова метода GetLastError компоненты XMLDSig.';en='Error calling the GetLastError components XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
