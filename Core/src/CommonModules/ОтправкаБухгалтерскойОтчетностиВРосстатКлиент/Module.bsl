...
"годовой бухгалтерской отчетности в Росстат.';ka='В соответствии со ст. 18 Федерального закона ""О бухгалтерском учете"" "
"№ 402-ФЗ от 06.12.2011 организации, обязанные составлять "
"бухгалтерскую (финансовую) отчетность, обязаны представлять экземпляр "
	...
	Если Не ЗначениеЗаполнено(ОтчетностьВРосстат) Тогда
		...
		Если ЗначениеЗаполнено(ИмяОтчетаРегламентированныйОтчетСтатистикаБухОтчетность) Тогда
			...
			Исключение
				...
				Сообщение.Текст = НСтр("ka='Не удалось открыть форму отчета ""Бухгалтерская отчетность в Росстат"".';ru='Не удалось открыть форму отчета ""Бухгалтерская отчетность в Росстат"".';en='Unable to open the report, ""Financial Reporting in Rosstat"".'");
				Сообщение.Сообщить();

			КонецПопытки;
		Иначе
			ТекстСообщения = НСтр("ka='Форма отчета ""Бухгалтерская отчетность в Росстат"" не поддерживается.';ru='Форма отчета ""Бухгалтерская отчетность в Росстат"" не поддерживается.';en='The form of the report ""Financial Reporting in Rosstat"" is not supported.'");
...
""
"Подготовить отчет для отправки в Росстат сейчас?';ka='В соответствии со ст. 18 Федерального закона ""О бухгалтерском учете"" "
"№ 402-ФЗ от 06.12.2011 данный отчет должен быть отправлен и в Росстат. "
""
"Подготовить отчет для отправки в Росстат сейчас?'");
	...
	ЗаголовокВопроса = НСтр("ka='Отправка отчета в Росстат';ru='Отправка отчета в Росстат';en='Sending the report to Rosstat'");
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("ПодготовитьСейчас", НСтр("ka='მომზადება ახლავე';ru='Подготовить сейчас';en='prepare now!'"));
	Кнопки.Добавить("Позже", НСтр("ka='მოგვიანებით';ru='Позже';en='Later'"));
