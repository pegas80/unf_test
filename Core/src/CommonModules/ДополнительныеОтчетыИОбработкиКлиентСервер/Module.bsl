...
Функция ФильтрДиалоговВыбораИСохранения() Экспорт
	...
	Фильтр = НСтр("ka='გარე ანგარიშგებები და დამუშავებები (*.%1, *.%2)|*.%1;*.%2|გარე ანგარიშგებები (*.%1)|*.%1|გარე დამუშავებები (*.%2)|*.%2';ru='Внешние отчеты и обработки (*.%1, *.%2)|*.%1;*.%2|Внешние отчеты (*.%1)|*.%1|Внешние обработки (*.%2)|*.%2';en='External reporting and processing (*% 1 *% 2..) | *% 1; *.% 2 | external reports (*% 1.) | *% 1 | External processing (*% 2.) | *.. .% 2'");
...
Функция НаименованиеПодсистемы(ДляПользователя) Экспорт
	...
	Возврат НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები';ru='Дополнительные отчеты и обработки';en='Advanced reporting and processing'", КодЯзыка);
КонецФункции
