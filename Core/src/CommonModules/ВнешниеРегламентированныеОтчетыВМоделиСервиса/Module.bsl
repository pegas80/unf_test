...
Процедура ОбработатьВнРеглОтчет(Дескриптор, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ka='Не удалось распаковать архив с поставляемыми данными.';ru='Не удалось распаковать архив с поставляемыми данными.'");
	...
	Если НЕ Выборка.Следующий() Тогда
		ТекстОшибки = НСтр("ka='Для внешнего отчета не найден соответствующий элемент.';ru='Для внешнего отчета не найден соответствующий элемент.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Внешний отчет ""%1"" вер. ""%2"" отключен.';ru='Внешний отчет ""%1"" вер. ""%2"" отключен.'"), Имя, СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору конфигурации в текущем подключенном отчете';ru='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору конфигурации в текущем подключенном отчете'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие версии в отчете версии в текущем подключенном отчете';ru='Не пройдена проверка на соответствие версии в отчете версии в текущем подключенном отчете'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойстваОтчета["ИсточникОтчета"] <> СвойстваКонфигурации["ИсточникОтчета"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на имени метаданных отчета имени метаданных текущего подключенного отчета';ru='Не пройдена проверка на имени метаданных отчета имени метаданных текущего подключенного отчета'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ka='მიწოდებული მონაცემები. გარე რეგლამენტირებული მონაცემების ჩატვირთვა';ru='Поставляемые данные.Загрузка внешних регламентированных отчетов'");
