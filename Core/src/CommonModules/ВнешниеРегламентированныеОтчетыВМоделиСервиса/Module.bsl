...
Процедура ОбработатьВнРеглОтчет(Дескриптор, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ka='Не удалось распаковать архив с поставляемыми данными.';ru='Не удалось распаковать архив с поставляемыми данными.';en='Unable to unzip the archive with the supplied data.'");
	...
	Если НЕ Выборка.Следующий() Тогда
		ТекстОшибки = НСтр("ka='Для внешнего отчета не найден соответствующий элемент.';ru='Для внешнего отчета не найден соответствующий элемент.';en='For external report is not found the appropriate item.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Внешний отчет ""%1"" вер. ""%2"" отключен.';ru='Внешний отчет ""%1"" вер. ""%2"" отключен.';en='External Report ""%1"" ver. ""%2"" is disabled.'"), Имя, СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору конфигурации в текущем подключенном отчете';ru='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору конфигурации в текущем подключенном отчете';en='Do not pass inspection for compliance with configuration identifier in the identifier configuration report in the current report attached'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ka='Не пройдена проверка на соответствие версии в отчете версии в текущем подключенном отчете';ru='Не пройдена проверка на соответствие версии в отчете версии в текущем подключенном отчете';en='Not passed compliance test version of the report in the current version of the attached report'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойстваОтчета["ИсточникОтчета"] <> СвойстваКонфигурации["ИсточникОтчета"] Тогда
		ТекстОшибки = НСтр("ka='Не пройдена проверка на имени метаданных отчета имени метаданных текущего подключенного отчета';ru='Не пройдена проверка на имени метаданных отчета имени метаданных текущего подключенного отчета';en='Not passed check on behalf of the metadata of the report name of the current metadata attached report'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
"Описание:"
"""%1""';ka='Получение объекта отчета по ссылке из выборки."
"Описание:"
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ka='მიწოდებული მონაცემები. გარე რეგლამენტირებული მონაცემების ჩატვირთვა';ru='Поставляемые данные.Загрузка внешних регламентированных отчетов';en='Supplied dannye.Zagruzka external regulated reports'");
