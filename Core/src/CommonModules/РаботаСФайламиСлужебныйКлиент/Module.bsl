...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Если Не ПараметрыВыполнения.Свойство("ПолноеИмяФайла") Тогда
		...
		ДиалогВыбораФайла.Заголовок = НСтр("ka='ფაილის არჩევა';ru='Выбор файла'");
		ДиалогВыбораФайла.Фильтр = НСтр("ka='ყველა ფაილი (*.*)|*.*';ru='Все файлы (*.*)|*.*'");
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("ka='ფაილი განთავისუფლებულია';ru='Файл освобожден'"),
...
Процедура ПеренестиФайлыВПапку(ОбъектыСсылка, Папка) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='ფაილის გადატანა';ru='Перенос файла'"),
...
"обновлен и освобожден.';ka='ფაილი ""%1"" განახლებულია და გათავისუფლებულია.'");
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='რედაქტირება დასრულებულია';ru='Редактирование закончено'"),
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='ფაილის დაშიფრვა';ru='Шифрование файла'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ka='ფაილი';ru='Файл'"));
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
		...
		#Если Не ВебКлиент Тогда
			Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоБазоваяВерсияКонфигурации Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მოცემულ ბრძანებას საბაზო ვერსიაში მხარდაჭერა არ გააჩნია.';ru='Данная команда не поддерживается в базовой версии.'"));
				ВернутьРезультат(ОбработчикРезультата, Ложь);
				Возврат;
			КонецЕсли;
		#КонецЕсли
	...
	Диалог.Заголовок                   = НСтр("ka='ფაილის არჩევა';ru='Выбор файла'");
	...
	Если ЗначениеЗаполнено(РасширениеДляЗашифрованныхФайлов) Тогда
		Фильтр = НСтр("ka='ფაილი (*.%1)|*.%1|დაშიფრული ფაილი (*.%2)|*.%2|ყველა ფაილი (*.*)|*.*';ru='Файл (*.%1)|*.%1|Зашифрованный файл (*.%2)|*.%2|Все файлы (*.*)|*.*'");
...
Процедура ОбновитьИзФайлаНаДискеПередРасшифровкой(ПомещаемыеФайлы, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",              НСтр("ka='ფაილის გაშიფრვა';ru='Расшифровка файла'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",       НСтр("ka='ფაილი';ru='Файл'"));
...
Процедура ЗанятьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ფაილის რედაქტირება';ru='Редактирование файла'"),
		ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1"""
...
Процедура ЗанятьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ფაილების დაკავება';ru='Занять файлы'"),
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილები (%2-დან %1) დაკავებულია რედაქტირებისთვის.';ru='Файлы (%1 из %2) заняты для редактирования.'"),
			КоличествоЗанятых,
...
Процедура ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ფაილების რედაქტირების გაუქმება';ru='Отменить редактирование файлов'"),,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილების რედაქტირება გაუქმებულია (%1 %2-დან).';ru='Отменено редактирование файлов (%1 из %2).'"),
...
Процедура ОсвободитьФайлПослеОтветаНаВопросОтменаРедактирования(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПродолжитьРаботу Тогда
		...
		Если Не ПараметрыВыполнения.НеЗадаватьВопрос Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='ფაილი განთავისუფლებულია';ru='Файл освобожден'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
...
"Пожалуйста, подождите.'"),
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ВерсияОбновлена Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='ახალი ვერსია შენახულია';ru='Новая версия сохранена'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии,
				БиблиотекаКартинок.Информация32);
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("ka='ახალი ვერსია არ არის შენახული';ru='Новая версия не сохранена'"),,
				НСтр("ka='ფაილი არ შეცვლილა';ru='Файл не изменился'"),
				БиблиотекаКартинок.Информация32);
...
Процедура КаталогФайла(ОбработчикРезультата, ДанныеФайла) Экспорт
	...
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Да, НСтр("ka='კატალოგის შენახვა და გახსნა';ru='Сохранить и открыть каталог'"));
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='გაუქმება';ru='Отмена'"));
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	#Если ВебКлиент Тогда
		...
		ВернутьРезультатПослеПоказаПредупреждения(
			ОбработчикРезультата,
			НСтр("ka='სამუშაო კატალოგის გასუფთავება ვებ-კლიენტში არ არის მხარდაჭერილი.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.'"),
			Неопределено);
		Возврат;
	#КонецЕсли
...
Процедура ОчиститьРабочийКаталог(ОбработчикРезультата, РазмерФайловВРабочемКаталоге, РазмерДобавляемогоФайла, ОчищатьВсе) Экспорт
	...
	#Если ВебКлиент Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("ka='სამუშაო კატალოგის გასუფთავება ვებ-კლიენტში არ არის მხარდაჭერილი.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.'"), Неопределено);
		Возврат;
	#КонецЕсли
...
"Удалить его из рабочего каталога?'"),
			...
			Кнопки.Добавить("Да",         НСтр("ka='დიახ';ru='Да'"));
			Кнопки.Добавить("ДаДляВсех",  НСтр("ka='დიახ ყველასთვის';ru='Да для всех'"));
			Кнопки.Добавить("Нет",        НСтр("ka='არა';ru='Нет'"));
			Кнопки.Добавить("НетДляВсех", НСтр("ka='არა ყველასთვის';ru='Нет для всех'"));
	...
	Если ПараметрыВыполнения.НомерШага = 2 Тогда
		...
		ФайлНаДиске.УстановитьТолькоЧтение(Ложь);
		Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
			ШапкаВопроса = НСтр("ka='ხორციელდება სამუშაო კატალოგის გაწმენდა ფაილის დამატებით.';ru='Выполняется очистка рабочего каталога при добавлении файла.'");
		Иначе
			ШапкаВопроса = НСтр("ka='მიმდინარეობს სამუშაო კატალოგის გასუფთავება.';ru='Выполняется очистка рабочего каталога.'");
		КонецЕсли;
...
"%2'"),
					...
					ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
						+ НСтр("ka='ფაილის სახელი შეცვალეთ უფრო მოკლე სახელით.';ru='Измените имя файла на более короткое.'");
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	Заголовок = НСтр("ka='აირჩიეთ სხვა ძირითადი სამუშაო კატალოგი';ru='Выберите другой основной рабочий каталог'");
...
"""%2"".'"),
	...
	Если СтрДлина(ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
			+ НСтр("ka='შეცვალეთ საქაღალდეების სახელები ან მიმდინარე საქაღალდე გადაიტანეთ სხვა საქაღალდეში.';ru='Измените имена папок или перенесите текущую папку в другую папку.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	// Выбираем другой путь к рабочему каталогу.
	Заголовок = НСтр("ka='აირჩიეთ სხვა სამუშაო კატალოგი';ru='Выберите другой рабочий каталог'");
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, НСтр("ka='დასამატებლად ფაილები არ არის';ru='Нет файлов для добавления'"), Неопределено);
		Возврат;
	КонецЕсли;
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილების იმპორტი დასრულებულია. იმპორტირებულია ფაილი: %1';ru='Импорт файлов завершен. Импортировано файлов: %1'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1'"), Расширение);
...
"Пожалуйста, подождите.'"),
	...
	Состояние(НСтр("ka='ფაილი წარმატებით იქნა შებახული';ru='Файл успешно сохранен'"), , ПолноеИмяФайла);
...
"Пожалуйста, подождите.'"),
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%2 მბ)';ru='%1 (%2 МБ)'"), Строка(Файл), Строка(РазмерФайлаВМб));
			ФайлыБольшие.Добавить(ТекстСтроки);
		КонецЦикла;
		
		Параметры.Вставить("ФайлыБольшие", ФайлыБольшие);
		Параметры.Вставить("Заголовок", НСтр("ka='გაფრთხილება ფაილების ჩატვირთვის დროს';ru='Предупреждение при загрузке файлов'"));
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		ТекстПредупреждения = НСтр("ka='ფაილი ""%1"" უკვე დაშიფრულია.';ru='Файл ""%1"" уже зашифрован.'");
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Редактирует) Тогда
		ТекстПредупреждения = НСтр("ka='დაკავებული ფაილის დაშიფრვა შეუძლებელია.';ru='Нельзя зашифровать занятый файл.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстПредупреждения, ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ვერსიები: %1';ru='Версий: %1'"), ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	Если ПустаяСтрока(СтруктураПути.Расширение) Тогда
		ТекстВопроса = НСтр("ka='აუცილებელია ფაილის მითითება გაფართოებითურთ.';ru='Необходимо указать файл с расширением.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Повторить, НСтр("ka='სხვა ფაილის არჩევა';ru='Выбрать другой файл'"));
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления,
	...
	МодульЭлектроннаяПодписьКлиент.ИнформироватьОШифрованииОбъекта(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილი: %1';ru='Файл: %1'"), ФайлСсылка));
...
Процедура ПодписатьФайл(МассивФайлов, ИдентификаторФормы, ОбработчикЗавершения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='ფაილზე ხელმოწერა';ru='Подписание файла'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ka='ფაილი';ru='Файл'"));
	ОписаниеДанных.Вставить("НаборДанных",         НаборДанных);
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru='Файлы (%1)'"));
...
Процедура ИмпортФайловРекурсивноБезДиалогов(Знач Владелец, Знач ВыбранныеФайлы, Знач ПараметрыВыполнения, Знач ЗадаватьВопросПапкаУжеСуществует)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если Не ВыбранныйФайл.Существует() Тогда
			...
			Запись.Вставить("Ошибка", НСтр("ka='დისკზე არ არის ფაილი.';ru='Файл отсутствует на диске.'"));
		...
		Попытка
			...
			НадписьПодробнее = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მუშავდება ფაილი ""%1"" (%2 მბ)...';ru='Обрабатывается файл ""%1"" (%2 Мб)...'"),
				ВыбранныйФайл.Имя, 
				ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
				
			ТекстСостояния = НСтр("ka='ფაილების იმპორტი დისკიდან';ru='Импорт файлов с диска...'");
	...
	Иначе
		...
		Если ОбщееКоличество > 0 Тогда 
			...
			ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ფაილები (%1 ც.) გადატანილია %2-ში';ru='Файлы (%1 шт) перенесены в %2'"),
				ОбщееКоличество,
				ВладелецФайла);
			
			ПоказатьОповещениеПользователя(
				НСтр("ka='ფაილები გადატანილია';ru='Файлы перенесены'"),
				,
				ПолноеОписание,
				БиблиотекаКартинок.Информация32);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеВопрос",      НСтр("ka='დავამთავროთ პროგრამასთან მუშაობა?';ru='Завершить работу с программой?'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок",   НСтр("ka='შემდეგი ფაილები თქვენს მიერ დაკავებულია რედაქტირებისთვის';ru='Следующие файлы заняты вами для редактирования:'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("Заголовок",            НСтр("ka='მუშაობის დამთავრება';ru='Завершение работы'"));
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='რედაქტირებადი ფაილების სიის გახსნა (%1)';ru='Открыть список редактируемых файлов (%1)'"),
