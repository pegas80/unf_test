...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Если Не ПараметрыВыполнения.Свойство("ПолноеИмяФайла") Тогда
		...
		ДиалогВыбораФайла.Заголовок = НСтр("ka='ფაილის არჩევა';ru='Выбор файла';en='file selection'");
		ДиалогВыбораФайла.Фильтр = НСтр("ka='ყველა ფაილი (*.*)|*.*';ru='Все файлы (*.*)|*.*';en='All files (* *.) | * *.'");
	...
	ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='მიმდინარეობს ფაილ ""%1""-ის (%2 მბ) შენახვა. გთხოვთ, მოიცადოთ...';ru='Идет сохранение файла ""%1"" (%2 Мб)."
"Пожалуйста, подождите...'"),
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='შექმნა:';ru='Создание:';en='Creature:'"),
...
"т.к. он никем не занят.';ka='ფაილ ""%1""-ის გათავისუფლება შეუძლებელია, ვინაიდან იგი არავის აქვს დაკავებული.'"),
			Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		...
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно освободить файл ""%1"","
"т.к. он занят пользователем ""%2"".';ka='ფაილ ""%1""-ის განთავისუფლება შეუძლებელია, ვინაიდან იგი დაკავებულია მომხმარებელ ""%2""-ის მიერ.'"),
			Строка(ОбъектСсылка),
			Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("ka='ფაილი განთავისუფლებულია';ru='Файл освобожден';en='released file'"),
...
Процедура ПеренестиФайлыВПапку(ОбъектыСсылка, Папка) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='ფაილის გადატანა';ru='Перенос файла';en='file Transfer'"),
			ДанныеФайла.НавигационнаяСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл ""%1"""
"перенесен в папку ""%2"".';ka='Файл ""%1"""
"перенесен в папку ""%2"".'"),
...
""
"Освободить файл?';ka='ვერ მოხერხდა ფაილ ""%1"" (%2)-ის მოთავსება ფაილების საცავში, ვინაიდან იგი ვერ მოიძებნა სამუშაო კატალოგში."
""
"გავათავისუფლოთ ფაილი?'"),
...
"в хранилище файлов, т.к. он не найден в рабочем каталоге."
""
"Освободить файл?';ka='ფაილების საცავში ვერ მოხერხდა ფაილ ""%1""-ის მოთავსება, ვინაიდან იგი სამუშაო კატალოგში ვერ მოიძებნა."
""
"გავათავისუფლოთ ფაილი?'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
...
"возможно он заблокирован другой программой.';ka='ფაილების საცავში ფაილ ""%1""-ის მოთავსება ვერ მოხერხდა, შეიძლება, რომ იგი ბლოკირებულია სხვა პროგრამის მიერ.'");
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ხორციელდება ფაილ ""%1""-ის  (%2 მბ) გადაცემა..."
"გთხოვთ, მოიცადოთ.';ru='Выполняется передача файла ""%1"" (%2 Мб)..."
"Пожалуйста, подождите.'"),
...
"""%1""."
""
"Повторить операцию?';ka='ფაილის მოთავსება ფაილების საცავში ვერ მოხერხდა შემდეგი მიზეზით:"
"""%1""."
""
"გავიმეოროთ ოპერაცია?'"),
...
"обновлен и освобожден.';ka='ფაილი ""%1"" განახლებულია და გათავისუფლებულია.'");
		Иначе
			ШаблонПояснения = НСтр("ru = 'Файл ""%1""
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='რედაქტირება დასრულებულია';ru='Редактирование закончено';en='editing completed'"),
...
"""%2""."
""
"Повторить операцию?';ka='ფაილ ""%1""-ის მოთავსება ფაილების საცავში ვერ მოხერხდა ""%2""-ის მიზეზით."
""
"გავიმეოროთ ოპერაცია?'"),
		Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='ფაილის დაშიფრვა';ru='Шифрование файла';en='Encrypt file'"));
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
		...
		#Если Не ВебКлиент Тогда
			Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоБазоваяВерсияКонфигурации Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მოცემულ ბრძანებას საბაზო ვერსიაში მხარდაჭერა არ გააჩნია.';ru='Данная команда не поддерживается в базовой версии.';en='This command is not supported in the base version.'"));
				ВернутьРезультат(ОбработчикРезультата, Ложь);
				Возврат;
			КонецЕсли;
		#КонецЕсли
	...
	Диалог.Заголовок                   = НСтр("ka='ფაილის არჩევა';ru='Выбор файла';en='file selection'");
	...
	Если ЗначениеЗаполнено(РасширениеДляЗашифрованныхФайлов) Тогда
		Фильтр = НСтр("ka='ფაილი (*.%1)|*.%1|დაშიფრული ფაილი (*.%2)|*.%2|ყველა ფაილი (*.*)|*.*';ru='Файл (*.%1)|*.%1|Зашифрованный файл (*.%2)|*.%2|Все файлы (*.*)|*.*';en='File (*% 1.) |. *% 1 | encrypted file (*% 2.) |. *% 2 | All files (* *.) | * *.'");
...
Процедура ОбновитьИзФайлаНаДискеПередРасшифровкой(ПомещаемыеФайлы, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",              НСтр("ka='ფაილის გაშიფრვა';ru='Расшифровка файла';en='decrypt files'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",       НСтр("ka='ფაილი';ru='Файл';en='File'"));
...
""
"Операция прервана.';ka='ფაილ ""%1""-ს ფაილების საცავში გააჩნია ცვლილების უფრო გვიანდელი თარიღი (%2), ვიდრე არჩეულ ფაილს (%3)."
""
...
"чем выбранный файл (%3)."
""
"Операция прервана.';ka='ფაილ ""%1""-ს სამუშაო კატალოგში გააჩნია ცვლილების უფრო გვიანდელი თარიღი (%2), ვიდრე არჩეულ ფაილს (%3). "
"ოპერაცია შეწყვეტილია.'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
"обновления из файла на диске.';ka='ფაილი ""%1"""
"სამუშაო კატალოგში გახსნილია რედაქტირებისთვის."
""
"ფაილიდან დისკზე განახლების შესრულებამდე დაასრულეთ რედაქტირება.'"),
						ПолноеИмяФайла);
...
Процедура ЗанятьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ფაილის რედაქტირება';ru='Редактирование файла';en='Edit file'"),
		ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1"""
"занят для редактирования.';ka='ფაილი ""%1"" დაკავებულია რედაქტირებისთვის.'"), Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка)),
		БиблиотекаКартинок.Информация32);
...
Процедура ЗанятьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ფაილების დაკავება';ru='Занять файлы';en='take files'"),
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილები (%2-დან %1) დაკავებულია რედაქტირებისთვის.';ru='Файлы (%1 из %2) заняты для редактирования.';en='Files (% 1% 2) employed for editing.'"),
			КоличествоЗанятых,
...
""
"Продолжить?';ka='ფაილების რედაქტირების გაუქმებამ შეიძლება გამოიწვიოს თქვენი ცვლილებების დაკარგვა. გავაგრძელოთ?'"),
...
Процедура ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ფაილების რედაქტირების გაუქმება';ru='Отменить редактирование файлов';en='Cancel editing files'"),,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილების რედაქტირება გაუქმებულია (%1 %2-დან).';ru='Отменено редактирование файлов (%1 из %2).';en='Canceled editing files (% 1% 2).'"),
...
""
"Продолжить?';ka='ფაილ ""%1""-ის რედაქტირების გაუქმებამ შეიძლება გამოიწვიოს თქვენი ცვლილებების დაკარგვა. გავაგრძელოთ?'"),
			Строка(ПараметрыВыполнения.ОбъектСсылка));
...
Процедура ОсвободитьФайлПослеОтветаНаВопросОтменаРедактирования(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПродолжитьРаботу Тогда
		...
		Если Не ПараметрыВыполнения.НеЗадаватьВопрос Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='ფაილი განთავისუფლებულია';ru='Файл освобожден';en='released file'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
...
"%2."
""
"Освободить файл?';ka='ფაილების საცავში ფაილ ""%1""-ის მოთავსება ვერ მოხერხდა:"
"სამუშაო კატალოგში ფაილი ვერ მოიძებნა:"
"%2."
""
"გავათავისუფლოთ ფაილი?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
"Пожалуйста, подождите.'"),
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ВерсияОбновлена Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='ახალი ვერსია შენახულია';ru='Новая версия сохранена';en='The new version is saved'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии,
				БиблиотекаКартинок.Информация32);
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("ka='ახალი ვერსია არ არის შენახული';ru='Новая версия не сохранена';en='The new version is not stored'"),,
				НСтр("ka='ფაილი არ შეცვლილა';ru='Файл не изменился';en='The file has not changed'"),
				БиблиотекаКартинок.Информация32);
...
"Расшифруйте файл и зашифруйте заново.';ka='დაშიფრულ ფაილს სერთიფიკატები არ აქვს მითითებული."
"გაშიფრეთ ფაილი და ხელახლა დაშიფრეთ.'"));
...
Процедура ЗанятьФайлПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1"""
"занят для редактирования.';ka='ფაილი ""%1"" დაკავებულია რედაქტირებისთვის.'"),
			Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка)),
		БиблиотекаКартинок.Информация32);
...
"из рабочего каталога?';ka='სამუშაო კატალოგიდან წავშალოთ ფაილი ""%1""?'");
...
"в рабочий каталог.';ka='ფაილების საცავიდან სამუშაო კატალოგში ფაილის მიღების შეცდომა.'"));
...
Процедура КаталогФайла(ОбработчикРезультата, ДанныеФайла) Экспорт
	...
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Да, НСтр("ka='კატალოგის შენახვა და გახსნა';ru='Сохранить и открыть каталог';en='Save and open the directory'"));
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
	ПоказатьВопрос(Обработчик,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილის კატალოგი არ არსებობს. შეიძლება, რომ მოცემულ კომპიუტერზე ფაილი ""%1"" ჯერ არ გახსნილა. შევინახოთ ფაილი კომპიუტერზე და გავხსნათ მისი კატალოგი?';ru='Каталог файла не существует. Возможно, на данном компьютере файл ""%1"" еще не открывался."
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	#Если ВебКлиент Тогда
		...
		ВернутьРезультатПослеПоказаПредупреждения(
			ОбработчикРезультата,
			НСтр("ka='სამუშაო კატალოგის გასუფთავება ვებ-კლიენტში არ არის მხარდაჭერილი.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';en='Cleaning the working directory is not supported by the Web client.'"),
			Неопределено);
		Возврат;
	#КонецЕсли
...
Процедура ОчиститьРабочийКаталог(ОбработчикРезультата, РазмерФайловВРабочемКаталоге, РазмерДобавляемогоФайла, ОчищатьВсе) Экспорт
	...
	#Если ВебКлиент Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("ka='სამუშაო კატალოგის გასუფთავება ვებ-კლიენტში არ არის მხარდაჭერილი.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';en='Cleaning the working directory is not supported by the Web client.'"), Неопределено);
		Возврат;
	#КонецЕсли
	...
	Если ПараметрыВыполнения.НомерШага = 1 Тогда
		Если Не ПараметрыВыполнения.ДаДляВсех
			...
			Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მიმდინარეობს სამუშაო კატალოგის გასუფთავება ფაილის დამატებისას."
""
"ფაილების საცავში ვერ მოიძებნა ფაილი"
"""%1""."
""
"წავშალოთ იგი სამუშაო კატალოგიდან?';ru='Выполняется очистка рабочего каталога при добавлении файла."
""
...
"Удалить его из рабочего каталога?'"),
			...
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ფაილების საცავში ვერ მოიძებნა ფაილი"
"""%1""."
""
"წავშალოთ იგი სამუშაო კატალოგიდან?';ru='В хранилище файлов не найден файл"
"""%1""."
""
"Удалить его из рабочего каталога?'"),
					ПараметрыВыполнения.ИмяКаталога + ПараметрыВыполнения.Элемент.Путь);
			...
			Кнопки.Добавить("Да",         НСтр("ka='დიახ';ru='Да';en='Yes'"));
			Кнопки.Добавить("ДаДляВсех",  НСтр("ka='დიახ ყველასთვის';ru='Да для всех';en='Yes to all'"));
			Кнопки.Добавить("Нет",        НСтр("ka='არა';ru='Нет';en='Not'"));
			Кнопки.Добавить("НетДляВсех", НСтр("ka='არა ყველასთვის';ru='Нет для всех';en='No to All'"));
	...
	Если ПараметрыВыполнения.НомерШага = 2 Тогда
		...
		ФайлНаДиске.УстановитьТолькоЧтение(Ложь);
		Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
			ШапкаВопроса = НСтр("ka='ხორციელდება სამუშაო კატალოგის გაწმენდა ფაილის დამატებით.';ru='Выполняется очистка рабочего каталога при добавлении файла.';en='Clears the working directory when a file is added.'");
		Иначе
			ШапкаВопроса = НСтр("ka='მიმდინარეობს სამუშაო კატალოგის გასუფთავება.';ru='Выполняется очистка рабочего каталога.';en='Clears the working directory.'");
		КонецЕсли;
...
#Если Не ВебКлиент Тогда
			Если СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути Тогда
				...
				Если СтрДлина(ПараметрыВыполнения.ИмяФайла) > МаксимальнаяДлинаИмениФайла Тогда
					ТекстСообщения =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ფაილისკენ არსებული გზის სიგრძე (სამუშაო კატალოგი, პლუს ფაილის სახელი) %1 სიმბოლოს აღემატება"
...
"%2'"),
					...
					ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
						+ НСтр("ka='ფაილის სახელი შეცვალეთ უფრო მოკლე სახელით.';ru='Измените имя файла на более короткое.';en='Change the file name to a shorter.'");
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПредложитьВыборКаталога(Ответ, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ფაილისკენ გზის სიგრძე %1 სიმბოლოს აღემატება:"
"%2"
""
"ავირჩიოთ სხვა ძირითადი სამუშაო კატალოგი?';ru='Длина пути к файлу превышает %1 символов:"
"%2"
""
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	Заголовок = НСтр("ka='აირჩიეთ სხვა ძირითადი სამუშაო კატალოგი';ru='Выберите другой основной рабочий каталог';en='Select another main working directory'");
	...
	Если ПараметрыВыполнения.ФайлЕстьВРегистре И Версия <> ПараметрыВыполнения.ДанныеФайла.ТекущаяВерсия Тогда
		...
		Если ПараметрыВыполнения.ДанныеФайла.Владелец = ВРегистреПапка Тогда // Одна и та же папка.
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სამუშაო კატალოგში უკვე არის ფაილი"
"""%1"","
"რომელიც დაკავშირებულია სხვა ფაილთან საინფორმაციო ბაზაში."
""
"ფაილის კოდი ფაილების საცავში: %2."
"ფაილის კოდი სამუშაო კატალოგში: %3."
""
"საინფორმაციო ბაზაში ერთ-ერთ ფაილს გადაარქვით.';ru='В рабочем каталоге уже есть файл"
"""%1"","
...
"Переименуйте один из файлов в информационной базе.'"),
		...
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სამუშაო კატალოგში უკვე არის ფაილი ""%1"", რომელიც დაკავშირებულია საინფორმაციო ბაზის სხვა ფაილთან."
""
"ფაილის კოდი ფაილების საცავში: %2."
"ფაილის კოდი სამუშაო კატალოგში: %3."
""
"საინფორმაციო ბაზაში შეცვალეთ ერთ-ერთი საქაღალდის სამუშაო კატალოგი."
"(ორ საქაღალდეს არ უნდა ჰქონდეს ერთნაირი სამუშაო კატალოგი).';ru='В рабочем каталоге уже есть файл"
"""%1"","
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеЗапуск(ПараметрыВыполнения)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ფაილისკენ სრული გზის სიგრძე (სამუშაო კატალოგი პლიუს ფაილის სახელი) აღემატება %1 სიმბოლოს ""%2"".';ru='Длина полного пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов"
"""%2"".'"),
		ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
		ПараметрыВыполнения.ПолноеИмяФайла);
	...
	Если СтрДлина(ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
			+ НСтр("ka='შეცვალეთ საქაღალდეების სახელები ან მიმდინარე საქაღალდე გადაიტანეთ სხვა საქაღალდეში.';ru='Измените имена папок или перенесите текущую папку в другую папку.';en='Change the names of the folders or move the current folder to another folder.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеПредложитьВыборКаталога(ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ფაილისკენ სრული გზის (სამუშაო კატალოგი პლუს ფაილის სახელი) სიგრძე %1 სიმბოლოს აღემატება"
"""%2""."
""
"ავირჩიოთ სხვა ძირითადი სამუშაო კატალოგი?"
"(სამუშაო კატალოგის შემცველობა გადატანილი იქნება არჩეულ კატალოგში).';ru='Длина полного пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов"
"""%2""."
""
"Выбрать другой основной рабочий каталог?"
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	// Выбираем другой путь к рабочему каталогу.
	Заголовок = НСтр("ka='აირჩიეთ სხვა სამუშაო კატალოგი';ru='Выберите другой рабочий каталог';en='Select a different working directory'");
...
""
"Повторить операцию?';ka='ფაილ ""%1""-ის კოპირება ვერ მოხერხდა. შეიძლება იგი დაკავებულია სხვა დანართით. გავიმეოროთ ოპერაცია?'"),
...
Процедура ПеренестиСодержимоеРабочегоКаталога(ОбработчикРезультата, КаталогИсточника, КаталогПриемника) Экспорт
	...
	Если СтрНайти(НРег(КаталогПриемника), НРег(КаталогИсточника)) <> 0 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='არჩეული სამუშაო კატალოგი ""%1"" შედის ძველ სამუშაო კატალოგ ""%2""-ში.';ru='Выбранный рабочий каталог"
"""%1"""
...
"""%1"""
"в каталог"
"""%2"".';ka='კატალოგ ""%1""-ის შემცველობის კატალოგ ""%2""-ში უკუკოპირება ვერ მოხერხდა.'"),
			ПараметрыВыполнения.КаталогПриемника,
			ПараметрыВыполнения.КаталогИсточника);
...
"Возможно он занят другим приложением."
""
"Повторить операцию?';ka='ფაილ ""%1""-ის წაშლა ვერ მოხერხდა. შეიძლება, იგი დაკავებულია სხვა დანართით. გავიმეოროთ ოპერაცია?'"),
				Путь);
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, НСтр("ka='დასამატებლად ფაილები არ არის';ru='Нет файлов для добавления';en='No files to add'"), Неопределено);
		Возврат;
	КонецЕсли;
...
""
"Продолжить импорт папки?';ka='საქაღალდე ""%1"" უკვე არსებობს. გავაგრძელოთ საქაღალდის იმპორტი?'"),
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილების იმპორტი დასრულებულია. იმპორტირებულია ფაილი: %1';ru='Импорт файлов завершен. Импортировано файлов: %1';en='Import completed files. Imported files:% 1'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() = 1 Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='ცვლილება:';ru='Изменение:';en='Change:'"),
			Ссылка,
			Элемент0.Файл,
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
		...
		Если ФайлВРабочемКаталоге = Истина Тогда
			...
			Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны).
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ფაილ ""%1""-ის ცვლილების თარიღი სამუშაო კატალოგში არის უფრო გვიანდელი (ახალი), ვიდრე ფაილების საცავში. შეიძლება, ფაილი შეცვლილია.';ru='Дата изменения файла ""%1"""
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';en='All files (*% 1.) |. *% 1'"), Расширение);
	...
	ТекстПояснения =
	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ხორციელდება ფაილ ""%1""-ის (%2 მბ) შენახვა..."
"გთხოვთ, მოიცადოთ.';ru='Выполняется сохранение файла ""%1"" (%2 Мб)..."
"Пожалуйста, подождите.'"),
		ИмяФайла, 
		ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
	...
	Состояние(НСтр("ka='ფაილი წარმატებით იქნა შებახული';ru='Файл успешно сохранен';en='File successfully saved'"), , ПолноеИмяФайла);
	...
	Для Каждого ИмяФайла Из ПараметрыПроверки.ВыбранныеФайлы Цикл
		...
		Если ВыбранКаталог Тогда
			Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მიმდინარეობს ინფორმაციის შეკრება კატალოგ ""%1""-ის შესახებ. გთხოვთ, მოიცადოთ.';ru='Идет сбор информации о каталоге"
"""%1""."
"Пожалуйста, подождите.'"),
				Путь));
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%2 მბ)';ru='%1 (%2 МБ)';en='1% (2% MB)'"), Строка(Файл), Строка(РазмерФайлаВМб));
			ФайлыБольшие.Добавить(ТекстСтроки);
		КонецЦикла;
		
		Параметры.Вставить("ФайлыБольшие", ФайлыБольшие);
		Параметры.Вставить("Заголовок", НСтр("ka='გაფრთხილება ფაილების ჩატვირთვის დროს';ru='Предупреждение при загрузке файлов';en='Caution when downloading files'"));
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		ТекстПредупреждения = НСтр("ka='ფაილი ""%1"" უკვე დაშიფრულია.';ru='Файл ""%1"" уже зашифрован.';en='File ""%1"" is already encrypted.'");
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Редактирует) Тогда
		ТекстПредупреждения = НСтр("ka='დაკავებული ფაილის დაშიფრვა შეუძლებელია.';ru='Нельзя зашифровать занятый файл.';en='You can not encrypt a file is busy.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстПредупреждения, ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ვერსიები: %1';ru='Версий: %1';en='Version:% 1'"), ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	Если ПустаяСтрока(СтруктураПути.Расширение) Тогда
		ТекстВопроса = НСтр("ka='აუცილებელია ფაილის მითითება გაფართოებითურთ.';ru='Необходимо указать файл с расширением.';en='You must specify the file extension.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Повторить, НСтр("ka='სხვა ფაილის არჩევა';ru='Выбрать другой файл';en='Select another file'"));
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления,
	...
	МодульЭлектроннаяПодписьКлиент.ИнформироватьОШифрованииОбъекта(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილი: %1';ru='Файл: %1';en='File:% 1'"), ФайлСсылка));
...
Процедура ПодписатьФайл(МассивФайлов, ИдентификаторФормы, ОбработчикЗавершения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='ფაილზე ხელმოწერა';ru='Подписание файла';en='Signing file'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ka='ფაილი';ru='Файл';en='File'"));
	ОписаниеДанных.Вставить("НаборДанных",         НаборДанных);
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru='Файлы (%1)';en='Files (1%)'"));
...
Процедура ИмпортФайловРекурсивноБезДиалогов(Знач Владелец, Знач ВыбранныеФайлы, Знач ПараметрыВыполнения, Знач ЗадаватьВопросПапкаУжеСуществует)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если Не ВыбранныйФайл.Существует() Тогда
			...
			Запись.Вставить("Ошибка", НСтр("ka='დისკზე არ არის ფაილი.';ru='Файл отсутствует на диске.';en='File does not exist on the disc.'"));
		...
		Попытка
			...
			НадписьПодробнее = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მუშავდება ფაილი ""%1"" (%2 მბ)...';ru='Обрабатывается файл ""%1"" (%2 Мб)...';en='Processed file ""%1"" (% 2 MB) ...'"),
				ВыбранныйФайл.Имя, 
				ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
				
			ТекстСостояния = НСтр("ka='ფაილების იმპორტი დისკიდან';ru='Импорт файлов с диска...';en='Import files from disk ...'");
...
"""%1"""
"во временное хранилище.';ka='დროებით საცავში ფაილ ""%1""-ის მოთავსების შეცდომა.'"),
	...
	Иначе
		...
		Если ОбщееКоличество > 0 Тогда 
			...
			ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ფაილები (%1 ც.) გადატანილია %2-ში';ru='Файлы (%1 шт) перенесены в %2';en='Files (% 1 PCS) transferred to 2%'"),
				ОбщееКоличество,
				ВладелецФайла);
			
			ПоказатьОповещениеПользователя(
				НСтр("ka='ფაილები გადატანილია';ru='Файлы перенесены';en='files transferred'"),
				,
				ПолноеОписание,
				БиблиотекаКартинок.Информация32);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеВопрос",      НСтр("ka='დავამთავროთ პროგრამასთან მუშაობა?';ru='Завершить работу с программой?';en='To complete the work with the program?'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок",   НСтр("ka='შემდეგი ფაილები თქვენს მიერ დაკავებულია რედაქტირებისთვის';ru='Следующие файлы заняты вами для редактирования:';en='The following files are busy you to edit:'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("Заголовок",            НСтр("ka='მუშაობის დამთავრება';ru='Завершение работы';en='shutdown'"));
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='რედაქტირებადი ფაილების სიის გახსნა (%1)';ru='Открыть список редактируемых файлов (%1)';en='View a list of editable file (% 1)'"),
		Ответ.КоличествоЗанятыхФайлов);
	ПредупреждениеПользователю.ТекстПредупреждения = НСтр("ru='Имеются занятые для редактирования файлы';en='There are employed for editing files'");
...
"""%1"".';ka='არასწორი გზა ან უფლებები არ არის კატალოგ ""%1""-ში ჩასაწერად.'"),
				ИмяКаталога);
...
#Если Не ВебКлиент Тогда
			...
			Если МассивФайловВКаталоге.Количество() <> 0 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='არჩეულ სამუშაო კატალოგ ""%1""-ში უკვე არის ფაილები."
""
"აირჩიეთ სხვა კატალოგი.';ru='В выбранном рабочем каталоге"
"""%1"""
"уже есть файлы."
""
