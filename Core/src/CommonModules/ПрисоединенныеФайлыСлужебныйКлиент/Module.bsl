...
Процедура ДобавитьФайлыРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Если Не ДополнительныеПараметры.Свойство("ПолноеИмяФайла") Тогда
			...
			ВыборФайла.Заголовок = НСтр("ka='ფაილის არჩევა';ru='Выбор файла';en='file selection'");
			ВыборФайла.Фильтр = ?(ЗначениеЗаполнено(Фильтр), Фильтр, НСтр("ka='ყველა ფაილი';ru='Все файлы';en='All files'") + " (*.*)|*.*");
...
Функция ПолучитьФайлВРабочийКаталог(Знач АдресДвоичныхДанныхФайла,
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ka='დისკზე კატალოგის შექმნის შეცდომა:';ru='Ошибка создания каталога на диске:';en='Failed to create a directory on the disc:'") + " " + СообщениеОбОшибке;
	...
	Если ПолучитьФайлы(ПолучаемыеФайлы, ПолученныеФайлы, , Ложь) Тогда
		...
		ПолноеИмяФайлаНаКлиенте = ПолученныеФайлы[0].ПолноеИмя;// +Lazare ONESOFT-3049
...
"""%1"""
"во временное хранилище.';ka='შეცდომა დროებით საცავში ფაილ ""%1""-ის მოთავსებისას.'"),
				ПутьКФайлу) );
		Возврат Результат;
	КонецЕсли;
...
Процедура ПоместитьРедактируемыйФайлНаДискеВХранилищеРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='სამუშაო კატალოგში ფაილი ვერ მოიძებნა.';ru='Файл не найден в рабочем каталоге.';en='File not found in the working directory.'"));
		КонецЕсли;
...
Процедура ПоместитьРедактируемыйФайлНаДискеВХранилищеВыполненоПомещение(ИнформацияОФайле, ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='არჩეული ფაილ ""%1""-ის სახელი განსხვავდება საცავში არსებული ფაილის სახელისგან ""%2"". გავაგრძელოთ?';ru='Имя выбранного файла"
"""%1"""
...
Процедура ВыбратьФайлНаДискеИПоместитьВХранилищеРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ВыборФайла.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';en='All files (*% 1.) |. *% 1'"), ДанныеФайла.Расширение);
...
Процедура ОткрытьКаталогСФайломРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Если ПустаяСтрока(РабочийКаталогПользователя) Тогда
			ПоказатьПредупреждение(, НСтр("ka='სამუშაო კატალოგი არ არის მიცემული';ru='Не задан рабочий каталог';en='the working directory is not set'"));
			Возврат;
		КонецЕсли;
		...
		Если Не Файл.Существует() Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილის კატალოგი არ არსებობს. შეიძლება, რომ მოცემულ კომპიუტერზე ფაილი ""%1"" ჯერ არ გახსნილა. შევინახოთ ფაილი კომპიუტერზე და გავხსნათ მისი კატალოგი?';ru='Каталог файла не существует. Возможно, на данном компьютере файл ""%1"" еще не открывался."
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ka='კომპიუტერიდან ფაილის მოთავსება ფაილების საცავში ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось поместить файл с компьютера в хранилище файлов по причине:';en='Unable to place the file on your computer to store files due to:'") + Символы.ПС);
...
Процедура ПоместитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ka='ინტერნეტ მიმომხილველში ფაილებთან სამუშაოდ გაფართოება არ არის დაყენებული.';ru='В обозреватель интернет не установлено расширение для работы с файлами.';en='The internet browser is not set extension for working with files.'"));
...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Если Контекст.ПолноеИмяФайла <> Контекст.ПолноеИмяПомещаемогоФайла Тогда
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ka='კატალოგის შექმნა არ შესრულებულა შემდეგი მიზეზით:';ru='Создание каталога не выполнено по причине:';en='Create a directory is not fulfilled due to:'"));
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ka='ფაილის თვისება ""მხოლოდ ნახვა""-ს შეცვლა არ შესრულდა შემდეგი მიზეზით:';ru='Изменение свойства файла ""Только просмотр"" не выполнено по причине:';en='Changing the properties of a file ""View Only"" is not fulfilled due to:'"));
		ДействияСФайлом.Добавить(Действие);
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ka='ფაილის კოპირება არ შესრულდა შემდეგი მიზეზით:';ru='Копирование файла не выполнено по причине:';en='File copying is not performed for a reason:'"));
		ДействияСФайлом.Добавить(Действие);
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("ka='ფაილის თვისებების მიღება არ შესრულდა შემდეგი მიზეზით:';ru='Получение свойств файла не выполнено по причине:';en='Getting the properties file is not created because:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ka='კომპიუტერზე ფაილის მიღება ფაილების საცავიდან ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить файл на компьютер из хранилища файлов по причине:';en='Failed to get the file to your computer from the store because of the file:'") + Символы.ПС);
	...
	Если Контекст.ДляРедактирования
	   И Контекст.ДанныеФайла.Редактирует <> ПользователиКлиентСервер.АвторизованныйПользователь() Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილის რედაქტირებას უკვე ახორციელებს მომხმარებელი %1.';ru='Файл уже редактирует пользователь %1.';en='The file is already editing the user% 1.'"), Строка(Контекст.ДанныеФайла.Редактирует)));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("ka='ფაილის თვისებების დაყენება არ შესრულდა შემდეგი მიზეზით:';ru='Установка свойств файла не выполнено по причине:';en='Set the properties file is not created because:'"));
	ДействияСФайлом.Добавить(Действие);
...
"уже зашифрован.'"), Строка(ПрисоединенныйФайл)));
	...
	Если ЗначениеЗаполнено(ДанныеФайла.Редактирует) Тогда
		ПоказатьПредупреждение(, НСтр("ka='დაკავებული ფაილის დაშიფრვა შეუძლებელია.';ru='Нельзя зашифровать занятый файл.';en='You can not encrypt a file is busy.'"));
		ВыполнитьОбработкуОповещения(ОбработчикРезультата, Неопределено);
		Возврат;
	КонецЕсли;
...
Процедура ОповеститьОбИзмененииИУдалитьФайлВРабочемКаталоге(Знач ПрисоединенныйФайл, Знач ДанныеФайла) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		МодульЭлектроннаяПодписьКлиент.ИнформироватьОШифрованииОбъекта(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ფაილი: %1';ru='Файл: %1';en='File:% 1'"), ПрисоединенныйФайл));
	КонецЕсли;
...
Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы,
	...
	Если ВыбранныеФайлы.Количество() > 1 Тогда
		...
		Если ТекущаяПозиция = Неопределено Тогда
			Состояние(НСтр("ka='Сохранение файлов завершено.';ru='Сохранение файлов завершено.';en='Saving files is completed.'"));
		Иначе
			...
			НадписьПодробнее = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Сохраняется файл ""%1"" (%2 Мб) ...';ru='Сохраняется файл ""%1"" (%2 Мб) ...';en='Saves the file ""%1"" (% 2 MB) ...'"), СохраняемоеИмяФайла, РазмерВМб);
				
			ТекстСостояния = НСтр("ka='Сохранение нескольких файлов.';ru='Сохранение нескольких файлов.';en='Save multiple files.'");
...
Процедура СохранитьФайлКакРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ТекстПояснения =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Сохраняется файл ""%1"" (%2 Мб)"
"Пожалуйста, подождите...';ka='მიმდინარეობს ფაილ ""%1""-ის (%2 მბ) შენახვა"
"გთხოვთ, მოიცადოთ...'"),
				ДанныеФайла.ИмяФайла, 
		...
		Если ПолучитьФайлы(ПолучаемыеФайлы, ПолученныеФайлы, , Ложь) Тогда
			Состояние(НСтр("ka='ფაილი წარმატებით იქნა შენახული.';ru='Файл успешно сохранен.';en='File saved successfully.'"), , ВыборФайла.ПолноеИмяФайла);
		КонецЕсли;
