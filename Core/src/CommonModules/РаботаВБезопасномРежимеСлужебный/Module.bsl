...
"поставляемой в макете %1: в архиве не обнаружен файл MANIFEST.XML!';ka='%1-ის მაკეტში მოწოდებული გარე კომპონენტის გამოყენების ნებართვის ჩამოყალიბება ვერ მოხერხდა: არქივში არ არის აღმოჩენილი ფაილი MANIFEST.XML!'"), ИмяМакета);
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='უსაფრთხო რეჟიმში მუშაობა. დროებითი ფაილის წაშლა ვერ მოხერხდა';ru='Работа в безопасном режиме.Не удалось удалить временный файл'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ПриЗаписиСлужебныхДанных(Объект) Экспорт
	...
	Если РаботаВБезопасномРежиме.УстановленБезопасныйРежим() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ობიექტ %1-ის ჩაწერა მიუწვდომელია: დაყენებულია უსაფრთხო რეჟიმი: %2!';ru='Запись объекта %1 недоступна: установлен безопасный режим: %2!'"),
