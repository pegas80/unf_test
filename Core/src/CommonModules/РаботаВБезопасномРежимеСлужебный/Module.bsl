...
"некорректное имя макета %1!';ka='გარე კომპონენტის გამოყენებაზე ნებართვების შედგენა ვერ მოხერხდა: მაკეტ %1-ის არაკორექტული სახელი!'"), ИмяМакета);
	КонецЕсли;
	
	Если Макет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: макет %1 не обнаружен в составе конфигурации!';ka='მაკეტ %1-ით მოწოდებული გარე კომპონენტის გამოყენების ნებართვის ჩამოყალიბება ვერ მოხერხდა: კონფიგურაციის შემადგენლობაში მაკეტი %1 არ არის აღმოჩენილი!'"), ИмяМакета);
	КонецЕсли;
	
	Если Метаданные.НайтиПоПолномуИмени(ИмяМакета).ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ДвоичныеДанные Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты:"
"макет %1 не содержит двоичных данных!';ka='გარე კომპონენტის გამოყენებაზე ნებართვის ჩამოყალიბება ვერ მოხერხდა: მაკეტი %1 ორობით მონაცემებს არ შეიცავს!'"), ИмяМакета);
	КонецЕсли;
	...
	Если ПустаяСтрока(ФайлМанифеста) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: в архиве не обнаружен файл MANIFEST.XML!';ka='%1-ის მაკეტში მოწოდებული გარე კომპონენტის გამოყენების ნებართვის ჩამოყალიბება ვერ მოხერხდა: არქივში არ არის აღმოჩენილი ფაილი MANIFEST.XML!'"), ИмяМакета);
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='უსაფრთხო რეჟიმში მუშაობა. დროებითი ფაილის წაშლა ვერ მოხერხდა';ru='Работа в безопасном режиме.Не удалось удалить временный файл';en='Work in a safe rezhime.Ne to delete the temporary file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ПриЗаписиСлужебныхДанных(Объект) Экспорт
	...
	Если РаботаВБезопасномРежиме.УстановленБезопасныйРежим() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ობიექტ %1-ის ჩაწერა მიუწვდომელია: დაყენებულია უსაფრთხო რეჟიმი: %2!';ru='Запись объекта %1 недоступна: установлен безопасный режим: %2!';en='Record object% 1 is not available: set to secure mode:% 2!'"),
