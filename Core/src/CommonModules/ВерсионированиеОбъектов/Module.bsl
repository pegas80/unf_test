...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ka='ობიექტების ჩაწერილი ვერსიების შესახებ ინფორმაციის განახლება. განახლების შესრულების შემდეგ, ვერსიონირებადი ობიექტების ჩაწერის სიჩქარე გაიზრდება.';ru='Обновление сведений о записанных версиях объектов. После выполнения обновления увеличится скорость записи версионируемых объектов.';en='Update information about recorded versions of objects. When the update is to increase the recording speed of versioned objects.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.4.13";
	Обработчик.Процедура = "ВерсионированиеОбъектов.ПеренестиНастройкиВерсионирования";
	Обработчик.Комментарий = НСтр("ka='ვერსიონირების გამართვების განახლება.';ru='Обновление настроек версионирования.';en='Update versioning settings.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Версионирование';ru='Версионирование';en='versioning'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"%3';ka='ობიექტ ""%2""-ის ვერსია №%1-ის შესახებ ცნობების განახლება ვერ მოხერხდა შემდეგი მიზეზით:"
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პროცედურა ობიექტებისვერსიებისშესახებცნობებისგანახლება-მ ვერ მოახერხა ინფორმაციის რეგისტრის ზოგიერთი ჩანაწერის დამუშავება ობიექტებისვერსიები (გამოტოვებულია): %1';ru='Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1';en='ObnovitSvedeniyaOVersiyahObektov procedure could not process some records registry information VersiiObektov (skipped):% 1'"), 
					Выборка.Количество());
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Подсказка = НСтр("ka='მოძველებული ვერსიები: %1 (%2)';ru='Устаревших версий: %1 (%2)';en='Obsolete versions: 1% (2%)'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='ობიექტების მოძველებული ვერსიები';ru='Устаревшие версии объектов';en='Older versions of objects'");
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("ka='ვერსია შექმნილია უკვე არსებული ობიექტის მიხედვით';ru='Версия создана по уже имеющемуся объекту';en='Version created by an existing object'"));
...
Процедура ПроверитьНаличиеПравНаИзменениеОбъекта(ОбъектМетаданных)
	...
	Если Не ПривилегированныйРежим() И Не ПравоДоступа("Изменение", ОбъектМетаданных)Тогда
		ТекстСообщения = НСтр("ka='%1-ის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав на изменение ""%1"".';en='Not have permission to change the ""%1"".'");
...
Функция ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено) Экспорт
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='№ %1 / (%2) / %3';ru='№ %1 / (%2) / %3';en='№% 1 / (2%) / 3%'"), ПользовательскийНомерВерсии,
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='Версионирование';ru='Версионирование';en='versioning'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"Техническая информация об ошибке: %1';ka='არჩეულ ვერსიაზე გადასვლა ვერ მოხერხდა. შესაძლებელი მიზეზი: ობიექტის ვერსია ჩაწერილი იქნა პროგრამის სხვა ვერსიაში. ტექნიკური ინფორმაცია შეცდომის შესახებ: %1'");
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	СообщениеНеУдалосьПолучитьВерсию = НСтр("ka='ობიექტის წინა ვერსიის მიღება ვერ მოხერხდა.';ru='Не удалось получить предыдущую версию объекта.';en='Could not get the previous version of the object.'");
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ka='ობიექტის არჩეული ვერსია პროგრამაში არ არის.';ru='Выбранная версия объекта отсутствует в программе.';en='The selected version of the object is not in the program.'");
	КонецЕсли;
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2';ru='%1 %2';en='%12'"), РазмерДанных, ЕдиницаИзмерения);
...
Функция ПолучитьПредставлениеРеквизитаНаЯзыке(Знач ИмяРеквизита)
	...
	Если      ИмяРеквизита = "Number" Тогда
		Возврат НСтр("ka='ნომერი';ru='Номер';en='room'");
	ИначеЕсли ИмяРеквизита = "Name" Тогда
		Возврат НСтр("ka='დასახელება';ru='Наименование';en='Name'");
	ИначеЕсли ИмяРеквизита = "Code" Тогда
		Возврат НСтр("ka='კოდი';ru='Код';en='The code'");
	ИначеЕсли ИмяРеквизита = "IsFolder" Тогда
		Возврат НСтр("ka='ЭтоГруппа';ru='ЭтоГруппа';en='Is folder'");
	ИначеЕсли ИмяРеквизита = "Description" Тогда
		Возврат НСтр("ka='დასახელება';ru='Наименование';en='Name'");
	ИначеЕсли ИмяРеквизита = "Date" Тогда
		Возврат НСтр("ka='თარიღი';ru='Дата';en='Date'");
	ИначеЕсли ИмяРеквизита = "Posted" Тогда
		Возврат НСтр("ka='გატარებულია';ru='Проведен';en='Posted'");
	ИначеЕсли ИмяРеквизита = "DeletionMark" Тогда
		Возврат НСтр("ka='წაშლის ნიშნული';ru='ПометкаУдаления';en='PometkaUdaleniya'");
	ИначеЕсли ИмяРеквизита = "Ref" Тогда
		Возврат НСтр("ka='ბმული';ru='Ссылка';en='Ref'");
	ИначеЕсли ИмяРеквизита = "Parent" Тогда
		Возврат НСтр("ka='მშობელი';ru='Родитель';en='Parent'");
	ИначеЕсли ИмяРеквизита = "Owner" Тогда
		Возврат НСтр("ka='მფლობელი';ru='Владелец';en='Owner'");
	Иначе
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	Объект.ДополнительныеСвойства.Вставить("ВерсионированиеОбъектовКомментарийКВерсии",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%2-დან #%1 ვერსიაზე გადასვლა შესრულებულია';ru='Выполнен переход к версии №%1 от %2';en='Transition is made to a version of №% 1% 2'"),
