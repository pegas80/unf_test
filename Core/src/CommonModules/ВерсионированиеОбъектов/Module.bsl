	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Версионирование';ru='Версионирование'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პროცედურა ობიექტებისვერსიებისშესახებცნობებისგანახლება-მ ვერ მოახერხა ინფორმაციის რეგისტრის ზოგიერთი ჩანაწერის დამუშავება ობიექტებისვერსიები (გამოტოვებულია): %1';ru='Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1'"), 
					Выборка.Количество());
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Подсказка = НСтр("ka='მოძველებული ვერსიები: %1 (%2)';ru='Устаревших версий: %1 (%2)'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='ობიექტების მოძველებული ვერსიები';ru='Устаревшие версии объектов'");
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("ka='ვერსია შექმნილია უკვე არსებული ობიექტის მიხედვით';ru='Версия создана по уже имеющемуся объекту'"));
...
Процедура ПроверитьНаличиеПравНаИзменениеОбъекта(ОбъектМетаданных)
	...
	Если Не ПривилегированныйРежим() И Не ПравоДоступа("Изменение", ОбъектМетаданных)Тогда
		ТекстСообщения = НСтр("ka='%1-ის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав на изменение ""%1"".'");
...
Функция ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено) Экспорт
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='№ %1 / (%2) / %3';ru='№ %1 / (%2) / %3'"), ПользовательскийНомерВерсии,
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='Версионирование';ru='Версионирование'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	СообщениеНеУдалосьПолучитьВерсию = НСтр("ka='ობიექტის წინა ვერსიის მიღება ვერ მოხერხდა.';ru='Не удалось получить предыдущую версию объекта.'");
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ka='ობიექტის არჩეული ვერსია პროგრამაში არ არის.';ru='Выбранная версия объекта отсутствует в программе.'");
	КонецЕсли;
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2';ru='%1 %2'"), РазмерДанных, ЕдиницаИзмерения);
...
Функция ПолучитьПредставлениеРеквизитаНаЯзыке(Знач ИмяРеквизита)
	...
	Если      ИмяРеквизита = "Number" Тогда
		Возврат НСтр("ka='ნომერი';ru='Номер'");
	ИначеЕсли ИмяРеквизита = "Name" Тогда
		Возврат НСтр("ka='დასახელება';ru='Наименование'");
	ИначеЕсли ИмяРеквизита = "Code" Тогда
		Возврат НСтр("ka='კოდი';ru='Код'");
	ИначеЕсли ИмяРеквизита = "IsFolder" Тогда
		Возврат НСтр("ka='ЭтоГруппа';ru='ЭтоГруппа'");
	ИначеЕсли ИмяРеквизита = "Description" Тогда
		Возврат НСтр("ka='დასახელება';ru='Наименование'");
	ИначеЕсли ИмяРеквизита = "Date" Тогда
		Возврат НСтр("ka='თარიღი';ru='Дата'");
	ИначеЕсли ИмяРеквизита = "Posted" Тогда
		Возврат НСтр("ka='გატარებულია';ru='Проведен'");
	ИначеЕсли ИмяРеквизита = "DeletionMark" Тогда
		Возврат НСтр("ka='წაშლის ნიშნული';ru='ПометкаУдаления'");
	ИначеЕсли ИмяРеквизита = "Ref" Тогда
		Возврат НСтр("ka='ბმული';ru='Ссылка'");
	ИначеЕсли ИмяРеквизита = "Parent" Тогда
		Возврат НСтр("ka='მშობელი';ru='Родитель'");
	ИначеЕсли ИмяРеквизита = "Owner" Тогда
		Возврат НСтр("ka='მფლობელი';ru='Владелец'");
	Иначе
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	Объект.ДополнительныеСвойства.Вставить("ВерсионированиеОбъектовКомментарийКВерсии",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%2-დან #%1 ვერსიაზე გადასვლა შესრულებულია';ru='Выполнен переход к версии №%1 от %2'"),
