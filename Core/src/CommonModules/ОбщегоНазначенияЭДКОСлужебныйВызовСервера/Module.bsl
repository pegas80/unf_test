	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		Иначе
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Электронный документооброт с контролирующими органами.Получение серийного номер ViPNet CSP';ru='Электронный документооброт с контролирующими органами.Получение серийного номер ViPNet CSP';en='Electronic dokumentoobrot with controlling organami.Poluchenie serial number ViPNet CSP'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			    УровеньЖурналаРегистрации.Ошибка,,, СтрШаблон("error_code: %1", ОтветJSON.error_code));
			ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(
				НСтр("ka='ViPNet CSP-ისთვის სერიული ნომრების მიღების სერვისი დროებით მიუწვდომელია. მოგვიანებით სცადეთ.';ru='Сервис получения серийных номер для ViPNet CSP временно не доступен. Повторите попытку позже.';en='Service of obtaining serial number for ViPNet CSP temporarily not available. Please try again later.'"));
		КонецЕсли;
