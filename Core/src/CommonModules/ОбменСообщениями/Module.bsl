...
Процедура ДоставитьСообщения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='სისტემის სწრაფი შეტყობინებების მიწოდება არ შეიძლება შესრულდეს აქტიურ ტრანზაქციაში.';ru='Доставка быстрых сообщений системы не может выполняться в активной транзакции.'");
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Получатель) = Тип("Массив") Тогда
		...
		Для Каждого Элемент Из Получатель Цикл
			...
			Если ТипЗнч(Элемент) <> Тип("ПланОбменаСсылка.ОбменСообщениями") Тогда
				...
				ВызватьИсключение НСтр("ka='მეთოდ შეტყობინებებისგაცვლა-სთვის მითითებულია არასწორი მიმღები. შეტყობინებისგაგზავნა().';ru='Указан неправильный получатель для метода ОбменСообщениями.ОтправитьСообщение().'");
...
Процедура ОтправитьСообщениеПолучателю(КаналСообщений, ТелоСообщения, Получатель, ЭтоБыстроеСообщение)
	...
	Если Не ТранзакцияАктивна() Тогда
		...
		ВызватьИсключение НСтр("ka='შეტყობინებების გაგზავნა შეიძლება განხორციელდეს მხოლოდ ტრანზაქციაში.';ru='Отправка сообщений может выполняться только в транзакции.'");
		
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(КаналСообщений) Тогда
		
		ВызватьИсключение НСтр("ka='მეთოდ შეტყობინებებისგაცვლა-სთვისპარამეტრ ""შეტყობინებებისარხი""-ს მნიშვნელობა მიცემული არ არის. შეტყობინებისგაგზავნა.';ru='Не задано значение параметра ""КаналСообщений"" для метода ОбменСообщениями.ОтправитьСообщение.'");
		
	ИначеЕсли СтрДлина(КаналСообщений) > 150 Тогда
		
		ВызватьИсключение НСтр("ka='შეტყობინებების არხის სახელის სიგრძე არ უნდა აღემატებოდეს 150 სიმბოლოს.';ru='Длина имени канала сообщений не должна превышать 150 символов.'");
		
	ИначеЕсли Не ЗначениеЗаполнено(Получатель) Тогда
		
		ВызватьИсключение НСтр("ka='მეთოდ შეტყობინებებისგაცვლა-სთვის პარამეტრ ""მიმღები""-ს მნიშვნელობა არ არის მიცემული. შეტყობინებისგაგზავნა.';ru='Не задано значение параметра ""Получатель"" для метода ОбменСообщениями.ОтправитьСообщение.'");
		
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Получатель, "Заблокирована") Тогда
		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაბლოკილი საბოლოო წერტილისთვის ""%1"" შეტყობინების გაგზავნის მცდელობა.';ru='Попытка отправки сообщения заблокированной конечной точке ""%1"".'"),
			Строка(Получатель));
	КонецЕсли;
