...
Процедура ЗаблокироватьТекущуюОбластьДанных(Знач ПроверитьОтсутствиеДругихСеансов = Ложь, Знач РазделяемаяБлокировка = Ложь) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение(НСтр("ka='არეალის ბლოკირება შეიძლება დაყენებული იქნას მხოლოდ გამყოფების ჩართული გამოყენებისას';ru='Блокировка области может быть установлена только при включенном использовании разделителей'"));
	КонецЕсли;
		...
		Если КонфликтующиеСеансы.Количество() > 0 Тогда
			...
			Для Каждого КонфликтующийСеанс Из КонфликтующиеСеансы Цикл
				...
				ТекстСеансы = ТекстСеансы + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 (სეანსი - %2)';ru='%1 (сеанс - %2)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';ru='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1'",
...
Процедура ПодготовитьОбластьДанныхКИспользованию(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, 
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение(НСтр("ka='ოპერაციის შესასრულებლად არასაკმარისი უფლებებია';ru='Недостаточно прав для выполнения операции'"));
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СменитьСтатусОбластиИУведомитьМенеджер(Менеджер, ?(Менеджер.Статус = Перечисления.СтатусыОбластейДанных.Новая,
				"ФатальнаяОшибка", "ОшибкаУдаления"), НСтр("ka='არეალის დამუშავების მცდელობის რაოდენობა ამოწურულია';ru='Исчерпано количество попыток обработки области'"));
...
Функция ПолучитьПроксиМенеджераСервиса(Знач ПарольПользователя = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(АдресМенеджераСервиса) Тогда
		ВызватьИсключение(НСтр("ka='სერვისის მენეჯერთან დაკავშირების პარამეტრები არ არის დაყენებული.';ru='Не установлены параметры связи с менеджером сервиса.'"));
	КонецЕсли;
...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
		ВызватьИсключение(НСтр("ka='სეანსის განაწილების შეცვლა შეიძლება მხოლოდ იმ სეანსიდან, რომელიც გაშვებულია გამყოფების მიუთითებლად';ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей'"));
	КонецЕсли;
...
Функция ЗначениеРазделителяСеанса() Экспорт
	...
	Иначе
		Если НЕ ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			ВызватьИсключение(НСтр("ka='არ არის დაყენებული გამყოფის მნიშვნელობა';ru='Не установлено значение разделителя'"));
		КонецЕсли;
...
Процедура ОбработатьИнформациюОбОшибкеWebСервиса(Знач ИнформацияОбОшибке, Знач ИмяПодсистемы = "", Знач ИмяВебСервиса = "", Знач ИмяОперации = "") Экспорт
	...
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka=' %1. web-სერვისის ოპერაციის მოხმობის შეცდომა';ru='%1.Ошибка вызова операции web-сервиса'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяПодсистемы);
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка при вызове операции %1 веб-сервиса %2: %3';ru='Ошибка при вызове операции %1 веб-сервиса %2: %3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяОперации,
		ИмяВебСервиса,
...
Функция ПараметрыВыборки(Знач ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Функция ПараметрыВыборки() не должна использоваться для объекта %1.';ru='Функция ПараметрыВыборки() не должна использоваться для объекта %1.'"),
...
Функция СобытиеЖурналаРегистрацииКопированиеОбластиДанных() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. მონაცემთა არეალის კოპირება';ru='Работа в модели сервиса.Копирование области данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииБлокировкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. მონაცემთა არეალის ბლოკირება';ru='Работа в модели сервиса.Блокировка области данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПодготовкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. მონაცემთა არეალის მომზადება';ru='Работа в модели сервиса.Подготовка области данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПолучениеФайлаИзХранилища() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. ფაილის მიღება საცავიდან';ru='Работа в модели сервиса.Получение файла из хранилища'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменЧерезФС() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა.ფაილის დამატება.გაცვლა ФС-ით';ru='Работа в модели сервиса.Добавление файла.Обмен через ФС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменНеЧерезФС() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. ფაილის დამატება. ФС-ით გაცვლა';ru='Работа в модели сервиса.Добавление файла.Обмен не через ФС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. დროებითი ფაილის წაშლა';ru='Работа в модели сервиса.Удаление временного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	СтрокаПараметра.Описание = НСтр("ka='კონფიგურაციის ვერსია';ru='Версия конфигурации'");
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
		...
		Попытка
			...
			Если СвойстваФайла.Существует() Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ka='ამოღება';ru='Извлечение'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					ИдентификаторФайла,
	...
	Исключение
		...
		ФайлОбъединен = Ложь;
		ЗаписьЖурналаРегистрации(НСтр("ka='ფაილების გაერთიანების ოპერაციის შესრულება';ru='Выполнение операции объединения файлов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлОбъединен Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='zip ფაილის წაკითხვის ოპერაციის შესრულება';ru='Выполнение операции чтения zip файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='დროებითი ფაილის წაშლის ოპერაციის შესრულება';ru='Выполнение операции удаления временного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ПрочитанZipФайл Тогда
		...
		ВызватьИсключение(НСтр("ka='zip ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении zip файла произошла ошибка.'"));
		
	КонецЕсли;
	
	Если ВПолученномАрхивеБолееОдногоФайла Тогда
		
		ВызватьИсключение(НСтр("ka='მიღებული არქივი შეიცავს ერთ ფაილზე მეტს.';ru='В полученном архиве содержится более одного файла.'"));
...
Функция ПолучитьМенеджерЗаписиОбластиДанных(Знач ОбластьДанных, Знач Статус)
	...
	Попытка
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			ШаблонСообщения = НСтр("ka='Область данных %1 не найдена';ru='Область данных %1 не найдена'");
		...
		ИначеЕсли МенеджерЗаписи.Статус <> Статус Тогда
			ШаблонСообщения = НСтр("ka='Статус области данных %1 не равен ""%2""';ru='Статус области данных %1 не равен ""%2""'");
...
Процедура СменитьСтатусОбластиИУведомитьМенеджер(Знач МенеджерЗаписи, Знач РезультатПодготовки, Знач СообщениеОбОшибке)
	...
	ИначеЕсли РезультатПодготовки = "ОшибкаУдаления" Тогда
		...
		ВключатьСообщениеОбОшибке = Истина;
	Иначе
		ВызватьИсключение НСтр("ka='Неожиданный код возврата';ru='Неожиданный код возврата'");
	КонецЕсли;
...
Функция ПодготовитьОбластьДанныхКИспользованиюИзВыгрузки(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, СообщениеОбОшибке)
	...
	Если ИмяФайлаВыгрузки = Неопределено Тогда
		...
		СообщениеОбОшибке = НСтр("ka='Нет файла начальных данных для области';ru='Нет файла начальных данных для области'");
...
Функция ПоместитьФайлВХранилище(Знач АдресДанныеФайл, Знач ПараметрыПодключения, Знач ИмяФайла = "")
	...
	Если ОбменЧерезФС Тогда
		...
		Если ЦелевойФайл.Существует() Тогда
			...
			Если СвойстваФайла.ПолноеИмя = ЦелевойФайл.ПолноеИмя Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ka='სათავსო';ru='Помещение'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					Результат,
	...
	Если Не ОбменЧерезФС Тогда
		...
		Если Описание.Данные = Неопределено Тогда
			...
			Иначе
				ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ფაილის დამატება საცავში. ფაილი %1 ვერ მოიძებნა.';ru='Добавление файла в хранилище. Не найден файл %1.'"),
...
Функция ПолучитьИмяФайлаСДанными(Знач АдресДанныеФайл, Знач ИмяФайла = "")
	...
	Если ТипЗнч(АдресДанныеФайл) = Тип("Строка") Тогда // Адрес данных файла во временном хранилище.
		Если ПустаяСтрока(АдресДанныеФайл) Тогда
			ВызватьИсключение(НСтр("ka='Неверный адрес хранилища.';ru='Неверный адрес хранилища.'"));
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(АдресДанныеФайл) = Тип("Файл") Тогда // Объект типа Файл.
		Если Не АдресДанныеФайл.Существует() Тогда
			ВызватьИсключение(НСтр("ka='Файл не найден.';ru='Файл не найден.'"));
		КонецЕсли;
	...
	Иначе
		ВызватьИсключение(НСтр("ka='Неверный тип данных';ru='Неверный тип данных'"));
	КонецЕсли;
...
Процедура ЗаписатьВЖурналСобытиеФайловогоХранилища(Знач Событие,
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='ფაილების საცავი';ru='Хранилище файлов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + "." + Событие,
...
Функция СтруктурныйОбъектВОбъектXDTO(Знач ЗначениеСтруктурногоТипа)
	...
	Если Не СериализуемыйСтруктурныйТип(СтруктурныйТип) Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Тип %1 не является структурным или его сериализация в настоящее время не поддерживается.';ru='Тип %1 не является структурным или его сериализация в настоящее время не поддерживается.'"),
			СтруктурныйТип);
...
"Дескриптор файла: %1'"),
	...
	Если Не ФайлДанныхНайден Тогда
		...
		Если Дескрипторы.Descriptor.Количество() = 0 Тогда
			...
			НСтр("ka='კონფიგურაციის მიმდინარე ვერსიისთვის სერვისის მენეჯერში არ არის საწყის მონაცემთა ფაილი.';ru='В менеджере сервиса нет файла начальных данных для текущей версии конфигурации.'");
			Возврат "ФатальнаяОшибка";
		КонецЕсли;
		...
		Если ИмяФайлаВыгрузки = Неопределено Тогда
			СообщениеОФатальнойОшибке = 
			НСтр("ka='სერვისის მენეჯერში საწყის მონაცემთა მოთხოვნილი ფაილი აღარ არის, სავარაუდოდ, იგი შეიცვალა. არეალი არ შეიძლება, რომ მომზადებული იქნას.';ru='В менеджере сервиса больше нет требуемого файла начальных данных, вероятно он был заменен. Область не может быть подготовлена.'");
			Возврат Ложь;
		КонецЕсли;
...
Процедура КонтрольНеразделенныхДанныхПриЗаписи(Знач Источник)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ПредставлениеИсключения = НСтр("ka='დაშვების უფლებების დარღვევა!';ru='Нарушение прав доступа!'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Следующие объекты не удовлетворяют этому критерию: %2'"),
	...
	Если Не ПустаяСтрока(РезультирующееИсключение) Тогда
		...
		РезультирующееИсключение = НСтр("ka='Обнаружены ошибки в структуре метаданных конфигурации:';ru='Обнаружены ошибки в структуре метаданных конфигурации:'") + Символы.ПС + Символы.ВК + РезультирующееИсключение;
