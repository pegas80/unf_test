...
Процедура ЗаблокироватьТекущуюОбластьДанных(Знач ПроверитьОтсутствиеДругихСеансов = Ложь, Знач РазделяемаяБлокировка = Ложь) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение(НСтр("ka='არეალის ბლოკირება შეიძლება დაყენებული იქნას მხოლოდ გამყოფების ჩართული გამოყენებისას';ru='Блокировка области может быть установлена только при включенном использовании разделителей';en='Lock area can only be established when the use of separators'"));
	КонецЕсли;
...
"%1';ka='მონაცემთა არეალის ბლოკირების დაყენება ვერ მოხერხდა შემდეგი მიზეზით: %1'");
		...
		Если КонфликтующиеСеансы.Количество() > 0 Тогда
			...
			Для Каждого КонфликтующийСеанс Из КонфликтующиеСеансы Цикл
				...
				ТекстСеансы = ТекстСеансы + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 (სეანსი - %2)';ru='%1 (сеанс - %2)';en='% 1 (Session - 2%)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';ru='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';en='The operation can not be performed because Other users are working in an application:% 1'",
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СменитьСтатусОбластиИУведомитьМенеджер(Менеджер, ?(Менеджер.Статус = Перечисления.СтатусыОбластейДанных.Новая,
				"ФатальнаяОшибка", "ОшибкаУдаления"), НСтр("ka='არეალის დამუშავების მცდელობის რაოდენობა ამოწურულია';ru='Исчерпано количество попыток обработки области';en='Exhausted the number of attempts at the treatment area'"));
...
Функция ПолучитьПроксиМенеджераСервиса(Знач ПарольПользователя = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(АдресМенеджераСервиса) Тогда
		ВызватьИсключение(НСтр("ka='სერვისის მენეჯერთან დაკავშირების პარამეტრები არ არის დაყენებული.';ru='Не установлены параметры связи с менеджером сервиса.';en='Do not set the communication parameters with the service manager.'"));
	КонецЕсли;
...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
		ВызватьИсключение(НСтр("ka='სეანსის განაწილების შეცვლა შეიძლება მხოლოდ იმ სეანსიდან, რომელიც გაშვებულია გამყოფების მიუთითებლად';ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей';en='Change the split session is possible only from a session running without separators'"));
	КонецЕсли;
...
Функция ЗначениеРазделителяСеанса() Экспорт
	...
	Иначе
		Если НЕ ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			ВызватьИсключение(НСтр("ka='არ არის დაყენებული გამყოფის მნიშვნელობა';ru='Не установлено значение разделителя';en='Not assigned separator value'"));
		КонецЕсли;
...
Процедура ОбработатьИнформациюОбОшибкеWebСервиса(Знач ИнформацияОбОшибке, Знач ИмяПодсистемы = "", Знач ИмяВебСервиса = "", Знач ИмяОперации = "") Экспорт
	...
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka=' %1. web-სერვისის ოპერაციის მოხმობის შეცდომა';ru='%1.Ошибка вызова операции web-сервиса';en='% 1.Oshibka call operation web-service'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяПодсистемы);
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка при вызове операции %1 веб-сервиса %2: %3';ru='Ошибка при вызове операции %1 веб-сервиса %2: %3';en='Error when calling% 1 operation web service% 2% 3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяОперации,
		ИмяВебСервиса,
...
Функция ПараметрыВыборки(Знач ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Функция ПараметрыВыборки() не должна использоваться для объекта %1.';ru='Функция ПараметрыВыборки() не должна использоваться для объекта %1.';en='ParametryVyborki () function should not be used for the object 1%.'"),
...
Функция СобытиеЖурналаРегистрацииКопированиеОбластиДанных() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. მონაცემთა არეალის კოპირება';ru='Работа в модели сервиса.Копирование области данных';en='Jobs servisa.Kopirovanie data domain model'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииБлокировкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. მონაცემთა არეალის ბლოკირება';ru='Работа в модели сервиса.Блокировка области данных';en='Jobs servisa.Blokirovka data domain model'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПодготовкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. მონაცემთა არეალის მომზადება';ru='Работа в модели сервиса.Подготовка области данных';en='Jobs servisa.Podgotovka data domain model'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПолучениеФайлаИзХранилища() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. ფაილის მიღება საცავიდან';ru='Работа в модели сервиса.Получение файла из хранилища';en='Work in the model file from storage servisa.Poluchenie'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменЧерезФС() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა.ფაილის დამატება.გაცვლა ФС-ით';ru='Работа в модели сервиса.Добавление файла.Обмен через ФС';en='Work in the model servisa.Dobavlenie fayla.Obmen by FS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменНеЧерезФС() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. ფაილის დამატება. ФС-ით გაცვლა';ru='Работа в модели сервиса.Добавление файла.Обмен не через ФС';en='Work in the model servisa.Dobavlenie fayla.Obmen not via FS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат НСтр("ka='სერვისის მოდელში მუშაობა. დროებითი ფაილის წაშლა';ru='Работа в модели сервиса.Удаление временного файла';en='Work in the model temporary file servisa.Udalenie'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	СтрокаПараметра.Описание = НСтр("ka='კონფიგურაციის ვერსია';ru='Версия конфигурации';en='configuration version'");
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
		...
		Попытка
			...
			Если СвойстваФайла.Существует() Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ka='ამოღება';ru='Извлечение';en='extraction'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					ИдентификаторФайла,
	...
	Исключение
		...
		ФайлОбъединен = Ложь;
		ЗаписьЖурналаРегистрации(НСтр("ka='ფაილების გაერთიანების ოპერაციის შესრულება';ru='Выполнение операции объединения файлов';en='Execute File merge operation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлОбъединен Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='zip ფაილის წაკითხვის ოპერაციის შესრულება';ru='Выполнение операции чтения zip файла';en='Performing zip file reads'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='დროებითი ფაილის წაშლის ოპერაციის შესრულება';ru='Выполнение операции удаления временного файла';en='An operation to delete the temporary file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ПрочитанZipФайл Тогда
		...
		ВызватьИсключение(НСтр("ka='zip ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении zip файла произошла ошибка.';en='When reading error occurred zip file.'"));
		
	КонецЕсли;
	
	Если ВПолученномАрхивеБолееОдногоФайла Тогда
		
		ВызватьИсключение(НСтр("ka='მიღებული არქივი შეიცავს ერთ ფაილზე მეტს.';ru='В полученном архиве содержится более одного файла.';en='The resulting archive contains more than one file.'"));
...
Процедура ПроверитьВозможностьИспользованияКонфигурацииВМоделиСервиса() Экспорт
	...
	Если ОписаниеБТС = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონფიგურაციაში არ არის დანერგილი ბიბლიოთეკა ""1С:სერვისის ტექნოლოგიის ბიბლიოთეკა""."
"ამ ბიბლიოთეკის დანერგვის გარეშე კონფიგურაცია არ შეიძლება, რომ გამოყენებული იქნას სერვისის მოდელში."
""
"ამ კონფიგურაციის გამოსაყენებლად სერვისის მოდელში საჭირო არის ბიბლიოთეკა ""1С:სერვისის ტექნოლოგიის ბიბლიოთეკა""-ს არა ნაკლებ %1 ვერსიის დანერგვა!';ru='В конфигурацию не внедрена библиотека ""1С:Библиотека технологии сервиса""."
...
"""1С:Библиотека технологии сервиса"" версии не младше %1!'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБТС, ТребуемаяВерсияБТС()) < 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სერვისის მოდელში კონფიგურაციის გამოსაყენებლად სქბ-ის მიმდინარე ვერსიიდან საჭირო არის ბიბლიოთეკა ""1С:სერვისის ტექნოლოგიის ბიბლიოთეკა""-ს გამოყენებადი ვერსიის განახლება!"
""
"გამოყენებული ვერსია: %1, საჭირო არის %2-ზე არა ნაკლები ვერსია!';ru='Для использования конфигурации в модели сервиса с текущей версией БСП требуется"
...
"Проверьте наличие и корректность внедрения подсистемы ""%1"".';ka='Невозможно выполнить операцию по причине - в конфигурации не внедрена подсистема ""%1""."
"Данная подсистема поставляется в состав библиотеки технологии сервиса, которая должна отдельно внедряться в состав конфигурации."
...
Процедура ПриПроверкеБлокировкиОбластиДанныхПриЗапуске(ОписаниеОшибки) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ОписаниеОшибки =
			НСтр("ka='დანართის გაშვება დროებით მიუწვდომელია. მიმდინარეობს დანართის მომსახურებასთან დაკავშირებული სარეგლამენტო ოპერაციების შესრულება."
""
"რამდენიმე წუთის შემდეგ სცადეთ დანართის გაშვება.';ru='Запуск приложения временно недоступен."
...
Функция ПолучитьМенеджерЗаписиОбластиДанных(Знач ОбластьДанных, Знач Статус)
	...
	Попытка
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			ШаблонСообщения = НСтр("ka='Область данных %1 не найдена';ru='Область данных %1 не найдена';en='Data Area% 1 Not Found'");
		...
		ИначеЕсли МенеджерЗаписи.Статус <> Статус Тогда
			ШаблонСообщения = НСтр("ka='Статус области данных %1 не равен ""%2""';ru='Статус области данных %1 не равен ""%2""';en='Status data field is not equal to 1% ""2%""'");
...
Процедура СменитьСтатусОбластиИУведомитьМенеджер(Знач МенеджерЗаписи, Знач РезультатПодготовки, Знач СообщениеОбОшибке)
	...
	ИначеЕсли РезультатПодготовки = "ОшибкаУдаления" Тогда
		...
		ВключатьСообщениеОбОшибке = Истина;
	Иначе
		ВызватьИсключение НСтр("ka='Неожиданный код возврата';ru='Неожиданный код возврата';en='Unexpected return code'");
	КонецЕсли;
...
Функция ПодготовитьОбластьДанныхКИспользованиюИзВыгрузки(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, СообщениеОбОшибке)
	...
	Если ИмяФайлаВыгрузки = Неопределено Тогда
		...
		СообщениеОбОшибке = НСтр("ka='Нет файла начальных данных для области';ru='Нет файла начальных данных для области';en='No primary data file for the area'");
...
Функция ПоместитьФайлВХранилище(Знач АдресДанныеФайл, Знач ПараметрыПодключения, Знач ИмяФайла = "")
	...
	Если ОбменЧерезФС Тогда
		...
		Если ЦелевойФайл.Существует() Тогда
			...
			Если СвойстваФайла.ПолноеИмя = ЦелевойФайл.ПолноеИмя Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ka='სათავსო';ru='Помещение';en='room'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					Результат,
	...
	Если Не ОбменЧерезФС Тогда
		...
		Если Описание.Данные = Неопределено Тогда
			...
			Иначе
				ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ფაილის დამატება საცავში. ფაილი %1 ვერ მოიძებნა.';ru='Добавление файла в хранилище. Не найден файл %1.';en='To add a file to the repository. file% 1 could not be found.'"),
...
Функция ПолучитьИмяФайлаСДанными(Знач АдресДанныеФайл, Знач ИмяФайла = "")
	...
	Если ТипЗнч(АдресДанныеФайл) = Тип("Строка") Тогда // Адрес данных файла во временном хранилище.
		Если ПустаяСтрока(АдресДанныеФайл) Тогда
			ВызватьИсключение(НСтр("ka='Неверный адрес хранилища.';ru='Неверный адрес хранилища.';en='Invalid address storage.'"));
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(АдресДанныеФайл) = Тип("Файл") Тогда // Объект типа Файл.
		Если Не АдресДанныеФайл.Существует() Тогда
			ВызватьИсключение(НСтр("ka='Файл не найден.';ru='Файл не найден.';en='File not found.'"));
		КонецЕсли;
	...
	Иначе
		ВызватьИсключение(НСтр("ka='Неверный тип данных';ru='Неверный тип данных';en='Invalid data type'"));
	КонецЕсли;
...
Процедура ЗаписатьВЖурналСобытиеФайловогоХранилища(Знач Событие,
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='ფაილების საცავი';ru='Хранилище файлов';en='file Storage'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + "." + Событие,
...
Функция СтруктурныйОбъектВОбъектXDTO(Знач ЗначениеСтруктурногоТипа)
	...
	Если Не СериализуемыйСтруктурныйТип(СтруктурныйТип) Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Тип %1 не является структурным или его сериализация в настоящее время не поддерживается.';ru='Тип %1 не является структурным или его сериализация в настоящее время не поддерживается.';en='% Type 1 is not a structural or serialization is not currently supported.'"),
			СтруктурныйТип);
...
"Дескриптор файла: %1';ka='Указанный файл начальных данных не подходит для данной конфигурации."
"Дескриптор файла: %1'"),
	...
	Если Не ФайлДанныхНайден Тогда
		...
		Если Дескрипторы.Descriptor.Количество() = 0 Тогда
			...
			НСтр("ka='კონფიგურაციის მიმდინარე ვერსიისთვის სერვისის მენეჯერში არ არის საწყის მონაცემთა ფაილი.';ru='В менеджере сервиса нет файла начальных данных для текущей версии конфигурации.';en='The service manager is not a file of initial data for the current version of the configuration.'");
			Возврат "ФатальнаяОшибка";
		КонецЕсли;
		
		Если Дескрипторы.Descriptor[0].FileGUID <> ИДФайлаВыгрузки Тогда
			СообщениеОФатальнойОшибке = 
			НСтр("ka='Файл начальных данных, имеющийся в менеджере сервиса, отличается от указанного в сообщении на подготовку области. Область не может быть подготовлена.';ru='Файл начальных данных, имеющийся в менеджере сервиса, отличается от указанного в сообщении на подготовку области. Область не может быть подготовлена.';en='initial data file available in the Service Manager differs from that specified in the report on the training field. The area can not be prepared.'");
			Возврат "ФатальнаяОшибка";
		КонецЕсли;
		...
		Если ИмяФайлаВыгрузки = Неопределено Тогда
			СообщениеОФатальнойОшибке = 
			НСтр("ka='სერვისის მენეჯერში საწყის მონაცემთა მოთხოვნილი ფაილი აღარ არის, სავარაუდოდ, იგი შეიცვალა. არეალი არ შეიძლება, რომ მომზადებული იქნას.';ru='В менеджере сервиса больше нет требуемого файла начальных данных, вероятно он был заменен. Область не может быть подготовлена.';en='The service manager is no longer required the initial data file, probably he was replaced. The area can not be prepared.'");
			Возврат Ложь;
		КонецЕсли;
...
Процедура КонтрольНеразделенныхДанныхПриЗаписи(Знач Источник)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ПредставлениеИсключения = НСтр("ka='დაშვების უფლებების დარღვევა!';ru='Нарушение прав доступа!';en='Access Violation!'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура КонтрольНеразделенныхДанныхПриОбновлении() Экспорт
	...
	КонецЕсли;
	// Kuzmich, 29.09.2021, Исправления для работы с расширениями
	...
	Если НарушенияКонтроляВключенияНеразделенныхДанныхВКонтролирующиеПодписки.Количество() > 0 Тогда
		...
		ВызываемыеИсключения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეტამონაცემთა ყველა ობიექტი, რომელიც არ შედის სქბ-ის გამყოფების შემადგენლობაში (%1), შეყვანილი უნდა იქნას იმ მოვლენების (%1) გამოწერის შემადგენლობაში, რომლებიც აკონტროლებენ განუცალკევებელ მონაცემთა განცალკევებეულ სეანსებში ჩაწერის შეუძლებლობას."
"მეტამონაცემთა შემდეგი ობიექტები ვერ აკმაყოფილებს ამ კრიტერიუმს: %3.';ru='Все объекты метаданных, не входящие в состав разделителей БСП (%1),"
...
"Следующие объекты метаданных не удовлетворяют этому критерию: %3.'"),
	...
	Если НарушенияКонтроляРазделенияОбъектовНесколькимиРазделителями.Количество() > 0 Тогда
		...
		ВызываемыеИсключения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კონფიგურაციის მეტამონაცემთა ყველა ობიექტი გაყოფილი უნდა იყოს სქბ-ის არა უმეტეს ერთი გამყოფით (%1)."
"შემდეგი ობიექტები ვერ აკმაყოფილებს ამ კრიტერიუმს: %2';ru='Все объекты метаданных конфигурации должны быть разделены не более чем одним разделителем БСП (%1)."
"Следующие объекты не удовлетворяют этому критерию: %2'"),
	...
	Если Не ПустаяСтрока(РезультирующееИсключение) Тогда
		...
		РезультирующееИсключение = НСтр("ka='Обнаружены ошибки в структуре метаданных конфигурации:';ru='Обнаружены ошибки в структуре метаданных конфигурации:';en='Errors found in the configuration metadata structure:'") + Символы.ПС + Символы.ВК + РезультирующееИсключение;
...
"%2 по порядку!';ka='Обнаружено нарушение структуры метаданных конфигурации: общий реквизит %1 должен"
"быть расположен в дереве метаданных конфигурации до общего реквизита"
"%2 по порядку!'"),
