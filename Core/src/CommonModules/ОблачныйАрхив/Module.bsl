...
Процедура СобратьДанныеПоОблачномуАрхиву(ДополнительныеСвойства, АдресХранилища = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Сбор данных в фоне';ru='Сбор данных в фоне'"));
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			НСтр("ka='Получение массива шагов';ru='Получение массива шагов'"));
		...
		Если МассивШагов.Количество() > 0 Тогда
			...
			ОписаниеРезультатаВложенный = СтрШаблон(НСтр("ka='Получено %1 шагов для выполнения';ru='Получено %1 шагов для выполнения'"), МассивШагов.Количество());
		Иначе
		...
		Для Каждого ТекущийШаг Из МассивШагов Цикл
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				ШагВыполненияВложенный,
				"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
				СтрШаблон(НСтр("ka='Выполнение шага: %1';ru='Выполнение шага: %1'"),ТекущийШаг.ОписаниеШага));
...
#Область ИнформацияОКлиенте
				...
				Иначе
					КодРезультатаВложенный      = 0;
					ОписаниеРезультатаВложенный = НСтр("ka='Собранные ранее данные еще не устарели';ru='Собранные ранее данные еще не устарели'");
				КонецЕсли;
...
"'"),
			...
			ДлительныеОперации.СообщитьПрогресс(
				(НомерШага / МассивШагов.Количество()) * 100,
				СтрШаблон(НСтр("ka='Выполнение шага: %1. Завершено.';ru='Выполнение шага: %1. Завершено.'"), ТекущийШаг.ОписаниеШага),
				Новый Структура("КодСостояния, ОписаниеРезультата",
					КодРезультатаВложенный,
					Неопределено));
	...
	ДлительныеОперации.СообщитьПрогресс(
		101, // > 100%
		НСтр("ka='СобратьДанныеПоОблачномуАрхиву. Завершено.';ru='СобратьДанныеПоОблачномуАрхиву. Завершено.'"),
		Новый Структура("КодСостояния, ОписаниеРезультата",
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // Имя события
		СтрШаблон(
			НСтр("ka='Облачный архив. Все обновления Облачного архива. %1';ru='Облачный архив. Все обновления Облачного архива. %1'"),
...
Функция ДоступнаАктивацияАгентаКопированияНаКомпьютере(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		СтрШаблон(НСтр("ka='Вызов процедуры / функции (%1)';ru='Вызов процедуры / функции (%1)'"), НаименованиеПроцедурыФункции));
...
"'"),
		...
		ШагВыполнения.ВремяНачала);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		СтрШаблон(
			НСтр("ka='Облачный архив. Все обновления Облачного архива. %1. Начало';ru='Облачный архив. Все обновления Облачного архива. %1. Начало'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)'"), "isMachineIDAvailable"));
			...
			Если (ТипЗнч(Результат) <> ТипОбъектXDTO) // Неправильный тип
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтрШаблон(
					НСтр("ka='Веб-сервис вернул значение неправильного типа: %1';ru='Веб-сервис вернул значение неправильного типа: %1'"),
					ТипЗнч(Результат));
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
				ЕстьОшибки = Истина;
			ИначеЕсли (ТипЗнч(Результат) = ТипОбъектXDTO)
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтрШаблон(
					НСтр("ka='Веб-сервис вернул ошибку с кодом: %1';ru='Веб-сервис вернул ошибку с кодом: %1'"),
					Результат.errorCode);
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
				ЕстьОшибки = Истина;
			КонецЕсли;
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			СтрШаблон(НСтр("ka='Отключение от веб-сервиса (%1)';ru='Отключение от веб-сервиса (%1)'"), "logout"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		СтрШаблон(
			НСтр("ka='Облачный архив. Все обновления Облачного архива. %1. Конец';ru='Облачный архив. Все обновления Облачного архива. %1. Конец'"),
			НаименованиеПроцедурыФункции), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
...
"'"),
	...
	Если (ДатаПредыдущегоЗапуска + 30) <= ТекущаяДатаДляРегистров Тогда // Запускалось предыдущий раз более 30 секунд назад.
		...
		Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)'"), "updateAccountStats"));
			...
			Если ЕстьОшибки <> Истина Тогда
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Сохранение информации в ИБ';ru='Сохранение информации в ИБ'"));
	...
	Иначе
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			...
			380,
			СтрШаблон(
				НСтр("ka='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)';ru='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)'"),
	...
	Иначе // Облачный архив
		...
		МассивШагов = Новый Массив;
			...
			ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
				...
				ШагСбораДанных.ОписаниеШага          = НСтр("ka='Сбор информации о клиентском компьютере';ru='Сбор информации о клиентском компьютере'");
			...
			ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
				...
				ШагСбораДанных.ОписаниеШага          = НСтр("ka='Проверка активации Агента резервного копирования на этом компьютере';ru='Проверка активации Агента резервного копирования на этом компьютере'");
				ШагСбораДанных.СрокУстареванияСекунд = 5*60; // Обновлять только если данные были собраны > 5 минут назад.
			МассивШагов.Добавить(ШагСбораДанных);
		...
		ФоновоеЗадание = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ka='Сбор данных для Облачного архива';ru='Сбор данных для Облачного архива'"),
			Ложь);
...
Процедура ПанельАдминистрированияБСП_ПереключитьНаПравильнуюСтраницу(Форма, СобратьДанныеОКлиенте = Ложь) Экспорт
	...
	ИначеЕсли Форма.СпособРезервногоКопирования = "1С:Облачный архив" Тогда
		...
		Если ИнформацияОКлиенте.ЭтоАдминистраторWindows Тогда
			...
			КонецЕсли;
		Иначе
			Элементы.ДекорацияОблачныйАрхивОшибки.Заголовок = НСтр("ka='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.';ru='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.'");
	...
	Если ОблачныйАрхивПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
		...
		Если НЕ МодульТекущиеДелаСервер.ДелоОтключено(ИдентификаторДела) Тогда
			...
			Если Результат.ТребуетсяУстановка = Истина Тогда
				...
				Дело.Важное         = Истина;
				Дело.Представление  = НСтр("ka='Для работы 1С:Облачный архив необходимо установить агента резервного копирования';ru='Для работы 1С:Облачный архив необходимо установить агента резервного копирования'");
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
				...
				Дело.Представление  = СтрШаблон(
					НСтр("ka='Доступно обновление %1 агента резервного копирования';ru='Доступно обновление %1 агента резервного копирования'"),
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 2 Тогда // Рекомендуется обновление.
				...
				Дело.Важное         = Ложь;
				Дело.Представление  = СтрШаблон(
					НСтр("ka='Рекомендуется обновление агента резервного копирования до версии %1';ru='Рекомендуется обновление агента резервного копирования до версии %1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 3 Тогда // Необходимо обновление.
				...
				Дело.Важное         = Истина;
				Дело.Представление  = СтрШаблон(
					НСтр("ka='Необходимо обновить агента резервного копирования до версии %1';ru='Необходимо обновить агента резервного копирования до версии %1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ka='Веб-сервисы авторизации в 1С';ru='Веб-сервисы авторизации в 1С'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ka='Веб-сервисы работы с облачным архивом (публичные и приватные)';ru='Веб-сервисы работы с облачным архивом (публичные и приватные)'"));
	НовыеРазрешения.Добавить(Разрешение);
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ka='Веб-сервисы информации об агенте копирования';ru='Веб-сервисы информации об агенте копирования'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ВыгрузитьФайлВОблако(ИмяФайла, ОтсрочкаСтартаСекунд = 30) Экспорт
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Чтение настроек Агента резервного копирования';ru='Чтение настроек Агента резервного копирования'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		МассивШагов.Добавить(ШагСбораДанных);
...
Функция ПолучитьПроксиВебСервисаАвторизации1С(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Подключение к прокси веб-сервиса (авторизация 1С)';ru='Подключение к прокси веб-сервиса (авторизация 1С)'"));
...
"%1'"),
		...
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ОблачныйАрхив.Веб-сервисы';ru='БИП:ОблачныйАрхив.Веб-сервисы'"), // Имя события
			НСтр("ka='Облачный архив. Веб-сервисы. Подключение к серверу';ru='Облачный архив. Веб-сервисы. Подключение к серверу'"), // ИмяСобытия
...
Функция ПолучитьПроксиВебСервисаБэкап1СПубличный(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Подключение к прокси веб-сервиса (публичный backup.1c.ru)';ru='Подключение к прокси веб-сервиса (публичный backup.1c.ru)'"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из публичного веб-сервиса (%1)';ru='Загрузка информации из публичного веб-сервиса (%1)'"), "getBackupAgentInfo"));
...
Функция ПолучитьПроксиВебСервисаБэкап1СПриватный(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если (ТипЗнч(ПроксиЛогин1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='ტიკეტის მიღება';ru='Получение тикета'"));
		...
		Если ЕстьОшибки <> Истина Тогда
			...
			Если (ТипЗнч(ПроксиПубличныйБэкап1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					СтрШаблон(НСтр("ka='Загрузка информации из публичного веб-сервиса (%1)';ru='Загрузка информации из публичного веб-сервиса (%1)'"), "login"));
				...
				Если ЕстьОшибки <> Истина Тогда
					...
					ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
						ШагВыполнения,
						"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
						НСтр("ka='Подключение к прокси веб-сервиса (приватный backup.1c.ru)';ru='Подключение к прокси веб-сервиса (приватный backup.1c.ru)'"));
...
Процедура ЗагрузитьИнформациюСтатусаАккаунта(КонтекстВыполнения = Неопределено)
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)'"), "getAccountInfo"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)'"), "getAccountStats"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)'"), "activateAccount"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)'"), "getFileList"));
...
Процедура УдалитьРезервнуюКопию(КонтекстВыполнения) Экспорт
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Удаление файла с сервера';ru='Удаление файла с сервера'"));
			...
			Попытка
				...
				ОписаниеРезультата = СтрШаблон(
					НСтр("ka='Удаление файла с UIN %1 выполнено успешно';ru='Удаление файла с UIN %1 выполнено успешно'"),
					КонтекстВыполнения.ИдентификаторФайла);
			Исключение
				Результат = -1;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
			...
			ИначеЕсли (ТипЗнч(Результат) = ТипЧисло) Тогда
				Если Результат = 0 Тогда
					// Файл удален
					ОписаниеРезультата = НСтр("ka='Файл резервной копии с идентификатором %1 успешно удален с сервера';ru='Файл резервной копии с идентификатором %1 успешно удален с сервера'");
					ОписаниеРезультата = СтрШаблон(
						НСтр("ka='Файл резервной копии с идентификатором %1 успешно удален с сервера.';ru='Файл резервной копии с идентификатором %1 успешно удален с сервера.'"),
						КонтекстВыполнения.ИдентификаторФайла);
				Иначе // Ошибка
					ОписаниеРезультата = СтрШаблон(
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Веб-сервисы';ru='БИП:ОблачныйАрхив.Веб-сервисы'"), // Имя события
		НСтр("ka='Облачный архив. Веб-сервисы. Удаление файла резервной копии';ru='Облачный архив. Веб-сервисы. Удаление файла резервной копии'"), // ИмяСобытия
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
...
#Область ПолучениеСпискаОСиВерсий
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Получение списка ОС и версий ОС';ru='Получение списка ОС и версий ОС'"));
...
#Область ПреобразованиеСпискаОСиВерсий
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Преобразование списка ОС и версий ОС';ru='Преобразование списка ОС и версий ОС'"));
...
#Область ЗагрузкаДанных
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполнения,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			НСтр("ka='Загрузка данных с серверов 1С';ru='Загрузка данных с серверов 1С'"));
...
#Область ПарсингПолученныхДанных
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Парсинг загруженных данных';ru='Парсинг загруженных данных'"));
			...
			Если (ЗагруженныеДанные.Свойство("backupAgentInfoList"))
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Парсинг загруженных данных, загрузка файлов описаний';ru='Парсинг загруженных данных, загрузка файлов описаний'"));
...
"%1'"),
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Парсинг загруженных данных, сохранение данных в ИБ';ru='Парсинг загруженных данных, сохранение данных в ИБ'"));
...
Процедура ВсеОбновленияОблачногоАрхива() Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Выполнение регламентных заданий подсистемы Облачный архив';ru='Выполнение регламентных заданий подсистемы Облачный архив'"));
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Все обновления Облачного архива. Начало';ru='Облачный архив. Все обновления Облачного архива. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
			НСтр("ka='Облачный архив. Все обновления Облачного архива. Конец';ru='Облачный архив. Все обновления Облачного архива. Конец'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияОблачногоАрхива, // ОбъектМетаданных
			, // Данные
...
#Область ЗагрузкаИнформацииОбАктуальныхВерсияхАгентаКопирования
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='1. %1';ru='1. %1'"), ВидОбновления));
...
#Область ЗагрузкаИнформацииОбАктивированныхАгентахКопирования
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='2. %1';ru='2. %1'"), ВидОбновления));
...
#Область ЗагрузкаСпискаРезервныхКопий
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='3. %1';ru='3. %1'"), ВидОбновления));
...
#Область ЗагрузкаИнформацииОСостоянииИспользованияОблачногоАрхива
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='4. %1';ru='4. %1'"), ВидОбновления));
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Если Использовать = Истина Тогда
		...
		ТекущееРегламентноеЗадание.Наименование                             = НСтр("ka='Все обновления Облачного архива';ru='Все обновления Облачного архива'");
...
"в параметре Использовать передано значение неправильного типа: %1'"),
			ТипЗнч(Использовать));
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
			НСтр("ka='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';ru='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
#Область ОсновноеОбновление
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='ძირითადი განახლება';ru='Основное обновление'"));
...
#Область ОсновноеОбновление_ЗагрузитьВсеСтандартныеЗначения
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Основное обновление, загрузка стандартных значений';ru='Основное обновление, загрузка стандартных значений'"));
...
#Область ПереопределяемоеОбновление
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='ხელახლა განსაზღვრული განახლება';ru='Переопределяемое обновление'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#Область ОсновноеОбновление_ИзменитьИспользованиеРегламентныхЗаданий
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Основное обновление, установка расписания';ru='Основное обновление, установка расписания'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Функция СобратьДанные_ИнформацияОКлиенте(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Обновление информации о клиенте';ru='Обновление информации о клиенте'"));
...
#Область Шаг_ПолучениеДанныхИзРеестраWindows
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)';ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)'"));
...
#Область Шаг_ПроверкаДанныхОбАгенте_Ключи
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Проверка наличия файлов программы и файлов ключей';ru='Проверка наличия файлов программы и файлов ключей'"));
...
"'"),
			...
			Если ЕстьОшибки = Истина Тогда
				...
				ОписаниеРезультатаШага = ОписаниеРезультатаШага
					+ НСтр("ka='Отсутствуют файлы, необходимые для работы Агента резервного копирования';ru='Отсутствуют файлы, необходимые для работы Агента резервного копирования'");
				КонтекстВыполнения.КодРезультата = КонтекстВыполненияВложенный.КодРезультата; // Установить параметры корневого контекста выполнения
		...
		Иначе
			...
			ОписаниеРезультатаШага = ОписаниеРезультатаШага
				+ СтрШаблон(
					НСтр("ka='Отсутствует каталог установки Агента резервного копирования (%1)';ru='Отсутствует каталог установки Агента резервного копирования (%1)'"),
...
Функция СобратьДанные_НастройкиАгентаКопированияКлиент(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Обновление информации о настройках агента копирования';ru='Обновление информации о настройках агента копирования'"));
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Каталоги
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Получение настроек Агента резервного копирования';ru='Получение настроек Агента резервного копирования'"));
		...
		Если Каталог.Существует() Тогда
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				...
				0,
				СтрШаблон(
					НСтр("ka='Каталог настроек Агента копирования обнаружен: %1';ru='Каталог настроек Агента копирования обнаружен: %1'"),
					ИмяКаталогаНастроек),
				Неопределено);
			КонтекстВыполненияВложенный.ЛогШаговВыполнения.Добавить(ШагВыполненияВложенный);
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Проверка каталогов для работы Агента резервного копирования';ru='Проверка каталогов для работы Агента резервного копирования'"));
				...
				КаталогиДляПроверки.Добавить(ИмяКаталогаЛогов, НСтр("ka='ლოგოების კატალოგი';ru='Каталог логов'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаСтатистики, НСтр("ka='სტატისტიკის კატალოგი';ru='Каталог статистики'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаРабочий, НСтр("ka='სამუშაო კატალოგი';ru='Рабочий каталог'"));
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Загрузка настроек этой ИБ из рабочего каталога';ru='Загрузка настроек этой ИБ из рабочего каталога'"));
	...
	#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание_Ежедневно
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("Time") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("ka='Найдена настройка расписания Time.';ru='Найдена настройка расписания Time.'")
												+ Символы.ПС;
...
#КонецОбласти
		...
		Иначе
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				...
				124,
				СтрШаблон(
					НСтр("ka='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.';ru='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.'"),
