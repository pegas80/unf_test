...
Функция ПолучитьПредставлениеДняНедели(ДеньНеделиКалендаря) Экспорт
	...
	Если НомерДняНедели = 1 Тогда
		...
		Возврат НСтр("ka='ორშ';ru='Пн'");
		
	ИначеЕсли НомерДняНедели = 2 Тогда
		
		Возврат НСтр("ka='ვატი';ru='Вт'");
		
	ИначеЕсли НомерДняНедели = 3 Тогда
		
		Возврат НСтр("ka='ოთხშაბათი';ru='Ср'");
		
	ИначеЕсли НомерДняНедели = 4 Тогда
		
		Возврат НСтр("ka='ხუთშაბათი';ru='Чт'");
		
	ИначеЕсли НомерДняНедели = 5 Тогда
		
		Возврат НСтр("ka='პარასკ.';ru='Пт'");
		
	ИначеЕсли НомерДняНедели = 6 Тогда
		
		Возврат НСтр("ka='შაბათი';ru='Сб'");
		
	Иначе
		
		Возврат НСтр("ka='კვირა დღე';ru='Вс'");
...
Процедура ОткрытьСчетФактуру(ФормаДокумента, Полученный = Ложь) Экспорт
	...
	Если ФормаДокумента.Объект.ПометкаУдаления 
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='ანგარიშფაქტურის შეყვანა არ შეიძლება წასაშლელად მონიშნული დოკუმენტის საფუძველზე!';ru='Счет-фактуру нельзя вводить на основании документа, помеченного на удаление!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФормаДокумента.Объект.Ссылка) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='დოკუმენტი არ ჩაწერილა. თავდაპირველად, საჭიროა დოკუმენტის ჩაწერა!';ru='Документ не записан. Сначала следует записать документ!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Если ПараметрКоманды.Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента" Тогда
		...
		Если НЕ ВсеРедактируемыеПараметрыЗаполнены Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ПечатьДоговорКонтрагентаВопрос", ЭтотОбъект,
			               ...
			               НСтр("ka='ხელით რედაქტირებადი ყველა ველი არ არის შევსებული, გავაგრძელოთ ამობეჭდვა?';ru='Не все редактируемые вручную поля заполнены, продолжить печать?'"), РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ОбъектФормы) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		ВызватьИсключение СтрЗаменить(НСтр("ka='საკონტაქტო ინფორმაციის თაობაზე წერილის შედგენა ტიპ ""%1""-ით არ შეიძლება';ru='Нельзя создать письмо по контактной информацию с типом ""%1""'"),
			"%1", ТипИнформации);
	КонецЕсли;
	...
	Если ТипЗнч(АдресПочты) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ka='ელექტრონული ფოსტის მისამართის მიღების შეცდომა, საკონტაქტო ინფორმაციის არასწორი ტიპი';ru='Ошибка получения адреса электронной почты, неверный тип контактной информации'");
	КонецЕсли;
	...
	ТекстПредложения = НСтр("ka='წერილის გასაგზავნად აუცილებელია გაფართოების დაყენება ფაილებთან სამუშაოდ.';ru='Для отправки письма необходимо установить расширение для работы с файлами.'");
...
Процедура ДействияПоВыборуФайла(ДополнительныеПараметры)
	...
	Диалог.Заголовок = НСтр("ka='ჩასატვირთად აირჩიეთ ფაილი...';ru='Выберите файл для загрузки...'");
	Диалог.Фильтр = НСтр("ka='1С-სთან გაცვლის ფაილები (*.txt)|*.txt|ყველა ფაილი (*.*)|*.*';ru='Файлы обмена с 1С (*.txt)|*.txt|Все файлы (*.*)|*.*'");
...
Функция ПолучитьТекстФайла(ДополнительныеПараметры, ИмяФайлаИзНастройки, ОдиночныйРазделительСтрок = Ложь)
	...
	Исключение
		...
		Если ДополнительныеПараметры.Свойство("ВыполнитьДействияЕслиФайлНеПрочитан") И ДополнительныеПараметры.ВыполнитьДействияЕслиФайлНеПрочитан Тогда
			ТекстСообщения = НСтр("ka='ფაილ %Файл%-ის წაკითხვის შეცდომა.';ru='Ошибка чтения файла %Файл%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Файл%", ИмяФайла);
			Если ИмяФайлаИзНастройки Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='შეიძლება, რომ გაცვლის გამართვებში მითითებული ფაილი არ არსებობს.';ru='Возможно файл, который указан в настройках обмена, не существует.'");
			КонецЕсли;
			ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='აირჩიეთ სხვა ფაილი.';ru='Выберите другой файл.'");
		...
		Иначе
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Файл%", ИмяФайла);
			Если ИмяФайлаИзНастройки Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='შეიძლება, რომ გაცვლის გამართვებში მითითებული ფაილი არ არსებობს. სხვა ფაილის არჩევა შეიძლება გამართვებში (მარჯვენა, ზემო კუთხეში არსებული ღილაკი).';ru='Возможно файл, который указан в настройках обмена, не существует. Выбрать другой файл можно в настройках (кнопка в правом верхнем углу).'");
			КонецЕсли;
...
Процедура ОткрытьЭлНакладную(ФормаДокумента, СтруктураПараметровНайденныйЭлНакладной = Неопределено) Экспорт
	...
	Если ФормаДокумента.Объект.ПометкаУдаления 
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='წაშლაზე მონიშნული დოკუმენტის საფუძველზე არ შეიძლება ელექტრონული ზედნაგების შექმნა!';ru='Электронная накладная нельзя вводить на основании документа, помеченного на удаление!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Иначе
		...
		Если ТипЗнч(ТекОбъект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная") И РС_ОбщегоНазначения.ПолучитьЗначениеРеквизита(ТекОбъект.СтруктурнаяЕдиница, "МобильныйСклад") Тогда
			...
			Если КоличетсвоАктивныхТН = 0 Тогда
				ТекстОшибки = НСтр("ka='ქვე-ზედნადების შესაქმნელად აუცილებელია აქტიური დისტრიბუციის ზედნადების არსებობა მობილური საწყობისთვის ""%Склад%""!';ru='Для создания подниненного ТТН, требуется наличие активного ТТН дестрибюции для мобильного склада ""%Склад%""!'");
