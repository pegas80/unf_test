...
//
Функция ПроверитьВозможностьПечатиЧека(Форма, ПоказыватьПредупреждение = Ложь, ТекстОшибки = "") Экспорт
...
Функция ПолучитьПредставлениеДняНедели(ДеньНеделиКалендаря) Экспорт
	...
	Если НомерДняНедели = 1 Тогда
		...
		Возврат НСтр("ka='ორშ';ru='Пн';en='Mon'");
		
	ИначеЕсли НомерДняНедели = 2 Тогда
		
		Возврат НСтр("ka='ვატი';ru='Вт';en='W'");
		
	ИначеЕсли НомерДняНедели = 3 Тогда
		
		Возврат НСтр("ka='ოთხშაბათი';ru='Ср';en='cf.'");
		
	ИначеЕсли НомерДняНедели = 4 Тогда
		
		Возврат НСтр("ka='ხუთშაბათი';ru='Чт';en='Th'");
		
	ИначеЕсли НомерДняНедели = 5 Тогда
		
		Возврат НСтр("ka='პარასკ.';ru='Пт';en='Fri'");
		
	ИначеЕсли НомерДняНедели = 6 Тогда
		
		Возврат НСтр("ka='შაბათი';ru='Сб';en='Sat.'");
		
	Иначе
		
		Возврат НСтр("ka='კვირა დღე';ru='Вс';en='Sun'");
...
Процедура ПроставитьСчетЗатратПоУмолчанию(ФормаДокумента, СтруктурнаяЕдиница = Неопределено) Экспорт
	...
	СтруктураПараметров = Новый Структура("СчетЗатрат, СчетЗатратРегл, ТипСчета");
	...
	Если ЗначениеЗаполнено(ДанныеТекущейСтроки.ВидНачисленияУдержания) Тогда
		...
		Попытка // временно
			ДанныеТекущейСтроки.СчетЗатратРегл = СтруктураПараметров.СчетЗатратРегл;
		Исключение
		КонецПопытки;
...
//                 которой обрабатываются процедурой.
//
Процедура ПроставитьСчетЗатратПоУмолчаниюРегл(ФормаДокумента, СтруктурнаяЕдиница = Неопределено) Экспорт  //+++ Дмгг
	...
	Если ЗначениеЗаполнено(ДанныеТекущейСтроки.ВидНачисленияУдержания) Тогда
		...
		УправлениеНебольшойФирмойСервер.ПолучитьСчетЗатратВидаНачисленияРегл(СтруктураПараметров);
		ДанныеТекущейСтроки.СчетЗатрат = СтруктураПараметров.СчетЗатрат;
		Попытка // временно
			ДанныеТекущейСтроки.СчетЗатратРегл = СтруктураПараметров.СчетЗатратРегл;
		Исключение
...
Процедура ПриИзмененииПериодаРегистрации(ПереданнаяФорма) Экспорт
	...
	Иначе
		...
		ПереданнаяФорма.ОтображениеПериодаРегистрации 	= Формат(ПереданнаяФорма.Объект.ПериодРегистрации, НСтр("ru = 'Л=ru_RU'; ka = 'Л=ka_GE'; en = 'Л=en_GB'") + "; ДФ='MMMM yyyy'");
		//--- Дмгг, 01.05.2025, ONESOFT-4039
	КонецЕсли;
...
Процедура ПересчитатьЦеныТабличнойЧастиПоВалюте(ФормаДокумента, ПредыдущаяВалюта, ИмяТабличнойЧасти) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из ФормаДокумента.Объект[ИмяТабличнойЧасти] Цикл
		//Паганини_ Сумма НДФЛ	
		Если СтрокаТабличнойЧасти.Свойство("СуммаНДФЛ") Тогда
			...
			СтрокаТабличнойЧасти.СуммаНДС = ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаНДС, 
																			  СтруктураКурсы.КурсНач, 
																			  СтруктураКурсы.Курс, 
																			  СтруктураКурсы.КратностьНач, 
																			  СтруктураКурсы.Кратность);
			
			СтрокаТабличнойЧасти.СуммаНДФЛ = ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаНДФЛ, 
																		СтруктураКурсы.КурсНач, 
																		СтруктураКурсы.Курс, 
																		СтруктураКурсы.КратностьНач, 
																		СтруктураКурсы.Кратность);
			СтрокаТабличнойЧасти.СуммаБезНалогов = ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаБезНалогов, 
																			  СтруктураКурсы.КурсНач, 
																			  СтруктураКурсы.Курс, 
																			  СтруктураКурсы.КратностьНач, 
																			  СтруктураКурсы.Кратность);
																			  
			СтрокаТабличнойЧасти.Всего = ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.Всего, 
																			  СтруктураКурсы.КурсНач, 
																			  СтруктураКурсы.Курс, 
																			  СтруктураКурсы.КратностьНач, 
																			  СтруктураКурсы.Кратность);
			Если СтрокаТабличнойЧасти.Свойство("ПроцентСкидкиНаценки") Тогда
				Если СтрокаТабличнойЧасти.ПроцентСкидкиНаценки = 0 Тогда
					СтрокаТабличнойЧасти.Цена 	= Окр(?(СтрокаТабличнойЧасти.Количество = 0, 0, Окр(СтрокаТабличнойЧасти.Сумма/СтрокаТабличнойЧасти.Количество, 2)), 2);
				Иначе
					пСтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтрокаТабличнойЧасти.СтавкаНДС);
					//СтрокаТабличнойЧасти.Цена	= Окр(?(ФормаДокумента.Объект.СуммаВключаетНДС, СтрокаТабличнойЧасти.СуммаБезНалогов * (100+пСтавкаНДС)/100, СтрокаТабличнойЧасти.СуммаБезНалогов)*100/СтрокаТабличнойЧасти.ПроцентСкидкиНаценки, 2);
				КонецЕсли;
			Иначе
				СтрокаТабличнойЧасти.Цена 	= Окр(?(СтрокаТабличнойЧасти.Количество = 0, 0, Окр(СтрокаТабличнойЧасти.Сумма/СтрокаТабличнойЧасти.Количество, 2)), 2);
			КонецЕсли;
			Продолжить;//Возврат; //+++ Дмгг, Нужно пересчитать все строки
		КонецЕсли;
		//_Паганини
...
//Soniva 27.06.2022
Процедура ПересчитатьЦеныТабличнойЧастиПоДиапазонуКоличестваНоменклатуры(ФормаДокумента, ИмяТабличнойЧасти, ПересчитатьСкидки = Ложь, ПересчитыватьЦеныСЗаказом = Истина) Экспорт
	...
	СтруктураДанных.Вставить("ВалютаДокумента",		ФормаДокумента.Объект.ВалютаДокумента);
	СтруктураДанных.Вставить("СуммаВключаетНДС",	ФормаДокумента.Объект.СуммаВключаетНДС);
	СтруктураДанных.Вставить("ДатаОбработки",	 	ФормаДокумента.Объект.Дата);
	СтруктураДанных.Вставить("Коэффициент", 1);
	СтруктураДанных.Вставить("ВидСкидкиНаценки", 	ФормаДокумента.Объект.ВидСкидкиНаценки);
	СтруктураДанных.Вставить("ДисконтнаяКарта", 	ФормаДокумента.Объект.ДисконтнаяКарта);
	Если ФормаДокумента.Объект.свойство("КартаЛояльности") Тогда 
		СтруктураДанных.Вставить("КартаЛояльности", 	ФормаДокумента.Объект.КартаЛояльности);
	КонецЕсли;
	СтруктураДанных.Вставить("ПроцентСкидкиПоДисконтнойКарте",ФормаДокумента.Объект.ПроцентСкидкиПоДисконтнойКарте);
	...
	Для каждого СтрокаТЧ Из ФормаДокумента.Объект[ИмяТабличнойЧасти] Цикл
		...
		СтруктураДанных.Вставить("Характеристика",СтрокаТЧ.Характеристика);
				
		ВидЦен = УправлениеНебольшойФирмойСервер.ПолучитьВидЦеныПоДиапазонуКоличестваНоменклатуры(СтруктураДанных,СтрокаТЧ.Количество);		
		...
		СтруктураДанные = ФормаДокумента.ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанных);
		
		Если  СтруктураДанные.Цена > 0 Тогда 		
			СтрокаТЧ.Цена = СтруктураДанные.Цена;	
			ФормаДокумента.РассчитатьСуммуВСтрокеТабличнойЧасти();
		КонецЕсли;
		...
		//СтрокаТЧ.ЕдиницаИзмерения = СтруктураДанные.ЕдиницаИзмерения;
		//СтрокаТЧ.СтранаПроисхождения = СтруктураДанные.СтранаПроисхождения;
		//СтрокаТЧ.НомерГТД = Неопределено;
		//СтрокаТабличнойЧасти.Количество = 1;//Паганини
		//СтрокаТЧ.Цена = СтруктураДанные.Цена;
		//СтрокаТЧ.ПроцентСкидкиНаценки = СтруктураДанные.ПроцентСкидкиНаценки;
		////+++ Дмгг
...
Процедура ПерезаполнитьСкидкиТабличнойЧастиПослеСчитыванияДисконтнойКарты(ФормаДокумента, ИмяТабличнойЧасти) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из ФормаДокумента.Объект[ИмяТабличнойЧасти] Цикл
	Скидка = УправлениеНебольшойФирмойСервер.ПолучитьПроцентСкдикиПоВидуСкидокНаценок(ФормаДокумента.Объект.ВидСкидкиНаценки) + СтрокаТабличнойЧасти.ПроцентСкидкиНаценки;
...
Процедура ОткрытьСчетФактуру(ФормаДокумента, Полученный = Ложь) Экспорт
	...
	Если ФормаДокумента.Объект.ПометкаУдаления 
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='ანგარიშფაქტურის შეყვანა არ შეიძლება წასაშლელად მონიშნული დოკუმენტის საფუძველზე!';ru='Счет-фактуру нельзя вводить на основании документа, помеченного на удаление!';en='An invoice can not be administered on the basis of the document marked for deletion!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФормаДокумента.Объект.Ссылка) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='დოკუმენტი არ ჩაწერილა. თავდაპირველად, საჭიროა დოკუმენტის ჩაწერა!';ru='Документ не записан. Сначала следует записать документ!';en='The document is not recorded. First, you must write the document!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Если ПараметрКоманды.Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента" Тогда
		...
		Если НЕ ВсеРедактируемыеПараметрыЗаполнены Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ПечатьДоговорКонтрагентаВопрос", ЭтотОбъект,
			               ...
			               НСтр("ka='ხელით რედაქტირებადი ყველა ველი არ არის შევსებული, გავაგრძელოთ ამობეჭდვა?';ru='Не все редактируемые вручную поля заполнены, продолжить печать?';en='Not all fields are filled in manually editable, continue printing?'"), РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ОбъектФормы) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		ВызватьИсключение СтрЗаменить(НСтр("ka='საკონტაქტო ინფორმაციის თაობაზე წერილის შედგენა ტიპ ""%1""-ით არ შეიძლება';ru='Нельзя создать письмо по контактной информацию с типом ""%1""';en='You can not create a letter using the contact information with the type ""%1""'"),
			"%1", ТипИнформации);
	КонецЕсли;
	...
	Если ТипЗнч(АдресПочты) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ka='ელექტრონული ფოსტის მისამართის მიღების შეცდომა, საკონტაქტო ინფორმაციის არასწორი ტიპი';ru='Ошибка получения адреса электронной почты, неверный тип контактной информации';en='Failed to get the email address, the wrong type of contact information'");
	КонецЕсли;
	...
	ТекстПредложения = НСтр("ka='წერილის გასაგზავნად აუცილებელია გაფართოების დაყენება ფაილებთან სამუშაოდ.';ru='Для отправки письма необходимо установить расширение для работы с файлами.';en='To send a letter, you must install the extension for the files.'");
...
Процедура ДействияПоВыборуФайла(ДополнительныеПараметры)
	...
	Диалог.Заголовок = НСтр("ka='ჩასატვირთად აირჩიეთ ფაილი...';ru='Выберите файл для загрузки...';en='Select file to upload ...'");
	Диалог.Фильтр = НСтр("ka='1С-სთან გაცვლის ფაილები (*.txt)|*.txt|ყველა ფაილი (*.*)|*.*';ru='Файлы обмена с 1С (*.txt)|*.txt|Все файлы (*.*)|*.*';en='exchange files with 1C (* .txt) | * .txt | All files (* *.) | * *.'");
...
Функция ПолучитьТекстФайла(ДополнительныеПараметры, ИмяФайлаИзНастройки, ОдиночныйРазделительСтрок = Ложь)
	...
	Исключение
		...
		Если ДополнительныеПараметры.Свойство("ВыполнитьДействияЕслиФайлНеПрочитан") И ДополнительныеПараметры.ВыполнитьДействияЕслиФайлНеПрочитан Тогда
			ТекстСообщения = НСтр("ka='ფაილ %Файл%-ის წაკითხვის შეცდომა.';ru='Ошибка чтения файла %Файл%.';en='Error reading file% file%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Файл%", ИмяФайла);
			Если ИмяФайлаИзНастройки Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='შეიძლება, რომ გაცვლის გამართვებში მითითებული ფაილი არ არსებობს.';ru='Возможно файл, который указан в настройках обмена, не существует.';en='Perhaps the file that is specified in the sharing settings, does not exist.'");
			КонецЕсли;
			ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='აირჩიეთ სხვა ფაილი.';ru='Выберите другой файл.';en='Select another file.'");
		...
		Иначе
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Файл%", ИмяФайла);
			Если ИмяФайлаИзНастройки Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='შეიძლება, რომ გაცვლის გამართვებში მითითებული ფაილი არ არსებობს. სხვა ფაილის არჩევა შეიძლება გამართვებში (მარჯვენა, ზემო კუთხეში არსებული ღილაკი).';ru='Возможно файл, который указан в настройках обмена, не существует. Выбрать другой файл можно в настройках (кнопка в правом верхнем углу).';en='Perhaps the file that is specified in the sharing settings, does not exist. You can select another file in the settings (button in the top right corner).'");
			КонецЕсли;
...
// Устанавливает надпись гиперссылки на Счет-фактуру
//
Процедура ОткрытьЭлНакладную(ФормаДокумента, СтруктураПараметровНайденныйЭлНакладной = Неопределено) Экспорт
	
	НайденныйЭлНакладная = УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйЭлНакладная(ФормаДокумента.Объект.Ссылка);
	
	Если ФормаДокумента.Объект.ПометкаУдаления 
		И НЕ ЗначениеЗаполнено(НайденныйЭлНакладная) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='წაშლაზე მონიშნული დოკუმენტის საფუძველზე არ შეიძლება ელექტრონული ზედნაგების შექმნა!';ru='Электронная накладная нельзя вводить на основании документа, помеченного на удаление!';en='The electronic consignment note can not be administered on the basis of the document marked for deletion!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	ИмяФормы = "Документ.РС_ЭлектроннаяНакладная.ФормаОбъекта";
	
	// Откроем или введем новый документ
	Если СтруктураПараметровНайденныйЭлНакладной = Неопределено Тогда
		СтруктураПараметровНайденныйЭлНакладной = Новый Структура;
	КонецЕсли;
	Если ЗначениеЗаполнено(НайденныйЭлНакладная) Тогда
		
		СтруктураПараметровНайденныйЭлНакладной.Вставить("Ключ", НайденныйЭлНакладная.Ссылка);
		
	Иначе
		//Паганини_
		ТекОбъект = ФормаДокумента.Объект;
		Если ТипЗнч(ТекОбъект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная") И РС_ОбщегоНазначения.ПолучитьЗначениеРеквизита(ТекОбъект.СтруктурнаяЕдиница, "МобильныйСклад") Тогда
			СписокДокументов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьСписокОткрытыхДистрибюции(ТекОбъект.СтруктурнаяЕдиница);
			КоличетсвоАктивныхТН = СписокДокументов.Количество();
			Если КоличетсвоАктивныхТН = 0 Тогда
				ТекстОшибки = НСтр("ka='ქვე-ზედნადების შესაქმნელად აუცილებელია აქტიური დისტრიბუციის ზედნადების არსებობა მობილური საწყობისთვის ""%Склад%""!';ru='Для создания подниненного ТТН, требуется наличие активного ТТН дестрибюции для мобильного склада ""%Склад%""!';en='To create podninennogo Waybill is required for an active Waybill destribyutsii mobile warehouse ""Warehouse%%!""'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Склад%", ТекОбъект.СтруктурнаяЕдиница);
				ВызватьИсключение ТекстОшибки;
			ИначеЕсли КоличетсвоАктивныхТН = 1 Тогда
				Для Каждого СтрокаС ИЗ СписокДокументов Цикл
					СтруктураПараметровНайденныйЭлНакладной.Вставить("НакладнаяДистрибюций", СтрокаС.Ключ);
					СтруктураПараметровНайденныйЭлНакладной.Вставить("СсылкаЭН", 			 СтрокаС.Значение);
					Прервать;
				КонецЦикла;
			ИначеЕсли КоличетсвоАктивныхТН > 1 Тогда
				МассивПеремещении = Новый Массив;
				Для Каждого СтрокаС ИЗ СписокДокументов Цикл
					МассивПеремещении.Добавить(СтрокаС.Ключ);
				КонецЦикла;
				СтруктураОтбора = Новый Структура;
				СтруктураОтбора.Вставить("Ссылка", Новый ФиксированныйМассив(МассивПеремещении));
				ВыбранныйДокумент = ОткрытьФормуМодально("Документ.ПеремещениеЗапасов.Форма.ФормаВыбора", Новый Структура("Отбор", СтруктураОтбора));
				Если ВыбранныйДокумент = Неопределено Тогда
					Возврат;
				КонецЕсли;
				СтруктураПараметровНайденныйЭлНакладной.Вставить("НакладнаяДистрибюций", ВыбранныйДокумент);
				СтруктураПараметровНайденныйЭлНакладной.Вставить("СсылкаЭН", 			 СписокДокументов.Получить(ВыбранныйДокумент));
			КонецЕсли;
		КонецЕсли;
		//_Паганини
		
		СтруктураПараметровНайденныйЭлНакладной.Вставить("Основание", ФормаДокумента.Объект.Ссылка);
		
	КонецЕсли;
	
	ОткрытьФорму(ИмяФормы, СтруктураПараметровНайденныйЭлНакладной, ФормаДокумента);
	
КонецПроцедуры // ЗаполнитьТекстПроСчетФактуру() 

Процедура ОткрытьНалоговыйДокумент(ФормаДокумента, СтруктураПараметровНайденныйЭлНакладной = Неопределено) Экспорт
	
	НайденныйЭлНакладная = УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйНалоговыйДокумент(ФормаДокумента.Объект.Ссылка);
	
	Если ФормаДокумента.Объект.ПометкаУдаления 
		И НЕ ЗначениеЗаполнено(НайденныйЭлНакладная) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru = 'Налоговый документ нельзя вводить на основании документа, помеченного на удаление!'; ka = 'წაშლაზე მონიშნული დოკუმენტის საფუძველზე არ შეიძლება საგადასახადო დოკუმენტის შექმნა!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Если ФормаДокумента.Модифицированность Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("en='The document has been changed. First, you must write the document!';ka='დოკუმენტი შეიცვალა. თავდაპირველად, საჭიროა დოკუმენტის ჩაწერა!';ru='Документ был изменен. Сначала следует записать документ!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФормаДокумента.Объект.Ссылка) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("en='The document is not recorded. First, you must write the document!';ka='დოკუმენტი არ ჩაწერილა. თავდაპირველად, საჭიროა დოკუმენტის ჩაწერა!';ru='Документ не записан. Сначала следует записать документ!'");	
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;
	
	ИмяФормы = "Документ.РС_НалоговыйДокумент.ФормаОбъекта";
...
// Устанавливает надпись гиперссылки на Счет-фактуру
//
Процедура ОткрытьСчетФактуруГрузия(ФормаДокумента, Полученный = Ложь) Экспорт
	
	НайденныйСчетФактура = УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйСчетФактуруГрузия(ФормаДокумента.Объект.Ссылка, Полученный);
	...
	Если Полученный Тогда
		ИмяФормы = "Документ.СчетФактураПолученныйГрузия.ФормаОбъекта";
	Иначе
		ИмяФормы = "Документ.СчетФактураОтправленныйГрузия.ФормаОбъекта";
	КонецЕсли;
