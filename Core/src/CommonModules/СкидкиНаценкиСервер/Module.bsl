	...
	Для Каждого СтрокаДерева Из ДеревоСкидок.Строки Цикл
		...
		Иначе
			...
			Если КоличествоСтрок > 1 Тогда
				ВызватьИсключение НСтр("ka='ფასდაკლებების ხის გენერაციის შეცდომა';ru='Ошибка генерации дерева скидок';en='Error generating tree discounts'");
			КонецЕсли;
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаДанных Цикл
		Если НЕ СтрокаТаблицы.КлючСвязи = 0  Тогда
			Для Каждого СтрокаСкидкиНаценки Из СтрокаТаблицы.Расшифровка Цикл
				...
				//Малышев 2024-10-25 ONESOFT-3539 (--
				Если Параметры.Свойство("Контрагент") и СтрокаСкидкиНаценки.СкидкаНаценка.ПолучателиСкидкиКонтрагенты.Количество()>0 Тогда
					Если СтрокаСкидкиНаценки.СкидкаНаценка.ПолучателиСкидкиКонтрагенты.Найти(Параметры.Контрагент, "Получатель")=Неопределено Тогда
						Продолжить;
					КонецЕсли;	
				КонецЕсли;	
				//Малышев 2024-10-25 ONESOFT-3539 --)
...
Функция РассчитатьДеревоСкидокНаценок(ПараметрыРасчета, ВходныеПараметры) Экспорт
	...
	//малышев 2024-10-25 ONESOFT-3539
	Если ПараметрыРасчета.Свойство("Контрагент") ТОгда
		Параметры.Вставить("Контрагент", ПараметрыРасчета.Контрагент); 
	КонецЕсли;	
...
Процедура ПрименитьРезультатРасчетаСкидокКОбъекту(Объект, ИмяТЧ, РезультатРасчетаСкидокНаценок, РеализацияСверхЗаказа = Ложь, ТоварыСверхЗаказа = Неопределено, ИмяТЧ2 = Неопределено, ДеревоСкидок = Неопределено) Экспорт
	...
	ИспользоватьКартыЛояльности = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункциональнаяОпцияИспользоватьКартыЛояльности");
	...
	ЕстьРеквизитКартаЛояльности = НЕ (Объект.Ссылка.Метаданные().Реквизиты.Найти("КартаЛояльности") = Неопределено); //+++ Дмгг, доработка для Расходной накладной
		...
		Для Каждого СтрокаТЧ Из Объект[ТекЭлементСоответствия.Ключ] Цикл
			...
			Если СтрокаТаблицы = Неопределено Тогда
				Если СтрокаТЧ.СуммаАвтоматическойСкидки = 0 И СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 Тогда
					...
					//Продолжить; //+++ Дмгг
					Если ЕстьРеквизитКартаЛояльности И ЗначениеЗаполнено(Объект.КартаЛояльности) Тогда 
						СтрокаТЧ.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
					Иначе
						Продолжить;
					КонецЕсли;
					//--- Дмгг
			...
			//Малышев 2024-10-29 ONESOFT-3544 (-- закоментировал чтобы скидки действовали одновременно
			////+++ Дмгг
			////СуммаСкидки = СуммаАвтоматическойСкидки + СуммаРучнойСкидки;
			//Если ЕстьРеквизитКартаЛояльности Тогда 
			//	Если ИспользоватьКартыЛояльности Тогда //ЗначениеЗаполнено(Объект.КартаЛояльности) Тогда 
			//		
			//		Если СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 И СтрокаТЧ.ПроцентСкидкаЛояльности = 0 Тогда
			//			
			//			СтрокаТЧ.ПроцентАвтоматическойСкидки = ?(СуммаБезСкидки = 0, 0 , 100 * СуммаАвтоматическойСкидки / СуммаБезСкидки);
			//			
			//			Если СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 Тогда 
			//				СтрокаТЧ.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
			//			КонецЕсли;
			//			
			//		КонецЕсли;
			//		
			//		Если СтрокаТЧ.ПроцентАвтоматическойСкидки <> 0 И СтрокаТЧ.ПроцентСкидкиНаценки <> 0 Тогда 
			//			СтрокаТЧ.ПроцентСкидкаЛояльности = 0; 
			//			СуммаСоСкидкой = СуммаБезСкидки * (1 - (СтрокаТЧ.ПроцентАвтоматическойСкидки) / 100) * (1 - (СтрокаТЧ.ПроцентСкидкиНаценки) / 100); 
			//			СуммаСкидки = СуммаБезСкидки - СуммаСоСкидкой;
			//		ИначеЕсли СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 И СтрокаТЧ.ПроцентСкидкиНаценки <> 0 И СтрокаТЧ.ПроцентСкидкаЛояльности = 0 Тогда
			//			СтрокаТЧ.ПроцентАвтоматическойСкидки = ?(СуммаБезСкидки = 0, 0 , 100 * СуммаАвтоматическойСкидки / СуммаБезСкидки);
			//			СуммаСоСкидкой = СуммаБезСкидки * (1 - (СтрокаТЧ.ПроцентАвтоматическойСкидки) / 100) * (1 - (СтрокаТЧ.ПроцентСкидкиНаценки) / 100);
			//			СуммаСкидки = СуммаБезСкидки - СуммаСоСкидкой;
			//		ИначеЕсли СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 И СтрокаТЧ.ПроцентСкидкиНаценки = 0 И СтрокаТЧ.ПроцентСкидкаЛояльности = 0 Тогда
			//			СуммаСкидки = СуммаАвтоматическойСкидки + СуммаРучнойСкидки;
			//			СтрокаТЧ.ПроцентАвтоматическойСкидки = ?(СуммаБезСкидки = 0, 0 , 100 * СуммаАвтоматическойСкидки / СуммаБезСкидки);
			//		ИначеЕсли СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 И СтрокаТЧ.ПроцентСкидкиНаценки <> 0 Тогда
			//			СуммаСоСкидкой = СуммаБезСкидки * (1 - (СтрокаТЧ.ПроцентСкидкаЛояльности) / 100) * (1 - (СтрокаТЧ.ПроцентСкидкиНаценки) / 100);
			//			СуммаСкидки = СуммаБезСкидки - СуммаСоСкидкой;
			//		ИначеЕсли СтрокаТЧ.ПроцентАвтоматическойСкидки <> 0 И СтрокаТЧ.ПроцентСкидкиНаценки = 0 Тогда
			//			СтрокаТЧ.ПроцентСкидкаЛояльности = 0; 
			//			СуммаСоСкидкой = СуммаБезСкидки * (1 - (СтрокаТЧ.ПроцентАвтоматическойСкидки) / 100);
			//			СуммаСкидки = СуммаБезСкидки - СуммаСоСкидкой;
			//		ИначеЕсли СтрокаТЧ.ПроцентАвтоматическойСкидки = 0 И СтрокаТЧ.ПроцентСкидкиНаценки = 0 Тогда
			//			СуммаСоСкидкой = СуммаБезСкидки * (1 - (СтрокаТЧ.ПроцентСкидкаЛояльности) / 100);	
			//			СуммаСкидки = СуммаБезСкидки - СуммаСоСкидкой;
			//		Иначе
			//			СуммаСкидки = СуммаАвтоматическойСкидки + СуммаРучнойСкидки;
			//		КонецЕсли;
			...
			//Малышев 2024-12-05 ONESOFT-3676
			Если ЕстьРеквизитКартаЛояльности и СтрокаТЧ.ПроцентАвтоматическойСкидки<>0 ТОгда 
				СтрокаТЧ.ПроцентСкидкаЛояльности = 0;
				СтрокаТЧ.СуммаНДСЛояльности = 0;
			Иначе     
				//СтрокаТЧ.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
			КонецЕсли;	
			...
			СкидкиНаценкиКлиентСервер.ПосчитатьСуммуСтрокиСоСкидками(СтрокаТЧ);		
		
			//СтрокаТЧ.ПроцентАвтоматическойСкидки = ?(СуммаБезСкидки = 0, 0 , 100 * СуммаАвтоматическойСкидки / СуммаБезСкидки); // Дмгг
