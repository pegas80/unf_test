...
"""%2"".';ka='შეცდომა კატალოგ ""%1""-ის შექმნისას: ""%2"".'"),
					ПолныйПодКаталог,
...
"превышает максимально допустимый размер файла (%3 Мб).';ka='Размер файла ""%1"" (%2 Мб)"
"превышает максимально допустимый размер файла (%3 Мб).'"),
	...
	Если Не ПроверитьРасширениеФайлаДляЗагрузки(Файл.Расширение, Ложь) Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილების ჩატვირთვა გაფართოება ""%1""-ით აკრძალულია. მიმართეთ ადმინისტრატორს.';ru='Загрузка файлов с расширением ""%1"" запрещена."
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("ka='Нельзя подписать занятый файл.';ru='Нельзя подписать занятый файл.';en='You can not sign a busy file.'");
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Нельзя подписать занятый файл: %1.';ru='Нельзя подписать занятый файл: %1.';en='You can not sign a busy file:% 1.'"),
			Строка(ФайлСсылка) );
	КонецЕсли;
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("ka='Нельзя подписать зашифрованный файл.';ru='Нельзя подписать зашифрованный файл.';en='You can not sign the encrypted file.'");
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Нельзя подписать зашифрованный файл: %1.';ru='Нельзя подписать зашифрованный файл: %1.';en='You can not sign the encrypted file:% 1.'"),
						Строка(ФайлСсылка) );
	КонецЕсли;
...
""
"%1';ka='Ошибка создания нового файла."
""
"%1'"),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
"Обратитесь к администратору.';ka='ფაილის გახსნის შეცდომა:"
"""%1""."
"%2"
"ფაილების საცავში ფაილი ვერ მოიძებნა."
"შეიძლება, ფაილი წაშლილია ანტივირუსული პროგრამის მიერ."
...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекстОшибки = "") Экспорт
	...
	Если СтрокаПодписи.ПодписьВерна Тогда
		СтрокаПодписи.Статус = НСтр("ka='სწორია';ru='Верна';en='Verne'");
		
	ИначеЕсли ЗначениеЗаполнено(ТекстОшибки) Тогда
		СтрокаПодписи.Статус = НСтр("ka='არ არის სწორი';ru='Неверна';en='incorrectly'");
