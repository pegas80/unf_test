...
Процедура РасчетДинамическихЦенНаПроцент(ПараметрыРасчета, ЦеныРасчитаны, ОписаниеОшибки = "", ЗаписыватьЦены = Истина) Экспорт
	...
	ТаблицаХранилише = Новый ТаблицаЗначений;
	Если ВидЦенРасчетный.КатегорииНоменклатуры.Количество() <> 0 Тогда
		Дерево = ДеревоКатегорий(ВидЦенРасчетный);
		Тз = Новый ТаблицаЗначений;
        Тз = ВидЦенРасчетный.КатегорииНоменклатуры.Выгрузить().СкопироватьКолонки();
		ОбойтиДерево(Дерево,ВидЦенРасчетный.Процент,Тз,ВидЦенРасчетный.КатегорииНоменклатуры.Выгрузить());
		Для Каждого Стр Из Тз Цикл
			Если Стр.КатегорияНоменклатуры.ЭтоГруппа Тогда
				Продолжить;
			КонецЕсли;	
			ЗаписатьДинамическийПроцент(ПараметрыРасчета, ЦеныРасчитаны, ОписаниеОшибки , ЗаписыватьЦены,Стр.КатегорияНоменклатуры,Стр.Процент,ТаблицаХранилише);		
		КонецЦикла;		
	Иначе
		ЗаписатьДинамическийПроцент(ПараметрыРасчета, ЦеныРасчитаны, ОписаниеОшибки , ЗаписыватьЦены,Неопределено,ВидЦенРасчетный.Процент,ТаблицаХранилише);	
	КонецЕсли;
	
	ЗаписатьЦеныИОчиститьОчередьЦен(ПараметрыРасчета.ПериодЗаписи, ВидЦенРасчетный, ТаблицаХранилише);
	
	ПараметрыРасчета.Вставить("АдресВХранилище", ПоместитьВоВременноеХранилище(ТаблицаХранилише, ПараметрыРасчета.УникальныйИдентификатор));
	
КонецПроцедуры

Функция ДеревоКатегорий(Ссылка) 
	...
	|	ВЫБОР
	|		КОГДА КатегорииНоменклатуры.ЭтоГруппа
	|			ТОГДА 0
	|		ИНАЧЕ 3
	...
	ВыборкаСИерархией= Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией,"Значение");
	Дерево=Новый ДеревоЗначений;
	Дерево.Колонки.Добавить("Значение");
	Дерево.Колонки.Добавить("Представление");
	Дерево.Колонки.Добавить("Процент");
	...
	Если Дерево.Строки.Количество() > 1 Тогда
		...
		СтрокаНетКатегории.Представление = НСтр("ka='<ყველა კატეგორია>';ru='<Все категории>';en='<All categories>'");
...
Процедура ВыбратьЭлементыВИерархии(ВыборкаСИерархией,Дерево)
    Пока ВыборкаСИерархией.Следующий() Цикл
        Если ВыборкаСИерархией.ТипЗаписи()=ТипЗаписиЗапроса.ИтогПоИерархии Тогда
            ...
            ВыбратьЭлементыВИерархии(ВыборкаСИерархией.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией,"Значение"),Строка);
		ИначеЕсли ВыборкаСИерархией.ТипЗаписи()=ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
			Если ВыборкаСИерархией.Значение.ЭтоГруппа Тогда 
				Продолжить;
			КонецЕсли;	
			Строка=Дерево.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(Строка,ВыборкаСИерархией);
...
Процедура ОбойтиДерево(Дерево,Процент,Таблица,КатегорииНоменклатуры)
	Для Каждого Подстрока Из Дерево.Строки Цикл
		...
		Структура.Вставить("КатегорияНоменклатуры",Подстрока.Значение);
		
		НайденаяСтрока = КатегорииНоменклатуры.НайтиСтроки(Структура);
		
		Если НайденаяСтрока.Количество() <> 0 Тогда
			ПроцентНов = НайденаяСтрока[0].Процент;
		КонецЕсли;
		
		ПроцентНов = ?(ЗначениеЗаполнено(ПроцентНов),ПроцентНов,Процент);
		НовСтрока.КатегорияНоменклатуры = Подстрока.Значение;
		
		НовСтрока.Процент = ПроцентНов;			
		ОбойтиДерево(Подстрока,ПроцентНов,Таблица,КатегорииНоменклатуры);		
	КонецЦикла;
КонецПроцедуры

Процедура ЗаписатьДинамическийПроцент(ПараметрыРасчета, ЦеныРасчитаны, ОписаниеОшибки = "", ЗаписыватьЦены = Истина,Категория,Процент,ТаблицаДляХранилиша)
	...
	Запрос.УстановитьПараметр("ЕстьКатегория",Категория <> Неопределено);
	...
	|	И ВЫБОР
	|			КОГДА &ЕстьКатегория
	|				ТОГДА ОчередьЦен.Номенклатура.КатегорияНоменклатуры  = &Категория
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	И ВЫБОР
	|			КОГДА &ЕстьКатегория
	|				ТОГДА РаннееРасчитаныеЦены.Номенклатура.КатегорияНоменклатуры = &Категория
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	Попытка
		...
		Если ТаблицаДляХранилиша.Количество() = 0 Тогда 
			ТаблицаДляХранилиша = ЗаписиРегистраЗаПериода.СкопироватьКолонки();
		КонецЕсли;
		...
		Для Каждого Стр Из ЗаписиРегистраЗаПериода Цикл 
			
			НовСтр = ТаблицаДляХранилиша.Добавить();				
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru='Ошибка расчета цен из очереди.';ka='რიგიდან ფასების გამოთვლის შეცდომა.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура РасчетОчередиЦенРазовымЗаданием() Экспорт
	...
	ЗаписьЖурналаРегистрации("Расчет очереди цен", УровеньЖурналаРегистрации.Информация, , , НСтр("ka='ფასების რიგის გამოთვლის შესრულების დასაწყისი';ru='Начало выполнения расчета очереди цен'"));
...
Процедура СнятьПризнакНовинка() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Исключение
			...
			Комментарий = НСтр("ru='Ошибка работы фонового регламентного задания: СнятиеПризнакаНовинка';ka='ფონური სარეგლამენტო დავალების მუშაობის შეცდომა: ნიშანსიახლისმოხსნა'")
...
Процедура ИзменитьПризнакАктуальностиУВидаЦен(ВидЦен, ЦеныАктуальны)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Ошибка обновления свойства ""Цены актуальны"" вида цен [%1]';ka='ფასების სახეობის თვისება ""ფასები აქტუალურია""-ს განახლების შეცდომა (%1)'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
