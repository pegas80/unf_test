...
Функция ТипНомераДокументаУдостоверяющегоЛичность(ВидДокумента) Экспорт
	...
	ИначеЕсли ВидДокумента = ПредопределенноеЗначение("Справочник.ВидыДокументовФизическихЛиц.ПаспортГР") Тогда 
		ТипДокумента = 4;		
	КонецЕсли;
...
Функция СерияДокументаУказанаПравильно(ВидДокумента, Знач Серия , ТекстОшибки) Экспорт
	...
	Если ТипДокумента = 1 Тогда // паспорт СССР и свидетельство о рождении
		...
		Поз = СтрНайти(Серия, "-");
		Если Поз = 0 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერია უნდა შედგებოდეს ორი ნაწილისგან, რომელიც გაყოფილი იქნება სიმბოლო ""-""-თი.';ru='Серия документа должна состоять из двух частей, разделенных символом ""-"".';en='document series shall consist of two parts, separated by ""-"".'");
			Возврат Ложь;
		КонецЕсли;
		...
		Если Поз <> 0 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიაში უნდა იყოს სიმბოლოების მხოლოდ ორი ჯგუფი';ru='В серии документа должно быть только две группы символов.';en='In a series of documents should be only two groups of characters.'");
			Возврат Ложь;
		КонецЕсли;
		
		Если ПустаяСтрока(ЧастьСерии1) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიაში არ არის რიცხვითი ნაწილი.';ru='В серии документа отсутствует числовая часть.';en='In a series of document missing the numerical part.'");
			Возврат Ложь;
			
		ИначеЕсли  ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("IVXLC1УХЛС", ЧастьСерии1, "          ")) = 0 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიის რიცხვით ნაწილი მითითებული უნდა იყოს სიმბოლოებით 1 У Х Л С  ან  I V X L C.';ru='Числовая часть серии документа должна указываться символами 1 У Х Л С  или  I V X L C.';en='The numeric part of a series of document 1 to be mentioned symbols Y X A or C I V X L C.'");
			Возврат Ложь;
		...
		ИначеЕсли СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("IVXLC", ЧастьСерии1, "1УХЛС") <> СтроковыеФункцииКлиентСервер.ПреобразоватьЧислоВРимскуюНотацию(СтроковыеФункцииКлиентСервер.ПреобразоватьЧислоВАрабскуюНотацию(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("IVXLC", ЧастьСерии1, "1УХЛС"))) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიის რიცხვითი ნაწილი არაკორექტულად არის მითითებული.';ru='Числовая часть серии документа указана некорректно.';en='The numeric part of the document series specified incorrectly.'");
			Возврат Ложь;
		...
		ИначеЕсли СтрДлина(ЧастьСерии2) <> 2 Или Не ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ", ЧастьСерии2, "                                 ")) Тогда
			ТекстОшибки = НСтр("ka='გამყოფ ""-""-ის შემდეგ დოკუმენტის სერიაში უნდა იყოს ორი რუსული დიდი ასო-ბგერა.';ru='После разделителя ""-"" в серии документа должны быть ДВЕ pусcкие заглавные буквы.';en='After the separator ""-"" in a series of documents must be TWO ENGLISH caps.'");
			Возврат Ложь;
	...
	ИначеЕсли ТипДокумента = 2 Тогда // серия - две буквы: военный билет, ...
		Если СтрДлина(Серия) <> 2 Или Не ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ", Серия, "                                 ")) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიაში უნდა იყოს ორი რუსული დიდი ასო-ბგერა.';ru='В серии документа должны быть ДВЕ pусcкие заглавные буквы.';en='In a series of documents must be TWO ENGLISH caps.'");
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТипДокумента = 3 Тогда // серия - две цифры: загранпаспорта
		Если СтрДлина(Серия) <> 2 Или Не ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("0123456789", Серия, "          ")) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიაში უნდა იყოს ორი ციფრი.';ru='В серии документа должно быть ДВЕ цифры.';en='In a series of documents must be two digits.'");
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТипДокумента = 4 Тогда // серия - две группы цифр: новый паспорт
		Поз = СтрНайти(Серия, " ");
		Если Поз = 0 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიაში უნდა იყოს ციფრების ორი ჯგუფი.';ru='В серии документа должно быть две группы цифр.';en='In a series of documents have to be two groups of digits.'");
			Возврат Ложь;
		КонецЕсли;
		...
		Если Поз <> 0 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიაში უნდა იყოს ციფრების მხოლოდ ორი ჯგუფი.';ru='В серии документа должно быть только две группы цифр.';en='In a series of documents should be only two groups of digits.'");
			Возврат Ложь;
		КонецЕсли;
		...
		Если СтрДлина(ПерваяЧасть) <> 2 Или Не ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("0123456789", ПерваяЧасть, "          ")) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიის სიმბოლოების პირველი ჯგუფი უნდა შეიცავდეს ორ ციფრს.';ru='Первая группа символов серии документа должна содержать две цифры.';en='The first group of document series symbols must contain two digits.'");
			Возврат Ложь;
		КонецЕсли;
		...
		Если СтрДлина(ВтораяЧасть) <> 2 Или Не ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("0123456789", ВтораяЧасть, "          ")) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის სერიის სიმბოლოების მეორე ჯგუფი უნდა შეიცავდეს ორ ციფრს.';ru='Вторая группа символов серии документа должна содержать две цифры.';en='The second group of the document series symbols must contain two digits.'");
			Возврат Ложь;
		КонецЕсли;
...
Функция НомерДокументаУказанПравильно(ВидДокумента, Знач Номер, ТекстОшибки) Экспорт
	...
	Если ТипДокумента <> 4 Тогда  
		Если Не ПустаяСтрока(СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими("0123456789", Номер, "          ")) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის ნომერი შეიცავს დაუშვებელ სიმბოლოებს.';ru='В номере документа присутствуют недопустимые символы.';en='In the document number contains invalid characters.'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипДокумента = 1 Тогда
		Если ДлинаНомера <> 6 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის ნომერი უნდა შედგებოდეს 6 სიმბოლოსგან.';ru='Номер документа должен состоять из 6 символов.';en='document number should consist of 6 characters.'");
			Возврат Ложь;
		КонецЕсли;
		
	ИначеЕсли ТипДокумента = 2 Тогда
		Если ДлинаНомера <> 7 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის ნომერი უნდა შედგებოდეს 7 სიმბოლოსგან.';ru='Номер документа должен состоять из 7 символов.';en='document number should consist of 7 characters.'");
			Возврат Ложь;
		КонецЕсли;
		
	ИначеЕсли ТипДокумента = 3 Тогда
		Если (ДлинаНомера < 6 ) Или ( ДлинаНомера > 7 ) Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის ნომერი უნდა შედგებოდეს 6 ან 7 სიმბოლოსგან.';ru='Номер документа должен состоять из 6 или 7 символов.';en='document number should be 6 or 7 characters.'");
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТипДокумента = 4 Тогда
		Если ДлинаНомера <> 9 Тогда
			ТекстОшибки = НСтр("ka='დოკუმენტის ნომერი უნდა შედგებოდეს 9 სიმბოლოსგან.';ru='Номер документа должен состоять из 9 символов.';en='document number should be 9 characters.'");
			Возврат Ложь;
		КонецЕсли;
