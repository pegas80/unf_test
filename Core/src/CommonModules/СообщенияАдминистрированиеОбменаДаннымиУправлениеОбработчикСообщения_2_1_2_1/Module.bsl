...
Процедура ПодключитьКорреспондента(Сообщение, Отправитель)
	...
	Если ЭтаКонечнаяТочка.Пустая()
		...
		ПредставлениеОшибки = НСтр("ka='საბოლოო წერტილი მოსალოდნელს არ შეესაბამება. მოსალოდნელი საბოლოო წერტილის კოდია %1. მიმდინარე საბოლოო წერტილის კოდია %2.';ru='Конечная точка не соответствует ожидаемой. Код ожидаемой конечной точки %1. Код текущей конечной точки %2.'");
	...
	Если Корреспондент.Пустая() Тогда // Подключаем конечную точку корреспондента
		...
		Если Отказ Тогда // Отправляем сообщение в менеджер сервиса об ошибке
			...
			ПредставлениеОшибки = НСтр("ka='გაცვლის კორესპონდენტის საბოლოო წერტილის შეერთების შეცდომა. გაცვლის კორესპონდენტის საბოლოო წერტილის კოდი %1.';ru='Ошибка подключения конечной точки корреспондента обмена. Код конечной точки корреспондента обмена %1.'");
			ПредставлениеОшибки = СтрШаблон(ПредставлениеОшибки,
				Тело.RecipientId);
...
Процедура УстановитьНастройкиТранспорта(Сообщение, Отправитель)
	...
	Если Корреспондент.Пустая() Тогда
		СтрокаСообщения = НСтр("ka='კორესპონდენტის საბოლოო წერტილი კოდით ""%1"" ვერ მოიძებნა.';ru='Не найдена конечная точка корреспондента с кодом ""%1"".'");
...
Функция СобытиеЖурналаРегистрацииПодключениеКорреспондента()
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა. გაცვლის კორესპონდენტის ჩართვა';ru='Обмен данными.Подключение корреспондента обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
