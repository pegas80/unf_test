					...
					Если Не РезультатЗапроса2.Пустой() Тогда
						...
						Если Выборка2.Следующий() Тогда
							...
							Если КонечнаяТочкаКорреспондента = Неопределено Тогда
								...
								ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კორესპონდენტის ბოლო წერტილი დადგენილი არ არის მისამართისთვის ""%1"".';ru='Не определена конечная точка корреспондента для адреса ""%1"".'"),
									Выборка2.Адрес);
							КонецЕсли;
	...
	Пока РезультатЗапроса.Следующий() Цикл
		Если НЕ ПустаяСтрока(РезультатЗапроса.Узел) 
			...
			Исключение
				...
				СобытиеЖурналаРегистрацииОбменДанными = НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ОтносительныйКаталогОбменаИнформацией(Знач Каталог)
	...
	Если ПустаяСтрока(Каталог) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინფორმაციის გაცვლის კატალოგის არასწორი ფორმატი ""%1"".';ru='Неправильный формат каталога обмена информацией ""%1"".'"),
			Каталог);
	КонецЕсли;
...
Процедура ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДанными) Экспорт
	...
	Если СтрокаСценария.НомерИнформационнойБазы = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='უცნობი მოქმედება ""%1"" მონაცემთა არეალებს შორის მონაცემთა გაცვლისას.';ru='Неизвестное действие ""%1"" при обмене данными между областями данных.'"),
...
Функция СформироватьНаименованиеПредопределенногоУзла() Экспорт
	...
	Возврат ?(ПустаяСтрока(ИмяПриложения), НСтр("ka='დანართი ინტერნეტში';ru='Приложение в Интернете'"), ИмяПриложения);
КонецФункции
...
Функция КонечнаяТочкаКорреспондента(Знач Корреспондент) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		СтрокаСообщения = НСтр("ka='კორესპონდენტის ბოლო წერტილი არ არის დანიშნული ""%1""-სთვის.';ru='Не назначена конечная точка корреспондента для ""%1"".'");
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ТекстСообщения = НСтр("ka='მიწოდებული გაცვლის წესები არ მიესადაგება მიმდინარე კონფიგურაციას და განკუთვნილია გაცვლის გეგმის %1 ვერსიის %3 კონფიგურაციისათვის%2';ru='Поставляемые правила обмена не подходят для текущей конфигурации и предназначены для плана обмена %1 конфигурации %2 версии %3'");
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა ცვლილებების მიწოდებული წესები.მიწოდებული  წესების ჩატვირთვა გაუქმებულია.';ru='Поставляемые правила обмена данными.Загрузка поставляемых правил отменена'",
...
Процедура СоздатьНастройкуОбмена(
	...
	Попытка
		...
		Если ПустаяСтрока(КодЭтогоУзла) Тогда
			...
			СтрокаСообщения = НСтр("ka='გაცვლის გეგმის წინასწარ განსაზღვრული კვანძისთვის ""%1"" არ არის მიცემული კოდი.';ru='Для предопределенного узла плана обмена ""%1"" не задан код.'");
		...
		Если ПустаяСтрока(ПолучитьФункциональнуюОпцию("ПрефиксИнформационнойБазы")) Тогда
			...
			Если ПустаяСтрока(Префикс) Тогда
				ВызватьИсключение НСтр("ka='სერვისის მენეჯერში დაყენებულია ამ დანართის პრეფიქსი.';ru='В Менеджере сервиса не установлен префикс для этого приложения.'");
			КонецЕсли;
...
"Фактическое значение ""%2"".';ka='კორესპონდენტის კვანძის კოდის დანიშვნის შეცდომა. დანიშნული მნიშვნელობა ""%1"" ფაქტობრივი მნიშვნელობა ""%2"".'");
		...
		Если НастройкиТранспорта.ВидТранспортаСообщенийОбменаПоУмолчанию = Перечисления.ВидыТранспортаСообщенийОбмена.FILE Тогда
			...
			Если ПустаяСтрока(FILEОбщийКаталогОбменаИнформацией) Тогда
				...
				СтрокаСообщения = НСтр("ka='არ არის მიცემული ინფორმაციის გაცვლის კატალოგი საბოლოო წერტილისთვის ""%1"".';ru='Не задан каталог обмена информацией для конечной точки ""%1"".'");
			...
			Если Не ОбщийКаталог.Существует() Тогда
				...
				СтрокаСообщения = НСтр("ka='ინფორმაციის გაცვლის კატალოგი ""%1"" არ არსებობს.';ru='Каталог обмена информацией ""%1"" не существует.'");
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ბოლო წერტილ %2-ისთვის გაცვლის შეტყობინებების ტრანსპორტის სახეობა ""%1"" არ არის მხარდაჭერილი.';ru='Вид транспорта сообщений обмена ""%1"" для конечной точки %2 не поддерживается.'"),
...
Процедура СохранитьДанныеСессии(Знач Сообщение, Знач Представление = "") Экспорт
	...
	Если Не ПустаяСтрока(Представление) Тогда
		Представление = " "+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='{%1}';ru='{%1}'"), Представление);
	КонецЕсли;
	
	СтрокаСообщения = НСтр("ka='სისტემა ""%1""-ის შეტყობინებების გაცვლის სესია წარმატებით დასრულდა.%2';ru='Сессия обмена сообщениями системы ""%1"" успешно завершена.%2'",
...
Функция СобытиеЖурналаРегистрацииНастройкаСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა სერვისის მოდელში. მონაცემთა სინქრონიზაციის გამართვა';ru='Обмен данными в модели сервиса.Настройка синхронизации данных'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииМониторСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა სერვისის მოდელში. მონაცემთა სინქრონიზაციის მონიტორი';ru='Обмен данными в модели сервиса.Монитор синхронизации данных'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა სერვისის მოდელში. მონაცემთა სინქრონიზაცია';ru='Обмен данными в модели сервиса.Синхронизация данных'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСессииОбменаСообщениямиСистемы()
	...
	Возврат НСтр("ka='მონაცემთა გაცვლა სერვისის მოდელში. სისტემის შეტყობინებების გაცვლის სესიები';ru='Обмен данными в модели сервиса.Сессии обмена сообщениями системы'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция НайтиУзелИнформационнойБазы(Знач ИмяПланаОбмена, Знач КодУзла)
	...
	Если Результат = Неопределено Тогда
		Сообщение = НСтр("ka='გაცვლის გეგმის კვანძი ვერ მოიძებნა. გაცვლის გეგმის სახელია %1; კვანძის კოდი %2 ან %3';ru='Не найден узел плана обмена. Имя плана обмена %1; код узла %2 или %3'");
