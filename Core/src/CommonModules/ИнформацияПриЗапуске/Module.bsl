...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ka='ახორციელებს პირველი ჩვენების მონაცემთა ანალიზს.';ru='Актуализирует данные первого показа.'");
...
Функция ПакетыСтраниц(НосительМакетов) Экспорт
	...
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если НРег(СтрокаТаблицы.Раздел) = НРег(НСтр("ka='რეკლამა';ru='Реклама'")) Тогда
...
Функция ИзвлечьФайлыПакета(НосительМакетов, ИмяМакета) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ინფორმაცია გაშვებისას';ru='Информация при запуске'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
