...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ka='ახორციელებს პირველი ჩვენების მონაცემთა ანალიზს.';ru='Актуализирует данные первого показа.';en='Updates the data of the first display.'");
...
Функция ПакетыСтраниц(НосительМакетов) Экспорт
	...
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если НРег(СтрокаТаблицы.Раздел) = НРег(НСтр("ka='რეკლამა';ru='Реклама';en='Advertising'")) Тогда
...
Функция ИзвлечьФайлыПакета(НосительМакетов, ИмяМакета) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ინფორმაცია გაშვებისას';ru='Информация при запуске';en='Information on startup'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
