...
#КонецЕсли
	...
	Если ПечатныеФормы.Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ბეჭდური ფორმის პრინტერზე გამოსატანად უფლებები არ არის, მიმართეთ ადმინისტრატორს.';ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';en='No permission to list the printing plate to the printer, contact your administrator.'"));
		Возврат;
	КонецЕсли;
...
Функция ОбластьМакета(Знач СсылкаНаМакет, Знач ОписаниеОбласти) Экспорт
	...
	Если СсылкаНаМакет.Тип = "DOC" Тогда
		...
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Список" Тогда
			Область = УправлениеПечатьюMSWordКлиент.ПолучитьОбластьМакетаMSWord(СсылкаНаМакет, ОписаниеОбласти.ИмяОбласти, 1, 0);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Тип области не указан или указан не корректно: %1.';ru='Тип области не указан или указан не корректно: %1.';en='the field type is not specified or is specified incorrectly:% 1.'"), ОписаниеОбласти.ТипОбласти);
		КонецЕсли;
	...
	ИначеЕсли СсылкаНаМакет.Тип = "ODT" Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Тип области не указан или указан не корректно: %1.';ru='Тип области не указан или указан не корректно: %1.';en='the field type is not specified or is specified incorrectly:% 1.'"), ОписаниеОбласти.ИмяОбласти);
		КонецЕсли;
...
Процедура ПрисоединитьОбласть(Знач ПечатнаяФорма,
	...
	Попытка
		...
		Если ПечатнаяФорма.Тип = "DOC" Тогда
			...
			ИначеЕсли	ОписаниеОбласти.ТипОбласти = "СтрокаТаблицы" Тогда
				...
				КонецЕсли;
			Иначе
				ВызватьИсключение(НСтр("ka='Тип области не указан или указан не корректно.';ru='Тип области не указан или указан не корректно.';en='the field type is not specified or is specified incorrectly.'"));
			КонецЕсли;
	...
	Исключение
		...
		СообщениеОбОшибке = СообщениеОбОшибке + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Ошибка при попытке вывести область ""%1"" из макета.';ru='Ошибка при попытке вывести область ""%1"" из макета.';en='An error occurred while trying to display the area ""%1"" from the layout.'"),
