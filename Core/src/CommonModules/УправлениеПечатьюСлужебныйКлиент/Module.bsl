...
Процедура ВыполнитьПодключаемуюКомандуПечатиЗавершение(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		ИмяПоказателя = НСтр("ka='ბეჭდვა';ru='Печать';en='Print'") + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("/%1/%2/%3/%4/%5/%6/%7",
...
Процедура ПроверитьПроведенностьДокументовДиалогПроведения(Параметры) Экспорт
	...
	Если УправлениеПечатьюВызовСервера.ДоступноПравоПроведения(Параметры.НепроведенныеДокументы) Тогда
		Если Параметры.НепроведенныеДокументы.Количество() = 1 Тогда
			ТекстВопроса = НСтр("ka='დოკუმენტის ამოსაბეჭდად, საჭიროა მისი წინასწარ გატარება. გავატაროთ დოკუმენტი და გავაგრძელოთ?';ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Выполнить проведение документа и продолжить?';en='To print a document, it must first carry out. Run holding a document and continue?'");
		Иначе
			ТекстВопроса = НСтр("ka='დოკუმენტების ამოსაბეჭდად, საჭიროა მათი წინასწარ გატარება. გავატაროთ დოკუმენტები და გავაგრძელოთ?';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Выполнить проведение документов и продолжить?';en='To print the documents, they must be performed first. Run holding documents and proceed?'");
		КонецЕсли;
	Иначе
		Если Параметры.НепроведенныеДокументы.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ka='დოკუმენტის ამოსაბეჭდად, საჭირო არის მისი წინასწარ გატარება. დოკუმენტის გასატარებლად არასაკმარისი უფლებებია, ამობეჭდვა შეუძლებელია.';ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Недостаточно прав для проведения документа, печать невозможна.';en='To print a document, it must first carry out. Insufficient permissions for the document, you can not print.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='დოკუმენტების ამოსაბეჭდად საჭირო არის მათი წინასწარ გატარება. დოკუმენტების გასატარებლად არასაკმარისი უფლებებია, ბეჭდვა შეუძლებელია.';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Недостаточно прав для проведения документов, печать невозможна.';en='To print the documents, they must be performed first. Insufficient permissions for documents, printing is not possible.'");
		КонецЕсли;
...
Процедура ПроверитьПроведенностьДокументовПроведениеДокументов(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ШаблонСообщения = НСтр("ka='დოკუმენტი %1 არ არის გატარებული: %2';ru='Документ %1 не проведен: %2';en='% Document 1 not conducted: 2%'");
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		ТекстДиалога = НСтр("ka='ვერ მოხერხდა ერთი ან რამდენიმე დოკუმენტის გატარება.';ru='Не удалось провести один или несколько документов.';en='Unable to carry out one or more documents.'");
		
		КнопкиДиалога = Новый СписокЗначений;
		Если ПроведенныеДокументы.Количество() > 0 Тогда
			ТекстДиалога = ТекстДиалога + " " + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?';en='Proceed?'");
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
...
Процедура ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ОписаниеОповещения)
	#Если ВебКлиент Тогда
		ТекстСообщения = НСтр("ka='ბეჭდვის გასაგრძელებლად აუცილებელია ვებ-კლიენტ 1C:საწარმოსთვის გაფართოების დაყენება.';ru='Для продолжения печати необходимо установить расширение для веб-клиента 1С:Предприятие.';en='To continue printing, you must install the extension for Web client 1C: Enterprise.'");
