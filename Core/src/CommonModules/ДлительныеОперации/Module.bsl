...
"не могут одновременно принимать значение Истина.';ka='Параметры длительной операции ""%1"" и ""%2"""
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='ხანგრძლივი ოპერაციები. ფონური დავალების შესრულების გაუქმება';ru='Длительные операции.Отмена выполнения фонового задания';en='Long-term performance of the background task operatsii.Otmena'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ЗаданиеВыполнено(Знач ИдентификаторЗадания) Экспорт
	...
	Если Задание = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ka='ხანგრძლივი ოპერაციები. ფონური დავალება ვერ მოიძებნა';ru='Длительные операции.Фоновое задание не найдено';en='Long operatsii.Fonovoe job is not found'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Иначе
		...
		ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ka='ხანგრძლივი ოპერაციები. ფონური დავალება გაუქმებულია ადმინისტრატორის მიერ';ru='Длительные операции.Фоновое задание отменено администратором';en='Long operatsii.Fonovoe job is canceled by the administrator'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				НСтр("ka='დავალება დასრულდა უცნობი შეცდომით.';ru='Задание завершилось с неизвестной ошибкой.';en='Task completed with an unknown error.'"));
		Иначе
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ОперацияНеВыполнена Тогда
		ВызватьИсключение(НСтр("ka='მოცემული ოპერაციის შესრულება ვერ მოხერხდა. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Не удалось выполнить данную операцию. "
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Задание = НайтиЗаданиеПоИдентификатору(ИдентификаторЗадания);
	Если Задание = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ka='ხანგრძლივი ოპერაციები';ru='Длительные операции';en='long-term operation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , НСтр("ka='ფონური დავალება ვერ მოიძებნა:';ru='Фоновое задание не найдено:';en='Background job is not found:'") + " " + Строка(ИдентификаторЗадания));
		Если ИсключениеПриОшибке Тогда
			ВызватьИсключение(НСтр("ka='მოცემული ოპერაციის შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить данную операцию.';en='Unable to perform this operation.'"));
		КонецЕсли;
		Результат.Статус = "Ошибка";
		Возврат Результат;
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно 
		...
		Если ИсключениеПриОшибке Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ka='მოცემული ოპერაციის შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить данную операцию.';en='Unable to perform this operation.'");
			КонецЕсли;
...
Процедура ВыполнитьПроцедуруМодуляОбъектаОбработки(Параметры, АдресХранилища) Экспорт 
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='დამუშავების მეთოდის უსაფრთხო შესრულება';ru='Безопасное выполнение метода обработки';en='Safe execution of the processing method'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მეთოდ ""%1""-ის სახელი ცვლადების სახელების წარმოქმნის მოთხოვნებს არ შეესაბამება.';ru='Имя метода ""%1"" не соответствует требованиям образования имен переменных.';en='The method name ""%1"" does not match the requirements of the education variables.'"), ИмяМетода);
	КонецПопытки;
