...
Процедура ПолучитьКонтекстЭДО(ВыполняемоеОповещение, ОбновитьСейчас = Ложь) Экспорт
	...
	ИначеЕсли ДокументооборотСКОВызовСервера.ПодключатьВнешнююОбработкуЭДО() Тогда
		Если ДокументооборотСКОВызовСервера.ЕстьПравоНаДОсКО(Истина) Тогда
			...
			Исключение
				Состояние(НСтр("ka='Не удалось загрузить внешний модуль для документооборота с налоговыми органами."
"Будет использован модуль, встроенный в конфигурацию.';ru='Не удалось загрузить внешний модуль для документооборота с налоговыми органами."
"Будет использован модуль, встроенный в конфигурацию.'"));
		...
		Иначе
			ТекстСообщения = НСтр("ka='Недостаточно прав для использования методов электронного документооборота с контролирующими органами.';ru='Недостаточно прав для использования методов электронного документооборота с контролирующими органами.';en='Not have permission to use electronic document management practices with regulatory authorities.'");
...
Процедура ОбновитьИПолучитьКонтекстЭДО(ВыполняемоеОповещение) Экспорт
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить внешний модуль для документооборота с налоговыми органами."
"%1"
"Будет продолжено использование текущего модуля конфигурации.';ru='Не удалось загрузить внешний модуль для документооборота с налоговыми органами."
"%1"
...
Функция ОрганизацияПодключенаКНаправлению(ОрганизацияОтчета, КонтролирующийОрган) Экспорт
	...
	Если НЕ ПодключенЭДО Тогда
		...
		ТекстПредупреждения = НСтр("ru='Для организации не включена возможность отправки в %1!';en='For organizations do not include the ability to send to% 1!'");
...
Процедура ПроверитьВИнтернете(Форма, КонтролирующийОрган = "ФНС", АдресГотовыхДанных = Неопределено, ИмяФайлаГотовыхДанных = Неопределено) Экспорт
	...
	ИначеЕсли ЭтоУведомлениеФНС Тогда
		...
		Если НЕ ЗначениеЗаполнено(СсылкаНаОтчет) ИЛИ Форма.Модифицированность Тогда
			ПоказатьПредупреждение(,НСтр("ka='Перед проверкой необходимо записать уведомление.';ru='Перед проверкой необходимо записать уведомление.';en='Before check must be written notice.'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ЭтоЖурналСчетовФактурФНС Тогда
		...
		Если НЕ ЗначениеЗаполнено(СсылкаНаОтчет) ИЛИ Форма.Модифицированность Тогда
			ПоказатьПредупреждение(,НСтр("ka='Перед проверкой необходимо записать журнал.';ru='Перед проверкой необходимо записать журнал.';en='Before checking you want to record log.'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ЭтоЗаявлениеОВвозе Тогда
		...
		Если НЕ ЗначениеЗаполнено(СсылкаНаОтчет) ИЛИ Форма.Модифицированность Тогда
			ПоказатьПредупреждение(,НСтр("ka='Перед проверкой необходимо записать заявление.';ru='Перед проверкой необходимо записать заявление.';en='Before testing the application you want to record.'"));
			Возврат;
		КонецЕсли;
	...
	Иначе // все, кроме документа РегламентированныйОтчет
		...
		Если НЕ ЗначениеЗаполнено(СсылкаНаОтчет) Тогда
			ПоказатьПредупреждение(,НСтр("ru='Перед проверкой необходимо записать отчет.';en='Before checking the report to be written.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьВИнтернетеПослеПолученияКонтекста(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("ka='Недостаточно прав для использования методов электронного документооборота с контролирующими органами.';ru='Недостаточно прав для использования методов электронного документооборота с контролирующими органами.';en='Not have permission to use electronic document management practices with regulatory authorities.'"));
	КонецЕсли;
