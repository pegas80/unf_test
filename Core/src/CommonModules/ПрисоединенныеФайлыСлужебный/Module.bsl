			...
			Иначе // Перечисления.ТипыХраненияФайлов.ВИнформационнойБаза
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						НСтр("ka='ფაილები. მონაცემთა გაცვლის დროს ფაილის გაგზავნა ვერ მოხერხდა';ru='Файлы.Не удалось отправить файл при обмене данными';en='Fayly.Ne able to send data exchange file'",
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка) Экспорт
	...
	Попытка
		...
		ТекстОписания = НСтр("ka='ფაილი წაიშალა უსარგებლო ფაილების გასუფთავებისას.';ru='Файл удален при очистке ненужных файлов.';en='File removed by cleaning junk files.'")
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ფაილები. ფაილის ჩაწერა დისკზე.';ru='Файлы.Запись файла на диск';en='Fayly.Zapis file to disk'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
	...
	Исключение
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ფაილები. ფაილების საცავში მიერთებული ფაილის მონაცემთა განახლება';ru='Файлы.Обновление данных присоединенного файла в хранилище файлов';en='Fayly.Obnovlenie data attached file in the file store'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
"указан несуществующий справочник ""%2"".';ka='შეცდომა ფაილების შესანახი ცნობარების სახელების დადგენისას. ""%1"" ტიპის ფაილების მფლობელს მითითებული აქვს არარსებული ცნობარი ""%2"".'"),
...
"без окончания ""ПрисоединенныеФайлы"".';ka='შეცდომა ფაილების შესანახად ცნობარების სახელების დადგენისას. ""%1"" ტიპის ფაილების მფლობელს მითითებული აქვს ცნობარის სახელი ""%2"" დაბოლოების გარეშე ""მიერთებულიფაილები"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
		...
		ИначеЕсли КлючИЗначение.Значение = Истина Тогда
			Если ОсновнойСправочникУказан Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"основной справочник указан более одного раза.';ka='შეცდომა ფაილების შესანახი ცნობარების სახელების დადგენისას. ""%1"" ტიპის ფაილების მფლობელს ძირითადი ცნობარი ერთზე მეტჯერ აქვს მითითებული.'"),
					Строка(ТипВладельцаФайлов),
					Строка(КлючИЗначение.Ключ));
			КонецЕсли;
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"не имеется справочников для хранения файлов.';ka='შეცდომა ფაილების შესანახი ცნობარების სახელების დადგენისას. ""%1"" ტიპის ფაილების მფლობელს ფაილების შესანახად ცნობარები არ გააჩნია.'"),
			Строка(ТипВладельцаФайлов));
	КонецЕсли;
...
"нет справочников для хранения файлов.';ka='%2 ტიპის ფაილების მფლობელ ""%1""-ს ფაილების შესანახი ცნობარები არ აქვს.'"),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС
			+ НСтр("ru='У владельца файлов ""%1"" типа ""%2"""
"нет справочника ""%3"" для хранения файлов.';ka='%2 ტიპის ფაილების მფლობელს ""%1"" ფაილების შესანახად ცნობარი ""%3"" არ გააჩნია.'"),
			Строка(ВладелецФайлов),
			Строка(ТипЗнч(ВладелецФайлов)),
	...
	ШаблонПричиныОшибки = 
		НСтр("ru='У владельца файлов ""%1"" типа ""%2"""
"не указан основной справочник для хранения файлов.';ka='%2 ტიპის ფაილების მფლობელს ""%1"" ფაილების შესანახად ძირითადი ცნობარი არ აქვს მითითებული.'") + Символы.ПС;
...
"""%1""."
""
"Ссылка на файл: ""%2"".';ka='შეცდომა, ფაილის შენახვისას ტომში:"
"""%1""."
""
"ბმული ფაილზე: ""%2"".'"),
		СообщениеОбОшибке,
