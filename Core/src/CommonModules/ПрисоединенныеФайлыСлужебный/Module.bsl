			...
			Иначе // Перечисления.ТипыХраненияФайлов.ВИнформационнойБаза
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						НСтр("ka='ფაილები. მონაცემთა გაცვლის დროს ფაილის გაგზავნა ვერ მოხერხდა';ru='Файлы.Не удалось отправить файл при обмене данными'",
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка) Экспорт
	...
	Попытка
		...
		ТекстОписания = НСтр("ka='ფაილი წაიშალა უსარგებლო ფაილების გასუფთავებისას.';ru='Файл удален при очистке ненужных файлов.'")
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ფაილები. ფაილის ჩაწერა დისკზე.';ru='Файлы.Запись файла на диск'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
	...
	Исключение
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации(
			НСтр("ka='ფაილები. ფაილების საცავში მიერთებული ფაილის მონაცემთა განახლება';ru='Файлы.Обновление данных присоединенного файла в хранилище файлов'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
