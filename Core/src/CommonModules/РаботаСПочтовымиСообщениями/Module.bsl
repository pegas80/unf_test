...
Функция ЗагрузитьПочтовыеСообщения(Знач УчетнаяЗапись,
	...
	Если НЕ ИспользоватьДляПолучения Тогда
		ВызватьИсключение НСтр("ka='სააღრიცხვო ჩანაწერი არ არის განკუთვნილი შეტყობინების მისაღებად.';ru='Учетная запись не предназначена для получения сообщений.';en='Account is not intended to receive messages.'");
	КонецЕсли;
