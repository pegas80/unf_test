...
Функция КодСериализацииПочтовогоИндекса() Экспорт
	...
	Возврат КодСериализацииОбъектаАдресации(НСтр("ka='საფოსტო ინდექსი';ru='Почтовый индекс'"));
...
Функция СокращенияОбъектовАдресацииАдресаРФ() Экспорт
	...
	Результат.Вставить(НСтр("ka='სახლი';ru='Дом'"), НСтр("ka='д.';ru='Д.'"));
	Результат.Вставить(НСтр("ka='მფლობელობა';ru='Владение'"), НСтр("ka='Вл.';ru='Вл.'"));
	Результат.Вставить(НСтр("ka='სახლის მფლობელობა';ru='Домовладение'"), НСтр("ka='სახლმფლობელობა';ru='Домовл.'"));
	
	Результат.Вставить(НСтр("ka='კორპუსი';ru='Корпус'"), НСтр("ka='კორპ.';ru='Корп.'"));
	Результат.Вставить(НСтр("ka='Строение';ru='Строение'"), НСтр("ka='Стр.';ru='Стр.'"));
	Результат.Вставить(НСтр("ka='ლიტერა';ru='Литера'"), НСтр("ka='ლიტ.';ru='Лит.'"));
	Результат.Вставить(НСтр("ka='შენობა';ru='Сооружение'"), НСтр("ka='ნაგებობა';ru='Сооруж.'"));
	Результат.Вставить(НСтр("ka='უბანი';ru='Участок'"), НСтр("ka='Уч.';ru='Уч.'"));
	
	Результат.Вставить(НСтр("ka='ბინა';ru='Квартира'"), НСтр("ka='კვ.';ru='Кв.'"));
	Результат.Вставить(НСтр("ka='Офис';ru='Офис'"), НСтр("ka='оф.';ru='Оф.'"));
	Результат.Вставить(НСтр("ka='Бокс';ru='Бокс'"), НСтр("ka='Бокс';ru='Бокс'"));
	Результат.Вставить(НСтр("ka='სათავსო';ru='Помещение'"), НСтр("ka='Пом.';ru='Пом.'"));
	Результат.Вставить(НСтр("ka='ოთახი';ru='Комната'"), НСтр("ka='Ком.';ru='Ком.'"));
	Результат.Вставить(НСтр("ka='Этаж';ru='Этаж'"), НСтр("ka='Этаж';ru='Этаж'"));
...
Функция СтруктураЧастейАдресаНаселенногоПункта(ВариантКлассификатора = "ФИАС") Экспорт
	...
	Результат.Вставить("Регион",           ЭлементАдреснойСтруктуры(НСтр("ka='რეგიონი';ru='Регион'"),      НСтр("ka='მისამართის რეგიონი';ru='Регион адреса'"),           "СубъектРФ",     1));
	Если ВариантКлассификатора = "ФИАС" Тогда
		Результат.Вставить("Округ",        ЭлементАдреснойСтруктуры(НСтр("ka='ოლქი';ru='Округ'"),       НСтр("ka='მისამართის არეალი';ru='Округ адреса'"),            "Округ",         2));
	КонецЕсли;
	Результат.Вставить("Район",            ЭлементАдреснойСтруктуры(НСтр("ka='რაიონი';ru='Район'"),       НСтр("ka='მისამართის რაიონი';ru='Район адреса'"),            "СвРайМО/Район", 3));
	Результат.Вставить("Город",            ЭлементАдреснойСтруктуры(НСтр("ka='ქალაქი';ru='Город'"),       НСтр("ka='მისამართის ქალაქი';ru='Город адреса'"),            "Город",         4));
	Если ВариантКлассификатора = "ФИАС" Тогда
		Результат.Вставить("ВнутригРайон", ЭлементАдреснойСтруктуры(НСтр("ka='Внутр. р-н.';ru='Внутр. р-н.'"), НСтр("ka='შიდასაქალაქო რაიონი';ru='Внутригородской район'"),   "ВнутригРайон",  5));
	КонецЕсли;
	Результат.Вставить("НаселенныйПункт", ЭлементАдреснойСтруктуры(НСтр("ka='დასახლებული პუნქტი';ru='Нас.пункт'"),
		НСтр("ka='დასახლებული პუნქტი მისამართები';ru='Населенный пункт адреса'"), "НаселПункт",  6, Истина));
		
	Если ВариантКлассификатора <> "БезКлассификатора" Тогда
		Результат.Вставить("Улица", ЭлементАдреснойСтруктуры(НСтр("ka='ქუჩა';ru='Улица'"),
			НСтр("ka='ქუჩა მისამართები';ru='Улица адреса'"), "Улица", 7));
		Результат.Вставить("ДополнительныйЭлемент", ЭлементАдреснойСтруктуры(НСтр("ka='ДополнительныйЭлемент';ru='ДополнительныйЭлемент'"),
			НСтр("ka='მისამართის დამატებითი ელემენტი';ru='Дополнительный элемент адреса'"), "ДопАдрЭл[ТипАдрЭл='10200000']", 90));
		Результат.Вставить("ПодчиненныйЭлемент", ЭлементАдреснойСтруктуры(НСтр("ka='დაქვემდებარებული ელემენტი';ru='Подчиненный элемент'"),
			НСтр("ka='მისამართის დაქვემდებარებული ელემენტი';ru='Подчиненный элемент адреса'"), "ДопАдрЭл[ТипАдрЭл='10400000']", 91));
	КонецЕсли;
...
Функция ТекстПустогоАдресаВВидеГиперссылки() Экспорт
	Возврат НСтр("ka='შევსება';ru='Заполнить'");
КонецФункции
