...
Функция ПолучитьСтруктуруПараметровОбменаДанными(ПараметрыОбменаДанными) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='შეცდომა მონაცემთა გაცვლის პარამეტრების წაკითხვისას:';ru='Ошибка при чтении параметров обмена данными:'");
...
Функция ПолучитьДеревоИспользуемыхМетаданных(ОписаниеМетаданных) Экспорт
	...
	Исключение
		...
		#Если ВнешнееСоединение Тогда
		ВызватьИсключение(НСтр("ka='მობილური დანართის მიერ ხმარებად მეტამონაცემთა ჩამონათვალის წაკითხვის შეცდომა';ru='Ошибка чтения перечня метаданных, используемых мобильным приложением'"));
		#КонецЕсли
...
Функция ПолучитьТипПоСтроке(XMLСтрокаТипа)
	...
	Иначе
		#Если ВнешнееСоединение Тогда
		ТекстСообщения = НСтр("ka='მონაცემთა უცნობი ტიპი გაცვლის პაკეტში';ru='Неизвестный тип данных в пакете обмена'");
...
Функция ПолучитьСтруктуруСоставаОбъекта(ОбъектМетаданных, ИмяКласса, ИмяОбъекта, ДеревоИспользуемыхМетаданных)
	...
	Если СтрокаКласса = Неопределено Тогда
		...
		#Если ВнешнееСоединение Тогда
		ТекстСообщения = НСтр("ka='გაცვლის პაკეტში არასწორი მონაცემები არის ჩაწერილი: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%დანართში არ გამოიყენება %ИмяКласса%';ru='В пакет обмена записаны неверные данные: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%В приложении не используются %ИмяКласса%'");
	...
	Если СтрокаОбъекта = Неопределено Тогда
		...
		#Если ВнешнееСоединение Тогда
		ТекстСообщения = НСтр("ka='გაცვლის პაკეტში ჩაწერილია არასწორი მონაცემები: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%დანართში არ გამოიყენება მოცემული ობიექტი';ru='В пакет обмена записаны неверные данные: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%В приложении не используется данный объект'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ИмяКласса%",ИмяКласса);
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ИмяОбъекта%",ИмяОбъекта);
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ПереводСтроки%",Символы.ПС);
