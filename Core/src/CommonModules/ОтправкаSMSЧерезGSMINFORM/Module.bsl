...
Функция ОтправитьSMS(НомераПолучателей, Текст, ИмяОтправителя, Логин, Пароль) Экспорт
	...
	Если ОтветСервера["error_no"] = 0 Тогда
		...
		Если Не ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
			Результат.ОписаниеОшибки = НСтр("ka='Не удалось отправить SMS';ru='Не удалось отправить SMS'") + ":" + СокрП(Результат.ОписаниеОшибки);
		КонецЕсли;
	Иначе
...
Функция ОписанияОшибок()
	...
	Результат.Вставить(1, НСтр("ka='არასწორი API-გასაღები.';ru='Неверный API-ключ.'"));
	Результат.Вставить(2, НСтр("ka='უცნობი ბრძანება.';ru='Неизвестная команда.'"));
	Результат.Вставить(3, НСтр("ka='Пользователь с указанным ID кабинета не найден.';ru='Пользователь с указанным ID кабинета не найден.'"));
	Результат.Вставить(4, НСтр("ka='Пустой список телефонов для отправки сообщений.';ru='Пустой список телефонов для отправки сообщений.'"));
	Результат.Вставить(5, НСтр("ka='Не указан текст сообщения.';ru='Не указан текст сообщения.'"));
	Результат.Вставить(6, НСтр("ka='Не удалось отправить сообщение на указанный номер.';ru='Не удалось отправить сообщение на указанный номер.'"));
	Результат.Вставить(7, НСтр("ka='Не указан отправитель по приоритетному трафику.';ru='Не указан отправитель по приоритетному трафику.'"));
	Результат.Вставить(8, НСтр("ka='Некорректный отправитель, допускается только латиница и цифры.';ru='Некорректный отправитель, допускается только латиница и цифры.'"));
	Результат.Вставить(9, НСтр("ka='Пустой список идентификаторов сообщений для получения статусов.';ru='Пустой список идентификаторов сообщений для получения статусов.'"));
	Результат.Вставить(10, НСтр("ka='Не найдено сообщение с таким идентификатором.';ru='Не найдено сообщение с таким идентификатором.'"));
	Результат.Вставить(11, НСтр("ka='Не удалось оплатить рассылку, проверьте баланс.';ru='Не удалось оплатить рассылку, проверьте баланс.'"));
...
Функция ОписаниеОшибкиПолученияСтатуса(КодОшибки)
	...
	Если ТекстСообщения = Неопределено Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Статус сообщения не получен (код ошибки: %1).';ru='Статус сообщения не получен (код ошибки: %1).'"), КодОшибки);
	КонецЕсли;
	Возврат ТекстСообщения;
КонецФункции
...
Функция Разрешения() Экспорт
	...
	Адрес = "gsm-inform.ru";
	Порт = Неопределено;
	Описание = НСтр("ka='Отправка SMS через ""GSM-INFORM"".';ru='Отправка SMS через ""GSM-INFORM"".'");
