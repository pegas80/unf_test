...
Функция ПодготовитьДанныеЗапросаВУТМ(ТранспортныйМодуль, ВидДокумента, Знач ВходныеПараметры, ТекстОшибки) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ФорматОбмена) Тогда
		ТекстОшибки = НСтр("ka='Не заполнен формат обмена в настройках обмена с ЕГАИС.';ru='Не заполнен формат обмена в настройках обмена с ЕГАИС.';en='Not filled exchange format with EGAIS sharing settings.'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПодготовитьДанныеЗапросаВУТМЧерезОбработкуОбслуживания(ТранспортныйМодуль, ВидДокумента, Знач ВходныеПараметры, ТекстОшибки) Экспорт
	...
	Попытка
		...
		Если НЕ ПараметрыВыполненияКоманды.Свойство("Результат") Тогда
			ТекстОшибки = НСтр("ka='Ошибка выполнения команды.';ru='Ошибка выполнения команды.';en='Error executing the command.'");
		...
		КонецЕсли;
	Исключение
		ТекстОшибки = НСтр("ka='Ошибка при вызове обработки обслуживания.';ru='Ошибка при вызове обработки обслуживания.';en='An error occurred while processing the service call.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ЗагрузитьДокументыИзУТМ(ТранспортныйМодуль, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого URLДокумента Из СписокURL Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ka='შემომავალი დოკუმენტ %1-ის მიღება ვერ მოხერხდა';ru='Не удалось получить входящий документ %1';en='It failed to get the incoming document% 1'");
	...
	Для Каждого URLДокумента Из Результат.ЗагруженныеДокументы Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ka='Не удалось удалить входящий документ %1';ru='Не удалось удалить входящий документ %1';en='Unable to remove the incoming document% 1'");
...
Функция ОбработатьОтветыИзУТМ(ТранспортныйМодуль, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатОперации.Результат Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Не удалось получить список исходящих запросов.';ru='Не удалось получить список исходящих запросов.';en='Failed to get a list of outgoing requests.'") + Символы.ПС + РезультатОперации.ОписаниеОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ РезультатОперации.Результат Тогда
		Результат.ОписаниеОшибки = НСтр("ka='Не удалось получить список ответов.';ru='Не удалось получить список ответов.';en='Unable to obtain a list of answers.'") + Символы.ПС + РезультатОперации.ОписаниеОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого Ответ Из СписокURL Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ka='Не удалось получить ответ %1';ru='Не удалось получить ответ %1';en='Unable to get a response% 1'");
	...
	Для Каждого URLОтвета Из Результат.ЗагруженныеДокументы Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ka='Не удалось удалить ответ %1';ru='Не удалось удалить ответ %1';en='Unable to delete answer 1%'");
...
Функция ОбработатьВходящиеДокументы(Знач ТранспортныйМодуль, Знач МассивДокументов, Знач ДополнительныеПараметры) Экспорт
	...
	Если НЕ ПустаяСтрока(ТранспортныйМодуль.ИмяОбработкиОбслуживания) Тогда
		...
		Попытка
			...
			Если НЕ ПараметрыВыполненияКоманды.Свойство("Результат") Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка выполнения команды.';ru='Ошибка выполнения команды.';en='Error executing the command.'"));
			Иначе
				Результат = ПараметрыВыполненияКоманды.Результат;
			КонецЕсли;
		Исключение
...
Функция ПроверитьВозможностьВыгрузкиДокумента(ВидДокумента, ВходныеПараметры)
	...
	Если НЕ РеквизитыДокумента.Проведен Тогда
		Результат.ОписаниеОшибки = НСтр("ka='დოკუმენტი არ არის გატარებული.';ru='Документ не проведен.';en='The document is not held.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПодтвержденияТТН
		...
		ИначеЕсли РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.Новый
			...
			Результат.ОписаниеОшибки = НСтр("ka='Акт отказа требует только отправленный в ЕГАИС документ.';ru='Акт отказа требует только отправленный в ЕГАИС документ.';en='failure Act requires only sent in EGAIS document.'");
			Возврат Результат;
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНВходящейЕГАИС.ПринятИзЕГАИС
			...
			Если ТипЗнч(ВходныеПараметры.ДокументСсылка) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС") Тогда
				Результат.ОписаниеОшибки = НСтр("ka='Документ уже обработан грузополучателем. Отозвать документ нельзя.';ru='Документ уже обработан грузополучателем. Отозвать документ нельзя.';en='The document has been processed by the consignee. it is impossible to withdraw the document.'");
			Иначе
				Результат.ОписаниеОшибки = НСтр("ka='Документ не требует подтверждения.';ru='Документ не требует подтверждения.';en='The document does not require confirmation.'");
			КонецЕсли;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ПодтверждениеАктаРасхожденийТТН
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ПринятАктРасхождений
			...
			Результат.ОписаниеОшибки = НСтр("ka='Для документа не был принят акт расхождений.';ru='Для документа не был принят акт расхождений.';en='The document was not adopted act discrepancies.'");
			Возврат Результат;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросНаОтменуПроведенияТТН Тогда
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения
			...
			Результат.ОписаниеОшибки = НСтр("ka='Документ не подтвержден.';ru='Документ не подтвержден.';en='The document is not certified.'");
			Возврат Результат;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросНаОтменуПроведенияАктаПостановкиНаБаланс Тогда
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиАктаПостановкиНаБалансЕГАИС.ПереданВЕГАИС
			...
			Результат.ОписаниеОшибки = НСтр("ka='Документ не проведен в ЕГАИС.';ru='Документ не проведен в ЕГАИС.';en='The document is not held in EGAIS.'");
			Возврат Результат;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросОстатков
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиОстатковЕГАИС.Новый
			...
			Результат.ОписаниеОшибки = НСтр("ka='По документу уже были получены остатки.';ru='По документу уже были получены остатки.';en='According to the document, the remains have already been received.'");
			Возврат Результат;
...
Функция ТекстОшибкиПередаваемогоДокумента()
	...
	Возврат НСтр("ka='Документ передается в ЕГАИС, дождитесь уведомления о фиксации.';ru='Документ передается в ЕГАИС, дождитесь уведомления о фиксации.';en='The document is transmitted to the EGAIS, wait about fixing notice.'");
...
Функция ТекстОшибкиПереданногоДокумента()
	...
	Возврат НСтр("ka='Документ уже передан в ЕГАИС.';ru='Документ уже передан в ЕГАИС.';en='The document has already been submitted in EGAIS.'");
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если НЕ НовыйСтатус = Неопределено Тогда
			...
			Если НЕ РезультатОперации.Результат Тогда
				...
				ТекстСообщения = НСтр("ka='Не удалось изменить статус обработки документа %1.';ru='Не удалось изменить статус обработки документа %1.';en='Unable to change the processing status of the document% 1.'");
...
Функция ЗаблокироватьТранспортныйМодульДляОбмена(ТранспортныйМодуль, ИдентификаторКлиента = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			СтрокаПоиска = НСтр("ka='Объект уже заблокирован:';ru='Объект уже заблокирован:';en='The object is already locked:'");
			Если НЕ ПустаяСтрока(СтрокаПоиска) Тогда
				...
				Если ПозицияСимвола > 0 Тогда
					...
					ОписаниеОшибки = НСтр("ka='Обмен с УТМ по адресу %1 уже выполняется:';ru='Обмен с УТМ по адресу %1 уже выполняется:';en='Communication with UTM at% 1 is already running:'");
