...
Процедура ФормаНастройкиПередЗакрытием(Отказ, Форма, ЗавершениеРаботы) Экспорт
	...
	ТекстВопроса = НСтр("ka='მონაცემები შეიცვალა. დავხუროთ ფორმა ცვლილების შენახვის გარეშე?';ru='Данные были изменены. Закрыть форму без сохранения изменений?';en='The data have been changed. Close the form without saving changes?'");
...
Функция ЗаголовкиГиперссылокЗагрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено",               НСтр("ka='მონაცემთა მიღება არ შესრულებულა';ru='Получение данных не выполнялось';en='Receiving data is not'"));
	Структура.Вставить("Ошибка",                     НСтр("ka='მონაცემთა მიღება ვერ მოხერხდა';ru='Не удалось получить данные';en='Failed to retrieve data'"));
	Структура.Вставить("ВыполненоСПредупреждениями", НСтр("ka='მონაცემები მიღებულია გაფრთხილებებით';ru='Данные получены с предупреждениями';en='Data received warnings'"));
	Структура.Вставить("Успех",                      НСтр("ka='მონაცემები წარმატებით იქნა მიღებული';ru='Данные успешно получены';en='Data is successfully obtained'"));
	Структура.Вставить("Выполнение",                 НСтр("ka='ხორციელდება მონაცემთა მიღება...';ru='Выполняется получение данных...';en='Retrieves data ...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ka='მისაღებად ახალი მონაცემები არ არის';ru='Нет новых данных для получения';en='No recent activity for'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ka='მონაცემთა მიღება ვერ მოხერხდა';ru='Не удалось получить данные';en='Failed to retrieve data'"));
...
Функция ЗаголовкиГиперссылокВыгрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено", НСтр("ka='მონაცემთა გაგზავნა არ შესრულებულა';ru='Отправка данных не выполнялась';en='Sending the data has not been performed'"));
	Структура.Вставить("Ошибка",       НСтр("ka='მონაცემთა გაგზავნა ვერ მოხერხდა';ru='Не удалось отправить данные';en='Failed to send data'"));
	Структура.Вставить("Успех",        НСтр("ka='მონაცემები წარმატებით გაიგზავნა';ru='Данные успешно отправлены';en='Successfully sent!'"));
	Структура.Вставить("Выполнение",   НСтр("ka='მიმდინარეობს მონაცემთა გაგზავნა...';ru='Выполняется отправка данных...';en='You are sending data ...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ka='მონაცემები გაიგზავნა გაფრთხილებებით';ru='Данные отправлены с предупреждениями';en='Data sent warning'"));
	Структура.Вставить("ВыполненоСПредупреждениями",                     НСтр("ka='მონაცემები გაიგზავნა გაფრთხილებებით';ru='Данные отправлены с предупреждениями';en='Data sent warning'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ka='მონაცემთა გაგზავნა ვერ მოხერხდა';ru='Не удалось отправить данные';en='Failed to send data'"));
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок", НСтр("ka='მიუთითეთ კატალოგი';ru='Укажите каталог';en='specify the directory'") );
	
	УстановитьЗначенияСтруктурыПоУмолчанию(ПараметрыДиалога, УмолчанияДиалога);
	
	ТекстПредупреждения = НСтр("ka='მოცემული ოპერაციისთვის აუცილებელია ვებ-კლიენტ 1С:საწარმოსთვის გაფართოების დაყენება.';ru='Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.';en='For this operation, you must install the extension for Web client 1C: Enterprise.'");
...
Процедура ОбработчикВыбораФайла(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок",                   НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл';en='select a file'"));
...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		...
		Если ПустаяСтрока(ИмяФайла) Тогда
			ЕстьПустые = Истина;
			ОписаниеФайла.ОписаниеОшибки = НСтр("ka='ფაილი არ არის არჩეული.';ru='Файл не выбран.';en='The file is not selected.'");
		КонецЕсли;
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ka='სერვერზე ფაილის გადასაცემად აუცილებელია ვებ-კლიენტ 1C:საწარმოსთვის გაფართოების დაყენება.';ru='Для передачи файла на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';en='Enterprise: You must install the extension for the web client to the server file 1C for transmission.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='ფაილების გადასაცემად აუცილებელია ვებ-კლიენტ 1С:საწარმოსთვის სერვერზე გაფართოების დაყენება.';ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';en='you must install the extension for Web client 1C to transfer files to the server: Enterprise.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередатьФайлНаСерверЗавершениеПодключенияРасширенияПослеПроверкиСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		...
		Элемент.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილი ""%1"" არ არსებობს ან მასთან წვდომა არ არის';ru='Файл ""%1"" не существует или к нему нет доступа';en='File ""%1"" does not exist or can not be accessed'"), Элемент.Имя);
...
Процедура ВыбратьИСохранитьФайлНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	УмолчанияДиалога = Новый Структура;
	УмолчанияДиалога.Вставить("Заголовок",               НСтр("ka='აირჩიეთ ფაილი შესანახად';ru='Выберите файл для сохранения';en='Select the file to save'"));
	УмолчанияДиалога.Вставить("МножественныйВыбор",      Ложь);
	УмолчанияДиалога.Вставить("ПредварительныйПросмотр", Ложь);
...
"с помощью параметра запуска конфигуратора /ResetMasterNode.';ka='მთავარი კვანძიდან საინფორმაციო ბაზის გამორთვა უკვე შეიძლება კონფიგურატორ /ResetMasterNode-ის გაშვების პარამეტრის მეშვეობით.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
		ТекстВопроса = НСтр("ka='წავშალოთ მონაცემთა სინქრონიზაციის გამართვა?';ru='Удалить настройку синхронизации данных?';en='Remove configuration data synchronization?'");
...
Процедура ПриОткрытииИнструкцииКакИзменитьПарольСинхронизацииДанных(Знач АдресДляВосстановленияПароляУчетнойЗаписи) Экспорт
	...
	Если ПустаяСтрока(АдресДляВосстановленияПароляУчетнойЗаписи) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='სააღრიცხვო ჩანაწერის პაროლის აღსადგენად პაროლი არ არის მიცემული.';ru='Адрес для восстановления пароля учетной записи не задан.';en='Address is not set to recover the account password.'"));
...
Функция ОткрытьФормуДополненияВыгрузкиВсеДокументы(Знач ДополнениеВыгрузки, Знач Владелец=Неопределено, Знач Уникальность=Неопределено, Знач Окно=Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ka='გასაგზავნი დოკუმენტების დამატება';ru='Добавление документов для отправки';en='Adding documents to send'") );
