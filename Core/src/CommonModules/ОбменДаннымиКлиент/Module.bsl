...
Процедура ФормаНастройкиПередЗакрытием(Отказ, Форма, ЗавершениеРаботы) Экспорт
	...
	ТекстВопроса = НСтр("ka='მონაცემები შეიცვალა. დავხუროთ ფორმა ცვლილების შენახვის გარეშე?';ru='Данные были изменены. Закрыть форму без сохранения изменений?'");
...
Функция ЗаголовкиГиперссылокЗагрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено",               НСтр("ka='მონაცემთა მიღება არ შესრულებულა';ru='Получение данных не выполнялось'"));
	Структура.Вставить("Ошибка",                     НСтр("ka='მონაცემთა მიღება ვერ მოხერხდა';ru='Не удалось получить данные'"));
	Структура.Вставить("ВыполненоСПредупреждениями", НСтр("ka='მონაცემები მიღებულია გაფრთხილებებით';ru='Данные получены с предупреждениями'"));
	Структура.Вставить("Успех",                      НСтр("ka='მონაცემები წარმატებით იქნა მიღებული';ru='Данные успешно получены'"));
	Структура.Вставить("Выполнение",                 НСтр("ka='ხორციელდება მონაცემთა მიღება...';ru='Выполняется получение данных...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ka='მისაღებად ახალი მონაცემები არ არის';ru='Нет новых данных для получения'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ka='მონაცემთა მიღება ვერ მოხერხდა';ru='Не удалось получить данные'"));
...
Функция ЗаголовкиГиперссылокВыгрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено", НСтр("ka='მონაცემთა გაგზავნა არ შესრულებულა';ru='Отправка данных не выполнялась'"));
	Структура.Вставить("Ошибка",       НСтр("ka='მონაცემთა გაგზავნა ვერ მოხერხდა';ru='Не удалось отправить данные'"));
	Структура.Вставить("Успех",        НСтр("ka='მონაცემები წარმატებით გაიგზავნა';ru='Данные успешно отправлены'"));
	Структура.Вставить("Выполнение",   НСтр("ka='მიმდინარეობს მონაცემთა გაგზავნა...';ru='Выполняется отправка данных...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ka='მონაცემები გაიგზავნა გაფრთხილებებით';ru='Данные отправлены с предупреждениями'"));
	Структура.Вставить("ВыполненоСПредупреждениями",                     НСтр("ka='მონაცემები გაიგზავნა გაფრთხილებებით';ru='Данные отправлены с предупреждениями'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ka='მონაცემთა გაგზავნა ვერ მოხერხდა';ru='Не удалось отправить данные'"));
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок", НСтр("ka='მიუთითეთ კატალოგი';ru='Укажите каталог'") );
	
	УстановитьЗначенияСтруктурыПоУмолчанию(ПараметрыДиалога, УмолчанияДиалога);
	
	ТекстПредупреждения = НСтр("ka='მოცემული ოპერაციისთვის აუცილებელია ვებ-კლიენტ 1С:საწარმოსთვის გაფართოების დაყენება.';ru='Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.'");
...
Процедура ОбработчикВыбораФайла(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок",                   НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл'"));
...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		...
		Если ПустаяСтрока(ИмяФайла) Тогда
			ЕстьПустые = Истина;
			ОписаниеФайла.ОписаниеОшибки = НСтр("ka='ფაილი არ არის არჩეული.';ru='Файл не выбран.'");
		КонецЕсли;
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ka='სერვერზე ფაილის გადასაცემად აუცილებელია ვებ-კლიენტ 1C:საწარმოსთვის გაფართოების დაყენება.';ru='Для передачи файла на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='ფაილების გადასაცემად აუცილებელია ვებ-კლიენტ 1С:საწარმოსთვის სერვერზე გაფართოების დაყენება.';ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередатьФайлНаСерверЗавершениеПодключенияРасширенияПослеПроверкиСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		...
		Элемент.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილი ""%1"" არ არსებობს ან მასთან წვდომა არ არის';ru='Файл ""%1"" не существует или к нему нет доступа'"), Элемент.Имя);
...
Процедура ВыбратьИСохранитьФайлНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	УмолчанияДиалога = Новый Структура;
	УмолчанияДиалога.Вставить("Заголовок",               НСтр("ka='აირჩიეთ ფაილი შესანახად';ru='Выберите файл для сохранения'"));
	УмолчанияДиалога.Вставить("МножественныйВыбор",      Ложь);
	УмолчанияДиалога.Вставить("ПредварительныйПросмотр", Ложь);
...
"с помощью параметра запуска конфигуратора /ResetMasterNode.';ka='მთავარი კვანძიდან საინფორმაციო ბაზის გამორთვა უკვე შეიძლება კონფიგურატორ /ResetMasterNode-ის გაშვების პარამეტრის მეშვეობით.'");
	...
	Иначе
		ТекстВопроса = НСтр("ka='წავშალოთ მონაცემთა სინქრონიზაციის გამართვა?';ru='Удалить настройку синхронизации данных?'");
...
Процедура ПриОткрытииИнструкцииКакИзменитьПарольСинхронизацииДанных(Знач АдресДляВосстановленияПароляУчетнойЗаписи) Экспорт
	...
	Если ПустаяСтрока(АдресДляВосстановленияПароляУчетнойЗаписи) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='სააღრიცხვო ჩანაწერის პაროლის აღსადგენად პაროლი არ არის მიცემული.';ru='Адрес для восстановления пароля учетной записи не задан.'"));
...
Функция ОткрытьФормуДополненияВыгрузкиВсеДокументы(Знач ДополнениеВыгрузки, Знач Владелец=Неопределено, Знач Уникальность=Неопределено, Знач Окно=Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ka='გასაგზავნი დოკუმენტების დამატება';ru='Добавление документов для отправки'") );
