...
Процедура УстановитьПараметрыЗадания(Задание, Использование, Параметры, Расписание) Экспорт
	...
	Если Не Константы.РазрешитьВыполнениеДОИОРегламентнымиЗаданиямиВМоделиСервиса.Получить() Тогда
		ВызватьИсключение НСтр("ka='დავალებების სახით დამატებითი დამუშავების ბრძანებების პერიოდული შესრულება სერვისის ადმინისტრაციის მიერ აკრძალულია!';ru='Периодическое выполнение команд дополнительных обработок в качестве заданий запрещено администрацией сервиса!';en='Periodic command execution additional processing tasks as smoking Services Administration!'");
	КонецЕсли;
	...
	Если Расписание.ТребуетсяВыполнение(ПроверяемаяДата, ИсходнаяДата) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='განრიგი, რომლის მიცემა ხდება დავალებების სახით დამატებითი ანგარიშგების ან დამუშავების ბრძანებების შესასრულებლად, %1 წამში 1-ზე უფრო ხშირად არ უნდა იყოს!';ru='Расписание, задаваемое для выполнения команд дополнительного отчета или обработки в качестве заданий, должно быть не чаще, чем 1 раз в %1 секунд!';en='It is set to perform an additional statement or command processing as a job, it should be no more than 1 time in% 1 seconds!'"), МинимальныйИнтервал);
