		...
		Если ИзмененияШаблонов.Удаленные.Количество() > 0
			...
			Пока Выборка.Следующий() Цикл
				...
				Если СтрокаШаблона = Неопределено Тогда
					ШаблонСообщения = НСтр("ka='განახლების დროს დავალების შაბლონი ვერ მოიძებნა %1';ru='При обновлении не найден шаблон задания %1';en='When updating the job template is not found% 1'");
