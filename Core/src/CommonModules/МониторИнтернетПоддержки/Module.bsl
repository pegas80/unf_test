...
Процедура ПриСозданииКонтекстаВзаимодействия(Контекст) Экспорт
	...
	Контекст.Вставить("СообщениеОНедоступностиДействия",
		НСтр("ka='ამ კონფიგურაციისთვის ინტერნეტ-მხარდაჭერის მონიტორის გამოსახვა მიუწვდომელია.';ru='Отображение монитора Интернет-поддержки недоступно для этой конфигурации.';en='Displays monitor Internet support is not available for this configuration.'"));
	...
	Иначе
		...
		Если НастройкаПоказыватьПриОбновлении = Истина
			...
			КонтекстОбработчика.Вставить("ОписаниеПричиныОстановки",
				НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერის სერვერზე ახალი ინფორმაცია არ არის.';ru='Новой информации на сервере Интернет-поддержки пользователей нет.';en='New information on the server to support Internet users there.'"));
