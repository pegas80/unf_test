...
Функция ПодключитьВнешнююОбработку(Ссылка) Экспорт
	...
	ЗаписатьПримечание(Ссылка, НСтр("ka='შეერთება, უსაფრთხორეჟიმი = ""%1"".';ru='Подключение, БезопасныйРежим = ""%1"".'"), БезопасныйРежим);
...
Процедура ПечатьПоВнешнемуИсточнику(ДополнительнаяОбработкаСсылка, ПараметрыИсточника, КоллекцияПечатныхФорм,
	...
	Если ВнешняяОбработкаОбъект = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='გარე დამუშავება ""%1""-ს (ტიპი ""%2"") ქვესისტემა ""%3"" არ ემსახურება';ru='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается подсистемой ""%3""'"),
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		Если ПечатнаяФорма.ТабличныйДокумент = Неопределено Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ბეჭდვის დამამუშავებელში არ იქნა ფორმირებული ცხრილი-დოკუმენტი %1-ისთვის';ru='В обработчике печати не был сформирован табличный документ для: %1'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები';ru='Дополнительные отчеты и обработки'");
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები';ru='Дополнительные отчеты и обработки'");
...
"Для включения подсистемы обратитесь к администратору программы.';ka='ელემენტი ""%1"" არ არის ჩართული, ვინაიდან ქვესისტემა ""%2"" პროგრამის გამართვებში გამორთულია. ქვესისტემის ჩასართავად მიმართეთ პროგრამის ადმინისტრატორს.'"),
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ანგარიშგება ""%1""-ის შეერთებისას შეცდომა წარმოიშვა:';ru='При подключении дополнительного отчета ""%1"" возникла ошибка:'"), Строка(Ссылка))
	...
	Иначе
		...
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ელემენტი %1 არ წარმოადგენს დამატებით ანგარიშგებას';ru='Элемент %1 не является дополнительным отчетом'"),
			"'"+ Строка(Ссылка) +"'");
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='В процедуру ""%1"" не передан отчет';ru='В процедуру ""%1"" не передан отчет'"),
...
"Обратитесь к администратору программы.'"),
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ობიექტი ""%1"" დამატებით ანგარიშგებას არ წარმოადგენს';ru='Объект ""%1"" не является дополнительным отчетом'"), Строка(Ссылка));
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';ru='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЕстьПравоЧтения Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';ru='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ОтчетДоступенВМоделиСервиса Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<Отчет ""%1"" недоступен в модели сервиса.';ru='<Отчет ""%1"" недоступен в модели сервиса.'"),
					СтрокаТаблицы.Представление);
			Иначе
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("ka='დამატებით ანგარიშგებებთან და დამუშავებებთან სწრაფი წვდომის გამართვები';ru='Настройки быстрого доступа к дополнительным отчетам и обработкам'");
...
Функция ПодключенныеОбъектыМетаданных(Вид) Экспорт
	...
	Для Каждого ТипИлиМетаданные Из МассивТиповИлиМетаданных Цикл
		...
		Если ОбъектМетаданных = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола() Тогда
			...
			СтрокаТаблицы.Представление = НСтр("ka='სამუშაო მაგიდა';ru='Рабочий стол'");
		Иначе
			...
			Если Не Выборка.ЭтоГруппа И Не ЗначениеЗаполнено(Выборка.РежимСовместимостиРазрешений) Тогда
				...
				Исключение
					...
					ТекстОшибки = """" + Объект.Наименование + """:"
						+ Символы.ПС + НСтр("ka='ნებართვების თავსებადობის რეჟიმის დადგენა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось определить режим совместимости разрешений по причине:'")
						+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
						+ Символы.ПС
						+ Символы.ПС + НСтр("ka='ობიექტი დაბლოკილია ვერსია 2.1.3-თან თავსებადობის რეჟიმში.';ru='Объект заблокирован в режиме совместимости с версией 2.1.3.'");
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, НСтр("ka='ბრძანება %1:გაშვება';ru='Команда %1: Запуск.'"), ИдентификаторКоманды);
	...
	Исключение
		ЗаписатьОшибку(
			ВнешняяОбработка,
			НСтр("ka='ბრძანება %1: შესრულების შეცდომა: %2';ru='Команда %1: Ошибка выполнения:%2'"),
			ИдентификаторКоманды,
			Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
	// Запись журнала регистрации
	ЗаписатьИнформацию(ВнешняяОбработка, НСтр("ka='ბრძანება %1: დასრულება.';ru='Команда %1: Завершение.'"), ИдентификаторКоманды);
...
Функция ПредставлениеРаздела(Раздел) Экспорт
	Если Раздел = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола()
		...
		Возврат НСтр("ka='სამუშაო მაგიდა';ru='Рабочий стол'");
	КонецЕсли;
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ნაწილი ""%1""';ru='Раздел ""%1""'"), ПредставлениеРаздела);
КонецФункции
...
Процедура СформироватьПодменюКомандЗаполнения(Форма, Параметры)
	...
	Если Подменю = Неопределено Тогда
		...
		Подменю.Заголовок = НСтр("ka='შევსება';ru='Заполнить'");
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბრძანება %1 არ არის აღმოჩენილი!';ru='Команда %1 не обнаружена!'"), ИдентификаторКоманды);
	КонецЕсли;
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.БезопасныйРежим
		...
		// ничего не делаем
	Иначе
		Результат.ТекстОшибки = НСтр("ka='არა უსაფრთხო რეჟიმში გასაშვები დამუშავების შესაერთებლად, საჭიროა ადმინისტრაციული უფლებები.';ru='Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.'");
		Возврат Результат;
	КонецЕсли;
...
"Для загрузки нового объекта нажмите ""Создать"".'"),
	...
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> (РегистрационныеДанные.Вид = ВидДополнительныйОтчет ИЛИ РегистрационныеДанные.Вид = ВидОтчет) Тогда
		Результат.ТекстОшибки = НСтр("ka='დამუშავების სახეობა, რომელიც გარე დამუშავების შესახებ ინფორმაციაში არის მითითებული, მის გაფართოებას არ შეესაბამება.';ru='Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბრძანება ""%1""-ისთვის გაშვების მეთოდი არ არის დადგენილი.';ru='Для команды ""%1"" не определен способ запуска.'"), КомандаОписание.Представление));
		КонецЕсли;
...
"Возможно, она не подходит для этой версии программы.'");
		...
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ka='Техническая информация:';ru='Техническая информация:'") + Символы.ПС;
