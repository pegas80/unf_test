...
Функция ПодключитьВнешнююОбработку(Ссылка) Экспорт
	...
	ЗаписатьПримечание(Ссылка, НСтр("ka='შეერთება, უსაფრთხორეჟიმი = ""%1"".';ru='Подключение, БезопасныйРежим = ""%1"".';en='Connection, BezopasnyyRezhim = ""%1"".'"), БезопасныйРежим);
...
Процедура ПриСозданииНаСервере(Форма, ТипФормы = Неопределено) Экспорт
	...
	РегламентированныйУчетСервер.УстановитьВидимостьУпрУчета(Форма);
...
Процедура ПечатьПоВнешнемуИсточнику(ДополнительнаяОбработкаСсылка, ПараметрыИсточника, КоллекцияПечатныхФорм,
	...
	Если ВнешняяОбработкаОбъект = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='გარე დამუშავება ""%1""-ს (ტიპი ""%2"") ქვესისტემა ""%3"" არ ემსახურება';ru='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается подсистемой ""%3""';en='External processing ""%1"" (type ""%2"") is not in service subsystem ""%3""'"),
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		Если ПечатнаяФорма.ТабличныйДокумент = Неопределено Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ბეჭდვის დამამუშავებელში არ იქნა ფორმირებული ცხრილი-დოკუმენტი %1-ისთვის';ru='В обработчике печати не был сформирован табличный документ для: %1';en='The print handler was not formed for a spreadsheet document:% 1'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები';ru='Дополнительные отчеты и обработки';en='Advanced reporting and processing'");
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები';ru='Дополнительные отчеты и обработки';en='Advanced reporting and processing'");
...
"Для включения подсистемы обратитесь к администратору программы.';ka='ელემენტი ""%1"" არ არის ჩართული, ვინაიდან ქვესისტემა ""%2"" პროგრამის გამართვებში გამორთულია. ქვესისტემის ჩასართავად მიმართეთ პროგრამის ადმინისტრატორს.'"),
			"'"+ Строка(Ссылка) +"'",
			ДополнительныеОтчетыИОбработкиКлиентСервер.НаименованиеПодсистемы(Истина));
		Возврат;
	КонецЕсли;
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დამატებითი ანგარიშგება ""%1""-ის შეერთებისას შეცდომა წარმოიშვა:';ru='При подключении дополнительного отчета ""%1"" возникла ошибка:';en='When you connect an additional report ""%1"" An error has occurred:'"), Строка(Ссылка))
	...
	Иначе
		...
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ელემენტი %1 არ წარმოადგენს დამატებით ანგარიშგებას';ru='Элемент %1 не является дополнительным отчетом';en='Element 1% is not an additional report'"),
			"'"+ Строка(Ссылка) +"'");
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='В процедуру ""%1"" не передан отчет';ru='В процедуру ""%1"" не передан отчет';en='The procedure ""%1"" not passed report'"),
...
"Обратитесь к администратору программы.';ka='Функциональная опция ""%2"" отключена в настройках программы."
"Обратитесь к администратору программы.'"),
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ობიექტი ""%1"" დამატებით ანგარიშგებას არ წარმოადგენს';ru='Объект ""%1"" не является дополнительным отчетом';en='Object ""%1"" is not an additional report'"), Строка(Ссылка));
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';ru='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';en='<Report ""%1"" is not available because Additional reports and processing are disabled in the settings.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЕстьПравоЧтения Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';ru='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';en='<Report ""%1"" is not available because there is no right to read more reports and data processors.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ОтчетДоступенВМоделиСервиса Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<Отчет ""%1"" недоступен в модели сервиса.';ru='<Отчет ""%1"" недоступен в модели сервиса.';en='<Report ""%1"" is not available in the service model.'"),
					СтрокаТаблицы.Представление);
			Иначе
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("ka='დამატებით ანგარიშგებებთან და დამუშავებებთან სწრაფი წვდომის გამართვები';ru='Настройки быстрого доступа к дополнительным отчетам и обработкам';en='Settings quick access to additional reports and treatments'");
...
Функция ПодключенныеОбъектыМетаданных(Вид) Экспорт
	...
	Для Каждого ТипИлиМетаданные Из МассивТиповИлиМетаданных Цикл
		...
		Если ОбъектМетаданных = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола() Тогда
			...
			СтрокаТаблицы.Представление = НСтр("ka='სამუშაო მაგიდა';ru='Рабочий стол';en='Desktop'");
		Иначе
			...
			Если Не Выборка.ЭтоГруппа И Не ЗначениеЗаполнено(Выборка.РежимСовместимостиРазрешений) Тогда
				...
				Исключение
					...
					ТекстОшибки = """" + Объект.Наименование + """:"
						+ Символы.ПС + НСтр("ka='ნებართვების თავსებადობის რეჟიმის დადგენა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось определить режим совместимости разрешений по причине:';en='Unable to determine permissions compatibility mode for a reason:'")
						+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
						+ Символы.ПС
						+ Символы.ПС + НСтр("ka='ობიექტი დაბლოკილია ვერსია 2.1.3-თან თავსებადობის რეჟიმში.';ru='Объект заблокирован в режиме совместимости с версией 2.1.3.';en='The object is locked in compatibility mode with version 2.1.3.'");
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, НСтр("ka='ბრძანება %1:გაშვება';ru='Команда %1: Запуск.';en='The% 1: Start.'"), ИдентификаторКоманды);
	...
	Исключение
		ЗаписатьОшибку(
			ВнешняяОбработка,
			НСтр("ka='ბრძანება %1: შესრულების შეცდომა: %2';ru='Команда %1: Ошибка выполнения:%2';en='The% 1: Run-time error:% 2'"),
			ИдентификаторКоманды,
			Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
	// Запись журнала регистрации
	ЗаписатьИнформацию(ВнешняяОбработка, НСтр("ka='ბრძანება %1: დასრულება.';ru='Команда %1: Завершение.';en='The% 1: Completion.'"), ИдентификаторКоманды);
...
Функция ПредставлениеРаздела(Раздел) Экспорт
	Если Раздел = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола()
		...
		Возврат НСтр("ka='სამუშაო მაგიდა';ru='Рабочий стол';en='Desktop'");
	КонецЕсли;
	...
	Если ТипЗнч(Раздел) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		...
		//Малышев 2024-07-21 ONESOFT-3315
		Если ЗначениеЗаполнено(Раздел) Тогда
			ПредставлениеРаздела = Метаданные.Подсистемы.Найти(Раздел.Имя).Представление();
		КонецЕсли;	
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ნაწილი ""%1""';ru='Раздел ""%1""';en='Section 1""'"), ПредставлениеРаздела);
КонецФункции
...
Процедура СформироватьПодменюКомандЗаполнения(Форма, Параметры)
	...
	Если Подменю = Неопределено Тогда
		...
		Подменю.Заголовок = НСтр("ka='შევსება';ru='Заполнить';en='Insert'");
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბრძანება %1 არ არის აღმოჩენილი!';ru='Команда %1 не обнаружена!';en='The% 1 was not found!'"), ИдентификаторКоманды);
	КонецЕсли;
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.БезопасныйРежим
		...
		// ничего не делаем
	Иначе
		Результат.ТекстОшибки = НСтр("ka='არა უსაფრთხო რეჟიმში გასაშვები დამუშავების შესაერთებლად, საჭიროა ადმინისტრაციული უფლებები.';ru='Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.';en='To connect the processing is started in unsecured mode, it requires administrative privileges.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ Объект.ЭтоНовый() И РегистрационныеДанные.Вид <> Объект.Вид Тогда
		Результат.ТекстОшибки = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ჩასატვირთი ობიექტის სახეობა (%1) მიმდინარეს (%2) არ შეესაბამება. ახალი ობიექტის ჩასატვირთად დააჭირეთ ""შექმნა""-ს.';ru='Вид загружаемого объекта (%1) не соответствует текущему (%2)."
"Для загрузки нового объекта нажмите ""Создать"".'"),
	...
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> (РегистрационныеДанные.Вид = ВидДополнительныйОтчет ИЛИ РегистрационныеДанные.Вид = ВидОтчет) Тогда
		Результат.ТекстОшибки = НСтр("ka='დამუშავების სახეობა, რომელიც გარე დამუშავების შესახებ ინფორმაციაში არის მითითებული, მის გაფართოებას არ შეესაბამება.';ru='Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';en='processing type specified in the information about the external treatment does not correspond to its expansion.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბრძანება ""%1""-ისთვის გაშვების მეთოდი არ არის დადგენილი.';ru='Для команды ""%1"" не определен способ запуска.';en='For command ""%1"" is not defined way to run.'"), КомандаОписание.Представление));
		КонецЕсли;
...
"%2';ka='Ошибка при получении регистрационных данных:"
"Ошибка при удалении временного файла ""%1"":"
"%2'"),
				РезультатРегистрации.ИмяОбъекта,
...
"Возможно, он не подходит для этой версии программы.';ka='Невозможно подключить дополнительный отчет из файла."
...
"Возможно, она не подходит для этой версии программы.';ka='Невозможно подключить дополнительную обработку из файла."
"Возможно, она не подходит для этой версии программы.'");
		...
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ka='Техническая информация:';ru='Техническая информация:';en='Technical information:'") + Символы.ПС;
