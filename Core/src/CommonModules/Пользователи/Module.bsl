...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ka='ცნობარი ""მომხმარებელი"" განუცალკევებელ რეჟიმში მიუწვდომელია.';ru='Справочник Пользователи недоступен в неразделенном режиме.'");
	КонецЕсли;
	...
	Если ПользовательИБ = Неопределено Тогда
		...
		Если ПользователиИБ.Количество() = 0 Тогда
			Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
				ВызватьИсключение
					НСтр("ka='მონაცემთა არეალის პირველი ადმინისტრატორის ავტომატურად შექმნა შეუძლებელია.';ru='Невозможно автоматически создать первого администратора области данных.'");
			КонецЕсли;
...
"ცნობარში მომხმარებლის ავტომატურად შესაქმნელად, საჭიროა მისთვის შემდეგი როლების დანიშვნა: ""სრული უფლებები"" და ""სისტემის ადმინისტრატორი"".'");
	...
	Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
		?(ПользовательИБ = Неопределено,
		  НСтр("ka='პირველი ადმინისტრატორის შექმნა შესრულებულია.';ru='Выполнено создание первого администратора.'"),
		  ?(ПользовательСоздан,
		    НСтр("ka='ადმინისტრატორი შეპირისპირებულია ცნობარის ახალ მომხმარებელთან.';ru='Администратор сопоставлен с новым пользователем справочника.'"),
		    НСтр("ka='ადმინისტრატორი შედარებულია ცნობარის არსებულ მომხმარებელთან.';ru='Администратор сопоставлен с существующим пользователем справочника.'")) ) );
...
Функция ПолноеИмяНеуказанногоПользователя() Экспорт
	...
	Возврат НСтр("ka='<მითითებული არ არის>';ru='<Не указан>'");
...
Функция ПрочитатьПользователяИБ(Знач Идентификатор,
	...
	Если ПользовательИБ = Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Пользователь информационной базы ""%1"" не найден.';ru='Пользователь информационной базы ""%1"" не найден.'"),
			Идентификатор);
		Возврат Ложь;
	КонецЕсли;
...
Процедура НайтиНеоднозначныхПользователейИБ(Знач Пользователь,
	...
	ОписаниеОшибки = НСтр("ka='შეცდომა მონაცემთა ბაზაში:';ru='Ошибка в базе данных:'") + Символы.ПС;
		...
		Если Строка.НеоднозначныйИдентификатор <> ТекущийНеоднозначныйИдентификатор Тогда
			...
			Иначе
				...
				Если ТекущийПользовательИБ = Неопределено Тогда
					ИмяДляВхода = НСтр("ka='<ვერ მოიძებნა>';ru='<не найден>'");
				Иначе
		...
		Иначе
			...
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + "	"
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='""%1"" ბმულის იდენტიფიკატორით %2';ru='""%1"" с идентификатором ссылки %2'"),
					Строка.Пользователь,
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მომხმარებლები. საინფორმაციო ბაზის მომხმარებლის ჩაწერის შეცდომა';ru='Пользователи.Ошибка записи пользователя ИБ'",
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
				"""" + ИмяДляВхода + """ (" + ?(ЗначениеЗаполнено(ИдентификаторПользователяИБ),
					НСтр("ka='Новый';ru='Новый'"), Строка(ИдентификаторПользователяИБ)) + ")",
