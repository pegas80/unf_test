...
Процедура ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(Источник,
	...
	ЗаписьЖурналаРегистрации(
		?(УзелПроверкиЗапретаЗагрузки <> Неопределено,
			НСтр("ka='შეცვლის აკრძალვის თარიღები. ნაპოვნია ჩატვირთვის აკრძალვები';ru='Даты запрета изменения.Найдены запреты загрузки';en='Dates ban bans izmeneniya.Naydeny download'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ka='ცვლილების აკრძალვის თარიღები. ცვლილების აკრძალვები მოიძებნა';ru='Даты запрета изменения.Найдены запреты изменения';en='Dates ban izmeneniya.Naydeny changes bans'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
"не предусмотрены ни для одного плана обмена.';ka='Даты запрета загрузки данных прошлых периодов из других программ"
...
Процедура УстановитьНачальнуюДатуЗапретаИзменения() Экспорт
	...
	Запись.Комментарий  = "(" + НСтр("ka='ძირითადი მნიშვნელობა';ru='По умолчанию';en='Default'") + ")";
	...
	Если ИсточникиДанных = Неопределено
	 Или ИсточникиДанных.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შეცვლის აკრძალვის შესამოწმებლად არ არის ნაპოვნი მონაცემთა წყაროები ცხრილ ""%1""-ისთვის.';ru='Для проверки запрета изменения не найдены"
...
Функция ТекущиеИсточникиДанныхДляПроверкиЗапретаИзменения(СсылкиИменРазделов)
	...
	Для Каждого Строка Из Таблицы Цикл
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მონაცემთა წყაროში ცვლილების აკრძალვის შესამოწმებლად არასწორად არის მითითებული ცხრილი"
"""%1"".';ru='Для проверки запрета изменения "
...
Функция ПолеТаблицы(Источник, ВидПоля, ОбъектМетаданных, ЭтоРегистр)
	...
	Если Поля.Количество() = 0 Тогда
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მონაცემთა წყაროში შეცვლის აკრძალვის შესამოწმებლად ცხრილ ""%1"" -ისთვის არ არის მითითებული თარიღის ველი.';ru='Для проверки запрета изменения "
...
"не задано поле даты.'"),
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Поля[0]) Тогда
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ცხრილ ""%1""-ისთვის მონაცემთა წყაროში ცვლილების აკრძალვის შესამოწმებლად თარიღის ველი არასწორად არის მიცემული: ""%2""';ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле даты: ""%2""'"),
				Источник.Таблица, Поле);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ცხრილ ""%1""-ისთვის მონაცემთა წყაროში ცვლილების აკრძალვის შესამოწმებლად ობიექტის ველი არასწორად არის მიცემული: ""%2""';ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле объекта: ""%2""'"),
	...
	Если Поля.Количество() = 1 Тогда
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მონაცემთა წყაროში ცვლილების აკრძალვის შესამოწმებლად ცხრილ ""%1""-ისთვის თარიღის ველი არასწორად არის მიცემული:"
"მოცემულ ცხრილ ""%2""-ის ველი არ არის მიცემული.';ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
...
"не задано поле заданной табличной части ""%2"".'"),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მონაცემთა წყაროში ცხრილ ""%1""-ისთვის ცვლილების აკრძალვის შესამოწმებლად ობიექტის ველი არასწორად არის მიცემული: მოცემული ცხრილ ""%2""-ის ველი არ არის მიცემული.';ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
...
"не задано поле заданной табличной части ""%2"".'"),
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Поля[1]) Тогда
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მონაცემთა წყაროში ცვლილების აკრძალვის შესამოწმებლად ცხრილ ""%1""-ისთვის თარიღის ველი არასწორად არის მიცემული:"
"მოცემული ცხრილ ""%2""-ის ველი არასწორად არის მიცემული.';ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле даты:"
"неверно задано поле заданной табличной части ""%2"".'"),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ცხრილ ""%1""-ისთვის მონაცემთა წყაროში ცვლილების აკრძალვის შემოწმებისას ობიექტის ველი არასწორად არის მიცემული: მიცემული ცხრილ ""%2""-ის ველი არასწორად არის მიცემული.';ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле объекта:"
"неверно задано поле заданной табличной части ""%2"".'"),
				Источник.Таблица, Поля[0]);
