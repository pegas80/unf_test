...
Процедура ПроверитьСсылкиНаОбъект(Знач ОбработкаПродолжения, Знач МассивСсылок, Знач СинонимыРеквизитов) Экспорт
	...
	ЗаголовокДиалога = НСтр("ka='რეკვიზიტების რედაქტირების ნებართვა';ru='Разрешение редактирования реквизитов';en='Resolution details edit'");
	...
	Если СинонимыРеквизитов.Количество() > 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='იმისთვის, რომ არ დავუშვათ მონაცემთა განთანხმოება პროგრამაში, რედაქტირებისთვის მიუწვდომელია შემდეგი რეკვიზიტები:"
"%1."
""
"მათი რედაქტირების ნებართვამდე, რეკომენდებულია შედეგების შეფასება, პროგრამაში ამ ელემენტის გამოყენების ყველა ადგილის შემოწმებით."
"გამოყენების ადგილების ძებნას შეიძლება ხანგრძლივი დრო დაჭირდეს.';ru='Для того чтобы не допустить рассогласования данных в программе,"
...
"Поиск мест использования может занять длительное время.'"),
	...
	Иначе
		Если МассивСсылок.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პროგრამაში მონაცემთა განთანხმოების არდასაშვებად, რეკვიზიტი %1 რედაქტირებისთვის მიუწვდომელია."
""
"მისი რედაქტირების ნებართვამდე, რეკომენდებულია შედეგების შეფასება პროგრამაში გამოყენების ყველა ადგილ ""%2""-ის შემოწმებით."
"გამოყენების ადგილების ძებნას შეიძლება ხანგრძლივი დრო დაჭირდეს.';ru='Для того чтобы не допустить рассогласования данных в программе,"
...
"Поиск мест использования может занять длительное время.'"),
		...
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პროგრამაში მონაცემთა განთანხმოების არდასაშვებად, რეკვიზიტი %1 რედაქტირებისთვის მიუწვდომელია."
""
"მისი რედაქტირების ნებართვამდე, რეკომენდებულია შედეგების შეფასება, არჩეული ელემენტების (%2) პროგრამაში გამოყენების ყველა ადგილის შემოწმებით."
"გამოყენების ადგილების ძებნას შეიძლება ხანგრძლივი დრო დაჭირდეს.';ru='Для того чтобы не допустить рассогласования данных в программе,"
"реквизит %1 не доступен для редактирования."
""
"Перед тем, как разрешить его редактирование, рекомендуется оценить последствия,"
...
"Поиск мест использования может занять длительное время.'"),
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='შემოწმება და დაშვება';ru='Проверить и разрешить';en='Check and resolve'"));
...
Процедура ПоказатьПредупреждениеВсеВидимыеРеквизитыРазблокированы(ОбработкаПродолжения = Неопределено) Экспорт
	...
	ПоказатьПредупреждение(ОбработкаПродолжения,
		НСтр("ka='ობიექტის ყველა ხილვადი რეკვიზიტის რედაქტირება უკვე დასრულებულია.';ru='Редактирование всех видимых реквизитов объекта уже разрешено.';en='Editing of all visible details of the object it is already allowed.'"));
