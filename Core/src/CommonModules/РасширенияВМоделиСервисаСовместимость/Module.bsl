...
Функция РазделитьВерсию(Знач ВерсияСтрокой, Разделитель = ".", ЛюбаяВерсия = "*") Экспорт
	...
	Если НЕ ФорматСоблюден Тогда
		...
		Описание = НСтр("ka='Версия """"{Значение}"""" не соответствует формату Редакция.Подредакция.Версия.Сборка.';ru='Версия """"{Значение}"""" не соответствует формату Редакция.Подредакция.Версия.Сборка.'");
