...
Процедура ЗаполнитьНаименованиеРабочегоМеста(Объект, ИмяПользователя) Экспорт
	...
	ИмяПустойПользователь = НСтр("ka='<მომხმარებელი>';ru='<Пользователь>';en='<User>'");
...
Функция ПолучитьСерийныйНомерПоTID(Знач TID, Знач EPC = Неопределено) Экспорт
	...
	ОписаниеОшибки = НСтр("ka='TID-ის მიხედვით სერიული ნომრის გენერაციის შეცდომა.';ru='Ошибка генерации серийного номера по TID.';en='generate an error on the serial number TID.'");
	...
	Если ПризнакG2 <> "E2" Тогда // Фиксированное значение "E2" признак того что тип соответствует EPC Class1Gen2. 
		РезультатВыполнения.ОписаниеОшибки = НСтр("ka='აღნიშნული TID არ შეესაბამება EPC-ის ტიპს Class1Gen2.';ru='Указанный TID не соответствует типу EPC Class1Gen2.';en='Said TID does not match EPC Class1Gen2 type.'");
	...
	ИначеЕсли Производитель = "003" Или Производитель = "803" Тогда // Чипы Alien Technology  
		...
		Если ПустаяСтрока(EPC) Тогда
			РезультатВыполнения.ОписаниеОшибки = НСтр("ka='Alien Technology-ის ჩიპებისთვის სერიული ნომრის გენერაციისთვის აუცილებელია EPC-ის მითითება.';ru='Для генерация серийного номера для чипов ""Alien Technology"" необходимо указание EPC .';en='For the generation of the serial number for the chips ""Alien Technology"" you must specify the EPC.'");
			Возврат РезультатВыполнения;
		КонецЕсли;
		
		СтруктураEPC = ДекодированиеДанныхSGTIN(EPC);
		Если Не СтруктураEPC.Результат Тогда
			РезультатВыполнения.ОписаниеОшибки = НСтр("ka='Alien Technology-ის ჩიპებისთვის EPC-ის დეკოდირების შეცდომა .';ru='Ошибка декодирования EPC для чипов ""Alien Technology"".';en='EPC decoding error for ""Alien Technology"" chips.'");
			Возврат РезультатВыполнения;
		КонецЕсли;
	...
	Иначе
		...
		РезультатВыполнения.ОписаниеОшибки = НСтр("ka='სერიული ნომრის გენერაციას TID-ში მითითებული მწარმოებლის ჩიპისთვის მხარდაჭერა არ გააჩნია.';ru='Генерация серийного номера для чипа производителя указанного в TID не поддерживается.';en='The generation of the serial number for the chip manufacturer of the TID is not supported.'");
		Возврат РезультатВыполнения;
...
Функция ПараметрыОперацииФискализацииЧека() Экспорт;
	...
	ПараметрыЧека.Вставить("ТекстШапки"         , НСтр("ka='ДОБРО ПОЖАЛОВАТЬ!';ru='ДОБРО ПОЖАЛОВАТЬ!';en='WELCOME!'"));
	ПараметрыЧека.Вставить("ТекстПодвала"       , НСтр("ka='СПАСИБО ЗА ПОКУПКУ!';ru='СПАСИБО ЗА ПОКУПКУ!';en='THANK YOU FOR YOUR PURCHASE!'"));
...
Функция ПолучитьНаименованиеСистемыНалогообложения(КодСистемыНалогообложения) Экспорт;
	...
	СистемыНалогообложения.Вставить(0, НСтр("ka='ზოგადი';ru='Общая';en='Overall'"));
	СистемыНалогообложения.Вставить(1, НСтр("ka='გამარტივებული სემოსავალი';ru='Упрощенная Доход';en='Simplified Income'"));
	СистемыНалогообложения.Вставить(2, НСтр("ka='Упрощенная Доход минус Расход';ru='Упрощенная Доход минус Расход';en='Simplified income minus expenses'"));
	СистемыНалогообложения.Вставить(3, НСтр("ka='მთლიან შემოსავალზე ერთიანი გადასახადი';ru='Единый налог на вмененный доход';en='A single tax on imputed income'"));
	СистемыНалогообложения.Вставить(4, НСтр("ka='Единый сельскохозяйственный налог';ru='Единый сельскохозяйственный налог';en='Unified agricultural tax'"));
	СистемыНалогообложения.Вставить(5, НСтр("ka='დაბეგვრის საპატენტო სისტემა';ru='Патентная система налогообложения';en='The patent system of taxation'"));
...
Функция ПараметрыОткрытияЗакрытияСмены() Экспорт;
	...
	ПараметрыОперации.Вставить("Кассир", НСтр("ka='ადმინისტრატორი';ru='Администратор';en='Administrator'")); // Должность и фамилия лица, осуществившего операцию.
...
Функция ТребуетсяЗакрытиеСмены(СтатусСмены) Экспорт
	...
	ИначеЕсли СтатусСмены = 2 или СтатусСмены = 3 Тогда // открыта или истекла
		Возврат Истина;
	Иначе
		ТекстОшибки = НСтр("ka='Неизвестный статус смены';ru='Неизвестный статус смены';en='Unknown status change'");
