...
Процедура ЗаполнитьНаименованиеРабочегоМеста(Объект, ИмяПользователя) Экспорт
	...
	ИмяПустойПользователь = НСтр("ka='<მომხმარებელი>';ru='<Пользователь>'");
...
Функция ПолучитьСерийныйНомерПоTID(Знач TID, Знач EPC = Неопределено) Экспорт
	...
	ОписаниеОшибки = НСтр("ka='TID-ის მიხედვით სერიული ნომრის გენერაციის შეცდომა.';ru='Ошибка генерации серийного номера по TID.'");
	...
	Если ПризнакG2 <> "E2" Тогда // Фиксированное значение "E2" признак того что тип соответствует EPC Class1Gen2. 
		РезультатВыполнения.ОписаниеОшибки = НСтр("ka='აღნიშნული TID არ შეესაბამება EPC-ის ტიპს Class1Gen2.';ru='Указанный TID не соответствует типу EPC Class1Gen2.'");
	...
	ИначеЕсли Производитель = "003" Или Производитель = "803" Тогда // Чипы Alien Technology  
		...
		Если ПустаяСтрока(EPC) Тогда
			РезультатВыполнения.ОписаниеОшибки = НСтр("ka='Alien Technology-ის ჩიპებისთვის სერიული ნომრის გენერაციისთვის აუცილებელია EPC-ის მითითება.';ru='Для генерация серийного номера для чипов ""Alien Technology"" необходимо указание EPC .'");
			Возврат РезультатВыполнения;
		КонецЕсли;
		
		СтруктураEPC = ДекодированиеДанныхSGTIN(EPC);
		Если Не СтруктураEPC.Результат Тогда
			РезультатВыполнения.ОписаниеОшибки = НСтр("ka='Alien Technology-ის ჩიპებისთვის EPC-ის დეკოდირების შეცდომა .';ru='Ошибка декодирования EPC для чипов ""Alien Technology"".'");
			Возврат РезультатВыполнения;
		КонецЕсли;
	...
	Иначе
		...
		РезультатВыполнения.ОписаниеОшибки = НСтр("ka='სერიული ნომრის გენერაციას TID-ში მითითებული მწარმოებლის ჩიპისთვის მხარდაჭერა არ გააჩნია.';ru='Генерация серийного номера для чипа производителя указанного в TID не поддерживается.'");
		Возврат РезультатВыполнения;
...
Функция ПараметрыОперацииФискализацииЧека() Экспорт;
	...
	ПараметрыЧека.Вставить("ТекстШапки"         , НСтр("ka='ДОБРО ПОЖАЛОВАТЬ!';ru='ДОБРО ПОЖАЛОВАТЬ!'"));
	ПараметрыЧека.Вставить("ТекстПодвала"       , НСтр("ka='СПАСИБО ЗА ПОКУПКУ!';ru='СПАСИБО ЗА ПОКУПКУ!'"));
...
Функция ПолучитьНаименованиеСистемыНалогообложения(КодСистемыНалогообложения) Экспорт;
	...
	СистемыНалогообложения.Вставить(0, НСтр("ka='ზოგადი';ru='Общая'"));
	СистемыНалогообложения.Вставить(1, НСтр("ka='გამარტივებული სემოსავალი';ru='Упрощенная Доход'"));
	СистемыНалогообложения.Вставить(2, НСтр("ka='Упрощенная Доход минус Расход';ru='Упрощенная Доход минус Расход'"));
	СистемыНалогообложения.Вставить(3, НСтр("ka='მთლიან შემოსავალზე ერთიანი გადასახადი';ru='Единый налог на вмененный доход'"));
	СистемыНалогообложения.Вставить(4, НСтр("ka='Единый сельскохозяйственный налог';ru='Единый сельскохозяйственный налог'"));
	СистемыНалогообложения.Вставить(5, НСтр("ka='დაბეგვრის საპატენტო სისტემა';ru='Патентная система налогообложения'"));
...
Функция ПараметрыОткрытияЗакрытияСмены() Экспорт;
	...
	ПараметрыОперации.Вставить("Кассир", НСтр("ka='ადმინისტრატორი';ru='Администратор'")); // Должность и фамилия лица, осуществившего операцию.
...
Функция ТребуетсяЗакрытиеСмены(СтатусСмены) Экспорт
	...
	ИначеЕсли СтатусСмены = 2 или СтатусСмены = 3 Тогда // открыта или истекла
		Возврат Истина;
	Иначе
		ТекстОшибки = НСтр("ka='Неизвестный статус смены';ru='Неизвестный статус смены'");
