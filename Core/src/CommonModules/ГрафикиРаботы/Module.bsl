...
Функция ДатыПоГрафику(Знач ГрафикРаботы, Знач ДатаОт, Знач МассивДней, Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь, ВызыватьИсключение = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПоКалендарю = Неопределено Тогда
			СообщениеОбОшибке = НСтр("ka='მუშაობის გრაფიკი ""%1"" არ არის შევსებული თარიღ %2-იდან სამუშაო დღეების აღნიშნული რაოდენობით.';ru='График работы «%1» не заполнен с даты %2 на указанное количество рабочих дней.';en='Schedule ""%1"" is not filled with the date of 2% on a specified number of working days.'");
