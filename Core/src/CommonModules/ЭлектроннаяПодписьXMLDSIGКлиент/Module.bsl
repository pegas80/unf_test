...
Процедура ПослеПодключенияКомпоненты(Подключено, Контекст) Экспорт
	...
	ИначеЕсли Контекст.УстанавливатьКомпоненту Тогда
		...
		ТекстВопроса = НСтр("ka='Для подписания XML необходима установка компоненты XMLDSIG."
"Перейти к установке данной компоненты?';ru='Для подписания XML необходима установка компоненты XMLDSIG."
...
Процедура Подписание_ПослеВыполненияGetSignOIDFromCert(OIDАлгоритмаПодписи, Параметры, Контекст) Экспорт
	...
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода GetSignOIDFromCert произошла ошибка:';en='An error occurred while performing GetSignOIDFromCert method:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Подписание_ПослеВыполненияC14N_ПодписываемыйТег(КаноникализированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КаноникализированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода C14N произошла ошибка:';en='In carrying out the method C14N error occurred:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Подписание_ПослеВыполненияHash_ПодписываемыйТег(DigestValue, Параметры, Контекст) Экспорт
	...
	Если DigestValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода Hash произошла ошибка:';en='When the Hash method, an error occurred:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Подписание_ПослеВыполненияSign(SignatureValue, Параметры, Контекст) Экспорт
	...
	Если SignatureValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода Sign произошла ошибка:';en='When the Sign method of error:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура НачатьПроверкуПодписиСообщенияSOAP(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			СтрШаблон(
				НСтр("ka='Не удалось определить тип криптопровайдена %1';ru='Не удалось определить тип криптопровайдена %1';en='Unable to determine the type of% 1 kriptoprovaydena'"), ИнформацияМодуляКриптографии.Имя)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода VerifySign произошла ошибка:';en='An error occurred while performing VerifySign method:'"), Контекст);
		Возврат;
	КонецЕсли;
