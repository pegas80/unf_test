...
Функция Параметры() Экспорт
	...
	Если НЕ СохраненныеПараметры.Свойство("ВозможныеПраваДляНастройкиПравОбъектов") Тогда
		ПредставлениеПараметра = НСтр("ka='შესაძლებელი უფლებები ობიექტების უფლებების გასამართავად';ru='Возможные права для настройки прав объектов';en='Possible rights to configure the rights objects'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ПоставляемыеПрофилиГруппДоступа") Тогда
		ПредставлениеПараметра = НСтр("ka='დაშვების ჯგუფების მოსაწოდებელი მონაცემები';ru='Поставляемые профили групп доступа';en='Supplied profiles Access Groups'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ПредопределенныеПрофилиГруппДоступа") Тогда
		ПредставлениеПараметра = НСтр("ka='დაშვების ჯგუფების წინასწარ განსაზღვრული პროფილები';ru='Предопределенные профили групп доступа';en='Predefined profiles Access Groups'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("СвойстваВидовДоступа") Тогда
		ПредставлениеПараметра = НСтр("ka='დაშვების ტიპების თვისებები';ru='Свойства видов доступа';en='Access Properties types'");
	КонецЕсли;
...
"""%1"".';ka='საინფორმაციო ბაზის განახლების შეცდომა."
"დაშვების შეზღუდვის პარამეტრი არ არის შევსებული:"
"""%1"".'"),
				ПредставлениеПараметра)
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(ОграниченияПрав) Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока) Тогда
			...
			Если СтрЧислоВхождений(ТекущаяСтрока, ".") <> 3 И СтрЧислоВхождений(ТекущаяСтрока, ".") <> 5 Тогда
				ПояснениеОшибки = НСтр("ka='სტრიქონი უნდა იყოს შემდეგ ფორმატში ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".';ru='Строка должна быть в формате ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".';en='The string must be in the format ""<Full name of the table>. <Rule Name>. <Name of the type of access> [. Table Object]"".'");
			Иначе
				...
				Если Метаданные.НайтиПоПолномуИмени(Таблица) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ცხრილი ""%1"" არ მოიძებნა.';ru='Не найдена таблица ""%1"".';en='Table not found ""%1"".'"), Таблица);
				
				ИначеЕсли Право <> "Чтение" И Право <> "Изменение" Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='უფლება ""%1"" არ მოიძებნა.';ru='Не найдено право ""%1"".';en='We found the right ""%1"".'"), Право);
				
				ИначеЕсли ВРег(ВидДоступа) = ВРег("Объект") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='ობიექტის ცხრილი ""%1"" არ მოიძებნა.';ru='Не найдена таблица объекта ""%1"".';en='Found a table object ""%1"".'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВРег(ВидДоступа) = ВРег("НастройкиПрав") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='უფლებების გამართვების მფლობელის ცხრილი ვერ მოიძებნა ""%1"".';ru='Не найдена таблица владельца настроек прав ""%1"".';en='Found a table setting rights holder ""%1"".'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВидыДоступаПоИменам.Получить(ВидДоступа) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='დაშვების ტიპი ""%1"" ნაპოვნი არ არის.';ru='Не найден вид доступа ""%1"".';en='Unable to find the type of access ""%1"".'"), ВидДоступа);
				Иначе
...
"""%1""."
""
"';ka='შეცდომა მეტამონაცემთა ობიექტის უფლების შეზღუდვის ტიპის აღწერის სტრიქონში:"
"""%1"".'"),
						ТекущаяСтрока)
					+ ПояснениеОшибки;
			Иначе
