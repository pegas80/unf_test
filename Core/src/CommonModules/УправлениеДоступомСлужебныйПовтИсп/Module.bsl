...
Функция Параметры() Экспорт
	...
	Если НЕ СохраненныеПараметры.Свойство("ВозможныеПраваДляНастройкиПравОбъектов") Тогда
		ПредставлениеПараметра = НСтр("ka='შესაძლებელი უფლებები ობიექტების უფლებების გასამართავად';ru='Возможные права для настройки прав объектов'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ПоставляемыеПрофилиГруппДоступа") Тогда
		ПредставлениеПараметра = НСтр("ka='დაშვების ჯგუფების მოსაწოდებელი მონაცემები';ru='Поставляемые профили групп доступа'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ПредопределенныеПрофилиГруппДоступа") Тогда
		ПредставлениеПараметра = НСтр("ka='დაშვების ჯგუფების წინასწარ განსაზღვრული პროფილები';ru='Предопределенные профили групп доступа'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("СвойстваВидовДоступа") Тогда
		ПредставлениеПараметра = НСтр("ka='დაშვების ტიპების თვისებები';ru='Свойства видов доступа'");
	КонецЕсли;
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(ОграниченияПрав) Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока) Тогда
			...
			Если СтрЧислоВхождений(ТекущаяСтрока, ".") <> 3 И СтрЧислоВхождений(ТекущаяСтрока, ".") <> 5 Тогда
				ПояснениеОшибки = НСтр("ka='სტრიქონი უნდა იყოს შემდეგ ფორმატში ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".';ru='Строка должна быть в формате ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".'");
			Иначе
				...
				Если Метаданные.НайтиПоПолномуИмени(Таблица) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ცხრილი ""%1"" არ მოიძებნა.';ru='Не найдена таблица ""%1"".'"), Таблица);
				
				ИначеЕсли Право <> "Чтение" И Право <> "Изменение" Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='უფლება ""%1"" არ მოიძებნა.';ru='Не найдено право ""%1"".'"), Право);
				
				ИначеЕсли ВРег(ВидДоступа) = ВРег("Объект") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='ობიექტის ცხრილი ""%1"" არ მოიძებნა.';ru='Не найдена таблица объекта ""%1"".'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВРег(ВидДоступа) = ВРег("НастройкиПрав") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='უფლებების გამართვების მფლობელის ცხრილი ვერ მოიძებნა ""%1"".';ru='Не найдена таблица владельца настроек прав ""%1"".'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВидыДоступаПоИменам.Получить(ВидДоступа) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='დაშვების ტიპი ""%1"" ნაპოვნი არ არის.';ru='Не найден вид доступа ""%1"".'"), ВидДоступа);
				Иначе
