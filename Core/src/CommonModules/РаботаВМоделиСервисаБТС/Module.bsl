...
Процедура СоздатьНеразделенныеПредопределенныеЭлементы() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ka='ოპერაცია შეიძლება შესრულდეს მხოლოდ იმ სეანსში, რომელშიც გამყოფების მნიშვნელობები არ არის დაყენებული';ru='Операция может быть выполнена только в сеансе, в котором не установлены значения разделителей'");
...
Процедура ПриФормированииМанифестаКонфигурации(РасширенныеСведения) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='ოპერაცია ვერ შესრულდება აქტიური გარე ტრანზაქციის შემთხვევაში!';ru='Операция не может быть выполнена при активной внешней транзакции!'");
	КонецЕсли;
