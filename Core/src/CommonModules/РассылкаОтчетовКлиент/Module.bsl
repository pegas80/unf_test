...
Процедура ВыбратьПолучателя(ОбработчикРезультата, Объект, МножественныйВыбор, ВозвращатьСоответствие) Экспорт
	...
	Если МножественныйВыбор Тогда
		ВозможныеПолучатели.Вставить(0, Неопределено, НСтр("ka='ყველა მიმღებლებს';ru='Всем получателям';en='all recipients'"));
	КонецЕсли;
	...
	ВозможныеПолучатели.ПоказатьВыборЭлемента(Обработчик, НСтр("ka='მიმღების არჩევა';ru='Выбор получателя';en='Selecting a destination'"));
КонецПроцедуры
...
Процедура ЗапускФоновогоЗадания(Получатели, Параметры) Экспорт
	...
	Если Параметры.ЭтоФормаЭлемента Тогда
		...
		КонецЕсли;
		ТекстСостояния = НСтр("ka='მიმდინარეობს ანგარიშგებების დაგზავნა';ru='Выполняется рассылка отчетов';en='Performed sending reports'");
	Иначе
		ТекстСостояния = НСтр("ka='მიმდინარეობს ანგარიშგებების დაგზავნა';ru='Выполняются рассылки отчетов';en='Performed mailing reports'");
	КонецЕсли;
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, Параметры) Экспорт
	...
	Иначе
		...
		Предупреждение.Текст = НСтр("ka='Не удалось выполнить рассылки отчетов.';ru='Не удалось выполнить рассылки отчетов.';en='Failed to sending reports.'")
			+ Символы.ПС
