	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Если ПоискЗапретовЗагрузки Тогда
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ka='აკრძალულ პერიოდში %1-ის ჩატვირთვა შეუძლებელია.';ru='%1 невозможно загрузить в запрещенный период.'");
			Иначе
				Шаблон = НСтр("ka='%1 აკრძალულ პერიოდში ჩასატვირთი მონაცემებით შეცვლა შეუძლებელია.';ru='%1 в запрещенном периоде невозможно заменить загружаемыми данными.'");
			КонецЕсли;
		Иначе
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ka='აკრძალულ პერიოდში %1-ის მოთავსება შეუძლებელია.';ru='%1 невозможно поместить в запрещенный период.'");
			Иначе
				Шаблон = НСтр("ka='%1 აკრძალულ პერიოდში შეცვლა შეუძლებელია.';ru='%1 в запрещенном периоде невозможно изменить.'");
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого Запрет Из Запреты Цикл
		...
		Если Запрет.Раздел = Запрет.Объект Тогда
			Если Запрет.Раздел = ДатыЗапретаИзмененияСлужебныйПовтИсп.ПустойРаздел() Тогда
				Текст = Текст + НСтр("ka='თარიღს %1';ru='Дате %1'");
			Иначе
				Текст = Текст + НСтр("ka='თარიღს %1 ""%2"" ნაწილის მიხედვით';ru='Дате %1 по разделу ""%2""'");
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(СвойстваВстраивания.ЕдинственныйРаздел) Тогда
			Текст = Текст + НСтр("ka='თარიღს %1 ობიექტ ""%3""-ის მიხედვით';ru='Дате %1 по объекту ""%3""'");
		Иначе
			Текст = Текст + НСтр("ka='თარიღს %1 ""%2"" ნაწილის ობიექტ ""%3""-ის მიხედვით';ru='Дате %1 по объекту ""%3"" раздела ""%2""'");
		КонецЕсли;
		Если ПоискЗапретовЗагрузки Тогда
			Текст = Текст + " " + НСтр("ka='შეესაბამება მონაცემთა ჩატვირთვის აკრძალვა';ru='соответствует запрет загрузки данных'") + " ";
		Иначе
			Текст = Текст + " " + НСтр("ka='შეესაბამება მონაცემთა შეცვლის აკრძალვა';ru='соответствует запрет изменения данных'") + " ";
		КонецЕсли;
		Если Запрет.Адресат = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей Тогда
			Текст = Текст + НСтр("ka='ყველა მომხმარებლისთვის';ru='для всех пользователей'");
			
		ИначеЕсли Запрет.Адресат = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехИнформационныхБаз Тогда
			Текст = Текст + НСтр("ka='ყველა საინფორმაციო ბაზისთვის';ru='для всех информационных баз'");
		...
		ИначеЕсли ТипЗнч(Запрет.Адресат) = Тип("СправочникСсылка.ГруппыПользователей")
			...
			Текст = Текст + НСтр("ka='მომხმარებელთა ჯგუფ ""%4""-ისთვის';ru='для группы пользователей ""%4""'");
		...
		ИначеЕсли ТипЗнч(Запрет.Адресат) = Тип("СправочникСсылка.Пользователи")
			...
			Текст = Текст + НСтр("ka='მომხმარებლისთვის ""%4""';ru='для пользователя ""%4""'");
			
		ИначеЕсли ЗначениеЗаполнено(Запрет.Адресат) Тогда
			Текст = Текст + НСтр("ka='საინფორმაციო ბაზა ""%4""-სთვის';ru='для информационной базы ""%4""'");
		Иначе
			Текст = Текст + НСтр("ka='ყველა საინფორმაციო ბაზისთვის ""%6""';ru='для всех информационных баз ""%6""'");
		КонецЕсли;
		Текст = Текст + " " + НСтр("ka='%5-ით';ru='по %5'");
		Если НЕ СвойстваВстраивания.БезРазделовИОбъектов Тогда
			Если ЗначениеЗаполнено(Запрет.Раздел) Тогда
				Если Запрет.Объект = Запрет.Раздел Тогда
					Текст = Текст + " " + НСтр("ka='(განყოფილება ""%2"" -ზე დაყენებულია აკრძალვა)';ru='(запрет установлен на раздел ""%2"")'");
				ИначеЕсли ЗначениеЗаполнено(СвойстваВстраивания.ЕдинственныйРаздел) Тогда
					Текст = Текст + " " + НСтр("ka='(ობიექტ ""%3"" -ზე დაყენებულია აკრძალვა)';ru='(запрет установлен на объект ""%3"")'");
				Иначе
					Текст = Текст + " " + НСтр("ka='(განყოფილება ""%2"" -ის ობიექტ ""%3"" -ზე დაყენებულია აკრძალვა)';ru='(запрет установлен на объект ""%3"" раздела ""%2"")'");
				КонецЕсли;
			Иначе
				Текст = Текст + " " + НСтр("ka='(დადგენილია აკრძალვის საერთო თარიღი)';ru='(установлена общая дата запрета)'");
			КонецЕсли;
		КонецЕсли;
	...
	Если ЭтоРегистр Тогда
		...
		Если КоличествоПолей = 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ka='ველთან';ru='с полем'")  + " " + Строка(Данные.Отбор);
			
		ИначеЕсли КоличествоПолей > 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ka='ველებთან';ru='с полями'") + " " + Строка(Данные.Отбор);
		КонецЕсли;
