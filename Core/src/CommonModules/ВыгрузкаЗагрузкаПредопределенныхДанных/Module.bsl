...
"т.к. не требуется обеспечивать сопоставление ссылок на его предопределенные элементы!';ka='მეტამონაცემთა ობიექტი %1 არ შეიძლება, რომ დამუშავდეს შემდეგი დამამუშავებლით წინასწარგანსაზღვრულიმონაცემებისგადმოტვირთვაჩატვირთვა.ობიექტისგადმოტვირთვამდე(), ვინაიდან მის წინასწარ განსაზღვრულ ელემენტებზე ბმულების შედარების უზრუნველყოფა არ არის საჭირო!'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком"
"ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта(),"
"т.к. не может содержать предопределенных элементов!';ka='მეტამონაცემთა ობიექტი %1 ვერ დამუშავდება შემდეგი დამამუშავებლით წინასწარგანსაზღვრულმონაცემთაგადმოტვირთვაჩატვირთვა.ობიექტისგადმოტვირთვამდე(), ვინაიდან არ შეიძლება, რომ წინასწარ განსაზღვრულ ელემენტებს შეიცავდეს!'", Метаданные.ОсновнойЯзык.КодЯзыка),
			Объект.Метаданные().ПолноеИмя()
		);
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыИсходныхСсылок.ИмяПредопределенныхДанных) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Иначе
					...
					ВызватьИсключение СтрШаблон(
						НСтр("ka='ცხრილ %2-ში აღმოჩენილია წინასწარ განსაზღვრული ელემენტების %1 დუბლირება!';ru='Обнаружено дублирование предопределенных элементов %1 в таблице %2!';en='Detected duplication predefined elements in% 1% Table 2!'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
"т.к. не может содержать предопределенных элементов!';ka='მეტამონაცემთა ობიექტი %1 ვერ დამუშავდება შემდეგი დამამუშავებლით წინასწარგანსაზღვრულიმონაცემებისგადმოტვირთვაჩატვირთვა.ობიექტისჩატვირთვამდე(), ვინაიდან არ შეიძლება, რომ წინასწარ განსაზღვრულ ელემენტებს შეიცავდეს!'", Метаданные.ОсновнойЯзык.КодЯзыка),
			Объект.Метаданные().ПолноеИмя()
		);
