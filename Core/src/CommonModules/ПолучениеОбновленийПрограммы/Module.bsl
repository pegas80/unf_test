	...
	Если Разделы.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(ИнформацияОДоступномОбновлении.ИмяОшибки) Тогда
			...
			Для Каждого Раздел Из Разделы Цикл
				...
				Дело.Важное         = Истина;
				Дело.Представление  = НСтр("ka='Не удалось проверить наличие обновлений программы.';ru='Не удалось проверить наличие обновлений программы.';en='Unable to check for program updates.'");
		...
		ИначеЕсли ИнформацияОДоступномОбновлении.ДоступноОбновление Тогда
			...
			Если РазмерОбновления <> 0 Тогда
				ПредставлениеДелаРазмер =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='კონფიგურაციის ზომა: %1.';ru='Размер дистрибутива: %1.';en='Size distribution% 1.'"),
			...
			Для Каждого Раздел Из Разделы Цикл
				...
				Дело.Представление  = НСтр("ka='ხელმისაწვდომია პროგრამის განახლება';ru='Доступно обновление программы';en='Available update program'");
				Дело.Форма          = "Обработка.ОбновлениеПрограммы.Форма.Форма";
				Дело.ПараметрыФормы = Новый Структура("ИнформацияОбОбновлении", ИнформацияОДоступномОбновлении);
				Дело.Владелец       = Раздел;
				...
				Если РекомендуетсяУстановить Тогда
					...
					ДелоРекомендуетсяУстановить.Представление  = НСтр("ka='Рекомендуется установить это обновление.';ru='Рекомендуется установить это обновление.';en='It is recommended to install this update.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"downloads.v8.1c.ru",
			80,
			НСтр("ka='Получение файлов обновлений программы (зона ru)';ru='Получение файлов обновлений программы (зона ru)';en='Getting files for program updates (zone ru)'"));
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"downloads.v8.1c.eu",
			80,
			НСтр("ka='Получение файлов обновлений программы (зона eu)';ru='Получение файлов обновлений программы (зона eu)';en='Getting files for program updates (eu Area)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"update-api.1c.ru",
			443,
			НСтр("ka='Сервис получения обновлений программы (зона ru)';ru='Сервис получения обновлений программы (зона ru)';en='Service receiving program updates (zone ru)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"update-api.1c.eu",
			443,
			НСтр("ka='Сервис получения обновлений программы (зона eu)';ru='Сервис получения обновлений программы (зона eu)';en='Service receiving program updates (eu Area)'"));
		НовыеРазрешения.Добавить(Разрешение);
...
Процедура ЗагрузитьИУстановитьОбновленияВФоновомРежиме(Параметры) Экспорт
	...
	Если Не ДоступноИспользованиеОбновленияПрограммы(Истина, Ложь) Тогда
		ВызватьИсключение НСтр("ka='Использование обновления программы недоступно в текущем режиме работы.';ru='Использование обновления программы недоступно в текущем режиме работы.';en='Using the update program is not available in the current operation.'");
	КонецЕсли;
	...
	Если Контекст.ОбновитьПлатформу Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Получение обновления платформы 1С:Предприятие: версия %1; URL: %2';ru='Получение обновления платформы 1С:Предприятие: версия %1; URL: %2';en='Getting updates platform 1C: Enterprise: Version 1%; URL:% 2'"),
		...
		Если Не ПустаяСтрока(Контекст.КаталогДистрибутиваПлатформы) Тогда
			...
			ЗаписатьИнформациюВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Обновление платформы 1С:Предприятие уже было загружено ранее в %1';ru='Обновление платформы 1С:Предприятие уже было загружено ранее в %1';en='Update platform 1C: Enterprise has already been downloaded earlier 1%'"),
		...
		Иначе
			...
			Если КаталогУстановкиПлатформы <> Неопределено Тогда
				...
				ЗаписатьИнформациюВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Версия %1 платформы 1С:Предприятие уже установлена на компьютере.';ru='Версия %1 платформы 1С:Предприятие уже установлена на компьютере.';en='Version% 1 platform 1C: Enterprise is already installed on the computer.'"),
						Контекст.ВерсияПлатформы));
...
"Имя файла обновления (cfu): %7.'"),
	...
	Если Контекст.ОбновитьПлатформу И Не Контекст.ОбновлениеПлатформыУстановлено Тогда
		...
		Контекст.ТекущееДействие = НСтр("ka='Установка платформы 1С:Предприятие';ru='Установка платформы 1С:Предприятие';en='Installing the 1C: Enterprise'");
...
Процедура ЗагрузитьОбновлениеПлатформы(Контекст, Параметры)
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა დისტრიბუტივის შესანახი კატალოგის შექმნისას (%1).';ru='Ошибка при создании каталога для сохранения дистрибутива (%1).';en='Error creating a directory for saving distribution (1%).'"),
				КаталогДистрибутива)
			+ Символы.ПС
		...
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დისტრიბუტივის შესანახად კატალოგ %1-ის შექმნა ვერ მოხერხდა. %2';ru='Не удалось создать каталог %1 для сохранения дистрибутива. %2';en='Unable to create directory% 1 for storing distribution. 2%'"),
			КаталогДистрибутива,
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა დისტრიბუტივის ჩასატვირთად კატალოგის შექმნისას (%1).';ru='Ошибка при создании каталога для загрузки дистрибутива (%1).';en='Error creating a directory to download the distribution (% 1).'"),
				КаталогДляЗагрузки)
			+ Символы.ПС
			+ ПодробноеПредставлениеОшибки(ИнфОшибка);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Получение файла обновления платформы 1С:Предприятие: %1';ru='Получение файла обновления платформы 1С:Предприятие: %1';en='Getting the update file platform 1C: Enterprise: 1%'"),
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა პლატფორმა 1С:საწარმოს დისტრიბუტივის ფაილის მიღებისას (%1). %2';ru='Ошибка при получении файла дистрибутива платформы 1С:Предприятие (%1). %2';en='Error retrieving file distribution platform 1C: Enterprise (1%). 2%'"),
				Контекст.URLФайлаОбновленияПлатформы,
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შეცდომა დისტრიბუტივის ფაილის მიღებისას: %1';ru='Ошибка при получении файла дистрибутива. %1';en='Error retrieving file distribution. %one'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Файл обновления платформы 1С:Предприятие успешно получен: %1';ru='Файл обновления платформы 1С:Предприятие успешно получен: %1';en='Update file platform 1C: Enterprise successfully received% 1'"),
			ПутьПолученногоФайла));
	...
	Попытка
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Извлечение дистрибутива платформы 1С:Предприятие в %1';ru='Извлечение дистрибутива платформы 1С:Предприятие в %1';en='Removing a distribution platform 1C: Enterprise in 1%'"),
				КаталогДистрибутива));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა არქივის ფაილების (%1) ამოღებისას კატალოგ %2-ში.';ru='Ошибка при извлечении файлов архива (%1) в каталог %2.';en='An error occurred while extracting the archive file (% 1) to the% 2.'"),
				ПутьПолученногоФайла,
				КаталогДистрибутива)
			+ Символы.ПС
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დისტრიბუტივის ფაილების ამოღება ვერ მოხერხდა. %1';ru='Не удалось извлечь файлы дистрибутива. %1';en='Failed to remove the distribution files. %one'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Дистрибутив платформы 1С:Предприятие успешно сохранен в %1';ru='Дистрибутив платформы 1С:Предприятие успешно сохранен в %1';en='Distribution platform 1C: Enterprise is successfully stored in the% 1'"),
			КаталогДистрибутива));
...
Процедура УстановитьОбновлениеПлатформы(Контекст, Параметры, УстановкаОтменена)
	...
	Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПротокола) Тогда
		...
		Исключение
			ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='შეცდომა პროტოკოლის ფაილის წაშლისას (%1). %2';ru='Ошибка при удалении файла протокола (%1). %2';en='The error log file when you remove (1%). 2%'"),
					ПутьФайлаПротокола,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		КонецПопытки;
	КонецЕсли;
	...
	Если Не ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПрограммыУстановки) Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe (%1).';ru='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe (%1).';en='Loaded incorrect distribution platform 1C: Enterprise. Missing file setup.exe (% 1).'"),
			ПутьФайлаПрограммыУстановки);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		
		Контекст.ИмяОшибки = "НекорректныйДистрибутивПлатформы";
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = НСтр("ka='ჩატვირთულია პლატფორმა 1С:საწარმოს არაკორექტული დისტრიბუტივი. ფაილი setup.exe არ არის.';ru='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe.';en='Loaded incorrect distribution platform 1C: Enterprise. Missing file setup.exe.'");
		Возврат;
	...
	Попытка
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='პლატფორმა 1С:საწარმოს ახალი ვერსიის დაყენება (%1). %2';ru='Установка новой версии платформы 1С:Предприятие (%1). %2';en='Installing a new version of the platform 1C: Enterprise (1%). 2%'"),
				Контекст.ВерсияПлатформы,
				КомандаЗапуска));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა პლატფორმა 1С:საწარმოს დაყენების პროგრამის გაშვებისას (%1). %2';ru='Ошибка при запуске программы установки платформы 1С:Предприятие (%1). %2';en='Error when you start the program installation platform 1C: Enterprise (1%). 2%'"),
				КомандаЗапуска,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შეცდომა პლატფორმა 1С:საწარმოს დაყენების პროგრამის გაშვებისას. %1';ru='Ошибка при запуске программы установки платформы 1С:Предприятие. %1';en='Error when you run the installation program 1C: Enterprise. %one'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	...
	Если КодВозврата = 0 Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ka='Новая версия платформы 1С:Предприятие успешно установлена.';ru='Новая версия платформы 1С:Предприятие успешно установлена.';en='The new version of the platform 1C: Enterprise has been successfully installed.'"));
...
"Версия %1;"
"Код возврата: %2;"
"Команда: %3';ka='პლატფორმა 1С:საწარმოს დაყენება დასრულებულია შეცდომით."
"ვერსია %1;"
"დაბრუნების კოდი: %2;"
"ბრძანება: %3'"),
					Контекст.ВерсияПлатформы,
			...
			ШаблонСообщенияПользователю =
				НСтр("ka='<body>პლატფორმა 1С:საწარმოს ახალი ვერსიის დაყენებისას შეცდომა მოხდა."
"<br />დაბრუნების კოდი: %1.';ru='<body>При установке новой версии платформы 1С:Предприятие произошла ошибка."
"<br />Код возврата: %1.'");
			
			Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПротокола) Тогда
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
					+ НСтр("ka='<br />ტექნიკური ინფორმაცია იმყოფება <a href=""open:debuglog"">დაყენების პროტოკოლში</a>.';ru='<br />Техническая информация содержится в <a href=""open:debuglog"">протоколе установки</a>.';en='<br /> Technical information contained in <a href=""open:debuglog""> protocol setting </a>.'");
			КонецЕсли;
			...
			Если Параметры.РежимУстановкиПлатформы = 0
				...
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
					+ НСтр("ka='<br /><br /><p>მოცემული შეცდომა უკავშირდება უსაფრთხოების სისტემური პოლიტიკების შეზღუდვებს."
"<br />რეკომენდებულია ხელის გამართვებიანი პლატფორმის დაყენება.</p>';ru='<br /><br /><p>Данная ошибка связана с ограничениями системных политик безопасности."
"<br />Рекомендуется установить платформу с ручными настройками.</p>'");
			КонецЕсли;
			
			ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
				+ НСтр("ka='<br /><br />პრობლემების წარმოქმნის შემთხვევაში მიწერეთ <a href=""mailto:webits-info@1c.ru"">ტექმხარდაჭერას</a>.</body>';ru='<br /><br />При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';en='<br /> <br /> <a href=""mailto:webits-info@1c.ru""> write to technical support </a> If you have problems. </ body>'");
...
Процедура ЗагрузитьОбновлениеКонфигурации(Обновление, Контекст, Параметры)
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка файла обновления конфигурации %1';en='Loading configuration file updates 1%'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Файл обновления успешно загружен.';en='The update file is successfully loaded.'"));
