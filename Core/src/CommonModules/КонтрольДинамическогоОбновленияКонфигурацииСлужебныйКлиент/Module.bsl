...
Процедура ПриВыполненииСтандартныхПериодическихПроверок(Параметры, ОбработкаПродолжения) Экспорт
	...
	ТекстСообщения =
		НСтр("ka='პროგრამის ვერსია განახლებულია (საინფორმაციო ბაზის კონფიგურაციაში შეტანილია ცვლილებები). შემდგომი მუშაობისთვის რეკომენდებულია პროგრამის ხელახლა გაშვება. მოვახდინოთ გაშვება?';ru='Версия программы обновлена (внесены изменения в конфигурацию информационной базы)."
