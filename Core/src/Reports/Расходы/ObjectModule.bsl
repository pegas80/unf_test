...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["Расходы"].Теги = НСтр("ka='Главное,Расходы,Траты,Хозяйственная операция';ru='Главное,Расходы,Траты,Хозяйственная операция'");
...
Процедура ОбработатьРезультат(ТабличныйДокумент)
	...
	Для НомерСтроки = 1 По ТабличныйДокумент.ВысотаТаблицы Цикл
		Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
			Если ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = НСтр("ka='სულ';ru='Итого'") Тогда
				ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = "";
			КонецЕсли;
			Если ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = НСтр("ka='Сумма расходов.% в группировке';ru='Сумма расходов.% в группировке'") Тогда
				ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = "%";
	...
	Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		Если ТабличныйДокумент.Область(ТабличныйДокумент.ВысотаТаблицы, НомерКолонки).Текст = НСтр("ka='სულ';ru='Итого'") Тогда
