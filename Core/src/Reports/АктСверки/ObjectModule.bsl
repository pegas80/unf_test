...
Функция Сформировать(Период, Контрагент, Договор)
	...
	Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(КонецПериода) Тогда
		ОбластьЗаголовок.Параметры.Период = НСтр("ka='весь период работы';ru='весь период работы';en='the entire work period'");
	Иначе
	...
	Если ЗначениеЗаполнено(Период.ДатаНачала) И ВыборкаИтоги.Количество()>0 И НЕ ВыборкаИтоги.СуммаНачальныйОстаток=0 Тогда
		...
		НСтр("ka='Наш долг перед контрагентом на %1';ru='Наш долг перед контрагентом на %1';en='We owe it to the counterparty on the 1%'"), НСтр("ka='Долг контрагента перед нами на %1';ru='Долг контрагента перед нами на %1';en='Debt counterparty before us at 1%'"));
	...
	Если ВыборкаИтоги.Количество()>0 Тогда
		...
		Если ВыборкаИтоги.СуммаКонечныйОстаток=0 Тогда
			ШаблонДолг = НСтр("ka='На %1 долги и авансы отсутствуют';ru='На %1 долги и авансы отсутствуют';en='% 1 no debts and advances'");
