...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Стр.Заголовок = НСтр("ka='რეჟიმი';ru='Режим';en='Mode'");
...
Процедура УстановитьТегиВариантов(НастройкиВариантов)
	...
	НастройкиВариантов["АктСверкиКонтекст"].Теги = НСТР("ka='Покупатели,Контрагенты';ru='Покупатели,Контрагенты';en='Customers Accounts'");
...
Функция Сформировать(Период, Контрагент, Договор)
	...
	Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(КонецПериода) Тогда
		//giga перевод
		ОбластьЗаголовок.Параметры.Период = НСтр("ka='მუშაობის სრული პერიოდ';ru='весь период работы';en='the entire work period'");
	Иначе
		//giga перевод
	...
	Если ЗначениеЗаполнено(Период.ДатаНачала) И ВыборкаИтоги.Количество()>0 И НЕ ВыборкаИтоги.СуммаНачальныйОстаток=0 Тогда
		...
		НСтр("ka='ჩვენი დავალიანება კონტრაგენტისადმი %1 სთვის';ru='Наш долг перед контрагентом на %1';en='We owe it to the counterparty on the 1%'"), НСтр("ka='კონტრაგენტის დავალიანება ჩვენსადმი %1 სთვის';ru='Долг контрагента перед нами на %1';en='Debt counterparty before us at 1%'"));
	...
	Если ВыборкаИтоги.Количество()>0 Тогда
		ПредставлениеПериода = ?(ЗначениеЗаполнено(КонецПериода), Формат(КонецПериода, "ДЛФ=D"), НСтр("ka='მიმდინარე მომენტი';ru='текущий момент';en='this moment'"));
		Если ВыборкаИтоги.СуммаКонечныйОстаток=0 Тогда
			//giga перевод
			ШаблонДолг = НСтр("ka='%1 სთვის ვალები და ავანსები არ არის';ru='На %1 долги и авансы отсутствуют';en='% 1 no debts and advances'");
	...
	//giga
	БуферДолгов = 0;
	СкрыватьПромежуточныйДолг = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ОтчетАктСверкиСкрытьПромежуточныйДолг");
	Результат.Область("C4").Видимость = Не СкрыватьПромежуточныйДолг;
	Результат.Область("C7").Видимость = Не СкрыватьПромежуточныйДолг;
	//giga
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		ОбластьСтрока.Параметры.ЭквПриход = ?(ЗначениеЗаполнено(СуммаПриход) И ВыборкаДокументы.Эквивалент, НСтр("ka='(экв)';ru='(экв)';en='(Eq)'"), "");
		ОбластьСтрока.Параметры.ЭквРасход = ?(ЗначениеЗаполнено(СуммаРасход) И ВыборкаДокументы.Эквивалент, НСтр("ka='(экв)';ru='(экв)';en='(Eq)'"), "");
		...
		//giga
		Если Не СкрыватьПромежуточныйДолг Тогда
			ОбластьСтрока.Параметры.НачальныйДолг = БуферДолгов;
			БуферДолгов = БуферДолгов + СуммаПриход - СуммаРасход;
			ОбластьСтрока.Параметры.КонечныйДолг = БуферДолгов;
		КонецЕсли;
