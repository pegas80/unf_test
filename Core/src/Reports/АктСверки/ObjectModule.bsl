...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Стр.Заголовок = НСтр("ka='რეჟიმი';ru='Режим'");
...
Процедура УстановитьТегиВариантов(НастройкиВариантов)
	...
	НастройкиВариантов["АктСверкиКонтекст"].Теги = НСТР("ka='Покупатели,Контрагенты';ru='Покупатели,Контрагенты'");
...
Функция Сформировать(Период, Контрагент, Договор)
	...
	Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(КонецПериода) Тогда
		ОбластьЗаголовок.Параметры.Период = НСтр("ka='весь период работы';ru='весь период работы'");
	Иначе
	...
	Если ЗначениеЗаполнено(Период.ДатаНачала) И ВыборкаИтоги.Количество()>0 И НЕ ВыборкаИтоги.СуммаНачальныйОстаток=0 Тогда
		...
		НСтр("ka='Наш долг перед контрагентом на %1';ru='Наш долг перед контрагентом на %1'"), НСтр("ka='Долг контрагента перед нами на %1';ru='Долг контрагента перед нами на %1'"));
	...
	Если ВыборкаИтоги.Количество()>0 Тогда
		ПредставлениеПериода = ?(ЗначениеЗаполнено(КонецПериода), Формат(КонецПериода, "ДЛФ=D"), НСтр("ka='მიმდინარე მომენტი';ru='текущий момент'"));
		Если ВыборкаИтоги.СуммаКонечныйОстаток=0 Тогда
			ШаблонДолг = НСтр("ka='На %1 долги и авансы отсутствуют';ru='На %1 долги и авансы отсутствуют'");
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		ОбластьСтрока.Параметры.ЭквПриход = ?(ЗначениеЗаполнено(СуммаПриход) И ВыборкаДокументы.Эквивалент, НСтр("ka='(экв)';ru='(экв)'"), "");
		ОбластьСтрока.Параметры.ЭквРасход = ?(ЗначениеЗаполнено(СуммаРасход) И ВыборкаДокументы.Эквивалент, НСтр("ka='(экв)';ru='(экв)'"), "");
