...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	НастройкиОтчета.Описание = НСтр("ka='Внешние ресурсы, используемые программой и дополнительными модулями';ru='Внешние ресурсы, используемые программой и дополнительными модулями';en='External resources used by the program and additional modules'");
	НастройкиОтчета.ОпределитьНастройкиФормы = Истина;
	НастройкиОтчета.НастройкиДляПоиска.НаименованияПолей = 
		НСтр("ka='Имя и идентификатор COM-класса"
"Имя компьютера"
"Адрес"
"Чтение данных"
"Запись данных"
...
"Порт';ru='Имя и идентификатор COM-класса"
"Имя компьютера"
"Адрес"
"Чтение данных"
"Запись данных"
...
Функция СловарьМодуляКонфигурации() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ka='პროგრამა';ru='Программа';en='Program'"));
	Результат.Вставить("Родительный", НСтр("ka='პროგრამები';ru='Программы';en='Programs'"));
...
Функция ПредставлениеЗапросовРазрешенийНаИспользованиеВнешнихРесурсов(Знач ОперацииАдминистрирования, Знач ОписаниеДобавляемыхРазрешений, Знач ОписаниеУдаляемыхРазрешений, Знач КакТребуемые = Ложь) Экспорт
	...
	Для Каждого СтрокаТаблицыМодулей Из ТаблицаМодулей Цикл
		...
		Если Добавляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				...
				ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Для %1 ""%2"" требуется использование следующих внешних ресурсов:';ru='Для %1 ""%2"" требуется использование следующих внешних ресурсов:';en='% 1 ""%2"" requires the use of the following external resources:'"),
			...
			Иначе
				...
				ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Для %1 ""%2"" будут предоставлены следующие разрешения на использование внешних ресурсов:';ru='Для %1 ""%2"" будут предоставлены следующие разрешения на использование внешних ресурсов:';en='% 1 ""%2"" will be granted the following permissions for the use of external resources:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
		...
		Если Удаляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				ВызватьИсключение НСтр("ka='Некорректный запрос разрешений';ru='Некорректный запрос разрешений';en='Invalid request permission'");
			КонецЕсли;
			
			ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Будут удалены следующие ранее предоставленные для %1 ""%2"" разрешения на использование внешних ресурсов:';ru='Будут удалены следующие ранее предоставленные для %1 ""%2"" разрешения на использование внешних ресурсов:';en='following previously granted for% 1 ""%2"" permission for the use of external resources will be deleted:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
...
Процедура СформироватьПредставлениеОпераций(ТабличныйДокумент, Знач Макет, Знач ОперацииАдминистрирования)
	...
	Для Каждого Описание Из ОперацииАдминистрирования Цикл
		...
		Если Описание.Операция = Перечисления.ОперацииАдминистрированияПрофилейБезопасности.Удаление Тогда
			...
			ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Будет удален профиль безопасности для %1 ""%2"".';ru='Будет удален профиль безопасности для %1 ""%2"".';en='It will be removed for the safety profile of% 1 ""%2"".'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
					...
					Иначе
						...
						Если ТипРазрешения = "FileSystemAccess" Тогда
							...
							Если Разрешение.Path = "/temp" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("ka='Каталог временных файлов';ru='Каталог временных файлов';en='Temporary directory'"));
							КонецЕсли;
							
							Если Разрешение.Path = "/bin" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("ka='Каталог, в который установлен сервер 1С:Предприятия';ru='Каталог, в который установлен сервер 1С:Предприятия';en='The directory in which the server is installed 1C: Enterprise'"));
							КонецЕсли;
