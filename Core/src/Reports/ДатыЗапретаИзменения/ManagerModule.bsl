...
Функция ПервыйВариант()
	...
	СвойстваВарианта.Вставить("Заголовок",
		НСтр("ka='Даты запрета изменения данных по пользователям';ru='Даты запрета изменения данных по пользователям'"));
	
	СвойстваВарианта.Вставить("Описание",
		НСтр("ka='Выводит даты запрета изменения, сгруппированные по пользователям.';ru='Выводит даты запрета изменения, сгруппированные по пользователям.'"));
...
Функция ВторойВариант()
	...
	Если Свойства.ПоказыватьРазделы И НЕ Свойства.ВсеРазделыБезОбъектов Тогда
		...
		Заголовок = НСтр("ka='Даты запрета изменения данных по разделам и объектам';ru='Даты запрета изменения данных по разделам и объектам'");
		ОписаниеВарианта =
			НСтр("ka='Выводит даты запрета изменения, сгруппированные по разделам с объектами.';ru='Выводит даты запрета изменения, сгруппированные по разделам с объектами.'");
		
	ИначеЕсли Свойства.ВсеРазделыБезОбъектов Тогда
		ИмяВарианта = "ДатыЗапретаИзмененияПоРазделамДляПользователей";
		Заголовок = НСтр("ka='Даты запрета изменения данных по разделам';ru='Даты запрета изменения данных по разделам'");
		ОписаниеВарианта =
			НСтр("ka='Выводит даты запрета изменения, сгруппированные по разделам.';ru='Выводит даты запрета изменения, сгруппированные по разделам.'");
	Иначе
		ИмяВарианта = "ДатыЗапретаИзмененияПоОбъектамДляПользователей";
		Заголовок = НСтр("ka='Даты запрета изменения данных по объектам';ru='Даты запрета изменения данных по объектам'");
		ОписаниеВарианта =
			НСтр("ka='Выводит даты запрета изменения, сгруппированные по объектам.';ru='Выводит даты запрета изменения, сгруппированные по объектам.'");
	КонецЕсли;
