	...
	Если ТипЗнч(НастройкиОтчета.РазрешеноМенятьВарианты) <> Тип("Булево") Тогда
		...
		Если НастройкиОтчета.РазрешеноМенятьВарианты // Команды не отключены параметрами открытия формы.
			...
			Если Отключенные.Найти(НастройкиОтчета.ПредопределенныйСсылка) <> Неопределено Тогда // Вариант отключен.
				Текст = НСтр("ka='ანგარიშგება ""%2""-ის კონტექსტური ვარიანტი ""%1"" გახსნილია კონტექსტის გარეშე.';ru='Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.'");
...
Процедура РедактироватьСхему(Команда)
	...
	#Иначе
		...
		ПоказатьПредупреждение(,(НСтр("ka='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.';ru='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.'")));
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების ვარიანტი ვერ მოიძებნა.';ru='Вариант отчета не найден.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ka='ანგარიშგება არ არის შედგენილი';ru='Отчет не сформирован'"), , Заголовок);
		КонецЕсли;
...
Процедура ПередФормированиемНаКлиенте()
	...
	Если ПеременныеКлиента.ВыполнятьЗамеры Тогда
		...
		Комментарий = НастройкиОтчета.ПрефиксЗамеров + "; " + НСтр("ka='უშუალოდ:';ru='Непосредственно:'") + " " + Строка(ПеременныеКлиента.Непосредственно);
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1""-ის გაგზავნა ფოსტით';ru='Отправка отчета ""%1"" по почте'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	КонецЕсли;
	Если РежимРасшифровки Тогда
		Заголовок = Заголовок + " (" + НСтр("ka='გაშიფრვა';ru='Расшифровка'") + ")";
	КонецЕсли;
КонецПроцедуры
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Иначе
		...
		ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='ანგარიშგების ვარიანტები: ანგარიშგების ჩამოყალიბება';ru='Варианты отчетов: Формирование отчета'");
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтображениеСостояния.Текст     = НСтр("ka='არჩეულია ანგარიშგების სხვა ვარიანტი. ანგარიშგების მისაღებად დააჭირეთ ""ჩამოყალიბება""-ს.';ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.'");
КонецПроцедуры
...
Процедура ВывестиЭлементыНастройки(Форма, Элементы, СвойстваНастройки, ГруппаВывода, Прочее) Экспорт
	...
	Если ВыводитьУсловие Тогда
		...
		КомандаВидСравнения.Заголовок   = НСтр("ka='შერჩევის პირობის წაშლა...';ru='Изменить условие отбора...'");
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СтандартныйПериод" Тогда
			...
			КомандаВыбора.Заголовок   = НСтр("ka='პერიოდის არჩევა...';ru='Выбрать период...'");
			...
			Если Не СвойстваНастройки.ВыводитьФлажок Или Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
				...
				Если Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
					Если ИнформацияОТипах.СодержитСсылочныеТипы Тогда
						...
						КомандаПодбор.Заголовок = НСтр("ka='შერჩევა';ru='Подбор'");
					Иначе
						КомандаПодбор = Форма.Команды.Добавить(КнопкаПодборИмя);
						КомандаПодбор.Действие  = "Подключаемый_СписокСПодбором_Добавить";
						КомандаПодбор.Заголовок = НСтр("ka='დამატება';ru='Добавить'");
					...
					Если СвойстваНастройки.ИнформацияОТипах.СодержитСсылочныеТипы И Прочее.ЕстьЗагрузкаДанныхИзФайла Тогда
						...
						КомандаВставить.Заголовок = НСтр("ka='გაცვლის ბუფერიდან ჩასმა...';ru='Вставить из буфера обмена...'");
...
Процедура ПослеИзмененияКлючевыхСостояний(ПараметрыЗаполнения)
	Если ПараметрыЗаполнения.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыЗаполнения.ВариантМодифицирован
			...
			ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
			ОтображениеСостояния.Видимость = Истина;
			ОтображениеСостояния.Текст     = НСтр("ka='გამართვები შეიცვალა. ანგარიშგების მისაღებად დააჭირეთ ""შედგენა""-ს.';ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.'");
