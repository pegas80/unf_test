	...
	Если ТипЗнч(НастройкиОтчета.РазрешеноМенятьВарианты) <> Тип("Булево") Тогда
		...
		Если НастройкиОтчета.РазрешеноМенятьВарианты // Команды не отключены параметрами открытия формы.
			...
			Если Отключенные.Найти(НастройкиОтчета.ПредопределенныйСсылка) <> Неопределено Тогда // Вариант отключен.
				Текст = НСтр("ka='ანგარიშგება ""%2""-ის კონტექსტური ვარიანტი ""%1"" გახსნილია კონტექსტის გარეშე.';ru='Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.';en='Context option ""%1"" report ""%2"" opened out of context.'");
...
Процедура РедактироватьСхему(Команда)
	...
	#Иначе
		...
		ПоказатьПредупреждение(,(НСтр("ka='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.';ru='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.';en='To edit the layout of the scheme, it is necessary to run the application in a thick client mode.'")));
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების ვარიანტი ვერ მოიძებნა.';ru='Вариант отчета не найден.';en='Report option is not found.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ka='ანგარიშგება არ არის შედგენილი';ru='Отчет не сформирован';en='The report has not been established'"), , Заголовок);
		КонецЕсли;
...
Процедура ПередФормированиемНаКлиенте()
	...
	Если ПеременныеКлиента.ВыполнятьЗамеры Тогда
		...
		Комментарий = НастройкиОтчета.ПрефиксЗамеров + "; " + НСтр("ka='უშუალოდ:';ru='Непосредственно:';en='Directly:'") + " " + Строка(ПеременныеКлиента.Непосредственно);
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1""-ის გაგზავნა ფოსტით';ru='Отправка отчета ""%1"" по почте';en='Submitting the report ""%1"" in the mail'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	КонецЕсли;
	Если РежимРасшифровки Тогда
		Заголовок = Заголовок + " (" + НСтр("ka='გაშიფრვა';ru='Расшифровка';en='transcript'") + ")";
	КонецЕсли;
КонецПроцедуры
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Иначе
		...
		ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='ანგარიშგების ვარიანტები: ანგარიშგების ჩამოყალიბება';ru='Варианты отчетов: Формирование отчета';en='reporting options: Report Generation'");
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтображениеСостояния.Текст     = НСтр("ka='არჩეულია ანგარიშგების სხვა ვარიანტი. ანგარიშგების მისაღებად დააჭირეთ ""ჩამოყალიბება""-ს.';ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';en='Select another version of the report. Click ""Create"" to obtain a report.'");
КонецПроцедуры
...
Процедура ВывестиЭлементыНастройки(Форма, Элементы, СвойстваНастройки, ГруппаВывода, Прочее) Экспорт
	...
	Если ВыводитьУсловие Тогда
		...
		КомандаВидСравнения.Заголовок   = НСтр("ka='შერჩევის პირობის წაშლა...';ru='Изменить условие отбора...';en='Change the selection condition ...'");
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СтандартныйПериод" Тогда
			...
			КомандаВыбора.Заголовок   = НСтр("ka='პერიოდის არჩევა...';ru='Выбрать период...';en='Choose period ...'");
			...
			Если Не СвойстваНастройки.ВыводитьФлажок Или Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
				...
				Если Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
					Если ИнформацияОТипах.СодержитСсылочныеТипы Тогда
						...
						КомандаПодбор.Заголовок = НСтр("ka='შერჩევა';ru='Подбор';en='selection'");
					Иначе
						КомандаПодбор = Форма.Команды.Добавить(КнопкаПодборИмя);
						КомандаПодбор.Действие  = "Подключаемый_СписокСПодбором_Добавить";
						КомандаПодбор.Заголовок = НСтр("ka='დამატება';ru='Добавить';en='Add'");
					...
					Если СвойстваНастройки.ИнформацияОТипах.СодержитСсылочныеТипы И Прочее.ЕстьЗагрузкаДанныхИзФайла Тогда
						...
						КомандаВставить.Заголовок = НСтр("ka='გაცვლის ბუფერიდან ჩასმა...';ru='Вставить из буфера обмена...';en='Paste from Clipboard ...'");
...
Процедура ПослеИзмененияКлючевыхСостояний(ПараметрыЗаполнения)
	Если ПараметрыЗаполнения.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыЗаполнения.ВариантМодифицирован
			...
			ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
			ОтображениеСостояния.Видимость = Истина;
			ОтображениеСостояния.Текст     = НСтр("ka='გამართვები შეიცვალა. ანგარიშგების მისაღებად დააჭირეთ ""შედგენა""-ს.';ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';en='Change settings. Click ""Create"" to obtain a report.'");
