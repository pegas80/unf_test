...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//	"ФормаОтчета",
	//	НСтр("ru='Новости: Обороты счета';en='News: Turnover accounts'"),
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ანგარიშის ბრუნვა %1%2';ru='Обороты счета %1%2';en='Turnover account% 1% 2'"),
		Отчет.Счет,
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка", ЗаполнятьКоллекцию) И ЗаполнятьКоллекцию Тогда
		...
		Для Индекс = 1 По КоличествоСубконто Цикл
			...
			Если ДанныеСчета.Валютный Тогда
				...
				НоваяСтрока.Использование = Использование;
				НоваяСтрока.Представление = НСтр("ka='ვალუტა';ru='Валюта';en='Currency'");
				НоваяСтрока.ТипГруппировки = 0;
			КонецЕсли;
