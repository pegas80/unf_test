...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	ТекстЗаголовкаОтчета = НСтр("ka='ბრუნვითი უწყისი ანგარიშის მიხედვით %1 %2';ru='Оборотно-сальдовая ведомость по счету %1 %2'");
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Если ПараметрыОтчета.РазвернутоеСальдо Тогда
		...
		НовыйЭлемент.Значение = НСтр("ka='შედეგი ჩაშლილი';ru='Итого развернутое'");
...
Процедура НастроитьСтруктуруОтчета(КомпоновщикНастроек, ПараметрыОтчета)
	...
	Для каждого ЭлементОформления Из КомпоновщикНастроек.Настройки.УсловноеОформление.Элементы Цикл
		Если ЭлементОформления.Представление = НСтр("ka='ავტოდაშორების შემცირება';ru='Уменьшенный автоотступ'") Тогда
	...
	Если УсловноеОформлениеАвтоотступа = Неопределено Тогда
		...
		УсловноеОформлениеАвтоотступа.Представление = НСтр("ka='ავტოდაშორების შემცირება';ru='Уменьшенный автоотступ'");
