...
Функция ОпределитьВидРегистраСиноним(МетаданныеРегистра)
	...
	Если Метаданные.РегистрыНакопления.Индекс(МетаданныеРегистра) >= 0 Тогда
		Возврат НСтр("ka='დაგროვებითი';ru='Накопления'");
		
	ИначеЕсли Метаданные.РегистрыСведений.Индекс(МетаданныеРегистра) >= 0 Тогда
		Возврат НСтр("ka='ინფორმაციული';ru='Сведений'");	
		
	ИначеЕсли Метаданные.РегистрыБухгалтерии.Индекс(МетаданныеРегистра) >= 0 Тогда
		Возврат НСтр("ka='ბუღალტრული';ru='Бухгалтерии'");
...
Процедура ВывестиЖурналПроводок()
	...
	ОбластьЗаголовок.Параметры.СинонимРегистра = НСТР("ka='ბუღალტრული რეგისტრი ""მმართველობითი""';ru='Регистр бухгалтерии ""Управленческий""';en='Accounting register ""Management""'");
	ТабличныйДокумент.Вывести(ОбластьЗаголовок);
	ТабличныйДокумент.НачатьГруппуСтрок();
...
Процедура ВывестиЖурналПроводокРегл()
	...
	ОбластьЗаголовок.Параметры.СинонимРегистра = НСтр("ka='ბუღალტრული რეგისტრი ""გატარებების ჟურნალი (ბუღალტრული და საგადასახადო აღრიცხვა)""';ru='Регистр бухгалтерии ""Хозрасчетный""'");
	ТабличныйДокумент.Вывести(ОбластьЗаголовок);
	ТабличныйДокумент.НачатьГруппуСтрок();
...
Процедура СформироватьОтчет()
    ...
    Если НЕ ЗначениеЗаполнено(Отчет.Документ) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не выбран документ!';ru='Не выбран документ!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
