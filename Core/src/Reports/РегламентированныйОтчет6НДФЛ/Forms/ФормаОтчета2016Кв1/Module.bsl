	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если Область.Имя = "ОКТМО" Тогда
			...
			Если ЗначениеЗаполнено(ОКТМО) И НЕ (СтрДлина(ОКТМО) = 8 ИЛИ СтрДлина(ОКТМО) = 11) Тогда
				Область.Очистить();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Длина кода по ОКТМО должна быть равна 8 или 11. Значение кода по ОКТМО очищено!';ru='Длина кода по ОКТМО должна быть равна 8 или 11. Значение кода по ОКТМО очищено!'");
				Сообщение.Сообщить();
			Иначе
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не задан код налогового органа, в который представляется расчет.';ru='Не задан код налогового органа, в который представляется расчет.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(ОКТМО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не задан код по ОКТМО.';ru='Не задан код по ОКТМО.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ОКТМО) <> 11 И СтрДлина(ОКТМО) <> 8 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Задан некорректный код по ОКТМО.';ru='Задан некорректный код по ОКТМО.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.1)';ru='Раздел 1 (КС 1.1)'");
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		ОписаниеКС = НСтр("ka='Дата представления Расчета должна быть не позднее 1 апреля года, следующего за истекшим налоговым периодом';ru='Дата представления Расчета должна быть не позднее 1 апреля года, следующего за истекшим налоговым периодом'");
	Иначе
		ОписаниеКС = НСтр("ka='Дата представления Расчета должна быть не позднее последнего дня месяца, следующего за отчетным периодом';ru='Дата представления Расчета должна быть не позднее последнего дня месяца, следующего за отчетным периодом'");
	КонецЕсли;
	Нарушение = НСтр("ka='п. 1.2 ст. 126, п. 2 ст. 230 НК РФ';ru='п. 1.2 ст. 126, п. 2 ст. 230 НК РФ'");
	ОписаниеОшибки  = НСтр("ka='Возможно непредставление в установленный НК РФ срок Расчета';ru='Возможно непредставление в установленный НК РФ срок Расчета'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.2)';ru='Раздел 1 (КС 1.2)'");
	ОписаниеКС      = НСтр("ka='Сумма начисленного дохода (строка 020 Раздела 1) должна быть больше или равна сумме налоговых вычетов (строка 030 Раздела 1)';ru='Сумма начисленного дохода (строка 020 Раздела 1) должна быть больше или равна сумме налоговых вычетов (строка 030 Раздела 1)'");
	Нарушение = НСтр("ka='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';ru='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ'");
	ОписаниеОшибки  = НСтр("ka='Завышена сумма налоговых вычетов';ru='Завышена сумма налоговых вычетов'");
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.3)';ru='Раздел 1 (КС 1.3)'");
	...
	Нарушение = НСтр("ka='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';ru='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Завышена сумма исчисленного налога';ru='Завышена сумма исчисленного налога'");
	ОписаниеОшибки2 = НСтр("ka='Занижена сумма исчисленного налога';ru='Занижена сумма исчисленного налога'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.4)';ru='Раздел 1 (КС 1.4)'");
	ОписаниеКС      = НСтр("ka='Сумма исчисленного налога (строка 040 Раздела 1) должна быть больше или равна сумме фиксированного авансового платежа (строка 050 Раздел 1)';ru='Сумма исчисленного налога (строка 040 Раздела 1) должна быть больше или равна сумме фиксированного авансового платежа (строка 050 Раздел 1)'");
	Нарушение = НСтр("ka='ст. 126.1, ст. 227.1. ст. 23 НК РФ, ст. 24 НК РФ';ru='ст. 126.1, ст. 227.1. ст. 23 НК РФ, ст. 24 НК РФ'");
	ОписаниеОшибки  = НСтр("ka='Завышена сумма фиксированного авансового платежа';ru='Завышена сумма фиксированного авансового платежа'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "ТитульныйДатаПодписи" Тогда
		...
		Результат.Вставить("Представление", Нстр("ka='ხელმოწერის თარიღი';ru='Дата подписи'"));
...
Функция ПредставлениеПоказателя(ИмяПоказателя)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='Графа %1 строка %2 %3';ru='Графа %1 строка %2 %3'"), ПодстрокаГрафы,
