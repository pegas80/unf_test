...
Процедура ВыгрузитьОтчет() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.Организация) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Организация не выбрана! Выгрузка невозможна.';ru='Организация не выбрана! Выгрузка невозможна.';en='The organization is not selected! Unloading impossible.'");
		Сообщение.Сообщить();
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если Область.Имя = "ОКТМО" Тогда
			...
			Если ЗначениеЗаполнено(ОКТМО) И НЕ (СтрДлина(ОКТМО) = 8 ИЛИ СтрДлина(ОКТМО) = 11) Тогда
				Область.Очистить();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Длина кода по ОКТМО должна быть равна 8 или 11. Значение кода по ОКТМО очищено!';ru='Длина кода по ОКТМО должна быть равна 8 или 11. Значение кода по ОКТМО очищено!';en='OKTMO length of code must be equal to 8 or 11. The value of the code OKTMO cleaned!'");
				Сообщение.Сообщить();
			Иначе
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		Если НЕ ЗначениеЗаполнено(КПП) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Для заполнения отчета необходимо значение КПП налогового агента. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").';ru='Для заполнения отчета необходимо значение КПП налогового агента. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").';en='To fill the need to report the value of PPC tax agent. Fixes in the business directory and update the information in the calculation (the menu ""More"" - ""Update"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КПП) <> 9 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Задан КПП налогового агента, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").';ru='Задан КПП налогового агента, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").';en='Transmission set tax agent not composed of 9 symbols. Fixes in the business directory and update the information in the calculation (the menu ""More"" - ""Update"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не задан код налогового органа, в который представляется расчет.';ru='Не задан код налогового органа, в который представляется расчет.';en='Do not set the tax code of the body, which appears to account.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(ОКТМО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не задан код по ОКТМО.';ru='Не задан код по ОКТМО.';en='Do not set the code for OKTMO.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ОКТМО) <> 11 И СтрДлина(ОКТМО) <> 8 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Задан некорректный код по ОКТМО.';ru='Задан некорректный код по ОКТМО.';en='Given incorrect code for OKTMO.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для налогового агента по значениям%1 кода налогового органа и кода по ОКТМО не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение отчета прервано.';ru='Для налогового агента по значениям%1 кода налогового органа и кода по ОКТМО не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение отчета прервано.';en='For the fiscal agent for the values ​​of 1% of the tax authority code and code of OKTMO not found a corresponding entry in the ""Registration with the tax authority."" Filling the report is interrupted.'"),
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.1)';ru='Раздел 1 (КС 1.1)';en='Unit 1 (1.1 KS)'");
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		ОписаниеКС = НСтр("ka='Дата представления Расчета должна быть не позднее 1 апреля года, следующего за истекшим налоговым периодом';ru='Дата представления Расчета должна быть не позднее 1 апреля года, следующего за истекшим налоговым периодом';en='representation Settlement Date shall be no later than April 1 of the year following the tax period'");
	Иначе
		ОписаниеКС = НСтр("ka='Дата представления Расчета должна быть не позднее последнего дня месяца, следующего за отчетным периодом';ru='Дата представления Расчета должна быть не позднее последнего дня месяца, следующего за отчетным периодом';en='Date of the calculations should be not later than the last day of the month following the reporting period'");
	КонецЕсли;
	Нарушение = НСтр("ka='п. 1.2 ст. 126, п. 2 ст. 230 НК РФ';ru='п. 1.2 ст. 126, п. 2 ст. 230 НК РФ';en='p. 1.2 Art. 126, n. 2, Art. 230 of the Tax Code'");
	ОписаниеОшибки  = НСтр("ka='Возможно непредставление в установленный НК РФ срок Расчета';ru='Возможно непредставление в установленный НК РФ срок Расчета';en='Perhaps the failure to set a term of payment of the Tax Code'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.2)';ru='Раздел 1 (КС 1.2)';en='Unit 1 (1.2 KS)'");
	ОписаниеКС      = НСтр("ka='Сумма начисленного дохода (строка 020 Раздела 1) должна быть больше или равна сумме налоговых вычетов (строка 030 Раздела 1)';ru='Сумма начисленного дохода (строка 020 Раздела 1) должна быть больше или равна сумме налоговых вычетов (строка 030 Раздела 1)';en='The amount of accrued income (line 020 of Section 1) must be greater than or equal to the amount of tax deduction (line 030 of Section 1)'");
	Нарушение = НСтр("ka='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';ru='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';en='Art. 126.1 Art. 210, Art. 23 of the Tax Code, Art. 24 of the Tax Code'");
	ОписаниеОшибки  = НСтр("ka='Завышена сумма налоговых вычетов';ru='Завышена сумма налоговых вычетов';en='Overstated amount of tax deductions'");
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.3)';ru='Раздел 1 (КС 1.3)';en='Unit 1 (1.3 KS)'");
	ОписаниеКС      = НСтр("ka='Сумма исчисленного налога (строка 040 Раздела 1) должна быть равна разнице Суммы начисленного дохода (строка 020 Раздела 1) и Суммы налоговых вычетов (строка 030 Раздела 1) / 100 * Ставка налога (строка 010 Раздела 1), если Сумма начисленного дохода (строка 020 Раздела 1) больше или равна сумме налоговых вычетов (строка 030 Раздела 1)';ru='Сумма исчисленного налога (строка 040 Раздела 1) должна быть равна разнице Суммы начисленного дохода (строка 020 Раздела 1) и Суммы налоговых вычетов (строка 030 Раздела 1) / 100 * Ставка налога (строка 010 Раздела 1), если Сумма начисленного дохода (строка 020 Раздела 1) больше или равна сумме налоговых вычетов (строка 030 Раздела 1)';en='The amount of the calculated tax (line 040 of Section 1) must be equal to the difference between the sums of accrued income (line 020 of Section 1) and the amount of tax deduction (line 030 of Section 1) / 100 * tax rate (line 010 of Section 1), if the added amount of income (line 020 Section 1) is greater than or equal to the amount of tax deduction (line 030 of Section 1)'");
	Нарушение = НСтр("ka='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';ru='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';en='Art. 126.1 Art. 210, Art. 23 of the Tax Code, Art. 24 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Завышена сумма исчисленного налога';ru='Завышена сумма исчисленного налога';en='Overestimated the amount of calculated tax'");
	ОписаниеОшибки2 = НСтр("ka='Занижена сумма исчисленного налога';ru='Занижена сумма исчисленного налога';en='Understated the amount of calculated tax'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ka='Раздел 1 (КС 1.4)';ru='Раздел 1 (КС 1.4)';en='Unit 1 (1.4 KS)'");
	ОписаниеКС      = НСтр("ka='Сумма исчисленного налога (строка 040 Раздела 1) должна быть больше или равна сумме фиксированного авансового платежа (строка 050 Раздел 1)';ru='Сумма исчисленного налога (строка 040 Раздела 1) должна быть больше или равна сумме фиксированного авансового платежа (строка 050 Раздел 1)';en='The amount of the calculated tax (line 040 of Section 1) must be greater than or equal to the sum of a fixed down payment (line 050 Section 1)'");
	Нарушение = НСтр("ka='ст. 126.1, ст. 227.1. ст. 23 НК РФ, ст. 24 НК РФ';ru='ст. 126.1, ст. 227.1. ст. 23 НК РФ, ст. 24 НК РФ';en='Art. 126.1 Art. 227.1. Art. 23 of the Tax Code, Art. 24 of the Tax Code'");
	ОписаниеОшибки  = НСтр("ka='Завышена сумма фиксированного авансового платежа';ru='Завышена сумма фиксированного авансового платежа';en='Overestimated the amount of a fixed down payment'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "ТитульныйДатаПодписи" Тогда
		...
		Результат.Вставить("Представление", Нстр("ka='ხელმოწერის თარიღი';ru='Дата подписи';en='signature Date'"));
...
Функция ПредставлениеПоказателя(ИмяПоказателя)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='Графа %1 строка %2 %3';ru='Графа %1 строка %2 %3';en='Box% 1 line 2% 3%'"), ПодстрокаГрафы,
