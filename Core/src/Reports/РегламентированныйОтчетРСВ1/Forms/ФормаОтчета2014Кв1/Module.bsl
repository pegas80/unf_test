...
Процедура ВыгрузитьЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ПерсУчетВедетсяВоВнешнейПрограмме Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВнешниеСведенияРаздела25) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='Выгрузка невозможна!';ru='Выгрузка невозможна!'"));
	...
	Иначе
		...
		Если КоличествоЗагруженныхФайлов = 1 Тогда
			...
			Если Прав(ВыбИмяФайла, 4) <> ".pcb" Тогда
				...
				ПоказатьПредупреждение(, НСтр("ka='Необходимо выбрать файл описания пакета сведений персонифицированного учета с расширением ""pcb""!';ru='Необходимо выбрать файл описания пакета сведений персонифицированного учета с расширением ""pcb""!'"));
			...
			Иначе
				...
				НСтр("ka='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';ru='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.'"), ВыбИмяФайла);
				Сообщение.Сообщить();
...
Процедура ПомещениеФайловРаздела6(Результат, АдресФайла, ВыбИмяФайла, ДопПараметры) Экспорт
	...
	Если Не Результат Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Импорт файлов пачек прерван!';ru='Импорт файлов пачек прерван!'"));
	...
	Иначе
		...
		Если КоличествоЗагруженныхФайлов > 0 Тогда
			...
			Если МассивВыбИменФайлов.Найти(ВыбИмяФайла) <> Неопределено Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Файл пачки %1 ранее уже был импортирован!';ru='Файл пачки %1 ранее уже был импортирован!'"), ВыбИмяФайла);
				Сообщение.Сообщить();
			...
			Иначе
				...
				Если НЕ ВыбранФайлПачки Тогда
					...
					Если КоличествоЗагруженныхФайлов > 0 Тогда
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Файл %1 не предназначен для импорта!';ru='Файл %1 не предназначен для импорта!'"), ВыбИмяФайла);
						Сообщение.Сообщить();
				...
				Иначе
					...
					Исключение
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Не удалость импортировать файл пачки %1!';ru='Не удалость импортировать файл пачки %1!'"), ВыбИмяФайла);
						Сообщение.Сообщить();
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Файл пачки %1 успешно импортирован.';ru='Файл пачки %1 успешно импортирован.'"), ВыбИмяФайла);
					Сообщение.Сообщить();
...
Процедура ДиалогПриПодключенномРасширенииРаботыСФайлами()
	...
	Диалог.Фильтр = НСтр("ka='Файл описания пакета выгрузки сведений персонифицированного учета';ru='Файл описания пакета выгрузки сведений персонифицированного учета'") + "(*.pcb)|*.pcb";
...
Процедура ДиалогПриПодключенномРасширенииРаботыСФайламиПослеОтображенияДиалогаВыбораФайлаПослеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';ru='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.'"),
	...
	Для ИндексМассива = 0 По МассивИменФайлов.Количество() - 1 Цикл
		...
		Исключение
			...
			НСтр("ka='Не удалость импортировать файл пачки %1!';ru='Не удалость импортировать файл пачки %1!'"), ИмяФайлаПачки));
			Возврат;
		КонецПопытки;
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ka='Файлы пачек:';ru='Файлы пачек:'");
	Сообщение.Сообщить();
	...
	Для ИндексМассива = 0 По МассивИменФайлов.Количество() - 1 Цикл
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1';ru='%1'"), ИмяФайлаПачки);
		Сообщение.Сообщить();
		
	КонецЦикла;
	
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ka='успешно импортированы.';ru='успешно импортированы.'");
	Сообщение.Сообщить();
...
Процедура РазобратьФайлСведенийРаздела25ПриПодключенномРасширенииРаботыСФайлами(ПомещенныеФайлы, Каталог)
	...
	Если Файл_РегистрационныйНомерПФР <> Отчет_РегистрационныйНомерПФР
	 ...
	 ИЛИ Файл_КодОтчетногоПериода <> Отчет_КодОтчетногоПериода Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Сведения файла описания пакета сведений персонифицированного учета не соответствуют экземпляру отчета!';ru='Сведения файла описания пакета сведений персонифицированного учета не соответствуют экземпляру отчета!'");
		Сообщение.Сообщить();
...
Процедура СообщитьОбОшибкеФорматаИмпорта()
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ka='Нарушен формат файла описания пакета сведений персонифицированного учета!';ru='Нарушен формат файла описания пакета сведений персонифицированного учета!'");
	Сообщение.Сообщить();
...
Процедура ПроверитьВыгрузкуЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ПерсУчетВедетсяВоВнешнейПрограмме Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВнешниеСведенияРаздела25) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='Проверка выгрузки невозможна!';ru='Проверка выгрузки невозможна!'"));
	...
	Иначе
		МаксКолвоЭкз = РегламентированнаяОтчетностьКлиентСервер.ПолучитьСвойствоРаздела(ЭтаФорма,
		 ...
		 Если МаксКолвоЭкз <> 0 И ЭтаФорма[мСтруктураМногостраничныхРазделов[ИмяТекущейСтраницыПанели]].Количество()
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.';ru='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.'"), ЗаголовокСтраницы, МаксКолвоЭкз);
		...
		Если Форма.СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Каждого Эл Из Форма.СтруктураРеквизитовФормы.мПечатныеФормы Цикл
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Не удалось проставить номер страницы для %1';ru='Не удалось проставить номер страницы для %1'"), Эл.Представление);
					Сообщение.Сообщить();
		...
		Если ЭтаФорма.СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Ном = 0 По ЭтаФорма.СтруктураРеквизитовФормы.мПечатныеФормы.Количество() - 1 Цикл
				...
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось проставить номер страницы для %1';ru='Не удалось проставить номер страницы для %1'"), Эл.Представление);
					Сообщение.Сообщить();
...
Процедура ПроверитьЗаполнениеРаздел1(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненной строке 120 раздела 1 должен быть заполнен раздел 4!';ru='При заполненной строке 120 раздела 1 должен быть заполнен раздел 4!'");
...
Процедура ПроверитьЗаполнениеРаздел21(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела21 Из Раздел21 Цикл
		...
		Если ПустаяСтрока(КодТарифа) Тогда
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2 не задан код тарифа!';ru='На странице %1 раздела 2 не задан код тарифа!'"), НомерЛиста);
		...
		ИначеЕсли РазрешенныеКодыТарифов.Найти(КодТарифа) = Неопределено Тогда
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2 неверно задан код тарифа!';ru='На странице %1 раздела 2 неверно задан код тарифа!'"), НомерЛиста);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, ТекстОписанияОшибки, "Раздел21", "П000210001001", НомерЛиста);
			СтатусПроверки = Ложь;
		Иначе
...
Процедура ПроверитьЗаполнениеРаздел24(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодОснования) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2.4 не указан код основания!';ru='На странице %1 раздела 2.4 не указан код основания!'"), НомерСтраницы);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
		...
		Если НЕ ЗначениеЗаполнено(Основание) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2.4 не указано основание заполнения раздела!';ru='На странице %1 раздела 2.4 не указано основание заполнения раздела!'"), НомерСтраницы);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Для Каждого Кандидат Из ПроверкаДубликатов Цикл
		...
		Если Кандидат.Счетчик > 1 И ЗначениеЗаполнено(Кандидат.Код) И ЗначениеЗаполнено(Кандидат.Основание) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обнаружены дублирующиеся сочетания кода основания и основания заполнения раздела на страницах %1 в разделе 2.4!'"),
				СписокСтраницДубликатов);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
		...
		Если ДанныеРаздела[ИмяПоказателя321] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя322])
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя323]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана численность членов организации в графе %1 строки 321 раздела 3.1!'"), Графа);
		...
		Если ДанныеРаздела[ИмяПоказателя331] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя332])
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя333]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана среднесписочная численность в графе %1 строки 331 раздела 3.2!'"), Графа);
		...
		Если ДанныеРаздела[ИмяПоказателя334] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя335])
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя336]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан фонд оплаты труда в графе %1 строки 334 раздела 3.2!'"), Графа);
...
Процедура ПроверитьЗаполнениеРаздел33(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
		...
		Если ДанныеРаздела[ИмяПоказателя341] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя342])
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя343]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана сумма доходов в графе %1 строки 341 раздела 3.3!'"), Графа);
	...
	Если НЕ ЗначениеЗаполнено(ДанныеРаздела.П000340034503) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указана дата записи в реестре аккредитованных организаций в строке 345 раздела 3.3!'");
	...
	Если ПустаяСтрока(ДанныеРаздела.П000340034504) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указан номер записи в реестре аккредитованных организаций в строке 345 раздела 3.3!'");
...
Процедура ПроверитьЗаполнениеРаздел34(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Пока ДанныеРаздела.Свойство("П000350000122_" + Формат(НомСтроки, "ЧГ=")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			Если ПустаяСтрока(КодДеятельностиПоОКВЭД) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не задан код вида деятельности по ОКВЭД в строке %1!';ru='Не задан код вида деятельности по ОКВЭД в строке %1!'"), НомерСтроки);
			...
			Иначе
				Если НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(
					...
					ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Указан некорректный код вида деятельности по ОКВЭД в строке %1!'"), НомерСтроки);
					СообщитьОбОшибкеЗаполнения(
						СтатусПроверки, ТекстОписанияОшибки, ИмяРаздела, "П000350000121_" + НомерСтроки);
				КонецЕсли;
	...
	Если НЕ ЕстьПредопределенныйОКВЭД Тогда
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='В таблице раздела 3.4 нет ни одной строки с кодом деятельности по ОКВЭД из списка %1!';ru='В таблице раздела 3.4 нет ни одной строки с кодом деятельности по ОКВЭД из списка %1!'"),
	...
	Если ДанныеРаздела.П000350000204 <> 100 Тогда
		ТекстОписанияОшибки = НСтр("ka='В разделе 3.4 итог по графе 4 должен равняться 100!';ru='В разделе 3.4 итог по графе 4 должен равняться 100!'");
	...
	Если ПустаяСтрока(ДанныеРаздела.П000350035104) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указан номер записи в реестре аккредитованных организаций в строке 351 раздела 3.4!'");
		...
		Если ДанныеРаздела[КонтрольныйПоказатель371] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель372])
			 ...
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель375]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана сумма доходов в графе %1 строки 371 раздела 3.6!'"), Графа);
				СообщитьОбОшибкеЗаполнения(
	...
	Пока ДанныеРаздела.Свойство("П000400000102_" + Формат(НомСтроки, "ЧГ=")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОснованиеДляДоначисленияСВ) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано основание для доначисления страховых взносов в строке %1 раздела 4!'"),
					НомерСтроки);
			...
			Если ЗначениеЗаполнено(ОПСПоДопТарифу21)
				...
				Иначе
					НачТекста = "Не";
				КонецЕсли;
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 указан код основания для применения части 2.1 статьи 58.3 Федерального закона от 24 июля 2009 г. № 212-ФЗ в строке %2 раздела 4!';ru='%1 указан код основания для применения части 2.1 статьи 58.3 Федерального закона от 24 июля 2009 г. № 212-ФЗ в строке %2 раздела 4!'"),
			...
			Если НЕ ЗначениеЗаполнено(ГодНачисления) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан год, за который выявлены и доначислены страховые взносы в строке %1 раздела 4!'"),
				НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(МесяцНачисления) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан месяц, за который выявлены и доначислены страховые взносы в строке %1 раздела 4!'"),
				НомерСтроки);
...
Процедура ПроверитьЗаполнениеРаздел5(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не заполнены сведения об обучающихся!';ru='В разделе 5 не заполнены сведения об обучающихся!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не указана дата записи в реестре!';ru='В разделе 5 не указана дата записи в реестре!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не указан номер записи в реестре!';ru='В разделе 5 не указан номер записи в реестре!'");
...
"пользующихся государственной поддержкой!';ka='В разделе 5 не указаны сведения из реестра молодежных и детских общественных объединений,"
...
Процедура ПроверитьЗаполнениеТитульный(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИНН)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не задан ИНН организации!';ru='Не задан ИНН организации!'"), "Титульный", "ИНН1");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ИНН) <> 0 И СтрДлина(ПараметрыВыгрузки.ИНН) <> 10
		И СтрДлина(ПараметрыВыгрузки.ИНН) <> 12 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Неверно задан ИНН организации!';ru='Неверно задан ИНН организации!'"), "Титульный", "ИНН1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.Свойство("КПП") И НЕ ПустаяСтрока(ПараметрыВыгрузки.КПП)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Неверно задан КПП организации!';ru='Неверно задан КПП организации!'"), "Титульный", "КПП1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если (НЕ ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.НаименованиеОрганизации))
	 ИЛИ (ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.ФамилияИП)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не задан налогоплательщик!';ru='Не задан налогоплательщик!'"), "Титульный", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ РегНомерПФРСоответствуетТребованиям(ПараметрыВыгрузки.РегистрационныйНомерПФР) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ka='საპენსიო ფონდში სარეგ. ნომრის მნიშვნელობა არ შეესაბამება შაბლონს:""XXX-XXX-XXXXXX"" (X - არის ნებისმიერი ციფრი)!';ru='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра)!'"),
			"Титульный", "НомРег");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ru='Не указан тип корректировки!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Признак прекращения деятельности должен указываться как ""Л"" !';ru='Признак прекращения деятельности должен указываться как ""Л"" !'");
	...
	Если ПустаяСтрока(СокрЛП(СтрЗаменить(ПараметрыВыгрузки.КодПоОКВЭД, ".", "")))
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Некорректное значение кода вида деятельности по ОКВЭД!';ru='Некорректное значение кода вида деятельности по ОКВЭД!'"), "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ЛицоПодтверждающееСведения) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не указан тип подписанта!';ru='Не указан тип подписанта!'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрНайти("1,2,3", ПараметрыВыгрузки.ЛицоПодтверждающееСведения) = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Неверно указан тип подписанта!';ru='Неверно указан тип подписанта!'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ФамилияПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не указана фамилия подписанта!';ru='Не указана фамилия подписанта!'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ИмяПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не указано имя подписанта!';ru='Не указано имя подписанта!'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "2" ИЛИ ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "3" Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.НаименованиеУдостоверяющего) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, НСтр("ka='Не указано наименование документа, подтверждающего полномочия представителя!';ru='Не указано наименование документа, подтверждающего полномочия представителя!'"),
				"Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ДатаВыдачи) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, НСтр("ka='Не указана дата выдачи документа, подтверждающего полномочия представителя!';ru='Не указана дата выдачи документа, подтверждающего полномочия представителя!'"),
				"Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверкаТитульногоЛиста(СтруктураРезультата)
	...
	ОписаниеКС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='В элементе ""%1"" указываются цифры 3, 6, 9, 0';ru='В элементе ""%1"" указываются цифры 3, 6, 9, 0'"), НазваниеПоказателя);
	...
	ОписаниеКС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='В элементе ""%1"" указываются значения в формате YYYY больше нуля';ru='В элементе ""%1"" указываются значения в формате YYYY больше нуля'"), НазваниеПоказателя);
	...
	ЗначениеПоказателя = ТитульныйЛист[ИмяПоказателя];
	ОписаниеКС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='В элементе ""%1"" указывается количество больше нуля';ru='В элементе ""%1"" указывается количество больше нуля'"), НазваниеПоказателя);
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
...
Процедура ПроверкаРаздела1(СтруктураРезультата)
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 2015 Тогда
		...
		ОписаниеКС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Значение показателя по ""%1"" должно быть равно или меньше «0».';ru='Значение показателя по ""%1"" должно быть равно или меньше «0».'"), НазваниеПоказателя);
	...
	ОписаниеКС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Значение показателя по ""%1"" должно быть равно или больше «0».';ru='Значение показателя по ""%1"" должно быть равно или больше «0».'"), НазваниеПоказателя);
	...
	ОписаниеКС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Все значения расчета кроме строк ""%1"" должны иметь положительные значения.';ru='Все значения расчета кроме строк ""%1"" должны иметь положительные значения.'"), СтрокиИсключения);
	...
	ОписаниеКС = НСтр("ka='Значение показателя по графе 3 строки 110 раздела 1 должно быть равно сумме строк 205 и 206 графы 3 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 3 строки 110 раздела 1 должно быть равно сумме строк 205 и 206 графы 3 подраздела 2.1 по всем кодам тарифов.'");
	...
	// 17
	ОписаниеКС = НСтр("ka='Значение показателя по графе 8 строки 110 раздела 1 должно быть равно сумме строк 215 графы 3 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 8 строки 110 раздела 1 должно быть равно сумме строк 215 графы 3 подраздела 2.1 по всем кодам тарифов.'");
	...
	ОписаниеКС = 
		НСтр("ka='Если значение графе 3 строки 110 раздела 1 больше «0», то среднесписочная численность не должна быть равна «0».';ru='Если значение графе 3 строки 110 раздела 1 больше «0», то среднесписочная численность не должна быть равна «0».'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя по графе 3 строки 111 раздела 1 должно быть равно сумме строк 205 и 206 графы 4 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 3 строки 111 раздела 1 должно быть равно сумме строк 205 и 206 графы 4 подраздела 2.1 по всем кодам тарифов.'");
	...
	// 23
	ОписаниеКС = НСтр("ka='Значение показателя по графе 8 строки 111 раздела 1 должно быть равно сумме строк 215 графы 4 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 8 строки 111 раздела 1 должно быть равно сумме строк 215 графы 4 подраздела 2.1 по всем кодам тарифов.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя по графе 3 строки 112 раздела 1 должно быть равно сумме строк 205 и 206 графы 5 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 3 строки 112 раздела 1 должно быть равно сумме строк 205 и 206 графы 5 подраздела 2.1 по всем кодам тарифов.'");
	...
	// 27
	ОписаниеКС = НСтр("ka='Значение показателя по графе 8 строки 112 раздела 1 должно быть равно сумме строк 215 графы 5 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 8 строки 112 раздела 1 должно быть равно сумме строк 215 графы 5 подраздела 2.1 по всем кодам тарифов.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя по графе 3 строки 113 раздела 1 должно быть равно сумме строк 205 и 206 графы 6 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 3 строки 113 раздела 1 должно быть равно сумме строк 205 и 206 графы 6 подраздела 2.1 по всем кодам тарифов.'");
	...
	// 31
	ОписаниеКС = НСтр("ka='Значение показателя по графе 8 строки 113 раздела 1 должно быть равно сумме строк 215 графы 6 подраздела 2.1 по всем кодам тарифов.';ru='Значение показателя по графе 8 строки 113 раздела 1 должно быть равно сумме строк 215 графы 6 подраздела 2.1 по всем кодам тарифов.'");
	...
	// 33
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр.3 стр. 114 должно быть равно сумме показателей графы 3 строк 111, 112 и 113.';ru='Значение показателя по гр.3 стр. 114 должно быть равно сумме показателей графы 3 строк 111, 112 и 113.'");
	...
	// 34
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр.6 стр. 114 должно быть равно сумме показателей графы 6 строк 111, 112 и 113.';ru='Значение показателя по гр.6 стр. 114 должно быть равно сумме показателей графы 6 строк 111, 112 и 113.'");
	...
	// 35
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр.7 стр. 114 должно быть равно сумме показателей графы 7 строк 111, 112 и 113.';ru='Значение показателя по гр.7 стр. 114 должно быть равно сумме показателей графы 7 строк 111, 112 и 113.'");
	...
	// 36
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр.8 стр. 114 должно быть равно сумме показателей графы 8 строк 111, 112 и 113.';ru='Значение показателя по гр.8 стр. 114 должно быть равно сумме показателей графы 8 строк 111, 112 и 113.'");
	...
	// 37
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр. 3 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 6 Раздела 4.';ru='Значение показателя по гр. 3 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 6 Раздела 4.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 38
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр. 4 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 8 Раздела 4.';ru='Значение показателя по гр. 4 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 8 Раздела 4.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 39
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр. 5 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 10 Раздела 4.';ru='Значение показателя по гр. 5 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 10 Раздела 4.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 40
	ОписаниеКС = 
		НСтр("ka='Значение показателя  по графе 6 строки 120 равно  сумме значений, указанных в строке «Итого доначислено» графы 11 Раздела 4 и графы 13 Раздела 4 с кодом основания «1» в графе 3.';ru='Значение показателя  по графе 6 строки 120 равно  сумме значений, указанных в строке «Итого доначислено» графы 11 Раздела 4 и графы 13 Раздела 4 с кодом основания «1» в графе 3.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 41
	ОписаниеКС = 
		НСтр("ka='Значение показателя  по графе 7 строки 120 равно  сумме значений, указанных в строке «Итого доначислено» графы 12 Раздела 4 и графы 13 Раздела 4 с кодом основания «2» в графе 3.';ru='Значение показателя  по графе 7 строки 120 равно  сумме значений, указанных в строке «Итого доначислено» графы 12 Раздела 4 и графы 13 Раздела 4 с кодом основания «2» в графе 3.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 42
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр. 8 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 14 Раздела 4.';ru='Значение показателя по гр. 8 стр. 120 должно быть равно значению, указанному в строке «Итого доначислено» графы 14 Раздела 4.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 43
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр. 3 стр. 121 должно быть равно значению, указанному в строке «Итого доначислено» графы 7 Раздела 4.';ru='Значение показателя по гр. 3 стр. 121 должно быть равно значению, указанному в строке «Итого доначислено» графы 7 Раздела 4.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 44
	ОписаниеКС = 
		НСтр("ka='Значение показателя по гр. 4 стр. 121 должно быть равно значению, указанному в строке «Итого доначислено» графы 9 Раздела 4.';ru='Значение показателя по гр. 4 стр. 121 должно быть равно значению, указанному в строке «Итого доначислено» графы 9 Раздела 4.'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	// 45
	ОписаниеКС = 
		НСтр("ka='Значение показателя по графе 3 строки 130 «Всего к уплате» равно сумме показателей графы 3 строк 100, 110 и 120.';ru='Значение показателя по графе 3 строки 130 «Всего к уплате» равно сумме показателей графы 3 строк 100, 110 и 120.'");
	...
	// 46
	ОписаниеКС = 
		НСтр("ka='Значение показателя по графе 4 строки 130 «Всего к уплате» равно сумме показателей графы 4 строк 100, 120.';ru='Значение показателя по графе 4 строки 130 «Всего к уплате» равно сумме показателей графы 4 строк 100, 120.'");
	...
	// 47
	ОписаниеКС = 
		НСтр("ka='Значение показателя по графе 5 строки 130 «Всего к уплате» равно сумме показателей графы 5 строк 100, 120.';ru='Значение показателя по графе 5 строки 130 «Всего к уплате» равно сумме показателей графы 5 строк 100, 120.'");
	...
	// 48
	ОписаниеКС = 
		НСтр("ka='Значение показателя по графе 6 строки 130 «Всего к уплате» равно сумме показателей графы 6 строк 100, 110 и 120.';ru='Значение показателя по графе 6 строки 130 «Всего к уплате» равно сумме показателей графы 6 строк 100, 110 и 120.'");
	...
	// 49
	ОписаниеКС = 
		НСтр("ka='Значение показателя по графе 7 строки 130 «Всего к уплате» равно сумме показателей графы 7 строк 100, 110 и 120.';ru='Значение показателя по графе 7 строки 130 «Всего к уплате» равно сумме показателей графы 7 строк 100, 110 и 120.'");
	...
	// 50
	ОписаниеКС = 
		НСтр("ka='Значение показателя по графе 8 строки 130 «Всего к уплате» равно сумме показателей графы 8 строк 100, 110 и 120.';ru='Значение показателя по графе 8 строки 130 «Всего к уплате» равно сумме показателей графы 8 строк 100, 110 и 120.'");
	...
	// 57
	ОписаниеКС = 
		НСтр("ka='Значение графы 4 строки 140 не должно быть больше значения графы 4 строки 100.';ru='Значение графы 4 строки 140 не должно быть больше значения графы 4 строки 100.'");
	...
	// 59
	ОписаниеКС = 
		НСтр("ka='Гр.3 строки 144 «Итого» должна быть равна сумме значений гр.3 строк 141, 142 и 143.';ru='Гр.3 строки 144 «Итого» должна быть равна сумме значений гр.3 строк 141, 142 и 143.'");
	...
	// 60
	ОписаниеКС = 
		НСтр("ka='Гр.4 строки 144 «Итого» должна быть равна сумме значений гр.4 строк 141,142 и 143.';ru='Гр.4 строки 144 «Итого» должна быть равна сумме значений гр.4 строк 141,142 и 143.'");
	...
	// 61
	ОписаниеКС = 
		НСтр("ka='Гр.5 строки 144 «Итого» должна быть равна сумме значений гр.5 строк 141,142 и 143.';ru='Гр.5 строки 144 «Итого» должна быть равна сумме значений гр.5 строк 141,142 и 143.'");
	...
	// 62
	ОписаниеКС = 
		НСтр("ka='Гр.6 строки 144 «Итого» должна быть равна сумме значений гр.6 строк 141,142 и 143.';ru='Гр.6 строки 144 «Итого» должна быть равна сумме значений гр.6 строк 141,142 и 143.'");
	...
	// 63
	ОписаниеКС = 
		НСтр("ka='Гр.7 строки 144 «Итого» должна быть равна сумме значений гр.7 строк 141,142 и 143.';ru='Гр.7 строки 144 «Итого» должна быть равна сумме значений гр.7 строк 141,142 и 143.'");
	...
	// 64
	ОписаниеКС = 
		НСтр("ka='Гр.8 строки 144 «Итого» должна быть равна сумме значений гр.8 строк 141,142 и 143.';ru='Гр.8 строки 144 «Итого» должна быть равна сумме значений гр.8 строк 141,142 и 143.'");
...
Процедура ПроверкаРаздела2(СтруктураРезультата)
	...
	// 1
	ОписаниеКС = 
		НСтр("ka='Значение показателя «Код тарифа» должно соответствовать справочнику кодов тарифов (01, 03, 04, 06, 07, 08, 09, 10, 11, 12, 13, 15, 16, 18, 19, 20, 36, 52, 53).';ru='Значение показателя «Код тарифа» должно соответствовать справочнику кодов тарифов (01, 03, 04, 06, 07, 08, 09, 10, 11, 12, 13, 15, 16, 18, 19, 20, 36, 52, 53).'");
	...
	// 6
	ОписаниеКС = НСтр("ka='На двух подразделах раздела 2.1 не может быть указан одинаковый код тарифа.';ru='На двух подразделах раздела 2.1 не может быть указан одинаковый код тарифа.'");
	...
	// 8(4)
	ОписаниеКС = 
		НСтр("ka='Если указан тариф 03, то должен быть заполнен раздел 3.1 или 3.2.';ru='Если указан тариф 03, то должен быть заполнен раздел 3.1 или 3.2.'");
	...
	Для Каждого СтраницаР21 Из Раздел21 Цикл
		...
		ОписаниеКС = 
			НСтр("ka='Если при указанном коде тарифа 01, 52, 53 сумма значений строк 203 больше 0, то и значение в строке 206 подраздела 2.1 больше 0.';ru='Если при указанном коде тарифа 01, 52, 53 сумма значений строк 203 больше 0, то и значение в строке 206 подраздела 2.1 больше 0.'");
	...
	// 13
	ОписаниеКС = НСтр("ka='Значения граф строки 200 должно быть меньше либо равно значениям соответствующих граф строки 210';ru='Значения граф строки 200 должно быть меньше либо равно значениям соответствующих граф строки 210'");
	...
	ОписаниеКС = НСтр("ka='Если заполнен раздел 5, то строка 200 должна быть меньше строки 210 раздела 2';ru='Если заполнен раздел 5, то строка 200 должна быть меньше строки 210 раздела 2'");
	...
	ОписаниеКС = НСтр("ka='Значение графы 3 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 3 строк 205-206.';ru='Значение графы 3 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 3 строк 205-206.'");
	НомГрСтр = "3";
	...
	ОписаниеКС = НСтр("ka='Значение графы 4 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 4 строк 205-206.';ru='Значение графы 4 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 4 строк 205-206.'");
	НомГрСтр = "4";
	...
	ОписаниеКС = НСтр("ka='Значение графы 5 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 5 строк 205-206.';ru='Значение графы 5 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 5 строк 205-206.'");
	НомГрСтр = "5";
	...
	ОписаниеКС = НСтр("ka='Значение графы 6 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 6 строк 205-206.';ru='Значение графы 6 строки 207 должно иметь ненулевое значение при ненулевой сумме значений графы 6 строк 205-206.'");
	НомГрСтр = "6";
	...
	// 34
	ОписаниеКС = НСтр("ka='Значения граф 4,5,6 строки 207 меньше либо равно количеству застрахованных лиц на титульном листе.';ru='Значения граф 4,5,6 строки 207 меньше либо равно количеству застрахованных лиц на титульном листе.'");
	...
	ОписаниеКС = НСтр("ka='Графа 3 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 3 строки 203';ru='Графа 3 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 3 строки 203'");
	НомГрСтр = "3";
	...
	ОписаниеКС = НСтр("ka='Графа 4 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 4 строки 203';ru='Графа 4 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 4 строки 203'");
	НомГрСтр = "4";
	...
	ОписаниеКС = НСтр("ka='Графа 5 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 5 строки 203';ru='Графа 5 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 5 строки 203'");
	НомГрСтр = "5";
	...
	ОписаниеКС = НСтр("ka='Графа 6 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 6 строки 203';ru='Графа 6 строки 208 должна иметь ненулевое значение при ненулевой сумме значений графы 6 строки 203'");
	НомГрСтр = "6";
	...
	ОписаниеКС = НСтр("ka='Значение гр.3 стр.208 должно быть не меньше значений граф 4, или 5, или 6';ru='Значение гр.3 стр.208 должно быть не меньше значений граф 4, или 5, или 6'");
	НомГрСтр = "3";
	...
	// 41
	ОписаниеКС = 
		НСтр("ka='Значения в строке 208 меньше либо равны соответствующим значениям строки 207';ru='Значения в строке 208 меньше либо равны соответствующим значениям строки 207'");
	...
	// 43
	ОписаниеКС = 
		НСтр("ka='Значение строки 211 равно значению соответствующей графы строки 201';ru='Значение строки 211 равно значению соответствующей графы строки 201'");
	...
	// 45
	ОписаниеКС = НСтр("ka='Значения граф строки 212 равны значениям соответствующих граф строки 202';ru='Значения граф строки 212 равны значениям соответствующих граф строки 202'");
	...
	// 47
	ОписаниеКС = НСтр("ka='Значения граф строки 213 равны значениям соответствующих граф строки 203';ru='Значения граф строки 213 равны значениям соответствующих граф строки 203'");
	...
	// 55(29)
	ОписаниеКС = НСтр("ka='Значение графы 3 строки 215 должно быть больше или равно сумме граф 4,5,6 строки 215';ru='Значение графы 3 строки 215 должно быть больше или равно сумме граф 4,5,6 строки 215'");
...
Процедура ПроверкаРаздела22(СтруктураРезультата)
	...
	// 11
	ОписаниеКС = НСтр("ka='Значения в графах 4, 5, 6 строки 225 не больше значения поля «Количество застрахованных лиц» на титульном листе.';ru='Значения в графах 4, 5, 6 строки 225 не больше значения поля «Количество застрахованных лиц» на титульном листе.'");
	...
	// 11
	ОписаниеКС = НСтр("ka='Значения в графах 4, 5, 6 строки 235 не больше значения поля «Количество застрахованных лиц» на титульном листе.';ru='Значения в графах 4, 5, 6 строки 235 не больше значения поля «Количество застрахованных лиц» на титульном листе.'");
...
Процедура ПроверкаРаздела24(СтруктураРезультата)
	...
	ОписаниеКС = 
		НСтр("ka='Должен быть заполнен один из элементов «результатов спецоценки», «результатов аттестации рабочих мест», «результатов спецоценки и результатов аттестации рабочих мест».';ru='Должен быть заполнен один из элементов «результатов спецоценки», «результатов аттестации рабочих мест», «результатов спецоценки и результатов аттестации рабочих мест».'");
	...
	// 14
	ОписаниеКС = НСтр("ka='Значения граф 4, 5, 6 строки 245 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.';ru='Значения граф 4, 5, 6 строки 245 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.'");
	...
	// 25
	ОписаниеКС = НСтр("ka='Значения граф 4, 5, 6 строки 251 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.';ru='Значения граф 4, 5, 6 строки 251 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.'");
	...
	// 36
	ОписаниеКС = НСтр("ka='Значения граф 4, 5, 6 строки 257 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.';ru='Значения граф 4, 5, 6 строки 257 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.'");
	...
	// 47
	ОписаниеКС = НСтр("ka='Значения граф 4, 5, 6 строки 263 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.';ru='Значения граф 4, 5, 6 строки 263 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.'");
	...
	// 58
	ОписаниеКС = НСтр("ka='Значения граф 4, 5, 6 строки 269 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.';ru='Значения граф 4, 5, 6 строки 269 не больше значения поля «Количество застрахованных лиц» на титульном листе по всем представленным основаниям раздела 2.4.'");
...
Процедура ПроверкаРаздела31(СтруктураРезультата)
	...
	// подраздел 3.1
	// 1
	ОписаниеКС = НСтр("ka='Если раздел заполнен, то в расчете должен быть подраздел 2.1 раздела 2 с кодом тарифа 03.';ru='Если раздел заполнен, то в расчете должен быть подраздел 2.1 раздела 2 с кодом тарифа 03.'");
	...
	// 2
	ОписаниеКС = НСтр("ka='Значения соответствующих граф стр. 321 не должно равняться 0, если в строке 322 не 0.';ru='Значения соответствующих граф стр. 321 не должно равняться 0, если в строке 322 не 0.'");
	Для ИндГр = 3 По 6 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	// 3(1)
	ОписаниеКС = НСтр("ka='Значение строки 321 должно быть больше или равно значению строки 322.';ru='Значение строки 321 должно быть больше или равно значению строки 322.'");
	Для ИндГр = 3 По 6 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	// 4(2)
	ОписаниеКС = НСтр("ka='Значение показателя строки 323 равно отношению строк 322 и 321, умноженному на 100% и не может быть больше 100%.';ru='Значение показателя строки 323 равно отношению строк 322 и 321, умноженному на 100% и не может быть больше 100%.'");
	...
	ОписаниеКС = НСтр("ka='Значение в строке 323 должно быть больше либо равно 80% только если значение в строке 321 не равно «0».';ru='Значение в строке 323 должно быть больше либо равно 80% только если значение в строке 321 не равно «0».'");
	...
	// 3(1)
	ОписаниеКС = НСтр("ka='Графа 3 строки 331 должна соответствовать данным ячейки «Среднесписочная численность» Титульного листа (при коде тарифа 03) при условии что стр.332 не равна «0»';ru='Графа 3 строки 331 должна соответствовать данным ячейки «Среднесписочная численность» Титульного листа (при коде тарифа 03) при условии что стр.332 не равна «0»'");
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , СчетчикФормулДляРасшифровок);
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 333 равно отношению строк 332 и 331, умноженному на 100% и не может быть больше 100%.';ru='Значение показателя строки 333 равно отношению строк 332 и 331, умноженному на 100% и не может быть больше 100%.'");
	Для ИндГр = 3 По 6 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 336 равно отношению строк 335 и 334, умноженному на 100% и не может быть больше 100%.';ru='Значение показателя строки 336 равно отношению строк 335 и 334, умноженному на 100% и не может быть больше 100%.'");
	Для ИндГр = 3 По 6 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	// 9
	ОписаниеКС = НСтр("ka='Значение показателя графы 3 строк 334 и 335 должно быть больше или равно сумме граф 4, 5, 6 соответствующих строк (по отчетности за 1 квартал значения данных граф должны быть равны).';ru='Значение показателя графы 3 строк 334 и 335 должно быть больше или равно сумме граф 4, 5, 6 соответствующих строк (по отчетности за 1 квартал значения данных граф должны быть равны).'");
	...
	ОписаниеКС = НСтр("ka='Значение в строке 333 больше либо равно 50% только если стр.331 не равна «0».';ru='Значение в строке 333 больше либо равно 50% только если стр.331 не равна «0».'");
	...
	ОписаниеКС = НСтр("ka='Значение в строке 336 больше либо равно 25% только если стр.331 не равна «0».';ru='Значение в строке 336 больше либо равно 25% только если стр.331 не равна «0».'");
	Для ИндГр = 3 По 6 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	// подраздел 3.3
	// 1
	ОписаниеКС = НСтр("ka='Если раздел заполнен, то в расчете должен быть подраздел 2.1 раздела 2 с кодом тарифа 06.';ru='Если раздел заполнен, то в расчете должен быть подраздел 2.1 раздела 2 с кодом тарифа 06.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 341, больше либо равно значению показателя 342';ru='Значение показателя строки 341, больше либо равно значению показателя 342'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 343 равно отношению строк 342 и 341, умноженному на 100% и не может быть больше 100%.';ru='Значение показателя строки 343 равно отношению строк 342 и 341, умноженному на 100% и не может быть больше 100%.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 343 больше либо равно 90%.';ru='Значение показателя строки 343 больше либо равно 90%.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 345 заполнено.';ru='Значение показателя строки 345 заполнено.'");
...
Процедура ПроверкаРаздела34(СтруктураРезультата)
	...
	// подраздел 3.4
	// 1
	ОписаниеКС = НСтр("ka='Если раздел заполнен, то в расчете должен быть заполнен подраздел раздела 2.1 с тарифом 09.';ru='Если раздел заполнен, то в расчете должен быть заполнен подраздел раздела 2.1 с тарифом 09.'");
	...
	ОписаниеКС = НСтр("ka='Графа 4 рассчитывается как отношение графы 3 соответствующего вида к строке «Итого» графы 3 умноженному на 100% и не может быть больше 100%.';ru='Графа 4 рассчитывается как отношение графы 3 соответствующего вида к строке «Итого» графы 3 умноженному на 100% и не может быть больше 100%.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателей в строке «Итого по всем видам деятельности» подраздела 3.4 по графе 3 равно сумме строк раздела по графе 3.';ru='Значение показателей в строке «Итого по всем видам деятельности» подраздела 3.4 по графе 3 равно сумме строк раздела по графе 3.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателей в строке «Итого по всем видам деятельности» подраздела 3.4 по графе 4 равна 100.';ru='Значение показателей в строке «Итого по всем видам деятельности» подраздела 3.4 по графе 4 равна 100.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 351 заполнено.';ru='Значение показателя строки 351 заполнено.'");
...
Процедура ПроверкаРаздела35(СтруктураРезультата)
	...
	// подраздел 3.5
	// 1
	ОписаниеКС = НСтр("ka='Если раздел заполнен, то в расчете должен быть заполнен подраздел раздела 2.1 с тарифом 07.';ru='Если раздел заполнен, то в расчете должен быть заполнен подраздел раздела 2.1 с тарифом 07.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 361, больше либо равно значению показателя 362';ru='Значение показателя строки 361, больше либо равно значению показателя 362'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 363 равно отношению строк 362 и 361, умноженному на 100% и не может быть больше 100%.';ru='Значение показателя строки 363 равно отношению строк 362 и 361, умноженному на 100% и не может быть больше 100%.'");
	...
	ОписаниеКС = НСтр("ka='Значение показателя строки 363 больше либо равно 70%.';ru='Значение показателя строки 363 больше либо равно 70%.'");
	...
	// 1
	ОписаниеКС = НСтр("ka='Если раздел заполнен, то в расчете должен быть заполнен подраздел раздела 2.1 с тарифом 12.';ru='Если раздел заполнен, то в расчете должен быть заполнен подраздел раздела 2.1 с тарифом 12.'");
	...
	ОписаниеКС = НСтр("ka='Значение строки 371 больше или равно сумме строк 372, 373, 374 по соответствующим графам.';ru='Значение строки 371 больше или равно сумме строк 372, 373, 374 по соответствующим графам.'");
	Для ИндГр = 3 По 4 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	ОписаниеКС = НСтр("ka='Значение строки 375 равно отношению суммы строк 372, 373, 374 к строке 371 умноженному на 100% и не может быть больше 100%.';ru='Значение строки 375 равно отношению суммы строк 372, 373, 374 к строке 371 умноженному на 100% и не может быть больше 100%.'");
	Для ИндГр = 3 По 4 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	ОписаниеКС = НСтр("ka='Значение в графах 3,4 строки 375 больше либо равно 70%.';ru='Значение в графах 3,4 строки 375 больше либо равно 70%.'");
	Для ИндГр = 3 По 4 Цикл 
		НомГрСтр = Формат(ИндГр, "ЧГ=0");
	...
	ОписаниеКС = НСтр("ka='Значение в графе 4 строки 371 по итогам расчетного периода должно быть больше 0.';ru='Значение в графе 4 строки 371 по итогам расчетного периода должно быть больше 0.'");
...
Процедура ПроверкаРаздела4(СтруктураРезультата)
	...
	ОписаниеКС = НСтр("ka='Значение графы 7 меньше либо равно значению графы 6 по всем строкам';ru='Значение графы 7 меньше либо равно значению графы 6 по всем строкам'");
	...
	ОписаниеКС = НСтр("ka='Значение графы 9 меньше либо равно значению графы 8 по всем строкам';ru='Значение графы 9 меньше либо равно значению графы 8 по всем строкам'");
	...
	ОписаниеКС = НСтр("ka='В графе 4 должен быть указан год:';ru='В графе 4 должен быть указан год:'") + Символы.ПС + 
		НСтр("ru=' - меньше или равный году, за который предоставлен РСВ;';ka=' - меньше или равный году, за который предоставлен РСВ;'") + Символы.ПС + НСтр("ru=' - больше или равный 2010.';ka=' - больше или равный 2010.'");
	...
	ОписаниеКС = НСтр("ka='В графе 5 должен быть указан месяц:';ru='В графе 5 должен быть указан месяц:'") + Символы.ПС + 
		НСтр("ka=' - больше или равен 1;';ru=' - больше или равен 1;'") + Символы.ПС + НСтр("ru=' - меньше или равен 12.';ka=' - меньше или равен 12.'") + Символы.ПС + 
	...
	ОписаниеКС = НСтр("ka='Должна быть заполнена хотя бы одна из граф: 6, 8, 10, 11, 12, 13 или 14';ru='Должна быть заполнена хотя бы одна из граф: 6, 8, 10, 11, 12, 13 или 14'");
	...
	ОписаниеКС = НСтр("ka='Если заполнена графа 13, то заполняется графа 3, если нет, то значений в графе 3 нет';ru='Если заполнена графа 13, то заполняется графа 3, если нет, то значений в графе 3 нет'");
	...
	ОписаниеКС = НСтр("ka='Значение графы 2 принимает значение  «1» или «2», либо «3»';ru='Значение графы 2 принимает значение  «1» или «2», либо «3»'");
	...
	ОписаниеКС = НСтр("ka='Значение графы 3 принимает код либо «1», либо «2»';ru='Значение графы 3 принимает код либо «1», либо «2»'");
	...
	ОписаниеКС = НСтр("ka='Значение показателей в строке «Итого доначислено» раздела 4 по графам 6-14 равно сумме строк раздела по каждой отдельно взятой графе.';ru='Значение показателей в строке «Итого доначислено» раздела 4 по графам 6-14 равно сумме строк раздела по каждой отдельно взятой графе.'");
...
Процедура ПроверкаРаздела5(СтруктураРезультата)
	...
	ОписаниеКС = НСтр("ka='Значение показателя графы 5 должно быть больше или равно сумме граф 6, 7, 8 соответствующих строк (по отчетности за 1 квартал значения данных граф должны быть равны).';ru='Значение показателя графы 5 должно быть больше или равно сумме граф 6, 7, 8 соответствующих строк (по отчетности за 1 квартал значения данных граф должны быть равны).'");
	...
	ОписаниеКС = НСтр("ka='Значение показателей в строке «Итого выплат» раздела 5 по графам 5-8 равно сумме строк раздела по каждой отдельно взятой графе.';ru='Значение показателей в строке «Итого выплат» раздела 5 по графам 5-8 равно сумме строк раздела по каждой отдельно взятой графе.'");
...
Процедура ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, ОписаниеКС, ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, ЭтоОшибка, Комментарий = "")
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), НСтр("ka='Нарушено соотношение показателей. Возможно, показатели заполнены некорректно.';ru='Нарушено соотношение показателей. Возможно, показатели заполнены некорректно.'"));
	СтруктураОшибок.Вставить("Расшифровка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), Расшифровка));
	СтруктураОшибок.Вставить("ЭтоОшибка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), ЭтоОшибка);
	СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1. %2';ru='%1. %2'"), ТекстФормулы, ОписаниеКС));
