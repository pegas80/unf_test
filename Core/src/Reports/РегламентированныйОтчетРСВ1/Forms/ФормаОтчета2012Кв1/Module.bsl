...
Функция ЗагрузкаОтчетаИзФайла(ДанныеДляЗагрузки) Экспорт
	...
	Если Модифицированность Тогда
		...
		Сообщение.Текст = СтрШаблон(НСтр("ka='Отчет ""%1"" не сохранен.';ru='Отчет ""%1"" не сохранен.';en='Report ""%1"" is not stored.'"), ЭтотОбъект.Заголовок);
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Процедура УдалитьСтраницу(Команда)
	...
	ТекстВопроса = НСтр("ka='Удалить текущую страницу?';ru='Удалить текущую страницу?';en='Delete current page?'");
...
Процедура УдалитьСтроку(Команда)
	...
	Если ИмяГруппы = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ka='Не выбрана удаляемая строка многострочного раздела.';ru='Не выбрана удаляемая строка многострочного раздела.';en='Not selected delete rows multi-section.'");
	...
	ТекстВопроса = НСтр("ka='Удалить выбранную строку?';ru='Удалить выбранную строку?';en='Remove the selected row?'");
...
Процедура ОбновитьПользовательскиеНастройки()
	...
	Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Изменилась версия формы отчета.%1Выполняется обновление пользовательских настроек.';ru='Изменилась версия формы отчета.%1Выполняется обновление пользовательских настроек.';en='Changed version of the report form.% 1Vypolnyaetsya update user preferences.'"), Символы.ПС);
	...
	Если ЕстьИзмененияВариантыЗаполнения ИЛИ ЕстьИзмененияНастройкаСтраниц Тогда
		...
		Сообщение.Текст = НСтр("ka='Обновление пользовательских настроек успешно завершено.';ru='Обновление пользовательских настроек успешно завершено.';en='Update user settings successfully completed.'");
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='Изменения в структуре пользовательских настроек не обнаружены.';ru='Изменения в структуре пользовательских настроек не обнаружены.';en='Changes in the structure of the custom settings are not found.'");
...
Процедура СохранитьНаКлиентеСВопросомПродолжение(Автосохранение, Вариант, ВыполняемоеОповещение, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки)
	...
	Иначе
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინახება %1...';ru='Сохраняется %1...';en='Saved% 1 ...'"), ЭтаФорма.Заголовок), , , БиблиотекаКартинок.Записать);
...
Процедура ВставитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено)
	...
	Иначе
		...
		Если МаксКолвоЭкз <> 0 И ЭтаФорма[мСтруктураМногостраничныхРазделов[ИмяТекущейСтраницыПанели]].Количество() >= МаксКолвоЭкз Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.';ru='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.';en='The ""%1"", the maximum number of copies of pages is 2%.'"), ЗаголовокСтраницы, МаксКолвоЭкз);
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если ИмяГруппы = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ka='არჩეული არ არის მრავალსტრიქონიანი ნაწილი, რომელშიც საჭიროა სტრიქონის დამატება.';ru='Не выбран многострочный раздел, в который необходимо добавить строку.';en='Not selected multi-section, in which you want to add a line.'");
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ВыгружатьСОшибками = Ложь) Экспорт
	...
	Если Открыта() Тогда
		Если СтруктураРеквизитовФормы.ЕдиницаИзмерения <> ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1 Тогда
			...
			Сообщение.Текст = НСтр("ka='Перед выгрузкой необходимо установить единицу измерения ""в рублях"".';ru='Перед выгрузкой необходимо установить единицу измерения ""в рублях"".';en='Before uploading, you need to install the unit ""ruble"".'");
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения <> 0 Тогда
			...
			Сообщение.Текст = НСтр("ka='Перед выгрузкой необходимо установить точность ""0"".';ru='Перед выгрузкой необходимо установить точность ""0"".';en='set ""0"" precision before unloading.'");
...
Процедура ПроверитьЗаполнениеРаздел1(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненной строке 120 раздела 1 должен быть заполнен раздел 4!';ru='При заполненной строке 120 раздела 1 должен быть заполнен раздел 4!';en='When filling line 120 of Section 1 must be completed by section 4!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 3 строки 110 раздела 1 не равно сумме значений в графе 3 строк 250 и 252 по всем листам раздела 2!';ru='Значение показателя в графе 3 строки 110 раздела 1 не равно сумме значений в графе 3 строк 250 и 252 по всем листам раздела 2!';en='Significance in column 3, line 110 of section 1 is not equal to the sum of the values ​​in column 3 lines 250 and 252 for all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 4 строки 110 раздела 1 не равно сумме значений в графе 3 строк 251 по всем листам раздела 2!';ru='Значение показателя в графе 4 строки 110 раздела 1 не равно сумме значений в графе 3 строк 251 по всем листам раздела 2!';en='Significance in column 4 lines 1 section 110 is not equal to the sum of the values ​​in column 3 lines 251 to all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 5 строки 110 раздела 1 не равно сумме значений в графе 3 строк 276 по всем листам раздела 2!';ru='Значение показателя в графе 5 строки 110 раздела 1 не равно сумме значений в графе 3 строк 276 по всем листам раздела 2!';en='Significance in column 5 lines 110 of section 1 is not equal to the sum of the values ​​in column 3 lines 276 on all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 3 строки 111 раздела 1 не равно сумме значений в графе 4 строк 250 и 252 по всем листам раздела 2!';ru='Значение показателя в графе 3 строки 111 раздела 1 не равно сумме значений в графе 4 строк 250 и 252 по всем листам раздела 2!';en='Significance in column 3, line 111 of section 1 is not equal to the sum of the values ​​in column 4, lines 250 and 252 for all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 4 строки 111 раздела 1 не равно сумме значений в графе 4 строк 251 по всем листам раздела 2!';ru='Значение показателя в графе 4 строки 111 раздела 1 не равно сумме значений в графе 4 строк 251 по всем листам раздела 2!';en='Significance in column 4 lines 1 section 111 is not equal to the sum of the values ​​in column 4 on lines 251 all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 5 строки 111 раздела 1 не равно сумме значений в графе 4 строк 276 по всем листам раздела 2!';ru='Значение показателя в графе 5 строки 111 раздела 1 не равно сумме значений в графе 4 строк 276 по всем листам раздела 2!';en='Significance in column 5 lines 111 of section 1 is not equal to the sum of the values ​​in column 4 on lines 276 all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 3 строки 112 раздела 1 не равно сумме значений в графе 5 строк 250 и 252 по всем листам раздела 2!';ru='Значение показателя в графе 3 строки 112 раздела 1 не равно сумме значений в графе 5 строк 250 и 252 по всем листам раздела 2!';en='Significance in column 3, line 112, section 1 is not equal to the sum of the values ​​in column 5 lines 250 and 252 for all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 4 строки 112 раздела 1 не равно сумме значений в графе 5 строк 251 по всем листам раздела 2!';ru='Значение показателя в графе 4 строки 112 раздела 1 не равно сумме значений в графе 5 строк 251 по всем листам раздела 2!';en='Significance in column 4 lines 1 section 112 is not equal to the sum of the values ​​in column 5 lines 251 of all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 5 строки 112 раздела 1 не равно сумме значений в графе 5 строк 276 по всем листам раздела 2!';ru='Значение показателя в графе 5 строки 112 раздела 1 не равно сумме значений в графе 5 строк 276 по всем листам раздела 2!';en='Significance in column 5 lines 112 of section 1 is not equal to the sum of the values ​​in column 5, lines 276 through all of the sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 3 строки 113 раздела 1 не равно сумме значений в графе 6 строк 250 и 252 по всем листам раздела 2!';ru='Значение показателя в графе 3 строки 113 раздела 1 не равно сумме значений в графе 6 строк 250 и 252 по всем листам раздела 2!';en='Significance in column 3, line 113 of section 1 is not equal to the sum of the values ​​in column 6 lines 250 and 252 for all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 4 строки 113 раздела 1 не равно сумме значений в графе 6 строк 251 по всем листам раздела 2!';ru='Значение показателя в графе 4 строки 113 раздела 1 не равно сумме значений в графе 6 строк 251 по всем листам раздела 2!';en='Significance in column 4 lines 1 section 113 is not equal to the sum of the values ​​in column 6 lines 251 to all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 5 строки 113 раздела 1 не равно сумме значений в графе 6 строк 276 по всем листам раздела 2!';ru='Значение показателя в графе 5 строки 113 раздела 1 не равно сумме значений в графе 6 строк 276 по всем листам раздела 2!';en='Significance in column 5 lines 113 of section 1 is not equal to the sum of the values ​​in column 6 lines 276 on all sheets section 2!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 3 строки 114 не равно сумме значений в графе 3 строк 111, 112 и 113!';ru='В разделе 1 значение показателя в графе 3 строки 114 не равно сумме значений в графе 3 строк 111, 112 и 113!';en='In the index value 1 in column 3, line 114 is not equal to the sum of the values ​​in column 3 lines 111, 112 and 113!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 4 строки 114 не равно сумме значений в графе 4 строк 111, 112 и 113!';ru='В разделе 1 значение показателя в графе 4 строки 114 не равно сумме значений в графе 4 строк 111, 112 и 113!';en='In the index value 1 in column 4 line 114 is not equal to the sum of the values ​​in column 4, lines 111, 112 and 113!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 5 строки 114 не равно сумме значений в графе 5 строк 111, 112 и 113!';ru='В разделе 1 значение показателя в графе 5 строки 114 не равно сумме значений в графе 5 строк 111, 112 и 113!';en='In the index value 1 in column 5 line 114 is not equal to the sum of the values ​​in column 5 lines 111, 112 and 113!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 3 строки 120 не равно значению в графе 4 строки ""Итого доначислено"" раздела 4!';ru='В разделе 1 значение показателя в графе 3 строки 120 не равно значению в графе 4 строки ""Итого доначислено"" раздела 4!';en='Section 1 of the value of the index in column 3 of row 120 is not equal to the value in column 4 of line ""Total assessed additional"" section 4!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 4 строки 120 не равно значению в графе 5 строки ""Итого доначислено"" раздела 4!';ru='В разделе 1 значение показателя в графе 4 строки 120 не равно значению в графе 5 строки ""Итого доначислено"" раздела 4!';en='Section 1 of the value of the index in column 4 line 120 is not equal to the value in column 5 line ""Total assessed additional"" section 4!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 5 строки 120 не равно значению в графе 6 строки ""Итого доначислено"" раздела 4!';ru='В разделе 1 значение показателя в графе 5 строки 120 не равно значению в графе 6 строки ""Итого доначислено"" раздела 4!';en='In the index value 1 in column 5 line 120 is not equal to the value in column 6 line ""Total assessed additional"" section 4!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 3 строки 130 не равно сумме значений в графе 3 строк 100, 110 и 120!';ru='В разделе 1 значение показателя в графе 3 строки 130 не равно сумме значений в графе 3 строк 100, 110 и 120!';en='In the index value 1 in column 3, line 130 is not equal to the sum of the values ​​in column 3 lines 100, 110 and 120!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 4 строки 130 не равно сумме значений в графе 4 строк 100, 110 и 120!';ru='В разделе 1 значение показателя в графе 4 строки 130 не равно сумме значений в графе 4 строк 100, 110 и 120!';en='In the index value 1 in column 4 line 130 is not equal to the sum of the values ​​in column 4, lines 100, 110 and 120!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 5 строки 130 не равно сумме значений в графе 5 строк 100, 110 и 120!';ru='В разделе 1 значение показателя в графе 5 строки 130 не равно сумме значений в графе 5 строк 100, 110 и 120!';en='In the index value 1 in column 5 line 130 is not equal to the sum of the values ​​in column 5 lines 100, 110 and 120!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 3 строки 144 не равно сумме значений в графе 3 строк 141, 142 и 143!';ru='В разделе 1 значение показателя в графе 3 строки 144 не равно сумме значений в графе 3 строк 141, 142 и 143!';en='Section 1 of the value of the index in column 3 of row 144 is not equal to the sum of the values ​​in column 3 lines of 141, 142 and 143!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 4 строки 144 не равно сумме значений в графе 4 строк 141, 142 и 143!';ru='В разделе 1 значение показателя в графе 4 строки 144 не равно сумме значений в графе 4 строк 141, 142 и 143!';en='Section 1 of the value of the index in column 4 line 144 is not equal to the sum of the values ​​in column 4 lines of 141, 142 and 143!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 5 строки 144 не равно сумме значений в графе 5 строк 141, 142 и 143!';ru='В разделе 1 значение показателя в графе 5 строки 144 не равно сумме значений в графе 5 строк 141, 142 и 143!';en='Section 1 of the value of the index in column 5 of line 144 does not equal the sum of the values ​​in column 5 lines of 141, 142 and 143!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 3 строки 150 не равно разности значений в графе 3 строк 130 и 140!';ru='В разделе 1 значение показателя в графе 3 строки 150 не равно разности значений в графе 3 строк 130 и 140!';en='In the index value 1 in column 3, line 150 is not equal to the difference values ​​in column 3 lines 130 and 140!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 4 строки 150 не равно разности значений в графе 4 строк 130 и 140!';ru='В разделе 1 значение показателя в графе 4 строки 150 не равно разности значений в графе 4 строк 130 и 140!';en='In the index value 1 in column 4 line 150 is not equal to the difference values ​​in column 4 lines 130 and 140!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 1 значение показателя в графе 5 строки 150 не равно разности значений в графе 5 строк 130 и 140!';ru='В разделе 1 значение показателя в графе 5 строки 150 не равно разности значений в графе 5 строк 130 и 140!';en='In the index value 1 in column 5 line 150 is not equal to the difference values ​​in column 5 lines 130 and 140!'");
...
Процедура ПроверитьЗаполнениеРаздел2(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненном разделе 2 с кодом тарифа 03 должен быть заполнен либо раздел 3.1, либо раздел 3.2, либо раздел 3.3!';ru='При заполненном разделе 2 с кодом тарифа 03 должен быть заполнен либо раздел 3.1, либо раздел 3.2, либо раздел 3.3!';en='When completing section 2 of the tariff code 03 must be filled with either Section 3.1 or Section 3.2 or Section 3.3!'");
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненном разделе 2 с кодом тарифа 06 должен быть заполнен раздел 3.4!';ru='При заполненном разделе 2 с кодом тарифа 06 должен быть заполнен раздел 3.4!';en='When completing section 2 of the tariff code 06 should be filled with Section 3.4!'");
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненном разделе 2 с кодом тарифа 07 должен быть заполнен раздел 3.6!';ru='При заполненном разделе 2 с кодом тарифа 07 должен быть заполнен раздел 3.6!';en='When completing section 2 of the tariff code 07 should be filled with section 3.6!'");
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненном разделе 2 с кодом тарифа 09 должен быть заполнен раздел 3.5!';ru='При заполненном разделе 2 с кодом тарифа 09 должен быть заполнен раздел 3.5!';en='When completing section 2 of the tariff code 09 should be filled with section 3.5!'");
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненном разделе 2 с кодом тарифа 12 должен быть заполнен раздел 3.7!';ru='При заполненном разделе 2 с кодом тарифа 12 должен быть заполнен раздел 3.7!';en='When completing section 2 of the tariff code 12 should be filled with section 3.7!'");
	...
	ТекстОписанияОшибки = НСтр("ka='При заполненном разделе 2 с кодом тарифа 14 должен быть заполнен раздел 3.8!';ru='При заполненном разделе 2 с кодом тарифа 14 должен быть заполнен раздел 3.8!';en='When completing section 2 of the tariff code 14 should be filled with section 3.8!'");
...
Процедура ПроверитьЗаполнениеРаздел31(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 5 раздела 31 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 5 раздела 31 не совпадает с суммой значений по строкам!';en='Total value of column 5 of section 31 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 6 раздела 31 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 6 раздела 31 не совпадает с суммой значений по строкам!';en='Total value of column 6 of section 31 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 7 раздела 31 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 7 раздела 31 не совпадает с суммой значений по строкам!';en='Total value of the column 7, section 31 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 8 раздела 31 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 8 раздела 31 не совпадает с суммой значений по строкам!';en='Total value of the column 8 of Section 31 does not coincide with the sum of the values ​​in the lines!'");
	...
	Для Графа = 3 По 6 Цикл
		...
		Если ДанныеРаздела[ИмяПоказателя321] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя322]) ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя322]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не указана численность членов организации в графе %1 строки 321 раздела 3.2!';ru='Не указана численность членов организации в графе %1 строки 321 раздела 3.2!';en='Do not indicate the number of members in the column% 1 line 321 section 3.2!'"), Графа);
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел32", ИмяПоказателя321);
			КонецЕсли;
		Иначе
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 323 раздела 3.2 не равно отношению значений показателей строк 321 и 322, умноженному на 100!';ru='Значение показателя в графе %1 строки 323 раздела 3.2 не равно отношению значений показателей строк 321 и 322, умноженному на 100!';en='Indicator value in column 1 row% 323 section 3.2 is not equal to the ratio of values ​​in lines 321 and 322, multiplied by 100!'"), Графа);
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 323 раздела 3.2 не может быть меньше 50!';ru='Значение показателя в графе %1 строки 323 раздела 3.2 не может быть меньше 50!';en='Indicator value in column 1 row% 323 section 3.2 can not be less than 50!'"), Графа);
...
Процедура ПроверитьЗаполнениеРаздел33(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Если ЕстьРаздел2СКодомТарифа("03") Тогда
		ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 3 строки 331 раздела 3.3 не равно значений показателя ""Среднесписочная численность"" на титульном листе!';ru='Значение показателя в графе 3 строки 331 раздела 3.3 не равно значений показателя ""Среднесписочная численность"" на титульном листе!';en='Value of the index in column 3 line 331 of Section 3.3 is not equal to the values ​​of the indicator ""average number"" on the title page!'");
	...
	Для Графа = 3 По 6 Цикл
		...
		Если ДанныеРаздела[ИмяПоказателя331] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя332]) ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя333]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не указана среднесписочная численность в графе %1 строки 331 раздела 3.3!';ru='Не указана среднесписочная численность в графе %1 строки 331 раздела 3.3!';en='The average number is not listed in column 1, line% 331 Section 3.3!'"), Графа);
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел33", ИмяПоказателя331);
			КонецЕсли;
		Иначе
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 333 раздела 3.3 не равно отношению значений показателей строк 331 и 332, умноженному на 100!';ru='Значение показателя в графе %1 строки 333 раздела 3.3 не равно отношению значений показателей строк 331 и 332, умноженному на 100!';en='Indicator value in column 1 row% 333 section 3.3 is not equal to the ratio of values ​​in lines 331 and 332, multiplied by 100!'"), Графа);
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 333 раздела 3.3 не может быть меньше 50!';ru='Значение показателя в графе %1 строки 333 раздела 3.3 не может быть меньше 50!';en='Indicator value in column 1 row% 333 section 3.3 can not be less than 50!'"), Графа);
		...
		Если ДанныеРаздела[ИмяПоказателя334] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя335]) ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя336]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не указан фонд оплаты труда в графе %1 строки 334 раздела 3.3!';ru='Не указан фонд оплаты труда в графе %1 строки 334 раздела 3.3!';en='Unknown wage fund in the column% 1 line 334 section 3.3!'"), Графа);
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел33", ИмяПоказателя334);
			КонецЕсли;
		Иначе
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 336 раздела 3.3 не равно отношению значений показателей строк 334 и 335, умноженному на 100!';ru='Значение показателя в графе %1 строки 336 раздела 3.3 не равно отношению значений показателей строк 334 и 335, умноженному на 100!';en='Indicator value in column 1 row% 336 section 3.3 is not equal to the ratio of values ​​in lines 334 and 335, multiplied by 100!'"), Графа);
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 336 раздела 3.3 не может быть меньше 25!';ru='Значение показателя в графе %1 строки 336 раздела 3.3 не может быть меньше 25!';en='Indicator value in column 1 row% 336 section 3.3 can not be less than 25!'"), Графа);
	...
	Если ЭтоПервыйКвартал Тогда
		ТекстОписанияОшибки = НСтр("ka='Сумма значений показателей в графах 4, 5, 6 строки 334 не равна значению показателя в графе 3 строки 334 (расчет за 1-й квартал)!';ru='Сумма значений показателей в графах 4, 5, 6 строки 334 не равна значению показателя в графе 3 строки 334 (расчет за 1-й квартал)!';en='The sum of the values ​​in the columns 4, 5, 6, line 334 is not equal to the index in column 3, line 334 (calculated for 1 quarter)!'");
		...
		КонецЕсли;
	Иначе
		ТекстОписанияОшибки = НСтр("ka='Сумма значений показателей в графах 4, 5, 6 строки 334 превышает значение показателя в графе 3 строки 334!';ru='Сумма значений показателей в графах 4, 5, 6 строки 334 превышает значение показателя в графе 3 строки 334!';en='The sum of the indices in columns 4, 5, 6, line 334 exceeds the value of the index in column 3, line 334!'");
		Если ДанныеРаздела.П000330033403 < СуммаПоСтроке334 Тогда
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел33", "П000330033403");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЭтоПервыйКвартал Тогда
		ТекстОписанияОшибки = НСтр("ka='Сумма значений показателей в графах 4, 5, 6 строки 335 не равна значению показателя в графе 3 строки 335 (расчет за 1-й квартал)!';ru='Сумма значений показателей в графах 4, 5, 6 строки 335 не равна значению показателя в графе 3 строки 335 (расчет за 1-й квартал)!';en='The sum of the values ​​in the columns 4, 5, 6, line 335 is not equal to the index in column 3, line 335 (calculated for 1 quarter)!'");
		...
		КонецЕсли;
	Иначе
		ТекстОписанияОшибки = НСтр("ka='Сумма значений показателей в графах 4, 5, 6 строки 335 превышает значение показателя в графе 3 строки 335!';ru='Сумма значений показателей в графах 4, 5, 6 строки 335 превышает значение показателя в графе 3 строки 335!';en='The sum of the indices in columns 4, 5, 6, line 335 exceeds the value of the index in column 3, line 335!'");
		Если ДанныеРаздела.П000330033503 < СуммаПоСтроке335 Тогда
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел33", "П000330033503");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеРаздел34(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Графа = 3 По 4 Цикл
		...
		Если ДанныеРаздела[ИмяПоказателя341] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя342]) ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя343]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не указана сумма доходов в графе %1 строки 341 раздела 3.4!';ru='Не указана сумма доходов в графе %1 строки 341 раздела 3.4!';en='Undisclosed income in box 1% line 341 section 3.4!'"), Графа);
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел34", ИмяПоказателя341);
			КонецЕсли;
		Иначе
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 343 раздела 3.4 не равно отношению значений показателей строк 341 и 342, умноженному на 100!';ru='Значение показателя в графе %1 строки 343 раздела 3.4 не равно отношению значений показателей строк 341 и 342, умноженному на 100!';en='Indicator value in column 1 row% 343 section 3.4 is not equal to the ratio of values ​​in lines 341 and 342, multiplied by 100!'"), Графа);
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 343 раздела 3.4 не может быть меньше 90!';ru='Значение показателя в графе %1 строки 343 раздела 3.4 не может быть меньше 90!';en='Indicator value in column 1 row% 343 section 3.4 can not be less than 90!'"), Графа);
	...
	ТекстОписанияОшибки = НСтр("ka='Не указана дата записи в реестре аккредитованных организаций в строке 345 раздела 3.4!';ru='Не указана дата записи в реестре аккредитованных организаций в строке 345 раздела 3.4!';en='Not specified the date of recording in the register of accredited organizations in line 345 of section 3.4!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Не указан номер записи в реестре аккредитованных организаций в строке 345 раздела 3.4!';ru='Не указан номер записи в реестре аккредитованных организаций в строке 345 раздела 3.4!';en='Unknown number of entries in the register of accredited organizations in line 345 of section 3.4!'");
...
Процедура ПроверитьЗаполнениеРаздел35(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 3.5 значение итога по графе 3 не равно сумме значений строк таблицы по графе 3!';ru='В разделе 3.5 значение итога по графе 3 не равно сумме значений строк таблицы по графе 3!';en='In section 3.5 of the value of total in column 3 is not equal to the sum of the values ​​of the rows of the table in column 3!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 3.5 итог по графе 4 должен равняться 100!';ru='В разделе 3.5 итог по графе 4 должен равняться 100!';en='In section 3.5 of the total for column 4 has to be 100!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 3.5 значение итога по графе 4 не равно сумме значений строк таблицы по графе 4!';ru='В разделе 3.5 значение итога по графе 4 не равно сумме значений строк таблицы по графе 4!';en='In section 3.5 of the value of total in column 4 is not equal to the sum of lines a table of values ​​in column 4!'");
	Если ДанныеРаздела.П000350000204 <> СуммаДолейДоходов Тогда
		СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел35", "П000350000204");
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ka='Не указана дата внесения в реестр СМИ в строке 351 раздела 3.5!';ru='Не указана дата внесения в реестр СМИ в строке 351 раздела 3.5!';en='Not the date of entering into the register of the media in line 351 of section 3.5!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Не указан номер записи в реестре аккредитованных организаций в строке 351 раздела 3.5!';ru='Не указан номер записи в реестре аккредитованных организаций в строке 351 раздела 3.5!';en='Unknown number of entries in the register of accredited organizations in line 351 of section 3.5!'");
...
Процедура ПроверитьЗаполнениеРаздел36(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Если ДанныеРаздела[ИмяПоказателя361] = 0 Тогда
		Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя362]) ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя363]) Тогда
			ТекстОписанияОшибки = НСтр("ka='Не указана сумма доходов в графе 3 строки 361 раздела 3.6!';ru='Не указана сумма доходов в графе 3 строки 361 раздела 3.6!';en='Undisclosed income in Column 3 line 361 section 3.6!'");
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел36", ИмяПоказателя361);
		КонецЕсли;
	Иначе
		ТекстОписанияОшибки = НСтр("ka='Значение показателя в графе 3 строки 363 раздела 3.6 не равно отношению значений показателей строк 361 и 362, умноженному на 100!';ru='Значение показателя в графе 3 строки 363 раздела 3.6 не равно отношению значений показателей строк 361 и 362, умноженному на 100!';en='Significance in column 3, line 363 section 3.6 is not equal to the ratio of values ​​in lines 361 and 362, multiplied by 100!'");
...
Процедура ПроверитьЗаполнениеРаздел37(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Графа = 3 По 4 Цикл
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='В разделе 3.7 сумма значений показателей в графе %1 по строкам 372, 373 и 374 превышает значение показателя в графе %2 строки 371!';ru='В разделе 3.7 сумма значений показателей в графе %1 по строкам 372, 373 и 374 превышает значение показателя в графе %2 строки 371!';en='In section 3.7 the sum of the figures in the column% 1 in the lines 372, 373 and 374 exceeds the value of the indicator in the column line 371 2%!'"), Графа, Графа);
		...
		Если ДанныеРаздела[КонтрольныйПоказатель371] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель372])
			 ...
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель375]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не указана сумма доходов в графе %1 строки 371 раздела 3.7!';ru='Не указана сумма доходов в графе %1 строки 371 раздела 3.7!';en='Undisclosed income in box 1% line 371 section 3.7!'"), Графа);
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел37", КонтрольныйПоказатель371);
			КонецЕсли;
		Иначе
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 363 раздела 3.7 не равно отношению суммы показателей строк 372, 373, 374 и значения показателя в строке 371, умноженному на 100!';ru='Значение показателя в графе %1 строки 363 раздела 3.7 не равно отношению суммы показателей строк 372, 373, 374 и значения показателя в строке 371, умноженному на 100!';en='Indicator value in column 1 row% 363 section 3.7 is not equal to ratio of the sum in lines 372, 373, 374 and indicator values ​​in row 371, multiplied by 100!'"), Графа);
			ПроверяемоеЗначение = ДанныеРаздела[ПроверяемыйПоказатель];
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 375 раздела 3.7 не может быть меньше 70!';ru='Значение показателя в графе %1 строки 375 раздела 3.7 не может быть меньше 70!';en='Indicator value in column 1 row% 375 section 3.7 can not be less than 70!'"), Графа);
...
Процедура ПроверитьЗаполнениеРаздел38(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Графа = 3 По 4 Цикл
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='В разделе 3.8 значение показателя в графе %1 строки 382 превышает значение показателя в графе %2 строки 381!';ru='В разделе 3.8 значение показателя в графе %1 строки 382 превышает значение показателя в графе %2 строки 381!';en='In section 3.8 the value of the indicator in the column% 1 line 382 exceeds the value of the indicator in the column line 381 2%!'"), Графа);
		...
		Если ДанныеРаздела[КонтрольныйПоказатель381] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель382]) ИЛИ ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель383]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не указана сумма доходов в графе %1 строки 381 раздела 3.8!';ru='Не указана сумма доходов в графе %1 строки 381 раздела 3.8!';en='Undisclosed income in box 1% line 381 section 3.8!'"), Графа);
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел38", КонтрольныйПоказатель381);
			КонецЕсли;
		Иначе
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 383 раздела 3.8 не равно отношению значений показателей строк 381 и 382, умноженному на 100!';ru='Значение показателя в графе %1 строки 383 раздела 3.8 не равно отношению значений показателей строк 381 и 382, умноженному на 100!';en='Indicator value in column 1 row% 383 section 3.8 is not equal to the ratio of values ​​in lines 381 and 382, ​​multiplied by 100!'"), Графа);
			ПроверяемоеЗначение = ДанныеРаздела[ПроверяемыйПоказатель];
		...
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Значение показателя в графе %1 строки 383 раздела 3.8 не может быть меньше 90!';ru='Значение показателя в графе %1 строки 383 раздела 3.8 не может быть меньше 90!';en='Indicator value in column 1 row% 383 section 3.8 can not be less than 90!'"), Графа);
	...
	ТекстОписанияОшибки = НСтр("ka='Не указана дата получения положительного заключения в строке 385 раздела 3.8!';ru='Не указана дата получения положительного заключения в строке 385 раздела 3.8!';en='Not the date of receipt of a positive conclusion to the line 385 section 3.8!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Не указан номер положительного заключения в строке 385 раздела 3.8!';ru='Не указан номер положительного заключения в строке 385 раздела 3.8!';en='Unknown number of positive conclusion in line 385 of section 3.8!'");
...
Процедура ПроверитьЗаполнениеРаздел4(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 4 раздела 4 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 4 раздела 4 не совпадает с суммой значений по строкам!';en='Total value of column 4 of Section 4 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 5 раздела 4 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 5 раздела 4 не совпадает с суммой значений по строкам!';en='Total value of column 5 of Section 4 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 6 раздела 4 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 6 раздела 4 не совпадает с суммой значений по строкам!';en='Total value of column 6 of section 4 does not coincide with the sum of the values ​​in the lines!'");
...
Процедура ПроверитьЗаполнениеРаздел5(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 5 раздела 5 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 5 раздела 5 не совпадает с суммой значений по строкам!';en='Total value of column 5 of section 5 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 6 раздела 5 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 6 раздела 5 не совпадает с суммой значений по строкам!';en='Total value of column 6 of section 5 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 7 раздела 5 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 7 раздела 5 не совпадает с суммой значений по строкам!';en='Total value of the column 7, section 5 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='Значение итога по графе 8 раздела 5 не совпадает с суммой значений по строкам!';ru='Значение итога по графе 8 раздела 5 не совпадает с суммой значений по строкам!';en='Total value of the column 8 section 5 does not coincide with the sum of the values ​​in the lines!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не указана дата записи в реестре!';ru='В разделе 5 не указана дата записи в реестре!';en='Section 5 is not specified the date of recording in the register!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не указан номер записи в реестре!';ru='В разделе 5 не указан номер записи в реестре!';en='Section 5 of the Unknown entry number in the register!'");
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не указаны сведения из реестра молодежных и детских общественных объединений, пользующихся государственной поддержкой!';ru='В разделе 5 не указаны сведения из реестра молодежных и детских общественных объединений, пользующихся государственной поддержкой!';en=""In Section 5 are not specified information from the register of youth and children's organizations, for public support!""");
...
Процедура ПроверитьЗаполнениеТитульный(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИНН) И НЕ РегламентированнаяОтчетностьПереопределяемый.ЭтоИностраннаяОрганизация(СтруктураРеквизитовФормы.Организация) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не задан ИНН организации!';ru='Не задан ИНН организации!';en='Not specified INN organization!'"), "Титульный", "ИНН1");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ИНН) <> 0 И СтрДлина(ПараметрыВыгрузки.ИНН) <> 10 И СтрДлина(ПараметрыВыгрузки.ИНН) <> 12 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Неверно задан ИНН организации!';ru='Неверно задан ИНН организации!';en='TIN is invalid organization!'"), "Титульный", "ИНН1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.Свойство("КПП") И НЕ ПустаяСтрока(ПараметрыВыгрузки.КПП) И СтрДлина(ПараметрыВыгрузки.КПП) <> 9 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Неверно задан КПП организации!';ru='Неверно задан КПП организации!';en='PPC is invalid organization!'"), "Титульный", "КПП1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.Свойство("ОГРН") Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.ОГРН) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не задан ОГРН организации!';ru='Не задан ОГРН организации!';en='Not specified BIN organization!'"), "Титульный", "ОГРН");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(ПараметрыВыгрузки.ОГРН) <> 13 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Неверно задан ОГРН организации!';ru='Неверно задан ОГРН организации!';en='Is invalid BIN organization!'"), "Титульный", "ОГРН");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.Свойство("ОГРНИП") Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.ОГРНИП) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не задан ОГРН индивидуального предпринимателя!';ru='Не задан ОГРН индивидуального предпринимателя!';en='Not specified BIN individual entrepreneur!'"), "Титульный", "ОГРН");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(ПараметрыВыгрузки.ОГРНИП) <> 15 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Неверно задан ОГРН индивидуального предпринимателя!';ru='Неверно задан ОГРН индивидуального предпринимателя!';en='BIN is invalid individual entrepreneur!'"), "Титульный", "ОГРН");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если (НЕ ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.НаименованиеОрганизации)) ИЛИ (ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.ФамилияИП)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не задан налогоплательщик!';ru='Не задан налогоплательщик!';en='The taxpayer is not given!'"), "Титульный", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ РегНомерПФРСоответствуетТребованиям(ПараметрыВыгрузки.РегистрационныйНомерПФР) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='საპენსიო ფონდში სარეგ. ნომრის მნიშვნელობა არ შეესაბამება შაბლონს:""XXX-XXX-XXXXXX"" (X - არის ნებისმიერი ციფრი)!';ru='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра)!';en='The value of reg. rooms at FIU does not match the pattern: ""XXXXXX-XXXXXX"" (X - any number)!'"), "Титульный", "НомРег");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ka='Признак прекращения деятельности должен указываться как ""Л"" !';ru='Признак прекращения деятельности должен указываться как ""Л"" !';en='Sign of cessation of activity should be referred to as ""L""!'");
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.КодПоОКАТО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не указан код ОКАТО!';ru='Не указан код ОКАТО!';en='Unknown code OKATO!'"), "Титульный", "ОКАТО");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодПоОКАТО) <> 11 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Неверно указан код ОКАТО!';ru='Неверно указан код ОКАТО!';en='Incorrect code OKATO!'"), "Титульный", "ОКАТО");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(СокрЛП(СтрЗаменить(ПараметрыВыгрузки.КодПоОКВЭД, ".", ""))) ИЛИ НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ПараметрыВыгрузки.КодПоОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Некорректное значение кода вида деятельности по ОКВЭД!';ru='Некорректное значение кода вида деятельности по ОКВЭД!';en='Incorrect value of the code type of activity under OKVED!'"), "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ЛицоПодтверждающееСведения) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не указан тип подписанта!';ru='Не указан тип подписанта!';en='Unknown type of the signatory!'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрНайти("1,2,3", ПараметрыВыгрузки.ЛицоПодтверждающееСведения) = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Неверно указан тип подписанта!';ru='Неверно указан тип подписанта!';en='Incorrect type of the signatory!'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ФамилияПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не указана фамилия подписанта!';ru='Не указана фамилия подписанта!';en='It does not specify the name of the signatory!'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ИмяПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не указано имя подписанта!';ru='Не указано имя подписанта!';en='It does not specify the name of the signatory!'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "2" ИЛИ ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "3" Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.НаименованиеУдостоверяющего) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не указано наименование документа, подтверждающего полномочия представителя!';ru='Не указано наименование документа, подтверждающего полномочия представителя!';en=""Unknown name of the document confirming the representative's powers!"""), "Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если ПустаяСтрока(ПараметрыВыгрузки.ДатаВыдачи) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не указана дата выдачи документа, подтверждающего полномочия представителя!';ru='Не указана дата выдачи документа, подтверждающего полномочия представителя!';en=""Not the date of issue of the document confirming the representative's powers!"""), "Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.Индекс) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Не задан почтовый индекс адреса регистрации!';ru='Не задан почтовый индекс адреса регистрации!';en='Not specified postcode registration addresses!'"), "Титульный", "Адрес1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ЛишьОдноИзЗначениеЗаполнено(ПараметрыВыгрузки.РегионНазвание, ПараметрыВыгрузки.РегионСокращение) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Регион не соответствует шаблону (Название Сокращение)!';ru='Регион не соответствует шаблону (Название Сокращение)!';en='The region does not match the pattern (Title Cut)!'"), "Титульный", "Адрес2");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ЛишьОдноИзЗначениеЗаполнено(ПараметрыВыгрузки.РайонНазвание, ПараметрыВыгрузки.РайонСокращение) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Район не соответствует шаблону (Название Сокращение)!';ru='Район не соответствует шаблону (Название Сокращение)!';en='Neighborhood does not match the pattern (Title Cut)!'"), "Титульный", "Адрес3");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ЛишьОдноИзЗначениеЗаполнено(ПараметрыВыгрузки.ГородНазвание, ПараметрыВыгрузки.ГородСокращение) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Город не соответствует шаблону (Название Сокращение)!';ru='Город не соответствует шаблону (Название Сокращение)!';en='The city does not match the pattern (Title Cut)!'"), "Титульный", "Адрес4");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ЛишьОдноИзЗначениеЗаполнено(ПараметрыВыгрузки.НаселенныйПунктНазвание, ПараметрыВыгрузки.НаселенныйПунктСокращение) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Населенный пункт не соответствует шаблону (Название Сокращение)!';ru='Населенный пункт не соответствует шаблону (Название Сокращение)!';en='Settlement does not match the pattern (Title Cut)!'"), "Титульный", "Адрес5");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ЛишьОдноИзЗначениеЗаполнено(ПараметрыВыгрузки.УлицаНазвание, ПараметрыВыгрузки.УлицаСокращение) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ka='Улица не соответствует шаблону (Название Сокращение)!';ru='Улица не соответствует шаблону (Название Сокращение)!';en='The street does not match the pattern (Title Cut)!'"), "Титульный", "Адрес6");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1. Формируется печатная форма...';ru='%1. Формируется печатная форма...';en='%one. Shaped printing forme ...'"), ЭтаФорма.Заголовок), , , БиблиотекаКартинок.Печать);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если НЕ РегламентированнаяОтчетность.ПринтерДоступен() Тогда
		...
		Сообщение.Текст = НСтр("ka='ბეჭდური ფორმების ჩამოყალიბებამდე აუცილებელია სისტემაში პრინტერის გარკვევა და მისი მიცემა როგორც სტანდარტულად გამოსაყენებელი!';ru='Перед формированием печатных форм необходимо определить в системе принтер и задать его в качестве используемого по умолчанию!';en='Before the formation of printing plates need to define the printer system and set it as the default!'");
