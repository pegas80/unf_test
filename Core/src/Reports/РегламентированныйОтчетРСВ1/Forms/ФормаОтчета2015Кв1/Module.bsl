...
Процедура ВыгрузитьЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ПерсУчетВедетсяВоВнешнейПрограмме Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВнешниеСведенияРаздела25) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='Выгрузка невозможна!';ru='Выгрузка невозможна!';en='Unloading is not possible!'"));
	...
	Иначе
		...
		Если КоличествоЗагруженныхФайлов = 1 Тогда
			...
			Если Прав(ВыбИмяФайла, 4) <> ".pcb" Тогда
				...
				ПоказатьПредупреждение(, НСтр("ka='Необходимо выбрать файл описания пакета сведений персонифицированного учета с расширением ""pcb""!';ru='Необходимо выбрать файл описания пакета сведений персонифицированного учета с расширением ""pcb""!';en='You must select a file of a package of personified registration information with the extension ""pcb""!'"));
			...
			Иначе
				...
				НСтр("ka='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';ru='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';en='packet data definition file personified account% 1 successfully imported.'"), ВыбИмяФайла);
				Сообщение.Сообщить();
...
Процедура ПомещениеФайловРаздела6(Результат, АдресФайла, ВыбИмяФайла, ДопПараметры) Экспорт
	...
	Если Не Результат Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Импорт файлов пачек прерван!';ru='Импорт файлов пачек прерван!';en='Import packs files interrupted!'"));
	...
	Иначе
		...
		Если КоличествоЗагруженныхФайлов > 0 Тогда
			...
			Если МассивВыбИменФайлов.Найти(ВыбИмяФайла) <> Неопределено Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Файл пачки %1 ранее уже был импортирован!';ru='Файл пачки %1 ранее уже был импортирован!';en='pack% 1 file has previously been imported!'"), ВыбИмяФайла);
				Сообщение.Сообщить();
			...
			Иначе
				...
				Если НЕ ВыбранФайлПачки Тогда
					...
					Если КоличествоЗагруженныхФайлов > 0 Тогда
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Файл %1 не предназначен для импорта!';ru='Файл %1 не предназначен для импорта!';en='% 1 file is not intended for import!'"), ВыбИмяФайла);
						Сообщение.Сообщить();
				...
				Иначе
					...
					Исключение
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Не удалость импортировать файл пачки %1!';ru='Не удалость импортировать файл пачки %1!';en='Do not remove the pack to import the file% 1!'"), ВыбИмяФайла);
						Сообщение.Сообщить();
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Файл пачки %1 успешно импортирован.';ru='Файл пачки %1 успешно импортирован.';en='pack% 1 successfully imported file.'"), ВыбИмяФайла);
					Сообщение.Сообщить();
...
Процедура ДиалогПриПодключенномРасширенииРаботыСФайлами()
	...
	Диалог.Фильтр = НСтр("ka='Файл описания пакета выгрузки сведений персонифицированного учета';ru='Файл описания пакета выгрузки сведений персонифицированного учета';en='A package definition file upload personified accounting information'") + "(*.pcb)|*.pcb";
...
Процедура ДиалогПриПодключенномРасширенииРаботыСФайламиПослеОтображенияДиалогаВыбораФайлаПослеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';ru='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';en='packet data definition file personified account% 1 successfully imported.'"),
	...
	Для ИндексМассива = 0 По МассивИменФайлов.Количество() - 1 Цикл
		...
		Исключение
			...
			НСтр("ka='Не удалость импортировать файл пачки %1!';ru='Не удалость импортировать файл пачки %1!';en='Do not remove the pack to import the file% 1!'"), ИмяФайлаПачки));
			Возврат;
		КонецПопытки;
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ka='Файлы пачек:';ru='Файлы пачек:';en='packs files:'");
	Сообщение.Сообщить();
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ka='успешно импортированы.';ru='успешно импортированы.';en='successfully imported.'");
	Сообщение.Сообщить();
...
Процедура РазобратьФайлСведенийРаздела25ПриПодключенномРасширенииРаботыСФайлами(ПомещенныеФайлы, Каталог)
	...
	Если Файл_РегистрационныйНомерПФР <> Отчет_РегистрационныйНомерПФР
	 ...
	 ИЛИ Файл_КодОтчетногоПериода <> Отчет_КодОтчетногоПериода Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Сведения файла описания пакета сведений персонифицированного учета не соответствуют экземпляру отчета!';ru='Сведения файла описания пакета сведений персонифицированного учета не соответствуют экземпляру отчета!';en='Data file of a package of information personified records do not match the instance of the report!'");
		Сообщение.Сообщить();
...
Процедура СообщитьОбОшибкеФорматаИмпорта()
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ka='Нарушен формат файла описания пакета сведений персонифицированного учета!';ru='Нарушен формат файла описания пакета сведений персонифицированного учета!';en='Broken file format of a package of information personified records!'");
	Сообщение.Сообщить();
...
Процедура ПроверитьВыгрузкуЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ПерсУчетВедетсяВоВнешнейПрограмме Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВнешниеСведенияРаздела25) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='Проверка выгрузки невозможна!';ru='Проверка выгрузки невозможна!';en='discharge check is not possible!'"));
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1. Формируется печатная форма...';ru='%1. Формируется печатная форма...';en='%one. Shaped printing forme ...'"), ЭтаФорма.Заголовок), , , БиблиотекаКартинок.Печать);
		...
		Если Форма.СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Каждого Эл Из Форма.СтруктураРеквизитовФормы.мПечатныеФормы Цикл
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Не удалось проставить номер страницы для %1';ru='Не удалось проставить номер страницы для %1';en='Failed to put down the page number to 1%'"), Эл.Представление);
					Сообщение.Сообщить();
		...
		Если ЭтаФорма.СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Ном = 0 По ЭтаФорма.СтруктураРеквизитовФормы.мПечатныеФормы.Количество() - 1 Цикл
				...
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось проставить номер страницы для %1';ru='Не удалось проставить номер страницы для %1';en='Failed to put down the page number to 1%'"), Эл.Представление);
					Сообщение.Сообщить();
...
Процедура ПроверитьЗаполнениеРаздел1(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненной строке 120 раздела 1 должен быть заполнен раздел 4.';en='When filled with 120 bar section 1 must be filled section 4.'");
...
Процедура ПроверитьЗаполнениеРаздел21(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела21 Из Раздел21 Цикл
		...
		Если ПустаяСтрока(КодТарифа) Тогда
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2 не задан код тарифа.';ru='На странице %1 раздела 2 не задан код тарифа.';en='On page 1 of 2% does not set the tariff code.'"), НомерЛиста);
		...
		ИначеЕсли РазрешенныеКодыТарифов.Найти(КодТарифа) = Неопределено Тогда
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2 неверно задан код тарифа.';ru='На странице %1 раздела 2 неверно задан код тарифа.';en='On page 1 of 2% set incorrectly tariff code.'"), НомерЛиста);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, ТекстОписанияОшибки, "Раздел21", "П000210001001", НомерЛиста);
			СтатусПроверки = Ложь;
		Иначе
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненном разделе 2 с кодом тарифа 06 должен быть заполнен раздел 3.1.';en='When completing section 2 with a rate 06 to be filled with code section 3.1.'");
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненном разделе 2 с кодом тарифа 07 должен быть заполнен раздел 3.2.';en='When completing section 2 with a rate 07 to be filled with code section 3.2.'");
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненном разделе 2 с кодом тарифа 12 должен быть заполнен раздел 3.3.';en='When completing section 2 with a rate of 12 must be filled with the code section 3.3.'");
...
Процедура ПроверитьЗаполнениеРаздел24(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодОснования) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2.4 не указан код основания.';ru='На странице %1 раздела 2.4 не указан код основания.';en='On page 1 of 2.4% Unknown base code.'"), НомерСтраницы);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
		...
		Если НЕ ЗначениеЗаполнено(Основание) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На странице %1 раздела 2.4 не указано основание заполнения раздела.';ru='На странице %1 раздела 2.4 не указано основание заполнения раздела.';en='On page 1 of 2.4% Unknown base filling section.'"), НомерСтраницы);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Для Каждого Кандидат Из ПроверкаДубликатов Цикл
		...
		Если Кандидат.Счетчик > 1 И ЗначениеЗаполнено(Кандидат.Код) И ЗначениеЗаполнено(Кандидат.Основание) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обнаружены дублирующиеся сочетания кода основания и основания заполнения раздела на страницах %1 в разделе 2.4.';en='Found duplicate code combining base and filling a base section on pages 1% in Section 2.4.'"),
				СписокСтраницДубликатов);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
...
Процедура ПроверитьЗаполнениеРаздел31(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
		...
		Если ДанныеРаздела[ИмяПоказателя341] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя342])
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя343]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана сумма доходов в графе %1 строки 341 раздела 3.1.';en='Undisclosed income in box 1% line 341 section 3.1.'"), Графа);
	...
	Если НЕ ЗначениеЗаполнено(ДанныеРаздела.П000340034503) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указана дата записи в реестре аккредитованных организаций в строке 345 раздела 3.1.';en='Not specified the date of recording in the register of accredited organizations in line 345 section 3.1.'");
	...
	Если ПустаяСтрока(ДанныеРаздела.П000340034504) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указан номер записи в реестре аккредитованных организаций в строке 345 раздела 3.1.';en='Unknown number of entries in the register of accredited organizations in line 345 section 3.1.'");
	...
	Если ДанныеРаздела[ИмяПоказателя361] = 0 Тогда
		Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя362])
		 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя363]) Тогда
			ТекстОписанияОшибки = НСтр("ru='Не указана сумма доходов в графе 3 строки 361 раздела 3.2.';en='Undisclosed income in Column 3 line 361 section 3.2.'");
		...
		Если ДанныеРаздела[КонтрольныйПоказатель371] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель372])
			 ...
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель375]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана сумма доходов в графе %1 строки 371 раздела 3.3.';en='Undisclosed income in box 1% line 371 section 3.3.'"), Графа);
	...
	Пока ДанныеРаздела.Свойство("П000400000102_" + Формат(НомСтроки, "ЧГ=")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОснованиеДляДоначисленияСВ) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано основание для перерасчета страховых взносов в строке %1 раздела 4.';en='Undisclosed basis for the allocation of premiums in row 1 of 4%.'"),
					НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(ГодНачисления) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан год, за который выявлены и доначислены страховые взносы в строке %1 раздела 4.';en='Unknown year for which identified and assessed additional premiums in line% 1 of Section 4.'"),
				НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(МесяцНачисления) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан месяц, за который выявлены и доначислены страховые взносы в строке %1 раздела 4.';en='Unknown month for which identified and assessed additional premiums in line% 1 of Section 4.'"),
				НомерСтроки);
...
Процедура ПроверитьЗаполнениеРаздел5(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ka='В разделе 5 не заполнены сведения об обучающихся.';ru='В разделе 5 не заполнены сведения об обучающихся.';en='Section 5 is not filled with information about students.'");
...
Процедура ПроверитьЗаполнениеТитульный(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИНН)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не задан ИНН организации.';ru='Не задан ИНН организации.';en='Not specified INN organization.'"), "Титульный", "ИНН");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ИНН) <> 0 И СтрДлина(ПараметрыВыгрузки.ИНН) <> 10
		И СтрДлина(ПараметрыВыгрузки.ИНН) <> 12 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Неверно задан ИНН организации.';en='Is invalid VAT number of the organization.'"), "Титульный", "ИНН");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.Свойство("КПП") И НЕ ПустаяСтрока(ПараметрыВыгрузки.КПП)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Неверно задан КПП организации.';ru='Неверно задан КПП организации.';en='PPC is invalid organizations.'"), "Титульный", "КПП");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если (НЕ ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.НаименованиеОрганизации))
	 ИЛИ (ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.ФамилияИП)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ka='Не задан налогоплательщик.';ru='Не задан налогоплательщик.';en='The taxpayer is not set.'"), "Титульный", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ РегНомерПФРСоответствуетТребованиям(ПараметрыВыгрузки.РегистрационныйНомерПФР) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ka='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра).';ru='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра).';en='The value of reg. rooms at FIU does not match the pattern: ""XXXXXX-XXXXXX"" (X - any number).'"),
			"Титульный", "НомРег");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ru='Не указана причина уточнения.';en='Not specified clarify the reason.'");
	...
	ТекстОписанияОшибки = НСтр("ru='Признак прекращения деятельности должен указываться как ""Л"".';en='Sign of cessation of activity should be referred to as ""L"".'");
	...
	Если ПустаяСтрока(СокрЛП(СтрЗаменить(ПараметрыВыгрузки.КодПоОКВЭД, ".", "")))
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Некорректное значение кода вида деятельности по ОКВЭД.';en='Incorrect value of the code type of activity under OKVED.'"), "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ru='Не указана среднесписочная численность.';en='Not specified average number.'");
	...
	ТекстОписанияОшибки = НСтр("ru='Не указано количество застрахованных лиц.';en='Not Specified number of insured persons.'");
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ЛицоПодтверждающееСведения) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не указан тип подписанта.';en='Unknown type of signatory.'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрНайти("1,2,3", ПараметрыВыгрузки.ЛицоПодтверждающееСведения) = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Неверно указан тип подписанта.';en='Incorrect type of signatory.'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ФамилияПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не указана фамилия подписанта.';en='It does not specify the name of the signatory.'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ИмяПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не указано имя подписанта.';en='It does not specify the name of the signatory.'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "2" ИЛИ ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "3" Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.НаименованиеУдостоверяющего) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, НСтр("ru='Не указано наименование документа, подтверждающего полномочия представителя.';en=""Unknown name of the document confirming the representative's powers."""),
				"Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ДатаВыдачи) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, НСтр("ru='Не указана дата выдачи документа, подтверждающего полномочия представителя.';en=""Not the date of issuance of the document confirming the representative's powers."""),
				"Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
