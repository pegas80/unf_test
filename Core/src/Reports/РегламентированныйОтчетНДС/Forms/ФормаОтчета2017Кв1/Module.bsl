...
Процедура ИзменитьЦветЯчеек(Форма, ТекущаяОбластьИмя = "")
	...
	Если КодОперации = "1011705" ИЛИ КодОперации = "1011707" Тогда
		...
		Если ТекущаяОбластьИмя = "П000200007003"
			...
			Сообщение.Текст = НСтр("ka='Значения по строкам 60, 80, 90, 100 раздела 2 очищены вследствие изменения кода операции по строке 070.';ru='Значения по строкам 60, 80, 90, 100 раздела 2 очищены вследствие изменения кода операции по строке 070.';en='The values ​​in rows 60, 80, 90, 100, section 2 cleaned due to a change operation on row 070 code.'");
	...
	Иначе
		...
		Если Строка060БылаАвторасчитываемая
			...
			Сообщение.Текст = НСтр("ka='Значение по строке 060 раздела 2 очищено вследствие изменения кода операции по строке 070.';ru='Значение по строке 060 раздела 2 очищено вследствие изменения кода операции по строке 070.';en='The value of line 060 cleared section 2 due to a change operation on row 070 code.'");
...
Процедура ВставитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено)
	...
	Иначе
		...
		Если МаксКолвоЭкз <> 0 И ЭтаФорма[мСтруктураМногостраничныхРазделов[ИмяТекущейСтраницыПанели]].Количество() >= МаксКолвоЭкз Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.';ru='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.';en='The ""%1"", the maximum number of copies of pages is 2%.'"),
...
Процедура УдалитьСтроку(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Удалить выбранную строку?';ru='Удалить выбранную строку?';en='Remove the selected row?'"), Символы.ПС);
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ПараметрВводаПоля.Количество() > 0 Тогда
		...
		Если СтрДлина(СокрЛП(Область.Значение)) <> 0 И ПараметрВводаПоля.Длина <> 0 И СтрДлина(СокрЛП(Область.Значение)) <> ПараметрВводаПоля.Длина Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Длина значения показателя должна быть равна %1!';ru='Длина значения показателя должна быть равна %1!';en='The length of the value of the indicator must be equal to 1%!'"), ПараметрВводаПоля.Длина);
			Сообщение.Сообщить();
		КонецЕсли;
	...
	Если Область.Имя = "ОКВЭД" Тогда
		...
		Если НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';ru='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';en='! On the front page is set to an invalid value code of activity OKVED% 1Dopustimy values ​​corresponding to the following patterns:% 2 ""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX .XX ""(X - any number).'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
		КонецЕсли;
...
Процедура ЗаполнитьАвтоПредупредитьОСохранении(Отказ, ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = НСтр("ka='Автоматическое заполнение приведет к сохранению отчета с новыми данными. Заполнить отчет?';ru='Автоматическое заполнение приведет к сохранению отчета с новыми данными. Заполнить отчет?';en='Auto-complete will result in the preservation of the report with the new data. Fill in the report?'");
	...
	ЗаголовокВопроса = НСтр("ka='Автоматическое заполнение отчета';ru='Автоматическое заполнение отчета';en='Automatic filling of the report'");
...
Функция ЗаполнитьАвтоНаСервере()
	...
	НаименованиеЗадания = НСтр("ka='Автоматическое заполнение по данным информационной базы';ru='Автоматическое заполнение по данным информационной базы';en='Automatic filling according to the information base'");
...
Процедура ВывестиРезультатПроверки() Экспорт
	...
	ИначеЕсли РезультатПроверкиВыгрузки.КоличествоОбнаруженныхОшибок = 0
		...
		Если РезультатПроверкиКонтрагентов.ОшибкаПодключенияКСервису Тогда
			Элементы.ТекстCРезультатомПроверки.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='Нет ошибок, препятствующих отправке декларации. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';ru='Нет ошибок, препятствующих отправке декларации. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';en='No error preventing sending the declaration. Checking counterparties FTS service is not executed due to an error while connecting to the service.'"));
			Элементы.РезультатПроверки.ЦветФона = Новый Цвет(255, 255, 192);
			
		Иначе
			Элементы.ТекстCРезультатомПроверки.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='Нет ошибок, препятствующих отправке декларации, и ошибок проверки контрагентов сервисом ФНС.';ru='Нет ошибок, препятствующих отправке декларации, и ошибок проверки контрагентов сервисом ФНС.';en='No error preventing sending the declaration and verification of contractors FTS service errors.'"));
	...
	ИначеЕсли РезультатПроверкиВыгрузки.КоличествоОбнаруженныхОшибок = 0
		...
		ТекстРезультатаПроверки = Новый Массив();
		ТекстРезультатаПроверки.Добавить(НСтр("ka='Нет ошибок, препятствующих отправке декларации.';ru='Нет ошибок, препятствующих отправке декларации.';en='No error preventing sending the declaration.'"));
		ТекстРезультатаПроверки.Добавить(ТекстРезультатаПроверкиКонтрагентов);
	...
	ИначеЕсли РезультатПроверкиВыгрузки.КоличествоОбнаруженныхОшибок > 0
		...
		Если РезультатПроверкиКонтрагентов.ОшибкаПодключенияКСервису Тогда
			...
			ТекстРезультатаПроверки = Новый Массив();
			ТекстРезультатаПроверки.Добавить(ТекстРезультатаПроверкиВыгрузки);
			ТекстРезультатаПроверки.Добавить(НСтр("ka='. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';ru='. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';en='. Checking counterparties FTS service is not executed due to an error while connecting to the service.'"));
		...
		Иначе
			...
			ТекстРезультатаПроверки = Новый Массив();
			ТекстРезультатаПроверки.Добавить(ТекстРезультатаПроверкиВыгрузки);
			ТекстРезультатаПроверки.Добавить(НСтр("ka='. Нет ошибок проверки контрагентов сервисом ФНС.';ru='. Нет ошибок проверки контрагентов сервисом ФНС.';en='. No error checking contractors FTS service.'"));
...
Процедура ВывестиРезультатПроверкиКонтрагентов()
	...
	ИначеЕсли РезультатПроверкиКонтрагентов.КоличествоОбнаруженныхОшибок = 0 Тогда
		...
		Если РезультатПроверкиКонтрагентов.ОшибкаПодключенияКСервису Тогда
			...
			Элементы.ТекстCРезультатомПроверки.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';ru='Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';en='Checking counterparties FTS service is not executed due to an error while connecting to the service.'"));
			
			Элементы.РезультатПроверки.ЦветФона = Новый Цвет(255, 255, 192);
			
		Иначе
			
			Элементы.ТекстCРезультатомПроверки.Заголовок = Новый ФорматированнаяСтрока(НСтр("ka='Нет ошибок проверки контрагентов сервисом ФНС.';ru='Нет ошибок проверки контрагентов сервисом ФНС.';en='No error checking contractors FTS service.'"));
...
Функция ФорматированныйТекстРезультатаПроверки(КолвоОшибок, НаимОперации)
	...
	Если Прав(Строка(КолвоОшибок), 1) = "1"
		...
		Если НаимОперации = "ПроверкаВыгрузки" Тогда
			...
			ТекстРезультатаПроверки.Добавить(НСтр("ka=', препятствующая отправке декларации';ru=', препятствующая отправке декларации';en='Which prevents sending Declaration'"));
	...
	ИначеЕсли (Прав(Строка(КолвоОшибок), 1) = "2"
		...
		Если НаимОперации = "ПроверкаВыгрузки" Тогда
			...
			ТекстРезультатаПроверки.Добавить(НСтр("ka=', препятствующие отправке декларации';ru=', препятствующие отправке декларации';en='Preventing sending Declaration'"));
	...
	Иначе
		...
		Если НаимОперации = "ПроверкаВыгрузки" Тогда
			...
			ТекстРезультатаПроверки.Добавить(НСтр("ka=', препятствующих отправке декларации';ru=', препятствующих отправке декларации';en='Preventing sending Declaration'"));
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПоказатьОшибкиПроверкиВыгрузки" Тогда
		...
		Если СтруктураРеквизитовФормы.Свойство("АдресХранилищаПроверкиВыгрузки") Тогда
			...
			Если РезультатПроверкиВыгрузки.КоличествоОшибок = 0 Тогда
				ПоказатьПредупреждение( , НСтр("ka='Ошибок не обнаружено!';ru='Ошибок не обнаружено!';en='No errors were found!'"));
			Иначе
...
Процедура ПроверитьВозможностьВыгрузкиВФоне()
	...
	НаименованиеЗадания = НСтр("ka='Проверка возможности формирования электронного представления';ru='Проверка возможности формирования электронного представления';en='Checking the possibility of forming the electronic submission'");
