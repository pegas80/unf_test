...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ბრუნვითი უწყისი%1';ru='Оборотно-сальдовая ведомость%1';en='Turnover balance sheet% 1'"),
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	// Kuzmich, 30.08.2023, Вставил "костылик"
	НужыйЭлемент = МакетКомпоновки.Тело[0];
	Для каждого ТекущийЭлемент Из МакетКомпоновки.Тело Цикл
		
		Попытка
		
			Если ТекущийЭлемент.Идентификатор = "Таблица" Тогда
				
				НужыйЭлемент = ТекущийЭлемент;
				Прервать;
	...
	КонецЦикла;
	Для Каждого Колонка Из НужыйЭлемент.Колонки Цикл
	// Kuzmich, 30.08.2023, Вставил "костылик"
	...
	// Kuzmich, 31.08.2023, Вставил "костылик"
	ЗаполнитьМакетыРесурсовПодвалаОтчета(НужыйЭлемент, МакетыРесурсовПодвалаОтчета, СоответствиеМакетовКолонкамОтчета);
	ЗаполнитьМакетыРесурсовГруппировки(НужыйЭлемент.Строки, МакетыРесурсовСчета, СоответствиеМакетовКолонкамОтчета, "Счет");
	// Kuzmich, 31.08.2023, Вставил "костылик"
	...
	// Kuzmich, 31.08.2023, Вставил "костылик"
	ЗаполнитьМакетыРесурсовГруппировки(НужыйЭлемент.Строки, МакетРесурсовГруппировкиВалюта, СоответствиеМакетовКолонкамОтчета, "Валюта", Истина);
	// Kuzmich, 31.08.2023, Вставил "костылик"
...
Функция ПолучитьНаборПоказателей() Экспорт
	...
	НаборПоказателей.Добавить("БУ");//НСтр("ka='ლარი';ru='БУ!';en='BOO!'"));
		...
		Если СтрокаГруппировкиПоСчету <> Неопределено Тогда
			...
			Для Индекс = 1 По КоличествоСубконто Цикл
				...
				Если ИндексСубконто = "0" Тогда
					...
					СтрокаГруппировки.Представление = 	НСтр("ka='განყოფილება';ru='Подразделение';en='Subdivision'");
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	...
	Правило.ШаблонПредставления = НСтр("ka='ბრუნვითი უწყისი ანგარიშის მიხედვით [Счет]';ru='ОСВ по счету [Счет]';en='WWS account [Account]'");
	
	Правило = Правила.Добавить();
	Правило.Отчет = "КарточкаСчета";
	БухгалтерскиеОтчетыРасшифровка.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	Правило.ШаблонПредставления = НСтр("ka='ანგარიშის ბარათი [Счет]';ru='Карточка счета [Счет]';en='The card account [Account]'");
	
	Правило = Правила.Добавить();
	Правило.Отчет = "АнализСчета";
	БухгалтерскиеОтчетыРасшифровка.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	Правило.ШаблонПредставления = НСтр("ka='ანგარიშის ანალიზი [Счет]';ru='Анализ счета [Счет]';en='Analysis of the account [Account]'");
	...
	Правило.ШаблонПредставления = НСтр("ka='ანგარიშის ბრუნვა [Счет] თვეების მიხედვით';ru='Обороты счета [Счет] по месяцам';en='Turnovers account [Account] by month'");
	
	Правило = Правила.Добавить();
	Правило.Отчет = "ОборотыСчета";
	БухгалтерскиеОтчетыРасшифровка.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Счет", "Счет");
	БухгалтерскиеОтчетыРасшифровка.ДобавитьТребуемыйРеквизитРасшифровки(Правило.ТребуемыеРеквизиты, "Периодичность", "", 6);
	Правило.ШаблонПредставления = НСтр("ka='ანგარიშის ბრუნვა [Счет] დღეების მიხედვით';ru='Обороты счета [Счет] по дням';en='Turnovers account [Account] by day'");
...
Процедура СоздатьКолонкиОтчета(КолонкиСтруктурыОтчета, ПараметрыОтчета)
	...
	ГруппаСальдоНаНачало.Заголовок     = НСтр("ka='ნაშთი პერიოდის დასაწყისისთვის';ru='Сальдо на начало периода';en='Beginning balance'");
	...
	ГруппаСальдоНаНачалоДт.Заголовок     = НСтр("ka='დებეტი';ru='Дебет';en='Debit'");
	...
	ГруппаСальдоНаНачалоКт.Заголовок     = НСтр("ka='კრედიტი';ru='Кредит';en='Credit'");
	...
	ГруппаОбороты.Заголовок     = НСтр("ka='ბრუნვა პერიოდში';ru='Обороты за период';en='Period transactions'");
	...
	ГруппаОборотыДт.Заголовок     = НСтр("ka='დებეტი';ru='Дебет';en='Debit'");
	...
	ГруппаОборотыКт.Заголовок     = НСтр("ka='კრედიტი';ru='Кредит';en='Credit'");
	...
	ГруппаСальдоНаКонец.Заголовок     = НСтр("ka='ნაშთი პერიოდის ბოლოსთვის';ru='Сальдо на конец периода';en='balance at the end of period'");
	...
	ГруппаСальдоНаКонецДт.Заголовок     = НСтр("ka='დებეტი';ru='Дебет';en='Debit'");
	...
	ГруппаСальдоНаКонецКт.Заголовок     = НСтр("ka='კრედიტი';ru='Кредит';en='Credit'");
