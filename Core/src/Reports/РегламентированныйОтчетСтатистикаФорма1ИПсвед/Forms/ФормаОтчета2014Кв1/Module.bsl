					...
					Исключение
						Если Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
							...
							Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П000005000101_" + Суффикс].Значение) Тогда
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = НСтр("ka='Не найдена единица измерения для наименования продукции, указанной в графе 1 раздела 5!';ru='Не найдена единица измерения для наименования продукции, указанной в графе 1 раздела 5!';en='Found a unit of measurement for the product names listed in column 1 of section 5!'");
								Сообщение.Сообщить();
							Иначе
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = НСтр("ka='Не указано наименование продукции в графе 1 раздела 5 для выбора единицы измерения!';ru='Не указано наименование продукции в графе 1 раздела 5 для выбора единицы измерения!';en='Unknown name of the product in column 1 of section 5 to select the units!'");
								Сообщение.Сообщить();
							КонецЕсли;
							СтандартнаяОбработка = Ложь;
						КонецЕсли;
...
Процедура ОчиститьОтчет(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Внимание! Отчет будет очищен.%1Продолжить операцию?';ru='Внимание! Отчет будет очищен.%1Продолжить операцию?';en='Attention! The report will be cleared.% 1Prodolzhit operation?'"), Символы.ПС);
...
Процедура ВыгрузитьИзФормыПослеСохраненияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ УниверсальнаяВыгрузкаXMLПроверкаВозможностиВыгрузкиНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Тогда
		ТекстВопроса = НСтр("ka='В ходе выгрузки были найдены ошибки.';ru='В ходе выгрузки были найдены ошибки.';en='During the unloading of the errors were found.'");
...
Процедура ЗаполнитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураРеквизитовФормы.ИнформационнаяБазаФайловая Тогда
		Состояние(НСтр("ka='Выполняется автоматическое заполнение отчета. Пожалуйста, подождите...';ru='Выполняется автоматическое заполнение отчета. Пожалуйста, подождите...';en='Performed automatically fill in the report. Please wait...'"));
	КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.ИнформационнаяБазаФайловая Тогда
		Состояние(НСтр("ka='Автоматическое заполнение выполнено.';ru='Автоматическое заполнение выполнено.';en='Automatic filling done.'"));
	КонецЕсли;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Иначе
		РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			"РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчетВФоне",
			Новый Структура("ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер", ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер),
			НСтр("ka='Выполняется автоматическое заполнение отчета. Пожалуйста, подождите...';ru='Выполняется автоматическое заполнение отчета. Пожалуйста, подождите...';en='Performed automatically fill in the report. Please wait...'"));
