...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ka='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.';ru='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.'");
...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Поле ""ИНН или наименование контрагента"" не заполнено';ru='Поле ""ИНН или наименование контрагента"" не заполнено'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормойНаСервере()
	...
	Если ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Досье контрагента: %1';ru='Досье контрагента: %1'"),
	...
	ИначеЕсли ЗначениеЗаполнено(Контрагент) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Контрагент);
	Иначе
		Заголовок = НСтр("ka='კონტრაგენტის დოსიე';ru='Досье контрагента'");
	КонецЕсли;
	Элементы.ДекорацияРазделДанныеГосРеестров.Заголовок = ?(ЭтоЮридическоеЛицо, 
		НСтр("ka='ЕГРЮЛ';ru='ЕГРЮЛ'"), 
		НСтр("ka='ЕГРИП';ru='ЕГРИП'"));
...
Процедура СформироватьОтчетНаКлиенте(ТекстПоиска)
	...
	Если ПоискПоИНН Тогда
		...
		Если СтрДлина(ТекстПоиска) <> 10 
			...
			ТекстОшибки = НСтр("ka='Для поиска по ИНН необходимо ввести 10 или 12 цифр.';ru='Для поиска по ИНН необходимо ввести 10 или 12 цифр.'");
	...
	Иначе // Поиск по наименованию
		...
		ПараметрыФормы.Вставить("Заголовок",    НСтр("ka='Поиск контрагента';ru='Поиск контрагента'"));
...
Функция СформироватьОтчетНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Формирование отчета: Досье контрагента: %1';ru='Формирование отчета: Досье контрагента: %1'"), 
		ИННПоиска);
