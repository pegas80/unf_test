...
Процедура СформироватьРезультатГлавное(ИнформацияОСвязях, ИнформацияОПроверках, ДанныеОтчета)
	...
	Если ИнформацияОСвязях.КодСтатуса <> Неопределено Тогда
		Если ИнформацияОСвязях.КодСтатуса = "001" Тогда
			ОбластиМакета.ЕГРЮЛ.Параметры.ДействующаяОрганизация = НСтр("ka='Действующая организация (по данным ЕГРЮЛ)';ru='Действующая организация (по данным ЕГРЮЛ)';en='Operating organization (according USRLE)'");
		...
		Иначе
			ОбластиМакета.ЕГРЮЛ.Параметры.ДействующаяОрганизация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 (по данным ЕГРЮЛ)';ru='%1 (по данным ЕГРЮЛ)';en='1% (by EGRYuL)'"),
	...
	Если КоличествоОрганизаций > 0 Тогда
		ОрганизацииПоЭтомуАдресу = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='По этому адресу %1 еще %2';ru='По этому адресу %1 еще %2';en='At this address,% 1% 2'"),
			?(КоличествоОрганизаций % 10 = 1 И НЕ КоличествоОрганизаций % 100 = 11, 
				НСтр("ka='зарегистрирована';ru='зарегистрирована';en='registered'"),
				НСтр("ka='зарегистрированы';ru='зарегистрированы';en='registered'")),
		...
		Если ЗначениеЗаполнено(ИнформацияОСвязях.Руководитель.Должность) Тогда
			Если ЗначениеЗаполнено(ИнформацияОСвязях.Руководитель.ДатаЗаписи) Тогда
				Руководитель = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1, %2 с %3';ru='%1, %2 с %3';en='% 1% 2% 3'"),
					Руководитель,
	...
	Если КоличествоОрганизацийГдеРуководитель > 0 Тогда
		ТекстРуководитель = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Руководитель еще в %1';ru='Руководитель еще в %1';en='Head back in the 1%'"),
			СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	...
	Если КоличествоОрганизацийГдеУчредитель > 0 Тогда
		...
		Если ВыводитьЕще Тогда
			СтрокаПодстановки = ?(ПустаяСтрока(ТекстРуководитель), 
				НСтр("ka='Учредитель еще в %1';ru='Учредитель еще в %1';en='Founder back in the 1%'"), 
				НСтр("ka=', учредитель еще в %1';ru=', учредитель еще в %1';en=', Founder even in 1%'"));
		Иначе
			СтрокаПодстановки = ?(ПустаяСтрока(ТекстРуководитель), 
				НСтр("ka='Учредитель в %1';ru='Учредитель в %1';en='Founder at 1%'"), 
				НСтр("ka=', учредитель в %1';ru=', учредитель в %1';en=', Founder in 1%'"));
		КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ИнформацияОРуководителе) Тогда
		ИнформацияОРуководителе = ИнформацияОРуководителе 
			+ ?(ИнформацияОСвязях.ОрганизацииГдеРуководительПоФИО ИЛИ ИнформацияОСвязях.ОрганизацииГдеУчредительПоФИО,
				" (" + НСтр("ka='поиск проведен по ФИО и может быть неточным';ru='поиск проведен по ФИО и может быть неточным';en='search performed by name and may be inaccurate'") + ")",
				"");
	КонецЕсли;		
	...
	Если ИнформацияОСвязях.УставныйКапитал > 0 Тогда
		ОбластиМакета.ЕГРЮЛ.Параметры.УставныйКапитал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 руб.';ru='%1 руб.';en='% 1 rub.'"),
	...
	Если ГодыЕстьОтчетность.Количество() = 3 Тогда
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Есть в базе Росстата за %1, %2 и %3 годы';ru='Есть в базе Росстата за %1, %2 и %3 годы';en='It exists in the Federal State Statistics Service for% 1% 2 and% 3 years'"),
			ГодыЕстьОтчетность[0],
			ГодыЕстьОтчетность[1],
			ГодыЕстьОтчетность[2]);
	ИначеЕсли ГодыЕстьОтчетность.Количество() = 2 Тогда
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Есть в базе Росстата за %1 и %2 годы, отсутствует за %3 год';ru='Есть в базе Росстата за %1 и %2 годы, отсутствует за %3 год';en='It exists in the Federal State Statistics Service for% 1 and% 2 years missing for the year 3%'"),
			ГодыЕстьОтчетность[0],
			ГодыЕстьОтчетность[1],
			ГодыНетОтчетности[0]);
	ИначеЕсли ГодыЕстьОтчетность.Количество() = 1 Тогда
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Есть в базе Росстата за %1 год, отсутствует за %2 и %3 годы';ru='Есть в базе Росстата за %1 год, отсутствует за %2 и %3 годы';en='It exists in the Federal State Statistics Service of 1% a year, missing for% 2 and% 3 years'"),
			ГодыЕстьОтчетность[0],
			ГодыНетОтчетности[0],
			ГодыНетОтчетности[1]);
	Иначе
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Отсутствует в базе Росстата за %1, %2 и %3 годы';ru='Отсутствует в базе Росстата за %1, %2 и %3 годы';en='It is not in the Federal State Statistics Service for% 1% 2 and% 3 years'"),
			ГодыНетОтчетности[0],
			ГодыНетОтчетности[1],
			ГодыНетОтчетности[2]);
	КонецЕсли;
	...
	Если (ИнформацияОСвязях.ЕстьОтчетностьГод_1 
		...
		АнализОтчетностиЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Показатели за %1 год';ru='Показатели за %1 год';en='Figures for% 1 year'"),
	...
	Если ВсегоПроверок > 0 Тогда
		...
		Если ПроверокСНарушениями = 0 Тогда
			РезультатыПроверок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Известны результаты %1, нарушений не выявлено';ru='Известны результаты %1, нарушений не выявлено';en='Announced the results of 1%, did not reveal violations'"), 
				СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
					ВсегоПроверок, НСтр("ka='проверки,проверок,проверок';ru='проверки,проверок,проверок';en='check, checks, inspections'")));
		Иначе
			ТекстПроверки = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
				ВсегоПроверок, НСтр("ka='проверки,проверок,проверок';ru='проверки,проверок,проверок';en='check, checks, inspections'"));
			Если ВсегоПроверок = 1 Тогда
				ТекстНарушения = НСтр("ka='выявлены нарушения';ru='выявлены нарушения';en='infringements'");
			ИначеЕсли ВсегоПроверок = ПроверокСНарушениями Тогда
				ТекстНарушения = НСтр("ka='все проверки выявили нарушения';ru='все проверки выявили нарушения';en='all inspections revealed violations'");
			Иначе
				ТекстНарушения = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
					ПроверокСНарушениями, 
					НСтр("ka='проверка выявила нарушения,проверки выявили нарушения,проверок выявили нарушения';ru='проверка выявила нарушения,проверки выявили нарушения,проверок выявили нарушения';en='inspection revealed violations, inspections revealed violations, inspections revealed violations'"));
			КонецЕсли;
			РезультатыПроверок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Известны результаты %1, %2';ru='Известны результаты %1, %2';en='Known results% 1,% 2'"), ТекстПроверки, ТекстНарушения);
		КонецЕсли;
	...
	ДанныеОтчета.Вставить("ИмяФайлаГлавное",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - главное';ru='Досье контрагента %1 - главное';en='Dossier counterparty% 1 - the main'"),
...
Процедура СформироватьРезультатДанныеГосРеестров(ИнформацияОСвязях, ДанныеОтчета)
	...
	ДанныеОтчета.Вставить("ИмяФайлаДанныеГосРеестров",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - данные реестра';ru='Досье контрагента %1 - данные реестра';en='counterparty dossier 1% - registry data'"),
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			ИнформацияОСвязях.НаименованиеСокращенное, "")));
...
Процедура ВывестиРазделРегистрация(ТабличныйДокумент, ИнформацияОСвязях, ПараметрыВывода)
	...
	Если ИнформацияОСвязях.КодСтатуса <> Неопределено Тогда
		...
		Если ИнформацияОСвязях.КодСтатуса = "001" Тогда
			Если ЭтоЮридическоеЛицо Тогда
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ka='Действующая организация (по данным ЕГРЮЛ)';ru='Действующая организация (по данным ЕГРЮЛ)';en='Operating organization (according USRLE)'");
			Иначе
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ka='Действующий ИП (по данным ЕГРИП)';ru='Действующий ИП (по данным ЕГРИП)';en='Acting SP (according EGRIP)'");
			КонецЕсли;
		...
		Иначе
			...
			Иначе
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='%1 (по данным ЕГРИП)';ru='%1 (по данным ЕГРИП)';en='1% (according EGRIP)'"),
						ИнформацияОСвязях.НаименованиеСтатуса);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИнформацияОСвязях.СостояниеПоДаннымЕГРН) Тогда
		...
		Если ИнформацияОСвязях.СостояниеПоДаннымЕГРН = 
			...
			Если ЭтоЮридическоеЛицо Тогда
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ka='Организация есть в базе ЕГРН';ru='Организация есть в базе ЕГРН';en='The organization is in the database USRRE'");
			Иначе
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ka='ИП есть в базе ЕГРН';ru='ИП есть в базе ЕГРН';en='IP is in the database USRRE'");
			КонецЕсли;
			ТабличныйДокумент.Вывести(ОбластиМакета.ДействующаяОрганизация);
		Иначе
			Если НЕ ЭтоЮридическоеЛицо 
				...
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация = 
					НСтр("ka='Не действует (по данным ЕГРН)';ru='Не действует (по данным ЕГРН)';en='Not Applicable (according USRRE)'");
			Иначе
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='%1 (по данным ЕГРН)';ru='%1 (по данным ЕГРН)';en='1% (according USRRE)'"),
...
Процедура ВывестиРазделРуководитель(ТабличныйДокумент, ИнформацияОСвязях, ПараметрыВывода)
	...
	Если ЭтоЮридическоеЛицо Тогда
		...
		Если ЗначениеЗаполнено(ИнформацияОСвязях.Телефон) Тогда
			Руководитель = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1, тел. %2';ru='%1, тел. %2';en='1% tel. 2%'"),
				Руководитель,
	...
	Если КоличествоОрганизаций > 0 Тогда
		...
		СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ЭтоЮридическоеЛицо, 
				НСтр("ka='Является руководителем еще в %1%2:';ru='Является руководителем еще в %1%2:';en='It is still the leader in% 1% 2:'"), 
				НСтр("ka='Является руководителем в %1%2:';ru='Является руководителем в %1%2:';en='It is the leader in% 1% 2:'")),
			СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	...
	Если КоличествоОрганизаций > 0 Тогда
		...
		СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ВыводитьЕще, 
				НСтр("ka='Является учредителем еще в %1%2:';ru='Является учредителем еще в %1%2:';en='Is the founder still in% 1% 2:'"), 
				НСтр("ka='Является учредителем в %1%2:';ru='Является учредителем в %1%2:';en='He is the founder in% 1% 2:'")),
			СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
				КоличествоОрганизаций, "организации,организациях, организациях"),
...
Процедура СформироватьРезультатДанныеПрограммы(ИнформацияОСвязях, ДанныеОтчета, ОписаниеДанныхПрограммы)
	...
	ДанныеОтчета.Вставить("ИмяФайлаДанныеПрограммы",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - данные программы';ru='Досье контрагента %1 - данные программы';en='counterparty dossier 1% - program data'"),
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			ИнформацияОСвязях.НаименованиеСокращенное, "")));
...
Процедура СформироватьРезультатБухгалтерскаяОтчетность(ИнформацияОСвязях, ДанныеОтчета)
	...
	ДанныеОтчета.Вставить("ИмяФайлаБухгалтерскаяОтчетность",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - бухгалтерская отчетность';ru='Досье контрагента %1 - бухгалтерская отчетность';en='Dossier counterparty 1% - accounting statements'"),
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			ИнформацияОСвязях.НаименованиеСокращенное, "")));
...
Функция ВалютаБухгалтерскойОтчетности(КодВалютыОтчетности)
	...
	Если КодВалютыОтчетности = "383" Тогда
		Возврат НСтр("ka='В рублях';ru='В рублях';en='In rubles'");
	ИначеЕсли КодВалютыОтчетности = "384" Тогда
		Возврат НСтр("ka='В тысячах рублей';ru='В тысячах рублей';en='Thousands of Russian Rubles'");
	Иначе // "385"
		Возврат НСтр("ka='В миллионах рублей';ru='В миллионах рублей';en='In millions of Russian Rubles'");
	КонецЕсли;
...
Процедура СформироватьРезультатАнализОтчетности(ИнформацияОСвязях, ДанныеОтчета)
	...
	ДанныеОтчета.Вставить("ИмяФайлаАнализОтчетности",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - анализ отчетности';ru='Досье контрагента %1 - анализ отчетности';en='counterparty dossier 1% - reporting analysis'"),
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			ИнформацияОСвязях.НаименованиеСокращенное, "")));
...
Процедура СформироватьРезультатФинансовыйАнализ(ИнформацияОСвязях, ДанныеОтчета)
	...
	Если ИнформацияОСвязях.ФинансовыйАнализ.Количество() = 0 Тогда
		...
		ОбластьШапка.Параметры.ТекстЗаголовка = НСтр("ka='Финансовый анализ';ru='Финансовый анализ';en='The financial analysis'");
		ТабличныйДокумент.Вывести(ОбластьШапка);
	...
	Иначе
		...
		ОбластьШапка.Параметры.ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Финансовый анализ за %1 год';ru='Финансовый анализ за %1 год';en='Financial analysis for% 1 year'"),
	...
	ДанныеОтчета.Вставить("ИмяФайлаФинансовыйАнализ",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - финансовый анализ';ru='Досье контрагента %1 - финансовый анализ';en='Dossier counterparty 1% - financial analysis'"),
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			ИнформацияОСвязях.НаименованиеСокращенное, "")));
...
Процедура ВывестиРазделРискБанкротства(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Индекс Альтмана Z = %1';ru='Индекс Альтмана Z = %1';en='Altman Index Z =% 1'"),
		ИндексАльтмана);
...
Процедура ВывестиРазделКредитоспособность(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Оценка кредитоспособности S = %1';ru='Оценка кредитоспособности S = %1';en='Credit Rating S =% 1'"),
		Кредитоспособность);
	Диаграмма.УстановитьЗначение(0, 0, ЗначениеТочкиДиаграммы, , ТекстТочкиДиаграммы);
	// Комментарии к диаграмме
...
Процедура ВывестиРазделРентабельностьАктивов(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Рентабельность активов %1';ru='Рентабельность активов %1';en='1% ROA'"),
	...
	Если ЗначениеЗаполнено(Показатели.ВидДеятельности) Тогда
		ПояснениеРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Средняя рентабельность приведена по данным Росстата для вида деятельности ""%1""';ru='Средняя рентабельность приведена по данным Росстата для вида деятельности ""%1""';en='The average profitability is given according to Rosstat data for activity ""%1""'"),
...
Процедура ВывестиРазделРентабельностьПродаж(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Рентабельность продаж %1';ru='Рентабельность продаж %1';en='Return on sales% 1'"),
	...
	Для каждого ДанныеГода Из ДанныеПоГодам Цикл
		...
		Если НомерГода = "1" Тогда
			...
			Если ОценкаРискБанкротства = "ПлохаяОценка" Тогда
				Показатели.ПояснениеОценкиРискБанкротства = НСтр("ka='Высокий риск банкротства';ru='Высокий риск банкротства';en='High risk of bankruptcy'");
			ИначеЕсли ОценкаРискБанкротства = "СредняяОценка" Тогда
				Показатели.ПояснениеОценкиРискБанкротства = НСтр("ka='Вероятность банкротства невысока';ru='Вероятность банкротства невысока';en='The probability of bankruptcy is low'");
			Иначе // "ХорошаяОценка"
				Показатели.ПояснениеОценкиРискБанкротства = НСтр("ka='Устойчивое финансовое состояние';ru='Устойчивое финансовое состояние';en='Stable financial condition'");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ДанныеГода Из ДанныеПоГодам Цикл
		...
		Если НомерГода = "1" Тогда
			...
			Если ОценкаКредитоспособность = "ПлохаяОценка" Тогда
				Показатели.ПояснениеОценкиКредитоспособность = НСтр("ka='Кредитование связано с повышенным риском';ru='Кредитование связано с повышенным риском';en='Lending associated with increased risk'");
			ИначеЕсли ОценкаКредитоспособность = "СредняяОценка" Тогда
				Показатели.ПояснениеОценкиКредитоспособность = НСтр("ka='Кредитование требует взвешенного подхода';ru='Кредитование требует взвешенного подхода';en='Lending requires a balanced approach'");
			Иначе // "ХорошаяОценка"
				Показатели.ПояснениеОценкиКредитоспособность = НСтр("ka='Кредитоспособность не вызывает сомнений';ru='Кредитоспособность не вызывает сомнений';en='Credibility is not in doubt'");
			КонецЕсли;
		КонецЕсли;
		...
		Если НомерГода = "1" Тогда
			...
			Если ОценкаРентабельностьАктивов = "ПлохаяОценка" Тогда
				Показатели.ПояснениеОценкиРентабельностьАктивов = ?(СредняяРентабельность = 0, 
					НСтр("ka='Ниже нуля';ru='Ниже нуля';en='Below zero'"), 
					НСтр("ka='Значительно ниже средней';ru='Значительно ниже средней';en='Well below the average'"));
			ИначеЕсли ОценкаРентабельностьАктивов = "СредняяОценка" Тогда
				Показатели.ПояснениеОценкиРентабельностьАктивов = ?(СредняяРентабельность = 0, 
					НСтр("ka='Равна нулю';ru='Равна нулю';en='zero'"), 
					НСтр("ka='Немного ниже средней';ru='Немного ниже средней';en='Slightly below average'"));
			Иначе // "ХорошаяОценка"
				Показатели.ПояснениеОценкиРентабельностьАктивов = ?(СредняяРентабельность = 0, 
					НСтр("ka='Выше нуля';ru='Выше нуля';en='Above zero'"), 
					НСтр("ka='Выше средней';ru='Выше средней';en='above the average'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьРезультатПроверкиКонтролирующимиОрганами(ИнформацияОПроверках, ДанныеОтчета)
		...
		Для каждого СтрокаПериода Из ТаблицаПериоды Цикл
			...
			Для каждого СтрокаПроверки Из ПроверкиПериода Цикл
				...
				Проверка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1, %2",
					...
					?(СтрокаПроверки.ПлановаяПроверка, НСтр("ka='плановая проверка';ru='плановая проверка';en='routine check'"), НСтр("ka='внеплановая проверка';ru='внеплановая проверка';en='unscheduled inspection'")));
	...
	ДанныеОтчета.Вставить("ИмяФайлаПроверки",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Досье контрагента %1 - проверки';ru='Досье контрагента %1 - проверки';en='counterparty dossiers% 1 - test'"),
		ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
...
Процедура ВывестиСтрокуЕще(ТабличныйДокумент, ОбластиМакета, ИмяДокумента, ИмяОбластиРасшифровки, КоличествоВсего)
	...
	Если КоличествоВсего = 0 Тогда
		ТекстЕще = НСтр("ka='კიდე';ru='Еще';en='Still'");
	Иначе
		ТекстЕще = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Еще %1';ru='Еще %1';en='1 more'"),
			КоличествоВсего - 1);
	КонецЕсли;
