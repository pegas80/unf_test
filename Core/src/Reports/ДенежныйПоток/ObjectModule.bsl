...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["Денежный поток"].Теги = НСТР("ka='მთავარი,ფული,შემოსვლა,გადახდები';ru='Главное,Деньги,Поступления,Платежи';en='The main thing is money, receipts, payments'");
...
Функция СформироватьОтчетДенежныйПоток(ПараметрыОтчета, ДанныеРасшифровки)
	...
	Для каждого СтрСчетКасса Из ИсходныеДанные.Строки Цикл
		...
		Если НЕ СтрОстатков=Неопределено И ЗначениеЗаполнено(СтрОстатков.КонечныйОстаток) Тогда
			Область = Макет.ПолучитьОбласть("Остаток|БезПериодов");
			КонецПериодаПредставление = ?(НЕ ПараметрыОтчета.Свойство("КонецПериода") ИЛИ ПараметрыОтчета.КонецПериода='0001-01-01' ИЛИ ПараметрыОтчета.КонецПериода='3999-12-31 23:59:59', НСтр("ka='მიმდინარე მომენტი';ru='текущий момент';en='this moment'"), Формат(ПараметрыОтчета.КонецПериода, "ДЛФ=D"));
...
Процедура УстановитьПризнакЭкв(Строка, Область)
	...
	Если НЕ Строка.Сумма=Строка.СуммаВал Тогда
		Область.Параметры.Экв = НСтр("ka='ექვ.';ru='экв.';en='eq.'");
	КонецЕсли; 
...
Функция СформироватьОтчетДенежныйПотокВДинамике(ПараметрыОтчета, ДанныеРасшифровки)
	...
	КонецЦикла; 
	Область = Макет.ПолучитьОбласть("Шапка|Период");
	Область.Параметры.Период = НСтр("ka='მთელი პერიოდი';ru='За весь период';en='Over the entire period'");
	Отчет.Присоединить(Область);
	...
	Для каждого СтрСчетКасса Из ИсходныеДанные.Строки Цикл
		...
		Область.Параметры.Дата = НСтр("ka='პერიოდის დასაწყისი';ru='начало периода';en='beginning of period'");
		Отчет.Вывести(Область);
		СуммаПредыдущая = 0;
		...
		// Конечные остатки
		Область = Макет.ПолучитьОбласть("Остаток|Основа");
		Область.Параметры.Дата = НСтр("ka='დასასრული პერიოდი';ru='конец периода';en='end of the period'");
		Отчет.Вывести(Область);
		СуммаПредыдущая = 0;
		СтруктураОтбораОстатков = Новый Структура;
		Если ПараметрыОтчета.ПоМестамХраненияДенег Тогда
...
Функция СформироватьРасшифровку(ПараметрыОтчета)
	...
	Для каждого СтрРасшифровка Из ИсходныеДанные.Строки Цикл
		...
		Иначе
			Область.Параметры.Валюта = НСтр("ka='экв.';ru='экв.';en='eq.'");
			Отчет.Вывести(Область);
