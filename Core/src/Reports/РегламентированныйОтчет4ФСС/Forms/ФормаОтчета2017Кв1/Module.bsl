...
Процедура ОбновитьПользовательскиеНастройки()
	...
	Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Изменилась версия формы отчета.%1Выполняется обновление пользовательских настроек.';ru='Изменилась версия формы отчета.%1Выполняется обновление пользовательских настроек.';en='Changed version of the report form.% 1Vypolnyaetsya update user preferences.'"), Символы.ПС);
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина) Экспорт
	...
	Если Открыта() Тогда
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения <> 2 Тогда
			...
			Сообщение.Текст = НСтр("ka='Перед выгрузкой необходимо установить точность ""2"".';ru='Перед выгрузкой необходимо установить точность ""2"".';en='you must set the ""2"" accuracy before unloading.'");
...
Функция ВыгрузкаВозможна(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НомерСтрахователя) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ka='Не задан номер страхователя.';ru='Не задан номер страхователя.';en='Do not set the number of the insured.'"),
			"Титульный", "ОргФССКод");
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодПодчиненности) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ka='Не задан код подчиненности.';ru='Не задан код подчиненности.';en='Not specified affiliation code.'"),
	...
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодПодчиненности) <> 5 Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ka='Неверно задан код подчиненности (должен состоять из 5 цифр и заканчиваться на 1, 2 или 3).';ru='Неверно задан код подчиненности (должен состоять из 5 цифр и заканчиваться на 1, 2 или 3).';en='Subordination code is invalid (must consist of 5 digits and end with 1, 2 or 3).'"),
			"Титульный", "КодПодчиненности");
	...
	Иначе
		...
		Если НЕ (ПоследнийСимвол = "1" ИЛИ ПоследнийСимвол = "2" ИЛИ ПоследнийСимвол = "3") Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				НСтр("ka='Код подчиненности должен заканчиваться на цифру 1, 2 или 3.';ru='Код подчиненности должен заканчиваться на цифру 1, 2 или 3.';en='subordination code must end with number 1, 2 or 3.'"),
				"Титульный", "КодПодчиненности");
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИНН)
	 ИЛИ ПараметрыВыгрузки.ИНН = "000000000000" Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ka='Не задан ИНН организации.';ru='Не задан ИНН организации.';en='Not specified INN organization.'"),
			"Титульный", "ИНН");
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(СтруктураРеквизитовФормы.Организация)
	   И СтрДлина(ПараметрыВыгрузки.КПП) <> 9 Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		НСтр("ka='Неверно задан КПП организации.';ru='Неверно задан КПП организации.';en='PPC is invalid organizations.'"),
		"Титульный", "КПП");
	...
	Если ПараметрыВыгрузки.НомерКорректировки <> 0 Тогда
		...
		Если ПараметрыВыгрузки.НомДопРасчета <> 0 Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				НСтр("ka='При номере корректировки, отличном от 000 вторая часть кода ""Отчетный период"" не должна заполняться.';ru='При номере корректировки, отличном от 000 вторая часть кода ""Отчетный период"" не должна заполняться.';en='When the room correction, other than the 000 the second part of the code ""Reporting period"" shall not be filled.'"),
	...
	Если ДанныеТаблица1["П000010000503"] = 0 Тогда
		...
		ЭтаФорма, НСтр("ka='В таблице 1, в строке 5 не указан размер страхового тарифа в соответствии с классом профессионального риска.';ru='В таблице 1, в строке 5 не указан размер страхового тарифа в соответствии с классом профессионального риска.';en='In Table 1, row 5 Unknown size insurance rate in accordance with the professional risk class.'"),
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяОбласти = "КодПодчиненности" Тогда
		Если СтрДлина(СокрЛП(Область.Значение)) <> 5 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Введенное значение кода подчиненности должно состоять из 5 символов.';ru='Введенное значение кода подчиненности должно состоять из 5 символов.';en='Permission value chain of command code must consist of 5 characters.'");
			Сообщение.Сообщить();
			Область.Значение = "";
		КонецЕсли;
	КонецЕсли;
