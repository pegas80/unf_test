...
Процедура МенеджерыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Представление",							НСтр("ka='менеджеры';ru='менеджеры'"));
...
Процедура ДиаграммаНовыеОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВРег(Расшифровка) = ВРег("Новые") Тогда
		ПризнакРасшифровки = НСтр("ka='Новые';ru='Новые'");
		ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Анализ базы контрагентов (расшифровка %1)';ru='Анализ базы контрагентов (расшифровка %1)'"),
			НСтр("ka='новых';ru='новых'"));
	ИначеЕсли ВРег(Расшифровка) = ВРег("Существующие") Тогда
		ПризнакРасшифровки = НСтр("ka='Существующие';ru='Существующие'");
		ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Анализ базы контрагентов (расшифровка %1)';ru='Анализ базы контрагентов (расшифровка %1)'"),
			НСтр("ka='существующих';ru='существующих'"));
	КонецЕсли;
	
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 контрагенты за период: с %2 по %3%4%5%6';ru='%1 контрагенты за период: с %2 по %3%4%5%6'"),
		ПризнакРасшифровки,
...
Процедура ДиаграммаНовыеПоПериодамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(ПараметрПериодичность) = ВРег("Неделя") ИЛИ ВРег(ПараметрПериодичность) = "კვირა" Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კვირა %1-დან %2-მდე';ru='неделю с %1 по %2'"),
	...
	ИначеЕсли ВРег(ПараметрПериодичность) = ВРег("Месяц") ИЛИ ВРег(ПараметрПериодичность) = "თვე" Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თვე %1-დან %2-მდე';ru='месяц с %1 по %2'"),
			Формат(Расшифровка, "ДФ=dd.MM.yyyy"),
			Формат(КонецМесяца(Расшифровка), "ДФ=dd.MM.yyyy"));
	КонецЕсли;
	
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ახალი კონტრაგენტები პერიოდისთვის %1 %2%3%4';ru='Новые контрагенты за %1 %2%3%4'"),
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='კონტრაგენტების ბაზის ანალიზი (ახლების გაშიფვრა პერიოდისთვის)';ru='Анализ базы контрагентов (расшифровка новых за период)'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ДиаграммаПоМенеджерамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Контрагенты менеджера: %1';ru='Контрагенты менеджера: %1'"),
		?(ТипЗнч(Расшифровка) = Тип("Массив"), "<сводно>", Расшифровка),
		?(ЗначениеЗаполнено(ОтборТег), ", с тегом: " + ОтборТег, ""),
		?(ЗначениеЗаполнено(ОтборСегмент), ", в сегменте: " + ОтборСегмент, ""));
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='Анализ базы контрагентов (расшифровка по менеджерам)';ru='Анализ базы контрагентов (расшифровка по менеджерам)'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ОбновитьДиаграммуНовые()
	...
	Иначе
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ka='ახლები: (პერიოდი არ არის მითითებული)';ru='Новых за: (период не указан)'");
	КонецЕсли;
...
Процедура ОбновитьДиаграммуПоМенеджерам()
	...
	Пока ВыборкаОтветственные.Следующий() Цикл
		...
		Если ПорядковыйНомерСерии < МаксимумСерийДиаграммы Тогда
			...
			ПредставлениеМенеджера = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ka='<Менеджер не указан>';ru='<Менеджер не указан>'"));
		...
		Иначе
			...
			ПредставлениеМенеджера = НСтр("ka='<Остальные>';ru='<Остальные>'");
			...
			ТекстПодробно = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ka='<Менеджер не указан>';ru='<Менеджер не указан>'"))
...
Процедура ВыборПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора) Экспорт
	...
	Если ПараметрыВыбора.Вариант = Неопределено Или ПараметрыВыбора.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ka='Относительный...';ru='Относительный...'"));
