...
Функция ПараметрыНекорректны()
	...
	Если Отчет.ДатаНачала > Отчет.ДатаОкончания Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Дата начала периода не должна превышать дату окончания.';ru='Дата начала периода не должна превышать дату окончания.';en='Date of commencement of the period must not exceed the expiry date.'"),, "Отчет.ДатаНачала");
		Результат = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Отчет.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана организация.';ru='Не указана организация.';en='Not specified organization.'"),, "Отчет.Организация");
		Результат = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Отчет.СтруктурнаяЕдиница) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана структурная единица.';ru='Не указана структурная единица.';en='Not indicated structural unit.'"),, "Отчет.СтруктурнаяЕдиница");
		Результат = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Отчет.СтруктурнаяЕдиница, "РозничныйВидЦен")) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='У структурной единицы не указан розничный вид цен.';ru='У структурной единицы не указан розничный вид цен.';en='In the structural units Unknown type of retail prices.'"),, "Отчет.СтруктурнаяЕдиница");
		Результат = Истина;
	КонецЕсли;
