...
Функция ПараметрыНекорректны()
	...
	Если Отчет.ДатаНачала > Отчет.ДатаОкончания Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Дата начала периода не должна превышать дату окончания.';ru='Дата начала периода не должна превышать дату окончания.'"),, "Отчет.ДатаНачала");
		Результат = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Отчет.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана организация.';ru='Не указана организация.'"),, "Отчет.Организация");
		Результат = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Отчет.СтруктурнаяЕдиница) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана структурная единица.';ru='Не указана структурная единица.'"),, "Отчет.СтруктурнаяЕдиница");
		Результат = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Отчет.СтруктурнаяЕдиница, "РозничныйВидЦен")) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='У структурной единицы не указан розничный вид цен.';ru='У структурной единицы не указан розничный вид цен.'"),, "Отчет.СтруктурнаяЕдиница");
		Результат = Истина;
	КонецЕсли;
