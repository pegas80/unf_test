...
Функция СформироватьОтчетТОРГ29(Знач ПараметрыОтчета) Экспорт
	...
	ОбластьОстатокНачала.Параметры.ДатаНачала = СтрШаблон(НСтр("ka='Остаток на %1';ru='Остаток на %1';en='The residue in 1%'"), Формат(ПараметрыОтчета.ДатаНачала, "ДЛФ=Д"));
...
Функция СформироватьВФоне(Знач ПараметрыЗадания, Знач АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Отчет Товарный отчет ТОРГ-29 (формирование).';ru='Отчет Товарный отчет ТОРГ-29 (формирование).';en='Report Trademark TORG-29 report (formation).'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПроверитьПараметрыОтчета(Знач ПараметрыОтчета)
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.ДатаНачала)
		...
		ВызватьИсключение НСтр("ka='Дата начала периода не должна превышать дату окончания.';ru='Дата начала периода не должна превышать дату окончания.';en='Date of commencement of the period must not exceed the expiry date.'");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыОтчета.СтруктурнаяЕдиница.РозничныйВидЦен) Тогда
		ВызватьИсключение НСтр("ka='У структурной единицы не указан розничный вид цен.';ru='У структурной единицы не указан розничный вид цен.';en='In the structural units Unknown type of retail prices.'");
	КонецЕсли;
...
Процедура ВывестиИтогиРасход(Знач Результат, Знач ТаблицаОстаткиИОбороты, Знач Макет, Знач ПараметрыОтчета, Знач КонОст)
	...
	ОбластьОстатокКонец.Параметры.ДатаКонца = СтрШаблон(НСтр("ka='Остаток на %1.';ru='Остаток на %1.';en='The residue in 1%.'"), Формат(ПараметрыОтчета.ДатаОкончания, "ДЛФ=Д"));
...
Функция ТаблицаИсходныеДанные(Знач ДатаНачала, Знач ДатаОкончания, Знач СтруктурнаяЕдиница, Знач НомерОтчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Процент <> ТекПроцент Тогда
			...
			Процент,
			СтрШаблон(НСтр("ka='Обработано %1%% исходных данных...';ru='Обработано %1%% исходных данных...';en='Processed% 1 %% initial data ...'"), Процент));
		КонецЕсли;
