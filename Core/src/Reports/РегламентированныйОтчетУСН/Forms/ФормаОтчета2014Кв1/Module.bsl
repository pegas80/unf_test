...
Процедура ОбновитьПользовательскиеНастройки()
	...
	Сообщение.Текст = НСтр("ka='Изменилась версия формы отчета. Выполняется обновление пользовательских настроек...';ru='Изменилась версия формы отчета. Выполняется обновление пользовательских настроек...'");
	Сообщение.Сообщить();
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяПоказателя = "ОКВЭД" Тогда
		...
		Если НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра)';ru='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра)'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
	...
	Если ИмяПоказателя = "П000210012003" Тогда
		...
		Если ЗначПоказателя > 6 Тогда
			Область.Значение = 6;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Значение ставки налога не может быть больше 6%';ru='Значение ставки налога не может быть больше 6%'");
			Сообщение.Сообщить();
		КонецЕсли;
	...
	Если ИмяПоказателя = "П000220026003" ИЛИ ИмяПоказателя = "П000220026103"
		...
		Если ЗначПоказателя < 5 И ЗначПоказателя <> 0 Тогда
			Область.Значение = 5;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Значение ставки налога не может быть меньше 5%';ru='Значение ставки налога не может быть меньше 5%'");
			Сообщение.Сообщить();
		ИначеЕсли ЗначПоказателя > 15 Тогда
			Область.Значение = 15;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Значение ставки налога не может быть больше 15%';ru='Значение ставки налога не может быть больше 15%'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если (ИмяПоказателя = "П000110001003"
	  ...
	  ИЛИ ИмяПоказателя = "П000120009003")
		...
		Если НЕ ДлинаЗначения = 8 И НЕ ДлинаЗначения = 11 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Код ОКТМО должен состоять из 8 или 11 цифр';ru='Код ОКТМО должен состоять из 8 или 11 цифр'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяПоказателя = "П000210010203" Тогда
		...
		Если НЕ (ЗначПоказателя = "1" ИЛИ ЗначПоказателя = "2") Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='В разделе 2.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';ru='В разделе 2.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2'"), Символы.ПС);
			Сообщение.Сообщить();
...
Процедура ИзмененитьФормуПриИзмененииСтавки(Форма, НовоеЗначение, СтароеЗначение)
	...
	ИначеЕсли НовоеЗначение = 6 И СтароеЗначение < 6 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Для ставки налога 6% используется форма, утвержденная приказом ФНС России от 04.07.2014 № ММВ-7-3/352@.';ru='Для ставки налога 6% используется форма, утвержденная приказом ФНС России от 04.07.2014 № ММВ-7-3/352@.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ПроверитьКС_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 1)';ru='Проверка раздела 1.1 (КС 1)'");
	...
	Нарушение       = НСтр("ka='п.3 ст. 346.21 НК РФ';ru='п.3 ст. 346.21 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';ru='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период'");
...
Процедура ПроверитьКС_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 2)';ru='Проверка раздела 1.1 (КС 2)'");
...
Процедура ПроверитьКС_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 3)';ru='Проверка раздела 1.2 (КС 3)'");
	...
	Нарушение       = НСтр("ka='п.4 ст. 346.21 НК РФ';ru='п.4 ст. 346.21 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';ru='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период'");
...
Процедура ПроверитьКС_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 4)';ru='Проверка раздела 1.2 (КС 4)'");
...
Процедура ПроверитьКС_5(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 5)';ru='Проверка раздела 1.1 (КС 5)'");
...
Процедура ПроверитьКС_6(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 6)';ru='Проверка раздела 1.1 (КС 6)'");
...
Процедура ПроверитьКС_7(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 7)';ru='Проверка раздела 1.2 (КС 7)'");
...
Процедура ПроверитьКС_8(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 8)';ru='Проверка раздела 1.2 (КС 8)'");
...
Процедура ПроверитьКС_9(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 9)';ru='Проверка раздела 1.2 (КС 9)'");
...
Процедура ПроверитьКС_10(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 10)';ru='Проверка раздела 1.2 (КС 10)'");
	...
	Нарушение      = НСтр("ka='п. 6 ст. 346.18 НК РФ';ru='п. 6 ст. 346.18 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Не исчислена сумма минимального налога за налоговый период';ru='Не исчислена сумма минимального налога за налоговый период'");
...
Процедура ПроверитьКС_12_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.1)';ru='Проверка раздела 2.2 (КС 12.1)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 260 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 260 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%'");
	
	Нарушение       = НСтр("ka='п.2 ст. 346.20 НК РФ';ru='п.2 ст. 346.20 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Неправильное применение налоговой ставки';ru='Неправильное применение налоговой ставки'");
	...
	ЭтоОшибка, ОписаниеОшибки1, Нарушение,
		НСтр("ka='Исключение: Налоговая ставка налога может быть понижена для отдельных категорий налогоплательщиков (пп.3, 4 ст.346.20)';ru='Исключение: Налоговая ставка налога может быть понижена для отдельных категорий налогоплательщиков (пп.3, 4 ст.346.20)'"));
...
Процедура ПроверитьКС_12_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.2)';ru='Проверка раздела 2.2 (КС 12.2)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 261 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 261 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%'");
...
Процедура ПроверитьКС_12_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.3)';ru='Проверка раздела 2.2 (КС 12.3)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 262 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 262 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%'");
...
Процедура ПроверитьКС_12_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.4)';ru='Проверка раздела 2.2 (КС 12.4)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 263 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 263 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%'");
...
Процедура ПроверитьКС_16_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.1)';ru='Проверка раздела 2.2 (КС 16.1)'");
	...
	Нарушение       = НСтр("ka='п. 2 и п. 7 ст. 346.18 НК РФ';ru='п. 2 и п. 7 ст. 346.18 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Неверно определена налоговая база за отчетный период';ru='Неверно определена налоговая база за отчетный период'");
...
Процедура ПроверитьКС_16_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.2)';ru='Проверка раздела 2.2 (КС 16.2)'");
...
Процедура ПроверитьКС_16_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.3)';ru='Проверка раздела 2.2 (КС 16.3)'");
...
Процедура ПроверитьКС_16_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.4)';ru='Проверка раздела 2.2 (КС 16.4)'");
...
Процедура ПроверитьКС_18_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.1)';ru='Проверка раздела 2.2 (КС 18.1)'");
	ОписаниеКС     = НСтр("ka='Сумма убытка, полученного за первый квартал (строка 250 Раздел 2.2) должна быть равна разнице значений по строкам 220 и 210 Раздела 2.2, если значение строки 210 Раздела 2.2 меньше значения по строке 220 Раздела 2.2';ru='Сумма убытка, полученного за первый квартал (строка 250 Раздел 2.2) должна быть равна разнице значений по строкам 220 и 210 Раздела 2.2, если значение строки 210 Раздела 2.2 меньше значения по строке 220 Раздела 2.2'");
...
Процедура ПроверитьКС_18_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.2)';ru='Проверка раздела 2.2 (КС 18.2)'");
	ОписаниеКС     = НСтр("ka='Сумма убытка, полученного за полугодие (строка 251 Раздел 2.2) должна быть равна разнице значений по строкам 221 и 211 Раздела 2.2, если значение строки 211 Раздела 2.2 меньше значения по строке 221 Раздела 2.2';ru='Сумма убытка, полученного за полугодие (строка 251 Раздел 2.2) должна быть равна разнице значений по строкам 221 и 211 Раздела 2.2, если значение строки 211 Раздела 2.2 меньше значения по строке 221 Раздела 2.2'");
...
Процедура ПроверитьКС_18_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.3)';ru='Проверка раздела 2.2 (КС 18.3)'");
...
Процедура ПроверитьКС_18_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.4)';ru='Проверка раздела 2.2 (КС 18.4)'");
...
Процедура ПроверитьКС_19_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.1)';ru='Проверка раздела 2.2 (КС 19.1)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного налога за первый квартал (строка 270 Раздела 2.2) должна быть равна произведению налоговой базы (строка 240 Раздел 2.2) и налоговой ставки (строка 260 Раздел 2.2) деленной на 100';ru='Сумма исчисленного налога за первый квартал (строка 270 Раздела 2.2) должна быть равна произведению налоговой базы (строка 240 Раздел 2.2) и налоговой ставки (строка 260 Раздел 2.2) деленной на 100'");
	
	Нарушение       = НСтр("ka='п. 1 ст. 246.21 НК РФ';ru='п. 1 ст. 246.21 НК РФ'");
	ОписаниеОшибки1 = НСтр("ka='Неверно исчислена сумма налога за налоговый период';ru='Неверно исчислена сумма налога за налоговый период'");
...
Процедура ПроверитьКС_19_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.2)';ru='Проверка раздела 2.2 (КС 19.2)'");
...
Процедура ПроверитьКС_19_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.3)';ru='Проверка раздела 2.2 (КС 19.3)'");
...
Процедура ПроверитьКС_19_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.4)';ru='Проверка раздела 2.2 (КС 19.4)'");
...
Процедура ПроверитьКС_21(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 21)';ru='Проверка раздела 2.2 (КС 21)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного минимального налога за налоговый период (строка 280 Раздел 2.2) должна быть равна 1% от значения по строке 213 Раздела 2.2';ru='Сумма исчисленного минимального налога за налоговый период (строка 280 Раздел 2.2) должна быть равна 1% от значения по строке 213 Раздела 2.2'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "Р11Стр020" Тогда // Раздел 1.1
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уплате по сроку не позднее 25-го апреля отчетного года';ru='Сумма авансового платежа к уплате по сроку не позднее 25-го апреля отчетного года'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр040" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уплате по сроку не позднее 25-го июля отчетного года';ru='Сумма авансового платежа к уплате по сроку не позднее 25-го июля отчетного года'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр050" Тогда
		Результат.Вставить("Имя", "П000110005003");
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уменьшению';ru='Сумма авансового платежа к уменьшению'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр070" Тогда
		Результат.Вставить("Имя", "П000110007003");
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уплате по сроку не позднее 25-го октября отчетного года';ru='Сумма авансового платежа к уплате по сроку не позднее 25-го октября отчетного года'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма налога, подлежащая доплате за налоговый период';ru='Сумма налога, подлежащая доплате за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр110" Тогда
		Результат.Вставить("Имя", "П000110011003");
		Результат.Вставить("Представление", НСтр("ka='Сумма налога к уменьшению';ru='Сумма налога к уменьшению'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р12Стр120" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма минимального налога, подлежащая уплате за налоговый период';ru='Сумма минимального налога, подлежащая уплате за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р21Стр133" Тогда // Раздел 2.1
		Результат.Вставить("Имя", "П000210013303");
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного налога за налоговый период';ru='Сумма исчисленного налога за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р21Стр143" Тогда
		Результат.Вставить("Имя", "П000210014303");
		Результат.Вставить("Представление", НСтр("ka='Строка 143 Раздел 2.1.';ru='Строка 143 Раздел 2.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр210" Тогда // Раздел 2.2
		Результат.Вставить("Имя", "П000220021003");
		Результат.Вставить("Представление", НСтр("ka='Сумма полученных доходов нарастающим итогом';ru='Сумма полученных доходов нарастающим итогом'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр220" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма произведенных расходов нарастающим итогом';ru='Сумма произведенных расходов нарастающим итогом'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр230" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма убытка, полученного в предыдущем налоговом периоде';ru='Сумма убытка, полученного в предыдущем налоговом периоде'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр240" Тогда
		Результат.Вставить("Имя", "П000220024003");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база для исчисления налога (авансового платежа по налогу)';ru='Налоговая база для исчисления налога (авансового платежа по налогу)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр250" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма полученного убытка за истекший налоговый (отчетный) период';ru='Сумма полученного убытка за истекший налоговый (отчетный) период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр260" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='გადასახადის განაკვეთი (%)';ru='Ставка налога (%)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр270" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного налога (авансового платежа по налогу)';ru='Сумма исчисленного налога (авансового платежа по налогу)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр280" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного минимального налога за налоговый период (ставка налога 1%)';ru='Сумма исчисленного минимального налога за налоговый период (ставка налога 1%)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
