...
Процедура ОбновитьПользовательскиеНастройки()
	...
	Сообщение.Текст = НСтр("ka='Изменилась версия формы отчета. Выполняется обновление пользовательских настроек...';ru='Изменилась версия формы отчета. Выполняется обновление пользовательских настроек...';en='Changed version of the report form. You upgrade user settings ...'");
	Сообщение.Сообщить();
...
Процедура ОткрытьФормуВыбораСтраницЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеАвторасчета <> СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
		...
		Если СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет = Ложь Тогда
			...
			ТекстВопроса = НСтр("ka='Будет произведен перерасчет всех рассчитываемых показателей. Продолжить?';ru='Будет произведен перерасчет всех рассчитываемых показателей. Продолжить?';en='It will be recalculated all the calculated indicators. Proceed?'");
...
Процедура ПроверитьТочность()
	...
	Если СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1 Тогда
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 2 Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1: при выводе сумм в рублях точность не может превышать 2 знака';ru='%1: при выводе сумм в рублях точность не может превышать 2 знака';en='% 1: the output sums in rubles accuracy can not exceed 2 signs'"), ЭтаФорма.Заголовок);
			Сообщение.Сообщить();
	...
	ИначеЕсли СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1000 Тогда
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 3 Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1: при выводе сумм в тысячах рублей точность не может превышать 3 знака';ru='%1: при выводе сумм в тысячах рублей точность не может превышать 3 знака';en='% 1: the derivation amounts in thousands rubles accuracy can not exceed 3 figures'"), ЭтаФорма.Заголовок);
			Сообщение.Сообщить();
	...
	ИначеЕсли СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1000000 Тогда
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 6 Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1: при выводе сумм в миллионах рублей точность не может превышать 6 знаков';ru='%1: при выводе сумм в миллионах рублей точность не может превышать 6 знаков';en='1%: the sum output RUB million accuracy can not exceed 6 characters'"), ЭтаФорма.Заголовок);
			Сообщение.Сообщить();
...
"более чем на %4. Исправить?';ka='Сумма страховых взносов, выплаченных работникам пособий по временной"
"нетрудоспособности и платежей (взносов) по договорам добровольного"
"личного страхования (нарастающим итогом) по код%1 стр. %2,"
"предусмотренных пунктом 3.1 статьи 346.21 Кодекса, уменьшает сумму"
"исчисленного налога (авансового платежа по налогу) по код%1 стр. %3"
...
Процедура ОбработкаВыбораИФНС()
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(КодИМНС) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Код налогового органа необходимо задать в справочнике ""Организации"".';ru='Код налогового органа необходимо задать в справочнике ""Организации"".';en='the tax authority code must be set in the ""Organization"".'");
		Сообщение.Сообщить();
		
	Иначе
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ინსპექციის ასარჩევად აუცილებელია შესაბამისი ჩანაწერის შეყვანა ცნობარ ""საგადასახადო ორგანოში რეგისტრაციები""-ში';ru='Для выбора инспекции, необходимо завести соответствующую запись в справочнике ""Регистрации в налоговом органе"".';en='To select the inspection, you must have a corresponding entry in the ""Registration with the tax authority.""'");
		Сообщение.Сообщить();
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяПоказателя = "ОКВЭД" Тогда
		...
		Если НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра)';ru='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра)';en='! On the front page is set to an invalid value code of activity OKVED% 1Dopustimy values ​​corresponding to the following patterns:% 2 ""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX .XX ""(X - any number)'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
	...
	Если ИмяПоказателя = "П000210012003" Тогда
		...
		Если ЗначПоказателя > 6 Тогда
			Область.Значение = 6;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Значение ставки налога не может быть больше 6%';ru='Значение ставки налога не может быть больше 6%';en='The value of the tax rate can not be more than 6%'");
			Сообщение.Сообщить();
		КонецЕсли;
	...
	Если ИмяПоказателя = "П000220026003" ИЛИ ИмяПоказателя = "П000220026103"
		...
		Если ЗначПоказателя < 5 И ЗначПоказателя <> 0 Тогда
			Область.Значение = 5;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Значение ставки налога не может быть меньше 5%';ru='Значение ставки налога не может быть меньше 5%';en='The value of the tax rate can not be less than 5%'");
			Сообщение.Сообщить();
		ИначеЕсли ЗначПоказателя > 15 Тогда
			Область.Значение = 15;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Значение ставки налога не может быть больше 15%';ru='Значение ставки налога не может быть больше 15%';en='The value of the tax rate can not be greater than 15%'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если (ИмяПоказателя = "П000110001003"
	  ...
	  ИЛИ ИмяПоказателя = "П000120009003")
		...
		Если НЕ ДлинаЗначения = 8 И НЕ ДлинаЗначения = 11 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Код ОКТМО должен состоять из 8 или 11 цифр';ru='Код ОКТМО должен состоять из 8 или 11 цифр';en='OKTMO code must consist of 8 or 11 digits'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяПоказателя = "П000210010203" Тогда
		...
		Если НЕ (ЗначПоказателя = "1" ИЛИ ЗначПоказателя = "2") Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='В разделе 2.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';ru='В разделе 2.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';en='Section 2.1 is given an incorrect value of the taxpayer sign!% 1Dopustimy 1 or 2'"), Символы.ПС);
			Сообщение.Сообщить();
...
Процедура ИзмененитьФормуПриИзмененииСтавки(Форма, НовоеЗначение, СтароеЗначение)
	...
	Если НовоеЗначение < 6 И СтароеЗначение = 6 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Для ставки налога отличной от 6% используется форма, рекомендованная письмом ФНС от 20.05.2015 № ГД-4-3/8533@. Внимание! Для этой формы не предусмотрена печать на бланках с двухмерным штрихкодом PDF417.';ru='Для ставки налога отличной от 6% используется форма, рекомендованная письмом ФНС от 20.05.2015 № ГД-4-3/8533@. Внимание! Для этой формы не предусмотрена печать на бланках с двухмерным штрихкодом PDF417.';en='For tax rates than the 6% use the form recommended by the Federal Tax Service Letter 5/20/2015 number DG-4-3 / 8533 @. Attention! For this form does not provide printing on letterhead from the two-dimensional bar code PDF417.'");
		Сообщение.Сообщить();
	ИначеЕсли НовоеЗначение = 6 И СтароеЗначение < 6 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Для ставки налога 6% используется форма, утвержденная приказом ФНС России от 04.07.2014 № ММВ-7-3/352@.';ru='Для ставки налога 6% используется форма, утвержденная приказом ФНС России от 04.07.2014 № ММВ-7-3/352@.';en='For tax rate of 6% using a form approved by the Russian Federal Tax Service Order of 7.4.2014 number MMV-7-3 / 352 @.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если СтруктураРеквизитовФормы.ЕдиницаИзмерения <> ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1 ИЛИ СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения <> 0 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Печать возможна только при установленной единице измерения ""в рублях"" и точности ""0"".';ru='Печать возможна только при установленной единице измерения ""в рублях"" и точности ""0"".';en='Printing is possible only when the unit of measure ""ruble"" and accuracy of ""0"".'");
		Сообщение.Сообщить();
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
			...
			Если РезультатПоиска = Неопределено Тогда
				...
				Если НЕ ЗначениеВыведено Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось заполнить область ""%1""!';ru='Не удалось заполнить область ""%1""!';en='Unable to fill the area ""%1""!'"), ИмяОбласти);
					Сообщение.Сообщить();
				КонецЕсли;
			Иначе
...
Процедура ПроверитьКС_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 1)';ru='Проверка раздела 1.1 (КС 1)';en='Checking section 1.1 (COP 1)'");
	ОписаниеКС     = НСтр("ka='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.1) должна быть равна (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1, если результат вычитания (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 больше либо равен 0';ru='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.1) должна быть равна (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1, если результат вычитания (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 больше либо равен 0';en='The amount of tax payable for the fiscal period (line 100 Section 1.1) should be equal to (line 133 - line 143), Section 2.1 - (line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.1, if the result of subtraction ( line 133 - line 143), Section 2.1 - (line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.1 is greater than or equal to 0'");
	
	Нарушение       = НСтр("ka='п.3 ст. 346.21 НК РФ';ru='п.3 ст. 346.21 НК РФ';en='3 article. 346.21 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';ru='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';en='Wrong is calculated the amount of tax payable for the tax period'");
...
Процедура ПроверитьКС_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 2)';ru='Проверка раздела 1.1 (КС 2)';en='1.1 Testing section (CS 2)'");
	ОписаниеКС     = НСтр("ka='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.1) должна быть равна 0, если (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1  < 0';ru='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.1) должна быть равна 0, если (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1  < 0';en='The amount of tax payable for the fiscal period (line 100 Section 1.1) must be equal to 0 if (line 133 - line 143) Section 2.1 - (line 020 + line 040 - line 050 + line 070 - line 080) Section 1.1 <0'");
...
Процедура ПроверитьКС_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 3)';ru='Проверка раздела 1.2 (КС 3)';en='1.2 Testing section (CS 3)'");
	ОписаниеКС     = НСтр("ka='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.2) должна быть равна строке 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2, если результат вычитания строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 больше либо равен 0 и строка 273 Раздела 2.2 больше либо равна строки 280 Раздела 2.2';ru='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.2) должна быть равна строке 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2, если результат вычитания строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 больше либо равен 0 и строка 273 Раздела 2.2 больше либо равна строки 280 Раздела 2.2';en='The amount of tax payable for the fiscal period (line 100 Section 1.2) must be equal to line 273 of Section 2.2 - (line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.2, if the result of subtracting line 273 Section 2.2 - ( line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.2 is greater than or equal to 0 and the line 273 of Section 2.2 is greater than or equal to the line 280, Section 2.2'");
	
	Нарушение       = НСтр("ka='п.4 ст. 346.21 НК РФ';ru='п.4 ст. 346.21 НК РФ';en='Claim 4 of Art. 346.21 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';ru='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';en='Wrong is calculated the amount of tax payable for the tax period'");
...
Процедура ПроверитьКС_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 4)';ru='Проверка раздела 1.2 (КС 4)';en='1.2 Testing section (CS 4)'");
	ОписаниеКС     = НСтр("ka='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздела 1.2) должна быть равна 0, если результат от вычитания (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 из строки 273 Раздела 2.2 < 0';ru='Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздела 1.2) должна быть равна 0, если результат от вычитания (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 из строки 273 Раздела 2.2 < 0';en='The amount of tax payable for the fiscal period (line 100 of Section 1.2) must be equal to 0 if the result of the subtraction (line 020 + line 040 - line 050 + line 070 - line 080) Section 1.2 of the line 273 of Section 2.2 <0'");
...
Процедура ПроверитьКС_5(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 5)';ru='Проверка раздела 1.1 (КС 5)';en='1.1 Testing section (CS 5)'");
	ОписаниеКС     = НСтр("ka='Сумма налога к уменьшению за налоговый период (строка 110 Раздела 1.1) должна быть равна (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 - (строка 133 - строка 143) Раздела 2.1, если результат вычитания (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 меньше 0';ru='Сумма налога к уменьшению за налоговый период (строка 110 Раздела 1.1) должна быть равна (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 - (строка 133 - строка 143) Раздела 2.1, если результат вычитания (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 меньше 0';en='Amount of tax reduction for the fiscal period (line 110 Section 1.1) should be equal to (row 020 + line 040 - line 050 + line 070 - line 080), Section 1.1 - (line 133 - line 143) of Section 2.1, if the result of subtraction (line 133 - line 143), Section 2.1 - (line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.1 is less than 0'");
	
	Нарушение       = НСтр("ka='п. 3 ст. 346.21 НК РФ';ru='п. 3 ст. 346.21 НК РФ';en='n. 3 tbsp. 346.21 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';ru='Неправильно исчислена сумма налога, подлежащая уплате за налоговый период';en='Wrong is calculated the amount of tax payable for the tax period'");
...
Процедура ПроверитьКС_6(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.1 (КС 6)';ru='Проверка раздела 1.1 (КС 6)';en='1.1 Testing section (CS 6)'");
	ОписаниеКС     = НСтр("ka='Сумма налога к уменьшению за налоговый период (строка 110 Раздела 1.1) должна быть равна 0, если результат вычитания значений (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 больше либо равен 0';ru='Сумма налога к уменьшению за налоговый период (строка 110 Раздела 1.1) должна быть равна 0, если результат вычитания значений (строка 133 - строка 143) Раздела 2.1 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.1 больше либо равен 0';en='Amount of tax reduction for the fiscal period (line 110 Section 1.1) should be equal to 0 if the result value of subtraction (line 133 - line 143), Section 2.1 - (line 020 + line 040 - line 050 + line 070 - line 080), Section 1.1 greater than or equal to 0'");
...
Процедура ПроверитьКС_7(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 7)';ru='Проверка раздела 1.2 (КС 7)';en='Checking section 1.2 (COP 7)'");
	ОписаниеКС     = НСтр("ka='Сумма налога к уменьшению за налоговый период (строка 110 Раздел 1.2) должна быть равна (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 - строка 273 Раздела 2.2, если строка 273 Раздела 2.2 ≥ строка 280 Раздела 2.2 и Строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 < 0';ru='Сумма налога к уменьшению за налоговый период (строка 110 Раздел 1.2) должна быть равна (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 - строка 273 Раздела 2.2, если строка 273 Раздела 2.2 ≥ строка 280 Раздела 2.2 и Строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 < 0';en='Amount of tax reduction for the fiscal period (line 110 Section 1.2) should be equal to (line 020 + line 040 - line 050 + line 070 - line 080) Section 1.2 - line 273 of Section 2.2, if Section 2.2 line 273 ≥ line 280 of Section 2.2 and line 273 Section 2.2 - (line 020 + line 040 - line 050 + line 070 - line 080) Section 1.2 <0'");
...
Процедура ПроверитьКС_8(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 8)';ru='Проверка раздела 1.2 (КС 8)';en='1.2 Testing section (CS 8)'");
	ОписаниеКС     = НСтр("ka='Сумма налога к уменьшению за налоговый период (строка 110 Раздел 1.2) должна быть равна (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 - строка 280 Раздела 2.2, если строка 273 Раздела 2.2 < строка 280 Раздела 2.2 и (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 > строки 280 Раздела 2.2';ru='Сумма налога к уменьшению за налоговый период (строка 110 Раздел 1.2) должна быть равна (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 - строка 280 Раздела 2.2, если строка 273 Раздела 2.2 < строка 280 Раздела 2.2 и (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 > строки 280 Раздела 2.2';en='Amount of tax reduction for the fiscal period (line 110 Section 1.2) should be equal to (line 020 + line 040 - line 050 + line 070 - line 080) Section 1.2 - line 280 of Section 2.2, if Section 2.2 line 273 <string 280 Section 2.2 and (line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.2> line 280, Section 2.2'");
...
Процедура ПроверитьКС_9(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 9)';ru='Проверка раздела 1.2 (КС 9)';en='Check section 1.2 (COP 9)'");
	ОписаниеКС     = НСтр("ka='Сумма налога к уменьшению за налоговый период (строка 110 Раздел 1.2) должна быть равна 0, если строка 273 Раздела 2.2 < строка 280 Раздела 2.2 и Строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 > 0 или строка 273 Раздела 2.2 > строка 280 Раздела 2.2 и (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 < строки 280 Раздела 2.2';ru='Сумма налога к уменьшению за налоговый период (строка 110 Раздел 1.2) должна быть равна 0, если строка 273 Раздела 2.2 < строка 280 Раздела 2.2 и Строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 > 0 или строка 273 Раздела 2.2 > строка 280 Раздела 2.2 и (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 < строки 280 Раздела 2.2';en='Amount of tax reduction for the fiscal period (line 110 Section 1.2) must be equal to 0 if the line 273 of Section 2.2 <line 280 of Section 2.2 and Line 273 Section 2.2 - (line 020 + line 040 - line 050 + line 070 - line 080) section 1.2> 0 or line 273 of section 2.2> string 280 and section 2.2 (line 020 + line 040 - line 050 + line 070 - line 080) section 1.2 <line 280 of section 2.2'");
...
Процедура ПроверитьКС_10(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 1.2 (КС 10)';ru='Проверка раздела 1.2 (КС 10)';en='1.2 Testing section (KS 10)'");
	ОписаниеКС     = НСтр("ka='Сумма минимального налога, подлежащая уплате за налоговый период (строка 120 Раздел 1.2) должна быть равна значению по строке 280 Раздел 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2, если значение по строке 280 Раздела 2.2 > строки 273 Раздела 2.2 и строка 280 Раздела 2.2 > (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2';ru='Сумма минимального налога, подлежащая уплате за налоговый период (строка 120 Раздел 1.2) должна быть равна значению по строке 280 Раздел 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2, если значение по строке 280 Раздела 2.2 > строки 273 Раздела 2.2 и строка 280 Раздела 2.2 > (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2';en='Amount minimum tax payable for fiscal period (line 120 Section 1.2) should be equal to the value on line 280 Section 2.2 - (line 020 + line 040 - line 050 + line 070 - line 080) of Section 1.2, if the value on line 280 Section 2.2> line 273, Section 2.2 and Section 2.2 line 280> (line 020 + line 040 - line 050 + line 070 - line 080), Section 1.2'");
	
	Нарушение      = НСтр("ka='п. 6 ст. 346.18 НК РФ';ru='п. 6 ст. 346.18 НК РФ';en='n. 6, Art. 346.18 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Не исчислена сумма минимального налога за налоговый период';ru='Не исчислена сумма минимального налога за налоговый период';en='the amount can not be numbered minimum tax for the tax period'");
...
Процедура ПроверитьКС_12_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.1)';ru='Проверка раздела 2.2 (КС 12.1)';en='Checking section 2.2 (COP 12.1)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 260 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 260 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';en='Tax rate (line 260 Section 2.2) should be greater than or equal to 5% but less than or equal to 15%'");
	
	Нарушение       = НСтр("ka='п.2 ст. 346.20 НК РФ';ru='п.2 ст. 346.20 НК РФ';en='Claim 2 Art. 346.20 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Неправильное применение налоговой ставки';ru='Неправильное применение налоговой ставки';en='Incorrect application of the tax rate'");
	...
	ЭтоОшибка, ОписаниеОшибки1, Нарушение,
		НСтр("ka='Исключение: Налоговая ставка налога может быть понижена для отдельных категорий налогоплательщиков (пп.3, 4 ст.346.20)';ru='Исключение: Налоговая ставка налога может быть понижена для отдельных категорий налогоплательщиков (пп.3, 4 ст.346.20)';en='Exception: tax rate may be lowered for certain categories of tax (claims 3, 4 st.346.20)'"));
...
Процедура ПроверитьКС_12_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.2)';ru='Проверка раздела 2.2 (КС 12.2)';en='Checking section 2.2 (COP 12.2)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 261 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 261 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';en='Tax rate (line 261 Section 2.2) should be greater than or equal to 5% but less than or equal to 15%'");
...
Процедура ПроверитьКС_12_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.3)';ru='Проверка раздела 2.2 (КС 12.3)';en='Checking section 2.2 (COP 12.3)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 262 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 262 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';en='Tax rate (line 262 Section 2.2) should be greater than or equal to 5% but less than or equal to 15%'");
...
Процедура ПроверитьКС_12_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 12.4)';ru='Проверка раздела 2.2 (КС 12.4)';en='Checking section 2.2 (CC 12.4)'");
	ОписаниеКС     = НСтр("ka='Ставка налога (строка 263 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';ru='Ставка налога (строка 263 Раздел 2.2) должна быть больше либо равна 5%, но меньше либо равна 15%';en='Tax rate (line 263 Section 2.2) should be greater than or equal to 5% but less than or equal to 15%'");
...
Процедура ПроверитьКС_16_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.1)';ru='Проверка раздела 2.2 (КС 16.1)';en='Check section 2.2 (COP 16.1)'");
	ОписаниеКС     = НСтр("ka='Налоговая база для исчисления налога за первый квартал (строка 240 Раздел 2.2) должна быть равна результату вычитания значения строки 220 Раздела 2.2 из величины по строке 210 Раздела 2.2, если результат этого вычитания больше нуля';ru='Налоговая база для исчисления налога за первый квартал (строка 240 Раздел 2.2) должна быть равна результату вычитания значения строки 220 Раздела 2.2 из величины по строке 210 Раздела 2.2, если результат этого вычитания больше нуля';en='Tax basis for calculating tax for the first quarter (line 240 Section 2.2) should be equal to the result of subtracting the line 220 of Section 2.2 of the value on a line 210, Section 2.2, if the result of this subtraction is greater than zero'");
	
	Нарушение       = НСтр("ka='п. 2 и п. 7 ст. 346.18 НК РФ';ru='п. 2 и п. 7 ст. 346.18 НК РФ';en='p. 2 and p. 7, Art. 346.18 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Неверно определена налоговая база за отчетный период';ru='Неверно определена налоговая база за отчетный период';en='Incorrectly determined the tax base for the reporting period'");
...
Процедура ПроверитьКС_16_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.2)';ru='Проверка раздела 2.2 (КС 16.2)';en='Checking section 2.2 (COP 16.2)'");
	ОписаниеКС     = НСтр("ka='Налоговая база для исчисления налога за полугодие (строка 241 Раздел 2.2) должна быть равна результату вычитания значения строки 221 Раздела 2.2 из величины по строке 211 Раздела 2.2, если результат этого вычитания больше 0';ru='Налоговая база для исчисления налога за полугодие (строка 241 Раздел 2.2) должна быть равна результату вычитания значения строки 221 Раздела 2.2 из величины по строке 211 Раздела 2.2, если результат этого вычитания больше 0';en='Tax basis for calculating tax for the half (line 241 Section 2.2) should be equal to the result of subtracting the line 221 of Section 2.2 of the value on a line 211, Section 2.2, if the result of this subtraction is greater than 0'");
...
Процедура ПроверитьКС_16_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.3)';ru='Проверка раздела 2.2 (КС 16.3)';en='Checking section 2.2 (COP 16.3)'");
	ОписаниеКС     = НСтр("ka='Налоговая база для исчисления налога за девять месяцев (строка 242 Раздел 2.2) должна быть равна результату вычитания значения строки 222 Раздела 2.2 из величины по строке 212 Раздела 2.2, если результат этого вычитания больше 0';ru='Налоговая база для исчисления налога за девять месяцев (строка 242 Раздел 2.2) должна быть равна результату вычитания значения строки 222 Раздела 2.2 из величины по строке 212 Раздела 2.2, если результат этого вычитания больше 0';en='Tax basis for calculating tax for the first nine months (line 242 Section 2.2) should be equal to the result of subtracting the line 222 of Section 2.2 of the value on a line 212, Section 2.2, if the result of this subtraction is greater than 0'");
...
Процедура ПроверитьКС_16_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 16.4)';ru='Проверка раздела 2.2 (КС 16.4)';en='Checking section 2.2 (COP 16.4)'");
	ОписаниеКС     = НСтр("ka='Налоговая база для исчисления налога за налоговый период (строка 243 Раздел 2.2) должна быть равна результату вычитания значения строки 223 и 230 Раздела 2.2 из величины по строке 213 Раздела 2.2, если результат этого вычитания больше 0';ru='Налоговая база для исчисления налога за налоговый период (строка 243 Раздел 2.2) должна быть равна результату вычитания значения строки 223 и 230 Раздела 2.2 из величины по строке 213 Раздела 2.2, если результат этого вычитания больше 0';en='Tax basis for calculating tax for the tax period (line 243 Section 2.2) should be equal to the result of subtracting line values ​​223 and 230 of Section 2.2 of the value on line 213, Section 2.2, if the result of this subtraction is greater than 0'");
...
Процедура ПроверитьКС_18_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.1)';ru='Проверка раздела 2.2 (КС 18.1)';en='Checking section 2.2 (COP 18.1)'");
	ОписаниеКС     = НСтр("ka='Сумма убытка, полученного за первый квартал (строка 250 Раздел 2.2) должна быть равна разнице значений по строкам 220 и 210 Раздела 2.2, если значение строки 210 Раздела 2.2 меньше значения по строке 220 Раздела 2.2';ru='Сумма убытка, полученного за первый квартал (строка 250 Раздел 2.2) должна быть равна разнице значений по строкам 220 и 210 Раздела 2.2, если значение строки 210 Раздела 2.2 меньше значения по строке 220 Раздела 2.2';en='The loss is obtained for the first quarter (line 250 Section 2.2) should be equal to the difference values ​​in the lines 220 and 210 of Section 2.2, if the value of line 210 is smaller than Section 2.2 on line 220, Section 2.2'");
...
Процедура ПроверитьКС_18_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.2)';ru='Проверка раздела 2.2 (КС 18.2)';en='Checking section 2.2 (COP 18.2)'");
	ОписаниеКС     = НСтр("ka='Сумма убытка, полученного за полугодие (строка 251 Раздел 2.2) должна быть равна разнице значений по строкам 221 и 211 Раздела 2.2, если значение строки 211 Раздела 2.2 меньше значения по строке 221 Раздела 2.2';ru='Сумма убытка, полученного за полугодие (строка 251 Раздел 2.2) должна быть равна разнице значений по строкам 221 и 211 Раздела 2.2, если значение строки 211 Раздела 2.2 меньше значения по строке 221 Раздела 2.2';en='Loss amount obtained for the half (line 251 Section 2.2) should be equal to the difference values ​​in the lines 221 and 211 of Section 2.2, if the value of line 211 is less than the Section 2.2 on line 221, Section 2.2'");
...
Процедура ПроверитьКС_18_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.3)';ru='Проверка раздела 2.2 (КС 18.3)';en='Checking section 2.2 (COP 18.3)'");
	ОписаниеКС     = НСтр("ka='Сумма убытка, полученного за девять месяцев (строка 252 Раздел 2.2) должна быть равна разнице значений по строкам 222 и 212 Раздела 2.2, если значение строки 212 Раздела 2.2 меньше значения по строке 222 Раздела 2.2';ru='Сумма убытка, полученного за девять месяцев (строка 252 Раздел 2.2) должна быть равна разнице значений по строкам 222 и 212 Раздела 2.2, если значение строки 212 Раздела 2.2 меньше значения по строке 222 Раздела 2.2';en='The amount of the loss incurred in the nine months (line 252 Section 2.2) should be equal to the difference values ​​in the lines 222 and 212 of Section 2.2, if the value of line 212 is less than the Section 2.2 on line 222, Section 2.2'");
...
Процедура ПроверитьКС_18_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 18.4)';ru='Проверка раздела 2.2 (КС 18.4)';en='Checking section 2.2 (COP 18.4)'");
	ОписаниеКС     = НСтр("ka='Сумма убытка, полученного за налоговый период (строка 253 Раздел 2.2) должна быть равна разнице значений по строкам 223 и 213 Раздела 2.2, если значение строки 213 Раздела 2.2 меньше значения по строке 223 Раздела 2.2';ru='Сумма убытка, полученного за налоговый период (строка 253 Раздел 2.2) должна быть равна разнице значений по строкам 223 и 213 Раздела 2.2, если значение строки 213 Раздела 2.2 меньше значения по строке 223 Раздела 2.2';en='loss amount obtained for the fiscal period (line 253 Section 2.2) should be equal to the difference values ​​in the lines 223 and 213 of Section 2.2, if the value of line 213 is less than the Section 2.2 on line 223, Section 2.2'");
...
Процедура ПроверитьКС_19_1(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.1)';ru='Проверка раздела 2.2 (КС 19.1)';en='Checking section 2.2 (COP 19.1)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного налога за первый квартал (строка 270 Раздела 2.2) должна быть равна произведению налоговой базы (строка 240 Раздел 2.2) и налоговой ставки (строка 260 Раздел 2.2) деленной на 100';ru='Сумма исчисленного налога за первый квартал (строка 270 Раздела 2.2) должна быть равна произведению налоговой базы (строка 240 Раздел 2.2) и налоговой ставки (строка 260 Раздел 2.2) деленной на 100';en='Amount calculated tax for the first quarter (line 270 Section 2.2) should be equal to the product of the fiscal base (line 240 Section 2.2) and the tax rate (line 260 Section 2.2) divided by 100'");
	
	Нарушение       = НСтр("ka='п. 1 ст. 246.21 НК РФ';ru='п. 1 ст. 246.21 НК РФ';en='n. 1 tbsp. 246.21 of the Tax Code'");
	ОписаниеОшибки1 = НСтр("ka='Неверно исчислена сумма налога за налоговый период';ru='Неверно исчислена сумма налога за налоговый период';en='Incorrect amount of tax is calculated for the tax period'");
...
Процедура ПроверитьКС_19_2(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.2)';ru='Проверка раздела 2.2 (КС 19.2)';en='Checking section 2.2 (COP 19.2)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного налога за полугодие (строка 271 Раздела 2.2) должна быть равна произведению налоговой базы (строка 241 Раздел 2.2) и налоговой ставки (строка 261 Раздел 2.2) деленой на 100';ru='Сумма исчисленного налога за полугодие (строка 271 Раздела 2.2) должна быть равна произведению налоговой базы (строка 241 Раздел 2.2) и налоговой ставки (строка 261 Раздел 2.2) деленой на 100';en='The amount of the estimated tax for the half year (line 271 of Section 2.2) must be equal to the product of the tax base (line 241 of Section 2.2) and the tax rate (line 261 Section 2.2) divided by 100'");
...
Процедура ПроверитьКС_19_3(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.3)';ru='Проверка раздела 2.2 (КС 19.3)';en='Checking section 2.2 (COP 19.3)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного налога за девять месяцев (строка 272 Раздела 2.2) должна быть равна произведению налоговой базы (строка 242 Раздел 2.2) и налоговой ставки (строка 262 Раздел 2.2) деленой на 100';ru='Сумма исчисленного налога за девять месяцев (строка 272 Раздела 2.2) должна быть равна произведению налоговой базы (строка 242 Раздел 2.2) и налоговой ставки (строка 262 Раздел 2.2) деленой на 100';en='The amount of the estimated tax for the first nine months (line 272 of Section 2.2) must be equal to the product of the tax base (line 242 Section 2.2) and the tax rate (line 262 Section 2.2) divided by 100'");
...
Процедура ПроверитьКС_19_4(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 19.4)';ru='Проверка раздела 2.2 (КС 19.4)';en='Checking section 2.2 (COP 19.4)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного налога за налоговый период (строка 273 Раздела 2.2) должна быть равна произведению налоговой базы (строка 243 Раздел 2.2) и налоговой ставки (строка 263 Раздел 2.2) деленой на 100';ru='Сумма исчисленного налога за налоговый период (строка 273 Раздела 2.2) должна быть равна произведению налоговой базы (строка 243 Раздел 2.2) и налоговой ставки (строка 263 Раздел 2.2) деленой на 100';en='The amount of the estimated tax for the tax period (line 273 of Section 2.2) must be equal to the product of the tax base (line 243 of Section 2.2) and the tax rate (line 263 Section 2.2) divided by 100'");
...
Процедура ПроверитьКС_21(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ka='Проверка раздела 2.2 (КС 21)';ru='Проверка раздела 2.2 (КС 21)';en='2.2 Testing section (KS 21)'");
	ОписаниеКС     = НСтр("ka='Сумма исчисленного минимального налога за налоговый период (строка 280 Раздел 2.2) должна быть равна 1% от значения по строке 213 Раздела 2.2';ru='Сумма исчисленного минимального налога за налоговый период (строка 280 Раздел 2.2) должна быть равна 1% от значения по строке 213 Раздела 2.2';en='Amount calculated minimum tax for the tax period (line 280 Section 2.2) should be equal to 1% of the value on line 213, Section 2.2'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "Р11Стр020" Тогда // Раздел 1.1
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уплате по сроку не позднее 25-го апреля отчетного года';ru='Сумма авансового платежа к уплате по сроку не позднее 25-го апреля отчетного года';en='The amount of the advance payment payable by due date no later than 25 April of the reporting year'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр040" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уплате по сроку не позднее 25-го июля отчетного года';ru='Сумма авансового платежа к уплате по сроку не позднее 25-го июля отчетного года';en='The amount of the advance payment payable by due date not later than the 25th of July of the reporting year'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр050" Тогда
		Результат.Вставить("Имя", "П000110005003");
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уменьшению';ru='Сумма авансового платежа к уменьшению';en='Down payment amount to a reduction in'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр070" Тогда
		Результат.Вставить("Имя", "П000110007003");
		Результат.Вставить("Представление", НСтр("ka='Сумма авансового платежа к уплате по сроку не позднее 25-го октября отчетного года';ru='Сумма авансового платежа к уплате по сроку не позднее 25-го октября отчетного года';en='The amount of the advance payment payable by due date no later than 25 October of the reporting year'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма налога, подлежащая доплате за налоговый период';ru='Сумма налога, подлежащая доплате за налоговый период';en='The amount of tax to be an extra charge for the tax period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р11Стр110" Тогда
		Результат.Вставить("Имя", "П000110011003");
		Результат.Вставить("Представление", НСтр("ka='Сумма налога к уменьшению';ru='Сумма налога к уменьшению';en='Amount of tax reduction'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р12Стр120" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма минимального налога, подлежащая уплате за налоговый период';ru='Сумма минимального налога, подлежащая уплате за налоговый период';en='The amount of the minimum tax payable for the tax period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р21Стр133" Тогда // Раздел 2.1
		Результат.Вставить("Имя", "П000210013303");
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного налога за налоговый период';ru='Сумма исчисленного налога за налоговый период';en='The amount of the estimated tax for the tax period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р21Стр143" Тогда
		Результат.Вставить("Имя", "П000210014303");
		Результат.Вставить("Представление", НСтр("ka='Строка 143 Раздел 2.1.';ru='Строка 143 Раздел 2.1.';en='String 143 Section 2.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр210" Тогда // Раздел 2.2
		Результат.Вставить("Имя", "П000220021003");
		Результат.Вставить("Представление", НСтр("ka='Сумма полученных доходов нарастающим итогом';ru='Сумма полученных доходов нарастающим итогом';en='Cumulative total of revenues'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр220" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма произведенных расходов нарастающим итогом';ru='Сумма произведенных расходов нарастающим итогом';en='The amount of produced a cumulative total costs'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр230" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма убытка, полученного в предыдущем налоговом периоде';ru='Сумма убытка, полученного в предыдущем налоговом периоде';en='loss amount obtained in the previous fiscal period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр240" Тогда
		Результат.Вставить("Имя", "П000220024003");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база для исчисления налога (авансового платежа по налогу)';ru='Налоговая база для исчисления налога (авансового платежа по налогу)';en='The tax base for the calculation of the tax (advance tax payment)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр250" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма полученного убытка за истекший налоговый (отчетный) период';ru='Сумма полученного убытка за истекший налоговый (отчетный) период';en='The amount of losses for the past tax (reporting) period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр260" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='გადასახადის განაკვეთი (%)';ru='Ставка налога (%)';en='Tax rate (%)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр270" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного налога (авансового платежа по налогу)';ru='Сумма исчисленного налога (авансового платежа по налогу)';en='The amount of the calculated tax (advance tax payment)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р22Стр280" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного минимального налога за налоговый период (ставка налога 1%)';ru='Сумма исчисленного минимального налога за налоговый период (ставка налога 1%)';en='Amount calculated minimum tax for the tax period (the rate of 1%)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
...
Процедура ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, ОписаниеКС, ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка,
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), Описание));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), Нарушение));
	...
	СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстовоеПредставлениеФормулы));
