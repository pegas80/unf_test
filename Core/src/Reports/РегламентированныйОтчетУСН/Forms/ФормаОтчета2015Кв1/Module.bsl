...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяПоказателя = "П002110010203" Тогда
		...
		Если НЕ (ЗначПоказателя = "1" ИЛИ ЗначПоказателя = "2") Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='В разделе 2.1.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';ru='В разделе 2.1.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';en='In section 2.1.1 of the taxpayer is given an incorrect value tag!% 1Dopustimy 1 or 2'"), Символы.ПС);
			Сообщение.Сообщить();
