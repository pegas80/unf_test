...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["ПоМестамХранения"].Теги = НСТР("ka='Главное,Товары,Запасы,Закупки,Номенклатура';ru='Главное,Товары,Запасы,Закупки,Номенклатура';en='Main Goods, Inventory, Procurement, Nomenclature'");
	НастройкиВариантов["ПоМестамХранения"].Рекомендуемый = Истина;
	НастройкиВариантов["ПоМестамХранения"].Вставить("РежимПериода", "НаДату");
	
	НастройкиВариантов["Прогноз"].Теги = НСТР("ka='Товары,Запасы,Закупки,Номенклатура,Заказы,План,Прогноз';ru='Товары,Запасы,Закупки,Номенклатура,Заказы,План,Прогноз';en='Products, Inventory, Purchasing, Inventory, Orders, plan, forecast'");
...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	СтрВидЦен.Заголовок = НСтр("ka='ფასების ტიპი';ru='Вид цен';en='Price View'");
	...
	Группа.Заголовок = НСтр("ka='რეჟიმი';ru='Режим';en='Mode'");
	...
	СтрСвой = ДобавитьПоле(Форма.ПоляНастроек, "Фильтр", "Режим", ОписаниеТиповБулево, НСтр("ka='რეჟიმი';ru='Режим';en='Mode'"), "Флажок");
	СтрСвой.Создан = Истина;
	СтрСвой.ИмяГруппы = Группа.Имя;
	СтрСвой.Поле = "Свой";
	ИмяРеквизита = "ФильтрРежимСвои";
	Если ИменаРеквизитов.Найти(ИмяРеквизита)=Неопределено Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизита, СтрСвой.ТипЗначения,, НСтр("ka='საკუთარი';ru='Свои';en='Their'")));
	КонецЕсли;
	...
	Если ИменаРеквизитов.Найти(ИмяРеквизита)=Неопределено Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизита, СтрСвой.ТипЗначения,, НСтр("ka='სხვისი';ru='Чужие';en='Aliens'")));
	КонецЕсли; 
	...
	СтрНаСкладе = ДобавитьПоле(Форма.ПоляНастроек, "Фильтр", "НаСкладе", ОписаниетиповБулево, НСтр("ka='რეჟიმი';ru='Режим';en='Mode'"), "Флажок");
	СтрНаСкладе.Создан = Истина;
	СтрНаСкладе.ИмяГруппы = Группа.Имя;
	ИмяРеквизита = "ФильтрРежимНаСкладах";
	Если ИменаРеквизитов.Найти(ИмяРеквизита)=Неопределено Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизита, СтрНаСкладе.ТипЗначения,, НСтр("ka='საწყობებში';ru='На складах';en='In warehouses'")));
	КонецЕсли; 
	...
	Если ИменаРеквизитов.Найти(ИмяРеквизита)=Неопределено Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизита, СтрНаСкладе.ТипЗначения,, НСтр("ka='რეალიზატორებთან';ru='У реализаторов';en='In implementers'")));
	КонецЕсли; 
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Параметр1 = КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти(Новый ПараметрКомпоновкиДанных("Параметр1"));
    Параметр1.Значение = Период;  
