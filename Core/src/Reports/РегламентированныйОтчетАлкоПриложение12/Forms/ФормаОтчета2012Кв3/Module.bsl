...
Процедура ПроверитьВыгрузкуОтчета()
	...
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.Организация) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Организация не выбрана! Проверка выгрузки невозможна.';ru='Организация не выбрана! Проверка выгрузки невозможна.';en='The organization is not selected! discharge check is not possible.'");
		Сообщение.Сообщить();
...
Процедура ОчисткаОтМусораПослеСохраненияНаКлиенте(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе	
	    // отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение( , НСтр("ka='Отчет не сохранен, продолжение очистки невозможно!';ru='Отчет не сохранен, продолжение очистки невозможно!';en='Report not saved, continued treatment is not possible!'"));
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если НЕ ФормаДлительнойОперации = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='При выполнении длительной операции переход из формы сообщений об ошибках невозможен!';ru='При выполнении длительной операции переход из формы сообщений об ошибках невозможен!';en='When performing a lengthy operation the transition from the form of error messages is not possible!'");
		Сообщение.Сообщить();
	    Возврат;
