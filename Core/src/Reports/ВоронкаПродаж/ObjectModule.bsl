...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["ВоронкаПродаж"].Теги = НСтр("ka='CRM';ru='CRM';en='CRM'");
...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Форма.Элементы.ГруппаПараметрыЭлементы.Заголовок = НСтр("ka='გაყიდვების ძაბრის სახეობა';ru='Вариант воронки';en='funnel option'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") Тогда
		СтрВидОперации = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидОперации", Новый ОписаниеТипов("Булево"), НСтр("ka='ოპერაციის ტიპი';ru='Вид операции';en='Type of transaction'"), "Флажок");
		СтрВидОперации.Создан = Истина;
		СтрВидОперации.ИмяГруппы = ГруппаФильтрВидОперацииВидЗаказа.Имя;
		СтрВидОперации.Поле = "ВидОперации";
		
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЗаказыПокупателей", НСтр("ka='მყიდველთა შეკვეთები';ru='Заказы покупателей';en='Customer orders'"), СтрВидОперации.ТипЗначения, СтрВидОперации, ИменаРеквизитов);
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЗаказНаряды", НСтр("ka='სერვისის დოკუმენტები';ru='Заказ-наряды';en='Work orders'"), СтрВидОперации.ТипЗначения, СтрВидОперации, ИменаРеквизитов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Или
		...
		СтрВидЗаказа = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидЗаказа", Новый ОписаниеТипов, НСтр("ka='შეკვეთის ტიპი';ru='Вид заказа';en='order Type'"), "Поле");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказаПокупателя", НСтр("ka='Вид заказа покупателя';ru='Вид заказа покупателя';en=""Type of buyer's order"""), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказовПокупателей"), СтрВидЗаказа, ИменаРеквизитов);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказНаряда", НСтр("ka='შეკვეთის-განწესის სახეობა';ru='Вид заказ-наряда';en='View purchase order'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказНарядов"), СтрВидЗаказа, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухПокупателейВСправочникеКонтрагенты() Тогда
		СтрКонтрагент = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"), НСтр("ka='მყიდველი';ru='Покупатель';en='Customer'"), "Поле");
		СтрКонтрагент.НестандартныйОбработчик = Ложь;
		СтрКонтрагент.Создан = Истина;
		СтрКонтрагент.Поле = "Контрагент";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрПокупатель", НСтр("ka='მყიდველი';ru='Покупатель';en='Customer'"), СтрКонтрагент.ТипЗначения, СтрКонтрагент, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухЭлементовВСправочникеСотрудники() Тогда
		СтрОтветственный = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Ответственный", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"), НСтр("ka='მენეჯერი';ru='Менеджер';en='Manager'"), "Поле");
		СтрОтветственный.НестандартныйОбработчик = Ложь;
		СтрОтветственный.Создан = Истина;
		СтрОтветственный.Поле = "Ответственный";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрМенеджер", НСтр("ka='მენეჯერი';ru='Менеджер';en='Manager'"), СтрОтветственный.ТипЗначения, СтрОтветственный, ИменаРеквизитов);
	КонецЕсли;
	...
	Если Не МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() 
		...
		СтрСтруктурнаяЕдиницаПродажи = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "СтруктурнаяЕдиницаПродажи", Новый ОписаниеТипов("СправочникСсылка.СтруктурныеЕдиницы"), НСтр("ka='ქვეგანყოფილება';ru='Подразделение';en='Subdivision'"), "Поле");
		...
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрПодразделение", НСтр("ka='ქვეგანყოფილება';ru='Подразделение';en='Subdivision'"), СтрСтруктурнаяЕдиницаПродажи.ТипЗначения, СтрСтруктурнаяЕдиницаПродажи, ИменаРеквизитов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказаПокупателя", НСтр("ka='Вид заказа покупателя';ru='Вид заказа покупателя';en=""Type of buyer's order"""), СтрВидЗаказа, Форма);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказНаряда", НСтр("ka='შეკვეთის-განწესის სახეობა';ru='Вид заказ-наряда';en='View purchase order'"), СтрВидЗаказа, Форма);
	КонецЕсли;
...
Функция ТекстСообщенияТребуетсяФильтр(ВариантВоронки, ПолеДляОбязательногоФильтра, НастройкиКД)
	...
	Если ВариантВоронки = ВариантВоронкиПоПокупателям() И МенееДвухПокупателейВСправочникеКонтрагенты() Тогда
		Возврат НСтр("ka='მყიდველების მიხედვით გაყიდვების ძაბრის ფორმირებისთვის აუცილებელია, რომ პროგრამაში არსებობდეს 1-ზე მეტი კონტრაგენტის ცნობარი.';ru='Для формирования воронки по покупателям требуется, чтобы в справочнике ""Контрагенты"" было не менее двух покупателей.';en='of customers required for the formation of the funnel to in the ""Accounts"" had at least two customers.'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоМенеджерам() И МенееДвухЭлементовВСправочникеСотрудники() Тогда
		Возврат НСтр("ka='გაყიდვების მენეჯერების მიხედვით გაყიდვების ძაბრის ფორმირებისთვის აუცილებელია, რომ პროგრამაში არსებობდეს 1-ზე მეტი თანამშრომლის ცნობარი.';ru='Для формирования воронки по менеджерам требуется, чтобы в справочнике ""Сотрудники"" было не менее двух элементов.';en='by managers required for the formation of a funnel, in reference to ""staff"" was at least two elements.'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоПодразделениям() И МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() Тогда
		Возврат НСтр("ka='ქვეგანყოფილებების მიხედვით გაყიდვების ძაბრის ფორმირებისთვის აუცილებელია, რომ პროგრამაში არსებობდეს 1-ზე მეტი ქვეგანყოფილების ცნობარი.';ru='Для формирования воронки по подразделениям требуется, чтобы в справочнике ""Подразделения"" было не менее двух элементов.';en='For the formation of funnel organizations need to in the ""Units"" have been at least two elements.'");
	КонецЕсли;
	
	Возврат СтрШаблон(НСтр("ka='ანგარიშგების ფორმირებისთვის %1 დააყენეთ ფილტრი %2.';ru='Для формирования воронки %1 установите фильтр %2.';en='To form funnels% 1% 2 filter set.'"),
...
Функция ПредставлениеГруппировки(ВариантВоронки, Выборка)
	...
	Если ВариантВоронки = ВариантВоронкиПоПокупателям() Тогда
		ЗаголовокПоля = НСтр("ka='მყიდველი';ru='Покупатель';en='Customer'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоМенеджерам() Тогда
		ЗаголовокПоля = НСтр("ka='მენეჯერი';ru='Менеджер';en='Manager'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоПодразделениям() Тогда
		ЗаголовокПоля = НСтр("ka='ქვეგანყოფილება';ru='Подразделение';en='Subdivision'");
	КонецЕсли;
...
Функция ПредставлениеВидЗаказа(ВидЗаказа)
	...
	Если ТипЗнч(ВидЗаказа) = Тип("СправочникСсылка.ВидыЗаказовПокупателей") Тогда
		...
		Иначе
			Возврат НСтр("ka='მყიდველთა შეკვეთები';ru='Заказы покупателей';en='Customer orders'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(ВидЗаказа) = Тип("СправочникСсылка.ВидыЗаказНарядов") Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
			Возврат  СтрШаблон("%1: %2", ТипЗнч(ВидЗаказа), ВидЗаказа);
		Иначе
			Возврат НСтр("ka='სერვისის დოკუმენტები';ru='Заказ-наряды';en='Work orders'");
		КонецЕсли;
	КонецЕсли;
