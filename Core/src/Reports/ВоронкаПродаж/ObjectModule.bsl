...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["ВоронкаПродаж"].Теги = НСтр("ka='CRM';ru='CRM'");
...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Форма.Элементы.ГруппаПараметрыЭлементы.Заголовок = НСтр("ka='გაყიდვების ძაბრის სახეობა';ru='Вариант воронки'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") Тогда
		СтрВидОперации = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидОперации", Новый ОписаниеТипов("Булево"), НСтр("ka='ოპერაციის ტიპი';ru='Вид операции'"), "Флажок");
		СтрВидОперации.Создан = Истина;
		СтрВидОперации.ИмяГруппы = ГруппаФильтрВидОперацииВидЗаказа.Имя;
		СтрВидОперации.Поле = "ВидОперации";
		
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЗаказыПокупателей", НСтр("ka='მყიდველთა შეკვეთები';ru='Заказы покупателей'"), СтрВидОперации.ТипЗначения, СтрВидОперации, ИменаРеквизитов);
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЗаказНаряды", НСтр("ka='შეკვეთა-განწესები';ru='Заказ-наряды'"), СтрВидОперации.ТипЗначения, СтрВидОперации, ИменаРеквизитов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Или
		...
		СтрВидЗаказа = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидЗаказа", Новый ОписаниеТипов, НСтр("ka='შეკვეთის ტიპი';ru='Вид заказа'"), "Поле");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказаПокупателя", НСтр("ka='Вид заказа покупателя';ru='Вид заказа покупателя'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказовПокупателей"), СтрВидЗаказа, ИменаРеквизитов);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказНаряда", НСтр("ka='შეკვეთის-განწესის სახეობა';ru='Вид заказ-наряда'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказНарядов"), СтрВидЗаказа, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухПокупателейВСправочникеКонтрагенты() Тогда
		СтрКонтрагент = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"), НСтр("ka='მყიდველი';ru='Покупатель'"), "Поле");
		СтрКонтрагент.НестандартныйОбработчик = Ложь;
		СтрКонтрагент.Создан = Истина;
		СтрКонтрагент.Поле = "Контрагент";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрПокупатель", НСтр("ka='მყიდველი';ru='Покупатель'"), СтрКонтрагент.ТипЗначения, СтрКонтрагент, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухЭлементовВСправочникеСотрудники() Тогда
		СтрОтветственный = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Ответственный", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"), НСтр("ka='მენეჯერი';ru='Менеджер'"), "Поле");
		СтрОтветственный.НестандартныйОбработчик = Ложь;
		СтрОтветственный.Создан = Истина;
		СтрОтветственный.Поле = "Ответственный";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрМенеджер", НСтр("ka='მენეჯერი';ru='Менеджер'"), СтрОтветственный.ТипЗначения, СтрОтветственный, ИменаРеквизитов);
	КонецЕсли;
	...
	Если Не МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() 
		...
		СтрСтруктурнаяЕдиницаПродажи = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "СтруктурнаяЕдиницаПродажи", Новый ОписаниеТипов("СправочникСсылка.СтруктурныеЕдиницы"), НСтр("ka='ქვეგანყოფილება';ru='Подразделение'"), "Поле");
		...
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрПодразделение", НСтр("ka='ქვეგანყოფილება';ru='Подразделение'"), СтрСтруктурнаяЕдиницаПродажи.ТипЗначения, СтрСтруктурнаяЕдиницаПродажи, ИменаРеквизитов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказаПокупателя", НСтр("ka='Вид заказа покупателя';ru='Вид заказа покупателя'"), СтрВидЗаказа, Форма);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказНаряда", НСтр("ka='შეკვეთის-განწესის სახეობა';ru='Вид заказ-наряда'"), СтрВидЗаказа, Форма);
	КонецЕсли;
...
Функция ТекстСообщенияТребуетсяФильтр(ВариантВоронки, ПолеДляОбязательногоФильтра, НастройкиКД)
	...
	Если ВариантВоронки = ВариантВоронкиПоПокупателям() И МенееДвухПокупателейВСправочникеКонтрагенты() Тогда
		Возврат НСтр("ka='მყიდველების მიხედვით გაყიდვების ძაბრის ფორმირებისთვის აუცილებელია, რომ პროგრამაში არსებობდეს 1-ზე მეტი კონტრაგენტის ცნობარი.';ru='Для формирования воронки по покупателям требуется, чтобы в справочнике ""Контрагенты"" было не менее двух покупателей.'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоМенеджерам() И МенееДвухЭлементовВСправочникеСотрудники() Тогда
		Возврат НСтр("ka='გაყიდვების მენეჯერების მიხედვით გაყიდვების ძაბრის ფორმირებისთვის აუცილებელია, რომ პროგრამაში არსებობდეს 1-ზე მეტი თანამშრომლის ცნობარი.';ru='Для формирования воронки по менеджерам требуется, чтобы в справочнике ""Сотрудники"" было не менее двух элементов.'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоПодразделениям() И МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() Тогда
		Возврат НСтр("ka='ქვეგანყოფილებების მიხედვით გაყიდვების ძაბრის ფორმირებისთვის აუცილებელია, რომ პროგრამაში არსებობდეს 1-ზე მეტი ქვეგანყოფილების ცნობარი.';ru='Для формирования воронки по подразделениям требуется, чтобы в справочнике ""Подразделения"" было не менее двух элементов.'");
	КонецЕсли;
	
	Возврат СтрШаблон(НСтр("ka='ანგარიშგების ფორმირებისთვის %1 დააყენეთ ფილტრი %2.';ru='Для формирования воронки %1 установите фильтр %2.'"),
...
Функция ПредставлениеГруппировки(ВариантВоронки, Выборка)
	...
	Если ВариантВоронки = ВариантВоронкиПоПокупателям() Тогда
		ЗаголовокПоля = НСтр("ka='მყიდველი';ru='Покупатель'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоМенеджерам() Тогда
		ЗаголовокПоля = НСтр("ka='მენეჯერი';ru='Менеджер'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоПодразделениям() Тогда
		ЗаголовокПоля = НСтр("ka='ქვეგანყოფილება';ru='Подразделение'");
	КонецЕсли;
...
Функция ПредставлениеВидЗаказа(ВидЗаказа)
	...
	Если ТипЗнч(ВидЗаказа) = Тип("СправочникСсылка.ВидыЗаказовПокупателей") Тогда
		...
		Иначе
			Возврат НСтр("ka='მყიდველთა შეკვეთები';ru='Заказы покупателей'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(ВидЗаказа) = Тип("СправочникСсылка.ВидыЗаказНарядов") Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
			Возврат  СтрШаблон("%1: %2", ТипЗнч(ВидЗаказа), ВидЗаказа);
		Иначе
			Возврат НСтр("ka='შეკვეთა-განწესები';ru='Заказ-наряды'");
		КонецЕсли;
	КонецЕсли;
