...
"в программе (насколько интенсивно и с какими объектами работают пользователи).';ka='Позволяет выполнять мониторинг активности пользователей"
...
"с какими объектами работал пользователь в программе.';ka='Подробная информация о том,"
"с какими объектами работал пользователь в программе.'");
	...
	НастройкиВарианта.Описание = НСтр("ka='Список критичных записей журнала регистрации.';ru='Список критичных записей журнала регистрации.';en='List of critical log entries.'");
	...
	НастройкиВарианта.Описание = НСтр("ka='Выводит график выполнения регламентных заданий в программе.';ru='Выводит график выполнения регламентных заданий в программе.';en='Displays the schedule for implementation of scheduled jobs in the program.'");
...
Функция СтрокаНеделяГода(ДатаГода)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Неделя %1';ru='Неделя %1';en='% 1 Week'"), НеделяГода(ДатаГода));
КонецФункции
	...
	Для Каждого Элемент Из МаксМассивПользователей Цикл
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%2)';ru='%1 (%2)';en='%12)'"),
					Элемент,
...
Функция СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения) Экспорт
	...
	Если ВыводитьОтбор.Значение = ТипВыводаТекстаКомпоновкиДанных.Выводить
		...
		Если МинимальнаяПродолжительностьСеансовРегламентныхЗаданий > 0 Тогда
			РежимОтображенияИнтервалов = НСтр("ka='გამორთულია';ru='Отключено';en='disabled'");
		Иначе
			РежимОтображенияИнтервалов = НСтр("ka='ჩართულია';ru='Включено';en='included'");
		КонецЕсли;
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (сеанс %2)';ru='%1 (сеанс %2)';en='% 1 (session 2%)'"),
						Элемент,
		...
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%2 из %3)';ru='%1 (%2 из %3)';en='1% (2% from 3%)'"),
						Точка.Значение,
				...
				ИначеЕсли Не ЗначениеЗаполнено(СтрокаРегламентныеЗадания.СобытиеМетаданные) Тогда
					ТочкаНазвание = НСтр("ka='ფონური დავალებები';ru='Фоновые задания';en='background jobs'");
					Точка = ДиаграммаГанта.УстановитьТочку(ТочкаНазвание);
					ОбщаяПродолжительностьРЗ = 0;
				КонецЕсли;
			КонецЕсли;
			...
			Интервал.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 - %2';ru='%1 - %2';en='%12'"),
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл 
		...
		Если ТекущаяДата <> ДатаОдновременноРегламентныхЗаданий Тогда
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 - %2';ru='%1 - %2';en='%12'"),
...
Функция РасшифровкаРегламентногоЗадания(Расшифровка) Экспорт
	...
	Область = Макет.ПолучитьОбласть("Таблица");
	Область.Параметры.ТипЗадания = НСтр("ka='Регламентное';ru='Регламентное';en='routine'");
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли ПродолжительностьРЗ <= 60 Тогда
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წამი';ru='%1 сек';en='% 1 seconds'"), ПродолжительностьРЗ);
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 мин %2 сек';ru='%1 мин %2 сек';en='% 1% 2 min sec'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 ч %2 мин';ru='%1 ч %2 мин';en='% 1% 2 h min'"), ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
