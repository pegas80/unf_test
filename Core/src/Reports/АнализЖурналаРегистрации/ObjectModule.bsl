...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВариантОтчета = "ДиаграммаГанта" Тогда
		...
		Если Не ЗначениеЗаполнено(ПериодДень.Дата) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнено значение поля День.';ru='Не заполнено значение поля День.';en='It is not filled Field Day value.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		И КонецВыборки.Использование Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Значение начала периода не может быть больше значения конца.';ru='Значение начала периода не может быть больше значения конца.';en='The value of the start period can not be more than the end value.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АктивностьПользователя" Тогда
		...
		Если Не ЗначениеЗаполнено(Пользователь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнено значение поля Пользователь.';ru='Не заполнено значение поля Пользователь.';en='Is not filled field User.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если Отчеты.АнализЖурналаРегистрации.ИмяПользователяИБ(Пользователь) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.';ru='Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.';en='Report generation is possible only for users who have a name to enter the program.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		Если Не ЗначениеЗаполнено(ПользователиИГруппы) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнено значение поля Пользователи.';ru='Не заполнено значение поля Пользователи.';en='Is not filled field User.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
