...
Процедура НастроитьРасписаниеРегламентногоЗадания(Команда)
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("ka='Невозможно получить расписание регламентного задания: регламентное задание было удалено или не указано его наименование.';ru='Невозможно получить расписание регламентного задания: регламентное задание было удалено или не указано его наименование.';en='Unable to get the job schedule routine: routine job has been deleted or is not specified his name.'"));
	КонецЕсли;
...
Процедура ПерейтиКЖурналуРегистрации(Команда)
	...
	Для Каждого Область Из Отчет.ВыделенныеОбласти Цикл
		...
		Если Расшифровка = Неопределено
			...
			ПоказатьПредупреждение(,НСтр("ka='Выберите строку или ячейку нужного сеанса задания';ru='Выберите строку или ячейку нужного сеанса задания';en='Select the desired row or cell job session'"));
			Возврат;
		КонецЕсли;
		ДатаНачала = Расшифровка.Получить(0);
		ДатаОкончания = Расшифровка.Получить(1);
