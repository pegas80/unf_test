...
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".'");
	...
	Если Параметры.Пользователь <> Пользователи.АвторизованныйПользователь()
	   И НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ВызватьИсключение НСтр("ka='Недостаточно прав для просмотра отчета.';ru='Недостаточно прав для просмотра отчета.'");
	КонецЕсли;
...
Процедура ВывестиОтчет(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам пользователя';ru='Отчет по правам пользователя'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли пользователя по профилям';ru='Роли пользователя по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Пользователь: %1';ru='Пользователь: %1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам внешнего пользователя';ru='Отчет по правам внешнего пользователя'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли внешнего пользователя по профилям';ru='Роли внешнего пользователя по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Внешний пользователь: %1';ru='Внешний пользователь: %1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам группы пользователей';ru='Отчет по правам группы пользователей'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли группы пользователей по профилям';ru='Роли группы пользователей по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Группа пользователей: %1';ru='Группа пользователей: %1'"));
	Иначе
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам группы внешних пользователей';ru='Отчет по правам группы внешних пользователей'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли группы внешних пользователей по профилям';ru='Роли группы внешних пользователей по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Группа внешних пользователей: %1';ru='Группа внешних пользователей: %1'"));
	КонецЕсли;
	...
	Если НЕ ВыводПравГруппы Тогда
		...
		Если ПользовательИБПрочитан Тогда
			...
			Если НЕ ЗначениеЗаполнено(СвойстваПользователяИБ.ПользовательОС) Тогда
				Область.Параметры.ПользовательОС = НСтр("ka='Не указан';ru='Не указан'");
			КонецЕсли;
			Документ.Вывести(Область, 2);
		Иначе
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		...
		Если ОднаПерсональнаяГруппа Тогда
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа пользователя';ru='Ограничения доступа пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа внешнего пользователя';ru='Ограничения доступа внешнего пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа группы пользователей';ru='Ограничения доступа группы пользователей'");
			Иначе
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа группы внешних пользователей';ru='Ограничения доступа группы внешних пользователей'");
			КонецЕсли;
		Иначе
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Группы доступа пользователя';ru='Группы доступа пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Группы доступа внешнего пользователя';ru='Группы доступа внешнего пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = НСтр("ka='Группы доступа группы пользователей';ru='Группы доступа группы пользователей'");
			Иначе
				ПредставлениеДоступа = НСтр("ka='Группы доступа группы внешних пользователей';ru='Группы доступа группы внешних пользователей'");
			КонецЕсли;
		КонецЕсли;
	...
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = НСтр("ka='Просмотр объектов';ru='Просмотр объектов'");
	Документ.Вывести(Область, 1);
	...
	Для каждого ОписаниеВидаОбъектов Из ПраваОбъектов.Строки Цикл
		...
		Если УпрощенныйИнтерфейс Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ka='პროფილები';ru='Профили'");
		Иначе
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ka='დაშვების ჯგუფი';ru='Группы доступа'");
		КонецЕсли;
		...
		Если ПодробныеСведенияОПравахДоступа Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ka='(профиль, роли)';ru='(профиль, роли)'");
		Иначе
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Если ОписаниеОбъекта.ЧтениеБезОграничения Тогда
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр, не ограничен)';ru='(просмотр, не ограничен)'");
				Иначе
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр*, не ограничен)';ru='(просмотр*, не ограничен)'");
				КонецЕсли;
			Иначе
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр, ограничен)';ru='(просмотр, ограничен)'");
				Иначе
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр*, ограничен)';ru='(просмотр*, ограничен)'");
				КонецЕсли;
			КонецЕсли;
...
"%2)'")
	...
	Область = Макет.ПолучитьОбласть("ПраваОбъектовГруппировка");
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = НСтр("ka='Редактирование объектов';ru='Редактирование объектов'");
	Документ.Вывести(Область, 1);
...
"изменение не доступно)'");
			...
			Иначе
				Если ОписаниеОбъекта.Изменение Тогда
					Если ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение, не ограничено)';ru='(изменение, не ограничено)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение*, не ограничено)';ru='(изменение*, не ограничено)'");
						КонецЕсли;
					Иначе
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение, ограничено)';ru='(изменение, ограничено)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение*, ограничено)';ru='(изменение*, ограничено)'");
						КонецЕсли;
					КонецЕсли;
				Иначе // НЕ ОписаниеОбъекта.Изменение
					УточнениеПредставленияОбъекта = НСтр("ka='(изменение не доступно)';ru='(изменение не доступно)'");
				КонецЕсли;
			КонецЕсли;
...
"подпапок'");
		ПодсказкаДляПодпапок = НСтр("ka='Права не только для текущей папки, но и для ее нижестоящих папок';ru='Права не только для текущей папки, но и для ее нижестоящих папок'");
		...
		ЗаголовокНастройкаПолученаОтГруппы = НСтр("ka='Настройка прав получена от группы';ru='Настройка прав получена от группы'");
		
		Область = Макет.ПолучитьОбласть("НастройкиПравЛегендаСтрокаНаследование");
		Область.Параметры.Подсказка = НСтр("ka='Наследование прав от вышестоящих папок';ru='Наследование прав от вышестоящих папок'");
		Документ.Вывести(Область, 2);
		...
		ТекстДа  = НСтр("ka='დიახ';ru='Да'");
		ТекстНет = НСтр("ka='არა';ru='Нет'");
	...
	Если ОписаниеВидаДоступа.Строки.Количество() = 0 Тогда
		...
		ИначеЕсли ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без запрещенных, текущий пользователь всегда разрешен)';ru='%1 (без запрещенных, текущий пользователь всегда разрешен)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без запрещенных, текущий внешний пользователь всегда разрешен)';ru='%1 (без запрещенных, текущий внешний пользователь всегда разрешен)'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (без запрещенных)';ru='%1 (без запрещенных)'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без разрешенных, текущий пользователь всегда разрешен)';ru='%1 (без разрешенных, текущий пользователь всегда разрешен)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без разрешенных, текущий внешний пользователь всегда разрешен)';ru='%1 (без разрешенных, текущий внешний пользователь всегда разрешен)'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (без разрешенных)';ru='%1 (без разрешенных)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (запрещенные, текущий пользователь всегда разрешен):';ru='%1 (запрещенные, текущий пользователь всегда разрешен):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (запрещенные, текущий внешний пользователь всегда разрешен):';ru='%1 (запрещенные, текущий внешний пользователь всегда разрешен):'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (запрещенные):';ru='%1 (запрещенные):'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (разрешенные, текущий пользователь всегда разрешен):';ru='%1 (разрешенные, текущий пользователь всегда разрешен):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (разрешенные, текущий внешний пользователь всегда разрешен):';ru='%1 (разрешенные, текущий внешний пользователь всегда разрешен):'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (разрешенные):';ru='%1 (разрешенные):'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеРежимаЗапуска(РежимЗапуска)
	...
	Если РежимЗапуска = "Авто" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ka='ავტო';ru='Авто'");
		
	ИначеЕсли РежимЗапуска = "ОбычноеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ka='Обычное приложение';ru='Обычное приложение'");
		
	ИначеЕсли РежимЗапуска = "УправляемоеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ka='Управляемое приложение';ru='Управляемое приложение'");
	Иначе
	...
	Пока Индекс >= 0 Цикл
		...
		Если СвойстваВидаДоступа = Неопределено Тогда
			...
			Если МетаданныеВладельцаНастроекПрав = Неопределено Тогда
				Строка.Представление = НСтр("ka='Неизвестный вид доступа';ru='Неизвестный вид доступа'");
			Иначе
				Строка.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Настройки прав на %1';ru='Настройки прав на %1'"),
