...
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".';ka='Чтобы открыть отчет откройте карточку пользователя,"
...
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".';ka='Чтобы открыть отчет откройте карточку пользователя или группы пользователей,"
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.Пользователь <> Пользователи.АвторизованныйПользователь()
	   И НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ВызватьИсключение НСтр("ka='Недостаточно прав для просмотра отчета.';ru='Недостаточно прав для просмотра отчета.';en='Not have permission to view the report.'");
	КонецЕсли;
...
Процедура ВывестиОтчет(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам пользователя';ru='Отчет по правам пользователя';en='Report on user rights'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли пользователя по профилям';ru='Роли пользователя по профилям';en='Roles User profiles'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Пользователь: %1';ru='Пользователь: %1';en='User:% 1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам внешнего пользователя';ru='Отчет по правам внешнего пользователя';en='Report on the Rights of the external user'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли внешнего пользователя по профилям';ru='Роли внешнего пользователя по профилям';en='Roles for external user profiles'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Внешний пользователь: %1';ru='Внешний пользователь: %1';en='External user:% 1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам группы пользователей';ru='Отчет по правам группы пользователей';en='Report on the Rights of the user group'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли группы пользователей по профилям';ru='Роли группы пользователей по профилям';en='The role of a group of users on profiles'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Группа пользователей: %1';ru='Группа пользователей: %1';en='User group:% 1'"));
	Иначе
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ka='Отчет по правам группы внешних пользователей';ru='Отчет по правам группы внешних пользователей';en='Report on the Rights of the group of external users'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ka='Роли группы внешних пользователей по профилям';ru='Роли группы внешних пользователей по профилям';en='The role of the group of external users on profiles'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ka='Группа внешних пользователей: %1';ru='Группа внешних пользователей: %1';en='The group of external users: 1%'"));
	КонецЕсли;
	...
	Если НЕ ВыводПравГруппы Тогда
		...
		Если ПользовательИБПрочитан Тогда
			...
			Если НЕ ЗначениеЗаполнено(СвойстваПользователяИБ.ПользовательОС) Тогда
				Область.Параметры.ПользовательОС = НСтр("ka='Не указан';ru='Не указан';en='Not specified'");
			КонецЕсли;
			Документ.Вывести(Область, 2);
		Иначе
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		...
		Если ОднаПерсональнаяГруппа Тогда
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа пользователя';ru='Ограничения доступа пользователя';en='User access restrictions'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа внешнего пользователя';ru='Ограничения доступа внешнего пользователя';en='external user access restrictions'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа группы пользователей';ru='Ограничения доступа группы пользователей';en='access user group restrictions'");
			Иначе
				ПредставлениеДоступа = НСтр("ka='Ограничения доступа группы внешних пользователей';ru='Ограничения доступа группы внешних пользователей';en='Access restrictions group of external users'");
			КонецЕсли;
		Иначе
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Группы доступа пользователя';ru='Группы доступа пользователя';en='User Access Groups'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = НСтр("ka='Группы доступа внешнего пользователя';ru='Группы доступа внешнего пользователя';en='External User Access group'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = НСтр("ka='Группы доступа группы пользователей';ru='Группы доступа группы пользователей';en='User Group Access Group'");
			Иначе
				ПредставлениеДоступа = НСтр("ka='Группы доступа группы внешних пользователей';ru='Группы доступа группы внешних пользователей';en='Group access group of external users'");
			КонецЕсли;
		КонецЕсли;
	...
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = НСтр("ka='Просмотр объектов';ru='Просмотр объектов';en='View objects'");
	Документ.Вывести(Область, 1);
	...
	Для каждого ОписаниеВидаОбъектов Из ПраваОбъектов.Строки Цикл
		...
		Если УпрощенныйИнтерфейс Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ka='პროფილები';ru='Профили';en='profiles'");
		Иначе
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ka='დაშვების ჯგუფი';ru='Группы доступа';en='access groups'");
		КонецЕсли;
		...
		Если ПодробныеСведенияОПравахДоступа Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ka='(профиль, роли)';ru='(профиль, роли)';en='(Profile, role)'");
		Иначе
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Если ОписаниеОбъекта.ЧтениеБезОграничения Тогда
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр, не ограничен)';ru='(просмотр, не ограничен)';en='(Viewing is not limited to)'");
				Иначе
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр*, не ограничен)';ru='(просмотр*, не ограничен)';en='(View * is not limited to)'");
				КонецЕсли;
			Иначе
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр, ограничен)';ru='(просмотр, ограничен)';en='(View limited)'");
				Иначе
					УточнениеПредставленияОбъекта = НСтр("ka='(просмотр*, ограничен)';ru='(просмотр*, ограничен)';en='(* Viewing is limited)'");
				КонецЕсли;
			КонецЕсли;
								...
								Если ПодробныеСведенияОПравахДоступа Тогда
									...
									Если КоличествоРолей > 1 Тогда
										УточнениеПредставленияГруппыДоступа =
											НСтр("ka='(профиль: %1, роли:"
"%2)';ru='(профиль: %1, роли:"
"%2)'")
									Иначе
										УточнениеПредставленияГруппыДоступа =
											НСтр("ka='(профиль: %1, роль:"
"%2)';ru='(профиль: %1, роль:"
"%2)'")
									КонецЕсли;
	...
	Область = Макет.ПолучитьОбласть("ПраваОбъектовГруппировка");
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = НСтр("ka='Редактирование объектов';ru='Редактирование объектов';en='Edit objects'");
	Документ.Вывести(Область, 1);
				...
				Если ОписаниеОбъекта.Добавление И ОписаниеОбъекта.Изменение Тогда
					Если ОписаниеОбъекта.ДобавлениеБезОграничения И ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление, не ограничено"
"изменение, не ограничено)';ru='(добавление, не ограничено"
...
"изменение*, не ограничено)'");
					...
					ИначеЕсли НЕ ОписаниеОбъекта.ДобавлениеБезОграничения И ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление, ограничено"
"изменение, не ограничено)';ru='(добавление, ограничено"
"изменение, не ограничено)'");
						ИначеЕсли НЕ ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление*, ограничено"
"изменение, не ограничено)';ru='(добавление*, ограничено"
...
"изменение*, не ограничено)'");
					...
					ИначеЕсли ОписаниеОбъекта.ДобавлениеБезОграничения И НЕ ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление, не ограничено"
"изменение, ограничено)';ru='(добавление, не ограничено"
...
"изменение*, ограничено)'");
					...
					Иначе // НЕ ОписаниеОбъекта.ДобавлениеБезОграничения И НЕ ОписаниеОбъекта.ИзменениеБезОграничения
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление, ограничено"
"изменение, ограничено)';ru='(добавление, ограничено"
"изменение, ограничено)'");
						ИначеЕсли НЕ ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление*, ограничено"
"изменение, ограничено)';ru='(добавление*, ограничено"
				...
				ИначеЕсли НЕ ОписаниеОбъекта.Добавление И ОписаниеОбъекта.Изменение Тогда
					...
					Если ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление не доступно"
"изменение, не ограничено)';ru='(добавление не доступно"
"изменение, не ограничено)'");
...
"изменение*, не ограничено)'");
					...
					Иначе // НЕ ОписаниеОбъекта.ИзменениеБезОграничения
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление не доступно"
"изменение, ограничено)';ru='(добавление не доступно"
"изменение, ограничено)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ka='(добавление не доступно"
"изменение*, ограничено)';ru='(добавление не доступно"
"изменение*, ограничено)'");
				...
				Иначе // НЕ ОписаниеОбъекта.Добавление И НЕ ОписаниеОбъекта.Изменение
					УточнениеПредставленияОбъекта = НСтр("ka='(добавление не доступно"
"изменение не доступно)';ru='(добавление не доступно"
"изменение не доступно)'");
			...
			Иначе
				Если ОписаниеОбъекта.Изменение Тогда
					Если ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение, не ограничено)';ru='(изменение, не ограничено)';en='(The change is not limited to)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение*, не ограничено)';ru='(изменение*, не ограничено)';en='(Change * There is no limit)'");
						КонецЕсли;
					Иначе
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение, ограничено)';ru='(изменение, ограничено)';en='(The change is limited)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ka='(изменение*, ограничено)';ru='(изменение*, ограничено)';en='(Change *, restricted)'");
						КонецЕсли;
					КонецЕсли;
				Иначе // НЕ ОписаниеОбъекта.Изменение
					УточнениеПредставленияОбъекта = НСтр("ka='(изменение не доступно)';ru='(изменение не доступно)';en='(The change is not available)'");
				КонецЕсли;
			КонецЕсли;
...
"%2)'")
	...
	Для каждого ОписаниеТипаОбъектов Из НастройкиПрав.Строки Цикл
		...
		ЗаголовокДляПодпапок =
			НСтр("ka='ქვესაქაღალდეებისთვის';ru='Для"
"подпапок'");
		ПодсказкаДляПодпапок = НСтр("ka='Права не только для текущей папки, но и для ее нижестоящих папок';ru='Права не только для текущей папки, но и для ее нижестоящих папок';en='Rights not only for the current folder, but also for its lower-level folders'");
		...
		ЗаголовокНастройкаПолученаОтГруппы = НСтр("ka='Настройка прав получена от группы';ru='Настройка прав получена от группы';en='Setting the rights derived from the group'");
		
		Область = Макет.ПолучитьОбласть("НастройкиПравЛегендаСтрокаНаследование");
		Область.Параметры.Подсказка = НСтр("ka='Наследование прав от вышестоящих папок';ru='Наследование прав от вышестоящих папок';en='Inheritance rights of the parent folder'");
		Документ.Вывести(Область, 2);
		...
		ТекстДа  = НСтр("ka='დიახ';ru='Да';en='Yes'");
		ТекстНет = НСтр("ka='არა';ru='Нет';en='Not'");
	...
	Если ОписаниеВидаДоступа.Строки.Количество() = 0 Тогда
		...
		ИначеЕсли ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без запрещенных, текущий пользователь всегда разрешен)';ru='%1 (без запрещенных, текущий пользователь всегда разрешен)';en='% 1 (no prohibited, the current user is always allowed)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без запрещенных, текущий внешний пользователь всегда разрешен)';ru='%1 (без запрещенных, текущий внешний пользователь всегда разрешен)';en='% 1 (no prohibited, the current external user is always allowed)'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (без запрещенных)';ru='%1 (без запрещенных)';en='1% (without prohibited)'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без разрешенных, текущий пользователь всегда разрешен)';ru='%1 (без разрешенных, текущий пользователь всегда разрешен)';en='1% (without permission, the current user is always allowed)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (без разрешенных, текущий внешний пользователь всегда разрешен)';ru='%1 (без разрешенных, текущий внешний пользователь всегда разрешен)';en='1% (without permission, the current external user is always allowed)'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (без разрешенных)';ru='%1 (без разрешенных)';en='1% (without permission)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (запрещенные, текущий пользователь всегда разрешен):';ru='%1 (запрещенные, текущий пользователь всегда разрешен):';en='% 1 (prohibited, the current user is always allowed):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (запрещенные, текущий внешний пользователь всегда разрешен):';ru='%1 (запрещенные, текущий внешний пользователь всегда разрешен):';en='% 1 (prohibited, the current external user is always allowed):'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (запрещенные):';ru='%1 (запрещенные):';en='1% (prohibited):'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (разрешенные, текущий пользователь всегда разрешен):';ru='%1 (разрешенные, текущий пользователь всегда разрешен):';en='% 1 (permitted, the current user is always allowed):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ka='%1 (разрешенные, текущий внешний пользователь всегда разрешен):';ru='%1 (разрешенные, текущий внешний пользователь всегда разрешен):';en='1% (allowed, current external user is always allowed):'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ka='%1 (разрешенные):';ru='%1 (разрешенные):';en='1% (resolved):'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеРежимаЗапуска(РежимЗапуска)
	...
	Если РежимЗапуска = "Авто" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ka='ავტო';ru='Авто';en='Auto'");
		
	ИначеЕсли РежимЗапуска = "ОбычноеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ka='Обычное приложение';ru='Обычное приложение';en='Normal application'");
		
	ИначеЕсли РежимЗапуска = "УправляемоеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ka='Управляемое приложение';ru='Управляемое приложение';en='managed application'");
	Иначе
	...
	Пока Индекс >= 0 Цикл
		...
		Если СвойстваВидаДоступа = Неопределено Тогда
			...
			Если МетаданныеВладельцаНастроекПрав = Неопределено Тогда
				Строка.Представление = НСтр("ka='Неизвестный вид доступа';ru='Неизвестный вид доступа';en='Unknown type of access'");
			Иначе
				Строка.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Настройки прав на %1';ru='Настройки прав на %1';en='Settings rights to 1%'"),
