	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыРаздела2 Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеТаблПоле.Значение.П000020003000) Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.СообщатьОНезаполненномКодеПоОКТМОВРазделе2 Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнен код ОКТМО на странице Раздела 2!';ru='Не заполнен код ОКТМО на странице Раздела 2!';en='code is empty OKTMO on Section 2 of the page!'");
				Сообщение.Сообщить();
			КонецЕсли;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если СтрНайти(Область.Имя, "П000010001003") > 0 ИЛИ Область.Имя = "П000020003000" Тогда
		...
		Если НЕ (ДлинаОКТМО = 0 ИЛИ ДлинаОКТМО = 8 ИЛИ ДлинаОКТМО = 11) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Код по ОКТМО должен состоять из 8 или 11 цифр!';ru='Код по ОКТМО должен состоять из 8 или 11 цифр!';en='by OKTMO code must be 8 or 11 digits!'");
			Сообщение.Сообщить();
...
Процедура ПроверитьКС_2(СтруктураРезультата)
	...
	ОписаниеКС = НСтр("ka='Значение базовой доходности на единицу физического показателя (строка 040 Раздела 2) должно соответствовать значению из Справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';ru='Значение базовой доходности на единицу физического показателя (строка 040 Раздела 2) должно соответствовать значению из Справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';en='The value of base yields per unit physical indicator (line 040 of section 2) must match the value from the Directory ""Codes types of business and yield values ​​of the base unit of the physical indicator""'");
	
	Нарушение      = НСтр("ka='Неправильное применение значений базовой доходности на единицу физического показателя';ru='Неправильное применение значений базовой доходности на единицу физического показателя';en='Improper use of the base values ​​of yield per unit physical indicator'");
	ОписаниеОшибки = НСтр("ru=''");
...
Процедура ПроверитьКС_4(СтруктураРезультата)
	...
	ОписаниеКС   = НСтр("ka='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки 15% по одноименному ОКТМО.';ru='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки 15% по одноименному ОКТМО.';en='Amount calculated single imputed income tax (line 110 of section 2) must be equal to the product of the fiscal base (line 100 in Section 2), and a tax rate of 15% on same OKTMO.'");
