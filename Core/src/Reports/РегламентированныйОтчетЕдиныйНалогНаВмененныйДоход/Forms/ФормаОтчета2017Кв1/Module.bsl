...
Процедура ОбработатьДополнительнуюСрокуДаты(ИмяОбласти)
	...
	Если НомерГиперссылки < МногострочнаяЧасть.Количество() Тогда
		...
		ТекстВопроса = НСтр("ka='Удалить выбранную дату?';ru='Удалить выбранную дату?'");
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.ИП Тогда
			...
			Если РегистрацииВНО.Количество() = 0 Тогда
				...
				КодНО = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""';ru='Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""'");
				Сообщение.Сообщить();
			...
			Иначе
				...
				Иначе
					...
					КодНО = "";
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ka='Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.';ru='Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.'");
					Сообщение.Сообщить();
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыРаздела2 Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеТаблПоле.Значение.П000020003000) Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.СообщатьОНезаполненномКодеПоОКТМОВРазделе2 Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнен код ОКТМО на странице Раздела 2.';ru='Не заполнен код ОКТМО на странице Раздела 2.'");
				Сообщение.Сообщить();
			КонецЕсли;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если СтрНайти(Область.Имя, "П000010001003") > 0 ИЛИ Область.Имя = "П000020003000" Тогда
		...
		Если НЕ (ДлинаОКТМО = 0 ИЛИ ДлинаОКТМО = 8 ИЛИ ДлинаОКТМО = 11) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Код по ОКТМО должен состоять из 8 или 11 цифр.';ru='Код по ОКТМО должен состоять из 8 или 11 цифр.'");
			Сообщение.Сообщить();
	...
	Если Область.Имя = "П000020010503" Тогда
		...
		Если НЕ (Область.Значение >= 7.5 И Область.Значение <= 15) Тогда
			...
			ПоказатьПредупреждение(,НСтр("ka='Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.';ru='Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.'"));
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не задан код налогового органа, в который представляется декларация.';ru='Не задан код налогового органа, в который представляется декларация.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(КодНО) <> 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Задан код налогового органа, не состоящий из 4 цифр.';ru='Задан код налогового органа, не состоящий из 4 цифр.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ СтатусПроверки Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Заполнение отчета прервано.';ru='Заполнение отчета прервано.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.';ru='Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.'"),
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 1 (КС 1.1)';ru='Проверка раздела 1 (КС 1.1)'");
...
"всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)'");
	...
	Нарушение      = НСтр("ka='Арифметическая ошибка';ru='Арифметическая ошибка'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.2)';ru='Проверка раздела 2 (КС 1.2)'");
	...
	Нарушение      = НСтр("ka='Статья 346.27 НК РФ';ru='Статья 346.27 НК РФ'");
	ОписаниеОшибки = НСтр("ka='Неправомерное применение системы налогообложения в виде ЕНВД';ru='Неправомерное применение системы налогообложения в виде ЕНВД'");
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.3)';ru='Проверка раздела 2 (КС 1.3)'");
	ОписаниеКС   = НСтр("ka='Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода';ru='Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода'");
	
	Нарушение      = НСтр("ka='Пункт 10 статьи 346.29 НК РФ';ru='Пункт 10 статьи 346.29 НК РФ'");
	ОписаниеОшибки = НСтр("ka='Неправильное исчисление налоговой базы за данный месяц';ru='Неправильное исчисление налоговой базы за данный месяц'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.4)';ru='Проверка раздела 2 (КС 1.4)'");
	ОписаниеКС   = НСтр("ka='Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода';ru='Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода'");
...
Процедура ПроверитьКС_1_5(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.5)';ru='Проверка раздела 2 (КС 1.5)'");
	ОписаниеКС   = НСтр("ka='Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода';ru='Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода'");
...
Процедура ПроверитьКС_1_6(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.6)';ru='Проверка раздела 2 (КС 1.6)'");
	ОписаниеКС   = НСтр("ka='Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)';ru='Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)'");
	
	Нарушение      = НСтр("ka='п.п. 2, 4 ст.346.29 НК РФ';ru='п.п. 2, 4 ст.346.29 НК РФ'");
	ОписаниеОшибки = НСтр("ka='Неправильно исчислена налоговая база за налоговый период.';ru='Неправильно исчислена налоговая база за налоговый период.'");
...
Процедура ПроверитьКС_1_7(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.7)';ru='Проверка раздела 2 (КС 1.7)'");
	ОписаниеКС   = НСтр("ka='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100';ru='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100'");
	
	Нарушение      = НСтр("ka='п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ';ru='п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ'");
	ОписаниеОшибки = НСтр("ka='Неправильное исчисление суммы ЕНВД за налоговый период';ru='Неправильное исчисление суммы ЕНВД за налоговый период'");
...
Процедура ПроверитьКС_1_8(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 3 (КС 1.8)';ru='Проверка раздела 3 (КС 1.8)'");
	ОписаниеКС   = НСтр("ka='Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2';ru='Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2'");
...
Процедура ПроверитьКС_1_9(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 3 (КС 1.9)';ru='Проверка раздела 3 (КС 1.9)'");
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 3 (КС 1.10)';ru='Проверка раздела 3 (КС 1.10)'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "КодОКТМО" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Код по ОКТМО (строка 010 раздела 1)';ru='Код по ОКТМО (строка 010 раздела 1)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр010" Тогда
		Результат.Вставить("Имя", "П000030001003");
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного за налоговый период единого налога на вмененный доход';ru='Сумма исчисленного за налоговый период единого налога на вмененный доход'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)';ru='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр040" Тогда
		Результат.Вставить("Имя", "П000030004003");
		Результат.Вставить("Представление", НСтр("ka='Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период';ru='Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр110" Тогда
		Результат.Вставить("Имя", "П000020011003");
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного единого налога на вмененный доход за налоговый период';ru='Сумма исчисленного единого налога на вмененный доход за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Налоговая база всего';ru='Налоговая база всего'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070" Тогда
		Результат.Вставить("Имя", "П000020007004");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база в 1 месяце квартала';ru='Налоговая база в 1 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080" Тогда
		Результат.Вставить("Имя", "П000020008004");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база во 2 месяце квартала';ru='Налоговая база во 2 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090" Тогда
		Результат.Вставить("Имя", "П000020009004");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база в 3 месяце квартала';ru='Налоговая база в 3 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога';ru='Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр005" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Признак налогоплательщика';ru='Признак налогоплательщика'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр040" Тогда
		Результат.Вставить("Имя", "П000020004003");
		Результат.Вставить("Представление", НСтр("ka='Базовая доходность на единицу физического показателя в месяц';ru='Базовая доходность на единицу физического показателя в месяц'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр010" Тогда
		Результат.Вставить("Имя", "П000020001000");
		Результат.Вставить("Представление", НСтр("ka='Код вида предпринимательской деятельности';ru='Код вида предпринимательской деятельности'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр050" Тогда
		Результат.Вставить("Имя", "П000020005003");
		Результат.Вставить("Представление", НСтр("ka='Корректирующий коэффициент К1';ru='Корректирующий коэффициент К1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр060" Тогда
		Результат.Вставить("Имя", "П000020006003");
		Результат.Вставить("Представление", НСтр("ka='Корректирующий коэффициент К2';ru='Корректирующий коэффициент К2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр02" Тогда
		Результат.Вставить("Имя", "П000020007002");
		Результат.Вставить("Представление", НСтр("ka='Величина физического показателя';ru='Величина физического показателя'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр03" Тогда
		Результат.Вставить("Имя", "П000020007003");
		Результат.Вставить("Представление", НСтр("ka='Количество календарных дней';ru='Количество календарных дней'"));
