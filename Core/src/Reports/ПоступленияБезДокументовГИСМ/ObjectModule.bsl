...
Процедура ВывестиДанныеОПоступлениях(Макет,
	...
	Если ИмяНабораДанных = "КиЗ" Тогда
		ЗаголовокНабораДанных = НСтр("ka='Поступления КиЗ, по которым нет уведомлений о выпуске КиЗ';ru='Поступления КиЗ, по которым нет уведомлений о выпуске КиЗ';en='Proceeds Keys for which no notification issue Keyes'");
	...
	ИначеЕсли ИмяНабораДанных = "МаркированныеТовары" Тогда
		ЗаголовокНабораДанных = НСтр("ka='Поступления маркированных товаров, по которым нет уведомлений о поступлении';ru='Поступления маркированных товаров, по которым нет уведомлений о поступлении';en='Proceeds marked goods for which there is a notification of a'");
	...
	Если Не ЗначениеЗаполнено(ОтборКонтрагент) Тогда
		...
		Область.Параметры.ИмяГруппировки = НСтр("ka='კონტრაგენტი';ru='Контрагент';en='Counterparty'");
		Результат.Вывести(Область);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОтборОрганизация) Тогда
		Область = Макет.ПолучитьОбласть("ШапкаГруппировка");
		Область.Параметры.ИмяГруппировки = НСтр("ka='ორგანიზაცია';ru='Организация';en='Organization'");
		Результат.Вывести(Область);
	КонецЕсли;
		...
		Пока ВыборкаОрганизации.Следующий() Цикл
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ИначеЕсли ВыборкаДетали.КоличествоВДокументахПоступления > 1 Тогда
					...
					Область.Параметры.КоличествоВДокументахПоступления = СтрШаблон(НСтр("ka='Документов поступления - %1';ru='Документов поступления - %1';en='Receipt of documents - 1%'"),
				...
				ИначеЕсли ВыборкаДетали.КоличествоВДокументахГИСМ > 1 Тогда
					...
					Область.Параметры.КоличествоВДокументахГИСМ      = СтрШаблон(НСтр("ka='Документов уведомлений - %1';ru='Документов уведомлений - %1';en='Notification of documents - 1%'"),
