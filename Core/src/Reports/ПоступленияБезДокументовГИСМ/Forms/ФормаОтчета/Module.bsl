...
Функция НастройкиОтчетаКорректны()
	...
	Если Не Отчет.ПоступленияЗаказанныхКиЗ И Не Отчет.ПоступленияМаркированныхТоваров Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ни один из типов документов поступления для анализа';ru='Не указан ни один из типов документов поступления для анализа';en='Unknown none of the types of revenue for the analysis of documents'"), ,"ПоступленияЗаказанныхКиЗ", "Отчет");
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ФормаЗаголовок = СтрЗаменить(НСтр("ka='ანგარიშგება ""%1""-ის გაგზავნა ფოსტით';ru='Отправка отчета ""%1"" по почте';en='Submitting the report ""%1"" in the mail'"), "%1", Заголовок);
...
Процедура УстановитьСостояниеОтчетНеСформирован()
	...
	Элементы.Результат.ОтображениеСостояния.Текст = НСтр("ka='ანგარიშგება არ არის ფორმირებული. ანგარიშგების მისაღებად დააჭირეთ ""ფორმირება""-ს.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';en='The report has not been formed. Click ""Create"" to obtain a report.'");
