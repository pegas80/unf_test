...
Функция НастройкиОтчетаКорректны()
	...
	Если Не Отчет.ПоступленияЗаказанныхКиЗ И Не Отчет.ПоступленияМаркированныхТоваров Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ни один из типов документов поступления для анализа';ru='Не указан ни один из типов документов поступления для анализа'"), ,"ПоступленияЗаказанныхКиЗ", "Отчет");
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ФормаЗаголовок = СтрЗаменить(НСтр("ka='ანგარიშგება ""%1""-ის გაგზავნა ფოსტით';ru='Отправка отчета ""%1"" по почте'"), "%1", Заголовок);
...
Процедура УстановитьСостояниеОтчетНеСформирован()
	...
	Элементы.Результат.ОтображениеСостояния.Текст = НСтр("ka='ანგარიშგება არ არის ფორმირებული. ანგარიშგების მისაღებად დააჭირეთ ""ფორმირება""-ს.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.'");
