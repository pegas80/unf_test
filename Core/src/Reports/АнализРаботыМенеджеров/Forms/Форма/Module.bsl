...
Процедура ОтборМенеджерыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Представление",							НСтр("ka='მენეჯერები';ru='менеджеры'"));
...
Процедура ОсновнаяДиаграммаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ГруппаПоказателей = "Выручка" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ამონაგები მენეჯერის მიხედვით: %1, პერიოდისთვის: %2 - %3';ru='Выручка от продаж по менеджеру: %1, за период: с %2 по %3'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (გაყიდვების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка продаж)'"));
	...
	ИначеЕсли ГруппаПоказателей = "Заказы" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეკვეთები მენეჯერის მიხედვით: %1, პერიოდისთვის: %2 - %3 %4';ru='Заказы по менеджеру: %1, за период: с %2 по %3 %4'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (შეკვეთების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка заказов)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "Задолженность" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტების დავალიანება მენეჯერისთვის: %1, პერიოდისთვის: %2';ru='Задолженность клиентов менеджера: %1, по состоянию на %2'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (დავალიანებების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка задолженности)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "События" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოვლენები მენეჯერის მიხედვით: %1, პერიოდისთვის %2 - %3 %4';ru='События по менеджеру: %1, за период: с %2 по %3 %4'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (მოვლენების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка событий)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ЛегендаДиаграммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ГруппаПоказателей = "Выручка" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ამონაგები გაყიდვების მიხედვით მენეჯერზე: %1, პერიოდისთვის: %2 - %3';ru='Выручка от продаж по менеджеру: %1, за период: с %2 по %3'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
	...
	ИначеЕсли ГруппаПоказателей = "Задолженность" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტების დავალიანება მენეჯერზე: %1, პერიოდისთვის %2';ru='Задолженность клиентов менеджера: %1, по состоянию на %2'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
			
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (დავალიანების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка задолженности)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "События" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოვლენები მენეჯერისთვის: %1,პერიოდისთვის: %2 - %3 %4';ru='События по менеджеру: %1, за период: с %2 по %3 %4'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
...
Процедура ОбновитьПоказателиВыручка()
	...
	Серия_Выручка.Текст = НСТР("ka='ამონაგები';ru='Выручка'");
	
	Серия_Себестоимость = ОсновнаяДиаграмма.Серии.Добавить("Себестоимость");
	Серия_Себестоимость.Текст = НСТР("ka='თვითღირებულება';ru='Себестоимость'");
	
	Серия_ВаловаяПрибыль = ОсновнаяДиаграмма.Серии.Добавить("Валовая прибыль");
	Серия_ВаловаяПрибыль.Текст = НСТР("ka='მთლიანი მოგება';ru='Валовая прибыль'");
	
	Серия_СредняяВыручка = ОсновнаяДиаграмма.Серии.Добавить("Средняя выручка по заказу");
	Серия_СредняяВыручка.Текст = НСТР("ka='საშუალო ამონაგები შეკვეთის მიხედვით';ru='Средняя выручка по заказу'");
	...
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Выручка", ОписаниеТипаЧисло, НСТР("ka='ამონაგები';ru='Выручка'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Себестоимость", ОписаниеТипаЧисло, НСТР("ka='თვითღირებულება';ru='Себестоимость'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ВаловаяПрибыль", ОписаниеТипаЧисло, НСТР("ka='მთლიანი მოგება';ru='Валовая прибыль'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_СредняяВыручка", ОписаниеТипаЧисло, НСТР("ka='საშუალო ამონაგები შეკვეთის მიხედვით';ru='Средняя выручка по заказу'"));
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		ПредставлениеОтветственного = ?(ЗначениеЗаполнено(ВыборкаОтветственный.Ответственный), ВыборкаОтветственный.ПредставлениеОтветственного, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'"));
...
Процедура ОбновитьПоказателиЗаказы()
	...
	Серия_ЗаказыВсего.Текст = НСТР("ka='სულ შეკვეთები';ru='Заказы всего'");
	Серия_ЗаказыВсего.Цвет = ЦветаСерий[3];
	
	Серия_Первичные = ОсновнаяДиаграмма.Серии.Добавить("Первичные");
	Серия_Первичные.Текст = НСТР("ka='ახალი კლიენტების შეკვეთები';ru='Заказы новых клиентов'");
	Серия_Первичные.Цвет = ЦветаСерий[1];
	
	Серия_Оплаченные = ОсновнаяДиаграмма.Серии.Добавить("Оплаченные");
	Серия_Оплаченные.Текст = НСТР("ka='გადახდილები';ru='Оплаченные'");
	Серия_Оплаченные.Цвет = ЦветаСерий[4];
	
	Серия_ЧастичноОплаченные = ОсновнаяДиаграмма.Серии.Добавить("Частично оплаченные");
	Серия_ЧастичноОплаченные.Текст = НСТР("ka='ნაწილობრივ გადახდილები';ru='Частично оплаченные'");
	Серия_ЧастичноОплаченные.Цвет = ЦветаСерий[2];
	
	Серия_Неоплаченные = ОсновнаяДиаграмма.Серии.Добавить("Неоплаченные");
	Серия_Неоплаченные.Текст = НСТР("ka='გადაუხდელები';ru='Неоплаченные'");
	Серия_Неоплаченные.Цвет = ЦветаСерий[0];
	
	ДанныеЛегендыДиаграммы = Новый ТаблицаЗначений;
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Точка", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ЗаказыВсего", ОписаниеТипаЧисло, НСТР("ka='სულ შეკვეთები';ru='Заказы всего'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Первичные", ОписаниеТипаЧисло, НСТР("ka='ახალი კლიენტების შეკვეთები';ru='Заказы новых клиентов'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Оплаченные", ОписаниеТипаЧисло, НСТР("ka='გადახდილები';ru='Оплаченные'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ЧастичноОплаченные", ОписаниеТипаЧисло, НСТР("ka='ნაწილობრივ გადახდილები';ru='Частично оплаченные'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Неоплаченные", ОписаниеТипаЧисло, НСТР("ka='გადაუხდელები';ru='Неоплаченные'"));
...
Процедура ОбновитьПоказателиЗадолженность()
	...
	Серия_Задолженность.Текст = НСТР("ka='დავალიანება';ru='Задолженность'");
	
	Серия_ПросроченнаяЗадолженность = ОсновнаяДиаграмма.Серии.Добавить("Просроченная задолженность");
	Серия_ПросроченнаяЗадолженность.Текст = НСТР("ka='ვადაგადაცილებული დავალიანება';ru='Просроченная задолженность'");
	
	ДанныеЛегендыДиаграммы = Новый ТаблицаЗначений;
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Точка", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Задолженность", ОписаниеТипаЧисло, НСТР("ka='დავალიანება';ru='Задолженность'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ПросроченнаяЗадолженность", ОписаниеТипаЧисло, НСТР("ka='ვადაგადაცილებული დავალიანება';ru='Просроченная задолженность'"));
	
	ДанныеЛегендыДиаграммы.Колонки.Добавить("КоличествоДолжников", ОписаниеТипаЧисло);
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеДолжников", Новый ОписаниеТипов("Строка"), НСТР("ka='მოვალეები';ru='Должники'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		ПредставлениеОтветственного = ?(ЗначениеЗаполнено(Выборка.Ответственный), Выборка.ПредставлениеОтветственного, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>'"));
...
Процедура ОбновитьПоказателиСобытия()
	...
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Точка", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Итого", ОписаниеТипаЧисло, НСТР("ka='სულ';ru='Итого'"));
...
Процедура ВыборПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора) Экспорт
	...
	Если ПараметрыВыбора.Вариант = Неопределено Или ПараметрыВыбора.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ka='დაკავშირებული...';ru='Относительный...'"));
	...
	Иначе
		...
		ОписаниеНавигационногоЭлемента.Вставить("Вариант",                  Неопределено);
		ОписаниеНавигационногоЭлемента.Вставить("ИндексНачальногоЗначения", Неопределено);
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ka='ფიქსირებული...';ru='Фиксированный...'"));
