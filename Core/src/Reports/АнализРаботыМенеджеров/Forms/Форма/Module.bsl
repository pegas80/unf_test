...
Процедура ОтборМенеджерыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Представление",							НСтр("ka='მენეჯერები';ru='менеджеры';en='managers'"));
...
Процедура ОсновнаяДиаграммаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ГруппаПоказателей = "Выручка" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ამონაგები მენეჯერის მიხედვით: %1, პერიოდისთვის: %2 - %3';ru='Выручка от продаж по менеджеру: %1, за период: с %2 по %3';en='Revenue from Sales Manager: 1% for the period: from 2% to% 3'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (გაყიდვების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка продаж)';en='Analysis of managers (transcript of sales)'"));
	...
	ИначеЕсли ГруппаПоказателей = "Заказы" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეკვეთები მენეჯერის მიხედვით: %1, პერიოდისთვის: %2 - %3 %4';ru='Заказы по менеджеру: %1, за период: с %2 по %3 %4';en='Orders for the manager 1%, for the period: of from 2% 3% 4%'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (შეკვეთების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка заказов)';en='Analysis of managers (decoding order)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "Задолженность" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტების დავალიანება მენეჯერისთვის: %1, პერიოდისთვის: %2';ru='Задолженность клиентов менеджера: %1, по состоянию на %2';en='Debt Manager clients: 1%, as of 2%'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (დავალიანებების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка задолженности)';en='Analysis of managers (debt decryption)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "События" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოვლენები მენეჯერის მიხედვით: %1, პერიოდისთვის %2 - %3 %4';ru='События по менеджеру: %1, за период: с %2 по %3 %4';en='Events by the manager 1%, for the period: of from 2% 3% 4%'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (მოვლენების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка событий)';en='Analysis of managers (event name)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ЛегендаДиаграммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ГруппаПоказателей = "Выручка" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ამონაგები გაყიდვების მიხედვით მენეჯერზე: %1, პერიოდისთვის: %2 - %3';ru='Выручка от продаж по менеджеру: %1, за период: с %2 по %3';en='Revenue from Sales Manager: 1% for the period: from 2% to% 3'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
	...
	ИначеЕсли ГруппаПоказателей = "Задолженность" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტების დავალიანება მენეჯერზე: %1, პერიოდისთვის %2';ru='Задолженность клиентов менеджера: %1, по состоянию на %2';en='Debt Manager clients: 1%, as of 2%'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
			
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='მენეჯერების მუშაობის ანალიზი (დავალიანების გაშიფვრა)';ru='Анализ работы менеджеров (расшифровка задолженности)';en='Analysis of managers (debt decryption)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "События" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოვლენები მენეჯერისთვის: %1,პერიოდისთვის: %2 - %3 %4';ru='События по менеджеру: %1, за период: с %2 по %3 %4';en='Events by the manager 1%, for the period: of from 2% 3% 4%'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
...
Процедура ОбновитьПоказателиВыручка()
	...
	Серия_Выручка.Текст = НСТР("ka='ამონაგები';ru='Выручка';en='revenue'");
	
	Серия_Себестоимость = ОсновнаяДиаграмма.Серии.Добавить("Себестоимость");
	Серия_Себестоимость.Текст = НСТР("ka='თვითღირებულება';ru='Себестоимость';en='Cost price'");
	
	Серия_ВаловаяПрибыль = ОсновнаяДиаграмма.Серии.Добавить("Валовая прибыль");
	Серия_ВаловаяПрибыль.Текст = НСТР("ka='მთლიანი მოგება';ru='Валовая прибыль';en='Gross profit'");
	
	Серия_СредняяВыручка = ОсновнаяДиаграмма.Серии.Добавить("Средняя выручка по заказу");
	Серия_СредняяВыручка.Текст = НСТР("ka='საშუალო ამონაგები შეკვეთის მიხედვით';ru='Средняя выручка по заказу';en='Average revenue by request'");
	...
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры';en='Managers'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Выручка", ОписаниеТипаЧисло, НСТР("ka='ამონაგები';ru='Выручка';en='revenue'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Себестоимость", ОписаниеТипаЧисло, НСТР("ka='თვითღირებულება';ru='Себестоимость';en='Cost price'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ВаловаяПрибыль", ОписаниеТипаЧисло, НСТР("ka='მთლიანი მოგება';ru='Валовая прибыль';en='Gross profit'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_СредняяВыручка", ОписаниеТипаЧисло, НСТР("ka='საშუალო ამონაგები შეკვეთის მიხედვით';ru='Средняя выручка по заказу';en='Average revenue by request'"));
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		ПредставлениеОтветственного = ?(ЗначениеЗаполнено(ВыборкаОтветственный.Ответственный), ВыборкаОтветственный.ПредставлениеОтветственного, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'"));
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', выручка: ';ka=', ამონაგები: '") + ВыборкаОтветственный.Выручка;
		ОсновнаяДиаграмма.УстановитьЗначение(Точка, Серия_Выручка, ВыборкаОтветственный.Выручка, Расшифровка, Подсказка);
		
		Расшифровка = Новый Структура("Точка", ВыборкаОтветственный.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', себестоимость: ';ka=', თვითღირებულება: '") + ВыборкаОтветственный.Себестоимость;
		ОсновнаяДиаграмма.УстановитьЗначение(Точка, Серия_Себестоимость, ВыборкаОтветственный.Себестоимость, Расшифровка, Подсказка);
		
		Расшифровка = Новый Структура("Точка", ВыборкаОтветственный.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', валовая прибыль: ';ka=', მთლიანი მოგება: '") + ВыборкаОтветственный.ВаловаяПрибыль;
		...
		Расшифровка = Новый Структура("Точка", ВыборкаОтветственный.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', средняя выручка по заказу: ';ka=', საშუალო ამონაგები შეკვეთის მიხედვით: '") + ВыборкаОтветственный.СредняяВыручка;
...
Процедура ОбновитьПоказателиЗаказы()
	...
	Серия_ЗаказыВсего.Текст = НСТР("ka='სულ შეკვეთები';ru='Заказы всего';en='orders all'");
	Серия_ЗаказыВсего.Цвет = ЦветаСерий[3];
	
	Серия_Первичные = ОсновнаяДиаграмма.Серии.Добавить("Первичные");
	Серия_Первичные.Текст = НСТР("ka='ახალი კლიენტების შეკვეთები';ru='Заказы новых клиентов';en='Orders of new customers'");
	Серия_Первичные.Цвет = ЦветаСерий[1];
	
	Серия_Оплаченные = ОсновнаяДиаграмма.Серии.Добавить("Оплаченные");
	Серия_Оплаченные.Текст = НСТР("ka='გადახდილები';ru='Оплаченные';en='Sponsored'");
	Серия_Оплаченные.Цвет = ЦветаСерий[4];
	
	Серия_ЧастичноОплаченные = ОсновнаяДиаграмма.Серии.Добавить("Частично оплаченные");
	Серия_ЧастичноОплаченные.Текст = НСТР("ka='ნაწილობრივ გადახდილები';ru='Частично оплаченные';en='partly paid'");
	Серия_ЧастичноОплаченные.Цвет = ЦветаСерий[2];
	
	Серия_Неоплаченные = ОсновнаяДиаграмма.Серии.Добавить("Неоплаченные");
	Серия_Неоплаченные.Текст = НСТР("ka='გადაუხდელები';ru='Неоплаченные';en='unpaid'");
	Серия_Неоплаченные.Цвет = ЦветаСерий[0];
	
	ДанныеЛегендыДиаграммы = Новый ТаблицаЗначений;
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Точка", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры';en='Managers'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ЗаказыВсего", ОписаниеТипаЧисло, НСТР("ka='სულ შეკვეთები';ru='Заказы всего';en='orders all'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Первичные", ОписаниеТипаЧисло, НСТР("ka='ახალი კლიენტების შეკვეთები';ru='Заказы новых клиентов';en='Orders of new customers'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Оплаченные", ОписаниеТипаЧисло, НСТР("ka='გადახდილები';ru='Оплаченные';en='Sponsored'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ЧастичноОплаченные", ОписаниеТипаЧисло, НСТР("ka='ნაწილობრივ გადახდილები';ru='Частично оплаченные';en='partly paid'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Неоплаченные", ОписаниеТипаЧисло, НСТР("ka='გადაუხდელები';ru='Неоплаченные';en='unpaid'"));
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', заказов всего: ';ka=', სულ შეკვეთები: '") + ВыборкаОтветственный.ЗаказыВсего;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', заказов новых клиентов: ';ka=', ახალი კლიენტების შეკვეთები: '") + ВыборкаОтветственный.Первичные;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', оплаченных заказов: ';ka=', გადახდილი შეკვეთები: '") + ВыборкаОтветственный.Оплаченные;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', частично оплаченных заказов: ';ka=', ნაწილობრივ გადახდილი შეკვეთები: '") + ВыборкаОтветственный.ЧастичноОплаченные;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', неоплаченных заказов: ';ka=', გადაუხდელი შეკვეთები: '") + ВыборкаОтветственный.Неоплаченные;
...
Процедура ОбновитьПоказателиЗадолженность()
	...
	Серия_Задолженность.Текст = НСТР("ka='დავალიანება';ru='Задолженность';en='debts'");
	
	Серия_ПросроченнаяЗадолженность = ОсновнаяДиаграмма.Серии.Добавить("Просроченная задолженность");
	Серия_ПросроченнаяЗадолженность.Текст = НСТР("ka='ვადაგადაცილებული დავალიანება';ru='Просроченная задолженность';en='past due accounts'");
	
	ДанныеЛегендыДиаграммы = Новый ТаблицаЗначений;
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Точка", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры';en='Managers'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_Задолженность", ОписаниеТипаЧисло, НСТР("ka='დავალიანება';ru='Задолженность';en='debts'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Серия_ПросроченнаяЗадолженность", ОписаниеТипаЧисло, НСТР("ka='ვადაგადაცილებული დავალიანება';ru='Просроченная задолженность';en='past due accounts'"));
	
	ДанныеЛегендыДиаграммы.Колонки.Добавить("КоличествоДолжников", ОписаниеТипаЧисло);
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеДолжников", Новый ОписаниеТипов("Строка"), НСТР("ka='მოვალეები';ru='Должники';en='debtors'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		ПредставлениеОтветственного = ?(ЗначениеЗаполнено(Выборка.Ответственный), Выборка.ПредставлениеОтветственного, НСтр("ka='<მენეჯერი არ არის მითითებული>';ru='<Менеджер не указан>';en='<Manager Unknown>'"));
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru = ', задолженность: '; ka = ', დავალიანება: '") + Выборка.Задолженность;
		ОсновнаяДиаграмма.УстановитьЗначение(Точка, Серия_Задолженность, Выборка.Задолженность, Расшифровка, Подсказка);
		
		Расшифровка = Новый Структура("Точка", Выборка.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru = ', просроченная задолженность: '; ka = ', ვადაგადაცილებული დავალიანება: '") + Выборка.ПросроченнаяЗадолженность;
...
Процедура ОбновитьПоказателиСобытия()
	...
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Точка", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("ПредставлениеТочки", Новый ОписаниеТипов("Строка"), НСТР("ka='მენეჯერები';ru='Менеджеры';en='Managers'"));
	ДанныеЛегендыДиаграммы.Колонки.Добавить("Итого", ОписаниеТипаЧисло, НСТР("ka='სულ';ru='Итого';en='Total:'"));
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		Пока ВыборкаСостояние.Следующий() Цикл
			...
			Подсказка = ПредставлениеОтветственного + НСтр("ru = ', событий в состоянии: '; ka = ', მოვლენები მოქმედებაშია: '") + ВыборкаСостояние.ПредставлениеСостояния + " = " + КоличествоСобытий;
...
Процедура ВыборПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора) Экспорт
	...
	Если ПараметрыВыбора.Вариант = Неопределено Или ПараметрыВыбора.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ka='დაკავშირებული...';ru='Относительный...';en='Relative...'"));
	...
	Иначе
		...
		ОписаниеНавигационногоЭлемента.Вставить("Вариант",                  Неопределено);
		ОписаниеНавигационногоЭлемента.Вставить("ИндексНачальногоЗначения", Неопределено);
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ka='ფიქსირებული...';ru='Фиксированный...';en='Fixed...'"));
