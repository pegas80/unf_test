...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДокументСсылка = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Открытие отчета предусмотрено только из документов.';ru='Открытие отчета предусмотрено только из документов.';en='Opening a report is provided only from documents.'");
	КонецЕсли;
...
Процедура Сформировать()
	...
	СтруктураПараметры.Вставить("ВидЦен",ДокументОбъект.ВидЦен);
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
