...
Процедура ПояснениеЗаголовкаПинКодОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ka='Не получается зарегистрировать программный продукт."
""
...
"Пинкод: %1.'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='Интернет-поддержка. Регистрация продукта.';ru='Интернет-поддержка. Регистрация продукта.';en='Internet support. Product Registration.'"),
			ТекстСообщения,
			,
			,
...
Функция ЗаполнениеПолейКорректно()
	...
	Если ПустаяСтрока(ПинКод) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не заполнено поле ""Пинкод""';ru='Не заполнено поле ""Пинкод""';en='Do not fill in the ""PIN code""'");
	...
	Если СтрДлина(ПинКодСтр) <> 16 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Пин-код должен состоять из 16 цифр.';ru='Пин-код должен состоять из 16 цифр.';en='PIN must consist of 16 digits.'");
		Сообщение.Поле  = "ПинКод";
		Сообщение.Сообщить();
