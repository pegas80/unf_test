...
Процедура АдресНажатие(Элемент, СтандартнаяОбработка)
	...
	Если РегионыСтраны = Неопределено Тогда
		...
		РегионыСтраны.Добавить("-1", НСтр("ka='<არ არის არჩეული>';ru='<не выбран>'"));
	КонецЕсли;
...
"Ответственный сотрудник: %5'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='Интернет-поддержка. Ввод информации об организации.';ru='Интернет-поддержка. Ввод информации об организации.'"),
...
Процедура КомандаОтмена(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы уверены, что хотите отказаться от регистрации программного продукта?';ru='Вы уверены, что хотите отказаться от регистрации программного продукта?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		,
		НСтр("ka='Регистрация программного продукта';ru='Регистрация программного продукта'"));
...
Процедура НастроитьОтображениеФормы()
	...
	ПредставлениеАдреса   = НСтр("ka='<введите адрес>';ru='<введите адрес>'");
...
Функция ЗаполнениеПолейКорректно()
	...
	Если Организация = "-1" Тогда
		...
		Если ПустаяСтрока(НазваниеОрганизации) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Название организации"" не заполнено.';ru='Поле ""Название организации"" не заполнено.'"),
				"НазваниеОрганизации",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ТипДеятельности) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Тип деятельности"" не заполнено.';ru='Поле ""Тип деятельности"" не заполнено.'"),
				"ТипДеятельности",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ИНН) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""ИНН"" не заполнено.';ru='Поле ""ИНН"" не заполнено.'"),
				"ИНН",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Руководитель) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Руководитель"" не заполнено.';ru='Поле ""Руководитель"" не заполнено.'"),
				"Руководитель",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Телефон) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Телефон"" не заполнено.';ru='Поле ""Телефон"" не заполнено.'"),
				"Телефон",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Адрес электронной почты"" не заполнено.';ru='Поле ""Адрес электронной почты"" не заполнено.'"),
				"АдресЭлектроннойПочты",
				Отказ);
		КонецЕсли;
	...
	Если ПустаяСтрока(ГдеПриобретенаПрограмма) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Место покупки"" не заполнено.';ru='Поле ""Место покупки"" не заполнено.'"),
			"ГдеПриобретенаПрограмма",
			Отказ);
	КонецЕсли;
	
	Если ДатаПриобретенияПрограммы = '00010101' Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Дата покупки"" не заполнено.';ru='Поле ""Дата покупки"" не заполнено.'"),
			"ДатаПриобретенияПрограммы",
			Отказ);
	КонецЕсли;
	
	Если ЧислоРабочихМест = 0 Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Число рабочих мест"" не заполнено.';ru='Поле ""Число рабочих мест"" не заполнено.'"),
			"ЧислоРабочихМест",
			Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(Ответственный) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Ответственный сотрудник"" не заполнено.';ru='Поле ""Ответственный сотрудник"" не заполнено.'"),
			"Ответственный",
			Отказ);
	КонецЕсли;
...
Процедура СформироватьПредставлениеАдреса()
	...
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Район, НСтр("ka='რაიონი';ru='район'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Город, НСтр("ka='წ.';ru='г.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Улица, НСтр("ka='ул.';ru='ул.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Дом, НСтр("ka='д.';ru='д.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Строение, НСтр("ka='стр.';ru='стр.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Квартира, НСтр("ka='кв.';ru='кв.'") + " ");
	
	Если ПустаяСтрока(ПредставлениеАдреса) Тогда
		Если Организация = "-1" Тогда
			ПредставлениеАдреса = НСтр("ka='<введите адрес>';ru='<введите адрес>'");
		Иначе
			ПредставлениеАдреса = НСтр("ka='<не заполнен>';ru='<не заполнен>'");
		КонецЕсли;
	КонецЕсли;
