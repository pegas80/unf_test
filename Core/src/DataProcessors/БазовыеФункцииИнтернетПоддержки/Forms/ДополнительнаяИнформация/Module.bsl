...
Процедура АдресНажатие(Элемент, СтандартнаяОбработка)
	...
	Если РегионыСтраны = Неопределено Тогда
		...
		РегионыСтраны.Добавить("-1", НСтр("ka='<არ არის არჩეული>';ru='<не выбран>';en='<Not Set>'"));
	КонецЕсли;
...
Процедура ПояснениеКЗаголовкуОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ka='Не получается ввести информацию об организации при регистрации программного продукта."
""
...
"Факс: %8';ru='Не получается ввести информацию об организации при регистрации программного продукта."
""
"Название организации: %1"
"Тип деятельности: %2"
"ИНН: %3"
...
"Факс: %8'");
		...
		ТекстСообщенияПродолжение = НСтр("ka='Адрес: %1"
"Место покупки: %2"
"Дата покупки: %3"
"Число рабочих мест: %4"
"Ответственный сотрудник: %5';ru='Адрес: %1"
"Место покупки: %2"
"Дата покупки: %3"
"Число рабочих мест: %4"
"Ответственный сотрудник: %5'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='Интернет-поддержка. Ввод информации об организации.';ru='Интернет-поддержка. Ввод информации об организации.';en='Internet support. Enter information about the organization.'"),
...
Процедура КомандаОтмена(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы уверены, что хотите отказаться от регистрации программного продукта?';ru='Вы уверены, что хотите отказаться от регистрации программного продукта?';en='Are you sure you want to stop recording software?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		,
		НСтр("ka='Регистрация программного продукта';ru='Регистрация программного продукта';en='Register software'"));
...
Процедура НастроитьОтображениеФормы()
	...
	ПредставлениеАдреса   = НСтр("ka='<введите адрес>';ru='<введите адрес>';en='<Enter the address>'");
...
Функция ЗаполнениеПолейКорректно()
	...
	Если Организация = "-1" Тогда
		...
		Если ПустаяСтрока(НазваниеОрганизации) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Название организации"" не заполнено.';ru='Поле ""Название организации"" не заполнено.';en='The ""Name of the organization"" is not filled.'"),
				"НазваниеОрганизации",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ТипДеятельности) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Тип деятельности"" не заполнено.';ru='Поле ""Тип деятельности"" не заполнено.';en='The field ""Business Type"" is not filled.'"),
				"ТипДеятельности",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ИНН) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""ИНН"" не заполнено.';ru='Поле ""ИНН"" не заполнено.';en='The ""inn"" is not filled.'"),
				"ИНН",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Руководитель) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Руководитель"" не заполнено.';ru='Поле ""Руководитель"" не заполнено.';en='The ""Leader"" is not filled.'"),
				"Руководитель",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Телефон) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Телефон"" не заполнено.';ru='Поле ""Телефон"" не заполнено.';en='The ""Telephone"" is not filled.'"),
				"Телефон",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Адрес электронной почты"" не заполнено.';ru='Поле ""Адрес электронной почты"" не заполнено.';en='The ""E-mail"" is not filled.'"),
				"АдресЭлектроннойПочты",
				Отказ);
		КонецЕсли;
	...
	Если ПустаяСтрока(ГдеПриобретенаПрограмма) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Место покупки"" не заполнено.';ru='Поле ""Место покупки"" не заполнено.';en='The ""Place of purchase"" is not filled.'"),
			"ГдеПриобретенаПрограмма",
			Отказ);
	КонецЕсли;
	
	Если ДатаПриобретенияПрограммы = '00010101' Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Дата покупки"" не заполнено.';ru='Поле ""Дата покупки"" не заполнено.';en='The ""Purchase Date"" is not filled.'"),
			"ДатаПриобретенияПрограммы",
			Отказ);
	КонецЕсли;
	
	Если ЧислоРабочихМест = 0 Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Число рабочих мест"" не заполнено.';ru='Поле ""Число рабочих мест"" не заполнено.';en='The ""number of jobs"" is not filled.'"),
			"ЧислоРабочихМест",
			Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(Ответственный) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ka='Поле ""Ответственный сотрудник"" не заполнено.';ru='Поле ""Ответственный сотрудник"" не заполнено.';en='The ""Responsible Officer"" is not filled.'"),
			"Ответственный",
			Отказ);
	КонецЕсли;
...
Процедура СформироватьПредставлениеАдреса()
	...
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Район, НСтр("ka='რაიონი';ru='район';en='district'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Город, НСтр("ka='წ.';ru='г.';en='city'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Улица, НСтр("ka='ул.';ru='ул.';en='Str.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Дом, НСтр("ka='д.';ru='д.';en='d.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Строение, НСтр("ka='стр.';ru='стр.';en='p.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Квартира, НСтр("ka='кв.';ru='кв.';en='sq.'") + " ");
	
	Если ПустаяСтрока(ПредставлениеАдреса) Тогда
		Если Организация = "-1" Тогда
			ПредставлениеАдреса = НСтр("ka='<введите адрес>';ru='<введите адрес>';en='<Enter the address>'");
		Иначе
			ПредставлениеАдреса = НСтр("ka='<не заполнен>';ru='<не заполнен>';en='<Is empty>'");
		КонецЕсли;
	КонецЕсли;
