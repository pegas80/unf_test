...
Процедура СписокЗарегистрированныхПродуктовРегНомерНажатие(Элемент)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ka='Список зарегистрированных продуктов';ru='Список зарегистрированных продуктов';en='A list of registered products'"),
...
Процедура ПояснениеКЗаголовкуРегНомерОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ka='Не получается ввести регистрационный номер программного продукта.';ru='Не получается ввести регистрационный номер программного продукта.';en='It is impossible to enter the registration number of the software product.'");
		
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='Интернет-поддержка. Ввод регистрационного номера.';ru='Интернет-поддержка. Ввод регистрационного номера.';en='Internet support. Enter the registration number.'"),
			ТекстСообщения,
			,
			,
...
Процедура ЗаполнитьФорму()
	...
	ЗаголовокПользователя = НСтр("ka='Логин:';ru='Логин:';en='Login:'") + " " + Параметры.login;
...
Функция ЗаполнениеПолейКорректно()
	...
	Если ПустаяСтрока(РегистрационныйНомерРегНомер) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не заполнено поле ""Регистрационный номер""';ru='Не заполнено поле ""Регистрационный номер""';en='Do not fill in the ""Registration number""'");
