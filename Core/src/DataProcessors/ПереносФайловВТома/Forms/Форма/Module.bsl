...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ДополнительныеПараметры.ПриОткрытииХранитьФайлыВТомахНаДиске Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не установлен тип хранения файлов ""В томах на диске""';ru='Не установлен тип хранения файлов ""В томах на диске""'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если НЕ ДополнительныеПараметры.ПриОткрытииЕстьТомаХраненияФайлов Тогда 
		ПоказатьПредупреждение(, НСтр("ka='Нет ни одного тома для размещения файлов';ru='Нет ни одного тома для размещения файлов'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПереносФайловВТома(Команда)
	...
	Если ЧислоВерсийВБазе = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нет ни одного файла в информационной базе';ru='Нет ни одного файла в информационной базе'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПереносФайловВТомаЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Состояние(НСтр("ka='Выполняется получение списка файлов...';ru='Выполняется получение списка файлов...'"));
	...
	Для Каждого ВерсияСтруктура Из МассивВерсий Цикл
		...
		Состояние(НСтр("ka='Выполняется перенос файла в том...';ru='Выполняется перенос файла в том...'"), Прогресс, ВерсияСтруктура.Текст, БиблиотекаКартинок.УстановитьВремя);
...
"Перенесено файлов: %1'"),
	...
	Если МассивФайловСОшибками.Количество() <> 0 Тогда
		...
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Количество ошибок при переносе: %1';ru='Количество ошибок при переносе: %1'"),
...
Функция ПеренестиВерсиюВТом(ВерсияСтруктура, МаксимальныйРазмерФайла, МассивФайловСОшибками)
	...
	Если Размер > МаксимальныйРазмерФайла Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Файлы.Ошибка переноса файла в том';ru='Файлы.Ошибка переноса файла в том'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"""Размер превышает максимальный"".'"),
	...
	ИмяДляЖурнала = ВерсияСтруктура.Текст;
	ЗаписьЖурналаРегистрации(НСтр("ka='Файлы.Начат перенос файла в том';ru='Файлы.Начат перенос файла в том'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"""%1"".'"),
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Файлы.Завершен перенос файла в том';ru='Файлы.Завершен перенос файла в том'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,, ФайлСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
