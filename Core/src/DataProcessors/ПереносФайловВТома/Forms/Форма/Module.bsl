...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ДополнительныеПараметры.ПриОткрытииХранитьФайлыВТомахНаДиске Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не установлен тип хранения файлов ""В томах на диске""';ru='Не установлен тип хранения файлов ""В томах на диске""';en='Do not set file storage type ""in the disk volumes""'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если НЕ ДополнительныеПараметры.ПриОткрытииЕстьТомаХраненияФайлов Тогда 
		ПоказатьПредупреждение(, НСтр("ka='Нет ни одного тома для размещения файлов';ru='Нет ни одного тома для размещения файлов';en='There are no volumes for the files'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПереносФайловВТома(Команда)
	...
	Если ЧислоВерсийВБазе = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нет ни одного файла в информационной базе';ru='Нет ни одного файла в информационной базе';en='There are no files in the information database'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ka='Выполнить перенос файлов в информационной базе в тома хранения файлов?"
""
"Эта операция может занять продолжительное время.';ru='Выполнить перенос файлов в информационной базе в тома хранения файлов?"
""
...
Процедура ВыполнитьПереносФайловВТомаЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Состояние(НСтр("ka='Выполняется получение списка файлов...';ru='Выполняется получение списка файлов...';en='Retrieves a list of files ...'"));
	...
	Для Каждого ВерсияСтруктура Из МассивВерсий Цикл
		...
		Состояние(НСтр("ka='Выполняется перенос файла в том...';ru='Выполняется перенос файла в том...';en='File transfer is performed in the ...'"), Прогресс, ВерсияСтруктура.Текст, БиблиотекаКартинок.УстановитьВремя);
	...
	Если ЧислоПеренесенных <> 0 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Завершен перенос файлов в тома."
"Перенесено файлов: %1';ru='Завершен перенос файлов в тома."
"Перенесено файлов: %1'"),
			ЧислоПеренесенных);
	...
	Если МассивФайловСОшибками.Количество() <> 0 Тогда
		...
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Количество ошибок при переносе: %1';ru='Количество ошибок при переносе: %1';en='The number of errors in the transfer: 1%'"),
...
"Перенос прерван.';ka='Не удалось перенести ни одного файла из пакета."
"Перенос прерван.'");
		КонецЕсли;
...
Функция ПеренестиВерсиюВТом(ВерсияСтруктура, МаксимальныйРазмерФайла, МассивФайловСОшибками)
	...
	Если Размер > МаксимальныйРазмерФайла Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Файлы.Ошибка переноса файла в том';ru='Файлы.Ошибка переноса файла в том';en='Fayly.Oshibka transfer file is'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,, ФайлСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При переносе в том файла"
"""%1"""
"возникла ошибка:"
"""Размер превышает максимальный"".';ka='При переносе в том файла"
"""%1"""
"возникла ошибка:"
"""Размер превышает максимальный"".'"),
	...
	ИмяДляЖурнала = ВерсияСтруктура.Текст;
	ЗаписьЖурналаРегистрации(НСтр("ka='Файлы.Начат перенос файла в том';ru='Файлы.Начат перенос файла в том';en='Fayly.Nachat file transfer is'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,, ФайлСсылка,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Начат перенос в том файла"
...
"""%1"".'"),
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Файлы.Завершен перенос файла в том';ru='Файлы.Завершен перенос файла в том';en='Fayly.Zavershen file transfer is'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,, ФайлСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Завершен перенос в том файла"
