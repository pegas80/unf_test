...
Процедура СформироватьДокументы(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	Если НЕ Параметры.Свойство("ПланДоставки") ИЛИ ТипЗнч(Параметры.ПланДоставки)<>Тип("ДеревоЗначений") Тогда
		Ошибка = НСтр("ka='Не задан план доставки';ru='Не задан план доставки'");
...
Процедура СоздатьОбновитьДокументыОтгрузки(Стр, Ошибки)
	...
	Если Стр.СостояниеОтгрузки=Перечисления.СостоянияДоставкиЗаказа.Доставлен И ТипЗнч(Стр.Группировка)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Исключение
			...
			Ошибка = СтрШаблон(НСтр("ka='Не удалось обновить данные расходной накладной по заказу %1: %2';ru='Не удалось обновить данные расходной накладной по заказу %1: %2'"), Стр.ПредставлениеГруппировки, РекурсивноеОписаниеОшибки(Информация, СообщенияПользователю));
		...
		Если Стр.Группировка.СостояниеЗаказа<>НовоеСостояниеЗаказа Тогда
			...
			Исключение
				Информация = ИнформацияОбОшибке();
				СообщенияПользователю = ПолучитьСообщенияПользователю();
				Ошибка = СтрШаблон(НСтр("ka='Не удалось обновить состояние заказа %1: %2';ru='Не удалось обновить состояние заказа %1: %2'"), Стр.ПредставлениеГруппировки, РекурсивноеОписаниеОшибки(Информация, СообщенияПользователю));
				Ошибки.Добавить(Стр.Группировка, Ошибка);
			КонецПопытки; 
		КонецЕсли;
	...
	ИначеЕсли Стр.СостояниеОтгрузки=Перечисления.СостоянияДоставкиЗаказа.НеДоставлен И ТипЗнч(Стр.Группировка)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				Информация = ИнформацияОбОшибке();
				СообщенияПользователю = ПолучитьСообщенияПользователю();
				Ошибка = СтрШаблон(НСтр("ka='Не удалось сформировать возврат по заказу %1: %2';ru='Не удалось сформировать возврат по заказу %1: %2'"), Стр.ПредставлениеГруппировки, РекурсивноеОписаниеОшибки(Информация, СообщенияПользователю));
				Ошибки.Добавить(Стр.Группировка, Ошибка);
			КонецПопытки; 
		КонецЦикла; 
	...
	ИначеЕсли Стр.СостояниеОтгрузки=Перечисления.СостоянияДоставкиЗаказа.ДоставленЧастично И ТипЗнч(Стр.Группировка)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Исключение
			Информация = ИнформацияОбОшибке();
			СообщенияПользователю = ПолучитьСообщенияПользователю();
			Ошибка = СтрШаблон(НСтр("ka='Не удалось выполнить частичную отгрузку по заказу %1: %2';ru='Не удалось выполнить частичную отгрузку по заказу %1: %2'"), Стр.ПредставлениеГруппировки, РекурсивноеОписаниеОшибки(Информация, СообщенияПользователю));
			Если Док.ЭтоНовый() Тогда
				Ошибки.Добавить(Стр.Группировка, Ошибка);
			Иначе
				Ошибки.Добавить(Док.Ссылка, Ошибка);
...
Процедура СоздатьОбновитьДокументыОплаты(Стр, Ошибки)
	...
	Если (Стр.СостояниеОплаты=Перечисления.СостоянияОплатыЗаказа.НеОплачен ИЛИ Стр.Оплачено=0) И ТипЗнч(Стр.Группировка)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				Информация = ИнформацияОбОшибке();
				СообщенияПользователю = ПолучитьСообщенияПользователю();
				Ошибка = СтрШаблон(НСтр("ka='Не отменить оплату заказа %1: %2';ru='Не отменить оплату заказа %1: %2'"), Стр.ПредставлениеГруппировки, РекурсивноеОписаниеОшибки(Информация, СообщенияПользователю));
	...
	ИначеЕсли Стр.СостояниеОплаты=Перечисления.СостоянияОплатыЗаказа.ОплаченНаличными И ТипЗнч(Стр.Группировка)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Исключение
			Информация = ИнформацияОбОшибке();
			СообщенияПользователю = ПолучитьСообщенияПользователю();
			Ошибка = СтрШаблон(НСтр("ka='Не зафиксировать оплату заказа %1: %2';ru='Не зафиксировать оплату заказа %1: %2'"), Стр.ПредставлениеГруппировки, РекурсивноеОписаниеОшибки(Информация, СообщенияПользователю));
			Если Док.ЭтоНовый() Тогда
				Ошибки.Добавить(Стр.Группировка, Ошибка);
			Иначе
				Ошибки.Добавить(Док.Ссылка, Ошибка);
