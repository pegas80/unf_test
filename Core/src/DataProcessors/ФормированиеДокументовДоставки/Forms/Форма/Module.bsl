...
Процедура ДекорацияСостояниеЗаказовПриДоставкеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтруктураОткрытия.Вставить("Заголовок", НСтр("ka='Состояние доставленных заказов';ru='Состояние доставленных заказов';en='State delivered orders'"));
	СтруктураОткрытия.Вставить("Подсказка", НСтр("ka='Укажите состояние, в которое будет переведен заказ покупателя после установки признака доставки';ru='Укажите состояние, в которое будет переведен заказ покупателя после установки признака доставки';en='Specify the state that will be transferred to the buyer after an order setting delivery sign'"));
...
Процедура ПланДоставкиСостояниеОплатыПриИзменении(Элемент)
	...
	Если ТекущаяСтрока.СостояниеОплаты<>ПредопределенноеЗначение("Перечисление.СостоянияОплатыЗаказа.НеОплачен")
		...
		Состояние(
		НСтр("ka='Заказ еще не доставлен. Невозможно определить сумму оплаты.';ru='Заказ еще не доставлен. Невозможно определить сумму оплаты.';en='The order has not yet been delivered. It is impossible to determine the amount of payment.'"), , ,
...
Процедура СформироватьДокументыКлиент()
	...
	Если НЕ ФормаПараметры.НесколькоВидовЗаказов И НЕ ЗначениеЗаполнено(СостояниеЗаказовПриДоставке) Тогда
		...
		НСтр("ka='Укажите состояние заказа, устанавливаемое при доставке';ru='Укажите состояние заказа, устанавливаемое при доставке';en='Specify the status of the order, to be ascertained at delivery'"), , 
		"СостояниеЗаказовПриДоставке");
		Возврат;
	КонецЕсли;
	
	ПараметрыСовета = Новый Структура;
	ПараметрыСовета.Вставить("Заголовок", НСтр("ka='Формирование документов доставки';ru='Формирование документов доставки';en='Formation Document Delivery'"));
...
Процедура НастроитьСостояниеЗаказов(Команда)
	...
	СтруктураОткрытия = Новый Структура;
	СтруктураОткрытия.Вставить("Заголовок", НСтр("ka='Состояния доставленных заказов';ru='Состояния доставленных заказов';en='The state of the delivered orders'"));
	СтруктураОткрытия.Вставить("Подсказка", НСтр("ka='Укажите состояния, в которые будет переведен заказ покупателя после установки признака доставки';ru='Укажите состояния, в которые будет переведен заказ покупателя после установки признака доставки';en='Specify the states to which the order will be transferred to the buyer after the delivery sign installation'"));
	СтруктураОткрытия.Вставить("ИмяРеквизита", "СостояниеДоставлен");
	ОткрытьФорму("ОбщаяФорма.ФормаНастройкиФиксированныхСостоянийЗаказов", СтруктураОткрытия, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьУсловноеОформление()
	...
	Инструкция.Оформление.Вставить("Текст", НСтр("ka='<Не сформирован документ отгрузки>';ru='<Не сформирован документ отгрузки>';en='<Not formed shipment document>'"));
...
Процедура ОбновитьЗаголовокФильтров()
	...
	Если ФильтрТолькоНедоставленные Тогда
		ЗаголовокГруппы = ЗаголовокГруппы+?(ПустаяСтрока(ЗаголовокГруппы), "", ", ")+НСтр("ka='недоставленные';ru='недоставленные';en='undeliverable'");
	КонецЕсли; 
	Если ФильтрТолькоНеоплаченные Тогда
		ЗаголовокГруппы = ЗаголовокГруппы+?(ПустаяСтрока(ЗаголовокГруппы), "", ", ")+НСтр("ka='неоплаченные';ru='неоплаченные';en='outstanding'");
	КонецЕсли; 
...
Процедура ПроверитьМодифицированностьИОбновить()
	...
	ПоказатьВопрос(
	Оповещение, 
	НСтр("ka='При обновлении списка внесенные изменения будут отменены. Все равно продолжить?';ru='При обновлении списка внесенные изменения будут отменены. Все равно продолжить?';en='When you update a list of the changes will be canceled. Continue anyway?'"), 
	РежимДиалогаВопрос.ОКОтмена, 
	, 
	КодВозвратаДиалога.Отмена, 
	НСтр("ka='Обновление списка заказов';ru='Обновление списка заказов';en='Updating the orders list'")); 
...
Процедура УстановитьВидимостьЭлементов(Форма)
	...
	Если Форма.ПоНоменклатуре Тогда
		Элементы.ПланДоставкиГруппировка.Заголовок = НСтр("ka='Заказ / Номенклатура';ru='Заказ / Номенклатура';en='Order / Nomenclature'");
		Элементы.ПланДоставкиПредставлениеГруппировки.Заголовок = НСтр("ka='Заказ / Номенклатура';ru='Заказ / Номенклатура';en='Order / Nomenclature'");
		Элементы.ПланДоставки.Отображение = ОтображениеТаблицы.Дерево;
	Иначе
		Элементы.ПланДоставкиГруппировка.Заголовок = НСтр("ka='შეკვეთა';ru='Заказ';en='Order'");
		Элементы.ПланДоставкиПредставлениеГруппировки.Заголовок = НСтр("ka='შეკვეთა';ru='Заказ';en='Order'");
...
Функция СостоянияЗаказовЗаполнены()
	...
	Пока Выборка.Следующий() Цикл
		Если ФормаПараметры.НесколькоВидовЗаказов Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Не задано состояние доставленных заказов для вида %1';ru='Не задано состояние доставленных заказов для вида %1';en='Do not set delivered state orders for the type of% 1'"),
			Выборка.ВидЗаказа),
			Выборка.ВидЗаказа,
			"СостояниеДоставлен",,);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не задано состояние доставленных заказов';ru='Не задано состояние доставленных заказов';en='state orders delivered is not specified'"),
			Выборка.ВидЗаказа,
			"СостояниеДоставлен",,);
		КонецЕсли;
	КонецЦикла;
