...
Функция ПредставлениеПФ(ЧастичнаяОплата = Ложь, ИспользоватьФаксимиле = Ложь) Экспорт
	...
	Если ЧастичнаяОплата Тогда 
		...
		ПредставлениеПФ = НСтр("ka='Счет на частичную оплату';ru='Счет на частичную оплату'");
		
	Иначе
		
		ПредставлениеПФ = НСтр("ka='გადახდის მოთხოვნა';ru='Счет на оплату'");
...
Функция СформироватьПФ(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ОбъектыПечати, ВывестиПланОплаты) Экспорт
		...
		Если ТипЗнч(ДанныеОбъекта.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			Если ДанныеОбъекта.ОжидаетсяВыборВариантаКП Тогда
				...
				Сообщение.Текст = СтрШаблон(
					НСтр("ka='Печатная форма ""%1"" (%2) не может быть сформирована: не выбран основной вариант.';ru='Печатная форма ""%1"" (%2) не может быть сформирована: не выбран основной вариант.'"),
	...
	КонецЦикла;
		...
		Если ВывестиПланОплаты И ЕстьТЧПланОплаты Тогда
			...
			ОбластьМакета = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "ИтогоНДСЧастичнаяОплата", НСтр("ka='Итого НДС по частичной оплате';ru='Итого НДС по частичной оплате'"), Ошибки);
			Если ОбластьМакета <> Неопределено Тогда
		...
		Если СтруктураИтогов.Вес<>0 ИЛИ (ДанныеОбъектовПечати.Колонки.Найти("Вес")<>Неопределено И ДанныеОбъекта.Вес<>0) Тогда
			...
			ОбластьМакета = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "ИтогоКОплатеСВесом", НСтр("ka='Итого к оплате и вес';ru='Итого к оплате и вес'"), Ошибки);
			Если ОбластьМакета <> Неопределено Тогда
			...
			Иначе
				...
				ОбластьМакета = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "ИтогоКОплате", НСтр("ka='Итого к оплате';ru='Итого к оплате'"), Ошибки);
				Если ОбластьМакета <> Неопределено Тогда
		...
		ОбластьМакета = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "СуммаПрописью", НСтр("ka='Итого к оплате';ru='Итого к оплате'"), Ошибки);
		Если ОбластьМакета <> Неопределено Тогда
			
			ИтоговаяСтрока = НСтр("ka='სულ არის %2 ოდენობის %1 დასახელება';ru='Всего наименований %1, на сумму %2'");
