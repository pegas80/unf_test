...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.АдресВХранилище) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Иначе
				ВызватьИсключение НСтр("ka='Режим печати не установлен';ru='Режим печати не установлен'");;
			КонецЕсли;
	...
	Если ТекущиеДанные.Выбран Тогда
		...
		Если Объект.Режим = "ПечатьЦенниковИЭтикеток" Тогда
			...
			Если (ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение.Текст = НСтр("ka='Не заполнено количество ценников';ru='Не заполнено количество ценников'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не выбран шаблон ценника';ru='Не выбран шаблон ценника'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество этикеток';ru='Не заполнено количество этикеток'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не выбран шаблон этикетки';ru='Не выбран шаблон этикетки'");
			...
			ИначеЕсли (   ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество ценников и(или) этикеток';ru='Не заполнено количество ценников и(или) этикеток'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЦенников";
				Сообщение.Сообщить();
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не выбраны шаблоны ценников и(или) этикеток';ru='Не выбраны шаблоны ценников и(или) этикеток'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].ШаблонЦенника";
				Сообщение.Сообщить();
			...
			ИначеЕсли (   ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество ценников и(или) шаблон этикетки';ru='Не заполнено количество ценников и(или) шаблон этикетки'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЦенников";
				Сообщение.Сообщить();
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество этикеток и(или) шаблон ценника';ru='Не заполнено количество этикеток и(или) шаблон ценника'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЭтикеток";
				Сообщение.Сообщить();
...
Процедура ЗаполнитьТаблицуЗапасов(Команда)
	...
	ТекстВопроса = НСтр("ka='При перезаполнении все введенные вручную данные будут потеряны, продолжить?';ru='При перезаполнении все введенные вручную данные будут потеряны, продолжить?'");
...
Процедура Печать(Команда)
	...
	Если НЕ НеобходимостьПечати.ЕстьВыбранныеСтроки Тогда
		...
		ПоказатьПредупреждение(Неопределено,НСтр("ka='არ არის არჩეული არცერთი საქონელი';ru='Не выбрано ни одного товара'"));
		Возврат;
	...
	Если ЭтаФорма.ПроверитьЗаполнение() Тогда
		...
		ПараметрыПечати.Вставить("ЗаголовокФормы", НСтр("ka='ფასდებულების და ეტიკეტების ბეჭდვა';ru='Печать этикеток и ценников'"));
...
Процедура УстановитьКоличествоЦенников(Команда)
	...
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьКоличествоЦенниковЗавершение", ЭтотОбъект, Новый Структура("ВыбранноеЗначение", ВыбранноеЗначение)), ВыбранноеЗначение, НСтр("ka='Введите количество ценников';ru='Введите количество ценников'"), 10, 2);
...
Процедура УстановитьКоличествоЦенниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ka='Установлено количество ценников %Количество%.';ru='Установлено количество ценников %Количество%.'");
...
Процедура УстановитьКоличествоЭтикетокЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ka='Установлено количество этикеток %Количество%.';ru='Установлено количество этикеток %Количество%.'");
		Текст = СтрЗаменить(Текст, "%Количество%", ВыбранноеЗначение);
...
Процедура УстановитьШаблонЦенниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ka='Установлен шаблон ""%Шаблон%"".';ru='Установлен шаблон ""%Шаблон%"".'");
...
Процедура УстановитьРежим(ПараметрРежим)
	...
	ИначеЕсли ПараметрРежим = "ПечатьЭтикеток" Тогда
		УстановитьРежимПечатьЭтикетокНаСервере();
	Иначе
		ВызватьИсключение НСтр("ka='Установленный режим печати не поддерживается';ru='Установленный режим печати не поддерживается'");
	КонецЕсли;
...
Функция ПоказатьОповещениеПользователюОВозможнойОшибке(Текст, Количество, КоличествоВсего)
	...
	Если Количество < КоличествоВсего Тогда
		...
		Текст = Текст
		        + ?(Текст <> "", Символы.ПС, "")
		        + НСтр("ka='Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.';ru='Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.'");
	
		Текст = Текст
		        + Символы.ПС
		        + НСтр("ka='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.';ru='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.'");
