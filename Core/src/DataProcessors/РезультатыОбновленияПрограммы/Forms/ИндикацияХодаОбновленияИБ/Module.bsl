...
"Правильный формат - ""ЧислоПотоковОбновления=Х"", где ""Х"" - максимальное количество потоков обновления.';ka='Неправильно указан параметр запуска программы ""ЧислоПотоковОбновления""."
"Правильный формат - ""ЧислоПотоковОбновления=Х"", где ""Х"" - максимальное количество потоков обновления.'");
	...
	Если ТолькоОбновлениеПараметровРаботыПрограммы Тогда
		Заголовок = НСтр("ka='Обновление параметров работы программы';ru='Обновление параметров работы программы';en='Updating the program of work'");
	...
	ИначеЕсли РежимОбновленияДанных = "НачальноеЗаполнение" Тогда
		Заголовок = НСтр("ka='Начальное заполнение данных';ru='Начальное заполнение данных';en='The initial data fill'");
	...
	ИначеЕсли РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Заголовок = НСтр("ka='Переход с другой программы';ru='Переход с другой программы';en='Switching from another program'");
	...
	Иначе
		Заголовок = НСтр("ka='პროგრამის ვერსიის განახლება';ru='Обновление версии программы';en='Update version'");
...
Функция ЗагрузитьОбновитьПараметрыРаботыПрограммыВФоне()
	...
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ka='პროგრამის პარამეტრების ფონის განახლება';ru='Фоновое обновление параметров работы программы';en='Background update application settings'");
...
Процедура ЗавершитьОбновлениеПараметровРаботыПрограммы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		КраткоеСообщениеОбОшибке = НСтр("ka='შეცდომა ფონური სამუსაოების შედეგის მიღებისას პროგრამის პარამეტრების განახლების დროს.';ru='Ошибка получения результата от фонового задания при обновлении параметров работы программы.';en='Failed to get result from a background job during the upgrade application settings.'");
...
Функция ЗапускЗадания()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Фоновая регистрация данных отложенного обновления';ru='Фоновая регистрация данных отложенного обновления';en='Background data recording deferred updates'");
