...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		ТекстПредупреждения = НСтр("ka='Обработка данных еще не выполнялась.';ru='Обработка данных еще не выполнялась.'");
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	Если ОбработчикиНеВыполнялись Тогда
		Элементы.ТекстПояснения.Заголовок = НСтр("ka='Рекомендуется запустить невыполненные процедуры обработки данных.';ru='Рекомендуется запустить невыполненные процедуры обработки данных.'");
	Иначе
		Элементы.ТекстПояснения.Заголовок = НСтр("ka='Невыполненные процедуры рекомендуется запустить повторно.';ru='Невыполненные процедуры рекомендуется запустить повторно.'");
	КонецЕсли;
...
"%2'");
	...
	ШаблонИнформацияОПроцедуреОбновления = НСтр("ka='%1%2Процедура ""%3"" обработки данных %4.';ru='%1%2Процедура ""%3"" обработки данных %4.'");
	...
	Если СтрокаОбработчика.Статус = "Выполнено" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ka='завершилась успешно';ru='завершилась успешно'");
		СтрокаСписка.СтатусПредставление = НСтр("ka='შესრულებულია';ru='Выполнено'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Выполняется" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ka='в данный момент выполняется';ru='в данный момент выполняется'");
		Если Прогресс <> Неопределено Тогда
			ШаблонСтатус = НСтр("ka='Выполняется (%1%)';ru='Выполняется (%1%)'");
			СтрокаСписка.СтатусПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтатус, Прогресс)
		Иначе
			СтрокаСписка.СтатусПредставление = НСтр("ka='სრულდება';ru='Выполняется'");
		КонецЕсли;
	ИначеЕсли СтрокаОбработчика.Статус = "Ошибка" Тогда
		
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ka='Процедура ""%1"" обработки данных завершилась с ошибкой:';ru='Процедура ""%1"" обработки данных завершилась с ошибкой:'") + Символы.ПС + Символы.ПС;
		...
		СтрокаСписка.СтатусПредставление = НСтр("ka='შეცდომა';ru='Ошибка'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Приостановлен" Тогда
		...
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ka='остановлена администратором';ru='остановлена администратором'");
		СтрокаСписка.СтатусПредставление = НСтр("ka='Остановлено';ru='Остановлено'");
		СтрокаСписка.СтатусКартинка    = БиблиотекаКартинок.ЗнакСтоп;
	Иначе
		
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ka='еще не выполнялась';ru='еще не выполнялась'");
		СтрокаСписка.СтатусПредставление = НСтр("ka='Не выполнялась';ru='Не выполнялась'");
	КонецЕсли;
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ШаблонСекунды = НСтр("ka='%1 сек.';ru='%1 сек.'");
	ШаблонМинуты = НСтр("ka='%1 мин. %2 сек.';ru='%1 мин. %2 сек.'");
	ШаблонЧасы = НСтр("ka='%1 ч. %2 мин.';ru='%1 ч. %2 мин.'");
	...
	Если ДлительностьВСекундах < 1 Тогда
		Возврат НСтр("ka='менее секунды';ru='менее секунды'")
