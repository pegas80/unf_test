...
Процедура ЗаполнитьГруппыКатегории()
	...
	ИначеЕсли КоличествоГрупп = 0 Тогда
	    ОтборКатегорииГруппыПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры;
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ka='Выберите категории номенклатуры для переноса на сайт';ru='Выберите категории номенклатуры для переноса на сайт'");
	...
	ИначеЕсли КоличествоКатегорий = 0 Тогда
		...
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ka='Выберите группы номенклатуры для переноса на сайт';ru='Выберите группы номенклатуры для переноса на сайт'");
...
Процедура ЗаполнитьДеревоГрупп()
	...
	СтрокаВсеГруппы.Представление= НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>'");
	СтрокаВсеГруппы.Флаг = Истина;
	
	СтрокаВсеГруппы = Дерево.Строки.Добавить();
	СтрокаВсеГруппы.Значение	 = Справочники.Номенклатура.ПустаяСсылка();
	СтрокаВсеГруппы.Представление= НСтр("ka='<არ არის ჯგუფი>';ru='<Нет группы>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Процедура ЗаполнитьДеревоКатегорий()
	...
	СтрокаВсеГруппы.Представление= НСтр("ka='<Все категории>';ru='<Все категории>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Процедура ПометкаИерархииПриИзменении()
	...
	Если ТекущиеДанные.Представление = НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>'") Тогда
		//Все элементы дерева
...
Функция СоздатьСайтВФоне()
	...
	Заголовок = НСтр("ka='Создание сайта';ru='Создание сайта'");
...
Функция СоздатьСайтВФонеПродолжить(ПараметрыСозданияСайта)
	...
	Заголовок = НСтр("ka='Заполнение сайта';ru='Заполнение сайта'");
...
Функция ПолучитьТаблицуКаталогов()
	...
	Если НЕ ВсеГруппы Тогда
		...
		Для Каждого ЭлементСЗ Из СписокГрупп Цикл
			...
			Если ЭлементСЗ.Представление = НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>'") Тогда
	...
	НовСтр.Каталог = НСтр("ka='საქონლის ძირითადი კატალოგი';ru='Основной каталог товаров'");
...
Процедура СоздатьСайт(Команда)
	...
	Если ТипСайта=0 Тогда
		Сообщить(НСтр("ka='Выберите тип сайта!';ru='Выберите тип сайта!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ПринимаюСоглашение Тогда
		Сообщить(НСтр("ka='Чтобы создать сайт, нужно принять лицензионное соглашение!';ru='Чтобы создать сайт, нужно принять лицензионное соглашение!'"));
		Возврат;
	КонецЕсли;
	
	Если СокрЛП(EmailРегистрацииСайта)="" Тогда
		Сообщить(НСтр("ka='Укажите e-mail, на который будет зарегистрирован сайт!';ru='Укажите e-mail, на который будет зарегистрирован сайт!'"));
		Возврат;
	КонецЕсли;
	...
	Если НЕ ИмяСайтаСвободно Тогда
		Сообщить(НСтр("ka='Предложенное имя сайта занято, выберите другое!';ru='Предложенное имя сайта занято, выберите другое!'"));
		Возврат;
	КонецЕсли;
...
Процедура СоздатьСайтПродолжить()
	...
	ИначеЕсли ДанныеСайтаСтруктура.Свойство("СайтСоздан") И ДанныеСайтаСтруктура.СайтСоздан = Ложь Тогда
		...
		Если (ТекущаяДата() - СчетчикОжиданияСозданияСайта)>300 Тогда // ждем создания сайта не более 5 мин, потом выдаем ошибку
			ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
			Сообщить(НСтр("ka='Время ответа от сайта превышено, попробуйте повторить операцию позже';ru='Время ответа от сайта превышено, попробуйте повторить операцию позже'"));
			Возврат;
		Иначе	
			ПодключитьОбработчикОжидания("СоздатьСайтПродолжить", 2, Истина);
		КонецЕсли;
...
Функция ПроверитьИмяСайтаТекст(Текст)
	...
	Пока н <= СтрДлина(Текст) Цикл
		...
		Если РазрешенныеСимволыИмениСайта.Получить(НРег(ПроверСимвол)) = Неопределено Тогда
			НадписьИмяСайтаДоступно = НСтр("ka='Разрешены латинские буквы и цифры';ru='Разрешены латинские буквы и цифры'");
			Возврат Ложь;
		Иначе
			н=н+1;
		КонецЕсли;
...
Функция ОбновитьНадписьИмяСайтаДоступно(ИмяСайтаПроверено, ИмяСайтаСвободно)
	...
	Если НЕ ИмяСайтаПроверено Тогда
		Возврат НСтр("ka='Проверяется ...';ru='Проверяется ...'");
	ИначеЕсли ИмяСайтаСвободно Тогда
		Возврат НСтр("ka='Имя свободно!';ru='Имя свободно!'");
	Иначе
		Возврат НСтр("ka='Имя занято!';ru='Имя занято!'");
	КонецЕсли;
