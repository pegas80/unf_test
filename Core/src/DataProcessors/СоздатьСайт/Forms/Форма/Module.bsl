...
Процедура ЗаполнитьГруппыКатегории()
	...
	ИначеЕсли КоличествоГрупп = 0 Тогда
	    ОтборКатегорииГруппыПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры;
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ka='Выберите категории номенклатуры для переноса на сайт';ru='Выберите категории номенклатуры для переноса на сайт';en='Select the range of categories for the transfer of the site'");
	...
	ИначеЕсли КоличествоКатегорий = 0 Тогда
		...
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ka='Выберите группы номенклатуры для переноса на сайт';ru='Выберите группы номенклатуры для переноса на сайт';en='Select the range of the group for the transfer of the site'");
...
Процедура ЗаполнитьДеревоКатегорий()
	...
	СтрокаВсеГруппы.Представление= НСтр("ka='<Все категории>';ru='<Все категории>';en='<All categories>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Функция СоздатьСайтВФоне()
	...
	Заголовок = НСтр("ka='Создание сайта';ru='Создание сайта';en='Create site'");
...
Функция СоздатьСайтВФонеПродолжить(ПараметрыСозданияСайта)
	...
	Заголовок = НСтр("ka='Заполнение сайта';ru='Заполнение сайта';en='filling the site'");
...
Процедура СоздатьСайт(Команда)
	...
	Если ТипСайта=0 Тогда
		Сообщить(НСтр("ka='Выберите тип сайта!';ru='Выберите тип сайта!';en='Select the type of site!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ПринимаюСоглашение Тогда
		Сообщить(НСтр("ka='Чтобы создать сайт, нужно принять лицензионное соглашение!';ru='Чтобы создать сайт, нужно принять лицензионное соглашение!';en='To create a website, you need to accept the license agreement!'"));
		Возврат;
	КонецЕсли;
	
	Если СокрЛП(EmailРегистрацииСайта)="" Тогда
		Сообщить(НСтр("ka='Укажите e-mail, на который будет зарегистрирован сайт!';ru='Укажите e-mail, на который будет зарегистрирован сайт!';en='Enter the e-mail, which will be registered with the site!'"));
		Возврат;
	КонецЕсли;
	...
	Если НЕ ИмяСайтаСвободно Тогда
		Сообщить(НСтр("ka='Предложенное имя сайта занято, выберите другое!';ru='Предложенное имя сайта занято, выберите другое!';en='The proposed name of the site is occupied, choose another!'"));
		Возврат;
	КонецЕсли;
...
Процедура СоздатьСайтПродолжить()
	...
	ИначеЕсли ДанныеСайтаСтруктура.Свойство("СайтСоздан") И ДанныеСайтаСтруктура.СайтСоздан = Ложь Тогда
		...
		Если (ТекущаяДата() - СчетчикОжиданияСозданияСайта)>300 Тогда // ждем создания сайта не более 5 мин, потом выдаем ошибку
			ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
			Сообщить(НСтр("ka='Время ответа от сайта превышено, попробуйте повторить операцию позже';ru='Время ответа от сайта превышено, попробуйте повторить операцию позже';en='The response time of the site is exceeded, try again later'"));
			Возврат;
		Иначе	
			ПодключитьОбработчикОжидания("СоздатьСайтПродолжить", 2, Истина);
		КонецЕсли;
...
Процедура ОткрытьСозданныйСайт(ДанныеСозданияСайтаСтруктура)
	...
	ПоказатьОповещениеПользователя(,,НСтр("ru='Создан сайт: ';ka='Создан сайт: '")+ДанныеСозданияСайтаСтруктура.АдресСайта,БиблиотекаКартинок.Информация32);
	ОбновитьИнтерфейс();
	Закрыть();
...
Функция ПолучитьДанныеЗапросом(HTTPМетод, ТелоЗапроса, АдресЗапроса, ЗаголовкиЗапроса, 
	...
	Исключение
		...
		Если СтрНайти(ТекстОшибки, "Couldn't resolve host name")<>0 Тогда
		    ТекстОшибки = НСтр("ka='Ошибка: Не могу подключиться к интернету!';ru='Ошибка: Не могу подключиться к интернету!';en='Error: Could not connect to the internet!'");
		КонецЕсли;
		Возврат "";
	КонецПопытки;
...
Функция ПроверитьИмяСайтаТекст(Текст)
	...
	Пока н <= СтрДлина(Текст) Цикл
		...
		Если РазрешенныеСимволыИмениСайта.Получить(НРег(ПроверСимвол)) = Неопределено Тогда
			НадписьИмяСайтаДоступно = НСтр("ka='Разрешены латинские буквы и цифры';ru='Разрешены латинские буквы и цифры';en='Allowed Latin letters and numbers'");
			Возврат Ложь;
		Иначе
			н=н+1;
		КонецЕсли;
...
Функция ОбновитьНадписьИмяСайтаДоступно(ИмяСайтаПроверено, ИмяСайтаСвободно)
	...
	Если НЕ ИмяСайтаПроверено Тогда
		Возврат НСтр("ka='Проверяется ...';ru='Проверяется ...';en='Checked ...'");
	ИначеЕсли ИмяСайтаСвободно Тогда
		Возврат НСтр("ka='Имя свободно!';ru='Имя свободно!';en='Name is available!'");
	Иначе
		Возврат НСтр("ka='Имя занято!';ru='Имя занято!';en='Name Busy!'");
	КонецЕсли;
