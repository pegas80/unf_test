...
Процедура КнопкаВыполнить(Команда)
	...
	Если ТолькоВыбранныеОрганизации = 1 И НЕ ЗначениеЗаполнено(ТаблицаОрганизаций) Тогда
		ТекстСообщения = НСтр("ka='Не корректно заполнен список организаций.';ru='Не корректно заполнен список организаций.';en='Not correctly filled with a list of organizations.'");
...
Процедура ПоказатьРезультатПерепроведения()
	...
	ТекстСообщения = НСтр("ka='Выполнено перепроведение документов:"
" - проведено документов: %1;"
"%2';ru='Выполнено перепроведение документов:"
" - проведено документов: %1;"
"%2'");
	Если НеУдалосьПровести = 0 Тогда
		ТекстСообщенияОбОбшибках = НСтр("ka=' - ошибок не обнаружено.';ru=' - ошибок не обнаружено.'");
	Иначе
		ТекстСообщенияОбОбшибках = НСтр("ka=' - не удалось провести документов: %1';ru=' - не удалось провести документов: %1'");
...
Процедура ИнициализацияКомандДокументаНаКлиенте()
	...
	Если Результат = 1 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='Перепроведение уже выполняется. Ожидайте завершения.';ru='Перепроведение уже выполняется. Ожидайте завершения.';en='Reposting is already running. Expect to complete.'"));
		Возврат;
	ИначеЕсли Результат = 2 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='Перепроведение уже выполняется другим пользователем.';ru='Перепроведение уже выполняется другим пользователем.';en='Reposting another user is already in progress.'"));
		Возврат;
	КонецЕсли;
