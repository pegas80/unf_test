...
Процедура ПерепроведениеДокументов(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Для Индекс = 0 ПО ВсегоДокументов - 1 Цикл
		...
		Если Тип("ДокументСсылка.ЧекККМ") = ТипЗнч(СтрокаДокумента.Ссылка) Или Тип("ДокументСсылка.ЗаданиеНаРаботу") = ТипЗнч(СтрокаДокумента.Ссылка) 
			Или Тип("ДокументСсылка.ЗакрытиеМесяца") = ТипЗнч(СтрокаДокумента.Ссылка) Тогда
			Продолжить;	
		КонецЕсли; 
		...
		Исключение
			...
			ТекстСообщения = НСтр("ka='Документ %1 не проведен! %2 По причине: %3';ru='Документ %1 не проведен! %2 По причине: %3';en='Document% 1 is not carried out! Due% 2% 3'");
...
"Необходимо исправить указанные ошибки, после чего принять решение о повторном перепроведении документов.';ka='При перепроведении выявлено %1 ошибок. "
"Сообщения о первых 50 ошибках предоставлены к ознакомлению. "
