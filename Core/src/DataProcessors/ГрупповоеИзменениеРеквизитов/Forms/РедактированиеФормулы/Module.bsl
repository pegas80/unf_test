...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ОперандыОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные.ПометкаУдаления Тогда
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ОперандыОкончаниеПеретаскиванияЗавершение", ЭтотОбъект), НСтр("ka='Выбранный элемент помечен на удаление';ru='Выбранный элемент помечен на удаление'") + Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Функция ПолучитьСтандартноеДеревоОператоров()
	...
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ka='Разделители';ru='Разделители'"));
	...
	ДобавитьОператор(Дерево, ГруппаОператоров, ",", " + "", "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ".", " + "". "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Пробел';ru='Пробел'"), " + "" "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "(", " + "" ("" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ")", " + "") "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, """", " + """""""" + ");
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ka='ოპერატორები';ru='Операторы'"));
	...
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ka='Логические операторы и константы';ru='Логические операторы и константы'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, "<", " < ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ">", " > ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "<=", " <= ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ">=", " >= ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "=", " = ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "<>", " <> ");
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='и';ru='И'"),      " " + НСтр("ka='и';ru='И'")      + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Или';ru='Или'"),    " " + НСтр("ka='Или';ru='Или'")    + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Не';ru='Не'"),     " " + НСтр("ka='Не';ru='Не'")     + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='ჭეშმარიტება';ru='ИСТИНА'"), " " + НСтр("ka='ჭეშმარიტება';ru='ИСТИНА'") + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='ЛОЖЬ';ru='ЛОЖЬ'"),   " " + НСтр("ka='ЛОЖЬ';ru='ЛОЖЬ'")   + " ");
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ka='Числовые функции';ru='Числовые функции'"));
	
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='მაქსიმუმი';ru='Максимум'"),    НСтр("ka='მაქს(,)';ru='Макс(,)'"), 2);
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='მინიმუმი';ru='Минимум'"),     НСтр("ka='წთ(,)';ru='Мин(,)'"),  2);
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='დამრგვალება';ru='Округление'"),  НСтр("ru='Окр(,)'"),  2);
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='მთელი ნაწილი';ru='Целая часть'"), НСтр("ru='Цел()'"),   1);
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ka='Строковые функции';ru='Строковые функции'"));
	
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Строка';ru='Строка'"), НСтр("ru='Строка()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='ВРег';ru='ВРег'"), НСтр("ka='ВРег()';ru='ВРег()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Лев';ru='Лев'"), НСтр("ka='Лев()';ru='Лев()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='НРег';ru='НРег'"), НСтр("ka='НРег()';ru='НРег()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Прав';ru='Прав'"), НСтр("ka='Прав()';ru='Прав()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='СокрЛ';ru='СокрЛ'"), НСтр("ka='СокрЛ()';ru='СокрЛ()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='СокрЛП';ru='СокрЛП'"), НСтр("ka='СокрЛП()';ru='СокрЛП()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='СокрП';ru='СокрП'"), НСтр("ka='СокрП()';ru='СокрП()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='ТРег';ru='ТРег'"), НСтр("ka='ТРег()';ru='ТРег()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='СтрЗаменить';ru='СтрЗаменить'"), НСтр("ka='СтрЗаменить(,,)';ru='СтрЗаменить(,,)'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='СтрДлина';ru='СтрДлина'"), НСтр("ka='СтрДлина()';ru='СтрДлина()'"));
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ka='Прочие функции';ru='Прочие функции'"));
	
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='პირობა';ru='Условие'"), "?(,,)", 3);
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='Предопределенное значение';ru='Предопределенное значение'"), НСтр("ka='ПредопределенноеЗначение()';ru='ПредопределенноеЗначение()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='მნიშვნელობა შევსებულია';ru='Значение заполнено'"), НСтр("ka='მნიშვნელობაშევსებულია()';ru='ЗначениеЗаполнено()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ka='ფორმატი';ru='Формат'"), НСтр("ru='Формат(,)'"));
...
Функция ПроверитьФормулу(Формула, Операнды, Знач Поле = "", СтроковаяФормула = Ложь)
	...
	Если ЗначениеЗаполнено(Формула) Тогда
			...
			Если СтроковаяФормула Тогда
				...
				Если ОтсутствиеРазделителей > 0 Тогда
					СообщитьПользователю(
						НСтр("ka='В формуле обнаружены ошибки. Между операндами должен присутствовать оператор или разделитель';ru='В формуле обнаружены ошибки. Между операндами должен присутствовать оператор или разделитель'"),
						Поле,
						ПутьКДанным);
					Результат = Ложь;
				КонецЕсли;
		...
		Исключение
			Результат = Ложь;
			СообщитьПользователю(
				НСтр("ka='В формуле обнаружены ошибки. Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке 1С:Предприятия.';ru='В формуле обнаружены ошибки. Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке 1С:Предприятия.'"),
				Поле,
				ПутьКДанным);
		КонецПопытки;
	...
	Если ЗначениеЗаполнено(Формула) Тогда
		Если ПроверитьФормулу(Формула, Операнды, Поле, СтроковаяФормула) Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='В формуле ошибок не обнаружено';ru='В формуле ошибок не обнаружено'"),
				,
				,
