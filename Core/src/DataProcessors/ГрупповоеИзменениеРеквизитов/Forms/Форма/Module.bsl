...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Заголовок = НСтр("ka='რეკვიზიტების ჯგუფური ცვლილება';ru='Групповое изменение реквизитов';en='Bulk change details'");
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='შესრულება';ru='Выполнить';en='Run'");
	...
	Иначе
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='Изменить реквизиты';ru='Изменить реквизиты';en='change details'");
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		Если КоличествоОбъектовДляОбработки = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Не указаны элементы для изменения';ru='Не указаны элементы для изменения';en='Not listed items to change'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ТекстВопроса = НСтр("ka='Отбор не задан. Изменить все элементы?';ru='Отбор не задан. Изменить все элементы?';en='The selection is not specified. Change all the elements?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкаОтбораВыполнена", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("ka='Изменение элементов';ru='Изменение элементов';en='Edit items'"));
		КонецЕсли;
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() И Объект.ТипОперации = "ВвестиЗначения" Тогда
		ТекстВопроса = НСтр("ka='Изменения не настроены. Выполнить перезапись элементов без изменений?';ru='Изменения не настроены. Выполнить перезапись элементов без изменений?';en='Changes are not set. Dub elements intact?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка = НСтр("ka='Изменение выделенных элементов ""%1"" (%2)';ru='Изменение выделенных элементов ""%1"" (%2)';en='Changing the selected items ""%1"" (2%)'");
	...
	Если ТипыОбъектов.Количество() > 1 Тогда
		ПредставлениеТипа = "";
		ШаблонЗаголовка = НСтр("ka='Изменение выделенных элементов (%2)';ru='Изменение выделенных элементов (%2)';en='Changing the selected elements (2%)'");
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов(ВыбранныйРеквизит)
	...
	Если КонтекстныйВызов Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='გადასვლა';ru='Перейти';en='Go'"));
		ТекстВопроса = НСтр("ka='Реквизит заблокирован, перейти к разблокированию реквизитов?';ru='Реквизит заблокирован, перейти к разблокированию реквизитов?';en='Props blocked, move to unblock the details?'");
	Иначе
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Разблокировать';ru='Разблокировать';en='release'"));
...
""
"Разблокировать реквизит ""%1"" для изменения?';ka='Для того чтобы не допустить рассогласования данных в программе,"
"реквизит ""%1"" не доступен для редактирования."
""
"Перед тем, как разрешить его редактирование, рекомендуется оценить последствия,"
"проверив все места использования выбранных элементов в программе."
...
"Разблокировать реквизит ""%1"" для изменения?'"),
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Да, НСтр("ka='Реквизит заблокирован';ru='Реквизит заблокирован';en='props blocked'"));
...
Процедура УстановитьПараметрыВыбораСервер(ПараметрыВыбора, ПараметрыВыбораМассив)
		...
		Для Индекс = 1 По СтрЧислоСтрок(ПараметрыВыбора) Цикл
			...
			//+++ Дмгг
			//Небольшой костыль, программа перевела имя типа на грузинский
			ИмяТипа       = ?(ИмяТипа = "ლოგიკურად", "Булево", ИмяТипа);
			//--- Дмгг
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Если НачалоИзменения Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать';en='abort'");
	Иначе
		Если ОбъектыКоторыеНеУдалосьИзменить.Количество() > 0 Тогда
			Элементы.ФормаИзменить.Заголовок = НСтр("ka='Повторить изменение';ru='Повторить изменение';en='repeat change'");
		Иначе
			Элементы.ФормаИзменить.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ka='Изменение выделенных элементов';ru='Изменение выделенных элементов';en='Change the selected elements'"), ,НСтр("ka='Пожалуйста подождите, обработка может занять некоторое время...';ru='Пожалуйста подождите, обработка может занять некоторое время...';en='Please wait, processing may take some time ...'"));
	...
	Иначе
		...
		Состояние(НСтр("ka='Обрабатываются элементы...';ru='Обрабатываются элементы...';en='Processed items ...'"), 0, НСтр("ka='Изменение выделенных элементов';ru='Изменение выделенных элементов';en='Change the selected elements'"));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		Если Объект.ИзменятьВТранзакции Тогда
			ТекстПредупреждения = НСтр("ka='При изменении элементов обнаружены ошибки - изменения отменены.';ru='При изменении элементов обнаружены ошибки - изменения отменены.';en='If you change the elements contain errors - changes are canceled.'");
...
"';ka='При изменении элементов (группы элементов) возникли ошибки."
"Прервать изменение элементов и перейти к просмотру ошибок?"
"'");
		Кнопки = Новый СписокЗначений;
		...
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='აღარ მკითხო';ru='Больше не спрашивать';en='Do not ask'"));
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьРезультатИзмененияОтветПолучен", ЭтотОбъект, РезультатИзменения);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("ka='Ошибки при изменении элементов';ru='Ошибки при изменении элементов';en='Errors when changing elements'"));
		Возврат;
	КонецЦикла;
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ka='Обрабатываются элементы...';ru='Обрабатываются элементы...';en='Processed items ...'"), ТекущийПроцент, НСтр("ka='Изменение выделенных элементов';ru='Изменение выделенных элементов';en='Change the selected elements'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='Изменение реквизитов элементов';ru='Изменение реквизитов элементов';en='Changing the details of elements'"), , 
			ПодставитьПараметрыВСтроку(НСтр("ka='Изменены элементы (%1).';ru='Изменены элементы (%1).';en='Modified elements (1%).'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = НСтр("ka='Изменения выполнены во всех выбранных элементах (%2).';ru='Изменения выполнены во всех выбранных элементах (%2).';en='Changes are made in all of the selected elements (2%).'");
	Иначе
		Если Объект.ИзменятьВТранзакции Или ТекущееСостояниеИзменения.КоличествоИзмененных = 0 Тогда
			ШаблонСообщения = НСтр("ka='Изменения не выполнены.';ru='Изменения не выполнены.';en='Changes are not made.'");
		Иначе
			ШаблонСообщения = НСтр("ka='Изменения выполнены частично."
"Изменено: %1; Не удалось изменить: %3';ru='Изменения выполнены частично."
"Изменено: %1; Не удалось изменить: %3'");
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = ПодставитьПараметрыВСтроку(НСтр("ka='... и другие элементы (%1)';ru='... и другие элементы (%1)';en='... and other elements (1%)'"), КоличествоПропущенных);
			СтрокаТаблицы.Причина = НСтр("ka='Пропущены, так как не были изменены один или более элементов.';ru='Пропущены, так как не были изменены один или более элементов.';en='Skipped, since it is not one or more elements have been changed.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НазадСервер()
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='შესრულება';ru='Выполнить';en='Run'");
		Элементы.ФормаИзменить.РасширеннаяПодсказка.Заголовок = НСтр("ka='Выполнить алгоритм';ru='Выполнить алгоритм';en='Run algorithm'");
	Иначе
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='Изменить реквизиты';ru='Изменить реквизиты';en='change details'");
	КонецЕсли;
...
"Всего изменено элементов: %1';ka='Реквизиты выбранных элементов были изменены."
"Всего изменено элементов: %1'"), ТекущееСостояниеИзменения.КоличествоИзмененных);
	Элементы.ФормаИзменить.Заголовок = НСтр("ka='მზად არის';ru='Готово';en='Done'");
	Элементы.ФормаНазад.Видимость = Истина;
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		НаименованиеЗадания = НСтр("ka='Групповое изменение элементов';ru='Групповое изменение элементов';en='Batch editing elements'");
...
Процедура СформироватьПояснениеПоАвтонумерации()
	...
	ИначеЕсли Автонумерация Тогда
		ПояснениеПоАвтонумерации = НСтр("ka='<Установить автоматически>';ru='<Установить автоматически>';en='<Set automatically>'");
	Иначе
		ПояснениеПоАвтонумерации = НСтр("ka='<Очистить>';ru='<Очистить>';en='<Delete>'");
	КонецЕсли;
...
Функция ОписанияКолонокТаблицыРеквизитов()
	...
	ОписаниеКолонки.Тип = Новый ОписаниеТипов("Строка");
	ОписаниеКолонки.Представление = НСтр("ka='რეკვიზიტი';ru='Реквизит';en='Props'");
	...
	ОписаниеКолонки.Представление = НСтр("ka='ახალი მნიშვნელობა';ru='Новое значение';en='The new value'");
	ОписаниеКолонки.ВидПоля = ВидПоляФормы.ПолеВвода;
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
		Пояснение = НСтр("ka='реквизит';ru='реквизит';en='props'") + " " + Пояснение;
	ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
		Пояснение = НСтр("ka='реквизиты';ru='реквизиты';en='requisites'") + " " + Пояснение;
	КонецЕсли;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Пояснение = Пояснение + " " + НСтр("ka='в выбранных элементах';ru='в выбранных элементах';en='in selected items'");
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			Если Не ПустаяСтрока(Пояснение) Тогда
				Пояснение = Пояснение + ", ";
			КонецЕсли;
			Пояснение = Пояснение + ПодставитьПараметрыВСтроку(НСтр("ka='реквизиты (%1)';ru='реквизиты (%1)';en='Props (1%)'"), ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
				...
				Если ИзменяемыеРеквизиты.Найти(Реквизит) = 0 Тогда
					Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
						Пояснение = Пояснение + НСтр("ka='реквизит';ru='реквизит';en='props'") + " ";
					ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
						Пояснение =  Пояснение + НСтр("ka='реквизиты';ru='реквизиты';en='requisites'") + " ";
					КонецЕсли;
				КонецЕсли;
				Пояснение = Пояснение + """" + Реквизит + """";
			КонецЦикла;
		КонецЕсли;
		Пояснение = Пояснение + " " 
			+ ПодставитьПараметрыВСтроку(НСтр("ka='в табличной части ""%1""';ru='в табличной части ""%1""';en='in the tables of ""%1""'"), ТабличнаяЧасть.Ключ);
	КонецЦикла;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("ka='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';ru='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';en='in the rows selected items that meet the selection criteria'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("ka='<b>во всех строках</b> выбранных элементов';ru='<b>во всех строках</b> выбранных элементов';en='<B> all lines </ b> selections'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		Если Не ПустаяСтрока(Пояснение) Тогда
			Пояснение = НСтр("ka='შეცვლა';ru='Изменить';en='Edit'") + " " + Пояснение + ".";
		Иначе
			Пояснение = НСтр("ka='Выполнить <b>перезапись</b> выбранных элементов.';ru='Выполнить <b>перезапись</b> выбранных элементов.';en='Run <b> overwrite </ b> selected items.'");
		КонецЕсли;
	Иначе
		Пояснение = НСтр("ka='Не выбраны элементы, реквизиты которых необходимо изменить.';ru='Не выбраны элементы, реквизиты которых необходимо изменить.';en='No item selected, details of which are to be changed.'");
	КонецЕсли;
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		ТекстНадписи = ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоВыбранныхОбъектов, НСтр("ka='элемент,элемента,элементов';ru='элемент,элемента,элементов';en='element, member, element'"));
	Иначе
		ТекстНадписи = НСтр("ka='Все элементы';ru='Все элементы';en='All the elements'");
	КонецЕсли;
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Если ЕстьЗаблокированные Тогда
		ПоказатьПредупреждение(, НСтр("ka='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';en='Some of the details are blocked for changes, the changes are not installed.'"));
	КонецЕсли;
...
Функция ПредставлениеСвязейПараметровВыбора(СвязиПараметровВыбора, ОбъектМетаданных)
		...
		Если ВладелецРеквизитов <> Неопределено Тогда
			...
			Если Реквизит <> Неопределено Тогда
				...
				Если Не ПустаяСтрока(ПредставлениеТабличнойЧасти) Тогда
					ПредставлениеРеквизита = ПредставлениеРеквизита + " (" + НСтр("ka='таблица';ru='таблица';en='table'") + " " 
						+ ПредставлениеТабличнойЧасти + ")";
				КонецЕсли;
	...
	Если СвязанныеРеквизиты.Количество() > 0 Тогда
		ШаблонПредставленияСвязей = НСтр("ka='Зависит от реквизитов: %1.';ru='Зависит от реквизитов: %1.';en='It depends on the details: 1%.'");
		Если СвязанныеРеквизиты.Количество() = 1 Тогда
			ШаблонПредставленияСвязей = НСтр("ka='Зависит от реквизита %1.';ru='Зависит от реквизита %1.';en='It depends on the requisite 1%.'");
		КонецЕсли;
...
Функция ПараметрыКонструктора(Формула)
	...
	Результат.Вставить("ОперандыЗаголовок", НСтр("ka='მისაწვდომი რეკვიზიტები';ru='Доступные реквизиты';en='available details'"));
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='მეტამონაცემთა ობიექტის უცნობი ტიპი ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';en='Unknown type of metadata object ""%1""'"), ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრის მნიშვნელობის არასწორი ტიპი';ru='Неверный тип значения параметра (%1)';en='Invalid type parameter value (1%)'"), Строка(Тип));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты)
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='მეორე პარამეტრის არასწორი ტიპი რეკვიზიტები: %1';ru='Неверный тип второго параметра Реквизиты: %1';en='Wrong type of second parameter Details: 1%'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='Общий модуль ""%1"" не найден.';ru='Общий модуль ""%1"" не найден.';en='Total module ""%1"" not found.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
...
"1С:Предприятие 8.3 с отключенным режимом совместимости или выше';ka='Обработка предназначена для запуска на версии платформы"
