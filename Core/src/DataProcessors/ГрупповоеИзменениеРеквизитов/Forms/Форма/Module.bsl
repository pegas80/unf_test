...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Заголовок = НСтр("ka='რეკვიზიტების ჯგუფური ცვლილება';ru='Групповое изменение реквизитов'");
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='შესრულება';ru='Выполнить'");
	...
	Иначе
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='Изменить реквизиты';ru='Изменить реквизиты'");
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		Если КоличествоОбъектовДляОбработки = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Не указаны элементы для изменения';ru='Не указаны элементы для изменения'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ТекстВопроса = НСтр("ka='Отбор не задан. Изменить все элементы?';ru='Отбор не задан. Изменить все элементы?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкаОтбораВыполнена", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("ka='Изменение элементов';ru='Изменение элементов'"));
		КонецЕсли;
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() И Объект.ТипОперации = "ВвестиЗначения" Тогда
		ТекстВопроса = НСтр("ka='Изменения не настроены. Выполнить перезапись элементов без изменений?';ru='Изменения не настроены. Выполнить перезапись элементов без изменений?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка = НСтр("ka='Изменение выделенных элементов ""%1"" (%2)';ru='Изменение выделенных элементов ""%1"" (%2)'");
	...
	Если ТипыОбъектов.Количество() > 1 Тогда
		ПредставлениеТипа = "";
		ШаблонЗаголовка = НСтр("ka='Изменение выделенных элементов (%2)';ru='Изменение выделенных элементов (%2)'");
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов(ВыбранныйРеквизит)
	...
	Если КонтекстныйВызов Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='გადასვლა';ru='Перейти'"));
		ТекстВопроса = НСтр("ka='Реквизит заблокирован, перейти к разблокированию реквизитов?';ru='Реквизит заблокирован, перейти к разблокированию реквизитов?'");
	Иначе
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Разблокировать';ru='Разблокировать'"));
...
"Разблокировать реквизит ""%1"" для изменения?'"),
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Да, НСтр("ka='Реквизит заблокирован';ru='Реквизит заблокирован'"));
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Если НачалоИзменения Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать'");
	Иначе
		Если ОбъектыКоторыеНеУдалосьИзменить.Количество() > 0 Тогда
			Элементы.ФормаИзменить.Заголовок = НСтр("ka='Повторить изменение';ru='Повторить изменение'");
		Иначе
			Элементы.ФормаИзменить.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ka='Изменение выделенных элементов';ru='Изменение выделенных элементов'"), ,НСтр("ka='Пожалуйста подождите, обработка может занять некоторое время...';ru='Пожалуйста подождите, обработка может занять некоторое время...'"));
	...
	Иначе
		...
		Состояние(НСтр("ka='Обрабатываются элементы...';ru='Обрабатываются элементы...'"), 0, НСтр("ka='Изменение выделенных элементов';ru='Изменение выделенных элементов'"));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		Если Объект.ИзменятьВТранзакции Тогда
			ТекстПредупреждения = НСтр("ka='При изменении элементов обнаружены ошибки - изменения отменены.';ru='При изменении элементов обнаружены ошибки - изменения отменены.'");
...
"'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Прервать, НСтр("ka='შეწყვეტა';ru='Прервать'"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='გაგრძელება';ru='Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='აღარ მკითხო';ru='Больше не спрашивать'"));
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьРезультатИзмененияОтветПолучен", ЭтотОбъект, РезультатИзменения);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("ka='Ошибки при изменении элементов';ru='Ошибки при изменении элементов'"));
		Возврат;
	КонецЦикла;
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ka='Обрабатываются элементы...';ru='Обрабатываются элементы...'"), ТекущийПроцент, НСтр("ka='Изменение выделенных элементов';ru='Изменение выделенных элементов'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='Изменение реквизитов элементов';ru='Изменение реквизитов элементов'"), , 
			ПодставитьПараметрыВСтроку(НСтр("ka='Изменены элементы (%1).';ru='Изменены элементы (%1).'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = НСтр("ka='Изменения выполнены во всех выбранных элементах (%2).';ru='Изменения выполнены во всех выбранных элементах (%2).'");
	Иначе
		Если Объект.ИзменятьВТранзакции Или ТекущееСостояниеИзменения.КоличествоИзмененных = 0 Тогда
			ШаблонСообщения = НСтр("ka='Изменения не выполнены.';ru='Изменения не выполнены.'");
		Иначе
...
"Изменено: %1; Не удалось изменить: %3'");
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = ПодставитьПараметрыВСтроку(НСтр("ka='... и другие элементы (%1)';ru='... и другие элементы (%1)'"), КоличествоПропущенных);
			СтрокаТаблицы.Причина = НСтр("ka='Пропущены, так как не были изменены один или более элементов.';ru='Пропущены, так как не были изменены один или более элементов.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НазадСервер()
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='შესრულება';ru='Выполнить'");
		Элементы.ФормаИзменить.РасширеннаяПодсказка.Заголовок = НСтр("ka='Выполнить алгоритм';ru='Выполнить алгоритм'");
	Иначе
		Элементы.ФормаИзменить.Заголовок = НСтр("ka='Изменить реквизиты';ru='Изменить реквизиты'");
	КонецЕсли;
...
"Всего изменено элементов: %1'"), ТекущееСостояниеИзменения.КоличествоИзмененных);
	Элементы.ФормаИзменить.Заголовок = НСтр("ka='მზად არის';ru='Готово'");
	Элементы.ФормаНазад.Видимость = Истина;
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		НаименованиеЗадания = НСтр("ka='Групповое изменение элементов';ru='Групповое изменение элементов'");
...
Процедура СформироватьПояснениеПоАвтонумерации()
	...
	ИначеЕсли Автонумерация Тогда
		ПояснениеПоАвтонумерации = НСтр("ka='<Установить автоматически>';ru='<Установить автоматически>'");
	Иначе
		ПояснениеПоАвтонумерации = НСтр("ka='<Очистить>';ru='<Очистить>'");
	КонецЕсли;
...
Функция ОписанияКолонокТаблицыРеквизитов()
	...
	ОписаниеКолонки.Тип = Новый ОписаниеТипов("Строка");
	ОписаниеКолонки.Представление = НСтр("ka='რეკვიზიტი';ru='Реквизит'");
	...
	ОписаниеКолонки.Представление = НСтр("ka='ახალი მნიშვნელობა';ru='Новое значение'");
	ОписаниеКолонки.ВидПоля = ВидПоляФормы.ПолеВвода;
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
		Пояснение = НСтр("ka='реквизит';ru='реквизит'") + " " + Пояснение;
	ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
		Пояснение = НСтр("ka='реквизиты';ru='реквизиты'") + " " + Пояснение;
	КонецЕсли;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Пояснение = Пояснение + " " + НСтр("ka='в выбранных элементах';ru='в выбранных элементах'");
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			Если Не ПустаяСтрока(Пояснение) Тогда
				Пояснение = Пояснение + ", ";
			КонецЕсли;
			Пояснение = Пояснение + ПодставитьПараметрыВСтроку(НСтр("ka='реквизиты (%1)';ru='реквизиты (%1)'"), ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
				...
				Если ИзменяемыеРеквизиты.Найти(Реквизит) = 0 Тогда
					Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
						Пояснение = Пояснение + НСтр("ka='реквизит';ru='реквизит'") + " ";
					ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
						Пояснение =  Пояснение + НСтр("ka='реквизиты';ru='реквизиты'") + " ";
					КонецЕсли;
				КонецЕсли;
				Пояснение = Пояснение + """" + Реквизит + """";
			КонецЦикла;
		КонецЕсли;
		Пояснение = Пояснение + " " 
			+ ПодставитьПараметрыВСтроку(НСтр("ka='в табличной части ""%1""';ru='в табличной части ""%1""'"), ТабличнаяЧасть.Ключ);
	КонецЦикла;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("ka='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';ru='в тех строках выбранных элементов, которые удовлетворяют условиям отбора'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("ka='<b>во всех строках</b> выбранных элементов';ru='<b>во всех строках</b> выбранных элементов'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		Если Не ПустаяСтрока(Пояснение) Тогда
			Пояснение = НСтр("ka='შეცვლა';ru='Изменить'") + " " + Пояснение + ".";
		Иначе
			Пояснение = НСтр("ka='Выполнить <b>перезапись</b> выбранных элементов.';ru='Выполнить <b>перезапись</b> выбранных элементов.'");
		КонецЕсли;
	Иначе
		Пояснение = НСтр("ka='Не выбраны элементы, реквизиты которых необходимо изменить.';ru='Не выбраны элементы, реквизиты которых необходимо изменить.'");
	КонецЕсли;
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		ТекстНадписи = ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоВыбранныхОбъектов, НСтр("ka='элемент,элемента,элементов';ru='элемент,элемента,элементов'"));
	Иначе
		ТекстНадписи = НСтр("ka='Все элементы';ru='Все элементы'");
	КонецЕсли;
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Если ЕстьЗаблокированные Тогда
		ПоказатьПредупреждение(, НСтр("ka='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.'"));
	КонецЕсли;
...
Функция ПредставлениеСвязейПараметровВыбора(СвязиПараметровВыбора, ОбъектМетаданных)
		...
		Если ВладелецРеквизитов <> Неопределено Тогда
			...
			Если Реквизит <> Неопределено Тогда
				...
				Если Не ПустаяСтрока(ПредставлениеТабличнойЧасти) Тогда
					ПредставлениеРеквизита = ПредставлениеРеквизита + " (" + НСтр("ka='таблица';ru='таблица'") + " " 
						+ ПредставлениеТабличнойЧасти + ")";
				КонецЕсли;
	...
	Если СвязанныеРеквизиты.Количество() > 0 Тогда
		ШаблонПредставленияСвязей = НСтр("ka='Зависит от реквизитов: %1.';ru='Зависит от реквизитов: %1.'");
		Если СвязанныеРеквизиты.Количество() = 1 Тогда
			ШаблонПредставленияСвязей = НСтр("ka='Зависит от реквизита %1.';ru='Зависит от реквизита %1.'");
		КонецЕсли;
...
Функция ПараметрыКонструктора(Формула)
	...
	Результат.Вставить("ОперандыЗаголовок", НСтр("ka='მისაწვდომი რეკვიზიტები';ru='Доступные реквизиты'"));
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='მეტამონაცემთა ობიექტის უცნობი ტიპი ""%1""';ru='Неизвестный тип объекта метаданных ""%1""'"), ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრის მნიშვნელობის არასწორი ტიპი';ru='Неверный тип значения параметра (%1)'"), Строка(Тип));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты)
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='მეორე პარამეტრის არასწორი ტიპი რეკვიზიტები: %1';ru='Неверный тип второго параметра Реквизиты: %1'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ka='Общий модуль ""%1"" не найден.';ru='Общий модуль ""%1"" не найден.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
