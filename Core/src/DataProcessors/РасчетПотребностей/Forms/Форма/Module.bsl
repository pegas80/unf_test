...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Kuzmich, 26.02.2021, Доработка калькуляции заказа и расчета потребностей
	Если Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить() Тогда 
		Элементы.ТекущийЗаказ.Видимость = ИСТИНА;
		Элементы.УчитыватьСвободныеОстатки.Видимость = ИСТИНА;
		УчитыватьСвободныеОстатки = ИСТИНА;
		Элементы.ВсеВОдинЗаказПоставщику.Видимость = ИСТИНА;
		ВсеВОдинЗаказПоставщику = ЛОЖЬ;
	Иначе	
		Элементы.ТекущийЗаказ.Видимость = ЛОЖЬ;
		Элементы.УчитыватьСвободныеОстатки.Видимость = ЛОЖЬ;
		УчитыватьСвободныеОстатки = ЛОЖЬ;
		Элементы.ВсеВОдинЗаказПоставщику.Видимость = ЛОЖЬ;
		ВсеВОдинЗаказПоставщику = ЛОЖЬ;
	КонецЕсли;
	// Kuzmich, 26.02.2021, Доработка калькуляции заказа и расчета потребностей
КонецПроцедуры
...
Процедура РекомендацииВыбранПриИзменении(Элемент)
	...
	// Kuzmich, 02.03.2021, Доработка калькуляции заказа и расчета потребностей
	Если Получить_Константы_НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа() ТОГДА
		...
		Если ТипЗНЧ(ТекущиеДанные.Номенклатура) = Тип("ДокументСсылка.ЗаказПокупателя")
			ИЛИ ТипЗНЧ(ТекущиеДанные.Номенклатура) = Тип("ДокументСсылка.ЗаказПоставщику")
			ИЛИ ТипЗНЧ(ТекущиеДанные.Номенклатура) = Тип("ДокументСсылка.ЗаказНаПроизводство")
			Тогда
			
			ТекЗначениеВыбран = ТекущиеДанные.Выбран;
			
			ТекущиеДанныеЗаказа = ТекущиеДанные.ПолучитьЭлементы();
			Для каждого СтрТекущиеДанныеЗаказа Из ТекущиеДанныеЗаказа Цикл
							
				ТекущиеДанныеРодителя = СтрТекущиеДанныеЗаказа.ПолучитьЭлементы();
				ВыбранРодитель = ТекЗначениеВыбран;
				Основной = Истина;
				...
				СтрТекущиеДанныеЗаказа.Выбран = Выбран;
				СтрТекущиеДанныеЗаказа.ПотребностьОбеспечена = (ОбеспеченноеКоличество >= Элементы.Рекомендации.ТекущиеДанные.Количество);				
	...
	КонецЕсли;
	КонецЕсли; // Kuzmich, 02.03.2021, Доработка калькуляции заказа и расчета потребностей
КонецПроцедуры
...
Процедура Обновить(Команда)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Сообщение.Текст = НСтр("ka='ორგანიზაცია არ არის არჩეული!';ru='Не выбрана организация!';en='Not selected organization!'");
...
Процедура Перечитать(Команда)
	...
	Если ЭтаФорма.Модифицированность Тогда
		...
		СтрВопроса = НСтр("ka='მონაცემები შეცვლილია. გადავიკითხოთ მონაცემები?';ru='Данные изменены. Перечитать данные?';en='These modified. Re-read the data?'");
	...
	Если Получить_Константы_НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа() ТОГДА
		...
		Для Каждого Строка ИЗ Рекомендации.ПолучитьЭлементы() Цикл
			Элементы.Рекомендации.Развернуть(Строка.ПолучитьИдентификатор());//, Истина);
		КонецЦикла;
...
// Kuzmich, 02.03.2021, Доработка калькуляции заказа и расчета потребностей
&НаСервере
Функция Получить_Константы_НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа()

	Возврат Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить();
...
Процедура ОбновитьНаСервере()
	...
	// Kuzmich, 11.03.2021, Доработка калькуляции заказа и расчета потребностей
	СписокДокументовПоЗаказу = Новый СписокЗначений();
	СписокДокументовПоЗаказу.ЗагрузитьЗначения(ТекущийЗаказДляОтбора.ВыгрузитьЗначения());
	Если (ТекущийЗаказДляОтбора.Количество()>0) Тогда
		Запрос.УстановитьПараметр("ПроверятьЗаказ", ИСТИНА);
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.АвансовыйОтчет.Запасы КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.АвансовыйОтчет.Расходы КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.АктВыполненныхРабот КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.АктВыполненныхРабот.РаботыИУслуги КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ЗаказНаПроизводство КАК ТекДок
		|ГДЕ
		|	ТекДок.ДокументОснование В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ЗаказПоставщику КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ПриходнаяНакладная КАК ТекДок
		|ГДЕ
		|	ТекДок.ДокументОснование В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ПриходнаяНакладная.Запасы КАК ТекДок
		|ГДЕ
		|	ТекДок.Заказ В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ПриходнаяНакладная.Расходы КАК ТекДок
		|ГДЕ
		|	ТекДок.Заказ В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ПриходнаяНакладная.ЗатратыНаИмущество КАК ТекДок
		|ГДЕ
		|	ТекДок.Заказ В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.ПрочиеРасходы.Расходы КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.РасходнаяНакладная КАК ТекДок
		|ГДЕ
		|	ТекДок.Заказ В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	Документ.РасходнаяНакладная КАК ТекДок
		|ГДЕ
		|	ТекДок.ТорговаяТочка В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.РасходнаяНакладная.Запасы КАК ТекДок
		|ГДЕ
		|	ТекДок.Заказ В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.РезервированиеЗапасов КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.СборкаЗапасов КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.СдельныйНаряд КАК ТекДок
		|ГДЕ
		|	ТекДок.ДокументОснование В(&ТекущийЗаказ)
		|
		|ОБЪЕДИНИТЬ
		...
		|	ТекДок.Ссылка
		|ИЗ
		|	Документ.СдельныйНаряд.Операции КАК ТекДок
		|ГДЕ
		|	ТекДок.ЗаказПокупателя В(&ТекущийЗаказ)";
		
		Запрос1.УстановитьПараметр("ТекущийЗаказ", ТекущийЗаказДляОтбора);
		...
		Пока ВыборкаДетальныеЗаписи1.Следующий() Цикл
			СписокДокументовПоЗаказу.Добавить(ВыборкаДетальныеЗаписи1.Ссылка);
		КонецЦикла; 		
	Иначе	
		Запрос.УстановитьПараметр("ПроверятьЗаказ", ЛОЖЬ);
	КонецЕсли;
	Запрос.УстановитьПараметр("ТекущийЗаказ", СписокДокументовПоЗаказу);
	Запрос.УстановитьПараметр("НоваяСхема", Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить());
	// Kuzmich, 11.03.2021, Доработка калькуляции заказа и расчета потребностей
...
Процедура ОбновитьДанные(РезультатЗапроса, ДатаНачала, ДатаОкончания)
	...
	Для каждого Выборка Из ТаблицаГрафикПотребностей Цикл
		...
		Если Выборка.Период = ДатаНачала Тогда
			...
			// Kuzmich, 26.02.2021, Доработка калькуляции заказа и расчета потребностей
			Если Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить() Тогда
				Если УчитыватьСвободныеОстатки Тогда
				    СтруктураДетализация.Просрочено.Детализация.Вставить("НачальныйОстаток", Выборка.РезервОстаток+Выборка.СвободныйОстаток);
				Иначе	
					СтруктураДетализация.Просрочено.Детализация.Вставить("НачальныйОстаток", Выборка.РезервОстаток);
				КонецЕсли;
			ИНАЧЕ	
				СтруктураДетализация.Просрочено.Детализация.Вставить("НачальныйОстаток", Выборка.СвободныйОстаток);
			КонецЕсли;
...
Процедура ДобавитьНоменклатуруХарактеристику(ТекущаяНоменклатура, СтруктураДетализация, СтруктураРасшифровка, ДатаНачала, ДатаОкончания)
	...
	Если ТолькоДефицит И СтруктураДетализация.Дефицит.ЗначениеПоказателя > 0
		//// Kuzmich, 26.02.2021, Доработка калькуляции заказа и расчета потребностей
		//ИЛИ (ТолькоДефицит И СтруктураДетализация.Просрочено.Детализация.НачальныйОстаток > 0 И Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить())
		//// Kuzmich, 26.02.2021, Доработка калькуляции заказа и расчета потребностей
		...
		НачальныйОстаток.Номенклатура = НСтр("ka='საწყისი ნაშთი';ru='Начальный остаток';en='balance at the starting date'");
		
		Поступление = НоменклатураЭлементы.Добавить();
		Поступление.Номенклатура = НСтр("ka='შემოსავალი';ru='Поступление';en='supply'");
		
		Потребность = НоменклатураЭлементы.Добавить();
		Потребность.Номенклатура = НСтр("ka='მოთხოვნილება';ru='Потребность';en='Need'");
		...
		Иначе
			...
			МинимальныйЗапас.Номенклатура = НСтр("ka='მინიმალური ნაშთი';ru='Минимальный запас';en='minimum stock'");
			
			МаксимальныйЗапас = НоменклатураЭлементы.Добавить();
			МаксимальныйЗапас.Номенклатура = НСтр("ka='მაქსიმალური ნაშთი';ru='Максимальный запас';en='maximum inventory'");
			
		КонецЕсли;
		
		КонечныйОстаток = НоменклатураЭлементы.Добавить();
		КонечныйОстаток.Номенклатура = НСтр("ka='საბოლოო ნაშთი';ru='Конечный остаток';en='balance at the closing date'");
...
Процедура ОбновитьРекомендацииНаСервере()
	...
	ТекИсточникДанных = Новый ТаблицаЗначений;
	ТекИсточникДанных.Колонки.Добавить("ИндексСтроки", Новый ОписаниеТипов("Число"));
	ТекИсточникДанных.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТекИсточникДанных.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТекИсточникДанных.Колонки.Добавить("Поставщик", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	ТекИсточникДанных.Колонки.Добавить("СпособПополнения", Новый ОписаниеТипов("ПеречислениеСсылка.СпособыПополненияЗапасов"));
	ТекИсточникДанных.Колонки.Добавить("СрокПополнения", Новый ОписаниеТипов("Число"));
	ТекИсточникДанных.Колонки.Добавить("ПорядокСпособаПополнения", Новый ОписаниеТипов("Число"));
	ТекИсточникДанных.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	ТекИсточникДанных.Колонки.Добавить("ДатаПоступления", Новый ОписаниеТипов("Дата"));
	ТекИсточникДанных.Колонки.Добавить("Заказ");
	
	ИндексСтроки = 0;
	
	Для каждого Номенклатура Из ТЧЗапасы.Строки Цикл
		
		Для каждого ТекущаяСтрока Из Номенклатура.Строки Цикл
			
			Если ТекущаяСтрока.Номенклатура = НСтр("ka='მოთხოვნილება';ru='Потребность';en='Need'") Тогда
				
				Для каждого ТекущаяСтрока1 Из ТекущаяСтрока.Строки Цикл
					
					Если 
						//ТекущаяСтрока1.Просрочено>0 
						//ИЛИ 
						ТекущаяСтрока1.Дефицит > 0
						Тогда
						...
						Пока НачалоДня(ТекущийПериод) <= НачалоДня(ОкончаниеПериода) Цикл
							...
							Если ТекущаяСтрока1[ИмяКолонки] > 0
								ИЛИ (
								//ТекущаяСтрока1.Просрочено > 0
								//ИЛИ 
								ТекущаяСтрока1.Дефицит > 0
								)
								И ТекущийПериод = НачалоДня(ТекущаяДатаСеанса()) Тогда
								
								НоваяСтрока = ТекИсточникДанных.Добавить();
								...
								НоваяСтрока.Заказ = ТекущаяСтрока1.Номенклатура;
								
								Если ТекущийПериод = НачалоДня(ТекущаяДатаСеанса()) Тогда
									
									НоваяСтрока.Количество = ТекущаяСтрока1[ИмяКолонки] + ТекущаяСтрока1.Просрочено;
									
								Иначе
									
									НоваяСтрока.Количество = ТекущаяСтрока1[ИмяКолонки];
								...
								Если НоваяСтрока.СпособПополнения = Перечисления.СпособыПополненияЗапасов.Закупка Тогда
									...
									Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуПроизводство.Получить() Тогда
										...
										НовыйСпособПополнения = ТекИсточникДанных.Добавить();
	...
	Если ТекИсточникДанных.Количество() = 0 Тогда
	...
	ТекИсточникДанных.Сортировать("Заказ,ИндексСтроки,Номенклатура,ПорядокСпособаПополнения");
	
	Для каждого СтрТекИсточникДанных Из ТекИсточникДанных Цикл
		
		Если НЕ (ТекущийЗаказ = СтрТекИсточникДанных.Заказ) Тогда
		
			ТекущийЗаказ = СтрТекИсточникДанных.Заказ;
			ИсточникДанных = ТекИсточникДанных.Скопировать(Новый Структура("Заказ", ТекущийЗаказ));
			
			ТекРекомендации = Рекомендации.ПолучитьЭлементы();
			ТекРекомендацииСтрока = ТекРекомендации.Добавить();
			ТекРекомендацииСтрока.Номенклатура = ТекущийЗаказ;
			ТекРекомендацииСтрока.Выбран = ИСТИНА;
			...
			НоменклатураЭлементы = ТекРекомендацииСтрока.ПолучитьЭлементы();
	...
	КонецЕсли;// Kuzmich, 15.02.2021, Доработка калькуляции заказа и расчета потребностей
КонецПроцедуры
...
Процедура СформироватьЗаказыНаСервере()
	...
	Если Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить() ТОГДА
		...
		ТекущийЗаказНаПроизводство = НЕОПРЕДЕЛЕНО;
		Для каждого СтрокаРекомендации1 из РекомендацииНоменклатура Цикл
			
			Если НЕ СтрокаРекомендации1.Выбран Тогда
			
				Продолжить;
			
			КонецЕсли;
			
			ТекущийЗаказПокупателя = СтрокаРекомендации1.Номенклатура;
			...
			РекомендацииНоменклатура = СтрокаРекомендации1.ПолучитьЭлементы();
			...
			Для каждого ПараметрыЗаказа из ТаблицаЗаказы Цикл
				...
				Если ПараметрыЗаказа.СпособПополнения = Перечисления.СпособыПополненияЗапасов.Закупка
					...
					Если ВсеВОдинЗаказПоставщику Тогда
						Если ТекущийЗаказПоставщику = НЕОПРЕДЕЛЕНО Тогда
							...
							ТекущийЗаказПоставщику = ТекущийЗаказ;
						Иначе
							
							ТекущийЗаказ = ТекущийЗаказПоставщику;
				...
				Иначе // Сформируем заказы на производство.
					...
					Если ВсеВОдинЗаказПоставщику Тогда
						Если ТекущийЗаказНаПроизводство = НЕОПРЕДЕЛЕНО Тогда
							...
							ТекущийЗаказНаПроизводство = ТекущийЗаказ;
							
						Иначе
							
							ТекущийЗаказ = ТекущийЗаказНаПроизводство;
				...
				Если НЕ ВсеВОдинЗаказПоставщику Тогда
					ТекущийЗаказ.ЗаказПокупателя = ТекущийЗаказПокупателя;
				КонецЕсли;
				ТекущийЗаказ.Комментарий = НСтр("ka='ფორმირებულია ავტომატურად სერვისით ""მარაგების მოთხოვნილებების გამოანგარიშება"".';ru='Сформирован автоматически сервисом ""Расчет потребностей в запасах"".';en='service is automatically generated, ""Calculation of reserves needs.""'");

				ТекущийЗаказ.Записать();
				МассивНайденныхСтрокЗаказы = Заказы.НайтиСтроки(Новый Структура("Заказ", ТекущийЗаказ.Ссылка));
				Если МассивНайденныхСтрокЗаказы.Количество()=0 Тогда
...
Процедура УстановитьУсловноеОформление(НачалоПериода, ОкончаниеПериода)
	...
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = НСтр("ka='შემოსავალი';ru='Поступление';en='supply'");
	
	ЭлементОтбора = ГруппаЭлементовОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Запасы.Номенклатура");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = НСтр("ka='Потребность';ru='Потребность';en='Need'");
