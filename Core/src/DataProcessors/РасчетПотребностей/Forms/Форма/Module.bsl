...
Процедура Обновить(Команда)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не выбрана организация!';ru='Не выбрана организация!'");
...
Процедура Перечитать(Команда)
	...
	Если ЭтаФорма.Модифицированность Тогда
		...
		СтрВопроса = НСтр("ka='მონაცემები შეცვლილია. გადავიკითხოთ მონაცემები?';ru='Данные изменены. Перечитать данные?'");
...
Процедура ДобавитьНоменклатуруХарактеристику(ТекущаяНоменклатура, СтруктураДетализация, СтруктураРасшифровка, ДатаНачала, ДатаОкончания)
	...
	Если ТолькоДефицит И СтруктураДетализация.Дефицит.ЗначениеПоказателя > 0 
		...
		НачальныйОстаток.Номенклатура = НСтр("ka='საწყისი ნაშთი';ru='Начальный остаток'");
		
		Поступление = НоменклатураЭлементы.Добавить();
		Поступление.Номенклатура = НСтр("ka='შემოსავალი';ru='Поступление'");
		
		Потребность = НоменклатураЭлементы.Добавить();
		Потребность.Номенклатура = НСтр("ka='მოთხოვნილება';ru='Потребность'");
		...
		Иначе
			...
			МинимальныйЗапас.Номенклатура = НСтр("ka='მინიმალური ნაშთი';ru='Минимальный запас'");
			
			МаксимальныйЗапас = НоменклатураЭлементы.Добавить();
			МаксимальныйЗапас.Номенклатура = НСтр("ka='მაქსიმალური ნაშთი';ru='Максимальный запас'");
			
		КонецЕсли;
		
		КонечныйОстаток = НоменклатураЭлементы.Добавить();
		КонечныйОстаток.Номенклатура = НСтр("ka='საბოლოო ნაშთი';ru='Конечный остаток'");
...
Процедура СформироватьЗаказыНаСервере()
	...
	Для каждого ПараметрыЗаказа из ТаблицаЗаказы Цикл
		...
		ТекущийЗаказ.Комментарий = НСтр("ka='ფორმირებულია ავტომატურად სერვისით ""მარაგების მოთხოვნილებების გამოანგარიშება"".';ru='Сформирован автоматически сервисом ""Расчет потребностей в запасах"".'");
...
Процедура УстановитьУсловноеОформление(НачалоПериода, ОкончаниеПериода)
	...
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = НСтр("ka='შემოსავალი';ru='Поступление'");
	
	ЭлементОтбора = ГруппаЭлементовОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Запасы.Номенклатура");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = НСтр("ka='Потребность';ru='Потребность'");
