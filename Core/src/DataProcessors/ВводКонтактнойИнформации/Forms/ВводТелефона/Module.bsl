...
Процедура КодГородаПриИзменении(Элемент)
	...
	Если (КодСтраны = "+7" ИЛИ КодСтраны = "8") И СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Кода мобильных телефонов начинающиеся на цифру 9 имеют фиксированную длину в 3 цифры, например - 916.';ru='Кода мобильных телефонов начинающиеся на цифру 9 имеют фиксированную длину в 3 цифры, например - 916.';en='Mobile phone code starting with the digit 9 have a fixed length in 3 digits, for example - 916.'"),, "КодГорода");
	КонецЕсли;
...
Функция РезультатВыбора()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.ВводКонтактнойИнформации.Форма.ВводТелефона", "КодыСтраныИГорода", Коды, НСтр("ka='Коды страны и города';ru='Коды страны и города';en='Codes Countries and cities'"));
...
Функция ОшибкиЗаполненияТелефона()
	...
	Если СтрДлина(НомерТелефонаТолькоЦифры) > 15 Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("ka='Номер телефона слишком длинный';ru='Номер телефона слишком длинный';en='The phone number is too long'"));
	КонецЕсли;
	Если НомерТелефонаСодержитНедопустимыеСимволы(ПолныйНомерТелефона) Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("ka='Номер телефона содержит недопустимые символы';ru='Номер телефона содержит недопустимые символы';en='Phone number contains invalid characters'"));
	КонецЕсли;
	Если КодСтраны = "7" И СтрДлина(НомерТелефона) > 7 Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("ka='В России номер телефона не может быть больше 7 цифр';ru='В России номер телефона не может быть больше 7 цифр';en='In Russia, the phone number can not be more than 7 digits'"));
	КонецЕсли;
	
	Если СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("ka='В России номера мобильных телефонов должны содержать 3 цифры';ru='В России номера мобильных телефонов должны содержать 3 цифры';en='The Russian mobile phone numbers must be 3 digits'"));
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок)
	...
	Если СписокОшибок.Количество()=0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Телефон введен корректно.';ru='Телефон введен корректно.';en='Phone number is correctly entered.'"));
		Возврат;
	КонецЕсли;
