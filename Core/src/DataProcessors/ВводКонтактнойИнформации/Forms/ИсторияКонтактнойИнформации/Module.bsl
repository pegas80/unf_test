...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СписокКонтактнойИнформации) = Тип("Массив") Тогда
		...
		КонецЦикла;
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='История изменений (%1)';ru='История изменений (%1)';en='Change History (1%)'"), ПредставлениеКонтактнойИнформации);
...
Процедура ИсторияПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ka='Удалить адрес действующий с';ru='Удалить адрес действующий с';en='Remove with a valid address'") + " " + Формат(ДействуетС, "ДЛФ=DD")+ "?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	Отказ = Истина;
КонецПроцедуры
...
Процедура СформироватьДанные(ВводНовогоАдреса = Ложь)
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не допускается ввод адресов с одинаковыми датами.';ru='Не допускается ввод адресов с одинаковыми датами.';en='It is not allowed entry of addresses with the same dates.'"),, "История[" + Строка(Индекс) + "].ДействуетС");
			Возврат;
		КонецЕсли;
	...
	Если НЕ ВводНовогоАдреса И ОтсутствуетНачальнаяДата Тогда
		ПоказатьПредупреждение(, НСтр("ka='Отсутствует адрес, действующий с даты начала ведения учета.';ru='Отсутствует адрес, действующий с даты начала ведения учета.';en='No address, valid from the date of accounting.'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='საწყისი მნისვნელობა';ru='Начальное значение';en='The initial value'"));
