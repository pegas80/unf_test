...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидКонтактнойИнформации.ПроверятьПоФИАС Тогда
		Команды.ПроверитьЗаполнениеАдреса.Подсказка = НСтр("ka='Проверить заполнение адреса в формате ФИАС';ru='Проверить заполнение адреса в формате ФИАС'");
	Иначе
		Команды.ПроверитьЗаполнениеАдреса.Подсказка  = НСтр("ka='Проверить заполнение адреса в формате КЛАДР';ru='Проверить заполнение адреса в формате КЛАДР'");
	КонецЕсли;
	...
	Если АдресТолькоРоссийский  Тогда
		...
		Если Страна <> СтранаРоссия Тогда
			...
			ТекстПредупрежденияПриОткрытии = НСтр("ka='Адрес введен некорректно: допускается ввод только российских адресов. Значение поля ""Страна"" было изменено на ""Россия"", необходимо проверить остальные поля.';ru='Адрес введен некорректно: допускается ввод только российских адресов. Значение поля ""Страна"" было изменено на ""Россия"", необходимо проверить остальные поля.'"); 
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					...
					Если СведенияОРегионе.Загружен = Ложь Тогда
						...
						ДоступностьКлассификатора.КраткоеПредставлениеОшибки = 
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").';ru='Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").'"),
							XDTOКонтактная.Состав.Состав.СубъектРФ,
							?(ЭтоТакси, НСтр("ka='კიდე';ru='Еще'"), НСтр("ka='ყველა მოქმედება';ru='Все действия'")));
	...
	ИначеЕсли ВыбранноеЗначение.Отказ Тогда
		...
		Иначе
			Если НЕ ПодборПоИндексуДоступен Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Индекс %1 не найден в адресном классификаторе';ru='Индекс %1 не найден в адресном классификаторе'"),
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если ЕстьПравоЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиент.ПредложениеЗагрузкиКлассификатора(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Данные для ""%1"" не загружены.';ru='Данные для ""%1"" не загружены.'"), ВыбранноеЗначение.Представление),
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Улица", Улица);
		ПараметрыФормы.Вставить("Заголовок", НаселенныйПункт + " " + НСтр("ka='(не найден)';ru='(не найден)'"));
	КонецЕсли;
...
Процедура АдресНаДатуАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если СтрСравнить(Текст, НСтр("ka='начала учета';ru='начала учета'")) = 0 Или ПустаяСтрока(Текст) Тогда
...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		Если ЗначениеЗаполнено(Результат.ДействуетПо) Тогда
			ТекстИсторическийАдрес = " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='действует по %1';ru='действует по %1'"), Формат(Результат.ДействуетПо - 10, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = НСтр("ka='действует по настоящее время.';ru='действует по настоящее время.'");
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ПоказатьПредупреждение(, НСтр("ka='Адрес не может быть проверен, так как он введен в свободной форме.';ru='Адрес не может быть проверен, так как он введен в свободной форме.'"));
		Возврат;
	КонецЕсли;
...
"Адреса, введенные в свободной форме, могут не пройти проверку по адресному классификатору.'");
	...
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ka='დადასტურება';ru='Подтверждение'"));
КонецПроцедуры
...
"Рекомендуется выполнить обновление, чтобы проверка адреса выполнялась корректно.'");
	...
	Иначе
		СообщениеОПроверкеАдреса = СообщениеОПроверкеАдреса + Символы.ПС
			+ НСтр("ka='Для обновления адресного классификатора необходимо обратиться к администратору.';ru='Для обновления адресного классификатора необходимо обратиться к администратору.'");
...
Процедура ПоказатьСписокДомов(ВключитьОтбор)
	...
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр("ka='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.';ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='Для просмотра список домов заполните поля адреса.';ru='Для просмотра список домов заполните поля адреса.'");
		КонецЕсли;
...
Процедура ВопросОбОбновлениеАдресногоКлассификатора(Знач СообщениеОПроверкеАдреса)
	...
	Кнопки.Добавить("ОбновитьКлассификатор", НСтр("ka='კლასიფიკატორის განახლება';ru='Обновить классификатор'"));
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РазрешитьВводАдресаВСвободнойФорме И ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
		Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ НСтр("ka='Исходный адрес:';ru='Исходный адрес:'") + " " + ПредставлениеАдреса;
		ПодключитьОбработчикОжидания("УстановитьПиктограммуКомментария", 0.1, Истина);
	КонецЕсли;
...
Функция СписокОшибокЗаполнения(Контекст, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если Результат.Количество() = 0 // Нет ошибок
		...
		Результат.Добавить("/", НСтр("ka='Адрес пуст.';ru='Адрес пуст.'"));
	КонецЕсли;
...
Процедура ОтобразитьКоличествоЗаписейВИсторииИзменений()
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='История изменений (%1)';ru='История изменений (%1)'"), НайденныеСтроки.Количество());
	...
	Иначе
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = НСтр("ka='ცვლილებების ისტორია';ru='История изменений'");
		Элементы.ИсторияИзмененийГиперссылка.Видимость = Истина;
	КонецЕсли;
...
"Для обновления адресного классификатора необходимо обратиться к администратору.'");
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Адрес введен корректно.';ru='Адрес введен корректно.'"));
		КонецЕсли;
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].Пометка = Ложь И НЕ ЭтоВебСервис Тогда
			ТекстВопроса = НСтр("ka='Адрес не может быть проверен.';ru='Адрес не может быть проверен.'") + Символы.ПС + СписокОшибок[0].Представление + Символы.ПС + НСтр("ka='Загрузить классификатор для проверки адреса?';ru='Загрузить классификатор для проверки адреса?'");
...
Процедура СостояниеВводаПредставленияАдреса(РедактироватьПредставлениеАдреса, ФормироватьПредставление = Истина)
	...
	Если РедактироватьПредставлениеАдреса Тогда
		...
		Элементы.ПредставлениеАдреса.Заголовок          = НСтр("ka='Адрес в свободной форме';ru='Адрес в свободной форме'");
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ДоступностьКлассификатора <> Неопределено Тогда
			...
			Если ДоступностьКлассификатора.Отказ Тогда
				ТекстСообщенияСервиса = НСтр("ka='Автоподбор и проверка адреса недоступны:';ru='Автоподбор и проверка адреса недоступны:'") + Символы.ПС 
				+ ДоступностьКлассификатора.КраткоеПредставлениеОшибки;
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='Проверка доступности сервиса адресного классификатора';ru='Проверка доступности сервиса адресного классификатора'");
