...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Родитель) И Уровень > 1 Тогда
		...
		КраткоеПредставлениеОшибки = НСтр("ka='Поле не содержит адресных сведений для выбора.';ru='Поле не содержит адресных сведений для выбора.';en='The field does not contain address information for selection.'");
		Возврат;
	КонецЕсли;
	...
	Если ДанныеКлассификатора.Отказ Тогда
		...
		КраткоеПредставлениеОшибки = НСтр("ka='Автоподбор и проверка адреса недоступны:';ru='Автоподбор и проверка адреса недоступны:';en='Autofit and address verification are not available:'") + Символы.ПС + ДанныеКлассификатора.КраткоеПредставлениеОшибки;
		Возврат;
...
Процедура ПроизвестиВыбор(Знач НомерСтроки)
	...
	ПредупреждениеНеактуальности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Адрес ""%1"" неактуален."
"Продолжить?';ru='Адрес ""%1"" неактуален."
"Продолжить?'"), Данные.Представление);
		
	ЗаголовокПредупреждения = НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'");
