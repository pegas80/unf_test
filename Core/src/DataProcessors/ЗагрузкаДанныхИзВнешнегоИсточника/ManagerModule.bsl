		...
		Для каждого ГруппаИлиПоле Из КопияГруппыИПоля.Строки Цикл
			...
			Если ПустаяСтрока(ГруппаИлиПоле.ИмяГруппыПолей) Тогда
				...
				Если ГруппаИлиПоле.НомерЦветаОригинал = 1
					...
					ТекстОшибки = Нстр("ka='В колонке {%1} присутствуют незаполенные ячейки. При обработке данные строки будут пропущены.';ru='В колонке {%1} присутствуют незаполенные ячейки. При обработке данные строки будут пропущены.';en='In column 1 {%} nezapolennye present cell. When processing the data line will be skipped.'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ГруппаИлиПоле.ПредставлениеПоля);
					МестоВозникновения = Нстр("ka='Строка № %1.';ru='Строка № %1.';en='Row 1 №%.'");
		...
		Если НЕ ПустаяСтрока(ПолеИлиГруппаПолей.ИмяГруппыПолей) Тогда
			...
			Для каждого ПолеИзГруппыПолей Из ПолеИлиГруппаПолей.Строки Цикл 
				...
				Если ПолеИзГруппыПолей.НомерЦветаОригинал = 1 
					...
					ТекстОшибки = Нстр("ka='Не выбрана обязательная колонка {%1}';ru='Не выбрана обязательная колонка {%1}';en='Not mandatory selected column 1 {%}'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ПолеИзГруппыПолей.ПредставлениеПоля);
					МестоВозникновения = Нстр("ka='Настройка заголовков.';ru='Настройка заголовков.';en='Setting headers.'");
