		...
		Для каждого ГруппаИлиПоле Из КопияГруппыИПоля.Строки Цикл
			...
			Если ПустаяСтрока(ГруппаИлиПоле.ИмяГруппыПолей) Тогда
				...
				Если ГруппаИлиПоле.НомерЦветаОригинал = 1
					...
					ТекстОшибки = Нстр("ka='სვეტი {%1} შეიცავს ცარიელ უჯრედებს. ეს ხაზები გამოტოვებული იქნება დამუშავების დროს.';ru='В колонке {%1} присутствуют незаполенные ячейки. При обработке данные строки будут пропущены.';en='In column 1 {%} nezapolennye present cell. When processing the data line will be skipped.'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ГруппаИлиПоле.ПредставлениеПоля);
					МестоВозникновения = Нстр("ka='ხაზი  № %1.';ru='Строка № %1.';en='Row 1 №%.'");
		...
		Если НЕ ПустаяСтрока(ПолеИлиГруппаПолей.ИмяГруппыПолей) Тогда
			...
			Для каждого ПолеИзГруппыПолей Из ПолеИлиГруппаПолей.Строки Цикл 
				...
				Если ПолеИзГруппыПолей.НомерЦветаОригинал = 1 
					...
					ТекстОшибки = Нстр("ka='საჭირო სვეტი არ არის არჩეული {%1}';ru='Не выбрана обязательная колонка {%1}';en='Not mandatory selected column 1 {%}'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ПолеИзГруппыПолей.ПредставлениеПоля);
					МестоВозникновения = Нстр("ka='სათაურების დაყენება.';ru='Настройка заголовков.';en='Setting headers.'");
			...
			Если ПолеИлиГруппаПолей.Строки.Количество() = НевыбраноКолонокВГруппе Тогда
				...
				ТекстОшибки = Нстр("ka='ველის ჯგუფისთვის {%1}, რომელიც შედგება სვეტების ნაკრებისგან {%2}, თქვენ უნდა აირჩიოთ მინიმუმ ერთი სვეტი ჩატვირთულ მონაცემებში.';ru='Для группы полей {%1}, состоящей из набора колонок {%2}, в загружаемых данных необходимо выбрать минимум одну колонку.';en='For field group 1 {%}, consisting of a set of columns% {2} in the downloadable data necessary to select at least one column.'");
...
Процедура ПредварительнаяОбработкаДанных(ТабличныйДокумент, ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных, ТабличныйДокументСообщения, ПропуститьСтраницу, ЗапретитьПереходДалее) Экспорт
	...
	Если ЗапретитьПереходДалее Тогда
		...
		ТекстОшибки = НСтр("ka='Незаполнены импортируемые данные...';ru='Незаполнены импортируемые данные...';en='Unfilled imported data ...'");
...
Процедура СоздатьДеревоПолейЗагрузкиДоступныхПользователю(ДеревоПолей, НастройкиЗагрузкиДанных) Экспорт
	...
	//НоваяСтрока.ПредставлениеПоля	= "აირჩიეთ ჩასატვირთი ველი";
	НоваяСтрока.ПредставлениеПоля	= НСтр("ka='აირჩიეთ ჩასატვირთი ველი';ru='Выберите поле для загрузки';en='Select the field to load'");
	...
	Для каждого ГруппаПолей Из ГруппыИПоля.Строки Цикл
		...
		Если ЭтоПользовательскаяГруппаПолей Тогда
			...
			//Малышев 2024-07-15 (--
			Если ЗначениеЗаполнено(ГруппаПолей.ПредставлениеПоля) тогда
				 НоваяСтрока.ПредставлениеПоля	= ГруппаПолей.ПредставлениеПоля;
			КонецЕсли;	
			//Малышев 2024-07-15 --)
