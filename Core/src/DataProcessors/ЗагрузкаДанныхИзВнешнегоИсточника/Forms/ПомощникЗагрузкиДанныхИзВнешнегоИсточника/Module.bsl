...
Процедура ЗаполнитьДанныеФайлаОбразцаЗагрузки(ДанныеФайла, Расширение)
	...
	НаименованиеФайлаБезРасширения = Нстр("ka='მონაცემების ჩატვირთვის ნიმუში';ru='ОбразецЗагрузкиДанных'");
...
Процедура ОткрытьОбразец(Расширение)
	...
	Если Расширение = "mxl" Тогда
		...
		Если ТипЗнч(ТабличныйДокумент) = Тип("ТабличныйДокумент") Тогда
			...
			ПечатнаяФорма.СинонимМакета = НСтр("ka='მომზადებული მონაცემების ნიმუში mxl ფორმატისთვის';ru='Образец подготовленных данных в формате mxl'");
			ПечатнаяФорма.ТабличныйДокумент = ТабличныйДокумент;
			ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ka='მომზადებული მონაცემების ნიმუში mxl ფორმატისთვის';ru='Образец подготовленных данных в формате mxl'");
...
Процедура ЗаполнитьСписокВыбораФильтраТаблицыСопоставления(ЭтоЗагрузкаТабличнойЧасти)
	...
	Если Параметры.НастройкиЗагрузкиДанных.ЭтоЗагрузкаТабличнойЧасти Тогда
		...
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='ОтборОшибокНет';ru='ОтборОшибокНет'"),	Нстр("ka='მონაცემები, რომლებიც მზადაა ჩასატვირთად';ru='Данные, готовые к загрузке'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(2, Нстр("ka='ОтборОшибки';ru='ОтборОшибки'"), 	Нстр("ka='მონაცემები, რომელთა ჩატვირთვაც არ არის შესაძლებელი';ru='Данные, которые загрузить невозможно'"));
		
	Иначе
		
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='Сопоставленные';ru='Сопоставленные'"), Нстр("ka='მონაცემები, რომლებიც შეესაბამება';ru='Данные, которые удалось сопоставить'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='БудутСозданы';ru='БудутСозданы'"), Нстр("ka='მონაცემები, რომლებიც არ მოიძებნა შესაბამისობის პროგრამაში';ru='Данные, которым не найдено соответствие в программе'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='Несогласованные';ru='Несогласованные'"), Нстр("ka='მონაცემები, რომლებიც შეიცავენ შეცდომას (შევსება არასრულია)';ru='Данные, которые содержат ошибку (заполнены не полностью)'"));
...
Процедура УстановитьТекстЗаголовкаДекорацииНесопоставленныеСтроки()
	...
	Если СоздаватьЕслиНеСопоставлено Тогда
		...
		ТекстЗаголовка = ?(Параметры.НастройкиЗагрузкиДанных.ЭтоЗагрузкаСправочника, 
			Нстр("ka='будет создано новых элементов:';ru='будет создано новых элементов:'"), 
			Нстр("ka='будет создано новых записей:';ru='будет создано новых записей:'"));
		
	Иначе
		
		ТекстЗаголовка = Нстр("ka='будет пропущено строк:';ru='будет пропущено строк:'");
...
Процедура УстановитьТекстЗаголовкаДекорацииСопоставленыхОбъектов()
	...
	ТекстЗаголовока = ?(ОбновлятьСуществующие,
		Нстр("ka='из них сопоставлены и будут обновлены';ru='из них сопоставлены и будут обновлены'"),
		Нстр("ka='из них сопоставлены';ru='из них сопоставлены'"));
...
"При проверке заполнения табличного документа '");
	...
	ТекстЖирный = НСтр("ka='აღმოჩენილია შეცდომები:';ru='обнаружены следующие ошибки:'");
...
Процедура ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(ПерейтиДалее)
	...
	Описание = НСтр("ka='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузка данных из файла';ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузка данных из файла'");
...
Процедура УстановитьТекстДекорацииКоличестваСтрок()
	...
	Если ФильтрТаблицаСопоставления = "БезОтбора" Тогда 
		...
		НовыйЗаголовок = Нстр("ka='Всего строк в таблице .......... %1';ru='Всего строк в таблице .......... %1'");
		ЗначениеПараметра = КоличествоСтрокТаблицы;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "ОтборОшибокНет" Тогда 
		
		НовыйЗаголовок = Нстр("ka='Строк с данными, которые возможно загрузить в приложение .......... %1';ru='Строк с данными, которые возможно загрузить в приложение .......... %1'");
		ЗначениеПараметра = КоличествоСтрокБезОшибок;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "ОтборОшибки" Тогда 
		
		НовыйЗаголовок = Нстр("ka='Строки, содержащие ошибки и препятствующие загрузке данных .......... %1';ru='Строки, содержащие ошибки и препятствующие загрузке данных .......... %1'");
	...
	ИначеЕсли ФильтрТаблицаСопоставления = "Сопоставленные" Тогда 
		...
		Если ОбновлятьСуществующие Тогда
			...
			НовыйЗаголовок = Нстр("ka='Данные, которые соответствуют элементам программы и будут обновлены .......... %1';ru='Данные, которые соответствуют элементам программы и будут обновлены .......... %1'");
			
		Иначе
			
			НовыйЗаголовок = Нстр("ka='Данные, которые соответствуют элементам программы .......... %1';ru='Данные, которые соответствуют элементам программы .......... %1'");
	...
	ИначеЕсли ФильтрТаблицаСопоставления = "БудутСозданы" Тогда 
		...
		НовыйЗаголовок = Нстр("ka='Данные, которые не удалось сопоставить .......... %1';ru='Данные, которые не удалось сопоставить .......... %1'");
		ЗначениеПараметра = НесопоставленоДанных;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "Несогласованные" Тогда 
		
		НовыйЗаголовок = Нстр("ka='Строки, которые содержат ошибку либо заполнены не полностью .......... %1';ru='Строки, которые содержат ошибку либо заполнены не полностью .......... %1'");
...
Процедура ВыбратьВнешнийФайлОбработкаЗавершение(Результат, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ka='Импорт данных из файлов данного типа не поддерживается.';ru='Импорт данных из файлов данного типа не поддерживается.'");
...
Процедура ОбработкаРасшифровкиЗаголовкаКолонки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Свойство("ОтменитьВыборВКолонке") Тогда
			...
			ОбластьЗаголовка.Текст 					= НСтр("ka='აირჩიეთ ჩასატვირთი ველი';ru='Не загружать'");
