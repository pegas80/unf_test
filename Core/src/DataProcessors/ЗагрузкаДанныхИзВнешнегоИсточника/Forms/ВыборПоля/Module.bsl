...
Процедура ОбработкаГруппыПолей(Поле, Отказ)
	...
	Если НЕ ПустаяСтрока(Поле.ИмяГруппыПолей) Тогда
		...
		Отказ = Истина;
		ПоказатьПредупреждение(, Нстр("ka='Выбор групп не предусмотрен...';ru='Выбор групп не предусмотрен...'"));
...
"Рекомендуется разделить загрузку на несколько итераций.'");
			...
			ТекстЗаголовка = СтрШаблон(НСтр("ka='Выбрано %1 реквизита';ru='Выбрано %1 реквизита'"), МаксимумДополнительныхРеквизитов);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗаголовокКолонки", ЗаголовокКолонки) Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ka='ველის არჩევა';ru='Выбор поля'") + ?(ПустаяСтрока(ЗаголовокКолонки), "", ": " + СокрЛП(ЗаголовокКолонки));
	...
	КэшЗначений.Вставить("ПолеВозвратаСписок", НСтр("ka='უკან,რეკვიზიტების სიაზე დაბრუნება';ru='Назад к списку реквизитов'"));
