...
Процедура ФормированиеСчетовПечатныеФормыПредставлениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормы",          Нстр("ka='Выбор печатной формы';ru='Выбор печатной формы'"));
...
Функция ПолучитьПериодичностиДоговоровОбслуживания()
	...
	Если ПериодичностиДоговоров.Количество() > 1 Тогда
		...
		Элементы.ПредставлениеПериода.Подсказка = Нстр("ka='Изменить периодичность';ru='Изменить периодичность'");
	Иначе
...
Процедура ВыбратьПериодичность(ВыбраннаяПериодичность)
	...
	Если ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.День Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='წინა დღე';ru='Предыдущий день'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий день';ru='Следующий день'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Неделя Тогда
		...
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующая неделя';ru='Следующая неделя'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Месяц Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='Предыдущий месяц';ru='Предыдущий месяц'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий месяц';ru='Следующий месяц'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Квартал Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='Предыдущий квартал';ru='Предыдущий квартал'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий квартал';ru='Следующий квартал'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Год Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='Предыдущий год';ru='Предыдущий год'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий год';ru='Следующий год'");
	КонецЕсли;
...
Функция ПроверитьКорректностьВыбораДатыФормированияДокументов(ДокументТип, НовоеЗначение, СтандартнаяОбработка = Неопределено)
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			СтрШаблон(
				НСтр("ka='Дата формируемых документов должна находиться в интервале выбранного периода: %1 — %2';ru='Дата формируемых документов должна находиться в интервале выбранного периода: %1 — %2'"),
...
Процедура ЗапуститьФоновоеЗаданиеОбновлениеДанныхФормы()
	...
	НаименованиеЗадания = НСтр("ka='Получение данных по договорам обслуживания';ru='Получение данных по договорам обслуживания'");
	...
	Если ФоновоеЗадание = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		...
		Если Результат.Свойство("АктыВыполненныхРабот") Тогда
			...
			ТекстОповещениеАкты = СтрШаблон(
				НСтр("ka='Актов выполненных работ — %1';ru='Актов выполненных работ — %1'"),
		...
		Если Результат.Свойство("СчетаНаОплату") Тогда
			...
			ТекстОповещениеСчета = СтрШаблон(
				НСтр("ka='Счетов на оплату — %1';ru='Счетов на оплату — %1'"),
		...
		Если Результат.Свойство("ЭлектронныеПисьма") Тогда
			...
			ТекстОповещениеЭлектронныеПисьма = СтрШаблон(
				НСтр("ka='Электронных писем — %1';ru='Электронных писем — %1'"),
		...
		ТекстОповещение = СтрШаблон(
			НСтр("ka='Создано документов:%1%2%3';ru='Создано документов:%1%2%3'"),
			ТекстОповещениеСчета,
			ТекстОповещениеАкты,
			ТекстОповещениеЭлектронныеПисьма
		);
		
		Текст = НСтр("ka='Создано документов:';ru='Создано документов:'");
		Пояснение = СтрШаблон(
			"%1%2%3",
			ТекстОповещениеСчета,
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	Текст = НСтр("ka='Без цены';ru='Без цены'");
...
Процедура ПроверитьНастройкиОбработкиПередФормированиемДокументов()
	...
	Если ФормированиеСчетовОтправлятьПоПочте ИЛИ ФормированиеАктовОтправлятьПоПочте Тогда
		Если НЕ ЗначениеЗаполнено(ФормированиеСчетовШаблонСообщения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не выбран шаблон рассылки электронных сообщений!';ru='Не выбран шаблон рассылки электронных сообщений!'"),,
	...
	Если ФормированиеСчетовПечатать Тогда
		Если ФормированиеСчетовПечатныеФормы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не выбрана ни одна печатная форма для Счетов на оплату!';ru='Не выбрана ни одна печатная форма для Счетов на оплату!'"),,
	...
	Если ФормированиеАктовПечатать Тогда
		Если ФормированиеАктовПечатныеФормы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не выбрана ни одна печатная форма для Актов выполненных работ!';ru='Не выбрана ни одна печатная форма для Актов выполненных работ!'"),,
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	Если НЕ ЗначениеЗаполнено(ФормированиеСчетовПечатныеФормыПредставление) Тогда
		ФормированиеСчетовПечатныеФормыПредставление = НСтр("ka='<Не выбрана печатная форма>';ru='<Не выбрана печатная форма>'");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФормированиеАктовПечатныеФормыПредставление) Тогда
		ФормированиеАктовПечатныеФормыПредставление = НСтр("ka='<Не выбрана печатная форма>';ru='<Не выбрана печатная форма>'");
	КонецЕсли;
