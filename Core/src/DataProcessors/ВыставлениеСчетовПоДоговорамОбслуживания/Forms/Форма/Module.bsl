...
Процедура ФормированиеСчетовПечатныеФормыПредставлениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормы",          Нстр("ka='Выбор печатной формы';ru='Выбор печатной формы';en='The choice of printing form'"));
...
Функция ПолучитьПериодичностиДоговоровОбслуживания()
	...
	Если ПериодичностиДоговоров.Количество() > 1 Тогда
		...
		Элементы.ПредставлениеПериода.Подсказка = Нстр("ka='Изменить периодичность';ru='Изменить периодичность';en='change the frequency'");
	Иначе
...
Процедура ВыбратьПериодичность(ВыбраннаяПериодичность)
	...
	Если ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.День Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='წინა დღე';ru='Предыдущий день';en='previous day'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий день';ru='Следующий день';en='The next day'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Неделя Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущая нелеля ';ka='Предыдущая нелеля '");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующая неделя';ru='Следующая неделя';en='next week'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Месяц Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='Предыдущий месяц';ru='Предыдущий месяц';en='Previous month'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий месяц';ru='Следующий месяц';en='Next month'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Квартал Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='Предыдущий квартал';ru='Предыдущий квартал';en='Previous quarter'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий квартал';ru='Следующий квартал';en='the next quarter'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Год Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ka='Предыдущий год';ru='Предыдущий год';en='Prior year'");
		ПодсказкаСледующийИнтервал  = НСтр("ka='Следующий год';ru='Следующий год';en='Next year'");
	КонецЕсли;
...
Функция ПроверитьКорректностьВыбораДатыФормированияДокументов(ДокументТип, НовоеЗначение, СтандартнаяОбработка = Неопределено)
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			СтрШаблон(
				НСтр("ka='Дата формируемых документов должна находиться в интервале выбранного периода: %1 — %2';ru='Дата формируемых документов должна находиться в интервале выбранного периода: %1 — %2';en='Date generated documents should be in the range of the selected period: 1% - 2%'"),
...
Процедура ЗапуститьФоновоеЗаданиеОбновлениеДанныхФормы()
	...
	НаименованиеЗадания = НСтр("ka='Получение данных по договорам обслуживания';ru='Получение данных по договорам обслуживания';en='The preparation of the contracts of service data'");
	...
	Если ФоновоеЗадание = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		...
		Если Результат.Свойство("АктыВыполненныхРабот") Тогда
			...
			ТекстОповещениеАкты = СтрШаблон(
				НСтр("ka='Актов выполненных работ — %1';ru='Актов выполненных работ — %1';en='Acts performed work - 1%'"),
		...
		Если Результат.Свойство("СчетаНаОплату") Тогда
			...
			ТекстОповещениеСчета = СтрШаблон(
				НСтр("ka='Счетов на оплату — %1';ru='Счетов на оплату — %1';en='Invoices for payment - 1%'"),
		...
		Если Результат.Свойство("ЭлектронныеПисьма") Тогда
			...
			ТекстОповещениеЭлектронныеПисьма = СтрШаблон(
				НСтр("ka='Электронных писем — %1';ru='Электронных писем — %1';en='Emails - 1%'"),
		...
		ТекстОповещение = СтрШаблон(
			НСтр("ka='Создано документов:%1%2%3';ru='Создано документов:%1%2%3';en='Created documents:% 1% 2% 3'"),
			ТекстОповещениеСчета,
			ТекстОповещениеАкты,
			ТекстОповещениеЭлектронныеПисьма
		);
		
		Текст = НСтр("ka='Создано документов:';ru='Создано документов:';en='Created documents:'");
		Пояснение = СтрШаблон(
			"%1%2%3",
			ТекстОповещениеСчета,
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	Текст = НСтр("ka='Без цены';ru='Без цены';en='no price'");
...
Процедура ПроверитьНастройкиОбработкиПередФормированиемДокументов()
	...
	Если ФормированиеСчетовОтправлятьПоПочте ИЛИ ФормированиеАктовОтправлятьПоПочте Тогда
		Если НЕ ЗначениеЗаполнено(ФормированиеСчетовШаблонСообщения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не выбран шаблон рассылки электронных сообщений!';ru='Не выбран шаблон рассылки электронных сообщений!';en='Not selected electronic messaging template!'"),,
	...
	Если ФормированиеСчетовПечатать Тогда
		Если ФормированиеСчетовПечатныеФормы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не выбрана ни одна печатная форма для Счетов на оплату!';ru='Не выбрана ни одна печатная форма для Счетов на оплату!';en='Is not selected, no printing form for the account to pay!'"),,
	...
	Если ФормированиеАктовПечатать Тогда
		Если ФормированиеАктовПечатныеФормы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не выбрана ни одна печатная форма для Актов выполненных работ!';ru='Не выбрана ни одна печатная форма для Актов выполненных работ!';en='Is not selected, no printing plate for the Acts of executed works!'"),,
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	Если НЕ ЗначениеЗаполнено(ФормированиеСчетовПечатныеФормыПредставление) Тогда
		ФормированиеСчетовПечатныеФормыПредставление = НСтр("ka='<Не выбрана печатная форма>';ru='<Не выбрана печатная форма>';en='<Print form is not selected>'");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФормированиеАктовПечатныеФормыПредставление) Тогда
		ФормированиеАктовПечатныеФормыПредставление = НСтр("ka='<Не выбрана печатная форма>';ru='<Не выбрана печатная форма>';en='<Print form is not selected>'");
	КонецЕсли;
