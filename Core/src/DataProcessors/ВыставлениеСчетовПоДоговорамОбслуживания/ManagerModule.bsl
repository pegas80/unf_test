		...
		Если ТипЗнч(НоваяСтрока.ОбъектДоговораОбслуживания) = Тип("СправочникСсылка.Номенклатура") Тогда
			Если КоличествоОказаноБезЦены <> 0 И СуммаОказаноБезЦены = 0
				...
				ТекстОшибки = СтрШаблон(
					НСтр("ka='По договору %1: %2 не может быть сформирован Счет! Для номенклатуры %3 не установлена цена во виду цен договора: %4';ru='По договору %1: %2 не может быть сформирован Счет! Для номенклатуры %3 не установлена цена во виду цен договора: %4';en='Under the contract of% 1:% 2 can not be formed Score! For the range of 3% is not set the price of the contract price mean: 4%'"),
	...
	Для Итератор = 0 По Параметры.СписокДоговорыОбслуживания.Количество() - 1 Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Пакетное формирование счетов и электронных писем';ru='Пакетное формирование счетов и электронных писем';en='Batch generation of invoices and emails'"),
				УровеньЖурналаРегистрации.Ошибка,,,
				ТекстОшибки
			);
			
			ТекстОшибки = СтрШаблон(
				НСтр("ka='Для договора %1 не удалось создать пакет документов по причине: %2';ru='Для договора %1 не удалось создать пакет документов по причине: %2';en='% 1 was unable to create contract documents for a reason:% 2'"),
...
Функция СоздатьСчетНаОплату(Дата, Договор, Контрагент, ОборотыПоДоговору)
	...
	СтруктураПараметры.Вставить("ВидЦен",СчетНаОплату.ВидЦен);// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
...
Функция СоздатьЭлектронноеПисьмо(ШаблонСообщения, Договор, ДокументыКОтправке)
	...
	Исключение
		ТекстСообщения = СтрШаблон(
			НСтр("ka='Не удалось автоматически отправить электронное письмо по причине: %2';ru='Не удалось автоматически отправить электронное письмо по причине: %2';en='Unable to automatically send an email to the following reason:% 2'"),
