...
Процедура ОбновитьСпискиДокументов()
	...
	ТекстОформите    = НСтр("ka='оформите';ru='оформите';en='Apply for'");
	ТекстОтработайте = НСтр("ka='отработайте';ru='отработайте';en='Practice this'");
	ТекстОжидайте    = НСтр("ka='ожидайте';ru='ожидайте';en='expect'");
	...
	Для ТекИндекс = 0 По МассивЗапросов.Количество()-1 Цикл
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОбОтгрузке" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОбОтгрузке, НСтр("ka='Уведомления об отгрузке';ru='Уведомления об отгрузке';en='Shipping Notes'"));
			КонецЕсли;
		...
		Если ИнтеграцияГИСМПереопределяемый.ДоступенОтчетОРозничныхПродажах() Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаОтчетыОРозничныхПродажах" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьОтчетыОРозничныхПродажах, НСтр("ka='Уведомления о розничных продажах';ru='Уведомления о розничных продажах';en='Notifications on retail sales'"));
			КонецЕсли;
		...
		Если ИнтеграцияГИСМПереопределяемый.ДоступенВозвратТоваровОтРозничногоКлиента() Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаВозвратыТоваровОтРозничныхКлиентов" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьВозвратыТоваровОтРозничныхКлиентов, НСтр("ka='Уведомления о розничных возвратах';ru='Уведомления о розничных возвратах';en='Notifications of retail returns'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.МаркировкаТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаМаркировкаТоваров" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьМаркировкуТоваров, НСтр("ka='საქონლის მარკირება';ru='Маркировка товаров';en='marking of goods'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.ПеремаркировкаТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаПеремаркировкаТоваров" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьПереМаркировкуТоваров, НСтр("ka='საქონლის გადამარკირება';ru='Перемаркировка товаров';en='remarking goods'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОСписанииКиЗГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОСписанииКиЗГИСМ" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьСписанияКиЗМаркировки, НСтр("ka='შეტყობინებები სსნ-ების ჩამოწერის თაობაზე';ru='Уведомления о списании КиЗ';en='Notice of cancellation of Keyes'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаявкаНаВыпускКиЗГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаЗаявкиНаВыпускКиЗГИСМ" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьЗаявкиНаВыпускКиЗДляМаркировки, НСтр("ka='განაცხადები სსნ-ის გამოშვებაზე';ru='Заявки на выпуск КиЗ';en='Applications for the release of Keys'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОПоступленииМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОПоступлении" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОПоступленииМаркированныхТоваров, НСтр("ka='Уведомления о поступлении и возврате';ru='Уведомления о поступлении и возврате';en='Notifications on the receipt and return of'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОВвозеМаркированныхТоваровИзЕАЭСГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОВвозеИзЕАЭС" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОВвозеМаркированныхТоваровИзЕАЭС, НСтр("ka='Уведомления о ввозе из ЕАЭС';ru='Уведомления о ввозе из ЕАЭС';en='Notifications on the import of the EAEC'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОбИмпортеМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОбИмпорте" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОбИмпортеМаркированныхТоваров, НСтр("ka='Уведомления об импорте';ru='Уведомления об импорте';en='Notification of imports'"));
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		Если Выборка.Следующий() И Выборка.КоличествоДокументов > 0 Тогда
			ТекстЗаголовка = СтрШаблон(НСтр("ka='Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.';ru='Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.';en='There are documents (1%), waiting to be sent in GISM. Perform exchange.'"), Выборка.КоличествоДокументов);
