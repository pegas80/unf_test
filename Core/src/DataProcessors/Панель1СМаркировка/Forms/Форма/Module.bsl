...
Процедура ОбновитьСпискиДокументов()
	...
	ТекстОформите    = НСтр("ka='оформите';ru='оформите'");
	ТекстОтработайте = НСтр("ka='отработайте';ru='отработайте'");
	ТекстОжидайте    = НСтр("ka='ожидайте';ru='ожидайте'");
	...
	Для ТекИндекс = 0 По МассивЗапросов.Количество()-1 Цикл
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОбОтгрузке" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОбОтгрузке, НСтр("ka='Уведомления об отгрузке';ru='Уведомления об отгрузке'"));
			КонецЕсли;
		...
		Если ИнтеграцияГИСМПереопределяемый.ДоступенОтчетОРозничныхПродажах() Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаОтчетыОРозничныхПродажах" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьОтчетыОРозничныхПродажах, НСтр("ka='Уведомления о розничных продажах';ru='Уведомления о розничных продажах'"));
			КонецЕсли;
		...
		Если ИнтеграцияГИСМПереопределяемый.ДоступенВозвратТоваровОтРозничногоКлиента() Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаВозвратыТоваровОтРозничныхКлиентов" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьВозвратыТоваровОтРозничныхКлиентов, НСтр("ka='Уведомления о розничных возвратах';ru='Уведомления о розничных возвратах'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.МаркировкаТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаМаркировкаТоваров" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьМаркировкуТоваров, НСтр("ka='საქონლის მარკირება';ru='Маркировка товаров'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.ПеремаркировкаТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаПеремаркировкаТоваров" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьПереМаркировкуТоваров, НСтр("ka='საქონლის გადამარკირება';ru='Перемаркировка товаров'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОСписанииКиЗГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОСписанииКиЗГИСМ" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьСписанияКиЗМаркировки, НСтр("ka='შეტყობინებები სსნ-ების ჩამოწერის თაობაზე';ru='Уведомления о списании КиЗ'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаявкаНаВыпускКиЗГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаЗаявкиНаВыпускКиЗГИСМ" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьЗаявкиНаВыпускКиЗДляМаркировки, НСтр("ka='განაცხადები სსნ-ის გამოშვებაზე';ru='Заявки на выпуск КиЗ'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОПоступленииМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОПоступлении" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОПоступленииМаркированныхТоваров, НСтр("ka='Уведомления о поступлении и возврате';ru='Уведомления о поступлении и возврате'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОВвозеМаркированныхТоваровИзЕАЭСГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОВвозеИзЕАЭС" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОВвозеМаркированныхТоваровИзЕАЭС, НСтр("ka='Уведомления о ввозе из ЕАЭС';ru='Уведомления о ввозе из ЕАЭС'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОбИмпортеМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОбИмпорте" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОбИмпортеМаркированныхТоваров, НСтр("ka='Уведомления об импорте';ru='Уведомления об импорте'"));
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		Если Выборка.Следующий() И Выборка.КоличествоДокументов > 0 Тогда
			ТекстЗаголовка = СтрШаблон(НСтр("ka='Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.';ru='Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.'"), Выборка.КоличествоДокументов);
