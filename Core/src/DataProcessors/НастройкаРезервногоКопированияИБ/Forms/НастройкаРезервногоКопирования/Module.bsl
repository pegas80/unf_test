...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='Резервное копирование поддерживается только в клиенте под управлением ОС Windows.';ru='Резервное копирование поддерживается только в клиенте под управлением ОС Windows.';en='Backup is only supported on the client running on Windows.'");
...
Процедура ПутьККаталогуАрхивовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Заголовок= НСтр("ka='Выберите каталог для сохранения резервных копий';ru='Выберите каталог для сохранения резервных копий';en='Select a folder to save the backup'");
...
Процедура ПерейтиСоСтраницыНастройки()
	...
	Если Объект.ВыполнятьАвтоматическоеРезервноеКопирование Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ka='სარეზერვო კოპირება';ru='Резервное копирование';en='Backup'"), ИмяФормыНастроек,
			НСтр("ka='Резервное копирование настроено.';ru='Резервное копирование настроено.';en='Backup configured.'"));
