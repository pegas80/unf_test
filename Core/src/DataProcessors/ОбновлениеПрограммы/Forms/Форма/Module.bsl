...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ
		...
		СообщениеЖурнала = НСтр("ka='Обновление программы невозможно. В подчиненном узле информационной базы обновление получается из главного узла.';ru='Обновление программы невозможно. В подчиненном узле информационной базы обновление получается из главного узла.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ka='<b>Обновление программы невозможно</b><br />В подчиненном узле информационной базы обновление получается из главного узла.';ru='<b>Обновление программы невозможно</b><br />В подчиненном узле информационной базы обновление получается из главного узла.'"),
			,
			СообщениеЖурнала);
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
		Возврат;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		СообщениеЖурнала = НСтр("ka='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.';ru='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
...
"Откройте программу в тонком или толстом клиенте и повторите попытку.'"),
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ka='Переход на новую версию платформы 1С:Предприятие недоступен в текущем режиме работы.';ru='Переход на новую версию платформы 1С:Предприятие недоступен в текущем режиме работы.'");
	...
	ИначеЕсли (ЭтоСценарийРабочегоОбновления(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ka='Получение обновлений программы недоступно в текущем режиме работы.';ru='Получение обновлений программы недоступно в текущем режиме работы.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Если Не ЭтоАдминистраторСистемы Тогда
			...
			ВызватьИсключение НСтр("ka='Недостаточно прав для перехода на новую версию платформы.';ru='Недостаточно прав для перехода на новую версию платформы.'");
		КонецЕсли;
		
		ЭтотОбъект.АвтоЗаголовок = Ложь;
		ЭтотОбъект.Заголовок     = НСтр("ka='Переход на новую версию платформы 1С:Предприятие';ru='Переход на новую версию платформы 1С:Предприятие'");
	...
	ИначеЕсли Не ЭтоСценарийРабочегоОбновления(ЭтотОбъект) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Неизвестный сценарий работы помощника обновления программы (""%1"").';ru='Неизвестный сценарий работы помощника обновления программы (""%1"").'"),
			Сценарий);
	...
	Элементы.ДекорацияНаписатьВТехПоддержку.Заголовок =
		ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				НСтр("ka='<body>При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';ru='<body>При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>'")));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПолучениеИУстановка
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ka='Не завершена установка платформы 1С:Предприятие.';ru='Не завершена установка платформы 1С:Предприятие.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ЗавершениеРаботы Тогда
		Если ЗначениеЗаполнено(ИДФоновогоЗадания) Тогда
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ka='Получение и установка обновлений не завершены.';ru='Получение и установка обновлений не завершены.'");
		ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.ВыборРежимаОбновленияФайловыйРежим
			...
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ka='Не установлено обновление программы.';ru='Не установлено обновление программы.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДекорацияЗагрузитьДистрибутивКомпонентаПлатформыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(URLСтраницыПлатформы) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Адрес страницы новой версии платформы не определен.';ru='Адрес страницы новой версии платформы не определен.'"));
		Возврат;
	КонецЕсли;
...
Процедура КаталогСохраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбора.Заголовок = НСтр("ka='Каталог хранения дистрибутивов платформы 1С:Предприятие';ru='Каталог хранения дистрибутивов платформы 1С:Предприятие'");
...
Процедура ДекорацияЗарегистрироватьсяНаПортале1СИТСНажатие(Элемент)
	...
	ОткрытьВебСтраницу(
		ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыСервисаLogin(
			"/registration",
			НастройкиСоединения()),
		НСтр("ka='რეგისტრაცია';ru='Регистрация'"));
...
Процедура КомандаДалее(Команда)
		...
		Если Элементы.ГруппаКаталогСохраненияПлатформы.Видимость
			...
			Если ПустаяСтрока(КаталогСохраненияПлатформы) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='Не выбран каталог хранения дистрибутивов платформы.';ru='Не выбран каталог хранения дистрибутивов платформы.'"),
					,
	...
	ИначеЕсли ТекСтраница = Элементы.СтраницаПодключениеКПорталу Тогда
		...
		Если ПустаяСтрока(Логин) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнен логин.';ru='Не заполнен логин.'"),
				,
				"Логин",
				,
				Отказ);
		...
		Если ПустаяСтрока(Пароль) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнен пароль.';ru='Не заполнен пароль.'"),
				,
				"Пароль",
				,
				Отказ);
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	Если Ссылка = "open:RCInstruction" Тогда
		...
		ОткрытьВнутреннийФайлИнструкции(
			...
			НСтр("ka='Создание резервной копии.htm';ru='Создание резервной копии.htm'"));
		
	ИначеЕсли Ссылка = "open:V8Update" Тогда
		
		ОткрытьВебСтраницу(
			ИнформацияОДоступномОбновлении.Платформа.URLОсобенностиПерехода,
			НСтр("ka='Особенности перехода на новую версию платформы 1С:Предприятие';ru='Особенности перехода на новую версию платформы 1С:Предприятие'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(АдресПолучателя)
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		Или ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект) Тогда
		Тема = НСтр("ka='Интернет-поддержка. Переход на новую версию Платформы 1С:Предприятие';ru='Интернет-поддержка. Переход на новую версию Платформы 1С:Предприятие'");
	Иначе
		Тема = НСтр("ka='Интернет-поддержка. Обновление программы';ru='Интернет-поддержка. Обновление программы'");
	КонецЕсли;
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПодключениеКПорталу Тогда
		...
		Сообщение = НСтр("ka='Не удалось подключиться к Порталу 1С:ИТС.';ru='Не удалось подключиться к Порталу 1С:ИТС.'")
			+ ?(ПустаяСтрока(ОписаниеПричины),
				"",
				Символы.ПС + Символы.ПС + НСтр("ka='Описание:';ru='Описание:'") + Символы.ПС + ОписаниеПричины);
		
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНеУдалосьПодключитьсяКСервису Тогда
		
		Сообщение = НСтр("ka='Ошибка при подключении сервиса автоматического обновления программы.';ru='Ошибка при подключении сервиса автоматического обновления программы.'")
			+ ?(ПустаяСтрока(ПодробноеОписаниеОшибки),
				"",
				Символы.ПС + Символы.ПС + НСтр("ka='Описание:';ru='Описание:'") + Символы.ПС + ПодробноеОписаниеОшибки);
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСообщениеСервиса Тогда
		...
		Если ПустаяСтрока(ПодробноеОписаниеОшибки) Тогда
			Сообщение = НСтр("ka='Сообщение при подключении сервиса автоматического обновления программы:';ru='Сообщение при подключении сервиса автоматического обновления программы:'")
				+ Символы.ПС
		...
		Иначе
				...
				Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(КонтекстОбновления.ПутьФайлаПротокола) Тогда
					Вложения.Добавить(
						Новый Структура("Представление, ИмяФайла",
							НСтр("ka='Протокол установки платформы 1С_Предприятие.txt';ru='Протокол установки платформы 1С_Предприятие.txt'"),
			...
			Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
				...
				Сообщение = НСтр("ka='Ошибка при получении и установке платформы 1С:Предприятие:';ru='Ошибка при получении и установке платформы 1С:Предприятие:'")
					+ Символы.ПС
					+ ПодробноеОписаниеОшибки;
				
			Иначе
				
				Сообщение = НСтр("ka='Не удалось обновить программу.';ru='Не удалось обновить программу.'")
					+ Символы.ПС
					+ НСтр("ka='Причина:';ru='Причина:'")
					+ Символы.ПС
					+ ПодробноеОписаниеОшибки;
...
Процедура ОткрытьСозданиеРезервнойКопииИБ()
	...
	Иначе
		ТекстСообщения = НСтр("ka='Не встроена подсистема ""Резервное копирование ИБ"".';ru='Не встроена подсистема ""Резервное копирование ИБ"".'");
...
"%2'"),
		...
		ПоказатьПредупреждение(, НСтр("ka='Ошибка при открытии файла.';ru='Ошибка при открытии файла.'"));
		Возврат;
	КонецПопытки;
...
Функция ПодготовитьФайлМакетаИнструкции(Знач ИмяМакета, Знач ИмяФайла)
	...
	Если Не ПолучениеОбновленийПрограммы.СлужебнаяДоступноИспользованиеПолученияОбновленийПлатформы() Тогда
		ВызватьИсключение НСтр("ka='Использование обновления платформы 1С:Предприятие недоступно в текущем режиме работы.';ru='Использование обновления платформы 1С:Предприятие недоступно в текущем режиме работы.'");
	КонецЕсли;
...
"%2'"),
					КаталогДляРаботыСОбновлениями,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
			ВызватьИсключение НСтр("ka='Ошибка при создании временного каталога.';ru='Ошибка при создании временного каталога.'");
		КонецПопытки;
	КонецЕсли;
...
"%2'"),
				ИмяФайла,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		ВызватьИсключение НСтр("ka='Ошибка при создании временного файла.';ru='Ошибка при создании временного файла.'");
	КонецПопытки;
...
Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновлении()
	...
	Состояние(, , НСтр("ka='Получение информации о доступном обновлении';ru='Получение информации о доступном обновлении'"));
	...
	Если Не ПустаяСтрока(ИнформацияОДоступномОбновлении.ИмяОшибки) Тогда
		...
		Иначе
			ОтобразитьСообщениеСервиса(
				ИнформацияОДоступномОбновлении.Сообщение,
				НСтр("ka='Повторить попытку подключения >';ru='Повторить попытку подключения >'"),
				ИнформацияОДоступномОбновлении.ИнформацияОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ОтобразитьИнформациюОДоступномОбновленииПлатформы(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(ОбновлениеПлатформы.Версия) Тогда
		...
		Элементы.ДекорацияНетВерсии.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Используемая сейчас версия: %1</body>';ru='<body>Используемая сейчас версия: %1</body>'"),
...
"<br />Используемая сейчас версия: %2</body>'"),
	...
	Если Не ЕстьОбновление Тогда
		...
		Элементы.КнопкаДалее.Заголовок    = НСтр("ka='შემდეგ >';ru='Далее >'");
		Элементы.КнопкаОтмена.Заголовок   = НСтр("ka='დახურვა';ru='Закрыть'");
		Возврат;
	...
	Иначе
		...
		Иначе
			...
			Элементы.НадписьВерсия.Заголовок = НСтр("ka='ვერსია';ru='Версия'")
				+ " " + ОбновлениеПлатформы.Версия;
			...
			Иначе
				...
				Элементы.КнопкаДалее.Доступность  = Истина;
				Элементы.КнопкаДалее.Заголовок    = НСтр("ka='მზად არის';ru='Готово'");
			...
			Элементы.КнопкаОтмена.Доступность = Истина;
			Элементы.ПродолжитьРаботуНаТекущейВерсии.Видимость = Ложь;
			Элементы.КнопкаОтмена.Заголовок   = НСтр("ka='გაუქმება';ru='Отмена'");
...
Процедура ОтобразитьСтраницуНоваяВерсияПлатформыУжеУстановленаНаКомпьютере()
	...
	Элементы.НадписьВерсияУстановлено.Заголовок =
		НСтр("ka='ვერსია';ru='Версия'") + " " + ОбновлениеПлатформы.Версия;
...
"<br />с <a href=""open:V8Update"">особенностями перехода</a> на эту версию платформы.</body>'"),
	...
	Иначе
		...
		Элементы.НадписьУстановленоДополнительно.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Версия платформы %1 уже установлена на компьютере.</body>';ru='<body>Версия платформы %1 уже установлена на компьютере.</body>'"),
					ОбновлениеПлатформы.Версия));
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Если Не ОбновлениеКомКонф.ДоступноОбновление И Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Если ЭтоСценарийРабочегоОбновления(ЭтотОбъект) Или ПустаяСтрока(ЗаголовокНетОбновления) Тогда
			ТекстСообщения = ТекстСообщения
				+ НСтр("ka='Обновление не требуется. Установлена актуальная версия программы.';ru='Обновление не требуется. Установлена актуальная версия программы.'");
		Иначе
	...
	Иначе
		...
		Если ПрямоеОбновление Тогда
			ТекстСообщения = ТекстСообщения + НСтр("ka='ხელმისაწვდომია პროგრამის განახლება.';ru='Доступно обновление программы.'");
		Иначе
			ТекстСообщения = ТекстСообщения
		...
		Если РазмерОбновления <> 0 Тогда
			ТекстСообщения = ТекстСообщения + ?(ПрямоеОбновление, " ", "<br />")
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='კონფიგურაციის ზომა: <b>%1</b>';ru='Размер дистрибутива: <b>%1</b>'"),
		...
		Если ОбновитьКонфигурацию Тогда
			Если ЭтоАдминистраторСистемы И Не ПустаяСтрока(ОбновлениеКомКонф.URLПорядокОбновления) Тогда
				ТекстСообщения = ТекстСообщения
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='<br /><b>Внимание!</b> Для продолжения необходимо ознакомиться с информацией о <a href=""%1"">порядке обновления</a>.';ru='<br /><b>Внимание!</b> Для продолжения необходимо ознакомиться с информацией о <a href=""%1"">порядке обновления</a>.'"),
		...
		Если Не ЭтоАдминистраторСистемы Тогда
			ТекстСообщения = ТекстСообщения
				+ НСтр("ka='<br />Для обновления программы обратитесь к администратору.';ru='<br />Для обновления программы обратитесь к администратору.'");
		КонецЕсли;
...
Процедура НастроитьКнопкиКоманднойПанелиДляКомпонентов()
	...
	ИначеЕсли ОбновитьКонфигурацию Тогда
		...
		Если ОбновитьПлатформу Тогда
			Элементы.КнопкаДалее.Доступность = Ложь;
			Элементы.КнопкаДалее.Заголовок   = НСтр("ka='შემდეგ >';ru='Далее >'");
			Элементы.КнопкаОтмена.Заголовок  = НСтр("ka='შემდეგ >';ru='Далее >'");
		Иначе
			Элементы.КнопкаДалее.Доступность = Истина;
			Элементы.КнопкаДалее.Заголовок   = НСтр("ka='შემდეგ >';ru='Далее >'");
			Элементы.КнопкаОтмена.Заголовок  = НСтр("ka='გაუქმება';ru='Отмена'");
...
Процедура ОтобразитьКомпонентКонфигурации()
	...
	Если ПустаяСтрока(ОбновлениеКомКонфигурации.Версия) Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеКонфигурации.Заголовок = НСтр("ka='კონფიგურაციის განახლება';ru='Обновление конфигурации'");
...
"%2</body>'"),
	...
	Иначе
		...
		Элементы.ДекорацияЗаголовокОбновлениеКонфигурации.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Обновление конфигурации, <b>%1</b></body>';ru='<body>Обновление конфигурации, <b>%1</b></body>'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
...
Процедура ОтобразитьКомпонентПлатформы()
	...
	Если Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
			НСтр("ka='Обновление платформы 1С:Предприятие';ru='Обновление платформы 1С:Предприятие'");
		Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Обновление не требуется. Текущая версия платформы 1С:Предприятие: <b>%1</b></body>';ru='<body>Обновление не требуется. Текущая версия платформы 1С:Предприятие: <b>%1</b></body>'"),
					ПолучениеОбновленийПрограммыКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие()));
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='<body>Обновление платформы 1С:Предприятие, <b>%1</b></body>';ru='<body>Обновление платформы 1С:Предприятие, <b>%1</b></body>'"),
						ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
							ОбновлениеКомПл.РазмерОбновления)));
		Иначе
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
						НСтр("ka='<body>Обновление платформы 1С:Предприятие</body>';ru='<body>Обновление платформы 1С:Предприятие</body>'"));
		КонецЕсли;
...
"Текущая версия платформы 1С:Предприятие: %2</body>'"),
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Обновление платформы является обязательным для работы с новой версией конфигурации %1.';ru='Обновление платформы является обязательным для работы с новой версией конфигурации %1.'"),
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки = 1 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Новая версия %1 платформы 1С:Предприятие рекомендуется для работы с новой версией конфигурации %2.';ru='Новая версия %1 платформы 1С:Предприятие рекомендуется для работы с новой версией конфигурации %2.'"),
							ОбновлениеКомПл.Версия,
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				Иначе
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Обновление платформы не является обязательным для работы с новой версией конфигурации %1.';ru='Обновление платформы не является обязательным для работы с новой версией конфигурации %1.'"),
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				КонецЕсли;
			...
			ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки < 2 Тогда
				...
				Элементы.ДекорацияПояснениеПлатформа.Заголовок =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Рекомендуется установить новую версию платформы 1С:Предприятие.';ru='Рекомендуется установить новую версию платформы 1С:Предприятие.'"),
						ИнформацияОДоступномОбновлении.Конфигурация.Версия);
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='<body>Для работы новой версии конфигурации требуется платформа 1С:Предприятие не ниже версии <b>%1</b>.</body>';ru='<body>Для работы новой версии конфигурации требуется платформа 1С:Предприятие не ниже версии <b>%1</b>.</body>'"),
...
Процедура НачатьПолучениеОбновлений()
	...
	Иначе
		...
		Состояние(, , НСтр("ka='Подключение к сервису обновлений';ru='Подключение к сервису обновлений'"));
		...
		Иначе
			...
			Иначе
				...
				ОтобразитьСообщениеСервиса(
					ОписаниеФайловОбновлений.Сообщение,
					НСтр("ka='Повторить попытку >';ru='Повторить попытку >'"),
					ОписаниеФайловОбновлений.ИнформацияОбОшибке);
...
Процедура СохранитьДанныеАутентификации(Знач ДанныеАутентификации)
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ka='ინტერნეტ-მხარდაჭერის ავთენტიფიკაციის მონაცემთა ჩასაწერად არასაკმარისი უფლებებია.';ru='Недостаточно прав для записи данных аутентификации Интернет-поддержки.'");
	КонецЕсли;
...
"Имя файла обновления (cfu): %7.'"),
		...
		Если Не ПолучениеОбновленийПрограммыКлиентСервер.ОбновлениеКонфигурацииПолучено(ТекОбновление, КонтекстОбновления) Тогда
			...
			КонтекстОбновления.ТекущееДействие =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Получение файла %1 из %2';ru='Получение файла %1 из %2'"),
					Строка(Итератор + 1),
		...
		Иначе
			...
			ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ka='Обновление конфигурации уже было получено ранее.';ru='Обновление конфигурации уже было получено ранее.'"));
...
Процедура ИнициализироватьСтраницуПолучениеОбновления()
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მიმდინარეობს პლატფორმა 1С:საწარმოს დისტრიბუტივის ფაილის მიღება (%1).';ru='Выполняется получение файла дистрибутива платформы 1С:Предприятие (%1).'") + " "
...
Процедура НачатьПолучениеОбновленийВФоновомРежиме()
	...
	Если КонтекстОбновления = Неопределено Тогда
		Прогресс = 0;
		ТекущееДействие = НСтр("ka='Подготовка к получению обновления...';ru='Подготовка к получению обновления...'");
	КонецЕсли;
...
Функция СостояниеЗадания(Знач ИДЗадания)
	...
	Если Задание = Неопределено Тогда
		...
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(НСтр("ka='დავალება ვერ მოიძებნა.';ru='Задание не найдено.'"));
		
		Результат.КодСостояния = "Ошибка";
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ka='დავალება ვერ მოიძებნა.';ru='Задание не найдено.'"),
				НСтр("ka='დავალება ვერ მოიძებნა.';ru='Задание не найдено.'")));
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		СообщениеЖурнала = НСтр("ka='დავალება გაუქმებულია სისტემის ადმინისტრატორის მიერ.';ru='Задание отменено администратором системы.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		...
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ka='დავალება გაუქმებულია სისტემის ადმინისტრატორის მიერ.';ru='Задание отменено администратором системы.'"),
				СообщениеЖурнала));
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დავალება დასრულებულია შეცდომით. %1';ru='Задание завершено с ошибкой. %1'"),
...
Процедура ОтобразитьОшибкуПодключения(Сообщение, ОписаниеОшибкиПодключения)
	...
	Элементы.КнопкаНазад.Доступность =
		...
		Или ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект));
	Элементы.КнопкаДалее.Заголовок    = НСтр("ka='Повторить попытку подключения >';ru='Повторить попытку подключения >'");
	Элементы.КнопкаДалее.Доступность  = Истина;
	Элементы.КнопкаОтмена.Доступность = Истина;
	Элементы.ПродолжитьРаботуНаТекущейВерсии.Видимость = Ложь;
...
Процедура ОтобразитьПолучениеОбновленийЗавершено(Инициализировать = Ложь)
	...
	Иначе
		...
		Если ПараметрыАдминистрирования = Неопределено Тогда
			...
			ЗаголовокФормы = Нстр("ka='განახლების დაყენება';ru='Установка обновления'");
			Если ЭтоФайловаяИБ Тогда
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, Инициализировать) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = Нстр("ka='Для установки обновления необходимо ввести параметры администрирования.';ru='Для установки обновления необходимо ввести параметры администрирования.'");
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Элементы.КнопкаДалее.Заголовок = ?(
				РежимОбновления = 0,
				НСтр("ka='შემდეგ >';ru='Далее >'"),
				НСтр("ka='მზად არის';ru='Готово'"));
...
Процедура ОтобразитьНерекомендуемаяВерсияПлатформы()
	...
	Элементы.КнопкаОтмена.Заголовок   = НСтр("ka='მუშაობის დასრულება';ru='Завершить работу'");
...
Функция ЗаголовокСтраницыНоваяВерсияПлатформы()
	...
	Возврат ?(ИнформацияОДоступномОбновлении.Платформа.РекомендуетсяПереход,
		...
		НСтр("ka='ხელმისაწვდომია პლატფორმის ახალი ვერსია';ru='Доступна новая версия платформы'"));
...
Функция ТекстИнструкцииПоПереходуНаНовуюВерсиюПлатформы(ПараметрСоздатьРезервнуюКопию)
	...
	Если ВнедренаПодсистемаРезервногоКопирования
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
			Строки.Добавить(
				НСтр("ka='<body>ავტომატურად დასრულდება პროგრამის <a href=""open:ActiveUsers"">აქტიური მომხმარებლების</a> მუშაობა, შექმნილია სარეზერვო ასლი';ru='<body>Будет автоматически завершена работа <a href=""open:ActiveUsers"">активных пользователей</a> программы, создана резервная копия'"));
		Иначе
			Строки.Добавить(
				НСтр("ka='<body>ავტომატურად დასრულდება პროგრამის აქტიური მომხმარებლების მუშაობა, შეიქმნება სარეზერვო ასლი';ru='<body>Будет автоматически завершена работа активных пользователей программы, создана резервная копия'"));
		КонецЕсли;
		
		Если ЭтоБазоваяВерсияКонфигурации Тогда
			Строки.Добавить(НСтр("ka='საინფორმაციო ბაზას და გაშვებულია პროგრამასთან პლატფორმის ახალ ვერსიაზე მუშაობის სეანსი.<body>';ru='информационной базы и запущен сеанс работы с программой на новой версии платформы.<body>'"));
		Иначе
...
"<br />პროგრამის სხვა მომხმარებლების კომპიუტერებზე აუცილებელია პლატფორმა 1С:საწარმოს ხელით განახლება.</body>'"));
	...
	Иначе
		...
		Строки.Добавить(НСтр("ka='<body>პლატფორმის ახალ ვერსიაზე მუშაობის დასაწყებად:';ru='<body>Чтобы начать работать на новой версии платформы:'"));
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
			Строки.Добавить(НСтр("ka=' - დაასრულეთ პროგრამის ყველა <a href=""open:ActiveUsers"">აქტიური მომხმარებლის</a> მუშაობა;';ru='- завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;'"));
		Иначе
			Строки.Добавить(НСтр("ka=' - დაასრულეთ პროგრამის ყველა აქტიური მომხმარებლის მუშაობა;';ru='- завершите работу всех активных пользователей программы;'"));
		КонецЕсли;
		
		Строки.Добавить(НСтр("ka='- завершите текущий сеанс работы с программой;';ru='- завершите текущий сеанс работы с программой;'"));
		
		Если Не ЭтоБазоваяВерсияКонфигурации Тогда
			Строки.Добавить(НСтр("ka=' - პროგრამის სხვა მომხმარებლების კომპიუტერებზე დააყენეთ პლატფორმის ახალი ვერსია;';ru='- установите новую версию платформы на компьютеры других пользователей программы;'"));
		КонецЕсли;
		
		Строки.Добавить(НСтр("ka=' - გახსენით პროგრამა პლატფორმის ახალი ვერსიით;';ru='- откройте программу на новой версии платформы;'"));
		
		Если ВнедренаПодсистемаРезервногоКопирования Тогда
			Строки.Добавить(НСтр("ka='<br />პლატფორმის ახალ ვერსიაზე მუშაობის დაწყებამდე რეკომენდებულია სარეზერვო ასლის შექმნა.</body>';ru='<br />Перед началом работы на новой версии платформы рекомендуется создать резервную копию.</body>'"));
		Иначе
			Строки.Добавить(НСтр("ka='<br />პლატფორმის ახალ ვერსიაზე მუშაობის დაწყებამდე რეკომენდებულია <a href=""open:RCInstruction"">სარეზერვო ასლის შექმნა</a>.</body>';ru='<br />Перед началом работы на новой версии платформы рекомендуется <a href=""open:RCInstruction"">создать резервную копию</a>.</body>'"));
		КонецЕсли;
