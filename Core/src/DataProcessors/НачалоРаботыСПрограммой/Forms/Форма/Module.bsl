...
Процедура ДобавитьРеквизитыИЭлементыФормы(ИндексОрганизации)
	...
	ДекорацияЗаголовокИНН.Заголовок = Нстр("ka='ს.კ / პ.ნ:';ru='ИНН:';en='INN:'");
	...
	ДекорацияЗаголовокНаименование.Заголовок = Нстр("ka='Название:';ru='Название:';en='Title:'");
	...
	ЭлементНаименование.ПодсказкаВвода = НСтр("ka='ორგანიზაციის დასახელება';ru='Наименование организации';en='Name of company'");
	...
	ЭлементИП_Фамилия.ПодсказкаВвода = НСтр("ka='გვარი';ru='Фамилия';en='Surname'");
	...
	ЭлементИП_Имя.ПодсказкаВвода = НСтр("ka='სახელი';ru='Имя';en='Name'");
	...
	ЭлементИП_Отчество.ПодсказкаВвода = НСтр("ka='მამის სახელი';ru='Отчество';en='middle name'");
	...
	ЭлементЕстьУСН.Заголовок = НСтр("ka='УСН';ru='УСН';en='USN'");
	...
	ЭлементЕстьЕНВД.Заголовок = НСтр("ka='ЕНВД';ru='ЕНВД';en='UTII'");
	...
	ЭлементЕстьПатент.Заголовок = НСтр("ka='Патент';ru='Патент';en='Patent'");
	...
	ЭлементВидыОбъектовНалогообложения.СписокВыбора.Добавить(Перечисления.ВидыОбъектовНалогообложения.Доходы, Нстр("ka='შემოსავლები';ru='Доходы';en='earnings'"));
	ЭлементВидыОбъектовНалогообложения.СписокВыбора.Добавить(Перечисления.ВидыОбъектовНалогообложения.ДоходыМинусРасходы, Нстр("ka='Доходы - расходы по налоговой ставке';ru='Доходы - расходы по налоговой ставке';en='Revenues - expenses at the tax rate'"));
	...
	ЭлементСтавкаНалога.Заголовок = НСтр("ka='%';ru='%';en='%'");
	...
	ЭлементИПИспользуетТрудНаемныхРаботников.Заголовок = НСтр("ka='Есть официально трудоустроенные сотрудники';ru='Есть официально трудоустроенные сотрудники';en='There are officially employees, employed'");
	...
	ЭлементАдресЛоготипа.ТекстНевыбраннойКартинки = НСтр("ka='ლოგოტიპის დამატება';ru='Добавить логотип';en='Add logo'");
...
Процедура ПослеЗаписиЗначенияЮрФизЛицо(ЗначениеЮрФизЛицо, ЗначениеЕстьУСН, Индекс)
	...
	Если ЗначениеЮрФизЛицо = КэшЗначений.ЮрЛицо Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияЗаголовокНаименование" + Индекс, "Заголовок", Нстр("ka='Название:';ru='Название:';en='Title:'"));
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияЗаголовокНаименование" + Индекс, "Заголовок", Нстр("ka='ФИО:';ru='ФИО:';en='FULL NAME:'"));
...
Процедура РеквизитыЗаполненыКорректно(Ошибки)
	...
	Если ПустаяСтрока(ПользовательИмя) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ПользовательИмя", НСтр("ka='Заполните имя пользователя';ru='Заполните имя пользователя';en='Fill in the username'"), "");
		
	КонецЕсли;
	
	Если ПустаяСтрока(ПользовательEmail) Тогда
		
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ПользовательEmail", НСтр("ka='Укажите e-mail';ru='Укажите e-mail';en='Enter the e-mail'"), "")
		
	КонецЕсли;
	
	Если ПользовательПароль <> ПользовательПарольПодтверждение Тогда
		
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ПользовательПароль", НСтр("ka='Не совпадают пароли';ru='Не совпадают пароли';en='Passwords do not match'"), "")
	...
	Для каждого СтрокаОрганизации Из Объект.Организации Цикл
		...
		Если ПустаяСтрока(СтрокаОрганизации.Наименование) Тогда
			...
			ТекстОшибки = НСтр("ka='Заполните название (организация %1)';ru='Заполните название (организация %1)';en='Fill in the name of the (organization 1%)'");
		...
		Если СтрокаОрганизации.ЮридическоеФизическоеЛицо = КэшЗначений.ФизЛицо Тогда
			...
			Если ПустаяСтрока(СтрокаОрганизации.Фамилия) Тогда
				...
				ТекстОшибки = НСтр("ka='Заполните фамилию (организация %1)';ru='Заполните фамилию (организация %1)';en='Fill in the name (organization 1%)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаОрганизации.НомерСтроки);
			...
			Если ПустаяСтрока(СтрокаОрганизации.Имя) Тогда
				...
				ТекстОшибки = НСтр("ka='Заполните имя (организация %1)';ru='Заполните имя (организация %1)';en='Fill in the name of (organization 1%)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаОрганизации.НомерСтроки);
...
Процедура ДобавитьОрганизацию(СоздатьРеквизитыЭлементы, ОрганизацияСсылка = Неопределено)
	...
	СтрокаТаблицы.Наименование = НСтр("ka='Наша фирма';ru='Наша фирма';en='our company'");
	СтрокаТаблицы.НаименованиеПолное = НСтр("ka='Наша фирма';ru='Наша фирма';en='our company'");
...
Функция СформироватьНаименованиеИП(Индекс)
	...
	Наименование = Наименование + НСтр("ka=' ИП';ru=' ИП'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ КэшЗначений.РазрешитьЗакрытие Тогда
		...
		ТекстПредупреждения = НСтр("ka='Заполните, пожалуйста, обязательные поля."
"Работа программы без указанных сведений затруднительна.';ru='Заполните, пожалуйста, обязательные поля."
...
Процедура ПарольПодтверждениеПриИзменении(Элемент)
	...
	Если ПользовательПароль <> ПользовательПарольПодтверждение Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не совпадают пароли';ru='Не совпадают пароли';en='Passwords do not match'"), , "ПользовательПарольПодтверждение");
