...
Процедура ДобавитьРеквизитыИЭлементыФормы(ИндексОрганизации)
	...
	ДекорацияЗаголовокИНН.Заголовок = Нстр("ka='ს.კ / პ.ნ:';ru='ИНН:'");
	...
	ДекорацияЗаголовокНаименование.Заголовок = Нстр("ka='Название:';ru='Название:'");
	...
	ЭлементНаименование.ПодсказкаВвода = НСтр("ka='ორგანიზაციის დასახელება';ru='Наименование организации'");
	...
	ЭлементИП_Фамилия.ПодсказкаВвода = НСтр("ka='გვარი';ru='Фамилия'");
	...
	ЭлементИП_Имя.ПодсказкаВвода = НСтр("ka='სახელი';ru='Имя'");
	...
	ЭлементИП_Отчество.ПодсказкаВвода = НСтр("ka='მამის სახელი';ru='Отчество'");
	...
	ЭлементЕстьУСН.Заголовок = НСтр("ka='УСН';ru='УСН'");
	...
	ЭлементЕстьЕНВД.Заголовок = НСтр("ka='ЕНВД';ru='ЕНВД'");
	...
	ЭлементЕстьПатент.Заголовок = НСтр("ka='Патент';ru='Патент'");
	...
	ЭлементВидыОбъектовНалогообложения.СписокВыбора.Добавить(Перечисления.ВидыОбъектовНалогообложения.Доходы, Нстр("ka='შემოსავლები';ru='Доходы'"));
	ЭлементВидыОбъектовНалогообложения.СписокВыбора.Добавить(Перечисления.ВидыОбъектовНалогообложения.ДоходыМинусРасходы, Нстр("ka='Доходы - расходы по налоговой ставке';ru='Доходы - расходы по налоговой ставке'"));
	...
	ЭлементСтавкаНалога.Заголовок = НСтр("ka='%';ru='%'");
	...
	ЭлементИПИспользуетТрудНаемныхРаботников.Заголовок = НСтр("ka='Есть официально трудоустроенные сотрудники';ru='Есть официально трудоустроенные сотрудники'");
	...
	ЭлементАдресЛоготипа.ТекстНевыбраннойКартинки = НСтр("ka='ლოგოტიპის დამატება';ru='Добавить логотип'");
...
Процедура ПослеЗаписиЗначенияЮрФизЛицо(ЗначениеЮрФизЛицо, ЗначениеЕстьУСН, Индекс)
	...
	Если ЗначениеЮрФизЛицо = КэшЗначений.ЮрЛицо Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияЗаголовокНаименование" + Индекс, "Заголовок", Нстр("ka='Название:';ru='Название:'"));
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияЗаголовокНаименование" + Индекс, "Заголовок", Нстр("ka='ФИО:';ru='ФИО:'"));
...
Процедура РеквизитыЗаполненыКорректно(Ошибки)
	...
	Если ПустаяСтрока(ПользовательИмя) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ПользовательИмя", НСтр("ka='Заполните имя пользователя';ru='Заполните имя пользователя'"), "");
		
	КонецЕсли;
	
	Если ПустаяСтрока(ПользовательEmail) Тогда
		
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ПользовательEmail", НСтр("ka='Укажите e-mail';ru='Укажите e-mail'"), "")
		
	КонецЕсли;
	
	Если ПользовательПароль <> ПользовательПарольПодтверждение Тогда
		
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ПользовательПароль", НСтр("ka='Не совпадают пароли';ru='Не совпадают пароли'"), "")
	...
	Для каждого СтрокаОрганизации Из Объект.Организации Цикл
		...
		Если ПустаяСтрока(СтрокаОрганизации.Наименование) Тогда
			...
			ТекстОшибки = НСтр("ka='Заполните название (организация %1)';ru='Заполните название (организация %1)'");
		...
		Если СтрокаОрганизации.ЮридическоеФизическоеЛицо = КэшЗначений.ФизЛицо Тогда
			...
			Если ПустаяСтрока(СтрокаОрганизации.Фамилия) Тогда
				...
				ТекстОшибки = НСтр("ka='Заполните фамилию (организация %1)';ru='Заполните фамилию (организация %1)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаОрганизации.НомерСтроки);
			...
			Если ПустаяСтрока(СтрокаОрганизации.Имя) Тогда
				...
				ТекстОшибки = НСтр("ka='Заполните имя (организация %1)';ru='Заполните имя (организация %1)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрокаОрганизации.НомерСтроки);
...
Процедура ДобавитьОрганизацию(СоздатьРеквизитыЭлементы, ОрганизацияСсылка = Неопределено)
	...
	СтрокаТаблицы.Наименование = НСтр("ka='Наша фирма';ru='Наша фирма'");
	СтрокаТаблицы.НаименованиеПолное = НСтр("ka='Наша фирма';ru='Наша фирма'");
...
Процедура ПарольПодтверждениеПриИзменении(Элемент)
	...
	Если ПользовательПароль <> ПользовательПарольПодтверждение Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не совпадают пароли';ru='Не совпадают пароли'"), , "ПользовательПарольПодтверждение");
