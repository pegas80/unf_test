...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы", КодУзлаИнформационнойБазы) Тогда
		...
		Если Объект.УзелИнформационнойБазы.Пустая() Тогда
			...
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Настройка обмена данными не найдена.';ru='Настройка обмена данными не найдена.'"), Отказ);
			Возврат;
	...
	Иначе
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Непосредственное открытие помощника не предусмотрено.';ru='Непосредственное открытие помощника не предусмотрено.'"), Отказ);
		Возврат;
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(
			НСтр("ka='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.'"), Отказ);
		Возврат;
	...
	Если МассивУзлов.Найти(Объект.УзелИнформационнойБазы) = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для выбранного узла сопоставление данных не предусмотрено.';ru='Для выбранного узла сопоставление данных не предусмотрено.'"),,,, Отказ);
		Возврат;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИзменитьЗаголовок Тогда
		Элементы.ЗаголовокСтраницы.Заголовок = НСтр("ka='დაკავშირების პარამეტრები';ru='Параметры подключения'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНачала Тогда
		ТекстПодтверждения = НСтр("ka='Отменить синхронизацию данных?';ru='Отменить синхронизацию данных?'");
	Иначе
		ТекстПодтверждения = НСтр("ka='Прервать синхронизацию данных?';ru='Прервать синхронизацию данных?'");
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхДляСтроки(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		НСтрока = НСтр("ka='Выберите имя таблицы в поле статистической информации.';ru='Выберите имя таблицы в поле статистической информации.'");
...
Процедура ОткрытьФормуСопоставления(Команда)
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ka='Для типа данных нельзя выполнить сопоставление объектов.';ru='Для типа данных нельзя выполнить сопоставление объектов.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДополнениеВыгрузкиОчисткаОбщегоОтбора(Команда)
	...
	ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение'");
	ТекстВопроса   = НСтр("ka='Очистить общий отбор?';ru='Очистить общий отбор?'");
...
Процедура ДополнениеВыгрузкиОчисткаДетальногоОтбора(Команда)
	ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение'");
	ТекстВопроса   = НСтр("ka='Очистить детальный отбор?';ru='Очистить детальный отбор?'");
...
Процедура ДополнениеВыгрузкиИсторияОтборов(Команда)
	...
	Текст = НСтр("ka='Сохранить текущую настройку...';ru='Сохранить текущую настройку...'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("ka='Восстановить настройки ""%1""?';ru='Восстановить настройки ""%1""?'"), ПредставлениеНастройки);
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(Знач ВыделенныеСтроки) 
	...
	Состояние(НСтр("ka='Выполняется загрузка данных...';ru='Выполняется загрузка данных...'"));
...
"Перейти в журнал регистрации?'");
	...
	Состояние(НСтр("ka='Загрузка данных завершена.';ru='Загрузка данных завершена.'"));
КонецПроцедуры
...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	Если ПустаяСтрока(ИмяКаталога) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не задан каталог обмена информацией.';ru='Не задан каталог обмена информацией.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("ka='Расширение для работы с файлами не установлено, открытие каталога не возможно.';ru='Расширение для работы с файлами не установлено, открытие каталога не возможно.'");
...
Процедура ПослеОпределенияСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Каталог ""%1"" не существует или к нему нет доступа.';ru='Каталог ""%1"" не существует или к нему нет доступа.'"),
...
Процедура ОпределитьЭтоКаталог(ЭтоКаталог, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='""%1"" является файлом, а не каталогом.';ru='""%1"" является файлом, а не каталогом.'"),
			ДополнительныеПараметры.ИмяКаталога);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ОбновитьСтраницыНастроекТранспорта()
	...
	Иначе
		...
		СписокВыбораТранспорта.Добавить(Неопределено, НСтр("ka='подключение не настроено';ru='подключение не настроено'") );
...
Процедура УстановитьОписаниеДополненияОбщегоОтбора()
	...
	Если НетОтбора Тогда
		Текст = НСтр("ka='ყველა დოკუმენტი';ru='Все документы'");
	КонецЕсли;
...
Процедура УстановитьОписаниеДополненияДетально()
	...
	НетОтбора = ПустаяСтрока(Текст);
	Если НетОтбора Тогда
		Текст = НСтр("ka='დამატებითი მონაცემები არ არის არჩეული';ru='Дополнительные данные не выбраны'");
	КонецЕсли;
...
Процедура ОбновитьОтображениеЭлементовСтатусаОбменаДанными()
	...
	Если Элементы.СтраницыСтатусаЗагрузкиДанных.ТекущаяСтраница=Элементы.СтатусЗагрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийЗагрузкиДанных.Заголовок = НСтр("ka='Загрузка данных не произведена';ru='Загрузка данных не произведена'");
	Иначе
	...
	Если Элементы.СтраницыСтатусаВыгрузкиДанных.ТекущаяСтраница=Элементы.СтатусВыгрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийВыгрузкиДанных.Заголовок = НСтр("ka='Выгрузка данных не произведена';ru='Выгрузка данных не произведена'");
	Иначе
...
Функция Подключаемый_СтраницаНачала_ПриПереходеДалее(Отказ)
	...
	Если Объект.УзелИнформационнойБазы.Пустая() Тогда
		...
		НСтрока = НСтр("ka='Укажите узел информационной базы';ru='Укажите узел информационной базы'");
	...
	ИначеЕсли Объект.ВидТранспортаСообщенийОбмена.Пустая() Тогда
		...
		НСтрока = НСтр("ka='Укажите вариант подключения';ru='Укажите вариант подключения'");
	...
	ИначеЕсли ОбменЧерезВебСервис И ПустаяСтрока(WSПароль) Тогда
		...
		НСтрока = НСтр("ka='Не указан пароль.';ru='Не указан пароль.'");
...
Функция Подключаемый_СтраницаОжиданияПроверкиПодключения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если ОбменЧерезВебСервис Тогда
		...
		Если Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='Не удалось выполнить операцию.';ru='Не удалось выполнить операцию.'"));
...
Функция Подключаемый_СтраницаОжиданияАнализаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Получение сообщения обмена во временный каталог';ru='Получение сообщения обмена во временный каталог'");
	...
	Если ДлительнаяОперацияЗавершена Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.ИмяФайлаСообщенияОбмена) Тогда
				...
				ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
				ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Получение файла с данными сообщения обмена во временный каталог';ru='Получение файла с данными сообщения обмена во временный каталог'");
...
Процедура ЗавершениеПолученияДанныхВоВременныйКаталогНаСервере();
	...
	ШаблонСообщенияОбОшибке = НСтр("ka='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';ru='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации'");
...
Функция Подключаемый_АнализДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Анализ данных сообщения обмена';ru='Анализ данных сообщения обмена'");
...
Процедура ЗавершениеАнализаДанныхНаСервере()
	...
	Если ЗафиксироватьОшибку Тогда
		...
		ТекстСообщения = ?(ЗначениеЗаполнено(СообщениеОбОшибке), СообщениеОбОшибке,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить данные из ""%1"" (этап анализа данных).';ru='Не удалось загрузить данные из ""%1"" (этап анализа данных).'"), Объект.УзелИнформационнойБазы));
...
Функция Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее(Отказ)
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,  НСтр("ka='გაგრძელება';ru='Продолжить'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='გაუქმება';ru='Отмена'"));
...
Функция Подключаемый_ЗагрузкаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Загрузка данных из сообщения обмена';ru='Загрузка данных из сообщения обмена'");
...
Процедура РегистрацияДополнительноОтобранныхДанных()
	...
	Исключение
		...
		ДлительнаяОперацияЗавершена = Истина;
		ДлительнаяОперация = Ложь;
		СообщениеОбОшибке = НСтр("ka='Возникла проблема при добавлении данных к выгрузке:';ru='Возникла проблема при добавлении данных к выгрузке:'") 
			+ Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке())
			+ Символы.ПС + НСтр("ka='Необходимо изменить условия отбора.';ru='Необходимо изменить условия отбора.'");
		Возврат;
	КонецПопытки;
...
Функция Подключаемый_СтраницаОжиданияВыгрузкиДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Выгрузка данных из сообщения обмена';ru='Выгрузка данных из сообщения обмена'");
...
Процедура ЗавершениеВыгрузкиНаСервере();
	...
	Если РезультатВыполненияМетода.Отказ
		И Не ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		СообщениеОбОшибке = НСтр("ka='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';ru='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации'");
	КонецЕсли;
...
Процедура ОчиститьОтборПоОрганизациям(Команда)
	...
	ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение'");
	ТекстВопроса   = НСтр("ka='გამოვრთოთ ორგანიზაციების მიხედვით შერჩევა?';ru='Очистить отбор по организациям?'");
...
Процедура ИнициализироватьСценарийПерехода(ОбменЧерезПодключениеККорреспонденту)
	...
	Если ОбменЧерезПодключениеККорреспонденту Тогда
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='Заданный сценарий синхронизации данных не поддерживается.';ru='Заданный сценарий синхронизации данных не поддерживается.'");
...
Функция ПредставлениеОтбора(Период, Отбор)
	...
	ДополнениеВыгрузкиОбъект = РеквизитФормыВЗначение("ДополнениеВыгрузки");
	ОписаниеПустогоОтбора = НСтр("ka='ყველა დოკუმენტი';ru='Все документы'");
