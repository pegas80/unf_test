...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы", КодУзлаИнформационнойБазы) Тогда
		...
		Если Объект.УзелИнформационнойБазы.Пустая() Тогда
			...
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Настройка обмена данными не найдена.';ru='Настройка обмена данными не найдена.';en='Changing Communication can not be found.'"), Отказ);
			Возврат;
	...
	Иначе
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Непосредственное открытие помощника не предусмотрено.';ru='Непосредственное открытие помощника не предусмотрено.';en='The immediate opening of the assistant is not provided.'"), Отказ);
		Возврат;
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(
			НСтр("ka='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';en='For the selected node performing data exchange with the configuration is not provided.'"), Отказ);
		Возврат;
	...
	Если МассивУзлов.Найти(Объект.УзелИнформационнойБазы) = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для выбранного узла сопоставление данных не предусмотрено.';ru='Для выбранного узла сопоставление данных не предусмотрено.';en='For the selected node mapping data is not provided.'"),,,, Отказ);
		Возврат;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИзменитьЗаголовок Тогда
		Элементы.ЗаголовокСтраницы.Заголовок = НСтр("ka='დაკავშირების პარამეტრები';ru='Параметры подключения';en='connection Settings'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНачала Тогда
		ТекстПодтверждения = НСтр("ka='Отменить синхронизацию данных?';ru='Отменить синхронизацию данных?';en='Cancel data synchronization?'");
	Иначе
...
Процедура ВыполнитьЗагрузкуДанныхДляСтроки(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		НСтрока = НСтр("ka='Выберите имя таблицы в поле статистической информации.';ru='Выберите имя таблицы в поле статистической информации.';en='Select the table name in the field of statistical information.'");
	...
	Если ЕстьНесопоставленныеОбъекты Тогда
		НСтрока = НСтр("ka='Имеются несопоставленные объекты."
"При загрузке данных будут созданы дубли несопоставленных объектов. Продолжить?';ru='Имеются несопоставленные объекты."
...
Процедура ОткрытьФормуСопоставления(Команда)
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ka='Для типа данных нельзя выполнить сопоставление объектов.';ru='Для типа данных нельзя выполнить сопоставление объектов.';en='For the data type can not map the objects.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("ka='Восстановить настройки ""%1""?';ru='Восстановить настройки ""%1""?';en='Restore ""%1"" setting?'"), ПредставлениеНастройки);
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(Знач ВыделенныеСтроки) 
	...
	Состояние(НСтр("ka='Выполняется загрузка данных...';ru='Выполняется загрузка данных...';en='Data is loading ...'"));
...
"Перейти в журнал регистрации?';ka='При загрузке данных возникли ошибки."
"Перейти в журнал регистрации?'");
	...
	Состояние(НСтр("ka='Загрузка данных завершена.';ru='Загрузка данных завершена.';en='Loading data is completed.'"));
КонецПроцедуры
...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	Если ПустаяСтрока(ИмяКаталога) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не задан каталог обмена информацией.';ru='Не задан каталог обмена информацией.';en='Not specified catalog of information exchange.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("ka='Расширение для работы с файлами не установлено, открытие каталога не возможно.';ru='Расширение для работы с файлами не установлено, открытие каталога не возможно.';en='Extension for working with files is not installed, catalog the opening is not possible.'");
...
Процедура ПослеОпределенияСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Каталог ""%1"" не существует или к нему нет доступа.';ru='Каталог ""%1"" не существует или к нему нет доступа.';en='The directory ""%1"" does not exist or can not be accessed.'"),
...
Процедура ОпределитьЭтоКаталог(ЭтоКаталог, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='""%1"" является файлом, а не каталогом.';ru='""%1"" является файлом, а не каталогом.';en='""%1"" is a file and not a directory.'"),
			ДополнительныеПараметры.ИмяКаталога);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ОбновитьСтраницыНастроекТранспорта()
	...
	Иначе
		...
		СписокВыбораТранспорта.Добавить(Неопределено, НСтр("ka='подключение не настроено';ru='подключение не настроено';en='the connection is not set'") );
...
Процедура ОбновитьОтображениеЭлементовСтатусаОбменаДанными()
	...
	Если Элементы.СтраницыСтатусаЗагрузкиДанных.ТекущаяСтраница=Элементы.СтатусЗагрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийЗагрузкиДанных.Заголовок = НСтр("ka='Загрузка данных не произведена';ru='Загрузка данных не произведена';en='Loading data is not made'");
	Иначе
	...
	Если Элементы.СтраницыСтатусаВыгрузкиДанных.ТекущаяСтраница=Элементы.СтатусВыгрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийВыгрузкиДанных.Заголовок = НСтр("ka='Выгрузка данных не произведена';ru='Выгрузка данных не произведена';en='Upload data is not made'");
	Иначе
...
Функция Подключаемый_СтраницаНачала_ПриПереходеДалее(Отказ)
	...
	Если Объект.УзелИнформационнойБазы.Пустая() Тогда
		...
		НСтрока = НСтр("ka='Укажите узел информационной базы';ru='Укажите узел информационной базы';en='Specify the information base unit'");
	...
	ИначеЕсли Объект.ВидТранспортаСообщенийОбмена.Пустая() Тогда
		...
		НСтрока = НСтр("ka='Укажите вариант подключения';ru='Укажите вариант подключения';en='Specify the type of connection'");
	...
	ИначеЕсли ОбменЧерезВебСервис И ПустаяСтрока(WSПароль) Тогда
		...
		НСтрока = НСтр("ka='Не указан пароль.';ru='Не указан пароль.';en='Unknown password.'");
...
Функция Подключаемый_СтраницаОжиданияПроверкиПодключения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если ОбменЧерезВебСервис Тогда
		...
		Если Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='Не удалось выполнить операцию.';ru='Не удалось выполнить операцию.';en='Unable to perform the operation.'"));
...
Функция Подключаемый_СтраницаОжиданияАнализаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Получение сообщения обмена во временный каталог';ru='Получение сообщения обмена во временный каталог';en='Receiving exchange messages to a temporary directory'");
	...
	Если ДлительнаяОперацияЗавершена Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.ИмяФайлаСообщенияОбмена) Тогда
				...
				ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
				ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Получение файла с данными сообщения обмена во временный каталог';ru='Получение файла с данными сообщения обмена во временный каталог';en='Getting file data messaging to a temporary directory'");
...
Процедура ЗавершениеПолученияДанныхВоВременныйКаталогНаСервере();
	...
	ШаблонСообщенияОбОшибке = НСтр("ka='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';ru='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';en='Unable to load data. For details, see. In the log'");
...
Функция Подключаемый_АнализДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Анализ данных сообщения обмена';ru='Анализ данных сообщения обмена';en='Analysis of the data exchange messages'");
...
Процедура ЗавершениеАнализаДанныхНаСервере()
	...
	Если ЗафиксироватьОшибку Тогда
		...
		ТекстСообщения = ?(ЗначениеЗаполнено(СообщениеОбОшибке), СообщениеОбОшибке,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить данные из ""%1"" (этап анализа данных).';ru='Не удалось загрузить данные из ""%1"" (этап анализа данных).';en='Unable to load the data from ""%1"" (data analysis phase).'"), Объект.УзелИнформационнойБазы));
...
Функция Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее(Отказ)
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,  НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
...
Функция Подключаемый_ЗагрузкаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Загрузка данных из сообщения обмена';ru='Загрузка данных из сообщения обмена';en='Loading data from the exchange of messages'");
...
Процедура РегистрацияДополнительноОтобранныхДанных()
	...
	Исключение
		...
		ДлительнаяОперацияЗавершена = Истина;
		ДлительнаяОперация = Ложь;
		СообщениеОбОшибке = НСтр("ka='Возникла проблема при добавлении данных к выгрузке:';ru='Возникла проблема при добавлении данных к выгрузке:';en='There is a problem with the addition of data to be unloaded:'") 
			+ Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке())
			+ Символы.ПС + НСтр("ka='Необходимо изменить условия отбора.';ru='Необходимо изменить условия отбора.';en='You must change the selection criteria.'");
		Возврат;
	КонецПопытки;
...
Функция Подключаемый_СтраницаОжиданияВыгрузкиДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Выгрузка данных из сообщения обмена';ru='Выгрузка данных из сообщения обмена';en='Upload data from the exchange of messages'");
...
Процедура ЗавершениеВыгрузкиНаСервере();
	...
	Если РезультатВыполненияМетода.Отказ
		И Не ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		СообщениеОбОшибке = НСтр("ka='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';ru='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';en='Failed to send data. For details, see. In the log'");
	КонецЕсли;
...
Процедура ИнициализироватьСценарийПерехода(ОбменЧерезПодключениеККорреспонденту)
	...
	Если ОбменЧерезПодключениеККорреспонденту Тогда
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='Заданный сценарий синхронизации данных не поддерживается.';ru='Заданный сценарий синхронизации данных не поддерживается.';en='The predetermined data synchronization scenario is not supported.'");
