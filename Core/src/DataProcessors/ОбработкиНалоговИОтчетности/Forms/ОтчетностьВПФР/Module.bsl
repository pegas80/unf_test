...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(КомплектОтчетности) Тогда
		...
		Если Выборка1.Следующий() Тогда
			...
			Если Месяц(ОтчетныйПериод) = 10 И ЭтоОтчетностьРанее2013года Тогда
				...
				Элементы.ДекорацияОписаниеРаботы2.Заголовок = НСтр("ka='Распечатайте РСВ-1, АДВ-6-2, СЗВ-6-2, СЗВ-6-3, АДВ-6-4 в 2-х экземплярах';ru='Распечатайте РСВ-1, АДВ-6-2, СЗВ-6-2, СЗВ-6-3, АДВ-6-4 в 2-х экземплярах';en='Print RSV-1, ADV-6-2, SZV-6-2, SZV-6-3, ADV-6-4 in 2 copies'");
				Элементы.ДекорацияТекстОписание7.Заголовок = НСтр("ka='Сохраните файлы РСВ-1, АДВ-6-2, СЗВ-6-2, СЗВ-6-3, АДВ-6-4 на флешку или дискету (разные налоговые по-разному принимают)';ru='Сохраните файлы РСВ-1, АДВ-6-2, СЗВ-6-2, СЗВ-6-3, АДВ-6-4 на флешку или дискету (разные налоговые по-разному принимают)';en='Save files RSV-1, ADV-6-2, SZV-6-2, SZV-6-3, ADV-6-4 on a flash drive or a floppy disk (different tax different take)'");
			...
			Если Не ЭтоОтчетностьРанее2014года Тогда
				...
				Элементы.ДекорацияОписаниеРаботы2.Заголовок = НСтр("ka='Распечатайте РСВ-1 в 2-х экземплярах';ru='Распечатайте РСВ-1 в 2-х экземплярах';en='Print RSV-1 in 2 copies'");
				Элементы.ДекорацияТекстОписание7.Заголовок = НСтр("ka='Сохраните файлы РСВ-1 на флешку или дискету (разные налоговые по-разному принимают)';ru='Сохраните файлы РСВ-1 на флешку или дискету (разные налоговые по-разному принимают)';en='Save files RSV-1 on a flash drive or a floppy disk (different tax different take)'");
			ИначеЕсли Не ЭтоОтчетностьРанее2013года Тогда
				Элементы.ДекорацияОписаниеРаботы2.Заголовок = НСтр("ka='Распечатайте РСВ-1, АДВ-6-2, СЗВ-6-4, АДВ-6-5 в 2-х экземплярах';ru='Распечатайте РСВ-1, АДВ-6-2, СЗВ-6-4, АДВ-6-5 в 2-х экземплярах';en='Print RSV-1, ADV-6-2, SZV-6-4, ADV-6-5 in 2 copies'");
				Элементы.ДекорацияТекстОписание7.Заголовок = НСтр("ka='Сохраните файлы РСВ-1, АДВ-6-2, СЗВ-6-4, АДВ-6-5 на флешку или дискету (разные налоговые по-разному принимают)';ru='Сохраните файлы РСВ-1, АДВ-6-2, СЗВ-6-4, АДВ-6-5 на флешку или дискету (разные налоговые по-разному принимают)';en='Save files RSV-1, ADV-6-2, SZV-6-4, ADV-6-5 on a flash drive or a floppy disk (different tax different take)'");
			КонецЕсли;
			...
			Если НЕ ЭтоОтчетностьРанее2014года И Не ЗначениеЗаполнено(ДокументРаздел6РасчетаРСВ1) И ИмеютсяСотрудникиВПФР Тогда
				...
				Элементы.ДекорацияПредупреждение.Заголовок = НСтр("ka='Внимание! В программе появились новые формы отчетности в ПФР. Сформируйте данный комплект отчетности заново.';ru='Внимание! В программе появились новые формы отчетности в ПФР. Сформируйте данный комплект отчетности заново.';en='Attention! In the program there are new forms of reporting to the FIU. Form the statements set anew.'");
	...
	Если РежимИспользованияИБ = Перечисления.РежимыИспользованияИнформационнойБазы.Демонстрационный Тогда
		Элементы.ОтправитьВКонтролирующийОрган.Заголовок = НСтр("ka='Отправить (в демо не доступно)';ru='Отправить (в демо не доступно)';en='Send (in the demo is not available)'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПрикрепленаДоверенность" И Параметр = Объект.Организация Тогда
		Элементы.ОтправитьВКонтролирующийОрган.Заголовок = НСтр("ka='Идет проверка доверенности';ru='Идет проверка доверенности';en='It is a power of attorney verification'");
	ИначеЕсли ИмяСобытия = "ИзмененаЗаявкаЭДО" Тогда
		Если Параметр.Организация = Объект.Организация Тогда
			Если Параметр.ЕстьАктуальнаяЗаявка Тогда
				Элементы.ОтправитьВКонтролирующийОрган.Заголовок = НСтр("ka='გაგზავნა';ru='Отправить';en='to send'");
				Элементы.ОтправитьВКонтролирующийОрган.Доступность = Истина;
				Элементы.ОформитьЗаявку.Видимость = Ложь;
			Иначе
				Элементы.ОтправитьВКонтролирующийОрган.Доступность = Ложь;
				Элементы.ОтправитьВКонтролирующийОрган.Заголовок = НСтр("ka='Отправить (нет заявки)';ru='Отправить (нет заявки)';en='Send (no application)'");
...
Процедура ОбновитьСтатус(Команда)
	...
	ИначеЕсли ВыбранныеОрганизации.Количество() = 1 Тогда
		ТекстВопроса = НСтр("ka='Информация будет запрошена по всем отчетам, ожидающим подтверждения';ru='Информация будет запрошена по всем отчетам, ожидающим подтверждения';en='Information will be requested for all reports, confirmation expected'");
	Иначе
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ПФР приняла отчетность?';ru='ПФР приняла отчетность?';en='FIU took statements?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что отчет заполнен полностью и корректно?';ru='Вы подтверждаете, что отчет заполнен полностью и корректно?';en='You acknowledge that the report is completely filled and correctly?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПриОтветеНаВопросОЗаполнении(РезультатВопроса, ДопПараметры) Экспорт
	...
	Если Отказ Тогда
		ПоказатьПредупреждение(,НСтр("ka='Отправка прервана! В отчете РСВ-1 найдены ошибки. Проверьте корректность заполнения отчета и повторите снова.';ru='Отправка прервана! В отчете РСВ-1 найдены ошибки. Проверьте корректность заполнения отчета и повторите снова.';en='Sending aborted! The report found the RSV-1 mistake. Check the correctness of filling of the report and try again.'"));
...
Процедура ЗаписьНаДискПослеПодключенияРасширенияРаботыСФайлами(Подключено, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОтчетности) Тогда
		...
		Если Ошибки.Количество() > 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='Сохранение прервано! В отчете РСВ-1 найдены ошибки. Проверьте корректность заполнения отчета и повторите снова.';ru='Сохранение прервано! В отчете РСВ-1 найдены ошибки. Проверьте корректность заполнения отчета и повторите снова.';en='Saving interrupted! The report found the RSV-1 mistake. Check the correctness of filling of the report and try again.'"));
