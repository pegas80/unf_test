...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПериодЗадачиПредставление = ПредставлениеПериода(
		...
		"ФП=Истина");
	ПериодЗадачиПредставление = Лев(ПериодЗадачиПредставление, 1)+ Нстр("ka='-й';ru='-й'") + Прав(ПериодЗадачиПредставление, СтрДлина(ПериодЗадачиПредставление)-1);
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Иначе
		...
		ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ka='Когда был уплачен ЕНВД?';ru='Когда был уплачен ЕНВД?'"));
		Возврат;
		...
		ИначеЕсли Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаЕНВД Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ЕНВД.Заполнение задачи';ru='ЕНВД.Заполнение задачи'"),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					НСтр("ka='Не удалось получить данные для ТаблицаЕНВД';ru='Не удалось получить данные для ТаблицаЕНВД'"));
			Иначе
		...
		ИначеЕсли Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаЕНВДПоОкато Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					...
					,
					,
					НСтр("ka='Не удалось получить данные для ТаблицаЕНВДПоОкато';ru='Не удалось получить данные для ТаблицаЕНВДПоОкато'"));
			Иначе
...
Процедура СверстатьФорму()
	...
	Для м = 0 по ТаблицаЕНВДПоОкато.Количество()-1 Цикл
		...
		реквизит = Новый РеквизитФормы(
			...
			Новый ОписаниеТипов("Строка"),
			,
			НСтр("ka='გადასახადის ოდენობა';ru='Сумма налога'"));
		...
		реквизит = Новый РеквизитФормы(
			...
			Новый ОписаниеТипов("Строка"),
			,
			НСтр("ka='სადაზღვევო შენატანები';ru='Страховые взносы'"));
