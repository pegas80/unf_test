...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Год(ДатаДокументаОбработкиСобытия) > 2013 Тогда
		Элементы.ДекорацияПФРСтраховаяСотрудники.Заголовок = НСтр("ka='ПФР';ru='ПФР';en='FIU'");
...
Процедура ДекорацияВсеПлатежкиНажатие(Элемент)
	Если ТолькоНулевыеСуммы() Тогда
		ПоказатьПредупреждение(,НСтр("ka='Суммы всех взносов и налогов нулевые. Уплата не требуется';ru='Суммы всех взносов и налогов нулевые. Уплата не требуется';en='The sum of all contributions and zero taxes. Payment is not required'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Список.Добавить(ДанныеНДФЛСотрудники,НСтр("ka='საშემოსავლო';ru='НДФЛ';en='НДФЛ'"));
	...
	Если Год(ДатаДокументаОбработкиСобытия) < 2014 Тогда
		...
		Список.Добавить(ДанныеПФРСтраховаяСотрудники,НСтр("ka='ПФР страховая';ru='ПФР страховая';en='FIU insurance'"));
		Список.Добавить(ДанныеПФРНакопительнаяСотрудники,НСтр("ka='ПФР накопительная';ru='ПФР накопительная';en='FIU funded'"));
	Иначе
		Список.Добавить(ДанныеПФРСтраховаяСотрудники,НСтр("ka='ПФР';ru='ПФР';en='FIU'"));
	КонецЕсли;
	...
	Список.Добавить(ДанныеФОМССотрудники,НСтр("ka='ФОМС';ru='ФОМС';en='HIF'"));
	...
	Список.Добавить(ДанныеФСССотрудники,НСтр("ka='ФСС';ru='ФСС';en='FSS'"));
	...
	Список.Добавить(ДанныеФССТравматизмСотрудники,НСтр("ka='ФСС травматизм';ru='ФСС травматизм';en='FSS injuries'"));
...
Функция ПроверитьНаличиеГосОргана(ИмяПроцедурыПоЗавершениюСозданияГосОргана)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ka='В справочнике «Контрагенты» не задан государственный орган для уплаты налога (взноса)."
"Создать его автоматически?';ru='В справочнике «Контрагенты» не задан государственный орган для уплаты налога (взноса)."
