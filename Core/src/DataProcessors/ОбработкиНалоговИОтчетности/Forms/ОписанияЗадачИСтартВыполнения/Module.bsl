...
Процедура ПеренестиВАрхив(Команда)
	...
	ТекстВопроса = Нстр("ka='Задача будет перенесена в архив и удалена из списка актуальных событий. Перенести в архив?';ru='Задача будет перенесена в архив и удалена из списка актуальных событий. Перенести в архив?'");
...
Процедура ОбработатьПараметрСобытияКалендаря()
	...
	Если СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалог Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать Единый налог »';ru='Рассчитать Единый налог »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать Авансовый платеж »';ru='Рассчитать Авансовый платеж »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоУСН Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать Декларацию »';ru='Сформировать Декларацию »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыИП Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать Страховые взносы »';ru='Рассчитать Страховые взносы »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоЕНВД Тогда
		...
		Элементы.ДекорацияТекстОписание20.Заголовок = 
			Новый ФорматированнаяСтрока(Новый ФорматированнаяСтрока(НСтр("ka='Заполните';ru='Заполните'"),,,,"ЕНВД"),
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалогЕНВД Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать и уплатить ЕНВД »';ru='Рассчитать и уплатить ЕНВД »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СреднесписочнаяЧисленность Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать и отправить Сведения о численности »';ru='Сформировать и отправить Сведения о численности »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Справки2НДФЛ Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать справки 2-НДФЛ по сотрудникам »';ru='Сформировать справки 2-НДФЛ по сотрудникам »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ОтчетностьВПФР Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать пакет отчетности в ПФР »';ru='Сформировать пакет отчетности в ПФР »'");
	// Налоги сотрудников
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать налоги сотрудников »';ru='Рассчитать налоги сотрудников »'");
	// Форма 4-ФСС
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма4ФСС Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать отчет 4-ФСС »';ru='Сформировать отчет 4-ФСС »'");
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыПриДоходахСвыше300тр Тогда
		...
		Заголовок = НСтр("ka='Страховые взносы индивидуального предпринимателя в ПФР при доходах свыше 300 000 рублей';ru='Страховые взносы индивидуального предпринимателя в ПФР при доходах свыше 300 000 рублей'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогПатент Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Заплатить налог с патента »';ru='Заплатить налог с патента »'");
	// Торговый сбор
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ТорговыйСбор Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Уплатить торговый сбор »';ru='Уплатить торговый сбор »'");
		
	// №1-Предприниматель
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма1Предприниматель Тогда
		
		Заголовок = НСтр("ka='ფორმა №1-მეწარმე';ru='Форма №1-предприниматель'");
		
		Элементы.ГруппаДекорацииОписанияФорма1Предприниматель.Видимость = Истина;
		
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать форму »';ru='Сформировать форму »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма6НДФЛ Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать форму 6-НДФЛ »';ru='Сформировать форму 6-НДФЛ »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СведенияОЗастрахованныхЛицах Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать СЗВ-М »';ru='Сформировать СЗВ-М »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.РасчетПоСтраховымВзносам Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать отчет по страховым взносам »';ru='Сформировать отчет по страховым взносам »'");
	КонецЕсли;
...
Функция ВыполнитьКонтроль()
	...
	Если РезультатПроверки.ЕстьОшибки Тогда
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Есть ошибки. Повторить проверку »';ru='Есть ошибки. Повторить проверку »'");
	ИначеЕсли РезультатПроверки.ЕстьПредупреждения Тогда
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Игнорировать предупреждения »';ru='Игнорировать предупреждения »'");
...
Процедура ДекорацияТекстОписаниеТорговыйСборОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымТорговогоСбора(НСтр("ka='« Вернуться к уплате торогового сбора';ru='« Вернуться к уплате торогового сбора'"));
КонецПроцедуры
...
Процедура ДекорацияТекстОписание20ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымЕНВД(НСтр("ka='Декларации по ЕНВД';ru='Декларации по ЕНВД'"));
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияТекстОписаниеЕНВДНалог2ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымЕНВД(НСтр("ka='Уплаты ЕНВД';ru='Уплаты ЕНВД'"));
КонецПроцедуры
