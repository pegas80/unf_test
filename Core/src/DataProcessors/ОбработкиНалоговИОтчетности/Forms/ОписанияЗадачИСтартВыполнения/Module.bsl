...
Процедура ПеренестиВАрхив(Команда)
	...
	ТекстВопроса = Нстр("ka='Задача будет перенесена в архив и удалена из списка актуальных событий. Перенести в архив?';ru='Задача будет перенесена в архив и удалена из списка актуальных событий. Перенести в архив?';en='The task will be moved to the archive and deleted from the list of current events. Move to archive?'");
...
Процедура ОбработатьПараметрСобытияКалендаря()
	...
	Если СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалог Тогда
		...
		Заголовок = НСтр("ru='Единый налог за ';ka='Единый налог за '") 
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать Единый налог »';ru='Рассчитать Единый налог »';en='Calculate single tax ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН Тогда
		...
		Заголовок = НСтр("ru='Авансовый платеж по УСН за ';ka='Авансовый платеж по УСН за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать Авансовый платеж »';ru='Рассчитать Авансовый платеж »';en='Calculate Down Payment ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоУСН Тогда
		...
		Заголовок = НСтр("ru='Декларация по УСН за ';ka='Декларация по УСН за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать Декларацию »';ru='Сформировать Декларацию »';en='Declaration form ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыИП Тогда
		...
		Заголовок = НСтр("ru='Страховые взносы индивидуального предпринимателя за ';ka='Страховые взносы индивидуального предпринимателя за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать Страховые взносы »';ru='Рассчитать Страховые взносы »';en='Calculate Premiums ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоЕНВД Тогда
		...
		Заголовок = НСтр("ru='Декларация по ЕНВД ';ka='Декларация по ЕНВД '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ДекорацияТекстОписание20.Заголовок = 
			Новый ФорматированнаяСтрока(Новый ФорматированнаяСтрока(НСтр("ka='Заполните';ru='Заполните';en='fill'"),,,,"ЕНВД"),
			НСтр("ru=' сведения по ЕНВД за ';ka=' сведения по ЕНВД за '"),
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалогЕНВД Тогда
		...
		Заголовок = НСтр("ru='Уплата ЕНВД за ';ka='Уплата ЕНВД за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать и уплатить ЕНВД »';ru='Рассчитать и уплатить ЕНВД »';en='Calculate and pay UTII ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СреднесписочнаяЧисленность Тогда
		...
		Заголовок = НСтр("ru='Сведения о среднесписочной численности за ';ka='Сведения о среднесписочной численности за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать и отправить Сведения о численности »';ru='Сформировать и отправить Сведения о численности »';en='Generate and send information about the number of ""'");
		
	// Справки 2-НДФЛ
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Справки2НДФЛ Тогда
		
		Заголовок = НСтр("ru='Справки 2-НДФЛ по сотрудникам за ';ka='Справки 2-НДФЛ по сотрудникам за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать справки 2-НДФЛ по сотрудникам »';ru='Сформировать справки 2-НДФЛ по сотрудникам »';en='To form a reference 2-PIT for employees ""'");
		
	// Отчетность в ПФР
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ОтчетностьВПФР Тогда
		
		Заголовок = НСтр("ru='Отчетность в ПФР за ';ka='Отчетность в ПФР за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать пакет отчетности в ПФР »';ru='Сформировать пакет отчетности в ПФР »';en='Form a reporting package to the FIU ""'");
	// Налоги сотрудников
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников Тогда
		
		Заголовок = НСтр("ru='Налоги сотрудников за ';ka='Налоги сотрудников за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Рассчитать налоги сотрудников »';ru='Рассчитать налоги сотрудников »';en='Calculate tax employees ""'");
	// Форма 4-ФСС
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма4ФСС Тогда
		
		Заголовок = НСтр("ru='Отчет 4-ФСС ';ka='Отчет 4-ФСС '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать отчет 4-ФСС »';ru='Сформировать отчет 4-ФСС »';en='Generate Report 4-FSS ""'");
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыПриДоходахСвыше300тр Тогда
		...
		Заголовок = НСтр("ka='Страховые взносы индивидуального предпринимателя в ПФР при доходах свыше 300 000 рублей';ru='Страховые взносы индивидуального предпринимателя в ПФР при доходах свыше 300 000 рублей';en='Insurance premiums to the Pension Fund of the individual entrepreneur, on revenue of more than 300 000 rubles'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогПатент Тогда
		...
		Если НачалоДня(ДатаПервогоПлатежа) = НачалоДня(ПериодДокументовОкончание) Тогда
			Если Не ЗначениеЗаполнено(ДатаВторогоПлатежа) Тогда
				Заголовок = НСтр("ru='Налог с патента ';ka='Налог с патента '");
			Иначе
				Заголовок = НСтр("ru='1/3 налога с патента ';ka='1/3 налога с патента '");
			КонецЕсли;
		ИначеЕсли НачалоДня(ДатаВторогоПлатежа) = НачалоДня(ПериодДокументовОкончание) Тогда
			Заголовок = НСтр("ru='2/3 налога с патента ';ka='2/3 налога с патента '");
		Иначе
			Заголовок = НСтр("ru='Налог с патента ';ka='Налог с патента '");
		КонецЕсли;
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Заплатить налог с патента »';ru='Заплатить налог с патента »';en='To pay tax on a patent ""'");
	// Торговый сбор
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ТорговыйСбор Тогда
		
		Заголовок = НСтр("ru='Торговый сбор за ';ka='Торговый сбор за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Уплатить торговый сбор »';ru='Уплатить торговый сбор »';en='To pay the merchant fees ""'");
		
	// №1-Предприниматель
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма1Предприниматель Тогда
		
		Заголовок = НСтр("ka='ფორმა №1-მეწარმე';ru='Форма №1-предприниматель';en='Form №1-employed'");
		
		Элементы.ГруппаДекорацииОписанияФорма1Предприниматель.Видимость = Истина;
		
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать форму »';ru='Сформировать форму »';en='Form the shape ""'");
		
	// 6-НДФЛ
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма6НДФЛ Тогда
		
		Заголовок = НСтр("ru='Форма 6-НДФЛ по сотрудникам за ';ka='Форма 6-НДФЛ по сотрудникам за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать форму 6-НДФЛ »';ru='Сформировать форму 6-НДФЛ »';en='Form the shape of 6-PIT ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СведенияОЗастрахованныхЛицах Тогда
		...
		Заголовок = НСтр("ru='Сведения о застрахованных лицах (СЗВ-М) по сотрудникам за ';ka='Сведения о застрахованных лицах (СЗВ-М) по сотрудникам за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать СЗВ-М »';ru='Сформировать СЗВ-М »';en='Form a BCR-M ""'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация12 Тогда
		...
		Заголовок = НСтр("ru='Декларация об объеме розничной продажи пива и пивных напитков (Приложение №12) за ';ka='Декларация об объеме розничной продажи пива и пивных напитков (Приложение №12) за '") 
			+ ПредставлениеПериода(
				ПериодДокументовНачало,
				ПериодДокументовОкончание,
				"ФП=Истина");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.РасчетПоСтраховымВзносам Тогда
		...
		Заголовок = НСтр("ru='Расчет по страховым взносам за ';ka='Расчет по страховым взносам за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Сформировать отчет по страховым взносам »';ru='Сформировать отчет по страховым взносам »';en='Generate a report on insurance premiums ""'");
	КонецЕсли;
...
Функция ВыполнитьКонтроль()
	...
	Если РезультатПроверки.ЕстьОшибки Тогда
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Есть ошибки. Повторить проверку »';ru='Есть ошибки. Повторить проверку »';en='There are mistakes. Repeat the test ""'");
	ИначеЕсли РезультатПроверки.ЕстьПредупреждения Тогда
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ka='Игнорировать предупреждения »';ru='Игнорировать предупреждения »';en='Ignore the warning ""'");
...
Процедура ДекорацияТекстОписаниеТорговыйСборОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымТорговогоСбора(НСтр("ka='« Вернуться к уплате торогового сбора';ru='« Вернуться к уплате торогового сбора';en='«Back to payment collection torogovogo'"));
КонецПроцедуры
...
Процедура ДекорацияТекстОписание20ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымЕНВД(НСтр("ka='Декларации по ЕНВД';ru='Декларации по ЕНВД';en='Declaration on UTII'"));
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияТекстОписаниеЕНВДНалог2ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымЕНВД(НСтр("ka='Уплаты ЕНВД';ru='Уплаты ЕНВД';en='payment UTII'"));
КонецПроцедуры
