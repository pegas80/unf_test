...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ПФР принял СЗВ-М?';ru='ПФР принял СЗВ-М?';en='The FIU received the SCB-M?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что СЗВ-М заполнены полностью и корректно?';ru='Вы подтверждаете, что СЗВ-М заполнены полностью и корректно?';en='You acknowledge that the SCB-M filled in completely and correctly?'"), РежимДиалогаВопрос.ДаНет);
