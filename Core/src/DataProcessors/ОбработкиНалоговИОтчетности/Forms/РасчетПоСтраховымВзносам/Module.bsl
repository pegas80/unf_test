...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ИФНС приняла Расчет по страховым взносам?';ru='ИФНС приняла Расчет по страховым взносам?';en='The IRS took calculation on insurance premiums?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что Расчет по страховым взносам заполнен полностью и корректно?';ru='Вы подтверждаете, что Расчет по страховым взносам заполнен полностью и корректно?';en='You acknowledge that the calculation of insurance premiums is filled completely and correctly?'"), РежимДиалогаВопрос.ДаНет);
