...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='Росстат принял форму №1-предприниматель?';ru='Росстат принял форму №1-предприниматель?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что отчет по форме №1-предприниматель заполнен полностью и корректно?';ru='Вы подтверждаете, что отчет по форме №1-предприниматель заполнен полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
