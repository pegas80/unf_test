...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Иначе
			Сообщить(НСтр("ka='Не выбрана текущая организация';ru='Не выбрана текущая организация';en='The current organization is not selected'"));
			Отказ = Истина;
			Возврат;
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Если ТаблицаТорговыйСбор.Количество() > 1 Тогда
		...
		Для Каждого Строка Из ТаблицаТорговыйСбор Цикл
			Список.Добавить(Строка.ДокументВзаиморасчетовСБюджетом,НСтр("ru='ОКАТО: ';ka='ОКАТО: '")+Строка.ОКАТО);
		КонецЦикла;
	...
	Иначе
		...
		ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ka='Когда был уплачен торговый сбор?';ru='Когда был уплачен торговый сбор?';en='When the collection was paid shopping?'"));
		Возврат;
...
Процедура ОповещениеВыбораДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Расходы зарегестрированы';ru='Расходы зарегестрированы';en='expenses are registered'"),
		ПолучитьНавигационнуюСсылку(ТаблицаТорговыйСбор[0].НаличнаяУплата),
		НСтр("ka='Документ регистрации расходов на налоги';ru='Документ регистрации расходов на налоги';en='Document registration costs for taxes'"),
...
Процедура РаспечататьПлатежноеПоручение(Команда)
	...
	Если ВсегоНалогаКУплате = "0,00 р." Тогда
		ПоказатьПредупреждение(,НСтр("ka='Сумма нулевая. Уплата не требуется.';ru='Сумма нулевая. Уплата не требуется.';en='zero sum. Payment is not required.'"));
		Возврат;
	КонецЕсли;
...
Процедура РаспечататьКвитанциюЗавершение() Экспорт
	...
	Если СтруктураПечати.АдресФайла = Неопределено Тогда
		...
		Сообщить(Нстр("ru='Произошла ошибка при выгрузке документа ';ka='Произошла ошибка при выгрузке документа '")+Объект);
		Возврат;
		...
		Если Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаТорговыхТочек Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ТорговыйСбор.Заполнение задачи';ru='ТорговыйСбор.Заполнение задачи';en='TorgovyySbor.Zapolnenie task'"),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					НСтр("ka='Не удалось получить данные для ТаблицаТорговыйСбор';ru='Не удалось получить данные для ТаблицаТорговыйСбор';en='Unable to get data for TablitsaTorgovyySbor'"));
			Иначе
				...
				Для Каждого стр из Табличка Цикл
					...
					Если СсылкиИФНС.Получить(Стр.КодИФНС) = Неопределено Тогда
						Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() Тогда
							ЗаписьЖурналаРегистрации(НСтр("ka='Не загружены классификаторы налоговых органов';ru='Не загружены классификаторы налоговых органов';en='Not loaded classifiers tax authorities'"),УровеньЖурналаРегистрации.Ошибка,Метаданные.Справочники.НалоговыеОрганы, , НСтр("ka='Необходимо выполнить загрузку налоговых органов';ru='Необходимо выполнить загрузку налоговых органов';en='To download from the tax authorities'"));
						Иначе
							Сообщить(НСтр("ka='Не загружены классификаторы налоговых органов';ru='Не загружены классификаторы налоговых органов';en='Not loaded classifiers tax authorities'"));
						КонецЕсли;
					Иначе
...
Функция ПолучитьИмяЭлементаКнопкиПоУмолчанию(ИмяСтраницы)
	...
	ИначеЕсли ИмяСтраницы = "ЗадачаВыполнена" Тогда
		Возврат "";
	Иначе
		ВызватьИсключение НСтр("ka='Неизвестное имя текущей страницы';ru='Неизвестное имя текущей страницы';en='Unknown name of the current page'");
	КонецЕсли;
...
Процедура ОбновитьДанныеРегистрацииРасходов()
	Если ТаблицаТорговыйСбор.Количество() > 1 Тогда
		Элементы.ОтразитьРасходыВПрограмме.Заголовок = НСтр("ka='Зарегистрировать расходы в программе';ru='Зарегистрировать расходы в программе';en='Sign-up costs in the program'");
	Иначе
...
Функция ВыгрузитьДокументВФайл(МассивОбъектов, ВидФайла)
	...
	ПризнакОшибки = Ложь;
	РасширениеФайла = ".pdf" ;
	НаименованиеФайла = Строка(?(МассивОбъектов.Количество() > 0,МассивОбъектов[0], НСтр("ka='შედეგი';ru='Результат';en='Result'")));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Ошибка сохранения файла на сервере';ru='Ошибка сохранения файла на сервере';en='Error saving the file on the server'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При сохранении файла на сервере в области данных %1 произошла ошибка:"
"%2';ka='При сохранении файла на сервере в области данных %1 произошла ошибка:"
"%2'"),
...
Функция ПроверитьНаличиеГосОргана(ИмяПроцедурыПоЗавершениюСозданияГосОргана)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ka='В справочнике «Контрагенты» не задан налоговый орган для уплаты налога."
"Создать его автоматически?';ru='В справочнике «Контрагенты» не задан налоговый орган для уплаты налога."
"Создать его автоматически?'");
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ავტომატურად შექმნა';ru='Создать автоматически';en='Create automatically'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='შექმნა მექანიკურად';ru='Создать вручную';en='Create manually'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
		
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,КодВозвратаДиалога.Да, НСтр("ka='Отсутствует налоговый орган';ru='Отсутствует налоговый орган';en='No tax authority'"));
...
Процедура ВыполнитьЗаполнениеСведенийОНалоговойИнспекции(ДопПараметры)
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ka='Для автоматического создания налогового органа"
"в справочнике «Контрагенты» необходимо подключиться к Интернет-поддержке"
"пользователей. Подключиться сейчас?';ru='Для автоматического создания налогового органа"
"в справочнике «Контрагенты» необходимо подключиться к Интернет-поддержке"
