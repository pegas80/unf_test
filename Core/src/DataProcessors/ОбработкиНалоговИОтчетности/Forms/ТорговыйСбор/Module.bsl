...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Иначе
			Сообщить(НСтр("ka='Не выбрана текущая организация';ru='Не выбрана текущая организация'"));
			Отказ = Истина;
			Возврат;
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Иначе
		...
		ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ka='Когда был уплачен торговый сбор?';ru='Когда был уплачен торговый сбор?'"));
		Возврат;
...
Процедура ОповещениеВыбораДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Расходы зарегестрированы';ru='Расходы зарегестрированы'"),
		...
		Если Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаТорговыхТочек Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='ТорговыйСбор.Заполнение задачи';ru='ТорговыйСбор.Заполнение задачи'"),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					НСтр("ka='Не удалось получить данные для ТаблицаТорговыйСбор';ru='Не удалось получить данные для ТаблицаТорговыйСбор'"));
			Иначе
				...
				Для Каждого стр из Табличка Цикл
					...
					Если СсылкиИФНС.Получить(Стр.КодИФНС) = Неопределено Тогда
						Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() Тогда
							ЗаписьЖурналаРегистрации(НСтр("ka='Не загружены классификаторы налоговых органов';ru='Не загружены классификаторы налоговых органов'"),УровеньЖурналаРегистрации.Ошибка,Метаданные.Справочники.НалоговыеОрганы, , НСтр("ka='Необходимо выполнить загрузку налоговых органов';ru='Необходимо выполнить загрузку налоговых органов'"));
						Иначе
							Сообщить(НСтр("ka='Не загружены классификаторы налоговых органов';ru='Не загружены классификаторы налоговых органов'"));
						КонецЕсли;
					Иначе
...
Процедура ОбновитьДанныеРегистрацииРасходов()
	Если ТаблицаТорговыйСбор.Количество() > 1 Тогда
		Элементы.ОтразитьРасходыВПрограмме.Заголовок = НСтр("ka='Зарегистрировать расходы в программе';ru='Зарегистрировать расходы в программе'");
	Иначе
