...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ФСРАР приняла Декларацию №12?';ru='ФСРАР приняла Декларацию №12?';en='FSRAR adopted the Declaration №12?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что Декларация №12 заполнена полностью и корректно?';ru='Вы подтверждаете, что Декларация №12 заполнена полностью и корректно?';en='You acknowledge that the Declaration №12 completely filled and correctly?'"), РежимДиалогаВопрос.ДаНет);
