...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ИФНС приняла декларацию?';ru='ИФНС приняла декларацию?';en='The IRS has adopted a declaration?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что Декларация заполнена полностью и корректно?';ru='Вы подтверждаете, что Декларация заполнена полностью и корректно?';en='You acknowledge that the Declaration is filled completely and correctly?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ДекорацияОписаниеРазеделовНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("Заголовок",НСтр("ka='მშეგ-ის დეკლარაცია';ru='Декларация по ЕНВД';en='Declaration on UTII'"));
