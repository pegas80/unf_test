...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ИФНС приняла справки 2-НДФЛ?';ru='ИФНС приняла справки 2-НДФЛ?';en='The IRS took reference 2-PIT?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что Справки 2-НДФЛ заполнены полностью и корректно?';ru='Вы подтверждаете, что Справки 2-НДФЛ заполнены полностью и корректно?';en='You acknowledge that the Help 2 PIT filled completely and correctly?'"), РежимДиалогаВопрос.ДаНет);
