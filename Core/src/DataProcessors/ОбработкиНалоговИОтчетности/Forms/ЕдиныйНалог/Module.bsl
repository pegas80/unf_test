...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ka='Когда был уплачен налог?';ru='Когда был уплачен налог?';en='When was the tax paid?'"), ЧастиДаты.Дата);
...
Процедура ОповещениеВыборДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Расходы зарегистрированы';ru='Расходы зарегистрированы';en='expenses account'"),
...
Процедура СуммаРасходовЗаГодЕННажатие(Элемент, СтандартнаяОбработка)
	...
	Если СтавкаНалогаЕН = "1%" Тогда
		...
		СпсКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Показать расходы';ru='Показать расходы';en='Show costs'"));
		СпсКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
...
"Доходы * 1%.';ka='При расчете налога по ставке в 1% расходы не учитываются."
"Ставка в 1% используется, когда налог, исчисленный по формуле"
"(Доходы – Расходы) * 15%,"
"меньше, чем налог, исчисленный по формуле"
"Доходы * 1%.'"),
	...
	ИначеЕсли ОбъектНалогообложения = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогообложения.Доходы") Тогда
		...
		ПоказатьВопрос(
			оп,
			НСтр("ka='Вы используете объект налоогообложения «Доходы»."
"В этом случае расходы не учитываются при расчете налога.';ru='Вы используете объект налоогообложения «Доходы»."
...
"чем налог, исчисленный по формуле Доходы * 1%.';ka='Ставка в 1% используется, когда налог исчисленный по формуле (Доходы – Расходы) * 15% меньше,"
...
Процедура СуммаСтраховыхВзносовЕННажатие(Элемент, СтандартнаяОбработка)
	...
	Если ОбъектНалогообложения = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогообложения.ДоходыМинусРасходы") Тогда
		...
		СпсКнопок = Новый СписокЗначений;
		СпсКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Показать страховые взносы';ru='Показать страховые взносы';en='Show premiums'"));
		СпсКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
		оп = Новый ОписаниеОповещения("ОповещениеФормаСтраховыхВзносов", ЭтотОбъект);
		ПоказатьВопрос(
			оп,
			НСтр("ka='Вы используете объект налоогообложения «Доходы, уменьшенные на величину расходов»."
"В этом случае страховые взносы включены в состав расходов.';ru='Вы используете объект налоогообложения «Доходы, уменьшенные на величину расходов»."
...
"В этом случае сумма страховых взносов вычитается не полностью.';ka='Сумма страховых взносов превысила сумму налога."
...
"В этом случае сумма налога уменьшается на 50%.';ka='Сумма страховых взносов превысила 50% от суммы налога."
...
"Вы уверены, что все выполнено?';ka='Платежки не создавались. Расходы через кассу не зарегистрированы."
