...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ka='Когда был уплачен налог?';ru='Когда был уплачен налог?'"), ЧастиДаты.Дата);
...
Процедура ОповещениеВыборДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Расходы зарегистрированы';ru='Расходы зарегистрированы'"),
		ПолучитьНавигационнуюСсылку(Ссылка),
		НСтр("ka='Документ регистрации расходов на налоги';ru='Документ регистрации расходов на налоги'"),
...
Процедура СуммаРасходовЗаГодЕННажатие(Элемент, СтандартнаяОбработка)
	...
	Если СтавкаНалогаЕН = "1%" Тогда
		...
		СпсКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Показать расходы';ru='Показать расходы'"));
		СпсКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='დახურვა';ru='Закрыть'"));
...
Процедура СуммаСтраховыхВзносовЕННажатие(Элемент, СтандартнаяОбработка)
	...
	Если ОбъектНалогообложения = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогообложения.ДоходыМинусРасходы") Тогда
		...
		СпсКнопок = Новый СписокЗначений;
		СпсКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Показать страховые взносы';ru='Показать страховые взносы'"));
		СпсКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='დახურვა';ru='Закрыть'"));
...
Процедура РаспечататьПлатежноеПоручение(Команда)
	...
	Если ВсегоКУплатеЕН = "0,00 р." Тогда
		ПоказатьПредупреждение(,НСтр("ka='Сумма нулевая. Уплата не требуется.';ru='Сумма нулевая. Уплата не требуется.'"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьИмяЭлементаКнопкиПоУмолчанию(ИмяСтраницы)
	...
	ИначеЕсли ИмяСтраницы = "ЗадачаВыполнена" Тогда
		Возврат "";
	Иначе
		ВызватьИсключение НСтр("ka='Неизвестное имя текущей страницы';ru='Неизвестное имя текущей страницы'");
	КонецЕсли;
...
"Создать его автоматически?'");
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ავტომატურად შექმნა';ru='Создать автоматически'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='შექმნა მექანიკურად';ru='Создать вручную'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена'"));
		
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,КодВозвратаДиалога.Да, НСтр("ka='Отсутствует налоговый орган';ru='Отсутствует налоговый орган'"));
...
Функция ВыгрузитьДокументВФайл(Объект, ВидФайла)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Ошибка сохранения файла на сервере';ru='Ошибка сохранения файла на сервере'"),
