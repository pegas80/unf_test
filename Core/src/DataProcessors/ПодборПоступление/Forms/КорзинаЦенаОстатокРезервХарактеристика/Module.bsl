...
Процедура ДобавитьНоменклатуруВКорзину()
	...
	//++Lazare 06.09.2022
	ДанныеСтрокиКорзины.Вставить("ДатаОбработки", Объект.Дата);
	ДанныеСтрокиКорзины.Вставить("Коэффициент", 1);
	ДанныеСтрокиКорзины.Вставить("ВалютаДокумента", Объект.ВалютаДокумента);
	ДанныеСтрокиКорзины.Вставить("ВидЦен", Объект.ВидЦен);
	Цена = УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(ДанныеСтрокиКорзины);
	Если ДанныеСтрокиКорзины.Цена  = 0 и  Цена >0 Тогда 
		 ДанныеСтрокиКорзины.Цена = Цена;
	КонецЕсли;
...
Процедура ВыводитьСоветУслугиВПриходныхДокументах()
	...
	Если КешНастройкиПодбора.ВыводитьСоветУслугиВПриходныхДокументах Тогда
		...
		ЗаголовокФормы = Нстр("ka='Совет';ru='Совет';en='Council'");
		ТекстСообщения = НСтр("ka='Услуги в приходных документах отражают расходы компании, поэтому выбор характеристик ограничен.';ru='Услуги в приходных документах отражают расходы компании, поэтому выбор характеристик ограничен.';en=""Services in the documents of receipts reflect the company's expenses, so the choice of characteristics is limited.""");
