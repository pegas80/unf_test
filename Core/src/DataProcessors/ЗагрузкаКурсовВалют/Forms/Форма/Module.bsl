...
Процедура ПроверитьСписокЗагружаемыхВалют()
	Если Объект.СписокВалют.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ka='В справочнике валют отсутствуют валюты, курсы которых можно загружать из сети Интернет.';ru='В справочнике валют отсутствуют валюты, курсы которых можно загружать из сети Интернет.'");
...
Функция ВыполнитьЗагрузкуКурсов()
	...
	НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელით გაშვება: %1';ru='Запуск вручную: %1'"), РегламентноеЗадание.Метаданные.Синоним);
...
Процедура ОбработатьРезультатЗагрузки(РезультатЗагрузки)
	...
	Иначе
		...
		Если КоличествоОшибок > 1 Тогда
			...
			Кнопки.Добавить("Подробнее", НСтр("ka='დაწვრილებით...';ru='Подробнее...'"));
			Кнопки.Добавить("Продолжить", НСтр("ka='გაგრძელება';ru='Продолжить'"));
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить курсы валют (%1).';ru='Не удалось загрузить курсы валют (%1).'"), КоличествоОшибок);
...
Процедура ПродолжитьЗагрузкуКурсовВалют()
	...
	Если Не ЗначениеЗаполнено(Объект.НачалоПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не задана дата начала периода загрузки.';ru='Не задана дата начала периода загрузки.'"),
			,
	...
	Если Не ЗначениеЗаполнено(Объект.ОкончаниеПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не задана дата окончания периода загрузки.';ru='Не задана дата окончания периода загрузки.'"),
			,
