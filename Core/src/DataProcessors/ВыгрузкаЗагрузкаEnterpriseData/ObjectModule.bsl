...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "") 
	...
	КонецЕсли;
	КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ka='Перенос данных через буфер обмена';ru='Перенос данных через буфер обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если МенеджерОбменаВнутренний Тогда
		...
		Если КомпонентыОбмена.МенеджерОбмена = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не поддерживается версия формата обмена: <%1>.';ru='Не поддерживается версия формата обмена: <%1>.'"), ТекВерсияФормата);
		КонецЕсли;
	КонецЕсли;
...
Функция РезультатВыгрузкиВXML(СтруктураПараметров, АдресДляРазмещенияРезультата = Неопределено) Экспорт
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ТекстСообщения = НСтр("ka='В ходе выполнения операции возникли ошибки';ru='В ходе выполнения операции возникли ошибки'") + ": " + Символы.ПС + КомпонентыОбмена.СтрокаСообщенияОбОшибке;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	ИначеЕсли НЕ ЕстьВыгруженныеОбъекты Тогда
		ТекстСообщения = НСтр("ka='Не найдено ни одного объекта к выгрузке.';ru='Не найдено ни одного объекта к выгрузке.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ЗагрузкаСообщения(СтруктураПараметров, АдресРезультата) Экспорт
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка чтения сообщения загрузки. Неверный формат сообщения.';ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.'"));
...
Процедура ЗаполнениеПравилВыгрузки() Экспорт
	...
	УзелДереваСправочники.Наименование = НСтр("ka='ცნობარები';ru='Справочники'");
	
	УзелДереваДокументы = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваДокументы.ЭтоГруппа = Истина;
	УзелДереваДокументы.Наименование = НСтр("ka='დოკუმენტები';ru='Документы'");

	УзелДереваПВХ = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваПВХ.ЭтоГруппа = Истина;
	УзелДереваПВХ.Наименование = НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик'");
	...
	Для Каждого СтрокаПОД Из КомпонентыОбмена.ПравилаОбработкиДанных Цикл
		...
		Если Метаданные.Справочники.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ka='Справочник %1';ru='Справочник %1'");
		ИначеЕсли Метаданные.Документы.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ka='Документ %1';ru='Документ %1'");
		ИначеЕсли  Метаданные.ПланыВидовХарактеристик.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ka='План видов характеристик %1';ru='План видов характеристик %1'");
		Иначе
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Некорректное имя метаданных ""%1""';ru='Некорректное имя метаданных ""%1""'"),
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='გაცვლის ფორმატის ვერსიის არაკანონიკური წარდგენა: <%1>.';ru='Неканоническое представление версии формата обмена: <%1>.'"), Результат.Версия);
	КонецЕсли;
