...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='В информационной базе не поддерживается синхронизация данных через универсальный формат';ru='В информационной базе не поддерживается синхронизация данных через универсальный формат';en='The information database is not supported by the data synchronization via a universal format'");
	КонецПопытки;
	...
	Если ВариантОткрытияФормы = "ТолькоЗагрузка" Тогда
		ЭтотОбъект.Заголовок = НСтр("ka='Загрузка данных EnterpriseData';ru='Загрузка данных EnterpriseData';en='Loading data EnterpriseData'");
		Элементы.НадписьЗагрузкаВстроеннойОбработкой.Видимость = Истина;
	Иначе
		Элементы.НадписьЗагрузкаВстроеннойОбработкой.Видимость = Ложь;
		ЭтотОбъект.Заголовок = НСтр("ka='Выгрузка и загрузка данных EnterpriseData';ru='Выгрузка и загрузка данных EnterpriseData';en='Upload and download data EnterpriseData'");
	КонецЕсли;
	...
	Если МассивДоступныхВерсий.Количество() = 0 Тогда
		СтрокаПоддерживаемыеВерсииФормата = НСтр("ka='Не обнаружены поддерживаемые версии формата';ru='Не обнаружены поддерживаемые версии формата';en='Not found the supported versions of the format'");
		Элементы.СтрокаПоддерживаемыеВерсииФормата.ЦветТекста = Новый Цвет(255,0,0);
		Элементы.ФормаВыполнитьОперацию.Доступность = Ложь;
	Иначе
		СтрокаПоддерживаемыеВерсииФормата = НСтр("ka='Поддерживаемые версии формата:';ru='Поддерживаемые версии формата:';en='Supported versions of the format:'");
...
Процедура ПрерватьВыгрузкуЗагрузку(Команда)
	...
	Сообщение.Текст = НСтр("ka='Выполнение операции прервано.';ru='Выполнение операции прервано.';en='The operation has stopped.'");
	Сообщение.Сообщить();
...
Процедура СохранитьНастройки(Команда)
	...
	ПолучитьФайл(АдресФайлаВХранилище, НСтр("ka='Файл настроек.xml';ru='Файл настроек.xml';en='nastroek.xml file'"));
КонецПроцедуры
...
Процедура ЗагрузитьСообщение()
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = НСтр("ka='Выполняется загрузка данных...';ru='Выполняется загрузка данных...';en='Data is loading ...'");
	...
	Если ИсточникЗагрузки = 1 Тогда
		Если НЕ ЗначениеЗаполнено(ДанныеДляЗагрузкиXDTO) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Текстовое поле с данными для загрузки не заполнено.';ru='Текстовое поле с данными для загрузки не заполнено.';en='Text field data is empty for downloads.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПриЗавершенииОперацииЗагрузки(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Загрузка данных завершена неудачно.';ru='Загрузка данных завершена неудачно.';en='Loading data failed.'");
...
Процедура ВыгрузитьДанные()
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = НСтр("ka='Выполняется выгрузка данных...';ru='Выполняется выгрузка данных...';en='Performed unloading data ...'");
	Элементы.ВыгрузкаЗагрузка.ТекущаяСтраница = Элементы.Ожидание;
	УстановитьВидимостьДоступностьКнопок(Ложь);
	МассивСообщений = Неопределено;
...
Процедура ПриЗавершенииОперацииВыгрузки(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Выгрузка данных завершена неудачно. Отсутствует результат выгрузки.';ru='Выгрузка данных завершена неудачно. Отсутствует результат выгрузки.';en='Upload data failed. No discharge result.'");
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ВызватьИсключение Результат.КраткоеПредставлениеОшибки;
	КонецЕсли;
...
Процедура ВыбратьФайлДляЗагрузкиНаКлиенте(ЗагружатьПослеВыбора = Ложь)
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='Данные загрузки';ru='Данные загрузки';en='data download'")+ "(*.xml)|*.xml";
...
Процедура ВыбратьФайлДляВыгрузкиНаКлиенте(ВыгружатьПослеВыбора = Ложь, АдресХраненияРезультата = "")
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='Данные выгрузки';ru='Данные выгрузки';en='data upload'")+ "(*.xml)|*.xml";
...
Функция ВыгрузитьДанныеНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Выгрузка данных EnterpriseData';ru='Выгрузка данных EnterpriseData';en='Upload data EnterpriseData'");
...
Функция ЗагрузитьДанныеНаСервере()
	...
	МодульДлительныеОперации = ОбщийМодуль("ДлительныеОперации");
	ПараметрыВыполнения = МодульДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Загрузка данных EnterpriseData';ru='Загрузка данных EnterpriseData';en='Loading data EnterpriseData'");
	РезультатФоновогоЗадания = МодульДлительныеОперации.ВыполнитьВФоне(ВыполняемыйМетод, ПараметрыЗадания, ПараметрыВыполнения);
	Возврат РезультатФоновогоЗадания;
	...
	Для Каждого СтрГруппыМД Из Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрМД Из СтрГруппыМД.ПолучитьЭлементы() Цикл
			...
			ИначеЕсли СтрокиДополнения.Количество() = 0 Тогда
				...
				НовСтрока.ОтборСтрокой = НСтр("ka='ყველა ობიექტი';ru='Все объекты';en='All objects'");
...
Процедура НачалоВыбораМодуляМенеджера(РеквизитМодульМенеджера, СтандартнаяОбработка, ОбновлятьВыгрузку)
	...
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='გაცვლის მენეჯერი (*.epf)';ru='Менеджер обмена (*.epf)';en='Exchange Manager (* .epf)'") + "|*.epf" ;
	ОповещениеВыбора = Новый ОписаниеОповещения("ВыбранФайл", ЭтотОбъект, 
...
Процедура СообщитьОбОкончанииОперации(Загрузка = Ложь)
	...
	Сообщение = Новый СообщениеПользователю();
	Если Загрузка Тогда
		Сообщение.Текст = НСтр("ka='Загрузка данных завершена.';ru='Загрузка данных завершена.';en='Loading data is completed.'");
	Иначе
		Сообщение.Текст = НСтр("ka='Выгрузка данных завершена.';ru='Выгрузка данных завершена.';en='Upload data is completed.'");
	КонецЕсли;
	Сообщение.Сообщить();
КонецПроцедуры
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ka='Общий модуль ""%1"" не найден.';ru='Общий модуль ""%1"" не найден.';en='Total module ""%1"" not found.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
