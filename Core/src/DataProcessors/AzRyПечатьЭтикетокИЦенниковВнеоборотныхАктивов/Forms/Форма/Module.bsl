...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.АдресВХранилище) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Иначе
				ВызватьИсключение НСтр("ka='Режим печати не установлен';ru='Режим печати не установлен';en='Print mode is not set'");;
			КонецЕсли;
	...
	КонецЕсли;
	элементы.ВидЦенДиапазона.Видимость = Константы.ТипЦеныНоменклатурыПоКоличеству.Получить(); //Lazare
	...
	Если ТекущиеДанные.Выбран Тогда
		...
		Если Объект.Режим = "ПечатьЦенниковИЭтикеток" Тогда
			...
			Если (ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение.Текст = НСтр("ka='Не заполнено количество ценников';ru='Не заполнено количество ценников';en='The number of price tags are not filled'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не выбран шаблон ценника';ru='Не выбран шаблон ценника';en='Not selected price tag template'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество этикеток';ru='Не заполнено количество этикеток';en='Do not select the number of labels'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не выбран шаблон этикетки';ru='Не выбран шаблон этикетки';en='Do not select a label template'");
			...
			ИначеЕсли (   ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество ценников и(или) этикеток';ru='Не заполнено количество ценников и(или) этикеток';en='Do not select the number of price tags and (or) label'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЦенников";
				Сообщение.Сообщить();
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не выбраны шаблоны ценников и(или) этикеток';ru='Не выбраны шаблоны ценников и(или) этикеток';en='Not selected price tags templates and (or) label'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].ШаблонЦенника";
				Сообщение.Сообщить();
			...
			ИначеЕсли (   ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество ценников и(или) шаблон этикетки';ru='Не заполнено количество ценников и(или) шаблон этикетки';en='Do not select the number of price tags and (or) label template'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЦенников";
				Сообщение.Сообщить();
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнено количество этикеток и(или) шаблон ценника';ru='Не заполнено количество этикеток и(или) шаблон ценника';en='Do not select the number of labels and (or) price tag template'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЭтикеток";
				Сообщение.Сообщить();
...
Процедура ЗаполнитьТаблицуЗапасов(Команда)
	...
	ТекстВопроса = НСтр("ka='При перезаполнении все введенные вручную данные будут потеряны, продолжить?';ru='При перезаполнении все введенные вручную данные будут потеряны, продолжить?';en='When refilling all manually entered data will be lost, continue?'");
...
Процедура Печать(Команда)
	...
	Если НЕ НеобходимостьПечати.ЕстьВыбранныеСтроки Тогда
		...
		ПоказатьПредупреждение(Неопределено,НСтр("ka='არ არის არჩეული არცერთი საქონელი';ru='Не выбрано ни одного товара';en='Do not choose any product'"));
		Возврат;
	...
	Если ЭтаФорма.ПроверитьЗаполнение() Тогда
		...
		ПараметрыПечати.Вставить("ЗаголовокФормы", НСтр("ru = 'Печать этикеток и ценников'; ka = 'ფასდებულების და ეტიკეტების ბეჭდვა'; en = 'Printing labels and price tags'"));
		...
		Если НеобходимостьПечати.НужноПечатать 
			...
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(
				"Обработка.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов", 
				ИменаМакетов,
				ПараметрКоманды,
				ЭтаФорма, 
				ПараметрыПечати
		...
		ПараметрыПечатиДляПринтераЭтикеток.Вставить("МенеджерПечати", "Обработка.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов");
		...
		ОткрытьФорму("Обработка.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов.Форма.ФормаПечать", ПараметрыОткрытия, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьКоличествоЦенников(Команда)
	...
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьКоличествоЦенниковЗавершение", ЭтотОбъект, Новый Структура("ВыбранноеЗначение", ВыбранноеЗначение)), ВыбранноеЗначение, НСтр("ka='Введите количество ценников';ru='Введите количество ценников';en='Enter the number of price tags'"), 10, 2);
...
Процедура УстановитьКоличествоЦенниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ka='Установлено количество ценников %Количество%.';ru='Установлено количество ценников %Количество%.';en='Price tags INSTALLATIONS number% number%.'");
...
Процедура УстановитьКоличествоЭтикетокЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ka='Установлено количество этикеток %Количество%.';ru='Установлено количество этикеток %Количество%.';en='INSTALLATIONS number of labels% No.%.'");
		Текст = СтрЗаменить(Текст, "%Количество%", ВыбранноеЗначение);
...
Процедура УстановитьШаблонЦенниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ka='Установлен шаблон ""%Шаблон%"".';ru='Установлен шаблон ""%Шаблон%"".';en='Set the pattern ''% pattern% "".'");
...
Процедура УстановитьРежимПечатьЦенниковИЭтикетокНаСервере()
	...
	Иначе
		//Заголовок = "Печать этикеток и ценников";
		  Заголовок = НСтр("ru = 'Печать этикеток и ценников'; ka = 'ფასდებულების და ეტიკეტების ბეჭდვა'; en = 'Printing labels and price tags'");
	КонецЕсли;
...
Процедура УстановитьРежим(ПараметрРежим)
	...
	ИначеЕсли ПараметрРежим = "ПечатьЭтикеток" Тогда
		УстановитьРежимПечатьЭтикетокНаСервере();
	Иначе
		ВызватьИсключение НСтр("ka='Установленный режим печати не поддерживается';ru='Установленный режим печати не поддерживается';en='Installed print mode is not supported'");
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	СхемаКомпоновкиДанных = Обработки.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов.ПолучитьМакет("ПоляШаблона");
...
Функция ПолучитьНеобходимостьПечатиМакетов()
	...
	ИсходныеДанные = Объект.Запасы.Выгрузить(Новый Структура("Выбран", Истина), "ВнеоборотныйАктив, Штрихкод, КоличествоЦенников, ШаблонЦенника, КоличествоЭтикеток, ШаблонЭтикетки");
...
Функция ПоказатьОповещениеПользователюОВозможнойОшибке(Текст, Количество, КоличествоВсего)
	...
	Если Количество < КоличествоВсего Тогда
		...
		Текст = Текст
		        + ?(Текст <> "", Символы.ПС, "")
		        + НСтр("ka='Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.';ru='Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.';en='The mark selection is set to KolichestvoVybrannyh%%% strings from KolichestvoVsego%.'");
	
		Текст = Текст
		        + Символы.ПС
		        + НСтр("ka='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.';ru='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.';en='Check: Do not filled with any number of, or do not specify a template price tag (label). In this case, the selection marker for lines can not be installed.'");
...
Процедура ЗаполнитьТаблицуТоваровНаСервере(ПроверятьЗаполнение = Истина)
	...
	СтруктураНастроек = Обработки.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов.ПолучитьПустуюСтруктуруНастроек();
	
	//СтруктураНастроек.ОбязательныеПоля.Добавить("Цена");
	//СтруктураНастроек.ОбязательныеПоля.Добавить("ЦенаВидЦен2");//Lazare 16.06.2021
	//СтруктураНастроек.ОбязательныеПоля.Добавить("Диапазон");//Lazare 10.09.2021
	СтруктураНастроек.ОбязательныеПоля.Добавить("Штрихкод");
	СтруктураНастроек.ОбязательныеПоля.Добавить("Количество");
	СтруктураНастроек.ОбязательныеПоля.Добавить("Имущество");
	...
	//СтруктураНастроек.ПараметрыДанных.Вставить("ВидЦен1"    , Объект.ВидЦен1);//Lazare 16.06.2021
	...
	СтруктураРезультата = Обработки.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов.ПодготовитьСтруктуруДанных(СтруктураНастроек);
	Для Каждого СтрокаТЧ Из СтруктураРезультата.ТаблицаТоваров Цикл
		
		НоваяСтрока = Объект.Запасы.Добавить();
		НоваяСтрока.ВнеоборотныйАктив    = СтрокаТЧ.Имущество;
		...
		//НоваяСтрока.Цена1                = СтрокаТЧ.ЦенаВидЦен2;//Lazare 16.06.2021
