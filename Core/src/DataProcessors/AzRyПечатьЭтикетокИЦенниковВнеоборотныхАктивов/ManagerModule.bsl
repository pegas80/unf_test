...
Функция ПодготовитьСтруктуруДанных(СтруктураНастроек) Экспорт
	...
	// Схема компоновки.
	СхемаКомпоновкиДанных = Обработки.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов.ПолучитьМакет(СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных);
	...
	Если СтруктураНастроек.ИсходныеДанные <> Неопределено Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Документ.ПринятиеКУчетуВА.ВнеоборотныеАктивы", "&Таблица");
...
Функция СформироватьПечатныеФормыЦенниковИЭтикеток(СтруктураНастроек) Экспорт
	...
	Эталон = Обработки.AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов.ПолучитьМакет("Эталон");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИсходныеДанные = ПараметрыПечати.ДанныеПечати[0].Запасы.Выгрузить(Новый Структура("Выбран", Истина), "ВнеоборотныйАктив,  Штрихкод, КоличествоЦенников, ШаблонЦенника, КоличествоЭтикеток, ШаблонЭтикетки");
	...
	СтруктураНастроек.ПараметрыДанных.Вставить("ВидЦен1", ПараметрыПечати.ДанныеПечати[0].ВидЦен1); //Lazare
	СтруктураНастроек.ПараметрыДанных.Вставить("ВидЦенДиапазона", ПараметрыПечати.ДанныеПечати[0].ВидЦенДиапазона); //Lazare 10.09.2021
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФормВнутренняя Цикл
		...
		НоваяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПараметрыПечати_AzRyПечатьЭтикетокИЦенниковВнеоборотныхАктивов";
