...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("ВсеОбъекты", НСтр("ka='Все данные';ru='Все данные'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, Новый ФиксированнаяСтруктура);
	
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъектыНеутвержденные", НСтр("ka='Изменения';ru='Изменения'"));
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъекты", НСтр("ka='Сопоставленные данные';ru='Сопоставленные данные'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 0));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъекты", НСтр("ka='Несопоставленные данные';ru='Несопоставленные данные'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыПриемника", НСтр("ka='Несопоставленные данные этой базы';ru='Несопоставленные данные этой базы'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления",  1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыИсточника", НСтр("ka='Несопоставленные данные второй базы';ru='Несопоставленные данные второй базы'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления", -1));
	...
	КонецЕсли;
	Заголовок = НСтр("ka='Сопоставление данных ""[ПредставлениеДанных]""';ru='Сопоставление данных ""[ПредставлениеДанных]""'");
	...
	НаименованиеЭтойПрограммы = ?(ПустаяСтрока(НаименованиеЭтойПрограммы), НСтр("ka='ეს პროგრამა';ru='Эта программа'"), НаименованиеЭтойПрограммы);
	
	НаименованиеВторойПрограммы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.УзелИнформационнойБазы, "Наименование");
	НаименованиеВторойПрограммы = ?(ПустаяСтрока(НаименованиеВторойПрограммы), НСтр("ka='Вторая программа';ru='Вторая программа'"), НаименованиеВторойПрограммы);
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	НСтрока = НСтр("ka='Получить данные в информационную базу?';ru='Получить данные в информационную базу?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Нет соответствия, объект будет скопирован';ru='Нет соответствия, объект будет скопирован'"));
...
Процедура ВыполнитьЗагрузкуДанныхПослеВопросаОПодтвержденииПолученияДанных(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		НСтрока = НСтр("ka='Данные уже были получены. Выполнить получение данных повторно?';ru='Данные уже были получены. Выполнить получение данных повторно?'");
...
Процедура ВыполнитьЗагрузкуДанныхПослеПодтвержденияПолученияДанных()
	...
	Состояние(НСтр("ka='Выполняется получение данных. Пожалуйста, подождите...';ru='Выполняется получение данных. Пожалуйста, подождите...'"));
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	Если ПользовательскиеПоля.Количество() = 0 Тогда
		...
		НСтрока = НСтр("ka='Следует указать хотя бы одно поле для отображения';ru='Следует указать хотя бы одно поле для отображения'");
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ka='Уменьшите количество полей (можно выбирать не более %1 полей)';ru='Уменьшите количество полей (можно выбирать не более %1 полей)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления=-1 Или ТекущиеДанные.СтатусСопоставления=+1) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Объекты уже сопоставлены';ru='Объекты уже сопоставлены'"), 2);
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,     НСтр("ka='დაყენება';ru='Установить'"));
	...
	ТекстВопроса = НСтр("ka='Установить соответствие между выбранными объектами?';ru='Установить соответствие между выбранными объектами?'");
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Если Результат.Количество() = 0 Тогда
		...
		Результат.Добавить(НСтр("ka='<не задан>';ru='<не задан>'"));
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(Отказ)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Сопоставление объектов';ru='Сопоставление объектов'");
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='Помощник сопоставления объектов.Анализ данных';ru='Помощник сопоставления объектов.Анализ данных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
