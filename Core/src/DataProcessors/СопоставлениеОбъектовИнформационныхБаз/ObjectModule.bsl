...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(Отказ, ТаблицыДляЗагрузки) Экспорт
	...
	Если ОбработкаОбменаДанными.ФлагОшибки() Тогда
		НСтрока = НСтр("ka='При загрузке сообщения обмена возникли ошибки: %1';ru='При загрузке сообщения обмена возникли ошибки: %1'");
	...
	Для каждого Элемент Из СписокПолейТаблицы Цикл
		...
		Если Реквизит = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Для объекта метаданных ""%1"" не определен реквизит с именем ""%2""';ru='Для объекта метаданных ""%1"" не определен реквизит с именем ""%2""'"),
