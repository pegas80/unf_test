...
Процедура ОбновитьЗаголовокСтраницы(КоличествоНастроек)
	...
	Если ВыбраннаяСтраницаНастроек = "НастройкиОтчетовСтраница" Тогда
		...
		Если КоличествоНастроекОтчетов = 0 Тогда
			ТекстЗаголовка = НСтр("ka='Настройки отчетов';ru='Настройки отчетов';en='reporting settings'");
		Иначе
			ТекстЗаголовка = НСтр("ka='Настройки отчетов (%1)';ru='Настройки отчетов (%1)';en='Reporting Settings (1%)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ВнешнийВидСтраница" Тогда
		...
		Если КоличествоНастроекВнешнегоВида = 0 Тогда
			ТекстЗаголовка = НСтр("ka='გარეგნული იერი';ru='Внешний вид';en='Appearance'");
		Иначе
			ТекстЗаголовка = НСтр("ka='Внешний вид (%1)';ru='Внешний вид (%1)';en='Appearance (1%)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ПрочиеНастройкиСтраница" Тогда
		...
		Если КоличествоПрочихНастроек = 0 Тогда
			ТекстЗаголовка = НСтр("ka='სხვა გამართვები';ru='Прочие настройки';en='Other settings'");
		Иначе
			ТекстЗаголовка = НСтр("ka='Прочие настройки (%1)';ru='Прочие настройки (%1)';en='Other settings (1%)'");
...
Процедура ЗагрузитьЗначенияПометок(ДеревоЗначений, ПомеченныеНастройки, ВидНастроек)
	...
	Если КоличествоПомеченных > 0 Тогда
		...
		Если ВидНастроек = "НастройкиОтчетов" Тогда
			...
			Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Настройки отчетов (%1)';ru='Настройки отчетов (%1)';en='Reporting Settings (1%)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ВнешнийВид" Тогда
			КоличествоНастроекВнешнегоВида = КоличествоПомеченных;
			Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Внешний вид (%1)';ru='Внешний вид (%1)';en='Appearance (1%)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ПрочиеНастройки" Тогда
			КоличествоПрочихНастроек = КоличествоПомеченных;
			Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Прочие настройки (%1)';ru='Прочие настройки (%1)';en='Other settings (1%)'"), КоличествоПомеченных);
		КонецЕсли;
