...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОчисткаНастроек Тогда
		Заголовок = НСтр("ka='Выбор пользователей для очистки настроек';ru='Выбор пользователей для очистки настроек';en='Select User settings for cleaning'");
		Элементы.Надпись.Заголовок = НСтр("ka='Выберите пользователей, которым необходимо очистить настройки';ru='Выберите пользователей, которым необходимо очистить настройки';en='Select the users who need to clear the settings'");
	КонецЕсли;
...
Процедура Выбрать(Команда)
	...
	Если ПользователиПриемник.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Необходимо отметить одного или несколько пользователей.';ru='Необходимо отметить одного или несколько пользователей.';en='It should be noted one or more users.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокГруппы(Форма, ГруппаПользователей, СтрокаСпискаПользователей, ЗначениеПометки)
	...
	Если ПомеченныйПользователь <> Неопределено И ЗначениеПометки <> СтрокаСпискаПользователей.Пометка Тогда
		...
		ГруппаПользователей.НаименованиеГруппыИПомеченоПользователей = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%2)';ru='%1 (%2)';en='%12)'"), Строка(ГруппаПользователей.Группа), ГруппаПользователей.ПомеченоПользователей);
	КонецЕсли;
