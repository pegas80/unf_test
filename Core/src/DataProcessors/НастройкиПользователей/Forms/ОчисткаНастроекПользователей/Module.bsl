...
Процедура ПереключательОчищаемыеНастройкиПриИзменении(Элемент)
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям"
		...
		ПоказатьПредупреждение(,НСтр("ka='Очистка отдельных настроек доступна только при выборе одного пользователя.';ru='Очистка отдельных настроек доступна только при выборе одного пользователя.'"));
...
"которым необходимо очистить настройки.'"), , "Источник");
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям" Тогда
		...
		Если КоличествоПользователей = 1 Тогда
			ПояснениеУКогоОчищеныНастройки = НСтр("ka='пользователя ""%1""';ru='пользователя ""%1""'");
		...
		Иначе
			ПояснениеУКогоОчищеныНастройки = НСтр("ka='%1 მომხმარებლებს';ru='%1 пользователям'");
	...
	Иначе
		ПояснениеУКогоОчищеныНастройки = НСтр("ka='всем пользователям';ru='всем пользователям'");
	КонецЕсли;
	
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки"
		И КоличествоНастроек = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите настройки, которые необходимо очистить.';ru='Выберите настройки, которые необходимо очистить.'"), , "ПереключательОчищаемыеНастройки");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки" Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = НСтр("ka='""%1"" очищена у %2';ru='""%1"" очищена у %2'");
		...
		Иначе
			...
			ТекстПояснения = НСтр("ka='Очищено %1 у %2';ru='Очищено %1 у %2'");
	...
	ИначеЕсли ПереключательОчищаемыеНастройки = "ОчиститьВсе" Тогда
		...
		ТекстПояснения = НСтр("ka='Очищены все настройки %1';ru='Очищены все настройки %1'");
