...
Процедура ПереключательОчищаемыеНастройкиПриИзменении(Элемент)
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям"
		...
		ПоказатьПредупреждение(,НСтр("ka='Очистка отдельных настроек доступна только при выборе одного пользователя.';ru='Очистка отдельных настроек доступна только при выборе одного пользователя.';en='Cleaning the individual settings is available only when a single user.'"));
...
Функция ОчисткаНастроек()
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям"
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите пользователя или пользователей,"
"которым необходимо очистить настройки.';ru='Выберите пользователя или пользователей,"
"которым необходимо очистить настройки.'"), , "Источник");
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям" Тогда
		...
		Если КоличествоПользователей = 1 Тогда
			ПояснениеУКогоОчищеныНастройки = НСтр("ka='пользователя ""%1""';ru='пользователя ""%1""';en='User ""%1""'");
		...
		Иначе
			ПояснениеУКогоОчищеныНастройки = НСтр("ka='%1 მომხმარებლებს';ru='%1 пользователям';en='% 1 people'");
	...
	Иначе
		ПояснениеУКогоОчищеныНастройки = НСтр("ka='всем пользователям';ru='всем пользователям';en='all users'");
	КонецЕсли;
	
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки"
		И КоличествоНастроек = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите настройки, которые необходимо очистить.';ru='Выберите настройки, которые необходимо очистить.';en='Select the settings that need to be cleaned.'"), , "ПереключательОчищаемыеНастройки");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки" Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = НСтр("ka='""%1"" очищена у %2';ru='""%1"" очищена у %2';en='""%1"" is cleared from 2%'");
		...
		Иначе
			...
			ТекстПояснения = НСтр("ka='Очищено %1 у %2';ru='Очищено %1 у %2';en='Cleaned 1% in 2%'");
	...
	ИначеЕсли ПереключательОчищаемыеНастройки = "ОчиститьВсе" Тогда
		...
		ТекстПояснения = НСтр("ka='Очищены все настройки %1';ru='Очищены все настройки %1';en='Cleared all the settings% 1'");
