...
"скопировать, выберите другого пользователя.';ka='У выбранного пользователя нет настроек, которые можно было бы"
...
"выберите другого пользователя.';ka='Нельзя копировать настройки пользователя самому себе,"
"выберите другого пользователя.'"));
	...
	Элементы.ВыбратьНастройки.Заголовок = НСтр("ka='არჩევა';ru='Выбрать';en='Select'");
...
Процедура Скопировать(Команда)
	...
	Если ПользовательСсылка = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите пользователя, чьи настройки необходимо скопировать.';ru='Выберите пользователя, чьи настройки необходимо скопировать.';en='Select the user whose settings you want to copy.'"), , "ПользовательСсылка");
		Возврат;
	КонецЕсли;
	
	Если КоличествоПользователей = 0 И ПереключательКомуКопироватьНастройки <> "ВсемПользователям" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите одного или несколько пользователей, которым необходимо скопировать настройки.';ru='Выберите одного или несколько пользователей, которым необходимо скопировать настройки.';en='Select one or more users who want to copy the settings.'"), , "Приемник");
		Возврат;
	КонецЕсли;
	
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" И КоличествоНастроек = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите настройки, которые необходимо скопировать.';ru='Выберите настройки, которые необходимо скопировать.';en='Select the settings you want to copy.'"), , "ПереключательКопируемыеНастройки");
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьНастройкиПослеВыбора(Параметр, Контекст) Экспорт
	...
	Если КоличествоНастроек = 0 Тогда
		ТекстЗаголовка = НСтр("ka='არჩევა';ru='Выбрать';en='Select'");
...
Процедура СкопироватьНастройки(ИмяКоманды)
	...
	Иначе
		ПояснениеКомуСкопированыНастройки = НСтр("ka='всем пользователям';ru='всем пользователям';en='all users'");
	КонецЕсли;
	...
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" Тогда
		...
		Если Отчет = Неопределено Тогда
			...
			ПоказатьОповещениеПользователя(НСтр("ka='Копирование настроек';ru='Копирование настроек';en='copy settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
	...
	Иначе
		...
		Если Не НастройкиСкопированы Тогда
			...
			ТекстПредупреждения = НСтр("ka='Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';ru='Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';en='Settings were not copied because the user ""%1"" was not saved any settings.'");
		...
		ТекстПояснения = НСтр("ka='Скопированы все настройки %1';ru='Скопированы все настройки %1';en='Copied all the settings% 1'");
