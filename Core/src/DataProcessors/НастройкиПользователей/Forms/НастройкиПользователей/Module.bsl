...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Заголовок = НСтр("ka='მომხმარებლის გასწორებები';ru='Настройки пользователя'");
...
Процедура ПользовательСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИспользоватьВнешнихПользователей Тогда
		...
		ВыборТипаПользователей.Добавить("ВнешниеПользователи", НСтр("ka='გარე მომხმარებლები';ru='Внешние пользователи'"));
		ВыборТипаПользователей.Добавить("Пользователи", НСтр("ka='მომხმარებლები';ru='Пользователи'"));
...
Процедура НастройкиПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ТекстВопроса = НСтр("ka='Очистить выбранные настройки?';ru='Очистить выбранные настройки?'");
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("НастройкиОтчетов", НСтр("ka='Настройки отчетов';ru='Настройки отчетов'"));
	КопируемыеНастройки.Добавить("НастройкиВнешнегоВида", НСтр("ka='Настройки внешнего вида';ru='Настройки внешнего вида'"));
	КопируемыеНастройки.Добавить("ДанныеФорм", НСтр("ka='Данные форм';ru='Данные форм'"));
	КопируемыеНастройки.Добавить("ПерсональныеНастройки", НСтр("ka='პერსონალური პარამეტრები';ru='Персональные настройки'"));
	КопируемыеНастройки.Добавить("Избранное", НСтр("ka='Избранное';ru='Избранное'"));
	КопируемыеНастройки.Добавить("НастройкиПечати", НСтр("ka='Настройки печати';ru='Настройки печати'"));
	КопируемыеНастройки.Добавить(
		"ПрочиеПользовательскиеНастройки", НСтр("ka='Настройки дополнительных отчетов и обработок';ru='Настройки дополнительных отчетов и обработок'"));
...
Процедура Очистить(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ka='Необходимо выбрать настройки, которые требуется удалить.';ru='Необходимо выбрать настройки, которые требуется удалить.'"));
		Возврат;
		
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ОчиститьЗавершение", ЭтотОбъект, ДеревоНастроек);
	ТекстВопроса = НСтр("ka='Очистить выделенные настройки?';ru='Очистить выделенные настройки?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = НСтр("ka='Очистить все настройки у пользователя ""%1""?';ru='Очистить все настройки у пользователя ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ka='გასუფთავება';ru='Очистить'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена'"));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = НСтр("ka='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';ru='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ka='გასუფთავება';ru='Очистить'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена'"));
...
"Продолжить?'");
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("ОчиститьВсе", НСтр("ka='ყველას გასუფთავება';ru='Очистить все'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена'"));
...
Функция ОбновлениеСпискаНастроек()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Обновление настроек пользователей';ru='Обновление настроек пользователей'");
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Настройки отчетов (%1)';ru='Настройки отчетов (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.НастройкиОтчетовСтраница.Заголовок = НСтр("ka='Настройки отчетов';ru='Настройки отчетов'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Внешний вид (%1)';ru='Внешний вид (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ВнешнийВидСтраница.Заголовок = НСтр("ka='გარეგნული იერი';ru='Внешний вид'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Прочие настройки (%1)';ru='Прочие настройки (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ПрочиеНастройкиСтраница.Заголовок = НСтр("ka='სხვა გამართვები';ru='Прочие настройки'");
	КонецЕсли;
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		...
		Если Параметр.КопироватьВсе Тогда
			...
			Если МассивНастроек.Количество() = 7 Тогда
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Скопированы все настройки %1';ru='Скопированы все настройки %1'"), ПояснениеКомуСкопированыНастройки);
			Иначе
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 скопированы %2';ru='%1 скопированы %2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
			КонецЕсли;
				
			Состояние(НСтр("ka='Выполняется копирование настроек';ru='Выполняется копирование настроек'"));
			
			Отчет = Неопределено;
			СкопироватьВсеНастройкиНаСервере(ПользовательИнформационнойБазы, ПользователиПриемник, МассивНастроек, Отчет);
			
			Если Отчет <> Неопределено Тогда
				ТекстВопроса = НСтр("ka='Не все варианты отчетов и настройки были скопированы.';ru='Не все варианты отчетов и настройки были скопированы.'");
				КнопкиВопроса = Новый СписокЗначений;
				КнопкиВопроса.Добавить("Ок", НСтр("ka='ОК';ru='ОК'"));
				КнопкиВопроса.Добавить("ПоказатьОтчет", НСтр("ka='ანგარიშგების ჩვენება';ru='Показать отчет'"));
			...
			ПоказатьОповещениеПользователя(
				НСтр("ka='Копирование настроек';ru='Копирование настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='Очистка настроек';ru='Очистка настроек'"), ,
		НСтр("ka='Очищены все настройки всех пользователя';ru='Очищены все настройки всех пользователя'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ka='Очищены все настройки пользователя ""%1""';ru='Очищены все настройки пользователя ""%1""'");
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПояснения, ПользовательСсылка);
	ПоказатьОповещениеПользователя(НСтр("ka='Очистка настроек';ru='Очистка настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ka='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""';ru='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""'");
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = Формат(КоличествоНастроек, "ЧДЦ=0") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНастроек,
			"Л = ru_RU", НСтр("ka='გამართვა,გამართვები,გამართვებს,,,,,,0';ru='настройка,настройки,настроек,,,,,,0'"));
	
	Если КоличествоНастроек = 1
		И КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = НСтр("ka='""%1"" очищена пользователю ""%2""';ru='""%1"" очищена пользователю ""%2""'");
	...
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = НСтр("ka='Очищено %1 пользователю ""%2""';ru='Очищено %1 пользователю ""%2""'");
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = НСтр("ka='""%1"" очищена %2';ru='""%1"" очищена %2'");
		...
		Иначе
			ТекстПояснения = НСтр("ka='Очищено %1 %2';ru='Очищено %1 %2'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Очистка настроек';ru='Очистка настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Необходимо выбрать настройки, которые требуется скопировать.';ru='Необходимо выбрать настройки, которые требуется скопировать.'"));
		Возврат;
