...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимРаботы.ЭтоАдминистраторПрограммы И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ФайловыеФункции") Тогда
		Если РежимРаботы.Файловый Тогда
			...
			СписокВыбора[0].Представление = НСтр("ka='Все рабочие станции работают под управлением ОС Windows';ru='Все рабочие станции работают под управлением ОС Windows'");
			
			СписокВыбора = Элементы.ИзвлекатьТекстыФайловНаСервереLinux.СписокВыбора;
			СписокВыбора[0].Представление = НСтр("ka='Одна или несколько рабочих станций работают под управлением ОС Linux';ru='Одна или несколько рабочих станций работают под управлением ОС Linux'");
		КонецЕсли;
	Иначе
	...
	Иначе
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='სრულტექსტიანი ძებნის მართვა';ru='Управление полнотекстовым поиском'");
		Элементы.ОписаниеРаздела.Заголовок =
			НСтр("ka='Включение и отключение полнотекстового поиска, обновление индекса полнотекстового поиска.';ru='Включение и отключение полнотекстового поиска, обновление индекса полнотекстового поиска.'");
	КонецЕсли;
...
"Пожалуйста, подождите.'"));
	...
	Состояние(НСтр("ka='Обновление полнотекстового индекса завершено.';ru='Обновление полнотекстового индекса завершено.'"));
КонецПроцедуры
...
"Пожалуйста, подождите.'"));
	...
	Состояние(НСтр("ka='Очистка полнотекстового индекса завершена.';ru='Очистка полнотекстового индекса завершена.'"));
КонецПроцедуры
...
"Пожалуйста, подождите.'"));
	...
	Исключение
		ТекстСообщенияОбОшибке = 
			НСтр("ka='В настоящее время проверка индекса невозможна, так как выполняется его очистка или обновление.';ru='В настоящее время проверка индекса невозможна, так как выполняется его очистка или обновление.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияОбОшибке);
	КонецПопытки;
	
	Состояние(НСтр("ka='Проверка полнотекстового индекса завершена.';ru='Проверка полнотекстового индекса завершена.'"));
КонецПроцедуры
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент, ОбновлятьИнтерфейс = Истина)
	...
	Если Результат.Свойство("НеУдалосьУстановитьРежимПолнотекстовогоПоиска") Тогда
		...
		ТекстВопроса = НСтр("ka='Для изменения режима полнотекстового поиска требуется завершение сеансов всех пользователей, кроме текущего.';ru='Для изменения режима полнотекстового поиска требуется завершение сеансов всех пользователей, кроме текущего.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("АктивныеПользователи", НСтр("ka='აქტიური მომხმარებლები';ru='Активные пользователи'"));
		...
		ИначеЕсли РеквизитПутьКДанным = "МаксимальныйРазмерИндексируемыхДанных"
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					НСтр("ka='სრულტექსტიანი ძებნა';ru='Полнотекстовый поиск'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "", ИндексПроверен = Ложь)
	...
	Если РеквизитПутьКДанным = ""
		...
		Если ИспользоватьПолнотекстовыйПоиск = 1 Тогда
			...
			Если ИндексПроверен И Не ИндексСодержитКорректныеДанные Тогда
				СтатусИндекса = НСтр("ka='Требуется очистка и обновление';ru='Требуется очистка и обновление'");
			ИначеЕсли ИндексАктуален Тогда
				СтатусИндекса = НСтр("ka='Обновление не требуется';ru='Обновление не требуется'");
			Иначе
				СтатусИндекса = НСтр("ka='Требуется обновление';ru='Требуется обновление'");
			КонецЕсли;
		Иначе
			ДатаАктуальностиИндекса = '00010101';
			ИндексАктуален = Ложь;
			ФлагДоступность = Ложь;
			СтатусИндекса = НСтр("ka='Полнотекстовый поиск отключен';ru='Полнотекстовый поиск отключен'");
		КонецЕсли;
	...
	Если Элементы.ГруппаАвтоматическоеИзвлечениеТекстов.Видимость
		...
		Иначе
			РасписаниеПредставление = НСтр("ka='Автоматическое извлечение текстов не выполняется.';ru='Автоматическое извлечение текстов не выполняется.'");
		КонецЕсли;
