...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.НемедленнаяОтправкаЭД() Тогда
		Элементы.КомандаПодписать.Заголовок = НСтр("ka='ხელმოწერა';ru='Подписать';en='Sign'");
		Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ka='Утвердить и подписать';ru='Утвердить и подписать';en='Approve and sign'");
		Команды.УтвердитьПодписатьИОтправить.Подсказка = НСтр("ru='Утвердить, подписать и подготовить к отправке"
"выделенные электронные документы';ka='Утвердить, подписать и подготовить к отправке"
"выделенные электронные документы'");
		Если НЕ ИспользоватьЭП Тогда
			Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ka='დამტკიცება';ru='Утвердить';en='approve'");
			Команды.УтвердитьПодписатьИОтправить.Подсказка = НСтр("ru='Утвердить и подготовить к отправке"
"выделенные электронные документы';ka='Утвердить и подготовить к отправке"
"выделенные электронные документы'");
		КонецЕсли;
	ИначеЕсли НЕ ИспользоватьЭП Тогда
		Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ka='დამტკიცება და გაგზავნა';ru='Утвердить и отправить';en='Approve and send'");
		Команды.УтвердитьПодписатьИОтправить.Подсказка = НСтр("ru='Утвердить и отправить"
"выделенные электронные документы';ka='Утвердить и отправить"
"выделенные электронные документы'");
	КонецЕсли;
...
Процедура ОтклонитьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ka='მოწვევების უარყოფა';ru='Отклоняются приглашения';en='decline invitations'");
		...
		ШаблонСообщения = НСтр("ka='უარყოფილია მოწვევა: %1';ru='Отклонено приглашений: %1';en='Declined invitations% 1'");
...
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';ka='При отклонении приглашений возникли ошибки."
...
Процедура ПринятьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ka='მიიღება მოწვევები';ru='Принимаются приглашения';en='accepted an invitation'");
		...
		ШаблонСообщения = НСтр("ka='მიღებულია მოწვევა: %1';ru='Принято приглашений: %1';en='Accepted invitations% 1'");
...
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';ka='При принятии приглашений возникли ошибки."
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ka='მიმღებებისთვის მოწვევების გაგზავნა';ru='Отправка приглашений получателям';en='Sending invitations recipients'");
		
		ОтправленоПриглашений = 0;
		ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейИПараметровСертификатов);
		
		ШаблонСообщения = НСтр("ka='გაგზავნილია მოსაწვევი: %1';ru='Отправлено приглашений: %1';en='Sent invitations% 1'");
...
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';ka='При отправке приглашений возникли ошибки."
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПакетыОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонОповещения = НСтр("ka='Отправлено пакетов: (%1).';ru='Отправлено пакетов: (%1).';en='Sent packages (1%).'");
	ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОповещения, КоличествоОтправленных);

	ЗаголовокОповещения = НСтр("ka='Отправка электронных документов';ru='Отправка электронных документов';en='Sending electronic documents'");
...
//
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='(';ru='(';en='('");
...
Процедура УстановитьОтборПоДополнительнойИнформацииВСписке(Отбор, Значение, ИспользоватьОтбор)
	...
	ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Отбор.Элементы, НСтр("ka='Доп. информация';ru='Доп. информация';en='Extras. information'"), ТипГруппы);
...
Функция ДанныеДереваДействий(Знач ДеревоДействий, Запросы)
	...
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Сформировать",       НСтр("ka='შექმნა';ru='Создать';en='Create'"),              Массив[0]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Обработать",         НСтр("ka='Отразить в учете';ru='Отразить в учете';en='Record a'"),     Массив[1]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Утвердить",          НСтр("ka='დამტკიცება';ru='Утвердить';en='approve'"),            Массив[2]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Подписать",          НСтр("ka='ხელმოწერა';ru='Подписать';en='Sign'"),            Массив[3]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Исправить",          НСтр("ka='Исправить';ru='Исправить';en='Correct'"),            Массив[4]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Аннулировать",       НСтр("ka='ანულირება';ru='Аннулировать';en='repeal'"),         Массив[5]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "НаКонтроле",         НСтр("ka='კონტროლზე';ru='На контроле';en='under control'"),          Массив[6]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Отправить",          НСтр("ka='გაგზავნა';ru='Отправить';en='to send'"),            Массив[7]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Распаковать",        НСтр("ka='Распаковать';ru='Распаковать';en='unpack'"),          Массив[8]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Приглашения",        НСтр("ka='Приглашения';ru='Приглашения';en='invitations'"),          Массив[9]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ТребуетсяПригласить",НСтр("ka='საჭიროა მოწვევა';ru='Требуется пригласить';en='I want to invite'"), Массив[10]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ЖдемСогласия",       НСтр("ka='Ждем согласия';ru='Ждем согласия';en='We are waiting for approval'"),        Массив[11]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ТребуетсяСогласие",  НСтр("ka='მოითხოვება თანხმობა';ru='Требуется согласие';en='consent is required'"),   Массив[12]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Входящие",           НСтр("ka='შემომავალი';ru='Входящие';en='inbox'"),             0);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Исходящие",          НСтр("ka='გამავლები';ru='Исходящие';en='outbox'"),            0);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ПодписатьИсх",       НСтр("ka='ხელმოწერა';ru='Подписать';en='Sign'"),            Массив[15]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "УтвердитьИсх",       НСтр("ka='დამტკიცება';ru='Утвердить';en='approve'"),            Массив[16]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ИсправитьИсх",       НСтр("ka='Исправить';ru='Исправить';en='Correct'"),            Массив[17]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "АннулироватьИсх",    НСтр("ka='ანულირება';ru='Аннулировать';en='repeal'"),         Массив[18]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "НаКонтролеИсх",      НСтр("ka='კონტროლზე';ru='На контроле';en='under control'"),          Массив[19]);
...
Процедура УстановитьСтатусПакетов(Знач МассивЭД, Знач СтатусПакета, КоличествоИзмененных)
	...
	Для Каждого СтрокаТаблицы Из МассивЭД Цикл
		...
		Исключение
			...
			ТекстОперации  = НСтр("ka='изменение статуса пакетов ЭД';ru='изменение статуса пакетов ЭД';en='ED packages status change'");
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	Если ИспользуетсяНесколькоОрганизацийЭД Тогда
		...
		НоваяСтрока.ПредставлениеПараметра = НСтр("ka='ორგანიზაცია';ru='Организация:';en='Organization:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='კონტრაგენტი:';ru='Контрагент:';en='Counterpart:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Дата с:';ru='Дата с:';en='Date from:'");
	НоваяСтрока.Тип = "Дата";
	НоваяСтрока.Значение = Дата(1,1,1);
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Дата_По";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Дата по:';ru='Дата по:';en='Date:'");
	НоваяСтрока.Тип = "Дата";
	НоваяСтрока.Значение = Дата(1,1,1);
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_С";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Сумма с:';ru='Сумма с:';en='Sum with:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_По";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Сумма по:';ru='Сумма по:';en='The sum of:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "ВидЭД";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Вид документа:';ru='Вид документа:';en='Type of document:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='მიმართულება:';ru='Направление:';en='Direction:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Доп. информация:';ru='Доп. информация:';en='Extras. information:'");
	НоваяСтрока.Тип = "Строка";
	НоваяСтрока.Значение = "";
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Ответственный";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='პასუხისმგებელი:';ru='Ответственный:';en='Responsible:'");
...
Процедура УстановитьСтатусОтменен(МассивЭД)
	...
	ТекстОповещения = НСтр("ka='Изменен статус пакетов на ""Отменен"": (%1)';ru='Изменен статус пакетов на ""Отменен"": (%1)';en='It changed the status of packages on the ""Canceled"": (% 1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами';en='Exchange of electronic documents'"), , ТекстОповещения);
	ОбновитьДанныеДереваНаКлиенте();
	ОбновитьДинамическиеСписки();
...
Процедура ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейНастроекЭДОМаркерам)
	...
	Для каждого НастройкаЭДО Из Элементы.ТребуетсяПригласить.ВыделенныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(СтруктураПараметровНастройкиЭДО.ЭлектроннаяПочтаДляПриглашения) Тогда
			ШаблонСообщения = НСтр("ka='ედ-ის გაცვლაზე მოწვევის გასაგზავნად მიმღებ %1-სთვის აუცილებელია ელექტრონული ფოსტის შევსება.';ru='Для отправки приглашения к обмену ЭД для получателя %1"
"необходимо заполнить электронную почту.'");
		...
		Если Не ЗначениеЗаполнено(СтруктураПараметровКонтрагента[ИмяРеквизитаИННКонтрагента]) Тогда
			ШаблонСообщения = НСтр("ka='ედ-ის გასაცვლელად მოწვევის გასაგზავნად მიმღებ %1-სთვის"
"აუცილებელია გსნ-ის შევსება.';ru='Для отправки приглашения к обмену ЭД для получателя %1"
...
Процедура ПоказатьСкрытьБыстрыйПоиск()
	...
	Элементы.ЗаголовокОтбора.Заголовок = НСтр("ka='Быстрый поиск';ru='Быстрый поиск';en='Quick search'") + ?(ПоказатьБыстрыйПоиск, " " + НСтр("ru='(скрыть)';en='(Hide)'"),  " " + НСтр("ru='(показать)';en='(Show)'"));
