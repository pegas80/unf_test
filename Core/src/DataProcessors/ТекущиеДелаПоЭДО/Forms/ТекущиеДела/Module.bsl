...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.НемедленнаяОтправкаЭД() Тогда
		Элементы.КомандаПодписать.Заголовок = НСтр("ka='ხელმოწერა';ru='Подписать'");
		Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ka='Утвердить и подписать';ru='Утвердить и подписать'");
...
"выделенные электронные документы'");
		Если НЕ ИспользоватьЭП Тогда
			Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ka='დამტკიცება';ru='Утвердить'");
...
"выделенные электронные документы'");
	...
	ИначеЕсли НЕ ИспользоватьЭП Тогда
		Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ka='დამტკიცება და გაგზავნა';ru='Утвердить и отправить'");
...
Процедура ОтклонитьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ka='მოწვევების უარყოფა';ru='Отклоняются приглашения'");
		...
		ШаблонСообщения = НСтр("ka='უარყოფილია მოწვევა: %1';ru='Отклонено приглашений: %1'");
...
Процедура ПринятьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ka='მიიღება მოწვევები';ru='Принимаются приглашения'");
		...
		ШаблонСообщения = НСтр("ka='მიღებულია მოწვევა: %1';ru='Принято приглашений: %1'");
...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ka='მიმღებებისთვის მოწვევების გაგზავნა';ru='Отправка приглашений получателям'");
		
		ОтправленоПриглашений = 0;
		ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейИПараметровСертификатов);
		
		ШаблонСообщения = НСтр("ka='გაგზავნილია მოსაწვევი: %1';ru='Отправлено приглашений: %1'");
...
Процедура ОтправитьПакетыОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонОповещения = НСтр("ka='Отправлено пакетов: (%1).';ru='Отправлено пакетов: (%1).'");
	ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОповещения, КоличествоОтправленных);

	ЗаголовокОповещения = НСтр("ka='Отправка электронных документов';ru='Отправка электронных документов'");
...
//
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='(';ru='('");
...
Процедура УстановитьОтборПоДополнительнойИнформацииВСписке(Отбор, Значение, ИспользоватьОтбор)
	...
	ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Отбор.Элементы, НСтр("ka='Доп. информация';ru='Доп. информация'"), ТипГруппы);
...
Функция ДанныеДереваДействий(Знач ДеревоДействий, Запросы)
	...
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Сформировать",       НСтр("ka='შექმნა';ru='Создать'"),              Массив[0]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Обработать",         НСтр("ka='Отразить в учете';ru='Отразить в учете'"),     Массив[1]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Утвердить",          НСтр("ka='დამტკიცება';ru='Утвердить'"),            Массив[2]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Подписать",          НСтр("ka='ხელმოწერა';ru='Подписать'"),            Массив[3]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Исправить",          НСтр("ka='Исправить';ru='Исправить'"),            Массив[4]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Аннулировать",       НСтр("ka='ანულირება';ru='Аннулировать'"),         Массив[5]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "НаКонтроле",         НСтр("ka='კონტროლზე';ru='На контроле'"),          Массив[6]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Отправить",          НСтр("ka='გაგზავნა';ru='Отправить'"),            Массив[7]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Распаковать",        НСтр("ka='Распаковать';ru='Распаковать'"),          Массив[8]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Приглашения",        НСтр("ka='Приглашения';ru='Приглашения'"),          Массив[9]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ТребуетсяПригласить",НСтр("ka='საჭიროა მოწვევა';ru='Требуется пригласить'"), Массив[10]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ЖдемСогласия",       НСтр("ka='Ждем согласия';ru='Ждем согласия'"),        Массив[11]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ТребуетсяСогласие",  НСтр("ka='მოითხოვება თანხმობა';ru='Требуется согласие'"),   Массив[12]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Входящие",           НСтр("ka='შემომავალი';ru='Входящие'"),             0);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Исходящие",          НСтр("ka='გამავლები';ru='Исходящие'"),            0);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ПодписатьИсх",       НСтр("ka='ხელმოწერა';ru='Подписать'"),            Массив[15]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "УтвердитьИсх",       НСтр("ka='დამტკიცება';ru='Утвердить'"),            Массив[16]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ИсправитьИсх",       НСтр("ka='Исправить';ru='Исправить'"),            Массив[17]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "АннулироватьИсх",    НСтр("ka='ანულირება';ru='Аннулировать'"),         Массив[18]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "НаКонтролеИсх",      НСтр("ka='კონტროლზე';ru='На контроле'"),          Массив[19]);
...
Процедура УстановитьСтатусПакетов(Знач МассивЭД, Знач СтатусПакета, КоличествоИзмененных)
	...
	Для Каждого СтрокаТаблицы Из МассивЭД Цикл
		...
		Исключение
			...
			ТекстОперации  = НСтр("ka='изменение статуса пакетов ЭД';ru='изменение статуса пакетов ЭД'");
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	Если ИспользуетсяНесколькоОрганизацийЭД Тогда
		...
		НоваяСтрока.ПредставлениеПараметра = НСтр("ka='ორგანიზაცია';ru='Организация:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='კონტრაგენტი:';ru='Контрагент:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Дата с:';ru='Дата с:'");
	НоваяСтрока.Тип = "Дата";
	НоваяСтрока.Значение = Дата(1,1,1);
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Дата_По";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Дата по:';ru='Дата по:'");
	НоваяСтрока.Тип = "Дата";
	НоваяСтрока.Значение = Дата(1,1,1);
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_С";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Сумма с:';ru='Сумма с:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_По";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Сумма по:';ru='Сумма по:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "ВидЭД";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Вид документа:';ru='Вид документа:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='მიმართულება:';ru='Направление:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Доп. информация:';ru='Доп. информация:'");
	НоваяСтрока.Тип = "Строка";
	НоваяСтрока.Значение = "";
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Ответственный";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='პასუხისმგებელი:';ru='Ответственный:'");
...
Процедура УстановитьСтатусОтменен(МассивЭД)
	...
	ТекстОповещения = НСтр("ka='Изменен статус пакетов на ""Отменен"": (%1)';ru='Изменен статус пакетов на ""Отменен"": (%1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами'"), , ТекстОповещения);
	ОбновитьДанныеДереваНаКлиенте();
	ОбновитьДинамическиеСписки();
...
Процедура ПоказатьСкрытьБыстрыйПоиск()
	...
	Элементы.ЗаголовокОтбора.Заголовок = НСтр("ka='Быстрый поиск';ru='Быстрый поиск'") + ?(ПоказатьБыстрыйПоиск, " " + НСтр("ru='(скрыть)'"),  " " + НСтр("ru='(показать)'"));
