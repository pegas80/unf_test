...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НадписьЛогина.Подсказка = НСтр("ka='Логин текущего пользователя Интернет-поддержки:';ru='Логин текущего пользователя Интернет-поддержки:';en='Username of the current user of Internet support:'")
...
#Если ВебКлиент Тогда
	ПоказатьПредупреждение(,
		НСтр("ka='В веб-клиенте некоторые ссылки могут работать неправильно."
"Приносим извинения за неудобства.';ru='В веб-клиенте некоторые ссылки могут работать неправильно."
...
Процедура HTMLДокументПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Найти(НРег(СокрЛП(КлассАктивногоЭлемента)),"openupdate") <> 0 Тогда
		...
		Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы") Тогда
			ПоказатьПредупреждение(
				,
				НСтр("ka='Отсутствует механизм автоматического обновления.';ru='Отсутствует механизм автоматического обновления.';en='There is no mechanism to automatically update.'"));
			Возврат;
		КонецЕсли;
...
"<Опишите возникшие проблемы>.';ka='При обновлении конфигурации на новый релиз возникли следующие проблемы:"
"<Опишите возникшие проблемы>.'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='Интернет-поддержка. Обновление конфигурации.';ru='Интернет-поддержка. Обновление конфигурации.';en='Internet support. Update configuration.'"),
			ТекстСообщения,
			,
			,
