...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщаяПроизводительностьСистемы.Пустая() Тогда
		Объект.ОбщаяПроизводительностьСистемы = НСтр("ka='სისტემის საერთო მწარმოებლურობა';ru='Общая производительность системы'");
	Иначе
	...
	Исключение
		ТекстСообщения = НСтр("ka='Не удалось загрузить настройки.';ru='Не удалось загрузить настройки.'");
...
"Рассчитать целевое время невозможно.'"));
	...
	Подсказка = НСтр("ka='Введите желаемое значение APDEX';ru='Введите желаемое значение APDEX'"); 
	ЗначениеAPDEX = 0;
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Если ТаблицаКлючевыхОпераций = Неопределено Тогда
		ОценкаПроизводительностиКлиентСервер.СообщитьПользователю(НСтр("ka='Период установлен не верно.';ru='Период установлен не верно.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	ТекстЗаголовка = НСтр("ka='Диаграмма производительности с %1 по %2 - шаг: %3';ru='Диаграмма производительности с %1 по %2 - шаг: %3'");
...
Процедура ОбменятьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
	...
	Исключение
		ОтменитьТранзакцию();
 		ЗаписьЖурналаРегистрации(НСтр("ka='ოპერაციის შესრულება';ru='Выполнение операции'", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
 		ВызватьИсключение;
	КонецПопытки;
...
Функция ВычислитьДатыВременногоОтрезка(ДатаНачала, ДатаОкончания, ИндексПериода)
	...
	Если КоличествоШагов <= ИндексПериода Тогда
		ВызватьИсключение НСтр("ka='Количество шагов не может быть меньше индекса.';ru='Количество шагов не может быть меньше индекса.'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			...
			ОценкаПроизводительностиКлиентСервер.СообщитьПользователю(
				НСтр("ka='Целевое время обязательно должно быть заполнено.';ru='Целевое время обязательно должно быть заполнено.'"),
				,
