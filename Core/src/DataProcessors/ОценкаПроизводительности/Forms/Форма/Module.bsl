...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщаяПроизводительностьСистемы.Пустая() Тогда
		Объект.ОбщаяПроизводительностьСистемы = НСтр("ka='სისტემის საერთო მწარმოებლურობა';ru='Общая производительность системы';en='Overall system performance'");
	Иначе
	...
	Исключение
		ТекстСообщения = НСтр("ka='Не удалось загрузить настройки.';ru='Не удалось загрузить настройки.';en='Unable to load settings.'");
...
"Рассчитать целевое время невозможно.';ka='Отсутствуют замеры производительности."
"Рассчитать целевое время невозможно.'"));
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("УказатьAPDEXЗавершение", ЭтотОбъект);
	Подсказка = НСтр("ka='Введите желаемое значение APDEX';ru='Введите желаемое значение APDEX';en='Enter the desired value APDEX'"); 
	ЗначениеAPDEX = 0;
...
Процедура УказатьAPDEXЗавершение(Знач ЗначениеAPDEX, Знач ДополнительныеПараметры) Экспорт
	...
	Если 0 > ЗначениеAPDEX Или ЗначениеAPDEX > 1 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Введен неправильный показатель APDEX."
"Допустимые значения от 0 до 1.';ru='Введен неправильный показатель APDEX."
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Если ТаблицаКлючевыхОпераций = Неопределено Тогда
		ОценкаПроизводительностиКлиентСервер.СообщитьПользователю(НСтр("ka='Период установлен не верно.';ru='Период установлен не верно.';en='The period is not set correctly.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	ТекстЗаголовка = НСтр("ka='Диаграмма производительности с %1 по %2 - шаг: %3';ru='Диаграмма производительности с %1 по %2 - шаг: %3';en='Diagram with performance of 1% 2% - Step: 3%'");
...
Процедура ОбменятьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
	...
	Исключение
		ОтменитьТранзакцию();
 		ЗаписьЖурналаРегистрации(НСтр("ka='ოპერაციის შესრულება';ru='Выполнение операции';en='operation'", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
 		ВызватьИсключение;
	КонецПопытки;
...
Функция ВычислитьДатыВременногоОтрезка(ДатаНачала, ДатаОкончания, ИндексПериода)
	...
	Если КоличествоШагов <= ИндексПериода Тогда
		ВызватьИсключение НСтр("ka='Количество шагов не может быть меньше индекса.';ru='Количество шагов не может быть меньше индекса.';en='The number of steps can not be less than the index.'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			...
			ОценкаПроизводительностиКлиентСервер.СообщитьПользователю(
				НСтр("ka='Целевое время обязательно должно быть заполнено.';ru='Целевое время обязательно должно быть заполнено.';en='Target time must be filled.'"),
				,
