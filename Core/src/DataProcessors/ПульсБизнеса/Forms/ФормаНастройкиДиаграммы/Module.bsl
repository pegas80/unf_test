...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИдентификаторСтроки") Тогда
		...
		Заголовок = НСтр("ka='დიაგრამის აწყობა';ru='Настройка диаграммы'");
	Иначе
		ИдентификаторСтроки = -1;
		Заголовок = НСтр("ka='დიაგრამის დამატება';ru='Добавление диаграммы'");
	КонецЕсли; 
	...
	Если ЗначениеЗаполнено(Диаграмма) Тогда
		...
		Если ЗначениеЗаполнено(Серия) Тогда
			...
			Если НастройкиСерии.ТипДиаграммы=ТипДиаграммы.Круговая 
				...
				Элементы.Серия.Заголовок = НСтр("ka='მაჩვენებელი';ru='Показатель'");
				Элементы.Точка.Заголовок = НСтр("ka='მნიშვნელობა';ru='Значения'");
				Элементы.ПериодСравненияПредставление.Видимость = Ложь;
			ИначеЕсли НастройкиСерии.ТипДиаграммы=ТипДиаграммы.ГистограммаСНакоплением 
				...
				Элементы.Серия.Заголовок = НСтр("ka='ვერტიკალურად';ru='Вертикально'");
				Элементы.Точка.Заголовок = НСтр("ka='ჰორიზონტალურად';ru='Горизонтально'");
				Элементы.ПериодСравненияПредставление.Видимость = Ложь;
			Иначе
				Элементы.Серия.Заголовок = НСтр("ka='ვერტიკალურად';ru='Вертикально'");
				Элементы.Точка.Заголовок = НСтр("ka='ჰორიზონტალურად';ru='Горизонтально'");
...
Процедура УстановитьСвостваПолейПериодов()
	...
	Если РежимОстатка Тогда
		...
		КонецЕсли; 
		Элементы.ПериодПредставление.Заголовок = НСтр("ka='თარიღისთვის';ru='На дату'");
	...
	Иначе
		...
		КонецЕсли; 
		Элементы.ПериодПредставление.Заголовок = НСтр("ka='პერიოდი';ru='Период'");
...
Процедура ВыборПериодаНачало(Элемент)
	...
	Если РежимОстатка Тогда
		Если Элемент=Элементы.ПериодПредставление Тогда
			...
			ДанныеВыбора.Добавить(Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала.ПроизвольнаяДата), НСтр("ka='თავისუფალი თარიღი';ru='Произвольная дата'"));
			СтруктураПараметров.Вставить("Значение", Период);
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			// Дата сравнения
			ДанныеВыбора.Добавить(Неопределено, НСтр("ka='არ შედარდეს';ru='Не сравнивать'"));
	...
	Иначе
		Если Элемент=Элементы.ПериодПредставление Тогда
			...
			ДанныеВыбора.Добавить(Новый Структура("Вариант", "Последние7ДнейНеСчитаяТекущего"), НСтр("ka='ბოლო 7 დღე, მიმდინარეს გაუთვალისწინებლად';ru='Последние 7 дней, не считая текущего'"));
			ДанныеВыбора.Добавить(Новый СтандартныйПериод(ВариантСтандартногоПериода.ПроизвольныйПериод), НСтр("ka='თავისუფალი თარიღი';ru='Произвольный период'"));
			СтруктураПараметров.Вставить("Значение", Период);
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			// Период сравнения
			ДанныеВыбора.Добавить(Неопределено, НСтр("ka='არ შედარდეს';ru='Не сравнивать'"));
...
Процедура ВыборПериодаЗавершение(ВыбранноеЗначение, СтруктураПараметров) Экспорт
	...
	Если РежимОстатка Тогда
		Если НЕ ЗначениеЗаполнено(ПериодРасчета) И НЕ ПериодРасчета=Неопределено Тогда
			...
			ПоказатьВводДаты(Оповещение, ТекущееЗначение.Дата, НСтр("ka='მიუთითეთ თარიღი';ru='Укажите дату'"), ЧастиДаты.Дата); 
...
Функция ПредставлениеСтандартногоПериода(Период)
	...
	Если НЕ ЗначениеЗаполнено(Период) Тогда
		Возврат НСтр("ka='არ არის შერჩეული';ru='Не выбран'");
	...
	ИначеЕсли Период.Вариант="ПредыдущийПлавающийПериод" Тогда
		Возврат НСтр("ka='გასული პერიოდი (';ru='Предыдущий период ('")+ПредставлениеПериода(НачалоДня(Период.Период.ДатаНачала), КонецДня(Период.Период.ДатаОкончания))+")";
	ИначеЕсли Период.Вариант="ЗаПрошлыйГод" Тогда
		Возврат НСтр("ka='გასული წლისთვის (';ru='За прошлый год ('")+ПредставлениеПериода(НачалоДня(Период.Период.ДатаНачала), КонецДня(Период.Период.ДатаОкончания))+")";
	ИначеЕсли Период.Вариант="Последние7ДнейНеСчитаяТекущего" Тогда
		Возврат НСтр("ka='ბოლო 7 დღე, მიმდინარეს გაუთვალისწინებლად';ru='Последние 7 дней, не считая текущего'");
	Иначе
...
Функция ПредставлениеСтандартнойДатыНачала(Период)
	...
	ИначеЕсли Период.Вариант="ТакойЖеДеньНаПрошлойНеделе" Тогда
		Возврат НСтр("ka='გასული კვირის იგივე დღე (';ru='Такой же день на прошлой неделе ('")+Формат(Период.Дата, "ДФ='dd.MM.yyyy, ддд'")+")";
	ИначеЕсли Период.Вариант="ТакойЖеДеньВПрошломМесяце" Тогда
		Возврат НСтр("ka='გასული თვის იგივე დღე (';ru='Такой же день прошлого месяца ('")+Формат(Период.Дата, "ДФ='dd.MM.yyyy, ддд'")+")";
	ИначеЕсли Период.Вариант="ТакойЖеДеньВПрошломГоду" Тогда
		Возврат НСтр("ka='გასული წლის იგივე დღე (';ru='Такой же день в прошлом году ('")+Формат(Период.Дата, "ДФ='dd.MM.yyyy'")+")";
	Иначе
		Возврат Строка(Период);
	КонецЕсли; 
