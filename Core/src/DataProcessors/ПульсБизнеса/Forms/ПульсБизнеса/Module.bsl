...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОткрытаНеНаНачальнойСтранице Тогда
		Заголовок = НСтр("ka='ბიზნესის პულსი';ru='Пульс бизнеса'");
	КонецЕсли; 
...
Процедура ДатаВыбор(Команда)
	...
	Если ДатаСравнения<>Неопределено Тогда
		Меню.Добавить("ВСравнении", НСтр("ka='შედარების გამორთვა';ru='Отключить сравнение'"));
	Иначе
		Меню.Добавить("ВСравнении", НСтр("ka='შედარებაში';ru='В сравнении'"));
	КонецЕсли; 
...
Процедура ДатаСравненияВыбор(Команда)
	...
	Меню.Добавить(Новый СтандартнаяДатаНачала, НСтр("ka='თავისუფალი თარიღი';ru='Произвольная дата'"));
...
Процедура ДатаСравненияВыборЗавершение(ВыбранноеЗначение, ДополнительныеДанные) Экспорт
	...
	Если ВыбранноеЗначение.Значение=Новый СтандартнаяДатаНачала Тогда
		...
		ПоказатьВводДаты(Оповещение, ДатаСравнения, НСтр("ka='მიუთითეთ თარიღი';ru='Укажите дату'"), ЧастиДаты.Дата);
	Иначе
...
Процедура ПериодВыбор(Команда)
	...
	Меню.Добавить(Новый СтандартныйПериод, НСтр("ka='თავისუფალი პერიოდი';ru='Произвольный период'"));
...
Процедура КонтекстУдалитьПоказатель(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ka='წავშალოთ მაჩვენებლები?';ru='Удалить показатель?'"), РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
...
Процедура КонтекстУдалитьДиаграмму(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ka='წავშალოთ დიაგრამა?';ru='Удалить диаграмму?'"), РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
...
Процедура ЗапуститьФоновоеЗаданиеНаСервере(Секция="", СохраняемыеПериоды = "")
	...
	НаименованиеЗадания = НСтр("ka='ბიზნესის პულსის ვიჯეტების განახლება';ru='Обновление виджетов пульса бизнеса'");
...
Процедура СоздатьЭлементыПоказатели()
	...
	Для каждого Стр Из ДобавленныеПоказатели Цикл
		...
		Если ПустаяСтрока(Стр.ИмяГруппы) Тогда
			...
			КонецЕсли; 
			// Контекстное меню
			ДобавитьКоманду(Стр.ИмяГруппы, "_Настроить", ЭлементЗаголовок, "КонтекстНастроитьПоказатель", НСтр("ka='მაჩვენებლის აწყობა';ru='Настроить показатель'"), БиблиотекаКартинок.ХранилищеНастроек);
			Кнопка = ДобавитьКоманду(Стр.ИмяГруппы, "_Удалить", ЭлементЗаголовок, "КонтекстУдалитьПоказатель", НСтр("ka='მაჩვენებლის წაშლა';ru='Удалить показатель'"), БиблиотекаКартинок.Удалить);
			Если ПустаяСтрока(Стр.Ресурс) Тогда
				Кнопка.Заголовок = НСтр("ka='სათაურის წაშლა';ru='Удалить заголовок'");
			КонецЕсли; 
			ДобавитьКоманду(Стр.ИмяГруппы, "_Вверх", ЭлементЗаголовок, "КонтекстСместитьВверхПоказатель", НСтр("ka='გადაადგილება მაღლა';ru='Сместить вверх'"), БиблиотекаКартинок.ПереместитьВверх);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Вниз", ЭлементЗаголовок, "КонтекстСместитьВнизПоказатель", НСтр("ka='გადაადგილება დაბლა';ru='Сместить вниз'"), БиблиотекаКартинок.ПереместитьВниз);
		Иначе
			...
			Иначе
				...
				Для каждого СтруктураЗначения Из ТаблицаЗначений Цикл
					Если ОписаниеСерии.ТипДиаграммы=ТипДиаграммы.Круговая ИЛИ 
						...
						НСтр("ka='<არ არის ჩევსებული>';ru='<не заполнено>'"), 
					...
					ИначеЕсли ОписаниеСерии.ТипДиаграммы=ТипДиаграммы.ГрафикСОбластямиИНакоплением Тогда
						...
						Иначе
							...
							НСтр("ka='არ არის შევსებული>';ru='<не заполнено>'"), 
...
Процедура ВывестиОшибкуДиаграммы(ОбъектДиаграмма, ИнформацияОбОшибке)
	...
	ПодробноеПредставлениеОшибки = НСтр("ka='შეცდომა შედგენისას:';ru='Ошибка при формировании:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
...
Процедура СоздатьЭлементыДиаграммы()
	...
	Для каждого Стр Из ДобавленныеДиаграммы Цикл
		...
		Если ПустаяСтрока(Стр.ИмяГруппы) Тогда
			...
			// Контекстное меню
			ДобавитьКоманду(Стр.ИмяГруппы, "_Настроить", ЭлементДиаграмма, "КонтекстНастроитьДиаграмму", НСтр("ka='გრაფიკის აწყობა';ru='Настроить график'"), БиблиотекаКартинок.ХранилищеНастроек);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Удалить", ЭлементДиаграмма, "КонтекстУдалитьДиаграмму", НСтр("ka='გრაფიკის წაშლა';ru='Удалить график'"), БиблиотекаКартинок.Удалить);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Вверх", ЭлементДиаграмма, "КонтекстСместитьВверхДиаграмму", НСтр("ka='გადაადგილება ზემოთ';ru='Сместить вверх'"), БиблиотекаКартинок.ПереместитьВверх);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Вниз", ЭлементДиаграмма, "КонтекстСместитьВнизДиаграмму", НСтр("ka='გადაადგილება ქვემოთ';ru='Сместить вниз'"), БиблиотекаКартинок.ПереместитьВниз);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Расшифровать", ЭлементДиаграмма, "КонтекстРасшифроватьДиаграмму", НСтр("ka='გაშიფრვა';ru='Расшифровать'"), БиблиотекаКартинок.Найти);
			// Контекстное меню заголовка
			ДобавитьКоманду(Стр.ИмяГруппы, "_Настроить", ЭлементЗаголовок, "КонтекстНастроитьДиаграмму", НСтр("ka='გრაფიკის აწყობა';ru='Настроить график'"), БиблиотекаКартинок.ХранилищеНастроек);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Удалить", ЭлементЗаголовок, "КонтекстУдалитьДиаграмму", НСтр("ka='გრაფიკის წაშლა';ru='Удалить график'"), БиблиотекаКартинок.Удалить);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Вверх", ЭлементЗаголовок, "КонтекстСместитьВверхДиаграмму", НСтр("ka='გადაადგილება ზემოთ';ru='Сместить вверх'"), БиблиотекаКартинок.ПереместитьВверх);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Вниз", ЭлементЗаголовок, "КонтекстСместитьВнизДиаграмму", НСтр("ka='გადაადგილება ქვემოთ';ru='Сместить вниз'"), БиблиотекаКартинок.ПереместитьВниз);
			ДобавитьКоманду(Стр.ИмяГруппы, "_Расшифровать", ЭлементЗаголовок, "КонтекстРасшифроватьДиаграмму", НСтр("ka='გაშიფრვა';ru='Расшифровать'"), БиблиотекаКартинок.Найти);
		Иначе
			Группа = Элементы[Стр.ИмяГруппы];
...
Процедура НаполнитьСлучайнымиДанными(ОбъектДиаграмма, ТаблицаЗначений, ОписаниеТочки, ОписаниеСерии)
	...
	Если ТаблицаЗначений.Количество()>0 Тогда
		Для каждого СтруктураЗначения Из ТаблицаЗначений Цикл
			...
			Иначе
				Точка = ОбъектДиаграмма.УстановитьТочку(
				?(НЕ ЗначениеЗаполнено(СтруктураЗначения.Точка), 
				НСтр("ka='<არ არის შევსებული>';ru='<не заполнено>'")+СуфиксСравнения, 
				Строка(СтруктураЗначения.Точка)+СуфиксСравнения));
			КонецЕсли; 
			Серия = ОбъектДиаграмма.УстановитьСерию(НСтр("ka='თანხა';ru='Сумма'"));
			...
			Если ОписаниеСерии.ТипДиаграммы=ТипДиаграммы.ГистограммаСНакоплением 
				...
				Серия = ОбъектДиаграмма.УстановитьСерию(НСтр("ka='თანხა 2';ru='Сумма 2'"));
	...
	Иначе
		Для ии = 1 По 8 Цикл
			Если ОписаниеСерии.ТипДиаграммы=ТипДиаграммы.Круговая ИЛИ 
				...
				Точка = ОбъектДиаграмма.УстановитьТочку(НСтр("ka='თანხა';ru='Сумма'"));
	...
	КонецЕсли; 
	ОбъектДиаграмма.ОтображатьЗаголовок = Истина;
	ОбъектДиаграмма.ОбластьЗаголовка.Текст = НСтр("ka='მაგალითი';ru='ПРИМЕР'");
...
Процедура УстановитьОтображениеДиаграммы(Диаграмма, ЕстьДанныеДляОтображения, ЦветаСерий = Неопределено)
	...
	Диаграмма.СводнаяСерия.Текст = НСтр("ka='სხვა დანარჩენი';ru='Прочее'");
...
Функция ПредставлениеСтандартногоПериода(Период, ПериодОснование = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(Период) Тогда
		Возврат НСтр("ka='არ არის შერცეული';ru='Не выбран'");
	ИначеЕсли Период.Вариант=ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		Иначе
			Возврат ПредставлениеПериода(НачалоДня(Период.ДатаНачала), ?(ЗначениеЗаполнено(Период.ДатаОкончания), КонецДня(Период.ДатаОкончания), Период.ДатаОкончания), НСТР("ka='Л=ka; ФП = Истина';ru='ФП = Истина'"));
		КонецЕсли; 
	ИначеЕсли Период.Вариант="ПредыдущийПлавающийПериод" Тогда
		Возврат НСтр("ka='გასული პერიოდი (';ru='Предыдущий период ('")+ПредставлениеПериода(НачалоДня(Период.ДатаНачала), КонецДня(Период.ДатаОкончания), НСТР("ka='Л=ka; ФП = Истина';ru='ФП = Истина'"))+")";
	ИначеЕсли Период.Вариант="ЗаПрошлыйГод" Тогда
		Возврат НСтр("ka='გასული წელი (';ru='За прошлый год ('")+ПредставлениеПериода(НачалоДня(Период.ДатаНачала), КонецДня(Период.ДатаОкончания), НСТР("ka='Л=ka; ФП = Истина';ru='ФП = Истина'"))+")";
	ИначеЕсли Период.Вариант="Последние7ДнейНеСчитаяТекущего" Тогда
		Возврат НСтр("ka='გასული 7 დღე, მიმდინარე დღის გაუთვალისწინებლად';ru='Последние 7 дней, не считая текущего'");
	Иначе
		пПериод = Строка(Период);
		Если пПериод = "Вчера" Тогда
			Возврат  НСТР("ka='გუშინ';ru='Вчера'");
		ИначеЕсли пПериод = "Сегодня" Тогда
			Возврат НСТР("ka='დღეს';ru='Сегодня'")
		Иначе
			Возврат пПериод;
		КонецЕсли;
	КонецЕсли; 
...
Функция ПредставлениеСтандартнойДатыНачала(Дата, ДатаСравнения = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(Дата) Тогда
		Возврат НСтр("ka='არ არის არჩეული';ru='Не выбрана'");
	...
	ИначеЕсли Дата.Вариант=ВариантСтандартнойДатыНачала.НачалоЭтогоДня Тогда
		Возврат НСтр("ka='დღეს, დღის დასაწყისისთვის';ru='Сегодня, на начало дня'");
	ИначеЕсли Дата.Вариант=ВариантСтандартнойДатыНачала.НачалоСледующегоДня Тогда
		Возврат НСтр("ka='ყოველთვის აქტუალურია';ru='Всегда актуально'");
	ИначеЕсли Дата.Вариант="ТакойЖеДеньНаПрошлойНеделе" Тогда
		Возврат НСтр("ka='წინა კვირას იგივე დღე (';ru='Такой же день на прошлой неделе ('")+Формат(Дата.Дата, "ДФ='dd.MM.yyyy, ддд'")+")";
	ИначеЕсли Дата.Вариант="ТакойЖеДеньВПрошломМесяце" Тогда
		Возврат НСтр("ka='წინა თვის იგივე დღე (';ru='Такой же день прошлого месяца ('")+Формат(Дата.Дата, "ДФ='dd.MM.yyyy, ддд'")+")";
	ИначеЕсли Дата.Вариант="ТакойЖеДеньВПрошломГоду" Тогда
		Возврат НСтр("ka='წინა წლის იგივე დღე (';ru='Такой же день в прошлом году ('")+Формат(Дата.Дата, "ДФ='dd.MM.yyyy'")+")";
	Иначе
		Возврат Строка(Дата);
	КонецЕсли; 
...
Процедура ОбновитьПредставленияПериодов(Форма)
	...
	Форма.Элементы.ДатаВыбор.Заголовок = ВРег(НСтр("ka='დღეისთვის';ru='На сегодня'"));
...
Процедура Инициализация()
	...
	// Продажи
	ДобавитьПоказатель("Продажи", "Выручка", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='შემოსავალი';ru='Выручка'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "Основной", "Сумма");
	ДобавитьПоказатель("Продажи", "Количество", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='გაყიდული საქონლის რაოდ-ბა';ru='Кол-во проданных товаров'"),,, "ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "Основной", "Количество");
	ДобавитьПоказатель("Продажи", "КоличествоДокументов", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='გაყიდვების რაოდენობა (დოკუმენტების)';ru='Кол-во продаж (документов)'"),,, "ЧДЦ=; ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "ВаловаяПрибыльПоМенеджерам", "КоличествоДокументов");
	ДобавитьПоказатель("Продажи", "Себестоимость", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='თვითღირებულება';ru='Себестоимость'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "ВаловаяПрибыль", "Себестоимость");
	ДобавитьПоказатель("Продажи", "Прибыль", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='მოგება';ru='Прибыль'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "ВаловаяПрибыль", "ВаловаяПрибыль");
	ДобавитьПоказатель("Продажи", "Наценка", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='მოგება';ru='Прибыль/с-сть'"),,, "ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "ВаловаяПрибыль", "Наценка");
	ДобавитьПоказатель("Продажи", "Рентабельность", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='მოგება/ამონაგები';ru='Прибыль/выручка'"),,, "ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "ВаловаяПрибыль", "Рентабельность");
	ДобавитьПоказатель("Продажи", "ВозвратыСумма", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='დაბრინებების თანხა';ru='Сумма возвратов'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "Возвраты", "Сумма");
	ДобавитьПоказатель("Продажи", "ВозвратыКоличество", НСтр("ka='გაყიდვები';ru='Продажи'"), НСтр("ka='დაბრუნების რაოდ-ბა';ru='Кол-во возвратов'"),,, "ЧН=-", "СКД_РН_Продажи", "Отчет.Продажи", "Возвраты", "Количество");
	
	// Розница
	ДобавитьПоказатель("Розница", "РозничныеПродажи", НСтр("ka='საცალო';ru='Розница'"), НСтр("ka='საცალო გაყიდვები';ru='Розничные продажи'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Продажи", "Отчет.РозничныеПродажи", "РозничныеПродажиКонтекст", "Сумма");
	ДобавитьПоказатель("Розница", "КоличествоЧеков", НСтр("ka='საცალო';ru='Розница'"), НСтр("ka='ჩეკების რაოდენობა';ru='Количество чеков'"),,, "ЧН=-", "СКД_Розница", "Отчет.РозничныеПродажи", "РозничныеПродажиПоТочкамКонтекст", "КоличествоЧеков");
	ДобавитьПоказатель("Розница", "СреднийЧек", НСтр("ka='საცალო';ru='Розница'"), НСтр("ka='საშუალო ჩეკი';ru='Средний чек'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_Розница", "Отчет.РозничныеПродажи", "РозничныеПродажиПоТочкамКонтекст", "СреднийЧек");
	ДобавитьПоказатель("Розница", "ПолученоНаличными", НСтр("ka='საცალო';ru='Розница'"), НСтр("ka='მიღებულია ნაღდი ფულით';ru='Получено наличными'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_Розница", "Отчет.РозничныеПродажи", "РозничныеПродажиПоТочкамКонтекст", "ПолученоНаличными");
	ДобавитьПоказатель("Розница", "ПолученоКартами", НСтр("ka='საცალო';ru='Розница'"), НСтр("ka='ბარათებით მიღებული';ru='Получено картами'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_Розница", "Отчет.РозничныеПродажи", "РозничныеПродажиПоТочкамКонтекст", "ПолученоКартами");
	ДобавитьПоказатель("Розница", "СуммаОстаток", НСтр("ka='საცალო';ru='Розница'"), НСтр("ka='ნაშთები სსა სალაროებში';ru='Остаток в кассах ККМ'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ДенежныеСредстваВКассахККМ", "Отчет.ДенежныеСредстваВКассахККМ", "Остатки", "СуммаКонечныйОстаток");

	// Товары
	ДобавитьПоказатель("Товары", "КоличествоОстаток", НСтр("ka='საქონელი';ru='Товары'"), НСтр("ka='ნაშთი (რაოდენობა)';ru='Остаток (кол-во)'"), Истина,, "ЧН=-", "СКД_РН_Запасы", "Отчет.Запасы", "Остатки",, "Запасы");
	ДобавитьПоказатель("Товары", "СуммаОстаток", НСтр("ka='საქონელი';ru='Товары'"), НСтр("ka='ნაშთი (თანხა)';ru='Остаток (сумма)'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Запасы", "Отчет.Запасы", "Остатки",, "Запасы");
	ДобавитьПоказатель("Товары", "КоличествоПриход", НСтр("ka='საქონელი';ru='Товары'"), НСтр("ka='შემოსავალი (რაოდენობა)';ru='Приход (кол-во)'"),,, "ЧН=-", "СКД_РН_Запасы", "Отчет.Запасы", "Ведомость");
	ДобавитьПоказатель("Товары", "СуммаПриход", НСтр("ka='საქონელი';ru='Товары'"), НСтр("ka='შემოსავალი (თანხა)';ru='Приход (сумма)'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Запасы", "Отчет.Запасы", "Ведомость");
	ДобавитьПоказатель("Товары", "КоличествоРасход", НСтр("ka='საქონელი';ru='Товары'"), НСтр("ka='გასავალი (რაოდენობა)';ru='Расход (кол-во)'"),,, "ЧН=-", "СКД_РН_Запасы", "Отчет.Запасы", "Ведомость");
	ДобавитьПоказатель("Товары", "СуммаРасход", НСтр("ka='საქონელი';ru='Товары'"), НСтр("ka='გასავალი (თანხა)';ru='Расход (сумма)'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_Запасы", "Отчет.Запасы", "Ведомость");

	// Деньги
	ДобавитьПоказатель("Деньги", "СуммаОстаток", НСтр("ka='ფული';ru='Деньги'"), НСтр("ka='თანხის ნაშთი';ru='Остаток денег'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ДенежныеСредства", "Отчет.ДенежныеСредства", "Остатки",, "Деньги");
	ДобавитьПоказатель("Деньги", "ДенежныйПоток", НСтр("ka='ფული';ru='Деньги'"), НСтр("ka='სუფთა ფულადი ნაკადი';ru='Чистый денежный поток'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ДенежныеСредства", "Отчет.ДенежныеСредства", "Ведомость");
	ДобавитьПоказатель("Деньги", "Поступления", НСтр("ka='ფული';ru='Деньги'"), НСтр("ka='შემოსვლები';ru='Поступления'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ДенежныеСредства", "Отчет.ДенежныеСредства", "Ведомость");
	ДобавитьПоказатель("Деньги", "Платежи", НСтр("ka='ფული';ru='Деньги'"), НСтр("ka='გადახდები';ru='Платежи'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ДенежныеСредства", "Отчет.ДенежныеСредства", "Ведомость");
	ДобавитьПоказатель("Деньги", "ПоступленияПлан", НСтр("ka='ფული';ru='Деньги'"), НСтр("ka='შემოსვლა (გეგმა)';ru='Поступления (план)'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ПлатежныйКалендарь", "Отчет.ПлановыеДвиженияДенежныхСредств", "Ведомость", "Поступления");
	ДобавитьПоказатель("Деньги", "ПлатежиПлан", НСтр("ka='ფული';ru='Деньги'"), НСтр("ka='გადახდები (გეგმა)';ru='Платежи (план)'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ПлатежныйКалендарь", "Отчет.ПлановыеДвиженияДенежныхСредств", "Ведомость", "Платежи");

	// Счета-заказы
	ДобавитьПоказатель("СчетаЗаказы", "СуммаСчетов", НСтр("ka='ანგარიშები - შეკვეთები';ru='Счета - заказы'"), НСтр("ka='წარმოდგენილია ანგარიშები თანხით';ru='Выставлено счетов на сумму'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ОплатаСчетовИЗаказов", "Отчет.АнализСчетовИЗаказовВВалютеУчета", "СчетаНаОплатуКонтекст", "Сумма");
	ДобавитьПоказатель("СчетаЗаказы", "КоличествоСчетов", НСтр("ka='ანგარიშები - შეკვეთები';ru='Счета - заказы'"), НСтр("ka='წარმოდგენილია ანგარიშები (ცალი)';ru='Выставлено счетов (штук)'"),,, "ЧДЦ=; ЧН=-", "СКД_РН_ОплатаСчетовИЗаказов", "Отчет.АнализСчетовИЗаказовВВалютеУчета", "СчетаНаОплатуКонтекст", "КоличествоДокументов");
	ДобавитьПоказатель("СчетаЗаказы", "СуммаЗаказов", НСтр("ka='ანგარიშები - შეკვეთები';ru='Счета - заказы'"), НСтр("ka='მიღებულია შეკვეთები თანხით';ru='Получено заказов на сумму'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ОплатаСчетовИЗаказов", "Отчет.АнализСчетовИЗаказовВВалютеУчета", "ЗаказыПокупателейКонтекст", "Сумма");
	ДобавитьПоказатель("СчетаЗаказы", "КоличествоЗаказов", НСтр("ka='ანგარიშები - შეკვეთები';ru='Счета - заказы'"), НСтр("ka='მიღებულია შეკვეთები (ცალი)';ru='Получено заказов (штук)'"),,, "ЧДЦ=; ЧН=-", "СКД_РН_ОплатаСчетовИЗаказов", "Отчет.АнализСчетовИЗаказовВВалютеУчета", "ЗаказыПокупателейКонтекст", "КоличествоДокументов");
	ДобавитьПоказатель("СчетаЗаказы", "СуммаЗаказовПоставщику", НСтр("ka='ანგარიშები - შეკვეთები';ru='Счета - заказы'"), НСтр("ka='გაგზავნილია შეკვეთები თანხით';ru='Отправлено заказов на сумму'"),, Истина, "ЧДЦ=2; ЧН=-", "СКД_РН_ОплатаСчетовИЗаказов", "Отчет.АнализСчетовИЗаказовВВалютеУчета", "ЗаказыПоставщикамКонтекст", "Сумма");
	ДобавитьПоказатель("СчетаЗаказы", "КоличествоЗаказовПоставщику", НСтр("ka='ანგარიშები - შეკვეთები';ru='Счета - заказы'"), НСтр("ka='გაგზავნილია შეკვეთები (ცალი)';ru='Отправлено заказов (штук)'"),,, "ЧДЦ=; ЧН=-", "СКД_РН_ОплатаСчетовИЗаказов", "Отчет.АнализСчетовИЗаказовВВалютеУчета", "ЗаказыПоставщикамКонтекст", "КоличествоДокументов");

	// Долги
	ДобавитьПоказатель("Долги", "ДолгиОбщие", НСтр("ka='დავალიანებები';ru='Долги'"), НСтр("ka='საერთო დავალიანება';ru='Общий долг'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_Расчеты", "Отчет.Взаиморасчеты", "Остатки",, "Покупатели");
	ДобавитьПоказатель("Долги", "ДолгиНам", НСтр("ka='დავალიანებები';ru='Долги'"), НСтр("ka='ვალები ჩვენ';ru='Долги нам'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_Расчеты", "Отчет.Взаиморасчеты", "Остатки",, "Покупатели");
	ДобавитьПоказатель("Долги", "ДолгиНаши", НСтр("ka='დავალიანებები';ru='Долги'"), НСтр("ka='ჩვენი ვალები';ru='Долги наши'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_Расчеты", "Отчет.Взаиморасчеты", "Остатки",, "Поставщики");

	// Фин. анализ
	ДобавитьПоказатель("ФинАнализ", "ЧистыеАктивы", НСтр("ka='ფინანსური ანალიზი';ru='Фин. анализ'"), НСтр("ka='სუფთა აქტივები';ru='Чистые активы'"), Истина, Истина, "ЧДЦ=2; ЧН=-", "СКД_РБ_Управленческий", "Отчет.ЧистыеАктивы", "ЧистыеАктивы");
	...
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Сумма", НСтр("ka='გაყიდვების თანხა';ru='Сумма продаж'"), НСтр("ka='გაყიდვები';ru='Продажи'"), ТипДиаграммы.Гистограмма, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Количество", НСтр("ka='გაყიდული საქონლის რაოდენობა';ru='Количество проданных товаров'"),, ТипДиаграммы.Гистограмма);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "КоличествоДокументов", НСтр("ka='გაყიდვების რაოდენობა (ჩეკების)';ru='Количество продаж (чеков)'"),, ТипДиаграммы.Гистограмма);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Прибыль", НСтр("ka='მოგება';ru='Прибыль'"),, ТипДиаграммы.Гистограмма, Истина);
	МассивПредставлений = Новый Массив;
	МассивПредставлений.Добавить(НСтр("ka='თვითღირებულება';ru='Себестоимость'"));
	МассивПредставлений.Добавить(НСтр("ka='მოგება';ru='Прибыль'"));
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "ПрибыльИСебестоимость", МассивПредставлений, НСтр("ka='თვითღირებულება და მოგება';ru='Себестоимость и прибыль'"), ТипДиаграммы.ГистограммаСНакоплением, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "ВозвратыСумма", НСтр("ka='დაბრუნებები (თანხა)';ru='Возвраты (сумма)'"),, ТипДиаграммы.Гистограмма, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "ВозвратыКоличество", НСтр("ka='დაბრუნებები (რაოდენობა)';ru='Возвраты (количество)'"),, ТипДиаграммы.Гистограмма);
	СтруктураТочек = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "День", НСтр("ka='დღეები';ru='Дни'"),,,, "ДЛФ=D");
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Неделя", НСтр("ka='კვირა';ru='Недели'"),,,, "ДЛФ=D");
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Месяц", НСтр("ka='თვეები';ru='Месяцы'"),,,, "ДФ='МММ гггг'");
	ДобавитьДиаграмму("ДинамикаПродаж", НСтр("ka='გაყიდვების დინამიკა';ru='Динамика продаж'"), СтруктураСерий, СтруктураТочек, Ложь, "СКД_РН_Продажи", "Отчет.Продажи", "Основной");  
	
	// Диаграмма "Структура продаж"
	СтруктураСерий = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "НоменклатураРодитель", НСтр("ka='Группы товаров';ru='Группы товаров'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "КатегорияНоменклатуры", НСтр("ka='ნომენკლატურის კატეგორიები';ru='Категории номенклатуры'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Склад", НСтр("ka='საწყობები';ru='Склады'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Номенклатура", НСтр("ka='საქონელი';ru='Товары'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Контрагент", НСтр("ka='მყიდველები';ru='Покупатели'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Организация", НСтр("ka='ორგანიზაციები';ru='Организации'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Подразделение", НСтр("ka='ქვეგანყოფილებები';ru='Подразделения'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Ответственный", НСтр("ka='მენეჯერები';ru='Менеджеры'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Валюта", НСтр("ka='ვალუტა';ru='Валюта'"),, ТипДиаграммы.Круговая);
	...
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "СвойствоНоменклатуры", НСтр("ka='ნომენკლატურის დამატებითი რეკვიზიტები';ru='Доп. реквизиты номенклатуры'"),, ТипДиаграммы.Круговая,,,, СтруктураНастроек);
	СтруктураОтбора = Новый Соответствие;
	...
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "СвойствоПокупателя", НСтр("ka='მყიდველების დამატებითი რეკვიზიტები';ru='Доп. реквизиты покупателей'"),, ТипДиаграммы.Круговая,,,, СтруктураНастроек);
	СтруктураТочек = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Сумма", НСтр("ka='გაყიდვების თანხა';ru='Сумма продаж'"), НСтр("ka='გაყიდვები';ru='Продажи'"),, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Количество", НСтр("ka='გაყიდული საქონლის რაოდენობა';ru='Количество проданных товаров'"));
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "КоличествоДокументов", НСтр("ka='გაყიდვების რაოდენობა (ჩეკების)';ru='Количество продаж (чеков)'"));
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Прибыль", НСтр("ka='მოგება';ru='Прибыль'"),,, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "ВозвратыСумма", НСтр("ka='დაბრუნებები (თანხა)';ru='Возвраты (сумма)'"),,, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "ВозвратыКоличество", НСтр("ka='დაბრუნებები (რაოდენობა)';ru='Возвраты (количество)'"));
	ДобавитьДиаграмму("СтруктураПродаж", НСтр("ka='გაყიდვების სტრუქტურა';ru='Структура продаж'"), СтруктураСерий, СтруктураТочек, Ложь, "СКД_РН_Продажи", "Отчет.Продажи", "Основной");  
	
	// Диаграмма "Структура розничных продаж"
	СтруктураСерий = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "НоменклатураРодитель", НСтр("ka='საქონლის ჯგუფები';ru='Группы товаров'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "КатегорияНоменклатуры", НСтр("ka='ნომენკლატურის კატეგორიები';ru='Категории номенклатуры'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Склад", НСтр("ka='საწყობები';ru='Склады'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Номенклатура", НСтр("ka='საქონელი';ru='Товары'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Организация", НСтр("ka='ორგანიზაციები';ru='Организации'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Подразделение", НСтр("ka='ქვეგანყოფილებები';ru='Подразделения'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "КассаККМ", НСтр("ka='სალარო საკონტროლო-საკასო აპარატი';ru='Касса ККМ'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Ответственный", НСтр("ka='მენეჯერები';ru='Менеджеры'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Валюта", НСтр("ka='ვალუტა';ru='Валюта'"),, ТипДиаграммы.Круговая);
	СтруктураОтбора = Новый Соответствие;
	СтруктураОтбора.Вставить("Отбор.НаборСвойств", НаборыСвойствНоменклатура);
	...
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "ВозвратыСумма", НСтр("ka='დაბრუნებები (თანხა)';ru='Возвраты (сумма)'"),,, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "ВозвратыКоличество", НСтр("ka='დაბრუნებები (რაოდენობა)';ru='Возвраты (количество)'"));
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "КоличествоЧеков", НСтр("ka='ჩეკების რაოდენობა';ru='Количество чеков'"));
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "СреднийЧек", НСтр("ka='საშუალო ჩეკი';ru='Средний чек'"));
	ДобавитьДиаграмму("СтруктураРозничныхПродаж", НСтр("ka='საცალო გაყიდვების სტრუქტურა';ru='Структура розничных продаж'"), СтруктураСерий, СтруктураТочек, Ложь, "СКД_Розница", "Отчет.РозничныеПродажи", "РозничныеПродажиКонтекст");  
	
	// Диаграмма "Динамика денег"
	СтруктураСерий = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "СуммаОстаток", НСтр("ka='ნაშთი';ru='Остаток'"), НСтр("ka='ფულის ნაშთი';ru='Остаток денег'"), ТипДиаграммы.Гистограмма, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "СуммаПриход", НСтр("ka='შემოსვლები';ru='Поступления'"), НСтр("ka='თანხის შემოსვლა';ru='Поступления денег'"), ТипДиаграммы.Гистограмма, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "СуммаРасход", НСтр("ka='Списания';ru='Списания'"), НСтр("ka='ფულის ჩამოწერა';ru='Списания денег'"), ТипДиаграммы.Гистограмма, Истина);
	СтруктураТочек = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "День", НСтр("ka='დღეები';ru='Дни'"),,,, "ДЛФ=D");
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Неделя", НСтр("ka='კვირა';ru='Недели'"),,,, "ДЛФ=D");
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Месяц", НСтр("ka='თვეები';ru='Месяцы'"),,,, "ДФ='МММ гггг'");
	ДобавитьДиаграмму("ДинамикаДенег", НСтр("ka='ფული (დინამიკა)';ru='Деньги (динамика)'"), СтруктураСерий, СтруктураТочек, Ложь, "СКД_РН_ДенежныеСредства", "Отчет.ДенежныеСредства", "Ведомость");  
	
	// Диаграмма "Структура денег"
	СтруктураСерий = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "БанковскийСчетКасса", НСтр("ka='საბანკო ანგარიში / სალარო';ru='Банковский счет / касса'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Валюта", НСтр("ka='ვალუტა';ru='Валюта'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Организация", НСтр("ka='ორგანიზაცია';ru='Организация'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "ТипДенежныхСредств", НСтр("ka='ფულადი სახსრების ტიპი';ru='Тип денежных средств'"),, ТипДиаграммы.Круговая);
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "ХозяйственнаяОперация", НСтр("ka='ბუღ. ოპერაცია';ru='Хоз. операция'"),, ТипДиаграммы.Круговая,,, "СуммаПриход,СуммаРасход");
	СтруктураОтбора = Новый Соответствие;
	...
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Аналитика", НСтр("ka='ანალიტიკა';ru='Аналитика'"),, ТипДиаграммы.Круговая,,, "СуммаПриход,СуммаРасход", СтруктураНастроек);
	СтруктураТочек = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "СуммаОстаток", НСтр("ka='ნაშთი';ru='Остаток'"), НСтр("ka='Остаток денег';ru='ფულის ნაშთი'"),, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "СуммаПриход", НСтр("ka='შემოსვლები';ru='Поступления'"), НСтр("ka='ფულის შემოსვლა';ru='Поступления денег'"),, Истина);
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "СуммаРасход", НСтр("ka='ჩამოწერა';ru='Списания'"), НСтр("ka='ფულის ჩამოწერა';ru='Списания денег'"),, Истина);
	ДобавитьДиаграмму("СтруктураДенег", НСтр("ka='ფულის სტრუქტურა';ru='Структура денег'"), СтруктураСерий, СтруктураТочек, Ложь, "СКД_РН_ДенежныеСредстваДвижения", "Отчет.ДенежныеСредства", "Ведомость");  
	
	// Диаграмма "Динамика активов"
	СтруктураСерий = Новый Структура;
	МассивПредставлений = Новый Массив;
	МассивПредставлений.Добавить(НСтр("ka='ვალები ჩვენ';ru='Долги нам'"));
	МассивПредставлений.Добавить(НСтр("ka='ჩვენი ვალი';ru='Наши долги'"));
	МассивПредставлений.Добавить(НСтр("ka='ფული';ru='Деньги'"));
	МассивПредставлений.Добавить(НСтр("ka='საქონელი';ru='Товары'"));
	МассивПредставлений.Добавить(НСтр("ka='ქონება';ru='Имущество'"));
	ДобавитьОписаниеСерииТочки(СтруктураСерий, "Сумма", МассивПредставлений, НСтр("ka='სუფთა აქტივები';ru='Чистые активы'"), ТипДиаграммы.ГрафикСОбластямиИНакоплением, Истина);
	СтруктураТочек = Новый Структура;
	ДобавитьОписаниеСерииТочки(СтруктураТочек, "Время", НСтр("ka='დრო';ru='Время'"),,,, "ДЛФ=D");
	ДобавитьДиаграмму("ДинамикаАктивов", НСтр("ka='წმინდა აქტივები (დინამიკა)';ru='Чистые активы (динамика)'"), СтруктураСерий, СтруктураТочек, Ложь, "СКД_РБ_Управленческий", "Отчет.ЧистыеАктивы", "ЧистыеАктивы", Истина);  
...
Процедура ПоказателиПоУмолчанию()
	...
	Если ПолучитьФункциональнуюОпцию("УчетРозничныхПродаж") Тогда
		...
		ОтобразитьПоказатель("Розница", "СуммаОстаток", НСтр("ka='სსა სალაროებში';ru='В кассах ККМ'"));
		...
		ОтобразитьПоказатель("Деньги", "Платежи", НСтр("ka='დახარჯული ფული';ru='Потрачено денег'")); 
	...
	Иначе
		...
		ДобавленныеПоказатели.Очистить();
		ОтобразитьПоказатель("Деньги", "СуммаОстаток");
		ОтобразитьПоказатель("Товары", "СуммаОстаток", НСтр("ka='საქონელი';ru='Товары'"));
		ОтобразитьПоказатель("Долги", "ДолгиНам");
		ОтобразитьПоказатель("Долги", "ДолгиНаши");
		ОтобразитьПоказатель("ФинАнализ", "ЧистыеАктивы", НСтр("ka='სულ წმინდა აქტივები';ru='Итого чистые активы'"));
		ОтобразитьПоказатель("Продажи", "Выручка", НСтр("ka='გაყიდვები';ru='Продажи'"));
		ОтобразитьПоказатель("Деньги", "Поступления", НСтр("ka='შემოსვლები';ru='Поступления'"));
		ОтобразитьПоказатель("Деньги", "Платежи", НСтр("ka='გადახდები';ru='Платежи'"));
...
Процедура ДиаграммыПоУмолчанию()
	...
	Если ПолучитьФункциональнуюОпцию("УчетРозничныхПродаж") Тогда
		...
		"День", 
		ПериодДиаграмм,, 
		НСтр("ka='გაყიდვების დინამიკა';ru='Динамика продаж'"),
		МассивФильтров);
		...
		"СуммаРасход", 
		ПериодДиаграмм,, 
		НСтр("ka='ფულის ჩამოწერის სტრუქტურა';ru='Структура списания денег'"), 
		МассивФильтров);
		...
		"Сумма", 
		ПериодДиаграмм,, 
		НСтр("ka='გაყიდვები კატეგორიების მიხედვით';ru='Продажи по категориям'"));
		...
		"Сумма", 
		ПериодДиаграмм,, 
		НСтр("ka='გაყიდვები წერტილების მიხედვით';ru='Продажи по точкам'"));
		...
		"СреднийЧек", 
		ПериодДиаграмм,, 
		НСтр("ka='საშუალო ჩეკი წერტილების მიხედვით';ru='Средний чек по точкам'"));
