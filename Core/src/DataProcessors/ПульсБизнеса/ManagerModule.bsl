...
Процедура ВыполнитьЗапросПоказатели(Запрос, Параметры, Результат)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='ბიზნესის პულსი: მაჩვენებლების გათვლის შეცდომა';ru='Пульс бизнеса: ошибка расчета показателя'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьЗапросДиаграммы(Запрос, ПараметрыЗапросов, Результат)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='ბიზნესის პულსი: დიაგრამის მონაცემების მიღების შეცდომა';ru='Пульс бизнеса: ошибка получения данных диаграммы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());
		Возврат;
	КонецПопытки; 
