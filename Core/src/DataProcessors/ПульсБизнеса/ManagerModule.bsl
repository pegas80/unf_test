...
Процедура ВыполнитьЗапросПоказатели(Запрос, Параметры, Результат)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='ბიზნესის პულსი: მაჩვენებლების გათვლის შეცდომა';ru='Пульс бизнеса: ошибка расчета показателя';en='Puls Biznesu: calculation error indicator'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьЗапросДиаграммы(Запрос, ПараметрыЗапросов, Результат)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='ბიზნესის პულსი: დიაგრამის მონაცემების მიღების შეცდომა';ru='Пульс бизнеса: ошибка получения данных диаграммы';en='Puls Biznesu: error getting the chart data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());
		Возврат;
	КонецПопытки; 
