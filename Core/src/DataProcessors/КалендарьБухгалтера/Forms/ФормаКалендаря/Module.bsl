...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ДляОрганизации) Тогда
		...
		Сообщение.Текст = НСтр("ka='Необходимо выбрать организацию, для составления календаря.';ru='Необходимо выбрать организацию, для составления календаря.';en='You must select an organization for drawing up the calendar.'");
	...
	Если СрокСдачи = КонецГода(СрокСдачи) Или СрокСдачи = НачалоДня(КонецГода(СрокСдачи)) Тогда
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							  НСтр("ka='Невозможно определить ближайшую рабочую дату для даты %1, возможно, календарь не заполнен.';ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, календарь не заполнен.';en='Unable to determine the closest date to the date of work% 1, may calendar is empty.'"), 
