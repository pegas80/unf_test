...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ДляОрганизации) Тогда
		...
		Сообщение.Текст = НСтр("ka='Необходимо выбрать организацию, для составления календаря.';ru='Необходимо выбрать организацию, для составления календаря.'");
	...
	Если СрокСдачи = КонецГода(СрокСдачи) Или СрокСдачи = НачалоДня(КонецГода(СрокСдачи)) Тогда
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							  НСтр("ka='Невозможно определить ближайшую рабочую дату для даты %1, возможно, календарь не заполнен.';ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, календарь не заполнен.'"), 
