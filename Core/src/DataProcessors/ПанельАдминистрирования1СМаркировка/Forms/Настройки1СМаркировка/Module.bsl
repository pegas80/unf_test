...
Процедура ПроверитьВерсиюНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.';ru='Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.';en='Unable to get GISM server version, you may need to update the configuration.'"));
	КонецЕсли;
...
Процедура ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(Задание, ТекстРасписания, РасписаниеАктивно) Экспорт
	...
	Если Задание = Неопределено Тогда
		...
		ТекстРасписания = НСтр("ka='<Расписание не задано>';ru='<Расписание не задано>';en='<Schedule is not specified>'");
		
	Иначе
		
		Если Задание.Использование Тогда
			РасписаниеАктивно = Истина;
			ТекстРасписания = СтрШаблон(НСтр("ka='Расписание: %1';ru='Расписание: %1';en='Schedule:% 1'"), Строка(Задание.Расписание));
		Иначе
			ТекстРасписания = СтрШаблон(НСтр("ka='Расписание (НЕ АКТИВНО): %1';ru='Расписание (НЕ АКТИВНО): %1';en='Schedule (inactive): 1%'"), Строка(Задание.Расписание));
		КонецЕсли;
...
Процедура ОбновитьВерсии()
	...
	Если Не ВерсияАктуальна Тогда
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ka='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)';ru='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)';en='The current version of the configuration does not support the current exchange format GISM (1%)'"), ВерсииСхемОбмена.Сервер);
	Иначе
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ka='Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)';ru='Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)';en='Communication with GISM performed using the current exchange format (1%)'"), ВерсииСхемОбмена.Сервер);
