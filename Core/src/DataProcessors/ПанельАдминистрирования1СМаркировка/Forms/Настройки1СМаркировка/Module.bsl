...
Процедура ПроверитьВерсиюНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.';ru='Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.'"));
	КонецЕсли;
...
Процедура ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(Задание, ТекстРасписания, РасписаниеАктивно) Экспорт
	...
	Если Задание = Неопределено Тогда
		...
		ТекстРасписания = НСтр("ka='<Расписание не задано>';ru='<Расписание не задано>'");
		
	Иначе
		
		Если Задание.Использование Тогда
			РасписаниеАктивно = Истина;
			ТекстРасписания = СтрШаблон(НСтр("ka='Расписание: %1';ru='Расписание: %1'"), Строка(Задание.Расписание));
		Иначе
			ТекстРасписания = СтрШаблон(НСтр("ka='Расписание (НЕ АКТИВНО): %1';ru='Расписание (НЕ АКТИВНО): %1'"), Строка(Задание.Расписание));
		КонецЕсли;
...
Процедура ОбновитьВерсии()
	...
	Если Не ВерсияАктуальна Тогда
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ka='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)';ru='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)'"), ВерсииСхемОбмена.Сервер);
	Иначе
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ka='Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)';ru='Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)'"), ВерсииСхемОбмена.Сервер);
