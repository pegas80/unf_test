...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ОтключеноОбновлениеЧерезИнтернет", НСтр("ka='(отключено обновление через интернет)';ru='(отключено обновление через интернет)';en='(Disabled update via the Internet)'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_СоздаютсяНапрямуюВСправочнике", НСтр("ka='Создаются напрямую в справочнике';ru='Создаются напрямую в справочнике';en='Created directly in the directory'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяИзФайла", НСтр("ka='Загружаются из файла';ru='Загружаются из файла';en='Loaded from file'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяЧерезИнтернетПоПротоколу", НСтр("ka='Загружаются через интернет по протоколу';ru='Загружаются через интернет по протоколу';en='Downloaded over the Internet protocol'"));

	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_СписокЗначенийКлассификатора", НСтр("ka='Список значений классификатора';ru='Список значений классификатора';en='The list of values ​​of the classifier'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ТребуетсяОбновление", НСтр("ka='Требуется обновление';ru='Требуется обновление';en='update required'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеОтсутствуют", НСтр("ka='Данные не требуют обновления';ru='Данные не требуют обновления';en='These do not require updates'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеАктуальны", НСтр("ka='Данные актуальны';ru='Данные актуальны';en='These are relevant'"));
...
"или закройте окно вручную, до выхода из программы.';ka='Данные не сохранены."
"Нажмите кнопку [Записать и закрыть]"
...
Процедура ДекорацияУдалитьНовостиОбработкаНавигационнойСсылки(Элемент, лкНавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(лкНавигационнаяСсылка) = ВРег("DeleteNow") Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Удалить устаревшие новости?';ru='Удалить устаревшие новости?';en='Remove outdated news?'"),
			РежимДиалогаВопрос.ДаНет,
			0,
			КодВозвратаДиалога.Нет,
			НСтр("ka='Удаление устаревших новостей вручную';ru='Удаление устаревших новостей вручную';en='Removal of outdated news manually'"));
	КонецЕсли;
...
Процедура КомандаЗагрузитьНовости(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		...
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ka='Загрузка новостей';ru='Загрузка новостей';en='Loading news'"),
			НСтр("ru='Происходит загрузка новостей"
"Это может занять некоторое время...';ka='Происходит загрузка новостей"
...
Процедура КомандаЗагрузитьСтандартныеСписки(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		"Обработка.УправлениеНовостями.Форма.ФормаСостоянияОперации",
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ka='Обновление классификаторов с сервера';ru='Обновление классификаторов с сервера';en='Update of classifiers from a server'"),
			НСтр("ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время...';ka='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время...'")),
		ЭтотОбъект,
...
"Это может занять некоторое время..."
""
"Обновление дат актуальности классификаторов';ka='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
...
"Это может занять некоторое время..."
""
"Обновление классификаторов';ka='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
...
""
"Данные получены."
"Обновление классификаторов на клиенте...';ka='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Данные получены."
...
""
"Данные получены."
"Обновление лент новостей на клиенте...';ka='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Данные получены."
...
Процедура КомандаПолучитьОтладочнуюИнформациюОНовости(Команда)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.УИННовости) Тогда
		...
		#Если ВебКлиент Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("ka='Отладочная информация о новости';ru='Отладочная информация о новости';en='Debugging information about news'"));
		...
		#Иначе
			...
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ИнформацияОНовости);
			ТекстовыйДокумент.Показать(НСтр("ka='Отладочная информация о новости';ru='Отладочная информация о новости';en='Debugging information about news'"), ИмяВременногоФайла);
...
"'"),
	...
	Для Каждого ТекущийШаг Из СписокШагов Цикл
		Состояние(
			НСтр("ka='Выполнение проверки';ru='Выполнение проверки';en='Performing checks'"),
...
"'"),
	...
	#Если ВебКлиент Тогда
		...
		ТекстовыйДокумент.Показать(НСтр("ka='Результат диагностики получения новостей';ru='Результат диагностики получения новостей';en='The result of the diagnosis receive news'"));
	...
	#Иначе
		...
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.УстановитьТекст(РезультатДиагностикиПолученияНовостей);
		ТекстовыйДокумент.Показать(НСтр("ka='Результат диагностики получения новостей';ru='Результат диагностики получения новостей';en='The result of the diagnosis receive news'"), ИмяВременногоФайла);
...
Процедура УстановитьУсловноеОформление()
	...
	// 1. Частота обновления = 0 (обновлять вручную), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='განვაახლოთ მხოლოდ ხელით';ru='Обновлять только вручную';en='Update manually'"));
	...
	// 2. Частота обновления = 1 (Ежедневно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველდღე';ru='Ежедневно';en='Daily'"));
	...
	// 3. Частота обновления = 2 (Ежечасно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველ საათს';ru='Ежечасно';en='Hourly'"));
	...
	// 4. Частота обновления = 3 (Каждые 15 минут), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველ 15 წუთში';ru='Каждые 15 минут';en='Every 15 minutes'"));
	...
	// 5. Частота обновления = 4 (Каждую минуту), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველ წუთს';ru='Каждую минуту';en='Every minute'"));
	...
	// 6. Частота обновления = не требуется, т.к. это локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='არ საჭიროებს';ru='Не требуется';en='Not required'"));
	...
	// 7. Дата последнего обновления = пустая, то вывести текст "никогда не обновлялся".
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='никогда не обновлялся';ru='никогда не обновлялся';en='I never updated'"));
	...
	// 12. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ka='Данные актуальны';ru='Данные актуальны';en='These are relevant'");

		ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТребуетсяОбновление");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ka='Данные не требуют обновления';ru='Данные не требуют обновления';en='These do not require updates'");
	...
	// 13. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТребуетсяОбновление");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ka='Требуется обновление';ru='Требуется обновление';en='update required'");
...
Процедура ПослеОтветаУдалениеНовостей(РезультатВопроса, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ТекстСообщения = НСтр("ka='Результат выполнения:"
"%РезультатВыполнения%';ru='Результат выполнения:"
"%РезультатВыполнения%'");
...
"';ka='Подсистема Новости внедрена правильно"
"'");
		КонецЕсли;
...
Функция СформироватьПолныйАдресЛентыНовостей(лкСсылка, лкПротокол, лкСайт, лкИмяФайла)
	...
	Если Найти(ВРег(лкПротокол), ВРег("http")) = 1 Тогда // http, https
		Если ПустаяСтрока(лкСайт) Тогда
			Результат = НСтр("ka='ველი ""საიტი"" არ არის შევსებული';ru='Не заполнено поле Сайт';en='Field filled not Website'");
		ИначеЕсли ПустаяСтрока(лкИмяФайла) Тогда
			Результат = НСтр("ka='ველი ""ფაილის სახელი"" არ არის შევსებული';ru='Не заполнено поле Имя файла';en='Not filled the File name box'");
		...
		КонецЕсли;
	Иначе
		Результат = НСтр("ka='ახალი ამბების ლენტის მისამართის მიღება შეიძლება მხოლოდ http და https პროტოკოლებისთვის';ru='Адрес ленты новостей можно получить только для протоколов http и https';en='Address news feeds are available only for http and https protocols'");
	КонецЕсли;
		...
		ИначеЕсли ТипЗнч(ТекущаяИнформация.Значение) = ТипСтруктура Тогда // Новости
			...
			Если ТекущаяИнформация.Значение.Свойство("Новости") Тогда
				Если ТипЗнч(ТекущаяИнформация.Значение.Новости)= ТипМассив Тогда
					Если ТекущаяИнформация.Значение.Новости.Количество() > 0 Тогда
						...
						Для Каждого ТекущаяНовость Из ТекущаяИнформация.Значение.Новости Цикл
							...
							СписокПервыхНовостей = СписокПервыхНовостей
								...
								+ ?(ТекущаяНовость.Прочтена, НСтр("ka='წაკითხულია';ru='Прочтена';en='read'"), НСтр("ka='არ არის წაკითხული';ru='НЕ прочтена';en='without reading'")) + "; "
								+ Символы.ПС;
							С = С + 1;
							Если С >= 5 Тогда
								Прервать;
					...
					Иначе
						СписокПервыхНовостей = НСтр("ru='Пустой список контекстных новостей.';en='Empty list of contextual news.'");
					КонецЕсли;
				Иначе
					СписокПервыхНовостей = СтрШаблон(
						НСтр("ru='В реквизите Новости ключ массива контекстных новостей Новостей неправильного типа: %1.';en='The requisite array of key contextual News News News of the wrong type:% 1.'"),
						ТипЗнч(ТекущаяИнформация.Значение.Новости));
				КонецЕсли;
			Иначе
				СписокПервыхНовостей = НСтр("ru='В реквизите Новости отсутствует ключ массива контекстных новостей Новостей.';en='The news is not requisite array of key contextual News News.'");
			КонецЕсли;
			Результат = Результат
				+ СтрШаблон(
					НСтр("ru='Имя формы: %1
