...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ОтключеноОбновлениеЧерезИнтернет", НСтр("ka='(отключено обновление через интернет)';ru='(отключено обновление через интернет)'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_СоздаютсяНапрямуюВСправочнике", НСтр("ka='Создаются напрямую в справочнике';ru='Создаются напрямую в справочнике'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяИзФайла", НСтр("ka='Загружаются из файла';ru='Загружаются из файла'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяЧерезИнтернетПоПротоколу", НСтр("ka='Загружаются через интернет по протоколу';ru='Загружаются через интернет по протоколу'"));

	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_СписокЗначенийКлассификатора", НСтр("ka='Список значений классификатора';ru='Список значений классификатора'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ТребуетсяОбновление", НСтр("ka='Требуется обновление';ru='Требуется обновление'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеОтсутствуют", НСтр("ka='Данные не требуют обновления';ru='Данные не требуют обновления'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеАктуальны", НСтр("ka='Данные актуальны';ru='Данные актуальны'"));
...
Процедура ДекорацияУдалитьНовостиОбработкаНавигационнойСсылки(Элемент, лкНавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(лкНавигационнаяСсылка) = ВРег("DeleteNow") Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Удалить устаревшие новости?';ru='Удалить устаревшие новости?'"),
			РежимДиалогаВопрос.ДаНет,
			0,
			КодВозвратаДиалога.Нет,
			НСтр("ka='Удаление устаревших новостей вручную';ru='Удаление устаревших новостей вручную'"));
	КонецЕсли;
...
Процедура КомандаЗагрузитьНовости(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		...
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ka='Загрузка новостей';ru='Загрузка новостей'"),
...
Процедура КомандаЗагрузитьСтандартныеСписки(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		"Обработка.УправлениеНовостями.Форма.ФормаСостоянияОперации",
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ka='Обновление классификаторов с сервера';ru='Обновление классификаторов с сервера'"),
...
Процедура КомандаПолучитьОтладочнуюИнформациюОНовости(Команда)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.УИННовости) Тогда
		...
		#Если ВебКлиент Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("ka='Отладочная информация о новости';ru='Отладочная информация о новости'"));
		...
		#Иначе
			...
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ИнформацияОНовости);
			ТекстовыйДокумент.Показать(НСтр("ka='Отладочная информация о новости';ru='Отладочная информация о новости'"), ИмяВременногоФайла);
...
"'"),
	...
	Для Каждого ТекущийШаг Из СписокШагов Цикл
		Состояние(
			НСтр("ka='Выполнение проверки';ru='Выполнение проверки'"),
...
"'"),
	...
	#Если ВебКлиент Тогда
		...
		ТекстовыйДокумент.Показать(НСтр("ka='Результат диагностики получения новостей';ru='Результат диагностики получения новостей'"));
	...
	#Иначе
		...
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.УстановитьТекст(РезультатДиагностикиПолученияНовостей);
		ТекстовыйДокумент.Показать(НСтр("ka='Результат диагностики получения новостей';ru='Результат диагностики получения новостей'"), ИмяВременногоФайла);
...
Процедура УстановитьУсловноеОформление()
	...
	// 1. Частота обновления = 0 (обновлять вручную), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='განვაახლოთ მხოლოდ ხელით';ru='Обновлять только вручную'"));
	...
	// 2. Частота обновления = 1 (Ежедневно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველდღე';ru='Ежедневно'"));
	...
	// 3. Частота обновления = 2 (Ежечасно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველ საათს';ru='Ежечасно'"));
	...
	// 4. Частота обновления = 3 (Каждые 15 минут), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველ 15 წუთში';ru='Каждые 15 минут'"));
	...
	// 5. Частота обновления = 4 (Каждую минуту), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ყოველ წუთს';ru='Каждую минуту'"));
	...
	// 6. Частота обновления = не требуется, т.к. это локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='არ საჭიროებს';ru='Не требуется'"));
	...
	// 7. Дата последнего обновления = пустая, то вывести текст "никогда не обновлялся".
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='никогда не обновлялся';ru='никогда не обновлялся'"));
	...
	// 12. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ka='Данные актуальны';ru='Данные актуальны'");

		ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТребуетсяОбновление");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ka='Данные не требуют обновления';ru='Данные не требуют обновления'");
	...
	// 13. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТребуетсяОбновление");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ka='Требуется обновление';ru='Требуется обновление'");
...
Функция СформироватьПолныйАдресЛентыНовостей(лкСсылка, лкПротокол, лкСайт, лкИмяФайла)
	...
	Если Найти(ВРег(лкПротокол), ВРег("http")) = 1 Тогда // http, https
		Если ПустаяСтрока(лкСайт) Тогда
			Результат = НСтр("ka='ველი ""საიტი"" არ არის შევსებული';ru='Не заполнено поле Сайт'");
		ИначеЕсли ПустаяСтрока(лкИмяФайла) Тогда
			Результат = НСтр("ka='ველი ""ფაილის სახელი"" არ არის შევსებული';ru='Не заполнено поле Имя файла'");
		...
		КонецЕсли;
	Иначе
		Результат = НСтр("ka='ახალი ამბების ლენტის მისამართის მიღება შეიძლება მხოლოდ http და https პროტოკოლებისთვის';ru='Адрес ленты новостей можно получить только для протоколов http и https'");
	КонецЕсли;
					...
					Если ТекущаяИнформация.Значение.Новости.Количество() > 0 Тогда
						...
						Для Каждого ТекущаяНовость Из ТекущаяИнформация.Значение.Новости Цикл
							...
							СписокПервыхНовостей = СписокПервыхНовостей
								...
								+ ?(ТекущаяНовость.Прочтена, НСтр("ka='წაკითხულია';ru='Прочтена'"), НСтр("ka='არ არის წაკითხული';ru='НЕ прочтена'")) + "; "
								+ Символы.ПС;
							С = С + 1;
							Если С >= 5 Тогда
								Прервать;
