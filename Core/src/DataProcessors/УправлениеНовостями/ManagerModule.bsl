...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Начало';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Начало'"), // ИдентификаторШага
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Загрузка: Получение настроек, формирование списка лент новостей';ru='Загрузка: Получение настроек, формирование списка лент новостей'"),
			ТШаг,
...
"%ИнформацияОбОшибке%'");
								...
								ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
									НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения файла из метаданных';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения файла из метаданных'"), // ИдентификаторШага
							...
							Если ЕстьОшибки = Ложь Тогда
								Попытка
									Если ТипЗнч(ОбъектМетаданныхМакет) = ТипОбъектМетаданных Тогда
										...
										Иначе
											Макет = Неопределено;
											ТекстСообщения = НСтр("ka='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей% - эти метаданные не обрабатываются';ru='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей% - эти метаданные не обрабатываются'");
											ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
											ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
											// Запись в журнал регистрации
											ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
"Рекомендуется вручную удалить этот временный файл.'");
													...
													ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
														НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
														НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка удаления временного файла';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка удаления временного файла'"), // ИдентификаторШага
												...
												КонецПопытки;
											Иначе
												ТекстСообщения = НСтр("ka='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей%: возможна загрузка только из текстового макета';ru='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей%: возможна загрузка только из текстового макета'");
												ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
												ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
												// Запись в журнал регистрации
												ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
									...
									КонецЕсли;
								Исключение
									ТекстСообщения = НСтр("ka='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине %ИнформацияОбОшибке%';ru='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине %ИнформацияОбОшибке%'");
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
									// Запись в журнал регистрации
					...
					КонецЕсли;
				Исключение
					ТекстСообщения = НСтр("ka='Ошибка получения файла %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине %ИнформацияОбОшибке%';ru='Ошибка получения файла %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине %ИнформацияОбОшибке%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					// Запись в журнал регистрации
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
						НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения файла';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения файла'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
"(URL %URL%).'");
								...
								ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
									НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Начало загрузки';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Начало загрузки'"), // ИдентификаторШага
									УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
									, // ОбъектМетаданных
									, // Данные
									ТекстСообщения, // Комментарий
...
"Время загрузки (сек): %ВремяЗагрузки%'");
								...
								ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
									НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Конец загрузки';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Конец загрузки'"), // ИдентификаторШага
									УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
									, // ОбъектМетаданных
									, // Данные
									ТекстСообщения, // Комментарий
...
"%ПервыеСтрокиФайла%'");
									...
									ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
										НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
										НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Успешная проверка чтения временного файла после загрузки';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Успешная проверка чтения временного файла после загрузки'"), // ИдентификаторШага
										УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
										, // ОбъектМетаданных
										, // Данные
										ТекстСообщения, // Комментарий
...
"(URL %URL%).'");
									...
									ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
										НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
										НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Ошибка чтения временного файла после загрузки';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Ошибка чтения временного файла после загрузки'"), // ИдентификаторШага
										УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
										, // ОбъектМетаданных
										, // Данные
										ТекстСообщения, // Комментарий
						...
						Исключение
							...
							ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
								НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Ошибка загрузки';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Ошибка загрузки'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения, // Комментарий
...
"Успешно? %Успешно%'");
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Загрузка: помещение ссылок на хранилища в таблицу лент новостей';ru='Загрузка: помещение ссылок на хранилища в таблицу лент новостей'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Загрузка: обработка неактивных лент новостей';ru='Загрузка: обработка неактивных лент новостей'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Загрузка файлов новостей. Конец';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Начало';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл
		...
		ТШагЛентаНовостей = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ЕстьОшибки = Ложь;
		СтатусОбработки = НСтр("ka='Начало обработки';ru='Начало обработки'");

		Если (ТекущаяСтрока.Пометка) ИЛИ (ТекущаяСтрока.ОбязательныйКанал) Тогда
			СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Помечена для обработки';ru='Помечена для обработки'");
...
"%ПервыеСтрокиФайла%'");
						...
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
							НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка формата файла';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка формата файла'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
							, // ОбъектМетаданных
							, // Данные
							ТекстСообщения, // Комментарий
							ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
						СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Формат файла не xml, или неправильно сформированный xml';ru='Формат файла не xml, или неправильно сформированный xml'");
						// Пропустить ленту новостей
						ЕстьОшибки = Истина;
					КонецПопытки;
					...
					Если ЕстьОшибки = Ложь Тогда
						...
						ИначеЕсли (НРег(ХМЛТип.ИмяТипа) = "rss") Тогда // И (ВРег(ХМЛТип.URIПространстваИмен)=ВРег("http://www.w3.org/2005/RSS_2_0")).
							...
							Если (ОбъектХДТО.Свойства().Получить("version") = Неопределено) Тогда
								ТекстСообщения = НСтр("ka='Невозможно определить версию rss в ленте новостей %ЛентаНовостей% - отсутствует свойство version. Необходимы данные в формате версии 2.0.';ru='Невозможно определить версию rss в ленте новостей %ЛентаНовостей% - отсутствует свойство version. Необходимы данные в формате версии 2.0.'");
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
								// Запись в журнал регистрации
								ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
									НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка версии rss';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка версии rss'"), // ИдентификаторШага
									УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
									, // ОбъектМетаданных
									, // Данные
									ТекстСообщения, // Комментарий
									ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
								СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Неправильно сформированный xml: отсутствует свойство version для rss';ru='Неправильно сформированный xml: отсутствует свойство version для rss'");
								// Пропустить ленту новостей
								ЕстьОшибки = Истина;
							Иначе
								лкВерсия = ОбъектХДТО.Получить("version");
								Если (Число(лкВерсия) <> 2.0) Тогда
									ТекстСообщения = НСтр("ka='Неподдерживаемая версия новостей RSS %version% в ленте новостей %ЛентаНовостей%';ru='Неподдерживаемая версия новостей RSS %version% в ленте новостей %ЛентаНовостей%'");
									...
									ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
										...
										ТекстСообщения, // Комментарий
										ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
									СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Неправильно сформированный xml: свойство version для rss не равно 2.0';ru='Неправильно сформированный xml: свойство version для rss не равно 2.0'");
									// Пропустить ленту новостей
									ЕстьОшибки = Истина;
								КонецЕсли;
							КонецЕсли;
...
"%ПервыеСтрокиФайла%'");
							...
							ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
								...
								ТекстСообщения, // Комментарий
								ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
							СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Неправильно сформированный xml: ожидаются данные в формате rss (rss.channel) или atom (feed)';ru='Неправильно сформированный xml: ожидаются данные в формате rss (rss.channel) или atom (feed)'");
							// Пропустить ленту новостей.
							ЕстьОшибки = Истина;
						КонецЕсли;
...
"Рекомендуется вручную удалить этот временный файл.'");
						...
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
							НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка удаления временного файла';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка удаления временного файла'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
							, // ОбъектМетаданных
							, // Данные
							ТекстСообщения, // Комментарий
				...
				Иначе
					ТекстСообщения = НСтр("ka='Отсутствует загруженный файл %ИмяФайла% для ленты новостей %ЛентаНовостей%. Возможно, недостаточно места на диске, нет прав для записи в этот каталог или произошла какая-то другая ошибка.';ru='Отсутствует загруженный файл %ИмяФайла% для ленты новостей %ЛентаНовостей%. Возможно, недостаточно места на диске, нет прав для записи в этот каталог или произошла какая-то другая ошибка.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", СокрЛП(ИмяВременногоФайла));
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
					// Запись в журнал регистрации
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
						НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка чтения файла';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка чтения файла'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
						ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
					СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Не удалось сохранить файл из временного хранилища';ru='Не удалось сохранить файл из временного хранилища'");
					// Пропустить ленту новостей.
					ЕстьОшибки = Истина;
				КонецЕсли;

			Иначе
				ТекстСообщения = НСтр("ka='Не загружен файл новостей для ленты новостей %ЛентаНовостей% (пустой адрес в хранилище после загрузки файла [%ЛентаНовостей.ИмяФайла%] с сайта [%ЛентаНовостей.Сайт%] по протоколу [%ЛентаНовостей.Протокол%]). Лента новостей не обработана.';ru='Не загружен файл новостей для ленты новостей %ЛентаНовостей% (пустой адрес в хранилище после загрузки файла [%ЛентаНовостей.ИмяФайла%] с сайта [%ЛентаНовостей.Сайт%] по протоколу [%ЛентаНовостей.Протокол%]). Лента новостей не обработана.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка чтения файла из временного хранилища';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка чтения файла из временного хранилища'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.ЛентыНовостей, // ОбъектМетаданных
					ТекущаяСтрока.ЛентаНовостей, // Данные
					ТекстСообщения, // Комментарий
					ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
				СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Отсутствует файл для обработки (пустой адрес в хранилище)';ru='Отсутствует файл для обработки (пустой адрес в хранилище)'");
			КонецЕсли;

		Иначе
			СтатусОбработки = СтатусОбработки + ", " + НСтр("ka='Не помечена для обработки';ru='Не помечена для обработки'");
		КонецЕсли;
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Конец';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Вспомогательные мероприятия после парсинга файлов новостей. Начало';ru='Новости. Загрузка новостей. Вспомогательные мероприятия после парсинга файлов новостей. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
#Область ВспомогательныеОбработкиОсновные_КатегорииНовостейПростыеНеобработанные
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия, ПересчитатьНеобработанныеКатегорииНовостей';ru='Парсинг: вспомогательные мероприятия, ПересчитатьНеобработанныеКатегорииНовостей'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
#Область ВспомогательныеОбработкиОсновные_ОптимизацияОтборовПоНовостям
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия, ОптимизироватьОтборыПоНовостям';ru='Парсинг: вспомогательные мероприятия, ОптимизироватьОтборыПоНовостям'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_РедкоМеняющиеся
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия, ПересчитатьОтборыПоНовостям_РедкоМеняющиеся';ru='Парсинг: вспомогательные мероприятия, ПересчитатьОтборыПоНовостям_РедкоМеняющиеся'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_Общие
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия, ПересчитатьОтборыПоНовостям_Общие';ru='Парсинг: вспомогательные мероприятия, ПересчитатьОтборыПоНовостям_Общие'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
#Область ВспомогательныеОбработкиОсновные_ОбновлениеПериодическихСвойствНовостей
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия, ОбновлениеПериодическихСвойствНовостей';ru='Парсинг: вспомогательные мероприятия, ОбновлениеПериодическихСвойствНовостей'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
#Область ВспомогательныеОбработкиОсновные_ОбновлениеПривязокКМетаданным
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия, ОбновлениеПривязокКМетаданным';ru='Парсинг: вспомогательные мероприятия, ОбновлениеПривязокКМетаданным'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Вспомогательные мероприятия после парсинга файлов новостей. Конец';ru='Новости. Загрузка новостей. Вспомогательные мероприятия после парсинга файлов новостей. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Настроенные разработчиками мероприятия после парсинга файлов новостей. Начало';ru='Новости. Загрузка новостей. Настроенные разработчиками мероприятия после парсинга файлов новостей. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ka='Парсинг: вспомогательные мероприятия (переопределяемые), ДополнительноОбработатьНовостиПослеПолучения';ru='Парсинг: вспомогательные мероприятия (переопределяемые), ДополнительноОбработатьНовостиПослеПолучения'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Настроенные разработчиками мероприятия после парсинга файлов новостей. Конец';ru='Новости. Загрузка новостей. Настроенные разработчиками мероприятия после парсинга файлов новостей. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для каждого ТекущаяСтрока Из ТаблицаОбновляемыхДанных Цикл
			Если ТипЗнч(ТекущаяСтрока.Список) = Тип("ПланВидовХарактеристикСсылка.КатегорииНовостей") Тогда
				ТекстСообщения = НСтр("ka='Загрузка значений категории %Выборка.Список% (%Выборка.Список.Код%)';ru='Загрузка значений категории %Выборка.Список% (%Выборка.Список.Код%)'");
			...
			Иначе
				Если ТекущаяСтрока.Список = "Список лент новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("ka='Загрузка списка лент новостей';ru='Загрузка списка лент новостей'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли ТекущаяСтрока.Список = "Список категорий новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("ka='Загрузка списка категорий новостей';ru='Загрузка списка категорий новостей'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли Найти(ВРег(ТекущаяСтрока.Список), ВРег("Значения категории новостей:")) = 1 Тогда // Идентификатор.
					ТекстСообщения = НСтр("ka='Загрузка значений категории с кодом %Выборка.Список%';ru='Загрузка значений категории с кодом %Выборка.Список%'");
...
"%ТаблицаОбновляемыхДанных%'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='Новости. Сервис и регламент. ИнформацияОбОбновлении';ru='Новости. Сервис и регламент. ИнформацияОбОбновлении'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка классификаторов. Список значений категории. Ошибка';ru='Новости. Загрузка классификаторов. Список значений категории. Ошибка'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"%Данные%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка классификаторов. Список значений категории. Информация';ru='Новости. Загрузка классификаторов. Список значений категории. Информация'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка классификаторов. Список лент новостей. Ошибка';ru='Новости. Загрузка классификаторов. Список лент новостей. Ошибка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"%Данные%'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка классификаторов. Список лент новостей. Информация';ru='Новости. Загрузка классификаторов. Список лент новостей. Информация'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
				...
				Если ТипЗнч(СписокВсехДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Для Каждого ТекущийЭлементСписка Из СписокВсехЗагруженныхСерверныхЛентНовостей Цикл
						Если ТекущийЭлементСписка.Пометка = Ложь Тогда
							...
							ТекстСообщения = НСтр("ka='Лента новостей %ЛентаНовостейНаименование% помечена на удаление, т.к. она была отключена на сервере 1С';ru='Лента новостей %ЛентаНовостейНаименование% помечена на удаление, т.к. она была отключена на сервере 1С'");
							...
							ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
								...
								НСтр("ka='Новости. Все обновления новостей. Обновление классификаторов. Отключение ленты новостей';ru='Новости. Все обновления новостей. Обновление классификаторов. Отключение ленты новостей'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения, // Комментарий
...
"%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка классификаторов. Список категорий. Ошибка';ru='Новости. Загрузка классификаторов. Список категорий. Ошибка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
		...
		Если НЕ ПустаяСтрока(СписокУИН) Тогда
			...
			Если НЕ ПустаяСтрока(Владелец.ВерсияДанных) Тогда
				...
				Исключение
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
						НСтр("ka='Новости. Загрузка классификаторов. Список значений категории (по УИН). Ошибка';ru='Новости. Загрузка классификаторов. Список значений категории (по УИН). Ошибка'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
"%Данные%'");
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
						НСтр("ka='Новости. Загрузка классификаторов. Список значений категории (по УИН). Информация';ru='Новости. Загрузка классификаторов. Список значений категории (по УИН). Информация'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка классификаторов. Список обновлений классификаторов. Начало';ru='Новости. Загрузка классификаторов. Список обновлений классификаторов. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДатыОбновленияСписков%", НСтр("ka='Получено значение с типом значения, отличным от ОбъектXDTO';ru='Получено значение с типом значения, отличным от ОбъектXDTO'"));
	КонецЕсли;
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка классификаторов. Список обновлений классификаторов. Выполнение';ru='Новости. Загрузка классификаторов. Список обновлений классификаторов. Выполнение'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка классификаторов. Список обновлений классификаторов. Конец';ru='Новости. Загрузка классификаторов. Список обновлений классификаторов. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Начало';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"Определен формат ленты новостей: %ФорматНовостей%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
			НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Выполнение';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Выполнение'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		Если ТипЗнч(ОбъектХДТО.Свойства().Получить("channel")) = ТипСвойствоXDTO
			...
			Если ОбъектХДТО.Свойства().Получить("channel") <> Неопределено Тогда
				...
				Иначе
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
						НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			...
			Иначе
				// Запись в журнал регистрации
				ТекстСообщения = НСтр("ka='В файле полученном для ленты новостей %ЛентаНовостей% отсутствует обязательное поле channel. Файл не был обработан.';ru='В файле полученном для ленты новостей %ЛентаНовостей% отсутствует обязательное поле channel. Файл не был обработан.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Ошибка';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Ошибка'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Определен формат ленты новостей: %ФорматНовостей%'");
		...
		Иначе // updated - обязательное поле, наверное формат некорректный
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Предупреждение';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Предупреждение'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		РезультатЗагрузкиНовостей = НСтр("ka='Новости не загружены, т.к. передан необрабатываемый формат ленты новостей.';ru='Новости не загружены, т.к. передан необрабатываемый формат ленты новостей.'");
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Конец';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Конец'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Конец';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Конец'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
...
"по причине: %ИнформацияОбОшибке%'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Ошибка';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Ошибка'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
...
"Возможно, новость уже существует в базе с такой же датой публикации или (при отмене публикации) этой новости нет в базе.'");
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ФорматНовостей%", ФорматНовостей);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначениеПометкаУдаления%", ?(ЗначениеПометкаУдаления = Истина, НСтр("ka='на сервере публикация отменена';ru='на сервере публикация отменена'"), НСтр("ka='опубликована на сервере';ru='опубликована на сервере'")));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ka='в базе еще не создана';ru='в базе еще не создана'"), НСтр("ka='существует в базе';ru='существует в базе'")));
			// Запись в журнал регистрации
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Новость пропущена';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Новость пропущена'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
		...
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗаписыватьНовость%", ?(ЗаписыватьНовость = Истина, НСтр("ka='Будет записана';ru='Будет записана'"), НСтр("ka='Будет пропущена';ru='Будет пропущена'")));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ka='В базе еще не создана';ru='В базе еще не создана'"), НСтр("ka='Уже существует в базе';ru='Уже существует в базе'")));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ДатаПубликацииИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ka='(в базе не было)';ru='(в базе не было)'"), " (" + Формат(ДатаПубликацииИБ, "ДЛФ=DT") + ")"));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗначениеПометкаУдаленияИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ka='(в базе не было)';ru='(в базе не было)'"), " (" + ЗначениеПометкаУдаленияИБ + ")"));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%УИН_ИБ%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ka='(еще не создана)';ru='(еще не создана)'"), СтруктураНовости.УИН_ИБ));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%НавигационнаяСсылка%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ka='(еще не создана)';ru='(еще не создана)'"), ПолучитьНавигационнуюСсылку(СтруктураНовости.Ссылка)));
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				Данные = Неопределено;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = НСтр("ka='Не удалось загрузить бинарные данные для новости с УИН %ОбъектНовость.УИННовости% по причине: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%';ru='Не удалось загрузить бинарные данные для новости с УИН %ОбъектНовость.УИННовости% по причине: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "ОбъектНовость.УИННовости", ОбъектНовость.УИННовости);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка(Обновление)
	...
	Если ТипЗнч(Обновление) = ТипОбъектXDTO Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
				ТекстСообщения = НСтр("ka='Ошибка загрузки строки обновления списков: %ИнформацияОбОшибке%';ru='Ошибка загрузки строки обновления списков: %ИнформацияОбОшибке%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				// Запись в журнал регистрации
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ka='БИП:სიახლეები. სიახლეების ატვირთვა';ru='БИП:Новости.Загрузка новостей'"), // ИмяСобытия
					НСтр("ka='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Блок дат обновления списков';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Блок дат обновления списков'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			Если ТипЗнч(ОбъектХДТО.categoriesRefList.categoryRef) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
				...
				Пока БылиУдаления = Истина Цикл
					...
					Для каждого ТекущаяКатегория Из лкОбъект.ДоступныеКатегорииНовостей Цикл
						...
						Если КатегорияЕстьНаСервере = Ложь Тогда
							ТекстСообщения = НСтр("ka='Из ленты новостей %Наименование% удалена категория %Категория%.';ru='Из ленты новостей %Наименование% удалена категория %Категория%.'");
							...
							ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
								НСтр("ka='Новости. Загрузка классификаторов. Список лент новостей. Лента новостей';ru='Новости. Загрузка классификаторов. Список лент новостей. Лента новостей'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения, // Комментарий
				...
				Для каждого ТекущаяКатегория Из МассивДоступныхКатегорий Цикл
					...
					Если КатегорияЕстьНаКлиенте = Ложь Тогда
						ТекстСообщения = НСтр("ka='В ленте новостей %Наименование% добавлена категория %Категория%.';ru='В ленте новостей %Наименование% добавлена категория %Категория%.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Наименование%", СокрЛП(лкОбъект.Наименование));
...
"МестоположениеWSDL = %МестоположениеWSDL%'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
		НСтр("ka='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Начало';ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Попытка
		...
		Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
			ТекстСообщения = НСтр("ka='Неудачное подключение к веб-сервису обновления стандартных списков';ru='Неудачное подключение к веб-сервису обновления стандартных списков'");
			// Запись в журнал регистрации
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Ошибка';ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Ошибка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		Иначе
			ТекстСообщения = НСтр("ka='Успешное подключение к веб-сервису обновления стандартных списков';ru='Успешное подключение к веб-сервису обновления стандартных списков'");
			// Запись в журнал регистрации
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ka='БИП:Новости.Загрузка классификаторов';ru='БИП:Новости.Загрузка классификаторов'"), // ИмяСобытия
				НСтр("ka='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Конец';ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
