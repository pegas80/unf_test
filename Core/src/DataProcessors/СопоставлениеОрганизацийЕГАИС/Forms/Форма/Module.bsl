...
Процедура ОтменитьСоответствие(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Отменить связь выбранных организаций?';ru='Отменить связь выбранных организаций?'"), РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.ОК);
...
Процедура СоздатьКонтрагентов(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Создать контрагентов по выделенным строкам организаций ЕГАИС?';ru='Создать контрагентов по выделенным строкам организаций ЕГАИС?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура Подключаемый_СоздатьКонтрагентов()
	...
	Если СозданныеКонтрагенты.Количество() = 1 Тогда
		ТекстСообщения = НСтр("ka='Создан контрагент %Наименование%.';ru='Создан контрагент %Наименование%.'");
	...
	Иначе
		ТекстСообщения = НСтр("ka='Создано %Количество% контрагентов.';ru='Создано %Количество% контрагентов.'");
...
Функция ПроверитьВозможностьУстановкиСоответствия(ОрганизацияЕГАИС, Контрагент, ТекстОшибки)
	...
	Если Контрагент.ЭтоГруппа Тогда
		ТекстОшибки = НСтр("ka='Связка организации ЕГАИС с группой контрагентов невозможна.';ru='Связка организации ЕГАИС с группой контрагентов невозможна.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ka='Выбранная организация уже имеет связку.';ru='Выбранная организация уже имеет связку.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверитьВозможностьСозданияКонтрагентов(МассивОрганизацийЕГАИС, ТекстОшибки)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если МассивОрганизацийЕГАИС.Количество() > 1 Тогда
			ТекстОшибки = НСтр("ka='Все выбранные организации уже имеют связку.';ru='Все выбранные организации уже имеют связку.'");
		Иначе
			ТекстОшибки = НСтр("ka='Выбранная организация уже имеет связку.';ru='Выбранная организация уже имеет связку.'");
		КонецЕсли;
