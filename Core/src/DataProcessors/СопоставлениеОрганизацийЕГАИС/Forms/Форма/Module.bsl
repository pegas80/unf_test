...
Процедура ОтменитьСоответствие(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Отменить связь выбранных организаций?';ru='Отменить связь выбранных организаций?';en='Unlink the selected organizations?'"), РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.ОК);
...
Процедура СоздатьКонтрагентов(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Создать контрагентов по выделенным строкам организаций ЕГАИС?';ru='Создать контрагентов по выделенным строкам организаций ЕГАИС?';en='Create counterparties via dedicated lines EGAIS organizations?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура Подключаемый_СоздатьКонтрагентов()
	...
	Если СозданныеКонтрагенты.Количество() = 1 Тогда
		ТекстСообщения = НСтр("ka='Создан контрагент %Наименование%.';ru='Создан контрагент %Наименование%.';en='Created counterparty% Name%.'");
	...
	Иначе
		ТекстСообщения = НСтр("ka='Создано %Количество% контрагентов.';ru='Создано %Количество% контрагентов.';en='Created% Number% of contractors.'");
...
Функция ПроверитьВозможностьУстановкиСоответствия(ОрганизацияЕГАИС, Контрагент, ТекстОшибки)
	...
	Если Контрагент.ЭтоГруппа Тогда
		ТекстОшибки = НСтр("ka='Связка организации ЕГАИС с группой контрагентов невозможна.';ru='Связка организации ЕГАИС с группой контрагентов невозможна.';en='Bunch EGAIS organization with a group of contractors is not possible.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ka='Выбранная организация уже имеет связку.';ru='Выбранная организация уже имеет связку.';en='The selected organization already has a bunch.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверитьВозможностьСозданияКонтрагентов(МассивОрганизацийЕГАИС, ТекстОшибки)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если МассивОрганизацийЕГАИС.Количество() > 1 Тогда
			ТекстОшибки = НСтр("ka='Все выбранные организации уже имеют связку.';ru='Все выбранные организации уже имеют связку.';en='All selected organizations already have a bunch.'");
		Иначе
			ТекстОшибки = НСтр("ka='Выбранная организация уже имеет связку.';ru='Выбранная организация уже имеет связку.';en='The selected organization already has a bunch.'");
		КонецЕсли;
