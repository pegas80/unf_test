...
Процедура СбросНастроек(Команда)
	...
	ПоказатьОповещениеПользователя(НСтр("ka='კონტრაგენტების შემოწმება';ru='Проверка контрагентов';en='Checking counterparties'"),,НСтр("ka='კონტრაგენტების შემოწმების გამართვები გასუფთავებულია';ru='Настройки проверки контрагентов сброшены';en='Tuning counterparties cleared checks'"));
КонецПроцедуры
...
Процедура СбросНастроекНаСервере()
	...
	Если ЭтоМодельСервисаВнутриОбласти ИЛИ НеМодельСервиса Тогда
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='1. Очищены состояния контрагентов в данной области."
"2. Очищены состояния документов."
"3. Проверка не отключена. Для отключения Запустите сброс настроек повторно из неразделенной области.';ru='1. Очищены состояния контрагентов в данной области."
"2. Очищены состояния документов."
"3. Проверка не отключена. Для отключения Запустите сброс настроек повторно из неразделенной области.'"));
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='1. Очищены состояния контрагентов."
"2. Проверка отключена."
"3. Состояния документов не очищены. Данные по документам необходимо очищать из области данных."
"Запустите сброс настроек повторно из нужной области данных.';ru='1. Очищены состояния контрагентов."
"2. Проверка отключена."
"3. Состояния документов не очищены. Данные по документам необходимо очищать из области данных."
