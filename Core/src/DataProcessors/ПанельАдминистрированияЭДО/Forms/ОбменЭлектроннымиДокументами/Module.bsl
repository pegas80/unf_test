...
Процедура Подключаемый_ВыполнитьКомандуСоздатьШаблон(Команда)
	...
	ПредставлениеДокумента  = СокрЛП(СтрЗаменить(СтрЗаменить(Кнопка.Заголовок, НСтр("ka='შაბლონი თვის';ru='Шаблон для'"), ""), """", ""));
	НаименованиеШаблона     = СтрШаблон(НСтр("ka='Счет на оплату через Яндекс.Кассу (%1)';ru='Счет на оплату через Яндекс.Кассу (%1)'"), ПредставлениеДокумента);
...
Процедура ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(ТекстРасписания, ИдентификаторЗадания)
	...
	Если Использование Тогда
		ПрефиксРасписания = НСтр("ka='Расписание:%1%2';ru='Расписание:%1%2'");
		
	Иначе
		ПрефиксРасписания = НСтр("ka='Расписание (НЕ АКТИВНО):%1%2';ru='Расписание (НЕ АКТИВНО):%1%2'");
	КонецЕсли;
...
Процедура ДобавитьКомандыСозданияШаблона()
	...
	Для Каждого ТипОснованияПлатежа Из ТипыОснованияПлатежа Цикл
		...
		ПредставлениеКоманды = СтрШаблон(НСтр("ka='Шаблон для ""%1""';ru='Шаблон для ""%1""'"), ПредставлениеОбъектаМетаданных);
...
Процедура СоздатьШаблон(ПараметрыСозданияШаблона)
	...
	ДанныеЗаполнения.Вставить("Тема",                    НСтр("ka='Вам выставлен счет';ru='Вам выставлен счет'"));
