...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Объект.СозданныеДокументы.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ka='Перед завершением работы помощника требуется удалить созданные документы';ru='Перед завершением работы помощника требуется удалить созданные документы'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='წაშლა';ru='Удалить'"));
...
Процедура ТаблицаОстатковАлкогольнаяПродукцияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
		Если СтрокаТаблицы.Номенклатура.Пустая() Тогда
			...
			Прервать;
		Иначе
			ВызватьИсключение НСтр("ka='Выбранная алкогольная продукция имеет связку с другой номенклатурой.';ru='Выбранная алкогольная продукция имеет связку с другой номенклатурой.'");
		КонецЕсли;
	КонецЦикла;
...
Функция Подключаемый_СтраницаНачальныеДанные_ПриПереходеДалее(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОрганизацияЕГАИС) Тогда
		ТекстСообщения = НСтр("ka='Не выбрана организация ЕГАИС.';ru='Не выбрана организация ЕГАИС.'");
	...
	Если КорректироватьТекущиеОстатки = 0 Тогда
		Если НЕ ЗначениеЗаполнено(Объект.ОстаткиВРегистре1) Тогда
			ТекстСообщения = НСтр("ka='Не выбран документ запроса остатков по регистру №1.';ru='Не выбран документ запроса остатков по регистру №1.'");
		...
		Если НЕ ЗначениеЗаполнено(Объект.ОстаткиВРегистре2) Тогда
			ТекстСообщения = НСтр("ka='Не выбран документ запроса остатков по регистру №2.';ru='Не выбран документ запроса остатков по регистру №2.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ОстаткиВРегистре2",, Отказ);
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ТранспортныйМодуль) Тогда
		ТекстСообщения = НСтр("ka='Не удалось определить транспортный модуль.';ru='Не удалось определить транспортный модуль.'");
...
Функция Подключаемый_СтраницаОкончание_ПриПереходеНазад(Отказ)
	...
	Если Объект.СозданныеДокументы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Созданные документы будут удалены';ru='Созданные документы будут удалены'"),
			Кнопки);
	Иначе
...
Процедура УстановитьПредставлениеДатыОстатковЕГАИС()
	...
	ПредставлениеЭлемента = НСтр("ka='На дату получения остатков из ЕГАИС (%1)';ru='На дату получения остатков из ЕГАИС (%1)'");
	
	ДатаОстатковЕГАИС = ДатаОстатковЕГАИС();
	
	Если НЕ ЗначениеЗаполнено(ДатаОстатковЕГАИС) Тогда
		ПредставлениеЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеЭлемента, НСтр("ka='не определена';ru='не определена'"));
	Иначе
...
Процедура ПроверкаНезагруженныхДокументов_ПослеПолученияСпискаДокументов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Результат.Результат Тогда
		...
		СтрокиПроверки.Добавить(НСтр("ka='Ошибка получения списка входящих документов:';ru='Ошибка получения списка входящих документов:'"));
	...
	Иначе
		...
		Если СписокURL.Количество() > 0 Тогда
			...
			СтрокиПроверки = Новый Массив;
			СтрокиПроверки.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='В УТМ присутствуют незагруженные';ru='В УТМ присутствуют незагруженные'"),, ОбщегоНазначенияКлиент.ЦветСтиля("ПоясняющийТекст")));
			СтрокиПроверки.Добавить(" ");
			СтрокиПроверки.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='документы (';ru='документы ('") + СписокURL.Количество() + ")",,,, "ПоказатьНезагруженныеДокументы"));
			СтрокиПроверки.Добавить(". ");
			СтрокиПроверки.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='Выполнить обмен с ЕГАИС';ru='Выполнить обмен с ЕГАИС'"),,,, "ВыполнитьОбмен"));
...
Процедура ЗапроситьОстаткиЕГАИС()
	...
	Если Элементы.ПанельНеподтвержденныеДокументы.ТекущаяСтраница <> Элементы.СтраницаНеподтвержденныеДокументы_НетДокументов
		...
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Запросить остатки';ru='Запросить остатки'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена'"));
		
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗапросОстатковЕГАИС_Подтверждение", ЭтотОбъект),
			НСтр("ka='Обнаружены ошибки в ходе предварительной проверки данных ЕГАИС';ru='Обнаружены ошибки в ходе предварительной проверки данных ЕГАИС'"), Кнопки);
	...
	Если ПустаяСтрока(ТекстОшибкиПолученияОстатковЕГАИС) Тогда
		Если НЕ Объект.ОстаткиВРегистре1.Пустая() Тогда
			...
			Если СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОстатковЕГАИС.ОшибкаПолученияОстатков") Тогда
				...
				Если ПустаяСтрока(ТекстОшибкиПолученияОстатковЕГАИС) Тогда
					ТекстОшибкиПолученияОстатковЕГАИС = НСтр("ka='Ошибка получения остатков из ЕГАИС.';ru='Ошибка получения остатков из ЕГАИС.'");
				Иначе
					ТекстОшибкиПолученияОстатковЕГАИС = НСтр("ka='Ошибка получения остатков из ЕГАИС:';ru='Ошибка получения остатков из ЕГАИС:'") + Символы.ПС + ТекстОшибкиПолученияОстатковЕГАИС;
				КонецЕсли;
...
Процедура ПроверитьКорректностьОстатков(ПроверяемыеСтроки = Неопределено)
	...
	Если МассивСтрок.Количество() <> 0 Тогда
		Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
			...
			СтрокаТаблицы.ТекстОшибки = НСтр("ka='Не указан объем в декалитрах базовой единицы измерения.';ru='Не указан объем в декалитрах базовой единицы измерения.'");
	...
	Если МассивСтрок.Количество() <> 0 Тогда
		Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
			СтрокаТаблицы.ЕстьОшибка = Истина;
			СтрокаТаблицы.ТекстОшибки = НСтр("ka='Не указан признак продажи в розлив.';ru='Не указан признак продажи в розлив.'");
			СтрокаТаблицы.ИндексКартинкиСтрок = 1;
		КонецЦикла;
	КонецЕсли;
	...
	Если МассивСтрок.Количество() <> 0 Тогда
		Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
			СтрокаТаблицы.ЕстьОшибка = Истина;
			СтрокаТаблицы.ТекстОшибки = НСтр("ka='Номенклатура не сопоставлена с классификатором алкогольной продукции.';ru='Номенклатура не сопоставлена с классификатором алкогольной продукции.'");
			СтрокаТаблицы.ИндексКартинкиСтрок = 1;
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<не сопоставлено>';ru='<не сопоставлено>'"));
	...
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ДАЛ';ru='ДАЛ'"));
