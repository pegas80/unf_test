...
Процедура ОбновитьПредставлениеРасписания()
	...
	Если Не РазделениеВключено Тогда
		...
		Если ПредставлениеРасписания = ПредставлениеПустогоРасписания Тогда
			ПредставлениеРасписания = НСтр("ka='განრიგი მოცემული არ არის';ru='Расписание не задано'");
		КонецЕсли;
...
Процедура ВыбратьКаталогЗавершение1(Подключено, ДополнительныеПараметры) Экспорт
    ...
    Если Не Подключено Тогда
        ПоказатьПредупреждение(Неопределено,НСтр("ka='მოცემული ოპერაციისთვის აუცილებელია ფაილებთან მუშაობის გაფართოების დაყენება!';ru='Для данной операции необходимо установить расширение работы с файлами!'"));
        Возврат;
    КонецЕсли;
    
    Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
    
    Диалог.Заголовок = НСтр("ka='მიუთითეთ გაცვლის კატალოგი';ru='Укажите каталог обмена'");
...
Процедура ВыполнитьПроверкиПриНажатииГотово(Отказ)
	...
	Если Объект.ИспользоватьРегламентныеЗадания Тогда
		...
		Если Объект.РасписаниеРегламентногоЗадания = Неопределено
			...
			Сообщение = НСтр("ka='При использовании автоматического обмена расписание должно быть установлено!';ru='При использовании автоматического обмена расписание должно быть установлено!'");
...
Процедура СоздатьНовыйОбменДаннымиНаКлиенте(Отказ)
	...
	Состояние(НСтр("ka='Выполняется создание настройки обмена данными с web-сайтом';ru='Выполняется создание настройки обмена данными с web-сайтом'"));
	
	СоздатьНовыйОбменДаннымиНаСервере(КомпоновщикНастроекИнициализирован, Отказ);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(Неопределено,НСтр("ka='При создании настройки обмена данными возникли ошибки!';ru='При создании настройки обмена данными возникли ошибки!'"));
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(
			...
			НСтр("ka='Новый узел обмена с WEB - сайтом создан успешно!';ru='Новый узел обмена с WEB - сайтом создан успешно!'"),
...
Процедура СоздатьНовыйОбменДаннымиНаСервере(КомпоновщикНастроекИнициализирован, Отказ)
	...
	НоваяСтрока.Каталог = НСтр("ka='საქონლის ძირითადი კატალოგი';ru='Основной каталог товаров'");
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	Если ТекущийНомерСтраницы = 2 Тогда 
		...
		Если НЕ Объект.ОбменТоварами
			...
			Сообщение = НСтр("ka='Для продолжения настроек должен быть выбран хотя бы один из режимов обмена данными!';ru='Для продолжения настроек должен быть выбран хотя бы один из режимов обмена данными!'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда 
		...
		Если Объект.ВыгружатьНаСайт Тогда 
			...
			Если ПустаяСтрока(Объект.АдресСайта) Тогда 
				...
				Сообщение = НСтр("ka='Укажите адрес сайта';ru='Укажите адрес сайта'");
			...
			Если ПустаяСтрока(Объект.ИмяПользователя) Тогда 
				...
				Сообщение = НСтр("ka='Укажите имя пользователя';ru='Укажите имя пользователя'");
		...
		Иначе
			...
			Если ПустаяСтрока(Объект.КаталогВыгрузки) Тогда 
				...
				Сообщение = НСтр("ka='Укажите каталог выгрузки';ru='Укажите каталог выгрузки'");
			...
			Если Объект.ОбменЗаказами И ПустаяСтрока(Объект.КаталогЗагрузки) Тогда 
				...
				Сообщение = НСтр("ka='Укажите каталог загрузки';ru='Укажите каталог загрузки'");
	...
	ИначеЕсли ТекущийНомерСтраницы = 5 Тогда 
		...
		Если Объект.ОбменЗаказами Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.СпособИдентификацииКонтрагентов) Тогда
				...
				Сообщение = НСтр("ka='Укажите способ идентификации контрагентов';ru='Укажите способ идентификации контрагентов'");
			...
			ИначеЕсли НеСоздаватьКонтрагентовПриЗагрузкеЗаказа 
				...
				Сообщение = НСтр("ka='Выберите контрагента для подстановки в заказы';ru='Выберите контрагента для подстановки в заказы'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ РольДоступнаДобавлениеИзменениеОбменовДанными Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Недостаточно прав доступа!';ru='Недостаточно прав доступа!'"), Отказ);
		Возврат;
	...
	Если НЕ Параметры.Свойство("ВызовИзПланаОбмена") Тогда
		...
		Сообщение = НСтр("ka='Работа помощника поддерживается только при вызове из командного интерфейса! Работа помощника завершена.';ru='Работа помощника поддерживается только при вызове из командного интерфейса! Работа помощника завершена.'");
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Элементы.НастроитьРасписаниеРегламентногоЗадания.Заголовок = НСтр("ka='განრიგი მოცემული არ არის';ru='Расписание не задано'");
	Иначе
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Отменить настройку обмена данными с web-сайтом и выйти из помощника?';ru='Отменить настройку обмена данными с web-сайтом и выйти из помощника?'"), РежимДиалогаВопрос.ДаНет);
