...
Процедура ОбновитьПредставлениеРасписания()
	...
	Если Не РазделениеВключено Тогда
		...
		Если ПредставлениеРасписания = ПредставлениеПустогоРасписания Тогда
			ПредставлениеРасписания = НСтр("ka='განრიგი მოცემული არ არის';ru='Расписание не задано';en='Schedule is not specified'");
		КонецЕсли;
...
Процедура ВыбратьКаталогЗавершение1(Подключено, ДополнительныеПараметры) Экспорт
    ...
    Если Не Подключено Тогда
        ПоказатьПредупреждение(Неопределено,НСтр("ka='მოცემული ოპერაციისთვის აუცილებელია ფაილებთან მუშაობის გაფართოების დაყენება!';ru='Для данной операции необходимо установить расширение работы с файлами!';en='For this operation, you must install the extension to work with files!'"));
        Возврат;
    КонецЕсли;
    
    Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
    
    Диалог.Заголовок = НСтр("ka='მიუთითეთ გაცვლის კატალოგი';ru='Укажите каталог обмена';en='Specify the exchange directory'");
...
Процедура ВыполнитьПроверкиПриНажатииГотово(Отказ)
	...
	Если Объект.ИспользоватьРегламентныеЗадания Тогда
		...
		Если Объект.РасписаниеРегламентногоЗадания = Неопределено
			...
			Сообщение = НСтр("ka='При использовании автоматического обмена расписание должно быть установлено!';ru='При использовании автоматического обмена расписание должно быть установлено!';en='When using the automatic exchange of timetable should be set!'");
...
Процедура СоздатьНовыйОбменДаннымиНаКлиенте(Отказ)
	...
	Состояние(НСтр("ka='Выполняется создание настройки обмена данными с web-сайтом';ru='Выполняется создание настройки обмена данными с web-сайтом';en='You will create a data sharing settings with web-site'"));
	
	СоздатьНовыйОбменДаннымиНаСервере(КомпоновщикНастроекИнициализирован, Отказ);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(Неопределено,НСтр("ka='При создании настройки обмена данными возникли ошибки!';ru='При создании настройки обмена данными возникли ошибки!';en='When you create a data sharing settings any errors!'"));
		
	Иначе
		
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='""%1""';ru='""%1""'"), Объект.СсылкаНаУзелОбмена),,
			НСтр("ka='Новый узел обмена с WEB - сайтом создан успешно!';ru='Новый узел обмена с WEB - сайтом создан успешно!';en='The new exchange unit to the WEB - site created successfully!'"),
...
Процедура СоздатьНовыйОбменДаннымиНаСервере(КомпоновщикНастроекИнициализирован, Отказ)
	...
	НоваяСтрока.Каталог = НСтр("ka='საქონლის ძირითადი კატალოგი';ru='Основной каталог товаров';en='The main catalog'");
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	Если ТекущийНомерСтраницы = 2 Тогда 
		...
		Если НЕ Объект.ОбменТоварами
			...
			Сообщение = НСтр("ka='Для продолжения настроек должен быть выбран хотя бы один из режимов обмена данными!';ru='Для продолжения настроек должен быть выбран хотя бы один из режимов обмена данными!';en='To continue settings must be selected at least one of the modes of data exchange!'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда 
		...
		Если Объект.ВыгружатьНаСайт Тогда 
			...
			Если ПустаяСтрока(Объект.АдресСайта) Тогда 
				...
				Сообщение = НСтр("ka='Укажите адрес сайта';ru='Укажите адрес сайта';en='Enter the address of the site'");
			...
			Если ПустаяСтрока(Объект.ИмяПользователя) Тогда 
				...
				Сообщение = НСтр("ka='Укажите имя пользователя';ru='Укажите имя пользователя';en='Specify the user name'");
		...
		Иначе
			...
			Если ПустаяСтрока(Объект.КаталогВыгрузки) Тогда 
				...
				Сообщение = НСтр("ka='Укажите каталог выгрузки';ru='Укажите каталог выгрузки';en='Specify the upload directory'");
			...
			Если Объект.ОбменЗаказами И ПустаяСтрока(Объект.КаталогЗагрузки) Тогда 
				...
				Сообщение = НСтр("ka='Укажите каталог загрузки';ru='Укажите каталог загрузки';en='Specify the upload directory'");
	...
	ИначеЕсли ТекущийНомерСтраницы = 5 Тогда 
		...
		Если Объект.ОбменЗаказами Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.СпособИдентификацииКонтрагентов) Тогда
				...
				Сообщение = НСтр("ka='Укажите способ идентификации контрагентов';ru='Укажите способ идентификации контрагентов';en='Specify the method for the identification of counterparties'");
			...
			ИначеЕсли НеСоздаватьКонтрагентовПриЗагрузкеЗаказа 
				...
				Сообщение = НСтр("ka='Выберите контрагента для подстановки в заказы';ru='Выберите контрагента для подстановки в заказы';en='Select a counterparty to insert into orders'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ РольДоступнаДобавлениеИзменениеОбменовДанными Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Недостаточно прав доступа!';ru='Недостаточно прав доступа!';en='Insufficient access rights!'"), Отказ);
		Возврат;
	...
	Если НЕ Параметры.Свойство("ВызовИзПланаОбмена") Тогда
		...
		Сообщение = НСтр("ka='Работа помощника поддерживается только при вызове из командного интерфейса! Работа помощника завершена.';ru='Работа помощника поддерживается только при вызове из командного интерфейса! Работа помощника завершена.';en='Assistant Job is only supported when called from the command line interface! Work Assistant completed.'");
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Элементы.НастроитьРасписаниеРегламентногоЗадания.Заголовок = НСтр("ka='განრიგი მოცემული არ არის';ru='Расписание не задано';en='Schedule is not specified'");
	Иначе
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Отменить настройку обмена данными с web-сайтом и выйти из помощника?';ru='Отменить настройку обмена данными с web-сайтом и выйти из помощника?';en='Cancel data sharing settings with web-site and out of the Assistant?'"), РежимДиалогаВопрос.ДаНет);
