...
Процедура ОбновитьЗаголовокФормыНаСервере()
	...
	ЭтаФорма.Заголовок	= НСтр("ka='Прайс-листы поставщиков';ru='Прайс-листы поставщиков';en='Prices vendors'") + 
		?(ЗначениеЗаполнено(НаДату), Нстр("ru=' на ';ka='ზე'") + Формат(НаДату, "ДЛФ=DD"), Нстр("ka='.';ru='.';en='.'"));
...
"Возможно выбрана пустая ячейка.';ka='Невозможно скопировать цену."
"Возможно выбрана пустая ячейка.'")
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для копирования.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для копирования.';en='The current price list for this commodity item no prices available for copying.'")
						);
	...
	Если Расшифровка = Неопределено 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Возможно указана пустая ячейка."
"Копирование не возможно.';ru='Возможно указана пустая ячейка."
...
"Возможно выбрана пустая ячейка.';ka='Невозможно изменить цену."
"Возможно выбрана пустая ячейка.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для изменения.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для изменения.';en='The current price list for this commodity item no prices available for change.'")
						);
...
"Возможно выбрана пустая ячейка.';ka='Невозможно удалить цену."
"Возможно выбрана пустая ячейка.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для удаления.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для удаления.';en='The current price list for this commodity item no prices available for removal.'")
						);
...
Процедура История(Команда)
	...
	Если НЕ ТипЗнч(РасшифровкаИзОбласти) = Тип("Структура")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Невозможно отобразить историю формирования цен.';ru='Невозможно отобразить историю формирования цен.';en='Can not display the history of the formation of prices.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Невозможно отобразить историю формирорвания цен для данной номенклатурной позиции.';ru='Невозможно отобразить историю формирорвания цен для данной номенклатурной позиции.';en='Can not display the history formirorvaniya prices for this commodity item.'")
						);
