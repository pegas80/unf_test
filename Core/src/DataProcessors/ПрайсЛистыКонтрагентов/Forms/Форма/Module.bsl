...
Процедура ОбновитьЗаголовокФормыНаСервере()
	...
	ЭтаФорма.Заголовок	= НСтр("ka='Прайс-листы поставщиков';ru='Прайс-листы поставщиков'") + 
		?(ЗначениеЗаполнено(НаДату), Нстр("ru=' на ';ka='ზე'") + Формат(НаДату, "ДЛФ=DD"), Нстр("ka='.';ru='.'"));
...
"Возможно выбрана пустая ячейка.'")
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для копирования.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для копирования.'")
						);
...
"Возможно выбрана пустая ячейка.'")
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для изменения.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для изменения.'")
						);
...
"Возможно выбрана пустая ячейка.'")
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для удаления.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для удаления.'")
						);
...
Процедура История(Команда)
	...
	Если НЕ ТипЗнч(РасшифровкаИзОбласти) = Тип("Структура")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Невозможно отобразить историю формирования цен.';ru='Невозможно отобразить историю формирования цен.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Невозможно отобразить историю формирорвания цен для данной номенклатурной позиции.';ru='Невозможно отобразить историю формирорвания цен для данной номенклатурной позиции.'")
						);
