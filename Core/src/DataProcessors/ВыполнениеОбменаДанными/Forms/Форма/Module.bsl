...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Не заданы параметры формы. Форма не может быть открыта.';ru='Не заданы параметры формы. Форма не может быть открыта.';en='It does not specify the form. The form can not be opened.'"), Отказ);
			Возврат;
		КонецЕсли;
	...
	ШаблонЗаголовка = НСтр("ka='Синхронизация данных с ""%1""';ru='Синхронизация данных с ""%1""';en='Synchronization with ""%1""'");
...
"Повторите попытку синхронизации.';ka='Не удалось подключиться к приложению в Интернете, по причине ""%1""."
"Убедитесь, что:"
"- введен правильный пароль;"
"- приложение доступно по указанному в настройках адресу."
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	...
	Если ПараметрыЗадания.ВыполнятьЗагрузку Тогда
		...
		ШаблонНаименования = НСтр("ka='Выполняется загрузка данных из %1';ru='Выполняется загрузка данных из %1';en='Performed data loading of 1%'");
		
	Иначе
		ШаблонНаименования = НСтр("ka='Выполняется выгрузка данных в %1';ru='Выполняется выгрузка данных в %1';en='Unloading data is performed in 1%'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	ТиповоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Не удалось выполнить %1. Подробности см. в журнале регистрации';ru='Не удалось выполнить %1. Подробности см. в журнале регистрации';en='Unable to% 1. For details, see. In the log'"),
		?(ФоновоеЗаданиеВыполняемоеДействие = 1, НСтр("ka='получение данных';ru='получение данных';en='receiving data'"), НСтр("ka='отправку данных';ru='отправку данных';en='sending data'")));
