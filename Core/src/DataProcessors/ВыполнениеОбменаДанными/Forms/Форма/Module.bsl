...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ka='Не заданы параметры формы. Форма не может быть открыта.';ru='Не заданы параметры формы. Форма не может быть открыта.'"), Отказ);
			Возврат;
		КонецЕсли;
	...
	ШаблонЗаголовка = НСтр("ka='Синхронизация данных с ""%1""';ru='Синхронизация данных с ""%1""'");
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	...
	Если ПараметрыЗадания.ВыполнятьЗагрузку Тогда
		...
		ШаблонНаименования = НСтр("ka='Выполняется загрузка данных из %1';ru='Выполняется загрузка данных из %1'");
		
	Иначе
		ШаблонНаименования = НСтр("ka='Выполняется выгрузка данных в %1';ru='Выполняется выгрузка данных в %1'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	ТиповоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Не удалось выполнить %1. Подробности см. в журнале регистрации';ru='Не удалось выполнить %1. Подробности см. в журнале регистрации'"),
		?(ФоновоеЗаданиеВыполняемоеДействие = 1, НСтр("ka='получение данных';ru='получение данных'"), НСтр("ka='отправку данных';ru='отправку данных'")));
