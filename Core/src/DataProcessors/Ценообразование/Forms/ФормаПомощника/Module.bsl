...
Процедура ЗаписатьНовыеЦеныНаСервере()
	...
	ОписаниеОперации	= НСтр("ka='ახალი ფასის ჩაწერა ინფორმაციულ ბაზაში';ru='Запись новых цен в информационную базу'");
...
Процедура РасчитатьДанныеКоллекцииВДлительнойОперации(ПараметрыРасчета)
	...
	ОписаниеОперации	= НСтр("ka='Изменение колонок с новыми ценами по формуле';ru='Изменение колонок с новыми ценами по формуле'");
...
Процедура ФоновоеЗаданиеВыполнено()
	...
	Исключение
		...
		ВызватьИсключение Нстр("ka='შეცდომა';ru='Ошибка'") + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ДобавитьНоменклатуруВДлительнойОперации(СтруктураТаблицДанных, ПараметрыКопирования)
	...
	ОписаниеОперации	= НСтр("ka='Копирование полученых строк в табличные части обработки';ru='Копирование полученых строк в табличные части обработки'");
...
Процедура ДобавитьРеквизитыИЭлементыТабличнойЧастиПоВидуЦен(ИмяТабличнойЧасти, ВыбранныйВидЦенСоответствие)
	...
	РеквизитЦенаДо		= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенаДо",		ТипЧислоПлюс,	ПутьРеквизита, НСтр("ka='Цена до';ru='Цена до'"));
	РеквизитДельта		= Новый РеквизитФормы(ПрефиксРеквизитов + "_Дельта",		ТипЧислоЛюбое,	ПутьРеквизита, "Δ");
	РеквизитЦенаНовая	= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенаНовая",		ТипЧислоПлюс,	ПутьРеквизита, НСтр("ka='Цена новая';ru='Цена новая'"));
	РеквизитЦенуИзменили= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенуИзменили",	ТипБулево,		ПутьРеквизита, НСтр("ka='_Служебный';ru='_Служебный'"));
	РеквизитЕдИзмерения	= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЕдИзм",			ТипЕдИзмерения,	ПутьРеквизита, НСтр("ka='საზომი ერთეული';ru='Ед. изм.'"));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='დავხუროთ დამხმარე?';ru='Закрыть помощник?'");
...
Процедура ЗаписыватьНовыеЦеныПоверхУстановленныхПриИзменении(Элемент)
	...
	Если НЕ ПоказыватьХарактеристики Тогда
		...
		Элементы.УстанавливатьХарактеристикамБезЦен.Заголовок = ?(ЗаписыватьНовыеЦеныПоверхУстановленных,
			НСтр("ka='ნომენკლატურის ფასების დაყენება მახასიათებლის მიხედვით';ru='Устанавливать цены номенклатуры характеристикам'"),
			НСтр("ka='ნომენკლატურის ფასების დაყენება მახასიათებლის მიხედვით ფასის გარეშე';ru='Устанавливать цены номенклатуры характеристикам без цен'"));
...
Процедура ДекорацияПредупреждениеНажатие(Элемент)
	...
	ТекстПредупреждения = НСтр("ka='Изменение параметра невозможно из-за заполненной табличной части номенклатуры';ru='Изменение параметра невозможно из-за заполненной табличной части номенклатуры'");
...
Процедура ПечатьТабличнойЧасти(Команда)
	...
	ПечатнаяФорма.СинонимМакета = НСтр("ka='ფასების ფორმირება დამუშავების ცხრილურ ნაწილში';ru='Табличная часть обработки формирования цен'");
	ПечатнаяФорма.ТабличныйДокумент = ТабличныйДокумент;
	ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ka='ფასების ფორმირება დამუშავების ცხრილურ ნაწილში';ru='Табличная часть обработки формирования цен'");
...
Функция Подключаемый_СтраницаОдин_ПриПереходеДалее(Отказ)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='საჭიროა იმ ფასის ტიპების მითითება, რისთვისაც დაგეგმილია ახალი ფასების დაფიქსირება.';ru='Необходимо указать виды цен, для которых планируется установка новых цен.'");
...
Функция Подключаемый_СтраницаЧетыре_ПриПереходеДалее(Отказ)
	...
	Иначе
		...
		ТекстОшибки = НСтр("ka='არ არის მითითებული ფასის ტიპი (პრაის-ლისტის ველში), რომლისთვისაც გსურთ ახალი ფასის დაფქსირება.';ru='Нет видов цен (колонок прайс-листа), для которых требуется записать новые цены.'");
