...
Процедура ЗаписатьНовыеЦеныНаСервере()
	...
	ОписаниеОперации	= НСтр("ka='ახალი ფასის ჩაწერა ინფორმაციულ ბაზაში';ru='Запись новых цен в информационную базу';en='Record new price information base'");
	...
	Если НЕ КэшЗначений.ЗаданиеВыполнено Тогда
		...
		Элементы.ДекорацияПять.РасширеннаяПодсказка.Заголовок = Нстр("ka='"
"გთხოვთ მოიცადოთ. "
"მიმდინარეობს ახალი ფასის ჩაწერა.';ru='"
...
Процедура РасчитатьДанныеКоллекцииВДлительнойОперации(ПараметрыРасчета)
	...
	ОписаниеОперации	= НСтр("ka='Изменение колонок с новыми ценами по формуле';ru='Изменение колонок с новыми ценами по формуле';en='Changing speaker with the new price formula'");
...
Процедура ФоновоеЗаданиеВыполнено()
	...
	Исключение
		...
		ВызватьИсключение Нстр("ka='შეცდომა';ru='Ошибка';en='Mistake'") + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ДобавитьНоменклатуруВДлительнойОперации(СтруктураТаблицДанных, ПараметрыКопирования)
	...
	ОписаниеОперации	= НСтр("ka='Копирование полученых строк в табличные части обработки';ru='Копирование полученых строк в табличные части обработки';en='Copying of Received rows in table processing part'");
...
Процедура ДобавитьРеквизитыИЭлементыТабличнойЧастиПоВидуЦен(ИмяТабличнойЧасти, ВыбранныйВидЦенСоответствие)
	...
	РеквизитЦенаДо		= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенаДо",		ТипЧислоПлюс,	ПутьРеквизита, НСтр("ka='ძველი ფასი';ru='Цена до';en='Price to'"));
	РеквизитДельта		= Новый РеквизитФормы(ПрефиксРеквизитов + "_Дельта",		ТипЧислоЛюбое,	ПутьРеквизита, "Δ");
	РеквизитЦенаНовая	= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенаНовая",		ТипЧислоПлюс,	ПутьРеквизита, НСтр("ka='ახალი ფასი';ru='Цена новая';en='new Price'"));
	РеквизитЦенуИзменили= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенуИзменили",	ТипБулево,		ПутьРеквизита, НСтр("ka='_სამომხმარებლო';ru='_Служебный';en='_Official'"));
	РеквизитЕдИзмерения	= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЕдИзм",			ТипЕдИзмерения,	ПутьРеквизита, НСтр("ka='საზომი ერთეული';ru='Ед. изм.';en='Unit of Measurement'"));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='დავხუროთ დამხმარე?';ru='Закрыть помощник?';en='Close aide?'");
...
Процедура ЗаписыватьНовыеЦеныПоверхУстановленныхПриИзменении(Элемент)
	...
	Если НЕ ПоказыватьХарактеристики Тогда
		...
		Элементы.УстанавливатьХарактеристикамБезЦен.Заголовок = ?(ЗаписыватьНовыеЦеныПоверхУстановленных,
			НСтр("ka='ნომენკლატურის ფასების დაყენება მახასიათებლის მიხედვით';ru='Устанавливать цены номенклатуры характеристикам';en='Set prices nomenclature characteristics'"),
			НСтр("ka='ნომენკლატურის ფასების დაყენება მახასიათებლის მიხედვით ფასის გარეშე';ru='Устанавливать цены номенклатуры характеристикам без цен';en='Set prices nomenclature characteristics without the price'"));
...
Процедура ДекорацияПредупреждениеНажатие(Элемент)
	...
	ТекстПредупреждения = НСтр("ka='Изменение параметра невозможно из-за заполненной табличной части номенклатуры';ru='Изменение параметра невозможно из-за заполненной табличной части номенклатуры';en='Changing a parameter can not be filled due to the tables of nomenclature'");
...
Процедура ПечатьТабличнойЧасти(Команда)
	...
	ПечатнаяФорма.СинонимМакета = НСтр("ka='ფასების ფორმირება დამუშავების ცხრილურ ნაწილში';ru='Табличная часть обработки формирования цен';en='A table of the processing pricing'");
	ПечатнаяФорма.ТабличныйДокумент = ТабличныйДокумент;
	ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ka='ფასების ფორმირება დამუშავების ცხრილურ ნაწილში';ru='Табличная часть обработки формирования цен';en='A table of the processing pricing'");
...
"Продолжить выполнение печати?';ka='ფასების ფორმირება შესაძლებელია მხოლოს ჩაწერილი ფასებით,"
"ამავე დროს ფორმას აქვს შეუნახავი მონაცემები."
"განაგრძობთ ბეჭდვას?'");
...
Функция Подключаемый_СтраницаОдин_ПриПереходеДалее(Отказ)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='საჭიროა იმ ფასის ტიპების მითითება, რისთვისაც დაგეგმილია ახალი ფასების დაფიქსირება.';ru='Необходимо указать виды цен, для которых планируется установка новых цен.';en='You must specify the types of prices for which you plan to install the new prices.'");
...
Функция Подключаемый_СтраницаЧетыре_ПриПереходеДалее(Отказ)
	...
	Иначе
		...
		ТекстОшибки = НСтр("ka='არ არის მითითებული ფასის ტიპი (პრაის-ლისტის ველში), რომლისთვისაც გსურთ ახალი ფასის დაფქსირება.';ru='Нет видов цен (колонок прайс-листа), для которых требуется записать новые цены.';en='No kinds of prices (price list column) for which you want to add the new prices.'");
