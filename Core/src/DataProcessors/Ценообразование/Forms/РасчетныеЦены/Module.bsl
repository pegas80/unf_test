...
Процедура НовыеЦеныЗаПериод(Ошибки)
	...
	Если ЦеныРасчитаны Тогда
		...
		Элементы.ФормаУтвердитьЦены.Заголовок = НСтр("ka='Утвердить новые цены (';ru='Утвердить новые цены (';en='To approve the new prices ('") + Строка(ЦеныНоменклатуры.Количество()) + НСтр("ka=')';ru=')';en=')'");
