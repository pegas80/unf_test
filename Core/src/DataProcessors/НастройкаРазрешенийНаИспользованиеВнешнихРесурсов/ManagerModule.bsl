...
Функция ВыполнитьОбработкуЗапросовПроверкиПрименения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='Транзакция активна';ru='Транзакция активна'");
	КонецЕсли;
...
Функция СтрокаТаблицыРазрешений(Знач ТаблицаРазрешений, Знач Отбор, Знач ДобавитьПриОтсутствии = Истина) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონების უნიკალურობის დარღვევა შერჩევის მიხედვით ნებართვების ცხრილში %1';ru='Нарушение уникальности строк в таблице разрешений по отбору %1'"),
