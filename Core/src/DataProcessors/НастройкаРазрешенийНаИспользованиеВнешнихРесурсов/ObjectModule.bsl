...
"Попробуйте выполнить операцию позже.';ka='Ошибка конкурентного доступа к настройке разрешений на использование внешних ресурсов."
...
Функция НовоеОписаниеПрофиляБезопасности(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля)
	...
	Шаблон = НСтр("ka='[ИБ %1] %2 ""%3""';ru='[ИБ %1] %2 ""%3""';en=""[IB 1%]% 2 '% 3'""");
	...
	Если ПустаяСтрока(ИмяИБ) Тогда
		ВызватьИсключение НСтр("ka='Строка соединения информационной базы не содержит имени информационной базы!';ru='Строка соединения информационной базы не содержит имени информационной базы!';en='String information database connection does not contain the name of the knowledge base!'");
	КонецЕсли;
	
	Если ТипПрограммногоМодуля = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяИБ,
			НСтр("ka='Профиль безопасности для информационной базы';ru='Профиль безопасности для информационной базы';en='The safety profile for the information base'"), СтрокаСоединенияИнформационнойБазы());
	Иначе
