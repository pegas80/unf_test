...
Функция НачатьОбработкуЗапросов(Знач Запросы, Знач РежимВключения, РежимОтключения, Знач РежимВосстановления, Знач РежимПроверкиПрименения)
	...
	Задание = ФоновыеЗадания.Выполнить("РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации",
			...
			,
			НСтр("ka='Обработка запросов на использование внешних ресурсов...';ru='Обработка запросов на использование внешних ресурсов...';en='Processing of requests for the use of external resources ...'"));
...
Функция ЗапросыОбработаны(Знач ИдентификаторЗадания)
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение(НСтр("ka='При обработке запросов произошла ошибка - не найдено задание обработки запросов.';ru='При обработке запросов произошла ошибка - не найдено задание обработки запросов.';en='In query processing error has occurred - did not match query processing task.'"));
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		Иначе
			ВызватьИсключение(НСтр("ka='При обработке запросов произошла ошибка - задание обработки запросов завершилось с неизвестной ошибкой.';ru='При обработке запросов произошла ошибка - задание обработки запросов завершилось с неизвестной ошибкой.';en='If there was an error processing requests - a task query completed with an unknown error.'"));
		КонецЕсли;
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение(НСтр("ka='При обработке запросов произошла ошибка - задание обработки запросов было отменено администратором.';ru='При обработке запросов произошла ошибка - задание обработки запросов было отменено администратором.';en='In query processing error has occurred - a task query was canceled by an administrator.'"));
	Иначе
...
Процедура ОтменитьОбработкуЗапросов(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Настройка разрешений на использование внешних ресурсов.Отмена выполнения фонового задания';ru='Настройка разрешений на использование внешних ресурсов.Отмена выполнения фонового задания';en='Set permissions for the use of external resursov.Otmena perform a background job'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
