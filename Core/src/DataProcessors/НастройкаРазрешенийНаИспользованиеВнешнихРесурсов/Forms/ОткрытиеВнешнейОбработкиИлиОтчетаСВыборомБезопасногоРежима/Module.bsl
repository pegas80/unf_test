...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ФайлОбработкиИмя) Или Не ЗначениеЗаполнено(ФайлОбработкиАдрес) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите файл внешнего отчета или обработки';ru='Укажите файл внешнего отчета или обработки';en='Specify an external report or file processing'"), , "ФайлОбработкиАдрес");
		Отказ = Истина;
	...
	Если Не ЗначениеЗаполнено(БезопасныйРежим) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите безопасный режим для подключения внешнего модуля';ru='Укажите безопасный режим для подключения внешнего модуля';en='Specify the secure mode for connecting an external module'"), , "БезопасныйРежим");
		Отказ = Истина;
...
Функция ПодключитьНаСервере()
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение НСтр("ka='დაშვების უფლებები არასაკმარისია.';ru='Недостаточно прав доступа.';en='It is not enough access rights.'");
	КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Файл %1 не является файлом внешнего отчета или обработки';ru='Файл %1 не является файлом внешнего отчета или обработки';en='File% 1 is not an external file or report processing'"), ФайлОбработкиИмя);
