...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ФайлОбработкиИмя) Или Не ЗначениеЗаполнено(ФайлОбработкиАдрес) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите файл внешнего отчета или обработки';ru='Укажите файл внешнего отчета или обработки'"), , "ФайлОбработкиАдрес");
		Отказ = Истина;
	...
	Если Не ЗначениеЗаполнено(БезопасныйРежим) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите безопасный режим для подключения внешнего модуля';ru='Укажите безопасный режим для подключения внешнего модуля'"), , "БезопасныйРежим");
		Отказ = Истина;
...
Функция ПодключитьНаСервере()
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение НСтр("ka='დაშვების უფლებები არასაკმარისია.';ru='Недостаточно прав доступа.'");
	КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Файл %1 не является файлом внешнего отчета или обработки';ru='Файл %1 не является файлом внешнего отчета или обработки'"), ФайлОбработкиИмя);
