...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "ВидМаркировки") <> Перечисления.ВидыМаркировки.МаркируемаяПродукция Тогда
		ТекстИсключения = НСтр("ka='Номенклатура %Номенклатуры% не является продукцией, которую нужно маркировать для ГИСМ.';ru='Номенклатура %Номенклатуры% не является продукцией, которую нужно маркировать для ГИСМ.'");
	...
	Если ЭтоМаркировкаПерсонифицированнымиКиЗ
		...
		ТекстСообщения = НСтр("ka='Выбранный КиЗ предназначен для маркировки товаров с GTIN %GTINИзКиЗ%, его нельзя использовать для маркировки товара с GTIN %GTIN%.';ru='Выбранный КиЗ предназначен для маркировки товаров с GTIN %GTINИзКиЗ%, его нельзя использовать для маркировки товара с GTIN %GTIN%.'");
	...
	Элементы.ОК.Заголовок = НСтр("ka='მზად არის';ru='Готово'");
...
Функция ОбязательныеРеквизитыЗаполнены(ВыводитьСообщения = Истина)
	...
	Если Не ЗначениеЗаполнено(НомерКиЗГИСМ) Тогда
		...
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ka='Поле ""Номер КиЗ"" не заполнено';ru='Поле ""Номер КиЗ"" не заполнено'");
	...
	ИначеЕсли Не ИнтеграцияГИСМКлиентСервер.ЭтоНомерКиЗ(НомерКиЗГИСМ) Тогда
		Отказ = Истина;
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ka='Значение в поле ""Номер КиЗ"" не является номером контрольного (идентификационного) знака ГИСМ';ru='Значение в поле ""Номер КиЗ"" не является номером контрольного (идентификационного) знака ГИСМ'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"НомерКиЗГИСМ");
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьФормуПоШагу(Шаг)
	...
	Если Шаг = "Шаг1" Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ka='Укажите списываемую серию';ru='Укажите списываемую серию'");
	...
	Иначе
		...
		Если Не ЭтоМаркировкаПерсонифицированнымиКиЗ Тогда
			Элементы.Номер.ПодсказкаВвода = НСтр("ka='<генерируются по данным RFID-метки>';ru='<генерируются по данным RFID-метки>'");
			Элементы.Номер.Подсказка = НСтр("ka='генерируются по данным RFID-метки';ru='генерируются по данным RFID-метки'");
		Иначе
			Элементы.Номер.ПодсказкаВвода = НСтр("ka='<заполняется по данным КиЗ>';ru='<заполняется по данным КиЗ>'");
			Элементы.Номер.Подсказка = НСтр("ka='заполняется по данным КиЗ';ru='заполняется по данным КиЗ'");
		КонецЕсли;
		
		ЭтаФорма.Заголовок = НСтр("ka='Укажите новую серию';ru='Укажите новую серию'");
...
Функция ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ДанныеШтрихкодов.Количество() <> 1 Тогда
		...
		ТекстСообщения = НСтр("ka='Не поддерживается одновременное распознование нескольких штрих-кодов серий. Сканируйте серии по одной.';ru='Не поддерживается одновременное распознование нескольких штрих-кодов серий. Сканируйте серии по одной.'");
