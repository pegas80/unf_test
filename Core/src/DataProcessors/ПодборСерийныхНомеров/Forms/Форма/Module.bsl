...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Параметры.Запасы.Номенклатура.ИспользоватьСерийныеНомера Тогда
		ТекстСообщения = НСтр("ka='Для номенклатуры не ведется учет по серийным номерам!';ru='Для номенклатуры не ведется учет по серийным номерам!';en='For the nomenclature do not keep records of the serial numbers!'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("КонтрольОстатковСерийныхНомеров") Тогда
		...
		Если Параметры.Свойство("СтруктурнаяЕдиница") Тогда
			...
			Если ЗначениеЗаполнено(Параметры.СтруктурнаяЕдиница) Тогда
				Элементы.СерииОстаткиСерия.Заголовок = НСтр("ru='В наличии на ';ka='В наличии на '")+Параметры.СтруктурнаяЕдиница;
			КонецЕсли;
		Иначе
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Новый';ru='Новый';en='Новый'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Зарегистрированный';ru='Зарегистрированный';en='Registered'"));
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		ИначеЕсли НЕ (ДанныеШтрихкода.Номенклатура = Номенклатура И ДанныеШтрихкода.Характеристика = Характеристика 
			...
			СтрокаСообщения = НСтр("ka='Считанный штрихкод привязан к другой номенклатуре: %1% %2% %3% %4%';ru='Считанный штрихкод привязан к другой номенклатуре: %1% %2% %3% %4%';en='Read the barcode attached to another nomenclature:% 1% 2% 3%%%% 4%'");
...
Функция ПроверитьМеткиНаНеобходимостьЗаписать()
	...
	Если ЭтоМаркировкаТоваровГИСМ
		...
		Для Каждого Строка Из СтрокиСТребованиемЗаписи Цикл
			...
			Сообщить(НСтр("ru='Необходимо записать данные в КИЗ с номером ';ka='Необходимо записать данные в КИЗ с номером '")+Строка.НомерКИЗГИСМ);
