...
Процедура ТекстПоЗаявкамОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьДокументы" Тогда
		...
		ИначеЕсли ДокументыКПодтверждению.Количество() > 1 Тогда
			...
			ИнтеграцияГИСМКлиентПереопределяемый.ОткрытьФормуСпискаДокументов(ДокументыКПодтверждению, НСтр("ka='განაცხადები სსნ-ის გამოშვებაზე';ru='Заявки на выпуск КиЗ'"));
...
Процедура Обновить(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='Таблица ""КиЗ к подтверждению"" будет перезаполнена. Продолжить?';ru='Таблица ""КиЗ к подтверждению"" будет перезаполнена. Продолжить?'");
...
Функция ПодтвердитьПоступлениеНаСервере()
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Не удалось заблокировать для записи документ %1';ru='Не удалось заблокировать для записи документ %1'"), ВыборкаПоДокументам.Документ),
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Не удалось изменить состояние подтверждения для документа %1';ru='Не удалось изменить состояние подтверждения для документа %1'"), ДокументСОшибкой),
			ДокументСОшибкой);
...
Процедура СформироватьТекстПоОснованиям()
	...
	Если КоличествоДокументов = 1 Тогда
		...
		ТекстЗаголовок = НСтр("ka='По документу:';ru='По документу:'");
	...
	ИначеЕсли КоличествоДокументов > 1 Тогда
		...
		Основание = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ka='По документам (%1).';ru='По документам (%1).'"), КоличествоДокументов),
		                                             Новый Шрифт(,,,,Истина),
		                                             ЦветаСтиля.ЦветГиперссылкиГИСМ,
		                                             ,
		                                             "ОткрытьДокументы");
