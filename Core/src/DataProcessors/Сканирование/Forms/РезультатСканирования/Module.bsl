	...
	Если КоличествоОшибок > 0 Тогда
		...
		Иначе
			ПараметрыПредупреждения.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შესრულებისას შეცდომები წარმოიშვა (%1).';ru='При выполнении возникли ошибки (%1).';en='When performing any errors (1%).'"),
...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	Если ИспользоватьImageMagickДляПреобразованияВPDF Тогда
		Если ФорматХраненияОдностраничный = Перечисления.ФорматыХраненияОдностраничныхФайлов.PDF Тогда
			...
			Представление = Представление + НСтр("ka='Формат хранения:';ru='Формат хранения:';en='Storage Format:'") + " ";
			Представление = Представление + "PDF";
			Представление = Представление + ". ";
			Представление = Представление + НСтр("ka='Формат сканирования:';ru='Формат сканирования:';en='Scan size:'") + " ";
	...
	Если ВРег(ФорматКартинки) = "JPG" Тогда
		Представление = Представление +  НСтр("ka='Качество:';ru='Качество:';en='Quality:'") + " " + Строка(КачествоJPG) + ". ";
	КонецЕсли;	
	
	Если ВРег(ФорматКартинки) = "TIF" Тогда
		Представление = Представление +  НСтр("ka='Сжатие:';ru='Сжатие:';en='Compression:'") + " " + Строка(СжатиеTIFF) + ". ";
	КонецЕсли;
	
	Представление = Представление + НСтр("ka='Формат хранения многостраничный:';ru='Формат хранения многостраничный:';en='multi-storage format:'") + " ";
	...
	Представление = Представление + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Разрешение: %1 dpi. %2.';ru='Разрешение: %1 dpi. %2.';en='Resolution:% 1 dpi. 2%.'") + " ",
		Строка(Разрешение), Строка(ЦветностьПеречисление));
	
	Если НЕ ПоворотПеречисление.Пустая() Тогда
		Представление = Представление +  НСтр("ka='Поворот:';ru='Поворот:';en='Turn:'")+ " " + Строка(ПоворотПеречисление) + ". ";
	КонецЕсли;	
	
	Если НЕ РазмерБумагиПеречисление.Пустая() Тогда
		Представление = Представление +  НСтр("ka='Размер бумаги:';ru='Размер бумаги:';en='Paper size:'") + " " + Строка(РазмерБумагиПеречисление) + ". ";
	КонецЕсли;	
	
	Если ДвустороннееСканирование = Истина Тогда
		Представление = Представление +  НСтр("ka='Двустороннее сканирование';ru='Двустороннее сканирование';en='duplex scanning'") + ". ";
	КонецЕсли;	
