...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяДействия = ?(ТолькоПолучитьУзелСсылки, НСтр("ka='Конвертация ссылки на объект';ru='Конвертация ссылки на объект'"), НСтр("ka='Конвертация объекта';ru='Конвертация объекта'"));
		
		ТекстСообщения = НСтр("ka='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)';ru='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)'");
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ОписаниеОшибки(), ПКО, Источник, НСтр("ka='ПередКонвертациейОбъекта (глобальный)';ru='ПередКонвертациейОбъекта (глобальный)'"));
		КонецПопытки;
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ka='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)'");
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='Попытка изменения неразделенных данных (%1) в разделенном сеансе.';ru='Попытка изменения неразделенных данных (%1) в разделенном сеансе.'");
...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Если ЭтоОбменЧерезВнешнееСоединение() Тогда
		...
		Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
			...
			СтрокаСообщения = НСтр("ka='Ошибка в базе-корреспонденте: %1';ru='Ошибка в базе-корреспонденте: %1'");
...
Процедура ВыполнитьЗагрузкуДанных() Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ka='Возникли ошибки при загрузке данных.';ru='Возникли ошибки при загрузке данных.'");
			КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = НСтр("ka='Начало процесса обмена данными для узла: %1';ru='Начало процесса обмена данными для узла: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	// Запись в журнале регистрации.
	СтрокаСообщения = НСтр("ka='%1, %2; Обработано %3 объектов';ru='%1, %2; Обработано %3 объектов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ka='Возникли ошибки при анализе данных.';ru='Возникли ошибки при анализе данных.'");
			КонецЕсли;
...
Процедура ВнешнееСоединениеОбработчикКонвертацииПередЗагрузкойДанных(Отказ) Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ka='ПередЗагрузкойДанных (конвертация)';ru='ПередЗагрузкойДанных (конвертация)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВнешнееСоединениеПослеЗагрузкиДанных() Экспорт
	...
	Если Не ФлагОшибки() Тогда
		...
		Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
			...
			Исключение
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ka='ПослеЗагрузкиДанных (конвертация)';ru='ПослеЗагрузкиДанных (конвертация)'"));
			КонецПопытки;
...
Функция ПолучитьИнформациюОПравилах(ЭтоПравилаКорреспондента = Ложь) Экспорт
	...
	Если ЭтоПравилаКорреспондента Тогда
		СтрокаИнформации = НСтр("ka='Правила конвертации корреспондента (%1) от %2';ru='Правила конвертации корреспондента (%1) от %2'");
	Иначе
		СтрокаИнформации = НСтр("ka='Правила конвертации этой информационной базы (%1) от %2';ru='Правила конвертации этой информационной базы (%1) от %2'");
	КонецЕсли;
...
Процедура ИнициализироватьВедениеПротоколаОбмена()
	...
	Исключение
		ФайлПротоколаДанных = Неопределено;
		СтрокаСообщения = НСтр("ka='შეცდომა ფაილში მონაცემთა პროტოკოლის ჩაწერის მცდელობისას: %1. შეცდომის აღწერილობა: %2';ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2'",
...
Процедура НачатьЧтениеСообщения(ЧтениеСообщения, АнализДанных = Ложь)
	...
	Если ФайлОбмена.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ka='Ошибка формата сообщения обмена.';ru='Ошибка формата сообщения обмена.'");
	КонецЕсли;
	...
	Если ТаблицаПравилКонвертации.Количество() = 0 Тогда
		...
		Если ФлагОшибки() Тогда
			ВызватьИсключение НСтр("ka='При загрузке правил обмена данными возникли ошибки.';ru='При загрузке правил обмена данными возникли ошибки.'");
		КонецЕсли;
	Иначе
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ka='ПередЗагрузкойДанных (конвертация)';ru='ПередЗагрузкойДанных (конвертация)'"));
		КонецПопытки;
		
		Если Отказ Тогда
			ВызватьИсключение НСтр("ka='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';ru='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).'");
		КонецЕсли;
	...
	Если ФайлОбмена.ЛокальноеИмя = "ИнформацияОТипахДанных" Тогда
		...
		Иначе
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ka='При загрузке информации о типах данных возникли ошибки.';ru='При загрузке информации о типах данных возникли ошибки.'");
			КонецЕсли;
		КонецЕсли;
		ФайлОбмена.Прочитать();
	КонецЕсли;
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='Блокировка получения данных не может быть установлена в активной транзакции.';ru='Блокировка получения данных не может быть установлена в активной транзакции.'");
	КонецЕсли;
...
Процедура ВыполнитьОбработчикПослеЗагрузкиДанных()
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ka='ПослеЗагрузкиДанных (конвертация)';ru='ПослеЗагрузкиДанных (конвертация)'"));
		КонецПопытки;
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			...
			Если Не ПустаяСтрока(ПричинаОтказа) Тогда
				...
				ВызватьИсключение СтрокаСообщения;
			Иначе
				ВызватьИсключение НСтр("ka='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).'");
			КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(73, ОписаниеОшибки(), "", "", 
				ТипОбъекта, Неопределено, НСтр("ka='Последовательность полей поиска';ru='Последовательность полей поиска'"));
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Исключение
		...
		Текст = НСтр("ka='ცხრილის სახელი: %1';ru='Имя табличной части: %1'");
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ka='Загрузка объекта № %1';ru='Загрузка объекта № %1'");
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта Тогда
		...
		Исключение
			...
			ТипОбъекта, Неопределено, НСтр("ka='ПередЗагрузкойОбъекта (глобальный)';ru='ПередЗагрузкойОбъекта (глобальный)'"));
			...
			Если ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта Тогда
				...
				Исключение
					...
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(54, ОписаниеОшибки(), ИмяПравила, Источник,
						ТипОбъекта, Объект, НСтр("ka='ПослеЗагрузкиОбъекта (глобальный)';ru='ПослеЗагрузкиОбъекта (глобальный)'"));
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	СтрокаСообщения = НСтр("ka='შესრულებულია მონაცემთა სინქრონიზაციაზე ავტომატური გადასვლა ფორმატ EnterpriseData-ით.';ru='Выполнен автоматический переход на синхронизацию данных через формат EnterpriseData.'");
	ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Информация);
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяСобытия = НСтр("ka='ობიექტისგადმოტვირთვა: %1';ru='ВыгрузкаОбъекта: %1'");
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, НСтр("ka='ПередВыгрузкойОбъектаВыборки (глобальный)';ru='ПередВыгрузкойОбъектаВыборки (глобальный)'"), Объект);
		КонецПопытки;
...
Процедура ВызватьСобытияПослеВыгрузкиОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, НСтр("ka='ПослеВыгрузкиОбъектаВыборки (глобальный)';ru='ПослеВыгрузкиОбъектаВыборки (глобальный)'"), Объект);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОтключитьОбработкуДляОтладки()
	...
	Если ОбработкаВыгрузки <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ИнициализацияСообщений()
	...
	СообщенияОбОшибках.Вставить(2,  НСтр("ka='გაცვლის ფაილის განფუთვის შეცდომა. ფაილი დაბლოკილია.';ru='Ошибка распаковки файла обмена. Файл заблокирован.'"));
	СообщенияОбОшибках.Вставить(3,  НСтр("ka='გაცვლის წესების აღნიშნული ფაილი არ არსებობს.';ru='Указанный файл правил обмена не существует.'"));
	СообщенияОбОшибках.Вставить(4,  НСтр("ka='შეცდომა მოხდა COM-ობიექტის შექმნისას Msxml2.DOMDocument';ru='Ошибка при создании COM-объекта Msxml2.DOMDocument'"));
	СообщенияОбОшибках.Вставить(5,  НСтр("ka='გაცვლის ფაილის გახსნის შეცდომა';ru='Ошибка открытия файла обмена'"));
	СообщенияОбОшибках.Вставить(6,  НСтр("ka='შეცდომა გაცვლის წესების ჩატვირთვისას';ru='Ошибка при загрузке правил обмена'"));
	СообщенияОбОшибках.Вставить(7,  НСтр("ka='გაცვლის წესების ფორმატის შეცდომა';ru='Ошибка формата правил обмена'"));
	СообщенияОбОшибках.Вставить(8,  НСтр("ka='არაკორექტულად არის მითითებული ფაილის სახელი მონაცემთა გადმოსატვირთად';ru='Не корректно указано имя файла для выгрузки данных'")); // не используется
	СообщенияОбОшибках.Вставить(9,  НСтр("ka='გაცვლის ფაილის ფორმატის შეცდომა';ru='Ошибка формата файла обмена'"));
	СообщенияОбОшибках.Вставить(10, НСтр("ka='არ არის მითიტებული ფაილის სახელი მონაცემთა გადმოსატვირთად (მონაცემთა ფაილის სახელი)';ru='Не указано имя файла для выгрузки данных (Имя файла данных)'"));
	СообщенияОбОшибках.Вставить(11, НСтр("ka='მინიშნება გაცვლის წესების მეტამონაცემთა არარსებულ ობიექტზე';ru='Ссылка на несуществующий объект метаданных в правилах обмена'"));
	СообщенияОбОшибках.Вставить(12, НСтр("ka='არ არის მითიტებული ფაილის სახელი გაცვლის წესებით (წესების ფაილის სახელი)';ru='Не указано имя файла с правилами обмена (Имя файла правил)'"));
			
	СообщенияОбОшибках.Вставить(13, НСтр("ka='ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (წყაროს თვისების სახელით)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)'"));
	СообщенияОбОшибках.Вставить(14, НСтр("ka='ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (მიმღების თვისების სახელით)';ru='Ошибка получения значения свойства объекта (по имени свойства приемника)'"));
	
	СообщенияОбОшибках.Вставить(15, НСтр("ka='არ არის მითიტებული ფაილის სახელი მონაცემთა ჩასატვირთად (ჩასატვირთი ფაილის სახელი)';ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)'"));
			
	СообщенияОбОшибках.Вставить(16, НСтр("ka='დაქვემდებარებული ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (წყაროს თვისების სახელით)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)'"));
	СообщенияОбОшибках.Вставить(17, НСтр("ka='დაქვემდებარებული ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (მიმღების თვისების სახელით)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)'"));
	СообщенияОбОшибках.Вставить(18, НСтр("ka='შეცდომა დამამუშავებლების კოდით დამუშავების შექმნისას';ru='Ошибка при создании обработки с кодом обработчиков'"));
	СообщенияОбОшибках.Вставить(19, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისჩატვირთვისწინ';ru='Ошибка в обработчике события ПередЗагрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(20, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისჩატვირთვისას';ru='Ошибка в обработчике события ПриЗагрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(21, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისჩატვირთვისშემდეგ';ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта'"));
	СообщенияОбОшибках.Вставить(22, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაჩატვირთვისწინ (კონვერტაცია)';ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)'"));
	СообщенияОбОшибках.Вставить(23, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაჩატვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)'"));
	СообщенияОбОшибках.Вставить(24, НСтр("ka='შეცდომა მოხდა ობიექტის წაშლისას';ru='Ошибка при удалении объекта'"));
	СообщенияОбОшибках.Вставить(25, НСтр("ka='შეცდომა დოკუმენტის ჩაწერისას';ru='Ошибка при записи документа'"));
	СообщенияОбОшибках.Вставить(26, НСтр("ka='ობიექტის ჩაწერის შეცდომა';ru='Ошибка записи объекта'"));
	СообщенияОбОшибках.Вставить(27, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გასუფთავებისწესისდამუშავებისწინ';ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки'"));
	СообщенияОбОшибках.Вставить(28, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გასუფთავებისწესისდამუშავებისშემდეგ';ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки'"));
	СообщенияОбОшибках.Вставить(29, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისწაშლამდე';ru='Ошибка в обработчике события ПередУдалениемОбъекта'"));
	
	СообщенияОбОшибках.Вставить(31, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწესისდამუშავებისწინ';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(32, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწესებისდამუშავებისშემდეგ';ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(33, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისწინ';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(34, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისშემდეგ';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта'"));
			
	СообщенияОбОшибках.Вставить(41, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისწინ';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(42, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისას';ru='Ошибка в обработчике события ПриВыгрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(43, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისშემდეგ';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта'"));
			
	СообщенияОбОшибках.Вставить(45, НСтр("ka='ობიექტების კონვერტაციის წესი ნაპოვნი არ არის';ru='Не найдено правило конвертации объектов'"));
		
	СообщенияОбОшибках.Вставить(48, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისდამუშავებისწინ თვისებათა ჯგუფები';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств'"));
	СообщенияОбОшибках.Вставить(49, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისდამუშავებისშემდეგ თვისებათა ჯგუფები';ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств'"));
	СообщенияОбОшибках.Вставить(50, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწინ (კოლექციის ობიექტი)';ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)'"));
	СообщенияОбОшибках.Вставить(51, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში  გადმოტვირთვისას (კოლექციის ობიექტის)';ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)'"));
	СообщенияОбОшибках.Вставить(52, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისშემდეგ (კოლექციის ობიექტის)';ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)'"));
	СообщенияОбОшибках.Вставить(53, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისჩატვირთვისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)'"));
	СообщенияОбОшибках.Вставить(54, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისჩატვირთვისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)'"));
	СообщенияОбОшибках.Вставить(55, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწინ (თვისებები)';ru='Ошибка в обработчике события ПередВыгрузкой (свойства)'"));
	СообщенияОбОшибках.Вставить(56, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისას (თვიდებები)';ru='Ошибка в обработчике события ПриВыгрузке (свойства)'"));
	СообщенияОбОшибках.Вставить(57, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისშემდეგ (თვისებები)';ru='Ошибка в обработчике события ПослеВыгрузки (свойства)'"));
	
	СообщенияОбОшибках.Вставить(62, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაგადმოტვირთვისწინ (კონვერტაცია)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)'"));
	СообщенияОбОшибках.Вставить(63, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაგადმოტვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)'"));
	СообщенияОбОшибках.Вставить(64, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისკონვერტაციისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)'"));
	СообщенияОбОшибках.Вставить(65, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისგადმოტვირთვისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)'"));
	СообщенияОбОшибках.Вставить(66, НСтр("ka='შემომავალი მონაცემებიდან დაქვემდებარებული ობიექტების კოლექციის მიღების შეცდომა';ru='Ошибка получения коллекции подчиненных объектов из входящих данных'"));
	СообщенияОбОшибках.Вставить(67, НСтр("ka='შემომავალი მონაცემებიდან დაქვემდებარებული ობიექტის თვისების მიღების შეცდომა';ru='Ошибка получения свойства подчиненного объекта из входящих данных'"));
	СообщенияОбОшибках.Вставить(68, НСтр("ka='შემომავალი მონაცემებიდან ობიექტის თვისების მიღების შეცდომა';ru='Ошибка получения свойства объекта из входящих данных'"));
	
	СообщенияОбОшибках.Вставить(69, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისგადმოტვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)'"));
	
	СообщенияОбОшибках.Вставить(71, НСтр("ka='მნიშვნელობა წყაროსთვის არ არის ნაპოვნი შესაბამისობა';ru='Не найдено соответствие для значения Источника'"));
	
	СообщенияОбОшибках.Вставить(72, НСтр("ka='შეცდომა მონაცემთა გადმოტვირთვისას გაცვლის გეგმის კვანძისთვის';ru='Ошибка при выгрузке данных для узла плана обмена'"));
	
	СообщенияОбОшибках.Вставить(73, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ძიებისველებისთანმიმდევრულობა';ru='Ошибка в обработчике события ПоследовательностьПолейПоиска'"));
	СообщенияОбОшибках.Вставить(74, НСтр("ka='აუცილებელია მონაცემთა გადმოტვირთვის გაცვლის წესების გადატვირთვა.';ru='Необходимо перезагрузить правила обмена для выгрузки данных.'"));
	
	СообщенияОбОшибках.Вставить(75, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გაცვლისწესებისჩატვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)'"));
	СообщенияОбОшибках.Вставить(76, НСтр("ka=' წაშლისშესახბინფორმაციისგაგზავნისწინ (კონვერტაცია)';ru='Ошибка в обработчике события ПередОтправкойИнформацииОбУдалении (конвертация)'"));
	СообщенияОбОшибках.Вставить(77, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში წაშლისშესახებინფორმაციისმიღებისას (კონვერტაცია)';ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)'"));
	
	СообщенияОбОшибках.Вставить(78, НСтр("ka='შეცდომა ალგორითმის შესრულებისას პარამეტრების მნიშვნელობების ჩატვირთვის შემდეგ';ru='Ошибка при выполнении алгоритма после загрузки значений параметров'"));
	
	СообщенияОбОшибках.Вставить(79, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ფაილშიობიექტისგადმოტვირვისშემდეგ';ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл'"));
...
"წინასწარგანსაზღვრული ელემენტის წასაშლელად მონიშვნა არ შეიძლება. ობიექტისთვის წასაშლელი ნიშნული დაყენებული არ არის.'"));
	//
	СообщенияОбОшибках.Вставить(83, НСтр("ka='ობიექტის ცხრილთან მიმართვის შეცდომა. ობიექტის ცხრილის შეცვლა შეუძლებელია.';ru='Ошибка обращения к табличной части объекта. Табличная часть объекта не может быть изменена.'"));
	СообщенияОбОшибках.Вставить(84, НСтр("ka='ცვლილების აკრძალვის თარიღების კოლიზია.';ru='Коллизия дат запрета изменения.'"));
	
	СообщенияОбОшибках.Вставить(173, НСтр("ru='Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется'"));
	СообщенияОбОшибках.Вставить(174, НСтр("ka='გაცვლის შეტყობინება მიღებული იქნა უწინ';ru='Сообщение обмена было принято ранее'"));
	СообщенияОбОшибках.Вставить(175, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში შეცვლილიობიექტებისმიღებისწინ (კონვერტაცია)';ru='Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)'"));
	СообщенияОбОшибках.Вставить(176, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გაცვლისკვანძებისშესახებინფორმაციისმიღებისშემდეგ (კონვერტაცია)';ru='Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)'"));
		
	СообщенияОбОшибках.Вставить(1000, НСтр("ka='შეცდომა მონაცემთა გადმოტვირთვის დროებითი ფაილის შექმნისას';ru='Ошибка при создании временного файла выгрузки данных'"));
				...
				Попытка
					...
					Если Отказ = Истина Тогда
						...
						Если Не ПустаяСтрока(ПричинаОтказа) Тогда
							...
							СтрокаСообщения = НСтр("ka='Загрузка данных отменена по причине: %1';ru='Загрузка данных отменена по причине: %1'");
							СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
							ВызватьИсключение СтрокаСообщения;
						Иначе
							ВызватьИсключение НСтр("ka='Загрузка данных отменена';ru='Загрузка данных отменена'");
						КонецЕсли;
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ka='გაცვლის კვანძი მონაცემთა ჩასატვირთად ნაპოვნი არ არის. გაცვლის გეგმა: %1, კოდი: %2';ru='Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2'");
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
		...
		Если НомерПринятого >= ЧтениеСообщения.НомерСообщения Тогда // Номер сообщения меньше либо равен ранее принятому.
			...
			ВызватьИсключение НСтр("ka='გაცვლის შეტყობინება მიღებული იქნა უწინ';ru='Сообщение обмена было принято ранее'");
		КонецЕсли;
	...
	Для Каждого ОбъектСоответствия Из ОбъектыДляОтложеннойЗаписи() Цикл
		...
		Попытка
			...
			Иначе
				...
				ОписаниеОшибки = НСтр("ka='რეკვიზიტების შევსების შემოწმების შეცდომა';ru='Ошибка проверки заполнения реквизитов'");
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("ka='ПередВыгрузкойДанных (конвертация)';ru='ПередВыгрузкойДанных (конвертация)'"));
			Отказ = Истина;
		КонецПопытки; 
	...
	Если Не Отказ И Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("ka='ПослеВыгрузкиДанных (конвертация)';ru='ПослеВыгрузкиДанных (конвертация)'"));
		КонецПопытки;
