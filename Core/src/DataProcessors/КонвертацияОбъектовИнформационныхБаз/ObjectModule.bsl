...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяДействия = ?(ТолькоПолучитьУзелСсылки, НСтр("ka='Конвертация ссылки на объект';ru='Конвертация ссылки на объект';en='Converting an object reference'"), НСтр("ka='Конвертация объекта';ru='Конвертация объекта';en='Converting object'"));
		
		ТекстСообщения = НСтр("ka='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)';ru='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)';en='[ImyaDeystviya] [Object] ([TipObekta]) SSP: [FFP] (NaimenovaniePKO)'");
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ОписаниеОшибки(), ПКО, Источник, НСтр("ka='ПередКонвертациейОбъекта (глобальный)';ru='ПередКонвертациейОбъекта (глобальный)';en='PeredKonvertatsieyObekta (global)'"));
		КонецПопытки;
	...
	Если Не НеНужноВыгружатьПоСоответствиюЗначений Тогда
		...
		Если УзелСсылки = Неопределено Тогда
			...
			Если СтруктураСвойств.ИмяТипа = "Перечисление"
				...
				ЗП.Текст               = НСтр("ka='В правиле конвертации значений (ПКЗ) необходимо сопоставить значение Источника значению Приемника."
"Если подходящего значения приемника нет, то указать пустое значение.';ru='В правиле конвертации значений (ПКЗ) необходимо сопоставить значение Источника значению Приемника."
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ka='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';en='RULE DISCHARGE DATA: 1% (2%)'");
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ka='Попытка изменения неразделенных данных (%1) в разделенном сеансе.';ru='Попытка изменения неразделенных данных (%1) в разделенном сеансе.';en='Attempting to change undivided data (1%) in the divided session.'");
...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Если ЭтоОбменЧерезВнешнееСоединение() Тогда
		...
		Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
			...
			СтрокаСообщения = НСтр("ka='Ошибка в базе-корреспонденте: %1';ru='Ошибка в базе-корреспонденте: %1';en='A bug in the correspondent:% 1'");
...
Процедура ВыполнитьЗагрузкуДанных() Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ka='Возникли ошибки при загрузке данных.';ru='Возникли ошибки при загрузке данных.';en='An error occurred while loading data.'");
			КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = НСтр("ka='Начало процесса обмена данными для узла: %1';ru='Начало процесса обмена данными для узла: %1';en='Starting data exchange process for the node: 1%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	// Запись в журнале регистрации.
	СтрокаСообщения = НСтр("ka='%1, %2; Обработано %3 объектов';ru='%1, %2; Обработано %3 объектов';en='%12; Processed objects 3%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ka='Возникли ошибки при анализе данных.';ru='Возникли ошибки при анализе данных.';en='Any errors in the data analysis.'");
			КонецЕсли;
...
Процедура ВнешнееСоединениеОбработчикКонвертацииПередЗагрузкойДанных(Отказ) Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ka='ПередЗагрузкойДанных (конвертация)';ru='ПередЗагрузкойДанных (конвертация)';en='PeredZagruzkoyDannyh (conversion)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВнешнееСоединениеПослеЗагрузкиДанных() Экспорт
	...
	Если Не ФлагОшибки() Тогда
		...
		Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
			...
			Исключение
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ka='ПослеЗагрузкиДанных (конвертация)';ru='ПослеЗагрузкиДанных (конвертация)';en='PosleZagruzkiDannyh (conversion)'"));
			КонецПопытки;
...
Функция ПолучитьИнформациюОПравилах(ЭтоПравилаКорреспондента = Ложь) Экспорт
	...
	Если ЭтоПравилаКорреспондента Тогда
		СтрокаИнформации = НСтр("ka='Правила конвертации корреспондента (%1) от %2';ru='Правила конвертации корреспондента (%1) от %2';en='Terms converting reporter (1%) by 2%'");
	Иначе
		СтрокаИнформации = НСтр("ka='Правила конвертации этой информационной базы (%1) от %2';ru='Правила конвертации этой информационной базы (%1) от %2';en='Rules of converting this information base (1%) of 2%'");
	КонецЕсли;
...
"Имя файла ""%1""."
"Описание ошибки:"
"%2';ka='Ошибка при открытии файла для записи сообщения обмена."
"Имя файла ""%1""."
"Описание ошибки:"
"%2'"),
...
Процедура ИнициализироватьВедениеПротоколаОбмена()
	...
	Исключение
		ФайлПротоколаДанных = Неопределено;
		СтрокаСообщения = НСтр("ka='შეცდომა ფაილში მონაცემთა პროტოკოლის ჩაწერის მცდელობისას: %1. შეცდომის აღწერილობა: %2';ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';en='Error when trying to write data to the log file:% 1. Description of error:% 2'",
...
Процедура ВосстановитьПравилаИзВнутреннегоФормата() Экспорт
	...
	Если ВерсияФорматаХраненияПравил <> ВерсияФорматаХраненияПравилОбмена() Тогда
		ВызватьИсключение НСтр("ka='Версия формата хранения правил обмена не соответствует ожидаемой."
"Требуется выполнить загрузку правил обмена повторно.';ru='Версия формата хранения правил обмена не соответствует ожидаемой."
...
Процедура НачатьЧтениеСообщения(ЧтениеСообщения, АнализДанных = Ложь)
	...
	Если ФайлОбмена.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ka='Ошибка формата сообщения обмена.';ru='Ошибка формата сообщения обмена.';en='Format error message exchange.'");
	КонецЕсли;
	...
	Если ТаблицаПравилКонвертации.Количество() = 0 Тогда
		...
		Если ФлагОшибки() Тогда
			ВызватьИсключение НСтр("ka='При загрузке правил обмена данными возникли ошибки.';ru='При загрузке правил обмена данными возникли ошибки.';en='When loading the rules of data exchange errors occurred.'");
		КонецЕсли;
	Иначе
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ka='ПередЗагрузкойДанных (конвертация)';ru='ПередЗагрузкойДанных (конвертация)';en='PeredZagruzkoyDannyh (conversion)'"));
		КонецПопытки;
		
		Если Отказ Тогда
			ВызватьИсключение НСтр("ka='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';ru='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';en='Refusal to download messaging handler PeredZagruzkoyDannyh (conversion).'");
		КонецЕсли;
	...
	Если ФайлОбмена.ЛокальноеИмя = "ИнформацияОТипахДанных" Тогда
		...
		Иначе
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ka='При загрузке информации о типах данных возникли ошибки.';ru='При загрузке информации о типах данных возникли ошибки.';en='When downloading information about these types of errors occur.'");
			КонецЕсли;
		КонецЕсли;
		ФайлОбмена.Прочитать();
	КонецЕсли;
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ka='Блокировка получения данных не может быть установлена в активной транзакции.';ru='Блокировка получения данных не может быть установлена в активной транзакции.';en='Lock obtain data can not be set in an active transaction.'");
	КонецЕсли;
...
""
"Подробности:"
"%1';ka='მონაცემთა გაცვლაზე ბლოკირების დაყენების შეცდომა."
"შეიძლება, რომ მონაცემთა გაცვლა ხორციელდება სხვა სეანსის მიერ."
""
...
Процедура ВыполнитьОбработчикПослеЗагрузкиДанных()
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ka='ПослеЗагрузкиДанных (конвертация)';ru='ПослеЗагрузкиДанных (конвертация)';en='PosleZagruzkiDannyh (conversion)'"));
		КонецПопытки;
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			...
			Если Не ПустаяСтрока(ПричинаОтказа) Тогда
				...
				СтрокаСообщения = НСтр("ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1';en='Refusal to download messaging handler PosleZagruzkiParametrov (conversion) due to:% 1'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
				ВызватьИсключение СтрокаСообщения;
			Иначе
				ВызватьИсключение НСтр("ka='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';en='Refusal to download messaging handler PosleZagruzkiParametrov (conversion).'");
			КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(73, ОписаниеОшибки(), "", "", 
				ТипОбъекта, Неопределено, НСтр("ka='Последовательность полей поиска';ru='Последовательность полей поиска';en='The sequence of the search fields'"));
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Исключение
		...
		Текст = НСтр("ka='ცხრილის სახელი: %1';ru='Имя табличной части: %1';en='Name of the table:% 1'");
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ka='Загрузка объекта № %1';ru='Загрузка объекта № %1';en='Download object number% 1'");
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта Тогда
		...
		Исключение
			...
			ТипОбъекта, Неопределено, НСтр("ka='ПередЗагрузкойОбъекта (глобальный)';ru='ПередЗагрузкойОбъекта (глобальный)';en='PeredZagruzkoyObekta (global)'"));
			...
			Если ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта Тогда
				...
				Исключение
					...
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(54, ОписаниеОшибки(), ИмяПравила, Источник,
						ТипОбъекта, Объект, НСтр("ka='ПослеЗагрузкиОбъекта (глобальный)';ru='ПослеЗагрузкиОбъекта (глобальный)';en='PosleZagruzkiObekta (global)'"));
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Исключение
						...
						СтрокаСообщения = НСтр("ru='Ошибка при записи документа: %1. Описание ошибки: %2';en='Error writing the document:% 1. Description of error:% 2'");
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	СтрокаСообщения = НСтр("ka='შესრულებულია მონაცემთა სინქრონიზაციაზე ავტომატური გადასვლა ფორმატ EnterpriseData-ით.';ru='Выполнен автоматический переход на синхронизацию данных через формат EnterpriseData.';en='Auto-upgraded to synchronize data across EnterpriseData format.'");
	ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Информация);
	ПолеРезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Отменено;
	ВызватьИсключение НСтр("ru='Синхронизация данных по старой настройке отменена.';en='Synchronizing data on the old setting is canceled.'");
КонецПроцедуры
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяСобытия = НСтр("ka='ობიექტისგადმოტვირთვა: %1';ru='ВыгрузкаОбъекта: %1';en='VygruzkaObekta:% 1'");
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, НСтр("ka='ПередВыгрузкойОбъектаВыборки (глобальный)';ru='ПередВыгрузкойОбъектаВыборки (глобальный)';en='PeredVygruzkoyObektaVyborki (global)'"), Объект);
		КонецПопытки;
...
Процедура ВызватьСобытияПослеВыгрузкиОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, НСтр("ka='ПослеВыгрузкиОбъектаВыборки (глобальный)';ru='ПослеВыгрузкиОбъектаВыборки (глобальный)';en='PosleVygruzkiObektaVyborki (global)'"), Объект);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОтработатьУдалениеОбъекта(ДанныеОбУдаленииОбъекта, СтрокаСообщенияОбОшибке = "")
	...
	Если Конвертация.Свойство("ПередОтправкойИнформацииОбУдалении", ТекстСобытия) Тогда
		...
		Если Не ПустаяСтрока(ТекстСобытия) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(76, ОписаниеОшибки(), НСтр("ru='ПередОтправкойИнформацииОбУдалении (конвертация)';en='PeredOtpravkoyInformatsiiObUdalenii (conversion)'"));
...
Функция ПолучитьСтруктуруРеквизитовРегистрации(ТаблицаПКС)
	...
	Для Каждого ПКС Из МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Загрузка правил конвертации';en='Exchange dannymi.Zagruzka conversion rules'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьПроверкуРеквизитовРегистрацииОбъектов(ТаблицаРеквизитовРегистрации)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеквизитовРегистрации Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru='Тип объекта не определен: %1';en='Object type not specified:% 1'");
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если ОбщегоНазначения.ЭтоЗадача(ОбъектМД) Тогда
					...
					Если НЕ (ОбъектМД.Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
						...
						СтрокаСообщения = НСтр("ru='Неправильно указаны реквизиты шапки объекта ""%1"". Реквизит ""%2"" не существует.';en='Incorrectly listed the details hat object ""%1"". Props ''% 2'' does not exist.'");
		...
		Иначе
			...
			Если МетаТаблица = Неопределено Тогда
				ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.';en='Tabular part (standard tabular part motion) ""% ​​1"" object ""%2"" does not exist.'"),
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если Не РеквизитНайденВТабличнойЧастиРеквизитовРегистрацииОбъектов(МетаТаблица, Реквизит.Ключ) Тогда
					ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';en='Props ""%3"" does not exist in the table part (standard tabular portion movements) ""%1"" object ""%2"".'"),
...
Процедура ОтключитьОбработкуДляОтладки()
	...
	Если ОбработкаВыгрузки <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными';en='Data exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ИнициализацияСообщений()
	...
	СообщенияОбОшибках.Вставить(8,  НСтр("ka='არაკორექტულად არის მითითებული ფაილის სახელი მონაცემთა გადმოსატვირთად';ru='Не корректно указано имя файла для выгрузки данных';en='Incorrectly stated the name of the file to upload data'")); // не используется
...
"წინასწარგანსაზღვრული ელემენტის წასაშლელად მონიშვნა არ შეიძლება. ობიექტისთვის წასაშლელი ნიშნული დაყენებული არ არის.'"));
	...
	СообщенияОбОшибках.Вставить(173, НСтр("ru='Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется';en='Error sharing the lock assembly. Perhaps, data synchronization is already in progress'"));
...
Процедура ЗагрузитьПравилаОбмена(Источник="",
	...
	Если РежимОбмена <> "Загрузка" И Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(75, ОписаниеОшибки(), НСтр("ru='ПослеЗагрузкиПравилОбмена (конвертация)';en='PosleZagruzkiPravilObmena (conversion)'"));
			Отказ = Истина;
...
Процедура УдалитьОбъектПоСсылке(Ссылка, СтрокаСообщенияОбОшибке)
	...
	Если Не ПустаяСтрока(Конвертация.ПриПолученииИнформацииОбУдалении) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(77, ОписаниеОшибки(), НСтр("ru='ПриПолученииИнформацииОбУдалении (конвертация)';en='PriPolucheniiInformatsiiObUdalenii (conversion)'"));
			Отказ = Истина;
					...
					Если СтрокаТаблицы = Неопределено Тогда
						...
						Для Каждого Колонка Из ТаблицаДанныхСообщенияОбмена.Колонки Цикл
							...
							Если Колонка.ТипЗначения.СодержитТип(ТипСтрока) Тогда
								...
								СтрокаТаблицы[Колонка.Имя] = НСтр("ru='Удаление объекта';en='Deleting an object'");
				...
				Попытка
					...
					Если Отказ = Истина Тогда
						...
						Если Не ПустаяСтрока(ПричинаОтказа) Тогда
							...
							СтрокаСообщения = НСтр("ka='Загрузка данных отменена по причине: %1';ru='Загрузка данных отменена по причине: %1';en='Loading data is canceled due to:% 1'");
							СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
							ВызватьИсключение СтрокаСообщения;
						Иначе
							ВызватьИсключение НСтр("ka='Загрузка данных отменена';ru='Загрузка данных отменена';en='Loading data canceled'");
						КонецЕсли;
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ka='გაცვლის კვანძი მონაცემთა ჩასატვირთად ნაპოვნი არ არის. გაცვლის გეგმა: %1, კოდი: %2';ru='Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2';en='Not found sharing site to download the data. exchange Plan: 1%, Code: 2%'");
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
		...
		Если НомерПринятого >= ЧтениеСообщения.НомерСообщения Тогда // Номер сообщения меньше либо равен ранее принятому.
			...
			ВызватьИсключение НСтр("ka='გაცვლის შეტყობინება მიღებული იქნა უწინ';ru='Сообщение обмена было принято ранее';en='sharing the message was received earlier'");
		КонецЕсли;
		...
		Если УдалитьРегистрациюИзменений Тогда // Удаляем регистрацию изменений.
			...
			Если ТранзакцияАктивна() Тогда
				ВызватьИсключение НСтр("ru='Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';en='Removal of recording data changes may be made in the active transaction.'");
			КонецЕсли;
	...
	Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(176, ОписаниеОшибки(), НСтр("ru='ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';en='PoslePolucheniyaInformatsiiObUzlahObmena (conversion)'"));
		КонецПопытки;
	...
	Для Каждого ОбъектСоответствия Из ОбъектыДляОтложеннойЗаписи() Цикл
		...
		Попытка
			...
			Иначе
				...
				ОписаниеОшибки = НСтр("ka='რეკვიზიტების შევსების შემოწმების შეცდომა';ru='Ошибка проверки заполнения реквизитов';en='Error checking fill details'");
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("ka='ПередВыгрузкойДанных (конвертация)';ru='ПередВыгрузкойДанных (конвертация)';en='PeredVygruzkoyDannyh (conversion)'"));
			Отказ = Истина;
		КонецПопытки; 
	...
	Если Не Отказ И Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("ka='ПослеВыгрузкиДанных (конвертация)';ru='ПослеВыгрузкиДанных (конвертация)';en='PosleVygruzkiDannyh (conversion)'"));
		КонецПопытки;
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Если Не ПустаяСтрока(Конвертация.ПередПолучениемИзмененныхОбъектов) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(175, ОписаниеОшибки(), НСтр("ru='ПередПолучениемИзмененныхОбъектов (конвертация)';en='PeredPolucheniemIzmenennyhObektov (conversion)'"));
			Возврат;
		КонецПопытки;
		...
		Если ЭтоОбменЧерезВнешнееСоединение Тогда
			Если ЗагрузкаДанныхВыполняетсяВоВнешнемСоединении Тогда
				Если ОбработкаДляЗагрузкиДанных().ИспользоватьТранзакции Тогда
					...
					Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
						ВызватьИсключение(НСтр("ru='Ошибка при отправке данных.';en='Error sending data.'"));
					Иначе
						ВнешнееСоединение.ЗафиксироватьТранзакцию();
					КонецЕсли;
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru='Ошибка при отправке данных';en='Error sending data'") + ": " + ОписаниеОшибки());
