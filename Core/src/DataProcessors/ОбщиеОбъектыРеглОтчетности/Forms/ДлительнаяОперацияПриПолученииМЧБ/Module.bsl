		...
		Если ПолучитьФайлы(МассивПередаваемыхФайлов, , , Ложь) Тогда
			...
			Если КодВозврата = Неопределено ИЛИ ТекущаяДатаНаСервере() < ВремяОткрытияФайла + 10 Тогда
				Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = 
					НСтр("ka='Файл отчета открыт ассоциированным приложением...';ru='Файл отчета открыт ассоциированным приложением...';en='The report file is open the associated application ...'");
				ПоказатьПредупреждение(ОписаниеОповещения,
					НСтр("ka='Сформированный PDF-файл отчета будет удален после закрытия формы ожидания';ru='Сформированный PDF-файл отчета будет удален после закрытия формы ожидания';en='The generated PDF-File Report will be deleted after closing the waiting form'"), 10);
			Иначе
