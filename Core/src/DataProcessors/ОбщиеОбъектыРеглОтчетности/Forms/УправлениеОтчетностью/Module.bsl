	...
	ИначеЕсли ИмяСобытия = "Позиционирование в списке отчетов" И ТипЗнч(Параметр) = Тип("Структура") Тогда
		Если ТипЗнч(Параметр.Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если Элементы.ЖурналОтчетов.ТекущаяСтрока <> Параметр.Ссылка Тогда // позиционирование не произошло => мешает отбор
				ПоказатьПредупреждение(,НСтр("ka='Введенный отчет не отображен в журнале, так как не соответствует отбору.';ru='Введенный отчет не отображен в журнале, так как не соответствует отбору.';en='Introduced the report is not displayed in the log, because it does not correspond to the selection.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ОткрытьФормуОтчетаНаКлиенте(Отчет)
	...
	Если РезультатОткрытияФормыНаСервере = "Недостаточно прав" Тогда
		...
		ПоказатьПредупреждение(,НСтр("ka='არასაკმარისი უფლებები.';ru='Недостаточно прав.';en='Enough rights.'"));
	...
	ИначеЕсли РезультатОткрытияФормыНаСервере = "Не удалось открыть отчет" Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось открыть отчет.';ru='Не удалось открыть отчет.';en='Unable to open the report.'");
		Сообщение.Сообщить();
...
Процедура ОткрытьФормуПодробнееОбОтчете(Команда)
	...
	Если Элементы.ДеревоОтчетов.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='შეარჩიეთ ანგარიშგება.';ru='Выберите отчет.';en='Select the report.'"));
		Возврат;
	КонецЕсли;
	
	Если Элементы.ДеревоОтчетов.ТекущиеДанные.ЭтоГруппа = Истина Тогда
		ПоказатьПредупреждение(,НСтр("ka='Функция недоступна для группы отчетов.';ru='Функция недоступна для группы отчетов.';en='Feature is not available for the report group.'"));
		Возврат;
	КонецЕсли;
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Вывод формы на предпросмотр перед печатью прерван.';ru='%1 Вывод формы на предпросмотр перед печатью прерван.';en='1% Output forms to preview before printing is interrupted.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
"Создайте новый отчет за период 2016 год или позже.'");
	...
	Если РегламентированнаяОтчетностьКлиент.ВебБраузер() = "MSIE" Тогда
		Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "РСВ-1 ПФР"
			...
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для выгрузки (проверки выгрузки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""';ru='Для выгрузки (проверки выгрузки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""';en='For discharging (unloading check) in the browser ""Internet Explorer"" is necessary to open the regulated Report ""%1""'"),
	...
	Для каждого Строка Из Элементы.ЖурналОтчетов.ВыделенныеСтроки Цикл
		...
		Если СтрокаДанные.НаименованиеОтчета = "Статистика: Форма П-1 Приложение 3" Тогда 
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';ru='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';en='Unloading report ""Statistics: a form of P-1 (Annex 3) for the 1%"" is made from a form of P-1 for the period.'"), СтрокаДанные.ПредставлениеПериода);
			Сообщение.Сообщить();
			СтандартнаяОбработка = Ложь;
...
Процедура ВыгрузитьПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	НСтр("ka='%1';ru='%1'"), ДополнительныеПараметры.ТекстВопроса), РежимДиалогаВопрос.ДаНет);
...
Процедура ВыгрузитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ РегламентированнаяОтчетностьКлиент.ВыгрузитьОтчеты(
			...
			ПоказатьПредупреждение(,НСтр("ru='Выгрузка прервана.';en='Uploading aborted.'"));
		КонецЕсли;
	...
	Если СтандартнаяОбработка Тогда
		...
		Если ОтчетыДляПакетнойОбработки.Количество() <> 0 Тогда
			...
			Если НЕ РегламентированнаяОтчетностьКлиент.ПроверитьРегламентированныеОтчеты(ОтчетыДляПакетнойОбработки) Тогда
				ПоказатьПредупреждение(,НСтр("ru='Проверка прервана.';en='Checking interrupted.'"));
			КонецЕсли;
...
Процедура Сохранить(Команда)
	...
	Если Элементы.ЖурналВыгрузки.ТекущаяСтрока = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не выбран документ.';en='Do not select a document.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru='В выбранном документе нет текстов для сохранения в файл.';en='The selected document is not to save the text file.'"));
...
Функция КалендарьЗаполнен()
	...
	Если График.Количество() = 0 Тогда		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст =НСтр("ka='საწარმოო კალენდარი არ არის შევსებული.';ru='Не заполнен производственный календарь.';en='Not filled the manufacturing calendar.'");
		Сообщение.Сообщить();
		Возврат Ложь;	
	КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.';ru='%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.';en='1% Output forms to preview before printing a two-dimensional bar code PDF417 interrupted.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
Процедура ОтправкиФТСВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли КолонкаВыбора = "ОтправкиФТСПервичноеСообщение" Тогда
		...
		ПолучитьФайл(Адрес, НСтр("ka='ხელმოწერა';ru='Подпись';en='Signature'") + ИмяФайлаВыгрузки);
