	...
	ИначеЕсли ИмяСобытия = "Позиционирование в списке отчетов" И ТипЗнч(Параметр) = Тип("Структура") Тогда
		Если ТипЗнч(Параметр.Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если Элементы.ЖурналОтчетов.ТекущаяСтрока <> Параметр.Ссылка Тогда // позиционирование не произошло => мешает отбор
				ПоказатьПредупреждение(,НСтр("ka='Введенный отчет не отображен в журнале, так как не соответствует отбору.';ru='Введенный отчет не отображен в журнале, так как не соответствует отбору.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ОткрытьФормуОтчетаНаКлиенте(Отчет)
	...
	Если РезультатОткрытияФормыНаСервере = "Недостаточно прав" Тогда
		...
		ПоказатьПредупреждение(,НСтр("ka='არასაკმარისი უფლებები.';ru='Недостаточно прав.'"));
	...
	ИначеЕсли РезультатОткрытияФормыНаСервере = "Не удалось открыть отчет" Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось открыть отчет.';ru='Не удалось открыть отчет.'");
		Сообщение.Сообщить();
...
Процедура ОткрытьФормуПодробнееОбОтчете(Команда)
	...
	Если Элементы.ДеревоОтчетов.ТекущиеДанные.ЭтоГруппа = Истина Тогда
		ПоказатьПредупреждение(,НСтр("ka='Функция недоступна для группы отчетов.';ru='Функция недоступна для группы отчетов.'"));
		Возврат;
	КонецЕсли;
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Вывод формы на предпросмотр перед печатью прерван.';ru='%1 Вывод формы на предпросмотр перед печатью прерван.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
"Создайте новый отчет за период 2016 год или позже.'");
	...
	Для каждого Строка Из Элементы.ЖурналОтчетов.ВыделенныеСтроки Цикл
		...
		Если СтрокаДанные.НаименованиеОтчета = "Статистика: Форма П-1 Приложение 3" Тогда 
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';ru='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.'"), СтрокаДанные.ПредставлениеПериода);
			Сообщение.Сообщить();
			СтандартнаяОбработка = Ложь;
...
Функция КалендарьЗаполнен()
	...
	Если График.Количество() = 0 Тогда		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст =НСтр("ka='საწარმოო კალენდარი არ არის შევსებული.';ru='Не заполнен производственный календарь.'");
		Сообщение.Сообщить();
		Возврат Ложь;	
	КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.';ru='%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
Процедура ОтправкиФТСВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли КолонкаВыбора = "ОтправкиФТСПервичноеСообщение" Тогда
		...
		ПолучитьФайл(Адрес, НСтр("ka='ხელმოწერა';ru='Подпись'") + ИмяФайлаВыгрузки);
