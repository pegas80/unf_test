...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПечатаемыхЛистов.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Не выбраны листы для вывода на печать.';ru='Не выбраны листы для вывода на печать.';en='Is selected sheets for printing.'");
...
Процедура ВыгрузитьНаДиск(ВФормате)
	...
	Если КоличествоВыбранныхТабличныхДокументов() = 0 Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Выберите листы в дереве печатаемых листов.';ru='Выберите листы в дереве печатаемых листов.';en='Select the sheets to be printed in the tree leaves.'"));
	...
	Если ВФормате = ТипФайлаТабличногоДокумента.PDF Тогда
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Сохранение %1';ru='Сохранение %1';en='Saving 1%'"), ИмяФайла), , , БиблиотекаКартинок.Записать);
