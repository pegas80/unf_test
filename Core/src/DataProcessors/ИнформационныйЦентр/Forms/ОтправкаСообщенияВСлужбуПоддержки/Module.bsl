...
Процедура Отправить(Команда)
	...
	Если Элементы.АдресОтвета.Видимость Тогда 
		Если ПустаяСтрока(АдресДляОтвета) Тогда 
			ВызватьИсключение НСтр("ka='Необходимо ввести адрес электронной почты для ответа';ru='Необходимо ввести адрес электронной почты для ответа'");
		КонецЕсли;
		...
		Если Результат.Количество() = 0 Тогда 
			...
			ПоказатьВопрос(Оповещение, НСтр("ka='Адрес электронной почты возможно введен неверно. Отправить сообщение?';ru='Адрес электронной почты возможно введен неверно. Отправить сообщение?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ОтправитьСообщениеСервер();
	ПоказатьОповещениеПользователя(НСтр("ka='Сообщение в службу поддержки отправлено.';ru='Сообщение в службу поддержки отправлено.'"));
...
Процедура ЗаполнитьШаблономСодержание()
	...
	СтрокаПозицияКурсора = НСтр("ka='ПозицияКурсора';ru='ПозицияКурсора'");
...
Процедура ПоместитьФайлыСРасширением()
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл'");
...
Процедура ПоместитьФайлСРасширениемОповещениеРазмерОповещение(Размер, ДополнительныеПараметры) Экспорт
	...
	Если Не ОбщийРазмерФайловОптимален(Размер) Тогда 
		...
		ТекстПредупреждения = НСтр("ka='Не удалось добавить файл. Размер выбранных файлов превышает предел в %1 Мб';ru='Не удалось добавить файл. Размер выбранных файлов превышает предел в %1 Мб'");
	...
	Состояние(НСтр("ka='Файл добавляется к сообщению.';ru='Файл добавляется к сообщению.'"));
...
Процедура ПоместитьФайлыБезРасширенияНаСервере(АдресХранилища, ИмяИРасширениеФайла)
	...
	Если Не ОбщийРазмерФайловОптимален(РазмерФайла) Тогда 
		ТекстПредупреждения = НСтр("ka='Размер выбранных файлов превышает предел в %1 Мб';ru='Размер выбранных файлов превышает предел в %1 Мб'");
		ТекстПредупреждения = СтрШаблон(ТекстПредупреждения, МаксимальныйРазмерФайлов);
		ПоказатьСообщениеПользователю(ТекстПредупреждения);
...
Процедура ОтправитьСообщениеСервер()
    ...
    Если ПустаяСтрока(ТекстСообщения) Тогда 
		ВызватьИсключение НСтр("ka='Текст сообщения не может быть пустым.';ru='Текст сообщения не может быть пустым.'");
	КонецЕсли;
...
Функция СоздатьЭлементыФормыДляВложенногоФайла()
	...
	Для Каждого ВыбираемыйФайл Из ВыбираемыеФайлы Цикл
		...
		ПредставлениеФайла = ВыбираемыйФайл.ИмяФайла + ВыбираемыйФайл.Расширение + " (" + Окр(ВыбираемыйФайл.Размер / 1024, 2) + " " + НСтр("ka='კბ';ru='Кб'") +")";
		...
		КнопкаУдаленияФайла.Подсказка = НСтр("ka='ფაილის წაშლა';ru='Удалить файл'");
