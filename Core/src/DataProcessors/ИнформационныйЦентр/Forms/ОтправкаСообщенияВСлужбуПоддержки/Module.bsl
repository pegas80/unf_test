...
Процедура Отправить(Команда)
	...
	Если Элементы.АдресОтвета.Видимость Тогда 
		Если ПустаяСтрока(АдресДляОтвета) Тогда 
			ВызватьИсключение НСтр("ka='Необходимо ввести адрес электронной почты для ответа';ru='Необходимо ввести адрес электронной почты для ответа';en='You must enter an email address for response'");
		КонецЕсли;
		...
		Если Результат.Количество() = 0 Тогда 
			...
			ПоказатьВопрос(Оповещение, НСтр("ka='Адрес электронной почты возможно введен неверно. Отправить сообщение?';ru='Адрес электронной почты возможно введен неверно. Отправить сообщение?';en='E-mail address may incorrectly. Send a message?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ОтправитьСообщениеСервер();
	ПоказатьОповещениеПользователя(НСтр("ka='Сообщение в службу поддержки отправлено.';ru='Сообщение в службу поддержки отправлено.';en='Message sent to customer support.'"));
...
Процедура ЗаполнитьШаблономСодержание()
	...
	СтрокаПозицияКурсора = НСтр("ka='ПозицияКурсора';ru='ПозицияКурсора';en='PozitsiyaKursora'");
...
Процедура ПоместитьФайлСРасширениемОповещениеРазмерОповещение(Размер, ДополнительныеПараметры) Экспорт
	...
	Если Не ОбщийРазмерФайловОптимален(Размер) Тогда 
		...
		ТекстПредупреждения = НСтр("ka='Не удалось добавить файл. Размер выбранных файлов превышает предел в %1 Мб';ru='Не удалось добавить файл. Размер выбранных файлов превышает предел в %1 Мб';en='Unable to add file. The size of the selected file exceeds the limit of 1% MB'");
	...
	Состояние(НСтр("ka='Файл добавляется к сообщению.';ru='Файл добавляется к сообщению.';en='The file is added to the message.'"));
...
Процедура ПоместитьФайлыБезРасширенияНаСервере(АдресХранилища, ИмяИРасширениеФайла)
	...
	Если Не ОбщийРазмерФайловОптимален(РазмерФайла) Тогда 
		ТекстПредупреждения = НСтр("ka='Размер выбранных файлов превышает предел в %1 Мб';ru='Размер выбранных файлов превышает предел в %1 Мб';en='The size of the selected file exceeds the limit of 1% MB'");
		ТекстПредупреждения = СтрШаблон(ТекстПредупреждения, МаксимальныйРазмерФайлов);
		ПоказатьСообщениеПользователю(ТекстПредупреждения);
...
Процедура ОтправитьСообщениеСервер()
    ...
    Если ПустаяСтрока(ТекстСообщения) Тогда 
		ВызватьИсключение НСтр("ka='Текст сообщения не может быть пустым.';ru='Текст сообщения не может быть пустым.';en='Message text can not be empty.'");
	КонецЕсли;
...
Функция СоздатьЭлементыФормыДляВложенногоФайла()
	...
	Для Каждого ВыбираемыйФайл Из ВыбираемыеФайлы Цикл
		...
		ПредставлениеФайла = ВыбираемыйФайл.ИмяФайла + ВыбираемыйФайл.Расширение + " (" + Окр(ВыбираемыйФайл.Размер / 1024, 2) + " " + НСтр("ka='კბ';ru='Кб';en='KB'") +")";
		...
		КнопкаУдаленияФайла.Подсказка = НСтр("ka='ფაილის წაშლა';ru='Удалить файл';en='Delete a file'");
