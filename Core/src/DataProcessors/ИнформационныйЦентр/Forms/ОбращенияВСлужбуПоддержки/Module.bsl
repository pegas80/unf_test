...
Процедура ЗаполнитьОбращения(ПредставлениеСпискаОбращений)
	...
	Для Каждого ОбращениеОбъект Из ПредставлениеСпискаОбращений.Incidents Цикл 
		...
		НовоеОбращение.Наименование = ?(ПустаяСтрока(ОбращениеОбъект.Name), НСтр("ka='<Без темы>';ru='<Без темы>'"), ОбращениеОбъект.Name);
		...
		Если НовоеОбращение.КоличествоНепросмотренныхВзаимодействий <> 0 Тогда 
			...
			ЕстьОтвет = ?(НовоеОбращение.КоличествоНепросмотренныхВзаимодействий = 1, НСтр("ka='Непрочитанное';ru='Непрочитанное'"), НСтр("ka='Непрочитанные';ru='Непрочитанные'"));
