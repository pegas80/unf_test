...
Процедура ЗаполнитьОбращения(ПредставлениеСпискаОбращений)
	...
	Для Каждого ОбращениеОбъект Из ПредставлениеСпискаОбращений.Incidents Цикл 
		...
		НовоеОбращение.Наименование = ?(ПустаяСтрока(ОбращениеОбъект.Name), НСтр("ka='<Без темы>';ru='<Без темы>';en='<No Subject>'"), ОбращениеОбъект.Name);
		...
		Если НовоеОбращение.КоличествоНепросмотренныхВзаимодействий <> 0 Тогда 
			...
			ЕстьОтвет = ?(НовоеОбращение.КоличествоНепросмотренныхВзаимодействий = 1, НСтр("ka='Непрочитанное';ru='Непрочитанное';en='unread'"), НСтр("ka='Непрочитанные';ru='Непрочитанные';en='unread'"));
