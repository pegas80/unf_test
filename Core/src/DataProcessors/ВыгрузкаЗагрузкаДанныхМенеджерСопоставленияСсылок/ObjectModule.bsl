...
Процедура Инициализировать(Контейнер, ПотокЗаменыСсылок, Обработчики) Экспорт
	...
	Если ТекущееИмяКолонкиИсходныхСсылок = Неопределено Или ПустаяСтрока(ТекущееИмяКолонкиИсходныхСсылок) Тогда 
		ВызватьИсключение НСтр("ka='Не найдено имя колонки с исходной ссылкой';ru='Не найдено имя колонки с исходной ссылкой';en='We found the name of the column with the original link'");
	КонецЕсли;
