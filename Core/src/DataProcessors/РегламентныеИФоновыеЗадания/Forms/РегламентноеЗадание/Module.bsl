...
"выполняется только администраторами.'");
	...
	Если СтрНайти(", Добавить, Скопировать, Изменить,", ", " + Действие + ",") = 0 Тогда
		...
		ВызватьИсключение НСтр("ka='Неверные параметры открытия формы ""Регламентное задание"".';ru='Неверные параметры открытия формы ""Регламентное задание"".';en='Incorrect parameters open the form ""Scheduled task"".'");
	КонецЕсли;
	...
	Иначе
		...
		Если Задание.Метаданные = Неопределено Тогда
			ИмяМетаданных        = НСтр("ka='<нет метаданных>';ru='<нет метаданных>';en='<No metadata>'");
			СинонимМетаданных    = НСтр("ka='<нет метаданных>';ru='<нет метаданных>';en='<No metadata>'");
			ИмяМетодаМетаданных  = НСтр("ka='<нет метаданных>';ru='<нет метаданных>';en='<No metadata>'");
		Иначе
	...
	Если Действие <> "Изменить" Тогда
		Идентификатор = НСтр("ka='<будет создан при записи>';ru='<будет создан при записи>';en='<Will be created when recording>'");
		Использование = Ложь;
...
Процедура ВыборШаблонаНовогоРегламентногоЗадания()
	...
	ОписанияМетаданныхРегламентныхЗаданий.ПоказатьВыборЭлемента(
		...
		НСтр("ka='Выберите шаблон регламентного задания';ru='Выберите шаблон регламентного задания';en='Select the scheduled job template'"));
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Действие = "Изменить" Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (Регламентное задание)';ru='%1 (Регламентное задание)';en='1% (Routine Reference)'"), Представление);
	Иначе
		Заголовок = НСтр("ka='Регламентное задание (создание)';ru='Регламентное задание (создание)';en='Scheduled task (creating)'");
	КонецЕсли;
