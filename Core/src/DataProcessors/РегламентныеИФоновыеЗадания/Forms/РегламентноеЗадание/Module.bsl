...
"выполняется только администраторами.'");
	...
	Если СтрНайти(", Добавить, Скопировать, Изменить,", ", " + Действие + ",") = 0 Тогда
		...
		ВызватьИсключение НСтр("ka='Неверные параметры открытия формы ""Регламентное задание"".';ru='Неверные параметры открытия формы ""Регламентное задание"".'");
	КонецЕсли;
	...
	Иначе
		...
		Если Задание.Метаданные = Неопределено Тогда
			ИмяМетаданных        = НСтр("ka='<нет метаданных>';ru='<нет метаданных>'");
			СинонимМетаданных    = НСтр("ka='<нет метаданных>';ru='<нет метаданных>'");
			ИмяМетодаМетаданных  = НСтр("ka='<нет метаданных>';ru='<нет метаданных>'");
		Иначе
	...
	Если Действие <> "Изменить" Тогда
		Идентификатор = НСтр("ka='<будет создан при записи>';ru='<будет создан при записи>'");
		Использование = Ложь;
...
Процедура ВыборШаблонаНовогоРегламентногоЗадания()
	...
	ОписанияМетаданныхРегламентныхЗаданий.ПоказатьВыборЭлемента(
		...
		НСтр("ka='Выберите шаблон регламентного задания';ru='Выберите шаблон регламентного задания'"));
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Действие = "Изменить" Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (Регламентное задание)';ru='%1 (Регламентное задание)'"), Представление);
	Иначе
		Заголовок = НСтр("ka='Регламентное задание (создание)';ru='Регламентное задание (создание)'");
	КонецЕсли;
