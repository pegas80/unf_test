...
"выполняется только администраторами.';ka='Недостаточно прав доступа."
""
"Работа с регламентными и фоновыми заданиями"
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите одно регламентное задание.';ru='Выберите одно регламентное задание.';en='Select one routine task.'"));
		
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		ПоказатьПредупреждение(, НСтр("ka='Невозможно удалить предопределенное регламентное задание.';ru='Невозможно удалить предопределенное регламентное задание.';en='Unable to remove a predetermined routine task.'") );
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПередУдалениемЗавершение", ЭтотОбъект),
			НСтр("ka='Удалить регламентное задание?';ru='Удалить регламентное задание?';en='Delete scheduled jobs?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите регламентное задание.';ru='Выберите регламентное задание.';en='Select scheduled jobs.'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.Параметризуемое И КоличествоВыделенныхЗаданий = 1 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Выбранное регламентное задание нельзя выполнить вручную.';ru='Выбранное регламентное задание нельзя выполнить вручную.';en='you can not perform the selected routine task manually.'"));
			Возврат;
		...
		Если ПараметрыВыполнения.ЗапускВыполнен Тогда
			...
			ПоказатьОповещениеПользователя(
				НСтр("ka='Запущена процедура регламентного задания';ru='Запущена процедура регламентного задания';en='Procedure is running scheduled jobs'"), ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1."
"Процедура запущена в фоновом задании %2';ru='%1."
...
"  уже выполняется в фоновом задании ""%2"", начатом %3.';ka='Процедура регламентного задания ""%1"""
"  уже выполняется в фоновом задании ""%2"", начатом %3.'"),
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Задания выполнены с ошибками (%1 из %2)';ru='Задания выполнены с ошибками (%1 из %2)';en='Tasks performed with errors (% 1% 2)'"),
		...
		Если КоличествоОшибок > 5 Тогда
			...
			Кнопки.Добавить(1, НСтр("ka='Показать ошибки';ru='Показать ошибки';en='Show error'"));
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	Если Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите фоновое задание.';ru='Выберите фоновое задание.';en='Select a background job.'"));
	Иначе
		ОтменитьФоновоеЗаданиеНаСервере(Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные.Идентификатор);
		ЗагрузитьРегламентныеЗадания(, Истина);
		
		ПоказатьПредупреждение(,
			НСтр("ka='Задание отменено, но состояние отмены будет"
"установлено сервером только через секунды,"
"возможно потребуется обновить данные вручную.';ru='Задание отменено, но состояние отмены будет"
"установлено сервером только через секунды,"
...
Процедура ВыполнитьНеВФоне(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Выполнена процедура регламентного задания';ru='Выполнена процедура регламентного задания';en='Follow the procedures scheduled jobs'"), ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1."
"Процедура выполнена вручную без фонового задания';ru='%1."
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<>';ru='<>';en='<>'"));
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='<გაურკვეველია>';ru='<не определено>';en='<Not specified>'");
...
Процедура СообщитьОбОкончанииРучногоВыполненияРегламентногоЗадания()
	...
	Для каждого Оповещение Из ОповещенияОбОкончанииВыполнения Цикл
		...
		ПоказатьОповещениеПользователя(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1."
"Процедура завершена в фоновом задании %2';ru='%1."
...
Функция ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Формирование таблицы регламентных заданий';ru='Формирование таблицы регламентных заданий';en='Formation of the scheduled jobs table'");
...
Функция СформироватьТаблицуФоновыхЗаданийВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Регламентные задания. Обновление списка фоновых заданий';ru='Регламентные задания. Обновление списка фоновых заданий';en='Scheduled jobs. Updating the list of background jobs'");
	...
	Для каждого Задание Из ТекущаяТаблица Цикл
		...
		Если ЗначениеЗаполнено(Обновляемое.ИдентификаторРегламентногоЗадания) Тогда
			...
			Обновляемое.НаименованиеРегламентногоЗадания
				= ?(Строки.Количество() = 0, НСтр("ka='<не найдено>';ru='<не найдено>';en='<Not found>'"), Строки[0].Наименование);
		Иначе
