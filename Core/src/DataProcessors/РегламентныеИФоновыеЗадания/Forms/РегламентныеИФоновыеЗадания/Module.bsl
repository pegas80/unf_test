...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите одно регламентное задание.';ru='Выберите одно регламентное задание.'"));
		
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		ПоказатьПредупреждение(, НСтр("ka='Невозможно удалить предопределенное регламентное задание.';ru='Невозможно удалить предопределенное регламентное задание.'") );
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПередУдалениемЗавершение", ЭтотОбъект),
			НСтр("ka='Удалить регламентное задание?';ru='Удалить регламентное задание?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите регламентное задание.';ru='Выберите регламентное задание.'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.Параметризуемое И КоличествоВыделенныхЗаданий = 1 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Выбранное регламентное задание нельзя выполнить вручную.';ru='Выбранное регламентное задание нельзя выполнить вручную.'"));
			Возврат;
		...
		Если ПараметрыВыполнения.ЗапускВыполнен Тогда
			...
			ПоказатьОповещениеПользователя(
				НСтр("ka='Запущена процедура регламентного задания';ru='Запущена процедура регламентного задания'"), ,
...
"  уже выполняется в фоновом задании ""%2"", начатом %3.'"),
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Задания выполнены с ошибками (%1 из %2)';ru='Задания выполнены с ошибками (%1 из %2)'"),
		...
		Если КоличествоОшибок > 5 Тогда
			...
			Кнопки.Добавить(1, НСтр("ka='Показать ошибки';ru='Показать ошибки'"));
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	Если Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите фоновое задание.';ru='Выберите фоновое задание.'"));
	Иначе
...
Процедура ВыполнитьНеВФоне(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Выполнена процедура регламентного задания';ru='Выполнена процедура регламентного задания'"), ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1."
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<>';ru='<>'"));
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='<გაურკვეველია>';ru='<не определено>'");
...
Функция ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Формирование таблицы регламентных заданий';ru='Формирование таблицы регламентных заданий'");
...
Функция СформироватьТаблицуФоновыхЗаданийВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Регламентные задания. Обновление списка фоновых заданий';ru='Регламентные задания. Обновление списка фоновых заданий'");
	...
	Для каждого Задание Из ТекущаяТаблица Цикл
		...
		Если ЗначениеЗаполнено(Обновляемое.ИдентификаторРегламентногоЗадания) Тогда
			...
			Обновляемое.НаименованиеРегламентногоЗадания
				= ?(Строки.Количество() = 0, НСтр("ka='<не найдено>';ru='<не найдено>'"), Строки[0].Наименование);
		Иначе
