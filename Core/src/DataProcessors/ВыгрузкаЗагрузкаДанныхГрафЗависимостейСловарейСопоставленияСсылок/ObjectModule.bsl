...
Процедура ДобавитьВершину(Знач ИмяОбъектаМетаданных, Знач ТолькоПриОтсутствии = Истина) Экспорт
	...
	Если УжеСуществует Тогда
		...
		Если ТолькоПриОтсутствии Тогда
			Возврат;
		Иначе
			ВызватьИсключение НСтр("ka='Попытка дублирования.';ru='Попытка дублирования.';en='Trying to duplicate.'");
		КонецЕсли;
...
Функция ОбъектМетаданных(Знач ПолноеИмя)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='В текущей конфигурации отсутствует объект метаданных %1, присутствующих в файле данных.';ru='В текущей конфигурации отсутствует объект метаданных %1, присутствующих в файле данных.';en='In the current configuration, there is no metadata object 1%, are present in the data file.'"),
			ПолноеИмя);
...
Функция Вершина(Знач ОбъектМетаданных, Знач ИсключениеПриОтсутствии = Истина)
	...
	ИначеЕсли Строки.Количество() = 0 Тогда
		...
		Если ИсключениеПриОтсутствии Тогда
			...
			ВызватьИсключение СтрШаблон(НСтр("ka='В графе отсутствует вершина для объекта метаданных %1.';ru='В графе отсутствует вершина для объекта метаданных %1.';en='The column is not the top of the metadata object% 1.'"),
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='Нарушение уникальности граф для объекта метаданных %1.';ru='Нарушение уникальности граф для объекта метаданных %1.';en='Violation Count unique metadata object% 1.'"),
			ОбъектМетаданных.ПолноеИмя());
...
Процедура ПоискВГлубину(Вершина, РезультатСортировки)
	...
	Если Вершина.Цвет = Серый Тогда
		...
		ВызватьИсключение НСтр("ka='Рекурсивная зависимость.';ru='Рекурсивная зависимость.';en='Recursive relationship.'");
