...
Функция ПараметрыВыгрузки() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Контейнер не инициализирован для выгрузки данных.';ru='Контейнер не инициализирован для выгрузки данных.';en='The container has not been initialized to upload data.'");
	КонецЕсли;
...
Процедура ИсключитьФайл(Знач ПолныйПутьКФайлу) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Файл %1 не найден в составе контейнера.';ru='Файл %1 не найден в составе контейнера.';en='The file% 1 could not be found in the composition of the container.'"), ПолныйПутьКФайлу);
	Иначе
...
Процедура ЗаменитьФайл(Знач ИмяВКонтейнере, Знач ПолныйПутьКФайлу, Знач УдалятьФайлЗаменыСДиска = Ложь) Экспорт
	...
	Если СтрокаИсходногоФайла = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Файл с идентификатором %1 не найден в составе контейнера.';ru='Файл с идентификатором %1 не найден в составе контейнера.';en='File ID% 1 was not found in the composition of the container.'"), ИмяВКонтейнере);
	Иначе
...
Процедура ОбновитьСоставВыгружаемыхФайлов()
	...
	Для Каждого ТекущийФайл Из Состав Цикл 
		...
		Если Не Файл.Существует() Тогда 
			ВызватьИсключение НСтр("ka='Файл был удален';ru='Файл был удален';en='The file was deleted'");
		КонецЕсли;
...
"Возможно, файл был получен из предыдущих версий программы или поврежден.';ka='Ошибка загрузки данных. Неверный формат файла. В архиве не обнаружен файл PackageContents.xml."
"Возможно, файл был получен из предыдущих версий программы или поврежден.'");
	...
	Если ПотокЧтения.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='XML-ის წაკითხვის შეცდომა. ფაილის არასწორი ფორმატი. მოსალოდნელია ელემენტ %1-ის დაწყება.';ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента %1.';en='XML reading error. Invalid file format. Start of the element 1%.'"),
			"Data");
...
Функция ПараметрыЗагрузки() Экспорт
	...
	Если ДляЗагрузки Тогда
		Возврат Новый ФиксированнаяСтруктура(Параметры);
	Иначе
		ВызватьИсключение НСтр("ka='Контейнер не инициализирован для загрузки данных.';ru='Контейнер не инициализирован для загрузки данных.';en='The container has not been initialized for data loading.'");
	КонецЕсли;
...
Функция ПолучитьФайлИзКаталога(Знач ВидФайла, Знач ТипДанных = Неопределено) Экспорт
	...
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ka='В выгрузке содержится дублирующаяся информация';ru='В выгрузке содержится дублирующаяся информация';en='The discharge contains the same information'");
	КонецЕсли;
...
Функция ПолучитьПроизвольныйФайл(Знач ТипДанных = Неопределено) Экспорт
	...
	Если Файлы.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='В выгрузке отсутствует произвольный файл с типом данным %1.';ru='В выгрузке отсутствует произвольный файл с типом данным %1.';en='The unloading offline arbitrary file type data 1%.'"),
			ТипДанных);
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ka='В выгрузке содержится дублирующаяся информация';ru='В выгрузке содержится дублирующаяся информация';en='The discharge contains the same information'");
	КонецЕсли;
...
Функция ПолучитьОписанияФайловИзКаталога(Знач ВидФайла, Знач ТипДанных = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='Неизвестный вид файла';ru='Неизвестный вид файла';en='Unknown file type'");
...
Функция ПолучитьПолноеИмяФайла(Знач ОтносительноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='В контейнере не обнаружен файл с относительным именем %1.';ru='В контейнере не обнаружен файл с относительным именем %1.';en='The container is not found a file with a relative named% 1.'"),
...
Функция ПолучитьОтносительноеИмяФайла(Знач ПолноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='В контейнере не обнаружен файл %1.';ru='В контейнере не обнаружен файл %1.';en='The container is not detected the file% 1.'"),
			ПолноеИмяФайла);
	Иначе
...
Процедура ПроверкаИнициализацииКонтейнера(Знач ПриИнициализации = Ложь)
	...
	Если ДляВыгрузки И ДляЗагрузки Тогда
		ВызватьИсключение НСтр("ka='Некорректная инициализация контейнера.';ru='Некорректная инициализация контейнера.';en='Incorrect initialization container.'");
	КонецЕсли;
	
	Если ПриИнициализации Тогда
		
		Если КонтейнерИнициализирован <> Неопределено И КонтейнерИнициализирован Тогда
			ВызватьИсключение НСтр("ka='Контейнер выгрузки уже был инициализирован ранее.';ru='Контейнер выгрузки уже был инициализирован ранее.';en='unloading the container has already been initialized earlier.'");
		КонецЕсли;
		
	Иначе
		
		Если Не КонтейнерИнициализирован Тогда
			ВызватьИсключение НСтр("ka='Контейнер выгрузки не инициализирован.';ru='Контейнер выгрузки не инициализирован.';en='unloading the container is not initialized.'");
		КонецЕсли;
...
Функция ПолучитьКаталогДляРазмещенияФайла(Знач ВидФайла)
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='Вид файла %1 не поддерживается.';ru='Вид файла %1 не поддерживается.';en='Type of file% 1 is not supported.'"), ВидФайла);
	КонецЕсли;
