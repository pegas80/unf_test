...
Функция ПараметрыВыгрузки() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Контейнер не инициализирован для выгрузки данных.';ru='Контейнер не инициализирован для выгрузки данных.'");
	КонецЕсли;
...
Процедура ИсключитьФайл(Знач ПолныйПутьКФайлу) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Файл %1 не найден в составе контейнера.';ru='Файл %1 не найден в составе контейнера.'"), ПолныйПутьКФайлу);
	Иначе
...
Процедура ЗаменитьФайл(Знач ИмяВКонтейнере, Знач ПолныйПутьКФайлу, Знач УдалятьФайлЗаменыСДиска = Ложь) Экспорт
	...
	Если СтрокаИсходногоФайла = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Файл с идентификатором %1 не найден в составе контейнера.';ru='Файл с идентификатором %1 не найден в составе контейнера.'"), ИмяВКонтейнере);
	Иначе
...
Процедура ОбновитьСоставВыгружаемыхФайлов()
	...
	Для Каждого ТекущийФайл Из Состав Цикл 
		...
		Если Не Файл.Существует() Тогда 
			ВызватьИсключение НСтр("ka='Файл был удален';ru='Файл был удален'");
		КонецЕсли;
...
"Возможно, файл был получен из предыдущих версий программы или поврежден.'");
	...
	Если ПотокЧтения.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='XML-ის წაკითხვის შეცდომა. ფაილის არასწორი ფორმატი. მოსალოდნელია ელემენტ %1-ის დაწყება.';ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента %1.'"),
			"Data");
...
Функция ПараметрыЗагрузки() Экспорт
	...
	Если ДляЗагрузки Тогда
		Возврат Новый ФиксированнаяСтруктура(Параметры);
	Иначе
		ВызватьИсключение НСтр("ka='Контейнер не инициализирован для загрузки данных.';ru='Контейнер не инициализирован для загрузки данных.'");
	КонецЕсли;
...
Функция ПолучитьФайлИзКаталога(Знач ВидФайла, Знач ТипДанных = Неопределено) Экспорт
	...
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ka='В выгрузке содержится дублирующаяся информация';ru='В выгрузке содержится дублирующаяся информация'");
	КонецЕсли;
...
Функция ПолучитьПроизвольныйФайл(Знач ТипДанных = Неопределено) Экспорт
	...
	Если Файлы.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='В выгрузке отсутствует произвольный файл с типом данным %1.';ru='В выгрузке отсутствует произвольный файл с типом данным %1.'"),
			ТипДанных);
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ka='В выгрузке содержится дублирующаяся информация';ru='В выгрузке содержится дублирующаяся информация'");
	КонецЕсли;
...
Функция ПолучитьОписанияФайловИзКаталога(Знач ВидФайла, Знач ТипДанных = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='Неизвестный вид файла';ru='Неизвестный вид файла'");
...
Функция ПолучитьПолноеИмяФайла(Знач ОтносительноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='В контейнере не обнаружен файл с относительным именем %1.';ru='В контейнере не обнаружен файл с относительным именем %1.'"),
...
Функция ПолучитьОтносительноеИмяФайла(Знач ПолноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='В контейнере не обнаружен файл %1.';ru='В контейнере не обнаружен файл %1.'"),
			ПолноеИмяФайла);
	Иначе
...
Процедура ПроверкаИнициализацииКонтейнера(Знач ПриИнициализации = Ложь)
	...
	Если ДляВыгрузки И ДляЗагрузки Тогда
		ВызватьИсключение НСтр("ka='Некорректная инициализация контейнера.';ru='Некорректная инициализация контейнера.'");
	КонецЕсли;
	
	Если ПриИнициализации Тогда
		
		Если КонтейнерИнициализирован <> Неопределено И КонтейнерИнициализирован Тогда
			ВызватьИсключение НСтр("ka='Контейнер выгрузки уже был инициализирован ранее.';ru='Контейнер выгрузки уже был инициализирован ранее.'");
		КонецЕсли;
		
	Иначе
		
		Если Не КонтейнерИнициализирован Тогда
			ВызватьИсключение НСтр("ka='Контейнер выгрузки не инициализирован.';ru='Контейнер выгрузки не инициализирован.'");
		КонецЕсли;
...
Функция ПолучитьКаталогДляРазмещенияФайла(Знач ВидФайла)
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='Вид файла %1 не поддерживается.';ru='Вид файла %1 не поддерживается.'"), ВидФайла);
	КонецЕсли;
