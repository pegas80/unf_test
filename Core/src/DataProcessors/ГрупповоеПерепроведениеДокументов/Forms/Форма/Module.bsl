...
Процедура КнопкаВыполнить(Команда)
	...
	Если ТолькоВыбранныеОрганизации = 1 И НЕ ЗначениеЗаполнено(ТаблицаОрганизаций) Тогда
		ТекстСообщения = НСтр("ka='Не корректно заполнен список организаций.';ru='Не корректно заполнен список организаций.'");
...
Процедура ИнициализацияКомандДокументаНаКлиенте()
	...
	Если Результат = 1 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='Перепроведение уже выполняется. Ожидайте завершения.';ru='Перепроведение уже выполняется. Ожидайте завершения.'"));
		Возврат;
	ИначеЕсли Результат = 2 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='Перепроведение уже выполняется другим пользователем.';ru='Перепроведение уже выполняется другим пользователем.'"));
		Возврат;
	КонецЕсли;
...
Функция ВыполнитьНаСервере()
	...
	Иначе
		НаименованиеЗадания = НСтр("ka='დოკუმენტების ჯგუფური ხელახლა გატარება';ru='Групповое перепроведение документов'");
