...
Процедура ГрафическаяСхемаВыбор(Элемент)
	...
	ИначеЕсли ИмяЭлемента = "СправочникПодключаемоеОборудование" Тогда
		...
		Иначе
			...
			ПоказатьВопрос(ОбовещениеПослеОтветаНаВопрос, НСтр("ka='Необходимо предварительно включить опцию ""Подключаемое оборудование"". Перейти к настройкам опций?';ru='Необходимо предварительно включить опцию ""Подключаемое оборудование"". Перейти к настройкам опций?'"), РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
	...
	ИначеЕсли СтрНайти(ИмяЭлемента, "Обработка") > 0 Тогда
		...
		Если ИмяЭлемента = "ОбработкаДисконтныеКарты" И НЕ ИспользоватьДисконтныеКарты Тогда
			...
			ПоказатьВопрос(ОбовещениеПослеОтветаНаВопрос, НСтр("ka='Необходимо предварительно включить опцию ""Дисконтные карты"". Перейти к настройкам опций?';ru='Необходимо предварительно включить опцию ""Дисконтные карты"". Перейти к настройкам опций?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
