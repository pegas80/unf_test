...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Заголовок") Тогда
		Заголовок = НСтр("ru='Подсказка: ';ka='Подсказка: '") + Параметры.Заголовок;
	КонецЕсли;
...
Процедура ГрафическаяСхемаВыбор(Элемент)
	...
	ИначеЕсли ИмяЭлемента = "СправочникПодключаемоеОборудование" Тогда
		...
		Иначе
			...
			ПоказатьВопрос(ОбовещениеПослеОтветаНаВопрос, НСтр("ka='Необходимо предварительно включить опцию ""Подключаемое оборудование"". Перейти к настройкам опций?';ru='Необходимо предварительно включить опцию ""Подключаемое оборудование"". Перейти к настройкам опций?';en='""The connected equipment"" necessary to enable the option in advance. Go to the settings options?'"), РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
	...
	ИначеЕсли СтрНайти(ИмяЭлемента, "Обработка") > 0 Тогда
		...
		Если ИмяЭлемента = "ОбработкаДисконтныеКарты" И НЕ ИспользоватьДисконтныеКарты Тогда
			...
			ПоказатьВопрос(ОбовещениеПослеОтветаНаВопрос, НСтр("ka='Необходимо предварительно включить опцию ""Дисконтные карты"". Перейти к настройкам опций?';ru='Необходимо предварительно включить опцию ""Дисконтные карты"". Перейти к настройкам опций?';en='""Discount card"" is necessary to enable the option in advance. Go to the settings options?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
...
"Перейти к настройкам опций?';ka='Необходимо предварительно включить опцию ""Скидки и наценки в продажах"" или ""Автоматические скидки"". "
