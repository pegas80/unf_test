...
Процедура ПопробоватьУдалитьСсылку(ПараметрыВыполнения, Ссылка, Информация, Результат)
	...
	Если Объект.ПометкаУдаления <> Истина Тогда
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("ka='Объект не помечен на удаление.';ru='Объект не помечен на удаление.'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("ka='Объект используется в других объектах программы.';ru='Объект используется в других объектах программы.'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьПредупреждение(Ссылка, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='მონიშნულების წაშლა';ru='Удаление помеченных'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если СтрокаТаблицы.ОбнаруженныйСсылка = Неопределено Тогда
		...
		Иначе
			Причина.ОбнаруженныйСсылка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Обнаружены неразрешимые ссылки (%1)';ru='Обнаружены неразрешимые ссылки (%1)'"),
...
Процедура КлиентОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Если ИмяКоллекции = "ПередПоискомПомеченныхНаУдаление" Тогда
		...
		Текст = НСтр("ka='Подготовка к поиску объектов, помеченных на удаление.';ru='Подготовка к поиску объектов, помеченных на удаление.'");
		
	ИначеЕсли ИмяКоллекции = "НайтиПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("ka='Поиск объектов, помеченных на удаление.';ru='Поиск объектов, помеченных на удаление.'");
		
	ИначеЕсли ИмяКоллекции = "ВсеПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("ka='Анализ помеченных на удаление.';ru='Анализ помеченных на удаление.'");
		
	ИначеЕсли ИмяКоллекции = "ТехнологическиеОбъекты" Тогда
		
		Текст = НСтр("ka='Подготовка к удалению.';ru='Подготовка к удалению.'");
		
	ИначеЕсли ИмяКоллекции = "МонопольноеУдаление" Тогда
		
		Текст = НСтр("ka='Выполняется удаление объектов.';ru='Выполняется удаление объектов.'");
	...
	ИначеЕсли ИмяКоллекции = "ПользовательскиеОбъекты" Тогда
		...
		Если НеУдалено = 0 Тогда // Переход на СтрШаблон невозможен.
			Текст = НСтр("ka='Удалено: %1 из %2 объектов.';ru='Удалено: %1 из %2 объектов.'");
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ПредставлениеНомер, ПредставлениеВсего);
		Иначе
			Текст = НСтр("ka='Обработано: %1 из %2 объектов, из них не удалено: %3.';ru='Обработано: %1 из %2 объектов, из них не удалено: %3.'");
	...
	ИначеЕсли ИмяКоллекции = "ПовторноУдаляемые" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Повторная проверка не удаленных объектов: %1 из %2.';ru='Повторная проверка не удаленных объектов: %1 из %2.'"),
	...
	ИначеЕсли ИмяКоллекции = "ПрепятствующиеУдалению" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Анализ объектов, препятствующих удалению: %1 из %2.';ru='Анализ объектов, препятствующих удалению: %1 из %2.'"),
			Формат(ПараметрыВыполнения.Номер, "ЧН=0; ЧГ="),
			Формат(ПараметрыВыполнения.Всего, "ЧН=0; ЧГ="));
