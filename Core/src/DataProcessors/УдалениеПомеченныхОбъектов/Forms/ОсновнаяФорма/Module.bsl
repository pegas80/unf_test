...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = НСтр("ka='ოპერაციის შესასრულებლად უფლებები არასაკმარისია.';ru='Недостаточно прав для выполнения операции.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		И Не ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
		ТекстОшибки = НСтр("ka='Для удаления помеченных необходимо войти в область данных.';ru='Для удаления помеченных необходимо войти в область данных.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
"Прервать?'");
	...
	Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='Не прерывать';ru='Не прерывать'"));
...
Процедура НеУдаленныеЗаменитьНа(Команда)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите объекты';ru='Выберите объекты'"));
		Возврат;
	КонецЕсли;
...
"по которой его не удалось удалить.'");
	...
	Иначе
		...
		Шаблон = НСтр("ka='Места использования объекта ""%1"" (%2):';ru='Места использования объекта ""%1"" (%2):'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		ТекстОшибки = НСтр("ka='Выберите объект.';ru='Выберите объект.'");
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ka='Удаление записи регистра выполняется из ее карточки.';ru='Удаление записи регистра выполняется из ее карточки.'");
			ИначеЕсли ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ka='Очистка значения константы выполняется из ее карточки.';ru='Очистка значения константы выполняется из ее карточки.'");
			КонецЕсли;
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ka='Удаление записей регистров или очистка значений констант выполняется из их карточек.';ru='Удаление записей регистров или очистка значений констант выполняется из их карточек.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Если КоличествоМожноУдалить = 1 Тогда
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ka='მოვნიშნოთ ""%1"" წასაშლელად?';ru='Пометить ""%1"" на удаление?'");
		Иначе
			ТекстВопроса = НСтр("ka='%1-ს მოვხსნათ წაშლის ნიშნული?';ru='Снять с ""%1"" пометку на удаление?'");
		КонецЕсли;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, МассивСтрокТаблицы[0].Представление);
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ka='Пометить выделенные объекты (%1) на удаление?';ru='Пометить выделенные объекты (%1) на удаление?'");
		Иначе
			ТекстВопроса = НСтр("ka='Снять с выделенных объектов (%1) пометку на удаление?';ru='Снять с выделенных объектов (%1) пометку на удаление?'");
		КонецЕсли;
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если КоличествоОбъектов = 0 Тогда
		ОповещениеЗаголовок = НСтр("ka='Объект не найден';ru='Объект не найден'");
	Иначе
		Если ПараметрыВыполнения.УстанавливатьПометку Тогда
			ОповещениеЗаголовок = НСтр("ka='Пометка удаления установлена';ru='Пометка удаления установлена'");
		Иначе
			ОповещениеЗаголовок = НСтр("ka='Пометка удаления снята';ru='Пометка удаления снята'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	Элементы.КнопкаДалее.Заголовок = НСтр("ka='წაშლა';ru='Удалить'");
	Элементы.КнопкаЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
	Элементы.КнопкаНазад.Заголовок = НСтр("ka='< В начало';ru='< В начало'");
	...
	Если ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаНазад.Заголовок = НСтр("ka='< უკან';ru='< Назад'");
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ka='წაშლა';ru='Удалить'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ka='Прервать и закрыть';ru='Прервать и закрыть'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ka='Повторить удаление';ru='Повторить удаление'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	...
	Для Каждого Причина Из ПрепятствующиеУдалению Цикл
		...
		Иначе
			...
			Если Причина.ОбнаруженныйСсылка = Неопределено Тогда // Константа
				...
				СтрокаПрепятствующегоУдалению.Представление = ОбнаруженныйИнформация.ПредставлениеЭлемента + " (" + НСтр("ka='კონსტანტა';ru='Константа'") + ")";
			Иначе
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("ka='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';ru='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").'");
		КонецЕсли;
	Иначе
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Текст = НСтр("ka='Удаляются объекты, помеченные на удаление...';ru='Удаляются объекты, помеченные на удаление...'");
	Если ПараметрыМетода.ИскатьПомеченные И ПараметрыМетода.УдалятьПомеченные Тогда
		Состояние(НСтр("ka='Поиск и удаление помеченных объектов...';ru='Поиск и удаление помеченных объектов...'"));
	ИначеЕсли ПараметрыМетода.ИскатьПомеченные Тогда
		Текст = НСтр("ka='Поиск помеченных на удаление объектов...';ru='Поиск помеченных на удаление объектов...'");
		Состояние(Текст);
	Иначе
		Состояние(НСтр("ka='Удаление выбранных объектов...';ru='Удаление выбранных объектов...'"));
	КонецЕсли;
	...
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			...
			ПараметрыПредупреждения.Текст    = НСтр("ka='Не удалось установить монопольный режим';ru='Не удалось установить монопольный режим'");
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	Если ПоказыватьБублик Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ka='გთხოვთ, მოიცადეთ...';ru='Пожалуйста, подождите...'");
	...
	ИначеЕсли ПараметрыМетода.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ka='Удаляются объекты, помеченные на удаление...';ru='Удаляются объекты, помеченные на удаление...'");
	...
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ka='Поиск объектов, помеченных на удаление...';ru='Поиск объектов, помеченных на удаление...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='Удаление помеченных объектов (интерактивное)';ru='Удаление помеченных объектов (интерактивное)'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ka='Не удалось прочитать результат удаления помеченных';ru='Не удалось прочитать результат удаления помеченных'"), "");
			Возврат;
		КонецЕсли;
		...
		Если Не ПустаяСтрока(Результат.ОповещениеТекст) Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='მონიშნულების წაშლა';ru='Удаление помеченных'"),
				НавигационнаяСсылка,
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Удалено объектов: %1.';ru='Удалено объектов: %1.'"),
		...
		Иначе
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Удалено: %1, не удалено: %2.';ru='Удалено: %1, не удалено: %2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Не получилось удалить объекты, помеченные на удаление (%1):';ru='Не получилось удалить объекты, помеченные на удаление (%1):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';ru='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ka='При удалении помеченных возникла ошибка:';ru='При удалении помеченных возникла ошибка:'") + Символы.ПС + КраткоеПредставлениеОшибки;
