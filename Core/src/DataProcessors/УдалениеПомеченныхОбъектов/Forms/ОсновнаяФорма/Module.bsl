...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = НСтр("ru = 'Недостаточно прав для выполнения операции.'; ka = 'ოპერაციის შესასრულებლად უფლებები არასაკმარისია.'; en = 'Insufficient permissions to perform the operation.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		И Не ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
		ТекстОшибки = НСтр("ru = 'Для удаления помеченных необходимо войти в область данных.'; ka = 'მონიშნულების წასაშლელად, თქვენ უნდა შეიყვანოთ მონაცემთა ზონა.'; en = 'To remove the marked log in the data area.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru = 'Удаление помеченных еще выполняется.""Прервать?'; ka = 'მონიშნულების წაშლა ჯერ კიდევ მიმდინარეობს.""შეწყდეს?'; en = 'Deleting the marked ones is still 
                         |in progress.""Abort?'");
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Прервать);
	Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ru = 'Не прерывать'; ka = 'არ შეწყდეს'; en = 'Do not interrupt'"));
...
Процедура НеУдаленныеЗаменитьНа(Команда)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите объекты'; ka = 'აირჩიეთ ობიექტები'; en = 'Select objects'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьСвязиНеУдаленныхНаСервере()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
		...
		НеУдаленныеПодсказка = НСтр("ru = 'Выберите объект, чтобы узнать причину,""по которой его не удалось удалить.'; ka = 'აირჩიეთ ობიექტი, რათა გაარკვიოთ მისი არ წაშლის მიზეზი.'; en = 'Select an object to find out the reason why it could not be deleted.'");
	Иначе
	...
	Иначе
		...
		Шаблон = НСтр("ru = 'Места использования объекта ""%1"" (%2):'; ka = 'ადგილები, სადაც გამოიყენება ობიექტი ""%1"" (%2):'; en = 'Designated object using ""%1"" (2%):'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		ТекстОшибки = НСтр("ru = 'Выберите объект.'; ka = 'აირჩიეთ ობიექტი.'; en = 'Select the object.'");
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ru = 'Удаление записи регистра выполняется из ее карточки.'; ka = 'რეგისტრის ჩანაწერი წაიშლება მისი ბარათიდან.'; en = 'Removing performed register-writing of her cards.'");
			ИначеЕсли ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ru = 'Очистка значения константы выполняется из ее карточки.'; ka = 'კონსტანტის მნიშვნელობის გასუფთავება ხდება მისი ბარათიდან.'; en = 'Cleaning the value of the constant is made of her cards.'");
			КонецЕсли;
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ru = 'Удаление записей регистров или очистка значений констант выполняется из их карточек.'; ka = 'რეგისტრის ჩანაწერების წაშლა ან კონსტანტის მნიშვნელობების გასუფთავება ხდება მათი ბარათებიდან.'; en = 'Deleting records or registers cleaning constants runs from their cards.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Если КоличествоМожноУдалить = 1 Тогда
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ka='მოვნიშნოთ ""%1"" წასაშლელად?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for removal?'");
		Иначе
			ТекстВопроса = НСтр("ka='%1-ს მოვხსნათ წაშლის ნიშნული?';ru='Снять с ""%1"" пометку на удаление?';en='Remove from the ""%1"" is marked for deletion?'");
		КонецЕсли;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, МассивСтрокТаблицы[0].Представление);
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ru = 'Пометить выделенные объекты (%1) на удаление?'; ka = 'მონიშნოთ არჩეული ობიექტები (%1) წასაშლელად?'; en = 'Mark selected item (% 1) to remove?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Снять с выделенных объектов (%1) пометку на удаление?'; ka = 'გსურთ ამოიღოთ მონიშნული ობიექტები (%1) წასაშლელად?'; en = 'Remove from selected objects (% 1) marked for deletion?'");
		КонецЕсли;
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если КоличествоОбъектов = 0 Тогда
		ОповещениеЗаголовок = НСтр("ru = 'Объект не найден'; ka = 'ობიექტი ვერ მოიძებნა'; en = 'Object not found'");
	Иначе
		Если ПараметрыВыполнения.УстанавливатьПометку Тогда
			ОповещениеЗаголовок = НСтр("ru = 'Пометка удаления установлена'; ka = 'წაშლის ნიშნული დაყენებულია'; en = 'Marking removal set'");
		Иначе
			ОповещениеЗаголовок = НСтр("ru = 'Пометка удаления снята'; ka = 'წაშლის ნიშნული მოხსნილია'; en = 'Marking removal filmed'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	Элементы.КнопкаДалее.Заголовок = НСтр("ka='წაშლა';ru='Удалить';en='Delete'");
	Элементы.КнопкаЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
	Элементы.КнопкаНазад.Заголовок = НСтр("ru = '< В начало'; ka = '< დასაწყისში'; en = '< In the beginning'");
	...
	Если ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаНазад.Заголовок = НСтр("ka='< უკან';ru='< Назад';en='<Back'");
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ka='წაშლა';ru='Удалить';en='Delete'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ru = 'Прервать и закрыть'; ka = 'შეწყვიტე და დახურე'; en = 'Interrupt and close'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ru = 'Повторить удаление'; ka = 'გაიმეორეთ წაშლა'; en = 'repeat removal'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	...
	Для Каждого Причина Из ПрепятствующиеУдалению Цикл
		...
		Иначе
			...
			Если Причина.ОбнаруженныйСсылка = Неопределено Тогда // Константа
				...
				СтрокаПрепятствующегоУдалению.Представление = ОбнаруженныйИнформация.ПредставлениеЭлемента + " (" + НСтр("ka='კონსტანტა';ru='Константа';en='Constant'") + ")";
			Иначе
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("ru = 'Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").'; ka = 'რთული დაქვემდებარებულობის აღმოსაფხვრელად, რეკომენდებულია წაშლის განმეორება, როდესაც ჩართულია ჩამრთველი ველი ""დაბლოკე ყველა სამუშაო პროგრამაში და დააჩქარე წაშლა"" (დააწკაპუნეთ ღილაკზე ""< დაბრუნება ზევით"").'; en = 'To eliminate the complex dependencies is recommended to repeat the removal with the included flag ""Block all the work in the program and to accelerate the removal of"" (click the ""<Start"").'");
		КонецЕсли;
	Иначе
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Текст = НСтр("ru = 'Удаляются объекты, помеченные на удаление...'; ka = 'წაშლისთვის მონიშნული ობიექტები წაიშლება...'; en = 'Deleted items marked for deletion ...'");
	Если ПараметрыМетода.ИскатьПомеченные И ПараметрыМетода.УдалятьПомеченные Тогда
		Состояние(НСтр("ru = 'Поиск и удаление помеченных объектов...'; ka = 'მონიშნული ობიექტების ძიება და წაშლა...'; en = 'Find and delete the marked objects ...'"));
	ИначеЕсли ПараметрыМетода.ИскатьПомеченные Тогда
		Текст = НСтр("ru = 'Поиск помеченных на удаление объектов...'; ka = 'წაშლისთვის მონიშნული ობიექტების ძიება...'; en = 'Search marked for deletion objects ...'");
		Состояние(Текст);
	Иначе
		Состояние(НСтр("ru = 'Удаление выбранных объектов...'; ka = 'არჩეული ობიექტების წაშლა...'; en = 'Deleting selected objects ...'"));
	КонецЕсли;
	...
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			...
			ПараметрыПредупреждения.Текст    = НСтр("ru = 'Не удалось установить монопольный режим'; ka = 'მონოპოლიური რეჟიმის დაყენება ვერ მოხერხდა'; en = 'Failed to install single-user mode'");
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	Если ПоказыватьБублик Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ka='გთხოვთ, მოიცადეთ...';ru='Пожалуйста, подождите...';en='Please wait...'");
	...
	ИначеЕсли ПараметрыМетода.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru = 'Удаляются объекты, помеченные на удаление...'; ka = 'წაშლისთვის მონიშნული ობიექტები წაიშლება...'; en = 'Deleted items marked for deletion ...'");
	...
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru = 'Поиск объектов, помеченных на удаление...'; ka = 'წაშლისთვის მონიშნული ობიექტების ძიება...'; en = 'Find objects that are marked for deletion ...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Удаление помеченных объектов (интерактивное)'; ka = 'მონიშნული ობიექტების წაშლა (ინტერაქტიული)'; en = 'Deleting marked objects (interactive)'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ru = 'Не удалось прочитать результат удаления помеченных'; ka = 'მონიშნული წაშლის შედეგის წაკითხვა ვერ მოხერხდა'; en = 'Could not read the result of the removal of labeled'"), "");
			Возврат;
		КонецЕсли;
		...
		Если Не ПустаяСтрока(Результат.ОповещениеТекст) Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='მონიშნულების წაშლა';ru='Удаление помеченных';en='Deleting flagged'"),
				НавигационнаяСсылка,
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Удалено объектов: %1.'; ka = 'ობიექტები წაშლილია: %1.'; en = 'Deleted objects:% 1.'"),
		...
		Иначе
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Удалено: %1, не удалено: %2.'; ka = 'წაშლილია: %1, არ წაშლილა: %2.'; en = 'Removed:% 1 is not removed:% 2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не получилось удалить объекты, помеченные на удаление (%1):'; ka = 'წაშლისთვის მონიშნული ობიექტების წაშლა ვერ მოხერხდა (% 1):'; en = 'It did not work to remove the objects that are marked for deletion (1%):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Успешно удалено: %1 из %2, остальные объекты не удалены (%3):'; ka = 'წარმატებით წაიშალა: %1 %2-დან, სხვა ობიექტები ვერ წაიშალა (%3):'; en = 'Successfully removed% 1% 2, the other objects are not removed (3%):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ru = 'При удалении помеченных возникла ошибка:'; ka = 'მონიშნულების წაშლისას მოხდა შეცდომა:'; en = 'When you delete a marked error occurred:'") + Символы.ПС + КраткоеПредставлениеОшибки;
