...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = НСтр("ka='ოპერაციის შესასრულებლად უფლებები არასაკმარისია.';ru='Недостаточно прав для выполнения операции.';en='Insufficient permissions to perform the operation.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		И Не ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
		ТекстОшибки = НСтр("ka='Для удаления помеченных необходимо войти в область данных.';ru='Для удаления помеченных необходимо войти в область данных.';en='To remove the marked log in the data area.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
"Прервать?';ka='Удаление помеченных еще выполняется."
"Прервать?'");
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Прервать);
	Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='Не прерывать';ru='Не прерывать';en='Do not interrupt'"));
...
Процедура НеУдаленныеЗаменитьНа(Команда)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Выберите объекты';ru='Выберите объекты';en='Select objects'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьСвязиНеУдаленныхНаСервере()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
		...
		НеУдаленныеПодсказка = НСтр("ka='Выберите объект, чтобы узнать причину,"
"по которой его не удалось удалить.';ru='Выберите объект, чтобы узнать причину,"
"по которой его не удалось удалить.'");
	...
	Иначе
		...
		Шаблон = НСтр("ka='Места использования объекта ""%1"" (%2):';ru='Места использования объекта ""%1"" (%2):';en='Designated object using ""%1"" (2%):'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		ТекстОшибки = НСтр("ka='Выберите объект.';ru='Выберите объект.';en='Select the object.'");
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ka='Удаление записи регистра выполняется из ее карточки.';ru='Удаление записи регистра выполняется из ее карточки.';en='Removing performed register-writing of her cards.'");
			ИначеЕсли ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ka='Очистка значения константы выполняется из ее карточки.';ru='Очистка значения константы выполняется из ее карточки.';en='Cleaning the value of the constant is made of her cards.'");
			КонецЕсли;
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ka='Удаление записей регистров или очистка значений констант выполняется из их карточек.';ru='Удаление записей регистров или очистка значений констант выполняется из их карточек.';en='Deleting records or registers cleaning constants runs from their cards.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ka='Пометить выделенные объекты (%1) на удаление?';ru='Пометить выделенные объекты (%1) на удаление?';en='Mark selected item (% 1) to remove?'");
		Иначе
			ТекстВопроса = НСтр("ka='Снять с выделенных объектов (%1) пометку на удаление?';ru='Снять с выделенных объектов (%1) пометку на удаление?';en='Remove from selected objects (% 1) marked for deletion?'");
		КонецЕсли;
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если КоличествоОбъектов = 0 Тогда
		ОповещениеЗаголовок = НСтр("ka='Объект не найден';ru='Объект не найден';en='Object not found'");
	Иначе
		Если ПараметрыВыполнения.УстанавливатьПометку Тогда
			ОповещениеЗаголовок = НСтр("ka='Пометка удаления установлена';ru='Пометка удаления установлена';en='Marking removal set'");
		Иначе
			ОповещениеЗаголовок = НСтр("ka='Пометка удаления снята';ru='Пометка удаления снята';en='Marking removal filmed'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	Элементы.КнопкаНазад.Заголовок = НСтр("ka='< В начало';ru='< В начало';en='<Back to top'");
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ka='Прервать и закрыть';ru='Прервать и закрыть';en='Interrupt and close'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ka='Повторить удаление';ru='Повторить удаление';en='repeat removal'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("ka='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';ru='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';en='To eliminate the complex dependencies is recommended to repeat the removal with the included flag ""Block all the work in the program and to accelerate the removal of"" (click the ""<Start"").'");
		КонецЕсли;
	Иначе
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Текст = НСтр("ka='Удаляются объекты, помеченные на удаление...';ru='Удаляются объекты, помеченные на удаление...';en='Deleted items marked for deletion ...'");
	Если ПараметрыМетода.ИскатьПомеченные И ПараметрыМетода.УдалятьПомеченные Тогда
		Состояние(НСтр("ka='Поиск и удаление помеченных объектов...';ru='Поиск и удаление помеченных объектов...';en='Find and delete the marked objects ...'"));
	ИначеЕсли ПараметрыМетода.ИскатьПомеченные Тогда
		Текст = НСтр("ka='Поиск помеченных на удаление объектов...';ru='Поиск помеченных на удаление объектов...';en='Search marked for deletion objects ...'");
		Состояние(Текст);
	Иначе
		Состояние(НСтр("ka='Удаление выбранных объектов...';ru='Удаление выбранных объектов...';en='Deleting selected objects ...'"));
	КонецЕсли;
	...
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			...
			ПараметрыПредупреждения.Текст    = НСтр("ka='Не удалось установить монопольный режим';ru='Не удалось установить монопольный режим';en='Failed to install single-user mode'");
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	ИначеЕсли ПараметрыМетода.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ka='Удаляются объекты, помеченные на удаление...';ru='Удаляются объекты, помеченные на удаление...';en='Deleted items marked for deletion ...'");
	...
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ka='Поиск объектов, помеченных на удаление...';ru='Поиск объектов, помеченных на удаление...';en='Find objects that are marked for deletion ...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='Удаление помеченных объектов (интерактивное)';ru='Удаление помеченных объектов (интерактивное)';en='Deleting marked objects (interactive)'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ka='Не удалось прочитать результат удаления помеченных';ru='Не удалось прочитать результат удаления помеченных';en='Could not read the result of the removal of labeled'"), "");
			Возврат;
		КонецЕсли;
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Удалено объектов: %1.';ru='Удалено объектов: %1.';en='Deleted objects:% 1.'"),
		...
		Иначе
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Удалено: %1, не удалено: %2.';ru='Удалено: %1, не удалено: %2.';en='Removed:% 1 is not removed:% 2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Не получилось удалить объекты, помеченные на удаление (%1):';ru='Не получилось удалить объекты, помеченные на удаление (%1):';en='It did not work to remove the objects that are marked for deletion (1%):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';ru='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';en='Successfully removed% 1% 2, the other objects are not removed (3%):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ka='При удалении помеченных возникла ошибка:';ru='При удалении помеченных возникла ошибка:';en='When you delete a marked error occurred:'") + Символы.ПС + КраткоеПредставлениеОшибки;
