...
Функция СоздатьОбластьШапка(НастройкиПрайсЛиста, ТабличныйДокумент, МакетПолотно)
	...
	//ПодольскийАИ #ОтборПоСкладу 16.03.2023 {
	СоздатьОбластьШапкаПоСкладам(НастройкиПрайсЛиста, ТабличныйДокумент, ОбластьШапка, НомерКолонки);
	//ПодольскийАИ #ОтборПоСкладу 16.03.2023 }
...
Функция СоздатьОбластьСтрока(НастройкиПрайсЛиста, МакетПолотно)
	...
	//ПодольскийАИ #ОтборПоСкладу 16.03.2023 {
	СоздатьОбластьСтрокаПоСкладам(НастройкиПрайсЛиста, ОбластьСтрока, НомерКолонки);
	//ПодольскийАИ #ОтборПоСкладу 16.03.2023 }
...
//ПодольскийАИ #ОтборПоСкладу 16.03.2023 {
Процедура СоздатьОбластьШапкаПоСкладам(НастройкиПрайсЛиста, ТабличныйДокумент, ОбластьШапка, НомерКолонки)
	
	Если Не НастройкиПрайсЛиста.ОстаткиПоСкладам Тогда
		Возврат ;
	КонецЕсли;
	
	ЛинияЯчейки		= Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	
	КоличествоСкладов = НастройкиПрайсЛиста.Склад.Количество();
	Для ИндексСклада = 1 По КоличествоСкладов Цикл
		ТабличныйДокумент.Область(1, НомерКолонки + 1, 1, НомерКолонки + 1).ШиринаКолонки = 7;
		
		ВыделениеОбласти = ОбластьШапка.Область(1, НомерКолонки + 1, 1, НомерКолонки + 1);
		ВыделениеОбласти.Объединить();
		ВыделениеОбласти.Текст = Строка(НастройкиПрайсЛиста.Склад[ИндексСклада - 1]);
		ВыделениеОбласти.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
		ВыделениеОбласти.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
		ВыделениеОбласти.Цветфона = ЦветаСтиля.ЦветРамкиГруппыЦеныИВалюты;
...
Процедура СоздатьОбластьСтрокаПоСкладам(НастройкиПрайсЛиста, ОбластьСтрока, НомерКолонки)
	...
	Для ИндексСклада = 1 По КоличествоСкладов Цикл
		...
		ВыделениеОбласти 			= ОбластьСтрока.Область(1, НомерКолонки, 1, НомерКолонки);
		ВыделениеОбласти.Заполнение	= ТипЗаполненияОбластиТабличногоДокумента.Параметр;
		ВыделениеОбласти.Параметр	= "ОстаткиПоСкладу_" + Строка(ИндексСклада);
		ВыделениеОбласти.ПараметрРасшифровки = "СтруктураРасшифровкиСклада" + Строка(ИндексСклада);
		ВыделениеОбласти.Формат		=  "ЧЦ=15; ЧДЦ=2";
...
Процедура ЗаполнитьВВидеПолотноИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
		...
		ЗначенияПараметров.Очистить();
		ЗначенияПараметров.Вставить("ОписаниеВалюты", НСтр("ru='Валюта: ';ka='ვალუტა: ';en='currency'") + НастройкиПрайсЛиста.Валюта);
		Если ЗначениеЗаполнено(НастройкиПрайсЛиста.Валюта) 
			...
			ЗначенияПараметров.ОписаниеВалюты = ЗначенияПараметров.ОписаниеВалюты + НСтр("ru=', курс ';ka=', კურსი ';en='Exchange Rate'") + КурсыВалюты.Курс;
			Если КурсыВалюты.Кратность <> 1 Тогда
				
				ЗначенияПараметров.ОписаниеВалюты = ЗначенияПараметров.ОписаниеВалюты + НСтр("ru=', кратность ';ka=', ჯერადობა ';en='Multiplier'") + КурсыВалюты.Кратность;
	...
	Если НастройкиПрайсЛиста.ВыводитьКонтактнуюИнформацию Тогда
		...
		ЗначенияПараметров.Очистить();
		ЗначенияПараметров.Вставить("Организация",	НСтр("ru='Организация: ';ka='ორგანიზაცია: ';en='Organization'") + НастройкиПрайсЛиста.Организация);
		ЗначенияПараметров.Вставить("Адрес", 		НСтр("ru='Адрес: ';ka='მისამართი: ';en=''") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации));
		ЗначенияПараметров.Вставить("Телефон", 		НСтр("ru='Телефон, факс: ';ka='ტელეფონი, ფაქსი: ';en='Tel/fax: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации) + ", " + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФаксОрганизации));
		ЗначенияПараметров.Вставить("Email", 		НСтр("ru='E-mail: ';ka='E-mail: ';en=''") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.EmailОрганизации));
	...
	Если НастройкиПрайсЛиста.ВыводитьДатуФормирования Тогда
		...
		ЗначенияПараметров.Очистить();
		ДатаФормирования = Нстр("ru='Сформирован ';ka='ფორმირებულია ';en='Completed '") + Формат(НастройкиПрайсЛиста.ДатаФормирования, "ДЛФ=DD");
...
Процедура ПолучитьЭлементыИИерархиюНоменклатуры(НастройкиПрайсЛиста, ДеревоИерархии)
	...
	//ПодольскийАИ #ОтборПоСкладу 13.03.2023 {
	ПараметрКД = СхемаКомпоновкиДанных.Параметры.Найти("ОтборПоСкладу");
	Если ПараметрКД <> Неопределено Тогда
		ПараметрКД.Значение = НастройкиПрайсЛиста.ОтборПоСкладу;
	КонецЕсли;
	
	ПараметрКД = СхемаКомпоновкиДанных.Параметры.Найти("Склады");
	Если ПараметрКД <> Неопределено Тогда
		ПараметрКД.Значение = НастройкиПрайсЛиста.Склад;
	КонецЕсли;
	//ПодольскийАИ #ОтборПоСкладу 13.03.2023 }
...
Процедура ПрочитатьНастройкиПрайсЛиста(ПрайсЛист, НастройкиПрайсЛиста)
	...
	//ПодольскийАИ #ОтборПоСкладу 13.03.2023 {
	НастройкиПрайсЛиста.Вставить("ОстаткиПоСкладам",	ПрайсЛист.ПоказыватьОстаткиПоСкладам);
	НастройкиПрайсЛиста.Вставить("ОтборПоСкладу",		ПрайсЛист.Склады.Количество() > 0);
	НастройкиПрайсЛиста.Вставить("Склад",				Новый Массив);
	Если НастройкиПрайсЛиста.ОтборПоСкладу Тогда
		НастройкиПрайсЛиста.Склад						= ПрайсЛист.Склады.ВыгрузитьКолонку("Склад");
	КонецЕсли;
	//ПодольскийАИ #ОтборПоСкладу 13.03.2023 }
...
Процедура ТранспонированиеТаблиц(НастройкиПрайсЛиста, ДеревоИерархии, ТаблицаЦен) 
	...
	//ПодольскийАИ #ОтборПоСкладу 16.03.2023 {
	ТранспонированиеТаблицСклады(НастройкиПрайсЛиста, ДеревоИерархии);
	//ПодольскийАИ #ОтборПоСкладу 16.03.2023 }
	
КонецПроцедуры

//ПодольскийАИ #ОтборПоСкладу 16.03.2023 {
Процедура ТранспонированиеТаблицСклады(НастройкиПрайсЛиста, ДеревоИерархии)
	...
	ИменаКолонок	= Новый Соответствие;
	РазмерТаблицы	= НастройкиПрайсЛиста.Склад.Количество();
	
	Для ИндексСтроки = 1 По РазмерТаблицы Цикл
		
		ТекСклад = НастройкиПрайсЛиста.Склад[ИндексСтроки - 1];
		
		НоваяКолонка = ДеревоИерархии.Колонки.Добавить("ОстаткиПоСкладу_" + ИндексСтроки);
		НоваяКолонка.Заголовок = Строка(ТекСклад);
		
		ИменаКолонок.Вставить(ТекСклад.Код, "ОстаткиПоСкладу_" + ИндексСтроки);
		
	КонецЦикла;
	
	ОтборПоТаблице = Новый Структура;
	
	ОбойтиДеревоПоГруппировкам(ДеревоИерархии, ИменаКолонок);
	
КонецПроцедуры

Процедура ОбойтиДеревоПоГруппировкам(ДеревоИерархии, ИменаКолонок)
	
	Для каждого ТекСтрока Из ДеревоИерархии.Строки Цикл
		
		Если ЗначениеЗаполнено(ТекСтрока.Склад) Тогда
			ДеревоИерархии[ИменаКолонок[ТекСтрока.Склад.Код]] = ТекСтрока.ОстатокПоСкладу;
		Иначе
			ОбойтиДеревоПоГруппировкам(ТекСтрока, ИменаКолонок);
		КонецЕсли;
