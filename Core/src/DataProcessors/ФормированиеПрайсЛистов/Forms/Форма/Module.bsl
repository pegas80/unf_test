...
Процедура ИсключитьЗаписьОЦене(ДанныеРасшифровки)
	...
	Иначе
		...
		ТекстСообщения = Нстр("ka='Для исключения позиции из прайс-листа необходимо указать номенклатуру.';ru='Для исключения позиции из прайс-листа необходимо указать номенклатуру.'");
...
Процедура СформироватьПрайсЛистНаСервере(ДокументСформирован)
	...
	ЗаданиеРезультат	= ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, "Обработки.ФормированиеПрайсЛистов.СформироватьПЛ", ПараметрыФЗ, НСтр("ka='პრაის-ლისტის ფორმირება.';ru='Формирование прайс-листа.'"));
	...
	Иначе
		...
		УправлениеНебольшойФирмойСервер.СостояниеТабличногоДокументаДлительнаяОперация(Элементы.ТабличныйДокумент, НСтр("ka='მიმდინარეობს ანგარიშგების ფორმირება...';ru='Отчет формируется...'"));
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	Если ЗаписьZipФайла <> Неопределено Тогда
		...
		ПутьВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, ЭтотОбъект.УникальныйИдентификатор);
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("Представление", Нстр("ka='Price.zip';ru='Price.zip'"));
		ОписаниеФайла.Вставить("АдресВоВременномХранилище", ПутьВоВременномХранилище);
		Результат.Добавить(ОписаниеФайла);
	КонецЕсли;
