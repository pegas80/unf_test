...
Процедура ПрайсЛистНеактуален()
	...
	ОтображениеСостояния.Текст = НСтр("ka='Возможно данные прайс-листа изменились. "
"Рекомендуется повторно сформировать прайс-лист.';ru='Возможно данные прайс-листа изменились. "
...
Процедура ИсключитьЗаписьОЦене(ДанныеРасшифровки)
	...
	Иначе
		...
		ТекстСообщения = Нстр("ka='Для исключения позиции из прайс-листа необходимо указать номенклатуру.';ru='Для исключения позиции из прайс-листа необходимо указать номенклатуру.';en='To exclude the item from the price list you must specify the range.'");
...
Процедура СформироватьПрайсЛистНаСервере(ДокументСформирован)
	...
	ЗаданиеРезультат	= ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, "Обработки.ФормированиеПрайсЛистов.СформироватьПЛ", ПараметрыФЗ, НСтр("ka='პრაის-ლისტის ფორმირება.';ru='Формирование прайс-листа.';en='Formation price list.'"));
	...
	Иначе
		...
		УправлениеНебольшойФирмойСервер.СостояниеТабличногоДокументаДлительнаяОперация(Элементы.ТабличныйДокумент, НСтр("ka='მიმდინარეობს ანგარიშგების ფორმირება...';ru='Отчет формируется...';en='It generates a report ...'"));
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	Если ЗаписьZipФайла <> Неопределено Тогда
		...
		ПутьВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, ЭтотОбъект.УникальныйИдентификатор);
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("Представление", Нстр("ka='Price.zip';ru='Price.zip';en='Price.zip'"));
		ОписаниеФайла.Вставить("АдресВоВременномХранилище", ПутьВоВременномХранилище);
		Результат.Добавить(ОписаниеФайла);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.ВыборФорматаВложений") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено 
			...
			Тема = НСтр("ru='Прайс-лист ';ka='Прайс-лист '") + ?(ЗначениеЗаполнено(КэшЗначений.Организация), КэшЗначений.ОрганизацияНаименование, "");
			ПараметрыНовогоПисьма.Вставить("Тема", Тема);
			
			Тело = ?(ЗначениеЗаполнено(КэшЗначений.ВыводитьДатуФормирования), Нстр("ru='Сформирован ';ka='Сформирован '") + Формат(КэшЗначений.ДатаФормирования, "ДЛФ=DD"), "");
