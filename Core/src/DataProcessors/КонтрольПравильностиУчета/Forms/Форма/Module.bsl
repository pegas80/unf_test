...
Процедура ЗаполнитьДеревоРазделов()
	...
	НоваяСтрока_Уровень0.РазделУчетаПредствление = НСтр("ka='აღრიცხვის განყოფილებები';ru='РАЗДЕЛЫ УЧЕТА';en='SECTIONS OF ACCOUNTING'");
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ანგარიშსწორებები მომწოდებლებთან';ru='Расчеты с поставщиками';en='Accounts payable'");
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ანგარიშსწორებები მყიდველებთან';ru='Расчеты с покупателями';en='Receivables from customers'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='საკურსო ცვლილებები';ru='Курсовые разницы';en='Exchange differences'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ნომენკლატურა სპეციფიკაციის გარეშე';ru='Номенклатура без спецификаций';en='Nomenclature without specifications'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень2.РазделУчетаПредствление = НСтр("ka='გადამამუშავებლის ანგარიშგებები სფეციფიკაციის გარეშე';ru='Отчеты переработчиков без спецификаций';en='Reports processors without specifications'");
	...
	НоваяСтрока_Уровень3.РазделУчетаПредствление = НСтр("ka='გადამამუშავებლის ანგარიშგებები - შეუსაბამობა ჩამოწერის სფეციფიკაციებთან';ru='Отчеты переработчиков - несоответствие списаний спецификациям';en='Records of processors - a discrepancy writedowns specifications'");
	...
	НоваяСтрока_Уровень2.РазделУчетаПредствление = НСтр("ka='წარმოების დოკუმენტები სფეციფიკაციის გარეშე';ru='Документы ""Производство"" без спецификаций';en='Documents ""production"" without specifications'");
	НоваяСтрока_Уровень2.Уровень = 2;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень2, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень2, СчСтрок);
	...
	НоваяСтрока_Уровень3.РазделУчетаПредствление = НСтр("ka='წარმოების დოკუმენტები - შეუსაბამობა ჩამოწერის სფეციფიკაციებთან';ru='Документы ""Производство"" - несоответствие списаний спецификациям';en='Documents ""Production"" - a discrepancy writedowns specifications'");
	НоваяСтрока_Уровень3.Уровень = 3;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень3, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень3, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='შესასყიდი ფასების ანალიზი';ru='Анализ закупочных цен';en='Analysis of procurement prices'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ორგანიზაციების და ხელშეკრულებების კონტროლი დოკუმენტებში';ru='Контроль организаций и договоров в документах';en='Control organizations and agreements in the documents'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ფულადი სახსრების მოძრაობის მუხლები';ru='Статьи движения денежных средств';en='Articles cash flow'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень0.РазделУчетаПредствление = НСтр("ka='ფინალური ანგარიშგება';ru='Финальный отчет';en='The final report'");
	НоваяСтрока_Уровень0.Уровень = 0;
	НумерацияСтрокДерева(НоваяСтрока_Уровень0, СчСтрок);
...
Процедура ВывестиСтрокуПереходаКСледующемуРазделу(МакетВывода)
	...
	Если НомерТекущегоРаздела = ВсегоРазделовДоступно Тогда
		...
		ОбластьМакета.Параметры.ФинальныйОтчет = НСтр("ka='ფინალური ანგარიშგება';ru='Финальный отчет';en='The final report'");
...
Процедура ВывестиФинальныйОтчет_НаСервере()
	...
	ОбластьМакета.Параметры.Выход = НСтр("ka='გამოსვლა';ru='ВЫХОД';en='EXIT'");
