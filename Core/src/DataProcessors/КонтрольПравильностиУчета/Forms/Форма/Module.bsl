...
Процедура ЗаполнитьДеревоРазделов()
	...
	НоваяСтрока_Уровень0.РазделУчетаПредствление = НСтр("ka='აღრიცხვის განყოფილებები';ru='РАЗДЕЛЫ УЧЕТА'");
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ანგარიშსწორებები მომწოდებლებთან';ru='Расчеты с поставщиками'");
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ანგარიშსწორებები მყიდველებთან';ru='Расчеты с покупателями'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='საკურსო ცვლილებები';ru='Курсовые разницы'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ნომენკლატურა სპეციფიკაციის გარეშე';ru='Номенклатура без спецификаций'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень2.РазделУчетаПредствление = НСтр("ka='გადამამუშავებლის ანგარიშგებები სფეციფიკაციის გარეშე';ru='Отчеты переработчиков без спецификаций'");
	...
	НоваяСтрока_Уровень3.РазделУчетаПредствление = НСтр("ka='გადამამუშავებლის ანგარიშგებები - შეუსაბამობა ჩამოწერის სფეციფიკაციებთან';ru='Отчеты переработчиков - несоответствие списаний спецификациям'");
	...
	НоваяСтрока_Уровень2.РазделУчетаПредствление = НСтр("ka='წარმოების დოკუმენტები სფეციფიკაციის გარეშე';ru='Документы ""Производство"" без спецификаций'");
	НоваяСтрока_Уровень2.Уровень = 2;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень2, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень2, СчСтрок);
	...
	НоваяСтрока_Уровень3.РазделУчетаПредствление = НСтр("ka='წარმოების დოკუმენტები - შეუსაბამობა ჩამოწერის სფეციფიკაციებთან';ru='Документы ""Производство"" - несоответствие списаний спецификациям'");
	НоваяСтрока_Уровень3.Уровень = 3;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень3, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень3, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='შესასყიდი ფასების ანალიზი';ru='Анализ закупочных цен'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ორგანიზაციების და ხელშეკრულებების კონტროლი დოკუმენტებში';ru='Контроль организаций и договоров в документах'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень1.РазделУчетаПредствление = НСтр("ka='ფულადი სახსრების მოძრაობის მუხლები';ru='Статьи движения денежных средств'");
	НоваяСтрока_Уровень1.Уровень = 1;
	УстановитьДатуВыполненияВСтроку(НоваяСтрока_Уровень1, СтруктураДанных);
	НумерацияСтрокДерева(НоваяСтрока_Уровень1, СчСтрок);
	...
	НоваяСтрока_Уровень0.РазделУчетаПредствление = НСтр("ka='ფინალური ანგარიშგება';ru='Финальный отчет'");
	НоваяСтрока_Уровень0.Уровень = 0;
	НумерацияСтрокДерева(НоваяСтрока_Уровень0, СчСтрок);
...
Процедура ВывестиСтрокуПереходаКСледующемуРазделу(МакетВывода)
	...
	Если НомерТекущегоРаздела = ВсегоРазделовДоступно Тогда
		...
		ОбластьМакета.Параметры.ФинальныйОтчет = НСтр("ka='ფინალური ანგარიშგება';ru='Финальный отчет'");
...
Процедура ВывестиФинальныйОтчет_НаСервере()
	...
	ОбластьМакета.Параметры.Выход = НСтр("ka='გამოსვლა';ru='ВЫХОД'");
