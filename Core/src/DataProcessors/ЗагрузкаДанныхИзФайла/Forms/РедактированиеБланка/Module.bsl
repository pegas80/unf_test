...
Процедура СброситьНастройки(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ka='Установить настройки колонок в первоначальное состояние?';ru='Установить настройки колонок в первоначальное состояние?';en='Set the speaker setting to its original state?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура УстановитьОформлениеДанных()
	...
	ЭлементОтбора.Использование = Истина;
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Стандартное наименование';ru='Стандартное наименование';en='standard name'"));
