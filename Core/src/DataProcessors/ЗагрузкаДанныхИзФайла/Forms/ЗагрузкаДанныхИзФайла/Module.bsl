...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ka='Недостаточно прав для открытия загрузки данных из файла';ru='Недостаточно прав для открытия загрузки данных из файла';en='Not have permission to open the download of data from file'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = НСтр("ka='Введенные данные не будут записаны.';ru='Введенные данные не будут записаны.';en='The data entered will not be recorded.'");
		Возврат;
	КонецЕсли;
		
	Если Отказ Тогда
		Оповещение = Новый ОписаниеОповещения("ЗакрытиеФормыЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ka='Введенные данные не будут записаны. Закрыть форму?';ru='Введенные данные не будут записаны. Закрыть форму?';en='The data entered will not be recorded. Close form?'");
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ka='Отменить сопоставление?';ru='Отменить сопоставление?';en='Cancel the comparison?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура Назад(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными Тогда
		...
		ЭтотОбъект.Заголовок = НСтр("ka='Загрузка данных в справочник';ru='Загрузка данных в справочник';en='Loading data into a directory'");
		ОчиститьТаблицу();
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных ИЛИ Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено Тогда
		...
		Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда 
			Элементы.Далее.Заголовок = НСтр("ka='Вставить в список';ru='Вставить в список';en='Insert the list'");
		Иначе
			Элементы.Далее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
		КонецЕсли;
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.РезультатыСопоставления Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ka='Вставить в список';ru='Вставить в список';en='Insert the list'");
		Элементы.Далее.КнопкаПоУмолчанию = Истина;
		Элементы.Назад.Заголовок = НСтр("ka='< В начало';ru='< В начало';en='<Back to top'");
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ka='Строки, в которых не заполнены обязательные колонки, будут пропущены.';ru='Строки, в которых не заполнены обязательные колонки, будут пропущены.';en='Line, which is missing a required column will be missed.'")
					+ Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?';en='Proceed?'"), РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
			...
			Закрыть(АдресЗагруженныхДанных);
		ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			Элементы.Назад.Заголовок = НСтр("ka='< В начало_';ru='< В начало_';en='<In nachalo_'");
			ЗакрытьФормуИВернутьМассивСсылок();
		Иначе
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ka='Выполнено сопоставление';ru='Выполнено сопоставление';en='The comparison'"),, НСтр("ka='Сопоставлено элементов:';ru='Сопоставлено элементов:';en='Mapped elements:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	ТекстВсе = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Все (%1)';ru='Все (%1)';en='All (1%)'"), Статистика.Всего);
	
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ka='Несопоставленные (';ru='Несопоставленные (';en='unmapped ('") + Статистика.НеСопоставленных + ")";
	Элементы.ОбновлятьСуществующие.Заголовок = НСтр("ka='Сопоставленные элементы (';ru='Сопоставленные элементы (';en='Matched items ('") + Строка(Статистика.Сопоставленных) + ")";
	
	СписокВыбора = Элементы.ФильтрТаблицаСопоставления.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("Все", ТекстВсе, Истина);
	СписокВыбора.Добавить("Несопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Несопоставленные (%1 из %2)';ru='Несопоставленные (%1 из %2)';en='Unmatched (% 1% 2)'"),
		Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Сопоставленные (%1 из %2)';ru='Сопоставленные (%1 из %2)';en='Matched (% 1% 2)'"),
		Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Неоднозначные (%1 из %2)';ru='Неоднозначные (%1 из %2)';en='Ambiguous (% 1% 2)'"),
	...
	Если Статистика.Неоднозначных > 0 Тогда 
		...
		Элементы.ОписаниеНеоднозначность.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='(неоднозначностей: %1)';ru='(неоднозначностей: %1)';en='(Ambiguities: 1%)'"),
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбораФайла.Фильтр                      = НСтр("ka='Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl';ru='Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl';en='Book Excel 97 (* .xls) | * .xls | Book Excel 2007 (* .xlsx) | * .xlsx | Electronic OpenDocument spreadsheet (* .ods) | * .ods | Text Document c delimited (* .csv) | * .csv | spreadsheet document (* .mxl) | * .mxl'");
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Вставка из буфера обмена (%1)';ru='Вставка из буфера обмена (%1)';en='Pasting from the clipboard (1%)'"), Параметры.ПредставлениеПоля);
	Иначе
		Заголовок = НСтр("ka='Вставка из буфера обмена';ru='Вставка из буфера обмена';en='Pasting from the clipboard'");
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	ЭтотОбъект.Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ka='Загрузка данных в справочник';ru='Загрузка данных в справочник';en='Loading data into a directory'"), Параметры.Заголовок);
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Элементы.СтраницаВариантЗаполнениеТаблицы.Видимость = Ложь;
		Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла;
		Элементы.ВариантЗагрузки.Видимость = Ложь;
		Элементы.ПояснениеДляВыбораСправочникаДляЗагрузки.Заголовок = НСтр("ka='Выбор справочника для загрузки данных из электронных таблиц, расположенных во внешних файлах "
" (например: Microsoft Office Excel, OpenOffice Calc и др.).';ru='Выбор справочника для загрузки данных из электронных таблиц, расположенных во внешних файлах "
" (например: Microsoft Office Excel, OpenOffice Calc и др.).'");
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Элементы.Закрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными;
		Элементы.ГруппаНастройкиСопоставления.Видимость = Ложь;
		Элементы.СписокКолонокСопоставления.Видимость = Ложь;
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка данных в справочник ""%1""';ru='Загрузка данных в справочник ""%1""';en='Loading data in the directory ""%1""'"), ПредставлениеСправочника(ИмяОбъектаСопоставления));
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТаблицаСДаннымиПустая() Тогда
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ka='Для сопоставления и загрузки данных, необходимо заполнить таблицу.';ru='Для сопоставления и загрузки данных, необходимо заполнить таблицу.';en='In order to compare and loading data, you must fill in the table.'")));
		Иначе
			ПоказатьПредупреждение(, (НСтр("ru='Невозможно выполнить сопоставление данных, т.к данные не были загружены в таблицу. "
"Возможно, имена колонок в файле не соответствуют колонкам в бланке.';ka='Невозможно выполнить сопоставление данных, т.к данные не были загружены в таблицу. "
"Возможно, имена колонок в файле не соответствуют колонкам в бланке.'")));
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ka='Обязательная колонка""';ru='Обязательная колонка""';en='Mandatory column ""'") + " " + СписокНезаполненныхКолонок[0]
				+ НСтр("ka='"" содержит незаполненные строки, эти строки будут пропущены при загрузке.';ru='"" содержит незаполненные строки, эти строки будут пропущены при загрузке.';en='""Contains blank lines, these lines will be skipped when loading.'");
		Иначе
			ТекстПроКолонки = НСтр("ka='Обязательные колонки""';ru='Обязательные колонки""';en='Required columns ""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
				+ НСтр("ka='"" содержат незаполненные строки, эти строки будут пропущены при загрузке.';ru='"" содержат незаполненные строки, эти строки будут пропущены при загрузке.';en='""Contain blank lines, these lines will be skipped when loading.'");
		КонецЕсли;
		ТекстПроКолонки = ТекстПроКолонки + Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?';en='Proceed?'");
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы", НСтр("ka='Все (';ru='Все (';en='Everything ('") + Отчет.Всего + ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Новые", НСтр("ka='Новые (';ru='Новые (';en='new ('") + Отчет.Создано+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Обновленные", НСтр("ka='Обновленные (';ru='Обновленные (';en='updated ('") + Отчет.Обновлено+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Пропущенные", НСтр("ka='Пропущенные (';ru='Пропущенные (';en='missing ('") + Отчет.Пропущено+ ")");
...
"Техническая информация: %1';ka='Возможная причина: Загружаемые данные некорректные."
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			ТекстНайдено = НСтр("ka='Из %1 введенных строк в список будут вставлены: %2.';ru='Из %1 введенных строк в список будут вставлены: %2.';en='From 1% strings entered in the list will be inserted% 2.'");
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				ТекстНеНайдено = НСтр("ka='%3 строк будут пропущены:';ru='%3 строк будут пропущены:';en='% 3 rows are skipped:'") + Символы.ПС + "  - " + НСтр("ka='Нет данных в программе: %1';ru='Нет данных в программе: %1';en='No data in the program: 1%'") 
					+ Символы.ПС + "  - " +НСтр("ka='Несколько вариантов для вставки: %2';ru='Несколько вариантов для вставки: %2';en='Multiple options for insertion: 2%'");
				ТекстНеНайдено = НСтр("ka='Будет пропущено строк: %3"
"  - Нет данных в программе: %1"
"  - Несколько вариантов для вставки: %2';ru='Будет пропущено строк: %3"
"  - Нет данных в программе: %1"
"  - Несколько вариантов для вставки: %2'");
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ka='Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1';ru='Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1';en='Line, for which the program has a number of options will be ignored% 1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ka='Строки, для которых в программе нет соответствующих данных, будут пропущены: %1';ru='Строки, для которых в программе нет соответствующих данных, будут пропущены: %1';en='Line, for which the program is no relevant data are missing:% 1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено, Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр("ka='Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';ru='Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';en='To view the missed lines and selection of data to be inserted, click ""Next"".'");
			...
			Элементы.Далее.Видимость = Истина;
			Элементы.Назад.Заголовок = НСтр("ka='< უკან';ru='< Назад';en='<Back'");
			Элементы.Далее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
			...
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПоискаСсылок, Ложь, НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'"));
		Иначе
			Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено;
			Элементы.Закрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
			Элементы.Назад.Видимость = Истина;
			Элементы.ВставитьВСписок.Видимость = Ложь;
	...
	Иначе 
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляСопоставленияДанных, Истина, НСтр("ka='Загрузить данные >';ru='Загрузить данные >';en='Upload data>'"));
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляТабличнойЧасти, Истина, НСтр("ka='Загрузить данные';ru='Загрузить данные';en='Download data'"));
			УстановитьОформлениеДляПолейСНеоднозначностью(Новый Структура("РезультатСопоставленияСтроки", "Неоднозначность"));
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ka='Загрузить данные >';ru='Загрузить данные >';en='Upload data>'"));
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ka='Загрузить данные >';ru='Загрузить данные >';en='Upload data>'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда 
		ТекстОбъектНеНайден = НСтр("ka='<Не найден>';ru='<Не найден>';en='<Not found>'");
		ЦветОбъектНеНайден = ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет;
		ЦветНеоднозначность = ЦветаСтиля.ПоясняющийОшибкуТекст;
	Иначе
		ТекстОбъектНеНайден = НСтр("ka='<Новый>';ru='<Новый>';en='<New>'");
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<неоднозначность>';ru='<неоднозначность>';en='<Ambiguity>'"));
		КонецЦикла;
	Иначе
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ПолеОформления = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Объект ""%1"" не найден';ru='Объект ""%1"" не найден';en='Object ""%1"" not found'"), ИмяОбъектаСопоставления);
		КонецЕсли;
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		ТекстПодсказки = НСтр("ka='Для загрузки данных необходимо, сохранить бланк в файл для заполнения в другой программе. "
...
"• Табличный документ (.mxl)';ru='Для загрузки данных необходимо, сохранить бланк в файл для заполнения в другой программе. "
"Затем загрузить заполненную таблицу в одном из форматов:"
"• Книги Microsoft Excel 97 (.xls) и Excel 2007 (.xlsx)"
"• Электронные таблицы LibreOffice Calc (.ods)"
"• Текст с разделителями (.csv)"
"• Табличный документ (.mxl)'") + Символы.ПС;
	Иначе
		ТекстПодсказки = НСтр("ka='Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.';ru='Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.';en='To fill the table you want to copy data into a table from an external file via the clipboard.'") + Символы.ПС;
	КонецЕсли;
	...
	Если ОбязательныеКолонки.Количество() > 0 Тогда 
		...
		Если ОбязательныеКолонки.Количество() = 1 Тогда
			ТекстПодсказки = ТекстПодсказки + НСтр("ka='Колонка, обязательная для заполнения:';ru='Колонка, обязательная для заполнения:';en='The column is required:'") + " " + СписокКолонок;
		Иначе
			ТекстПодсказки = ТекстПодсказки + НСтр("ka='Колонки, обязательные для заполнения:';ru='Колонки, обязательные для заполнения:';en='Columns must be filled:'") + " " + СписокКолонок;
		КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор,
	...
	Если ДобавитьИдентификатор Тогда 
		ВременнаяТЗ.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число"), НСтр("ka='п/п';ru='п/п';en='p / p'"));
	КонецЕсли;
	...
	Если ДобавитьРезультатСопоставленияСтроки Тогда 
		ВременнаяТЗ.Колонки.Добавить("РезультатСопоставленияСтроки", Новый ОписаниеТипов("Строка"), НСтр("ka='შედეგი';ru='Результат';en='Result'"));
	КонецЕсли;
	Если ДобавитьОписаниеОшибки Тогда
		ВременнаяТЗ.Колонки.Добавить("ОписаниеОшибки", Новый ОписаниеТипов("Строка"), НСтр("ka='მიზეზი:';ru='Причина';en='Cause'"));
	КонецЕсли;
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			...
			Если КолонкиГруппировки.Количество() = 1 Тогда
				...
				Если КолонкаУровень2 <> Неопределено Тогда 
					...
					Если ТипКолонки <> Неопределено И СтрНайти(ТипКолонки.ПолноеИмя(), "Справочник") > 0 Тогда
						НовыйЭлемент.Заголовок = НСтр("ka='Данные из файла';ru='Данные из файла';en='Your file'");
					Иначе
			...
			ИначеЕсли КолонкиГруппировки.Количество() > 1 Тогда
				...
				Префикс = НСтр("ka='Данные из файла:';ru='Данные из файла:';en='Data from the file:'");
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			...
			Иначе
				...
				Иначе
					ПоказатьПредупреждение(, НСтр("ka='Шаблон файла не был сохранен.';ru='Шаблон файла не был сохранен.';en='template file was not saved.'"));
					Возврат;
				КонецЕсли;
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ka='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла';ru='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла';en='Subsystem ZagruzkaDannyhIzFayla: Performing server method of loading data from a file'");
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		СопоставитьЗагружаемыеДанные();
	Иначе
		ВывестиСообщениеОбОшибке(НСтр("ka='Не удалось произвести загрузку данных.';ru='Не удалось произвести загрузку данных.';en='Failed to load data.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
	КонецЕсли;
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ka='Заполнение таблицы сопоставления загруженными данными из файла.';ru='Заполнение таблицы сопоставления загруженными данными из файла.';en='Filling the table mapping data loaded from a file.'");
...
Процедура ПослеСопоставленияЗагружаемыхДанных(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ka='Не удалось произвести сопоставление данных.';ru='Не удалось произвести сопоставление данных.';en='Unable to perform data mapping.'"),
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Запись загруженных данных из файла';ru='Запись загруженных данных из файла';en='Recording of data downloaded from a file'");
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ka='Не удалось произвести запись данных.';ru='Не удалось произвести запись данных.';en='Unable to perform data entry.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Создание отчета о загрузке данных из файла';ru='Создание отчета о загрузке данных из файла';en='Create report about downloading data from a file'");
