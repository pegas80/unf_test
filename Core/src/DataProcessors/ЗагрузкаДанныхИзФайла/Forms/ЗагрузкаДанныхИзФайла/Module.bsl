...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ka='Недостаточно прав для открытия загрузки данных из файла';ru='Недостаточно прав для открытия загрузки данных из файла'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = НСтр("ka='Введенные данные не будут записаны.';ru='Введенные данные не будут записаны.'");
		Возврат;
	КонецЕсли;
		
	Если Отказ Тогда
		Оповещение = Новый ОписаниеОповещения("ЗакрытиеФормыЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ka='Введенные данные не будут записаны. Закрыть форму?';ru='Введенные данные не будут записаны. Закрыть форму?'");
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ka='Отменить сопоставление?';ru='Отменить сопоставление?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура Назад(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными Тогда
		...
		ЭтотОбъект.Заголовок = НСтр("ka='Загрузка данных в справочник';ru='Загрузка данных в справочник'");
		ОчиститьТаблицу();
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных ИЛИ Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено Тогда
		...
		Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда 
			Элементы.Далее.Заголовок = НСтр("ka='Вставить в список';ru='Вставить в список'");
		Иначе
			Элементы.Далее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >'");
		КонецЕсли;
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.РезультатыСопоставления Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ka='Вставить в список';ru='Вставить в список'");
		Элементы.Далее.КнопкаПоУмолчанию = Истина;
		Элементы.Назад.Заголовок = НСтр("ka='< В начало';ru='< В начало'");
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ka='Строки, в которых не заполнены обязательные колонки, будут пропущены.';ru='Строки, в которых не заполнены обязательные колонки, будут пропущены.'")
					+ Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?'"), РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
			...
			Закрыть(АдресЗагруженныхДанных);
		ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			Элементы.Назад.Заголовок = НСтр("ka='< В начало_';ru='< В начало_'");
			ЗакрытьФормуИВернутьМассивСсылок();
		Иначе
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ka='Выполнено сопоставление';ru='Выполнено сопоставление'"),, НСтр("ka='Сопоставлено элементов:';ru='Сопоставлено элементов:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	ТекстВсе = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Все (%1)';ru='Все (%1)'"), Статистика.Всего);
	
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ka='Несопоставленные (';ru='Несопоставленные ('") + Статистика.НеСопоставленных + ")";
	Элементы.ОбновлятьСуществующие.Заголовок = НСтр("ka='Сопоставленные элементы (';ru='Сопоставленные элементы ('") + Строка(Статистика.Сопоставленных) + ")";
	
	СписокВыбора = Элементы.ФильтрТаблицаСопоставления.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("Все", ТекстВсе, Истина);
	СписокВыбора.Добавить("Несопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Несопоставленные (%1 из %2)';ru='Несопоставленные (%1 из %2)'"),
		Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Сопоставленные (%1 из %2)';ru='Сопоставленные (%1 из %2)'"),
		Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Неоднозначные (%1 из %2)';ru='Неоднозначные (%1 из %2)'"),
	...
	Если Статистика.Неоднозначных > 0 Тогда 
		...
		Элементы.ОписаниеНеоднозначность.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='(неоднозначностей: %1)';ru='(неоднозначностей: %1)'"),
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Вставка из буфера обмена (%1)';ru='Вставка из буфера обмена (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
		Заголовок = НСтр("ka='Вставка из буфера обмена';ru='Вставка из буфера обмена'");
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	ЭтотОбъект.Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ka='Загрузка данных в справочник';ru='Загрузка данных в справочник'"), Параметры.Заголовок);
...
" (например: Microsoft Office Excel, OpenOffice Calc и др.).'");
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Элементы.Закрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными;
		Элементы.ГруппаНастройкиСопоставления.Видимость = Ложь;
		Элементы.СписокКолонокСопоставления.Видимость = Ложь;
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка данных в справочник ""%1""';ru='Загрузка данных в справочник ""%1""'"), ПредставлениеСправочника(ИмяОбъектаСопоставления));
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТаблицаСДаннымиПустая() Тогда
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ka='Для сопоставления и загрузки данных, необходимо заполнить таблицу.';ru='Для сопоставления и загрузки данных, необходимо заполнить таблицу.'")));
		Иначе
...
"Возможно, имена колонок в файле не соответствуют колонкам в бланке.'")));
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ka='Обязательная колонка""';ru='Обязательная колонка""'") + " " + СписокНезаполненныхКолонок[0]
				+ НСтр("ka='"" содержит незаполненные строки, эти строки будут пропущены при загрузке.';ru='"" содержит незаполненные строки, эти строки будут пропущены при загрузке.'");
		Иначе
			ТекстПроКолонки = НСтр("ka='Обязательные колонки""';ru='Обязательные колонки""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
				+ НСтр("ka='"" содержат незаполненные строки, эти строки будут пропущены при загрузке.';ru='"" содержат незаполненные строки, эти строки будут пропущены при загрузке.'");
		КонецЕсли;
		ТекстПроКолонки = ТекстПроКолонки + Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?'");
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы", НСтр("ka='Все (';ru='Все ('") + Отчет.Всего + ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Новые", НСтр("ka='Новые (';ru='Новые ('") + Отчет.Создано+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Обновленные", НСтр("ka='Обновленные (';ru='Обновленные ('") + Отчет.Обновлено+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Пропущенные", НСтр("ka='Пропущенные (';ru='Пропущенные ('") + Отчет.Пропущено+ ")");
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			ТекстНайдено = НСтр("ka='Из %1 введенных строк в список будут вставлены: %2.';ru='Из %1 введенных строк в список будут вставлены: %2.'");
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				ТекстНеНайдено = НСтр("ka='%3 строк будут пропущены:';ru='%3 строк будут пропущены:'") + Символы.ПС + "  - " + НСтр("ka='Нет данных в программе: %1';ru='Нет данных в программе: %1'") 
					+ Символы.ПС + "  - " +НСтр("ka='Несколько вариантов для вставки: %2';ru='Несколько вариантов для вставки: %2'");
...
"  - Несколько вариантов для вставки: %2'");
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ka='Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1';ru='Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ka='Строки, для которых в программе нет соответствующих данных, будут пропущены: %1';ru='Строки, для которых в программе нет соответствующих данных, будут пропущены: %1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено, Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр("ka='Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';ru='Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".'");
			...
			Элементы.Далее.Видимость = Истина;
			Элементы.Назад.Заголовок = НСтр("ka='< უკან';ru='< Назад'");
			Элементы.Далее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >'");
			...
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПоискаСсылок, Ложь, НСтр("ka='შემდეგ >';ru='Далее >'"));
		Иначе
			Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено;
			Элементы.Закрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
			Элементы.Назад.Видимость = Истина;
			Элементы.ВставитьВСписок.Видимость = Ложь;
	...
	Иначе 
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляСопоставленияДанных, Истина, НСтр("ka='Загрузить данные >';ru='Загрузить данные >'"));
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляТабличнойЧасти, Истина, НСтр("ka='Загрузить данные';ru='Загрузить данные'"));
			УстановитьОформлениеДляПолейСНеоднозначностью(Новый Структура("РезультатСопоставленияСтроки", "Неоднозначность"));
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ka='Загрузить данные >';ru='Загрузить данные >'"));
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ka='Загрузить данные >';ru='Загрузить данные >'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<неоднозначность>';ru='<неоднозначность>'"));
		КонецЦикла;
	Иначе
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ПолеОформления = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Объект ""%1"" не найден';ru='Объект ""%1"" не найден'"), ИмяОбъектаСопоставления);
		КонецЕсли;
...
"• Табличный документ (.mxl)'") + Символы.ПС;
	Иначе
		ТекстПодсказки = НСтр("ka='Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.';ru='Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.'") + Символы.ПС;
	КонецЕсли;
	...
	Если ОбязательныеКолонки.Количество() > 0 Тогда 
		...
		Если ОбязательныеКолонки.Количество() = 1 Тогда
			ТекстПодсказки = ТекстПодсказки + НСтр("ka='Колонка, обязательная для заполнения:';ru='Колонка, обязательная для заполнения:'") + " " + СписокКолонок;
		Иначе
			ТекстПодсказки = ТекстПодсказки + НСтр("ka='Колонки, обязательные для заполнения:';ru='Колонки, обязательные для заполнения:'") + " " + СписокКолонок;
		КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор,
	...
	Если ДобавитьИдентификатор Тогда 
		ВременнаяТЗ.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число"), НСтр("ka='п/п';ru='п/п'"));
	КонецЕсли;
	...
	Если ДобавитьРезультатСопоставленияСтроки Тогда 
		ВременнаяТЗ.Колонки.Добавить("РезультатСопоставленияСтроки", Новый ОписаниеТипов("Строка"), НСтр("ka='შედეგი';ru='Результат'"));
	КонецЕсли;
	Если ДобавитьОписаниеОшибки Тогда
		ВременнаяТЗ.Колонки.Добавить("ОписаниеОшибки", Новый ОписаниеТипов("Строка"), НСтр("ka='მიზეზი:';ru='Причина'"));
	КонецЕсли;
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			...
			Если КолонкиГруппировки.Количество() = 1 Тогда
				...
				Если КолонкаУровень2 <> Неопределено Тогда 
					...
					Если ТипКолонки <> Неопределено И СтрНайти(ТипКолонки.ПолноеИмя(), "Справочник") > 0 Тогда
						НовыйЭлемент.Заголовок = НСтр("ka='Данные из файла';ru='Данные из файла'");
					Иначе
			...
			ИначеЕсли КолонкиГруппировки.Количество() > 1 Тогда
				...
				Префикс = НСтр("ka='Данные из файла:';ru='Данные из файла:'");
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			...
			Иначе
				...
				Иначе
					ПоказатьПредупреждение(, НСтр("ka='Шаблон файла не был сохранен.';ru='Шаблон файла не был сохранен.'"));
					Возврат;
				КонецЕсли;
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ka='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла';ru='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла'");
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		СопоставитьЗагружаемыеДанные();
	Иначе
		ВывестиСообщениеОбОшибке(НСтр("ka='Не удалось произвести загрузку данных.';ru='Не удалось произвести загрузку данных.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
	КонецЕсли;
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ka='Заполнение таблицы сопоставления загруженными данными из файла.';ru='Заполнение таблицы сопоставления загруженными данными из файла.'");
...
Процедура ПослеСопоставленияЗагружаемыхДанных(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ka='Не удалось произвести сопоставление данных.';ru='Не удалось произвести сопоставление данных.'"),
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Запись загруженных данных из файла';ru='Запись загруженных данных из файла'");
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ka='Не удалось произвести запись данных.';ru='Не удалось произвести запись данных.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Создание отчета о загрузке данных из файла';ru='Создание отчета о загрузке данных из файла'");
