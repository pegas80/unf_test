...
Процедура УстановитьРежимВставкиИзБуфераОбмена(ШаблонСДанными, ИнформацияПоКолонкам, ОписаниеТипов) Экспорт
	...
	Для каждого Тип Из ОписаниеТипов.Типы() Цикл 
		...
		ЗаголовокКолонки = НСтр("ka='Введенные данные';ru='Введенные данные';en='entered data'");
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(Тип("Булево")) Тогда 
			ОписаниеТипаКолонки = НСтр("ka='Флаг, Да или 1 / Нет или 0';ru='Флаг, Да или 1 / Нет или 0';en='Flag, Yes or 1 / N or 0'");
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Число")) Тогда 
			ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Число, Длина: %1, Точность: %2';ru='Число, Длина: %1, Точность: %2';en='The number, length: 1% Accuracy: 2%'"),
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Если Реквизит.Тип.КвалификаторыСтроки.Длина > 0 Тогда
				...
				ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Строка, макс. количество символов: %1';ru='Строка, макс. количество символов: %1';en='String, max. Characters: 1%'"), ДлинаСтроки);
			Иначе
				ОписаниеТипаКолонки = НСтр("ka='Строка неограниченной длины';ru='Строка неограниченной длины';en='A string of unlimited length'");
			КонецЕсли;
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("УникальныйИдентификатор")) Тогда
			ОписаниеТипаКолонки = НСтр("ka='УникальныйИдентификатор';ru='УникальныйИдентификатор';en='Unique identificator'");
		КонецЕсли;
...
Процедура СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, ИмяКолонки, Позиция)
	...
	Если ТипДанных = Тип("Строка") Тогда 
		ПредставлениеТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Строка (не более %1 символов)';ru='Строка (не более %1 символов)';en='String (not more than 1% characters)'"), ОписаниеТипа.КвалификаторыСтроки.Длина);
	...
	ИначеЕсли ТипДанных = Тип("Число") Тогда	
		ПредставлениеТипа = НСтр("ka='რიცხვი';ru='Число';en='Number'");
	Иначе
...
Процедура ОпределитьИнформацияПоКолонкамТабличнаяЧасть(Знач ИнформацияПоКолонкам, Макет, ОбластьЗаголовокТаблицы, Знач ПараметрыЗагрузки)
	...
	Иначе
		ВызватьИсключение НСтр("ka='Не найден макет для загрузки данных из файла';ru='Не найден макет для загрузки данных из файла';en='the layout is not found to load data from a file'");
		Возврат;
	КонецЕсли;
...
Процедура СоздатьИнформациюПоКолонкамНаОснованиеШаблона(ОбластьЗаголовокТаблицы, ПараметрыЗагрузкиИзФайла, ИнформацияПоКолонкам, ИменаДобавляемыхКолонок = Неопределено)
	...
	Для НомерКолонки = 1 По ОбластьЗаголовокТаблицы.ШиринаТаблицы Цикл
		...
		Если ПредставлениеРеквизита = НСтр("ka='<დამატებითი რეკვიზიტები>';ru='<Дополнительные реквизиты>';en='<More details>'") Тогда
			МетаданныеСправочника = Метаданные.НайтиПоПолномуИмени(ПараметрыЗагрузкиИзФайла.ПолноеИмяОбъекта);
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
				МодульУправлениеСвойствамиСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеСвойствамиСлужебный");
				МодульУправлениеСвойствамиСлужебный.КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам);
			КонецЕсли;
		ИначеЕсли ПредставлениеРеквизита = НСтр("ka='<Контактная информация>';ru='<Контактная информация>';en='<Contact>'") Тогда
			МетаданныеСправочника = Метаданные.НайтиПоПолномуИмени(ПараметрыЗагрузкиИзФайла.ПолноеИмяОбъекта);
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
				МодульУправлениеКонтактнойИнформацией.КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам);
...
Процедура ЗагрузитьCSVФайлВТаблицу(ИмяФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	Если Строка = Неопределено Тогда 
		ТекстСообщения = НСтр("ka='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';ru='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';en='It did not work to load the data from that file. Ensure the correctness of the data in the file.'");
		Возврат;
	КонецЕсли;
		...
		Попытка
			Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектСопоставления) Тогда
				...
				Если ЭлементСправочника = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Номенклатура с артикулом %1 не существует.';ru='Номенклатура с артикулом %1 не существует.';en='Nomenclature with part number% 1 does not exist.'"),
...
"%2';ka='Не удалось записать элемент справочника %1 по причине:"
"%2'"), 
...
Процедура СформироватьМакетОтчета(ТаблицаОтчет, ШаблонСДанными)
	...
	ЗаполнитьЯчейкуЗаголовкаМакета(Ячейка, НСтр("ka='შედეგი';ru='Результат';en='Result'"), 12, НСтр("ka='Результат загрузки данных';ru='Результат загрузки данных';en='Result data download'"), Истина);
...
Функция СобытиеЖурналаРегистрации() 
	...
	Возврат НСтр("ka='მონაცემთა ჩატვირთვა ფაილიდან';ru='Загрузка данных из файла';en='Loading data from a file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
