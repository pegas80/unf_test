...
Процедура УстановитьРежимВставкиИзБуфераОбмена(ШаблонСДанными, ИнформацияПоКолонкам, ОписаниеТипов) Экспорт
	...
	Для каждого Тип Из ОписаниеТипов.Типы() Цикл 
		...
		ЗаголовокКолонки = НСтр("ka='Введенные данные';ru='Введенные данные'");
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(Тип("Булево")) Тогда 
			ОписаниеТипаКолонки = НСтр("ka='Флаг, Да или 1 / Нет или 0';ru='Флаг, Да или 1 / Нет или 0'");
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Число")) Тогда 
			ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Число, Длина: %1, Точность: %2';ru='Число, Длина: %1, Точность: %2'"),
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Если Реквизит.Тип.КвалификаторыСтроки.Длина > 0 Тогда
				...
				ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Строка, макс. количество символов: %1';ru='Строка, макс. количество символов: %1'"), ДлинаСтроки);
			Иначе
				ОписаниеТипаКолонки = НСтр("ka='Строка неограниченной длины';ru='Строка неограниченной длины'");
			КонецЕсли;
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("УникальныйИдентификатор")) Тогда
			ОписаниеТипаКолонки = НСтр("ka='УникальныйИдентификатор';ru='УникальныйИдентификатор'");
		КонецЕсли;
...
Процедура СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, ИмяКолонки, Позиция)
	...
	Если ТипДанных = Тип("Строка") Тогда 
		ПредставлениеТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Строка (не более %1 символов)';ru='Строка (не более %1 символов)'"), ОписаниеТипа.КвалификаторыСтроки.Длина);
	...
	ИначеЕсли ТипДанных = Тип("Число") Тогда	
		ПредставлениеТипа = НСтр("ka='რიცხვი';ru='Число'");
	Иначе
...
Процедура ОпределитьИнформацияПоКолонкамТабличнаяЧасть(Знач ИнформацияПоКолонкам, Макет, ОбластьЗаголовокТаблицы, Знач ПараметрыЗагрузки)
	...
	Иначе
		ВызватьИсключение НСтр("ka='Не найден макет для загрузки данных из файла';ru='Не найден макет для загрузки данных из файла'");
		Возврат;
	КонецЕсли;
...
Процедура СоздатьИнформациюПоКолонкамНаОснованиеШаблона(ОбластьЗаголовокТаблицы, ПараметрыЗагрузкиИзФайла, ИнформацияПоКолонкам, ИменаДобавляемыхКолонок = Неопределено)
	...
	Для НомерКолонки = 1 По ОбластьЗаголовокТаблицы.ШиринаТаблицы Цикл
		...
		Если ПредставлениеРеквизита = НСтр("ka='<დამატებითი რეკვიზიტები>';ru='<Дополнительные реквизиты>'") Тогда
			МетаданныеСправочника = Метаданные.НайтиПоПолномуИмени(ПараметрыЗагрузкиИзФайла.ПолноеИмяОбъекта);
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
				МодульУправлениеСвойствамиСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеСвойствамиСлужебный");
				МодульУправлениеСвойствамиСлужебный.КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам);
			КонецЕсли;
		ИначеЕсли ПредставлениеРеквизита = НСтр("ka='<Контактная информация>';ru='<Контактная информация>'") Тогда
			МетаданныеСправочника = Метаданные.НайтиПоПолномуИмени(ПараметрыЗагрузкиИзФайла.ПолноеИмяОбъекта);
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
				МодульУправлениеКонтактнойИнформацией.КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам);
...
Процедура ЗагрузитьCSVФайлВТаблицу(ИмяФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	Если Строка = Неопределено Тогда 
		ТекстСообщения = НСтр("ka='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';ru='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.'");
		Возврат;
	КонецЕсли;
		...
		Попытка
			Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектСопоставления) Тогда
				...
				Если ЭлементСправочника = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Номенклатура с артикулом %1 не существует.';ru='Номенклатура с артикулом %1 не существует.'"),
...
Процедура СформироватьМакетОтчета(ТаблицаОтчет, ШаблонСДанными)
	...
	ЗаполнитьЯчейкуЗаголовкаМакета(Ячейка, НСтр("ka='შედეგი';ru='Результат'"), 12, НСтр("ka='Результат загрузки данных';ru='Результат загрузки данных'"), Истина);
...
Функция СобытиеЖурналаРегистрации() 
	...
	Возврат НСтр("ka='მონაცემთა ჩატვირთვა ფაილიდან';ru='Загрузка данных из файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
