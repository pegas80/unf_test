...
Функция ИндивидуальныеНастройкиОбработкиСвойства()
	...
	Если Элементы.СтраницаПравилоОбработкиЧисла.Видимость Тогда
		...
		Если ПравилоЧисло = "Умножить" Тогда
			ПредставлениеЧисла = НСтр("ka='Умножить на %1';ru='Умножить на %1';en='Multiply by 1%'");
		...
		ИначеЕсли ПравилоЧисло = "Разделить" Тогда
			ПредставлениеЧисла = НСтр("ka='Разделить на %1';ru='Разделить на %1';en='Divide by 1%'");
			ПредставлениеЧисла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеЧисла, ЗначениеЧисла);
			
			ИтоговоеПравило = ПравилоЧисло + ";" + ЗначениеЧисла;
		ИначеЕсли ПравилоЧисло = "Прибавить" Тогда
			ПредставлениеЧисла = НСтр("ka='Прибавить %1';ru='Прибавить %1';en='Add 1%'");
			ПредставлениеЧисла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеЧисла, ЗначениеЧисла);
			
			ИтоговоеПравило = ПравилоЧисло + ";" + ЗначениеЧисла;
		ИначеЕсли ПравилоЧисло = "Вычесть" Тогда
			ПредставлениеЧисла = НСтр("ka='Вычесть %1';ru='Вычесть %1';en='Subtract 1%'");
			ПредставлениеЧисла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеЧисла, ЗначениеЧисла);
			
			ИтоговоеПравило = ПравилоЧисло + ";" + ЗначениеЧисла;
		ИначеЕсли ПравилоЧисло = "НеИзменять" Тогда
			ПредставлениеЧисла = НСтр("ka='არ სეიცვალოს';ru='Не изменять';en='Do not change'");
			ИтоговоеПравило = ПравилоЧисло;
		ИначеЕсли ПравилоЧисло = "СлучайноеЗначение" Тогда
			ПредставлениеЧисла = НСтр("ka='Случайное значение';ru='Случайное значение';en='a random value'");
			ИтоговоеПравило = ПравилоЧисло;
		Иначе
			ПредставлениеЧисла = НСтр("ka='გასუფთავება';ru='Очистить';en='clean'");
			ИтоговоеПравило = ПравилоЧисло;
		КонецЕсли;
	...
	Если Элементы.СтраницаПравилоОбработкиДаты.Видимость Тогда
		...
		Если ПравилоДата = "Прибавить" Тогда
			ПредставлениеДаты = НСтр("ka='Прибавить дней: %1';ru='Прибавить дней: %1';en='Add days: 1%'");
		...
		ИначеЕсли ПравилоДата = "Вычесть" Тогда
			ПредставлениеДаты = НСтр("ka='Вычесть дней: %1';ru='Вычесть дней: %1';en='Subtract days: 1%'");
			ПредставлениеДаты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеДаты, ЗначениеДата);
			
			ИтоговоеПравило = ПравилоДата + ";" + ЗначениеДата;
		ИначеЕсли ПравилоДата = "СлучайноеЗначение" Тогда
			ПредставлениеДаты = НСтр("ka='Случайное значение';ru='Случайное значение';en='a random value'");
			ИтоговоеПравило = ПравилоДата;
		Иначе
			ПредставлениеДаты = НСтр("ka='არ სეიცვალოს';ru='Не изменять';en='Do not change'");
			ИтоговоеПравило = ПравилоДата;
		КонецЕсли;
	...
	Если Представление = "" Тогда
		Представление = НСтр("ka='Индивидуальные настройки';ru='Индивидуальные настройки';en='individual settings'");
	КонецЕсли;
