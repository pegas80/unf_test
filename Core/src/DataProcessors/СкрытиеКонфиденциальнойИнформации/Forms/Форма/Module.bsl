...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации';en='Privacy Protection'");
	...
	Если ДеревоМетаданныхЗаполнено Тогда
		Если Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности Тогда
			...
			Если МакетОпределен Тогда
				Заголовок = НСтр("ka='Скрытие конфиденциальной информации - полные возможности';ru='Скрытие конфиденциальной информации - полные возможности';en='Privacy Protection - full of possibilities'");
			КонецЕсли;
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ka='Обычный вид';ru='Обычный вид';en='normal view'");
		Иначе
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности;
			Заголовок = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации';en='Privacy Protection'");
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ka='Полные возможности';ru='Полные возможности';en='full capabilities'");
		КонецЕсли;
	Иначе
		Если ДеревоМетаданныхЗаполнено() Тогда
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаРасширенныеВозможности;
...
Процедура ЗначениеЧислоПриИзменении(Элемент)
	Если ЗначениеЧисло = 0 Тогда
		ТекстПредупреждения = НСтр("ka='Значение должно быть не нулевым.';ru='Значение должно быть не нулевым.';en='The value must be zero.'");
...
"Продолжить?';ka='Скрытие конфиденциальной информации необходимо выполнять на копии рабочей базы,"
"все изменения будут выполнены безвозвратно."
"Продолжить?'");
...
Процедура Подключаемый_ВыполнитьСкрытиеЗавершение()
	...
	Если РезультатЗапуска.Статус <> "Выполнено" Тогда
		...
		КонецЕсли;
	Иначе
		СообщитьПользователю(НСтр("ka='Скрытие конфиденциальной информации завершено.';ru='Скрытие конфиденциальной информации завершено.';en='Privacy Protection is complete.'"));
	КонецЕсли;
...
Процедура СохранитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогСохранения.Фильтр = НСтр("ka='აწყობების ფაილი';ru='Файл настроек';en='configuration file'") + "(*.xml)|*.xml";
...
Процедура ЗагрузитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбора.Фильтр = НСтр("ka='აწყობების ფაილი';ru='Файл настроек';en='configuration file'") + "(*.xml)|*.xml";
...
Процедура ЗаполнитьДеревоОбрабатываемыхОбъектов(СохраненныеНастройки, ОбработкаОбъект)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации';en='Privacy Protection'");
...
Функция ОбрабатываемыеМетаданные()
	...
	Структура.Вставить("Константы", НСтр("ka='კონსტანტები';ru='Константы';en='constants'"));
	Структура.Вставить("Справочники", НСтр("ka='ცნობარები';ru='Справочники';en='References'"));
	Структура.Вставить("Документы", НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'"));
	Структура.Вставить("ПланыВидовХарактеристик", НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик';en='Plans species'"));
	Структура.Вставить("ПланыСчетов", НСтр("ka='ანგარიშეგების გეგმები';ru='Планы счетов';en='Charts of accounts'"));
	Структура.Вставить("ПланыВидовРасчета", НСтр("ka='ანგარიშსწორებების ტიპების გეგმები';ru='Планы видов расчета';en='Plans of calculation types'"));
	Структура.Вставить("РегистрыСведений", НСтр("ka='ცნობების რეგისტრები';ru='Регистры сведений';en='registers information'"));
	Структура.Вставить("РегистрыНакопления", НСтр("ka='დაგროვების რეგისტრები';ru='Регистры накопления';en='accumulation registers'"));
	Структура.Вставить("РегистрыБухгалтерии", НСтр("ka='ბუღალტერიის რეგისტრები';ru='Регистры бухгалтерии';en='accounting registers'"));
	Структура.Вставить("РегистрыРасчета", НСтр("ka='ანგარიშსწორების რეგისტრები';ru='Регистры расчета';en='registers calculation'"));
	Структура.Вставить("БизнесПроцессы", НСтр("ka='Бизнес процессы';ru='Бизнес процессы';en='Business processes'"));
	Структура.Вставить("Задачи", НСтр("ka='ამოცანები';ru='Задачи';en='Tasks'"));
...
Функция СкрытиеКонфиденциальнойИнформации()
	...
	НаименованиеЗадания = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации';en='Privacy Protection'");
