...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации'");
	...
	Если ДеревоМетаданныхЗаполнено Тогда
		Если Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности Тогда
			...
			Если МакетОпределен Тогда
				Заголовок = НСтр("ka='Скрытие конфиденциальной информации - полные возможности';ru='Скрытие конфиденциальной информации - полные возможности'");
			КонецЕсли;
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ka='Обычный вид';ru='Обычный вид'");
		Иначе
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности;
			Заголовок = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации'");
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ka='Полные возможности';ru='Полные возможности'");
		КонецЕсли;
	Иначе
		Если ДеревоМетаданныхЗаполнено() Тогда
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаРасширенныеВозможности;
...
Процедура ЗначениеЧислоПриИзменении(Элемент)
	Если ЗначениеЧисло = 0 Тогда
		ТекстПредупреждения = НСтр("ka='Значение должно быть не нулевым.';ru='Значение должно быть не нулевым.'");
...
Процедура Подключаемый_ВыполнитьСкрытиеЗавершение()
	...
	Если РезультатЗапуска.Статус <> "Выполнено" Тогда
		...
		КонецЕсли;
	Иначе
		СообщитьПользователю(НСтр("ka='Скрытие конфиденциальной информации завершено.';ru='Скрытие конфиденциальной информации завершено.'"));
	КонецЕсли;
...
Процедура СохранитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогСохранения.Фильтр = НСтр("ka='აწყობების ფაილი';ru='Файл настроек'") + "(*.xml)|*.xml";
...
Процедура ЗагрузитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбора.Фильтр = НСтр("ka='აწყობების ფაილი';ru='Файл настроек'") + "(*.xml)|*.xml";
...
Процедура ЗаполнитьДеревоОбрабатываемыхОбъектов(СохраненныеНастройки, ОбработкаОбъект)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации'");
...
Функция ОбрабатываемыеМетаданные()
	...
	Структура.Вставить("Константы", НСтр("ka='კონსტანტები';ru='Константы'"));
	Структура.Вставить("Справочники", НСтр("ka='ცნობარები';ru='Справочники'"));
	Структура.Вставить("Документы", НСтр("ka='დოკუმენტები';ru='Документы'"));
	Структура.Вставить("ПланыВидовХарактеристик", НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик'"));
	Структура.Вставить("ПланыСчетов", НСтр("ka='ანგარიშეგების გეგმები';ru='Планы счетов'"));
	Структура.Вставить("ПланыВидовРасчета", НСтр("ka='ანგარიშსწორებების ტიპების გეგმები';ru='Планы видов расчета'"));
	Структура.Вставить("РегистрыСведений", НСтр("ka='ცნობების რეგისტრები';ru='Регистры сведений'"));
	Структура.Вставить("РегистрыНакопления", НСтр("ka='დაგროვების რეგისტრები';ru='Регистры накопления'"));
	Структура.Вставить("РегистрыБухгалтерии", НСтр("ka='ბუღალტერიის რეგისტრები';ru='Регистры бухгалтерии'"));
	Структура.Вставить("РегистрыРасчета", НСтр("ka='ანგარიშსწორების რეგისტრები';ru='Регистры расчета'"));
	Структура.Вставить("БизнесПроцессы", НСтр("ka='Бизнес процессы';ru='Бизнес процессы'"));
	Структура.Вставить("Задачи", НСтр("ka='ამოცანები';ru='Задачи'"));
...
Функция СкрытиеКонфиденциальнойИнформации()
	...
	НаименованиеЗадания = НСтр("ka='Скрытие конфиденциальной информации';ru='Скрытие конфиденциальной информации'");
