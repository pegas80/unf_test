...
Функция ЗаголовокГруппыОтбораВсехДокументов() Экспорт
	Возврат НСтр("ka='ყველა დოკუმენტი';ru='Все документы'");
КонецФункции
...
Функция ЗаголовокГруппыОтбораВсехСправочников() Экспорт
	Возврат НСтр("ka='ყველა ცნობარი';ru='Все справочники'");
КонецФункции
...
Функция ИнициализироватьКомпоновщик(СписокИменМетаданных = Неопределено, ОграничиватьИспользованиеОтбором = Ложь, АдресСохраненияСхемы = Неопределено)
	...
	ПараметрАвтоматически.Значение = НСтр("ka='По общим правилам';ru='По общим правилам'");
	
	ПараметрДополнительно = Параметры.Найти("ПричинаРегистрацииДополнительно");
	ПараметрДополнительно.Значение = НСтр("ka='დამატებით';ru='Дополнительно'");
	
	ПараметрПоСсылке = Параметры.Найти("ПричинаРегистрацииПоСсылке");
	ПараметрПоСсылке.Значение = НСтр("ka='По ссылке';ru='По ссылке'");
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';ru='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""'"),
...
Функция ТекстДополнительныхПараметров()
	...
	Если ВариантВыгрузки = 0 Тогда
		...
		Возврат НСтр("ka='Без дополнительных данных.';ru='Без дополнительных данных.'");
