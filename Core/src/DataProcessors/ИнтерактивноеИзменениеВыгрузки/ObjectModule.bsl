...
Функция ЗаголовокГруппыОтбораВсехДокументов() Экспорт
	Возврат НСтр("ka='ყველა დოკუმენტი';ru='Все документы';en='All documents'");
КонецФункции
...
Функция ЗаголовокГруппыОтбораВсехСправочников() Экспорт
	Возврат НСтр("ka='ყველა ცნობარი';ru='Все справочники';en='All directories'");
КонецФункции
...
Функция ИнициализироватьКомпоновщик(СписокИменМетаданных = Неопределено, ОграничиватьИспользованиеОтбором = Ложь, АдресСохраненияСхемы = Неопределено)
	...
	ПараметрАвтоматически.Значение = НСтр("ka='По общим правилам';ru='По общим правилам';en='According to the general rules'");
	
	ПараметрДополнительно = Параметры.Найти("ПричинаРегистрацииДополнительно");
	ПараметрДополнительно.Значение = НСтр("ka='დამატებით';ru='Дополнительно';en='Additionally'");
	
	ПараметрПоСсылке = Параметры.Найти("ПричинаРегистрацииПоСсылке");
	ПараметрПоСсылке.Значение = НСтр("ka='По ссылке';ru='По ссылке';en='Link'");
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';ru='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';en='Incorrect metadata name ""%1"" to register for the site ''% 2'''"),
...
Функция ТекстДополнительныхПараметров()
	...
	Если ВариантВыгрузки = 0 Тогда
		...
		Возврат НСтр("ka='Без дополнительных данных.';ru='Без дополнительных данных.';en='Without additional data.'");
