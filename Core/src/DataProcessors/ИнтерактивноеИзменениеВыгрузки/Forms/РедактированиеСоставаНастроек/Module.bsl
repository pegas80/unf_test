...
Процедура ВариантыНастроекПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса   = НСтр("ka='Удалить настройку ""%1""?';ru='Удалить настройку ""%1""?';en='Remove the setting ""%1""?'");
...
Процедура СохранитьНастройку(Команда)
	...
	Если ПустаяСтрока(ПредставлениеТекущейНастройки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не заполнено имя для текущей настройки.';ru='Не заполнено имя для текущей настройки.';en='Do not fill in the name of the current setup.'"), , "ПредставлениеТекущейНастройки");
		Возврат;
	КонецЕсли;
		
	Если ВариантыНастроек.НайтиПоЗначению(ПредставлениеТекущейНастройки)<>Неопределено Тогда
		ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'");
		ТекстВопроса   = НСтр("ka='Перезаписать существующую настройку ""%1""?';ru='Перезаписать существующую настройку ""%1""?';en='Overwrite the existing setting of ""%1""?'");
