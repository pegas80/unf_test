...
Процедура ВариантыНастроекПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса   = НСтр("ka='Удалить настройку ""%1""?';ru='Удалить настройку ""%1""?'");
...
Процедура СохранитьНастройку(Команда)
	...
	Если ПустаяСтрока(ПредставлениеТекущейНастройки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не заполнено имя для текущей настройки.';ru='Не заполнено имя для текущей настройки.'"), , "ПредставлениеТекущейНастройки");
		Возврат;
	КонецЕсли;
		
	Если ВариантыНастроек.НайтиПоЗначению(ПредставлениеТекущейНастройки)<>Неопределено Тогда
		ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение'");
		ТекстВопроса   = НСтр("ka='Перезаписать существующую настройку ""%1""?';ru='Перезаписать существующую настройку ""%1""?'");
