...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.УзелИнформационнойБазы) Тогда
		Текст = НСтр("ka='Настройка обмена данными не найдена.';ru='Настройка обмена данными не найдена.'");
...
Процедура ДополнительнаяРегистрацияПередУдалением(Элемент, Отказ)
	...
	Если Количество>1 Тогда
		ТекстПредставления = НСтр("ka='выбранные строки';ru='выбранные строки'");
	...
	ТекстВопроса = НСтр("ka='Удалить из дополнительных данных %1 ?';ru='Удалить из дополнительных данных %1 ?'");    
	...
	ИначеЕсли ТипВыбранного= Тип("Структура") Тогда
		Если ВыбранноеЗначение.ДействиеВыбора=3 Тогда
			...
			Если Не ПустаяСтрока(ПредставлениеТекущейНастройки) И ПредставлениеНастройки<>ПредставлениеТекущейНастройки Тогда
				ТекстВопроса  = НСтр("ka='Восстановить настройки ""%1""?';ru='Восстановить настройки ""%1""?'");
...
Функция РедактированиеОтбораСтрокиДополнительныйСоставСервер(СтруктураВыбора)
	...
	ТекущиеДанные.Количество   = НСтр("ka='Не рассчитано';ru='Не рассчитано'");
...
Функция ДобавитьВДополнительныйСоставВыгрузки(Элемент)
	...
	Иначе
		...
		Строка.Количество = НСтр("ka='Не рассчитано';ru='Не рассчитано'");
		ОбновитьНадписьОбщегоКоличества();
	КонецЕсли;
...
Функция ОбновитьКоличествоСервер()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = 
		НСтр("ka='Расчет количества объектов для отправки при синхронизации';ru='Расчет количества объектов для отправки при синхронизации'");
...
Процедура ОбновитьНадписьОбщегоКоличества(Количество = Неопределено) 
	...
	Если Количество = Неопределено Тогда
		ТекстКоличества = НСтр("ka='<не рассчитано>';ru='<не рассчитано>'");
	Иначе
		ТекстКоличества = НСтр("ka='Объектов: %1';ru='Объектов: %1'");
...
Процедура СброситьНадписьКоличествТаблицы()
	ТекстКоличеств = НСтр("ka='Не рассчитано';ru='Не рассчитано'");
	Для Каждого Строка Из Объект.ДополнительнаяРегистрация Цикл
