...
Процедура КомандаПродолжить(Команда)
	...
	ПараметрыПечати.Вставить("КомандаПечати"       , НСтр("ka='კონვერტი';ru='Конверт';en='The envelope'"));
	ПараметрыПечати.Вставить("ЗаголокФормы"        , НСтр("ka='Печать конверта';ru='Печать конверта';en='Printing an envelope'"));
...
Процедура ПроверитьВозможностьПечати(ОбъектыПечати)
	...
	Если ОбъектыПечати.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='Конверт может быть напечатан только для элементов справочника.';ru='Конверт может быть напечатан только для элементов справочника.';en='The envelope can be printed only for the directory elements.'");
	КонецЕсли;
...
Процедура СообщитьПользователюОсобенностиПечатиВВебБраузере(Форма)
	...
	Если Форма.НаименованиеБраузера = "CHROME" Тогда
		ТекстСообщения = НСтр("ka='На следующем шаге, в окне предпросмотра, выберите размер бумаги';ru='На следующем шаге, в окне предпросмотра, выберите размер бумаги';en='In the next step, in the preview window, select the paper size'") + " " + РазмерБумаги;
		
	ИначеЕсли Форма.НаименованиеБраузера = "MSIE" Тогда
		ТекстСообщения = НСтр("ka='На следующем шаге выберите в Параметрах страницы размер бумаги';ru='На следующем шаге выберите в Параметрах страницы размер бумаги';en='In the next step, select the Page Setup paper size'") + " " + РазмерБумаги;
		
	Иначе
		ТекстСообщения = НСтр("ka='На следующем шаге выберите в Свойствах принтера размер бумаги';ru='На следующем шаге выберите в Свойствах принтера размер бумаги';en='In the next step, select the printer properties, paper size'") + " " + РазмерБумаги;
