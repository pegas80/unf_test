...
"Закройте окно вручную, до выхода из программы.';ka='Работа в форме не завершена."
"Закройте окно вручную, до выхода из программы.'");
	...
	Если ТребуетсяПодтверждениеЗакрытияФормы Тогда
		...
		ТекстВопроса = НСтр("ka='დავხუროთ ფორმა?';ru='Закрыть форму?';en='Close form?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Закрыть", НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='არ დაიხუროს';ru='Не закрывать';en='Do not close'"));
...
Процедура КонсолидироватьИСформироватьДекларациюПоНДС(Команда);
	...
	Если ФайлыВыгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не добавлены файлы для консолидации';ru='Не добавлены файлы для консолидации';en='Do not add files to consolidate'"));
		Возврат;
	КонецЕсли;
	...
	Если Организация.Пустая() Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не выбрана или не найдена организация %1 для формирования декларации по НДС';ru='Не выбрана или не найдена организация %1 для формирования декларации по НДС';en='Is not selected or is not found an organization 1% for the formation of the VAT return'"), ПараметрОписания);
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ПараметрыФормыОтчета", ПараметрыФормыОтчета) ИЛИ ПараметрыФормыОтчета = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='Не сформированы параметры для создания декларации по НДС!';ru='Не сформированы параметры для создания декларации по НДС!';en='Not formed the parameters for the creation of a VAT return!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
	...
	Если ФормаДекларации = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='Декларация по НДС не сформирована!';ru='Декларация по НДС не сформирована!';en='VAT returns has not been formed!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлы(ЗаменитьСтроку = Ложь)
	...
	Если ПоддерживаетсяРасширениеРаботыСФайлами = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.';ru='Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.';en='For multiple choice upload files it is recommended to install the extension to work with files.'");
...
Процедура ВыбратьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='Выберите файл(ы) выгрузки декларации по НДС';ru='Выберите файл(ы) выгрузки декларации по НДС';en='Select the file (s) upload the VAT return'");
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*';ru='Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*';en='upload files (*% 1.) |. *% 1 | All files (* *.) | * *.'"), "xml");
...
"%2';ka='При получении параметров из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ПомещенныйФайл.ИмяФайла, ОписаниеОшибки));
		...
		Если (НомКорр <> "0" И НомКорр <> "") И ПризнСвед = "1" Тогда
			Контекст.ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='В файла выгрузки ""%1"" установлен признак актуальности ранее представленных сведений для корректирующего отчета:"
"%2';ru='В файла выгрузки ""%1"" установлен признак актуальности ранее представленных сведений для корректирующего отчета:"
...
"%2';ka='При получении параметров из идентификатора файла выгрузки ""%1"" произошла ошибка:"
...
"%2';ka='При добавлении файла выгрузки ""%1"" произошла ошибка:"
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ka='Заполнение консолидированной декларации по НДС';ru='Заполнение консолидированной декларации по НДС';en='Filling consolidated VAT declaration'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗагрузитьВЭлектронноеПредставление(ФайлыРазделовДекларации)
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ТекстОшибкиИнициализацииКонтекстаЭДО';ru='ТекстОшибкиИнициализацииКонтекстаЭДО';en='TekstOshibkiInitsializatsiiKontekstaEDO'"));
		Возврат;
	КонецЕсли;
...
"%2';ka='При формировании параметров выгрузки из файла ""%1"" произошла ошибка:"
...
"%2';ka='При заполнении данных разделов 1-7 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ИдФайл") Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ka='Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.';ru='Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.';en='Not established parameters for consolidation. No upload files sections 1-7.'"));
		Возврат Ложь;
	КонецЕсли;
...
"%2';ka='При заполнении данных раздела 8 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';ka='При заполнении данных приложения 1 к разделу 8 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';ka='При заполнении данных раздела 9 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';ka='При заполнении данных приложения 1 к разделу 9 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';ka='При заполнении данных раздела 10 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';ka='При заполнении данных раздела 11 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';ka='При заполнении данных раздела 12 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
	...
	Если ЗаполнитьДекларацию Тогда
		...
		Если СохраненныйДок = Неопределено Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ka='Не сформирован документ для заполнения консолидированными данными.';ru='Не сформирован документ для заполнения консолидированными данными.';en='Not formed document to fill the consolidated data.'"));
			Возврат Ложь;
		КонецЕсли;
...
"%2';ka='При заполнении сохраненного отчета ""%1"" произошла ошибка:"
...
Функция КонсолидироватьВФоновомЗадании(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено)
	...
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыЗадания,
		НСтр("ka='Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...';ru='Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...';en='Performed filling consolidated data report. Please wait...'"));
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	ИначеЕсли ПараметрыФормыОтчета = "Отчет не найден" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Отчет не найден!';ru='Отчет не найден!';en='The report could not be found!'"));
		Возврат Неопределено;
		
	ИначеЕсли ПараметрыФормыОтчета = "Не удалось открыть отчет" Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების გახსნა ვერ მოხერხდა!';ru='Не удалось открыть отчет!';en='Unable to open the report!'"));
		Возврат Неопределено;
		
	ИначеЕсли ПараметрыФормыОтчета = "Открыть внутренний отчет-документ" Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='Заполнение отчета-документа не предусмотрено!';ru='Заполнение отчета-документа не предусмотрено!';en='Filling the report, the document is not provided!'"));
		Возврат Неопределено;
	...
	Если ВыбраннаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Не удалось получить форму отчета по периоду!';ru='Не удалось получить форму отчета по периоду!';en='Unable to get in shape over the period of the report!'"));
		Возврат Неопределено;
...
Функция ПолучитьФормуОтчетаИзДокумента(СохраненныйДок)
		...
		Исключение
			...
			Если ИнформацияОбОшибке.Причина = Неопределено Тогда
				Сообщение.Текст = НСтр("ka='При открытии формы регламентированного отчета произошла ошибка!';ru='При открытии формы регламентированного отчета произошла ошибка!';en='An error occurred while opening a form of regulated reports!'");
			ИначеЕсли Найти(ИнформацияОбОшибке.Причина.Описание, ".Форма.") > 0 Тогда
				Сообщение.Текст = НСтр("ka='Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';ru='Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';en='The report could not be opened! An old revision of the report form is not supported by the current configuration version.'");
			Иначе
...
Функция ПолучитьПараметрыОтчетаИзДокументаНаСервере(СохраненныйДок)
	...
	Если ОбъектОтчет = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось открыть сохраненные данные! Отчет не найден!';ru='Не удалось открыть сохраненные данные! Отчет не найден!';en='Unable to access the stored data! The report could not be found!'");
		Сообщение.Сообщить();
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела8(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 8';ru='Заполнен раздел 8';en='Complete section 8'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела81(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 8 приложение 1';ru='Заполнен раздел 8 приложение 1';en='Complete section 8 Appendix 1'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела9(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 9';ru='Заполнен раздел 9';en='Complete section 9'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела91(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 9 приложение 1';ru='Заполнен раздел 9 приложение 1';en='Complete section 9 Appendix 1'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела10(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 10';ru='Заполнен раздел 10';en='Full Section 10'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела11(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 11';ru='Заполнен раздел 11';en='Full Section 11'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела12(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 12';ru='Заполнен раздел 12';en='Full Section 12'"));
	КонецЕсли;
