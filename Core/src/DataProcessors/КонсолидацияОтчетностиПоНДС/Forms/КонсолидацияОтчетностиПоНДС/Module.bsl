...
"Закройте окно вручную, до выхода из программы.'");
	...
	Если ТребуетсяПодтверждениеЗакрытияФормы Тогда
		...
		ТекстВопроса = НСтр("ka='დავხუროთ ფორმა?';ru='Закрыть форму?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Закрыть", НСтр("ka='დახურვა';ru='Закрыть'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='არ დაიხუროს';ru='Не закрывать'"));
...
Процедура КонсолидироватьИСформироватьДекларациюПоНДС(Команда);
	...
	Если ФайлыВыгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не добавлены файлы для консолидации';ru='Не добавлены файлы для консолидации'"));
		Возврат;
	КонецЕсли;
	...
	Если Организация.Пустая() Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не выбрана или не найдена организация %1 для формирования декларации по НДС';ru='Не выбрана или не найдена организация %1 для формирования декларации по НДС'"), ПараметрОписания);
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ПараметрыФормыОтчета", ПараметрыФормыОтчета) ИЛИ ПараметрыФормыОтчета = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='Не сформированы параметры для создания декларации по НДС!';ru='Не сформированы параметры для создания декларации по НДС!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
	...
	Если ФормаДекларации = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='Декларация по НДС не сформирована!';ru='Декларация по НДС не сформирована!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлы(ЗаменитьСтроку = Ложь)
	...
	Если ПоддерживаетсяРасширениеРаботыСФайлами = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.';ru='Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.'");
...
Процедура ВыбратьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='Выберите файл(ы) выгрузки декларации по НДС';ru='Выберите файл(ы) выгрузки декларации по НДС'");
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*';ru='Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*'"), "xml");
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ka='Заполнение консолидированной декларации по НДС';ru='Заполнение консолидированной декларации по НДС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗагрузитьВЭлектронноеПредставление(ФайлыРазделовДекларации)
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ТекстОшибкиИнициализацииКонтекстаЭДО';ru='ТекстОшибкиИнициализацииКонтекстаЭДО'"));
		Возврат;
	КонецЕсли;
...
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ИдФайл") Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ka='Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.';ru='Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.'"));
		Возврат Ложь;
	КонецЕсли;
...
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
	...
	Если ЗаполнитьДекларацию Тогда
		...
		Если СохраненныйДок = Неопределено Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ka='Не сформирован документ для заполнения консолидированными данными.';ru='Не сформирован документ для заполнения консолидированными данными.'"));
			Возврат Ложь;
		КонецЕсли;
...
Функция КонсолидироватьВФоновомЗадании(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено)
	...
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыЗадания,
		НСтр("ka='Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...';ru='Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...'"));
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	Если ПараметрыФормыОтчета = "Недостаточно прав" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='არასაკამრისი უფლებები!';ru='Недостаточно прав!'"));
		Возврат Неопределено;
		
	ИначеЕсли ПараметрыФормыОтчета = "Отчет не найден" Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='Отчет не найден!';ru='Отчет не найден!'"));
		Возврат Неопределено;
		
	ИначеЕсли ПараметрыФормыОтчета = "Не удалось открыть отчет" Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების გახსნა ვერ მოხერხდა!';ru='Не удалось открыть отчет!'"));
		Возврат Неопределено;
		
	ИначеЕсли ПараметрыФормыОтчета = "Открыть внутренний отчет-документ" Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='Заполнение отчета-документа не предусмотрено!';ru='Заполнение отчета-документа не предусмотрено!'"));
		Возврат Неопределено;
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ka='Недостаточно параметров для создания отчета!';ru='Недостаточно параметров для создания отчета!'"));
		Возврат Неопределено;
	...
	Если ОсновнаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Не удалось получить основную форму отчета!';ru='Не удалось получить основную форму отчета!'"));
		Возврат Неопределено;
	...
	Если ВыбраннаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Не удалось получить форму отчета по периоду!';ru='Не удалось получить форму отчета по периоду!'"));
		Возврат Неопределено;
...
Функция ПолучитьФормуОтчетаИзДокумента(СохраненныйДок)
		...
		Исключение
			...
			Если ИнформацияОбОшибке.Причина = Неопределено Тогда
				Сообщение.Текст = НСтр("ka='При открытии формы регламентированного отчета произошла ошибка!';ru='При открытии формы регламентированного отчета произошла ошибка!'");
			ИначеЕсли Найти(ИнформацияОбОшибке.Причина.Описание, ".Форма.") > 0 Тогда
				Сообщение.Текст = НСтр("ka='Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';ru='Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.'");
			Иначе
...
Функция ПолучитьПараметрыОтчетаИзДокументаНаСервере(СохраненныйДок)
	...
	Если ОбъектОтчет = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось открыть сохраненные данные! Отчет не найден!';ru='Не удалось открыть сохраненные данные! Отчет не найден!'");
		Сообщение.Сообщить();
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела8(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 8';ru='Заполнен раздел 8'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела81(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 8 приложение 1';ru='Заполнен раздел 8 приложение 1'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела9(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 9';ru='Заполнен раздел 9'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела91(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 9 приложение 1';ru='Заполнен раздел 9 приложение 1'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела10(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 10';ru='Заполнен раздел 10'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела11(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 11';ru='Заполнен раздел 11'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела12(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ka='Заполнен раздел 12';ru='Заполнен раздел 12'"));
	КонецЕсли;
