...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Настройки были изменены. Закрыть без сохранения?';ru='Настройки были изменены. Закрыть без сохранения?';en='Settings have been changed. Close without saving?'");
...
Функция СохранитьНастройки()
	...
	Если ИспользоватьВнешнийМодуль И НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() И НЕ ЗначениеЗаполнено(ДанныеМодуля) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите внешний модуль.';ru='Выберите внешний модуль.';en='Select the external module.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ВыбратьВнешнийМодульЗавершение(ФайлыБылиВыбраны, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранКорректныйФайл Тогда
		ПоказатьПредупреждение(, НСтр("ka='Загружать можно только файлы с расширением *.epf';ru='Загружать можно только файлы с расширением *.epf';en='You can only upload files with the extension * .epf'"));
	Иначе
		ДанныеМодуля = Адрес;
...
Процедура ВыбратьВнешнийМодульИзменитьПеременные(ФайлыБылиВыбраны, ВыбранноеИмяФайла)
	...
	Если ФайлыБылиВыбраны И ЗначениеЗаполнено(ДанныеМодуля) Тогда
		...
		Если стрВерсияМодуля = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Выбранный файл не является внешним модулем документооборота!';ru='Выбранный файл не является внешним модулем документооборота!';en='The selected file is not an external module workflow!'"));
...
Процедура УправлениеЭУ()
	...
	Если ЗначениеЗаполнено(ВерсияМодуля) Тогда
		Элементы.ДекорацияЗагруженВнешнийМодульВРежимеСервиса.Заголовок = СтрШаблон(
			НСтр("ka='Загружен внешний модуль версии %1';ru='Загружен внешний модуль версии %1';en='Loaded external module version% 1'"),
			ВерсияМодуля);
	Иначе
		Элементы.ДекорацияЗагруженВнешнийМодульВРежимеСервиса.Заголовок = НСтр("ka='Загружен внешний модуль';ru='Загружен внешний модуль';en='Loaded external module'");
	КонецЕсли;
