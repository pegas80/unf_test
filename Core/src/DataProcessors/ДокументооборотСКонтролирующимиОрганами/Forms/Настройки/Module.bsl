...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Настройки были изменены. Закрыть без сохранения?';ru='Настройки были изменены. Закрыть без сохранения?'");
...
Функция СохранитьНастройки()
	...
	Если ИспользоватьВнешнийМодуль И НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() И НЕ ЗначениеЗаполнено(ДанныеМодуля) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Выберите внешний модуль.';ru='Выберите внешний модуль.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ВыбратьВнешнийМодульЗавершение(ФайлыБылиВыбраны, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранКорректныйФайл Тогда
		ПоказатьПредупреждение(, НСтр("ka='Загружать можно только файлы с расширением *.epf';ru='Загружать можно только файлы с расширением *.epf'"));
	Иначе
		ДанныеМодуля = Адрес;
...
Процедура ВыбратьВнешнийМодульИзменитьПеременные(ФайлыБылиВыбраны, ВыбранноеИмяФайла)
	...
	Если ФайлыБылиВыбраны И ЗначениеЗаполнено(ДанныеМодуля) Тогда
		...
		Если стрВерсияМодуля = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Выбранный файл не является внешним модулем документооборота!';ru='Выбранный файл не является внешним модулем документооборота!'"));
...
Процедура УправлениеЭУ()
	...
	Если ЗначениеЗаполнено(ВерсияМодуля) Тогда
		Элементы.ДекорацияЗагруженВнешнийМодульВРежимеСервиса.Заголовок = СтрШаблон(
			НСтр("ka='Загружен внешний модуль версии %1';ru='Загружен внешний модуль версии %1'"),
			ВерсияМодуля);
	Иначе
		Элементы.ДекорацияЗагруженВнешнийМодульВРежимеСервиса.Заголовок = НСтр("ka='Загружен внешний модуль';ru='Загружен внешний модуль'");
	КонецЕсли;
