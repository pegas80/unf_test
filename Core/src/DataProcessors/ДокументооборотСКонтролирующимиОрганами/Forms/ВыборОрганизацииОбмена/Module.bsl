...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КонтекстЭДО = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка инициализации модуля документооборота.';ru='Ошибка инициализации модуля документооборота.'"),,,, Отказ);
		Возврат ;
	КонецЕсли;
	...
	Если ДопустимыеОрганизации.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Вы не являетесь зарегистрированным пользователем ни одной учетной записи.';ru='Вы не являетесь зарегистрированным пользователем ни одной учетной записи.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
