...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КонтекстЭДО = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка инициализации модуля документооборота.';ru='Ошибка инициализации модуля документооборота.';en='Workflow module initialization error.'"),,,, Отказ);
		Возврат ;
	КонецЕсли;
	...
	Если ДопустимыеОрганизации.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Вы не являетесь зарегистрированным пользователем ни одной учетной записи.';ru='Вы не являетесь зарегистрированным пользователем ни одной учетной записи.';en='You are not a registered user of any account.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
