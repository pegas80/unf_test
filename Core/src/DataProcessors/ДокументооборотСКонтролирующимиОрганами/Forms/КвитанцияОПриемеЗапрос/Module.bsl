...
"%1';ka='Ошибка выгрузки квитанции о приеме во временный файл:"
...
"%1';ka='Ошибка чтения XML из файла квитанции о приеме:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML квитанции: не обнаружен узел ""Файл"".';ru='Некорректная структура XML квитанции: не обнаружен узел ""Файл"".';en='Invalid XML structure of receipts: not detected node ""File"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML квитанции: не обнаружен узел ""Документ"".';ru='Некорректная структура XML квитанции: не обнаружен узел ""Документ"".';en='Invalid XML structure of receipts: not detected node ""document"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвКвит) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML квитанции: не обнаружен узел ""СвКвит"".';ru='Некорректная структура XML квитанции: не обнаружен узел ""СвКвит"".';en='Invalid XML structure of receipts: not detected node ""SvKvit"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвНОКвит) Тогда
		Если НЕ ЗначениеЗаполнено(УзелСвПолДок) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Некорректная структура XML квитанции: не обнаружен узел ""НОПром"", ""СвНОКвит"" или ""СвПолДок"".';ru='Некорректная структура XML квитанции: не обнаружен узел ""НОПром"", ""СвНОКвит"" или ""СвПолДок"".';en='Invalid XML structure of receipts: not detected node ""NAAL"", ""SvNOKvit"" or ""SvPolDok"".'"),,,, Отказ);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Некорректная структура XML квитанции: не обнаружен узел ""НОПром"", ""СвНОКвит"" или ""ОтпрНО"".';ru='Некорректная структура XML квитанции: не обнаружен узел ""НОПром"", ""СвНОКвит"" или ""ОтпрНО"".';en='Invalid XML structure of receipts: not detected node ""NAAL"", ""SvNOKvit"" or ""OtprNO"".'"),,,, Отказ);
		КонецЕсли;
		Возврат;
	КонецЕсли;
