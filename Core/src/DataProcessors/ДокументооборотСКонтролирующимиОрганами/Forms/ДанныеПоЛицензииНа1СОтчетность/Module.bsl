...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ЛицензияДатаНачала) И ЗначениеЗаполнено(Объект.ЛицензияДатаОкончания) 
		...
		ИнформацияОЛицензии = НСтр("ka='Лицензия ""%1"""
"Действует с %2 по %3';ru='Лицензия ""%1"""
"Действует с %2 по %3'");
		...
		Если КонтекстЭДОСервер <> Неопределено Тогда 
			...
			Если СрокИстеченияЛицензии = Неопределено Тогда
				// Лицензия истекла
				СрокИстеченияЛицензии 	= НСтр("ru='Срок действия лицензии истек ';ka='Срок действия лицензии истек '") + Формат(ЛицензияДатаОкончания, "ДЛФ=D");
			Иначе
				...
				Если НачалоДня(ЛицензияДатаОкончания) - НачалоДня(ТекДата) < Месяц Тогда
					...
					СрокИстеченияЛицензии = НСтр("ru='Срок действия лицензии истекает через ';ka='Срок действия лицензии истекает через '") + СрокИстеченияЛицензии;
				Иначе
	...
	Иначе
		...
		Элементы.ИнформацияОЛицензии.Заголовок = НСтр("ka='Информация о лицензии на 1С-Отчетность отсутсвует';ru='Информация о лицензии на 1С-Отчетность отсутсвует';en='License information on 1C-Accounting is missing'");
		Элементы.ИнформацияОбОкончанииСрокаДействия.Видимость = Ложь;
