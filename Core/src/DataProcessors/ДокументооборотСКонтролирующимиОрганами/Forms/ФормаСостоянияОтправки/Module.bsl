...
Процедура ПодробнаяИнформацияНажатие(Элемент)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='Подробная информация отсутствует';ru='Подробная информация отсутствует';en='More information is'"));
	КонецЕсли;
...
Процедура ВыгрузитьПакетДокументовДляПредставления(Команда)
	...
	Если ЗначениеЗаполнено(Отправка) Тогда
		...
		ТекстВопроса = НСтр("ka='Выгрузить пакет документов для представления по месту требования?';ru='Выгрузить пакет документов для представления по месту требования?';en='Unload a package of documents for submission to the requirements of the place?'");
...
Процедура ВыгрузитьЦиклыОбмена()
	...
	ТекстВопроса = НСтр("ka='Выгрузить все файлы и подписи?';ru='Выгрузить все файлы и подписи?';en='Unload all the files and signatures?'");
...
Процедура УдалитьСтрокиТаблицыСостоянияСоСтатусомНеОтправлено(ТаблицаСостояний)
	...
	Для каждого СтрокаТаблицыСостояний Из ТаблицаСостояний Цикл
		Если СтрокаТаблицыСостояний.ТекстНадписи = НСтр("ka='არ არის გაგზავნილი';ru='Не отправлено';en='Not sent'") Тогда
...
Функция ПолучитьАдресФайлаПакета(ОтправкаСсылка, ИмяФайлаПакета)
	...
	Иначе
		ИмяФайлаПакета = НСтр("ka='ხელმოწერა';ru='Подпись';en='Signature'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
...
Функция ПолучитьАдресФайлаКвитанции(ОтправкаСсылка, ИмяФайлаКвитанции)
	...
	Иначе
		ИмяФайлаКвитанции = НСтр("ka='კვიტანცია';ru='Квитанция';en='Receipt'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
