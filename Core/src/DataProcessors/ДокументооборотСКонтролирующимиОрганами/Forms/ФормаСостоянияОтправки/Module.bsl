...
Процедура ПодробнаяИнформацияНажатие(Элемент)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='Подробная информация отсутствует';ru='Подробная информация отсутствует'"));
	КонецЕсли;
...
Процедура ВыгрузитьПакетДокументовДляПредставления(Команда)
	...
	Если ЗначениеЗаполнено(Отправка) Тогда
		...
		ТекстВопроса = НСтр("ka='Выгрузить пакет документов для представления по месту требования?';ru='Выгрузить пакет документов для представления по месту требования?'");
...
Процедура ВыгрузитьЦиклыОбмена()
	...
	ТекстВопроса = НСтр("ka='Выгрузить все файлы и подписи?';ru='Выгрузить все файлы и подписи?'");
...
Процедура УдалитьСтрокиТаблицыСостоянияСоСтатусомНеОтправлено(ТаблицаСостояний)
	...
	Для каждого СтрокаТаблицыСостояний Из ТаблицаСостояний Цикл
		Если СтрокаТаблицыСостояний.ТекстНадписи = НСтр("ka='არ არის გაგზავნილი';ru='Не отправлено'") Тогда
...
Функция ПолучитьАдресФайлаПакета(ОтправкаСсылка, ИмяФайлаПакета)
	...
	Иначе
		ИмяФайлаПакета = НСтр("ka='ხელმოწერა';ru='Подпись'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
...
Функция ПолучитьАдресФайлаКвитанции(ОтправкаСсылка, ИмяФайлаКвитанции)
	...
	Иначе
		ИмяФайлаКвитанции = НСтр("ka='კვიტანცია';ru='Квитанция'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
