...
Процедура ПодтвердитьВосстановление(ВыполняемоеОповещение)
	...
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Сверки") Тогда
		...
		Иначе
			ВидОбъекта = НСтр("ka='запрос на сверку';ru='запрос на сверку';en='request for reconciliation'");
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ") Тогда
		...
		Иначе
			ВидОбъекта = НСтр("ka='выписку из ЕГРЮЛ';ru='выписку из ЕГРЮЛ';en='Certificate of Incorporation'");
		КонецЕсли;
	...
	ТекстСообщения = НСтр("ka='Вы уверены, что хотите восстановить %1?';ru='Вы уверены, что хотите восстановить %1?';en='Are you sure you want to restore% 1?'");
