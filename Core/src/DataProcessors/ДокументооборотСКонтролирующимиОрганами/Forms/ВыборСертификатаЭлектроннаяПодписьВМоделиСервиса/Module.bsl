...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого ЭлементСписка Из НеНайденныеСертификаты Цикл 
		ШаблонСообщения = НСтр("ka='Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.';ru='Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.';en='Certificate with the imprint ""%1"" was not found in the certificate store.'");
...
"Вы уверены, что хотите продолжить выбор?';ka='Среди выбранных сертификатов есть такие, срок действия которых истек."
"Вы уверены, что хотите продолжить выбор?'");
	...
	Иначе
		...
		Если ТекДанные.Просрочен Тогда
			...
			ТекстВопроса = НСтр("ka='Вы уверены, что хотите выбрать сертификат, срок действия которого истек?';ru='Вы уверены, что хотите выбрать сертификат, срок действия которого истек?';en='Are you sure you want to choose a certificate that has expired?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
