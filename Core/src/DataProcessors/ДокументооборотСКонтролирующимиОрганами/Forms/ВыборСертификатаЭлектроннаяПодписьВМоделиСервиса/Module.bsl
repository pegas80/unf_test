...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого ЭлементСписка Из НеНайденныеСертификаты Цикл 
		ШаблонСообщения = НСтр("ka='Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.';ru='Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.'");
...
"Вы уверены, что хотите продолжить выбор?'");
	...
	Иначе
		...
		Если ТекДанные.Просрочен Тогда
			...
			ТекстВопроса = НСтр("ka='Вы уверены, что хотите выбрать сертификат, срок действия которого истек?';ru='Вы уверены, что хотите выбрать сертификат, срок действия которого истек?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
