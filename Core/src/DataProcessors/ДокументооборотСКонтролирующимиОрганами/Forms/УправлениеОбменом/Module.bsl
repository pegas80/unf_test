...
Процедура ВыгрузитьПакеты(ВидимыеЦиклы)
	...
	Если ВидимыеЦиклы.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбраны циклы обмена!';ru='Не выбраны циклы обмена!'"));
	Иначе
		Если ВидимыеЦиклы.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузить цикл обмена ""%1""?';ru='Выгрузить цикл обмена ""%1""?'"), КонтекстЭДОКлиент.ПредставлениеЦиклаОбмена(ВидимыеЦиклы[0]));
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузить выбранные %1 циклов обмена?';ru='Выгрузить выбранные %1 циклов обмена?'"), Строка(ВидимыеЦиклы.Количество()));
		КонецЕсли;
...
Функция СодержимоеОтправкиФССВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСС, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол'") + " " + СсылкаНаОбъектОтправкиФСС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбраны отправки ФСС!';ru='Не выбраны отправки ФСС!'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Зашифрованный пакет отсутствует.';ru='Зашифрованный пакет отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки отправленного пакета';ru='Выберите каталог для выгрузки отправленного пакета'");
		
	ИначеЕсли РежимВыгрузки = 1 Тогда
		
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Квитанция об успехе отсутствует.';ru='Квитанция об успехе отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки полученной квитанции';ru='Выберите каталог для выгрузки полученной квитанции'");
		
	ИначеЕсли РежимВыгрузки = 2 Тогда
		
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Протокол обработки отсутствует.';ru='Протокол обработки отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки полученного протокола обработки';ru='Выберите каталог для выгрузки полученного протокола обработки'");
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
		СписокВариантовВыбора.Добавить("Документы", НСтр("ka='შეტყობინება კონტროლირებადი გარიგებების შესახებ';ru='Уведомление о контролируемых сделках'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОРозничныхЦенахНаТабак", НСтр("ka='Уведомления о розничных ценах на табак';ru='Уведомления о розничных ценах на табак'"));
	КонецЕсли;
	
	СписокВариантовВыбора.Добавить("УведомленияОПолучателеДокументов", НСтр("ka='Уведомления о получателе документов';ru='Уведомления о получателе документов'"));
	
	СписокВариантовВыбора.Добавить("РеестрыФНС", НСтр("ka='Реестры по НДС';ru='Реестры по НДС'"));
	
	Если СписокВариантовВыбора.Количество() = 0 Тогда
		ПредставлениеУведомленийОСпецрежимах = НСтр("ka='Уведомления';ru='Уведомления'");
	Иначе
		ПредставлениеУведомленийОСпецрежимах = НСтр("ka='Прочие уведомления';ru='Прочие уведомления'");
	КонецЕсли;
	
	СписокВариантовВыбора.Добавить("ПрочиеУведомления", ПредставлениеУведомленийОСпецрежимах);
			
	СписокВариантовВыбора.Добавить("Файлы", НСтр("ka='ფაილიდან ჩატვირთვა';ru='Загрузить из файла'"));
...
Функция СодержимоеОтправкиФСРАРВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСРАР, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол'") + " " + СсылкаНаОбъектОтправкиФСРАР["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФСРАР(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбраны отправки в Росалкогольрегулирование!';ru='Не выбраны отправки в Росалкогольрегулирование!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиРПНВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиРПН, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол'") + " " + СсылкаНаОбъектОтправкиРПН["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиРПН(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не выбраны отправки в Росприродназдор!';ru='Не выбраны отправки в Росприродназдор!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	Если РежимВыгрузки = 0 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ხელმოწერა';ru='Подпись'") + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 1 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Квитанция"];
		ИмяФайлаДляКлиента = НСтр("ka='კვიტანცია';ru='Квитанция'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 2 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Протокол"];
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол'") + " " + СсылкаНаОбъектОтправкиФТС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФТС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не выбраны отправки в ФТС!';ru='Не выбраны отправки в ФТС!'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ka='Подпись отсутствует.';ru='Подпись отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки отправленного пакета';ru='Выберите каталог для выгрузки отправленного пакета'");
	...
	ИначеЕсли РежимВыгрузки = 1 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ka='Квитанция отсутствует.';ru='Квитанция отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки полученной квитанции';ru='Выберите каталог для выгрузки полученной квитанции'");
...
Процедура ОсновнаяКоманднаяПанельОбменятьсяПослеПолученияКонтекста(Результат, ВходящийКонтекст) Экспорт
	...
	Если ВыбранныеОрганизации.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Учетная запись не выбрана.';ru='Учетная запись не выбрана.'"));
		Возврат;
	...
	ИначеЕсли ВыбранныеОрганизации.Количество() = 1 Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Произвести обмен сообщениями по учетной записи ""%1""?';ru='Произвести обмен сообщениями по учетной записи ""%1""?'"), 
			ВыбранныеОрганизации[0]);
			
	Иначе
		ТекстВопроса = НСтр("ka='Произвести обмен сообщениями по выбранным учетным записям?';ru='Произвести обмен сообщениями по выбранным учетным записям?'");
	КонецЕсли;
...
Процедура СообщитьОСменеРежимов(ПредыдущийРежим, НовыйРежим)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Режим отображения списка был переключен с ""%1"" на ""%2""';ru='Режим отображения списка был переключен с ""%1"" на ""%2""'"), ПредыдущийРежим, НовыйРежим);
