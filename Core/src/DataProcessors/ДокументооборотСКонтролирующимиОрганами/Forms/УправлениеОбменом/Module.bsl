...
Процедура ВыгрузитьПакеты(ВидимыеЦиклы)
	...
	Если ВидимыеЦиклы.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбраны циклы обмена!';ru='Не выбраны циклы обмена!';en='Not selected exchange cycles!'"));
	Иначе
		Если ВидимыеЦиклы.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузить цикл обмена ""%1""?';ru='Выгрузить цикл обмена ""%1""?';en='Unload exchange cycle ""%1""?'"), КонтекстЭДОКлиент.ПредставлениеЦиклаОбмена(ВидимыеЦиклы[0]));
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузить выбранные %1 циклов обмена?';ru='Выгрузить выбранные %1 циклов обмена?';en='Unload selected% 1 exchange cycles?'"), Строка(ВидимыеЦиклы.Количество()));
		КонецЕсли;
...
Функция СодержимоеОтправкиФССВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСС, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол';en='Protocol'") + " " + СсылкаНаОбъектОтправкиФСС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбраны отправки ФСС!';ru='Не выбраны отправки ФСС!';en='Do not choose to send the FSS!'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Зашифрованный пакет отсутствует.';ru='Зашифрованный пакет отсутствует.';en='The encrypted packet is missing.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки отправленного пакета';ru='Выберите каталог для выгрузки отправленного пакета';en='Select the directory to upload sent packet'");
		
	ИначеЕсли РежимВыгрузки = 1 Тогда
		
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Квитанция об успехе отсутствует.';ru='Квитанция об успехе отсутствует.';en='Notification of success is missing.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки полученной квитанции';ru='Выберите каталог для выгрузки полученной квитанции';en='Select the directory to upload the resulting receipts'");
		
	ИначеЕсли РежимВыгрузки = 2 Тогда
		
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='Протокол обработки отсутствует.';ru='Протокол обработки отсутствует.';en='Protocol processing offline.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки полученного протокола обработки';ru='Выберите каталог для выгрузки полученного протокола обработки';en='Select the directory to upload the resulting treatment protocol'");
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
		СписокВариантовВыбора.Добавить("Документы", НСтр("ka='შეტყობინება კონტროლირებადი გარიგებების შესახებ';ru='Уведомление о контролируемых сделках';en='Notice of controlled transactions'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОРозничныхЦенахНаТабак", НСтр("ka='Уведомления о розничных ценах на табак';ru='Уведомления о розничных ценах на табак';en='Notification of retail prices for tobacco'"));
	КонецЕсли;
	
	СписокВариантовВыбора.Добавить("УведомленияОПолучателеДокументов", НСтр("ka='Уведомления о получателе документов';ru='Уведомления о получателе документов';en='Alert the recipient of documents'"));
	
	СписокВариантовВыбора.Добавить("РеестрыФНС", НСтр("ka='Реестры по НДС';ru='Реестры по НДС';en='VAT registers'"));
	
	Если СписокВариантовВыбора.Количество() = 0 Тогда
		ПредставлениеУведомленийОСпецрежимах = НСтр("ka='Уведомления';ru='Уведомления';en='notice'");
	Иначе
		ПредставлениеУведомленийОСпецрежимах = НСтр("ka='Прочие уведомления';ru='Прочие уведомления';en='Other notices'");
	КонецЕсли;
	
	СписокВариантовВыбора.Добавить("ПрочиеУведомления", ПредставлениеУведомленийОСпецрежимах);
			
	СписокВариантовВыбора.Добавить("Файлы", НСтр("ka='ფაილიდან ჩატვირთვა';ru='Загрузить из файла';en='Upload a file'"));
...
"Перейти к форме ""1С-Отчетность""?';ka='Создание реестров НДС выполняется из формы ""1С-Отчетность""."
...
Функция СодержимоеОтправкиФСРАРВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСРАР, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол';en='Protocol'") + " " + СсылкаНаОбъектОтправкиФСРАР["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФСРАР(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбраны отправки в Росалкогольрегулирование!';ru='Не выбраны отправки в Росалкогольрегулирование!';en='Do not choose to send Rosalkogolregulirovanie!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиРПНВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиРПН, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол';en='Protocol'") + " " + СсылкаНаОбъектОтправкиРПН["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиРПН(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не выбраны отправки в Росприродназдор!';ru='Не выбраны отправки в Росприродназдор!';en='Do not choose to send Rosprirodnazdor!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	Если РежимВыгрузки = 0 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ka='ხელმოწერა';ru='Подпись';en='Signature'") + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 1 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Квитанция"];
		ИмяФайлаДляКлиента = НСтр("ka='კვიტანცია';ru='Квитанция';en='Receipt'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 2 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Протокол"];
		ИмяФайлаДляКлиента = НСтр("ka='ოქმი';ru='Протокол';en='Protocol'") + " " + СсылкаНаОбъектОтправкиФТС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФТС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не выбраны отправки в ФТС!';ru='Не выбраны отправки в ФТС!';en='Do not choose to send to the FCS!'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ka='Подпись отсутствует.';ru='Подпись отсутствует.';en='The signature is missing.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки отправленного пакета';ru='Выберите каталог для выгрузки отправленного пакета';en='Select the directory to upload sent packet'");
	...
	ИначеЕсли РежимВыгрузки = 1 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ka='Квитанция отсутствует.';ru='Квитанция отсутствует.';en='The receipt is missing.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ka='Выберите каталог для выгрузки полученной квитанции';ru='Выберите каталог для выгрузки полученной квитанции';en='Select the directory to upload the resulting receipts'");
...
Процедура ОсновнаяКоманднаяПанельОбменятьсяПослеПолученияКонтекста(Результат, ВходящийКонтекст) Экспорт
	...
	Если ВыбранныеОрганизации.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Учетная запись не выбрана.';ru='Учетная запись не выбрана.';en='The account is not selected.'"));
		Возврат;
	...
	ИначеЕсли ВыбранныеОрганизации.Количество() = 1 Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Произвести обмен сообщениями по учетной записи ""%1""?';ru='Произвести обмен сообщениями по учетной записи ""%1""?';en='To exchange messages over the ""%1"" account?'"), 
			ВыбранныеОрганизации[0]);
			
	Иначе
		ТекстВопроса = НСтр("ka='Произвести обмен сообщениями по выбранным учетным записям?';ru='Произвести обмен сообщениями по выбранным учетным записям?';en='To exchange messages for the selected account?'");
	КонецЕсли;
...
Процедура СообщитьОСменеРежимов(ПредыдущийРежим, НовыйРежим)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Режим отображения списка был переключен с ""%1"" на ""%2""';ru='Режим отображения списка был переключен с ""%1"" на ""%2""';en='Display Mode list has been switched from ""%1"" to ""%2""'"), ПредыдущийРежим, НовыйРежим);
