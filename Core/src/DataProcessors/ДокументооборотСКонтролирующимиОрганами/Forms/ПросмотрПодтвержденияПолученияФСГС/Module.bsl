...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Ошибка чтения содержимого подтверждения из файла: %1';ru='Ошибка чтения содержимого подтверждения из файла: %1';en='Error reading the contents of the confirmation of the file:% 1'"),
	...
	Если НЕ ЗначениеЗаполнено(УзелИзвещение) Тогда
		ТекстСообщения = НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Извещение"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Извещение"".';en='Incorrect structure of the XML notification: not detected node ""Notification"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелПосылка) Тогда
		ТекстСообщения = НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Посылка"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Посылка"".';en='Incorrect structure of the XML notification: not detected node ""Sending"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокументы) Тогда
		ТекстСообщения = НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Документы"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Документы"".';en='""Documents"" node was not found: Incorrect structure of the XML notification.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если (ДокументыЦиклаОбмена = Неопределено) ИЛИ (ДокументыЦиклаОбмена.Количество() = 0) Тогда
		...
		ТекстСообщения = НСтр("ka='Не обнаружены документы, указанные в извещении.';ru='Не обнаружены документы, указанные в извещении.';en='Not found the documents specified in the notice.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
