...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтзыв) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".';ru='Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если ОбщиеСведения.Свойство("ПричЗам") Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".';ru='Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".'"),,,, Отказ);
			Возврат;	
		КонецЕсли;
	Иначе
