...
"%1';ka='Ошибка выгрузки сообщения об отзыве во временный файл:"
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтзыв) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".';ru='Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".';en='Incorrect structure of the XML file not found ""SvOtzyv"" node.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если ОбщиеСведения.Свойство("ПричЗам") Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".';ru='Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".';en='Incorrect structure of the XML file: invalid attribute value ""PrichZam"".'"),,,, Отказ);
			Возврат;	
		КонецЕсли;
	Иначе
