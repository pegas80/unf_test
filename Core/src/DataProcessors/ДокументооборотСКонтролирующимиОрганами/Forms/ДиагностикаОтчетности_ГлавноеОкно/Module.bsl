	...
	Если ТипЗнч(РезультатНастройки) = Тип("Булево") Тогда
		...
		Если ДополнительныеПараметры.ИспользоватьИнтерактивноеУведомление Тогда 
			Если РезультатНастройки = Истина Тогда
				Сообщение = НСтр("ka='Автоматическая настройка успешно завершена.';ru='Автоматическая настройка успешно завершена.';en='Automatic configuration completed successfully.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
			ИначеЕсли РезультатНастройки = Ложь Тогда
				Сообщение = НСтр("ka='Текущие настройки учетной записи актуальны!';ru='Текущие настройки учетной записи актуальны!';en='The current account is set up to date!'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура НачатьПроверку(Команда, АвтонастройкаВыполнена = Ложь) //0
	...
	Если Организация.Пустая() Тогда 
		Сообщение = НСтр("ka='Необходимо сначала выбрать организацию';ru='Необходимо сначала выбрать организацию';en='You must first select the organization'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
		Возврат;
	КонецЕсли;
	...
	Если ОсновнаяУчетнаяЗапись.ЭлектроннаяПодписьВМоделиСервиса Тогда 
		Сообщение = НСтр("ka='Запуск средства диагностики возможен только при наличии подключения к 1С-Отчетности с хранением ключей на локальном компьютере';ru='Запуск средства диагностики возможен только при наличии подключения к 1С-Отчетности с хранением ключей на локальном компьютере';en='Run diagnostic tools is only possible when connected to 1C reporting key storage on a local computer'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьСписокОшибокПослеРасшифровки(СписокОшибок, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ИнтерактивныеУведомления Тогда 
		Сообщение = НСтр("ka='Тестовая операция расшифровки завершилась неудачей';ru='Тестовая операция расшифровки завершилась неудачей';en='Test decryption operation is unsuccessful'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);		
...
Процедура ВыполнитьТестовыеОперацииРасшифроватьЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ВходящийКонтекст.ИнтерактивныеУведомления Тогда 
			Сообщение = НСтр("ka='Тестовые криптооперации шифрования и расшифровки завершились успешно';ru='Тестовые криптооперации шифрования и расшифровки завершились успешно';en='Test kriptooperatsii encryption and decryption were successful'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
...
Процедура ДинамическийЭлементОбработкаНавигационнойСсылки(Элемент, Ссылка, СтандартнаяОбработка)
	...
	Иначе
		Сообщение = НСтр("ka='Не удалось найти обработчик данного элемента';ru='Не удалось найти обработчик данного элемента';en='Could not find a handler for that element'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
	КонецЕсли;
...
Процедура ДекорацияТекстОбщийНажатие(Ссылка, ВхПараметры) Экспорт 
	...
	Иначе
		Сообщение = НСтр("ka='Не удалось найти подробное описание данного элемента';ru='Не удалось найти подробное описание данного элемента';en='Unable to find a detailed description of the item'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
	КонецЕсли;
