	...
	Если ТипЗнч(РезультатНастройки) = Тип("Булево") Тогда
		...
		Если ДополнительныеПараметры.ИспользоватьИнтерактивноеУведомление Тогда 
			Если РезультатНастройки = Истина Тогда
				Сообщение = НСтр("ka='Автоматическая настройка успешно завершена.';ru='Автоматическая настройка успешно завершена.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
			ИначеЕсли РезультатНастройки = Ложь Тогда
				Сообщение = НСтр("ka='Текущие настройки учетной записи актуальны!';ru='Текущие настройки учетной записи актуальны!'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура НачатьПроверку(Команда, АвтонастройкаВыполнена = Ложь) //0
	...
	Если Организация.Пустая() Тогда 
		Сообщение = НСтр("ka='Необходимо сначала выбрать организацию';ru='Необходимо сначала выбрать организацию'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
		Возврат;
	КонецЕсли;
	...
	Если ОсновнаяУчетнаяЗапись.ЭлектроннаяПодписьВМоделиСервиса Тогда 
		Сообщение = НСтр("ka='Запуск средства диагностики возможен только при наличии подключения к 1С-Отчетности с хранением ключей на локальном компьютере';ru='Запуск средства диагностики возможен только при наличии подключения к 1С-Отчетности с хранением ключей на локальном компьютере'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьСписокОшибокПослеРасшифровки(СписокОшибок, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ИнтерактивныеУведомления Тогда 
		Сообщение = НСтр("ka='Тестовая операция расшифровки завершилась неудачей';ru='Тестовая операция расшифровки завершилась неудачей'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);		
...
Процедура ВыполнитьТестовыеОперацииРасшифроватьЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ВходящийКонтекст.ИнтерактивныеУведомления Тогда 
			Сообщение = НСтр("ka='Тестовые криптооперации шифрования и расшифровки завершились успешно';ru='Тестовые криптооперации шифрования и расшифровки завершились успешно'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
...
Процедура ДинамическийЭлементОбработкаНавигационнойСсылки(Элемент, Ссылка, СтандартнаяОбработка)
	...
	Иначе
		Сообщение = НСтр("ka='Не удалось найти обработчик данного элемента';ru='Не удалось найти обработчик данного элемента'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
	КонецЕсли;
...
Процедура ДекорацияТекстОбщийНажатие(Ссылка, ВхПараметры) Экспорт 
	...
	Иначе
		Сообщение = НСтр("ka='Не удалось найти подробное описание данного элемента';ru='Не удалось найти подробное описание данного элемента'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение);
	КонецЕсли;
