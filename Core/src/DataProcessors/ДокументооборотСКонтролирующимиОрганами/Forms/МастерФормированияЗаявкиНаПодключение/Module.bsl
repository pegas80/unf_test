...
Процедура КоманднаяПанельМастерДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.МестоХраненияЭлектроннойПодписи Тогда
		...
		Если ДоступнаЭлектроннаяПодписьВМоделиСервиса Тогда
			Если РежимРаботыСКлючами = 0 Тогда  // Не выбрали ничего
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите место хранения ключа электронной подписи';ru='Укажите место хранения ключа электронной подписи'"), ,"РежимРаботыСКлючами");
				МастерДалее = Ложь;
			ИначеЕсли РежимРаботыСКлючами = 1 Тогда // Облачная ЭП
				Если НЕ ПроверкаТелефонДляПаролей.ЗначениеВведено Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ka='Укажите номер мобильного телефона';ru='Укажите номер мобильного телефона'"),, "ТелефонДляПаролей");
					МастерДалее = Ложь;
				Иначе
					Если Не ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
						Если ПроверкаТелефонДляПаролей.ВыполняетсяПроверка Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								НСтр("ka='Введите код из SMS';ru='Введите код из SMS'"),, "КодПодтверждения");
							МастерДалее = Ложь;
						Иначе
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								НСтр("ka='Выполните проверку номера телефона';ru='Выполните проверку номера телефона'"),, "ТелефонДляПаролей");
							МастерДалее = Ложь;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				Если ПроверкаЭлектроннаяПочтаДляПаролей.ЗначениеВведено Тогда
					Если Не ПроверкаЭлектроннаяПочтаДляПаролей.ПодтверждениеВыполнено Тогда
						Если ПроверкаЭлектроннаяПочтаДляПаролей.ВыполняетсяПроверка Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								НСтр("ka='Введите код из письма';ru='Введите код из письма'"),, "КодПодтверждения");
							МастерДалее = Ложь;
						Иначе
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								НСтр("ka='Выполните проверку адреса электронной почты';ru='Выполните проверку адреса электронной почты'"),, "ЭлектроннаяПочтаДляПаролей");
							МастерДалее = Ложь;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ИспользуемыйКриптопровайдер Тогда
		...
		Если ВыборКриптопровайдера = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите криптопровайдер';ru='Укажите криптопровайдер'"), ,"ВыборКриптопровайдера");
			МастерДалее = Ложь;
			Возврат;
		Иначе
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СоглашениеОПрисоединенииКРегламентуУЦ Тогда
		...
		Если Спецоператор = Неопределено ИЛИ Спецоператор= КонтекстЭДОКлиент.СтруктураСсылочныхДанных.Перечисления_СпецоператорыСвязи_ПустаяСсылка Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Удостоверяющий центр""';ru='Заполните поле ""Удостоверяющий центр""'"), ,"Спецоператор");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// проверка принятия лицензионного соглашения
		Если НЕ СогласиеСЛицензионнымСоглашением Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для продолжения работы необходимо принять соглашение';ru='Для продолжения работы необходимо принять соглашение'"), ,"СогласиеСЛицензионнымСоглашением");
			МастерДалее = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ГруппаДанныхДляЗаявления Тогда 
		...
		Если ПустаяСтрока(НомерОсновнойПоставки1с) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Регистрационный номер программы""';ru='Заполните поле ""Регистрационный номер программы""'"), ,"НомерОсновнойПоставки1с");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(НомерОсновнойПоставки1с, НСтр("ka='პროგრამის სარეგისტრაციო ნომერი';ru='Регистрационный номер программы'"), "НомерОсновнойПоставки1с") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			КонецЕсли;
		Иначе 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Организация, по которой будет сдаваться отчетность""';ru='Заполните поле ""Организация, по которой будет сдаваться отчетность""'"), ,"Организация");
			МастерДалее = Ложь;
			ГруппаДанныхДляЗаявления(МастерДалее);
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Документы Тогда 
		...
		Для Каждого ВидДокумента Из ВидыДокументов() Цикл
			...
			Если Не ЕстьФайлы Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ka='Добавьте файлы для документа ""%1""';ru='Добавьте файлы для документа ""%1""'"), ВидДокумента.Значение),,
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Реквизиты Тогда 
		...
		Если ПустаяСтрока(КраткоеНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните краткое наименование';ru='Заполните краткое наименование'"), ,"ДекорацияКраткоеНаименование");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(КраткоеНаименование, НСтр("ka='მოკლე დასახელება';ru='Краткое наименование'"), "ДекорацияКраткоеНаименование") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// полное наименование
		Если ПустаяСтрока(ПолноеНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните полное наименование';ru='Заполните полное наименование'"), ,"ДекорацияПолноеНаименование");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ПолноеНаименование, НСтр("ka='სრული დასახელება';ru='Полное наименование'"), "ДекорацияПолноеНаименование") Тогда
			МастерДалее = Ложь;
		КонецЕсли;	
		...
		Если ЭтоЮридическоеЛицо Тогда
			...
			Если ПустаяСтрока(ИНН) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните ИНН';ru='Заполните ИНН'"), ,"ДекорацияИНН");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ИНН,10) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ИНН должен состоять из 10 цифр';ru='ИНН должен состоять из 10 цифр'"), ,"ДекорацияИНН");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// КПП
			Если ПустаяСтрока(КПП) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните КПП';ru='Заполните КПП'"), ,"ДекорацияКПП");
				МастерДалее = Ложь;
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьКПП(КПП)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='КПП должен состоять из 9 цифр';ru='КПП должен состоять из 9 цифр'"), ,"ДекорацияКПП");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// ОГРН
			Если ПустаяСтрока(ОГРН) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните ОГРН';ru='Заполните ОГРН'"), ,"ОГРН");
				МастерДалее = Ложь;
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ОГРН,13)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ОГРН должен состоять из 13 цифр';ru='ОГРН должен состоять из 13 цифр'"), ,"ОГРН");
				МастерДалее = Ложь;
			КонецЕсли;
		...
		Иначе
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ИНН,12) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ИНН должен состоять из 12 цифр';ru='ИНН должен состоять из 12 цифр'"), ,"ДекорацияИНН");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// ОГРН
			Если ПустаяСтрока(ОГРН) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните ОГРНИП';ru='Заполните ОГРНИП'"), ,"ОГРН");
				МастерДалее = Ложь;
			ИначеЕсли ЗначениеЗаполнено(ОГРН) И НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ОГРН,15, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ОГРНИП должен состоять из 15 цифр';ru='ОГРНИП должен состоять из 15 цифр'"), ,"ОГРН");
				МастерДалее = Ложь;
			КонецЕсли;
		...
		Если СдаватьВПФР Тогда		
			Если ПустаяСтрока(СтрЗаменить(РегНомерПФР,"-","")) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните регистрационный номер в ПФР';ru='Заполните регистрационный номер в ПФР'"), ,"ДекорацияРегНомерПФР");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьРегистрационныйНомерПФР(РегНомерПФР, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Регистрационный номер в ПФР должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)';ru='Регистрационный номер в ПФР должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)'"), ,"ДекорацияРегНомерПФР");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если СдаватьВФСС Тогда
			...
			Если ПустаяСтрока(РегНомерФСС) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните регистрационный номер в ФСС';ru='Заполните регистрационный номер в ФСС'"), ,"ДекорацияРегНомерФСС");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(РегНомерФСС, 10, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Регистрационный номер в ФСС должен состоять из 10 цифр';ru='Регистрационный номер в ФСС должен состоять из 10 цифр'"), ,"ДекорацияРегНомерФСС");
				МастерДалее = Ложь;
			КонецЕсли;
			...
			Если ПризнакОбособленногоПодразделения И ЭтоЮридическоеЛицо И СдаватьВФСС Тогда
				Если ПустаяСтрока(ДополнительныйКодФСС) Тогда 
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните дополнительный код ФСС';ru='Заполните дополнительный код ФСС'"), ,"ДополнительныйКодФСС");
					МастерДалее = Ложь;
				ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ДополнительныйКодФСС, 10, Истина) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Дополнительный код ФСС должен состоять из 10 цифр';ru='Дополнительный код ФСС должен состоять из 10 цифр'"), ,"ДополнительныйКодФСС");
					МастерДалее = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(СтрЗаменить(АдрЮР,",","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Юридический адрес""';ru='Заполните поле ""Юридический адрес""'"), ,"АдресЮридический");
			МастерДалее = Ложь;
		Иначе	
			СтрокаОшибкаАдреса = КонтекстЭДОКлиент.ПроверитьАдрес(АдрЮР);
			Если НЕ ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Юридический адрес: %1';ru='Юридический адрес: %1'"),СтрокаОшибкаАдреса), ,"АдресЮридический");
				МастерДалее = Ложь;
			ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(АдрЮР, НСтр("ka='იურიდიული მისამართი';ru='Юридический адрес'"), "АдресЮридический") Тогда
				МастерДалее = Ложь;
			Иначе
...
"Сократите длину адреса, убрав некоторые его части (например, оставив только дом и корпус)'");
		...
		Если ПустаяСтрока(СтрЗаменить(АдрФакт,",","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Фактический адрес""';ru='Заполните поле ""Фактический адрес""'"), ,"АдресФактический");
			МастерДалее = Ложь;
		Иначе
			СтрокаОшибкаАдреса = КонтекстЭДОКлиент.ПроверитьАдрес(АдрФакт);
			Если НЕ ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Фактический адрес: %1';ru='Фактический адрес: %1'"),СтрокаОшибкаАдреса), ,"АдресФактический");
				МастерДалее = Ложь;
			ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(АдрФакт, НСтр("ka='ფაქტობრივი მისამართი';ru='Фактический адрес'"), "АдресФактический") Тогда
				МастерДалее = Ложь;
			Иначе
...
"Сократите длину адреса, убрав некоторые его части (например, оставив только дом и корпус)'");
		...
		Если СтрДлина(СокрЛП(ТелефонОсновной)) = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Телефон организации""';ru='Заполните поле ""Телефон организации""'"), ,"ТелефонОсновной");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ТелефонОсновной, НСтр("ka='ორგანიზაციის ტელეფონი';ru='Телефон организации'"), "ТелефонОсновной") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если НЕ КонтекстЭДОКлиент.ЭлектроннаяПочтаВведенаКорректно(
			...
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ЭлектроннаяПочта, НСтр("ka='ორგანიზაციის ელექტრონული ფოსტა';ru='Электронная почта организации'"), "ЭлектроннаяПочта") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ПодатьЗаявкуНаСертификатДляФСРАР Тогда
			...
			Если НЕ ЗначениеЗаполнено(КодРегионаФСРАР) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Регион""';ru='Заполните поле ""Регион""'"), ,"КодРегионаФСРАР");
				МастерДалее = Ложь;
			КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВладельцыЭцп Тогда
		...
		Если ПустаяСтрока(ВладелецЭЦПФамилия) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните фамилию';ru='Заполните фамилию'"), ,"ВладелецЭЦПФамилия");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПФамилия, НСтр("ka='გვარი';ru='Фамилия'"), "ВладелецЭЦПФамилия") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// имя
		Если ПустаяСтрока(ВладелецЭЦПИмя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните имя';ru='Заполните имя'"), ,"ВладелецЭЦПИмя");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПИмя, НСтр("ka='სახელი';ru='Имя'"), "ВладелецЭЦПИмя") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// отчетство
		Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПОтчество, НСтр("ka='მამის სახელი';ru='Отчество'"), "ВладелецЭЦПОтчество") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// СНИЛС
		Если ПустаяСтрока(СтрЗаменить(ВладелецЭЦПСНИЛС, "-","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეავსეთ  СНИЛС';ru='Заполните СНИЛС'"), ,"ВладелецЭЦПСНИЛС");
			МастерДалее = Ложь;
		Иначе	
			Если НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректно указан СНИЛС сотрудника. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра';ru='Некорректно указан СНИЛС сотрудника. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра'"), ,"ВладелецЭЦПСНИЛС");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС, Ложь, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)';ru='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)'"), ,"ВладелецЭЦПСНИЛС");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ВладелецЭЦПДатаРождения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Дата рождения""';ru='Заполните поле ""Дата рождения""'"), ,"ВладелецЭЦПДатаРождения");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// Пол
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦППол) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Пол""';ru='Заполните поле ""Пол""'"), ,"ВладелецЭЦППол");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// должность
		Если ЭтоЮридическоеЛицо И ПустаяСтрока(ВладелецЭЦПДолжность) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Должность""';ru='Заполните поле ""Должность""'"), ,"ВладелецЭЦПДолжность");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПДолжность, НСтр("ka='თანამდებობა';ru='Должность'"), "ВладелецЭЦПДолжность") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// Подразделение
		Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦППодразделение, НСтр("ka='ქვეგანყოფილება';ru='Подразделение'"), "ВладелецЭЦППодразделение") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// Гражданство
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦПГражданство) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Гражданство""';ru='Заполните поле ""Гражданство""'"), ,"ВладелецЭЦПГражданство");
			МастерДалее = Ложь;
		ИначеЕсли НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.КодАльфа2Заполнен(ВладелецЭЦПГражданство)
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП';ru='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП'"), ,"ВладелецЭЦПГражданство");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПВидДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Документ, удостоверяющий личность""';ru='Заполните поле ""Документ, удостоверяющий личность""'"), ,"ВладелецЭЦПВидДокумента");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПВидДокумента, НСтр("ka='პირადობის დამადასტურებელი დოკუმენტი';ru='Документ, удостоверяющий личность'"), "ВладелецЭЦПВидДокумента") Тогда
			МастерДалее = Ложь;
		ИначеЕсли НЕ ЗначениеЗаполнено(КонтекстЭДОКлиент.ПолучитьКодВидаДокументаФизическогоЛица(ВладелецЭЦПВидДокумента)) Тогда
			ТекстОшибки = НСтр("ka='Заполните код ИФНС в виде документа ""%1""';ru='Заполните код ИФНС в виде документа ""%1""'");
		...
		Если ПустаяСтрока(ВладелецЭЦПСерияДокумента) И НЕ КонтекстЭДОКлиент.ПроверитьСериюДокумента(ВладелецЭЦПВидДокумента,ВладелецЭЦПСерияДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Серия""';ru='Заполните поле ""Серия""'"), ,"ВладелецЭЦПСерияДокумента");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПСерияДокумента, НСтр("ka='სერია';ru='Серия'"), "ВладелецЭЦПСерияДокумента") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// номер
		Если ПустаяСтрока(ВладелецЭЦПНомерДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Номер""';ru='Заполните поле ""Номер""'"), ,"ВладелецЭЦПНомерДокумента");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПНомерДокумента, НСтр("ka='ნომერი';ru='Номер'"), "ВладелецЭЦПНомерДокумента") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// кем выдан
		Если ПустаяСтрока(ВладелецЭЦПКемВыданДокумент) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Кем выдан""';ru='Заполните поле ""Кем выдан""'"), ,"ВладелецЭЦПКемВыданДокумент");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПКемВыданДокумент, НСтр("ka='გაცემულია';ru='Кем выдан'"), "ВладелецЭЦПКемВыданДокумент") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// дата выдачи
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦПДатаВыдачиДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Дата выдачи""';ru='Заполните поле ""Дата выдачи""'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
			МастерДалее = Ложь;
		ИначеЕсли ВладелецЭЦПДатаВыдачиДокумента > ДатаЗапускаМастера ИЛИ Год(ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда	
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректно указана дата выдачи';ru='Некорректно указана дата выдачи'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ВладелецЭЦПВидДокумента = ПаспортРФ Тогда
			...
			Если ПустаяСтрока(КодПодразделенияБезТире) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Код подразделения""';ru='Заполните поле ""Код подразделения""'"), ,"ВладелецЭЦПКодПодразделения");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(ВладелецЭЦПКодПодразделения) <> 7
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра';ru='Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра'"), ,"ВладелецЭЦПКодПодразделения");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПМестоРождения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Место рождения""';ru='Заполните поле ""Место рождения""'"), ,"ВладелецЭЦПМестоРождения");
			МастерДалее = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Направления Тогда
		...
		Если СдаватьВФНС И ПолучателиФНС.Количество()=0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните коды ФНС';ru='Заполните коды ФНС'"), ,"ПолучателиФНС");
			МастерДалее = Ложь;
		ИначеЕсли СдаватьВФНС Тогда
			Для каждого Получатель Из ПолучателиФНС Цикл
				Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КПП, НСтр("ka='КПП';ru='КПП'"), "ПолучателиФНС") Тогда
					МастерДалее = Ложь;
					Прервать;
				КонецЕсли;
				Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КодПолучателя, НСтр("ka='კოდიФНС';ru='Код ФНС'"), "ПолучателиФНС") Тогда
					МастерДалее = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла; 
		...
		Если СдаватьВПФР Тогда
			Если ПустаяСтрока(СтрЗаменить(КодПФР,"-","")) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Код отделения ПФР""';ru='Заполните поле ""Код отделения ПФР""'"), ,"КодПФР");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(СокрЛП(КодПФР))<> 7 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код отделения ПФР должен состоять из 6 цифр';ru='Код отделения ПФР должен состоять из 6 цифр'"), ,"КодПФР");
				МастерДалее = Ложь;
			ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(КодПФР, НСтр("ka='Код отделения ПФР';ru='Код отделения ПФР'"), "КодПФР") Тогда
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если СдаватьВРосстат Тогда
			...
			Если ПолучателиФСГС.Количество() = 0 Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните коды органа Росстата';ru='Заполните коды органа Росстата'"), ,"ПолучателиФСГС");
				МастерДалее = Ложь;
			...
			ИначеЕсли ПолучателиФСГС.Количество() > 0 Тогда
				...
				Для каждого Получатель Из ПолучателиФСГС Цикл
					...
					Если ПустаяСтрока(СтрЗаменить(Получатель.КодПолучателя,"-","")) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Код органа Росстата""';ru='Заполните поле ""Код органа Росстата""'"), ,"ПолучателиФСГС");
						МастерДалее = Ложь;
					ИначеЕсли СтрДлина(СокрЛП(Получатель.КодПолучателя))<> 5 Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код органа Росстата должен состоять из 4 цифр';ru='Код органа Росстата должен состоять из 4 цифр'"), ,"ПолучателиФСГС");
						МастерДалее = Ложь;
					ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КодПолучателя, НСтр("ka='Код органа Росстата';ru='Код органа Росстата'"), "ПолучателиФСГС") Тогда
						МастерДалее = Ложь;
					КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ПодключениеЭДО Тогда 
		...
		Если ПодключитьЭДО Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОператорЭДО) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Оператор ЭДО""';ru='Заполните поле ""Оператор ЭДО""'"), ,"ОператорЭДО");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// код ФНС
			Если ПустаяСтрока(КодНалоговогоОрганаЭДО) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Код ФНС""';ru='Заполните поле ""Код ФНС""'"), ,"КодНалоговогоОрганаЭДО");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(СокрЛП(КодНалоговогоОрганаЭДО))<> 4 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код ФНС должен состоять из 4 цифр';ru='Код ФНС должен состоять из 4 цифр'"), ,"КодНалоговогоОрганаЭДО");
				МастерДалее = Ложь;
			КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Подтверждение Тогда 
		...
		Если ЭтоРежимОграниченнойФункциональности И Не ФайлЗаявленияДобавлен(ЭтаФорма) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Добавьте файлы для документа ""%1""';ru='Добавьте файлы для документа ""%1""'"), ВидыДокументов().Заявление),,
				"СсылкаЗаявление");
...
Функция СформироватьЗаявлениеПФР()
	...
	Иначе
		...
		Если УказанПаспорт Тогда
			...
			Выдан = НСтр("ka='%1, код подразделения %2, выдан %3';ru='%1, код подразделения %2, выдан %3'");
...
Процедура ПодтвердитьВосстановлениеНаправления(ДополнительныеПараметры)
	...
	ТекстВопроса = НСтр("ka='Восстановить значения кодов по умолчанию?';ru='Восстановить значения кодов по умолчанию?'");
...
Процедура ОпределитьОрганизацию(Отказ)
	...
	Если ИспользуетсяОднаОрганизация Тогда
		...
		Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Организация) Тогда
			Отказ = Истина;
			Сообщить(НСтр("ka='Поддержка регламентированнной отчетности доступна только для индивидуальных предпринимателей!';ru='Поддержка регламентированнной отчетности доступна только для индивидуальных предпринимателей!'"),СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	Иначе
...
Процедура ГруппаДанныхДляЗаявления(МастерДалее)
	...
	Если ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.Руководитель") 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните сведения о руководителе';ru='Заполните сведения о руководителе'"), ,"ДекорацияРуководитель");
		МастерДалее = Ложь;
	КонецЕсли;
	...
	Если ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.ГлавныйБухгалтер")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните сведения о бухгалтере';ru='Заполните сведения о бухгалтере'"), ,"ДекорацияГлБухгалтер");
		МастерДалее = Ложь;
	КонецЕсли;
	...
	Если ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.ДругойСотрудник")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выберите сотрудника';ru='Выберите сотрудника'"), ,"СотрудникВыбор");
		МастерДалее = Ложь;
	КонецЕсли;	
	...
	Если НЕ (СдаватьВФНС ИЛИ СдаватьВПФР ИЛИ СдаватьВФСС ИЛИ СдаватьВРосстат ИЛИ ПодатьЗаявкуНаСертификатДляФСРАР ИЛИ ПодатьЗаявкуНаПодключениеРПН ИЛИ ПодатьЗаявкуНаПодключениеФТС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность';ru='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность'"), ,"СдаватьВФНС");
		МастерДалее = Ложь;
	ИначеЕсли НЕ СдаватьВФНС И НЕ СдаватьВПФР Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР';ru='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР'"), ,"СдаватьВФНС");
		МастерДалее = Ложь;
	КонецЕсли;
	...
	Если ПолучатьСМС Тогда
		...
		Если ПустаяСтрока(ТелефонМобильныйБезРазделителей) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните мобильный телефон';ru='Заполните мобильный телефон'"), ,"ТелефонМобильный");
			МастерДалее = Ложь;
...
Процедура УстановитьЗаголовокФормы()
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ДобавитьОрганизациюВЗаголовок(
		...
		КраткоеНаименование,
		// Разный заголовок!
		НСтр("ka='Подключение к 1С-Отчетности';ru='Подключение к 1С-Отчетности'"));
...
Процедура ЗаполнитьСписокВыбораСотрудника()
	...
	НаименованиеРуководителя = ?(
		...
		НСтр("ka='ხელმძღვანელი';ru='Руководитель'"), 
		НСтр("ka='Предприниматель';ru='Предприниматель'"));
	...
	Если ПоказыватьТриТипаСотрудников Тогда
		СписокВыбора.Добавить(
			...
			НСтр("ka='მთავარი ბუღალტერი';ru='Главный бухгалтер'"));
	КонецЕсли;
		
	СписокВыбора.Добавить(
		Перечисления.ТипыВладельцевЭЦП.ДругойСотрудник, 
		НСтр("ka='Другой сотрудник:';ru='Другой сотрудник:'"));
...
Процедура ОбновитьДанныеСпецОператора()
	...
	ЭлементСсылкаНаСайтУдостоверяющегоЦентра.Заголовок  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Сайт %1';ru='Сайт %1'"),Строка(Спецоператор));
...
Процедура ОбновитьДоступностьПодачиВФСРАР()
	...
	Элементы.ПодатьЗаявкуНаСертификатДляФСРАР.Подсказка = ?(ПризнакПоддержкиФСРАР, НСтр("ka='Федеральная служба по регулированию алкогольного рынка';ru='Федеральная служба по регулированию алкогольного рынка'"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Федеральная служба по регулированию алкогольного рынка (не поддерживается %1)';ru='Федеральная служба по регулированию алкогольного рынка (не поддерживается %1)'"), Строка(Спецоператор)));
...
Процедура ОбновитьДоступностьПодачиВРПН()
	...
	Элементы.ПодатьЗаявкуНаПодключениеРПН.Подсказка = ?(ПризнакПоддержкиРПН, НСтр("ka='Федеральная служба по надзору в сфере природопользования';ru='Федеральная служба по надзору в сфере природопользования'"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Федеральная служба по надзору в сфере природопользования (не поддерживается %1)';ru='Федеральная служба по надзору в сфере природопользования (не поддерживается %1)'"), Строка(Спецоператор)));
...
Процедура ОбновитьДоступностьПодачиВФТС()
	...
	Элементы.ПодатьЗаявкуНаПодключениеФТС.Подсказка = ?(ПризнакПоддержкиФТС, НСтр("ka='Федеральная таможенная служба';ru='Федеральная таможенная служба'"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Федеральная таможенная служба (не поддерживается %1)';ru='Федеральная таможенная служба (не поддерживается %1)'"), Строка(Спецоператор)));
...
Процедура ИзменитьОформлениеРеквизитовОрганизации()
	...
	Если ЭтоИП Тогда
		...
		Если ИПИспользуетТрудНаемныхРаботников Тогда
			Элементы.НадписьРегистрационныйНомерВФСС.Заголовок = НСтр("ka='Рег. номер в ФСС (за сотрудников):';ru='Рег. номер в ФСС (за сотрудников):'");
			Элементы.НадписьРегистрационныйНомерВПФР.Заголовок = НСтр("ka='Рег. номер в ПФР (за сотрудников):';ru='Рег. номер в ПФР (за сотрудников):'");
		Иначе
			Элементы.НадписьРегистрационныйНомерВФСС.Заголовок = НСтр("ka='Рег. номер в ФСС (за себя):';ru='Рег. номер в ФСС (за себя):'");
			Элементы.НадписьРегистрационныйНомерВПФР.Заголовок = НСтр("ka='Рег. номер в ПФР (за себя):';ru='Рег. номер в ПФР (за себя):'");
		КонецЕсли;
		
	Иначе
		
		Элементы.НадписьРегистрационныйНомерВФСС.Заголовок = НСтр("ka='Регистрационный номер в ФСС:';ru='Регистрационный номер в ФСС:'");
		Элементы.НадписьРегистрационныйНомерВПФР.Заголовок = НСтр("ka='Регистрационный номер в ПФР:';ru='Регистрационный номер в ПФР:'");
...
Процедура СформироватьИОтправитьЗаявление()
	...
	Если РежимРаботыСКлючами = 1 И Не ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите и подтвердите телефон для получения временных паролей';ru='Укажите и подтвердите телефон для получения временных паролей'"),, "ТелефонДляПаролей");
		УправлениеФормой(ЭтаФорма);
		Возврат;
	КонецЕсли;
...
"2. Проверьте, что отключен контроль учетных записей (UAC) в Windows.'");
	Если ВыборКриптопровайдера = 1 Тогда
		 Элементы.ПодсказкаПоРезультатам8.Заголовок = Элементы.ПодсказкаПоРезультатам8.Заголовок + Символы.ПС + 
		 	НСтр("ka='3. Проверьте, что VipNet зарегистрирован.';ru='3. Проверьте, что VipNet зарегистрирован.'");
	КонецЕсли;
...
Функция ПредставлениеФайловПоУмолчанию()
	...
	Возврат НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл'");
