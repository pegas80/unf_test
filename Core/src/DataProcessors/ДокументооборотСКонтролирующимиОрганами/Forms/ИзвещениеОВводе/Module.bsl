...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрИзвещения.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ka='Извещение о вводе не обнаружено среди содержимого сообщения.';ru='Извещение о вводе не обнаружено среди содержимого сообщения.'");
		Возврат;
	КонецЕсли;
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Файл"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Файл"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Документ"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Документ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвИзвещВ) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""СвИзвещВ"".';ru='Некорректная структура XML извещения: не обнаружен узел ""СвИзвещВ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелКодНоОтпр) Тогда
		ТекстСообщения = НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""КодНоОтпр"".';ru='Некорректная структура XML извещения: не обнаружен узел ""КодНоОтпр"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
