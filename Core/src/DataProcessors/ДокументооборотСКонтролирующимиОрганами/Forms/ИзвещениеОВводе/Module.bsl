...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрИзвещения.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ka='Извещение о вводе не обнаружено среди содержимого сообщения.';ru='Извещение о вводе не обнаружено среди содержимого сообщения.';en='Notification of commissioning not found among the contents of the message.'");
		Возврат;
	КонецЕсли;
...
"%1';ka='Ошибка выгрузки извещения о вводе во временный файл:"
...
"%1';ka='Ошибка чтения XML из файла извещения о вводе::"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Файл"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Файл"".';en='Incorrect structure of the XML notification: not detected node ""File"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""Документ"".';ru='Некорректная структура XML извещения: не обнаружен узел ""Документ"".';en='Incorrect structure of the XML notification: not detected node ""document"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвИзвещВ) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""СвИзвещВ"".';ru='Некорректная структура XML извещения: не обнаружен узел ""СвИзвещВ"".';en='Incorrect structure of the XML notification: not detected node ""SvIzveschV"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелКодНоОтпр) Тогда
		ТекстСообщения = НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""КодНоОтпр"".';ru='Некорректная структура XML извещения: не обнаружен узел ""КодНоОтпр"".';en='Incorrect structure of the XML notification: not detected node ""KodNoOtpr"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
