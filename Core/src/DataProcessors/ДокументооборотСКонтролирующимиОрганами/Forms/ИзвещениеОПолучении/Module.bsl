...
"%1';ka='Ошибка выгрузки извещения о получении во временный файл:"
...
"%1';ka='Ошибка чтения XML из файла извещения о получении:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвИзвещП) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML извещения: не обнаружен узел ""СвИзвещП"".';ru='Некорректная структура XML извещения: не обнаружен узел ""СвИзвещП"".';en='Incorrect structure of the XML notification: not detected node ""SvIzveschP"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
