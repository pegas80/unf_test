...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СканПаспорт = НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл'");
...
Процедура ПродлитьСертификатПриИзменении(Элемент)
	...
	Если ПродлитьСертификатИсходный И НЕ ПродлитьСертификат Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='დიახ';ru='Да'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена'"));
...
Процедура ПодписатьЗаявлениеИОтправить(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите скан-копию паспорта владельца сертификата';ru='Укажите скан-копию паспорта владельца сертификата'"), ,"СканПаспорт");
	КонецЕсли;
...
Процедура КоманднаяПанельМастерДалее(Команда, БезбумажноеПродление = Ложь)
	...
	Если ТекущаяСтраница = Элементы.ОсновнаяПанель.ПодчиненныеЭлементы.ВыборДействия Тогда
		...
		Если НЕ ЗначениеЗаполнено(Организация) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Организация""';ru='Заполните поле ""Организация""'"), ,"Организация");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ИзменитьСоставКонтролирующихОрганов Тогда
			...
			Если СдаватьВФНС И ПолучателиФНС.Количество()=0 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните коды ФНС';ru='Заполните коды ФНС'"), ,"СдаватьВФНС");
				МастерДалее = Ложь;
			КонецЕсли;
			...
			Если СдаватьВПФР Тогда
				...
				Если ПустаяСтрока(СтрЗаменить(КодПФР,"-","")) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Код отделения ПФР""';ru='Заполните поле ""Код отделения ПФР""'"), ,"СдаватьВПФР");
					МастерДалее = Ложь;
				ИначеЕсли СтрДлина(СокрЛП(КодПФР))<> 7 Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код отделения ПФР должен состоять из 6 цифр';ru='Код отделения ПФР должен состоять из 6 цифр'"), ,"СдаватьВПФР");
					МастерДалее = Ложь;
				ИначеЕсли СокрЛП(КодПФР) <> КодПФРВОрганизации Тогда
					ТекстСообщения = НСтр("ka='Код отделения ПФР не совпадает с кодом, указанным в организации (%1)';ru='Код отделения ПФР не совпадает с кодом, указанным в организации (%1)'");
			...
			Если СдаватьВРосстат Тогда
				...
				Если ПолучателиФСГС.Количество() = 0 Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните коды органа Росстата';ru='Заполните коды органа Росстата'"), ,"СдаватьВРосстат");
					МастерДалее = Ложь;
				...
				ИначеЕсли ПолучателиФСГС.Количество() > 0 Тогда
					...
					Для каждого Получатель Из ПолучателиФСГС Цикл
						...
						Если ПустаяСтрока(СтрЗаменить(Получатель.КодПолучателя,"-","")) Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните поле ""Код органа Росстата""';ru='Заполните поле ""Код органа Росстата""'"), ,"СдаватьВРосстат");
							МастерДалее = Ложь;
						ИначеЕсли СтрДлина(СокрЛП(Получатель.КодПолучателя))<> 5 Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код органа Росстата должен состоять из 4 цифр';ru='Код органа Росстата должен состоять из 4 цифр'"), ,"СдаватьВРосстат");
							МастерДалее = Ложь;
						ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КодПолучателя, НСтр("ka='Код органа Росстата';ru='Код органа Росстата'"), "СдаватьВРосстат") Тогда
							МастерДалее = Ложь;
						КонецЕсли;
		...
		Если ИзменитьСоставКонтролирующихОрганов Тогда
			Если НЕ (СдаватьВФНС ИЛИ СдаватьВПФР ИЛИ СдаватьВФСС ИЛИ СдаватьВРосстат ИЛИ СдаватьВФСРАР ИЛИ СдаватьВРПН ИЛИ СдаватьВФТС) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность';ru='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность'")
					, ,"ИзменитьСоставКонтролирующихОрганов");
				МастерДалее = Ложь;
			ИначеЕсли НЕ СдаватьВФНС И НЕ СдаватьВПФР Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР';ru='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР'")
					, ,"ИзменитьСоставКонтролирующихОрганов");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выберите руководителя';ru='Выберите руководителя'"), ,"ДекорацияРуководитель");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выберите бухгалтера';ru='Выберите бухгалтера'"), ,"ДекорацияГлБухгалтер");
			МастерДалее = Ложь;
		КонецЕсли;
...
"Установите флажок ""Изменение сотрудника-владельца сертификата"" и выберите владельца сертификата'");
		...
		Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выберите сотрудника-владельца сертификата';ru='Выберите сотрудника-владельца сертификата'"), ,"СотрудникВыбор");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если НЕ ПереиздатьСертификат 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Вы не выбрали никакого действия';ru='Вы не выбрали никакого действия'"), ,"");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ИзменитьМобильныйТелефон Тогда
			Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
				...
				Если Не ТелефонИзменен И Не ЭлектроннаяПочтаИзменена И НЕ ПолучатьУведомленияИзменен Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите номер мобильного телефона';ru='Укажите номер мобильного телефона'"),, "ТелефонДляПаролей");
					МастерДалее = Ложь;
				ИначеЕсли ТелефонИзменен И Не ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Подтвердите номер мобильного телефона';ru='Подтвердите номер мобильного телефона'"),, "ТелефонДляПаролей");
					МастерДалее = Ложь;
				ИначеЕсли ЭлектроннаяПочтаИзменена И Не ПроверкаЭлектроннаяПочтаДляПаролей.ПодтверждениеВыполнено И ЗначениеЗаполнено(ЭлектроннаяПочтаДляПаролей) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Подтвердите адрес электронной почты';ru='Подтвердите адрес электронной почты'"),, "ЭлектроннаяПочтаДляПаролей");
					МастерДалее = Ложь;
				КонецЕсли;
			Иначе
				Если ПолучатьУведомления Тогда
					...
					Если ПустаяСтрока(ТелефонМобильныйБезРазделителей) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите номер мобильного телефона';ru='Укажите номер мобильного телефона'"),, "ТелефонМобильный");
						МастерДалее = Ложь;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ОсновнаяПанель.ПодчиненныеЭлементы.ПодключениеЭДО Тогда
		...
		Если ПодключитьЭДО Тогда
			...
			Если ПустаяСтрока(СтрЗаменить(АдрЮР,",","")) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Заполните юридический адрес';ru='Заполните юридический адрес'"), ,"АдресЮридический");
				МастерДалее = Ложь;
			Иначе	
...
Процедура ПроверитьЗаполнениеЗаявления(МастерДалее)
	...
	Если НомерОсновнойПоставки1сИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если ПустаяСтрока(НомерОсновнойПоставки1с)  Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.НомерОсновнойПоставки1С");
			ТекстОшибки 		= НСтр("ka='Заполните регистрационный номер программы';ru='Заполните регистрационный номер программы'");
	...
	Если ПолноеНаименованиеИзменилось И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если ПустаяСтрока(ПолноеНаименование) Тогда
			...
			ТекстОшибки 		= НСтр("ka='Заполните полное наименование организации';ru='Заполните полное наименование организации'");
			ВывестиОшибку(ТекстОшибки);
	...
	Если ЭтоЮридическоеЛицо Тогда
		...
		Если ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
			Если ПустаяСтрока(ИНН) Тогда 
				...
				ТекстОшибки 		= НСтр("ka='Заполните ИНН';ru='Заполните ИНН'");
				ВывестиОшибку(ТекстОшибки);
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ИНН,10) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ИНН");
				ТекстОшибки 		= НСтр("ka='ИНН должен состоять из 10 цифр';ru='ИНН должен состоять из 10 цифр'");
				ВывестиОшибку(ТекстОшибки);
		...
		Если КППИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
			Если ПустаяСтрока(КПП) Тогда
				...
				ТекстОшибки 		= НСтр("ka='Заполните КПП';ru='Заполните КПП'");
				ВывестиОшибку(ТекстОшибки);
			...
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьКПП(КПП)) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.КПП");
				ТекстОшибки 		= НСтр("ka='КПП должен состоять из 9 цифр';ru='КПП должен состоять из 9 цифр'");
				ВывестиОшибку(ТекстОшибки);
		...
		Если ОГРНИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
			Если ПустаяСтрока(ОГРН) Тогда
				...
				ТекстОшибки 		= НСтр("ka='Заполните ОГРН';ru='Заполните ОГРН'");
				ВывестиОшибку(ТекстОшибки);
			...
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ОГРН,13)) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ОГРН");
				ТекстОшибки 		= НСтр("ka='ОГРН должен состоять из 13 цифр';ru='ОГРН должен состоять из 13 цифр'");
				ВывестиОшибку(ТекстОшибки);
	...
	Иначе
		...
		Если ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ИНН,12) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ИНН");
				ТекстОшибки 		= НСтр("ka='ИНН должен состоять из 12 цифр';ru='ИНН должен состоять из 12 цифр'");
				ВывестиОшибку(ТекстОшибки);
		...
		Если ОГРНИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
			...
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ОГРН,15, Истина)) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ОГРН");
				ТекстОшибки 		= НСтр("ka='ОГРНИП должен состоять из 15 цифр';ru='ОГРНИП должен состоять из 15 цифр'");
				ВывестиОшибку(ТекстОшибки);
	...
	Если СдаватьВПФР И ИзменитьСоставКонтролирующихОрганов Тогда
		Если ПустаяСтрока(СтрЗаменить(РегНомерПФР,"-","")) Тогда
			...
			ТекстОшибки 		= НСтр("ka='Заполните регистрационный номер в ПФР';ru='Заполните регистрационный номер в ПФР'");
			ВывестиОшибку(ТекстОшибки);
		...
		ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьРегистрационныйНомерПФР(РегНомерПФР, Истина) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.РегНомерПФР");
			ТекстОшибки 		= НСтр("ka='Регистрационный номер в ПФР должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)';ru='Регистрационный номер в ПФР должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)'");
			ВывестиОшибку(ТекстОшибки);
	...
	Если СдаватьВФСС И ИзменитьСоставКонтролирующихОрганов Тогда
		...
		Если ПустаяСтрока(РегНомерФСС) Тогда
			...
			ТекстОшибки 		= НСтр("ka='Заполните регистрационный номер в ФСС';ru='Заполните регистрационный номер в ФСС'");
			ВывестиОшибку(ТекстОшибки);
		...
		ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(РегНомерФСС, 10, Истина) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.РегНомерФСС");
			ТекстОшибки 		= НСтр("ka='Регистрационный номер в ФСС должен состоять из 10 цифр';ru='Регистрационный номер в ФСС должен состоять из 10 цифр'");
			ВывестиОшибку(ТекстОшибки);
		...
		Если ПризнакОбособленногоПодразделения и ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоЮрЛицо(Организация) и СдаватьВФСС И ЕстьРеквизитДопКодФССУОрганизации() Тогда
			Если ПустаяСтрока(ДополнительныйКодФСС) Тогда 
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ДополнительныйКодФСС");
				ТекстОшибки 		= НСтр("ka='Заполните дополнительный код ФСС у организации';ru='Заполните дополнительный код ФСС у организации'");
				ВывестиОшибку(ТекстОшибки);
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ДополнительныйКодФСС, 10, Истина) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ДополнительныйКодФСС");
				ТекстОшибки 		= НСтр("ka='Дополнительный код ФСС должен состоять из 10 цифр';ru='Дополнительный код ФСС должен состоять из 10 цифр'");
				ВывестиОшибку(ТекстОшибки);
	...
	Если ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если ПустаяСтрока(СтрЗаменить(АдрЮР,",","")) Тогда
			...
			ТекстОшибки 		= НСтр("ka='Заполните юридический адрес';ru='Заполните юридический адрес'");
			ВывестиОшибку(ТекстОшибки);
		...
		Иначе	
			...
			Если НЕ ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.АдресЮридический");
				ТекстОшибки 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Юридический адрес: %1';ru='Юридический адрес: %1'"),СтрокаОшибкаАдреса);
				ВывестиОшибку(ТекстОшибки);
	...
	Если ТелефонОсновнойИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если СтрДлина(СокрЛП(ТелефонОсновной)) = 0 Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ТелефонОсновной");
			ТекстОшибки 		= НСтр("ka='Заполните телефон организации';ru='Заполните телефон организации'");
			ВывестиОшибку(ТекстОшибки);
	...
	Если СдаватьВФСРАР И ИзменитьСоставКонтролирующихОрганов Тогда
		...
		Если НЕ ЗначениеЗаполнено(КодРегионаФСРАР) Тогда
			...
			ТекстОшибки 		= НСтр("ka='Заполните регион в адресе организации';ru='Заполните регион в адресе организации'");
			ВывестиОшибку(ТекстОшибки);
...
Процедура ПроверитьДанныеВладельцаЭП(МастерДалее)
	...
	Если ПустаяСтрока(ВладелецЭЦПФамилия) И (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) Тогда
		...
		ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦП");
		ТекстОшибки 		= НСтр("ka='Заполните фамилию сотрудника-владельца ЭП';ru='Заполните фамилию сотрудника-владельца ЭП'");
	...
	Если ПустаяСтрока(ВладелецЭЦПИмя) И (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) Тогда
		...
		ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦП");
		ТекстОшибки 		= НСтр("ka='Заполните имя сотрудника-владельца ЭП';ru='Заполните имя сотрудника-владельца ЭП'");
		ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если ВладелецЭЦПСНИЛСИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности 
		...
		Если ПустаяСтрока(СтрЗаменить(ВладелецЭЦПСНИЛС, "-","")) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПСНИЛС");
			ТекстОшибки 		= НСтр("ka='Заполните СНИЛС сотрудника-владельца ЭП';ru='Заполните СНИЛС сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		Иначе	
			Если НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПСНИЛС");
				ТекстОшибки 		= НСтр("ka='Некорректно указан СНИЛС сотрудника. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра';ru='Некорректно указан СНИЛС сотрудника. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра'");
				ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС, Ложь, Истина) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПСНИЛС");
				ТекстОшибки 		= НСтр("ka='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)';ru='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)'");
				ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) И НЕ ЗначениеЗаполнено(ВладелецЭЦПДатаРождения) Тогда
		...
		ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПДатаРождения");
		ТекстОшибки 		= НСтр("ka='Заполните дату рождения сотрудника-владельца ЭП';ru='Заполните дату рождения сотрудника-владельца ЭП'");
		ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) И НЕ ЗначениеЗаполнено(ВладелецЭЦППол) Тогда
		...
		ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦППол");
		ТекстОшибки 		= НСтр("ka='Заполните пол сотрудника-владельца ЭП';ru='Заполните пол сотрудника-владельца ЭП'");
		ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если ВладелецЭЦПДолжностьИзменилась И ИзменитьРеквизитыПодключенияК1СОтчетности 
		...
		Если ЭтоЮридическоеЛицо И ПустаяСтрока(ВладелецЭЦПДолжность) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПДолжность");
			ТекстОшибки 		= НСтр("ka='Заполните должность сотрудника-владельца ЭП';ru='Заполните должность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) И НЕ ЗначениеЗаполнено(ВладелецЭЦПГражданство) Тогда
		...
		ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПГражданство");
		ТекстОшибки 		= НСтр("ka='Заполните гражданство сотрудника-владельца ЭП';ru='Заполните гражданство сотрудника-владельца ЭП'");
		ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	ИначеЕсли (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
		...
		ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПГражданство");
		ТекстОшибки 		= НСтр("ka='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП';ru='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП'");
		ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат Тогда
		...
		Если ПустаяСтрока(ВладелецЭЦПВидДокумента) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПВидДокумента");
			ТекстОшибки 		= НСтр("ka='Заполните документ, удостоверяющий личность сотрудника-владельца ЭП';ru='Заполните документ, удостоверяющий личность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		Если ПустаяСтрока(ВладелецЭЦПСерияДокумента) И НЕ КонтекстЭДОКлиент.ПроверитьСериюДокумента(ВладелецЭЦПВидДокумента, ВладелецЭЦПСерияДокумента) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПСерияДокумента");
			ТекстОшибки 		= НСтр("ka='Заполните серию документа, удостоверяющего личность сотрудника-владельца ЭП';ru='Заполните серию документа, удостоверяющего личность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		Если ПустаяСтрока(ВладелецЭЦПНомерДокумента) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПНомерДокумента");
			ТекстОшибки 		= НСтр("ka='Заполните номер документа, удостоверяющего личность сотрудника-владельца ЭП';ru='Заполните номер документа, удостоверяющего личность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦПДатаВыдачиДокумента) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПДатаВыдачиДокумента");
			ТекстОшибки 		= НСтр("ka='Заполните дату выдачи документа, удостоверяющего личность сотрудника-владельца ЭП';ru='Заполните дату выдачи документа, удостоверяющего личность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		ИначеЕсли ВладелецЭЦПДатаВыдачиДокумента > ТекущаяДатаСервер ИЛИ Год(ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда	
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПДатаВыдачиДокумента");
			ТекстОшибки 		= НСтр("ka='Некорректно указана дата выдачи документа, удостоверяющего личность сотрудника-владельца ЭП';ru='Некорректно указана дата выдачи документа, удостоверяющего личность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		Если ПустаяСтрока(ВладелецЭЦПКемВыданДокумент) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПКемВыданДокумент");
			ТекстОшибки 		= НСтр("ka='Заполните поле ""Кем выдан"" документа, удостоверяющего личность сотрудника-владельца ЭП';ru='Заполните поле ""Кем выдан"" документа, удостоверяющего личность сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
		...
		Если ВладелецЭЦПВидДокумента = ПаспортРФ Тогда
			...
			Если ПустаяСтрока(КодПодразделенияБезТире) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПКодПодразделения");
				ТекстОшибки 		= НСтр("ka='Заполните код подразделения, выдавшего документ, удостоверяющего личность сотрудника-владельца ЭП';ru='Заполните код подразделения, выдавшего документ, удостоверяющего личность сотрудника-владельца ЭП'");
				ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
...
"Не соответствует маске ХХХ-ХХХ, где X – любая цифра'");
		...
		Если ПустаяСтрока(ВладелецЭЦПМестоРождения) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПМестоРождения");
			ТекстОшибки 		= НСтр("ka='Заполните место рождения сотрудника-владельца ЭП';ru='Заполните место рождения сотрудника-владельца ЭП'");
			ВывестиОшибку(ТекстОшибки, ИзмененныйРеквизит);
	...
	Если ТаблицаДанныхЗаявленияНаПодключение.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда
		...
		ТекстОшибки = НСтр("ka='Не изменено ни одной настройки подключения к 1С-Отчетности';ru='Не изменено ни одной настройки подключения к 1С-Отчетности'"); 
...
Функция ПолучитьКодыФНСПрописью(Знач Получатели)
	...
	ИначеЕсли КоличествоКодовФНС > 1 Тогда
		Результат = НСтр("ka='Коды ФНС (%1)';ru='Коды ФНС (%1)'");
...
Функция ПолучитьКодыРосстатПрописью(Знач Получатели)
	...
	ИначеЕсли КоличествоКодовФСГС > 1 Тогда
		Результат = НСтр("ka='Коды Росстата (%1)';ru='Коды Росстата (%1)'");
...
Процедура ОформитьПодсказкуДляГалкиПродлитьСертификат()
	...
	ИначеЕсли НЕ СертификатДоступен Тогда
		ТекстЗаголовка = НСтр("ka='Сертификат недоступен';ru='Сертификат недоступен'");
	ИначеЕсли ПродлитьСертификатИсходный Тогда
		...
		Если КоличествоОставшегосяВремени = Неопределено Тогда
			ТекстЗаголовка = НСтр("ka='Срок действия сертификата истек %1';ru='Срок действия сертификата истек %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(СертификатДействителенПо,"ДЛФ=DD"));
		Иначе
			ТекстЗаголовка = НСтр("ka='Срок действия сертификата истекает через %1';ru='Срок действия сертификата истекает через %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", КоличествоОставшегосяВремени);
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ka='Сертификат действует до %1';ru='Сертификат действует до %1'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(СертификатДействителенПо,"ДЛФ=DD"));
	КонецЕсли;
...
Процедура ОформитьПодсказкуДляГалкиПродлитьЛицензиюНа1СОтчетность()
	...
	ИначеЕсли ПродлитьЛицензиюНа1СОтчетностьИсходный Тогда
		...
		Если КоличествоОставшегосяВремени = Неопределено Тогда
			ТекстЗаголовка = НСтр("ka='Срок действия лицензии истек %1';ru='Срок действия лицензии истек %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(ЛицензияДатаОкончания,"ДЛФ=DD"));
		Иначе
			ТекстЗаголовка = НСтр("ka='Срок действия лицензии истекает через %1';ru='Срок действия лицензии истекает через %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", КоличествоОставшегосяВремени);
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ka='Лицензия действует до %1';ru='Лицензия действует до %1'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(ЛицензияДатаОкончания,"ДЛФ=DD"));
	КонецЕсли;
	Надпись.Заголовок = ТекстЗаголовка;
...
Процедура ОформитьЭлементыДляГалкиИзменитьСоставНалоговыхОргановШаг1(ДанныеОрганизации)
	...
	Иначе
		...
		Если КодПФРВОрганизации <> КодПФР И НЕ ПустаяСтрока(СтрЗаменить(КодПФРВОрганизации,"-","")) Тогда
			Элементы.ВзятьКодПФРИзОрганизации.Видимость = Истина;
			ТекстЗаголовка = НСтр("ka='Изменить на %1';ru='Изменить на %1'");
...
Процедура СделатьНедоступнымиНеподдерживаемыеНаправления()
	...
	Элементы.СдаватьВФСРАР.Подсказка = ?(ОператорПоддерживаетФСРАР, НСтр("ka='Федеральная служба по регулированию алкогольного рынка';ru='Федеральная служба по регулированию алкогольного рынка'"),
	...
	Элементы.СдаватьВРПН.Подсказка = ?(ОператорПоддерживаетРПН, НСтр("ka='Федеральная служба по надзору в сфере природопользования';ru='Федеральная служба по надзору в сфере природопользования'"),
	...
	Элементы.СдаватьВФТС.Подсказка = ?(ОператорПоддерживаетФТС, НСтр("ka='Федеральная таможенная служба';ru='Федеральная таможенная служба'"),
...
Процедура ОформитьПодсказкуДляГалкиИзменитьРеквизитыПодключенияК1СОтчетности()
	...
	Иначе
		ТекстЗаголовка = НСтр("ka='реквизиты не менялись';ru='реквизиты не менялись'");
	КонецЕсли;
	Надпись.Заголовок = ТекстЗаголовка;
...
Процедура УстановитьЗаголовокФормы()
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ДобавитьОрганизациюВЗаголовок(
		...
		КраткоеНаименование,
		// Разный заголовок!
		НСтр("ka='Изменение настроек подключения к 1С-Отчетности';ru='Изменение настроек подключения к 1С-Отчетности'"));
...
Процедура СформироватьИОтправитьЗаявлениеПослеВводаКодаПодтвреждения(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		Если ТипЗнч(ВходящийКонтекст) = Тип("Структура")
			...
			Элементы.ПодсказкаПоРезультатам.Заголовок = НСтр("ka='Ваше заявление успешно отправлено в автоматическую обработку';ru='Ваше заявление успешно отправлено в автоматическую обработку'");
		КонецЕсли;
		ОткрытьФормуНаСтраницеРезультатаОтправки();
		
		ПоказатьСледующуюСтраницу();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Изменение номера телефона не было подтверждено';ru='Изменение номера телефона не было подтверждено'"));
	КонецЕсли;
...
Процедура ОпределитьПричинуПереизданияСертификата()
	...
	Если ПереиздатьСертификат = 2 Тогда
		...
		НоваяСтрока.Префикс		= НСтр("ka='изменилось';ru='изменилось'");
		...
		НоваяСтрока.Группа		= "Организация";
		НоваяСтрока.Префикс		= НСтр("ka='изменился';ru='изменился'");
		...
		НоваяСтрока.Группа		= "Организация";
		НоваяСтрока.Префикс		= НСтр("ka='изменилась';ru='изменилась'");
		...
		Если СертификатДоступен Тогда
			ИзменившийсяРеквизит = НСтр("ka='инициировано продление сертификата';ru='инициировано продление сертификата'")
		Иначе
			ИзменившийсяРеквизит = НСтр("ka='сертификат недоступен';ru='сертификат недоступен'");
		КонецЕсли;
		...
		СформироватьТекстПричиныПереизданияСертификата(Отбор, НСтр("ka='ორგანიზაციები';ru='организации'"));
		...
		Если ИзменилсяТолькоВладелец Тогда
			Постфикс = "";
		Иначе
			Постфикс = НСтр("ka='сотрудника-владельца ЭП';ru='сотрудника-владельца ЭП'");
		КонецЕсли;
	...
	ИначеЕсли ПереиздатьСертификат = 1 Тогда
		...
		Если СертификатДоступен Тогда
			ПричиныПереизданияСертификата = НСтр("ka='Причины переиздания сертификата: принудительное переиздание сертификата';ru='Причины переиздания сертификата: принудительное переиздание сертификата'");
		Иначе
			ПричиныПереизданияСертификата = НСтр("ka='Причины переиздания сертификата: сертификат недоступен';ru='Причины переиздания сертификата: сертификат недоступен'");
		КонецЕсли;
...
Процедура ОбновитьТекстПодсказки()
	...
	Если ЗначениеЗаполнено(ТелефонМобильныйБезРазделителей) И Не ТелефоныРавны Тогда
		ТекстПодсказки = СтрШаблон(
			НСтр("ka='Вы указали отдельный номер для SMS-уведомлений - <a href = ""#ВводТелефонаДляОповещений"">%1</a>.';ru='Вы указали отдельный номер для SMS-уведомлений - <a href = ""#ВводТелефонаДляОповещений"">%1</a>.'"), 
			ТелефонМобильный);
	Иначе
