...
Функция НаправленияИмеютКоректныеЗначения()
	...
	Если ТипПолучателя = ФНС Тогда
		...
		Если НЕ(КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(КодПолучателя, 4)) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не корректно заполнен код получателя.';ru='Не корректно заполнен код получателя.';en='Not correctly filled with the code of the recipient.'"),
				,
				"КодПолучателя");
		...
		ИначеЕсли Элементы.КПП.Доступность И НЕ(КонтекстЭДОКлиент.ПроверитьКПП(КПП)) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не корректно заполнен КПП.';ru='Не корректно заполнен КПП.';en='Not correctly filled with PPC.'"),
				,
				"КПП");
	...
	ИначеЕсли ТипПолучателя = ПФР Тогда
		...
		Если СтрДлина(СокрЛП(КодПолучателя))<> 7 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не введен код получателя.';ru='Не введен код получателя.';en='Do not put the code of the recipient.'"),
				,
				"КодПолучателя");
...
Процедура УправлениеЭУЕслиЭтоТолькоФНС()
	...
	Если ТолькоФНС Тогда
		...
		Заголовок = НСтр("ka='ФНС';ru='ФНС';en='FTS'");
		Элементы.КПП.Заголовок 				= НСтр("ka='КПП';ru='КПП';en='PPC'");
		Элементы.ТипПолучателя.Заголовок 	= НСтр("ka='კოდიФНС';ru='Код ФНС';en='FNS code'");
