...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСведУведУточ) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML уведомления: не обнаружен узел ""УзелСведУведУточ"".';ru='Некорректная структура XML уведомления: не обнаружен узел ""УзелСведУведУточ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
