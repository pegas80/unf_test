...
Функция ПредставлениеПФ(ВключаяУслуги = Ложь) Экспорт
	...
	Если ВключаяУслуги Тогда
		...
		Возврат НСтр("ka='ТОРГ12 (Товарная накладная с услугами)';ru='ТОРГ12 (Товарная накладная с услугами)';en='TORG12 (consignment note to the services)'");
		
	Иначе
		
		Возврат НСтр("ka='ТОРГ-12 (Товарная накладная)';ru='ТОРГ-12 (Товарная накладная)';en='12 TORG (invoices)'");
...
Функция СформироватьПФ(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ОбъектыПечати, ВключаяРаботыУслуги) Экспорт
	...
	ОбластиМакета.Вставить("ОбластьМакетаЗаголовокТаблицы", ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "ЗаголовокТаб", 	НСтр("ka='Заголовок таблицы';ru='Заголовок таблицы';en='table header'"),Ошибки));
	ОбластиМакета.Вставить("ОбластьМакетаСтрока",			ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "Строка", 			"",								Ошибки));
	ОбластиМакета.Вставить("ОбластьМакетаИтогоПоСтранице",	ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "ИтогоПоСтранице",	НСтр("ka='Итоги по странице';ru='Итоги по странице';en='Results on page'"),Ошибки));
