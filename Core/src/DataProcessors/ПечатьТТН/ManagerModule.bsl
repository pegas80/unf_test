...
Функция ПредставлениеОснованияПечати(ОснованиеПечатиСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ОснованиеПечатиСсылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		Возврат Нстр("ka='Договор: ';ru='Договор: '") + Строка(ОснованиеПечатиСсылка.Наименование);
...
Функция МассаГрузаПрописью(Знач ИтогоМассаБрутто, Знач ДанныеОбъектаВес)
	...
	Прописью = СтрЗаменить(Прописью, НСтр("ru=' ноль килограммов';ka=' ноль килограммов'"), "");
...
Функция ОбъемГрузаПрописью(Знач ИтогоОбъем, Знач ДанныеОбъектаОбъем)
	...
	Возврат СтрШаблон(НСтр("ru='%1  м³';ka='%1  м³'"), Формат(ЗначениеДляПечати, "ЧГ=0"));
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИтогоМассаБрутто>0 ИЛИ Шапка.Вес>0 Тогда
			ОтпущеноНаСуммуПрописью = ОтпущеноНаСуммуПрописью+?(ПустаяСтрока(ОтпущеноНаСуммуПрописью), "", ", ")+
			НСтр("ru='масса брутто: ';ka='масса брутто: '")+НРег(МассаГрузаПрописью(ИтогоМассаБрутто, Шапка.Вес));
		КонецЕсли; 
		Если ИтогоОбъем>0 ИЛИ Шапка.Объем>0  Тогда
			ОтпущеноНаСуммуПрописью = ОтпущеноНаСуммуПрописью+?(ПустаяСтрока(ОтпущеноНаСуммуПрописью), "", ", ")+
			НСтр("ru='объем: ';ka='объем: '")+ОбъемГрузаПрописью(ИтогоОбъем, Шапка.Объем);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Шапка.Водитель) Тогда
			...
			Если ДокументыФизическихЛиц.Количество() > 0 Тогда
				...
				ВодительскоеУдостоверение = Нстр("ru='Серия ';ka='Серия '") + Строка(ДокументыФизическихЛиц[0].Серия) + Нстр("ru=' № ';ka=' № '") + Строка(ДокументыФизическихЛиц[0].Номер);
