...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ka='Извлечение текстов не поддерживается в Веб-клиенте.';ru='Извлечение текстов не поддерживается в Веб-клиенте.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='Извлечение текстов поддерживается только в клиенте под управлением ОС Windows.';ru='Извлечение текстов поддерживается только в клиенте под управлением ОС Windows.'");
...
Процедура ОбновлениеОбратногоОтсчета()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='До начала извлечения текстов осталось %1 сек';ru='До начала извлечения текстов осталось %1 сек'"),
		Осталось);
...
Процедура ИзвлечениеТекстовКлиент(РазмерПорции = Неопределено)
	...
	Состояние(НСтр("ka='Начато извлечение текста';ru='Начато извлечение текста'"));
	...
	Попытка
		...
		Если МассивФайлов.Количество() = 0 Тогда
			Состояние(НСтр("ka='Нет файлов для извлечения текста';ru='Нет файлов для извлечения текста'"));
			Возврат;
		КонецЕсли;
		...
		Для Индекс = 0 По МассивФайлов.Количество() - 1 Цикл
			...
			Попытка
				...
				Состояние(НСтр("ka='Идет извлечение текста файла';ru='Идет извлечение текста файла'"), Прогресс, ИмяСРасширением);
