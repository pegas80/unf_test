...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ka='Извлечение текстов не поддерживается в Веб-клиенте.';ru='Извлечение текстов не поддерживается в Веб-клиенте.';en='Text extraction is not supported in the Web client.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='Извлечение текстов поддерживается только в клиенте под управлением ОС Windows.';ru='Извлечение текстов поддерживается только в клиенте под управлением ОС Windows.';en='Extract text is only supported on the client running on Windows.'");
...
Процедура ИзвлечьВсе(Команда)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Завершено извлечение текста из"
"всех файлов с неизвлеченным текстом."
""
"Обработано файлов: %1.';ru='Завершено извлечение текста из"
"всех файлов с неизвлеченным текстом."
""
"Обработано файлов: %1.'"),
...
Процедура ОбновлениеОбратногоОтсчета()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='До начала извлечения текстов осталось %1 сек';ru='До начала извлечения текстов осталось %1 сек';en='Until the beginning of the text extraction have% 1 second'"),
		Осталось);
...
Процедура ИзвлечениеТекстовКлиент(РазмерПорции = Неопределено)
	...
	Состояние(НСтр("ka='Начато извлечение текста';ru='Начато извлечение текста';en='Started extracting text'"));
	...
	Попытка
		...
		Если МассивФайлов.Количество() = 0 Тогда
			Состояние(НСтр("ka='Нет файлов для извлечения текста';ru='Нет файлов для извлечения текста';en='No files to extract the text'"));
			Возврат;
		КонецЕсли;
		...
		Для Индекс = 0 По МассивФайлов.Количество() - 1 Цикл
			...
			Попытка
				...
				Состояние(НСтр("ka='Идет извлечение текста файла';ru='Идет извлечение текста файла';en='Being retrieved text file'"), Прогресс, ИмяСРасширением);
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Во время извлечения текста из файла ""%1"""
"произошла неизвестная ошибка.';ru='Во время извлечения текста из файла ""%1"""
"произошла неизвестная ошибка.'"),
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Извлечение текста завершено."
"Обработано файлов: %1';ru='Извлечение текста завершено."
"Обработано файлов: %1'"),
