...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Прервать объединение элементов и закрыть форму?';ru='Прервать объединение элементов и закрыть форму?';en='Abort union elements and close the form?'");
...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Удалить из списка для объединения элемент ""%1""?';ru='Удалить из списка для объединения элемент ""%1""?';en='Remove from list for the association element ""%1""?'"),
...
Функция ПроверитьОбъединяемыеСсылки(Знач НаборСсылок)
	...
	Если КоличествоСсылок < 2 Тогда
		Результат.Ошибка = НСтр("ka='Для объединения необходимо указать несколько элементов.';ru='Для объединения необходимо указать несколько элементов.';en='To join you must specify the number of elements.'");
		Возврат Результат;
	КонецЕсли;
...
"Группы не могут быть объединены.';ka='Один из объединяемых элементов является группой."
...
"Такие элементы не могут быть объединены.';ka='У объединяемых элементов различные владельцы."
"Такие элементы не могут быть объединены.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ka='Все объединяемые элементы должны быть одного типа.';ru='Все объединяемые элементы должны быть одного типа.';en='All of the combining elements must be of the same type.'");
	Иначе 
		// Все нормально
...
"использования на ""%2"" (отмечен стрелкой).';ka='Элементы (%1) будут <a href = ""ПереключениеРежимаУдаления"">помечены на удаление</a> и заменены во всех местах"
...
"использования на ""%2"" (отмечен стрелкой).';ka='Элементы (%1) будут <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a> и заменены во всех местах"
"использования на ""%2"" (отмечен стрелкой).'");
		КонецЕсли;
	Иначе
		ТекстПодсказки = НСтр("ru='Элементы (%1) будут помечены на удаление и заменены во всех местах"
"использования на ""%2"" (отмечен стрелкой).';ka='Элементы (%1) будут помечены на удаление и заменены во всех местах"
"использования на ""%2"" (отмечен стрелкой).'");
	КонецЕсли;
...
Функция СообщениеОЗавершении()
	Возврат СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		...
		НСтр("ka='элемент объединен в:';ru='элемент объединен в:';en='member is integrated in:'") + "," + НСтр("ka='элемента объединено в:';ru='элемента объединено в:';en='element integrated into:'") + "," + НСтр("ka='элементов объединено в:';ru='элементов объединено в:';en='elements integrated into:'"));
КонецФункции
...
"автоматическая замена на ""%1""';ka='Объединение элементов не выполнено. В некоторых местах использования не может быть произведена"
...
Процедура ДобавитьСтрокиМестИспользования(Знач МассивСсылок)
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Если СуществующиеСтроки.Количество() = 0 Тогда
			...
			Строка.НеИспользуется    = НСтр("ka='Не рассчитано';ru='Не рассчитано';en='not calculated'");
		Иначе
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от объединения элементов';ru='Отказаться от объединения элементов';en='Refuse combining elements'");
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Объединить >';ru='Объединить >';en='Merge>'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Начать объединение элементов';ru='Начать объединение элементов';en='Start union elements'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать';en='abort'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать объединение элементов';ru='Прервать объединение элементов';en='Abort union elements'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Закрыть результаты объединения';ru='Закрыть результаты объединения';en='Close association of'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ka='< В начало';ru='< В начало';en='<Back to top'");
	КнопкиМастера.Назад.Подсказка = НСтр("ka='Вернутся к выбору основного элемента';ru='Вернутся к выбору основного элемента';en='Return to the main cell selection'");
	КнопкиМастера.Далее.КнопкаПоУмолчанию = Истина;
	КнопкиМастера.Далее.Заголовок = НСтр("ka='გამეორება';ru='Повторить';en='repeat'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Повторить объединение';ru='Повторить объединение';en='repeat association'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Закрыть результаты объединения';ru='Закрыть результаты объединения';en='Close association of'");
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ka='Невозможно объединить элементы';ru='Невозможно объединить элементы';en='It is impossible to combine elements'"));
...
Процедура ДобавитьШагМастера(Знач Страница, Знач Действия, Знач Кнопки)
	...
	ОписаниеШага.КнопкаДалее.Вставить("Заголовок", НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'"));
	
	ОписаниеШага.Вставить("КнопкаНазад", Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаНазад.Вставить("Заголовок", НСтр("ka='< უკან';ru='< Назад';en='<Back'"));
	
	ОписаниеШага.Вставить("КнопкаОтмена",Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаОтмена.Вставить("Заголовок", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Определение мест использования';ru='Поиск и удаление дублей: Определение мест использования';en='Find and delete duplicate: Definition of use Places'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ЗаменитьСсылки" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Объединение элементов';ru='Поиск и удаление дублей: Объединение элементов';en='Find and delete duplicate: Combining elements'");
...
Процедура ЗаполнитьМестаИспользования(Знач АдресРезультата)
	...
	Для Каждого Строка Из ТаблицаИспользования Цикл
		...
		СтрокаИспользования.НеИспользуется = ?(Мест = 0, НСтр("ka='არ გამოიყენება';ru='Не используется';en='Not used'"), "");
	КонецЦикла;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Объединение элементов %1 в один';ru='Объединение элементов %1 в один';en='Combining elements of 1% in one'"), Представление);
КонецПроцедуры
...
Функция ЗаполнитьНеудачныеЗамены(Знач АдресРезультата)
	...
	Для Каждого СтрокаРезультата Из РезультатыЗамены Цикл
		...
		ИначеЕсли ТипОшибки = "ОшибкаУдаления" Тогда
			СтрокаОшибки.Причина = НСтр("ka='Невозможно удалить данные.';ru='Невозможно удалить данные.';en='You can not delete the data.'");
