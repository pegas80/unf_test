...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Прервать объединение элементов и закрыть форму?';ru='Прервать объединение элементов и закрыть форму?'");
...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Удалить из списка для объединения элемент ""%1""?';ru='Удалить из списка для объединения элемент ""%1""?'"),
...
Функция ПроверитьОбъединяемыеСсылки(Знач НаборСсылок)
	...
	Если КоличествоСсылок < 2 Тогда
		Результат.Ошибка = НСтр("ka='Для объединения необходимо указать несколько элементов.';ru='Для объединения необходимо указать несколько элементов.'");
		Возврат Результат;
	КонецЕсли;
...
"Такие элементы не могут быть объединены.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ka='Все объединяемые элементы должны быть одного типа.';ru='Все объединяемые элементы должны быть одного типа.'");
	Иначе 
		// Все нормально
...
Функция СообщениеОЗавершении()
	Возврат СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		...
		НСтр("ka='элемент объединен в:';ru='элемент объединен в:'") + "," + НСтр("ka='элемента объединено в:';ru='элемента объединено в:'") + "," + НСтр("ka='элементов объединено в:';ru='элементов объединено в:'"));
КонецФункции
...
Процедура ДобавитьСтрокиМестИспользования(Знач МассивСсылок)
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Если СуществующиеСтроки.Количество() = 0 Тогда
			...
			Строка.НеИспользуется    = НСтр("ka='Не рассчитано';ru='Не рассчитано'");
		Иначе
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от объединения элементов';ru='Отказаться от объединения элементов'");
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Объединить >';ru='Объединить >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Начать объединение элементов';ru='Начать объединение элементов'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать объединение элементов';ru='Прервать объединение элементов'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Закрыть результаты объединения';ru='Закрыть результаты объединения'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ka='< В начало';ru='< В начало'");
	КнопкиМастера.Назад.Подсказка = НСтр("ka='Вернутся к выбору основного элемента';ru='Вернутся к выбору основного элемента'");
	КнопкиМастера.Далее.КнопкаПоУмолчанию = Истина;
	КнопкиМастера.Далее.Заголовок = НСтр("ka='გამეორება';ru='Повторить'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Повторить объединение';ru='Повторить объединение'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Закрыть результаты объединения';ru='Закрыть результаты объединения'");
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ka='Невозможно объединить элементы';ru='Невозможно объединить элементы'"));
...
Процедура ДобавитьШагМастера(Знач Страница, Знач Действия, Знач Кнопки)
	...
	ОписаниеШага.КнопкаДалее.Вставить("Заголовок", НСтр("ka='შემდეგ >';ru='Далее >'"));
	
	ОписаниеШага.Вставить("КнопкаНазад", Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаНазад.Вставить("Заголовок", НСтр("ka='< უკან';ru='< Назад'"));
	
	ОписаниеШага.Вставить("КнопкаОтмена",Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаОтмена.Вставить("Заголовок", НСтр("ka='გაუქმება';ru='Отмена'"));
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Определение мест использования';ru='Поиск и удаление дублей: Определение мест использования'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ЗаменитьСсылки" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Объединение элементов';ru='Поиск и удаление дублей: Объединение элементов'");
...
Процедура ЗаполнитьМестаИспользования(Знач АдресРезультата)
	...
	Для Каждого Строка Из ТаблицаИспользования Цикл
		...
		СтрокаИспользования.НеИспользуется = ?(Мест = 0, НСтр("ka='არ გამოიყენება';ru='Не используется'"), "");
	КонецЦикла;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Объединение элементов %1 в один';ru='Объединение элементов %1 в один'"), Представление);
КонецПроцедуры
...
Функция ЗаполнитьНеудачныеЗамены(Знач АдресРезультата)
	...
	Для Каждого СтрокаРезультата Из РезультатыЗамены Цикл
		...
		ИначеЕсли ТипОшибки = "ОшибкаУдаления" Тогда
			СтрокаОшибки.Причина = НСтр("ka='Невозможно удалить данные.';ru='Невозможно удалить данные.'");
