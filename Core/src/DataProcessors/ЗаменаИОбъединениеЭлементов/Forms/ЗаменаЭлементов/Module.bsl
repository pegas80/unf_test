...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы["СписокСсылкаНовый"].Заголовок = НСтр("ka='დასახელება';ru='Наименование'");
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		...
		НСтр("ka='Не показывать заменяемые';ru='Не показывать заменяемые'"),
		Истина,
	...
	Если ЗаменяемыеСсылки.Количество() > 1 Тогда
		Элементы.НадписьТипВыбираемого.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Выберите один из элементов ""%1"", на который следует заменить выбранные значения (%2):';ru='Выберите один из элементов ""%1"", на который следует заменить выбранные значения (%2):'"),
			ОсновныеМетаданные.Представление(), ЗаменяемыеСсылки.Количество());
	Иначе
		Заголовок = НСтр("ka='Замена элемента';ru='Замена элемента'");
		Элементы.НадписьТипВыбираемого.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Выберите один из элементов ""%1"", на который следует заменить ""%2"":';ru='Выберите один из элементов ""%1"", на который следует заменить ""%2"":'"),
			ОсновныеМетаданные.Представление(), ЗаменяемыеСсылки[0].Ссылка);
	КонецЕсли;
	Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = НСтр("ka='Элемент для замены не выбран.';ru='Элемент для замены не выбран.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Прервать замену элементов и закрыть форму?';ru='Прервать замену элементов и закрыть форму?'");
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Прервать, НСтр("ka='შეწყვეტა';ru='Прервать'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет,      НСтр("ka='Не прерывать';ru='Не прерывать'"));
...
Функция СообщениеОЗавершении()
	...
	Если Количество = 1 Тогда
		ТекстРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Элемент ""%1"" заменен на ""%2""';ru='Элемент ""%1"" заменен на ""%2""'"),
			ЗаменяемыеСсылки[0].Ссылка,
			ЦелевойЭлементРезультат);
	Иначе
		ТекстРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Элементы (%1) заменены на ""%2""';ru='Элементы (%1) заменены на ""%2""'"),
			Количество,
			ЦелевойЭлементРезультат);
	КонецЕсли;
...
Процедура ИнициализироватьЗаменяемыеСсылки(Знач МассивСсылок)
	...
	Если КоличествоСсылок = 0 Тогда
		ТекстОшибкиПараметров = НСтр("ka='Не указано ни одного элемента для замены.';ru='Не указано ни одного элемента для замены.'");
		Возврат;
	КонецЕсли;
...
"Такие элементы не могут быть заменены.'");
	...
	ИначеЕсли Условия.КоличествоСсылок <> КоличествоСсылок Тогда
		ТекстОшибкиПараметров = НСтр("ka='Все заменяемые элементы должны быть одного типа.';ru='Все заменяемые элементы должны быть одного типа.'");
		Возврат;
	КонецЕсли;
	...
	Если Результат[2].Выгрузить().Количество() = 0 Тогда
		Если КоличествоСсылок > 1 Тогда
			ТекстОшибкиПараметров = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Выбранные элементы (%1) не на что заменить.';ru='Выбранные элементы (%1) не на что заменить.'"), КоличествоСсылок);
		Иначе
			ТекстОшибкиПараметров = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Выбранный элемент ""%1"" не на что заменить.';ru='Выбранный элемент ""%1"" не на что заменить.'"), ОбщегоНазначения.ПредметСтрокой(ЦелевойЭлемент));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Заменить >';ru='Заменить >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Начать замену элементов';ru='Начать замену элементов'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от замены элементов';ru='Отказаться от замены элементов'");
	...
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать замену элементов';ru='Прервать замену элементов'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ka='Повторить замену элементов';ru='Повторить замену элементов'");
	КнопкиМастера.Назад.Подсказка = НСтр("ka='Вернутся к выбору целевого элемента';ru='Вернутся к выбору целевого элемента'");
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Повторить замену >';ru='Повторить замену >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Повторить замену элементов';ru='Повторить замену элементов'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Закрыть результаты замены элементов';ru='Закрыть результаты замены элементов'");
...
Процедура ШагВыборЦелевогоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ЗаменяемыеСсылки.Количество() = 1 И ТекущиеДанные.Ссылка = ЗаменяемыеСсылки.Получить(0).Ссылка Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нельзя заменять элемент сам на себя.';ru='Нельзя заменять элемент сам на себя.'"));
		Возврат;
		
	ИначеЕсли ЗначениеРеквизита(ТекущиеДанные, "ЭтоГруппа", Ложь) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Нельзя заменять элемент на группу.';ru='Нельзя заменять элемент на группу.'"));
		Возврат;
	КонецЕсли;
...
"У выбранного элемента владелец ""%1"", а у заменяемого - ""%2"".'");
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Элемент %1 помечен на удаление. Продолжить?';ru='Элемент %1 помечен на удаление. Продолжить?'"), ТекущиеДанные.Ссылка);
...
Процедура ПроверкаДопустимостиЗаменыПрикладнойОбласти(Знач ПараметрыШага)
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ka='Невозможно заменить элементы';ru='Невозможно заменить элементы'"));
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(0, НСтр("ka='ОК';ru='ОК'"));
...
Функция КнопкиМастера()
	...
	Результат.Далее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >'");
	Результат.Назад.Заголовок = НСтр("ka='< უკან';ru='< Назад'");
	Результат.Отмена.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
	Возврат Результат;
	...
	Для Каждого ОписаниеШага Из НастройкиПошаговогоМастера.Шаги Цикл
		Для Каждого КлючЗначение Из ВозможныеДействия Цикл
			...
			Если Не ПустаяСтрока(ИмяОбработчика) Тогда
				...
				Исключение
					Текст = НСтр("ka='Ошибка создания обработчика события %1 для страницы %2, не определена процедура %3';ru='Ошибка создания обработчика события %1 для страницы %2, не определена процедура %3'");
...
Процедура ШагМастера(Знач КодКоманды)
	...
	ИначеЕсли КодКоманды = "Отмена" Тогда
		Направление = 0;
	Иначе
		ВызватьИсключение НСтр("ka='Некорректная команда шага помощника';ru='Некорректная команда шага помощника'");
	КонецЕсли;
...
Процедура ПерейтиНаШагМастера(Знач ИдентификаторШага, Знач ИнициироватьСобытия = Ложь)
	...
	Если СледующийШаг = Неопределено Тогда
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не найден шаг помощника %1';ru='Не найден шаг помощника %1'"), ИдентификаторШага);
...
Процедура ЗавершитьШагМастера(Знач ПараметрыШага)
	...
	ИначеЕсли ПараметрыШага = 1 И СледующийШаг > ПоследнийШаг Тогда
		...
		ВызватьИсключение НСтр("ka='Попытка выхода за последний шаг мастера';ru='Попытка выхода за последний шаг мастера'");
		
	ИначеЕсли ПараметрыШага = -1 И СледующийШаг < 0 Тогда
		// Попытка сделать шаг за пределы назад.
		ВызватьИсключение НСтр("ka='Попытка выхода назад из первого шага мастера';ru='Попытка выхода назад из первого шага мастера'");
...
Функция НомерШагаМастераПоИдентификатору(Знач ИдентификаторШага)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не найден шаг ""%1"".';ru='Не найден шаг ""%1"".'"), ИмяПоиска);
КонецФункции
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода, Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='Поиск и удаление дублей: Замена ссылок';ru='Поиск и удаление дублей: Замена ссылок'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Кратко = НСтр("ka='При замене элементов возникла ошибка:';ru='При замене элементов возникла ошибка:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
...
Функция ЗаполнитьНеудачныеЗамены(Знач АдресРезультата)
	...
	Для Каждого СтрокаРезультата Из РезультатыЗамены Цикл
		...
		Если ТипОшибки = "НеизвестныеДанные" Тогда
			СтрокаОшибки.Причина = НСтр("ka='Обнаружена данные, обработка которых не планировалась.';ru='Обнаружена данные, обработка которых не планировалась.'");
			
		ИначеЕсли ТипОшибки = "ОшибкаБлокировки" Тогда
			СтрокаОшибки.Причина = НСтр("ka='Не удалось заблокировать данные.';ru='Не удалось заблокировать данные.'");
			
		ИначеЕсли ТипОшибки = "ДанныеИзменены" Тогда
			СтрокаОшибки.Причина = НСтр("ka='Данные изменены другим пользователем.';ru='Данные изменены другим пользователем.'");
		...
		ИначеЕсли ТипОшибки = "ОшибкаУдаления" Тогда
			СтрокаОшибки.Причина = НСтр("ka='Неизвестная ошибка.';ru='Неизвестная ошибка.'");
