...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗаголовка = НСтр("ka='Период: с %1 по %2';ru='Период: с %1 по %2';en='Period:% 1 to% 2'");
	...
	ЗаголовокЗаявки = НСтр("ka='Заявка №%1 от %2';ru='Заявка №%1 от %2';en='Application №% 1% 2 from'");
	...
	Если Не ПрограммноеЗакрытие Тогда
		...
		Если Модифицированность Тогда
			...
			Если ЗавершениеРаботы Тогда
				ТекстПредупреждения =
					НСтр("ka='Заявка на изменение тарифа обмена электронными документами не отправлена.';ru='Заявка на изменение тарифа обмена электронными документами не отправлена.';en='Application for change of tariffs for electronic document exchange is not sent.'");
			Иначе
				ОписаниеОповещения = Новый ОписаниеОповещения(
					"ПриОтветеНаВопросОЗакрытииМодифицированнойФормы",
					ЭтотОбъект);
				ТекстВопроса = НСтр("ka='Данные изменены. Закрыть форму без сохранения данных?';ru='Данные изменены. Закрыть форму без сохранения данных?';en='These modified. Close the form without saving?'");
...
Процедура ОбработкаНавигационнойСсылкиСообщениеВТехПоддержку(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ka='Не получается изменить тариф работы с оператором обмена ЭД."
""
...
"%1'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='Интернет-поддержка. Изменение тарифа в 1С-Такском.';ru='Интернет-поддержка. Изменение тарифа в 1С-Такском.';en='Internet support. Changing the tariff 1C Taxcom.'"),
			ТекстСообщения,
			"taxcom",
			,
...
Процедура УстановитьНадписьНаГиперссылкеОбновленияСтатуса()
	...
	ТекстЗаголовка = НСтр("ka='Проверить выполнение заявки (осталось %1 сек.)';ru='Проверить выполнение заявки (осталось %1 сек.)';en='Check the performance of the application (% remaining 1 sec.)'");
