...
Процедура ЗаписатьСоответствие(Команда)
	...
	Если ЕстьПредупреждения() Тогда
		...
		ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Между связываемыми элементами есть расхождения. Продолжить?';ru='Между связываемыми элементами есть расхождения. Продолжить?';en='there is a discrepancy between binding members. Proceed?'"), РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.Отмена);
	Иначе
