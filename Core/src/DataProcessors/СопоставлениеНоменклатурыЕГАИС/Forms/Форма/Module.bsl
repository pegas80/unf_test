...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если ВводДоступен() Тогда
		...
		Если Результат = Неопределено Тогда 
			ТекстСообщения = НСтр("ka='მოწყობილობისგან გარე მოვლენის დამუშავებისას შეცდომა მოხდა:';ru='При обработке внешнего события от устройства произошла ошибка:';en='In processing the external event from a device error has occurred:'")
...
Процедура ОтменитьСоответствие(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Отменить связь выбранной алкогольной продукции?';ru='Отменить связь выбранной алкогольной продукции?';en='Unlink the selected alcoholic beverages?'"), РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.ОК);
...
Процедура СоздатьНоменклатуру(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Создать номенклатуру по выделенным строкам алкогольной продукции?';ru='Создать номенклатуру по выделенным строкам алкогольной продукции?';en='Create a range of selected lines of alcoholic beverages?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура Подключаемый_СоздатьНоменклатуру()
	...
	Если СозданнаяНоменклатура.Количество() = 1 Тогда
		ТекстСообщения = НСтр("ka='Создан элемент номенклатуры %Наименование%.';ru='Создан элемент номенклатуры %Наименование%.';en='Created element Nomenclature% Name%.'");
	...
	Иначе
		ТекстСообщения = НСтр("ka='Создано %Количество% элементов номенклатуры.';ru='Создано %Количество% элементов номенклатуры.';en='Created% Number% of the items nomenclature.'");
	...
	Для каждого АлкогольнаяПродукция Из ПараметрыСозданияНоменклатуры.МассивЭлементов Цикл
		...
		Если ЗначениеЗаполнено(РеквизитыАлкогольнойПродукции.ПроизводительЕГАИС) Тогда
			Если ЗначениеЗаполнено(РеквизитыАлкогольнойПродукции.Производитель) Тогда
				...
				КонецЕсли;
			Иначе
				ТекстОшибкиПроизводитель = НСтр("ka='При создании номенклатуры %Наименование% не удалось определить сопоставленного производителя.';ru='При создании номенклатуры %Наименование% не удалось определить сопоставленного производителя.';en='When you create a nomenclature% Name% could not determine the associated producer.'");
		...
		Если ЗначениеЗаполнено(РеквизитыАлкогольнойПродукции.ИмпортерЕГАИС) Тогда
			...
			Иначе
				ТекстОшибкиИмпортер = НСтр("ka='При создании номенклатуры %Наименование% не удалось определить сопоставленного импортера.';ru='При создании номенклатуры %Наименование% не удалось определить сопоставленного импортера.';en='When you create a nomenclature% Name% could not determine the associated importer.'");
...
Функция ПроверитьВозможностьУстановкиСоответствия(АлкогольнаяПродукция, Номенклатура, ТекстОшибки)
	...
	Если Номенклатура.ЭтоГруппа Тогда
		ТекстОшибки = НСтр("ka='Связка алкогольной продукции с группой номенклатуры невозможна.';ru='Связка алкогольной продукции с группой номенклатуры невозможна.';en='A bunch of alcohol with a group of nomenclature is not possible.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ka='Выбранная алкогольная продукция уже имеет связку.';ru='Выбранная алкогольная продукция уже имеет связку.';en='Selected alcoholic beverages already have a bunch.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПоискАлкогольнойПродукции(СтрокаПоиска, ОписаниеОшибки)
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		ОписаниеОшибки = НСтр("ka='ძალიან ბევრი შედეგია. დააზუსტეთ მოთხოვნა.';ru='Слишком много результатов. Уточните запрос.';en='Too many results. Refine your search.'");
		Возврат РезультатПоиска;
	КонецЕсли;
	
	КоличествоНайденныхЭлементов = СписокПоиска.ПолноеКоличество();
	Если КоличествоНайденныхЭлементов = 0 Тогда
		ОписаниеОшибки = НСтр("ka='არაფერი მოიძებნა';ru='Ничего не найдено';en='Nothing found'");
		Возврат РезультатПоиска;
	КонецЕсли;
	...
	Пока ЕстьСледующаяПорция Цикл
		...
		Для СчетчикЭлементов = 0 По КонечнаяПозиция Цикл
			...
			Иначе
				ВызватьИсключение НСтр("ka='უცნობი შეცდომა';ru='Неизвестная ошибка';en='Unknown error'");
			КонецЕсли;
...
Процедура ОбновлениеИндексаПолнотекстовогоПоискаДанных()
	...
	Состояние(НСтр("ka='Идет обновление индекса полнотекстового поиска ...';ru='Идет обновление индекса полнотекстового поиска ...';en='Updating a full-text search index ...'"));
	ОбновитьИндексПолнотекстовогоПоиска();
	Состояние(НСтр("ka='Обновление индекса полнотекстового поиска завершено...';ru='Обновление индекса полнотекстового поиска завершено...';en='Updating the index the full-text search is completed ...'")); 
...
Процедура ВыполнитьПолнотекстовыйПоискНаКлиенте(ОбластьПоиска, Список, СтрокаПоиска)
	...
	Если Не ИндексПолнотекстовогоПоискаАктуален 
		...
		ПоказатьВопрос(ОписаниеОповещения, 
			НСтр("ka='Индекс полнотекстового поиска неактуален. Обновить индекс?';ru='Индекс полнотекстового поиска неактуален. Обновить индекс?';en='full-text search index is irrelevant. Update the index?'"),
...
Процедура ПолученШтрихкод(ДанныеШтрихкода) Экспорт
	...
	Иначе
		СтрокаСообщения = НСтр("ka='შტრიხკოდის მიხედვით მონაცემები ვერ მოიძებნა: %1%';ru='Данные по штрихкоду не найдены: %1%';en='Data on the barcode is not found:% 1%'");
