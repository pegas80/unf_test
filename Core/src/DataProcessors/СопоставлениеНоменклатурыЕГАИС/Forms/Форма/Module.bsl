...
Процедура ОтменитьСоответствие(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Отменить связь выбранной алкогольной продукции?';ru='Отменить связь выбранной алкогольной продукции?'"), РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.ОК);
...
Процедура СоздатьНоменклатуру(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ka='Создать номенклатуру по выделенным строкам алкогольной продукции?';ru='Создать номенклатуру по выделенным строкам алкогольной продукции?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура Подключаемый_СоздатьНоменклатуру()
	...
	Если СозданнаяНоменклатура.Количество() = 1 Тогда
		ТекстСообщения = НСтр("ka='Создан элемент номенклатуры %Наименование%.';ru='Создан элемент номенклатуры %Наименование%.'");
	...
	Иначе
		ТекстСообщения = НСтр("ka='Создано %Количество% элементов номенклатуры.';ru='Создано %Количество% элементов номенклатуры.'");
	...
	Для каждого АлкогольнаяПродукция Из ПараметрыСозданияНоменклатуры.МассивЭлементов Цикл
		...
		Если ЗначениеЗаполнено(РеквизитыАлкогольнойПродукции.ПроизводительЕГАИС) Тогда
			Если ЗначениеЗаполнено(РеквизитыАлкогольнойПродукции.Производитель) Тогда
				...
				КонецЕсли;
			Иначе
				ТекстОшибкиПроизводитель = НСтр("ka='При создании номенклатуры %Наименование% не удалось определить сопоставленного производителя.';ru='При создании номенклатуры %Наименование% не удалось определить сопоставленного производителя.'");
		...
		Если ЗначениеЗаполнено(РеквизитыАлкогольнойПродукции.ИмпортерЕГАИС) Тогда
			...
			Иначе
				ТекстОшибкиИмпортер = НСтр("ka='При создании номенклатуры %Наименование% не удалось определить сопоставленного импортера.';ru='При создании номенклатуры %Наименование% не удалось определить сопоставленного импортера.'");
...
Функция ПроверитьВозможностьУстановкиСоответствия(АлкогольнаяПродукция, Номенклатура, ТекстОшибки)
	...
	Если Номенклатура.ЭтоГруппа Тогда
		ТекстОшибки = НСтр("ka='Связка алкогольной продукции с группой номенклатуры невозможна.';ru='Связка алкогольной продукции с группой номенклатуры невозможна.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ka='Выбранная алкогольная продукция уже имеет связку.';ru='Выбранная алкогольная продукция уже имеет связку.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПоискАлкогольнойПродукции(СтрокаПоиска, ОписаниеОшибки)
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		ОписаниеОшибки = НСтр("ka='ძალიან ბევრი შედეგია. დააზუსტეთ მოთხოვნა.';ru='Слишком много результатов. Уточните запрос.'");
		Возврат РезультатПоиска;
	КонецЕсли;
	
	КоличествоНайденныхЭлементов = СписокПоиска.ПолноеКоличество();
	Если КоличествоНайденныхЭлементов = 0 Тогда
		ОписаниеОшибки = НСтр("ka='არაფერი მოიძებნა';ru='Ничего не найдено'");
		Возврат РезультатПоиска;
	КонецЕсли;
	...
	Пока ЕстьСледующаяПорция Цикл
		...
		Для СчетчикЭлементов = 0 По КонечнаяПозиция Цикл
			...
			Иначе
				ВызватьИсключение НСтр("ka='უცნობი შეცდომა';ru='Неизвестная ошибка'");
			КонецЕсли;
...
Процедура ОбновлениеИндексаПолнотекстовогоПоискаДанных()
	...
	Состояние(НСтр("ka='Идет обновление индекса полнотекстового поиска ...';ru='Идет обновление индекса полнотекстового поиска ...'"));
	ОбновитьИндексПолнотекстовогоПоиска();
	Состояние(НСтр("ka='Обновление индекса полнотекстового поиска завершено...';ru='Обновление индекса полнотекстового поиска завершено...'")); 
...
Процедура ВыполнитьПолнотекстовыйПоискНаКлиенте(ОбластьПоиска, Список, СтрокаПоиска)
	...
	Если Не ИндексПолнотекстовогоПоискаАктуален 
		...
		ПоказатьВопрос(ОписаниеОповещения, 
			НСтр("ka='Индекс полнотекстового поиска неактуален. Обновить индекс?';ru='Индекс полнотекстового поиска неактуален. Обновить индекс?'"),
...
Процедура ПолученШтрихкод(ДанныеШтрихкода) Экспорт
	...
	Иначе
		СтрокаСообщения = НСтр("ka='შტრიხკოდის მიხედვით მონაცემები ვერ მოიძებნა: %1%';ru='Данные по штрихкоду не найдены: %1%'");
