...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		...
		ВызватьИсключение НСтр("ka='კონფიგურაციისთვის მონაცემთა სინქრონიზაციას მხარდაჭერა არ გააჩნია!';ru='Синхронизация данных для конфигурации не поддерживается!';en='Data synchronization configuration is not supported!'");
	...
	Заголовок = СтрШаблон(НСтр("ka='Настройка синхронизации данных с ""%1""';ru='Настройка синхронизации данных с ""%1""';en='Configuring the synchronization of data with ""1%""'"), НаименованиеКорреспондента);
	Элементы.ОписаниеСинхронизацииДанных.Заголовок = СтрШаблон(НСтр("ka='Описание синхронизации данных с ""%1""';ru='Описание синхронизации данных с ""%1""';en='Description synchronization data with ""1%""'"), НаименованиеКорреспондента);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Отменить настройку синхронизации данных?';ru='Отменить настройку синхронизации данных?';en='Cancel the setting data synchronization?'");
...
Процедура СообщитьОбОшибке(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось выполнить операцию.';ru='Не удалось выполнить операцию.';en='Unable to perform the operation.'"),,,, Отказ);
...
Функция Подключаемый_Начало_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныНастройкиОтборовНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки.';ru='Необходимо задать обязательные настройки.';en='required settings must be set.'"),,, "ОписаниеНастроекВыгрузкиДанных", Отказ);
...
Процедура ОжиданиеАнализаДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)';en='Configuring data synchronization between applications on the Internet (step 1)'"));
...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)';ru='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)';en='Set up sync data between applications on the Internet (automatic comparison of data)'"));
...
Процедура ПолучитьСтатистикуСопоставления(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)';ru='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)';en='Set up sync data between applications on the Internet (getting matching statistics)'"));
...
Процедура ОжиданиеСинхронизацииДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)';ru='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)';en='Configuring data synchronization between applications on the Internet (directories synchronization)'"));
...
Процедура ЗагрузитьПараметрыУчетаКорреспондента(Отказ, СообщениеОбОшибке = "", СообщениеОбОшибкеКорреспондента = "")
	...
	Попытка
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ka='Не заданы параметры учета в приложении ""%1"".';ru='Не заданы параметры учета в приложении ""%1"".';en='Do not set the account settings in the application ""%1"".'");
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ka='Не заданы параметры учета в этом приложении.';ru='Не заданы параметры учета в этом приложении.';en='Do not set the account settings in this application.'");
		КонецЕсли;
...
Функция Подключаемый_ПравилаПолученияДанных_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки.';ru='Необходимо задать обязательные настройки.';en='required settings must be set.'"),,, "ОписаниеЗначенийПоУмолчанию", Отказ);
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзлеБазыКорреспондента Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки для второго приложения.';ru='Необходимо задать обязательные настройки для второго приложения.';en='You must set the required settings for the second application.'")
...
Процедура ОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)';en='Configuring data synchronization between applications on the Internet (step 2)'"));
