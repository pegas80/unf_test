...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		...
		ВызватьИсключение НСтр("ka='კონფიგურაციისთვის მონაცემთა სინქრონიზაციას მხარდაჭერა არ გააჩნია!';ru='Синхронизация данных для конфигурации не поддерживается!'");
	...
	Заголовок = СтрШаблон(НСтр("ka='Настройка синхронизации данных с ""%1""';ru='Настройка синхронизации данных с ""%1""'"), НаименованиеКорреспондента);
	Элементы.ОписаниеСинхронизацииДанных.Заголовок = СтрШаблон(НСтр("ka='Описание синхронизации данных с ""%1""';ru='Описание синхронизации данных с ""%1""'"), НаименованиеКорреспондента);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Отменить настройку синхронизации данных?';ru='Отменить настройку синхронизации данных?'");
...
Процедура СообщитьОбОшибке(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось выполнить операцию.';ru='Не удалось выполнить операцию.'"),,,, Отказ);
...
Функция Подключаемый_Начало_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныНастройкиОтборовНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки.';ru='Необходимо задать обязательные настройки.'"),,, "ОписаниеНастроекВыгрузкиДанных", Отказ);
...
Процедура ОжиданиеАнализаДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)'"));
...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)';ru='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)'"));
...
Процедура ПолучитьСтатистикуСопоставления(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)';ru='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)'"));
...
Процедура ОжиданиеСинхронизацииДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)';ru='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)'"));
...
Процедура ЗагрузитьПараметрыУчетаКорреспондента(Отказ, СообщениеОбОшибке = "", СообщениеОбОшибкеКорреспондента = "")
	...
	Попытка
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ka='Не заданы параметры учета в приложении ""%1"".';ru='Не заданы параметры учета в приложении ""%1"".'");
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ka='Не заданы параметры учета в этом приложении.';ru='Не заданы параметры учета в этом приложении.'");
		КонецЕсли;
...
Функция Подключаемый_ПравилаПолученияДанных_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки.';ru='Необходимо задать обязательные настройки.'"),,, "ОписаниеЗначенийПоУмолчанию", Отказ);
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзлеБазыКорреспондента Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки для второго приложения.';ru='Необходимо задать обязательные настройки для второго приложения.'")
...
Процедура ОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ka='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)'"));
