...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при получении сообщения обмена из внешнего ресурса для корреспондента ""%1"".';ru='Возникли ошибки при получении сообщения обмена из внешнего ресурса для корреспондента ""%1"".';en='Errors occurred when receiving a message from an external resource sharing to the correspondent ""%1"".'"),
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при анализе сообщения обмена для корреспондента ""%1"".';ru='Возникли ошибки при анализе сообщения обмена для корреспондента ""%1"".';en='Any errors in the analysis of the exchange of messages to the correspondent of ""%1"".'"),
			Строка(Корреспондент));
	КонецЕсли;
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при выполнении автоматического сопоставления данных, полученных от корреспондента ""%1"".';ru='Возникли ошибки при выполнении автоматического сопоставления данных, полученных от корреспондента ""%1"".';en='Errors occurred during the automatic comparison of data received from the correspondent ""%1"".'"),
			Строка(Корреспондент));
	КонецЕсли;
...
Процедура ПолучитьСтатистикуСопоставления(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при получении информации статистики для корреспондента ""%1"".';ru='Возникли ошибки при получении информации статистики для корреспондента ""%1"".';en='An error occurred while obtaining statistical information to the correspondent of ""%1"".'"),
