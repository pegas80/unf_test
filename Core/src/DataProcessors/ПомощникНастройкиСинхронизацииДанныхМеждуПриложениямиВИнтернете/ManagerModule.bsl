...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при получении сообщения обмена из внешнего ресурса для корреспондента ""%1"".';ru='Возникли ошибки при получении сообщения обмена из внешнего ресурса для корреспондента ""%1"".'"),
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при анализе сообщения обмена для корреспондента ""%1"".';ru='Возникли ошибки при анализе сообщения обмена для корреспондента ""%1"".'"),
			Строка(Корреспондент));
	КонецЕсли;
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при выполнении автоматического сопоставления данных, полученных от корреспондента ""%1"".';ru='Возникли ошибки при выполнении автоматического сопоставления данных, полученных от корреспондента ""%1"".'"),
			Строка(Корреспондент));
	КонецЕсли;
...
Процедура ВыполнитьСинхронизациюСправочников(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='კორესპონდენტ %1-სგან ცნობარების ჩატვირთვის პროცესში შეცდომები წარმოიშვა.';ru='Возникли ошибки в процессе загрузки справочников от корреспондента %1.'"),
			Строка(Корреспондент));
	КонецЕсли;
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='კორესპონდენტ %1-სთვის ცნობარების გადმოტვირთვის პროცესში შეცდომები წარმოიშვა.';ru='Возникли ошибки в процессе выгрузки справочников для корреспондента %1.'"),
			Строка(Корреспондент));
	КонецЕсли;
...
Процедура ПолучитьСтатистикуСопоставления(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='Возникли ошибки при получении информации статистики для корреспондента ""%1"".';ru='Возникли ошибки при получении информации статистики для корреспондента ""%1"".'"),
