...
"в справочнике найдены не уникальные записи.';ka='Перед загрузкой идентификаторов объектов метаданных"
"в справочнике найдены не уникальные записи.'"));
			...
			Если Отказ Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Загрузка приоритетных данных не выполнена."
...
"%1'"),
			...
			Если ЗначениеЗаполнено(ДублиПредопределенных) Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='Предопределенные элементы.Найдены не уникальные записи.';ru='Предопределенные элементы.Найдены не уникальные записи.';en='Predefined elementy.Naydeny not unique records.'",
...
"%1';ka='При загрузке предопределенных элементов найдены не уникальные записи."
"%1'"),
						ДублиПредопределенных));
			КонецЕсли;
...
Процедура ДобавитьОписаниеДублейПредопределенного(ЗаписанныйОбъект, ДублиПредопределенных, Отказ, ОписаниеОтказа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибкиНеУникальностиЗаписей(),
				НСтр("ka='При загрузке предопределенных элементов найдены не уникальные записи.';ru='При загрузке предопределенных элементов найдены не уникальные записи.';en='When you load a pre-defined elements is not unique records found.'"));
		КонецЕсли;
	...
	Если ЗаписатьВЖурнал Тогда
		Если КоличествоДублей = 1 Тогда
			Шаблон = НСтр("ka='(загружаемая ссылка: %1, ссылка дубля: %2)';ru='(загружаемая ссылка: %1, ссылка дубля: %2)';en='(Downloadable link:% 1 double link: 2%)'");
		Иначе
			Шаблон = НСтр("ka='(загружаемая ссылка: %1, ссылки дублей: %2)';ru='(загружаемая ссылка: %1, ссылки дублей: %2)';en='(Downloadable link:% 1, reference duplicates: 2%)'");
		КонецЕсли;
...
"   - нажмите Выполнить."
"3. После этого запустите 1С:Предприятие и выполните повторную синхронизацию данных.';ru='Загрузка приоритетных данных не выполнена."
"%1"
"Требуется исправление информационной базы."
"1. Откройте конфигуратор, перейдите в меню Администрирование,"
"   выберите пункт ""Тестирование и исправление ...""."
...
Функция ОшибкаОткрытияФайлаСообщенияОбмена()
	...
	Возврат НСтр("ka='Ошибка открытия файла сообщения обмена';ru='Ошибка открытия файла сообщения обмена';en='Error opening file sharing posts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка при начале чтения файла сообщения обмена';ru='Ошибка при начале чтения файла сообщения обмена';en='Error beginning reading file sharing posts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка при начале записи файла сообщения обмена';ru='Ошибка при начале записи файла сообщения обмена';en='Error when recording file sharing posts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка чтения файла сообщения обмена';ru='Ошибка чтения файла сообщения обмена';en='Error reading file sharing posts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка записи данных в файл сообщения обмена';ru='Ошибка записи данных в файл сообщения обмена';en='Error writing data to the file sharing posts'");
	
КонецФункции

Функция ОшибкаВидаОбменаДанными()
	
	Возврат НСтр("ka='Обмен не по правилам конвертации не поддерживается';ru='Обмен не по правилам конвертации не поддерживается';en='Exchange is not according to the rules of conversion is not supported'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
