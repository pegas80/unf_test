...
"%1'"),
			...
			Если ЗначениеЗаполнено(ДублиПредопределенных) Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ka='Предопределенные элементы.Найдены не уникальные записи.';ru='Предопределенные элементы.Найдены не уникальные записи.'",
...
Процедура ДобавитьОписаниеДублейПредопределенного(ЗаписанныйОбъект, ДублиПредопределенных, Отказ, ОписаниеОтказа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибкиНеУникальностиЗаписей(),
				НСтр("ka='При загрузке предопределенных элементов найдены не уникальные записи.';ru='При загрузке предопределенных элементов найдены не уникальные записи.'"));
		КонецЕсли;
	...
	Если ЗаписатьВЖурнал Тогда
		Если КоличествоДублей = 1 Тогда
			Шаблон = НСтр("ka='(загружаемая ссылка: %1, ссылка дубля: %2)';ru='(загружаемая ссылка: %1, ссылка дубля: %2)'");
		Иначе
			Шаблон = НСтр("ka='(загружаемая ссылка: %1, ссылки дублей: %2)';ru='(загружаемая ссылка: %1, ссылки дублей: %2)'");
		КонецЕсли;
...
Функция ОшибкаОткрытияФайлаСообщенияОбмена()
	...
	Возврат НСтр("ka='Ошибка открытия файла сообщения обмена';ru='Ошибка открытия файла сообщения обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка при начале чтения файла сообщения обмена';ru='Ошибка при начале чтения файла сообщения обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка при начале записи файла сообщения обмена';ru='Ошибка при начале записи файла сообщения обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка чтения файла сообщения обмена';ru='Ошибка чтения файла сообщения обмена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ka='Ошибка записи данных в файл сообщения обмена';ru='Ошибка записи данных в файл сообщения обмена'");
	
КонецФункции

Функция ОшибкаВидаОбменаДанными()
	
	Возврат НСтр("ka='Обмен не по правилам конвертации не поддерживается';ru='Обмен не по правилам конвертации не поддерживается'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
