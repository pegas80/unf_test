...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если НЕ СвойстваРазделов.ДатыЗапретаЗагрузкиВнедрены Тогда
			ВызватьИсключение НСтр("ka='Даты запрета загрузки данных не применяются.';ru='Даты запрета загрузки данных не применяются.';en='Dates ban downloading data do not apply.'");
		КонецЕсли;
		
		Заголовок = НСтр("ka='მონაცემთა ჩატვირთვის აკრძალვის თარიღები';ru='Даты запрета загрузки данных';en='Dates ban downloading data'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("НетЗапрета").Представление =
			НСтр("ka='Нет запрета загрузки данных';ru='Нет запрета загрузки данных';en='No ban downloading data'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ДляВсехПользователей").Представление =
			НСтр("ka='ყველა საინფორმაციო ბაზისთვის';ru='Для всех информационных баз';en='For all information bases'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ПоПользователям").Представление =
			НСтр("ka='საინფორმაციო ბაზებით';ru='По информационным базам';en='According to information databases'");
		
		Элементы.ПользователиПолноеПредставление.Заголовок =
			НСтр("ka='პროგრამა: საინფორმაციო ბაზა';ru='Программа: информационная база';en='Program: information base'");
	...
	Иначе
		...
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.Пользователи"),        НСтр("ka='მომხმარებელი';ru='Пользователь';en='User'"));
		
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.ВнешниеПользователи"), НСтр("ka='გარე მომხმარებელი';ru='Внешний пользователь';en='external user'"));
	КонецЕсли;
		...
		Если ТекущийСпособУказания <> СпособУказанияЗначениеВСписке.Значение Тогда
			...
			ПоказатьВопрос(
				...
				ТекстСообщенияСпособУказанияНеИспользован(
					...
					ТекущийПользователь,
					ЭтотОбъект) + Символы.ПС + Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?';en='Proceed?'"),
				РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьКоманднуюПанельДатЗапрета(Контекст)
	...
	Иначе
		Если Контекст.СпособУказанияДатыЗапрета = "ПоРазделам" Тогда
			...
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Заголовок,   НСтр("ka='ნაწილები';ru='Разделы';en='Sections'"));
			УстановитьСвойство(Элементы.ДатыЗапретаПодобрать.Видимость, Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаДобавить.Видимость,  Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Видимость, Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаИзменить.Отображение, ОтображениеКнопки.Авто);
		...
		ИначеЕсли Контекст.СпособУказанияДатыЗапрета = "ПоОбъектам" Тогда
			...
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Видимость,   Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Заголовок,   НСтр("ka='საერთო თარიღი';ru='Общая дата';en='Overall date'"));
			УстановитьСвойство(Элементы.ДатыЗапретаПодобрать.Видимость, Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаДобавить.Видимость,  Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Видимость, Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаИзменить.Отображение, ОтображениеКнопки.Авто);
	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Если Контекст.НачалоДня > СрокРазрешения Тогда
				ШаблонНадписи = НСтр("ka='Срок возможности изменения данных с %2 по %3 истек %1';ru='Срок возможности изменения данных с %2 по %3 истек %1';en='Time capabilities data changes to% 2% 3% 1 expired'");
			Иначе
				Если РассчитатьДатуЗапрета Тогда
...
"После %1 будет запрещено изменение данных по %3';ka='По %1 возможно изменение данных с %2 по %3"
"После %1 будет запрещено изменение данных по %3'")
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок =
			СтрШаблон(НСтр("ka='აკრძალულია მონაცემების შეცვლა %1-ის';ru='Запрещено изменение данных по %1';en='It is forbidden to change the data on% 1'"),
...
Функция СписокОписанийДатыЗапрета()
	...
	Список.Добавить("",                "(" + НСтр("ka='ძირითადი მნიშვნელობა';ru='По умолчанию';en='Default'") + ")");
	Список.Добавить("ПроизвольнаяДата",      НСтр("ka='თავისუფალი თარიღი';ru='Произвольная дата';en='arbitrary date'"));
	Список.Добавить("КонецПрошлогоГода",     НСтр("ka='წინა წლის დასასრული';ru='Конец прошлого года';en='End of last year'"));
	Список.Добавить("КонецПрошлогоКвартала", НСтр("ka='წინა კვარტლის დასასრული';ru='Конец прошлого квартала';en='The end of last quarter'"));
	Список.Добавить("КонецПрошлогоМесяца",   НСтр("ka='წინა თვის დასასრული';ru='Конец прошлого месяца';en='End of last month'"));
	Список.Добавить("КонецПрошлойНедели",    НСтр("ka='წინა კვირის დასასრული';ru='Конец прошлой недели';en='The end of last week'"));
	Список.Добавить("ПредыдущийДень",        НСтр("ka='წინა დღე';ru='Предыдущий день';en='previous day'"));
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Для каждого ЗначениеСписка Из Контекст.СписокТиповПользователей Цикл
			Если ТипЗнч(ЗначениеСписка.Значение) = ТипЗнч(Пользователь) Тогда
				...
				Иначе
					Возврат ЗначениеСписка.Представление + ": " + НСтр("ka='<ყველა საინფორმაციო ბაზა>';ru='<Все информационные базы>';en='<All information databases>'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстПредставленияОбщейДаты()
	...
	Возврат "<" + НСтр("ka='საერთო თარიღი';ru='Общая дата';en='Overall date'") + ">";
...
Функция ТекстВопросаЗакрытьФорму()
	...
	Возврат НСтр("ka='დავხუროთ ფორმა?';ru='Закрыть форму?';en='Close form?'");
...
Функция ТекстВопросаОбновитьДанные()
	...
	Возврат НСтр("ka='Обновить данные?';ru='Обновить данные?';en='Update the data?'");
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ka='Удалить все даты запрета, кроме установленных %1?';ru='Удалить все даты запрета, кроме установленных %1?';en='Remove all the dates the ban but set% 1?'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстВопросаУдалитьВсеДатыЗапрета()
	...
	Возврат НСтр("ka='Удалить все даты запрета?';ru='Удалить все даты запрета?';en='Delete all of the ban date?'");
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ka='Значение %1 не изменяется.';ru='Значение %1 не изменяется.';en='The% 1 does not change.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ka='Комментарий %1 не изменяется.';ru='Комментарий %1 не изменяется.';en='% Note 1 is not changed.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияСначалаВыберитеПользователя()
	...
	Возврат НСтр("ka='Сначала выберите пользователя.';ru='Сначала выберите пользователя.';en='First, select the user.'");
...
"поэтому будет сохранена установка даты запрета ""%2"".';ka='Установка даты запрета ""%1"" не настроена,"
...
"способ указания даты запрета ""%3"".';ka='Способ указания даты запрета ""%1"""
"не настроен ""%2"", поэтому будет сохранен"
...
Функция ТекстВопросаОчиститьНезаполненныеСтроки()
	...
	Возврат НСтр("ka='Очистить незаполненные строки?';ru='Очистить незаполненные строки?';en='Clear blank lines?'");
...
Функция ТекстСообщенияДляУдаленияВыделитеОднуСтроку()
	...
	Возврат НСтр("ka='Для удаления выделите одну строку.';ru='Для удаления выделите одну строку.';en='To remove the highlight one line.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ka='Удалить даты запрета %1?';ru='Удалить даты запрета %1?';en='Remove the date of the ban% 1?'"),
...
Функция ТекстВопросаУдалитьДатыЗапретаДляПользователя()
	...
	Возврат НСтр("ka='Удалить даты запрета для пользователя?';ru='Удалить даты запрета для пользователя?';en='Remove the date of the ban for the user?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыПользователей()
	...
	Возврат НСтр("ka='Удалить даты запрета для группы пользователей?';ru='Удалить даты запрета для группы пользователей?';en='Remove the date of the ban for the user group?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВнешнегоПользователя()
	...
	Возврат НСтр("ka='Удалить даты запрета для внешнего пользователя?';ru='Удалить даты запрета для внешнего пользователя?';en='Remove the date of the ban for an external user?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыВнешнихПользователей()
	...
	Возврат НСтр("ka='Удалить даты запрета для группы внешних пользователей?';ru='Удалить даты запрета для группы внешних пользователей?';en='Remove the date of the ban for the group of external users?'");
...
Функция ТекстВопросаУдалитьДатыЗапрета()
	...
	Возврат НСтр("ka='Удалить даты запрета?';ru='Удалить даты запрета?';en='Remove the date of the ban?'");
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтрШаблон(НСтр("ka='%1 даты запрета не установлены.';ru='%1 даты запрета не установлены.';en='% 1 is the date of the ban have not been established.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияЗначениеУжеДобавленоВСписок()
	...
	Возврат НСтр("ka='Значение уже добавлено в список.';ru='Значение уже добавлено в список.';en='The value has already been added to the list.'");
...
Функция ТекстСообщенияОбновитьДанныеФормыF5()
	...
	Возврат НСтр("ka='Обновите данные формы (F5).';ru='Обновите данные формы (F5).';en='Update data form (F5).'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ka='Удалить даты запрета для разделов и объектов?';ru='Удалить даты запрета для разделов и объектов?';en='Remove the ban date for sections and objects?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляОбъектов()
	...
	Возврат НСтр("ka='Удалить даты запрета для объектов?';ru='Удалить даты запрета для объектов?';en='Remove the date of the ban for objects?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделов()
	...
	Возврат НСтр("ka='Удалить даты запрета для разделов?';ru='Удалить даты запрета для разделов?';en='Remove the date of the ban for the sections?'");
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются()
	...
	Возврат НСтр("ka='В выбранном разделе даты запрета для объектов не устанавливаются.';ru='В выбранном разделе даты запрета для объектов не устанавливаются.';en='In this section the date of the ban for the objects are not installed.'");
...
Функция ТекстСообщенияОбщаяДатаМожетБытьУстановлена()
	...
	Возврат НСтр("ka='<Общая дата> может быть установлена.';ru='<Общая дата> может быть установлена.';en='<Overall date> can be installed.'");
...
"для разделов.';ka='Разделы уже заполнены,"
"можно установить даты запрета"
"для разделов.'");
...
Функция ТекстСообщенияРазделыУжеЗаполненыМожноУстановитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ru='Разделы уже заполнены,"
"можно установить даты запрета"
"для разделов и объектов.';ka='Разделы уже заполнены,"
"можно установить даты запрета"
...
"можно установить дату запрета.';ka='Объект уже выбран,"
...
Функция ТекстСообщенияСначалаВыберитеОбъект()
	...
	Возврат НСтр("ka='Сначала выберите объект.';ru='Сначала выберите объект.';en='First select the object.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделаИОбъектов()
	...
	Возврат НСтр("ka='Удалить даты запрета для раздела и объектов?';ru='Удалить даты запрета для раздела и объектов?';en='Remove the date of the ban for the section and objects?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРаздела()
	...
	Возврат НСтр("ka='Удалить дату запрета для раздела?';ru='Удалить дату запрета для раздела?';en='Remove the date of the ban for the section?'");
...
Функция ТекстВопросаУдалитьОбщуюДатуЗапрета()
	...
	Возврат НСтр("ka='Удалить общую дату запрета?';ru='Удалить общую дату запрета?';en='Remove the general date of the ban?'");
...
Функция ТекстВопросаУдалитьДатуЗапретаОбъекта()
	...
	Возврат НСтр("ka='Удалить дату запрета для объекта?';ru='Удалить дату запрета для объекта?';en='Remove the ban date for the project?'");
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтрШаблон(НСтр("ka='%1 разделы всегда показываются.';ru='%1 разделы всегда показываются.';en='% 1 partitions are always displayed.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияПриУдаленииДатаЗапретаОчищается()
	...
	Возврат НСтр("ka='При удалении дата запрета очищается.';ru='При удалении дата запрета очищается.';en='When you delete data ban cleared.'");
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтрШаблон(НСтр("ka='%1 <Общая дата> всегда показывается.';ru='%1 <Общая дата> всегда показывается.';en='% 1 <Overall date> is always displayed.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстЗаголовкаРаздела()
	...
	Возврат НСтр("ka='ნაწილი';ru='Раздел';en='Section'");
...
Функция ТекстЗаголовкаРазделаСОбъектами()
	...
	Возврат НСтр("ka='ნაწილი, ობიექტი';ru='Раздел, объект';en='The section object'");
...
Функция ТекстСообщенияНедостаточноПравДляВыбораВнешнихПользователей()
	...
	Возврат НСтр("ka='გარე მომხმარებელბის ასარცევად არასაკმარისი უფლებებია';ru='Недостаточно прав для выбора внешних пользователей.';en='Insufficient privileges to select external users.'");
...
Функция ТекстЗаголовкаВыборТипаДанных()
	...
	Возврат НСтр("ka='მონაცემთა ტიპის არჩევა';ru='Выбор типа данных';en='Selecting the type of data'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиПользователямиНеСохранены(Контекст)
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Возврат НСтр("ka='Настройки с невыбранными информационными базами не сохранены.';ru='Настройки с невыбранными информационными базами не сохранены.';en='Settings with unselected information databases are not saved.'");
	КонецЕсли;
	
	Возврат НСтр("ka='Настройки с невыбранными пользователями не сохранены.';ru='Настройки с невыбранными пользователями не сохранены.';en='Setup with the non users are not saved.'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиОбъектамиНеСохранены()
	...
	Возврат НСтр("ka='Настройки с невыбранными объектами не сохранены.';ru='Настройки с невыбранными объектами не сохранены.';en='Settings with unselected objects are not saved.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляОбъектовНеСохранены()
	...
	Возврат НСтр("ka='Настройки с незаполненными датами запрета для объектов не сохранены.';ru='Настройки с незаполненными датами запрета для объектов не сохранены.';en='Settings with blank ban dates for the objects are not saved.'");
...
Функция ТекстСообщенияПользователиБезНастроекДатЗапретаНеСохранены()
	...
	Возврат НСтр("ka='Пользователи без настроек дат запрета не сохранены.';ru='Пользователи без настроек дат запрета не сохранены.';en='Users without setting the date of the ban is not saved.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляРазделовНеСохранены()
	...
	Возврат НСтр("ka='Настройки с незаполненными датами запрета для разделов не сохранены.';ru='Настройки с незаполненными датами запрета для разделов не сохранены.';en='Settings with blank ban dates are not stored for sections.'");
...
Функция ТекстСообщенияОбщаяДатаУжеПоказана()
	...
	Возврат НСтр("ka='<Общая дата> уже показана.';ru='<Общая дата> уже показана.';en='<Overall date> is already displayed.'");
...
Функция ТекстСообщенияВсеРазделыУжеПоказаны()
	...
	Возврат НСтр("ka='Все разделы уже показаны.';ru='Все разделы уже показаны.';en='All partitions are shown.'");
...
Функция ЗаголовокВыборТребуемыхРазделов()
	...
	Возврат НСтр("ka='Выбор требуемых разделов';ru='Выбор требуемых разделов';en='Select the desired partition'");
...
Функция ТекстСообщенияСнятоВыделениеСНезаполненныхСтрок()
	...
	Возврат НСтр("ka='შეუვსებელი სტრიქონებიდან გამოყოფა მოხსნილია.';ru='Снято выделение с незаполненных строк.';en='Unselect blank rows.'");
...
Функция ТекстСообщенияВыделенныеСтрокиНеЗаполнены()
	...
	Возврат НСтр("ka='გამოყოფილი სტრიქონები შეუვსებელია:';ru='Выделенные строки не заполнены.';en='Dedicated lines are not filled.'");
