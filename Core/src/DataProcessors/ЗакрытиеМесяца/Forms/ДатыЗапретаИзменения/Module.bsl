...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если НЕ СвойстваРазделов.ДатыЗапретаЗагрузкиВнедрены Тогда
			ВызватьИсключение НСтр("ka='Даты запрета загрузки данных не применяются.';ru='Даты запрета загрузки данных не применяются.'");
		КонецЕсли;
		
		Заголовок = НСтр("ka='მონაცემთა ჩატვირთვის აკრძალვის თარიღები';ru='Даты запрета загрузки данных'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("НетЗапрета").Представление =
			НСтр("ka='Нет запрета загрузки данных';ru='Нет запрета загрузки данных'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ДляВсехПользователей").Представление =
			НСтр("ka='ყველა საინფორმაციო ბაზისთვის';ru='Для всех информационных баз'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ПоПользователям").Представление =
			НСтр("ka='საინფორმაციო ბაზებით';ru='По информационным базам'");
		
		Элементы.ПользователиПолноеПредставление.Заголовок =
			НСтр("ka='პროგრამა: საინფორმაციო ბაზა';ru='Программа: информационная база'");
	...
	Иначе
		...
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.Пользователи"),        НСтр("ka='მომხმარებელი';ru='Пользователь'"));
		
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.ВнешниеПользователи"), НСтр("ka='გარე მომხმარებელი';ru='Внешний пользователь'"));
	КонецЕсли;
		...
		Если ТекущийСпособУказания <> СпособУказанияЗначениеВСписке.Значение Тогда
			...
			ПоказатьВопрос(
				...
				ТекстСообщенияСпособУказанияНеИспользован(
					...
					ТекущийПользователь,
					ЭтотОбъект) + Символы.ПС + Символы.ПС + НСтр("ka='გავაგრძელოთ?';ru='Продолжить?'"),
				РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьКоманднуюПанельДатЗапрета(Контекст)
	...
	Иначе
		Если Контекст.СпособУказанияДатыЗапрета = "ПоРазделам" Тогда
			...
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Заголовок,   НСтр("ka='ნაწილები';ru='Разделы'"));
			УстановитьСвойство(Элементы.ДатыЗапретаПодобрать.Видимость, Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаДобавить.Видимость,  Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Видимость, Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаИзменить.Отображение, ОтображениеКнопки.Авто);
		...
		ИначеЕсли Контекст.СпособУказанияДатыЗапрета = "ПоОбъектам" Тогда
			...
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Видимость,   Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Заголовок,   НСтр("ka='საერთო თარიღი';ru='Общая дата'"));
			УстановитьСвойство(Элементы.ДатыЗапретаПодобрать.Видимость, Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаДобавить.Видимость,  Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Видимость, Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаИзменить.Отображение, ОтображениеКнопки.Авто);
	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Если Контекст.НачалоДня > СрокРазрешения Тогда
				ШаблонНадписи = НСтр("ka='Срок возможности изменения данных с %2 по %3 истек %1';ru='Срок возможности изменения данных с %2 по %3 истек %1'");
			Иначе
				Если РассчитатьДатуЗапрета Тогда
...
"После %1 будет запрещено изменение данных по %3'")
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок =
			СтрШаблон(НСтр("ka='აკრძალულია მონაცემების შეცვლა %1-ის';ru='Запрещено изменение данных по %1'"),
...
Функция СписокОписанийДатыЗапрета()
	...
	Список.Добавить("",                "(" + НСтр("ka='ძირითადი მნიშვნელობა';ru='По умолчанию'") + ")");
	Список.Добавить("ПроизвольнаяДата",      НСтр("ka='თავისუფალი თარიღი';ru='Произвольная дата'"));
	Список.Добавить("КонецПрошлогоГода",     НСтр("ka='წინა წლის დასასრული';ru='Конец прошлого года'"));
	Список.Добавить("КонецПрошлогоКвартала", НСтр("ka='წინა კვარტლის დასასრული';ru='Конец прошлого квартала'"));
	Список.Добавить("КонецПрошлогоМесяца",   НСтр("ka='წინა თვის დასასრული';ru='Конец прошлого месяца'"));
	Список.Добавить("КонецПрошлойНедели",    НСтр("ka='წინა კვირის დასასრული';ru='Конец прошлой недели'"));
	Список.Добавить("ПредыдущийДень",        НСтр("ka='წინა დღე';ru='Предыдущий день'"));
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Для каждого ЗначениеСписка Из Контекст.СписокТиповПользователей Цикл
			Если ТипЗнч(ЗначениеСписка.Значение) = ТипЗнч(Пользователь) Тогда
				...
				Иначе
					Возврат ЗначениеСписка.Представление + ": " + НСтр("ka='<ყველა საინფორმაციო ბაზა>';ru='<Все информационные базы>'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстПредставленияОбщейДаты()
	...
	Возврат "<" + НСтр("ka='საერთო თარიღი';ru='Общая дата'") + ">";
...
Функция ТекстВопросаЗакрытьФорму()
	...
	Возврат НСтр("ka='დავხუროთ ფორმა?';ru='Закрыть форму?'");
...
Функция ТекстВопросаОбновитьДанные()
	...
	Возврат НСтр("ka='Обновить данные?';ru='Обновить данные?'");
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ka='Удалить все даты запрета, кроме установленных %1?';ru='Удалить все даты запрета, кроме установленных %1?'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстВопросаУдалитьВсеДатыЗапрета()
	...
	Возврат НСтр("ka='Удалить все даты запрета?';ru='Удалить все даты запрета?'");
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ka='Значение %1 не изменяется.';ru='Значение %1 не изменяется.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ka='Комментарий %1 не изменяется.';ru='Комментарий %1 не изменяется.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияСначалаВыберитеПользователя()
	...
	Возврат НСтр("ka='Сначала выберите пользователя.';ru='Сначала выберите пользователя.'");
...
Функция ТекстВопросаОчиститьНезаполненныеСтроки()
	...
	Возврат НСтр("ka='Очистить незаполненные строки?';ru='Очистить незаполненные строки?'");
...
Функция ТекстСообщенияДляУдаленияВыделитеОднуСтроку()
	...
	Возврат НСтр("ka='Для удаления выделите одну строку.';ru='Для удаления выделите одну строку.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ka='Удалить даты запрета %1?';ru='Удалить даты запрета %1?'"),
...
Функция ТекстВопросаУдалитьДатыЗапретаДляПользователя()
	...
	Возврат НСтр("ka='Удалить даты запрета для пользователя?';ru='Удалить даты запрета для пользователя?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыПользователей()
	...
	Возврат НСтр("ka='Удалить даты запрета для группы пользователей?';ru='Удалить даты запрета для группы пользователей?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВнешнегоПользователя()
	...
	Возврат НСтр("ka='Удалить даты запрета для внешнего пользователя?';ru='Удалить даты запрета для внешнего пользователя?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыВнешнихПользователей()
	...
	Возврат НСтр("ka='Удалить даты запрета для группы внешних пользователей?';ru='Удалить даты запрета для группы внешних пользователей?'");
...
Функция ТекстВопросаУдалитьДатыЗапрета()
	...
	Возврат НСтр("ka='Удалить даты запрета?';ru='Удалить даты запрета?'");
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтрШаблон(НСтр("ka='%1 даты запрета не установлены.';ru='%1 даты запрета не установлены.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияЗначениеУжеДобавленоВСписок()
	...
	Возврат НСтр("ka='Значение уже добавлено в список.';ru='Значение уже добавлено в список.'");
...
Функция ТекстСообщенияОбновитьДанныеФормыF5()
	...
	Возврат НСтр("ka='Обновите данные формы (F5).';ru='Обновите данные формы (F5).'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ka='Удалить даты запрета для разделов и объектов?';ru='Удалить даты запрета для разделов и объектов?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляОбъектов()
	...
	Возврат НСтр("ka='Удалить даты запрета для объектов?';ru='Удалить даты запрета для объектов?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделов()
	...
	Возврат НСтр("ka='Удалить даты запрета для разделов?';ru='Удалить даты запрета для разделов?'");
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются()
	...
	Возврат НСтр("ka='В выбранном разделе даты запрета для объектов не устанавливаются.';ru='В выбранном разделе даты запрета для объектов не устанавливаются.'");
...
Функция ТекстСообщенияОбщаяДатаМожетБытьУстановлена()
	...
	Возврат НСтр("ka='<Общая дата> может быть установлена.';ru='<Общая дата> может быть установлена.'");
...
Функция ТекстСообщенияСначалаВыберитеОбъект()
	...
	Возврат НСтр("ka='Сначала выберите объект.';ru='Сначала выберите объект.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделаИОбъектов()
	...
	Возврат НСтр("ka='Удалить даты запрета для раздела и объектов?';ru='Удалить даты запрета для раздела и объектов?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРаздела()
	...
	Возврат НСтр("ka='Удалить дату запрета для раздела?';ru='Удалить дату запрета для раздела?'");
...
Функция ТекстВопросаУдалитьОбщуюДатуЗапрета()
	...
	Возврат НСтр("ka='Удалить общую дату запрета?';ru='Удалить общую дату запрета?'");
...
Функция ТекстВопросаУдалитьДатуЗапретаОбъекта()
	...
	Возврат НСтр("ka='Удалить дату запрета для объекта?';ru='Удалить дату запрета для объекта?'");
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтрШаблон(НСтр("ka='%1 разделы всегда показываются.';ru='%1 разделы всегда показываются.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияПриУдаленииДатаЗапретаОчищается()
	...
	Возврат НСтр("ka='При удалении дата запрета очищается.';ru='При удалении дата запрета очищается.'");
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтрШаблон(НСтр("ka='%1 <Общая дата> всегда показывается.';ru='%1 <Общая дата> всегда показывается.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстЗаголовкаРаздела()
	...
	Возврат НСтр("ka='ნაწილი';ru='Раздел'");
...
Функция ТекстЗаголовкаРазделаСОбъектами()
	...
	Возврат НСтр("ka='ნაწილი, ობიექტი';ru='Раздел, объект'");
...
Функция ТекстСообщенияНедостаточноПравДляВыбораВнешнихПользователей()
	...
	Возврат НСтр("ka='გარე მომხმარებელბის ასარცევად არასაკმარისი უფლებებია';ru='Недостаточно прав для выбора внешних пользователей.'");
...
Функция ТекстЗаголовкаВыборТипаДанных()
	...
	Возврат НСтр("ka='მონაცემთა ტიპის არჩევა';ru='Выбор типа данных'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиПользователямиНеСохранены(Контекст)
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Возврат НСтр("ka='Настройки с невыбранными информационными базами не сохранены.';ru='Настройки с невыбранными информационными базами не сохранены.'");
	КонецЕсли;
	
	Возврат НСтр("ka='Настройки с невыбранными пользователями не сохранены.';ru='Настройки с невыбранными пользователями не сохранены.'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиОбъектамиНеСохранены()
	...
	Возврат НСтр("ka='Настройки с невыбранными объектами не сохранены.';ru='Настройки с невыбранными объектами не сохранены.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляОбъектовНеСохранены()
	...
	Возврат НСтр("ka='Настройки с незаполненными датами запрета для объектов не сохранены.';ru='Настройки с незаполненными датами запрета для объектов не сохранены.'");
...
Функция ТекстСообщенияПользователиБезНастроекДатЗапретаНеСохранены()
	...
	Возврат НСтр("ka='Пользователи без настроек дат запрета не сохранены.';ru='Пользователи без настроек дат запрета не сохранены.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляРазделовНеСохранены()
	...
	Возврат НСтр("ka='Настройки с незаполненными датами запрета для разделов не сохранены.';ru='Настройки с незаполненными датами запрета для разделов не сохранены.'");
...
Функция ТекстСообщенияОбщаяДатаУжеПоказана()
	...
	Возврат НСтр("ka='<Общая дата> уже показана.';ru='<Общая дата> уже показана.'");
...
Функция ТекстСообщенияВсеРазделыУжеПоказаны()
	...
	Возврат НСтр("ka='Все разделы уже показаны.';ru='Все разделы уже показаны.'");
...
Функция ЗаголовокВыборТребуемыхРазделов()
	...
	Возврат НСтр("ka='Выбор требуемых разделов';ru='Выбор требуемых разделов'");
...
Функция ТекстСообщенияСнятоВыделениеСНезаполненныхСтрок()
	...
	Возврат НСтр("ka='შეუვსებელი სტრიქონებიდან გამოყოფა მოხსნილია.';ru='Снято выделение с незаполненных строк.'");
...
Функция ТекстСообщенияВыделенныеСтрокиНеЗаполнены()
	...
	Возврат НСтр("ka='გამოყოფილი სტრიქონები შეუვსებელია:';ru='Выделенные строки не заполнены.'");
