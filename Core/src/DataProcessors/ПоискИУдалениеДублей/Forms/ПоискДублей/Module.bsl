...
"Задайте условия отбора и сравнения и нажмите ""Найти дубли"".';ka='Поиск дублей не выполнялся. "
"Задайте условия отбора и сравнения и нажмите ""Найти дубли"".'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='Поиск дублей...';ru='Поиск дублей...';en='Search duplicate ...'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='Удаление дублей...';ru='Удаление дублей...';en='Removing duplicates ...'");
	ОтображениеСостояния.Картинка = Элементы.ДлительнаяОперация48.Картинка;
	
	ОтображениеСостояния = Элементы.ДублейНеНайдено.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='Не обнаружено дублей по указанным параметрам."
"Измените условия отбора и сравнения, нажмите ""Найти дубли""';ru='Не обнаружено дублей по указанным параметрам."
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		ТекстВопроса = НСтр("ka='Прервать поиск дублей и закрыть форму?';ru='Прервать поиск дублей и закрыть форму?';en='Abort search for duplicates and close the form?'");
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		ТекстВопроса = НСтр("ka='Прервать удаление дублей и закрыть форму?';ru='Прервать удаление дублей и закрыть форму?';en='Interrupt the removal of duplicates and close the form?'");
	КонецЕсли;
...
Процедура ОбновитьМестаИспользованияКандидата(Знач ИдентификаторСтроки)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Для элемента ""%1"" найдено дублей: %2';ru='Для элемента ""%1"" найдено дублей: %2';en='The element ""%1"" found duplicates:% 2'"),
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Элемент ""%1"" не используется';ru='Элемент ""%1"" не используется';en='The ""%1"" is not used'"), 
	...
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Места использования ""%1"" (%2)';ru='Места использования ""%1"" (%2)';en='Recreational use ""%1"" (2%)'"), 
			ДанныеСтроки.Наименование,
			ДанныеСтроки.Количество);
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = НСтр("ka='Для просмотра причин выберите проблемный элемент-дубль.';ru='Для просмотра причин выберите проблемный элемент-дубль.';en='To view the causes of the problem, select the item-take.'");
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Замена дубля ""%1"" возможна, но была отменена из-за невозможности замены в других местах.';ru='Замена дубля ""%1"" возможна, но была отменена из-за невозможности замены в других местах.';en='Replacing double ""%1"" is possible, but it was canceled due to the impossibility of replacement elsewhere.'"), 
			ДанныеСтроки.Наименование);
		
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось заменить дубли в некоторых местах (%1)';ru='Не удалось заменить дубли в некоторых местах (%1)';en='Failed to replace duplicates in some places (1%)'"), 
			ДанныеСтроки.Количество);
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ВсеВариантыСравнения.Добавить("Равно",   НСтр("ka='Совпадает';ru='Совпадает';en='coincides'"));
	ВсеВариантыСравнения.Добавить("Подобно", НСтр("ka='Совпадает по похожим словам';ru='Совпадает по похожим словам';en='Coincides similar terms'"));
КонецПроцедуры
...
Процедура УстановитьЦветаИУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='-';ru='-'"));
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Форма.ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).';ru='Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).';en='Found duplicates: 2% (among elements: 1%). All checked items will be marked for deletion and replaced in all areas of use in the original (marked by arrow).'"),
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Найти дубли >';ru='Найти дубли >';en='Find duplicates>'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Найти дубли по указанным критериям';ru='Найти дубли по указанным критериям';en='Find duplicates on specified criteria'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от поиска и замены дублей';ru='Отказаться от поиска и замены дублей';en='Abandon the search and replace duplicates'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать поиск дублей';ru='Прервать поиск дублей';en='Interrupt the search takes'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Удалить дубли >';ru='Удалить дубли >';en='Remove duplicates>'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Удалить дубли';ru='Удалить дубли';en='Remove duplicates'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от поиска и замены дублей';ru='Отказаться от поиска и замены дублей';en='Abandon the search and replace duplicates'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать';en='abort'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать удаление дублей';ru='Прервать удаление дублей';en='Abort removing duplicates'");
	...
	// 4. Успешное удаление.
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ka='< Новый поиск';ru='< Новый поиск';en='<New search'");
	КнопкиМастера.Назад.Подсказка = НСтр("ka='Начать новый поиск с другими параметрами';ru='Начать новый поиск с другими параметрами';en='Start a new search with different parameters'");
	КнопкиМастера.Далее.Видимость = Ложь;
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Повторить удаление >';ru='Повторить удаление >';en='Repeat removal>'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Удалить дубли';ru='Удалить дубли';en='Remove duplicates'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
...
Процедура ШагПоискНеВыполнялсяПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ПредставлениеПравилОтбора) Тогда
		ПредставлениеПравилОтбора = НСтр("ka='Все элементы';ru='Все элементы';en='All the elements'");
	КонецЕсли;
	
	// Представление правил поиска.
	Союз = " " + НСтр("ka='и';ru='И';en='AND'") + " ";
	ТекстПравил = "";
	Для Каждого Правило Из ПравилаПоиска Цикл
		Если Правило.Правило = "Равно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ka='совпадает';ru='совпадает';en='coincides'");
		ИначеЕсли Правило.Правило = "Подобно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ka='совпадает по похожим словам';ru='совпадает по похожим словам';en='coincides similar terms'");
		Иначе
			Продолжить;
		КонецЕсли;
	...
	Если ПустаяСтрока(ТекстПравил) Тогда
		ТекстПравил = НСтр("ka='Правила не заданы';ru='Правила не заданы';en='Rules are not set'");
	КонецЕсли;
...
Процедура ШагПоискНеВыполнялсяПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстПредупреждения = НСтр("ka='Необходимо выбрать область поиска дублей';ru='Необходимо выбрать область поиска дублей';en='You must select a search area doubles'");
...
Процедура ЗапуститьРаботуМастера()
	...
	Если НастройкиПошаговогоМастера.СтраницаСтарта = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Перед запуском мастера должна быть установлена начальная страница.';ru='Перед запуском мастера должна быть установлена начальная страница.';en='Before running the wizard must be installed starting page.'");
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Поиск дублей';ru='Поиск и удаление дублей: Поиск дублей';en='Find and delete duplicate: Finding duplicates'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Удаление дублей';ru='Поиск и удаление дублей: Удаление дублей';en='Find and remove duplicates: Delete duplicates'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
			Кратко = НСтр("ka='При поиске дублей возникла ошибка:';ru='При поиске дублей возникла ошибка:';en='When searching for duplicate error occurred:'");
		ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
			Кратко = НСтр("ka='При удалении дублей возникла ошибка:';ru='При удалении дублей возникла ошибка:';en='When you delete a duplicate error occurred:'");
		КонецЕсли;
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач АдресРезультата)
	...
	МестаИспользованияКандидата.Очистить();
	Элементы.ОписаниеТекущейГруппыДублей.Заголовок = НСтр("ka='Дублей не найдено';ru='Дублей не найдено';en='Duplicates found'");
	...
	Если ВсегоНайденоДублей = ЭлементыДерева.Количество() Тогда
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ",
			НСтр("ka='Не обнаружено дублей по указанным условиям.';ru='Не обнаружено дублей по указанным условиям.';en='Not found duplicates for these conditions.'"));
	Иначе
		ОбновитьОписаниеСостоянияНайденныхДублей(ЭтотОбъект);
	КонецЕсли;
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Если ПоследнийКандидат = Неопределено Тогда
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Все найденные дубли (%1) успешно объединены';ru='Все найденные дубли (%1) успешно объединены';en='All the found duplicates (1%) successfully merged'"),
						ВсегоОбработано));
			Иначе
				ПоследнийКандидатСтрокой = ОбщегоНазначения.ПредметСтрокой(ПоследнийКандидат);
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Все найденные дубли (%1) успешно объединены"
"в ""%2""';ru='Все найденные дубли (%1) успешно объединены"
"в ""%2""'"),
		...
		Иначе
			...
			ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Все найденные дубли (%1) успешно объединены."
"Оставлено элементов (%2).';ru='Все найденные дубли (%1) успешно объединены."
