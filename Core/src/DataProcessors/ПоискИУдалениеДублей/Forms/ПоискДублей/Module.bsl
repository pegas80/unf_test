...
"Задайте условия отбора и сравнения и нажмите ""Найти дубли"".'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='Поиск дублей...';ru='Поиск дублей...'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='Удаление дублей...';ru='Удаление дублей...'");
	ОтображениеСостояния.Картинка = Элементы.ДлительнаяОперация48.Картинка;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		ТекстВопроса = НСтр("ka='Прервать поиск дублей и закрыть форму?';ru='Прервать поиск дублей и закрыть форму?'");
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		ТекстВопроса = НСтр("ka='Прервать удаление дублей и закрыть форму?';ru='Прервать удаление дублей и закрыть форму?'");
	КонецЕсли;
...
Процедура ОбновитьМестаИспользованияКандидата(Знач ИдентификаторСтроки)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Для элемента ""%1"" найдено дублей: %2';ru='Для элемента ""%1"" найдено дублей: %2'"),
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Элемент ""%1"" не используется';ru='Элемент ""%1"" не используется'"), 
	...
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Места использования ""%1"" (%2)';ru='Места использования ""%1"" (%2)'"), 
			ДанныеСтроки.Наименование,
			ДанныеСтроки.Количество);
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = НСтр("ka='Для просмотра причин выберите проблемный элемент-дубль.';ru='Для просмотра причин выберите проблемный элемент-дубль.'");
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Замена дубля ""%1"" возможна, но была отменена из-за невозможности замены в других местах.';ru='Замена дубля ""%1"" возможна, но была отменена из-за невозможности замены в других местах.'"), 
			ДанныеСтроки.Наименование);
		
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Не удалось заменить дубли в некоторых местах (%1)';ru='Не удалось заменить дубли в некоторых местах (%1)'"), 
			ДанныеСтроки.Количество);
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ВсеВариантыСравнения.Добавить("Равно",   НСтр("ka='Совпадает';ru='Совпадает'"));
	ВсеВариантыСравнения.Добавить("Подобно", НСтр("ka='Совпадает по похожим словам';ru='Совпадает по похожим словам'"));
КонецПроцедуры
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Форма.ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).';ru='Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).'"),
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Найти дубли >';ru='Найти дубли >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Найти дубли по указанным критериям';ru='Найти дубли по указанным критериям'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от поиска и замены дублей';ru='Отказаться от поиска и замены дублей'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать поиск дублей';ru='Прервать поиск дублей'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Удалить дубли >';ru='Удалить дубли >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Удалить дубли';ru='Удалить дубли'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Отказаться от поиска и замены дублей';ru='Отказаться от поиска и замены дублей'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='შეწყვეტა';ru='Прервать'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ka='Прервать удаление дублей';ru='Прервать удаление дублей'");
	...
	// 4. Успешное удаление.
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ka='< Новый поиск';ru='< Новый поиск'");
	КнопкиМастера.Назад.Подсказка = НСтр("ka='Начать новый поиск с другими параметрами';ru='Начать новый поиск с другими параметрами'");
	КнопкиМастера.Далее.Видимость = Ложь;
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ka='Повторить удаление >';ru='Повторить удаление >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ka='Удалить дубли';ru='Удалить дубли'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
...
Процедура ШагПоискНеВыполнялсяПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ПредставлениеПравилОтбора) Тогда
		ПредставлениеПравилОтбора = НСтр("ka='Все элементы';ru='Все элементы'");
	КонецЕсли;
	...
	Для Каждого Правило Из ПравилаПоиска Цикл
		Если Правило.Правило = "Равно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ka='совпадает';ru='совпадает'");
		ИначеЕсли Правило.Правило = "Подобно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ka='совпадает по похожим словам';ru='совпадает по похожим словам'");
		Иначе
			Продолжить;
		КонецЕсли;
	...
	Если ПустаяСтрока(ТекстПравил) Тогда
		ТекстПравил = НСтр("ka='Правила не заданы';ru='Правила не заданы'");
	КонецЕсли;
...
Процедура ШагПоискНеВыполнялсяПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстПредупреждения = НСтр("ka='Необходимо выбрать область поиска дублей';ru='Необходимо выбрать область поиска дублей'");
...
Процедура ЗапуститьРаботуМастера()
	...
	Если НастройкиПошаговогоМастера.СтраницаСтарта = Неопределено Тогда
		ВызватьИсключение НСтр("ka='Перед запуском мастера должна быть установлена начальная страница.';ru='Перед запуском мастера должна быть установлена начальная страница.'");
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Поиск дублей';ru='Поиск и удаление дублей: Поиск дублей'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		...
		НаименованиеМетода = НСтр("ka='Поиск и удаление дублей: Удаление дублей';ru='Поиск и удаление дублей: Удаление дублей'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
			Кратко = НСтр("ka='При поиске дублей возникла ошибка:';ru='При поиске дублей возникла ошибка:'");
		ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
			Кратко = НСтр("ka='При удалении дублей возникла ошибка:';ru='При удалении дублей возникла ошибка:'");
		КонецЕсли;
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач АдресРезультата)
	...
	МестаИспользованияКандидата.Очистить();
	Элементы.ОписаниеТекущейГруппыДублей.Заголовок = НСтр("ka='Дублей не найдено';ru='Дублей не найдено'");
	...
	Если ВсегоНайденоДублей = ЭлементыДерева.Количество() Тогда
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ",
			НСтр("ka='Не обнаружено дублей по указанным условиям.';ru='Не обнаружено дублей по указанным условиям.'"));
	Иначе
		ОбновитьОписаниеСостоянияНайденныхДублей(ЭтотОбъект);
	КонецЕсли;
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Если ПоследнийКандидат = Неопределено Тогда
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Все найденные дубли (%1) успешно объединены';ru='Все найденные дубли (%1) успешно объединены'"),
						ВсегоОбработано));
			Иначе
