...
Функция МенеджерОбластиПоискаДублей(Знач ИмяОбластиПоискаДанных) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მეტამონაცემთა ობიექტის უცნობი ტიპი ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';en='Unknown type of metadata object ""%1""'"), ИмяОбластиПоискаДанных);
КонецФункции
	...
	Для Каждого ИмяПоля Из ИменаПолейВВыборе Цикл
		...
		Если ДоступноеПолеКД = Неопределено Тогда
			ЗаписьЖурналаРегистрации(
				...
				ОбъектМетаданных,
				ЭталонныйОбъект,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Поле ""%1"" не найдено.';ru='Поле ""%1"" не найдено.';en='The ""%1"" not found.'"), Строка(ПолеКД)));
			Продолжить;
		КонецЕсли;
	...
	Пока СледующийЭлементВыборки(Выборка1) Цикл
		...
		Если РазмерВозвращаемойПорции > 0 И (ТаблицаДублей.Количество() > РазмерВозвращаемойПорции) Тогда
			...
			Если ТаблицаДублей.Количество() = 0 Тогда
				Результат.ОписаниеОшибки = НСтр("ka='Найдено слишком много элементов, определены не все группы дублей.';ru='Найдено слишком много элементов, определены не все группы дублей.';en='There are too many elements that are not defined all the group takes.'");
			Иначе
				Результат.ОписаниеОшибки = НСтр("ka='Найдено слишком много элементов. Уточните критерии поиска дублей.';ru='Найдено слишком много элементов. Уточните критерии поиска дублей.';en='Too many elements. Refine the search criteria for duplicates.'");
			КонецЕсли;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыполнитьКоманду(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='Поиск и удаление дублей: Команда ""%1"" не поддерживается.';ru='Поиск и удаление дублей: Команда ""%1"" не поддерживается.';en='Find and delete duplicate: Command ""%1"" is not supported.'");
