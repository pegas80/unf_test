...
Процедура СохранитьПослеОтображенияДиалогаВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ka='Не удалось записать файл на диск. Возможно, диск защищен от записи или недостаточно места на диске.';ru='Не удалось записать файл на диск. Возможно, диск защищен от записи или недостаточно места на диске.'");
		Сообщение.Сообщить();
	КонецПопытки;
...
Процедура Показать(Расшифровка) Экспорт
	...
	Если Показатели.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Расшифровка для данного соотношения отсутствует.';ru='Расшифровка для данного соотношения отсутствует.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура АктивизироватьЯчейкуОтчета(Расшифровка)
	...
	Если ЭтаФорма.ВладелецФормы = Неопределено ИЛИ ТекДок <> ЭтаФорма.ВладелецФормы.КлючУникальности Тогда
		...
		Исключение
			...
			ПоказатьПредупреждение(,НСтр("ka='Внимание! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';ru='Внимание! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.'"));
