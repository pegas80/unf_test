...
Процедура ДекорацияЛогинИППОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ka='Личный кабинет пользователя';ru='Личный кабинет пользователя';en='Personal user account'"),
...
Процедура ВойтиИлиВыйтиИПП(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Логин и пароль для подключения к сервисам Интернет-поддержки пользователей будут удалены из программы."
"Отключить Интернет-поддержку?';ru='Логин и пароль для подключения к сервисам Интернет-поддержки пользователей будут удалены из программы."
"Отключить Интернет-поддержку?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			КодВозвратаДиалога.Нет,
			НСтр("ka='Выход из Интернет-поддержки пользователей';ru='Выход из Интернет-поддержки пользователей';en='Exit the Internet-user support'"));
...
Процедура СообщениеВТехПоддержку(Команда)
	...
	ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
		НСтр("ka='<Заполните текст сообщения>';ru='<Заполните текст сообщения>';en='<Complete text of the message>'"),
		НСтр("ka='<Заполните текст сообщения>';ru='<Заполните текст сообщения>';en='<Complete text of the message>'"));
...
Процедура ОтобразитьСостояниеПодключенияИПП()
	...
	Если ДанныеАутентификации = Неопределено Тогда
		Элементы.ДекорацияЛогинИПП.Заголовок = ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			НСтр("ka='Подключение к Интернет-поддержке не выполнено.';ru='Подключение к Интернет-поддержке не выполнено.';en='Connecting to the Internet support is not fulfilled.'"));
		Элементы.ВойтиИлиВыйтиИПП.Заголовок = НСтр("ka='მივუერთოთ';ru='Подключить';en='To plug'");
	...
	Иначе
		ШаблонЗаголовка = ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
			НСтр("ka='<body>Подключена Интернет-поддержка для пользователя <a href=""action:openUsersSite"">%1</body>';ru='<body>Подключена Интернет-поддержка для пользователя <a href=""action:openUsersSite"">%1</body>';en='<Body> Connected Internet support user <a href=""action:openUsersSite"">% 1 </ body>'"));
		Элементы.ДекорацияЛогинИПП.Заголовок = ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонЗаголовка,
				ДанныеАутентификации.Логин));
		Элементы.ВойтиИлиВыйтиИПП.Заголовок = НСтр("ka='გამორთვა';ru='Отключить';en='shut off'");
...
Процедура ПриИзмененииРасписания(Расписание, ДопПараметры) Экспорт
	...
	Если АвтоматическаяПроверкаОбновлений = 2
		...
		ПоказатьПредупреждение(,
			НСтр("ka='Интервал проверки не может быть задан чаще, чем один раз 5 минут.';ru='Интервал проверки не может быть задан чаще, чем один раз 5 минут.';en='Check interval can not be set more frequently than once every 5 minutes.'"));
		Возврат;
	КонецЕсли;
...
Функция ПредставлениеРасписания(Расписание)
	...
	Если Расписание = Неопределено Тогда
		Возврат НСтр("ka='განრიგის გამართვა';ru='Настроить расписание';en='configure schedule'");
	Иначе
