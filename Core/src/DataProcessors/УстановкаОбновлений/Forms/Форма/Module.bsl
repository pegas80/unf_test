...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления() Тогда
		ВызватьИсключение НСтр("ka='Недостаточно прав для обновления конфигурации. Обратитесь к администратору.';ru='Недостаточно прав для обновления конфигурации. Обратитесь к администратору.';en='Insufficient privileges to update the configuration. Contact your administrator.'");
	ИначеЕсли ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		ВызватьИсключение НСтр("ka='Данная операция не доступна внешнему пользователю системы.';ru='Данная операция не доступна внешнему пользователю системы.';en='This operation is not available for external users of the system.'");
	КонецЕсли;
...
Процедура ПослеПодтвержденияБезопасности(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = "Продолжить" Тогда
		...
		Диалог.Фильтр = НСтр("ka='Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu';ru='Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu';en='All files supply (. * .Cf *; * cfu) | * .cf *;. * Cfu | delivery configuration files (* .cf) | * .cf | supplies Files configuration updates (* cfu.) | * .Cfu'");
		Диалог.Заголовок = НСтр("ka='Выбор поставки обновления конфигурации';ru='Выбор поставки обновления конфигурации';en='The choice of delivery configuration updates'");
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	КнопкаДалее.Заголовок = ?(ФункцияКнопкиДалее, НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'"), НСтр("ka='მზად არის';ru='Готово';en='Done'"));
	КнопкаЗакрыть.Заголовок = ?(ФункцияКнопкиЗакрыть, НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"), НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
...
Процедура ПерейтиСоСтраницыФайлаОбновления()
	...
	Если НуженФайлОбновления = 1 Тогда
		Если Не ЗначениеЗаполнено(Объект.ИмяФайлаОбновления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите файл поставки обновления конфигурации.';ru='Укажите файл поставки обновления конфигурации.';en='Specify configuration file updates delivery.'"),,"Объект.ИмяФайлаОбновления");
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоФайл() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Файл поставки обновления конфигурации не найден.';ru='Файл поставки обновления конфигурации не найден.';en='delivery configuration file updates can not be found.'"),,"Объект.ИмяФайлаОбновления");
			ТекущийЭлемент = Элементы.ПолеФайлОбновления;
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ВозможенЗапускОбновления()
	...
	#Если ВебКлиент Тогда
		...
		ТекстСообщения = НСтр("ka='Обновление программы недоступно в веб-клиенте.';ru='Обновление программы недоступно в веб-клиенте.';en='Software Update is not available in the web client.'");
	#КонецЕсли
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ka='Обновление программы доступно только в клиенте под управлением ОС Windows.';ru='Обновление программы доступно только в клиенте под управлением ОС Windows.';en='Update program is available only in the client running on Windows.'");
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ka='Обновление программы недоступно при подключении через веб-сервер.';ru='Обновление программы недоступно при подключении через веб-сервер.';en='Software Update is not available when connected to a web server.'");
	КонецЕсли;
...
Процедура ПрочитатьДанныеВЖурналРегистрации()
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Сред(ТекущаяСтрока, 3, 1) = "." И Сред(ТекущаяСтрока, 6, 1) = "." Тогда // Строка с датой
			...
			Если Комментарий = НСтр("ka='Обновление выполнено';ru='Обновление выполнено';en='upgrade Success'") Тогда
				Объект.РезультатОбновления = Истина;
				Продолжить;
			ИначеЕсли Комментарий = НСтр("ka='Обновление не выполнено';ru='Обновление не выполнено';en='Upgrading is not satisfied'") Тогда
