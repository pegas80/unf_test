...
#Область ПолучениеПараметров
	...
	Элементы.ДекорацияВерсияИРазмерАгентаТекст.Заголовок =
		Новый ФорматированнаяСтрока(
			НСтр("ka='Версия дистрибутива:';ru='Версия дистрибутива:';en='distribution Version:'"),
			" ",
			Новый ФорматированнаяСтрока(
			...
			Символы.ПС,
			НСтр("ka='კონფიგურაციის ზომა:';ru='Размер дистрибутива:';en='Size:'"),
			" ",
			Новый ФорматированнаяСтрока(
				Формат(ЭтотОбъект.РазмерДистрибутиваБайт, "ЧЦ=10; ЧДЦ=2; ЧС=6; ЧРД=,; ЧРГ=' '; ЧН=0,00; ЧГ=3,0") + " " + НСтр("ka='МБайт';ru='МБайт';en='MByte'"),
				Новый Шрифт(Элементы.ДекорацияВерсияИРазмерАгентаТекст.Шрифт,,,Истина))); // Жирный.
			...
			Если (ЭтотОбъект.АгентКопированияУстановлен) И (ЭтотОбъект.ТребуетсяОбновление = 0) Тогда
				Если (ЭтотОбъект.АгентКопированияАктивированНаТекущийЛогин = Истина) Тогда
					Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаУспешноТекст.Заголовок =
						Новый ФорматированнаяСтрока(
							СтрШаблон(
								НСтр("ka='Установлена версия Агента резервного копирования: %1';ru='Установлена версия Агента резервного копирования: %1';en='Installed version of Backup Agent:% 1'"),
							...
							Символы.ПС,
							Символы.ПС,
							НСтр("ka='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.';ru='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.';en='To set up an automatic backup schedule, you must run the 1C: Enterprise on the computer on which the backup agent.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ka='Просматривать список созданных резервных копий можно с любого компьютера.';ru='Просматривать список созданных резервных копий можно с любого компьютера.';en='View the list of backups from any computer.'"));
				...
				Иначе
					...
					Иначе
						Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаСОшибкамиТекст.Заголовок =
							Новый ФорматированнаяСтрока(
							НСтр("ka='Недостаточно свободных лицензий для активации Агента резервного копирования.';ru='Недостаточно свободных лицензий для активации Агента резервного копирования.';en='Not enough licenses available for the activation of a backup agent.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ka='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в';ru='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в';en='Purchase additional licenses for the use of a backup agent on the computer can be'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ka='личном кабинете';ru='личном кабинете';en='private office'"),
								,
...
"Для этого из';ka='Также в личном кабинете можно освободить лицензии с других компьютеров."
"Для этого из'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ka='личного кабинета';ru='личного кабинета';en='private office'"),
								,
								ЭтотОбъект.ЦветГиперссылки,
								,
								"backup1C:OpenWebPersonalAccount_DeactivateAgent"),
							" ",
							НСтр("ka='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.';ru='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.';en='you must select an unused backup the Agent and click Release.'"),
							Символы.ПС);
						ПереключитьсяНаСтраницуНаСервере("СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками");
					КонецЕсли;
				КонецЕсли;
...
Процедура КаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		Если Элемент.Имя = "КаталогЛогов" Тогда
			ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога для сохранения лог-файлов';ru='Выбор каталога для сохранения лог-файлов';en='Selecting directory to save the log files'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогЛогов;
		ИначеЕсли Элемент.Имя = "КаталогСтатистики" Тогда
			ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога для сохранения файлов статистики';ru='Выбор каталога для сохранения файлов статистики';en='Selecting directory to save the statistics files'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогСтатистики;
		ИначеЕсли Элемент.Имя = "КаталогРабочий" Тогда
			ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога для сохранения рабочих и временных файлов';ru='Выбор каталога для сохранения рабочих и временных файлов';en='Selecting directory to save the workers and temporary files'");
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики=300, СписокШагов = "")
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Сбор информации об использовании облачного хранилища';ru='Сбор информации об использовании облачного хранилища';en='Gathering information about the use of cloud storage'");
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Сбор информации об актуальных версиях Агентов резервного копирования';ru='Сбор информации об актуальных версиях Агентов резервного копирования';en='Gathering information about current versions of Backup Agents'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		Если (ПустаяСтрока(СписокШагов))
				ИЛИ (НЕ ПустаяСтрока(СписокШагов) И СтрНайти(СписокШагов, ШагСбораДанных.КодШага)) Тогда
			МассивШагов.Добавить(ШагСбораДанных);
...
#Область СтраницаПодключениеКВебИТС
		...
		лкДалее.Заголовок = НСтр("ka='მიერთება';ru='Подключиться';en='connect'");
		лкДалее.КнопкаПоУмолчанию = Истина;

		лкЗакрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
...
#Область СтраницаУстановкаАгентаРезервногоКопирования
		...
		лкДалее.Заголовок = НСтр("ka='დაყენება';ru='Установить';en='establish'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками
		...
		лкЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаУспешно
		...
		лкДалее.Заголовок = НСтр("ka='გავასწოროთ';ru='Настроить';en='Tune'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
"Проверка правильности логина и пароля.';ka='Подключение к сервисам интернет-поддержки."
"Проверка правильности логина и пароля.'");
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Проверка установки Агента резервного копирования.';ru='Проверка установки Агента резервного копирования.';en='Verifying the Installation Backup Agent.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"Проверьте правильность логина и пароля и повторите попытку.';ka='Произошла ошибка подключения к веб-сервисам."
...
Процедура ВыполнитьПереходНаКлиенте(ИмяТекущейСтраницы, НаправлениеПерехода)
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаПодключениеКВебИТС" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Подключение к сервисам интернет-поддержки.';ru='Подключение к сервисам интернет-поддержки.';en='Connecting to internet services support.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"Для этого из'"),
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Загрузка и установка Агента резервного копирования.';ru='Загрузка и установка Агента резервного копирования.';en='Download and install the Backup Agent.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
#Область ЗагрузкаФайлаИнсталляции
		...
		ОбновитьНадписиНаСервере(НСтр("ka='Загрузка файла установки агента резервного копирования.';ru='Загрузка файла установки агента резервного копирования.';en='Download the installation file backup agent.'"));
			...
			Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
				...
				Если СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Агент резервного копирования успешно загружен';ru='Агент резервного копирования успешно загружен';en='Backup Agent is successfully loaded'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если НеобходимаЗагрузка = Истина Тогда
				...
				Если ФайлУстановки.Существует() Тогда
					...
					Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
						...
						Если НЕ СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
							...
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
								НСтр("ka='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)';ru='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)';en='distribution file is corrupted (checksum downloaded file is different from that on your website)'"));
						Иначе
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
					...
					Иначе
						...
						ОписаниеРезультата = Новый ФорматированнаяСтрока(
							ОписаниеРезультата,
							Символы.ПС,
							НСтр("ka='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)';ru='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)';en='installation file is damaged (the downloaded distribution different from that indicated on the website)'"));
					КонецЕсли;
				Иначе
					ЕстьОшибки = Истина;
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Файл дистрибутива отсутствует во временном каталоге.';ru='Файл дистрибутива отсутствует во временном каталоге.';en='installation file is missing from the temporary folder.'"));
				КонецЕсли;
			Иначе // Не удалось загрузить...
				...
				ОписаниеРезультата = Новый ФорматированнаяСтрока(
					ОписаниеРезультата,
					Символы.ПС,
					НСтр("ka='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.';ru='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.';en='installation file is not loaded. An unexpected error occurred.'"));
			КонецЕсли;
		КонецЕсли;
...
#Область ЗапускИнсталляции
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Запуск установки агента резервного копирования.';ru='Запуск установки агента резервного копирования.';en='Run Setup backup agent.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Агент резервного копирования успешно установлен';ru='Агент резервного копирования успешно установлен';en='Backup Agent is successfully installed'"));
...
#Область СборИнформации_ИнформацияОКлиенте
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Сбор данных для агента резервного копирования (анализ состояния компьютера).';ru='Сбор данных для агента резервного копирования (анализ состояния компьютера).';en='Gathering data backup for the agent (the analysis of the state of the computer).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Все операции с агентом на компьютере завершены.';ru='Все операции с агентом на компьютере завершены.';en='All operations with the agent on the computer completed.'"));
...
#Область СозданиеФайловНастроек
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Создание настроек по-умолчанию для агента резервного копирования.';ru='Создание настроек по-умолчанию для агента резервного копирования.';en='Create default settings for the backup agent.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Успешно сохранены настройки для агента резервного копирования';ru='Успешно сохранены настройки для агента резервного копирования';en='Successfully saved settings for the backup agent'"));
...
#Область АктивацияАгентаКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Активация агента резервного копирования (получение ключей, первый запуск).';ru='Активация агента резервного копирования (получение ключей, первый запуск).';en='Activation of backup agent (receiving keys, the first run).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск';ru='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск';en='Activation Backup Agent: successfully launch codes obtained, completed the first run'"));
			...
			Если (НЕ ПустаяСтрока(РезультатАктивацииАгента.ЛогинАктивации))
				...
				Исключение
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Не удалось активировать аккаунт.';ru='Не удалось активировать аккаунт.';en='Failed to reactivate your account.'"));
				КонецПопытки;
				...
				Если КодВозврата = Неопределено Тогда
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Не удалось запустить Агент резервного копирования для активации аккаунта.';ru='Не удалось запустить Агент резервного копирования для активации аккаунта.';en='Failed to start backup agent to activate your account.'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
#Область СборИнформации_ИнформацияОКлиенте_АктивацииАгентовКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).';ru='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).';en='Gathering data backup for the agent (the analysis of the state of the computer, the activation state).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Проверка состояния активации завершена.';ru='Проверка состояния активации завершена.';en='Check the activation status is completed.'"));
...
#Область Окончание
		...
		ОбновитьНадписиНаСервере(НСтр("ka='Всё готово';ru='Всё готово';en='All is ready'"));
