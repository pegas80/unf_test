...
#Область ПолучениеПараметров
	...
	Элементы.ДекорацияВерсияИРазмерАгентаТекст.Заголовок =
		Новый ФорматированнаяСтрока(
			НСтр("ka='Версия дистрибутива:';ru='Версия дистрибутива:'"),
			" ",
			Новый ФорматированнаяСтрока(
			...
			Символы.ПС,
			НСтр("ka='კონფიგურაციის ზომა:';ru='Размер дистрибутива:'"),
			" ",
			Новый ФорматированнаяСтрока(
				Формат(ЭтотОбъект.РазмерДистрибутиваБайт, "ЧЦ=10; ЧДЦ=2; ЧС=6; ЧРД=,; ЧРГ=' '; ЧН=0,00; ЧГ=3,0") + " " + НСтр("ka='МБайт';ru='МБайт'"),
				Новый Шрифт(Элементы.ДекорацияВерсияИРазмерАгентаТекст.Шрифт,,,Истина))); // Жирный.
			...
			Если (ЭтотОбъект.АгентКопированияУстановлен) И (ЭтотОбъект.ТребуетсяОбновление = 0) Тогда
				Если (ЭтотОбъект.АгентКопированияАктивированНаТекущийЛогин = Истина) Тогда
					Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаУспешноТекст.Заголовок =
						Новый ФорматированнаяСтрока(
							СтрШаблон(
								НСтр("ka='Установлена версия Агента резервного копирования: %1';ru='Установлена версия Агента резервного копирования: %1'"),
							...
							Символы.ПС,
							Символы.ПС,
							НСтр("ka='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.';ru='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ka='Просматривать список созданных резервных копий можно с любого компьютера.';ru='Просматривать список созданных резервных копий можно с любого компьютера.'"));
				...
				Иначе
					...
					Иначе
						Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаСОшибкамиТекст.Заголовок =
							Новый ФорматированнаяСтрока(
							НСтр("ka='Недостаточно свободных лицензий для активации Агента резервного копирования.';ru='Недостаточно свободных лицензий для активации Агента резервного копирования.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ka='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в';ru='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ka='личном кабинете';ru='личном кабинете'"),
								,
...
"Для этого из'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ka='личного кабинета';ru='личного кабинета'"),
								,
								ЭтотОбъект.ЦветГиперссылки,
								,
								"backup1C:OpenWebPersonalAccount_DeactivateAgent"),
							" ",
							НСтр("ka='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.';ru='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.'"),
							Символы.ПС);
						ПереключитьсяНаСтраницуНаСервере("СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками");
					КонецЕсли;
				КонецЕсли;
...
Процедура КаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		Если Элемент.Имя = "КаталогЛогов" Тогда
			ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога для сохранения лог-файлов';ru='Выбор каталога для сохранения лог-файлов'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогЛогов;
		ИначеЕсли Элемент.Имя = "КаталогСтатистики" Тогда
			ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога для сохранения файлов статистики';ru='Выбор каталога для сохранения файлов статистики'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогСтатистики;
		ИначеЕсли Элемент.Имя = "КаталогРабочий" Тогда
			ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога для сохранения рабочих и временных файлов';ru='Выбор каталога для сохранения рабочих и временных файлов'");
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики=300, СписокШагов = "")
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Сбор информации об использовании облачного хранилища';ru='Сбор информации об использовании облачного хранилища'");
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Сбор информации об актуальных версиях Агентов резервного копирования';ru='Сбор информации об актуальных версиях Агентов резервного копирования'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		Если (ПустаяСтрока(СписокШагов))
				ИЛИ (НЕ ПустаяСтрока(СписокШагов) И СтрНайти(СписокШагов, ШагСбораДанных.КодШага)) Тогда
			МассивШагов.Добавить(ШагСбораДанных);
...
#Область СтраницаПодключениеКВебИТС
		...
		лкДалее.Заголовок = НСтр("ka='მიერთება';ru='Подключиться'");
		лкДалее.КнопкаПоУмолчанию = Истина;

		лкЗакрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
...
#Область СтраницаУстановкаАгентаРезервногоКопирования
		...
		лкДалее.Заголовок = НСтр("ka='დაყენება';ru='Установить'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками
		...
		лкЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаУспешно
		...
		лкДалее.Заголовок = НСтр("ka='გავასწოროთ';ru='Настроить'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
"Проверка правильности логина и пароля.'");
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Проверка установки Агента резервного копирования.';ru='Проверка установки Агента резервного копирования.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
Процедура ВыполнитьПереходНаКлиенте(ИмяТекущейСтраницы, НаправлениеПерехода)
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаПодключениеКВебИТС" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Подключение к сервисам интернет-поддержки.';ru='Подключение к сервисам интернет-поддержки.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"Для этого из'"),
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Загрузка и установка Агента резервного копирования.';ru='Загрузка и установка Агента резервного копирования.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
#Область ЗагрузкаФайлаИнсталляции
		...
		ОбновитьНадписиНаСервере(НСтр("ka='Загрузка файла установки агента резервного копирования.';ru='Загрузка файла установки агента резервного копирования.'"));
			...
			Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
				...
				Если СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Агент резервного копирования успешно загружен';ru='Агент резервного копирования успешно загружен'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если НеобходимаЗагрузка = Истина Тогда
				...
				Если ФайлУстановки.Существует() Тогда
					...
					Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
						...
						Если НЕ СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
							...
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
								НСтр("ka='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)';ru='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)'"));
						Иначе
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
					...
					Иначе
						...
						ОписаниеРезультата = Новый ФорматированнаяСтрока(
							ОписаниеРезультата,
							Символы.ПС,
							НСтр("ka='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)';ru='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)'"));
					КонецЕсли;
				Иначе
					ЕстьОшибки = Истина;
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Файл дистрибутива отсутствует во временном каталоге.';ru='Файл дистрибутива отсутствует во временном каталоге.'"));
				КонецЕсли;
			Иначе // Не удалось загрузить...
				...
				ОписаниеРезультата = Новый ФорматированнаяСтрока(
					ОписаниеРезультата,
					Символы.ПС,
					НСтр("ka='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.';ru='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.'"));
			КонецЕсли;
		КонецЕсли;
...
#Область ЗапускИнсталляции
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Запуск установки агента резервного копирования.';ru='Запуск установки агента резервного копирования.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Агент резервного копирования успешно установлен';ru='Агент резервного копирования успешно установлен'"));
...
#Область СборИнформации_ИнформацияОКлиенте
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Сбор данных для агента резервного копирования (анализ состояния компьютера).';ru='Сбор данных для агента резервного копирования (анализ состояния компьютера).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Все операции с агентом на компьютере завершены.';ru='Все операции с агентом на компьютере завершены.'"));
...
#Область СозданиеФайловНастроек
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Создание настроек по-умолчанию для агента резервного копирования.';ru='Создание настроек по-умолчанию для агента резервного копирования.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Успешно сохранены настройки для агента резервного копирования';ru='Успешно сохранены настройки для агента резервного копирования'"));
...
#Область АктивацияАгентаКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Активация агента резервного копирования (получение ключей, первый запуск).';ru='Активация агента резервного копирования (получение ключей, первый запуск).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск';ru='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск'"));
			...
			Если (НЕ ПустаяСтрока(РезультатАктивацииАгента.ЛогинАктивации))
				...
				Исключение
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Не удалось активировать аккаунт.';ru='Не удалось активировать аккаунт.'"));
				КонецПопытки;
				...
				Если КодВозврата = Неопределено Тогда
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ka='Не удалось запустить Агент резервного копирования для активации аккаунта.';ru='Не удалось запустить Агент резервного копирования для активации аккаунта.'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
#Область СборИнформации_ИнформацияОКлиенте_АктивацииАгентовКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).';ru='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ka='Проверка состояния активации завершена.';ru='Проверка состояния активации завершена.'"));
...
#Область Окончание
		...
		ОбновитьНадписиНаСервере(НСтр("ka='Всё готово';ru='Всё готово'"));
