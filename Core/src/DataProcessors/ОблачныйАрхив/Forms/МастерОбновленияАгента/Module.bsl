...
#Область ДекорацияИнформацияОНовомАгентеРезервногоКопированияТекст
	...
	МассивПодстрок.Добавить(НСтр("ka='Версия дистрибутива:';ru='Версия дистрибутива:'"));
	...
	МассивПодстрок.Добавить(НСтр("ka='კონფიგურაციის ზომა:';ru='Размер дистрибутива:'"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстЧтоНовогоВВерсии) Тогда
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("ka='Новое в версии.';ru='Новое в версии.'"),
					,
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстПорядокОбновления) Тогда
		МассивПодстрок.Добавить(НСтр("ka='Перед установкой обновления необходимо ознакомиться с';ru='Перед установкой обновления необходимо ознакомиться с'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("ka='особенностями перехода';ru='особенностями перехода'"),
					,
					ЦветаСтиля.ЦветГиперссылки,
					,
					"backup1C:BackupAgentUpdate_HowToUpdate")); // Идентификатор.
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(НСтр("ka='на эту версию Агента резервного копирования.';ru='на эту версию Агента резервного копирования.'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;

	Если ЭтотОбъект.АгентКопированияУстановлен Тогда
		МассивПодстрок.Добавить(НСтр("ka='Установлен Агент резервного копирования версии:';ru='Установлен Агент резервного копирования версии:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
		...
		МассивПодстрок.Добавить(Символы.ПС);
	Иначе
		МассивПодстрок.Добавить(НСтр("ka='Агент резервного копирования не установлен';ru='Агент резервного копирования не установлен'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;
...
"'"),
					Новый ФорматированнаяСтрока(
						НСтр("ka='Журнал регистрации.';ru='Журнал регистрации.'"),
						,
						ЦветаСтиля.ЦветГиперссылки,
						,
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаОбновлениеАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			...
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Загрузка и обновление Агента резервного копирования.';ru='Загрузка и обновление Агента резервного копирования.'");
...
#Область Окончание
		...
		Если ЕстьОшибки = Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='შეცდომები არის';ru='Есть ошибки'"));
