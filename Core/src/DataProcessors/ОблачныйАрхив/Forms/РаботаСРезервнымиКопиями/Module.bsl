...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияГиперссылкаКабинетКлиента.Подсказка =
		СтрШаблон(
			НСтр("ka='Логин: %1';ru='Логин: %1'"),
			?(ПустаяСтрока(ЭтотОбъект.Логин), НСтр("ka='не введен, будет запрошен в браузере.';ru='не введен, будет запрошен в браузере.'"), ЭтотОбъект.Логин));
	...
	Элементы.ФильтрПоЭтойИБ.Подсказка = СтрШаблон(НСтр("ka='Идентификатор ИБ: %1';ru='Идентификатор ИБ: %1'"), ЭтотОбъект.ИдентификаторИБ);
...
Процедура КомандаУдалитьФайл(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		СписокКнопок.Добавить(НСтр("ka='წაშლა';ru='Удалить'")); // Идентификатор
		СписокКнопок.Добавить(НСтр("ka='გაუქმება';ru='Отмена'")); // Идентификатор
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Удалить файл с сервера?';ru='Удалить файл с сервера?'"),
			СписокКнопок,
			0,
			НСтр("ka='გაუქმება';ru='Отмена'"), // Идентификатор
			НСтр("ka='Запрос на удаление файла с сервера';ru='Запрос на удаление файла с сервера'"));
	КонецЕсли;
...
Процедура УдалитьФайл_НаКлиенте(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = НСтр("ka='წაშლა';ru='Удалить'") Тогда // Идентификатор
