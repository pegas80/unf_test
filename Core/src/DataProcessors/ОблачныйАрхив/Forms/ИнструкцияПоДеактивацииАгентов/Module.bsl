...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.КоманднаяСтрокаДругойКомпьютер = 
		НСтр("ka='<Путь к установленному Агенту резервного копирования>';ru='<Путь к установленному Агенту резервного копирования>';en='<Path to the installed agent backup>'")
		+ ПолучитьРазделительПути()
		+ "BackupAgent.exe stop_backup -db_id="
		+ ИдентификаторИБ_Полный;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстПодсказки = НСтр("ka='Агенты резервного копирования были установлены на следующих компьютерах:';ru='Агенты резервного копирования были установлены на следующих компьютерах:';en='backup agents installed on the following computers:'")
			+ Символы.ПС
			+ ТекстПодсказки;
