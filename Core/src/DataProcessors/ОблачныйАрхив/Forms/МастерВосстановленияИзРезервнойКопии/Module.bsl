...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Загрузка файла резервной копии';ru='Загрузка файла резервной копии'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_BackUp
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Downloads
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Scripts
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)'"));
...
#Область ПолучениеКлючаАвторизации
	Если ПустаяСтрока(ЭтотОбъект.authKey) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Получение ключа авторизации (authKey) из приватных веб-сервисов';ru='Получение ключа авторизации (authKey) из приватных веб-сервисов'"));
...
Процедура ПриОткрытии(Отказ)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ЭтотОбъект.Заголовок = СтрШаблон(
			НСтр("ka='Загрузка резервной копии базы %1';ru='Загрузка резервной копии базы %1'"),
			ЭтотОбъект.DBName);

		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Проверка загруженного файла...';ru='Проверка загруженного файла...'");
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='ფაილის ატვირთვა....';ru='Загрузка файла...'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка = "";
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаСостояние;
...
Процедура ПутьДругогоКаталогаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога восстановления резервной копии';ru='Выбор каталога восстановления резервной копии'");
...
Процедура КомандаВосстановить(Команда)
	...
	Если НЕ ПустаяСтрока(Результат) Тогда
		...
		Если Каталог.Существует() Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ka='Необходимо указать имя каталога, а не имя файла.';ru='Необходимо указать имя каталога, а не имя файла.'");
		...
		Иначе
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Каталога %1 не существует. проверьте правильность написания имени каталога.';ru='Каталога %1 не существует. проверьте правильность написания имени каталога.'"),
				Результат);
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ka='Необходимо выбрать каталог восстановления резервной копии.';ru='Необходимо выбрать каталог восстановления резервной копии.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Поле  = "ПутьДругогоКаталога";
		Сообщение.УстановитьДанные(Объект);
...
Процедура КомандаОтменаЗагрузки(Команда)
	...
	Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Отменить загрузку?';ru='Отменить загрузку?'"), // ТекстВопроса
			РежимДиалогаВопрос.ДаНет, // Кнопки
			10, // Таймаут
			КодВозвратаДиалога.Нет, // КнопкаПоУмолчанию
			НСтр("ka='დადასტურება';ru='Подтверждение'"), // Заголовок
...
Процедура КомандаДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВыборКаталога Тогда
		...
		Если ПустаяСтрока(ЭтотОбъект.ПутьДругогоКаталога) Тогда
			ЕстьОшибки = Истина;
			ТекстСообщения = НСтр("ka='Необходимо выбрать каталог для восстановления.';ru='Необходимо выбрать каталог для восстановления.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
			Сообщение.УстановитьДанные(Объект);
		...
		Иначе
			...
			Если ТекущийКаталогИБ = КаталогВосстановления Тогда
				Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					НСтр("ka='Нельзя восстанавливать резервную копию в каталог с текущей информационной базой.';ru='Нельзя восстанавливать резервную копию в каталог с текущей информационной базой.'");
			...
			Иначе
				...
				Иначе
					Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
						СтрШаблон(
							НСтр("ka='Каталог %1 не существует.';ru='Каталог %1 не существует.'"),
...
#Область СтраницаЗагрузкаФайла
		...
		лкДалее.Заголовок       = НСтр("ka='ჩატვირთვა';ru='Загрузить'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ФайлЗагружен" Тогда
			...
			лкДалее.Заголовок   = НСтр("ka='Распаковать';ru='Распаковать'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки" Тогда
			лкДалее.Заголовок   = НСтр("ka='Загрузить заново';ru='Загрузить заново'");
			лкДалее.Доступность    = Истина;
			лкЗакрыть.Доступность  = Истина;
...
#Область СтраницаВыборКаталога
		...
		Форма.Заголовок = НСтр("ka='Выберите каталог для разархивации';ru='Выберите каталог для разархивации'");

		лкОтмена.Видимость    = Ложь;
		лкНазад.Видимость     = Ложь;
		лкДалее.Видимость     = Истина;
		лкДалее.Доступность   = Истина;
		лкДалее.Заголовок     = НСтр("ka='Разархивировать';ru='Разархивировать'");
		лкЗакрыть.Видимость   = Истина;
		лкЗакрыть.Доступность = Истина;
...
#Область СтраницаОтключениеСеансов
		...
		Форма.Заголовок = НСтр("ka='Отключение активных сеансов';ru='Отключение активных сеансов'");
		...
		Если Форма.КоличествоАктивныхСеансов = 0 Тогда
			лкДалее.Заголовок = НСтр("ka='Разархивировать';ru='Разархивировать'");
		Иначе
			лкДалее.Заголовок = НСтр("ka='განახლება';ru='Обновить'");
		КонецЕсли;
...
#Область СтраницаУспешноеЗавершение
		...
		Форма.Заголовок = НСтр("ka='Данные успешно восстановлены из резервной копии';ru='Данные успешно восстановлены из резервной копии'");
		...
		лкЗакрыть.Заголовок = НСтр("ka='მზად არის';ru='Готово'");
	...
	#Если НЕ ВебКлиент Тогда
		...
		Попытка
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Соединение с сервером для загрузки файла резервной копии';ru='Соединение с сервером для загрузки файла резервной копии'"));
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				ШагВыполнения,
				0,
				НСтр("ka='Успешное подключение к серверу для загрузки файла резервной копии';ru='Успешное подключение к серверу для загрузки файла резервной копии'"),
				Неопределено);
...
"%1'"),
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Ошибка подключения к http-сервисам.';ru='Ошибка подключения к http-сервисам.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка =
				СтрШаблон(
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Загрузка части файла резервной копии';ru='Загрузка части файла резервной копии'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
...
"'"),
			...
			Если ЭтотОбъект.КоличествоСбойныхЧастей = 0 Тогда
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = СтрШаблон(
					НСтр("ka='Загружено %1 частей из %2 (%3 из %4).';ru='Загружено %1 частей из %2 (%3 из %4).'"),
...
"Во время загрузки были сбои: %5'"),
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Завершение загрузки файла резервной копии';ru='Завершение загрузки файла резервной копии'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
			...
			Если МассивНеЗагруженных.Количество() = 0 Тогда
				...
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Объединение частей файла в один архив...';ru='Объединение частей файла в один архив...'");
				ЭтотОбъект.ОбновитьОтображениеДанных();
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
					ШагВыполнения,
					0,
					НСтр("ka='Загрузка файла резервной копии успешно завершена';ru='Загрузка файла резервной копии успешно завершена'"),
					Неопределено);
				ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
			...
			Иначе // Есть незагруженные части файлов
				...
				Если ЕстьЧтоЗагружать = Ложь Тогда
					...
					ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки"; // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
					// Текущее состояние загрузки.
					ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Остались незагруженные части файла.';ru='Остались незагруженные части файла.'");
					Элементы.ДекорацияОшибкаТекст.Подсказка =
...
"Загрузка завершена с ошибками.'");
					...
					ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
						ШагВыполнения,
						1002,
						НСтр("ka='Загрузка файла резервной копии не завершена из-за ошибок';ru='Загрузка файла резервной копии не завершена из-за ошибок'"),
						Неопределено);
					ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
	...
	#Если НЕ ВебКлиент Тогда
		...
		Элементы.ДекорацияУспешноеЗавершениеТекст.Заголовок =
			Новый ФорматированнаяСтрока(
				НСтр("ka='Информационная база из резервной копии успешно восстановлена в каталог';ru='Информационная база из резервной копии успешно восстановлена в каталог'"),
				Символы.ПС,
				Новый ФорматированнаяСтрока(
					ЭтотОбъект.ПутьДругогоКаталога,
					,
		...
		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Очистка каталога перед распаковкой файла...';ru='Очистка каталога перед распаковкой файла...'");
		Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок = "";
		Элементы.ГруппаВыборКаталогаПодвал.ТекущаяСтраница = Элементы.ГруппаВыборКаталогаПодвалПредупреждения;
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии';ru='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии'"));
		...
		Если ЕстьОшибкиУдаления = Истина Тогда
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Ошибка разархивации файла.';ru='Ошибка разархивации файла.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
...
"'"),
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Распаковка файла...';ru='Распаковка файла...'");
			...
			Попытка
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Разархивация загруженного файла резервной копии';ru='Разархивация загруженного файла резервной копии'"));
				...
				Иначе
					ЕстьОшибки = Истина;
					ПодробноеПредставлениеОшибки = НСтр("ka='Код возврата программы разархивации отличается от 0.';ru='Код возврата программы разархивации отличается от 0.'");
				КонецЕсли;
...
Процедура УдалитьВременныеФайлы()
	...
	ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Удаление временных файлов...';ru='Удаление временных файлов...'");
	ЭтотОбъект.ОбновитьОтображениеДанных();
	УдалитьФайлы(
		ЭтотОбъект.КаталогРабочий
			+ ПолучитьРазделительПутиКлиента()
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='Облачный архив. Веб-сервисы. Загрузка файла резервной копии';ru='Облачный архив. Веб-сервисы. Загрузка файла резервной копии'"), // ИмяСобытия
...
Процедура ПослеВопросаПодтвержденияОтменыЗагрузки(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Отмена загрузки файла резервной копии';ru='Отмена загрузки файла резервной копии'"));
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			ШагВыполнения,
			0,
			НСтр("ka='Отменена загрузка файла резервной копии';ru='Отменена загрузка файла резервной копии'"),
			Неопределено);
		ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
