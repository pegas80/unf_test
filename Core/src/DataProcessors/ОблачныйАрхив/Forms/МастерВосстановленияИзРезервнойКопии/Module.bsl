...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Загрузка файла резервной копии';ru='Загрузка файла резервной копии';en='Download a backup file'"));
...
"DBName: %3"
"UIDName: %4"
"Размер файла: %5';ka='Попытка загрузки файла с нулевым размером. Загрузка отменена."
"Логин: %1"
"UIDFile: %2"
"DBName: %3"
"UIDName: %4"
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)';en='download Tuning backup file, create a directory structure (UIDName)'"));
...
"%1"
"по причине:"
"%2';ka='Ошибка создания структуры рабочего каталога:"
"%1"
"по причине:"
"%2'"),
				лкКаталогВременныхФайлов,
...
"%1"
"Описание результата:"
"%2';ka='Завершено создание каталога"
"%1"
"Описание результата:"
"%2'"),
			лкКаталогВременныхФайлов,
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)';en='download Tuning backup file, create a directory structure (UIDName \ UIDFile)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_BackUp
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)';en='download Tuning backup file, create a directory structure (UIDName \ UIDFile \ BackUp)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Downloads
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)';en='download Tuning backup file, create a directory structure (UIDName \ UIDFile \ Downloads)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Scripts
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)';ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)';en='download Tuning backup file, create a directory structure (UIDName \ UIDFile \ Scripts)'"));
...
#Область ПолучениеКлючаАвторизации
	Если ПустаяСтрока(ЭтотОбъект.authKey) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Получение ключа авторизации (authKey) из приватных веб-сервисов';ru='Получение ключа авторизации (authKey) из приватных веб-сервисов';en='Preparation authorization key (authKey) from private web services'"));
...
"Код результата: %1"
"Описание результата:"
"%2';ka='Завершено получение ключа авторизации."
"Код результата: %1"
"Описание результата:"
"%2'"),
				ВложенныйКонтекстВыполненияПолученияКлючаАвторизации.КодРезультата,
...
"DBName: %3"
"UIDName: %4"
"Размер файла: %5';ka='Завершена инициализация (ПриСозданииНаСервере):"
"Логин: %1"
"UIDFile: %2"
"DBName: %3"
"UIDName: %4"
...
"Вначале отмените загрузку.';ka='Нельзя закрывать форму, пока происходит загрузка файла."
...
Процедура ПриОткрытии(Отказ)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ЭтотОбъект.Заголовок = СтрШаблон(
			НСтр("ka='Загрузка резервной копии базы %1';ru='Загрузка резервной копии базы %1';en='Download a copy of the backup database% 1'"),
			ЭтотОбъект.DBName);

		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Проверка загруженного файла...';ru='Проверка загруженного файла...';en='Checking the downloaded file ...'");
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='ფაილის ატვირთვა....';ru='Загрузка файла...';en='Loading file ...'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка = "";
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаСостояние;
...
Процедура ПутьДругогоКаталогаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		ДиалогВыбора.Заголовок = НСтр("ka='Выбор каталога восстановления резервной копии';ru='Выбор каталога восстановления резервной копии';en='Selecting catalog recovery backup'");
...
Процедура КомандаВосстановить(Команда)
	...
	Если НЕ ПустаяСтрока(Результат) Тогда
		...
		Если Каталог.Существует() Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ka='Необходимо указать имя каталога, а не имя файла.';ru='Необходимо указать имя каталога, а не имя файла.';en='You must specify the name of the directory, not a file name.'");
		...
		Иначе
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Каталога %1 не существует. проверьте правильность написания имени каталога.';ru='Каталога %1 не существует. проверьте правильность написания имени каталога.';en='Directory% 1 does not exist. check the spelling of the name of the directory.'"),
				Результат);
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ka='Необходимо выбрать каталог восстановления резервной копии.';ru='Необходимо выбрать каталог восстановления резервной копии.';en='You must select a catalog recovery backup.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Поле  = "ПутьДругогоКаталога";
		Сообщение.УстановитьДанные(Объект);
...
Процедура КомандаОтменаЗагрузки(Команда)
	...
	Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Отменить загрузку?';ru='Отменить загрузку?';en='Cancel the download?'"), // ТекстВопроса
			РежимДиалогаВопрос.ДаНет, // Кнопки
			10, // Таймаут
			КодВозвратаДиалога.Нет, // КнопкаПоУмолчанию
			НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'"), // Заголовок
...
Процедура КомандаДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВыборКаталога Тогда
		...
		Если ПустаяСтрока(ЭтотОбъект.ПутьДругогоКаталога) Тогда
			ЕстьОшибки = Истина;
			ТекстСообщения = НСтр("ka='Необходимо выбрать каталог для восстановления.';ru='Необходимо выбрать каталог для восстановления.';en='You need to select the directory to restore.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
			Сообщение.УстановитьДанные(Объект);
		...
		Иначе
			...
			Если ТекущийКаталогИБ = КаталогВосстановления Тогда
				Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					НСтр("ka='Нельзя восстанавливать резервную копию в каталог с текущей информационной базой.';ru='Нельзя восстанавливать резервную копию в каталог с текущей информационной базой.';en='You can not restore a backup copy of the catalog with the current information base.'");
			...
			Иначе
				...
				Иначе
					Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
						СтрШаблон(
							НСтр("ka='Каталог %1 не существует.';ru='Каталог %1 не существует.';en='The directory% 1 does not exist.'"),
...
#Область СтраницаЗагрузкаФайла
		...
		лкДалее.Заголовок       = НСтр("ka='ჩატვირთვა';ru='Загрузить';en='Download'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ФайлЗагружен" Тогда
			...
			лкДалее.Заголовок   = НСтр("ka='Распаковать';ru='Распаковать';en='unpack'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки" Тогда
			лкДалее.Заголовок   = НСтр("ka='Загрузить заново';ru='Загрузить заново';en='reloaded'");
			лкДалее.Доступность    = Истина;
			лкЗакрыть.Доступность  = Истина;
...
#Область СтраницаВыборКаталога
		...
		Форма.Заголовок = НСтр("ka='Выберите каталог для разархивации';ru='Выберите каталог для разархивации';en='Select the directory to unzip'");

		лкОтмена.Видимость    = Ложь;
		лкНазад.Видимость     = Ложь;
		лкДалее.Видимость     = Истина;
		лкДалее.Доступность   = Истина;
		лкДалее.Заголовок     = НСтр("ka='Разархивировать';ru='Разархивировать';en='unzip'");
		лкЗакрыть.Видимость   = Истина;
		лкЗакрыть.Доступность = Истина;
...
#Область СтраницаОтключениеСеансов
		...
		Форма.Заголовок = НСтр("ka='Отключение активных сеансов';ru='Отключение активных сеансов';en='Disabling active sessions'");
		...
		Если Форма.КоличествоАктивныхСеансов = 0 Тогда
			лкДалее.Заголовок = НСтр("ka='Разархивировать';ru='Разархивировать';en='unzip'");
		Иначе
			лкДалее.Заголовок = НСтр("ka='განახლება';ru='Обновить';en='update'");
		КонецЕсли;
...
#Область СтраницаУспешноеЗавершение
		...
		Форма.Заголовок = НСтр("ka='Данные успешно восстановлены из резервной копии';ru='Данные успешно восстановлены из резервной копии';en='Data is successfully restored from a backup'");
		...
		лкЗакрыть.Заголовок = НСтр("ka='მზად არის';ru='Готово';en='Done'");
	...
	#Если НЕ ВебКлиент Тогда
		...
		Попытка
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Соединение с сервером для загрузки файла резервной копии';ru='Соединение с сервером для загрузки файла резервной копии';en='The connection to the server to download the backup file'"));
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				ШагВыполнения,
				0,
				НСтр("ka='Успешное подключение к серверу для загрузки файла резервной копии';ru='Успешное подключение к серверу для загрузки файла резервной копии';en='A successful connection to the server to download a backup file'"),
				Неопределено);
...
"%1';ka='Ошибка подключения к серверу:"
"%1'"),
					ПодробноеПредставлениеОшибки),
				Неопределено);
			ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Ошибка подключения к http-сервисам.';ru='Ошибка подключения к http-сервисам.';en='Error connecting to http-services.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка =
				СтрШаблон(
					НСтр("ru='Ошибка подключения к http-сервисам:"
"%1';ka='Ошибка подключения к http-сервисам:"
"%1'"),
					ПодробноеПредставлениеОшибки);
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаОшибка;
		КонецЕсли;
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Загрузка части файла резервной копии';ru='Загрузка части файла резервной копии';en='Loading of the backup file'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
...
"%1"
"';ka='Произошла ошибка загрузки части файла:"
"%1"
"'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецПопытки;
...
"%1"
"%2"
"';ka='Неправильный код возврата (в HTTP ответе):"
"%1"
"%2"
"'"),
...
"';ka='Процесс загрузки был слишком длительный, ключ авторизации просрочился и будет получен заново."
"'");
					Иначе
						ОписаниеРезультата = ОписаниеРезультата
							+ СтрШаблон(
								НСтр("ru='Код возврата (errorCode) вернул ошибку: %1"
"';ka='Код возврата (errorCode) вернул ошибку: %1"
"'"),
								КодВозврата);
					КонецЕсли;
...
"';ka='Успешно загружена часть файла."
"Имя временного файла: %1"
"Размер файла (байт): %2"
"Контрольная сумма (md5): %3"
"'"),
...
"';ka='Не совпадает ожидаемая (%1) и реальная (%2) контрольная сумма (md5) части файла."
"'"),
										ТекущаяСтрока.КонтрольнаяСумма,
...
"';ka='Не совпадает ожидаемый (%1) и реальный (%2) размер части файла."
"'"),
									ТекущаяСтрока.Размер,
...
"';ka='Не найден временный файл (%1). Возможно, он не был загружен."
"'"),
								ТекущаяСтрока.ИмяВременногоФайла);
					КонецЕсли;
				КонецЕсли;
			...
			Если ЭтотОбъект.КоличествоСбойныхЧастей = 0 Тогда
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = СтрШаблон(
					НСтр("ka='Загружено %1 частей из %2 (%3 из %4).';ru='Загружено %1 частей из %2 (%3 из %4).';en='Uploaded parts of 1% 2% (3% of 4%).'"),
...
"Во время загрузки были сбои: %5';ka='Загружено %1 частей из %2 (%3 из %4)."
"Во время загрузки были сбои: %5'"),
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Завершение загрузки файла резервной копии';ru='Завершение загрузки файла резервной копии';en='Completion of the download file backup'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
			...
			Если МассивНеЗагруженных.Количество() = 0 Тогда
				...
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Объединение частей файла в один архив...';ru='Объединение частей файла в один архив...';en='Combining parts of the file into one file ...'");
				ЭтотОбъект.ОбновитьОтображениеДанных();
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
					ШагВыполнения,
					0,
					НСтр("ka='Загрузка файла резервной копии успешно завершена';ru='Загрузка файла резервной копии успешно завершена';en='Loading backup file successfully completed'"),
					Неопределено);
				ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
			...
			Иначе // Есть незагруженные части файлов
				...
				Если ЕстьЧтоЗагружать = Ложь Тогда
					...
					ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки"; // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
					// Текущее состояние загрузки.
					ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Остались незагруженные части файла.';ru='Остались незагруженные части файла.';en='Were unloaded part of the file.'");
					Элементы.ДекорацияОшибкаТекст.Подсказка =
						НСтр("ru='Остались незагруженные части файла,"
"по которым не осталось попыток загрузки."
"Загрузка завершена с ошибками.';ka='Остались незагруженные части файла,"
"по которым не осталось попыток загрузки."
"Загрузка завершена с ошибками.'");
					...
					ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
						ШагВыполнения,
						1002,
						НСтр("ka='Загрузка файла резервной копии не завершена из-за ошибок';ru='Загрузка файла резервной копии не завершена из-за ошибок';en='Download the backup file is not completed due to errors'"),
						Неопределено);
					ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
	...
	#Если НЕ ВебКлиент Тогда
		...
		Элементы.ДекорацияУспешноеЗавершениеТекст.Заголовок =
			Новый ФорматированнаяСтрока(
				НСтр("ka='Информационная база из резервной копии успешно восстановлена в каталог';ru='Информационная база из резервной копии успешно восстановлена в каталог';en='The information base from a backup is successfully restored to the catalog'"),
				Символы.ПС,
				Новый ФорматированнаяСтрока(
					ЭтотОбъект.ПутьДругогоКаталога,
					,
		...
		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Очистка каталога перед распаковкой файла...';ru='Очистка каталога перед распаковкой файла...';en='Cleaning the directory before unpacking the file ...'");
		Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок = "";
		Элементы.ГруппаВыборКаталогаПодвал.ТекущаяСтраница = Элементы.ГруппаВыборКаталогаПодвалПредупреждения;
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии';ru='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии';en='Cleaning the catalog recipient to unzip the downloaded file backup'"));
		...
		Если ЕстьОшибкиУдаления = Истина Тогда
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Ошибка разархивации файла.';ru='Ошибка разархивации файла.';en='Error uncompressing file.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			...
			Элементы.ДекорацияВыборКаталогаОшибкаТекст.Подсказка =
				СтрШаблон(
					НСтр("ka='Информация об ошибке:"
"%1';ru='Информация об ошибке:"
"%1'"),
					ПодробноеПредставлениеОшибки);
			Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					НСтр("ru='Ошибка очистки каталога."
"Возможно, запущена информационная база, связанная с этим каталогом.';ka='Ошибка очистки каталога."
...
"Ошибка:"
"%4"
"';ka='Ошибки очистки каталога-приемника перед распаковкой резервной копии."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Путь для очистки: %3"
"Ошибка:"
...
"Путь к загруженному файлу: %2"
"Путь для очистки: %3"
"';ka='Успешная очистка каталога-приемника."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Путь для очистки: %3"
"'"),
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Распаковка файла...';ru='Распаковка файла...';en='File unpacking ...'");
			...
			Попытка
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Разархивация загруженного файла резервной копии';ru='Разархивация загруженного файла резервной копии';en='Unzip the downloaded file backup'"));
				...
				Иначе
					ЕстьОшибки = Истина;
					ПодробноеПредставлениеОшибки = НСтр("ka='Код возврата программы разархивации отличается от 0.';ru='Код возврата программы разархивации отличается от 0.';en='unzipping program return code is different from 0.'");
				КонецЕсли;
...
"';ka='Успешная разархивация файла резервной копии."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Размер загруженного файла: %3"
"Код возврата программы: %4"
...
"Ошибка:"
"%7"
"';ka='Ошибка разархивации файла резервной копии."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Размер загруженного файла: %3"
"Код возврата программы: %4"
...
"%1'"),
				...
				Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					СтрШаблон(
						НСтр("ka='Ошибка разархивации файла:"
...
Процедура УдалитьВременныеФайлы()
	...
	ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ka='Удаление временных файлов...';ru='Удаление временных файлов...';en='Deleting temporary files ...'");
	ЭтотОбъект.ОбновитьОтображениеДанных();
	УдалитьФайлы(
		ЭтотОбъект.КаталогРабочий
			+ ПолучитьРазделительПутиКлиента()
...
"%4"
"';ka='Загрузка файла резервной копии, итоговый отчет."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ka='Облачный архив. Веб-сервисы. Загрузка файла резервной копии';ru='Облачный архив. Веб-сервисы. Загрузка файла резервной копии';en='Cloud archive. Web services. Download a backup file'"), // ИмяСобытия
...
Процедура ПослеВопросаПодтвержденияОтменыЗагрузки(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Отмена загрузки файла резервной копии';ru='Отмена загрузки файла резервной копии';en='Cancel download a backup file'"));
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			ШагВыполнения,
			0,
			НСтр("ka='Отменена загрузка файла резервной копии';ru='Отменена загрузка файла резервной копии';en='Cancel download a backup file'"),
			Неопределено);
		ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
