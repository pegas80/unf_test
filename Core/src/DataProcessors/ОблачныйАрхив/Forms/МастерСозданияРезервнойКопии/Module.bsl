...
#Область СборДанных
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Сбор данных о настройках';ru='Сбор данных о настройках'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ПроверкаВозможностиСозданияРезервнойКопии_НастройкиКлиентскогоКомпьютера
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Информация о клиентском компьютере';ru='Информация о клиентском компьютере'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"%5'"),
					...
					?(ИнформацияОКлиенте.АгентКопированияУстановлен = Истина,
						" - " + НСтр("ka='Агент резервного копирования установлен';ru='Агент резервного копирования установлен'"),
						" - " + НСтр("ka='Агент резервного копирования НЕ установлен';ru='Агент резервного копирования НЕ установлен'")),
					?(ИнформацияОКлиенте.АгентКопированияАктивирован_ЕстьФайлыСертификатов = Истина,
						" - " + НСтр("ka='Файлы сертификатов загружены';ru='Файлы сертификатов загружены'"),
						" - " + НСтр("ka='Файлы сертификатов НЕ загружены (агент не активирован)';ru='Файлы сертификатов НЕ загружены (агент не активирован)'")),
					?(АктивацииАгентовКопирования.АгентКопированияАктивирован_ДанныеВебСервисов = Истина,
						" - " + НСтр("ka='Агент резервного копирования активирован на текущий логин';ru='Агент резервного копирования активирован на текущий логин'"),
						" - " + НСтр("ka='Агент резервного копирования НЕ активирован на текущий логин';ru='Агент резервного копирования НЕ активирован на текущий логин'")),
					?(АктивацииАгентовКопирования.АгентКопированияАктивированНаДругойЛогин = Истина,
						" - " + НСтр("ka='Агент резервного копирования активирован на другой логин';ru='Агент резервного копирования активирован на другой логин'"),
						" - " + НСтр("ka='Агент резервного копирования НЕ активирован на другой логин';ru='Агент резервного копирования НЕ активирован на другой логин'")));
		КонецЕсли;
...
#Область ПроверкаВозможностиСозданияРезервнойКопии_НастройкиКлиентскогоКомпьютера
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Проверка свободного места в облачном хранилище';ru='Проверка свободного места в облачном хранилище'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Доступно %4 байт (%5 Мбайт, %6 Гбайт)'"),
		...
		Если БайтДоступно < 10*1024*1024 Тогда
			...
			ОписаниеРезультата = ОписаниеРезультата + Символы.ПС + Символы.ПС
				+ НСтр("ka='В облачном хранилище недостаточно места для создания резервной копии (менее 10 МБайт).';ru='В облачном хранилище недостаточно места для создания резервной копии (менее 10 МБайт).'");
		КонецЕсли;

		Элементы.ДекорацияСтатистика.Заголовок = СтрШаблон(
			НСтр("ka='Всего доступно %1 из %2 Мбайт (%3)';ru='Всего доступно %1 из %2 Мбайт (%3)'"),
...
#Область ПолучениеОбщихНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Чтение общих настроек';ru='Чтение общих настроек'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЧтениеНастроекАгентаРезервногоКопирования
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Чтение настроек агента резервного копирования';ru='Чтение настроек агента резервного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЧтениеСпискаРезервныхКопий
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Анализ списка резервных копий';ru='Анализ списка резервных копий'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
		...
		Если ДатаПоследнегоРезервногоКопирования = '00010101' Тогда
			ТекстЗаголовка = НСтр("ka='Резервное копирование еще ни разу не проводилось';ru='Резервное копирование еще ни разу не проводилось'");
		Иначе
			ТекстЗаголовка =
				СтрШаблон(
					НСтр("ka='В последний раз резервное копирование проводилось: %1';ru='В последний раз резервное копирование проводилось: %1'"),
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // Имя события
		НСтр("ka='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация';ru='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация'"), // ИмяСобытия
...
"работающих в программе.'"),
			...
			Новый ФорматированнаяСтрока(
				СтрШаблон(
					НСтр("ka='Активные сеансы: %1';ru='Активные сеансы: %1'"),
...
#Область СтраницаПредупреждениеОДлительнойОперации
		...
		лкДалее.Заголовок = НСтр("ka='სარეზერვო ასლის შექმნა';ru='Создать резервную копию'");
...
Процедура НачатьРезервноеКопирование()
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, установка блокировки';ru='Запуск резервного копирования, установка блокировки'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, подготовка к выходу из программы';ru='Запуск резервного копирования, подготовка к выходу из программы'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"'"),
	...
	ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент'"), // Имя события
		НСтр("ka='Облачный архив. Сервис и регламент. Резервное копирование. Завершение';ru='Облачный архив. Сервис и регламент. Резервное копирование. Завершение'"), // ИмяСобытия
...
Процедура УстановитьБлокировкуСеансовНа2Минуты()
	...
	Блокировка.Сообщение = СоединенияИБ.СформироватьСообщениеБлокировки(
		НСтр("ka='Выполняется резервное копирование. Вход в информационную базу заблокирован на 2 минуты.';ru='Выполняется резервное копирование. Вход в информационную базу заблокирован на 2 минуты.'"),
		КодРазрешения);
...
#Область ЧтениеНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, чтение настроек';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, чтение настроек'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ИзменениеНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, добавление настроек для немедленного копирования';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, добавление настроек для немедленного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЗаписьНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, запись файла настроек';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, запись файла настроек'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область РеинициализацияАгентаРезервногоКопирования
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, реинициализация агента резервного копирования';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, реинициализация агента резервного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область УдалениеВременногоФайла
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, удаление временных файлов';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, удаление временных файлов'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
		...
		Попытка
			...
			ТекстРезультата = НСтр("ka='Успешно удален';ru='Успешно удален'");
		Исключение
			ФайлУдален = Ложь;
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики = 300)
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Получение списка резервных копий';ru='Получение списка резервных копий'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		МассивШагов.Добавить(ШагСбораДанных);
