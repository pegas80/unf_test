...
#Область СборДанных
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Сбор данных о настройках';ru='Сбор данных о настройках';en='Collection of data on preferences'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ПроверкаВозможностиСозданияРезервнойКопии_НастройкиКлиентскогоКомпьютера
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Информация о клиентском компьютере';ru='Информация о клиентском компьютере';en='Information about a client computer'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"%3"
"%4"
"%5';ka='На этом компьютере (идентификатор: %1) невозможно работать с Агентом резервного копирования:"
"%2"
"%3"
"%4"
"%5'"),
					...
					?(ИнформацияОКлиенте.АгентКопированияУстановлен = Истина,
						" - " + НСтр("ka='Агент резервного копирования установлен';ru='Агент резервного копирования установлен';en='backup agent is installed'"),
						" - " + НСтр("ka='Агент резервного копирования НЕ установлен';ru='Агент резервного копирования НЕ установлен';en='Backup Agent is not installed'")),
					?(ИнформацияОКлиенте.АгентКопированияАктивирован_ЕстьФайлыСертификатов = Истина,
						" - " + НСтр("ka='Файлы сертификатов загружены';ru='Файлы сертификатов загружены';en='certificate files downloaded'"),
						" - " + НСтр("ka='Файлы сертификатов НЕ загружены (агент не активирован)';ru='Файлы сертификатов НЕ загружены (агент не активирован)';en='certificate files are not loaded (the agent is not activated)'")),
					?(АктивацииАгентовКопирования.АгентКопированияАктивирован_ДанныеВебСервисов = Истина,
						" - " + НСтр("ka='Агент резервного копирования активирован на текущий логин';ru='Агент резервного копирования активирован на текущий логин';en='Backup Agent is enabled for the current login'"),
						" - " + НСтр("ka='Агент резервного копирования НЕ активирован на текущий логин';ru='Агент резервного копирования НЕ активирован на текущий логин';en='Backup Agent is not enabled for the current login'")),
					?(АктивацииАгентовКопирования.АгентКопированияАктивированНаДругойЛогин = Истина,
						" - " + НСтр("ka='Агент резервного копирования активирован на другой логин';ru='Агент резервного копирования активирован на другой логин';en='Backup Agent is activated to another login'"),
						" - " + НСтр("ka='Агент резервного копирования НЕ активирован на другой логин';ru='Агент резервного копирования НЕ активирован на другой логин';en='Backup Agent is not activated on another login'")));
		КонецЕсли;
...
#Область ПроверкаВозможностиСозданияРезервнойКопии_НастройкиКлиентскогоКомпьютера
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Проверка свободного места в облачном хранилище';ru='Проверка свободного места в облачном хранилище';en='Check the available space in the cloud storage'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Доступно %4 байт (%5 Мбайт, %6 Гбайт)';ka='Состояние облачного хранилища:"
"Куплено: %1 байт (%2 Мбайт, %3 Гбайт)"
"Доступно %4 байт (%5 Мбайт, %6 Гбайт)'"),
		...
		Если БайтДоступно < 10*1024*1024 Тогда
			...
			ОписаниеРезультата = ОписаниеРезультата + Символы.ПС + Символы.ПС
				+ НСтр("ka='В облачном хранилище недостаточно места для создания резервной копии (менее 10 МБайт).';ru='В облачном хранилище недостаточно места для создания резервной копии (менее 10 МБайт).';en='The cloud storage is not enough space to create a backup (less than 10 MB).'");
		КонецЕсли;

		Элементы.ДекорацияСтатистика.Заголовок = СтрШаблон(
			НСтр("ka='Всего доступно %1 из %2 Мбайт (%3)';ru='Всего доступно %1 из %2 Мбайт (%3)';en='Total available from 1% 2% MB (3%)'"),
...
#Область ПолучениеОбщихНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Чтение общих настроек';ru='Чтение общих настроек';en='Reading the general settings'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЧтениеНастроекАгентаРезервногоКопирования
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Чтение настроек агента резервного копирования';ru='Чтение настроек агента резервного копирования';en='Reading settings backup agent'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Рабочий каталог: %1"
"Идентификатор ИБ: %2"
"Имя файла настроек: %3';ka='Чтение настроек Агента резервного копирования:"
"Рабочий каталог: %1"
"Идентификатор ИБ: %2"
...
#Область ЧтениеСпискаРезервныхКопий
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Анализ списка резервных копий';ru='Анализ списка резервных копий';en='Analysis of the list of backups'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
		...
		Если ДатаПоследнегоРезервногоКопирования = '00010101' Тогда
			ТекстЗаголовка = НСтр("ka='Резервное копирование еще ни разу не проводилось';ru='Резервное копирование еще ни разу не проводилось';en='Backing up has never conducted'");
		Иначе
			ТекстЗаголовка =
				СтрШаблон(
					НСтр("ka='В последний раз резервное копирование проводилось: %1';ru='В последний раз резервное копирование проводилось: %1';en='In the last backup was performed: 1%'"),
...
"Идентификатор ИБ: %1"
"Дата самой последней резервной копии: %2';ka='Анализ списка резервных копий:"
"Идентификатор ИБ: %1"
...
"%4"
"';ka='Создание резервной копии, инициализация формы, завершение."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // Имя события
		НСтр("ka='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация';ru='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация';en='Cloud archive. Service and regulations. Backup. Initialization'"), // ИмяСобытия
...
"работающих в программе.';ka='Производится отключение сеансов других пользователей,"
"работающих в программе.'"),
			Символы.ПС,
			Символы.ПС,
			Новый ФорматированнаяСтрока(
				СтрШаблон(
					НСтр("ka='Активные сеансы: %1';ru='Активные сеансы: %1';en='Active sessions: 1%'"),
...
"которые не могут быть завершены принудительно.';ka='Имеются активные сеансы работы с программой,"
...
#Область СтраницаПредупреждениеОДлительнойОперации
		...
		лкДалее.Заголовок = НСтр("ka='სარეზერვო ასლის შექმნა';ru='Создать резервную копию';en='Create a backup'");
...
Процедура НачатьРезервноеКопирование()
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, установка блокировки';ru='Запуск резервного копирования, установка блокировки';en='Running backup, installation lock'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования';en='Running the backup, change the settings for the backup agent'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, подготовка к выходу из программы';ru='Запуск резервного копирования, подготовка к выходу из программы';en='Running backup, preparing to exit from the program'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Результат выполнения:"
"%4"
"';ka='Запуск резервного копирования."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // Имя события
		НСтр("ka='Облачный архив. Сервис и регламент. Резервное копирование. Завершение';ru='Облачный архив. Сервис и регламент. Резервное копирование. Завершение';en='Cloud archive. Service and regulations. Backup. completion'"), // ИмяСобытия
...
Процедура УстановитьБлокировкуСеансовНа2Минуты()
	...
	Блокировка.Сообщение = СоединенияИБ.СформироватьСообщениеБлокировки(
		НСтр("ka='Выполняется резервное копирование. Вход в информационную базу заблокирован на 2 минуты.';ru='Выполняется резервное копирование. Вход в информационную базу заблокирован на 2 минуты.';en='Backed up. Entrance to the information base is blocked for 2 minutes.'"),
		КодРазрешения);
...
#Область ЧтениеНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, чтение настроек';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, чтение настроек';en='Running the backup, change settings to backup the Agent, setting reading'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
		...
		ОписаниеРезультата = СтрШаблон(
			НСтр("ka='Созданы настройки по-умолчанию:"
...
#Область ИзменениеНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, добавление настроек для немедленного копирования';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, добавление настроек для немедленного копирования';en='Running the backup, change the settings for the backup agent, the addition of settings for immediate backup'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЗаписьНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, запись файла настроек';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, запись файла настроек';en='Running the backup, change settings to backup the Agent configuration file entry'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область РеинициализацияАгентаРезервногоКопирования
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, реинициализация агента резервного копирования';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, реинициализация агента резервного копирования';en='Running the backup, change the settings for the backup agent, the backup agent reinitialized'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Код возврата: %2';ka='Команда реинициализации: %1"
"Код возврата: %2'"),
		СтрокаКоманды,
		КодРезультата);
...
#Область УдалениеВременногоФайла
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Запуск резервного копирования, изменение настроек для Агента резервного копирования, удаление временных файлов';ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, удаление временных файлов';en='Running the backup, change the settings for the backup agent, delete temporary files'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
		...
		Попытка
			...
			ТекстРезультата = НСтр("ka='Успешно удален';ru='Успешно удален';en='successfully deleted'");
		Исключение
			ФайлУдален = Ложь;
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстРезультата = СтрШаблон(
				НСтр("ka='Ошибка удаления:"
...
"Результат: %2';ka='Удаление временного файла: %1"
"Результат: %2'"),
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики = 300)
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Получение списка резервных копий';ru='Получение списка резервных копий';en='Getting the list of backups'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		МассивШагов.Добавить(ШагСбораДанных);
