...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("ka='Некорректные параметры объектов назначения открытия команды ""%1""';ru='Некорректные параметры объектов назначения открытия команды ""%1""'"),
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ka='Для ""%1"" редактирование регистрации объектов недоступно.';ru='Для ""%1"" редактирование регистрации объектов недоступно.'"),
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") 
		...
		Ошибка = НСтр("ka='Неожиданный результат выбора из консоли запросов';ru='Неожиданный результат выбора из консоли запросов'");
	Иначе
...
"на узле ""%1""?'");
	...
	ЗаголовокВопроса = НСтр("ka='დადასტურება';ru='Подтверждение'");
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Сериализация.Количество() > 0 Тогда
		...
		ЗаголовокТекста = НСтр("ka='Результат стандартной выгрузки (РИБ)';ru='Результат стандартной выгрузки (РИБ)'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Не выгружалось';ru='Не выгружалось'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Нет изменений';ru='Нет изменений'"));
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("ka='გაგზავნილის ნომერი';ru='Номер отправленного'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ka='№ отправленного %1, № принятого %2';ru='№ отправленного %1, № принятого %2'");
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("ka='Регистрация изменений:';ru='Регистрация изменений:'");
			ТекстПредупреждения = НСтр("ka='Зарегистрировано %1 изменений из %2"
"на узле ""%0""';ru='Зарегистрировано %1 изменений из %2"
"на узле ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("ka='Отмена регистрации:';ru='Отмена регистрации:'");
			ТекстПредупреждения = НСтр("ru='Отменена регистрация %1 изменений "
"на узле ""%0"".';ka='Отменена регистрация %1 изменений "
"на узле ""%0"".'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("ka='Изменение номера сообщения:';ru='Изменение номера сообщения:'");
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ТекИмяФормы <> Неопределено Тогда
		...
		Если КомандаОперации Тогда
			Текст = НСтр("ka='Регистрация изменений результата запроса';ru='Регистрация изменений результата запроса'");
		Иначе
			Текст = НСтр("ka='Отмена регистрации изменений результата запроса';ru='Отмена регистрации изменений результата запроса'");
		КонецЕсли;
...
"Настроить сейчас?'");
	...
	ЗаголовокВопроса = НСтр("ka='პარამეტრები';ru='Настройки'");
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("ka='В последнем результате запроса отсутствует колонка ""Ссылка""';ru='В последнем результате запроса отсутствует колонка ""Ссылка""'");
		КонецЕсли;
	Иначе		
		Возврат НСтр("ka='Ошибка получения данных результата запроса';ru='Ошибка получения данных результата запроса'");
	КонецЕсли;
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено, МетаИмяУзел = Неопределено)
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("ka='все объекты %1 по выбранной иерархии вида';ru='все объекты %1 по выбранной иерархии вида'");
	...
	Иначе
		...
		Текст = НСтр("ka='все объекты типа ""%3"" %1';ru='все объекты типа ""%3"" %1'");
	...
	Если БезУчетаАвторегистрации Тогда
		ТекстФлага = "";
	Иначе
		ТекстФлага = НСтр("ka='с признаком авторегистрации';ru='с признаком авторегистрации'");
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ka='Регистрация изменений для обмена данными';ru='Регистрация изменений для обмена данными'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ka='Регистрация изменений для обмена с  ""%1""';ru='Регистрация изменений для обмена с  ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
