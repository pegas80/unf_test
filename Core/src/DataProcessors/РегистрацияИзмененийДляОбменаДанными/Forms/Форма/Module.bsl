...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("ka='Некорректные параметры объектов назначения открытия команды ""%1""';ru='Некорректные параметры объектов назначения открытия команды ""%1""';en='Incorrect settings objects purpose of opening the command ""%1""'"),
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ka='Для ""%1"" редактирование регистрации объектов недоступно.';ru='Для ""%1"" редактирование регистрации объектов недоступно.';en='For ""%1"" Registry editing facilities available.'"),
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") 
		...
		Ошибка = НСтр("ka='Неожиданный результат выбора из консоли запросов';ru='Неожиданный результат выбора из консоли запросов';en='An unexpected result of the selection requests from console'");
	Иначе
	...
	Если ВыбранноеЗначение.ДействиеВыбора Тогда
		Текст = НСтр("ka='Зарегистрировать результат запроса"
"на узле ""%1""?';ru='Зарегистрировать результат запроса"
"на узле ""%1""?'"); 
	Иначе
		Текст = НСтр("ru='Отменить регистрацию результата запроса"
"на узле ""%1""?';ka='Отменить регистрацию результата запроса"
"на узле ""%1""?'");
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", Строка(УзелОбменаСсылка));
					 
	ЗаголовокВопроса = НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'");
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Сериализация.Количество() > 0 Тогда
		...
		ЗаголовокТекста = НСтр("ka='Результат стандартной выгрузки (РИБ)';ru='Результат стандартной выгрузки (РИБ)';en='Result standard discharge (RIB)'");
...
Процедура ЗарегистрироватьИОМИПредопределенные(Команда)
	...
	ТекстВопроса     = СтрЗаменить( 
		НСтр("ka='Зарегистрировать данные для восстановления подчиненного узла РИБ"
"на узле ""%1""?';ru='Зарегистрировать данные для восстановления подчиненного узла РИБ"
"на узле ""%1""?'"),
		"%1", УзелОбменаСсылка);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Не выгружалось';ru='Не выгружалось';en='It is not unloaded'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Нет изменений';ru='Нет изменений';en='No change'"));
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("ka='გაგზავნილის ნომერი';ru='Номер отправленного';en='number sent'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ka='№ отправленного %1, № принятого %2';ru='№ отправленного %1, № принятого %2';en='№ sent% 1, № received 2%'");
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("ka='Регистрация изменений:';ru='Регистрация изменений:';en='Registration of changes:'");
			ТекстПредупреждения = НСтр("ka='Зарегистрировано %1 изменений из %2"
"на узле ""%0""';ru='Зарегистрировано %1 изменений из %2"
"на узле ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("ka='Отмена регистрации:';ru='Отмена регистрации:';en='Cancellation of registration:'");
			ТекстПредупреждения = НСтр("ru='Отменена регистрация %1 изменений "
"на узле ""%0"".';ka='Отменена регистрация %1 изменений "
"на узле ""%0"".'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("ka='Изменение номера сообщения:';ru='Изменение номера сообщения:';en='Changing the message number:'");
		ТекстПредупреждения = НСтр("ru='Номер сообщения изменен на %3"
"у %1 объекта(ов)';ka='Номер сообщения изменен на %3"
"у %1 объекта(ов)'");
	КонецЕсли;
...
"на узле ""%1""?';ka='Отменить регистрацию ""%2"""
"на узле ""%1""?'"); 
	Иначе
		Текст = НСтр("ru='Отменить регистрацию выбранных констант"
"на узле ""%1""?';ka='Отменить регистрацию выбранных констант"
"на узле ""%1""?'"); 
	КонецЕсли;
...
"на узле ""%1""?';ka='Отменить регистрацию выбранных объектов"
"на узле ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ka='Зарегистрировать %1 для выгрузки на узле ""%2""?"
""
"Изменение регистрации большого количества объектов может занять продолжительное время!';ru='Зарегистрировать %1 для выгрузки на узле ""%2""?"
""
...
""
"Изменение регистрации большого количества объектов может занять продолжительное время!';ka='Отменить регистрацию %1 для выгрузки на узле ""%2""?"
""
"Изменение регистрации большого количества объектов может занять продолжительное время!'");
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ТекИмяФормы <> Неопределено Тогда
		...
		Если КомандаОперации Тогда
			Текст = НСтр("ka='Регистрация изменений результата запроса';ru='Регистрация изменений результата запроса';en='Register query result changes'");
		Иначе
			Текст = НСтр("ka='Отмена регистрации изменений результата запроса';ru='Отмена регистрации изменений результата запроса';en='Canceling a query result changes'");
		КонецЕсли;
	...
	Текст = НСтр("ka='В настройках не указана обработка для выполнения запросов."
"Настроить сейчас?';ru='В настройках не указана обработка для выполнения запросов."
"Настроить сейчас?'");
	
	ЗаголовокВопроса = НСтр("ka='პარამეტრები';ru='Настройки';en='settings'");
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("ka='В последнем результате запроса отсутствует колонка ""Ссылка""';ru='В последнем результате запроса отсутствует колонка ""Ссылка""';en='In the last column of the query result is not the ""Link""'");
		КонецЕсли;
	Иначе		
		Возврат НСтр("ka='Ошибка получения данных результата запроса';ru='Ошибка получения данных результата запроса';en='Error retrieving query result data'");
	КонецЕсли;
...
"';ka='Зарегистрировано объектов: %1"
"Выгружено объектов: %2"
"Не выгружено объектов: %3"
"'");
...
"командами ""Зарегистрировать"" или ""Отменить регистрацию""';ru='%1."
""
"%2"
"Для регистрации или отмены регистрации обмена данными на узле"
"""%3"""
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено, МетаИмяУзел = Неопределено)
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("ka='все объекты %1 по выбранной иерархии вида';ru='все объекты %1 по выбранной иерархии вида';en='objects% 1 to the selected hierarchy type'");
	...
	Иначе
		...
		Текст = НСтр("ka='все объекты типа ""%3"" %1';ru='все объекты типа ""%3"" %1';en=""all objects of type '% 3'% 1""");
	...
	Если БезУчетаАвторегистрации Тогда
		ТекстФлага = "";
	Иначе
		ТекстФлага = НСтр("ka='с признаком авторегистрации';ru='с признаком авторегистрации';en='a sign of auto-enrollment'");
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ka='Регистрация изменений для обмена данными';ru='Регистрация изменений для обмена данными';en='Registration of changes for data exchange'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ka='Регистрация изменений для обмена с  ""%1""';ru='Регистрация изменений для обмена с  ""%1""';en='Registration of changes to share with the ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
"1С:Предприятие 8.3.5 с отключенным режимом совместимости или выше';ka='Обработка предназначена для запуска на версии платформы"
