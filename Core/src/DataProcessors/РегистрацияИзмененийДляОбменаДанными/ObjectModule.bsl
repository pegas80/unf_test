...
// Параметры:
	...
	ТекНомерСтроки = 1;
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 1,  2,  Ложь,   "Константы",               НСтр("ka='კონსტანტები';ru='Константы'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 3,  4,  Истина, "Справочники",             НСтр("ka='ცნობარები';ru='Справочники'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 5,  6,  Истина, "Последовательности",      НСтр("ka='Последовательности';ru='Последовательности'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 7,  8,  Истина, "Документы",               НСтр("ka='დოკუმენტები';ru='Документы'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 9,  10, Истина, "ПланыВидовХарактеристик", НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 11, 12, Истина, "ПланыСчетов",             НСтр("ka='ანგარიშეგების გეგმები';ru='Планы счетов'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 13, 14, Истина, "ПланыВидовРасчета",       НСтр("ka='ანგარიშსწორებების ტიპების გეგმები';ru='Планы видов расчета'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 15, 16, Истина, "РегистрыСведений",        НСтр("ka='ცნობების რეგისტრები';ru='Регистры сведений'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 17, 18, Истина, "РегистрыНакопления",      НСтр("ka='დაგროვების რეგისტრები';ru='Регистры накопления'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 19, 20, Истина, "РегистрыБухгалтерии",     НСтр("ka='ბუღალტერიის რეგისტრები';ru='Регистры бухгалтерии'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 21, 22, Истина, "РегистрыРасчета",         НСтр("ka='ანგარიშსწორების რეგისტრები';ru='Регистры расчета'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 23, 24, Истина, "БизнесПроцессы",          НСтр("ka='ბიზნეს-პროცესები';ru='Бизнес-процессы'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 25, 26, Истина, "Задачи",                  НСтр("ka='ამოცანები';ru='Задачи'"));
...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Метаданные.РегистрыНакопления.Содержит(Мета)
	 ...
	 Или Метаданные.Последовательности.Содержит(Мета) Тогда
		...
		Строка.Заголовок   = НСтр("ka='რეგისტრატორი';ru='Регистратор'");
	КонецЕсли;
	...
	Если ЭтоРегистрСведений И Мета.ОсновнойОтборПоПериоду Тогда
		...
		Строка.Заголовок   = НСтр("ka='პერიოდი';ru='Период'");
	КонецЕсли;
	...
	Если Метаданные.РегистрыРасчета.Содержит(Мета.Родитель()) Тогда
		...
		Строка.ТипЗначения = Документы.ТипВсеСсылки();
		Строка.Заголовок   = НСтр("ka='Объект перерасчета';ru='Объект перерасчета'");
	КонецЕсли;
...
Функция ПредставлениеСсылки(ОбъектПредставления) Экспорт
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = НСтр("ka='не задано';ru='не задано'");
	КонецЕсли;
...
Функция ПроверитьКорректностьНастроек(КлючНастройки = "") Экспорт
		...
		Попытка
			...
			Иначе
				Если ЭтоФайловаяБаза() Тогда
					Текст = НСтр("ka='Файл ""%1"" не доступен';ru='Файл ""%1"" не доступен'");
				Иначе
					Текст = НСтр("ka='Файл ""%1"" не доступен на сервере';ru='Файл ""%1"" не доступен на сервере'");
				КонецЕсли;
	...
	Иначе
		...
		Если Метаданные.Обработки.Найти(НастройкаАдресВнешнейОбработкиЗапросов) = Неопределено Тогда
			Текст = НСтр("ka='Обработка ""%1"" не найдена в составе конфигурации';ru='Обработка ""%1"" не найдена в составе конфигурации'");
			Результат.НастройкаАдресВнешнейОбработкиЗапросов = СтрЗаменить(Текст, "%1", НастройкаАдресВнешнейОбработкиЗапросов);
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Инфо.Вставить("Наименование", НСтр("ka='Регистрация изменений для обмена данными';ru='Регистрация изменений для обмена данными'"));
	...
	Команда.Представление = НСтр("ka='Редактирование регистрации изменений объекта';ru='Редактирование регистрации изменений объекта'");
