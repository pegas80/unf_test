...
// Параметры:
	...
	ТекНомерСтроки = 1;
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 1,  2,  Ложь,   "Константы",               НСтр("ka='კონსტანტები';ru='Константы';en='constants'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 3,  4,  Истина, "Справочники",             НСтр("ka='ცნობარები';ru='Справочники';en='References'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 5,  6,  Истина, "Последовательности",      НСтр("ka='Последовательности';ru='Последовательности';en='sequences'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 7,  8,  Истина, "Документы",               НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 9,  10, Истина, "ПланыВидовХарактеристик", НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик';en='Plans species'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 11, 12, Истина, "ПланыСчетов",             НСтр("ka='ანგარიშეგების გეგმები';ru='Планы счетов';en='Charts of accounts'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 13, 14, Истина, "ПланыВидовРасчета",       НСтр("ka='ანგარიშსწორებების ტიპების გეგმები';ru='Планы видов расчета';en='Plans of calculation types'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 15, 16, Истина, "РегистрыСведений",        НСтр("ka='ცნობების რეგისტრები';ru='Регистры сведений';en='registers information'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 17, 18, Истина, "РегистрыНакопления",      НСтр("ka='დაგროვების რეგისტრები';ru='Регистры накопления';en='accumulation registers'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 19, 20, Истина, "РегистрыБухгалтерии",     НСтр("ka='ბუღალტერიის რეგისტრები';ru='Регистры бухгалтерии';en='accounting registers'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 21, 22, Истина, "РегистрыРасчета",         НСтр("ka='ანგარიშსწორების რეგისტრები';ru='Регистры расчета';en='registers calculation'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 23, 24, Истина, "БизнесПроцессы",          НСтр("ka='ბიზნეს-პროცესები';ru='Бизнес-процессы';en='Business processes'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 25, 26, Истина, "Задачи",                  НСтр("ka='ამოცანები';ru='Задачи';en='Tasks'"));
...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Метаданные.РегистрыНакопления.Содержит(Мета)
	 ...
	 Или Метаданные.Последовательности.Содержит(Мета) Тогда
		...
		Строка.Заголовок   = НСтр("ka='რეგისტრატორი';ru='Регистратор';en='recorder'");
	КонецЕсли;
	...
	Если ЭтоРегистрСведений И Мета.ОсновнойОтборПоПериоду Тогда
		...
		Строка.Заголовок   = НСтр("ka='პერიოდი';ru='Период';en='Period'");
	КонецЕсли;
	...
	Если Метаданные.РегистрыРасчета.Содержит(Мета.Родитель()) Тогда
		...
		Строка.ТипЗначения = Документы.ТипВсеСсылки();
		Строка.Заголовок   = НСтр("ka='Объект перерасчета';ru='Объект перерасчета';en='object allocation'");
	КонецЕсли;
...
Функция ПредставлениеСсылки(ОбъектПредставления) Экспорт
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = НСтр("ka='не задано';ru='не задано';en='not specified'");
	КонецЕсли;
...
Функция ПроверитьКорректностьНастроек(КлючНастройки = "") Экспорт
		...
		Попытка
			...
			Иначе
				Если ЭтоФайловаяБаза() Тогда
					Текст = НСтр("ka='Файл ""%1"" не доступен';ru='Файл ""%1"" не доступен';en='File ""%1"" is not available'");
				Иначе
					Текст = НСтр("ka='Файл ""%1"" не доступен на сервере';ru='Файл ""%1"" не доступен на сервере';en='File ""%1"" is not available on the server'");
				КонецЕсли;
	...
	Иначе
		...
		Если Метаданные.Обработки.Найти(НастройкаАдресВнешнейОбработкиЗапросов) = Неопределено Тогда
			Текст = НСтр("ka='Обработка ""%1"" не найдена в составе конфигурации';ru='Обработка ""%1"" не найдена в составе конфигурации';en='Processing ""%1"" is not found as part of the configuration'");
			Результат.НастройкаАдресВнешнейОбработкиЗапросов = СтрЗаменить(Текст, "%1", НастройкаАдресВнешнейОбработкиЗапросов);
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Инфо.Вставить("Наименование", НСтр("ka='Регистрация изменений для обмена данными';ru='Регистрация изменений для обмена данными';en='Registration of changes for data exchange'"));
	Инфо.Вставить("Версия",       "1.0");
	Инфо.Вставить("ВерсияБСП",    "1.2.1.4");
	Инфо.Вставить("Информация",    НСтр("ka='Обработка для управления регистрацией объектов на узлах обмена до формирования выгрузки. При работе в составе конфигурации с БСП версии 2.1.2.0 и старше производит контроль ограничений миграции данных для узлов обмена.';ru='Обработка для управления регистрацией объектов на узлах обмена до формирования выгрузки. При работе в составе конфигурации с БСП версии 2.1.2.0 и старше производит контроль ограничений миграции данных для узлов обмена.';en='Treatment to control the registration of objects in the exchange nodes to the formation of discharge. When operating as a part of the configuration with the BSP version 2.1.2.0 and older produces control limits for the exchange of data migration nodes.'"));
	...
	Команда.Представление = НСтр("ka='Редактирование регистрации изменений объекта';ru='Редактирование регистрации изменений объекта';en='Editing object registration changes'");
