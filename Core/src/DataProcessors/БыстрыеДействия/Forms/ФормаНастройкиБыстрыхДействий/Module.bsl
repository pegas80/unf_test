...
Процедура Подключаемый_БыстроеДействиеНажатие(Элемент)
	...
	Если НЕ ТекущаяСтрока.Выводить И КоличествоВыводимыхКнопок()>=7 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ერთდროულად შესაძლებელია არაუმეტეს 7 სწრაფი ღილაკის გამოტანა';ru='Одновременно можно вывести не более 7 кнопок быстрых действий';en='At the same time, you can withdraw a maximum of 7 quick action buttons'"), 0);
		Возврат;
	КонецЕсли;
