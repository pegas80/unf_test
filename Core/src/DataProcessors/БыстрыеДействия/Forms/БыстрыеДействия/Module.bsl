...
Процедура Инициализация(Отказ)
	...
	Если ПолучитьФункциональнуюОпцию("УчетРозничныхПродаж") И ДоступныПродажи Тогда
		ДобавитьБыстроеДействие("РабочееМестоКассира", НСтр("ka='მსა';ru='РМК';en='RMK'"), "БыстроеДействиеОткрытьРМК");
	КонецЕсли;
	Если ДоступныПродажи Тогда
		ДобавитьБыстроеДействие("ДобавитьЗаказПокупателя", НСтр("ka='შეკვეთის გაფორმება';ru='Оформить заказ';en='Checkout'"), "БыстроеДействиеСоздатьЗаказПокупателя");
		ДобавитьБыстроеДействие("ДобавитьСчетНаОплату", НСтр("ka='ანგარიშის გამოწერა';ru='Выписать счет';en='to issue an invoice'"), "БыстроеДействиеСоздатьСчетНаОплату");
		ДобавитьБыстроеДействие("ДобавитьРасходнуюНакладную", НСтр("ka='გაყიდვა';ru='Продать';en='sell'"), "БыстроеДействиеСоздатьРасходнуюНакладную");
	КонецЕсли;
	Если ДоступныЗакупки Тогда
		ДобавитьБыстроеДействие("ДобавитьПриходнуюНакладную", НСтр("ka='ყიდვა';ru='Купить';en='Buy'"), "БыстроеДействиеСоздатьПриходнуюНакладную");
	КонецЕсли;
	Если ДоступнаКасса Тогда
		ДобавитьБыстроеДействие("ДобавитьПоступлениеВКассу", НСтр("ka='გადახდის მიღება';ru='Принять оплату';en='accept payment'"), "БыстроеДействиеСоздатьПоступлениеВКассу");
		ДобавитьБыстроеДействие("ДобавитьРасходИзКассы", НСтр("ka='გადავიხადოთ';ru='Оплатить';en='pay'"), "БыстроеДействиеСоздатьРасходИзКассы");
	КонецЕсли;
	Если ДоступныПродажи Тогда
		ДобавитьБыстроеДействие("СписокЗаказовПокупателя", НСтр("ka='შეკვეთები';ru='Заказы';en='orders'"), "БыстроеДействиеСписокЗаказовПокупателей");
		ДобавитьБыстроеДействие("СписокРасходныхНакладных", НСтр("ka='გაყიდვები';ru='Продажи';en='the sale'"), "БыстроеДействиеСписокРасходныхНакладных");
	КонецЕсли; 
	Если ДоступнаКасса Тогда
		ДобавитьБыстроеДействие("СписокКассовыхДокументов", НСтр("ka='გადახდები';ru='Оплаты';en='payment'"), "БыстроеДействиеЖурналКассовыхДокументов");
	КонецЕсли;
	Если ДоступныЗакупки Тогда
		ДобавитьБыстроеДействие("СписокПриходныхНакладных", НСтр("ka='შესყიდვები';ru='Закупки';en='purchases'"), "БыстроеДействиеСписокПриходныхНакладных");
	КонецЕсли; 
	Если ДоступноДобавлениеТоваров Тогда
		ДобавитьБыстроеДействие("СписокНоменклатуры", НСтр("ka='საქონელი და მომსახურება';ru='Товары и услуги';en='Products and service'"), "БыстроеДействиеСписокНоменклатуры");
	КонецЕсли;
	Если ДоступныПродажи Тогда
		ДобавитьБыстроеДействие("СписокПокупателей", НСтр("ka='მყიდველები';ru='Покупатели';en='Buyers'"), "БыстроеДействиеСписокПокупателей");
	КонецЕсли;
	Если ДоступныЗакупки Тогда
		ДобавитьБыстроеДействие("СписокПоставщиков", НСтр("ka='მომწოდებლები';ru='Поставщики';en='Suppliers'"), "БыстроеДействиеСписокПоставщиков");
	КонецЕсли; 
...
Процедура СоздатьЭлементыБыстрыеДействия()
	...
	Элемент.Подсказка = НСтр("ka='პარამეტრები';ru='Настройка';en='Settings'");
