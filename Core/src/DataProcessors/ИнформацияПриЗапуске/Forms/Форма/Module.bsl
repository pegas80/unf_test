...
Функция ПодготовитьДанныеФормы()
	...
	Для Каждого ПакетСтраниц Из ПодготовленныеПакеты Цикл
		ПакетСтраниц.ЗаголовокФормы = НСтр("ka='ინფორმაცია';ru='Информация'");
		...
		Если СтрНачинаетсяС(ПакетСтраниц.Раздел, "_") Тогда
			...
			Иначе
				...
				Если ТекущееПодменю = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не найдена группа ""%1""';ru='Не найдена группа ""%1""'"), ИмяПодменю);
				КонецЕсли;
		...
		ИначеЕсли НаименованиеТекущегоРаздела <> ПакетСтраниц.Раздел Тогда
			...
			ЭтоГлавная = (ПакетСтраниц.Раздел = НСтр("ka='Главная';ru='Главная'"));
