...
Процедура ЗапуститьФоновоеЗаданиеНаСервере()
	...
	НаименованиеЗадания = НСтр("ka='ნახლდება ხელმძღვანელის მონიტორის ვიჯეტები';ru='Обновление виджетов монитора руководителя'");
...
Процедура ОбновитьОтображениеВиджетов()
	...
	ФорматДатыПодсказки = НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ=dd.MM.yyyy";
	ФорматДатыТочки = НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + "ДФ=d.MM";
	ФорматСуммы = НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + " ЧДЦ=2; ЧРД=,; ЧН=0,00";
	ФорматКоличества = НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + " ЧДЦ=; ЧН=0";
...
Процедура ОбновитьВиджетПродажи(ДанныеПродажи, НастройкаВиджета)
	...
	Если Выборка.Следующий() Тогда
		Элементы.ДанныеПродажи.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='გაყიდვები';ru='Продажи'"), Выборка.Выручка, , Выборка.ПредставлениеВалютыУчета);
		Элементы.ДанныеПрибыль.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='მოგება';ru='Прибыль'"), Выборка.Прибыль, Истина, Выборка.ПредставлениеВалютыУчета);
	Иначе
		Элементы.ДанныеПродажи.Заголовок = "";
		Элементы.ДанныеПрибыль.Заголовок = "";
	КонецЕсли;
	Элементы.ДанныеРейтингТоваров.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='საქონლის რეიტინგი';ru='Рейтинг товаров'"),
		НастройкаВиджета.ПервыеТовары, , НСтр("ka='საუკეთესოები';ru='лучших'"));
	Элементы.ДанныеРейтингМенеджеров.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='მენეჯერების რეიტინგი';ru='Рейтинг менеджеров'"),
		НастройкаВиджета.ПервыеМенеджеры, , НСтр("ka='საუკეთესოები';ru='лучших'"));
	...
	СерияСебестоимость = ДиаграммаПродажи.Серии.Добавить(НСтр("ka='თვითღირებულება';ru='Себестоимость'"));
	СерияСебестоимость.Маркер = ТипМаркераДиаграммы.Круг;
	СерияСебестоимость.Индикатор = Истина;
	СерияВаловаяПрибыль = ДиаграммаПродажи.Серии.Добавить(НСтр("ka='საერთო მოგება';ru='Валовая прибыль'"));
	СерияВаловаяПрибыль.Маркер = ТипМаркераДиаграммы.Круг;
	СерияВыручка = ДиаграммаПродажи.Серии.Добавить(НСтр("ka='შემოსავალი';ru='Выручка'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მოგება: %1 на %2';ru='Выручка: %1 на %2'"),
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='თვითღირებულება: %1 на %2';ru='Себестоимость: %1 на %2'"),
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='საერთო მოგება: %1 на %2';ru='Валовая прибыль: %1 на %2'"),
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		Если НЕ ЕстьДанныеДляОтображения Тогда
			...
			Если ДиаграммаРейтингТоваров.Серии.Количество() = 0 Тогда
				...
				Серия.Текст = НСтр("ka='საუკეთესო საქონელი';ru='Лучший товар'");
			Иначе
		...
		Пока ВыборкаНоменклатура.Следующий() Цикл
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მოგება: %1 ნომენკლატურისთვის: ""%2""   %3';ru='Выручка: %1 за номенклатуру: ""%2"" на %3'"),
				Формат(Выручка, ФорматСуммы),
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		Если НЕ ЕстьДанныеДляОтображения Тогда
			...
			Если ДиаграммаРейтингМенеджеров.Серии.Количество() = 0 Тогда
				...
				Серия.Текст = НСтр("ka='საუკეთესო მენეჯერი';ru='Лучший менеджер'");
			Иначе
		...
		Пока ВыборкаМенеджеры.Следующий() Цикл
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მოგება: %1 по менеджеру: %2, на %3';ru='Выручка: %1 по менеджеру: %2, на %3'"),
				Формат(Выручка, ФорматСуммы),
	...
	СерияРасходы = ДиаграммаПрибыль.Серии.Добавить(НСтр("ka='დანახარჯები';ru='Расходы'"));
	СерияРасходы.Маркер = ТипМаркераДиаграммы.Круг;
	СерияРасходы.Индикатор = Истина;
	СерияПрибыль = ДиаграммаПрибыль.Серии.Добавить(НСтр("ka='მოგება';ru='Прибыль'"));
	СерияПрибыль.Маркер = ТипМаркераДиаграммы.Круг;
	СерияДоходы = ДиаграммаПрибыль.Серии.Добавить(НСтр("ka='შემოსავლები';ru='Доходы'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მოგება: %1 на %2';ru='Доход: %1 на %2'"),
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დანახარჯი: %1 на %2';ru='Расход: %1 на %2'"),
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შემოსავალი: %1 на %2';ru='Прибыль: %1 на %2'"),
...
Процедура ОбновитьВиджетДенежныеСредства(ДанныеДенежныхСредств, НастройкаВиджета)
	...
	Если Выборка.Следующий() Тогда
		Элементы.ДанныеОстаткиДС.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='ფულის ნაშთი';ru='Остаток денег'"), Выборка.ОстатокДенег, , Выборка.ПредставлениеВалютыУчета);
		Элементы.ДанныеНаличные.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='ნაღდი ფული';ru='Наличные'"), Выборка.Наличные, , Выборка.ПредставлениеВалютыУчета);
		Элементы.ДанныеБезналичные.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='უნაღდო';ru='Безналичные'"), Выборка.Безналичные, , Выборка.ПредставлениеВалютыУчета);
		Элементы.ДанныеДенежныйПоток.Заголовок = СформироватьЗаголовокВарианта(НСтр("ka='ფულის ნაკადი';ru='Денежный поток'"), Выборка.ДенежныйПоток, Истина, Выборка.ПредставлениеВалютыУчета);
	Иначе
	...
	Серия = ДиаграммаОстатокДенег.Серии.Добавить(НСтр("ka='ნაშთი';ru='Остаток'"));
	Серия.Маркер = ТипМаркераДиаграммы.Круг;
	...
	Пока Выборка.Следующий() Цикл
		...
		Точка.Текст = Формат(Период, ФорматДатыТочки);
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფულის ნაშთი: %1 на %2';ru='Остаток денег: %1 на %2'"),
	...
	Серия = ДиаграммаНаличные.Серии.Добавить(НСтр("ka='ნაშთი';ru='Остаток'"));
	Серия.Маркер = ТипМаркераДиаграммы.Круг;
	...
	Пока Выборка.Следующий() Цикл
		...
		Точка.Текст = Формат(Период, ФорматДатыТочки);
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ნაღდი: %1 на %2';ru='Наличные: %1 на %2'"),
	...
	Серия = ДиаграммаБезналичные.Серии.Добавить(НСтр("ka='ნაშთი';ru='Остаток'"));
	Серия.Маркер = ТипМаркераДиаграммы.Круг;
	...
	Пока Выборка.Следующий() Цикл
		...
		Точка.Текст = Формат(Период, ФорматДатыТочки);
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='უნაღდო: %1 на %2';ru='Безналичные: %1 на %2'"),
	...
	Серия = ДиаграммаДенежныйПоток.Серии.Добавить(НСтр("ka='ნაშთი';ru='Остаток'"));
	Серия.Маркер = ТипМаркераДиаграммы.Круг;
	...
	Пока Выборка.Следующий() Цикл
		...
		Точка.Текст = Формат(Период, ФорматДатыТочки);
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფულადი ნაკადი: %1 на %2';ru='Денежный поток: %1 на %2'"),
...
Процедура ОбновитьВиджетДолги(ДанныеВзаиморасчетов, НастройкаВиджета)
	...
	Если Выборка.Следующий() Тогда
		Элементы.ДанныеДолгПокупателей.Заголовок =	СформироватьЗаголовокВарианта(НСтр("ka='მყიდველთა დავალიანება';ru='Долг покупателей'"),
			Выборка.ДолгПокупателей, , Выборка.ПредставлениеВалютыУчета);
		Элементы.ДанныеДолгПоставщикам.Заголовок =	СформироватьЗаголовокВарианта(НСтр("ka='დავალიანება მომწოდებლებთან';ru='Долг поставщикам'"),
	...
	Точка.Текст = НСтр("ka='დავალიანება';ru='Долг'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КлючиСерий.Количество() < МаксСерийДиаграммы Тогда
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დავალიანება: %1 от покупателя: %2';ru='Задолженность: %1 от покупателя: %2'"),
		...
		Иначе
			...
			СводнаяСерия.Текст = НСтр("ka='სხვები';ru='Прочие'");
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='სხვა მყიდველთა დავალიანება: %1';ru='Задолженность прочих покупателей: %1'"),
	...
	Серия = ДиаграммаДолгПокупателей.Серии.Добавить(НСтр("ka='დავალიანება';ru='Задолженность'"));
	Серия.Маркер = ТипМаркераДиаграммы.Круг;
	...
	Пока Выборка.Следующий() Цикл
		...
		Точка.Текст = Формат(Период, ФорматДатыТочки);
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='საერთო დავალიანება: %1 на %2';ru='Общая задолженность: %1 на %2'"),
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КлючиСерий.Количество() < МаксСерийДиаграммы Тогда
			...
			Серия.Раздвигать = Истина;
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დავალიანება: %1 მომწოდებელთან: %2';ru='Задолженность: %1 поставщику: %2'"),
		...
		Иначе
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დავალიანება სხვა მომწოდებლებთან: %1';ru='Задолженность прочим поставщикам: %1'"),
				Формат(НовоеЗначение, ФорматСуммы),
				);
	...
	Серия = ДиаграммаДолгПоставщикам.Серии.Добавить(НСтр("ka='დავალიანება';ru='Задолженность'"));
	Серия.Маркер = ТипМаркераДиаграммы.Круг;
...
Функция СформироватьПредставлениеПериода(ДатаОкончания, ВидПериода, Интервал)
	...
	ФорматнаяСтрока = НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + " ДФ='d MMMM'";
	
	Если Интервал = Перечисления.Периодичность.День Тогда
		ПредставлениеИнтервала = НСтр("ka='დღეებით';ru='по дням'");
	ИначеЕсли Интервал = Перечисления.Периодичность.Неделя Тогда
		ПредставлениеИнтервала = НСтр("ka='კვირებით';ru='по неделям'");
	ИначеЕсли Интервал = Перечисления.Периодичность.Декада Тогда
		ПредставлениеИнтервала = НСтр("ka='დეკადებით';ru='по декадам'");
	ИначеЕсли Интервал = Перечисления.Периодичность.Месяц Тогда
		ПредставлениеИнтервала = НСтр("ka='თვიურად';ru='помесячно'");
	ИначеЕсли Интервал = Перечисления.Периодичность.Квартал Тогда
		ПредставлениеИнтервала = НСтр("ka='კვარტალურად';ru='поквартально'");
	ИначеЕсли Интервал = Перечисления.Периодичность.Полугодие Тогда
		ПредставлениеИнтервала = НСтр("ka='ნახევარწლედად';ru='по полугодиям'");
	ИначеЕсли Интервал = Перечисления.Периодичность.Год Тогда
		ПредставлениеИнтервала = НСтр("ka='კვირეებად';ru='по неделям'");
	Иначе
...
Процедура ИнициализироватьДиаграмму(Диаграмма, ВидДиаграммы = Неопределено)
	...
	Диаграмма.ФорматЗначенийВПодписях	= НСтр("ka='Л=ka_GE;';ru='Л=ru_RU;'") + " ЧДЦ=; ЧН=' '";
	...
	Диаграмма.ОбластьЗаголовка.Текст			= НСтр("ka='არ არის მონაცემები';ru='Нет данных для отображения'");
