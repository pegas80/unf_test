...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ka='Правила регистрации объектов этой информационной базы (%1) от %2';ru='Правила регистрации объектов этой информационной базы (%1) от %2'");
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("ka='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';ru='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2'");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(0, НСтр("ka='Внутренняя ошибка';ru='Внутренняя ошибка'", КодОсновногоЯзыка));
	Сообщения.Вставить(1, НСтр("ka='Ошибка открытия файла правил';ru='Ошибка открытия файла правил'", КодОсновногоЯзыка));
	Сообщения.Вставить(2, НСтр("ka='Ошибка при загрузке правил';ru='Ошибка при загрузке правил'", КодОсновногоЯзыка));
	Сообщения.Вставить(3, НСтр("ka='Ошибка формата правил';ru='Ошибка формата правил'", КодОсновногоЯзыка));
	Сообщения.Вставить(4, НСтр("ka='Ошибка при получении файла правил для чтения';ru='Ошибка при получении файла правил для чтения'", КодОсновногоЯзыка));
	Сообщения.Вставить(5, НСтр("ka='Загружаемые правила регистрации не предназначены для текущего плана обмена.';ru='Загружаемые правила регистрации не предназначены для текущего плана обмена.'", КодОсновногоЯзыка));
