...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ka='Правила регистрации объектов этой информационной базы (%1) от %2';ru='Правила регистрации объектов этой информационной базы (%1) от %2';en='Rules of registration of objects of the information base (1%) of 2%'");
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("ka='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';ru='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';en='The rules of registration of the exchange plan 1%, and the download is performed to exchange 2% of the plan'");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(0, НСтр("ka='Внутренняя ошибка';ru='Внутренняя ошибка';en='Internal error'", КодОсновногоЯзыка));
	Сообщения.Вставить(1, НСтр("ka='Ошибка открытия файла правил';ru='Ошибка открытия файла правил';en='Error opening file rules'", КодОсновногоЯзыка));
	Сообщения.Вставить(2, НСтр("ka='Ошибка при загрузке правил';ru='Ошибка при загрузке правил';en='Error loading rules'", КодОсновногоЯзыка));
	Сообщения.Вставить(3, НСтр("ka='Ошибка формата правил';ru='Ошибка формата правил';en='Error Rule format'", КодОсновногоЯзыка));
	Сообщения.Вставить(4, НСтр("ka='Ошибка при получении файла правил для чтения';ru='Ошибка при получении файла правил для чтения';en='Error retrieving file permissions to read'", КодОсновногоЯзыка));
	Сообщения.Вставить(5, НСтр("ka='Загружаемые правила регистрации не предназначены для текущего плана обмена.';ru='Загружаемые правила регистрации не предназначены для текущего плана обмена.';en='Downloadable registration rules are not intended for current sharing plan.'", КодОсновногоЯзыка));
