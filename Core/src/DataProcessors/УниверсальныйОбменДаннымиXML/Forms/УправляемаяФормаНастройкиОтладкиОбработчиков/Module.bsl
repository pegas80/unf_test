...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ka='Настройка отладки обработчиков при %Событие% данных';ru='Настройка отладки обработчиков при %Событие% данных'");	
	Событие = ?(Параметры.ОбработчикиСобытийЧитаемИзФайлаПравилОбмена, НСтр("ka='выгрузке';ru='выгрузке'"), НСтр("ka='загрузке';ru='загрузке'"));
	ЗаголовокФормы = СтрЗаменить(ЗаголовокФормы, "%Событие%", Событие);
	Заголовок = ЗаголовокФормы;
	
	ЗаголовокКнопки = НСтр("ka='Сформировать модуль отладки %Событие%';ru='Сформировать модуль отладки %Событие%'");
	Событие = ?(Параметры.ОбработчикиСобытийЧитаемИзФайлаПравилОбмена, НСтр("ka='выгрузки';ru='выгрузки'"), НСтр("ka='загрузки';ru='загрузки'"));
...
Процедура ИмяФайлаВнешнейОбработкиОбработчиковСобытийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Фильтр     = НСтр("ka='Файл внешней обработки обработчиков событий (*.epf)|*.epf';ru='Файл внешней обработки обработчиков событий (*.epf)|*.epf'");
...
Процедура Готово(Команда)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаВнешнейОбработкиОбработчиковСобытий) Тогда
		...
		СообщитьПользователю(НСтр("ka='Укажите имя файла внешней обработки.';ru='Укажите имя файла внешней обработки.'"), "ИмяФайлаВнешнейОбработкиОбработчиковСобытий");
		Возврат;
	...
	Если Не ФайлВнешнейОбработкиОбработчиковСобытий.Существует() Тогда
		...
		СообщитьПользователю(НСтр("ka='Указанный файл внешней обработки не существует.';ru='Указанный файл внешней обработки не существует.'"), "ИмяФайлаВнешнейОбработкиОбработчиковСобытий");
		Возврат;
...
Процедура ВыгрузитьКодОбработчиков(Команда)
	...
	Если Не ПустаяСтрока(Объект.ИмяВременногоФайлаОбработчиковСобытий) Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='Выгрузить повторно';ru='Выгрузить повторно'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='Открыть модуль';ru='Открыть модуль'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Отмена);
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьКодОбработчиковЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Модуль отладки с кодом обработчиков уже выгружен.';ru='Модуль отладки с кодом обработчиков уже выгружен.'"), СписокКнопок,,КодВозвратаДиалога.Нет);
...
Процедура ПоказатьОбработчикиСобытийВОкне()
	...
	Если ОбработчикиСобытий <> Неопределено Тогда
		ОбработчикиСобытий.Показать(НСтр("ka='Модуль отладки обработчиков';ru='Модуль отладки обработчиков'"));
	КонецЕсли;
	
	
	ПротоколОбмена = ПротоколОбмена();
	Если ПротоколОбмена <> Неопределено Тогда
		ПротоколОбмена.Показать(НСтр("ka='Ошибки выгрузки модуля обработчиков';ru='Ошибки выгрузки модуля обработчиков'"));
	КонецЕсли;
