...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
		ВызватьИсключение НСтр("ka='Использование обработки в интерактивном режиме доступно только администратору.';ru='Использование обработки в интерактивном режиме доступно только администратору.'");
	КонецЕсли;
	...
	ЗаголовокФормы = НСтр("ka='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';ru='Универсальный обмен данными в формате XML (%ВерсияОбработки%)'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Выполнить загрузку данных из файла обмена?';ru='Выполнить загрузку данных из файла обмена?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ka='Выберите каталог информационной базы';ru='Выберите каталог информационной базы'");
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ИмяФайлаПравил) Или Не Файл.Существует() Тогда
		СообщитьПользователю(НСтр("ka='Не найден файл правил обмена';ru='Не найден файл правил обмена'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Загрузить правила обмена данными?';ru='Загрузить правила обмена данными?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("ka='გაცვლის ფაილი';ru='Файл обмена'"),, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			СообщитьПользователю(НСтр("ka='Не удалось прочитать файл обмена.';ru='Не удалось прочитать файл обмена.'"));
			ОчиститьДанныеОФайлеДляЗагрузкиДанных();
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Удалить выбранные данные в информационной базе?';ru='Удалить выбранные данные в информационной базе?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура УдалениеУдалитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(НСтр("ka='Выполняется удаление данных. Пожалуйста, подождите...';ru='Выполняется удаление данных. Пожалуйста, подождите...'"));
		УдалитьНаСервере();
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("ka='გაცვლის ფაილი';ru='Файл обмена'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';ru='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,НСтр("ka='Файл правил обмена';ru='Файл правил обмена'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='Выполняется чтение правил обмена. Пожалуйста, подождите...';ru='Выполняется чтение правил обмена. Пожалуйста, подождите...'"));
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	СтрокаДерева.Представление = НСтр("ka='ცნობარები';ru='Справочники'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ka='დოკუმენტები';ru='Документы'");
...
Процедура ВыборФайла(Элемент, ИмяСвойства, ПроверятьСуществование, Знач РасширениеПоУмолчанию = "xml",
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл'");
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("ka='მიერთება წარმატებით არის დაყენებული.';ru='Подключение успешно установлено.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ka='Не найден файл обмена на сервере.';ru='Не найден файл обмена на сервере.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуИзФормыЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს მონაცემების ჩატვირთვა. გთხოვთ მოიცადოთ...';ru='Выполняется загрузка данных. Пожалуйста, подождите...'"));
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("ka='Не указан файл правил для загрузки данных.';ru='Не указан файл правил для загрузки данных.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='Универсальный обмен данными в формате XML';ru='Универсальный обмен данными в формате XML'", ОбъектДляСервера.КодОсновногоЯзыка()),
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() И ПроверятьСуществование Тогда
			...
			СообщитьПользователю(НСтр("ka='Указанный файл не существует.';ru='Указанный файл не существует.'"));
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Состояние(НСтр("ka='Выполняется выгрузка данных. Пожалуйста, подождите...';ru='Выполняется выгрузка данных. Пожалуйста, подождите...'"));
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("ka='Файл выгрузки.zip';ru='Файл выгрузки.zip'"),НСтр("ka='Файл выгрузки.xml';ru='Файл выгрузки.xml'"));
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			СообщитьПользователю(НСтр("ka='Не определен файл данных';ru='Не определен файл данных'"));
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ka='Файл внешней обработки отладчиков событий не существует на сервере';ru='Файл внешней обработки отладчиков событий не существует на сервере'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			СообщитьПользователю(НСтр("ka='Не удалось выгрузить обработчики событий';ru='Не удалось выгрузить обработчики событий'"));
			Возврат "";
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("ka='Режим работы (на клиенте)';ru='Режим работы (на клиенте)'"), НСтр("ka='Режим работы (на сервере)';ru='Режим работы (на сервере)'"));
...
Функция ПустоеЗначениеРеквизита(Реквизит, ПутьКДанным, Заголовок)
	...
	Если ПустаяСтрока(Реквизит) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Поле ""%1"" не заполнено'");
...
Процедура ПоместитьФайлПравилЗагрузкиВХранилище()
	...
	НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,
		?(ЧерезХранилище, НСтр("ka='გაცვლის ფაილი';ru='Файл обмена'"), ИмяФайлаПравилЗагрузки), ЧерезХранилище, УникальныйИдентификатор);
