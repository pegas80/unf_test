...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ka='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';ru='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';en='Universal data exchange format XML (% VersiyaObrabotki%)'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Выполнить загрузку данных из файла обмена?';ru='Выполнить загрузку данных из файла обмена?';en='Perform data downloading from file sharing?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ka='Выберите каталог информационной базы';ru='Выберите каталог информационной базы';en='Select the directory information base'");
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ИмяФайлаПравил) Или Не Файл.Существует() Тогда
		СообщитьПользователю(НСтр("ka='Не найден файл правил обмена';ru='Не найден файл правил обмена';en='exchange rules file is not found'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Загрузить правила обмена данными?';ru='Загрузить правила обмена данными?';en='Download the rules of data exchange?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("ka='გაცვლის ფაილი';ru='Файл обмена';en='file sharing'"),, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			СообщитьПользователю(НСтр("ka='Не удалось прочитать файл обмена.';ru='Не удалось прочитать файл обмена.';en='Unable to read file sharing.'"));
			ОчиститьДанныеОФайлеДляЗагрузкиДанных();
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Удалить выбранные данные в информационной базе?';ru='Удалить выбранные данные в информационной базе?';en='Remove the selected data in the information database?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура УдалениеУдалитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(НСтр("ka='Выполняется удаление данных. Пожалуйста, подождите...';ru='Выполняется удаление данных. Пожалуйста, подождите...';en='Data is deletion. Please wait...'"));
		УдалитьНаСервере();
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("ka='გაცვლის ფაილი';ru='Файл обмена';en='file sharing'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';ru='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';en='Direct connection to the database is only supported on the client running on Windows.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,НСтр("ka='Файл правил обмена';ru='Файл правил обмена';en='exchange rules file'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='Выполняется чтение правил обмена. Пожалуйста, подождите...';ru='Выполняется чтение правил обмена. Пожалуйста, подождите...';en='Reads the exchange rules. Please wait...'"));
...
"Выберите другой файл для выгрузки данных.';ka='Файл правил обмена не может совпадать с файлом данных."
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	СтрокаДерева.Представление = НСтр("ka='ცნობარები';ru='Справочники';en='References'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик';en='Plans species'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'");
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("ka='მიერთება წარმატებით არის დაყენებული.';ru='Подключение успешно установлено.';en='The connection is successfully established.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ka='Не найден файл обмена на сервере.';ru='Не найден файл обмена на сервере.';en='Unable to find a file sharing server.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуИзФормыЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მიმდინარეობს მონაცემების ჩატვირთვა. გთხოვთ მოიცადოთ...';ru='Выполняется загрузка данных. Пожалуйста, подождите...';en='Data is loading. Please wait...'"));
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("ka='Не указан файл правил для загрузки данных.';ru='Не указан файл правил для загрузки данных.';en='Unknown rules file for loading data.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='Универсальный обмен данными в формате XML';ru='Универсальный обмен данными в формате XML';en='Universal data exchange in XML format'", ОбъектДляСервера.КодОсновногоЯзыка()),
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() И ПроверятьСуществование Тогда
			...
			СообщитьПользователю(НСтр("ka='Указанный файл не существует.';ru='Указанный файл не существует.';en='The specified file does not exist.'"));
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Состояние(НСтр("ka='Выполняется выгрузка данных. Пожалуйста, подождите...';ru='Выполняется выгрузка данных. Пожалуйста, подождите...';en='Data is unloaded. Please wait...'"));
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("ka='Файл выгрузки.zip';ru='Файл выгрузки.zip';en='file vygruzki.zip'"),НСтр("ka='Файл выгрузки.xml';ru='Файл выгрузки.xml';en='vygruzki.xml file'"));
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			СообщитьПользователю(НСтр("ka='Не определен файл данных';ru='Не определен файл данных';en='Not Evaluated data file'"));
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ka='Файл внешней обработки отладчиков событий не существует на сервере';ru='Файл внешней обработки отладчиков событий не существует на сервере';en='external processing debuggers event file does not exist on the server'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			СообщитьПользователю(НСтр("ka='Не удалось выгрузить обработчики событий';ru='Не удалось выгрузить обработчики событий';en='Windows can not unload event handlers'"));
			Возврат "";
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("ka='Режим работы (на клиенте)';ru='Режим работы (на клиенте)';en='Mode of operation (on the client)'"), НСтр("ka='Режим работы (на сервере)';ru='Режим работы (на сервере)';en='Mode of operation (on the server)'"));
...
Функция ПустоеЗначениеРеквизита(Реквизит, ПутьКДанным, Заголовок)
	...
	Если ПустаяСтрока(Реквизит) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Поле ""%1"" не заполнено';en='The ""%1"" filled'");
...
Процедура ПоместитьФайлПравилЗагрузкиВХранилище()
	...
	НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,
		?(ЧерезХранилище, НСтр("ka='გაცვლის ფაილი';ru='Файл обмена';en='file sharing'"), ИмяФайлаПравилЗагрузки), ЧерезХранилище, УникальныйИдентификатор);
