...
Процедура ЗаписатьНастройкиАвторизацииДляСтандартногоИнтерфейсаOData(Знач НастройкиАвторизации) Экспорт
	...
	Если НастройкиАвторизации.Используется Тогда
		...
		ПользовательСтандартногоИнтерфейсаOData.Наименование = НСтр("ka='Автоматический REST-сервис';ru='Автоматический REST-сервис'");
...
"-------------------------------------------'"),
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(НСтр("ka='ЗаписьПользователя';ru='ЗаписьПользователя'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
Процедура ПроверитьСоставРолиOData() Экспорт
	...
	Если ИзбыточныеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ka='Следующие права избыточно включены в состав роли:';ru='Следующие права избыточно включены в состав роли:'") + Символы.ПС + Символы.ВК + 
			ПредставлениеИзбыточныхИлиНедостающихПрав(ИзбыточныеПрава, 2);
		Ошибки.Добавить(ТекстОшибки);
	КонецЕсли;
	
	Если НедостающиеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ka='Следующие права должны быть включены в состав роли:';ru='Следующие права должны быть включены в состав роли:'") + Символы.ПС + Символы.ВК + 
	...
	Если Ошибки.Количество() > 0 Тогда
		...
		ТекстИсключения = СтрШаблон(НСтр("ka='Обнаружены ошибки в составе прав роли %1:';ru='Обнаружены ошибки в составе прав роли %1:'"),
			РольДляСтандартногоИнтерфейсаOData().Имя);
...
Функция СвойстваПользователяСтандартногоИнтерфейсаOData()
	...
	Если Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		ВызватьИсключение НСтр("ka='Недостаточно прав доступа для настройки автоматического REST-сервиса';ru='Недостаточно прав доступа для настройки автоматического REST-сервиса'");
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации(Знач Суффикс)
	...
	Возврат НСтр("ka='НастройкаСтандартногоИнтерфейсаOData.';ru='НастройкаСтандартногоИнтерфейсаOData.'") + СокрЛП(Суффикс);
...
Процедура ПроверитьСоставРолиODataПриОбновлении() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Обработчик не должен использоваться при выключенном разделении по областям данных';ru='Обработчик не должен использоваться при выключенном разделении по областям данных'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьСозданияПользователяДляВызововСтандартногоИнтерфейсаOData()
	...
	Если КоличествоПользователей = 0 Тогда
		...
		ВызватьИсключение НСтр("ka='Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в программе отсутствуют другие пользователи.';ru='Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в программе отсутствуют другие пользователи.'");
