...
Процедура ЗаписатьНастройкиАвторизацииДляСтандартногоИнтерфейсаOData(Знач НастройкиАвторизации) Экспорт
	...
	Если НастройкиАвторизации.Используется Тогда
		...
		ПользовательСтандартногоИнтерфейсаOData.Наименование = НСтр("ka='Автоматический REST-сервис';ru='Автоматический REST-сервис';en='Automatic REST-service'");
		...
		Попытка
			...
			Комментарий = СтрШаблон(
				НСтр("ka='Выполнена запись пользователя для стандартного интерфейса OData."
""
...
"-------------------------------------------"
"%2"
"-------------------------------------------';ru='Выполнена запись пользователя для стандартного интерфейса OData."
""
"Описание пользователя ИБ:"
"-------------------------------------------"
"%1"
...
"-------------------------------------------'"),
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(НСтр("ka='ЗаписьПользователя';ru='ЗаписьПользователя';en='ZapisPolzovatelya'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
"-------------------------------------------"
"%2"
"-------------------------------------------';ka='При записи пользователя для стандартного интерфейса OData произошла ошибка."
""
"Описание пользователя ИБ:"
"-------------------------------------------"
"%1"
...
Процедура ПроверитьСоставРолиOData() Экспорт
	...
	Если ИзбыточныеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ka='Следующие права избыточно включены в состав роли:';ru='Следующие права избыточно включены в состав роли:';en='The following rights excessively included in the role:'") + Символы.ПС + Символы.ВК + 
			ПредставлениеИзбыточныхИлиНедостающихПрав(ИзбыточныеПрава, 2);
		Ошибки.Добавить(ТекстОшибки);
	КонецЕсли;
	
	Если НедостающиеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ka='Следующие права должны быть включены в состав роли:';ru='Следующие права должны быть включены в состав роли:';en='The following rights should be included in the role:'") + Символы.ПС + Символы.ВК + 
	...
	Если Ошибки.Количество() > 0 Тогда
		...
		ТекстИсключения = СтрШаблон(НСтр("ka='Обнаружены ошибки в составе прав роли %1:';ru='Обнаружены ошибки в составе прав роли %1:';en='Errors found in the part of the rights of the role of the 1%:'"),
			РольДляСтандартногоИнтерфейсаOData().Имя);
...
Функция СвойстваПользователяСтандартногоИнтерфейсаOData()
	...
	Если Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		ВызватьИсключение НСтр("ka='Недостаточно прав доступа для настройки автоматического REST-сервиса';ru='Недостаточно прав доступа для настройки автоматического REST-сервиса';en='It is not enough access rights to configure automatic REST-service'");
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации(Знач Суффикс)
	...
	Возврат НСтр("ka='НастройкаСтандартногоИнтерфейсаOData.';ru='НастройкаСтандартногоИнтерфейсаOData.';en='NastroykaStandartnogoInterfeysaOData.'") + СокрЛП(Суффикс);
...
Процедура ПроверитьСоставРолиODataПриОбновлении() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Обработчик не должен использоваться при выключенном разделении по областям данных';ru='Обработчик не должен использоваться при выключенном разделении по областям данных';en='The handler should not be used during the off division of data areas'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьСозданияПользователяДляВызововСтандартногоИнтерфейсаOData()
	...
	Если КоличествоПользователей = 0 Тогда
		...
		ВызватьИсключение НСтр("ka='Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в программе отсутствуют другие пользователи.';ru='Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в программе отсутствуют другие пользователи.';en='You can not create a separate username and password for automatic REST-service, as in the program there are no other users.'");
