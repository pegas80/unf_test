...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		ВызватьИсключение НСтр("ka='Недостаточно прав для настройки автоматического REST-сервиса';ru='Недостаточно прав для настройки автоматического REST-сервиса'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если СоздатьПользователяСтандартногоИнтерфейсаOData Тогда
		...
		Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано имя пользователя';ru='Не указано имя пользователя'"), , "ИмяПользователя");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Пароль) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан пароль';ru='Не указан пароль'"), , "Пароль");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПодтверждениеПароля) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано подтверждение пароля';ru='Не указано подтверждение пароля'"), , "ПодтверждениеПароля");
			Отказ = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Пароль) И ЗначениеЗаполнено(ПодтверждениеПароля) И Пароль <> ПодтверждениеПароля Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Подтверждение пароля не совпадает с паролем';ru='Подтверждение пароля не совпадает с паролем'"), , "ПодтверждениеПароля");
			Отказ = Истина;
		КонецЕсли;
	...
	Если Строки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не выбрано ни одного объекта, к которому разрешен доступ через автоматический REST-сервис';ru='Не выбрано ни одного объекта, к которому разрешен доступ через автоматический REST-сервис'"), , "ОбъектыМетаданных");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура СохранитьНаСервере()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='Пароль и подтверждение пароля не совпадают!';ru='Пароль и подтверждение пароля не совпадают!'");
