...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		ВызватьИсключение НСтр("ka='Недостаточно прав для настройки автоматического REST-сервиса';ru='Недостаточно прав для настройки автоматического REST-сервиса';en='Not have permission to configure automatic REST-service'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если СоздатьПользователяСтандартногоИнтерфейсаOData Тогда
		...
		Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано имя пользователя';ru='Не указано имя пользователя';en='Unknown username'"), , "ИмяПользователя");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Пароль) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан пароль';ru='Не указан пароль';en='Unknown password'"), , "Пароль");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПодтверждениеПароля) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано подтверждение пароля';ru='Не указано подтверждение пароля';en='Unknown password confirmation'"), , "ПодтверждениеПароля");
			Отказ = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Пароль) И ЗначениеЗаполнено(ПодтверждениеПароля) И Пароль <> ПодтверждениеПароля Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Подтверждение пароля не совпадает с паролем';ru='Подтверждение пароля не совпадает с паролем';en='Confirm password does not match password'"), , "ПодтверждениеПароля");
			Отказ = Истина;
		КонецЕсли;
	...
	Если Строки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не выбрано ни одного объекта, к которому разрешен доступ через автоматический REST-сервис';ru='Не выбрано ни одного объекта, к которому разрешен доступ через автоматический REST-сервис';en='You have no object to which access is allowed through the automatic REST-service'"), , "ОбъектыМетаданных");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?';en='The data have been changed. I save changes?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура СохранитьНаСервере()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='Пароль и подтверждение пароля не совпадают!';ru='Пароль и подтверждение пароля не совпадают!';en='Password and confirmation password do not match!'");
