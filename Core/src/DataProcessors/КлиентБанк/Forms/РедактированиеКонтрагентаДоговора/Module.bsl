...
Процедура КонтрагентГруппаПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(КонтрагентГруппа) Тогда
		КонтрагентГруппа = НСтр("ka='<<Авто выбор>>';ru='<<Авто выбор>>';en='<< >> Auto selection'");
	КонецЕсли;
...
Процедура КонтрольПараметровФормы(Отказ)
	...
	Если НЕ ЗаполненаСсылка И НЕ ЗаполненыРеквизиты Тогда
		...
		Сообщить(НСтр("ka='Не заданы обязательные параметры формы."
"Это программная ошибка и сведения о ней направлены администраторам';ru='Не заданы обязательные параметры формы."
"Это программная ошибка и сведения о ней направлены администраторам'"));
		
		ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка выписки банка.Форма выбора контрагента';ru='Загрузка выписки банка.Форма выбора контрагента';en='Loading statements banka.Forma contractor selection'"), 
			УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ka='Была попытка открытия формы заполнения контрагентов с полностью незаполненными параметрами';ru='Была попытка открытия формы заполнения контрагентов с полностью незаполненными параметрами';en='Was an attempt to open a form filling counterparties to completely empty parameters'"));
...
Процедура ИнициализироватьФорму()
	...
	ПредставлениеСклоненияСтрок = ПодобратьСловоПодЧисло(
		...
		НСтр("ka='строка';ru='строка';en='line'"),
		НСтр("ka='строки';ru='строки';en='lines'"),
		НСтр("ka='строк';ru='строк';en='lines'"));
	
	Если ПустаяСтрока(КонтрагентИНН) Тогда
		Элементы.ПрименитьДляВсехСтрок.Заголовок = НСтр("ka='Применить для всех строк без ИНН (';ru='Применить для всех строк без ИНН (';en='Apply all rows without INN ('") + Параметры.КоличествоСтрокКонтрагента + " " + ПредставлениеСклоненияСтрок +")";
		ПрименитьДляВсехСтрок = Ложь;
	Иначе
		Элементы.ПрименитьДляВсехСтрок.Заголовок = НСтр("ka='Применить для всех строк с таким же ИНН и р/счетом (';ru='Применить для всех строк с таким же ИНН и р/счетом (';en='Apply all rows with same TIN and p / account ('") + Параметры.КоличествоСтрокКонтрагента + " " + ПредставлениеСклоненияСтрок +")";
	...
	Если Параметры.НеВерныйФорматСуммы Тогда
		НадписьСуммаДокумента = НСтр("ka='Введите нужное число вместо строки ""%1"":';ru='Введите нужное число вместо строки ""%1"":';en='Enter the desired number instead of ""%1"" line:'");
	...
	Иначе
		...
		Элементы.НадписьСуммаДокумента.Заголовок = НСтр("ka='Сумма документа:';ru='Сумма документа:';en='The amount of the document:'");
		СуммаДокумента = Параметры.СуммаДокумента;
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюОДоговореНаСервере(Контрагент, Договор, ПараметрыДоговора)
	...
	ДоговорНаименованиеПраваяЧасть = НСтр("ka='%ДатаДоговора%-ის № %НомерДоговора%  (%ВалютаРасчетов%)';ru='№ %НомерДоговора% от %ДатаДоговора% (%ВалютаРасчетов%)';en='NomerDogovora №%% of% DataDogovora% (% ValyutaRaschetov%)'");
	...
	Если ТипЗнч(Договор) = Тип("Строка") Тогда
		Если СтрНайти(Договор, НСтр("ka='Не найден';ru='Не найден';en='Not found'")) > 0 Тогда
...
Процедура СформироватьИнформационныеНадписи()
	...
	Если Не ПустаяСтрока(КонтрагентИНН) ИЛИ Не ПустаяСтрока(КонтрагентКПП) Тогда
		ИНН_КПП = " ("+?(Не ПустаяСтрока(КонтрагентИНН), НСтр("ru='ИНН ';ka='ИНН '")+СокрЛП(КонтрагентИНН)+", ", "")+
			?(Не ПустаяСтрока(КонтрагентКПП), НСтр("ru='КПП ';ka='КПП '")+СокрЛП(КонтрагентКПП)+", ", "")+")";
	КонецЕсли;
	...
	Если ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С Или 
		...
		ОКонтрагентеЦентр = НСтр("ka='Создать нового при загрузке в 1С';ru='Создать нового при загрузке в 1С';en='Create a new boot 1C'");
	...
	ИначеЕсли ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоСейчас Тогда
		...
		ОКонтрагентеЦентр = НСтр("ka='Создать нового СЕЙЧАС';ru='Создать нового СЕЙЧАС';en='Create a new NOW'");
		ОКонтрагентеПраво = КонтрагентНаименование+ИНН_КПП;
	...
	ИначеЕсли ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента Тогда
		...
		ОКонтрагентеЦентр = НСтр("ka='НЕ создавать';ru='НЕ создавать';en='not create'");
		ОКонтрагентеПраво = КонтрагентНаименование+ИНН_КПП;
	...
	ИначеЕсли ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего Тогда
		...
		Если Не ПустаяСтрока(КонтрагентСсылка.ИНН) ИЛИ Не ПустаяСтрока(КонтрагентСсылка.КПП) Тогда
			ИНН_КПП = " ("+?(Не ПустаяСтрока(КонтрагентСсылка.ИНН), НСтр("ru='ИНН ';ka='ИНН '")+СокрЛП(КонтрагентСсылка.ИНН)+", ", "")+
				?(Не ПустаяСтрока(КонтрагентСсылка.КПП), НСтр("ru='КПП ';ka='КПП '")+СокрЛП(КонтрагентСсылка.КПП), "")+")";
		КонецЕсли;
		
		ОКонтрагентеЦентр = НСтр("ka='Использовать существующего';ru='Использовать существующего';en='use existing'");
	...
	Если ДействиеСоСчетом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С Тогда
		...
		ОРасчетномСчетеЦентр = НСтр("ka='Создать новый при загрузке в 1С';ru='Создать новый при загрузке в 1С';en='Create a new boot 1C'");
	...
	ИначеЕсли ДействиеСоСчетом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоСейчас Тогда
		...
		ОРасчетномСчетеЦентр = НСтр("ka='Создать новый СЕЙЧАС';ru='Создать новый СЕЙЧАС';en='Create a new NOW'");
		ОРасчетномСчетеПраво = СчетКонтрагентаНомер; //+?(ЗначениеЗаполнено(СчетБанк), ", банк - """+СокрЛП(СчетБанк)+"""", "");
		
	ИначеЕсли ДействиеСоСчетом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента Тогда
		
		ОРасчетномСчетеЦентр = НСтр("ka='НЕ создавать';ru='НЕ создавать';en='not create'");
		ОРасчетномСчетеПраво = СчетКонтрагентаНомер; //+?(ЗначениеЗаполнено(СчетБанк), ", банк - """+СокрЛП(СчетБанк)+"""", "");
		
	ИначеЕсли ДействиеСоСчетом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего Тогда
		
		ОРасчетномСчетеЦентр = НСтр("ka='Использовать существующий';ru='Использовать существующий';en='use existing'");
	...
	Если ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С Тогда
		...
		ОДоговореЦентр = НСтр("ka='Создать новый при загрузке в 1С';ru='Создать новый при загрузке в 1С';en='Create a new boot 1C'");
		ОДоговореПраво = НСтр("ka='Основной договор';ru='Основной договор';en='main contract'");
		
	ИначеЕсли ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоСейчас Тогда
		
		ОДоговореЦентр = НСтр("ka='Создать новый СЕЙЧАС';ru='Создать новый СЕЙЧАС';en='Create a new NOW'");
	...
	ИначеЕсли ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента Тогда
		...
		ОДоговореЦентр = НСтр("ka='НЕ создавать';ru='НЕ создавать';en='not create'");
		ОДоговореПраво = "";
		
	ИначеЕсли ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего Тогда
		
		ОДоговореЦентр = НСтр("ka='Использовать существующий';ru='Использовать существующий';en='use existing'");
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	// Контрагент.
	Если НЕ ПустаяСтрока(СоздаватьНового) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ka='Записать и выбрать';ru='Записать и выбрать';en='Record and select'");
	...
	ИначеЕсли НЕ ПустаяСтрока(СоздаватьНовогоПриЗагрузкеВыпискиВ1С) Или НЕ ПустаяСтрока(НеСоздавать) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ka='ОК';ru='ОК';en='OK'");
		Элементы.КонтрагентСсылка.АвтоОтметкаНезаполненного = Ложь;
		Элементы.КонтрагентСсылка.ОтметкаНезаполненного = Ложь;
	ИначеЕсли НЕ ПустаяСтрока(ИспользоватьСуществующего) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ka='არჩევა';ru='Выбрать';en='Select'");
...
Функция ОбработатьОКНаСервере()
	...
	//		
	//		ТекстСообщения = НСтр("ka='Необходимо выбрать кассу организации.';ru='Необходимо выбрать кассу организации.';en='You must select the cashier of the organization.'");
	...
	//		
	//		ТекстСообщения = НСтр("ka='Необходимо выбрать счет организации.';ru='Необходимо выбрать счет организации.';en='You must select the expense of the organization.'");
	//		ЕстьОшибкиЗаполнения = Истина;
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Попытка записи Контрагента не удалась по причине: %1';ru='Попытка записи Контрагента не удалась по причине: %1';en='Attempt to write Counterparty failed due to:% 1'"),
					ОписаниеОшибки()));
			Возврат Ложь;
		КонецПопытки;
	...
	ИначеЕсли Не ПустаяСтрока(ИспользоватьСуществующего) Тогда
		...
		Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Или Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Вначале укажите контрагента на закладке ""Контрагент""';ru='Вначале укажите контрагента на закладке ""Контрагент""';en='At first enter the counterparty on the ""Counterparty"" tab'"));
				Возврат Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьИННиКППИзВыписки(Команда)
	...
	Если КонтрагентСсылка.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Предварительно необходимо выбрать контрагента';ru='Предварительно необходимо выбрать контрагента';en='You must first select a contractor'"));
		Возврат;
	КонецЕсли;
	...
	Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) ИЛИ
		...
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) Тогда
			СписокВыбора.Добавить("ИНН", НСтр("ru='Обновить ИНН из виписки ';ka='Обновить ИНН из виписки '")+"("+КонтрагентИНН+")");
		Иначе
			СписокВыбора.Добавить("Пропустить", НСтр("ru='ИНН менять не требуется ';ka='ИНН менять не требуется '"));
		КонецЕсли;
		
		// КПП.
		Если СокрЛП(КонтрагентКПП) <> СокрЛП(Элементы.КонтрагентСсылкаКПП.ТекстРедактирования) Тогда
			СписокВыбора.Добавить("КПП", НСтр("ru='Обновить КПП из виписки ';ka='Обновить КПП из виписки '")+"("+КонтрагентКПП+")");
		Иначе
			СписокВыбора.Добавить("Пропустить", НСтр("ru='КПП менять не требуется ';ka='КПП менять не требуется '"));
		КонецЕсли;
		...
		Если СокрЛП(КонтрагентНаименованиеПолное) <> СокрЛП(КонтрагентСсылкаНаименованиеПолное) Тогда
			...
			СписокВыбора.Добавить("НаименованиеПолное", НСтр("ru='Обновить юр. название из выписки ';ka='Обновить юр. название из выписки '")+"("+НаименованиеДляПодменю+")");
		Иначе
			СписокВыбора.Добавить("Пропустить", НСтр("ru='Юр. название менять не требуется ';ka='Юр. название менять не требуется '"));
		КонецЕсли;
		...
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) И
			...
			СписокВыбора.Добавить("ИННКПП", НСтр("ka='Обновить ИНН и КПП из выписки';ru='Обновить ИНН и КПП из выписки';en='Refresh the TIN and CRR of the extract'"));
		КонецЕсли;
		
		// Все поля.
		СписокВыбора.Добавить("ВСЕ", НСтр("ka='Обновить все реквизиты';ru='Обновить все реквизиты';en='Update all details'"));
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ИНН, КПП и юридическое название контрагента и в выписке совпадают, заполнение не требуется';ru='ИНН, КПП и юридическое название контрагента и в выписке совпадают, заполнение не требуется';en='INN, CAT and legal name of the company and in the same statement, the filling is not required'"));
	КонецЕсли;
...
Функция НайтиИнформациюОДоговоре(ПараметрНазначениеПлатежа, СтрокаПоиска)
	...
	СтрокаПоиска = ВРег(НСтр("ka='по договору банковского счета №';ru='по договору банковского счета №';en='under the bank account number'"));
	ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. банковского счета №';ru='по дог. банковского счета №';en='on the dog. bank account number'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору банковского счета N';ru='по договору банковского счета N';en='under the bank account N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. банковского счета N';ru='по дог. банковского счета N';en='on the dog. bank account N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору №';ru='по договору №';en='by contract number'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. №';ru='по дог. №';en='on the dog. number'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору N';ru='по договору N';en='by contract N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. N';ru='по дог. N';en='on the dog. N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору';ru='по договору';en='under contract'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог.';ru='по дог.';en='on the dog.'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
...
Процедура ЗаполнитьДоговорПоДаннымНазначенияПлатежа(Команда)
		...
		Пока ТекСНомер > 0 И КолвоЦиклов <= 5 Цикл
			...
			Если ТекСПробел > 0 Тогда
				...
				Если ТекСПробел > 0 Тогда
					...
					Если СокрЛП(ВтороеСлово) <> НСтр("ka='(საიდან, ვისგან)';ru='от';en='From'") Тогда
	...
	Если СписокНомеровИДатДоговора.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='В назначении платежа не найдена информация о договоре. Установите номер и дату вручную.';ru='В назначении платежа не найдена информация о договоре. Установите номер и дату вручную.';en='The purpose of the payment can not be found information about the contract. Establish number and date manually.'"));
	Иначе
