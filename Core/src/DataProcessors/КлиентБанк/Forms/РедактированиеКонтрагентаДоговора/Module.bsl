...
Процедура КонтрагентГруппаПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(КонтрагентГруппа) Тогда
		КонтрагентГруппа = НСтр("ka='<<Авто выбор>>';ru='<<Авто выбор>>'");
	КонецЕсли;
...
"Это программная ошибка и сведения о ней направлены администраторам'"));
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Загрузка выписки банка.Форма выбора контрагента';ru='Загрузка выписки банка.Форма выбора контрагента'"), 
			УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ka='Была попытка открытия формы заполнения контрагентов с полностью незаполненными параметрами';ru='Была попытка открытия формы заполнения контрагентов с полностью незаполненными параметрами'"));
...
Процедура ИнициализироватьФорму()
	...
	ПредставлениеСклоненияСтрок = ПодобратьСловоПодЧисло(
		...
		НСтр("ka='строка';ru='строка'"),
		НСтр("ka='строки';ru='строки'"),
		НСтр("ka='строк';ru='строк'"));
	
	Если ПустаяСтрока(КонтрагентИНН) Тогда
		Элементы.ПрименитьДляВсехСтрок.Заголовок = НСтр("ka='Применить для всех строк без ИНН (';ru='Применить для всех строк без ИНН ('") + Параметры.КоличествоСтрокКонтрагента + " " + ПредставлениеСклоненияСтрок +")";
		ПрименитьДляВсехСтрок = Ложь;
	Иначе
		Элементы.ПрименитьДляВсехСтрок.Заголовок = НСтр("ka='Применить для всех строк с таким же ИНН и р/счетом (';ru='Применить для всех строк с таким же ИНН и р/счетом ('") + Параметры.КоличествоСтрокКонтрагента + " " + ПредставлениеСклоненияСтрок +")";
	...
	Если Параметры.НеВерныйФорматСуммы Тогда
		НадписьСуммаДокумента = НСтр("ka='Введите нужное число вместо строки ""%1"":';ru='Введите нужное число вместо строки ""%1"":'");
	...
	Иначе
		...
		Элементы.НадписьСуммаДокумента.Заголовок = НСтр("ka='Сумма документа:';ru='Сумма документа:'");
		СуммаДокумента = Параметры.СуммаДокумента;
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюОДоговореНаСервере(Контрагент, Договор, ПараметрыДоговора)
	...
	ДоговорНаименованиеПраваяЧасть = НСтр("ka='%ДатаДоговора%-ის № %НомерДоговора%  (%ВалютаРасчетов%)';ru='№ %НомерДоговора% от %ДатаДоговора% (%ВалютаРасчетов%)'");
	...
	Если ТипЗнч(Договор) = Тип("Строка") Тогда
		Если СтрНайти(Договор, НСтр("ka='Не найден';ru='Не найден'")) > 0 Тогда
...
Процедура СформироватьИнформационныеНадписи()
	...
	Если ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С Или 
		...
		ОКонтрагентеЦентр = НСтр("ka='Создать нового при загрузке в 1С';ru='Создать нового при загрузке в 1С'");
	...
	ИначеЕсли ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоСейчас Тогда
		...
		ОКонтрагентеЦентр = НСтр("ka='Создать нового СЕЙЧАС';ru='Создать нового СЕЙЧАС'");
		ОКонтрагентеПраво = КонтрагентНаименование+ИНН_КПП;
	...
	ИначеЕсли ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента Тогда
		...
		ОКонтрагентеЦентр = НСтр("ka='НЕ создавать';ru='НЕ создавать'");
		ОКонтрагентеПраво = КонтрагентНаименование+ИНН_КПП;
	...
	ИначеЕсли ДействиеСКонтрагентом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего Тогда
		...
		ОКонтрагентеЦентр = НСтр("ka='Использовать существующего';ru='Использовать существующего'");
	...
	Если ДействиеСоСчетом = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С Тогда
		...
		ОРасчетномСчетеЦентр = НСтр("ka='Создать новый при загрузке в 1С';ru='Создать новый при загрузке в 1С'");
	...
	Если ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоПриЗагрузкеВыпискиВ1С Тогда
		...
		ОДоговореЦентр = НСтр("ka='Создать новый при загрузке в 1С';ru='Создать новый при загрузке в 1С'");
		ОДоговореПраво = НСтр("ka='Основной договор';ru='Основной договор'");
		
	ИначеЕсли ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.СоздатьНовогоСейчас Тогда
		
		ОДоговореЦентр = НСтр("ka='Создать новый СЕЙЧАС';ru='Создать новый СЕЙЧАС'");
	...
	ИначеЕсли ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента Тогда
		...
		ОДоговореЦентр = НСтр("ka='НЕ создавать';ru='НЕ создавать'");
		ОДоговореПраво = "";
		
	ИначеЕсли ДействиеСДоговором = Перечисления.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.ИспользоватьСуществующего Тогда
		
		ОДоговореЦентр = НСтр("ka='Использовать существующий';ru='Использовать существующий'");
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	// Контрагент.
	Если НЕ ПустаяСтрока(СоздаватьНового) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ka='Записать и выбрать';ru='Записать и выбрать'");
	...
	ИначеЕсли НЕ ПустаяСтрока(СоздаватьНовогоПриЗагрузкеВыпискиВ1С) Или НЕ ПустаяСтрока(НеСоздавать) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ka='ОК';ru='ОК'");
		Элементы.КонтрагентСсылка.АвтоОтметкаНезаполненного = Ложь;
		Элементы.КонтрагентСсылка.ОтметкаНезаполненного = Ложь;
	ИначеЕсли НЕ ПустаяСтрока(ИспользоватьСуществующего) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ka='არჩევა';ru='Выбрать'");
...
Функция ОбработатьОКНаСервере()
	...
	//		
	//		ТекстСообщения = НСтр("ka='Необходимо выбрать кассу организации.';ru='Необходимо выбрать кассу организации.'");
	...
	//		
	//		ТекстСообщения = НСтр("ka='Необходимо выбрать счет организации.';ru='Необходимо выбрать счет организации.'");
	//		ЕстьОшибкиЗаполнения = Истина;
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Попытка записи Контрагента не удалась по причине: %1';ru='Попытка записи Контрагента не удалась по причине: %1'"),
					ОписаниеОшибки()));
			Возврат Ложь;
		КонецПопытки;
	...
	ИначеЕсли Не ПустаяСтрока(ИспользоватьСуществующего) Тогда
		...
		Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Или Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Вначале укажите контрагента на закладке ""Контрагент""';ru='Вначале укажите контрагента на закладке ""Контрагент""'"));
				Возврат Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьИННиКППИзВыписки(Команда)
	...
	Если КонтрагентСсылка.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Предварительно необходимо выбрать контрагента';ru='Предварительно необходимо выбрать контрагента'"));
		Возврат;
	КонецЕсли;
	...
	Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) ИЛИ
		...
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) И
			...
			СписокВыбора.Добавить("ИННКПП", НСтр("ka='Обновить ИНН и КПП из выписки';ru='Обновить ИНН и КПП из выписки'"));
		КонецЕсли;
		
		// Все поля.
		СписокВыбора.Добавить("ВСЕ", НСтр("ka='Обновить все реквизиты';ru='Обновить все реквизиты'"));
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ИНН, КПП и юридическое название контрагента и в выписке совпадают, заполнение не требуется';ru='ИНН, КПП и юридическое название контрагента и в выписке совпадают, заполнение не требуется'"));
	КонецЕсли;
...
Функция НайтиИнформациюОДоговоре(ПараметрНазначениеПлатежа, СтрокаПоиска)
	...
	СтрокаПоиска = ВРег(НСтр("ka='по договору банковского счета №';ru='по договору банковского счета №'"));
	ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. банковского счета №';ru='по дог. банковского счета №'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору банковского счета N';ru='по договору банковского счета N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. банковского счета N';ru='по дог. банковского счета N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору №';ru='по договору №'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. №';ru='по дог. №'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору N';ru='по договору N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по дог. N';ru='по дог. N'"));
		ТекС = СтрНайти(ВРег(ПараметрНазначениеПлатежа), СтрокаПоиска);
	КонецЕсли;
	Если ТекС = 0 Тогда
		СтрокаПоиска = ВРег(НСтр("ka='по договору';ru='по договору'"));
...
Процедура ЗаполнитьДоговорПоДаннымНазначенияПлатежа(Команда)
		...
		Пока ТекСНомер > 0 И КолвоЦиклов <= 5 Цикл
			...
			Если ТекСПробел > 0 Тогда
				...
				Если ТекСПробел > 0 Тогда
					...
					Если СокрЛП(ВтороеСлово) <> НСтр("ka='(საიდან, ვისგან)';ru='от'") Тогда
	...
	Если СписокНомеровИДатДоговора.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='В назначении платежа не найдена информация о договоре. Установите номер и дату вручную.';ru='В назначении платежа не найдена информация о договоре. Установите номер и дату вручную.'"));
	Иначе
