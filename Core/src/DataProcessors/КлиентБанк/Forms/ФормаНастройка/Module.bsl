...
"Отправка подписанных платежных поручений и запрос банковской выписки осуществляется из 1С:Управление нашей фирмой.'");
	...
	Если Параметры.Свойство("ЗагруженыНастройкиСчета") И Параметры.ЗагруженыНастройкиСчета Тогда
		...
		КонецЕсли;
	Иначе
		Заголовок = НСтр("ka='Общая настройка обмена с банком';ru='Общая настройка обмена с банком'");
	КонецЕсли;
...
Процедура ВыборФайлаДляВыгрузки(Элемент) Экспорт
	...
	ДиалогВыбора.Фильтр                  = НСтр("ka='Текстовый файл';ru='Текстовый файл'") + " (*.txt)|*.txt";
	ДиалогВыбора.Заголовок               = НСтр("ka='Выберите папку для выгрузки данных из 1C';ru='Выберите папку для выгрузки данных из 1C'");
