...
"Отправка подписанных платежных поручений и запрос банковской выписки осуществляется из 1С:Управление нашей фирмой.';ka='С банком %1 действует соглашение о прямом обмене."
"Отправка подписанных платежных поручений и запрос банковской выписки осуществляется из 1С:Управление нашей фирмой.'");
	...
	Если Параметры.Свойство("ЗагруженыНастройкиСчета") И Параметры.ЗагруженыНастройкиСчета Тогда
		ТекстСчетаДляЗаголовка = НСтр("ka=' (счет %БанковскийСчетНомер%)';ru=' (счет %БанковскийСчетНомер%)'");
		...
		КонецЕсли;
	Иначе
		Заголовок = НСтр("ka='Общая настройка обмена с банком';ru='Общая настройка обмена с банком';en='General setup with bank exchange'");
	КонецЕсли;
...
Процедура ВыборФайлаДляВыгрузки(Элемент) Экспорт
	...
	ДиалогВыбора.Фильтр                  = НСтр("ka='Текстовый файл';ru='Текстовый файл';en='Text file'") + " (*.txt)|*.txt";
	ДиалогВыбора.Заголовок               = НСтр("ka='Выберите папку для выгрузки данных из 1C';ru='Выберите папку для выгрузки данных из 1C';en='Select a folder to upload data from 1C'");
