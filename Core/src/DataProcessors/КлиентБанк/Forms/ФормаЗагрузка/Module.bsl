...
Функция ПроверитьЗаполнениеРеквизитовФормы()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Кодировка) Тогда
		ТекстСообщения = НСтр("ka='В настройках не указана кодировка (кнопка ""Еще"" - ""Настройка"")!';ru='В настройках не указана кодировка (кнопка ""Еще"" - ""Настройка"")!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВерсияФормата) Тогда
		ТекстСообщения = НСтр("ka='В настройках не указана версия формата обмена (кнопка ""Еще"" - ""Настройка"")!';ru='В настройках не указана версия формата обмена (кнопка ""Еще"" - ""Настройка"")!'");
...
Функция ПолучитьДатуИзСтроки(Приемник, Источник)
	...
	Если ПозицияТочки = 0 Тогда
		Возврат НСтр("ka='თარიღიანი სტრიქონის არასწორი ფორმატი';ru='Неверный формат строки с датой'");
	КонецЕсли;
	...
	Исключение
		Возврат НСтр("ka='Не удалось преобразовать строку в дату';ru='Не удалось преобразовать строку в дату'");
	КонецПопытки;
...
Функция НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора = Неопределено, СписокВидовДоговора = Неопределено)
	...
	СтруктураВозврат = Новый Структура("Договор, КоличествоДоговоров", НСтр("ka='Не найден договор по умолчанию';ru='Не найден договор по умолчанию'"), 0);
	...
	Пока Лев(Врег(СокрЛП(СтрокаРазбора)), 14) <> "КОНЕЦДОКУМЕНТА" Цикл
		...
		Если РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Тогда
			...
			Иначе
				...
				ТекстСообщения = НСтр("ka='Неверный реквизит платежного документа, строка %Импорт%: %СтрокаРазбора%';ru='Неверный реквизит платежного документа, строка %Импорт%: %СтрокаРазбора%'"
				);
		...
		Иначе
			...
			// Неправиьный реквизит заголовка.
			ТекстСообщения = НСтр("ka='Нарушена структура платежного документа, строка %Импорт%: %СтрокаРазбора%';ru='Нарушена структура платежного документа, строка %Импорт%: %СтрокаРазбора%'"
			);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтрокаРазбора%", СтрокаРазбора);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения);
...
Функция ЗагрузитьСекциюРасчСчета(СтрокаРССчета, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ТегиРасчетногоСчета)
	...
	Пока РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Цикл
		...
		Иначе
			// Неправиьный реквизит заголовка.
			ТекстСообщения = НСтр("ka='Неверный реквизит в секции описания расчетного счета, строка %Импорт%: %СтрокаРазбора%';ru='Неверный реквизит в секции описания расчетного счета, строка %Импорт%: %СтрокаРазбора%'"
			);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтрокаРазбора%", СтрокаРазбора);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения);
...
Процедура ПроверитьНаПустоеЗначениеИмпорта(СтрокаИмпорта, ИмяСвойства, ПредставлениеСвойства, ИмпортНеПустые)
	...
	Если ИмпортНеПустые[0][ИмяСвойства] = Истина Тогда
		Если НЕ ЗначениеЗаполнено(СтрокаИмпорта[ИмяСвойства]) Тогда
			СтрокаЗамечание = НСтр("ka='Не заполнено ""%ИмяСвойства%""!';ru='Не заполнено ""%ИмяСвойства%""!'");
...
Функция ЗагрузитьСтрокуЗаголовка(ТекстСтрокиЗаголовка, ТегиЗаголовка, ИмпортЗаголовок, ИмпортТекущаяСтрока)
	...
	Иначе
		...
		// Неправиьный реквизит заголовка.
		ТекстСообщения = НСтр("ka='Неверный реквизит заголовка, строка %Импорт%: %ТекстСтрокиЗаголовка%';ru='Неверный реквизит заголовка, строка %Импорт%: %ТекстСтрокиЗаголовка%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
		...
		Если Не НашлиЗаказ И Не НашлиСчет Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Если ВыборкаЗаказовИСчетов.Количество() = 1 Тогда
					...
					//СтрНайти(СтрокаДокумента.НазначениеПлатежа, НСтр("ka='შეკვეთა';ru='Заказ'")) = 0 И
					//СтрНайти(СтрокаДокумента.НазначениеПлатежа, НСтр("ka='ანგარიში';ru='Счет'")) = 0 Тогда
					Если (СтрНайти(СтрокаДокумента.НазначениеПлатежа, "№"+НомерДляПоиска) > 0 ИЛИ
					СтрНайти(СтрокаДокумента.НазначениеПлатежа, " "+НомерДляПоиска) > 0) Тогда
...
Функция ПроверитьВидОперации(СтрокаДокумента, ПараметрыВыделенияКолонок, СтруктураЗначенийИзСоответствия)
	...
	ЕстьКлючевоеСлово = СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='СЧЕТ';ru='СЧЕТ'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='ЗАКАЗ';ru='ЗАКАЗ'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='СЧЁТ';ru='СЧЁТ'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='НАКЛ';ru='НАКЛ'")) > 0;
		
	ЕстьСловаОРасчетномОбслуживании = СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='Р/О';ru='Р/О'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчетное обслуживание';ru='Расчетное обслуживание'"))) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчетные услуги';ru='Расчетные услуги'"))) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчётные услуги';ru='Расчётные услуги'"))) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='Р\О';ru='Р\О'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='Р\У';ru='Р\У'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчётное обслуживание';ru='Расчётное обслуживание'"))) > 0;
		
	ЕслиСловоВозврат = СтрНайти(СтрокаДокумента.НазначениеПлатежа, НСтр("ka='დაბრუნება';ru='Возврат'")) > 0;
	...
	Если Не ЕстьКлючевоеСлово Или ЕстьСловаОРасчетномОбслуживании ИЛИ ЕслиСловоВозврат Тогда
		...
		СтрокаЗамечание = НСтр("ka='Проверьте операцию!';ru='Проверьте операцию!'");
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, ПараметрыВыделенияКолонок, ПослеЗагрузкиВыписокВ1С = Ложь)
	...
	Если ПустаяСтрока(Буфер) Тогда
		...
		СтрокаЗамечание = НСтр("ka='Сумма не указана!';ru='Сумма не указана!'");
	...
	Иначе
		СтрокаДокумента.НеВерныйФорматСуммыКартинка = 0;
		СтрокаЗамечание = НСтр("ka='Укажите сумму вручную (%Буфер%)!';ru='Укажите сумму вручную (%Буфер%)!'");
	...
	Если ВсеРеквизитыПоискаЕсть Тогда
				...
				Если КоличествоДок = 0 Тогда
					...
					Если ДокументПодходит Тогда
						...
						Если СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета
							...
							//СтрокаДокумента.Контрагент = НСтр("ka='Наша организация (';ru='Наша организация ('") + ВыборкаДокументов.Организация + ")";
		...
		Если КоличествоДок > 1 Тогда
			...
			СтрокаЗамечание = НСтр("ka='В информационной базе найдено несколько (%КоличествоДок%) соответствующих документов!';ru='В информационной базе найдено несколько (%КоличествоДок%) соответствующих документов!'");
			СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%КоличествоДок%", КоличествоДок);
			
			Если Не ПослеЗагрузкиВыписокВ1С Тогда
				СтрокаДокумента.ДокументНайден = СтрокаДокумента.ДокументНайден + НСтр("ka='. И еще %КоличествоДок% док.';ru='. И еще %КоличествоДок% док.'");
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) Тогда
		...
		Если ВыборкаПоиска.Следующий() Тогда
			...
			Если НЕ ПустаяСтрока(ИННКонтрагента) И ВыборкаПоиска.ПриоритетИНН = 2 И ВыборкаПоиска.ПриоритетНаименования = 2 Тогда
				...
				СтрокаЗамечание = НСтр("ka='Банковский счет найден у контрагента с другим ИНН (%ВладелецИНН%)!';ru='Банковский счет найден у контрагента с другим ИНН (%ВладелецИНН%)!'");
				...
				Иначе
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%ВладелецИНН%", НСтр("ka='<Не заполнен>';ru='<Не заполнен>'"));
				КонецЕсли;
				ДобавитьЗамечание(СтрокаДокумента, 1, СтрокаЗамечание);
			...
			Иначе
				...
				Если (ВыборкаПоиска.ПриоритетБИК = 2) Тогда
					...
					СтрокаЗамечание = НСтр("ka='БИК банка счета контрагента в базе (%БИК_из_Базы%) не совпадает с БИК банка в выписке (%БИК_в_Выписке%).';ru='БИК банка счета контрагента в базе (%БИК_из_Базы%) не совпадает с БИК банка в выписке (%БИК_в_Выписке%).'");
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%БИК_из_Базы%", ?(ЗначениеЗаполнено(ВыборкаПоиска.БИК_из_Базы), ВыборкаПоиска.БИК_из_Базы, НСтр("ka='<пустой>';ru='<пустой>'")));
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%БИК_в_Выписке%", ?(ЗначениеЗаполнено(БИКСчета), БИКСчета, НСтр("ka='<пустой>';ru='<пустой>'")));
					ДобавитьЗамечание(СтрокаДокумента, 4, СтрокаЗамечание);
				КонецЕсли;
		...
		Если КонтрагентНеНайден Тогда
			СтрокаЗамечание = НСтр("ka='Не найден счет контрагента (%СчетКонтрагента%)!';ru='Не найден счет контрагента (%СчетКонтрагента%)!'");
			...
			СтрокаСчетКонтрагента = НСтр("ka='Не найден (%СчетКонтрагента%).';ru='Не найден (%СчетКонтрагента%).'");
		...
		Если КоличествоЭлементовВВыборке > 1 //И 
			...
			Тогда
			СтрокаЗамечание = НСтр("ka='В информационной базе найдено несколько (%Количество%) одинаковых банковских счетов! Проверьте контрагента и р/счет!';ru='В информационной базе найдено несколько (%Количество%) одинаковых банковских счетов! Проверьте контрагента и р/счет!'");
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Тогда
		...
		ИначеЕсли НЕ ПустаяСтрока(ИННКонтрагента) Тогда
			...
			Если ВыборкаПоиска.Следующий() Тогда
				...
				Если ПустаяСтрока(ВыборкаПоиска.ИНН) Тогда
					СтрокаЗамечание = НСтр("ka='У контрагента не заполнен ИНН!';ru='У контрагента не заполнен ИНН!'");
					ДобавитьЗамечание(СтрокаДокумента, 2, СтрокаЗамечание);
			...
			Если ВыборкаПоиска.Количество() > 1 Тогда
				СтрокаЗамечание = НСтр("ka='В информационной базе найдено несколько (%Количество%) контрагентов с одинаковым ИНН!';ru='В информационной базе найдено несколько (%Количество%) контрагентов с одинаковым ИНН!'");
			...
			ИначеЕсли ВыборкаПоиска.Количество() = 0 Тогда
				СтрокаЗамечание = НСтр("ka='Не найден контрагент (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!';ru='Не найден контрагент (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!'");
				...
				СтрокаКонтрагент = НСтр("ka='Не найден (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!';ru='Не найден (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!'");
		...
		Иначе
			...
			СтрокаКонтрагент = НСтр("ka='Не найден (%ИмяКонтрагента%, не указан ИНН)!';ru='Не найден (%ИмяКонтрагента%, не указан ИНН)!'");
			СтрокаКонтрагент = СтрЗаменить(СтрокаКонтрагент, "%ИмяКонтрагента%", ИмяКонтрагента);
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.Контрагент) И СтрокаДокумента.КонтрагентНеНайденКартинка = -1 Тогда
			Если ИННКонтрагентаВБазе <> ИННКонтрагента
				...
				Если КолвоРеквизитовДляПроверки = 1 Тогда
					СтрокаЗамечание = НСтр("ka='Проверьте %СтрокаПроверки% контрагента!';ru='Проверьте %СтрокаПроверки% контрагента!'");
				Иначе
					СтрокаЗамечание = НСтр("ka='Проверьте реквизиты контрагента: %СтрокаПроверки%!';ru='Проверьте реквизиты контрагента: %СтрокаПроверки%!'");
				КонецЕсли;
	...
	Иначе
		...
		СтрокаДокумента.ОписаниеОшибок = УдалитьДанныеИзОписанияОшибкиНаСервере(НСтр("ka='Не найден счет контрагента';ru='Не найден счет контрагента'"), СтрокаДокумента.ОписаниеОшибок, СтрокаДокумента);
	КонецЕсли;
	...
	И НЕ ЗначениеЗаполнено(СтрокаДокумента.Договор) Тогда
		...
		Если СтрНайти(СтрокаДокумента.Договор, НСтр("ka='Не найден';ru='Не найден'")) > 0 Тогда
		...
		Если СтрНайти(СтрокаДокумента.Договор, НСтр("ka='Не найден';ru='Не найден'")) > 0 И ТипЗнч(СтрокаДокумента.Контрагент) <> Тип("Строка") Тогда
	...
	Если //ТипЗнч(СтрокаДокумента.СчетКонтрагента) = Тип("СправочникСсылка.БанковскиеСчета")
		...
		И СтрНайти(""+СтрокаДокумента.Контрагент, НСтр("ka='Наша организация (';ru='Наша организация ('")) = 0 Тогда
		//СтрокаДокумента.Контрагент = НСтр("ka='Наша организация (';ru='Наша организация ('") + СтрокаДокумента.Контрагент + ")";
	КонецЕсли;
	...
	Пока ИмпортТекущаяСтрока <= ИмпортКоличествоСтрок Цикл
		...
		Если Лев(ВРег(СокрЛП(Стр)), 14) = "СЕКЦИЯДОКУМЕНТ" Тогда
			...
			Если Тег = "СЕКЦИЯДОКУМЕНТ" Тогда
				...
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ka='Нарушена структура файла импорта, строка %Импорт%: %Стр%';ru='Нарушена структура файла импорта, строка %Импорт%: %Стр%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
		...
		ИначеЕсли Лев(Врег(СокрЛП(Стр)), 14) = "СЕКЦИЯРАСЧСЧЕТ" Тогда
			...
			Если НЕ ЗагрузитьСекциюРасчСчета(СтрокаРССчетов, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ТегиРасчетногоСчета) Тогда
				ТекстСообщения = НСтр("ka='Нарушена структура файла импорта в секции описания расчетного счета! Строка: %Импорт%';ru='Нарушена структура файла импорта в секции описания расчетного счета! Строка: %Импорт%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
				Возврат ТекстСообщения;
			КонецЕсли;
...
"Для проверки можете использовать гиперссылку ""Посмотреть содержимое файла"" в правом нижнем углу.'");
					...
					ИначеЕсли ЕстьОтборПоБанковскомуСчету Тогда
						...
						//ТекстСообщения = НСтр("ka='В заголовке файла не указан банковский счет. В файле есть строка ""РасчСчет="", но после символа ""="" нет номера счета.';ru='В заголовке файла не указан банковский счет. В файле есть строка ""РасчСчет="", но после символа ""="" нет номера счета.'");
						//Возврат ТекстСообщения;
				...
				Если Выборка.Следующий() Тогда
					...
					Если НЕ ЕстьОтборПоБанковскомуСчету И ЗначениеЗаполнено(Объект.БанковскийСчет)
					   ...
					   Если Объект.БанковскийСчет.НомерСчета = Значение Тогда
							ТекстСообщения = НСтр("ka='В базе есть несколько банковский счетов организаций с одинаковым номером!';ru='В базе есть несколько банковский счетов организаций с одинаковым номером!'");
						Иначе
							ТекстСообщения = НСтр("ka='В заголовке файла указан счет (%Значение%) отличный от указанного!';ru='В заголовке файла указан счет (%Значение%) отличный от указанного!'");
						КонецЕсли;
					...
					КонецЕсли;
				Иначе
					ТекстСообщения = НСтр("ka='В заголовке файла указан счет, не принадлежащий организации: %Значение%!';ru='В заголовке файла указан счет, не принадлежащий организации: %Значение%!'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Значение%", Значение);
					Возврат ТекстСообщения;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли Лев(Врег(СокрЛП(Стр)), 10) = "КОНЕЦФАЙЛА" Тогда
			...
			Если НЕ ИмпортПризнакОбмена Тогда
				ТекстСообщения = НСтр("ka='В файле импорта отсутствует признак обмена ""1CClientBankExchange""!';ru='В файле импорта отсутствует признак обмена ""1CClientBankExchange""!'");
				Возврат ТекстСообщения;
			КонецЕсли;
	...
	Если ЕстьОтборПоБанковскомуСчету И НЕ НашлиБанковскийСчет Тогда
		...
		ТекстСообщения = НСтр("ka='В заголовке файла не найден указанный банковский счет (%Значение%)!';ru='В заголовке файла не найден указанный банковский счет (%Значение%)!'");
	...
	Если НЕ НайденКонецФайла Тогда
		...
		ДокументыКИмпорту.Очистить();
		ТаблицаТаблицДокументов.Очистить();
		ТекстСообщения = НСтр("ka='Файл загрузки не соответствует стандарту (не найдена секция КонецФайла)!';ru='Файл загрузки не соответствует стандарту (не найдена секция КонецФайла)!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения);
	КонецЕсли;
	...
	Для каждого СтрокаДокумента Из ДокументыКИмпорту Цикл
		...
		Если СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ДатаПоступило) Тогда
				...
				СтрокаЗамечание = НСтр("ka='Это данные по другому счету: %1%!';ru='Это данные по другому счету: %1%!'");
		...
		Если СтрокаДокумента.Документ = Неопределено Тогда
			СтрокаДокумента.ДокументНеЗагружен = ?(СтрокаДокумента.Загружать, НСтр("ka='Будет загружен';ru='Будет загружен'"), НСтр("ka='Не будет загружен';ru='Не будет загружен'"))
...
Функция ПрочитатьЭлектроннуюВыпискуБанкаНаСервере(ЭлектроннаяВыписка, СтекОповещений)
	...
	Исключение
		ТекстСообщения = НСтр("ka='Файл не прочитан.';ru='Файл не прочитан.'");
		СтекОповещений.Добавить(Новый Структура("Текст", ТекстСообщения));
		Возврат Неопределено;
	КонецПопытки;
	
	Если ПотокЧтения.КоличествоСтрок() < 1 Тогда
		ТекстСообщения = НСтр("ka='В файле нет данных!';ru='В файле нет данных!'");
		СтекОповещений.Добавить(Новый Структура("Текст", ТекстСообщения));
		Возврат Неопределено;
	КонецЕсли;
	
	Если СокрЛП(ПотокЧтения.ПолучитьСтроку(1)) <> "1CClientBankExchange" Тогда
		ТекстСообщения = НСтр("ka='Указанный файл не является файлом обмена или неверно указана кодировка!';ru='Указанный файл не является файлом обмена или неверно указана кодировка!'");
		СтекОповещений.Добавить(Новый Структура("Текст", ТекстСообщения));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПрочитатьЭлектронныеВыпискиБанка()
	...
	Если ИмпортТекстДляРазбора = Неопределено Тогда
		ТекстСообщения = НСтр("ka='Файл загрузки не содержит данных!';ru='Файл загрузки не содержит данных!'");
	...
	Если ЗначениеЗаполнено(СоглашениеПрямогоОбменаСБанками) Тогда
		...
		Если ВыпискиБанка.Количество() = 0 Тогда
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма,
				Нстр("ka='Для получения электронной выписки банка нажмите кнопку ""Запросить выписку""';ru='Для получения электронной выписки банка нажмите кнопку ""Запросить выписку""'")
...
Функция ПериодЗаполнен()
	...
	Если НЕ ЗначениеЗаполнено(Объект.НачПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Нстр("ka='Не заполнена дата начала периода';ru='Не заполнена дата начала периода'")
			,, "Объект.НачПериода");
		ПериодЗаполнен = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Нстр("ka='Не заполнена дата окончания периода';ru='Не заполнена дата окончания периода'")
			,, "Объект.КонПериода");
		ПериодЗаполнен = Ложь;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресФайлаКОбработке") Тогда
		...
		Если ТипЗнч(ФайлЗагрузки) = Тип("ДвоичныеДанные") Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ka='Обмен с банком.Временный файл';ru='Обмен с банком.Временный файл'"), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Не удалось сохранение временного файла на диск по причине: %1';ru='Не удалось сохранение временного файла на диск по причине: %1'"),
						ОписаниеОшибки()));
	...
	Если Параметры.Свойство("СоглашениеПрямогоОбменаСБанками") Тогда
		...
		Если ЗначениеЗаполнено(СоглашениеПрямогоОбменаСБанками) Тогда
			ТекстШаблона = НСтр("ka='С %1 действует соглашение о прямом обмене: банковская выписка будет загружена в 1С:Управление нашей фирмой напрямую из банка';ru='С %1 действует соглашение о прямом обмене: банковская выписка будет загружена в 1С:Управление нашей фирмой напрямую из банка'");
...
Процедура ЗагрузкаОбновитьВыполнить(Команда)
	...
	Состояние(
		НСтр("ka='მიმდინარეობს ამონაწერის ფაილის წაკითხვა...';ru='Выполняется чтение файла выписки...'"),
		,
		НСтр("ka='გთხოვთ, დაელოდეთ';ru='Пожалуйста, подождите'"),
	...
	Если Объект.Загрузка.Количество() = 0 Тогда
		//ПоказатьПредупреждение(Неопределено, НСтр("ka='В файле нет данных о платежах или их не удалось прочитать';ru='В файле нет данных о платежах или их не удалось прочитать'"));
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма,
				НСтр("ka='В файле нет данных о платежах или их не удалось прочитать';ru='В файле нет данных о платежах или их не удалось прочитать'")
				,, "ЭлектроннаяВыпискаБанка");
	КонецЕсли;
...
Процедура ЗагрузитьВыполнить(Команда)
	...
	Если Объект.Загрузка.Количество() > 0 Тогда
		...
		Если Не ЕстьОтмеченныеСтроки Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Не отмечено ни одной строки! Выписки не будут загружены. Нужно установить отметку в загружаемых строках и нажать кнопку еще раз.';ru='Не отмечено ни одной строки! Выписки не будут загружены. Нужно установить отметку в загружаемых строках и нажать кнопку еще раз.'");
		...
		Состояние(
			НСтр("ka='Выполняется загрузка выписки из файла в 1С...';ru='Выполняется загрузка выписки из файла в 1С...'"),
			,
			НСтр("ka='გთხოვთ, დაელოდეთ';ru='Пожалуйста, подождите'"),
			БиблиотекаКартинок.ЗагрузкаДанных32
		);
	...
	Иначе
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Список документов для загрузки пуст.';ru='Список документов для загрузки пуст.'"));
	...
	Если ТаблицаПомеченныхНаУдалениеКакДубль.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПротокола Из ТаблицаПомеченныхНаУдалениеКакДубль Цикл
			...
			Если Не СтрокаПротокола.ОперацияВыполнена Тогда
				Область.Параметры.Комментарий = НСтр("ka='Не получилось отменить проведение!';ru='Не получилось отменить проведение!'");
			КонецЕсли;
...
Функция ТекстСсылкиНаЭлектронныеВыписки(НачалоПериода, КонецПериода)
	...
	ШаблонСтроки = НСтр("ka='Выписка банка за период с %1 по %2';ru='Выписка банка за период с %1 по %2'");
...
Функция ПодготовитьОднуСтрокуТекстаПредупреждения(ОписаниеОшибок, ПоказыватьЗнакПредупреждения, ДанныеСтроки, ТолькоИнформацияОДублях = Неопределено)
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Не найден контрагент';ru='Не найден контрагент'"));
	Если ТекС > 0 Тогда
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Не найден счет';ru='Не найден счет'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + 
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Не найден договор';ru='Не найден договор'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ?(ДанныеСтроки.ДействиеСоСчетомКонтрагента.Пустая(), 
			НСтр("ka='Не найден основной договор. Будет создан новый при загрузке в 1С и установлен основным.';ru='Не найден основной договор. Будет создан новый при загрузке в 1С и установлен основным.'"), 
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Проверьте операцию';ru='Проверьте операцию'"));
	Если ТекС > 0 Тогда
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Укажите сумму вручную';ru='Укажите сумму вручную'"));
	Если ТекС > 0 Тогда
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Сумма не указана';ru='Сумма не указана'"));
	Если ТекС > 0 Тогда
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='соответствующих документов';ru='соответствующих документов'"));
	Если ТекС > 0 Тогда
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='контрагентов с одинаковым ИНН!';ru='контрагентов с одинаковым ИНН!'"));
	Если ТекС > 0 Тогда
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='одинаковых банковских счетов!';ru='одинаковых банковских счетов!'"));
	Если ТекС > 0 Тогда
...
Функция ПодготовитьТекстПредупреждения(ДанныеСтроки, ПоказыватьЗнакПредупреждения = Ложь)
	...
	Если НЕ ДанныеСтроки.Загружать Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka='<Флаг загрузки снят. Строка не будет загружена>';ru='<Флаг загрузки снят. Строка не будет загружена>'") + Символы.ПС + Символы.ПС;
	КонецЕсли;
...
Процедура ЗагрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(Элементы.Загрузка.ТекущиеДанные.Документ)
		...
		Если ДанныеСтроки.НесколькоДокументовКартинка = 0 ИЛИ СтрНайти(ДанныеСтроки.ДокументНайден, НСтр("ka='И еще';ru='И еще'")) > 0 Тогда
			СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли Поле.Имя = "ЗагрузкаНеВерныйФорматСуммыКартинка" Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Сумма) Тогда
			ПоказатьВводЧисла(Новый ОписаниеОповещения("ПоказатьВводСуммыДокументаЗавершение", ЭтотОбъект, ТекущиеДанные.ПолучитьИдентификатор()),
				...
				НСтр("ka='Введите нужное число!';ru='Введите нужное число!'"), 
				СтрЗаменить(НСтр("ka='Введите нужное число (%1)!';ru='Введите нужное число (%1)!'"), "%1", СокрЛП(ТекущиеДанные.Сумма)))
				, 15, 2);
		Иначе
	...
	ИначеЕсли (Поле.Имя = "ЗагрузкаСуммаДокумента"
		...
		Если НеВерныйФорматСуммы Тогда
			ПоказатьВводЧисла(Новый ОписаниеОповещения("ПоказатьВводСуммыДокументаЗавершение", ЭтотОбъект, ДанныеСтроки.ПолучитьИдентификатор()),
				...
				НСтр("ka='Введите нужное число!';ru='Введите нужное число!'"), 
				СтрЗаменить(НСтр("ka='Введите нужное число (%1)!';ru='Введите нужное число (%1)!'"), "%1", СокрЛП(ДанныеСтроки.Сумма)))
				, 15, 2);
		Иначе
...
Функция ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, СтрокаСНазваниемДокумента)
	...
	Если ТипЗнч(ТекущаяСтрокаТабличнойЧасти.Контрагент) = Тип("Строка") Тогда
		...
		ТекстСообщения = НСтр("ka='Контрагент не идентифицирован, выбор %1 не возможен.';ru='Контрагент не идентифицирован, выбор %1 не возможен.'");
...
Функция ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, СтрокаСНазваниемДокумента, ИмяРеквизита)
	...
	Если Не ПолучитьРеквизитКонтрагента(ТекущаяСтрокаТабличнойЧасти.Контрагент, ИмяРеквизита) Тогда
		...
		ТекстСообщения = НСтр("ka='С контрагент не ведется учет расчетов по %1. Выбор документа не требуется.';ru='С контрагент не ведется учет расчетов по %1. Выбор документа не требуется.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", СтрокаСНазваниемДокумента);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ЗагрузкаЗаказНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='заказа';ru='заказа'"));
	СтандартнаяОбработка = ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='заказам';ru='заказам'"), "ВестиРасчетыПоЗаказам");
...
Процедура ЗагрузкаСчетНаОплатуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущаяСтрокаТабличнойЧасти = Элементы.Загрузка.ТекущиеДанные;
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='счета на оплату';ru='счета на оплату'"));
	СтандартнаяОбработка = ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='счетам на оплату';ru='счетам на оплату'"), "ВестиУчетОплатыПоСчетам");
...
Процедура ЗагрузкаДокументРасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущаяСтрокаТабличнойЧасти = Элементы.Загрузка.ТекущиеДанные;
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='документа расчетов';ru='документа расчетов'"));
	СтандартнаяОбработка = ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='документам расчетов';ru='документам расчетов'"), "ВестиРасчетыПоДокументам");
...
Процедура ЗагрузкаПлатежноеПоручениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущаяСтрокаТабличнойЧасти = Элементы.Загрузка.ТекущиеДанные;
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='платежного поручения';ru='платежного поручения'"));
...
Процедура ЗаполнитьСуммыВыделеныхНаКлиенте(ТекСтрока)
	...
	ОстатокКон = ОстатокНач + Объект.Загрузка.Итог("СуммаПоступилоВыделено") - Объект.Загрузка.Итог("СуммаСписаноВыделено");
	ТекСтрока.ДокументНеЗагружен = ?(ТекСтрока.Загружать, НСтр("ka='Будет загружен';ru='Будет загружен'"), НСтр("ka='Не будет загружен';ru='Не будет загружен'"))
...
//		ДанныеСтроки.ОписаниеОшибок = СтрЗаменить(НСтр("ka='Проверьте операцию!';ru='Проверьте операцию!'"), "", "");
...
Процедура ОповещениеРедактированиеКонтрагента(Результат, Параметры) Экспорт
	...
	Если (ТипЗнч(Результат.Контрагент) = Тип("СправочникСсылка.Контрагенты")) И ЗначениеЗаполнено(Результат.Контрагент) Тогда
		...
		ИначеЕсли Результат.ДействиеСконтрагентом = ПредопределенноеЗначение("Перечисление.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента") Тогда
			ДанныеСтроки.Контрагент = ДанныеСтроки.КонтрагентИсходный;
		КонецЕсли;
		//ДанныеСтроки.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден контрагент';ru='Не найден контрагент'"), ДанныеСтроки.ОписаниеОшибокПодТЧ, ДанныеСтроки);
				...
				Если ДанныеСтроки <> Строка И ИНН = ?(Строка.Исходящий, Строка.ПолучательИНН, Строка.ПлательщикИНН) И
					...
					Если ЗначениеЗаполнено(Результат.Договор) Тогда
						...
						Если ТипЗнч(Строка.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
							//Строка.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден договор';ru='Не найден договор'"), Строка.ОписаниеОшибокПодТЧ, Строка);
						КонецЕсли;
					КонецЕсли;
					...
					Если ЗначениеЗаполнено(Результат.БанковскийСчетПоУмолчанию) Тогда
						...
						//Строка.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден счет контрагента';ru='Не найден счет контрагента'"), Строка.ОписаниеОшибокПодТЧ, Строка);
					КонецЕсли;
...
Процедура ОткрытьФормуРедактированияКонтрагентов(ДанныеСтроки, ИмяЗакладкиНаФорме)
	...
	КонецЕсли;
	СтруктураПараметров.Вставить("ЕстьНесколькоДокументов", ?(СтрНайти(ДанныеСтроки.ДокументНайден, НСтр("ka='И еще';ru='И еще'")) > 0 ИЛИ ДанныеСтроки.НесколькоДокументовКартинка = 0, Истина, Ложь));
...
Процедура ОткрытьФормуРедактированияСчетаКорреспонденции(ДанныеСтроки)
	...
	Если ДанныеСтроки.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.Прочее") И
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Корреспонденцию нужно устанавливать только для операции ""Прочее"".';ru='Корреспонденцию нужно устанавливать только для операции ""Прочее"".'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузкаСчетКонтрагентаПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТипЗнч(ТекущиеДанные.СчетКонтрагента) = Тип("СправочникСсылка.БанковскиеСчета") И ЗначениеЗаполнено(ТекущиеДанные.СчетКонтрагента) Тогда
			...
			ТекущиеДанные.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден счет контрагента';ru='Не найден счет контрагента'"), ТекущиеДанные.ОписаниеОшибокПодТЧ, ТекущиеДанные);
...
Процедура УбратьПредупреждающийЗнакВТекущейКолонке(Команда)
	...
	Исключение
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Выберите в таблице ячейку с предупреждающим знаком';ru='Выберите в таблице ячейку с предупреждающим знаком'"), 30);
		Возврат;
	КонецПопытки;
	...
	ИначеЕсли ИмяКолонки = "ЗагрузкаКонтрагентНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаКонтрагент" Тогда
		...
		//УдалятьПредупреждение = НСтр("ka='Не найден контрагент';ru='Не найден контрагент'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаВидОперацииНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаВидОперацииПоступление" ИЛИ 
		...
		УдалятьПредупреждение = НСтр("ka='Проверьте операцию';ru='Проверьте операцию'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаДоговорНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаДоговор" Тогда
		...
		//УдалятьПредупреждение = НСтр("ka='Не найден договор';ru='Не найден договор'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаСчетКонтрагентаНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаСчетКонтрагента" Тогда
		...
		//УдалятьПредупреждение = НСтр("ka='Не найден счет';ru='Не найден счет'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаНеВерныйФорматСуммыКартинка" Тогда
		ИмяКолонки = "НеВерныйФорматСуммыКартинка";
		//УдалятьПредупреждение = НСтр("ka='Укажите сумму вручную';ru='Укажите сумму вручную'");
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Действие доступно для колонок с предупреждающим знаком';ru='Действие доступно для колонок с предупреждающим знаком'"), 30);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидОперации(ДанныеСтроки, ВыбранноеЗначение)
	...
	ДанныеСтроки.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Проверьте операцию';ru='Проверьте операцию'"), ДанныеСтроки.ОписаниеОшибокПодТЧ, ДанныеСтроки);
	ПоменятьСимволПСНаПробел(ДанныеСтроки.ОписаниеОшибокПодТЧ);
	...
	Если ДанныеСтроки.ВидОперации = ВидОперацииПереводНаДругойСчет ИЛИ ДанныеСтроки.ВидОперации = ВидОперацииПереводСДругогоСчета Тогда
		...
		//ДанныеСтроки.Контрагент = НСтр("ka='Наша организация (';ru='Наша организация ('") + Объект.Организация + ")";
