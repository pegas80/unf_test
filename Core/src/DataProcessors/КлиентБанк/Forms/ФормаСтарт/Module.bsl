	...
	Если ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		...
		Элементы.ФильтрПоСчету.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Банковский счет: %1';ru='Банковский счет: %1';en='Bank account: 1%'"),
				Объект.БанковскийСчет);
	Иначе
		Элементы.ФильтрПоСчету.Заголовок = НСтр("ka='Банковский счет: <по данным выписки>';ru='Банковский счет: <по данным выписки>';en='Bank account: <According to statements>'")
	КонецЕсли;
