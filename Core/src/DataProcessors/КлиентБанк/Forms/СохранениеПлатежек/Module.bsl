...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.БанковскийСчет) Тогда
		Отказ = Истина;
		ВызватьИсключение НСтр("ka='Открытие формы сохранение данных по платежкам невозможно без указание параметра банковского счета';ru='Открытие формы сохранение данных по платежкам невозможно без указание параметра банковского счета';en='Opening a form of preservation of data bills can not be no parameters bank account'");
	КонецЕсли;
	Если Параметры.СписокПлатежек.Количество() = 0 Тогда
		Отказ = Истина;
		ВызватьИсключение НСтр("ka='Не выбрано ни одного платежного поручения. Выгрузка не будет выполнена';ru='Не выбрано ни одного платежного поручения. Выгрузка не будет выполнена';en='You have no payment order. Unloading will fail'");
	КонецЕсли;
...
Процедура ВыгрузитьВыполнить()
	...
	Если Объект.Выгрузка.Количество() > 0 Тогда
		...
		Если Не ЕстьОтмеченныеСтроки Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Платежные поручения не будут выгружены. Нужно устранить замечания и повторить выгрузку.';ru='Платежные поручения не будут выгружены. Нужно устранить замечания и повторить выгрузку.';en='Payment orders will not be offloaded. It is necessary to remove comments and restart the download.'");
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
...
Функция ПроверитьЗаполнениеРеквизитовФормы(ПрямойОбменСБанками = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ka='Не заполнена организация в карточке банковского счета!';ru='Не заполнена организация в карточке банковского счета!';en='Not filled in the card organization bank account!'");
