...
Функция ПроверитьЗаполнениеРеквизитовФормы(ПрямойОбменСБанками = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(Объект.НачПериода) Тогда
		ТекстСообщения = НСтр("ka='Не заполнен начальный период!';ru='Не заполнен начальный период!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонПериода) Тогда
		ТекстСообщения = НСтр("ka='Не заполнен конечный период!';ru='Не заполнен конечный период!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		ТекстСообщения = НСтр("ka='Не заполнен банковский счет!';ru='Не заполнен банковский счет!'");
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ka='Не заполнена организация!';ru='Не заполнена организация!'");
	...
	Если НЕ ПрямойОбменСБанками Тогда
		Если НЕ ЗначениеЗаполнено(Объект.Программа) Тогда
			ТекстСообщения = НСтр("ka='Не заполнено название программы в настройках обмена (команда ""Еще - Настройка"")!';ru='Не заполнено название программы в настройках обмена (команда ""Еще - Настройка"")!'");
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения, , , "Программа"); //, Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Объект.Кодировка) Тогда
			ТекстСообщения = НСтр("ka='Не заполнена кодировка!';ru='Не заполнена кодировка!'");
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения, , , "Кодировка", Отказ);
			ОткрытьФормуНастройки = Истина;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Объект.ВерсияФормата) Тогда
			ТекстСообщения = НСтр("ka='Не указана версия формата обмена!';ru='Не указана версия формата обмена!'");
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ka='Устраните замечания и нажмите кнопку еще раз!';ru='Устраните замечания и нажмите кнопку еще раз!'");
...
Процедура ЗаполнитьВыгрузка()
	...
	Для каждого СтрокаДокумента Из Выгрузка Цикл
		...
		Если ПустаяСтрока(СтрокаДокумента.ОписаниеОшибок) Тогда
			СтрокаДокумента.ОписаниеОшибокПодТЧ = НСтр("ka='Текущий документ готов к выгрузке! Проверьте значения в других строках, где есть предупреждающий знак.';ru='Текущий документ готов к выгрузке! Проверьте значения в других строках, где есть предупреждающий знак.'");
		Иначе
	...
	Если Результат <> Неопределено Тогда
		Если Результат.Количество() > 0 Тогда
			...
			Если ПараметрыЗавершения.ВыгружатьИнтерактивно Тогда
				ТекстСообщения = НСтр("ka='Данные успешно выгружены в файл.';ru='Данные успешно выгружены в файл.'");
			Иначе
				ТекстСообщения = НСтр("ru='Данные успешно выгружены в файл ';ka='Данные успешно выгружены в файл '") + Объект.ФайлВыгрузки + ".";
			КонецЕсли;
			Если Результат.Количество() > 1 Тогда
				ПоказатьПредупреждение(Неопределено, НСтр("ka='Выгружено более одного файла!!!';ru='Выгружено более одного файла!!!'"));
			КонецЕсли;
			
			ВыгрузитьФайлНаКлиенте(Результат[0].Имя, ПараметрыЗавершения.АдресФайлаВоВременномХранилище);
			
		Иначе
			ТекстСообщения = НСтр("ka='Операция отменена';ru='Операция отменена'");
		КонецЕсли;
...
Процедура ВыгрузитьФайлНаКлиентеЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ТекстЗаголовка = НСтр("ka='Данные успешно выгружены в файл';ru='Данные успешно выгружены в файл'");
		...
		Если Объект.КонтролироватьБезопасностьОбменаСБанком Тогда
			...
			Если РасширениеРаботыСФайламиПодключено Тогда
				...
				Если НЕ ВыгружатьИнтерактивно Тогда
					...
					Если НЕ ЗначениеЗаполнено(Объект.ФайлВыгрузки) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							НСтр("ka='В настройках не указан файл данных для выгрузки документов в банк. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.';ru='В настройках не указан файл данных для выгрузки документов в банк. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.'")
							,, "Объект.ФайлВыгрузки");
						Возврат;
					КонецЕсли;
				...
				Иначе
					...
					Фильтр = НСтр("ka='Текстовый файл';ru='Текстовый файл'")
						+ " (*.txt)|*.txt";
...
"%1'");
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='მონაცემთა ჩაწერა ფაილში ვერ მოხერხდა. შეიძლება, დისკი ჩაწერისგან დაცულია.';ru='Не удалось записать данные в файл. Возможно, диск защищен от записи.'");
...
Процедура УправлениеФормойНаСервере()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменСБанками")
		...
		ТекстШаблона = НСтр("ka='С %1 действует соглашение о прямом обмене: платежные поручения будут отправлены в банк из 1С:Управление нашей фирмой';ru='С %1 действует соглашение о прямом обмене: платежные поручения будут отправлены в банк из 1С:Управление нашей фирмой'");
...
Функция ВывестиПротокол()
	...
	//Заголовок = НСтр("ka='Протокол сохранения';ru='Протокол сохранения'");
...
Процедура ОтправитьВБанк(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещенияВопрос, НСтр("ka='Данные будут выгружены в банк, продолжить?';ru='Данные будут выгружены в банк, продолжить?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
...
Процедура ВыгрузкаОбновитьВыполнить(Команда)
	...
	Если Элементы.ВыгрузкаБанковскийСчет.Видимость И Объект.БанковскийСчет.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Выберите банковский счет вашей организации';ru='Выберите банковский счет вашей организации'");
	...
	Если Объект.Выгрузка.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='По выбранному счету нет платежных поручений за указанный период';ru='По выбранному счету нет платежных поручений за указанный период'"));
	КонецЕсли;
...
Процедура ЗагрузкаНомерКартинкиДляПодвалаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Элементы.Выгрузка.ТекущиеДанные.ОписаниеОшибок) Тогда
		ТекстПредупреждения = Элементы.Выгрузка.ТекущиеДанные.ОписаниеОшибок + 
			Символы.ПС + Символы.ПС + НСтр("ka='После устранения замечаний нажмите кнопку ""Обновить""';ru='После устранения замечаний нажмите кнопку ""Обновить""'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Документ готов к выгрузке!';ru='Документ готов к выгрузке!'"));
	КонецЕсли;
