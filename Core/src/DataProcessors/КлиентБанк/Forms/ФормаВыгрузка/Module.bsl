...
Функция ПроверитьЗаполнениеРеквизитовФормы(ПрямойОбменСБанками = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(Объект.НачПериода) Тогда
		ТекстСообщения = НСтр("ka='Не заполнен начальный период!';ru='Не заполнен начальный период!';en='Is empty initial period!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонПериода) Тогда
		ТекстСообщения = НСтр("ka='Не заполнен конечный период!';ru='Не заполнен конечный период!';en='Not filled during the final!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		ТекстСообщения = НСтр("ka='Не заполнен банковский счет!';ru='Не заполнен банковский счет!';en='Is empty your bank account!'");
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ka='Не заполнена организация!';ru='Не заполнена организация!';en='Not filled with organization!'");
	...
	Если НЕ ПрямойОбменСБанками Тогда
		Если НЕ ЗначениеЗаполнено(Объект.Программа) Тогда
			ТекстСообщения = НСтр("ka='Не заполнено название программы в настройках обмена (команда ""Еще - Настройка"")!';ru='Не заполнено название программы в настройках обмена (команда ""Еще - Настройка"")!';en='It is not filled in the name of the program sharing settings (the command ""More - Settings"")!'");
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения, , , "Программа"); //, Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Объект.Кодировка) Тогда
			ТекстСообщения = НСтр("ka='Не заполнена кодировка!';ru='Не заполнена кодировка!';en='Not full charset!'");
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения, , , "Кодировка", Отказ);
			ОткрытьФормуНастройки = Истина;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Объект.ВерсияФормата) Тогда
			ТекстСообщения = НСтр("ka='Не указана версия формата обмена!';ru='Не указана версия формата обмена!';en='Not the version of the exchange format!'");
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ka='Устраните замечания и нажмите кнопку еще раз!';ru='Устраните замечания и нажмите кнопку еще раз!';en='Eliminate the comments and click again!'");
...
Процедура ЗаполнитьВыгрузка()
	...
	Для каждого СтрокаДокумента Из Выгрузка Цикл
		...
		Если ПустаяСтрока(СтрокаДокумента.ОписаниеОшибок) Тогда
			СтрокаДокумента.ОписаниеОшибокПодТЧ = НСтр("ka='Текущий документ готов к выгрузке! Проверьте значения в других строках, где есть предупреждающий знак.';ru='Текущий документ готов к выгрузке! Проверьте значения в других строках, где есть предупреждающий знак.';en='The current document is ready to be unloaded! Check the values ​​in other rows, where there is a warning sign.'");
		Иначе
	...
	Если Результат <> Неопределено Тогда
		Если Результат.Количество() > 0 Тогда
			...
			Если ПараметрыЗавершения.ВыгружатьИнтерактивно Тогда
				ТекстСообщения = НСтр("ka='Данные успешно выгружены в файл.';ru='Данные успешно выгружены в файл.';en='Data is successfully uploaded to the file.'");
			Иначе
				ТекстСообщения = НСтр("ru='Данные успешно выгружены в файл ';ka='Данные успешно выгружены в файл '") + Объект.ФайлВыгрузки + ".";
			КонецЕсли;
			Если Результат.Количество() > 1 Тогда
				ПоказатьПредупреждение(Неопределено, НСтр("ka='Выгружено более одного файла!!!';ru='Выгружено более одного файла!!!';en='Discharged more than one file !!!'"));
			КонецЕсли;
			
			ВыгрузитьФайлНаКлиенте(Результат[0].Имя, ПараметрыЗавершения.АдресФайлаВоВременномХранилище);
			
		Иначе
			ТекстСообщения = НСтр("ka='Операция отменена';ru='Операция отменена';en='operation canceled'");
		КонецЕсли;
...
"Выберите файл выгрузки. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.';ka='В настройках указан некорректный путь к файлу для выгрузки документов в банк - выбран ""каталог"" или указан неправильный путь к файлу."
...
"Выберите файл выгрузки. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.';ka='В настройках указан некорректный путь к файлу для выгрузки документов в банк."
"Выберите файл выгрузки. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстСообщения
			,, "Объект.ФайлВыгрузки");
...
Процедура ВыгрузитьФайлНаКлиентеЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ТекстЗаголовка = НСтр("ka='Данные успешно выгружены в файл';ru='Данные успешно выгружены в файл';en='The data was successfully uploaded a file'");
		...
		Если Объект.КонтролироватьБезопасностьОбменаСБанком Тогда
			...
			Если РасширениеРаботыСФайламиПодключено Тогда
				...
				Если НЕ ВыгружатьИнтерактивно Тогда
					...
					Если НЕ ЗначениеЗаполнено(Объект.ФайлВыгрузки) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							НСтр("ka='В настройках не указан файл данных для выгрузки документов в банк. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.';ru='В настройках не указан файл данных для выгрузки документов в банк. Для перехода к настройкам нажмите кнопку в правом верхнем углу формы.';en='In the settings you do not specify a data file to upload documents to the bank. To go to the settings, click the button in the upper right corner of the form.'")
							,, "Объект.ФайлВыгрузки");
						Возврат;
					КонецЕсли;
				...
				Иначе
					...
					Фильтр = НСтр("ka='Текстовый файл';ru='Текстовый файл';en='Text file'")
						+ " (*.txt)|*.txt";
...
"%1';ka='При записи файла возникла ошибка"
"%1'");
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='მონაცემთა ჩაწერა ფაილში ვერ მოხერხდა. შეიძლება, დისკი ჩაწერისგან დაცულია.';ru='Не удалось записать данные в файл. Возможно, диск защищен от записи.';en='Unable to write data to a file. The disk may be write-protected.'");
...
Процедура УправлениеФормойНаСервере()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменСБанками")
		...
		ТекстШаблона = НСтр("ka='С %1 действует соглашение о прямом обмене: платежные поручения будут отправлены в банк из 1С:Управление нашей фирмой';ru='С %1 действует соглашение о прямом обмене: платежные поручения будут отправлены в банк из 1С:Управление нашей фирмой';en='From% 1 an agreement on direct exchange: payment orders will be sent to the bank of the 1C: our company'");
...
Функция ВывестиПротокол()
	...
	//Заголовок = НСтр("ka='Протокол сохранения';ru='Протокол сохранения';en='preservation protocol'");
...
Процедура ВыгрузитьВыполнить(Команда)
	...
	Если Объект.Выгрузка.Количество() > 0 Тогда
		...
		Если Не ЕстьОтмеченныеСтроки Тогда
			...
			Сообщение.Текст = НСтр("ka='Не отмечено ни одной строки! Платежные поручения не будут выгружены. Нужно установить отметку в выгружаемых строках и нажать кнопку еще раз.';ru='Не отмечено ни одной строки! Платежные поручения не будут выгружены. Нужно установить отметку в выгружаемых строках и нажать кнопку еще раз.';en='There was no single line! Payment orders will not be offloaded. It is necessary to place a check mark in the uploaded row and press the button again.'");
...
"Проверьте правильность указанного банковского счета и периода выгрузки.';ka='Список документов для выгрузки пуст."
...
Процедура ОтправитьВБанк(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещенияВопрос, НСтр("ka='Данные будут выгружены в банк, продолжить?';ru='Данные будут выгружены в банк, продолжить?';en='The data will be uploaded to the bank to continue?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
...
Процедура ВыгрузкаОбновитьВыполнить(Команда)
	...
	Если Элементы.ВыгрузкаБанковскийСчет.Видимость И Объект.БанковскийСчет.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Выберите банковский счет вашей организации';ru='Выберите банковский счет вашей организации';en='Select the bank account of your organization'");
	...
	Если Объект.Выгрузка.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='По выбранному счету нет платежных поручений за указанный период';ru='По выбранному счету нет платежных поручений за указанный период';en='According to the selected account no payment orders for the period'"));
	КонецЕсли;
...
Процедура ЗагрузкаНомерКартинкиДляПодвалаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Элементы.Выгрузка.ТекущиеДанные.ОписаниеОшибок) Тогда
		ТекстПредупреждения = Элементы.Выгрузка.ТекущиеДанные.ОписаниеОшибок + 
			Символы.ПС + Символы.ПС + НСтр("ka='После устранения замечаний нажмите кнопку ""Обновить""';ru='После устранения замечаний нажмите кнопку ""Обновить""';en='After removal of comments, click the ""Update"" button'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Документ готов к выгрузке!';ru='Документ готов к выгрузке!';en='The document is ready to be unloaded!'"));
	КонецЕсли;
