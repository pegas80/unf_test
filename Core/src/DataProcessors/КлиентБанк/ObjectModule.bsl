		...
		Тогда
			...
			Если Не ЭтоНовыйДокумент Тогда
				...
				СтрокаИзмененныхРеквизитов = СтрокаИзмененныхРеквизитов + ?(СтрокаИзмененныхРеквизитов = "", "", Символы.ПС) + 
					ИмяСвойстваДляПротокола + ?(ЗначениеЗаполнено(ПредЗнач), НСтр("ru=' (с ';ka=' (с '") + ПредЗнач + НСтр("ru=' на ';ka='ზე'"), НСтр("ka='(';ru=' ('")) + ЗначениеСвойства + ")";
			КонецЕсли;
...
Функция ПолучитьПредставлениеОбъекта(Объект)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПоступлениеНаСчет") Тогда
		НазваниеОбъекта = НСтр("ka='документ ""Поступление на счет"" № %Номер% от %Дата%';ru='документ ""Поступление на счет"" № %Номер% от %Дата%';en='document ""to the account"" number% number% of% date%'"
		);
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.РасходСоСчета") Тогда
		НазваниеОбъекта = НСтр("ka='документ ""Расход со счета"" № %Номер% от %Дата%';ru='документ ""Расход со счета"" № %Номер% от %Дата%';en='document ""flow from the account"" number% number% of% date%'"
		);
		НазваниеОбъекта = СтрЗаменить(НазваниеОбъекта, "%Номер%", Строка(СокрЛП(Объект.Номер)));
		НазваниеОбъекта = СтрЗаменить(НазваниеОбъекта, "%Дата%", Строка(Объект.Дата));
	Иначе
		НазваниеОбъекта = НСтр("ka='ობიექტი';ru='объект';en='an object'");
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыРасходСоСчета(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, СтрокаИзмененныхРеквизитов)
	...
	Если ЭтоНовыйДокумент Тогда
		...
		Если СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='НДС не облагается';ru='НДС не облагается';en='NDS is not appearing'"))) > 0 ИЛИ 
			СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='დღგ-ის გარეშე';ru='Без НДС';en='Without VAT'"))) > 0 ИЛИ
			СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)';en='Without tax (VAT)'"))) > 0 ИЛИ
			СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='НДС не предусмотрен';ru='НДС не предусмотрен';en='VAT not applicable'"))) > 0
		Тогда
			...
			Если ОбъектДокумента.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
				...
				Если ОбъектДокумента.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
					...
					Если СтрНайти(ИсходныеДанные.НазначениеПлатежа, "НДС 18%") > 0 ИЛИ
						...
						СтавкаНДСВНазначенииПлатежа = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДС("НДС18");
...
Функция ЗадатьПометкуНаУдаление(ОбъектДляПометки, Пометка, ТаблицаРезультата = Неопределено, СтрокаСекции = Неопределено, ТаблицаПомеченныхНаУдалениеКакДубль = Неопределено)
	...
	НазваниеДействия = ?(Пометка, НСтр("ka=' помечен на удаление';ru=' помечен на удаление'"), НСтр("ka=' отменена пометка на удаление';ru=' отменена пометка на удаление'"));
	НазваниеДействияДляПротокола = ?(Пометка, НСтр("ka='• Помечен на удаление';ru='• Помечен на удаление';en='• For deletion'"), НСтр("ka='• Отменена пометка на удаление';ru='• Отменена пометка на удаление';en='• unmark for deletion'"));
	
	Попытка
		
		ОбъектДляПометки.Записать(РежимЗаписиДокумента.Запись);
		ОбъектДляПометки.УстановитьПометкуУдаления(Пометка);
		ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: %НазваниеДействия%.';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: %НазваниеДействия%.';en='%%% NazvanieObektaLev NazvanieObektaSred%:% NazvanieDeystviya%.'");
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: не %НазваниеДействия%! Произошли ошибки при записи!';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: не %НазваниеДействия%! Произошли ошибки при записи!';en='NazvanieObektaLev% %%% NazvanieObektaSred not NazvanieDeystviya%%! An error occurred while recording!'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеДействия%", НазваниеДействия);
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		Если ТаблицаРезультата <> Неопределено Тогда
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НазваниеДействияДляПротокола, Ложь, Ложь, НСтр("ka='• Произошли ошибки при записи!';ru='• Произошли ошибки при записи!';en='• An error occurred while recording!'"), ОписаниеОшибки());
		КонецЕсли;
...
Процедура ЗаписатьОбъект(ОбъектДляЗаписи, СтрокаСекции, ЭтоНовыйДокумент, ТаблицаРезультата, ЕстьПредупреждения, ВыдаватьСообщения, СтрокаИзмененныхРеквизитов)
	...
	Если ОбъектМодифицирован Тогда
		Попытка
			...
			//Если Не ЭтоНовыйДокумент Тогда
				ТекстСообщения = НСтр("ka='%Статус%%НазваниеОбъекта%.';ru='%Статус%%НазваниеОбъекта%.';en='% Status %% Name Accommodations%.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Статус%" , ?(ЭтоНовыйДокумент, НСтр("ru='Создан ';ka='Создан '"), НСтр("ru='Перезаписан ';ka='Перезаписан '")));
				...
				КонецЕсли;
				ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'"), ЭтоНовыйДокумент, Истина, 
					?(ЭтоНовыйДокумент, НСтр("ka='• Создан новый документ';ru='• Создан новый документ';en='• Create a new document'"), НСтр("ka='• Перезаписан документ';ru='• Перезаписан документ';en='• Overwrite document'")), "",,, 
			...
			КонецЕсли;
		Исключение
			ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% %Статус%! Произошли ошибки при записи!';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% %Статус%! Произошли ошибки при записи!';en='%%% NazvanieObektaLev NazvanieObektaSred%% status%! An error occurred while recording!'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Статус%", ?(ОбъектДляЗаписи.ЭтоНовый(), НСтр("ka=' не создан';ru=' не создан'"), НСтр("ka=' не записан';ru=' не записан'")));
			Если ВыдаватьСообщения Тогда
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ОписаниеОшибки());
			КонецЕсли;
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='Ошибка при загрузке';ru='Ошибка при загрузке';en='Error loading'"), ЭтоНовыйДокумент, Ложь, ТекстСообщения, ОписаниеОшибки());
			ЕстьПредупреждения = Истина;
			Возврат;
		КонецПопытки;
	Иначе
		ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не перезаписан, т.к. данные в нем не менялись. Возможно загрузка производилась ранее.';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не перезаписан, т.к. данные в нем не менялись. Возможно загрузка производилась ранее.';en='% NazvanieObektaLev %%% NazvanieObektaSred not overwritten because data in it have not changed. Ability to load previously been made.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
		Если ВыдаватьСообщения Тогда
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		КонецЕсли;
		ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'"), ЭтоНовыйДокумент, Ложь, "• Не перезаписан", ТекстСообщения);
		ЕстьПредупреждения = Истина;
	КонецЕсли;
	...
	Если ПроводитьЗагружаемые И (ОбъектМодифицирован ИЛИ НЕ ОбъектПроведен) 
		...
		Попытка
			...
			Если ОшибкиНеОбнаружены Тогда
				...
				ТекстСообщения = НСтр("ka='%Статус%%НазваниеОбъекта%';ru='%Статус%%НазваниеОбъекта%';en='% Status %% Name Accommodations%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Статус%", ?(ОбъектПроведен, НСтр("ru='Перепроведен ';ka='Перепроведен '"), НСтр("ru='Проведен ';ka='Проведен '")));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъекта%", НазваниеОбъекта);
				Если ВыдаватьСообщения Тогда
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
				КонецЕсли;
				СтрокаСекции.Проведен = ОбъектДляЗаписи.Проведен;
				ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'"), ЭтоНовыйДокумент, Истина, 
					?(ОбъектПроведен, НСтр("ru='• Перепроведен ';ka='• Перепроведен '"), НСтр("ru='• Проведен ';ka='• Проведен '")), ТекстСообщения);
			Иначе
				ТекстСообщения = НСтр("ka='%Статус% %НазваниеОбъекта% %Статус%';ru='%Статус% %НазваниеОбъекта% %Статус%';en='Status%%%%% Name Accommodations status%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Статус%", НСтр("ru='Не проведен ';ka='Не проведен '"));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъекта%", НазваниеОбъекта);
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='Заполнены не все реквизиты';ru='Заполнены не все реквизиты';en='Not filled in all the details'");
				Если ВыдаватьСообщения Тогда
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
				КонецЕсли;
				СтрокаСекции.Проведен = ОбъектДляЗаписи.Проведен;
				ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'"), ЭтоНовыйДокумент, Истина, 
					НСтр("ka='• Произошли ошибки при проведении (заполнены не все реквизиты)!';ru='• Произошли ошибки при проведении (заполнены не все реквизиты)!';en='• The following errors occurred during (not filled in all details)!'"), ТекстСообщения,, 1);
				ЕстьПредупреждения = Истина;
			КонецЕсли;
		Исключение
			// Поищем сообщения об ошибках.
			ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не проведен! Произошли ошибки при проведении!';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не проведен! Произошли ошибки при проведении!';en='%%% NazvanieObektaLev NazvanieObektaSred% not held! Errors occurred during!'");// + Символы.ПС + ОписаниеОшибки();
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
			Если ВыдаватьСообщения Тогда
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
			КонецЕсли;
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='Ошибка при загрузке';ru='Ошибка при загрузке';en='Error loading'"), ЭтоНовыйДокумент, Ложь, 
				НСтр("ka='• Произошли ошибки при проведении!';ru='• Произошли ошибки при проведении!';en='• An error occurred while carrying!'"), ТекстСообщения,, 1);
			ЕстьПредупреждения = Истина;
		КонецПопытки;
			...
			Если ИспользоватьГраницуОбработки Тогда
				Если ЗначениеЗаполнено(ДатаГраницыОбработки) Тогда
					Если НачалоДня(ОбъектДокументаКУдаление.Дата) <= НачалоДня(ДатаГраницыОбработки) Тогда
						ТекстСообщения = НСтр("ka='Платежный документ ""%ОбъектДокументаКУдаление%"" не помечен на удаление, так как имеет дату равной или меньшей границы обработки!';ru='Платежный документ ""%ОбъектДокументаКУдаление%"" не помечен на удаление, так как имеет дату равной или меньшей границы обработки!';en='Payment document ""% ObektDokumentaKUdalenie%"" is not marked for deletion, since it has a date equal to or smaller processing border!'");
...
Функция СоздатьКонтрагента(СтрокаКонтрагент = Неопределено) Экспорт
	...
	Если СообщатьОСозданномКонтрагенте Тогда
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Контрагент (%1) создан.';ru='Контрагент (%1) создан.';en='Counterparty (% 1) was created.'"), СтрокаКонтрагент.Представление);
	...
	Для каждого Строка Из СтрокаКонтрагент.ПолучитьЭлементы() Цикл
		...
		Если Строка.Представление = "Р/счет" Тогда
			...
			Исключение
				...
				Сообщение.Текст = НСтр("ka='Не удалось создать банковский счет контрагента!';ru='Не удалось создать банковский счет контрагента!';en='Failed to create a bank account of a company!'");
		...
		Если СообщатьОСозданномБанковскомСчете Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 добавлен банковский счет (%2).';ru='%1 добавлен банковский счет (%2).';en='1% added to a bank account (2%).'"), ПредставлениеКонтрагента, НомерСчета);
...
Процедура ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаТаблицы, Состояние, ЭтоНовый, Загружен, СписокДействий, 
	...
	ДокументСтрока = СтрокаТаблицы.ИмяДокумента + НСтр("ru=' №';ka=' №'") + СтрокаТаблицы.Номер + НСтр("ru=' от ';ka='ვისგან'") + СтрокаТаблицы.Дата;
	...
	Если ЭтоНовый И Не ЗначениеЗаполнено(СписокДействий) Тогда
		...
		СтрокаРезультата.Действия = НСтр("ka='• Создан новый документ';ru='• Создан новый документ';en='• Create a new document'");
...
Функция Загрузить(ИмпортЗаголовок, ТаблицаРезультата, ТаблицаПомеченныхНаУдалениеКакДубль) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		ТекстСообщения  = НСтр("ka='В заголовке файла загрузки неверно указана дата начала интервала! Файл не может быть загружен!';ru='В заголовке файла загрузки неверно указана дата начала интервала! Файл не может быть загружен!';en='The header file is loaded incorrectly indicate the date of the beginning of the interval! The file could not be loaded!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		ТекстСообщения = НСтр("ka='В заголовке файла импорта неверно указана дата окончания интервала!';ru='В заголовке файла импорта неверно указана дата окончания интервала!';en='In the header of the import file incorrectly indicate the date of the end of the interval!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
	КонецЕсли;
	...
	Для каждого СтрокаСекции Из ДокументыКИмпорту Цикл
		...
		Иначе
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='Пропущен, не загружается';ru='Пропущен, не загружается';en='Omitted, does not boot'"), Ложь, Ложь, НСтр("ka='• Пропущен, не загружается';ru='• Пропущен, не загружается';en='• Missing, not loaded'"), "");
		КонецЕсли;
...
Процедура ЗагрузитьСтроку(ЕстьПредупреждения, СтрокаСекции, ТаблицаРезультата = Неопределено, ТаблицаОбъектовСозданныхПриЗагрузке = Неопределено, ВыдаватьСообщения = Истина)
	...
	Если НЕ ПустаяСтрока(РезультатПроверки) Тогда
		ТекстСообщения = НСтр("ru='По документу ""%Операция%"" №%Номер% от %Дата% есть предупреждения:"
"%РезультатПроверки%!';ka='По документу ""%Операция%"" №%Номер% от %Дата% есть предупреждения:"
"%РезультатПроверки%!'"
		);
		...
		Если ВыдаватьСообщения Тогда
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		КонецЕсли;
		ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий';en='Комментарий'"), Ложь, Ложь, "", ТекстСообщения, , 1);
		ЕстьПредупреждения = Истина;
	КонецЕсли;
...
Функция ПересчитатьИзВалютыВВалюту(Сумма, КурсНач, КурсКон,	КратностьНач = 1, КратностьКон = 1) Экспорт
	...
	Если КурсНач = 0
	 ...
	 ИЛИ КратностьКон = 0 Тогда
		ТекстСообщения = НСтр("ka='აღმოჩენილია ვალუტის ნულოვანი კურსი. გადათვლა არ შესრულდა.';ru='Обнаружен нулевой курс валюты. Пересчет не выполнен.';en='Discovered a zero rate. Adjustment is not performed.'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		Возврат Сумма;
	КонецЕсли;
