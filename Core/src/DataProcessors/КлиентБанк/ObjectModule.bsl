...
Функция ПолучитьПредставлениеОбъекта(Объект)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПоступлениеНаСчет") Тогда
		НазваниеОбъекта = НСтр("ka='документ ""Поступление на счет"" № %Номер% от %Дата%';ru='документ ""Поступление на счет"" № %Номер% от %Дата%'"
		);
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.РасходСоСчета") Тогда
		НазваниеОбъекта = НСтр("ka='документ ""Расход со счета"" № %Номер% от %Дата%';ru='документ ""Расход со счета"" № %Номер% от %Дата%'"
		);
		НазваниеОбъекта = СтрЗаменить(НазваниеОбъекта, "%Номер%", Строка(СокрЛП(Объект.Номер)));
		НазваниеОбъекта = СтрЗаменить(НазваниеОбъекта, "%Дата%", Строка(Объект.Дата));
	Иначе
		НазваниеОбъекта = НСтр("ka='ობიექტი';ru='объект'");
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыРасходСоСчета(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, СтрокаИзмененныхРеквизитов)
	...
	Если ЭтоНовыйДокумент Тогда
		...
		Если СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='НДС не облагается';ru='НДС не облагается'"))) > 0 ИЛИ 
			СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='დღგ-ის გარეშე';ru='Без НДС'"))) > 0 ИЛИ
			СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)'"))) > 0 ИЛИ
			СтрНайти(ВРег(ИсходныеДанные.НазначениеПлатежа), ВРег(НСтр("ka='НДС не предусмотрен';ru='НДС не предусмотрен'"))) > 0
		Тогда
...
Функция ЗадатьПометкуНаУдаление(ОбъектДляПометки, Пометка, ТаблицаРезультата = Неопределено, СтрокаСекции = Неопределено, ТаблицаПомеченныхНаУдалениеКакДубль = Неопределено)
	...
	НазваниеДействияДляПротокола = ?(Пометка, НСтр("ka='• Помечен на удаление';ru='• Помечен на удаление'"), НСтр("ka='• Отменена пометка на удаление';ru='• Отменена пометка на удаление'"));
	
	Попытка
		
		ОбъектДляПометки.Записать(РежимЗаписиДокумента.Запись);
		ОбъектДляПометки.УстановитьПометкуУдаления(Пометка);
		ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: %НазваниеДействия%.';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: %НазваниеДействия%.'");
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: не %НазваниеДействия%! Произошли ошибки при записи!';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред%: не %НазваниеДействия%! Произошли ошибки при записи!'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеДействия%", НазваниеДействия);
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		Если ТаблицаРезультата <> Неопределено Тогда
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НазваниеДействияДляПротокола, Ложь, Ложь, НСтр("ka='• Произошли ошибки при записи!';ru='• Произошли ошибки при записи!'"), ОписаниеОшибки());
		КонецЕсли;
...
Процедура ЗаписатьОбъект(ОбъектДляЗаписи, СтрокаСекции, ЭтоНовыйДокумент, ТаблицаРезультата, ЕстьПредупреждения, ВыдаватьСообщения, СтрокаИзмененныхРеквизитов)
	...
	Если ОбъектМодифицирован Тогда
		Попытка
			...
			//Если Не ЭтоНовыйДокумент Тогда
				ТекстСообщения = НСтр("ka='%Статус%%НазваниеОбъекта%.';ru='%Статус%%НазваниеОбъекта%.'");
				...
				КонецЕсли;
				ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий'"), ЭтоНовыйДокумент, Истина, 
					?(ЭтоНовыйДокумент, НСтр("ka='• Создан новый документ';ru='• Создан новый документ'"), НСтр("ka='• Перезаписан документ';ru='• Перезаписан документ'")), "",,, 
			...
			КонецЕсли;
		Исключение
			ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% %Статус%! Произошли ошибки при записи!';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% %Статус%! Произошли ошибки при записи!'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
			...
			КонецЕсли;
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='Ошибка при загрузке';ru='Ошибка при загрузке'"), ЭтоНовыйДокумент, Ложь, ТекстСообщения, ОписаниеОшибки());
			ЕстьПредупреждения = Истина;
			Возврат;
		КонецПопытки;
	Иначе
		ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не перезаписан, т.к. данные в нем не менялись. Возможно загрузка производилась ранее.';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не перезаписан, т.к. данные в нем не менялись. Возможно загрузка производилась ранее.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
		Если ВыдаватьСообщения Тогда
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		КонецЕсли;
		ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий'"), ЭтоНовыйДокумент, Ложь, "• Не перезаписан", ТекстСообщения);
		ЕстьПредупреждения = Истина;
	КонецЕсли;
	...
	Если ПроводитьЗагружаемые И (ОбъектМодифицирован ИЛИ НЕ ОбъектПроведен) 
		...
		Попытка
			...
			Иначе
				ТекстСообщения = НСтр("ka='%Статус% %НазваниеОбъекта% %Статус%';ru='%Статус% %НазваниеОбъекта% %Статус%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Статус%", НСтр("ru='Не проведен ';ka='Не проведен '"));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъекта%", НазваниеОбъекта);
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ka='Заполнены не все реквизиты';ru='Заполнены не все реквизиты'");
				Если ВыдаватьСообщения Тогда
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
				КонецЕсли;
				СтрокаСекции.Проведен = ОбъектДляЗаписи.Проведен;
				ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий'"), ЭтоНовыйДокумент, Истина, 
					НСтр("ka='• Произошли ошибки при проведении (заполнены не все реквизиты)!';ru='• Произошли ошибки при проведении (заполнены не все реквизиты)!'"), ТекстСообщения,, 1);
				ЕстьПредупреждения = Истина;
			КонецЕсли;
		Исключение
			// Поищем сообщения об ошибках.
			ТекстСообщения = НСтр("ka='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не проведен! Произошли ошибки при проведении!';ru='%НазваниеОбъектаЛев%%НазваниеОбъектаСред% не проведен! Произошли ошибки при проведении!'");// + Символы.ПС + ОписаниеОшибки();
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаЛев%", ВРег(Лев(НазваниеОбъекта, 1)));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеОбъектаСред%", Сред(НазваниеОбъекта, 2));
			Если ВыдаватьСообщения Тогда
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
			КонецЕсли;
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='Ошибка при загрузке';ru='Ошибка при загрузке'"), ЭтоНовыйДокумент, Ложь, 
				НСтр("ka='• Произошли ошибки при проведении!';ru='• Произошли ошибки при проведении!'"), ТекстСообщения,, 1);
			ЕстьПредупреждения = Истина;
		КонецПопытки;
			...
			Если ИспользоватьГраницуОбработки Тогда
				Если ЗначениеЗаполнено(ДатаГраницыОбработки) Тогда
					Если НачалоДня(ОбъектДокументаКУдаление.Дата) <= НачалоДня(ДатаГраницыОбработки) Тогда
						ТекстСообщения = НСтр("ka='Платежный документ ""%ОбъектДокументаКУдаление%"" не помечен на удаление, так как имеет дату равной или меньшей границы обработки!';ru='Платежный документ ""%ОбъектДокументаКУдаление%"" не помечен на удаление, так как имеет дату равной или меньшей границы обработки!'");
...
Функция СоздатьКонтрагента(СтрокаКонтрагент = Неопределено) Экспорт
	...
	Если СообщатьОСозданномКонтрагенте Тогда
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Контрагент (%1) создан.';ru='Контрагент (%1) создан.'"), СтрокаКонтрагент.Представление);
	...
	Для каждого Строка Из СтрокаКонтрагент.ПолучитьЭлементы() Цикл
		...
		Если Строка.Представление = "Р/счет" Тогда
			...
			Исключение
				...
				Сообщение.Текст = НСтр("ka='Не удалось создать банковский счет контрагента!';ru='Не удалось создать банковский счет контрагента!'");
		...
		Если СообщатьОСозданномБанковскомСчете Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 добавлен банковский счет (%2).';ru='%1 добавлен банковский счет (%2).'"), ПредставлениеКонтрагента, НомерСчета);
...
Процедура ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаТаблицы, Состояние, ЭтоНовый, Загружен, СписокДействий, 
	...
	Если ЭтоНовый И Не ЗначениеЗаполнено(СписокДействий) Тогда
		...
		СтрокаРезультата.Действия = НСтр("ka='• Создан новый документ';ru='• Создан новый документ'");
...
Функция Загрузить(ИмпортЗаголовок, ТаблицаРезультата, ТаблицаПомеченныхНаУдалениеКакДубль) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		ТекстСообщения  = НСтр("ka='В заголовке файла загрузки неверно указана дата начала интервала! Файл не может быть загружен!';ru='В заголовке файла загрузки неверно указана дата начала интервала! Файл не может быть загружен!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		ТекстСообщения = НСтр("ka='В заголовке файла импорта неверно указана дата окончания интервала!';ru='В заголовке файла импорта неверно указана дата окончания интервала!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
	КонецЕсли;
	...
	Для каждого СтрокаСекции Из ДокументыКИмпорту Цикл
		...
		Иначе
			ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='Пропущен, не загружается';ru='Пропущен, не загружается'"), Ложь, Ложь, НСтр("ka='• Пропущен, не загружается';ru='• Пропущен, не загружается'"), "");
		КонецЕсли;
...
"%РезультатПроверки%!'"
		...
		Если ВыдаватьСообщения Тогда
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		КонецЕсли;
		ДобавитьСтрокуВТаблицуРезультат(ТаблицаРезультата, СтрокаСекции, НСтр("ka='კომენტარი';ru='Комментарий'"), Ложь, Ложь, "", ТекстСообщения, , 1);
		ЕстьПредупреждения = Истина;
	КонецЕсли;
...
Функция ПересчитатьИзВалютыВВалюту(Сумма, КурсНач, КурсКон,	КратностьНач = 1, КратностьКон = 1) Экспорт
	...
	Если КурсНач = 0
	 ...
	 ИЛИ КратностьКон = 0 Тогда
		ТекстСообщения = НСтр("ka='აღმოჩენილია ვალუტის ნულოვანი კურსი. გადათვლა არ შესრულდა.';ru='Обнаружен нулевой курс валюты. Пересчет не выполнен.'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения);
		Возврат Сумма;
	КонецЕсли;
