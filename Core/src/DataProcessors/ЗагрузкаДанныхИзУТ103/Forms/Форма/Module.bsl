...
Процедура Загрузить(Команда)
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ka='Ошибка при загрузке данных.';ru='Ошибка при загрузке данных.';en='Error loading data.'");
		Сообщение.Сообщить();
	КонецПопытки;
...
Процедура ЗагрузитьЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ka='მონაცემთა ჩატვირთვა...';ru='Загрузка данных...';en='Loading data ...'"),, НСтр("ka='ხორციელდება მონაცემთა ჩატვირთვა ""ვაჭრობის მართვა 10.3""-დან';ru='Выполняется загрузка данных из УТ 10.3';en='Download data from UT 10.3'"));
		ЗагрузитьНаСервере(АдресВременногоХранилища);
		
		ОбновитьИнтерфейс();
		СтандартныеПодсистемыКлиент.УстановитьРасширенныйЗаголовокПриложения();
		
		ПоказатьОповещениеПользователя(НСтр("ka='Загрузка завершена';ru='Загрузка завершена';en='loading is complete'"),,, БиблиотекаКартинок.Информация32);
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='Выберите путь к файлу выгрузки данных из УТ 10.3';ru='Выберите путь к файлу выгрузки данных из УТ 10.3';en='Select the path to the file upload data from UT 10.3'");
	...
	Иначе
		ПоказатьПредупреждение(Неопределено,НСтр("ka='Для выбора каталога необходимо установить расширение для работы с файлами в Веб-клиенте.';ru='Для выбора каталога необходимо установить расширение для работы с файлами в Веб-клиенте.';en='To select a directory, you must install the extension for the files in the Web client.'"));
	КонецЕсли;
