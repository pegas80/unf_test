...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбъектыДляОбработки.Количество() > 1 Тогда
		Элементы.ОбъектыДляОбработки.Заголовок = НСтр("ka='სია';ru='Список'");
	КонецЕсли;
	...
	Иначе
		ТекстГиперссылки = НСтр("ka='Электронные документы (%1)';ru='Электронные документы (%1)'");
...
Процедура ОК(Команда)
	...
	Если Не ЗначениеЗаполнено(Пользователь) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ответственный.';ru='Не указан ответственный.'"),, "Пользователь");
		Возврат;
	КонецЕсли;
	ВсегоЭД = 0;
