...
Процедура ИзменитьВидимостьДоступностьПриСозданииНаСервере()
	...
	Если ЗагрузкаЭД Тогда
		...
		Если ВРег(ВидЭД) = ВРег("РеквизитыОрганизации") Тогда
			...
			Заголовок = НСтр("ka='მონაცემთა ჩატვირთვა ფაილიდან';ru='Загрузка данных из файла'");
			
			Элементы.ГруппаНастроекСправочники.Видимость = Истина;
			Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
		Иначе
			
			Заголовок = НСтр("ka='Загрузка документа из файла';ru='Загрузка документа из файла'");
	...
	Иначе
		Текст = НСтр("ka='ელექტრონული დოკუმენტი';ru='Электронный документ'");
		Заголовок = Текст;
		Элементы.ГруппаНастроекСправочники.Видимость = Ложь;
		Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
	...
	Если ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
		...
		Элементы.ТипОбъекта.Заголовок = НСтр("ka='ჩატვირთვა';ru='Загрузить'");
		ТипОбъекта = НСтр("ka='საქონლის კატალოგი';ru='Каталог товаров'");
	КонецЕсли;
...
Процедура ВыполнитьПросмотрЭДСервер(СтруктураЭД, Отказ)
	...
	Если СтруктураЭД.ФайлАрхива Тогда
		...
		Исключение
			...
			КонецЕсли;
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='Распаковка архива ЭД';ru='Распаковка архива ЭД'"),
		...
		Если ФайлКарточки = Неопределено Или ФайлИнформации = Неопределено Тогда
			...
			ШаблонСообщения = НСтр("ka='Возникла ошибка при чтении данных из файла ""%1"".';ru='Возникла ошибка при чтении данных из файла ""%1"".'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураЭД.ИмяФайла);
			
			ШаблонСообщения = НСтр("ka='Файл ""%1"" не содержит электронных документов.';ru='Файл ""%1"" не содержит электронных документов.'");
	...
	Если ТипЗнч(СтруктураРазбора) = Тип("Структура") Тогда
		...
		Если ИсключаемыеТипыЭД.Получить(СтруктураРазбора.СтрокаОбъекта.ТипЭлементаВерсииЭД) <> Неопределено Тогда
			ШаблонСообщения = НСтр("ka='Загрузка из файла электронного документа вида ""%1"" не возможна.';ru='Загрузка из файла электронного документа вида ""%1"" не возможна.'");
...
Процедура СформироватьТекстСлужебногоСообщения()
	...
	Иначе
		...
		ЗаголовокЭлемента = НСтр("ka='Не найден файл электронного документа ""* .xml.""';ru='Не найден файл электронного документа ""* .xml.""'");
...
Процедура ЗагрузитьРеквизитыОрганизации()
	...
	Если СпособЗагрузкиДокумента = 1 И Не ЗначениеЗаполнено(ДокументИБ) Тогда
		ТекстСообщения = НСтр("ka='Не указан элемент справочника для перезаполнения.';ru='Не указан элемент справочника для перезаполнения.'");
...
Процедура ЗагрузитьДокументЭДО(ЗакрытьФорму)
	...
	Если СпособЗагрузкиДокумента = 1 Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументИБ) Тогда
			ТекстСообщения = НСтр("ka='Не указан документ для перезаполнения.';ru='Не указан документ для перезаполнения.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Отказ = Истина;
		КонецЕсли;
...
Процедура ЗагрузитьДокументВИБ(СопоставлятьНоменклатуруПередЗаполнениемДокумента, ОбновитьСтруктуруРазбора = Ложь)
	...
	Иначе
		...
		ТекстОповещения	= НСтр("ka='დოკუმენტი ატვირთულია.';ru='Документ загружен.'");
