...
Процедура ИзменитьВидимостьДоступностьПриСозданииНаСервере()
	...
	Если ЗагрузкаЭД Тогда
		...
		Если ВРег(ВидЭД) = ВРег("РеквизитыОрганизации") Тогда
			...
			Заголовок = НСтр("ka='მონაცემთა ჩატვირთვა ფაილიდან';ru='Загрузка данных из файла';en='Loading data from a file'");
			
			Элементы.ГруппаНастроекСправочники.Видимость = Истина;
			Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
		Иначе
			
			Заголовок = НСтр("ka='Загрузка документа из файла';ru='Загрузка документа из файла';en='Download the document from a file'");
	...
	Иначе
		Текст = НСтр("ka='ელექტრონული დოკუმენტი';ru='Электронный документ';en='Electronic document'");
		Заголовок = Текст;
		Элементы.ГруппаНастроекСправочники.Видимость = Ложь;
		Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
	...
	Если ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
		...
		Элементы.ТипОбъекта.Заголовок = НСтр("ka='ჩატვირთვა';ru='Загрузить';en='Download'");
		ТипОбъекта = НСтр("ka='საქონლის კატალოგი';ru='Каталог товаров';en='Catalog'");
	КонецЕсли;
...
Процедура ВыполнитьПросмотрЭДСервер(СтруктураЭД, Отказ)
	...
	Если СтруктураЭД.ФайлАрхива Тогда
		...
		Исключение
			...
			КонецЕсли;
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='Распаковка архива ЭД';ru='Распаковка архива ЭД';en='Unpacking the ED'"),
		...
		Если ФайлКарточки = Неопределено Или ФайлИнформации = Неопределено Тогда
			...
			ШаблонСообщения = НСтр("ka='Возникла ошибка при чтении данных из файла ""%1"".';ru='Возникла ошибка при чтении данных из файла ""%1"".';en='An error occurred while reading data from the file ""%1"".'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураЭД.ИмяФайла);
			
			ШаблонСообщения = НСтр("ka='Файл ""%1"" не содержит электронных документов.';ru='Файл ""%1"" не содержит электронных документов.';en='File ""%1"" does not contain any electronic documents.'");
	...
	Если ТипЗнч(СтруктураРазбора) = Тип("Структура") Тогда
		...
		Если ИсключаемыеТипыЭД.Получить(СтруктураРазбора.СтрокаОбъекта.ТипЭлементаВерсииЭД) <> Неопределено Тогда
			ШаблонСообщения = НСтр("ka='Загрузка из файла электронного документа вида ""%1"" не возможна.';ru='Загрузка из файла электронного документа вида ""%1"" не возможна.';en='Loading of the electronic document file type ""%1"" is not possible.'");
...
Процедура СформироватьТекстСлужебногоСообщения()
	...
	Иначе
		...
		ЗаголовокЭлемента = НСтр("ka='Не найден файл электронного документа ""* .xml.""';ru='Не найден файл электронного документа ""* .xml.""';en='Unable to find an electronic document file ""* .xml.""'");
...
Процедура ЗагрузитьРеквизитыОрганизации()
	...
	Если СпособЗагрузкиДокумента = 1 И Не ЗначениеЗаполнено(ДокументИБ) Тогда
		ТекстСообщения = НСтр("ka='Не указан элемент справочника для перезаполнения.';ru='Не указан элемент справочника для перезаполнения.';en='Unknown guide element for refilling.'");
...
Процедура ЗагрузитьДокументЭДО(ЗакрытьФорму)
	...
	Если СпособЗагрузкиДокумента = 1 Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументИБ) Тогда
			ТекстСообщения = НСтр("ka='Не указан документ для перезаполнения.';ru='Не указан документ для перезаполнения.';en='Unknown document for refilling.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Отказ = Истина;
		КонецЕсли;
...
Процедура ЗагрузитьДокументВИБ(СопоставлятьНоменклатуруПередЗаполнениемДокумента, ОбновитьСтруктуруРазбора = Ложь)
	...
	Иначе
		...
		ТекстОповещения	= НСтр("ka='დოკუმენტი ატვირთულია.';ru='Документ загружен.';en='The document is loaded.'");
