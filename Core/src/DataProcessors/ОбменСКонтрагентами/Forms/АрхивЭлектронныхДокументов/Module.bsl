...
Процедура КомандаОтправитьОповещение(КолОтправленныхПакетов, ДополнительныеПараметры) Экспорт
	...
	ТекстОповещения     = НСтр("ka='Отправленных пакетов нет';ru='Отправленных пакетов нет';en='Packets sent no'");
	
	Если ЗначениеЗаполнено(КолОтправленныхПакетов) Тогда
	
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Отправлено пакетов: (%1)';ru='Отправлено пакетов: (%1)';en='Sent packages (1%)'"), КолОтправленныхПакетов);
	КонецЕсли;
...
Процедура УстановитьСтатусКРаспаковке(Команда)
	...
	ТекстОповещения = НСтр("ka='Изменен статус пакетов на ""К распаковке""';ru='Изменен статус пакетов на ""К распаковке""';en='It changed the status of packages on the ""To unpack""'") + ": (%1)";
...
Процедура УстановитьСтатусОтменен(Команда)
	...
	ТекстОповещения = НСтр("ka='Изменен статус пакетов на ""Отменен""';ru='Изменен статус пакетов на ""Отменен""';en='It changed the status of packages on the ""Undo""'") + ": (%1)";
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами';en='Exchange of electronic documents'"), , ТекстОповещения);
	Элементы[ТаблицаПакетов].Обновить();
...
Процедура УстановитьСтатусПодготовленКОтправке(Команда)
	...
	ТекстОповещения = НСтр("ka='Изменен статус пакетов на ""Подготовлен к отправке""';ru='Изменен статус пакетов на ""Подготовлен к отправке""';en='It changed the status of packages on the ""prepared for shipment""'" + ": (%1)");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами';en='Exchange of electronic documents'"), , ТекстОповещения);
	Элементы[ТаблицаПакетов].Обновить();
...
Процедура УстановитьОтветственногоЭД(Знач СписокОбъектов, НовыйОтветственный, КоличествоОбработанныхЭД)
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстОшибки = НСтр("ka='Не удалось выполнить запись электронного документа (%Объект%). %ОписаниеОшибки%';ru='Не удалось выполнить запись электронного документа (%Объект%). %ОписаниеОшибки%';en='Unable to electronic document recording (% object%). % OpisanieOshibki%'");
...
Процедура СравнитьДанныеЭД(ТекущийСписок)
	...
	#Если НЕ ТолстыйКлиентУправляемоеПриложение И НЕ ТолстыйКлиентОбычноеПриложение Тогда
		ТекстСообщения = НСтр("ka='Сравнение электронных документов можно сделать только в режиме толстого клиента.';ru='Сравнение электронных документов можно сделать только в режиме толстого клиента.';en='Comparison of electronic documents can only be done in the thick client mode.'");
	...
	Для Каждого СтрокаТаблицы Из Элементы[ТаблицаПакетов].ВыделенныеСтроки Цикл
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ka='изменение статуса пакетов ЭД';ru='изменение статуса пакетов ЭД';en='ED packages status change'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Процедура ВыполнитьСравнениеЭД(СтруктураПараметров)
	...
	#Иначе
		...
		Если Не (ЗначениеЗаполнено(ЭДПервый) И ЗначениеЗаполнено(ЭДВторой)) Тогда
			ТекстСообщения = НСтр("ka='Не указан один из сравниваемых электронных документов.';ru='Не указан один из сравниваемых электронных документов.';en='Unknown one of the compared electronic documents.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ПереченьВременныхФайлов) Тогда
			ТекстСообщения = НСтр("ka='Ошибка при разборе электронного документа.';ru='Ошибка при разборе электронного документа.';en='Error while parsing an electronic document.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
