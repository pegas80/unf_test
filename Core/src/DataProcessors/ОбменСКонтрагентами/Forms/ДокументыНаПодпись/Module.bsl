...
Функция ЕстьДокументыНаПодпись() 
	...
	Если ПроверочныеДанные.КоличествоДокументов = 0 Тогда
		ТекстПредупреждения = НСтр("ka='По данному сертификату нет документов на подпись';ru='По данному сертификату нет документов на подпись'");
...
Процедура ПерейтиНаСтраницу(КДетализации)
	...
	Если КДетализации Тогда
		...
		Заголовок = НСтр("ka='Документы на подпись по сертификату';ru='Документы на подпись по сертификату'")+ ": " + СертификатПодписи;
	Иначе
		Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.СтраницаСводки;
		Заголовок = НСтр("ka='Документы на подпись';ru='Документы на подпись'");
	КонецЕсли;
	
	Элементы.Подписать.Заголовок = НСтр("ka='Подписать и отправить отмеченные';ru='Подписать и отправить отмеченные'");
