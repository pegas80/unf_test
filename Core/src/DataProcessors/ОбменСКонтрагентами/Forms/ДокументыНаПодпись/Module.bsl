...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ka='Для работы с ЭП необходимо установить"
"расширение работы с криптографией.';ru='Для работы с ЭП необходимо установить"
...
Функция ЕстьДокументыНаПодпись() 
	...
	Если ПроверочныеДанные.КоличествоДокументов = 0 Тогда
		ТекстПредупреждения = НСтр("ka='По данному сертификату нет документов на подпись';ru='По данному сертификату нет документов на подпись';en='For this certificate is not to sign documents'");
...
Процедура ПерейтиНаСтраницу(КДетализации)
	...
	Если КДетализации Тогда
		...
		Заголовок = НСтр("ka='Документы на подпись по сертификату';ru='Документы на подпись по сертификату';en='Documents to be signed by a certificate'")+ ": " + СертификатПодписи;
	Иначе
		Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.СтраницаСводки;
		Заголовок = НСтр("ka='Документы на подпись';ru='Документы на подпись';en='Documents to be signed'");
	КонецЕсли;
	
	Элементы.Подписать.Заголовок = НСтр("ka='Подписать и отправить отмеченные';ru='Подписать и отправить отмеченные';en='Sign and send the marked'");
...
"или не настроены правила подписи документов.';ka='Нет сертификатов подписи для пользователя"
