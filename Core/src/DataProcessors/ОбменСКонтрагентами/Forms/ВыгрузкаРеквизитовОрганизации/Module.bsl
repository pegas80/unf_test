...
Процедура ВыгрузитьРеквизиты(Команда)
	...
	Если СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
		Если НЕ ЗначениеЗаполнено(АдресВыгрузки) Тогда
			ТекстСообщения = НСтр("ka='Не выбрана учетная запись электронной почты.';ru='Не выбрана учетная запись электронной почты.';en='Do not select an e-mail account.'");
...
Процедура ВыгрузитьРеквизитыОрганизации(АдресХранилища, УникальныйИдентификатор)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Телефон) Тогда
			...
			ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(
				КонтактнаяИнформацияXDTO, "Тип", НСтр("ka='სამუშაო ტელეფონი';ru='Телефон рабочий';en='work phone'"), Истина, ТекстОшибки);
		...
		Иначе
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='ედ-ის ჩამოყალიბება';ru='Формирование ЭД';en='formation of ED'"),
																						ТекстОшибки,
																						ТекстОшибки);
		КонецЕсли;
...
Процедура РазобратьАдрес(ОбъектXDTO, Значение, ТекстОшибки)
	...
	Значение = СтрЗаменить(Значение, "Индекс", НСтр("ka='საფოსტო ინდექსი';ru='Почтовый индекс';en='Postcode'"));
	Значение = СтрЗаменить(Значение, "НаселенныйПункт", НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт';en='Locality'"));
...
Процедура ОбновитьФорму()
	...
	Если ТипУчастника = ИндивидуальныйПредприниматель Тогда
		...
		НадписьСвидетельство = НСтр("ka='%2-ის მოწმობა %%1';ru='Свидетельство №%1 от %2';en='Evidence №% 1% 2 from'");
