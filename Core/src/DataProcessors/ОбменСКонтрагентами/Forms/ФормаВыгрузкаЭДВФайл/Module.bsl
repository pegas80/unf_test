...
Процедура ИзменитьВидимостьДоступностьПриСозданииНаСервере()
	...
	Текст = НСтр("ka='Выгрузка документов в файл';ru='Выгрузка документов в файл';en='Uploading documents in the file'");
	ТекстГиперссылки = НСтр("ka='Документы не найдены.';ru='Документы не найдены.';en='Documents not found.'");
	Если ТаблицаДанных.Количество() > 1 Тогда
		ТекстГиперссылки = НСтр("ka='Открыть список электронных документов (%1)';ru='Открыть список электронных документов (%1)';en='View a list of electronic documents (% 1)'");
		ТекстГиперссылки = СтрЗаменить(ТекстГиперссылки, "%1", ТаблицаДанных.Количество());
	ИначеЕсли ТаблицаДанных.Количество() = 1 Тогда
		ТекстГиперссылки = НСтр("ka='Электронный документ: %1';ru='Электронный документ: %1';en='Electronic document% 1'");
...
Функция ВыгрузитьЭД(ТекстСообщения)
	...
	Если НЕ ЗначениеЗаполнено(СпособВыгрузки) Тогда
		ТекстСообщения = НСтр("ka='Необходимо указать способ выгрузки.';ru='Необходимо указать способ выгрузки.';en='You must specify the method of discharge.'");
		Отказ = Истина;
	КонецЕсли;
	Если СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту")
		...
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
			+ НСтр("ka='Необходимо указать учетную запись.';ru='Необходимо указать учетную запись.';en='You must specify an account.'");
		Отказ = Истина;
	КонецЕсли;
	Если НЕ Отказ Тогда
		СтруктураПараметров = Новый Структура;
...
Процедура БыстрыйОбменВыгрузитьЭД(МассивСтруктурОбмена, СтруктураПараметров)
	...
	Если СтруктураПараметров.СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
		...
		ТекстСообщения = НСтр("ka='Для сохранения документа необходимо установить расширение работы с файлами.';ru='Для сохранения документа необходимо установить расширение работы с файлами.';en='To save a document, you must install the extension to work with files.'");
	...
	Иначе
		...
		Если МассивСтруктурОбмена.Количество() > 1 Тогда
			ТемаПисьма = НСтр("ka='ელექტრონული დოკუმენტების პაკეტები';ru='Пакеты электронных документов';en='Packages of electronic documents'");
		Иначе
			ТемаПисьма = НСтр("ka='Пакет электронного документа:';ru='Пакет электронного документа:';en='The package of an electronic document:'") + " " + МассивСтруктурОбмена[0].НаименованиеФайла;
		КонецЕсли;
