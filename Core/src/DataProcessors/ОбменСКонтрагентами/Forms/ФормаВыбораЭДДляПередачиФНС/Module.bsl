...
"Вы действительно хотите закрыть форму?';ka='В списке выбора есть невыгруженные документы."
...
Процедура БыстрыеОтборыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(Элементы.БыстрыеОтборы.ТекущиеДанные.Параметр) = ВРег("СостояниеВерсииЭД") Тогда
		...
		СостоянияЭД.ПоказатьОтметкуЭлементов(ВыборЭлементаЗавершить,НСтр("ka='Состояния ЭД';ru='Состояния ЭД';en='ED status'"));
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Состояние ЭД:';ru='Состояние ЭД:';en='Status ED:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Только отраженные в учете:';ru='Только отраженные в учете:';en='Only the accounting for:'");
...
Процедура ДобавитьВВыбранныеДокументы(МассивСтрок)
	...
	ШаблонСообщения = НСтр("ka='Для документа ""%1"" не завершен электронный документооборот.';ru='Для документа ""%1"" не завершен электронный документооборот.';en='To document ""%1"" is not electronic document is completed.'");
	ТекстСообщения = "";
	...
	Если ВерсияВызова <> 1 И МножественныйВыбор = Ложь И ТаблицаВыбранныхДокументов.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			ТекстВопроса = ТекстСообщения + Символы.ПС + НСтр("ka='Продолжить выгрузку?';ru='Продолжить выгрузку?';en='Continue upload?'");
...
Функция СформироватьФайлВыгрузки(Знач ТаблицаДокументов)
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Если Файлы.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='Не удалось выгрузить документы по Организации ""%1"".';ru='Не удалось выгрузить документы по Организации ""%1"".';en='Unable to upload documents on the Organization''s ""%1"".'");
...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='Формирование выгрузки ЭД в 1с-Отчетность';ru='Формирование выгрузки ЭД в 1с-Отчетность';en='Formation discharge ED-1c Reporting'"),
...
#Если НЕ ВебКлиент Тогда
				...
				ДиалогВыбора.Заголовок = НСтр("ka='Выберите каталог для сохранения файла (файлов) выгрузки';ru='Выберите каталог для сохранения файла (файлов) выгрузки';en='Select the directory to save the file (s) upload'");
...
Процедура Выгрузить()
	...
	Если ТаблицаВыбранныхДокументов.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='Для формирования выгрузки необходимо выбрать хотя бы один документ.';ru='Для формирования выгрузки необходимо выбрать хотя бы один документ.';en='For the formation of the discharge must select at least one document.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе
...
"Продолжить выгрузку?';ka='В списке выбранных документов, присутствуют документы вида ""%1"","
"с незаполненными реквизитами документов-оснований (номер, дата)."
"Продолжить выгрузку?'");
