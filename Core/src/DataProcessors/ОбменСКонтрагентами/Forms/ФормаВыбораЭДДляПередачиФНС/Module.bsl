...
Процедура БыстрыеОтборыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(Элементы.БыстрыеОтборы.ТекущиеДанные.Параметр) = ВРег("СостояниеВерсииЭД") Тогда
		...
		СостоянияЭД.ПоказатьОтметкуЭлементов(ВыборЭлементаЗавершить,НСтр("ka='Состояния ЭД';ru='Состояния ЭД'"));
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Состояние ЭД:';ru='Состояние ЭД:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ka='Только отраженные в учете:';ru='Только отраженные в учете:'");
...
Процедура ДобавитьВВыбранныеДокументы(МассивСтрок)
	...
	ШаблонСообщения = НСтр("ka='Для документа ""%1"" не завершен электронный документооборот.';ru='Для документа ""%1"" не завершен электронный документооборот.'");
	ТекстСообщения = "";
	...
	Если ВерсияВызова <> 1 И МножественныйВыбор = Ложь И ТаблицаВыбранныхДокументов.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			ТекстВопроса = ТекстСообщения + Символы.ПС + НСтр("ka='Продолжить выгрузку?';ru='Продолжить выгрузку?'");
...
Функция СформироватьФайлВыгрузки(Знач ТаблицаДокументов)
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Если Файлы.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='Не удалось выгрузить документы по Организации ""%1"".';ru='Не удалось выгрузить документы по Организации ""%1"".'");
...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ka='Формирование выгрузки ЭД в 1с-Отчетность';ru='Формирование выгрузки ЭД в 1с-Отчетность'"),
...
#Если НЕ ВебКлиент Тогда
				...
				ДиалогВыбора.Заголовок = НСтр("ka='Выберите каталог для сохранения файла (файлов) выгрузки';ru='Выберите каталог для сохранения файла (файлов) выгрузки'");
...
Процедура Выгрузить()
	...
	Если ТаблицаВыбранныхДокументов.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='Для формирования выгрузки необходимо выбрать хотя бы один документ.';ru='Для формирования выгрузки необходимо выбрать хотя бы один документ.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе
