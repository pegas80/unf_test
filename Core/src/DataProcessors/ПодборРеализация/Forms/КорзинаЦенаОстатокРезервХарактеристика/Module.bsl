...
Процедура УстановитьПодсказкуВводаСтрокиПоискаНаСервере()
	...
	ПодсказкаВвода = ?(ПолнотекстовыйПоискНастроенЧастично, НСтр("ka='Необходимо обновить индекс полнотекстового поиска...';ru='Необходимо обновить индекс полнотекстового поиска...'"), НСтр("ka='(ALT+F3) Введите текст поиска...';ru='(ALT+F3) Введите текст поиска...'"));
...
Процедура ОтборПоАналогамНоменклатуры(Номенклатура, ТекстСообщения)
	...
	Если СписокАналоговНоменклатуры.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ka='Для номенклатуры не заведены аналоги.';ru='Для номенклатуры не заведены аналоги.'");
...
Процедура ПерейтиКСпискуХарактеристикНоменклатуры(НоменклатураСсылка)
	...
	Элементы.НавигацияСписковНоменклатураХарактеристики.Заголовок = НСтр("ka='ნომენკლატურა';ru='Номенклатура'");
...
Процедура ПерейтиКСпискуНоменклатуры()
	...
	Элементы.НавигацияСписковНоменклатураХарактеристики.Заголовок = НСтр("ka='მახასიათებლები';ru='Характеристики'");
...
Процедура ВыводитьСоветИспользоватьПредыдущийПодбор()
	...
	Если КешНастройкиПодбора.ВыводитьСоветИспользоватьПредыдущийПодбор Тогда
		...
		ЗаголовокФормы = Нстр("ka='Совет';ru='Совет'");
		ТекстСообщения = НСтр("ka='Выключить/включить использование новой формы подбора можно в настройках пользователя.';ru='Выключить/включить использование новой формы подбора можно в настройках пользователя.'");
...
Процедура ВыводитьСоветВернутьсяКНоменклатуре()
	...
	Если КешНастройкиПодбора.ВыводитьСоветВернутьсяКНоменклатуре Тогда
		...
		ЗаголовокФормы = Нстр("ka='Совет';ru='Совет'");
		ТекстСообщения = НСтр("ka='Вернуться к списку номенклатуры можно при помощи контекстного меню или клавиши BackSpace.';ru='Вернуться к списку номенклатуры можно при помощи контекстного меню или клавиши BackSpace.'");
...
Процедура ОтборПоСтруктурнойЕдинице(Команда)
	...
	Если ДанныеТекущейСтроки = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Выделите структурную единицу из списка складов';ru='Выделите структурную единицу из списка складов'"), 10);
		Возврат;
	...
	Если ЗначениеЗаполнено(ДанныеТекущейСтроки.СтруктурнаяЕдиница) Тогда
		...
		ЗаголовокДекорации = СтрШаблон(НСтр("ka='Удалить отбор по складу: %1';ru='Удалить отбор по складу: %1'"), Строка(ДанныеТекущейСтроки.СтруктурнаяЕдиница));
