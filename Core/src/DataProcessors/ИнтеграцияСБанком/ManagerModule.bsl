...
Функция ВыполнитьМетодСервиса(ИмяМетода, ПараметрыМетода = Неопределено, Режим = 1, Token, ЗапускатьРеальныйБанк = ЛОЖЬ) Экспорт
	...
	Если ИмяМетода = "api/documents/status/" Тогда
		...
		Если РезультатЗапроса = Неопределено Тогда 
			...
			Сообщение.Текст = НСтр("ka='ინფორმაციის გადაცემის შეცდომა! დოკუმენტს არ აქვს მინიჭებული უნიკალური გასაღები!';ru='Ошибка передачи информации! Документу не присвоен уникальный ключ!'");		
			Сообщение.Сообщить();
		Иначе 
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='BoG ის სერვისთან მიმართვის შეცდომა!';ru='Ошибка обращения к сервисам BoG!'") + Символы.ПС
	...
	ИначеЕсли ИмяМетода = "api/statement////" Тогда
		...
		Если Не ТипЗнч(РезультатЗапроса) = Тип("Массив") И Не РезультатЗапроса = Неопределено Тогда
			...
			Если РезультатЗапроса["Message"] = "An error has occurred." Тогда 
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='მითითებული პერიოდისთვის მონაცემები ვერ მოიძებნა!';ru='Не найдено данных на указанный период!'") + Символы.ПС
				+ НСтр("ru = 'Код ошибки: '; ka = 'შეცდომის კოდი: '") + "500" + Символы.ПС;
...
Функция ИсключениеПустойПакетДанных()
	...
	Возврат НСтр("ka='JSON: მონაცემების პაკეტი ცარიელია.';ru='JSON: Пустой пакет данных.'");
	
КонецФункции // ИсключениеНеожиданноеОкончаниеСтроки()

Функция ИсключениеНекорректныйПакетДанных(Индекс)
	
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არაკორექტული მონაცემების პაკეტი პოზიციაში [Индекс].';ru='JSON: Некорректный пакет данных в позиции [Индекс].'"),
...
Функция ИсключениеНеожиданноеОкончаниеПакетаДанных()
	...
	Возврат НСтр("ka='JSON: მონაცემების პაკეტის მოულოდნელი დასრულება.';ru='JSON: Неожиданное окончание пакета данных.'");
	
КонецФункции // ИсключениеНеожиданноеОкончаниеПакетаДанных()

Функция ИсключениеНеожиданноеОкончаниеМассива(Индекс)
	
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: მასივის მოულოდნელი დასრულება პოზიციაში [Индекс].';ru='JSON: Неожиданное окончание массива в позиции [Индекс].'"),
	Новый Структура("Индекс", Индекс));
...
Функция ИсключениеНеожиданноеОкончаниеОбъетка(Индекс)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: ობიექტის მოულოდნელი დასრულება პოზიციაში [Индекс].';ru='JSON: Неожиданное окончание объекта в позиции [Индекс].'"),
	Новый Структура("Индекс", Индекс));
...
Функция ИсключениеНекорректныйТипNull(Индекс)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არაკორექტული ტიპი Null პოზიციაში [Индекс].';ru='JSON: Некорректный тип Null в позиции [Индекс].'"),
	Новый Структура("Индекс", Индекс));
...
Функция ИсключениеНекорректныйТипБулево(Индекс)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არაკორექტული ტიპი boolean პოზიციაში [Индекс].';ru='JSON: Некорректный тип Булево в позиции [Индекс].'"),
	Новый Структура("Индекс", Индекс));
...
Функция ИсключениеНекорректныйТипНеопределено(Индекс)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არაკორექტული ტიპი განუსაზღვრელი პოზიციაში [Индекс].';ru='JSON: Некорректный тип Неопределено в позиции [Индекс].'"),
	Новый Структура("Индекс", Индекс));
...
Функция ИсключениеНекорректныйФорматСтроки(Индекс)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არაკორექტული სტრიქონი ფორმატი პოზიციაში [Индекс].';ru='JSON: Некорректный формат строки в позиции [Индекс].'"),
	Новый Структура("Индекс", Индекс));
...
Функция ИсключениеНекорректныйФорматДаты(Индекс, Значение)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: თარიღის არაკორექტული ფორმატი პოზიციაში [Индекс].';ru='JSON: Некорректный формат даты [Значение] в позиции [Индекс].'"),
...
Функция ИсключениеНекорректныйФорматЧисла(Индекс, Значение)
	...
	Если ПустаяСтрока(Значение) Тогда
		...
		Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არასწორი მონაცემის ტიპი პოზიციაში [Индекс].';ru='JSON: Неверный формат данных в позиции [Индекс].'"),
		Новый Структура("Индекс", Индекс));
		
	Иначе
		
		Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: არაკორექტული რიცხვის ფორმატი [Значение] у პოზიციაში [Индекс].';ru='JSON: Некорректный формат числа [Значение] в позиции [Индекс].'"),
...
Функция ИсключениеНедопустимыйСимвол(Индекс, Символ)
	...
	Если (Символ = Неопределено) Тогда
		...
		Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: დაუშვებელი სიმბოლო პოზიციაში [Индекс].';ru='JSON: Недопустимый символ в позиции [Индекс].'"),
		Новый Структура("Индекс", Индекс));
		
	Иначе
		
		Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: დაუშვებელი სიმბოლო პოზიციაში [Индекс], მოსალოდნელია [Символ].';ru='JSON: Недопустимый символ в позиции [Индекс], ожидается [Символ].'"),
...
Функция ИсключениеНеопознанныйТип(Индекс, Тип)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: დაუშვებელი ტიპი [Тип] პოზიციაში [Индекс].';ru='JSON: Недопустимый тип [Тип] в позиции [Индекс].'"),
...
Функция ИсключениеНевозможноПреобразоватьЗначение(Индекс, Значение)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: შეუძლებელია ტიპის გარდაქმნა [Значение] პოზიციაში [Индекс].';ru='JSON: Невозможно преобразовать значение [Значение] в позиции [Индекс].'"),
	Новый Структура("Индекс,Значение", Индекс, Символ(034) + Значение + Символ(034)));
...
Функция ИсключениеНевозможноПреобразоватьЗначениеНаКлиенте(Индекс, Значение)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: შეუძლებელია კლიენტის მხარეს გარდაიქმნას მნიშვნელობა [Значение] პოზიციაში [Индекс].';ru='JSON: Невозможно на клиенте преобразовать значение [Значение] в позиции [Индекс].'"),
	Новый Структура("Индекс,Значение", Индекс, Символ(034) + Значение + Символ(034)));
...
Функция ИсключениеНекорректныйПараметр(Параметр)
	...
	Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: პარამეტრის [Параметр] დაუშვებელი ტიპი .';ru='JSON: Недопустимое значение параметра [Параметр].'"),
...
Функция ИсключениеНедопустимыйТипКлюча(Индекс, Значение)
	...
	Если (Индекс = Неопределено) Тогда
		...
		Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: გასაღების [Тип] მნიშვნელობის დაუშვებელი ტიპი.';ru='JSON: Недопустимый тип значения ключа [Тип].'"),
		Новый Структура("Тип", ТипЗнч(Значение)));
		
	Иначе
		
		Возврат СформироватьСтрокуПоШаблону(НСтр("ka='JSON: გასაღების [Тип] მნიშვნელობის დაუშვებელი ტიპი პოზიციაში [Индекс].';ru='JSON: Недопустимый тип значения ключа [Тип] в позиции [Индекс].'"),
