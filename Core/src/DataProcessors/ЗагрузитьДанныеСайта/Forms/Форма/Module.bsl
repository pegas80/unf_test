...
Процедура ЗагрузитьДанные(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ВидЦен) И НЕ ЗагрузитьВидЦенССайта Тогда
		ТекстОшибки = НСтр("ka='Не выбран вид цен товаров';ru='Не выбран вид цен товаров'");
...
Процедура ПодробныеРезультатыЗагрузкиНажатие(Элемент)
	...
	Текст.Показать(НСтр("ka='Данные, полученные с сайта';ru='Данные, полученные с сайта'"));
...
Функция ЗапуститьЗагрузкуВФонеСервер(ИмяКоманды)
	...
	Если ИмяКоманды = "ПредварительнаяЗагрузка" Тогда
		...
		СтрокаЛогЗагрузкиССайта = НСтр("ka='Подключение к сайту ...';ru='Подключение к сайту ...'");
...
Процедура ОбработатьРезультатВыполненияЗадания()
	...
	Если Не ПустаяСтрока(АдресХранилища) Тогда
		...
		Если ТипЗнч(ДанныеРезультат) = Тип("Структура") И ДанныеРезультат.Свойство("ДанныеУспешноЗагружены") Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Данные успешно загружены';ru='Данные успешно загружены'"));
			ОбновитьИнтерфейс();
			Закрыть();
...
Функция НаименованиеФоновойЗагрузкиССайта() Экспорт
	...
	Возврат НСтр("ka='Загрузка данных сайта';ru='Загрузка данных сайта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
