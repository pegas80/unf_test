...
Процедура ПрочитатьДанныеСайта(Параметры, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	Если СтруктураКаталогов.Количество()=0 Тогда
		ЗаписатьОшибку(Параметры, НСтр("ka='Не удалось получить данные с сайта';ru='Не удалось получить данные с сайта';en='Failed to get the data from the site'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого КаталогСайта Из СтруктураКаталогов Цикл
		...
		СформироватьЛогЗагрузки(Параметры.ШагиЗагрузки, НСтр("ka='Загрузка :';ru='Загрузка :';en='Download:'") + КаталогСайта.Ключ);
	...
	Параметры.ШагиЗагрузки.Вставить(3, НСтр("ka='Обработано объектов: 100%';ru='Обработано объектов: 100%';en='Processed objects: 100%'"));
	...
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Иерархия товаров:';ru='Иерархия товаров:';en='The hierarchy of goods:'"), СписокГруппУпорядоченный, "Значение");	
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Категории товаров:';ru='Категории товаров:';en='Categories:'"), Категории);
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Товары:';ru='Товары:';en='Goods:'"), Товары);
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Картинки товаров:';ru='Картинки товаров:';en='Pictures of the goods:'"), КартинкиТоваров);
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Свойства товаров:';ru='Свойства товаров:';en='Attributes:'"), Свойства);
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Типы цен:';ru='Типы цен:';en='Types of prices:'"), ТипыЦен);
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Товарные предложения:';ru='Товарные предложения:';en='Product offers:'"), Предложения);
	ДобавитьБлокЛогов(Параметры, НСтр("ka='Картинки предложений:';ru='Картинки предложений:';en='Images offers:'"), КартинкиПредложений);
...
Процедура ЗагрузитьДанные(Параметры, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	СформироватьЛогЗагрузки(Параметры.ШагиЗагрузки, НСтр("ka='Настройка программы ...';ru='Настройка программы ...';en='Setting up the program ...'"));
	...
	Параметры.ШагиЗагрузки.Вставить(ПорядковыйНомерКаталога, НСтр("ka='Загружено: 100%';ru='Загружено: 100%';en='Loaded: 100%'"));
	СформироватьЛогЗагрузки(Параметры.ШагиЗагрузки, НСтр("ka='Загружено: 100%';ru='Загружено: 100%';en='Loaded: 100%'"));
...
Функция URLАдминзоны(Параметры, ОписаниеОшибки)
	...
	Если Соединение = Неопределено Тогда
		ОбменССайтом.ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='ავტორიზაცია არ არის შესრულებული.';ru='Авторизация не выполнена.';en='Authorization is not performed.'"));
		Возврат "";
	КонецЕсли;
...
Функция ЗагрузитьТипЦен(ТипЦенXDTO, УзелОбмена)
	...
	ДопРеквизиты.Вставить("Комментарий", НСтр("ka='Загружено с сайта';ru='Загружено с сайта';en='Uploaded to Site'"));
...
Функция УстановитьСоединениеССервером(Параметры, ОписаниеОшибки="")
	...
	СтрокаЛогЗагрузкиССайтаПодробно = НСтр("ka='Соединение установлено.';ru='Соединение установлено.';en='Connection is established.'") + Символы.ПС;
...
Процедура ЗагрузитьXMLССайтаНаСервереЮМИ(Параметры, СтруктураКаталогов, ОписаниеОшибки)
	...
	Если Соединение=Неопределено Тогда
		ЗаписатьОшибку(Параметры, НСтр("ka='Не удалось установить соединение';ru='Не удалось установить соединение';en='Could not connect'"));
		Возврат;
	КонецЕсли;
	...
	Пока НЕ КонецПередачи Цикл
		...
		Пока Истина Цикл
			...
			Если ДанныеСтрока = Неопределено Тогда 
				ЗаписатьОшибку(Параметры, НСтр("ka='Загрузка не выполнена.';ru='Загрузка не выполнена.';en='Loading failed.'"));
				Возврат;
			КонецЕсли;
			...
			Иначе
				...
				Если ПустаяСтрока(СтрокаXML) Тогда
					ЗаписатьОшибку(Параметры, НСтр("ka='სერვერიდან ჩატვირთულ მონაცემთა წაკითხვა ვერ მოხერხდა.';ru='Не удалось прочитать данные, загруженные с сервера.';en='Could not read data downloaded from the server.'"));
					Возврат;
				КонецЕсли;
			...
			Иначе
				...
				Если НачалоКоммерческаяИнформация<>0 Тогда
					...
					Если НомерПакета=Неопределено Тогда
						ТекстОшибки = НСтр("ka='Не удалось прочитать данные, загруженные с сервера. Не найден параметр НомерПакета';ru='Не удалось прочитать данные, загруженные с сервера. Не найден параметр НомерПакета';en='Could not read data downloaded from the server. is not found NomerPaketa'")
							+ Символы.ПС + НСтр("ka='Возможно, требуется обновление модулей сайта (см. справку)';ru='Возможно, требуется обновление модулей сайта (см. справку)';en='You may need to update the site modules (see. Help)'");
						ЗаписатьОшибку(Параметры, ТекстОшибки);
						Возврат;
					КонецЕсли;
				Иначе
					ЗаписатьОшибку(Параметры, НСтр("ka='Не удалось прочитать данные, загруженные с сервера. Не найден раздел КоммерческаяИнформация';ru='Не удалось прочитать данные, загруженные с сервера. Не найден раздел КоммерческаяИнформация';en='Could not read data downloaded from the server. Not found section KommercheskayaInformatsiya'"));
					Возврат;
				КонецЕсли;
