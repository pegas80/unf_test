...
Процедура ОткрытьФайл(Знач ИмяФайла) Экспорт
	...
	Если ТекущаяИнициализация Тогда
		...
		ВызватьИсключение НСтр("ka='Объект уже был инициализирован ранее!';ru='Объект уже был инициализирован ранее!';en='The object has already been initialized before!'");
	...
	Иначе
		...
		Если ПотокЧтения.ТипУзла <> ТипУзлаXML.НачалоЭлемента
			...
			ВызватьИсключение(НСтр("ka='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Data.';ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Data.';en='XML reading error. Invalid file format. Data element will start.'"));
		КонецЕсли;
...
Функция ПрочитатьОбъектДанныхИнформационнойБазы() Экспорт
	...
	Если ПотокЧтения.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если ПотокЧтения.Имя <> "DumpElement" Тогда
			ВызватьИсключение НСтр("ka='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента DumpElement.';ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента DumpElement.';en='XML reading error. Invalid file format. Start of DumpElement element.'");
		КонецЕсли;
...
"%1';ka='Ошибка копирования фрагмента исходного файла. Частично скопированный фрагмент:"
"%1'"),
				ЗаписьФрагмента.Закрыть());
		ЗаписьЖурналаРегистрации(НСтр("ka='Выгрузка/загрузка данных.Ошибка чтения XML';ru='Выгрузка/загрузка данных.Ошибка чтения XML';en='Upload / download dannyh.Oshibka XML reader'", 
...
"произошла ошибка:"
""
"%3.';ka='Ошибка при чтении данных из файла %1: при чтении фрагмента"
""
"%2"
""
"произошла ошибка:"
