...
Процедура Подключаемый_ОбработатьАктивизациюСтрокиСписка()
	...
	Если ТекущаяСтрока <> Неопределено
		...
		Дата = НСтр("ka='ფულის უწყისი ';ru='Ведомость за ';en='Report Money'") + Формат(ТекущаяСтрока.Дата, "ДЛФ=D");
		НомерБанковскогоСчета = СтруктураДанные.НомерБанковскогоСчета;
		Если СтруктураДанные.УчетВалютныхОпераций Тогда
			НомерБанковскогоСчета = НомерБанковскогоСчета + " (" + ?(ЗначениеЗаполнено(ТекущаяСтрока.Валюта), ТекущаяСтрока.Валюта, НСтр("ka='<валюта пустая>';ru='<валюта пустая>';en='<Currency empty>'")) + ")";
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Малышев 2024-07-05 ONESOFT-3315 - перевод
	БанковскиеВыписки.Параметры.УстановитьЗначениеПараметра("Фраза_КассаКасса", НСтр("ka='სალარო/სალარო';ru='Касса/Касса';en='Cashier/Cashier'"));
	БанковскиеВыписки.Параметры.УстановитьЗначениеПараметра("Фраза_КассаБанк", НСтр("ka='სალარო/ბანკი';ru='Касса/Банк';en='Cashier/Bank'"));
	БанковскиеВыписки.Параметры.УстановитьЗначениеПараметра("Фраза_БанкБанк", НСтр("ka='ბანკი/ბანკი';ru='Банк/Банк';en='Bank/Bank'"));
	БанковскиеВыписки.Параметры.УстановитьЗначениеПараметра("Фраза_БанкКасса", НСтр("ka='ბანკი/სალარო';ru='Банк/Касса';en='Bank/Cashier'"));
	БанковскиеВыписки.Параметры.УстановитьЗначениеПараметра("Фраза_Касса", НСтр("ka='სალარო';ru='Касса';en='Cashier'"));
	БанковскиеВыписки.Параметры.УстановитьЗначениеПараметра("Фраза_Банк", НСтр("ka='ბანკი';ru='Банк';en='Bank'"));
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаСписка",
		Неопределено,
		НСтр("ka='Новости: Банк и касса';ru='Новости: Банк и касса';en='News: Bank and insurance fund'"),
		Ложь,
...
Процедура НастройкаИтоговЗавершениеНаСервере() Экспорт
	...
	Если ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ВедомостьЗаДень") Тогда
		...
		Элементы.ИнформацияСуммаВалНачальныйОстаток.Заголовок = НСтр("ka='დღის დასაწყისი';ru='Начало дня';en='Beginning of the day'");
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Заголовок = НСтр("ka='დღის დასასრული';ru='Конец дня';en='End of the day'");
		
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Шрифт = Новый Шрифт(Элементы.ИнформацияСуммаВалКонечныйОстаток.Шрифт,,, Ложь);
		
		ТаблицаИтогов[0].Показатель = НСтр("ka='დღის დასაწყისი';ru='Начало дня';en='Beginning of the day'");
		ТаблицаИтогов[3].Показатель = НСтр("ka='დღის დასასრული';ru='Конец дня';en='End of the day'");
	...
	ИначеЕсли ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ВедомостьЗаПериод") Тогда
		...
		//Элементы.ДекорацияВедомостьПоВалюте.Заголовок = НСтр("ka='Ведомость по валюте ';ru='Ведомость по валюте '")+""""+ОтборВалютаВедомости + """";
		Элементы.ДекорацияВедомостьПоВалюте.Заголовок = НСтр("ka='Ведомость по валюте: ';ru='Ведомость по валюте: '")+ОтборВалютаВедомости;
		...
		Если НЕ ЗначениеЗаполнено(ОтборПериод.ДатаНачала) И НЕ ЗначениеЗаполнено(ОтборПериод.ДатаОкончания) Тогда
			ТаблицаИтогов[0].Показатель = НСтр("ka='მთელი პერიოდი';ru='За все время';en='During all this time'");
			ТаблицаИтогов[3].Показатель = НСтр("ka='ნაშთი';ru='Остаток';en='the balance'");
		Иначе
			ТаблицаИтогов[0].Показатель = НСтр("ru='С ';ka='С '")+Формат(ОтборПериод.ДатаНачала, "ДФ=dd.MM.yyyy");
			ТаблицаИтогов[3].Показатель = НСтр("ka='По ';ru='По '")+Формат(ОтборПериод.ДатаОкончания, "ДФ=dd.MM.yyyy");
		КонецЕсли;
	...
	ИначеЕсли ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ОстаткиПересчитанныеВВалюту") Тогда
		...
		Элементы.ОтборКурс.Заголовок = НСтр("ka='Курс на ';ru='Курс на '")+Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy")+"";
		ОтборКурс = ""+СтруктураДанных.Курс;
		ИнформацияСуммаВалКонечныйОстаток = СтруктураДанных.Остаток;
		Элементы.ДекорацияОстатокВВалюте.Заголовок = НСтр("ka='Остатки в валюте: ';ru='Остатки в валюте: '")+ОтборВалютаОстатков;
		...
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Заголовок = НСтр("ka='ნაშთი';ru='Остаток';en='the balance'");
...
Процедура ОтборХозОперацияНачалоВыбораЗавершение(РезультатЗавершения, ПараметрыЗавершения) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатЗавершения) Тогда
		Если ТипЗнч(РезультатЗавершения) = Тип("Строка") Тогда
			...
			УстановитьМеткуИОтборСписка("БанковскиеВыписки", "ХозяйственнаяОперация", "ГруппаОтборХозОперация", "" + РезультатЗавершения);  //Малышев 2024-07-07 
		Иначе
			СтандартнаяОбработка = Ложь;
...
// Обработчик команды "РасходВыдачаКредитаБанк" формы.
//
&НаКлиенте
Процедура РасходВыдачаКредитаБанк(Команда)
	
	ДвиженияДенежныхСредствКлиент.СоздатьДокумент("РасходСоСчета", "ВыдачаКредитаКонтрагенту", ДанныеМеток, "БанковскиеВыписки");
...
&НаКлиенте
Процедура ПоступлениеВозвратКредитаКонтрагентомБанк(Команда)
	
	ДвиженияДенежныхСредствКлиент.СоздатьДокумент("ПоступлениеНаСчет", "ВозвратКредитаКонтрагентом", ДанныеМеток, "БанковскиеВыписки");
...
&НаКлиенте
Процедура ПоступлениеВозвратКредитаКонтрагентомКасса(Команда)
	
	ДвиженияДенежныхСредствКлиент.СоздатьДокумент("ПоступлениеВКассу", "ВозвратКредитаКонтрагентом", ДанныеМеток, "БанковскиеВыписки");
...
&НаКлиенте
Процедура РасходВыдачаКредитаКасса(Команда)
	
	ДвиженияДенежныхСредствКлиент.СоздатьДокумент("РасходИзКассы", "ВыдачаКредитаКонтрагенту", ДанныеМеток, "БанковскиеВыписки");
