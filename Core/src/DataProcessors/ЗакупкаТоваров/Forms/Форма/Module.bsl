...
Процедура ПоказатьНулевыеПродажи(Команда)
	...
	Если Потребность.ПолучитьЭлементы().Количество() > 0 Тогда
		ТекстВопроса = НСтр("ka='ცხრილური ნაწილი გადაივსება. გაგრძელდეს?';ru='Табличная часть будет перезаполнена. Продолжить?'");
		Ответ = Неопределено;
...
Процедура СформироватьЗаказы(Команда)
	...
	Если СформироватьЗаказыСервер() Тогда
		...
		ПоказатьОповещениеПользователя(
			,,
			НСтр("ka='Заказы поставщикам успешно созданы.';ru='Заказы поставщикам успешно созданы.'"),
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='შეკვეთების ფორმორებისთვის არ არის საკმარისი ინფორმაცია.';ru='Нет данных для формирования заказов.'");
		Сообщение.Сообщить();
...
Процедура УдалитьЗаказ(Команда)
	...
	ИначеЕсли ЗаказыНаУдаление.Количество() = 1 Тогда
		...
		ТекстВопроса = НСтр("ka='Выбранный документ будет удален. Продолжить?';ru='Выбранный документ будет удален. Продолжить?'");
		
	Иначе
		
		ТекстВопроса = НСтр("ka='Выбранные документы будут удалены. Продолжить?';ru='Выбранные документы будут удалены. Продолжить?'");
...
Функция СформироватьЗаказыСервер()
	...
	Попытка
		...
		Для каждого СтрокаПоставщик Из ДеревоПотребность.Строки Цикл
			...
			ДокументОбъект.Комментарий = НСтр("ka='შექმნილია ავტომატურად დამუშავებით ""მარაგების მოთხოვნილების გამოანგარიშება""';ru='Создан автоматически обработкой ""Расчет потребности товаров""'");
	...
	Для каждого СтрокаТаблицы Из Заказы Цикл
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა: %1.';ru='Не удалось провести документ: %1.'"), Строка(ДокументОбъект));
...
Процедура УдалитьЗаказыСервер()
	...
	Для Каждого СтрокаТаблицы Из Заказы Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დოკუმენტის წაშლაზე მონიშვნა ვერ მოხერხდა: %1.';ru='Не удалось пометить на удаление документ: %1.'"), Строка(ДокументОбъект));
