...
Процедура ПоказатьНулевыеПродажи(Команда)
	...
	Если Потребность.ПолучитьЭлементы().Количество() > 0 Тогда
		ТекстВопроса = НСтр("ka='ცხრილური ნაწილი გადაივსება. გაგრძელდეს?';ru='Табличная часть будет перезаполнена. Продолжить?';en='Tabular part will be refilled. Proceed?'");
		Ответ = Неопределено;
...
Процедура СформироватьЗаказы(Команда)
	...
	Если СформироватьЗаказыСервер() Тогда
		...
		ПоказатьОповещениеПользователя(
			,,
			НСтр("ka='Заказы поставщикам успешно созданы.';ru='Заказы поставщикам успешно созданы.';en='Orders to suppliers successfully created.'"),
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='შეკვეთების ფორმორებისთვის არ არის საკმარისი ინფორმაცია.';ru='Нет данных для формирования заказов.';en='No data to generate orders.'");
		Сообщение.Сообщить();
...
Процедура УдалитьЗаказ(Команда)
	...
	ИначеЕсли ЗаказыНаУдаление.Количество() = 1 Тогда
		...
		ТекстВопроса = НСтр("ka='Выбранный документ будет удален. Продолжить?';ru='Выбранный документ будет удален. Продолжить?';en='The selected document will be deleted. Proceed?'");
		
	Иначе
		
		ТекстВопроса = НСтр("ka='Выбранные документы будут удалены. Продолжить?';ru='Выбранные документы будут удалены. Продолжить?';en='Selected documents will be deleted. Proceed?'");
...
Функция СформироватьЗаказыСервер()
	...
	Попытка
		...
		Для каждого СтрокаПоставщик Из ДеревоПотребность.Строки Цикл
			...
			ДокументОбъект.Комментарий = НСтр("ka='შექმნილია ავტომატურად დამუშავებით ""მარაგების მოთხოვნილების გამოანგარიშება""';ru='Создан автоматически обработкой ""Расчет потребности товаров""';en='Created automatically processing ""Calculation of the need of the goods""'");
...
"%ДополнительноеОписание%';ka='При формировании заказов произошла ошибка."
"Формирование заказов отменено."
"Дополнительное описание:"
	...
	Для каждого СтрокаТаблицы Из Заказы Цикл
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა: %1.';ru='Не удалось провести документ: %1.';en='Unable to hold the document:% 1.'"), Строка(ДокументОбъект));
...
Процедура УдалитьЗаказыСервер()
	...
	Для Каждого СтрокаТаблицы Из Заказы Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დოკუმენტის წაშლაზე მონიშვნა ვერ მოხერხდა: %1.';ru='Не удалось пометить на удаление документ: %1.';en='Failed to mark for deletion document:% 1.'"), Строка(ДокументОбъект));
