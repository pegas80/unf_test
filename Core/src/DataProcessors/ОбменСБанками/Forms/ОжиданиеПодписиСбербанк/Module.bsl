...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТипУстройства = "SCR" Тогда // экранный
		Элементы.Пояснение.Заголовок = НСтр("ka='Подтвердите подпись на электронном ключе';ru='Подтвердите подпись на электронном ключе';en='Confirm the signature on the electronic key'");
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьУстановкуПодписи()
	...
	Элементы.Отсчет.Заголовок = СтрШаблон(НСтр("ka='%1 сек.';ru='%1 сек.';en='% 1 seconds.'"), Счетчик);
...
"Код состояния: %1'");
		...
		ТекстСообщения = НСтр("ru='При подписании электронного документа произошла ошибка';en='When signing an electronic document encountered an error'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1, Параметры.ОбъектПодписи);
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
		Закрыть(Результат);
	ИначеЕсли Состояние = "REJECTED" Тогда
		ТекстОповещения = НСтр("ru='Подпись документа не подтверждена';en='Signature of the document has not been confirmed'");
...
Процедура ОбработатьОшибкуПолученияДанныхПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ВидОперации = НСтр("ka='ხლმოწერის მონაცემების მიღება';ru='Получение данных подписи.';en='Obtaining signature data.'");
