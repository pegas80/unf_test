...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТипУстройства = "SCR" Тогда // экранный
		Элементы.Пояснение.Заголовок = НСтр("ka='Подтвердите подпись на электронном ключе';ru='Подтвердите подпись на электронном ключе'");
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьУстановкуПодписи()
	...
	Элементы.Отсчет.Заголовок = СтрШаблон(НСтр("ka='%1 сек.';ru='%1 сек.'"), Счетчик);
...
Процедура ОбработатьОшибкуПолученияДанныхПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ВидОперации = НСтр("ka='ხლმოწერის მონაცემების მიღება';ru='Получение данных подписи.'");
