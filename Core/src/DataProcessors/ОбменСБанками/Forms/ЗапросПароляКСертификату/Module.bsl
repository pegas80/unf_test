...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа';en='The signing of the electronic document'")
		ИЛИ Параметры.ВидОперации = НСтр("ka='Подписание электронных документов';ru='Подписание электронных документов';en='The signing of electronic documents'") Тогда
		Элементы.Выбрать.Заголовок = НСтр("ka='ხელმოწერა';ru='Подписать';en='Sign'");
	КонецЕсли;
	...
	Если ТипЗнч(Параметры.ОбъектыДляОбработки) = Тип("Массив") И Параметры.ОбъектыДляОбработки.Количество() > 0 Тогда
		Если Параметры.ОбъектыДляОбработки.Количество() = 1 Тогда
			...
			ШаблонГиперссылки = НСтр("ka='%1 № %2  %3 -დან';ru='%1 № %2 от %3';en='№% 1% 2% 3 from'");
		...
		Иначе
			...
			ШаблонГиперссылки = НСтр("ka='Электронные документы (%1)';ru='Электронные документы (%1)';en='Electronic documents (1%)'");
...
Процедура КомандаГотово(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВводПароляКСертификату
		...
		ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
			"Поле", "Заполнение", НСтр("ka='Сертификат подписи';ru='Сертификат подписи';en='signing certificate'"));
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВводЛогинаИПароля И ПустаяСтрока(Пользователь) Тогда
		ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
			"Поле", "Заполнение", НСтр("ka='მომხმარებელი';ru='Пользователь';en='User'"));
