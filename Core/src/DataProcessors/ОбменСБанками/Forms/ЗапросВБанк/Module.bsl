...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СообщениеОбмена) Тогда
		...
		Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросОСостоянииЭД Тогда
			Заголовок = НСтр("ka='Получение состояния электронного документа';ru='Получение состояния электронного документа';en='Obtaining the status of the electronic document'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросВыписки Тогда
			// не меняем предопределенный
		Иначе
			Заголовок = НСтр("ka='Тестирование обмена данными';ru='Тестирование обмена данными';en='Testing data exchange'");
		КонецЕсли;
	КонецЕсли;
...
Функция ОтправитьЗапросВыпискиНаСервере(Знач МассивЗапросовСсылка, Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания)
	...
	НаименованиеЗадания = НСтр("ka='ამონაწერის მოთხოვნის გაგზავნა ბანკში';ru='Отправка запроса выписки в банк';en='Sending a request to the bank statement'");
...
Функция ОтправитьЗапросСостоянияЭДНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, ЕстьОшибка)
	...
	НаименованиеЗадания = НСтр("ka='Отправка запроса состояния ЭД в банк';ru='Отправка запроса состояния ЭД в банк';en='Sending ED status request to the bank'");
	ПараметрыВыполнения = Новый Массив;
	ПараметрыВыполнения.Добавить(ПараметрыЗапроса);
	ПараметрыВыполнения.Добавить(АдресХранилища);
...
Процедура ОбработатьРезультатЗапросаВыписки()
	...
	ТекстОповещения = НСтр("ka='Отправлено документов: (1)';ru='Отправлено документов: (1)';en='Submitted documents: (1)'");
...
Функция ПолучитьВыпискуАсинхронноНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, ЕстьОшибка)
	...
	НаименованиеЗадания = НСтр("ka='Получение выписки из банка';ru='Получение выписки из банка';en='Receiving bank statement'");
	ПараметрыВыполнения = Новый Массив;
	ПараметрыВыполнения.Добавить(ПараметрыЗапроса);
	ПараметрыВыполнения.Добавить(АдресХранилища);
...
Процедура ПослеСохраненияПолученныхПодписейВыписки(ЕстьОшибка, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыпискиБанка.Количество() Тогда
		ЗаголовокОповещения = НСтр("ka='1С:დირექტბანკი';ru='1С:ДиректБанк';en='1C: DirektBank'");
		ТекстОповещения = НСтр("ka='Получено документов: (%1).';ru='Получено документов: (%1).';en='Retrieved documents: (1%).'");
...
"Получено документов: (%1).';ka='Отправлено документов: (1)."
...
Процедура ПолучитьВыпискуЧерезДополнительнуюОбработку()
	...
	НазваниеЭД = НСтр("ka='Выписка банка за период с %1 по %2';ru='Выписка банка за период с %1 по %2';en='Extract the bank for the period from% 1 to% 2'");
	...
	Если КолПолученных = 0 Тогда
		ТекстОповещения = НСтр("ka='Полученных документов нет.';ru='Полученных документов нет.';en='Received no documents.'");
	Иначе
		ТекстОповещения = НСтр("ka='Получено документов: (%1).';ru='Получено документов: (%1).';en='Retrieved documents: (1%).'");
...
Функция ЗапросОтклонен(Знач СообщениеОбменаЗапрос)
	...
	ИначеЕсли РеквизитыСообщенияОбмена.Статус = Перечисления.СтатусыОбменСБанками.ОтклоненБанком
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Запрос не выполнен';ru='Запрос не выполнен';en='The request is not configured'"));
		Возврат Истина;
	КонецЕсли;
