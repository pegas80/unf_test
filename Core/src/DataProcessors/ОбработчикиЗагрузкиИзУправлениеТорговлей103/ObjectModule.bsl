...
Процедура ПКО_ПослеЗагрузки_ДокументыФизическихЛиц(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ТекстСерия				= НСтр("ka=', სერია: %1';ru=', серия: %1'");
	ТекстНомер				= НСтр("ka=', № %1';ru=', № %1'");
	ТекстДатаВыдачи			= НСтр("ka=', გაცემულია: %1 წლის';ru=', выдан: %1 года'");
	ТекстСрокДействия		= НСтр("ka=', მოქმედებს: %1 წლამდე';ru=', действует до: %1 года'");
	ТекстКодПодразделения	= НСтр("ka=', № подр. %1';ru=', № подр. %1'");
