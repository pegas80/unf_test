	...
	Если РежимРаботы.МодельСервиса Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации("Загрузка из УТ 10.3",
				...
				,
				,
				НСтр("ru='Не удалось инициализировать константы по причине: ';ka='Не удалось инициализировать константы по причине: '") + ОписаниеОшибки());
		КонецПопытки;
...
Процедура ПКО_ПослеЗагрузки_ДокументыФизическихЛиц(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ТекстСерия				= НСтр("ka=', სერია: %1';ru=', серия: %1';en='Series: 1%'");
	ТекстНомер				= НСтр("ka=', № %1';ru=', № %1'");
	ТекстДатаВыдачи			= НСтр("ka=', გაცემულია: %1 წლის';ru=', выдан: %1 года';en=', Issued:%1 year'");
	ТекстСрокДействия		= НСтр("ka=', მოქმედებს: %1 წლამდე';ru=', действует до: %1 года';en='Valid until% 1 year'");
	ТекстКодПодразделения	= НСтр("ka=', № подр. %1';ru=', № подр. %1';en=', Consecutive number. %one'");
