...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстОрганизации = НСтр("ka='Для отправки документа подключите организацию ""%1"" к сервису 1С:Бизнес-сеть.';ru='Для отправки документа подключите организацию ""%1"" к сервису 1С:Бизнес-сеть.';en='To send a document to connect the organization ""%1"" to the service of 1C: Business Network.'");
	ТекстОрганизации = СтрШаблон(ТекстОрганизации, Организация);
	Элементы.ТекстПодключенияОрганизации.Заголовок = ТекстОрганизации;
	
	ТекстКонтрагента = НСтр("ka='Контрагент ""%1"" не подключен к сервису 1С:Бизнес-сеть.';ru='Контрагент ""%1"" не подключен к сервису 1С:Бизнес-сеть.';en='Counterparty ""%1"" is not connected to the service of 1C: Business Network.'");
	...
	Если ОрганизацияПодключена И КонтрагентПодключен Тогда
		...
		Если Отказ Тогда
			ТекстСообщения = НСтр("ka='Ошибка запроса истории документов в сервисе 1С:Бизнес-сеть.';ru='Ошибка запроса истории документов в сервисе 1С:Бизнес-сеть.';en='Error history query documents when 1C: Business Network.'");
...
Процедура ЗагрузитьПодготовленныеДанныеЭД()
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		ШаблонНаименования = НСтр("ka='%1 %2 %3-ის';ru='%1 %2 от %3';en='% 1% 2% 3 from'");
		
		Тип = ТипыДокументов.НайтиПоЗначению(СтрокаТаблицы.ВидЭД);
		Если Тип = Неопределено Тогда
			ВызватьИсключение НСтр("ka='Отправка данного вида документа не поддерживается.';ru='Отправка данного вида документа не поддерживается.';en='Sending this type of document is not supported.'");
		КонецЕсли;
	...
	ТекстГиперссылки = НСтр("ka='документы не найдены';ru='документы не найдены';en='Documents Found'");
	Если ТаблицаДанных.Количество() > 1 Тогда
		ТекстГиперссылки = НСтр("ka='открыть список (%1)';ru='открыть список (%1)';en='open the list (% 1)'");
...
Процедура ИзменитьВидимостьДоступностьЭлементов()
	...
	Если СписокИстории.Количество() = 0 Тогда
		...
		Состояние = НСтр("ka='не отправлен';ru='не отправлен';en='is not sent'");
	ИначеЕсли СписокИстории.Количество() = 1 Тогда 
		...
		Если ВРег(ОтправленныеДанные.Статус) = "ДОСТАВЛЕН" Тогда
			Состояние = НСтр("ka='доставлен';ru='доставлен';en='delivered'") + " " + Формат(ОтправленныеДанные.ДатаДоставки, "ДЛФ=D");
		Иначе
			Состояние = НСтр("ka='отправлен';ru='отправлен';en='sent'") + " " + Формат(ОтправленныеДанные.Дата, "ДЛФ=D");
		КонецЕсли;
	Иначе
		Элементы.История.Гиперссылка = Истина;	
		ШаблонСостояния = НСтр("ka='отправлено (%1)';ru='отправлено (%1)';en='sent (1%)'");
...
Процедура ОтправитьДокументыВСервис()
	...
	Если Не Отказ Тогда
		ТекстОповещения	= НСтр("ka='Отправка выполнена.';ru='Отправка выполнена.';en='Sending performed.'");
		ТекстПояснения	= НСтр("ka='Отправлен документ через сервис 1С:Бизнес-сеть.';ru='Отправлен документ через сервис 1С:Бизнес-сеть.';en='Documents sent through the service of 1C: Business Network.'");
