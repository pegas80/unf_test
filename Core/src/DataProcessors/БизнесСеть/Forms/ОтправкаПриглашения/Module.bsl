...
Процедура ОтправитьПриглашенияЧерезБизнесСеть()
	...
	Если Результат.КодСостояния = 200 Тогда
		ТекстОповещения= НСтр("ka='Отправка приглашения';ru='Отправка приглашения'");
		ТекстПояснения = НСтр("ka='Приглашение отправлено контрагенту %1';ru='Приглашение отправлено контрагенту %1'");
		...
		Закрыть();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка отправления приглашения.';ru='Ошибка отправления приглашения.'"));
	КонецЕсли;
...
Процедура ТекстСообщенияНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ka='Пример текста приглашения к обмену электронными документами';ru='Пример текста приглашения к обмену электронными документами'"));
	ПараметрыФормы.Вставить("Пояснение", НСтр("ka='Скопируйте текст сообщения в буфер обмена и отправьте получателю любым удобным способом.';ru='Скопируйте текст сообщения в буфер обмена и отправьте получателю любым удобным способом.'"));
...
Процедура ОтправитьПриглашениеИзПрограммы()
	...
	ПараметрыФормы = Новый Структура;
	ТемаПисьма = НСтр("ka='Приглашение для подключения к сервису 1С:Бизнес-сеть';ru='Приглашение для подключения к сервису 1С:Бизнес-сеть'");
