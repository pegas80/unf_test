...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СтруктураЭД) <> Тип("Структура") 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка открытия электронного документа';ru='Ошибка открытия электронного документа';en='Error opening the electronic document'"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоВходящийДокумент Тогда
		Шаблон = НСтр("ka='%1 (электронный документ)';ru='%1 (электронный документ)';en='1% (electronic document)'");
	...
	Иначе
		...
		Если ВРЕГ(Статус) = "ОТПРАВЛЕН" Тогда
			Шаблон = НСтр("ka='отправлен %1';ru='отправлен %1';en='Sending% 1'");
			Состояние = СтрШаблон(Шаблон, 
				БизнесСетьКлиентСервер.ДатаИзUnixTime(Источник.sentDate));
		ИначеЕсли ВРЕГ(Статус) = "ДОСТАВЛЕН" Тогда
			Шаблон = НСтр("ka='отправлен %1, доставлен %2';ru='отправлен %1, доставлен %2';en='Sending% 1,% 2 delivered'");
			Состояние = СтрШаблон(Шаблон, 
				БизнесСетьКлиентСервер.ДатаИзUnixTime(Источник.sentDate),
...
Процедура ЗагрузитьЭлектронныйДокумент(Отказ)
	...
	Если СпособЗагрузкиДокумента = 0 Тогда
		Если Не ЗначениеЗаполнено(ТипОбъекта) Тогда
			ТекстОшибки = НСтр("ka='Поле ""Тип"" не заполнено';ru='Поле ""Тип"" не заполнено';en='""Type"" field is not filled'");
	...
	ИначеЕсли СпособЗагрузкиДокумента = 1 Тогда
		Если Не ЗначениеЗаполнено(ДокументИБ) Тогда
			ТекстОшибки = НСтр("ka='ველი ""დოკუმენტი"" არ არის შევსებული';ru='Поле ""Документ"" не заполнено';en='The ""document"" is not filled'");
...
Процедура ВыполнитьПросмотрЭДСервер(СтруктураЭД, Отказ)
	...
	Если СтруктураЭД.ФайлАрхива Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ka='Отсутствует подсистема ""Обмен с контрагентами"".';ru='Отсутствует подсистема ""Обмен с контрагентами"".';en='No subsystem ""with counterparts exchange.""'");
		КонецЕсли;
