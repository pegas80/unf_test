...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СтруктураЭД) <> Тип("Структура") 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка открытия электронного документа';ru='Ошибка открытия электронного документа'"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоВходящийДокумент Тогда
		Шаблон = НСтр("ka='%1 (электронный документ)';ru='%1 (электронный документ)'");
...
Процедура ЗагрузитьЭлектронныйДокумент(Отказ)
	...
	Если СпособЗагрузкиДокумента = 0 Тогда
		Если Не ЗначениеЗаполнено(ТипОбъекта) Тогда
			ТекстОшибки = НСтр("ka='Поле ""Тип"" не заполнено';ru='Поле ""Тип"" не заполнено'");
	...
	ИначеЕсли СпособЗагрузкиДокумента = 1 Тогда
		Если Не ЗначениеЗаполнено(ДокументИБ) Тогда
			ТекстОшибки = НСтр("ka='ველი ""დოკუმენტი"" არ არის შევსებული';ru='Поле ""Документ"" не заполнено'");
...
Процедура ВыполнитьПросмотрЭДСервер(СтруктураЭД, Отказ)
	...
	Если СтруктураЭД.ФайлАрхива Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ka='Отсутствует подсистема ""Обмен с контрагентами"".';ru='Отсутствует подсистема ""Обмен с контрагентами"".'");
		КонецЕсли;
