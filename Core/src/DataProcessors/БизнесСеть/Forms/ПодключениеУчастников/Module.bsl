...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("АдминистрированиеАбонентаБизнесСеть") Тогда
		ТекстОшибки = НСтр("ka='Недостаточно прав для выполнения операции в сервисе 1С:Бизнес-сеть.';ru='Недостаточно прав для выполнения операции в сервисе 1С:Бизнес-сеть.';en='Insufficient permissions to perform the operation when 1C: Business Network.'");
	...
	Если Не ОдиночноеПодключение И Не ИспользуетсяНесколькоОрганизаций Тогда 
		...
		Если Не ЗначениеЗаполнено(Параметры.Ссылка) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось определить организацию.';ru='Не удалось определить организацию.';en='Unable to determine the organization.'"),,,, Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
"При этом, для других участников сервиса организации будут отображаться как подключенные.';ka='Организации будут отключены от сервиса 1С:БизнесСеть в программе (информационной базе)."
...
"Обмен электронными документами и другие функции сервиса будут недоступны.';ka='Выбранные организации будут удалены в сервисе 1С:БизнесСеть."
...
"Обмен электронными документами и другие функции сервиса будут недоступны.';ka='Организация будет удалена в сервисе 1С:БизнесСеть."
"Обмен электронными документами и другие функции сервиса будут недоступны.'");
...
Процедура ОтключитьУчастниковОдиночноеИБ(Команда)
	...
	ТекстВопроса = НСтр("ka='Будет произведено отключение организации от сервиса 1С:БизнесСеть в программе (информационной базе)."
"При этом, для других участников сервиса организация будет отображаться как подключенная.';ru='Будет произведено отключение организации от сервиса 1С:БизнесСеть в программе (информационной базе)."
...
Процедура ОтобразитьСостояниеПодключенияИПП()
	...
	Если ЗначениеЗаполнено(Логин) Тогда
		...
		ШаблонЛогина    = НСтр("ka='Интернет-поддержка: подключена для пользователя <b>%1</b>';ru='Интернет-поддержка: подключена для пользователя <b>%1</b>';en='Internet support: is connected to the user <b>% 1 </ b>'");
		ШаблонПодсказки = НСтр("ka='Подключение организаций производится с присоединением к учетной записи Интернет-поддержки <b>%1</b>.';ru='Подключение организаций производится с присоединением к учетной записи Интернет-поддержки <b>%1</b>.';en='Connection is made with the organizations joining the account online support <b>% 1 </ b>.'");
		ШаблонЛогина = СтрШаблон(ШаблонЛогина, Логин);
		ШаблонПодсказки = СтрШаблон(ШаблонПодсказки, Логин);
	Иначе
		ШаблонЛогина    = НСтр("ka='Интернет-поддержка: требуется подключение.';ru='Интернет-поддержка: требуется подключение.';en='Internet support: connection is required.'");
		ШаблонПодсказки = НСтр("ru='Требуется подключение Интернет-поддержки в программе."
"Подключение организаций производится с присоединением к учетной записи Интернет-поддержки.';ka='Требуется подключение Интернет-поддержки в программе."
...
Процедура ПодключитьПослеПроверкиАутентификации(Знач ПараметрыАутентификации, Контекст) Экспорт
	...
	Если Не Отказ Тогда
		Если ОдиночноеПодключение Тогда
			ТекстОповещения = НСтр("ka='Подключение организации успешно выполнено.';ru='Подключение организации успешно выполнено.';en='Connecting the organization successful.'");
			СсылкаОповещения = ПолучитьНавигационнуюСсылку(Параметры.Ссылка);
			ОповеститьОбИзменении(Параметры.Ссылка);
		Иначе
			ТекстОповещения = НСтр("ka='Подключение организаций успешно выполнено.';ru='Подключение организаций успешно выполнено.';en='Connecting organizations successful.'");
			СсылкаОповещения = Неопределено;
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть';en='1C: Business Network'"), СсылкаОповещения, ТекстОповещения, 
...
Процедура ПроверитьСсылкиПодключения(МассивСсылок, Отказ)
	...
	Если Результат.Пустой() Тогда
		ТекстОшибки = НСтр("ka='Команда не может быть выполнена для выделенных строк.';ru='Команда не может быть выполнена для выделенных строк.';en='Command can not be executed for the selected row.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		Если ПустаяСтрока(Выборка.ИНН) Тогда
			ТекстОшибки = НСтр("ka='Не заполнено свойство ""ИНН"" %1 ""%2"".';ru='Не заполнено свойство ""ИНН"" %1 ""%2"".';en='Not filled property ""INN""% 1 ""%2"".'");
...
Процедура ВыполнитьОтключениеОрганизаций(РежимУдаленияВСервисе)
	...
	Если Не Отказ Тогда
		Если РежимУдаленияВСервисе Тогда
			ТекстОповещения = НСтр("ka='Удаление организаций в сервисе успешно выполнено.';ru='Удаление организаций в сервисе успешно выполнено.';en='Removal companies when successfully completed.'");
		Иначе
			ТекстОповещения = НСтр("ka='Отключение организаций в программе успешно выполнено.';ru='Отключение организаций в программе успешно выполнено.';en='Disable organizations in the program successfully.'");
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть';en='1C: Business Network'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
	КонецЕсли;
...
Процедура ВыполнитьОтключениеОрганизацииОдиночное(РежимУдаленияВСервисе)
	...
	Если Не Отказ Тогда
		Если РежимУдаленияВСервисе Тогда
			ТекстОповещения = НСтр("ka='Удаление организации в сервисе успешно выполнено.';ru='Удаление организации в сервисе успешно выполнено.';en='Deleting when the organization is successful.'");
		Иначе
			ТекстОповещения = НСтр("ka='Отключение организации в программе успешно выполнено.';ru='Отключение организации в программе успешно выполнено.';en=""Disabling the organization's program successfully.""");
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть';en='1C: Business Network'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
