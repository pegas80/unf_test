...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("АдминистрированиеАбонентаБизнесСеть") Тогда
		ТекстОшибки = НСтр("ka='Недостаточно прав для выполнения операции в сервисе 1С:Бизнес-сеть.';ru='Недостаточно прав для выполнения операции в сервисе 1С:Бизнес-сеть.'");
	...
	Если Не ОдиночноеПодключение И Не ИспользуетсяНесколькоОрганизаций Тогда 
		...
		Если Не ЗначениеЗаполнено(Параметры.Ссылка) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось определить организацию.';ru='Не удалось определить организацию.'"),,,, Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтобразитьСостояниеПодключенияИПП()
	...
	Если ЗначениеЗаполнено(Логин) Тогда
		...
		ШаблонЛогина    = НСтр("ka='Интернет-поддержка: подключена для пользователя <b>%1</b>';ru='Интернет-поддержка: подключена для пользователя <b>%1</b>'");
		ШаблонПодсказки = НСтр("ka='Подключение организаций производится с присоединением к учетной записи Интернет-поддержки <b>%1</b>.';ru='Подключение организаций производится с присоединением к учетной записи Интернет-поддержки <b>%1</b>.'");
		ШаблонЛогина = СтрШаблон(ШаблонЛогина, Логин);
		ШаблонПодсказки = СтрШаблон(ШаблонПодсказки, Логин);
	Иначе
		ШаблонЛогина    = НСтр("ka='Интернет-поддержка: требуется подключение.';ru='Интернет-поддержка: требуется подключение.'");
...
Процедура ПодключитьПослеПроверкиАутентификации(Знач ПараметрыАутентификации, Контекст) Экспорт
	...
	Если Не Отказ Тогда
		Если ОдиночноеПодключение Тогда
			ТекстОповещения = НСтр("ka='Подключение организации успешно выполнено.';ru='Подключение организации успешно выполнено.'");
			СсылкаОповещения = ПолучитьНавигационнуюСсылку(Параметры.Ссылка);
			ОповеститьОбИзменении(Параметры.Ссылка);
		Иначе
			ТекстОповещения = НСтр("ka='Подключение организаций успешно выполнено.';ru='Подключение организаций успешно выполнено.'");
			СсылкаОповещения = Неопределено;
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть'"), СсылкаОповещения, ТекстОповещения, 
...
Процедура ПроверитьСсылкиПодключения(МассивСсылок, Отказ)
	...
	Если Результат.Пустой() Тогда
		ТекстОшибки = НСтр("ka='Команда не может быть выполнена для выделенных строк.';ru='Команда не может быть выполнена для выделенных строк.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		Если ПустаяСтрока(Выборка.ИНН) Тогда
			ТекстОшибки = НСтр("ka='Не заполнено свойство ""ИНН"" %1 ""%2"".';ru='Не заполнено свойство ""ИНН"" %1 ""%2"".'");
...
Процедура ВыполнитьОтключениеОрганизаций(РежимУдаленияВСервисе)
	...
	Если Не Отказ Тогда
		Если РежимУдаленияВСервисе Тогда
			ТекстОповещения = НСтр("ka='Удаление организаций в сервисе успешно выполнено.';ru='Удаление организаций в сервисе успешно выполнено.'");
		Иначе
			ТекстОповещения = НСтр("ka='Отключение организаций в программе успешно выполнено.';ru='Отключение организаций в программе успешно выполнено.'");
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
	КонецЕсли;
...
Процедура ВыполнитьОтключениеОрганизацииОдиночное(РежимУдаленияВСервисе)
	...
	Если Не Отказ Тогда
		Если РежимУдаленияВСервисе Тогда
			ТекстОповещения = НСтр("ka='Удаление организации в сервисе успешно выполнено.';ru='Удаление организации в сервисе успешно выполнено.'");
		Иначе
			ТекстОповещения = НСтр("ka='Отключение организации в программе успешно выполнено.';ru='Отключение организации в программе успешно выполнено.'");
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
