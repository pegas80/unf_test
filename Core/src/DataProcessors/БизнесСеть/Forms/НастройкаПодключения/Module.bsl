...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ИнформационнаяБазаПодключенаКСервису() Тогда
		ТекстОшибки = НСтр("ka='Информационная база не подключена к сервису 1С:Бизнес-сеть.';ru='Информационная база не подключена к сервису 1С:Бизнес-сеть.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура СинхронизироватьПользователей(Команда)
	...
	Текст = НСтр("ka='Пользователи информационной базы синхронизированы с сервисом 1С:Бизнес-сеть.';ru='Пользователи информационной базы синхронизированы с сервисом 1С:Бизнес-сеть.'");
	ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть'"),, Текст, БиблиотекаКартинок.БизнесСеть);
...
Процедура СинхронизироватьПодключение(Команда)
	...
	Текст = НСтр("ka='Информационная база синхронизирована с сервисом 1С:Бизнес-сеть.';ru='Информационная база синхронизирована с сервисом 1С:Бизнес-сеть.'");
...
Процедура ОбновитьИнформациюДляТехническойПоддержки()
	...
	Шаблон = НСтр("ka='Подключение к сервису 1С:Бизнес-сеть:';ru='Подключение к сервису 1С:Бизнес-сеть:'");
	Если НЕ ПустаяСтрока(ИдентификаторИнформационнойБазы) Тогда
		Шаблон = Шаблон + Символы.ПС + НСтр("ka='Ключ информационной базы: %1';ru='Ключ информационной базы: %1'");
	КонецЕсли; 
	Если НЕ ПустаяСтрока(ИдентификаторПользователя) Тогда
		Шаблон = Шаблон + Символы.ПС + НСтр("ka='Ключ пользователя: %2';ru='Ключ пользователя: %2'");
	КонецЕсли;
	
	Шаблон = Шаблон + Символы.ПС + НСтр("ka='Версия библиотеки электронных документов: %3';ru='Версия библиотеки электронных документов: %3'");
...
Процедура ОбновитьИнформациюСостояниеСинхронизацииПользователей()
	...
	Если Запрос.Выполнить().Пустой() Тогда
		...
		СостояниеСинхронизацииПользователей = НСтр("ka='Синхронизация пользователей не требуется.';ru='Синхронизация пользователей не требуется.'");
		Элементы.СостояниеСинхронизацииПользователей.ЦветТекста = ЦветаСтиля.ПоясняющийТекст;
		
	Иначе
		
		СостояниеСинхронизацииПользователей = НСтр("ka='Требуется синхронизация пользователей.';ru='Требуется синхронизация пользователей.'");
...
Процедура ОтключитьИнформационнуюБазуПродолжение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='1С:Бизнес-сеть';ru='1С:Бизнес-сеть'"),, НСтр("ka='Информационная база отключена от сервиса 1С:Бизнес-сеть.';ru='Информационная база отключена от сервиса 1С:Бизнес-сеть.'"), БиблиотекаКартинок.БизнесСеть);
