...
Функция СформироватьТекстПрофиляHTML(Реквизиты)
	...
	ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[НадписьИНН]", НСтр("ka='ИНН';ru='ИНН'"));
	ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[ИНН]", Реквизиты.ИНН);
	Если ЗначениеЗаполнено(Реквизиты.КПП) Тогда
		ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[НадписьКПП]", НСтр("ka='КПП';ru='КПП'"));
	...
	КонецЕсли;
	ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[НадписьДатаПодключения]", НСтр("ka='Дата подключения:';ru='Дата подключения:'"));
	...
	Если ЗначениеЗаполнено(Реквизиты.НаименованиеЕГРН) Тогда
		Строка1 = НСтр("ka='Наименование в едином государственном реестре налогоплательщиков';ru='Наименование в едином государственном реестре налогоплательщиков'");
...
Процедура ЗаполнитьПрофиль(ПерезаполнитьДанные = Ложь)
	...
	Заголовок = НСтр("ka='%1 (профиль 1С:Бизнес-сеть)';ru='%1 (профиль 1С:Бизнес-сеть)'");
