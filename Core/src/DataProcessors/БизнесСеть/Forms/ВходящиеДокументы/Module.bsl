...
Процедура СписокДокументовПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ka='Выделенные документы будут удалены в сервисе 1С:Бизнес-сеть."
"Документы учета информационной базы не изменятся."
"Продолжить выполнение операции?';ru='Выделенные документы будут удалены в сервисе 1С:Бизнес-сеть."
"Документы учета информационной базы не изменятся."
...
Процедура ОбновитьСписок(Отказ)
	...
	Если МассивОрганизаций.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Отсутствуют подключенные организации к сервису 1С:Бизнес-сеть.';ru='Отсутствуют подключенные организации к сервису 1С:Бизнес-сеть.';en='No connection to the service company 1C: Business Network.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура УдалитьДокументВСервисеПослеВопроса(Результат, МассивСтрок) Экспорт
	...
	Если КоличествоСтрок = 1 Тогда
		ТекстОповещения	= НСтр("ka='Документ удален.';ru='Документ удален.';en='Deleted document.'");
		ТекстПояснения	= НСтр("ka='Удален входящий документ в сервисе 1С:Бизнес-сеть.';ru='Удален входящий документ в сервисе 1С:Бизнес-сеть.';en='Deleted incoming document when 1C: Business Network.'");
	Иначе
		ТекстОповещения	= НСтр("ka='Документы удалены (%1).';ru='Документы удалены (%1).';en='Documents removed (1%).'");
		ТекстОповещения = СтрШаблон(ТекстОповещения, КоличествоСтрок);
		ТекстПояснения	= НСтр("ka='Удалены входящие документы в сервисе 1С:Бизнес-сеть.';ru='Удалены входящие документы в сервисе 1С:Бизнес-сеть.';en='Deleted incoming documents when 1C: Business Network.'");
	КонецЕсли;
