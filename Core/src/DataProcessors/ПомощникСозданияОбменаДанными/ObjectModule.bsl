...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ka='При создании настройки обмена во второй информационной базе возникли ошибки.';ru='При создании настройки обмена во второй информационной базе возникли ошибки.'");
		КонецЕсли;
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ka='Возникла ошибка при создании настройки синхронизации данных.';ru='Возникла ошибка при создании настройки синхронизации данных.'"));
		КонецЕсли;
...
Процедура ОбновитьНастройкиОбменаДанными(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ka='Возникла ошибка при обновлении настройки синхронизации данных.';ru='Возникла ошибка при обновлении настройки синхронизации данных.'"));
		КонецЕсли;
...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ka='При создании настройки синхронизации данных возникли ошибки.';ru='При создании настройки синхронизации данных возникли ошибки.'"));
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если СтруктураНастроек.Свойство("ИмяПланаОбмена")
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ka='Файл содержит настройки обмена для другой информационной базы.';ru='Файл содержит настройки обмена для другой информационной базы.'");
	...
	Если Не Отказ Тогда
		...
		Если ВерсияФорматаФайлаНастроекОбменаДанными = "1.0" Тогда
			...
			ЭтотОбъект.НаименованиеЭтойБазы = НСтр("ka='Эта информационная база';ru='Эта информационная база'");
