...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ka='Продолжение настройки синхронизации данных с главным узлом';ru='Продолжение настройки синхронизации данных с главным узлом'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';ru='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Если ДлительнаяОперация Тогда
			ТекстПредупреждения = НСтр("ka='Настройка синхронизации данных будет прервана.';ru='Настройка синхронизации данных будет прервана.'");
		КонецЕсли;
		
		Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
			ТекстПредупреждения = НСтр("ka='Настройка подчиненного узла распределенной информационной базы не выполнена.';ru='Настройка подчиненного узла распределенной информационной базы не выполнена.'");
		КонецЕсли;
...
"Отказаться от настройки и использовать значения по умолчанию?'");
	...
	ТекстПредупреждения = НСтр("ka='Отменить настройку синхронизации и выйти из помощника?';ru='Отменить настройку синхронизации и выйти из помощника?'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыСопоставленияОбъектов" Тогда
		...
		Состояние(НСтр("ka='Выполняется сбор информации сопоставления...';ru='Выполняется сбор информации сопоставления...'"));
		
		ОбновитьДанныеСтатистикиСопоставленияНаСервере(Отказ, Параметр);
		
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("ka='При получении информации статистики возникли ошибки.';ru='При получении информации статистики возникли ошибки.'"));
		Иначе
			
			РазвернутьДеревоИнформацииСтатистики(Параметр.КлючУникальности);
			
			Состояние(НСтр("ka='Сбор информации завершен';ru='Сбор информации завершен'"));
		КонецЕсли;
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ka='Файл настроек синхронизации данных (*.xml)';ru='Файл настроек синхронизации данных (*.xml)'") + "|*.xml" );
...
Процедура СохранитьФайлНастроекОбменаДанными(Команда)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='При сохранении файла настроек синхронизации данных возникли ошибки.';ru='При сохранении файла настроек синхронизации данных возникли ошибки.'"));
	...
	Иначе
		...
		#Иначе
			...
			ТекстПредложения = НСтр("ka='კატალოგის გასახსნელად აუცილებელია ფაილებთან მუშაობის გაფართოების დაყენება.';ru='Для открытия каталога необходимо необходимо установить расширение работы с файлами.'");
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Заголовок      = НСтр("ka='Укажите имя файла настроек синхронизации данных';ru='Укажите имя файла настроек синхронизации данных'");
...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Если СтрокиПереходаТекущие.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ka='გამოსასახად გვერდი არ არის დადგენილი.';ru='Не определена страница для отображения.'");
	КонецЕсли;
...
Процедура ПроверитьПодключениеCOMЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("ka='Проверка подключения успешно завершена.';ru='Проверка подключения успешно завершена.'");
		...
		Иначе
			...
			Если ЭтоОшибкаРегистрацииКомпоненты Тогда
				...
				ТекстРегистрацияКомпоненты = Символы.ПС
					+ НСтр("ka='Возможно, необходимо зарегистрировать COM-компоненту comcntr.dll';ru='Возможно, необходимо зарегистрировать COM-компоненту comcntr.dll'")
					+ ?(ЭтоФайловаяИБ, ".", НСтр("ka=', после чего выполнить перезапуск сервера 1С:Предприятие.';ru=', после чего выполнить перезапуск сервера 1С:Предприятие.'"))
			КонецЕсли;
...
Процедура ПроверитьПодключениеWSЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если Не Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='მიერთება წარმატებით არის დაყენებული.';ru='Подключение успешно установлено.'"));
...
Процедура УстановитьВидимостьНаСервере()
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ka='Создание начального образа подчиненного узла РИБ';ru='Создание начального образа подчиненного узла РИБ'");
...
"%1'");
	...
	Иначе
		...
		Результат = Строка(Объект.ВидТранспортаСообщенийОбмена)
			+ НСтр("ka=', параметры:';ru=', параметры:'") + Символы.ПС
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	Для Каждого Элемент Из ПредставленияНастроек Цикл
		...
		Если РежимРаботыМастера = "ОбменЧерезВнешнееСоединение" Тогда
			...
			Если Элемент.Ключ = "COMВариантРаботыИнформационнойБазы" Тогда
				...
				ЗначениеНастройки = ?(Объект[Элемент.Ключ] = 0, НСтр("ka='ფაილური';ru='Файловый'"), НСтр("ka='კლიენტ-სერვერული';ru='Клиент-серверный'"));
		...
		ИначеЕсли  Не ТипЗначения(ЗначениеНастройки, "Число")
			...
			ЗначениеНастройки = НСтр("ka='<пусто>';ru='<пусто>'");
	...
	Если ПустаяСтрока(Результат) Тогда
		...
		Результат = НСтр("ka='Настройки подключения не заданы.';ru='Настройки подключения не заданы.'");
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	Если Не Отказ Тогда
		...
		Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.FILE") Тогда
			ТекстПредупреждения = НСтр("ka='Запись данных в указанный каталог разрешена.';ru='Запись данных в указанный каталог разрешена.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='მიერთება წარმატებით არის დაყენებული.';ru='Подключение успешно установлено.'");
		КонецЕсли;
...
Процедура ПроверитьКорректностьПараметровПодключенияWS(Отказ)
	...
	Если ПустаяСтрока(Объект.WSURLВебСервиса) Тогда
		...
		НСтрока = НСтр("ka='Укажите адрес приложения в Интернете.';ru='Укажите адрес приложения в Интернете.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSИмяПользователя) Тогда
		...
		НСтрока = НСтр("ka='Укажите имя пользователя.';ru='Укажите имя пользователя.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSПароль) Тогда
		...
		НСтрока = НСтр("ka='Укажите пароль пользователя.';ru='Укажите пароль пользователя.'");
...
Процедура ПроверитьПодключениеWSНаСервере(Отказ, РасширеннаяПроверка, ПредлагатьПерейтиВЖурналРегистрации)
	...
	Если РасширеннаяПроверка Тогда
		...
		Если Не ПараметрыПриемника.ПланОбменаСуществует Тогда
			...
			Сообщение = НСтр("ka='Другая программа не предназначена для синхронизации с текущей.';ru='Другая программа не предназначена для синхронизации с текущей.'");
...
Процедура ПроверитьЗаполнениеРеквизитовВФорме(Отказ, ИмяФормыДляПроверки, ПараметрыФормы, ИмяРеквизитаФормы)
	...
	Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки.';ru='Необходимо задать обязательные настройки.'"),,, ИмяРеквизитаФормы, Отказ);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		НСтрока = НСтр("ka='Сохраните файл с настройками для другой программы';ru='Сохраните файл с настройками для другой программы'");
	...
	Иначе
		...
		Если ВыполнитьОбменДаннымиСейчас Тогда
			...
			Состояние(НСтр("ka='მიმდინარეობს მონაცემთა გაგზავნა...';ru='Выполняется отправка данных...'"));
			...
			ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='მონაცემთა გადმოტვირთვა';ru='Выгрузка данных'");
...
Процедура ЗавершениеВыгрузкиДанных()
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Отменено" Тогда
		СообщениеОбОшибке = НСтр("ka='Действие отменено пользователем.';ru='Действие отменено пользователем.'");
	Иначе
		
		РезультатВыполнения = ПолучитьРезультатВыполненияФоновогоЗаданияНаСервере();
		
		Если РезультатВыполнения.Отказ Тогда
			СообщениеОбОшибке = НСтр("ka='Во время отправки данных возникли ошибки(см. журнал регистрации).';ru='Во время отправки данных возникли ошибки(см. журнал регистрации).'");
		КонецЕсли;
	...
	ДлительнаяОперация      = Ложь;
	Состояние(НСтр("ka='Отправка данных завершена';ru='Отправка данных завершена'"));
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ, Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Состояние(НСтр("ka='Выполняется создание настройки синхронизации данных';ru='Выполняется создание настройки синхронизации данных'"));
	
	НастроитьНовыйОбменДаннымиНаСервере(Отказ, НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='При создании настройки синхронизации данных возникли ошибки.';ru='При создании настройки синхронизации данных возникли ошибки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуСопоставления()
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ka='Для этих данных нельзя выполнить сопоставление.';ru='Для этих данных нельзя выполнить сопоставление.'"));
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2';ru='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2'"),
				Символы.ПС,
...
Функция НадписьДалееFTP()
	...
	Возврат НСтр("ka='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.';ru='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.'");
...
Функция НадписьДалееEMAIL()
	...
	Возврат НСтр("ka='Нажмите кнопку ""Далее"" для настройки подключения по почте.';ru='Нажмите кнопку ""Далее"" для настройки подключения по почте.'");
...
Функция НадписьДалееНастройки()
	...
	Возврат НСтр("ka='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.';ru='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование этой программы.';ru='Укажите наименование этой программы.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование другой программы.';ru='Укажите наименование другой программы.'");
	...
	Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ka='Укажите существующий или желаемый префикс второй информационной базы.';ru='Укажите существующий или желаемый префикс второй информационной базы.'");
	...
	Если СокрЛП(Объект.ПрефиксИнформационнойБазыИсточника) = СокрЛП(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ka='Префиксы информационных баз должны быть различными.';ru='Префиксы информационных баз должны быть различными.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Функция Подключаемый_СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование информационной базы.';ru='Укажите наименование информационной базы.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеЭтойБазы",, Отказ);
		Возврат Неопределено;
	КонецЕсли;
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаНастроекСинхронизацииДанныхДляЗагрузки) Тогда
		ТекстОшибки = НСтр("ka='Выберите файл с настройками синхронизации данных';ru='Выберите файл с настройками синхронизации данных'");
	...
	ТекстПредупреждения = НСтр("ka='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.';ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.'");
...
Процедура ЗавершениеВыбораФайлаНастроекОбменаДаннымиДляЗагрузки(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("ka='Ошибка передачи файла настроек синхронизации данных на сервер';ru='Ошибка передачи файла настроек синхронизации данных на сервер'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		Если ОшибкаЗагрузкиПараметровМастера Тогда
			ТекстОшибки = НСтр("ka='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.';ru='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.'");
		Иначе
...
"Префикс второй информационной базы: %5'");
	...
	Иначе
		...
		// Представление результата настройки обмена.
		СтрокаСообщения = НСтр("ka='%1%2%3Префикс этой информационной базы: %4';ru='%1%2%3Префикс этой информационной базы: %4'");
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхАвтоматическоеСопоставление_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе анализа данных.';ru='Возникли ошибки на этапе анализа данных.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Загрузка справочников из сообщения обмена';ru='Загрузка справочников из сообщения обмена'");
...
Процедура ПроверитьНастройкиПараметровУчетаНаСервере(
	...
	Если Не НастройкиПараметровУчетаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ka='Не заданы параметры учета в этой программе.';ru='Не заданы параметры учета в этой программе.'");
		КонецЕсли;
	...
	Если Не НастройкиПараметровУчетаКорреспондентаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ka='Не заданы параметры учета в приложении, расположенном в Интернете.';ru='Не заданы параметры учета в приложении, расположенном в Интернете.'");
		КонецЕсли;
...
Процедура ПроверитьЗаданиеНастроекДляПервойБазы(Отказ, ТипСоединения = "ВебСервис")
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование приложения в Интернете.';ru='Укажите наименование приложения в Интернете.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура ПроверитьЗаданиеНастроекДляВторойБазы(Отказ, ТипСоединения)
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование программы.';ru='Укажите наименование программы.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура СтраницаМастераОжиданиеПроверкиПодключенияВнешнегоСоединения_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если Объект.COMВариантРаботыИнформационнойБазы = 0 Тогда
		...
		Если ПустаяСтрока(Объект.COMКаталогИнформационнойБазы) Тогда
			...
			НСтрока = НСтр("ka='Укажите каталог информационной базы.';ru='Укажите каталог информационной базы.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Объект.COMИмяСервера1СПредприятия) Тогда
			...
			НСтрока = НСтр("ka='Укажите имя кластера серверов.';ru='Укажите имя кластера серверов.'");
		...
		ИначеЕсли ПустаяСтрока(Объект.COMИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			...
			НСтрока = НСтр("ka='Укажите имя информационной базы.';ru='Укажите имя информационной базы.'");
	...
	Если Не ПланОбменаСуществует Тогда
		...
		Сообщение = НСтр("ka='Синхронизация данных с указанной программой не предусмотрена.';ru='Синхронизация данных с указанной программой не предусмотрена.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
	...
	Если НРег(СтрокаСоединенияИнформационнойБазы()) = НРег(ВнешнееСоединение.СтрокаСоединенияИнформационнойБазы()) Тогда
		...
		Сообщение = НСтр("ka='Заданы настройки подключения к этой информационной базе.';ru='Заданы настройки подключения к этой информационной базе.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
...
Процедура ПроверитьНаличиеСуществующегоОбменаДаннымиСВторойБазой(Отказ)
	...
	Если Не ПустаяСтрока(КодУзла)
		...
		Сообщение = НСтр("ka='Синхронизация данных между программами уже была настроена ранее.';ru='Синхронизация данных между программами уже была настроена ранее.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
