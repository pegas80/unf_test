...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ka='Продолжение настройки синхронизации данных с главным узлом';ru='Продолжение настройки синхронизации данных с главным узлом';en='Continue setup data synchronization with the master node'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';ru='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';en='Before configuring synchronization is recommended to <a href = %1> backup data </a>.'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Если ДлительнаяОперация Тогда
			ТекстПредупреждения = НСтр("ka='Настройка синхронизации данных будет прервана.';ru='Настройка синхронизации данных будет прервана.';en='data synchronization setting will be aborted.'");
		КонецЕсли;
		
		Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
			ТекстПредупреждения = НСтр("ka='Настройка подчиненного узла распределенной информационной базы не выполнена.';ru='Настройка подчиненного узла распределенной информационной базы не выполнена.';en='Configuring a slave node of the distributed information database failed.'");
		КонецЕсли;
...
"Работа помощника не может быть завершена.';ka='Выполняется настройка синхронизации данных."
"Работа помощника не может быть завершена.'"));
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		ТекстПредупреждения = НСтр("ka='Выполняется настройка подчиненного узла распределенной информационной базы."
"Отказаться от настройки и использовать значения по умолчанию?';ru='Выполняется настройка подчиненного узла распределенной информационной базы."
"Отказаться от настройки и использовать значения по умолчанию?'");
	...
	ТекстПредупреждения = НСтр("ka='Отменить настройку синхронизации и выйти из помощника?';ru='Отменить настройку синхронизации и выйти из помощника?';en='Cancel sync settings and exit the wizard?'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыСопоставленияОбъектов" Тогда
		...
		Состояние(НСтр("ka='Выполняется сбор информации сопоставления...';ru='Выполняется сбор информации сопоставления...';en='Collects information matching ...'"));
		
		ОбновитьДанныеСтатистикиСопоставленияНаСервере(Отказ, Параметр);
		
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("ka='При получении информации статистики возникли ошибки.';ru='При получении информации статистики возникли ошибки.';en='Upon receipt of the statistics information errors occurred.'"));
		Иначе
			
			РазвернутьДеревоИнформацииСтатистики(Параметр.КлючУникальности);
			
			Состояние(НСтр("ka='Сбор информации завершен';ru='Сбор информации завершен';en='Collect complete information'"));
		КонецЕсли;
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ka='Файл настроек синхронизации данных (*.xml)';ru='Файл настроек синхронизации данных (*.xml)';en='data synchronization settings file (* .xml)'") + "|*.xml" );
...
Процедура СохранитьФайлНастроекОбменаДанными(Команда)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='При сохранении файла настроек синхронизации данных возникли ошибки.';ru='При сохранении файла настроек синхронизации данных возникли ошибки.';en='When you save a file data synchronization settings any errors.'"));
	...
	Иначе
		...
		#Иначе
			...
			ТекстПредложения = НСтр("ka='კატალოგის გასახსნელად აუცილებელია ფაილებთან მუშაობის გაფართოების დაყენება.';ru='Для открытия каталога необходимо необходимо установить расширение работы с файлами.';en='To open the directory, you must need to install the extension to work with files.'");
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Заголовок      = НСтр("ka='Укажите имя файла настроек синхронизации данных';ru='Укажите имя файла настроек синхронизации данных';en='Specify the file name of the data synchronization settings'");
...
Процедура ПроверитьПодключениеCOMЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("ka='Проверка подключения успешно завершена.';ru='Проверка подключения успешно завершена.';en='Test Connection succeeded.'");
		...
		Иначе
			...
			Если ЭтоОшибкаРегистрацииКомпоненты Тогда
				...
				ТекстРегистрацияКомпоненты = Символы.ПС
					+ НСтр("ka='Возможно, необходимо зарегистрировать COM-компоненту comcntr.dll';ru='Возможно, необходимо зарегистрировать COM-компоненту comcntr.dll';en='You may need to register the COM-component comcntr.dll'")
					+ ?(ЭтоФайловаяИБ, ".", НСтр("ka=', после чего выполнить перезапуск сервера 1С:Предприятие.';ru=', после чего выполнить перезапуск сервера 1С:Предприятие.';en='And then perform a server restart 1C: Enterprise.'"))
			КонецЕсли;
...
Процедура ПроверитьПодключениеWSЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если Не Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='მიერთება წარმატებით არის დაყენებული.';ru='Подключение успешно установлено.';en='The connection is successfully established.'"));
...
"Перейти в журнал регистрации?';ka='При создании синхронизации данных возникли ошибки."
...
Процедура УстановитьВидимостьНаСервере()
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ka='Создание начального образа подчиненного узла РИБ';ru='Создание начального образа подчиненного узла РИБ';en='Creating the initial image of the slave node RIB'");
...
"%1';ka='Параметры подключения к приложению в сервисе:"
"%1'");
	...
	Иначе
		...
		Результат = Строка(Объект.ВидТранспортаСообщенийОбмена)
			+ НСтр("ka=', параметры:';ru=', параметры:';en=', options:'") + Символы.ПС
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	Для Каждого Элемент Из ПредставленияНастроек Цикл
		...
		Если РежимРаботыМастера = "ОбменЧерезВнешнееСоединение" Тогда
			...
			Если Элемент.Ключ = "COMВариантРаботыИнформационнойБазы" Тогда
				...
				ЗначениеНастройки = ?(Объект[Элемент.Ключ] = 0, НСтр("ka='ფაილური';ru='Файловый';en='File'"), НСтр("ka='კლიენტ-სერვერული';ru='Клиент-серверный';en='Client / server'"));
		...
		ИначеЕсли  Не ТипЗначения(ЗначениеНастройки, "Число")
			...
			ЗначениеНастройки = НСтр("ka='<пусто>';ru='<пусто>';en='<Empty>'");
	...
	Если ПустаяСтрока(Результат) Тогда
		...
		Результат = НСтр("ka='Настройки подключения не заданы.';ru='Настройки подключения не заданы.';en='Connection settings are not set.'");
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	Если Не Отказ Тогда
		...
		Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.FILE") Тогда
			ТекстПредупреждения = НСтр("ka='Запись данных в указанный каталог разрешена.';ru='Запись данных в указанный каталог разрешена.';en='Writing data to the specified directory is allowed.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='მიერთება წარმატებით არის დაყენებული.';ru='Подключение успешно установлено.';en='The connection is successfully established.'");
		КонецЕсли;
...
Процедура ПроверитьКорректностьПараметровПодключенияWS(Отказ)
	...
	Если ПустаяСтрока(Объект.WSURLВебСервиса) Тогда
		...
		НСтрока = НСтр("ka='Укажите адрес приложения в Интернете.';ru='Укажите адрес приложения в Интернете.';en='Specify applications Internet address.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSИмяПользователя) Тогда
		...
		НСтрока = НСтр("ka='Укажите имя пользователя.';ru='Укажите имя пользователя.';en='Specify a user name.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSПароль) Тогда
		...
		НСтрока = НСтр("ka='Укажите пароль пользователя.';ru='Укажите пароль пользователя.';en=""Enter the user's password.""");
...
"Перейти в журнал регистрации?';ka='Ошибка установки подключения."
"Перейти в журнал регистрации?'");
...
Процедура ПроверитьПодключениеWSНаСервере(Отказ, РасширеннаяПроверка, ПредлагатьПерейтиВЖурналРегистрации)
	...
	Если РасширеннаяПроверка Тогда
		...
		Если Не ПараметрыПриемника.ПланОбменаСуществует Тогда
			...
			Сообщение = НСтр("ka='Другая программа не предназначена для синхронизации с текущей.';ru='Другая программа не предназначена для синхронизации с текущей.';en='Another program is not designed to sync with the current.'");
...
Процедура ПроверитьЗаполнениеРеквизитовВФорме(Отказ, ИмяФормыДляПроверки, ПараметрыФормы, ИмяРеквизитаФормы)
	...
	Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Необходимо задать обязательные настройки.';ru='Необходимо задать обязательные настройки.';en='required settings must be set.'"),,, ИмяРеквизитаФормы, Отказ);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		НСтрока = НСтр("ka='Сохраните файл с настройками для другой программы';ru='Сохраните файл с настройками для другой программы';en='Save the file with settings for other programs'");
	...
	Иначе
		...
		Если ВыполнитьОбменДаннымиСейчас Тогда
			...
			Состояние(НСтр("ka='მიმდინარეობს მონაცემთა გაგზავნა...';ru='Выполняется отправка данных...';en='You are sending data ...'"));
			...
			ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='მონაცემთა გადმოტვირთვა';ru='Выгрузка данных';en='Upload data'");
...
Процедура ЗавершениеВыгрузкиДанных()
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Отменено" Тогда
		СообщениеОбОшибке = НСтр("ka='Действие отменено пользователем.';ru='Действие отменено пользователем.';en='The action was canceled by the user.'");
	Иначе
		
		РезультатВыполнения = ПолучитьРезультатВыполненияФоновогоЗаданияНаСервере();
		
		Если РезультатВыполнения.Отказ Тогда
			СообщениеОбОшибке = НСтр("ka='Во время отправки данных возникли ошибки(см. журнал регистрации).';ru='Во время отправки данных возникли ошибки(см. журнал регистрации).';en='During data sending any errors (see. Log).'");
		КонецЕсли;
	...
	ДлительнаяОперация      = Ложь;
	Состояние(НСтр("ka='Отправка данных завершена';ru='Отправка данных завершена';en='Sending data is completed'"));
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ, Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Состояние(НСтр("ka='Выполняется создание настройки синхронизации данных';ru='Выполняется создание настройки синхронизации данных';en='You are creating a configuration data synchronization'"));
	
	НастроитьНовыйОбменДаннымиНаСервере(Отказ, НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(, НСтр("ka='При создании настройки синхронизации данных возникли ошибки.';ru='При создании настройки синхронизации данных возникли ошибки.';en='When you create a configuration synchronization errors occur.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуСопоставления()
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ka='Для этих данных нельзя выполнить сопоставление.';ru='Для этих данных нельзя выполнить сопоставление.';en='you can not perform a comparison of these data.'"));
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2';ru='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2';en='An error occurred while performing PriPodklyucheniiKKorrespondentu handler:% 1% 2'"),
				Символы.ПС,
...
Функция НадписьДалееFTP()
	...
	Возврат НСтр("ka='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.';ru='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.';en='Click ""Next"" to set up a connection via FTP-resource.'");
...
Функция НадписьДалееEMAIL()
	...
	Возврат НСтр("ka='Нажмите кнопку ""Далее"" для настройки подключения по почте.';ru='Нажмите кнопку ""Далее"" для настройки подключения по почте.';en='Click ""Next"" button to configure the connection by mail.'");
...
Функция НадписьДалееНастройки()
	...
	Возврат НСтр("ka='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.';ru='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.';en='Click ""Next"" button to configure advanced data synchronization options.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование этой программы.';ru='Укажите наименование этой программы.';en='Provide the name of the program.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование другой программы.';ru='Укажите наименование другой программы.';en='Enter the name of another program.'");
	...
	Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ka='Укажите существующий или желаемый префикс второй информационной базы.';ru='Укажите существующий или желаемый префикс второй информационной базы.';en='Specify existing or desired second prefix information base.'");
	...
	Если СокрЛП(Объект.ПрефиксИнформационнойБазыИсточника) = СокрЛП(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ka='Префиксы информационных баз должны быть различными.';ru='Префиксы информационных баз должны быть различными.';en='Prefix information databases should be different.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Функция Подключаемый_СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование информационной базы.';ru='Укажите наименование информационной базы.';en='Specify the name of the knowledge base.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеЭтойБазы",, Отказ);
		Возврат Неопределено;
	КонецЕсли;
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаНастроекСинхронизацииДанныхДляЗагрузки) Тогда
		ТекстОшибки = НСтр("ka='Выберите файл с настройками синхронизации данных';ru='Выберите файл с настройками синхронизации данных';en='Select the file with the data synchronization settings'");
	...
	ТекстПредупреждения = НСтр("ka='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.';ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.';en='File synchronization settings for the transmission of data, you must install the extension for Web client 1C: Enterprise.'");
...
Процедура ЗавершениеВыбораФайлаНастроекОбменаДаннымиДляЗагрузки(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("ka='Ошибка передачи файла настроек синхронизации данных на сервер';ru='Ошибка передачи файла настроек синхронизации данных на сервер';en='setting file transmission error data synchronization to the server'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		Если ОшибкаЗагрузкиПараметровМастера Тогда
			ТекстОшибки = НСтр("ka='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.';ru='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.';en='An incorrect file data synchronization settings. Please enter a valid file.'");
		Иначе
...
"Следует настроить хотя бы один вариант подключения.';ka='Не указаны параметры подключения для синхронизации данных."
...
Функция Подключаемый_СтраницаМастераИтогНастройкиОбмена_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если ВариантРаботыМастера = "НастроитьНовыйОбменДанными" Тогда
		...
		СтрокаСообщения = НСтр("ka='%1%2%3Префикс этой информационной базы: %4"
"Префикс второй информационной базы: %5';ru='%1%2%3Префикс этой информационной базы: %4"
"Префикс второй информационной базы: %5'");
	...
	Иначе
		...
		// Представление результата настройки обмена.
		СтрокаСообщения = НСтр("ka='%1%2%3Префикс этой информационной базы: %4';ru='%1%2%3Префикс этой информационной базы: %4';en='% 1% 2% 3Prefiks this information base: 4%'");
...
"========================================================"
"%5%6Префикс информационной базы: %7';ru='%1"
"Параметры синхронизации данных для этой программы:"
"========================================================"
"%2%3Префикс информационной базы: %4"
""
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхСозданиеНастройкиОбмена_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе создания настройки синхронизации данных."
"Для решения проблем воспользуйтесь журналом регистрации.';ru='Возникли ошибки на этапе создания настройки синхронизации данных."
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхПолучениеСообщения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе анализа данных."
"Для решения проблем воспользуйтесь журналом регистрации.';ru='Возникли ошибки на этапе анализа данных."
"Для решения проблем воспользуйтесь журналом регистрации.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхАвтоматическоеСопоставление_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе анализа данных.';ru='Возникли ошибки на этапе анализа данных.';en='Any errors during data analysis.'"));
...
Функция Подключаемый_СтраницаМастераСопоставлениеДанных_ПриПереходеДалее(Отказ)
	...
	Если Не ВсеДанныеСопоставлены Тогда
		...
		Сообщение = НСтр("ka='Не все данные сопоставлены. Наличие несопоставленных данных"
"может привести к появлению одинаковых элементов справочников (дублей)."
"Продолжить?';ru='Не все данные сопоставлены. Наличие несопоставленных данных"
"может привести к появлению одинаковых элементов справочников (дублей)."
"Продолжить?'");
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ka='Загрузка справочников из сообщения обмена';ru='Загрузка справочников из сообщения обмена';en='Download guides from the exchange of messages'");
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе синхронизации справочников."
"Для решения проблем воспользуйтесь журналом регистрации.';ru='Возникли ошибки на этапе синхронизации справочников."
"Для решения проблем воспользуйтесь журналом регистрации.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе сохранения настроек."
"Для решения проблем воспользуйтесь журналом регистрации.';ru='Возникли ошибки на этапе сохранения настроек."
"Для решения проблем воспользуйтесь журналом регистрации.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Возникли ошибки на этапе синхронизации данных."
"Для решения проблем воспользуйтесь журналом регистрации.';ru='Возникли ошибки на этапе синхронизации данных."
"Для решения проблем воспользуйтесь журналом регистрации.'"));
...
Процедура ПроверитьНастройкиПараметровУчетаНаСервере(
	...
	Если Не НастройкиПараметровУчетаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ka='Не заданы параметры учета в этой программе.';ru='Не заданы параметры учета в этой программе.';en='You do not specify the account in this program.'");
		КонецЕсли;
	...
	Если Не НастройкиПараметровУчетаКорреспондентаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ka='Не заданы параметры учета в приложении, расположенном в Интернете.';ru='Не заданы параметры учета в приложении, расположенном в Интернете.';en='You do not specify the account in the application located on the Internet.'");
		КонецЕсли;
...
Процедура ПроверитьЗаданиеНастроекДляПервойБазы(Отказ, ТипСоединения = "ВебСервис")
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование приложения в Интернете.';ru='Укажите наименование приложения в Интернете.';en='Specify the name of the application on the Internet.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура ПроверитьЗаданиеНастроекДляВторойБазы(Отказ, ТипСоединения)
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ka='Укажите наименование программы.';ru='Укажите наименование программы.';en='Specify the program name.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура СтраницаМастераНастройкаПараметров_ПриПереходеДалееНаСервере(Отказ)
	...
	Если Не ПланыОбмена[Объект.ИмяПланаОбмена].НайтиПоКоду(ОбменДаннымиСервер.КодУзлаПланаОбменаСтрокой(Объект.ПрефиксИнформационнойБазыПриемника)).Пустая() Тогда
		...
		НСтрока = НСтр("ka='Значение префикса второй информационной базы не уникально."
"В системе уже существует синхронизация данных для информационной базы (программы) с указанным префиксом."
"Измените значение префикса или используйте существующую синхронизацию.';ru='Значение префикса второй информационной базы не уникально."
"В системе уже существует синхронизация данных для информационной базы (программы) с указанным префиксом."
...
Процедура СтраницаМастераОжиданиеПроверкиПодключенияВнешнегоСоединения_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если Объект.COMВариантРаботыИнформационнойБазы = 0 Тогда
		...
		Если ПустаяСтрока(Объект.COMКаталогИнформационнойБазы) Тогда
			...
			НСтрока = НСтр("ka='Укажите каталог информационной базы.';ru='Укажите каталог информационной базы.';en='Specify the directory information base.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Объект.COMИмяСервера1СПредприятия) Тогда
			...
			НСтрока = НСтр("ka='Укажите имя кластера серверов.';ru='Укажите имя кластера серверов.';en='Specify the name of the cluster server.'");
		...
		ИначеЕсли ПустаяСтрока(Объект.COMИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			...
			НСтрока = НСтр("ka='Укажите имя информационной базы.';ru='Укажите имя информационной базы.';en='Specify the name of the knowledge base.'");
	...
	Если Не ПланОбменаСуществует Тогда
		...
		Сообщение = НСтр("ka='Синхронизация данных с указанной программой не предусмотрена.';ru='Синхронизация данных с указанной программой не предусмотрена.';en='Synchronization with the specified program is not provided.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
	...
	Если НРег(СтрокаСоединенияИнформационнойБазы()) = НРег(ВнешнееСоединение.СтрокаСоединенияИнформационнойБазы()) Тогда
		...
		Сообщение = НСтр("ka='Заданы настройки подключения к этой информационной базе.';ru='Заданы настройки подключения к этой информационной базе.';en='Set the connection settings to the information base.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
...
Процедура ПроверитьНаличиеСуществующегоОбменаДаннымиСВторойБазой(Отказ)
	...
	Если Не ПустаяСтрока(КодУзла)
		...
		Сообщение = НСтр("ka='Синхронизация данных между программами уже была настроена ранее.';ru='Синхронизация данных между программами уже была настроена ранее.';en='Synchronize data between programs have been set previously.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
