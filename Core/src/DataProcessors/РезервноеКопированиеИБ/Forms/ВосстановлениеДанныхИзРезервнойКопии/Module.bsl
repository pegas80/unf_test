...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Прервать подготовку к восстановлению данных?';ru='Прервать подготовку к восстановлению данных?';en='Interrupt preparations for data recovery?'");
...
Процедура Готово(Команда)
	...
	СоединенияИБВызовСервера.УстановитьБлокировкуСоединений(НСтр("ka='Выполняется восстановление информационной базы.';ru='Выполняется восстановление информационной базы.';en='You restore the information base.'"), "РезервноеКопирование");
...
Процедура ВыбратьФайлРезервнойКопии()
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='Архив с резервной копией (*.zip)|*.zip';ru='Архив с резервной копией (*.zip)|*.zip';en='Archive backup (* .zip) | * .zip'");
	ДиалогОткрытияФайла.Заголовок= НСтр("ka='Выберите файл резервной копии';ru='Выберите файл резервной копии';en='Select the backup file'");
...
Функция ПроверитьЗаполнениеРеквизитов()
	...
	Если ПустаяСтрока(ИмяФайла) Тогда
		ТекстСообщения = НСтр("ka='Не выбран файл с резервной копией.';ru='Не выбран файл с резервной копией.';en='Do not select the backup file.'");
	...
	Если ФайлАрхива.Расширение <> ".zip" Тогда
		...
		ТекстСообщения = НСтр("ka='Выбранный файл не является архивом с резервной копией.';ru='Выбранный файл не является архивом с резервной копией.';en='The selected file is not an archive backup.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ZipФайл.Элементы.Количество() <> 1 Тогда
		...
		ТекстСообщения = НСтр("ka='Выбранный файл не является архивом с резервной копией (содержит более одного файла).';ru='Выбранный файл не является архивом с резервной копией (содержит более одного файла).';en='The selected file is not an archive backup (which contains more than one file).'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ВРЕГ(ФайлВАрхиве.Расширение) <> "1CD" Тогда
		...
		ТекстСообщения = НСтр("ka='Выбранный файл не является архивом с резервной копией (не содержит файл информационной базы).';ru='Выбранный файл не является архивом с резервной копией (не содержит файл информационной базы).';en='The selected file is not an archive backup (does not contain information database file).'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ВРЕГ(ФайлВАрхиве.ИмяБезРасширения) <> "1CV8" Тогда
		...
		ТекстСообщения = НСтр("ka='Выбранный файл не является архивом с резервной копией (неправильное имя файла информационной базы).';ru='Выбранный файл не является архивом с резервной копией (неправильное имя файла информационной базы).';en='The selected file is not an archive backup (incorrect information base file name).'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
...
"Подготовка к восстановлению данных из резервной копии отменена. Информационная база разблокирована.';ru='%1."
...
Процедура НачатьРезервноеКопирование() 
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(РезервноеКопированиеИБКлиент.СобытиеЖурналаРегистрации(), 
		"Информация",
		НСтр("ka='Выполняется восстановление данных информационной базы:';ru='Выполняется восстановление данных информационной базы:';en='You restore an information database:'") + " " + ИмяГлавногоФайлаСкрипта);
