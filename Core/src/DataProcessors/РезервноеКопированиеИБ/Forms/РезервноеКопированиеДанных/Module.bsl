...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ВызватьИсключение НСтр("ka='Резервное копирование недоступно в веб-клиенте.';ru='Резервное копирование недоступно в веб-клиенте.';en='Backup is not available on the Web client.'");
	КонецЕсли;
	
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ВызватьИсключение НСтр("ka='В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).';ru='В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).';en='In the client-server mode backup should be performed by external means (DBMS means).'");
	КонецЕсли;
	...
	Иначе
		ТекстЗаголовка = НСтр("ka='В последний раз резервное копирование проводилось: %1';ru='В последний раз резервное копирование проводилось: %1';en='In the last backup was performed: 1%'");
	...
	Если РучнойЗапуск Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ka='Сохранить резервную копию';ru='Сохранить резервную копию';en='Save backup'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='Резервное копирование доступно только в клиенте под управлением ОС Windows.';ru='Резервное копирование доступно только в клиенте под управлением ОС Windows.';en='Backup is only available in the client running on Windows.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Прервать подготовку к резервному копированию данных?';ru='Прервать подготовку к резервному копированию данных?';en='Interrupt preparation for backing up your data?'");
...
Процедура ПерейтиНаСтраницуИнформацииИВыполненияРезервногоКопирования(ПерейтиДалее)
	...
	Если КоличествоСеансовИнформационнойБазы() = 1 Тогда
		...
		СоединенияИБВызовСервера.УстановитьБлокировкуСоединений(НСтр("ka='Для выполнения резервного копирования.';ru='Для выполнения резервного копирования.';en='To perform a backup.'"), "РезервноеКопирование");
...
Процедура ПерейтиНаСтраницуРезультатовРезервногоКопирования()
	...
	Элементы.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
...
Функция ПроверитьЗаполнениеРеквизитов(ВыдаватьОшибку = Истина)
	...
	Если ПустаяСтрока(Объект.КаталогСРезервнымиКопиями) Тогда
		...
		ТекстСообщения = НСтр("ka='Не выбран каталог для резервной копии.';ru='Не выбран каталог для резервной копии.';en='Do not select a directory for backup.'");
	...
	ИначеЕсли НайтиФайлы(Объект.КаталогСРезервнымиКопиями).Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ka='Указан несуществующий каталог.';ru='Указан несуществующий каталог.';en='Please enter a valid directory.'");
		ЗафиксироватьОшибкуПроверкиРеквизитов(ТекстСообщения, "Объект.КаталогСРезервнымиКопиями", ВыдаватьОшибку);
		РеквизитыЗаполнены = Ложь;
	...
	Иначе
		...
		Исключение
			ТекстСообщения = НСтр("ka='Нет доступа к каталогу с резервными копиями.';ru='Нет доступа к каталогу с резервными копиями.';en='No access to the folder with the backup.'");
			ЗафиксироватьОшибкуПроверкиРеквизитов(ТекстСообщения, "Объект.КаталогСРезервнымиКопиями", ВыдаватьОшибку);
			РеквизитыЗаполнены = Ложь;
		КонецПопытки;
	...
	Если ТребуетсяВводПароля И ПустаяСтрока(ПарольАдминистратораИБ) Тогда
		...
		ТекстСообщения = НСтр("ka='Не задан пароль администратора.';ru='Не задан пароль администратора.';en='Do not set an administrator password.'");
...
Процедура ИстечениеВремениОжидания()
	...
	ОтключитьОбработчикОжидания("ПроверкаНаЕдинственностьПодключения");
	ТекстВопроса = НСтр("ka='Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)';ru='Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)';en='Failed to disconnect all users from the database. To back up? (Errors can occur when archiving)'");
	ТекстПояснения = НСтр("ka='Не удалось отключить пользователя.';ru='Не удалось отключить пользователя.';en='Unable to disable the user.'");
...
Процедура ОтменитьПодготовку()
	...
	Элементы.НадписьНеУдалось.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1."
"Подготовка к резервному копированию отменена. Информационная база разблокирована.';ru='%1."
...
Процедура УстановитьЗаголовокКнопкиДалее(ЭтоКнопкаДалее)
	...
	Элементы.Далее.Заголовок = ?(ЭтоКнопкаДалее, НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'"), НСтр("ka='მზად არის';ru='Готово';en='Done'"));
...
Функция ПолучитьПуть(РежимДиалога)
	...
	Иначе
		ДиалогОткрытияФайла.Заголовок= НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл';en='select a file'");
	КонецЕсли;	
...
Процедура НачатьРезервноеКопирование()
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(РезервноеКопированиеИБКлиент.СобытиеЖурналаРегистрации(),
		"Информация",  НСтр("ka='Выполняется резервное копирование информационной базы:';ru='Выполняется резервное копирование информационной базы:';en='Backs up the information base:'") + " " + ИмяГлавногоФайлаСкрипта);
