...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ВызватьИсключение НСтр("ka='Резервное копирование недоступно в веб-клиенте.';ru='Резервное копирование недоступно в веб-клиенте.'");
	КонецЕсли;
	
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ВызватьИсключение НСтр("ka='В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).';ru='В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).'");
	КонецЕсли;
	...
	Иначе
		ТекстЗаголовка = НСтр("ka='В последний раз резервное копирование проводилось: %1';ru='В последний раз резервное копирование проводилось: %1'");
	...
	Если РучнойЗапуск Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ka='Сохранить резервную копию';ru='Сохранить резервную копию'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='Резервное копирование доступно только в клиенте под управлением ОС Windows.';ru='Резервное копирование доступно только в клиенте под управлением ОС Windows.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Прервать подготовку к резервному копированию данных?';ru='Прервать подготовку к резервному копированию данных?'");
...
Процедура ПерейтиНаСтраницуИнформацииИВыполненияРезервногоКопирования(ПерейтиДалее)
	...
	Если КоличествоСеансовИнформационнойБазы() = 1 Тогда
		...
		СоединенияИБВызовСервера.УстановитьБлокировкуСоединений(НСтр("ka='Для выполнения резервного копирования.';ru='Для выполнения резервного копирования.'"), "РезервноеКопирование");
...
Процедура ПерейтиНаСтраницуРезультатовРезервногоКопирования()
	...
	Элементы.Отмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
...
Функция ПроверитьЗаполнениеРеквизитов(ВыдаватьОшибку = Истина)
	...
	Если ПустаяСтрока(Объект.КаталогСРезервнымиКопиями) Тогда
		...
		ТекстСообщения = НСтр("ka='Не выбран каталог для резервной копии.';ru='Не выбран каталог для резервной копии.'");
	...
	ИначеЕсли НайтиФайлы(Объект.КаталогСРезервнымиКопиями).Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ka='Указан несуществующий каталог.';ru='Указан несуществующий каталог.'");
		ЗафиксироватьОшибкуПроверкиРеквизитов(ТекстСообщения, "Объект.КаталогСРезервнымиКопиями", ВыдаватьОшибку);
		РеквизитыЗаполнены = Ложь;
	...
	Иначе
		...
		Исключение
			ТекстСообщения = НСтр("ka='Нет доступа к каталогу с резервными копиями.';ru='Нет доступа к каталогу с резервными копиями.'");
			ЗафиксироватьОшибкуПроверкиРеквизитов(ТекстСообщения, "Объект.КаталогСРезервнымиКопиями", ВыдаватьОшибку);
			РеквизитыЗаполнены = Ложь;
		КонецПопытки;
	...
	Если ТребуетсяВводПароля И ПустаяСтрока(ПарольАдминистратораИБ) Тогда
		...
		ТекстСообщения = НСтр("ka='Не задан пароль администратора.';ru='Не задан пароль администратора.'");
...
Процедура ИстечениеВремениОжидания()
	...
	ОтключитьОбработчикОжидания("ПроверкаНаЕдинственностьПодключения");
	ТекстВопроса = НСтр("ka='Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)';ru='Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)'");
	ТекстПояснения = НСтр("ka='Не удалось отключить пользователя.';ru='Не удалось отключить пользователя.'");
...
Процедура УстановитьЗаголовокКнопкиДалее(ЭтоКнопкаДалее)
	...
	Элементы.Далее.Заголовок = ?(ЭтоКнопкаДалее, НСтр("ka='შემდეგ >';ru='Далее >'"), НСтр("ka='მზად არის';ru='Готово'"));
...
Функция ПолучитьПуть(РежимДиалога)
	...
	Если Режим = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
		ДиалогОткрытияФайла.Заголовок= НСтр("ka='აირჩიეთ კატალოგი';ru='Выберите каталог'");
	Иначе
		ДиалогОткрытияФайла.Заголовок= НСтр("ka='აირჩიეთ ფაილი';ru='Выберите файл'");
	КонецЕсли;	
...
Процедура НачатьРезервноеКопирование()
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(РезервноеКопированиеИБКлиент.СобытиеЖурналаРегистрации(),
		"Информация",  НСтр("ka='Выполняется резервное копирование информационной базы:';ru='Выполняется резервное копирование информационной базы:'") + " " + ИмяГлавногоФайлаСкрипта);
