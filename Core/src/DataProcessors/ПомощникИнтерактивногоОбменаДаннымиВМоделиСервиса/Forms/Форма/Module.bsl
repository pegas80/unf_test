...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		ТекстОшибки = НСтр("ka='კონფიგურაციისთვის მონაცემთა სინქრონიზაციას მხარდაჭერა არ გააჩნია!';ru='Синхронизация данных для конфигурации не поддерживается!'");
		
	ИначеЕсли Не Параметры.Свойство("УзелИнформационнойБазы", Объект.УзелИнформационнойБазы) Тогда
		ТекстОшибки = НСтр("ka='Обработка не предназначена для непосредственного использования.';ru='Обработка не предназначена для непосредственного использования.'");
		
	ИначеЕсли Объект.УзелИнформационнойБазы.Пустая() Тогда
		ТекстОшибки = НСтр("ka='Настройка обмена данными не найдена.';ru='Настройка обмена данными не найдена.'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение'");
		ТекстВопроса = СтрШаблон(НСтр("ka='Восстановить настройки ""%1""?';ru='Восстановить настройки ""%1""?'"), ПредставлениеНастройки
		);
...
Процедура ОбработчикОжиданияКорреспондента()
	...
	Иначе
		СостояниеДлительнойОперации.ИнформацияОбОшибке = НСтр("ka='Ошибка сообщения корреспонденту';ru='Ошибка сообщения корреспонденту'");
...
Функция ФоновоеЗаданиеВыгрузкиНаСервере()
	...
	Задание = ФоновыеЗадания.Выполнить("ОбменДаннымиВМоделиСервиса.ОбменПоТребованию", 
		ПараметрыФоновогоВыполнения, , НСтр("ka='Интерактивный обмен по требованию.';ru='Интерактивный обмен по требованию.'"));
