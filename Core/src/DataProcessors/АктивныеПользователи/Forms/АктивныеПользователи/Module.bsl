...
Процедура ЗавершитьСеанс(Команда)
	...
	Если КоличествоВыделенныхСтрок = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не выбраны пользователи для завершения сеансов.';ru='Не выбраны пользователи для завершения сеансов.';en='Not selected users to disconnect.'"));
		Возврат;
	ИначеЕсли КоличествоВыделенныхСтрок = 1 Тогда
		Если Элементы.СписокПользователей.ТекущиеДанные.Сеанс = НомерСеансаИнформационнойБазы Тогда
			ПоказатьПредупреждение(,НСтр("ka='Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.';ru='Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.';en='It is impossible to end the current session. To exit the program, you can close the main program window.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если ЗапрашиватьПараметрыАдминистрированияИБ Тогда
			...
			ЗаголовокФормы = НСтр("ka='სეანსის დასრულება';ru='Завершение сеанса';en='Logout'");
			ПоясняющаяНадпись = НСтр("ka='Для завершения сеанса необходимо ввести параметры"
"администрирования кластера серверов';ru='Для завершения сеанса необходимо ввести параметры"
...
Процедура ОткрытьЖурналРегистрации()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Выберите пользователей для просмотра журнала регистрации.';ru='Выберите пользователей для просмотра журнала регистрации.';en='Select the users to view the log.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеЗавершенияСеанса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ДополнительныеПараметры.НомераСеансов.Количество() > 1 Тогда
			...
			ТекстОповещения = НСтр("ka='Сеансы %1 завершены.';ru='Сеансы %1 завершены.';en='Sessions 1% completed.'");
			...
			ПоказатьОповещениеПользователя(НСтр("ka='Завершение сеансов';ru='Завершение сеансов';en='Completion sessions'"),, ТекстОповещения);
			
		Иначе
			
			ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Сеанс %1 завершен.';ru='Сеанс %1 завершен.';en='Session% 1 completed.'"), ДополнительныеПараметры.НомераСеансов[0]);
			ПоказатьОповещениеПользователя(НСтр("ka='სეანსის დასრულება';ru='Завершение сеанса';en='Logout'"),, ТекстОповещения);
