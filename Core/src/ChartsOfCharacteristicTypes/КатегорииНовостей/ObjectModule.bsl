...
"Разрешено использовать цифры, английские буквы, подчеркивание и минус.'"),
	...
	Если ЭтотОбъект.ТипЗначения.Типы().Количество() > 1 Тогда
		...
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("ka='შემადგენელ ტიპებს მხარდაჭერა არ გააჩნიათ. აირჩიეთ მნიშვნელობის მხოლოდ ერთი ტიპი.';ru='Составные типы не поддерживаются. Выберите только один тип значения.'");
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ЭтотОбъект.ТипЗначения.Типы().Количество() = 0 Тогда
		...
		Сообщение.Поле = "ТипЗначения";
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("ka='ცარიელ ტიპებს მხარდაჭერა არ გააჩნიათ. აირჩიეთ მნიშვნელობის ერთი ნებისმიერი ტიპი.';ru='Пустые типы не поддерживаются. Выберите один любой тип значения.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтотОбъект.ЗагруженоССервера = Истина Тогда
		...
		Если (ЭтотОбъект.ПометкаУдаления = Истина)
			...
			ТекстСообщения = НСтр("ka='ახალი ამბების სერვერიდან ავტომატურად ჩატვირთული კატეგორიების წასაშლელად მონიშვნა არ შეიძლება.';ru='Нельзя помечать на удаление категории, загруженные автоматически с сервера новостей.'");
			Сообщение = Новый СообщениеПользователю();
