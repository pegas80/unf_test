		...
		Для каждого КлючЗначение Из СтруктураДействий Цикл
			...
			ИначеЕсли КлючЗначение.Ключ = "ОбновляетсяССервера" Тогда
				...
				Если Объект.ОбновляетсяССервера = Истина Тогда
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ka='ხელმისაწვდომი მნიშვნელობების სია (განახლება ხდება სერვერიდან, რედაქტირება აკრძალულია)';ru='Список доступных значений (обновляется с сервера, редактирование запрещено)'");
				Иначе
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ka='ხელმისაწვდომი მნიშვნელობების სია';ru='Список доступных значений'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция УправлениеФормойСервер(Знач СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполняетсяАвтоматически = Истина Тогда
		...
		Если ПустаяСтрока(Значение) Тогда
			СтруктураДействий.Вставить("ФиксированноеЗначение", НСтр("ka='ამ კატეგორიისთვის მნიშვნელობის დადგენა შეუძლებელია';ru='Невозможно определить значение для этой категории'"));
		...
		Иначе
			СтруктураДействий.Вставить(
				...
				СтрШаблон(
					НСтр("ka='Для этой конфигурации значение этой категории на текущий момент равно %1';ru='Для этой конфигурации значение этой категории на текущий момент равно %1'"),
					Значение));
		КонецЕсли;
	Иначе
		...
		Если Объект.ОбновляетсяССервера Тогда
			...
			Если Запись.Выбран() Тогда
				Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
					ТекстНадписи = НСтр("ka='მონაცემები აქტუალურია და სერვერის მონაცემებს შეესაბამება %ТекущаяВерсияНаСервере%.';ru='Данные актуальны и соответствуют данным с сервера от %ТекущаяВерсияНаСервере%.'");
...
"а на сервере уже версия от %ТекущаяВерсияНаСервере%.'");
				...
				КонецЕсли;
			Иначе
				ТекстНадписи = НСтр("ka='მონაცემები არასდროს განახლებულა სერვერიდან.';ru='Данные никогда не обновлялись с сервера.'");
				ТребуетсяОбновление = Истина;
			КонецЕсли;
					...
					Иначе
						Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок   = Новый ФорматированнаяСтрока(
							...
							Новый ФорматированнаяСтрока(
								НСтр("ka='განახლებების შემოწმება.';ru='Проверить обновления.'"),
								,
		...
		Иначе
			Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок  = НСтр("ka='მონაცემთა შეყვანა ხელით ხდება და სერვერიდან განახლება არ არის საჭირო.';ru='Данные вводятся вручную и обновление с сервера не требуется.'");
			Элементы.ДекорацияТребуетсяОбновлениеССервера.ЦветТекста = ЦветаСтиля.ЦветМикротекста;
		КонецЕсли;
