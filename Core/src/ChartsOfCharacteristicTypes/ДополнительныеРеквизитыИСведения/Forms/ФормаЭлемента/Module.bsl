...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ka='ჩაწერის გაგრძელება';ru='Продолжить запись';en='continue recording'"));
			Кнопки.Добавить("ВернутьсяКВводуНаименования", НСтр("ka='სახელწოდების შეყვანაზე დაბრუნება';ru='Вернуться к вводу наименования';en='Back to entry names'"));
...
""
"Данные будут записаны.';ka='გავასუფთავოთ შეყვანილი წონითი კოეფიციენტები?"
""
"მონაცემები ჩაიწერება.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ОчиститьИЗаписать", НСтр("ka='გასუფთავება და ჩაწერა';ru='Очистить и записать';en='Clear and burn'"));
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ka='ელემენტი არ არის არჩეული!';ru='Элемент не выбран!';en='The item is not selected!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Если ЭтоДополнительноеСведение = 1 Тогда
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ru='Только общие дополнительные сведения';en='Only the total additional information'");
	Иначе
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ka='Только общие дополнительные реквизиты';ru='Только общие дополнительные реквизиты';en='Only the total additional details'");
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеСведения", НСтр("ru='Общие дополнительные сведения';en='The total additional information'"), Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеРеквизиты", НСтр("ru='Общие дополнительные реквизиты';en='The total additional details'"), Истина);
...
""
"Данные будут записаны.';ka='მნიშვნელობების სიასთან მუშაობაზე გადასვლა შესაძლებელია მხოლოდ მონაცემთა ჩაწერის შემდეგ."
""
"მონაცემები ჩაიწერება.'");
	Иначе
		ТекстВопроса =
			НСтр("ka='მონაცემები ჩაიწერება.';ru='Данные будут записаны.';en='The data will be recorded.'")
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Записать", НСтр("ka='ჩაწერა';ru='Записать';en='record'"));
	Кнопки.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Неопределен) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Следующие настройки формата автоматически не применяются в большинстве мест:';en='The following format settings are automatically applied in most places:'");
		...
		Для каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 ИЛИ СтрНайти(Подстрока, "DE=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru='представление пустой даты';en='submission dates blank'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ka='ცარიელი რიცხვის ჩვენება';ru='представление пустого числа';en='representation of the number of empty'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ддд") > 0 ИЛИ СтрНайти(Подстрока, "ddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru='кратное название дня недели';en='fold weekday name'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ka='полное название дня недели';ru='полное название дня недели';en='full weekday name'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ka='кратное название месяца';ru='кратное название месяца';en='fold name of the month'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "ММММ") > 0 ИЛИ СтрНайти(Подстрока, "MMMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ka='полное название месяца';ru='полное название месяца';en='full month name'");
				КонецЕсли;
			КонецЕсли;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = НСтр("ka='აირჩიეთ %1 ""%2""-ის კრებულში შესაყვანად';ru='Выберите %1 для включения в набор ""%2""';en='Select% 1 for inclusion in the set of ""%2""'");
	ШаблонЗаголовкаПереключателя = НСтр("ka='აირჩიეთ კრებულ ""%3""-ში დამატებითი %1 ""%2""-ის დამატების ვარიანტი';ru='Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""';en='Select adding an additional 1% ""2%"" in the set ""3%""'");
	
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ka='დამატებითი ინფორმაციის დამატება';ru='Добавление дополнительного сведения';en='Adding additional information'");
		Иначе
			Заголовок = НСтр("ka='დამატებითი რეკვიზიტის დამატება';ru='Добавление дополнительного реквизита';en='Adding extra props'");
		КонецЕсли;
		
		Элементы.КоманднаяПанельЛево.Доступность = Ложь;
		Элементы.КомандаДалее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
		
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаСписка,
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ka='დამატებითი ცნობა';ru='дополнительное сведение';en='For more information,'"), НСтр("ka='დამატებითი რეკვიზიტი';ru='дополнительный реквизит';en='additional props'")),
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ka='ცნობები';ru='сведения';en='intelligence'"), НСтр("ka='რეკვიზიტს';ru='реквизита';en='props'")),
	...
	Иначе
		Элементы.КомандаДалее.Заголовок = НСтр("ka='მზად არის';ru='Готово';en='Done'");
		Элементы.КоманднаяПанельЛево.Доступность = Истина;
	КонецЕсли;
...
""
"Удаленный тип восстановлен.';ka='ტიპ ""%1""-ის წაშლა დაუშვებელია, ვინაიდან დამატებითი მნიშვნელობები უკვე შეყვანილია."
"თავდაპირველად, საჭიროა შეყვანილი დამატებითი მნიშვნელობების წაშლა."
""
"წაშლილი ტიპი აღდგენილია.'"),
...
"""%2""."
""
"Второй тип удален.';ka='დაუშვებელია მნიშვნელობის ტიპების ""%1""-ის და"
"""%2""-ის ერთდროული გამოყენება."
""
"მეორე ტიპი წაშლილია.'"),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) );
...
"""%2""."
""
"Первый тип удален.';ka='მნიშვნელობის ტიპების ""%1""-ის და ""%2""-ის ერთდროული გამოყენება დაუშვებელია."
""
"პირველი ტიპი წაშლილია..'"),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) );
	...
	Иначе
		...
		Если СвойстваВладельца.ЭтоДополнительноеСведение <> Истина Тогда
			ШаблонУточнения = НСтр("ru='Список значений общий с реквизитом ""%1""';en='The list of values ​​in common with props ""%1""'");
		Иначе
			ШаблонУточнения = НСтр("ru='Список значений общий со сведением ""%1""';en='List of common values ​​with the information ""%1""'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СвойстваВладельца.НаборСвойств) Тогда
			ШаблонУточненияНабор = " " + НСтр("ru='набора ""%1""';en='set ""%1""'");
	...
	Иначе
		...
		Если СписокНаборов.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств)
			   И СписокНаборов.Количество() < 2 Тогда
				...
				Если Объект.ЭтоДополнительноеСведение Тогда
					ШаблонУточнения = НСтр("ka='ინფორმაცია შედის კრებულში: %1';ru='Сведение входит в набор: %1';en='The reduction is included in the set: 1%'");
				Иначе
					ШаблонУточнения = НСтр("ka='რეკვიზიტი შედის კრებულში: %1';ru='Реквизит входит в набор: %1';en='Props included in the set: 1%'");
				КонецЕсли;
			...
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ka='ზოგადი ინფორმაცია შედის %1 %2-ში';ru='Общее сведение входит в %1 %2';en='The general information includes at% 1% 2'");
					Иначе
						ШаблонУточнения = НСтр("ka='საერთო რეკვიზიტი შედის %1 %2-ში';ru='Общий реквизит входит в %1 %2';en='Overall prop enters in% 1% 2'");
					КонецЕсли;
					
					СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
						"Л = ru_RU", НСтр("ka='კრებული,კრებლს,კრებულებს,,,,,,0';ru='набор,набора,наборов,,,,,,0';en='set, set, set ,,,,,, 0'"));
				...
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ka='საერთო ინფორმაცია შედის კრებულში: %1';ru='Общее сведение входит в набор: %1';en='The total reduction in the set includes: 1%'");
					Иначе
						ШаблонУточнения = НСтр("ka='საერთო რეკვიზიტი შედის კრებულში: %1';ru='Общий реквизит входит в набор: %1';en='The total set of props included: 1%'");
					КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ka='ინფორმაცია არ შედის კრებულში';ru='Сведение не входит в набор';en='The information is not included in the set'");
				Иначе
					ТекстКомментария = НСтр("ka='რეკვიზიტი არ შედის კრებულში';ru='Реквизит не входит в набор';en='Props not included in the set'");
				КонецЕсли;
			Иначе
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ka='ზოგადი ინფორმაცია კრებულებში არ შედის';ru='Общее сведение не входит в наборы';en='Total mixing is not included in the kits'");
				Иначе
					ТекстКомментария = НСтр("ka='საერთო რეკვიზიტი კრებულებში არ შედის';ru='Общий реквизит не входит в наборы';en='Total props not included in the kit'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Элементы.УточнениеНаборовКомментарий.Гиперссылка Тогда
			Элементы.УточнениеНаборовКомментарий.Подсказка = НСтр("ka='კრებულზე გადასვლა';ru='Переход к набору';en='The transition to the set'");
		Иначе
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = НСтр("ka='ზოგადი ცნობის დამატება კრებულში (რეკომენდებულია)"
""
"არჩეული ცნობა უკვე შედის რამდენიმე კრებულში, ამიტომ, რეკომენდებულია ასევე, ამ კრებულში მისი შეყვანა ""როგორც არის""."
"ამ შემთხვევაში, შესაძლებელი იქნება სიებში და ანგარიშგებებში მის მიხედვით სხვადასხვა ტიპის მონაცემთა გადარჩევა.';ru='Добавить общее сведение в набор (рекомендуется)"
""
...
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';ka='დამატებითი ინფორმაცია გავხადოთ საერთო და კრებულში დავამატოთ"
""
"ეს ვარიანტი გამოსადეგია იმ შემთხვევებში, როდესაც ინფორმაცია ერთნაირი უნდა იყოს ორივე კრებულისთვის."
"ამ შემთხვევაში, შესაძლებელი იქნება მის მიხედვით სიებში და ანგარიშგებებში სხვადასხვა ტიპის მონაცემთა შერჩევა.'");
...
"При этом можно отредактировать наименование и ряд других свойств сведения.';ka='ნიმუშის მიხედვით ინფორმაციის ასლის გაკეთება (მნიშვნელობების საერთო სიით)"
""
"ამ ინფორმაციის მნიშვნელობების სია ორივე კრებულისთვის ერთნაირი იქნება."
"ამ ვარიანტის დახმარებით მოსახერხებელია მნიშვნელობების სიის ცენტრალიზებული გამართვის შესრულება ერთბაშად, რამდენიმე ერთი ტიპის ინფორმაციისთვის."
"ამასთან, შეიძლება სახელწოდების და ინფორმაციის სხვა თვისებების რედაქტირება.'");
		СоздатьКопированием = НСтр("ru='Сделать копию сведения"
""
"Будет создана копия сведения%1';ka='ცნობის ასლის გაკეთება"
""
"შეიქმნება ცნობის ასლი%1'");
	Иначе
		ДобавитьОбщий = НСтр("ka='საერთო რეკვიზიტის დამატება კრებულში (რეკომენდებულია)"
""
"არჩეული რეკვიზიტი უკვე შედის რამდენიმე კრებულში, ამიტომ რეკომენდებულია ასევე, ამ კრებულში მისი შეყვანა ""როგორც არის""."
"ამ შემთხვევაში, შესაძლებელი იქნება სიებში და ანგარიშგებებში მის მიხედვით სხვადასხვა ტიპის მონაცემთა შერჩევა.';ru='Добавить общий реквизит в набор (рекомендуется)"
""
...
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';ka='Сделать реквизит общим и добавить в набор"
""
"Этот вариант подходит для тех случаев, когда реквизит должен быть одинаков для обоих наборов."
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'");
...
"При этом можно отредактировать наименование и ряд других свойств реквизита.';ka='Сделать копию реквизита по образцу (с общим списком значений)"
""
"Список значений этого реквизита будет одинаков для обоих наборов."
"С помощью этого варианта удобно выполнять централизованную настройку списка значений сразу для нескольких однотипных реквизитов."
...
""
"Будет создана копия реквизита%1';ka='Сделать копию реквизита"
""
"Будет создана копия реквизита%1'");
	...
	Если РеквизитСоСпискомДополнительныхЗначений Тогда
		ШаблонВставки = " " + НСтр("ka='და ყველა მის მნიშვნელობას.';ru='и всех его значений.';en='and all of its values.'");
	Иначе
		ШаблонВставки = ".";
	КонецЕсли;
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = НСтр("ka='მოცემული პირობისას';ru='при заданном условии';en='under given conditions'");
	ШаблонЗависимостьНеЗадана = НСтр("ka='ყოველთვის';ru='всегда';en='always'");
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(დამატებითი ცნობები)';ru='(Дополнительное сведение)';en='(For further details)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(დამატებითი რეკვიზიტი)';ru='(Дополнительный реквизит)';en='(Additional props)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(საერთო დამატებითი ინფორმაცია)';ru='(Общее дополнительное сведение)';en='(General additional mixing)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(საერთო დამატებითი რეკვიზიტი)';ru='(Общий дополнительный реквизит)';en='(General additional props)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ka='დამატებითი ცნობა (შექმნა)';ru='Дополнительное сведение (создание)';en='Additional information (creation)'");
			Иначе
				Заголовок = НСтр("ka='დამატებითი რეკვიზიტი (შექმნა)';ru='Дополнительный реквизит (создание)';en='Additional props (creation)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ka='საერთო დამატებითი ინფორმაცია (შექმნა)';ru='Общее дополнительное сведение (создание)';en='Total additional mixing (creation)'");
			Иначе
				Заголовок = НСтр("ka='საერთო დამატებითი რეკვიზიტი (შექმნა)';ru='Общий дополнительный реквизит (создание)';en='Common additional props (creation)'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		   ...
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = НСтр("ka='მნიშვნელობები ჯერ არ არის შეყვანილი';ru='Значения еще не введены';en='The values ​​are not yet'");
				Иначе
					ПредставлениеЗначений = НСтр("ka='მნიშვნელობები მონიშნულია წასაშლელად';ru='Значения помечены на удаление';en='Values ​​are marked for deletion'");
				КонецЕсли;
			Иначе
...
"""%1"".';ka='არსებობს დამატებითი ინფორმაცია სახელწოდებით ""%1"".'");
		Иначе
...
"""%1"".';ka='არსებობს საერთო დამატებითი ინფორმაცია სახელწოდება ""%1""-ით.'");
		Иначе
...
"иначе программа может работать некорректно.';ka='რეკომენდებულია სხვა სახელწოდების გამოყენება, სხვაგვარად, პროგრამამ შეიძლება არაკორექტულად იმუშაოს.'"),
		НовоеНаименование);
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = НСтр("ka='ძირითადი ფორმატი';ru='Формат по умолчанию';en='The default format'");
	Иначе
		ТекстЗаголовка = НСтр("ka='ფორმატი დაყენებულია';ru='Формат установлен';en='The format is set'");
	КонецЕсли;
