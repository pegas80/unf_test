...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ka='ჩაწერის გაგრძელება';ru='Продолжить запись'"));
			Кнопки.Добавить("ВернутьсяКВводуНаименования", НСтр("ka='სახელწოდების შეყვანაზე დაბრუნება';ru='Вернуться к вводу наименования'"));
...
"მონაცემები ჩაიწერება.'");
		...
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ОчиститьИЗаписать", НСтр("ka='გასუფთავება და ჩაწერა';ru='Очистить и записать'"));
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ka='ელემენტი არ არის არჩეული!';ru='Элемент не выбран!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Иначе
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ka='Только общие дополнительные реквизиты';ru='Только общие дополнительные реквизиты'");
	КонецЕсли;
...
"მონაცემები ჩაიწერება.'");
	Иначе
		ТекстВопроса =
			НСтр("ka='მონაცემები ჩაიწერება.';ru='Данные будут записаны.'")
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Записать", НСтр("ka='ჩაწერა';ru='Записать'"));
	Кнопки.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена'"));
	...
	Если Текст <> Неопределено Тогда
		...
		Для каждого Подстрока Из Массив Цикл
			...
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ka='ცარიელი რიცხვის ჩვენება';ru='представление пустого числа'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда
				...
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ka='полное название дня недели';ru='полное название дня недели'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ka='кратное название месяца';ru='кратное название месяца'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "ММММ") > 0 ИЛИ СтрНайти(Подстрока, "MMMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ka='полное название месяца';ru='полное название месяца'");
				КонецЕсли;
			КонецЕсли;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = НСтр("ka='აირჩიეთ %1 ""%2""-ის კრებულში შესაყვანად';ru='Выберите %1 для включения в набор ""%2""'");
	ШаблонЗаголовкаПереключателя = НСтр("ka='აირჩიეთ კრებულ ""%3""-ში დამატებითი %1 ""%2""-ის დამატების ვარიანტი';ru='Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""'");
	
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ka='დამატებითი ინფორმაციის დამატება';ru='Добавление дополнительного сведения'");
		Иначе
			Заголовок = НСтр("ka='დამატებითი რეკვიზიტის დამატება';ru='Добавление дополнительного реквизита'");
		КонецЕсли;
		
		Элементы.КоманднаяПанельЛево.Доступность = Ложь;
		Элементы.КомандаДалее.Заголовок = НСтр("ka='შემდეგ >';ru='Далее >'");
		
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаСписка,
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ka='დამატებითი ცნობა';ru='дополнительное сведение'"), НСтр("ka='დამატებითი რეკვიზიტი';ru='дополнительный реквизит'")),
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ka='ცნობები';ru='сведения'"), НСтр("ka='რეკვიზიტს';ru='реквизита'")),
	...
	Иначе
		Элементы.КомандаДалее.Заголовок = НСтр("ka='მზად არის';ru='Готово'");
		Элементы.КоманднаяПанельЛево.Доступность = Истина;
	КонецЕсли;
...
"პირველი ტიპი წაშლილია..'"),
	...
	Иначе
		...
		Если СписокНаборов.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств)
			   И СписокНаборов.Количество() < 2 Тогда
				...
				Если Объект.ЭтоДополнительноеСведение Тогда
					ШаблонУточнения = НСтр("ka='ინფორმაცია შედის კრებულში: %1';ru='Сведение входит в набор: %1'");
				Иначе
					ШаблонУточнения = НСтр("ka='რეკვიზიტი შედის კრებულში: %1';ru='Реквизит входит в набор: %1'");
				КонецЕсли;
			...
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ka='ზოგადი ინფორმაცია შედის %1 %2-ში';ru='Общее сведение входит в %1 %2'");
					Иначе
						ШаблонУточнения = НСтр("ka='საერთო რეკვიზიტი შედის %1 %2-ში';ru='Общий реквизит входит в %1 %2'");
					КонецЕсли;
					
					СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
						"Л = ru_RU", НСтр("ka='კრებული,კრებლს,კრებულებს,,,,,,0';ru='набор,набора,наборов,,,,,,0'"));
				...
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ka='საერთო ინფორმაცია შედის კრებულში: %1';ru='Общее сведение входит в набор: %1'");
					Иначе
						ШаблонУточнения = НСтр("ka='საერთო რეკვიზიტი შედის კრებულში: %1';ru='Общий реквизит входит в набор: %1'");
					КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ka='ინფორმაცია არ შედის კრებულში';ru='Сведение не входит в набор'");
				Иначе
					ТекстКомментария = НСтр("ka='რეკვიზიტი არ შედის კრებულში';ru='Реквизит не входит в набор'");
				КонецЕсли;
			Иначе
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ka='ზოგადი ინფორმაცია კრებულებში არ შედის';ru='Общее сведение не входит в наборы'");
				Иначе
					ТекстКомментария = НСтр("ka='საერთო რეკვიზიტი კრებულებში არ შედის';ru='Общий реквизит не входит в наборы'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Элементы.УточнениеНаборовКомментарий.Гиперссылка Тогда
			Элементы.УточнениеНаборовКомментарий.Подсказка = НСтр("ka='კრებულზე გადასვლა';ru='Переход к набору'");
		Иначе
...
"Будет создана копия реквизита%1'");
	...
	Если РеквизитСоСпискомДополнительныхЗначений Тогда
		ШаблонВставки = " " + НСтр("ka='და ყველა მის მნიშვნელობას.';ru='и всех его значений.'");
	Иначе
		ШаблонВставки = ".";
	КонецЕсли;
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = НСтр("ka='მოცემული პირობისას';ru='при заданном условии'");
	ШаблонЗависимостьНеЗадана = НСтр("ka='ყოველთვის';ru='всегда'");
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(დამატებითი ცნობები)';ru='(Дополнительное сведение)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(დამატებითი რეკვიზიტი)';ru='(Дополнительный реквизит)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(საერთო დამატებითი ინფორმაცია)';ru='(Общее дополнительное сведение)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ka='(საერთო დამატებითი რეკვიზიტი)';ru='(Общий дополнительный реквизит)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ka='დამატებითი ცნობა (შექმნა)';ru='Дополнительное сведение (создание)'");
			Иначе
				Заголовок = НСтр("ka='დამატებითი რეკვიზიტი (შექმნა)';ru='Дополнительный реквизит (создание)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ka='საერთო დამატებითი ინფორმაცია (შექმნა)';ru='Общее дополнительное сведение (создание)'");
			Иначе
				Заголовок = НСтр("ka='საერთო დამატებითი რეკვიზიტი (შექმნა)';ru='Общий дополнительный реквизит (создание)'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		   ...
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = НСтр("ka='მნიშვნელობები ჯერ არ არის შეყვანილი';ru='Значения еще не введены'");
				Иначе
					ПредставлениеЗначений = НСтр("ka='მნიშვნელობები მონიშნულია წასაშლელად';ru='Значения помечены на удаление'");
				КонецЕсли;
			Иначе
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = НСтр("ka='ძირითადი ფორმატი';ru='Формат по умолчанию'");
	Иначе
		ТекстЗаголовка = НСтр("ka='ფორმატი დაყენებულია';ru='Формат установлен'");
	КонецЕсли;
