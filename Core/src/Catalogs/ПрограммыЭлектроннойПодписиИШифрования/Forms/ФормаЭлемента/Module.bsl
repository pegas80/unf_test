...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЦикла;
	Элементы.Наименование.СписокВыбора.Добавить("", НСтр("ka='<Другая программа>';ru='<Другая программа>'"));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აღნიშნული სახელის და ტიპის მქონე პროგრამა უკვე დამატებულია სიაში.';ru='Программа с указанным именем и типом уже добавлена в список.'"),
			,
			"Объект.ИмяПрограммы");
	КонецЕсли;
