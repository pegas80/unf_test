...
Процедура НастроитьАвтоматическиСейчасПослеСохранения()
	...
	Если Организация = Неопределено Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Учетная запись не привязана ни к какой организации. Настройка невозможна.';ru='Учетная запись не привязана ни к какой организации. Настройка невозможна.';en='The account is not tied to any organization. Setting is not possible.'"));
		КонецЕсли;
	Иначе
...
Функция ПараметрыДоступаВерны()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронный документооборот с КО.Проверка параметров подключения';ru='Электронный документооборот с КО.Проверка параметров подключения';en='Electronic document management with KO.Proverka connection settings'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(Инф));
			
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При проверке настроек учетной записи произошли ошибки:"
"%1';ka='При проверке настроек учетной записи произошли ошибки:"
...
""
"Продолжить?';ka='მიმდინარე მომხმარებელი სააღრიცხვო ჩანაწერების მომხმარებელთა სიიდან წაიშლება. სააღრიცხვო ჩანაწერის ფორმა დაიხურება. გავაგრძელოთ?'");
