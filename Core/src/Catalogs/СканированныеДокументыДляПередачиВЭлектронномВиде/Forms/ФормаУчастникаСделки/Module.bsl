...
Функция ПроверкаЗаполнения()
	...
	Если НЕ ЗначениеЗаполнено(РольУчастника) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана роль участника сделки.';ru='Не указана роль участника сделки.';en='It does not specify the role of the participant of the transaction.'"));
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	Если ЯвляетсяЮрЛицом Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЮрЛицоНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано наименование организации.';ru='Не указано наименование организации.';en='Unknown name.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ЮрЛицоИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ИНН организации.';ru='Не указан ИНН организации.';en='Unknown INN organization.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли СтрДлина(СокрЛП(ЮрЛицоИНН)) <> 10 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректная длина ИНН организации.';ru='Некорректная длина ИНН организации.';en='Incorrect length INN organization.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ЮрЛицоКПП) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан КПП организации.';ru='Не указан КПП организации.';en='Unknown PPC organization.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли СтрДлина(СокрЛП(ЮрЛицоКПП)) <> 9 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректная длина КПП организации.';ru='Некорректная длина КПП организации.';en='Incorrect length PPC organization.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ФизЛицоФамилия) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана фамилия физического лица.';ru='Не указана фамилия физического лица.';en='It does not specify the name of an individual.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицоИмя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано имя физического лица.';ru='Не указано имя физического лица.';en='It does not specify the name of an individual.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		ИННОбязательно = ОбязательноЗаполнениеИННДляИП(ВидДокумента);
		Если ИННОбязательно И НЕ ЗначениеЗаполнено(ФизЛицоИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ИНН физического лица.';ru='Не указан ИНН физического лица.';en='Unknown INN individual.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли ИННОбязательно И СтрДлина(СокрЛП(ФизЛицоИНН)) <> 12 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректная длина ИНН физического лица.';ru='Некорректная длина ИНН физического лица.';en='Incorrect length INN individual.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
