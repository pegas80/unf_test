...
Функция ПроверкаЗаполнения()
	...
	Если НЕ ЗначениеЗаполнено(РольУчастника) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана роль участника сделки.';ru='Не указана роль участника сделки.'"));
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	Если ЯвляетсяЮрЛицом Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЮрЛицоНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано наименование организации.';ru='Не указано наименование организации.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ЮрЛицоИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ИНН организации.';ru='Не указан ИНН организации.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли СтрДлина(СокрЛП(ЮрЛицоИНН)) <> 10 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректная длина ИНН организации.';ru='Некорректная длина ИНН организации.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ЮрЛицоКПП) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан КПП организации.';ru='Не указан КПП организации.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли СтрДлина(СокрЛП(ЮрЛицоКПП)) <> 9 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректная длина КПП организации.';ru='Некорректная длина КПП организации.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ФизЛицоФамилия) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указана фамилия физического лица.';ru='Не указана фамилия физического лица.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицоИмя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указано имя физического лица.';ru='Не указано имя физического лица.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		ИННОбязательно = ОбязательноЗаполнениеИННДляИП(ВидДокумента);
		Если ИННОбязательно И НЕ ЗначениеЗаполнено(ФизЛицоИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан ИНН физического лица.';ru='Не указан ИНН физического лица.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли ИННОбязательно И СтрДлина(СокрЛП(ФизЛицоИНН)) <> 12 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректная длина ИНН физического лица.';ru='Некорректная длина ИНН физического лица.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
