...
Процедура ЗаполнитьВычисляемыеПоляСтрокиТаблицыФайлов(СтрокаТаблицы)
	...
	НСтр("ka='%1 (%2)';ru='%1 (%2)';en='%12)'"), СтрокаТаблицы.ИмяФайла, ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ТекстовоеПредставлениеРазмераФайла(СтрокаТаблицы.Размер));
...
Процедура ТаблицаФайловПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ka='Удалить выделенные файлы изображений?';ru='Удалить выделенные файлы изображений?';en='Delete selected image files?'");
