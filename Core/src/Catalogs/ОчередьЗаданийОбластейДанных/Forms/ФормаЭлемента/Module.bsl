...
Процедура ПредставлениеРасписанияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Шаблон) Тогда
		ПоказатьПредупреждение(, НСтр("ka='შაბლონების საფუძველზე დავალებებისთვის, განრიგის მიცემა ხდება შაბლონში.';ru='Для заданий на основе шаблонов, расписание задается в шаблоне.';en='For jobs based on templates, schedule specified in the template.'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьРасписание(НовоеРасписание, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='გადაგეგმვა';ru='Перепланирование';en='rescheduling'"), , НСтр("ru='Новое расписание будет учтено при"
"следующем выполнении задания';ka='ახალი განრიგი მხედველობაში იქნება მიღებული დავალების მომდევნო შესრულებისას'"));
...
Процедура УстановитьПредставлениеРасписания(Знач Форма)
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.Объект.Шаблон) Тогда
		Форма.ПредставлениеРасписания = НСтр("ka='<მიცემა ხდება შაბლონში>';ru='<Задается в шаблоне>';en='<Set the template>'");
	Иначе
		Форма.ПредставлениеРасписания = НСтр("ka='<Не задано>';ru='<Не задано>';en='<Not specified>'");
	КонецЕсли;
