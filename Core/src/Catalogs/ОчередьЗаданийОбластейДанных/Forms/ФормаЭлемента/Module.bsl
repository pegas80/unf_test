...
Процедура ПредставлениеРасписанияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Шаблон) Тогда
		ПоказатьПредупреждение(, НСтр("ka='შაბლონების საფუძველზე დავალებებისთვის, განრიგის მიცემა ხდება შაბლონში.';ru='Для заданий на основе шаблонов, расписание задается в шаблоне.'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьРасписание(НовоеРасписание, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='გადაგეგმვა';ru='Перепланирование'"), , НСтр("ru='Новое расписание будет учтено при"
...
Процедура УстановитьПредставлениеРасписания(Знач Форма)
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.Объект.Шаблон) Тогда
		Форма.ПредставлениеРасписания = НСтр("ka='<მიცემა ხდება შაბლონში>';ru='<Задается в шаблоне>'");
	Иначе
		Форма.ПредставлениеРасписания = НСтр("ka='<Не задано>';ru='<Не задано>'");
	КонецЕсли;
