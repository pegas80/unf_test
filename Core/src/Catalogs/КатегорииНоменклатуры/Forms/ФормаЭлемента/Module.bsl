...
Процедура ДобавитьДопРеквизит(ВладелецНабораЭлементов)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ka='თვისებების მართვის დაწყებამდე, აუცილებელია ობიექტის ჩაწერა. ჩავწეროთ?';ru='Перед началом управления свойствами необходимо записать объект. Записать?';en='Before starting control of the properties you want to record the object. Record?'");
		Ответ = Неопределено;
