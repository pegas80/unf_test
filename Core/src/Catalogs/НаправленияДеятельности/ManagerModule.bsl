...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Номенклатура") Тогда
		...
		Если НЕ Параметры.Номенклатура.СчетУчетаЗатрат.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
			...
			ТекстСообщения = НСтр("ka='ამ ხარჯისთვის არ შეიძლება საქმიანობის მიმართულების არჩევა!';ru='Для этого расхода нельзя выбрать направление деятельности!'");
	...
	Если Параметры.Свойство("СчетЗатрат") Тогда
		...
		Если ЗначениеЗаполнено(Параметры.СчетЗатрат) Тогда
			...
			Если Параметры.СчетЗатрат.ТипСчета = Перечисления.ТипыСчетов.ПрочиеРасходы Тогда
				...
				ТекстСообщения = НСтр("ka='ისეთი ტიპის ანგარიშისთვის, როგორიც არის ""სხვა დანახარჯები"" საქმიანობის მიმართულება არ მიეთითება!';ru='Для счета типа ""Прочие расходы"" направление деятельности не указывается!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(, ТекстСообщения);
			...
			ИначеЕсли Параметры.СчетЗатрат.ТипСчета = Перечисления.ТипыСчетов.НезавершенноеПроизводство
				...
				ТекстСообщения = НСтр("ka='ისეთი ტიპის ანგარიშისთვის, როგორიც არის ""დაუსრულებელი წარმოება"" ან ""ირიბი დანახარჯები"" საქმიანობის მიმართულება არ მიეთითება!';ru='Для счета типа ""Незавершенное производство"" или ""Косвенные затраты"" направление деятельности не указывается!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(, ТекстСообщения);
		...
		Иначе
			...
			ТекстСообщения = НСтр("ka='არ არის არჩეული ანაგარიში!';ru='Не выбран счет!'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(, ТекстСообщения);
