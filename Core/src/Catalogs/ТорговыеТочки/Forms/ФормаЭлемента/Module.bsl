...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.КИ_0.ПодчиненныеЭлементы.ПредставлениеКИ_0.Заголовок = НСтр("ka='Адрес торговой точки';ru='Адрес торговой точки';en='Address outlet'");
...
Процедура ИзменитьКодВидаТорговойДеятельности(Команда)
	...
	СписокКодов.ПоказатьВыборЭлемента(Оповещние,НСтр("ka='Выберите код вида торговой деятельности';ru='Выберите код вида торговой деятельности';en='Select the code of the type of trading activity'"));
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	Если Объект.ЛьготаПрименяется Тогда
		...
		ИсчисленнаяСуммаСоЛьготой = Новый ФорматированнаяСтрока(
			НСтр("ru='Исчисленная сумма : ';ka='Исчисленная сумма : '"),
			Новый ФорматированнаяСтрока(Строка(Объект.ИсчисленнаяСумма)+ " р.", Новый Шрифт("",,,,,Истина,,)),
			НСтр("ka=' Применяется льгота. Сумма платежей 0 р.';ru=' Применяется льгота. Сумма платежей 0 р.'"));
...
Процедура РассчитатьТорговыйСбор()
	...
	Если Объект.КодОбъектаОсуществленияТорговли = "08" Тогда
		...
		Объект.РасшифровкаРасчета = НСтр("ka='Ставка за объект составляет';ru='Ставка за объект составляет';en='The rate for the facility is'") + Объект.ИсчисленнаяСумма + " р. ";
		
	ИначеЕсли Объект.КодОбъектаОсуществленияТорговли = "07" Тогда
		
		Объект.ИсчисленнаяСумма = ДанныеТарифа.СтавкаДо50квм;
		
		Объект.РасшифровкаРасчета = НСтр("ka='Исчисленная сумма при развозной и разносной торговле составляет';ru='Исчисленная сумма при развозной и разносной торговле составляет';en='When calculating the amount of beverages and tobacco trade is'") + Объект.ИсчисленнаяСумма + " р. ";
	...
	Иначе
		...
		Если Объект.ПлощадьТорговогоЗала > 50 И ТребуетсяПлощадьТороговогоЗала(Объект.КодОбъектаОсуществленияТорговли) Тогда
			...
			Объект.РасшифровкаРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Исчисленная сумма %1 р. состоит из %2 р. (ставка за объект) + %3 кв.м. (превышение 50 кв. м. площади) * %4 р. (ставка за превышение)';ru='Исчисленная сумма %1 р. состоит из %2 р. (ставка за объект) + %3 кв.м. (превышение 50 кв. м. площади) * %4 р. (ставка за превышение)';en='Calculation of amounts of 1% p. It consists of 2% p. (Rate per object) + 3% sq.m. (In excess of 50 sq. M. Area) * 4% p. (The rate for excess)'"),
		...
		Иначе 
			Объект.РасшифровкаРасчета = НСтр("ru='Ставка за объект составляет ';ka='Ставка за объект составляет '") + Объект.СтавкаЗаОбъект + " р. ";
		КонецЕсли;
	...
	Если Объект.ЛьготаПрименяется Тогда
		...
		Объект.РасшифровкаРасчета = Объект.РасшифровкаРасчета + НСтр("ka=' Применяется льгота. Итоговая ставка равна нулю.';ru=' Применяется льгота. Итоговая ставка равна нулю.'");
	Иначе
