...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.КИ_0.ПодчиненныеЭлементы.ПредставлениеКИ_0.Заголовок = НСтр("ka='Адрес торговой точки';ru='Адрес торговой точки'");
...
Процедура ИзменитьКодВидаТорговойДеятельности(Команда)
	...
	СписокКодов.ПоказатьВыборЭлемента(Оповещние,НСтр("ka='Выберите код вида торговой деятельности';ru='Выберите код вида торговой деятельности'"));
...
Процедура РассчитатьТорговыйСбор()
	...
	Если Объект.КодОбъектаОсуществленияТорговли = "08" Тогда
		...
		Объект.РасшифровкаРасчета = НСтр("ka='Ставка за объект составляет';ru='Ставка за объект составляет'") + Объект.ИсчисленнаяСумма + " р. ";
		
	ИначеЕсли Объект.КодОбъектаОсуществленияТорговли = "07" Тогда
		
		Объект.ИсчисленнаяСумма = ДанныеТарифа.СтавкаДо50квм;
		
		Объект.РасшифровкаРасчета = НСтр("ka='Исчисленная сумма при развозной и разносной торговле составляет';ru='Исчисленная сумма при развозной и разносной торговле составляет'") + Объект.ИсчисленнаяСумма + " р. ";
	...
	Иначе
		...
		Если Объект.ПлощадьТорговогоЗала > 50 И ТребуетсяПлощадьТороговогоЗала(Объект.КодОбъектаОсуществленияТорговли) Тогда
			...
			Объект.РасшифровкаРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Исчисленная сумма %1 р. состоит из %2 р. (ставка за объект) + %3 кв.м. (превышение 50 кв. м. площади) * %4 р. (ставка за превышение)';ru='Исчисленная сумма %1 р. состоит из %2 р. (ставка за объект) + %3 кв.м. (превышение 50 кв. м. площади) * %4 р. (ставка за превышение)'"),
