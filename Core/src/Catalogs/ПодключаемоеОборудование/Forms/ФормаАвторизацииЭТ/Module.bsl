...
Процедура ВыполнитьОперацию(Команда)
	...
	Если Сумма = 0 Тогда
		Ошибки = НСтр("ka='გადახდა არ შეიძლება განხორციელდეს ნულოვან თანხაზე.';ru='Платеж не может быть осуществлен на нулевую сумму.'");
	КонецЕсли;
	
	Если ПоказыватьНомерКарты И Не ЗначениеЗаполнено(НомерКарты) Тогда
		Ошибки = Ошибки + ?(ПустаяСтрока(Ошибки),"",Символы.ПС) + НСтр("ka='გადახდა არ შეიძლება განხორციელდეს ბარათის ნომრის გარეშე';ru='Платеж не может быть осуществлен без номера карты.'");
	КонецЕсли;
