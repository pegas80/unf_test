...
Процедура ВыполнитьОперацию(Команда)
	...
	Если Сумма = 0 Тогда
		Ошибки = НСтр("ka='გადახდა არ შეიძლება განხორციელდეს ნულოვან თანხაზე.';ru='Платеж не может быть осуществлен на нулевую сумму.';en='Payment can not be carried out on the zero sum.'");
	КонецЕсли;
	
	Если ПоказыватьНомерКарты И Не ЗначениеЗаполнено(НомерКарты) Тогда
		Ошибки = Ошибки + ?(ПустаяСтрока(Ошибки),"",Символы.ПС) + НСтр("ka='გადახდა არ შეიძლება განხორციელდეს ბარათის ნომრის გარეშე';ru='Платеж не может быть осуществлен без номера карты.';en='Payment can not be implemented without the card number.'");
	КонецЕсли;
...
"Значение будет изменено на максимально возможное.';ka='ბარათის მიხედვით გადახდის თანხა აღემატება აუცილებელ უნაღდო გადახდას. მნიშვნელობა შეიცვლება მაქსიმალურად შესაძლებელზე.'"));
