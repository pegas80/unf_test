...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТипОперации") Тогда
		...
		Если ТипОперации = 1 Тогда
			ЭтаФорма.Заголовок = НСтр("ka='Регистрация фискального накопителя';ru='Регистрация фискального накопителя'")
		ИначеЕсли ТипОперации = 3 Тогда
			ЭтаФорма.Заголовок = НСтр("ka='Закрытие фискального накопителя';ru='Закрытие фискального накопителя'");
		Иначе
			ЭтаФорма.Заголовок = НСтр("ka='Изменение параметров регистрации фискального накопителя';ru='Изменение параметров регистрации фискального накопителя'");
		КонецЕсли;
...
Процедура ПолучениеПараметровФискальногоУстройства_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если ТипОперации = 1 Тогда // Регистрация ФН
		...
		Если РезультатВыполнения.Результат И ПараметрыККТ.ПризнакФискализации Тогда
			...
			ТекстСообщения = НСтр("ka='Фискальный накопитель уже зарегистрирован. Воспользуйтесь функцией изменение параметров регистрации фискального накопителя.';ru='Фискальный накопитель уже зарегистрирован. Воспользуйтесь функцией изменение параметров регистрации фискального накопителя.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;                                                            
...
Процедура ПолучениеТекущегоСостоянияФискальногоУстройства_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если СтатусСмены <> 1 Тогда
			Элементы.ОперацияПродолжить.Видимость = Ложь;
			ТекстСообщения = НСтр("ka='Операция доступна только при закрытой смене на фискальном устройстве. Закройте смену и повторите операцию.';ru='Операция доступна только при закрытой смене на фискальном устройстве. Закройте смену и повторите операцию.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли; 
	КонецЕсли; 
	...
	Если Элементы.ГруппаРеквизитыОрганизации.Видимость Тогда
		КодыСистемыУстановлен = КодыСистемыНалогообложения0 Или КодыСистемыНалогообложения1 Или КодыСистемыНалогообложения2 
			...
			Если Не КодыСистемыУстановлен Тогда
				ПоказатьПредупреждение(, НСтр("ka='Не указан не один код налогообложения.';ru='Не указан не один код налогообложения.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
