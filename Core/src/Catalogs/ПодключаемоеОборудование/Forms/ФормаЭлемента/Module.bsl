...
Процедура ОбработатьВыборОбработчика(ВыбранныйОбработчик, СтандартнаяОбработка = Истина)
	...
	Объект.Наименование = "'" + Строка(ВыбранныйОбработчик) + "'"
						+ ?(ПустаяСтрока(Строка(Объект.РабочееМесто)),
							"",
							" " + НСтр("ka='ზე';ru='на';en='on'") + " " + Строка(Объект.РабочееМесто));
...
Процедура ОбновитьПараметрыРегистрацииККТ();
	...
	Для Каждого ПараметрРегистрации Из Объект.ПараметрыРегистрации Цикл
		...
		Если НаименованиеПараметра = "РегистрационныйНомерККТ" Тогда
			НаименованиеПараметра = НСтр("ka='Регистрационный номер ККТ';ru='Регистрационный номер ККТ';en='Registration CCV number'")
		ИначеЕсли НаименованиеПараметра = "ОрганизацияНазвание" Тогда
			НаименованиеПараметра = НСтр("ka='ორგანიზაციის დასახელება';ru='Название организации';en='Name of the organization'")
		ИначеЕсли НаименованиеПараметра = "ОрганизацияИНН" Тогда
			НаименованиеПараметра = НСтр("ka='ორგანიზაციის ИНН';ru='ИНН организации';en='INN organization'")
		ИначеЕсли НаименованиеПараметра = "АдресУстановкиККТ" Тогда
			НаименованиеПараметра = НСтр("ka='Адрес установки ККТ';ru='Адрес установки ККТ';en='Address setting CCPs'")
		ИначеЕсли НаименованиеПараметра = "КодыСистемыНалогообложения" Тогда
			НаименованиеПараметра = НСтр("ka='საგადასახადო სისტემები';ru='Системы налогообложения';en='taxation system'");
		...
		ИначеЕсли НаименованиеПараметра = "ПризнакАвтономногоРежима" Тогда
			НаименованиеПараметра = НСтр("ka='ავტონომური რეჟიმი';ru='Автономный режим';en='Offline mode'")
		ИначеЕсли НаименованиеПараметра = "ПризнакАвтоматическогоРежима" Тогда
			НаименованиеПараметра = НСтр("ka='ავტომატური რეჟიმი';ru='Автоматический режим';en='Auto mode'")
		ИначеЕсли НаименованиеПараметра = "НомерАвтоматаДляАвтоматическогоРежима" Тогда
			НаименованиеПараметра = НСтр("ka='Номер автомата для автоматического режима';ru='Номер автомата для автоматического режима';en='machine number for automatic mode'")
		ИначеЕсли НаименованиеПараметра = "ПризнакШифрованиеДанных" Тогда
			НаименованиеПараметра = НСтр("ka='მონაცემების დაშიფვრა';ru='Шифрование данных';en='data encryption'")
		ИначеЕсли НаименованиеПараметра = "ПризнакРасчетовЗаУслуги" Тогда
			НаименованиеПараметра = НСтр("ka='Расчет за услуги';ru='Расчет за услуги';en='Payment for Services'")
		ИначеЕсли НаименованиеПараметра = "ПризнакФормированияТолькоБСО" Тогда
			НаименованиеПараметра = НСтр("ka='Формирования только БСО';ru='Формирования только БСО';en='Only the formation of the BSO'")
		ИначеЕсли НаименованиеПараметра = "ПризнакРасчетовТолькоВИнтернет" Тогда
			НаименованиеПараметра = НСтр("ka='Расчет только в интернет';ru='Расчет только в интернет';en='Calculation only in the Internet'")
		ИначеЕсли НаименованиеПараметра = "ОрганизацияОФДИНН" Тогда
			НаименованиеПараметра = НСтр("ka='ОФД ИНН';ru='ОФД ИНН';en='CRF INN'")
		ИначеЕсли НаименованиеПараметра = "ОрганизацияОФДНазвание" Тогда
			НаименованиеПараметра = НСтр("ka='ОФД დასახელება';ru='Наименование ОФД';en='name of the CRF'")
		ИначеЕсли НаименованиеПараметра = "ЗаводскойНомерККТ" Тогда
			НаименованиеПараметра = НСтр("ka='Заводской номер ККТ';ru='Заводской номер ККТ';en='The serial number of the CCP'")
		ИначеЕсли НаименованиеПараметра = "ПризнакФискализации" Тогда
			НаименованиеПараметра = НСтр("ka='ფისკალიზაციის ნიშანი';ru='Признак фискализации';en='Symptom fiscalization'")
		ИначеЕсли НаименованиеПараметра = "ЗаводскойНомерФН" Тогда
			НаименованиеПараметра = НСтр("ka='Заводской номер ФН';ru='Заводской номер ФН';en='Serial number FN'")
		ИначеЕсли НаименованиеПараметра = "НомерДокументаФискализации" Тогда
			НаименованиеПараметра = НСтр("ka='Номер документа фискализации';ru='Номер документа фискализации';en='Number fiscalization document'")
		ИначеЕсли НаименованиеПараметра = "ДатаВремяФискализации" Тогда
			НаименованиеПараметра = НСтр("ka='Дата и время фискализации';ru='Дата и время фискализации';en='Date and time of fiscalization'")
		ИначеЕсли НаименованиеПараметра = "ВерсияФФДККТ" Тогда
			НаименованиеПараметра = НСтр("ka='Версия ФФД ККТ';ru='Версия ФФД ККТ';en='Version FFD CCP'")
		ИначеЕсли НаименованиеПараметра = "ВерсияФФДФН" Тогда
			НаименованиеПараметра = НСтр("ka='Версия ФФ ДФН';ru='Версия ФФ ДФН';en='FF version DPN'")
		КонецЕсли;
...
Процедура ОперацияФискальногоНакопителя_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='ოპერაცია დასრულებულია';ru='Операция завершена.';en='The transaction is completed.'");
	Иначе
