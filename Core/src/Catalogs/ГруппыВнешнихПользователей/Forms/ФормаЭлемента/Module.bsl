...
Процедура СоставПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если СообщениеПользователю <> Неопределено Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ka='მომხმარებლების გადაადგილება';ru='Перемещение пользователей';en='Moving users'"), , СообщениеПользователю, БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ka='მომხმარებლების სახეობის არჩევა';ru='Выбор вида пользователей';en='The choice of the user'"), Ложь, Ложь, ОписаниеОповещения);
...
Функция ПараметрыРасширеннойФормыПодбора()
	...
	ЗаголовокФормыПодбора = НСтр("ka='გარე მომხმარებლების ჯგუფის მონაწილეების შერჩევა';ru='Подбор участников группы внешних пользователей';en='Selection of members of the group of external users'");
