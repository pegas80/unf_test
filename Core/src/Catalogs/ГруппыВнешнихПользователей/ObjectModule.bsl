...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Родитель",
			НСтр("ka='წინასწარ განსაზღვრული ჯგუფი ""ყველა გარე მომხმარებელი"" არ შეიძლება იყოს მშობელი.';ru='Предопределенная группа ""Все внешние пользователи"" не может быть родителем.'"),
			"");
	КонецЕсли;
	...
	Если Назначение.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Назначение",
			НСтр("ka='მომხმარებლების სახეობა არ არის მითითებული.';ru='Не указан вид пользователей.'"),
			"");
	КонецЕсли;
	...
	Для каждого ТекущаяСтрока Из Состав Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.ВнешнийПользователь) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ka='გარე მომხმარებელი არ არის არჩეული.';ru='Внешний пользователь не выбран.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("ka='გარე მომხმარებელი სტრიქონ %1-ში არჩეული არ არის.';ru='Внешний пользователь в строке %1 не выбран.'"));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"Объект.Состав[%1].ВнешнийПользователь",
				НСтр("ka='გარე მომხმარებელი მეორდება.';ru='Внешний пользователь повторяется.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("ka='გარე მომხმარებელი სტრიქონ %1-ში მეორდება.';ru='Внешний пользователь в строке %1 повторяется.'"));
		КонецЕсли;
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		...
		Если НЕ Родитель.Пустая() Тогда
			ВызватьИсключение
				НСтр("ka='წინასწარ განსაზღვრული ჯგუფ ""ყველა გარე მომხმარებელი""-ს გადაადგილება ვერ მოხდება.';ru='Предопределенная группа ""Все внешние пользователи"" не может быть перемещена.'");
		КонецЕсли;
		Если Состав.Количество() > 0 Тогда
			ВызватьИсключение
				НСтр("ka='წინასწარ განსაზღვრულ ჯგუფ ""ყველა გარე მომხმარებელი""-ში მონაწილეების დამატება აკრძალულია.';ru='Добавление участников в предопределенную группу ""Все внешние пользователи"" запрещено.'");
		КонецЕсли;
	Иначе
		Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			ВызватьИсключение
				НСтр("ka='წინასწარ განსაზღვრული ჯგუფ ""ყველა გარე მომხმარებელი""-სთვის ქვეჯგუფის დამატება შეუძლებელია.';ru='Невозможно добавить подгруппу к предопределенной группе ""Все внешние пользователи"".'");
...
"так как в число ее участников входят все пользователи.';ka='ქვეჯგუფის დამატება ჯგუფ ""%1""-ში შეუძლებელია, ვინაიდან მის შემადგენლობაში ყველა მომხმარებელი შედის.'"), Родитель);
		...
		Если ВсеОбъектыАвторизации И ЗначениеЗаполнено(Родитель) Тогда
			ВызватьИсключение
				НСтр("ka='ჯგუფის, რომლის შემადგენლობაში ყველა მომხმარებელი შედის, გადაადგილება შეუძლებელია.';ru='Невозможно переместить группу, в число участников которой входят все пользователи.'");
		КонецЕсли;
		...
		Если ВсеОбъектыАвторизации Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(НазначениеВсеВнешниеПользователи, МассивНазначения) Тогда
				ВызватьИсключение
					НСтр("ka='შეუძლებელია ჯგუფის შექმნა, რომელიც დანიშნულებით, წინასწარ განსაზღვრულ ჯგუფ ""ყველა გარე მომხმარებელი""-ს ემთხვევა.';ru='Невозможно создать группу, совпадающую по назначению с предопределенной группой ""Все внешние пользователи"".'");
			КонецЕсли;
