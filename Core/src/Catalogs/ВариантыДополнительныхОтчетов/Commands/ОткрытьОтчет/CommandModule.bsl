...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если ВыполняемаяКоманда.Контекстный Тогда
		Состояние(НСтр("ka='ანგარიშგების გახსნა შეიძლება მხოლოდ БД-ს ობიექტებიდან';ru='Отчет можно открыть только из объектов БД';en='The report can only be opened from the database objects'"));
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыполняемаяКоманда.Автор) И ВыполняемаяКоманда.ТолькоДляАвтора И НЕ ВыполняемаяКоманда.ЭтоПолноправныйПользователь И НЕ ВыполняемаяКоманда.ТекущийПользователь=ВыполняемаяКоманда.Автор Тогда
		Состояние(НСтр("ka='დაშვების უფლებების დარღვევა';ru='Нарушение прав доступа';en='Access Violation'"));
		Возврат;
	КонецЕсли; 
