...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Профиль) Тогда
		...
		Заголовок = НСтр("ka='დაშვების ჯგუფი';ru='Группы доступа';en='access groups'");
	...
	Список.Параметры.УстановитьЗначениеПараметра(
		"ОшибкаОбъектНеНайден",
		НСтр("ka='<ობიექტი ნაპოვნი არ იქნა>';ru='<Объект не найден>';en='<Object not found>'"));
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Заголовок = НСтр("ka='დაშვების ჯგუფების შერჩევა';ru='Подбор групп доступа';en='Selection of access groups'");
		Иначе
			Заголовок = НСтр("ka='დაშვების ჯგუფის არჩევა';ru='Выбор группы доступа';en='Select the access group'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	Если Значение = РодительПерсональныхГруппДоступа Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='ეს ჯგუფი არის მხოლოდ დაშვების პერსონალური ჯგუფებისთვის.';ru='Эта группа только для персональных групп доступа.';en='This group is only for personal access groups.'"));
	КонецЕсли;
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Родитель = РодительПерсональныхГруппДоступа Тогда
		...
		Если Группа Тогда
			ПоказатьПредупреждение(, НСтр("ka='ამ ჯგუფში არ გამოიყენება ქვეჯგუფები.';ru='В этой группе не используются подгруппы.';en='In this group, do not use sub-groups.'"));
...
"создаются только в форме ""Права доступа"".';ka='დაშვების პერსონალური ჯგუფები იქმნება მხოლოდ ფორმით ""დაშვების უფლებები"".'"));
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='დაშვების პერსონალური ჯგუფები არ გამოიყენება.';ru='Персональные группы доступа не используются.';en='Personal access group not used.'"));
		КонецЕсли;
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если Строка = РодительПерсональныхГруппДоступа Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='ეს საქაღალდე არის მხოლოდ დაშვების პერსონალურ ჯგუფებისთვის.';ru='Эта папка только для персональных групп доступа.';en='This folder is only for personal access groups.'"));
		
	ИначеЕсли ПараметрыПеретаскивания.Значение = РодительПерсональныхГруппДоступа Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='დაშვების პერსონალური ჯგუფების საქაღალდის გადატანა ვერ ხერხდება.';ru='Папка персональных групп доступа не переносится.';en='Personal folder access groups are not tolerated.'"));
	КонецЕсли;
