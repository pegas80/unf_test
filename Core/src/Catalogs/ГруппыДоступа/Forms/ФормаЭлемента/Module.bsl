	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если НЕ ДействияСПользователемСервиса.ИзменениеАдминистративногоДоступа Тогда
			ВызватьИсключение
				НСтр("ka='ადმინისტრატორების შემადგენლობის შესაცვლელად დაშვების უფლებები არასაკმარისია.';ru='Не достаточно прав доступа для изменения состава администраторов.'");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Участник) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				УточнитьСообщение(НСтр("ka='მომხმარებელი არ არის არჩეული.';ru='Пользователь не выбран.'"), Участник),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("ka='სტრიქონ %1-ში მომხმარებელი არ არის არჩეული.';ru='Пользователь в строке %1 не выбран.'"), Участник));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებელი ""%2"" მეორდება.';ru='Пользователь ""%2"" повторяется.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებელი ""%2"" მეორდება.';ru='Пользователь ""%2"" в строке %1 повторяется.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებელი ""%2"" მეორდება.';ru='Внешний пользователь ""%2"" повторяется.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებელი ""%2"" მეორდება.';ru='Внешний пользователь ""%2"" в строке %1 повторяется.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа пользователей ""%2"" повторяется.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа пользователей ""%2"" в строке %1 повторяется.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа внешних пользователей ""%2"" повторяется.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа внешних пользователей ""%2"" в строке %1 повторяется.'");
			КонецЕсли;
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებელი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებელი ""%2"" დაუშვებელია დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში.';ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებლების ჯგუფი ""%2"" წინასწარ განსაზღვრულ დაშვების ჯგუფ ადმინისტრატორებში დაუშვებელი არის.';ru='Группа пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებლების ჯგუფი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Группа пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებლების ჯგუფი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Группа внешних пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებლების ჯგუფი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.'");
			КонецЕсли;
		...
		Если НедопустимыеПользователи.Найти(ТекущаяСтрока.Пользователь) <> Неопределено Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებელი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Пользователь ""%2"" недопустим для указанного типа участников.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებელი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Пользователь ""%2"" в строке %1 недопустим для указанного типа участников.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებლების ჯგუფი ""%2"" მონაწილეების აღნიშნული ტიპისთვის დაუშვებელია.';ru='Группа пользователей ""%2"" недопустима для указанного типа участников.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებლების ჯგუფი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებელი ""%2"" დასაშვებია მონაწილეთა აღნიშნული ტიპისთვის.';ru='Внешний пользователь ""%2"" недопустим для указанного типа участников.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებელი ""%2"" დაუშვებელია მონაწილეთა აღნიშნული ტიპისთვის.';ru='Внешний пользователь ""%2"" в строке %1 недопустим для указанного типа участников.'");
			Иначе // Группа внешних пользователей.
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებლების ჯგუფი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Группа внешних пользователей ""%2"" недопустима для указанного типа участников.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებლების ჯგუფი ""%2"" მონაწილეთა მითითებული ტიპისთვის დაუშვებელია.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима для указанного типа участников.'");
			КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(ТекущийОбъект.Пользователь) Тогда
			...
			Заголовок
				...
				+ " "
				+ НСтр("ka='(დაშვების ჯგუფი)';ru='(Группа доступа)'");
...
Процедура ОбновитьНазначение()
	...
	Для Каждого Участник Из Объект.Профиль.Назначение Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Элементы.Пользователи.Подсказка = НСтр("ka='დაშვებული მონაწილეები:';ru='Допустимые участники:'") + " " + НазначениеПредставление;
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Если Не ИспользоватьВнешнихПользователей Тогда
			ПоказатьПредупреждение(, НСтр("ka='გარე მომხმარებლების შეყვანა პროგრამის გამართვებში გამორთულია.';ru='Ведение внешних пользователей отключено в настройках программы.'"));
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='გარე მომხმარებელბის ასარცევად არასაკმარისი უფლებებია';ru='Недостаточно прав для выбора внешних пользователей.'"));
		КонецЕсли;
	Иначе
...
Функция ПараметрыРасширеннойФормыПодбора()
	...
	ЗаголовокФормыПодбора = НСтр("ka='დაშვების ჯგუფის მონაწილეთა შერჩევა';ru='Подбор участников группы доступа'");
