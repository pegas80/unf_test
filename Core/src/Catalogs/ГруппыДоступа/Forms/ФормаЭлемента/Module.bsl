	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если НЕ ДействияСПользователемСервиса.ИзменениеАдминистративногоДоступа Тогда
			ВызватьИсключение
				НСтр("ka='ადმინისტრატორების შემადგენლობის შესაცვლელად დაშვების უფლებები არასაკმარისია.';ru='Не достаточно прав доступа для изменения состава администраторов.';en='Not enough access rights to change the composition of the administrators.'");
		КонецЕсли;
	КонецЕсли;
...
""
"Установить профиль в группе доступа (нет - открыть только для просмотра)?';ka='დაშვების ჯგუფ ადმინისტრატორებს უნდა ჰქონდეს პროფილი ადმინისტრატორი"
""
"დავაყენოთ პრფილი დაშვების ჯგუფში (არა - მხოლოდ სანახავად გახსნა)?'"),
	...
	Иначе
		...
		ИначеЕсли НЕ ТолькоПросмотр И ИзменилсяСоставВидовДоступаПриЧтении Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ka='დაშვების ამ ჯგუფის პროფილის დაშვების ტიპების შემადგენლობა შეიცვალა."
""
"დაშვების ჯგუფში განვაახლოთ დაშვების ტიპები (არა - მხოლოდ სანახავად გახსნა)?';ru='Изменился состав видов доступа профиля этой группы доступа."
""
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ПредупредитьЧтоПрофильПомеченНаУдаление") Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ka='დაშვების ჯგუფი მონაწილეთა უფლებებზე გავლენას არ ახდენს, ვინაიდან მისი პროფილი წასაშლელად არის მონიშნული.';ru='Группа доступа не влияет на права участников"
	...
	Для Каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Участник) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				УточнитьСообщение(НСтр("ka='მომხმარებელი არ არის არჩეული.';ru='Пользователь не выбран.';en='This user has not selected.'"), Участник),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("ka='სტრიქონ %1-ში მომხმარებელი არ არის არჩეული.';ru='Пользователь в строке %1 не выбран.';en='User% 1 line is not selected.'"), Участник));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებელი ""%2"" მეორდება.';ru='Пользователь ""%2"" повторяется.';en='User ""%2"" is repeated.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებელი ""%2"" მეორდება.';ru='Пользователь ""%2"" в строке %1 повторяется.';en='User ""%2"" at line% 1 is repeated.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებელი ""%2"" მეორდება.';ru='Внешний пользователь ""%2"" повторяется.';en='External User ""%2"" is repeated.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებელი ""%2"" მეორდება.';ru='Внешний пользователь ""%2"" в строке %1 повторяется.';en='External User ""%2"" at line 1 is repeated%.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа пользователей ""%2"" повторяется.';en='User Group ""%2"" is repeated.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа пользователей ""%2"" в строке %1 повторяется.';en='User Group ""%2"" at line% 1 is repeated.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа внешних пользователей ""%2"" повторяется.';en='External User Group ""%2"" is repeated.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებლების ჯგუფი ""%2"" მეორდება.';ru='Группа внешних пользователей ""%2"" в строке %1 повторяется.';en='External User Group ""%2"" at line% 1 is repeated.'");
			КонецЕсли;
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებელი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';en='External User ""%2"" is not valid in a predefined access to the Administrators group.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებელი ""%2"" დაუშვებელია დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში.';ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.';en='External User ""%2"" at line 1% in a predetermined invalid access Administrators group.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებლების ჯგუფი ""%2"" წინასწარ განსაზღვრულ დაშვების ჯგუფ ადმინისტრატორებში დაუშვებელი არის.';ru='Группа пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';en='User Group ""%2"" is not allowed in the predefined Administrators access group.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებლების ჯგუფი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Группа пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';en='User Group ""%2"" at line% 1 is not allowed to access the predefined Administrators group.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებლების ჯგუფი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Группа внешних пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';en='External User Group ""%2"" is not allowed in the predefined Administrators access group.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებლების ჯგუფი ""%2"" დაშვების წინასწარ განსაზღვრულ ჯგუფ ადმინისტრატორებში დაუშვებელია.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';en='External User Group ""%2"" at line% 1 is not allowed to access the predefined Administrators group.'");
			КонецЕсли;
		...
		Если НедопустимыеПользователи.Найти(ТекущаяСтрока.Пользователь) <> Неопределено Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებელი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Пользователь ""%2"" недопустим для указанного типа участников.';en='User ""%2"" is not valid for the specified type of participants.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებელი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';en='User ""%2"" at line% 1 is not valid for the specified type of participants.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='მომხმარებლების ჯგუფი ""%2"" მონაწილეების აღნიშნული ტიპისთვის დაუშვებელია.';ru='Группа пользователей ""%2"" недопустима для указанного типа участников.';en=""A user group '% 2' is not valid for the specified type of participants.""");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში მომხმარებლების ჯგუფი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';en='User Group ""%2"" at line% 1 is not valid for the specified type of participants.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებელი ""%2"" დასაშვებია მონაწილეთა აღნიშნული ტიპისთვის.';ru='Внешний пользователь ""%2"" недопустим для указанного типа участников.';en='External User ""%2"" is not valid for the specified type of participants.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებელი ""%2"" დაუშვებელია მონაწილეთა აღნიშნული ტიპისთვის.';ru='Внешний пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';en='External User ""%2"" at line 1% valid for this type of participants.'");
			Иначе // Группа внешних пользователей.
				ТекстДляОднойОшибки      = НСтр("ka='გარე მომხმარებლების ჯგუფი ""%2"" მონაწილეთა აღნიშნული ტიპისთვის დაუშვებელია.';ru='Группа внешних пользователей ""%2"" недопустима для указанного типа участников.';en='The group of external users ""%2"" is not valid for the specified type of participants.'");
				ТекстДляНесколькихОшибок = НСтр("ka='სტრიქონ %1-ში გარე მომხმარებლების ჯგუფი ""%2"" მონაწილეთა მითითებული ტიპისთვის დაუშვებელია.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';en='External User Group ""%2"" at line% 1 is not valid for the specified type of participants.'");
			КонецЕсли;
...
"Их нельзя удалить в этом списке.';ka='ჯგუფების მომხმარებლები აისახება ცნობისათვის, რომ ისინი იღებენ მომხმარებელთა ჯგუფების დაშვებას. მათი წაშლა ამ სიაში არ შეიძლება.'"));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ka='ჯგუფების გარე მომხმარებლების ასახვა ხდება ცნობისათვის, რომ ისინი იღებენ გარე მომხმარებელთა ჯგუფების დაშვებას. მათი წაშლა ამ სიაში არ შეიძლება.';ru='Внешние пользователи групп отображаются для сведения,"
	...
	Иначе
		Если ЗначениеЗаполнено(ТекущийОбъект.Пользователь) Тогда
			...
			Заголовок
				...
				+ " "
				+ НСтр("ka='(დაშვების ჯგუფი)';ru='(Группа доступа)';en='(Access Group)'");
...
Процедура ОбновитьНазначение()
	...
	Для Каждого Участник Из Объект.Профиль.Назначение Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Элементы.Пользователи.Подсказка = НСтр("ka='დაშვებული მონაწილეები:';ru='Допустимые участники:';en='Permitted participants:'") + " " + НазначениеПредставление;
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Если Не ИспользоватьВнешнихПользователей Тогда
			ПоказатьПредупреждение(, НСтр("ka='გარე მომხმარებლების შეყვანა პროგრამის გამართვებში გამორთულია.';ru='Ведение внешних пользователей отключено в настройках программы.';en='Maintaining external users is disabled in the settings.'"));
...
Функция ПараметрыРасширеннойФормыПодбора()
	...
	ЗаголовокФормыПодбора = НСтр("ka='დაშვების ჯგუფის მონაწილეთა შერჩევა';ru='Подбор участников группы доступа';en='Selection of participants in the group access'");
