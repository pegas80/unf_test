...
Процедура ПередЗаписью(Отказ)
	...
	Если Не Результат.Пустой() Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შესაბამისობა ""%1,%2,%3 - %4"" უკვე არის ცნობარში. ჩაწერა გაუქმებულია.';ru='Соответствие ""%1,%2,%3 - %4"" уже присутствует в справочнике. Запись отменена.';en='Value ""1%, 2%, 3% - 4%"" is already present in the directory. Writing canceled.'"),
			Владелец, Артикул, Идентификатор, Номенклатура
		);
