...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НеУказаныДоступныеСпособыДоставки() Тогда
		...
		НСтр("ka='Укажите доступные способы доставки.';ru='Укажите доступные способы доставки.';en='Specify the available delivery methods.'"),,
		"ПунктВыдачи");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ФормулаКорректна(ИмяРеквизита)
	...
	Для каждого Стр Из ТаблицаУсловий Цикл
		Если Стр.Количество>1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Дублирование условий';ru='Дублирование условий';en='Duplicating conditions'"));
			Возврат Ложь;
		КонецЕсли; 
	КонецЦикла; 
...
Процедура СоздатьЭлементыУсловий(ОсноваИмени, ИмяГруппы)
	...
	Для каждого СтрокаТабличнойЧасти Из Условия Цикл
		...
		ЭлементПараметр.ПодсказкаВвода = НСтр("ka='Без условий';ru='Без условий';en='without conditions'");
		...
		ЭлементВидСравнения.ПодсказкаВвода = НСтр("ka='-';ru='-'");
		...
		ЭлементФормула.ПодсказкаВвода = НСтр("ka='ფორმულა';ru='Формула';en='Formula'");
...
Процедура ОбновитьЭлементыСтрокиУсловий(ИдентификаторСтроки, ИзмененныйЭлемент = "", ОсноваИмени)
	...
	Иначе
		...
		Если ИзмененныйЭлемент="Параметр" ИЛИ ПустаяСтрока(ИзмененныйЭлемент) Тогда
			...
			КонецЕсли;
			Если ЭтоЧисло Тогда
				ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.Содержит, НСтр("ka='В пределах';ru='В пределах';en='Within'"));
				ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.Равно);
			...
			Если ЭтоДата Тогда
				Если СтрокаТабличнойЧасти.Тип.КвалификаторыДаты.ЧастиДаты=ЧастиДаты.Время Тогда
					ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.Содержит, НСтр("ka='В промежутке';ru='В промежутке';en='In the interim'"));
				Иначе
					ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.Содержит, НСтр("ka='პერიოდის მანძილზე';ru='За период';en='During the period'"));
				КонецЕсли; 
				ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.Больше);
				ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
				ДобавитьВидСравнения(ЭлементВидСравнения, ВидСравненияКомпоновкиДанных.Меньше);
