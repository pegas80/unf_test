...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() И Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='პროგრამაში გამორთულია აღრიცხვა რამდენიმე ორგანიზაციის მიხედვით';ru='В программе отключен учет по нескольким организациям.'"));
		Отказ = Истина;
	КонецЕсли;
