...
Функция НеЗаполненЮрАдрес()
	...
	Для Каждого Элемент Из ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если Элемент.Вид = Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации Тогда
			...
			Возврат ЭтотОбъект[Элемент.ИмяРеквизита] = НСтр("ka='შევსება';ru='Заполнить'");
...
Процедура ДобавитьИзображение(ИмяРеквизитаСКартинкой)
	...
	НСтр("ka='გამოსახულების ასარჩევად საჭიროა ობიექტის ჩაწერა. ჩავწეროთ?';ru='Для выбора изображения необходимо записать объект. Записать?'"),
...
Функция НомерСчетаУказанНекорректно()
	...
	НСтр("ka='საჭირო არის ზუსტად 20 ციფრის მითითება.';ru='Требуется указать строго 20 цифр.'"),,
	"НомерСчета");
