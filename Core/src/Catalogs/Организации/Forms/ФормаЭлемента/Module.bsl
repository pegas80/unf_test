...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		ТекстОшибки = НСтр("ka='ახალი ორგანიზაციის შექმნა აკრძალულია როდესაც გამორთულია აღრიცხვა ""რამდენიმე ორგანიზაციის მიხედვით აღრიცხვა""-ს პარამეტრის გამართვა.';ru='Запрещено создавать новую организацию"
"при выключенной настройке параметра учета ""Учет по нескольким организациям"".'");
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаЭлемента",
		Неопределено,
		НСтр("ka='Новости: Организации';ru='Новости: Организации';en='News: Organizations'"),
		Ложь,
	...
	////-------------------------- // Россофт // --->
	УстановитьВидимостьЭлементовНалоговойСлужбы();
	ЭлементОтбора = ПользователиИнтеграцийНалоговойСлужбы.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	...
	Элементы.НалогообложениеНДС.Видимость = НЕ ПолучитьФункциональнуюОпцию("ФункциональнаяОпцияИспользоватьИсториюНалогообложенияОрганизации");
КонецПроцедуры
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "ОбщаяФорма.ПрисоединенныеФайлы"
		...
		ИначеЕсли РаботаСПечатью Тогда
			
			Объект.ФайлПечати = ВыбранноеЗначение;
			ДвоичныеДанныеКартинки = УправлениеНебольшойФирмойСервер.СсылкаНаДвоичныеДанныеФайла(Объект.ФайлПечати, УникальныйИдентификатор);
			Если ДвоичныеДанныеКартинки <> Неопределено Тогда
				АдресПечати = ДвоичныеДанныеКартинки;
			КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ПрисоединенныйФайл" Тогда
		...
		ИначеЕсли РаботаСПечатью Тогда
			...
			Объект.ФайлПечати = ?(ТипЗнч(Источник) = Тип("Массив"), Источник[0], Источник);
			ДвоичныеДанныеКартинки = УправлениеНебольшойФирмойСервер.СсылкаНаДвоичныеДанныеФайла(Объект.ФайлПечати, УникальныйИдентификатор);
			Если ДвоичныеДанныеКартинки <> Неопределено Тогда 
				АдресПечати = ДвоичныеДанныеКартинки;
			КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.ФайлПечати) Тогда
		ДвоичныеДанныеКартинки = УправлениеНебольшойФирмойСервер.СсылкаНаДвоичныеДанныеФайла(ТекущийОбъект.ФайлПечати, УникальныйИдентификатор);
		Если ДвоичныеДанныеКартинки <> Неопределено Тогда
			АдресПечати = ДвоичныеДанныеКартинки;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		Если Не ЗначениеЗаполнено(ФИОФизическихЛиц.Фамилия) 
			...
			ТекстСообщения = НСтр("ka='გვარი და სახელი არ არის შევსებული';ru='Не заполнено ФИО';en='Not filled Name'");
	...
	Если Не ПустаяСтрока(ОсновнойСчет_Номер) И Не ЗначениеЗаполнено(ОсновнойСчет_Банк) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "Заполнение", НСтр("ka='ბანკი';ru='Банк';en='Bank'"));
	...
	Если ЗначениеЗаполнено(ОсновнойСчет_Банк) И ПустаяСтрока(ОсновнойСчет_Номер) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "Заполнение", НСтр("ka='ანგარიშის ნომერი';ru='Номер счета';en='Account number'"));
...
"Записать и открыть настройки?';ka='Для просмотра настроек факсимиле необходимо записать организацию."
...
Процедура ПрефиксПриИзменении(Элемент)
	...
	Если СтрНайти(Объект.Префикс, "-") > 0 Тогда
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ორგანიზაციის პრეფიქსში არ შეიძლება, რომ გამოყენებული იქნას სიმბოლო ""-"".';ru='Нельзя в префиксе организации использовать символ ""-"".';en='Can not be used in the prefix of the organization ""-"" symbol.'"));
...
Процедура КодОКОПФОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтрЗаканчиваетсяНа(ВыбранноеЗначение, "0000") Тогда
		СтандартнаяОбработка = Ложь;
		ТекстСообщения = НСтр("ka='აირჩიეთ საზოგადოებრივ-სამართლებრივი ფორმების კლასიფიკატორის კოდი, რომელიც კოდების ჯგუფს არ წარმოადგენს';ru='Выберите код ОКОПФ, не являющийся группой кодов';en='Select OKOPF code, not a group of codes'");
...
Процедура ИсторияСтраховыхВзносов(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ჩაწერა';ru='Записать';en='record'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
"Записать?';ka='სადაზღვევო შენატანების ისტორიაზე გადასვლა შესაძლებელია მხოლოდ ჩაწერის შემდეგ."
"ჩავწეროთ?'"),
			СписокКнопок);
		Возврат;
	КонецЕсли;
...
"Записать?';ka='დაბეგვრის ისტორიაზე გადასვლა შესაძლებელია მხოლოდ ჩაწერის შემდეგ."
"ჩავწეროთ?'"),
			СписокКнопок);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьИзображениеПечати(Команда)
	...
	ДобавитьИзображениеПечатиФрагмент();
...
Процедура ИзменитьИзображениеПечати(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ФайлПечати) Тогда
		
		ПрисоединенныеФайлыКлиент.ОткрытьФормуПрисоединенногоФайла(Объект.ФайлПечати);
		
	Иначе
		
		ТекстСообщения = НСтр("ka='რედაქტირებისთვის გამოსახულება არ არის';ru='Отсутстует изображение для редактирования';en='Otsutst image editing'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "АдресПечати");
...
Процедура ОчиститьИзображениеПечати(Команда)
	...
	Объект.ФайлПечати = Неопределено;
	АдресПечати = "";
...
&НаКлиенте
Процедура ПечатьИзПрисоединенныхФайлов(Команда)
...
Процедура ЗаполнитьРеквизитыНалоговогоОрганаПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодНалоговогоОрганаПолучателя) Тогда
		ПоказатьПредупреждение(, НСтр("ka='ველი ""ინსპექციის კოდი"" არ არის შევსებული';ru='Поле ""Код инспекции"" не заполнено';en='The ""code inspection"" not filled'"));
...
Процедура ЗаполнитьРеквизитыПФРПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодОрганаПФР) Тогда
		ПоказатьПредупреждение(, НСтр("ka='ველი ""საპენსიო ორგანოს კოდი"" არ არის შევსებული';ru='Поле ""Код органа ПФР"" не заполнено';en='The ""Code of FIU authority"" not filled'"));
...
Процедура ЗаполнитьРеквизитыФССПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодОрганаФСС) Тогда
		ПоказатьПредупреждение(, НСтр("ka='ველი ""სდფ-ის ორგანოს კოდი"" არ არის შევსებული';ru='Поле ""Код органа ФСС"" не заполнено';en='The ""body of the FSS Code"" not filled'"));
...
Процедура УправлениеФормой(Форма)
	...
	Если Объект.ЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо") Тогда
		...
		Элементы.ИНН.ПодсказкаВвода			= НСтр("ka='13 ციფრი';ru='13 цифр';en='13 digits'");
		Элементы.ИНН.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
		Элементы.ОГРН.Заголовок				= НСтр("ka='ძსსნ';ru='ОГРН';en='BIN'");
		Элементы.ОГРН.Подсказка				= НСтр("ka='იურიდიული პირის ძირითადი სახელმწიფო სარეგისტრაციო ნომერი';ru='Основной государственный регистрационный номер юридического лица';en='Primary state registration number of legal entity'");
		Элементы.ОГРН.ПодсказкаВвода		= НСтр("ka='13 ციფრი';ru='13 цифр';en='13 digits'");
	...
	Иначе
		...
		Элементы.ИНН.ПодсказкаВвода			= НСтр("ka='13 ციფრი';ru='13 цифр';en='13 digits'");
		Элементы.ИНН.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
		Элементы.ОГРН.Заголовок				= НСтр("ka='ОГРН ИП';ru='ОГРН ИП';en='BIN SP'");
...
"указан в Свидетельстве о государственной регистрации физического лица в качестве ИП'");
		Элементы.ОГРН.ПодсказкаВвода		= НСтр("ka='15 ციფრი';ru='15 цифр';en='15 digits'");
		Элементы.ОГРН.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(15));
		Элементы.КодПоОКПО.ПодсказкаВвода	= НСтр("ka='10 ციფრი';ru='10 цифр';en='10 digits'");
...
Процедура СформироватьНадписьНастройкиЕНВД()
	...
	Если Не Выборка.Следующий() Или Выборка.КоличествоВидов = 0 Тогда
		Элементы.НастроитьЕНВД.Заголовок = НСтр("ka='<საქმიანობის ტიპების მითითება მშეგ-ისთვის>';ru='<<Указать виды деятельности для ЕНВД>>';en='<< Show activities for UTII >>'");
	...
	Иначе
		Элементы.НастроитьЕНВД.Заголовок = НСтр("ka='მშეგ-ის მიხედვით შევსებულია %1 %2 საქმიანობა';ru='Заполнено %1 %2 деятельности по ЕНВД';en='Completed% 1% 2 on UTII activities'");
		Элементы.НастроитьЕНВД.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПодобратьСловоПодЧисло(
				Выборка.КоличествоВидов,
				НСтр("ka='вид';ru='вид';en='view'"),
				НСтр("ka='სახეობის';ru='вида';en='species'"),
				НСтр("ka='სახეობებს';ru='видов';en='species'")));
	КонецЕсли;
...
Процедура СформироватьНадписьНастройкиПатента()
	...
	Если Не Выборка.Следующий() Или Выборка.КоличествоПатентов = 0 Тогда
		Элементы.НастроитьПатент.Заголовок = НСтр("ka='<<Указать патент>>';ru='<<Указать патент>>';en='<< >> These patents'");
	ИначеЕсли Выборка.КоличествоПатентов = 1 Тогда
		ВыборкаИмени = Результат[1].Выбрать();
		ВыборкаИмени.Следующий();
		Элементы.НастроитьПатент.Заголовок = ВыборкаИмени.НазваниеПатента;
	Иначе
		Элементы.НастроитьПатент.Заголовок = НСтр("ka='შევსებულია %1 %2';ru='Заполнено %1 %2';en='Completed% 1% 2'");
		Элементы.НастроитьПатент.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПодобратьСловоПодЧисло(
				Выборка.КоличествоПатентов,
				НСтр("ru='патент';en='patent'"),
				НСтр("ka='პატენტი';ru='патента';en='patent'"),
				НСтр("ka='патентов';ru='патентов';en='patents'")));
	КонецЕсли;
...
Процедура УстановитьПодсказкуПоТарифу()
	...
	Элементы.ДекорацияОписаниеТарифа.Заголовок = СтрШаблон(
		НСтр("ka='(საპენსიო ფონდი - %1%4, სდფ - %2%4, სსდფ - %3%4)';ru='(ПФР - %1%4, ФСС - %2%4, ФФОМС - %3%4)';en='(PFR -% 1% 4 FSS -% 2% 4 FFOMS - 4% 3%)'"),
		СрезПоТарифу.ПФР,
		СрезПоТарифу.ФСС,
		СрезПоТарифу.ФФОМС,"%"
	);
...
Процедура ОбработатьПараметрыЗадачиНалогов()
	...
	Если Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоЕНВД
		...
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ მშეგ-ის გამოყენების ოპცია';ru='Включите опцию применения ЕНВД в разделе Налоогобложение';en='Including the use of option UTII in Naloogoblozhenie section'");
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Справки2НДФЛ
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ დაქირავებული მუშაკების შრომის გამოყენება';ru='Включите испольльзование труда наемный рабочих в разделе Налогообложение';en='Turn ispollzovanie hired labor under Taxation'");
		Сообщи.КлючДанных = Объект.Ссылка;
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ დგს-ის გამოყენების ოპცია';ru='Включите опцию применения УСН в разделе Налоогобложение';en='Including the use of the option in the USN Naloogoblozhenie section'");
		Сообщи.КлючДанных = Объект.Ссылка;
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогПатент Тогда
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ პატენტის გამოყენების ოპცია';ru='Включите опцию применения патента в разделе Налоогобложение';en='Enable the use of the patent in section Naloogoblozhenie'");
		Сообщи.КлючДанных = Объект.Ссылка;
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ТорговыйСбор Тогда
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ სავაჭრო მოსაკრებლის გამოყენების ოპცია';ru='Включите опцию применения торгового сбора в разделе Налоогобложение';en='Including the use of option trading fees in Naloogoblozhenie section'");
		Сообщи.КлючДанных = Объект.Ссылка;
...
Функция ЗаблокироватьФизическоеЛицоПриРедактировании()
	...
	Если Не Параметры.Ключ.Пустая() И Не ФизическоеЛицоЗаблокировано Тогда
		Если Не ЗаблокироватьФизическоеЛицоПриРедактированииНаСервере() Тогда
			ПоказатьПредупреждение(, НСтр("ka='ფიზიკური პირის პირად მონაცემებში ცვლილებების შეტანა ვერ ხერხდება. შესაძლებელია, რომ მონაცემების რედაქტირება ხორციელდება სხვა მომხმარებლის მიერ.';ru='Не удается внести изменения в личные данные физического лица. Возможно данные редактируются другим пользователем.';en='Unable to make changes to the personal data of an individual. Perhaps the data is edited by another user.'"));
			ПрочитатьФизЛицо(Объект.ФизическоеЛицо);
			Возврат Ложь;
		Иначе
...
Процедура ПроверитьОднофамильцев(ОповещениеЗавершения = Неопределено)
	...
	Если Однофамильцы.Количество() > 0 Тогда
		...
		Если Однофамильцы.Количество() = 1 Тогда
			ПараметрыОткрытия.ЗаголовокФормы = НСтр("ka='Найден человек с похожим именем';ru='Найден человек с похожим именем';en='Find someone with the same name'");
			ПараметрыОткрытия.ТекстИнформационнойНадписи = 
				НСтр("ka='თუ ინდმეწარმედ გსურთ ამ ფიზიკური პირის მითითება, მაშინ დააჭირეთ ""დიახ, ეს არის ის, ვინც მჭირდება""-ს. თუ ეს, მისი მოგვარეა, მაშინ დააჭირეთ ""არა, ეს სხვა ადამიანია""-ს.';ru='Если вы хотите указать в качестве индивидуального предпринимателя это физическое лицо, нажмите ""Да, это тот, кто мне нужен""."
"Если это однофамилец, нажмите ""Нет, это другой человек"".'");
		Иначе
			ПараметрыОткрытия.ЗаголовокФормы = НСтр("ka='Найдены люди с похожими именами';ru='Найдены люди с похожими именами';en='Found people with similar names'");
			ПараметрыОткрытия.ТекстИнформационнойНадписи = 
				НСтр("ka='თუ გსურთ ინდმეწარმედ მიუთითოთ სიაში მოყვანილი ერთ-ერთი ფიზიკური პირი, მაშინ აირჩიეთ იგი და დააჭირეთ ""აღნიშნული ადამიანი არის ის, ვინც მჭირდება""-ს. თუ იგი მოგვარეა, მაშინ დააჭირეთ ""არა, ეს სხვა ადამიანია""-ს.';ru='Если вы хотите указать в качестве индивидуального предпринимателя одно из приведенных в списке физических лиц, выберите его и нажмите ""Отмеченный человек тот, кто мне нужен""."
"Если это однофамилец, нажмите ""Нет, это другой человек"".'");
		КонецЕсли;
...
Процедура СформироватьПредставлениеПроверкиОКВЭД(Форма)
	...
	Если Не ПустаяСтрока(Объект.КодОКВЭД) Тогда
		Если Не ПроверкаДанныхКлиентСервер.КодОКВЭДСоответствуетТребованиям(СокрЛП(Объект.КодОКВЭД)) Тогда
			ОписаниеОшибки = НСтр("ka='ესსკ-ის კოდი მოთხოვნებს არ შეესაბამება';ru='Код ОКВЭД не соответствует требованиям';en='NACE code does not comply with'");
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура УправлениеФлагамиРаботыСКартинками(ЭтоРаботаСЛоготипом = Ложь, ЭтоРаботаСФаксимиле = Ложь, ЭтоРаботаСПечатью = Ложь)
	
	РаботаСЛоготипом = ЭтоРаботаСЛоготипом;
	РаботаСФаксимиле = ЭтоРаботаСФаксимиле;
	РаботаСПечатью	 = ЭтоРаботаСПечатью;
...
Процедура ПросмотретьПрисоединенныйФайл()
	...
	ИначеЕсли РаботаСПечатью Тогда
		...
		ИмяРеквизитаОбъекта = "ФайлПечати";
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='სანახავად გამოსახულება არ არის';ru='Отсутстует изображение для просмотра';en='Otsutst image to view'");
...
&НаКлиенте
Процедура ДобавитьИзображениеПечатиФрагмент()
...
Процедура ВыполнитьЗаполнениеРеквизитовПоИНН(знач ИНН)
	...
	// Обработка ошибок
		Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ka='ორგანიზაციის რეკვიზიტების ავტომატურად შესავსებად, აუცილებელია მომხმარებლების ინტერნეტ-მხარდაჭერაზე შეერთება. შევუერთდეთ ეხლა?';ru='Для автоматического заполнения реквизитов организации"
...
Процедура ВыполнитьЗаполнениеСведенийОбОтделенииПФР()
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ka='ცნობარ ""კონტრაგენტები""-ში საპენსიო ფონდის განყოფილების ავტომატურად შესაქმნელად, საჭიროა მომხმარებლების ინტერნეტ-მხარდაჭერასთან შეერთება. საპენსიო ფონდის განყოფილების თაობაზე მონაცემები გამოგადგებათ გადასახადების გადახდისას. შევუერთდეთ ეხლა?';ru='Для автоматического создания отделения ПФ РФ"
...
Процедура ВыполнитьЗаполнениеСведенийОбОтделенииФСС()
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ka='ცნობარ ""კონტრაგენტები""-ში სდფ-ის განყოფილების ავტომატურად შესაქმნელად, საჭიროა მომხმარებლების ინეტრნეტ-მხარდაჭერაზე შეერთება. სდფ-ის განყოფილების მონაცემები გამოდგება გადასახადების გადახდისას. შევუერთოთ ეხლა?';ru='Для автоматического создания отделения ФСС"
"в справочнике «Контрагенты» необходимо подключиться к Интернет-поддержке"
...
Процедура ВыполнитьЗаполнениеСведенийОНалоговойИнспекции()
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ka='ცნობარ ""კონტრაგენტები""-ში საგადასახადო ორგანოს ავტომატურად შესაქმნელად, საჭიროა მომხმარებლების ინტერნეტ-მხარდაჭერასთან შეერთება. საგადასახადო ორგანოს თაობაზე მონაცემები გამოგადგებათ გადასახადების გადახდისას. შევუერთდეთ ეხლა?';ru='Для автоматического создания налогового органа"
"в справочнике «Контрагенты» необходимо подключиться к Интернет-поддержке"
...
Процедура УстановитьЗаголовокЮридическихДанных(Форма)
	...
	Если Не ПустаяСтрока(Объект.КПП) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='КПП';ru='КПП';en='PPC'") + " " + Объект.КПП);
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.ОГРН) Тогда
		Если Объект.ЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо") Тогда
			ДинамическиеПараметры.Добавить(НСтр("ka='ძსსნ';ru='ОГРН';en='BIN'") + " " + Объект.ОГРН);
		Иначе
			ДинамическиеПараметры.Добавить(НСтр("ka='ОГРН ИП';ru='ОГРН ИП';en='BIN SP'") + " " + Объект.ОГРН);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьЗаголовокКонтактнойИнформации(Форма)
	...
	Если ЗначениеЗаполнено(Телефон) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='тел.:';ru='тел.:';en='tel .:'") + " " + Телефон);
	КонецЕсли;
...
Процедура УстановитьЗаголовокНалогообложения(Форма)
	...
	Если Объект.ПлательщикУСН Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='УСН';ru='УСН';en='USN'") +НадписьСтавка );
	КонецЕсли;
	
	Если Объект.ПлательщикЕНВД Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ЕНВД';ru='ЕНВД';en='UTII'"));
	КонецЕсли;
	
	Если Объект.ПрименяетсяПатент Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='Патент';ru='Патент';en='Patent'"));
	КонецЕсли;
	
	Если Объект.ПлательщикТорговыйСбор Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='სავაჭრო შეკრება';ru='Торговый сбор';en='shopping collection'"));
	КонецЕсли;
...
Процедура УстановитьЗаголовокКодовСтатистики(Форма)
	...
	Если Не ПустаяСтрока(Объект.КодОКВЭД) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='ОКВЭД ';ka='ОКВЭД '")+Объект.КодОКВЭД);
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КодОКФС) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='ОКФС ';ka='ОКФС '")+Объект.КодОКФС);
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КодОКОПФ) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='ОКОПФ ';ka='ОКОПФ '")+Объект.КодОКОПФ);
	КонецЕсли;
...
////-------------------------- // Россофт // --->
#Область ПроцедурыИФункцииДоработки 

&НаСервере
Процедура УстановитьВидимостьЭлементовНалоговойСлужбы()
	
	впМногопользовательскийРежимОбменаRS = Объект.МногопользовательскийРежимОбменаRS;
	Элементы.РежимовПротоколаДляОбменаRS.Видимость		= Не впМногопользовательскийРежимОбменаRS;
	Элементы.ОдиночныйРежимОбменаRS.Видимость			= Не впМногопользовательскийРежимОбменаRS;
	//Элементы.МногопользовательскийРежимОбмена.Видимость = впМногопользовательскийРежимОбменаRS;
	Элементы.ПлательщикТранспортировкиТН.Доступность = Константы.ПлательщикТранспортировкиТН_в_Организации.Получить();// Kuzmich, 17.05.2022. ПлательщикТранспортировкиТН по-умолчанию
	
КонецПроцедуры 

&НаКлиенте
Процедура МногопользовательскийРежимОбменаСНСПриИзменении(Элемент)
	УстановитьВидимостьЭлементовНалоговойСлужбы();
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьсоединениеКНалоговойСлужбе(Команда)
	ОтветТН = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ИнициализацияОтправкиТН(Новый Структура,Объект.Ссылка);
	Если ТипЗнч(ОтветТН) = Тип("Строка") Тогда 
		Сообщить(ОтветТН);
	Иначе 
		Сообщить(НСтр("ka='შემოსავლების სამსახურის სერვისთან დაკავშირება წარმატებით შესრულდა';ru=' соединение к серверу налоговой службы  прошло успешно'"));
	КонецЕсли;
