...
"при выключенной настройке параметра учета ""Учет по нескольким организациям"".'");
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаЭлемента",
		Неопределено,
		НСтр("ka='Новости: Организации';ru='Новости: Организации'"),
		Ложь,
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		Если Не ЗначениеЗаполнено(ФИОФизическихЛиц.Фамилия) 
			...
			ТекстСообщения = НСтр("ka='გვარი და სახელი არ არის შევსებული';ru='Не заполнено ФИО'");
	...
	Если Не ПустаяСтрока(ОсновнойСчет_Номер) И Не ЗначениеЗаполнено(ОсновнойСчет_Банк) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "Заполнение", НСтр("ka='ბანკი';ru='Банк'"));
	...
	Если ЗначениеЗаполнено(ОсновнойСчет_Банк) И ПустаяСтрока(ОсновнойСчет_Номер) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "Заполнение", НСтр("ka='ანგარიშის ნომერი';ru='Номер счета'"));
...
Процедура ПрефиксПриИзменении(Элемент)
	...
	Если СтрНайти(Объект.Префикс, "-") > 0 Тогда
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ორგანიზაციის პრეფიქსში არ შეიძლება, რომ გამოყენებული იქნას სიმბოლო ""-"".';ru='Нельзя в префиксе организации использовать символ ""-"".'"));
...
Процедура КодОКОПФОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтрЗаканчиваетсяНа(ВыбранноеЗначение, "0000") Тогда
		СтандартнаяОбработка = Ложь;
		ТекстСообщения = НСтр("ka='აირჩიეთ საზოგადოებრივ-სამართლებრივი ფორმების კლასიფიკატორის კოდი, რომელიც კოდების ჯგუფს არ წარმოადგენს';ru='Выберите код ОКОПФ, не являющийся группой кодов'");
...
Процедура ЗаполнитьРеквизитыПоДаннымЕГР(Команда)
	...
	// Выполняем заполнение по ИНН без открытия вспомогательной формы
		...
		Если КлючевыеРеквизитыЗаполнены(ЭтотОбъект) Тогда
			ТекстВопроса = НСтр("ka='გადავავსოთ მიმდინარე რეკვიზიტები?';ru='Перезаполнить текущие реквизиты?'");
...
Процедура ИсторияСтраховыхВзносов(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ჩაწერა';ru='Записать'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='გაუქმება';ru='Отмена'"));
...
Процедура ДобавитьИзображениеЛоготипа(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ka='გამოსახულების ასარჩევად საჭიროა ობიექტის ჩაწერა. ჩავწეროთ?';ru='Для выбора изображения необходимо записать объект. Записать?'");
...
Процедура ИзменитьИзображениеЛоготипа(Команда)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='რედაქტირებისთვის გამოსახულება არ არის';ru='Отсутстует изображение для редактирования'");
...
Процедура ЗаполнитьРеквизитыНалоговогоОрганаПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодНалоговогоОрганаПолучателя) Тогда
		ПоказатьПредупреждение(, НСтр("ka='ველი ""ინსპექციის კოდი"" არ არის შევსებული';ru='Поле ""Код инспекции"" не заполнено'"));
...
Процедура ЗаполнитьРеквизитыПФРПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодОрганаПФР) Тогда
		ПоказатьПредупреждение(, НСтр("ka='ველი ""საპენსიო ორგანოს კოდი"" არ არის შევსებული';ru='Поле ""Код органа ПФР"" не заполнено'"));
...
Процедура ЗаполнитьРеквизитыФССПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодОрганаФСС) Тогда
		ПоказатьПредупреждение(, НСтр("ka='ველი ""სდფ-ის ორგანოს კოდი"" არ არის შევსებული';ru='Поле ""Код органа ФСС"" не заполнено'"));
...
Процедура УправлениеФормой(Форма)
	...
	Если Объект.ЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо") Тогда
		...
		Элементы.ИНН.ПодсказкаВвода			= НСтр("ka='13 ციფრი';ru='13 цифр'");
		...
		Элементы.ОГРН.Подсказка				= НСтр("ka='იურიდიული პირის ძირითადი სახელმწიფო სარეგისტრაციო ნომერი';ru='Основной государственный регистрационный номер юридического лица'");
		Элементы.ОГРН.ПодсказкаВвода		= НСтр("ka='13 ციფრი';ru='13 цифр'");
		Элементы.ОГРН.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
		Элементы.КодПоОКПО.ПодсказкаВвода	= НСтр("ka='8 ციფრი';ru='8 цифр'");
	...
	Иначе
		...
		Элементы.ИНН.ПодсказкаВвода			= НСтр("ka='13 ციფრი';ru='13 цифр'");
		Элементы.ИНН.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
		Элементы.ОГРН.Заголовок				= НСтр("ka='ОГРН ИП';ru='ОГРН ИП'");
...
"указан в Свидетельстве о государственной регистрации физического лица в качестве ИП'");
		Элементы.ОГРН.ПодсказкаВвода		= НСтр("ka='15 ციფრი';ru='15 цифр'");
		Элементы.ОГРН.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(15));
		Элементы.КодПоОКПО.ПодсказкаВвода	= НСтр("ka='10 ციფრი';ru='10 цифр'");
...
Процедура СформироватьНадписьНастройкиЕНВД()
	...
	Если Не Выборка.Следующий() Или Выборка.КоличествоВидов = 0 Тогда
		Элементы.НастроитьЕНВД.Заголовок = НСтр("ka='<საქმიანობის ტიპების მითითება მშეგ-ისთვის>';ru='<<Указать виды деятельности для ЕНВД>>'");
	...
	Иначе
		Элементы.НастроитьЕНВД.Заголовок = НСтр("ka='მშეგ-ის მიხედვით შევსებულია %1 %2 საქმიანობა';ru='Заполнено %1 %2 деятельности по ЕНВД'");
		Элементы.НастроитьЕНВД.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПодобратьСловоПодЧисло(
				Выборка.КоличествоВидов,
				НСтр("ka='вид';ru='вид'"),
				НСтр("ka='სახეობის';ru='вида'"),
				НСтр("ka='სახეობებს';ru='видов'")));
	КонецЕсли;
...
Процедура СформироватьНадписьНастройкиПатента()
	...
	Если Не Выборка.Следующий() Или Выборка.КоличествоПатентов = 0 Тогда
		Элементы.НастроитьПатент.Заголовок = НСтр("ka='<<Указать патент>>';ru='<<Указать патент>>'");
	ИначеЕсли Выборка.КоличествоПатентов = 1 Тогда
		ВыборкаИмени = Результат[1].Выбрать();
		ВыборкаИмени.Следующий();
		Элементы.НастроитьПатент.Заголовок = ВыборкаИмени.НазваниеПатента;
	Иначе
		Элементы.НастроитьПатент.Заголовок = НСтр("ka='შევსებულია %1 %2';ru='Заполнено %1 %2'");
		Элементы.НастроитьПатент.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПодобратьСловоПодЧисло(
				Выборка.КоличествоПатентов,
				НСтр("ru='патент'"),
				НСтр("ka='პატენტი';ru='патента'"),
				НСтр("ka='патентов';ru='патентов'")));
	КонецЕсли;
...
Процедура УстановитьПодсказкуПоТарифу()
	...
	Элементы.ДекорацияОписаниеТарифа.Заголовок = СтрШаблон(
		НСтр("ka='(საპენსიო ფონდი - %1%4, სდფ - %2%4, სსდფ - %3%4)';ru='(ПФР - %1%4, ФСС - %2%4, ФФОМС - %3%4)'"),
		СрезПоТарифу.ПФР,
		СрезПоТарифу.ФСС,
		СрезПоТарифу.ФФОМС,"%"
	);
...
Процедура ОбработатьПараметрыЗадачиНалогов()
	...
	Если Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоЕНВД
		...
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ მშეგ-ის გამოყენების ოპცია';ru='Включите опцию применения ЕНВД в разделе Налоогобложение'");
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Справки2НДФЛ
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ დაქირავებული მუშაკების შრომის გამოყენება';ru='Включите испольльзование труда наемный рабочих в разделе Налогообложение'");
		Сообщи.КлючДанных = Объект.Ссылка;
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ დგს-ის გამოყენების ოპცია';ru='Включите опцию применения УСН в разделе Налоогобложение'");
		Сообщи.КлючДанных = Объект.Ссылка;
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогПатент Тогда
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ პატენტის გამოყენების ოპცია';ru='Включите опцию применения патента в разделе Налоогобложение'");
		Сообщи.КлючДанных = Объект.Ссылка;
	...
	ИначеЕсли Параметры.ЗадачаНалогов = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ТорговыйСбор Тогда
		...
		Сообщи = новый СообщениеПользователю();
		Сообщи.Текст = НСтр("ka='ნაწილ ""დაბეგვრა""-ში ჩართეთ სავაჭრო მოსაკრებლის გამოყენების ოპცია';ru='Включите опцию применения торгового сбора в разделе Налоогобложение'");
		Сообщи.КлючДанных = Объект.Ссылка;
...
Функция ЗаблокироватьФизическоеЛицоПриРедактировании()
	...
	Если Не Параметры.Ключ.Пустая() И Не ФизическоеЛицоЗаблокировано Тогда
		Если Не ЗаблокироватьФизическоеЛицоПриРедактированииНаСервере() Тогда
			ПоказатьПредупреждение(, НСтр("ka='ფიზიკური პირის პირად მონაცემებში ცვლილებების შეტანა ვერ ხერხდება. შესაძლებელია, რომ მონაცემების რედაქტირება ხორციელდება სხვა მომხმარებლის მიერ.';ru='Не удается внести изменения в личные данные физического лица. Возможно данные редактируются другим пользователем.'"));
			ПрочитатьФизЛицо(Объект.ФизическоеЛицо);
			Возврат Ложь;
		Иначе
...
Процедура ПроверитьОднофамильцев(ОповещениеЗавершения = Неопределено)
	...
	Если Однофамильцы.Количество() > 0 Тогда
		...
		Если Однофамильцы.Количество() = 1 Тогда
			ПараметрыОткрытия.ЗаголовокФормы = НСтр("ka='Найден человек с похожим именем';ru='Найден человек с похожим именем'");
...
"Если это однофамилец, нажмите ""Нет, это другой человек"".'");
		Иначе
			ПараметрыОткрытия.ЗаголовокФормы = НСтр("ka='Найдены люди с похожими именами';ru='Найдены люди с похожими именами'");
			ПараметрыОткрытия.ТекстИнформационнойНадписи = 
...
Процедура СформироватьПредставлениеПроверкиОКВЭД(Форма)
	...
	Если Не ПустаяСтрока(Объект.КодОКВЭД) Тогда
		Если Не ПроверкаДанныхКлиентСервер.КодОКВЭДСоответствуетТребованиям(СокрЛП(Объект.КодОКВЭД)) Тогда
			ОписаниеОшибки = НСтр("ka='ესსკ-ის კოდი მოთხოვნებს არ შეესაბამება';ru='Код ОКВЭД не соответствует требованиям'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПросмотретьПрисоединенныйФайл()
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='სანახავად გამოსახულება არ არის';ru='Отсутстует изображение для просмотра'");
...
Процедура УстановитьЗаголовокЮридическихДанных(Форма)
	...
	Если Не ПустаяСтрока(Объект.ИНН) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ს/კ';ru='ИНН'") + " " + Объект.ИНН);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='ს/კ ...';ru='ИНН ...'"));
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КПП) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='КПП';ru='КПП'") + " " + Объект.КПП);
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.ОГРН) Тогда
		Если Объект.ЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо") Тогда
			ДинамическиеПараметры.Добавить(НСтр("ka='ძსსნ';ru='ОГРН'") + " " + Объект.ОГРН);
		Иначе
			ДинамическиеПараметры.Добавить(НСтр("ka='ОГРН ИП';ru='ОГРН ИП'") + " " + Объект.ОГРН);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьЗаголовокКонтактнойИнформации(Форма)
	...
	Если ЗначениеЗаполнено(Телефон) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='тел.:';ru='тел.:'") + " " + Телефон);
	КонецЕсли;
...
Процедура УстановитьЗаголовокНалогообложения(Форма)
	...
	Если Объект.ПлательщикУСН Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='УСН';ru='УСН'") +НадписьСтавка );
	КонецЕсли;
	
	Если Объект.ПлательщикЕНВД Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ЕНВД';ru='ЕНВД'"));
	КонецЕсли;
	
	Если Объект.ПрименяетсяПатент Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='Патент';ru='Патент'"));
	КонецЕсли;
	
	Если Объект.ПлательщикТорговыйСбор Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='სავაჭრო შეკრება';ru='Торговый сбор'"));
	КонецЕсли;
