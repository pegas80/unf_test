...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='რეკვიზიტები';ru='Реквизиты'");
	КомандаПечати.СписокФорм = "ФормаЭлемента,ФормаСписка";
	КомандаПечати.ЗаголовокФормы = НСтр("ka='ორგანიზაციის რეკვიზიტების ბეჭდვა';ru='Печать реквизитов организации'");
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов)
	...
	Запрос.УстановитьПараметр("Договор", 							НСтр("ka='ხელშეკრულება № 000001 от 30.12.1922';ru='Договор № 000001 от 30.12.1922'"));
	Запрос.УстановитьПараметр("ОснованиеПечати", 					НСтр("ka='ხელშეკრულება № 000001 от 30.12.1922';ru='Договор № 000001 от 30.12.1922'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеквизитыОрганизации") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='ორგანიზაციის რეკვიზიტები';ru='Реквизиты организации'"),
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НапечататьПомощникСозданияФаксимилеПечати") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='როგორ მარტივად და სწრაფად შეიძლება შეიქმნას ფაქსიმილია?';ru='Как быстро и просто создать факсимиле печати?'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Дело.Важное			= Ложь;
		Дело.Представление	= НСтр("ka='1C-ედბ-სთან შეერთება';ru='Подключиться к 1С-ЭДО'") + ", " + Выборка.Наименование;
		Дело.Количество		= 0;
		...
		Если ВключенОбменЭД Тогда
			...
			Если ЕстьПравоНастройкиЭДО Тогда
				...
				Дело.Подсказка	= НСтр("ka='1C-ედბ-სთან (ელექტრონული დოკუმენტების გაცვლასთან) შეერთება';ru='Подключиться к 1С-ЭДО (обмену электронными документами)'") + ", " + Выборка.Наименование;
			Иначе
				Дело.Форма			= "";
				Дело.ПараметрыФормы = Новый Структура;
				Дело.Подсказка	= НСтр("ka='ელექტრონული დოკუმენტების გაცვლის გასამართად არასაკმარისი უფლებებია. მიმართეთ ადმინისტრატორს.';ru='Недостаточно прав для настройки обмена электронными документами. Обратитесь к администратору.'");
			КонецЕсли;
		...
		ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
			...
			Дело.ПараметрыФормы = Новый Структура;
			Дело.Подсказка	= НСтр("ka='აუცილებელია კონტრაგენტებთან ელექტრონული დოკუმენტების გაცვლის ჩართვა.';ru='Необходимо включить обмен электронными документами с контрагентами.'");
			
		Иначе
			
			Дело.Форма			= "";
			Дело.ПараметрыФормы = Новый Структура;
			Дело.Подсказка	= НСтр("ka='კონტრაგენტებთან ელექტრონული დოკუმენტების გაცვლის შესაძლებლობის ჩასართავად მიმართეთ ადმინისტრატორს.';ru='Для включения возможности обмена электронными документами с контрагентами обратитесь к администратору.'");
