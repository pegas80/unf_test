...
Функция ПолучитьСхемуКритериевОтбора(ОписаниеОбъектаМетаданных, ИмяМакетаШаблона = "СКД_КритерииПравилаРабочегоПроцесса") Экспорт
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ka='მეტამონაცემთა უცნობი ობიექტი სამუშაო პროცესის წესისთვის';ru='Неизвестный объект метаданных для правила рабочего процесса'");
	КонецЕсли;
	
	Если МетаданныеОбъекта.Макеты.Найти(ИмяМакетаШаблона) = Неопределено Тогда
		ВызватьИсключение НСтр("ka='სამუშაო პროცესის წესისთვის მონაცემთა შეერთების სქემა ვერ მოიძებნა';ru='Не найдена схема компоновки данных для правила рабочего процесса'");
	КонецЕсли;
...
Процедура ЗаполнитьПоставляемыеПравила() Экспорт
	...
	Попытка
		...
		Действие.Наименование		= НСтр("ka='საფუძვლის მიხედვით შეხსენების შექმნა: მყიდველის შეკვეთა';ru='Создать напоминание по основанию: Заказ покупателя'");
		...
		Правило.Наименование	= НСтр("ka='მდგომარეობის შეცვლის შესახებ შევატყობინოთ მყიდველის შეკვეთის ავტორს';ru='Оповещать автора заказа покупателя об изменении состояния'");
		Правило.Автор			= Пользователи.СсылкаНеуказанногоПользователя(Истина);
		Правило.Включено		= Ложь;
		Правило.Комментарий		= НСтр("ka='#შექმნილია ავტომატურად სწრაფი სტარტისთვის';ru='#Создано автоматически для быстрого старта'");
...
"С уважением, [ЗаказПокупателя.Ответственный].'");
		...
		Шаблон.Наименование			= НСтр("ka='მყიდველის შეტყობინება შეკვეთის მდგომარეობის ცვლილების შესახებ';ru='Уведомление покупателя об изменении состояния заказа'");
		...
		Шаблон.ТемаПисьма = НСтр("ka='Уведомление о вашем заказе №[ЗаказПокупателя.Номер]';ru='Уведомление о вашем заказе №[ЗаказПокупателя.Номер]'");
		...
		Действие = Справочники.ДействияРабочегоПроцесса.СоздатьЭлемент();
		Действие.Наименование		= НСтр("ka='E-mail-ის გაგზავნა საფუძვლის მიხედვით: მყიდველის შეკვეთა';ru='Отправить E-mail по основанию: Заказ покупателя'");
		...
		Правило = Справочники.ПравилаРабочегоПроцесса.СоздатьЭлемент();
		Правило.Наименование	= НСтр("ka='შეკვეთის მდგომარეობის ცვლილების შესახებ მყიდველის შეტყობინება';ru='Оповещать покупателя об изменении состояния заказа'");
		Правило.Автор			= Пользователи.СсылкаНеуказанногоПользователя(Истина);
		Правило.Включено		= Ложь;
		Правило.Комментарий		= НСтр("ka='#შექმნილია ავტომატურად სწრაფი სტარტისთვის';ru='#Создано автоматически для быстрого старта'");
		Правило.СписокДействий	= Действие.Наименование;
