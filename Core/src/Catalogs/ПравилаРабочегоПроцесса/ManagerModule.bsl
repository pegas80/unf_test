...
Функция ПолучитьСхемуКритериевОтбора(ОписаниеОбъектаМетаданных, ИмяМакетаШаблона = "СКД_КритерииПравилаРабочегоПроцесса") Экспорт
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ka='მეტამონაცემთა უცნობი ობიექტი სამუშაო პროცესის წესისთვის';ru='Неизвестный объект метаданных для правила рабочего процесса';en='Unknown object metadata workflow rules'");
	КонецЕсли;
	
	Если МетаданныеОбъекта.Макеты.Найти(ИмяМакетаШаблона) = Неопределено Тогда
		ВызватьИсключение НСтр("ka='სამუშაო პროცესის წესისთვის მონაცემთა შეერთების სქემა ვერ მოიძებნა';ru='Не найдена схема компоновки данных для правила рабочего процесса';en='Found pattern layout data for rules workflow'");
	КонецЕсли;
...
Процедура ЗаполнитьПоставляемыеПравила() Экспорт
	...
	Попытка
		...
		Действие.Наименование		= НСтр("ka='საფუძვლის მიხედვით შეხსენების შექმნა: მყიდველის შეკვეთა';ru='Создать напоминание по основанию: Заказ покупателя';en=""Create a reminder to the base: Buyer's order""");
		...
		Правило.Наименование	= НСтр("ka='მდგომარეობის შეცვლის შესახებ შევატყობინოთ მყიდველის შეკვეთის ავტორს';ru='Оповещать автора заказа покупателя об изменении состояния';en='Notify the author of the order the customer status changes'");
		Правило.Автор			= Пользователи.СсылкаНеуказанногоПользователя(Истина);
		Правило.Включено		= Ложь;
		Правило.Комментарий		= НСтр("ka='#შექმნილია ავტომატურად სწრაფი სტარტისთვის';ru='#Создано автоматически для быстрого старта';en='# Created automatically for quick start'");
...
""
"С уважением, [ЗаказПокупателя.Ответственный].';ka='[ЗаказПокупателя.Обращение] [ЗаказПокупателя.Покупатель]!"
""
"Состояние вашего заказа №[ЗаказПокупателя.Номер] от [ЗаказПокупателя.Дата] изменилось на ""[ЗаказПокупателя.СостояниеЗаказа]""."
""
"С уважением, [ЗаказПокупателя.Ответственный].'");
		...
		Шаблон.Наименование			= НСтр("ka='მყიდველის შეტყობინება შეკვეთის მდგომარეობის ცვლილების შესახებ';ru='Уведомление покупателя об изменении состояния заказа';en=""Buyer's notice of change of order status""");
		...
		Шаблон.ТемаПисьма = НСтр("ka='Уведомление о вашем заказе №[ЗаказПокупателя.Номер]';ru='Уведомление о вашем заказе №[ЗаказПокупателя.Номер]';en='Notification of your order number [ZakazPokupatelya.Nomer]'");
		...
		Действие = Справочники.ДействияРабочегоПроцесса.СоздатьЭлемент();
		Действие.Наименование		= НСтр("ka='E-mail-ის გაგზავნა საფუძვლის მიხედვით: მყიდველის შეკვეთა';ru='Отправить E-mail по основанию: Заказ покупателя';en=""Send E-mail to the base: Buyer's order""");
		...
		Правило = Справочники.ПравилаРабочегоПроцесса.СоздатьЭлемент();
		Правило.Наименование	= НСтр("ka='შეკვეთის მდგომარეობის ცვლილების შესახებ მყიდველის შეტყობინება';ru='Оповещать покупателя об изменении состояния заказа';en='Notify the buyer to change the status of an order'");
		Правило.Автор			= Пользователи.СсылкаНеуказанногоПользователя(Истина);
		Правило.Включено		= Ложь;
		Правило.Комментарий		= НСтр("ka='#შექმნილია ავტომატურად სწრაფი სტარტისთვის';ru='#Создано автоматически для быстрого старта';en='# Created automatically for quick start'");
		Правило.СписокДействий	= Действие.Наименование;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სტანდარტულად ცნობარ ""სამუშაო პროცესის წესები""-ს შევსება ვერ მოხერხდა შემდეგი მიზეზით: %1';ru='Не удалось заполнить справочник ""Правила рабочего процесса"" по умолчанию по причине:"
"%1'"), 
