...
Процедура РабочийКаталогВладельцаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Фильтр = НСтр("ka='ყველა ფაილი(*.*)|*.*';ru='Все файлы(*.*)|*.*'");
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Исключение
			...
			ТекстОшибки =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='არასწორი გზაა ან კატალოგ ""%1""-ში არ არსებობს ჩაწერის უფლება';ru='Неверный путь или отсутствуют права на запись в каталог ""%1""'"), ИмяКаталога);
