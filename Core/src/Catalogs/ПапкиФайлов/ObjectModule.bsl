...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() Или ТекущаяПапка.Родитель <> Родитель Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", Родитель) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ფაილების საქაღალდე ""%1""-ში ქვესაქაღალდეების დასამატებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для добавления подпапок в папку файлов ""%1"".'"),
				Строка(Родитель));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПометкаУдаления И ТекущаяПапка.ПометкаУдаления <> Истина Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", Ссылка) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ფაილების საქაღალდე ""%1""-ის შესაცვლელად არასკმარისი უფლებებია.';ru='Недостаточно прав для изменения папки файлов ""%1"".'"),
				Строка(Ссылка));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПометкаУдаления <> ТекущаяПапка.ПометкаУдаления И Не Ссылка.Пустая() Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				                     НСтр("ka='საქაღალდე %1-ის წაშლა არ შეიძლება, ვინაიდან იგი შეიცავს ფაილ ""%2""-ს, რომელიც დაკავებულია რედაქტირებისთვის.';ru='Папку %1 нельзя удалить, т.к. она содержит файл ""%2"", занятый для редактирования.'"),
				                     Строка(Ссылка),
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если МассивНайденныхНедопустимыхСимволов.Количество() <> 0 Тогда
		...
		Текст = НСтр("ka='საქაღალდის სახელწოდება შეიცავს აკრძალულ სიმბოლოებს ( \ / : * ? "" < > | .. )';ru='Наименование папки содержит запрещенные символы ( \ / : * ? "" < > | .. )'");
