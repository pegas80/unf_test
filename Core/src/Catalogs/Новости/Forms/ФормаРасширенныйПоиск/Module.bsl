...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПустаяСтрока(ЭтаФорма.СтрокаПоиска) Тогда
		...
		Сообщение.Текст = НСтр("ka='ძებნის სტრიქონი ცარიელია';ru='Строка поиска пустая';en='Search string empty'");
	...
	Если (ЭтаФорма.ПоискДатаОТ <> '00010101') И (ЭтаФорма.ПоискДатаДО <> '00010101') Тогда
		Если (ЭтаФорма.ПоискДатаОТ > ЭтаФорма.ПоискДатаДО) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='თარიღების მიხედვით ძებნის ინტერვალი არასწორად არის მიცემული: დაწყების თარიღი დასრულების თარიღზე უფრო გვიანია';ru='Неверно задан интервал поиска по датам: дата начала позже даты окончания';en='search interval is invalid dates: the date is later than the end date'");
