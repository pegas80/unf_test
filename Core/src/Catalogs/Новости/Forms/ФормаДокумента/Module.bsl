...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ka='(все время указано для часового пояса %1 (%2))';ru='(все время указано для часового пояса %1 (%2))'"),
		ЧасовойПояс(),
	...
	Если (ЭтотОбъект.БезАвторизации = Истина)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ka='ამ ახალი ამბის რედაქტირებისთვის, საჭიროა ახალი ამბების ცენტრში ავტორიზაციის გავლა.';ru='Для редактирования этой новости необходимо авторизоваться в новостном центре.'");
	...
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ka='ახალი ამბავი წარმატებით გამოქვეყნდა. მის შესაცვლელად, თავდაპირველად საჭიროა პუბლიკაციის გაუქმება.';ru='Новость была успешно опубликована. Чтобы изменить ее, необходимо вначале отменить публикацию.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
			И (ЭтотОбъект.СтатусНовостиНаСервере = 3) Тогда // Есть на сервере, опубликована, нельзя повторно публиковать.
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ka='ახალი ამბავი წარმატებით გამოქვეყნდა. ვინაიდან ახალი ამბავი გამოქვეყნდა არხებში, რომლებზეც აკრძალულია ხელახლა გამოქვეყნება (იმელი, სმს), მისი შეცვლა უკვე არ შეიძლება, მაგრამ შეიძლება ამ ახალი ამბის კოპირება და მისი გამართვა.';ru='Новость была успешно опубликована. Так как новость была опубликована в каналах, по которым запрещена перепубликация (емейл, смс), то изменить ее уже нельзя, но можно скопировать эту новость и править уже ее.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для С=0 По Объект.Действия.Количество()-1 Цикл
		...
		Если НайденныеПовторения.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='მოქმედების იდენტიფიკატორი %УИНДействия% მეორდება %КоличествоПовторений%';ru='Идентификатор действия %УИНДействия% повторяется %КоличествоПовторений%'");
		...
		Если Лев(ВРег(ТекущаяСтрока.УИНДействия), "3") <> ВРег("1C:") Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='მოქმედების იდენტიფიკატორი უნდა იწყებოდეს 1C-ით:';ru='Идентификатор действия должен начинаться с 1C:'");
			Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		КонецЕсли;
		...
		Для ТекущийСимвол=4 По СтрДлина(ТекущаяСтрока.УИНДействия) Цикл
			...
			Если (Найти(РазрешенныеСимволы, Символ) = 0) Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='მოქმედების იდენტიფიკატორი უნდა იწყებოდეს 1C-ით: და უნდა შედგებოდეს მხოლოდ ლათინური სიმბოლოებისგან, ციფრებისგან და ხაზგასმის ნიშნისგან';ru='Идентификатор действия должен начинаться с 1C: и состоять только из английских символов, цифр и знака подчеркивания'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЦикла;
		...
		Если ПустаяСтрока(ТекущаяСтрока.Действие) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='ცარიელი ქმედება';ru='Пустое действие'");
		...
		Иначе
			...
			Иначе
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='არასწორი მოქმედება. აირჩიეთ მოქმედების მნიშვნელობა სიიდან';ru='Неправильное действие. Выберите значение действия из списка'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].Действие";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
	...
	Для С=0 По Объект.ПараметрыДействий.Количество()-1 Цикл
		...
		ИначеЕсли НайденныеВладельцы.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='პარამეტრს მხოლოდ ერთი მფლობელი (ერთი მოქმედება) უნდა ჰყავდეს';ru='У параметра должен быть только один владелец (одно действие)'");
			Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		Иначе // =1
			...
			Если ТекущийВладелец.Действие = "Переход по навигационной ссылке" Тогда
				...
				Иначе
					Отказ = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ka='მოქმედება [%Действие%]-ს არასწორი პარამეტრი [%Параметр%] აქვს. აირჩიეთ მნიშვნელობა სიიდან';ru='У действия [%Действие%] параметр [%Параметр%] неправильный. Выберите значение из списка'");
			...
			ИначеЕсли ТекущийВладелец.Действие = "Показать картинку" Тогда
				Если ТекущаяСтрока.Параметр = "УИН" Тогда
					...
					Если НайденныеБинарныеДанные.Количество() = 0 Тогда
						Отказ = Истина;
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = НСтр("ka='მოქმედება [%Действие%]-ის პარამეტრი [%Параметр%] მიუთითებს არარსებულ ბინარულ მონაცემებზე. აირჩიეთ ბინარულ მონაცემთა სწორი მნიშვნელობა.';ru='У действия [%Действие%] параметр [%Параметр%] указывает на несуществующие бинарные данные. Выберите правильное значение бинарных данных.'");
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Действие%", ТекущийВладелец.Действие);
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Параметр%", ТекущаяСтрока.Параметр);
						Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].Параметр";
						Сообщение.Сообщить();
...
Процедура ПримерНовостиДля1СПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ПродолжитьОбработку = Истина Тогда
		...
		Иначе
			СтандартнаяОбработка = Ложь;
			ТекстСообщения = НСтр("ka='უცნობი ბმული: %НавигационнаяСсылка%';ru='Неизвестная ссылка: %НавигационнаяСсылка%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НавигационнаяСсылка%", НавигационнаяСсылка);
			ПоказатьПредупреждение(
				, // ОписаниеОповещенияОЗавершении
				ТекстСообщения,
				0,
				НСтр("ka='შეცდომა';ru='Ошибка'")); 
		КонецЕсли;
	КонецЕсли;
	...
	Если ОтменаРедактирования <> Истина Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.ВерсияОТ > ТекущиеДанные.ВерсияДО Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='ვერსია -დან [ВерсияОТ] არ უნდა იყოს უფრო მეტი, ვიდრე ვერსია -მდე [ВерсияДО].';ru='Версия ОТ [ВерсияОТ] не должна быть больше, чем Версия ДО [ВерсияДО].'");
...
"А в поле Действие должно быть значение, выбранное из списка.'");
			ПоказатьПредупреждение(
				...
				0, // Таймаут
				НСтр("ka='შეცდომა';ru='Ошибка'")); // Заголовок
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаЗагрузитьФайлКакБинарныеДанные(Команда)
	...
	Иначе
		ТекстОшибки = НСтр("ka='აირჩიეთ სტრიქონი, სადაც უნდა მოთავსდეს მონაცემები';ru='Выберите строку, куда помещать данные'");
		ПоказатьПредупреждение(
			, // ОписаниеоповещенияОЗавершении
			ТекстОшибки, // ТекстПредупреждения
			0, // Таймаут
...
Процедура КомандаБуферОбмена_Копировать(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Копировать_КатегорииПростые")
		...
		Если ЭлементФормы.ВыделенныеСтроки.Количество() > 0 Тогда
			...
			КонецЦикла;
			ТекстСообщения = НСтр("ka='კოპირებულია სტრიქონი: %ТекущийБуферОбмена.Количество()%';ru='Скопировано строк: %ТекущийБуферОбмена.Количество()%'");
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ka='გამოჰყავით თუნდ ერთი სტრიქონი';ru='Выделите хотя бы одну строку'"), , , БиблиотекаКартинок.Внимание);
		КонецЕсли;
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Вставить_ТекущийБуферОбмена_КатегорииПростые")
		...
		Если ДанныеБуфераОбмена.Количество() > 0 Тогда
			Для каждого ТекущаяСкопированнаяСтрока Из ДанныеБуфераОбмена Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ka='ჩასმულია სტრიქონი: %ДанныеБуфераОбмена.Количество()%';ru='Вставлено строк: %ДанныеБуфераОбмена.Количество()%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеБуфераОбмена.Количество()%", ДанныеБуфераОбмена.Количество());
			ПоказатьОповещениеПользователя(ТекстСообщения, , , БиблиотекаКартинок.Информация);
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ka='გაცვლის ბუფერში მონაცემები არ არის.';ru='Нет данных в буфере обмена.'"), , НСтр("ka='თავდაპირველად, მოახდინეთ სტრიქონების კოპირება.';ru='Вначале скопируйте строки.'"), БиблиотекаКартинок.Внимание);
		КонецЕсли;
...
"Для проверки новости необходимо открыть форму вручную или исправить значения в колонках.'");
			...
			ПоказатьПредупреждение(
				...
				ТекстОшибки, // ТекстПредупреждения
				0, // Таймаут
				НСтр("ka='გაფრთხილება';ru='Предупреждение'")); // Заголовок
		КонецПопытки;
	...
	Если Найти(ВРег(Команда.Имя), ВРег(ШаблонИмениКоманды)) = 1 Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если ПустаяСтрока(ПолеТекстовогоДокумента.ВыделенныйТекст) Тогда
				ТекстГиперссылки = НСтр("ka='დააჭირეთ აქ';ru='Нажмите здесь'");
			Иначе
...
Процедура КомандаИмпортЭкспортXML(Команда)
	...
	ИначеЕсли Команда.Имя = "КомандаИмпортИзXML" Тогда
		Если Объект.ЛентаНовостей.Пустая() Тогда
			ТекстСообщения = НСтр("ka='Необходимо заполнить ленту новостей.';ru='Необходимо заполнить ленту новостей.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
...
"%Предупреждение%'");
	...
	Если Объект.БинарныеДанные.Итог("ДанныеРазмер") > 50000 Тогда
		лкТекст = СтрЗаменить(лкТекст, "%Предупреждение%", НСтр("ka='დიდი ზომის ახალმა ამბებმა შეიძლება შეანელოს პროგრამის მუშაობა 1С-ის პლატფრომაზე';ru='Новости большого размера могут замедлить работу программы на платформе 1С'"));
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(СписокТабличныхЧастей = Неопределено)
	...
	Если МассивТабличныхЧастей.Найти("БинарныеДанные") <> Неопределено Тогда
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='ბინარული მონაცემების ჩატვირთვა ვერ მოხერხდა (ფორმაში და ობიექტში სტრიქონების რაოდენობა არ ემთხვევა). შენახვისას, ისინი დაიკარგება. რეკომენდებულია შენახვის გარეშე ახალი ამბების დახურვა / გახსნა';ru='Не удалось загрузить бинарные данные (не совпадает количество строк в форме и в объекте). При сохранении они будут утеряны. Рекомендуется закрыть / открыть форму новости без сохранения'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеКатегорииДляЛентыНовостейСервер()
	...
	Если МассивКатегорий_Простые.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("ka='მარტივი კატეგორიები';ru='Простые категории'");
	Иначе
	...
	Если МассивКатегорий_ИнтервалыВерсий.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("ka='ვერსიებზე მიბმა';ru='Привязка к версиям'");
	Иначе
	...
	Если ТипЗнч(РезультатЗакрытия) = ТипСтруктура Тогда
		...
		Если ВРег(РезультатЗакрытия.Действие) = ВРег("Переход по интернет ссылке") Тогда
			...
			Если ПустаяСтрока(лкВыделенныйТекст) Тогда
				лкВыделенныйТекст = НСтр("ka='დააჭირეთ აქ';ru='Нажмите здесь'");
			КонецЕсли;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	НоваяКоманда.Заголовок                 = НСтр("ka='Разрыв строки <br/>';ru='Разрыв строки <br/>'");
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Горизонтальная линия <hr/>';ru='Горизонтальная линия <hr/>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='აბზაცი<p></p>';ru='Абзац <p></p>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Блок для абзаца <div></div>';ru='Блок для абзаца <div></div>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Блок для текста внутри абзаца <span></span>';ru='Блок для текста внутри абзаца <span></span>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Жирный шрифт (<b></b>)';ru='Жирный шрифт (<b></b>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Курсивный шрифт (<i></i>)';ru='Курсивный шрифт (<i></i>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Подчеркнутый шрифт (<u></u>)';ru='Подчеркнутый шрифт (<u></u>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Зачеркнутый шрифт (<s></s>)';ru='Зачеркнутый шрифт (<s></s>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Верхний индекс (<sup></sup>)';ru='Верхний индекс (<sup></sup>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Нижний индекс (<sub></sub>)';ru='Нижний индекс (<sub></sub>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	Для С=1 По 6 Цикл
		...
		НоваяКоманда.Заголовок                 = СтрШаблон(
			НСтр("ka='Заголовок %1 уровня (<h%1></h%1>)';ru='Заголовок %1 уровня (<h%1></h%1>)'"),
			С);
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматАбзаца
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='გასწორება (გათანაბრება) მარცხვნივ';ru='Выравнивание влево'"); // <p style="text-align: left;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='გასწორება მარჯვნის';ru='Выравнивание вправо'"); // <p style="text-align: right;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='ცენტრზე გასწორება';ru='Выравнивание по центру'"); // <p style="text-align: center;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='სიგანეში გათანაბრება';ru='Выравнивание по ширине'"); // <p style="text-align: justify;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_СтилиТекста
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Длинная цитата (<blockquote></blockquote>)';ru='Длинная цитата (<blockquote></blockquote>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Короткая цитата (<q></q>)';ru='Короткая цитата (<q></q>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Источник цитаты (<cite></cite>)';ru='Источник цитаты (<cite></cite>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Ввод с клавиатуры (<kbd></kbd>)';ru='Ввод с клавиатуры (<kbd></kbd>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Программный код (<code></code>)';ru='Программный код (<code></code>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Акцентированный текст (<em></em>)';ru='Акцентированный текст (<em></em>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	ГруппаКоманд.Заголовок = НСтр("ka='Блоки и разрывы';ru='Блоки и разрывы'");
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ka='შრიფტის ფორმატი';ru='Формат шрифта'");
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	ГруппаКоманд.Заголовок = НСтр("ka='სათაურები';ru='Заголовки'");
...
#Область ФорматированиеHTML_СтилиТекста
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ka='ტექსტის სტილები';ru='Стили текста'");
...
Процедура СоздатьКомандыБуфераОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		НоваяКоманда.Заголовок                 = НСтр("ka='კოპირება';ru='Копировать'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("ka='გამოყოფილი სტრიქონების გაცვლის ბუფერში კოპირება';ru='Копировать выделенные строки в буфер обмена'");
		...
		НоваяКоманда.Заголовок                 = НСтр("ka='გაცვლის შენახული ბუფერების მართვა';ru='Управление сохраненными буферами обмена'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("ka='Управление сохраненными раньше буферами обмена';ru='Управление сохраненными раньше буферами обмена'");
		...
		НоваяКоманда.Заголовок                 = НСтр("ka='გაცვლის ბუფერის შემცველობის ჩასმა';ru='Вставить содержимое буфера обмена'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
Процедура СоздатьКомандыБуфераОбмена_СохраненныеБуферыОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Для каждого ТекущийЭлементСписка Из ЭтотОбъект["СохраненныеБуферыОбмена_" + ТекущийБуферОбмена] Цикл
			...
			НоваяКоманда.Действие                  = "КомандаБуферОбмена_Вставить";
			НоваяКоманда.Заголовок                 = СтрШаблон(
				НСтр("ka='Вставить из %1';ru='Вставить из %1'"),
...
Процедура СоздатьПодменюБуфераОбмена(Подменю, БуферОбмена)
	...
	ГруппаКомандВставить.Заголовок   = "Вставить";
		...
		ГруппаКомандВставитьСохраненный.Заголовок = НСтр("ka='შენახულის ჩადგმა';ru='Вставить сохраненный'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 2. Заменить в поле Объект.Важность число (0, 1, 2) на текст ("Обычная", "Очень важная", "Важная").
		УсловияОтбора = Новый Соответствие;
		УсловияОтбора.Вставить(0, НСтр("ka='ჩვეულებრივი';ru='Обычная'"));
		УсловияОтбора.Вставить(1, НСтр("ka='ძალიან მნიშვნელოვანი';ru='Очень важная'"));
		УсловияОтбора.Вставить(2, НСтр("ka='მნიშვნელოვანი';ru='Важная'"));
	...
	// 8. Поле "Единственный параметр".
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='რამდენიმე პარამეტრის გამართვა ხდება ცხრილში [Параметры]';ru='Несколько параметров настраиваются в таблице [Параметры]'"));
		Элемент.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);
...
"'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='სტრიქონების გადაყვანის სტატისტიკა:';ru='Статистика по переводам строк:'"),
		,
		ТекстПояснения,
			...
			Если СвойствоСписокКатегорийПростых <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияПростаяХДТО Из СписокКатегорийПростыхХДТО Цикл
					...
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("ka='Категория (простая) с УИН %УИН% и кодом %Код% не найдена.';ru='Категория (простая) с УИН %УИН% и кодом %Код% не найдена.'");
		...
		Если СвойствоКатегорииИнтервалыВерсий <> Неопределено Тогда
			...
			Если СвойствоСписокКатегорийИнтервалыВерсий <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияИнтервалыВерсийХДТО Из СписокКатегорийПростыхХДТО Цикл
					...
					НайденнаяКатегория = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду(КодКатегории);
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("ka='Категория (интервал версий) с УИН %УИН% и кодом %Код% не найдена.';ru='Категория (интервал версий) с УИН %УИН% и кодом %Код% не найдена.'");
...
Функция ПолучитьПредставлениеНовостиXML()
	...
	Если ПустаяСтрока(ЗаголовокНовостиЛокальный) Тогда
		ЗаголовокНовостиЛокальный = НСтр("ka='სათაური სიახლეები';ru='Заголовок новости'");
	КонецЕсли;
