...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ka='(все время указано для часового пояса %1 (%2))';ru='(все время указано для часового пояса %1 (%2))';en='(All times are given 1% (2%) for the time zone)'"),
		ЧасовойПояс(),
	...
	Если (ЭтотОбъект.БезАвторизации = Истина)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ka='ამ ახალი ამბის რედაქტირებისთვის, საჭიროა ახალი ამბების ცენტრში ავტორიზაციის გავლა.';ru='Для редактирования этой новости необходимо авторизоваться в новостном центре.';en='To edit the news you need to log in to the news center.'");
	...
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ka='ახალი ამბავი წარმატებით გამოქვეყნდა. მის შესაცვლელად, თავდაპირველად საჭიროა პუბლიკაციის გაუქმება.';ru='Новость была успешно опубликована. Чтобы изменить ее, необходимо вначале отменить публикацию.';en='The news has been successfully published. To change it, you must first unpublish.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
			И (ЭтотОбъект.СтатусНовостиНаСервере = 3) Тогда // Есть на сервере, опубликована, нельзя повторно публиковать.
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ka='ახალი ამბავი წარმატებით გამოქვეყნდა. ვინაიდან ახალი ამბავი გამოქვეყნდა არხებში, რომლებზეც აკრძალულია ხელახლა გამოქვეყნება (იმელი, სმს), მისი შეცვლა უკვე არ შეიძლება, მაგრამ შეიძლება ამ ახალი ამბის კოპირება და მისი გამართვა.';ru='Новость была успешно опубликована. Так как новость была опубликована в каналах, по которым запрещена перепубликация (емейл, смс), то изменить ее уже нельзя, но можно скопировать эту новость и править уже ее.';en='The news has been successfully published. Since the news was published in the channels, which prohibited sharing on (email, SMS), then change it is already impossible, but you can copy and edit the news to her already.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	КонецЕсли;
...
"или роль РедактированиеНовостей (вместе с настройкой лент новостей, в которых можно редактировать новости).';ka='ახალი ამბების რედაქტირების უფლებები არ არის."
"ჩართული უნდა იქნას როლები სისტემისადმინისტრატორი და სრულიუფლებები (მაშინ, შესაძლებელი იქნება ახალი ამბების ყველა ლოკალური ლენტის რედაქტირება) ან როლი ახალიამბებისრედაქტირება (ახალი ამბების ლენტების გამართვასთან ერთად, რომლებშიც შეიძლება ახალი ამბების რედაქტირება).'");
		...
		Если МассивРазрешенныхДляРедактированияЛентНовостей.Количество() = 0 Тогда
			ЭтотОбъект.ТолькоПросмотр = Истина;
			ТекстСообщения = НСтр("ka='რედაქტირებისთვის ნებადართული ახალი ამბების არცერთი ლენტი არ მოიძებნა."
"ახალი ამბის რედაქტირება შეუძლებელია.';ru='Не найдено ни одной разрешенной для редактирования ленты новостей."
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для С=0 По Объект.Действия.Количество()-1 Цикл
		...
		Если НайденныеПовторения.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='მოქმედების იდენტიფიკატორი %УИНДействия% მეორდება %КоличествоПовторений%';ru='Идентификатор действия %УИНДействия% повторяется %КоличествоПовторений%';en='Identifier action UINDeystviya%%% repeated KolichestvoPovtoreny%'");
		...
		Если Лев(ВРег(ТекущаяСтрока.УИНДействия), "3") <> ВРег("1C:") Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='მოქმედების იდენტიფიკატორი უნდა იწყებოდეს 1C-ით:';ru='Идентификатор действия должен начинаться с 1C:';en='ID of the action should begin with 1C:'");
			Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		КонецЕсли;
		...
		Для ТекущийСимвол=4 По СтрДлина(ТекущаяСтрока.УИНДействия) Цикл
			...
			Если (Найти(РазрешенныеСимволы, Символ) = 0) Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='მოქმედების იდენტიფიკატორი უნდა იწყებოდეს 1C-ით: და უნდა შედგებოდეს მხოლოდ ლათინური სიმბოლოებისგან, ციფრებისგან და ხაზგასმის ნიშნისგან';ru='Идентификатор действия должен начинаться с 1C: и состоять только из английских символов, цифр и знака подчеркивания';en='ID of the action should begin with 1C: and contain only English letters, numbers and underscores'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЦикла;
		...
		Если ПустаяСтрока(ТекущаяСтрока.Действие) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='ცარიელი ქმედება';ru='Пустое действие';en='empty action'");
		...
		Иначе
			...
			Иначе
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='არასწორი მოქმედება. აირჩიეთ მოქმედების მნიშვნელობა სიიდან';ru='Неправильное действие. Выберите значение действия из списка';en='Invalid operation. Select from the list of actions'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].Действие";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
	...
	Для С=0 По Объект.ПараметрыДействий.Количество()-1 Цикл
		...
		Если НайденныеВладельцы.Количество() = 0 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='პარამეტრი არცერთ მოქმედებაზე არ არის მიბმული (უნდა იყოს თუნდ ერთი მოქმედება [%УИНДействия%]-ის ასეთივე იდენტიფიკატორით). რეკომენდებულია სტრიქონის წაშლა.';ru='Параметр не привязан ни к одному действию (должно быть хотя бы одно действие с таким же идентификатором [%УИНДействия%]). Рекомендуется удалить строку.';en='Parameter is not tied to any action (must be at least one action with the same identifier [% UINDeystviya%]). It is recommended to remove the line.'");
			Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%УИНДействия%", ТекущаяСтрока.УИНДействия);
		...
		ИначеЕсли НайденныеВладельцы.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='პარამეტრს მხოლოდ ერთი მფლობელი (ერთი მოქმედება) უნდა ჰყავდეს';ru='У параметра должен быть только один владелец (одно действие)';en='In parameter must be only one owner (one action)'");
			Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		Иначе // =1
			...
			Если ТекущийВладелец.Действие = "Переход по навигационной ссылке" Тогда
				...
				Иначе
					Отказ = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ka='მოქმედება [%Действие%]-ს არასწორი პარამეტრი [%Параметр%] აქვს. აირჩიეთ მნიშვნელობა სიიდან';ru='У действия [%Действие%] параметр [%Параметр%] неправильный. Выберите значение из списка';en='In action [% Action%] parameter [% Parameter%] wrong. Select from the list'");
			...
			ИначеЕсли ТекущийВладелец.Действие = "Показать картинку" Тогда
				Если ТекущаяСтрока.Параметр = "УИН" Тогда
					...
					Если НайденныеБинарныеДанные.Количество() = 0 Тогда
						Отказ = Истина;
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = НСтр("ka='მოქმედება [%Действие%]-ის პარამეტრი [%Параметр%] მიუთითებს არარსებულ ბინარულ მონაცემებზე. აირჩიეთ ბინარულ მონაცემთა სწორი მნიშვნელობა.';ru='У действия [%Действие%] параметр [%Параметр%] указывает на несуществующие бинарные данные. Выберите правильное значение бинарных данных.';en='In action [% Action%] parameter [% Parameter%] indicates a non-existent binary data. Select the correct value of binary data.'");
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Действие%", ТекущийВладелец.Действие);
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Параметр%", ТекущаяСтрока.Параметр);
						Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].Параметр";
						Сообщение.Сообщить();
...
Процедура ПримерНовостиДля1СПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ПродолжитьОбработку = Истина Тогда
		...
		Иначе
			СтандартнаяОбработка = Ложь;
			ТекстСообщения = НСтр("ka='უცნობი ბმული: %НавигационнаяСсылка%';ru='Неизвестная ссылка: %НавигационнаяСсылка%';en='Unknown link:% NavigatsionnayaSsylka%'");
	...
	Если ОтменаРедактирования <> Истина Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.ВерсияОТ > ТекущиеДанные.ВерсияДО Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='ვერსია -დან [ВерсияОТ] არ უნდა იყოს უფრო მეტი, ვიდრე ვერსია -მდე [ВерсияДО].';ru='Версия ОТ [ВерсияОТ] не должна быть больше, чем Версия ДО [ВерсияДО].';en='Versions [VersiyaOT] should not be greater than the version to [VersiyaDO].'");
...
Процедура ДействияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		...
		Если Отказ = Истина Тогда
			ТекстОшибки = НСтр("ka='ჰიპერბმულის იდენტიფიკატორი უნდა იწყებოდეს 1C:-ით (სადაც C არის ინგლისური ასო-ბგერა),"
"და შედგებოდეს მხოლოდ ინგლისური ასო-ბგერებისგან, ციფრებისგან და ხაზგასმის ნიშნებისგან."
""
"ხოლო ველ ""მოქმედება""-ში უნდა იყოს სიიდან არჩეული მნიშვნელობა.';ru='Идентификатор гиперссылки должен начинаться с 1C: (где C - английская буква),"
...
"А в поле Действие должно быть значение, выбранное из списка.'");
			ПоказатьПредупреждение(
				...
				0, // Таймаут
				НСтр("ka='შეცდომა';ru='Ошибка';en='Mistake'")); // Заголовок
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаЗагрузитьФайлКакБинарныеДанные(Команда)
	...
	Иначе
		ТекстОшибки = НСтр("ka='აირჩიეთ სტრიქონი, სადაც უნდა მოთავსდეს მონაცემები';ru='Выберите строку, куда помещать данные';en='Select the row where to place data'");
		ПоказатьПредупреждение(
			, // ОписаниеоповещенияОЗавершении
			ТекстОшибки, // ТекстПредупреждения
			0, // Таймаут
...
Процедура КомандаБуферОбмена_Копировать(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Копировать_КатегорииПростые")
		...
		Если ЭлементФормы.ВыделенныеСтроки.Количество() > 0 Тогда
			...
			КонецЦикла;
			ТекстСообщения = НСтр("ka='კოპირებულია სტრიქონი: %ТекущийБуферОбмена.Количество()%';ru='Скопировано строк: %ТекущийБуферОбмена.Количество()%';en='Selected rows:% TekuschiyBuferObmena.Kolichestvo ()%'");
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ka='გამოჰყავით თუნდ ერთი სტრიქონი';ru='Выделите хотя бы одну строку';en='Select at least one row'"), , , БиблиотекаКартинок.Внимание);
		КонецЕсли;
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Вставить_ТекущийБуферОбмена_КатегорииПростые")
		...
		Если ДанныеБуфераОбмена.Количество() > 0 Тогда
			Для каждого ТекущаяСкопированнаяСтрока Из ДанныеБуфераОбмена Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ka='ჩასმულია სტრიქონი: %ДанныеБуфераОбмена.Количество()%';ru='Вставлено строк: %ДанныеБуфераОбмена.Количество()%';en='Inserted rows:% DannyeBuferaObmena.Kolichestvo ()%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеБуфераОбмена.Количество()%", ДанныеБуфераОбмена.Количество());
			ПоказатьОповещениеПользователя(ТекстСообщения, , , БиблиотекаКартинок.Информация);
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ka='გაცვლის ბუფერში მონაცემები არ არის.';ru='Нет данных в буфере обмена.';en='No data on the clipboard.'"), , НСтр("ka='თავდაპირველად, მოახდინეთ სტრიქონების კოპირება.';ru='Вначале скопируйте строки.';en='Initially, copy the string.'"), БиблиотекаКартинок.Внимание);
		КонецЕсли;
...
"Для проверки новости необходимо открыть форму вручную или исправить значения в колонках.';ka='Форма %ИмяФормыПроверкиНовости% не обнаружена."
"Возможно, для описания формы использованы не имена в метаданных, а другой текст,"
"	либо неправильно введены идентификаторы метаданных и / или формы."
"Для проверки новости необходимо открыть форму вручную или исправить значения в колонках.'");
			...
			ПоказатьПредупреждение(
				...
				ТекстОшибки, // ТекстПредупреждения
				0, // Таймаут
				НСтр("ka='გაფრთხილება';ru='Предупреждение';en='A warning'")); // Заголовок
		КонецПопытки;
	...
	Если Найти(ВРег(Команда.Имя), ВРег(ШаблонИмениКоманды)) = 1 Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если ПустаяСтрока(ПолеТекстовогоДокумента.ВыделенныйТекст) Тогда
				ТекстГиперссылки = НСтр("ka='დააჭირეთ აქ';ru='Нажмите здесь';en='Click here'");
			Иначе
...
Процедура КомандаИмпортЭкспортXML(Команда)
	...
	ИначеЕсли Команда.Имя = "КомандаИмпортИзXML" Тогда
		Если Объект.ЛентаНовостей.Пустая() Тогда
			ТекстСообщения = НСтр("ka='Необходимо заполнить ленту новостей.';ru='Необходимо заполнить ленту новостей.';en='It is necessary to fill a news feed.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
...
"%Предупреждение%';ka='ყველა ბინარული მონაცემის ზომა: %РазмерБинарныхДанных% ბაიტი"
"ახალი ამბის მიახლოებითი ზომა (ახალი ამბის ტექსტის ჩათვლით): %ОбщийРазмер% ბაიტი"
"%Предупреждение%'");
	...
	Если Объект.БинарныеДанные.Итог("ДанныеРазмер") > 50000 Тогда
		лкТекст = СтрЗаменить(лкТекст, "%Предупреждение%", НСтр("ka='დიდი ზომის ახალმა ამბებმა შეიძლება შეანელოს პროგრამის მუშაობა 1С-ის პლატფრომაზე';ru='Новости большого размера могут замедлить работу программы на платформе 1С';en='large news may slow the program work on the platform 1C'"));
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(СписокТабличныхЧастей = Неопределено)
	...
	Если МассивТабличныхЧастей.Найти("БинарныеДанные") <> Неопределено Тогда
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='ბინარული მონაცემების ჩატვირთვა ვერ მოხერხდა (ფორმაში და ობიექტში სტრიქონების რაოდენობა არ ემთხვევა). შენახვისას, ისინი დაიკარგება. რეკომენდებულია შენახვის გარეშე ახალი ამბების დახურვა / გახსნა';ru='Не удалось загрузить бинарные данные (не совпадает количество строк в форме и в объекте). При сохранении они будут утеряны. Рекомендуется закрыть / открыть форму новости без сохранения';en='Unable to load binary data (number of rows is not the same in shape and in the object). When you save will be lost. It is recommended to close / open the news without saving'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеКатегорииДляЛентыНовостейСервер()
	...
	Если МассивКатегорий_Простые.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("ka='მარტივი კატეგორიები';ru='Простые категории';en='simple search'");
	Иначе
	...
	Если МассивКатегорий_ИнтервалыВерсий.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("ka='ვერსიებზე მიბმა';ru='Привязка к версиям';en='Binding to versions'");
	Иначе
	...
	Если ТипЗнч(РезультатЗакрытия) = ТипСтруктура Тогда
		...
		Если ВРег(РезультатЗакрытия.Действие) = ВРег("Переход по интернет ссылке") Тогда
			...
			Если ПустаяСтрока(лкВыделенныйТекст) Тогда
				лкВыделенныйТекст = НСтр("ka='დააჭირეთ აქ';ru='Нажмите здесь';en='Click here'");
			КонецЕсли;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	НоваяКоманда.Заголовок                 = НСтр("ka='Разрыв строки <br/>';ru='Разрыв строки <br/>';en='line break <br/>'");
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Горизонтальная линия <hr/>';ru='Горизонтальная линия <hr/>';en='The horizontal line <hr />'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='აბზაცი<p></p>';ru='Абзац <p></p>';en='Paragraph <p> </ p>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Блок для абзаца <div></div>';ru='Блок для абзаца <div></div>';en='Block for a paragraph <div> </ div>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Блок для текста внутри абзаца <span></span>';ru='Блок для текста внутри абзаца <span></span>';en='Block for the text within a paragraph <span> </ span>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Жирный шрифт (<b></b>)';ru='Жирный шрифт (<b></b>)';en='Bold (<b> </ b>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Курсивный шрифт (<i></i>)';ru='Курсивный шрифт (<i></i>)';en='Italics (<i> </ i>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Подчеркнутый шрифт (<u></u>)';ru='Подчеркнутый шрифт (<u></u>)';en='Underline font (<u> </ u>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Зачеркнутый шрифт (<s></s>)';ru='Зачеркнутый шрифт (<s></s>)';en='Strikethrough font (<s> </ s>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Верхний индекс (<sup></sup>)';ru='Верхний индекс (<sup></sup>)';en='Superscript (<sup> </ sup>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Нижний индекс (<sub></sub>)';ru='Нижний индекс (<sub></sub>)';en='The subscript (<sub> </ sub>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	Для С=1 По 6 Цикл
		...
		НоваяКоманда.Заголовок                 = СтрШаблон(
			НСтр("ka='Заголовок %1 уровня (<h%1></h%1>)';ru='Заголовок %1 уровня (<h%1></h%1>)';en='Header 1% level (<h% 1> </ h% 1>)'"),
			С);
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматАбзаца
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='გასწორება (გათანაბრება) მარცხვნივ';ru='Выравнивание влево';en='Flushleft'"); // <p style="text-align: left;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='გასწორება მარჯვნის';ru='Выравнивание вправо';en='right justified'"); // <p style="text-align: right;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='ცენტრზე გასწორება';ru='Выравнивание по центру';en='Centered'"); // <p style="text-align: center;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='სიგანეში გათანაბრება';ru='Выравнивание по ширине';en='Width adjustment'"); // <p style="text-align: justify;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_СтилиТекста
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Длинная цитата (<blockquote></blockquote>)';ru='Длинная цитата (<blockquote></blockquote>)';en='Long quotation (<blockquote> </ blockquote>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Короткая цитата (<q></q>)';ru='Короткая цитата (<q></q>)';en='A short quote (<q> </ q>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Источник цитаты (<cite></cite>)';ru='Источник цитаты (<cite></cite>)';en='Source citations (<cite> </ cite>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Ввод с клавиатуры (<kbd></kbd>)';ru='Ввод с клавиатуры (<kbd></kbd>)';en='Keyboard Input (<kbd> </ kbd>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Программный код (<code></code>)';ru='Программный код (<code></code>)';en='The code (<code> </ code>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ka='Акцентированный текст (<em></em>)';ru='Акцентированный текст (<em></em>)';en='Accented text (<em> </ em>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	ГруппаКоманд.Заголовок = НСтр("ka='Блоки и разрывы';ru='Блоки и разрывы';en='Blocks and breaks'");
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ka='შრიფტის ფორმატი';ru='Формат шрифта';en='format font'");
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	ГруппаКоманд.Заголовок = НСтр("ka='სათაურები';ru='Заголовки';en='headlines'");
...
#Область ФорматированиеHTML_СтилиТекста
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ka='ტექსტის სტილები';ru='Стили текста';en='text styles'");
...
Процедура СоздатьКомандыБуфераОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		НоваяКоманда.Заголовок                 = НСтр("ka='კოპირება';ru='Копировать';en='Copy'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("ka='გამოყოფილი სტრიქონების გაცვლის ბუფერში კოპირება';ru='Копировать выделенные строки в буфер обмена';en='Copy selected rows to clipboard'");
		...
		НоваяКоманда.Заголовок                 = НСтр("ka='გაცვლის შენახული ბუფერების მართვა';ru='Управление сохраненными буферами обмена';en='Manage saved clipboard'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("ka='Управление сохраненными раньше буферами обмена';ru='Управление сохраненными раньше буферами обмена';en='Manage saved before clipboard'");
		...
		НоваяКоманда.Заголовок                 = НСтр("ka='გაცვლის ბუფერის შემცველობის ჩასმა';ru='Вставить содержимое буфера обмена';en='Paste clipboard'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
Процедура СоздатьКомандыБуфераОбмена_СохраненныеБуферыОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Для каждого ТекущийЭлементСписка Из ЭтотОбъект["СохраненныеБуферыОбмена_" + ТекущийБуферОбмена] Цикл
			...
			НоваяКоманда.Действие                  = "КомандаБуферОбмена_Вставить";
			НоваяКоманда.Заголовок                 = СтрШаблон(
				НСтр("ka='Вставить из %1';ru='Вставить из %1';en='Paste from 1%'"),
...
Процедура СоздатьПодменюБуфераОбмена(Подменю, БуферОбмена)
	...
	ГруппаКомандВставить.Заголовок   = "Вставить";
		...
		ГруппаКомандВставитьСохраненный.Заголовок = НСтр("ka='შენახულის ჩადგმა';ru='Вставить сохраненный';en='insert the saved'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 2. Заменить в поле Объект.Важность число (0, 1, 2) на текст ("Обычная", "Очень важная", "Важная").
		УсловияОтбора = Новый Соответствие;
		УсловияОтбора.Вставить(0, НСтр("ka='ჩვეულებრივი';ru='Обычная';en='usual'"));
		УсловияОтбора.Вставить(1, НСтр("ka='ძალიან მნიშვნელოვანი';ru='Очень важная';en='Very important'"));
		УсловияОтбора.Вставить(2, НСтр("ka='მნიშვნელოვანი';ru='Важная';en='important'"));
	...
	// 8. Поле "Единственный параметр".
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='რამდენიმე პარამეტრის გამართვა ხდება ცხრილში [Параметры]';ru='Несколько параметров настраиваются в таблице [Параметры]';en='Several parameters are set in the table [options]'"));
		Элемент.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);
...
"';ka='Текст для внешних источников:"
" - 10 = %ТекстВИ_10_Э% / %ТекстВИ_10_О%,"
" - 13 = %ТекстВИ_13_Э% / %ТекстВИ_13_О%,"
"Текст для 1С:"
" - 10 = %Текст1С_10_Э% / %Текст1С_10_О%,"
...
"'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='სტრიქონების გადაყვანის სტატისტიკა:';ru='Статистика по переводам строк:';en='Statistics on translation strings:'"),
		,
		ТекстПояснения,
			...
			Если СвойствоСписокКатегорийПростых <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияПростаяХДТО Из СписокКатегорийПростыхХДТО Цикл
					...
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("ka='Категория (простая) с УИН %УИН% и кодом %Код% не найдена.';ru='Категория (простая) с УИН %УИН% и кодом %Код% не найдена.';en='Category (simple) with UIN UIN%% code and% code% not found.'");
		...
		Если СвойствоКатегорииИнтервалыВерсий <> Неопределено Тогда
			...
			Если СвойствоСписокКатегорийИнтервалыВерсий <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияИнтервалыВерсийХДТО Из СписокКатегорийПростыхХДТО Цикл
					...
					НайденнаяКатегория = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду(КодКатегории);
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("ka='Категория (интервал версий) с УИН %УИН% и кодом %Код% не найдена.';ru='Категория (интервал версий) с УИН %УИН% и кодом %Код% не найдена.';en='Category (range version) with UIN UIN%% code and% code% not found.'");
...
Функция ПолучитьПредставлениеНовостиXML()
	...
	Если ПустаяСтрока(ЗаголовокНовостиЛокальный) Тогда
		ЗаголовокНовостиЛокальный = НСтр("ka='სათაური სიახლეები';ru='Заголовок новости';en='news Title'");
	КонецЕсли;
