...
Процедура КомандаПоиск(Команда)
	...
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		...
		Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
			...
			Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
				ТекущаяПапка.Представление = НСтр("ka='ძებნის შედეგები';ru='Результаты поиска';en='searching results'");
			Иначе
				ТекущаяПапка.Представление = НСтр("ka='ძებნის შედეგები';ru='Результаты поиска';en='searching results'") + " (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
			КонецЕсли;
...
"%2"
"%3';ka='Последнее обновление списка новостей было"
"%1"
"Следующее обновление списка новостей произойдет"
"%2"
"%3'"),
...
Процедура ОбновитьИнформационныеСтрокиСервер()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ТекстСообщения = НСтр("ka='%Выборка.ЛентаНовостейНаименование%, უკანასკნელი ახალი ამბავი %Выборка.ДатаПоследнейНовости%-ის, უკანასკნელი ჩატვირთვა %Выборка.ДатаПоследнейЗагрузки%';ru='%Выборка.ЛентаНовостейНаименование%, последняя новость от %Выборка.ДатаПоследнейНовости%, последняя загрузка %Выборка.ДатаПоследнейЗагрузки%';en='% Vyborka.LentaNovosteyNaimenovanie%, the latest news from% Vyborka.DataPosledneyNovosti% last loading% Vyborka.DataPosledneyZagruzki%'");
	...
	Если СтрДлина(ЭтотОбъект.ДатыПоследнихЗагрузокНовостей) > 0 Тогда
		ЭтотОбъект.ДатыПоследнихЗагрузокНовостей =
			СтрШаблон(
				НСтр("ru='"
"Состояние загрузки новостей: %1';ka='"
...
"Можно использовать символы подстановки * и ?';ka='Полнотекстовый поиск включен."
"Можно использовать символы подстановки * и ?'");
	Иначе
		Элементы.СтрокаПоиска.Подсказка = НСтр("ru='Полнотекстовый поиск отключен."
"Доступен поиск только точных фраз';ka='Полнотекстовый поиск отключен."
...
Процедура ОбновитьСписокПапокОтбора()
	...
	НоваяСтрока.Наименование           = НСтр("ka='ყველა ახალი ამბავი';ru='Все новости';en='All news'");
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
		НоваяСтрока.Наименование = НСтр("ka='ძებნის შედეგები';ru='Результаты поиска';en='searching results'");
	Иначе
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ka='მონიშნული სიახლეები';ru='Помеченные новости';en='Noted news'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ka='მნიშვნელოვანი ახალი ამბები';ru='Важные новости';en='Important news'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ka='კალათა';ru='Корзина';en='Basket'");
