...
Процедура КомандаПоиск(Команда)
	...
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		...
		Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
			...
			Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
				ТекущаяПапка.Представление = НСтр("ka='ძებნის შედეგები';ru='Результаты поиска'");
			Иначе
				ТекущаяПапка.Представление = НСтр("ka='ძებნის შედეგები';ru='Результаты поиска'") + " (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
			КонецЕсли;
...
Процедура ОбновитьСписокПапокОтбора()
	...
	НоваяСтрока.Наименование           = НСтр("ka='ყველა ახალი ამბავი';ru='Все новости'");
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
		НоваяСтрока.Наименование = НСтр("ka='ძებნის შედეგები';ru='Результаты поиска'");
	Иначе
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ka='მონიშნული სიახლეები';ru='Помеченные новости'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ka='მნიშვნელოვანი ახალი ამბები';ru='Важные новости'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ka='კალათა';ru='Корзина'");
