...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПустаяСтрока(ЭтаФорма.Действие) Тогда
		...
		Сообщение.Текст = НСтр("ka='ველი ""მოქმედება"" არ არის შევსებული';ru='Не заполнено поле Действие'");
	...
	Если ПустаяСтрока(ЭтаФорма.УИНДействия) Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ჰიპერბმულის იდენტიფიკატორი არ არის შევსებული (შევსება უნდა მოხდეს ავტომატურად, ""მოქმედება""-ს არჩევის შემდეგ)';ru='Не заполнен идентификатор гиперссылки (должен заполниться автоматически после выбора Действия)'");
	...
	Если ЭтаФорма.Действие = "Переход по навигационной ссылке" Тогда
		Если ПустаяСтрока(ЭтаФорма.ПараметрДействия_НавигационнаяСсылка) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='სანავიგაციო ბმული არ არის შევსებული';ru='Не заполнена навигационная ссылка'");
	...
	ИначеЕсли ЭтаФорма.Действие = "Открытие раздела справки" Тогда
		Если ПустаяСтрока(ЭтаФорма.ПараметрДействия_РазделСправки) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='ნაწილი ცნობები არ არის შევსებული';ru='Не заполнен раздел справки'");
	...
	ИначеЕсли ЭтаФорма.Действие = "Переход по интернет ссылке" Тогда
		Если ПустаяСтрока(ЭтаФорма.ПараметрДействия_ИнтернетСсылка) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='ინტერნეტ-ბმული არ არის შევსებული';ru='Не заполнена интернет-ссылка'");
	...
	ИначеЕсли ЭтаФорма.Действие = "Показать картинку" Тогда
		Если ПустаяСтрока(ЭтаФорма.КартинкаЗаголовок) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='სურათის სათაური არ არის შევსებული';ru='Не заполнен заголовок картинки'");
		...
		Если ПустаяСтрока(ЭтаФорма.КартинкаДанные) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='სურათი არ არის ჩატვირთული. დააჭირეთ <Выбрать картинку из файла>-ს';ru='Не загружена картинка. Нажмите <Выбрать картинку из файла>'");
			Сообщение.Поле  = "КартинкаЗаголовок";
			Сообщение.Сообщить();
		КонецЕсли;
