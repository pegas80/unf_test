...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.Владелец.Заголовок	= НСтр("ka='ორგანიზაცია';ru='Организация';en='Organization'");
	...
	Если ЭтоФизическоеЛицо Тогда
		...
		Элементы.ОтчетностьПодписываетПредставитель.СписокВыбора[0].Представление = НСтр("ka='ინდივიდუალური მეწარმე';ru='Индивидуальный предприниматель';en='Individual entrepreneur'");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтчетностьПодписываетПредставитель = 1
		...
		ТекстСообщения = НСтр("ka='შეავსეთ ინფორმაცია წარმომადგენლის შესახებ';ru='Заполните сведения о представителе';en='Fill in the information about the representative'"); 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СуществующаяЗапись <> Неопределено И СуществующаяЗапись <> Объект.Ссылка Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Для данной организации уже существует запись с указанными кодом налогового органа';ru='Для данной организации уже существует запись с указанными кодом налогового органа';en='For the organization is already an entry with the specified tax authority code'") 
						         + ?(СтрДлина(Объект.КПП) < 9, ".", НСтр("ka=' и КПП.';ru=' и КПП.'")));
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Объект.НаименованиеИФНС) Тогда
		...
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ka='МИФНС';ru='МИФНС';en='MIFNS'"),	НСтр("ka='საგადასახადო სამსახური რაიონული ინსპექცია';ru='Межрайонная инспекция федеральной налоговой службы';en='Interdistrict Inspectorate of the Federal Tax Service'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ka='ИФНС';ru='ИФНС';en='IFTS'"),	НСтр("ka='საგადასახადო სამსახურის ინსპექცია';ru='Инспекция федеральной налоговой службы';en='Inspectorate of the Federal Tax Service'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ka='ФНС';ru='ФНС';en='FTS'"),	НСтр("ka='საგადასახადო სამსახური';ru='Федеральная налоговая служба';en='The Federal Tax Service'"));
...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Представитель) Тогда
		Форма.ПредставлениеПредставителя = НСтр("ka='შევსება';ru='Заполнить';en='Insert'");
...
Процедура ВладелецПриИзменении(Элемент)
	...
	Иначе	
		...
		Элементы.ОтчетностьПодписываетПредставитель.СписокВыбора[0].Представление = НСтр("ka='ხელმძღვანელი';ru='Руководитель';en='head'");
