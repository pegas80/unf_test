...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.Владелец.Заголовок	= НСтр("ka='ორგანიზაცია';ru='Организация'");
	...
	Если ЭтоФизическоеЛицо Тогда
		...
		Элементы.ОтчетностьПодписываетПредставитель.СписокВыбора[0].Представление = НСтр("ka='ინდივიდუალური მეწარმე';ru='Индивидуальный предприниматель'");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтчетностьПодписываетПредставитель = 1
		...
		ТекстСообщения = НСтр("ka='შეავსეთ ინფორმაცია წარმომადგენლის შესახებ';ru='Заполните сведения о представителе'"); 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СуществующаяЗапись <> Неопределено И СуществующаяЗапись <> Объект.Ссылка Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Для данной организации уже существует запись с указанными кодом налогового органа';ru='Для данной организации уже существует запись с указанными кодом налогового органа'") 
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Объект.НаименованиеИФНС) Тогда
		...
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ka='МИФНС';ru='МИФНС'"),	НСтр("ka='საგადასახადო სამსახური რაიონული ინსპექცია';ru='Межрайонная инспекция федеральной налоговой службы'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ka='ИФНС';ru='ИФНС'"),	НСтр("ka='საგადასახადო სამსახურის ინსპექცია';ru='Инспекция федеральной налоговой службы'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ka='ФНС';ru='ФНС'"),	НСтр("ka='საგადასახადო სამსახური';ru='Федеральная налоговая служба'"));
...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Представитель) Тогда
		Форма.ПредставлениеПредставителя = НСтр("ka='შევსება';ru='Заполнить'");
...
Процедура ВладелецПриИзменении(Элемент)
	...
	Иначе	
		...
		Элементы.ОтчетностьПодписываетПредставитель.СписокВыбора[0].Представление = НСтр("ka='ხელმძღვანელი';ru='Руководитель'");
