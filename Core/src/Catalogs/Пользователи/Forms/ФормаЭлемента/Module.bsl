...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВходВПрограммуРазрешен Тогда
		ЗаголовокВопросов = НСтр("ka='საინფორმაციო ბაზის მომხმარებლის ჩანაწერი';ru='Запись пользователя информационной базы'");
		   ...
		   И ПользовательИБРоли.Количество() = 0 Тогда
			...
			Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
				...
				ПоказатьВопрос(
					...
					НСтр("ka='საინფორმაციო ბაზის მომხმარებლის მიერ აყენებული არ არის არცერთი როლი. გავაგრძელოთ?';ru='Пользователю информационной базы не установлено ни одной роли. Продолжить?'"),
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		...
		Если ПараметрыЗаписи.Свойство("ССозданиемПервогоАдминистратора") Тогда
			ТекущийОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("ka='საინფორმაციო ბაზის პირველი მომხმარებლის დანიშვნას ახორციელებს ადმინისტრატორი.';ru='Первый пользователь информационной базы назначается администратором.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ВходВПрограммуРазрешен
	   ...
	   И СрокДействия <= НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='შეზღუდვა უნდა იყოს ხვალამდე ან უფრო მეტი.';ru='Ограничение должно быть до завтра или более.'"),, "ВходВПрограммуРазрешен",, Отказ);
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='როლი ""%1"" მეტამონაცემებში ვერ მოიძებნა.';ru='Роль ""%1"" не найдена в метаданных.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში როლი ""%1"" მეტამონაცემებში ვერ მოიძებნა.';ru='Роль ""%1"" в строке %%1 не найдена в метаданных.'"), Строка.Синоним));
			КонецЕсли;
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='როლი ""%1"" მომხმარებლებისთვის მიუწვდომელია.';ru='Роль ""%1"" недоступна для пользователей.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში არსებული როლი ""%1"" მომხმარებლებისთვის მიუწვდომელია.';ru='Роль ""%1"" в строке %%1 недоступна для пользователей.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
"пометку на удаление с этого пользователя.';ka='პროგრამაში შესვლის ნებართვისთვის, საჭირო არის ამ მომხმარებლისგან წაშლის ნიშნულის მოხსნა.'"));
	...
	Если Не УровеньДоступа.ИзменениеРазрешенияНаВход
	   И Не ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ka='ჩაწერის შემდეგ პროგრამაში შესვლის ნებართვას იძლევა მხოლოდ ადმინისტრატორი';ru='После записи вход в программу сможет разрешить только администратор.'"));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='ПолныеПрава';ru='ПолныеПрава'");
...
Процедура ПроверитьЗаполнениеEmail(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ka='ელექტრონული ფოსტის მისამართი არ არის შევსებული';ru='Не заполнен адрес электронной почты'"),,
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБПолноеИмя <> Объект.Наименование Тогда
			...
			УточнениеСвойств.Вставить(0, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სრული სახელი ""%1""';ru='Полное имя ""%1""'"),
		...
		Если УточнениеСвойств.Количество() > 0 Тогда
			...
			Если ПоказатьКомандыУстраненияРазличий Тогда
				Рекомендация = Символы.ПС
					+ НСтр("ka='დააჭირეთ ""ჩაწერა""-ს სხვადასხვაობის აღმოსაფხვრელად და რომ ეს გაფრთხილება არ იქნას გამოტანილი.';ru='Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.'");
			
			ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
				Рекомендация = Символы.ПС
					+ НСтр("ka='სხვადასხვაობის აღმოსაფხვრელად მიმართეთ ადმინისტრატორს.';ru='Обратитесь к администратору, чтобы устранить различия.'");
			Иначе
				Рекомендация = "";
			КонецЕсли;
...
"%1.';ka='საინფორმაციო ბაზის მომხმარებლის შემდეგი თვისებები განსხვავდება ამ ფორმაში მითითებულისგან: %1.'"),
	...
	Если ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ka='პრობლემის აღმოსაფხვრელად და ამ გაფრთხილების არგამოსატანად დააჭირეთ ""ჩაწერა""-ს.';ru='Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.'");
	...
	Элементы.СопоставлениеПояснениеНесоответствия.Заголовок =
		НСтр("ka='საინფორმაციო ბაზის მომხმარებელი ვერ მოიძებნა.';ru='Пользователь информационной базы не найден.'") + Рекомендация;
...
Процедура ПраваРедактированияДокументовНажатие(Элемент)
	...
	Текст = "";
	Подсказка = НСтр("ka='შეიყვანეთ დაშვების პაროლი';ru='Введите ключ редактирования'");
