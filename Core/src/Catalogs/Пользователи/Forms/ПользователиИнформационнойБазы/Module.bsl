...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		ВызватьИсключение НСтр("ka='საინფორმაციო ბაზის მომხმარებელთა სიის გასახსნელად უფლებები არასაკმარისია.';ru='Недостаточно прав для открытия списка пользователей информационной базы.';en='Insufficient permissions to open a list of the information base of users.'");
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	Кнопки.Добавить("ОтменитьСопоставление", НСтр("ka='შედარების გაუქმება';ru='Отменить сопоставление';en='unmap'"));
	Кнопки.Добавить("ОставитьСопоставление", НСтр("ka='შედარების დატოვება';ru='Оставить сопоставление';en='leave a comparison'"));
...
"при обновлении информационной базы, поэтому не рекомендуется отменять сопоставление по любой другой причине.';ka='საინფორმაციო ბაზის მომხმარებლის ცნობარში არსებულ მომხმარებელთან შეპირისპირების გაუქმება."
""
"შეპირისპირების გაუქმება იშვიათად არის საჭირო - მხოლოდ მაშინ, თუ შეპირისპირება არაკორექტულად შესრულდა, მაგალითად, საინფორმაციო ბაზის განახლებისას, ამიტომ, რაიმე სხვა მიზეზის გამო შეპირისპირების გაუქმება არ არის რეკომენდებული.'"),
		Кнопки,
		,
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<Нет данных>';ru='<Нет данных>';en='<No data>'"));
...
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	...
	ПоказатьВопрос(
		...
		НСтр("ka='წავშალოთ საინფორმაციო ბაზის მომხმარებელი?';ru='Удалить пользователя информационной базы?';en='Remove user information base?'"),
