	...
	Если ВидСрока = "ДоДаты" Тогда
		Если Не ЗначениеЗаполнено(Срок) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='თარიღი არ არის მითითებული.';ru='Дата не указана.';en='Date not available.'"),, "Срок");
			Возврат;
			
		ИначеЕсли Срок <= НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='შეზღუდვა უნდა იყოს ხვალამდე ან უფრო მეტი.';ru='Ограничение должно быть до завтра или более.';en='Restriction should be up tomorrow or more.'"),, "Срок");
			Возврат;
		КонецЕсли;
	КонецЕсли;
