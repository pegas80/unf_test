...
Функция СсылкаПоДаннымКлассификатора(Знач Отбор, Знач ДополнительныеДанные = Неопределено) Экспорт
	...
	Если ДанныеПоиска=Неопределено Тогда
		ВызватьИсключение НСтр("ka='მსოფლიოს ქვეყნის არაკორექტული კოდი კლასიფიკატორში მოსაძებნად';ru='Некорректный код страны мира для поиска в классификаторе';en='Incorrect code countries around the world to search in the classifier'");
	КонецЕсли;
