...
Процедура ИскатьСтрокуВТаблице(ИскатьВперед)
	...
	Если ПустаяСтрока(СтрокаПоиска) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='ძებნის სტრიქონი არ არის მიცემული';ru='Не задана строка поиска';en='the search string is not specified'"));
	...
	Если НайденнаяОбласть = Неопределено Тогда
		...
		Если НайденнаяОбласть = Неопределено Тогда
			...
			ТекстСообщения = НСтр("ka='საზომი ერთეული ვერ მოიძებნა';ru='Единица измерения не найдена';en='Unit of measure is not found'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьВыбор(ТекущаяОбласть, ЗакрытьФорму = Истина)
	...
	Если ПустаяСтрока(КодЧисловой) Тогда
		...
		ТекстСообщения = НСтр("ka='რიცხვის კოდი';ru='числовой код';en='numerical code'");
		
	КонецЕсли;
	
	Если ПустаяСтрока(НаименованиеКраткое) Тогда
		
		ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", ", ") + НСтр("ka='მოკლე დასახელება';ru='краткое наименование';en='short title'");
		
	КонецЕсли;
	
	Если ПустаяСтрока(НаименованиеПолное) Тогда
		
		ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", ", ") + НСтр("ka='სრული დასახელება';ru='полное наименование';en='full name'");
		
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ТекстСообщения) Тогда
		
		ТекстСообщения = НСтр("ru='Указана не результирующая ячейка (не заполнены показатели: ';ka='Указана не результирующая ячейка (не заполнены показатели: '") + ТекстСообщения + НСтр("ka=')';ru=')';en=')'");
	...
	Если ЕдИзм <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ka='საზომი ერთეული უწინ იქნა დამატებული.';ru='Единица измерения была добавлена раннее.';en='Unit has been added earlier.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СоздатьЕдиницуИзмеренияПоКлассификатору(ЗначенияЗаполнения)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='საზომი ერთეულების დამატება კლასიფიკატორიდან';ru='Добавление единиц измерения из ОКЕИ';en='Adding units of OKEI'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());
