...
Процедура ОтправитьИПолучитьСообщения(Команда)
	...
	Если ЭтоСеансСУстановленнымиЗначениямиРазделителей Тогда
		ТекстСообщения = НСтр("ka='შეტყობინებების დამუშავება შესაძლებელია მხოლოდ სერვისის ადმინისტრატორის განუცალკევებელ სეანსში.';ru='Обработка сообщений возможна только в неразделенном сеансе администратора сервиса.'");
...
Процедура Удалить(Команда)
	...
	Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Список.ТекущиеДанные.Свойство("ГруппировкаСтроки")
			...
			ПоказатьПредупреждение(, НСтр("ka='მოქმედება მიუწვდომელია სიის დაჯგუფების სტრიქონისთვის.';ru='Действие недоступно для строки группировки списка.'"));
			
		Иначе
			
			Если Элементы.Список.ВыделенныеСтроки.Количество() > 1 Тогда
				
				СтрокаВопроса = НСтр("ka='წავშალოთ გამოყოფილი შეტყობინებები?';ru='Удалить выделенные сообщения?'");
				
			Иначе
				
				СтрокаВопроса = НСтр("ka='წავშალოთ შეტყობინება ""[Сообщение]""?';ru='Удалить сообщение ""[Сообщение]""?'");
