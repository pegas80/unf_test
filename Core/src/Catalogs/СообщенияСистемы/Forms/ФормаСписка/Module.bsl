...
Процедура ОтправитьИПолучитьСообщения(Команда)
	...
	Если ЭтоСеансСУстановленнымиЗначениямиРазделителей Тогда
		ТекстСообщения = НСтр("ka='შეტყობინებების დამუშავება შესაძლებელია მხოლოდ სერვისის ადმინისტრატორის განუცალკევებელ სეანსში.';ru='Обработка сообщений возможна только в неразделенном сеансе администратора сервиса.';en='processing of messages is only possible in undivided service administrator session.'");
...
Процедура Удалить(Команда)
	...
	Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Список.ТекущиеДанные.Свойство("ГруппировкаСтроки")
			...
			ПоказатьПредупреждение(, НСтр("ka='მოქმედება მიუწვდომელია სიის დაჯგუფების სტრიქონისთვის.';ru='Действие недоступно для строки группировки списка.';en='Action is not available for a list of groups line.'"));
			
		Иначе
			
			Если Элементы.Список.ВыделенныеСтроки.Количество() > 1 Тогда
				
				СтрокаВопроса = НСтр("ka='წავშალოთ გამოყოფილი შეტყობინებები?';ru='Удалить выделенные сообщения?';en='Delete the selected messages?'");
				
			Иначе
				
				СтрокаВопроса = НСтр("ka='წავშალოთ შეტყობინება ""[Сообщение]""?';ru='Удалить сообщение ""[Сообщение]""?';en='Delete the message ""[message]""?'");
