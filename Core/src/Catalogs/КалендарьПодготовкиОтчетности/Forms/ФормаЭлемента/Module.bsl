...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если Не Объект.ПрименяетсяДляИП И Не Объект.ПрименяетсяДляООО Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოვლენა გამოყენებული უნდა იქნას ინდმეწარმისთვის ან შპს-ისთვის!';ru='Событие должно быть применено для ИП или ООО!';en='The event is to be applied to the PI or company!'"));
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
