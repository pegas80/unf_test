...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(СправкаА) И НЕ ПустаяСтрока(НомерСправкиА) Тогда
		Если ВРег(СокрЛП(НомерСправкиА)) <> ВРег(СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СправкаА, "РегистрационныйНомер"))) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Номер справки А не совпадает с выбранной справкой.';ru='Номер справки А не совпадает с выбранной справкой.';en='A reference number is not the same as the selected certificate.'"),,,, Отказ);
		КонецЕсли;
	КонецЕсли;
