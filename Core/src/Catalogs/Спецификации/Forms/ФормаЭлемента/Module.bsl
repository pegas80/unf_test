...
Процедура СоставНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = Объект.Владелец Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სპეციფიკაციის შემადგენლობაში არ შეიძლება, რომ პროდუქცია შედიოდეს.';ru='В состав спецификации не может входить продукция.'"));
