	...
	Если Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Номенклатура)
			...
			Если ИспользоватьПодсистемуПроизводство И ИспользоватьПодсистемуРаботы Тогда
				Заголовок = НСтр("ka='სპეციფიკაცია ინახება მხოლოდ მარაგებისთვის და სამუშაოებისთვის';ru='Спецификации хранятся только для запасов и работ'");
			ИначеЕсли ИспользоватьПодсистемуПроизводство Тогда
				Заголовок = НСтр("ka='სპეციფიკაციები ინახება მხოლოდ მარაგებისთვის';ru='Спецификации хранятся только для запасов'");
			Иначе
				Заголовок = НСтр("ka='სპეციფიკაცია ინახება მხოლოდ სამუშაოებისთვის';ru='Спецификации хранятся только для работ'");
			КонецЕсли;
