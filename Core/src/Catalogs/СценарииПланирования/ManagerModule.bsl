...
Процедура ЗаполнитьФактическийСценарий() Экспорт
	...
	СценарийОбъект.Наименование		= НСтр("ka='ფაქტობრივი';ru='Фактический';en='Actual'");
...
Процедура ЗаполнитьПоставляемыеСценарии() Экспорт
	...
	Сценарий.Наименование			= НСтр("ka='ნომენკლატურის კატეგორიების გაყიდვა (თვეების თანმიმდევრობით)';ru='Продажи категорий номенклатуры (помесячно)';en='Sales categories Nomenclature (monthly)'");
	...
	Сценарий.Комментарий			= НСтр("ka='#შექმნილია ავტომატურად სწრაფი სტარტისთვის';ru='#Создан автоматически для быстрого старта';en='# Created automatically for quick start'");
	...
	Сценарий = Справочники.СценарииПланирования.СоздатьЭлемент();
	Сценарий.УстановитьНовыйКод();
	Сценарий.Наименование			= НСтр("ka='მენეჯერების გაყიდვის გეგმა (თვეების თანმიმდევრობით)';ru='План продаж менеджеров (помесячно)';en='Sales managers plan (monthly)'");
