...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='მიმდინარე ვერსიასთან არათავსებადია';ru='Не совместимы с текущей версией'");
		...
		Дело.ЕстьДела      = КоличествоНесовместимыхПрофилейГруппДоступа > 0;
		Дело.Важное        = Истина;
		Дело.Представление = НСтр("ka='დაშვების ჯგუფის პროფილები';ru='Профили групп доступа'");
		Дело.Количество    = КоличествоНесовместимыхПрофилейГруппДоступа;
...
"должно быть установлено Ложь.'");
	...
	ОписаниеПрофиляАдминистратор.Наименование  = НСтр("ka='ადმინისტრატორი';ru='Администратор'");
...
"""%4"".'"),
		...
		Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Идентификатор) <> Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='იდენტიფიკატორ ""%1""-ით პროფილი უკვე არსებობს.';ru='Профиль с идентификатором ""%1"" уже существует.'"),
				ОписаниеПрофиля.Идентификатор);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеПрофиля.Имя) Тогда
			Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Имя) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 სახელით პროფილი უკვე არსებობს.';ru='Профиль с именем ""%1"" уже существует.'"),
					ОписаниеПрофиля.Имя);
			КонецЕсли;
...
Функция ПредставлениеНазначенияПрофиля(НазначениеПрофиля)
	...
	Если НазначениеПрофиля = "СовместноДляПользователейИВнешнихПользователей" Тогда
		Возврат НСтр("ka='ერთობლივად, მომხმარებლებისთვის და გარე მომხმარებლებისთვის';ru='Совместно для пользователей и внешних пользователей'");
		
	ИначеЕсли НазначениеПрофиля = "ДляВнешнихПользователей" Тогда
		Возврат НСтр("ka='გარე მომხმარებლებისთვის';ru='Для внешних пользователей'");
	КонецЕсли;
	
	Возврат НСтр("ka='მომხმარებლებისთვის';ru='Для пользователей'");
