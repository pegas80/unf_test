...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ka='მიმდინარე ვერსიასთან არათავსებადია';ru='Не совместимы с текущей версией';en='Not compatible with the current version'");
		...
		Дело.ЕстьДела      = КоличествоНесовместимыхПрофилейГруппДоступа > 0;
		Дело.Важное        = Истина;
		Дело.Представление = НСтр("ka='დაშვების ჯგუფის პროფილები';ru='Профили групп доступа';en='Profiles of access groups'");
		Дело.Количество    = КоличествоНесовместимыхПрофилейГруппДоступа;
...
"роль ""%2"" не найдена в метаданных.';ka='მოსაწოდებელი პროფილ ""%1""-ის შემოწმებისას, მეტამონაცემებში როლი ""%2"" ვერ მოიძებნა.'"),
...
""
"';ka='Заданы недопустимые значения в процедуре ПриЗаполненииПоставляемыхПрофилейГруппДоступа"
"общего модуля УправлениеДоступомПереопределяемый."
""
"'");
	...
	Если ПараметрыОбновления.ЗапретитьИзменениеПрофилей
	   И НЕ ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
		...
		ВызватьИсключение ЗаголовокОшибки
			+ НСтр("ka='როდესაც პარამეტრ განახლებისპარამეტრები-ში თვისება შეცვლილიპროფილებისგანახლება არის სიცრუე, მაშინ თვისება პროფილებისცვლილებისაკრძალვა ასევე უნდა იყოს დაყენებული სიცრუე.';ru='Когда в параметре ПараметрыОбновления свойство"
...
"должно быть установлено Ложь.'");
	...
	ОписаниеПрофиляАдминистратор.Наименование  = НСтр("ka='ადმინისტრატორი';ru='Администратор';en='Administrator'");
...
"- Продажи, Закупки, Сервис, Производство, Деньги, Зарплата, Компания, Анализ и Настройки.';ka='პროფილი განკუთვნილია მმართველობითი პერსონალის სამუშაოდ და სერვისული ფუნქციების შესასრულებლად. "
"იძლევა განუსაზღვრელ დაშვებას საინფორმაციო სისტემის ყველა მონაცემთან. "
""
"პროფილით ხორციელდება მუშაობა ქვემოთ ჩამოთვლილ ყველა ნაწილთან: "
"- გაყიდვები, შესყიდვები, სერვისი, წარმოება, ფული, ხელფასი, კომპანია, ანალიზი და გამართვები.'");
	//Конец УНФ.
				 ...
				 Или Тип <> Тип("СправочникСсылка.Пользователи")
				   И Не Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.СодержитТип(Тип) Тогда
					ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='პროფილ ""%1""-ის აღწერილობაში მითითებულია დაუშვებელი დანიშნულება ""%2 (%3)""."
"მოსალოდნელია დანიშნულება, როგორც ტიპ ""ტიპი""-ს მნიშვნელობა ბმულისთვის, რომელიც მითითებულია განსაზღვრებად ტიპ გარემომხმარებელი-ში (ტიპ ცნობარიბმული.მომხმარებლები-ს გარდა).';ru='В описании профиля ""%1"""
...
"(кроме типа СправочникСсылка.Пользователи).'"),
		...
		Для Каждого Роль Из ОписаниеПрофиля.Роли Цикл
			...
			Если ВсеРоли.Получить(Роль) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1 (%2)""-ის აღწერილობაში როლი ""%3"" მეტამონაცემებში ვერ მოიძებნა.';ru='В описании профиля ""%1 (%2)"""
"роль ""%3"" не найдена в метаданных.'"),
			...
			Если НедоступныеРоли.Получить(Роль) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1 (%2)""-ის აღწერილობაში მითითებულია როლი ""%3"", "
"რომელიც პროფილის დანიშნულებას არ შეესაბამება:"
...
"""%4"".'"),
		...
		Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Идентификатор) <> Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='იდენტიფიკატორ ""%1""-ით პროფილი უკვე არსებობს.';ru='Профиль с идентификатором ""%1"" уже существует.';en='Profile with ID ""%1"" already exists.'"),
				ОписаниеПрофиля.Идентификатор);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеПрофиля.Имя) Тогда
			Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Имя) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 სახელით პროფილი უკვე არსებობს.';ru='Профиль с именем ""%1"" уже существует.';en='Profile with the name ""%1"" already exists.'"),
					ОписаниеПрофиля.Имя);
			КонецЕсли;
		...
		Для Каждого ЭлементСписка Из ОписаниеПрофиля.ВидыДоступа Цикл
			...
			Если СвойстваВидовДоступа.ПоИменам.Получить(ИмяВидаДоступа) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში მითითებულია დაშვების არარსებული სახეობა ""%2"".';ru='В описании профиля ""%1"""
"указан несуществующий вид доступа ""%2"".'"),
			...
			Если Не ВидДоступаСоответствуетНазначениюПрофиля Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში მითითებულია დაშვების სახეობა ""%2"", რომელიც პროფილის დანიშნულებას არ შეესაბამება: ""%3"".';ru='В описании профиля ""%1"""
...
"""%3"".'"),
			...
			Если УточнениеВидаДоступа <> ""
			   ...
			   И УточнениеВидаДоступа <> "ВначалеВсеРазрешены" Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში დაშვების სახეობა ""%2""-ისთვის მითითებულია უცნობი დაზუსტება ""%3""."
""
"დასაშვებია მხოლოდ შემდეგი დაზუსტებები:"
"- ""თავიდანყველააკრძალულია"" ან """","
"- ""თავიდანყველანებადართულია"","
"- ""წინასწარდაყენებული"".';ru='В описании профиля ""%1"""
...
"- ""Предустановленный"".'"),
		...
		Для Каждого ЭлементСписка Из ОписаниеПрофиля.ЗначенияДоступа Цикл
			...
			Если СвойстваВидаДоступа = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში დაშვების მნიშვნელობა ""%3""-ისთვის მითითებულია დაშვების არარსებული სახეობა ""%2"".';ru='В описании профиля ""%1"""
"указан несуществующий вид доступа ""%2"""
...
"""%3"".'"),
			...
			Если ОбъектМетаданных = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში დაშვების სახეობა ""%2""-ისთვის დაშვების აღნიშნული მნიშვნელობის ტიპი ""%3"" არ არსებობს.';ru='В описании профиля ""%1"""
"для вида доступа ""%2"""
...
"""%3"".'"),
			...
			Если ПустаяСсылкаЗначенияДоступа = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='დაშვების სახეობა ""%2""-ისთვის პროფილ ""%1""-ის აღწერილობაში მითითებულია დაშვების მნიშვნელობის არა მიმთითებელი ტიპი ""%3"".';ru='В описании профиля ""%1"""
"для вида доступа ""%2"""
...
"""%3"".'"),
			...
			Если СвойстваВидаДоступаПоТипу = Неопределено
			 ИЛИ СвойстваВидаДоступаПоТипу.Имя <> ВидДоступа Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში მითითებულია ""%3"" ტიპის დაშვების მნიშვნელობა, რომელიც არ არის მითითებული დაშვების სახეობის თვისებებში ""%2"".';ru='В описании профиля ""%1"""
...
"типа, который не указан в свойствах вида доступа ""%2"".'"),
			...
			Если ТаблицаЗначенийДоступа.НайтиСтроки(Отбор).Количество() > 0 Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='პროფილ ""%1""-ის აღწერილობაში დაშვების სახეობა ""%2""-ისთვის დაშვების მნიშვნელობა ""%3"" განმეორებით არის მითითებული.';ru='В описании профиля ""%1"""
"для вида доступа ""%2"""
...
Функция ПредставлениеНазначенияПрофиля(НазначениеПрофиля)
	...
	Если НазначениеПрофиля = "СовместноДляПользователейИВнешнихПользователей" Тогда
		Возврат НСтр("ka='ერთობლივად, მომხმარებლებისთვის და გარე მომხმარებლებისთვის';ru='Совместно для пользователей и внешних пользователей';en='Together for users and external users'");
		
	ИначеЕсли НазначениеПрофиля = "ДляВнешнихПользователей" Тогда
		Возврат НСтр("ka='გარე მომხმარებლებისთვის';ru='Для внешних пользователей';en='For external users'");
	КонецЕсли;
	
	Возврат НСтр("ka='მომხმარებლებისთვის';ru='Для пользователей';en='for registered users'");
...
"роль ""%2"" не найдена в метаданных.';ka='მისაწოდებელი პროფილ ""%1""-ის განახლებისას როლი ""%2"" მეტამონაცემებში ვერ მოიძებნა.'"),
					СвойстваПрофиля.Наименование,
					Роль);
			КонецЕсли;
