	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='როლი ""%1"" პროფილის დასანიშნად მიუწვდომელია.';ru='Роль ""%1"" недоступна для назначения профиля.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში როლი ""%1"" მიუწვდომელია პროფილის დასანიშნად.';ru='Роль ""%1"" в строке %%1 недоступна для назначения профиля.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьПоНачальномуЗаполнению(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ka='აღვადგინოთ პროფილი საწყისი შევსების შემცველობის მიხედვით?';ru='Восстановить профиль по содержимому начального заполнения?'"),
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ka='დაშვების ჯგუფების პროფილის დანიშნულების არჩევა';ru='Выбор назначения профиля групп доступа'"),,, ОписаниеОповещения);
КонецПроцедуры
