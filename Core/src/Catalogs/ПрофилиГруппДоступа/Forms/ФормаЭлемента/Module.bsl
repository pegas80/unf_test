	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='როლი ""%1"" პროფილის დასანიშნად მიუწვდომელია.';ru='Роль ""%1"" недоступна для назначения профиля.';en='The role of the ""%1"" is not available for the destination profile.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში როლი ""%1"" მიუწვდომელია პროფილის დასანიშნად.';ru='Роль ""%1"" в строке %%1 недоступна для назначения профиля.';en='The role of the ""%1"" in a string %% 1 is not available for the destination profile.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьПоНачальномуЗаполнению(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ka='აღვადგინოთ პროფილი საწყისი შევსების შემცველობის მიხედვით?';ru='Восстановить профиль по содержимому начального заполнения?';en='Restore the profile for the initial filling of content?'"),
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ka='დაშვების ჯგუფების პროფილის დანიშნულების არჩევა';ru='Выбор назначения профиля групп доступа';en='Selecting a destination access profile groups'"),,, ОписаниеОповещения);
КонецПроцедуры
...
"группы доступа профиля обновлены.';ka='პროფილი ""%1"" აღდგენილია საწყისი შევსების შემცველობით, პროფილის დაშვების ჯგუფები არ არის განახლებული.'");
	Иначе
		Текст =
			НСтр("ru='Профиль ""%1"" восстановлен по содержимому начального заполнения,"
"группы доступа профиля не обновлены.';ka='პროფილი ""%1"" აღდგენილია საწყისი შევსების შემცველობით, პროფილის დაშვების ჯგუფები არ არის განახლებული.'");
	КонецЕсли;
...
"значениями доступа и добавлены недостающие виды доступа.';ka='განვაახლოთ ამ პროფილით მოსარგებლე დაშვების ჯგუფები?"
""
"წაიშლება ზედმეტი დაშვების ტიპები მათთვის მიცემული დაშვების მნიშვნელობებით და დამატებული იქნება უკმარი დაშვების ტიპები.'");
