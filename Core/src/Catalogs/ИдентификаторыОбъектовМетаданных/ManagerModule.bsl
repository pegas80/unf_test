...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	...
	Если СтандартныеПодсистемыПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ცნობარი ""%1"" არ გამოიყენება.';ru='Справочник ""%1"" не используется.';en='Directory ""%1"" is not used.'"), НазваниеСправочника(ОбъектыРасширений));
	КонецЕсли;
...
"нажмите кнопку Отключить или программно очистите константу Главный узел.';ka='ცნობარი ""%1"" არ შეიძლება, რომ საინფორმაციო ბაზაში გამოყენებული იქნას მთავარი კვანძის დაუდასტურებელი გაუქმებით."
""
"მთავარ კვანძთან კავშირის აღსადგენად გაუშვით 1С:საწარმო და დააჭირეთ ღილაკ ""აღდგენა""-ს ან პროგრამულად დააყენეთ კონსტანტა ""მთავარი კვანძი""-ში შენახული მთავარი კვანძი."
""
"მთავარ კვანძთან კავშირის გაუქმების დასადასტურებლად გაუშვით 1С:საწარმო და დააჭირეთ ღილაკ ""გამორთვა""-ს ან პროგრამულად გაასუფთავეთ კონსტანტა ""მთავარი კვანძი"".'"),
...
"""%1"".';ka='მეტამონაცემთა ობიექტების იდენტიფიკატორების ჩატვირთვის შეცდომა. ახალი ელემენტის ჩატვირთვა, რომელსაც ახლის ბმული არ აქვს მითითებული, შეუძლებელია: ""%1"".'"),
						Объект.ПолноеИмя);
				КонецЕсли;
			КонецЕсли;
...
"""%1"".';ka='მეტამონაცემთა ობიექტების იდენტიფიკატორების ჩატვირთვის შეცდომა. შეუძლებელია ორი ელემენტის ჩატვირთვა, რომელთა სრული სახელი ერთმანეთს ემთხვევა: ""%1"".'"),
					Объект.ПолноеИмя);
			КонецЕсли;
...
"""%1"".';ka='მეტამონაცემთა ობიექტების იდენტიფიკატორების ჩატვირთვის შეცდომა. შეუძლებელია ორი ელემენტის ჩატვირთვა, რომელთა მეტამონაცემთა ობიექტის გასაღები ერთმანეთს ემთხვევა: ""%1"".'"),
...
"""%1"" не соответствует полному имени ""%2"".';ka='მეტამონაცემთა ობიექტების იდენტიფიკატორების ჩატვირთვის შეცდომა. შეუძლებელია იმ ელემენტის ჩატვირთვა, რომლის მეტამონაცემთა ობიექტის გასაღები ""%1"" სრულ სახელს ""%2"" არ შეესაბამება.'"),
...
Функция ОбъектыМетаданныхПоИдентификаторам(Идентификаторы) Экспорт
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა ფუნქციის შესრულებისას საერთოდანიშნულება.მეტამონაცემთაობიექტიიდენტიფიკატორისმიხედვით().';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().';en='Error executing ObschegoNaznacheniya.ObektMetadannyhPoIdentifikatoru () function.'");
	...
	Если Выгрузка.Количество() < Идентификаторы.Количество() Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ცნობარ ""%2""-ში იდენტიფიკატორი ""%1"" ვერ მოიძებნა.';ru='Идентификатор ""%1"""
		...
		Если РезультатПроверки.НеСоответствует Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				...
				ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ცნობარ ""%2""-ში მოძებნილ იდენტიფიკატორ ""%1""-ს შეესაბამება მეტამონაცემთა არარსებული ობიექტი ""%3"".';ru='Идентификатору ""%1"""
...
"""%3"".'"),
				...
				Иначе
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ცნობარ ""%2""-ში მოძებნილ იდენტიფიკატორ ""%1""-ს შეესაბამება მეტამონაცემთა მოშორებული ობიექტი.';ru='Идентификатору ""%1"""
"найденному в справочнике ""%2"","
"соответствует удаленный объект метаданных.'"),
			...
			Иначе
				ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ცნობარ ""%2""-ში მოძებნილ იდენტიფიკატორ ""%1""-ს შეესაბამება მეტამონაცემთა ობიექტი ""%3"", რომლის სრული სახელი განსხვავდება იდენტიფიკატორში  მიცემული სახელისგან.';ru='Идентификатору ""%1"""
"найденному в справочнике ""%2"","
...
"полное имя которого отличается от заданного в идентификаторе.'"),
		...
		Если Не ОбъектыРасширений И Свойства.ПометкаУдаления Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ცნობარ ""%2""-ში იდენტიფიკატორი ""%1"" მოიძებნა, მაგრამ რეკვიზიტ ""წაშლის ნიშნული""-ს მნიშვნელობა დაყენებულია ჭეშმარიტება.';ru='Идентификатор ""%1"""
...
"""%1"".';ka='შეცდომა საერთოდანიშნულების ფუნქციის შესრულებისას.მეტამონაცემთაობიექტისიდენტიფიკატორი()."
""
"მეტამონაცემთა ობიექტი ვერ მოიძებნა ტიპის მიხედვით:"
"""%1"".'"),
...
Функция ИдентификаторОбъектаМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Выгрузка = Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки = НСтр("ka='შეცდომა საერთოდანიშნულების ფუნქციის შესრულებისას. მეტამონაცემთაობიექტისიდენტიფიკატორი().';ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().';en='Error executing ObschegoNaznacheniya.IdentifikatorObektaMetadannyh () function.'");
...
"""%1"".';ka='მეტამონაცემთა ობიექტი სრული სახელით ვერ მოიძებნა:"
"""%1"".'"),
...
"""%1""."
""
"Допустимы только типы объектов метаданных перечисленные в комментарии к функции.';ka='მეტამონაცემთა ობიექტს მხარდაჭერა არ გააჩნია:"
"""%1""."
""
"დასაშვებია მხოლოდ მეტამონაცემთა ობიექტების ის ტიპები, რომლებიც ფუნქციების კომენტარებში არის ჩამოთვლილი.'"),
				ПолноеИмяОбъектаМетаданных);
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + ОписаниеОшибки;
		КонецЕсли;
		
		ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
			НСтр("ka='მეტამონაცემთა ობიექტ ""%1""-ისთვის ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ში და ინფორმაციის რეგისტრ ""გაფართოების ვერსიების ობიექტების იდენტიფიკატორები""-ში იდენტიფიკატორი ვერ მოიძებნა.';ru='Для объекта метаданных ""%1"""
...
"регистре сведений ""Идентификаторы объектов версий расширений"".'"),
			НСтр("ka='მეტამონაცემთა ობიექტ ""%1""-ისთვის ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ში იდენტიფიკატორი ვერ მოიძებნა.';ru='Для объекта метаданных ""%1"""
"не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".'"));
	...
	ИначеЕсли Выгрузка.Количество() > 1 Тогда
		ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
			НСтр("ka='ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორებში"" და ინფორმაციის რეგისტრ ""გაფართოებების ვერსიების ობიექტების იდენტიფიკატორებში"" მეტამონაცემთა ობიექტ ""%1""-ისთვის რამდენიმე იდენტიფიკატორი მოიძებნა.';ru='Для объекта метаданных ""%1"""
"найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"" и"
"регистре сведений ""Идентификаторы объектов версий расширений"".'"),
			НСтр("ka='მეტამონაცემთა ობიექტ ""%1""-ისთვის ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ში რამდენიმე იდენტიფიკატორი მოიძებნა.';ru='Для объекта метаданных ""%1"""
	...
	Если РезультатПроверки.НеСоответствует Тогда
		...
		Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მეტამონაცემთა ობიექტ ""%1""-ისთვის ცნობარ ""%2""-ში მოიძებნა იდენტიფიკატორი, რომელსაც შეესაბამება მეტამონაცემთა დაშორებული ობიექტი.';ru='Для объекта метаданных ""%1"""
...
"которому соответствует удаленный объект метаданных.'"),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მეტამონაცემთა ობიექტ ""%1""-ისთვის ცნობარ ""%2""-ში მოიძებნა იდენტიფიკატორი, რომელიც მეტამონაცემთა სხვა ობიექტ ""%3""-ს შეესაბამება.';ru='Для объекта метаданных ""%1"""
"найден идентификатор в справочнике ""%2"","
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ka='შეცდომა საერთო მოდულ საერთოდანიშნულებისხელახლაგანსაზღვრებადი-ს პროცედურა  მეტამონაცემთაობიექტებისდამატებისასგადარქმევისას-ში';ru='Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных общего модуля ОбщегоНазначенияПереопределяемый.';en='The error in procedure PriDobavleniiPereimenovaniyObektovMetadannyh common module ObschegoNaznacheniyaPereopredelyaemy.'");
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეტამონაცემთა გადარქმეული ობიექტის ტიპების სახელები ერთმანეთს არ ემთხვევა. წინა ტიპი: ""%1"", ახალი ტიპი: ""%2"".';ru='Не совпадают имена типов переименованного объекта метаданных."
"Прежний тип: ""%1"","
"новый тип: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
	...
	Если Итог.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეტამონაცემთა ობიექტის ტიპისთვის ""%1"" გადარქმევის აღწერილობა არ არის საჭირო, ვინაიდან მეტამონაცემთა ობიექტების ამ ტიპის შესახებ ინფორმაციის განახლება ავტომატურად ხდება."
""
"გადარქმევის აღწერილობა საჭირო არის მხოლოდ შემდეგი ტიპებისთვის:"
"%2.';ru='Для типа объекта метаданных ""%1"" не требуется описывать переименование,"
...
Функция СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений = Ложь) Экспорт
	...
	Строка.Синоним         = НСтр("ka='კონსტანტები';ru='Константы';en='constants'");
	Строка.ИмяВЕдЧисле     = "Константа";
	Строка.СинонимВЕдЧисле = НСтр("ka='კონსტანტა';ru='Константа';en='Constant'");
	
	// Подсистемы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("cdf5ac50-08e8-46af-9a80-4e63fd4a88ff");
	Строка.Имя             = "Подсистемы";
	Строка.Синоним         = НСтр("ka='ქვესისტემები';ru='Подсистемы';en='subsystems'");
	Строка.ИмяВЕдЧисле     = "Подсистема";
	Строка.СинонимВЕдЧисле = НСтр("ka='ქვესისტემა';ru='Подсистема';en='Subsystem'");
	...
	Строка.Имя             = "Роли";
	Строка.Синоним         = НСтр("ka='როლები';ru='Роли';en='role'");
	Строка.ИмяВЕдЧисле     = "Роль";
	Строка.СинонимВЕдЧисле = НСтр("ka='როლი';ru='Роль';en='Role'");
	Строка.БезДанных       = Истина;
	Строка.БезКлючаОбъектаМетаданных = Истина;
	...
	Строка.Синоним         = НСтр("ka='გაცვლის გეგმები';ru='Планы обмена';en='sharing plans'");
	Строка.ИмяВЕдЧисле     = "ПланОбмена";
	Строка.СинонимВЕдЧисле = НСтр("ka='გაცვლის გეგმა';ru='План обмена';en='sharing plan'");
	
	// Справочники
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("ede89702-30f5-4a2a-8e81-c3a823b7e161");
	Строка.Имя             = "Справочники";
	Строка.Синоним         = НСтр("ka='ცნობარები';ru='Справочники';en='References'");
	Строка.ИмяВЕдЧисле     = "Справочник";
	Строка.СинонимВЕдЧисле = НСтр("ka='ცნობარი';ru='Справочник';en='Directory'");
	
	// Документы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("96c6ab56-0375-40d5-99a2-b83efa3dac8b");
	Строка.Имя             = "Документы";
	Строка.Синоним         = НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'");
	Строка.ИмяВЕдЧисле     = "Документ";
	Строка.СинонимВЕдЧисле = НСтр("ka='დოკუმენტი';ru='Документ';en='Document'");
	
	// ЖурналыДокументов
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("07938234-e29b-4cff-961a-9af07a4c6185");
	Строка.Имя             = "ЖурналыДокументов";
	Строка.Синоним         = НСтр("ka='დოკუმენტთა ჟურნალები';ru='Журналы документов';en='journals documents'");
	Строка.ИмяВЕдЧисле     = "ЖурналДокументов";
	Строка.СинонимВЕдЧисле = НСтр("ka='დოკუმენტების ჟურნალი';ru='Журнал документов';en='journal of documents'");
	Строка.БезДанных       = Истина;
	...
	Строка.Имя             = "Отчеты";
	Строка.Синоним         = НСтр("ka='ანგარიშგებები';ru='Отчеты';en='Reports'");
	Строка.ИмяВЕдЧисле     = "Отчет";
	Строка.СинонимВЕдЧисле = НСтр("ka='ანგარიშგება';ru='Отчет';en='Report'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("ka='დამუშავებები';ru='Обработки';en='processing'");
	Строка.ИмяВЕдЧисле     = "Обработка";
	Строка.СинонимВЕдЧисле = НСтр("ka='დამუშავება';ru='Обработка';en='Treatment'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик';en='Plans species'");
	Строка.ИмяВЕдЧисле     = "ПланВидовХарактеристик";
	Строка.СинонимВЕдЧисле = НСтр("ka='მახასიათებლების ტიპების გეგმა';ru='План видов характеристик';en='Plans species'");
	
	// ПланыСчетов
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("4295af27-543f-4373-bcfc-c0ace9b7620c");
	Строка.Имя             = "ПланыСчетов";
	Строка.Синоним         = НСтр("ka='ანგარიშეგების გეგმები';ru='Планы счетов';en='Charts of accounts'");
	Строка.ИмяВЕдЧисле     = "ПланСчетов";
	Строка.СинонимВЕдЧисле = НСтр("ka='ანგარიშების გეგმა';ru='План счетов';en='Chart of accounts'");
	
	// ПланыВидовРасчета
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("fca3e7e1-1bf1-49c8-9921-aafb4e787c75");
	Строка.Имя             = "ПланыВидовРасчета";
	Строка.Синоним         = НСтр("ka='ანგარიშსწორებების ტიპების გეგმები';ru='Планы видов расчета';en='Plans of calculation types'");
	Строка.ИмяВЕдЧисле     = "ПланВидовРасчета";
	Строка.СинонимВЕдЧисле = НСтр("ka='ანგარიშსწორების ტიპების გეგმა';ru='План видов расчета';en='Plan of calculation types'");
	
	// РегистрыСведений
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("d7ecc1e9-c068-44dd-83c2-1323ec52dbbb");
	Строка.Имя             = "РегистрыСведений";
	Строка.Синоним         = НСтр("ka='ცნობების რეგისტრები';ru='Регистры сведений';en='registers information'");
	Строка.ИмяВЕдЧисле     = "РегистрСведений";
	Строка.СинонимВЕдЧисле = НСтр("ka='ცნობების რეგისტრი';ru='Регистр сведений';en='information register'");
	
	// РегистрыНакопления
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("74083488-b01e-4441-84a6-c386ce88cdb5");
	Строка.Имя             = "РегистрыНакопления";
	Строка.Синоним         = НСтр("ka='დაგროვების რეგისტრები';ru='Регистры накопления';en='accumulation registers'");
	Строка.ИмяВЕдЧисле     = "РегистрНакопления";
	Строка.СинонимВЕдЧисле = НСтр("ka='დაგროვების რეგისტრი';ru='Регистр накопления';en='accumulation register'");
	
	// РегистрыБухгалтерии
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("9a0d75ff-0eda-454e-b2b7-d2412ffdff18");
	Строка.Имя             = "РегистрыБухгалтерии";
	Строка.Синоним         = НСтр("ka='ბუღალტერიის რეგისტრები';ru='Регистры бухгалтерии';en='accounting registers'");
	Строка.ИмяВЕдЧисле     = "РегистрБухгалтерии";
	Строка.СинонимВЕдЧисле = НСтр("ka='ბუღალტერიის რეგისტრი';ru='Регистр бухгалтерии';en='accounts register'");
	
	// РегистрыРасчета
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("f330686a-0acf-4e26-9cda-108f1404687d");
	Строка.Имя             = "РегистрыРасчета";
	Строка.Синоним         = НСтр("ka='ანგარიშსწორების რეგისტრები';ru='Регистры расчета';en='registers calculation'");
	Строка.ИмяВЕдЧисле     = "РегистрРасчета";
	Строка.СинонимВЕдЧисле = НСтр("ka='ანგარიშსწორების რეგისტრი';ru='Регистр расчета';en='calculation register'");
	
	// БизнесПроцессы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("a8cdd0e0-c27f-4bf0-9718-10ec054dc468");
	Строка.Имя             = "БизнесПроцессы";
	Строка.Синоним         = НСтр("ka='ბიზნეს-პროცესები';ru='Бизнес-процессы';en='Business processes'");
	Строка.ИмяВЕдЧисле     = "БизнесПроцесс";
	Строка.СинонимВЕдЧисле = НСтр("ka='ბიზნეს-პროცესი';ru='Бизнес-процесс';en='Buisness process'");
	
	// Задачи
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("8d9153ad-7cea-4e25-9542-a557ee59fd16");
	Строка.Имя             = "Задачи";
	Строка.Синоним         = НСтр("ka='ამოცანები';ru='Задачи';en='Tasks'");
	Строка.ИмяВЕдЧисле     = "Задача";
	Строка.СинонимВЕдЧисле = НСтр("ka='ამოცანა';ru='Задача';en='Task'");
...
"реквизита ""Пометка удаления"" установлено Ложь недопустимо.';ka='ობიექტების იდენტიფიკატორების წაშლა, რომლებსაც რეკვიზიტ ""წაშლის ნიშნული""-ის მნიშვნელობა დაყენებული აქვთ სიცრუე, დაუშვებელია.'"));
	КонецЕсли;
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, РежимОбновления,
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = НСтр("ka='მეტამონაცემთა ობიექტების იდენტიფიკატორების გადარქმევა ძველისრულისახელი -> ახალისრულისახელი:';ru='Переименование идентификаторов объектов метаданных СтароеПолноеИмя -> НовоеПолноеИмя:';en='Renaming object identifier metadata StaroePolnoeImya -> NovoePolnoeImya:'")
		...
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений
				+ НСтр("ka='მეტამონაცემთა ობიექტების ახალი იდენტიფიკატორების დამატება:';ru='Добавление новых идентификаторов объектов метаданных:';en='Adding new identifiers metadata objects:'")
		...
		Если Не ТолькоПроверка Тогда
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЗаписьЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("ka='გაფართოების ობიექტების იდენტიფიკატორები. შესრულებულია კრიტიკული ცვლილებები';ru='Идентификаторы объектов расширений.Выполнены критичные изменения';en='Object IDs rasshireniy.Vypolneny critical changes'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						НСтр("ka='მეტამონაცემთა ობიექტების იდენტიფიკატორები. შესრულებულია კრიტიკული ცვლილებები';ru='Идентификаторы объектов метаданных.Выполнены критичные изменения';en='Object IDs metadannyh.Vypolneny critical changes'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
"Не следует называть подсистемы одинаково, либо использовать более новую версию платформы.';ka='შეცდომა სრული სახელით შვილობილი ქვესისტემის ძებნისას (""%1""-ის ძებნისას მოიძებნა ""%2""). ქვესისტემებს ერთნაირი სახელები არ უნდა დავარქვათ, ან უნდა გამოვიყენოთ პლატფორმის უფრო ახალი ვერსია.'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შეცდომა სრული სახელით მეტამონაცემთა ობიექტის ძებნისას (""%1""-ის ძებნისას მოიძებნა ""%2"").';ru='Ошибка при поиске объекта метаданных по полному имени (при поиске ""%1"" был найден ""%2"").';en='Error while searching the metadata using the full name of the object (for ""%1"" was found ''% 2'').'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		КонецЕсли;
	КонецЕсли;
...
Функция НазваниеСправочника(ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		НазваниеСправочника = НСтр("ka='გაფართოებების ობიექტების იდენტიფიკატორები';ru='Идентификаторы объектов расширений';en='Object IDs extensions'");
	Иначе
		НазваниеСправочника = НСтр("ka='მეტამონაცემთა ობიექტების იდენტიფიკატორები';ru='Идентификаторы объектов метаданных';en='Object IDs metadata'");
	КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='შეცდომა ცნობარ ""%1""-თან მუშაობისას.';ru='Ошибка при работе со справочником ""%1"".';en='Error when working with the directory ""%1"".'"),
		НазваниеСправочника(ОбъектыРасширений));
...
"возможно только автоматически при обновлении данных справочника.';ka='ობიექტის ახალი იდენტიფიკატორის შექმნა შესაძლებელია მხოლოდ ავტომატურად, ცნობარის მონაცემთა განახლებისას.'"));
...
"установлено только автоматически при обновлении данных справочника.';ka='ობიექტის იდენტიფიკატორის შეცვლისას მითითებულია სრული სახელი ""%1"", რომელიც შეიძლება დაყენებული იქნას მხოლოდ ავტომატურად, ცნობარის მონაცემთა განახლებისას.'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При изменении идентификатора объекта указано"
"полное имя ""%1"","
"которое уже используется в справочнике ""%2"".';ka='ობიექტის იდენტიფიკატორის შეცვლისას მითითებულია სრული სახელი ""%1"", რომელიც უკვე გამოიყენება ცნობარ ""%2""-ში.'"),
	...
	Если Не ОбъектыРасширений И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		Если Объект.ЭтоНовый()
		   И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового(), ЭтоОбъектРасширений(Объект)) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ka='ახალი ელემენტების დამატება შეიძლება შესრულდეს მხოლოდ განაწილებული საინფორმაციო ბაზის მთავარ კვანძში!';ru='Добавление новых элементов может быть выполнено"
		...
		Если Не Объект.ПометкаУдаления
		   И Не ЭтоКоллекция(Объект.Ссылка, ЭтоОбъектРасширений(Объект)) Тогда
			...
			Если ВРег(Объект.ПолноеИмя) <> ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений,
					НСтр("ka='რეკვიზიტ ""სრული სახელის"" შეცვლა შეიძლება შესრულდეს მხოლოდ განაწილებული საინფორმაციო ბაზის მთავარ კვანძში.';ru='Изменение реквизита ""Полное имя"" может быть выполнено"
"только в главном узле распределенной информационной базы.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
"не может использоваться в неразделенном режиме.';ka='ცნობარი ""გაფართოების ობიექტების იდენტიფიკატორები"" არ შეიძლება, რომ განუცალკევებელ რეჟიმში იქნას გამოყენებული.'");
	...
	Попытка
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				КонецПопытки;
			Иначе
				СообщитьОбОшибке(НСтр("ka='ტიპის მონაცემებში მნიშვნელობების შეცვლა არ ხდება';ru='Значения не заменяются в данных типа';en='The values ​​are not replaced in the type of data'")
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ka='გაფართოების ობიექტების იდენტიფიკატორები.იდენტიფიკატორის შეცვლა';ru='Идентификаторы объектов расширений.Замена идентификатора';en='Object IDs rasshireniy.Zamena ID'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ka='მეტამონაცემთა ობიექტების იდენტიფიკატორები.იდენტიფიკატორის შეცვლა';ru='Идентификаторы объектов метаданных.Замена идентификатора';en='Object IDs metadannyh.Zamena ID'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
