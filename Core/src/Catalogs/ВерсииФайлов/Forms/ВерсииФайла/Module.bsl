...
Процедура СделатьАктивнойВыполнить()
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='აქტიური ვერსიის გამოცვლა ნებადართულია მხოლოდ დაუკავებელი ფაილებისთვის.';ru='Смена активной версии разрешена только для незанятых файлов.';en='Changing the active version is only allowed to idle files.'"));
	КонецЕсли;
...
"   Текстовый документ OpenDocument (.odt)';ka='ვერსიების შედარებას მხარდაჭერა გააჩნია მხოლოდ შემდეგი ტიპის ფაილებისთვის: "
"   ტექსტური დოკუმენტი (.txt)"
"   დოკუმენტის ფორმატი RTF (.rtf) "
"   დოკუმენტი Microsoft Word (.doc, .docx) "
"   დოკუმენტი HTML (.html .htm) "
"   ტექსტური დოკუმენტი OpenDocument (.odt)'");
		...
		Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоБазоваяВерсияКонфигурации Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='მოცემულ ოპერაციას ძირითად ვერსიაში მხარდაჭერა არ გააჩნია.';ru='Данная операция не поддерживается в базовой версии.';en='This operation is not supported in the base version.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПометитьНаУдалениеСнятьПометку()
	...
	Если ТекущиеДанные.ПометкаУдаления Тогда 
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1-ს მოვხსნათ წაშლის ნიშნული?';ru='Снять с ""%1"" пометку на удаление?';en='Remove from the ""%1"" is marked for deletion?'"),
			Строка(ТекущиеДанные.Ссылка));
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მოვნიშნოთ ""%1"" წასაშლელად?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for removal?'"),
			Строка(ТекущиеДанные.Ссылка));
	КонецЕсли;
...
Процедура СравнениеВерсийАвтомат(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 2 Тогда
		...
		ТекстСостояния = НСтр("ka='ხორციელდება ფაილ ""%1""-ის ვერსიების შედარება';ru='Выполняется сравнение версий файла ""%1""...';en='Comparison is performed versions of the file ""%1"" ...'");
