...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоНовый() Тогда
		...
		Если ЗаписьПодписанногоОбъекта <> Истина Тогда
			...
			Если Зашифрован И СсылкаЗашифрован И ПодписанЭП И НЕ СсылкаПодписан Тогда
				ВызватьИсключение НСтр("ka='ფაილის დაშიფრულ ვერსიაზე ხელმოწერა არ შეიძლება.';ru='Зашифрованную версию файла нельзя подписывать.';en='Encrypted version of the file can not be signed.'");
			КонецЕсли;
	...
	Если Владелец.ТекущаяВерсия = Ссылка Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение НСтр("ka='აქტიური ვერსიის წაშლა არ შეიძლება.';ru='Активную версию нельзя удалить.';en='Active version can not be deleted.'");
		КонецЕсли;
	ИначеЕсли РодительскаяВерсия.Пустая() Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение НСтр("ka='პირველი ვერსიის წაშლა არ შეიძლება.';ru='Первую версию нельзя удалить.';en='You can not delete the first version.'");
		КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ Отказ И НЕ Ссылка.ПометкаУдаления Тогда  
		Двоичка = Неопределено;
		//Если ЗначениеЗаполнено(Ссылка.ТекстХранилище) Тогда 
		//	Двоичка = Ссылка.ТекстХранилище;
		//Иначе
			Двоичка = Ссылка.Владелец.ТекстХранилище;
		//КонецЕсли;
		
		Если ЗначениеЗаполнено(Двоичка.Получить()) Тогда
			НаборЗаписей = РегистрыСведений.ДвоичныеДанныеФайлов.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Файл.Установить(Ссылка);
			
			Запись = НаборЗаписей.Добавить();
			Запись.Файл = Ссылка;
			Запись.ДвоичныеДанныеФайла = Двоичка;//Новый ХранилищеЗначения(ДвоичныеДанныеФайла);
