...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоНовый() Тогда
		...
		Если ЗаписьПодписанногоОбъекта <> Истина Тогда
			...
			Если Зашифрован И СсылкаЗашифрован И ПодписанЭП И НЕ СсылкаПодписан Тогда
				ВызватьИсключение НСтр("ka='ფაილის დაშიფრულ ვერსიაზე ხელმოწერა არ შეიძლება.';ru='Зашифрованную версию файла нельзя подписывать.'");
			КонецЕсли;
	...
	Если Владелец.ТекущаяВерсия = Ссылка Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение НСтр("ka='აქტიური ვერსიის წაშლა არ შეიძლება.';ru='Активную версию нельзя удалить.'");
		КонецЕсли;
	ИначеЕсли РодительскаяВерсия.Пустая() Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение НСтр("ka='პირველი ვერსიის წაშლა არ შეიძლება.';ru='Первую версию нельзя удалить.'");
		КонецЕсли;
