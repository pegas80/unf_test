	   ...
	   И ПользовательИБРоли.Количество() = 0 Тогда
		...
		Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
			...
			ПоказатьВопрос(
				...
				,
				,
				НСтр("ka='საინფორმაციო ბაზის მომხმარებლის ჩანაწერი';ru='Запись пользователя информационной базы'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='როლი ""%1"" გარე მომხმარებლებისთვის მიუწვდომელია.';ru='Роль ""%1"" недоступна для внешних пользователей.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში არსებული როლი ""%1"" გარე მომხმარებლებისთვის მიუწვდომელია.';ru='Роль ""%1"" в строке %%1 недоступна для внешних пользователей.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБАутентификацияОС <> Ложь Тогда
			УточнениеСвойств.Добавить(НСтр("ka='ავთენტიფიკაცია ОС (ჩართულია)';ru='Аутентификация ОС (включена)'"));
		КонецЕсли;
		...
		Если ПользовательИБПользовательОС <> "" Тогда
			УточнениеСвойств.Добавить(НСтр("ka='ძს-ის მომხმარებელი (მითითებულია)';ru='Пользователь ОС (указан)'"));
		КонецЕсли;
		
		Если ПользовательИБПоказыватьВСпискеВыбора Тогда
			ЕстьРазличияУстранимыеБезАдминистратора = Истина;
			УточнениеСвойств.Добавить(НСтр("ka='არჩევის სიაში ჩვენება (ჩართულია)';ru='Показывать в списке выбора (включено)'"));
		КонецЕсли;
		
		Если ПользовательИБРежимЗапуска <> "Авто" Тогда
			ЕстьРазличияУстранимыеБезАдминистратора = Истина;
			УточнениеСвойств.Добавить(НСтр("ka='გაშვების რეჟიმი (არა ავტო)';ru='Режим запуска (не Авто)'"));
		КонецЕсли;
