...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("ka='სიაზე პასუხისმგებლის მიერ მომზადებული';ru='Подготовленные ответственным за список'"), Ложь,
...
Процедура ВидПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ka='მომხმარებლების სახეობის არჩევა';ru='Выбор вида пользователей'"), Ложь, Истина, ОписаниеОповещения);
...
"флажок ""Показывать пользователей нижестоящих групп"".'"));
	...
	Если Строка = ХранимыеПараметры.ГруппаВсеПользователи
		...
		СообщениеПользователю = Новый Структура("Сообщение, ЕстьОшибки, Пользователи",
			НСтр("ka='მონაწილეთა ტიპ ""მოცემული ტიპის ყველა მომხმარებელი""-ს ჯგუფებიდან მომხმარებლების გარიცხვა შეუძლებელია.';ru='Из групп с типом участников ""Все пользователи заданного типа"" исключение пользователей невозможно.'"),
			Истина,
			Неопределено);
	Иначе
		...
		ДействиеСПользователем = 
			...
			НСтр("ka='ჩართვა';ru='Включить'"),
			?(Перемещение, НСтр("ka='გადაადგილება';ru='Переместить'"), НСтр("ka='კოპირება';ru='Скопировать'")));
		
		Если ГруппаПомеченаНаУдаление Тогда
			ШаблонДействия = ?(Перемещение, НСтр("ka='ჯგუფი ""%1"" წასაშლელად არის მონიშნული. %2';ru='Группа ""%1"" помечена на удаление. %2'"), 
				НСтр("ka='ჯგუფი ""%1"" წასაშლელად არის მონიშნული. %2';ru='Группа ""%1"" помечена на удаление. %2'"));
		...
		Если КоличествоПользователей = 1 Тогда
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მომხმარებელი ""%1"" გავრიცხოთ ჯგუფ ""%2""-იდან?';ru='Исключить пользователя ""%1"" из группы ""%2""?'"),
			...
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი ""%2"" ჯგუფში ""%3""?';ru='%1 пользователя ""%2"" в группу ""%3""?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]),
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი ""%2"" ამ ჯგუფში?';ru='%1 пользователя ""%2"" в эту группу?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]));
			КонецЕсли;
		Иначе
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ჯგუფ ""%2""-დან გავრიცხოთ მომხმარებლები (%1)?';ru='Исключить пользователей (%1) из группы ""%2""?'"),
					КоличествоПользователей,
					Строка(Элементы.ГруппыВнешнихПользователей.ТекущаяСтрока));
				
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი (%2) ჯგუფ ""%3""-ში?';ru='%1 пользователей (%2) в группу ""%3""?'"),
					ДействиеСПользователем,
					КоличествоПользователей,
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი (%2) ამ ჯგუფში?';ru='%1 пользователей (%2) в эту группу?'"),
					ДействиеСПользователем,
					КоличествоПользователей);
			КонецЕсли;
		КонецЕсли;
...
Процедура ИзменитьПараметрыРасширеннойФормыПодбора()
	...
	Если ИспользоватьГруппыПользователей Тогда
		...
		Элементы.ВнешниеПользователиСписок.Заголовок             = НСтр("ka='მომხმარებლები ჯგუფში';ru='Пользователи в группе'");
...
Процедура ОбновитьЗаголовокСпискаВыбранныхПользователейИГрупп()
	...
	Если ХранимыеПараметры.ИспользоватьГруппы Тогда
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ka='არჩეული მომხმარებლები და ჯგუფები (%1)';ru='Выбранные пользователи и группы (%1)'");
	Иначе
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ka='არჩეული მომხმარებლები (%1)';ru='Выбранные пользователи (%1)'");
	КонецЕсли;
	...
	Иначе
		...
		Если ХранимыеПараметры.ИспользоватьГруппы Тогда
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("ka='არჩეული მომხმარებლები და ჯგუფები';ru='Выбранные пользователи и группы'");
		Иначе
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("ka='არჩეული მომხმარებლები';ru='Выбранные пользователи'");
		КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если ВыборГруппВнешнихПользователей Тогда
				...
				Иначе
					Заголовок = НСтр("ka='გარე მომხმარებლების და ჯგუფების შერჩევა';ru='Подбор внешних пользователей и групп'");
				КонецЕсли;
				
				Элементы.ВыбратьВнешнегоПользователя.Заголовок =
					НСтр("ka='გარე მომხმარებლების არჩევა';ru='Выбрать внешних пользователей'");
				
				Элементы.ВыбратьГруппуВнешнихПользователей.Заголовок =
					НСтр("ka='ჯგუფების არჩევა';ru='Выбрать группы'");
				
			Иначе
				Если ХранимыеПараметры.РасширенныйПодбор Тогда
					Заголовок = ХранимыеПараметры.ЗаголовокФормыПодбора;
				Иначе
					Заголовок = НСтр("ka='გარე მომხმარებლების შერჩევა';ru='Подбор внешних пользователей'");
				КонецЕсли;
			КонецЕсли;
		Иначе
			// Режим выбора.
			Если ВыборГруппВнешнихПользователей Тогда
				Заголовок = НСтр("ka='გარე მომხმარებლის ან ჯგუფის არჩევა';ru='Выбор внешнего пользователя или группы'");
				
				Элементы.ВыбратьВнешнегоПользователя.Заголовок = НСтр("ka='გარე მომხმარებლის არჩევა';ru='Выбрать внешнего пользователя'");
			Иначе
				Заголовок = НСтр("ka='გარე მომხმარებლის არჩევა';ru='Выбор внешнего пользователя'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ГруппыВнешнихПользователейПеретаскиваниеЗавершение(СообщениеПользователю)
	...
	Если СообщениеПользователю.ЕстьОшибки = Ложь Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ka='მომხმარებლების გადაადგილება';ru='Перемещение пользователей'"), , СообщениеПользователю.Сообщение, БиблиотекаКартинок.Информация32);
	Иначе
		...
		Если СообщениеПользователю.Пользователи <> Неопределено Тогда
			Подробно = НСтр("ka='შემდეგი მომხმარებლები არ არის შეყვანილი არჩეულ ჯგუფში:';ru='Следующие пользователи не были включены в выбранную группу:'");
