...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("ka='სიაზე პასუხისმგებლის მიერ მომზადებული';ru='Подготовленные ответственным за список';en='Prepared responsible for the list'"), Ложь,
...
Процедура ВидПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ka='მომხმარებლების სახეობის არჩევა';ru='Выбор вида пользователей';en='The choice of the user'"), Ложь, Истина, ОписаниеОповещения);
...
Процедура ГруппыВнешнихПользователейПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ВыбиратьИерархически Тогда
		ПоказатьПредупреждение(,
			НСтр("ka='მომხმარებლის გადასატანად ჯგუფებში, აუცილებელია ალამ ""ქვემდგომი ჯგუფების მომხმარებლების ჩვენება""-ს გამორთვა.';ru='Для перетаскивания пользователя в группы необходимо отключить"
"флажок ""Показывать пользователей нижестоящих групп"".'"));
	...
	Если Строка = ХранимыеПараметры.ГруппаВсеПользователи
		...
		СообщениеПользователю = Новый Структура("Сообщение, ЕстьОшибки, Пользователи",
			НСтр("ka='მონაწილეთა ტიპ ""მოცემული ტიპის ყველა მომხმარებელი""-ს ჯგუფებიდან მომხმარებლების გარიცხვა შეუძლებელია.';ru='Из групп с типом участников ""Все пользователи заданного типа"" исключение пользователей невозможно.';en='Groups of participants with type ""All users of a given type"" exception can not be members.'"),
			Истина,
			Неопределено);
	Иначе
		...
		ДействиеСПользователем = 
			...
			НСтр("ka='ჩართვა';ru='Включить';en='enable'"),
			?(Перемещение, НСтр("ka='გადაადგილება';ru='Переместить';en='move'"), НСтр("ka='კოპირება';ru='Скопировать';en='copy'")));
		
		Если ГруппаПомеченаНаУдаление Тогда
			ШаблонДействия = ?(Перемещение, НСтр("ka='ჯგუფი ""%1"" წასაშლელად არის მონიშნული. %2';ru='Группа ""%1"" помечена на удаление. %2';en='The group ""%1"" is marked for deletion. 2%'"), 
				НСтр("ka='ჯგუფი ""%1"" წასაშლელად არის მონიშნული. %2';ru='Группа ""%1"" помечена на удаление. %2';en='The group ""%1"" is marked for deletion. 2%'"));
		...
		Если КоличествоПользователей = 1 Тогда
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მომხმარებელი ""%1"" გავრიცხოთ ჯგუფ ""%2""-იდან?';ru='Исключить пользователя ""%1"" из группы ""%2""?';en='Delete the user ""%1"" from ""%2""?'"),
			...
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი ""%2"" ჯგუფში ""%3""?';ru='%1 пользователя ""%2"" в группу ""%3""?';en='User% 1 ""%2"" in group ""%3""?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]),
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი ""%2"" ამ ჯგუფში?';ru='%1 пользователя ""%2"" в эту группу?';en='User% 1 ""%2"" in this group?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]));
			КонецЕсли;
		Иначе
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ჯგუფ ""%2""-დან გავრიცხოთ მომხმარებლები (%1)?';ru='Исключить пользователей (%1) из группы ""%2""?';en=""Exclude users (1%) of the group '% 2'?"""),
					КоличествоПользователей,
					Строка(Элементы.ГруппыВнешнихПользователей.ТекущаяСтрока));
				
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი (%2) ჯგუფ ""%3""-ში?';ru='%1 пользователей (%2) в группу ""%3""?';en='1% of users (% 2) to the group ""%3""?'"),
					ДействиеСПользователем,
					КоличествоПользователей,
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='%1 მომხმარებელი (%2) ამ ჯგუფში?';ru='%1 пользователей (%2) в эту группу?';en='1% of users (% 2) in this group?'"),
					ДействиеСПользователем,
					КоличествоПользователей);
			КонецЕсли;
		КонецЕсли;
...
"так как в число ее участников входят все пользователи выбранных видов.';ka='ქვეჯგუფის დამატება ჯგუფ ""%1""-ში შეუძლებელია, ვინაიდან მის მონაწილეებში შედის არჩეული სახეობების ყველა მომხმარებელი.'"),
...
Процедура ИзменитьПараметрыРасширеннойФормыПодбора()
	...
	Если ИспользоватьГруппыПользователей Тогда
		...
		Элементы.ВнешниеПользователиСписок.Заголовок             = НСтр("ka='მომხმარებლები ჯგუფში';ru='Пользователи в группе';en='Members in the group'");
...
Процедура ОбновитьЗаголовокСпискаВыбранныхПользователейИГрупп()
	...
	Если ХранимыеПараметры.ИспользоватьГруппы Тогда
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ka='არჩეული მომხმარებლები და ჯგუფები (%1)';ru='Выбранные пользователи и группы (%1)';en='Selected users and groups (1%)'");
	Иначе
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ka='არჩეული მომხმარებლები (%1)';ru='Выбранные пользователи (%1)';en='Selected users (1%)'");
	КонецЕсли;
	...
	Иначе
		...
		Если ХранимыеПараметры.ИспользоватьГруппы Тогда
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("ka='არჩეული მომხმარებლები და ჯგუფები';ru='Выбранные пользователи и группы';en='The selected users and groups'");
		Иначе
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("ka='არჩეული მომხმარებლები';ru='Выбранные пользователи';en='Selected users'");
		КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если ВыборГруппВнешнихПользователей Тогда
				...
				Иначе
					Заголовок = НСтр("ka='გარე მომხმარებლების და ჯგუფების შერჩევა';ru='Подбор внешних пользователей и групп';en='Selection of external users and groups'");
				КонецЕсли;
				
				Элементы.ВыбратьВнешнегоПользователя.Заголовок =
					НСтр("ka='გარე მომხმარებლების არჩევა';ru='Выбрать внешних пользователей';en='Select external users'");
				
				Элементы.ВыбратьГруппуВнешнихПользователей.Заголовок =
					НСтр("ka='ჯგუფების არჩევა';ru='Выбрать группы';en='select group'");
				
			Иначе
				Если ХранимыеПараметры.РасширенныйПодбор Тогда
					Заголовок = ХранимыеПараметры.ЗаголовокФормыПодбора;
				Иначе
					Заголовок = НСтр("ka='გარე მომხმარებლების შერჩევა';ru='Подбор внешних пользователей';en='Selection of external users'");
				КонецЕсли;
			КонецЕсли;
		Иначе
			// Режим выбора.
			Если ВыборГруппВнешнихПользователей Тогда
				Заголовок = НСтр("ka='გარე მომხმარებლის ან ჯგუფის არჩევა';ru='Выбор внешнего пользователя или группы';en='Selection of external user or group'");
				
				Элементы.ВыбратьВнешнегоПользователя.Заголовок = НСтр("ka='გარე მომხმარებლის არჩევა';ru='Выбрать внешнего пользователя';en='Select an external user'");
			Иначе
				Заголовок = НСтр("ka='გარე მომხმარებლის არჩევა';ru='Выбор внешнего пользователя';en='Selecting an external user'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ГруппыВнешнихПользователейПеретаскиваниеЗавершение(СообщениеПользователю)
	...
	Если СообщениеПользователю.ЕстьОшибки = Ложь Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ka='მომხმარებლების გადაადგილება';ru='Перемещение пользователей';en='Moving users'"), , СообщениеПользователю.Сообщение, БиблиотекаКартинок.Информация32);
	Иначе
		...
		Если СообщениеПользователю.Пользователи <> Неопределено Тогда
			Подробно = НСтр("ka='შემდეგი მომხმარებლები არ არის შეყვანილი არჩეულ ჯგუფში:';ru='Следующие пользователи не были включены в выбранную группу:';en='The following users have been included in the selected group:'");
