...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ОбъектАвторизации) Тогда
		ВызватьИсключение НСтр("ka='გარე მომხმარებელს ავტორიზაციის ობიექტი არ აქვს მიცემული.';ru='У внешнего пользователя не задан объект авторизации.';en='External users do not specify authorization object.'");
	Иначе
		ТекстОшибки = "";
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтарыйОбъектАвторизации)
		   И СтарыйОбъектАвторизации <> ОбъектАвторизации Тогда
			...
			ВызватьИсключение НСтр("ka='ავტორიზაციის უწინ მითითებული ობიექტის შეცვლა არ შეიძლება.';ru='Невозможно изменить ранее указанный объект авторизации.';en='It is impossible to change a previously specified authorization object.'");
		КонецЕсли;
	КонецЕсли;
