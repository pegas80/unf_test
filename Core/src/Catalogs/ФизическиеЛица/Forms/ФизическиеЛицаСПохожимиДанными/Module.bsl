...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(НоваяСтрокаДанных.ДатаРождения) Тогда
			НоваяСтрокаДанных.ДатаРождения = "<" + НСтр("ka='არ არის შევსებული';ru='Не заполнена'") + ">";
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(НоваяСтрокаДанных.ИНН) Тогда
			НоваяСтрокаДанных.ИНН = "<" + НСтр("ka='არ არის შევსებული';ru='Не заполнен'") + ">";
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(НоваяСтрокаДанных.СтраховойНомерПФР) Тогда
			НоваяСтрокаДанных.СтраховойНомерПФР = "<" + НСтр("ka='არ არის შევსებული';ru='Не заполнен'") + ">";
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(НоваяСтрокаДанных.ДокументПредставление) Тогда
			НоваяСтрокаДанных.ДокументПредставление = "<" + НСтр("ka='არ არის შევსებული';ru='Не заполнен'") + ">";
		КонецЕсли;
	...
	Иначе
		...
		Элементы.ДаЭтоТотКтоМнеНужен.Заголовок = НСтр("ka='აღნიშნული ადამიანი არის ის, ვინც მჭირდება';ru='Отмеченный человек тот, кто мне нужен'");
...
Процедура ВыбратьФизическоеЛицоИЗакрытьФорму()
	...
	Если ДанныеФизическихЛиц.Количество() > 1 Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='საჭირო ადამიანის გასწვრივ აუცილებელია ნიშნულის დაყენება';ru='Необходимо установить отметку напротив нужного человека'"));
		КонецЕсли;
