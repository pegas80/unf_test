...
Процедура ЗаполнитьДеревоОперандов(ИсключитьСсылку, ЭтоФормированиеЦен, ВключаяЦеныНоменклатуры, ВключаяЦеныКонтрагентов)
	...
	Если ЭтоФормированиеЦен Тогда
		...
		НоваяГруппа.Представление	= НСтр("ka='საანგარიშსწორებო მნიშვნელობები';ru='РАСЧЕТНЫЕ ЗНАЧЕНИЯ'");
		
		ДобавитьРасчетныйОперанд(НоваяГруппа, "ТекущееЗначение",		НСтр("ka='მიმდინარე მნიშვნელობა';ru='Текущее значение'"));
		ДобавитьРасчетныйОперанд(НоваяГруппа, "ПоследняяЦенаВПриходе",	НСтр("ka='შემოსულის ბოლო ფასი';ru='Последняя цена поступления'"));
		ДобавитьРасчетныйОперанд(НоваяГруппа, "ПоследняяЦенаВРасходе",	НСтр("ka='რეალიზაციის ბოლო ფასი';ru='Последняя цена реализации'"));
		
		Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
			
			Если Справочники.Валюты.НайтиПоКоду("840") <> Неопределено Тогда
				
				ДобавитьРасчетныйОперанд(НоваяГруппа, "КурсДоллара", 	НСтр("ka='დოლარის მიმდინარე კურსი';ru='Текущий курс доллара'"));
				
			КонецЕсли;
			
			Если Справочники.Валюты.НайтиПоКоду("978") <> Неопределено Тогда
				
				ДобавитьРасчетныйОперанд(НоваяГруппа, "КурсЕвро", 		НСтр("ka='ევროს მიმდინარე კურსი';ru='Текущий курс евро'"));
				
			КонецЕсли;
			
		КонецЕсли;
		
		ДобавитьРасчетныйОперанд(НоваяГруппа, "Себестоимость", 			НСтр("ka='თვითღირებულება';ru='Себестоимость';en='Cost price'"));
		
		Если Константы.ВалютаУчета.Получить() <> Константы.НациональнаяВалюта.Получить() Тогда
			
			ДобавитьРасчетныйОперанд(НоваяГруппа, "СебестоимостьНацВалюта", НСтр("ka='თვითღირებულება (ეროვნ. ვალუტა)';ru='Себестоимость (нац. валюта)'"));
...
Процедура ЗаполнитьДеревоОператоров()
	...
	ГруппаСтрок.Наименование	= НСтр("ka='არიფმეტიკული ოპერატორები';ru='АРИФМЕТИЧЕСКИЕ ОПЕРАТОРЫ'");
	ГруппаСтрок.Картинка		= 1;
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='მიმატება ""+""';ru='Сложение ""+""'");
	НоваяСтрока.Оператор		= " + ";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='გამოქვითვა  ""-""';ru='Вычитание ""-""'");
	НоваяСтрока.Оператор		= " - ";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='გამრავლება  ""*""';ru='Умножение ""*""'");
	НоваяСтрока.Оператор		= " * ";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='გაყოფა ""/""';ru='Деление ""/""'");
	НоваяСтрока.Оператор		= " / ";
	
	ГруппаСтрок 				= ДеревоОператоров.Строки.Добавить();
	ГруппаСтрок.Наименование	= НСтр("ru='ПРОЧЕЕ';ka='სხვა'");
	ГруппаСтрок.Картинка		= 1;
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='პროცენტი ""%1""';ru='Процент ""1%""'");
	НоваяСтрока.Оператор		= " %1";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='პროცენტი ""%5""';ru='Процент ""5%""'");
	НоваяСтрока.Оператор		= " %5";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='პროცენტი ""20%""';ru='Процент ""20%""'");
	НоваяСтрока.Оператор		= " %20";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='პროცენტი ""50%""';ru='Процент ""50%""'");
	НоваяСтрока.Оператор		= " %50";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='ხსნადი ფრჩხილი ""(""';ru='Открывающая скобка ""(""'");
	НоваяСтрока.Оператор		= " (";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ka='დახურვის ფრჩხილი "")""';ru='Закрывающая скобка "")""'");
	НоваяСтрока.Оператор		= ") ";
...
Процедура ОператорыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СокрЛП(ТекстВставки) = "%1"
		...
		Если ДанныеОперанда = Неопределено 
			...
			ТекстСообщения = НСтр("ru='Укажите вид цен, от которого необходимо вычислить процент';ka='მიუთითეთ ფასების სახეობა, რომლისგან საჭიროა პროცენტის გამოთვლა'");
...
Процедура ФормулаПриИзменении(Элемент)
	...
	Если Найти(Формула, ",") > 0 Тогда
		...
		ТекстСообщения = НСтр("ka='წილადი ნაწილის მისათითებლად აუცილებელია, რომ გამოვიყენოთ წერტილი, და არა - მძიმე.';ru='Для указания дробной части необходимо использовать точку, а не запятую.'");
...
Процедура ПроверитьФормулу(Команда)
	...
	Если Ошибки = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Формула пригодна для расчетов.';ka='ფორმულა გამოსადეგია გამოსათვლელად.'");
		ПоказатьПредупреждение(, ТекстПредупреждения, , НСтр("ka='ფორმულის შემოწმება';ru='Проверка формулы'"));
