...
"Загрузка данных завершена.';ka='ნომენკლატურის ფასები. მონაცემთა ჩატვირთვა დასრულებულია.'"));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ТипВидаЦен = Перечисления.ТипыВидовЦен.ДинамическийПроцент Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Ссылка) Тогда
		...
		Если НЕ РезультатВыполненияЗапроса.Пустой()
			...
			ТекстСообщения = НСтр("ru='Текущий вид цен используется в розничных структурных единицах, по этому для него можно использовать только национальную валюту.';ka='ფასების მიმდინარე ტიპი გამოიყენება საცალო ვაჭრობის სტრუქტურულ ერთეულებში, ამიტომ, მისთვის შეიძლება მხოლოდ ეროვნული ვალუტის გამოყენება.'");
	...
	//ДД
	Если Не Отказ Тогда
		ТекущийОбъект.КатегорииНоменклатуры.Очистить();
		ДЗ = ДанныеФормыВЗначение(КатегорииНоменклатуры, Тип("ДеревоЗначений"));
		ОбойтиДерево(ДЗ,ТекущийОбъект); 
	КонецЕсли;	
	
КонецПроцедуры // ПередЗаписьюНаСервере()

&НаСервере
Процедура ОбойтиДерево(Дерево,ДокОбъект)
	Для Каждого Подстрока Из Дерево.Строки Цикл
		
		НовСтрока = ДокОбъект.КатегорииНоменклатуры.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтрока,Подстрока);	
		НовСтрока.КатегорияНоменклатуры = Подстрока.Значение; 
		ОбойтиДерево(Подстрока,ДокОбъект);
...
Процедура УдалитьЦены(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ka='ყურადღება!"
"პრაის-ლისტის ამ სვეტის ყველა ფასი წაიშლება. "
"გავაგრძელოთ?';ru='Внимание!"
...
"Продолжить?'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Операция имеет смыл только для записанных объектов';ka='ოპერაციას აზრი გააჩნია მხოლოდ ჩაწერილი ობიექტებისთვის'");
...
Процедура РассчитатьЦены(Команда)
	...
	Если Модифицированность = Истина Тогда
		...
		ТекстВопроса = НСтр("ka='ფასების გამოსათვლელად საჭიროა მიმდინარე ობიექტის ჩაწერა. ჩავწეროთ და გავაგრძელოთ?';ru='Для расчета цен необходимо записать текущий объект."
...
Процедура ЗагрузитьЦеныИзВнешнегоИсточника(Команда)
	...
	Если Объект.ТипВидаЦен <> КэшЗначений.Статический Тогда
		...
		ТекстСообщения = НСтр("ru='Загрузка предназначена только для статических видов цен.';ka='Загрузка предназначена только для статических видов цен.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения, 15, НСтр("ka='ფასების ჩატვირთვა გარე წყაროდან';ru='Загрузить цены из внешнего источника';en='Download the price from an external source'"));
		Возврат;
...
Процедура ЗаполнитьДеревоКатегорий()
	...
	Дерево = ДеревоКатегорий();
	ЗначениеВРеквизитФормы(Дерево, "КатегорииНоменклатуры");
	
КонецПроцедуры

&НаСервере
Функция ДеревоКатегорий() 
...
Процедура ВыбратьЭлементыВИерархии(ВыборкаСИерархией,Дерево)
    Пока ВыборкаСИерархией.Следующий() Цикл
		...
		Структура.Вставить("КатегорияНоменклатуры",ВыборкаСИерархией.Значение);
		
		НайденаяСтрока = Объект.КатегорииНоменклатуры.НайтиСтроки(Структура);
		
		Если НайденаяСтрока.Количество() <> 0 Тогда
			Строка.Процент = НайденаяСтрока[0].Процент;
		КонецЕсли;	
