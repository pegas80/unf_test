...
Процедура ФоновоеЗаданиеВыполнено(Прогресс)
	...
	Исключение
		...
		ВызватьИсключение Нстр("ru='Ошибка';ka='შეცდომა'") + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(Прогресс) = Тип("Структура")
			...
			МассивСтрок.Добавить(НСтр("ru='Пожалуйста, подождите...';ka='გთხოვთ, მოიცადეთ...'"));
...
Процедура ЗагрузитьЦеныИзВнешнегоИсточника(Команда)
	...
	Если Объект.ТипВидаЦен <> КэшЗначений.Статический Тогда
		...
		ПоказатьПредупреждение(Неопределено, ТекстСообщения, 15, НСтр("ka='ფასების ჩატვირთვა გარე წყაროდან';ru='Загрузить цены из внешнего источника'"));
		Возврат;
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Описание = НСтр("ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузки результата';ka='ქვესისტემა მონაცემთაჩატვირთვაგარეწყაროდან: შედეგის ჩატვირთვის სერვერული მეთოდის შესრულება'");
