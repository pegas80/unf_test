...
Процедура ОткрытьНовуюФорму(ВидФормы, СерверныеПараметры, КлиентскиеПараметры = Неопределено,
	...
	Если СтрНайти(ВидыФорм, "," + ВидФормы + ",") = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შეცდომა პროცედურა ახალიფორმისგახსნა-ში. ფორმისსახეობა ""%1""-ს მხარდაჭერა არ გააჩნია.';ru='Ошибка в процедуре ОткрытьНовуюФорму. ВидФормы ""%1"" не поддерживается.';en='Error in OtkrytNovuyuFormu procedure. VidFormy ""%1"" is not supported.'"),
			ВидФормы);
	КонецЕсли;
