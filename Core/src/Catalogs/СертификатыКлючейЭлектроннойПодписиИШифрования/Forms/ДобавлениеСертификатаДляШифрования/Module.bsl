...
Процедура Далее(Команда)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='გამოჰყავით დასამატებელი სერთიფიკატები.';ru='Выделите сертификаты, которые требуется добавить.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДалееПослеПоискаСертификата(Результат, Неопределен) Экспорт
	...
	Если Результат.Свойство("СертификатНеНайден") Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა (შეიძლება წაშლილია).';ru='Сертификат не найден на компьютере (возможно удален).'"));
	Иначе
...
Процедура ДалееПослеПоискаСертификатаВОблачномСервисе(Результат, Неопределен) Экспорт
	...
	Если Не Результат.Выполнено Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ka='Сертификат не найден в облачном сервисе (возможно удален).';ru='Сертификат не найден в облачном сервисе (возможно удален).'"));
	Иначе
		Контекст.Вставить("ОписаниеОшибки", Результат.ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПоказатьСтраницуУточнениеСвойствСертификата(Форма, СертификатКриптографии, ДанныеСертификата)
	...
	Строка = ?(ЗначениеЗаполнено(Форма.Сертификат), НСтр("ka='განახლება';ru='Обновить'"), НСтр("ka='დამატება';ru='Добавить'"));
