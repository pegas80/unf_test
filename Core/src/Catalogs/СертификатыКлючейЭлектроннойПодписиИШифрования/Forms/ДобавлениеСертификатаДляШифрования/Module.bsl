...
Процедура Далее(Команда)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='გამოჰყავით დასამატებელი სერთიფიკატები.';ru='Выделите сертификаты, которые требуется добавить.';en='Highlight the certificate that you want to add.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДалееПослеПоискаСертификата(Результат, Неопределен) Экспорт
	...
	Если Результат.Свойство("СертификатНеНайден") Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა (შეიძლება წაშლილია).';ru='Сертификат не найден на компьютере (возможно удален).';en='Certificate can not be found on your computer (possibly deleted).'"));
	Иначе
...
Процедура ДалееПослеПоискаСертификатаВОблачномСервисе(Результат, Неопределен) Экспорт
	...
	Если Не Результат.Выполнено Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ka='Сертификат не найден в облачном сервисе (возможно удален).';ru='Сертификат не найден в облачном сервисе (возможно удален).';en='The certificate is not found in the cloud service (possibly deleted).'"));
	Иначе
		Контекст.Вставить("ОписаниеОшибки", Результат.ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПоказатьСтраницуУточнениеСвойствСертификата(Форма, СертификатКриптографии, ДанныеСертификата)
	...
	Строка = ?(ЗначениеЗаполнено(Форма.Сертификат), НСтр("ka='განახლება';ru='Обновить';en='update'"), НСтр("ka='დამატება';ru='Добавить';en='Add'"));
