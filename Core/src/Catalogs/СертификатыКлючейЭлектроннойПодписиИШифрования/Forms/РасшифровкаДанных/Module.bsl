...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАутентификация Тогда
		Элементы.ФормаРасшифровать.Заголовок = НСтр("ka='ОК';ru='ОК';en='OK'");
		Элементы.ПояснениеУсиленногоПароля.Заголовок = НСтр("ka='პაროლის შეყვანაზე გადასასვლელად, დააჭირეთ OK-ს.';ru='Нажмите ОК, чтобы перейти к вводу пароля.';en='Click OK to go on to enter the password.'");
	КонецЕсли;
...
"откройте сертификат и укажите программу вручную.';ka='არჩეულ სერთიფიკატს დახურული გასაღებისთვის პროგრამა არ აქვს მითითებული. სრული სიიდან განმეორებით აირჩიეთ სერთიფიკატი ან გახსენით სერთიფიკატი და პროგრამა ხელით მიუთითეთ.'"));
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ka='დაშიფრვის სერთიფიკატების გასუფთავებისას შეცდომა წარმოიშვა:';ru='При очистке сертификатов шифрования возникла ошибка:';en='When cleaning the error occurred encryption certificates:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='მონაცემთა გაშიფრვა ვერ მოხერხდა';ru='Не удалось расшифровать данные';en='Unable to decrypt the data'"), "",
