...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАутентификация Тогда
		Элементы.ФормаРасшифровать.Заголовок = НСтр("ka='ОК';ru='ОК'");
		Элементы.ПояснениеУсиленногоПароля.Заголовок = НСтр("ka='პაროლის შეყვანაზე გადასასვლელად, დააჭირეთ OK-ს.';ru='Нажмите ОК, чтобы перейти к вводу пароля.'");
	КонецЕсли;
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ka='დაშიფრვის სერთიფიკატების გასუფთავებისას შეცდომა წარმოიშვა:';ru='При очистке сертификатов шифрования возникла ошибка:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='მონაცემთა გაშიფრვა ვერ მოხერხდა';ru='Не удалось расшифровать данные'"), "",
