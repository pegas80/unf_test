		...
		Для каждого Строка Из Строки Цикл
			...
			Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='სერთიფიკატ ""%1""-ის მონაცემები ცნობარში ვერ მოიძებნა';ru='Данные сертификата ""%1"" не найдены в справочнике';en='These certificate ""%1"" is not found in the directory'"), Выборка.Представление);
			КонецЕсли;
			Попытка
			...
			Исключение
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ცნობარში სერთიფიკატ ""%1""-ის მონაცემები არაკორექტულია შემდეგი მიზეზით:"
...
"Выберите другой сертификат.';ka='არჩეული პირადი სერთიფიკატის მოქმედების ვადა ამოიწურა."
"აირჩიეთ სხვა სერთიფიკატი.'"));
...
"Выберите другой сертификат.';ka='არჩეულ პირად სერთიფიკატს დახურული გასაღებისთვის პროგრამა არ აქვს მითითებული."
"აირჩიეთ სხვა სერთიფიკატი.'"));
			ПоказатьОшибку(Контекст.ОшибкаНаКлиенте, Контекст.ОшибкаНаСервере);
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
			Возврат;
...
Функция СвойстваСертификатов(Знач Ссылки, Знач ИдентификаторФормы)
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеСертификата = Выборка.ДанныеСертификата.Получить();
		Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერთიფიკატ ""%1""-ის მონაცემები ცნობარში ვერ მოიძებნა';ru='Данные сертификата ""%1"" не найдены в справочнике';en='These certificate ""%1"" is not found in the directory'"),
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, АдресСертификатов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОтменитьТранзакцию();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ka='დაშიფრვის სერთიფიკატების ჩაწერისას შეცდომა წარმოიშვა:';ru='При записи сертификатов шифрования возникла ошибка:';en='An error occurred while recording encryption certificates:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='მონაცემთა დაშიფრვა ვერ მოხერხდა';ru='Не удалось зашифровать данные';en='Failed to encrypt data'"), "",
