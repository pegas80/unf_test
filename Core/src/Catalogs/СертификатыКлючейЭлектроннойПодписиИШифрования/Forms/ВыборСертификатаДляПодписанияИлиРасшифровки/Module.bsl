...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДобавлениеВСписок Тогда
		...
		Элементы.Выбрать.Заголовок = НСтр("ka='დამატება';ru='Добавить'");
		
		Элементы.ПояснениеУсиленногоПароля.Заголовок =
			НСтр("ka='პაროლის შეყვანაზე გადასასვლელად, დააჭირეთ ""დამატება""-ს.';ru='Нажмите Добавить, чтобы перейти к вводу пароля.'");
		
		ЛичныйСписокПриДобавлении = Параметры.ЛичныйСписокПриДобавлении;
		Элементы.ПоказыватьВсе.Подсказка =
			НСтр("ka='ყველა სერთიფიკატის ჩვენება შერჩევის გარეშე (მაგალითად, დამატებულების და ვადაგასულების ჩათვლით)';ru='Показать все сертификаты без отбора (например, включая добавленные и просроченные)'");
	КонецЕсли;
	...
	Если ДляШифрованияИРасшифровки = Истина Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემთა დასაშიფრად და გასაშიფრად';ru='Добавление сертификата для шифрования и расшифровки данных'");
		Иначе
			Заголовок = НСтр("ka='სერთიფიკატის არჩევა მონაცემთა დასაშიფრად და გასაშიფრად';ru='Выбор сертификата для шифрования и расшифровки данных'");
		КонецЕсли;
	ИначеЕсли ДляШифрованияИРасшифровки = Ложь Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემებზე ხელმოსაწერად';ru='Добавление сертификата для подписания данных'");
		КонецЕсли;
	ИначеЕсли ЭлектроннаяПодпись.ИспользоватьШифрование() Тогда
		Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემთა ხელმოსაწერად და დასაშიფრად';ru='Добавление сертификата для подписания и шифрования данных'");
	Иначе
		Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემებზე ხელმოსაწერად';ru='Добавление сертификата для подписания данных'");
	КонецЕсли;
	...
	Если СоздаватьЭлектронныеПодписиНаСервере Тогда
		Элементы.ГруппаСертификаты.Заголовок =
			НСтр("ka='პირადი სერთიფიკატები კომპიუტერზე და სერვერზე';ru='Личные сертификаты на компьютере и сервере'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Элементы.СертификатОрганизация.Видимость
	   ...
	   И Не ЗначениеЗаполнено(СертификатОрганизация) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ორგანიზაცია არ არის შევსებული.';ru='Поле Организация не заполнено.'");
...
Процедура СертификатыНедоступныНаКлиентеНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='სერთიფიკატები კომპიუტერზე';ru='Сертификаты на компьютере'"), "", ОшибкаПолученияСертификатовНаКлиенте, Новый Структура);
...
Процедура СертификатыНедоступныНаСервереНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='სერთიფიკატები სერვერზე';ru='Сертификаты на сервере'"), "", ОшибкаПолученияСертификатовНаСервере, Новый Структура);
...
Процедура ВыбратьПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ЗаголовокФормы = НСтр("ka='დაშიფრვის და გაშიფრვის შეცდომა';ru='Проверка шифрования и расшифровки'");
		Иначе
			ЗаголовокФормы = НСтр("ka='ელექტრონული ხელმოწერის დაყენების შემოწმება';ru='Проверка установки электронной подписи'");
		КонецЕсли;
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
...
Процедура ПерейтиКВыборуТекущегоСертификата(Оповещение)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("ka='გამოჰყავით სერთიფიკატი, რომელიც გამოყენებული იქნება.';ru='Выделите сертификат, который будет использоваться.'");
...
"Откройте заявление на выпуск сертификата и выполните требуемые шаги.'");
	...
	Если СертификатНаСервере Тогда
		...
		Иначе
			Результат.ОписаниеОшибки = НСтр("ka='სერვერზე სერთიფიკატი ვერ მოიძებნა (შეიძლება წაშლილია).';ru='Сертификат не найден на сервере (возможно удален).'");
			Результат.ОбновитьСписокСертификатов = Истина;
			ВыполнитьОбработкуОповещения(Оповещение, Результат);
		КонецЕсли;
		Возврат;
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеПоискаСертификата(РезультатПоиска, Контекст) Экспорт
	...
	Если ТипЗнч(РезультатПоиска) <> Тип("СертификатКриптографии") Тогда
		Если РезультатПоиска.Свойство("СертификатНеНайден") Тогда
			Контекст.Результат.ОписаниеОшибки = НСтр("ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა (შეიძლება წაშლილია).';ru='Сертификат не найден на компьютере (возможно удален).'");
		Иначе
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеЗаполненияСвойствСертификата(Контекст)
	...
	Если ДобавлениеВСписок Тогда
		Строка = ?(ЗначениеЗаполнено(Сертификат), НСтр("ka='განახლება';ru='Обновить'"), НСтр("ka='დამატება';ru='Добавить'"));
...
Процедура ПроверитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии")
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			Результат.Вставить("ЗаголовокОшибки", НСтр("ka='დაშიფრვის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку шифрования по причине:'"));
		Иначе
			Результат.Вставить("ЗаголовокОшибки", НСтр("ka='ხელმოწერის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку подписания по причине:'"));
		КонецЕсли;
...
Функция ПроверитьСертификатИЗаписатьВСправочник(Знач ЗначениеПароля, ОшибкаНаСервере)
	...
	Если ЭлектроннаяПодписьСлужебный.МенеджерКриптографии("", Ложь, ОшибкаНаСервере) = Неопределено
	   И ОшибкаНаСервере.Общая Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ka='დაშიფრვის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку шифрования по причине:'"));
		Иначе
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ka='ხელმოწერის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку подписания по причине:'"));
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
