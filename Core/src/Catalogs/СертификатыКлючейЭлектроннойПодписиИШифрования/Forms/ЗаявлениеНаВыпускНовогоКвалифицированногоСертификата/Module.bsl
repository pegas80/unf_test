...
Процедура ЮридическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ЮридическийАдрес", НСтр("ka='ორგანიზაციის იურიდიული მისამართი';ru='Юридический адрес организации';en='Legal address'"));
...
Процедура ФактическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ФактическийАдрес", НСтр("ka='ორგანიზაციის ფაქტობრივი მისამართი';ru='Фактический адрес организации';en='Actual address'"));
...
Процедура ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеТелефонаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"Телефон", НСтр("ka='ორგანიზაციის ტელეფონი';ru='Телефон организации';en='Telephone organization'"));
	...
	Если Не ЗначениеЗаполнено(ГражданствоОКСМКодАльфа3) Тогда
		Если ЗначениеЗаполнено(Гражданство) Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ქვეყნების კლასიფიკატორის მიხედვით ქვეყანა ""%1""-ის სამნიშნა ასო-ბგერითი კოდი ალფა-3 არ არის მითითებული.';ru='У страны ""%1"" не указан трехзначный буквенный код альфа-3 по ОКСМ.';en='The country ""%1"" is missing the three-digit alphabetic code alpha-3 OKSM.'"),
				Гражданство));
		КонецЕсли;
...
"Обратитесь к администратору.';ka='პროგრამა %1 ჯერ არ არის გამოყენებადი პროგრამების სიაში დამატებული. მიმართეთ ადმინისტრატორს.'"),
					Представление));
			
		ИначеЕсли ЭтоПодчиненныйУзелРИБ Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Программа %1 еще не добавлена в список используемых программ."
"Выполните добавление в главном узле информационной базы.';ka='პროგრამა %1 ჯერ არ არის გამოყენებადი პროგრამების სიაში დამატებული. მოახდინეთ საინფორმაციო ბაზის მთავარ კვანძში დამატება.'"),
					Представление));
		Иначе
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Добавить",    НСтр("ka='დამატება';ru='Добавить';en='Add'"));
			Кнопки.Добавить("НеДобавлять", НСтр("ka='არ დაიმატოს';ru='Не добавлять';en='Do not add'"));
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПрограммаОбработкаВыбораПродолжение", ЭтотОбъект, ВыбранноеЗначение),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Программа %1 еще не добавлена в список используемых программ."
"Добавить?';ka='პროგრამა %1 ჯერ არ არის გამოყენებადი პროგრამების სიაში დამატებული. დავამატოთ?'"),
					Представление),
				Кнопки,, "НеДобавлять");
		КонецЕсли;
...
Процедура ДекорацияСозданиеКлючаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "КлючЭлектроннойПодписи" Тогда
		ПоказатьПредупреждение(,
			НСтр("ka='ელექტრონული ხელმოწერის გასაღები - ეს არის საიდუმლო ინფორმაცია, რომელიც ინახება კომპიუტერზე, ფლეშ-მეხსიერებაზე, დისკეტზე ან ინფორმაციის სხვა შემცველზე და შემდგომში, გამოიყენება ელექტრონული ხელმოწერების შესაქმნელად.';ru='Ключ электронной подписи - это секретная информация, которая сохраняется"
...
"используется в дальнейшем для создания электронных подписей.'"));
	...
	ИначеЕсли НавигационнаяСсылка = "ЗапросНаСертификат" Тогда
		ПоказатьПредупреждение(,
			НСтр("ka='სერთიფიკატის მოთხოვნა - ეს არ არის საიდუმლო ინფორმაცია, რომელიც ელექტრონული ხელმოწერის გასაღების საფუძველზე იქმნება, სერთიფიკატის გამოშვების თაობაზე განცხადებასთან ერთად იგზავნება და სერთიფიკატის გამოსაშვებად არის საჭირო.';ru='Запрос на сертификат - это не секретная информация, которая создается"
...
Процедура ИнструкцияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		ТекстПредупреждения =
			НСтр("ka='დოკუმენტების კომპლექტი:"
"1. განცხადება სერთიფიკატის გამოშვებაზე (წინა ეტაპზე მომზადებული)."
"2. საგადასახადო ორგანოში აღრიცხვაზე აყვანის მოწმობის (გსნ) ასლი."
"3. ინდივიდუალური მეწარმის სახელმწიფო რეგისტრაციის მოწმობის (ძსსნ) ასლი."
"4. სერთიფიკატის მფლობელის პიროვნების დამადასტურებელი დოკუმენტის ასლი."
"5. სერთიფიკატის მფლობელის სავალდებულო საპენსიო დაზღვევის სადაზღვევო მოწმობის (იპასნ) ასლი.';ru='Комплект документов:"
...
"5. Копия страхового свидетельства обязательного пенсионного страхования (СНИЛС) владельца сертификата.'");
	Иначе
		ТекстПредупреждения =
			НСтр("ka='დოკუმენტების კომპლექტი:"
"1. განცხადება სერთიფიკატის გამოშვებაზე (წინა ეტაპზე მომზადებული)."
"2. საგადასახადო ორგანოში აღრიცხვაზე აყვანის მოწმობის ასლი* (გსნ)."
"3. იურიდიული პირის სახელმწიფო რეგისტრაციის მოწმობის ასლი* (ძსსნ)."
"4. სერთიფიკატის გამოსაშვებად მითითებული ორგანიზაციის წარმომადგენლის პირადობის დამადასტურებელი დოკუმენტის ასლი*."
"5. სერთიფიკატის გამოსაშვებად მითითებული ორგანიზაციის წარმომადგენლის სავალდებულო საპენსიო დაზღვევის სადაზღვევო (იპასნ) მოწმობის ასლი*."
"6. განცხადებაზე ხელმომწერი (დამფუძნებლების შეკრების პროტოკოლი, მესაკუთრის გადაწყვეტილება, წესდება) ხელმძღვანელის უფლებამოსილების დამადასტურებელი დოკუმენტის ასლი* ან საგადასახადო ორგანოს მიერ დამოწმებული ამონაწერი იპესრ-იდან."
""
"* დოკუმენტების ასლი დამოწმებული უნდა იყოს ხელმძღვანელის ხელმოწერით და ორგანიზაციის ბეჭდით.';ru='Комплект документов:"
"1. Заявление на выпуск сертификата (подготовленное на предыдущем шаге)."
"2. Копия* свидетельства о постановке на учет в налоговом органе (ИНН)."
...
Процедура ВыгрузитьКорневойСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(Неопределено,
		АдресКорневогоСертификата, НСтр("ka='ძირეული სერთიფიკატი შპს სსც 1C';ru='Корневой сертификат ООО НПЦ 1С';en='Root certificate Ltd. SPC 1C'"));
...
Процедура НапечататьДокументы(Команда)
	...
	НазваниеПечатнойФормы = НСтр("ka='განცხადება სერთიფიკატის გამოშვების თაობაზე';ru='Заявление на выпуск сертификата';en='Application for certificate issuance'");
...
Процедура ПерейтиНаСтраницуСоглашение()
	...
	Элементы.ФормаЗакрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
...
Процедура ПерейтиНаСтраницуПодготовкаЗапросаНаСертификатНаСервереПриЗагрузке()
	...
	Элементы.ФормаНазад.Видимость = Истина;
	Элементы.ФормаЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявления()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Поле ""Программа электронной подписи"" не заполнено';ru='Поле ""Программа электронной подписи"" не заполнено';en='The ""e-signature program"" not filled'"), , "Программа");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявленияПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='Создание ключа электронной подписи';ru='Создание ключа электронной подписи';en='Creating a digital signature key'"), "", Результат, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуОжиданиеОбработкиЗаявления()
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Отправить",   НСтр("ka='გაგზავნა';ru='Отправить';en='to send'"));
	Кнопки.Добавить("НеОтправить", НСтр("ka='Не отправлять';ru='Не отправлять';en='Do not send'"));
	
	Текст = "";
	Если Не ДокументыПечатались Тогда
		Текст =
			НСтр("ka='დოკუმენტები ჯერ არ დაბეჭდილა. განცხადება მიღებული არ იქნება, ვიდრე ბეჭდური დოკუმენტები არ იქნება მიღებული.';ru='Документы еще не печатались."
...
"Заявление может обрабатываться дольше обычного.';ka='მომსახურე ორგანიზაციის გსნ არ არის მითითებული."
"განცხადების დამუშავებას შეიძლება ჩვეულებრივზე მეტი დრო დაჭირდეს.'");
		КонецЕсли;
...
"Заявление может обрабатываться дольше обычного.';ka='მომსახურე ორგანიზაციის გსნ ან აამკ არ არის მითითებული. განცხადების დამუშავებას შეიძლება ჩვეულებრივზე მეტი დრო დაჭირდეს.'");
	КонецЕсли;
...
"Библиотека интернет поддержки пользователей не внедрена в конфигурацию.';ka='Невозможно подключиться к порталу интернет-поддержки по причине:"
...
Функция ПерейтиНаСтраницуОжиданиеОбработкиЗаявленияНаСервере(ОписаниеОшибки)
	...
	СостояниеОбработкиЗаявления = НСтр("ka='განცხადება მიღებულია დასამუშავებლად.';ru='Заявление принято для обработки.';en='The application is accepted for processing.'");
...
Процедура ОбновитьДатаОбновленияСостоянияВЗаголовке(Форма)
	...
	Форма.Элементы.СостояниеОбработкиЗаявления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Состояние обработки заявления (на %1)';ru='Состояние обработки заявления (на %1)';en='Status processing application (% 1)'"),
...
Процедура ПерейтиНаСтраницуУстановкаСертификата()
	...
	Если Результат <> Истина Тогда
		ПослеЗаписи(НСтр("ka='განცხადება უარყოფილია.';ru='Заявление отклонено.';en='Application is rejected.'"));
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуУстановкаСертификатаПослеУстановки(Установлен, Неопределен) Экспорт
	...
	Если Установлен Тогда
		...
		ПослеЗаписи(НСтр("ka='განცხადება შესრულებულია.';ru='Заявление исполнено.';en='The statement executed.'"));
		Закрыть();
	Иначе
		ПослеЗаписи();
		ЗаписатьЗаявление();
...
Процедура ОбновитьДатаУстановкиСертификатаВЗаголовке(Форма)
	...
	Форма.Элементы.ОшибкаУстановкиСертификата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка установки сертификата (на %1)';ru='Ошибка установки сертификата (на %1)';en='Certificate installation error (% 1)'"),
...
Процедура ЗакрытьПомощникПослеУстановкиСертификатаЗавершение(Установлен, Неопределен) Экспорт
	...
	ПослеЗаписи(НСтр("ka='სერტიფიკატი დადგენილია.';ru='Сертификат установлен.';en='The certificate is installed.'"));
	Закрыть();
...
Процедура ПриИзмененииОрганизацииНаСервере(Загрузка = Ложь)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Элементы.ОГРН.Заголовок = НСтр("ka='ОГРНИП';ru='ОГРНИП';en='OGRN'");
	Иначе
		Элементы.КПП.Видимость = Истина;
		Элементы.ОГРН.Заголовок = НСтр("ka='ძსსნ';ru='ОГРН';en='BIN'");
	КонецЕсли;
	...
	Попытка
		...
		Если ПодробныйИтог.Результат <> "Корректный" Тогда
			...
			Если Не ЗначениеЗаполнено(Сообщение) Тогда
				Сообщение = НСтр("ka='მისამართი არ არის შევსებული';ru='Адрес не заполнен';en='Address is empty'");
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Функция РасчетныйСчетСоответствуетТребованиям(РасчетныйСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru='Расчетный счет должен состоять только из цифр.';en='Current account shall consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
		...
		Если Не МодульРегламентированныеДанныеКлиентСервер.КонтрольныйКлючЛицевогоСчетаСоответствуетТребованиям(Значение, БИК, ЭтоБанк) Тогда
			ТекстСообщения = НСтр("ka='ანგარიშის საკონტროლო რიცხვი ბსკ-ის გამოანგარიშებულ აღრიცხვას არ ემთხვევა';ru='Контрольное число счета не совпадает с рассчитанным с учетом БИК';en='Control account number does not coincide with that calculated based on BIC'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция БИКСоответствуетТребованиям(БИК, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ka='ბსკ მხოლოდ ციფრებისგან უნდა შედგებოდეს.';ru='БИК должен состоять только из цифр.';en='BIC must consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 9 Тогда
		ТекстСообщения = НСтр("ka='ბსკ უნდა შედგებოდეს 9 ციფრისგან';ru='БИК должен состоять из 9 цифр.';en='BIC should consist of 9 digits.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция КорреспондентскийСчетСоответствуетТребованиям(КорреспондентскийСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ka='საკორესპონდენტო ანგარიში მხოლოდ ციფრებისგან უნდა შედგებოდეს.';ru='Корреспондентский счет должен состоять только из цифр.';en='Correspondent account shall consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;

	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("ka='საკორესპონდენტო ანგარიში 20 ციფრისგან უნდა შედგებოდეს.';ru='Корреспондентский счет должен состоять из 20 цифр.';en='Correspondent account should consist of 20 digits.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция АдресСоответствуетТребованиям(Знач АдресXML, ТекстСообщения)
	...
	Если Не АдресСтруктура.Свойство("КодРегиона") Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='Это не российский адрес';ru='Это не российский адрес';en='This is not a Russian address'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что указан регион.
	Если Не АдресСтруктура.Свойство("Регион") Или Не ЗначениеЗаполнено(АдресСтруктура.Регион) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='რაიონი არ არის მითითებული';ru='Не указан регион';en='Unknown region'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что регион указан правильно - код региона определен.
	Если Не ЗначениеЗаполнено(АдресСтруктура.КодРегиона) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='არაკორექტული რაიონი (რაიონის კოდი არ არის დადგენილი)';ru='Некорректный регион (код региона не определен)';en='Invalid region (region code is not specified)'");
		Возврат Ложь; 
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НаселенныйПунктПолностью) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='დასახლებული პუნქტი არ არის მითითებული';ru='Не указан населенный пункт';en='Unknown location'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ТелефонСоответствуетТребованиям(Знач ТелефонXML, ТекстСообщения)
	...
	Если СтрЗаменить(ТелефонСтруктура.КодСтраны, "+", "") <> "7" Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='ქვეყნის კოდი არ არის საქართველოსი (უნდა იყოს ""7"")';ru='Код страны не российский (должен быть ""7"")';en='The country code is not Russian (must be ""7"")'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НомерТелефонаБезКодаСтраны) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='ტელეფონის ნომერი არ არის შევსებული';ru='Не заполнен номер телефона';en='the phone number is not filled'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(ТолькоЦифры(НомерТелефонаБезКодаСтраны)) <> 10 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='ტელეფონის ნომერი ქალაქის კოდიანად 10 ციფრისგან უნდა შედგებოდეს';ru='Номер телефона с кодом города должен состоять из 10-и цифр';en='Phone number with area code must consist of 10 figures'");
		Возврат Ложь;
	КонецЕсли;
		...
		Если Не ЭтоИндивидуальныйПредприниматель Тогда
			Если ЗначениеЗаполнено(Реквизиты.Директор) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.Директор, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='დირექტორი: %1';ru='Директор: %1';en='Director: 1%'"), Реквизиты.Директор));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Реквизиты.ГлавныйБухгалтер) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.ГлавныйБухгалтер, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მთავარი ბუღალტერი: %1';ru='Главный бухгалтер: %1';en='Chief Accountant: 1%'"), Реквизиты.ГлавныйБухгалтер));
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоИндивидуальныйПредприниматель Тогда
			Элементы.Сотрудник.Заголовок = НСтр("ka='ინდივიდუალური მეწარმე';ru='Индивидуальный предприниматель';en='Individual entrepreneur'");
		Иначе
			Элементы.Сотрудник.Заголовок = НСтр("ka='თანამშრომელი';ru='Сотрудник';en='Employee'");
		КонецЕсли;
...
Процедура ПриИзмененииВидаДокументаНаСервере()
	...
	Если ДокументВид = "21" Тогда
		Элементы.ДокументНомер.Заголовок = НСтр("ka='სერია და ნომერი';ru='Серия и номер';en='Series and number'");
		Элементы.ДокументНомер.Маска = "99 99 999999";
		Элементы.ДокументКодПодразделения.Видимость = Истина;
	Иначе
		Элементы.ДокументНомер.Заголовок = НСтр("ka='ნომერი';ru='Номер';en='room'");
...
Функция НомерПаспортаРФСоответствуетТребованиям(Знач НомерПаспортаРФ, ТекстСообщения)
	...
	Если ПустаяСтрока(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='პასპორტის ნომერი არ არის შევსებული';ru='Номер паспорта не заполнен';en='passport number is empty'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(СтрокаЦифр) < 10 Тогда
		ТекстСообщения  =  ТекстСообщения + НСтр("ka='პასპორტის ნომერი სრულად არის მიცემული';ru='Номер паспорта задан неполностью';en='Passport number given incompletely'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		Результат = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ka='პასპორტის ნომერი მხოლოდ ციფრებისგან უნდა შედგებოდეს.';ru='Номер паспорта должен состоять только из цифр.';en='passport number must consist of numbers only.'");
	КонецЕсли;
...
Функция КодПодразделенияВыдачиПаспортаРФСоответствуетТребованиям(КодПодразделения, ТекстСообщения)
	...
	Если СтрДлина(СтрокаЦифр) < 6 Тогда
		ТекстСообщения  = НСтр("ru='Код подразделения задан неполностью';en='Code division incompletely specified'");
		Возврат Ложь;
	КонецЕсли;
	
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		ТекстСообщения = НСтр("ru='Код подразделения должен состоять только из цифр.';en='department code must consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрокаЦифр = "000000" Тогда
		ТекстСообщения = НСтр("ru='Код подразделения не может быть нулевым.';en='Code division can not be zero.'");
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		СписокВыбораТипов.ПоказатьВыборЭлемента(
			...
			НСтр("ka='მონაცემთა ტიპის არჩევა';ru='Выбор типа данных';en='Selecting the type of data'"),
...
Функция ПроверитьРеквизит(Отказ, ИмяРеквизита)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизита]) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Поле ""%1"" не заполнено';en='The ""%1"" filled'"), ЗаголовокПоля), , ИмяРеквизита, , Отказ);
...
Функция ПодготовленныйДокумент()
	...
	Макет.Параметры.УдостоверениеЛичности = ДокументВидПредставление + " " + ДокументНомерПредставление + " "
		+ НСтр("ka='(საიდან, ვისგან)';ru='от';en='From'") + " " + ДокументДатаВыдачиПредставление + " "
		+ НСтр("ka='გაცემული';ru='выданный';en='issued'") + " " + ДокументКемВыдан
		+ ?(ЗначениеЗаполнено(ДокументКодПодразделенияПредставление), ", " + НСтр("ru='код подразделения';en='subdivision code'")
...
"%1';ka='განცხადების გაგზავნა ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'");
...
"%1';ka='Не удалось подключиться к порталу интернет-поддержки по причине:"
"%1'"),
	...
	Если Не ЗначениеЗаполнено(Ответ) Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
			НСтр("ka='Сервер вернул пустой ответ.';ru='Сервер вернул пустой ответ.';en='The server returned an empty response.'"));
		Возврат Ложь;
	КонецЕсли;
...
"Удалите его и создайте новое.';ka='უწინ შექმნილი სერთიფიკატის მოთხოვნის წაკითხვა ვერ მოხერხდა. განცხადება ვერ გაიგზავნება. წაშალეთ იგი და შექმენით ახალი.'");
...
"%1';ka='მდგომარეობის განახლება ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'");
	...
	Если КодРезультата = "0" Тогда
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
				НСтр("ka='Сервер вернул пустые данные.';ru='Сервер вернул пустые данные.';en='The server returned an empty data.'"));
		КонецЕсли;
		
	ИначеЕсли КодРезультата = "1" Тогда
		ОписаниеОшибки = НСтр("ka='განცხადება ჯერ არ არის დამუშავებული, მოგვიანებით სცადეთ.';ru='Заявление еще не обработано, попробуйте позже.';en='The statement has not yet been processed, try again later.'");
	Иначе
		УзелDOM = ПостроительDOM.ПолучитьЭлементыПоИмени("errorMessage");
		Если УзелDOM.Количество() > 0 Тогда
			ОписаниеОшибки = УзелDOM[0].ТекстовоеСодержимое;
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Сервер вернул код ошибки: %1';ru='Сервер вернул код ошибки: %1';en='The server returned an error code:% 1'"),
	...
	Попытка
		...
		Если Не ФайлНайден Тогда
			ВызватьИсключение НСтр("ka='მონაცემთა არასწორი ფორმატი.';ru='Неверный формат данных.';en='Invalid data format.'");
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось разобрать ответ сервера по причине:"
"%1';ka='სერვერის პასუხის გარჩევა ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
Процедура УстановитьСертификатПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='Установка сертификата на компьютер';ru='Установка сертификата на компьютер';en='Installing a certificate on a computer'"), "", Менеджер, Новый Структура);
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ka='კომპიუტერზე სერთიფიკატის დასაყენებლად საჭიროა ვებ-კლიენტ 1С:საწარმოსთვის გაფართოების დაყენება.';ru='Для установки сертификата на компьютер требуется"
"установить расширение для веб-клиента 1С:Предприятия.'"),
			НСтр("ka='კომპიუტერზე სერთიფიკატის დასაყენებლად საჭიროა დამატებითი გარე კომპონენტის დაყენება.';ru='Для установки сертификата на компьютер требуется"
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='ფაილებთან სამუშაოდ გაფართოება არ არის დაყენებული.';ru='Не установлено расширение для работы с файлами.';en='It does not set the extension for the files.'");
	Иначе
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='კრიპტოგრაფიასთან სამუშაოდ გაფართოება არ არის დაყენებული.';ru='Не установлено расширение для работы с криптографией.';en='It does not set the extension to work with cryptography.'");
	Иначе
		ОшибкаУстановкиСертификата = НСтр("ka='დამატებითი გარე კომპონენტა არ არის დაყენებული.';ru='Не установлена дополнительная внешняя компонента.';en='Do not set additional external components.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если Не РазрешенияПолучены Тогда
		ОшибкаУстановкиСертификата =
			НСтр("ka='Сохранение сертификатов во временную папку отменено пользователем.';ru='Сохранение сертификатов во временную папку отменено пользователем.';en='Saving certificates temporary folder canceled by the user.'");
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено
	 Или ПолученныеФайлы.Количество() <> 2 Тогда
		...
		ОшибкаУстановкиСертификата = НСтр("ka='Сертификаты не были сохранены во временную папку.';ru='Сертификаты не были сохранены во временную папку.';en='Certificates have not been saved to a temporary folder.'");
...
"Имя контейнера: ""%1""."
"Путь к контейнеру: ""%2"".';ka='კომპიუტერზე გასაღების კონტეინერი ვერ მოიძებნა."
"კონტეინერის სახელი: ""%1""."
"გზა კონტეინერისკენ: ""%2"".'"),
			КонтейнерКлючаИмя,
			КонтейнерКлючаПуть);
	Иначе
		ОшибкаУстановкиСертификата = "";
...
"%1';ka='Не удалось установить личный сертификат в контейнер ключей и хранилище ОС по причине:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке))
...
Процедура УстановитьСертификатПослеВызоваУстановитьСертификатВКонтейнерИХранилище(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='პირადი სერთიფიკატის დადგენა ვერ მოხერხდა.';ru='Не удалось установить личный сертификат.';en='Unable to establish a personal certificate.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ka='კომპიუტერზე დაყენებული პირადი სერთიფიკატი ვერ მოიძებნა.';ru='Не удалось найти личный сертификат, установленный на компьютер.';en='Unable to find a personal certificate installed on the computer.'");
		Иначе
...
"%1';ka='კომპიუტერზე დაყენებული პირადი სერთიფიკატის მოძებნა ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'"),
...
Процедура УстановитьСертификатПослеВызоваИмпортироватьСертификат(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='ძირეული სერთიფიკატის დაყენება ვერ მოხერხდა:';ru='Не удалось установить корневой сертификат.';en='Failed to install the root certificate.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаКорневогоСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ka='კომპიუტერზე დაყენებული ძირეული სერთიფიკატი ვერ მოიძებნა.';ru='Не удалось найти корневой сертификат, установленный на компьютер.';en='Could not find the root certificate installed on the computer.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось найти корневой сертификат, установленный на компьютер по причине:"
"%1';ka='კომპიუტერზე დაყენებული ძირეული სერთიფიკატი ვერ მოიძებნა შემდეგი მიზეზით:"
"%1'"),
				Результат.ОписаниеОшибки);
		КонецЕсли;
		ОшибкаУстановкиСертификата = СокрЛ(ОшибкаУстановкиСертификата + Символы.ПС + Символы.ПС) + ОшибкаПоиска;
...
Функция ИмяРегионаРФПоРекомендациямДляСКПЭП(КодРегиона)
	...
	Если Не СтрНачинаетсяС(Строка, КодРегиона + " ") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Регион РФ с кодом ""%1"" не существует.';ru='Регион РФ с кодом ""%1"" не существует.';en='Region of the Russian Federation with the code ""%1"" does not exist.'"),
			КодРегиона);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კოდ ""%1""-ის მქონე საქართველოს რაიონისთვის სახელი, რომელიც რეკომენდებულია ეხშგს-ისთვის, ჯერ არ არის დანიშნული.';ru='Для региона РФ с кодом ""%1"" имя, рекомендованное для СКПЭП, еще не назначено.';en='For the region of Russia with the code ""%1"" is the name recommended for SKPEP not yet scheduled.'"),
			КодРегиона);
	КонецЕсли;
...
Процедура ЗагрузитьЗаявление()
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='განცხადება, რომლის მიხედვით ვერ მოხერხდა სერთიფიკატის მიღება';ru='Заявление, по которому не удалось получить сертификат';en='The statement according to which it was not possible to obtain a certificate'");
		Элементы.ФормаДалее.Видимость = Ложь;
		Элементы.ФормаЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='განცხადება, რომლის მიხედვით მიღებული იქნა სერთიფიკატი';ru='Заявление, по которому был получен сертификат';en='The statement according to which the certificate was received'");
...
Процедура ЗаполнитьРеквизитыЗаявления(Все = Ложь)
	...
	Если Все Тогда
		...
		Строка.Реквизит = НСтр("ka='Программа электронной подписи';ru='Программа электронной подписи';en='Electronic Signature Program'");
		Строка.Значение = ПредставлениеПрограммы;
	КонецЕсли;
	
	// Заполнение сведений об организации.
	ЗаполнитьРеквизит(НСтр("ka='Сведения об организации';ru='Сведения об организации';en='Information about the organization'"),, Истина);
	...
	ЗаполнитьРеквизит(НСтр("ka='Сведения о владельце сертификата';ru='Сведения о владельце сертификата';en='Information about the certificate owner'"),, Истина);
	...
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ka='პირადობის დამადასტურებელი დოკუმენტი';ru='Документ, удостоверяющий личность';en='Identity document'");
	Строка.Значение = Элементы.ДокументВид.СписокВыбора.НайтиПоЗначению(ДокументВид).Представление;
	
	Строка = РеквизитыЗаявления.Добавить();
	Если ДокументВид = "21" Тогда
		Строка.Реквизит = НСтр("ka='სერია და ნომერი';ru='Серия и номер';en='Series and number'");
		Строка.Значение = ПредставлениеРеквизитаНомерПаспортаРФ(ДокументНомер);
	Иначе
		Строка.Реквизит = НСтр("ka='ნომერი';ru='Номер';en='room'");
	...
	Если Не ЭтоИндивидуальныйПредприниматель Тогда
		...
		ЗаполнитьРеквизит(НСтр("ka='Сведения о руководителе';ru='Сведения о руководителе';en='About the head'"),, Истина);
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='ხელმძღვანელის გვარი და სახელი';ru='ФИО руководителя';en='Full name of the head'");
		Строка.Значение = ДокументыРуководитель;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='ხელმძღვანელის თანამდებობა';ru='Должность руководителя';en='as head of'");
		Строка.Значение = ДокументыРуководительДолжность;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='Основание действий руководителя';ru='Основание действий руководителя';en='The base of the head action'");
		Строка.Значение = ДокументыРуководительОснование;
	КонецЕсли;
	
	// Заполнение сведений о печатных документах.
	ЗаполнитьРеквизит(НСтр("ru='Сведения об обслуживающей организации';en='Information about the service organization'"),, Истина);
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ka='პარტნიორის სახელწოდება';ru='Наименование партнера';en='name of partner'");
	Строка.Значение = ДокументыПартнер;
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ka='პარტნიორის გსნ';ru='ИНН партнера';en='INN partner'");
	Строка.Значение = ДокументыПартнерИНН;
	
	Если Не ДокументыПартнерЭтоИП Тогда
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='პარტნიორის ააამკ';ru='КПП партнера';en='PPC partner'");
...
Функция ПредставлениеРеквизитаПол(Пол)
	...
	Если Пол = "Мужской" Тогда
		Возврат НСтр("ka='მამრობითი';ru='Мужской';en='Male'");
		
	ИначеЕсли Пол = "Женский" Тогда
		Возврат НСтр("ka='მდედრობითი';ru='Женский';en='female'");
	КонецЕсли;
...
Процедура УстановитьСоглашение()
	...
	ТекстHTML = СтрЗаменить(ТекстHTML, "%1",
		...
		НСтр("ka='ООО «Научно-производственный центр ""1С""»';ru='ООО «Научно-производственный центр ""1С""»';en='""Research and Production Center"" 1C """"'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%2",
		// Адрес регламента удостоверяющего центра.
		НСтр("ka='http://ca.1c.ru/reglament.pdf';ru='http://ca.1c.ru/reglament.pdf';en='http://ca.1c.ru/reglament.pdf'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%3",
		// Сеть доверенных удостоверяющих центров.
		НСтр("ka='уполномоченного федерального органа в области использования электронной подписи';ru='уполномоченного федерального органа в области использования электронной подписи';en='authorized federal body in the use of electronic signatures'"));
...
Процедура ПослеВыбораПрограммыПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.ПоказатьОшибку Тогда
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
				НСтр("ka='ელექტრონული ხელმოწერის პროგრამის არჩევა';ru='Выбор программы электронной подписи';en='Choosing a digital signature program'"), "", Менеджер, Новый Структура);
		КонецЕсли;
...
Процедура ЗаписатьЗаявление()
	...
	Если ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено Тогда
		...
		ТекущийОбъект.Наименование = СокрП(Фамилия + " " + Имя + " " + Отчество)
			+ " (" + НСтр("ka='განცხადება ახალ სერთიფიკატზე';ru='Заявление на новый сертификат';en='An application for a new certificate'") + ")";
		// Организация.
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		Если ЭтоНовоеСостояние Тогда
			ОбновитьЗначение(Содержание, "ДатаОбновленияСостояния");
			ОбновитьЗначение(Содержание, "СостояниеОбработкиЗаявления");
			НовоеНаименование = СокрП(Фамилия + " " + Имя + " " + Отчество) + " (" + НСтр("ka='წაშლა';ru='Удалить';en='Delete'") + ")";
...
Процедура ПослеЗаписи(ТекстОповещения = "")
	...
	Если Не ЗначениеЗаполнено(ТекстОповещения) Тогда
		ТекстОповещения = НСтр("ka='განცხადება შენახულია.';ru='Заявление сохранено.';en='Statement saved.'");
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификат(Оповещение)
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ka='ელექტრონული ხელმოწერის გასაღების შესაქმნელად და სერთიფიკატის მოსათხოვნად, საჭიროა 1С:საწარმოს ვებ-კლიენტისთვის გაფართოების დაყენება.';ru='Для создания ключа электронной подписи и запроса на сертификат"
"требуется установить расширение для веб-клиента 1С:Предприятия.'"),
			НСтр("ka='ელექტრონული ხელმოწერის გასაღების შესაქმნელად და სერთიფიკატის მოსათხოვნად, საჭიროა დამატებითი გარე კომპონენტის დაყენება.';ru='Для создания ключа электронной подписи и запроса на сертификат"
...
Процедура СоздатьКлючИЗапросНаСертификатПослеПроверкиСуществованияКонтейнераКлючей(Существует, Контекст) Экспорт
	...
	Если Существует Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='გამოიყენება უწინ შექმნილი გასაღების კონტეინერი:';ru='Используется ранее созданный контейнер ключа:';en='Used previously to create a key container is:'"),,
			КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьКонтейнер(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Ошибка.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:"
"%1';ru='Не удалось выполнить операцию по причине:"
"%1'"),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='Создание ключа электронной подписи';ru='Создание ключа электронной подписи';en='Creating a digital signature key'"), "", Ошибка, Новый Структура);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваСоздатьКонтейнер(Путь, ПараметрыВызова, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(Путь) Тогда
		...
		Если Строки[0].Идентификатор = "CryptoPro" Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ელექტრონული ხელმოწერის გასაღები არ არის შექმნილი."
""
"უნდა გავითვალისწინოთ, რომ პროგრამა %1-ის დახმარებით გასაღების შესაქმნელად, საჭიროა საოპერაციო სისტემის ადმინისტრატორის უფლებები.';ru='Ключ электронной подписи не создан."
""
...
"требуются права администратора операционной системы.'"),
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ka='ელექტრონული ხელმოწერის გასაღები არ არის შექმნილი.';ru='Ключ электронной подписи не создан.';en='an electronic signature is not created the key.'"));
		КонецЕсли;
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ka='Создан новый контейнер ключа:';ru='Создан новый контейнер ключа:';en='A new key container:'"),, КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьЗапросНаСертификат(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ka='ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось выполнить операцию по причине:';en='The operation failed due to:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваПолучитьОткрытыйКлюч(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ka='გასაღების ღია ნაწილის მიღება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить открытую часть ключа по причине:';en='Unable to get the open part of the key reasons:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваВычислитьИдентификаторКлючаСубъекта(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ka='სუბიექტის გასაღების იდენტიფიკატორის გამოთვლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось вычислить идентификатор ключа субъекта по причине:';en='Unable to compute subject key identifier for a reason:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке = Неопределено, ЗаголовокОшибки = "")
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='Создание запроса на сертификат';ru='Создание запроса на сертификат';en='Creating the certificate request'"), "", Ошибка, Новый Структура);
		
	ИначеЕсли Не Контекст.ЗапросСоздан Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატის მოთხოვნა არ არის შექმნილი.';ru='Запрос на сертификат не создан.';en='The certificate request is created.'"));
	КонецЕсли;
...
Процедура ПолучитьИмяНовогоКонтейнераКлюча(Оповещение)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Субъект = ПодготовитьСтрокуДляИмениКонтейнера(Фамилия) + " "
			...
			+ НСтр("ka='ინდივიდუალური მეწარმე';ru='Индивидуальный предприниматель';en='Individual entrepreneur'");
	Иначе
...
Процедура СоздатьОбъектКриптографииПослеПодключенияВнешнейКомпоненты(Подключена, Контекст) Экспорт
	...
	Иначе
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Установить",      НСтр("ka='დაყენება';ru='Установить';en='establish'"));
		Кнопки.Добавить("НеУстанавливать", НСтр("ka='არ დავაყენოთ';ru='Не устанавливать';en='Do not install'"));
...
"%1';ka='დამატებითი გარე კომპონენტის დაყენება ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	...
	Если Не Подключена Тогда
		ПоказатьПредупреждение(
			Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке",
				ЭтотОбъект, Контекст),
			НСтр("ka='დამატებითი გარე კომპონენტის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить дополнительную внешнюю компоненту.';en='Unable to connect additional external components.'"));
	Иначе
		СоздатьОбъектКриптографииПродолжение(Контекст);
	КонецЕсли;
...
"%1';ka='კრიპტოგრაფიასთან სამუშაოდ გარე კომპონენტის ობიექტის შექმნა ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
		ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
		Возврат;
...
"%1';ka='ორობით მონაცემებთან სამუშაოდ გარე კომპონენტის ობიექტის შექმნა ვერ მოხერხდა შემდეგი მიზეზით:"
"%1'"),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
			ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
			Возврат;
...
"%1';ka='Программа электронной подписи не доступна через внешнюю компоненту по причине:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
