...
Процедура ЮридическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ЮридическийАдрес", НСтр("ka='ორგანიზაციის იურიდიული მისამართი';ru='Юридический адрес организации'"));
...
Процедура ФактическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ФактическийАдрес", НСтр("ka='ორგანიზაციის ფაქტობრივი მისამართი';ru='Фактический адрес организации'"));
...
Процедура ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеТелефонаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"Телефон", НСтр("ka='ორგანიზაციის ტელეფონი';ru='Телефон организации'"));
	...
	Если Не ЗначениеЗаполнено(ГражданствоОКСМКодАльфа3) Тогда
		Если ЗначениеЗаполнено(Гражданство) Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ქვეყნების კლასიფიკატორის მიხედვით ქვეყანა ""%1""-ის სამნიშნა ასო-ბგერითი კოდი ალფა-3 არ არის მითითებული.';ru='У страны ""%1"" не указан трехзначный буквенный код альфа-3 по ОКСМ.'"),
				Гражданство));
		КонецЕсли;
...
"Выполните добавление в главном узле информационной базы.';ka='პროგრამა %1 ჯერ არ არის გამოყენებადი პროგრამების სიაში დამატებული. მოახდინეთ საინფორმაციო ბაზის მთავარ კვანძში დამატება.'"),
		...
		Иначе
			...
			Кнопки.Добавить("Добавить",    НСтр("ka='დამატება';ru='Добавить'"));
			Кнопки.Добавить("НеДобавлять", НСтр("ka='არ დაიმატოს';ru='Не добавлять'"));
			ПоказатьВопрос(
...
Процедура ВыгрузитьКорневойСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(Неопределено,
		АдресКорневогоСертификата, НСтр("ka='ძირეული სერთიფიკატი შპს სსც 1C';ru='Корневой сертификат ООО НПЦ 1С'"));
...
Процедура НапечататьДокументы(Команда)
	...
	НазваниеПечатнойФормы = НСтр("ka='განცხადება სერთიფიკატის გამოშვების თაობაზე';ru='Заявление на выпуск сертификата'");
...
Процедура ПерейтиНаСтраницуСоглашение()
	...
	Элементы.ФормаЗакрыть.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
...
Процедура ПерейтиНаСтраницуПодготовкаЗапросаНаСертификатНаСервереПриЗагрузке()
	...
	Элементы.ФормаНазад.Видимость = Истина;
	Элементы.ФормаЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявления()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Поле ""Программа электронной подписи"" не заполнено';ru='Поле ""Программа электронной подписи"" не заполнено'"), , "Программа");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявленияПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='Создание ключа электронной подписи';ru='Создание ключа электронной подписи'"), "", Результат, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуОжиданиеОбработкиЗаявления()
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Отправить",   НСтр("ka='გაგზავნა';ru='Отправить'"));
	Кнопки.Добавить("НеОтправить", НСтр("ka='Не отправлять';ru='Не отправлять'"));
...
Функция ПерейтиНаСтраницуОжиданиеОбработкиЗаявленияНаСервере(ОписаниеОшибки)
	...
	СостояниеОбработкиЗаявления = НСтр("ka='განცხადება მიღებულია დასამუშავებლად.';ru='Заявление принято для обработки.'");
...
Процедура ОбновитьДатаОбновленияСостоянияВЗаголовке(Форма)
	...
	Форма.Элементы.СостояниеОбработкиЗаявления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Состояние обработки заявления (на %1)';ru='Состояние обработки заявления (на %1)'"),
...
Процедура ПерейтиНаСтраницуУстановкаСертификата()
	...
	Если Результат <> Истина Тогда
		ПослеЗаписи(НСтр("ka='განცხადება უარყოფილია.';ru='Заявление отклонено.'"));
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуУстановкаСертификатаПослеУстановки(Установлен, Неопределен) Экспорт
	...
	Если Установлен Тогда
		...
		ПослеЗаписи(НСтр("ka='განცხადება შესრულებულია.';ru='Заявление исполнено.'"));
		Закрыть();
	Иначе
		ПослеЗаписи();
		ЗаписатьЗаявление();
...
Процедура ОбновитьДатаУстановкиСертификатаВЗаголовке(Форма)
	...
	Форма.Элементы.ОшибкаУстановкиСертификата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Ошибка установки сертификата (на %1)';ru='Ошибка установки сертификата (на %1)'"),
...
Процедура ЗакрытьПомощникПослеУстановкиСертификатаЗавершение(Установлен, Неопределен) Экспорт
	...
	ПослеЗаписи(НСтр("ka='სერტიფიკატი დადგენილია.';ru='Сертификат установлен.'"));
	Закрыть();
...
Процедура ПриИзмененииОрганизацииНаСервере(Загрузка = Ложь)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Элементы.ОГРН.Заголовок = НСтр("ka='ОГРНИП';ru='ОГРНИП'");
	Иначе
		Элементы.КПП.Видимость = Истина;
		Элементы.ОГРН.Заголовок = НСтр("ka='ძსსნ';ru='ОГРН'");
	КонецЕсли;
	...
	Попытка
		...
		Если ПодробныйИтог.Результат <> "Корректный" Тогда
			...
			Если Не ЗначениеЗаполнено(Сообщение) Тогда
				Сообщение = НСтр("ka='მისამართი არ არის შევსებული';ru='Адрес не заполнен'");
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Функция РасчетныйСчетСоответствуетТребованиям(РасчетныйСчет, ТекстСообщения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
		...
		Если Не МодульРегламентированныеДанныеКлиентСервер.КонтрольныйКлючЛицевогоСчетаСоответствуетТребованиям(Значение, БИК, ЭтоБанк) Тогда
			ТекстСообщения = НСтр("ka='ანგარიშის საკონტროლო რიცხვი ბსკ-ის გამოანგარიშებულ აღრიცხვას არ ემთხვევა';ru='Контрольное число счета не совпадает с рассчитанным с учетом БИК'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция БИКСоответствуетТребованиям(БИК, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ka='ბსკ მხოლოდ ციფრებისგან უნდა შედგებოდეს.';ru='БИК должен состоять только из цифр.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 9 Тогда
		ТекстСообщения = НСтр("ka='ბსკ უნდა შედგებოდეს 9 ციფრისგან';ru='БИК должен состоять из 9 цифр.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция КорреспондентскийСчетСоответствуетТребованиям(КорреспондентскийСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ka='საკორესპონდენტო ანგარიში მხოლოდ ციფრებისგან უნდა შედგებოდეს.';ru='Корреспондентский счет должен состоять только из цифр.'");
		Возврат Ложь;
	КонецЕсли;

	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("ka='საკორესპონდენტო ანგარიში 20 ციფრისგან უნდა შედგებოდეს.';ru='Корреспондентский счет должен состоять из 20 цифр.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция АдресСоответствуетТребованиям(Знач АдресXML, ТекстСообщения)
	...
	Если Не АдресСтруктура.Свойство("КодРегиона") Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='Это не российский адрес';ru='Это не российский адрес'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что указан регион.
	Если Не АдресСтруктура.Свойство("Регион") Или Не ЗначениеЗаполнено(АдресСтруктура.Регион) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='რაიონი არ არის მითითებული';ru='Не указан регион'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что регион указан правильно - код региона определен.
	Если Не ЗначениеЗаполнено(АдресСтруктура.КодРегиона) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='არაკორექტული რაიონი (რაიონის კოდი არ არის დადგენილი)';ru='Некорректный регион (код региона не определен)'");
		Возврат Ложь; 
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НаселенныйПунктПолностью) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='დასახლებული პუნქტი არ არის მითითებული';ru='Не указан населенный пункт'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ТелефонСоответствуетТребованиям(Знач ТелефонXML, ТекстСообщения)
	...
	Если СтрЗаменить(ТелефонСтруктура.КодСтраны, "+", "") <> "7" Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='ქვეყნის კოდი არ არის საქართველოსი (უნდა იყოს ""7"")';ru='Код страны не российский (должен быть ""7"")'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НомерТелефонаБезКодаСтраны) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='ტელეფონის ნომერი არ არის შევსებული';ru='Не заполнен номер телефона'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(ТолькоЦифры(НомерТелефонаБезКодаСтраны)) <> 10 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='ტელეფონის ნომერი ქალაქის კოდიანად 10 ციფრისგან უნდა შედგებოდეს';ru='Номер телефона с кодом города должен состоять из 10-и цифр'");
		Возврат Ложь;
	КонецЕсли;
		...
		Если Не ЭтоИндивидуальныйПредприниматель Тогда
			Если ЗначениеЗаполнено(Реквизиты.Директор) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.Директор, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='დირექტორი: %1';ru='Директор: %1'"), Реквизиты.Директор));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Реквизиты.ГлавныйБухгалтер) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.ГлавныйБухгалтер, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მთავარი ბუღალტერი: %1';ru='Главный бухгалтер: %1'"), Реквизиты.ГлавныйБухгалтер));
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоИндивидуальныйПредприниматель Тогда
			Элементы.Сотрудник.Заголовок = НСтр("ka='ინდივიდუალური მეწარმე';ru='Индивидуальный предприниматель'");
		Иначе
			Элементы.Сотрудник.Заголовок = НСтр("ka='თანამშრომელი';ru='Сотрудник'");
		КонецЕсли;
...
Процедура ПриИзмененииВидаДокументаНаСервере()
	...
	Если ДокументВид = "21" Тогда
		Элементы.ДокументНомер.Заголовок = НСтр("ka='სერია და ნომერი';ru='Серия и номер'");
		Элементы.ДокументНомер.Маска = "99 99 999999";
		Элементы.ДокументКодПодразделения.Видимость = Истина;
	Иначе
		Элементы.ДокументНомер.Заголовок = НСтр("ka='ნომერი';ru='Номер'");
...
Функция НомерПаспортаРФСоответствуетТребованиям(Знач НомерПаспортаРФ, ТекстСообщения)
	...
	Если ПустаяСтрока(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='პასპორტის ნომერი არ არის შევსებული';ru='Номер паспорта не заполнен'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(СтрокаЦифр) < 10 Тогда
		ТекстСообщения  =  ТекстСообщения + НСтр("ka='პასპორტის ნომერი სრულად არის მიცემული';ru='Номер паспорта задан неполностью'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		Результат = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ka='პასპორტის ნომერი მხოლოდ ციფრებისგან უნდა შედგებოდეს.';ru='Номер паспорта должен состоять только из цифр.'");
	КонецЕсли;
...
Процедура ЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		СписокВыбораТипов.ПоказатьВыборЭлемента(
			...
			НСтр("ka='მონაცემთა ტიპის არჩევა';ru='Выбор типа данных'"),
...
Функция ПроверитьРеквизит(Отказ, ИмяРеквизита)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизита]) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Поле ""%1"" не заполнено'"), ЗаголовокПоля), , ИмяРеквизита, , Отказ);
...
Функция ПодготовленныйДокумент()
	...
	Макет.Параметры.УдостоверениеЛичности = ДокументВидПредставление + " " + ДокументНомерПредставление + " "
		+ НСтр("ka='(საიდან, ვისგან)';ru='от'") + " " + ДокументДатаВыдачиПредставление + " "
		+ НСтр("ka='გაცემული';ru='выданный'") + " " + ДокументКемВыдан
...
"%1'"),
	...
	Если Не ЗначениеЗаполнено(Ответ) Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
			НСтр("ka='Сервер вернул пустой ответ.';ru='Сервер вернул пустой ответ.'"));
		Возврат Ложь;
	КонецЕсли;
...
"%1'");
	...
	Если КодРезультата = "0" Тогда
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
				НСтр("ka='Сервер вернул пустые данные.';ru='Сервер вернул пустые данные.'"));
		КонецЕсли;
		
	ИначеЕсли КодРезультата = "1" Тогда
		ОписаниеОшибки = НСтр("ka='განცხადება ჯერ არ არის დამუშავებული, მოგვიანებით სცადეთ.';ru='Заявление еще не обработано, попробуйте позже.'");
	Иначе
		УзелDOM = ПостроительDOM.ПолучитьЭлементыПоИмени("errorMessage");
		Если УзелDOM.Количество() > 0 Тогда
			ОписаниеОшибки = УзелDOM[0].ТекстовоеСодержимое;
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Сервер вернул код ошибки: %1';ru='Сервер вернул код ошибки: %1'"),
	...
	Попытка
		...
		Если Не ФайлНайден Тогда
			ВызватьИсключение НСтр("ka='მონაცემთა არასწორი ფორმატი.';ru='Неверный формат данных.'");
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура УстановитьСертификатПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='Установка сертификата на компьютер';ru='Установка сертификата на компьютер'"), "", Менеджер, Новый Структура);
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='ფაილებთან სამუშაოდ გაფართოება არ არის დაყენებული.';ru='Не установлено расширение для работы с файлами.'");
	Иначе
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='კრიპტოგრაფიასთან სამუშაოდ გაფართოება არ არის დაყენებული.';ru='Не установлено расширение для работы с криптографией.'");
	Иначе
		ОшибкаУстановкиСертификата = НСтр("ka='დამატებითი გარე კომპონენტა არ არის დაყენებული.';ru='Не установлена дополнительная внешняя компонента.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если Не РазрешенияПолучены Тогда
		ОшибкаУстановкиСертификата =
			НСтр("ka='Сохранение сертификатов во временную папку отменено пользователем.';ru='Сохранение сертификатов во временную папку отменено пользователем.'");
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено
	 Или ПолученныеФайлы.Количество() <> 2 Тогда
		...
		ОшибкаУстановкиСертификата = НСтр("ka='Сертификаты не были сохранены во временную папку.';ru='Сертификаты не были сохранены во временную папку.'");
...
Процедура УстановитьСертификатПослеВызоваУстановитьСертификатВКонтейнерИХранилище(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='პირადი სერთიფიკატის დადგენა ვერ მოხერხდა.';ru='Не удалось установить личный сертификат.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ka='კომპიუტერზე დაყენებული პირადი სერთიფიკატი ვერ მოიძებნა.';ru='Не удалось найти личный сертификат, установленный на компьютер.'");
		Иначе
...
Процедура УстановитьСертификатПослеВызоваИмпортироватьСертификат(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ka='ძირეული სერთიფიკატის დაყენება ვერ მოხერხდა:';ru='Не удалось установить корневой сертификат.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаКорневогоСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ka='კომპიუტერზე დაყენებული ძირეული სერთიფიკატი ვერ მოიძებნა.';ru='Не удалось найти корневой сертификат, установленный на компьютер.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ИмяРегионаРФПоРекомендациямДляСКПЭП(КодРегиона)
	...
	Если Не СтрНачинаетсяС(Строка, КодРегиона + " ") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Регион РФ с кодом ""%1"" не существует.';ru='Регион РФ с кодом ""%1"" не существует.'"),
			КодРегиона);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='კოდ ""%1""-ის მქონე საქართველოს რაიონისთვის სახელი, რომელიც რეკომენდებულია ეხშგს-ისთვის, ჯერ არ არის დანიშნული.';ru='Для региона РФ с кодом ""%1"" имя, рекомендованное для СКПЭП, еще не назначено.'"),
			КодРегиона);
	КонецЕсли;
...
Процедура ЗагрузитьЗаявление()
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='განცხადება, რომლის მიხედვით ვერ მოხერხდა სერთიფიკატის მიღება';ru='Заявление, по которому не удалось получить сертификат'");
		Элементы.ФормаДалее.Видимость = Ложь;
		Элементы.ФормаЗакрыть.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='განცხადება, რომლის მიხედვით მიღებული იქნა სერთიფიკატი';ru='Заявление, по которому был получен сертификат'");
...
Процедура ЗаполнитьРеквизитыЗаявления(Все = Ложь)
	...
	Если Все Тогда
		...
		Строка.Реквизит = НСтр("ka='Программа электронной подписи';ru='Программа электронной подписи'");
		Строка.Значение = ПредставлениеПрограммы;
	КонецЕсли;
	
	// Заполнение сведений об организации.
	ЗаполнитьРеквизит(НСтр("ka='Сведения об организации';ru='Сведения об организации'"),, Истина);
	...
	ЗаполнитьРеквизит(НСтр("ka='Сведения о владельце сертификата';ru='Сведения о владельце сертификата'"),, Истина);
	...
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ka='პირადობის დამადასტურებელი დოკუმენტი';ru='Документ, удостоверяющий личность'");
	Строка.Значение = Элементы.ДокументВид.СписокВыбора.НайтиПоЗначению(ДокументВид).Представление;
	
	Строка = РеквизитыЗаявления.Добавить();
	Если ДокументВид = "21" Тогда
		Строка.Реквизит = НСтр("ka='სერია და ნომერი';ru='Серия и номер'");
		Строка.Значение = ПредставлениеРеквизитаНомерПаспортаРФ(ДокументНомер);
	Иначе
		Строка.Реквизит = НСтр("ka='ნომერი';ru='Номер'");
	...
	Если Не ЭтоИндивидуальныйПредприниматель Тогда
		...
		ЗаполнитьРеквизит(НСтр("ka='Сведения о руководителе';ru='Сведения о руководителе'"),, Истина);
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='ხელმძღვანელის გვარი და სახელი';ru='ФИО руководителя'");
		Строка.Значение = ДокументыРуководитель;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='ხელმძღვანელის თანამდებობა';ru='Должность руководителя'");
		Строка.Значение = ДокументыРуководительДолжность;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='Основание действий руководителя';ru='Основание действий руководителя'");
	...
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ka='პარტნიორის სახელწოდება';ru='Наименование партнера'");
	Строка.Значение = ДокументыПартнер;
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ka='პარტნიორის გსნ';ru='ИНН партнера'");
	Строка.Значение = ДокументыПартнерИНН;
	
	Если Не ДокументыПартнерЭтоИП Тогда
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ka='პარტნიორის ააამკ';ru='КПП партнера'");
...
Функция ПредставлениеРеквизитаПол(Пол)
	...
	Если Пол = "Мужской" Тогда
		Возврат НСтр("ka='მამრობითი';ru='Мужской'");
		
	ИначеЕсли Пол = "Женский" Тогда
		Возврат НСтр("ka='მდედრობითი';ru='Женский'");
	КонецЕсли;
...
Процедура УстановитьСоглашение()
	...
	ТекстHTML = СтрЗаменить(ТекстHTML, "%1",
		...
		НСтр("ka='ООО «Научно-производственный центр ""1С""»';ru='ООО «Научно-производственный центр ""1С""»'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%2",
		// Адрес регламента удостоверяющего центра.
		НСтр("ka='http://ca.1c.ru/reglament.pdf';ru='http://ca.1c.ru/reglament.pdf'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%3",
		// Сеть доверенных удостоверяющих центров.
		НСтр("ka='уполномоченного федерального органа в области использования электронной подписи';ru='уполномоченного федерального органа в области использования электронной подписи'"));
...
Процедура ПослеВыбораПрограммыПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.ПоказатьОшибку Тогда
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
				НСтр("ka='ელექტრონული ხელმოწერის პროგრამის არჩევა';ru='Выбор программы электронной подписи'"), "", Менеджер, Новый Структура);
		КонецЕсли;
...
Процедура ЗаписатьЗаявление()
	...
	Если ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено Тогда
		...
		ТекущийОбъект.Наименование = СокрП(Фамилия + " " + Имя + " " + Отчество)
			+ " (" + НСтр("ka='განცხადება ახალ სერთიფიკატზე';ru='Заявление на новый сертификат'") + ")";
		// Организация.
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		Если ЭтоНовоеСостояние Тогда
			ОбновитьЗначение(Содержание, "ДатаОбновленияСостояния");
			ОбновитьЗначение(Содержание, "СостояниеОбработкиЗаявления");
			НовоеНаименование = СокрП(Фамилия + " " + Имя + " " + Отчество) + " (" + НСтр("ka='წაშლა';ru='Удалить'") + ")";
...
Процедура ПослеЗаписи(ТекстОповещения = "")
	...
	Если Не ЗначениеЗаполнено(ТекстОповещения) Тогда
		ТекстОповещения = НСтр("ka='განცხადება შენახულია.';ru='Заявление сохранено.'");
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификатПослеПроверкиСуществованияКонтейнераКлючей(Существует, Контекст) Экспорт
	...
	Если Существует Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='გამოიყენება უწინ შექმნილი გასაღების კონტეინერი:';ru='Используется ранее созданный контейнер ключа:'"),,
			КонтейнерКлючаПуть);
...
"%1'"),
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='Создание ключа электронной подписи';ru='Создание ключа электронной подписи'"), "", Ошибка, Новый Структура);
...
"требуются права администратора операционной системы.'"),
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ka='ელექტრონული ხელმოწერის გასაღები არ არის შექმნილი.';ru='Ключ электронной подписи не создан.'"));
		КонецЕсли;
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ka='Создан новый контейнер ключа:';ru='Создан новый контейнер ключа:'"),, КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьЗапросНаСертификат(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ka='ოპერაციის შესრულება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось выполнить операцию по причине:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваПолучитьОткрытыйКлюч(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ka='გასაღების ღია ნაწილის მიღება ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось получить открытую часть ключа по причине:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваВычислитьИдентификаторКлючаСубъекта(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ka='სუბიექტის გასაღების იდენტიფიკატორის გამოთვლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось вычислить идентификатор ключа субъекта по причине:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке = Неопределено, ЗаголовокОшибки = "")
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='Создание запроса на сертификат';ru='Создание запроса на сертификат'"), "", Ошибка, Новый Структура);
		
	ИначеЕсли Не Контекст.ЗапросСоздан Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატის მოთხოვნა არ არის შექმნილი.';ru='Запрос на сертификат не создан.'"));
	КонецЕсли;
...
Процедура ПолучитьИмяНовогоКонтейнераКлюча(Оповещение)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Субъект = ПодготовитьСтрокуДляИмениКонтейнера(Фамилия) + " "
			...
			+ НСтр("ka='ინდივიდუალური მეწარმე';ru='Индивидуальный предприниматель'");
	Иначе
...
Процедура СоздатьОбъектКриптографииПослеПодключенияВнешнейКомпоненты(Подключена, Контекст) Экспорт
	...
	Иначе
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Установить",      НСтр("ka='დაყენება';ru='Установить'"));
		Кнопки.Добавить("НеУстанавливать", НСтр("ka='არ დავაყენოთ';ru='Не устанавливать'"));
	...
	Если Не Подключена Тогда
		ПоказатьПредупреждение(
			Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке",
				ЭтотОбъект, Контекст),
			НСтр("ka='დამატებითი გარე კომპონენტის შეერთება ვერ მოხერხდა.';ru='Не удалось подключить дополнительную внешнюю компоненту.'"));
	Иначе
		СоздатьОбъектКриптографииПродолжение(Контекст);
	КонецЕсли;
