...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого КлючИЗначение Из ПараметрыРеквизитов Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი %1 არ არის შევსებული.';ru='Поле %1 не заполнено.';en='Field 1% unfilled.'"),
...
Процедура ПроверитьСертификат(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატი ჯერ არ არის ჩაწერილი.';ru='Сертификат еще не записан.';en='The certificate has not yet been recorded.'"));
		Возврат;
	КонецЕсли;
...
Процедура СертификатОтозван(Команда)
	...
	Если Объект.Отозван Тогда
		ПоказатьПредупреждение(, НСтр("ka='ჩაწერის შემდეგ მოთხოვნის გაუქმება შეუძლებელი იქნება ';ru='После записи отменить отзыв будет невозможно.';en='After recording, cancel the review will not be possible.'"));
	КонецЕсли;
