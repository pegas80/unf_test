...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого КлючИЗначение Из ПараметрыРеквизитов Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი %1 არ არის შევსებული.';ru='Поле %1 не заполнено.'"),
...
Процедура ПроверитьСертификат(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატი ჯერ არ არის ჩაწერილი.';ru='Сертификат еще не записан.'"));
		Возврат;
	КонецЕсли;
...
Процедура СертификатОтозван(Команда)
	...
	Если Объект.Отозван Тогда
		ПоказатьПредупреждение(, НСтр("ka='ჩაწერის შემდეგ მოთხოვნის გაუქმება შეუძლებელი იქნება ';ru='После записи отменить отзыв будет невозможно.'"));
	КонецЕсли;
