...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПроверкаПриВыборе Тогда
		Элементы.ФормаПроверить.Заголовок = НСтр("ka='შემოწმება და გაგრძელება';ru='Проверить и продолжить';en='Check and continue'");
	...
	Если Не СтандартныеПроверки Тогда
		Если ДополнительныеПроверки.Количество() = 0 Тогда
			ВызватьИсключение
				НСтр("ka='სერთიფიკატის შესამოწმებლად სტანდარტული შემოწმებები გამორთულია, ამასთან, დამატებითი შემოწმებები არ არის მითითებული.';ru='Для проверки сертификата отключены стандартные проверки,"
...
Процедура Подключаемый_КартинкаНажатие(Элемент)
	...
	Если ЗначениеЗаполнено(Элемент.Подсказка) Тогда
		Если Элемент.Подсказка = НСтр("ka='შემოწმება წარმატებით შესრულდა.';ru='Проверка выполнена успешно.';en='Verification successful.'") Тогда
...
Процедура ПоказатьПредупреждениеОНевозможностиПродолжения()
	...
	ПоказатьПредупреждение(,
		НСтр("ka='გაგრძელება ვერ მოხერხდა, ვინაიდან ყველა მოთხოვნილი შემოწმება არ არის გავლილი.';ru='Не удалось продолжить, т.к. пройдены не все требуемые проверки.';en='Unable to continue because passed all the required checks.'"));
...
Процедура ПроверитьНаСторонеКлиентаПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если Не Элементы.ГруппаЗаконныйСертификат.Видимость
	 Или Контекст.СертификатКриптографии.Субъект.Свойство("SN") Тогда
		...
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ka='სერთიფიკატის სუბიექტის აღწერილობაში ველი ""SN"" ვერ მოიძებნა.';ru='В описании субъекта сертификата не найдено поле ""SN"".';en='In the description of the subject of the certificate is not found field, ""SN"".'");
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ka='ხელმოწერის, შექმნილი ხელმოწერის და გაშიფრვის შემოწმება ვერ შესრულდება.';ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';en='Checking the signature created signature and decryption can not be performed.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	ИначеЕсли Результат.Сертификат = Неопределено Тогда
		ОписаниеОшибки = НСтр("ka='სერთიფიკატების საცავში სერთიფიკატი ვერ მოიძებნა.';ru='Сертификат не найден в хранилище сертификатов.';en='The certificate is not found in the certificate store.'") + Символы.ПС
			+ НСтр("ka='ხელმოწერის, შექმნილი ხელმოწერის და გაშიფრვის შემოწმება ვერ შესრულდება.';ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';en='Checking the signature created signature and decryption can not be performed.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ka='დახურული გასაღების შესამოწმებელი პროგრამა სერთიფიკატში არ არის მითითებული.';ru='Программа для использования закрытого ключа не указана в сертификате.';en='A program to use the private key is not specified in the certificate.'");
...
"расшифровки не могут быть выполнены.';ka='ხელმოწერის, შექმნილი ხელმოწერის, დაშიფრვის და გაშიფრვის შემოწმება ვერ შესრულდება.'");
	КонецЕсли;
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере, ОписаниеОшибки = Неопределено, ЭтоОшибка = Ложь)
	...
	Если ОписаниеОшибки = Неопределено Тогда
		...
		ЭлементКартинка.Подсказка   = НСтр("ka='შემოწმება არ შესრულებულა.';ru='Проверка не выполнялась.';en='Check is not performed.'");
	...
	Иначе
		...
		ЭлементКартинка.Подсказка   = НСтр("ka='შემოწმება წარმატებით შესრულდა.';ru='Проверка выполнена успешно.';en='Verification successful.'");;
