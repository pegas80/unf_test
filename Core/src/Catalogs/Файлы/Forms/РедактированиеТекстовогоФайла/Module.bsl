...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РежимРедактирования Тогда
		ЗаголовокСтрока = ЗаголовокСтрока + " " + НСтр("ka='(მხოლოდ დათვალიერება)';ru='(только просмотр)'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеФайла.Версия) Тогда
		...
		Если ЗначениеЗаполнено(КодировкаТекстаФайла) Тогда
			...
			КонецЕсли;
		Иначе
			КодировкаПредставление = НСтр("ka='ძირითადი მნიშვნელობა';ru='По умолчанию'");
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Изменения в файле ""%1"" будут потеряны.';ru='Изменения в файле ""%1"" будут потеряны.'"), ИмяИРасширение);
		Возврат;
	КонецЕсли;

	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილი ""%1"" შეიცვალა.';ru='Файл ""%1"" был изменен.'"), 
		ИмяИРасширение);
...
Процедура СохранитьКак(Команда)
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		Состояние(НСтр("ka='ფაილი წარმატებით იქნა შებახული';ru='Файл успешно сохранен'"), , ВыбранноеПолноеИмяФайла);
...
Процедура ПоказатьОтличия(Команда)
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ka='ვერსიების შედარებას ვებ-კლიენტში მხარდაჭერა არ გააჩნია.';ru='Сравнение версий в веб-клиенте не поддерживается.'"));
	#Иначе
