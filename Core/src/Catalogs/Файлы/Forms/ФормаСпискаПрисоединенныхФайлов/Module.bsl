...
Процедура ИмпортФайлов(Команда)
	#Если ВебКлиент Тогда
		ТекстПредупреждения =  НСтр("ka='ვებ-კლიენტში საქაღალდეების იმპორტს მხარდაჭერა არ გააჩნია. ფაილების სიაში გამოიყენეთ ბრძანება ""შექმნა"".';ru='В Веб-клиенте импорт файлов не поддерживается."
...
// Параметры:
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл") И Не ПараметрыПеретаскивания.Значение.ЭтоФайл() Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ მხოლოდ ფაილები კატალოგების გარეშე.';ru='Выберите только файлы без каталогов.';en='Select only the files without a directory.'"));
		Возврат;
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Массив") Тогда
		...
		Если ТипПеретаскиваемого <> Тип("Файл") Тогда
			ПоказатьПредупреждение(, НСтр("ka='შეარჩიეთ ფაილები';ru='Выберите файлы.';en='Select the files.'"));
			Возврат;
		КонецЕсли;
