...
"Используйте команду ""Создать"" в списке файлов.'");
	...
	Если ТолькоКаталоги Тогда
		Заголовок = НСтр("ka='ფოლდერების ატვირთვა';ru='Загрузка папок'");
	КонецЕсли;
...
Процедура ЗагрузитьФайлы()
	...
	Если ВыбранныеФайлы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='დასამატებლად ფაილები არ არის.';ru='Нет файлов для добавления.'"), , "ВыбранныеФайлы");
		ПоляНеЗаполнены = Истина;
	КонецЕсли;
	
	Если ПапкаДляДобавления.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მიუთითეთ ფოლდერი';ru='Укажите папку.'"), , "ПапкаДляДобавления");
		ПоляНеЗаполнены = Истина;
	КонецЕсли;
