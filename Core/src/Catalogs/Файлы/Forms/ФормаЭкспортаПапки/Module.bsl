...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ka='В Веб-клиенте экспорт папок не поддерживается.';ru='В Веб-клиенте экспорт папок не поддерживается.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
...
Процедура ПапкаДляЭкспортаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Элемент.ТекстРедактирования) Тогда
		...
		Если Не Файл.Существует() Тогда
			ТекстПредупреждения = НСтр("ka='Невозможно открыть папку, так как она не существует.';ru='Невозможно открыть папку, так как она не существует.'");
...
"Выберите другую папку.'"),
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Попытка
			...
			Если Не КаталогВыгрузки.Существует() Тогда
				ВызватьИсключение НСтр("ka='წარმატებული მოქმედების შემდეგ ქვესაქაღალდე არ მოიძებნა';ru='После успешного создания подпапка не найдена.'");
			КонецЕсли;
		Исключение
...
Процедура ОбойтиДеревоФайлов4(ПараметрыВыполнения)
	...
	Исключение
		ТекстОшибки = НСтр("ka='საქაღალდე ""%1""-ის შექმნის შეცდომა:';ru='Ошибка создания папки ""%1"":'");
...
Процедура ОбойтиДеревоФайлов14(ПараметрыВыполнения)
	...
	СостояниеЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='საქაღალდე ""%1""-ის ექსპორტი';ru='Экспорт папки ""%1""'"),
		ПараметрыВыполнения.ЗаписьФайла.НаименованиеПапки);
