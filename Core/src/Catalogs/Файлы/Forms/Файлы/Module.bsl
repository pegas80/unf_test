...
Процедура ИмпортФайловПослеУстановкиРасширения(Результат, ПараметрыВыполнения) Экспорт
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='ყველა ფაილი(*.*)|*.*';ru='Все файлы(*.*)|*.*';en='All files (* *.) | * *.'");
	ДиалогОткрытияФайла.МножественныйВыбор = Истина;
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ფაილები';ru='Выберите файлы';en='Select files'");
...
Процедура ИмпортПапки(Команда)
	#Если ВебКлиент Тогда
		ТекстПредупреждения = НСтр("ka='ვებ-კლიენტში საქაღალდეების იმპორტი მიუწვდომელია."
"გამოიყენეთ ბრძანება ""შექმნა"" ფაილების სიაში.';ru='В веб-клиенте импорт папок недоступен."
...
Процедура НайтиВыполнить()
	...
	Если СтрокаПоиска = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ძიებისთვის ტექსტი მითითებული არ არის';ru='Не указан текст для поиска.';en='Unknown search text.'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
"наименование или код которых содержит ""%1"".';ka='ვერ მოხერხდა იმ ფაილის ან საქაღალდის მოძებნა, რომლის სახელწოდება ან კოდი შეიცავს ""%1""-ს.'"),
			СтрокаПоиска);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
...
Процедура ПеренестиВПапку(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок",    НСтр("ka='საქაღალდის არჩევა';ru='Выбор папки';en='Selecting a folder'"));
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.Файлы") Тогда
			...
			Если Действие = ДействиеПеретаскивания.Копирование Тогда
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("ka='ფაილი კპირებილია.';ru='Файл скопирован.';en='The file is copied.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл ""%1"""
"скопирован в папку ""%2""';ka='ფაილი ""%1"" კოპირებულია საქაღალდე ""%2""-ში'"),
						ЗначениеПеретаскивания[0],
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ka='ფაილები კოპირებულია.';ru='Файлы скопированы.';en='Files are copied.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ფაილი (%1 ც.) კოპირებულია საქაღალდე ""%2""-ში';ru='Файлы (%1 шт.) скопированы в папку ""%2""';en='Files (% 1 pc.) Copied to the folder ""%2""'"),
			...
			Иначе
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("ka='ფაილი გადატანილია';ru='Файл перенесен.';en='moved file.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл ""%1"""
"перенесен в папку ""%2""';ka='ფაილი ""%1"" გადატანილია საქაღალდე ""%2""-ში'"),
						Строка(ЗначениеПеретаскивания[0]),
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ka='ფაილიები გადატანილია.';ru='Файлы перенесены.';en='Files transferred.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ფაილები (%1 ც.) გადატანილია საქაღალდე ""%2""-ში';ru='Файлы (%1 шт.) перенесены в папку ""%2""';en='Files (% 1 pc.) Are transferred to folder ""%2""'"),
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.ПапкиФайлов") Тогда
			...
			Если РодительСменен <> Истина Тогда
				Если НайденоЗацикливание = Истина Тогда
					ПоказатьПредупреждение(, НСтр("ka='დონეების დაციკლება';ru='Зацикливание уровней.';en='Looping levels.'"));
				КонецЕсли;
				Возврат;
			КонецЕсли;
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				...
				ОповещениеЗаголовок = НСтр("ka='ფოლდერი გადატანილია.';ru='Папка перенесена.';en='Folder transferred.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Папка ""%1"""
"перенесена в папку ""%2""';ka='საქაღალდე ""%1"" გადატანილია საქაღალდე ""%2""-ში'"),
					Строка(ЗначениеПеретаскивания[0]),
					Строка(ПапкаДляДобавления));
			Иначе
				ОповещениеЗаголовок = НСтр("ka='ფოლდერები გადატანილია';ru='Папки перенесены.';en='Folders have been moved.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='საქაღალდეები (%1 ც.) გადატანილია საქაღალდე ""%2""-ში';ru='Папки (%1 шт.) перенесены в папку ""%2""';en='Folder (1% pc.) Are transferred to folder ""%2""'"),
					Строка(ЗначениеПеретаскивания.Количество()),
					Строка(ПапкаДляДобавления));
			КонецЕсли;
			ПоказатьОповещениеПользователя(ОповещениеЗаголовок, , ОповещениеТекст, БиблиотекаКартинок.Информация32);
