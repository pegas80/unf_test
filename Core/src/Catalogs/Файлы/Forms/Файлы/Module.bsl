...
Процедура ИмпортФайловПослеУстановкиРасширения(Результат, ПараметрыВыполнения) Экспорт
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ka='ყველა ფაილი(*.*)|*.*';ru='Все файлы(*.*)|*.*'");
	ДиалогОткрытияФайла.МножественныйВыбор = Истина;
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ფაილები';ru='Выберите файлы'");
...
Процедура НайтиВыполнить()
	...
	Если СтрокаПоиска = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ძიებისთვის ტექსტი მითითებული არ არის';ru='Не указан текст для поиска.'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура ПеренестиВПапку(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок",    НСтр("ka='საქაღალდის არჩევა';ru='Выбор папки'"));
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.Файлы") Тогда
			...
			Если Действие = ДействиеПеретаскивания.Копирование Тогда
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("ka='ფაილი კპირებილია.';ru='Файл скопирован.'");
...
"скопирован в папку ""%2""';ka='ფაილი ""%1"" კოპირებულია საქაღალდე ""%2""-ში'"),
				...
				Иначе
					ОповещениеЗаголовок = НСтр("ka='ფაილები კოპირებულია.';ru='Файлы скопированы.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ფაილი (%1 ც.) კოპირებულია საქაღალდე ""%2""-ში';ru='Файлы (%1 шт.) скопированы в папку ""%2""'"),
			...
			Иначе
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("ka='ფაილი გადატანილია';ru='Файл перенесен.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл ""%1"""
"перенесен в папку ""%2""';ka='ფაილი ""%1"" გადატანილია საქაღალდე ""%2""-ში'"),
						...
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ka='ფაილიები გადატანილია.';ru='Файлы перенесены.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='ფაილები (%1 ც.) გადატანილია საქაღალდე ""%2""-ში';ru='Файлы (%1 шт.) перенесены в папку ""%2""'"),
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.ПапкиФайлов") Тогда
			...
			Если РодительСменен <> Истина Тогда
				Если НайденоЗацикливание = Истина Тогда
					ПоказатьПредупреждение(, НСтр("ka='დონეების დაციკლება';ru='Зацикливание уровней.'"));
				КонецЕсли;
				Возврат;
			КонецЕсли;
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				...
				ОповещениеЗаголовок = НСтр("ka='ფოლდერი გადატანილია.';ru='Папка перенесена.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Папка ""%1"""
"перенесена в папку ""%2""';ka='საქაღალდე ""%1"" გადატანილია საქაღალდე ""%2""-ში'"),
					...
					Строка(ПапкаДляДобавления));
			Иначе
				ОповещениеЗаголовок = НСтр("ka='ფოლდერები გადატანილია';ru='Папки перенесены.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='საქაღალდეები (%1 ც.) გადატანილია საქაღალდე ""%2""-ში';ru='Папки (%1 шт.) перенесены в папку ""%2""'"),
					Строка(ЗначениеПеретаскивания.Количество()),
					Строка(ПапкаДляДобавления));
			КонецЕсли;
			ПоказатьОповещениеПользователя(ОповещениеЗаголовок, , ОповещениеТекст, БиблиотекаКартинок.Информация32);
