...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РасширениеФайлаВСписке Тогда
		...
		Если Не ЗначениеЗаполнено(Кодировка) Тогда
			Кодировка = НСтр("ka='ძირითადი მნიშვნელობა';ru='По умолчанию';en='Default'");
		КонецЕсли;
...
""
"Скопировать сведения об ЭП в новый файл?';ka='ფაილი ""%1"" ხელმოწერილია."
"ეხ-ის შესახებ ინფორმაციის კოპირება ახალ ფაილში შესაცვლელად მას მიუწვდომელს გახდის."
""
"მოვახდინოთ ეხ-ის შესახებ ინფორმაციის კოპირება ახალ ფაილში?'"),
...
""
"Закрыть карточку?';ka='ფაილი დაკავებულია რედაქტირებისთვის."
""
"დავხუროთ ბარათი?'");
	...
	Если ПредложитьОткрытьФайлДляРедактирования Тогда
		Отказ = Истина;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილ ""%1""-ის გახსნა რედაქტირებისთვის';ru='Открыть файл ""%1"" для редактирования?';en='Open file ""%1"" for editing?'"),
...
Процедура Зашифровать(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда 
		ТекстВопроса =
			НСтр("ka='მონაცემები ჯერ არ არის ჩაწერილი. ბრძანება ""დაშიფრვა""-ს შესრულება შესაძლებელია მხოლოდ მონაცემთა ჩაწერის შემდეგ."
""
"მონაცემები ჩაწერილი იქნება.';ru='Данные еще не записаны. Выполнение команды"
...
Процедура Удалить(Команда)
	...
	ПоказатьВопрос(Обработчик, НСтр("ka='წავშალოთ გამოყოფილი ხელმოწერები?';ru='Удалить выделенные подписи?';en='Delete the selected signatures?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ЗаполнитьСписокПодписей()
	...
	Иначе
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ელექტრონული ხელმოწერები (%1)';ru='Электронные подписи (%1)';en='Electronic signatures (1%)'"),
