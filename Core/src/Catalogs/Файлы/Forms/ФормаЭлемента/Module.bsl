...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РасширениеФайлаВСписке Тогда
		...
		Если Не ЗначениеЗаполнено(Кодировка) Тогда
			Кодировка = НСтр("ka='ძირითადი მნიშვნელობა';ru='По умолчанию'");
		КонецЕсли;
...
"დავხუროთ ბარათი?'");
	...
	Если ПредложитьОткрытьФайлДляРедактирования Тогда
		Отказ = Истина;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილ ""%1""-ის გახსნა რედაქტირებისთვის';ru='Открыть файл ""%1"" для редактирования?'"),
...
Процедура Удалить(Команда)
	...
	ПоказатьВопрос(Обработчик, НСтр("ka='წავშალოთ გამოყოფილი ხელმოწერები?';ru='Удалить выделенные подписи?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ЗаполнитьСписокПодписей()
	...
	Иначе
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ელექტრონული ხელმოწერები (%1)';ru='Электронные подписи (%1)'"),
