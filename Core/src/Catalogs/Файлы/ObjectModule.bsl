	...
	Если ЭтоНовый() Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ДобавлениеФайлов", ВладелецФайла) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საქაღალდე ""%1""-ში ფაილების დასამატებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для добавления файлов в папку ""%1"".'"),
	...
	Иначе
		...
		Если ЗаписьПодписанногоОбъекта <> Истина Тогда
			...
			Если ПодписанЭП И СсылкаПодписан И Занят И Не СсылкаЗанят Тогда
				ВызватьИсключение НСтр("ka='ხელმოწერილი ფაილის რედაქტირება არ შეიძლება';ru='Подписанный файл нельзя редактировать.'");
			КонецЕсли;
			
			Если Зашифрован И СсылкаЗашифрован И ПодписанЭП И НЕ СсылкаПодписан Тогда
				ВызватьИсключение НСтр("ka='დაშიფრული ფაილზე ხელმოწერა არ შეიძლება.';ru='Зашифрованный файл нельзя подписывать.'");
			КонецЕсли;
		...
		Если ИзмененаПометкаУдаления Тогда
			...
			Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ПометкаУдаленияФайлов", ВладелецФайла) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='საქაღალდე ""%1""-ში ფაილების წასაშლელად მოსანიშნად არასაკმარისი უფლებები არის.';ru='Недостаточно прав для пометки файлов на удаление в папке ""%1"".'"),
					Строка(ВладелецФайла));
			КонецЕсли;
