...
Функция ПолучитьОтчет(НаименованиеОтчета = Неопределено)
	...
	Если ПравоДоступаКОтчету = Ложь Тогда
		...
		Сообщение.Текст = НСтр("ka='არასაკმარისი უფლებები.';ru='Недостаточно прав.'");
		Сообщение.Сообщить();
	...
	ИначеЕсли ПравоДоступаКОтчету = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Отчет не найден.';ru='Отчет не найден.'");
		Сообщение.Сообщить();
	...
	Если ТекОтчет = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось получить отчет.';ru='Не удалось получить отчет.'");
		Сообщение.Сообщить();
...
Функция УстановленаСортировкаПоКоду()
	...
	Если НЕ СортировкаПоКоду Тогда
		...
		ПоказатьПредупреждение(,НСтр("ka='Для изменения порядка следования необходимо установить сортировку по коду.';ru='Для изменения порядка следования необходимо установить сортировку по коду.'"));
...
Функция СоздатьНовыйОтчет()
	...
	Если Элементы.Список.ТекущаяСтрока = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შეარჩიეთ ანგარიშგება.';ru='Выберите отчет.'");
		Сообщение.Сообщить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Объект.ЭтоГруппа Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინფორმაციის რეგისტრში ფარულ ანგარიშგებაზე ბმულის ჩაწერა ვერ მოხერხდა: %1';ru='Не удалось записать ссылку на скрытый отчет в регистр сведений. %1'"), ОписаниеОшибки());
...
Функция ПроверитьСписокПередОперацией(СписокРеквизит, СписокЭлемент, УстановленныеОтборы)
	...
	Для Каждого ЭлементГруппировки Из ЭлементыГруппировки Цикл
		Если ЭлементГруппировки.Использование Тогда
			ПоказатьПредупреждение(,НСтр("ka='Изменение порядка элементов невозможно при установленной группировке.';ru='Изменение порядка элементов невозможно при установленной группировке.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
