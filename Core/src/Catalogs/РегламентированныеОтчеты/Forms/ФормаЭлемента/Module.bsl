...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ИсточникОтчета) Тогда
		ПоказатьПредупреждение(,НСтр("ka='Укажите наименование объекта.';ru='Укажите наименование объекта.'"));
	...
	Если Объект.ВнешнийОтчетИспользовать И НЕ ВнешнийОтчетЗарегистрирован Тогда
		ПоказатьПредупреждение(,НСтр("ka='Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".';ru='Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".'"));
		Модифицированность = Истина;
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ОпределитьТипОтчета(ФайлВнешнегоОтчета, Расширение = "", НеВыводитьСообщения = Ложь) Экспорт
	...
	Если НЕ ПустаяСтрока(ФайлВнешнегоОтчета) Тогда
		...
		Если ТипОтчета = 0 Тогда
			...
			Если НЕ НеВыводитьСообщения Тогда
				...
				НСтр("ka='Не найден отчет ""%1"".';ru='Не найден отчет ""%1"".'"), ФайлВнешнегоОтчета);
	...
	Иначе
		...
		Если НЕ НеВыводитьСообщения Тогда
			...
			Сообщение.Текст = НСтр("ka='Не выбран файл внешнего отчета.';ru='Не выбран файл внешнего отчета.'");
...
Процедура ПолеИсточникОтчетаФайлОчистка(Элемент, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Удалить хранящийся в информационной базе отчет?';ru='Удалить хранящийся в информационной базе отчет?'");
