...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ИсточникОтчета) Тогда
		ПоказатьПредупреждение(,НСтр("ka='Укажите наименование объекта.';ru='Укажите наименование объекта.';en='Specify the name of the object.'"));
	...
	Если Объект.ВнешнийОтчетИспользовать И НЕ ВнешнийОтчетЗарегистрирован Тогда
		ПоказатьПредупреждение(,НСтр("ka='Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".';ru='Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".';en='External Report no. Load an external report or set the attribute ""use"" to ""object"".'"));
		Модифицированность = Истина;
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ОпределитьТипОтчета(ФайлВнешнегоОтчета, Расширение = "", НеВыводитьСообщения = Ложь) Экспорт
	...
	Если НЕ ПустаяСтрока(ФайлВнешнегоОтчета) Тогда
		...
		Если ТипОтчета = 0 Тогда
			...
			Если НЕ НеВыводитьСообщения Тогда
				...
				НСтр("ka='Не найден отчет ""%1"".';ru='Не найден отчет ""%1"".';en='Unable to find the report ""%1"".'"), ФайлВнешнегоОтчета);
	...
	Иначе
		...
		Если НЕ НеВыводитьСообщения Тогда
			...
			Сообщение.Текст = НСтр("ka='Не выбран файл внешнего отчета.';ru='Не выбран файл внешнего отчета.';en='Not selected external report file.'");
...
""
"Продолжить действие (не рекомендуется)?';ka='Внимание!"
""
"Отчет не предназначен для использования с текущей конфигурацией."
""
...
Процедура ПолеИсточникОтчетаФайлОчистка(Элемент, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Удалить хранящийся в информационной базе отчет?';ru='Удалить хранящийся в информационной базе отчет?';en='Delete stored in the information database report?'");
