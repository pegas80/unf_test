...
Функция СохранитьИзмененияЗаписейКалендарей(ОбрабатываемыеЭлементы) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Успешно = Ложь;
		ВызватьИсключение СтрШаблон(НСтр("ka='კალენდარში ცვლილებების შენახვა ვერ მოხერხდა შემდეგი მიზეზით: %1';ru='Не удалось сохранить изменения в календаре по причине: %1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура СоставПолейЗаполненияДляРабочегоПроцесса(ТаблицаОписанияПолей, знач ТипДействия) Экспорт
	...
	НовоеОписание.Заголовок					= НСтр("ka='დასახელება';ru='Представление'");
	НовоеОписание.ВариантЗаполнения			= "Указанный";
