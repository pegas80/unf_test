...
Процедура ПодменитьРеквизитыПриСовпаденииКБК(Объект) Экспорт
	...
	Если ДатаАктуальностиКлассификатора() < ПлатежиВБюджетКлиентСервер.НачалоДействияПриказа90н() Тогда
		...
		Если ЗначениеЗаполнено(Объект.ВидНалога) И Объект.ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_ФФОМС Тогда
			...
			Объект.Наименование      = НСтр("ka='Фиксированные взносы в ФФОМС';ru='Фиксированные взносы в ФФОМС';en='Fixed contributions into FFOMS'");
			Если НЕ Объект.НазначениеПлатежаИзменено Тогда
				Объект.НазначениеПлатежа = НСтр("ka='Страховые взносы на ОМС, зачисляемые в бюджет ФФОМС. Регистрационный номер в ПФР <ПФР_ИП>';ru='Страховые взносы на ОМС, зачисляемые в бюджет ФФОМС. Регистрационный номер в ПФР <ПФР_ИП>';en='Insurance premiums for compulsory health insurance to be transferred to the budget FFOMS. Registration number of the FIU <PFR_IP>'");
			КонецЕсли;
		КонецЕсли;
