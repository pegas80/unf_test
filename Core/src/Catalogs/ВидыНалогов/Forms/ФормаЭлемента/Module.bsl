...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредупреждениеПриРедактировании = НСтр("ka='Автоматическое обновление реквизитов будет отключено';ru='Автоматическое обновление реквизитов будет отключено'");
...
Процедура КонструкторКБК(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ВидНалога) И Не ПустаяСтрока(ПредупреждениеПриРедактировании) Тогда
		...
		ТекстВопроса = ПредупреждениеПриРедактировании + "." + Символы.ПС +
			НСтр("ka='რედაქტირების გაგრძელება';ru='Продолжить редактирование?'");
...
Процедура ЗаполнитьРеквизитыПоКБК(Форма)
	...
	Иначе
		...
		Если ПлатежиВБюджетКлиентСервер.ЭтоКБКШтраф(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(ჯარიმა)';ru='(штраф)'");
		ИначеЕсли ПлатежиВБюджетКлиентСервер.ЭтоКБКПени(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(პენი)';ru='(пени)'");
		ИначеЕсли ПлатежиВБюджетКлиентСервер.ЭтоКБКПроценты(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(პროცენტები)';ru='(проценты)'");
		ИначеЕсли ПлатежиВБюджетКлиентСервер.ЭтоКБКПениПроценты(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(პენი, პროცენტები)';ru='(пени, проценты)'");
		КонецЕсли;
...
Функция ТекстДополнительногоПараметра(Знач КБК, Знач ВидНалога)
	...
	Если ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(КБК) Тогда
		ДополнительныйПараметр.Наименование = НСтр("ka='<Период>';ru='<Период>'");
		ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен налоговый период платежа';ru='- текст, вместо которого в назначение платежа будет подставлен налоговый период платежа'");
	...
	Если ПараметрНеЗадан И ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяПенсионнымФондом(КБК) Тогда
		Если ПлатежиВБюджетКлиентСервер.ЭтоФиксированныеВзносы(ВидНалога) Тогда
			...
			ДополнительныйПараметр.Наименование = НСтр("ka='<ПФР_ИП>';ru='<ПФР_ИП>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""'");
			ПараметрНеЗадан = Ложь;
		Иначе
			ДополнительныйПараметр.Наименование = НСтр("ka='<ПФР>';ru='<ПФР>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер'");
			ПараметрНеЗадан = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрНеЗадан И ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяФСС(КБК) Тогда
		Если ПлатежиВБюджетКлиентСервер.ЭтоФиксированныеВзносы(ВидНалога) Тогда
			// Данный параметр поддерживаем только для "поставляемых" налогов
			ДополнительныйПараметр.Наименование = НСтр("ka='<ФСС_ИП>';ru='<ФСС_ИП>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""'");
			ПараметрНеЗадан = Ложь;
		Иначе
			ДополнительныйПараметр.Наименование = НСтр("ka='<ФСС>';ru='<ФСС>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер'");
			ПараметрНеЗадан = Ложь;
		КонецЕсли;
	КонецЕсли;
