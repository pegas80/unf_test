...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредупреждениеПриРедактировании = НСтр("ka='Автоматическое обновление реквизитов будет отключено';ru='Автоматическое обновление реквизитов будет отключено';en='Automatic update is disabled details'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменилисьСчетаВидыНалогов" Тогда
		...
		Объект.СчетУчетаКВозмещениюРегл = Параметр.СчетУчетаКВозмещениюРегл;
...
Процедура КонструкторКБК(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ВидНалога) И Не ПустаяСтрока(ПредупреждениеПриРедактировании) Тогда
		...
		ТекстВопроса = ПредупреждениеПриРедактировании + "." + Символы.ПС +
			НСтр("ka='რედაქტირების გაგრძელება';ru='Продолжить редактирование?';en='Continue editing?'");
...
"Все ручные изменения будут потеряны. Продолжить?';ka='Реквизиты элемента будут заменены значениями из классификатора."
...
Процедура ЗаполнитьРеквизитыПоКБК(Форма)
	...
	Иначе
		...
		Если ПлатежиВБюджетКлиентСервер.ЭтоКБКШтраф(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(ჯარიმა)';ru='(штраф)';en='(fine)'");
		ИначеЕсли ПлатежиВБюджетКлиентСервер.ЭтоКБКПени(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(პენი)';ru='(пени)';en='(Fine)'");
		ИначеЕсли ПлатежиВБюджетКлиентСервер.ЭтоКБКПроценты(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(პროცენტები)';ru='(проценты)';en='(Percent)'");
		ИначеЕсли ПлатежиВБюджетКлиентСервер.ЭтоКБКПениПроценты(Объект.КодБК) Тогда
			НаименованиеПодвидаДохода = НСтр("ka='(პენი, პროცენტები)';ru='(пени, проценты)';en='(Fines, percentage)'");
		КонецЕсли;
...
Функция ТекстДополнительногоПараметра(Знач КБК, Знач ВидНалога)
	...
	Если ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(КБК) Тогда
		ДополнительныйПараметр.Наименование = НСтр("ka='<Период>';ru='<Период>';en='<Period>'");
		ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен налоговый период платежа';ru='- текст, вместо которого в назначение платежа будет подставлен налоговый период платежа';en='- the text, instead of which in the payment function will be replaced by a tax payment period'");
	...
	Если ПараметрНеЗадан И ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяПенсионнымФондом(КБК) Тогда
		Если ПлатежиВБюджетКлиентСервер.ЭтоФиксированныеВзносы(ВидНалога) Тогда
			...
			ДополнительныйПараметр.Наименование = НСтр("ka='<ПФР_ИП>';ru='<ПФР_ИП>';en='<PFR_IP>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""';en='- the text, instead of which in the payment function will be replaced by the registration number ""for themselves""'");
			ПараметрНеЗадан = Ложь;
		Иначе
			ДополнительныйПараметр.Наименование = НСтр("ka='<ПФР>';ru='<ПФР>';en='<FIU>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер';en='- the text, instead of which in the payment function will be replaced by registration number'");
			ПараметрНеЗадан = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрНеЗадан И ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяФСС(КБК) Тогда
		Если ПлатежиВБюджетКлиентСервер.ЭтоФиксированныеВзносы(ВидНалога) Тогда
			// Данный параметр поддерживаем только для "поставляемых" налогов
			ДополнительныйПараметр.Наименование = НСтр("ka='<ФСС_ИП>';ru='<ФСС_ИП>';en='<FSS_IP>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер ""за себя""';en='- the text, instead of which in the payment function will be replaced by the registration number ""for themselves""'");
			ПараметрНеЗадан = Ложь;
		Иначе
			ДополнительныйПараметр.Наименование = НСтр("ka='<ФСС>';ru='<ФСС>';en='<FSS>'");
			ДополнительныйПараметр.Подсказка = НСтр("ka='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер';ru='- текст, вместо которого в назначение платежа будет подставлен регистрационный номер';en='- the text, instead of which in the payment function will be replaced by registration number'");
			ПараметрНеЗадан = Ложь;
		КонецЕсли;
	КонецЕсли;
