...
Функция НаименованиеДокументаРеализацииПолномочий(Знач Требование, ПриложенияТребования) Экспорт
	...
	Если Требование.ВидДокумента = Перечисления.ВидыНалоговыхДокументов.ТребованиеОПредставленииПоясненийКДекларацииНДС Тогда
		...
		Если ВТребованииЕстьXMLФайл(ПриложенияТребования) Тогда
			Наименование = НСтр("ka='Требование %Номер% от %Дата% о представлении пояснений к декларации по НДС %ПредставлениеДекларации%';ru='Требование %Номер% от %Дата% о представлении пояснений к декларации по НДС %ПредставлениеДекларации%';en='The requirement of% Number%% date% of the explanations to the VAT declaration% PredstavlenieDeklaratsii%'");
			Наименование = СтрЗаменить(Наименование, "%ПредставлениеДекларации%", ПредставлениеДекларации(Требование, ПриложенияТребования));
		Иначе
			Наименование = НСтр("ka='Требование %Номер% от %Дата% о представлении пояснений';ru='Требование %Номер% от %Дата% о представлении пояснений';en='The requirement of% Number%% date% of the explanations'");
		КонецЕсли;
...
Функция ПредставлениеДекларации(Требование, ПриложенияТребования = Неопределено) Экспорт
	...
	Если СвойстваДекларации <> Неопределено Тогда
		...
		Представление = НСтр("ka='за %Период% %Год% г.%Уточнение%';ru='за %Период% %Год% г.%Уточнение%';en='in% Period Year%%%% g% Refinement'");
		...
		Если СвойстваДекларации.НомерКорректировки = 0 Тогда
			Уточнение = "";
		Иначе
			Уточнение = НСтр("ka=' (К/%НомерУточнения%)';ru=' (К/%НомерУточнения%)'");
