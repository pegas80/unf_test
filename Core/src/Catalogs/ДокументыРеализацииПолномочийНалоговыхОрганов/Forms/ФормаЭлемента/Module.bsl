...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		ТекстПредупреждения = НСтр("ka='Копирование входящих сообщений запрещено!';ru='Копирование входящих сообщений запрещено!'");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ПолучитьВложениеНаСервере(ИмяФайла)
	...
	Если СтрВложения.Количество() = 0 Тогда
		Результат.ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Вложение с именем  %1 не обнаружено.';ru='Вложение с именем  %1 не обнаружено.'"), Символ(34) + ИмяФайла + Символ(34));
		Возврат Результат;
	КонецЕсли;
...
Процедура ПрорисоватьПанельПриема()
	...
	Если ЭтоТребованиеФНС Тогда
		...
		Элементы.ЗаголовокНеотправленныхСообщений.Заголовок = 
			НСтр("ka='Оператору не отправлено извещение о получении документа';ru='Оператору не отправлено извещение о получении документа'");
...
Процедура УправлениеЭУ(КонтекстЭДОСервер)
	...
	Если ЭтоТребованиеОПредставленииДокументов Тогда
		...
		Элементы.ИнформацияСроки.Заголовок = 
			НСтр("ka='Обратите внимание на срок представления документов, указанный в приложенном файле';ru='Обратите внимание на срок представления документов, указанный в приложенном файле'");
	КонецЕсли;
		
	Если ЭтоТребованиеОПредставленииПояснений Тогда
		
		Элементы.КомандаСоздатьОтвет.Заголовок = 
			НСтр("ka='განმარტებების მომზადება';ru='Подготовить пояснения'");
		Элементы.ИнформацияСроки.Заголовок = 
			НСтр("ka='Ответ на требование должен быть направлен в течение 5 дней с момента получения';ru='Ответ на требование должен быть направлен в течение 5 дней с момента получения'");
