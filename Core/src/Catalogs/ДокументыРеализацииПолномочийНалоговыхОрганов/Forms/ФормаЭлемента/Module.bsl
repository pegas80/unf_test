...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		ТекстПредупреждения = НСтр("ka='Копирование входящих сообщений запрещено!';ru='Копирование входящих сообщений запрещено!';en='Copy incoming messages is forbidden!'");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ПолучитьВложениеНаСервере(ИмяФайла)
	...
	Если СтрВложения.Количество() = 0 Тогда
		Результат.ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Вложение с именем  %1 не обнаружено.';ru='Вложение с именем  %1 не обнаружено.';en='Attachment with the name% 1 was not found.'"), Символ(34) + ИмяФайла + Символ(34));
		Возврат Результат;
	КонецЕсли;
...
Процедура ПрорисоватьПанельПриема()
	...
	Если ЭтоТребованиеФНС Тогда
		...
		Элементы.ЗаголовокНеотправленныхСообщений.Заголовок = 
			НСтр("ka='Оператору не отправлено извещение о получении документа';ru='Оператору не отправлено извещение о получении документа';en='The operator does not sent a notice of receipt of the document'");
...
Процедура УправлениеЭУ(КонтекстЭДОСервер)
	...
	Если ЭтоТребованиеОПредставленииДокументов Тогда
		...
		Элементы.ИнформацияСроки.Заголовок = 
			НСтр("ka='Обратите внимание на срок представления документов, указанный в приложенном файле';ru='Обратите внимание на срок представления документов, указанный в приложенном файле';en='Note the deadline for submission of the documents specified in the attached file'");
	КонецЕсли;
		
	Если ЭтоТребованиеОПредставленииПояснений Тогда
		
		Элементы.КомандаСоздатьОтвет.Заголовок = 
			НСтр("ka='განმარტებების მომზადება';ru='Подготовить пояснения';en='prepare explanations'");
		Элементы.ИнформацияСроки.Заголовок = 
			НСтр("ka='Ответ на требование должен быть направлен в течение 5 дней с момента получения';ru='Ответ на требование должен быть направлен в течение 5 дней с момента получения';en='Response to a request should be sent within 5 days of receipt of'");
