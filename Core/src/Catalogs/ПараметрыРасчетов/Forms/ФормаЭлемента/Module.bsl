...
Функция ЗапросКорректный()
	...
	Попытка
		...
		Если ПостроительЗапроса.Измерения.Количество() > 0 Тогда
			...
			Сообщение.Текст = НСтр("ka='მოთხოვნა არ უნდა შეიცავდეს ჯამებს!';ru='Запрос не должен содержать итоги!'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли; 
		...
		Если ПостроительЗапроса.ВыбранныеПоля.Количество() > 1 Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='მოთხოვნა უნდა შეიცავდეს არა უმეტეს ერთ მაჩვენებელს!';ru='Запрос должен содержать не более одного показателя!'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
...
Функция ПроверкаНаВыборПоказателя()
	...
	Сообщение = Новый СообщениеПользователю();
	Сообщение.Текст = НСтр("ka='არ არის არჩეული მაჩვენებელი!';ru='Не выбран показатель!'");
	Сообщение.Сообщить();
...
Процедура ЗадаватьЗначениеПриРасчетеЗППриИзменении(Элемент)
	...
	Если Объект.ЗадаватьЗначениеПриРасчетеЗП Тогда
		...
		ТекстСообщения = НСтр("ka='ალმის დაყენების შემდეგ ყველა რეკვიზიტები გაიწმინდება? გავაგრძელოთ?';ru='После установки флага все реквизиты будут очищены! Продолжить?'");
...
Процедура ИнициализироватьЗаполнениеПоШаблону(ИмяШаблона)
	...
	ТекстВопроса = НСтр("ka='გაანგარიშების პარამეტრი სრულად იქნება გადავსებული! გავაგრძელოთ ოპერაციის შესრულება?';ru='Параметр расчета будет полностью перезаполнен! Продолжить выполнение операции?'");
