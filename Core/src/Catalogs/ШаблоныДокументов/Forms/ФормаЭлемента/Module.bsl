...
Процедура ПроверитьСогласованностьДисконтнойКарты(Ошибки)
	...
	Если Не ЗначениеЗаполнено(СтрокиКонтрагент) Тогда
		...
		НСтр("ka='დისკონტური ბარათი არ შეიძლება, რომ შაბლონში კონტრაგენტის გარეშე იქნას მითითებული.';ru='Дисконтная карта не может быть указана в шаблоне без контрагента.'"), "",,,
...
Процедура ДобавитьОшибкуНесогласованностиЗначений(Ошибки, ТекСтрока1, ТекСтрока2)
	...
	НСтр("ka='ველ ""%2""-ის მნიშვნელობა ველ ""%1""-ის მნიშვნელობასთან არ არის შეთანხმებული. შეიძლება, რომ ველ ""%1""-ისთვის ალამი არ არის დაყენებული';ru='Значение поля ""%2"" не согласовано со значением поля ""%1"". Возможно, не установлен флажок для поля ""%1""'"),
