...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого Стр Из УчетНоменклатуры Цикл
		Если Стр.ФормированиеЦены = Перечисления.БиллингФормированиеЦеныНоменклатуры.ФиксированноеЗначение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ka='სია ""ნომენკლატურა""-ს სტრიქონ %1-ში სვეტი ""ფასი"" არ არის შევსებული.';ru='Не заполнена колонка ""Цена"" в строке %1 списка ""Номенклатура"".';en='Not filled column ""Price"" on line% 1 ""Nomenclature"" list.'"),
				Стр.НомерСтроки
			);
	...
	Для каждого Стр Из УчетЗатрат Цикл
		...
		Если Стр.ФормированиеЦены = Перечисления.БиллингФормированиеЦеныЗатрат.ФиксированноеЗначение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ka='სია ""დანახარჯები""-ს სტრიქონ %1-ში სვეტი ""ფასი"" არ არის შევსებული.';ru='Не заполнена колонка ""Цена"" в строке %1 списка ""Затраты"".';en='Not filled column ""Price"" on line% 1 list ""costs"".'"),
				Стр.НомерСтроки
			);
