...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+++ Дмгг
	Если Константы.КонтрагентыБезВзаиморасчетовПоЗаказам.Получить() И НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		Объект.ВестиРасчетыПоЗаказам = ЛОЖЬ;
	Иначе  
...
Процедура ДобавитьПолеКонтактнойИнформацииКонтактногоЛица(Команда)
	...
	ДополнительныеПараметры.Вставить("МножественнаяФормаВладельца", НСтр("ka='საკონტაქტო პირები';ru='контактных лиц'"));
...
Процедура УправлениеФормой()
	...
	Если Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо") Тогда
		...
		Элементы.ИНН.ПодсказкаВвода							= НСтр("ka='11 ციფრი';ru='11 цифр'");
		...
		Если КонтрагентЗарегистрированВРФ Тогда
			Элементы.РегистрационныйНомер.Заголовок			= НСтр("ka='ძსსნ';ru='ОГРН'");
			Элементы.РегистрационныйНомер.Подсказка			= НСтр("ka='იურიდიული პირის ძირითადი სახელმწიფო სარეგისტრაციო ნომერი';ru='Основной государственный регистрационный номер юридического лица'");
			Элементы.РегистрационныйНомер.ПодсказкаВвода	= НСтр("ka='13 ციფრი';ru='13 цифр'");
			Элементы.РегистрационныйНомер.ОграничениеТипа	= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
			Элементы.ИНН.Подсказка							= НСтр("ka='გადამხდელის საიდენტიფიკაციო ნომერი';ru='Идентификационный номер налогоплательщика'");
		Иначе
			Элементы.РегистрационныйНомер.Заголовок			= НСтр("ka='სარეგ. ნომერი';ru='Рег. номер'");
			Элементы.РегистрационныйНомер.Подсказка			= НСтр("ka='უცხოური ორგანიზაციისთვის რეგისტრაციის (ინკორპორაციის) ქვეყანაში მინიჭებული სარეგისტრაციო ნომერი';ru='Регистрационный номер, присвоенный иностранной организации в стране регистрации (инкорпорации)'");
			Элементы.РегистрационныйНомер.ПодсказкаВвода	= НСтр("ka='სარეგისტრაციო ნომერი რეგისტრაციის ქვეყანაში';ru='Рег. номер в стране регистрации'");
...
"при первой постановке на учет в налоговом органе Российской Федерации'");
	...
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ИндивидуальныйПредприниматель") Тогда
		...
		Элементы.РегистрационныйНомер.Заголовок				= НСтр("ka='ОГРН ИП';ru='ОГРН ИП'");
...
"указан в Свидетельстве о государственной регистрации физического лица в качестве ИП'");
		Элементы.РегистрационныйНомер.ПодсказкаВвода		= НСтр("ka='15 ციფრი';ru='15 цифр'");
		Элементы.РегистрационныйНомер.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(15));
		Элементы.ИНН.ПодсказкаВвода							= НСтр("ka='11 ციფრი';ru='11 цифр'");
		Элементы.ИНН.ОграничениеТипа						= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(11));
		Элементы.КодПоОКПО.ПодсказкаВвода					= НСтр("ka='13 ციფრი';ru='13 цифр'");
	...
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ГосударственныйОрган") Тогда
		...
		Если ЗначениеЗаполнено(Объект.ВидГосударственногоОргана) Тогда
			...
			Если Объект.ВидГосударственногоОргана = ПредопределенноеЗначение("Перечисление.ВидыГосударственныхОрганов.НалоговыйОрган") Тогда
				...
				Элементы.КодГосударственногоОргана.Заголовок		= НСтр("ka='Код инспекции';ru='Код инспекции'");
			ИначеЕсли Объект.ВидГосударственногоОргана = ПредопределенноеЗначение("Перечисление.ВидыГосударственныхОрганов.ОрганПФР") Тогда
				...
				Элементы.КодГосударственногоОргана.Заголовок		= НСтр("ka='განყოფილების კოდი';ru='Код отделения'");
			...
			Иначе
				...
				Элементы.КодГосударственногоОргана.Заголовок		= НСтр("ka='კოდი';ru='Код'");
			КонецЕсли;
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Объект.Покупатель Тогда
		ВидыОтношений.Добавить(НСтр("ka='მყიდველი';ru='Покупатель'"));
	КонецЕсли;
	
	Если Объект.Поставщик Тогда
		ВидыОтношений.Добавить(НСтр("ka='მომწოდებელი';ru='Поставщик'"));
	КонецЕсли;
	
	Если Объект.ПрочиеОтношения Тогда
		ВидыОтношений.Добавить(НСтр("ka='სხვა ურთიერთობები';ru='Прочие отношения'"));
	КонецЕсли;
	
	Заголовок = Объект.Наименование + " (" + НСтр("ka='კონტრაგენტი';ru='Контрагент'");
...
Процедура ОбновитьЭлементыКонтактныхЛиц()
	...
	Для Каждого ДанныеКонтактногоЛица Из ДанныеКонтактныхЛиц Цикл
		...
		Если ИндексКонтакта > 0 Тогда
			...
			ПолеНаименование.ПодсказкаВвода = НСтр("ka='სახელი გვარი';ru='Имя Фамилия'");
		...
		Для Каждого ДанныеКИ Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			...
			Если КонтактнаяИнформацияУНФ.ДляТипаКонтактнойИнформацииДоступенВводКомментария(ДанныеКИ.Тип) Тогда
				...
				ПолеКомментарий.ПодсказкаВвода = НСтр("ka='Прим.';ru='Прим.'");
	...
	Для Каждого ДанныеКонтактногоЛица Из ДанныеКонтактныхЛиц Цикл
		...
		Для Каждого СтрокаТаблицы Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			Если Не ПустаяСтрока(СтрокаТаблицы.ЗначенияПолей) Или Не ПустаяСтрока(СтрокаТаблицы.Представление) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ФИО контакта не заполнено.';ru='ФИО контакта не заполнено.'"),,,ИмяРеквизита, Отказ);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаТаблицы Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			...
			Если СвойстваВида.ОбязательноеЗаполнение И ПустаяСтрока(СтрокаТаблицы.Представление)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ka='საკონტაქტო ინფორმაციის ტიპი ""%1"" არ არის შევსებული.';ru='Вид контактной информации ""%1"" не заполнен.'"), СвойстваВида.ПредставлениеВида),,, ИмяРеквизита);
...
"Продолжить?'"), ВыбранныйЭлемент.Значение, ДополнительныеПараметры.МножественнаяФормаВладельца);
		ЗаголовокВопроса = НСтр("ka='დამატების დადასტირება';ru='Подтверждение добавления'");
...
Процедура Подключаемый_ДействиеКИКонтактаНажатие(Элемент)
	...
	Если ДанныеКИ.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Skype") Тогда
		...
		Список.Добавить("Позвонить", НСтр("ka='ჩატის დაწყება';ru='Начать чат'"));
		Список.Добавить("НачатьЧат", НСтр("ka='ჩატის დაწყება';ru='Начать чат'"));
...
Процедура СформироватьПредставлениеПроверкиДублей(Форма)
	...
	Если ИННиКППКорректны(Форма) И Не ПустаяСтрока(Объект.ИНН) Тогда
		...
		Если КоличествоДублей > 0 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("ИННиКПП", ?(ЭтоЮрЛицо(Объект.ВидКонтрагента), НСтр("ka='გსნ და აამკ';ru='ИНН и КПП'"), НСтр("ka='ИНН';ru='ИНН'")));
			
			Если КоличествоДублей = 1 Тогда
				СтруктураПараметровСообщенияОДублях.Вставить("КоличествоДублей", НСтр("ka='один';ru='один'"));
				СтруктураПараметровСообщенияОДублях.Вставить("СклонениеКонтрагентов", НСтр("ka='კონტრაგენტი';ru='контрагент'"));
			ИначеЕсли КоличествоДублей < 5 Тогда
				СтруктураПараметровСообщенияОДублях.Вставить("КоличествоДублей", КоличествоДублей);
				СтруктураПараметровСообщенияОДублях.Вставить("СклонениеКонтрагентов", НСтр("ka='კონტრაგენტს';ru='контрагента'"));
			Иначе
				СтруктураПараметровСообщенияОДублях.Вставить("КоличествоДублей", КоличествоДублей);
				СтруктураПараметровСообщенияОДублях.Вставить("СклонениеКонтрагентов", НСтр("ka='კონტარგენტებს';ru='контрагентов'"));
			КонецЕсли;
			
			ОписаниеОшибки = НСтр("ka='С таким [ИННиКПП] есть [КоличествоДублей] [СклонениеКонтрагентов]';ru='С таким [ИННиКПП] есть [КоличествоДублей] [СклонениеКонтрагентов]'");
...
Процедура УстановитьЗаголовокЮридическихДанных(Форма)
	...
	Если Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='იურიდ. პირი';ru='Юр.лицо'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ИндивидуальныйПредприниматель") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ИП';ru='ინდ. მეწარმე'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ФизическоеЛицо") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ფიზ. პირი';ru='Физ.лицо'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ГосударственныйОрган") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='სახ. ორგანო';ru='Гос.орган'"));
	КонецЕсли;
...
Процедура ПрочитатьДанныеПанелиДопИнформации()
	...
	Если ПросмотрПродаж Тогда
		...
		КомпонентыФС = Новый Массив;
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='გაყიდვები';ru='Продажи на'") + " ", КрупныйШрифт));
		...
		КомпонентыФС = Новый Массив;
		КомпонентыФС.Добавить(НСтр("ka='ბოლო გაყიდვა';ru='Последняя продажа'") + " ");
	...
	КомпонентыФС = Новый Массив;
	КомпонентыФС.Добавить(НСтр("ka='ბოლო მოვლენა';ru='Последнее событие'") + " ");
	КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(Формат(Дата, "Л=ru_RU; ДЛФ=D; ДП=<нет>"), , , , НавСсылка));
...
Процедура ПолучитьИнфоИзRS(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ИНН) Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='გსნ არ არის შევსებული';ru='ИНН не заполнено.'"));
		Возврат;
	КонецЕсли;
