...
Процедура ИерархияУстановитьПометкуУдаления(Команда)
	...
	ТекстОповещения = СтрШаблон(НСтр("ka='წაშლის ნიშნული %1';ru='Пометка удаления %1'"),
		?(ПометкаУдаления, НСтр("ka='დაყენებულია';ru='установлена'"), НСтр("ka='მოხსნილია';ru='снята'")));
...
Процедура УстановитьТег(Команда)
	...
	КлассификацияКонтактовКлиент.УстановитьТег(Элементы.Список.ВыделенныеСтроки, НСтр("ka='контрагенту,контрагентам,контрагентам';ru='контрагенту,контрагентам,контрагентам'"));
...
Процедура ПрочитатьИерархию()
	...
	СтрокаДерева.ПредставлениеГруппы = НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>'");
	
	СтрокаДерева = ЭлементыКоллекции.Добавить();
	СтрокаДерева.ИндексПиктограммы = -1;
	СтрокаДерева.ГруппаКонтрагентов = "БезГруппы";
	СтрокаДерева.ПредставлениеГруппы = НСтр("ka='<არ არის ჯგუფი>';ru='<Нет группы>'");
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Форма.ОтборПокупатель Тогда
		ВидыОтношений.Добавить(НСтр("ka='მყიდველები';ru='Покупатели'"));
	КонецЕсли;
	
	Если Форма.ОтборПоставщик Тогда
		ВидыОтношений.Добавить(НСтр("ka='მომწოდებლები';ru='Поставщики'"));
	КонецЕсли;
	...
	Заголовок = НСтр("ka='კონტრაგენტები';ru='Контрагенты'");
