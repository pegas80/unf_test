...
Процедура ИерархияУстановитьПометкуУдаления(Команда)
	...
	ТекстОповещения = СтрШаблон(НСтр("ka='წაშლის ნიშნული %1';ru='Пометка удаления %1';en='Tagging removing 1%'"),
		?(ПометкаУдаления, НСтр("ka='დაყენებულია';ru='установлена';en='installed'"), НСтр("ka='მოხსნილია';ru='снята';en='withdrawn'")));
...
Процедура УстановитьТег(Команда)
	...
	КлассификацияКонтактовКлиент.УстановитьТег(Элементы.Список.ВыделенныеСтроки, НСтр("ka='контрагенту,контрагентам,контрагентам';ru='контрагенту,контрагентам,контрагентам';en='counterparty, contractors, contractors'"));
...
Процедура ПрочитатьИерархию()
	...
	СтрокаДерева.ПредставлениеГруппы = НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'");
	
	СтрокаДерева = ЭлементыКоллекции.Добавить();
	СтрокаДерева.ИндексПиктограммы = -1;
	СтрокаДерева.ГруппаКонтрагентов = "БезГруппы";
	СтрокаДерева.ПредставлениеГруппы = НСтр("ka='<არ არის ჯგუფი>';ru='<Нет группы>';en='<No Group>'");
...
Процедура ПослеВыполненияФоновогоЗадания()
	...
	Элементы.Список.Обновить();
	ПоказатьПредупреждение(,Нстр("ka='კონტრაგენტები. მონაცემთა ჩატვირთვა დასრულებულია.';ru='Контрагенты."
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Форма.ОтборПокупатель Тогда
		ВидыОтношений.Добавить(НСтр("ka='მყიდველები';ru='Покупатели';en='Buyers'"));
	КонецЕсли;
	
	Если Форма.ОтборПоставщик Тогда
		ВидыОтношений.Добавить(НСтр("ka='მომწოდებლები';ru='Поставщики';en='Suppliers'"));
	КонецЕсли;
	...
	Заголовок = НСтр("ka='კონტრაგენტები';ru='Контрагенты';en='contracting parties'");
