...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидГосударственногоОргана = Перечисления.ВидыГосударственныхОрганов.НалоговыйОрган Тогда
		ЭтаФорма.Заголовок = НСтр("ka='საგადასახადო ინსპექციის საგადახდო რეკვიზიტები';ru='Платежные реквизиты налоговой инспекции'");
	ИначеЕсли ВидГосударственногоОргана = Перечисления.ВидыГосударственныхОрганов.ОрганПФР Тогда
		ЭтаФорма.Заголовок = НСтр("ka='საგადახდო რეკვიზიტები საპენსიო ფონდის განყოფილებები';ru='Платежные реквизиты отделения ПФР'");
	ИначеЕсли ВидГосударственногоОргана = Перечисления.ВидыГосударственныхОрганов.ОрганФСС Тогда
		ЭтаФорма.Заголовок = НСтр("ka='საგადახდო რეკვიზიტები სოცდაზღვევის ფონდის განყოფილებები';ru='Платежные реквизиты отделения ФСС'");
	Иначе
		ЭтаФорма.Заголовок = НСтр("ka='საგადახდო რეკვიზიტები';ru='Платежные реквизиты'");
	КонецЕсли;
...
Процедура БИКБанкаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	#Если ВебКлиент Тогда
		...
		Если СтрДлина(Текст) > 9 Тогда
			...
			Сообщение.Текст = НСтр("ka='შეყვანილი მნიშვნელობა აღემატება ბსკ-ის დასაშვებ სიგრძეს 9 სიმბოლოს!';ru='Введенное значение превышает допустимую длину БИК 9 символов!'");
			Сообщение.Сообщить();
...
Функция НайтиБанки(ТекстДляПоиска, Поле, Валютный = Ложь)
	...
	Если ПустаяСтрока(ТекстДляПоиска) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ბსკ არაკორექტულად არის შევსებული.';ru='Поле БИК заполнено не корректно.'"); 
