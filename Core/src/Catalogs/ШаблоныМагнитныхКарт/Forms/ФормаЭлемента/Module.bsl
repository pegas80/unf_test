...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СчетчикПолей = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='არცერთ ხელმისაწვდომ ბილიკში არცერთი ველი დამატებული არ არის.';ru='Не добавлено ни одного поля ни в одной из доступных дорожек.'"), , , , Отказ);
	КонецЕсли;
...
Процедура ВзятьПрефикс1(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='კოდის მონაკვეთი გამოჰყავით მაუსით';ru='Выделите мышкой участок кода.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОпределитьКоординатыПоля(Оповещение, ДанныеДорожки, Префикс, Суффикс, Разделитель, НКол, ККол)
	...
	Если НЕ ПустаяСтрока(Префикс)
		...
		Если НКол < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='კოდის გამოყოფილი მონაკვეთი არ უნდა კვეთდეს ბლოკების სუფიქსს, პრეფიქსს ან გამყოფებს.';ru='Выделенный участок кода не должен пересекаться с суффиксом, префиксом или разделителем блоков.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ККол = СтрДлина(ДанныеСтрока)+1
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ველის სიგრძე ფიქსირებული';ru='Длина поля фиксированная'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='ველის სიგრძე იზღუდება გამყოფით ან სტრიქონის ბოლოთი';ru='Длина поля ограничивается разделителем или концом строки'"));
...
Процедура КонтрольУникальностиПоля(СписокДублей, Поле, НомерТекущейСтроки, ИмяТаблицы, Отказ)
				...
				Для каждого текПоле Из Объект["ПоляДорожки"+Строка(ВремИндекс)] Цикл
					Если текПоле.Поле = Поле Тогда
						...
						Если СчетчикДублей > 1 Тогда
							СтрСообщение = НСтр("ka='ბილიკი %1, სტრიქონი %2: ველი უნდა იყოს უნიკალური!';ru='Дорожка %1, строка %2: Поле должно быть уникальным!'");
	...
	Иначе
		СтрСообщение = НСтр("ka='ბილიკი %1, სტრიქონი %2: ველი არ შეიძლება ცარიელი იყოს!';ru='Дорожка %1, строка %2: Поле не может быть пустым!'");
...
Процедура ПроверитьШаблонЗавершение(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ ЭтотОбъект.Записать() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შაბლონის ჩაწერა ვერ მოხერხდა';ru='Не удалось записать шаблон'"));
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьШаблонКоманда(Команда)
	...
	Если ЭтотОбъект.Модифицированность Тогда
		ТекстВопроса = НСтр("ka='შაბლონი შეიცვალა, ჩავწეროთ ცვლილებები?';ru='Шаблон был изменен, записать изменения?'");
