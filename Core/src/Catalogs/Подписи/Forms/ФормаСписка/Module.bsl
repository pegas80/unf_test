...
Процедура ОтменаАннулированияПраваПодписиНаСервере(ПодписьОтветственного)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='Не удалось занять для записи право подписи, возможно элемент используется другим пользователем.';ru='Не удалось занять для записи право подписи, возможно элемент используется другим пользователем.';en='Could not take the right to record a signature element of possible use by another user.'");
...
Процедура ПравоПодписиОтозваноПоОшибке(Команда)
	...
	Если НЕ ДанныеТекущейСтроки = Неопределено
		...
		ТекстВопроса = НСтр("ka='Отменить аннулирование права подписи для %1?';ru='Отменить аннулирование права подписи для %1?';en='Cancel revocation authority to sign for the 1%?'");
