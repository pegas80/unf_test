...
Процедура ОтменаАннулированияПраваПодписиНаСервере(ПодписьОтветственного)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ka='Не удалось занять для записи право подписи, возможно элемент используется другим пользователем.';ru='Не удалось занять для записи право подписи, возможно элемент используется другим пользователем.'");
...
Процедура ПравоПодписиОтозваноПоОшибке(Команда)
	...
	Если НЕ ДанныеТекущейСтроки = Неопределено
		...
		ТекстВопроса = НСтр("ka='Отменить аннулирование права подписи для %1?';ru='Отменить аннулирование права подписи для %1?'");
