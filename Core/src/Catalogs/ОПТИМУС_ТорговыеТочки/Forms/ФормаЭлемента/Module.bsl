...
Процедура УстановитьВремяВсехДнейНедели(Команда)
	...
	Если Элементы.ГрафикПосещения.ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='დროის მისათითებლად კვირის დღეები არ არის მითითებული';ru='Не выбраны дни недели для указания времени';en='days of the week to indicate the time is not selected'");
...
Процедура ОтправитьEmail(Команда)
	...
	Если СписокЭлектронныхАдресов = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='ობიექტი არ არის ჩაწერილი. ელექტრონული მისამართების სია ცარიელი იქნება.';ru='Объект не записан. Список электронных адресов будет пуст.';en='The object is not recorded. E-mail list will be empty.'");
