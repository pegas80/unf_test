	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеДисконтнойКарты Тогда
		...
		Если Не ЗначениеЗаполнено(КодКарты) Тогда
			...
			Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				ТекстСообщения = НСтр("ka='შტრიხკოდი არ არის შევსებული.';ru='Штрихкод не заполнен.'");
			Иначе
				ТекстСообщения = НСтр("ka='მაგნიტური კოდი არ არის შევსებული.';ru='Магнитный код не заполнен.'");
			КонецЕсли;
...
Процедура СформироватьЗаголовокФормы()
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеДисконтнойКарты Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ka='დისკონტური ბარათის კითხვა';ru='Считывание дисконтной карты'");
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаВыборДисконтнойКарты Тогда
		ЭтаФорма.АвтоЗаголовок = Ложь;
		ТекущиеДанные = Элементы.НайденныеДисконтныеКарты.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
...
Функция ОбработатьПолученныйКодНаСервере(Данные, ТипКодаКарты, Предобработка, ЕстьНайденныеКарты = Ложь)
	...
	Если ДисконтныеКарты.ЗарегистрированныеДисконтныеКарты.Количество() = 0 Тогда
		Для Каждого СтрокаТЧ Из ДисконтныеКарты.НеЗарегистрированныеДисконтныеКарты Цикл
			...
			НоваяСтрока.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დავარეგისტრიროთ ახალი ბარათი: %1';ru='Зарегистрировать новую карту: %1'"), Строка(СтрокаТЧ.ВидКарты))+?(СтрокаТЧ.ЭтоИменнаяКарта, " (Именная, ", " (")+СтрокаТЧ.ТипКарты+")";
...
Процедура ОбработатьПолученныйКодНаКлиенте(Данные, ПолученныйТипКода, Предобработка)
	...
	Если Не Результат Тогда
		...
		Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ka='შტრიხკოდ ""%1""-ით ბარათი არ არის რეგისტრირებული და დისკონტური ბარათის არცერთი შესაფერისი სახეობა არ არის.';ru='Карта со штрихкодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.'");
		Иначе
			ТекстСообщения = НСтр("ka='ბარათი მაგნიტური კოდ ""%1""-ით არ არის რეგისტრირებული და დისკონური ბარათის არცერთი შესაფერისი სახეობა არ არის.';ru='Карта с магнитным кодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.'");
		КонецЕсли;
