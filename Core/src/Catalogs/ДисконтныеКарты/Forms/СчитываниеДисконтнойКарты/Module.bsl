...
"(считывателя магнитных карт) или введите код вручную';ka='დისკონტური ბარათი წაიკითხეთ შტრიხკოდის სკანერით (მაგნიტური ბარათების წამკითხველით) ან ხელით შეიყვანეთ კოდი'");
...
"магнитных карт или введите магнитный код вручную';ka='დისკონტური ბარათი წაიკითხეთ მაგნიტური ბარათების წამკითხველით ან ხელით შეიყვანეთ მაგნიტური კოდი'");
...
"штрихкода или введите штрихкод вручную';ka='დისკონტური ბარათი წაიკითხეთ შტრიხკოდის სკანერით ან ხელით შეიყვანეთ შტრიხკოდი'");
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ОсновнойТипКода) Тогда
			Текст = НСтр("ru='Считайте дисконтную карту при помощи сканера штрихкода"
"(считывателя магнитных карт)';ka='დისკონტური ბარათი წაიკითხეთ შტრიხკოდის სკანერით (მაგნიტური ბარათების წამკითხველით)'");
		ИначеЕсли ОсновнойТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
			Текст = НСтр("ru='Считайте дисконтную карту при помощи считывателя"
"магнитных карт';ka='დისკონტური ბარათი წაიკითხეთ მაგნიტური ბარათების წამკითხველით'");
		ИначеЕсли ОсновнойТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Текст = НСтр("ru='Считайте дисконтную карту при помощи сканера"
"штрихкода';ka='დისკონტური ბარათი წაიკითხეთ შტრიხკოდის სკანერით'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеДисконтнойКарты Тогда
		...
		Если Не ЗначениеЗаполнено(КодКарты) Тогда
			...
			Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				ТекстСообщения = НСтр("ka='შტრიხკოდი არ არის შევსებული.';ru='Штрихкод не заполнен.';en='Barcode is empty.'");
			Иначе
				ТекстСообщения = НСтр("ka='მაგნიტური კოდი არ არის შევსებული.';ru='Магнитный код не заполнен.';en='Magnetic code is empty.'");
			КонецЕсли;
...
Процедура СформироватьЗаголовокФормы()
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеДисконтнойКарты Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ka='დისკონტური ბარათის კითხვა';ru='Считывание дисконтной карты';en='Reading discount card'");
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаВыборДисконтнойКарты Тогда
		ЭтаФорма.АвтоЗаголовок = Ложь;
		ТекущиеДанные = Элементы.НайденныеДисконтныеКарты.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
...
"Выберите подходящую карту.';ka='მაგნიტური კოდ ""%1""-ით აღმოჩენილია რამდენიმე დისკონტური ბარათი. აირჩიეთ შესაფერისი ბარათი.'");
		Иначе
			Текст = НСтр("ru='Обнаружено несколько дисконтных карт со штрихкодом ""%1""."
"Выберите подходящую карту.';ka='შტრიხკოდ ""%1""-ით აღმოჩენილია რამდენიმე დისკონტური ბარათი. აირჩიეთ შესაფერისი ბარათი.'");
		КонецЕсли;
...
Функция ОбработатьПолученныйКодНаСервере(Данные, ТипКодаКарты, Предобработка, ЕстьНайденныеКарты = Ложь)
	...
	Для Каждого СтрокаТЧ Из ДисконтныеКарты.ЗарегистрированныеДисконтныеКарты Цикл
		...
		НоваяСтрока.Наименование = Строка(СтрокаТЧ.Ссылка) + ?(ЗначениеЗаполнено(СтрокаТЧ.Контрагент) И ЗначениеЗаполнено(СтрокаТЧ.Ссылка), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კლიენტი: %1';ru=' Клиент: %1'"), Строка(СтрокаТЧ.Контрагент)), "");
	...
	Если ДисконтныеКарты.ЗарегистрированныеДисконтныеКарты.Количество() = 0 Тогда
		Для Каждого СтрокаТЧ Из ДисконтныеКарты.НеЗарегистрированныеДисконтныеКарты Цикл
			...
			НоваяСтрока.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დავარეგისტრიროთ ახალი ბარათი: %1';ru='Зарегистрировать новую карту: %1';en='Register your new card: 1%'"), Строка(СтрокаТЧ.ВидКарты))+?(СтрокаТЧ.ЭтоИменнаяКарта, " (Именная, ", " (")+СтрокаТЧ.ТипКарты+")";
...
Процедура ОбработатьПолученныйКодНаКлиенте(Данные, ПолученныйТипКода, Предобработка)
	...
	Если Не Результат Тогда
		...
		Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ka='შტრიხკოდ ""%1""-ით ბარათი არ არის რეგისტრირებული და დისკონტური ბარათის არცერთი შესაფერისი სახეობა არ არის.';ru='Карта со штрихкодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.';en='Map bar code ""%1"" is not registered and there is no suitable type of discount cards.'");
		Иначе
			ТекстСообщения = НСтр("ka='ბარათი მაგნიტური კოდ ""%1""-ით არ არის რეგისტრირებული და დისკონური ბარათის არცერთი შესაფერისი სახეობა არ არის.';ru='Карта с магнитным кодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.';en='Card with a magnetic code ""%1"" is not registered and there is no suitable type of discount cards.'");
		КонецЕсли;
...
"Выберите подходящую карту.';ka='""%1"" კოდით აღმოჩენილია რამდენიმე დისკონტური ბარათი."
"აირჩიეთ შესაფერისი ბარათი.'");
			НадписьВыборДисконтнойКарты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КодКарты);
		Иначе // Только виды карт для регистрации новой карты.
			Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				Текст = НСтр("ka='%1 შტრიხკოდიანი ბარათი არ არის რეგისტრირებული. ახალი დისკონტური ბარათის სარეგისტრაციოდ აირჩიეთ ბარათის შესაფერისი სახეობა.';ru='Карта со штрихкодом ""%1"" не зарегистрирована."
"Выберите подходящий вид карты для регистрации новой дисконтной карты.'");
			Иначе
				Текст = НСтр("ka='ბარათი მაგნიტური კოდ ""%1""-ით არ არის რეგისტრირებული. ახალი დისკონტური ბარათის სარეგისტრაციოდ აირჩიეთ ბარათის შესაფერისი სახეობა.';ru='Карта с магнитным кодом ""%1"" не зарегистрирована."
"Выберите подходящий вид карты для регистрации новой дисконтной карты.'");			   
			КонецЕсли;				   
			НадписьВыборДисконтнойКарты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КодКарты);
		КонецЕсли;
