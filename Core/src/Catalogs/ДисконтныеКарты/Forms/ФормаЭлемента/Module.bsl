...
Процедура ПроверитьДублиДисконтныхКарт(Форма)
	...
	Если КоличествоЭлементовДублей > 0 Тогда
		...
		Если КоличествоЭлементовДублей = 1 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("КоличествоЭлементовДублей", НСтр("ka='ერთი';ru='одна'"));
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ka='რუკა/ბარათი';ru='карта'"));
		...
		ИначеЕсли КоличествоЭлементовДублей < 5 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ka='ბართები';ru='карты'"));
			
		Иначе
			
			СтруктураПараметровСообщенияОДублях.Вставить("КоличествоЭлементовДублей", КоличествоЭлементовДублей);
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ka='карт';ru='карт'"));
			
		КонецЕсли;	
		
		ТекстНадписиОДублях = НСтр("ka='ასეთ კოდით არის [КоличествоЭлементовДублей] [Склонение]';ru='С таким кодом есть [КоличествоЭлементовДублей] [Склонение]'");
	...
	Если ПолученныйТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
		...
		Иначе
			...
			ИначеЕсли Результат.Количество() = 0 Тогда
				...
				КонецЕсли;
			Иначе
				ВыбранноеЗначение = Результат.ВыбратьЭлемент(НСтр("ka='მაგნიტური ბარათის კოდის არჩევა';ru='Выбор кода магнитной карты'"));
