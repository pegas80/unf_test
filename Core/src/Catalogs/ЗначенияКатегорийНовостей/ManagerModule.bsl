...
"%1"
"';ka='Попытка загрузки стандартных значений из строки (первые 2000 символов):"
"%1"
"'"),
...
"%1"
"';ka='Ошибка загрузки стандартных значений из строки по причине:"
"%1"
"'"),
...
Процедура ЗагрузитьСтандартноеЗначение(ОбъектХДТО, ОбъектМетаданных, ИмяСвойства, ЛогЗагрузки)
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ИдентификаторШага = НСтр("ka='ახალი ამბები. სერვისი და რეგლამენტი. სტანდარტული მნიშვნელობების ჩატვირთვა. %ИмяСвойства%. შეცდომა';ru='Новости. Сервис и регламент. Загрузка стандартных значений. %ИмяСвойства%. Ошибка';en='News. Service and regulations. Loading standard values. % PropertyName%. Mistake'");
		ИдентификаторШага = СтрЗаменить(ИдентификаторШага, "%ИмяСвойства%", ИмяСвойства);
		ТекстСообщения = НСтр("ka='მეტამონაცემთა ობიექტის ჩაწერა ვერ მოხერხდა შემდეგი მიზეზით:"
"%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%"
"კატეგორია    = %Владелец%"
"კოდი          = %Код%"
"დასახელება = %Наименование%';ru='Не удалось записать объект метаданных по причине:"
"%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%"
"Категория    = %Владелец%"
"Код          = %Код%"
"Наименование = %Наименование%'");
		...
		СтрокаЛогаЗагрузки = СтрокаЛогаЗагрузки + ". "
			+ СтрШаблон(
				НСтр("ru='Произошла ошибка записи: %1';en='An entry error:% 1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
