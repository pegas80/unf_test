...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ცნობარი არ არის განკუთვნილი ხელით შესავსებად!';ru='Справочник не предназначен для ручного заполнения!'"));
		Отказ = Истина;
		Возврат;
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ka='ოქმი';ru='Протокол'") + "_" + Объект.ИдентификаторОтправкиНаСервере + ".html");
...
Процедура ОбновитьФорму()
	...
	Иначе
		...
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.ПринятЕстьОшибки Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Квитанция о сдаче отчета получена, протокол ошибок получен';ru='Квитанция о сдаче отчета получена, протокол ошибок получен'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Сдан Тогда
			Элементы.КартинкаСтатусОтправки.Картинка = БиблиотекаКартинок.ПисьмоПодтверждениеПолучено;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Квитанция о сдаче отчета получена';ru='Квитанция о сдаче отчета получена'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		КонецЕсли;
	КонецЕсли;
