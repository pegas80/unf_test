...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "СчетУчетаРегл");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "СчетЗатратРегл");
...
Процедура ПриЗаписи(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстСообщения = НСтр("ka='საგადახდო ბარათის (%1) სახეობის შესახებ ცნობარ ""საგადახდო ბარათის სახეობები""-ში ინფორმაციის ჩაწერა ვერ მოხერხდა';ru='Не удалось записать информацию о виде платежной карты (%1) в справочнке ""Виды платежных карт""';en='Failed to write information on the type of payment card (1%) in spravochnke ""Forms of payment cards""'");
	...
	Если ТипЗнч(Касса) = Тип("СправочникСсылка.КассыККМ") Тогда
		...
		Если ЗначениеЗаполнено(Организация)
		   ...
		   И Организация <> Реквизиты.Организация Тогда
			...
			Текст = НСтр("ka='სალაროს ორგანიზაცია არ შეესაბამება ეკვაირინგის ხელშეკრულების ორგანიზაციას';ru='Организация кассы не соответствует организации договора эквайринга';en='Organization of cash does not correspond to the organization acquiring agreement'");
