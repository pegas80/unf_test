...
Процедура СформироватьСтрокуОКУН()
	...
	Если Объект.ЗначенияОКУН.Количество() = 0 Тогда
		Элементы.СсылкаЗаполненияОКУН.Заголовок = (НСтр("ka='<<Заполнить ОКУН>>';ru='<<Заполнить ОКУН>>'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		РезСтрока = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='არჩეულია მმკ-ის %1 კოდი';ru='Выбрано %1 кода ОКУН'"),
				Выборка.Количество() );
...
Процедура Декорация8ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(Объект.КодНалоговогоОрганаПолучателя) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='შეავსეთ საგადასახადო სამსახურის ინსპექციის კოდი და მაშინ, მოსაძებნად შევძლებთ ბმულის დადგენას';ru='Заполните код ИФНС и мы тогда сможем определить ссылку для поиска'"),
			,
