...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ВидОтчета) 
		...
		Если Не ЗначениеЗаполнено(ДатаНачала) ИЛИ Не ЗначениеЗаполнено(ДатаОкончания) ИЛИ Не ЗначениеЗаполнено(Периодичность) Тогда
			ТекстСообщения = НСтр("ka='ველი ""პერიოდი"" არ არის შევსებული';ru='Поле ""Период"" не заполнено';en='""Period"" field is not filled'"); 
...
""
"Дата начала периода больше, чем дата его окончания.';ka='Поле ""Период"" заполнено некорректно."
""
