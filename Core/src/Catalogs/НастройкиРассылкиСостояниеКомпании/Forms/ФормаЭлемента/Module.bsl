...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НеЗаполненаОрганизация() Тогда
		...
		НСтр("ka='Не заполнена организация.';ru='Не заполнена организация.'"),,
		"Организация",
		"Объект",
		Отказ);
		Возврат;
...
"%2'"),
	...
	Получатель.Добавить("sbm@1c.ru", НСтр("ka='ფირმა 1С';ru='Фирма 1С'"));
	
	ПараметрыПисьма = Новый Структура;
	ПараметрыПисьма.Вставить("Получатель", Получатель);
	ПараметрыПисьма.Вставить("Тема", НСтр("ka='Хочу эти возможности в будущей версии отчета ""Состояние компании""';ru='Хочу эти возможности в будущей версии отчета ""Состояние компании""'"));
...
Функция ПараметрыДляОтправкиСодержатОшибки()
	...
	Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
		...
		НСтр("ka='Не указана учетная запись для отправки почты.';ru='Не указана учетная запись для отправки почты.'"),
		Неопределено);
	КонецЕсли;
	...
	Если Не УказанХотяБыОдинПолучатель() Тогда
		...
		НСтр("ka='Не указан хотя бы один адрес электронной почты получателя.';ru='Не указан хотя бы один адрес электронной почты получателя.'"),
		Неопределено);
	КонецЕсли;
...
Функция АдресаПолучателейСодержатОшибки()
	...
	Для Индекс = 0 По Получатели.Количество() - 1 Цикл
		...
		Ошибки,
		"Получатели[%1].Получатель",
		НСтр("ka='Адрес электронной почты указан некорректно.';ru='Адрес электронной почты указан некорректно.'"),
		"Получатели",
		Индекс,
		НСтр("ka='Адрес электронной почты в строке %1 указан некорректно.';ru='Адрес электронной почты в строке %1 указан некорректно.'"),
		Индекс);
	...
	Если СостояниеЗадания.ЗаданиеВыполнено Тогда
		...
		Если ЗначениеЗаполнено(АдресХранилища) Тогда
			...
			Если ТипЗнч(ТабличныйДокумент) = Тип("ТабличныйДокумент") Тогда
				...
				ТабличныйДокумент.Показать(НСтр("ka='Состояние компании (предварительный просмотр)';ru='Состояние компании (предварительный просмотр)'"));
			КонецЕсли;
