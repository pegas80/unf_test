...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Переключатель = "Год" Тогда
		...
		Если НомерМесяца < 1 ИЛИ НомерМесяца > 12 ИЛИ НомерДня < 1 ИЛИ НомерДня > 31 ИЛИ НомерДня > КоличествоДнейВМесяцеНаКлиенте(НомерМесяца, Год(ДатаГрафика)) Тогда
			ПоказатьПредупреждение(Неопределено,НСтр("ka='თავდაპირველად აუცილებელია გრაფიკის დაყენების დღის გამოყოფა!';ru='Вначале необходимо выделить день установки графика!'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьГрафик(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Текст = НСтр("ka='გრაფიკის დაყენება შესაძლებელია მხოლოდ საწარმოს რესურსის ჩაწერის შემდეგ! რესურსი ჩაიწერება.';ru='Установка графика возможна только после записи ресурса предприятия! Ресурс будет записан.'");
...
Функция ПолучитьТекущийГрафикНаДату(Дата)
	...
	Если НЕ ЗначениеЗаполнено(ГрафикРаботы) Тогда
		ГрафикРаботы = НСтр("ka='<Не установлен>';ru='<Не установлен>'");
...
Процедура ОтметитьВыбранноеКакНеРабочееГод()
	...
	Если ЕстьПодходящиеДни
	   И ТекГрафик = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='დასაწყისში დააყენეთ არცეული დღეების გრაფიკი!';ru='Установите вначале график для выбранных дней!'"));
	КонецЕсли;
...
Процедура ОтметитьВыбранноеКакНеРабочееДень()
	...
	Если ТекГрафик = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='დასაწყისში დააყენეთ ამ დღის გრაფიკი!';ru='Установите вначале график для этого дня!'"));
		Возврат;
	КонецЕсли;
