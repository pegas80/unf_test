...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Переключатель = "Год" Тогда
		...
		Если НомерМесяца < 1 ИЛИ НомерМесяца > 12 ИЛИ НомерДня < 1 ИЛИ НомерДня > 31 ИЛИ НомерДня > КоличествоДнейВМесяцеНаКлиенте(НомерМесяца, Год(ДатаГрафика)) Тогда
			ПоказатьПредупреждение(Неопределено,НСтр("ka='თავდაპირველად აუცილებელია გრაფიკის დაყენების დღის გამოყოფა!';ru='Вначале необходимо выделить день установки графика!';en='You must first select the day installation schedule!'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьГрафик(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Текст = НСтр("ka='გრაფიკის დაყენება შესაძლებელია მხოლოდ საწარმოს რესურსის ჩაწერის შემდეგ! რესურსი ჩაიწერება.';ru='Установка графика возможна только после записи ресурса предприятия! Ресурс будет записан.';en='Setting the schedule is possible only after the enterprise resource record! The resource will be recorded.'");
...
Функция ПолучитьТекущийГрафикНаДату(Дата)
	...
	Если НЕ ЗначениеЗаполнено(ГрафикРаботы) Тогда
		ГрафикРаботы = НСтр("ka='<Не установлен>';ru='<Не установлен>';en='<Not set>'");
...
Процедура ОтметитьВыбранноеКакНеРабочееГод()
	...
	Если ЕстьПодходящиеДни
	   И ТекГрафик = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='დასაწყისში დააყენეთ არცეული დღეების გრაფიკი!';ru='Установите вначале график для выбранных дней!';en='Install the first schedule for selected days!'"));
	КонецЕсли;
...
Процедура ОтметитьВыбранноеКакНеРабочееДень()
	...
	Если ТекГрафик = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='დასაწყისში დააყენეთ ამ დღის გრაფიკი!';ru='Установите вначале график для этого дня!';en='Install the first schedule for that day!'"));
		Возврат;
	КонецЕсли;
