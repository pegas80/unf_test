...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.КлючНазначенияИспользования = "Просроченные" Тогда
		ЭтаФорма.Заголовок = ЭтаФорма.Заголовок + НСтр("ka=':ვადაგასული';ru=': просроченные';en=': overdue'");
	...
	ИначеЕсли Параметры.КлючНазначенияИспользования = "ОжидаютВыполнения" Тогда
		...
		ЭтаФорма.Заголовок = ЭтаФорма.Заголовок + НСтр("ka=': ელოდება შესრულებას';ru=': ожидают выполнения';en=': Awaiting execution'");
	КонецЕсли;
...
Функция НастройкиОформленияСпискаЗадач()
	...
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		Неопределено, -1, НСтр("ka='ვადაგადაცილებულია';ru='Просрочено';en='Expired'"), ЦветаСтиля.ПросроченныеДанныеЦвет);
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		0, 0, НСтр("ka='დღეს';ru='Сегодня';en='Today'"), ЦветаСтиля.ПросроченныеДанныеЦвет);
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		1, 1, НСтр("ka='ხვალ';ru='Завтра';en='Tomorrow'"));
	
	Шаблон = НСтр("ka='დარჩენილა %1';ru='Осталось %1';en='1% remaining'");
	
	Для РазностьДат = 2 По 6 Цикл
		СтрокаРазностьДат = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
			РазностьДат, НСтр("ka='დღე,დღეს,დღეები';ru='день,дня,дней';en='day, days, days'"));
		ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
			РазностьДат, РазностьДат, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, СтрокаРазностьДат));
	КонецЦикла;
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		7, 13, НСтр("ka='დარჩენილია კვირა';ru='Осталась неделя';en='rest of the week'"));
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		14, 18, НСтр("ka='დარჩა 2 კვირა';ru='Осталось 2 недели';en='2 weeks left'"));
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		19, 22, НСтр("ka='დარჩა 3 კვირა';ru='Осталось 3 недели';en='It left 3 weeks'"));
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		23, 34, НСтр("ka='დარჩა თვე';ru='Остался месяц';en='month left'"));
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ანგარიშგება არ არის ჩაბარებული';ru='Отчет не сдан';en='The report does not put'"));
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ანგარიშგება ჩაბარებულია!';ru='Отчет сдан!';en='commissioned report!'"));
...
Функция ЗапуститьОбновлениеДанныхНаСервере()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыФункции, 
		НСтр("ka='ანგარიშგებულობის ამოცანების სიის განახლება';ru='Обновление списка задач отчетности';en='Updating the list of reporting tasks'"));
