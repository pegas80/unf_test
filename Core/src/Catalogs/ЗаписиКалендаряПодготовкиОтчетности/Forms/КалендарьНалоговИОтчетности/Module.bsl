...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.КлючНазначенияИспользования = "Просроченные" Тогда
		ЭтаФорма.Заголовок = ЭтаФорма.Заголовок + НСтр("ka=':ვადაგასული';ru=': просроченные'");
	...
	ИначеЕсли Параметры.КлючНазначенияИспользования = "ОжидаютВыполнения" Тогда
		...
		ЭтаФорма.Заголовок = ЭтаФорма.Заголовок + НСтр("ka=': ელოდება შესრულებას';ru=': ожидают выполнения'");
	КонецЕсли;
...
Функция НастройкиОформленияСпискаЗадач()
	...
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		Неопределено, -1, НСтр("ka='ვადაგადაცილებულია';ru='Просрочено'"), ЦветаСтиля.ПросроченныеДанныеЦвет);
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		0, 0, НСтр("ka='დღეს';ru='Сегодня'"), ЦветаСтиля.ПросроченныеДанныеЦвет);
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		1, 1, НСтр("ka='ხვალ';ru='Завтра'"));
	
	Шаблон = НСтр("ka='დარჩენილა %1';ru='Осталось %1'");
	
	Для РазностьДат = 2 По 6 Цикл
		СтрокаРазностьДат = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
			РазностьДат, НСтр("ka='დღე,დღეს,დღეები';ru='день,дня,дней'"));
		ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
			РазностьДат, РазностьДат, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, СтрокаРазностьДат));
	КонецЦикла;
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		7, 13, НСтр("ka='დარჩენილია კვირა';ru='Осталась неделя'"));
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		14, 18, НСтр("ka='დარჩა 2 კვირა';ru='Осталось 2 недели'"));
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		19, 22, НСтр("ka='დარჩა 3 კვირა';ru='Осталось 3 недели'"));
	
	ДобавитьЭлементУсловногоОформленияСпискаЗадач(НастройкиКомпоновкиДанных,
		23, 34, НСтр("ka='დარჩა თვე';ru='Остался месяц'"));
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ანგარიშგება არ არის ჩაბარებული';ru='Отчет не сдан'"));
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='ანგარიშგება ჩაბარებულია!';ru='Отчет сдан!'"));
...
Функция ЗапуститьОбновлениеДанныхНаСервере()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыФункции, 
		НСтр("ka='ანგარიშგებულობის ამოცანების სიის განახლება';ru='Обновление списка задач отчетности'"));
