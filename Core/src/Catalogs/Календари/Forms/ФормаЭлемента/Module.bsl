...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მუშაობის გრაფიკში შესაყვანი დღეები არ არის აღნიშნული';ru='Не отмечены дни, включаемые в график работы'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если МодифицированностьРезультата Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ჩავწეროთ შეცვლილი მონაცემები %1 წლის მანძილზე?';ru='Записать измененные данные за %1 год?'"), Формат(НомерПредыдущегоГода, "ЧГ=0"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='განრიგის შევსება';ru='Заполнить расписание'"));
...
Процедура СформироватьШаблонЗаполнения(СпособЗаполнения, ШаблонЗаполнения, Знач ДлинаЦикла, Знач ДатаОтсчета = Неопределено)
	...
	Если СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафикаРаботы.ПоНеделям") Тогда
		ШаблонЗаполнения[0].ПредставлениеДня = НСтр("ka='ორშაბათი';ru='Понедельник'");
		ШаблонЗаполнения[1].ПредставлениеДня = НСтр("ka='სამშაბათი';ru='Вторник'");
		ШаблонЗаполнения[2].ПредставлениеДня = НСтр("ka='ოთხშაბათი';ru='Среда'");
		ШаблонЗаполнения[3].ПредставлениеДня = НСтр("ka='ხუთშაბათი';ru='Четверг'");
		ШаблонЗаполнения[4].ПредставлениеДня = НСтр("ka='პარასკევი';ru='Пятница'");
		ШаблонЗаполнения[5].ПредставлениеДня = НСтр("ka='შაბათი';ru='Суббота'");
		ШаблонЗаполнения[6].ПредставлениеДня = НСтр("ka='კვირა დღე';ru='Воскресенье'");
	Иначе
		ДатаДня = ДатаОтсчета;
...
Функция ПредставлениеРасписанияДня(Форма, НомерДня)
	...
	Если Секунд = 0 Тогда
		Возврат НСтр("ka='განრიგის შევსება';ru='Заполнить расписание'");
	КонецЕсли;
	
	Часов = Окр(Секунд / 3600, 1);
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 სთ. (%2)';ru='%1 ч. (%2)'"), Часов, ПредставлениеИнтервалов);
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.ДатаЗаполненности) Тогда
		Форма.ЗаполненностьИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკი არ არის შევსებული';ru='График работы не заполнен'");
		Форма.ТребуетЗаполнения = Истина;
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			ИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკი შევსებულია %1-მდე';ru='График работы заполнен до %1'");
		...
		Иначе											
			...
			ИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკი შევსებულია %1-მდე, დაგეგმვის ჰორიზონტის გათვალისწინებით გრაფიკი უნდა შეივსოს %2-მდე';ru='График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2'");
...
Процедура ЗаполнитьИнформационныйТекстРезультатаЗаполнения(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационныйТекст = НСтр("ka='მიმდინარე წლის სამუშაო გრაფიკი შეცვლილია ხელით. ავტომატურ შევსებაზე დასაბრუნებლად, დააჭირეთ ""შაბლონის მიხედვით შევსებას"".';ru='График работы на текущий год изменен вручную. Нажмите «Заполнить по шаблону» чтобы вернуться к автоматическому заполнению.'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				ИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკის განახლება ავტომატურად ხდება, მიმდინარე წლის საწარმოო კალენდრის შეცვლისას.';ru='График работы автоматически обновляется при изменении производственного календаря за текущий год.'");
...
Процедура ЗаполнитьИнформационныйТекстРучногоРедактирования(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
		ИнформационныйТекст = НСтр("ka='მიმდინარე წლის მუშაობის გრაფიკი შეცვლილია ხელით. ცვლილებები გამოყოფილია შევსების შედეგებში.';ru='График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.'");
	КонецЕсли;
