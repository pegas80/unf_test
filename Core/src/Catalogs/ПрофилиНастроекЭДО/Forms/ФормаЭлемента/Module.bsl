...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ Объект.ПометкаУдаления И Не ПропускатьПроверкуНастроекЭДО И ОтсутствуютПодчиненныеНастройкиЭДО() Тогда
		ТекстВопроса = НСтр("ka='მოვახდინოთ კონტრაგენტის შეერთება ელექტრონული დოკუმენტების გაცვლასთან?';ru='Подключить контрагента к обмену электронными документами?';en='Connect the counterparty to the exchange of electronic documents?'");
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			Если ТипВыбранногоЗначения = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
				...
				Если МассивСтрок.Количество() > 0 Тогда
					ТекстПредупреждения = НСтр("ka='არჩეული სერთიფიკატი უკვე რეგისტრირებულია ედბ-ის გამართვების პროფილში.';ru='Выбранный сертификат уже зарегистрирован в профиле настроек ЭДО.';en='Selected certificate is already registered in the profile of EDM settings.'");
...
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ka='პირად კაბინეტში შესასვლელად, თუნდ ერთი სერთიფიკატი უნდა იყოს რეგისტრირებული';ru='Для входа в личный кабинет должен быть зарегистрирован хотя бы один сертификат';en='To access the Members Area at least one certificate must be registered'");
		ПоказатьПредупреждение(, ТекстПредупреждения, 30);
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ka='მოქმედების შესრულება შეიძლება მხოლოდ ედბ-ის გამართვების ჩაწერილ პროფილში."
"ჩავწეროთ?';ru='Выполнить действие можно только в записанном профиле настроек ЭДО."
"Записать?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ТекстПриглашенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ka='ტექსტის შაბლონი კონტრაგენტების მოსაწვევად';ru='Шаблон текста для приглашений контрагентов';en='Template text for parties invitations'");
...
Процедура ИдентификаторОрганизацииОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Не КонтролироватьСодержимоеНастроекЭД Тогда
		ТекстВопроса = НСтр("ka='""ედბ-ის გამართვების პროფილი""-ს ცვლილებები გამოყენებული იქნება დაკავშირებული ""ედბ-ის გამართვები""-სთვის გამორთული გაფართოებული რეჟიმით."
"გავაგრძელოთ?';ru='Изменения ""Профиля настроек ЭДО"" будут применены для связанных ""Настроек ЭДО"" с отключенным расширенным режимом."
"Продолжить?'");
...
Процедура ТестПрофиляНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='ედბ-ის გამართვების მიმდინარე პროფილში შეტანილია ცვლილებები. გავაგრძელოთ ტესტის შესრულება?';ru='В текущий профиль настроек ЭДО внесены изменения. Продолжить выполнение теста?';en='In the current EDI profile changes the settings. Continue the test?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить("Выполнить", НСтр("ka='ტესტის შენახვა და შესრულება';ru='Сохранить и выполнить тест';en='Save and run the test'"));
		СписокКнопок.Добавить("Отменить", НСтр("ka='ტესტის გაუქმება';ru='Отменить тест';en='cancel the test'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , "Выполнить", НСтр("ka='აწყობების ტესტი';ru='Тест настроек';en='test settings'"));
	Иначе
...
"Продолжить?';ka='ედბ-ის გამართვების პროფილში სერთიფიკატის დამატების ოპერაციას რამდენიმე წუთი დაჭირდება. გავაგრძელოთ?'");
...
Процедура НастройкаУведомлений(Команда)
	...
	ПредставлениеОперации = НСтр("ka='ედბ-ის მოვლენების შესახებ შეტყობინებების გასწორება';ru='Настройка уведомлений о событиях ЭДО';en='Configuring event notification EDI'");
...
Процедура ИсходящиеДокументыПриИзменении(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД")
		...
		Если Элемент.ТекущийЭлемент.Имя = "ИсходящиеДокументыИспользоватьЭП" И НЕ Элемент.ТекущиеДанные.ИспользоватьЭП Тогда
			ШаблонСообщения = НСтр("ka='დოკუმენტ %1-ის გაგზავნა შესაძლებელია მხოლოდ ელექტრონული ხელმოწერის გამოყენებით.';ru='Отправка документа %1 возможна только с использованием электронной подписи.';en='Sending a document% 1 is only possible with the use of electronic signatures.'");
	...
	Если (Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура")
		...
		ШаблонСообщения = НСтр("ka='დოკუმენტ %1-ის გაგზავნა შესაძლებელია მხოლოდ ედბ-ის ოპერატორით.';ru='Отправка документа %1 возможна только через оператора ЭДО.';en='Sending a document% 1 is only possible through the EDM operator.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			Элемент.ТекущиеДанные.ИсходящийДокумент);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
		...
		ШаблонСообщения = НСтр("ru='Данные документ %1 передаются в составе счет-фактуры(универсального передаточного документа).';en='These document% 1 are transferred as part of the invoice (universal transmission of the document).'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			Элемент.ТекущиеДанные.ИсходящийДокумент);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель")
		...
		ШаблонСообщения = НСтр("ru='Данные документ %1 передаются в составе корректировочного счет-фактуры(универсального корректировочного документа).';en='1% document data transferred as part of the correction invoice (universal document correction).'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			Элемент.ТекущиеДанные.ИсходящийДокумент);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		Если Не Объект.ИспользоватьУПД Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"));
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"), НСтр("ka='ФНС 5.01 (УПД:Счет-фактура)';ru='ФНС 5.01 (УПД:Счет-фактура)';en='FTS 5.01 (UPD: Invoice)'"));
		КонецЕсли;
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';ru='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';en='FTS 5.01 (UPD: The invoice and the original document)'"), НСтр("ka='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';ru='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';en='FTS 5.01 (UPD: The invoice and the original document)'"));
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.КорректировочныйСчетФактура") Тогда
		Если Не Объект.ИспользоватьУКД Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"));
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура)';en='FTS 5.01 (SACs: Correction invoice)'"), НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура)';en='FTS 5.01 (SACs: Correction invoice)'"));
		КонецЕсли;
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';en='FTS 5.01 (SACs: Correction invoice and the original document)'"), НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';en='FTS 5.01 (SACs: Correction invoice and the original document)'"));
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав") Тогда
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"), НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)';en='FTS 5.01 (UPD: The primary document)'"), НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)';en='FTS 5.01 (UPD: The primary document)'"));
		КонецЕсли;

	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктИсполнитель") Тогда
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01';ru='ФНС 5.01';en='FTS 5.01'"), НСтр("ru='ФНС 5.01 (до 01.07.2017г.)';en='FTS 5.01 (01.07.2017g up.)'"));
		СписокВыбора.Добавить(НСтр("ka='საგადასახადო სამსახური 5.01 (2016 წლიდან)';ru='ФНС 5.01 (с 2016г.)';en='FTS 5.01 (s 2016).'"), НСтр("ru='ФНС 5.01 (с 10.01.2016г.)';en='FTS 5.01 (s 10.01.2016g).'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)';en='FTS 5.01 (UPD: The primary document)'"), НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)';en='FTS 5.01 (UPD: The primary document)'"));
		КонецЕсли;
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"), НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УКД:Первичный документ)';ru='ФНС 5.01 (УКД:Первичный документ)';en='FTS 5.01 (SACs: The primary document)'"), НСтр("ka='ФНС 5.01 (УКД:Первичный документ)';ru='ФНС 5.01 (УКД:Первичный документ)';en='FTS 5.01 (SACs: The primary document)'"));
		КонецЕсли;
	Иначе
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"), НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"));
		СписокВыбора.Добавить(НСтр("ka='CML 4.02';ru='CML 4.02';en='CML 4.02'"), НСтр("ka='CML 4.02';ru='CML 4.02';en='CML 4.02'"));
	КонецЕсли;
...
Процедура ТестПрофиляНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			Состояние(НСтр("ka='აწყობეების ტესტი.';ru='Тест настроек.';en='Test Settings.'"),
			,
...
"%1';ka='ტესტი. საერთო კატალოგთან დაშვების შემოწმება ედ-ის გასაცვლელად."
"%1'");
			Попытка
...
"%1';ka='ტესტი. FTP სერვერთან დაშვების შემოწმება ედ-ის გასაცვლელად."
"%1'");
			Попытка
...
"%2'");
	...
	Иначе
		...
		Состояние(НСтр("ka='აწყობეების ტესტი.';ru='Тест настроек.';en='Test Settings.'"),
			,
			НСтр("ka='მიმდინარეობს ედბ-ის სერვისთან კავშირის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование связи с сервисом ЭДО. Подождите...';en='A test is performed due to the EDI service. Wait...'"));
...
Процедура ПослеПолученияОтпечатковПроверитьСертификаты(Отпечатки, Параметры = Неопределено) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='ხელმისაწვდომი სერთიფიკატები არ არის. ტესტი არ შესრულებულა.';ru='Нет доступных сертификатов. Тест не выполнен.';en='No certificates available. The test is not made.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПослеПолученияОтпечатковПроверитьСвязьЭДО(Отпечатки, Параметры = Неопределено) Экспорт
	...
	ШаблонОперация = НСтр("ka='ავთენტიფიკაციის ტესტი: %1';ru='Тест аутентификации: %1';en='Authentication test:% 1'");
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		Если РезультатВыполнения.Успех = Истина Тогда
			ТекстСообщения = НСтр("ka='გამართვების პროფილის ტესტი წარმატებით შესრულდა.';ru='Тест профиля настроек выполнен успешно.';en='Test the profile settings successful.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстСообщения = НСтр("ka='ედბ-ის გამართვების მოცემული პროფილით დარეგისტრირებულებს შორის ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов, среди зарегистрированных по данному профилю настроек ЭДО.';en='No certificates available, among registered at the profile of EDM settings.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,
			,
...
"Необходимо повторить добавление сертификата.';ka='ედბ-ის ოპერატორის სერთიფიკატის მიღების შეცდომა. აუცილებელია სერთიფიკატის დამატების გამეორება.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ka='სერვის 1C-ედბ-ზე მონაცემთა შეცვლის თაობაზე განცხადების გაგზავნა';ru='Отправка заявления на изменение данных на сервисе 1С-ЭДО';en='Submitting an application for modification of data on service-1C EDT'");
...
Процедура ОткрытьФормуВыбораСертификатаЭП()
	...
	Если Объект.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ka='მოქმედების შესასრულებლად საჭიროა წაშლის ნიშნულის მოხსნა.';ru='Для выполнения действия необходимо снять пометку удаления.';en='To perform the actions necessary to remove the deletion mark.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
"необходимо получить идентификатор участника обмена ЭД';ka='ედბ-ის გამართვების პროფილში ახალი სერთიფიკატების დამატების წინ, აუცილებელია ედ-ის გაცვლის მონაწილის იდენტიფიკატორის მიღება'");
			ПоказатьПредупреждение(, ТекстПредупреждения, 30);
			Возврат;
...
Процедура УстановитьНастройкуУведомлений()
	...
	Если УведомлятьОСобытиях Тогда
		ЗаголовокШаблон = НСтр("ka='Email-ი შეტყობინებებისთვის: %1';ru='Email для уведомлений: %1';en='Email notification: 1%'");
	Иначе
		ЗаголовокШаблон = НСтр("ka='შეტყობინებები არ არის შეერთებული';ru='Уведомления не подключены';en='Notifications are not connected'");
	КонецЕсли;
	...
	ИначеЕсли УведомлятьОСобытиях Тогда
		АдресУведомлений = НСтр("ka='<მითითებული არ არის>';ru='<не указан>';en='<Unknown>'");
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеРегламентаЭДО(ТекСтрока)
	...
	Если ТекСтрока.ИспользоватьЭП И ИспользоватьЭП Тогда
		Подпись = НСтр("ka='ხელმოწერა';ru='Подпись';en='Signature'");
	Иначе
		Подпись = НСтр("ru='Без подписи';en='Without a signature'");
	КонецЕсли;
	...
	Если ТекСтрока.ТребуетсяИзвещениеОПолучении Тогда
		ИзвещениеОПолучении = НСтр("ru='извещение о получении';en='notice of receipt'");
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='საგადასახადო სამსახური 5.01 (2016 წლიდან)';ru='ФНС 5.01 (с 2016г.)';en='FTS 5.01 (s 2016).'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='ФНС 5.01 (с 10.01.2016)';en='FTS 5.01 (s 01.10.2016)'"));
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ИсходящиеДокументы.ВерсияФормата");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='ФНС 5.01';ru='ФНС 5.01';en='FTS 5.01'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='ФНС 5.01 (до 01.07.2017)';en='FTS 5.01 (until 01.07.2017)'"));
