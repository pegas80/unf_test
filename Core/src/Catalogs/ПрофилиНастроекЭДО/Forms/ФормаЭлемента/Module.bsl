...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ Объект.ПометкаУдаления И Не ПропускатьПроверкуНастроекЭДО И ОтсутствуютПодчиненныеНастройкиЭДО() Тогда
		ТекстВопроса = НСтр("ka='მოვახდინოთ კონტრაგენტის შეერთება ელექტრონული დოკუმენტების გაცვლასთან?';ru='Подключить контрагента к обмену электронными документами?'");
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			Если ТипВыбранногоЗначения = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
				...
				Если МассивСтрок.Количество() > 0 Тогда
					ТекстПредупреждения = НСтр("ka='არჩეული სერთიფიკატი უკვე რეგისტრირებულია ედბ-ის გამართვების პროფილში.';ru='Выбранный сертификат уже зарегистрирован в профиле настроек ЭДО.'");
...
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ka='პირად კაბინეტში შესასვლელად, თუნდ ერთი სერთიფიკატი უნდა იყოს რეგისტრირებული';ru='Для входа в личный кабинет должен быть зарегистрирован хотя бы один сертификат'");
		ПоказатьПредупреждение(, ТекстПредупреждения, 30);
		Возврат;
	КонецЕсли;
...
Процедура ТекстПриглашенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ka='ტექსტის შაბლონი კონტრაგენტების მოსაწვევად';ru='Шаблон текста для приглашений контрагентов'");
...
Процедура ТестПрофиляНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='ედბ-ის გამართვების მიმდინარე პროფილში შეტანილია ცვლილებები. გავაგრძელოთ ტესტის შესრულება?';ru='В текущий профиль настроек ЭДО внесены изменения. Продолжить выполнение теста?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить("Выполнить", НСтр("ka='ტესტის შენახვა და შესრულება';ru='Сохранить и выполнить тест'"));
		СписокКнопок.Добавить("Отменить", НСтр("ka='ტესტის გაუქმება';ru='Отменить тест'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , "Выполнить", НСтр("ka='აწყობების ტესტი';ru='Тест настроек'"));
	Иначе
...
Процедура НастройкаУведомлений(Команда)
	...
	ПредставлениеОперации = НСтр("ka='ედბ-ის მოვლენების შესახებ შეტყობინებების გასწორება';ru='Настройка уведомлений о событиях ЭДО'");
...
Процедура ИсходящиеДокументыПриИзменении(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД")
		...
		Если Элемент.ТекущийЭлемент.Имя = "ИсходящиеДокументыИспользоватьЭП" И НЕ Элемент.ТекущиеДанные.ИспользоватьЭП Тогда
			ШаблонСообщения = НСтр("ka='დოკუმენტ %1-ის გაგზავნა შესაძლებელია მხოლოდ ელექტრონული ხელმოწერის გამოყენებით.';ru='Отправка документа %1 возможна только с использованием электронной подписи.'");
	...
	Если (Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура")
		...
		ШаблонСообщения = НСтр("ka='დოკუმენტ %1-ის გაგზავნა შესაძლებელია მხოლოდ ედბ-ის ოპერატორით.';ru='Отправка документа %1 возможна только через оператора ЭДО.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			Элемент.ТекущиеДанные.ИсходящийДокумент);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		Если Не Объект.ИспользоватьУПД Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.02';ru='ФНС 5.02'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02'"));
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.02';ru='ФНС 5.02'"), НСтр("ka='ФНС 5.01 (УПД:Счет-фактура)';ru='ФНС 5.01 (УПД:Счет-фактура)'"));
		КонецЕсли;
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';ru='ФНС 5.01 (УПД:Счет-фактура и первичный документ)'"), НСтр("ka='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';ru='ФНС 5.01 (УПД:Счет-фактура и первичный документ)'"));
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.КорректировочныйСчетФактура") Тогда
		Если Не Объект.ИспользоватьУКД Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.02';ru='ФНС 5.02'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02'"));
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура)'"), НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура)'"));
		КонецЕсли;
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)'"), НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)'"));
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав") Тогда
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08'"), НСтр("ka='CML 2.08';ru='CML 2.08'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)'"), НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)'"));
		КонецЕсли;

	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктИсполнитель") Тогда
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01';ru='ФНС 5.01'"), НСтр("ru='ФНС 5.01 (до 01.07.2017г.)'"));
		СписокВыбора.Добавить(НСтр("ka='საგადასახადო სამსახური 5.01 (2016 წლიდან)';ru='ФНС 5.01 (с 2016г.)'"), НСтр("ru='ФНС 5.01 (с 10.01.2016г.)'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)'"), НСтр("ka='ФНС 5.01 (УПД:Первичный документ)';ru='ФНС 5.01 (УПД:Первичный документ)'"));
		КонецЕсли;
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08'"), НСтр("ka='CML 2.08';ru='CML 2.08'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УКД:Первичный документ)';ru='ФНС 5.01 (УКД:Первичный документ)'"), НСтр("ka='ФНС 5.01 (УКД:Первичный документ)';ru='ФНС 5.01 (УКД:Первичный документ)'"));
		КонецЕсли;
	Иначе
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08'"), НСтр("ka='CML 2.08';ru='CML 2.08'"));
		СписокВыбора.Добавить(НСтр("ka='CML 4.02';ru='CML 4.02'"), НСтр("ka='CML 4.02';ru='CML 4.02'"));
	КонецЕсли;
...
Процедура ТестПрофиляНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			Состояние(НСтр("ka='აწყობეების ტესტი.';ru='Тест настроек.'"),
			,
...
"%2'");
	...
	Иначе
		...
		Состояние(НСтр("ka='აწყობეების ტესტი.';ru='Тест настроек.'"),
			,
			НСтр("ka='მიმდინარეობს ედბ-ის სერვისთან კავშირის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование связи с сервисом ЭДО. Подождите...'"));
...
Процедура ПослеПолученияОтпечатковПроверитьСертификаты(Отпечатки, Параметры = Неопределено) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='ხელმისაწვდომი სერთიფიკატები არ არის. ტესტი არ შესრულებულა.';ru='Нет доступных сертификатов. Тест не выполнен.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПослеПолученияОтпечатковПроверитьСвязьЭДО(Отпечатки, Параметры = Неопределено) Экспорт
	...
	ШаблонОперация = НСтр("ka='ავთენტიფიკაციის ტესტი: %1';ru='Тест аутентификации: %1'");
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		Если РезультатВыполнения.Успех = Истина Тогда
			ТекстСообщения = НСтр("ka='გამართვების პროფილის ტესტი წარმატებით შესრულდა.';ru='Тест профиля настроек выполнен успешно.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстСообщения = НСтр("ka='ედბ-ის გამართვების მოცემული პროფილით დარეგისტრირებულებს შორის ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов, среди зарегистрированных по данному профилю настроек ЭДО.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,
			,
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ka='სერვის 1C-ედბ-ზე მონაცემთა შეცვლის თაობაზე განცხადების გაგზავნა';ru='Отправка заявления на изменение данных на сервисе 1С-ЭДО'");
...
Процедура ОткрытьФормуВыбораСертификатаЭП()
	...
	Если Объект.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ka='მოქმედების შესასრულებლად საჭიროა წაშლის ნიშნულის მოხსნა.';ru='Для выполнения действия необходимо снять пометку удаления.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьНастройкуУведомлений()
	...
	Если УведомлятьОСобытиях Тогда
		ЗаголовокШаблон = НСтр("ka='Email-ი შეტყობინებებისთვის: %1';ru='Email для уведомлений: %1'");
	Иначе
		ЗаголовокШаблон = НСтр("ka='შეტყობინებები არ არის შეერთებული';ru='Уведомления не подключены'");
	КонецЕсли;
	...
	ИначеЕсли УведомлятьОСобытиях Тогда
		АдресУведомлений = НСтр("ka='<მითითებული არ არის>';ru='<не указан>'");
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеРегламентаЭДО(ТекСтрока)
	...
	Если ТекСтрока.ИспользоватьЭП И ИспользоватьЭП Тогда
		Подпись = НСтр("ka='ხელმოწერა';ru='Подпись'");
	Иначе
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='საგადასახადო სამსახური 5.01 (2016 წლიდან)';ru='ФНС 5.01 (с 2016г.)'");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ИсходящиеДокументы.ВерсияФормата");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ka='ФНС 5.01';ru='ФНС 5.01'");
