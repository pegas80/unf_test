...
Функция ЕстьВключенныеСобытия()
	...
	Если Не Результат Тогда
		...
		ТекстСообщения = НСтр("ka='შეტყობინებისთვის მოვლენები არ არის';ru='Нет событий для уведомления';en='There are no events for notification'");
...
Функция ТекстКодаОшибки(КодСостояния)
	...
	Если КодСостояния = 405 Тогда
		ТекстОшибки = НСтр("ka='კლიენტის მიერ მითითებული მეთოდის გამოყენება მიმდინარე რესურსზე არ შეიძლება';ru='Указанный клиентом метод нельзя применить к текущему ресурсу';en='Specified by the client method can not be applied to the current resource,'");
	КонецЕсли;
...
Функция СохранитьПараметрыПодписок(РасшифрованныйМаркер, ПараметрыУведомления)
	...
	Исключение
		...
		ВидОперации = НСтр("ka='ედბ-ის გამოწერის თვისებების ცვლილება';ru='Изменение свойств подписки ЭДО';en='Changing the properties of EDI subscription'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ka='შეცდომა გამართვის შენახვისას: %1';ru='Ошибка при сохранении настройки: %1';en='Error saving configuration% 1'");
		ТекстОшибки = ТекстКодаОшибки(Ответ.КодСостояния);
		
		ШаблонСообщения = НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა. შეცდომის კოდი: %1. %2';ru='Ошибка выполнения команды сервиса. Код ошибки: %1. %2';en='Error executing service commands. Error code:% 1. 2%'");
