...
Функция ЕстьВключенныеСобытия()
	...
	Если Не Результат Тогда
		...
		ТекстСообщения = НСтр("ka='შეტყობინებისთვის მოვლენები არ არის';ru='Нет событий для уведомления'");
...
Функция ТекстКодаОшибки(КодСостояния)
	...
	Если КодСостояния = 405 Тогда
		ТекстОшибки = НСтр("ka='კლიენტის მიერ მითითებული მეთოდის გამოყენება მიმდინარე რესურსზე არ შეიძლება';ru='Указанный клиентом метод нельзя применить к текущему ресурсу'");
	КонецЕсли;
...
Функция СохранитьПараметрыПодписок(РасшифрованныйМаркер, ПараметрыУведомления)
	...
	Исключение
		...
		ВидОперации = НСтр("ka='ედბ-ის გამოწერის თვისებების ცვლილება';ru='Изменение свойств подписки ЭДО'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ka='შეცდომა გამართვის შენახვისას: %1';ru='Ошибка при сохранении настройки: %1'");
		ТекстОшибки = ТекстКодаОшибки(Ответ.КодСостояния);
		
		ШаблонСообщения = НСтр("ka='სერვისის ბრძანების შესრულების შეცდომა. შეცდომის კოდი: %1. %2';ru='Ошибка выполнения команды сервиса. Код ошибки: %1. %2'");
