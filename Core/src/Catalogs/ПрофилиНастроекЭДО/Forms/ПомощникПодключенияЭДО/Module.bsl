...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
		...
		Элементы.НадписьИдентификаторУчастникаОбменаЭД.Заголовок = НСтр("ka='ედ-ის გაცვლის მონაწილის უნიკალური იდენტიფიკატორის მიღება.';ru='Получить уникальный идентификатор участника обмена ЭД.'");
		ШрифтЭлемента = Элементы.НадписьИдентификаторУчастникаОбменаЭД.Шрифт;
	...
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
		...
		Элементы.НадписьИдентификаторУчастникаОбменаЭДЧерезЭДО.Заголовок = НСтр("ka='ედ-ის გაცვლის მონაწილის უნიკალური იდენტიფიკატორის მიღება.';ru='Получить уникальный идентификатор участника обмена ЭД.'");
		ШрифтЭлемента = Элементы.НадписьИдентификаторУчастникаОбменаЭДЧерезЭДО.Шрифт;
	...
	Иначе
		Если ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			...
			ТекстВопроса = НСтр("ka='შეიცვალა ორგანიზაცია. შევცვალოთ ორგანიზაციის გაცვლის იდენტიფიკატორი?';ru='Была изменена организация. Изменить идентификатор обмена организации?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		Иначе
...
Процедура ИспользоватьЭППриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СертификатКриптографии) Тогда
		...
		ТекстВопроса = НСтр("ka='სერთიფიკატის თაობაზე მონაცემები გასუფთავდება. გავაგრძელოთ?';ru='Данные по сертификату будут очищены. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура НадписьИдентификаторУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Не ЗначениеЗаполнено(СертификатКриптографии) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='ელექტრონული ხელმოწერის გასაღების სერთიფიკატი';ru='Сертификат ключа электронной подписи'")),
			,
...
Процедура ПолучитьИдентификаторУчастникаОбменаЭДЧерез1СЭДО()
	...
	Если Не ЗначениеЗаполнено(КодНалоговогоОргана) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='საგადასახადო ორგანოს კოდი';ru='Код налогового органа'")),
			,
			"КодНалоговогоОргана");
		ВозниклиОшибки = Истина;
	Иначе
		Если СтрДлина(КодНалоговогоОргана) <> 4 Тогда
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "КОРРЕКТНОСТЬ", НСтр("ka='საგადასახადო ორგანოს კოდი';ru='Код налогового органа'"), , ,
				НСтр("ka='ველის სიგრძე არ არის 4-ის ტოლი.';ru='Длина поля не равна 4.'"));
	...
	Если Не ЗначениеЗаполнено(АдресОрганизации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='იურიდიული მისამართი';ru='Юридический адрес'")),
			,
			"АдресОрганизации");
		ВозниклиОшибки = Истина;
...
Процедура КаталогОбмена(ПутьККаталогу)
	...
	ДиалогКаталога.Заголовок = НСтр("ka='აირჩიეთ ქსელური კატალოგი გაცვლისთვის';ru='Выберите сетевой каталог для обмена'");
...
Процедура РеквизитыЮрЛицаПоИНН(Знач ИНН)
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ИНН %1:';ru='ИНН %1:'"), ИНН)
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	// Уточняющие объекты
	Результат.Добавить(СтрокаОбъектаАдресации("10100000", НСтр("ka='სამისამართო წერტილი';ru='Адресная точка'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10200000", НСтр("ka='მებაღეთა ამხანაგობა';ru='Садовое товарищество'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10300000", НСтр("ka='მებაღეთა ამხანაგობა';ru='Садовое товарищество'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10400000", НСтр("ka='სამრეწველო ზონა';ru='Промышленная зона'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10500000", НСтр("ka='გარაჟების სამშენებლო კოოპერატივი';ru='Гаражно-строительный кооператив'")));
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, ИНН)
	...
	Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
		...
		ОписаниеОшибки = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'");
		ТекстСобытия   = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, """status"":401") > 0 Тогда
		ТекстОшибки  = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'");
		ТекстСобытия = НСтр("ka='შეცდომა მისაწვდომია';ru='Ошибка доступа'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-1") > 0 Тогда
		ТекстОшибки  = НСтр("ka='იურიდიული პირის გსნ არ არის მითითებული';ru='Не указан ИНН юридического лица'");
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-2") > 0 Тогда
		ТекстОшибки  = НСтр("ka='ИНН юридического лица должен состоять из 10 цифр';ru='ИНН юридического лица должен состоять из 10 цифр'");
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-3") > 0 Тогда
		ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='გსნ %1-ის მიხედვით რეკვიზიტების შესავსებად მონაცემები ვერ მოიძებნა';ru='Не удалось найти данные для заполнения реквизитов по ИНН %1'"), ИНН);
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-4") > 0 Тогда
		ТекстОшибки  = НСтр("ka='მეწარმის გსნ არ არის მითითებული';ru='Не указан ИНН предпринимателя'");
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-5") > 0 Тогда
		ТекстОшибки  = НСтр("ka='მეწარმის გსნ 12 ციფრისგან უნდა შედგებოდეს';ru='ИНН предпринимателя должен состоять из 12 цифр'");
		ТекстСобытия = НСтр("ka='მონაცემთა მიღების შეცდომა';ru='Ошибка получения данных'", КодОсновногоЯзыка);
	...
	Иначе
		ТекстОшибки  = НСтр("ka='Ошибка при работе с сервисом (подробнее см. Журнал регистрации)';ru='Ошибка при работе с сервисом (подробнее см. Журнал регистрации)'");
		ТекстСобытия = НСтр("ka='Ошибка при работе с сервисом';ru='Ошибка при работе с сервисом'", КодОсновногоЯзыка);
	КонецЕсли;
	
	ИмяСобытия = НСтр("ka='ერთიანი სახ_რეესტრების მონაცემთა სერვისი.';ru='Сервис данных единых гос_реестров.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
...
Процедура ПодготовитьДанныеДляРегистрационногоПакета1СЭДОНачало()
	...
	Операция = НСтр("ka='სერვის 1C-ედბ-სთან შეერთებაზე სარეგისტრაციო მონაცემებზე ხელმოწერა';ru='Подписание регистрационных данных на подключение к сервису 1С-ЭДО'");
	...
	Представление = НСтр("ka='სამომხმარებლო ხელშეკრულება 1C:ედბ-ის სერვისთან შეერთების თაობაზე';ru='Пользовательское соглашение на подключение к сервису 1С-ЭДО'");
	...
	Представление = НСтр("ka='მონაცემები ედბ-ის ოპერატორთან სააღრიცხვო ჩანაწერის რეგისტრაციისთვის';ru='Данные для регистрации учетной записи у оператора ЭДО'");
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ka='სამომხმარებლო შეთანხმება, მონაცემები ედბ-ის ოპერატორთან რეგისტრაციისთვის';ru='Пользовательское соглашение, Данные для регистрации у оператора ЭДО'"));
...
Процедура ОбработатьПредставлениеДанныхДляОператораЭДО(РезультатВыполнения, Контекст) Экспорт
	...
	ШаблонИмениВременногоФайла = НСтр("ka='მონაცემები ედბ-ის ოპერატორთან რეგისტრაციისთვის_%1';ru='Данные для регистрации у оператора ЭДО_%1'");
...
"Необходимо повторить получение идентификатора участника ЭДО.';ka='ედბ-ის ოპერატორის სერთიფიკატის მიღების შეცდომა. აუცილებელია ედბ-ის მონაწილის იდენტიფიკატორის მიღების გამეორება.'");
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='მონაცემთა დაშიფრვა ედბ-ის ოპერატორთან რეგისტრაციისთვის';ru='Шифрование данных для регистрации у оператора ЭДО'"));
	...
	Представление.Вставить("Представление", НСтр("ka='ედბ-ის ოპერატორთან სარეგისტრაციო მონაცემები';ru='Данные для регистрации у оператора ЭДО'"));
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ka='სერვის 1С-ედბ-ზე რეგისტრაციის შესახებ განცხადების გაგზავნა';ru='Отправка заявления о регистрации на сервисе 1С-ЭДО'");
...
Процедура ТестПрофиляНастроекЭДО()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторОрганизации)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='ორგანიზაციის იდენტიფიკატორი';ru='Идентификатор организации'")),
			,
			"ИдентификаторОрганизации",
			,
			Отказ);
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
		Если Не ЗначениеЗаполнено(ЭлектроннаяПочтаОрганизации) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='ორგანიზაციის ელექტრონული ფოსტა';ru='Электронная почта организации'")),
			,
	...
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
		Если Не ЗначениеЗаполнено(КаталогВходящихДокументов) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='შემომავალი დოკუმენტების კატალოგი';ru='Каталог входящих документов'")),
			,
	...
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезFTP") Тогда
		Если Не ЗначениеЗаполнено(АдресСервераFTP) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ka='სერვერის მისამართი';ru='Адрес сервера'")),
			,
			"АдресСервераFTP",
			,
			Отказ);
	...
	Если Не ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
		...
		Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
			...
			ИначеЕсли ТипРегистрации = 2 И Не ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='აუცილებელია ედ-ის გაცვლის მონაწილის უნიკალური იდენტიფიკატორის მიღება.';ru='Необходимо получить уникальный идентификатор участника обмена ЭД.'"), , , , Отказ);
			КонецЕсли;
		ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
			...
			Иначе
				...
				Если ДлинаИдентификатора <> 46 Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "КОРРЕКТНОСТЬ", НСтр("ka='ორგანიზაციის იდენტიფიკატორი';ru='Идентификатор организации'"), , ,
						НСтр("ka='ველის სიგრძე არ არის 46-ის ტოლი.';ru='Длина поля не равна 46.'")),
						,
						"ИдентификаторОрганизации",
						,
						Отказ);
	...
	Если Не ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
		...
		Состояние(НСтр("ka='ედბ-ის გამართვების პროფილის შექმნა.';ru='Создание профиля настроек ЭДО.'"),
			,
			НСтр("ka='მიმდინარეობს ოპერატორთან კავშირის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование связи с оператором. Подождите...'"));
	...
	Иначе
		...
		Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			...
			Состояние(НСтр("ka='ედბ-ის გამართვების პროფილის შექმნა.';ru='Создание профиля настроек ЭДО.'"),
				,
				НСтр("ka='მიმდინარეობს ელექტრონული ფოსტით ედ-ის გაცვლის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование обмена ЭД через электронную почту. Подождите...'"));
		...
		Иначе
			Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
				...
				Состояние(НСтр("ka='ედბ-ის გამართვების პროფილის შექმნა.';ru='Создание профиля настроек ЭДО.'"),
					,
					НСтр("ka='მიმდინარეობს კატალოგით ედ-ის გაცვლის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование обмена ЭД через каталог. Подождите...'"));
			...
			ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезFTP") Тогда
				...
				Состояние(НСтр("ka='ედბ-ის გამართვების პროფილის შექმნა.';ru='Создание профиля настроек ЭДО.'"),
					,
					НСтр("ka='მიმდინარეობს FTP-ით ედ-ის გაცვლის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование обмена ЭД через FTP. Подождите...'"));
...
"%1';ka='სააღრიცხვო ჩანაწერის პარამეტრების შემოწმება შეცდომებით დასრულდა: %1'"), СообщениеОбОшибке ));
	Иначе
		ПоказатьОповещениеПользователя(НСтр("ru='Проверка учетной записи эл. почты'"),,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სააღრიცხვო ჩანაწერის პარამეტრების შემოწმება წარმატებით დასრულდა. %1';ru='Проверка параметров учетной записи завершилась успешно. %1'"),
			ДополнительноеСообщение));
	КонецЕсли;
...
Процедура СохранитьПараметры(ДанныеСохранены)
	...
	Попытка
		...
		ШаблонНаименование = НСтр("ka='%1, %2';ru='%1, %2'");
...
Процедура ТестСвязиСервисаЭДОЗавершение()
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ka='ედბ-ის სერვისში ავთენტიფიკაციის ტესტი';ru='Тест аутентификации в сервисе ЭДО'"));
...
Функция НастройкиОповещения(РасшифрованныйМаркер, Идентификатор)
	...
	ВидОперации = НСтр("ka='ინფორმაციის მიღება ედბ-ის ხელმოწერის თვისებების შესახებ';ru='Получение информации о свойствах подписки ЭДО'");
