...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
		...
		Если СертификатыПодписейОрганизации.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Список", "Заполнение", , , НСтр("ka='ორგანიზაციის სერტიფიკატები';ru='Сертификаты организации'")),
				ЭтотОбъект,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПометкаУдаления Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			ТекстСообщения = НСтр("ka='ოპერაცია გაუქმებულია. ედბ-ის გამართვების მიმდინარე პროფილი გამოიყენება ედბ-ის მოქმედ გამართვებში.';ru='Операция отменена. Текущий профиль настроек ЭДО используется в действующих настройках ЭДО.'");
