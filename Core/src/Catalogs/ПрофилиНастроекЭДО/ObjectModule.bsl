...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
		...
		Если СертификатыПодписейОрганизации.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Список", "Заполнение", , , НСтр("ka='ორგანიზაციის სერტიფიკატები';ru='Сертификаты организации';en='certificates organization'")),
				ЭтотОбъект,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПометкаУдаления Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			ТекстСообщения = НСтр("ka='ოპერაცია გაუქმებულია. ედბ-ის გამართვების მიმდინარე პროფილი გამოიყენება ედბ-ის მოქმედ გამართვებში.';ru='Операция отменена. Текущий профиль настроек ЭДО используется в действующих настройках ЭДО.';en='Operation canceled. Current profile settings EDC is used in the existing EDM settings.'");
...
Функция ПрофильНастроекЭДОУникален() Экспорт
	...
	Если Не ТекущийПрофильНастроекУникален Тогда
		ШаблонСообщения = НСтр("ka='საინფორმაციო ბაზაში უკვე არსებობს გამართვების პროფილი შემდეგი რეკვიზიტებით:"
"ორგანიზაცია - %1;"
"ორგანიზაციის იდენტიფიკატორი - %2;"
"გაცვლის მეთოდი - %3;';ru='В информационной базе уже существует профиль настроек с реквизитами:"
"Организация - %1;"
