...
Функция ПроверитьУмещение()
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Лево = 0 И ПолеТабличногоДокумента.ОбластьПечати.Право = 0) Тогда
		...
		Если КоличествоПоГоризонтали <> МассивТаблиц.Количество() Тогда
			ТекстСообщения = НСтр("ru='Максимальное количество по горизонтали: ';ka='მაქსიმალური რაოდენობა თარაზულად:'") + МассивТаблиц.Количество();
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Верх = 0 И ПолеТабличногоДокумента.ОбластьПечати.Низ = 0) Тогда
		...
		Если КоличествоПоВертикали <> МассивТаблиц.Количество() Тогда
			ТекстСообщения = НСтр("ru='Максимальное количество по вертикали: ';ka='მაქსიმალური რაოდენობა შვეულად:'") + МассивТаблиц.Количество();
...
Процедура УстановитьОбластьПечати(Команда)
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='ბეჭდვის არაკორექტული არეალი';ru='Некорректная область печати';en='Incorrect print area'");
...
Процедура ЭтикеткаПоУмолчанию(Команда)
	...
	ПоказатьВопрос(Оповещение,НСтр("ka='რედაქტირებადი შაბლონი შეიცვლება ძირითადი შაბლონით, გავაგრძელოთ?';ru='Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?';en='Editable template will be replaced with the default template, proceed?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура Объединить(Команда)
	...
	Иначе
		...
		ОчиститьСообщения();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='არაკორექტული არეალი!';ru='Некорректная область!';en='Invalid area!'");
		Сообщение.Поле = "ПолеТабличногоДокумента";
		Сообщение.Сообщить();
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если СохранятьИзменения Тогда
		Если ПолеТабличногоДокумента.ОбластьПечати = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='ბეჭდვის არეალი არ არის დადგენილი';ru='Не установлена область печати';en='Do not set the print area'"),
				ЭтаФорма,
	...
	ИначеЕсли Модифицированность Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?';en='The data have been changed. I save changes?'"),
