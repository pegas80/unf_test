...
Процедура УстановитьОбластьПечати(Команда)
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ka='ბეჭდვის არაკორექტული არეალი';ru='Некорректная область печати'");
...
Процедура ЭтикеткаПоУмолчанию(Команда)
	...
	ПоказатьВопрос(Оповещение,НСтр("ka='რედაქტირებადი შაბლონი შეიცვლება ძირითადი შაბლონით, გავაგრძელოთ?';ru='Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура Объединить(Команда)
	...
	Иначе
		...
		ОчиститьСообщения();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='არაკორექტული არეალი!';ru='Некорректная область!'");
		Сообщение.Поле = "ПолеТабличногоДокумента";
		Сообщение.Сообщить();
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если СохранятьИзменения Тогда
		Если ПолеТабличногоДокумента.ОбластьПечати = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='ბეჭდვის არეალი არ არის დადგენილი';ru='Не установлена область печати'"),
				ЭтаФорма,
	...
	ИначеЕсли Модифицированность Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Данные были изменены. Сохранить изменения?'"),
