...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Наименование = ?(ПустаяСтрока(Строка(Код)), НСтр("ka='სამუშაო ადგილი';ru='Рабочее место';en='Workplace'"), Строка(Код))
	             + ?(ПустаяСтрока(Строка(Код)), ": ", "/")
	             + ?(ПустаяСтрока(Строка(ТекущийПользователь)), НСтр("ka='მომხმარებელი';ru='Пользователь';en='User'"), Строка(ТекущийПользователь));
