...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Наименование = ?(ПустаяСтрока(Строка(Код)), НСтр("ka='სამუშაო ადგილი';ru='Рабочее место'"), Строка(Код))
	             + ?(ПустаяСтрока(Строка(Код)), ": ", "/")
	             + ?(ПустаяСтрока(Строка(ТекущийПользователь)), НСтр("ka='მომხმარებელი';ru='Пользователь'"), Строка(ТекущийПользователь));
