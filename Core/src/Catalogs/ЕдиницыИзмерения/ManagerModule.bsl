	...
	Если НЕ Параметры.Свойство("Рекурсия")
		...
		Если НЕ (Параметры.Свойство("НеИспользоватьКлассификатор") И Параметры.НеИспользоватьКлассификатор = Истина) Тогда
			...
			// Дополним стандартный список базовой единицей измерения номенклатуры по классификатору.
				ПредставлениеЕдиницыИзмерения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (შენახვის ერთეული)';ru='%1 (ед. хранения)'"),
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ნომენკლატურა არ არის შევსებული!';ru='Не заполнена номенклатура!'"));
			КонецЕсли;
		КонецЕсли;
