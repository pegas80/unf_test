...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	Если КоличествоЛишнихФайлов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='დისკზე არცერთი ზედმეტი ფაილი არ არის';ru='Нет ни одного лишнего файла на диске'"));
		Возврат;
	КонецЕсли;
	...
	Если ЧислоУдаленныхФайлов <> 0 Тогда
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='წაშლილია ფაილი: %1';ru='Удалено файлов: %1'"),
			ЧислоУдаленныхФайлов);
		ПоказатьОповещениеПользователя(
			НСтр("ka='ზედმეტი ფაილების წაშლა დასრულებულია.';ru='Завершено удаление лишних файлов.'"),
			,
			ТекстОповещения,
...
Процедура ЗаполнитьТаблицуЛишнихФайлов()
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		НоваяСтрока.СтатусПроверки = НСтр("ka='ზედმეტი ფაილები (არის დისკზე, მაგრამ მათ შესახებ ინფორმაცია არ არის)';ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)'");
	...
	МассивЛишнихФайлов = ТаблицаФайловНаДиске.НайтиСтроки(
		Новый Структура("СтатусПроверки", НСтр("ka='ზედმეტი ფაილები (არის დისკზე, მაგრამ მათ შესახებ ინფორმაცია არ არის)';ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)'")));
...
Процедура ОбработатьСообщениеОбОшибке(ИмяФайла, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. ზედმეტი ფაილების წაშლის შეცდომა';ru='Файлы.Ошибка удаления лишних файлов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура СформироватьОтчетОбОшибках(ОтчетОбОшибках, МассивФайловСОшибками)
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("ka='შეცდომებიანი ფაილები:';ru='Файлы с ошибками:'");
