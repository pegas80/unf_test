...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ДополнительныеСвойства.Свойство("ПропуститьОсновнуюПроверкуЗаполнения") Тогда
		...
		Если Не НомерПорядкаУникален(ПорядокЗаполнения, Ссылка) Тогда
			ТекстОшибки = НСтр("ka='შევსების რიგითობა არ არის უნიკალური - სისტემაში უკვე არის ტომი ამგვარი რიგითობით';ru='Порядок заполнения не уникален - в системе уже есть том с таким порядком'");
		...
		Если МаксимальныйРазмер <> 0 Тогда
			...
			Если МаксимальныйРазмер < ТекущийРазмер Тогда
				ТекстОшибки = НСтр("ka='ტომის მაქსიმალური ზომა ნაკლებია, ვიდრე მიმდინარე ზომა';ru='Максимальный размер тома меньше, чем текущий размер'");
		...
		Если ПустаяСтрока(ПолныйПутьWindows) И ПустаяСтрока(ПолныйПутьLinux) Тогда
			ТекстОшибки = НСтр("ka='არ არის შევსებული სრული გზა';ru='Не заполнен полный путь'");
		...
		Если Не ПолучитьФункциональнуюОпцию("ИспользуютсяПрофилиБезопасности")
		   ...
		   И (    Лев(ПолныйПутьWindows, 2) <> "\\"
			...
			ТекстОшибки = НСтр("ka='ტომისკენ გზა უნდა იყოს UNC ფორმატში (\\servername\resource).';ru='Путь к тому должен быть в формате UNC (\\servername\resource).'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ПолныйПутьWindows", "Объект", Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
