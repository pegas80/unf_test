...
Процедура Загрузить(Команда)
	...
	Если СтрокиВыбранныхДокументов.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='Для загрузки необходимо выбрать хотя бы один документ.';ru='Для загрузки необходимо выбрать хотя бы один документ.'");
...
Процедура ЗагрузитьФайлЗавершение(Результат, АдресФайлаОбменаВоВременномХранилище, ИмяФайлаОбмена, ДополнительныеПараметры) Экспорт
	...
	Если НЕ КонтекстЭДОКлиент.ВыбранКорректныйФайл(ИмяФайлаОбмена, ".zip") Тогда
		ТекстПредупреждения = НСтр("ka='Имя выбираемого файла должно иметь формат %1*.zip';ru='Имя выбираемого файла должно иметь формат %1*.zip'");
...
Функция ЗаполнитьТаблицуЗагруженныеДокументы(ДеревоXML)
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла описания.';ru='Некорректная структура XML файла описания.'"));
		Возврат Ложь;
	КонецЕсли;
