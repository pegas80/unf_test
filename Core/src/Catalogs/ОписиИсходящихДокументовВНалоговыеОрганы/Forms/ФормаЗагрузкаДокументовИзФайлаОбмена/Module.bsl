...
Процедура Загрузить(Команда)
	...
	Если СтрокиВыбранныхДокументов.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='Для загрузки необходимо выбрать хотя бы один документ.';ru='Для загрузки необходимо выбрать хотя бы один документ.';en='To download you must select at least one document.'");
...
Процедура ЗагрузитьФайлЗавершение(Результат, АдресФайлаОбменаВоВременномХранилище, ИмяФайлаОбмена, ДополнительныеПараметры) Экспорт
	...
	Если НЕ КонтекстЭДОКлиент.ВыбранКорректныйФайл(ИмяФайлаОбмена, ".zip") Тогда
		ТекстПредупреждения = НСтр("ka='Имя выбираемого файла должно иметь формат %1*.zip';ru='Имя выбираемого файла должно иметь формат %1*.zip';en='The name of the file must be in the format% 1 * .zip'");
...
Функция ЗаполнитьТаблицуЗагруженныеДокументы(ДеревоXML)
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла описания.';ru='Некорректная структура XML файла описания.';en='Incorrect structure of the XML description file.'"));
		Возврат Ложь;
	КонецЕсли;
