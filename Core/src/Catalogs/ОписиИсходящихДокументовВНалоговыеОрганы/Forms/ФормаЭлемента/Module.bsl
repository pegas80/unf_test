...
Процедура РазбитьНаНесколько(Команда)
	...
	ТекстВопроса = НСтр("ka='გასაგრძელებლად საჭიროა შეყვანილი ინფორმაციის შენახვა. გავაგრძელოთ?';ru='Для продолжения необходимо сохранить введеную информацию."
"Продолжить?'");
	...
	Если НЕ РезультатЗаписи Тогда
		ПоказатьПредупреждение(, НСтр("ka='შენახვისას შეცდომები წარმოიშვა! გაგრძელება შეუძლებელია.';ru='Во время сохранения произошли ошибки! Продолжение невозможно.';en='While saving error occured! It can not continue.'"));
		Возврат;
	КонецЕсли;

	ТекстВопроса = НСтр("ka='Текущий ответ на требование будет разбит на несколько. Продолжить?';ru='Текущий ответ на требование будет разбит на несколько. Продолжить?';en='The current response to the demand will be split into several. Proceed?'");
...
Процедура ДобавитьДокументы(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнено поле ""Требование""';ru='Не заполнено поле ""Требование""';en='Do not fill in the ""requirement""'"), , "Объект.Требование");
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьДокументыНДСЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если ВидДокумента = Неопределено  Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выбор отменен. Не удалось определить вид выбранного документа.';ru='Выбор отменен. Не удалось определить вид выбранного документа.';en='Unselected. Unable to determine the type of the selected document.'"));
			Возврат;
		КонецЕсли;
...
Функция ОтработатьРезультатДобавленияДокументовФайлаОбменаНаСервере(ПунктТребования, СтруктураВыбора)
	...
	Если НЕ ЗагрузитьФайлыИзФайлаОбменаВИБ(Объект.Ссылка, ПолноеИмяФайлаОбмена, МассивИменНовыхФайлов) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Произошла ошибка при загрузке документов из файла обмена.';ru='Произошла ошибка при загрузке документов из файла обмена.';en='An error occurred while loading the documents from the file sharing.'"));
		Возврат Ложь;
	КонецЕсли; 
		...
		Для каждого ЭлементГруппы Из ГруппаДокументов.ПолучитьЭлементы() Цикл
			...
			Если ЭлементГруппы.КоличествоФайлов = 0 Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='У документа %1 отсутствуют прикрепленные файлы.';ru='У документа %1 отсутствуют прикрепленные файлы.';en='Your document% 1 no attachments.'"), ЭлементГруппы.СоставноеПоле);
