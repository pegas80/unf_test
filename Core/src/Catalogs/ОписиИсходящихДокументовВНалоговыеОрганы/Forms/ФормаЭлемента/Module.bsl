...
"Продолжить?'");
	...
	Если НЕ РезультатЗаписи Тогда
		ПоказатьПредупреждение(, НСтр("ka='შენახვისას შეცდომები წარმოიშვა! გაგრძელება შეუძლებელია.';ru='Во время сохранения произошли ошибки! Продолжение невозможно.'"));
		Возврат;
	КонецЕсли;

	ТекстВопроса = НСтр("ka='Текущий ответ на требование будет разбит на несколько. Продолжить?';ru='Текущий ответ на требование будет разбит на несколько. Продолжить?'");
...
Процедура ДобавитьДокументы(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнено поле ""Требование""';ru='Не заполнено поле ""Требование""'"), , "Объект.Требование");
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьДокументыНДСЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если ВидДокумента = Неопределено  Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выбор отменен. Не удалось определить вид выбранного документа.';ru='Выбор отменен. Не удалось определить вид выбранного документа.'"));
			Возврат;
		КонецЕсли;
...
Функция ОтработатьРезультатДобавленияДокументовФайлаОбменаНаСервере(ПунктТребования, СтруктураВыбора)
	...
	Если НЕ ЗагрузитьФайлыИзФайлаОбменаВИБ(Объект.Ссылка, ПолноеИмяФайлаОбмена, МассивИменНовыхФайлов) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Произошла ошибка при загрузке документов из файла обмена.';ru='Произошла ошибка при загрузке документов из файла обмена.'"));
		Возврат Ложь;
	КонецЕсли; 
		...
		Для каждого ЭлементГруппы Из ГруппаДокументов.ПолучитьЭлементы() Цикл
			...
			Если ЭлементГруппы.КоличествоФайлов = 0 Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='У документа %1 отсутствуют прикрепленные файлы.';ru='У документа %1 отсутствуют прикрепленные файлы.'"), ЭлементГруппы.СоставноеПоле);
