...
Процедура НадписьПервичноеСообщениеНажатие(Элемент)
	...
	ПолучитьФайл(Адрес, НСтр("ka='ხელმოწერა';ru='Подпись';en='Signature'") + Объект.ИмяФайлаВыгрузки);
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ka='ოქმი';ru='Протокол';en='Protocol'") + ИмяФайлаВыгрузкиДоТочки + ".html");
...
Процедура ВыгрузитьКвитанцию(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ka='კვიტანცია';ru='Квитанция';en='Receipt'") + ИмяФайлаВыгрузкиДоТочки + ".xml");
...
Процедура ОбновитьФорму()
	...
	Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
		...
		Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='ანგარიშგების დამუშავების პროტოკოლი არ არის';ru='Протокол обработки отчета отсутствует';en='report no treatment protocol'");
	...
	Иначе
		...
		Если Объект.НаКонтроле Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='პროტოკოლი მიღებულია';ru='Протокол получен';en='protocol received'");
