...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если НЕ ЗначениеЗаполнено(НоменклатураВладелец)
			...
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ka='სერიული ნომრები შენახულია მხოლოდ მარაგებისთვის';ru='Серийные номера хранятся только для запасов'");
...
Функция СоздатьСерйныйНомер(СерийныйНомерСтрока, НоменклатураВладелец)
	...
	Попытка
		...
		СтрокаСообщения = НСтр("ka='შექმნილია სერიული ნომერი: %1%';ru='Создан серийный номер: %1%'");
...
Функция ПолучитьСерийныйНомерПоШтрихкоду(ДанныеШтрихкода, НоменклатураВладелец)
	...
	Иначе	
		СтрокаСообщения = НСтр("ka='შეყვანილი შტრიხკოდი %1% მიბმულია სხვა ნომენკლატურაზე (სერიულ ნომერზე): %2%';ru='Введенный штрихкод %1% привязан к другой номенклатуре (серийному номеру): %2%'");
