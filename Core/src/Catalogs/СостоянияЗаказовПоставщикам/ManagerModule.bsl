...
Процедура ЗаполнитьПоставляемыеСостояния() Экспорт
	...
	Попытка
		...
		Состояние.Наименование	= НСтр("ka='მუშაობაში';ru='В работе';en='The paper'");
...
"%1';ka='Не удалось заполнить справочник ""Состояния заказов поставщикам"" по умолчанию по причине:"
"%1'"), 
...
Функция ПолучитьСостояниеПоЗначениюСтатуса(Знач СтатусЗаказа) Экспорт

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СтатусВнутреннегоЗаказа", СтатусЗаказа);
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	Справочник.СостоянияЗаказовПоставщикам КАК СостоянияЗаказовПоставщикам
	|ГДЕ
	|	СостоянияЗаказовПоставщикам.СтатусВнутреннегоЗаказа = &СтатусВнутреннегоЗаказа";
	...
	Иначе
		...
		Выборка.Следующий();
		Возврат Выборка.СостояниеЗаказа;
	КонецЕсли;
