...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("Владелец") И ТипЗнч(Параметры.Отбор.Владелец) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		ИначеЕсли Параметры.Свойство("ЭтоПриходныйДокумент") И Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			ТекстСообщения = НСтр("ka='გარეშე კონტრაგენტების მომსახურებებისთვის მახასიათებლების მიხედვით აღრიცხვა არ წარმოებს!';ru='Для услуг сторонних контрагентов не ведется учет по характеристикам!'");
		ИначеЕсли НЕ Номенклатура.ИспользоватьХарактеристики Тогда
			ТекстСообщения = НСтр("ka='ნომენკლატურისთვის არ წარმოებს აღრიცხვა მახასიათებლების მიხედვით!';ru='Для номенклатуры не ведется учет по характеристикам!'");
		КонецЕсли;
