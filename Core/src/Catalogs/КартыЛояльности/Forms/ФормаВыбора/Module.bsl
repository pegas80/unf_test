...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ОтборПоДатеНачала") И Параметры.ОтборПоДатеНачала Тогда
		
		Если Не Параметры.Свойство("ДатаНачала") Тогда
		
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(Список, "ДатаНачала", Параметры.ДатаНачала, Истина, ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
		КонецЕсли;
	КонецЕсли;
	
	Если Параметры.Свойство("ОтборПоДатеОкончания") И Параметры.ОтборПоДатеНачала Тогда
		
		Если Не Параметры.Свойство("ДатаНачала") Тогда
		
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(Список, "ДатаНачала", Параметры.ДатаНачала, Истина, ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
		КонецЕсли;
	КонецЕсли;
	...
	Если ПолученныйТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
		...
		Иначе
			...
			Иначе
				...
				Результат.ПоказатьВыборЭлемента(Оповещение, НСтр("ka='მაგნიტური ბარათის კოდის არჩევა';ru='Выбор кода магнитной карты';en='Choosing a magnetic card code'"));
			КонецЕсли;
		КонецЕсли;
	Иначе
