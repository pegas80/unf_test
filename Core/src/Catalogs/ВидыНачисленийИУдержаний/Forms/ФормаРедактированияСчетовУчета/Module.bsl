...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетЗатрат(ВидНачисленияУдержания) Тогда
		...
		Элементы.ГруппаСчетовУчета.Подсказка	= НСтр("ka='ბაზაში არის მოძრაობები დარიცხვის (დაკავების) ამ ტიპის მიხედვით! აღრიცხვის დათვლის შეცვლა აკრძალულია!';ru='В базе есть движения по этому виду начисления (удержания)! Изменение счета учета запрещено!'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоНалог Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='დარიცხვის ტიპისთვის სახეობისთვის აღრიცხვის ანგარიშის გადასახადი არ რედაქტირდება!';ru='Для типа вида начисления Налог счета учета не редактируются!'"));
		Отказ = Истина;
