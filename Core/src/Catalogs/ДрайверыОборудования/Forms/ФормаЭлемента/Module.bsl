...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ИдентификаторОбъекта.ПодсказкаВвода = ?(ПоставляемыйВСоставеКонфигурации, НСтр("ka='<მითითებული არ არის>';ru='<Не указан>'"), 
		НСтр("ka='<ProgID компоненты не введен>';ru='<ProgID компоненты не введен>'"));
	Элементы.ИмяМакетаДрайвера.ПодсказкаВвода = ?(ПоставляемыйВСоставеКонфигурации, НСтр("ka='<მითითებული არ არის>';ru='<Не указан>'"), "");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ТипОборудования) Тогда 
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოწყობილობის ტიპი არ არის მითითებული.';ru='Тип оборудования не указан.'")); 
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.Наименование) Тогда 
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სახელწოდება არ არის მითითებული.';ru='Наименование не указано.'")); 
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузитьФайлДрайвераКоманда(Команда)
	...
	Если ПоставляемыйВСоставеКонфигурации Тогда
		...
		Если ПустаяСтрока(Объект.ИмяМакетаДрайвера) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დრაივერის მაკეტის სახელი არ არის მითითებული.';ru='Имя макета драйвера не указано.'"));
			Возврат;
		Иначе
	...
	Иначе 
		...
		Если ПустаяСтрока(Объект.ИмяФайлаДрайвера) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დრაივერის ფაილი არ არის ჩატვირთული.';ru='Файл драйвера не загружен.'"));
			Возврат;
		КонецЕсли;
...
Процедура ЗагрузитьФайлДрайвераКоманда(Команда)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ka='მოცემული ფუნქციონალი ხელმისაწვდომია მხოლოდ თხელი და მსხვილი კლიენტის რეჟიმში.';ru='Данный функционал доступен только в режиме тонкого и толстого клиента.'"));
		Возврат;
	#КонецЕсли
...
Процедура ПолучитьИнформациюДрайвераПоФайлу_Завершение(Результат, ПараметрыВыполнения) Экспорт 
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='ფაილის არასწორი გაფართოება.';ru='Неверное расширение файла.'"));
	КонецЕсли;
...
Функция ПолучитьОписаниеРевизииИнтерфейса(РевизияИнтерфейса)
	...
	Если НоваяАрхитектура Тогда
		Если РевизияИнтерфейса >= 2000 Тогда
			Результат = Символы.НПП + НСтр("ka='(დრაივერების შემუშავებისადმი არსებული მოთხოვნების ვერსია %Версия%)';ru='(Версия требований к разработке драйверов %Версия%)'");
...
Процедура ОбновитьТекущийСтатусДрайвера()
	...
	Если НоваяАрхитектура И ИнтеграционныйКомпонент Тогда
		ТекущийСтатусДрайвера = НСтр("ka='დაყენებულია საინტეგრაციო კომპონენტი.';ru='Установлен интеграционный компонент.'") + Символы.НПП;
		ТекущийСтатусДрайвера = ТекущийСтатусДрайвера + ?(ОсновнойДрайверУстановлен, НСтр("ka='დაყენებულია დრაივერის ძირითადი მიწოდება.';ru='Установлена основная поставка драйвера.'"),
																					 НСтр("ka='დრაივერის ძირითადი მიწოდება არ არის დაყენებული.';ru='Основная поставка драйвера не установлена.'")); 
	Иначе
		ТекущийСтатусДрайвера = НСтр("ka='დაყენებულია მიმდინარე კომპიუტერზე.';ru='Установлен на текущем компьютере.'");
	КонецЕсли;
...
Процедура ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, Параметры) Экспорт
	...
	Если ПустаяСтрока(Объект.ИдентификаторОбъекта) И ПоставляемыйВСоставеКонфигурации Тогда
		ТекущийСтатусДрайвера = НСтр("ka='დრაივერის დაყენება არ არის საჭირო.';ru='Установка драйвера не требуется.'");
	ИначеЕсли ПустаяСтрока(ОбъектДрайвера) Тогда
		ТекущийСтатусДрайвера = НСтр("ka='მიმდინარე კომპიუტერზე არ არის დაყენებული. დადგენილი არ არის ტიპი:';ru='Не установлен на текущем компьютере. Не определен тип:'") + Символы.НПП + Объект.ИдентификаторОбъекта;
...
Процедура ОбновитьСостояниеЭлементов();
	...
	Если НЕ ПустаяСтрока(ИмяФайлДрайвераНаФорме) Или ПоставляемыйВСоставеКонфигурации Тогда
		Если ПустаяСтрока(Объект.ИдентификаторОбъекта) Тогда
			ДополнительнаяИнформация = НСтр("ka='კომპონენტის ProgID არ არის მითითებული, ან დრაივერის დაყენება არ არის საჭირო.';ru='Не указан ProgID компоненты или установка драйвера не требуется.'");
		ИначеЕсли Объект.ПоставляетсяДистрибутивом Тогда
			ДополнительнаяИнформация = НСтр("ka='დრაივერის მოწოდება ხდება მომწოდებლის დისტრიბუტივის სახით.';ru='Драйвер поставляется в виде дистрибутива поставщика.'");
		Иначе
			ДополнительнаяИнформация = НСтр("ka='დრაივერის მოწოდება ხორციელდება არქივის სახით.';ru='Драйвер поставляется в виде архива.'")
				+ ?(ПустаяСтрока(Объект.ВерсияДрайвера), "", Символы.ПС + НСтр("ka='დრაივერის ვერსია არქივში:';ru='Версия драйвера в архиве:'")
				+ Символы.НПП + Объект.ВерсияДрайвера);
		КонецЕсли;
	Иначе
		ДополнительнаяИнформация = НСтр("ka='დაყენებული დრაივერის შეერთება ლოკალურ კომპიუტერებზე.';ru='Подключение установленного драйвера на локальных компьютерах.'");
	КонецЕсли;
...
Процедура УстановитьДрайверИзАрхиваПриЗавершении(Результат) Экспорт 
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Установка драйвера завершена.';ru='Установка драйвера завершена.'")); 
	ОбновитьСтатусДрайвера();
...
Процедура УстановитьДрайверИзДистрибутиваПриЗавершении(Результат, Параметры) Экспорт 
	...
	Если Результат Тогда
		...
		ОбновитьСтатусДрайвера();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='При установке драйвера из дистрибутива произошла ошибка.';ru='При установке драйвера из дистрибутива произошла ошибка.'")); 
	КонецЕсли;
