...
Процедура ПометитьНаУдалениеСнятьПометку(Команда)
	...
	Иначе
		Если ЕстьПометкаУдаления Тогда
			ТекстВопроса = НСтр("ka='გამოყოფილ ელემენტებს მოვხსნათ წაშლის ნიშნული?';ru='Снять с выделенных элементов пометку на удаление?'");
		Иначе
			ТекстВопроса = НСтр("ka='გამოყოფილი ელემენტები მოვნიშნოთ წასაშლელად?';ru='Пометить выделенные элементы на удаление?'");
		КонецЕсли;
	КонецЕсли;
