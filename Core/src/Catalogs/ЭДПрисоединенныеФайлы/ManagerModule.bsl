...
Процедура ЗаполнитьНаименованиеФайла() Экспорт
	...
	Пока ЭлементСсылка.Следующий() Цикл
		...
		Исключение
			Операция = НСтр("ka='მიერთებული ფაილის ჩაწერა';ru='Запись присоединенного файла';en='Recording attached file'");
...
Процедура СообщитьОбОшибке(Знач Описание)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='Сертификаты электронной подписи.Перенос настроек в новый объект метаданных';ru='Сертификаты электронной подписи.Перенос настроек в новый объект метаданных';en='Certificates podpisi.Perenos email settings to the new object metadata'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
