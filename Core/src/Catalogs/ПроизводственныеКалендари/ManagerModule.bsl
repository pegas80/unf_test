...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		МодульУправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				...
				"ПроизводственныйКалендарь", НСтр("ka='საწარმოო კალენდარი';ru='Производственный календарь';en='Production calendar'"),
...
Функция ПечатнаяФормаПроизводственногоКалендаря(ПараметрыПодготовкиПечатнойФормы) Экспорт
	...
	Пока ВыборкаПоГоду.Следующий() Цикл
		...
		Пока ВыборкаПоКварталу.Следующий() Цикл
			...
			НомерКвартала.Параметры.НомерКвартала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 კვარტალი';ru='%1 квартал';en='% 1Q'"), ВыборкаПоКварталу.КварталКалендаря);
			...
			КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 კვარტალი';ru='%1 квартал';en='% 1Q'"), ВыборкаПоКварталу.КварталКалендаря);
			ТабличныйДокумент.Присоединить(КолонкаМесяца);
			...
			Если ВыборкаПоКварталу.КварталКалендаря = 2 
				...
				КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 ნახევარი წელი';ru='%1 полугодие';en='% H1'"), ВыборкаПоКварталу.КварталКалендаря / 2);
				ТабличныйДокумент.Присоединить(КолонкаМесяца);
			КонецЕсли;
		...
		КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წელი';ru='%1 год';en='%1 year'"), Формат(ВыборкаПоГоду.ГодКалендаря, "ЧГ="));
		ТабличныйДокумент.Присоединить(КолонкаМесяца);
	...
	КолонкаМесяца.Параметры.РабочееВремя36 	= РабочееВремя36Год;
	КолонкаМесяца.Параметры.РабочееВремя24 	= РабочееВремя24Год;
	КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 წელი';ru='%1 год';en='%1 year'"), Формат(НомерГода, "ЧГ="));
	...
	КолонкаМесяца.Параметры.ИмяМесяца 		= НСтр("ka='საშუალო თვიური რაოდენობა';ru='Среднемесячное количество';en='average number'");
	ТабличныйДокумент.Присоединить(КолонкаМесяца);
