...
Процедура ПеренестиДень(Команда)
	...
	ПараметрыВыбораДаты.Заголовок = НСтр("ka='გადატანის თარიღის არჩევა';ru='Выбор даты переноса'");
	
	ТекстСообщения = НСтр("ka='აირჩიეთ თარიღი, რომელზეც განხორციელდება დღის გადატანა %1 (%2)';ru='Выберите дату, на которую будет осуществлен перенос дня %1 (%2)'");
...
Процедура ЗаполнитьПредставлениеПереносов(Форма)
	...
	Для Каждого КлючИЗначение Из Форма.ПереносыДней Цикл
		...
		Форма.СписокПереносов.Добавить(ДатаИсточник, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%3) გადატანილია %2 (%4)-ისთვის';ru='%1 (%3) перенесен на %2 (%4)'"), Формат(ДатаИсточник, "ДФ='д ММММ'"), Формат(ДатаПриемник, "ДФ='д ММММ'"), ВидДняИсточник, ВидДняПриемник));
	КонецЦикла;
