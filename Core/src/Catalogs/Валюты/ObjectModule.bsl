...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ვალუტის კურსის დაკავშირება შეიძლება მხოლოდ დამოუკიდებელი ვალუტის კურსთან.';ru='Курс валюты можно связать только с курсом независимой валюты.'"));
	КонецЕсли;
	...
	Если Не ЭтоНовый()
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ვალუტა არ შეიძლება იყოს დაქვემდებარებული, ვინაიდან იგი სხვა ვალუტებისთვის ძირითადს წარმოადგენს.';ru='Валюта не может быть подчиненной, так как она является основной для других валют.'"));
		Отказ = Истина;
	КонецЕсли;
