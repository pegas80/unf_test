...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ვალუტის კურსის დაკავშირება შეიძლება მხოლოდ დამოუკიდებელი ვალუტის კურსთან.';ru='Курс валюты можно связать только с курсом независимой валюты.';en='exchange rate can only be associated with the course of an independent currency.'"));
	КонецЕсли;
	...
	Если Не ЭтоНовый()
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ვალუტა არ შეიძლება იყოს დაქვემდებარებული, ვინაიდან იგი სხვა ვალუტებისთვის ძირითადს წარმოადგენს.';ru='Валюта не может быть подчиненной, так как она является основной для других валют.';en='The currency can not be slave, since it is the basis for other currencies.'"));
		Отказ = Истина;
	КонецЕсли;
