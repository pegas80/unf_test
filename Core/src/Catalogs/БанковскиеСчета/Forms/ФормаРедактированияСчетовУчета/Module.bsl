...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетУчета(Ссылка) Тогда
		Элементы.ГруппаСчетовУчета.Подсказка = НСтр("ka='ბაზაში არის მოძრაობები ამ საბანკო ანგარიშის მიხედვით! აღრიცხვის დათვლის შეცვლა აკრძალულია!';ru='В базе есть движения по этому банковскому счету! Изменение счета учета запрещено!'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ВладелецОрганизация Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ka='აღრიცხვის ანგარიშები რედაქტირდება მხოლოდ ორგანიზაციის საბანკო ანგარიშებისთვის!';ru='Счета учетов редактируются только для банковских счетов организаций!'"));
	КонецЕсли;
