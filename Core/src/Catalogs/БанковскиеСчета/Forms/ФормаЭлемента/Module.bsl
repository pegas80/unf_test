...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не УправлениеНебольшойФирмойКлиентСервер.НомерСчетаКорректен(Объект.НомерСчета, БИКБанка, ЯвляетсяБанкомРФ, ТекстОшибки) Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Не удалось записать банковский счет';ru='Не удалось записать банковский счет';en='Could not write the bank account'"),
...
Процедура НомерСчетаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если ВебКлиент Тогда
		...
		Если СтрДлина(Текст) > 20 Тогда
			...
			Сообщение.Текст = НСтр("ka='Введенное значение превышает допустимую длину номера счета 20 символов!';ru='Введенное значение превышает допустимую длину номера счета 20 символов!';en='The entered value exceeds the acceptable account number length of 20 characters!'");
			Сообщение.Сообщить();
...
Функция ПроверитьКорректностьНомераСчета(Номер, ВалютныйСчет = Ложь, ТекстОшибки = "")
	...
	Если НЕ ВалютныйСчет И СтрДлина(Номер) <> 20 Тогда
		...
		ТекстОшибки = НСтр("ka='შეიძლება, ანგარიშის ნომერი სრულად არ არის მითითებული';ru='Возможно номер счета указан не полностью';en='Possible account number is not fully'");
		Результат = Ложь;
...
Функция ПроверитьКорректностьБИК(БИК, ТекстОшибки = "")
	...
	Если СтрДлина(БИК) <> 9 Тогда
		...
		ТекстОшибки = НСтр("ka='აღნიშნული ბსკ-ით ბანკი ვერ მოიძებნა. შეიძლება, ბსკ არასრულად არის მითითებული.';ru='По указанному БИК банк не найден. Возможно БИК указан не полностью.';en='The specified BIC Bank is not found. Perhaps not fully BIC Set.'");
		
	ИначеЕсли НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(БИК) Тогда
		
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", " ") +
			НСтр("ka='ბანკის ბსკ-ის შემადგენლობაში მხოლოდ ციფრები უნდა იყოს';ru='В составе БИК банка должны быть только цифры';en='As part of the BIC should be only numbers'");
		
	ИначеЕсли НЕ Лев(БИК, 2) = "04" Тогда
		
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", " ") +
			НСтр("ka='ბანკის ბსკ-ის პირველი 2 ციფრი უნდა იყოს ""04""';ru='Первые 2 цифры БИК банка должны быть ""04""';en='The first 2 digits of the BIC bank should be ""04""'");
...
Функция НайтиБанки(ТекстДляПоиска, Поле, Валютный = Ложь)
	...
	Если ПустаяСтрока(ТекстДляПоиска) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ველი ""%1"" არაკორექტულად არის შევსებული.';ru='Поле ""%1"" заполнено не корректно.';en='The ""%1"" is not correctly filled.'"), 
	...
	Если СписокНайденныхБанков.Количество() = 0 Тогда
		...
		Если ОбластьПоиска = "Код" Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ბსკ ""%1""-ით ბანკების ცნობარში ბანკი ვერ მოიძებნა';ru='Банк с БИК ""%1"" не найден в справочнике банков';en='Bank with BIC ""%1"" was not found in the reference book of banks'"), ТекстДляПоиска);
		...
		ИначеЕсли ОбластьПоиска = "КоррСчет" Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ბანკი საკორესპ. ანგარიშ ""%1""-ით ბანკების ცნობარში ვერ მოიძებნა';ru='Банк с корр. счетом ""%1"" не найден в справочнике банков';en='A bank with a correspondent. account ""%1"" was not found in the reference book of banks'"), ТекстДляПоиска);
			
		КонецЕсли;
		
		// Сформируем варианты
		Кнопки	= Новый СписокЗначений;
		Кнопки.Добавить("Выбрать",     НСтр("ka='ცნობარიდან ამორჩევა';ru='Выбрать из справочника';en='Select from the directory'"));
		Кнопки.Добавить("Отменить",   НСтр("ka='შეყვანის გაუქმება';ru='Отменить ввод';en='Undo'"));
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,, "Выбрать", НСтр("ka='ბანკი ვერ მოიძებნა';ru='Банк не найден';en='The Bank can not be found'"));
		Возврат Неопределено;
...
Процедура ПроверитьВозможностьИзменений(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ka='ბაზაში არის ეკვაირინგის ტერმინალები, რომლებშიც არჩეულია მიმდინარე საბანკო ანგარიში. ანგარიშსწორებების ვალუტის შეცვლა აკრძალულია.';ru='В базе присутствуют эквайринговые терминалы, в которых выбран текущий банковский счет. Изменение валюты расчетов запрещено.';en='The database contains acquiring terminals in which current bank account is selected. Change Currency calculations prohibited.'");
