...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Оповещение,
		НСтр("ka='კალათაში დარჩა შეუნახავი საქონელი. გავაგრძელოთ ფორმის დახურვა?';ru='В корзине остались несохраненные товары. Продолжить закрытие формы?';en='In the basket were unsaved products. Continue closing form?'"),
		РежимДиалогаВопрос.ОКОтмена,
		0,
		КодВозвратаДиалога.ОК,
		НСтр("ka='საქონლის შერჩევა';ru='Подбор товаров';en='selection of goods'")
	);
...
Процедура ОтборИерархияПриАктивизацииСтроки(Элемент)
	...
	УстановитьОтборПоИерархии(ЭтотОбъект, ПоказыватьВсеПодчиненные);
...
Процедура ОтборИерархияНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	Если СтрокаИерархии = Неопределено
		Или СтрокаИерархии.Представление = НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'")
...
Процедура ОтборИерархияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если СтрокаИерархии = Неопределено Или СтрокаИерархии.Представление = НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПослеВыполненияФоновогоЗадания()
	...
	ПоказатьПредупреждение(,Нстр("ka='ნომენკლატურა. მონაცემთა ჩატვირთვა დასრულებულია.';ru='Номенклатура."
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(Прогресс) = Тип("Структура")
			...
			МассивСтрок.Добавить(НСтр("ka='გთხოვთ, მოიცადეთ...';ru='Пожалуйста, подождите...';en='Please wait...'"));
...
Процедура УстановитьФиксированныйОтборПоТипамНоменклатуры(ОграничениеТипов)
	...
	Элементы.ОтборТипЛицензия.Доступность = Ложь; ///+Lazare 21.09.2022
...
Процедура ЗаполнитьДеревоИерархии()
	...
	СтрокаНетГруппы.Представление= НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'");
	СтрокаНетГруппы.Картинка = -1;
	
	СтрокаНетГруппы = Дерево.Строки.Добавить();
	СтрокаНетГруппы.Значение	 = Справочники.Номенклатура.ПустаяСсылка();
	СтрокаНетГруппы.Представление= НСТР("ka='<არ არის ჯგუფი>';ru='<Нет группы>';en='<No Group>'");
	СтрокаНетГруппы.Картинка = -1;
...
Процедура УстановитьОтборЦенСервер(ЦенаОт, ЦенаДо)
	...
	Элементы.ЦеныОтбор.Заголовок = НСтр("ka='ფასები';ru='Цены';en='Prices'")+
		?(ЦенаОт<>0, НСтр("ru=' от ';ka='ვისგან'")+ЦенаОт,"")+
		?(ЦенаДо<>0, НСтр("ru=' до ';ka=' до '")+ЦенаДо,"")
		;	
...
Процедура УстановитьОтборПоТипуНоменклатуры()
	...
	КонецЕсли;
	 //+Lazare 21.09.2022
	Если ОтборТипЛицензия Тогда
		ОтборТипНоменклатуры.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Лицензия"));
	КонецЕсли;
	//-Lazare 21.09.2022
...
Процедура УстановитьЗаголовокОтборТипыНоменклатуры(ОтборТипНоменклатуры)
	...
	СтрокаТипов = НСтр("ka='ტიპი:';ru='Тип:';en='Type of:'");
...
Процедура УстановитьОтборПоИерархии(Форма, ПоказыватьВсеПодчиненные)
	...
	Если ЭтоОтборПоГруппе Тогда
		...
		//Паганини_
		Если ПоказыватьВсеПодчиненные Тогда
			Сравнение	= ВидСравненияКомпоновкиДанных.ВИерархии;
		КонецЕсли;
		
	ИначеЕсли Элементы.ОтборИерархия.ТекущиеДанные.Представление = НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'") Тогда
		
		Использование = Ложь;
		
	ИначеЕсли Элементы.ОтборИерархия.ТекущиеДанные.Представление = НСТР("ka='<არ არის ჯგუფი>';ru='<Нет группы>';en='<No Group>'") Тогда
...
Процедура ДобавитьНоменклатуруВКорзину(СтруктураВыбора)
	...
	ПоказатьОповещениеПользователя(НСтр("ka='საქონლის შერჩევა';ru='Подбор товаров';en='selection of goods'")
		,
		,
...
Процедура СоздатьНоменклатуру()
	...
	//+Lazare 21.09.2022
	Если ОтборТипЛицензия Тогда		
		ЗаполнениеТип.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Лицензия"));
	КонецЕсли;
	//-Lazare 21.09.2022
...
Процедура ПометитьНаУдалениеНоменклатуру()
	...
	ПоказатьОповещениеПользователя(
		СтрШаблон(НСтр("ka='წაშლის ნიშნული %1';ru='Пометка удаления %1';en='Tagging removing 1%'"), ?(НовоеЗначениеПометкиУдаления, НСтр("ka='დაყენებულია';ru='установлена';en='installed'"), НСтр("ka='მოხსნილია';ru='снята';en='withdrawn'"))),
...
Процедура УстановитьПометкуУдаленияГруппы(Команда)
	...
	ПоказатьОповещениеПользователя(
		СтрШаблон(НСтр("ka='წაშლის ნიშნული %1';ru='Пометка удаления %1';en='Tagging removing 1%'"), ?(ТекущаяСтрокаДерева.ПометкаУдаления, НСтр("ka='დაყენებულია';ru='установлена';en='installed'"), НСтр("ka='მოხსნილია';ru='снята';en='withdrawn'"))),
...
Процедура ОбновитьНадписьПодобраноТоваров() Экспорт
	...
	Если Корзина.Количество()=0 Тогда
		НадписьПодобраноТоваров = НСтр("ka='საქონელი გადაიტანეთ კალათაში';ru='перетащите товары в корзину';en='Drag Items to Cart'");
	Иначе	
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Описание = НСтр("ka='ქვესისტემა მონაცემთაჩატვირთვაგარეწყაროდან: შედეგის ჩატვირთვის სერვერული მეთოდის შესრულება';ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузки результата';en='Subsystem ZagruzkaDannyhIzVneshnegoIstochnika: Performing server boot method results'");
...
//Паганини ПФ_
&НаКлиенте
Процедура ПоказыватьВсеПодчиненныеПриИзменении(Элемент)
	УстановитьОтборПоИерархии(ЭтотОбъект, ПоказыватьВсеПодчиненные);
КонецПроцедуры
//_Паганини ПФ
