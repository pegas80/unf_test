...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Оповещение,
		НСтр("ka='კალათაში დარჩა შეუნახავი საქონელი. გავაგრძელოთ ფორმის დახურვა?';ru='В корзине остались несохраненные товары. Продолжить закрытие формы?'"),
		РежимДиалогаВопрос.ОКОтмена,
		0,
		КодВозвратаДиалога.ОК,
		НСтр("ka='საქონლის შერჩევა';ru='Подбор товаров'")
	);
...
Процедура ОтборИерархияНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	Если СтрокаИерархии = Неопределено
		Или СтрокаИерархии.Представление = НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>'")
...
Процедура ОтборИерархияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если СтрокаИерархии = Неопределено Или СтрокаИерархии.Представление = НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>'") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(Прогресс) = Тип("Структура")
			...
			МассивСтрок.Добавить(НСтр("ka='გთხოვთ, მოიცადეთ...';ru='Пожалуйста, подождите...'"));
...
Процедура ЗаполнитьДеревоИерархии()
	...
	СтрокаНетГруппы.Представление= НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>'");
	СтрокаНетГруппы.Картинка = -1;
	
	СтрокаНетГруппы = Дерево.Строки.Добавить();
	СтрокаНетГруппы.Значение	 = Справочники.Номенклатура.ПустаяСсылка();
	СтрокаНетГруппы.Представление= НСТР("ka='<არ არის ჯგუფი>';ru='<Нет группы>'");
	СтрокаНетГруппы.Картинка = -1;
...
Процедура УстановитьОтборЦенСервер(ЦенаОт, ЦенаДо)
	...
	Элементы.ЦеныОтбор.Заголовок = НСтр("ka='ფასები';ru='Цены'")+
...
Процедура УстановитьЗаголовокОтборТипыНоменклатуры(ОтборТипНоменклатуры)
	...
	СтрокаТипов = НСтр("ka='ტიპი:';ru='Тип:'");
...
Процедура УстановитьОтборПоИерархии(Форма, ПоказыватьВсеПодчиненные)
	...
	ИначеЕсли Элементы.ОтборИерархия.ТекущиеДанные.Представление = НСТР("ka='<ყველა ჯგუფი>';ru='<Все группы>'") Тогда
		
		Использование = Ложь;
		
	ИначеЕсли Элементы.ОтборИерархия.ТекущиеДанные.Представление = НСТР("ka='<არ არის ჯგუფი>';ru='<Нет группы>'") Тогда
...
Процедура ДобавитьНоменклатуруВКорзину(СтруктураВыбора)
	...
	ПоказатьОповещениеПользователя(НСтр("ka='საქონლის შერჩევა';ru='Подбор товаров'")
		,
		,
...
Процедура ПометитьНаУдалениеНоменклатуру()
	...
	ПоказатьОповещениеПользователя(
		СтрШаблон(НСтр("ka='წაშლის ნიშნული %1';ru='Пометка удаления %1'"), ?(НовоеЗначениеПометкиУдаления, НСтр("ka='დაყენებულია';ru='установлена'"), НСтр("ka='მოხსნილია';ru='снята'"))),
...
Процедура УстановитьПометкуУдаленияГруппы(Команда)
	...
	ПоказатьОповещениеПользователя(
		СтрШаблон(НСтр("ka='წაშლის ნიშნული %1';ru='Пометка удаления %1'"), ?(ТекущаяСтрокаДерева.ПометкаУдаления, НСтр("ka='დაყენებულია';ru='установлена'"), НСтр("ka='მოხსნილია';ru='снята'"))),
...
Процедура ОбновитьНадписьПодобраноТоваров() Экспорт
	...
	Если Корзина.Количество()=0 Тогда
		НадписьПодобраноТоваров = НСтр("ka='საქონელი გადაიტანეთ კალათაში';ru='перетащите товары в корзину'");
	Иначе	
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Описание = НСтр("ka='ქვესისტემა მონაცემთაჩატვირთვაგარეწყაროდან: შედეგის ჩატვირთვის სერვერული მეთოდის შესრულება';ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузки результата'");
