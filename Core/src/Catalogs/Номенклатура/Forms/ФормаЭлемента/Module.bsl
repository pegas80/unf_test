...
Процедура СпецификацияНачалоВыбора(Элемент,  ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Сообщение.Текст = НСтр("ka='ცნობარის ელემენტი ჯერ არ ჩაწერილა.';ru='Элемент справочника еще не записан.'");
		Сообщение.Сообщить();
...
Процедура ИзменитьИзображение(Команда)
	...
	Если ДанныеФайлаДляИзменения = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='გამოსახულება რედაქტირებისთვის მიუწვდომელია';ru='Изображение не доступно для редактирования'");
...
Процедура СозданиеЦеныПоказатьВопрос()
	...
	ТекстВопроса = НСтр("ka='ფასის დაყენებამდე საჭიროა ობიექტის ჩაწერა. ჩავწეროთ?';ru='Перед установлением цены необходимо записать объект. Записать?'");
	Ответ = Неопределено;
...
Процедура ЗаполнитьДиаграммуПродажи(Данные)
	...
	Иначе
		...
		Если Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас
			...
			СерияСебестоимость.Текст = НСТР("ka='თვითღირებულება';ru='Себестоимость'");//Паганини
			СерияСебестоимость.Цвет = ЦветаСерий[0];
		КонецЕсли;
		
		СерияПрибыль = ДиаграммаПродажи.Серии.Добавить("Валовая прибыль");
		СерияПрибыль.Текст = НСТР("ka='მთლიანი მოგება';ru='Валовая прибыль'");//Паганини
		...
		СерияКоличество.Текст = НСТР("ka='რაოდენობა';ru='Количество'");//Паганини
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ВаловаяПрибыль < 0 Тогда
				...
				Если НЕ ПоказыватьУбыток Тогда
					...
					СерияУбыток.Текст = НСТР("ka='დანაკარგი';ru='Убыток'");//Паганини
...
Процедура ИнициализироватьДиаграммуЦеныПустую()
	...
	Если ЗначениеЗаполнено(ДиаграммаЦеныПериодЗаголовок) Тогда
		ДиаграммаЗаголовок = НСтр("ka='არჩეული პერიოდის მონაცემები არ არის.';ru='Нет данных за выбранный период.'");
	Иначе
		ДиаграммаЗаголовок = НСтр("ka='ფასები არ არის დაყენებული.';ru='Цены не установлены.'");
	КонецЕсли;
...
Процедура ИнициализироватьДиаграммуПродажиПустую()
	...
	Если ЗначениеЗаполнено(ДиаграммаПродажиПериодЗаголовок) Тогда
		ДиаграммаЗаголовок = НСтр("ka='არჩეული პერიოდის მონაცემები არ არის.';ru='Нет данных за выбранный период.'");
	Иначе
		ДиаграммаЗаголовок = НСтр("ka='გაყიდვების მონაცემები არ არის.';ru='Отсутствуют данные по продажам.'");
	КонецЕсли;
