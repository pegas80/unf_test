...
Процедура ПроверитьНастройки(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='გამართვების შესამოწმებლად აუცილებელია სააღრიცხვო ჩანაწერის მონაცემთა ჩაწერა. გავაგრძელოთ?';ru='Для проверки настроек необходимо записать данные учетной записи. Продолжить?';en='To check the settings, you need to record your account details. Proceed?'");
...
Процедура ПроверитьВозможностьСинхронизацииСОблачнымСервисом()
	...
	Состояние(НСтр("ka='სააღრიცხვო ჩანაწერის შემოწმება';ru='Проверка учетной записи';en='Checking account'"),,НСтр("ka='ხორციელდება სააღრიცხვო ჩანაწერის შემოწმება. მოიცადეთ...';ru='Выполняется проверка учетной записи. Подождите...';en='Account verification is performed. Wait...'"));
...
"%1'"),
			...
			НСтр("ka='სააღრიცხვო ჩანაწერის შემოწმება';ru='Проверка учетной записи';en='Checking account'"));
	Иначе
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
