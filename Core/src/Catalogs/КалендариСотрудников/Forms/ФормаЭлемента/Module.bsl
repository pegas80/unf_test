	...
	Если ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			...
			Если Не ПравоПросмотра Тогда
				ВызватьИсключение НСтр("ka='კალენდრის სანახავად არასაკმარისი უფლებებია.';ru='Недостаточно прав для просмотра календаря.';en='Not allowed to view the calendar.'");
			КонецЕсли;
...
Процедура ВладелецКалендаряПриИзменении(Элемент)
	...
	Если СотрудникиПользователя.НайтиПоЗначению(Объект.ВладелецКалендаря) = Неопределено Тогда
		...
		Если Не ЕстьДоступ Тогда
			...
			ПараметрыВопроса.Заголовок = НСтр("ka='წვდომის შეცვლა';ru='Изменение доступа';en='Changing access'");
			ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
			
			ТекстВопроса = СтрШаблон(НСтр("ru='Текущий сотрудник %1 не имеет доступа к календарю."
"Разрешить доступ?';ka='მიმდინარე თანამშრომელ %1-ს კალენდართან დაშვება არ გააჩნია. დავუშვათ კალენდართან?'"), Строка(СотрудникиПользователя));
