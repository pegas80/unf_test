...
Процедура СписокРесурсовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Сообщение.Текст = НСтр("ka='მონაცემები ჯერ არ ჩაწერილა';ru='Данные еще не записаны.'");
		Сообщение.Сообщить();
	Иначе
...
Процедура СписокРесурсовПередУдалением(Элемент, Отказ)
	...
	Если Объект.Ссылка = ПредопределенноеЗначение("Справочник.ВидыРесурсовПредприятия.ВсеРесурсы") Тогда
		ТекстСообщения = НСтр("ka='ობიექტი არ არის წაშლილი, ვინაიდან საწარმოს რესურსი უნდა შედიოდეს ტიპში ""ყველა რესურსი"".';ru='Объект не удален, т. к. ресурс предприятия должен входить в вид ""Все ресурсы"".'");
