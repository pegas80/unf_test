...
Процедура ВыполнитьДействияПоИсточнику(Действия, Источник, Правило, Автор) Экспорт
	...
	Пока ВыборкаДействий.Следующий() Цикл
		...
		Если МетаданныеПриемника.Реквизиты.Найти("Комментарий") <> Неопределено Тогда
			ПриемникОбъект.Комментарий = "#" + НСтр("ka='შექმნილია ავტომატურად, სამუშაო პროცესის წესების შესაბამისად';ru='Создан автоматически согласно правилам рабочего процесса';en='Automatically created according to rules workflow'");
		КонецЕсли;
		...
		ИначеЕсли ВыборкаДействий.ТипДействия = Перечисления.ТипыДействийРабочегоПроцесса.ОтправитьEmail Тогда
			...
			Если Не РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
				...
				Исключение
					ТекстСообщения = СтрШаблон(НСтр("ka='ელექტრონული წერილის ავტომატურად გაგზავნა ვერ მოხერხდა, საჭიროა ""მოვლენა""-დან გაგზავნა."
"სამუშაო პროცესის მოქმედების შესრულებისას: ""%1"". შემდეგი მიზეზით: %2';ru='Не удалось автоматически отправить электронное письмо, требуется отправка из События. При выполнении действия рабочего процесса: ""%1""."
...
"Для разблокирования перейдите в Еще больше возможностей - Администрирование - Регламентные операции - Разблокировать работу с внешними ресурсами.';ka='Автоматическая отправка письма невозможна, т.к. в базе установлена блокировка работы с внешними ресурсами."
"Для разблокирования перейдите в Еще больше возможностей - Администрирование - Регламентные операции - Разблокировать работу с внешними ресурсами.'");
		...
		ИначеЕсли ВыборкаДействий.ТипДействия = Перечисления.ТипыДействийРабочегоПроцесса.ОтправитьSMS Тогда
			...
			Если Не РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
				...
				Исключение
					ТекстСообщения = СтрШаблон(НСтр("ka='SMS-ის ავტომატურად გაგზავნა ვერ მოხერხდა, საჭიროა ""მოვლენა""-დან გაგზავნა. სამუშაო პროცესის მოქმედების შესრულებისას: ""%1""."
"შემდეგი მიზეზით: %2';ru='Не удалось автоматически отправить SMS, требуется отправка из События. При выполнении действия рабочего процесса: ""%1""."
"По причине: %2'"), ВыборкаДействий.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
						Метаданные.Справочники.ПравилаРабочегоПроцесса, Правило, ТекстСообщения);
				КонецПопытки;
...
"Для разблокирования перейдите в Еще больше возможностей - Администрирование - Регламентные операции - Разблокировать работу с внешними ресурсами.';ka='Автоматическая отправка SMS невозможна, т.к. в базе установлена блокировка работы с внешними ресурсами."
"Для разблокирования перейдите в Еще больше возможностей - Администрирование - Регламентные операции - Разблокировать работу с внешними ресурсами.'");
				ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
					Метаданные.Справочники.ПравилаРабочегоПроцесса, Правило, ТекстСообщения);
			КонецЕсли;
		...
		Если Не ПриемникОбъект.ПроверитьЗаполнение() Тогда
			ТекстСообщения = СтрШаблон(НСтр("ka='სამუშაო პროცესის მოქმედის შესრულება ვერ მოხერხდა: ""%1"". შესაქმნელი ობიექტი არაკორექტულად არის შევსებული: %2';ru='Не удалось выполнить действие рабочего процесса: ""%1""."
"Некорретно заполнен создаваемый объект: %2'"), ВыборкаДействий.Ссылка, МетаданныеПриемника.Представление());
		...
		ТекстСообщения = СтрШаблон(НСтр("ka='სამუშაო პროცეს ""%1""-ის მოქმედება წარმატებით შესრულდა, შექმნილია ობიექტი: ""%2""';ru='Действие рабочего процесса ""%1"" выполнено успешно,"
