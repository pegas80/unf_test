...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаОкончания)
		И ДатаОкончания < ДатаНачала Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='დასრულების თარიღი არ შეიძლება დაწყების თარიღზე ნაკლები იყოს.';ru='Дата окончания не может быть меньше даты начала.'"),,,,Отказ);
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеОснования()
	...
	Если Объект.ВидОснования = "ЗаказНаряд" Тогда
		ПредставлениеОснования = НСтр("ka='შეკვეთა-განწესი';ru='Заказ-наряд'");
	КонецЕсли;
...
Процедура ОбновитьЭлементыНастроекЗаполнения()
	...
	Для Каждого ОписаниеПоля Из ОписаниеПолей Цикл
		...
		Если ОписаниеПоля.ТипЗначения.СодержитТип(Тип("Дата")) Тогда
			ВариантЗаполнения.СписокВыбора.Добавить("Смещение",		НСтр("ka='თარიღის გადაწევა';ru='смещение даты'"));
			ВариантЗаполнения.СписокВыбора.Добавить("Указанный",	НСтр("ka='მითითებული';ru='указанный'"));
		Иначе
			ВариантЗаполнения.СписокВыбора.Добавить("ИзОснования",	НСтр("ka='საფუძვლიდან';ru='из основания'"));
			ВариантЗаполнения.СписокВыбора.Добавить("Указанный",	НСтр("ka='მითითებული';ru='указанный'"));
		КонецЕсли;
...
Процедура ЗаполнитьСписокСтандартныхИнтервалов(СписокВыбора)
	...
	СписокВыбора.Добавить("Сразу",		НСтр("ka='წესის ამოქმედების თარიღი';ru='Дата срабатывания правила'"));
	СписокВыбора.Добавить("ЧерезЧас",	НСтр("ka='ერთი საათის შემდეგ';ru='Через час'"));
	СписокВыбора.Добавить("ЧерезДень",	НСтр("ka='დღეგამოშვებით';ru='Через день'"));
	СписокВыбора.Добавить("ЧерезНеделю",НСтр("ka='კვირის შემდეგ';ru='Через неделю'"));
	СписокВыбора.Добавить("ЧерезМесяц",	НСтр("ka='ერთი თვის შემდეგ';ru='Через месяц'"));
...
Функция АвтоматическоеНаименованиеДействия(Форма)
	...
	Шаблон = НСтр("ka='%1 საფუძვლის მიხედვით: %2';ru='%1 по основанию: %2'");
