...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаОкончания)
		И ДатаОкончания < ДатаНачала Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='დასრულების თარიღი არ შეიძლება დაწყების თარიღზე ნაკლები იყოს.';ru='Дата окончания не может быть меньше даты начала.';en='The end date can not be less than the start date.'"),,,,Отказ);
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеОснования()
	...
	Если Объект.ВидОснования = "ЗаказНаряд" Тогда
		ПредставлениеОснования = НСтр("ka='სერვისის დოკუმენტი';ru='Заказ-наряд';en='Work Order'");
	КонецЕсли;
...
Процедура ОбновитьЭлементыНастроекЗаполнения()
	...
	Для Каждого ОписаниеПоля Из ОписаниеПолей Цикл
		...
		Если ОписаниеПоля.ТипЗначения.СодержитТип(Тип("Дата")) Тогда
			ВариантЗаполнения.СписокВыбора.Добавить("Смещение",		НСтр("ka='თარიღის გადაწევა';ru='смещение даты';en='date biasing'"));
			ВариантЗаполнения.СписокВыбора.Добавить("Указанный",	НСтр("ka='მითითებული';ru='указанный';en='specified'"));
		Иначе
			ВариантЗаполнения.СписокВыбора.Добавить("ИзОснования",	НСтр("ka='საფუძვლიდან';ru='из основания';en='from the bottom'"));
			ВариантЗаполнения.СписокВыбора.Добавить("Указанный",	НСтр("ka='მითითებული';ru='указанный';en='specified'"));
		КонецЕсли;
...
Процедура ЗаполнитьСписокСтандартныхИнтервалов(СписокВыбора)
	...
	СписокВыбора.Добавить("Сразу",		НСтр("ka='წესის ამოქმედების თარიღი';ru='Дата срабатывания правила';en='rules of operation Date'"));
	СписокВыбора.Добавить("ЧерезЧас",	НСтр("ka='ერთი საათის შემდეგ';ru='Через час';en='in an hour'"));
	СписокВыбора.Добавить("ЧерезДень",	НСтр("ka='დღეგამოშვებით';ru='Через день';en='In one day'"));
	СписокВыбора.Добавить("ЧерезНеделю",НСтр("ka='კვირის შემდეგ';ru='Через неделю';en='In a week'"));
	СписокВыбора.Добавить("ЧерезМесяц",	НСтр("ka='ერთი თვის შემდეგ';ru='Через месяц';en='In a month'"));
...
Функция АвтоматическоеНаименованиеДействия(Форма)
	...
	Шаблон = НСтр("ka='%1 საფუძვლის მიხედვით: %2';ru='%1 по основанию: %2';en='1% of the base: 2%'");
