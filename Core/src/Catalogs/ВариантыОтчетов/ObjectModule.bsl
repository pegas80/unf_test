...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Наименование <> "" И ВариантыОтчетов.НаименованиеЗанято(Отчет, Ссылка, Наименование) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='""%1"" დაკავებულია, აუცილებელია სხვა სახელწოდების მითითება.';ru='""%1"" занято, необходимо указать другое наименование.'"), Наименование),
			,
			"Наименование");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ Пользовательский И ПользователемИзмененаПометкаУдаления Тогда
		Если ПометкаУдаления Тогда
			ТекстОшибки = НСтр("ka='ანგარიშგების წინასწარ განსაზღვრული ვარიანტის წასაშლელად მონიშვნა აკრძალულია.';ru='Пометка на удаление предопределенного варианта отчета запрещена.'");
		Иначе
			ТекстОшибки = НСтр("ka='ანგარიშგების წინასწარ განსაზღვრული ვარიანტისთვის წაშლის ნიშნულის მოხსნა აკრძალულია.';ru='Снятие пометки удаления предопределенного варианта отчета запрещена.'");
		КонецЕсли;
	...
	Если НЕ ПометкаУдаления И ПользователемИзмененаПометкаУдаления Тогда
		...
		Если НаименованиеЗанято ИЛИ КлючВариантаЗанят Тогда
			ТекстОшибки = НСтр("ka='ანგარიშგების ვარიანტის წაშლის ნიშნულის მოხსნის შეცდომა:';ru='Ошибка снятия пометки удаления варианта отчета:'");
			Если НаименованиеЗанято Тогда
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='სახელწოდება ""%1"" უკვე დაკავებულია ამ ანგარიშგების სხვა ვარიანტით.';ru='Наименование ""%1"" уже занято другим вариантом этого отчета.'"),
					Наименование);
			Иначе
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ვარიანტ ""%1""-ის გასაღები უკვე დაკავებულია ამ ანგარიშგების სხვა ვარიანტის მიერ.';ru='Ключ варианта ""%1"" уже занят другим вариантом этого отчета.'"),
					КлючВарианта);
			КонецЕсли;
...
Процедура ПроиндексироватьНастройки()
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგება ""%2""-ის ვარიანტ ""%1""-ის სქემის ინდექსირება ვერ მოხერხდა:';ru='Не удалось проиндексировать схему варианта ""%1"" отчета ""%2"":'"),
			КлючВарианта,
			Строка(Отчет));
...
Процедура ПроверитьЗаполнениеПредопределенного(Отказ)
	...
	ИначеЕсли ТипОтчета <> ВариантыОтчетовКлиентСервер.ТипОтчета(Отчет) Тогда
		ТекстОшибки = НСтр("ka='ველ ""%1""-ის და ""%2""-ის წინააღმდეგობრივი მნიშვნელობები';ru='Противоречивые значения полей ""%1"" и ""%2""'");
...
Функция НеЗаполненоПоле(ИмяПоля)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი ""%1"" არ არის შევსებული';ru='Не заполнено поле ""%1""'"), ИмяПоля);
КонецФункции
