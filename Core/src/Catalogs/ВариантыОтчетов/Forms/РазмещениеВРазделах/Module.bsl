...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СмешаннаяВажность = НСтр("ka='განსხვავებული';ru='Различная'");
...
Процедура Разместить(Команда)
	Если ПроверитьКоличествоВариантов() Тогда
		ЗаписатьНаСервере();
		ТекстОповещения = НСтр("ka='შეცვლილია ანგარიშგებების ვარიანტების გამართვები (%1 ც.).';ru='Изменены настройки вариантов отчетов (%1 шт.).'");
...
Функция ПроверитьКоличествоВариантов()
	...
	Если КоличествоВариантов = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აუცილებელია სია ""ანგარიშგების ვარიანტების"" შევსება';ru='Необходимо заполнить список ""Варианты отчетов""'"),
			,
			"ИзменяемыеВарианты");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		ТекстСообщения = НСтр("ka='აუცილებელია ანგარიშგებების ვარიანტების არჩევა';ru='Необходимо выбрать варианты отчетов'");
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='წასაშლელად მონიშნულები';ru='Помеченные на удаление:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='შესაცვლელად არასაკმარისი უფლებებია:';ru='Недостаточно прав для изменения:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='ანგარიშგება გამორთულია ან უფლებების მიხედვით მიუწვდომელია:';ru='Отчет отключен или недоступен по правам:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='ანგარიშგების ვარიანტი გამორთულია ფუნქციური ოპციით:';ru='Вариант отчета отключен по функциональной опции:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			РезультатВыполнения.Текст = НСтр("ka='ანგარიშგებების არჩეული ვარიანტების ნაწილებში განსათავსებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для размещения в разделах выбранных вариантов отчетов.'");
		...
		Иначе
			РезультатВыполнения.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგებების ზოგიერთი ვარიანტის ნაწილებში განსათავსებლად უფლებები არასაკმარისია (%1).';ru='Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).'"),
