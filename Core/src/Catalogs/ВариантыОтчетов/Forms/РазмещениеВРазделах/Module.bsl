...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СмешаннаяВажность = НСтр("ka='განსხვავებული';ru='Различная';en='different'");
...
Процедура Разместить(Команда)
	Если ПроверитьКоличествоВариантов() Тогда
		ЗаписатьНаСервере();
		ТекстОповещения = НСтр("ka='შეცვლილია ანგარიშგებების ვარიანტების გამართვები (%1 ც.).';ru='Изменены настройки вариантов отчетов (%1 шт.).';en='Reports of settings change (% 1 pc.).'");
...
Функция ПроверитьКоличествоВариантов()
	...
	Если КоличествоВариантов = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აუცილებელია სია ""ანგარიშგების ვარიანტების"" შევსება';ru='Необходимо заполнить список ""Варианты отчетов""';en='It is necessary to fill in the list ""Options report""'"),
			,
			"ИзменяемыеВарианты");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		ТекстСообщения = НСтр("ka='აუცილებელია ანგარიშგებების ვარიანტების არჩევა';ru='Необходимо выбрать варианты отчетов';en='You must select options for records'");
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='წასაშლელად მონიშნულები';ru='Помеченные на удаление:';en='Marked for deletion:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='შესაცვლელად არასაკმარისი უფლებებია:';ru='Недостаточно прав для изменения:';en='Not allowed to edit:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='ანგარიშგება გამორთულია ან უფლებების მიხედვით მიუწვდომელია:';ru='Отчет отключен или недоступен по правам:';en='The report is disabled or not available on the Rights:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ka='ანგარიშგების ვარიანტი გამორთულია ფუნქციური ოპციით:';ru='Вариант отчета отключен по функциональной опции:';en='Report option is disabled by functional options:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			РезультатВыполнения.Текст = НСтр("ka='ანგარიშგებების არჩეული ვარიანტების ნაწილებში განსათავსებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для размещения в разделах выбранных вариантов отчетов.';en='Not have permission to be placed in selected sections of the report options.'");
		...
		Иначе
			РезультатВыполнения.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგებების ზოგიერთი ვარიანტის ნაწილებში განსათავსებლად უფლებები არასაკმარისია (%1).';ru='Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).';en='Not have permission to be placed in some sections of the report options (% 1).'"),
