...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("ka='ყველა, გარეს გარდა';ru='Все, кроме внешних'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Внутренний,     НСтр("ka='შიდა';ru='Внутренние'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Расширение,     НСтр("ka='გაფართოება';ru='Расширения'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Дополнительный, НСтр("ka='დამატებითები';ru='Дополнительные'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Внешний,        НСтр("ka='გარე';ru='Внешние'"));
...
Процедура ДеревоПодсистемПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ПараметрыРазмещения.Источник.Ссылка = ПараметрыРазмещения.Приемник.Ссылка Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგებების არჩეული ვარიანტები უკვე მოცემულ განყოფილებაშია.';ru='Выбранные варианты отчетов уже в данном разделе.'"));
		Возврат;
	КонецЕсли;
	
	Если ПараметрыРазмещения.Варианты.Всего = 1 Тогда
		Если ПараметрыРазмещения.Действие = "Копирование" Тогда
			ШаблонВопроса = НСтр("ka='%1 განვათავსოთ ""%4""-ში?';ru='Разместить ""%1"" в ""%4""?'");
		Иначе
			ШаблонВопроса = НСтр("ka='გადავაადგილოთ ""%1""  ""%3""-დან ""%4""-ში?';ru='Переместить ""%1"" из ""%3"" в ""%4""?'");
		КонецЕсли;
	...
	Иначе
		...
		КонецЦикла;
		Если ПараметрыРазмещения.Действие = "Копирование" Тогда
			ШаблонВопроса = НСтр("ka='განვათავსოთ ანგარიშგების ვარიანტები ""%1"" (%2 ც.) ""%4""-ში ?';ru='Разместить варианты отчетов ""%1"" (%2 шт.) в ""%4""?'");
		Иначе
			ШаблонВопроса = НСтр("ka='ანგარიშგებების ვარიანტები ""%1"" (%2 ც.) გადავიტანოთ ""%3""-დან ""%4""-ში?';ru='Переместить варианты отчетов ""%1"" (%2 шт.) из ""%3"" в ""%4""?'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыРазмещения.Варианты.Всего = РезультатВыполнения.Размещено Тогда
		Если ПараметрыРазмещения.Варианты.Всего = 1 Тогда
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("ka='წარმატებით გადაადგილდა ""%1""-ში.';ru='Успешно перемещен в ""%1"".'");
			Иначе
				Шаблон = НСтр("ka='წარმატებით განთავსდა ""%1""-ში.';ru='Успешно размещен в ""%1"".'");
			КонецЕсли;
			Текст = ПараметрыРазмещения.Варианты.Представление;
			Ссылка = ПолучитьНавигационнуюСсылку(ПараметрыРазмещения.Варианты.Массив[0]);
		Иначе
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("ka='წარმატებით გადაადგილდა ""%1""-ში';ru='Успешно перемещены в ""%1"".'");
			Иначе
				Шаблон = НСтр("ka='წარმატებით განთავსდა ""%1""-ში.';ru='Успешно размещены в ""%1"".'");
			КонецЕсли;
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგებების ვარიანტები (%1).';ru='Варианты отчетов (%1).'"), Формат(ПараметрыРазмещения.Варианты.Всего, "ЧН=0; ЧГ=0"));
			Ссылка = Неопределено;
		КонецЕсли;
	...
	Иначе
		...
		Если Не ПустаяСтрока(РезультатВыполнения.НеМогутРазмещаться) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("ka='საბრძანებო ინტერფეისში ვერ განთავსდება:';ru='Не могут размещаться в командном интерфейсе:'")
				+ Символы.ПС
				+ РезультатВыполнения.НеМогутРазмещаться;
		КонецЕсли;
		Если Не ПустаяСтрока(РезультатВыполнения.УжеРазмещены) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("ka='უკვე განთავსებულია ამ ნაწილში:';ru='Уже размещены в этом разделе:'")
				+ Символы.ПС
				+ РезультатВыполнения.УжеРазмещены;
		КонецЕсли;
		
		Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
			Шаблон = НСтр("ka='გადაადგილებულია ანგარიშგებების ვარიანტი: %2-იდან %1.';ru='Перемещено вариантов отчетов: %1 из %2.'");
		Иначе
			Шаблон = НСтр("ka='განთავსებულია ანგარიშგებების ვარიანტები: %2-დან %1.';ru='Размещено вариантов отчетов: %1 из %2.'");
		КонецЕсли;
		...
		Для Каждого ВариантСсылка Из ПараметрыРазмещения.Варианты.Массив Цикл
			...
			ИначеЕсли ВариантСсылка.ПометкаУдаления Тогда
				НеМогутРазмещаться = ?(НеМогутРазмещаться = "", "", НеМогутРазмещаться + Символы.ПС)
					...
					+ Строка(ВариантСсылка)
					+ " ("
					+ НСтр("ka='მონიშნულია წასაშლელად';ru='помечен на удаление'")
					+ ")";
				Продолжить;
			КонецЕсли;
...
Функция НачатьЗамер(Событие)
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			...
			+ "; " + НСтр("ka='დაქვემდებარებულების ჩართვით:';ru='Включая подчиненные:'") + " " + Строка(ВключаяПодчиненные);
	Иначе
