...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если Не ЗначениеЗаполнено(СсылкаВарианта) Тогда
			ВызватьИсключение НСтр("ka='ანგარიშგების ახალი ვარიანტის შექმნა შეიძლება მხოლოდ ანგარიშგების ფორმიდან';ru='Новый вариант отчета можно создать только из формы отчета';en='The new version of the report can only be created from the report form'");
		КонецЕсли;
		...
		ИначеЕсли ПараметрыОткрытия.ТипОтчета = "Дополнительный" Тогда
			...
			Если Не ПараметрыОткрытия.Подключен Тогда
				ВызватьИсключение НСтр("ka='გარე ანგარიშგების ვარიანტის გახსნა შეიძლება მხოლოდ ანგარიშგების ფორმიდან.';ru='Вариант внешнего отчета можно открыть только из формы отчета.';en='Optionally external report can only be opened from the report form.'");
			КонецЕсли;
		Иначе
			ВызватьИсключение НСтр("ka='გარე ანგარიშგების ვარიანტის გახსნა შეიძლება მხოლოდ ანგარიშგების ფორმიდან.';ru='Вариант внешнего отчета можно открыть только из формы отчета.';en='Optionally external report can only be opened from the report form.'");
		КонецЕсли;
