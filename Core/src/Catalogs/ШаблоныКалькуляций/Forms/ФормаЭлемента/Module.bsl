...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	Элементы.СтраницаЗапасы.Заголовок = НСтр("ka='მარაგები';ru='Запасы'")+?(Объект.Запасы.Количество()=0, "", " ("+Объект.Запасы.Количество()+")");	
	Элементы.СтраницаРасходы.Заголовок = НСтр("ka='დანახარჯები';ru='Расходы'")+?(Объект.Расходы.Количество()=0, "", " ("+Объект.Расходы.Количество()+")");	
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанныхЗапасы.Вставить("Заголовок", НСтр("ka='მარაგების ჩატვირთვა ფაილიდან';ru='Загрузка запасов из файла'"));
...
Процедура ЗагрузкаИзФайлаРасходы(Команда)
	...
	НастройкиЗагрузкиДанныхРасходы.Вставить("Заголовок", НСтр("ka='Загрузка расходов из файла';ru='Загрузка расходов из файла'"));
...
Процедура РасчитатьСебестоимостьНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Себестоимость=0 Тогда
			...
			НСтр("ka='სტრიქონში %1 ცხრილური ნაწილის ""%2"" ვერ მოხერხდა დანარიცხის თვითღირებულება დაანგარიშება';ru='В строке %1 табличной части ""%2"" не удалось рассчитать сумму себестоимости'"),
			Выборка.НомерСтроки,НСтр("ka='მარაგები';ru='Запасы'"));
	...
	Для каждого СтрРасходы Из Объект.Расходы Цикл
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სტრიქონში %1 ცხრილური ნაწილის ""%2"" ვერ მოხერხდა დანარიცხის თვითღირებულება დაანგარიშება';ru='В строке %1 табличной части ""%2"" не удалось рассчитать сумму себестоимости'"),
			СтрРасходы.НомерСтроки,НСтр("ka='დანახარჯები';ru='Расходы'"));
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Сообщить();
		КонецЕсли;
