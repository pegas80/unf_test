...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Объект.РасчетнаяСебестоимость = 0;// Kuzmich, 12.06.2019, Расчет себестоимости в шаблоне калькуляции
	Элемент.ТекущиеДанные.РасчетнаяСебестоимость = 0;// Kuzmich, 12.06.2019, Расчет себестоимости в шаблоне калькуляции
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	Элементы.СтраницаЗапасы.Заголовок = НСтр("ka='მარაგები';ru='Запасы';en='Stocks'")+?(Объект.Запасы.Количество()=0, "", " ("+Объект.Запасы.Количество()+")");	
	Элементы.СтраницаРасходы.Заголовок = НСтр("ka='დანახარჯები';ru='Расходы';en='Costs'")+?(Объект.Расходы.Количество()=0, "", " ("+Объект.Расходы.Количество()+")");	
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанныхЗапасы.Вставить("Заголовок", НСтр("ka='მარაგების ჩატვირთვა ფაილიდან';ru='Загрузка запасов из файла';en='Download the file from the reserves'"));
...
Процедура ЗагрузкаИзФайлаРасходы(Команда)
	...
	НастройкиЗагрузкиДанныхРасходы.Вставить("Заголовок", НСтр("ka='Загрузка расходов из файла';ru='Загрузка расходов из файла';en='Loading costs of file'"));
...
КонецПроцедуры
// Kuzmich, 12.06.2019, Расчет себестоимости в шаблоне калькуляции
&НаКлиенте
...
Процедура РасчитатьСебестоимостьНаСервере()
	...
	Объект.РасчетнаяСебестоимость = 0;
	
	Если НЕ ЗначениеЗаполнено(Объект.ДатаРасчетаСебестоимости) Тогда
	
		Объект.ДатаРасчетаСебестоимости = ТекущаяДата();
	...
	ЗапросТекст =
	"ВЫБРАТЬ
	|	ТЗ_Запасы.НомерСтроки,
	|	ТЗ_Запасы.Номенклатура,
	|	ТЗ_Запасы.Характеристика,
	|	ТЗ_Запасы.ЕдиницаИзмерения,
	|	ТЗ_Запасы.Количество
	|ПОМЕСТИТЬ ТЗ_Запасы
	|ИЗ
	...
	|				ТОГДА ЗапасыОстатки.СуммаОстаток * ТЗ_Запасы.Количество * ВЫРАЗИТЬ(ТЗ_Запасы.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения).Коэффициент / ЗапасыОстатки.КоличествоОстаток
	|			ИНАЧЕ ЗапасыОстатки.СуммаОстаток * ТЗ_Запасы.Количество / ЗапасыОстатки.КоличествоОстаток
	...
	|							ТЗ_Запасы.Характеристика
	|						ИЗ
	|							ТЗ_Запасы)) КАК ЗапасыОстатки
	|		ПО (ЗапасыОстатки.Номенклатура = ТЗ_Запасы.Номенклатура)
	|			И (ЗапасыОстатки.Характеристика = ТЗ_Запасы.Характеристика)
	|";
	Если ЗначениеЗаполнено(Объект.ОрганизацияСебестоимости) Тогда
		ЗапросТекст = ЗапросТекст +"ГДЕ
		|	ЗапасыОстатки.Организация = &Организация
		|";
		
		Запрос.УстановитьПараметр("Организация", Объект.ОрганизацияСебестоимости);
	КонецЕсли;
	
	Запрос.Текст = ЗапросТекст;
	
	Запрос.УстановитьПараметр("Период", Объект.ДатаРасчетаСебестоимости);
	Запрос.УстановитьПараметр("ТЗ_Запасы", Объект.Запасы.Выгрузить());
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		Объект.Запасы[Выборка.НомерСтроки-1].РасчетнаяСебестоимость = 0;
		
		Если Выборка.Себестоимость=0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სტრიქონში %1 ცხრილური ნაწილის ""%2"" ვერ მოხერხდა დანარიცხის თვითღირებულება დაანგარიშება';ru='В строке %1 табличной части ""%2"" не удалось рассчитать сумму себестоимости';en='Line% 1 of the table ""%2"" was unable to calculate the amount of the cost'"),
			Выборка.НомерСтроки,НСтр("ka='მარაგები';ru='Запасы';en='Stocks'"));
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Сообщить();
		Иначе
			Объект.РасчетнаяСебестоимость = Объект.РасчетнаяСебестоимость + Выборка.Себестоимость;
			Объект.Запасы[Выборка.НомерСтроки-1].РасчетнаяСебестоимость = Выборка.Себестоимость;
		КонецЕсли;
	...
	Для каждого СтрРасходы Из Объект.Расходы Цикл
		...
		Если СтрРасходы.СпособРасчета = Перечисления.СпособыРасчетаСуммыЗатрат.ФиксированнаяСумма Тогда
			Если ЗначениеЗаполнено(СтрРасходы.Валюта) Тогда
				Если СтрРасходы.Валюта = ВалютаУчета Тогда
					Объект.РасчетнаяСебестоимость = Объект.РасчетнаяСебестоимость + СтрРасходы.Значение;
				Иначе
					СтруктураКурса = РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрРасходы.Валюта, Объект.ДатаРасчетаСебестоимости);
					ТекЗначение = УправлениеНебольшойФирмойСервер.ПересчитатьИзВалютыВВалюту(СтрРасходы.Значение,СтруктураКурса.Курс,1,СтруктураКурса.Кратность,1);
					Объект.РасчетнаяСебестоимость = Объект.РасчетнаяСебестоимость + ТекЗначение;
				КонецЕсли;
			Иначе	
				Объект.РасчетнаяСебестоимость = Объект.РасчетнаяСебестоимость + СтрРасходы.Значение;
			КонецЕсли;
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სტრიქონში %1 ცხრილური ნაწილის ""%2"" ვერ მოხერხდა დანარიცხის თვითღირებულება დაანგარიშება';ru='В строке %1 табличной части ""%2"" не удалось рассчитать сумму себестоимости';en='Line% 1 of the table ""%2"" was unable to calculate the amount of the cost'"),
			СтрРасходы.НомерСтроки,НСтр("ka='დანახარჯები';ru='Расходы';en='Costs'"));
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Сообщить();
		КонецЕсли;
