...
Процедура ОформитьНаборыИСвойства()
	...
	ЭлементЦветаОформления.Значение = НСтр("ka='ნაკრებები';ru='Наборы'");
...
Процедура НастроитьОтображениеНаборов()
	...
	Если ЭтоНаборыДополнительныхСведений Тогда
		Заголовок = НСтр("ka='დამატებითი ცნობები';ru='Дополнительные сведения'");
		
		КомандаСоздать.Подсказка          = НСтр("ka='უნიკალური ინფორმაციის შექმნა';ru='Создать уникальное сведение'");
		КомандаСоздать.Заголовок          = НСтр("ka='ახალი';ru='Новое'");
		КомандаСоздать.Подсказка          = НСтр("ka='უნიკალური ინფორმაციის შექმნა';ru='Создать уникальное сведение'");
		
		КомандаСкопировать.Подсказка        = НСтр("ka='ახალი ინფორმაციის შექმნა მიმდინარეს კოპირებით';ru='Создать новое сведение копированием текущего'");
		КомандаИзменить.Подсказка           = НСтр("ka='მიმდინარე ინფორმაციის შეცვლა (ან გახსნა)';ru='Изменить (или открыть) текущее сведение'");
		КомандаПометитьНаУдаление.Подсказка = НСтр("ka='მიმდინარე ინფორმაციის წასაშლელად მონიშვნა (Del)';ru='Пометить текущее сведение на удаление (Del)'");
		КомандаПереместитьВверх.Подсказка   = НСтр("ka='მიმდინარე ინფორმაციის გადაადგილება ზემოთ';ru='Переместить текущее сведение вверх'");
		КомандаПереместитьВниз.Подсказка    = НСтр("ka='მიმდინარე ინფორმაციის გადაადგილება ქვემოთ';ru='Переместить текущее сведение вниз'");
		...
		Элементы.СвойстваТипЗначения.Подсказка =
			НСтр("ka='მნიშვნელობის ტიპები, რომლის შეყვანა შეიძლება ინფორმაციის შევსებისას.';ru='Типы значения, которое можно ввести при заполнении сведения.'");
		
		Элементы.СвойстваОбщиеЗначения.Подсказка =
			НСтр("ka='ინფორმაცია იყენებს ნიმუშ-ინფორმაციის მნიშვნელობების სიას.';ru='Сведение использует список значений сведения-образца.'");
		
		Элементы.СвойстваОбщее.Заголовок = НСтр("ka='საერთო';ru='Общее'");
...
"нескольких наборах дополнительных сведений.';ka='საერთო დამატებითი ინფორმაცია, რომელიც დამატებითი ინფორმაციის რამდენიმე კრებულში გამოიყენება.'");
	Иначе
		Заголовок = НСтр("ka='დამატებითი რეკვიზიტები';ru='Дополнительные реквизиты'");
		КомандаСоздать.Заголовок          = НСтр("ka='Новый';ru='Новый'");
		КомандаСоздать.Подсказка          = НСтр("ka='უნიკალური რეკვიზიტის შექმნა';ru='Создать уникальный реквизит'");
		
		КомандаСкопировать.Подсказка        = НСтр("ka='ახალი რეკვიზიტის შექმნა მიმდინარეს კოპირების გზით';ru='Создать новый реквизит копированием текущего'");
		КомандаИзменить.Подсказка           = НСтр("ka='მიმდინარე რეკვიზიტის შეცვლა (ან გახსნა)';ru='Изменить (или открыть) текущий реквизит'");
		КомандаПометитьНаУдаление.Подсказка = НСтр("ka='მიმდინარე რეკვიზიტის წასაშლელად მონიშვნა (Del)';ru='Пометить текущий реквизит на удаление (Del)'");
		КомандаПереместитьВверх.Подсказка   = НСтр("ka='მიმდინარე რეკვიზიტის გადაადგილება ზემოთ';ru='Переместить текущий реквизит вверх'");
		КомандаПереместитьВниз.Подсказка    = НСтр("ka='მიმდინარე რეკვიზიტის გადაადგილება ქვემოთ';ru='Переместить текущий реквизит вниз'");
		...
		Элементы.СвойстваТипЗначения.Подсказка =
			НСтр("ka='მნიშვნელობის ტიპები, რომლის შეყვანა შეიძლება რეკვიზიტის შევსებისას.';ru='Типы значения, которое можно ввести при заполнении реквизита.'");
		
		Элементы.СвойстваОбщиеЗначения.Подсказка =
			НСтр("ka='რეკვიზიტი იყენებს ნიმუში რეკვიზიტის მნიშვნელობების სიას.';ru='Реквизит использует список значений реквизита-образца.'");
		
		Элементы.СвойстваОбщее.Заголовок = НСтр("ka='საერთო';ru='Общий'");
	...
	Если Элементы.Свойства.ТекущиеДанные <> Неопределено Тогда
		...
		Если ЭтоНаборыДополнительныхСведений Тогда
			Если Элементы.Свойства.ТекущиеДанные.Общее Тогда
				ТекстВопроса = НСтр("ka='მიმდინარე საერთო ინფორმაცია გავრიცხოთ კრებულიდან?';ru='Исключить текущее общее сведения из набора?'");
				
			ИначеЕсли Элементы.Свойства.ТекущиеДанные.ПометкаУдаления Тогда
				ТекстВопроса = НСтр("ka='მიმდინარე ინფორმაციას მოვხსნათ წაშლის ნიშნული?';ru='Снять с текущего сведения пометку на удаление?'");
			Иначе
				ТекстВопроса = НСтр("ka='მიმდინარე ინფორმაცია მოვნიშნოთ წასაშლელად?';ru='Пометить текущее сведение на удаление?'");
			КонецЕсли;
		Иначе
			Если Элементы.Свойства.ТекущиеДанные.Общее Тогда
				ТекстВопроса = НСтр("ka='მიმდინარე საერთო რეკვიზიტი გავრიცხოთ კრებულიდან?';ru='Исключить текущий общий реквизит из набора?'");
				
			ИначеЕсли Элементы.Свойства.ТекущиеДанные.ПометкаУдаления Тогда
				ТекстВопроса = НСтр("ka='მიმდინარე რეკვიზიტს მოვხსნათ წაშლის ნიშნული?';ru='Снять с текущего реквизита пометку на удаление?'");
			Иначе
				ТекстВопроса = НСтр("ka='მიმდინარე რეკვიზიტი მოვნიშნოთ წასაშლელად?';ru='Пометить текущий реквизит на удаление?'");
			КонецЕсли;
		КонецЕсли;
