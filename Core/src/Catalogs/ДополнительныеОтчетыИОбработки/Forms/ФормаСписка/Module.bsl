...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВсеПубликацииКромеНеиспользующихся.Количество() > 1 Тогда
		...
		ПредставлениеМассива = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ან %2';ru='%1 или %2';en='%1 or 2'"),
	...
	СписокВыбора.Добавить(1, НСтр("ka='მხოლოდ ანგარიშგებები';ru='Только отчеты';en='reports only'"));
	СписокВыбора.Добавить(2, НСтр("ka='მხოლოდ დამუშავებები';ru='Только обработки';en='process only'"));
...
Функция ВыбранЭлемент(ДанныеСтроки)
	Если ТипЗнч(ДанныеСтроки.Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ПоказатьПредупреждение(, НСтр("ka='ბრძანება არ შეიძლება შესრულდეს აღნიშნული ობიექტისთვის. აირჩიეთ დამატებითი ანგარიშგება ან დამუშავება.';ru='Команда не может быть выполнена для указанного объекта."
"Выберите дополнительный отчет или обработку.'"));
		Возврат Ложь;
	КонецЕсли;
	Если ДанныеСтроки.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ka='ბრძანება არ შეიძლება, რომ ჯგუფისთვის შესრულდეს. აირჩიეთ დამატებითი ანგარიშგება ან დამუშავება.';ru='Команда не может быть выполнена для группы."
"Выберите дополнительный отчет или обработку.'"));
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	Если КоличествоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='არცერთი დამატებითი ანგარიშგება (დამუშავება) არ არის არჩეული';ru='Не выбран ни один дополнительный отчет (обработка)';en='Do not select any additional report (processing)'"));
		Возврат;
	КонецЕсли;
	...
	Если КоличествоСтрок = 1 Тогда
		ТекстСообщения = НСтр("ka='დამატებითი ანგარიშგების (დამუშავების) პუბლიკაცია შეცვლილია ""%1""';ru='Изменена публикация дополнительного отчета (обработки) ""%1""';en='Changed publication further reports (treatment) ""%1""'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка(ВыделенныеСтроки[0]));
	Иначе
		ТекстСообщения = НСтр("ka='Изменена публикация у дополнительных отчетов (обработок): %1';ru='Изменена публикация у дополнительных отчетов (обработок): %1';en='Changed publication in additional reports (treatments): 1%'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КоличествоСтрок);
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("ka='პუბლიკაცია შეცვლილია';ru='Изменена публикация';en='changed publication'"),, ТекстСообщения);
