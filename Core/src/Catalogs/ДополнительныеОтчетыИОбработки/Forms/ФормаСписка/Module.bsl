...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("ka='მხოლოდ ანგარიშგებები';ru='Только отчеты'"));
	СписокВыбора.Добавить(2, НСтр("ka='მხოლოდ დამუშავებები';ru='Только обработки'"));
...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	Если КоличествоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='არცერთი დამატებითი ანგარიშგება (დამუშავება) არ არის არჩეული';ru='Не выбран ни один дополнительный отчет (обработка)'"));
		Возврат;
	КонецЕсли;
	...
	Если КоличествоСтрок = 1 Тогда
		ТекстСообщения = НСтр("ka='დამატებითი ანგარიშგების (დამუშავების) პუბლიკაცია შეცვლილია ""%1""';ru='Изменена публикация дополнительного отчета (обработки) ""%1""'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка(ВыделенныеСтроки[0]));
	Иначе
		ТекстСообщения = НСтр("ka='Изменена публикация у дополнительных отчетов (обработок): %1';ru='Изменена публикация у дополнительных отчетов (обработок): %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КоличествоСтрок);
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("ka='პუბლიკაცია შეცვლილია';ru='Изменена публикация'"),, ТекстСообщения);
