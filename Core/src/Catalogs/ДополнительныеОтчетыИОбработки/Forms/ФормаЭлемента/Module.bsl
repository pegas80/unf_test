...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების დასამატებლად არასაკმარისი დაშვების უფლებებია.';ru='Недостаточно прав доступа для добавления дополнительных отчетов или обработок.'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების ჩასატვირთად არასაკმარისი უფლებებია';ru='Недостаточно прав для загрузки дополнительных отчетов и обработок'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების წინასწარ განსაზღვრული ვარიანტის წასაშლელად მონიშვნა აკრძალულია.';ru='Пометка на удаление предопределенного варианта отчета запрещена.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოვნიშნოთ ""%1"" წასაშლელად?';ru='Пометить ""%1"" на удаление?'"), Вариант.Наименование);
...
Процедура ВариантыДополнительногоОтчетаОткрыть(Команда)
	...
	Если Вариант = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ანგარიშგების ვარიანტი.';ru='Выберите вариант отчета.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='ბეჭდვის ბრძანებების ხილვადობის გასამართად, აუცილებელია დამუშავების ჩაწერა. გავაგრძელოთ?';ru='Для настройки видимости команд печати обработку необходимо записать. Продолжить?'");
...
Процедура ВыполнитьКоманду(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		ТекстВопроса = НСтр("ka='Для выполнения команды необходимо записать данные.';ru='Для выполнения команды необходимо записать данные.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ЗаписатьИПродолжить", НСтр("ka='ჩავიწეროთ და გავაგრძელოთ';ru='Записать и продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = НСтр("ka='ღია ფანჯრებში ცვლილებების გამოსაყენებლად აუცილებელია მათი დახურვა და ხელახლა გახსნა.';ru='Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.'");
...
Процедура ОбновлениеИзФайлаПослеПодтверждения(Ответ, ПараметрыРегистрации) Экспорт
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыДиалога.Заголовок = НСтр("ka='აირჩიეთ გარე ანგარიშგების ან დამუშავების ფაილი';ru='Выберите файл внешнего отчета или обработки'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыДиалога.ИндексФильтра = 1;
		ПараметрыДиалога.Заголовок = НСтр("ka='აირჩიეთ გარე ანგარიშგების ფაილი';ru='Выберите файл внешнего отчета'");
	Иначе
		ПараметрыДиалога.ИндексФильтра = 2;
		ПараметрыДиалога.Заголовок = НСтр("ka='აირჩიეთ გარე დამუშავების ფაილი';ru='Выберите файл внешней обработки'");
	КонецЕсли;
...
Процедура ОбновлениеИзФайлаПослеВыбораФайла(ПомещенныеФайлы, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ka='ფაილის გაფართოება არ შეესაბამება გარე ანგარიშგების (ERF) ან დამუშავების (EPF) გაფართოებას.';ru='Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).'");
...
Процедура ОбновлениеИзФайлаМеханикаНаКлиенте(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.Успех Тогда
		ОповещениеЗаголовок = ?(ПараметрыРегистрации.ЭтоОтчет, НСтр("ka='გარე ანგარიშგების ფაილი ჩატვირთულია';ru='Файл внешнего отчета загружен'"), НСтр("ka='გარე დამუშავების ფაილი ჩატვირთულია';ru='Файл внешней обработки загружен'"));
	...
	Иначе
		...
		Иначе
			...
			Если ПараметрыРегистрации.ЭтоОтчет Тогда
				ЗаголовокОшибки = НСтр("ka='ანგარიშგება არ არის შეერთებული';ru='Отчет не подключен'");
			Иначе
				ЗаголовокОшибки = НСтр("ka='დამუშავება არ არის შეერთებული';ru='Обработка не подключена'");
			КонецЕсли;
...
Процедура ОбновлениеИзФайлаПоказатьКонфликты(ПараметрыРегистрации)
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ka='კონფლიქტები გარე ანგარიშგების ჩატვირთვისას';ru='Конфликты при загрузке внешнего отчета'");
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих отчетов.'");
		Иначе
			ЗаголовокВопроса = НСтр("ka='კონფლიქტები გარე ანგარიშგების ჩატვირთვისას';ru='Конфликты при загрузке внешней обработки'");
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих обработок.'");
		КонецЕсли;
		ПредставлениеКнопкиОтключить = НСтр("ka='კონფლიქტურების გამორთვა';ru='Отключить конфликтующие'");
		ПредставлениеКнопкиОткрыть = НСтр("ka='გაუქმება და სიის ჩვენება';ru='Отменить и показать список'");
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ka='კონფლიქტი გარე ანგარიშგების ჩატვირთვისას';ru='Конфликт при загрузке внешнего отчета'");
			ТекстВопроса = НСтр("ka='ანგარიშგების შიდა სახელი ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующего отчета.'");
			ПредставлениеКнопкиОтключить = НСтр("ka='სხვა ანგარიშგების გამორთვა';ru='Отключить другой отчет'");
		Иначе
			ЗаголовокВопроса = НСтр("ka='კონფლიქტი გარე დამუშავების ჩატვირთვისას';ru='Конфликт при загрузке внешней обработки'");
			ТекстВопроса = НСтр("ka='დამუშავების შიდა სახელი ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующей обработки.'");
			ПредставлениеКнопкиОтключить = НСтр("ka='სხვა დამუშავების გამორთვა';ru='Отключить другую обработку'");
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("ka='გაუქმება და გახსნა';ru='Отменить и открыть'");
	КонецЕсли;
	ПредставлениеКнопкиПродолжить = НСтр("ka='გასწორების რეჟიმში';ru='В режиме отладки'");
...
Процедура ОбновлениеИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები შიდა სახელით ""%1""';ru='Дополнительные отчеты и обработки с внутреннем именем ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგების ვარიანტი ""%1"" არ არის რეგისტრირებული.';ru='Вариант отчета ""%1"" не зарегистрирован.'"), Вариант.Наименование);
...
Процедура ПослеЗавершенияВыполненияСервернойКомандыВФоне(Задание, ВыполняемаяКоманда) Экспорт
	Если Задание.Статус = "Ошибка" Тогда
		...
		ВводныйТекст = НСтр("ka='ბრძანების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить команду.'");
...
Функция НачатьВыполнениеСервернойКомандыВФоне(ВыполняемаяКоманда, УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები: დამუშავების სერვერული მეთოდის შესრულება';ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки'");
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ka='დამატებითი დამუშავების დანიშნულება';ru='Назначение дополнительной обработки'"));
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если Объект.Вид = ВидДополнительныйОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = НСтр("ka='ანგარიშგების ვარიანტები';ru='Варианты отчета'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგების ვარიანტები (%1).';ru='Варианты отчета (%1)'"),
	...
	Иначе
		...
		Элементы.СтраницаРазрешения.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ნებართვები (%1)';ru='Разрешения (%1)'"),
...
"профиля безопасности %1, в котором будут разрешены только следующие операции:'"),
	...
	Если ЭтоГлобальнаяОбработка Тогда
		ПредставлениеНазначения = НСтр("ka='<Не задано>';ru='<Не задано>'");
	...
	Иначе
		Если Объект.Назначение.Количество() = 0 Тогда
			ПредставлениеНазначения = НСтр("ka='გაურკვეველია';ru='Не определено'");
		Иначе
			ПредставлениеНазначения = "";
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("ka='დამატებითი ანგარიშგება (შექმნა)';ru='Дополнительный отчет (создание)'"), НСтр("ka='დამატებითი დამუშავება (შექმნა)';ru='Дополнительная обработка (создание)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, НСтр("ka='(დამატებითი ანგარიშგება)';ru='(Дополнительный отчет)'"), НСтр("ka='(დამატებითი დამუშავება)';ru='(Дополнительная обработка)'"));
	КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("ka='ცვლის ბეჭდვის სტანდარტულ ბრძანებას %1';ru='Заменяет стандартную команду печати %1'");
					Иначе
						ШаблонКомментария = НСтр("ka='ელემენტი ამობეჭდვის სტანდარტული გამართვები %1';ru='Заменяет стандартные команды печати: %1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაშვების ვარიანტ ""%1""-ის მქონე ბრანებებისთვის გამოუყენებელია';ru='Не применимо для команд с вариантом запуска ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = НСтр("ka='ანგარიშგების ვარიანტების სიის მიღება ვერ მოხერხდა ამ ანგარიშგების ჩართვისას წარმოშობილი მიზეზის გამო:';ru='Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:'");
...
Функция НазваниеСтраницыКоманд()
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		Возврат НСтр("ka='ბეჭდვის ბრძანებები';ru='Команды печати'");
	Иначе
		Возврат НСтр("ka='ბრძანებები';ru='Команды'");
	КонецЕсли;
КонецФункции
