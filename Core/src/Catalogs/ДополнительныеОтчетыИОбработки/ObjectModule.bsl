	...
	Если НЕ ДополнительныеОтчетыИОбработки.ПроверитьГлобальнаяОбработка(Вид) Тогда
		Если НЕ ИспользоватьДляФормыОбъекта И НЕ ИспользоватьДляФормыСписка 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='აუცილებელია პუბლიკაციის გამორთვა ან გამოსაყენებლად, სულ მცირე ერთ-ერთი ფორმის არჩევა';ru='Необходимо отключить публикацию или выбрать для использования как минимум одну из форм'")
				,
				,
				,
...
"Для продолжения необходимо изменить вид Публикации с ""%2"" на ""%3"" или ""%4"".'"),
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ანგარიშგება (დამუშავება) ""%2""-ის მიერ გამოყენებადი სახელი ""%1"" უკვე დაკავებული აქვს სხვა გამოქვეყნებულ დამატებით ანგარიშგებას (დამუშავებას).';ru='Имя ""%1"", используемое отчетом (обработкой) ""%2"", уже занято другим опубликованным дополнительным отчетом (обработкой).'"),
					ИмяОбъекта,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() И НЕ ДополнительныеОтчетыИОбработки.ПравоДобавления(ЭтотОбъект) Тогда
		ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების ან დამუშავებების დასამატებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для добавления дополнительных отчетов или обработок.'");
	КонецЕсли;
	
	// Предварительные проверки
	Если НЕ ЭтоНовый() И Вид <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Вид") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='არსებული დამატებითი ანგარიშგების ან დამუშავების იერის შეცვლა შეუძლებელია.';ru='Невозможно сменить вид существующего дополнительного отчета или обработки.'"),,,,
			Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписиОтчета(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТекстОшибки = НСтр("ka='შეერთების შეცდომა:';ru='Ошибка подключения:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
