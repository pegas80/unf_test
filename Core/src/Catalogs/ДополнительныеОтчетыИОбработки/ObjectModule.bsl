	...
	Если НЕ ДополнительныеОтчетыИОбработки.ПроверитьГлобальнаяОбработка(Вид) Тогда
		Если НЕ ИспользоватьДляФормыОбъекта И НЕ ИспользоватьДляФормыСписка 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='აუცილებელია პუბლიკაციის გამორთვა ან გამოსაყენებლად, სულ მცირე ერთ-ერთი ფორმის არჩევა';ru='Необходимо отключить публикацию или выбрать для использования как минимум одну из форм';en='You must disable publishing or choose to use at least one form'")
				,
				,
				,
		...
		Если Конфликтующие.Количество() > 0 Тогда
			...
			Если ПроверкаЭлемента Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='მოცემული ანგარიშგების (დამუშავების) მიერ გამოყენებული სახელი ""%1"" უკვე დაკავებული აქვს სხვა, გამოქვეყნებულ დამატებით ანგარიშგებას (დამუშავებას). "
""
"გასაგრძელებლად აუცილებელია პუბლიკაციის ტიპის შეცვლა ""%2""-დან ""%3""-ზე ან ""%4""-ზე.';ru='Имя ""%1"", используемое данным отчетом (обработкой), уже занято другим опубликованным дополнительным отчетом (обработкой). "
""
"Для продолжения необходимо изменить вид Публикации с ""%2"" на ""%3"" или ""%4"".'"),
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='ანგარიშგება (დამუშავება) ""%2""-ის მიერ გამოყენებადი სახელი ""%1"" უკვე დაკავებული აქვს სხვა გამოქვეყნებულ დამატებით ანგარიშგებას (დამუშავებას).';ru='Имя ""%1"", используемое отчетом (обработкой) ""%2"", уже занято другим опубликованным дополнительным отчетом (обработкой).';en='The name ""%1"" used the report (enhanced), ""%2"" is already taken by another published report additional (enhanced).'"),
					ИмяОбъекта,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() И НЕ ДополнительныеОтчетыИОбработки.ПравоДобавления(ЭтотОбъект) Тогда
		ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების ან დამუშავებების დასამატებლად არასაკმარისი უფლებებია.';ru='Недостаточно прав для добавления дополнительных отчетов или обработок.';en='Not have permission to add additional reports or treatments.'");
	КонецЕсли;
	
	// Предварительные проверки
	Если НЕ ЭтоНовый() И Вид <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Вид") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='არსებული დამატებითი ანგარიშგების ან დამუშავების იერის შეცვლა შეუძლებელია.';ru='Невозможно сменить вид существующего дополнительного отчета или обработки.';en='It is impossible to change the current view or report additional processing.'"),,,,
			Отказ);
		Возврат;
	КонецЕсли;
...
Функция ПредставлениеЗадания(Команда)
	...
	Возврат (
		...
		+ СокрЛП(Наименование)
		+ " / "
		+ НСтр("ka='ბრძანება';ru='Команда';en='Team'")
		+ ": "
...
Процедура ПриЗаписиОтчета(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТекстОшибки = НСтр("ka='შეერთების შეცდომა:';ru='Ошибка подключения:';en='Connection error:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
