	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокументов Цикл
		Если Не СтрокаТаблицы.ФайлыВыбраны И ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Добавьте файлы для документа ""%1""';ru='Добавьте файлы для документа ""%1""'"), СтрокаТаблицы.Документ),, 
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Заполните вид документа в строке %1';ru='Заполните вид документа в строке %1'"), ТаблицаДокументов.Индекс(СтрокаТаблицы) + 1),,
...
Функция ПредставлениеФайлов(Файлы)
	...
	Если ЗначениеЗаполнено(Файлы) Тогда
		...
		Если КоличествоСтраниц = 1 Тогда
			ШаблонПредставления = НСтр("ka='Страница № %1 (%2)';ru='Страница № %1 (%2)'");
		Иначе
			ШаблонПредставления = НСтр("ka='Страницы № %1 (%2)';ru='Страницы № %1 (%2)'");
		КонецЕсли;
...
Функция ПроверитьЗаполнениеПоляОтправитель()
	...
	Если Не ЗначениеЗаполнено(Объект.Отправитель) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ka='Заполните поле ""От кого""';ru='Заполните поле ""От кого""'");
	...
	Если ЗначениеЗаполнено(Объект.Отправитель) И Не ЗначениеЗаполнено(УчетнаяЗаписьОтправителя(Объект.Отправитель)) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ka='Организация не подключена к 1С-Отчетности';ru='Организация не подключена к 1С-Отчетности'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.Отправитель");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
...
Функция ПроверитьЗаполнениеПоляСотрудник()
	...
	Если Не ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Заполните поле ""Сотрудник""';ru='Заполните поле ""Сотрудник""'"),, 
			"Объект.Сотрудник");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
	...
	Если ПроверитьЗаполнениеПоляОтправитель() И ПроверитьЗаполнениеПоляПолучатель() Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Документ) Тогда
				ОчиститьСообщения();
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='ველი ""დოკუმენტი"" არ არის შევსებული';ru='Поле ""Документ"" не заполнено'"),, 
