...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТаблицы Из ЭлектронныеДокументы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Заполните вид документа в строке %1';ru='Заполните вид документа в строке %1';en='Fill in the form of a document at line% 1'"), ЭлектронныеДокументы.Индекс(СтрокаТаблицы) + 1),,
	...
	Если Не ЕстьРеестр И ЭлектронныеДокументы.Количество() <> 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Добавьте реестр направляемых документов';ru='Добавьте реестр направляемых документов';en='Add the registry of the document guides'"),,
			"РеестрДокументов",,
			Отказ);
	КонецЕсли;
	...
	Если СтрДлина(СокрЛП(СтрЗаменить(ОрганПФРПоМестуНазначенияПенсии, "-", ""))) <> 6 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Код органа ПФР по месту назначения пенсии должен состоят из 6 цифр';ru='Код органа ПФР по месту назначения пенсии должен состоят из 6 цифр';en='Authority Code of FIU destination pension should consist of 6 digits'"),
			Ссылка, 
	...
	Если Не ЗначениеЗаполнено(СокрЛП(СтрЗаменить(СтраховойНомерПФР, "-", ""))) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='შეავსეთ  СНИЛС';ru='Заполните СНИЛС';en='fill SNILS'"),, "СтраховойНомерПФР",, Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(АдресРегистрации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Заполните адрес регистрации';ru='Заполните адрес регистрации';en='Fill in the registration address'"),, "АдресРегистрации",, Отказ);
	КонецЕсли;
...
Процедура ЗаполнитьСодержание()
	...
	ШаблонПисьма = НСтр("ka='Направляем Вам макет пенсионного дела сотрудника:"
"%1 (%2)"
""
"Адрес регистрации: %3"
"Контактный телефон: %4';ru='Направляем Вам макет пенсионного дела сотрудника:"
"%1 (%2)"
""
"Адрес регистрации: %3"
