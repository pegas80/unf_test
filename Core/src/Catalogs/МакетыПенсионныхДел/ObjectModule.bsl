...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТаблицы Из ЭлектронныеДокументы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Заполните вид документа в строке %1';ru='Заполните вид документа в строке %1'"), ЭлектронныеДокументы.Индекс(СтрокаТаблицы) + 1),,
	...
	Если Не ЕстьРеестр И ЭлектронныеДокументы.Количество() <> 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Добавьте реестр направляемых документов';ru='Добавьте реестр направляемых документов'"),,
			"РеестрДокументов",,
			Отказ);
	КонецЕсли;
	...
	Если СтрДлина(СокрЛП(СтрЗаменить(ОрганПФРПоМестуНазначенияПенсии, "-", ""))) <> 6 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Код органа ПФР по месту назначения пенсии должен состоят из 6 цифр';ru='Код органа ПФР по месту назначения пенсии должен состоят из 6 цифр'"),
			Ссылка, 
	...
	Если Не ЗначениеЗаполнено(СокрЛП(СтрЗаменить(СтраховойНомерПФР, "-", ""))) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='შეავსეთ  СНИЛС';ru='Заполните СНИЛС'"),, "СтраховойНомерПФР",, Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(АдресРегистрации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Заполните адрес регистрации';ru='Заполните адрес регистрации'"),, "АдресРегистрации",, Отказ);
	КонецЕсли;
