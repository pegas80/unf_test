...
Функция ПолучитьНастройки(Организация, ОрганПФР) Экспорт
	...
	Если ЗначениеЗаполнено(Организация)
		...
		Если Организация.УчетнаяЗаписьОбмена.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.Такском Тогда
			ВызватьИсключение(НСтр("ka='Для абонентов оператора ""Такском"" в настоящее время не предусмотрена возможность отправки макетов пенсионных дел';ru='Для абонентов оператора ""Такском"" в настоящее время не предусмотрена возможность отправки макетов пенсионных дел';en='For subscribers of the operator ""Taxcom"" are not available at the moment the possibility of sending pension cases layouts'"));
		КонецЕсли;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='შემოწმების აქტი';ru='Акт сверки';en='Act of reconciliation'");
