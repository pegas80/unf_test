...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ТипСтруктурнойЕдиницы = ТипСтруктурнойЕдиницыСклад Тогда
	    Элементы.МобильныйСклад.Видимость 		= Истина;
		Элементы.ТранспортноеСредство.Видимость = Объект.МобильныйСклад;
	Иначе
	    Элементы.МобильныйСклад.Видимость 		= Ложь;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменилисьСчетаСтруктурныеЕдиницы" Тогда
		...
		Объект.СчетУчетаВРозницеРегл = Параметр.СчетУчетаВРозницеРегл;
		Объект.СчетУчетаНаценкиРегл = Параметр.СчетУчетаНаценкиРегл;
...
Процедура РозничныйВидЦенПриИзменении(Элемент)
	...
	Если НЕ СтруктураДанных.ВалютаЦены = СтруктураДанных.НациональнаяВалюта Тогда
		...
		ТекстСообщения = НСтр("ka='ფასების ტიპ ""%ВидЦен%""-ს, საცალო სტრუქტურული ერთეულისთვის უნდა მიეცეს ეროვნული ვალუტა (%НацВалюта%).';ru='У вида цен ""%ВидЦен%"", для розничной структурной единицы, должна быть задана национальная валюта (%НацВалюта%).';en='We kind of prices ""% VidTsen%"", for retail structural units of the national currency (% of national currency%) must be set.'");
...
//Паганини ПФ_
&НаКлиенте
Процедура МобильныйСкладПриИзменении(Элемент)
	Если Объект.ТипСтруктурнойЕдиницы = ТипСтруктурнойЕдиницыСклад Тогда
	    Элементы.МобильныйСклад.Видимость 		= Истина;
		Элементы.ТранспортноеСредство.Видимость = Объект.МобильныйСклад;
	Иначе
...
Процедура УстановитьВидимость()
	...
	АвтоматическоеСозданияПроизводстваПриНедостачиКассовойСмены	= УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("АвтоматическоеСозданияПроизводстваПриНедостачиКассовойСмены");
	
	Элементы.СкладПроизводства.Видимость = АвтоматическоеСозданияПроизводстваПриНедостачиКассовойСмены;
