...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Получатели рассылки (%1)';ru='Получатели рассылки (%1)';en='Recipients distribution (1%)'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьПредупреждение(, НСтр("ka='საჭიროა თუნდ ერთი მიმღების მითითება';ru='Требуется указать хотя бы одного получателя';en='You must specify at least one recipient'"));
...
Процедура ОткрытьФормаДобавленияПолучателей(ЭтоПодбор)
	...
	Если ЭтоПодбор Тогда
		...
		ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("ka='დაგზავნის მიმღებთა შერჩევა';ru='Подбор получателей рассылки';en='Selection of mailing recipients'"));
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если МассивНовыхСтрок.Количество() > 0 Тогда
		Если МассивНовыхСтрок.Количество() = 1 Тогда
			ЗаголовокОповещения = НСтр("ka='მიმღები დამატებულია დაგზავნაში';ru='Получатель добавлен в рассылку';en='The recipient is added to the mailing list'");
		Иначе
			ЗаголовокОповещения = НСтр("ka='მიმღებები დამატებულია დაგზავნაში';ru='Получатели добавлены в рассылку';en='Recipients added to the mailing list'");
		КонецЕсли;
