...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ka='ანგარიშგებების დაგზავნა. ელემენტის ფორმის გახსნა';ru='Рассылка отчетов. Открытие формы элемента'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='სახელწოდება არ არის შეყვანილი';ru='Не введено наименование'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='მიმღებების ტიპი არ არის არჩეული';ru='Не выбран тип получателей'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='არცერთი ანგარიშგება არ არის არჩეული';ru='Не выбрано ни одного отчета'");
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='გაშვების პერიოდულობა არ არის არჩეული';ru='Не выбрана периодичность запуска'");
		...
		Если Объект.ИспользоватьFTPРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.FTPСервер)
				...
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='FTP მისამართი არ არის შეყვანილი';ru='Не введен FTP адрес'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='Windows-ის ქსელური კატალოგი არ არის შეყვანილი';ru='Не введен сетевой каталог Windows'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='ქსელური კატალოგი Linux-ი არ არის შეყვანილი';ru='Не введен сетевой каталог Linux'");
		...
		Если Объект.ИспользоватьПапку Тогда
			Если Не ЗначениеЗаполнено(Объект.Папка) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='საქაღალდე არ არის არჩეული';ru='Не выбрана папка'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ka='საფოსტო მისამართი არ არის არჩეული';ru='Не выбран почтовый адрес'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ka='მიმღებების საფოსტო მისამართის ტიპი არ არის არჩეული';ru='Не выбран тип почтового адреса получателей'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='გასაგზავნად სააღრიცხვო ჩანაწერი არ არის არჩეული';ru='Не выбрана учетная запись для отправки'");
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		СтрокаВопроса = НСтр("ka='გასაგრძელებლად საჭიროა მიმღებების სიის გასუფთავება.';ru='Для продолжения необходимо очистить список получателей.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='გასუფთავება';ru='Очистить'"));
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ka='<Укажите получателя>';ru='<Укажите получателя>'"));
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = НСтр("ka='საქაღალდე ""%1""-ის ფაილების შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения файлов папки ""%1"".'");
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='არჩეული ჯგუფი გამოიყენება მხოლოდ პირადი დაგზავნისთვის ელექტრონული ფოსტით';ru='Выбранная группа используется только для личных рассылок по электронной почте'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = НСтр("ka='შეცდომების გამო ანგარიშგების დამატება ვერ მოხერხდა';ru='Не удалось добавить отчет из-за ошибок'");
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = НСтр("ka='შეცდომების გამო ანგარიშგებების დამატება ვერ მოხერხდა';ru='Не удалось добавить отчеты из-за ошибок'");
...
"%2'"),
		...
		ПараметрыПредупреждения.Заголовок = НСтр("ka='დაგზავნაში ანგარიშგებების დამატების შეცდომა';ru='Ошибки добавления отчетов в рассылку'");
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("ka='ყველა არჩეული ანგარიშგება უკვე შეტანილია დაგზავნაში';ru='Все выбранные отчеты уже включены в рассылку'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("ka='ანგარიშგება შეყვანილია დაგზავნაში';ru='Отчет включен в рассылку'");
			Иначе
				ЗаголовокОповещения = НСтр("ka='ანგარიშგებები შეტანილია დაგზავნაში';ru='Отчеты включены в рассылку'");
			КонецЕсли;
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = НСтр("ka='მიმღებების შესაყვანად აუცილებელია მათი ტიპის არჩევა';ru='Для ввода получателей необходимо выбрать их тип'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, НСтр("ka='დაგზავნა არ არის მომზადებული';ru='Рассылка не подготовлена'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	СтрокаОтчеты = Элементы.Отчеты.ТекущиеДанные;
	Если СтрокаОтчеты = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='შეარჩიეთ ანგარიშგება';ru='Выберите отчет'"));
		Возврат;
	КонецЕсли;
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("ka='მიმღების გამოყენება პარამეტრებში შესაძლებელია მხოლოდ დაგზავნის სახეობა ""%1""-სთვის.';ru='Использовать получателя в параметрах возможно только для вида рассылки ""%1""'");
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ველი ""მიმღებები"" არ არის შევსებული';ru='Поле ""Получатели"" не заполнено'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеИОМД.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='მიმღებების ტიპი ვერ მოიძებნა';ru='Тип получателей не найден'"));
		Возврат;
	КонецЕсли;
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ანგარიშგების გამართვა';ru='Выберите настройку отчета'"));
			Возврат;
		КонецЕсли;
		...
		Если СтрокаНастроек = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების გამართვა ვერ მოიძებნა';ru='Настройка отчета не найдена'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='მიმღების მითითება შეიძლება მხოლოდ ანგარიშგებების პარამეტრებისთვის და შერჩევებისთვის';ru='Указывать получателя можно только для параметров и отборов отчетов'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების გასამართად ხელმისაწვდომი ველის აღწერილობა ვერ მოიძებნა';ru='Для настройки отчета не найдено описание доступного поля'"));
					Возврат;
				КонецЕсли;
...
Процедура ЗаполнитьРасписаниеПоШаблону(Команда)
	...
	СписокВариантов.ПоказатьВыборЭлемента(Обработчик, НСтр("ka='აირჩიეთ განრიგის შაბლონი';ru='Выберите шаблон расписания'"));
КонецПроцедуры
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შაბლონის ტექსტში მიმღების გამოყენება შესაძლებელია მხოლოდ დაგზავნის ტიპ ""%1""-ისთვის';ru='Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = НСтр("ka='წერილის თემა უკვე შეესაბამება შაბლონს სტანდარტულად.';ru='Тема письма уже соответствует шаблону по умолчанию.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='წერილის ტექსტი უკვე შეესაბამება შაბლონს სტანდარტულად.';ru='Текст письма уже соответствует шаблону по умолчанию.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = НСтр("ka='წერილის თემაში სტანდარტულად შაბლონის დამატება';ru='Добавить в тему письма шаблон по умолчанию'");
			ТекстВопроса = НСтр("ka='წერილის თემა შევცვალოთ შაბლონით სტანდარტულად?';ru='Заменить тему письма на шаблон по умолчанию?'");
		Иначе
			ЗаголовокВопроса = НСтр("ka='წერილის ტექსტში შაბლონის დამატება სტანდარტულად';ru='Добавить в текст письма шаблон по умолчанию'");
			ТекстВопроса = НСтр("ka='წერილის ტექსტი შევცვალოთ შაბლონით სტანდარტულად?';ru='Заменить текст письма на шаблон по умолчанию?'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ka='შეცვლა';ru='Заменить'"));
		Кнопки.Добавить(2, НСтр("ka='დამატება';ru='Добавить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ka='ტესტური შეტყობინება 1С:საწარმო';ru='Тестовое сообщение 1С:Предприятие'"));
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = НСтр("ka='აირჩიეთ ფორმატები სტანდარტულად';ru='Выберите форматы по умолчанию'");
	Иначе
		ЗаголовокДиалога = НСтр("ka='ანგარიშგება ""%1""-სთვის აირჩიეთ ფორმატები';ru='Выберите форматы для отчета ""%1""'");
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		ЗаголовокВопроса = НСтр("ka='მიწოდების მეთოდის შემოწმება';ru='Проверка способа доставки'");
		Если Не Объект.Подготовлена Тогда
...
"Нажмите ""Продолжить"", чтобы записать рассылку.';ka='შემოწმებამდე დაგზავნა უნდა ჩაიწეროს. დაგზავნის ჩასაწერად დააჭირეთ ""გაგრძელებას"".'");
		...
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ka='გაგრძელება';ru='Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				НСтр("ka='ჯგუფი დაყენებულია დაგზავნის ტიპის შესაბამისად';ru='Группа установлена в соответствии с видом рассылки'"));
	...
	Если Изменения = "" Или Изменения = "Отчеты" Тогда
		...
		Если КоличествоОтчетов > 0 Тогда
			Элементы.СтраницаОтчеты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგებები (%1)';ru='Отчеты (%1)'"), 
				Формат(КоличествоОтчетов, "ЧН=0; ЧГ="));
		Иначе
			Элементы.СтраницаОтчеты.Заголовок = НСтр("ka='ანგარიშგებები';ru='Отчеты'") ;
		КонецЕсли;
	КонецЕсли;
	...
	Если Изменения = ""
		...
		СпособыДоставки = "";
		Если Объект.ИспользоватьПапку Тогда
			СпособыДоставки = НСтр("ka='საქაღალდე';ru='папка'");
		КонецЕсли;
		Если Объект.ИспользоватьСетевойКаталог Тогда
			СпособыДоставки = НСтр("ka='ქსელური კატალოგი';ru='сетевой каталог'");
		КонецЕсли;
		Если Объект.ИспользоватьFTPРесурс Тогда
			СпособыДоставки = НСтр("ka='FTP';ru='FTP'");
		КонецЕсли;
		Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.ТолькоУведомить Тогда
			СпособыДоставки = СпособыДоставки + ?(СпособыДоставки = "", НСтр("ka='ელ. ფოსტა';ru='эл. почта'"), " "+ НСтр("ka='და ელ. ფოსტა';ru='и эл. почта'"));
		КонецЕсли;
		
		Элементы.СтраницаДоставка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიტანა (%1)';ru='Доставка (%1)'"), СпособыДоставки);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ВыполнятьПоРасписанию" Тогда
		Если Объект.ВыполнятьПоРасписанию Тогда
			Элементы.СтраницаРасписание.Заголовок = НСтр("ka='განრიგი (აქტიურად)';ru='Расписание (активно)'");
		Иначе
			Элементы.СтраницаРасписание.Заголовок = НСтр("ka='განრიგი (არ არის აქტიური)';ru='Расписание (не активно)'");
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ka='დასაწყისი';ru='начала'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ka='ბოლოს';ru='конца'");
			КонецЕсли;
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = НСтр("ka='<მიუთითეთ მიმღებები>';ru='<Укажите получателей>'");
	...
	Если ПредставлениеОтключенных.Всего > 0 Тогда
		ШаблонРазделения = НСтр("ka='გარდა';ru='Кроме'")+ ": ";
	...
	Если Представление.МаксимумПревышен
		...
		Если ПредставлениеОтключенных.Всего > 0 Тогда
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='(სულ %1, %2-ის გამოკლებით)';ru='(всего %1, исключая %2)'"),
				Представление.Всего,
				ПредставлениеОтключенных.Всего);
		Иначе
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='(სულ %1)';ru='(всего %1)'"),
				Представление.Всего);
		КонецЕсли;
...
Функция ПредставлениеФорматовПоУмолчанию()
	Возврат НСтр("ka='ძირითადი მნიშვნელობა';ru='по умолчанию'");
КонецФункции
...
Функция ПолучателиУказаны(Получатели)
	Для Каждого СтрокаТаблицы Из Получатели Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТекстСообщения = НСтр("ka='არცერთი მიმღები არ არის არჩეული';ru='Не выбрано ни одного получателя'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПолучателиРассылки");
	Возврат Ложь;
КонецФункции
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ka='ანგარიშგებების დაგზავნა. მიწოდების მეთოდის შემოწმება';ru='Рассылка отчетов. Проверка способа доставки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = НСтр("ka='მიწოდების მეთოდის ტესტი წარმატებით იქნა გავლილი.';ru='Тест способа доставки успешно пройден.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("ka='მიწოდების მეთოდის ტესტი ვერ იქნა გავლილი.';ru='Тест способа доставки не пройден.'");
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ka='ანგარიშგებების დაგზავნა: ანგარიშგების ინიციალიზაცია';ru='Рассылка отчетов. Инициализация отчета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = НСтр("ka='ანგარიშგება ""%1""-ისთვის ჩატვირთულია ახალი სამომხმარებლო გამართვები.';ru='Для отчета ""%1"" загружены новые пользовательские настройки.'");
			Иначе
				ШаблонСтрокиСообщения = НСтр("ka='დამატებულია ანგარიშგება ""%1"".';ru='Добавлен отчет ""%1"".'");
			КонецЕсли;
...
Процедура ЗаписатьНаКлиенте(Результат, ПараметрыЗаписи) Экспорт
	...
	Если ПараметрыЗаписи.Шаг = 3 И ЖелательноОтключитьАрхивацию() Тогда
		...
		// Вопрос.
		ЗаголовокВопроса = НСтр("ka='დაარქივების გამორთვა';ru='Отключить архивацию'");
		ТекстВопроса = НСтр("ka='საქაღალდეში ანგარიშგებების გამოქვეყნებისას რეკომენდებულია ZIP-ში დაარქივების გამორთვა.';ru='При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ZIP-ში დაარქივების გამორთვა';ru='Отключить архивацию в ZIP'"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='გაგრძელება';ru='Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
