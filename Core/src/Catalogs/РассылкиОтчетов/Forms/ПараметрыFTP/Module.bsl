...
Процедура Заполнить(Команда)
	Если Сервер = "" Тогда
		ПолныйАдрес = НСтр("ka='ftp://სახელი:პაროლი@სერვერი:პორტი/კატალოგი/';ru='ftp://логин:пароль@сервер:порт/каталог/';en='ftp: // username: password @ server: port / directory /'");
	Иначе
		Если Логин = "" Тогда
	...
	ПоказатьВводСтроки(Обработчик, ПолныйАдрес, НСтр("ka='შეიყვანეთ სრული ftp მისამართი';ru='Введите полный ftp адрес';en='Enter the full address of the ftp'"))
КонецПроцедуры
