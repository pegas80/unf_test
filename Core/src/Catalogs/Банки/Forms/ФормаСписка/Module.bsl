...
"Подобрать?';ka='არის კლასიფიკატორიდან ბანკის შერჩევის შესაძლებლობა. შევარჩიოთ?'");
...
"კლასიფიკატორიდან განხორციელდება ყველა ბანკის განახლება. თუ ბანკების მონაცემები შეცვლილი იქნა ხელით, მაშინ ცვლილებები შეიძლება დაიკარგოს."
"შემდგომში, ავტომატური განახლებიდან ბანკის გამოსარიცხად, აუცილებელია ხელის რეჟიმში შეცვლის ნიშნის ჩართვა (ბრძანება ""შეცვლა"")."
...
Процедура ЗагрузитьПодготовленныеДанныеНаКлиенте(СтруктураДанных)
	...
	Если СтруктураДанных.Свойство("УспешноВыполнена") Тогда
		...
		ТекстОповещения = НСтр("ka='ბანკები განახლებულია კლასიფიკატორიდან';ru='Банки обновлены из классификатора';en='Banks are updated from the classifier'");
...
Функция ВыполнитьНаСервере(ИБФайловая)
	...
	Иначе
		НаименованиеФоновогоЗадания = НСтр("ka='ბანკების განახლება კლასიფიკატორიდან';ru='Обновление банков из классификатора';en='Updating the banks of the classifier'");
