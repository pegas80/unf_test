...
Функция СформироватьАвтоНаименованиеНаКлиенте()
	...
	Если Объект.СпособПредоставления = СпособПредоставленияПроцент Тогда
		...
		СтрокаНаименования = "" + Объект.ЗначениеСкидкиНаценки + НСтр("ka='%';ru='%';en='%'");
	...
	ИначеЕсли Объект.УсловияПредоставления.Количество() > 1 Тогда
		...
		Если КоличествоУсловий >= 2 Тогда
			СтрокаНаименования = СтрокаНаименования + " " +НСтр("ka='(რამდენიმე პირობა)';ru='(несколько условий)';en='(Some conditions)'");
			Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
		КонецЕсли;
		
	ИначеЕсли Объект.УсловияПредоставления.Количество() = 0 Тогда
		СтрокаНаименования = СтрокаНаименования + " " + НСтр("ka='პირობების გარეშე';ru='без условий';en='without conditions'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	...
	Если Объект.НоменклатураГруппыЦеновыеГруппы.Количество() > 0 И НЕ СуммаВДокументе Тогда
		СтрокаНаименования = СтрокаНаименования + НСтр("ka=', დაზუსტებით';ru=', с уточнением';en=', Specifying'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	...
	Если (Объект.ПолучателиСкидкиКонтрагенты.Количество() > 0 И Объект.Назначение <> НазначениеРозница) 
		...
		СтрокаНаименования = СтрокаНаименования + НСтр("ka=', მითითებულია მიმღებები';ru=', указаны получатели';en='Are listed recipients'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	
	Если Объект.ВремяПоДнямНедели.Количество() > 0 Тогда
		СтрокаНаименования = СтрокаНаименования + НСтр("ka=', по расписанию';ru=', по расписанию';en=', Scheduled'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьЭлементаАртикул();//05.04.2024
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ДатаНачала > Объект.ДатаОкончания Тогда
		...
		Сообщение.Текст = НСтр("ka='Дата начала больше даты окончания!';ru='Дата начала больше даты окончания!';en='Start date more than the end date!'");
...
Процедура УправлениеВидимостьюНаСервере()
	...
	Если Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоНоменклатуре Тогда
		...
		Элементы.НоменклатураГруппыЦеновыеГруппыДобавитьГруппу.Заголовок = НСтр("ka='ნომენკლატურის ჯგუფის დამატება';ru='Добавить группу номенклатуры';en='Add a group of nomenclature'");
	ИначеЕсли Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоКатегориям Тогда
		...
		Элементы.НоменклатураГруппыЦеновыеГруппыДобавитьГруппу.Видимость = Истина;
		Элементы.НоменклатураГруппыЦеновыеГруппыДобавитьГруппу.Заголовок = НСтр("ka='კატეგორიის ჯგუფის დამატება';ru='Добавить группу категорий';en='Add category group'");
	ИначеЕсли Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоЦеновымГруппам Тогда
		Элементы.НоменклатураГруппыЦеновыеГруппыЗначениеУточнения.Заголовок = Нстр	("ru = 'Ценовая группа'; ka = 'ფასების ჯგუფი'; en = 'Price group'");
		...
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = НСтр ("ru = 'Заполните уточнения, если требуется, чтобы у товаров определённых ценовых групп значение скидки отличалось от основного значения. Если список не заполнен, то для всех ценовых групп используется основное значение скидки'; ka = 'შეავსეთ სპეციფიკაციები, თუ გსურთ, რომ გარკვეული ფასების ჯგუფების პროდუქციის ფასდაკლების ღირებულება განსხვავდებოდეს ძირითადი ღირებულებისგან. თუ სია არ არის შევსებული, მაშინ ძირითადი ფასდაკლების ღირებულება გამოიყენება ფასების ყველა ჯგუფისთვის.'; en = 'Fill in the clarifications, if necessary, so that for products in certain price groups, the discount value differs from the default value. If the list is not filled, then the default discount value is used for all price groups.'") ; 			 //chilo
...
Процедура ЗначениеСкидкиНаценкиПриИзменении(Элемент)
	...
	Если Объект.СпособПредоставления = СпособПредоставленияПроцент Тогда
		Если Объект.ЗначениеСкидкиНаценки > 100 Тогда
			ТекстСообщения = НСтр("ka='ფასდაკლების პროცენტი 100%-ზე მეტი არ უნდა იყოს';ru='Процент скидки должен быть не более 100%';en='Discount percentage to be not more than 100%'");
...
Процедура НастройкаВидимостиПолучателейНаСервере()
	...
	Элементы.ВидЦен.Видимость = (Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияАвтоматическихСкидок.ПоВидуЦен"));// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
...
//Paganini(LTB)_{
&НаКлиенте
Процедура ЗаполнитьПоДопРеквизитамХарактеристики(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ЗначениеСкидкиНаценки",	Истина);
	ОбработкаОписание = Новый ОписаниеОповещения("ОбработатьРезультатЗаполнения", ЭтотОбъект);
	ОткрытьФорму("Обработка.Ценообразование.Форма.ФормаЗаполненияПоДопРеквизитамХарактеристики", ПараметрыОткрытия, ЭтотОбъект, , , , ОбработкаОписание);
...
Процедура ОбработатьРезультатЗаполненияНаСервере(Результат, ДополнительныеПараметры)
	...
	Если ЗначениеЗаполнено(Результат.АдресВременногоХранилища) Тогда
		ТаблицаХарактеристик = ПолучитьИзВременногоХранилища(Результат.АдресВременногоХранилища).ТаблицаХарактеристик;
		ТаблицаХарактеристик.Колонки.Номенклатура.Имя = "ЗначениеУточнения";
		Для Каждого СтрокаХ ИЗ ТаблицаХарактеристик Цикл
			НайденныеСтрокиХ = Объект.НоменклатураГруппыЦеновыеГруппы.НайтиСтроки(Новый Структура("ЗначениеУточнения, Характеристика", СтрокаХ.ЗначениеУточнения, СтрокаХ.Характеристика));
			Если НайденныеСтрокиХ.Количество() = 0 Тогда
				НоваяСтрока = Объект.НоменклатураГруппыЦеновыеГруппы.Добавить();
			Иначе
				НоваяСтрока = НайденныеСтрокиХ[0];
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаХ);
		КонецЦикла;
	КонецЕсли;
...
//05.04.2024
&НаКлиенте
Процедура НоменклатураГруппыЦеновыеГруппыЗначениеУточненияПриИзменении(Элемент)
	...
	Если Объект.ВариантОграниченияПоНоменклатуре = ПредопределенноеЗначение("Перечисление.ВариантыОграниченийСкидокПоНоменклатуре.ПоНоменклатуре") Тогда 
		СтрокаТабличнойЧасти = Элементы.НоменклатураГруппыЦеновыеГруппы.ТекущиеДанные;
		СтрокаТабличнойЧасти.Артикул = ПолучитьАртикул(СтрокаТабличнойЧасти.ЗначениеУточнения);        
	КонецЕсли;
...
&НаСервере
Процедура ВидимостьЭлементаАртикул()
	Элементы.НоменклатураГруппыЦеновыеГруппыАртикул.Видимость = Объект.ВариантОграниченияПоНоменклатуре = ПредопределенноеЗначение("Перечисление.ВариантыОграниченийСкидокПоНоменклатуре.ПоНоменклатуре");//05.04.2024	
КонецПроцедуры
