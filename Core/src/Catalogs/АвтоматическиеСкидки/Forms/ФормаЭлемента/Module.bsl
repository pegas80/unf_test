...
Функция СформироватьАвтоНаименованиеНаКлиенте()
	...
	Если Объект.СпособПредоставления = СпособПредоставленияПроцент Тогда
		...
		СтрокаНаименования = "" + Объект.ЗначениеСкидкиНаценки + НСтр("ka='%';ru='%'");
	...
	ИначеЕсли Объект.УсловияПредоставления.Количество() > 1 Тогда
		...
		Если КоличествоУсловий >= 2 Тогда
			СтрокаНаименования = СтрокаНаименования + " " +НСтр("ka='(რამდენიმე პირობა)';ru='(несколько условий)'");
			Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
		КонецЕсли;
		
	ИначеЕсли Объект.УсловияПредоставления.Количество() = 0 Тогда
		СтрокаНаименования = СтрокаНаименования + " " + НСтр("ka='პირობების გარეშე';ru='без условий'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	...
	Если Объект.НоменклатураГруппыЦеновыеГруппы.Количество() > 0 И НЕ СуммаВДокументе Тогда
		СтрокаНаименования = СтрокаНаименования + НСтр("ka=', დაზუსტებით';ru=', с уточнением'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	...
	Если (Объект.ПолучателиСкидкиКонтрагенты.Количество() > 0 И Объект.Назначение <> НазначениеРозница) 
		...
		СтрокаНаименования = СтрокаНаименования + НСтр("ka=', მითითებულია მიმღებები';ru=', указаны получатели'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	
	Если Объект.ВремяПоДнямНедели.Количество() > 0 Тогда
		СтрокаНаименования = СтрокаНаименования + НСтр("ka=', по расписанию';ru=', по расписанию'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ДатаНачала > Объект.ДатаОкончания Тогда
		...
		Сообщение.Текст = НСтр("ka='Дата начала больше даты окончания!';ru='Дата начала больше даты окончания!'");
...
Процедура УправлениеВидимостьюНаСервере()
	...
	Если Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоНоменклатуре Тогда
		...
		Элементы.НоменклатураГруппыЦеновыеГруппыДобавитьГруппу.Заголовок = НСтр("ka='ნომენკლატურის ჯგუფის დამატება';ru='Добавить группу номенклатуры'");
	ИначеЕсли Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоКатегориям Тогда
		...
		Элементы.НоменклатураГруппыЦеновыеГруппыДобавитьГруппу.Видимость = Истина;
		Элементы.НоменклатураГруппыЦеновыеГруппыДобавитьГруппу.Заголовок = НСтр("ka='Добавить группу категорий';ru='Добавить группу категорий'");
...
Процедура ЗначениеСкидкиНаценкиПриИзменении(Элемент)
	...
	Если Объект.СпособПредоставления = СпособПредоставленияПроцент Тогда
		Если Объект.ЗначениеСкидкиНаценки > 100 Тогда
			ТекстСообщения = НСтр("ka='ფასდაკლების პროცენტი 100%-ზე მეტი არ უნდა იყოს';ru='Процент скидки должен быть не более 100%'");
