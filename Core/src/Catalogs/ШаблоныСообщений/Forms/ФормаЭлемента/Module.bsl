...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВариантыНазначения = 1 И ПустаяСтрока(Объект.ПолноеИмяТипаПараметраВводаНаОсновании) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Укажите назначение шаблона сообщения';ru='Укажите назначение шаблона сообщения'"),, "Объект.ПолноеИмяТипаПараметраВводаНаОсновании");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(РезультатПроверкиШаблоны) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Шаблон сообщения не может быть записан.';ru='Шаблон сообщения не может быть записан.'") + Символы.ПС
...
Процедура СвойстваВложения(Команда)
	...
	Если ТекущиеДанные.Ссылка = ПредопределенноеЗначение("Справочник.ШаблоныСообщенийПрисоединенныеФайлы.ПустаяСсылка") Тогда
		...
		ТекстВопроса = НСтр("ka='Свойства файла доступны только после его записи. Записать?';ru='Свойства файла доступны только после его записи. Записать?'");
...
Процедура ПроверитьШаблон(Команда)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='Шаблон заполнен корректно';ru='Шаблон заполнен корректно'"));
	КонецЕсли;
...
Процедура ОтображениеЭлементовФормы()
	...
	Если Объект.ПредназначенДляSMS Тогда
		СуффиксЗаголовка = НСтр("ka='Шаблон сообщения SMS';ru='Шаблон сообщения SMS'");
	...
	Иначе
		СуффиксЗаголовка = НСтр("ka='Шаблон сообщения электронного письма';ru='Шаблон сообщения электронного письма'");
	...
	Иначе
		Заголовок = СуффиксЗаголовка + " (" + НСтр("ka='ქმნილება';ru='создание'")+ ")";
	КонецЕсли;
...
Функция РезультатПроверкиШаблоны()
	...
	Если ОшибочныеРеквизиты.Количество() > 0 Тогда
		ТекстОшибки = ?(ОшибочныеРеквизиты.Количество() = 1,
			НСтр("ka='Некорректный реквизит в шаблоне сообщения:';ru='Некорректный реквизит в шаблоне сообщения:'"),
			НСтр("ka='Некорректные реквизиты в шаблоне сообщения:';ru='Некорректные реквизиты в шаблоне сообщения:'")) + " ";
		Разделитель = "";
...
Процедура УстановитьЭлектронноеПисьмоОбычныйТекст(ТребуетсяПереносТекста = Ложь)
	Элементы.ФормаВидТекстаЭлектронногоПисьма.Заголовок = НСтр("ka='ჩვეულებრივი ტექსტი';ru='Обычный текст'");
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Письмо, АдресВоВременномХранилище, ИмяФайла,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Если ПустаяСтрока(ИмяБезРасширения) Тогда
			...
			ИмяФайлаБезРасширения = НСтр("ka='Вложение без имени';ru='Вложение без имени'") 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		Иначе
			...
			Если ЭлементНазначения = Неопределено Тогда
				...
				ОписаниеОшибки = НСтр("ka='Предмет %1 определенный в внешней обработке не найдено. Внешняя обработка не может быть подключена.';ru='Предмет %1 определенный в внешней обработке не найдено. Внешняя обработка не может быть подключена.'");
