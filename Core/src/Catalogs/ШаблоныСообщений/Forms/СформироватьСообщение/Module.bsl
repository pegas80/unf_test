...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидСообщения = "СообщениеSMS" Тогда
		...
		Заголовок = НСтр("ka='Шаблоны сообщений SMS';ru='Шаблоны сообщений SMS'");
	Иначе
...
Процедура ОтравитьСообщениеПроверкаУчетнойЗаписиВыполнена(Результат, ПараметрыОтправки) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = Сообщение.ОписаниеОшибки + Символы.ПС + НСтр("ka='Открыть форму отправки сообщения?';ru='Открыть форму отправки сообщения?'");
...
Процедура ЗаполнитьСписокДоступныхШаблонов()
	...
	Если НЕ РежимВыбора Тогда
		...
		ПерваяСтрока.Представление = НСтр("ka='<Без шаблона>';ru='<Без шаблона>'");
	КонецЕсли;
