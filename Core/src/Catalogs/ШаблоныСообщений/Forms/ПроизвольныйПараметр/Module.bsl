...
Функция ТипПараметраСтрокой(ПолноеИмяТипа)
	...
	Если СтрСравнить(ПолноеИмяТипа, НСтр("ka='თარიღი';ru='Дата'")) = 0 Тогда
		Результат = Тип("Дата");
	ИначеЕсли СтрСравнить(ПолноеИмяТипа, НСтр("ka='Строка';ru='Строка'")) = 0 Тогда
...
Процедура ОК(Команда)
	...
	Для Каждого ПараметрИзФормы Из СписокПараметров Цикл
		Если СтрСравнить(ПараметрИзФормы.Значение, ИмяПараметра) = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Некорректное имя параметра. Параметр с таким именем уже был добавлен ранее.';ru='Некорректное имя параметра. Параметр с таким именем уже был добавлен ранее.'"));
			Возврат;
		КонецЕсли;
		Если СтрСравнить(ПараметрИзФормы.Представление, ПредставлениеПараметра) = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Некорректное представление параметра. Параметр с таким представлением уже был добавлен ранее.';ru='Некорректное представление параметра. Параметр с таким представлением уже был добавлен ранее.'"));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	Если НедопустимоеИмяПараметра(ИмяПараметра) ИЛИ ПустаяСтрока(ИмяПараметра) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Некорректное имя параметра. Нельзя использовать пробелы, знаки пунктуации и другие спец. символы.';ru='Некорректное имя параметра. Нельзя использовать пробелы, знаки пунктуации и другие спец. символы.'"));
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(ПредставлениеПараметра) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Некорректное представление параметра.';ru='Некорректное представление параметра.'"));
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(ТипСтрокой) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Некорректный тип параметра.';ru='Некорректный тип параметра.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораВводНаОсновании(ТипПараметра)
	...
	Элементы.ТипСтрокой.СписокВыбора.Вставить(0, "Дата", НСтр("ka='თარიღი';ru='Дата'"));
	Элементы.ТипСтрокой.СписокВыбора.Вставить(0, "Строка", НСтр("ka='Строка';ru='Строка'"));
