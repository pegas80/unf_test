...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(КонтрагентВладелец) Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='ხელშეკრულებები   თან';ru='Договоры с';en='with contracts'") + " """ + КонтрагентВладелец + """";
...
Процедура ЗаписатьОсновнойДоговорКонтрагента(СтруктураПараметров)
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ka='ძირითადი ხელშეკრულების შესაცვლელად კონტრაგენტის დაბლოკვა ვერ მოხერხდა: %1, შემდეგი მიზეზით: %2';ru='Не удалось заблокировать контрагента: %1, для изменения основного договора, по причине:"
