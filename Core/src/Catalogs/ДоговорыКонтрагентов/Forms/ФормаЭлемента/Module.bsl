	...
	Если Объект.ЭтоДоговорОбслуживания И НЕ Объект.ПометкаУдаления Тогда
		...
		Пока Выборка.Следующий() Цикл
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='ხელშეკრულების მოქმედების დაწყების თარიღისთვის (%3) ფასების სახეობა ""%2""-ის მიხედვით ნომენკლატურა ""%1""-ისთვის ფასი არ არის დაყენებული!';ru='Для номенклатуры ""%1"" не установлена цена по виду цен ""%2"" на дату начала действия договора (%3)!'"),
...
Процедура ПроверитьКорректностьДатыОкончанияДоговора()
	...
	Если Объект.ДоговорОбслуживанияДатаОкончания < Объект.ДоговорОбслуживанияДатаНачала Тогда
		...
		Сообщение.Текст = НСтр("ka='Дата окончания действия договора обслуживания не может быть раньше даты начала.';ru='Дата окончания действия договора обслуживания не может быть раньше даты начала.'");
		Сообщение.Сообщить();
		Возврат;
...
Функция СформироватьНаименование(НомерДоговора, ДатаДоговора, ВалютаРасчетов)
	...
	ТекстНаименования = НСтр("ka='%ДатаДоговора%№ %НомерДоговора% (%ВалютаРасчетов%)';ru='№ %НомерДоговора% %ДатаДоговора% (%ВалютаРасчетов%)'");
...
Процедура ПроверитьВозможностьИзменений(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ka='ბაზაში არის ეკვაირინგის ტერმინალები, რომლებშიც არჩეულია მიმდინარე ხელშეკრულება. ანგარიშსწორებების ვალუტის შეცვლა აკრძალულია';ru='В базе присутствуют эквайринговые терминалы, в которых выбран текущий договор. Изменение валюты расчетов запрещено.'");
...
Процедура СоздатьНаправлениеДеятельностиВопрос()
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ka='მოქმედების მიმართულების შექმნამდე აუცილებელია ობიექტის ჩაწერა. ჩავწეროთ?';ru='Перед созданием направления действия необходимо записать объект. Записать?'");
