...
Функция ДоговорСоответствуетУсловиямДокумента(ТекстСообщения, Договор, Организация, Контрагент, СписокВидовДоговора) Экспорт
	...
	ТекстСообщения = НСтр("ka='ხელშეკრულების რეკვიზიტები არ შეესაბამება დოკუმენტის პირობებს:';ru='Реквизиты договора не соответствуют условиям документа:';en='Essential elements of the contract does not comply with the terms of the document:'");
	
	Если НеСоответствуетОрганизация Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka=' - არ ემთხვევა ორგანიზაცია';ru='"
" - Не совпадает организация'");
	КонецЕсли;
	
	Если НеСоответствуетВидДоговора Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ka='"
" - ხელშეკრულების ტიპი არ ემთხვევა';ru='"
...
Функция ПолучитьСписокВидовДоговораДляДокумента(Документ, ВидОперации = Неопределено, ИмяТабличнойЧасти = "") Экспорт
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ПоступлениеВКассу") 
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ПрочиеРасчеты
			...
			ИЛИ ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.РасходИзКассы") 
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ПрочиеРасчеты
			...
			ИЛИ ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту
			...
			ИЛИ ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту
...
Функция ВводПФДляДоговора_Актуальность(ДатаОтчета, Контрагент, Договор) Экспорт
	...
	|	УдержанияДляПенсионногоФондаСрезПоследних.УчаствуетДобровольно КАК УчаствуетДобровольно,
	|	УдержанияДляПенсионногоФондаСрезПоследних.Актуальность КАК Актуальность
	|ИЗ
	|	РегистрСведений.УдержанияДляПенсионногоФонда.СрезПоследних(
	|			&ДатаОтчета,
	|			Контрагент = &Контрагент
	|				И Договор = &Договор) КАК УдержанияДляПенсионногоФондаСрезПоследних";
	...
	Если Выборка.Следующий() Тогда
		Результат = Выборка.Актуальность;	
	КонецЕсли;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='ხელშეკრულების ფორმა (HTML)';ru='Форма договора (HTML)';en='Contract form (HTML)'");
	...
	КомандаПечати.Представление = НСтр("ka='ხელშეკრულების ფორმა (Microsoft word)';ru='Форма договора (Microsoft word)';en='Contract form (Microsoft word)'");
	КомандаПечати.СписокФорм = "";
	КомандаПечати.ФункциональныеОпции = "ДоговорыКонтрагентовТехническиеКомандыПечати";
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ТарифныйПланДоговораОбслуживания";
	КомандаПечати.Представление = НСтр("ka='სატარიფო გეგმა';ru='Тарифный план';en='Tariff plan'");
	КомандаПечати.СписокФорм = "ФормаЭлемента,ФормаСписка,ФормаВыбора,ФормаВыбораСКонтрагентом";
...
Функция ПечатьТарифныйПланДоговораОбслуживания(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	...
	Для каждого Эл Из МассивОбъектов Цикл
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ka='ხელშეკრულება %1 მომსახურების ხელშეკრულებას არ წარმოადგენს!';ru='Договор %1 не является договором обслуживания!';en='Contract% 1 is not a contract of service!'"),
			Эл.Ссылка
		);
