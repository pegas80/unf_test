...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Наименование		= НСТР("ka='ძირითადი ხელშეკრულება';ru='Основной договор'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ТорговыйПредставитель) Тогда
		...
		//	ТекстСообщения = НСтр("ka='ველი ""მისამართი"" არ არის შევსებული';ru='Поле ""Адрес"" не заполнено'");
		...
		Если ЗначениеЗаполнено(Водитель) И НЕ ЗначениеЗаполнено(Водитель.ИНН) Тогда
			ТекстСообщения = НСтр("ka='ველი ""ს/ნ"" არ არის შევსებული ცნობარში ""მძროლი""';ru='Поле ""ИНН"" не заполнено в справочнике водителя'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, , , , Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПометкаУдаления Тогда
		...
		Если Не ЗначенияРеквизитовКонтрагента.ПометкаУдаления И ЗначенияРеквизитовКонтрагента.ДоговорПоУмолчанию = Ссылка Тогда
			ТекстСообщения = НСтр("ka='ძირითად ხელშეკრულებად დაყენებული კონტრაგენტის ხელშეკრულების წასაშლელად მონიშვნა არ შეიძლება.';ru='Договор контрагента, установленный в качестве основного, не может быть помечен на удаление.'");
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ka='კონტრაგენტისთვის ხელშეკრულებების მიხედვით აღრიცხვა არ წარმოებს.';ru='Для контрагента не ведется учет по договорам.'");
...
Процедура СформироватьНаименование() Экспорт
	...
	Если Не ПустаяСтрока(НомерДоговора) Тогда
		Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='№ %1 %2 (%3)';ru='№ %1 %2 (%3)'"),
