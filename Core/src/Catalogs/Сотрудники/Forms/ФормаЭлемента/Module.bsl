...
Функция ПроверитьСотрудникаНаНовый()
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(,НСТр("ka='ხელმისაწვდომია მოცემული თანამშრომლის ჩაწერის შემდეგ';ru='Доступно после записи данного сотрудника'"));
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОпределитьВычетНаДетей()
	...
	Если Выборка.Следующий() И Выборка.КоличествоДетей > 0 Тогда
		ВычетыПоНДФЛНаДетей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2, დაქვითვა %3 ლარის ოდენობით';ru='%1 %2, вычетов на сумму %3 руб.'"),
			Выборка.КоличествоДетей,
			ПодобратьСловоПодЧисло(
				Выборка.КоличествоДетей,
				НСтр("ka='ბავშვი';ru='ребенок'"),
				НСтр("ka='ბავშვს';ru='ребенка'"),
				НСтр("ka='ბავშვებს';ru='детей'")),
...
Процедура УстановитьЗаголовокКадровыхДанных(Форма)
	...
	Если Не ЗначениеЗаполнено(КлючТекущиеКадровыеДанныеСотрудников.ДатаПриема) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='არ არის დასაქმებული';ru='не трудоустроен'"));
	ИначеЕсли ЗначениеЗаполнено(КлючТекущиеКадровыеДанныеСотрудников.ДатаУвольнения) Тогда
		// Уволен
		ДинамическиеПараметры.Добавить(НСтр("ka='გათავისუფლებულია. გათავისუფლების თარიღი';ru='уволен. Дата увольнения'")+ " " + Формат(КлючТекущиеКадровыеДанныеСотрудников.ДатаУвольнения, "ДФ=dd.MM.yyyy")+ НСтр("ka='წ.';ru=' г.'"));
	Иначе
		// Принят
		ДинамическиеПараметры.Добавить(НСтр("ka='მიღებულია სამუშაოზე';ru='принят на работу'")+  " " + Формат(КлючТекущиеКадровыеДанныеСотрудников.ДатаПриема, "ДФ=dd.MM.yyyy")+ НСтр("ka='წ.';ru=' г.'"));
		ДинамическиеПараметры.Добавить(НСтр("ka='თანამდებობა';ru='должность'")+ " " + КлючТекущиеКадровыеДанныеСотрудников.ТекущаяДолжность);
		ДинамическиеПараметры.Добавить(НСтр("ka='განაკვეთი';ru='оклад'")+ " " + КлючТекущиеКадровыеДанныеСотрудников.ТекущаяТарифнаяСтавка + " " + КлючТекущиеКадровыеДанныеСотрудников.ВалютаТарифнойСтавки);
	КонецЕсли;
...
Процедура УстановитьЗаголовокВычетовНДФЛ(Форма)
	...
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='არ არის.';ru='отсутствуют.'"));
	КонецЕсли;
