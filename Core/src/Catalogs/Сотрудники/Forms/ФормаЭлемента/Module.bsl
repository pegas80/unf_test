...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РольДоступна("ЗапретДоступаКСправочникуСотрудники") Тогда 
		Отказ = Истина;     
		Сообщить("Вас ограничели в просмотре и редактировании данного справочника");
		Возврат;
	КонецЕсли;
	//--- Дмгг, 11.08.2025, ONESOFT-4283
	...
	// Счета регл по умолачнию
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
	
		Объект.СчетРасчетовСПерсоналомРегл = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3130");
		Объект.СчетРасчетовСПодотчетникамиРегл = ПланыСчетов.Хозрасчетный.НайтиПоКоду("1430");
		Объект.СчетРасчетовПоПерерасходуРегл = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3160");
		Объект.ВидВыдачиНДФЛ = "1";
		Объект.КатегорияПолучателейДоходов = "4";//"1.1"; Дмгг
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменилисьСчетаСотрудники" Тогда
		...
		Объект.СчетРасчетовСПерсоналомРегл = Параметр.СчетРасчетовСПерсоналомРегл;
		Объект.СчетРасчетовСПодотчетникамиРегл = Параметр.СчетРасчетовСПодотчетникамиРегл;
		Объект.СчетРасчетовПоПерерасходуРегл = Параметр.СчетРасчетовПоПерерасходуРегл;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//ПроверитьУникалностШтрихкода
	//++Lazare 29.05.2023 
	Если ЗначениеЗаполнено(Объект.ОсновнойШтрихКод) Тогда 	  
		НайденнийСотрудник = Справочники.Сотрудники.НайтиПоРеквизиту("ОсновнойШтрихКод",СокрЛП(Объект.ОсновнойШтрихКод));		
		Если ЗначениеЗаполнено(НайденнийСотрудник) Тогда 			 		
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Такой штрихкод уже назначен для сотрудника- %1'; ka = 'ასეთი შტრიხკოდი უკვე დანიშნულია თანამშრომლისთვის- %1 '"),НайденнийСотрудник);	 
...
Процедура ПолучитьАктуальнуюКадровуюИнформациюОСотруднике(Сотрудник, КлючТекущаяКадроваяИнформация)
	...
	|	ТекущиеКадровыеДанныеСотрудников.Организация КАК Организация,
	...
	|	ТекущиеКадровыеДанныеСотрудников.ТекущаяТарифнаяСтавка КАК ТекущаяТарифнаяСтавка,
	|	ТекущиеКадровыеДанныеСотрудников.ВалютаТарифнойСтавки КАК ВалютаТарифнойСтавки,
	|	ТекущиеКадровыеДанныеСотрудников.ДатаОкончанияДоговора КАК ДатаОкончанияДоговора
	|ИЗ
...
Функция ПроверитьСотрудникаНаНовый()
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(,НСТр("ka='ხელმისაწვდომია მოცემული თანამშრომლის ჩაწერის შემდეგ';ru='Доступно после записи данного сотрудника';en='Available after the recording of the employee'"));
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОпределитьВычетНаДетей()
	...
	Если Выборка.Следующий() И Выборка.КоличествоДетей > 0 Тогда
		ВычетыПоНДФЛНаДетей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2, დაქვითვა %3 ლარის ოდენობით';ru='%1 %2, вычетов на сумму %3 руб.';en='% 1% 2% residue in the amount of 3 rubles.'"),
			Выборка.КоличествоДетей,
			ПодобратьСловоПодЧисло(
				Выборка.КоличествоДетей,
				НСтр("ka='ბავშვი';ru='ребенок';en='child'"),
				НСтр("ka='ბავშვს';ru='ребенка';en='child'"),
				НСтр("ka='ბავშვებს';ru='детей';en='children'")),
...
Процедура УстановитьЗаголовокКадровыхДанных(Форма)
	...
	Если Не ЗначениеЗаполнено(КлючТекущиеКадровыеДанныеСотрудников.ДатаПриема) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='არ არის დასაქმებული';ru='не трудоустроен';en='not employed'"));
	ИначеЕсли ЗначениеЗаполнено(КлючТекущиеКадровыеДанныеСотрудников.ДатаУвольнения) Тогда
		// Уволен
		ДинамическиеПараметры.Добавить(НСтр("ka='გათავისუფლებულია. გათავისუფლების თარიღი';ru='уволен. Дата увольнения';en='I fired. date of dismissal'")+ " " + Формат(КлючТекущиеКадровыеДанныеСотрудников.ДатаУвольнения, "ДФ=dd.MM.yyyy")+ НСтр("ka='წ.';ru=' г.'"));
	Иначе
		// Принят
		ДинамическиеПараметры.Добавить(НСтр("ka='მიღებულია სამუშაოზე';ru='принят на работу';en='hired'")+  " " + Формат(КлючТекущиеКадровыеДанныеСотрудников.ДатаПриема, "ДФ=dd.MM.yyyy")+ НСтр("ka='წ.';ru=' г.'"));
		ДинамическиеПараметры.Добавить(НСтр("ka='თანამდებობა';ru='должность';en='должность'")+ " " + КлючТекущиеКадровыеДанныеСотрудников.ТекущаяДолжность);
		ДинамическиеПараметры.Добавить(НСтр("ka='განაკვეთი';ru='оклад';en='salary'")+ " " + КлючТекущиеКадровыеДанныеСотрудников.ТекущаяТарифнаяСтавка + " " + КлючТекущиеКадровыеДанныеСотрудников.ВалютаТарифнойСтавки);
	КонецЕсли;
...
Процедура УстановитьЗаголовокВычетовНДФЛ(Форма)
	...
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='არ არის.';ru='отсутствуют.';en='no.'"));
	КонецЕсли;
...
КонецПроцедуры // Подключаемый_РедактироватьСоставСвойств()    //++Lazare 27.12.2021
...
Процедура ПолучитьИнфоИзRS(Команда)
	...
	Если Не ЗначениеЗаполнено(ФизическоеЛицо.ИНН) Тогда 
...
Процедура ПолучитьИнфоИзRSСервер()
	...
	ДанныеКонтрагента = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьИнфоИзRSНаСервере(ФизическоеЛицо.ИНН);
...
Процедура  ДобавитьЮрАдресс(ЮрАдресс)
	...
	СтруктураПараметров = Новый Структура("Вид",Справочники.ВидыКонтактнойИнформации.АдресФизЛицаПоПрописке);
	СтрокаКИ = ТЗ.НайтиСтроки(СтруктураПараметров);
	Если СтрокаКИ.Количество() = 0 Тогда
		КонтактнаяИнформацияУНФ.ДобавитьКонтактнуюИнформацию(ЭтотОбъект, Справочники.ВидыКонтактнойИнформации.АдресФизЛицаПоПрописке, Истина);
	Иначе
		СтрокаКИ = СтрокаКИ[0];
	КонецЕсли;
	...
	СтрокаКИ.Вид = Справочники.ВидыКонтактнойИнформации.АдресФизЛицаПоПрописке;
	СтрокаКИ.Представление = ЮрАдресс;
	СтрокаКИ.ЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияXMLПоПредставлениюГрузия(ЮрАдресс);
		
	ЭтаФорма.КонтактнаяИнформация.Загрузить(ТЗ);
