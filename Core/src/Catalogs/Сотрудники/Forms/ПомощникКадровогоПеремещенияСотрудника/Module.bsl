...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПринятьНаРаботу Тогда
		ЭтаФорма.Заголовок = НСтр("ru='Прием на работу сотрудника ';ka='Прием на работу сотрудника '")+ Сотрудник;
	...
	ИначеЕсли ОформитьУвольнение Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru='Увольнение сотрудника ';ka='Увольнение сотрудника '")+ Сотрудник;
	ИначеЕсли ИзменитьОклад Тогда
		Элементы.ПанельОсновная.Видимость = Истина;
		Элементы.ГруппаУвольнение.Видимость = Ложь;
		
		ЭтаФорма.Заголовок = НСтр("ru='Изменение заработной зарплаты сотрудника ';ka='Изменение заработной зарплаты сотрудника '")+ Сотрудник;
		Элементы.ДекорацияТри.Заголовок = НСтр("ka='ხელფასისი შეცვლა';ru='Изменение оклада';en='change salary'");
		Элементы.ПриемНаРаботуДатаПриема.Заголовок = НСтр("ka='ცვლილების თარიღი';ru='Дата изменения';en='Date of change'");
	...
	ИначеЕсли ИзменитьДолжность Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru='Кадровое перемещение сотрудника ';ka='Кадровое перемещение сотрудника '")+ Сотрудник;
		Элементы.ДекорацияТри.Заголовок = НСтр("ka='საკადრო გადაადგილება';ru='Кадровое перемещение';en='personnel movement'");
		Элементы.ПриемНаРаботуДатаПриема.Заголовок = НСтр("ka='გადაადგილების თარიღი';ru='Дата перемещения';en='Date of travel'");
		Элементы.ПриемНаРаботуОрганизация.ТолькоПросмотр = Истина;
...
Процедура СоздатьИтоговыйДокумент(Отказ)
	...
	Если ОформитьУвольнение Тогда
		Если Не ЗначениеЗаполнено(УвольнениеДатаУвольнения) Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია გათავისუფლების თარიღის შევსება.';ru='Необходимо заполнить дату увольнения.';en='It is necessary to fill the dismissal date.'");
	...
	Попытка
		...
		Если ПринятьНаРаботу Тогда
			...
			СобытиеЖурналаРегистрации = НСтр("ka='განაკვეთების დამატება საშტატო განრიგში';ru='Добавление ставок в штатное расписание';en='Addition rates in staffing'");
			...
			СобытиеЖурналаРегистрации = НСтр("ka='სამუშაოზე ახალი თანამშრომლის მიღების ჩაწერა';ru='Запись приема на работу нового сотрудника';en='Record hiring a new employee'");
		...
		Если ОформитьУвольнение Тогда
			...
			СобытиеЖурналаРегистрации = НСтр("ka='თანამშრომლის გათავისუფლების ჩანაწერი';ru='Запись увольнения сотрудника';en='Recording employee layoffs'");
		...
		Если ИзменитьДолжность Тогда
			...
			СобытиеЖурналаРегистрации = НСтр("ka='ჩაწერა თანამშრომლის საკადრო გადაადგილება';ru='Запись кадровое перемещение сотрудника';en='Record recruitment the movement of staff'");
...
Функция ПриПереходеДалее(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ПриемНаРаботуДатаПриема) Тогда
		Если ПринятьНаРаботу Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია სამუშაოზე მიღების თარიღის შევსება.';ru='Необходимо заполнить дату приема на работу.';en='It is necessary to fill the date of employment.'");
		ИначеЕсли ИзменитьОклад Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია ხელფასის შეცვლის თარიღის შევსება.';ru='Необходимо заполнить дату изменения заработной платы.';en='It is necessary to fill the date of change of wages.'");
		ИначеЕсли ИзменитьДолжность Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია საკადრო გადაადგილების თარიღის შევსება.';ru='Необходимо заполнить дату кадрового перемещения.';en='It is necessary to fill the date personnel move.'");
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		...
		ТекстСообщения = НСтр("ka='აუცილებელია ორგანიზაციის შევსება.';ru='Необходимо заполнить организацию.';en='It is necessary to fill the organization.'");
	...
	Если ИспользуютсяПодразделения
		...
		ТекстСообщения = НСтр("ka='აუცილებელია ქვეგანყოფილების შევსება.';ru='Необходимо заполнить подразделение.';en='You must fill unit.'");
	...
	Если НЕ ЗначениеЗаполнено(ПриемНаРаботуДолжность) И Не ИзменитьОклад Тогда
		...
		ТекстСообщения = НСтр("ka='აუცილებელია თანამშრომლის თანამდებობის შევსება.';ru='Необходимо заполнить должность сотрудника.';en='It is necessary to fill the position of an employee.'");
	...
	Если ИспользуетсяШтатноеРасписание И  Не ИзменитьОклад Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПриемНаРаботуЗанимаемыхСтавок) Тогда
			...
			ТекстСообщения = НСтр("ka='აუცილებელია დაკავებული განაკვეთების რაოდენობის შევსება.';ru='Необходимо заполнить количество занимаемых ставок.';en='It is necessary to fill the number of occupied rates.'");
...
Процедура ВыполнитьКонтрольШтатногоРасписания(СтруктураДанных, Ошибки, Отказ)
	...
	ИначеЕсли НЕ ВыборкаКоличествоСтавок.Следующий() Тогда
		...
		ТекстСообщения = НСтр("ka='ორგანიზაცია %1-ის საშტატო განრგიში სტრუქტურულ ერთეულ %2-ში თანამდებობა %3-ისთვის განაკვეთები არ არის გათვალისწინებული!';ru='В штатном расписании организации %1 по структурной единице %2 не предусмотрены ставки для должности %3!';en='The staffing organization% 1 to 2% of the structural unit rates are not provided for the post% 3!'");
	...
	Иначе
		...
		Если СвободноСтавок < СтруктураДанных.ПланируетсяЗанятьСтавок Тогда
			...
			ТекстСообщения = НСтр("ka='ორგანიზაცია %1-ის საშტატო განრიგში სტრუქტურულ ერთეულ %2-ში თანამდებობა %3-ისთვის თავისუფალი განაკვეთების საკმარისი რაოდენობა არ არის!"
"თავისუფალია %4 განაკვეთი, ხოლო საჭიროა %5 განაკვეთი.';ru='В штатном расписании организации %1 по структурной единице %2 нет достаточного количества свободных ставок для должности %3!"
