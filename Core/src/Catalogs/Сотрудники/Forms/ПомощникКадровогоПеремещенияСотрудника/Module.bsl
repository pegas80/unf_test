...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ИзменитьОклад Тогда
		...
		Элементы.ДекорацияТри.Заголовок = НСтр("ka='ხელფასისი შეცვლა';ru='Изменение оклада'");
		Элементы.ПриемНаРаботуДатаПриема.Заголовок = НСтр("ka='ცვლილების თარიღი';ru='Дата изменения'");
	...
	ИначеЕсли ИзменитьДолжность Тогда
		...
		Элементы.ДекорацияТри.Заголовок = НСтр("ka='საკადრო გადაადგილება';ru='Кадровое перемещение'");
		Элементы.ПриемНаРаботуДатаПриема.Заголовок = НСтр("ka='გადაადგილების თარიღი';ru='Дата перемещения'");
		Элементы.ПриемНаРаботуОрганизация.ТолькоПросмотр = Истина;
...
Процедура СоздатьИтоговыйДокумент(Отказ)
	...
	Если ОформитьУвольнение Тогда
		Если Не ЗначениеЗаполнено(УвольнениеДатаУвольнения) Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია გათავისუფლების თარიღის შევსება.';ru='Необходимо заполнить дату увольнения.'");
	...
	Попытка
		...
		Если ПринятьНаРаботу Тогда
			...
			СобытиеЖурналаРегистрации = НСтр("ka='განაკვეთების დამატება საშტატო განრიგში';ru='Добавление ставок в штатное расписание'");
			...
			СобытиеЖурналаРегистрации = НСтр("ka='სამუშაოზე ახალი თანამშრომლის მიღების ჩაწერა';ru='Запись приема на работу нового сотрудника'");
		...
		Если ОформитьУвольнение Тогда
			...
			СобытиеЖурналаРегистрации = НСтр("ka='თანამშრომლის გათავისუფლების ჩანაწერი';ru='Запись увольнения сотрудника'");
		...
		Если ИзменитьДолжность Тогда
			...
			СобытиеЖурналаРегистрации = НСтр("ka='ჩაწერა თანამშრომლის საკადრო გადაადგილება';ru='Запись кадровое перемещение сотрудника'");
...
Функция ПриПереходеДалее(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ПриемНаРаботуДатаПриема) Тогда
		Если ПринятьНаРаботу Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია სამუშაოზე მიღების თარიღის შევსება.';ru='Необходимо заполнить дату приема на работу.'");
		ИначеЕсли ИзменитьОклад Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია ხელფასის შეცვლის თარიღის შევსება.';ru='Необходимо заполнить дату изменения заработной платы.'");
		ИначеЕсли ИзменитьДолжность Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია საკადრო გადაადგილების თარიღის შევსება.';ru='Необходимо заполнить дату кадрового перемещения.'");
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		...
		ТекстСообщения = НСтр("ka='აუცილებელია ორგანიზაციის შევსება.';ru='Необходимо заполнить организацию.'");
	...
	Если ИспользуютсяПодразделения
		...
		ТекстСообщения = НСтр("ka='აუცილებელია ქვეგანყოფილების შევსება.';ru='Необходимо заполнить подразделение.'");
	...
	Если НЕ ЗначениеЗаполнено(ПриемНаРаботуДолжность) И Не ИзменитьОклад Тогда
		...
		ТекстСообщения = НСтр("ka='აუცილებელია თანამშრომლის თანამდებობის შევსება.';ru='Необходимо заполнить должность сотрудника.'");
	...
	Если ИспользуетсяШтатноеРасписание И  Не ИзменитьОклад Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПриемНаРаботуЗанимаемыхСтавок) Тогда
			...
			ТекстСообщения = НСтр("ka='აუცილებელია დაკავებული განაკვეთების რაოდენობის შევსება.';ru='Необходимо заполнить количество занимаемых ставок.'");
...
Процедура ВыполнитьКонтрольШтатногоРасписания(СтруктураДанных, Ошибки, Отказ)
	...
	ИначеЕсли НЕ ВыборкаКоличествоСтавок.Следующий() Тогда
		...
		ТекстСообщения = НСтр("ka='ორგანიზაცია %1-ის საშტატო განრგიში სტრუქტურულ ერთეულ %2-ში თანამდებობა %3-ისთვის განაკვეთები არ არის გათვალისწინებული!';ru='В штатном расписании организации %1 по структурной единице %2 не предусмотрены ставки для должности %3!'");
