...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТрудовойДоговорМикропредприятий") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='Трудовой договор (микропредприятий)';ru='Трудовой договор (микропредприятий)'"),
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='Трудовой договор (микропредприятий)';ru='Трудовой договор (микропредприятий)'");
...
Функция ДанныеТрудовыхДоговоров(МассивОбъектов, ДистанционныеРаботы, ДоговорСНадомником) Экспорт
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Сотрудник %1 не принят на работу';ru='Сотрудник %1 не принят на работу'"),
			Выборка.Сотрудник);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ДатаЗавершенияТрудовогоДоговора) Тогда
			...
			ДанныеДоговора.СрочностьТрудовогоДоговора = НСтр("ka='срочный трудовой договор';ru='срочный трудовой договор'");
		...
		//		ДанныеДоговора.ВидЗанятости = НСтр("ka='по основной работе';ru='по основной работе'");
		//	Иначе
		//		ДанныеДоговора.ВидЗанятости = НСтр("ka='по совместительству';ru='по совместительству'");
		//	КонецЕсли;
		//	
		//КонецЕсли;
		...
		Если ЗначениеЗаполнено(Выборка.ДлительностьРабочейНедели) Тогда
			...
			ДанныеДоговора.ДлительностьРабочейНедели =
				ЧислоПрописью(Выборка.ДлительностьРабочейНедели, "Л = ru_RU", НСтр("ka='час,часа,часов,м,,,,,0';ru='час,часа,часов,м,,,,,0'"))
				+ " " + НСтр("ka='в неделю';ru='в неделю'");
		...
		Если ЗначениеЗаполнено(Выборка.ОписаниеДлительностиРабочейНеделиВДнях)
			...
			Если Выборка.ОписаниеДлительностиРабочейНеделиВДнях = 5 Тогда
				ДанныеДоговора.ОписаниеДлительностиРабочейНеделиВДнях = НСтр("ka='пятидневная с двумя выходными днями';ru='пятидневная с двумя выходными днями'");
			ИначеЕсли Выборка.ОписаниеДлительностиРабочейНеделиВДнях = 6 Тогда
				ДанныеДоговора.ОписаниеДлительностиРабочейНеделиВДнях = НСтр("ka='шестидневная с одним выходными днем';ru='шестидневная с одним выходными днем'");
			ИначеЕсли Выборка.СокращеннаяРабочаяНеделя = Истина Тогда
				ДанныеДоговора.ОписаниеДлительностиРабочейНеделиВДнях = НСтр("ka='неполная рабочая неделя';ru='неполная рабочая неделя'");
			ИначеЕсли Выборка.НеполноеРабочееВремя = Истина Тогда
				ДанныеДоговора.ОписаниеДлительностиРабочейНеделиВДнях = НСтр("ka='сокращенное рабочее время';ru='сокращенное рабочее время'");
			Иначе
				ДанныеДоговора.ОписаниеДлительностиРабочейНеделиВДнях = НСтр("ka='рабочая неделя с предоставлением выходных дней по скользящему графику';ru='рабочая неделя с предоставлением выходных дней по скользящему графику'");
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Выборка.ДлительностьИспытательногоСрока) Тогда
			...
			ДанныеДоговора.ИспытательныйСрок = НСтр("ka='устанавливается';ru='устанавливается'");
...
Функция ПараметрыТрудовогоДоговора(ДистанционныеРаботы = Ложь, ДоговорСНадомником = Ложь)
	...
	ПараметрыДоговора.Вставить("СрочностьТрудовогоДоговора", НСтр("ka='трудовой договор на неопределенный срок';ru='трудовой договор на неопределенный срок'"));
	ПараметрыДоговора.Вставить("ДатаЗавершенияТрудовогоДоговора");
	ПараметрыДоговора.Вставить("ИспытательныйСрок", НСтр("ka='не устанавливается';ru='не устанавливается'"));
	ПараметрыДоговора.Вставить("ИспытательныйСрокМесяцев");
	ПараметрыДоговора.Вставить("ВидЗанятости");
	ПараметрыДоговора.Вставить("МестоОпределенияДолжностныхОбязанностей", НСтр("ka='в должностной инструкции';ru='в должностной инструкции'"));
	...
	ПараметрыДоговора.Вставить("ДлительностьРабочейНедели", НСтр("ka='40 часов в неделю';ru='40 часов в неделю'"));
	ПараметрыДоговора.Вставить("ОписаниеДлительностиРабочейНеделиВДнях", НСтр("ka='пятидневная с двумя выходными днями';ru='пятидневная с двумя выходными днями'"));
	...
	ПараметрыДоговора.Вставить("ОснованиеРуководителяВРодительномПадеже", НСтр("ka='устава';ru='устава'"));
...
Процедура СоздатьВТКадровыеДанныеТрудовогоДоговора(МенеджерВременныхТаблиц, ИмяВременнойТаблицыОтборовСотрудников, ИменаПолейВременнойТаблицыОтборовСотрудников) Экспорт
	...
	Запрос.УстановитьПараметр("ОснованиеПредставителя", НСтр("ka='Устава';ru='Устава'"));
