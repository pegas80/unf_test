...
Процедура ПараметрНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Параметр не может быть изменен.';en='The parameter can not be changed.'")
			+ Символы.ПС
			+ НСтр("ru='Для выбора параметра вручную необходимо записать типовую операцию';en='To select an option, you must manually record the typical operation'");
