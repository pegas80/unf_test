	...
	Иначе
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ka='ხელშეკრულების ფორმის გახსნა';ru='открытие формы соглашения';en='opening form of agreement'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Процедура ПолучательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторКонтрагента) Тогда
		Если ВыбранноеЗначение <> Объект.Контрагент Тогда
			ТекстВопроса = НСтр("ka='შეიცვალა ორგანიზაცია მიმღები. შევცვალოთ მიმღების იდენტიფიკატორი?';ru='Была изменена организация получатель. Изменить идентификатор получателя?';en='The recipient organization has been changed. Change the identifier of the recipient?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		КонецЕсли;
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, КодВозвратаДиалога.Да);
...
Процедура ПеречитатьДанныеПоСертификатам(ДокументОбъект)
	...
	Для Каждого ЭлементСтрока Из ДокументОбъект.СертификатыПодписейКонтрагента Цикл
		...
		Если ДвоичныеДанныеСертификата <> Неопределено Тогда
			...
			Исключение
				Операция = НСтр("ka='სერთიფიკატის მონაცემების წაკითხვა';ru='Чтение данных сертификата';en='Reading the certificate data'");
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%5."
"';ka='ელექტრონული დოკუმენტების %1 %2 ტიპის მიხედვით უკვე არსებობს მოქმედი შეთანხმება მონაწილეებს შორის %3 - %4: %5.'");
...
Процедура ПроверитьЗаписатьСоглашение(ОбработчикПродолжения)
	...
	Если НЕ Объект.УдалитьЭтоТиповое Тогда
		ТекстВопроса = НСтр("ka='გარე სერთიფიკატების არჩევა შეიძლება მხოლოდ ჩაწერილ ხელშეკრულებაში. ჩავწეროთ ხელშეკრულება?';ru='Внешние сертификаты можно выбирать только в записанном соглашении."
"Записать соглашение?'");
...
Процедура ИзменитьНадписьНаправлениеДокументов()
	...
	ШаблонСообщения = НСтр("ka='ვისგან: %1, ვის: %2';ru='От кого: %1, кому: %2';en='From:% 1 To:% 2'");
...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	ТекстЗаголовкаСоздание = НСтр("ka='ორგანიზაციებს შორის ედბ-ის გასწორება (შექმნა)';ru='Настройка ЭДО между организациями (создание)';en='Setting up the EDI between organizations (creation)'");
	ТекстЗаголовка = НСтр("ka='%1 (Настройка ЭДО между организациями)';ru='%1 (Настройка ЭДО между организациями)';en='% 1 (Configure EDI between organizations)'");
