...
Процедура ШифроватьДанныеПакетаЭДПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СертификатОрганизацииДляРасшифровки)
		...
		ТекстВопроса = НСтр("ka='დაშიფრვის გამართვები გასუფთავდება. გავაგრძელოთ?';ru='Настройки шифрования будут очищены. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ПроверятьСертификатыПодписейПриИзменении(Элемент)
	...
	Если Объект.СертификатыПодписейКонтрагента.Количество() <> 0 Тогда
		ТекстВопроса = НСтр("ka='გამართვები სერთიფიკატების შემოწმება კონტრაგენტების ხელმოწერები გასუფთავდება. გავაგრძელოთ?';ru='Настройки проверка сертификатов подписи контрагентов будут очищены. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ДоговорКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Исключение
		...
		ВидОперации = НСтр("ka='ხელშეკრულების არჩევის ფორმის გახსნა';ru='Открытие формы выбора договора'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='ხელშეკრულების არჩევის ფორმის გახსნის შეცდომა';ru='Ошибка открытия формы выбора договора'");
...
Процедура ОтправитьПриглашение(Команда)
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ka='ედბ-ის მიმდინარე გამართვაში შეტანილია ცვლილებები. ჩავწეროთ?';ru='В текущую настройку ЭДО внесены изменения. Записать?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ТестНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='ედბ-ის მიმდინარე გამართვაში შეტანილია ცვლილებები. გავაგრძელოთ ტესტის შესრულება?';ru='В текущую настройку ЭДО внесены изменения. Продолжить выполнение теста?'");
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		Если Не Объект.ИспользоватьУПД Тогда
			...
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Счет-фактура)';ru='ФНС 5.01 (УПД:Счет-фактура)'"), НСтр("ka='ФНС 5.01 (УПД:Счет-фактура)';ru='ФНС 5.01 (УПД:Счет-фактура)'"));
		КонецЕсли;
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.КорректировочныйСчетФактура") Тогда
		...
		КонецЕсли;
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08'"), НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)'"));
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01';ru='ФНС 5.01'"), НСтр("ka='ФНС 5.01';ru='ФНС 5.01'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Заголовок = НСтр("ka='ედბ-ის გამართვების გაფართოებული რეჟიმის ჩართვა';ru='Включить расширенный режим настроек ЭДО'");
	Если Объект.РасширенныйРежимНастройкиСоглашения Тогда
		Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Заголовок = НСтр("ka='ედბ-ის გამართვების გაფართოებული რეჟიმის გამორთვა';ru='Выключить расширенный режим настроек ЭДО'");
	КонецЕсли;
...
Процедура ОбновитьВходящийКаталога(Форма)
	...
	ШаблонЗаголовка = НСтр("ka='სრული გზა: %1';ru='Полный путь: %1'");
...
Процедура ОпределитьИспользуемыеСпособыОбменаЭД(ПрофильНастроекЭДО = Неопределено, ИдентификаторКонтрагента = Неопределено)
	...
	Для каждого СтрокаТаблицы Из Объект.ИсходящиеДокументы Цикл
		...
		Если СтрокаТаблицы.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
			...
			ШаблонНаименование = НСтр("ka='%1_%2';ru='%1_%2'");
...
#Если НЕ ВебКлиент Тогда
	...
	ДиалогКаталога.Заголовок = НСтр("ka='მიუთითეთ კატალოგი ედ-ის გასაცვლელად';ru='Укажите каталог для обмена ЭД'");
...
Процедура ДобавитьДанныеПоТабЧасти(ВыбранноеЗначение, АдресВХранилище = Неопределено)
	...
	Если АдресВХранилище <> Неопределено Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ka='სერთიფიკატის ფაილი უნდა იყოს შემდეგ ფორმატში DER X.509,  ოპერაცია შეწყვეტილია.';ru='Файл сертификата должен быть в формате DER X.509, операция прервана.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецПопытки;
...
Процедура ТестНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ЧерезОператораЭДО Тогда
		Состояние(НСтр("ka='აწყობეების ტესტი.';ru='Тест настроек.'"),
			,
			НСтр("ka='ედბ-ის სერვისით მიმდინარეობს ედ-ის გაცვლის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование обмена ЭД через сервис ЭДО. Подождите...'"));
	КонецЕсли;
