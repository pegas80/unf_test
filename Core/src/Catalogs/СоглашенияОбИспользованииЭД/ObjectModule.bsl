...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоИнтеркампани
		...
		Если Не ЗначениеЗаполнено(Контрагент) Тогда
			НазваниеПоля = НСтр("ka='კონტრაგენტი';ru='Контрагент';en='Counterparty'");
		...
		Если РасширенныйРежимНастройкиСоглашения Тогда
			...
			Для каждого СтрокаТаблицы Из МассивСтрокЭлектронныхДокументов Цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаТаблицы.ПрофильНастроекЭДО) Тогда
					НазваниеПоля = НСтр("ka='ედბ-ის გამართვების პროფილი';ru='Профиль настроек ЭДО';en='EDI profile settings'");
					НазваниеТабличнойЧасти = НСтр("ka='გამავალი ელექტრონული დოკუმენტები';ru='Исходящие электронные документы';en='Outgoing electronic documents'");
				...
				Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВерсияФормата)
					...
					НазваниеПоля = НСтр("ka='Формат электронного документа';ru='Формат электронного документа';en='The format of the electronic document'");
					НазваниеТабличнойЧасти = НСтр("ka='გამავალი ელექტრონული დოკუმენტები';ru='Исходящие электронные документы';en='Outgoing electronic documents'");
					ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
						"Колонка", "ЗАПОЛНЕНИЕ", НазваниеПоля, СтрокаТаблицы.НомерСтроки, НазваниеТабличнойЧасти);
				...
				Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СтрокаТаблицы.СпособОбменаЭД)
					...
					НазваниеПоля = НСтр("ka='კონტრაგენტის იდენტიფიკატორი';ru='Идентификатор контрагента';en='counterparty identifier'");
					НазваниеТабличнойЧасти = НСтр("ka='გამავალი ელექტრონული დოკუმენტები';ru='Исходящие электронные документы';en='Outgoing electronic documents'");
					ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
						"Колонка", "ЗАПОЛНЕНИЕ", НазваниеПоля, СтрокаТаблицы.НомерСтроки, НазваниеТабличнойЧасти);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
	...
	Если ЭтоИнтеркампани И СтатусСоглашения = Перечисления.СтатусыСоглашенийЭД.Действует Тогда
		...
		Если НЕ ЗначениеЗаполнено(Организация) Тогда
			НазваниеПоля = НСтр("ka='გამგზავნი-ორგანიზაცია';ru='Организация-отправитель';en='The Organization of the sender'");
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НазваниеПоля);
		...
		Если НЕ ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			НазваниеПоля = НСтр("ka='გამგზავნის იდენტიფიკატორი';ru='Идентификатор отправителя';en='sender ID'");
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НазваниеПоля);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ИдентификаторОрганизации", , Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
			НазваниеПоля = НСтр("ka='ორგანიზაცია-მიმღები';ru='Организация-получатель';en='Recipient Organization'");
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НазваниеПоля);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "Контрагент", , Отказ);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ИдентификаторКонтрагента) Тогда
			НазваниеПоля = НСтр("ka='მიმღების იდენტიფიკატორი';ru='Идентификатор получателя';en='identifier of the recipient'");
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НазваниеПоля);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ИдентификаторКонтрагента", , Отказ);
		КонецЕсли;
...
"между контрагентом %1 и организацией %2'");
			...
			Если ЗначениеЗаполнено(Выборка.ДоговорКонтрагента) Тогда
				Шаблон = НСтр("ka='%1 ხელშეკრულება %2-ის მიხედვით';ru='%1 по договору %2';en='% 1 to% 2 agreement'");
