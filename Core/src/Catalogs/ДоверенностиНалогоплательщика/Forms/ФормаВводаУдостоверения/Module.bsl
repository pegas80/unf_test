...
Функция СохранениеВозможно()
	...
	Если НЕ ЗначениеЗаполнено(ДокументВид) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტის ტიპი არ არის მიცემული.';ru='Не задан вид документа.'"), , "ДокументВид", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(СокрЛП(ДокументСерия) + СокрЛП(ДокументНомер)) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტის სერია და ნომერი არ არის მიცემული.';ru='Не заданы серия и номер документа.'"), , "ДокументСерия", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДокументДатаВыдачи) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტის გაცემის თარიღი არ არის მიცემული.';ru='Не задана дата выдачи документа.'"), , "ДокументДатаВыдачи", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДокументКемВыдан) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტის გამცემი ორგანოს სახელწოდება არ არის მიცემული.';ru='Не задано наименование органа, выдавшего документ.'"), , "ДокументКемВыдан", , Отказ);
	КонецЕсли;
