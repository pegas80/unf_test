...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='ბეჭდვა';ru='Печать'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для каждого СтрокаМассиваОбъектов Из МассивОбъектов Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ДоверенностьНалогоплательщика", НСтр("ka='გადამხდელის მინდობილობა';ru='Доверенность налогоплательщика'"), СформироватьДоверенность(СтрокаМассиваОбъектов));
	КонецЦикла; 	
...
Функция СформироватьДоверенность(Знач Доверенность) Экспорт
	...
	Если Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 36) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='სამი წლის ვადით.';ru='сроком на три года.'");
	ИначеЕсли Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 24) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='ორი წლის ვადით.';ru='сроком на два года.'");
	ИначеЕсли Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 12) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='сроком на год.';ru='сроком на год.'");
	ИначеЕсли Не ЗначениеЗаполнено(Доверенность.ДатаОкончания) Тогда 
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='.';ru='.'");
	Иначе
		СрокВыдачи.Параметры.СрокВыдачи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='сроком на период с %1 г. по %2 г.';ru='сроком на период с %1 г. по %2 г.'"),
