...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='ბეჭდვა';ru='Печать';en='Print'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для каждого СтрокаМассиваОбъектов Из МассивОбъектов Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ДоверенностьНалогоплательщика", НСтр("ka='გადამხდელის მინდობილობა';ru='Доверенность налогоплательщика';en='taxpayer attorney'"), СформироватьДоверенность(СтрокаМассиваОбъектов));
	КонецЦикла; 	
...
Функция СформироватьДоверенность(Знач Доверенность) Экспорт
	...
	Шапка.Параметры.ДатаВыдачи = Формат(Доверенность.ДатаВыдачи, "ДФ='«dd» MMMM yyyy'")+НСтр("ka='წლები.';ru=' года.'");
	...
	Если Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 36) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='სამი წლის ვადით.';ru='сроком на три года.';en='period of three years.'");
	ИначеЕсли Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 24) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='ორი წლის ვადით.';ru='сроком на два года.';en='for two years.'");
	ИначеЕсли Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 12) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='сроком на год.';ru='сроком на год.';en='for one year.'");
	ИначеЕсли Не ЗначениеЗаполнено(Доверенность.ДатаОкончания) Тогда 
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ka='.';ru='.';en='.'");
	Иначе
		СрокВыдачи.Параметры.СрокВыдачи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='сроком на период с %1 г. по %2 г.';ru='сроком на период с %1 г. по %2 г.';en='for a period of between 1% till 2% of'"),
	...
	Если ЗначениеЗаполнено(УдЛичнПредстФЛ) Тогда
		Подвал.Параметры.ДанныеУдостоверения_Представитель = "серия "+УдЛичнПредстФЛ.СерДок+", № " +УдЛичнПредстФЛ.НомДок+", выдан "+ УдЛичнПредстФЛ.ВыдДок + " "+Формат(УдЛичнПредстФЛ.ДатаДок, "ДФ='«dd» MMMM yyyy'")+НСтр("ka='წლები.';ru=' года.'");
	КонецЕсли;
