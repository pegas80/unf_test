...
Процедура ПутьКДискуИТСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораКаталога.Заголовок = НСтр("ka='მიუთითეთ გზა სტთ-ის დისკისკენ';ru='Укажите путь к диску ИТС';en='Enter the path to the ETS drive'");
	...
	Если НЕ ФайлДанных.Существует() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აღნიშნულ კატალოგში კლასიფიკატორის მონაცემები არ არის აღმოჩენილი. აუცილებელია 1С:სტთ-ის დისკისკენ გზის მითითება, რომელზეც მდებარეობს ""გარანტი. გადასახადები, საბუღ. აღრიცხვა, მეწარმეობა"".';ru='В указанном каталоге не обнаружены данные классификатора. Необходимо указать путь к диску 1С:ИТС, на котором содержится база ""Гарант. Налоги, бухучет, предпринимательство.""';en='In the specified directory data classifier not found. You must specify the path to the disk 1C: ITS, which contains the base ""Guarantor Tax, accounting, entrepreneurship..""'"),
			,
			"ПутьКДискуИТС");
	КонецЕсли;
	...
	Иначе
		...
		Если ВариантЗагрузки = "ИТС" Тогда
			Если НЕ ЗначениеЗаполнено(ПутьКДискуИТС) И Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='При работе не в ОС Windows необходимо явно указать путь к диску';ru='При работе не в ОС Windows необходимо явно указать путь к диску';en='When the work is not in Windows, you must explicitly specify the path to the drive'"),
					,
					"ПутьКДискуИТС");
				Возврат;
			КонецЕсли;
...
"Подключиться сейчас?';ka='Для загрузки классификатора банков из Интернета"
"необходимо подключиться к Интернет-поддержке пользователей."
"Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='მიერთება';ru='Подключиться';en='connect'"));
...
Процедура ПриЗавершенииОперацииЗагрузки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა';ru='Загрузка классификатора банков';en='Loading classifier banks'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Элементы.ПоясняющийТекст.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა შეწყდა შემდეგი მიზეზით:"
"%1"
"დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Загрузка классификатора банков прервана по причине:"
"%1"
...
Процедура ЗагрузитьРезультат(АдресХранилища)
	...
	Если ВариантЗагрузки = "ИТС" Тогда
		Источник = НСтр("ka='საინფორმაციო ტექნოლოგიური თანხლების დისკი';ru='Диск ИТС';en='ITS disk'");
	Иначе
		Источник = НСтр("ka='1С';ru='1С';en='1C'");
	КонецЕсли;
	
	ПараметрыЗагрузкиКлассификатора = ПолучитьИзВременногоХранилища(АдресХранилища);
	
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка классификатора банков.%1';en='Loading classifier banks.% 1'"), Источник);
...
// Параметры:
	...
	Если ФайлНайден Тогда
		...
		ФайлДанных = Неопределено;
	Иначе
		ТекстСообщения = НСтр("ka='1С:სტთ-ის დისკზე ბსკ-ის კლასიფიკატორის მონაცემები არ არის აღმოჩენილი. "
"დასაყენებლად საჭიროა 1С:სტთ-ის დისკი, რომელშიც არის ბაზა ""გარანტი. გადასახადები, საბუღალტრო აღრიცხვა, მეწარმეობა.""';ru='На диске 1С:ИТС не обнаружены данные классификатора БИК РФ. "
...
// Параметры:
	...
	Иначе
		ПараметрыЗагрузкиФайловИТС.Вставить("ТекстСообщения", НСтр("ka='სტთ-ის დისკზე ბსკ-ის კლასიფიკატორის დამუშავების მომზადების ფაილი ვერ მოიძებნა.';ru='На диске ИТС не обнаружен файл обработки подготовки данных классификатора БИК РФ.';en='ITS on the disk is detected processing the file of training data classifier BIC Code.'"));
	КонецЕсли;
...
Процедура УстановитьИзмененияВИнтерфейсе()
	...
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаВыборИсточника
		...
		Элементы.ФормаКнопкаДалее.Заголовок = НСтр("ka='ჩატვირთვა';ru='Загрузить';en='Download'");
	...
	Иначе
		...
		Элементы.ФормаКнопкаДалее.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
...
Функция ПолучитьДанныеРБКНаСервере(ПараметрыЗагрузкиФайловРБК)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='საქართველოს ბანკების კლასიფიკატორის ჩატვირთვა';ru='Загрузка классификатора банков РФ';en='Loading classifier Russian banks'");
...
Функция ТекстЗагрузкаЗапрещена()
	Возврат НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა განცალკევებულ რეჟიმში აკრძალულია.';ru='Загрузка классификатора банков в разделенном режиме запрещена.';en='Loading classifier banks in split mode is prohibited.'");
КонецФункции
