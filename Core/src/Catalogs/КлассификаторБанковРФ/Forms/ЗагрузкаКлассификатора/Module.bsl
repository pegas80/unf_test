...
Процедура ПутьКДискуИТСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораКаталога.Заголовок = НСтр("ka='მიუთითეთ გზა სტთ-ის დისკისკენ';ru='Укажите путь к диску ИТС'");
	...
	Если НЕ ФайлДанных.Существует() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აღნიშნულ კატალოგში კლასიფიკატორის მონაცემები არ არის აღმოჩენილი. აუცილებელია 1С:სტთ-ის დისკისკენ გზის მითითება, რომელზეც მდებარეობს ""გარანტი. გადასახადები, საბუღ. აღრიცხვა, მეწარმეობა"".';ru='В указанном каталоге не обнаружены данные классификатора. Необходимо указать путь к диску 1С:ИТС, на котором содержится база ""Гарант. Налоги, бухучет, предпринимательство.""'"),
			,
			"ПутьКДискуИТС");
	КонецЕсли;
	...
	Иначе
		...
		Если ВариантЗагрузки = "ИТС" Тогда
			Если НЕ ЗначениеЗаполнено(ПутьКДискуИТС) И Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='При работе не в ОС Windows необходимо явно указать путь к диску';ru='При работе не в ОС Windows необходимо явно указать путь к диску'"),
					,
					"ПутьКДискуИТС");
				Возврат;
			КонецЕсли;
...
"Подключиться сейчас?'");
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='მიერთება';ru='Подключиться'"));
...
Процедура ПриЗавершенииОперацииЗагрузки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა';ru='Загрузка классификатора банков'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьРезультат(АдресХранилища)
	...
	Если ВариантЗагрузки = "ИТС" Тогда
		Источник = НСтр("ka='საინფორმაციო ტექნოლოგიური თანხლების დისკი';ru='Диск ИТС'");
	Иначе
		Источник = НСтр("ka='1С';ru='1С'");
	КонецЕсли;
...
// Параметры:
	...
	Иначе
		ПараметрыЗагрузкиФайловИТС.Вставить("ТекстСообщения", НСтр("ka='სტთ-ის დისკზე ბსკ-ის კლასიფიკატორის დამუშავების მომზადების ფაილი ვერ მოიძებნა.';ru='На диске ИТС не обнаружен файл обработки подготовки данных классификатора БИК РФ.'"));
	КонецЕсли;
...
Процедура УстановитьИзмененияВИнтерфейсе()
	...
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаВыборИсточника
		...
		Элементы.ФормаКнопкаДалее.Заголовок = НСтр("ka='ჩატვირთვა';ru='Загрузить'");
	...
	Иначе
		...
		Элементы.ФормаКнопкаДалее.Заголовок = НСтр("ka='დახურვა';ru='Закрыть'");
...
Функция ПолучитьДанныеРБКНаСервере(ПараметрыЗагрузкиФайловРБК)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='საქართველოს ბანკების კლასიფიკატორის ჩატვირთვა';ru='Загрузка классификатора банков РФ'");
...
Функция ТекстЗагрузкаЗапрещена()
	Возврат НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა განცალკევებულ რეჟიმში აკრძალულია.';ru='Загрузка классификатора банков в разделенном режиме запрещена.'");
КонецФункции
