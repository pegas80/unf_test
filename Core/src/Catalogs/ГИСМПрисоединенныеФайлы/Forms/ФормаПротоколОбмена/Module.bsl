...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ВызватьИсключение НСтр("ka='Протокол обмена может быть открыт только в контексте документа.';ru='Протокол обмена может быть открыт только в контексте документа.';en='The communications protocol can be opened only in the context of the document.'");
	КонецЕсли;
...
Процедура ДеревоФайловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.Файл) Тогда
		...
		ПараметрыФормы.Вставить("Заголовок", СтрШаблон(НСтр("ka='Сообщения операции: %1';ru='Сообщения операции: %1';en='Operation Messages: 1%'"), ДанныеСтроки.Операция));
...
Процедура ПоказатьСообщенияXML(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ka='По данной операции еще не создано сообщений';ru='По данной операции еще не создано сообщений';en='Of the transaction has not created posts'"));
...
Процедура ПодтвердитьПоступление_ПослеПолученияДокументовКПодтверждению(ДокументыКПодтверждению, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='В форме протокола обмена подтвержение получения по нескольким документам не поддерживается.';ru='В форме протокола обмена подтвержение получения по нескольким документам не поддерживается.';en='In the form of a protocol exchange confirmed receipt of multiple documents is not supported.'");
...
#КонецОбласти
	...
	ПредставлениеЭлемента = НСтр("ka='Представление даты ""<не передано>""';ru='Представление даты ""<не передано>""';en='Submission date ""<not transmitted>""'");
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<не передано>';ru='<не передано>';en='<Not transferred>'"));
...
Функция ПредставлениеОперации(Операция, ДокументСсылка, ВыборкаПоФайлам = Неопределено)
	...
	Если Операция = Перечисления.ОперацииОбменаГИСМ.ПередачаДанных Тогда
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.МаркировкаТоваровГИСМ") Тогда
			...
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат НСтр("ka='Передача сведений о маркированной продукции';ru='Передача сведений о маркированной продукции';en='Transmission of information on labeled products'");
			Иначе
				Возврат СтрШаблон(НСтр("ka='Передача сведений о маркированной продукции, Версия %1';ru='Передача сведений о маркированной продукции, Версия %1';en='Transmission of information on labeled products, version% 1'"), ВыборкаПоФайлам.Версия);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ") Тогда
			
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат НСтр("ka='Передача уведомления об отгрузке маркированной продукции';ru='Передача уведомления об отгрузке маркированной продукции';en='Transmission of the notification of the shipment labeled products'");
			Иначе
				Возврат СтрШаблон(НСтр("ka='Передача уведомления об отгрузке маркированной продукции, Версия %1';ru='Передача уведомления об отгрузке маркированной продукции, Версия %1';en='Transmission of the notification of the shipment labeled products, version% 1'"), ВыборкаПоФайлам.Версия);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаявкаНаВыпускКиЗГИСМ") Тогда
			
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат НСтр("ka='Передача заявки на выпуск КиЗ';ru='Передача заявки на выпуск КиЗ';en='Transfer requests for the release of Keys'");
			Иначе
				Возврат СтрШаблон(НСтр("ka='Передача заявки на выпуск КиЗ, Версия %1';ru='Передача заявки на выпуск КиЗ, Версия %1';en='Transfer application to the production of Keyes, Version% 1'"), ВыборкаПоФайлам.Версия);
			КонецЕсли;
		...
		Иначе
			...
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат СтрШаблон(НСтр("ka='Передача документа ""%1""';ru='Передача документа ""%1""';en='Transfer document ""%1""'"), ПредставлениеОбъекта);
			Иначе
				Возврат СтрШаблон(НСтр("ka='Передача документа ""%1"", Версия %2';ru='Передача документа ""%1"", Версия %2';en='Transfer document ""%1""% 2 Version'"), ПредставлениеОбъекта, ВыборкаПоФайлам.Версия);
			КонецЕсли;
	...
	ИначеЕсли Операция = Перечисления.ОперацииОбменаГИСМ.ПолучениеТоваров Тогда
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОПоступленииМаркированныхТоваровГИСМ") Тогда
			Возврат НСтр("ka='შეტყობინება მარკირებული საქონლების შემოსვლის შესახებ';ru='Уведомление о поступлении маркированных товаров';en='Notification to the marked goods'");
		КонецЕсли;
		
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаявкаНаВыпускКиЗГИСМ") Тогда
			Возврат НСтр("ka='Уведомление о выпуске КиЗ эмитентом';ru='Уведомление о выпуске КиЗ эмитентом';en='Notification of release of Keys Issuer'");
		КонецЕсли;
		
	ИначеЕсли Операция = Перечисления.ОперацииОбменаГИСМ.ПолучениеНовогоСтатуса Тогда
		
		Возврат СтрШаблон(НСтр("ka='Обновлено состояние: %1';ru='Обновлено состояние: %1';en='Updated state: 1%'"), ВыборкаПоФайлам.СтатусОбработкиЭмитентом);
...
Процедура ОбработатьПередачуДанных(ВыборкаПоФайлам, СтрокаВторогоУровня, ПередачаДанных)
	...
	Если ИнтеграцияГИСМ.ОперацииПередачиДанных().Найти(ВыборкаПоФайлам.Операция) <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.ОтветНаЗапросКвитанции) Тогда
				...
				Если СтрокаТЧ.ОтветНаЗапросСостояниеОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Принято Тогда
					...
					Если СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.Принято Тогда
						...
						СтрокаВторогоУровня.Состояние = НСтр("ka='Квитанция получена: Документ принят в обработку';ru='Квитанция получена: Документ принят в обработку';en='Receipt received: The document was adopted in the processing'");
						
					ИначеЕсли СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.Отклонено Тогда
						
						СтрокаВторогоУровня.Состояние = НСтр("ka='Квитанция получена: Документ отклонен';ru='Квитанция получена: Документ отклонен';en='Receipt received: Document rejected'");
					...
					ИначеЕсли СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.Обрабатывается Тогда
						...
						СтрокаВторогоУровня.Состояние = НСтр("ka='Ожидается получение квитанции';ru='Ожидается получение квитанции';en='It is expected to receive a receipt'");
					...
					ИначеЕсли СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.НаМодерации Тогда
						...
						СтрокаВторогоУровня.Состояние = НСтр("ka='Ожидается утвержение в ФНС';ru='Ожидается утвержение в ФНС';en='Anticipated authorized in the Federal Tax Service'");
						СтрокаВторогоУровня.ИндексКартинки = 3;
						
					Иначе
						
						СтрокаВторогоУровня.Состояние = НСтр("ka='დამუშავების შეცდომა';ru='Ошибка обработки';en='error processing'");
						СтрокаВторогоУровня.УсловноеОформление = "УсловноеОформлениеОшибка";
				...
				ИначеЕсли СтрокаТЧ.ОтветНаЗапросСостояниеОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Отклонено Тогда
					...
					СтрокаВторогоУровня.Состояние = НСтр("ka='დოკუმენტი უარყოფილია';ru='Документ отклонен';en='document is rejected'");
					СтрокаВторогоУровня.УсловноеОформление = "УсловноеОформлениеОшибка";
					СтрокаВторогоУровня.ИндексКартинки = 4;
		...
		ИначеЕсли ЗначениеЗаполнено(ВыборкаПоФайлам.Ссылка) Тогда
			...
			СтрокаВторогоУровня.Состояние = НСтр("ka='Ожидается передача данных';ru='Ожидается передача данных';en='Expected data transfer'");
			СтрокаВторогоУровня.ИндексКартинки = 3;
