...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ka='ახალი ამბების ამ ლენტის მიხედვით შერჩევების გამართვა (მოქმედებს მონაცემთა ყველა არეალზე):';ru='Настройки отборов по этой ленте новостей (действуют на все области данных):';en='Setting selections for this newswire (act on all of the data area):'");
	Иначе
		Если (ПравоДоступа("Изменение", Метаданные.Справочники.ЛентыНовостей) = Истина) Тогда
			ЭтаФорма.ТолькоПросмотр = Ложь;
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ka='ახალი ამბების ამ ლენტის მიხედვით შერჩევების გამართვა:';ru='Настройки отборов по этой ленте новостей:';en='setting selections for this newswire:'");
	КонецЕсли;
	...
	Иначе
		...
		Если Объект.ЗагруженоССервера = Ложь Тогда
			...
			Для Каждого ТекущийРедактор Из МассивРедакторовЛентыНовостей Цикл
				...
				НоваяСтрока.ПользовательПредставление = СтрЗаменить(НоваяСтрока.ПользовательПредставление, НСтр("ka='(სრული უფლებები)';ru='(полные права)';en='(Full access)'"), НСтр("ka='(სრული უფლებები, ახალი ამბების რედაქტირება შეუძლია ახალი ამბების ყველა ლენტში)';ru='(полные права, может редактировать новости во всех лентах новостей)';en='(Full rights can edit news in all news agencies)'"));
			КонецЦикла;
	...
	Если (Объект.ЗагруженоССервера = Истина) Тогда
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок = НСтр("ka='ახალი ამბების ფაილის მიღება (ამ ველების გადავსება ავტომატურად ხდება, 1С-ის სერვერზე ცვლილებების შემთხვევაში)';ru='Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)';en='The news file (this field refill automatically when changes to the server 1C)'");
		КонецЕсли;
...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВРег(Объект.Протокол) = ВРег("file") Тогда
		...
		Диалог.Заголовок = НСтр("ka='აირჩიეთ ახალი ამბების ფაილი Atom-ის ან RSS-ის ფორმატში';ru='Выберите файл новостей в формате Atom или RSS';en='Select a news file to the Atom or RSS format'");
...
Процедура ДоступныеКатегорииНовостейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ka='სერვერიდან ჩატვირთული ახალი ამბების ლენტისთვის კატეგორიების დამატება არ შეიძლება.';ru='Нельзя добавлять категории для ленты новостей, загруженной с сервера.';en='You can not add categories to news feeds loaded from the server.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='სერვერიდან ჩატვირთული ახალი ამბების ლენტისთვის კატეგორიების წაშლა არ შეიძლება.';ru='Нельзя удалять категории для ленты новостей, загруженной с сервера.';en='You can not remove the category for news feeds loaded from the server.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура КомандаПодборКатегорий(Команда)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='სერვერიდან ჩატვირთული ახალი ამბების ლენტისთვის კატეგორიების შეცვლა არ შეიძლება.';ru='Нельзя изменять категории для ленты новостей, загруженной с сервера.';en='You can not change the category for news feeds loaded from the server.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок   = НСтр("ka='გამონაკლისების გარეშე. სერვისის მოდელში მომხმარებლების მიხედვით ახალი ამბების ლენტის ხილვადობის გამართვა არ შეიძლება.';ru='без исключений. В модели сервиса нельзя настроить видимость ленты новостей по пользователям.';en='with no exceptions. The service model is impossible to set the visibility of news feeds for users.'");
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Ложь;
	Иначе
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Истина;
		Если Объект.ИсключенияВидимости.Количество() = 0 Тогда
			Если Объект.ВидимостьПоУмолчанию = Истина Тогда
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ka='გამონაკლისების გარეშე';ru='без исключений';en='with no exceptions'");
			Иначе
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ka='გამონაკლისების გარეშე. ვერავინ შეძლებს ახალი ამბების მისაღებად ახალი ამბების ამ ლენტის არჩევას.';ru='без исключений. Никто не сможет выбрать эту ленту новостей для получения новостей.';en='with no exceptions. Nobody will be able to select the news feed for news.'");
		...
		Иначе // > 3
			...
			СписокИсключений = СтрЗаменить(СписокИсключений, "##", ", ");
			СписокИсключений = СтрЗаменить(СписокИсключений, "#", "");
			ТекстСообщения = НСтр("ka='%СписокИсключений% და კიდევ %Объект.ИсключенияВидимости.Количество()-2% მომხმარებელი';ru='%СписокИсключений% и еще %Объект.ИсключенияВидимости.Количество()-2% пользователей';en='SpisokIsklyucheny%% and% Obekt.IsklyucheniyaVidimosti.Kolichestvo () - 2% of users'");
	...
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ДекорацияСтатистика.Заголовок = НСтр("ka='თავდაპირველად, საჭიროა ახალი ამბების ლენტის ჩაწერა მისი სტატისტიკის მისაღებად.';ru='Необходимо вначале записать ленту новостей, чтобы получить по ней статистику.';en='You must first record the news feed to get the statistics on it.'");
	Иначе
...
"';ka='и среди них нет ни одной помеченной на удаление"
"'");
...
"';ka='все они помечены на удаление (а значит не видны в списке новостей)"
"'");
				Иначе
					ТекстСтатистики = СтрШаблон(
						НСтр("ru=', из них помечено на удаление %1"
"';ka=', из них помечено на удаление %1"
"'"),
...
"';ka='Нет ни одной загруженной новости"
"'");
			Иначе
				ТекстСтатистики = СтрШаблон(
					НСтр("ka='Всего новостей в этой ленте новостей: %1';ru='Всего новостей в этой ленте новостей: %1';en='Total articles in the newswire:% 1'"),
					КоличествоНовостейВсего);
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("ru='и среди них нет ни одной помеченной на удаление"
"';ka='и среди них нет ни одной помеченной на удаление"
...
"'"),
			...
			Если ДатаПоследнейУспешнойЗагрузки = '00010101' Тогда
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + НСтр("ka='ახალი ამბების ამ ლენტის მიხედვით ახალი ამბები ჯერ არ ჩატვირთულა';ru='Новости по этой ленте новостей еще не загружались';en='on this tape News News not yet loaded'");
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ka='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';ru='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';en='News of this newswire has never been successfully uploaded (% 1 was unsuccessful attempts to download news)'"),
			...
			Иначе
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ka='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';en='News of this newswire recently been successfully uploaded% 1 (local time)'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"));
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ka='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';en='News of this newswire recently been successfully uploaded% 1 (local time), after that it was% 2 failed attempts to download news.'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"),
							КоличествоНеудачПриЗагрузкеНовостей);
				КонецЕсли;
			КонецЕсли;
