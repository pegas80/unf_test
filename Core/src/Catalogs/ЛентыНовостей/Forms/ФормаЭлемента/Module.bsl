...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ka='ახალი ამბების ამ ლენტის მიხედვით შერჩევების გამართვა (მოქმედებს მონაცემთა ყველა არეალზე):';ru='Настройки отборов по этой ленте новостей (действуют на все области данных):'");
	Иначе
		Если (ПравоДоступа("Изменение", Метаданные.Справочники.ЛентыНовостей) = Истина) Тогда
			ЭтаФорма.ТолькоПросмотр = Ложь;
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ka='ახალი ამბების ამ ლენტის მიხედვით შერჩევების გამართვა:';ru='Настройки отборов по этой ленте новостей:'");
	КонецЕсли;
	...
	Иначе
		...
		Если Объект.ЗагруженоССервера = Ложь Тогда
			...
			Для Каждого ТекущийРедактор Из МассивРедакторовЛентыНовостей Цикл
				...
				НоваяСтрока.ПользовательПредставление = СтрЗаменить(НоваяСтрока.ПользовательПредставление, НСтр("ka='(სრული უფლებები)';ru='(полные права)'"), НСтр("ka='(სრული უფლებები, ახალი ამბების რედაქტირება შეუძლია ახალი ამბების ყველა ლენტში)';ru='(полные права, может редактировать новости во всех лентах новостей)'"));
			КонецЦикла;
	...
	Если (Объект.ЗагруженоССервера = Истина) Тогда
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок = НСтр("ka='ახალი ამბების ფაილის მიღება (ამ ველების გადავსება ავტომატურად ხდება, 1С-ის სერვერზე ცვლილებების შემთხვევაში)';ru='Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)'");
		КонецЕсли;
...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВРег(Объект.Протокол) = ВРег("file") Тогда
		...
		Диалог.Заголовок = НСтр("ka='აირჩიეთ ახალი ამბების ფაილი Atom-ის ან RSS-ის ფორმატში';ru='Выберите файл новостей в формате Atom или RSS'");
...
Процедура ДоступныеКатегорииНовостейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ka='სერვერიდან ჩატვირთული ახალი ამბების ლენტისთვის კატეგორიების დამატება არ შეიძლება.';ru='Нельзя добавлять категории для ленты новостей, загруженной с сервера.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='სერვერიდან ჩატვირთული ახალი ამბების ლენტისთვის კატეგორიების წაშლა არ შეიძლება.';ru='Нельзя удалять категории для ленты новостей, загруженной с сервера.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура КомандаПодборКатегорий(Команда)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='სერვერიდან ჩატვირთული ახალი ამბების ლენტისთვის კატეგორიების შეცვლა არ შეიძლება.';ru='Нельзя изменять категории для ленты новостей, загруженной с сервера.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Иначе
		...
		Если Объект.ИсключенияВидимости.Количество() = 0 Тогда
			Если Объект.ВидимостьПоУмолчанию = Истина Тогда
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ka='გამონაკლისების გარეშე';ru='без исключений'");
			Иначе
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ka='გამონაკლისების გარეშე. ვერავინ შეძლებს ახალი ამბების მისაღებად ახალი ამბების ამ ლენტის არჩევას.';ru='без исключений. Никто не сможет выбрать эту ленту новостей для получения новостей.'");
		...
		Иначе // > 3
			...
			СписокИсключений = СтрЗаменить(СписокИсключений, "##", ", ");
			СписокИсключений = СтрЗаменить(СписокИсключений, "#", "");
			ТекстСообщения = НСтр("ka='%СписокИсключений% და კიდევ %Объект.ИсключенияВидимости.Количество()-2% მომხმარებელი';ru='%СписокИсключений% и еще %Объект.ИсключенияВидимости.Количество()-2% пользователей'");
	...
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ДекорацияСтатистика.Заголовок = НСтр("ka='თავდაპირველად, საჭიროა ახალი ამბების ლენტის ჩაწერა მისი სტატისტიკის მისაღებად.';ru='Необходимо вначале записать ленту новостей, чтобы получить по ней статистику.'");
	Иначе
...
"'");
			Иначе
				ТекстСтатистики = СтрШаблон(
					НСтр("ka='Всего новостей в этой ленте новостей: %1';ru='Всего новостей в этой ленте новостей: %1'"),
					КоличествоНовостейВсего);
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("ru='и среди них нет ни одной помеченной на удаление"
"';ka='и среди них нет ни одной помеченной на удаление"
...
"'"),
			...
			Если ДатаПоследнейУспешнойЗагрузки = '00010101' Тогда
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + НСтр("ka='ახალი ამბების ამ ლენტის მიხედვით ახალი ამბები ჯერ არ ჩატვირთულა';ru='Новости по этой ленте новостей еще не загружались'");
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ka='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';ru='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)'"),
			...
			Иначе
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ka='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"));
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ka='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"),
							КоличествоНеудачПриЗагрузкеНовостей);
				КонецЕсли;
			КонецЕсли;
