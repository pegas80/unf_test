...
Процедура ДекорацияТребуетсяОбновлениеССервераОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(НавигационнаяСсылка) = ВРег("Update") Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='განახლება დასრულებულია';ru='Обновление завершено'"),
			,
			НСтр("ka='მონაცემთა განახლება, აგრეთვე, ჩატვირთულ მონაცემთა შემდგომი ოპტიმიზაცია დასრულებულია';ru='Обновление данных, а также последующая оптимизация загруженных данных завершены'"));
	...
	Если Запись.Выбран() Тогда
		Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
			ТекстНадписи = СтрШаблон(
				НСтр("ka='Данные актуальны и соответствуют данным с сервера от %1.';ru='Данные актуальны и соответствуют данным с сервера от %1.'"),
			...
			Иначе
				Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок   = Новый ФорматированнаяСтрока(
					...
					Новый ФорматированнаяСтрока(
						НСтр("ka='განახლებების სემოწმება';ru='Проверить обновления'"),
						,
