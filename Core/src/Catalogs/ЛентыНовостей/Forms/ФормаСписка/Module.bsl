...
Процедура ДекорацияТребуетсяОбновлениеССервераОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(НавигационнаяСсылка) = ВРег("Update") Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='განახლება დასრულებულია';ru='Обновление завершено';en='upgrade complete'"),
			,
			НСтр("ka='მონაცემთა განახლება, აგრეთვე, ჩატვირთულ მონაცემთა შემდგომი ოპტიმიზაცია დასრულებულია';ru='Обновление данных, а также последующая оптимизация загруженных данных завершены';en='Update data, as well as subsequent optimization of the downloaded data completed'"));
	...
	Если Запись.Выбран() Тогда
		Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
			ТекстНадписи = СтрШаблон(
				НСтр("ka='Данные актуальны и соответствуют данным с сервера от %1.';ru='Данные актуальны и соответствуют данным с сервера от %1.';en='Data relevant and correspond to the data from the server 1%.'"),
...
"а на сервере уже версия от %2.';ka='Данные никогда не обновлялись с сервера,"
"а на сервере уже версия от %2.'");
			Иначе
				ТекстНадписи = НСтр("ru='Последний раз данные обновлялись с сервера %1,"
"а на сервере уже версия от %2.';ka='Последний раз данные обновлялись с сервера %1,"
"а на сервере уже версия от %2.'");
			КонецЕсли;
			ТекстНадписи = СтрШаблон(
				ТекстНадписи,
			...
			Иначе
				Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок   = Новый ФорматированнаяСтрока(
					...
					Новый ФорматированнаяСтрока(
						НСтр("ka='განახლებების სემოწმება';ru='Проверить обновления';en='check for updates'"),
						,
