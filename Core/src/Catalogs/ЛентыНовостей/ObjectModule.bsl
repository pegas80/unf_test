...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого ТекущаяСтрока Из ТаблицаПроверки Цикл
		Если ТекущаяСтрока.КоличествоСтрок > 1 Тогда
			...
			Сообщение.Текст = НСтр("ka='კატეგორია [%КатегорияНовостей%] რამდენიმეჯერ მეორდება."
"ყოველი კატეგორიის შეყვანა ნებადართულია მხოლოდ ერთხელ.';ru='Категория [%КатегорияНовостей%] повторяется несколько раз."
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтотОбъект.ЗагруженоССервера = Истина Тогда
		...
		Если (ЭтотОбъект.ПометкаУдаления = Истина)
			...
			Иначе
				ТекстСообщения = НСтр("ka='ახალი ამბების სერვერიდან ავტომატურად ჩატვირთული ახალი ამბების ლენტების წასაშლელად მონიშვნა არ შეიძლება.';ru='Нельзя помечать на удаление ленты новостей, загруженные автоматически с сервера новостей.';en='Can not be marked for deletion newsfeeds, uploaded automatically from the news server.'");
				Сообщение = Новый СообщениеПользователю();
