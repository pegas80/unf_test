...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КоличествоТоваров > 0 Тогда
		Если Товары[КоличествоТоваров-1].Код > ТекущийОбъект.МаксимальныйКод И ТекущийОбъект.МаксимальныйКод <> 0 Тогда
			...
			НСтр("ka='ცხრილ ""საქონლებში"" მოიძებნა სტრიქონები კოდით, რომელიც აღემატება მაქსიმალურად დასაშვებ მნიშვნელობას: %1. შეცვალეთ მაქსიმალური კოდი ან შეამცირეთ გადმოსატვირთი საქონლის რაოდენობა შერჩევის მეშვეობით.';ru='В табличной части ""Товары"" найдены строки с кодом, превышающим максимальное допустимое значение: %1. Измените максимальный код или уменьшите количество товаров к выгрузке при помощи отбора.';en='In the tables of ""Goods"" with the code string found exceeding the maximum allowable value: 1%. Change the maximum code or reduce the quantity of goods to be unloaded by means of selection.'"), ТекущийОбъект.МаксимальныйКод),,"Товары",,Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура МаксимальныйКодПриИзменении(Элемент)
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის სიის განახლება...';ru='Выполняется обновление списка товаров...';en='Are updating the list of goods ...'"));
	ОбновитьСписокТоваровНаСервере();
...
Процедура ПрефиксВесовогоТовараПриИзменении(Элемент)
	...
	Если Объект.ПрефиксВесовогоТовара = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Префикс = 0, обычно, используется для штучного, а не весового товара. Рекомендуется устанавливать префикс > 0.';ru='Префикс = 0, обычно, используется для штучного, а не весового товара. Рекомендуется устанавливать префикс > 0.';en='Prefix = 0 is usually used for the piece, rather than the weight of the goods. It is recommended to set the prefix> 0.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			ОписаниеОшибки = НСтр("ka='ასეთი კოდი უკვე დანიშნულია ნომენკლატურა %Номенклатура%-სთვის';ru='Такой код уже назначен для номенклатуры %Номенклатура%';en='This code has already been assigned for the range of% Range%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + НайденнаяСтрока.Номенклатура + """"
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Характеристика), " " + НСтр("ka='მახასიათებლით';ru='с характеристикой';en='with characteristic'") + " """ + НайденнаяСтрока.Характеристика + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Партия), " " + НСтр("ka='პარტიასთან';ru='с партией';en='with the party'") + " """ + НайденнаяСтрока.Партия + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.ЕдиницаИзмерения), " " + НСтр("ka='ერთეულით';ru='с единицей';en='with unity'") + " """ + НайденнаяСтрока.ЕдиницаИзмерения + """", ""))+НСтр("ka='. შესრულებულია ადგილების გაცვლა.';ru='. Выполнен обмен местами.';en='. Performed by swapping.'");
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) И ТекущиеДанные.Используется Тогда
			ПоказатьПредупреждение(, (НСтр("ka='მოცემული შერჩევის შესაბამის ნომენკლატურას, წაშლის შემდეგ, ხელახლა დაენიშნება საქონლის კოდები';ru='Номенклатуре, соответствующей заданному отбору, после удаления будут заново назначены коды товаров.';en='Nomenclature corresponding to this selection, after the removal will be re-assigned product codes.'")));
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура УдалитьСвободныеКодыТоваровВКонцеСписка(Команда)
	...
	Если Товары.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ka='წასაშლელად მონაცემები არ არის';ru='Нет данных для удаления';en='No data to delete'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено,НСтр("ka='წასაშლელად მონაცემები არ არის';ru='Нет данных для удаления';en='No data to delete'"));
	КонецЕсли;
