...
Процедура МаксимальныйКодПриИзменении(Элемент)
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის სიის განახლება...';ru='Выполняется обновление списка товаров...'"));
	ОбновитьСписокТоваровНаСервере();
...
Процедура ПрефиксВесовогоТовараПриИзменении(Элемент)
	...
	Если Объект.ПрефиксВесовогоТовара = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Префикс = 0, обычно, используется для штучного, а не весового товара. Рекомендуется устанавливать префикс > 0.';ru='Префикс = 0, обычно, используется для штучного, а не весового товара. Рекомендуется устанавливать префикс > 0.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			ОписаниеОшибки = НСтр("ka='ასეთი კოდი უკვე დანიშნულია ნომენკლატურა %Номенклатура%-სთვის';ru='Такой код уже назначен для номенклатуры %Номенклатура%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + НайденнаяСтрока.Номенклатура + """"
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Характеристика), " " + НСтр("ka='მახასიათებლით';ru='с характеристикой'") + " """ + НайденнаяСтрока.Характеристика + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Партия), " " + НСтр("ka='პარტიასთან';ru='с партией'") + " """ + НайденнаяСтрока.Партия + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.ЕдиницаИзмерения), " " + НСтр("ka='ერთეულით';ru='с единицей'") + " """ + НайденнаяСтрока.ЕдиницаИзмерения + """", ""))+НСтр("ka='. შესრულებულია ადგილების გაცვლა.';ru='. Выполнен обмен местами.'");
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) И ТекущиеДанные.Используется Тогда
			ПоказатьПредупреждение(, (НСтр("ka='მოცემული შერჩევის შესაბამის ნომენკლატურას, წაშლის შემდეგ, ხელახლა დაენიშნება საქონლის კოდები';ru='Номенклатуре, соответствующей заданному отбору, после удаления будут заново назначены коды товаров.'")));
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура УдалитьСвободныеКодыТоваровВКонцеСписка(Команда)
	...
	Если Товары.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ka='წასაშლელად მონაცემები არ არის';ru='Нет данных для удаления'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено,НСтр("ka='წასაშლელად მონაცემები არ არის';ru='Нет данных для удаления'"));
	КонецЕсли;
