...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
		...
		Исключение
			...
			Операция = НСтр("ka='Открытие настройки обмена с сервисом 1С:ДиректБанк.';ru='Открытие настройки обмена с сервисом 1С:ДиректБанк.'");
		...
		Если ТипЗнч(ДвоичныеДанныеКомпоненты) = Тип("ДвоичныеДанные") Тогда
			Элементы.ИнформацияОКомпонентеСбербанка.Заголовок = НСтр("ka='ფაილიდან განახლება...';ru='Обновить из файла...'");
			ВерсияКомпонентыСбербанк = ?(ЗначениеЗаполнено(Объект.ВерсияВнешнейКомпоненты), Объект.ВерсияВнешнейКомпоненты,
				НСтр("ka='<Неизвестна>';ru='<Неизвестна>'"));
			НазваниеМодуля = ?(ЗначениеЗаполнено(Объект.ИмяВнешнегоМодуля), Объект.ИмяВнешнегоМодуля,
				НСтр("ka='<Неизвестно>';ru='<Неизвестно>'"));
			ПредставлениеКомпоненты = НСтр("ka='%1. ვერსია %2';ru='%1. Версия %2'");
		...
		Иначе
			...
			Элементы.ИнформацияОКомпонентеСбербанка.Заголовок = НСтр("ka='არ არის ჩატვირთული. ფაილიდან ჩატვირთვა...';ru='Не загружен. Загрузить из файла...'");
		КонецЕсли;
		Элементы.ГруппаВнешнегоМодуля.Видимость = Ложь;
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() И Не ТекущаяОрганизация = Объект.Организация
		...
		ТекстВопроса = НСтр("ka='ორგანიზაციის შეცვლის შემდეგ სერტიფიკატების სია გასუფთავდება';ru='После изменения организации список сертификатов будет очищен.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(Истина, НСтр("ka='ორგანიზაციის შეცვლა';ru='Сменить организацию'"));
		Кнопки.Добавить(Ложь, НСтр("ka='ცვლილების გაუქმება';ru='Отменить изменение'"));
		ПоказатьВопрос(Оповещение, ТекстВопроса, Кнопки, , Истина, НСтр("ka='ორგანიზაციის შეცვლა';ru='Изменение организации'"));
		Возврат;
...
Процедура ЗагрузитьСертификат(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ka='აუცილებელია ორგანიზაციის არჩევა';ru='Необходимо выбрать организацию'");
	...
	Если (Объект.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезДопОбработку")
		...
		ТекстСообщения = НСтр("ka='აუცილებელია გარე მოდულის ჩატვირთვა';ru='Необходимо загрузить внешний модуль'");
...
Процедура ТестНастроек(Команда)
	...
	Если Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстВопроса = НСтр("ka='აუცილებელია გაცვლის მიმდინარე გამართვის შენახვა. გავაგრძელოთ ტესტის შესრულება?';ru='Необходимо сохранить текущую настройку обмена. Продолжить выполнение теста?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить(Истина, НСтр("ka='ტესტის შენახვა და შესრულება';ru='Сохранить и выполнить тест'"));
		СписокКнопок.Добавить(Ложь, НСтр("ka='ტესტის გაუქმება';ru='Отменить тест'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , Истина, НСтр("ka='აწყობების ტესტი';ru='Тест настроек'"));
	Иначе
...
Процедура ПослеВыводаПредупрежденияЗагрузитьВнешнийМодульСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	#Иначе
		...
		Фильтр = НСтр("ka='გარე მოდულის ფაილი';ru='Файл внешнего модуля'") + "(*.zip)|*.zip";
...
"Файл внешнего модуля должен иметь расширение ""zip"" или ""epf""';ka='მითითებულია არასწორი ფაილი. გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""zip"" ან ""epf""'");
	...
	Если РасширениеФайла = "ZIP" Тогда // внешняя компонента
		...
		Шаблон = НСтр("ka='%1. ვერსია %2.';ru='%1. Версия %2.'");
	...
	Иначе // внешняя обработка
		...
		#Иначе
			...
			Исключение
				...
				ТекстСообщения = НСтр("ka='არჩეული გარე მოდულის შეერთება ვერ მოხერხდა';ru='Не удалось подключить выбранный внешний модуль'");
		...
		Элементы.ЗагрузитьВнешнийМодульИзФайла.Заголовок = НСтр("ka='ფაილიდან განახლება...';ru='Обновить из файла...'");
...
Процедура ПослеТестаНастройкиОбмена(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Отмена ИЛИ Результат = Неопределено Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='ტესტი არ არის დასრულებული.';ru='Тест не завершен.'"));
	...
	ИначеЕсли Результат = Истина ИЛИ (ТипЗнч(Результат) = Тип("Структура") И Результат.Успех) Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='Тестирование выполнено успешно.';ru='Тестирование выполнено успешно.'"));
	Иначе
		Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("МассивСообщений") Тогда
			...
			КонецЦикла
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='ტესტი არ არის გავლილი.';ru='Тест не пройден.'"));
	КонецЕсли;
