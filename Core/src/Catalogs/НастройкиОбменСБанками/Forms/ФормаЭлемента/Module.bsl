...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
		...
		Исключение
			...
			Операция = НСтр("ka='Открытие настройки обмена с сервисом 1С:ДиректБанк.';ru='Открытие настройки обмена с сервисом 1С:ДиректБанк.';en='Opening sharing settings service 1C: DirektBank.'");
		...
		Если ТипЗнч(ДвоичныеДанныеКомпоненты) = Тип("ДвоичныеДанные") Тогда
			Элементы.ИнформацияОКомпонентеСбербанка.Заголовок = НСтр("ka='ფაილიდან განახლება...';ru='Обновить из файла...';en='Update from File ...'");
			ВерсияКомпонентыСбербанк = ?(ЗначениеЗаполнено(Объект.ВерсияВнешнейКомпоненты), Объект.ВерсияВнешнейКомпоненты,
				НСтр("ka='<Неизвестна>';ru='<Неизвестна>';en='<Unknown>'"));
			НазваниеМодуля = ?(ЗначениеЗаполнено(Объект.ИмяВнешнегоМодуля), Объект.ИмяВнешнегоМодуля,
				НСтр("ka='<Неизвестно>';ru='<Неизвестно>';en='<Unknown>'"));
			ПредставлениеКомпоненты = НСтр("ka='%1. ვერსია %2';ru='%1. Версия %2';en='%one. Version% 2'");
		...
		Иначе
			...
			Элементы.ИнформацияОКомпонентеСбербанка.Заголовок = НСтр("ka='არ არის ჩატვირთული. ფაილიდან ჩატვირთვა...';ru='Не загружен. Загрузить из файла...';en='Not loaded. Load from file ...'");
		КонецЕсли;
		Элементы.ГруппаВнешнегоМодуля.Видимость = Ложь;
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() И Не ТекущаяОрганизация = Объект.Организация
		...
		ТекстВопроса = НСтр("ka='ორგანიზაციის შეცვლის შემდეგ სერტიფიკატების სია გასუფთავდება';ru='После изменения организации список сертификатов будет очищен.';en='After changing the organization of the list of certificates will be cleared.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(Истина, НСтр("ka='ორგანიზაციის შეცვლა';ru='Сменить организацию';en='change organization'"));
		Кнопки.Добавить(Ложь, НСтр("ka='ცვლილების გაუქმება';ru='Отменить изменение';en='cancel change'"));
		ПоказатьВопрос(Оповещение, ТекстВопроса, Кнопки, , Истина, НСтр("ka='ორგანიზაციის შეცვლა';ru='Изменение организации';en='change organization'"));
		Возврат;
...
Процедура ЗагрузитьСертификат(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ka='აუცილებელია ორგანიზაციის არჩევა';ru='Необходимо выбрать организацию';en='You must select the organization'");
	...
	Если (Объект.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезДопОбработку")
		...
		ТекстСообщения = НСтр("ka='აუცილებელია გარე მოდულის ჩატვირთვა';ru='Необходимо загрузить внешний модуль';en='You must download an external module'");
...
Процедура ТестНастроек(Команда)
	...
	Если Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстВопроса = НСтр("ka='აუცილებელია გაცვლის მიმდინარე გამართვის შენახვა. გავაგრძელოთ ტესტის შესრულება?';ru='Необходимо сохранить текущую настройку обмена. Продолжить выполнение теста?';en='To save the current sharing settings. Continue the test?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить(Истина, НСтр("ka='ტესტის შენახვა და შესრულება';ru='Сохранить и выполнить тест';en='Save and run the test'"));
		СписокКнопок.Добавить(Ложь, НСтр("ka='ტესტის გაუქმება';ru='Отменить тест';en='cancel the test'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , Истина, НСтр("ka='აწყობების ტესტი';ru='Тест настроек';en='test settings'"));
	Иначе
...
"Удалите программу ""VpnKey-TLS для 1С:Предприятия 8"" и перезагрузите компьютер.';ka='На компьютере уже установлен внешний модуль другой версии."
...
Процедура ПослеВыводаПредупрежденияЗагрузитьВнешнийМодульСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	#Иначе
		...
		Фильтр = НСтр("ka='გარე მოდულის ფაილი';ru='Файл внешнего модуля';en='external module file'") + "(*.zip)|*.zip";
...
"Файл внешнего модуля должен иметь расширение ""zip"" или ""epf""';ka='მითითებულია არასწორი ფაილი. გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""zip"" ან ""epf""'");
	...
	Если РасширениеФайла = "ZIP" Тогда // внешняя компонента
		...
		Шаблон = НСтр("ka='%1. ვერსია %2.';ru='%1. Версия %2.';en='%one. Version% 2.'");
	...
	Иначе // внешняя обработка
		...
		#Иначе
			...
			Исключение
				...
				ТекстСообщения = НСтр("ka='არჩეული გარე მოდულის შეერთება ვერ მოხერხდა';ru='Не удалось подключить выбранный внешний модуль';en='Failed to connect the selected external module'");
		...
		Элементы.ЗагрузитьВнешнийМодульИзФайла.Заголовок = НСтр("ka='ფაილიდან განახლება...';ru='Обновить из файла...';en='Update from File ...'");
...
Процедура ПослеТестаНастройкиОбмена(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Отмена ИЛИ Результат = Неопределено Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='ტესტი არ არის დასრულებული.';ru='Тест не завершен.';en='The test has not been completed.'"));
	...
	ИначеЕсли Результат = Истина ИЛИ (ТипЗнч(Результат) = Тип("Структура") И Результат.Успех) Тогда
		ПоказатьОповещениеПользователя(НСтр("ka='Тестирование выполнено успешно.';ru='Тестирование выполнено успешно.';en='The test is OK.'"));
	Иначе
		Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("МассивСообщений") Тогда
			...
			КонецЦикла
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ka='ტესტი არ არის გავლილი.';ru='Тест не пройден.';en='Fail.'"));
	КонецЕсли;
...
Процедура ПоместитьВХранилищеСертификат(Знач ДвоичныеДанные, Знач УникальныйИдентификатор, АдресСертификатаСбербанка, ПредставлениеСертификата = Неопределено)
	...
	Иначе
		...
		Исключение
			...
			Исключение
				ТекстСообщения = НСтр("ru='Не удалось прочитать файл сертификата, операция прервана.';en='Could not read the certificate file, the operation is aborted.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				АдресСертификатаСбербанка = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
				Возврат;
			КонецПопытки;
...
Функция НастройкаОбменаЗаписана(Обработчик)
	...
	ТекстВопроса = НСтр("ka='სერთიფიკატების ჩატვირთვა შეიძლება მხოლოდ გაცვლის ჩაწერილ გამართვაში. ჩავწეროთ?';ru='Загружать сертификаты можно только в записанной настройке обмена."
"Записать?'");
