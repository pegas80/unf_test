...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = НСтр("ka='1С:ДиректБанк: Обновление списка банков, поддерживающих обмен.';ru='1С:ДиректБанк: Обновление списка банков, поддерживающих обмен.'");
...
СтандартнаяОбработка = Ложь;
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='გარე მოდული (*.zip;*.epf)|*.zip;*.epf';ru='Внешний модуль(*.zip;*.epf)|*.zip;*.epf'");
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ბანკის გარე მოდულის ფაილი';ru='Выберите файл внешнего модуля банка'");
...
"Модуль какой версии нужно теперь использовать?'");
	...
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	ЗаголовокВопроса = НСтр("ka='მოდულის ვერსიის არჩევა';ru='Выбор версии модуля'");
...
Процедура ОбработатьОшибкуПолученияНастроекИзБанкаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='ბანკიდან გაცვლის გამართვების მიღებისას შეცდომა მოხდა.';ru='При получении настроек обмена из банка произошла ошибка.'");
	ВидОперации = НСтр("ka='ბანკის სერვერიდან გაცვლის გასწორების მიღება.';ru='Получение настроек обмена с сервера банка.'");
...
Функция ДобавитьСертификатВНастройкуОбменаВК(Знач НастройкаОбмена, Знач СертификатBase64, Знач СтруктураСертификата)
	...
	Исключение
		Операция = НСтр("ka='Добавление сертификата в настройку обмена';ru='Добавление сертификата в настройку обмена'");
...
"Обратитесь в свой банк.'");
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		ТекстСообщения = НСтр("ka='Файл содержит настройки для неизвестной организации: %1%2%3';ru='Файл содержит настройки для неизвестной организации: %1%2%3'");
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = НСтр("ka='Файл содержит настройки для неизвестного банка: %1%2';ru='Файл содержит настройки для неизвестного банка: %1%2'");
...
Процедура ВывестиСсылкуНаСайтБанка(ИнформацияНаСайтеБанка, АдресСайта)
	...
	ИнформацияНаСайтеБанка.Заголовок = Новый ФорматированнаяСтрока(
		НСтр("ka='ბანკის მხარეს შეერთების წესის და პირობების შესახებ დაწვრილებითი ინფორმაციის მიღება შეიძლება';ru='Подробную информацию о порядке и условиях подключения на стороне банка можно получить'"), " ",
		Новый ФорматированнаяСтрока(НСтр("ka='აქ.';ru='здесь.'"), , , , АдресСайта));
...
Процедура ВключитьНастройкуОбмена(Знач НастройкаОбмена, Успех)
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ka='აწყობის ჩართვა.';ru='Включение настройки.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
