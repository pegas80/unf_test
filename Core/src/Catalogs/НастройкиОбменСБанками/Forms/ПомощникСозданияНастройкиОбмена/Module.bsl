...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = НСтр("ka='1С:ДиректБанк: Обновление списка банков, поддерживающих обмен.';ru='1С:ДиректБанк: Обновление списка банков, поддерживающих обмен.';en='1C: DirektBank: Updating the list of banks that support sharing.'");
...
"Обратитесь к администратору';ka='Нет прав настройки прямого обмена с банком."
...
"Продолжить закрытие программы?';ka='Подключение к сервису 1С:ДиректБанк не завершено."
"Продолжить закрытие программы?'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Подключение к сервису 1С:ДиректБанк не завершено."
"Закрыть помощник подключения?';ka='Подключение к сервису 1С:ДиректБанк не завершено."
...
СтандартнаяОбработка = Ложь;
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='გარე მოდული (*.zip;*.epf)|*.zip;*.epf';ru='Внешний модуль(*.zip;*.epf)|*.zip;*.epf';en='External module (. * Zip; * epf.) | * .Zip; * epf.'");
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ბანკის გარე მოდულის ფაილი';ru='Выберите файл внешнего модуля банка';en='Select external bank module file'");
...
"Файл внешнего модуля должен иметь расширение ""zip"" или ""epf""';ka='მითითებულია არასწორი ფაილი. გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""zip"" ან ""epf""'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВнешнегоМодуля");
					Возврат;
				КонецЕсли;
...
"Файл настроек содержит настройки для организации: %2';ka='Указан неверный файл."
"Требуются настройки для организации: %1"
...
"Файл настроек содержит настройки для банка: %2';ka='Указан неверный файл."
"Требуются настройки для банка: %1"
...
Процедура ОтправитьТестовыйЗапросПослеУстановкиПодписиСбербанк(ПодписьВерна, ДополнительныеПараметры) Экспорт
	...
	Если Не ПодписьВерна Тогда
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(НСтр("ru='Установленная подпись неверна.';en='Installed the signature is invalid.'"), УникальныйИдентификатор);
		ОбработатьРезультатТестаНастройки(Ложь);
		Возврат;
	КонецЕсли;
...
"Модуль какой версии нужно теперь использовать?'");
	...
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	ЗаголовокВопроса = НСтр("ka='მოდულის ვერსიის არჩევა';ru='Выбор версии модуля';en='Selecting the module version'");
...
"Требуются настройки для организации: %1"
"Получены настройки для организации: %2';ka='Указана неверная учетная запись."
"Требуются настройки для организации: %1"
...
"Требуются настройки для банка: %1"
"Получены настройки для банка: %2';ka='Подключено неверное аппаратное устройство."
"Требуются настройки для банка: %1"
...
Процедура ПослеПовторнойПопыткиПодключенияВнешнейКомпоненты(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Подключено Тогда
		ТекстСообщения = НСтр("ka='გარე მოდულის შეერთება ვერ მოხერხდა. მიმართეთ ადმინისტრატორს.';ru='Внешний модуль не удалось подключить."
"Обратитесь к администратору.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ОбработатьРезультатПолученияНастроекССервераБанка(Ложь);
		Возврат;
...
Процедура ОбработатьОшибкуПолученияНастроекИзБанкаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='ბანკიდან გაცვლის გამართვების მიღებისას შეცდომა მოხდა.';ru='При получении настроек обмена из банка произошла ошибка.';en='When you receive an error occurred setting the exchange from the bank.'");
	ВидОперации = НСтр("ka='ბანკის სერვერიდან გაცვლის გასწორების მიღება.';ru='Получение настроек обмена с сервера банка.';en='Getting sharing settings from the bank server.'");
...
Функция ДобавитьСертификатВНастройкуОбменаВК(Знач НастройкаОбмена, Знач СертификатBase64, Знач СтруктураСертификата)
	...
	Исключение
		Операция = НСтр("ka='Добавление сертификата в настройку обмена';ru='Добавление сертификата в настройку обмена';en='Add the certificate to the exchange adjustment'");
		ШаблонСообщения = НСтр("ru='При добавлении сертификата в настройку обмена произошла ошибка."
"%1';ka='При добавлении сертификата в настройку обмена произошла ошибка."
"%1'");
...
"Обратитесь в свой банк.'");
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		ТекстСообщения = НСтр("ka='Файл содержит настройки для неизвестной организации: %1%2%3';ru='Файл содержит настройки для неизвестной организации: %1%2%3';en='This file contains the settings for an unknown organization:% 1% 2% 3'");
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = НСтр("ka='Файл содержит настройки для неизвестного банка: %1%2';ru='Файл содержит настройки для неизвестного банка: %1%2';en='This file contains the settings for unknown bank:% 1% 2'");
...
Процедура ВывестиСсылкуНаСайтБанка(ИнформацияНаСайтеБанка, АдресСайта)
	...
	ИнформацияНаСайтеБанка.Заголовок = Новый ФорматированнаяСтрока(
		НСтр("ka='ბანკის მხარეს შეერთების წესის და პირობების შესახებ დაწვრილებითი ინფორმაციის მიღება შეიძლება';ru='Подробную информацию о порядке и условиях подключения на стороне банка можно получить';en='For more information about the order and conditions of connection on the side of the bank can be obtained'"), " ",
		Новый ФорматированнаяСтрока(НСтр("ka='აქ.';ru='здесь.';en='here.'"), , , , АдресСайта));
...
Процедура ВключитьНастройкуОбмена(Знач НастройкаОбмена, Успех)
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ka='აწყობის ჩართვა.';ru='Включение настройки.';en='Enable setting.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
