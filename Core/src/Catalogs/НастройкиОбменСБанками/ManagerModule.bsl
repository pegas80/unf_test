...
Процедура ЗаполнитьВерсиюФормата(Параметры) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Операция = НСтр("ka='1С:ДиректБанк: Заполнение версии формата.';ru='1С:ДиректБанк: Заполнение версии формата.'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='1С:ДиректБанк: При заполнении версии формата произошла ошибка.';ru='1С:ДиректБанк: При заполнении версии формата произошла ошибка.'");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Иначе
		Представление = НСтр("ka='შეუვსებელი გამართვა';ru='Не заполненная настройка'");
	КонецЕсли;
...
Функция ПолучитьНастройкиОбмена(ПараметрыПолученияНастроек)
	...
	ВидОперации = НСтр("ka='გამართვების მიღება ბანკის სერვერიდან';ru='Получение настроек с сервера банка'");
...
"%2'");
	...
	Попытка
		...
		Если НЕ ResultBank.Error = Неопределено Тогда
			ОсновнойТекст = НСтр("ka='ბანკის სერვერზე ავთენტიფიკაციის დროს შეცდომა მოხდა.';ru='Произошла ошибка при аутентификации на сервере банка.'");
			Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
	...
	Исключение
		Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
			ТекстСообщения = НСтр("ka='ბანკიდან გამართვების მიღებისას შეცდომა მოხდა';ru='Произошла ошибка при получении настроек из банка'");
...
"გზა ფაილისკენ: %2'");
			...
			ВидОперации = НСтр("ka='გაცვლის გამართვების მიღება ბანკიდან';ru='Получение настроек обмена из банка'");
