...
Процедура ЗаполнитьВерсиюФормата(Параметры) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Операция = НСтр("ka='1С:ДиректБанк: Заполнение версии формата.';ru='1С:ДиректБанк: Заполнение версии формата.';en='1C: DirektBank: Filling format version.'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='1С:ДиректБанк: При заполнении версии формата произошла ошибка.';ru='1С:ДиректБанк: При заполнении версии формата произошла ошибка.';en='1C: DirektBank: An error occurred while completing the format version.'");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Иначе
		Представление = НСтр("ka='შეუვსებელი გამართვა';ru='Не заполненная настройка';en='Not completed setup'");
	КонецЕсли;
...
Функция ПолучитьНастройкиОбмена(ПараметрыПолученияНастроек)
	...
	ВидОперации = НСтр("ka='გამართვების მიღება ბანკის სერვერიდან';ru='Получение настроек с сервера банка';en='Receiving settings from the bank server'");
...
"Код ошибки: %1."
"%2';ka='ბანკის სერვერიდან გამართვების ჩატვირთვის შეცდომა."
"შეცდომის კოდი: %1."
"%2'");
	...
	Попытка
		...
		Если НЕ ResultBank.Error = Неопределено Тогда
			ОсновнойТекст = НСтр("ka='ბანკის სერვერზე ავთენტიფიკაციის დროს შეცდომა მოხდა.';ru='Произошла ошибка при аутентификации на сервере банка.';en=""An error occurred while authenticating the bank's server.""");
			Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
	...
	Исключение
		Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
			ТекстСообщения = НСтр("ka='ბანკიდან გამართვების მიღებისას შეცდომა მოხდა';ru='Произошла ошибка при получении настроек из банка';en='An error occurred while receiving the settings from the bank'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = НСтр("ru='Ошибка чтения ответа банка."
"Текст ошибки: %1"
"Путь к файлу: %2';ka='ბანკის პასუხის წაკითხვის შეცდომა."
"შეცდომის ტექსტი: %1"
"გზა ფაილისკენ: %2'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, ПодробноеПредставлениеОшибки, ИмяФайлаРезультата);
			ВидОперации = НСтр("ka='გაცვლის გამართვების მიღება ბანკიდან';ru='Получение настроек обмена из банка';en='Getting sharing settings from the bank'");
