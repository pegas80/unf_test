...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаУчетнойЗаписиЗаголовок.Заголовок = ?(КонтекстныйРежим,
		НСтр("ka='წერილების გასაგზავნად საჭირო არის ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის გამართვა';ru='Для отправки писем необходимо настроить учетную запись электронной почты';en='To send messages, you must configure an email account'"),
		НСтр("ka='შეიყვანეთ სააღრიცხვო ჩანაწერის პარამეტრები';ru='Введите параметры учетной записи';en='Enter account settings'"));
		
	Если Не КонтекстныйРежим Тогда
		Заголовок = НСтр("ka='ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის შექმნა';ru='Создание учетной записи электронной почты';en='Create an email account'");
	Иначе
		Заголовок = НСтр("ka='ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის გამართვა';ru='Настройка учетной записи электронной почты';en='Set up an email account'");
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("ka='შეყვანილი მონაცემები არ არის ჩაწერილი. დავხუროთ ფორმა?';ru='Введенные данные не записаны. Закрыть форму?';en='The entered data is not recorded. Close form?'");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='შეყვანილი მონაცემები არ არის ჩაწერილი. დავხუროთ ფორმა?';ru='Введенные данные не записаны. Закрыть форму?';en='The entered data is not recorded. Close form?'"), Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ka='სააღრიცხვო ჩანაწერის გამართვა';ru='Настройка учетной записи';en='Setting up an account'"));
КонецПроцедуры
	...
	Если ИспользоватьДляОтправки Тогда
		Результат.Добавить(
			РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				НСтр("ka='ელექტრონული ფოსტა';ru='Электронная почта.';en='Email.'")));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეიყვანეთ ელექტრონული ფოსტის მისამართი';ru='Введите адрес электронной почты';en='Enter your email address'"), , "АдресЭлектроннойПочты", , Отказ);
	ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ელექტრონული ფოსტის მისამართი არასწორად არის შეყვანილი';ru='Адрес электронной почты введен неверно';en='e-mail address is incorrect'"), , "АдресЭлектроннойПочты", , Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(ПарольДляПолученияПисем) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აუცილებელია პაროლის შეყვანა';ru='Необходимо ввести пароль';en='You must enter your password'"), , "ПарольДляПолученияПисем", , Отказ);
	КонецЕсли;
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Если КонтекстныйРежим Тогда
			ЗаголовокКнопкиДалее = НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'");
		Иначе
			ЗаголовокКнопкиДалее = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
		КонецЕсли;
	Иначе
		Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И ПроверкаЗавершиласьСОшибками Тогда
				ЗаголовокКнопкиДалее = НСтр("ka='გამეორება';ru='Повторить';en='repeat'");
		ИначеЕсли ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И СпособНастройки = "Автоматически" Тогда
			Если КонтекстныйРежим Тогда
				ЗаголовокКнопкиДалее = НСтр("ka='გავასწოროთ';ru='Настроить';en='Tune'");
			Иначе
				ЗаголовокКнопкиДалее = НСтр("ka='შექმნა';ru='Создать';en='Create'");
			КонецЕсли;
		Иначе
			ЗаголовокКнопкиДалее = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не КонтекстныйРежим И ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ka='სააღრიცხვო ჩანაწერზე გადასვლა';ru='Перейти к учетной записи';en='Jump to account'");
	Иначе
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ka='მექანიკური აწყობა';ru='Настроить вручную';en='manually configure'");
	КонецЕсли;
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.НадписьУчетнаяЗаписьУспешноНастроена.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სააღრიცხვო ჩანაწერ %1-ის პარამეტრების გამართვა დასრულებულია.';ru='Настройка параметров учетной записи"
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты()
	...
	Тема = НСтр("ka='ტესტური შეტყობინება 1С:საწარმო';ru='Тестовое сообщение 1С:Предприятие';en='The test message 1C: Enterprise'");
	Тело = НСтр("ka='ეს შეტყობინება გაიგზავნა 1С:საწარმოს ელექტრონული ფოსტის ქვესისტემის მიერ';ru='Это сообщение отправлено подсистемой электронной почты 1С:Предприятие';en='This message was sent subsystem e-mail address 1C: Enterprise'");
...
Процедура ПроверитьНастройкиУчетнойЗаписи()
	...
	Если Не ПустаяСтрока(СообщениеСервераИсходящейПочты) Тогда
		ТекстОшибки = НСтр("ka='გამავალი ფოსტის სერვერთან შეერთება ვერ მოხერხდა:';ru='Не удалось подключиться к серверу исходящей почты:';en='Could not connect to your outgoing mail server:'" + Символы.ПС)
			+ СообщениеСервераИсходящейПочты + Символы.ПС;
	КонецЕсли;
	
	Если Не ПустаяСтрока(СообщениеСервераВходящейПочты) Тогда
		ТекстОшибки = ТекстОшибки
			+ НСтр("ka='შემომავალი ფოსტის სერვერთან შეერთება ვერ მოხერხდა:';ru='Не удалось подключиться к серверу входящей почты:';en='Unable to connect to the incoming mail server:'" + Символы.ПС)
			+ СообщениеСервераВходящейПочты;
	КонецЕсли;
...
Процедура СоздатьУчетнуюЗапись()
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ka='შეცდომა ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის შექმნისას';ru='Ошибка при создании учетной записи электронной почты';en='Error creating an email account'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"Настройте параметры подключения вручную.';ka='შეერთების გამართვების დადგენა ვერ მოხერხდა. შეერთების პარამეტრების გამართვა მოახდინეთ ხელით.'");
...
"http://buh.ru/articles/documents/42429/#briefly_43166';ka='აგრ. იხ. რეკომენდაციები Gmail-ის ფოსტის გამართვასთან დაკავშირებით:"
...
Процедура ПодобратьНастройкиУчетнойЗаписи()
	...
	НаименованиеЗадания = НСтр("ka='საფოსტო სერვერის გამართვების ძებნა';ru='Поиск настроек почтового сервера';en='Search mail server settings'");
