...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаУчетнойЗаписиЗаголовок.Заголовок = ?(КонтекстныйРежим,
		НСтр("ka='წერილების გასაგზავნად საჭირო არის ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის გამართვა';ru='Для отправки писем необходимо настроить учетную запись электронной почты'"),
		НСтр("ka='შეიყვანეთ სააღრიცხვო ჩანაწერის პარამეტრები';ru='Введите параметры учетной записи'"));
		
	Если Не КонтекстныйРежим Тогда
		Заголовок = НСтр("ka='ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის შექმნა';ru='Создание учетной записи электронной почты'");
	Иначе
		Заголовок = НСтр("ka='ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის გამართვა';ru='Настройка учетной записи электронной почты'");
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("ka='შეყვანილი მონაცემები არ არის ჩაწერილი. დავხუროთ ფორმა?';ru='Введенные данные не записаны. Закрыть форму?'");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='შეყვანილი მონაცემები არ არის ჩაწერილი. დავხუროთ ფორმა?';ru='Введенные данные не записаны. Закрыть форму?'"), Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ka='სააღრიცხვო ჩანაწერის გამართვა';ru='Настройка учетной записи'"));
КонецПроцедуры
	...
	Если ИспользоватьДляОтправки Тогда
		Результат.Добавить(
			РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				НСтр("ka='ელექტრონული ფოსტა';ru='Электронная почта.'")));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეიყვანეთ ელექტრონული ფოსტის მისამართი';ru='Введите адрес электронной почты'"), , "АдресЭлектроннойПочты", , Отказ);
	ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ელექტრონული ფოსტის მისამართი არასწორად არის შეყვანილი';ru='Адрес электронной почты введен неверно'"), , "АдресЭлектроннойПочты", , Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(ПарольДляПолученияПисем) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აუცილებელია პაროლის შეყვანა';ru='Необходимо ввести пароль'"), , "ПарольДляПолученияПисем", , Отказ);
	КонецЕсли;
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Если КонтекстныйРежим Тогда
			ЗаголовокКнопкиДалее = НСтр("ka='გაგრძელება';ru='Продолжить'");
		Иначе
			ЗаголовокКнопкиДалее = НСтр("ka='დახურვა';ru='Закрыть'");
		КонецЕсли;
	Иначе
		Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И ПроверкаЗавершиласьСОшибками Тогда
				ЗаголовокКнопкиДалее = НСтр("ka='გამეორება';ru='Повторить'");
		ИначеЕсли ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И СпособНастройки = "Автоматически" Тогда
			Если КонтекстныйРежим Тогда
				ЗаголовокКнопкиДалее = НСтр("ka='გავასწოროთ';ru='Настроить'");
			Иначе
				ЗаголовокКнопкиДалее = НСтр("ka='შექმნა';ru='Создать'");
			КонецЕсли;
		Иначе
			ЗаголовокКнопкиДалее = НСтр("ka='შემდეგ >';ru='Далее >'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не КонтекстныйРежим И ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ka='სააღრიცხვო ჩანაწერზე გადასვლა';ru='Перейти к учетной записи'");
	Иначе
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ka='მექანიკური აწყობა';ru='Настроить вручную'");
	КонецЕсли;
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты()
	...
	Тема = НСтр("ka='ტესტური შეტყობინება 1С:საწარმო';ru='Тестовое сообщение 1С:Предприятие'");
	Тело = НСтр("ka='ეს შეტყობინება გაიგზავნა 1С:საწარმოს ელექტრონული ფოსტის ქვესისტემის მიერ';ru='Это сообщение отправлено подсистемой электронной почты 1С:Предприятие'");
...
Процедура ПроверитьНастройкиУчетнойЗаписи()
	...
	Если Не ПустаяСтрока(СообщениеСервераИсходящейПочты) Тогда
		ТекстОшибки = НСтр("ka='გამავალი ფოსტის სერვერთან შეერთება ვერ მოხერხდა:';ru='Не удалось подключиться к серверу исходящей почты:'" + Символы.ПС)
			+ СообщениеСервераИсходящейПочты + Символы.ПС;
	КонецЕсли;
	
	Если Не ПустаяСтрока(СообщениеСервераВходящейПочты) Тогда
		ТекстОшибки = ТекстОшибки
			+ НСтр("ka='შემომავალი ფოსტის სერვერთან შეერთება ვერ მოხერხდა:';ru='Не удалось подключиться к серверу входящей почты:'" + Символы.ПС)
			+ СообщениеСервераВходящейПочты;
	КонецЕсли;
...
Процедура СоздатьУчетнуюЗапись()
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ka='შეცდომა ელექტრონული ფოსტის სააღრიცხვო ჩანაწერის შექმნისას';ru='Ошибка при создании учетной записи электронной почты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПодобратьНастройкиУчетнойЗаписи()
	...
	НаименованиеЗадания = НСтр("ka='საფოსტო სერვერის გამართვების ძებნა';ru='Поиск настроек почтового сервера'");
