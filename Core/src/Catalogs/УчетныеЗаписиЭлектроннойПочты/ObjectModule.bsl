...
Процедура ЗаполнитьОбъектЗначениямиПоУмолчанию() Экспорт
	...
	ИмяПользователя = НСтр("ka='1С:საწარმო';ru='1С:Предприятие';en='1C: Enterprise'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ПустаяСтрока(АдресЭлектроннойПочты) И Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='საფოსტო მისამართი შევსებულია არა სწორად';ru='Почтовый адрес заполнен неверно.';en='The mailing address is not valid.'"), ЭтотОбъект, "АдресЭлектроннойПочты");
