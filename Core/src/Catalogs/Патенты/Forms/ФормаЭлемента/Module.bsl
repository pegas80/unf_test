...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Объект.Наименование = НСтр("ka='Патент';ru='Патент'") + ?(ПустаяСтрока(Объект.НомерПатента),""," №"+ Объект.НомерПатента)+
		 НСтр("ka=', მოქმედების ვადა:';ru=', срок действия:'") + Формат(Объект.ДатаНачала,"ДЛФ=D") + " - " + Формат(Объект.ДатаОкончания,"ДЛФ=D");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если НЕ ЗначениеЗаполнено(Объект.НомерПатента) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ka='პატენტის ნომერი';ru='Номер патента'"));
	...
	Если СуммаНалога > 0  И НЕ ЗначениеЗаполнено(Объект.КБК) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ka='КБК';ru='КБК'"));
	...
	Если Объект.СуммаВторогоПлатежа > 0 И Объект.СуммаПервогоПлатежа > 0
		...
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "КОРРЕКТНОСТЬ", НСтр("ka='პირველი გადახდის თარიღი';ru='Дата первого платежа'"));
...
Процедура УстановитьПредставлениеОплаты(Форма);
	...
	Если Форма.ОдинПлатеж Тогда
		...
		Элементы.СуммаПервогоПлатежа.Заголовок  	= Нстр("ka='გადასახადის ოდენობა';ru='Сумма налога'");
	Иначе
		...
		Элементы.СуммаПервогоПлатежа.Заголовок  	= Нстр("ka='გადასახადის 1/3';ru='1/3 налога'");
	КонецЕсли;
