...
Процедура СоздатьСценарий(УзелИнформационнойБазы, Расписание = Неопределено) Экспорт
	...
	Наименование = НСтр("ka='მონაცემთა ავტომატური სინქრონიზაცია %1-თან';ru='Автоматическая синхронизация данных с %1';en='Automatic synchronization of data with 1%'");
...
Процедура УстановитьПараметрыРегламентногоЗадания(РегламентноеЗаданиеОбъект, РасписаниеРегламентногоЗадания, ТекущийОбъект)
	...
	НаименованиеРегламентногоЗадания = НСтр("ka='გაცვლის შესრულების სცენარი: %1';ru='Выполнение обмена по сценарию: %1';en='Performing scripted exchange: 1%'");
...
Процедура ЗаписатьРегламентноеЗадание(Отказ, РегламентноеЗаданиеОбъект)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ka='გაცვლების შესრულების განრიგის შენახვის დროს შეცდომა მოხდა. შეცდომის დაწვრილებითი აღწერა: %1';ru='Произошла ошибка при сохранении расписания выполнения обменов. Подробное описание ошибки: %1';en='An error occurred saving schedule execution exchanges. Detailed description of the error: 1%'");
