...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый
		...
		Наименование = НСтр("ka='სინქრონიზაციის სცენარი %1-სთვის';ru='Сценарий синхронизации для %1';en='Scenario synchronization to 1%'");
...
Процедура НастройкиОбменаУзелИнформационнойБазыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Тип") И СписокУзловОбмена.НайтиПоЗначению(ВыбранноеЗначение) = Неопределено Тогда
		ТекстСообщения = НСтр("ka='არჩეული ტიპის მონაცემები ამ ფორმაში ვერ იქნება გამოყენებული. აირჩიეთ მონაცემთა სხვა ტიპი.';ru='Данные выбранного типа не могут быть использованы в этой форме."
...
Процедура ВыполнитьОбменДаннымиНаКлиенте()
	...
	Если НомерОбрабатываемойСтроки > КоличествоСтрок Тогда // выход из рекурсии
		...
		Состояние(НСтр("ka='მონაცემები სინქრონიზებულია.';ru='Данные синхронизированы.';en='We synchronized.'"), ?(ВыводитьСостояние, 100, Неопределено));
		Возврат; // выходим
	КонецЕсли;
	...
	СтрокаСообщения = НСтр("ka='%2-ისთვის სრულდება %1';ru='Выполняется %1 для %2';en='Running% 1% 2'");
