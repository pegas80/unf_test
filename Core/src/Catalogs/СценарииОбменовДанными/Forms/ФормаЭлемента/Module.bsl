...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый
		...
		Наименование = НСтр("ka='სინქრონიზაციის სცენარი %1-სთვის';ru='Сценарий синхронизации для %1'");
...
Процедура ВыполнитьОбменДаннымиНаКлиенте()
	...
	Если НомерОбрабатываемойСтроки > КоличествоСтрок Тогда // выход из рекурсии
		...
		Состояние(НСтр("ka='მონაცემები სინქრონიზებულია.';ru='Данные синхронизированы.'"), ?(ВыводитьСостояние, 100, Неопределено));
		Возврат; // выходим
	КонецЕсли;
	...
	СтрокаСообщения = НСтр("ka='%2-ისთვის სრულდება %1';ru='Выполняется %1 для %2'");
