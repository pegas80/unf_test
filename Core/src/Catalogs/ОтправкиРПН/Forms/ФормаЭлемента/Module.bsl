...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Сообщение.Текст = НСтр("ka='ცნობარი არ არის განკუთვნილი ხელით შესავსებად!';ru='Справочник не предназначен для ручного заполнения!'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	...
	ДатаЗакрытия = НСтр("ka='<не завершена>';ru='<не завершена>'");
	ДатаОбновления = НСтр("ka='<გაურკვეველია>';ru='<не определено>'");
	ОбновитьФорму();
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ka='ოქმი';ru='Протокол'") + ИмяФайлаПакетаДоТочки + ".html");
...
Процедура ОбновитьФорму()
	...
	ДатаОбновления = ?(Объект.ДатаПолученияРезультата = ПустаяДата, НСтр("ka='<გაურკვეველია>';ru='<не определено>'"), Объект.ДатаПолученияРезультата);
	...
	Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
		Если Объект.СтатусПроверкиНаПортале = Перечисления.СтатусыОтправки.Сдан Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Промежуточный протокол обработки порталом получен, отчет ждет доставки в управление';ru='Промежуточный протокол обработки порталом получен, отчет ждет доставки в управление'");
		...
		ИначеЕсли ПротоколЗаполнен(Объект.Ссылка) Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Промежуточный протокол обработки порталом получен';ru='Промежуточный протокол обработки порталом получен'");
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		Иначе
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСерый;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Протокол обработки пакета отсутствует';ru='Протокол обработки пакета отсутствует'");
	...
	Иначе
		...
		Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.НеПринят Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='შეცდომების პროტოკოლი მიღებულია';ru='Протокол ошибок получен'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		...
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Сдан Тогда
			Элементы.КартинкаСтатусОтправки.Картинка = БиблиотекаКартинок.ПисьмоПодтверждениеПолучено;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='ანგარიშგების ჩაბარების შესახებ პროტოკოლი მიღებულია';ru='Протокол о сдаче отчета получен'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		КонецЕсли;
	КонецЕсли;
