...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Сообщение.Текст = НСтр("ka='ცნობარი არ არის განკუთვნილი ხელით შესავსებად!';ru='Справочник не предназначен для ручного заполнения!';en='Reference is not designed for manual filling!'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	...
	ДатаЗакрытия = НСтр("ka='<не завершена>';ru='<не завершена>';en='<Not completed>'");
	ДатаОбновления = НСтр("ka='<გაურკვეველია>';ru='<не определено>';en='<Not specified>'");
	ОбновитьФорму();
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ka='ოქმი';ru='Протокол';en='Protocol'") + ИмяФайлаПакетаДоТочки + ".html");
...
Процедура ОбновитьФорму()
	...
	ДатаОбновления = ?(Объект.ДатаПолученияРезультата = ПустаяДата, НСтр("ka='<გაურკვეველია>';ru='<не определено>';en='<Not specified>'"), Объект.ДатаПолученияРезультата);
	...
	Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
		Если Объект.СтатусПроверкиНаПортале = Перечисления.СтатусыОтправки.Сдан Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Промежуточный протокол обработки порталом получен, отчет ждет доставки в управление';ru='Промежуточный протокол обработки порталом получен, отчет ждет доставки в управление';en='The interim report portal treatment received, the report waits for delivery management'");
		...
		ИначеЕсли ПротоколЗаполнен(Объект.Ссылка) Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Промежуточный протокол обработки порталом получен';ru='Промежуточный протокол обработки порталом получен';en='Intermediate protocol processing received portal'");
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		Иначе
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСерый;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='Протокол обработки пакета отсутствует';ru='Протокол обработки пакета отсутствует';en='packet processing protocol offline'");
	...
	Иначе
		...
		Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.НеПринят Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='შეცდომების პროტოკოლი მიღებულია';ru='Протокол ошибок получен';en='The error log is obtained'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		...
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Сдан Тогда
			Элементы.КартинкаСтатусОтправки.Картинка = БиблиотекаКартинок.ПисьмоПодтверждениеПолучено;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ka='ანგარიშგების ჩაბარების შესახებ პროტოკოლი მიღებულია';ru='Протокол о сдаче отчета получен';en='Protocol for the delivery of the report received'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		КонецЕсли;
	КонецЕсли;
