...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьАналитикуПрочихДоходовИРасходов(Объект.Ссылка) Тогда
		Элементы.ГруппаАналитикаУчетаПрочихДоходовИРасходов.Подсказка = НСтр("ka='ბაზაში არის ამ ანგარიშის მოძრაობები! ანალიტიკის შეცვლა არ არის რეკომენდებული!';ru='В базе есть движения по этому счету! Изменение аналитики не рекомендуется!'");
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Если Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.КосвенныеЗатраты") Тогда
		...
		Элементы.СчетЗакрытия.Подсказка = ?(
			...
			НСтр("ka='ავტომატური დახურვის ანგარიში კალენდარული თვის დახურვისას და ბიუჯეტირებისას';ru='Счет автоматического закрытия при закрытии месяца и бюджетировании'"),
			НСтр("ka='ავტომატური დახურვის ანგარიში კალენდარული თვის დახურვისას';ru='Счет автоматического закрытия при закрытии месяца'")
		);
		Элементы.СпособРаспределения.Подсказка = НСтр("ka='გამოშვებული პროდუქციის თვითღირებულებაზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца'"
		);
	ИначеЕсли Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.НезавершенноеПроизводство") Тогда
		...
		Элементы.СпособРаспределения.Подсказка = НСтр("ka='გამოშვებული პროდუქციის თვითღირებულებაზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას არამატერიალური საშუალებებისთვის';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца для нематериальных затрат'"
		);
	ИначеЕсли (ТипСчета <>  ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеДоходы")
		...
		Элементы.СпособРаспределения.Подсказка = ?(
			ИспользоватьБюджетирование,
			НСтр("ka='ფინანსურ შედეგზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას და ბიუჯეტირებისას';ru='Способ автоматического распределения на финансовый результат при закрытии месяца и бюджетировании'"),
			НСтр("ka='კალენდარული თვის დახურვისას ფინანსურ შედეგზე ავტომატური განაწილების მეთოდი';ru='Способ автоматического распределения на финансовый результат при закрытии месяца'")
		);
