...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Простой список';en='simple list'");
	КомандаПечати.ЗаголовокФормы= НСтр("ru='План счетов';en='Chart of accounts'");
	...
	КомандаПечати.Представление = НСтр("ru='С подробными описаниями';en='With detailed descriptions'");
	КомандаПечати.ЗаголовокФормы= НСтр("ru='План счетов (с подробными описаниями)';en='Chart of accounts (with detailed descriptions)'");
	КомандаПечати.СписокФорм    = "ФормаСписка, ФормаВыбора";
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ВыполнитьКомандуПечати";
...
Функция ПечатьПланаСчетов(ВыводитьОписания = Ложь, ПараметрыПриказа = Неопределено) Экспорт
	...
	Иначе
		Шапка.Параметры.ЗаголовокШапки = НСтр("ru='План счетов бухгалтерского учета';en='Chart of accounts'");
	КонецЕсли;
...
Процедура ОбработатьДобавлениеСчетовУчетаНДСВТаможенномСоюзе() Экспорт
	...
	//	ЗаписьЖурналаРегистрации(НСтр("ru='Обновление информационной базы';en='Update the information base'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
	//КонецПопытки;
	//
...
Процедура ПереименоватьСчетНалогаУСН() Экспорт
	...
	Попытка
		...
		Если НЕ РеквизитыСчета.ЗапретитьИспользоватьВПроводках
			...
			И СокрЛП(РеквизитыСчета.Наименование) = НСтр("ru='Единый налог при применении упрощенной системы налогообложения';en='Single tax under the simplified system of taxation'") Тогда

			// Счет не модифицировался пользователем
			// Можно менять наименование
			
			СчетОбъект = СчетСсылка.ПолучитьОбъект();
			СчетОбъект.Наименование = НСтр("ru='Налог при упрощенной системе налогообложения';en='Tax under the simplified system of taxation'");
...
Процедура УстановитьУчетЗатратПоПодразделениям(ВестиУчетЗатратПоПодразделениям, СобытиеЖурналаРегистрации) Экспорт
	...
	Если ВестиУчетЗатратПоПодразделениям Тогда
		...
		ШаблонСообщенияОбУспехе = НСтр("ru='На счете %1 включен учет по подразделениям';en='On account% 1 included accounting units'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		...
		ШаблонСообщенияОбУспехе = НСтр("ru='На счете %1 отключен учет по подразделениям';en='On account of 1% off accounting units'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ОбработатьДобавлениеСчетовУчетаЛизинг() Экспорт
	...
	Для Каждого Счет Из МассивНовыхСчетов Цикл
		...
		Попытка
			...
			СчетОбъект.Записать();
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Обновление информационной базы';en='Update the information base'"), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ОбновитьПараметрыСчета01_К() Экспорт
	...
	Попытка
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(СчетОбъект);
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Обновление информационной базы';en='Update the information base'"), 
