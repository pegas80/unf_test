...
Процедура ОбновитьЗаголовокФормы(Форма, Счет) 
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		ТекстЗаголовка = НСтр("en='Account Description:% 1';ru='Описание счета: %1'");
		Форма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗаголовка, Счет);
	Иначе
		Форма.Заголовок = НСтр("en='By Unknown';ru='Не указан счет'");
	КонецЕсли;
...
Функция ПолучитьОписаниеСчета(Счет) 
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("en='Account% 1% 2 <br>';ru='Счет %1<br>%2'"), 
		Счет.Код, 
		Счет.Наименование);
		...
		Если ПустаяСтрока(СписокСубсчетов) Тогда
			...
			Если УровеньСчета > 0 Тогда
				ТекстСпискаСубсчетов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("en='To open the account following subaccounts% Level 1:';ru='К счету открыты следующие субсчета %1-го уровня:'"),
					УровеньСчета + 1);
			Иначе
				ТекстСпискаСубсчетов = НСтр("en='To account following subaccounts open:';ru='К счету открыты следующие субсчета:'");
			КонецЕсли;
	...
	Если НЕ Счет.Родитель.Пустая() Тогда
		ТекстОписаниеРодителя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("en='Account Description% 1 ""%2""';ru='Описание счета %1 ""%2""'"), 
			Счет.Родитель.Код, 
