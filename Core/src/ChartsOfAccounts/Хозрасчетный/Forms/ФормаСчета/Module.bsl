	...
	Если СтрНайти(Код, ".") > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(РодительПоКоду) Тогда
			...
			ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='План счетов не содержит счета с кодом %1';en='chart of accounts contains the account with code% 1'"),
				КодРодителя));
...
Функция ПолучитьСсылкуНастройкиСубконто(ИдентификаторОграниченияСпискаСубконто)
	...
	Если ИдентификаторОграниченияСпискаСубконто = "УчетЗапасов" Тогда
		ЗаголовокФормы = НСтр("ru='Учет запасов';en='accounting for reserves'");
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетТоваровВРознице" Тогда
		ЗаголовокФормы = НСтр("ru='Учет товаров в рознице';en='Accounting for goods in retail'");
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетДвиженияДенежныхСредств" Тогда
		ЗаголовокФормы = НСтр("ru='Учет движения денежных средств';en='Cash flow accounting'");
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетРасчетовСПерсоналом" Тогда
		ЗаголовокФормы = НСтр("ru='Учет расчетов с персоналом';en='Accounting for settlements with employees'");
	Иначе
		ЗаголовокФормы = НСтр("ru='Раздел учета';en='accounting section'");
	КонецЕсли;
...
Процедура ПредупреждениеОНевозможностиИзмененияСоставаВидовСубконто(Отказ)
	...
	ПоказатьПредупреждение( , НСтр("ru='Состав видов субконто на этом счете определяется настройкой плана счетов.';en='Subkonto species composition of this account is determined by the chart of accounts.'"));
	Отказ = Истина;
...
Процедура ПредупредитьИзменениеНастроекСубконтоПродукция(ОписаниеСубконто)
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройка субконто %1';en='Setting subkonto% 1'"), ПредставлениеСубконто);
...
Процедура УправлениеФормой2()
	...
	Если Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.КосвенныеЗатраты") Тогда
		...
		Элементы.СчетЗакрытия.Подсказка = ?(
			...
			НСтр("ka='ავტომატური დახურვის ანგარიში კალენდარული თვის დახურვისას და ბიუჯეტირებისას';ru='Счет автоматического закрытия при закрытии месяца и бюджетировании';en='Automatic closing of the accounts at the close of the month and budgeting'"),
			НСтр("ka='ავტომატური დახურვის ანგარიში კალენდარული თვის დახურვისას';ru='Счет автоматического закрытия при закрытии месяца';en='Automatic closing of the accounts at the close of the month'")
		);
		Элементы.СпособРаспределения.Подсказка = НСтр("ka='გამოშვებული პროდუქციის თვითღირებულებაზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца';en='automatic distribution method to the cost of output at close of the month'"
		);
	ИначеЕсли Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.НезавершенноеПроизводство") Тогда
		...
		Элементы.СпособРаспределения.Подсказка = НСтр("ka='გამოშვებული პროდუქციის თვითღირებულებაზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას არამატერიალური საშუალებებისთვის';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца для нематериальных затрат';en='automatic distribution method to the cost of manufactured products at the close of the month for the intangible costs'"
		);
	ИначеЕсли (ТипСчета <>  ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеДоходы")
		...
		Элементы.СпособРаспределения.Подсказка = ?(
			ИспользоватьБюджетирование,
			НСтр("ka='ფინანსურ შედეგზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას და ბიუჯეტირებისას';ru='Способ автоматического распределения на финансовый результат при закрытии месяца и бюджетировании';en='automatic distribution method on the financial results at the close of the month and budgeting'"),
			НСтр("ka='კალენდარული თვის დახურვისას ფინანსურ შედეგზე ავტომატური განაწილების მეთოდი';ru='Способ автоматического распределения на финансовый результат при закрытии месяца';en='automatic distribution method on the financial results at the close of the month'")
		);
