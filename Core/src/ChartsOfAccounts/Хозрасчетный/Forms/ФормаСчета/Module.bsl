...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьБюджетирование = ПолучитьФункциональнуюОпцию("ИспользоватьБюджетирование");//+++ Дмгг
	...
	Если СтрНайти(Код, ".") > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(РодительПоКоду) Тогда
			...
			ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("en='chart of accounts contains the account with code% 1';ru='План счетов не содержит счета с кодом %1'"),
				КодРодителя));
...
Функция ПолучитьСсылкуНастройкиСубконто(ИдентификаторОграниченияСпискаСубконто)
	...
	Если ИдентификаторОграниченияСпискаСубконто = "УчетЗапасов" Тогда
		ЗаголовокФормы = НСтр("en='accounting for reserves';ru='Учет запасов'");
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетТоваровВРознице" Тогда
		ЗаголовокФормы = НСтр("en='Accounting for goods in retail';ru='Учет товаров в рознице'");
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетДвиженияДенежныхСредств" Тогда
		ЗаголовокФормы = НСтр("en='Cash flow accounting';ru='Учет движения денежных средств'");
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетРасчетовСПерсоналом" Тогда
		ЗаголовокФормы = НСтр("en='Accounting for settlements with employees';ru='Учет расчетов с персоналом'");
	Иначе
		ЗаголовокФормы = НСтр("en='accounting section';ru='Раздел учета'");
	КонецЕсли;
...
Процедура ПредупреждениеОНевозможностиИзмененияСоставаВидовСубконто(Отказ)
	...
	ПоказатьПредупреждение( , НСтр("en='Subkonto species composition of this account is determined by the chart of accounts.';ru='Состав видов субконто на этом счете определяется настройкой плана счетов.'"));
	Отказ = Истина;
...
Процедура УстановитьВидимостьПредупрежденияУчетПоПодразделениям()
	...
	Если Истина Тогда//ПолучитьФункциональнуюОпцию("ВестиУчетПоПодразделениям") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПредупредитьИзменениеНастроекСубконтоПродукция(ОписаниеСубконто)
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("en='Setting subkonto% 1';ru='Настройка субконто %1'"), ПредставлениеСубконто);
...
Процедура УправлениеФормой2()
	...
	Если Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.КосвенныеЗатраты") Тогда
		...
		Элементы.СчетЗакрытия.Подсказка = ?(
			...
			НСтр("en='Automatic closing of the accounts at the close of the month and budgeting';ka='ავტომატური დახურვის ანგარიში კალენდარული თვის დახურვისას და ბიუჯეტირებისას';ru='Счет автоматического закрытия при закрытии месяца и бюджетировании'"),
			НСтр("en='Automatic closing of the accounts at the close of the month';ka='ავტომატური დახურვის ანგარიში კალენდარული თვის დახურვისას';ru='Счет автоматического закрытия при закрытии месяца'")
		);
		Элементы.СпособРаспределения.Подсказка = НСтр("en='automatic distribution method to the cost of output at close of the month';ka='გამოშვებული პროდუქციის თვითღირებულებაზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца'"
		);
	ИначеЕсли Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.НезавершенноеПроизводство") Тогда
		...
		Элементы.СпособРаспределения.Подсказка = НСтр("en='automatic distribution method to the cost of manufactured products at the close of the month for the intangible costs';ka='გამოშვებული პროდუქციის თვითღირებულებაზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას არამატერიალური საშუალებებისთვის';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца для нематериальных затрат'"
		);
	ИначеЕсли (ТипСчета <>  ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеДоходы")
		...
		Элементы.СпособРаспределения.Подсказка = ?(
			ИспользоватьБюджетирование,
			НСтр("en='automatic distribution method on the financial results at the close of the month and budgeting';ka='ფინანსურ შედეგზე ავტომატური განაწილების მეთოდი კალენდარული თვის დახურვისას და ბიუჯეტირებისას';ru='Способ автоматического распределения на финансовый результат при закрытии месяца и бюджетировании'"),
			НСтр("en='automatic distribution method on the financial results at the close of the month';ka='კალენდარული თვის დახურვისას ფინანსურ შედეგზე ავტომატური განაწილების მეთოდი';ru='Способ автоматического распределения на финансовый результат при закрытии месяца'")
		);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("Запись_ПланСчетовХозрасчетный", Объект.Ссылка);
КонецПроцедуры
//--- Дмгг
