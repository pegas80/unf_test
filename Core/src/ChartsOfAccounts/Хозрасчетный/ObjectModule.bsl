...
Процедура ПередЗаписью(Отказ)
	...
	Если Предопределенный 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("en='You can not change the subordination of predefined accounts!';ka='არ შეიძლება წინასწარგანსაზღვრული ანგარიშების დაქვემდებარების შეცვლა';ru='Нельзя изменять подчиненность предопределенных счетов!'"), ЭтотОбъект, "Родитель", , Отказ);
	КонецЕсли;
