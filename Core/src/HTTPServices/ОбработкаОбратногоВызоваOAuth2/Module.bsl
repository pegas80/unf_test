...
Функция oauth2callbackGET(Запрос)
	...
	Если Не ЗначениеЗаполнено(code) Тогда
		...
		400,
		"Bad Request",
		НСтр("ka='ავტორიზაციის შეცდომა: პარამეტრი ""code"" არ არის';ru='Ошибка авторизации: отсутствует параметр ""code""';en='Authorization Error: there is no option ""code""'"));
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(state) Тогда
		...
		400,
		"Bad Request",
		НСтр("ka='ავტორიზაციის შეცდომა: პარამეტრი ""state""-ი არ არის';ru='Ошибка авторизации: отсутствует параметр ""state""';en='Authorization Error: there is no option ""state""'"));
	КонецЕсли;
	...
	Если Не ПараметрыАвторизации.Свойство("id") Тогда
		...
		400,
		"Bad Request",
		НСтр("ka='ავტორიზაციის შეცდომა: პარამეტრ ""state""-ში გასაღები ""id"" არ არის';ru='Ошибка авторизации: отсутствует ключ ""id"" в параметре ""state""';en='Authorization Error: Missing the key ""id"" parameter in the ""state""'"));
	КонецЕсли;
...
Процедура УстановитьРазделениеСеанса(ПараметрыАвторизации)
	...
	Если Не ПараметрыАвторизации.Свойство("zone") Тогда
		ВызватьИсключение НСтр("ka='პარამეტრ ""state""-ში გასაღები ""zone"" არ არის';ru='Отсутствует ключ ""zone"" в параметре ""state""';en='No ""zone"" key parameter ""state""'");
	КонецЕсли;
