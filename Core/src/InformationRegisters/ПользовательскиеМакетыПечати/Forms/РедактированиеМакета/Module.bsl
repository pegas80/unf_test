...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТолькоОткрытие Тогда
		Заголовок = НСтр("ka='ბეჭდური ფორმის მაკეტის გახსნა';ru='Открытие макета печатной формы'");
	КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() И ТипМакета = "MXL" Тогда
		Элементы.НадписьЗавершениеИзмененияНеВебКлиент.Заголовок = НСтр("ka='მაკეტში საჭირო ცვლილებების შეტანის შემდეგ, დააჭირეთ ღილაკ ""ცვლილების დასრულება""-ს';ru='После внесения необходимых изменений в макет нажмите на кнопку ""Завершить изменение""'");
	КонецЕсли;
...
Процедура УстановитьНазваниеПрограммыДляОткрытияМакета()
	...
	Если ТипФайла = "mxl" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("ka='1С:საწარმო - ფაილებთან მუშაობა';ru='1С:Предприятие - Работа с файлами'");
		АдресПрограммыДляОткрытияМакета = "http://v8.1c.ru/metod/fileworkshop.htm";
	ИначеЕсли ТипФайла = "doc" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("ka='Microsoft Word';ru='Microsoft Word'");
		АдресПрограммыДляОткрытияМакета = "http://office.microsoft.com/ru-ru/word";
	ИначеЕсли ТипФайла = "odt" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("ka='OpenOffice Writer';ru='OpenOffice Writer'");
	...
	СведенияДляЗаполнения.Вставить("ОписаниеДействия", ?(Параметры.ТолькоОткрытие, НСтр("ka='გახსნები';ru='открытия'"), НСтр("ka='ცვლილებების შეტანა';ru='внесения изменений'")));
