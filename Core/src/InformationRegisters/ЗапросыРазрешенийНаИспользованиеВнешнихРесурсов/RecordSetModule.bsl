		...
		Для Каждого Запись Из ЭтотОбъект Цикл
			...
			Если Запись.БезопасныйРежим <> ТекущийБезопасныйРежим Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='უსაფრთხო რეჟიმი %1 მიმდინარე %2-ისგან განსხვავდება';ru='Безопасный режим %1 отличается от текущего %2';en='Safe Mode 1% different from the current 2%'"),
			...
			Если ПрограммныйМодуль <> Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
				...
				Если Запись.БезопасныйРежим <> БезопасныйРежимПрограммногоМодуля Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='პროგრამული მოდულ %1-ისთვის ნებართვების მოთხოვნა ვერ შესრულდება უსაფრთხო რეჟიმიდან %2';ru='Для программного модуля %1 не может быть выполнен запрос разрешений из безопасного режима %2';en='A software module% 1 could not be executed permits a request from safe mode% 2'"),
