...
Функция АбсолютныйКаталогОбменаИнформацией(Знач Корреспондент) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ka='კორესპონდენტ ""%1""-სთვის შეერთების გამართვები არ არის მიცემული.';ru='Не заданы настройки подключения для корреспондента ""%1"".';en='Do not set the connection settings for the correspondent of ""%1"".'"), Строка(Корреспондент));
	КонецЕсли;
...
Функция НастройкиТранспортаWS(Знач Корреспондент) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='კორესპონდენტ %1-სთვის ვებ-სერვისის შეერთების გამართვები არ არის მიცემული.';ru='Не заданы настройки подключения веб-сервиса для корреспондента %1.';en='Do not set the connection settings for Web Service correspondent 1%.'"),
