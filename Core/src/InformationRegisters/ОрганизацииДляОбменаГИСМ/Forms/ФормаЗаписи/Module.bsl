...
Процедура ПолучитьGLNПоИННКППГИСМ(Команда)
	...
	Если Не ЗначениеЗаполнено(РеквизитыОрганизации.ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнено ИНН организации';ru='Не заполнено ИНН организации';en='Not filled INN organization'"), Запись.Организация, "Запись.Организация");
		Возврат;
	КонецЕсли;
