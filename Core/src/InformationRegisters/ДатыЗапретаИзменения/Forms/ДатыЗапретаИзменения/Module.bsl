...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ka='ჩატვირთვის აკრძალვის თარიღებთან სამუშაოდ არასაკმარისი უფლებებია.';ru='Недостаточно прав для работы с датами запрета загрузки.'");
		КонецЕсли;
		
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ka='Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.';ru='Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.'");
		...
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("НетЗапрета").Представление =
			НСтр("ka='არ არის დაყენებული';ru='Не установлено'");
	...
	Иначе
		Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеДатЗапретаИзменения") Тогда
			ВызватьИсключение НСтр("ka='ცვლილების აკრძალვის თარიღებთან სამუშაოდ არასაკმარისი უფლებებია.';ru='Недостаточно прав для работы с датами запрета изменения.'");
		КонецЕсли;
		
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ka='Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.';ru='Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Заданные настройки дат запрета будут автоматически скорректированы.';ru='Заданные настройки дат запрета будут автоматически скорректированы.'") 
		+ Символы.ПС + Символы.ПС + ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ka='დაიხუროს?';ru='Закрыть?'");
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям"
	   И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		ТекстВопроса = НСтр("ka='Отключить все даты запрета, кроме установленных для всех пользователей?';ru='Отключить все даты запрета, кроме установленных для всех пользователей?'");
	...
	ИначеЕсли (ТекущаяУстановкаДатыЗапрета = "ПоПользователям" Или ТекущаяУстановкаДатыЗапрета = "ДляВсехПользователей")
		...
		ТекстВопроса = НСтр("ka='Отключить все установленные даты запрета?';ru='Отключить все установленные даты запрета?'")
...
Процедура СпособУказанияДатыЗапретаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ОбщаяДата" Тогда
		ТекстВопроса = НСтр("ka='Отключить даты запрета, установленные для разделов и объектов?';ru='Отключить даты запрета, установленные для разделов и объектов?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоРазделам"
		...
		ТекстВопроса = НСтр("ka='Отключить даты запрета, установленные для объектов?';ru='Отключить даты запрета, установленные для объектов?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоОбъектам"
		...
		ТекстВопроса = НСтр("ka='Отключить даты запрета, установленные для разделов?';ru='Отключить даты запрета, установленные для разделов?'");
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление)
	   И Не ТекущиеДанные.БезДатыЗапрета Тогда
		...
		Если ДополнительныеПараметры.ДатыЗапретаДляВсехПользователей Тогда
			ТекстВопроса = НСтр("ka='Отключить даты запрета для всех пользователей?';ru='Отключить даты запрета для всех пользователей?'");
		Иначе
			Если ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для ""%1""?';ru='Отключить даты запрета для ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для группы пользователей ""%1""?';ru='Отключить даты запрета для группы пользователей ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для внешнего пользователя ""%1""?';ru='Отключить даты запрета для внешнего пользователя ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для группы внешних пользователей ""%1""?';ru='Отключить даты запрета для группы внешних пользователей ""%1""?'"), ТекущиеДанные.Пользователь);
			Иначе
				ТекстВопроса = НСтр("ka='Отключить даты запрета?';ru='Отключить даты запрета?'")
			КонецЕсли;
		КонецЕсли;
...
Процедура ПрочитатьДанныеПользователя(Контекст, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	...
	Если Контекст.ПоказыватьРазделыТекущегоПользователя Тогда
		Если Контекст.ВсеРазделыБезОбъектов Тогда
			...
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ka='ნაწილი';ru='Раздел'");
			Контекст.Элементы.ДатыЗапрета.Отображение = ОтображениеТаблицы.Список;
			
		Иначе
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ka='ნაწილი, ობიექტი';ru='Раздел, объект'");
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ka='Даты запрета загрузки данных из других программ действуют одинаково для всех пользователей.';ru='Даты запрета загрузки данных из других программ действуют одинаково для всех пользователей.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ka='Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.';ru='Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.'");
		КонецЕсли;
	Иначе
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешены ввод и редактирование любых данных прошлых периодов.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ka='Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.';ru='Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ka='Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.';ru='Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РедактироватьДатуЗапретаВФорме()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='გამოყოფილი სტრიქონები შეუვსებელია:';ru='Выделенные строки не заполнены.'"));
		Возврат;
	КонецЕсли;
	...
	Если ОбновитьВыделение Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ka='შეუვსებელი სტრიქონებიდან გამოყოფა მოხსნილია.';ru='Снято выделение с незаполненных строк.'"));
	Иначе
