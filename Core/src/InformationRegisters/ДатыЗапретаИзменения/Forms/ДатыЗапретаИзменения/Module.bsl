...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ka='ჩატვირთვის აკრძალვის თარიღებთან სამუშაოდ არასაკმარისი უფლებებია.';ru='Недостаточно прав для работы с датами запрета загрузки.';en='Not have permission to work with dates download ban.'");
		КонецЕсли;
		
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ka='Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.';ru='Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.';en='Dates ban download historical data from other programs are turned off in the settings.'");
		...
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("НетЗапрета").Представление =
			НСтр("ka='არ არის დაყენებული';ru='Не установлено';en='Not installed'");
	...
	Иначе
		Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеДатЗапретаИзменения") Тогда
			ВызватьИсключение НСтр("ka='ცვლილების აკრძალვის თარიღებთან სამუშაოდ არასაკმარისი უფლებებია.';ru='Недостаточно прав для работы с датами запрета изменения.';en='Not have permission to work with the dates the ban changes.'");
		КонецЕсли;
		
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ka='Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.';ru='Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.';en='The date of the prohibition of entering and editing data of past periods are disabled in the settings.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='Заданные настройки дат запрета будут автоматически скорректированы.';ru='Заданные настройки дат запрета будут автоматически скорректированы.';en='Presets ban dates will be automatically adjusted.'") 
		+ Символы.ПС + Символы.ПС + ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ka='დაიხუროს?';ru='Закрыть?';en='Close?'");
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям"
	   И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		ТекстВопроса = НСтр("ka='Отключить все даты запрета, кроме установленных для всех пользователей?';ru='Отключить все даты запрета, кроме установленных для всех пользователей?';en='Disable all the dates the ban, other than those stated for all users?'");
	...
	ИначеЕсли (ТекущаяУстановкаДатыЗапрета = "ПоПользователям" Или ТекущаяУстановкаДатыЗапрета = "ДляВсехПользователей")
		...
		ТекстВопроса = НСтр("ka='Отключить все установленные даты запрета?';ru='Отключить все установленные даты запрета?';en='Disable all installed ban date?'")
...
Процедура СпособУказанияДатыЗапретаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ОбщаяДата" Тогда
		ТекстВопроса = НСтр("ka='Отключить даты запрета, установленные для разделов и объектов?';ru='Отключить даты запрета, установленные для разделов и объектов?';en='Disable the date of the ban set for sections and objects?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоРазделам"
		...
		ТекстВопроса = НСтр("ka='Отключить даты запрета, установленные для объектов?';ru='Отключить даты запрета, установленные для объектов?';en='Disable the date of the ban set for objects?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоОбъектам"
		...
		ТекстВопроса = НСтр("ka='Отключить даты запрета, установленные для разделов?';ru='Отключить даты запрета, установленные для разделов?';en='Disable the date of the ban set for partition?'");
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление)
	   И Не ТекущиеДанные.БезДатыЗапрета Тогда
		...
		Если ДополнительныеПараметры.ДатыЗапретаДляВсехПользователей Тогда
			ТекстВопроса = НСтр("ka='Отключить даты запрета для всех пользователей?';ru='Отключить даты запрета для всех пользователей?';en='Disable ban date for all users?'");
		Иначе
			Если ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для ""%1""?';ru='Отключить даты запрета для ""%1""?';en='Disable the date of the ban for the ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для группы пользователей ""%1""?';ru='Отключить даты запрета для группы пользователей ""%1""?';en='Disable the date of the ban for the user group ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для внешнего пользователя ""%1""?';ru='Отключить даты запрета для внешнего пользователя ""%1""?';en='Disable the date of the ban for an external user ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Отключить даты запрета для группы внешних пользователей ""%1""?';ru='Отключить даты запрета для группы внешних пользователей ""%1""?';en='Disable the date of the ban for the group of external users ""%1""?'"), ТекущиеДанные.Пользователь);
			Иначе
				ТекстВопроса = НСтр("ka='Отключить даты запрета?';ru='Отключить даты запрета?';en='Disable ban date?'")
			КонецЕсли;
		КонецЕсли;
...
"Обновите данные формы (клавиша F5).';ka='""%1"" уже есть в списке пользователей."
"Обновите данные формы (клавиша F5).'"), ВыбранноеЗначение));
	...
	Иначе
		ПоказатьПредупреждение(, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" уже есть в списке пользователей.';en='""%1"" already exists in the user list.'"), ВыбранноеЗначение));
	КонецЕсли;
	...
	Если ТекущиеДанные.ЭтоРаздел Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Раздел) Тогда
			Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?';en='Disable all configured for the date of the prohibition of ""%1"" and its facilities?'"), ТекущиеДанные.Раздел);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить дату запрета для раздела ""%1""?';en='Disable the date for the ban of ""%1""?'"), ТекущиеДанные.Раздел);
			КонецЕсли;
		Иначе
			ТекстВопроса = НСтр("ru='Отключить общую дату запрета для всех разделов?';en='Disable date total ban for all sections?'");
		КонецЕсли;
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отключить дату запрета для объекта ""%1""?';en='Disable the date of the ban for the object ""%1""?'"), ТекущиеДанные.Объект);
	КонецЕсли;
		...
		Если ДатаЗапретаУстановлена(ТекущиеДанные, ТекущийПользователь)
		 Или ЭлементыРаздела.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
			ТекстСообщения = НСтр("ru='Для удаления даты запрета по конкретному объекту необходимо выбрать интересующий объект в одном из разделов.';en='To delete the date of a ban on a particular item you need to select the object of interest in one of the sections.'");
...
"Обновите данные формы (клавиша F5).';ka='""%1"" уже есть в списке объектов."
"Обновите данные формы (клавиша F5).'"), ВыбранноеЗначение));
				Возврат;
			Иначе
				ОбновитьЗначенияПрочитанныхСвойств(
	...
	Иначе
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='""%1"" уже есть в списке объектов';en='""%1"" already exists in the object list'"), ВыбранноеЗначение));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ДП='%1'", НСтр("ru='Без запрета';en='without the ban'")));
...
Процедура ПрочитатьДанныеПользователя(Контекст, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	...
	ИначеЕсли Контекст.УстановкаДатыЗапрета = "ПоПользователям" Тогда
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Контекст.Элементы.ТекущийПользовательПредставление.Заголовок = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройка для ""%1"":';en='Setting for ""%1"":'"), 
	...
	Если Контекст.ПоказыватьРазделыТекущегоПользователя Тогда
		Если Контекст.ВсеРазделыБезОбъектов Тогда
			...
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ka='ნაწილი';ru='Раздел';en='Section'");
			Контекст.Элементы.ДатыЗапрета.Отображение = ОтображениеТаблицы.Список;
			
		Иначе
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ka='ნაწილი, ობიექტი';ru='Раздел, объект';en='The section object'");
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='გარე მომხმარებელბის ასარცევად არასაკმარისი უფლებებია';ru='Недостаточно прав для выбора внешних пользователей.';en='Insufficient privileges to select external users.'"));
		КонецЕсли;
	Иначе
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешена загрузка из других программ любых данных прошлых периодов.';en='It allows downloading from other programs any historical data.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ka='Даты запрета загрузки данных из других программ действуют одинаково для всех пользователей.';ru='Даты запрета загрузки данных из других программ действуют одинаково для всех пользователей.';en='Dates ban downloading data from other programs are the same for all users.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ka='Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.';ru='Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.';en='Customization dates you want to load historical data from other programs to selected users.'");
		КонецЕсли;
	Иначе
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешены ввод и редактирование любых данных прошлых периодов.';en='Allowed input and editing of any historical data.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ka='Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.';ru='Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.';en='The date of the prohibition of entering and editing data of past periods are the same for all users.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ka='Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.';ru='Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.';en='Customization dates prohibition of entering and editing data of previous periods for selected users.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РедактироватьДатуЗапретаВФорме()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='გამოყოფილი სტრიქონები შეუვსებელია:';ru='Выделенные строки не заполнены.';en='Dedicated lines are not filled.'"));
		Возврат;
	КонецЕсли;
	...
	Если ОбновитьВыделение Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ka='შეუვსებელი სტრიქონებიდან გამოყოფა მოხსნილია.';ru='Снято выделение с незаполненных строк.';en='Unselect blank rows.'"));
	Иначе
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Объект", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбранные строки (%1)';en='Selected rows (1%)'"), ВыделенныеСтроки.Количество()));
	КонецЕсли;
...
Функция ТекстПредставленияОбщейДаты()
	...
	Возврат "<" + НСтр("ru='Общая дата для всех разделов';en='General data for all sections'") + ">";
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются(Раздел)
	...
	Возврат ?(Раздел <> РазделПустаяСсылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В разделе ""%1"" не предусмотрена установка дат запрета для отдельных объектов.';en='The ""%1"" can not accommodate dates for the prohibition of individual objects.'"), Раздел),
		НСтр("ru='Для установки дат запрета по отдельным объектам выберите один из разделов ниже и нажмите ""Подобрать"".';en='To set the date of the ban on individual objects, select one of the topics below and click ""Choose.""'"));
