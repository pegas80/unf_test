...
Процедура УдалитьСтарыеПакеты() Экспорт
	...
	Если ВсегоПакетов > ПакетовДляОтправки Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='УдалитьСтарыеПакеты';ru='УдалитьСтарыеПакеты';en='UdalitStaryePakety'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
...
Процедура УдалитьПакет(НомерПакета) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='УдалитьПакеты';ru='УдалитьПакеты';en='UdalitPakety'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецПроцедуры
