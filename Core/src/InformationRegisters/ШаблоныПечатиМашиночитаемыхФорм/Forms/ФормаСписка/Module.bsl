...
Процедура ДобавитьШаблоныНаВебКлиентеПоместитьФайлЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ОбъектФайл.Расширение = ".tif" Тогда
		ПоказатьПредупреждение(,НСтр("ka='Выбранный файл не является шаблоном печати машиночитаемой формы.';ru='Выбранный файл не является шаблоном печати машиночитаемой формы.';en='The selected file is not a machine-readable form of the print pattern.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьШаблоныВВебКлиенте(АдресФайлаВоВременномХранилище, КоличествоФайлов, ОбъектФайл, РежимОдногоФайла)
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка шаблона ""%1""...';ru='Загрузка шаблона ""%1""...';en='Loading template ""%1"" ...'"), ОбъектФайл.Имя), , , БиблиотекаКартинок.ЗагрузкаФайла);
	...
	Если НЕ ЗагрузитьШаблоныМашиночитаемыхФорм(АдресФайлаВоВременномХранилище, ОбъектФайл.Имя, КоличествоНезагруженныхШаблонов) Тогда
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შაბლონ ""%1""-ის გადმოტვირთვა ვერ მოხერხდა:%2';ru='Не удалось загрузить шаблон ""%1"":%2';en='Failed to load the template ""%1"":% 2'"), ОбъектФайл.Имя, ОписаниеОшибки()));
...
Процедура ЗаписатьШаблоныНаТонкомКлиенте(КоличествоФайлов, ОбъектыФайл, РежимОдногоФайла)
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Исключение
				Если РежимОдногоФайла Тогда
					...
					Возврат;
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შაბლონ ""%1""-ის გადმოტვირთვა ვერ მოხერხდა:%2';ru='Не удалось загрузить шаблон ""%1"":%2';en='Failed to load the template ""%1"":% 2'"), ОбъектФайл.Имя, Символы.ПС + Символы.ПС + ОписаниеОшибки());
	...
	Состояние(НСтр("ka='შაბლონების ჩაწერა...';ru='Запись шаблонов...';en='Recording templates ...'"), , , БиблиотекаКартинок.Записать);
...
Процедура ПоказатьПредупрежденияПослеЗагрузкиШаблонов(КоличествоНезагруженныхШаблонов, КоличествоФайлов, РежимОдногоФайла)
	...
	Если КоличествоНезагруженныхШаблонов = КоличествоФайлов Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не удалось загрузить ни один из указанных шаблонов.';ru='Не удалось загрузить ни один из указанных шаблонов.';en='Could not load any of the given patterns.'"));
	Иначе
		Если РежимОдногоФайла Тогда
...
Процедура ЗагрузитьШаблоны(АдресВременногоХранилища, КоличествоНезагруженныхШаблонов)
	...
	Для Каждого ШаблонМашиночитаемойФормы Из ШаблоныМашиночитаемыхФорм Цикл
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось записать шаблон ""%1"":%2';ru='Не удалось записать шаблон ""%1"":%2';en='Failed to write the template ""%1"":% 2'"), ШаблонМашиночитаемойФормы.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
...
Процедура Выгрузить(Команда)
	...
	Если ТекСтроки.Количество() <> 0 Тогда
		...
		#КонецЕсли
	Иначе
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выберите шаблоны, которые следует сохранить на диск, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';ru='Выберите шаблоны, которые следует сохранить на диск, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';en='Select the template you want to save to disk, and then try again.% 1 To use the multi-select Ctrl.'"), Символы.ПС));
	КонецЕсли;
...
Процедура ПоказатьПредупреждениПослеВыгрузкиШаблонов(Знач КоличествоОшибок, Знач ТекСтроки)
	...
	Если ТекСтроки.Количество() = 1 Тогда
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='Шаблон успешно выгружен.';ru='Шаблон успешно выгружен.';en='The template was successfully unloaded.'"));
		КонецЕсли;
	Иначе
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='შაბლონების წარმატებით გადმოიტვირთა.';ru='Шаблоны успешно выгружены.';en='Templates successfully unloaded.'"));
		Иначе
			ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Во время сохранения произошли ошибки. Не удалось сохранить шаблонов: %1.';ru='Во время сохранения произошли ошибки. Не удалось сохранить шаблонов: %1.';en='While saving error occurred. Unable to save the template:% 1.'"), КоличествоОшибок));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьШаблоны(ТекСтроки, КоличествоОшибок, ВозвращатьВСтруктуреСсылкуНаШаблонВоВременномХранилище = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შაბლონ %1-ის მიღებისას წარმოიშვა შეცდომა:%2';ru='Во время получения шаблона %1 возникла ошибка:%2';en='During the production of the template% 1 An error occurred:% 2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
...
Процедура УдалитьШаблоныНаКлиенте()
	...
	Иначе
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выберите шаблоны, которые следует удалить, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';ru='Выберите шаблоны, которые следует удалить, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';en='Select the template you want to delete, and then try again.% 1 To use the multi-select Ctrl.'"), Символы.ПС));
...
"Установка компоненты печати машиночитаемых форм невозможна.';ka='Не удалось подключить расширение работы с файлами."
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ТекстВопроса = НСтр("ka='შაბლონი გადმოიტვირთება დისკზე და გაიხსნება. გავაგრძელოთ?';ru='Шаблон будет выгружен на диск и открыт. Продолжить?';en='The template will be swapped out to disk and open. Proceed?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура СписокВыборЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если НЕ АдресВременногоХранилища = Неопределено Тогда
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დისკზე არსებული დროებით ფაილში შაბლონის გადმოტვირთვა ვერ მოხერხდა:%1';ru='Не удалось выгрузить шаблон во временный файл на диске:%1';en='Unable to unload the template to a temporary file on disk: 1%'"), Символы.ПС + ОписаниеОшибки());
...
Функция ПолучитьШаблонНаСервере(ВыбраннаяСтрока)
	...
	Если Выборка.Следующий() Тогда
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შაბლონის მიღება საცავიდან ვერ მოხერხდა:%1';ru='Не удалось получить шаблон из хранилища:%1';en='Unable to get a template from the repository:% 1'"), Символы.ПС + ОписаниеОшибки());
