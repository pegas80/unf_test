...
Процедура ПодготовитьДанныеДляЗаписи() Экспорт
	...
	Если Не ДополнительныеСвойства.Свойство("ПараметрыПолучения", ПараметрыПолучения) Тогда
		ВызватьИсключение НСтр("ka='მონაცემთა მიღების პარამეტრები არ არის დადგენილი';ru='Не определены параметры получения данных';en='Data acquisition parameters are not defined'");
	КонецЕсли;
...
"внутри транзакции записи из сеанса с включенным разделением!';ka='რესურსის დაუშვებელი ცვლილება ინფორმაციის რეგისტრ პროგრამულიინტერფეისებისქეში-ს მოცემული ჩანაწერები ჩართული განცალკევების სეანსიდან ჩანაწერის ტრანზაქციის შიგნით არის!'");
