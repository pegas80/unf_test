...
Процедура ЗаписатьТокенЗапроса(Знач Идентификатор, Знач ТокенЗапроса) Экспорт
	...
	НСтр("ka='პარამეტრ state.id-ში გადაცემული მნიშვნელობა არ არის უნიკალური იდენტიფიკატორი.';ru='Значение, переданное в параметре state.id, не является уникальным идентификатором.';en='The value passed state.id parameter is not a unique identifier.'"));
	...
	Если Выборка.Следующий() И Выборка.КоличествоЗаписей > 1000 Тогда
		ВызватьИсключение НСтр("ka='ავტორიზაციის მცდელობის რაოდენობა გადაჭარბებულია.';ru='Превышено количество попыток авторизации.';en='Exceeded the number of login attempts.'");
	КонецЕсли;
