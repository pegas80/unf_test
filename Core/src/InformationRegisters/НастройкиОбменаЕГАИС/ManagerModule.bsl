...
Функция ДоступныеТранспортныеМодули(Отбор = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеМодуля.Представление = СокрЛП(ДанныеМодуля.Представление) + ?(Выборка.ОбменНаСервере, " " + НСтр("ka='(на сервере 1С:Предприятия)';ru='(на сервере 1С:Предприятия)';en='(On the server 1C: Enterprise)'"), "");
