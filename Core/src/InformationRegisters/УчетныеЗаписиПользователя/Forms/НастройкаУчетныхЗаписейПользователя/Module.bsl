...
Процедура ЗаполнитьСписокУчетныхЗаписей()
	...
	Если ЗначениеЗаполнено(УчетныеЗаписи) Тогда
		...
		Заголовок = НСтр("ka='ჩემი სააღრიცხვო ჩანაწერები';ru='Мои учетные записи'");
	Иначе
		Элементы.УчетныеЗаписи.Видимость = Ложь;
		Заголовок = НСтр("ka='სააღრიცხვო ჩანაწერები არ არის შეერთებული';ru='Учетные записи не подключены'");
	КонецЕсли;
...
Процедура ОтобразитьКомментарий(ТекУчетнаяЗапись)
	...
	Если ТекУчетнаяЗапись.ЗагружатьПочту Тогда
		...
		НСтр("ka='ошибка подключения,ошибки  подключения,ошибок подключения';ru='ошибка подключения,ошибки  подключения,ошибок подключения'"));
	Иначе
		ТекУчетнаяЗапись.Комментарий = НСтр("ka='Отключена из-за множественных ошибок';ru='Отключена из-за множественных ошибок'");
	КонецЕсли;
