...
Функция КурсВалютыПоФормуле(Валюта, Формула, Период)
	...
	Исключение
		...
		Если ОшибкаРасчетаКурсаПоФормуле[Валюта] = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვალუტა ""%1""-ის კურსის გამოთვლა ფორმულა ""%2""-ით არ შესრულდა:';ru='Расчет курса валюты ""%1"" по формуле ""%2"" не выполнен:';en='Calculation of the exchange rate ""%1"" according to the ""%2"" is not executed:'",
			...
			Иначе
				ЗаписьЖурналаРегистрации(НСтр("ka='ვალუტები. ვალუტების კურსის ჩატვირთვა';ru='Валюты.Загрузка курсов валют';en='Valyuty.Zagruzka currency'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
