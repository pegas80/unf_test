...
Функция КурсВалютыПоФормуле(Валюта, Формула, Период)
	...
	Исключение
		...
		Если ОшибкаРасчетаКурсаПоФормуле[Валюта] = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვალუტა ""%1""-ის კურსის გამოთვლა ფორმულა ""%2""-ით არ შესრულდა:';ru='Расчет курса валюты ""%1"" по формуле ""%2"" не выполнен:'",
			...
			Иначе
				ЗаписьЖурналаРегистрации(НСтр("ka='ვალუტები. ვალუტების კურსის ჩატვირთვა';ru='Валюты.Загрузка курсов валют'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
