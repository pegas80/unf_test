...
Процедура ПеренестиВДокумент(Команда)
	...
	Если ПроверитьЗаполнение() Тогда
		...
		Если НайденыНезарегистрированныеТовары.Количество() > 0 Тогда
			...
			ТекстВопроса = НСтр("ka='არა ყველა ახალი შტრიხკოდისთვის არის მითითებული შესაბამისი ნომენკლატურა."
"ეს საქონლები არ იქნება გადატანილი დოკუმენტში."
"გვერდზე გადადეთ ისინი როგორც დაუსკანერებლები.';ru='Не для всех новых штрихкодов указана соответствующая номенклатура."
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Выборка.Следующий() Тогда // Штрихкод уже записан в БД
		...
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + Выборка.НоменклатураПредставление + """"
						...
						+ ?(ЗначениеЗаполнено(Выборка.Партия), " """ + НСтр("ka='პარტიაში';ru='в партии';en='in the party'") + " " + Выборка.ПартияПредставление + """", "")
						);
...
Процедура Отмена(Команда, Отказ = Ложь)
	...
	Если НЕ ОбработкаЗакрытия Тогда
		...
		ТекстВопроса = НСтр("ka='ყველა საქონელი არ იქნება დოკუმენტში გადატანილი. გადადეთ ისინი როგორც დაუსკანერებლები.';ru='Все товары не будут перенесены в документ."
"Отложите их в сторону как неотсканированные.'"
		);
