...
Процедура ПроверитьСуществованиеФайлаВнешнейОбработки(ИмяПроверяемогоФайла, ЭтоФайловаяБаза, Отказ)
	...
	РасположениеКаталога = ? (ЭтоФайловаяБаза, НСтр("ka='კლიენტზე';ru='на клиенте'"), НСтр("ka='სერვერზე';ru='на сервере'"));
	
	Если Не КаталогПроверки.Существует() Тогда
		
		СтрокаСообщения = НСтр("ka='კატალოგი ""%1"" ვერ მოიძებნა %2.';ru='Каталог ""%1"" не найден %2.'");
	...
	ИначеЕсли Не ФайлНаДиске.Существует() Тогда 
		...
		СтрокаСообщения = НСтр("ka='გარე დამუშავების ფაილი ""%1"" ვერ მოიძებნა %2.';ru='Файл внешней обработки ""%1"" не найден %2.'");
...
Процедура ПроверитьДоступностьФайлаПротоколаОбмена(ИмяФайлаПротоколаОбмена, Отказ)
	...
	Если Не КаталогПроверки.Существует() Тогда
		...
		СтрокаСообщения = НСтр("ka='გაცვლის პროტოკოლის ფაილის კატალოგი ""%1"" ვერ მოიძებნა.';ru='Каталог файла протокола обмена ""%1"" не найден.'");
	...
	ИначеЕсли Не СоздатьФайлПроверки(ИмяКаталогаПроверки, ИмяФайлаПроверки) Тогда
		...
		СтрокаСообщения = НСтр("ka='გაცვლის პროტოკოლის საქაღალდეში ფაილის შექმნა ვერ მოხერხდა: ""%1"".';ru='Не удалось создать файл в папке протокола обмена: ""%1"".'");
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ИмяКаталогаПроверки);
		Отказ = Истина;
		
	ИначеЕсли Не УдалитьФайлПроверки(ИмяКаталогаПроверки, ИмяФайлаПроверки) Тогда
		
		СтрокаСообщения = НСтр("ka='გაცვლის პროტოკოლის საქაღალდეში ფაილის წაშლა ვერ მოხერხდა: ""%1"".';ru='Не удалось удалить файл в папке протокола обмена: ""%1"".'");
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ИмяКаталогаПроверки);
		Отказ = Истина;
