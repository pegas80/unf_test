...
Процедура ЗагрузитьПоставляемыеПравила(ИмяПланаОбмена, ИмяФайлаПравил) Экспорт
	...
	Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяФайлаПравил, ИмяВременнойПапки) Тогда
		...
		Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ka='არქივის განფუთვისას ფაილი წესებით ვერ მოიძებნა.';ru='При распаковке архива не найден файл с правилами.';en='When you unpack the archive file is not found with the rules.'");
		КонецЕсли;
		...
		Если СписокРаспакованныхФайлов.Количество() <> 3 Тогда
			ВызватьИсключение НСтр("ka='წესების კომპლექტის არასწორი ფორმატი. არქივში ფაილების მოსალოდნელი რაოდენობა არის სამი. მოსალოდნელია ფაილები:"
"ExchangeRules.xml - კონვერტაციის წესები მიმდინარე პროგრამისთვის;"
"CorrespondentExchangeRules.xml - კონვერტაციის წესები კორესპონდენტი პროგრამისთვის;"
"RegistrationRules.xml - რეგისტრაციის წესები მიმდინარე პროგრამისთვის.';ru='Не верный формат комплекта правил. Ожидаемое количество файлов в архиве - три. Ожидаются файлы:"
...
"RegistrationRules.xml - правила регистрации для текущей программы.'");
		...
		Для Каждого ПолученныйФайл Из СписокРаспакованныхФайлов Цикл
			...
			Иначе
				ВызватьИсключение НСтр("ka='არქივში ფაილების სახელები მოსალოდნელს არ შეესაბამება. მოსალოდნელია ფაილები:"
"ExchangeRules.xml - კონვერტაციის წესები მიმდინარე პროგრამისთვის;"
"CorrespondentExchangeRules.xml - კონვერტაციის წესები პროგრამა-კორესპონდენტისთვის;"
"RegistrationRules.xml - რეგისტრაციის წესები მიმდინარე პროგრამისთვის.';ru='Имена файлов в архиве не соответствуют ожидаемым. Ожидаются файлы:"
"ExchangeRules.xml - правила конвертации для текущей программы;"
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента;"
"RegistrationRules.xml - правила регистрации для текущей программы.'");
			КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='წესების არქივის განფუთვა ვერ მოხერხდა.';ru='Не удалось распаковать архив с правилами.';en='Unable to unzip the archive with the rules.'");
	КонецЕсли;
	...
	Если ЗагрузкаПравилРегистрации.ФлагОшибки Тогда
		ВызватьИсключение НСтр("ka='შეცდომა რეგისტრაციის წესების ჩატვირთვისას.';ru='Ошибка при загрузке правил регистрации.';en='Error loading the registration rules.'");
	КонецЕсли;
...
Процедура УдалитьПоставляемыеПравила(ИмяПланаОбмена) Экспорт
	...
	Для Каждого ВидПравил Из Перечисления.ВидыПравилДляОбменаДанными Цикл
		...
		Если ЕстьОшибки Тогда
			ВызватьИсключение НСтр("ka='შეცდომა კონფიგურაციიდან წესების ჩატვირთვისას.';ru='Ошибка при загрузке правил из конфигурации.';en='Error loading the configuration rules.'");
		Иначе
...
Процедура ЗагрузитьПравила(Отказ, Запись, АдресВременногоХранилища = "", ИмяФайлаПравил = "", ЭтоАрхив = Ложь) Экспорт
	...
	Если СписокПлановОбмена.НайтиПоЗначению(Запись.ИмяПланаОбмена) = Неопределено Тогда
		НСтрока = НСтр("ru='План обмена %1 не используется для синхронизации данных, правила не обновлены.';en='Plan 1% exchange is not used for data synchronization, the rules are not updated.'");
	...
	Если ЭтоАрхив Тогда
		...
		Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяВременногоАрхива, ИмяВременнойПапки) Тогда
			...
			// В архиве не было ни одного файла - отказываемся от загрузки.
			Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
				НСтрока = НСтр("ka='არქივის განფუთვისას ფაილი წესებით ვერ მოიძებნა.';ru='При распаковке архива не найден файл с правилами.';en='When you unpack the archive file is not found with the rules.'");
				ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
			КонецЕсли;
			...
			Если ЭтоПравилаКонвертации Тогда
				...
				Если СписокРаспакованныхФайлов.Количество() = 2 Тогда
					...
					Иначе
						...
						НСтрока = НСтр("ka='არქივში ფაილების სახელი მოსალოდნელს არ შეესაბამება. მოსალოდნელია ფაილები:"
"ExchangeRules.xml - კონვერტაციის წესები მიმდინარე პროგრამისთვის;"
"CorrespondentExchangeRules.xml - კონვერტაციის წესები კორესპონდენტი პროგრამისთვის.';ru='Имена файлов в архиве не соответствуют ожидаемым. Ожидаются файлы:"
"ExchangeRules.xml - правила конвертации для текущей программы;"
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.'");
						ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				...
				ИначеЕсли СписокРаспакованныхФайлов.Количество() = 1 Тогда
					НСтрока = НСтр("ka='არქივში მოიძებნა კონვერტაციის წესების ერთი ფაილი. არქივში ფაილების მოსალოდნელი რაოდენობა არის ორი. მოსალოდნელია ფაილები:"
"ExchangeRules.xml - კონვერტაციის წესები მიმდინარე პროგრამისთვის;"
"CorrespondentExchangeRules.xml - კონვერტაციის წესები კორესპონდენტი პროგრამისთვის.';ru='В архиве найден один файл правил конвертации. Ожидаемое количество файлов в архиве - два. Ожидаются файлы:"
"ExchangeRules.xml - правила конвертации для текущей программы;"
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				// В архиве оказалось несколько файлов, хотя должен быть один - отказываемся от загрузки.
				ИначеЕсли СписокРаспакованныхФайлов.Количество() > 1 Тогда
					НСтрока = НСтр("ka='არქივის განფუთვისას მოიძებნა რამდენიმე ფაილი. უნდა იყოს მხოლოდ ერთი ფაილი წესებითურთ.';ru='При распаковке архива найдено несколько файлов. Должен быть только один файл с правилами.';en='When you unpack the archive found several files. There must be only one file with the rules.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				КонецЕсли;
		...
		Иначе // Если не удалось распаковать файл - отказываемся от загрузки.
			НСтрока = НСтр("ka='წესების არქივის განფუთვა ვერ მოხერხდა.';ru='Не удалось распаковать архив с правилами.';en='Unable to unzip the archive with the rules.'");
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
		КонецЕсли;
...
Процедура ЗагрузитьКомплектПравил(Отказ, ДанныеДляЗаписи, ОписаниеОшибки, АдресВременногоХранилища = "", ИмяФайлаПравил = "") Экспорт
	...
	Если ЗаписьПравилКонвертации.ИсточникПравил = Перечисления.ИсточникиПравилДляОбменаДанными.Файл Тогда
		...
		Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяВременногоАрхива, ИмяВременнойПапки) Тогда
			...
			// В архиве количество файлов не соответствует ожидаемому - отказываемся от загрузки.
			Если СписокРаспакованныхФайлов.Количество() <> 3 Тогда
				НСтрока = НСтр("ka='წესების კომპლექტის არასწორი ფორმატი. არქივში ფაილების მოსალოდნელი რაოდენობა არის სამი. მოსალოდნელია ფაილები:"
"ExchangeRules.xml - კონვერტაციის წესები მიმდინარე პროგრამისთვის;"
"CorrespondentExchangeRules.xml - კონვერტაციის წესები კორესპონდენტი პროგრამისთვის;"
"RegistrationRules.xml - რეგისტრაციის წესები მიმდინარე პროგრამისთვის.';ru='Не верный формат комплекта правил. Ожидаемое количество файлов в архиве - три. Ожидаются файлы:"
"ExchangeRules.xml - правила конвертации для текущей программы;"
...
"RegistrationRules.xml - правила регистрации для текущей программы.'");
			...
			Для Каждого ПолученныйФайл Из СписокРаспакованныхФайлов Цикл
				...
				ИначеЕсли ПолученныйФайл.Имя ="RegistrationRules.xml" Тогда
					ДвоичныеДанныеРегистрации = Новый ДвоичныеДанные(ПолученныйФайл.ПолноеИмя);
				Иначе
					НСтрока = НСтр("ka='არქივში ფაილების სახელები მოსალოდნელს არ შეესაბამება. მოსალოდნელია ფაილები:"
"ExchangeRules.xml - კონვერტაციის წესები მიმდინარე პროგრამისთვის;"
"CorrespondentExchangeRules.xml - კონვერტაციის წესები პროგრამა-კორესპონდენტისთვის;"
"RegistrationRules.xml - რეგისტრაციის წესები მიმდინარე პროგრამისთვის.';ru='Имена файлов в архиве не соответствуют ожидаемым. Ожидаются файлы:"
"ExchangeRules.xml - правила конвертации для текущей программы;"
...
Функция ПолучитьДвоичныеДанныеИзМакетаКонфигурации(Отказ, ИмяПланаОбмена, ИмяМакета)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ka='კონფიგურაციის მაკეტ %1-ის მიღების შეცდომა გაცვლის გეგმა %2-ისთვის';ru='Ошибка получения макета конфигурации %1 для плана обмена %2';en='Error obtain configuration layout 1% to 2% of the target exchange'");
...
Процедура ВыполнитьПроверкуЗаполненияПолей(Отказ, Запись)
	...
	Если ПустаяСтрока(Запись.ИмяПланаОбмена) Тогда
		...
		НСтрока = НСтр("ka='მიუთითეთ გაცვლის გეგმა.';ru='Укажите план обмена.';en='Specify an exchange plan.'");
	...
	ИначеЕсли Запись.ИсточникПравил = Перечисления.ИсточникиПравилДляОбменаДанными.МакетКонфигурации
		...
		НСтрока = НСтр("ka='მიუთითეთ ტიპიური წესები.';ru='Укажите типовые правила.';en='Specify the model rules.'");
...
Функция ИнформацияОПравилахИзФайла(ИмяФайлаПравил)
	...
	Если Не ((ПравилаОбмена.ЛокальноеИмя = "ПравилаОбмена") И (ПравилаОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента)) Тогда
		ВызватьИсключение НСтр("ka='გაცვლის წესების ფორმატის შეცდომა';ru='Ошибка формата правил обмена';en='Format error exchange rules'");
	КонецЕсли;
...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки, ДанныеОПравилах)
	...
	Если ИмяКонфигурацииИзПравил <> ИмяКонфигурацииИнформационнойБазы Тогда
		...
		ОписаниеОшибки.Вставить("ТекстОшибки", НСтр("ka='წესები არ შეიძლება ჩაიტვირთოს, ვინაიდან ისინი განკუთვნილია პროგრამა ""%1""-ისთვის. საჭიროა წესების გამოყენება კონფიგურაციიდან ან ფაილიდან წესების კორექტული კომპლექტის ჩატვირთვა.';ru='Правила не могут быть загружены, т.к. они предназначены для программы ""%1"". Следует использовать правила из конфигурации или загрузить корректный комплект правил из файла.';en='Rules can not be uploaded, because the they are intended for the program ""%1"". It is necessary to use the rules of the configuration, or download the correct set of rules from a file.'"));
	...
	Если РезультатСравнения <> 0 Тогда
		...
		Если РезультатСравнения < 0 Тогда
			...
			ТекстОшибки = НСтр("ka='მონაცემთა სინქრონიზაციამ შეიძლება არაკორექტულად იმუშაოს, ვინაიდან ჩასატვირთი წესები განკუთვნილია პროგრამა ""%1""-ის (%2) წინა ვერსიისთვის. რეკომენდებულია კონფიგურაციიდან წესების გამოყენება ან პროგრამის მიმდინარე ვერსიისთვის განკუთვნილი წესების კომპლექტის ჩატვირთვა (%3).';ru='Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).';en='Data synchronization may not work properly as downloadable rules designed for a previous version ""%1"" program (2%). It is recommended to use the rules of the configuration, or download a set of rules designed for the current version of the program (3%).'");
			ВидОшибки = "УстаревшаяВерсияКонфигурации";
			
		Иначе
			
			ТекстОшибки = НСтр("ka='მონაცემთა სინქრონიზაციამ შეიძლება არაკორექტულად იმუშაოს, ვინაიდან გადმოსატვირთი წესები განკუთვნილია პროგრამა ""%1""-ის (%2) უფრო ახალი ვერსიისთვის. რეკომენდებულია პროგრამის ვერსიის განახლება ან პროგრამის მიმდინარე ვერსიისთვის (%3) განკუთვნილი წესების კომპლექტის გამოყენება.';ru='Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).';en='Data synchronization may not work properly as downloadable rules designed for a newer version ""%1"" program (2%). It is recommended to update the version of the program or use a set of rules designed for the current version of the program (3%).'");
