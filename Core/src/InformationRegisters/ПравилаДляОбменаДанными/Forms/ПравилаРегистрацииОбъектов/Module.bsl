...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ka='მიუთითეთ, თუ რომელი ფაილიდან უნდა ჩაიტვირთოს წესები';ru='Укажите, из какого файла загрузить правила'"));
	ПараметрыДиалога.Вставить("Фильтр",
		  НСтр("ka='რეგისტრაციის წესების ფაილები (*.xml)';ru='Файлы правил регистрации (*.xml)'") + "|*.xml|"
		+ НСтр("ka='არქივები ZIP (*.zip)';ru='Архивы ZIP (*.zip)'")   + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	ФильтрИмени = НСтр("ka='წესების ფაილები (*.xml)';ru='Файлы правил (*.xml)'") + "|*.xml";
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("ka='რეგისტრაციის წესები';ru='Правила регистрации'");
	Иначе
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("ka='სერვერზე ფაილის გადაცემის შეცდომა';ru='Ошибка передачи файла на сервер'");
	КонецЕсли;
