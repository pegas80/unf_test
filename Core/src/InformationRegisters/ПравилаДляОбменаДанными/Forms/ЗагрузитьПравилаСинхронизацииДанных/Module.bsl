...
"или найти в %2'");
	...
	ШаблонКаталогОбновлений = НСтр("ka='პროგრამა ""%1""-ის მოწოდების კატალოგში';ru='каталоге поставки программы ""%1""'");
	ШаблонКаталогОбновлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКаталогОбновлений, ИмяПрограммы);
	
	ШаблонПользовательскийСайт = НСтр("ka='სისტემა ""1С:საწარმო 8""-ის მომხმარებლების მხარდაჭერის საიტს';ru='сайта поддержки пользователей системы ""1С:Предприятие 8""'");
	...
	Если РежимОтладки Тогда
		...
		Если РежимОтладкиВыгрузки Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ka='გარე დამუშავების ფაილის სახელი არ არის მიცემული.';ru='Не задано имя файла внешней обработки.'");
		...
		Если РежимПротоколированияОбменаДанными Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ka='გაცვლის პროტოკოლის ფაილის სახელი არ არის მიცემული.';ru='Не задано имя файла протокола обмена.'");
...
Процедура ВнешняяОбработкаДляОтладкиВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ka='Внешняя обработка(*.epf)';ru='Внешняя обработка(*.epf)'") + "|*.epf" );
...
Процедура ФайлПротоколаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИзмененыНастройкиРежимаОтладки = Истина;
	НастройкиДиалога = Новый Структура;
	НастройкиДиалога.Вставить("Фильтр", НСтр("ka='სატექსტო დოკუმენტი (*.txt)';ru='Текстовый документ(*.txt)'")+ "|*.txt" );
...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ka='Укажите архив с правилами обмена';ru='Укажите архив с правилами обмена'"));
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ka='არქივები ZIP (*.zip)';ru='Архивы ZIP (*.zip)'") + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("ka='კონვერტაციის წესები';ru='Правила конвертации'");
	Иначе
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ka='Укажите в какой файл выгрузить правила';ru='Укажите в какой файл выгрузить правила'") );
...
"Использовать типовые правила конвертации?'");
			...
			Кнопки.Добавить("Использовать", НСтр("ka='გამოყენება';ru='Использовать'"));
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Состояние(НСтр("ka='ხორციელდება საინფორმაციო ბაზაში წესების ჩატვირთვა...';ru='Выполняется загрузка правил в информационную базу...'"));
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			Кнопки.Добавить("Отмена", НСтр("ka='დახურვა';ru='Закрыть'"));
		Иначе
		...
		ПараметрыФормы.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ka='Правила не могут быть загружены';ru='Правила не могут быть загружены'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ka='Синхронизация данных может работать некорректно';ru='Синхронизация данных может работать некорректно'");
		КонецЕсли;
...
"Перейти в журнал регистрации?'");
	...
	Иначе
		ПоказатьОповещениеПользователя(,, НСтр("ka='წესები წარმატებით ჩაიტვირთა საინფორმაციო ბაზაში';ru='Правила успешно загружены в информационную базу.'"));
		Закрыть();
	КонецЕсли;
...
Функция ПолучитьАдресВременногоХранилищаАрхиваПравилНаСервере()
	...
	Если Результат.Пустой() Тогда
		...
		НСтрока = НСтр("ka='ვერ მოხერხდა გაცვლის წესების მიღება.';ru='Не удалось получить правила обмена.'");
...
Процедура ОбновитьИнформациюОПравилах()
	...
	Если ИсточникПравил = "ЗагруженныеИзФайла" Тогда
		ИнформацияОбИспользовании = НСтр("ka='გამოიყენება ფაილიდან ჩატვირთული წესები.';ru='Используются правила загруженные из файла.'");
	Иначе
		ИнформацияОбИспользовании = НСтр("ka='კონფიგურაციის შემადგენლობიდან გამოიყენება ტიპიური წესები.';ru='Используются типовые правила из состава конфигурации.'");
	КонецЕсли;
