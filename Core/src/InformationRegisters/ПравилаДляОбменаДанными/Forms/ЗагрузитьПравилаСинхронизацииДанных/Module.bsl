...
Процедура ПриОткрытии(Отказ)
	...
	ШаблонПодсказки = НСтр("ka='წესების კომპლექტის გადმოწერა შეიძლება %1-იდან ან %2-ში მოძებნა';ru='Комплект правил можно скачать с %1"
"или найти в %2'");
	
	ШаблонКаталогОбновлений = НСтр("ka='პროგრამა ""%1""-ის მოწოდების კატალოგში';ru='каталоге поставки программы ""%1""';en='catalog delivery ""%1"" program'");
	ШаблонКаталогОбновлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКаталогОбновлений, ИмяПрограммы);
	
	ШаблонПользовательскийСайт = НСтр("ka='სისტემა ""1С:საწარმო 8""-ის მომხმარებლების მხარდაჭერის საიტს';ru='сайта поддержки пользователей системы ""1С:Предприятие 8""';en='support system users of the site ""1C: Enterprise 8""'");
	...
	Если РежимОтладки Тогда
		...
		Если РежимОтладкиВыгрузки Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ka='გარე დამუშავების ფაილის სახელი არ არის მიცემული.';ru='Не задано имя файла внешней обработки.';en='Not defined external processing of the file name.'");
		...
		Если РежимПротоколированияОбменаДанными Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ka='გაცვლის პროტოკოლის ფაილის სახელი არ არის მიცემული.';ru='Не задано имя файла протокола обмена.';en='Not defined communications protocol file name.'");
...
Процедура ВнешняяОбработкаДляОтладкиВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ka='Внешняя обработка(*.epf)';ru='Внешняя обработка(*.epf)';en='External Processing (*. Epf)'") + "|*.epf" );
...
Процедура ФайлПротоколаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИзмененыНастройкиРежимаОтладки = Истина;
	НастройкиДиалога = Новый Структура;
	НастройкиДиалога.Вставить("Фильтр", НСтр("ka='სატექსტო დოკუმენტი (*.txt)';ru='Текстовый документ(*.txt)';en='Text Document (*. Txt)'")+ "|*.txt" );
...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ka='Укажите архив с правилами обмена';ru='Укажите архив с правилами обмена';en='Specify a file with exchange rules'"));
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ka='არქივები ZIP (*.zip)';ru='Архивы ZIP (*.zip)';en='Archives ZIP (* .zip)'") + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("ka='კონვერტაციის წესები';ru='Правила конвертации';en='rules of conversion'");
	Иначе
...
"Использовать типовые правила конвертации?';ka='Правила из файла не загружены. Закрытие приведет к использованию типовых правил конвертации."
"Использовать типовые правила конвертации?'");
			
			Оповещение = Новый ОписаниеОповещения("ЗакрытьФормуЗагрузкиПравил", ЭтотОбъект);
			
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Использовать", НСтр("ka='გამოყენება';ru='Использовать';en='Use'"));
...
"RegistrationRules.xml - правила регистрации для текущей программы.';ka='წესების კომპლექტის ფაილის არაკორექტული ფორმატი. მოსალოდნელია zip არქივი, რომელიც სამ ფაილს შეიცავს:"
...
"RegistrationRules.xml - რეგისტრაციის წესები მიმდინარე პროგრამისთვის.'"));
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			Кнопки.Добавить("Отмена", НСтр("ka='დახურვა';ru='Закрыть';en='close'"));
		Иначе
		...
		ПараметрыФормы.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ka='Правила не могут быть загружены';ru='Правила не могут быть загружены';en='Rules can not be downloaded'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ka='Синхронизация данных может работать некорректно';ru='Синхронизация данных может работать некорректно';en='Data synchronization may not work properly'");
		КонецЕсли;
...
Функция ПолучитьАдресВременногоХранилищаАрхиваПравилНаСервере()
	...
	Если Результат.Пустой() Тогда
		...
		НСтрока = НСтр("ka='ვერ მოხერხდა გაცვლის წესების მიღება.';ru='Не удалось получить правила обмена.';en='Unable to obtain exchange rules.'");
...
Процедура ОбновитьИнформациюОПравилах()
	...
	Если ИсточникПравил = "ЗагруженныеИзФайла" Тогда
		ИнформацияОбИспользовании = НСтр("ka='გამოიყენება ფაილიდან ჩატვირთული წესები.';ru='Используются правила загруженные из файла.';en='Use rules loaded from a file.'");
	Иначе
		ИнформацияОбИспользовании = НСтр("ka='კონფიგურაციის შემადგენლობიდან გამოიყენება ტიპიური წესები.';ru='Используются типовые правила из состава конфигурации.';en='Uses standard rules of the configuration structure.'");
	КонецЕсли;
