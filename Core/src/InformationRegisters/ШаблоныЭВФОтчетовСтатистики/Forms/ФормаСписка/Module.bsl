...
Процедура ДобавитьШаблоныНаТонкомКлиенте()
	...
	Если ХотяБыОдинШаблонПрисутствуетВИБ(КороткиеИменаФайлов) Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстВопроса), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ЗаписатьШаблоныНаТонкомКлиенте(КоличествоФайлов, ОбъектыФайл, РежимОдногоФайла)
	...
	Для Каждого ОбъектФайл Из ОбъектыФайл Цикл
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка XML-шаблона ""%1""...';ru='Загрузка XML-шаблона ""%1""...';en='Loading XML-template ""%1"" ...'"), ОбъектФайл.Имя), , , БиблиотекаКартинок.ЗагрузкаФайла);
			...
			Исключение
				...
				Если РежимОдногоФайла тогда
					ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить XML-шаблон ""%1"":%2';ru='Не удалось загрузить XML-шаблон ""%1"":%2';en='Failed to load the XML-template ""%1"":% 2'"), ОбъектФайл.Имя, ОписаниеОшибки()));
					Возврат;
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить XML-шаблон ""%1"":%2';ru='Не удалось загрузить XML-шаблон ""%1"":%2';en='Failed to load the XML-template ""%1"":% 2'"), ОбъектФайл.Имя, Символы.ПС + Символы.ПС + ОписаниеОшибки());
	...
	Состояние(НСтр("ka=' XML-შაბლონების ჩაწერა';ru='Запись XML-шаблонов...';en='Writing XML-templates ...'"), , , БиблиотекаКартинок.Записать);
...
Процедура ДобавитьШаблоныНаВебКлиентеПоместитьФайлЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ОбъектФайл.Расширение = ".xml" Тогда
		ПоказатьПредупреждение(,НСтр("ka='Выбранный файл не является XML-шаблоном электронной версии формы отчета статистики.';ru='Выбранный файл не является XML-шаблоном электронной версии формы отчета статистики.';en='The selected file is not XML-structured electronic version of the form statistics report.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьПредупрежденияПослеЗагрузкиШаблонов(КоличествоНезагруженныхШаблонов, КоличествоФайлов, РежимОдногоФайла)
	...
	Если КоличествоНезагруженныхШаблонов = КоличествоФайлов Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не удалось загрузить ни один из указанных XML-шаблонов.';ru='Не удалось загрузить ни один из указанных XML-шаблонов.';en='Could not load any of these XML-templates.'"));
	Иначе
		Если РежимОдногоФайла Тогда
		...
		КонецЕсли;
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстПредупреждения));
...
Процедура ЗагрузитьШаблоныИзКонфигурации(АдресВременногоХранилища, Интерактивно, КоличествоНезагруженныхШаблонов, КоличествоФайлов, РежимОдногоФайла)
	...
	Если Интерактивно Тогда
		...
		Иначе
			...
			ПоказатьПредупреждение(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстПредупреждения));
...
Процедура Выгрузить(Команда)
			...
			Если Диалог.Выбрать() Тогда
				...
				Для Каждого ШаблонЭВФ Из ШаблоныЭВФ Цикл
					...
					Иначе
						...
						Исключение
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილ %1-ის შენახვისას წარმოიშვა შეცდომა:%2';ru='Во время сохранения файла %1 возникла ошибка:%2';en='While saving the file% 1 An error occurred:% 2'"), КороткоеИмяФайла, Символы.ПС + ОписаниеОшибки());
							Сообщение.Сообщить();
				...
				Если СписокСуществующихОбъектов.Количество() = 1 Тогда
					...
					КороткоеИмяФайла = ШаблонЭВФ.ИмяФайлаШаблона;
					ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='არჩეულ კატალოგში უკვე არსებობს ფაილი %1.%2შევცვალოთ მითითებული ფაილი?';ru='В выбранном каталоге уже существует файл %1.%2Заменить указанный файл?';en='The selected directory already exists the file% 1% 2Zamenit specified file?'"),
				...
				ИначеЕсли СписокСуществующихОбъектов.Количество() > 1 Тогда
					...
					СписокСуществующихОбъектов.ПоказатьОтметкуЭлементов(ОписаниеОповещения, НСтр("ka='Файлы существуют. Отметьте файлы для замены.';ru='Файлы существуют. Отметьте файлы для замены.';en='existing file. Mark files to be replaced.'"));
				Иначе
	...
	Иначе
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='აირჩიეთ XML-შაბლონები, რომლებიც დისკზე უნდა იქნას შენახული, და გაიმეორეთ მცდელობა.%1მრავლობითი არჩევისთვის გამოიყენეთ ღილაკი Ctrl.';ru='Выберите XML-шаблоны, которые следует сохранить на диск, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';en='Select XML-templates to be saved to disk, and then try again.% 1 To use the multi-select Ctrl.'"), Символы.ПС));
...
Процедура ПоказатьПредупреждениПослеВыгрузкиШаблонов(Знач КоличествоОшибок, Знач ТекСтроки)
	...
	Если ТекСтроки.Количество() = 1 Тогда
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='XML-შაბლონი წარმატებით გადმოიტვირთა.';ru='XML-шаблон успешно выгружен.';en='XML-template was successfully unloaded.'"));
		КонецЕсли;
	Иначе
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='XML-შაბლონები წარმატებით არის გადმოტვირთული.';ru='XML-шаблоны успешно выгружены.';en='XML-templates successfully unloaded.'"));
		Иначе
			ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Во время сохранения произошли ошибки. Не удалось сохранить XML-шаблонов: %1.';ru='Во время сохранения произошли ошибки. Не удалось сохранить XML-шаблонов: %1.';en='While saving error occurred. Failed to save XML-templates:% 1.'"), КоличествоОшибок));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьШаблоны(ТекСтроки, КоличествоОшибок, ВозвращатьВСтруктуреСсылкуНаШаблонВоВременномХранилище = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XMK-შაბლონის %1 მიღებისას წარმოიშვა შეცდომა:%2';ru='Во время получения XML-шаблона %1 возникла ошибка:%2';en='During the production of XML-template% 1 An error occurred:% 2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
...
Процедура УдалитьШаблоныНаКлиенте()
	...
	Иначе
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='აირჩიეთ XML-შაბლონები, რომლებიც წასაშლელია, და განმეორებით სცადეთ.%1მრავლობითი არჩევისთვის გამოიყენეთ ღილაკი Ctrl.';ru='Выберите XML-шаблоны, которые следует удалить, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';en='Select XML-templates that must be removed, and then try again.% 1 To use the multi-select Ctrl.'"), Символы.ПС));
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ТекстВопроса = НСтр("ka='XML-шаблон будет выгружен на диск и открыт. Продолжить?';ru='XML-шаблон будет выгружен на диск и открыт. Продолжить?';en='XML-template will be downloaded to disk and open. Proceed?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если НЕ АдресВременногоХранилища = Неопределено Тогда
			...
			Исключение
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის გადმოტვირთვა დისკზე არსებულ დროებით ფაილში ვერ მოხერხდა:%1';ru='Не удалось выгрузить XML-шаблон во временный файл на диске:%1';en='Unable to unload the XML-template to a temporary file on disk: 1%'"), Символы.ПС + ОписаниеОшибки());
				Сообщение.Сообщить();
...
Функция ПолучитьШаблонНаСервере(ВыбраннаяСтрока)
	...
	Если Выборка.Следующий() Тогда
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის მიღება საცავიდან ვერ მოხერხდა:%1';ru='Не удалось получить XML-шаблон из хранилища:%1';en='Unable to retrieve XML-template repository:% 1'"), Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
...
Процедура ПереходНаСайтРосстатаНажатие(Элемент)
	...
	Исключение
		ПоказатьПредупреждение(, НСтр("ka='Не удалось открыть страницу!';ru='Не удалось открыть страницу!';en='Could not open the page!'"));
	КонецПопытки;
КонецПроцедуры
