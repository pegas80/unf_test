...
Процедура ЗаписатьШаблоныНаТонкомКлиенте(КоличествоФайлов, ОбъектыФайл, РежимОдногоФайла)
	...
	Для Каждого ОбъектФайл Из ОбъектыФайл Цикл
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка XML-шаблона ""%1""...';ru='Загрузка XML-шаблона ""%1""...'"), ОбъектФайл.Имя), , , БиблиотекаКартинок.ЗагрузкаФайла);
			...
			Исключение
				...
				Если РежимОдногоФайла тогда
					ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить XML-шаблон ""%1"":%2';ru='Не удалось загрузить XML-шаблон ""%1"":%2'"), ОбъектФайл.Имя, ОписаниеОшибки()));
					Возврат;
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить XML-шаблон ""%1"":%2';ru='Не удалось загрузить XML-шаблон ""%1"":%2'"), ОбъектФайл.Имя, Символы.ПС + Символы.ПС + ОписаниеОшибки());
	...
	Состояние(НСтр("ka=' XML-შაბლონების ჩაწერა';ru='Запись XML-шаблонов...'"), , , БиблиотекаКартинок.Записать);
...
Процедура ДобавитьШаблоныНаВебКлиентеПоместитьФайлЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ОбъектФайл.Расширение = ".xml" Тогда
		ПоказатьПредупреждение(,НСтр("ka='Выбранный файл не является XML-шаблоном электронной версии формы отчета статистики.';ru='Выбранный файл не является XML-шаблоном электронной версии формы отчета статистики.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьПредупрежденияПослеЗагрузкиШаблонов(КоличествоНезагруженныхШаблонов, КоличествоФайлов, РежимОдногоФайла)
	...
	Если КоличествоНезагруженныхШаблонов = КоличествоФайлов Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не удалось загрузить ни один из указанных XML-шаблонов.';ru='Не удалось загрузить ни один из указанных XML-шаблонов.'"));
	Иначе
		Если РежимОдногоФайла Тогда
	...
	Для Каждого ОбъектФайл Из ОбъектыФайл Цикл
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Исключение
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить XML-шаблон ""%1"":%2';ru='Не удалось загрузить XML-шаблон ""%1"":%2'"), ОбъектФайл.Имя, Символы.ПС + ОписаниеОшибки());
				Сообщение.Сообщить();
...
Процедура ЗагрузитьШаблоны(АдресВременногоХранилища, КоличествоНезагруженныхШаблонов)
	...
	Для Каждого ШаблонЭВФ Из ШаблоныЭВФ Цикл
		...
		Если НЕ ДобавитьРеквизитыИзФайлаШаблона(ШаблонЭВФ) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონ ""%1""-ის ფორმატი არ არის ამოცნობილი';ru='Не распознан формат XML-шаблона ""%1""'"), ШаблонЭВФ.ИмяФайлаШаблона);
			Сообщение.Сообщить();
		...
		Если СтрНайти(ШаблонЭВФ.ВерсияФормата, "1.3") = 0 Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის ""%1"" ფორმატის ვერსია ვერსია ""1.3""-ს არ შეესაბამება';ru='Версия формата XML-шаблона ""%1"" не соответствует версии ""1.3""'"), ШаблонЭВФ.ИмяФайлаШаблона);
			Сообщение.Сообщить();
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის ჩაწერა ვერ მოხერხდა ""%1"":%2';ru='Не удалось записать XML-шаблон ""%1"":%2'"), ШаблонЭВФ.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
			...
			Если Диалог.Выбрать() Тогда
				...
				Для Каждого ШаблонЭВФ Из ШаблоныЭВФ Цикл
					...
					Иначе
						...
						Исключение
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილ %1-ის შენახვისას წარმოიშვა შეცდომა:%2';ru='Во время сохранения файла %1 возникла ошибка:%2'"), КороткоеИмяФайла, Символы.ПС + ОписаниеОшибки());
							Сообщение.Сообщить();
				...
				Если СписокСуществующихОбъектов.Количество() = 1 Тогда
					...
					КороткоеИмяФайла = ШаблонЭВФ.ИмяФайлаШаблона;
					ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='არჩეულ კატალოგში უკვე არსებობს ფაილი %1.%2შევცვალოთ მითითებული ფაილი?';ru='В выбранном каталоге уже существует файл %1.%2Заменить указанный файл?'"),
				...
				ИначеЕсли СписокСуществующихОбъектов.Количество() > 1 Тогда
					...
					СписокСуществующихОбъектов.ПоказатьОтметкуЭлементов(ОписаниеОповещения, НСтр("ka='Файлы существуют. Отметьте файлы для замены.';ru='Файлы существуют. Отметьте файлы для замены.'"));
				Иначе
...
Процедура ПоказатьПредупреждениПослеВыгрузкиШаблонов(Знач КоличествоОшибок, Знач ТекСтроки)
	...
	Если ТекСтроки.Количество() = 1 Тогда
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='XML-შაბლონი წარმატებით გადმოიტვირთა.';ru='XML-шаблон успешно выгружен.'"));
		КонецЕсли;
	Иначе
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ka='XML-შაბლონები წარმატებით არის გადმოტვირთული.';ru='XML-шаблоны успешно выгружены.'"));
		Иначе
			ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Во время сохранения произошли ошибки. Не удалось сохранить XML-шаблонов: %1.';ru='Во время сохранения произошли ошибки. Не удалось сохранить XML-шаблонов: %1.'"), КоличествоОшибок));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьШаблоны(ТекСтроки, КоличествоОшибок, ВозвращатьВСтруктуреСсылкуНаШаблонВоВременномХранилище = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XMK-შაბლონის %1 მიღებისას წარმოიშვა შეცდომა:%2';ru='Во время получения XML-шаблона %1 возникла ошибка:%2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ТекстВопроса = НСтр("ka='XML-шаблон будет выгружен на диск и открыт. Продолжить?';ru='XML-шаблон будет выгружен на диск и открыт. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если НЕ АдресВременногоХранилища = Неопределено Тогда
			...
			Исключение
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის გადმოტვირთვა დისკზე არსებულ დროებით ფაილში ვერ მოხერხდა:%1';ru='Не удалось выгрузить XML-шаблон во временный файл на диске:%1'"), Символы.ПС + ОписаниеОшибки());
				Сообщение.Сообщить();
...
Функция ПолучитьШаблонНаСервере(ВыбраннаяСтрока)
	...
	Если Выборка.Следующий() Тогда
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის მიღება საცავიდან ვერ მოხერხდა:%1';ru='Не удалось получить XML-шаблон из хранилища:%1'"), Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
...
Процедура ПереходНаСайтРосстатаНажатие(Элемент)
	...
	Исключение
		ПоказатьПредупреждение(, НСтр("ka='Не удалось открыть страницу!';ru='Не удалось открыть страницу!'"));
	КонецПопытки;
КонецПроцедуры
