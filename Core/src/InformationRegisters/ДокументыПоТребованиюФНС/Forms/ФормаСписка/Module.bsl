...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			...
			"ФормаСписка",
			,
			НСтр("ka='Новости: Документы по требованию ФНС';ru='Новости: Документы по требованию ФНС'"),
			Ложь,
...
Процедура СписокПередУдалением(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(СканированныйДокумент) Тогда
		...
		ТекстВопроса = НСтр("ka='Удалить данные, подготовленные к отправке?';ru='Удалить данные, подготовленные к отправке?'");
	...
	ИначеЕсли Элемент.ТекущиеДанные.ЕстьЭлектронныйДокумент Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Данные  документа-источника и связанные с ним данные электронного документооборота с контрагентом нельзя удалить из списка';ru='Данные  документа-источника и связанные с ним данные электронного документооборота с контрагентом нельзя удалить из списка'"));	
		Возврат;
		
	Иначе
		
		ПоказатьПредупреждение(, НСтр("ka='Данные документа-источника нельзя удалить из списка';ru='Данные документа-источника нельзя удалить из списка'"));	
		Возврат;
...
Процедура ЗагрузитьИзВнешнегоИсточникаЗавершение(СтуктураРезультат, ДополнительныеПараметры) Экспорт
	...
	Если СтуктураРезультат <> Неопределено Тогда
		Если ЗагрузитьИзВнешнегоИсточникаЗавершениеСервере(СтуктураРезультат) Тогда
			ПоказатьПредупреждение(, НСтр("ka='Выбранные документы успешно загружены.';ru='Выбранные документы успешно загружены.'"));
			Элементы.Список.Обновить();  
		КонецЕсли;
	КонецЕсли;
