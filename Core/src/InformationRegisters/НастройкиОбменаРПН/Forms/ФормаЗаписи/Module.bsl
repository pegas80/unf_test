...
Процедура ОбменНапрямуюПриИзменении(Элемент)
	...
	Если Запись.ОбменНапрямую Тогда
		ПоказатьПредупреждение(, НСтр("ka='Внимание! Обмен напрямую с Росприроднадзором рекомендуется использовать только при экстренной необходимости и невозможности отправки через оператора электронного документооборота.';ru='Внимание! Обмен напрямую с Росприроднадзором рекомендуется использовать только при экстренной необходимости и невозможности отправки через оператора электронного документооборота.';en='Attention! Exchange directly with Rosprirodnadzor recommended only when urgent necessity and impossibility of sending through electronic document management provider.'"));
	КонецЕсли;
