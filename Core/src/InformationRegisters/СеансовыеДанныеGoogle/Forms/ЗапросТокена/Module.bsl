...
Процедура ОбработатьВводТокенаЗапроса(Команда)
	...
	ПоказатьОповещениеПользователя(НСтр("ka='მიმდინარეობს მოთხოვნის ტოკენის დამუშავება';ru='Выполняется обработка токена запроса';en='The processing request token'"));
...
Процедура ОбработатьСобытиеБраузерДокументСформирован()
	...
	Если СтруктураЗаголовка.Статус=СтатусОтказ() Тогда
		...
		НСтр("ka='შეცდომა დაშვების ტოკენის მიღებისას';ru='Ошибка при получении токена доступа';en='Error retrieving access token'"),,
		КодОшибки,
...
Процедура ОбработатьПолучениеТокенаЗапроса()
	...
	ПоказатьОповещениеПользователя(
	НСтр("ka='მოთხოვნის ტოკენი მიღებულია, მიმდინარეობს დაშვების ტოკენზე გაცვლა...';ru='Токен запроса получен, выполняется обмен на токен доступа...';en='The token request is received, it is exchanged for an access token ...'"),,,
	...
	Иначе
		ПоказатьОповещениеПользователя(
		НСтр("ka='დაშვების ტოკენი წარმატებით იქნა მიღებული';ru='Токен доступа получен успешно';en='The access token is received successfully'"),,,
