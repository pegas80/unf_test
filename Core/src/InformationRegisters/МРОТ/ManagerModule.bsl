...
Функция МРОТНаДату(Дата = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='შრომის ანაზღაურების მინიმალური ოდენობის (შამო) ინფორმაციის რეგისტრი არ არის შევსებული';ru='Регистр сведений МРОТ не заполнен';en='SMIC data register is empty'");
	КонецЕсли;
