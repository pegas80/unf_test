...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = "Товары" + " " + НСтр("ka=' -სთვის';ru='для'") + " " + Устройство;
...
Процедура ОтборПриИзменении(Элемент)
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის ცხრილის განახლება...';ru='Выполняется обновление таблицы товаров...'"));
...
Процедура ЗарегистрироватьИзменения(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების რეგისტრაციის პროცესში წარმოიშვა შეცდომა:';ru='В процессе регистрации изменений произошла ошибка:'") + " " + ОписаниеОшибки);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების სარეგისტრაციოდ სტრიქონები არ არის არჩეული';ru='Не выбраны строки для регистрации изменений'"));
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзмененийДляВыделенныхСтрок(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Если Результат Тогда
			...
			Оповестить("Запись_КодыТоваровПодключаемогоОборудования", Новый Структура, Неопределено);
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების რეგისტრაციის წაშლის პროცესში შეცდომა მოხდა:';ru='В процессе удаления регистрации изменений произошла ошибка:'") + " " + ОписаниеОшибки);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების რეგისტრაციის წასაშლელად სტრიქონები არ არის არჩეული';ru='Не выбраны строки для удаления регистрации изменений'"));
	КонецЕсли;
