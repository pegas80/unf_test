...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = "Товары" + " " + НСтр("ka=' -სთვის';ru='для';en='for'") + " " + Устройство;
...
Процедура ОтборПриИзменении(Элемент)
	...
	Состояние(НСтр("ka='მიმდინარეობს საქონლის ცხრილის განახლება...';ru='Выполняется обновление таблицы товаров...';en='Are updating a table of goods ...'"));
...
Процедура ЗарегистрироватьИзменения(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების რეგისტრაციის პროცესში წარმოიშვა შეცდომა:';ru='В процессе регистрации изменений произошла ошибка:';en='During the registration process changes the error occurred:'") + " " + ОписаниеОшибки);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების სარეგისტრაციოდ სტრიქონები არ არის არჩეული';ru='Не выбраны строки для регистрации изменений';en='Do not select a row to record changes'"));
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзмененийДляВыделенныхСтрок(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Если Результат Тогда
			...
			Оповестить("Запись_КодыТоваровПодключаемогоОборудования", Новый Структура, Неопределено);
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების რეგისტრაციის წაშლის პროცესში შეცდომა მოხდა:';ru='В процессе удаления регистрации изменений произошла ошибка:';en='In the process of removing the registration of the changes the error occurred:'") + " " + ОписаниеОшибки);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ცვლილებების რეგისტრაციის წასაშლელად სტრიქონები არ არის არჩეული';ru='Не выбраны строки для удаления регистрации изменений';en='Not selected line to remove the registration of changes'"));
	КонецЕсли;
