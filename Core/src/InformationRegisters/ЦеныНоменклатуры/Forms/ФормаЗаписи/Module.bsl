...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВидЦен.РассчитыватьАвтоматически Тогда
		...
		ТекстСообщения = Нстр("ka='ფასების მოცემული სახეობის მიხედვით ჩანაწერების გამოთვლა მხოლოდ ავტომატურად ხდება';ru='Записи по данному виду цен рассчитываются только автоматически';en='Entries for this price mean only automatically calculated'");
...
Если ЗапретРедактированияСозданияИУдаленияЦенПрошлойДатой Тогда
	//НоваяЗапись = НЕ ЗначениеЗаполнено(Параметры.Ключ);
	//Если НоваяЗапись Тогда
	//	ЭтаФорма.ТолькоПросмотр = НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоДобавленияЦеныНоменклатуры") ИЛИ
	//							  Запись.Период < НачалоДня(ТекущаяДата());
	//Иначе
	//	ЭтаФорма.ТолькоПросмотр = НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоИзмененияЦеныНоменклатуры") ИЛИ
	//							  Запись.Период < НачалоДня(ТекущаяДата());
	//КонецЕсли;
Иначе
//}_Paganini
	...
	НоваяЗапись = НЕ ЗначениеЗаполнено(Параметры.Ключ);
	Если НоваяЗапись Тогда
		ЭтаФорма.ТолькоПросмотр = НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоДобавленияЦеныНоменклатуры");
	Иначе
		ЭтаФорма.ТолькоПросмотр = НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоИзмененияЦеныНоменклатуры");
	КонецЕсли;
...
Процедура УстановкаФорматовЦен()
	...
	Если КонстантыКоличествоЗнаковПослеЗапятойВЦене=0 Тогда
		СтрокаФормата = "ЧДЦ=2";
	КонецЕсли;
	Элементы.Цена.Формат = НСтр("ru='"+СтрокаФормата+"';ka='"+СтрокаФормата+"'");
	Элементы.Цена.ФорматРедактирования = НСтр("ru='"+СтрокаФормата+"';ka='"+СтрокаФормата+"'");
КонецПроцедуры
