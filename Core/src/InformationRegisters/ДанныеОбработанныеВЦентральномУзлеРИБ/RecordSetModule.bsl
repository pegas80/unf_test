...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Иначе
		Если Количество() > 0
			...
			Отказ = Истина;
			ТекстИсключения = НСтр("ka='ინფორმაციისრეგისტრი.გსბცენტრალურკვანძშიდამუშავებულიმონაცემები-ში ჩაწერა შესაძლებელია მხოლოდ საინფორმაციო ბაზის განახლების გადადებული დამამუშავებლის შესრულების აღნიშვნის შემთხვევაში, რომლის შესრულება მხოლოდ ცენტრალურ კვანძში ხდება.';ru='Запись в РегистрСведений.ДанныеОбработанныеВЦентральномУзлеРИБ возможна только при отметке выполнения отложенного обработчика обновления информационной базы, который выполняется только в центральном узле.';en='Entry RegistrSvedeniy.DannyeObrabotannyeVTsentralnomUzleRIB is possible only at the level of implementation of the deferred handler updates the knowledge base, which is performed only in the central node.'");
