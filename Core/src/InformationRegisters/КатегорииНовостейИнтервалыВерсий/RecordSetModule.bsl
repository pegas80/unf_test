...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		...
		Если ВРег(ТекущаяЗапись.КатегорияНовостей.Код) = ВРег("ProductVersion")
			...
			// Версия ОТ.
			ТекстСообщения = НСтр("ka='ვერსიის მნიშვნელობის ფორმატი (%ВерсияОТ%) უნდა იყოს 99.99.999.9999 %ПодробныйТекстОшибки%.';ru='Формат значения версии ОТ (%ВерсияОТ%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.';en='Value Format version FROM (% VersiyaOT%) should be 99.99.999.9999% PodrobnyyTekstOshibki%.'");
			...
			// Версия ДО.
			ТекстСообщения = НСтр("ka='ვერსიის მნიშვნელობის ფორმატი (%ВерсияДО%) უნდა იყოს 99.99.999.9999 %ПодробныйТекстОшибки%.';ru='Формат значения версии ДО (%ВерсияДО%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.';en='Value Format version to (% VersiyaDO%) should be 99.99.999.9999% PodrobnyyTekstOshibki%.'");
