	...
	Для Каждого текСтрока Из ТаблицаИзмененийТабличныхДокументов Цикл
		...
		Для Индекс = 0 По ВГраница Цикл
			...
			Если ТекСтрока[ИмяКолонки] = "И" Тогда
				...
				ШаблонТекста = НСтр("ka='ვერსია #%1 შევადაროთ ვერსია #%2-ს';ru='сравнить версию №%1 с версией №%2'");
			...
			ИначеЕсли текСтрока[ИмяКолонки] = "У" Тогда
				...
				Область.Текст = НСтр("ka='ცხრილი-დოკუმენტის ცვლილებების შენახვა გამორთულია';ru='Сохранение изменений табличного документа отключено'");
				
			ИначеЕсли текСтрока[ИмяКолонки] = "Д" Тогда
				Область = ТЧОтчета.Присоединить(ШаблонЯчейкаДобавлена);
				Область.Текст = НСтр("ka='გახსნა';ru='открыть'");
...
Функция ВывестиШапку(ТЧОтчета, СписокВерсий, ЧислоВерсий, ОбщийШаблон, СсылкаНаОбъект)
	...
	ОбластьШапка.Параметры.НаименованиеОтчета = НСтр("ka='ანგარიში ობიექტის ვერსიის ცვლილებების შესახებ';ru='Отчет по изменениям версий объекта'");
...
Функция ПолучитьОписаниеПоВерсии(СсылкаНаОбъект, НомерВерсии)
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='№ %1 / (%2) / %3';ru='№ %1 / (%2) / %3'"), НомерВерсии.Представление, 
...
Процедура ДобавитьНомераСтрокВТабличныеЧасти(ТабличныеЧасти)
	...
	Для Каждого Соответствие Из ТабличныеЧасти Цикл
		...
		Если Таблица.Колонки.Найти("НомерСтроки") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Таблица.Колонки.Вставить(0, "НомерСтроки",,НСтр("ka='სტრიქონის №';ru='№ строки'"));
