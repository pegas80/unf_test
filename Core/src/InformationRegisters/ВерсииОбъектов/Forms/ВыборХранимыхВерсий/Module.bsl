...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВерсионированиеОбъектов.НомерПоследнейВерсии(Ссылка) = 0 Тогда
		...
		Элементы.НетВерсий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='წინა ვერსიები არ არის: ""%1"".';ru='Предыдущие версии отсутствуют: ""%1"".';en='Previous versions are no ""%1"".'"), Строка(Ссылка));
	КонецЕсли;
	...
	Реквизиты = НСтр("ka='ყველა';ru='Все';en='Everything'")
КонецПроцедуры
...
Процедура СформироватьОтчетПоИзменениям(Команда)
	...
	Если СравниваемыеВерсии.Количество() < 2 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ცვლილებათა ანგარიშგების ჩამოსაყალიბებლად აუცილებელია თუნდ ორი ვერსიის არჩევა.';ru='Для формирования отчета по изменениям необходимо выбрать хотя бы две версии.';en='To create a report on the changes you need to select at least two versions.'"));
		Возврат;
	КонецЕсли;
...
"%1"
"Перейти на выбранную версию с отменой проведения?';ka='ვერსიაზე გადასვლა არ განხორციელდა შემდეგი მიზეზით:"
"%1"
"გადავიდეთ არჩეულ ვერსიაზე და გავაუქმოთ გატარება?'"),
			ТекстСообщенияОбОшибке);
			
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьПереходНаВыбраннуюВерсиюВопросЗадан", ЭтотОбъект);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Перейти", НСтр("ka='გადასვლა';ru='Перейти';en='Go'"));
	...
	Иначе //Результат = "ВосстановлениеВыполнено"
		...
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='აღდგენილია ვერსია # %1.';ru='Восстановлена версия № %1.';en='Restored version number% 1.'"), ПредставлениеНомераВерсии),
...
Функция ИмяЭтойИнформационнойБазы()
	Возврат НСтр("ka='ეს პროგრამა';ru='Эта программа';en='This program'");
КонецФункции
