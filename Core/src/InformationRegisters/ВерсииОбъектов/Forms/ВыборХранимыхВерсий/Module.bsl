...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВерсионированиеОбъектов.НомерПоследнейВерсии(Ссылка) = 0 Тогда
		...
		Элементы.НетВерсий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='წინა ვერსიები არ არის: ""%1"".';ru='Предыдущие версии отсутствуют: ""%1"".'"), Строка(Ссылка));
	КонецЕсли;
	...
	Реквизиты = НСтр("ka='ყველა';ru='Все'")
КонецПроцедуры
...
Процедура СформироватьОтчетПоИзменениям(Команда)
	...
	Если СравниваемыеВерсии.Количество() < 2 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ცვლილებათა ანგარიშგების ჩამოსაყალიბებლად აუცილებელია თუნდ ორი ვერსიის არჩევა.';ru='Для формирования отчета по изменениям необходимо выбрать хотя бы две версии.'"));
		Возврат;
	КонецЕсли;
...
"გადავიდეთ არჩეულ ვერსიაზე და გავაუქმოთ გატარება?'"),
		...
		Кнопки.Добавить("Перейти", НСтр("ka='გადასვლა';ru='Перейти'"));
	...
	Иначе //Результат = "ВосстановлениеВыполнено"
		...
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='აღდგენილია ვერსია # %1.';ru='Восстановлена версия № %1.'"), ПредставлениеНомераВерсии),
...
Функция ИмяЭтойИнформационнойБазы()
	Возврат НСтр("ka='ეს პროგრამა';ru='Эта программа'");
КонецФункции
