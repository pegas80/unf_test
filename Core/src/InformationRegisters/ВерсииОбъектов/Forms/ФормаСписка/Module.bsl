...
"выполнения анализа всей цепочки изменений этих объектов. Продолжить?';ka='ობიექტების ვერსიების მიხედვით ჩანაწერების წაშლამ შეიძლება გამოიწვიოს ამ ობიექტების ცვლილების მთელი ჯაჭვის ანალიზის შესრულების შეუძლებლობა. გავაგრძელოთ?'");
		
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьЗаписиЗавершение", ЭтотОбъект, Элементы.Список.ВыделенныеСтроки);
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ka='გაფრთხილება';ru='Предупреждение';en='A warning'"));
