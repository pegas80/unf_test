...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СравниваемыеВерсии.Количество() > 1 Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვერსია ""%1""-ის შედარება (##%2)';ru='Сравнение версий ""%1"" (№№ %2)'"),
			ОбщегоНазначения.ПредметСтрокой(СсылкаНаОбъект), СтрокаНомераВерсий);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ობიექტის ვერსია ""%1"" №%2';ru='Версия объекта ""%1"" №%2'"),
...
Процедура ОткрытьФормуСравненияТабличныхДокументов(ИмяТабличногоДокумента, Версия0, Версия1)
	...
	ЗаголовокШаблон = НСтр("ka='ვერსია №%1';ru='Версия №%1'");
