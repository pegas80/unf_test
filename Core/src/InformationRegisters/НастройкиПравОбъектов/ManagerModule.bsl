...
"Для объектов таблицы ""%2"" права не настраиваются.';ka='შეცდომა პროცედურაში ინფორმაციისრეგისტრები.ობიექტებისუფლებებისგამართვები.წავიკითხოთ()"
""
"პარამეტრ ბმულიობიექტზე ""%1"" არასწორი მნიშვნელობა."
"ცხრილ ""%2""-ის ობიექტებისთვის უფლებების გამართვა არ ხდება.'"),
...
"Требуется исправить данные регистра.';ka='შეცდომა პროცედურაში"
"ინფორმაციისრეგისტრები.ობიექტებისუფლებებისგამართვები.წავიკითხოთ()"
""
"ცხრილ ""%1""-ის ობიექტებისთვის უფლება ""%2""-ის გამართვა არ ხორციელდება, თუმცა იგი ჩაწერილია ინფორმაციის რეგისტრ ობიექტებისუფლებებისგამართვები-ში ობიექტ ""%3""-ისთვის."
""
"შესაძლებელია, რომ საინფორმაციო ბაზის განახლება არ შესრულდა ან შესრულდა შეცდომის თანხლებით. საჭიროა რეგისტრის მონაცემთა შესწორება.'"),
				СсылкаНаОбъект.Метаданные().ПолноеИмя(),
				Строка.Право,
				Строка(СсылкаНаОбъект));
		КонецЕсли;
...
""
"Тип владельцев прав ""%1"" не указан в измерении Объект.';ka='შეცდომა ინფორმაციის რეგისტრის მენეჯერის მოდულ ობიექტებისუფლებებისგამართვები-ს პროცედურა რეგისტრისდამხმარემონაცემებისგანახლება-ში."
""
"უფლებების მფლობელებისთვის ""%1"" არ არის მითითებული ზომილობაში ობიექტი.'"),
				ТипВладельцаПрав);
		КонецЕсли;
...
"общего модуля УправлениеДоступомПереопределяемый.';ka='შეცდომა საერთო მოდულ დაშვებისმართვახელახლაგანსაზღვრებადი-ს პროცედურა შესაძლებელიუფლებებისშევსებისასობიექტებისუფლებებისგასამართად-ში.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Для каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='უფლებების მფლობელი ვერ მოიძებნა ""%1"".';ru='Не найден владелец прав ""%1"".';en='copyright owner can not be found ""%1"".'"),
...
"не указан в определяемом типе ""Владелец настроек прав"".';ka='უფლებების მფლობელის ტიპი ""%1"" გარკვეულ ტიპში არ არის მითითებული ""უფლებების გამართვების მფლობელი"".'"),
					Строка(ТипСсылки));
			КонецЕсли;
...
"для корректного заполнения регистра НаборыЗначенийДоступа.';ka='უფლებების მფლობელის ტიპი ""%1"" განსაზღვრებად ტიპ ""დაშვების მნიშვნელობა""-ში არ არის მითითებული, მაგრამ გამოიყენება დაშვების მნიშვნელობების კრებულების შესავსებად, ვინაიდან მითითებულია მოვლენის ერთ-ერთ გამოწერაში:"
"- დაშვებისდამოკიდებულიმნიშვნელობებისჩაწერა*,"
"- დაშვებისმნიშვნელობებისკრებულებისჩაწერა*."
"საჭირო არის ტიპის მითითება განსაზღვრებად ტიპ ""დაშვების მნიშვნელობა""-ში რეგისტრ დაშვებისმნიშვნელობებისკრებული-ს კორექტულად შესავსებად.'"),
					Строка(ТипСсылки));
			КонецЕсли;
...
"но обнаружен в описании вида доступа ""%2"".';ka='უფლებების მფლობელის ტიპი ""%1"" არ შეიძლება გამოყენებული იქნას როგორც დაშვების მნიშვნელობების ტიპი, დაშვების სახეობის აღწერილობაში არ არის აღმოჩენილი ""%2"".'"),
					Строка(ТипСсылки),
...
"но обнаружен в описании вида доступа ""%2"".';ka='უფლებების მფლობელის ტიპი ""%1"" არ შეიძლება გამოყენებული იქნას როგორც დაშვების მნიშვნელობების ჯგუფების ტიპი, დაშვების სახეობის აღწერილობაში არ არის აღმოჩენილი ""%2"".'"),
					Строка(ТипСсылки),
					СвойстваВидовДоступа.ПоТипамЗначений.Получить(ТипСсылки).Имя);
			КонецЕсли;
			...
			Если ТипыПодпискиОбновитьГруппыВладельцевНастроекПрав.Получить(ТипОбъекта) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1"""
"не указан в определяемом типе ""Владелец настроек прав объект"".';ka='უფლებების მფლობელის ტიპი ""%1"" განსაზღვრულ ტიპ ""უფლებების გამართვების მფლობელი ობიექტი""-ში არ არის მითითებული.'"),
					Строка(ТипОбъекта));
			КонецЕсли;
		...
		Если ПраваВладельца.Получить(ВозможноеПраво.Имя) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='უფლებების მფლობელ ""%1""-სთვის განმეორებით არის განსაზღვრული უფლება ""%2"".';ru='Для владельца прав ""%1"""
		...
		Если Значение = "*" Тогда
			Если ВозможноеПраво[Свойство].Количество() <> 1 Тогда
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("ka='უფლებების მფლობელ ""%1""-სთვის უფლება ""%2""-სთვის წასაკითხ ცხრილებში მითითებულია სიმბოლო ""*"". ამ შემთხვევაში, ცალკეული ცხრილების მითითება არ არის საჭირო.';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для чтения указан символ ""*""."
"В этом случае отдельных таблиц указывать не нужно.'")
				Иначе
					ОписаниеОшибки =
						НСтр("ka='უფლებების მფლობელ ""%1""-სთვის უფლება ""%2""-სთვის შესაცვლელ ცხრილებში მითითებულია სიმბოლო ""*"". ამ შემთხვევაში, ცალკე ცხრილების მითითება არ არის საჭირო.';ru='Для владельца прав ""%1"""
...
"В этом случае отдельных таблиц указывать не нужно.'")
			...
			Если ЗначениеЗаполнено(ОбщиеПрава[Свойство]) Тогда
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("ka='უფლებების მფლობელისთვის ""%1"" უფლება ""%2""-სთვის წასაკითხ ცხრილებში მითითებულია სიმბოლო ""*"". თუმცა, სიმბოლო ""*"" უკვე მითითებულია წასაკითხ ცხრილებში უფლება ""%3""-სთვის.';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для чтения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для чтения для права ""%3"".'")
				Иначе
					ОписаниеОшибки =
						НСтр("ka='უფლებების მფლობელ ""%1""-სთვის უფლება ""%2""-სთვის ცხრილებში შესაცვლელად მითითებულია სიმბოლო ""*""."
"თუმცა, სიმბოლო ""*"" უკვე მითითებულია ცხრილებში შესაცვლელად უფლება ""%3""-სთვის.';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для изменения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для изменения для права ""%3"".'")
		...
		ИначеЕсли Свойство = "ЧтениеВТаблицах" Тогда
			ОписаниеОшибки =
				НСтр("ka='უფლებების მფლობელ ""%1""-სთვის უფლება ""%2""-სთვის წასაკითხად მითითებულია კონკრეტული ცხრილი ""%3""."
"თუმცა, ამას აზრი არ აქვს, ვინაიდან, უფლება წაკითხვა შეიძლება დამოკიდებული იყოს მხოლოდ უფლება წაკითხვა-ზე. აზრი აქვს მხოლოდ სიმბოლოს გამოყენებას ""*"".';ru='Для владельца прав ""%1"""
...
"Имеет смысл использовать только символ ""*"".'");
		...
		ИначеЕсли Метаданные.НайтиПоПолномуИмени(Значение) = Неопределено Тогда
			...
			Если Свойство = "ЧтениеВТаблицах" Тогда
				ОписаниеОшибки = НСтр("ka='უფლებების მფლობელისთვის ""%1"" უფლებებისთვის ""%2"" წასაკითხად ცხრილი ვერ მოიძებნა ""%3"".';ru='Для владельца прав ""%1"""
"для права ""%2"" не найдена таблица для чтения ""%3"".'")
			Иначе
				ОписаниеОшибки = НСтр("ka='უფლებების მფლობელისთვის ""%1"" უფლება ""%2""-სთვის შესაცვლელად ცხრილი ვერ მოიძებნა ""%3"".';ru='Для владельца прав ""%1"""
