...
""
"Указаны неверные параметры.';ka='შეცდომა ინფორმაციის რეგისტრის მენეჯერის მოდულის დაშვების მნიშვნელობების ჯგუფების პროცედურაში მომხმარებლებისდაჯგუფებებისგანახლება."
""
"მითითებულია არასწორი პარამეტრები.'");
	КонецЕсли;
...
""
"';ka='შეცდომა დაშვების მნიშვნელობების ჯგუფების განახლებისას.'");
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ტიპისთვის არ არის გამართული დაშვების მნიშვნელობების ჯგუფების გამოყენება.';ru='Для типа ""%1"""
...
"со специальным реквизитом ГруппаДоступа.';ka='დაშვების მნიშვნელობების ტიპ ""%1""-ზე სპეციალური რეკვიზიტ დაშვებისჯგუფი-თ სპეციალური ცხრილი დაშვებისჯგუფები არ არის შექმნილი.'"),
					Строка(ТипЗначенияДоступа));
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"%3';ka='""%2"" ტიპის დაშვების მნიშვნელობა ""%1""-ზე რეკვიზიტ დაშვებისჯგუფი-თ ცხრილ დაშვებისმნიშვნელობა-ს წაკითხვა ვერ მოხერხდა შეცდომის გამო:"
"%3'"),
		...
		Иначе
			Если МетаданныеТипа.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У типа значений доступа ""%1"""
"не создан специальный реквизит ГруппаДоступа.';ka='დაშვების მნიშვნელობების ტიპზე ""%1"""
"სპეციალური რეკვიზიტი დაშვებისჯგუფი არ არის შექმნილი.'"),
					Строка(ТипЗначенияДоступа));
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У значения доступа ""%1"""
...
"%3';ka='%2 ტიპის დაშვების მნიშვნელობა ""%1""-ზე ვერ მოხერხდა რეკვიზიტ დაშვებისჯგუფი-ს წაკითხვა შეცდომის გამო: %3'"),
					Строка(ЗначениеДоступа),
					Строка(ТипЗначенияДоступа),
					КраткоеПредставлениеОшибки(Ошибка));
			КонецЕсли;
...
""
"Указаны неверные параметры.';ka='შეცდომა ინფორმაციის რეგისტრის მენეჯერის მოდულის დაშვებისმნიშვნელობებისჯგუფები პროცედურაში შემსრულებლებისჯგუფებისგანახლება."
""
"მითითებულია არასწორი პარამეტრები.'");
	КонецЕсли;
