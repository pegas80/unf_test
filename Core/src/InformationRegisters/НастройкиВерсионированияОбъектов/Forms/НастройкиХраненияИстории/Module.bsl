...
Процедура УстановитьВариантВерсионированияПриПроведении(Команда)
	...
	Если ВыбранТипДокументовБезВозможностиПроведения() Тогда
		ПоказатьПредупреждение(, НСтр("ka='დოკუმენტებს, რომელთა გატარება შეუძლებელია, დაყენებული ექნებათ ვერსიონირების რეჟიმი ""ჩაწერისას ვერსიონირება"".';ru='Документам, которые не могут быть проведены, установлен режим версионирования ""Версионировать при записи"".';en='Documents that can not be conducted, set versioning mode ""versioned when writing"".'"));
	КонецЕсли;
	...
	Для Каждого Тип Из МассивТипов Цикл
		Если ВсеСправочники.СодержитТип(Тип) Тогда
			Если УзелСправочники = НеОпределено Тогда
				...
				УзелСправочники.СинонимНаименованияОбъекта = НСтр("ka='ცნობარები';ru='Справочники';en='References'");
		...
		ИначеЕсли ВсеДокументы.СодержитТип(Тип) Тогда
			Если УзелДокументы = НеОпределено Тогда
				...
				УзелДокументы.СинонимНаименованияОбъекта = НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'");
		...
		ИначеЕсли БизнесПроцессы.ТипВсеСсылки().СодержитТип(Тип) Тогда
			Если УзелБизнесПроцессы = Неопределено Тогда
				...
				УзелБизнесПроцессы.СинонимНаименованияОбъекта = НСтр("ka='ბიზნეს-პროცესები';ru='Бизнес-процессы';en='Business processes'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელით გაშვება: %1';ru='Запуск вручную: %1';en='Manual start:% 1'"), РегламентноеЗаданиеМетаданные.Синоним);
...
Процедура ПриЗавершенииОперацииПоискаУстаревшихВерсий(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ka='Поиск устаревших версий объектов';ru='Поиск устаревших версий объектов';en='Search for outdated versions of objects'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ТекстСостоянияПодсчет()
	Возврат НСтр("ka='მოძველებული ვერსიების ძებნა...';ru='Поиск устаревших версий...';en='Search older versions ...'");
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстСостоянияОчистка()
	Возврат НСтр("ka='მიმდინარეობს მოძველებული ვერსიების გასუფთავება...';ru='Выполняется очистка устаревших версий...';en='Clears older versions ...'");
КонецФункции
...
Функция ВыполнитьПоискУстаревшихВерсий()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Поиск устаревших версий объектов';ru='Поиск устаревших версий объектов';en='Search for outdated versions of objects'");
...
Процедура ВывестиИнформациюОбУстаревшихВерсиях()
	...
	Если ИнформацияОбУстаревшихВерсиях.РазмерДанных > 0 Тогда
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სულ, მოძველებულია ვერსია: %1 (%2)';ru='Всего устаревших версий: %1 (%2)';en='All older versions of the% 1 (% 2)'"),
			ИнформацияОбУстаревшихВерсиях.КоличествоВерсий,
			ИнформацияОбУстаревшихВерсиях.РазмерДанныхСтрокой);
	Иначе
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = НСтр("ka='სულ, მოძველებული ვერსია: არ არის';ru='Всего устаревших версий: нет';en='All older versions: no'");
	КонецЕсли;
