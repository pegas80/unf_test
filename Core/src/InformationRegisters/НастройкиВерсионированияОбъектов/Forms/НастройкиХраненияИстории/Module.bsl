...
Процедура УстановитьВариантВерсионированияПриПроведении(Команда)
	...
	Если ВыбранТипДокументовБезВозможностиПроведения() Тогда
		ПоказатьПредупреждение(, НСтр("ka='დოკუმენტებს, რომელთა გატარება შეუძლებელია, დაყენებული ექნებათ ვერსიონირების რეჟიმი ""ჩაწერისას ვერსიონირება"".';ru='Документам, которые не могут быть проведены, установлен режим версионирования ""Версионировать при записи"".'"));
	КонецЕсли;
	...
	Для Каждого Тип Из МассивТипов Цикл
		Если ВсеСправочники.СодержитТип(Тип) Тогда
			Если УзелСправочники = НеОпределено Тогда
				...
				УзелСправочники.СинонимНаименованияОбъекта = НСтр("ka='ცნობარები';ru='Справочники'");
		...
		ИначеЕсли ВсеДокументы.СодержитТип(Тип) Тогда
			Если УзелДокументы = НеОпределено Тогда
				...
				УзелДокументы.СинонимНаименованияОбъекта = НСтр("ka='დოკუმენტები';ru='Документы'");
		...
		ИначеЕсли БизнесПроцессы.ТипВсеСсылки().СодержитТип(Тип) Тогда
			Если УзелБизнесПроцессы = Неопределено Тогда
				...
				УзелБизнесПроцессы.СинонимНаименованияОбъекта = НСтр("ka='ბიზნეს-პროცესები';ru='Бизнес-процессы'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელით გაშვება: %1';ru='Запуск вручную: %1'"), РегламентноеЗаданиеМетаданные.Синоним);
...
Процедура ПриЗавершенииОперацииПоискаУстаревшихВерсий(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ka='Поиск устаревших версий объектов';ru='Поиск устаревших версий объектов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ТекстСостоянияПодсчет()
	Возврат НСтр("ka='მოძველებული ვერსიების ძებნა...';ru='Поиск устаревших версий...'");
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстСостоянияОчистка()
	Возврат НСтр("ka='მიმდინარეობს მოძველებული ვერსიების გასუფთავება...';ru='Выполняется очистка устаревших версий...'");
КонецФункции
...
Функция ВыполнитьПоискУстаревшихВерсий()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Поиск устаревших версий объектов';ru='Поиск устаревших версий объектов'");
...
Процедура ВывестиИнформациюОбУстаревшихВерсиях()
	...
	Если ИнформацияОбУстаревшихВерсиях.РазмерДанных > 0 Тогда
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='სულ, მოძველებულია ვერსია: %1 (%2)';ru='Всего устаревших версий: %1 (%2)'"),
			ИнформацияОбУстаревшихВерсиях.КоличествоВерсий,
			ИнформацияОбУстаревшихВерсиях.РазмерДанныхСтрокой);
	Иначе
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = НСтр("ka='სულ, მოძველებული ვერსია: არ არის';ru='Всего устаревших версий: нет'");
	КонецЕсли;
