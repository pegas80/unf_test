...
Процедура ЗагрузитьСтандартныеЗначения() Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		СтрШаблон(
			НСтр("ka='Загрузка стандартных значений (%1)';ru='Загрузка стандартных значений (%1)';en='Loading standard value (1%)'"),
			ИмяСвойства));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		СтрШаблон(
			НСтр("ka='Облачный архив. Сервис и регламент. Загрузка стандартных значений. %1';ru='Облачный архив. Сервис и регламент. Загрузка стандартных значений. %1';en='Cloud archive. Service and regulations. Loading standard values. %one'"),
...
Процедура ЗагрузитьСтандартноеЗначение(НаборЗаписей, ТекущаяЗаписьХДТО, ОбъектМетаданных, ИмяСвойства, КонтекстВыполнения)
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Загрузка записи регистра сведений';ru='Загрузка записи регистра сведений';en='Download information register records'"));
...
"%1';ka='Не удалось добавить запись в регистр сведений по причине:"
"%1'"),
