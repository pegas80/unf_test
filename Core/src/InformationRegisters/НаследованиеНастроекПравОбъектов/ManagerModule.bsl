...
Процедура ОбновитьРодителейВладельца(ВладелецНастроекПрав, ЕстьИзменения = Ложь, ОбновитьИерархию = Ложь, ОбъектыСИзменениями = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ka='შეცდომა დაშვების მნიშვნელობებით უფლებების მფლობელთა იერარქიის განახლებისას.';ru='Ошибка при обновлении иерархии владельцев прав по значениям доступа.';en='Error updating the hierarchy of rights holders for access values.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Если ВозможныеПрава.ПоТипам.Получить(ТипВладельца) = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 ტიპისთვის ობიექტების უფლებების გამართვების გამოყენება არ არის გასწორებული.';ru='Для типа ""%1"""
