	...
	Если Поле.Имя = "ПротоколГиперссылкаРасшифровки" Тогда
		...
		Если Элементы.Протокол.ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда
			...
			Если КлючПротокола = Неопределено Тогда
				ПоказатьПредупреждение(,НСтр("ka='განმეორებითი შემოწმება არ შესრულდა შიდა შეცდომის გამო';ru='Повторная проверка не выполнена из-за внутренней ошибки';en='Retesting is not performed due to internal error'"));
				Возврат;
			КонецЕсли;
...
Процедура ЗаполнитьПротокол(ДеревоРезультата, ДанныеПротокола)
	...
	Для Каждого Строка Из ДеревоРезультата.Строки Цикл
		...
		Если Строка.РезультатКонтроля > 0 Тогда
			...
			Если Строка.Родитель <> Неопределено Тогда
				...
				Если Строка.Расшифровка.ТипРасшифровки = "НоваяЗаписьРегистраСведений" Тогда
					СтрокаПротокола.ГиперссылкаРасшифровки = НСтр("ka='შექმნა';ru='Создать';en='Create'");
				Иначе
					СтрокаПротокола.ГиперссылкаРасшифровки = НСтр("ka='ჩვენება';ru='Показать';en='show'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Строка.Родитель = Неопределено Тогда
			...
			СтрокаПротокола.ГиперссылкаРасшифровки = НСтр("ka='შემოწმების გამეორება';ru='Повторить проверку';en='repeat the test'");
...
Процедура СформироватьЗаголовок()
	...
	Заголовок = НСтр("ru='Протокол проверки данных от ';ka='Протокол проверки данных от '") + Запись.ДатаПроверки;
