...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СуммаНаДату = НСтр("ka='მიმდინარე თარიღი';ru='текущую дату';en='the current date'");
...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფპსგ-ის მიხედვით ბავშვებზე დაქვითვა (სულ: %1)';ru='Вычеты по НДФЛ на детей (Итого: %1)';en='Deductions for personal income tax on children (Total: 1%)'"),
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ БылиНажатыКнопкиЗакрытия Тогда
		...
		спсОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ka='შენახვა';ru='Сохранить';en='retain'"));
		спсОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='არა';ru='Нет';en='Not'"));
		спсОтветов.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
		
		оп = Новый ОписаниеОповещения("ОповещениеПриЗакрытии", ЭтотОбъект);
		
		ТекстВопроса = НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Даные были изменены. Сохранить изменения?';en='The figures have been changed. I save changes?'");
...
Процедура ДатаРожденияРебенкаПриИзменении(Элемент)
	...
	Если СтрокаВТаблице.Номер <> ТекущийНомерСтроки Тогда
		...
		СпсОтветов = Новый СписокЗначений;
		СпсОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ka='კორექტირება';ru='Скорректировать';en='adjust'"));
		СпсОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='დავტოვოთ, როგორც არის';ru='Оставить как есть';en='Leave as is'"));
		
		оп = Новый ОписаниеОповещения("ОповещениеКорректировкиКодовВычетов", ЭтотОбъект);
		
		ПоказатьВопрос(оп, НСтр("ka='ცხრილში სტრიქონების განლაგება შეიცვალა."
"მოვახდინოთ სტრიქონებში დაქვითვის კოდების კორექტირება ახალი წესის შესაბამისად?';ru='В таблице изменился порядок строк."
...
Процедура ТаблицаВычетовНаДетейПриАктивизацииСтроки(Элемент)
	...
	Если Текданные<> Неопределено Тогда
		...
		Иначе
			...
			Строка.Представление = ?(ЗначениеЗаполнено(Строка.КодВычета), Нстр("ru='Код ';ka='Код '")+СокрЛП(Строка.КодВычета), "")
...
Процедура КодВычетаПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ТекДанные.НаименованиеВычета) Тогда
		ТекДанные.НаименованиеВычета = Нстр("ka='<<დაქვითვა არ არის არჩეული>>';ru='<<Вычет не выбран>>';en='<< >> The deduction is not selected'");
	КонецЕсли;
...
Процедура СуммаНаДатуНажатие(Элемент, СтандартнаяОбработка)
	...
	Если СуммаНаДату = НСтр("ka='მიმდინარე თარიღი';ru='текущую дату';en='the current date'") Тогда
	...
	ПоказатьВводДаты(оп,АктуальнаяДата, НСтр("ka='დაქვითვის თანხის გამოსათვლელად აირჩიეთ თარიღი';ru='Выберите дату для расчета суммы вычета';en='Choose the date for calculating the amount of the deduction'"));
...
Процедура ЗаполнитьДанныеПоВычету()
	...
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		...
		Если ПустаяСтрока(Строка.НаименованиеВычета) Тогда
			Строка.НаименованиеВычета = Нстр("ka='<<დაქვითვა არ არის არჩეული>>';ru='<<Вычет не выбран>>';en='<< >> The deduction is not selected'");
		КонецЕсли;
		Строка.Представление = ?(ЗначениеЗаполнено(Строка.КодВычета), Нстр("ru='Код ';ka='Код '")+СОКРЛП(Строка.КодВычета), "")
			+?(Строка.СуммаВычета> 0 , ", "+Формат(Строка.СуммаВычета,"ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0")+ " руб. ", "");
	КонецЦикла;
...
Функция ВыполнитьПроверку(ОписаниеОшибок)
	...
	НомерСтроки = 1;
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		ТекстОшибки = ?(ЗначениеЗаполнено(Строка.Период),"", НСтр("ka='განცხადების თარიღი;';ru='Дата заявления;';en='The date of application;'"))
			+ ?(ЗначениеЗаполнено(Строка.ДатаРожденияРебенка),"", НСтр("ka='ბავშვის დაბადების თარიღი;';ru='Дата рождения ребенка;';en='Date of birth of the child;'"))
			+ ?(ЗначениеЗаполнено(Строка.КодВычета),"", НСтр("ka='გამოკლების კოდი;';ru='Код вычета;';en='deduction code;'"));
		Если ТекстОшибки <> "" Тогда
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке №%1 не заполнены следующие поля: ';ka='В строке №%1 не заполнены следующие поля: '"), НомерСтроки) +
				ТекстОшибки));
		КонецЕсли;
		НомерСтроки = НомерСтроки+1;
	КонецЦикла;
...
Функция КодВычетаПоПереключателям()
	...
	Если ПустаяСтрока(НаименованиеВычета) Тогда
		НаименованиеВычета = Нстр("ka='<<დაქვითვა არ არის არჩეული>>';ru='<<Вычет не выбран>>';en='<< >> The deduction is not selected'");
	КонецЕсли;
