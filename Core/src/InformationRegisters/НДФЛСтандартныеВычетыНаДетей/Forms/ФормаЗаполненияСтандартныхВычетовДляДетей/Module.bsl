...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СуммаНаДату = НСтр("ka='მიმდინარე თარიღი';ru='текущую дату'");
...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფპსგ-ის მიხედვით ბავშვებზე დაქვითვა (სულ: %1)';ru='Вычеты по НДФЛ на детей (Итого: %1)'"),
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ БылиНажатыКнопкиЗакрытия Тогда
		...
		спсОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ka='შენახვა';ru='Сохранить'"));
		спсОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='არა';ru='Нет'"));
		спсОтветов.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='გაუქმება';ru='Отмена'"));
		
		оп = Новый ОписаниеОповещения("ОповещениеПриЗакрытии", ЭтотОбъект);
		
		ТекстВопроса = НСтр("ka='მონაცემები შეიცვალა. შევინახოთ ცვლილებები?';ru='Даные были изменены. Сохранить изменения?'");
...
Процедура ДатаРожденияРебенкаПриИзменении(Элемент)
	...
	Если СтрокаВТаблице.Номер <> ТекущийНомерСтроки Тогда
		...
		СпсОтветов = Новый СписокЗначений;
		СпсОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ka='კორექტირება';ru='Скорректировать'"));
		СпсОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='დავტოვოთ, როგორც არის';ru='Оставить как есть'"));
...
Процедура КодВычетаПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ТекДанные.НаименованиеВычета) Тогда
		ТекДанные.НаименованиеВычета = Нстр("ka='<<დაქვითვა არ არის არჩეული>>';ru='<<Вычет не выбран>>'");
	КонецЕсли;
...
Процедура СуммаНаДатуНажатие(Элемент, СтандартнаяОбработка)
	...
	Если СуммаНаДату = НСтр("ka='მიმდინარე თარიღი';ru='текущую дату'") Тогда
	...
	ПоказатьВводДаты(оп,АктуальнаяДата, НСтр("ka='დაქვითვის თანხის გამოსათვლელად აირჩიეთ თარიღი';ru='Выберите дату для расчета суммы вычета'"));
...
Процедура ЗаполнитьДанныеПоВычету()
	...
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		...
		Если ПустаяСтрока(Строка.НаименованиеВычета) Тогда
			Строка.НаименованиеВычета = Нстр("ka='<<დაქვითვა არ არის არჩეული>>';ru='<<Вычет не выбран>>'");
		КонецЕсли;
		Строка.Представление = ?(ЗначениеЗаполнено(Строка.КодВычета), Нстр("ru='Код ';ka='Код '")+СОКРЛП(Строка.КодВычета), "")
			+?(Строка.СуммаВычета> 0 , ", "+Формат(Строка.СуммаВычета,"ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0")+ " руб. ", "");
	КонецЦикла;
...
Функция ВыполнитьПроверку(ОписаниеОшибок)
	...
	НомерСтроки = 1;
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		ТекстОшибки = ?(ЗначениеЗаполнено(Строка.Период),"", НСтр("ka='განცხადების თარიღი;';ru='Дата заявления;'"))
			+ ?(ЗначениеЗаполнено(Строка.ДатаРожденияРебенка),"", НСтр("ka='ბავშვის დაბადების თარიღი;';ru='Дата рождения ребенка;'"))
			+ ?(ЗначениеЗаполнено(Строка.КодВычета),"", НСтр("ka='გამოკლების კოდი;';ru='Код вычета;'"));
...
Функция КодВычетаПоПереключателям()
	...
	Если ПустаяСтрока(НаименованиеВычета) Тогда
		НаименованиеВычета = Нстр("ka='<<დაქვითვა არ არის არჩეული>>';ru='<<Вычет не выбран>>'");
	КонецЕсли;
