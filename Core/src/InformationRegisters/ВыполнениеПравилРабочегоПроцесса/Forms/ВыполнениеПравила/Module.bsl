...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.Правило) Тогда
		ВызватьИсключение НСтр("ka='სამუშაო პროცესის წესი არ არის მითითებული';ru='Не указано правило рабочего процесса'")
	КонецЕсли;
	
	Список.Параметры.УстановитьЗначениеПараметра("Правило", Параметры.Правило);
	
	Заголовок = СтрШаблон(НСтр("ka='სამუშაო პროცესის წესის შესრულება: ""%1""';ru='Выполнение правила рабочего процесса: ""%1""'"), Параметры.Правило);
