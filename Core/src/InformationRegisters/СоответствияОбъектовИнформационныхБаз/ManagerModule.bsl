...
Процедура ДобавитьЗапись(СтруктураЗаписи, Загрузка = Ложь) Экспорт
	...
	Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
		Если СтруктураЗаписи.Свойство(ПроверяемыйРеквизит)
			...
			ОписаниеСобытия = НСтр("ka='ინფორმაციის რეგისტრ ""საინფორმაციო ბაზის ობიექტების შესაბამისობა""-ს ჩანაწერის დამატება';ru='Добавление записи регистра сведений ""Соответствия объектов информационных баз""'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			Комментарий     = НСтр("ka='რეკვიზიტი %1 არ არის შევსებული. რეგისტრის ჩანაწერის შექმნა შეუძლებელია.';ru='Не заполнен реквизит %1. Создание записи регистра невозможно.'");
