...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезВК")
		...
		ТекстСообщения = НСтр("ka='ჟურნალირებისთვის მიუთითეთ კატალოგი.';ru='Укажите каталог для журналирования.';en='Specify the directory for logging.'");
...
Процедура КаталогДляЖурналированияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='Выберите каталог для сохранения файлов журнала.';ru='Выберите каталог для сохранения файлов журнала.';en='Select the directory to save the log files.'");
