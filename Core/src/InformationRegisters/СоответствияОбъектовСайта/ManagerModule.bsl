...
Процедура ДобавитьЗапись(СтруктураЗаписи, Загрузка = Ложь) Экспорт
	...
	Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
		Если СтруктураЗаписи.Свойство(ПроверяемыйРеквизит)
			...
			ОписаниеСобытия = НСтр("ka='ინფორმაციის რეგისტრ ""საინფორმაციო ბაზის ობიექტების შესაბამისობა""-ს ჩანაწერის დამატება';ru='Добавление записи регистра сведений ""Соответствия объектов информационных баз""';en='Adding information register records ""the corresponding object information databases""'");
			Комментарий     = НСтр("ka='რეკვიზიტი %1 არ არის შევსებული. რეგისტრის ჩანაწერის შექმნა შეუძლებელია.';ru='Не заполнен реквизит %1. Создание записи регистра невозможно.';en='Not filled with props 1%. Creating a registry entry is not possible.'");
