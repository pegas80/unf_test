...
Процедура КаталогПрограммыЭлектроннойПочтыНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='Выберите путь к каталогу программы электронной почты';ru='Выберите путь к каталогу программы электронной почты';en='Select the path to the e-mail program'");
...
Процедура КаталогОтправкиДанныхОтчетностиНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ДиалогОткрытияФайла.Каталог = Запись.КаталогПрограммыЭлектроннойПочты;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ გზა ანგარიშგებულობის მონაცემთა გაგზავნის კატალოგისკენ';ru='Выберите путь к каталогу отправки данных отчетности';en='Select the path to the directory to send reporting data'");
