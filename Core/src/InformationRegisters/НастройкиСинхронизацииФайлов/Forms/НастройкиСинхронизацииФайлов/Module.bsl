...
Процедура СинхронизацияЭлемента(Команда)
	...
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = НСтр("ka='გამართვის დამატება შესაძლებელია მხოლოდ იერარქიული ცნობარებისთვის';ru='Добавление настройки возможно только для иерархических справочников';en='Adding a configuration is possible only for hierarchical directories'");
	...
	ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("ka='გამართვის ელემენტების გადარჩევა';ru='Подбор элементов настроек';en='Selection of setting elements'"));
	...
	Для Каждого Тип Из ТаблицаТипов Цикл
			...
			Если ДетализированныеНастройки.Количество() > 1 Тогда
				...
				Для Каждого Настройка Из ДетализированныеНастройки Цикл
					...
					ДетализированнаяНастройка.ПравилоОтбора =
						?(ЕстьПравилоОтбора, НСтр("ka='არჩეული ფაილები';ru='Выбранные файлы';en='Selected files'"), НСтр("ka='ყველა ფაილი';ru='Все файлы';en='All files'"));
		...
		Если Не НастройкаИмеетГлобальныеПравила Тогда
			...
			Если НайденныеНастройки.Количество() > 0 Тогда
				...
				НоваяСтрокаТаблицы.ПравилоОтбора =    ?(ЕстьПравилоОтбора, НСтр("ka='არჩეული ფაილები';ru='Выбранные файлы';en='Selected files'"), НСтр("ka='ყველა ფაილი';ru='Все файлы';en='All files'"));
			Иначе
				НоваяСтрокаТаблицы.Синхронизировать = Перечисления.ВариантыОчисткиФайлов.НеОчищать;
				НоваяСтрокаТаблицы.ПравилоОтбора = НСтр("ka='ყველა ფაილი';ru='Все файлы';en='All files'");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьСинхронизациюФайлов(Команда)
	...
	СтрокаДерева = Элементы.ДеревоОбъектовМетаданных.ТекущиеДанные;
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = НСтр("ka='გამართვის დამატება შესაძლებელია მხოლოდ ცნობარებისთვის';ru='Добавление настройки возможно только для справочников';en='Adding a configuration is possible only for directories'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
