...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ka='ფაილების სინქრონიზაციის გამართვა:';ru='Настройка синхронизации файлов:';en='Customize File Synchronization:'")
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОбъектСинхронизации = "ТолькоФайлыЭлемента" И Не ЗначениеЗаполнено(ЭлементСправочника) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მიერთებულფაილებიანი ობიექტი არ არის შევსებული.';ru='Не заполнен объект с присоединенными файлами.';en='Not filled an object with attached files.'"),
			,
			"ЭлементСправочника");
	КонецЕсли;
...
Процедура ДобавитьВОтборИнтервалИсключение(ВыбранноеЗначение)
	...
	ТекстПредставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1 დღის შემდეგ გავასუფთავოთ შემდეგი თარიღიდან (%2)';ru='Очищать спустя %1 дней относительно даты (%2)';en='Clean% after 1 day relative to the date (2%)'"), 
