...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ka='ფაილების სინქრონიზაციის გამართვა:';ru='Настройка синхронизации файлов:'")
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОбъектСинхронизации = "ТолькоФайлыЭлемента" И Не ЗначениеЗаполнено(ЭлементСправочника) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მიერთებულფაილებიანი ობიექტი არ არის შევსებული.';ru='Не заполнен объект с присоединенными файлами.'"),
			,
			"ЭлементСправочника");
	КонецЕсли;
...
Процедура ДобавитьВОтборИнтервалИсключение(ВыбранноеЗначение)
	...
	ТекстПредставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1 დღის შემდეგ გავასუფთავოთ შემდეგი თარიღიდან (%2)';ru='Очищать спустя %1 дней относительно даты (%2)'"), 
