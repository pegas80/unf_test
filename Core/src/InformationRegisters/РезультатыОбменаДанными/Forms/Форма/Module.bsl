...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ИспользуетсяВерсионирование Тогда
		...
		Элементы.КоллизииАвторДругойВерсии.Заголовок = НСтр("ka='ვერსია მიღებულია დანართიდან';ru='Версия получена из приложения'");
...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='ჩატვირთვის აკრძალვის მიუხედავად, მივიღოთ ვერსია?';ru='Принять версию, несмотря на запрет загрузки?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ИзменитьРезультатКонфликта(Команда)
	...
	Если Элементы.Коллизии.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Коллизии.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ТекстВопроса = НСтр("ka='სხვა პროგრამიდან მიღებული ვერსია შევცვალოთ ამ პროგრამის ვერსიით?';ru='Заменить версию, полученную из другой программы, на версию из этой программы?'");
			
		Иначе
			
			ТекстВопроса = НСтр("ka='ამ პროგრამის ვერსია შევცვალოთ სხვა პროგრამიდან მიღებული ვერსიით?';ru='Заменить версию этой программы на версию, полученную из другой программы?'");
...
Процедура ЗаполнитьСписокУзлов()
	...
	Если КонтекстноеОткрытие И ОтсутствуетОбменПоПравилам Тогда
		Заголовок = НСтр("ka='კონფლიქტები მონაცემთა სინქრონიზაციის დროს';ru='Конфликты при синхронизации данных'");
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если СравниваемыеВерсии.Количество() <> 2 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შესადარებელი ვერსიები არ არის.';ru='Нет версии для сравнения.'"));
		Возврат;
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНепроведенныеДокументы, НСтр("ka='გაუტარებელი დოკუმენტები';ru='Непроведенные документы'"), КоличествоНепроведенныхДокументов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНезаполненныеРеквизиты, НСтр("ka='შეუვსებელი რეკვიზიტები';ru='Незаполненные реквизиты'"), КоличествоНезаполненныхРеквизитов());
	
	Если ИспользуетсяВерсионирование Тогда
		УстановитьЗаголовокСтраницы(Элементы.СтраницаКоллизии, НСтр("ka='კონფლიქტები';ru='Конфликты'"), КоличествоКоллизий());
		УстановитьЗаголовокСтраницы(Элементы.СтраницаНепринятыеПоДатеЗапрета, НСтр("ka='აკრძალვის თარიღის მიხედვით არმიღებულები';ru='Непринятые по дате запрета'"), КоличествоНепринятых());
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='წაშლილია';ru='Удалена'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='არ არიან';ru='Отсутствуют'"));
