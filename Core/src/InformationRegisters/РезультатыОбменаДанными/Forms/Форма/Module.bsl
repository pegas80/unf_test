...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ИспользуетсяВерсионирование Тогда
		...
		Элементы.КоллизииАвторДругойВерсии.Заголовок = НСтр("ka='ვერსია მიღებულია დანართიდან';ru='Версия получена из приложения';en='Version obtained from the application'");
...
"Версия в этой программе была заменена на версию из другой программы.';ka='Конфликт был разрешен автоматически в пользу программы ""%1""."
"Версия в этой программе была заменена на версию из другой программы.'");
		...
		Иначе
			...
			ПричинаКонфликта =НСтр("ka='კონფლიქტი ავტომატურად გადაწყდა ამ პროგრამის სასარგებლოდ."
"ვერსია შენახული იქნა ამ პროგრამაში, ვერსია სხვა პროგრამიდან უარყოფილია.';ru='Конфликт был разрешен автоматически в пользу этой программы."
...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='ჩატვირთვის აკრძალვის მიუხედავად, მივიღოთ ვერსია?';ru='Принять версию, несмотря на запрет загрузки?';en='Accept the version, in spite of the ban download?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ИзменитьРезультатКонфликта(Команда)
	...
	Если Элементы.Коллизии.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Коллизии.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ТекстВопроса = НСтр("ka='სხვა პროგრამიდან მიღებული ვერსია შევცვალოთ ამ პროგრამის ვერსიით?';ru='Заменить версию, полученную из другой программы, на версию из этой программы?';en='Replace the version received from another program, a version of this program?'");
			
		Иначе
			
			ТекстВопроса = НСтр("ka='ამ პროგრამის ვერსია შევცვალოთ სხვა პროგრამიდან მიღებული ვერსიით?';ru='Заменить версию этой программы на версию, полученную из другой программы?';en='Replace version of this software to the version received from another program?'");
...
Процедура ЗаполнитьСписокУзлов()
	...
	Если КонтекстноеОткрытие И ОтсутствуетОбменПоПравилам Тогда
		Заголовок = НСтр("ka='კონფლიქტები მონაცემთა სინქრონიზაციის დროს';ru='Конфликты при синхронизации данных';en='Conflicts synchronization'");
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если СравниваемыеВерсии.Количество() <> 2 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შესადარებელი ვერსიები არ არის.';ru='Нет версии для сравнения.';en='No version for comparison.'"));
		Возврат;
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНепроведенныеДокументы, НСтр("ka='გაუტარებელი დოკუმენტები';ru='Непроведенные документы';en='Not approved documents'"), КоличествоНепроведенныхДокументов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНезаполненныеРеквизиты, НСтр("ka='შეუვსებელი რეკვიზიტები';ru='Незаполненные реквизиты';en='unfilled details'"), КоличествоНезаполненныхРеквизитов());
	
	Если ИспользуетсяВерсионирование Тогда
		УстановитьЗаголовокСтраницы(Элементы.СтраницаКоллизии, НСтр("ka='კონფლიქტები';ru='Конфликты';en='conflicts'"), КоличествоКоллизий());
		УстановитьЗаголовокСтраницы(Элементы.СтраницаНепринятыеПоДатеЗапрета, НСтр("ka='აკრძალვის თარიღის მიხედვით არმიღებულები';ru='Непринятые по дате запрета';en='Missed by date of ban'"), КоличествоНепринятых());
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='წაშლილია';ru='Удалена';en='deleted'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='არ არიან';ru='Отсутствуют';en='There are no'"));
