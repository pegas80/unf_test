...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ka='შევწყვიტოთ სამისამართო კლასიფიკატორის გასუფთავება?';ru='Прервать очистку адресного классификатора?';en='Interrupt the cleaning of the address classifier?'");
...
Процедура ОбновитьИнтерфейсПоКоличествуОчищаемых()
	...
	// Страница загрузки
	ТекстОписанияОчистки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიმდინარეობს არჩეული რაიონების მონაცემთა გასუფთავება (%1)';ru='Очищаются данные выбранных регионов (%1)';en='Cleaned data selected regions (1%)'"), ВыбраноРегионовДляОчистки);
...
Процедура ОчиститьКлассификатор()
	...
	ТекстСостоянияОчистки = НСтр("ka='სამისამართო კლასიფიკატორის გასუფთავება ...';ru='Очистка адресного классификатора ...';en='Cleaning the address classifier ...'");
...
Процедура ПослеФоновойОчистки(Задание, ДополнительныеПараметры) Экспорт
	Если Задание.Статус = "Выполнено" Тогда
		...
		ТекстОписанияОчистки = НСтр("ka='სამისამართო კლასიფიკატორი წარმატებით გასუფთავდა.';ru='Адресный классификатор успешно очищен.';en='The address classifier successfully cleared.'");
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		...
		ТекстОшибки = НСтр("ka='Не удается очистить адресный классификатор.';ru='Не удается очистить адресный классификатор.';en='Unable to clear the address qualifier.'");
		ТекстОшибки = ТекстОшибки + Символы.ПС + + НСтр("ka='Техническая информация:';ru='Техническая информация:';en='Technical information:'") + Задание.ПодробноеПредставлениеОшибки;
...
Функция ЗапуститьФоновуюОчисткуНаСервере(Знач КодыРегионов, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='სამისამართო კლასიფიკატორის გასუფთავება';ru='Очистка адресного классификатора';en='Cleaning the address classifier'");
