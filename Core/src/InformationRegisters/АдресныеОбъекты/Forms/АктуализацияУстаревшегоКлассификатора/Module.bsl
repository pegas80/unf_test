...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ka='უარი ვთქვათ კლასიფიკატორის მონაცემთა განახლებაზე?';ru='Отказаться от обновления данных классификатора?'");
...
Процедура Отмена(Команда)
	...
	Если ЗначениеЗаполнено(ИдентификаторФоновогоЗадания) Тогда
		...
		ПоказатьОповещениеПользователя(,, НСтр("ka='სამისამართო კლასიფიკატორის განახლება არ არის დასრულებული.';ru='Обновление адресного классификатора не завершено.'"));
	КонецЕсли;
...
Процедура АктуализироватьАдресныйКлассификаторКлиент()
	...
	Иначе 
		Если Результат = "Выполнено" Тогда
			...
			ПоказатьОповещениеПользователя(,, НСтр("ka='სამისამართო კლასიფიკატორის განახლება წარმატებით დასრულდა.';ru='Обновление адресного классификатора успешно завершено.'"));
		Иначе
			ПоказатьОповещениеПользователя(,, НСтр("ka='სამისამართო კლასიფიკატორის განახლება არ არის დასრულებული.';ru='Обновление адресного классификатора не завершено.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ka='სამისამართო კლასიფიკატორის განახლება';ru='Обновление адресного классификатора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			"Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), , Истина);
		ТекстОбОшибке = НСтр("ka='სამისამართო კლასიფიკატორის განახლება შეწყვეტილია.|დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Обновление адресного классификатора прервана.|Подробности см. в журнале регистрации.'");
	...
	Если ЗаданиеВыполнено Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ka='სამისამართო კლასიფიკატორის განახლება';ru='Обновление адресного классификатора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			"Информация", НСтр("ka='სამისამართო კლასიფიკატორის განახლება წარმატებით დასრულდა.';ru='Обновление адресного классификатора успешно завершено.'"), , Истина);
		ПоказатьОповещениеПользователя(,, НСтр("ka='სამისამართო კლასიფიკატორის განახლება წარმატებით დასრულდა.';ru='Обновление адресного классификатора успешно завершено.'"));
		ОбновитьПовторноИспользуемыеЗначения(); 
		Оповестить("АдресныйКлассификаторАктуализирован", , ЭтотОбъект);
		ПодтверждениеЗакрытияФормы = Истина;
...
Процедура АктуализироватьАдресныйКлассификатор(Результат, ФоновоеЗадание = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='ქვესისტემა სამისამართო კლასიფიკატორი მონაცემთა აქტუალიზაცია';ru='Подсистема Адресный классификатор: актуализация данных'");
