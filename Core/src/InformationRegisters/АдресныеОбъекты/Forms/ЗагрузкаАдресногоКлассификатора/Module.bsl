...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если КоличествоОбновлений > 0 Тогда
			...
			ИнформацияОбОбновление = НСтр("ka='Доступно обновление от';ru='Доступно обновление от'") + " " + ДатаОбновления;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ka='შევწყვიტოთ სამისამართო კლასიფიკატორის ჩატვირთვა?';ru='Прервать загрузку адресного классификатора?'");
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
		НСтр("ka='კატალოგი სამისამართო კლასიფიკატორის ფაილებიანად';ru='Каталог с файлами адресного классификатора'"),
...
"необходимо установить расширение для работы с файлами.'");
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю( НСтр("ka='Не указан вариант загрузки классификатора.';ru='Не указан вариант загрузки классификатора.'") );
	КонецЕсли;
...
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Даты.ДатаОбновления > Даты.ДатаСамойСтаройЗагрузки Тогда
			...
			ИнформацияОбОбновление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ის განახლება ხელმისაწვდომია';ru='Доступно обновление от %1'"), ДатаОбновленияСтрокой);
			Элементы.ИнформацияОбОбновление.Видимость = НЕ Элементы.АвторизацияНаСайтеПоддержкиПользователей.Видимость;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапуститьПроверкуДоступностиОбновления(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Проверка обновления для адресного классификатора';ru='Проверка обновления для адресного классификатора'");
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='არჩეული რეგიონების მიხედვით მიმდინარეობს მონაცემთა ჩატვირთვა (%1)';ru='Загружаются данные по выбранным регионам (%1)'"), ВыбраноРегионовДляЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач Аутентификация, Знач КодыРегионов)
	...
	Элементы.ШагиЗагрузки.ТекущаяСтраница = Элементы.ОжиданиеЗагрузки;
	ТекстСостоянияЗагрузки = НСтр("ka='ფაილების ჩატვირთვა მომხმარებლების მხარდაჭერის საიტიდან...';ru='Загрузка файлов с сайта поддержки пользователей...'");
...
Процедура ПослеФоновойЗагрузкиССайта(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		Если СтрНайти(Задание.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Задание.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			ТекстОшибки = НСтр("ka='Не удается загрузить адресные сведения.';ru='Не удается загрузить адресные сведения.'");
			ТекстОшибки = ТекстОшибки + НСтр("ka='Возможные причины:';ru='Возможные причины:'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='• Некорректно введен или не введен логин и пароль;';ru='• Некорректно введен или не введен логин и пароль;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* ინტერნეტთან შეერთება არ არის;';ru='• Нет подключения к Интернету;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* საიტზე მიმდინარეობს ტექნიკური სამუშაოები. მოგვიანებით სცადეთ ჩატვირთვის გამეორება.';ru='• На сайте ведутся технические работы. Попробуйте повторить загрузку позднее.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* ბრანდმაუერი ან სხვა შუალედური პროგრამული უზრუნველყოფა (ანტივირუსები და მისთ.) ბლოკავენ ინტერნეტთან შეერთების პროგრამის მცდელობებს;';ru='• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* ინტერნეტთან შეერთება ხორციელდება პროქსი-სერვერით, მაგრამ მისი პარამეტრები პროგრამაში არ არის მიცემული.';ru='• Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в программе.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='Техническая информация:';ru='Техническая информация:'") + " " + Символы.ПС;
			ТекстОшибки = ТекстОшибки + СтрПолучитьСтроку(Задание.КраткоеПредставлениеОшибки, 1);
		Иначе
			ТекстОшибки = НСтр("ka='Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.';ru='Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='Техническая информация:';ru='Техническая информация:'") + " " + Символы.ПС;
...
Процедура ЗавершениеЗагрузки()
	...
	// Для сброса признака АдресныйКлассификаторУстарел в параметрах работы клиента.
		...
		ТекстОписанияЗагрузки = НСтр("ka='სამისამართო კლასიფიკატორი წარმატებით ჩაიტვირთა.';ru='Адресный классификатор успешно загружен.'");
		...
		Если ЕстьУстаревшиеАдресныеСведенияКЛАДРДляУдаления() Тогда
			...
			ТекстВопроса = НСтр("ka='Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.';ru='Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ka='თუ ჩატვირთული რაიონები ბევრია, მაშინ გასუფთავების პროცესს შეიძლება ხანგრძლივი დრო დაჭირდეს.';ru='Если загруженных регионов много, то процесс очистки может занять продолжительное время.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ka='Очистить?';ru='Очистить?'");
...
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, Аутентификация, УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='სამისამართო კლასიფიკატორის ჩატვირთვა საიტიდან';ru='Загрузка адресного классификатора с сайта'");
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиента(Знач ПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстСостоянияЗагрузки = НСтр("ka='Передача файлов на сервер приложения...';ru='Передача файлов на сервер приложения...'");
...
Процедура ПослеФоновойЗагрузкиИзКаталога(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		ТекстОшибки = НСтр("ka='Не удается загрузить адресные сведения из файлов.';ru='Не удается загрузить адресные сведения из файлов.'");
		ТекстОшибки = ТекстОшибки + НСтр("ka='Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.';ru='Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.'") + Символы.ПС;
		ТекстОшибки = ТекстОшибки + НСтр("ka='Техническая информация:';ru='Техническая информация:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
		ВывестиСообщениеОбОшибке(ТекстОшибки);
...
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='სამისამართო კლასიფიკატორის ჩატვირთვა';ru='Загрузка адресного классификатора'");
...
Процедура ОчиститьУстаревшиеАдресныеСведенияКЛАДР()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Очистка адресного классификатора в формате КЛАДР';ru='Очистка адресного классификатора в формате КЛАДР'");
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Очистка адресного классификатора в формате КЛАДР';ru='Очистка адресного классификатора в формате КЛАДР'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,
