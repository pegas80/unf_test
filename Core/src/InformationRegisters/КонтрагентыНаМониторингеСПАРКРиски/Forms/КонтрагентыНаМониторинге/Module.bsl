...
Процедура СнятьСМониторинга(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ka='Снять выбранных контрагентов с мониторинга?';ru='Снять выбранных контрагентов с мониторинга?'"),
...
Процедура ПодборКонтрагентовДляМониторинга(Команда)
	...
	Иначе
		СправочникиКонтрагенты.ПоказатьВыборЭлемента(
			...
			НСтр("ka='Выбор справочника контрагентов';ru='Выбор справочника контрагентов'"));
	КонецЕсли;
...
Процедура ПриОтветеНаВопросОСнятииСМониторинга(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		...
		Если УдаленоЗаписей = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Среди выбранных контрагентов нет поставленных вручную.';ru='Среди выбранных контрагентов нет поставленных вручную.'"));
		КонецЕсли;
	КонецЕсли;
