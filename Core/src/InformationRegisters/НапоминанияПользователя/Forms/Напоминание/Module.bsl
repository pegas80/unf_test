...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		СпособУстановкиВремениНапоминания = НСтр("ka='შემდეგ';ru='через';en='across'") + " " + НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Если ИнтервалВремени > 0 Тогда
		...
		СпособУстановкиВремениНапоминания = НСтр("ka='შემდეგ';ru='через';en='across'") + " " + ИнтервалВремениСтрокой;
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დროის ინტერვალი დაუდგენელია.';ru='Интервал времени не определен.';en='the time interval is not defined.'"), , "СпособУстановкиВремениНапоминания");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Если Объект.ИнтервалВремениНапоминания > 0 Тогда
		ИнтервалВремениСтрокой = НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დროის ინტერვალი დაუდგენელია';ru='Интервал времени не определен';en='The time interval is not defined'"), , "ИнтервалВремениСтрокой");
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьПредопределенныеСпособыОповещения()
	...
	Результат.Вставить(НСтр("ka='საგანთან მიმართებაში';ru='относительно предмета';en='on the subject'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета"));
	Результат.Вставить(НСтр("ka='აღნიშნულ დროს';ru='в указанное время';en='at a specified time'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ВУказанноеВремя"));
	Результат.Вставить(НСтр("ka='პერიოდულად';ru='периодически';en='periodically'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.Периодически"));
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьСпособыОповещения()
	...
	Для Каждого Интервал Из ИнтервалыВремени Цикл
		СпособыОповещения.Добавить(НСтр("ka='შემდეგ';ru='через';en='across'") + " " + Интервал);
...
Процедура ЗаполнитьВариантыПериодичности()
	...
	Элементы.ВариантПериодичности.СписокВыбора.Добавить("", НСтр("ka='მიცემულის მიხედვით';ru='по заданному расписанию...';en='on the schedule ...'"));	
КонецПроцедуры
...
Процедура ОткрытьДиалогНастройкиРасписанияЗавершение(ВыбранноеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если Не РасписаниеСоответствуетТребованиям(Расписание) Тогда 
		ПоказатьПредупреждение(, НСтр("ka='პერიოდულობას დღის განმავლობაში მხარდაჭერა არ გააჩნია, შესაბამის გამართვები გასუფთავებულია.';ru='Периодичность в течение дня не поддерживается, соответствующие настройки очищены.';en='Periodicity during the day is not supported, the appropriate settings cleared.'"));
	КонецЕсли;
