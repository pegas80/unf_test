...
Функция ПолучитьДанныеПоШтрихкодам(Штрихкоды, СтруктурнаяЕдиница = Неопределено) Экспорт
	...
	|	Рег.ВнеоборотныйАктив КАК ВнеоборотныйАктив
	|ИЗ
	|	РегистрСведений.ШтрихкодыВА КАК Рег
	|ГДЕ
	...
	Пока Выборка.СледующийПоЗначениюПоля("Штрихкод") Цикл
		ТекДанные = ДанныеПоШтрихкодам[Выборка.Штрихкод];		
		ТекДанные.Вставить("ВнеоборотныйАктив", Выборка.ВнеоборотныйАктив);		
	КонецЦикла;
...
Функция ПолучитьМаксимальноеЗначениеКодаШтрихкодаЧислом(ПрефиксШтучногоТовара = "0", ПрефиксВнутреннегоШтрихкода = "00") Экспорт
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	МАКСИМУМ(ПОДСТРОКА(ШтрихкодыВА.Штрихкод, 5, 9)) КАК Код
		|ИЗ
		|	РегистрСведений.ШтрихкодыВА КАК ШтрихкодыВА
		|ГДЕ
		|	ШтрихкодыВА.Штрихкод ПОДОБНО &ШаблонШтрихкод"
	);
	
	ШаблонШтрихкод = "0" + ПрефиксШтучногоТовара + ПрефиксВнутреннегоШтрихкода + "_________";
...
Функция ПолучитьШтрихкодПоКоду(Код, ПрефиксШтучногоТовара = "0", ПрефиксВнутреннегоШтрихкода = "00") Экспорт
	...
	Штрихкод = "0" + ПрефиксШтучногоТовара + ПрефиксВнутреннегоШтрихкода + Формат(Код, "ЧЦ=9; ЧВН=; ЧГ=");
...
//  Строка
//
Функция СформироватьШтрихкод(ПрефиксШтучногоТовара = "0", ПрефиксВнутреннегоШтрихкода = "00", МаксимальныйКод = 99999999) Экспорт
...
Функция ПолучитьМаксимальноеЗначениеКодаВесовогоШтрихкодаЧислом(ПрефиксВесовогоТовара = "1") Экспорт
	...
	Если ТипЗнч(ПрефиксВесовогоТовара) = Тип("Число") Тогда
		текПрефиксВесовогоТовара = Формат(ПрефиксВесовогоТовара,"ЧДЦ=0; ЧГ=0");
	Иначе
		текПрефиксВесовогоТовара = ПрефиксВесовогоТовара;
	КонецЕсли;	
	...
	Запрос.УстановитьПараметр("ФорматШтрихкода", РегистрыСведений.ШтрихкодыНоменклатуры.ФорматВесовогоШтрихкода(текПрефиксВесовогоТовара));
	...
	//Рукожопокод 1Софта отключен начало
	////+++++Rajak
	//
	//	Запрос = Новый Запрос("ВЫБРАТЬ
	//                      |	ПОДСТРОКА(ШтрихкодыНоменклатуры.Штрихкод, 3, 5) КАК Код
	//                      |ИЗ
	//                      |	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	//                      |ГДЕ
	//                      |	(ШтрихкодыНоменклатуры.Штрихкод ПОДОБНО &ФорматШтрихкода
	//                      |			ИЛИ ШтрихкодыНоменклатуры.Штрихкод ПОДОБНО &ФорматШтрихкода2)
	//                      |
	//                      |УПОРЯДОЧИТЬ ПО
	//                      |	Код");
	//
	//Запрос.УстановитьПараметр("ФорматШтрихкода", РегистрыСведений.ШтрихкодыНоменклатуры.ФорматВесовогоШтрихкода("1"));
	//Запрос.УстановитьПараметр("ФорматШтрихкода2", РегистрыСведений.ШтрихкодыНоменклатуры.ФорматВесовогоШтрихкода("2"));
	//	
	//Выборка = Запрос.Выполнить().Выбрать();
	//
	//ПредШтрих = 0;
	...
	//	ЗначениеКодаЧислом = ОписаниеТипаЧисла.ПривестиЗначение(Выборка.Код);
	//	Если ЗначениеКодаЧислом <> ПредШтрих + 1 Тогда
	//		Прервать;
	//	КонецЕсли;
	//	
	//	ПредШтрих = ЗначениеКодаЧислом;
	//КонецЦикла;	
	//
	//
	//Возврат ПредШтрих;
	////-------Rajak
	//Рукожопокод 1Софта отключен конец
...
//Kuzmich, 04.06.2019, Проблемы с выгрузкой штучного товара, продолжение
// Функция возвращает массив префиксов весовых штрихкодов
Функция ПолучитьПрефиксыВесовыхШтучныхШтрихкодов() Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПравилаОбменаСПодключаемымОборудованиемOffline.ПрефиксШтучногоТовара КАК Префикс
	|ИЗ
	|	Справочник.ПравилаОбменаСПодключаемымОборудованиемOffline КАК ПравилаОбменаСПодключаемымОборудованиемOffline
	|ГДЕ
	|	ПравилаОбменаСПодключаемымОборудованиемOffline.ТипПодключаемогоОборудования = &ТипПодключаемогоОборудования");
...
Процедура ПреобразоватьВесовойШтрихкод(ТекШтрихкод) Экспорт
	...
	ПрефиксыВесовыхШтучныхШтрихкодов = ПолучитьПрефиксыВесовыхШтучныхШтрихкодов();//Kuzmich, 04.06.2019, Проблемы с выгрузкой штучного товара, продолжение
	...
	ИначеЕсли СтрДлина(ТекШтрихкод.Штрихкод) = 13 // EAN13
		И Лев(ТекШтрихкод.Штрихкод, 1) = "2" // Внутренний штрихкод
		И ПрефиксыВесовыхШтучныхШтрихкодов.Найти(Сред(ТекШтрихкод.Штрихкод, 2, 1)) <> Неопределено // Найден префикс весового но ШТУЧНОГО товара //Kuzmich, 04.06.2019, Проблемы с выгрузкой штучного товара, продолжение
		ТОГДА
	
		ВнутреннийШтрихкод12 = Лев(ТекШтрихкод.Штрихкод, 7) + "00000";
		ВнутреннийШтрихкод13 = ВнутреннийШтрихкод12 + РегистрыСведений.ШтрихкодыНоменклатуры.КонтрольныйСимволEAN(ВнутреннийШтрихкод12, 13);
		Вес = Число(Сред(ТекШтрихкод.Штрихкод, 8, 2)) + Число(Сред(ТекШтрихкод.Штрихкод, 10, 3));// вот здесь мы НЕ делим на 1000, т.к. считаем, что это штучный товар
...
&НаСервере
Процедура УcтановитьШтрихкодВА(ВнеоборотныйАктив,Штрихкод) Экспорт      
	
	ВнеоборотныйАктивОбъект = ВнеоборотныйАктив.ПолучитьОбъект();
	ВнеоборотныйАктивОбъект.Штрихкод = Штрихкод;
	ВнеоборотныйАктивОбъект.Записать();

КонецПроцедуры   

&НаСервере
Функция ПроверитьУникалностьШтрихкода(ВнеоборотныйАктив, ПроверяемыйШтрихкод, ИмеющийсяШтрихкод = "", Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("Штрихкод", ПроверяемыйШтрихкод);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		Если Выборка.Штрихкод <> ПроверяемыйШтрихкод Тогда
			 
			ИмеющийсяШтрихкод = Выборка.Штрихкод;
...
Функция ПроверитьКоличествоШтрихкодов(ВнеоборотныйАктив, ПроверяемыйШтрихкод, ИмеющийсяШтрихкод = "", Отказ) Экспорт		
