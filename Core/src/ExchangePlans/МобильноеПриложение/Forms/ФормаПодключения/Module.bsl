...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Найти(АдресДляПодключенияКЦентральнойБазеСМобильногоУстройства, "e1c://") = Истина Тогда
		Элементы.ДекорацияШаг2Заголовок.Заголовок = НСТр("ka='ეწვიეთ მობილურდანართიან გვერდს სინქრონიზაციის მოწყობის აღწერით';ru='Посетите страницу с описанием настройки синхронизации с мобильным приложением'");
		Элементы.ДекорацияСсылка.Видимость = Истина;
		Элементы.QRКод.Видимость = Ложь;
	Иначе
		Элементы.ДекорацияШаг2Заголовок.Заголовок = НСТр("ka='კავშირის დასამყარებლად მობილური ჩატვირთვის შემდეგ გამოიყენეთ გამოიყენეთ QRკოდი';ru='После загрузки мобильного приложения используйте этот QR-код для установки соединения'");
