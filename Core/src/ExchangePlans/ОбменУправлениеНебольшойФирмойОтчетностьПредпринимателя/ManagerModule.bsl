...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
		...
		НСтрока = НСтр("ka='დაწყებული %1-დან';ru='Начиная с %1';en='Beginning with 1%'");
	...
	Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
		...
		НСтрока = НСтр("ka='მხოლოდ ორგანიზაციების მიხედვით: %1';ru='Только по организациям: %1';en='Only organizations: 1%'");
	...
	НСтрока = НСтр("ka='დოკუმენტების და საცნობარო ინფორმაციის გადმოტვირთვა:"
"%1"
"%2';ru='Выгружать документы и справочную информацию:"
"%1"
"%2'");
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ka='კონფიგურაცია ""1С:მეწარმის ანგარიშგებულობა 8, რედ. 1.0""-თან გაცვლის შექმნა';ru='Создать обмен с конфигурацией ""1С:Отчетность предпринимателя 8, ред. 1.0""';en='Create a sharing configuration ""1C: Accounting 8 entrepreneur, Ed 1.0.""'");
