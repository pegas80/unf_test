...
Процедура ПолучитьОписаниеКонтекста()
	...
	Если ЗначениеЗаполнено(ДатаНачалаВыгрузкиДокументов) Тогда
		ДатаНачалаВыгрузкиДокументовОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='განხორციელდება მონაცემთა სინქრონიზაცია, %1-დან დაწყებული';ru='Данные будут синхронизироваться, начиная с %1'"),
			Формат(ДатаНачалаВыгрузкиДокументов, "ДЛФ=DD")
		);
	Иначе
		ДатаНачалаВыгрузкиДокументовОписание = НСтр("ka='მონაცემთა სინქრონიზება მოხდება პროგრამებში აღრიცხვის წარმოების მთელი პერიოდისთვის';ru='Данные будут синхронизироваться за весь период ведения учета в программах'");
	КонецЕсли;
	
	// отбор по Организациям
	Если ИспользоватьОтборПоОрганизациям Тогда
		ОрганизацииОписание = НСтр("ka='მხოლოდ ორგანიზაციების მიხედვით:';ru='Только по организациям:'") + Символы.ПС + ИспользуемыеЭлементы("Организации");
	Иначе
		ОрганизацииОписание = НСтр("ka='ყველა ორგანიზაციის მიხედვით.';ru='По всем организациям.'");
	КонецЕсли;
