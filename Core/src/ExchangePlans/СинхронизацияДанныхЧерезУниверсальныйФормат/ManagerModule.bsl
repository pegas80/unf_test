...
//
// Например:
//	Возврат НСтр("ka='გაცვლის შექმნა განაწილებულ საინფორმაციო ბაზაში';ru='Создать обмен в распределенной информационной базе';en='Create an exchange in a distributed information base'");
//
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	
	Возврат НСтр("ka='მონაცემთა უნივერსალური ფორმატის მხარდაჭერის მქონე ნებისმიერი პროგრამა';ru='Любая программа с поддержкой универсального формата данных';en='Any program that supports universal data format'");
...
"и ""Бухгалтерия предприятия, редакция 3.0"".  В синхронизации участвуют документы и нормативно-справочной информация.';ka='Данная настройка позволит синхронизировать данные между программами ""1С:Управление нашей фирмой, редакция 1.6"","
"и ""Бухгалтерия предприятия, редакция 3.0"".  В синхронизации участвуют документы и нормативно-справочной информация.'");
	Иначе
		ПоясняющийТекст = НСтр("ka='იძლევა გაცვლის უნივერსალური ფორმატ ""Enterprise Data""-ის მხარდამჭერ ნებისმიერ პროგრამებს შორის მონაცემთა სინქრონიზაციის საშუალებას.';ru='Позволяет синхронизировать данные между любыми программами, поддерживающими универсальный формат обмена ""Enterprise Data"".';en='It allows you to synchronize data between any applications that support a universal interchange format ""Enterprise Data"".'");
	КонецЕсли;
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	ТекстОписания = НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაცია ავტომატურად რეგისტრირდება გასაგზავნად;';ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;';en='All normative-reference information is automatically registered for shipment;'");
	
	Если НастройкаОтборовНаУзле.РучнойОбмен Тогда
		
		ТекстОписания = НСтр("ka='მომხმარებელი დამოუკიდებლად ახდენს გასაგზავნად დოკუმენტების შერჩევას და რეგისტრირებას:';ru='Пользователь самостоятельно отбирает и регистрирует документы к отправке;';en='User self-select and register the documents to be sent;'");
		
	Иначе
		
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоВидамДокументов Тогда
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='გასაგზავნად დოკუმენტების რეგისტრირება ავტომატურად ხდება:';ru='Документы автоматически регистрируются к отправке:';en='Documents are automatically registered for shipment:'");
		Иначе
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='გასაგზავნად ყველა დოკუმენტი ავტომატურად რეგისტრირდება:';ru='Все документы автоматически регистрируются к отправке:';en='All documents are automatically registered for shipment:'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='%ДатаНачала%-იდან დაწყებული';ru='начиная с %ДатаНачала%';en='since DataNachala%%'");
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
			...
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='ორგანიზაციების მიხედვით შერჩევით: %ПредставлениеКоллекции%';ru='с отбором по организациям: %ПредставлениеКоллекции%';en='with the selection of organizations to:%% PredstavlenieKollektsii'");
			ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
		Иначе
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='ყველა ორგანიზაციით';ru='по всем организациям';en='of all organizations'");
		КонецЕсли;
		
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоВидамДокументов Тогда
			КоллекцияЗначений = НастройкаОтборовНаУзле.ВидыДокументов.Представление;
			ПредставлениеКоллекции = СокращенноеПредставлениеКоллекцииЗначений(КоллекцияЗначений);
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='შერჩევით, დოკუმენტების სახეობების მიხედვით: %ПредставлениеКоллекции%';ru='с отбором по видам документов: %ПредставлениеКоллекции%';en='with the selection of document types:% PredstavlenieKollektsii%'");
			ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
		КонецЕсли;
	...
	Если НастройкаОтборовНаУзле.АвтоматическиЗачитыватьАвансы Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='ავანსები ავტომატურად ჩაითვლება ""საწარმოს ბუღალტერიაში"" დოკუმენტების გადატანისას';ru='Авансы автоматически зачитываются при переносе документов в ""Бухгалтерию предприятия""';en='Advances automatically credited when transferring documents to ""company accountant""'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
	КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ТекстОписания = НСтр("ka='დანახარჯების ძირითადი მუხლი დოკუმენტებში სტანდარტულად ჩასასმელად: %Значение%';ru='Основная статья затрат для подстановки в документы по умолчанию: %Значение%';en='The main cost item to insert into documents by default:% Value%'");
		...
		Иначе
			ТекстОписания = СтрЗаменить(ТекстОписания, "%Значение%", НСтр("ka='არ არის მითითებული';ru='не указана';en='not specified'"));
		КонецЕсли;
		
		ТекстОписания = ТекстОписания + Символы.ПС + Символы.ПС + НСтр("ka='სხვა დანარჩენი შემოსავლების და ხარჯების მუხლი დოკუმენტებში სტანდარტულად გასატარებლად: %Значение%';ru='Основная статья прочих доходов и расходов для подстановки в документы по умолчанию: %Значение%';en='The main items of other income and expenses to insert into documents by default:% Value%'");
		...
		ТекстОписания = ТекстОписания + Символы.ПС + Символы.ПС + НСтр("ka='მომსახურება საკომისიო ანაზღაურებაში დოკუმენტ ""ანგარიშგება კომიტენტს""-ში სტანდარტულად ჩასასმელად: %Значение%';ru='Услуга по комиссионному вознаграждению для подстановки в документ Отчет комитенту по умолчанию: %Значение%';en='Service on the commission fee for substitution in the document Report to the principal default:% Value%'");
		...
		ТекстОписания = ТекстОписания + Символы.ПС + Символы.ПС + НСтр("ka='ხარჯების ასახვის მეთოდი დოკუმენტ ""ექსპლუატაციაში მასალების გადაცემა""-ში შესავსებად: %Значение%';ru='Способ отражения расходов для заполнения документа Передача материалов в эксплуатацию: %Значение%';en='reflection process costs for filling materials document transmission into operation:% Value%'");
		...
		Иначе
			ТекстОписания = СтрЗаменить(ТекстОписания, "%Значение%", НСтр("ka='არ არის მითითებული';ru='не указан';en='not specified'"));
		КонецЕсли;
...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки = "") Экспорт
	...
	Если ИдентификаторНастройки = "ОбменУниверсальный" Тогда
		Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ka='სხვა პროგრამა';ru='Другая программа';en='another program'"));
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ka='მონაცემთა სინქრონიზაციის გასწორება უნივერსალური ფორმატით';ru='Настройка синхронизации данных через универсальный формат';en='Configuring data synchronization via a universal format'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ka='მონაცემთა სინქრონიზაცია უნივერსალური ფორმატით';ru='Синхронизация данных через универсальный формат';en='Synchronizing data via a universal format'"));
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ka='სხვა პროგრამა';ru='Другая программа';en='another program'"));
	ИначеЕсли ИдентификаторНастройки = "ОбменБП30" Тогда
		Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ka='საწარმოს ბუღალტერია, რედაქცია 3.0';ru='Бухгалтерия предприятия, редакция 3.0';en='enterprise accounting, revision 3.0'"));
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ka='პროგრამა ""საწარმოს ბუღალტერია, რედაქცია 3.0""-თან სინქრონიზაციის გასწორება';ru='Настройка синхронизации с программой ""Бухгалтерия предприятия, редакция. 3.0""';en='Set up sync with the program ""Enterprise Accounting, edition. 3.0""'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ka='სინქრონიზაცია პროგრამა ""საწარმოს ბუღალტერია, რედაქცია 3.0""-თან';ru='Синхронизация с программой ""Бухгалтерия предприятия, редакция 3.0""';en='Synchronization with the program ""Enterprise Accounting, edition 3.0""'"));
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ka='საწარმოს ბუღალტერია, რედაქცია 3.0';ru='Бухгалтерия предприятия, редакция 3.0';en='enterprise accounting, revision 3.0'"));
	Иначе
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ka='გაცვლის უნივერსალური ფორმატით';ru='Через универсальный формат обмена';en='Through universal exchange format'"));
	КонецЕсли;
...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ka='მონაცემთა სინქრონიზაცია უნივერსალური ფორმატით';ru='Синхронизация данных через универсальный формат';en='Synchronizing data via a universal format'");
...
Функция ПереходНаСинхронизациюЧерезУниверсальныйФорматВнешнееСоединение(ПараметрыНастройкиСинхронизацииДанных) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВариантНастройки) Тогда
		...
		ТекстСообщения = НСтр("ka='კორესპონდენტ-ბაზაში სინქრონიზაციის გასწორების ვარიანტი არ არის მითითებული';ru='Не указан вариант настройки синхронизации в базе-корреспонденте';en='Unknown version of the synchronization settings in base-correspondent'");
	...
	Если НЕ ЗначениеЗаполнено(ИмяПланаОбменаСтаройНастройки) Тогда
		ПараметрыНастройкиСинхронизацииДанных.Ошибка = Истина;
		НСтрока = НСтр("ka='გადაცემულია სინქრონიზაციის გასწორების უცნობი ვარიანტი: %1.';ru='Передан неизвестный вариант настройки синхронизации: %1.';en='Transferred to an unknown version of the synchronization settings:% 1.'");
	...
	Если Метаданные.ПланыОбмена.Найти(ИмяПланаОбменаСтаройНастройки) = Неопределено Тогда
		ПараметрыНастройкиСинхронизацииДанных.Ошибка = Истина;
		НСтрока = НСтр("ka='გამართვის ვარიანტის შესაბამისი გაცვლის გეგმა არ არის აღმოჩენილი: %1.';ru='Не обнаружен план обмена, соответствующий варианту настройки: %1.';en='Sharing plan is not detected, the appropriate configuration options:% 1.'");
		ПараметрыНастройкиСинхронизацииДанных.СообщениеОбОшибке = 
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, ВариантНастройки);
		Возврат Неопределено;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(НастройкаСинхронизацииДанных) Тогда
		ПараметрыНастройкиСинхронизацииДанных.Ошибка = Истина;
		НСтрока = НСтр("ka='გაცვლის გეგმისთვის კორესპონდენტის კვანძი ვერ მოიძებნა: %1, კოდი: %2.';ru='Не найден узел корреспондента для плана обмена: %1, код: %2.';en='Unable to find a correspondent node for sharing plan:% 1 code:% 2.'");
		ПараметрыНастройкиСинхронизацииДанных.СообщениеОбОшибке = 
...
Функция ВыполнитьПереходНаСинхронизациюДанныхЧерезУниверсальныйФормат(НастройкаСинхронизацииДанных) Экспорт
	...
	ИмяСобытияЖурналРегистрации = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა';ru='Переход на синхронизацию данных через универсальный формат';en='Transition to synchronize data through a universal format'");
	
	Сообщение = НСтр("ka='გადასვლის დასაწყისი';ru='Начало перехода';en='The transition'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);

	Сообщение = НСтр("ka='მონაცემთა სინქრონიზაციის ახალი გასწორების შექმნა';ru='Создание новой настройки синхронизации данных';en='Creating a new configuration data synchronization'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	Попытка
		...
		Сообщение = НСтр("ka='ცვლილებების რეგისტრაცია';ru='Регистрация изменений';en='Registration of changes'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);

		ВыполнитьРегистрациюИзменений(НастройкаСинхронизацииДанных, НастройкаСинхронизацииЧерезУниверсальныйФормат);
		
		// Настройки транспорта.
		Сообщение = НСтр("ka='შეტყობინებების ტრანსპორტის გამართვების შევსება';ru='Заполнение настроек транспорта сообщений';en='Filling transport settings Messaging'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
		...
		Сообщение = НСтр("ka='მონაცემთა ცვლილების აკრძალვის გამართვების შევსება';ru='Заполнение настроек запрета изменения данных';en='Filling settings barring information changes'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
		...
		Сообщение = НСтр("ka='მონაცემთა გაცვლის სცენარების შევსება';ru='Заполнение сценариев обмена данными';en='Filling in data exchange scenarios'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
		...
		Сообщение = НСтр("ka='მთავარი კვანძის გამართვების შევსება';ru='Заполнение настроек главного узла';en='Filling the master node configuration'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
		...
		Сообщение = НСтр("ka='მონაცემთა გაცვლის შედეგების შევსება';ru='Заполнение результатов обмена данными';en='Filling the data exchange results'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
		...
		Если НЕ НастройкаСинхронизацииДанных.ПометкаУдаления Тогда
			Сообщение = НСтр("ka='ძველი გამართვის წაშლის ნიშნული';ru='Пометка на удаление старой настройки';en='Marking to remove the old settings'");
			ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	Если ВыгружатьСообщениеПослеПерехода Тогда
		...
		Если Отказ Тогда
			Сообщение = НСтр("ka='უნივერსალური ფორმატით მონაცემთა გადმოტვირთვა არ შესრულებულა.';ru='Выгрузка данных через универсальный формат не выполнена.';en='Unloading of data via the universal format is not performed.'");
			ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Ошибка, , , Сообщение);
		Иначе
			Сообщение = НСтр("ka='მონაცემთა გადმოტვირთვა უნივერსალური ფორმატით.';ru='Выгрузка данных через универсальный формат.';en='Upload data through a universal format.'");
			ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
		КонецЕсли;
		
	КонецЕсли;
	Сообщение = НСтр("ka='გადასვლის დასრულება';ru='Окончание перехода';en='The end of the transition'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	Возврат НастройкаСинхронизацииЧерезУниверсальныйФормат;
КонецФункции
...
Процедура ОбработатьСоответствияОбъектовИнформационныхБаз(СтарыйУзел, НовыйУзел)
	...
	ИмяСобытияЖурналРегистрации = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა. მონაცემები გაცვლებში სარეგისტრაციოდ';ru='Переход на синхронизацию данных через универсальный формат. Данные для регистрации в обменах';en='Transition to synchronize data through a universal format. Data exchanges for registration'");
	Сообщение = НСтр("ka='მონაცემთა შევსება გაცვლებში სარეგისტრაციოდ';ru='Заполнение данных для регистрации в обменах';en='Filling data exchanges registration'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	ИмяСобытияЖурналРегистрации = НСтр("ka='მონაცემთა სინქრონიზაციაზე გადასვლა უნივერსალური ფორმატით.საჯარო იდენტიფიკატორების შევსება';ru='Переход на синхронизацию данных через универсальный формат.Заполнение публичных идентификаторов';en='Transition to synchronize data through a universal public identifier format.Zapolnenie'");
	Сообщение = НСтр("ka='საჯარო იდენტიფიკატორების ჩამოყალიბება - დასაწყისი';ru='Формирование публичных идентификаторов - начало';en='Formation of public IDs - the beginning'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);

	// Отображение ссылок "один ко многим"
	Если НЕ РезультатЗапроса[1].Пустой() Тогда
		Сообщение = НСтр("ka='აღმოჩენილია ჩანაწერები, რომლებშიც წყაროზე ერთ ბმულს რამდენიმე მიმღები შეესაბამება';ru='Обнаружены записи, в которых одной ссылке на источник соответствуют несколько приемников';en='Found records where a link to the source correspond to multiple receivers'") 
					+ Символы.ПС + НСтр("ka='ეს ჩანაწერები არ დამუშავდება.';ru='Эти записи обработаны не будут.';en='These records will not be processed.'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Ошибка, , , Сообщение);

		Выборка = РезультатЗапроса[1].Выбрать();
		Пока Выборка.Следующий() Цикл
			НСтрока = НСтр("ka='წყაროს უნიკალური იდენტიფიკატორი: %1, მიმღების უნიკალური იდენტიფიკატორი: %2, მიმღების ტიპი: %3';ru='Уникальный идентификатор источника: %1, Уникальный идентификатор приемника: %2, Тип приемника: %3';en='A unique source identifier% 1, a unique receiver identifier% 2, receiver type: 3%'");
	...
	КонецЦикла;
	Сообщение = НСтр("ka='საჯარო იდენტიფიკატორების ჩამოყალიბება დასრულებულია. ჩანაწერების რაოდენობა:';ru='Формирование публичных идентификаторов завершено. Количество записей:';en='Formation of public identifiers completed. Number of records:'") 
				+ СчетчикЗаписей;
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	Сообщение = НСтр("ka='საინფორმაციო ბაზების ობიექტების შესაბამისობა გასუფთავებულია';ru='Соответствие объектов информационных баз очищен';en='Matching objects database information is cleared'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	Если НастройкиТранспортаОбменаСтарогоУзла.Количество()>0 Тогда
		...
		Для Каждого НастройкаУзла Из НастройкиТранспортаОбменаНовогоУзла Цикл
			...
			Если МассивДоступныеТранспортыСообщений.Найти(ВидТранспортаПоУмолчанию) = Неопределено Тогда
				...
				ИмяСобытияЖурналРегистрации = НСтр("ka='უნივერსალური ფორმატით მონაცემთა სინქრონიზაციაზე გადასვლა. შეტყობინებების ტრანსპორტის გამართვების შევსება';ru='Переход на синхронизацию данных через универсальный формат. Заполнение настроек транспорта сообщений';en='Transition to synchronize data through a universal format. Filling transport settings Messaging'");
				НСтрока = НСтр("ka='უწინ გამოყენებული სტანდარტული შეერთება მიუწვდომელია უნივერსალური ფორმატით მონაცემთა სინქრონიზაციისთვის (%1)';ru='Используемое ранее подключение по умолчанию недоступно для синхронизации данных через универсальный формат (%1)';en='Previously used default connection is unavailable for synchronizing data through a universal format (1%)'");
				...
				Если ЗначениеЗаполнено(НастройкаУзла.ВидТранспортаСообщенийОбменаПоУмолчанию) Тогда
					НСтрока = НСтр("ka='სანაცვლოდ დანიშნულია სტანდარტულად შეერთება: %1. აუცილებელია შეტყობინებების ტრანსპორტის გამართვების კორექტულობის შემოწმება';ru='Взамен назначено подключение по умолчанию: %1. Необходимо проверить корректность настроек транспорта сообщений';en='Instead assigned to the default connection:% 1. It is necessary to check the correctness of transport settings Messaging'");
					Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, 
																НастройкаУзла.ВидТранспортаСообщенийОбменаПоУмолчанию);

				Иначе
					Сообщение = НСтр("ka='სტანდარტულად შეერთება გასუფთავებულია. აუცილებელია შეტყობინებების ტრანსპორტის ხელის რეჟიმში გასწორება';ru='Подключение по умолчанию очищено. Необходимо выполнить настройку транспорта сообщений вручную';en='The default connection is cleared. You must configure transport messages manually'");
				КонецЕсли;
				ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Ошибка, , , Сообщение);
			КонецЕсли;
		КонецЦикла;
