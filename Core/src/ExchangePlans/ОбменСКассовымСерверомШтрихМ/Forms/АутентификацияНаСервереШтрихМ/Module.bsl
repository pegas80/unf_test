...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторСервиса) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не указан идентификатор сервиса (agent_id)';ru='Не указан идентификатор сервиса (agent_id)';en='Unknown service identifier (agent_id)'",
...
Процедура ОК(Команда)
	...
	Если НЕ ЗначениеЗаполнено(КлючевоеСлово) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Введите ключевое слово';ru='Введите ключевое слово';en='Enter a keyword'",
	...
	Если Подпись = ТекущаяПодпись Тогда
		ТекстПредупреждения = НСтр("ka='Ключевое слово не изменено';ru='Ключевое слово не изменено';en='The keyword is not changed'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		ТекстПредупреждения = НСтр("ka='Изменено ключевое слово';ru='Изменено ключевое слово';en='Edit Keywords'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
