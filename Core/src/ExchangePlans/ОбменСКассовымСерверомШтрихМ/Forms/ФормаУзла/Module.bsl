...
Процедура ОчиститьИдентификатор(Команда)
	...
	ЗаголовокВопроса = НСтр("ka='Очистка идентификатора подсистемы';ru='Очистка идентификатора подсистемы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"товаров и документов в новую подсистему из существующей.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	ТекстВопроса = ТекстВопроса + НСтр("ka='Выполнить очистку идентификатора подсистемы (agent_node_id)?';ru='Выполнить очистку идентификатора подсистемы (agent_node_id)?'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
	СписокКнопок = Новый СписокЗначений;
	СписокКнопок.Добавить(КодВозвратаДиалога.Да,
		НСтр("ka='შესრულება';ru='Выполнить'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	СписокКнопок.Добавить(КодВозвратаДиалога.Отмена,
		НСтр("ka='გაუქმება';ru='Отменить'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ПроверитьПодключение(Команда)
	...
	Иначе
		СтрокаПредупреждения = НСтр("ka='Соединенение успешно установлено.';ru='Соединенение успешно установлено.'",
			ОбщегоназначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	ЗаголовокПредупреждения = НСтр("ka='შეერთების შემოწმება';ru='Проверка подключения'",
			ОбщегоназначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриИзмененииПодписи()
	...
	Если ЗначениеЗаполнено(Подпись) Тогда
		Элементы.ИзменитьПодпись.Заголовок = НСтр("ka='შეცვლა';ru='Изменить'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Элементы.ИзменитьПодпись.Заголовок = НСтр("ka='შევსება';ru='Заполнить'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
