...
Процедура ОчиститьОтборПоВидамДокументов(Команда)
	...
	ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение'");
	ТекстВопроса   = НСтр("ka='დოკუმენტის სახეობებით შერჩევა გავასუფთავოთ?';ru='Очистить отбор по видам документов?'");
	Ответ = Неопределено;
...
Функция СформироватьОписаниеПравилОтправкиУНФ()
	...
	ТекстОписания = НСтр("ka='Правила отправки данных из программы ""Управление нашей фирмой"" :';ru='Правила отправки данных из программы ""Управление нашей фирмой"" :'") + Символы.ПС;
	
	ТекстОписания = ТекстОписания + НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაცია ავტომატურად რეგისტრირდება გასაგზავნად;';ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;'");
...
Функция СформироватьОписаниеПравилОтправкиБП()
	...
	ТекстОписания = НСтр("ka='პროგრამა ""საწარმოს ბუღალტერიიდან"" მონაცემთა გაგზავნის წესები';ru='Правила отправки данных из программы ""Бухгалтерия предприятия"" :'") + Символы.ПС;
	
	ТекстОписания = ТекстОписания + НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაცია ავტომატურად რეგისტრირდება გასაგზავნად;';ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;'");
	
	Если ПравилаОтправкиДокументовБП = "НеСинхронизировать" Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='დოკუმენტები არ გაიგზავნება;';ru='Документы отправляться не будут;'");
	Иначе
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='გასაგზავნად ყველა დოკუმენტი ავტომატურად რეგისტრირდება:';ru='Все документы автоматически регистрируются к отправке:'");
