...
Процедура ОчиститьОтборПоВидамДокументов(Команда)
	...
	ТекстЗаголовка = НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'");
	ТекстВопроса   = НСтр("ka='დოკუმენტის სახეობებით შერჩევა გავასუფთავოთ?';ru='Очистить отбор по видам документов?';en='Clear the selection of types of documents?'");
	Ответ = Неопределено;
...
Процедура ОбновитьНаименованиеКомандФормы()
	...
	Иначе
		...
		НовыйЗаголовокОрганизаций = НСтр("ru='Выбрать организации ';ka='Выбрать организации '");
	...
	Иначе
		...
		НовыйЗаголовокВидовДокументов = НСтр("ru='Выбрать виды документов ';ka='Выбрать виды документов '");
...
Функция СформироватьОписаниеПравилОтправкиУНФ()
	...
	ТекстОписания = НСтр("ka='Правила отправки данных из программы ""Управление нашей фирмой"" :';ru='Правила отправки данных из программы ""Управление нашей фирмой"" :';en='Rules for sending data from the program ""Management of our company"":'") + Символы.ПС;
	
	ТекстОписания = ТекстОписания + НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაცია ავტომატურად რეგისტრირდება გასაგზავნად;';ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;';en='All normative-reference information is automatically registered for shipment;'");
...
Функция СформироватьОписаниеПравилОтправкиБП()
	...
	ТекстОписания = НСтр("ka='პროგრამა ""საწარმოს ბუღალტერიიდან"" მონაცემთა გაგზავნის წესები';ru='Правила отправки данных из программы ""Бухгалтерия предприятия"" :';en='Rules for sending data from the program ""Enterprise Accounting"":'") + Символы.ПС;
	
	ТекстОписания = ТекстОписания + НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაცია ავტომატურად რეგისტრირდება გასაგზავნად;';ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;';en='All normative-reference information is automatically registered for shipment;'");
	
	Если ПравилаОтправкиДокументовБП = "НеСинхронизировать" Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='დოკუმენტები არ გაიგზავნება;';ru='Документы отправляться не будут;';en='The documents will not be sent;'");
	Иначе
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='გასაგზავნად ყველა დოკუმენტი ავტომატურად რეგისტრირდება:';ru='Все документы автоматически регистрируются к отправке:';en='All documents are automatically registered for shipment:'");
