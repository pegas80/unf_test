...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если НастройкаОтборовНаУзле.ПереноситьАктыКакРеализациюУслуг Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ka='Документы ""Акт выполненных работ"" переносятся в ""Бухгалтерию предприятия"" как документы ""Реализация товаров и услуг""';ru='Документы ""Акт выполненных работ"" переносятся в ""Бухгалтерию предприятия"" как документы ""Реализация товаров и услуг""'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
	КонецЕсли;
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ka='კონფიგურაცია ""1C: საწარმოს ბუღალტერია 8, რედ. 3.0""-თან გაცვლის შექმნა';ru='Создать обмен с конфигурацией ""1C: Бухгалтерия предприятия 8, ред. 3.0""'");
