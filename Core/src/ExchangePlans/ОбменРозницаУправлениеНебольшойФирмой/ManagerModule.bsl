...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ka='გაცვლის გამართვები მფმ-საცალოვაჭრობა';ru='Настройки обмена УНФ-РТ';en='sharing settings UNF-RT'");
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию Тогда
		...
		Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
			ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka=', დაწყებული %1-იდან';ru=', начиная с %1';en='Starting with 1%'"),
				Формат(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов, "ДЛФ=DD"));
		Иначе
			ПредставлениеПериода = НСтр("ka='პროგრამაში აღრიცხვის წარმოების მთელი პერიოდის მანძილზე.';ru=' за весь период ведения учета в программе.'");
		КонецЕсли;
		
		ПредставлениеДокументов = НСтр("ka='Отправлять документы[ПредставлениеПериода]';ru='Отправлять документы[ПредставлениеПериода]';en='Send documents [PredstavleniePerioda]'");
	...
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьВручную Тогда
		...
		ПредставлениеДокументов = НСтр("ka='დოკუმენტები გავაგზავნოთ მხოლოდ ხელით.';ru='Отправлять документы только вручную.';en='Send documents by hand.'");
		
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать Тогда
		
		ПредставлениеДокументов = НСтр("ka='არ გავაგზავნოთ დოკუმენტები.';ru='Не отправлять документы.';en='Do not send documents.'");
	...
	Если НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию Тогда
		...
		ПредставлениеНормативноСправочнойИнформации = НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაციის გაგზავნა.';ru='Отправлять всю нормативно-справочную информацию.';en='Send all the reference data.'");
	...
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
		...
		ПредставлениеНормативноСправочнойИнформации = НСтр("ka='ნორმატიულ-საცნობარო ინფორმაციის გაგზავნა, რომელიც გასაგზავნ დოკუმენტებში გამოიყენება.';ru='Отправлять нормативно-справочную информацию, которая используется в отправляемых документах.';en='Send reference data that is used to send documents.'");
		
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать Тогда
		
		ПредставлениеНормативноСправочнойИнформации = НСтр("ka='არ გავაგზავნოთ ნორმატიულ-საცნობარო ინფორმაცია.';ru='Не отправлять нормативно-справочную информацию.';en='Do not send reference data.'");
	...
	Если (НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию)
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
			...
			ПредставлениеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მხოლოდ ორგანიზაციების მიხედვით: %1.';ru='Только по организациям: %1.';en='Only organizations:% 1.'"),
				СтрокаПредставленияОтбора);
		Иначе
			ПредставлениеОрганизаций = НСтр("ka='ყველა ორგანიზაციის მიხედვით.';ru='По всем организациям.';en='For all organizations.'");
		КонецЕсли;
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоСкладам Тогда
			...
			ПредставлениеСкладов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მხოლოდ საწყობებში: %1.';ru='Только по складам: %1.';en='Only on stock: 1%.'"),
				СтрокаПредставленияОтбора);
		Иначе
			ПредставлениеСкладов = НСтр("ka='ყველა საწყობში.';ru='По всем складам.';en='For all warehouses.'");
		КонецЕсли;
		
		ПредставлениеОграниченийОтправкиДанных = НСтр("ru='Отправлять данные:"
"[ПредставлениеОрганизаций]"
"[ПредставлениеСкладов]';ka='მონაცემთა გაგზავნა: [ПредставлениеОрганизаций] [ПредставлениеСкладов]'");
	...
	Результат = НСтр("ka='[ПредставлениеДокументов]¶[ПредставлениеНормативноСправочнойИнформации]¶[ПредставлениеОграниченийОтправкиДанных]';ru='[ПредставлениеДокументов]"
"[ПредставлениеНормативноСправочнойИнформации]"
"[ПредставлениеОграниченийОтправкиДанных]'");
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ka='კონფიგურაცია ""1C: საცალო ვაჭრობა 8, რედ. 2.2-თან გაცვლის შექმნა';ru='Создать обмен с конфигурацией ""1C: Розница 8, ред. 2.2';en='Create a sharing configuration ""1C: Retail 8 Rev 2.2.'");
...
"Синхронизация является двухсторонней и позволяет иметь актуальные данные в каждой из информационных баз.';ka='Позволяет синхронизировать данные между программами 1С:Розница и 1С:Управление нашей фирмой, "
...
Функция ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
		ОграничениеДатаНачалаВыгрузкиДокументов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დაწყებული %1-დან';ru='Начиная с %1';en='Beginning with 1%'"),
			Формат(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов, "ДЛФ=DD"));
	Иначе
		ОграничениеДатаНачалаВыгрузкиДокументов = НСтр("ka='პროგრამაში აღრიცხვის მთელი პერიოდისთვის';ru='За весь период ведения учета в программе';en='Over the entire period of record-keeping in the program'");
	КонецЕсли;
	...
	Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
		...
		ОграничениеОтборПоОрганизациям = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			СтрокаПредставленияОтбора);
	Иначе
		ОграничениеОтборПоОрганизациям = НСтр("ka='ყველა ორგანიზაციით';ru='По всем организациям';en='For all organizations'");
	КонецЕсли;
	
	Возврат (
	НСтр("ka='დოკუმენტების და საცნობარო ინფორმაციის გადმოტვირთვა:';ru='Выгружать документы и справочную информацию:';en='Upload documents and background information:'")
		+ Символы.ПС
