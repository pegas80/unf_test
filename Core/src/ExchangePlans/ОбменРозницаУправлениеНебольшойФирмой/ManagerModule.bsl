...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ka='გაცვლის გამართვები მფმ-საცალოვაჭრობა';ru='Настройки обмена УНФ-РТ'");
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию Тогда
		...
		Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
			ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka=', დაწყებული %1-იდან';ru=', начиная с %1'"),
		...
		ПредставлениеДокументов = НСтр("ka='Отправлять документы[ПредставлениеПериода]';ru='Отправлять документы[ПредставлениеПериода]'");
	...
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьВручную Тогда
		...
		ПредставлениеДокументов = НСтр("ka='დოკუმენტები გავაგზავნოთ მხოლოდ ხელით.';ru='Отправлять документы только вручную.'");
		
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать Тогда
		
		ПредставлениеДокументов = НСтр("ka='არ გავაგზავნოთ დოკუმენტები.';ru='Не отправлять документы.'");
	...
	Если НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию Тогда
		...
		ПредставлениеНормативноСправочнойИнформации = НСтр("ka='მთელი ნორმატიულ-საცნობარო ინფორმაციის გაგზავნა.';ru='Отправлять всю нормативно-справочную информацию.'");
	...
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
		...
		ПредставлениеНормативноСправочнойИнформации = НСтр("ka='ნორმატიულ-საცნობარო ინფორმაციის გაგზავნა, რომელიც გასაგზავნ დოკუმენტებში გამოიყენება.';ru='Отправлять нормативно-справочную информацию, которая используется в отправляемых документах.'");
		
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать Тогда
		
		ПредставлениеНормативноСправочнойИнформации = НСтр("ka='არ გავაგზავნოთ ნორმატიულ-საცნობარო ინფორმაცია.';ru='Не отправлять нормативно-справочную информацию.'");
	...
	Если (НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию)
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
			...
			ПредставлениеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მხოლოდ ორგანიზაციების მიხედვით: %1.';ru='Только по организациям: %1.'"),
				СтрокаПредставленияОтбора);
		Иначе
			ПредставлениеОрганизаций = НСтр("ka='ყველა ორგანიზაციის მიხედვით.';ru='По всем организациям.'");
		КонецЕсли;
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоСкладам Тогда
			...
			ПредставлениеСкладов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მხოლოდ საწყობებში: %1.';ru='Только по складам: %1.'"),
				СтрокаПредставленияОтбора);
		Иначе
			ПредставлениеСкладов = НСтр("ka='ყველა საწყობში.';ru='По всем складам.'");
		КонецЕсли;
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ka='კონფიგურაცია ""1C: საცალო ვაჭრობა 8, რედ. 2.2-თან გაცვლის შექმნა';ru='Создать обмен с конфигурацией ""1C: Розница 8, ред. 2.2'");
...
Функция ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
		ОграничениеДатаНачалаВыгрузкиДокументов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დაწყებული %1-დან';ru='Начиная с %1'"),
			Формат(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов, "ДЛФ=DD"));
	Иначе
		ОграничениеДатаНачалаВыгрузкиДокументов = НСтр("ka='პროგრამაში აღრიცხვის მთელი პერიოდისთვის';ru='За весь период ведения учета в программе'");
	КонецЕсли;
	...
	Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
		...
		ОграничениеОтборПоОрганизациям = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			СтрокаПредставленияОтбора);
	Иначе
		ОграничениеОтборПоОрганизациям = НСтр("ka='ყველა ორგანიზაციით';ru='По всем организациям'");
	КонецЕсли;
	
	Возврат (
	НСтр("ka='დოკუმენტების და საცნობარო ინფორმაციის გადმოტვირთვა:';ru='Выгружать документы и справочную информацию:'")
		+ Символы.ПС
