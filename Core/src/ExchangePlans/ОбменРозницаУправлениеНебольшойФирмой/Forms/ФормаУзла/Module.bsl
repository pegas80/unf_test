...
Процедура ОткрытьСписокВыбранныхОрганизацийНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормыВыбора",                   НСтр("ka='შერჩევისთვის აირჩიეთ ორგანიზაციები:';ru='Выберите организации для отбора:'"));
...
Процедура ОткрытьСписокВыбранныхСкладовНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормыВыбора",                   НСтр("ka='შერჩევისთვის აირჩიეთ საწყობები:';ru='Выберите склады для отбора:'"));
	ПараметрыФормы.Вставить("МассивВыбранныхЗначений",                СформироватьМассивВыбранныхЗначений(ПараметрыФормы));
	ПараметрыФормы.Вставить("ПараметрыВнешнегоСоединения",            Неопределено);
	ПараметрыФормы.Вставить("КоллекцияФильтров",                      КоллекцияФильтров);
...
Процедура ОткрытьСписокВыбранныхВидовЦенНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормыВыбора",                   НСтр("ka='შერჩევისთვის აირჩიეთ ფასების სახეობები:';ru='Выберите виды цен для отбора:'"));
	ПараметрыФормы.Вставить("МассивВыбранныхЗначений",                СформироватьМассивВыбранныхЗначений(ПараметрыФормы));
	ПараметрыФормы.Вставить("ПараметрыВнешнегоСоединения",            Неопределено);
	ПараметрыФормы.Вставить("КоллекцияФильтров",                      КоллекцияФильтров);
...
Процедура ОбновитьНаименованиеКомандФормы()
	...
	Иначе
		НовыйЗаголовок = НСтр("ka='ორგანიზაციის არჩევა';ru='Выбрать организации'");
	КонецЕсли;
	...
	Если Объект.Склады.Количество() > 0 Тогда
		...
		НовыйЗаголовок = СтрСоединить(ВыбранныеЭлементы,", ");
	Иначе
		НовыйЗаголовок = НСтр("ka='მაღაზიის არჩევა';ru='Выбрать магазины'");
	КонецЕсли;
	...
	Если Объект.ВидыЦен.Количество() > 0 Тогда
		...
		НовыйЗаголовок = СтрСоединить(ВыбранныеЭлементы,", ");
	Иначе
		НовыйЗаголовок = НСтр("ka='ფასის სახეობების არჩევა';ru='Выбрать виды цен'");
	КонецЕсли;
