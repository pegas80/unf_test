...
Процедура УстановитьПараметрыТаблицыКаталоговСервер()
	...
	ЗаголовокТаблицы = НСтр("ka='კატალოგების ცხრილი (ნომენკლატურის ჯგუფების შესაბამისობა საიტზე არსებულ კატალოგებთან)';ru='Таблица каталогов (соответствие групп номенклатуры каталогам на сайте)';en='directory table (matching groups Nomenclature Online Catalog)'");
	Если Объект.ОтборГруппыКатегорииНоменклатуры = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатуры Тогда
		ЗаголовокКолонки = НСтр("ka='ნომენკლატურის ჯგუფები';ru='Группы номенклатуры';en='Группы номенклатуры'");
		ВыборГруппИЭлементов = ГруппыИЭлементы.Группы;
	Иначе
		ЗаголовокКолонки = НСтр("ka='ნომენკლატურის კატეგორიები';ru='Категории номенклатуры';en='category nomenclature'");
...
Процедура УстановитьНадписьРасписанияОбмена()
	...
	Если Не РазделениеВключено Тогда
		...
		Если РасписаниеРегламентногоЗадания = Неопределено Тогда
			ТекстЗаголовка = НСтр("ka='გაცვლის განრიგის გამართვა';ru='Настроить расписание обмена';en='Configure sharing schedule'");
		Иначе
...
Функция ПроверитьУникальностьИдентификатора()
	...
	Если НЕ ИдентификаторыУникальны Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='კატალოგის იდენტიფიკატორი უნდა იყოს უნიკალური!';ru='Идентификатор каталога должен быть уникальным!';en='directory ID must be unique!'"),
			Объект.Ссылка,
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ОбменТоварами И ТаблицаКаталогов.Количество() = 0 Тогда
		...
		Сообщение = НСтр("ka='კატალოგების ცხრილი შევსებული არ არის!';ru='Таблица каталогов не заполнена!';en='directory table is not full!'");
...
Процедура КомандаПроверитьСоединение(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаПроверитьСоединениеЗавершение", ЭтотОбъект), 
			НСтр("ka='გაცვლის გამართვა შეცვლილია და არ ჩაწერილა. ჩავწეროთ?';ru='Настройка обмена изменена и не записана. Записать?';en='Setting the exchange changed and is not recorded. Record?'"),
...
Процедура СоответствиеСтатусовЗаказовПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ПроверитьДублированиеСтатусов(ИмяКолонки) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ასეთი სტატუსი უკვე მითითებულია ცხრილის სხვა სტრიქონში!';ru='Такой статус уже указан в другой строке таблицы!';en='This status is already specified for another row of the table!'"),
			Объект.Ссылка,
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
...
"Установить расширение работы с файлами ?';ka='Для данной операции необходимо установить расширение работы с файлами."
...
Процедура ФайлЗагрузкиНачалоВыбораЗавершение1(Подключено, ДополнительныеПараметры) Экспорт
    ...
    Диалог.Заголовок = НСтр("ka='აირჩიეთ შეკვეთებიანი xml-ფაილი';ru='Выберите xml-файл с заказами';en='Select the xml-file with orders'");
    Диалог.ПолноеИмяФайла = Объект.ФайлЗагрузки;
    Диалог.Фильтр = НСтр("ka='დოკუმენტი XML';ru='Документ XML';en='An XML document'") + " (*.xml)|*.xml";
...
Процедура ТаблицаКаталоговПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ПустаяСтрока(Элемент.ТекущиеДанные.Каталог) Тогда
		Элемент.ТекущиеДанные.Каталог = НСтр("ka='საქონლის კატალოგი';ru='Каталог товаров';en='Catalog'") + " " + ВРег(СокрЛП(Лев(Элемент.ТекущиеДанные.ИдентификаторКаталога, 8)));
	КонецЕсли;
