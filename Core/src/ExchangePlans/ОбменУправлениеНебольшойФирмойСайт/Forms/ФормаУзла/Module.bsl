...
Процедура УстановитьПараметрыТаблицыКаталоговСервер()
	...
	ЗаголовокТаблицы = НСтр("ka='კატალოგების ცხრილი (ნომენკლატურის ჯგუფების შესაბამისობა საიტზე არსებულ კატალოგებთან)';ru='Таблица каталогов (соответствие групп номенклатуры каталогам на сайте)'");
	Если Объект.ОтборГруппыКатегорииНоменклатуры = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатуры Тогда
		ЗаголовокКолонки = НСтр("ka='ნომენკლატურის ჯგუფები';ru='Группы номенклатуры'");
		ВыборГруппИЭлементов = ГруппыИЭлементы.Группы;
	Иначе
		ЗаголовокКолонки = НСтр("ka='ნომენკლატურის კატეგორიები';ru='Категории номенклатуры'");
...
Процедура УстановитьНадписьРасписанияОбмена()
	...
	Если Не РазделениеВключено Тогда
		...
		Если РасписаниеРегламентногоЗадания = Неопределено Тогда
			ТекстЗаголовка = НСтр("ka='გაცვლის განრიგის გამართვა';ru='Настроить расписание обмена'");
		Иначе
...
Функция ПроверитьУникальностьИдентификатора()
	...
	Если НЕ ИдентификаторыУникальны Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='კატალოგის იდენტიფიკატორი უნდა იყოს უნიკალური!';ru='Идентификатор каталога должен быть уникальным!'"),
			Объект.Ссылка,
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ОбменТоварами И ТаблицаКаталогов.Количество() = 0 Тогда
		...
		Сообщение = НСтр("ka='კატალოგების ცხრილი შევსებული არ არის!';ru='Таблица каталогов не заполнена!'");
...
Процедура КомандаПроверитьСоединение(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаПроверитьСоединениеЗавершение", ЭтотОбъект), 
			НСтр("ka='გაცვლის გამართვა შეცვლილია და არ ჩაწერილა. ჩავწეროთ?';ru='Настройка обмена изменена и не записана. Записать?'"),
...
Процедура СоответствиеСтатусовЗаказовПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ПроверитьДублированиеСтатусов(ИмяКолонки) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ასეთი სტატუსი უკვე მითითებულია ცხრილის სხვა სტრიქონში!';ru='Такой статус уже указан в другой строке таблицы!'"),
			Объект.Ссылка,
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
...
Процедура ФайлЗагрузкиНачалоВыбораЗавершение1(Подключено, ДополнительныеПараметры) Экспорт
    ...
    Диалог.Заголовок = НСтр("ka='აირჩიეთ შეკვეთებიანი xml-ფაილი';ru='Выберите xml-файл с заказами'");
    Диалог.ПолноеИмяФайла = Объект.ФайлЗагрузки;
    Диалог.Фильтр = НСтр("ka='დოკუმენტი XML';ru='Документ XML'") + " (*.xml)|*.xml";
...
Процедура ТаблицаКаталоговПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ПустаяСтрока(Элемент.ТекущиеДанные.Каталог) Тогда
		Элемент.ТекущиеДанные.Каталог = НСтр("ka='საქონლის კატალოგი';ru='Каталог товаров'") + " " + ВРег(СокрЛП(Лев(Элемент.ТекущиеДанные.ИдентификаторКаталога, 8)));
	КонецЕсли;
