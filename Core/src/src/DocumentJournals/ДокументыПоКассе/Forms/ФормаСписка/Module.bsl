...
Процедура Подключаемый_ОбработатьАктивизациюСтрокиСписка()
	...
	Если ТекущаяСтрока <> Неопределено
		...
		Дата = НСтр("ru = 'Ведомость за '; ka = 'უწყისი თვის '") + Формат(ТекущаяСтрока.Дата, "ДЛФ=D");
...
Процедура ПоступлениеВозвратКредитаКонтрагентомКасса(Команда)
	...
	ДвиженияДенежныхСредствКлиент.СоздатьДокумент("ПоступлениеВКассу", "ВозвратКредитаКонтрагентом", ДанныеМеток, "ДокументыПоКассе");
...
Процедура РасходВыдачаКредитаКасса(Команда)
	...
	ДвиженияДенежныхСредствКлиент.СоздатьДокумент("РасходИзКассы", "ВыдачаКредитаКонтрагенту", ДанныеМеток, "ДокументыПоКассе");
...
Процедура ЗаполнитьТаблицуИтогов()
	...
	ТаблицаИтогов.Загрузить(ДвиженияДенежныхСредствВызовСервера.ПолучитьЧистуюТаблицуИтоговДляЖурналаДокументов());
...
Процедура НастройкаИтоговЗавершениеНаСервере() Экспорт
	...
	ИначеЕсли ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ВедомостьЗаПериод") Тогда
		...
		Элементы.ДекорацияВедомостьПоВалюте.Заголовок = НСтр("ru = 'Ведомость по валюте: '; ka = 'უწყისი ვალუტის მიხედვით: '")+ОтборВалютаВедомости;
	...
	ИначеЕсли ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ОстаткиПересчитанныеВВалюту") Тогда
		...
		Элементы.ОтборКурс.Заголовок = НСтр("ru = 'Курс на '; ka = 'კურსი '")+Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy")+"";
		ОтборКурс = ""+СтруктураДанных.Курс;
		ИнформацияСуммаВалКонечныйОстаток = СтруктураДанных.Остаток;
		Элементы.ДекорацияОстатокВВалюте.Заголовок = НСтр("ru = 'Остатки в валюте: '; ka = 'ნაშთი ვალუტაში: '")+ОтборВалютаОстатков;
