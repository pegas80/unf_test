...
Функция ЗамаскироватьНомерТелефона(Знач НомерТелефона) Экспорт
	...
	Если МассивСимволов.Количество() = 12 Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение(НСтр("ka='ტელეფონის ნომრის გარდაქმნის შეცდომა.';ru='Ошибка преобразования номера телефона.';en='Error converting phone numbers.'"));
	КонецЕсли;
...
Функция НомерТелефона(Знач НомерТелефона) Экспорт 
	...
	Иначе
		ВызватьИсключение(НСтр("ka='ფორმატ +79999999999-ში ტელეფონის ნომრის გარდაქმნის შეცდომა.';ru='Ошибка преобразования номера телефона к формату +79999999999.';en='Error converting to the format of the telephone number 79999999999.'"));
	КонецЕсли;
