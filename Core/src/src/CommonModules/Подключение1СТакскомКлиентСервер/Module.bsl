...
Процедура ОпределитьВозможностьЗапуска(МестоЗапуска, ПараметрыИнтернетПоддержки, ОписаниеДействия) Экспорт
	...
	Если НЕ ПараметрыИнтернетПоддержки.ИспользоватьПодключение1СТакском Тогда
		...
		ОписаниеДействия.Вставить("Сообщение",
			НСтр("ka='1С-ტაქსკომის შეერთების გამოყენება მუშაობის მიმდინარე რეჟიმში მიუწვდომელია.';ru='Использование подключения 1С-Такском недоступно в текущем режиме работы.';en='Using connections 1C Taxcom not available in the current operation.'"));
		Возврат;
