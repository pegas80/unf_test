...
"Возможно нет прав доступа к документу!';ka='Произошла ошибка, при попытке доступа к документу ""Заявление абонента оператора электронного документооборота"": %1."
...
Процедура ВключитьОтслеживаниеИзмененияСтатусаЗаявления(ДокументЗаявление) Экспорт
	...
	Если ДокументЗаявление.Статус = Перечисления.СтатусыЗаявленияАбонентаСпецоператораСвязи.Отправлено Тогда
		...
		ПараметрыЗадания.Вставить("Наименование", 	НСтр("ru='Отслеживание заявления ';ka='Отслеживание заявления '") + ДокументЗаявление.Номер + НСтр("ka='1С-ანგარიშგებობის მიხედვით';ru=' по 1С-Отчетности'"));
...
Функция ОбновитьРеквизитыЗаявления(Документ, РеквизитыДокумента) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Обработка заявлений.Обновление статуса';ru='Обработка заявлений.Обновление статуса';en='Processing zayavleniy.Obnovlenie status'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбменССерверомПолучитьОтвет(ДокументЗаявление, ЭтоВызовИзМастера, ТекстОшибокДляМастера)
	...
	Если Сервис = Неопределено Тогда
		Результат.Вставить("СтатусКомментарий", НСтр("ka='Не удалось соединиться с сервером';ru='Не удалось соединиться с сервером';en='Could not connect to server'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если кодРезультата = "0" Тогда
		...
		Иначе
			...
			СтатусКомментарий = НСтр("ka='При получении ответа возникла ошибка: ответ нечитаем';ru='При получении ответа возникла ошибка: ответ нечитаем';en='When you receive an error occurred Answer: The answer is unreadable'");
	...
	ИначеЕсли Кодрезультата = "1" Тогда
		...
		СтатусКомментарий = НСтр("ka='Заявление еще не обработано сервером, попробуйте позже.';ru='Заявление еще не обработано сервером, попробуйте позже.';en='The statement has not yet been processed by the server, try again later.'");
		Результат.Вставить("СтатусКомментарий", СтатусКомментарий);
	...
	Иначе
		...
		Если УзелDOM.Количество() > 0 Тогда
			...
			СтатусКомментарий = НСтр("ru='При получении ответа возникла ошибка: ';ka='При получении ответа возникла ошибка: '") + сообщениеОшибки;
		Иначе
			СтатусКомментарий = НСтр("ru='При получении ответа возникла ошибка: ';ka='При получении ответа возникла ошибка: '") + кодРезультата;
		КонецЕсли;
...
Функция РегистрацияРазобратьОтвет(ДокументЗаявление, РезультатОбмена, ЭтоВызовИзМастера, ТекстОшибокДляМастера)
		...
		Для Счетчик = 0 по Архив1.Элементы.Количество() - 1 Цикл
			...
			Если Архив1.Элементы[Счетчик].расширение = "bin" Тогда
				...
				Иначе
					...
					ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СообщитьИлиПередатьВМастерОшибку(
									НСтр("ka='განცხადება უარყოფილია';ru='Заявление отклонено';en='application is rejected'"),
									ЭтоВызовИзМастера,
									ТекстОшибокДляМастера, 
									не ЭтоВызовИзМастера);
	...
	Исключение
		...
		СтатусКомментарий = НСтр("ka='Ошибка. Не удалось разобрать ответ сервера.';ru='Ошибка. Не удалось разобрать ответ сервера.';en='Mistake. Unable to parse the server response.'");
