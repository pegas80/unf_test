...
Функция ИмяСобытияУдалениеФайла()
	...
	Возврат НСтр("ka='Хранилище сертификатов.Удаление файла';ru='Хранилище сертификатов.Удаление файла';en='Storage File sertifikatov.Udalenie'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ДобавитьПроверкаВходящихПараметров(Сертификат, ТипХранилища)
	...
	Если ТипЗнч(Сертификат) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ka='Недопустимое значение параметра Сертификат (указан адрес, который не является адресом временного хранилища)';ru='Недопустимое значение параметра Сертификат (указан адрес, который не является адресом временного хранилища)';en='Certificate Invalid parameter value (the address that is not a temporary storage location)'"), 
			"ХранилищеСертификатов.Добавить");
	КонецЕсли;
