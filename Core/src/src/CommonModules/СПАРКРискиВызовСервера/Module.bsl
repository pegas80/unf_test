...
Процедура ВключитьОтключитьМониторингСобытийКонтрагента(Контрагент, МониторингВключен) Экспорт
	...
	Если Не СПАРКРиски.ИспользованиеРазрешено("ПостановкаНаМониторинг;") Тогда
		ВызватьИсключение НСтр("ka='Недостаточно прав для постановки контрагентов на мониторинг.';ru='Недостаточно прав для постановки контрагентов на мониторинг.';en='Not have permission for the production of counterparties to monitor.'");
	КонецЕсли;
...
Функция ПолучитьПервыйИндекс(АдресВременногоХранилища) Экспорт
	...
	Если ПустаяСтрока(АдресВременногоХранилища) Тогда
		...
		Результат.Вставить("ТекстОшибки", НСтр("ka='Вызов веб-сервиса не сохранил данные в хранилище.';ru='Вызов веб-сервиса не сохранил данные в хранилище.';en='Call web service does not save the data in the repository.'"));
