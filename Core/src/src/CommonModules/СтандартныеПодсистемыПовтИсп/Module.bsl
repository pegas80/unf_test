...
"указано имя подсистемы ""%2"", которое уже зарегистрировано ранее.';ka='შეცდომა ქვესისტემების აღწერილობის მომზადებისას: ქვესისტემის აღწერილობაში (იხ. პროცედურა %1.ქვესისტემისდამატებისას) მითითებულია ქვესისტემის სახელი ""%2"", რომელიც უკვე რეგისტრირებულია უწინ.'"),
...
"не совпадает с версией конфигурации в метаданных ""%4"".';ka='შეცდომა ქვესისტემების აღწერილობის მომზადებისას:"
"კონფიგურაცია ""%1""-ის ვერსია ""%2"" (იხ. პროცედურა %3.ქვესისტემისდამატებისას)"
"მეტამონაცემებში ""%4"" კონფიგურაციის ვერსიას არ ემთხვევა.'"),
				Описание.Имя,
				Описание.Версия,
	...
	Для каждого КлючИЗначение Из ВсеТребуемыеПодсистемы Цикл
		Если ОписанияПодсистем.ПоИменам.Получить(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при подготовке описаний подсистем:"
"не найдена подсистема ""%1"" требуемая для подсистем: %2.';ka='შეცდომა ქვესისტემების აღწერილობის მომზადებისას: ქვესისტემა ""%1"" ვერ მოიძებნა, რომელიც საჭიროა ქვესისტემებისთვის: %2.'"),
				КлючИЗначение.Ключ,
				ЗависимыеПодсистемы);
		КонецЕсли;
	КонецЦикла;
...
"- РассылкаОтчетов,"
"- УправлениеДоступом.';ka='ცნობარ მეტამონაცემთა ობიექტების იდენტიფიკატორების გამორთვა შეუძლებელია, თუ ნებისმიერი ქვემოთ ჩამოთვლილი ქვესისტემა გამოიყენება:"
"- ანგარიშგებებისვარიანტები,"
"- დამატებითიანგარიშგებებიდადამუშავებები,"
"- ანგარიშგებებისდგზავნა,"
"- წვდომისმართვა.'");
	КонецЕсли;
...
Функция ПараметрыПрограммныхСобытий() Экспорт
	...
	Если НЕ СохраненныеПараметры.Свойство("ОбработчикиСобытий") Тогда
		ПредставлениеПараметра = НСтр("ka='მოვლენების დამმუშავაბელი';ru='Обработчики событий';en='event handlers'");
	КонецЕсли;
...
"""%1"".';ka='Ошибка обновления информационной базы."
"Не заполнен параметр служебных событий:"
"""%1"".'")
...
Функция ПодготовленныеОбработчикиСерверногоСобытия(Событие, Служебное = Ложь, ПерваяПопытка = Истина)
	...
	Если Обработчики = Неопределено Тогда
		Если Служебное Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერული სამსახურებრივი მოვლენა ""%1"" ვერ მოიძებნა.';ru='Не найдено серверное служебное событие ""%1"".';en='We found the server service event ""%1"".'"), Событие);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერული დავალება ""%1"" ვერ მოიძებნა.';ru='Не найдено серверное событие ""%1"".';en='Server not found the event ""%1"".'"), Событие);
		КонецЕсли;
	КонецЕсли;
