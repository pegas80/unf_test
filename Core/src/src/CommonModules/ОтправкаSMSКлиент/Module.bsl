...
Процедура ПроверитьНаличиеНастроекДляОтправкиSMS(ОбработчикРезультата)
	...
	Иначе
		...
		Иначе
			ТекстСообщения = НСтр("ka='SMS-ის გასაგზავნად საჭიროა შეერთების პარამეტრების გამართვის შესრულება. მიმართეთ ადმინისტრატორს.';ru='Для отправки SMS требуется произвести настройку параметров подключения"
