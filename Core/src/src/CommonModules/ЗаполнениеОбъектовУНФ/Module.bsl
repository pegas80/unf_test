...
Процедура ЗаполнитьДокумент(ДокументОбъект, Знач ДанныеЗаполнения, Знач СтратегияЗаполнения = Неопределено, ИсключаяСвойства = "") Экспорт
	...
	// Олег Литвиненко 29.05.23 --->
	ДополнитьТипСобытияВСобытии(ДанныеЗаполнения, ДокументОбъект);
	// Олег Литвиненко <---
	
	ДанныеЗаполнения.Вставить("Автор", Пользователи.ТекущийПользователь());
	
	// Олег Литвиненко 25.05.23 --->
	//ДополнитьПроектВСобытии(ДанныеЗаполнения, ДокументОбъект);
	// Олег Литвиненко <---
...
Процедура ДополнитьВалюты(ЗначенияИзНастроек, ДокументОбъект) Экспорт
	...
	Для Каждого ИмяРеквизита Из ИменаРеквизитов(ВалютаПоУмолчанию, ДокументОбъект) Цикл
		...
		//+++ Дмгг, 25.07.24, ONESOFT-3349
		Если ИмяРеквизита = "ВалютаАрендыКвартир" Тогда
			Продолжить;
		КонецЕсли;
		//--- Дмгг, 25.07.24, ONESOFT-3349
...
// Олег Литвиненко 25.05.23 --->
Процедура ДополнитьПроектВСобытии(ДанныеЗаполнения, ДокументОбъект)
	
	Если ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.Событие") Тогда
		Возврат;
	КонецЕсли;

	//ДокументОбъект.Проект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Проект");
	//ДокументОбъект.ТипСобытия = Перечисления.ТипыСобытий.Прочее;
	ДанныеЗаполнения.Вставить("Проект", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Проект"));	
	
КонецПроцедуры// Олег Литвиненко <---

// Олег Литвиненко 29.05.23 --->
Процедура ДополнитьТипСобытияВСобытии(ДанныеЗаполнения, ДокументОбъект)
	
	Если ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.Событие") Тогда
		Возврат;
	КонецЕсли;

	ДанныеЗаполнения.Вставить("ТипСобытия", Перечисления.ТипыСобытий.Прочее);
...
Процедура ДополнитьПредопределеннымиЭлементамиСправочников(ДанныеЗаполнения, ДокументОбъект)
	...
	//ДополнитьПредопределеннойОрганизацией(ПредопределенныеЭлементыСправочников, ДокументОбъект); //+++ Дмгг, 12.12.24, ONESOFT-3697
...
Процедура ДополнитьПредопределеннымиСтруктурнымиЕдиницами(ПредопределенныеЭлементыСправочников, ДокументОбъект)
	...
	Если Не ОтсутствуетНезаполненныйРеквизит("СтруктурнаяЕдиница", ДокументОбъект) Тогда
		...
		//Малышев 2024-07-05 ONESOFT-3000 (-- - если нет прав на предопределенный - то чистим	
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		Запрос.УстановитьПараметр("СтруктурнаяЕдиница", ПредопределеннаяСтруктурнаяЕдиница);
		Если Запрос.Выполнить().Пустой() Тогда
			 ПредопределеннаяСтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
		КонецЕсли;	
		//Малышев 2024-07-05 ONESOFT-3000 --)	
