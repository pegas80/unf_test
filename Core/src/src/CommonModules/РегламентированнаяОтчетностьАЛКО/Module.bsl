	...
	Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
		...
		Если РазныеСпискиВидовДеятельности Тогда
			...
			Если Форма.мСтруктураМногостраничныхРазделов.Свойство("Лицензии") Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='კოპირებისას გასუფთავდა  ლიცენზიის განყოფილების ყველა სახის მოქმედება. "
"';ru='При копировании были очищены виды деятельности в разделе лицензий.';en='When copying were cleaned activities in the license section.'");
				Сообщение.Сообщить();
...
"будет произведена попытка перехода на последнюю строку';ka='Номер строки оказался больше максимально возможного."
...
"изменяется двойным щелчком, клавишей DEL или действием ""Удалить"".';ka='Помеченные крестиком не выгружаются, не печатаются и не учавствуют в итогах,"
"изменяется двойным щелчком, клавишей DEL или действием ""Удалить"".'");
	...
	Поле.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
	Поле.Подсказка = НСтр("ka='სტრიქონის ნომერი';ru='Номер строки.';en='line number.'");
	...
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		...
		Если Прав(Результат, 4) = ".tmp" Тогда
			...
			Исключение
			    ...
			    ЗаписьЖурналаРегистрации(НСтр("ka='Запись документа Выгрузка регламентированного отчета.';ru='Запись документа Выгрузка регламентированного отчета.';en='Writing the document Unloading regulated reports.'"),
		...
		РегламентированнаяОтчетность.ОбработатьСобытие1СОтчетности(
					НСтр("ka='Регламентированный отчет. Выгрузка';ru='Регламентированный отчет. Выгрузка';en='Regulated report. Unloading'"), ДокументСсылкаАлкоОтчет);
		Возврат;
