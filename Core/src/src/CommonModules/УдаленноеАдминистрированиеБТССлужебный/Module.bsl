...
Функция ИспользуемаяВерсияСервисаУправляющегоПриложения(Знач ПарольПользователя = "") Экспорт
	...
	Если ПоддерживаемыеВерсии.Найти(ИспользуемаяВерсия) = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ka='კორესპონდენტი %1-ის მიერ ინტერფეისი %2 არ არის მხარდაჭერილი!';ru='Корреспондент %1 не поддерживает интерфейс %2!';en='Correspondent% 1 does not support the interface% 2!'"),
...
Процедура ЗавершитьСеансыОбластиДанных(Знач НомераСеансов, Знач ПарольПользователя) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='დანართის მმართველის მიმდინარე ვერსიის მიერ დანართებიდან სეანსის დასრულებას მხარდაჭერა არ გააჩნია. აუცილებელია მმართველი დანართის განახლება.';ru='Текущая версия управляющего приложения не поддерживает завершение сеанса из приложений. Необходимо обновить управляющее приложение.';en='The current version of the control application does not support the conclusion of the session of the application. It is necessary to update the management application.'");
...
Процедура ОтправитьЗапросыНаИспользованиеВнешнихРесурсов(Знач СериализацияЗапросов) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ka='Текущая версия управляющего приложения не поддерживает запросы разрешений на использование внешних ресурсов. Необходимо обновить управляющее приложение.';ru='Текущая версия управляющего приложения не поддерживает запросы разрешений на использование внешних ресурсов. Необходимо обновить управляющее приложение.';en='The current version does not support management application requests permission to use external resources. It is necessary to update the management application.'");
