...
"</DIV><BR>'"),
		...
		Если НачалоДня(ТекущаяДатаСеанса())=НачалоДня(Выборка.МоментВремени) Тогда
			МоментВремениСтрокой = НСтр("ru='Сегодня в ';ka='დღეს'")+Формат(Выборка.МоментВремени, НСтр("ka='ДФ=чч:мм';ru='ДФ=ЧЧ:мм';en='DF = hh: mm'"));
		ИначеЕсли НачалоДня(НачалоДня(ТекущаяДатаСеанса())-1)=НачалоДня(Выборка.МоментВремени) Тогда
			МоментВремениСтрокой = НСтр("ru='Вчера в ';ka='გუშინ '")+Формат(Выборка.МоментВремени, НСтр("ka='ДФ=чч:мм';ru='ДФ=ЧЧ:мм';en='DF = hh: mm'"));
		ИначеЕсли Год(Выборка.МоментВремени)<>Год(ТекущаяДатаСеанса()) Тогда 
			МоментВремениСтрокой = Формат(Выборка.МоментВремени, НСтр("ru=""ДФ='дд МММ гггг ЧЧ:мм'"";en=""DF = 'MMM dd YYYY HH: MM'"""))
		Иначе
			МоментВремениСтрокой = Формат(Выборка.МоментВремени, НСтр("ka=""ДФ='дд МММ ЧЧ:мм'"";ru=""ДФ='дд МММ ЧЧ:мм'"";en=""DF = 'MMM dd HH: MM'"""))
		КонецЕсли; 
		Результат = Результат+СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='<DIV STYLE=""DISPLAY: BLOCK; WIDTH: 100%; MARGIN-BOTTOM: 5px; %1"">"
"<DIV STYLE=""WIDTH: 9em; FLOAT: LEFT; COLOR: gray;"">%2</DIV>"
...
Процедура ОбсужденияПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") Тогда
		Если ОбщегоНазначения.ЭтоДокумент(Источник.Метаданные()) Тогда
			Сообщение = ?(Источник.Проведен, НСтр("ka='შექმნილია და გატარებულია';ru='Создан и проведен';en='Created and performed'"), НСтр("ka='შექმნილია';ru='Создан';en='created'"));
		Иначе
			Сообщение = НСтр("ka='შექმნილია';ru='Создан';en='created'");
		КонецЕсли; 
...
Функция ОбсужденияСравнитьОбъектыСправочник(НовыйОбъект)
	...
	Если НЕ СтарыйОбъект.ПометкаУдаления И НовыйОбъект.ПометкаУдаления Тогда
		Сообщение = НСтр("ka='Помечен на удаление';ru='Помечен на удаление';en='For deletion'");
	КонецЕсли; 
	
	Если СтарыйОбъект.ПометкаУдаления И НЕ НовыйОбъект.ПометкаУдаления Тогда
		Сообщение = НСтр("ka='წაშლის ნიშნული მოხსნილია';ru='Снята пометка на удаление';en='Unstarred removal'");
	КонецЕсли;
...
Функция ОбсужденияСравнитьОбъектыДокумент(НовыйОбъект, РежимЗаписи, РежимПроведения)
	...
	Если ЕстьРеквизитОбъекта("Запасы", СтарыйОбъект) Тогда
		...
		Если СтароеКоличество<НовоеКоличество Тогда
			Префикс = НСтр("ru='Табличная часть ""Запасы"": Добавлены строки';en='A table of the ""Stocks"": Added line'");
			ДобавитьСообщениеОбИзмененииРеквизита(ОписаниеИзменений, Префикс);
		ИначеЕсли СтароеКоличество>НовоеКоличество Тогда
			Префикс = НСтр("ru='Табличная часть ""Запасы"": Удалены строки';en='A table of the ""Stocks"": delete a row'");
			ДобавитьСообщениеОбИзмененииРеквизита(ОписаниеИзменений, Префикс);
		КонецЕсли; 
	...
	Если МетаданныеДокумента = Метаданные.Документы.ЗаказПокупателя Тогда
		...
		Если НЕ СтарыйОбъект.КалькуляцияРассчитана И НовыйОбъект.КалькуляцияРассчитана Тогда
			Префикс = НСтр("ru='Калькуляция заказа сформирована';en='Calculation of the order is formed'");
			ДобавитьСообщениеОбИзмененииРеквизита(ОписаниеИзменений, Префикс);
		ИначеЕсли СтарыйОбъект.КалькуляцияРассчитана И НЕ НовыйОбъект.КалькуляцияРассчитана Тогда
			Префикс = НСтр("ru='Калькуляция заказа не актуальна';en='Calculation of the order is not relevant'");
			ДобавитьСообщениеОбИзмененииРеквизита(ОписаниеИзменений, Префикс);
		ИначеЕсли НовыйОбъект.КалькуляцияРассчитана Тогда
			...
			Если СтараяТЧ.Количество()<>НоваяТЧ.Количество()
				...
				Префикс = НСтр("ru='Изменена калькуляция заказа';en='Change the order of calculation'");
				ДобавитьСообщениеОбИзмененииРеквизита(ОписаниеИзменений, Префикс);
			КонецЕсли; 
		КонецЕсли;
	...
	Если ОписаниеИзменений = "" И ОбъектМодифицирован = Истина Тогда
		...
		ОписаниеИзменений = НСтр("ka='შეყვანილია ცვლილებები';ru='Внесены изменения';en='amended'")
	КонецЕсли;
...
Процедура ДобавитьТекстСтандартныхСообщений(Сообщение, СтарыйОбъект, НовыйОбъект, РежимЗаписи)
	...
	Если НЕ СтарыйОбъект.Проведен И РежимЗаписи=РежимЗаписиДокумента.Проведение Тогда
		//Сообщение = НСтр("ka='Помечен на удаление';ru='Помечен на удаление';en='For deletion'");
		Сообщение = НСтр("ka='დოკუმენტი გატარებულია';ru='Документ проведен';en='Document posted'");//+Giga
	КонецЕсли; 
	
	Если СтарыйОбъект.Проведен И РежимЗаписи=РежимЗаписиДокумента.ОтменаПроведения Тогда
		Сообщение = НСтр("ka='გატარება გაუქმებულია';ru='Проведение отменено';en='Carrying canceled'");
	КонецЕсли; 
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ГруппаДел	= НСтр("ka='შეხსენებები';ru='Напоминания';en='reminders'");
	...
	Дело.Важное			= Ложь;
	Дело.Представление	= НСтр("ka='ჩემი შეხსენებები';ru='Мои напоминания';en='My Alerts'");
...
Процедура СоставПолейЗаполненияДляРабочегоПроцесса(ТаблицаОписанияПолей, знач ТипДействия) Экспорт
	...
	НовоеОписание.Заголовок					= НСтр("ka='ვის შევახსენოთ';ru='Кому напомнить';en='To remind'");
	...
	НовоеОписание.Заголовок					= НСтр("ka='შეხსენების თარიღი';ru='Дата напоминания';en='reminder Date'");
