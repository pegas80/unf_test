...
Функция Расшифровать(ЗашифрованныеДанные, Сертификат, ТипШифрования, ПараметрыШифрования) Экспорт
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			ВызватьИсключение(НСтр("ka='Не удалось выполнить расшифровку сообщения';ru='Не удалось выполнить расшифровку сообщения';en='Failed to decrypt the message'"));
		КонецЕсли;
...
Функция Подписать(Данные, Подписант, ТипПодписи, ПараметрыПодписания) Экспорт 
	...
	Если Не ЗначениеЗаполнено(Сертификат) Тогда
		ВызватьИсключение(НСтр("ka='Сертификат подписанта не найден в хранилище сертификатов.';ru='Сертификат подписанта не найден в хранилище сертификатов.';en='Certificate signer not found in the certificate store.'"));
	КонецЕсли;
...
Функция ПолучитьСертификатыИзПодписи(Подпись) Экспорт
	...
	Иначе
		ВызватьИсключение(НСтр("ka='Параметр <Подпись> не является файлом подписи';ru='Параметр <Подпись> не является файлом подписи';en='The <Signature> is not a valid signature'"));
	КонецЕсли;	
...
Функция ПолучитьДвоичныеДанныеСертификата(Сертификат)
	...
	Если Не ЗначениеЗаполнено(ДвоичныеДанныеСертификата) Тогда
		ВызватьИсключение(НСтр("ka='Не удалось извлечь двоичные данные сертификата';ru='Не удалось извлечь двоичные данные сертификата';en='Unable to retrieve the binary data of the certificate'"));
	КонецЕсли;
...
Функция ВыполнитьМетодКриптосервиса(Метод, ПараметрыМетода)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронная подпись в модели сервиса.Сервис криптографии.Выполнение запроса';ru='Электронная подпись в модели сервиса.Сервис криптографии.Выполнение запроса';en='Electronic signature model servisa.Servis kriptografii.Vypolnenie request'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура ЗашифроватьПроверкаВходящихПараметров(Данные, Получатели, ТипШифрования, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		ТипШифрования = "CMS",
		НСтр("ka='Недопустимое значение параметра ТипШифрования (неизвестный тип шифрования)';ru='Недопустимое значение параметра ТипШифрования (неизвестный тип шифрования)';en='Invalid parameter value TipShifrovaniya (unknown encryption type)'"), 
		"СервисКриптографии.Зашифровать");
	...
	Если ТипЗнч(Данные) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ka='Недопустимое значение параметра Данные (указан адрес, который не является адресом временного хранилища)';ru='Недопустимое значение параметра Данные (указан адрес, который не является адресом временного хранилища)';en='Invalid parameter data (the address that is not a temporary storage location)'"), 
			"СервисКриптографии.Зашифровать");
	КонецЕсли;
...
Процедура ПодписатьПроверкаВходящихПараметров(Данные, Подписант, ТипПодписи, ПараметрыПодписания)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ka='Недопустимое значение параметра ТипПодписи (неизвестный тип подписи)';ru='Недопустимое значение параметра ТипПодписи (неизвестный тип подписи)';en='Invalid parameter value TipPodpisi (unknown signature type)'"), 
		"СервисКриптографии.Подписать");
...
Процедура ПолучитьСвойстваКриптосообщенияПроверкаВходящихПараметров(Криптосообщение)
	...
	Если ТипЗнч(Криптосообщение) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ka='Недопустимое значение параметра Криптосообщение (указан адрес, который не является адресом временного хранилища)';ru='Недопустимое значение параметра Криптосообщение (указан адрес, который не является адресом временного хранилища)';en='Invalid parameter value Kriptosoobschenie (the address that is not a temporary storage location)'"), 
			"СервисКриптографии.ПолучитьСвойстваКриптосообщения");
	КонецЕсли;
...
Процедура РасшифроватьПроверкаВходящихПараметров(ЗашифрованныеДанные, ТипШифрования, ПараметрыШифрования)
	...
	Если ТипЗнч(ЗашифрованныеДанные) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ka='Недопустимое значение параметра ЗашифрованныеДанные (указан адрес, который не является адресом временного хранилища)';ru='Недопустимое значение параметра ЗашифрованныеДанные (указан адрес, который не является адресом временного хранилища)';en='Invalid parameter value ZashifrovannyeDannye (the address that is not a temporary storage location)'"), 
			"СервисКриптографии.Расшифровать");
	КонецЕсли;
...
Процедура ПроверитьПодписьПроверкаВходящихПараметров(Подпись, Данные, ТипПодписи, ПараметрыПодписания)
	...
	Если ТипЗнч(Подпись) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ka='Недопустимое значение параметра Подпись (указан адрес, который не является адресом временного хранилища)';ru='Недопустимое значение параметра Подпись (указан адрес, который не является адресом временного хранилища)';en='Invalid parameter value Text (Set an address that is not the address of the temporary storage)'"), 
			"СервисКриптографии.ПроверитьПодпись");
	КонецЕсли;
	...
	Если ТипПодписи = "GOST3410" Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ka='Для проверки подписи по ГОСТ Р 34.10-94 необходимо указание сертификата';ru='Для проверки подписи по ГОСТ Р 34.10-94 необходимо указание сертификата';en='To verify the signature of the GOST R 34.10-94 certificate must be instructed'"), 
			"СервисКриптографии.ПроверитьПодпись");
	КонецЕсли;
...
Процедура ХешированиеДанныхПроверкаВходящихПараметров(Данные, АлгоритмХеширования, ПараметрыХеширования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ka='Недопустимое значение параметра АлгоритмХеширования (неизвестный алгоритм хеширования)';ru='Недопустимое значение параметра АлгоритмХеширования (неизвестный алгоритм хеширования)';en='Invalid parameter value AlgoritmHeshirovaniya (unknown hash algorithm)'"), 
		"СервисКриптографии.ХешированиеДанных");
...
Функция ИмяСобытияШифрование()
	...
	Возврат НСтр("ka='კრიპტოგრაფიის სერვისი. დაშიფვრა';ru='Сервис криптографии.Шифрование';en='service kriptografii.Shifrovanie'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияСвойстваСертификата()
	
	Возврат НСтр("ka='Сервис криптографии.Свойства сертификата';ru='Сервис криптографии.Свойства сертификата';en='Service certificate kriptografii.Svoystva'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияСвойстваКриптосообщения()
	
	Возврат НСтр("ka='Сервис криптографии.Свойства криптосообщения';ru='Сервис криптографии.Свойства криптосообщения';en='Service kriptografii.Svoystva kriptosoobscheniya'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияАутентификация()
	
	Возврат НСтр("ka='კრიპტოგრაფიის სერვისი. აუთენთიფიკაცია';ru='Сервис криптографии.Аутентификация';en='service kriptografii.Autentifikatsiya'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияПроверкаПодписи()
	
	Возврат НСтр("ka='Сервис криптографии.Проверка подписи';ru='Сервис криптографии.Проверка подписи';en='Service signatures kriptografii.Proverka'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияПроверкаСертификата()
	
	Возврат НСтр("ka='Сервис криптографии.Проверка сертификата';ru='Сервис криптографии.Проверка сертификата';en='Service certificate kriptografii.Proverka'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияРасшифровка()
	
	Возврат НСтр("ka='კრიპტოგრაფიის სერვივი.გაშიფვრა';ru='Сервис криптографии.Расшифровка';en='service kriptografii.Rasshifrovka'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияХеширование()
	
	Возврат НСтр("ka='კრიპტოგრაფიის სევისი.ხეშირება';ru='Сервис криптографии.Хеширование';en='service kriptografii.Heshirovanie'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияПодписание()
	
	Возврат НСтр("ka='Сервис криптографии.Подписание';ru='Сервис криптографии.Подписание';en='service kriptografii.Podpisanie'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияУстановкаСоединенияССервером()
	
	Возврат НСтр("ka='Сервис криптографии.Установка соединения с сервером';ru='Сервис криптографии.Установка соединения с сервером';en='Service server connection kriptografii.Ustanovka'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
