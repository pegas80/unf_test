...
Функция ОтправитьПочтовоеСообщение(Знач ПараметрыПисьма, Знач УчетнаяЗапись) Экспорт
	...
	Если ТипЗнч(УчетнаяЗапись) <> Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты")
		...
		ВызватьИсключение НСтр("ka='სააღრიცხვო ჩანაწერი შეუვსებელია ან არასწორად შევსებულია.';ru='Учетная запись не заполнена или заполнена неправильно.';en='Account is not filled or is filled correctly.'");
	КонецЕсли;
	...
	Если ОбменСGoogle.ЭтоУчетнаяЗаписьGoogle(УчетнаяЗапись) Тогда
		...
		Если ОбменСGoogleКлиентСервер.НеЗаполненТокенДоступа(СеансовыеДанные) Тогда
			ВызватьИсключение НСтр("ka='საფოსტო შეტყობინების გასაგზავნად დაშვების ტოკენი არ არის შევსებული. განმეორებით მოახდინეთ Gmail-ის სააღრიცხვო ჩანაწერის შეერთება.';ru='Не заполнен токен доступа для отправки почтового сообщения."
...
Процедура ЗагрузитьПочтуПоКомандеПользователя(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='ელექტრონული ფოსტის შეტყობინებების სათაურების ჩატვირთვა';ru='Загрузка заголовков сообщений электронной почты';en='Download e-mail headers'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьСодержаниеСообщения(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Параметры.УчетнаяЗапись) Тогда
			ВызватьИсключение НСтр("ka='დავალება ""შეტყობინებისშემცველობისჩატვირთვა()""-სთვის სააღრიცხვო ჩანაწერი არ არის მითითებული.';ru='Не указана учетная запись для задания ""ЗагрузитьСодержаниеСообщения()"".';en='Not specified account for the task ""ZagruzitSoderzhanieSoobscheniya ()"".'");
		КонецЕсли;
		...
		Если ОбменСGoogleКлиентСервер.НеЗаполненТокенДоступа(Параметры.СеансовыеДанные) Тогда
			ВызватьИсключение НСтр("ka='შეტყობინების შინაარსის ჩატვირთვის მისაცემად დაშვების ტოკენი არ არის შევსებული."
"განმეორებით მოახდინეთ Gmail-ის სააღრიცხვო ჩანაწერის შეერთება.';ru='Не заполнен токен доступа для задания загрузки содержания сообщения."
"Повторно подключите учетную запись Gmail.'");
		КонецЕсли;
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ka='ელექტრონული ფოსტის შეტყობინების შემცველობის ჩატვირთვა';ru='Загрузка содержания сообщения электронной почты';en='Loading content e-mail messages'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		СообщитьОбОшибкеДлительногоЗадания(ИнформацияОбОшибке);
...
Функция ЗаголовокПрогресса(Знач УчетнаяЗапись, Знач ОтборПоКонтакту) Экспорт
	...
	Если ЗначениеЗаполнено(ОтборПоКонтакту) Тогда
		Возврат СтрШаблон(НСтр("ka='მიმდინარეობს მიმოწერის ჩატვირთვა ""%1""-დან ""%2""-ში';ru='Загружается переписка с ""%1"" из ""%2""';en='Loading correspondence with ""%1"" from ""%2""'"), ОтборПоКонтакту, УчетнаяЗапись);
	Иначе
		Возврат СтрШаблон(НСтр("ka='Получение почты ""%1""';ru='Получение почты ""%1""';en='Receiving Mail ""%1""'"), УчетнаяЗапись);
	КонецЕсли;
...
Процедура ЗаданиеЗагрузитьПочту() Экспорт
	...
	|ГДЕ
	|	УчетныеЗаписиПользователя.УчетнаяЗапись.ИспользоватьДляПолучения = ИСТИНА //Малышев 2025-09-08 - 
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ОбработатьПараметрыПисьма(ПараметрыПисьма, УчетнаяЗапись)
	...
	Если ПараметрыПисьма = Неопределено Тогда
		ВызватьИсключение НСтр("ka='არ არის მითითებული გაგზავნის პარამეტრები.';ru='Не заданы параметры отправки.';en='Do not set sending options.'");
	КонецЕсли;
	...
	Если ТипЗнчКому = Неопределено И ТипЗнчКопии = Неопределено И ТипЗнчСлепыеКопии = Неопределено Тогда
		ВызватьИсключение НСтр("ka='მითითებული არ არის არცერთი მიმღები.';ru='Не указано ни одного получателя.';en='Unknown any recipient.'");
	КонецЕсли;
...
Функция ЗагрузитьЗаголовкиСообщений(УчетнаяЗапись, РежимЗагрузки, ОтборПоКонтакту)
	...
	Если ОбменСGoogleКлиентСервер.НеЗаполненТокенДоступа(СеансовыеДанные) Тогда
		ВызватьИсключение НСтр("ka='შეტყობინების სათაურების ჩატვირთვის დავალებისთვის დაშვების ტოკენი არ არის შევსებული."
"განმეორებით ჩართეთ Gmail-ის სააღრიცხვო ჩანაწერი.';ru='Не заполнен токен доступа для задания загрузки заголовков сообщений."
"Повторно подключите учетную запись Gmail.'");
	КонецЕсли;
...
Функция ЗагрузитьЗаголовкиСообщенийИнтернетПочты(УчетнаяЗапись, РежимЗагрузки, Знач Пользователь, Знач ОтборПоКонтакту = Неопределено)
	...
	Для Каждого ТекСтрока Из ТаблицаИнтернетПочтовыхСообщений Цикл
		...
		СтрШаблон(НСтр("ka='%1: ჩატვირთულია %3-დან %2';ru='%1: загружено %2 из %3';en='% 1: uploaded% 2% 3'"),
		ЗаголовокПрогресса,
...
Функция ЗаписатьСобытие(ИнтернетПочтовоеСообщение, ПочтовыйЯщик, Идентификатор, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СобытияПоИдентификатору) Тогда
			...
			НСтр("ka='შესაძლებელია, რომ ფოსტის ჩატვირთვის რამდენიმე დავალება იყოს გაშვებული: შეტყობინება იდენტიფიკატორ %1-ით უკვე ჩაწერილია.';ru='Возможно, запущено несколько заданий загрузки почты: сообщение с идентификатором %1 уже записано.';en='Perhaps running multiple download tasks mail address: Message ID% 1 has already been recorded.'"),
			Идентификатор);
...
Функция ПротоколИнтернетПочтыIMAP(УчетнаяЗапись)
	...
	ВызватьИсключение НСтр("ka='Подключение к серверу входящей почты может быть выполнено только по протоколу IMAP.';ru='Подключение к серверу входящей почты может быть выполнено только по протоколу IMAP.';en='Connecting to the server of incoming mail can only be done via IMAP.'");
...
Процедура ОтключитьЗаданиеЗагрузкиПочты()
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка электронной почты по расписанию';en='Download e-mail on a schedule'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОтправкаОтчетовПоЭлектроннойПочте() Экспорт

	ИмяСервераSMTP			= "";
	ЛогинSMTP               = "";
	ПарольSMTP              = "";
	ИспользоватьSSLSMTP		= "";
	КоличествоПисемЗаРаз    = "";
	ПортSMTP    			= "";
	
	ПолучитьНастройкиПочты(ИмяСервераSMTP,
												ИспользоватьSSLSMTP,
												КоличествоПисемЗаРаз,
												ЛогинSMTP,
												ПарольSMTP,
	...
	Если НЕ ЗначениеЗаполнено(ИмяСервераSMTP) 
	...
	ТаблицаОтправки = ПолучитьТаблицуОтправки(КоличествоПисемЗаРаз);
	...
	Для Каждого Строк Из ТаблицаОтправки Цикл 
		
		//ТекстПисьма = "Расчетный Лист";
		ТекстПисьма = "PaySlip";
		
		Сообщение = Новый ИнтернетПочтовоеСообщение;
		Сообщение.Отправитель = ЛогинSMTP;
		//Сообщение.Тема = "Расчетный Лист";
		Сообщение.Тема = "PaySlip";
		Сообщение.Получатели.Добавить(Строк.ПолучательМейл);
		Сообщение.Тексты.Добавить(ТекстПисьма);
		
		КаталогВременныхФайлов = КаталогВременныхФайлов();
		
		ТабДок = Строк.ВложеныйФайл.Получить();
		Если ТабДок <> Неопределено Тогда
			ПутьКФайлу = КаталогВременныхФайлов+"РасчетныйЛист.xlsx";
			//--------------------------------------------------------------------------
			// © Шишка В.В. [10 ноября 2023]  НАЧАЛО
			Если Строк.ИмяОтчета = "РасчетныеЛисткиПлюсПФ_СОтправкой" Тогда
			
				ПутьКФайлу = КаталогВременныхФайлов+"PaySlip.xlsx";
				
			КонецЕсли; 	
			// © Шишка В.В. [10 ноября 2023]  КОНЕЦ
			//--------------------------------------------------------------------------
			
			ТабДок.Записать(ПутьКФайлу, "XLSX");
		КонецЕсли;
		...
		НаборЗаписей = РегистрыСведений.ОтправкаОтчетаНаЭлектроннуюПочту.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ПолучательМейл.Установить(Строк.ПолучательМейл);
		НаборЗаписей.Отбор.ИмяОтчета.Установить(Строк.ИмяОтчета);	
...
Функция ПолучитьТаблицуОтправки(КоличествоПисемЗаРаз) Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ "+КоличествоПисемЗаРаз+"
		|	ОтправкаОтчетаНаЭлектроннуюПочту.ПолучательМейл КАК ПолучательМейл,
		|	ОтправкаОтчетаНаЭлектроннуюПочту.ИмяОтчета КАК ИмяОтчета,
		|	ОтправкаОтчетаНаЭлектроннуюПочту.ВложеныйФайл КАК ВложеныйФайл
		|ИЗ
		|	РегистрСведений.ОтправкаОтчетаНаЭлектроннуюПочту КАК ОтправкаОтчетаНаЭлектроннуюПочту";
	
	Возврат Запрос.Выполнить().Выгрузить();

КонецФункции // ПолучитьТаблицуОтправки(КоличествоПисемЗаРаз)()

Процедура ПолучитьНастройкиПочты(ИмяСервераSMTP, ИспользоватьSSLSMTP, КоличествоПисемЗаРаз, ЛогинSMTP, ПарольSMTP, ПортSMTP) Экспорт
	...
	Если ХЗНастройкиОбмена <> Неопределено И ТипЗнч(ХЗНастройкиОбмена) = Тип("ХранилищеЗначения") Тогда
		...
		Если ТипЗнч(НастройкиОбмена) = Тип("Структура") Тогда
			...
			ИмяСервераSMTP			= НастройкиОбмена.ИмяСервераSMTP;	
		    ИспользоватьSSLSMTP     = НастройкиОбмена.ИспользоватьSSLSMTP;
			КоличествоПисемЗаРаз    = НастройкиОбмена.КоличествоПисемЗаРаз;
			ЛогинSMTP               = НастройкиОбмена.ЛогинSMTP;
			ПарольSMTP              = НастройкиОбмена.ПарольSMTP;
			ПортSMTP                = НастройкиОбмена.ПортSMTP;
		КонецЕсли; 
