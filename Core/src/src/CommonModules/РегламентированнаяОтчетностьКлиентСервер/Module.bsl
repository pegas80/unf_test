...
Процедура СообщитьОбОшибке(Знач ТекстСообщения, Отказ, Заголовок = "", Знач Статус = Неопределено, ВызыватьИсключение = Истина) Экспорт
	...
	#Если Сервер Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='რეგლამენტირებული ანგარიშგებობა';ru='Регламентированная отчетность';en='regulatory reporting'"), УровеньЖурналаРегистрации.Ошибка, , , Заголовок + Символы.ПС + ТекстСообщения);
	...
	#Иначе
		...
		Если ЗначениеЗаполнено(Заголовок) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), Заголовок);
