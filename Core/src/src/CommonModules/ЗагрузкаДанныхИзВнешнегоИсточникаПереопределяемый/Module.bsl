...
Процедура ДобавитьУсловноеОформлениеТаблицыСопоставлений(ЭтотОбъект, ПутьРеквизита, НастройкиЗагрузкиДанных) Экспорт
	...
	ИначеЕсли НастройкиЗагрузкиДанных.ЭтоЗагрузкаСправочника Тогда
		...
		ТекстНовыйЭлемент	= Нстр("ka='<შეიქმნება ახალი ელემენტი>';ru='<Будет создан новый элемент>';en='<New item will be created>'");
		ТекстПропущены		= Нстр("ka='<მონაცემები გამოტოვებული იქნება>';ru='<Данные будут пропущены>';en='<Data will be ignored>'");
	...
	ИначеЕсли НастройкиЗагрузкиДанных.ЭтоЗагрузкаРегистраСведений Тогда
		...
		ТекстУсловногоОформления = Нстр("ka='<სტრიქონი გამოტოვებული იქნება...>';ru='<Строка будет пропущена...>';en='<String will be skipped ...>'");
...
Процедура ПреобразоватьСтрокуВДату(ДатаРезультат, ДатаСтрокой) Экспорт
	...
	Иначе
		...
		// Kuzmich, 09.10.2025, (ONESOFT-4416) Загрузка поле "дата" в документе "списания топлива и запасов"
		Если СтрНайти(КопияДатаСтрокой, ":")>0 Тогда
		...
		Для каждого Разделитель Из МассивРазделителей Цикл
			...
			// Kuzmich, 09.10.2025, (ONESOFT-4416) Загрузка поле "дата" в документе "списания топлива и запасов"
			Если ИщемВремя Тогда
			
				ПозицияРазделителя = СтрНайти(ГодСтрокой, " ");
				Если ПозицияРазделителя > 0 Тогда
					
					ГодСтрокой = Лев(КопияДатаСтрокой, ПозицияРазделителя - 1);
					КопияДатаСтрокой = Сред(КопияДатаСтрокой, ПозицияРазделителя + 1);
					
					ПозицияРазделителя = СтрНайти(КопияДатаСтрокой, ":");
					Если ПозицияРазделителя > 0 Тогда
						
						ЧасСтрокой = Лев(КопияДатаСтрокой, ПозицияРазделителя - 1);
						КопияДатаСтрокой = Сред(КопияДатаСтрокой, ПозицияРазделителя + 1);
						
						ПозицияРазделителя = СтрНайти(КопияДатаСтрокой, ":");
						Если ПозицияРазделителя > 0 Тогда
							
							МинутаСтрокой = Лев(КопияДатаСтрокой, ПозицияРазделителя - 1);
							СекундаСтрокой = Сред(КопияДатаСтрокой, ПозицияРазделителя + 1);
			...
			Если НЕ ПустаяСтрока(ЧислоСтрокой) 
				...
				Попытка
					...
					,Число(ЧасСтрокой), Число(МинутаСтрокой), Число(СекундаСтрокой)); // Kuzmich, 09.10.2025, (ONESOFT-4416) Загрузка поле "дата" в документе "списания топлива и запасов"
...
Процедура СопоставитьПроект(ЗначениеДопРеквизита, СтроковоеЗначение, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	//ПодольскийАИ #БезПроекта 09.09.2021 {
	ИначеЕсли ЗначениеПоУмолчанию <> Неопределено Тогда
		ЗначениеДопРеквизита = ЗначениеПоУмолчанию;
	//ПодольскийАИ #БезПроекта 09.09.2021 }
	КонецЕсли;
...
// Олег Литвиненко 14.10.22 --->
Процедура СопоставитьРасходнуюНакладную(РасходнаяНакладная, РасходнаяНакладная_ВходящиеДанные) Экспорт
	
	Если ПустаяСтрока(РасходнаяНакладная_ВходящиеДанные) Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Запрос = Новый Запрос("Выбрать Документ.РасходнаяНакладная.Ссылка Где Номер ПОДОБНО &Номер УПОРЯДОЧИТЬ ПО Дата Убыв");
	Запрос.УстановитьПараметр("Номер", "%" + РасходнаяНакладная_ВходящиеДанные + "%");
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
...
Процедура СопоставитьЗаказ(Заказ, Заказ_ВходящиеДанные) Экспорт
	...
	Для каждого ТегИзМассива Из МассивТеговПоставщиков Цикл
		...
		Если СтрНайти(Заказ_ВходящиеДанные, ТегИзМассива) > 0 Тогда
			...
			НомерДляПоиска = СокрЛП(СтрЗаменить(НомерДляПоиска, ТегИзМассива, ""));//++Lazare 28.07.2023

		КонецЕсли;
		
	КонецЦикла;
	
	Запрос = Новый Запрос("Выбрать Документ.%Док.Ссылка Где Номер ПОДОБНО &Номер УПОРЯДОЧИТЬ ПО Дата Убыв");
	Запрос.Текст = СтрЗаменить(Запрос.Текст,"%Док",ВидДокумента); //++Lazare 28.07.2023
...
//+++ Дмгг
Процедура СопоставитьТранспортноеСредство(ТранспортноеСредство, ТранспортноеСредство_ВходящиеДанные) Экспорт
	
	Если НЕ ПустаяСтрока(ТранспортноеСредство_ВходящиеДанные) Тогда
		
		СправочникСсылка = Справочники.ТранспортныеСредства.НайтиПоНаименованию(ТранспортноеСредство_ВходящиеДанные, Истина);
		
		Если НЕ ЗначениеЗаполнено(СправочникСсылка) Тогда
			
			СправочникСсылка = Справочники.ТранспортныеСредства.НайтиПоКоду(ТранспортноеСредство_ВходящиеДанные);
	...
	Если ЗначениеЗаполнено(СправочникСсылка) Тогда
		...
		ТранспортноеСредство = СправочникСсылка;
...
//ПодольскийАИ #УчетПроектов 31.10.2020 {
Процедура СопоставитьДолжность(ЗначениеДопРеквизита, СтроковоеЗначение) Экспорт
	...
	Запрос = Новый Запрос("Выбрать Справочник.Должности.Ссылка КАК Должность Где Наименование Подобно &Наименование");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ЗначениеДопРеквизита = Выборка.Должность;
...
Процедура СопоставитьВидРабочегоВремени(ЗначениеДопРеквизита, СтроковоеЗначение) Экспорт
	...
	Запрос = Новый Запрос("Выбрать Справочник.ВидыРабочегоВремени.Ссылка КАК ВидВремени Где Наименование Подобно &Наименование");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ЗначениеДопРеквизита = Выборка.ВидВремени;
