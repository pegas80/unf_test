...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	//Паганини_
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалЗапретаРедактированияДокументов") Тогда
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнч(ТекущийОбъект));
		Если ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда	
			Форма.ТолькоПросмотр = ТекущийОбъект.ЗапретРедактированияДокументов;
		КонецЕсли;
	КонецЕсли;
	//_Паганини
...
Процедура ОбработкаПроверкиЗаполнения(Форма, Отказ, ПроверяемыеРеквизиты, Объект = Неопределено) Экспорт
	...
	//+++ Дмгг
	//Не Знаю что за код, но срочно надо было исправить Приходный ордер
	Если Форма.ИмяФормы = "Документ.ПриходныйОрдер.Форма.ФормаДокумента" Тогда 
		Возврат;
	КонецЕсли;
	//--- Дмгг
		...
		Если Строка.ЗаполнятьОбязательно И НЕ Строка.Удалено Тогда
			...
			Если НЕ ЗначениеЗаполнено(Форма[Строка.ИмяРеквизитаЗначение]) Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ველი ""%1"" არ არის შევსებული.';ru='Поле ""%1"" не заполнено.';en='The ""%1"" is not filled.'"), Строка.Наименование),
					"");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
		...
		Если НаборыСвойствОбъекта.Количество() > 1 Тогда
			...
			Иначе
				...
				Если ОписаниеНабора = Неопределено Тогда
					...
					ОписаниеНабора.Заголовок = НСтр("ka='მოშორებული ელემენტები';ru='Удаленные реквизиты';en='deleted details'")
				КонецЕсли;
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			Если Пользователи.РолиДоступны("ДобавлениеИзменениеДополнительныхРеквизитовИСведений") Тогда
				...
				Команда.Заголовок = НСтр("ka='დამატებითი რეკვიზიტების შემადგენლობის შეცვლა';ru='Изменить состав дополнительных реквизитов';en='Change the composition of the additional details'");
				Команда.Действие = "Подключаемый_РедактироватьСоставСвойств";
				Команда.Подсказка = НСтр("ka='დამატებითი რეკვიზიტების შემადგენლობის შეცვლა';ru='Изменить состав дополнительных реквизитов';en='Change the composition of the additional details'");
	...
	Если ОпцияИспользоватьСвойства
		...
		Если Родитель <> Неопределено Тогда
			...
			ПредупреждениеОРазмещении = НСтр("ka='დამატებითი რეკვიზიტების ასასახად აუცილებელია ჯგუფ ""%1""-ის არა პირველ ელემენტად (ნებისმიერი სხვა ჯგუფის შემდეგ) განთავსება ჯგუფ ""%2""-ში (მენიუ კიდევ - ფორმის შეცვლა).';ru='Для отображения дополнительных реквизитов необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).';en='To display additional details necessary to place group ""%1"" is not the first element (after any other group) in the group ""%2"" (Another menu - Change shape).'");
