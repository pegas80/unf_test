...
"%ИнформацияОбОшибке%';ka='სბ-ის მომხმარებლის სახელის მიღების შეცდომა (ახალიამბებისდამუშავებაგანმშესრ.სბმომხმარებლისსახელისმიღება()) შემდეგი მიზეზით:"
"%ИнформацияОбОшибке%'");
...
"%ИнформацияОбОшибке%';ka='ცნობარ ""მომხმარებლები""-იდან შემდეგი სახელის მქონე მომხმარებლის მიღების შეცდომა (%ИмяПользователяИБ%) სბ-ში (ახალიამბებისდამუშავებაგანმშესრ.სბმომხმარებლისახელისმიხედვითმომხმარებლისმიღება()) შემდეგი მიზეზით:"
"%ИнформацияОбОшибке%'");
...
"Причина:"
"%ИнформацияОбОшибке%"
"';ka='Ошибка поиска пользователя (%ПолныеПрава%) по уникальному идентификатору:"
"Краткое имя пользователя: %КраткоеИмяПользователя%"
"Полное имя пользователя: %ПолноеИмяПользователя%"
"Уникальный идентификатор: %УникальныйИдентификатор%"
"Причина:"
"%ИнформацияОбОшибке%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПолныеПрава%", ?(ПолныеПрава = Истина, НСтр("ka='ხელმისაწვდომია სრული უფლებები';ru='доступны полные права';en='available full rights'"), НСтр("ka='роль РедакторНовостей';ru='роль РедакторНовостей';en='role RedaktorNovostey'")));
				...
				Если НЕ НайденныйПользователь.Пустая() Тогда
					Результат.Добавить(
						Новый Структура("Пользователь, ПользовательПредставление, ПолныеПрава",
							...
							?(ПолныеПрава = Истина,
								НайденныйПользователь.Наименование + " " + НСтр("ka='(სრული უფლებები)';ru='(полные права)';en='(Full access)'"),
								НайденныйПользователь.Наименование + " " + НСтр("ka='(სიახლეების რედატორი)';ru='(редактор новостей)';en='(News editor)'")),
							ПолныеПрава));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьОписаниеОтборов(ЛентаНовостей, Пользователь) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если ствПользовательскиеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ka='მომხმარებლის მიერ გამართული შერჩევები (შეცვლა შეიძლება):';ru='Отборы, настроенные пользователем (можно изменить):';en='Selections, user-configured (you can change):'") + Символы.ПС;
			Для Каждого ТекущееПредставлениеОтбора Из ствПользовательскиеОтборы Цикл
				...
				Если МассивЗначений.Количество() = 0 Тогда
					ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + "  " + ТекущееПредставлениеОтбора.Ключ + НСтр("ka=': * (ნებისმიერი მნიშვნელობა)';ru=': * (любое значение)';en=': * (Any value)'") + Символы.ПС;
		...
		Если ствАдминистративныеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ka='ადმინისტრატორის მიერ გამართული შერჩევები (შეცვლა არ შეიძლება):';ru='Отборы, настроенные администратором (нельзя изменить):';en='Selections are configured by the administrator (you can not change):'") + Символы.ПС;
		...
		Если ствАвтоматическиЗаполняемыеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ka='ავტომატურად შევსებადი შერჩევები (შეცვლა არ შეიძლება):';ru='Автоматически заполняемые отборы (нельзя изменить):';en='Automatically fills selections (do not change):'") + Символы.ПС;
		...
		Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
			ПредставлениеВсехОтборовПодробно = НСтр("ka='შერჩევები არ არის გამართული';ru='Отборы не настроены';en='Selections are not set'");
		КонецЕсли;

	Иначе
		ПредставлениеВсехОтборовПодробно = НСтр("ka='ამ ლენტზე შერჩევების გამართვა არ ხდება';ru='По этой ленте отборы не настраиваются';en='For this tape selections can not be adjusted'");
	КонецЕсли;
