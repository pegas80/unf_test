...
Процедура ПолучитьМаркерБезопасностиПослеПолучения(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		КонецЕсли;
	Иначе
		ТекстИсключения = НСтр("ka='Пользователь отказался от ввода пароля';ru='Пользователь отказался от ввода пароля';en='User refused to enter a password'");
	КонецЕсли;
...
Процедура Расшифровать(ОповещениеОЗавершении, ЗашифрованныеДанные, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		Если СвойстваСообщения.Тип = "envelopedData" Тогда
			...
			КонецЦикла;			
		Иначе
			ВызватьИсключение(НСтр("ka='Недопустимое значение параметра ЗашифрованныеДанные - файл не является криптосообщением';ru='Недопустимое значение параметра ЗашифрованныеДанные - файл не является криптосообщением';en='Invalid parameter value ZashifrovannyeDannye - file is not kriptosoobscheniem'"));
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Идентификаторы) Тогда
			ВызватьИсключение(НСтр("ka='В хранилище отсутствуют сертификаты для расшифровки сообщения.';ru='В хранилище отсутствуют сертификаты для расшифровки сообщения.';en='In store no certificates to decrypt the message.'"));
		КонецЕсли;
...
Процедура РасшифроватьПереборомСертификатов(ВходящийКонтекст, ИнформацияОбОшибке = Неопределено)
	...
	Если ВходящийКонтекст.ТекущийИдентификатор < ВходящийКонтекст.Идентификаторы.Количество() Тогда
		Попытка
			...
			Если ТипЗнч(Результат) = Тип("Структура") Тогда
				...
				Иначе
					ВызватьИсключение(НСтр("ka='Неизвестный код возврата';ru='Неизвестный код возврата';en='Unknown return code'"));
				КонецЕсли;
			Иначе
				РезультатВыполнения = Новый Структура;
				РезультатВыполнения.Вставить("Выполнено", Истина);
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ТекстИсключения) Тогда
			ТекстИсключения = НСтр("ka='Не удалось выполнить расшифровку сообщения';ru='Не удалось выполнить расшифровку сообщения';en='Failed to decrypt the message'");
		КонецЕсли;
		Попытка			
			ВызватьИсключение(ТекстИсключения);
		Исключение
