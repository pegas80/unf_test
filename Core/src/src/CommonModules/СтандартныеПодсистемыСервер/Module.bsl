...
Функция ПараметрыАдминистрирования() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ka='ოპერაციის შესასრულებლად არასაკმარისი უფლებებია';ru='Недостаточно прав для выполнения операции';en='Insufficient permissions to perform the operation'");
		КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ПолноеИмяРеквизита = "Список.Дата", ИмяЭлемента = "Дата") Экспорт
	...
	ЭлементОформления.Представление = НСтр("ru = 'Представление даты: ""10.06.2012""'; ka = 'თარიღის წარმოდგენა: ""10.06.2012""'; en = 'Submission date: ""10.06.2012""'");
	...
	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Представление = НСтр("ru = 'Представление даты сегодня: ""09:46""'; ka = 'დღევანდელი თარიღის წარმოდგენა: ""09:46""'; en = 'Submission date today: ""9:46""'");
...
Процедура ПроверитьСоставПланаОбмена(Знач ИмяПланаОбмена) Экспорт
	...
	Если ВключитьВСостав.Количество() <> 0
		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			НСтр("ru = 'В состав плана обмена %1 должны входить следующие объекты метаданных: %2'; ka = 'გაცვლის გეგმა %1-ის შემადგენლობაში უნდა შედიოდეს მეტამონაცემთა შემდეგი ობიექტები: %2'; en = 'The structure of the exchange %1 of the plan should include the following metadata objects: %2'"),
			ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			НСтр("ru = 'В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2'; ka = 'გაცვლის გეგმის %1 შემადგენლობაში არ უნდა შედიოდეს მეტამონაცემთა შემდეგი ობიექტები: %2'; en = 'The structure of the exchange %1 should not be metadata objects must include a plan: %2'"),
			ИмяПланаОбмена,
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			НСтр("ru = 'В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации.""Требуется запретить авторегистрацию для следующих объектов метаданных: %2'; ka = 'გაცვლის გეგმა %1-ის შემადგენლობაში ავტორეგისტრაციის დაყენებული ნიშან-თვისების ობიექტები არ უნდა იყოს. საჭიროა ავტორეგისტრაციის აკრძალვა მეტამონაცემთა შემდეგი ობიექტებისთვის: %2'; en = 'The exchange plan %1 should not contain objects with the auto-registration flag set."" It is required to disable auto-registration for the following metadata objects: %2'"),
			ИмяПланаОбмена,
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ПараметрыКлиента.Количество() = 0
		...
		ВызватьИсключение НСтр("ru = 'Не заполнены параметры клиента на сервере.'; ka = 'სერვერზე კლიენტის პარამეტრები არ არის შევსებული.'; en = 'Not filled client settings on the server.'");
	КонецЕсли;
	...
	Если ИменаПараметров <> "" Тогда
		...
		Если ТребуетсяОбновление Тогда
			...
			Если ТекущийРежимЗапуска() = Неопределено Тогда
				ВызватьИсключение
				НСтр("ka='პროგრამაში შესვლა დროებით შეუძლებელია ახალი ვერსიით განახლების გამო.';ru='Вход в программу временно невозможен в связи с обновлением на новую версию.';en='Entrance to the program is currently not possible due to the upgrade to the new version.'");
			Иначе
				ВызватьИсключение
				НСтр("ru = 'Недопустимое обращение к необновленным параметрам работы программы""(например, к некоторым параметрам сеанса).""""Необходимо перенести вызов прикладного кода""после обновления параметров работы программы.'; ka = 'არასწორი წვდომა პროგრამის არაგანახლებულ ოპერაციულ პარამეტრებზე"" (მაგალითად, სესიის ზოგიერთ პარამეტრზე)."" ""აუცილებელია აპლიკაციის კოდის ზარის გადატანა"" პროგრამის მუშაობის პარამეტრების განახლების შემდეგ.'; en = 'Invalid access to non-updated program operation parameters"" (for example, to some session parameters)."" ""It is necessary to transfer the application code call"" after updating the program operation parameters.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
					...
					Если НЕ МонопольныйРежим() Тогда
						...
						Исключение
							Если ОшибкаУстановкиМонопольногоРежима <> Неопределено Тогда
								...
								НСтр("ru = 'Невозможно выполнить обновление информационной базы:""- Невозможно установить монопольный режим""- Версия конфигурации не предусматривает обновление без установки монопольного режима.'; ka = 'შეუძლებელია ინფო ბაზის განახლება: ""- ექსკლუზიური რეჟიმის დაყენება შეუძლებელია"" - კონფიგურაციის ვერსია არ ითვალისწინებს განახლებას ექსკლუზიური რეჟიმის დაყენების გარეშე.'; en = 'It is impossible to update the infobase: ""- Unable to set exclusive mode"" - The configuration version does not provide for updating without setting exclusive mode.'");
							КонецЕсли;
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ИмяСобытия = НСтр("ru = 'Идентификаторы объектов метаданных.Требуется загрузить критичные изменения'; ka = 'მეტამონაცემების ობიექტების იდენტიფიკატორები. შეწყვეტილი ცვლილებები უნდა ჩაიტვირთოს'; en = 'Object Identifiers metadannyh.Trebuetsya download critical changes'",
				...
				ТекстОшибки =
				НСтр("ru = 'Информационная база не может быть обновлена из-за проблемы в главном узле:""- главный узел был некорректно обновлен (возможно не был увеличен номер версии конфигурации,""  из-за чего не заполнился справочник Идентификаторы объектов метаданных);""- либо были отменены к выгрузке приоритетные данные (элементы""  справочника Идентификаторы объектов метаданных).""""Необходимо заново выполнить обновление главного узла, зарегистрировать к выгрузке""приоритетные данные и повторить синхронизацию данных:""- в главном узле запустите программу с параметром /C ЗапуститьОбновлениеИнформационнойБазы;""%1'; ka = 'საინფორმაციო ბაზა არ შეიძლება განახლდეს მთავარ კვანძში არსებული პრობლემის გამო:""- მთავარი კვანძი არაკორექტულად იქნა განახლებული (შეიძლება, რომ არ გაზრდილა კონფიგურაციის ვერსიის ნომერი, რის გამოც, არ მოხდა ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ს შევსება);""- ან პრიორიტეტულ მონაცემთა (ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორები""-ს ელემენტები) გადმოსატვირთად გაუქმება მოხდა.""""აუცილებელია მთავარი კვანძის განახლების განმეორებით შესრულება, პრიორიტეტულ მონაცემთა გადმოსატვირთად რეგისტრირება და მონაცემთა სინქრონიზაციის გამეორება:""- მთავარ კვანძში გაუშვით პროგრამა პარამეტრით /საინფორმაციობაზისგანახლებისგაშვება;""%1'; en = 'The infobase cannot be updated due to a problem in the main node: ""- the main node was updated incorrectly (perhaps the configuration version number was not increased,"" which is why the Metadata Object Identifiers directory was not filled);"" - or were canceled for uploading priority data (elements of the directory Metadata Object Identifiers)."" ""It is necessary to re-update the main node, register priority data for uploading and repeat data synchronization:"" - in the main node, run the program with the parameter /C RunInformationBaseUpdate;""%1'");
				
				Если ВыполнитьЗагрузку Тогда
					// Обновление подчиненного узла РИБ.
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					НСтр("ru = '- затем повторите синхронизацию данных с этой информационной базой"" (сначала в главном узле, затем в этой информационной базе после перезапуска).'; ka = ' - შემდეგ გაიმეორეთ მონაცემთა სინქრონიზაცია ამ საინფორმაცი ბაზასთან (თავდაპირველად, მთავარ კვანძში, შემდეგ, ამ საინფორმაციო ბაზაში, ხელახლა გაშვების შემდეგ)'; en = '- then repeat the data synchronization with this infobase"" (first in the main node, then in this infobase after a restart).'"));
				Иначе
					// Настройка подчиненного узла РИБ при первом запуске.
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					НСтр("ka=' - შემდეგ გაიმეორეთ დაქვემდებარებული კვანძის შექმნა.';ru='- затем повторите создание подчиненного узла.';en='- and then try to create the slave node.'"));
				КонецЕсли;
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			ВызватьИсключение НСтр("ka='გაყოფილ რეჟიმში განუყოფელ მონაცემთა ცვლილებების რეგისტრაცია.';ru='Регистрация изменений неразделенных данных в разделенном режиме.';en='Registration of changes in the undivided data sharing mode.'");
		КонецЕсли;
		...
		Если Не ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных("ПланОбмена." + ИмяПланаОбмена,
			...
			ВызватьИсключение НСтр("ka='გაცვლის განუყოფელი გეგმებისთვის ცვლილებების რეგისტრაციას მხარდაჭერა არ გააჩნია.';ru='Регистрация изменений для неразделенных планов обмена не поддерживается.';en='Registration of changes for undivided sharing plans are not supported.'");
		КонецЕсли;
		
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Объект.Метаданные().ПолноеИмя(),
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
			ВызватьИсключение НСтр("ru = 'Регистрация изменений для разделенных объектов не поддерживается.'; ka = 'გაყოფილი ობიექტებისთვის ცვლილებების ჩაწერა არ არის მხარდაჭერილი.'; en = 'Registration of changes for shared objects is not supported.'");
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	НСтр("ka='პროგრამის მუშაობის შესაძლებლობისთვის.';ru='Для возможности работы программы.';en='To be able to work the program.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное        = Истина;
	Дело.Представление = НСтр("ka='მუშაობის სიჩქარე შემცირებულია';ru='Скорость работы снижена';en='The speed of the reduced'");
	Дело.Форма         = "Обработка.РекомендацияПоПовышениюСкоростиРаботы.Форма.Форма";
	Дело.Владелец      = НСтр("ka='პროგრამის მუშაობის სიჩქარე';ru='Скорость работы программы';en='The speed of the program'");
...
Процедура ОчиститьКаталогВременныхФайлов(ПутьККаталогу) Экспорт
	...
	Исключение
		...
		НСтр("ka='სტანდარტული ქვესისტემები';ru='Стандартные подсистемы';en='standard subsystem'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დროებითი ფაილების კატალოგის გასუფთავების შეცდომა ""%1"":%2';ru='Ошибка очистки каталога временных файлов ""%1"":%2';en='cleaning temporary files directory error ""%1"":% 2'"),
		ПутьККаталогу,
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	+ НСтр("ru = 'Для разработчика: возможно требуется обновить вспомогательные данные,""которые влияют на работу программы. Для выполнения обновления можно:""- воспользоваться внешней обработкой""  ""Инструменты разработчика: Обновление вспомогательных данных"",""- либо запустить программу с параметром командной строки 1С:Предприятия 8""  ""/С ЗапуститьОбновлениеИнформационнойБазы"",""- либо увеличить номер версии конфигурации, чтобы при очередном запуске""  выполнились процедуры обновления данных информационной базы.'; ka = 'შემმუშავებლისთვის: შეიძლება, საჭირო არის დამხმარე მონაცემების განახლება, რომლებიც გავლენას ახდენენ პროგრამის მუშაობაზე. განახლების შესასრულებლად შეიძლება:""- გამოვიყენოთ გარე დამუშავება ""შემმუშავებლის ინსტრუმენტები: დამხმარე მონაცემების განახლება"",""- ან გავუშვათ პროგრამა 1С:საწარმოს საბრძანებო სტრიქონის პარამეტრით ""/საინფორმაციობაზისგანახლებისგაშვება"",""- ან გავზარდოთ კონფიგურაციის ვერსიის ნომერი იმისათვის, რომ მომდევნო გაშვებისას შესრულდეს საინფორმაციო ბაზის მონაცემთა განახლების პროცედურები.'; en = 'For the developer: it may be necessary to update auxiliary data,"" which affects the operation of the program. To perform the update, you can: ""- use external processing"" ""Developer Tools: Update auxiliary data"",""- or run the program with the 1C:Enterprise 8 command line parameter"" ""/C StartInfoBaseUpdate"","" - or increase the configuration version number so that the next time you start"" the procedures for updating infobase data will be performed.'");
...
Функция ДобавитьЭлементУсловногоОформления(Форма, Инструкция) Экспорт
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если РежимОтладки И Форма.Элементы.Найти(ИмяЭлемента) = Неопределено Тогда
			Шаблон = НСтр("ka='პირობითი გაფორმების დამატების შეცდომა: ელემენტი ""%1"" ვერ მოიძებნა.';ru='Ошибка добавления условного оформления: Не найден элемент ""%1"".';en='Error adding a conditional appearance: Could not find item ""%1"".'");
...
Функция ОбработчикиСобытий() Экспорт
	...
	Если ОбязательныеСобытияБезОбработчиков.Количество() > 0 Тогда
		ИмяСобытия  = НСтр("ka='მოვლენების დამმუშავაბელი';ru='Обработчики событий';en='event handlers'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
		Комментарий = НСтр("ka='შემდეგი სავალდებულო მოვლენებისთვის დაუდგენელია დამამუშავებლები:';ru='Для следующих обязательных событий не определены обработчики:';en='For the following mandatory event handlers are not defined:'")
		...
		ВызватьИсключение НСтр("ru = 'Для обязательных событий не определены обработчики.
                                |Подробности см. в журнале регистрации.'; ka = 'სავალდებულო მოვლენებისთვის დამამუშავებლები არ არის განსაზღვრული. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.'; en = 'There are no handlers defined for mandatory events.
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
		НСтр("ru = 'Недопустимо создавать предопределенный элемент помеченный на удаление.'; ka = 'მიუღებელია წინასწარ განსაზღვრული ელემენტის შექმნა, რომელიც მონიშნულია წაშლისთვის.'; en = 'It is unacceptable to create a predefined element is marked for deletion.'");
	Иначе
		...
		Если СтарыеСвойства.ИмяПредопределенныхДанных <> ""
			...
			НСтр("ru = 'Недопустимо помечать на удаление предопределенный элемент:""""%1"".'; ka = 'წინასწარ განსაზღვრული ელემენტი: """"%1"" ვერ მოინიშნება წასაშლელად.'; en = 'The predefined element: """"%1"" cannot be marked for deletion.'"),
		...
		ИначеЕсли СтарыеСвойства.ИмяПредопределенныхДанных = ""
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:""""%1"".'; ka = 'დაუშვებელია წასაშლელად მონიშნული ელემენტის დაკავშირება წინასწარ განსაზღვრული ელემენტის სახელთან: """"%1"".'; en = 'It is not allowed to associate an element marked for deletion with the name of a predefined item: """"%1"".'"),
			Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапретитьУдалениеПредопределенныхЭлементовПередУдалением(Источник, Отказ) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	НСтр("ru = 'Недопустимо удалять предопределенный элемент""""%1"".'; ka = 'მიუღებელია წინასწარ განსაზღვრული ელემენტის """"%1"" წაშლა.'; en = 'It is not acceptable to delete the predefined element """"%1"".'"),
	Строка(Источник.Ссылка));
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Вариант встроенного языка конфигурации ""%1"" не поддерживается.""Необходимо использовать вариант языка ""%2"".'; ka = 'კონფიგურაცია ""%1""-ის ჩაშენებული ენის ვარიანტს მხარდაჭერა არ გააჩნია.""აუცილებელია ენის ვარიანტ ""%2""-ის გამოყენება.'; en = 'The built-in configuration language option ''%1'' is not supported."" Language option ''%2'' must be used.""'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для запуска необходима версия платформы 1С:Предприятие %1 или выше.'; ka = 'გასაშვებად საჭიროა პლატფორმა 1С:საწარმოს ვერსია %1 ან უფრო მაღალი.'; en = 'To run the required version of the 1C: Enterprise %1 or higher.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НедоступныйРежим) Тогда
		...
		|версии не ниже 8.3.8 или ""Не использовать"".'; ka = 'კონფიგურაციის თავსებადობის რეჟიმი 1C:Enterprise ვერსია %1 მხარდაჭერილი არ არის.
		|დასაწყებად, დააყენეთ კონფიგურაცია თავსებადობის რეჟიმში 1C: Enterprise-თან
		|ვერსია არანაკლებ 8.3.8 ან ""არ გამოიყენო"".'; en = 'The configuration compatibility mode with 1C:Enterprise version %1 is not supported.
		|To start, set the configuration to compatibility mode with 1C:Enterprise
		|version no lower than 8.3.8 or ""Do not use"".'"),
		НедоступныйРежим);
	КонецЕсли;
	
	// Проверка заполнения версии конфигурации.
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено свойство конфигурации Версия.'; ka = 'კონფიგურაციის თვისება ""ვერსია"" არ არის შევსებული.'; en = 'Not filled configuration property version.'");
	Иначе
		Попытка
			НулеваяВерсия = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Метаданные.Версия, "0.0.0.0") = 0;
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".""Правильный формат, например: ""1.2.3.45"".'; ka = 'კონფიგურაცია ""ვერსია""-ს თვისება არასწორად არის შევსებული: ""%1"".""სწორი ფორმატია, მაგალითად: ""1.2.3.45"".'; en = 'The configuration property Version is not filled in correctly: ""%1"". Correct format, for example: ""1.2.3.45"".'"),
			Метаданные.Версия);
		КонецПопытки;
		Если НулеваяВерсия Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".""Версия не может быть нулевой.'; ka = 'კონფიგურაციის თვისება ვერსია არასწორად იყო შევსებული: ""%1."" ""ვერსია არ შეიძლება იყოს ნული.'; en = 'The configuration property Version was filled in incorrectly: ""%1."" ""Version cannot be zero.'"),
			Метаданные.Версия);
		КонецЕсли;
	КонецЕсли;
	...
	Если (Метаданные.ОсновныеРоли.Количество() <> 2 И Метаданные.ОсновныеРоли.Количество() <> 3)
		...
		ВызватьИсключение
		НСтр("ru = 'В конфигурации в свойстве ОсновныеРоли не указаны стандартные роли""АдминистраторСистемы и ПолныеПрава или указаны лишние роли.'; ka = 'კონფიგურაციაში თვისება ძირითადიროლები-ში სტანდარტული როლი სისტემისადმინისტრატორი და სრულიუფლებები არ არის მითითებული ან მითითებულია ზედმეტი როლები.'; en = 'In the configuration, the Main Roles property does not indicate the standard roles “System Administrator” and “Full Rights” or unnecessary roles are specified.'");
	КонецЕсли;
...
Процедура ПроверкаУникальностиИменСобытий(События)
	...
	Для каждого Событие Из События Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при подготовке списка событий.""""Событие ""%1""""уже добавлено.'; ka = 'მოხდა შეცდომა მოვლენათა სიის მომზადებაში.""""მოვლენა ""%1""""უკვე დამატებულია.'; en = 'There was an error preparing the event list.""""Event ""%1""""has already been added.'"),
			Событие);
		КонецЕсли;
		...
		Для каждого КлючИЗначение Из ОбработчикиСобытийПодсистемы Цикл
			...
			Для каждого ОписаниеОбработчика Из ОписанияОбработчиков Цикл
				...
				Если ТипЗнч(Обработчик.Модуль) <> Тип("Строка")
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка при подготовке обработчиков события""""%1"".""""Ошибка в имени модуля ""%2"".'; ka = 'მოხდა მოვლენის დამმუშავებლების მომზადების შეცდომა """"%1"".""შეცდომა მოდულის სახელში ""%2"".'; en = 'Error preparing event handlers """"%1"".""""Error in module name ""%2"".'"),
					Событие,
					Обработчик.Модуль);
				КонецЕсли;
				...
				Иначе
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка при подготовке обработчиков события""""%1"".""""Модуль ""%2"" уже добавлен.'; ka = 'მოხდა ""%1"" მოვლენისთვის მოვლენის დამმუშავებლების მომზადების შეცდომა."" ""მოდული ""%2"" უკვე დამატებულია.'; en = 'Error preparing event handlers for ""%1"" event."" ""Module ""%2"" has already been added.'"),
					Событие,
					Обработчик.Модуль);
				КонецЕсли;
				...
				Иначе
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка при подготовке обработчиков события""""%1"".""""Обработчик ""%2"" уже добавлен для события""""%3"".'; ka = '""%1"" მოვლენისთვის მოვლენის დამმუშავებლების მომზადებისას მოხდა შეცდომა. ""დამმუშავებელი ""%2"" უკვე დამატებულია მოვლენისთვის ""%3"".'; en = 'An error occurred while preparing event handlers for the event ""%1"". ""Handler ""%2"" has already been added for the event ""%3"".'"),
					Событие,
					ИмяОбработчика,
