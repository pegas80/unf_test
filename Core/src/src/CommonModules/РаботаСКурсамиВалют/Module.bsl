...
//
Функция СформироватьСуммуПрописью(СуммаЧислом, Валюта, ВыводитьСуммуБезКопеек = Ложь, Язык = Неопределено) Экспорт
	...
	//КонецЕсли;
	//
	//Результат = ЧислоПрописью(Сумма, Язык, ПараметрыПредмета.ПараметрыПрописиНаРусском);
	//
	...
	Сумма             = ?(СуммаЧислом < 0, -СуммаЧислом, СуммаЧислом);
	ПараметрыПредметаГруз = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Валюта, "ПараметрыПрописиНаГрузинском");
	ПараметрыПредметаРус = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Валюта, "ПараметрыПрописиНаРусском");
	ПараметрыПредметаАнгл = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Валюта, "ПараметрыПрописиНаАнглиском");
	...
	ПараметрыПредметаПрописи = Нстр("ka='"+ПараметрыПредметаГруз.ПараметрыПрописиНаГрузинском+"';ru='"+ПараметрыПредметаРус.ПараметрыПрописиНаРусском+"';en='"+ПараметрыПредметаАнгл.ПараметрыПрописиНаАнглиском+"'");
		
	Результат = ЧислоПрописью(Сумма,Язык,ПараметрыПредметаПрописи);
...
Процедура ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ka='დაწყებულია ვალუტების კურსის სარეგლამენტო ჩატვირთვა';ru='Начата регламентная загрузка курсов валют';en='Initiated routine loading currency'"));
	...
	Если ПриЗагрузкеВозниклиОшибки Тогда
		ЗаписьЖурналаРегистрации(
			...
			, 
			,
			НСтр("ka='ვალუტის კურსების ჩატვირთვის სარეგლამენტო დავალებისას წარმოიშვა შეცდომები';ru='Во время регламентного задания загрузки курсов валют возникли ошибки';en='During the scheduled download jobs currency any errors'"));
		ВызватьИсключение НСтр("ka='Загрузка курсов не выполнена. Подробности см. в журнале регистрации.';ru='Загрузка курсов не выполнена. Подробности см. в журнале регистрации.';en='Loading courses failed. For details, see. In the log.'");
	Иначе
		ЗаписьЖурналаРегистрации(
			ИмяСобытия,
			...
			,
			,
			НСтр("ka='დასრულებულია ვალუტის კურსების სარეგლამენტო ჩატვირთვა.';ru='Завершена регламентная загрузка курсов валют.';en='Complete routine loading rates.'"));
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='ვალუტის კურსები მოძველდა';ru='Курсы валют устарели';en='Exchange rates are outdated'");
		Дело.Важное         = Истина;
...
Функция Разрешения()
	...
	Порт = Неопределено;
	Описание = НСтр("ka='ვალუტების კურსის ჩატვირთვა ინტერნეტიდან.';ru='Загрузка курсов валют из Интернета.';en='Download currency exchange rates from the Internet.'");
	Разрешения.Добавить( 
...
Процедура ОбновитьКурсВалюты(Параметры, АдресРезультата) Экспорт
	...
	ИначеЕсли СпособУстановкиКурса = Перечисления.СпособыУстановкиКурсаВалюты.РасчетПоФормуле Тогда
		...
		Если ОсновныеВалюты.Количество() = 0 Тогда
			ТекстОшибки = НСтр("ka='ფორმულაში გამოყენებული უნდა იქნას თუნდაც ერთი ძირითადი ვალუტა.';ru='В формуле должна быть использована хотя бы одна основная валюта.';en='The formula should be used at least one base currency.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ka='Отключает ошибочно включенную загрузку курсов валюты ""Российский рубль (643)"" из интернета.';ru='Отключает ошибочно включенную загрузку курсов валюты ""Российский рубль (643)"" из интернета.';en='Disables mistakenly included downloading exchange rates ""Russian Ruble (643)"" from the Internet.'");
...
Процедура ОбновитьСведенияОВалюте937() Экспорт
	...
	Если Не Валюта.Пустая() Тогда
		...
		Валюта.НаименованиеПолное = НСтр("ka='ბოლივარი';ru='Боливар';en='Bolivar'");
...
"Курсы данной валюты не предоставляются.';ka='Невозможно получить файл данных с курсами валюты (%1 - %2):"
		...
		Иначе
			...
			Иначе
				ПоясняющееСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Невозможно получить файл данных с курсами валюты (%1 - %2):"
"%3"
"Возможно, нет доступа к веб сайту с курсами валют, либо указана несуществующая валюта.';ka='შეუძლებელია ვალუტის კურსებიანი მონაცემთა ფაილის მიღება (%1 - %2):"
"%3"
"შეიძლება, ვალუტის კურსებიან ვებ საიტთან არ არის დაშვება, ან მითითებულია არარსებული ვალუტა.'"),
					Валюта.КодВалюты,
					Валюта.Валюта,
...
Функция ЗагрузитьКурсВалютыИзФайла(Знач Валюта, Знач ПутьКФайлу, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки)
	...
	Если ЧислоЗагружаемыхДнейВсего <> ЧислоЗагруженныхДней Тогда
		Если ЧислоЗагруженныхДней = 0 Тогда
			ПояснениеОЗагрузке = НСтр("ka='ვალუტის კურსები %1 (%2) არ არის ჩატვირთული. მონაცემები არ არის.';ru='Курсы валюты %1 (%2) не загружены. Нет данных.';en='Currency Currency% 1 (% 2) is not loaded. There is no data.'");
		Иначе
			ПояснениеОЗагрузке = НСтр("ka='ვალუტა %1 (%2)-ის თაობაზე ყველა კურსი არ არის ჩატვირთული.';ru='Загружены не все курсы по валюте %1 (%2).';en='Loaded, not all courses in currency% 1 (% 2).'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ДатаЗапрета <> Неопределено Тогда
		ПояснениеОЗагрузке = ПояснениеОЗагрузке + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ვალუტის კურსების ჩატვირთვა %1(%2) შეზღუდულია ცვლილებების აკრძალვის თარიღით %3."
"აკრძალული პერიოდის კურსები ჩატვირთვისას გამოტოვებული იქნა.';ru='Загрузка курсов валюты %1(%2) ограничена датой запрета изменений %3."
...
Функция ИмяСобытияЖурналаРегистрации()
	Возврат НСтр("ka='ვალუტები. ვალუტების კურსის ჩატვირთვა';ru='Валюты.Загрузка курсов валют';en='Valyuty.Zagruzka currency'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

#КонецОбласти

#Область СлужебныеПроцедурыИФункцииГрузия
// Загружает курсы валют на текущую дату
//
Процедура ЗагрузитьАктуальныйКурсГрузия() Экспорт
	
	Если ПустаяСтрока(ИмяПользователя()) Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
	
	ИмяСобытия = НСтр("ka='ვალუტები. ვალუტების კურსის ჩატვირთვა';ru='Валюты.Загрузка курсов валют';en='Valyuty.Zagruzka currency'");
	...
	ОбработкаЗагрузкиКурсовВалют = Обработки.ЗагрузкаКурсовВалютГрузия.Создать();
	//ОбработкаЗагрузкиКурсовВалют.ЗагрузитьКурсыВалют();
	ОбработкаЗагрузкиКурсовВалют.ЗагрузитьКурсыВалютSSl();
