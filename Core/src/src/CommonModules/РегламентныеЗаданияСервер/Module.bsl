...
Процедура УдалитьЗадание(Знач Идентификатор) Экспорт
	...
	Иначе
		Если ТипЗнч(Идентификатор) = Тип("ОбъектМетаданных") И Идентификатор.Предопределенное Тогда
			ВызватьИсключение( НСтр("ka='წინასწარ განსაზღვრული სარეგლამენტო დავალების წაშლა შეუძლებელია.';ru='Предопределенное регламентное задание удалить невозможно.';en='Predefined scheduled jobs can not be deleted.'") );
...
"Возможно, оно удалено другим пользователем.';ka='სარეგლამენტო დავალება ვერ მოიძებნა. შეიძლება, იგი წაშლილია სხვა მომხმარებლის მიერ.'") );
	КонецЕсли;
