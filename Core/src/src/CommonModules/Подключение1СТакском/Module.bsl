...
Процедура ПриСозданииКонтекстаВзаимодействия(Контекст) Экспорт
	...
	Если МестоЗапуска = "taxcomGetID" Тогда
		Контекст.Вставить("СообщениеОНедоступностиДействия",
			НСтр("ka='ამ კონფიგურაციისთვის ტაქსკომის აბონენტის უნიკალური იდენტიფიკატორის მიღება მიუწვდომელია.';ru='Получение уникального идентификатора абонента Такском недоступно для этой конфигурации.';en='Obtaining a unique identifier Taxcom is not available for this user configuration.'"));
	ИначеЕсли МестоЗапуска = "taxcomPrivat" Тогда
		Контекст.Вставить("СообщениеОНедоступностиДействия",
			НСтр("ka='ტაქსკომის აბონენტის პირად კაბინეტთან მუშაობა ამ კონფიგურაციისთვის მიუწვდომელია.';ru='Работа с личным кабинетом абонента Такском недоступна для этой конфигурации.';en='Working with the private office user Taxcom available for this configuration.'"));
	КонецЕсли;
...
Процедура ПодготовитьСертификатЭПКОтправке(КСКонтекст, КонтекстОбработчика)
	...
	Если СертификатЭП = Неопределено Тогда
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ka='სერთიფიკატის ორობითი მონაცემების მიღება ვერ მოხერხდა. პარამეტრების სიაში სერთიფიკატი არ არის აღმოჩენილი.';ru='Не удалось получить двоичные данные сертификата. Сертификат не обнаружен в списке параметров.';en='Unable to get the binary data of the certificate. The certificate is not found in the parameter list.'");
	...
	Если ДвоичныеДанныеСертификата = Неопределено Тогда
		...
		СообщениеДляЖурналаРегистрации = НСтр("ka='შეცდომა სერთიფიკატის ორობითი მონაცემების მიღებისას. სერთიფიკატის ორობითი მონაცემები არ არის, ან სერთიფიკატი წაშლილია.';ru='Ошибка при получении двоичных данных сертификата. Двоичные данные сертификата отсутствуют, либо сертификат был удален.';en='Error obtaining the certificate of binary data. Binary data are missing certificate, or the certificate has been removed.'");
		...
		КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("ПрерватьБизнесПроцесс");
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ka='შეცდომა სერთიფიკატის მონაცემთა მიღებისას. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Ошибка при получении данных сертификата. Подробнее см. в журнале регистрации.';en='Error obtaining the certificate data. For details, see. In the log.'");
		КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение";
		Возврат;
...
Процедура ПодготовитьДанныеОбОрганизации(КСКонтекст, КонтекстОбработчика)
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки =
			НСтр("ka='ორგანიზაციის სარეგისტრაციო მონაცემების მიღება ვერ მოხერხდა.';ru='Не удалось получить регистрационные данные организации.';en='Unable to retrieve registration data.'")
...
"Подробнее см. в журнале регистрации';ka='შეცდომა ორგანიზაციის მონაცემთა მიღებისას. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში'");
		КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение";
		Возврат;
