...
Процедура ЗаписатьОбъект(Знач Объект, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, 
	...
	Если ДокументРежимЗаписи <> Неопределено Тогда
		Если ТипЗнч(ДокументРежимЗаписи) <> Тип("РежимЗаписиДокумента") Тогда
			ТекстИсключения = НСтр("ka='პარამეტრ დოკუმენტიჩაწერისრეჟიმი-ს არასწორი ტიპი';ru='Неправильный тип параметра ДокументРежимЗаписи';en='Wrong parameter type DokumentRezhimZapisi'");
...
Функция ОбъектОбработан(Данные) Экспорт
	...
	Если НезавершенныеОбработчики.Количество() > 0 Тогда
		ТекстИсключения = НСтр("ka='ობიექტთან მოქმედებები დროებით აკრძალულია, ვინაიდან პროგრამის ახალ ვერსიაზე გადასვლა არ დასრულებულა."
"მონაცემთა დამუშავების შემდეგი პროცედურები არ არის დასრულებული:';ru='Действия с объектом временно запрещены, так как не завершен переход на новую версию программы."
...
Процедура ОтметитьВыполнениеОбработки(Данные, ДополнительныеПараметры = Неопределено, Очередь = Неопределено) Экспорт
	...
	Если (ТипЗнч(Данные) = Тип("Массив")
		...
		ТекстИсключения = НСтр("ka='პროცედურა საინფორმაციობაზისგანახლება.დამუშავებისშესრულებისაღნიშვნა-ში გადაცემულია ცარიელი მასივი. დამუშავების შესრულების აღნიშვნა შეუძლებელია.';ru='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки передан пустой массив. Не возможно отметить выполнение обработки.';en='In ObnovlenieInformatsionnoyBazy.OtmetitVypolnenieObrabotki procedure passed an empty array. It is not possible to note the implementation of treatment.'");
		ВызватьИсключение ТекстИсключения;
	...
	Иначе
		Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
			ТекстИсключения = НСтр("ka='მთლიანად მეტამონაცემთა ობიექტის განახლების დამუშავების შესრულების აღნიშვნას მხარდაჭერა არ გააჩნია. საჭიროა კონკრეტულ მონაცემთა დამუშავების აღნიშვნა.';ru='Не поддерживается отметка выполнения обработки обновления целиком объекта метаданных. Нужно отмечать обработку конкретных данных.';en='Not Supported mark perform update processing is entirely metadata object. It is necessary to emphasize specific data processing.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Функция МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипДанных = Тип("ДанныеФормыСтруктура") Тогда
		...
		Иначе
			ТекстИсключения = НСтр("ka='პროცედურა საინფორმაციობაზისგანახლება.მეტამონაცემებიდამონაცემთამიხედვითშერჩევა არ შეიძლება, რომ ამ ფორმისთვის იქნას გამოყენებული.';ru='Процедура ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным не может быть использована для этой формы.';en='ObnovlenieInformatsionnoyBazy.MetadannyeIOtborPoDannym procedure can not be used for this form.'");
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = НСтр("ka='მეტამონაცემთა ამ ტიპისთვის ანალიზს მხარდაჭერა არ გააჩნია შემდეგ ფუნქციაში საინფორმაციობაზისგანახლება.მეტამონაცემებიდამონაცემთამიხედვითშერჩევა.';ru='Для этого типа метаданных не поддерживается анализ в функции ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.';en='For this type of metadata is not supported by analysis ObnovlenieInformatsionnoyBazy.MetadannyeIOtborPoDannym in function.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоДвижения Тогда
		...
		Если ДополнительныеПараметры.ОтметитьВсеРегистраторы Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ka='რეგისტრის ყველა რეგისტრატორის რეგისტრაციისთვის აუცილებელია პარამეტრ ""მონაცემები""-ში მეტამონაცემთაობიექტი:დოკუმენტი ან დოკუმენტიბმული-ს გადაცემა.';ru='Для регистрации всех регистраторов регистра необходимо в параметре ""Данные"" передать ОбъектМетаданных:Документ или ДокументСсылка.';en='To register, you must register all registrars in the parameter ""data"" to transfer ObektMetadannyh: Document or DokumentSsylka.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Иначе
			Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
				ТекстИсключения = НСтр("ka='მთლიანად, მეტამონაცემთა ობიექტის განახლებაზე რეგისტრაციას მხარდაჭერა არ გააჩნია. საჭიროა კონკრეტული მონაცემების განახლება.';ru='Не поддерживается регистрация к обновлению целиком объекта метаданных. Нужно обновлять конкретные данные.';en='Registration is not supported to upgrade the entire object metadata. Need to update specific data.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ka='გადაცემულია პარამეტრ ""სრულისახელიობიექტისმეტამონაცემები""-ს არასწორი ტიპი შემდეგ ფუნქციაში საინფორმაციობაზისგანახლება.დასამუშავებლადმონაცემებიარის';ru='Передан неправильный тип параметра ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';en='Passed the wrong type of parameter ''PolnoeImyaMetadannyeObekta ""in ObnovlenieInformatsionnoyBazy.EstDannyeDlyaObrabotki function'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	...
	Для каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл 
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеОбъекта)
			...
			Если ПолныеИменаОбрабатываемыхОбъектов.Количество() > 1 Тогда
				ТекстИсключения = НСтр("ka='სახელების მასივში პარამეტრ ""სრულისახელიობიექტისმეტამონაცემები""-ში ფუნქცია საინფორმაციობაზისგანახლება.დასამუშავებლადარისმონაცემები-ში გადაცემულია ინფორმაციის დამოუკიდებელი რეგისტრი.';ru='В массиве имен в параметре ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки передан независимый регистр сведений.';en='In the array name for ""PolnoeImyaMetadannyeObekta"" to the function register transmitted independently ObnovlenieInformatsionnoyBazy.EstDannyeDlyaObrabotki information.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;	
		...
		Иначе
			ТекстИсключения = НСтр("ka='მეტამონაცემთა ტიპ ""%МетаданныеОбъекта%""-ისთვის ფუნქცია საინფორმაციობაზისგანახლება.დასამუშავებლადმონაცემებიარის-ში შემოწმებას მხარდაჭერა არ გააჩნია';ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';en='For metadata such as ""% MetadannyeObekta%"" is not supported in check ObnovlenieInformatsionnoyBazy.EstDannyeDlyaObrabotki function'");
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ka='მეტამონაცემთა ამ ტიპისთვის შემოწმებას მხარდაჭერა არ გააჩნია შემდეგ ფუნქციაში საინფორმაციობაზისგანახლება.წასაკითხადდაშესაცვლელადდაბლოკილიმონაცემებისდროებითიცხრილისშექმნა.';ru='Для этого типа метаданных не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных.';en='To do this, the metadata type is not supported by checking ObnovlenieInformatsionnoyBazy.SozdatVremennuyuTablitsuZablokirovannyhDlyaChteniyaIIzmeneniyaDannyh in function.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок(Очередь, ПолныеИменаОбъектов, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе	
		...
		Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбъектовМассив Цикл
			...
			Иначе
				ТекстИсключения = НСтр("ka='მეტამონაცემთა ტიპ ""%МетаданныеОбъекта%""-ისთვის შემოწმებას მხარდაჭერა არ გააჩნია შემდეგ ფუნქციაში საინფორმაციობაზისგანახლება.წასაკითხადდაშესაცვლელადბლოკირებულიბმულებისდროებითიცხრილებისშექმნა';ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок';en='For metadata such as ""% MetadannyeObekta%"" is not supported in check ObnovlenieInformatsionnoyBazy.SozdatVremennuyuTablitsuZablokirovannyhDlyaChteniyaIIzmeneniyaSsylok function'");
				ТекстИсключения = СтрЗаменить(ТекстИсключения, "%МетаданныеОбъекта%", Строка(МетаданныеОбъекта)); 
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Если ДополнительныеИсточникиСсылки.Количество() > 0 Тогда
			...
			Если ПолноеИмяОбъекта = Неопределено Тогда
				ТекстИсключения = НСтр("ka='ფუნქცია %ИмяФункции%-ის მოხმობის შეცდომა: დოკუმენტის სახელი არ არის გადაცემული, მაგრამ გადაცემულია მონაცემთა დამატებითი წყაროები.';ru='Ошибка вызова функции %ИмяФункции%: не передано имя документа, но переданы дополнительные источники данных.';en='Error calling functions FunctionName%%: not transferred to the document name, but transferred to additional data sources.'");
		...
		Если ДополнительныеИсточникиРегистры.Количество() > 0 Тогда
			...
			Для Каждого ИсточникДанных Из ДополнительныеИсточникиРегистры Цикл
				...
				Если ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеИсточника)
					...
					ТекстИсключения = НСтр("ru='Регистр %ИсточникДанных% независимый. Поддерживается проверка только по регистрам, подчиненным регистраторам.';en='%% IstochnikDannyh register independent. Supported by checking only on registers, slave recorders.'");
