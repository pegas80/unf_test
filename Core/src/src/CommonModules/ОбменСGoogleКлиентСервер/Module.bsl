...
Функция ОписанияОбластейДоступаКалендарь() Экспорт
	...
	Календарь.Представление = НСтр("ka='კალენდარი';ru='Календарь';en='The calendar'");
...
Функция ОписанияОбластейДоступаКонтакты() Экспорт
	...
	Контакты.Представление = НСтр("ka='კონტაქტები';ru='Контакты';en='Contacts'");
...
Функция ОписанияОбластейДоступаПочта() Экспорт
	...
	Почта.Представление = НСтр("ka='ფოსტა';ru='Почта';en='post office'");
	...
	Профиль.Представление = НСтр("ka='პროფილი';ru='Профиль';en='Profile'");
...
Функция ОписанияОбластейДоступаТаблица() Экспорт //Малышев 2025-02-17
	
	Результат = Новый Массив;
	
	Таблицы = ОбменСGoogleКлиентСервер.НовоеОписаниеОбластиДоступа();
	Таблицы.Представление = НСтр("ka='ფურცელი';ru='Таблица';en='The sheet'");
	Таблицы.ОбластьДоступа = ОбменСGoogleКлиентСервер.ОбластьДоступа(ПредопределенноеЗначение("Перечисление.ОбластиДоступаGoogle.Таблицы"));
...
Функция ОписанияОбластейДоступаДиск() Экспорт //Малышев 2025-02-17
	
	Результат = Новый Массив;
	
	Диск = ОбменСGoogleКлиентСервер.НовоеОписаниеОбластиДоступа();
	Диск.Представление = НСтр("ka=წამყვანი';ru='Диск';en='The drive'");
	Диск.ОбластьДоступа = ОбменСGoogleКлиентСервер.ОбластьДоступа(ПредопределенноеЗначение("Перечисление.ОбластиДоступаGoogle.Диск"));
	Результат.Добавить(Диск);
...
Функция ОбластьДоступа(Вид) Экспорт
	...
	//Малышев 2025-02-14 GOOGLE SHEETS
	Если Вид = ПредопределенноеЗначение("Перечисление.ОбластиДоступаGoogle.Таблицы") Тогда
		Возврат "https://www.googleapis.com/auth/spreadsheets";
	КонецЕсли;
	
	Если Вид = ПредопределенноеЗначение("Перечисление.ОбластиДоступаGoogle.Диск") Тогда
		Возврат "https://www.googleapis.com/auth/drive.file";
	КонецЕсли;
