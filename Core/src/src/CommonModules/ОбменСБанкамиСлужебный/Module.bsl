...
Процедура ПодключитьВнешнююОбработку(Знач АдресФайла, ИмяОбъекта) Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ka='გარე დამუშავების შეერთება';ru='Подключение внешней обработки';en='Connecting external processing'");
...
Функция ПараметрыВК(ПутьКФайлуИнформации) Экспорт
	...
	Исключение
		Операция = НСтр("ka='გარე კომპონენტის შესახებ ინფორმაციის ფაილის წაკითხვა.';ru='Чтение файла информации о внешней компоненте.';en='Reading the file information on the external component.'");
		ТекстСообщения = НСтр("ka='გარე კომპონენტის შესახებ ინფორმაციის წაკითხვისას შეცდომა მოხდა.';ru='При чтении информации о внешней компоненте произошла ошибка.';en='When reading the information on the external component of the error occurred.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ИнформацияОВКБанка(АдресВнешнейКомпоненты) Экспорт
	...
	Исключение
		Операция = НСтр("ka='გარე მოდულის ფაილის წაკითხვა.';ru='Чтение файла внешнего модуля.';en='external module to read the file.'");
		ТекстСообщения = НСтр("ka='გარე მოდულის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла внешнего модуля произошла ошибка.';en='When reading a file external module error occurred.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	...
	Если Не НайденаИнформация Тогда
		Операция = НСтр("ka='გარე მოდულის ფაილის შესახებ ინფორმაციის წაკითხვა';ru='Чтение информации о файле внешнего модуля';en='Reading of an external module file information'");
		ТекстОшибки = НСтр("ka='გარე კომპონენტის არქივში ფაილი INFO.XML არ არის';ru='В архиве внешней компоненты отсутствует файл INFO.XML';en='The archive file is missing external components INFO.XML'");
		ТекстСообщения = НСтр("ka='აღნიშნული ფაილი გარე მოდულს არ წარმოადგენს';ru='Указанный файл не является внешним модулем';en='The specified file is not an external module'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	...
	Исключение
		Операция = НСтр("ka='გარე მოდულის ფაილის განფუთვა.';ru='Распаковка файла внешнего модуля.';en='external module file unpacking.'");
		ТекстСообщения = НСтр("ka='При распаковки файла внешнего модуля произошла ошибка.';ru='При распаковки файла внешнего модуля произошла ошибка.';en='When you unzip the file, an error occurred external module.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ВремКаталог);
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ВремФайл);
...
"Внешний идентификатор: %2';ka='В информационной базе уже есть электронный документ с таким же внешним идентификатором."
"Электронный документ: %1"
"Внешний идентификатор: %2'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Ссылка, ПараметрыСообщения.ВнешнийИдентификатор);
			ВидОперации = НСтр("ka='Сохранение сообщения обмена в информационной базе.';ru='Сохранение сообщения обмена в информационной базе.';en='Saving sharing posts in the information database.'");
...
Процедура ОтправитьВБанк(Настройки, Данные, Результат, ТекстОшибки) Экспорт
	...
	Иначе
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			Шаблон = НСтр("ka='ბანკში დოკუმენტის გაგზავნისას შეცდომა მოხდა (%1). %2';ru='При отправке документа в банк произошла ошибка (%1). %2';en='When sending a document to the bank there was an error (% 1). 2%'");
...
Процедура ОтправитьЗапросВыпискиВБанк(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ВидОперации = НСтр("ka='ამონაწერის მოთხოვნის გაგზავნა ბანკში';ru='Отправка запроса выписки в банк';en='Sending a request to the bank statement'");
...
"Причина: %1';ka='ამონაწერის მოთხოვნის ბანკში გაგზავნა ვერ მოხერხდა"
"მიზეზი: %1'");
...
Процедура РаспаковатьПакетОбменСБанками(ПакетОбменСБанками, ДанныеВозврата) Экспорт
	...
	Исключение
		Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
			ШаблонСообщения = НСтр("ka='პაკეტ %1-ის გარჩევისას შეცდომა წარმოიშვა."
"%2';ru='Возникла ошибка при разборе пакета %1."
"%2'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтрШаблон(
				ШаблонСообщения, ПакетОбменСБанками, КраткоеПредставлениеОшибки);
		Иначе
			ТекстСообщения = НСтр("ka='ბანკიდან მიღებული ფაილის წაკითხვა შეუძლებელია, ვინაიდან იგი შეცდომას შეიცავს';ru='Файл, полученный из банка невозможно прочитать так как он содержит ошибку';en='File received from the bank can not be read as it contains an error'");
		КонецЕсли;
		ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
"Подробное представление ошибки:"
"%2"
"Файл пакета: %3';ka='შეცდომა პაკეტ %1-ის განფუთვისას"
"შეცდომის დაწვრილებითი წარმოდგენა:"
"%2"
"პაკეტის ფაილი: %3'");
		...
		ВидОперации = НСтр("ka='Чтение ЭД.';ru='Чтение ЭД.';en='ED reading.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Подробное представление ошибки:"
"%2"
"Файл с ошибкой: %3';ka='შეცდომა პაკეტ %1-ის განფუთვისას"
"შეცდომის დაწვრილებითი აღწერილობა:"
"%2"
"შეცდომიანი ფაილი: %3'");
					ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ПодробнаяИнформация = СтрШаблон(
						ПодробнаяИнформация, ПакетОбменСБанками, ПодробноеПредставлениеОшибки, ФайлАрхива);
					Операция = НСтр("ka='პაკეტის განფუთვა';ru='Распаковка пакета';en='Unpacking package'");
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			...
			Если Document.dockind = "02" Тогда // Извещение о состоянии электронного документа
				...
				Попытка
					...
					Если Фабрика = Неопределено Тогда
						ВызватьИсключение НСтр("ru='Неизвестная версия формата файла.';en='Unknown version of the file format.'");
					КонецЕсли;
...
"პაკეტის ფაილი: %3'");
				...
				Если ЗначениеЗаполнено(СообщениеЭДО) Тогда
					...
					НазваниеДокумента = НСтр("ka='უწყება ელექტრონული დოკუმენტის მდგომარეობის შესახებ';ru='Извещение о состоянии электронного документа';en='Notification of an electronic document status'");
						...
						Иначе
							...
							Если StatusDocNotice.Result.Error.Code = "2207" Тогда
								...
								РеквизитыДляИзменения.Вставить(
									"ПричинаОтклонения", НСтр("ka='Документ отсутствует в информационной базе банка.';ru='Документ отсутствует в информационной базе банка.';en=""The document does not exist in the bank's information base."""));
					...
					КонецЕсли;
				Иначе
					ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის მდგომარეობის შესახებ უწყების წაკითხვა';ru='Чтение извещения о состоянии электронного документа';en='Reading the notice of the electronic document status'");
					ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
"Не найден электронный документ по идентификатору: %2';ka='შეცდომა პაკეტ %1-ის განფუთვისას"
"ელექტრონული დოკუმენტი იდენტიფიკატორის მიხედვით ვერ მოიძებნა: %2'");
					ПодробнаяИнформация = СтрШаблон(
...
"შეცდომიანი ფაილი: %3'");
				...
				Если ЗначениеЗаполнено(ИдентификаторЗапроса) Тогда
					...
					Если НЕ ЗначениеЗаполнено(СообщениеЗапрос) Тогда
						ВидОперации = НСтр("ka='პაკეტის განფუთვა';ru='Распаковка пакета';en='Unpacking package'");
						ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
"Не найден  запрос выписки по идентификатору: %2';ka='შეცდომა პაკეტ %1-ის განფუთვისას"
"ამონაწერის მოთხოვნა ვერ მოიძებნა იდენტიფიკატორის მიხედვით: %2'");
						ПодробнаяИнформация = СтрШаблон(
				...
				Если ЗначениеЗаполнено(СообщениеЗапрос) Тогда
					...
					НазваниеДокумента = СтрЗаменить(
						НазваниеЗапроса, НСтр("ka='ამონაწერის მოთხოვნა';ru='Запрос выписки';en='Query statements'"), НСтр("ka='ბანკის ამონაწერი პერიოდის მანძილზე';ru='Выписка банка за период';en='A bank statement for the period'"));
				Иначе
					НазваниеДокумента = НСтр("ka='%1-ის ბანკის ამონაწერი';ru='Выписка банка от %1';en='A bank statement from the 1%'");
			...
			ИначеЕсли Document.dockind = "20" Тогда // Подтверждение открытия счетов по зарплатному проекту
				...
				НазваниеДокумента = НСтр("ka='სახელფასო პროექტის მიხედვით ანგარიშების გახსნის დადასტურება';ru='Подтверждение открытия счетов по зарплатному проекту';en='Confirmation of the opening of payroll project accounts'");
			...
			ИначеЕсли Document.dockind = "22" Тогда // Подтверждение зачисления денежных средств на счета сотрудников
				...
				НазваниеДокумента = НСтр("ka='თანამშრომლების ანგარიშებზე ფულადი სახსრების ჩარიცხვის დადასტურება';ru='Подтверждение зачисления денежных средств на счета сотрудников';en='Confirmation of funds credited to the accounts of employees'");
...
Процедура СохранитьПолученныйПакет(НастройкаОбмена, ИмяФайла, ДанныеВозврата) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='ბანკიდან მიღებულია არაკორექტული მონაცემები. პრობლემის გადასაჭრელად მიმართეთ თქვენს ბანკს.';ru='Получены некорректные данные из банка. Для решения проблемы обратитесь в свой банк.';en='Received incorrect data from the bank. To solve the problem, contact your bank.'");
		
		ШаблонОшибкиДляЖурнала = НСтр("ka='ბანკიდან მიღებულია არაკორექტული შემცველობის მქონე ფაილი."
"გზა ფაილისკენ: %1"
"შეცდომის ტექსტი: %2.';ru='Из банка получен файл с некорректным содержимым."
"Путь к файлу: %1"
"Текст ошибки: %2.'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(ШаблонОшибкиДляЖурнала, ИмяФайла, ПодробноеПредставлениеОшибки);
		
		ВидОперации = НСтр("ka='ბანკიდან მიღებული ელექტრონული დოკუმენტის წაკითხვა.';ru='Чтение полученного из банка электронного документа.';en=""The reading obtained from the bank's electronic document.""");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ГотовыеКРаспаковкеПакетыЭД(Знач ПакетыЭД) Экспорт
	...
	Пока ВыборкаПакеты.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(ВыборкаПакеты.Ссылка) Тогда
			Операция = НСтр("ka='პაკეტის განფუთვა';ru='Распаковка пакета';en='Unpacking package'");
			ТекстСообщения = НСтр("ka='მიერთებული ფაილი არ არის პაკეტისთვის: %1';ru='Нет присоединенного файла для пакета: %1';en='No attached file for the package: 1%'");
		...
		Если ВыборкаПакеты.Статус <> Перечисления.СтатусыПакетовЭД.КРаспаковке Тогда
			...
			ШаблонСообщения = НСтр("ka='პაკეტ %1-ის სტატუსი განსხვავდება მნიშვნელობა ""განსაფუთად""-ისგან.';ru='Статус пакета %1 отличен от значения ""К распаковке"".';en='Package status% 1 is different from the value of ""K unpacking.""'");
...
Процедура ПрочитатьИдентификаторыПакетов(НастройкаОбмена, ИмяФайла, МассивИдентификаторов, ЕстьОшибка) Экспорт
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='При чтении данных, полученных из банка, произошла ошибка.';ru='При чтении данных, полученных из банка, произошла ошибка.';en='When reading the data received from the bank, an error occurred.'");
		ТекстОшибки = НСтр("ru='При чтении данных, полученных из банка, произошла ошибка."
"Путь к файлу: %1"
"Подробное представление ошибки: %2';ka='При чтении данных, полученных из банка, произошла ошибка."
"Путь к файлу: %1"
"Подробное представление ошибки: %2'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(ТекстОшибки, ИмяФайла, ПодробноеПредставлениеОшибки);
		ВидОперации = НСтр("ka='ახალი დოკუმენტების სიის წაკითხვა';ru='Чтение списка новых документов';en='Reading the list of new documents'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
		ОбъектXML.Закрыть();
	...
	Если НЕ ResultBank.Success = Неопределено Тогда
		...
		Иначе
			Операция = НСтр("ka='ბანკის პასუხის წაკითხვა.';ru='Чтение ответа банка.';en='Reading bank response.'");
			ТекстОшибки = НСтр("ru='Получен некорректный ответ из банка"
"Файл ответа: %1';ka='ბანკიდან მიღებულია არაკორექტული პასუხი"
"პასუხის ფაილი: %1'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, ИмяФайла);
			ТекстСообщения = НСтр("ka='ბანკიდან მიღებულია არაკორექტული მონაცემები';ru='Получены некорректные данные из банка';en='Received incorrect data from the bank'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%1';ka='დოკუმენტების მიღების თარიღის ჩაწერისას შეცდომა მოხდა:."
"%1'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки);
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='დოკუმენტების მიღების უკანასკნელი თარიღის შენახვა';ru='Сохранение последней даты получения документов';en='Saving the last date of receipt of the documents'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура СоздатьПакетОбменСБанками(НастройкаОбмена, МассивСообщений, ПакетОбменСБанками) Экспорт
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='ედბ-ის პაკეტის შედგენა';ru='формирование пакета ЭДО';en='EDI package formation'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура СоздатьЭДПакетаAsync(Конверт, ЕстьОшибка, СвойстваСертификатов = Неопределено) Экспорт
			...
			Для Каждого СтрокаПодписи Из МассивПодписей Цикл
				...
				Иначе
					...
					Если СвойстваСертификата = Неопределено Тогда
						ВызватьИсключение НСтр("ka='სერთიფიკატის თვისებების წაკითხვის შეცდომა';ru='Ошибка чтения свойств сертификата';en='Error reading certificate properties'");
					КонецЕсли;
	...
	Исключение
		...
		ВидОперации = НСтр("ka='ედ-ის ჩამოყალიბება';ru='Формирование ЭД';en='formation of ED'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
...
"%3%4';ka='მიღებულია შეცდომა ბანკიდან (%1). შეცდომის კოდი %2."
"%3%4'");
...
Процедура ОбработатьОтветИзБанка(ОтветБанка, ОтправленноеСообщениеОбмена, ПолученноеСообщениеОбмена = Неопределено, ЕстьОшибка = Неопределено) Экспорт
	...
	Иначе
		...
		НаименованиеВыписки = СтрЗаменить(НаименованиеЗапроса, НСтр("ka='ამონაწერის მოთხოვნა';ru='Запрос выписки';en='Query statements'"), НСтр("ka='ბანკის ამონაწერი პერიოდის მანძილზე';ru='Выписка банка за период';en='A bank statement for the period'"));
...
"Необходимо проверить компьютер антивирусной программой.';ka='Файл изменен внешней программой."
"Необходимо проверить компьютер антивирусной программой.'");
		ПодробноеПредставлениеОшибки = ТекстСообщения + Символы.ПС + НСтр("ka='Путь к файлу: %1';ru='Путь к файлу: %1';en='The path to the file:% 1'");
		ПодробноеПредставлениеОшибки = СтрШаблон(ПодробноеПредставлениеОшибки, ИмяФайла);
		ВидОперации = НСтр("ka='Проверка неизменности данных временного файла.';ru='Проверка неизменности данных временного файла.';en='Check the immutability of the data of the temporary file.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СформироватьСообщенияОбмена(МассивОбъектов, ПараметрыОбмена) Экспорт
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если (СтруктураОбмена.СтруктураЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение
			...
			ПараметрыСообщения = Новый Структура;
			ПараметрыСообщения.Вставить("Представление", НСтр("ka='მონაცემების სქემა';ru='Схема данных';en='data scheme'"));
...
Процедура УстановитьСостояниеПриЗаписиОбъектаИБ(Источник, Отказ) Экспорт
	...
	Если НЕ Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ka='არსებობს აქტუალური ელექტრონული დოკუმენტი. დოკუმენტის საკვანძო რეკვიზიტების რედაქტირება აკრძალულია.';ru='Существует актуальный электронный документ. Запрещено редактирование ключевых реквизитов документа.';en='There is a current electronic document. It is forbidden to edit the key details of the document.'");
...
Функция НайтиСоздатьСертификатВнешнегоМодуля(ДанныеСертификата, Организация, ПрограммаБанка, Параметры) Экспорт
	...
	ШаблонНаименования = НСтр("ka='%1, %2-მდე';ru='%1, до %2';en='1% to 2%'");
...
Процедура ЗагрузитьСписокDirectBank(ЗапускИзРегламентногоЗадания = Истина) Экспорт
	...
	Если Результат.Статус Тогда
		...
		Исключение
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='სერვის 1С:დირექტბანკით გაცვლის მხარდამჭერი ბანკების სიის ფაილის წაკითხვა.';ru='Чтение файла списка банков, поддерживающих обмен через сервис 1С:ДиректБанк.';en='Reading the file list of banks that support the exchange through the service of 1C: DirektBank.'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ШаблонОшибки = НСтр("ru='Ошибка выполнения операции: %1"
"%2';ka='Ошибка выполнения операции: %1"
"%2'");
	...
	ИначеЕсли ЗначениеЗаполнено(Результат.СообщениеОбОшибке) Тогда
		...
		ВидОперации = НСтр("ka='ბანკების სიის ჩატვირთვა, რომელთა მიერ მხარდაჭერილია გაცვლა DirectBank-ი';ru='Загрузка списка банков, поддерживающих обмен DirectBank';en='Download list of banks that support the exchange of DirectBank'");
		ШаблонОшибки = НСтр("ka='ოპერაციის შესრულება: %1"
"%2';ru='Выполнение операции: %1"
"%2'");
		...
		Шаблон = НСтр("ka='ელექტრონული ურთიერთქმედება.ბანკებთან გაცვლა';ru='Электронное взаимодействие.Обмен с банками';en='Electronic vzaimodeystvie.Obmen with banks'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СоздатьНастройкуОбмена(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ka='გაცვლის გამართვის შექმნის შეცდომა: %1';ru='Ошибка создания настройки обмена: %1';en='Error creating exchange settings:% 1'");
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ka='Создание настройки обмена';ru='Создание настройки обмена';en='Creating a sharing settings'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СоздатьНастройкуОбменаИзФайла(НастройкиЗаполнения, Организация, Недействительна = Ложь, МассивСертификатов = Неопределено, ЗагружатьВК = Истина) Экспорт
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Организация) Тогда
		ТекстСообщения = НСтр("ka='გამართვების ფაილში ორგანიზაციის შესახებ ინფორმაცია არ არის. მიმართეთ თქვენს ბანკს.';ru='В файле настроек нет информации об организации."
"Обратитесь в свой банк.'");
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		ТекстСообщения = НСтр("ka='Не найдена организация по реквизитам: %1%2%3';ru='Не найдена организация по реквизитам: %1%2%3';en='Found Organization details:% 1% 2% 3'");
		Если СтруктураПоискаОрганизации.Свойство("ИНН") Тогда
			ТекстИНН = Символы.ПС + НСтр("ka='ИНН: %1';ru='ИНН: %1';en='INN:% 1'");
			ТекстИНН = СтрШаблон(ТекстИНН, СтруктураПоискаОрганизации.ИНН);
		Иначе
			ТекстИНН = "";
		КонецЕсли;
		Если СтруктураПоискаОрганизации.Свойство("КПП") Тогда
			ТекстКПП = Символы.ПС + НСтр("ka='КПП: %1';ru='КПП: %1';en='Transmission: 1%'");
			ТекстКПП = СтрШаблон(ТекстКПП, СтруктураПоискаОрганизации.КПП);
		Иначе
			ТекстКПП = "";
		КонецЕсли;
		Если СтруктураПоискаОрганизации.Свойство("Наименование") Тогда
			ТекстНаименование = Символы.ПС + НСтр("ka='დასახელება: %1';ru='Наименование: %1';en='Name:% 1'");
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = НСтр("ka='Не найден банк по реквизитам: %1%2';ru='Не найден банк по реквизитам: %1%2';en='Not found on the bank account:% 1% 2'");
		Если СтруктураПоискаБанка.Свойство("Код") Тогда
			ТекстБИК = Символы.ПС + НСтр("ka='БИК: %1';ru='БИК: %1';en='BIC:% 1'");
		...
		Если Результат.Статус Тогда
			...
			Если ПараметрыВК <> Неопределено Тогда
				...
				Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
					...
					Иначе
						ТекстСообщения = НСтр("ka='უცნობი გარე კომპონენტების გამოყენება აკრძალულია:"
"სახელწოდება: %1."
"მოდული: %2."
"ვერსია: %3.';ru='Запрещено использование неизвестных внешних компонент:"
"Наименование: %1."
"Модуль: %2."
"Версия: %3.'");
	...
	Если ЗаписыватьОбъект Тогда
		Попытка
			Объект.Записать();
		Исключение
			ТекстСообщения = НСтр("ka='Возникла ошибка при создании настройки обмена с сервисом 1С:ДиректБанк.';ru='Возникла ошибка при создании настройки обмена с сервисом 1С:ДиректБанк.';en='An error occurred when creating the service settings exchange with 1C: DirektBank.'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='Создание настройки обмена с сервисом 1С:ДиректБанк.';ru='Создание настройки обмена с сервисом 1С:ДиректБанк.';en='Creating a sharing settings service 1C: DirektBank.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Неопределено;
		КонецПопытки
...
Функция XDTOДанныеФайлаНастроек(АдресФайла) Экспорт
	...
	Попытка
		...
		ЭД.Проверить();
	Исключение
		ТекстСообщения = НСтр("ka='გამართვების წაკითხვისას შეცდომა წარმოიშვა.';ru='Возникла ошибка при чтении настроек.';en='An error occurred while reading the configuration.'");
		ВидОперации = НСтр("ru='Чтение настроек обмена с сервисом 1С:ДиректБанк из файла.';en='sharing settings, read with the service of 1C: DirektBank from the file.'");
...
Функция ОпределитьНастройкиОбменаЭДПоИсточнику(Источник, ВыводитьСообщения, МассивОтпечатковСертификатов, СообщениеОбмена = Неопределено, ВидЭД = Неопределено, ФлагДействующиеНастройкиОбмена = Истина) Экспорт
	...
	Если ВыводитьСообщения И ДействующиеПараметрыОбмена.НастройкаОбменаДействует
		...
		ТекстСообщения = НСтр("ka='Данный вид электронного документа банк не принимает.';ru='Данный вид электронного документа банк не принимает.';en='This type of electronic document Bank will not accept.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	Исключение
		...
		Попытка
			...
			Если ДанныеСертификата = Неопределено Тогда
				ВызватьИсключение НСтр("ka='Некорректное содержимое сертификата криптографии.';ru='Некорректное содержимое сертификата криптографии.';en='Incorrect certificate contents cryptography.'");
			КонецЕсли;
			ТекСертификат = Новый СертификатКриптографии(ДанныеСертификата);
		Исключение
			ТекстСообщения = НСтр("ka='ფაილიდან სერთიფიკატის მონაცემთა წაკითხვისას შეცდომა წარმოიშვა.';ru='Возникла ошибка при чтении данных сертификата из файла.';en='An error occurred while reading data from the certificate file.'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ka='Чтение настроек ЭДО из файла.';ru='Чтение настроек ЭДО из файла.';en='EDM settings Reading from a file.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Неопределено;
		КонецПопытки;
...
"Длина номера счета должна состоять из 20 цифр.';ka='Некорректный номер счета получателя %1"
"Длина номера счета должна состоять из 20 цифр.'");
	...
	Исключение
		...
		Операция = НСтр("ka='ედ-ის ჩამოყალიბება';ru='Формирование ЭД';en='formation of ED'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%2';ka='%1-ის ჩამოყალიბებისას შემდეგი შეცდომები წარმოიშვა: %2'");
...
Функция ПрочитатьФайлПоСхеме(ИмяФайла, ДеревоРазбора, НовыйЭД, ПоказыватьОшибки = Ложь)
	...
	Попытка
		...
		Если НеизвестныйЭД Тогда
			Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru='Неизвестный тип ЭД при чтении данных из файла %1.';en='Unknown type of ED when reading data from the file% 1.'"), ИмяФайла);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
			Ошибка = Истина;
		КонецЕсли;
		...
		Иначе
			...
			Если НайденнаяСтрока.ОписаниеОбъекта <> ОписаниеОбъекта Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка в данных эл.документа: на один Объект <%1>, два Описания <%2> и <%3>.';en='Data error el.dokumenta: one object <% 1>, two Description <% 2> and <3%>.'"), СсылкаНаОбъект,
					НайденнаяСтрока.ОписаниеОбъекта, ОписаниеОбъекта);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				НайденнаяСтрока.ОписаниеОбъекта = ОписаниеОбъекта;
			КонецЕсли;
...
Процедура ПрочитатьЗапросВыпискиAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	ТекстБанк = " " + НСтр("ka='ბანკში';ru='в банке';en='in the bank'") + " ";
	Если НЕ DataRequest.Bank.Name = Неопределено Тогда
		ТекстБанк = ТекстБанк + DataRequest.Bank.Name + ", ";
	КонецЕсли;
	ТекстБанк = ТекстБанк + НСтр("ka='ბანკის კოდი';ru='БИК:';en='BIC:'") + " " + DataRequest.Bank.BIC;
...
Функция СформироватьПлатежноеПоручениеCML(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	...
	Попытка 
		...
		Если СтрДлина(Номер) > 6 Тогда
			ВызватьИсключение НСтр("ru='Длина номера документа не должна превышать 6 знаков.';en='the document number can be up to 6 characters.'")
		КонецЕсли;
...
Функция СформироватьПлатежноеПоручениеAsync(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	...
	Исключение
		ТекстСообщения = ?(ЗначениеЗаполнено(ТекстОшибки), ТекстОшибки, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='Формирование электронного документа';en='Formation of an electronic document'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, СсылкаНаДокумент);
		ФайлСформирован = Ложь;
	КонецПопытки;
...
Функция ПредставлениеЭД(ВидЭД, СсылкаНаОбъект, НомерДокумента)
	...
	Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение
		...
		ШаблонПредставленияЭД = НСтр("ka='%1 № %2  %3 -დან';ru='%1 № %2 от %3';en='№% 1% 2% 3 from'");
	Иначе
		ШаблонПредставленияЭД = НСтр("ka='%1 %2 %3';ru='%1 %2 %3';en='% 1% 2% 3'");
	КонецЕсли;
...
Функция СформироватьНовыйПакетОбменСБанками(СтруктураПараметров)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Операция = НСтр("ka='გაცვლის პაკეტის შექმნა';ru='Создание пакета обмена';en='Creating a sharing package'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
