...
Функция ПояснениеНедействительногоБанка(Банк) Экспорт
	...
	Если ЗначениеЗаполнено(Банк) И ЗначениеЗаполнено(НовыеРеквизитыБанка) Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ბანკის ბსკ შეიცვალა <a href = ""%1"">%2</a>-ით';ru='БИК банка изменился на <a href = ""%1"">%2</a>';en='BIC changed to <a href = ""%1"">% 2 </a>'"),
			ПолучитьНавигационнуюСсылку(НовыеРеквизитыБанка.Ссылка), НовыеРеквизитыБанка.БИК);
	Иначе
		Результат = НСтр("ka='ბანკის საქმიანობა შეწყვეტილია';ru='Деятельность банка прекращена';en='Activity Bank terminated'");
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ka='ბანკების კლასიფიკატორი მოძველდა';ru='Классификатор банков устарел';en='banks Qualifier outdated'");
		Дело.Подсказка      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ბოლო განახლებები %1 ადრე';ru='Последнее обновление %1 назад';en='Last updated% 1 ago'"), Результат.ВеличинаПросрочкиСтрокой);
...
Функция Разрешения()
	...
	Порт = Неопределено;
	Описание = НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა ინტერნეტიდან.';ru='Загрузка классификатора банков из интернета.';en='Loading classifier banks from the Internet.'");
	Разрешения.Добавить( 
...
Процедура ДополнитьТекстСообщения(ПараметрыЗагрузкиКлассификатора)
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		Если Загружено = 0 И Обновлено = 0 Тогда
			ТекстСообщения = НСтр("ka='Загрузка завершена. Изменений в классификаторе нет.';ru='Загрузка завершена. Изменений в классификаторе нет.';en='Loading is complete. No changes in the classifier.'");
		Иначе
			ТекстСообщения = НСтр("ka='საქართველოს ბანკების კლასიფიკატორის ჩატვირთვა წარმატებით შესრულდა.';ru='Загрузка классификатора банков РФ выполнена успешно.';en='Loading classifier Russian banks is successful.'");
		КонецЕсли;
	КонецЕсли;
	
	Если Загружено > 0 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ჩატვირთულია ახალი: %1.';ru='Загружено новых: %1.';en='Uploaded New:% 1.'"), Загружено);
	КонецЕсли;
	
	Если Обновлено > 0 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='განახლებულია ჩანაწერი: %1.';ru='Обновлено записей: %1.';en='Updated records:% 1.'"), Обновлено);
	КонецЕсли;
...
Процедура ПолучитьДанныеРБК(ПараметрыЗагрузкиКлассификатора, АдресХранилища = "") Экспорт
	...
	Если ПараметрыПолученияФайловРБК["ЕстьОбновлениеНаСервере"] Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ka='Возникли проблемы с файлом классификатора банков, полученным из Интернета.';ru='Возникли проблемы с файлом классификатора банков, полученным из Интернета.';en='Having a problem with the file classifier banks received from the Internet.'");
...
"Архив не содержит информацию - классификатор банков.';ka='Возникли проблемы с файлом классификатора банков, полученным из Интернета. "
"Архив не содержит информацию - классификатор банков.'");
		...
		Если Не ФайлРегионовРБК.Существует() Тогда
			ТекстСообщения = НСтр("ru='Возникли проблемы с файлом классификатора банков, полученным из Интернета. "
"Архив не содержит информацию о регионах.';ka='Возникли проблемы с файлом классификатора банков, полученным из Интернета. "
"Архив не содержит информацию о регионах.'");
		...
		Если Не ФайлНедействующихБанков.Существует() Тогда
			ТекстСообщения = НСтр("ru='Возникли проблемы с файлом классификатора банков, полученным из Интернета. "
"Архив не содержит информацию о недействующих банках.';ka='Возникли проблемы с файлом классификатора банков, полученным из Интернета. "
...
Процедура ЗагрузитьКлассификаторБанков() Экспорт
	...
	Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияВЖурналеРегистрации(), УровеньСобытия, , , НСтр("ka='გსბ-ის დაქვემდებარებულ კვანძში ჩატვირთვა არ არის გათვალისწინებული';ru='Загрузка в подчиненном узле РИБ не предусмотрена';en='Loading subordinate node RIB is not provided'"));
		Возврат;
	КонецЕсли;
		...
		Иначе
			Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
				ДополнительноеСообщение = Символы.ПС
					+ НСтр("ka='შესაძლებელია ინტერნეტთან დაკავშირების უზუსტო ან არასწორი გამართვები.';ru='Возможно неточные или неправильные настройки подключения к Интернету.';en='Possibly inaccurate or incorrect Internet connection settings.'");
			Иначе
				ДополнительноеСообщение = Символы.ПС
					+ НСтр("ka='შესაძლებელია, რომ ადგილი ჰქონდეს 1С:საწარმოს სერვერზე ინტერნეტთან დაკავშირების უზუსტო ან არასწორ გამართვებს';ru='Возможно неточные или неправильные настройки подключения к Интернету на сервере 1С:Предприятие.';en='Possibly inaccurate or incorrect settings to connect to the Internet on a server 1C: Enterprise.'");
			КонецЕсли;
...
Функция ПолучитьСтруктуруПолейБанка(Знач Строка, Регионы)
	...
	Если Регион = Неопределено Тогда
		Регион = НСтр("ka='სხვა ტერიტორიები';ru='Другие территории';en='other territories'");
		КодРегиона = "";
	КонецЕсли;
...
Функция ФайлыКлассификатораПолучены(ПапкаСФайлами)
	...
	Для Каждого ИмяФайла Из ИменаФайловДляПроверки Цикл
		...
		Если Не Файл.Существует() Тогда
			ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ფაილი %1 ვერ მოიძებნა';ru='Не найден файл %1';en='File not found% 1'"), ИмяФайла));
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Если ИспользоватьАльтернативныйСервер Тогда
		Возврат НСтр("ka='ბანკების კლასიფიკატორის ჩატვირთვა.';ru='Загрузка классификатора банков.РБК';en='Loading classifier bankov.RBK'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	
	Возврат НСтр("ka='Загрузка классификатора банков.1С';ru='Загрузка классификатора банков.1С';en='Loading classifier bankov.1S'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
