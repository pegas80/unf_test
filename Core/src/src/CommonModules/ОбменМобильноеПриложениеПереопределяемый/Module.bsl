...
Функция ПолучитьОбъектXDTO(Данные, КоличествоОбъектов)
	...
	// Сериализация справочника Характеристики номенклатуры.
	ИначеЕсли ТипЗнч(Данные) = Тип("СправочникОбъект.ХарактеристикиНоменклатуры")
		...
		ПередаваемыйОбъект = СоздатьОбъектXDTO("Characteristicitems");
		ПередаваемыйОбъект.Id = Строка(Данные.Ссылка.УникальныйИдентификатор());
		ПередаваемыйОбъект.Name = Данные.Наименование;
		ПередаваемыйОбъект.DeletionMark = Данные.ПометкаУдаления;
		ПередаваемыйОбъект.Nomenclature = ПолучитьОбъектXDTO(Данные.Владелец, КоличествоОбъектов);
...
Процедура ДобавитьСообщениеВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди, СообщениеОбмена) Экспорт
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მობილურ კლიენტთან გაცვლა. შეტყობინების დამატება გაცვლის შეტყობინებების რიგში';ru='Обмен с мобильным клиентом.Добавление сообщения в очередь сообщений обмена';en='Exchange with mobile klientom.Dobavlenie messages in all message exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ВызватьИсключение(НСтр("ka='მონაცემთა გაგზავნის შესრულება ვერ მოხერხდა. დაწვრილებით, იხ. საინფორმაციო ბაზის რეგისტრაციის ჟურნალში.';ru='Не удалось выполнить отправку данных. Подробности см. в Журнале регистрации информационной базы.';en='Failed to send data. For details, see. In the Journal of registration information database.'"));
...
Процедура ПроверитьОчередьСообщенийОбмена(УзелОбмена, Знач НомерПринятого) Экспорт
	...
	Пока ВыборкаСообщенийОбмена.Следующий() Цикл
		...
		ИначеЕсли ВыборкаСообщенийОбмена.НомерСообщения > НомерСообщенияОчереди Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='მობილურ კლიენტთან გაცვლა. გაცვლის შეტყობინებების რიგის შემოწმება';ru='Обмен с мобильным клиентом.Проверка очереди сообщений обмена';en='Exchange with mobile klientom.Proverka turns sharing messages'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				ВыборкаСообщенийОбмена.МобильныйКлиент,
				НСтр("ka='დარღვეულია გაცვლის შეტყობინებების თანმიმდევრობა.';ru='Нарушен порядок следования сообщений обмена.';en='Violation of the order of the exchange of messages.'"));
...
Процедура ЗагрузитьОбъекты(УзелОбмена, Объекты, ЭтоНовыйОбмен) Экспорт
	...
	Если Объекты <> Неопределено Тогда
		Для каждого ОбъектXDTO Из Объекты.objects Цикл
			...
			//--------------------------------------------------------------------------
			// © Шишка В.В. [11 мая 2022]  НАЧАЛО
			ИначеЕсли ОбъектXDTO.Тип().Имя = "Characteristicitems" Тогда
				НайтиСоздатьХарактеристикаНоменклатура(УзелОбмена, ОбъектXDTO, ЭтоНовыйОбмен);
			// © Шишка В.В. [11 мая 2022]  КОНЕЦ
			//--------------------------------------------------------------------------
...
// Функция находит/создает номенклатуру.
//
Функция НайтиСоздатьХарактеристикаНоменклатура(УзелОбмена, ОбъектXDTO, ЭтоНовыйОбмен)
	...
	Если Объект.Наименование <> ОбъектXDTO.Name Тогда
		...
		НужноЗаписыватьОбъект = Истина;
	КонецЕсли;
	Владелец = НайтиСоздатьНоменклатура(УзелОбмена, ОбъектXDTO.Nomenclature, ЭтоНовыйОбмен);
	Если Объект.Владелец <> Владелец Тогда
...
Функция ПометитьОбъектНаУдаление(УзелОбмена, ОбъектXDTO)
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [11 мая 2022]  НАЧАЛО
	ИначеЕсли ОбъектXDTO.Type = "Characteristicitems" Тогда
		Возврат Неопределено;
	// © Шишка В.В. [11 мая 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
