	...
	Если АвтономнаяРаботаСлужебный.ЭтоАвтономноеРабочееМесто() Тогда
		...
		Если (Источник.ПометкаУдаления ИЛИ Источник.Публикация = Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена) И ЭтоОбработкаСервиса(Источник.Ссылка) Тогда
			...
			ВызватьИсключение СтрШаблон(
				НСтр("ka='დამატებითი ანგარიშგება ან დამუშავება %1 ჩაიტვირთა სერვისიდან და არ შეიძლება, რომ გამორთული იქნას ავტონომიური სამუშაო ადგილიდან!"
"დამატებითი ანგარიშგების ან დამუშავების წასაშლელად, აუცილებელია სერვისის დანართში გამორთვის ოპერაციის შესრულება და ავტონომიური სამუშაო ადგილის მონაცემთა სინქრონიზაციის ჩატარება სერვისთან.';ru='Дополнительный отчет или обработка %1 был загружен из сервиса и не может быть отключен из автономного рабочего места!"
