...
Функция ЗаполнитьТранзакцию(Строка, Транзакция, ФорматОбмена, ТекстОшибки)
	...
	Попытка
		...
		Для Индекс = 8 По КоличествоПолей Цикл
			...
			ПолеОшибки = Нстр("ka='ველი №%1% (%1%)';ru='Поле №%1% (%1%)';en='Field №% 1% (% 1%)'");
...
Функция РазобратьОтчет(ТекстовыйДокумент, Разделитель, ВыходныеПараметры)
	...
	Если ТекущаяСтрока = "#" Тогда
		...
		Пока Не ПустаяСтрока(ТекущаяСтрока) Цикл
			...
			Попытка
				...
				ПолеОшибки = Нстр("ka='დოკუმენტის ნომერი (6)';ru='Номер документа (6)';en='Document number (6)'");
				НомерДокумента     = Число(НомерДокументаСтр);
				ПолеОшибки = Нстр("ka='ცვლის ნომერი (14)';ru='Номер смены (14)';en='changing room (14)'");
				НомерСмены         = Число(НомерСменыСтр);
			Исключение
				ТекстОшибки = НСтр("ka='ფაილის არასწორი ფორმატი. ველის გარდაქმნა რიცხვზე შეუძლებელია:';ru='Неверный формат файла. Невозможно преобразовать к числу поле:';en='Invalid file format. Can not be converted to the number of field:'") + Символы.НПП + ПолеОшибки;
				Результат = Ложь;
				Прервать;
			КонецПопытки;
			...
			Если ТипТранзакции =  1 Тогда
				...
				ТекстОшибки = НСтр("ka='გაყიდვის რეგისტრაცია საქონლის კოდის აღრიცხვის გარეშე დაუშვებელია';ru='Регистрация продаж без учета кода товара не допускается';en='Registration of sales excluding the commodity code is not allowed'");
				Результат = Ложь;
				Прервать;
			ИначеЕсли ТипТранзакции = 11 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ka='გაყიდვის თანხა (16)';ru='Сумма продажи (16)';en='The sum of sales (16)'");
	...
	ИначеЕсли ТекущаяСтрока = "@" Тогда
		...
		Результат = Ложь;
	Иначе
		ТекстОшибки = НСтр("ka='მონაცემთა არასწორი ფორმატი ან მონაცემები არ არის.';ru='Неверный формат данных или данные отсутствуют.';en='Invalid data format or data are not available.'");
		Результат = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			Для Каждого ТекКомбинация Из КомбинацииНалогов Цикл
				...
				Текст = НСтр("ka='Группа №';ru='Группа №';en='Group number'") + КодКомбинации;
