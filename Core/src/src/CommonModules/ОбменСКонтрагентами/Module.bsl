...
Процедура СформироватьКомандыЭДО(ПолноеИмя, КомандыЭДО, НаправлениеЭД = Неопределено, ТолькоВМенюЕще = Ложь) Экспорт
	...
	Если СоставКоманд.Исходящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ЗначениеФОИспользоватьОбменЭД Тогда
			...
			КомандаЭДО.Представление = НСтр("ka='ელექტრონული დოკუმენტის ნახვა';ru='Просмотреть электронный документ';en='See the electronic document'");
			...
			КомандаЭДО.Представление = НСтр("ka='ელექტრონული დოკუმენტის შექმნა';ru='Создать электронный документ';en='Create an electronic document'");
			...
			КомандаЭДО.Представление = НСтр("ka='ელექტრონული დოკუმენტის გაგზავნა';ru='Отправить электронный документ';en='Send an electronic document'");
			...
			КомандаЭДО.Представление = НСтр("ka='ელექტრონული დოკუმენტების გახსნა';ru='Открыть электронные документы';en='Open electronic documents'");
			...
			КомандаЭДО.Представление = НСтр("ka='ედბ-ის მიმდინარე საქმეები';ru='Текущие дела ЭДО';en='Current affairs EDT'");
			...
			КомандаЭДО.Представление = НСтр("ka='თავისუფალი დოკუმენტის შექმნა';ru='Создать произвольный документ';en='Create an arbitrary document'");
		...
		Если ЕстьПравоВывода И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
			...
			КомандаЭДО.Представление = НСтр("ka='დოკუმენტის გადმოტვირთვა ფაილში';ru='Выгрузить документ в файл';en='Unload the document in the file'");
			КомандаЭДО.Порядок       = 50;
		...
		Если ЕстьПравоВывода И ЕстьПодсистемаБизнесСеть И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
			...
			КомандаЭДО.Представление = НСтр("ka='დოკუმენტის გაგზავნა ელექტრონული ხელმოწერის გარეშე';ru='Отправить документ без электронной подписи';en='Send document without electronic signature'");
	...
	Если СоставКоманд.Входящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ЗначениеФОИспользоватьОбменЭД Тогда
			...
			КомандаЭДО.Представление = НСтр("ka='ელექტრონული დოკუმენტის მიხედვით გადავსება';ru='Перезаполнить по электронному документу';en='Refill of electronic document'");
			КомандаЭДО.Порядок       = 30;
		...
		Если ЕстьПравоОбменаЭД Тогда
			...
			КомандаЭДО.Представление = НСтр("ka='დოკუმენტის ჩატვირთვა ფაილიდან';ru='Загрузить документ из файла';en='Upload a document from a file'");
			КомандаЭДО.Порядок       = 50;
			...
			Если ЗначениеФОИспользоватьОбменБизнесСеть И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
				...
				КомандаЭДО.Представление = НСтр("ka='დოკუმენტის ჩატვირთვა ელექტრონული ხელმოწერის გარეშე';ru='Загрузить документ без электронной подписи';en='Upload a document without a digital signature'");
	...
	Если ЗначениеФОИспользоватьИнтеркампани И СоставКоманд.Интеркампани.Найти(ПолноеИмя) <> Неопределено Тогда
		...
		КомандаЭДО.СписокФорм    = "";
		КомандаЭДО.Идентификатор = "СформироватьПодписатьОтправитьЭД";
		КомандаЭДО.Представление = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписать электронный документ';en='To sign an electronic document'");
...
Процедура ПроверитьКонтрагентаВ1СЭДОПриСозданииНаСервере(СсылкаНаКонтрагента) Экспорт
	...
	Попытка
		...
		ФоновыеЗадания.Выполнить("ОбменСКонтрагентамиСлужебный.ПроверитьКонтрагентаФоновоеЗадание", 
			Параметры, ПараметрыЗапуска.ИНН + " " + ПараметрыЗапуска.КПП, НСтр("ka='შეერთების შემოწმება';ru='Проверка подключения';en='Check connections'"));
	...
	Исключение
		...
		Операция = НСтр("ka='ფონურ დავალებაში 1C ედბ-სთან კონტრაგენტის შეერთების შემოწმება.';ru='Проверка подключения контрагента к 1С-ЭДО в фоновом задании.';en='Check the connection to the counterparty 1C-EDM in the background job.'");
...
Процедура НастроитьАвтоПереходНаНовыеВерсииФорматовЭД() Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Операция = НСтр("ka='კონტრაგენტებთან გაცვლის ქვესისტემის განახლება';ru='Обновление подсистемы обмена с контрагентами';en='Update exchange subsystem with contractors'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='კონტრაგენტებთან გაცვლის ქვესისტემის განახლებისას შეცდომა მოხდა';ru='При обновлении подсистемы обмена с контрагентами произошла ошибка';en='When you update exchange subsystem with counterparties error occurred'");
...
Процедура УдалитьНовыеВидыЭДУПД_УКД() Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		Для каждого Строка Из ИскомыйПрофильНастроек.ИсходящиеДокументы Цикл
			...
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомыйПрофильНастроек.ИспользоватьУПД, НСтр("ka='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';ru='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';en='FTS 5.01 (UPD: The invoice and the original document)'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"));
				Строка.Приоритет = 4;
			КонецЕсли;
			...
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомыйПрофильНастроек.ИспользоватьУКД, НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';en='FTS 5.01 (SACs: Correction invoice and the original document)'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"));
				Строка.Приоритет = 6;
			КонецЕсли;
	...
	Пока Результат.Следующий() Цикл
		...
		Для каждого Строка Из ИскомаяНастройка.ИсходящиеДокументы Цикл
			...
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомаяНастройка.ИспользоватьУПД, НСтр("ka='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';ru='ФНС 5.01 (УПД:Счет-фактура и первичный документ)';en='FTS 5.01 (UPD: The invoice and the original document)'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"));
				Строка.Приоритет = 4;
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель Тогда
			...
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомаяНастройка.ИспользоватьУКД, НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';en='FTS 5.01 (SACs: Correction invoice and the original document)'"), НСтр("ka='ФНС 5.02';ru='ФНС 5.02';en='FTS 5.02'"));
				Строка.Приоритет = 6;
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
				Строка.Приоритет = 7;
	...
	Попытка
		...
		ЗафиксироватьТранзакцию();
	Исключение
		ВидОперации = НСтр("ka='საინფორმაციო ბაზის განახლება';ru='Обновление информационной базы';en='Update the information base'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПриФормированииСпискаУслуг(ПоставщикиУслуг) Экспорт
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ka='ელექტრონული დოკუმენტებით გაცვლა';ru='Обмен электронными документами';en='Exchange of electronic documents'"));
...
Процедура ЗаполнитьДанныеОПрофиляхНастроекЭДО() Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			НастройкаЭДО.Комментарий = НСтр("ka='##ედბ-ის გამართვა მონიშნულია წასაშლელად ავტომატურად განახლებისას.';ru='##Настройка ЭДО помечена на удаление автоматически при обновлении.';en='## Configuring EDI tagged for deletion automatically during the upgrade.'");
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НастройкаЭДО);
		КонецЦикла;
				...
				Для Каждого ЗначениеПеречисления Из АктуальныеВидыЭД Цикл
					Если ЗначениеПеречисления <> Перечисления.ВидыЭД.Подтверждение
						...
						ИначеЕсли ЗначениеПеречисления = Перечисления.ВидыЭД.АктЗаказчик
							...
							ВерсияФормата = НСтр("ka='ФНС 5.01';ru='ФНС 5.01';en='FTS 5.01'");
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				Строка.ВерсияФормата            = НСтр("ka='ФНС 5.01';ru='ФНС 5.01';en='FTS 5.01'");
		...
		Пока ВыборкаУчастников.Следующий() Цикл
			...
			Если Не ВыборкаУчастников.СоглашениеОбИспользованииЭД.ПометкаУдаления Тогда
				...
				НастройкаЭДО.УстановитьПометкуУдаления(Истина);
				НастройкаЭДО.Наименование = НСтр("ka='არ გამოიყენოს';ru='Не использовать';en='Do not use'") + " - " + НастройкаЭДО.Наименование;
				НастройкаЭДО.Комментарий = НСтр("ka='##ედბ-ის გამართვა მონიშნულია წასაშლელად ავტომატურად განახლებისას.';ru='##Настройка ЭДО помечена на удаление автоматически при обновлении.';en='## Configuring EDI tagged for deletion automatically during the upgrade.'");
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НастройкаЭДО);
			КонецЕсли;
		...
		Пока Результат.Следующий() Цикл
			...
			Если СтрокаВидЭД = Неопределено Тогда
				...
				ИначеЕсли ВидЭД = Перечисления.ВидыЭД.УдалитьУПД_ИнформацияПродавца Тогда
					...
					ВерсияФормата = НСтр("ka='საგადასახადო სამსახური 5.01 (აფ+დამატ)';ru='ФНС 5.01 (СЧФ+ДОП)';en='FTS 5.01 (FES + DOP)'");
				ИначеЕсли ВидЭД = Перечисления.ВидыЭД.УдалитьУКД_ИнформацияПродавца Тогда
					
					ВерсияФормата = НСтр("ka='საგადასახადო სამსახური 5.01 (საკორექტირებო ანგარიშფაქტურა+ღირებულების შეცვლის დოკუმენტი)';ru='ФНС 5.01 (КСЧФ+ДИС)';en='FTS 5.01 (KSCHF + DIS)'");
				КонецЕсли;
				НоваяСтрока.ВерсияФормата = ВерсияФормата;
