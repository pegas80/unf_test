...
"%3';ka='რეკომენდებულია მიმდინარე მუშაობის დასრულება და ყველა საკუთარი მონაცემის შენახვა. პროგრამის მუშაობა დასრულდება %1 %2-ში. "
"%3'");
...
Процедура ЗавершитьРаботуПользователей() Экспорт
	...
	Если ТекущийМомент < ВремяНачалаБлокировки Тогда
		ТекстСообщения = НСтр("ka='მომხმარებელთა მუშაობის ბლოკირება დაგეგმილია %1-სთვის.';ru='Блокировка работы пользователей запланирована на %1.';en='Blocking the user experience is scheduled for 1%.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ВремяНачалаБлокировки);
		ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების მუშაობის დასრულება';ru='Завершение работы пользователей';en='Shut users'"), 
	...
	Исключение
		СоединенияИБКлиент.УстановитьПризнакРаботаПользователейЗавершается(Ложь);
			ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების მუშაობის დასრულება';ru='Завершение работы пользователей';en='Shut users'"),
			"e1cib/app/Обработка.БлокировкаРаботыПользователей", 
			НСтр("ka='სეანსების დასრულება არ არის შესრულებული. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Завершение сеансов не выполнено. Подробности см. в Журнале регистрации.';en='Completion of the sessions is not satisfied. For details, see. In the Journal of registration.'"), БиблиотекаКартинок.Предупреждение32);
	...
	ПоказатьОповещениеПользователя(НСтр("ka='მომხმარებლების მუშაობის დასრულება';ru='Завершение работы пользователей';en='Shut users'"),
		"e1cib/app/Обработка.БлокировкаРаботыПользователей", 
		НСтр("ka='სეანსები წარმატებით დასრულდა';ru='Завершение сеансов выполнено успешно';en='Completion of the sessions was successful'"), БиблиотекаКартинок.Информация32);
	Оповестить("ЗавершениеРаботыПользователей", Новый Структура("Статус,КоличествоСеансов", "Готово", КоличествоСеансов));
	СоединенияИБКлиент.ЗавершитьРаботуЭтогоСеанса();
