...
Процедура УстановитьНастройкиПоУмолчанию(ФормаОтчета) Экспорт
	...
	Если ИдентификаторОбъекта = "ОборотноСальдоваяВедомостьРегл" Тогда
...
Процедура ДобавитьГруппировкуПоПериоду(ПараметрыОтчета, Структура) Экспорт
	...
	ИначеЕсли ПараметрыОтчета.Периодичность >= 6 Тогда
		...
		ГруппаОтбора.Представление = НСтр("ru='Исключение пустых оборотов';en='Exclusion of empty speed'");
...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ВыводитьПолностью Тогда
		...
		Исключение
			...
			КонецЦикла;
			ТекстСообщения = НСтр("ka='ანგარიშგება არ დაფორმირდა! %1';ru='Отчет не сформирован! %1';en='The report is not generated! %one'");
...
Процедура ОбработкаПроверкиЗаполнения(ОтчетОбъект, Отказ, Проверки = Неопределено) Экспорт
	...
	Если Проверки.Свойство("КорректностьПериода") И Проверки.КорректностьПериода Тогда
		Если ЗначениеЗаполнено(ОтчетОбъект.КонецПериода) И ОтчетОбъект.НачалоПериода > ОтчетОбъект.КонецПериода Тогда
			ТекстСообщения = НСтр("ru='Дата начала периода не может быть больше даты конца периода';en='Date of commencement of the period can not be more than the end date of the period'");
	...
	Если Проверки.Свойство("ВыборПоказателя") И Проверки.ВыборПоказателя Тогда
		...
		Если КоличествоПоказателей = 0 И НаборПоказателей.Количество() > 0 Тогда
			ТекстСообщения = НСтр("ka='არ არის არჩეული მაჩვენებელი(ები)';ru='Не выбран показатель(и)';en='Not selected indicator (s)'");
	...
	Если Проверки.Свойство("СписокВидовСубконто") И Проверки.СписокВидовСубконто Тогда
		...
		Если ОтчетОбъект.СписокВидовСубконто.Количество() > МаксКоличествоСубконто Тогда
			ТекстСообщения = НСтр("ka='არჩეულია ძალიან ბევრი სუბკონტოს სახეობა. დასაშვებია მაქსიმუმ %1';ru='Выбрано слишком много видов субконто, максимально допустимо %1';en='Too many types subkonto maximum permissible% 1'");
...
Процедура ОбработкаПроверкиЗаполненияОтборов(ОтчетОбъект, Отказ) Экспорт
	...
	Для каждого Отбор Из ОтборыВерхнегоУровня Цикл
		...
		ТекстСообщения = НСтр("ka='ველი სუბკონტოს რეკვიზიტებით ""%1"" არ შეიძლება გამოყენებული იყოს დაჯგუფურ შერჩევაში სხვა ველებთან ერთად.';ru='Поле с реквизитами субконто ""%1"" нельзя использовать в групповом отборе с иными полями.';en='Field with the details subkonto ""%1"" can not be used in the selection of the group with other fields.'");
...
Процедура ВывестиЗаголовокОтчета(ПараметрыОтчета, КомпоновщикНастроек, Результат) Экспорт
	...
	Если ПоказыватьВыводимыеДанные Тогда 
		ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru='Выводимые данные:';en='Output data:'");
	...
	Если Не ПустаяСтрока(ТекстОтбор) Тогда
		ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru='Отбор:';en='Selection:'");
	...
	Если ИдентификаторОбъекта <>  "ОтчетОбъект.ОборотноСальдоваяВедомость" 
		...
		Если Не ПустаяСтрока(ТекстСортировка) Тогда
			ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru='Сортировка:';en='Sorting:'");
...
Функция ПолучитьТекстОбособленныхПодразделений(Организация) Экспорт
	...
	ТекстОрганизации = ТекстОрганизации + " " + НСтр("ru='с обособленными подразделениями';en='with separate subdivisions'");
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(ФормаОтчета, Настройки, ЗагружатьТолькоРеквизиты = Ложь) Экспорт
	...
	Если ИдентификаторОбъекта = "ОборотноСальдоваяВедомостьРегл" Тогда
		Если ФормаОтчета.Отчет.ДополнительныеПоля.НайтиСтроки(Новый Структура("Поле", "Счет.Наименование")).Количество() = 0 Тогда
			...
			НоваяСтрока.Представление = НСтр("ka='ანგარიშის დასახელების გამოტანა';ru='Выводить наименование счета';en='Show the name of the account'");// "Выводить наименование счета";
...
Процедура ВывестиОписаниеОтбора(ФормаОтчета, ОписаниеОтбора)
	...
	// Какое-то исключение
	Если БухгалтерскиеОтчеты.ИмяОтчета(ФормаОтчета) = "ОборотноСальдоваяВедомостьРегл" Тогда
		Возврат;
	КонецЕсли;
	...
	ОбластьОписания.Текст = НСтр("ru='Отбор:';en='Selection:'");
...
Функция ПолучитьПараметрыРасшифровкиОтчета(Адрес, ИдентификаторОбъекта, Расшифровка) Экспорт
	...
	Если ИдентификаторОбъекта = "АнализСубконто"
		...
		ИЛИ ИдентификаторОбъекта = "ОборотноСальдоваяВедомостьРегл"
		...
		ПредставлениеДляОСВПоСчету            = НСтр("ka='ОСВ по счету %Счет%';ru='ОСВ по счету %Счет%';en='WWS account% account%'");
		ПредставлениеДляКарточкиСчета         = НСтр("ka='Карточка счета %Счет%';ru='ანგარიშის ბარათი  %Счет%';en='Card account% account%'");
		ПредставлениеДляАнализаСчета          = НСтр("ka='Анализ счета %Счет%';ru='Анализ счета %Счет%';en='Analysis of the account% account%'");
		ПредставлениеДляОборотыСчета          = НСтр("ka='Обороты счета %Счет%';ru='Обороты счета %Счет%';en='Turnovers account% account%'");
		ПредставлениеДляОборотыСчетаПоМесяцам = НСтр("ka='Обороты счета %Счет% по месяцам';ru='Обороты счета %Счет% по месяцам';en='Turnovers account% account% on month'");
		ПредставлениеДляОборотыСчетаПоДням    = НСтр("ka='Обороты счета %Счет% по дням';ru='Обороты счета %Счет% по дням';en='Turnovers account% account% on days'");
		ПредставлениеДляКарточкиСубконто      = НСтр("ka='Карточка субконто';ru='სუბკონტოს ბარათი';en='card subkonto'");
		ПредставлениеДляОтчетаПоПроводкам     = НСтр("ka='Отчет по проводкам';ru='Отчет по проводкам';en='Report on transactions'");
		ПредставлениеОткрытьОбъект 			  = НСтр("ka='Открыть ""%Значение%""';ru='Открыть ""%Значение%""';en='Open ""% The%""'");
	...
	Если ПоляТекущейРасшифровки.Количество() > 0
		...
		ИЛИ ПоляТекущейРасшифровки.Количество() = 1) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЗначениеПоля)) Тогда
				ВозможныеРасшифровки.Добавить(ПоляТекущейРасшифровки[0].Значение, СтрШаблон(НСтр("ka='გახსნა ""%1""';ru='Открыть ""%1""';en='Open ""%1""'"), ПоляТекущейРасшифровки[0].Значение));
			КонецЕсли;
...
Функция ПолучитьОписаниеПросроченнойЗадолженностиПокупателями() Экспорт
	...
	Ссылка = Новый ФорматированнаяСтрока(НСтр("ru='Сроки оплаты покупателями';en='Payment Terms buyers'"),,,, "e1cib/command/ОбщаяКоманда.СрокиОплатыПокупателями");
...
Функция ПолучитьОписаниеПросроченнойЗадолженностиПоставщикам() Экспорт
	...
	Ссылка = Новый ФорматированнаяСтрока(НСтр("ru='Срок оплаты поставщикам';en='Term of payment to suppliers'"),,,, "e1cib/command/ОбщаяКоманда.СрокОплатыПоставщикам");
...
Процедура ИнициализацияФормыОтчета(Отчет, РеквизитыФормы, Форма)
	...
	Если РеквизитыФормы.Свойство("ИспользуетсяНесколькоОрганизаций") Тогда
		Форма.ИспользуетсяНесколькоОрганизаций = Истина; //Справочники.Организации.ИспользуетсяНесколькоОрганизаций();
	КонецЕсли;
	
	Если РеквизитыФормы.Свойство("ВестиУчетЗатратПоПодразделениям") Тогда
		Форма.ВестиУчетЗатратПоПодразделениям = Ложь; //ПолучитьФункциональнуюОпцию("ВестиУчетЗатратПоПодразделениям");
	КонецЕсли;
		...
		Для каждого Отбор из МассивПолей Цикл
			Если ТипЗнч(Отбор) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") тогда
				...
				ИначеЕсли СтрНайти(ПолеОтбора, "Субконто") = 1 Тогда
					...
					ИначеЕсли ИдентификаторОбъекта = "ОборотноСальдоваяВедомостьРегл" Тогда
...
Процедура УстановитьОрганизацию(Форма)
	...
	Если Истина Тогда//Справочники.Организации.ИспользуетсяНесколькоОрганизаций() Тогда
...
Функция ПолучитьОписаниеПросроченнойЗадолженности(СрокОплаты)
	...
	Если СрокОплаты = 0 Тогда
		Описание = НСтр("ru = 'Просроченной считается любая задолженность, если срок оплаты не установлен в договоре или в отдельных документах.
                         |Установить срок оплаты задолженности по умолчанию можно в меню'; ka = 'Просроченной считается любая задолженность, если срок оплаты не установлен в договоре или в отдельных документах.
	...
	Иначе
	    ...
	    Описание = НСтр("ru = 'Просроченной считается задолженность, не оплаченная в течение %1,
                         ...
                         |Изменить значение по умолчанию можно в меню'; ka = 'Просроченной считается задолженность, не оплаченная в течение %1,
                         |если в договоре или в отдельных документах не установлен другой срок оплаты.
...
Функция ОписаниеОтборовОтчета(ПараметрыОтчета, КомпоновщикНастроек)
	...
	Иначе
		...
		Если ПараметрыОтчета.Свойство("Подразделение") И ЗначениеЗаполнено(ПараметрыОтчета.Подразделение) Тогда
			ТекстПодразделение = СтрШаблон(НСтр("ka='განყოფილება უდრის ""%1""';ru='Подразделение Равно ""%1""';en='Equal division of ""%1""'"), ПараметрыОтчета.Подразделение);
		КонецЕсли;
		Если Не ПустаяСтрока(ТекстОтбор) И Не ПустаяСтрока(ТекстПодразделение) Тогда
			ТекстОтбор = ТекстПодразделение + " " + НСтр("ru='И';en='AND'") + " " + ТекстОтбор;
...
Процедура ПодставитьИменаСубконто(ФормаОтчета, ТекстОтбор)
	...
	ИмяПоляПрефикс = НСтр("ka='Субконто';ru='Субконто';en='subkonto'");	
	Если СтрНайти(ТекстОтбор, ИмяПоляПрефикс) = 0
	 Или Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета.Отчет, "Счет") Тогда
		Возврат;
	КонецЕсли;
	ИмяПоляПриставка = НСтр("ka='Кор';ru='Кор';en='Cor'");
