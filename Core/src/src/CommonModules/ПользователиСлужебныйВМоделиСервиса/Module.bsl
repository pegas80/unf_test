	...
	Если ДоступноИзменениеПользователей() Тогда
		...
		Иначе
			...
			Если ЕстьПравоДобавленияПользователей() Тогда
				Возврат ДействияСНовымПользователемСервиса();
			Иначе
				ВызватьИсключение НСтр("ka='ახალი მომხმარებლების დასამატებლად დაშვების უფლებები არასაკმარისია';ru='Недостаточно прав доступа для добавления новых пользователей';en='It is not enough access rights to add new users'");
			КонецЕсли;
...
Процедура ПередНачаломОбработкиПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.ВнешниеПользователи")
	   И ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение НСтр("ka='სერვისის მოდელში გარე მომხმარებლებს მხარდაჭერა არ გააჩნია.';ru='Внешние пользователи не поддерживаются в модели сервиса.';en='External users are not supported in the service model.'");
	КонецЕсли;
...
"доступно только неразделенным пользователям.';ka='მოშორებული ადმინისტრირების არხის შეტყობინების მიხედვით მომხმარებლის განახლება ხელმისაწვდომია მხოლოდ განუყოფელი მომხმარებლებისთვის.'");
		КонецЕсли;
...
"пользователя сервиса в элементе справочника.';ka='შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. ცნობარის ელემენტში სერვისის მომხმარებლის უკვე დაყენებული იდენტიფიკატორის შეცვლა არ შეიძლება.'"),
...
"уже используется в элементе ""%3"".';ka='შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას."
"სერვის ""%2""-ის მომხმარებლის იდენტიფიკატორის დაყენება ცნობარის ამ ელემენტში არ შეიძლება, ვინაიდან იგი უკვე გამოიყენება ელემენტ ""%3""-ში.'"),
				ПользовательОбъект.Наименование,
				АвтоРеквизиты.ИдентификаторПользователяСервиса,
				НайденныйПользователь);
		КонецЕсли;
...
"Обновление реквизита выполняется автоматически.';ka='შეცდომა მომხმარებელ ""%1""-ის ჩაწერისას. რეკვიზიტ სერვისისმომხმარებლისიდენტიფიკატორი-ს შეცვლა დაუშვებელია. რეკვიზიტის განახლება წარმოებს ავტომატურად.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Для каждого ПраваДоступаОбъектаXDTO Из ПраваДоступаОбъектовXDTO.Item Цикл
		...
		ИначеЕсли ПраваДоступаОбъектаXDTO.Object.Тип() = ТипКИПользователя Тогда
			...
			Если ВидКИ = Неопределено Тогда
				ШаблонСообщения = НСтр("ka='მიღებულია საკონტაქტო ინფორმაციის უცნობი ტიპი: %1';ru='Получен неизвестный вид контактной информации: %1';en='An unknown type of contact information: 1%'");
			...
			КонецЦикла;
		Иначе
			ШаблонСообщения = НСтр("ka='მიღებულია დაშვების ობიექტების უცნობი ტიპი: %1';ru='Получен неизвестный тип объектов доступа: %1';en='An unknown object type access:% 1'");
