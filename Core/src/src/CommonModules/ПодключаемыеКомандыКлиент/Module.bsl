...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяРаботаСФайлами Тогда
		...
		ТекстСообщения = НСтр("ka='Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.';ru='Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.';en='To continue, you must install the extension for Web client 1C: Enterprise.'");
		...
		Если ИнформацияОДокументах.КоличествоНеПроведенных > 0 Тогда
			Если ИнформацияОДокументах.ЕстьПравоПроведенияХотяБыОдногоНепроведенного Тогда
				Если ИнформацияОДокументах.КоличествоНеПроведенных = 1 Тогда
					ТекстВопроса = НСтр("ka='Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?';ru='Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?';en='To execute the command to carry out a pre document should be. Run holding a document and continue?'");
				Иначе
					ТекстВопроса = НСтр("ka='Для выполнения команды необходимо предварительно провести документы. Выполнить проведение документов и продолжить?';ru='Для выполнения команды необходимо предварительно провести документы. Выполнить проведение документов и продолжить?';en='To execute the command, you must first carry out document. Run holding documents and proceed?'");
				КонецЕсли;
			...
			Иначе
				Если ИнформацияОДокументах.КоличествоНеПроведенных = 1 Тогда
					ТекстПредупреждения = НСтр("ka='Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.';ru='Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.';en='To execute the command to carry out a pre document should be. Insufficient permissions for the document.'");
				Иначе
					ТекстПредупреждения = НСтр("ka='Для выполнения команды необходимо предварительно провести документы. Недостаточно прав для проведения документов.';ru='Для выполнения команды необходимо предварительно провести документы. Недостаточно прав для проведения документов.';en='To execute the command, you must first carry out document. Insufficient permissions for documents.'");
				КонецЕсли;
...
Процедура ПродолжитьВыполнениеКомандыПослеПодтверждениеПроведения(Ответ, Контекст) Экспорт
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		Если Контекст.МассивСсылок.Количество() = 0 Тогда
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ka='ОК';ru='ОК';en='OK'"));
		Иначе
