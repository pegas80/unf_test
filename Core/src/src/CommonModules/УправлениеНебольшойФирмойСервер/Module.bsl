...
Процедура ПриКопированииОбъекта(Источник) Экспорт
	...
	Если НЕ ПустаяСтрока(Источник.Комментарий) Тогда
		//DD ჩახსნილია კლიენტის მოთხოვნით
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მეორე პარამეტრის არასწორი ტიპი რეკვიზიტები: %1';ru='Неверный тип второго параметра Реквизиты: %1';en='Wrong type of second parameter Details: 1%'"),
...
//
//Функция НалогообложениеНДС(Организация, Склад = Неопределено, Дата, Контрагент = Неопределено, Договор = Неопределено) Экспорт
Функция НалогообложениеНДС(Организация, Склад = Неопределено, Дата, Контрагент = Неопределено, Договор = Неопределено, ПолюбомуОблагать = ЛОЖЬ) Экспорт	
	//Паганини_
	//Запрос = Новый Запрос;
	//
...
//	Дата - Дата определения системы налогообложения
//
Функция НалогообложениеНДС_СтруктураСНДС(Организация, Склад = Неопределено, Дата, Контрагент = Неопределено, Договор = Неопределено) Экспорт //+++ Дмгг
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		Если ДанныеОрганизации.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(ДанныеКонтрагента.НалогообложениеНДС) Тогда 
					НалогообложениеНДС = ?(ЗначениеЗаполнено(Контрагент.НалогообложениеНДС), Контрагент.НалогообложениеНДС, Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС);
				...
				Иначе
					НалогообложениеНДС = ДанныеКонтрагента.НалогообложениеНДС;
					СтавкаНДС = ДанныеКонтрагента.СтавкаНДС;
				КонецЕсли;
			КонецЕсли;
		Иначе
			НалогообложениеНДС = ДанныеОрганизации.НалогообложениеНДС;
...
Функция НайтиКонтрагентов(СтрокаПоиска, СписокКонтрагентов)
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		Возврат НСтр("ka='ძალიან ბევრი შედეგია, დააზუსტეთ მოთხოვნა:';ru='Слишком много результатов, уточните запрос.';en='Too many results, specify your inquiry.'");
	КонецЕсли;

	Если СписокПоиска.ПолноеКоличество() = 0 Тогда
		Возврат НСтр("ka='არაფერი მოიძებნა';ru='Ничего не найдено';en='Nothing found'");
	КонецЕсли;
	...
	Пока ЕстьСледующаяПорция Цикл
		...
		Для СчетчикЭлементов = 0 По КонечнаяПозиция Цикл
			...
			Если Элемент.Метаданные = Метаданные.Справочники.Контрагенты Тогда
				...
				Основание = НСтр("ru='Найдено: Контрагент - ';ka='მოიძებნა: კონტრაგენტი -'") + Элемент.Описание;
				
			// Контактные лица
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.КонтактныеЛица Тогда
				
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования = НСтр("ka='მოიძებნა: საკონტაქტო პირი ""%1"" - %2';ru='Найдено: Контактное лицо ""%1"" - %2';en='Found: Contact person ""%1"" - 2%'");
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.ФизическиеЛица Тогда
				...
				Если ТаблицаНайденныхКонтрагентов <> Неопределено Тогда
					...
					Для каждого СтрокаТаблицы Из ТаблицаНайденныхКонтрагентов Цикл
						ШаблонОснования = НСтр("ka='მოიძებნა: საკონტაქტო პირის ""%2"" - %3 ფიზიკური პირი ""%1""';ru='Найдено: Физическое лицо ""%1"" контактного лица ""%2"" - %3';en='Found: Person ""%1"" contact face ""%2"" - 3%'");
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.ДоговорыКонтрагентов Тогда
				...
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования =  НСтр("ka='მოიძებნა: ხელშეკრულება ""%1"" - %2';ru='Найдено: Договор ""%1"" - %2';en='Found: Contract ""%1"" - 2%'");
				Основание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОснования, Элемент.Значение, Элемент.Описание);
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.БанковскиеСчета Тогда
				...
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования =  НСтр("ka='მოიძებნა: საბანკო ანგარიში ""%1"" - %2';ru='Найдено: Банковский счет ""%1"" - %2';en='Found: Bank Account ""%1"" - 2%'");
				Основание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОснования, Элемент.Значение, Элемент.Описание);
...
Процедура ИнициализироватьДополнительныеСвойстваДляПроведения(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	//СтруктураДополнительныеСвойства.ДляПроведения.Вставить("МоментКонтроля", Дата('00010101')); //+++ Дмгг
	СтруктураДополнительныеСвойства.ДляПроведения.Вставить("МоментКонтроля", ДокументСсылка.Дата+1); //+++ Дмгг
	...
	|	,ЕСТЬNULL(УчетнаяПолитикаРеглСрезПоследних.ВозвратЗапасовПоСебестоимостиПродаж, ЛОЖЬ) КАК ВозвратЗапасовПоСебестоимостиПродаж
	// © Шишка В.В. [17 мая 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
	//--------------------------------------------------------------------------
	// © Шишка В.В. [12 августа 2021]  НАЧАЛО
	//|ИЗ
	//|	Константы КАК Константы");
	|	,ЕСТЬNULL(УчетнаяПолитикаРеглСрезПоследних.УчетСебестоимостиВРазрезеОрганизации, ЛОЖЬ) КАК УчетСебестоимостиВРазрезеОрганизации
	|ИЗ
	|	Константы КАК Константы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаРегл.СрезПоследних(&МоментВремени, Организация = &Организация) КАК УчетнаяПолитикаРеглСрезПоследних
	|		ПО (ИСТИНА)");
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(ДокументСсылка.МоментВремени(), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	// © Шишка В.В. [12 августа 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	ИспользованиеТолькоРеглПланаСчетов = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьРегламентированныйПланСчетов")
											И УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ДатаНачалаИспользованияРеглПланаСчетов") < ДокументСсылка.Дата;
	СтруктураДополнительныеСвойства.Вставить("ИспользованиеТолькоРеглПланаСчетов", ИспользованиеТолькоРеглПланаСчетов);			
	
	РезервированиеБезСебестоимости = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("РезервированиеБезСебестоимости");
	СтруктураДополнительныеСвойства.Вставить("РезервированиеБезСебестоимости", РезервированиеБезСебестоимости);
	//--- Дмгг
	
	//--------------------------------------------------------------------------
	// © Шишка В.В. [13 августа 2020 г.]  НАЧАЛО
	ИспользоватьГарантииВНарядЗаказах = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьГарантииВНарядЗаказах");
	СтруктураДополнительныеСвойства.Вставить("ИспользоватьГарантииВНарядЗаказах", ИспользоватьГарантииВНарядЗаказах);
...
Функция ЕстьПроцедураСоздатьПустуюВременнуюТаблицуИзменение(ПолноеИмяРегистра)
	...
	РегистрыСПроцедурой.Добавить(Метаданные.РегистрыНакопления.ОстаткиНалоговыхЛьгот.ПолноеИмя());
...
Процедура ОтразитьДоходыИРасходы(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	//+++ Дмгг
	Если ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда
		КолонкаСчетУчета = ТаблицаДоходыИРасходы.Колонки.Найти("СчетУчета");
		Если КолонкаСчетУчета <> Неопределено Тогда 
			ТаблицаДоходыИРасходы.ЗаполнитьЗначения(ПланыСчетов.Управленческий.ПустаяСсылка(), "СчетУчета");
		КонецЕсли;
	КонецЕсли;
	//--- Дмгг
...
Процедура ОтразитьЗапасы(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	//+++ Дмгг
	Если ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда
		КолонкаСчетУчета = ТаблицаЗапасы.Колонки.Найти("СчетУчета");
		Если КолонкаСчетУчета <> Неопределено Тогда 
			ТаблицаЗапасы.ЗаполнитьЗначения(ПланыСчетов.Управленческий.ПустаяСсылка(), "СчетУчета");
		КонецЕсли;
		КолонкаКоррСчетУчета = ТаблицаЗапасы.Колонки.Найти("КолонкаКоррСчетУчета");
		Если КолонкаКоррСчетУчета <> Неопределено Тогда 
			ТаблицаЗапасы.ЗаполнитьЗначения(ПланыСчетов.Управленческий.ПустаяСсылка(), "КолонкаКоррСчетУчета");
		КонецЕсли;
	КонецЕсли;
	//--- Дмгг
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [12 августа 2021]  НАЧАЛО
	УчетСебестоимостиВРазрезеОрганизации = Ложь;
	Если ДополнительныеСвойства.ДляПроведения.МетаданныеДокумента <> Метаданные.Документы.КорректировкаРегистров
		И ДополнительныеСвойства.УчетнаяПолитика.Свойство("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации)
		И УчетСебестоимостиВРазрезеОрганизации = Истина
		Тогда
	
		Для каждого ТекущаяСтрока Из ТаблицаЗапасы Цикл
		
			Если ТипЗнч(ТекущаяСтрока.СтруктурнаяЕдиница) = Тип("СправочникСсылка.СтруктурныеЕдиницы")
				И ТекущаяСтрока.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.Склад")
				Тогда
			
				ТекущаяСтрока.СтруктурнаяЕдиница = Неопределено;
...
Процедура ОтразитьПробег(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	Если НЕ ДополнительныеСвойства.ТаблицыДляДвижений.Свойство("ТаблицаПробег") Тогда 
		Возврат;
	КонецЕсли;
	
	ТаблицаПробег = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПробег;
	
	Если Отказ
	 ИЛИ ТаблицаПробег.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияПродажи = Движения.Пробег;
	ДвиженияПродажи.Записывать = Истина;
	ДвиженияПродажи.Загрузить(ТаблицаПробег);
...
Процедура ОтразитьПараметрыВнеоборотныхАктивов(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	//+++ Дмгг
	Если ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда
		КолонкаСчетУчета = ТаблицаПараметрыВнеоборотныхАктивов.Колонки.Найти("СчетЗатрат");
		Если КолонкаСчетУчета <> Неопределено Тогда 
			ТаблицаПараметрыВнеоборотныхАктивов.ЗаполнитьЗначения(ПланыСчетов.Управленческий.ПустаяСсылка(), "СчетЗатрат");
		КонецЕсли;
	КонецЕсли;
	//--- Дмгг
...
Процедура ОтразитьУправленческий(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	Если НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПоказатьРеквизитыУправленческогоУчета") Тогда //РегламентированныйУчетСервер.ПроверитьИспользованиеУпрУчета()
		Возврат;
	КонецЕсли;
	...
	Если ВРег(Метаданные.Имя) = ВРег("УправлениеНебольшойФирмойБазовая") Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.КоличествоОрганизаций > 2 Тогда
				Отказ = Истина;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ограничение базовой версии. В информационной базе могут быть введены только две организации.';ru='Ограничение базовой версии. В информационной базе могут быть введены только две организации.';en='Limiting the basic version. Only two organizations can be incorporated in the information database.'"));
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОтразитьПроизводственныеПотери(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаПроизводственныеПотери = ДополнительныеСвойства.ТаблицыДляДвижений.ПроизводственныеПотери;
	
	Если Отказ
	 ИЛИ ТаблицаПроизводственныеПотери.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияЗапасы = Движения.ПроизводственныеПотери;
	ДвиженияЗапасы.Записывать = Истина;
	ДвиженияЗапасы.Загрузить(ТаблицаПроизводственныеПотери);
	
КонецПроцедуры

//--------------------------------------------------------------------------
// © Шишка В.В. [17 августа 2021]  НАЧАЛО
Процедура ВыполнитьКорректировкуВременнойТаблицыЗапасыПоУчетуВРазрезеОрганизаций(СтруктураДополнительныеСвойства) Экспорт

	Перем УчетнаяПолитика;
	Перем УчетСебестоимостиВРазрезеОрганизации;
	
	Если СтруктураДополнительныеСвойства.Свойство("УчетнаяПолитика", УчетнаяПолитика)
		И ТипЗнч(УчетнаяПолитика) = Тип("Структура")
		И УчетнаяПолитика.Свойство("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации)
		И УчетСебестоимостиВРазрезеОрганизации = Истина Тогда
	
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		ВременнаяТаблицаЗапасы = Запрос.МенеджерВременныхТаблиц.Таблицы.Получить("ВременнаяТаблицаЗапасы");
		Если ВременнаяТаблицаЗапасы <> Неопределено Тогда
		
			ПоляТаблицы = "";
			Для каждого ТекущаяКолонка Из ВременнаяТаблицаЗапасы.Колонки Цикл
			
				ПоляТаблицы = ПоляТаблицы + ?(ПустаяСтрока(ПоляТаблицы), "", "," + Символы.ПС) + ?(ТекущаяКолонка.Имя = "СтруктурнаяЕдиница", "НЕОПРЕДЕЛЕНО", "ВременнаяТаблицаЗапасы." + ТекущаяКолонка.Имя) + " КАК " + ТекущаяКолонка.Имя;	
			
			КонецЦикла;
			Запрос.Текст = 
			"ВЫБРАТЬ " + ПоляТаблицы + Символы.ПС 
			+ "ПОМЕСТИТЬ ВременнаяТаблицаЗапасы_
			|ИЗ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
			|;
			|УНИЧТОЖИТЬ ВременнаяТаблицаЗапасы
			|;
			|ВЫБРАТЬ *
			|ПОМЕСТИТЬ ВременнаяТаблицаЗапасы
			|ИЗ ВременнаяТаблицаЗапасы_
			|;
			|УНИЧТОЖИТЬ ВременнаяТаблицаЗапасы_";
			Запрос.Выполнить();
...
//Lazare 28.01.2022
Процедура ОтразитьТаблицаВозвратиDG(ДополнительныеСвойства, Движения, Отказ,Контрагент) Экспорт
	
	
	ТаблицаПродажи = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПродажи;
	ТаблицаПродажи.ЗаполнитьЗначения(Контрагент,"Контрагент");	
	ТаблицаПродажи.Свернуть("Документ,ЗаказПокупателя,Контрагент,Номенклатура,Характеристика,Организация,Ответственный,Партия,Период,Подразделение","Сумма,СуммаНДС,Себестоимость,Количество");		
	
	Построитель = Новый ПостроительЗапроса;
	Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТаблицаПродажи);	
	тОтбор = Построитель.Отбор.Добавить("Себестоимость");
	...
	//|////////////////////////////////////////////////////////////////////////////////
	//|ВЫБРАТЬ
	//|	ВТ_ТаблицаПродажи.Организация КАК Организация,
	//|	ВТ_ТаблицаПродажи.Номенклатура КАК Номенклатура,
	//|	ВТ_ТаблицаПродажи.Характеристика КАК Характеристика,
	//|	ВТ_ТаблицаПродажи.Партия КАК Партия,
	//|	ВТ_ТаблицаПродажи.Контрагент КАК Контрагент,
	//|	ВТ_ТаблицаПродажи.Документ КАК Документ,
	//|	ВТ_ТаблицаПродажи.Подразделение КАК Подразделение,
	//|	ВТ_ТаблицаПродажи.Количество КАК Количество,
	//|	ВТ_ТаблицаПродажи.СуммаНДС КАК СуммаНДС,
	//|	ВТ_ТаблицаПродажи.Сумма КАК Сумма,
	//|	ВТ_ТаблицаПродажи.Себестоимость КАК Себестоимость
	//|ИЗ
	//|	ВТ_ТаблицаПродажи КАК ВТ_ТаблицаПродажи";
	...
	ДвиженияТаблицаВозвратиDG = Движения.ЦеныВозвратаНоменклатурDgFIFO;
	ДвиженияТаблицаВозвратиDG.Записывать = Истина;
	ДвиженияТаблицаВозвратиDG.Загрузить(ТабРезультат);
	
КонецПроцедуры

//Lazare 28.01.2022
Процедура ОтразитьТаблицаВозвратиDGПриходнаяНак(ДополнительныеСвойства, Движения, Отказ,Контрагент) Экспорт
	
	ТаблицаТаблицаВозвратиDG = ДополнительныеСвойства.ТаблицыДляДвижений.СформироватьТаблицаВозвратиDG;
	
	Если Отказ
	 ИЛИ ТаблицаТаблицаВозвратиDG.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ДвиженияТаблицаВозвратиDG = Движения.ЦеныВозвратаНоменклатурDgFIFO;
	ДвиженияТаблицаВозвратиDG.Записывать = Истина;
	ДвиженияТаблицаВозвратиDG.Загрузить(ТаблицаТаблицаВозвратиDG);
	
	//ТаблицаПродажи = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПродажи;
	//ТаблицаПродажи.Колонки.Добавить("Контрагент");
	//ТаблицаПродажи.ЗаполнитьЗначения(Контрагент,"Контрагент");	
	//ТаблицаПродажи.Свернуть("Документ,ЗаказПокупателя,Контрагент,Номенклатура,Характеристика,Организация,Ответственный,Партия,Период,Подразделение,Проект","Сумма,СуммаНДС,Себестоимость,Количество");	
	//		
	//Если Отказ
	// ИЛИ ТаблицаПродажи.Количество() = 0 Тогда
	//	Возврат;
	...
	//ДвиженияТаблицаВозвратиDG = Движения.ЦеныВозвратаНоменклатурDgFIFO;
	//ДвиженияТаблицаВозвратиDG.Записывать = Истина;
	//ДвиженияТаблицаВозвратиDG.Загрузить(ТаблицаПродажи);
...
Функция РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Договор, ОбъектДоговораОбслуживания, Характеристика = Неопределено) Экспорт
	...
	//+++ Дмгг
	ИначеЕсли ТипЗнч(ОбъектДоговораОбслуживания) = Тип("ПланСчетовСсылка.Хозрасчетный") Тогда
		
		Строки = ТарифныйПлан.УчетЗатрат.НайтиСтроки(
		Новый Структура("СтатьяЗатратРегл", ОбъектДоговораОбслуживания)
		);
...
Функция ОкруглитьЦену(Число, ПравилоОкругления, ОкруглятьВБольшуюСторону) Экспорт
	...
	КонстантыКоличествоЗнаковПослеЗапятойВЦене = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("КоличествоЗнаковПослеЗапятойВЦене");
	
	Если КонстантыКоличествоЗнаковПослеЗапятойВЦене > 2 Тогда
		Возврат Число;
	КонецЕсли;
...
//+++ Дмгг
// Возвращает процент скидки по карте лоялдьности.
//
// Параметры:
//  КартаЛояльности - СправочникСсылка.КартыЛояльности - Ссылка на дисконтную карту.
//
// Возвращаемое значение: 
//   Число - процент скидки.
//
Функция ВычислитьПроцентСкидкиПоКартеЛояльности(Знач ДатаСкидки, КартаЛояльности, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если КартаЛояльности.Владелец.ВидСкидкиВДисконтныхКартах = Перечисления.ВидыСкидокВДисконтныхКартах.ФиксированнаяСкидка Тогда
		
		Если ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.ПолучатьСуммуПродаж Тогда
			ПериодНакопления = ПолучитьПериодРасчетаНакопительныхСкидокПоДисконтнойКарте(ДатаСкидки, КартаЛояльности.Ссылка);
			...
			Запрос = Новый Запрос("ВЫБРАТЬ
			                      |	ЕСТЬNULL(СУММА(РегПродажи.СуммаОборот), 0) КАК СуммаОборот
			                      |ИЗ
			                      |	РегистрНакопления.ПродажиПоДисконтнымКартам.Обороты(&ДатаНач, &ДатаКон, , КартаЛояльности = &КартаЛояльности) КАК РегПродажи");

			Запрос.УстановитьПараметр("ДатаНач", ПериодНакопления.НачалоПериода);
			Запрос.УстановитьПараметр("ДатаКон", ПериодНакопления.КонецПериода);
			Запрос.УстановитьПараметр("КартаЛояльности", КартаЛояльности.Ссылка);
		...
		Возврат КартаЛояльности.Владелец.Скидка;
	...
	Иначе
		...
		Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                      ...
		                      |		ЕСТЬNULL(СУММА(РегПродажи.СуммаОборот), 0) КАК СуммаОборот
		                      |	ИЗ
		                      |		РегистрНакопления.БалыЛояльности.Обороты(&ДатаНач, &ДатаКон, , КартаЛояльности = &КартаЛояльности) КАК РегПродажи) КАК РегПродажи
		                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		                      |			Пороги.НижняяГраница КАК НижняяГраница,
		                      |			Пороги.Скидка КАК Скидка
		                      |		ИЗ
		...
		Запрос.УстановитьПараметр("ДатаКон", ПериодНакопления.КонецПериода);
		Запрос.УстановитьПараметр("КартаЛояльности", КартаЛояльности.Ссылка);
        Запрос.УстановитьПараметр("ВидДисконтнойКарты", КартаЛояльности.Владелец);
		...
		Если ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.ПолучатьСуммуПродаж Тогда
			...
			Запрос.Текст = Запрос.Текст + ";
			                              ...
			                              |	ПродажиПоДисконтнымКартамОбороты.СуммаОборот
			                              |ИЗ
			                              |	РегистрНакопления.БалыЛояльности.Обороты(&ДатаНач, &ДатаКон, , КартаЛояльности = &КартаЛояльности) КАК ПродажиПоДисконтнымКартамОбороты";
			МРезультатов = Запрос.ВыполнитьПакет();
...
Функция ПолучитьПаспортныеДанныеСтрокой(СтруктураДанных) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураДанных.ФизЛицо) Тогда
		Возврат НСтр("ka='პირადობის დამადასტურებელი მონაცემები არ არის.';ru='Отсутствуют данные об удостоверении личности.';en='There are no data on the identity card.'");
	КонецЕсли; 
	...
	Иначе
		...
		Если НЕ (НЕ ЗначениеЗаполнено(ДатаВыдачи)
			...
			СтрокаПаспортныеДанные = НСтр("ka='%ВидДокумента% სერია: %Серия%, № %Номер%, გაცემულია; %ДатаВыдачи% წელს, %КемВыдан%; № ქვეგანყ. %НомерПодразделения%';ru='%ВидДокумента% Серия: %Серия%, № %Номер%, Выдан: %ДатаВыдачи% года, %КемВыдан%; № подр. %НомерПодразделения%';en='VidDokumenta%% Series: Series%% number% number%, Issued:% DataVydachi% year% KemVydan%; Number implied. % NomerPodrazdeleniya%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მარაგები საკმარისი არ არის %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';ru='Не хватает запасов %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток %КоличествоОстаток% %ЕдиницаИзмерения%,"
"недостаточно %Количество% %ЕдиницаИзмерения%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"ნაშთი %КоличествоОстаток% %ЕдиницаИзмерения%,"
"არ არის საკმარისი %Количество% %ЕдиницаИзмерения%'");
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		ПредставлениеНоменклатуры = ПредставлениеНоменклатуры(ВыборкаЗаписей.НоменклатураПредставление, ВыборкаЗаписей.ХарактеристикаПредставление, ВыборкаЗаписей.ПартияПредставление); // Артикул  + " არტიკულით: " + ВыборкаЗаписей.НоменклатураПредставление.Артикул
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладахСписком(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მარაგები საკმარისი არ არის';ru='Не хватает запасов'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მარაგების და დანახარჯების აღრიცხვის მიხედვით ნაშთი საკმარისი არ არის %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';ru='Не хватает остатка по учету запасов и затрат %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
...
// Процедура сообщает об ошибках проведения по регистру Запасы.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыОприходование(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	ШаблонСообщения = НСтр("en='Nomenclature:% NomenklaturaHarakteristikaPartiya%';ka='Номенклатура: %НоменклатураХарактеристикаПартия%';ru='Номенклатура: %НоменклатураХарактеристикаПартия%'");
...
// для списка стурктурных единиц.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыСписком(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მარაგების და დანახარჯების აღრიცხვის მიხედვით ნაშთი საკმარისი არ არის';ru='Не хватает остатка по учету запасов и затрат'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданные(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გარეშე კონტრაგენტისთვის გადაცემული მარაგები საკმარისი არ არის %КонтрагентПредставление%';ru='Не хватает запасов, переданных стороннему контрагенту %КонтрагентПредставление%'");
	ШаблонСообщения = НСтр("en='Nomenclature:% NomenklaturaHarakteristikaPartiya%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,';ru='Номенклатура: %НоменклатураХарактеристикаПартия%,'");
...
"недостаточно %Количество% %ЕдиницаИзмерения%';ka='ნაშთი %КоличествоОстаток% %ЕдиницаИзмерения%,"
"არ არის საკმარისი %Количество% %ЕдиницаИзмерения%'");
...
"недостаточно %Сумма% %Валюта%';ka='ანგარიშსწორებების თანხა:"
"ნაშთი %СуммаОстаток% %Валюта%,"
"არ არის საკმარისი %Сумма% %Валюта%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданныеСписком(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გარეშე კონტრაგენტისთვის გადაცემული მარაგები საკმარისი არ არის';ru='Не хватает запасов, переданных стороннему контрагенту'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"контрагент %КонтрагентПредставление%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"კონტრაგენტი %КонтрагентПредставление%'");
	ВалютаУчета = Константы.ВалютаУчета.Получить();
	Пока ВыборкаЗаписей.Следующий() Цикл
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გარეშე კონტრაგენტისგან მიღებული მარაგები საკმარისი არ არის %КонтрагентПредставление%';ru='Не хватает запасов, поступивших от стороннего контрагента %КонтрагентПредставление%'");
	ШаблонСообщения = НСтр("ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,';ru='Номенклатура: %НоменклатураХарактеристикаПартия%,';en='Nomenclature:% NomenklaturaHarakteristikaPartiya%'");
...
// Процедура сообщает об ошибках проведения по регистру Запасы принятые.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятыеСписком(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გარეშე კონტრაგენტისგან მიღებული მარაგები საკმარისი არ არის';ru='Не хватает запасов, поступивших от стороннего контрагента'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"контрагент %КонтрагентПредставление%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"კონტრაგენტი %КонтрагентПредставление%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыВРазрезеГТД(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='მარაგები საკმარისი არ არის საბაჟო დეკლარაციების ჭრილში';ru='Не хватает запасов в разрезе ГТД'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыПокупателей(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გაფორმებულია მეტი, ვიდრე მითითებულია მყიდველის შეკვეთაში';ru='Оформлено больше, чем указано в заказе покупателя'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ, МассивСообщений);
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%. %ЗаказПокупателя%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"ნაშთი შეკვეთის მიხედვით %КоличествоОстаток% %ЕдиницаИзмерения%,"
"აღემატება %Количество% %ЕдиницаИзмерения%-ით. %ЗаказПокупателя%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыПоставщикам(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გაფორმებულია მეტი, ვიდრე მომწოდებლისადმი შეკვეთასი არის მითითებული';ru='Оформлено больше, чем указано в заказе поставщику'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%"
"%ЗаказПоставщику%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"ნაშთი შეკვეთის მიხედვით %КоличествоОстаток% %ЕдиницаИзмерения%,"
"აღემატება %Количество% -ით %ЕдиницаИзмерения%"
"%ЗаказПоставщику%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыНаПроизводство(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გაფორმებულია მეტი, ვიდრე საწარმოო შეკვეთაში არის მითითებული';ru='Оформлено больше, чем указано в заказе на производство'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%"
"%ЗаказНаПроизводство%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"ნაშთი შეკვეთის მიხედვით %КоличествоОстаток% %ЕдиницаИзмерения%,"
"აღემატება %Количество% -ით %ЕдиницаИзмерения%"
"%ЗаказНаПроизводство%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструПотребностьВЗапасах(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გაფორმებულია მეტი, ვიდრე არის მარაგებში მოთხოვნილება';ru='Оформлено больше, чем есть потребность в запасах'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"потребность %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%"
"%ЗаказПокупателя%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"მოთხოვნილება %КоличествоОстаток% %ЕдиницаИзмерения%,"
"აღემატება %Количество% -ით %ЕдиницаИзмерения%"
"%ЗаказПокупателя%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструРазмещениеЗаказов(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='გაფორმებულია მეტი, ვიდრე შეკვეთებში განთავსებულია მარაგები';ru='Оформлено больше, чем размещено запасов в заказах'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
...
"превышает на %Количество% %ЕдиницаИзмерения%"
"по %ЗаказПокупателя%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"განთავსებული %КоличествоОстаток% %ЕдиницаИзмерения%"
"%ИсточникОбеспечения% -ში,"
"აღემატება %Количество% -ით %ЕдиницаИзмерения%"
"по %ЗаказПокупателя%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструДенежныеСредства(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='ფულადი სახსრები საკმარისი არ არის';ru='Не хватает денежных средств'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ka='%СчетКасса%: %ПредставлениеСчетаКассы%,"
"ნაშთი %СуммаОстаток% %Валюта%,"
"%Сумма% %Валюта% საკმარისი არ არის';ru='%СчетКасса%: %ПредставлениеСчетаКассы%,"
"остаток %СуммаОстаток% %Валюта%,"
...
// Процедура сообщает об ошибках проведения по регистру Денежные средства.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструОстаткиНалоговыхЛьгот(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='ფულადი სახსრები საკმარისი არ არის';ru='Не хватает остатков по льготам'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ka='%Сотрудник%: %ПредставлениеСотрудник%,"
"ნაშთი %СуммаОстаток%';ru='%Сотрудник%: %ПредставлениеСотрудник%,"
"остаток %СуммаОстаток%'");
		
	Пока ВыборкаЗаписей.Следующий() Цикл
		
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%ПредставлениеСотрудник%", ВыборкаЗаписей.СотрудникПредставление);
		
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сотрудник%", НСтр("ka='თანამშრომელი';ru='Сотрудник';en='Employee:'"));
			
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаОстаток%", Строка(-ВыборкаЗаписей.СуммаОстаток));
		
		СообщитьОбОшибке(ДокОбъект, ТекстСообщения, , , , Отказ);
		
	КонецЦикла;
	
КонецПроцедуры // СообщитьОбОшибкахПроведенияПоРегиструОстаткиНалоговыхЛьгот()

// Процедура сообщает об ошибках проведения по регистру Денежные средства в кассах ККМ.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструДенежныеСредстваВКассахККМ(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='სსა სალაროში ფულადი სახსრები აკლია';ru='Не хватает денежных средств в кассе ККМ'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ka='სსა სალარო: %ПредставлениеКассыККМ%,"
"ნაშთი %СуммаОстаток% %Валюта%,"
"არასაკმარისია %Сумма% %Валюта%';ru='Касса ККМ: %ПредставлениеКассыККМ%,"
"остаток %СуммаОстаток% %Валюта%,"
"недостаточно %Сумма% %Валюта%'"
	);
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПодотчетниками(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='ანგარიშსწორებები საქვეანგარიშო პირთან უარყოფითი გახდება';ru='Расчеты с подотчетным лицом станут отрицательными'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ka='%ПредставлениеПодотчетногоЛица%,"
"ანგარიშვალდებული პირისთვის გაცემული ფულადი სახსრების ნაშთი: %ОстатокУПодотчетногоЛица% %ВалютаПредставление%';ru='%ПредставлениеПодотчетногоЛица%,"
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПоставщиками(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.ДвиженияДенежныхДокументов Тогда
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - მომწოდებლის წინაშე დავალიანების ნაშთი ანგარიშსწორებების დოკუმენტის მიხედვით გადახდილ თანხაზე ნაკლებია."
"გადახდის დატარებული თანხა: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"დავალიანების ნაშთი მომწოდებლის წინაშე: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - остаток задолженности перед поставщиком по документу расчетов меньше оплаченной суммы."
...
"Остаток задолженности перед поставщиком: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				Если ВыборкаЗаписей.СуммаНепогашенныхАвансов = 0 Тогда
					ТекстСообщения = НСтр("en='% PredstavlenieKontragenta% - the document issued in advances to suppliers are already fully utilized in commercial documents.';ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით გაცემული ავანსები მიმწოდებლისადმი უკვე მთლიანად ჩათვლილია სასაქონლო დოკუმენტებში.';ru='%ПредставлениеКонтрагента% - выданные по документу авансы поставщику уже полностью зачтены в товарных документах.'"
					);
				Иначе
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით გაცემული ავანსები მიმწოდებლისადმი უკვე ნაწილობრივ ჩათვლილია სასაქონლო დოკუმენტებში."
"ჩაუთვლელი ავანსების ნაშთი: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - выданные по документу авансы поставщику уже частично зачтены в товарных документах."
		...
		Иначе
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				Если ВыборкаЗаписей.СуммаНепогашеннойЗадолженности = 0 Тогда
					ТекстСообщения = НСтр("en='% PredstavlenieKontragenta% - payable to suppliers have been paid in full through the document.';ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მომწოდებლისადმი დავალიანება უკვე მთლიანად გადახდილია.';ru='%ПредставлениеКонтрагента% - задолженность перед поставщиком по документу уже полностью оплачена.'"
					);
				Иначе
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მომწოდებლისადმი დავალიანება უკვე ნაწილობრივ გადახდილია."
"დავალიანების დაუფარავი თანხის ნაშთი: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - задолженность перед поставщиком по документу уже частично оплачена."
"Остаток непогашенной суммы задолженности: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.'"
			...
			КонецЕсли;
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - ავანსების ჩასათვლელი თანხა არ შეიძლება აჭარბებდეს მომწოდებლისათვის გაცემული ავანსების ნაშთს."
"ჩასათვლელი თანხა: %СуммаВалПриЗаписи% %ВалютаПредставление%"
"გაცემული ავანსების ნაშთი: %СуммаВыданныхАвансов% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - зачитываемая сумма авансов не может быть больше остатка выданных авансов поставщику."
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПокупателями(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.ДвиженияДенежныхДокументов Тогда
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - მყიდველის დავალიანების ნაშთი ანგარიშსწორებების დოკუმენტის მიხედვით გადახდის დატარებულ თანხაზე ნაკლებია."
"გადახდის დატარებული თანხა: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"მყიდველის დავალიანების ნაშთი: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - остаток задолженность покупателя по документу расчетов меньше разнесенной суммы платежа."
"Разнесенная сумма платежа: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Остаток задолженности покупателя: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				Если ВыборкаЗаписей.СуммаНепогашенныхАвансов = 0 Тогда
					ТекстСообщения = НСтр("en='% PredstavlenieKontragenta% - advances received the document from the buyer is already fully utilized in commercial documents.';ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მყიდველისგან მიღებული ავანსები უკვე მთლიანად ჩათვლილია სასაქონლო დოკუმენტებში.';ru='%ПредставлениеКонтрагента% - полученные по документу авансы от покупателя уже полностью зачтены в товарных документах.'"
					);
				Иначе
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მყიდველისგან მიღებული ავანსები უკვე ნაწილობრივ ჩათვლილია სასაქონლო დოკუმენტებში."
"ჩაუთვლელი ავანსების ნაშთი: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - полученные по документу авансы от покупателя уже частично зачтены в товарных документах."
"Остаток незачтенных авансов: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.'"
					);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаНепогашенныхАвансов%", Строка(ВыборкаЗаписей.СуммаНепогашенныхАвансов));
				КонецЕсли;
		...
		Иначе
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				Если ВыборкаЗаписей.СуммаНепогашеннойЗадолженности = 0 Тогда
					ТекстСообщения = НСтр("en='% PredstavlenieKontragenta% - payable by the buyer to the document has already been paid in full.';ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მყიდველის დავალიანება უკვე მთლიანად გადახდილია.';ru='%ПредставлениеКонтрагента% - задолженность покупателя по документу уже полностью оплачена.'"
					);
				Иначе
					ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - დოკუმენტის მიხედვით მყიდველის დავალიანება უკვე ნაწილობრივ გადახდილია."
"დავალიანების დაუფარავი თანხის ნაშთი: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - задолженность покупателя по документу уже частично оплачена."
"Остаток непогашенной суммы задолженности: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.'"
					);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаНепогашеннойЗадолженности%", Строка(ВыборкаЗаписей.СуммаНепогашеннойЗадолженности));
				КонецЕсли;
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - ავანსების ჩასათვლელი თანხა არ შეიძლება მყიდველისგან მიღებული ავანსების ნაშთზე მეტი იყოს."
"ჩასათვლელი თანხა: %СуммаВалПриЗаписи% %ВалютаПредставление%"
"მიღებული ავანსების ნაშთი: %СуммаПолученныхАвансов% %ВалютаПредставление%.';ru='%ПредставлениеКонтрагента% - зачитываемая сумма авансов не может быть больше остатка полученных авансов от покупателя."
"Зачитываемая сумма: %СуммаВалПриЗаписи% %ВалютаПредставление%"
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструВнеоборотныеАктивы(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='შეიძლება ქონება უკვე ჩამოწერილია ან გადაცემულია';ru='Возможно имущество уже списано или передано'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		
	ШаблонСообщения = НСтр("ka='ქონება: %НоменклатураХарактеристикаПартия%,¶ქონების ნარჩენი ღირებულება: %Стоимость%';ru='Имущество: %НоменклатураХарактеристикаПартия%,"
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструСуммовойУчетВРознице(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='საცალო წერტილის დავალიანება %СтруктурнаяЕдиницаПредставление% უკვე დაფარულია';ru='Задолженность розничной точки %СтруктурнаяЕдиницаПредставление% уже погашена'");
	
	ШаблонСообщения = НСтр("ka='დავალიანების ნაშთი (თანხობრივი აღრიცხვა): %ОстатокВРознице% %ВалютаПредставление%';ru='Остаток задолженности (суммовой учет): %ОстатокВРознице% %ВалютаПредставление%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомера(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='სერიული ნომრები არ არის საკმარისი %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';ru='Не хватает серийных номеров %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"серийный номер %СерийныйНомер%';ka='ნომენკლატურა: %НоменклатураХарактеристикаПартия%,"
"სერიული ნომერი %СерийныйНомер%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыВРемонте(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='არ არის %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';ru='Нет в наличии %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("en='Nomenclature:% NomenklaturaHarakteristika%';ka='ნომენკლატურა: %НоменклатураХарактеристика%';ru='Номенклатура: %НоменклатураХарактеристика%'");
...
// Процедура сообщает об ошибках проведения по регистру Товары в ремонте.
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыПереданныеВРемонте(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='В сервисном центре %СервисныйЦентрПредставление% нет в наличии ';ka='სერვის-ცენტრ %СервисныйЦентрПредставление%-ში არ არის'");
	
	ШаблонСообщения = НСтр("ka='ნომენკლატურა: %НоменклатураХарактеристика%';ru='Номенклатура: %НоменклатураХарактеристика%';en='Nomenclature:% NomenklaturaHarakteristika%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЦеныВозвратаНоменклатурDgFIFO(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	
	ЗаголовокОшибки = ?(ПоИтогамРегистра, НСтр("ka='შეცდომა, რეესტრის შედეგების საფუძველზე:';ru='Ошибка, по итогам регистра:'"), НСтр("ka='შეცდომა:';ru='Ошибка:';en='Mistake:'"));
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'უკან დასაბრუნებელი მარაგების რაოდენობა აღემატება რეალიზაციის რაოდენობას  '; ka = 'უკან დასაბრუნებელი მარაგების რაოდენობა აღემატება რეალიზაციის რაოდენობას  '");
...
"არ არის საკმარისი %Количество% %ЕдиницаИзмерения%'");
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыводитьВЗаголовокРасшифровку Тогда
			...
			//ТекстЗаголовкаСообщения = СтрЗаменить(ШаблонЗаголовкаСообщения, "%СтруктурнаяЕдиницаПредставление%", ПредставлениеСтруктурнойЕдиницы);
			//ТекстЗаголовкаСообщения = СтрЗаменить(ТекстЗаголовкаСообщения, "%ТипСтруктурнойЕдиницы%", ПолучитьПредставлениеТипаСтруктурныхЕдиниц(ВыборкаЗаписей.ТипСтруктурнойЕдиницы));
			СообщитьОбОшибке(ДокОбъект, ШаблонЗаголовкаСообщения, , , , Отказ, МассивСообщений);
			ВыводитьВЗаголовокРасшифровку = Ложь;
		КонецЕсли;
...
Функция РассчитатьЗначениеПараметра(СтруктураПараметров, ПараметрРасчета, ТекстОшибки = "") Экспорт
	...
	Для каждого ПараметрЗапроса Из ПараметрРасчета.ПараметрыЗапроса Цикл
		...
		Иначе
			...
			Иначе
				...
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = НСтр("ru='Не задано значение параметра запроса ';ka='მოთხოვნის პარამეტრის მნიშვნელობა მიცემული არ არის'") + ПараметрЗапроса.Имя + ТекстОшибки;
				Сообщение.Сообщить();
	...
	//+++ Дмгг
	ТекстДоИзменения = Запрос.Текст;
	Если СтруктураПараметров.Свойство("ДобавитьОтборПоСотруднику") И СтруктураПараметров.ДобавитьОтборПоСотруднику Тогда 
		
		Запрос.Текст = Запрос.Текст + СтруктураПараметров.ОтборСотрудникТекст;
...
Процедура ПолучитьСчетЗатратВидаНачисления(СтруктураДанных) Экспорт
	...
	СчетЗатратРегл = ВидНачисленияУдержания.СчетЗатратРегл;
	...
	ИначеЕсли ВидНачисленияУдержания.Тип = Перечисления.ТипыНачисленийИУдержаний.Удержание Тогда
		...
		//СчетЗатратРегл = ПланыСчетов.Хозрасчетный.Вспомогательный;
		
	КонецЕсли;
	
	СтруктураДанных.СчетЗатрат = СчетЗатрат;
	СтруктураДанных.ТипСчета = СчетЗатрат.ТипСчета;
	СтруктураДанных.СчетЗатратРегл = СчетЗатратРегл;
...
//                 получения.
//
Процедура ПолучитьСчетЗатратВидаНачисленияРегл(СтруктураДанных) Экспорт //+++ Дмгг
	...
	Если ВидНачисленияУдержания.Тип = Перечисления.ТипыНачисленийИУдержаний.Налог Тогда
		...
		Если СчетЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.Кредиторы Тогда
	...
	ИначеЕсли ВидНачисленияУдержания.Тип = Перечисления.ТипыНачисленийИУдержаний.Начисление Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктурнаяЕдиница) Тогда
			...
			ТипСчета = СчетЗатратРегл.ТипСчета;
			Если СтруктураДанных.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение
				И НЕ (ТипСчета = Перечисления.ТипыСчетов.КосвенныеЗатраты
					ИЛИ ТипСчета = Перечисления.ТипыСчетов.Расходы
					ИЛИ ТипСчета = Перечисления.ТипыСчетов.ПрочиеОборотныеАктивы
	...
	ИначеЕсли ВидНачисленияУдержания.Тип = Перечисления.ТипыНачисленийИУдержаний.Удержание Тогда
		...
		СчетЗатрат = ПланыСчетов.Управленческий.ПрочиеДоходы;
		СчетЗатратРегл = ВидНачисленияУдержания.СчетЗатратРегл;//ПланыСчетов.Хозрасчетный.Вспомогательный;
		
	КонецЕсли;
	
	СтруктураДанных.СчетЗатрат = СчетЗатрат;
	СтруктураДанных.ТипСчета = СчетЗатратРегл.ТипСчета;
	СтруктураДанных.СчетЗатратРегл = СчетЗатратРегл;
...
Процедура СформироватьТаблицуПроводок(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	ТаблицаХозрасчетный = Новый ТаблицаЗначений;
	
	ТаблицаХозрасчетный.Колонки.Добавить("НомерСтроки");
	ТаблицаХозрасчетный.Колонки.Добавить("Период");
	ТаблицаХозрасчетный.Колонки.Добавить("Организация");
	ТаблицаХозрасчетный.Колонки.Добавить("СчетДт");
	ТаблицаХозрасчетный.Колонки.Добавить("ВалютаДт");
	ТаблицаХозрасчетный.Колонки.Добавить("СуммаВалДт");
	ТаблицаХозрасчетный.Колонки.Добавить("СчетКт");
	ТаблицаХозрасчетный.Колонки.Добавить("ВалютаКт");
	ТаблицаХозрасчетный.Колонки.Добавить("СуммаВалКт");
	ТаблицаХозрасчетный.Колонки.Добавить("Сумма");
	ТаблицаХозрасчетный.Колонки.Добавить("КоличествоДт");
	ТаблицаХозрасчетный.Колонки.Добавить("КоличествоКт");
	ТаблицаХозрасчетный.Колонки.Добавить("Содержание");
	ТаблицаХозрасчетный.Колонки.Добавить("СубконтоДт1");
	ТаблицаХозрасчетный.Колонки.Добавить("СубконтоДт2");
	ТаблицаХозрасчетный.Колонки.Добавить("СубконтоДт3");
	ТаблицаХозрасчетный.Колонки.Добавить("ВидСубконтоДт1", Новый ОписаниеТипов("Строка"));
	ТаблицаХозрасчетный.Колонки.Добавить("ВидСубконтоДт2", Новый ОписаниеТипов("Строка"));
	ТаблицаХозрасчетный.Колонки.Добавить("ВидСубконтоДт3", Новый ОписаниеТипов("Строка"));
	ТаблицаХозрасчетный.Колонки.Добавить("СубконтоКт1");
	ТаблицаХозрасчетный.Колонки.Добавить("СубконтоКт2");
	ТаблицаХозрасчетный.Колонки.Добавить("СубконтоКт3");
	ТаблицаХозрасчетный.Колонки.Добавить("ВидСубконтоКт1", Новый ОписаниеТипов("Строка"));
	ТаблицаХозрасчетный.Колонки.Добавить("ВидСубконтоКт2", Новый ОписаниеТипов("Строка"));
	ТаблицаХозрасчетный.Колонки.Добавить("ВидСубконтоКт3", Новый ОписаниеТипов("Строка"));
		...
		Пока ВыборкаНоменклатуры.Следующий() Цикл
			...
			Если СтрокиРекурсивногоВхождения.Количество() <> 0 Тогда
				...
				Для каждого СтрокаРекВх Из СтрокиРекурсивногоВхождения Цикл
					...
					НСтр("ru='Обнаружено рекурсивное вхождение элемента %1 в элемент %2!';en='Detected recursive element occurrence% 1 to% 2 element!'"),
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	СоответствиеПараметров.Вставить("ИНН",						" ს/კ ");
	...
	Для Счетчик = 1 по ЧислоПараметров Цикл
		...
		Если ПозЗапятой > 0  Тогда
			...
			Исключение
				...
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = НСтр("ru='Не удалось определить значение параметра организации: ';ka='ვერ მოხერხდა ორგანიზაციის პარამეტრის მნიშვნელობის დადგენა'") + ИмяПараметра;
				Сообщение.Сообщить();
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	Если НЕ (ЮрФизЛицо.Метаданные().Реквизиты.Найти("ЮридическоеФизическоеЛицо") = Неопределено) Тогда
		...
		ИначеЕсли ИмяСправочника = "Организации" Тогда
			...
			Сведения.Свидетельство = НСтр("ka='მოწმობა';ru='свидетельство';en='certificate'") + " " + Данные.СвидетельствоСерияНомер + " " + НСтр("ka='(საიდან, ვისგან)';ru='от';en='From'") + " " + Формат(Данные.СвидетельствоДатаВыдачи, "ДЛФ=D");
	...
	Если Данные <> Неопределено Тогда
		...
		Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") Тогда
			...
			Телефон 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "ТелефонОрганизации");
			Факс 		= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "ФаксОрганизации");
			ЮрАдрес		= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "ЮрАдресОрганизации");
			ФактАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "ФактАдресОрганизации");
			ПочтАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "ПочтовыйАдресОрганизации");
			ЭлПочта 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "EmailОрганизации");
			
		ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			ПустаяКИ = УправлениеНебольшойФирмойПовтИсп.ПолучитьПустуюСсылкуСправочника("ВидыКонтактнойИнформации");
			Телефон 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"ТелефонФизЛица");
			Факс 		= ПустаяКИ;
			ЮрАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"АдресФизЛицаПоПрописке");
			ФактАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"АдресФизЛицаПоПрописке");
			ПочтАдрес 	= ПустаяКИ;
			ЭлПочта 	= Неопределено;
			
		ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
			
			Телефон 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"ТелефонКонтрагента");
			Факс 		= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"ФаксКонтрагента");
			ЮрАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"ЮрАдресКонтрагента");
			ФактАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"ФактАдресКонтрагента");
			ПочтАдрес 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"ПочтовыйАдресКонтрагента");
			ЭлПочта 	= УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации",	"EmailКонтрагента");
...
Функция ПроверитьВыводТабличногоДокумента(ТабДокумент, ВыводимыеОбласти, РезультатПриОшибке = Истина)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСТр("ka='მიმდინარე პრინტერის შესახებ ინფორმაციის მიღება შეუძლებელია (შეიძლება სისტემაში არცერთი პრინტერი არ არის დაყენებული)';ru='Невозможно получить информацию о текущем принтере (возможно, в системе не установлено ни одного принтера)';en='Unable to get information about the current printer (the system may not set any printer)'"),
...
"უნივერსალური გადასაცემი დოკუმენტის ბეჭდვა ხელმისაწვდომია 2013 წლის 1 იანვრიდან. "
"დოკუმენტ %1-ისთვის ბეჭდური ფორმა არ არის შედგენილი.';ru='__________________"
...
"%1, "
"т.к. действие доступно только для заказ-нарядов!';ka='უნივერსალური გადასაცემი დოკუმენტ %1-ის ამობეჭდვა არ შეიძლება, "
"ვინაიდან მოქმედება ხელმისაწვდომია მხოლოდ სერვისის დოკუმენტებისთვის!'");
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ДокументСсылка.Контрагент) Тогда 
			...
			ТекстСообщения = Нстр("ka='__________________"
"უნივერსალური გადასაცემი დოკუმენტის ამოსაბეჭდად საჭიროა კონტრაგენტის შევსება. "
"დოკუმენტ %1-ისთვის ბეჭდური ფორმა არ არის შედგენილი.';ru='__________________"
...
Процедура ПроверитьВозможностьПечатиУКД(ПараметрКоманды, Ошибки) Экспорт
	...
	Пока Счетчик <= ПараметрКоманды.Количество()-1 Цикл
		...
		Если ДокументСсылка.Дата < Дата('20130101') Тогда 
			...
			ТекстСообщения = Нстр("ka='__________________"
"უნივერსალური საკორექტირებო დოკუმენტის ბეჭდვა ხელმისაწვდომია 2013 წლის 1 იანვრიდან. "
"დოკუმენტ %1-ისთვის ბეჭდური ფორმა არ არის შედგენილი.';ru='__________________"
...
"Для документа %1 печатная форма не сформирована.'");
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ДокументСсылка.Контрагент) Тогда 
			...
			ТекстСообщения = Нстр("ka='__________________"
"უნივერსალური საკორექტირებო დოკუმენტის ამოსაბეჭდად საჭიროა კონტრაგენტის შევსება. "
"დოკუმენტ %1-ისთვის ბეჭდური ფორმა არ არის ჩამოყალიბებული.';ru='__________________"
...
Процедура УстановитьТекстПроСчетФактуру(ФормаДокумента, Полученный = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(НайденныйСчетФактура) Тогда
		ТекстПроСчетФактуру = НСтр("ka='№ %Номер% %Дата% წლიდან';ru='№ %Номер% от %Дата% г.';en='Number% number% of% of% Date'");
...
Функция ПроверитьВыпискуСчетовФактур(ИсточникиПечати, ДокументРеализации, СообщенияПользователю, ВидОперации = Неопределено)
	...
	Если СчетФактураВыданный = Неопределено Тогда
		...
		Попытка 
			...
			ТекстСообщения = НСтр("ka='შედგენილია დაქვემდებარებული დოკუმენტი %2-ის ანგარიშფაქტურა # %1.';ru='Сформирован подчиненный документ Cчет-фактура № %1 от %2.';en='Formed subordinate document Account Invoice №% from 1% 2.'");
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось сформировать подчиненный документ Счет-фактура для документа %1. Для формирования счёт-фактуры нажмите на гиперссылку ""Создать"" внизу.';en='It failed to generate a subordinate document Invoice to document% 1. For the formation of the invoice, click on the hyperlink ""New"" at the bottom.'");
...
Процедура УстановитьСтандартныеНастройкиПодбора(ТекущийПользователь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекущийПользователь) Тогда
		...
		Нстр("ka='მიუთითებელია მომხმარებელი, რომლისთვის ხდება გადარჩევის გამართვების დაყენება სტანდარტულად.';ru='Неуказан пользователь, для которого устанавливаются настройки подбора по умолчанию.';en='Unspecified user who is set to the default settings selection.'")
		);
...
Процедура ЗаполнитьТемуТекстОтправки(ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм)
	...
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2 %3';ru='%1 %2 %3';en='% 1% 2% 3'"),
		ПредставлениеЗаголовкаДокумента,
		ПредставлениеДляКого,
		ПредставлениеОтКого
		);
...
//+++ Дмгг
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщиками_КорректировкаДолга(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса) Экспорт
	...
	Если НЕ РассчитыватьКурсовыеРазницы Тогда
		...
		//|		КОГДА ТаблицаДокумента.ПризнакАванса И ТаблицаДокумента.Ссылка.ДокументРасчетов = Неопределено
		////|			И (ТаблицаДокумента.Документ ССЫЛКА Документ.РасходСоСчета ИЛИ ТаблицаДокумента.Документ ССЫЛКА Документ.РасходИзКассы)
		//|			ТОГДА ТаблицаДокумента.ВидДвижения
		//|		ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|		ИНАЧЕ ТаблицаДокумента.ВидДвижения
		...
		//+++ Дмгг
		|ГДЕ
		|   ТаблицаДокумента.Контрагент = &Получатель
		|   И &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.ПереуступкаДолгаПоставщику)
		|   И ТаблицаДокумента.Договор = &ДоговорПолучатель
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаРасчетыСПоставщиками КАК ТаблицаДокумента
		|ГДЕ
		|   ТаблицаДокумента.Контрагент = &Отправитель
		|   И &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.ПереуступкаДолгаПоставщику)
		|   И ТаблицаДокумента.Договор <> &ДоговорПолучатель
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|	ВременнаяТаблицаРасчетыСПоставщиками КАК ТаблицаДокумента
		|ГДЕ
		|   &ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.ПереуступкаДолгаПоставщику)
		//--- Дмгг
		|
		|УПОРЯДОЧИТЬ ПО
		|	ТаблицаДокумента.СодержаниеПроводки,
...
КонецФункции // ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщиками_КорректировкаДолга()
//--- Дмгг

// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщикамиРегл(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса, ЭтоВозвратПоставщика = Ложь) Экспорт
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателями(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса) Экспорт
	...
	ИначеЕсли СЗачетомАванса Тогда
		...
		|	ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) * КурсыВалютРасчетовСрезПоследних.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютРасчетовСрезПоследних.Кратность) - ЕСТЬNULL(ТаблицаОстатки.СуммаОстаток, 0) КАК СуммаКурсовойРазницы,
		|	ТаблицаРасчеты.Валюта КАК Валюта,
		|	ТаблицаРасчеты.СчетУчетаРел КАК СчетУчета
		//+++ Дмгг
		|,
		|	ТаблицаРасчеты.Проект
		//--- Дмгг
		...
		|	ТаблицаКурсовыхРазниц.ТипРасчетов,
		|	ТаблицаКурсовыхРазниц.Валюта,
		|	ТаблицаКурсовыхРазниц.СчетУчетаРегл
		//+++ Дмгг
		|,
		|	ТаблицаКурсовыхРазниц.Проект
		//--- Дмгг
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателямиРегл(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса) Экспорт
	...
	ИначеЕсли СЗачетомАванса Тогда
		...
		//+++ Дмгг
		|,
		|		ТаблицаОстатки.Проект КАК Проект
		//--- Дмгг
		|	ИЗ
		|		РегистрНакопления.РасчетыСПокупателями.Остатки(
		|				&МоментВремени,
		...
		//+++ Дмгг
		|,
		|		ДвиженияДокумента.Проект КАК Проект
		//--- Дмгг
		|	ИЗ
		|		РегистрНакопления.РасчетыСПокупателями КАК ДвиженияДокумента
		|	ГДЕ
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыДенежныеСредства(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|				(Организация, ТипДенежныхСредств, БанковскийСчетКасса, Валюта, Проект) В
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		//+++ Дмгг
		|	ТаблицаДенежныеСредства.Проект
		//--- Дмгг
		...
		//+++ Дмгг
		|			И ТаблицаДенежныеСредства.Проект = ТаблицаОстатки.Проект
		//--- Дмгг
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
		|				&МоментВремени,
		|				Валюта В
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыДенежныеСредстваРегл(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	ДеньгиОстатки.Валюта КАК Валюта,
		//+++ Дмгг
		|	ДеньгиОстатки.Проект КАК Проект,
		//--- Дмгг
		|	ДеньгиОстатки.БанковскийСчетКасса.СчетУчетаРегл КАК СчетУчета,
		|	СУММА(ДеньгиОстатки.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(ДеньгиОстатки.СуммаВалОстаток) КАК СуммаВалОстаток
		|ПОМЕСТИТЬ ВременнаяТаблицаОстаткиПослеПроведения
		|ИЗ
		...
		|	ДеньгиОстатки.Проект,
		//--- Дмгг
		|	ДеньгиОстатки.БанковскийСчетКасса.СчетУчетаРегл
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	ТипДенежныхСредств,
		...
		|	ТаблицаДенежныеСредства.Валюта КАК Валюта,
		//+++ Дмгг
		|	ТаблицаДенежныеСредства.Проект КАК Проект,
		//--- Дмгг
		|	ТаблицаДенежныеСредства.СчетУчетаРегл КАК СчетУчета
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницДенежныеСредстваРегл
		|ИЗ
		|	ВременнаяТаблицаДенежныеСредства КАК ТаблицаДенежныеСредства
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаДенежныеСредства.Организация = ТаблицаОстатки.Организация
		...
		|	КОНЕЦ
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницДенежныеСредстваРегл КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	СодержаниеПроводки,
		|	ВидДвижения
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыДенежныеСредстваВКассахККМРегл(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	ДеньгиОстатки.Организация КАК Организация,
		|	ДеньгиОстатки.КассаККМ КАК КассаККМ,
		|	ДеньгиОстатки.КассаККМ.СчетУчетаРегл КАК СчетУчета,
		|	ДеньгиОстатки.Валюта КАК Валюта,
		|	СУММА(ДеньгиОстатки.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(ДеньгиОстатки.СуммаВалОстаток) КАК СуммаВалОстаток
		|ПОМЕСТИТЬ ВременнаяТаблицаОстаткиПослеПроведения
		...
		|	ТаблицаДенежныеСредства.Валюта КАК Валюта,
		|	ТаблицаДенежныеСредства.СчетУчетаРегл КАК СчетУчета
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницДенежныеСредстваВКассахККМРегл
		|ИЗ
		|	ВременнаяТаблицаДенежныеСредстваВКассахККМ КАК ТаблицаДенежныеСредства
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаДенежныеСредства.Организация = ТаблицаОстатки.Организация
		...
		|	&КурсоваяРазница
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницДенежныеСредстваВКассахККМРегл КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	СодержаниеПроводки,
		|	ВидДвижения
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПодотчетниками(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	СУММА(РасчетыОстатки.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(РасчетыОстатки.СуммаВалОстаток) КАК СуммаВалОстаток,
		//ПодольскийАИ #УчетПроектов 25.04.2021 {
		//Комментируем Проект
		//+++ Дмгг
		//|	РасчетыОстатки.Проект КАК Проект
		//--- Дмгг
		...
		//|				(Организация, Сотрудник, Валюта, Документ, Проект) В
		//ПодольскийАИ #УчетПроектов 25.04.2021 }
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПодотчетникамиРегл(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	РасчетыОстатки.Валюта КАК Валюта,
		|	РасчетыОстатки.Документ КАК Документ,
		|	РасчетыОстатки.Сотрудник.СчетРасчетовСПодотчетникамиРегл КАК СчетУчета,
		//ПодольскийАИ #УчетПроектов 25.04.2021 {
		//Комментируем Проект
		//+++ Дмгг
		//|	РасчетыОстатки.Проект КАК Проект,
		...
		|	ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка),
		//ПодольскийАИ #УчетПроектов 25.04.2021 }
		|	РасчетыОстатки.Сотрудник.СчетРасчетовСПодотчетникамиРегл
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Сотрудник,
		...
		|	ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка) КАК Проект
		//ПодольскийАИ #УчетПроектов 25.04.2021 }
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПодотчетникамиРегл
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПодотчетниками КАК ТаблицаРасчеты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаРасчеты.Организация = ТаблицаОстатки.Организация
		...
		//ПодольскийАИ #УчетПроектов 25.04.2021 }
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницРасчетыСПодотчетникамиРегл КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	НомерСтроки
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПерсоналом(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|				(Организация, СтруктурнаяЕдиница, Сотрудник, Валюта, ПериодРегистрации, Проект) В
		//--- Дмгг
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ТаблицаДокумента.СчетУчета КАК СчетУчета,
		//Паганини#Удержания_
	    |	ТаблицаДокумента.ВедомостьВидОперации,
		//_Паганини#Удержания
		|	ТаблицаДокумента.СодержаниеПроводки КАК СодержаниеПроводки,
		//+++ Дмгг
		|   ТаблицаДокумента.Проект КАК Проект
		...
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ТаблицаДокумента.ВедомостьВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежнаяВедомость.УдержаниеДляКонтрагента)
		|				ТОГДА ЛОЖЬ //НЕ &СуммыНалоговУдержаниеДляКонтрагентаКОплатеПриНачислении
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
	 	//_Паганини#Удержания
		|
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПерсоналомРегл(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	РасчетыОстатки.Валюта КАК Валюта,
		|	РасчетыОстатки.ПериодРегистрации КАК ПериодРегистрации,
		|	РасчетыОстатки.Сотрудник.СчетРасчетовСПерсоналомРегл КАК СчетУчета,
		|	СУММА(РасчетыОстатки.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(РасчетыОстатки.СуммаВалОстаток) КАК СуммаВалОстаток,
		...
		|	ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) * КурсыВалютРасчетовСрезПоследних.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютРасчетовСрезПоследних.Кратность) - ЕСТЬNULL(ТаблицаОстатки.СуммаОстаток, 0) КАК СуммаКурсовойРазницы,
		|	ТаблицаРасчеты.СчетУчетаРегл КАК СчетУчета,
		|	ТаблицаРасчеты.Статья КАК Статья,
		//ПодольскийАИ #УчетПроектов 12.02.2021 {
		|   ТаблицаРасчеты.Проект КАК Проект
		//ПодольскийАИ #УчетПроектов 12.02.2021 }
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПерсоналомРегл
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПерсоналом КАК ТаблицаРасчеты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаРасчеты.Организация = ТаблицаОстатки.Организация
	...
	Иначе
		...
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ТаблицаДокумента.ВедомостьВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежнаяВедомость.УдержаниеДляКонтрагента)
	 	//_Паганини#Удержания
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыСуммовойУчетВРозницеРегл(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|		ВременнаяТаблица.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|		ВременнаяТаблица.Валюта КАК Валюта,
		|		ВременнаяТаблица.СчетУчетаРегл КАК СчетУчета,
		|		ВременнаяТаблица.СуммаДляОстатка КАК СуммаОстаток,
		|		ВременнаяТаблица.СуммаВалДляОстатка КАК СуммаВалОстаток
		|	ИЗ
		|		ВременнаяТаблицаСуммовойУчетВРознице КАК ВременнаяТаблица
		...
		|	ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) * КурсыВалютКассаСрезПоследних.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютКассаСрезПоследних.Кратность) - ЕСТЬNULL(ТаблицаОстатки.СуммаОстаток, 0) КАК СуммаКурсовойРазницы,
		|	ТаблицаСуммовойУчетВРознице.Валюта КАК Валюта,
		|	ТаблицаСуммовойУчетВРознице.СчетУчетаРегл КАК СчетУчета
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницСуммовойУчетВРозницеРегл
		|ИЗ
		|	ВременнаяТаблицаСуммовойУчетВРознице КАК ТаблицаСуммовойУчетВРознице
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаСуммовойУчетВРознице.Организация = ТаблицаОстатки.Организация
		...
		|	&КурсоваяРазница
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницСуммовойУчетВРозницеРегл КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	НомерСтроки
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПрочимиКонтрагентами(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса) Экспорт
	...
	Если НЕ РассчитыватьКурсовыеРазницы Тогда
		...
		|	ТаблицаРасчеты.Валюта КАК Валюта,
		|	ТаблицаРасчеты.СчетУчета КАК СчетУчета,
		|	ТаблицаРасчеты.СчетУчета КАК СчетУчетаРегл
	...
	ИначеЕсли СЗачетомАванса Тогда
		...
		|			И ТаблицаРасчеты.Валюта = ТаблицаОстатки.ВалютаРасчетов
		|			И ТаблицаРасчеты.СчетУчета = ТаблицаОстатки.СчетУчета
		|			И ТаблицаРасчеты.СчетУчетаРегл = ТаблицаОстатки.СчетУчетаРегл
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
		|				&МоментВремени,
		|				Валюта В
		|					(ВЫБРАТЬ
		...
		|	ТаблицаКурсовыхРазниц.Валюта КАК Валюта,
		|	ТаблицаКурсовыхРазниц.СчетУчета КАК СчетУчета,
		|	ТаблицаКурсовыхРазниц.СчетУчета КАК СчетУчетаРегл
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПрочимиКонтрагентами
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаДокумента.Дата КАК Дата,
...
Функция ОчиститьДанныеВБазе() Экспорт
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение(НСтр("ka='ოპერაციის შესასრულებლად უფლებები არასაკმარისია';ru='Не достаточно прав для выполнения операции';en='Do not have sufficient rights to perform the operation'"));
	КонецЕсли;
	...
	Исключение
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მონოპოლიური რეჟიმის დაყენება ვერ მოხერხდა (%1)';ru='Не удалось установить монопольный режим (%1)';en='Failed to install single-user mode (% 1)'"),
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა წაშლა';ru='Удаление данных';en='Deleting data'"), 
...
Функция ПолучитьКалендарьПоПроизводственномуКалендарюРФ() Экспорт
	...
	Если ПроизводственныйКалендарьРФ = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='საქართველოს საწარმოო კალენდრის საფუძველზე ორგანიზაციებისთვის მუშაობის გრაფიკების მონაცემთა შევსება ვერ მოხერხდა.';ru='Неудалось заполнить данные граффиков работы для организации на основании производственного календаря РФ.';en='It failed to complete the data graffika work for the organization based on the factory calendar of the Russian Federation.'"), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//+++ Дмгг
Процедура ОтразитьОтчетКурсовыхРазниц(ДополнительныеСвойства, Движения, Отказ) Экспорт

	ТаблицаОтчетПоКурсовымРазницам = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаОтчетПоКурсовымРазницам;
	
	Если Отказ
	 ИЛИ ТаблицаОтчетПоКурсовымРазницам.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияРасчетыСПокупателями = Движения.ОтчетПоКурсовымРазницам;
	ДвиженияРасчетыСПокупателями.Записывать = Истина;
	ДвиженияРасчетыСПокупателями.Загрузить(ТаблицаОтчетПоКурсовымРазницам);
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыПоЭквайрингу(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Иначе
		ЗаголовокОшибки = НСтр("ka='Информация:';ru='Информация:';en='Information:'");
		...
		Иначе
			ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='საგადახდო ბარათებით ანგარიშსწორებებში არის ნაშთი';ru='Есть остаток в расчетах по платежным картам'");
		КОнецЕсли;
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.СуммаВалОстаток < 0 Или ВыборкаЗаписей.КомиссияВалОстаток < 0 Тогда
			Если ПоказатьСообщениеОбОшибки Тогда
				...
				ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ka='საგადახდო ბარათებით ანგარიშსწორების დაფიქსირების შესაძლებლობა არ არის';ru='Нет возможности зафиксировать расчеты по платежным картам';en='There is no way to fix calculations by payment cards'");
				СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
			КонецЕсли;
			
			ТекстСообщения = НСтр("ka='%ПредставлениеОперацииОплаты%"
"გადახდის გატარებული თანხა: %СуммаВалПриЗаписи% %ВалютаПредставление% (კომისია %КомиссияВалПриЗаписи%)."
"გადახდის ოპერაციის თანხის ნაშთი: %СуммаОстаткаЗадолженности% %ВалютаПредставление% (კომისია %СуммаОстаткаКомиссии%).';ru='%ПредставлениеОперацииОплаты%"
...
"Остаток суммы операции оплаты: %СуммаОстаткаЗадолженности% %ВалютаПредставление% (комиссия %СуммаОстаткаКомиссии%).'"
		...
		ИначеЕсли ВыборкаЗаписей.СуммаВалОстаток > 0 Или ВыборкаЗаписей.КомиссияВалОстаток > 0 Тогда
			...
			ТекстСообщения = НСтр("ka='%ПредставлениеОперацииОплаты%"
"გატარების შემდეგ, ბანკის ვალის ნაშთმა შეადგინა: %СуммаВалОстаток% %ВалютаПредставление%."
"გადახდის ოპერაციის თანხის ნაშთი: %КомиссияВалОстаток% %ВалютаПредставление%.';ru='%ПредставлениеОперацииОплаты%"
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструКонтрольОперацийЭквайринга(ДокОбъект, ВыборкаЗаписей, Отказ, МассивСообщений = Неопределено, ПоИтогамРегистра = ЛОЖЬ) Экспорт
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ka='%ВидПлатежнойКартыПредставление% %НомерПлатежнойКартыПредставление% (%ДатаПлатежаПредставление%) - ბარათის მიხედვით თანხის ნაშთი გადახდის გატარებულ თანხაზე ნაკლებია."
"გადახდის გატარებული თანხა: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"ბარათის მიხედვით თანხის ნაშთი: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';ru='%ВидПлатежнойКартыПредставление% %НомерПлатежнойКартыПредставление% (%ДатаПлатежаПредставление%) - остаток суммы по карте меньше разнесенной суммы платежа."
"Разнесенная сумма платежа: %СуммаВалПриЗаписи% %ВалютаПредставление%."
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоКредитамИЗаймам(МенеджерВременныхТаблиц, НомерЗапроса, ЕстьСтруктурнаяЕдиница = Ложь) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|				(Организация, Контрагент, ДоговорКредитаЗайма, ВидДоговора, Проект) В
		//--- Дмгг
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		//+++ Дмгг
		|	ВременнаяТаблицаКурсовыхРазниц.Проект
		//--- Дмгг
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоКредитамИЗаймам_НачислениеЗарплаты(МенеджерВременныхТаблиц, НомерЗапроса, ЕстьСтруктурнаяЕдиница = Ложь) Экспорт
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоКредитамИЗаймамРегл(МенеджерВременныхТаблиц, НомерЗапроса, ЕстьСтруктурнаяЕдиница = Ложь) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	ТаблицаРасчеты.Проект КАК Проект
		//--- Дмгг
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРегл
		|ИЗ
		|	ВременнаяТаблицаРасчетыПоКредитамИЗаймам КАК ТаблицаРасчеты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаРасчеты.Организация = ТаблицаОстатки.Организация
		...
		|	ВременнаяТаблицаКурсовыхРазниц.СуммаКурсовойРазницыОсновнойДолг КАК СуммаКурсовойРазницы,
		//+++ Дмгг
		|	ВременнаяТаблицаКурсовыхРазниц.Проект КАК Проект
		//--- Дмгг
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыПоКредитамИЗаймамРегл
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницРегл КАК ВременнаяТаблицаКурсовыхРазниц
		|ГДЕ
		|	(ВременнаяТаблицаКурсовыхРазниц.СуммаКурсовойРазницыОсновнойДолг >= 0.005
		|			ИЛИ ВременнаяТаблицаКурсовыхРазниц.СуммаКурсовойРазницыОсновнойДолг <= -0.005)
		|
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоКредитамИЗаймамРегл_НачислениеЗарплаты(МенеджерВременныхТаблиц, НомерЗапроса, ЕстьСтруктурнаяЕдиница = Ложь) Экспорт
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоПрочимОперациям(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Иначе
		...
		|				(Организация, Контрагент, Договор, СчетУчета, Проект) В
		//--- Дмгг
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		//+++ Дмгг
		|						ВременнаяТаблицаПрочиеРасчеты.Проект
		//--- Дмгг
		|					ИЗ
...
// Функция возвращает текст запроса для расчета курсовых разниц.
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыПоПрочимОперациямРегл(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если НЕ РассчитыватьКурсовыеРазницы Тогда
		...
		|	ТаблицаРасчеты.Проект КАК Проект
		//--- Дмгг
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницПрочиеРасчетыРегл
		|ИЗ
		|	ВременнаяТаблицаПрочиеРасчеты КАК ТаблицаРасчеты
		|ГДЕ
		|	ЛОЖЬ
	...
	Иначе
		...
		|	РасчетыОстатки.Договор КАК Договор,
		|	РасчетыОстатки.СчетУчета КАК СчетУчета,
		|	РасчетыОстатки.СчетУчетаРегл КАК СчетУчетаРегл,
		|	СУММА(РасчетыОстатки.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(РасчетыОстатки.СуммаВалОстаток) КАК СуммаВалОстаток,
		//+++ Дмгг
		|	РасчетыОстатки.Проект КАК Проект
		...
		|				&МоментВремени,
		//+++ Дмгг
		//|				(Организация, Контрагент, Договор, СчетУчета, СчетУчетаРегл) В
		|				(Организация, Контрагент, Договор, СчетУчета, СчетУчетаРегл, Проект) В
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|						ВременнаяТаблицаПрочиеРасчеты.Организация,
		|						ВременнаяТаблицаПрочиеРасчеты.Контрагент,
		|						ВременнаяТаблицаПрочиеРасчеты.Договор,
		|						ВременнаяТаблицаПрочиеРасчеты.СчетУчета,
		|						ВременнаяТаблицаПрочиеРасчеты.СчетУчетаРегл,
		//+++ Дмгг
		|						ВременнаяТаблицаПрочиеРасчеты.Проект
		//--- Дмгг
		|					ИЗ
		...
		|		ДвиженияДокумента.Договор,
		|		ДвиженияДокумента.СчетУчета,
		|		ДвиженияДокумента.СчетУчетаРегл,
		|		ВЫБОР
		|			КОГДА ДвиженияДокумента.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|				ТОГДА -ЕСТЬNULL(ДвиженияДокумента.Сумма, 0)
		|			ИНАЧЕ ЕСТЬNULL(ДвиженияДокумента.Сумма, 0)
		...
		//--- Дмгг
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницПрочиеРасчетыРегл КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	НомерСтроки
...
Функция ОтказСнятьФункциональнаяУчетВалютныхОпераций() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ka='ბაზაში შემოღებულია ეროვნულისგან განსხვავებული ვალუტები. გამართვის შეცვლა შეუძლებელია!';ru='В базе заведены валюты, отличные от национальной. Изменение настройки невозможно!';en='The base currency instituted other than national. settings can not be changed!'");
	КонецЕсли;
...
Функция ОтказСнятьИспользоватьОбменССайтом() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ka='პროგრამაში გამართულია და გამოიყენება საიტებთან გაცვლა! გასწორების გამორთვა შეუძლებელია!';ru='В программе настроен и используется обмен с сайтами! Отключение настройки невозможно!';en='The program is set up and used with the exchange sites! Disabling setting is not possible!'");
	КонецЕсли;
...
Функция ОтказИзменитьВалютаУчета() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстСообщения = НСтр("ka='ბაზაში არის მოძრაობები დაგროვების ""ჯამური"" რეგისტრებით! აღრიცხვის ვალუტის შეცვლა აკრძალულია!';ru='В базе есть движения по ""суммовым"" регистрам накопления! Изменение валюты учета запрещено!';en='At the base there is traffic on the ""accrual"" registers of accumulation! Change Currency account banned!'");	
...
Функция ОтказСнятиеУчетПоНесколькимСкладам() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ძირითადისგან განსხვავებული საწყობები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются склады, отличные от основного! Снятие опции запрещено!';en='The database used by warehouses, other than the main! Removing options forbidden!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетВРазличныхЕдиницахИзмерения() Экспорт
	...
	Пока ВыборкаЕдиницыИзмерения.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='დანართებში დოკუმენტები შეყვანილია სამომხმარებლო საზომ ერთეულებში! ოპციის მოხსნა აკრძალულია!';ru='В приложении введены документы в пользовательских единицах измерения! Снятие опции запрещено!';en='The application documents are entered in the user units of measurement! Removing options forbidden!'");
			Прервать;
...
Функция ОтказСнятьФункциональнаяОпцияПередачаЗапасовНаОтветхранение() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""გადაცემული მარაგები"" შეიცავს ინფორმაციას საპასუხისმგებლო შენახვაზე გადაცემის შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче на ответхранение! Снятие флага запрещено!';en='Register accumulation ""Inventories transferred"" contains information on the transfer of maintenance responsibility! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияПриемЗапасовНаОтветхранение() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""მიღებული მარაგები"" შეიცავს ინფორმაციას საპასუხისმგებლო შენახვაზე მიღების შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме на ответхранение! Снятие флага запрещено!';en='Register accumulation ""Inventories taken"" contains information about applying for a maintenance responsibility! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьОрдерныйСклад() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის საორდერო საწყობები! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют ордерные склады! Снятие флага запрещено!';en='The database contains a warrant warehouses! Removing forbidden flag!'");
	...
	Если НЕ РезультатЗапросаЗапасыКПоступлениюНаСклады.Пустой()
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='ბაზაში არის მოძრაობები საორდერო საწყობის მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют движения по ордерному складу! Снятие флага запрещено!';en='The database contains the motion for an order stock! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияРезервированиеЗапасов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზა შეიცავს ინფორმაციას რეზერვების შესახებ! ალმის მოხსნა აკრძალულია!';ru='В базе содержится информация о резервах! Снятие флага запрещено!';en='The database contains information on reserves! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетЦенКонтрагентов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის კონტრაგენტების ხელშეკრულებები შემომავალი ფასების დაყენებული სახეობებით! ალმის მოხსნა აკრძალულია!';ru='В базе содержатся договоры контрагентов с установлинными видами входящих цен! Снятие флага запрещено!';en='The database contains contracts with counterparties ustanovlinnymi species included in prices! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетПоЯчейкам() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზა შეიცავს მოძრაობებს უჯრედების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе содержатся движения по ячейкам! Снятие флага запрещено!';en='The database contains the movement of the cells! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьХарактеристики() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები მახასიათებლების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть движения по характеристикам! Снятие флага запрещено!';en='At the base there is movement on the characteristics! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПартии() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები პარტიების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть движения по партиям! Снятие флага запрещено!';en='At the base there is traffic on the parties! Removing forbidden flag!'");
	...
	Если ПолучитьФункциональнуюОпцию("ПриемТоваровНаКомиссию") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ჩართულია ოპცია ""საკომისიოდ საქონლის მიღება"" (ნაწილი გაყიდვები)! ალმის მოხსნა აკრძალულია!';ru='Включена опция ""Прием товаров на комиссию"" (раздел Продажи)! Снятие флага запрещено!';en='The option ""Acceptance of goods on commission"" (Sales Section)! Removing forbidden flag!'");
	...
	Если ПолучитьФункциональнуюОпцию("ПереработкаДавальческогоСырья") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ოპცია ""გამცემის ნედლეულის გადამუშავება"" (ნაწილი წარმოება) ჩართულია! ალმის მოხსნა აკრძალულია!';ru='Включена опция ""Переработка давальческого сырья"" (раздел Производство)! Снятие флага запрещено!';en='The option ""tolling"" (section Production)! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСерийныеНомера() Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ka='ბაზაში არის ნაშთი სერიული ნომრების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть остатки по серийным номерам! Снятие флага запрещено!';en='At the base there are the remains of serial numbers! Removing forbidden flag!'");
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркировкиПродукцииВГИСМ") Тогда
		ТекстОшибки = НСтр("ru='Включен учет маркировки продукции в государственной информационной системе маркировки (ГИСМ). Снятие флага запрещено.';en='Including the integration of labeling in the marking system of the state information (GISM). Removing banned flag.'");
	КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияПередачаСырьяВПереработку() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""გადაცემული მარაგები"" შეიცავს ინფორმაციას გადამუშავებაზე გადაცემის შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче в переработку! Снятие флага запрещено!';en='Register accumulation ""Inventories transferred"" contains information on the transfer of recycling! Removing forbidden flag!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСкидкиНаценки() Экспорт
	...
	Пока ВыборкаВидыСкидокНаценок.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ფასდაკლებათა, ფასნამატთა სახეობები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются виды скидок, наценок! Снятие опции запрещено!';en='The database used types of discounts, markups! Removing options forbidden!'");
			Прервать;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='დოკუმენტებში გამოიყენება ფასდაკლებები და ფასნამატები! ოპციის მოხსნა აკრძალულია!';ru='В документах используются скидки и наценки! Снятие опции запрещено!';en='The documents used discounts and mark-ups! Removing options forbidden!'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДисконтныеКарты") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ჩართულია ოპცია ""დისკონტური ბარათების გამოყენება""! ალმის მოხსნა აკრძალულია!';ru='Включена опция ""Использовать дисконтные карты""! Снятие флага запрещено!';en='The option ""Use discount cards""! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияПередачаТоваровНаКомиссию() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""გადაცემული მარაგები"" შეიცავს ინფორმაციას საკომისიოდ გადაცემის შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче на комиссию! Снятие флага запрещено!';en='Register accumulation ""Inventories transferred"" contains information on the transfer of a commission! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияПриемТоваровНаКомиссию() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""მიღებული მარაგები"" შეიცავს ინფორმაციას საკომისიოდ მიღების შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме на комиссию! Снятие флага запрещено!';en='Register accumulation ""Inventories taken"" contains information about applying for a fee! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетРозничныхПродаж() Экспорт
	...
	Если ВыборкаЗапроса.Следующий()
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები ან ობიექტები, რომლებიც განეკუთვნება საცალო გაყიდვების ოპერაციების აღრიცხვას! ალმის მოხსნა აკრძალულია!';ru='В базе есть движения или объекты, относящиеся к учету операций розничных продаж! Снятие флага запрещено!';en='At the base there is motion or objects relating to the integration of retail sales operations! Removing forbidden flag!'");
	...
	Пока ВыборкаНаправленияДеятельности.Следующий() Цикл
		...
		Если ВыборкаНаправленияДеятельности.Ссылка <> Справочники.НаправленияДеятельности.ОсновноеНаправление
			...
			Если ТаблицаСсылок.Количество() > 0 Тогда
				...
				ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ძირითადისგან განსხვავებული მოღვაწეობის მიართულებები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются направления деятельности, отличные от основного! Снятие опции запрещено!';en='The database used by activities other than the main! Removing options forbidden!'");
				Прервать;
...
Функция ОтказСнятиеУчетПоНесколькимПодразделениям() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ძირითადისგან განსხვავებული ქვეგანყოფილებები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются подразделения, отличные от основного! Снятие опции запрещено!';en='The base units are used other than basic! Removing options forbidden!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетВнеоборотныхАктивов() Экспорт
	...
	Если Отказ Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის მოძრაობები გრძელვადიანი აქტივების მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют движения по внеоборотным активам! Снятие флага запрещено!';en='The database contains the movement of non-current assets! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуРаботы() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სერვისის დოკუმენტი""! ალამ ""სამუშაოები""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ - наряд""! Снятие флага ""Работы"" запрещено!';en='The information database contains documents ""order - order!"" Removing the ""Works"" flag is prohibited!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуПроизводство() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""საწარმოო შეკვეთა""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ на производство""! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""on production orders""! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""წარმოება""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Производство""! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Production""! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""დანახარჯების განაწილება""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Распределение затрат""! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Cost sharing""! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""მყიდველის შეკვეთა"" ოპერაციის ტიპით ""შეკვეთა გადამუშავებაზე""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ покупателя"" с видом операции ""Заказ на переработку""! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Order of the buyer"" with a view of operations ""for processing orders""! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სანარდო განწესი""! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Сдельный наряд""! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""piece-outfit!"" Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[5].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""უჯრედებში გადაადგილება"", სადაც კომპანიის სტრუქტურულ ერთეულს გააჩნია ტიპი ქვეგანყოფილება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Перемещение по ячейкам"", где структурная единица компании имеет тип Подразделение! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Moving into cells"", where the structural unit of the company has the type of division! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[6].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""მარაგების გადაადგილება"", სადაც კომპანიის სტრუქტურულ ერთეულს გააჩნია ტიპი ქვეგანყოფილება და/ან დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Перемещение запасов"", где структурная единица компании имеет тип Подразделение и/или счет затрат имеет тип Косвенные затраты! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""stock transfer"" where the structural unit of the company has the type of unit and / or by cost type is Indirect costs! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[7].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""საწყისი ნაშთების შეყვანა"", სადაც კომპანიის სტრუქტურულ ერთეულს გააჩნია ტიპი ქვეგანყოფილება და/ან დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანხარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Ввод начальных остатков"", где структурная единица компании имеет тип Подразделение  и/или счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Entering opening balances"" where the structural unit of the company has the type of unit and / or a type of indirect costs or work in process by the cost! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[8].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ქონების მიღება აღრიცხვაზე"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Принятие к учету имущества"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The knowledge base contains the documents'' acceptance to the account of the property "", by which the cost of type Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[9].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""მარაგების შემოსავალში გატარება"", სადაც კომპანიის სტრუქტურულ ერთეულს გააჩნია ტიპი ქვეგანყოფილება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Оприходование запасов"", где структурная единица компании имеет тип Подразделение! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Posting reserves"" where the structural unit of the company has type unit! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[10].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ბიუჯეტი"", სადაც ანგარიშებს გააჩნიათ ტიპი არაპირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Бюджет"", где счета затрат имеют тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Budget"", where the account of costs are of type Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[11].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ქონების პარამეტრების შეცვლა"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Изменение параметров имущества"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents'' property Parameter Change "", where the bill of costs is of type Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[12].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ხელფასის დარიცხვა"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანხარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Начисление зарплаты"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Payment accounting,"" where by cost type is Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[13].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""გადასახადების დარიცხვა"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Начисление налогов"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Charging of taxes"", where by cost type is Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[14].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ოპერაცია"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Операция"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Operation"", where by cost type is Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[15].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სხვა დანახარჯები (ხარჯები)"", სადაც დანახარჯების ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Прочие затраты (расходы)"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information database contains documents ""Other costs (expenses)"", where the bill of costs is of type Indirect costs or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[16].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის ცნობარ ""ნომენკლატურა""-ს ელემენტები ტიპით "", სადაც აღრიცხვის ანგარიშს გააჩნია ტიპი პირდაპირი დანახარჯები. დაუსრულებელი წარმოება ან მარაგ ნომენკლატურა-ს შევსების მეთოდი! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют элементы справочника ""Номенклатура"", где счет учета затрат имеет тип Косвенные затраты, Незавершенное производство или способ пополнения запаса Производство! Снятие флага ""Производство"" запрещено!';en='The information base includes elements of reference ""Nomenclature"", where by taking into account the cost of type Indirect costs, work in process or method of replenishment production! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[17].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის ცნობარ ""სტრუქტურული ერთეულის"" ელემენტები, სადაც ავტოგადაადგილების (გადაადგილება, კომპლექტაცია) პარამეტრს გააჩნია ტიპი ქვეგანყოფილება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют элементы справочника ""Структурная единица"", где параметр автоперемещения (перемещение, комплектация) имеет тип Подразделение! Снятие флага ""Производство"" запрещено!';en='The information base includes elements of reference ""structural unit"", where the parameter Auto Advance (moving, equipment) has type unit! Removing the ""Production"" flag is prohibited!'");
...
Функция ПроверитьДвиженияПоРегистрамПодсистемыПроизводство()
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის მოძრაობები რეგისტრ ""მარაგების"" მიხედვით, სადაც აღრიცხვის დათვლას გააჩნია ტიპი არაპირდაპირი დანახარჯები ან დაუსრულებელი წარმოება! ალამ ""წარმოება""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют движения по регистру ""Запасы"", где счет учета имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';en='The information base on present movement register ""Inventories"", which is by taking into account the type of indirect costs, or work in progress! Removing the ""Production"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='დაგროვების რეგისტრი ""მარაგები მიღებული"" შეიცავს ინფორმაციას მიღების და გადამუშავების შესახებ! ალამ ""წარმოების"" მოხსნა აკრძალულია!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме в переработку! Снятие флага ""Производство"" запрещено!';en='Register accumulation ""Inventories taken"" contains information about the reception in the processing! Removing the ""Production"" flag is prohibited!'");
...
Функция ОтказСнятьФункциональнаяОпцияПереработкаДавальческогоСырья() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='დაგროვების რეგისტრი ""მიღებული მარაგები"" შეიცავს ინფორმაციას გადამუშავებაზე მიღების შესახებ! ალმის მოხსნა იკრძალება!';ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме в переработку! Снятие флага запрещено!';en='Register accumulation ""Inventories taken"" contains information about the reception in the processing! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьТехоперации() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის ინფორმაცია სამუშაო ცენტრების ჩატვირთვის შესახებ ან ""სანარდო განწესის"" ტიპის დოკუმენტები! ალმის მოხსნა აკრძალულია!';ru='В базе присутствует информация о загрузке рабочих центров или документы вида ""Сдельный наряд""! Снятие флага запрещено!';en='The database contains information about how to download the work center or documents like ""piece-outfit!"" Removing forbidden flag!'");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ka='ბაზაში არის ნომენკლატურა ტიპით ""ოპერაცია""! ალმის მოხსნა აკრძალულია!';ru='В базе присутствует номенклатура с типом ""Операция""! Снятие флага запрещено!';en='The base is present nomenclature of the type of ""Operation""! Removing forbidden flag!'");
...
Функция ПроверитьДвиженияПоРегистрамПодсистемыЗарплата() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის მოძრაობები რეგისტრ ""დარცხვები და დაკავებების"" მიხედვით! ალამ ""ხელფასი""-ს აკრძალულია!';ru='В информационной базе присутствуют движения по регистру ""Начисления и удержания""! Снятие флага ""Зарплата"" запрещено!';en='The information base on present movement register ""Accruals and retention""! Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის მოძრაობები რეგისტრ ""პერსონალთან ანგარიშსწორების"" მიხედვით! ალამ ""ხელფასის"" მოხსნა აკრძალულია!';ru='В информационной базе присутствуют движения по регистру ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!';en='The information base on present movement register ""Settlements with personnel!"" Removing the ""Salary"" flag is prohibited!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуЗарплата() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ხელფასის დარიცხვა""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Начисление зарплаты""! Снятие флага ""Зарплата"" запрещено!';en='The information database contains documents ""Payment accounting""! Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სანარდო განწესი"", რომლებიც თანამშრომლებს ხელფასს ურიცხავენ! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Сдельный наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!';en='The information database contains documents ""piece-dress"", which is charged salaries of employees! Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სერვისის დოკუმენტი"", რომლებიც თანამშრომლებს ხელფასს ურიცხავენ! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ - наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!';en='The information database contains documents ""Order - dress"", which is charged salaries of employees! Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) +  НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""საწყისი ნაშთების შეყვანა"", აღრიცხვის ნაწილის ""პერსონალთან ანგარიშსწორების"" მიხედვით! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Ввод начальных остатвков"", по разделу учета ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!';en='The information database contains documents ""Entering initial ostatvkov"" section on account ""Settlements with personnel!"" Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""სალაროდან გასავალი"", ოპერაციის ტიპით ""ხელფასი უწყისის მიხედვით"" და/ან ""ხელფასი თანამშრომელს""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Расход из кассы"", с видом операции ""Зарплата по ведомости"" и/или ""Зарплата сотруднику""! Снятие флага ""Зарплата"" запрещено!';en='The information database contains documents ""from the cash flow"", with a view of the operation ""The salary of statements"" and / or ""The salary of the employee""! Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[5].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""ანგარიშიდან გასავალი"", ოპერაციის ტიპით ""ხელფასი""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Расод со счета"", с видом операции ""Зарплата""! Снятие флага ""Зарплата"" запрещено!';en='The information database contains documents ""Rasod account"", with a view of the operation ""salary""! Removing the ""Salary"" flag is prohibited!'");
	...
	Если НЕ МассивРезультатов[6].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არიან თანამშრომლები დასაქმების ტიპით ""შეთავსება""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага ""Зарплата"" запрещено!';en='The knowledge base has employees with employment type ""combination""! Removing the ""Salary"" flag is prohibited!'");	
	...
	Если НЕ МассивРезультатов[7].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საინფორმაციო ბაზაში არის ცნობარ ""დარიცხვების და დაკავებების ტიპების"" ელემენტები ტიპით ""გადასახადი""! ალამ ""ხელფასი""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага ""Зарплата"" запрещено!';en='The information base includes elements of the directory ""Forms of charges and deductions"" to the type of ""tax""! Removing the ""Salary"" flag is prohibited!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСовместительство() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არიან თანამშრომლები, რომელთა დასაქმების ტიპია ""შეთავსება""! ალმის მოხსნა აკრძალულია!';ru='В базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага запрещено!';en='At the base there are employees with the type of employment ""Moonlighting""! Removing forbidden flag!'");
...
Функция ОтказСнятьФункциональнаяОпцияВестиУчетНалогаНаДоходыИВзносов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის ცნობარ ""დარიცხვების და დაკავების სახეობათა"" ელემენტები ტიპით ""ბეგარა""! ალმის მოხსნა აკრძალულია!';ru='В базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага запрещено!';en='At the base there are elements of the directory ""Forms of charges and deductions"" to the type of ""tax""! Removing forbidden flag!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьДисконтныеКарты() Экспорт
	...
	Пока ВыборкаДисконтныеКарты.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება დისკონტური ბარათები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются дисконтные карты! Снятие опции запрещено!';en='The database used discount cards! Removing options forbidden!'");
			Прервать;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьАвтоматическиеСкидкиНаценки() Экспорт
	...
	Пока ВыборкаАвтоматическиеСкидки.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ka='ბაზაში გამოიყენება ავტომატური ფასდაკლებების, ფასნამატების სახეობები! ოპციის მოხსნა აკრძალულია!';ru='В базе используются виды автоматических скидок, наценок! Снятие опции запрещено!';en='The database used types of automatic discounts, markups! Removing options forbidden!'");
			Прервать;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='დოკუმენტებში გამოიყენება ავტომატური ფასდაკლებები და ფასნამატები! ოპციების მოხსნა აკრძაქლულია!';ru='В документах используются автоматические скидки и наценки! Снятие опции запрещено!';en='The documents used automatic discounts and mark-ups! Removing options forbidden!'");
...
Функция ОтказСнятьФункциональнаяОпцияРазноситьОплатуОтЭквайрераПоЭквайринговымОперациям() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("КассовыйМетодУчетаДоходовИРасходов") Тогда
		ТекстОшибки = НСтр("ka='როდესაც დაყენებულია ოპცია ""შემოსავლების და ხარჯების აღრიცხვის სალარო მეთოდი"", ოპციის მოხსნა აკრძალულია!';ru='При установленной опции ""Кассовый метод учета доходов и расходов"" снятие опции запрещено!';en='When you set the option ""cash accounting of income and expenses"" removal prohibited option!'");
	КонецЕсли;
...
"Выполнить анализ можно в отчете ""Расчеты по эквайрингу (сводно)"" с детализацией до регистратора. Отчет вызывается из раздела ""Деньги"".';ka='დაგროვების რეგისტრი ""ეკვაირინგის მიხედვით ანგარიშსწორებები"" შეიცავს ინფორმაციას ეკვაირინგის ოპერაციების შესახებ! ალმის შეცვლა აკრძალულია!"
...
"Выполнить анализ можно в отчетах ""Займы сотрудникам"" и ""Кредиты (полученные)"". Отчет вызывается из раздела ""Деньги"".';ka='დაგროვების რეგისტრი ""კრედიტების და სესხების მიხედვით ანგარიშსწორებები"" ინფორმაციას შეიცავს! ალმის შეცვლა აკრძალულია!"
"ანალიზის შესრულება შესაძლებელია ანგარიშგება ""სესხები თანამშრომლებს""-ში და ""კრედიტები (მიღებული)""-ში. ანგარიშგების მოხმობა ხდება ნაწილ ""ფული""-დან.'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьЗаказыВРозничнойТорговле() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ka='საცალო დოკუმენტებში გამოიყენება მყიდველთა შეკვეთები! ოპციის მოხსნა აკრძალულია!';ru='В розничных документах используются заказы покупателей! Снятие опции запрещено!';en='customer orders are used in retail documents! Removing options forbidden!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьРемонты() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ka='ბაზაში არის ნაშთი რემონტში მყოფი საქონლის მიხედვით! ალმის მოხსნა აკრძალულია!';ru='В базе есть остатки по товарам в ремонте! Снятие флага запрещено!';en='At the base there are the remains of the goods to be repaired! Removing forbidden flag!'");
...
Функция ОтказСнятьИспользоватьНесколькоОрганизаций() Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстОшибки = НСтр("ka='ძირითადის გარდა, ბაზაში არის სხვა ორგანიზაციებიც. ოპციის გამოსართავად, საჭიროა ძირითადის გარდა, ყველას წაშლა.';ru='В базе есть организации помимо основной. Для выключения опции необходимо удалить все кроме основной.';en='The database has the organization in addition to the main one. To turn off the option, you must remove all but basic.'");
	КонецЕсли;
...
Функция ОтказСнятьУчетПоКомпании() Экспорт
	...
	Для Каждого Документ Из Метаданные.Документы Цикл
		...
		Если Счетчик = ПорцияДляПроверки Тогда
			Счетчик = 0;
			Если Не Запрос.Выполнить().Пустой() Тогда
				Возврат НСтр("ka='ბაზაში არის კომპანიისგან განსხვავებული ორგანიზაციის მიხედვით გატარებული დოკუმენტები. ოპციის გამოსართავად, აუცილებელია ასეთ დოკუმენტებში ორგანიზაციის შეცვლა.';ru='В базе есть проведенные документы по организации, отличной от компании. Для выключения опции необходимо изменить организацию в таких документах.';en='The database has spent on the organization of documents, other than the company. To turn off the option, you must change the organization in such documents.'");
			КонецЕсли;
			Запрос.Текст = "";
		КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьВидыЗаказовПокупателей() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Для выключения опции необходимо удалить все виды заказов покупателей кроме основного.';en='To turn off the option, all kinds of customer orders must be removed in addition to the main one.'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьВидыЗаказНарядов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Для выключения опции необходимо удалить все виды заказ-нарядов кроме основного.';en='To turn off the option, you must remove all kinds of work orders in addition to the main one.'");
...
Функция ОтказСнятьФункциональнаяОпцияНесколькоВариантовКП() Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ru='Для выключения опции необходимо удалить все заказы содержащие несколько вариантов КП.';en='To turn off the option you want to remove all of the orders containing several variants of the CP.'");
	КонецЕсли;
...
Процедура УстановитьАдресСервисаЦентраМониторинга(НовоеЗначение) Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ka='დერვისის მისამართი';ru='Адрес сервиса';en='service address'") + " "
			+ НовоеЗначение + " "
			+ НСтр("ka='პროგრამის გამოყენების შესახებ ანგარიშგებების გასაგზავნად ვებ-სერვისის დასაშვებ მისამართს არ წარმოადგენს.';ru='не является допустимым адресом веб-сервиса для отправки отчетов об использовании программы.';en='It is not a valid address of a Web service to send reports on the use of the program.'"); 
...
Процедура ПроверитьДанныеЕГАИС(ТаблицаТоваров, ДанныеЕГАИСДостаточны, ДокументОбъект) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ka='ესასს-ში გადასაცემად მონაცემები არასაკმარისია:';ru='Не хватает данных для передачи в ЕГАИС:';en='Not enough data to be transmitted in EGAIS:'"),
		ДокументОбъект);
	
	ДополнительныйСимвол = "";
	Пока Выборка.Следующий() Цикл
		
		ТекстТекущейОшибки = НСтр("ka='სტრიქონი #%1';ru='Строка №%1:';en='String №% 1:'");
		...
		Если Выборка.ОшибкаШтрихкода Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='შტრიხკოდი';ru='штрихкод';en='barcode'");
		КонецЕсли;
		Если Выборка.ОшибкаОбъема Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='პროდუქციის ერთეულის მოცულობა';ru='объем единицы продукции';en='unit of production volume'");
		КонецЕсли;
		Если Выборка.ОшибкаМарки Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='აქციზური მარკა';ru='акцизная марка';en='excise stamp'");
		КонецЕсли;
		Если Выборка.ОшибкаКрепости Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='სიმაგრე';ru='крепость';en='fortress'");
		КонецЕсли;
		Если Выборка.ОшибкаКодаВидаПродукции Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ka='ალკოჰოლური პროდუქციის სახეობის კოდი';ru='код вида алкогольной продукции';en='the type of alcohol code'");
		КонецЕсли;
...
Процедура УстановитьОтображаниеПодменюПечати(ГруппаФормыПечать) Экспорт
	...
	ГруппаФормыПечать.Подсказка		= НСтр("ka='ამობეჭდვა წინასწარი ნახვით';ru='Печать с предварительным просмотром';en='Print with preview'");
	ГруппаФормыПечать.Заголовок		= НСтр("ka='ბეჭდვა';ru='Печать';en='Print'");
...
Функция КомандаПечатаетсяВСерверномКонтексте(знач ОписаниеКомандыПечати) Экспорт
	...
	Если ОписаниеКомандыПечати.Представление = НСтр("ka='Настраиваемый комплект документов';ru='Настраиваемый комплект документов';en='Customizable set of documents'") Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Функция возвращает ссылку на подчиненный счет-фактуру
//
Функция ПолучитьПодчиненныйЭлНакладная(ДокументОснование) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	РС_ЭлектроннаяНакладнаяДокументыОснования.Ссылка,
	|	РС_ЭлектроннаяНакладнаяДокументыОснования.Ссылка.Номер,
	|	РС_ЭлектроннаяНакладнаяДокументыОснования.Ссылка.Дата
	|ИЗ
...
// Выполняет движения регистра накопления РасчетыПоНалогам.
//
Процедура ОтразитьРасчетыНДФЛ(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаРасчетыПоНалогам = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыНДФЛ;
	...
	ДвиженияРасчетыПоНалогам = Движения.РасчетыНДФЛ;
	ДвиженияРасчетыПоНалогам.Записывать = Истина;
	ДвиженияРасчетыПоНалогам.Загрузить(ТаблицаРасчетыПоНалогам);
...
//Паганини_
Процедура ОтразитьАнализПоПодразделениям(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	Если НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьПодразделениеВЗапасеПриходнаяНакладнаяИПриходныйОрдер") ИЛИ Отказ Тогда
		Возврат;
	КонецЕсли;
	ТаблицаЗакупки = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаАнализПоПодразделениям;
	Если ТаблицаЗакупки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияЗакупки = Движения.АнализПоПодразделениям;
	ДвиженияЗакупки.Записывать = Истина;
	ДвиженияЗакупки.Загрузить(ТаблицаЗакупки);
...
//--------------------------------------------------------------------------
// © Шишка В.В. [13 августа 2020 г.]  НАЧАЛО
Процедура ОтразитьГарантийноеОбслуживание(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	Если НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьГарантииВНарядЗаказах") ИЛИ Отказ
		ИЛИ ТипЗнч(ДополнительныеСвойства.ДляПроведения.Ссылка) <> Тип("ДокументСсылка.ЗаказПокупателя")
		ИЛИ ДополнительныеСвойства.ДляПроведения.Ссылка.ВидОперации <> Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд
		Тогда
		Возврат;
	КонецЕсли;
	ТаблицаГарантийноеОбслуживание = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаГарантийноеОбслуживание;
	Если ТаблицаГарантийноеОбслуживание.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияЗакупки = Движения.ГарантийноеОбслуживание;
	ДвиженияЗакупки.Записывать = Истина;
	ДвиженияЗакупки.Загрузить(ТаблицаГарантийноеОбслуживание);
...
Функция ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(СсылкаНаДокумент, СтруктурнаяЕдиница=НЕОПРЕДЕЛЕНО) ЭКСПОРТ
	...
	Запрос.Текст = 
		...
		|	ЗапасыНаСкладах.Организация КАК ОрганизацияПредставление,
		|	ЗапасыНаСкладах.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаПредставление,
		|	ЗапасыНаСкладах.Номенклатура КАК НоменклатураПредставление,
		|	ЗапасыНаСкладах.Характеристика КАК ХарактеристикаПредставление,
		|	ЗапасыНаСкладах.Партия КАК ПартияПредставление,
		|	ЗапасыНаСкладах.Ячейка КАК ЯчейкаПредставление,
		...
		|	-(ЕСТЬNULL(ЗапасыНаСкладах.Количество, 0) - ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0)) КАК КоличествоОстатокЗапасыНаСкладах
		|ИЗ
		|	РегистрНакопления.ЗапасыНаСкладах КАК ЗапасыНаСкладах
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(&МоментКонтроля, ) КАК ЗапасыНаСкладахОстатки
		|		ПО ЗапасыНаСкладах.Организация = ЗапасыНаСкладахОстатки.Организация
		|			И ЗапасыНаСкладах.СтруктурнаяЕдиница = ЗапасыНаСкладахОстатки.СтруктурнаяЕдиница
		|			И ЗапасыНаСкладах.Номенклатура = ЗапасыНаСкладахОстатки.Номенклатура
		|			И ЗапасыНаСкладах.Характеристика = ЗапасыНаСкладахОстатки.Характеристика
		|			И ЗапасыНаСкладах.Партия = ЗапасыНаСкладахОстатки.Партия
		|			И ЗапасыНаСкладах.Ячейка = ЗапасыНаСкладахОстатки.Ячейка
		|ГДЕ
		|	ЗапасыНаСкладах.Регистратор = &Регистратор
		|	И ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0) - ЕСТЬNULL(ЗапасыНаСкладах.Количество, 0) < 0";
	Если НЕ СтруктурнаяЕдиница=НЕОПРЕДЕЛЕНО Тогда
		Запрос.Текст = Запрос.Текст +"
		|	И ЗапасыНаСкладах.СтруктурнаяЕдиница В (&СтруктурнаяЕдиница)";
	...
	Запрос.УстановитьПараметр("Регистратор", СсылкаНаДокумент.Ссылка);
	Запрос.УстановитьПараметр("МоментКонтроля", СсылкаНаДокумент.МоментВремени());
...
// Функция возвращает ссылку на подчиненный счет-фактуру
//
Функция ПолучитьПодчиненныйСчетФактуруГрузия(ДокументОснование, Полученный = Ложь) Экспорт
	...
	Если Полученный Тогда
		...
		|	СчетФактураДокументыОснования.Ссылка.Дата
		|ИЗ
		|	Документ.СчетФактураПолученныйГрузия.ДокументыОснования КАК СчетФактураДокументыОснования
		|ГДЕ
		|	СчетФактураДокументыОснования.ДокументОснование = &ДокументОснование";
	...
	Иначе
		...
		|	СчетФактураДокументыОснования.Ссылка.Дата
		|ИЗ
		|	Документ.СчетФактураОтправленныйГрузия.ДокументыОснования КАК СчетФактураДокументыОснования
		|ГДЕ
		|	СчетФактураДокументыОснования.ДокументОснование = &ДокументОснование";
...
Функция ПолучитьПодчиненныйНалоговыйДокумент(ДокументОснование) Экспорт
	...
	|	РС_ЭлектроннаяНакладнаяДокументыОснования.Ссылка.Дата
	|ИЗ
	|	Документ.РС_НалоговыйДокумент.ДокументыОснования КАК РС_ЭлектроннаяНакладнаяДокументыОснования
	|ГДЕ
	|	РС_ЭлектроннаяНакладнаяДокументыОснования.ДокументОснование = &ДокументОснование";
...
Процедура ОтразитьНалоговыеЛготыИнформация(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаНалоговыеЛготыИнформация = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаНалоговыеЛготыИнформация;
	
	Если Отказ
	 ИЛИ ТаблицаНалоговыеЛготыИнформация.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияНалоговыеЛготыИнформация = Движения.НалоговыеЛготыИнформация;
	ДвиженияНалоговыеЛготыИнформация.Записывать = Истина;
	ДвиженияНалоговыеЛготыИнформация.Загрузить(ТаблицаНалоговыеЛготыИнформация);
	
КонецПроцедуры

Процедура ОтразитьНалоговыеЛготы(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаНалоговыеЛготы = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаНалоговыеЛготы;
	
	Если Отказ
	 ИЛИ ТаблицаНалоговыеЛготы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияНалоговыеЛготы = Движения.НалоговыеЛготы;
	ДвиженияНалоговыеЛготы.Записывать = Истина;
	ДвиженияНалоговыеЛготы.Загрузить(ТаблицаНалоговыеЛготы);
	
КонецПроцедуры

Процедура ОтразитьОстаткиНалоговыхЛьгот(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаОстаткиНалоговыхЛьгот = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаОстаткиНалоговыхЛьгот;
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияОстаткиНалоговыхЛьгот = Движения.ОстаткиНалоговыхЛьгот;
	ДвиженияОстаткиНалоговыхЛьгот.Записывать = Истина;
	ДвиженияОстаткиНалоговыхЛьгот.Загрузить(ТаблицаОстаткиНалоговыхЛьгот);
	
КонецПроцедуры

//_Lazare
Функция ПолучитьОсновнойБанковскийСчетКонтрагента(ИмяКонтрагента) Экспорт	
	
	Возврат ИмяКонтрагента.БанковскийСчетПоУмолчанию;
		
КонецФункции

&НаСервере  
Функция ПолучитьВидЦеныПоДиапазонуКоличестваНоменклатуры(СтруктураДанные,СтрКоличество) Экспорт	
	...
	Запрос.Текст = 
		...
		|	ЦеныНоменклатурыПоКоличествоСрезПоследних.НеИспользуется
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатурыПоКоличество.СрезПоследних КАК ЦеныНоменклатурыПоКоличествоСрезПоследних
		|ГДЕ
		|	ЦеныНоменклатурыПоКоличествоСрезПоследних.Номенклатура = &Номенклатура
		|	И ЦеныНоменклатурыПоКоличествоСрезПоследних.Организация = &Организация
	...
	Для Каждого стр из РезультатЗапроса Цикл 
		Если  (СтрКоличество >= стр.НачальноеКаличество ) и (СтрКоличество < стр.КанечноеКаличество ) тогда			
			ВидЦен = стр.ВидЦен;
		Возврат   ВидЦен ;	
		КонецЕсли;
	...
	Если  ВидЦен = неопределено Тогда 
		...
		Для Каждого   стр из РезультатЗапроса Цикл 
			Если  стр.КанечноеКаличество = 0 Тогда 
				Возврат   стр.ВидЦен ;
				КонецЕсли;
...
&НаСервере  
Функция ПолучитьКоличествоДиапазонаПоВидЦены(ВидЦен, Характеристика ) Экспорт	
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЦеныНоменклатурыПоКоличествоСрезПоследних.КанечноеКаличество КАК КанечноеКаличество
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатурыПоКоличество.СрезПоследних(
		|			,
		|			НеИспользуется = ЛОЖЬ
		|				И ВидЦен = &ВидЦен
		|				И Характеристика = &Характеристика) КАК ЦеныНоменклатурыПоКоличествоСрезПоследних";
	...
	РезультатЗапроса  = Запрос.Выполнить().Выгрузить();
	Если РезультатЗапроса.Количество()>0 Тогда 
		Возврат  РезультатЗапроса[0].КанечноеКаличество ;
	КонецЕсли;
...
// Выполняет движения регистра накопления НачисленияИУдержания.
//
Процедура ОтразитьНачисленияИУдержанияВРазрезеПроектов(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаНачисленияИУдержания = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаНачисленияИУдержанияВРазрезеПроекта;
	...
	ДвиженияНачисленияИУдержания = Движения.НачисленияИУдержанияВРазрезеПроекта;
	ДвиженияНачисленияИУдержания.Записывать = Истина;
	ДвиженияНачисленияИУдержания.Загрузить(ТаблицаНачисленияИУдержания);
...
// Выполняет движения регистра сведений ПервоначальныеСведенияПараметрыАмортизации.
//
Процедура ОтразитьУчетПоПроектамВнеоборотныхАктивов(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	Если Не ДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаУчетПоПроектамВнеоборотныхАктивов = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУчетПоПроектамВнеоборотныхАктивов;
	
	Если Отказ
	 ИЛИ ТаблицаУчетПоПроектамВнеоборотныхАктивов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
		
	ДвиженияУчетПоПроектамВнеоборотныхАктивов = Движения.УчетПоПроектамВнеоборотныхАктивов;
	ДвиженияУчетПоПроектамВнеоборотныхАктивов.Записывать = Истина;
	ДвиженияУчетПоПроектамВнеоборотныхАктивов.Загрузить(ТаблицаУчетПоПроектамВнеоборотныхАктивов);
...
Процедура ОтразитьУсушкa_СобранныеPаказы(ДополнительныеСвойства, Движения, Отказ) ЭКСПОРТ
	
	ТаблицаЗаказыПокупателей = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУсушкa_СобранныеPаказы;
	...
	ДвиженияЗаказыПокупателей = Движения.Усушкa_СобранныеPаказы;
	ДвиженияЗаказыПокупателей.Записывать = Истина;
	ДвиженияЗаказыПокупателей.Загрузить(ТаблицаЗаказыПокупателей);
...
&НаСервере  
Функция ПолучитьКоличествоЗапасовНаСкладе(Дата,Номенклатура,Организация,СтруктурнаяЕдиница) Экспорт	
	...
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	Если РезультатЗапроса.Количество()>0 Тогда 
		Возврат  РезультатЗапроса[0].КоличествоОстаток;
	КонецЕсли;
