...
"ვინაიდან მოქმედება ხელმისაწვდომია მხოლოდ შეკვეთა-განწესებისთვის!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуРаботы() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='საინფორმაციო ბაზაში არის დოკუმენტები ""შეკვეთა-განწესი""! ალამ ""სამუშაოები""-ს მოხსნა აკრძალულია!';ru='В информационной базе присутствуют документы ""Заказ - наряд""! Снятие флага ""Работы"" запрещено!';en='The information database contains documents ""order - order!"" Removing the ""Works"" flag is prohibited!'");
...
Функция ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(СсылкаНаДокумент, СтруктурнаяЕдиница=НЕОПРЕДЕЛЕНО) ЭКСПОРТ
	...
	Запрос.Текст = 
		...
		|	И НЕ(ТИПЗНАЧЕНИЯ(&Регистратор) = ТИП(Документ.ПересортицаЗапасов) И ЗапасыНаСкладах.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
		//ПодольскийАИ #ОшибкаКонтроля 13.09.2021 }
