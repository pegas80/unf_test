...
Процедура ПроверитьКорректностьПерсонифицованностиЗаказываемыхКиЗ(Объект, ИмяТабличнойЧасти, Отказ) Экспорт
	...
	Если Выборка.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ka='Нельзя заказывать в одном документе персонифицированные и неперсонифицированные КиЗ.';ru='Нельзя заказывать в одном документе персонифицированные и неперсонифицированные КиЗ.';en='You can not order a single document personalized and non-personalized Keyes.'");
...
Функция GTINКиЗ(Номенклатура, Характеристика) Экспорт
	...
	Иначе
		...
		Если РеквизитыНоменклатуры.ИспользоватьХарактеристики Тогда
			ТекстИсключения = НСтр("ka='Невозможно определить GTIN КиЗ, т.к. не передана характеристика.';ru='Невозможно определить GTIN КиЗ, т.к. не передана характеристика.';en='Unable to determine the GTIN Keyes, because not transferred characteristic.'");
...
Процедура ЗаполнитьЗаявкуНаВыпускКиЗНаОснованииДругогоДокумента(ЗаявкаОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не РегистрыСведений.СтатусыЗаявокНаВыпускКиЗГИСМ.ЭтоСтатусНеАктуальнойЗаявки(РеквизитыЗаказа.СтатусЗаявкиНаВыпускКиЗ) Тогда
		...
		ТекстОшибки = НСтр("ka='Для %Документ% уже существует актуальная заявка на выпуск КиЗ - %Заявка%.';ru='Для %Документ% уже существует актуальная заявка на выпуск КиЗ - %Заявка%.';en='To document%% already exists topical application for the release of Keys -% application%.'");
	...
	Если РезультатЗапроса[1].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='В %Документ% отсутствуют контрольные (идентификационные) знаки.';ru='В %Документ% отсутствуют контрольные (идентификационные) знаки.';en='The document%% no control (identification) signs.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура ЗаполнитьУведомлениеОСписанииКиЗНаОснованииВнутреннегоПотребления(УведомлениеОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не РегистрыСведений.СтатусыИнформированияГИСМ.ЭтоСтатусНеАктуальногоУведомления(РеквизитыОснования.Статус) Тогда
		...
		ТекстОшибки = НСтр("ka='Для %Документ% уже существует актуальное уведомление об списании КиЗ - %Уведомление%.';ru='Для %Документ% уже существует актуальное уведомление об списании КиЗ - %Уведомление%.';en='To document%% already exists actual notice of cancellation Kiz -%% Notice.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
	...
	Если РезультатЗапроса[1].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками и контрольные (идентификационные) знаки.';ru='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками и контрольные (идентификационные) знаки.';en='The document%% no products marked with control (identification) signs and control (identification) signs.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура ЗаполнитьУведомлениеОбОтгрузкеГИСМНаОснованииРеализации(УведомлениеОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не РегистрыСведений.СтатусыУведомленийОбОтгрузкеМаркированныхТоваровГИСМ.ЭтоСтатусНеАктуальногоУведомления(РеквизитыОснования.Статус) Тогда
		...
		ТекстОшибки = НСтр("ka='Для %Документ% уже существует актуальное уведомление об отгрузке ГИСМ - %Уведомление%.';ru='Для %Документ% уже существует актуальное уведомление об отгрузке ГИСМ - %Уведомление%.';en='To document%% already exists actual shipping notice GISM -%% Notice.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
	...
	Если РезультатЗапроса[5].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ka='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками.';ru='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками.';en='The document%% no products marked with control (identification) signs.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура СформироватьТекстЗаявкаНаВыпускКиЗ(Форма) Экспорт
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ka='Создать заявку на выпуск КиЗ';ru='Создать заявку на выпуск КиЗ';en='Create an application for the release of Keys'");
		ИмяКоманды   = "СоздатьЗаявкуНаВыпускКиЗ";
	Иначе
		ТекстНадписи =  НСтр("ka='Заявка на выпуск КиЗ не создана';ru='Заявка на выпуск КиЗ не создана';en='Keys to issue application has not been created'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ka='Заявка на выпуск КиЗ: %1';ru='Заявка на выпуск КиЗ: %1';en='An application for the release of Keys:% 1'"), Выборка.СостояниеЗаявкиНаВыпускКиЗ);
...
Процедура СформироватьТекстУведомлениеОбИмпортеВвозеИзЕАЭС(Форма) Экспорт
	...
	Если ВидИмпорта = "ПоступлениеПоИмпорту" Тогда
		...
		Если ПравоДобавленияУведомлениеОбИпморте Тогда
			ТекстНадписи = НСтр("ka='Создать уведомление об импорте ГИСМ';ru='Создать уведомление об импорте ГИСМ';en='Create an alert about importing GISM'");
			ИмяКоманды   = "СоздатьУведомлениеГИСМ";
		Иначе
			ТекстНадписи = НСтр("ka='Уведомление об импорте ГИСМ не созадано';ru='Уведомление об импорте ГИСМ не созадано';en='Notification of import GISM not sozadano'");
			ИмяКоманды   = "";
		КонецЕсли;
	ИначеЕсли ВидИмпорта = "ПоступлениеИзЕАЭС" Тогда
		...
		Если ПравоДобавленияУведомлениеОВвозе Тогда
			ТекстНадписи = НСтр("ka='Создать уведомление о ввозе из ЕАЭС ГИСМ';ru='Создать уведомление о ввозе из ЕАЭС ГИСМ';en='Create an alert about the import of the EAEC GISM'");
			ИмяКоманды   = "СоздатьУведомлениеГИСМЕАЭС";
		Иначе
			ТекстНадписи = НСтр("ka='Уведомление о ввозе из ЕАЭС ГИСМ не созадано';ru='Уведомление о ввозе из ЕАЭС ГИСМ не созадано';en='Notice of importation of the EAEC GISM not sozadano'");
			ИмяКоманды   = "";
		КонецЕсли;
	Иначе 
		ВызватьИсключение (НСтр("ka='Ошибочный вид импорта';ru='Ошибочный вид импорта';en='Wrong type of import'"));
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		Если ВидИмпорта = "ПоступлениеПоИмпорту" Тогда
			ТекстНадписи = СтрШаблон(НСтр("ka='Уведомление об импорте ГИСМ: %1';ru='Уведомление об импорте ГИСМ: %1';en='Notification of import GISM:% 1'"), Выборка.Статус);
		ИначеЕсли ВидИмпорта = "ПоступлениеИзЕАЭС" Тогда
			ТекстНадписи = СтрШаблон(НСтр("ka='Уведомление о ввозе из ЕАЭС ГИСМ: %1';ru='Уведомление о ввозе из ЕАЭС ГИСМ: %1';en='Notice of importation of the EAEC GISM:% 1'"), Выборка.Статус);
		КонецЕсли;
...
Процедура СформироватьТекстУведомлениеОСписанииКиЗ(Форма) Экспорт
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ka='Создать уведомление о списании КиЗ ГИСМ';ru='Создать уведомление о списании КиЗ ГИСМ';en='Create a notice of cancellation of Keyes GISM'");
		ИмяКоманды   = "СоздатьУведомлениеГИСМ";
	Иначе
		ТекстНадписи = НСтр("ka='Уведомление о списании КиЗ ГИСМ не создано';ru='Уведомление о списании КиЗ ГИСМ не создано';en='Notice of cancellation Keyes GISM not created'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ka='Уведомление о списании КиЗ ГИСМ: %1';ru='Уведомление о списании КиЗ ГИСМ: %1';en='Notice of cancellation GISM Keys:% 1'"), Выборка.Статус);
		ИмяКоманды    = "ОткрытьПротоколОбмена";
...
Процедура СформироватьТекстУведомлениеОбОтгрузкеМаркированнойПродукции(Форма) Экспорт
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ka='Создать уведомление об отгрузке ГИСМ';ru='Создать уведомление об отгрузке ГИСМ';en='Create a shipping notice GISM'");
		ИмяКоманды   = "СоздатьУведомлениеГИСМ";
	Иначе
		ТекстНадписи = НСтр("ka='Уведомление об отгрузке ГИСМ не создано';ru='Уведомление об отгрузке ГИСМ не создано';en='Shipping Note GISM not created'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ka='Уведомление об отгрузке ГИСМ: %1';ru='Уведомление об отгрузке ГИСМ: %1';en='Shipping Note GISM:% 1'"), Выборка.Статус);
		ИмяКоманды   = "ОткрытьПротоколОбмена";
