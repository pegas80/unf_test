...
Процедура ОповеститьОНовыхДокументахЭДО() Экспорт
	...
	НавигационнаяСсылка = НавигационнаяСсылкаТекущиеДелаЭДО();
		ЗаголовокОповещения = НСтр("ka='სერვისი ЭДО';ru='Сервис ЭДО';en='EDI service'");
		ТекстОповещения = НСтр("ka='ახალი დოკუმენტების მიღება';ru='Получить новые документы';en='Get new documents'");
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если РезультатПроверки = Неопределено Тогда
			...
			Если Форма.Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
				...
				СтрПроведен = ?(Проведен, НСтр("ka='ჩავწეროთ და გავატაროთ. ჩავწეროთ და გავატაროთ?';ru='записать и провести."
"Записать и провести?'"), НСтр("ka='ჩავწეროთ."
"ჩავწეროთ?';ru='записать."
"Записать?'"));
				
				ШаблонСообщения = НСтр("ka='დოკუმენტი შეცვლილია. ელექტრონული დოკუმენტის ჩამოსაყალიბებლად აუცილებელია %1';ru='Документ изменен. Для формирования электронного документа его необходимо %1';en='Document is changed. For the formation of an electronic document must be 1%'");
				...
				ПоказатьВопрос( Обработчик, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.Отмена,
					НСтр("ka='დოკუმენტი შეიცვალა.';ru='Документ изменен.';en='Document is changed.'"));
...
Процедура ЗаписатьВФорме(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если Проведен Тогда
			...
			Исключение
				ПоказатьПредупреждение(, НСтр("ka='Операция не выполнена.';ru='Операция не выполнена.';en='The operation is not performed.'"));
				Отказ = Истина;
			КонецПопытки;
		Иначе
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ka='%1. ელექტრონული დოკუმენტი ვერ მოიძებნა.';ru='%1. Электронный документ не найден.';en='%one. The electronic document can not be found.'");
...
Процедура ЗакрытьПринудительноЭДО(МассивВладельцевЭД) Экспорт
	...
	ПричинаЗакрытия = "";
	ПоказатьВводСтроки(Обработчик, ПричинаЗакрытия, НСтр("ka='Укажите причину закрытия документооборота';ru='Укажите причину закрытия документооборота';en='Enter the reason for closing the document'"),,Истина);
...
"Операция возможна только для непроведенных документов.';ka='Перезаполнение документа %1."
"Операция возможна только для непроведенных документов.'");
	...
	Если СоответствиеВладельцевИЭД.Количество() = 0 Тогда
		Для Каждого ТекущийДокумент Из МассивСсылок Цикл
			Шаблон = НСтр("ka='Электронный документ для %1 не найден';ru='Электронный документ для %1 не найден';en='Electronic document% 1 not found'");
	...
	Если МассивИзмененныхВладельцев.Количество() > 0 Тогда
		...
		Если МассивИзмененныхВладельцев.Количество() = 1 Тогда
			ТекстСостоянияВывод = НСтр("ka='დოკუმენტი გადავსებულია.';ru='Документ перезаполнен.';en='refilled document.'");
		Иначе
			ТекстСостоянияВывод = НСтр("ka='დოკუმენტები გადავსებულია (%1).';ru='Документы перезаполнены (%1).';en='Documents refill (1%).'");
...
"Выберите файл с расширением ""zip"".';ka='Некорректный формат файла."
...
Процедура ЗакрытьПринудительноРезультатВводаСтроки(ПричинаЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПричинаЗакрытия) Тогда
		...
		ТекстСообщения = НСтр("ka='Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.';ru='Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.';en='To close the chosen ED document must specify the reason for the closure.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	ТекстОповещения = НСтр("ka='Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)';ru='Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)';en='Change the status of ED documents ""Force close"" (1%)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", КоличествоОбработанныхЭД);
	ПоказатьОповещениеПользователя(НСтр("ka='დოკუმენტების დამუშავება';ru='Обработка документов';en='Document processing'"), , ТекстОповещения);
...
"Обратитесь к администратору.';ka='Недостаточно прав для настройки ЭДО с контрагентом."
...
Процедура ОбновитьКартинкуЭДО(Форма, КартинкаЭДО, МестоРазмещенияКоманд)
	...
	Если Не КартинкаЭДО = БиблиотекаКартинок.ЭмблемаСервиса1СЭДО Тогда
		ПодменюЭДО.Подсказка = НСтр("ka='სერვის 1C-ედბ-ში არის ახალი დოკუმენტები';ru='В сервисе 1С-ЭДО есть новые документы';en='The service 1C-EDC has new documents'");
	Иначе
		ПодменюЭДО.Подсказка = НСтр("ka='ბრძანებები ЭДО';ru='Команды ЭДО';en='EDO team'");
	КонецЕсли;
