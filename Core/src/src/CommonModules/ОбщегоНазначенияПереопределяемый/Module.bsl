...
// Kuzmich, 07.08.2019, Дополнительные инструменты в табличных частях документов движения товаров
Процедура УстановитьДополнительноеПоведениеСтандартныхКнопокТЧЗапасы(ТекФорма) ЭКСПОРТ

	Для каждого ТекЭлемент Из ТекФорма.Элементы Цикл
	
		Если ТипЗНЧ(ТекЭлемент)=Тип("КнопкаФормы") Тогда
		
			Если ТекЭлемент.Имя = "ЗапасыСкопировать"
				ИЛИ ТекЭлемент.Имя = "ЗапасыИзменить"
				ИЛИ ТекЭлемент.Имя = "ЗапасыУдалить"
				ИЛИ ТекЭлемент.Имя = "ЗапасыСортироватьСписокПоВозрастанию"
				ИЛИ ТекЭлемент.Имя = "ЗапасыСортироватьСписокПоУбыванию"
				Тогда
			
				Если Константы.ДополнительныеИнструментыВТЧдокументовДвиженияТоваров.Получить() Тогда
					ТекЭлемент.ТолькоВоВсехДействиях = ЛОЖЬ;
				Иначе	
					ТекЭлемент.ТолькоВоВсехДействиях = НЕОПРЕДЕЛЕНО;
				КонецЕсли;
