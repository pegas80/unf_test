...
"Регламентное задание ""%1"", работающее с внешними ресурсами, отключено.';ka='დანართი გადანაცვლებულია. გარე რესურსებთან მომუშავე სარეგლამენტო დავალება ""%1"" გამორთულია.'"), РегламентноеЗадание.Синоним);
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='საინფორმაციო ბაზის შეერთების სტრიქონი შეიცვალა."
"შეიძლება, რომ მოხდა საინფორმაციო ბაზის გადაადგილება."
"სარეგლამენტო დავალება ""%1"" გამორთულია.';ru='Изменилась строка соединения информационной базы."
...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("ka='ბლოკირების მოხსნა ასევე, შეიძლება მოგვიანებით, შემდეგ ნაწილში <b>ადმინისტრირება - მხარდაჭერა და მომსახურება</b>.';ru='Блокировку также можно снять позднее в разделе <b>Администрирование - Поддержка и обслуживание</b>.';en='Lock and can be removed later in the <b> Administrative Tools - Service and support </ b>.'"));
...
Процедура ВызватьИсключениеЕслиНетПраваАдминистрирования() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ka='Нарушение прав доступа.';ru='Нарушение прав доступа.';en='An access violation.'");
		КонецЕсли;
	Иначе
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание) Экспорт
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელით გაშვება: %1';ru='Запуск вручную: %1';en='Manual start:% 1'"), ПредставлениеРегламентногоЗадания(Задание));
...
Функция ТекстНеОпределено() Экспорт
	...
	Возврат НСтр("ka='<გაურკვეველია>';ru='<не определено>';en='<Not specified>'");
...
Процедура ОтменитьФоновоеЗадание(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='ფონური დავალება სერვერზე ვერ მოიძებნა.';ru='Фоновое задание не найдено на сервере.';en='Background job is not found on the server.'");
	КонецЕсли;
	
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Активно Тогда
		ВызватьИсключение НСтр("ka='დავალების შესრულება არ ხდება, მისი შეცვლა არ შეიძლება.';ru='Задание не выполняется, его нельзя отменить.';en='Task does not run, can not be undone.'");
	КонецЕсли;
...
Функция ИдентификаторФайловойБазы(ЭтоФайловаяБаза)
	...
	Если ЭтоФайловаяБаза Тогда
		...
		СодержимоеФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1"
""
...
"информационная база работать с внешними ресурсами.';ru='%1"
""
"Файл создан автоматически прикладным решением ""%2""."
"Он содержит идентификатор информационной базы и позволяет определить, что эта информационная база была скопирована."
""
...
Процедура УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='გარე რესურსებთან მუშაობა დაბლოკილია';ru='Работа с внешними ресурсами заблокирована';en='Working with external resources locked'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы = Ложь, ПоказатьФорму = Ложь) Экспорт
	...
	Если РазделениеВключено <> ПараметрыБлокировки.РазделениеВключено Тогда
		...
		ТекстСообщения = НСтр("ka='საინფორმაციო ბაზა დანართიდან ინტერნეტში გადაადგილდა.';ru='Информационная база была перемещена из приложения в Интернете.';en='Information base has been moved to the Internet from within the application.'");
	...
	Если ЭтоФайловаяБаза <> ПараметрыБлокировки.ЭтоФайловаяБаза Тогда
		ТекстСообщения = ?(ЭтоФайловаяБаза, НСтр("ka='საინფორმაციო ბაზამ მუშაობის კლიენტ-სერვერული რეჟიმიდან გადაინაცვლა ფაილურ რეჟიმში.';ru='Информационная база была перемещена из клиент-серверного режима работы в файловый.';en='The information base was moved from the client-server mode in the file.'"),
			НСтр("ka='საინფორმაციო ბაზამ მუშაობის ფაილური რეჟიმიდან გადაინაცვლა კლიენტ-სერვერულ რეჟიმში.';ru='Информационная база была перемещена из файловый режима работы в клиент-серверный.';en='The information base was moved from the file operation mode in a client-server.'"));
		УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
		Возврат Истина;
	КонецЕсли;
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='საინფორმაციო ბაზის კატალოგში შემოწმების ფაილი %1 არ არის.';ru='В каталоге информационной базы отсутствует файл проверки %1.';en='The directory information base is not checking the file% 1.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИдентификаторБазы <> ПараметрыБлокировки.ИдентификаторБазы Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='შემოწმების ფაილ %1-ში საინფორმაციო ბაზის იდენტიფიკატორი მიმდინარე ბაზაში არსებულ იდენტიფიკატორს არ შეესაბამება.';ru='Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.';en='The ID information database in a file for checking% 1 does not match the identity in the current database.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
...
"Сохраненное имя компьютера: %3, текущее имя компьютера: %4';ka='საინფორმაციო ბაზის სახელი ან კომპიუტერის სახელი განსხვავდება."
"ბაზის შენახული სახელი: %1, ბაზის მიმდინარე სახელი: %2."
"კომპიუტერის შენახული სახელი: %3, კომპიუტერის მიმდინარე სახელი: %4'"),
