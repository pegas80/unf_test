...
Процедура ПроверитьСписокПередОперацией(ОбработчикРезультата, СписокРеквизит)
	...
	Если Не СортировкаВСпискеУстановленаПравильно(СписокРеквизит) Тогда
		ТекстВопроса = НСтр("ka='ელემენტების რიგითობის შესაცვლელად აუცილებელია ველ ""საჩუქარი""-ს მიხედვით სიის დახარისხების გამართვა. გავმართოთ აუცილებელი დახარისხება?';ru='Для изменения порядка элементов необходимо настроить сортировку списка"
"по полю ""Порядок"". Настроить необходимую сортировку?'");
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='გავასწოროთ';ru='Настроить';en='Tune'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ka='არ აიწყოს';ru='Не настраивать';en='Do not configure'"));
...
Процедура ПроверитьСписокПередОперациейОтветПоСортировкеПолучен(РезультатОтвета, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ПользовательскиеНастройкиПорядка <> Неопределено, НСтр("ka='Пользовательская настройка порядка не найдена.';ru='Пользовательская настройка порядка не найдена.';en='Custom order setting not found.'"));
