...
Процедура ИнициализироватьСеансовыеДанные(Данные, Пользователь, ОбластьДоступа, УчетнаяЗаписьЭлектроннойПочты = Неопределено) Экспорт
	...
	Если ОбластьДоступа = Перечисления.ОбластиДоступаGoogle.Почта
		...
		ВызватьИсключение НСтр("ka='ელექტრონული ფოსტის სააღრიცხვო ჩანაწერი არ არის მითითებული.';ru='Не указана учетная запись электронной почты.';en='Not available e-mail account.'");
	КонецЕсли;
...
Функция КлючИзИдентификатора(Идентификатор, КонтекстПроверки) Экспорт
	...
	НСтр("ka='იდენტიფიკატორი არ არის მითითებული მის გარდასაქმნელად გასაღებად';ru='Не указан Идентификатор для преобразования его в Ключ';en='Unknown ID to convert it into a key'"),
	КонтекстПроверки);
	
	ОбщегоНазначенияКлиентСервер.Проверить(
	ТипЗнч(Идентификатор) = Тип("Строка"),
	НСтр("ka='იდენტიფიკატორი უნდა იყოს სტრიქონული ტიპის';ru='Идентификатор должен быть строкового типа';en='The identifier must be of type string'"),
	КонтекстПроверки);
...
Процедура ИнициализироватьУзелПланаОбменаДляКалендаряGoogle() Экспорт
	...
	УзелОбъект.Наименование = НСтр("ka='ეს კვანძია';ru='Этот узел';en='this node'");
	УзелОбъект.Записать();
...
Функция ИдентификацияПриложенияGoogleКорректна(ИдентификацияПриложенияGoogle) Экспорт
	...
	Исключение
		Результат = НСтр("ka='საჭიროა JSON ფორმატში მონაცემთა მითითება';ru='Требуется указать данные в формате JSON';en='It is required to specify the data in JSON format'");
		Возврат Результат;
	КонецПопытки;
	...
	Результат = СтрШаблон(
		НСтр("ka='კლიენტის იდენტიფიკაციის არაკორექტული ფორმატი Google: მოსალოდნელია ერთ-ერთი გასაღები: %1';ru='Некорректный формат идентификации клиента Google: ожидается один из ключей: %1';en='Incorrect Google customer identification format is expected one of the keys: 1%'"),
...
"%1';ka='Во время отправки сообщения произошла ошибка:"
"%1'"),
...
Функция СоздатьУчетнуюЗаписьGoogle(СеансовыеДанные) Экспорт
	...
	Если Не ЗначениеЗаполнено(АдресЭлектроннойПочты) Тогда
		ВызватьИсключение НСтр("ka='Google-ის ელექტრონული ფოსტის მისამართის დადგენა ვერ მოხერხდა.';ru='Не удалось определить адрес электронной почты Google.';en='Unable to determine Google email address.'");
	КонецЕсли;
	...
	Для Каждого ТекОписаниеКалендаря Из ОписанияВсехКалендарей["items"] Цикл
		...
		Пока ЗначениеЗаполнено(СтраничныйТокен) Или СтраничныйТокен= "" Цикл
			...
			Если СобытияКалендаря["error"] <> Неопределено Тогда
				ВызватьИсключение 
				СтрШаблон(
				НСтр("ka='Google Calendar-თან სინქრონიზაციის შეცდომა მოხდა: %1';ru='Произошла ошибка синхронизации с Google Calendar: %1';en='There was an error syncing with Google Calendar:% 1'"),
...
Функция ВосстановитьДатуJSON(Свойство, Значение, Параметры) Экспорт
	...
	Исключение
		...
		НСтр("ka='Google-ის კალენდრის სინქრონიზაცია';ru='Синхронизация календаря Google.';en='Google Calendar Sync.'"),
		УровеньЖурналаРегистрации.Предупреждение,,,
		СтрШаблон(
		НСтр("ka='თვისება ""%1""-ისთვის თარიღ JSON-ის წაკითხვა ვერ მოხერხდა: ""%2"".';ru='Не удалось прочитать дату JSON для свойства ""%1"": ""%2"".';en='Unable to read JSON date for the property ""%1"" ""%2"".'"), 
		Свойство,
		Значение));
...
Функция СсылкаНаКалендарьСотрудника(ОписаниеКалендаря, Пользователь)
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ДанныеЗаполнения.ВладелецКалендаря) Тогда
			ВызватьИсключение СтрШаблон(
			НСтр("ka='მომხმარებელ ""%2""-ისთვის კალენდარ ""%1""-ის მფლობელის დადგენა ვერ მოხერხდა.';ru='Не удалось определить владельца календаря ""%1"" для пользователя ""%2"".';en='Unable to identify the calendar owner ""%1"" for user ""% 2.""'"),
			КалендарьСотрудника,
			Пользователь);
		КонецЕсли;
...
Функция КомпонентыПакетногоОтвета(ПакетныйОтветHTTP, Менеджер)
	...
	НСтр("ka='პაკეტური მოთხოვნის პასუხში პარამეტრი boundary არ არის აღმოჩენილი';ru='Не обнаружен параметр boundary в ответе на пакетный запрос';en='Not detected boundary parameter in response to the batch request'"),
...
Функция ЗаписатьСобытиеПоМетаданнымGMail(МетаданныеСообщения, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='ელექტრონული ფოსტის შეტყობინებების სათაურების ჩატვირთვა';ru='Загрузка заголовков сообщений электронной почты';en='Download e-mail headers'"),
