...
Функция СобытиеЖурналаРегистрации(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ka='Обновление информации о направлениях сдачи отчетности';ru='Обновление информации о направлениях сдачи отчетности';en='Updating information about the directions of delivery of the reporting'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებობის გაგზავნა';ru='Отправка отчетности';en='sending reports'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ka='Получение результатов отправки отчетности';ru='Получение результатов отправки отчетности';en='Getting results send reports'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюНачалаРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ka='Начато обновление информации о направлениях сдачи отчетности';ru='Начато обновление информации о направлениях сдачи отчетности';en='Began updating information about the directions of delivery of the reporting'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ka='Начата отправка отчетности';ru='Начата отправка отчетности';en='Started sending reports'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ka='Начато получение результатов отправки отчетности';ru='Начато получение результатов отправки отчетности';en='Started receiving reports of sending results'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюОкончанияРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებულობის ჩაბარების მიმართულებების შესახებ ინფორმაციის განახლება დასრულებულია';ru='Завершено обновление информации о направлениях сдачи отчетности';en='Completed updating information about the directions of delivery of the reporting'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებულობის გაგზავნა დასრულებულია';ru='Завершена отправка отчетности';en='Completed sending reports'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ka='ანგარიშგებულობის გაგზავნის შედეგების მიღება დასრულებულია';ru='Завершено получение результатов отправки отчетности';en='Completed obtaining statements send the results'");
	КонецЕсли;
...
Функция СсылкаПоСтрокеСтатуса(СтрокаСтатуса, ОписаниеОшибки = "")
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სერვერის პასუხის მიხედვით ანგარიშგებულობის ჩაბარების სტატუსის დადგენა ვერ მოხერხდა (%1)';ru='Не удалось определить статус сдачи отчетности по ответу сервера (%1)';en='Unable to determine the delivery of the reporting status of the server response (1%)'"), ТекстИзФайла);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПроверитьПередРегистрациейЗаявкиНаОтправкуОтчетности(Отчет, КонтролирующийОрган = "ФНС", ТекстВопроса = "") Экспорт
	...
	Если НЕ НаправлениеСдачиОтчетностиОткрыто(Организация, ТипНаправления, КодНаправления) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ორგანიზაციის თაობაზე ანგარიშგებულობის ჩაბარების მიმართულება დახურულია. გაგზავნა შეუძლებელია.';ru='Направление сдачи отчетности по организации закрыто. Отправка невозможна.';en='reporting delivery direction of the organization is closed. Not sent.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТипОтчета = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		Если АналогичныеОтправки.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ka='თქვენ უკვე გაგზავნეთ ანალოგიური ანგარიშგება კორექტირების ანალოგიური ნიშნით, იმავე პერიოდისთვის, იმავე ორგანიზაციის მიხედვით და იგივე ინსპექციაში."
""
"დარწმუნებული ხართ, რომ ანგარიშგების გაგზავნა ხელახლა გსურთ?';ru='Вы уже отправляли аналогичный отчет с аналогичным признаком корректировки,"
...
"Вы уверены, что хотите отправить отчет заново?'");
	...
	Иначе
		...
		Если ОтправкиОтчета.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ka='თქვენ უკვე გაგზავნილი გაქვთ მოცემული ანგარშგება. დარწმუნებული ხართ, რომ განმეორებით გსურთ ანგარიშგების გაგზავნა?';ru='Вы уже отправляли данный отчет."
"Вы уверены, что хотите отправить отчет заново?'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьОтправкуОтчетностиВСправочнике(ДанныеПакета, ИмяФайлаПакета, Отчет) Экспорт
	...
	Если ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФНС Тогда
		...
		ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНаправления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код инспекции-получателя отчета должен состоять строго из четырех цифр.';ru='Код инспекции-получателя отчета должен состоять строго из четырех цифр.';en='Inspection of the recipient of the report code should consist strictly of four digits.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФСГС Тогда
		...
		ИЛИ Сред(КодНаправления, 3, 1) <> "-" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XX, где X - любая цифра.';ru='Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XX, где X - любая цифра.';en='Authority code FSGS, in which the reporting organization (set in the ""Organization""), shall meet the pattern XX-XX, where X - any number.'"));
			Возврат Ложь;
		КонецЕсли;
	Иначе
		...
		ИЛИ Сред(КодНаправления, 4, 1) <> "-" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.';ru='Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.';en='Body Code FIU, in which the reporting organization (set in the ""Organization""), shall meet the pattern XXX-XXX, where X - any number.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если МетаданныеОтчета = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Неизвестный вид отчета.';ru='Неизвестный вид отчета.';en='Unknown type of report.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВидОтчета) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ცნობარ ""რეგლამენტირებული ანგარიშგებები""-ში %1 ტიპის ანგარიშგების განსაზღვრება ვერ მოიძებნა.';ru='В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.';en='The reference ""Scheduled reports"" found the definition of the form of the report% 1.'"), ИсточникОтчета));
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		ОтменитьТранзакцию();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось запланировать отправку отчета:"
""
"%1';ru='Не удалось запланировать отправку отчета:"
""
...
Функция ПолучитьСведенияИзУправляющегоПриложения()
	...
	Если НЕ ЗначениеЗаполнено(ВебАдрес) Тогда
		ЗаписьЖурналаРегистрации(СобытиеЖурнала,
								...
								Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности,
								,
								НСтр("ka='Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.';ru='Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.';en='Do not set the web service address to receive information about the directions of delivery of the reporting.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция СформироватьФайлПакета(
	...
	Для Инд = 0 По КороткиеИменаФайловВыгрузки.Количество() - 1 Цикл
		...
		Иначе
			Если НЕ ЗаписатьТекстВФайл(ЭлТекстВыгрузки, ПолноеИмяФайлаВыгрузки, ЭлКодировкаТекстаВыгрузки) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.';ru='Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.';en='Failed to schedule report delivery: unable to write temp file reporting.'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если КонтролирующийОрган = "ФНС" Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.';ru='Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.';en='Failed to schedule the sending of the report: there is no information about the power of attorney.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось запланировать отправку отчета:"
"не удалось записать файл пакета с отчетностью.';ru='Не удалось запланировать отправку отчета:"
...
Функция ИзменитьФайлВыгрузкиПриОтправкеЧерезУполномоченногоПредставителя(ТекстВыгрузки, ИмяФайлаВыгрузки, Доверенность, КодНО, ДатаПериода = '00010101')
	...
	Если нрег(РасширениеФайла) = ".xml" Тогда // формат xml - меняем ИдФайл и имя файла
		...
		Если КомпонентыИмени.Количество() < 5 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.';ru='Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.';en=""You can not change the sender's details in the file name: incorrect initial unloading of the file name."""));
			Возврат Ложь;
		КонецЕсли;
...
Функция ВыгрузитьСведенияОДоверенностиПоНаправлениюОтчета(Отчет, ДанныеДоверенности, ИмяФайлаДоверенности, Доверенность = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(РегистрацияВИФНС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось запланировать отправку отчета:"
"не найдена запись в справочнике ""Регистрации в налоговом органе"", соответствующая направлению отправки (%1).';ru='Не удалось запланировать отправку отчета:"
"не найдена запись в справочнике ""Регистрации в налоговом органе"", соответствующая направлению отправки (%1).'"), КодИФНС));
	...
	Если ФайлДоверенности = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось запланировать отправку отчета:"
"не удалось выгрузить сведения о доверенности налогоплательщика.';ru='Не удалось запланировать отправку отчета:"
