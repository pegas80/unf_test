...
Функция ПриЗагрузкеОтчетаОРозничныхПродажах(Устройство, МассивДанных, РасширеннаяЗагрузка = Ложь) Экспорт
	...
	Если РасширеннаяЗагрузка Тогда
		...
		Для Каждого ОтчетОПродажах Из МассивДанных Цикл
			...
			Комментарий = НСтр("ka='ჩატვირთულია %ТипОборудования%:%Устройство%-იდან';ru='Загружено из %ТипОборудования%:%Устройство%';en='Uploaded% of TipOborudovaniya%:% apparatus%'");
	...
	Иначе
		...
		Комментарий = НСтр("ka='ჩატვირთულია სსა-დან Offline:';ru='Загружено из ККМ Offline:';en='Uploaded from CMC Offline:'") + Символы.НПП + Устройство;
...
Процедура ОбновитьКодыТоваров(ПравилоОбмена) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='შესაერთებელი მოწყობილობის არაკორექტული ტიპი';ru='Некорректный тип подключаемого оборудования';en='Incorrect type of equipment to be connected'");
	КонецЕсли;
...
Процедура РегламентноеЗаданиеОбновитьКодыТоваров() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='შესაერთებელი მოწყობილობის საქონლის კოდების განახლება Offline';ru='Обновление кодов товаров подключаемого оборудования Offline';en='Updating the code plug the equipment goods Offline'"),
	                         УровеньЖурналаРегистрации.Информация, , ,
	                         НСтр("ka='დაწყებულია შესაერთებელი მოწყობილობის საქონლის კოდების სარეგლამენტო განახლება Offline.';ru='Начато регламентное обновление кодов товаров подключаемого оборудования Offline.';en='Initiated routine update product codes Offline connected equipment.'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='შესაერთებელი მოწყობილობის საქონლის კოდების განახლება Offline';ru='Обновление кодов товаров подключаемого оборудования Offline';en='Updating the code plug the equipment goods Offline'"),
			                         ...
			                         НСтр("ka='შესაერთებელი მოწყობილობის Offline საქონლის კოდების სარეგლამენტო განახლებისას შეცდომა მოხდა.';ru='Во время регламентного обновления кодов товаров подключаемого оборудования Offline произошла ошибка.';en='During routine updating of codes of goods connected Offline hardware error occurred.'")
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='შესაერთებელი მოწყობილობის საქონლის კოდების განახლება Offline';ru='Обновление кодов товаров подключаемого оборудования Offline';en='Updating the code plug the equipment goods Offline'"),
	                         УровеньЖурналаРегистрации.Информация, , ,
	                         НСтр("ka='შესაერთებელი მოწყობილობის საქონლის კოდების სარეგლამენტო განახლება დასრულებულია Offline.';ru='Закончено регламентное обновление кодов товаров подключаемого оборудования Offline.';en='Finished routine update product codes Offline connected equipment.'"));
...
Функция ПолучитьДанныеДляВесов(Устройство, ТолькоИзмененные = Истина) Экспорт
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		ЭлементМассива = Новый Структура("PLU, Код, Штрихкод, Наименование, Цена, ОписаниеТовара, Штучный", 0, 0, "", "" , 0, "", ЛОЖЬ);//Kuzmich, Проблемы с выгрузкой штучного товара
		...
		Если СтрокаТЧ.Используется Тогда
			...
			ЭлементМассива.Штучный		= СтрокаТЧ.Номенклатура.ЕдиницаИзмерения.Штучный;//Kuzmich, Проблемы с выгрузкой штучного товара
		КонецЕсли;
...
Функция ПолучитьТаблицуТоваровКВыгрузке(Устройство, Параметры, ОбновитьКодыТоваров = Ложь) Экспорт
	...
	// KUZMICH, 13.03.2019, Ошибка при выгрузке штучных/весовых товаров в весы
	|	И (ВЫБОР КОГДА НЕ (КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмерения = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)) ТОГДА НЕ КодыТоваровПодключаемогоОборудованияOffline.ЕдиницаИзмерения.ПометкаУдаления ИНАЧЕ ИСТИНА КОНЕЦ)
	// KUZMICH, 13.03.2019, Ошибка при выгрузке штучных/весовых товаров в весы
	...
	//Если Параметры.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток Тогда  // KUZMICH, 13.03.2019, Ошибка при выгрузке штучных/весовых товаров в весы
	...
	//+++ Дмгг
	// Костыль, как минимум пока
	Транслит_GE_EN = "a  b  g  d  e  v  z  t  i  k  l  m  n  o  p  zh r  s  t  u  p  k  gh q  sh ch ts dz ts ch kh j  h  ";  
	ПереводитьВЛатиницуНазванияПриВыгрузкеВВесы = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПереводитьВЛатиницуНазванияПриВыгрузкеВВесы");
	//--- Дмгг
	...
	Пока ВыборкаПоКодам.Следующий() Цикл
		...
		//НоваяСтрока = ТаблицаТоваров.Добавить();
		НоваяСтрока = НЕОПРЕДЕЛЕНО; // KUZMICH, 13.03.2019, Ошибка при выгрузке штучных/весовых товаров в весы
		
		Выборка = ВыборкаПоКодам.Выбрать();
		Пока Выборка.Следующий() Цикл
			
			Если Выборка.Используется Тогда // KUZMICH, 13.03.2019, Ошибка при выгрузке штучных/весовых товаров в весы
				НоваяСтрока = ТаблицаТоваров.Добавить();
			Иначе
				Прервать;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.Код) Тогда
				...
				//Если ПереводитьВЛатиницуНазванияПриВыгрузкеВВесы Тогда 
				//	Выполнить("НоваяСтрока.Номенклатура=Транслит_GE(Строка(Строка(Выборка.Номенклатура)),Транслит_GE_EN)"); 
				//Иначе      
				//	НоваяСтрока.Номенклатура            = Выборка.Номенклатура;
				//КонецЕсли;
				//--- Дмгг
				...
				Если ПереводитьВЛатиницуНазванияПриВыгрузкеВВесы Тогда   
					...
					Выполнить("НоваяСтрока.Наименование=Транслит_GE(Строка(Строка(НпименованиеНом)),Транслит_GE_EN)");
					Выполнить("НоваяСтрока.НаименованиеПолное=Транслит_GE(Строка(Строка(НпименованиеНом)),Транслит_GE_EN)");
				Иначе	    
					НоваяСтрока.Наименование                = 
						УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(
						Выборка.НоменклатураНаименование,
		...
		// KUZMICH, 14.03.2019, Ошибка при выгрузке штучных/весовых товаров в весы
		Если НоваяСтрока = НЕОПРЕДЕЛЕНО Тогда
			Продолжить;
		КонецЕсли;
		Если НоваяСтрока.Номенклатура.ЕдиницаИзмерения.Весовая Тогда
			Если Параметры.ПравилоОбмена.ПрефиксВесовогоТовара>0 Тогда
				ШтрихКодВрем = РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьШтрихкодВесовогоТовараПоКоду(НоваяСтрока.Код, Параметры.ПравилоОбмена.ПрефиксВесовогоТовара);
			Иначе				
				ШтрихКодВрем = РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьШтрихкодВесовогоТовараПоКоду(НоваяСтрока.Код, Параметры.ПрефиксВесовогоТовара);
			КонецЕсли;
		ИначеЕсли НоваяСтрока.Номенклатура.ЕдиницаИзмерения.Штучный Тогда
			Если Параметры.ПравилоОбмена.ПрефиксВесовогоТовара>0 Тогда
				ШтрихКодВрем = РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьШтрихкодВесовогоТовараПоКоду(НоваяСтрока.Код, Параметры.ПравилоОбмена.ПрефиксШтучногоТовара);
			Иначе				
				ШтрихКодВрем = РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьШтрихкодВесовогоТовараПоКоду(НоваяСтрока.Код, Параметры.ПрефиксВесовогоТовара);
			КонецЕсли;	
		Иначе
		...
		Если Параметры.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток
			И ОбновитьКодыТоваров
			И (Не (НоваяСтрока.Штрихкод=ШтрихКодВрем)) Тогда
			...
			Запрос.Текст = 
				...
				Запрос.УстановитьПараметр("Штрихкод", ШтрихКодВрем);
				
				РезультатЗапроса = Запрос.Выполнить();
				
				Если РезультатЗапроса.Пустой() Тогда
					
					Попытка  // Двойная попытка - очень некрасиво...
						МенеджерЗаписиШтрихкода = РегистрыСведений.ШтрихкодыНоменклатуры.СоздатьМенеджерЗаписи();
						МенеджерЗаписиШтрихкода.Номенклатура   = НоваяСтрока.Номенклатура;
						МенеджерЗаписиШтрихкода.Характеристика = НоваяСтрока.Характеристика;
						МенеджерЗаписиШтрихкода.Партия = НоваяСтрока.Партия;
						МенеджерЗаписиШтрихкода.ЕдиницаИзмерения = НоваяСтрока.ЕдиницаИзмерения;
						МенеджерЗаписиШтрихкода.Штрихкод       = ШтрихКодВрем;
						МенеджерЗаписиШтрихкода.Записать();
						
						НоваяСтрока.Штрихкод = ШтрихКодВрем;
						НоваяСтрока.МассивШтрихкодов.Добавить(НоваяСтрока.Штрихкод);
					Исключение
						ПредыдущееЗначениеМаксимальноеЗначениеВесовогоКода = МаксимальноеЗначениеВесовогоКода;
						МаксимальноеЗначениеВесовогоКода                   = Мин(МаксимальноеЗначениеВесовогоКода + 1, 99999);
						...
						НоваяСтрока.Штрихкод = РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьШтрихкодВесовогоТовараПоКоду(МаксимальноеЗначениеВесовогоКода, Параметры.ПрефиксВесовогоТовара);
						НоваяСтрока.МассивШтрихкодов.Добавить(НоваяСтрока.Штрихкод);
						НоваяСтрока.Код = МаксимальноеЗначениеВесовогоКода;				
...
//+++ Дмгг
Функция Транслит_GE(Стр,Транслит)
	Перем НовСтр, Смв, Поз;
	
	Алфавит_GE = "ა  ბ  გ  დ  ე  ვ  ზ  თ  ი  კ  ლ  მ  ნ  ო  პ  ჟ  რ  ს  ტ  უ  ფ  ქ  ღ  ყ  შ  ჩ  ც  ძ  წ  ჭ  ხ  ჯ  ჰ  ";
	...
	Для Сч=1 По ДлинаСтр Цикл
		Смв =Сред(Стр,Сч,1);	
		Поз =Найти(Алфавит_GE,Смв);
		
		
		Если (Поз=0) ИЛИ (Смв=" ") Тогда 
			НовСтр=НовСтр+Смв;
		Иначе
			НовСтр=НовСтр+СокрЛП(?(ВерхРег,ТРег(Сред(Транслит,Поз,3)),Сред(Транслит,Поз,3)));
		КонецЕсли;
		
		//Сообщить(""+Поз+" - "+Смв+" -- "+НовСтр);
		ВерхРег=(Смв=" ") ИЛИ (Смв=";");
