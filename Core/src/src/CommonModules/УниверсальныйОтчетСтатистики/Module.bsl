...
Процедура ПодготовитьДанныеОтчетаПоШаблону(КонтекстФормы, ДанныеШаблонаОтчета, Отказ = Ложь) Экспорт 
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='XML-შაბლონის ტექსტის მიღება ვერ მოხერხდა:%1';ru='Не удалось получить текст XML-шаблона:%1';en='Unable to get text XML-template:% 1'"), Символы.ПС + ОписаниеОшибки());
		Сообщение.Сообщить();
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки) Экспорт 
	...
	Для каждого КонтрольноеСоотношение Из ПараметрыДляПроверки.ТаблицаКонтрольныхСоотношений Цикл
		...
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ""));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ВерсияШаблона));
		...
		СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), Описание));
		Если ПараметрыДляПроверки.КвоОшибокКС > 0 Тогда
			Стр1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), "Не удалось вычислить условие! Соотношение не используется.");
			Стр2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ?(ОбязательностьКонтроля, "", "Контролировать не обязательно"));
