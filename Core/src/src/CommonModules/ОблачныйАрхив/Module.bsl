...
Процедура СобратьДанныеПоОблачномуАрхиву(ДополнительныеСвойства, АдресХранилища = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ka='Сбор данных в фоне';ru='Сбор данных в фоне';en='Data collection in the background'"));
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			НСтр("ka='Получение массива шагов';ru='Получение массива шагов';en='Getting an array of steps'"));
		...
		Если МассивШагов.Количество() > 0 Тогда
			...
			ОписаниеРезультатаВложенный = СтрШаблон(НСтр("ka='Получено %1 шагов для выполнения';ru='Получено %1 шагов для выполнения';en='Retrieved% 1 steps to perform'"), МассивШагов.Количество());
		Иначе
		...
		Для Каждого ТекущийШаг Из МассивШагов Цикл
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				ШагВыполненияВложенный,
				"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
				СтрШаблон(НСтр("ka='Выполнение шага: %1';ru='Выполнение шага: %1';en='Executing step: 1%'"),ТекущийШаг.ОписаниеШага));
...
#Область ИнформацияОКлиенте
				...
				Иначе
					КодРезультатаВложенный      = 0;
					ОписаниеРезультатаВложенный = НСтр("ka='Собранные ранее данные еще не устарели';ru='Собранные ранее данные еще не устарели';en='data collected previously has not become obsolete'");
				КонецЕсли;
...
"%3"
"';ka='Произошла ошибка %1 сбора данных на шаге %2:"
"%3"
"'"),
						КодРезультатаВложенный,
						ТекущийШаг.КодШага,
...
"';ka='Сбор данных шага %1 успешно завершен"
"'"),
						ТекущийШаг.КодШага);
			КонецЕсли;
			...
			ДлительныеОперации.СообщитьПрогресс(
				(НомерШага / МассивШагов.Количество()) * 100,
				СтрШаблон(НСтр("ka='Выполнение шага: %1. Завершено.';ru='Выполнение шага: %1. Завершено.';en='Executing step% 1. Completed.'"), ТекущийШаг.ОписаниеШага),
				Новый Структура("КодСостояния, ОписаниеРезультата",
					КодРезультатаВложенный,
					Неопределено));
	...
	ДлительныеОперации.СообщитьПрогресс(
		101, // > 100%
		НСтр("ka='СобратьДанныеПоОблачномуАрхиву. Завершено.';ru='СобратьДанныеПоОблачномуАрхиву. Завершено.';en='SobratDannyePoOblachnomuArhivu. Completed.'"),
		Новый Структура("КодСостояния, ОписаниеРезультата",
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='BIP: OblachnyyArhiv.Vse upgrade Cloud archive'"), // Имя события
		СтрШаблон(
			НСтр("ka='Облачный архив. Все обновления Облачного архива. %1';ru='Облачный архив. Все обновления Облачного архива. %1';en='Cloud archive. All updates Cloud archive. %one'"),
...
Функция ДоступнаАктивацияАгентаКопированияНаКомпьютере(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		СтрШаблон(НСтр("ka='Вызов процедуры / функции (%1)';ru='Вызов процедуры / функции (%1)';en='Calling the procedure / function (1%)'"), НаименованиеПроцедурыФункции));
...
"'"),
		...
		ШагВыполнения.ВремяНачала);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='BIP: OblachnyyArhiv.Vse upgrade Cloud archive'"), // ИмяСобытия
		СтрШаблон(
			НСтр("ka='Облачный архив. Все обновления Облачного архива. %1. Начало';ru='Облачный архив. Все обновления Облачного архива. %1. Начало';en='Cloud archive. All updates Cloud archive. %one. Start'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from the private Web service (% 1)'"), "isMachineIDAvailable"));
			...
			Если (ТипЗнч(Результат) <> ТипОбъектXDTO) // Неправильный тип
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтрШаблон(
					НСтр("ka='Веб-сервис вернул значение неправильного типа: %1';ru='Веб-сервис вернул значение неправильного типа: %1';en='Web service returned the wrong type of value:% 1'"),
					ТипЗнч(Результат));
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
				ЕстьОшибки = Истина;
			ИначеЕсли (ТипЗнч(Результат) = ТипОбъектXDTO)
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтрШаблон(
					НСтр("ka='Веб-сервис вернул ошибку с кодом: %1';ru='Веб-сервис вернул ошибку с кодом: %1';en='The Web service returns an error code:% 1'"),
					Результат.errorCode);
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
				ЕстьОшибки = Истина;
			КонецЕсли;
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			СтрШаблон(НСтр("ka='Отключение от веб-сервиса (%1)';ru='Отключение от веб-сервиса (%1)';en='Disconnecting from a web service (% 1)'"), "logout"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='BIP: OblachnyyArhiv.Vse upgrade Cloud archive'"), // ИмяСобытия
		СтрШаблон(
			НСтр("ka='Облачный архив. Все обновления Облачного архива. %1. Конец';ru='Облачный архив. Все обновления Облачного архива. %1. Конец';en='Cloud archive. All updates Cloud archive. %one. the end'"),
			НаименованиеПроцедурыФункции), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
...
"'"),
	...
	Если (ДатаПредыдущегоЗапуска + 30) <= ТекущаяДатаДляРегистров Тогда // Запускалось предыдущий раз более 30 секунд назад.
		...
		Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from the private Web service (% 1)'"), "updateAccountStats"));
			...
			Если ЕстьОшибки <> Истина Тогда
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Сохранение информации в ИБ';ru='Сохранение информации в ИБ';en='Storing information in the IB'"));
	...
	Иначе
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			...
			380,
			СтрШаблон(
				НСтр("ka='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)';ru='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)';en='The update is not carried out: the previous update was less than 30 seconds ago (it was 1%, currently 2%)'"),
	...
	Иначе // Облачный архив
		...
		МассивШагов = Новый Массив;
			...
			ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
				...
				ШагСбораДанных.ОписаниеШага          = НСтр("ka='Сбор информации о клиентском компьютере';ru='Сбор информации о клиентском компьютере';en='Collecting client information'");
			...
			ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
				...
				ШагСбораДанных.ОписаниеШага          = НСтр("ka='Проверка активации Агента резервного копирования на этом компьютере';ru='Проверка активации Агента резервного копирования на этом компьютере';en='Check the activation of a backup agent on this computer'");
				ШагСбораДанных.СрокУстареванияСекунд = 5*60; // Обновлять только если данные были собраны > 5 минут назад.
			МассивШагов.Добавить(ШагСбораДанных);
		...
		ФоновоеЗадание = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ka='Сбор данных для Облачного архива';ru='Сбор данных для Облачного архива';en='Data collection for cloud backup'"),
			Ложь);
...
Процедура ПанельАдминистрированияБСП_ПереключитьНаПравильнуюСтраницу(Форма, СобратьДанныеОКлиенте = Ложь) Экспорт
	...
	ИначеЕсли Форма.СпособРезервногоКопирования = "1С:Облачный архив" Тогда
		...
		Если ИнформацияОКлиенте.ЭтоАдминистраторWindows Тогда
			...
			КонецЕсли;
		Иначе
			Элементы.ДекорацияОблачныйАрхивОшибки.Заголовок = НСтр("ka='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.';ru='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.';en='For connectivity, backup and set up a backup agent, you must have Windows administrator rights on the computer.'");
	...
	Если ОблачныйАрхивПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
		...
		Если НЕ МодульТекущиеДелаСервер.ДелоОтключено(ИдентификаторДела) Тогда
			...
			Если Результат.ТребуетсяУстановка = Истина Тогда
				...
				Дело.Важное         = Истина;
				Дело.Представление  = НСтр("ka='Для работы 1С:Облачный архив необходимо установить агента резервного копирования';ru='Для работы 1С:Облачный архив необходимо установить агента резервного копирования';en='To work 1C: Cloud archive, you must install the backup agent'");
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
				...
				Дело.Представление  = СтрШаблон(
					НСтр("ka='Доступно обновление %1 агента резервного копирования';ru='Доступно обновление %1 агента резервного копирования';en='An update is available% 1 backup agent'"),
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 2 Тогда // Рекомендуется обновление.
				...
				Дело.Важное         = Ложь;
				Дело.Представление  = СтрШаблон(
					НСтр("ka='Рекомендуется обновление агента резервного копирования до версии %1';ru='Рекомендуется обновление агента резервного копирования до версии %1';en='It is recommended to update the backup agent to version% 1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 3 Тогда // Необходимо обновление.
				...
				Дело.Важное         = Истина;
				Дело.Представление  = СтрШаблон(
					НСтр("ka='Необходимо обновить агента резервного копирования до версии %1';ru='Необходимо обновить агента резервного копирования до версии %1';en='You must update the backup agent to version% 1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ka='Веб-сервисы авторизации в 1С';ru='Веб-сервисы авторизации в 1С';en='Web services authorization in 1C'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ka='Веб-сервисы работы с облачным архивом (публичные и приватные)';ru='Веб-сервисы работы с облачным архивом (публичные и приватные)';en='Web services are working with cloud archives (public and private)'"));
	НовыеРазрешения.Добавить(Разрешение);
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ka='Веб-сервисы информации об агенте копирования';ru='Веб-сервисы информации об агенте копирования';en='Web services information about the agent backup'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ВыгрузитьФайлВОблако(ИмяФайла, ОтсрочкаСтартаСекунд = 30) Экспорт
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ka='Чтение настроек Агента резервного копирования';ru='Чтение настроек Агента резервного копирования';en='Reading the Backup Agent'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		МассивШагов.Добавить(ШагСбораДанных);
...
Функция ПолучитьПроксиВебСервисаАвторизации1С(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Подключение к прокси веб-сервиса (авторизация 1С)';ru='Подключение к прокси веб-сервиса (авторизация 1С)';en='Connecting to the Web proxy service (authorization 1C)'"));
...
"%1';ka='Неудачное подключение к веб-сервису (авторизация 1С) по причине:"
"%1'"),
			лкПодробноеПредставлениеОшибки);
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ОблачныйАрхив.Веб-сервисы';ru='БИП:ОблачныйАрхив.Веб-сервисы';en='BIP: OblachnyyArhiv.Veb services'"), // Имя события
			НСтр("ka='Облачный архив. Веб-сервисы. Подключение к серверу';ru='Облачный архив. Веб-сервисы. Подключение к серверу';en='Cloud archive. Web services. Connecting to the server'"), // ИмяСобытия
...
Функция ПолучитьПроксиВебСервисаБэкап1СПубличный(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Подключение к прокси веб-сервиса (публичный backup.1c.ru)';ru='Подключение к прокси веб-сервиса (публичный backup.1c.ru)';en='Connecting to the Web proxy service (public backup.1c.ru)'"));
...
"%1';ka='Неудачное подключение к веб-сервису (публичный, backup.1c.ru) по причине:"
"%1'"),
			лкПодробноеПредставлениеОшибки);
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из публичного веб-сервиса (%1)';ru='Загрузка информации из публичного веб-сервиса (%1)';en='Loading information from public web service (% 1)'"), "getBackupAgentInfo"));
...
Функция ПолучитьПроксиВебСервисаБэкап1СПриватный(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если (ТипЗнч(ПроксиЛогин1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='ტიკეტის მიღება';ru='Получение тикета';en='Getting the ticket'"));
		...
		Если ЕстьОшибки <> Истина Тогда
			...
			Если (ТипЗнч(ПроксиПубличныйБэкап1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					СтрШаблон(НСтр("ka='Загрузка информации из публичного веб-сервиса (%1)';ru='Загрузка информации из публичного веб-сервиса (%1)';en='Loading information from public web service (% 1)'"), "login"));
				...
				Если ЕстьОшибки <> Истина Тогда
					...
					ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
						ШагВыполнения,
						"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
						НСтр("ka='Подключение к прокси веб-сервиса (приватный backup.1c.ru)';ru='Подключение к прокси веб-сервиса (приватный backup.1c.ru)';en='Connecting to the Web proxy service (private backup.1c.ru)'"));
...
"%1';ka='Неудачное подключение к веб-сервису (приватный, backup.1c.ru) по причине:"
"%1'"),
							лкПодробноеПредставлениеОшибки);
						// Запись в журнал регистрации.
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
Процедура ЗагрузитьИнформациюСтатусаАккаунта(КонтекстВыполнения = Неопределено)
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from the private Web service (% 1)'"), "getAccountInfo"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from the private Web service (% 1)'"), "getAccountStats"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from the private Web service (% 1)'"), "activateAccount"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ka='Загрузка информации из приватного веб-сервиса (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from the private Web service (% 1)'"), "getFileList"));
...
Процедура УдалитьРезервнуюКопию(КонтекстВыполнения) Экспорт
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Удаление файла с сервера';ru='Удаление файла с сервера';en='Deleting a file from the server'"));
			...
			Попытка
				...
				ОписаниеРезультата = СтрШаблон(
					НСтр("ka='Удаление файла с UIN %1 выполнено успешно';ru='Удаление файла с UIN %1 выполнено успешно';en='deleting a file with UIN% 1 was successful'"),
					КонтекстВыполнения.ИдентификаторФайла);
			Исключение
				Результат = -1;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
			...
			ИначеЕсли (ТипЗнч(Результат) = ТипЧисло) Тогда
				Если Результат = 0 Тогда
					// Файл удален
					ОписаниеРезультата = НСтр("ka='Файл резервной копии с идентификатором %1 успешно удален с сервера';ru='Файл резервной копии с идентификатором %1 успешно удален с сервера';en='The backup file ID% 1 was successfully removed from the server'");
					ОписаниеРезультата = СтрШаблон(
						НСтр("ka='Файл резервной копии с идентификатором %1 успешно удален с сервера.';ru='Файл резервной копии с идентификатором %1 успешно удален с сервера.';en='The backup file ID% 1 was successfully removed from the server.'"),
						КонтекстВыполнения.ИдентификаторФайла);
				Иначе // Ошибка
					ОписаниеРезультата = СтрШаблон(
						НСтр("ru='Произошла ошибка удаления файла резервной копии с сервера:"
"Код ошибки: %1"
"Идентификатор файла: %2';ka='Произошла ошибка удаления файла резервной копии с сервера:"
"Код ошибки: %1"
...
"Результат выполнения:"
"%4"
"';ka='Удаление файла резервной копии с сервера, итоговый отчет."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Веб-сервисы';ru='БИП:ОблачныйАрхив.Веб-сервисы';en='BIP: OblachnyyArhiv.Veb services'"), // Имя события
		НСтр("ka='Облачный архив. Веб-сервисы. Удаление файла резервной копии';ru='Облачный архив. Веб-сервисы. Удаление файла резервной копии';en='Cloud archive. Web services. Deleting a backup file'"), // ИмяСобытия
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
...
#Область ПолучениеСпискаОСиВерсий
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Получение списка ОС и версий ОС';ru='Получение списка ОС и версий ОС';en='Getting a list of operating systems and OS versions'"));
...
"%1';ka='Получен список ОС и версий:"
"%1'"),
...
#Область ПреобразованиеСпискаОСиВерсий
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Преобразование списка ОС и версий ОС';ru='Преобразование списка ОС и версий ОС';en='Convert list OS and OS versions'"));
...
"%1';ka='Преобразован список ОС и версий:"
"%1'"),
			ТелоОтправки);
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			ШагВыполнения,
...
#Область ЗагрузкаДанных
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполнения,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			НСтр("ka='Загрузка данных с серверов 1С';ru='Загрузка данных с серверов 1С';en='Loading data from the server 1C'"));
...
"%2"
"Результат:"
"%3';ka='Получены данные для версий Агента резервного копирования."
"Код состояния: %1"
"Заголовки:"
"%2"
"Результат:"
...
#Область ПарсингПолученныхДанных
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Парсинг загруженных данных';ru='Парсинг загруженных данных';en='Parsing of downloaded data'"));
			...
			Если (ЗагруженныеДанные.Свойство("backupAgentInfoList"))
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Парсинг загруженных данных, загрузка файлов описаний';ru='Парсинг загруженных данных, загрузка файлов описаний';en='Parsing of downloaded data, loading description files'"));
...
"%1';ka='Список URL файлов описаний:"
"%1'"),
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ka='Парсинг загруженных данных, сохранение данных в ИБ';ru='Парсинг загруженных данных, сохранение данных в ИБ';en='Parsing of downloaded data, storing data in IB'"));
...
"';ka='Для строки %1 не заполнены обязательные поля: %2"
"'"),
...
"%1"
"';ka='Обработаны данные для версий Агента резервного копирования:"
"%1"
"'"),
...
Процедура ВсеОбновленияОблачногоАрхива() Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Выполнение регламентных заданий подсистемы Облачный архив';ru='Выполнение регламентных заданий подсистемы Облачный архив';en='Performing routine tasks subsystem Cloud archive'"));
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='BIP: OblachnyyArhiv.Vse upgrade Cloud archive'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Все обновления Облачного архива. Начало';ru='Облачный архив. Все обновления Облачного архива. Начало';en='Cloud archive. All updates Cloud archive. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ОблачныйАрхив.Все обновления Облачного архива';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='BIP: OblachnyyArhiv.Vse upgrade Cloud archive'"), // ИмяСобытия
			НСтр("ka='Облачный архив. Все обновления Облачного архива. Конец';ru='Облачный архив. Все обновления Облачного архива. Конец';en='Cloud archive. All updates Cloud archive. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияОблачногоАрхива, // ОбъектМетаданных
			, // Данные
...
#Область ЗагрузкаИнформацииОбАктуальныхВерсияхАгентаКопирования
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='1. %1';ru='1. %1';en='eleven'"), ВидОбновления));
...
#Область ЗагрузкаИнформацииОбАктивированныхАгентахКопирования
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='2. %1';ru='2. %1';en='2. 1%'"), ВидОбновления));
...
#Область ЗагрузкаСпискаРезервныхКопий
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='3. %1';ru='3. %1';en='3. 1%'"), ВидОбновления));
...
#Область ЗагрузкаИнформацииОСостоянииИспользованияОблачногоАрхива
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ka='4. %1';ru='4. %1';en='4.% 1'"), ВидОбновления));
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Если Использовать = Истина Тогда
		...
		ТекущееРегламентноеЗадание.Наименование                             = НСтр("ka='Все обновления Облачного архива';ru='Все обновления Облачного архива';en='All updates Cloud archive'");
...
"в параметре Использовать передано значение неправильного типа: %1';ka='ОблачныйАрхив.ИзменитьИспользованиеРегламентныхЗаданий:"
"в параметре Использовать передано значение неправильного типа: %1'"),
			ТипЗнч(Использовать));
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // ИмяСобытия
			НСтр("ka='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';ru='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';en='Cloud archive. Service and regulations. IzmenenieIspolzovaniyaReglamentnyhZadany'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"%2"
"';ka='Описание времени некорректное: %1"
"Произошла ошибка преобразования:"
"%2"
"'"),
					ТекстВремя,
...
#Область ОсновноеОбновление
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='ძირითადი განახლება';ru='Основное обновление';en='Major update'"));
...
#Область ОсновноеОбновление_ЗагрузитьВсеСтандартныеЗначения
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Основное обновление, загрузка стандартных значений';ru='Основное обновление, загрузка стандартных значений';en='Major update, load the standard values'"));
...
#Область ПереопределяемоеОбновление
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='ხელახლა განსაზღვრული განახლება';ru='Переопределяемое обновление';en='overrides update'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';en='Cloud archive. Service and regulations. ObnovlenieInformatsionnoyBazy. Total information. First start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';en='Cloud archive. Service and regulations. ObnovlenieInformatsionnoyBazy. OblastDannyh. First start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';en='Cloud archive. Service and regulations. ObnovlenieInformatsionnoyBazy. Total information. PereytiNaVersiyu'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#Область ОсновноеОбновление_ИзменитьИспользованиеРегламентныхЗаданий
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Основное обновление, установка расписания';ru='Основное обновление, установка расписания';en='Major update, installation schedules'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ka='БИП:ОблачныйАрхив.Сервис и регламент';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='BIP: OblachnyyArhiv.Servis and regulations'"), // ИмяСобытия
		НСтр("ka='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';en='Cloud archive. Service and regulations. ObnovlenieInformatsionnoyBazy. OblastDannyh. PereytiNaVersiyu'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Функция СобратьДанные_ИнформацияОКлиенте(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Обновление информации о клиенте';ru='Обновление информации о клиенте';en='Update customer information'"));
...
#Область Шаг_ПолучениеДанныхИзРеестраWindows
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ka='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)';ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)';en='Getting information about the installed agent backup options and the computer (from the Windows registry)'"));
...
"%1"
"';ka='Ошибка получения параметров агента копирования (тип операционной системы Windows) (через объекты OLE, переменные окружения) по причине:"
"%1"
"'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
"';ka='Ошибка получения параметров агента копирования (ТипОС). Причины были указаны выше."
"'");
		ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
	КонецЕсли;
#КонецОбласти
...
"Тип платформы: %1"
"Тип ОС: %2';ka='Не получен каталог установки и версия агента, т.к. операционная система не относится к Windows."
"Тип платформы: %1"
"Тип ОС: %2'"),
...
"';ka='Данные успешно получены из реестра Windows."
"'");
		ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
	КонецЕсли;
#КонецОбласти
...
"%1"
"';ka='Ошибка получения параметров агента копирования из реестра Windows (через объекты OLE) по причине:"
"%1"
"'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"%1"
"';ka='Ошибка получения параметров агента копирования из реестра Windows (через reg.exe) по причине:"
"%1"
"'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"%1"
"';ka='Ошибка удаления временного файла для получения значения из реестра (через reg.exe) по причине:"
"%1"
"'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"%1"
"';ka='Ошибка получения параметров агента копирования из реестра Windows (через BackupAgent.exe) по причине:"
"%1"
"'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"';ka='Ошибка получения параметров агента копирования (ИдентификаторКомпьютера). Причины были указаны выше."
"'");
			ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
		КонецЕсли;
...
#Область Шаг_ПроверкаДанныхОбАгенте_Ключи
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Проверка наличия файлов программы и файлов ключей';ru='Проверка наличия файлов программы и файлов ключей';en='Check for program files and key files'"));
...
"';ka='Файл программы %1 отсутствует"
"'"),
							ТекущийФайл);
				Иначе
					ОписаниеРезультатаШага = ОписаниеРезультатаШага
...
"'"),
			...
			Если ЕстьОшибки = Истина Тогда
				...
				ОписаниеРезультатаШага = ОписаниеРезультатаШага
					+ НСтр("ka='Отсутствуют файлы, необходимые для работы Агента резервного копирования';ru='Отсутствуют файлы, необходимые для работы Агента резервного копирования';en='There are no files necessary to run a backup agent'");
				КонтекстВыполнения.КодРезультата = КонтекстВыполненияВложенный.КодРезультата; // Установить параметры корневого контекста выполнения
		...
		Иначе
			...
			ОписаниеРезультатаШага = ОписаниеРезультатаШага
				+ СтрШаблон(
					НСтр("ka='Отсутствует каталог установки Агента резервного копирования (%1)';ru='Отсутствует каталог установки Агента резервного копирования (%1)';en='No installation backup Agent directory (% 1)'"),
...
Функция СобратьДанные_НастройкиАгентаКопированияКлиент(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ka='Обновление информации о настройках агента копирования';ru='Обновление информации о настройках агента копирования';en='Update information about setting up an agent'"));
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Каталоги
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ka='Получение настроек Агента резервного копирования';ru='Получение настроек Агента резервного копирования';en='Getting the Backup Agent'"));
		...
		Если Каталог.Существует() Тогда
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				...
				0,
				СтрШаблон(
					НСтр("ka='Каталог настроек Агента копирования обнаружен: %1';ru='Каталог настроек Агента копирования обнаружен: %1';en='Agent directory settings copy found:% 1'"),
					ИмяКаталогаНастроек),
				Неопределено);
			КонтекстВыполненияВложенный.ЛогШаговВыполнения.Добавить(ШагВыполненияВложенный);
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Проверка каталогов для работы Агента резервного копирования';ru='Проверка каталогов для работы Агента резервного копирования';en='Check the directory for backup Agent'"));
				...
				КаталогиДляПроверки.Добавить(ИмяКаталогаЛогов, НСтр("ka='ლოგოების კატალოგი';ru='Каталог логов';en='logs directory'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаСтатистики, НСтр("ka='სტატისტიკის კატალოგი';ru='Каталог статистики';en='statistics catalog'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаРабочий, НСтр("ka='სამუშაო კატალოგი';ru='Рабочий каталог';en='working directory'"));
...
"';ka='%1 (%2) отсутствовал, но был успешно создан."
"'"),
									ТекущийКаталог.Представление,
									ТекущийКаталог.Значение);
						Исключение
...
"%3"
"';ka='%1 (%2) отсутствовал, создать его не удалось по причине:"
"%3"
"'"),
									ТекущийКаталог.Представление,
									ТекущийКаталог.Значение,
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ka='Загрузка настроек этой ИБ из рабочего каталога';ru='Загрузка настроек этой ИБ из рабочего каталога';en='Download settings of IB from the working directory'"));
...
"';ka='Найден файл настроек Агента резервного копирования для ИБ (%1). Производится считывание настроек."
"'"),
								ИмяФайлаНастроекИБ);
...
"';ka='Настройка расписания BackupNow пропущена."
"'");
									КонецЕсли;
	...
	#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание_Ежедневно
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("Time") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("ka='Найдена настройка расписания Time.';ru='Найдена настройка расписания Time.';en='Look for the Time schedule.'")
												+ Символы.ПС;
...
"';ka='Найдена настройка расписания BackupDaily."
"'");
...
"';ka='Найдена настройка расписания BackupDaily (начало, окончание, периодичность)."
"'");
...
"';ka='Найдена настройка расписания BackupDaily (ежедневно)."
"'");
...
"';ka='Найдена настройка расписания BackupWeekly."
"'");
...
"';ka='Найдена настройка расписания BackupMonthly."
"'");
...
"';ka='Найдена настройка расписания BackupMonthly (время, дни месяца)."
"'");
												ТекстВремя = ТекущийЭлементРасписания.Атрибуты.time;
												Время = ПолучитьВремяИзТекста(ТекстВремя, ВложенноеОписаниеРезультата);
...
"';ka='Найдена настройка расписания BackupMonthly (время, дни недели)."
"'");
													ТекстВремя = ТекущийЭлементРасписания.Атрибуты.time;
													Время = ПолучитьВремяИзТекста(ТекстВремя, ВложенноеОписаниеРезультата);
...
"';ka='Файл настроек Агента резервного копирования для ИБ НЕ найден. Будет настроено расписание по-умолчанию."
"'");
					КонецЕсли;
				Иначе
...
"';ka='Настройки расписания сгенерированы по-умолчанию."
"'");
				Иначе
...
#КонецОбласти
		...
		Иначе
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				...
				124,
				СтрШаблон(
					НСтр("ka='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.';ru='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.';en='No catalog of the Backup Agent installed (1%). It may have been manually removed.'"),
