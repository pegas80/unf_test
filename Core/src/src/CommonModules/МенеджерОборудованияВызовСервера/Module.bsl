...
Функция ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке)
	...
	Исключение
		ТекстСообщенияОбОшибке = НСтр("ka='მაკეტის XML-აღწერილობის წაკითხვისას შეცდომა მოხდა.';ru='При чтении XML-описания макета произошла ошибка.';en='When reading error occurred XML-description layout.'");
		Результат = Ложь;
	КонецПопытки;
...
Функция ПолучитьОписаниеМакета(Источник, СообщениеОбОшибке, Результат) Экспорт
	...
	Пока ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке) Цикл
		...
		Если ЧтениеXML.Имя="Formatting" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда			
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ka='ელემენტ Formatting-ის ატრიბუტ Width-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Width элемента Formatting произошла ошибка.';en='When reading the file Width item attribute Formatting error has occurred.'");
				Возврат Ложь;				
			КонецЕсли;
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ka='ელემენტ Formatting-ის ატრიბუტ Height-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Height элемента Formatting произошла ошибка.';en='When reading the file Height element attribute Formatting error has occurred.'");
				Возврат Ложь;				
			КонецЕсли;
		...
		ИначеЕсли (ЧтениеXML.Имя="Text" ИЛИ ЧтениеXML.Имя="Barcode" ИЛИ ЧтениеXML.Имя="Image" ИЛИ ЧтениеXML.Имя="UserData") И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ka='ელემენტ UserData-ის ატრიბუტ FieldName-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута FieldName элемента UserData произошла ошибка.';en='When reading the file FieldName element UserData attribute error occurred.'");
				Возврат Ложь;				
			КонецЕсли;
			...
			Если ПрочитатьАтрибут(ЧтениеXML, "TypeFill", ЗначениеАтрибута) Тогда
				...
				Если ЗначениеАтрибута=Неопределено Тогда					
					СообщениеОбОшибке = НСтр("ka='ელემენტ UserData-ის ატრიბუტ TypeFill-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута TypeFill элемента UserData произошла ошибка.';en='When reading the file TypeFill element UserData attribute error occurred.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Если ЗначениеАтрибута="Parameter" Тогда					
					...
					Если ПрочитатьАтрибут(ЧтениеXML, "Value", ЗначениеАтрибута) Тогда
						...
						Если ЗначениеАтрибута=Неопределено Тогда							
							СообщениеОбОшибке = НСтр("ka='ელემენტ UserData-ის ატრიბუტ Value-ის ფაილის მიღებისას შეცდომა მოხდა.';ru='При чтении файла атрибута Value элемента UserData произошла ошибка.';en='When reading the file Value element UserData attribute error occurred.'");
							Возврат Ложь;							
						Иначе							
			...
			Если НовоеПоле.Тип<>"UserData" Тогда				
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ Left-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Left элемента Text произошла ошибка.';en='When reading a file Left Text elements attribute error occurred.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ Top-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Top элемента Text произошла ошибка.';en='When reading the file Top Text elements attribute error occurred.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტის ფაილ Width-ის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Width элемента Text произошла ошибка.';en='When reading the file Width Text elements attribute error occurred.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ Height-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Height элемента Text произошла ошибка.';en='When reading the file Height Text elements attribute error occurred.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Если НовоеПоле.Тип="Text" Тогда
					...
					Иначе
						СообщениеОбОшибке = НСтр("ka='ელემენტ Text-ის ატრიბუტ FontSize-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута FontSize элемента Text произошла ошибка.';en='When reading the file Text elements FontSize attribute error occurred.'");
						Возврат Ложь;
					КонецЕсли;
				...
				ИначеЕсли НовоеПоле.Тип="Barcode" Тогда
					...
					Иначе
						СообщениеОбОшибке = НСтр("ka='ელემენტ Barcode-ის ატრიბუტ Type-ის ფაილის წაკითხვისას შეცდომა მოხდა.';ru='При чтении файла атрибута Type элемента Barcode произошла ошибка.';en='When reading the file Type attribute element Barcode error occurred.'");
						Возврат Ложь;
					КонецЕсли;
					...
					Если ПрочитатьАтрибут(ЧтениеXML, "FontSize", ЗначениеАтрибута) Тогда
						...
						Если ПодписьШтрихкода И ЗначениеАтрибута=Неопределено Тогда							
							СообщениеОбОшибке = НСтр("ka='ელემენტ Barcode-ის ატრიბუტ FontSize-ის ფაილის წაკითხვისას შეცდომა მოხდა. როდესაც ატრიბუტის მნიშვნელობა PrintHRI = True ატრიბუტის მნიშვნელობის მითითება FontSize სავალდებულოა!';ru='При чтении файла атрибута FontSize элемента Barcode произошла ошибка. При значении атрибута PrintHRI = True указание значения атрибута FontSize обязательно!';en='When reading the file FontSize attribute element Barcode error occurred. When the attribute value PrintHRI = True indication FontSize attribute values ​​must!'");
							Возврат Ложь;							
						КонецЕсли;
...
Функция ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода) Экспорт
	...
	Если ВнешняяКомпонента = Неопределено Тогда
		ВызватьИсключение НСтр("ka='შტრიხკოდის ბეჭდვის გარე კომპონენტის შეერთების შეცდომა!';ru='Ошибка подключения внешней компоненты печати штрихкода!';en='Error connecting external components barcode printing!'");
	КонецЕсли;
