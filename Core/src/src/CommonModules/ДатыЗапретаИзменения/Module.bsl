...
Процедура ПроверитьДатуЗапретаИзмененияПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	//Lazare 11.03.2024
	Если Типзнч(Источник)=Тип("ДокументОбъект.ПеремещениеЗапасов") Тогда   
		Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ПравоРедактированияПеремещениеЗапасовВоВремяИспользованияЗапретаРедактированияДокументов") Тогда 
			//Если Не ЕстьТТН(Источник.Ссылка) Тогда 
			Если Не ОбщегоНазначения.ЕстьТТН(Источник.Ссылка) Тогда
				Отказ = ложь;
				Возврат;
			КонецЕсли;		
		КонецЕсли;
			
		//Отказ = не (УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ПравоРедактированияЗаписанногоПеремещениеЗапасовВоВремяИспользованияЗапретаРедактированияДокументов") и ЕстьТТН(Источник.Ссылка));	 
		//Если Отказ Тогда 
		//	Возврат;
		//КонецЕсли;   
	ИначеЕсли Типзнч(Источник)=Тип("ДокументОбъект.РасходнаяНакладная") Тогда     		
		Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ПравоРедактированияРасхаднойНакладнойВоВремяИспользованияЗапретаРедактированияДокументов") Тогда  
			Если Не ОбщегоНазначения.ЕстьТТН(Источник.Ссылка) Тогда
				Отказ = ложь;
				Возврат;
			КонецЕсли;		
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Если ПоискЗапретовЗагрузки Тогда
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ka='აკრძალულ პერიოდში %1-ის ჩატვირთვა შეუძლებელია.';ru='%1 невозможно загрузить в запрещенный период.';en='% 1 can not be loaded in a prohibited period.'");
			Иначе
				Шаблон = НСтр("ka='%1 აკრძალულ პერიოდში ჩასატვირთი მონაცემებით შეცვლა შეუძლებელია.';ru='%1 в запрещенном периоде невозможно заменить загружаемыми данными.';en='1% in a prohibited period, it is impossible to replace the downloadable data.'");
			КонецЕсли;
		Иначе
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ka='აკრძალულ პერიოდში %1-ის მოთავსება შეუძლებელია.';ru='%1 невозможно поместить в запрещенный период.';en='% 1 can not be placed in a prohibited period.'");
			Иначе
				Шаблон = НСтр("ka='%1 აკრძალულ პერიოდში შეცვლა შეუძლებელია.';ru='%1 в запрещенном периоде невозможно изменить.';en='1% in a prohibited period can not be changed.'");
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого Запрет Из Запреты Цикл
		...
		Если Запрет.Раздел = Запрет.Объект Тогда
			Если Запрет.Раздел = ДатыЗапретаИзмененияСлужебныйПовтИсп.ПустойРаздел() Тогда
				Текст = Текст + НСтр("ka='თარიღს %1';ru='Дате %1';en='Date 1%'");
			Иначе
				Текст = Текст + НСтр("ka='თარიღს %1 ""%2"" ნაწილის მიხედვით';ru='Дате %1 по разделу ""%2""';en='Date% 1 under the heading ""%2""'");
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(СвойстваВстраивания.ЕдинственныйРаздел) Тогда
			Текст = Текст + НСтр("ka='თარიღს %1 ობიექტ ""%3""-ის მიხედვით';ru='Дате %1 по объекту ""%3""';en='Date 1% of object ""%3""'");
		Иначе
			Текст = Текст + НСтр("ka='თარიღს %1 ""%2"" ნაწილის ობიექტ ""%3""-ის მიხედვით';ru='Дате %1 по объекту ""%3"" раздела ""%2""';en='Date 1% of object ""%3"" of ""2%""'");
		КонецЕсли;
		Если ПоискЗапретовЗагрузки Тогда
			Текст = Текст + " " + НСтр("ka='შეესაბამება მონაცემთა ჩატვირთვის აკრძალვა';ru='соответствует запрет загрузки данных';en='corresponds to the data you want to load'") + " ";
		Иначе
			Текст = Текст + " " + НСтр("ka='შეესაბამება მონაცემთა შეცვლის აკრძალვა';ru='соответствует запрет изменения данных';en='corresponds to the data change prohibition'") + " ";
		КонецЕсли;
		Если Запрет.Адресат = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей Тогда
			Текст = Текст + НСтр("ka='ყველა მომხმარებლისთვის';ru='для всех пользователей';en='for all users'");
			
		ИначеЕсли Запрет.Адресат = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехИнформационныхБаз Тогда
			Текст = Текст + НСтр("ka='ყველა საინფორმაციო ბაზისთვის';ru='для всех информационных баз';en='for all data bases'");
		...
		ИначеЕсли ТипЗнч(Запрет.Адресат) = Тип("СправочникСсылка.ГруппыПользователей")
			...
			Текст = Текст + НСтр("ka='მომხმარებელთა ჯგუფ ""%4""-ისთვის';ru='для группы пользователей ""%4""';en='user group ""%4""'");
		...
		ИначеЕсли ТипЗнч(Запрет.Адресат) = Тип("СправочникСсылка.Пользователи")
			...
			Текст = Текст + НСтр("ka='მომხმარებლისთვის ""%4""';ru='для пользователя ""%4""';en='User ""%4""'");
			
		ИначеЕсли ЗначениеЗаполнено(Запрет.Адресат) Тогда
			Текст = Текст + НСтр("ka='საინფორმაციო ბაზა ""%4""-სთვის';ru='для информационной базы ""%4""';en=""for information database '% 4'""");
		Иначе
			Текст = Текст + НСтр("ka='ყველა საინფორმაციო ბაზისთვის ""%6""';ru='для всех информационных баз ""%6""';en='for all information database ""% 6""'");
		КонецЕсли;
		Текст = Текст + " " + НСтр("ka='%5-ით';ru='по %5';en='of 5%'");
		Если НЕ СвойстваВстраивания.БезРазделовИОбъектов Тогда
			Если ЗначениеЗаполнено(Запрет.Раздел) Тогда
				Если Запрет.Объект = Запрет.Раздел Тогда
					Текст = Текст + " " + НСтр("ka='(განყოფილება ""%2"" -ზე დაყენებულია აკრძალვა)';ru='(запрет установлен на раздел ""%2"")';en='(Prohibition is set to ""2%"" section)'");
				ИначеЕсли ЗначениеЗаполнено(СвойстваВстраивания.ЕдинственныйРаздел) Тогда
					Текст = Текст + " " + НСтр("ka='(ობიექტ ""%3"" -ზე დაყენებულია აკრძალვა)';ru='(запрет установлен на объект ""%3"")';en='(Prohibition is set to ""3%"" object)'");
				Иначе
					Текст = Текст + " " + НСтр("ka='(განყოფილება ""%2"" -ის ობიექტ ""%3"" -ზე დაყენებულია აკრძალვა)';ru='(запрет установлен на объект ""%3"" раздела ""%2"")';en='(Prohibition is set to the object ""%3"" of ""2%"")'");
				КонецЕсли;
			Иначе
				Текст = Текст + " " + НСтр("ka='(დადგენილია აკრძალვის საერთო თარიღი)';ru='(установлена общая дата запрета)';en='(Set the overall date of the ban)'");
			КонецЕсли;
		КонецЕсли;
	...
	Если ЭтоРегистр Тогда
		...
		Если КоличествоПолей = 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ka='ველთან';ru='с полем';en='the field'")  + " " + Строка(Данные.Отбор);
			
		ИначеЕсли КоличествоПолей > 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ka='ველებთან';ru='с полями';en='with fields'") + " " + Строка(Данные.Отбор);
		КонецЕсли;
...
//+++ Lazare_13.12.2021
Процедура ПравоРедактированияПроведенныхДокументовРеализацияПеремещенияПроизводство(Источник,Отказ,РежимЗаписи,РежимПроведения) Экспорт
	
	Если Источник.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
	
	Если Источник.Ссылка.Проведен и Типзнч(Источник)=Тип("ДокументОбъект.РасходнаяНакладная")  Тогда
		...
		Если ЗапретРедактирования Тогда
			ТекстСообщения = "მოცემულ მომხმარებელს არა აქვს გატარებული რეალიზაციის, გადაადგილების და წარმოების რედაქტირების უფლება!";
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(Источник,ТекстСообщения,,,,Отказ);
		КонецЕсли;	
	ИначеЕсли  Источник.Ссылка.Проведен и Типзнч(Источник)=Тип("ДокументОбъект.ПеремещениеЗапасов") Тогда 
		ЗапретРедактирования = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияПроведенныхДокументовПеремещениеЗапасов");
		Если ЗапретРедактирования Тогда
			ТекстСообщения = "მოცემულ მომხმარებელს არა აქვს გატარებული რეალიზაციის, გადაადგილების და წარმოების რედაქტირების უფლება!";
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(Источник,ТекстСообщения,,,,Отказ);
		КонецЕсли;		
	ИначеЕсли  Источник.Ссылка.Проведен и Типзнч(Источник)=Тип("ДокументОбъект.СборкаЗапасов") Тогда 
		ЗапретРедактирования = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияПроведенныхДокументовСборкаЗапасов");
		Если ЗапретРедактирования Тогда
			ТекстСообщения = "მოცემულ მომხმარებელს არა აქვს გატარებული რეალიზაციის, გადაადგილების და წარმოების რედაქტირების უფლება!";
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(Источник,ТекстСообщения,,,,Отказ);
		КонецЕсли;		
	//Paganini_{
	//ИначеЕсли (Источник.Ссылка.Проведен или ЗначениеЗаполнено(Источник.Ссылка))и Типзнч(Источник)=Тип("ДокументОбъект.ПриходнаяНакладная") Тогда 
	//	ЗапретРедактирования = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированиеПроведенногоЗаписаногоДокументаПриходнаяНакладная");
	//	Если ЗапретРедактирования Тогда
	//		ТекстСообщения = "მოცემულ მომხმარებელს არა აქვს გატარებული(ჩაწერილი) შემოსავლის ზედდებულის რედაქტირების უფლება!";
	//		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(Источник,ТекстСообщения,,,,Отказ);
	//	КонецЕсли;
	//}_Paganini
	ИначеЕсли Типзнч(Источник) = Тип("ДокументОбъект.ПриходнаяНакладная") И ЗначениеЗаполнено(Источник.Ссылка) И
			  УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияДокументаПриходнаяНакладнаяСозданнымДругимПользователем") И
			  НЕ Источник.Автор = Пользователи.ТекущийПользователь() Тогда 
			ТекстСообщения = "მოცემულ მომხმარებელს არა აქვს სხვა მომხმარებლის მიერ შექმნილი შემოსავლის ზედდებულის რედაქტირების უფლება!";
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(Источник,ТекстСообщения,,,,Отказ);
	КонецЕсли;     
