...
&НаСервере
Функция ПолучитьВариантРаспознаванияШтрихкода() Экспорт
...
&НаСервере
Функция ПолучитьКодЕдиницыИзмерения(Номенклатура) Экспорт
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если ЗначениеЗаполнено(ЕдиницаИзмерения) Тогда
			...
			Результат = ЕдиницаИзмерения.Код;
...
Функция ПоискПУ_ВЭквайреновомТерминале(ПодключаемоеОборудование) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ЭквайринговыеТерминалы.ИспользовыатьВнешнееПечатноеОборудование
		|			ТОГДА ЭквайринговыеТерминалы.ПечатноеУстройство
		|		ИНАЧЕ ЭквайринговыеТерминалы.ПодключаемоеОборудование
		|	КОНЕЦ КАК ПечатноеУстройство
		|ИЗ
		|	Справочник.ЭквайринговыеТерминалы КАК ЭквайринговыеТерминалы
		|ГДЕ
		|	ЭквайринговыеТерминалы.ПодключаемоеОборудование = &ПодключаемоеОборудование";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ПУ = ВыборкаДетальныеЗаписи.ПечатноеУстройство
	КонецЦикла;
	
	Возврат ПУ;

КонецФункции // ПоискПУ_ВЭквайреновомТерминале()

Функция ПолучитьЗначениеКонстанты(ИмяКонстанты) Экспорт
	Возврат Константы[ИмяКонстанты].Получить();
КонецФункции

Функция ПолучитьПредставлениеТипаРесурса(ПорядокРесурса) Экспорт
	Результат = "";
	
	Если ПорядокРесурса = 1 Тогда
		Результат = НСтр("ru='Начальный остаток';ka='დასაწყისი'");
	ИначеЕсли ПорядокРесурса = 2 Тогда
		Результат = НСтр("ru='Приход';ka='შემოსავალი'");
	ИначеЕсли ПорядокРесурса = 3 Тогда
		Результат = НСтр("ru='Расход';ka='გასავალი'");
	ИначеЕсли ПорядокРесурса = 4 Тогда
		Результат = НСтр("ru='Конечный остаток';ka='დასასრული'");
	КонецЕсли;	
