...
Функция КодСериализацииПочтовогоИндекса() Экспорт
	...
	Возврат КодСериализацииОбъектаАдресации(НСтр("ka='საფოსტო ინდექსი';ru='Почтовый индекс';en='Postcode'"));
...
Функция СокращенияОбъектовАдресацииАдресаРФ() Экспорт
	...
	Результат.Вставить(НСтр("ka='სახლი';ru='Дом';en='House'"), НСтр("ka='д.';ru='Д.';en='D.'"));
	Результат.Вставить(НСтр("ka='მფლობელობა';ru='Владение';en='possession'"), НСтр("ka='Вл.';ru='Вл.';en='Ow.'"));
	Результат.Вставить(НСтр("ka='სახლის მფლობელობა';ru='Домовладение';en='house and grounds'"), НСтр("ka='სახლმფლობელობა';ru='Домовл.';en='Domovl.'"));
	
	Результат.Вставить(НСтр("ka='კორპუსი';ru='Корпус';en='housing'"), НСтр("ka='კორპ.';ru='Корп.';en='Bldg.'"));
	Результат.Вставить(НСтр("ka='Строение';ru='Строение';en='Structure'"), НСтр("ka='Стр.';ru='Стр.';en='P.'"));
	Результат.Вставить(НСтр("ka='ლიტერა';ru='Литера';en='letter'"), НСтр("ka='ლიტ.';ru='Лит.';en='Lit.'"));
	Результат.Вставить(НСтр("ka='შენობა';ru='Сооружение';en='structure'"), НСтр("ka='ნაგებობა';ru='Сооруж.';en='Constructed.'"));
	Результат.Вставить(НСтр("ka='უბანი';ru='Участок';en='lot'"), НСтр("ka='Уч.';ru='Уч.';en='Ouch.'"));
	
	Результат.Вставить(НСтр("ka='ბინა';ru='Квартира';en='Apartment'"), НСтр("ka='კვ.';ru='Кв.';en='Sq.'"));
	Результат.Вставить(НСтр("ka='Офис';ru='Офис';en='Office'"), НСтр("ka='оф.';ru='Оф.';en='Of.'"));
	Результат.Вставить(НСтр("ka='Бокс';ru='Бокс';en='Boxing'"), НСтр("ka='Бокс';ru='Бокс';en='Boxing'"));
	Результат.Вставить(НСтр("ka='სათავსო';ru='Помещение';en='room'"), НСтр("ka='Пом.';ru='Пом.';en='Pom.'"));
	Результат.Вставить(НСтр("ka='ოთახი';ru='Комната';en='Room'"), НСтр("ka='Ком.';ru='Ком.';en='Com.'"));
	Результат.Вставить(НСтр("ka='Этаж';ru='Этаж';en='Floor'"), НСтр("ka='Этаж';ru='Этаж';en='Floor'"));
...
Функция СтруктураЧастейАдресаНаселенногоПункта(ВариантКлассификатора = "ФИАС") Экспорт
	...
	Результат.Вставить("Регион",           ЭлементАдреснойСтруктуры(НСтр("ka='რეგიონი';ru='Регион';en='Region'"),      НСтр("ka='მისამართის რეგიონი';ru='Регион адреса';en='region address'"),           "СубъектРФ",     1));
	Если ВариантКлассификатора = "ФИАС" Тогда
		Результат.Вставить("Округ",        ЭлементАдреснойСтруктуры(НСтр("ka='ოლქი';ru='Округ';en='county'"),       НСтр("ka='მისამართის არეალი';ru='Округ адреса';en='District address'"),            "Округ",         2));
	КонецЕсли;
	Результат.Вставить("Район",            ЭлементАдреснойСтруктуры(НСтр("ka='რაიონი';ru='Район';en='District'"),       НСтр("ka='მისამართის რაიონი';ru='Район адреса';en='district address'"),            "СвРайМО/Район", 3));
	Результат.Вставить("Город",            ЭлементАдреснойСтруктуры(НСтр("ka='ქალაქი';ru='Город';en='Town'"),       НСтр("ka='მისამართის ქალაქი';ru='Город адреса';en='city ​​address'"),            "Город",         4));
	Если ВариантКлассификатора = "ФИАС" Тогда
		Результат.Вставить("ВнутригРайон", ЭлементАдреснойСтруктуры(НСтр("ka='Внутр. р-н.';ru='Внутр. р-н.';en='Ext. rn.'"), НСтр("ka='შიდასაქალაქო რაიონი';ru='Внутригородской район';en='inner city areas'"),   "ВнутригРайон",  5));
	КонецЕсли;
	Результат.Вставить("НаселенныйПункт", ЭлементАдреснойСтруктуры(НСтр("ka='დასახლებული პუნქტი';ru='Нас.пункт';en='Locality'"),
		НСтр("ka='დასახლებული პუნქტი მისამართები';ru='Населенный пункт адреса';en='Location address'"), "НаселПункт",  6, Истина));
		
	Если ВариантКлассификатора <> "БезКлассификатора" Тогда
		Результат.Вставить("Улица", ЭлементАдреснойСтруктуры(НСтр("ka='ქუჩა';ru='Улица';en='Street'"),
			НСтр("ka='ქუჩა მისამართები';ru='Улица адреса';en='Street address'"), "Улица", 7));
		Результат.Вставить("ДополнительныйЭлемент", ЭлементАдреснойСтруктуры(НСтр("ka='ДополнительныйЭлемент';ru='ДополнительныйЭлемент';en='DopolnitelnyyElement'"),
			НСтр("ka='მისამართის დამატებითი ელემენტი';ru='Дополнительный элемент адреса';en='Additional element addresses'"), "ДопАдрЭл[ТипАдрЭл='10200000']", 90));
		Результат.Вставить("ПодчиненныйЭлемент", ЭлементАдреснойСтруктуры(НСтр("ka='დაქვემდებარებული ელემენტი';ru='Подчиненный элемент';en='A subordinate element'"),
			НСтр("ka='მისამართის დაქვემდებარებული ელემენტი';ru='Подчиненный элемент адреса';en='Slave address element'"), "ДопАдрЭл[ТипАдрЭл='10400000']", 91));
	КонецЕсли;
