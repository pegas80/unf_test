...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ka='ახორციელებს ინფორმაციის განახლებას ფორმა 1С-ანგარიშგებულობაში ყველა ნაწილში, ""ანგარიშგებების"" გარდა. შესრულების დამთავრებამდე მონაცემები ფორმა 1С-ანგარიშგებულობაში შეიძლება არაკორექტულად გამოისახოს';ru='Выполняет обновление информации в форме 1С-Отчетность во всех разделах, кроме Отчеты. До завершения выполнения данные в форме 1С-Отчетность могут отображаться некорректно';en='Updates the information in the form 1C-reporting in all sections, except for reports. Prior to the completion of the data in the form of 1C-reporting may not be displayed correctly'");
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор();
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Комментарий = НСтр("ka='ახორციელებს ჩატვირთული ანგარიშგებების სახელწოდების განახლებას.';ru='Выполняет обновление наименования загруженных отчетов.';en='Updates the name of the downloaded reports.'");
...
"До завершения обработки рекомендуется воздержаться от формирования и отправки документов в ответ на требование о представлении документов в ФНС';ka='საგადასახადო სამსახურში გასაგზავნად განკუთვნილი სკანირებული დოკუმენტების გამოსახულების ფაილები გარდაიქმნება. დამუშავების დამთავრებამდე, რეკომენდებულია საგადასახადო სამსახურში დოკუმენტების წარდგენის თაობაზე მოთხოვნის საპასუხოდ, დოკუმენტების შედგენისგან და გაგზავნისგან თავის შეკავება'");
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор();
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Комментарий = НСтр("ka='წარმოებს დოკუმენტების ერთიანი სიის მონაცემთა შევსება საგადასახადო სამსახურში წარსადგენად. "
"დამუშავების დასრულებამდე რეკომენდებულია, რომ საგადასახადო სამსახურში დოკუმენტების წარდგენის მოთხოვნის საპასუხოდ, თავი შეიკავოთ დოკუმენტების შედგენისგან და გაგზავნისგან';ru='Заполняются данные единого списка документов для представления в ФНС. "
...
"До завершения обработки рекомендуется воздержаться от формирования и отправки ответов на требование о представлении документов в ФНС';ka='Добавляются новые виды документов для ответа на требование ФНС. "
...
Процедура РазнестиДублиПредопределенныхЭлементовВИБ()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='БРО. Восстановление предопределенных элементов';ru='БРО. Восстановление предопределенных элементов';en='BRO. Restoring predefined elements'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
