...
//						Текст,
//						0,
//						НСтр("ka='ინფორმაცია';ru='Информация';en='Information'"));
//				КонецЕсли;
//			Иначе
//				// Другие действия
//			КонецЕсли;
	...
	Если НайденноеДействие <> Неопределено Тогда
		...
		Если НайденноеДействие.Значение = "ПодключитьЭДО" Тогда
			...
			Иначе
				...
				ТекстПредупреждения = НСтр("ka='კონტრაგენტებთან ელექტრონული დოკუმენტების გაცვლის შესაძლებლობის ჩასართავად მიმართეთ ადმინისტრატორს.';ru='Для включения возможности обмена электронными документами с контрагентами обратитесь к администратору.';en='To turn to contractors for electronic document exchange capabilities, contact your administrator.'");
				ПоказатьПредупреждение(,ТекстПредупреждения);
		...
		ИначеЕсли НайденноеДействие.Значение = "Отписаться" Тогда
			...
			Если Результат.Успешно Тогда
				ПоказатьОповещениеПользователя(
					НСтр("ka='ახალი ამბების მიღებაზე უარის თქმა';ru='Отказ получения новостей';en='Disclaimer receive news'"),
					,
					СтрШаблон(НСтр("ka='ახალი ამბების ლენტი ""%1"" გამორთულია';ru='Лента новостей ""%1"" отключена';en='News Feed ""%1"" is disabled'"), Результат.ПредставлениеЛентыНовостей),
