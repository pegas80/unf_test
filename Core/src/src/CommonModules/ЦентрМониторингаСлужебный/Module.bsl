	...
	Если (ПараметрыЦентраМониторинга.ВключитьЦентрМониторинга ИЛИ ПараметрыЦентраМониторинга.ЦентрОбработкиИнформацииОПрограмме) И ЭтоГлавныйУзел() Тогда 
		Если ПараметрыЦентраМониторинга.РегистрироватьДампы И ДатаЗапуска >= ПараметрыЦентраМониторинга.РегистрацияДамповСледующееФормирование Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - დამპების რეგისტრაცია';ru='Центр мониторинга - регистрация дампов';en='monitoring center - registration of dumps'",
		...
		Если ПараметрыЦентраМониторинга.РегистрироватьБизнесСтатистику И ДатаЗапуска >= ПараметрыЦентраМониторинга.БизнесСтатистикаСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - სტატისტიკის ოპერაციების რეგისტრაცია';ru='Центр мониторинга - регистрация операций статистики';en='monitoring center - registration statistics operations'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		Если (ПараметрыЦентраМониторинга.РегистрироватьСтатистикуКонфигурации ИЛИ ПараметрыЦентраМониторинга.РегистрироватьНастройкуКонфигурации) И ДатаЗапуска >= ПараметрыЦентраМониторинга.СтатистикаКонфигурацииСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - კონფიგურაციის სტატისტიკის შეკრება';ru='Центр мониторинга - собрать статистику конфигурации';en='monitoring center - to collect configuration Statistics'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		Если ДатаЗапуска >= ПараметрыЦентраМониторинга.ОтправкаДанныхСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - გასაგზავნად პაკეტის ჩამოყალიბება';ru='Центр мониторинга - сформировать пакет для отправки';en='monitoring center - to form a package to be sent'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ka='მონიტორინგის ცენტრი - მონიტორინგის მონაცემების გაგზავნა';ru='Центр мониторинга - отправить данные мониторинга';en='monitoring center - send monitoring data'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
...
Процедура АгрегироватьЗамерыОперацийСтатистики(ТекущаяДата)
	...
	Если ОбработатьЗаписиДо > ГраницаАгрегирования Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. სტატისტიკის ოპერაციების გაზომვების აგრეგირება';ru='Центр мониторинга.Агрегировать замеры операций статистики';en='Center monitoringa.Agregirovat measurement statistics operations'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
			ВызватьИсключение Ошибка;
		КонецПопытки;
	КонецЕсли;
...
Процедура УдалитьУстаревшиеДанныеОперацийСтатистики()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. სტატისტიკის ოპერაციების მოძველებული მონაცემების წაშლა';ru='Центр мониторинга.Удалить устаревшие данные операций статистики';en='Center monitoringa.Udalit outdated statistical data operations'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если РегистрыСведений.ОбластиСтатистики.СобиратьСтатистикуКонфигурации(ОбластьДанныхСтрока) Тогда
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. კონფიგურაციის სტატისტიკა ხელახლა განსაზღვრებადი';ru='Центр мониторинга.Статистика конфигурации переопределяемая';en='Center monitoringa.Statistika configuration redefined'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,
						,,
						НСтр("ka='სეანსის განცალკევების დაყენება ვერ მოხერხდა. მონაცემთა არეალი';ru='Не удалось установить разделение сеанса.Область данных';en='Failed to set data separation seansa.Oblast'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " = " + Формат(Выборка.ОбластьДанных, "ЧГ=0")
...
Процедура СформироватьПакетДляОтправки()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. პაკეტის შედგენა გასაგზავნად';ru='Центр мониторинга.Формирование пакета для отправки';en='Center monitoringa.Formirovanie packet to send'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыОтправки(Параметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. გაგზავნის პარამეტრების დაყენება';ru='Центр мониторинга.Установка параметров отправки';en='Center monitoringa.Ustanovka options for sending'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыЦентраМониторинга(НовыеПараметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. მონიტორინგის ცენტრის პარამეტრების დაყენება';ru='Центр мониторинга.Установка параметров центра мониторинга';en='monitoringa.Ustanovka center monitoring center settings'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция УдалитьПараметрыЦентраМониторинга()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი. მონიტორინგის ცენტრის პარამეტრების წაშლა';ru='Центр мониторинга.Удалить параметры центра мониторинга';en='Center monitoringa.Udalit parameters monitoring center'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция НайтиКаталогДампов(КаталогНастроек) 
	...
	Если Файл.Существует() Тогда
		Попытка
			...
			КонецЦикла;
		Исключение
			Сообщение = НСтр("ka='ტექნოლოგიური ჟურნალის გამართვების ფაილის წაკითხვის შეცდომა';ru='Ошибка чтения файла настроек технологического журнала';en='Error reading file technological log settings'");
			Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
			Сообщение = Сообщение + НСтр("ka='Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';ru='Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';en='Most likely the file is corrupted. Register dump is not possible. Delete the damaged file or restore the settings.'");
			
			ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი';ru='Центр мониторинга';en='monitoring center'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
		...
		Если ПараметрыДампов <> Неопределено Тогда
			Если НЕ ПараметрыДампов.Свойство("location") ИЛИ НЕ ПараметрыДампов.Свойство("create") ИЛИ НЕ ПараметрыДампов.Свойство("type") Тогда
				Сообщение = НСтр("ka='Ошибка секции сбора дампов в файле настроек технологического журнала';ru='Ошибка секции сбора дампов в файле настроек технологического журнала';en='Error section collecting dump file technological log settings'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + НСтр("ka='Регистрация дампов не возможна. Удалите файл или восстановите настройки.';ru='Регистрация дампов не возможна. Удалите файл или восстановите настройки.';en='Register dump is not possible. Delete the file or restore the settings.'");
				ЧтениеXML.Закрыть();
	...
	ЧтениеXML.Закрыть();
		...
		Если ВРЕГ(ПервыйДочерний.ИмяУзла) = "CONFIG" Тогда
			...
			Попытка
				...
				ЗаписьXML.Закрыть();
			Исключение
				Сообщение = НСтр("ka='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';ru='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';en='Error writing file technological log settings. Register dump is not possible.'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				
				ЗаписьЖурналаРегистрации(НСтр("ka='მონიტორინგის ცენტრი - დამპების რეგისტრაცია';ru='Центр мониторинга - регистрация дампов';en='monitoring center - registration of dumps'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
...
Функция СоздатьФайлНастроекСбораДампов(ПутьКаталога)
	...
	Попытка
		...
		ЗаписьXML.Закрыть();
	Исключение
		Сообщение = НСтр("ka='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';ru='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';en='Error creating file technological log settings. Register dump is not possible.'");
...
Функция ПолучитьКаталогНастроекТехнологическогоЖурнала()
	...
	Пока ПоискКаталога = Истина Цикл
		...
		Если КаталогиНастроек.Найти(ПутьКаталога) <> Неопределено Тогда
			...
			КаталогНастроек.ОписаниеОшибки = НСтр("ka='აღმოჩენილია ციკლური ბმული';ru='Обнаружена циклическая ссылка';en='Circular reference found'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Иначе
				...
				Иначе
					КаталогНастроек.Путь = "";
					КаталогНастроек.Существует = Ложь;
					КаталогНастроек.ОписаниеОшибки = НСтр("ka='კატალოგში გამართვების კონფიგურაციის ფაილი ვერ მოიძებნა';ru='Не найден файл конфигурации настроек в каталоге';en='Unable to find settings configuration file in the directory'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ПутьКаталога;
...
Функция ПолучитьКаталогИзФайлаКонфигурацииНастроек(ФайлКонфигурацииНастроек)
	...
	Иначе
		...
		КаталогНастроек.Существует = Ложь;
		КаталогНастроек.ОписаниеОшибки = НСтр("ka='ფაილში სექცია ConfLocation-ი ვერ მოიძებნა';ru='Не найдена секция ConfLocation в файле';en='Found ConfLocation section in the file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ФайлКонфигурацииНастроек.ПолноеИмя;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииЦентрМониторингаУдалениеДампа()
	Возврат НСтр("ka='მონიტორინგის ცენტრი. დამპის წაშლა';ru='Центр мониторинга.Удаление дампа';en='Center monitoringa.Udalenie dump'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

Функция СобытиеЖурналаРегистрацииЦентрМониторингаРазобратьБуферОперацийСтатистики()
	Возврат НСтр("ka='მონიტორინგის ცენტრი. სტატისტიკის ოპერაციების ბუფერის გარჩევა';ru='Центр мониторинга.Разобрать буфер операций статистики';en='Center monitoringa.Razobrat buffer statistics operations'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
#КонецОбласти
