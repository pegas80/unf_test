...
Процедура УдалениеДанныхПоСпискуURL_ПослеУдаленияЭлементаСписка(Результат, Контекст) Экспорт
	...
	Если НЕ Результат.Результат Тогда
		ТекстОшибки = НСтр("ka='Не удалось удалить документ %1';ru='Не удалось удалить документ %1';en='Failed to delete the document% 1'");
...
Процедура ФормированиеИсходящегоЗапроса_ПослеВыбораТранспортногоМодуля(ТранспортныйМодуль, Контекст) Экспорт
	...
	Если (Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросОрганизаций")
		...
		Если Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции") Тогда
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ka='Введите ИНН производителя или импортера алкогольной продукции';ru='Введите ИНН производителя или импортера алкогольной продукции';en='Enter the VAT number of the manufacturer or importer of alcoholic beverages'"), 12);
		Иначе
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ka='Введите ИНН производителя, импортера или собственной организации';ru='Введите ИНН производителя, импортера или собственной организации';en='Enter INN manufacturer, importer, or private organizations'"), 12);
		КонецЕсли;
	...
	ИначеЕсли (Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправкиА")
		...
		Если Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправкиА") Тогда
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ka='Введите регистрационный номер справки 1';ru='Введите регистрационный номер справки 1';en='Enter the registration number of the information 1'"), 50);
		Иначе
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ka='Введите регистрационный номер справки 2';ru='Введите регистрационный номер справки 2';en='Enter the registration number of the information 2'"), 50);
		КонецЕсли;
	...
	ИначеЕсли Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН")
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ФормированиеИсходящегоЗапроса_ПослеВводаДополнительныхПараметров", ЭтотОбъект, Контекст);
		ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ka='Введите идентификатор ТТН';ru='Введите идентификатор ТТН';en='Enter the ID Waybill'"), 50);
...
Процедура ФормированиеИсходящегоЗапроса_ПослеВводаДополнительныхПараметров(ПараметрЗапроса, Контекст) Экспорт
	...
	Если Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросОрганизаций")
		...
		Если ПустаяСтрока(ПараметрЗапроса) Тогда
			ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Не введен ИНН.';ru='Не введен ИНН.';en='Not introduced VAT.'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправкиА")
		...
		Если ПустаяСтрока(ПараметрЗапроса) Тогда
			ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Не введен регистрационный номер.';ru='Не введен регистрационный номер.';en='Do not put the registration number.'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН") Тогда
		...
		Если ПустаяСтрока(ПараметрЗапроса) Тогда
			ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Не введен идентификатор ТТН.';ru='Не введен идентификатор ТТН.';en='Do not enter the ID Waybill.'"));
			Возврат;
		КонецЕсли;
...
Процедура ЗагрузкаДокументов_ПослеВыбораТранспортногоМодуля(ТранспортныйМодуль, Контекст) Экспорт
	...
	Если НЕ ТранспортныйМодуль.ЗагружатьВходящиеДокументы Тогда
		ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Загрузка документов запрещена.';ru='Загрузка документов запрещена.';en='Loading documents is prohibited.'"));
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьВходящиеДанныеДляПодписиЧека(ПараметрыЧека, РезультатПодготовки)
	...
	Если ТипЗнч(ПараметрыЧека) = Тип("Массив") Тогда
		...
		Если ТаблицаТоваров.Количество() > 0 И ТаблицаТоваров[0].Количество() < 18 ИЛИ ШапкаЧека.Количество() < 21 Тогда
			РезультатПодготовки.ОписаниеОшибки = НСтр("ka='В процедуру подготовки входящих данных передана некорректная структура параметров.';ru='В процедуру подготовки входящих данных передана некорректная структура параметров.';en='The procedure for the preparation of input data structure passed incorrect parameters.'");
...
Функция ПроверитьВходящиеДанныеПередПодписьюЧека(ТранспортныйМодуль, ПараметрыЧекаЕГАИС)
	...
	Если ПустаяСтрока(ПараметрыЧекаЕГАИС.ИНН) Тогда
		ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ka='Не указан ИНН организации.';ru='Не указан ИНН организации.';en='Unknown INN organization.'"), "");
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыЧекаЕГАИС.КПП) И СтрДлина(ПараметрыЧекаЕГАИС.ИНН) = 10 Тогда
		ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ka='Не указан КПП торгового объекта.';ru='Не указан КПП торгового объекта.';en='Unknown PPC commercial facility.'"), "");
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыЧекаЕГАИС.АдресМагазина) Тогда
		ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ka='Не указан адрес магазина.';ru='Не указан адрес магазина.';en='Unknown store address.'"), "");
	КонецЕсли;
...
"Не указан штрихкод марки алкогольной продукции.'"), СтрокаЧека.Наименование);
		...
		Если МассивКодовМарок.Найти(СокрЛП(СтрокаЧека.ШтрихкодМарки)) <> Неопределено Тогда
			ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ka='В чеке обнаружены дублирующиеся позиции.';ru='В чеке обнаружены дублирующиеся позиции.';en='The check found duplicate items.'"), "");
		КонецЕсли;
...
Процедура ВыбратьТранспортныйМодуль(ОповещениеВыбора, Контекст, Отбор = Неопределено)
	...
	Если СписокДоступныхМодулей.Количество() = 0 Тогда
		ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ka='Отсутствуют доступные транспортные модули.';ru='Отсутствуют доступные транспортные модули.';en='There are no available transport modules.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		СписокДляВыбора.ПоказатьВыборЭлемента(
			...
			НСтр("ka='Выберите транспортный модуль ЕГАИС';ru='Выберите транспортный модуль ЕГАИС';en='Select a transport module EGAIS'"));
	КонецЕсли;
...
Процедура ОбработкаОтветовПоРасписанию_ПослеОбработкиОтветовУТМ(Результат, Контекст) Экспорт
	...
	Если НЕ Результат.Результат Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ka='ЕГАИС';ru='ЕГАИС';en='ЕГАИС'"),
			"Ошибка",
