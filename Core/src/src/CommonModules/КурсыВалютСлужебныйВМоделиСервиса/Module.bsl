...
Процедура ЗагрузитьКурсы() Экспорт
	...
	Если Дескрипторы.Descriptor.Количество() < 1 Тогда
		ВызватьИсключение(НСтр("ka='სერვისის მენეჯერში არ არის ტიპ ""ვალუტისკურსები""-ს მონაცემები';ru='В менеджере сервиса отсутствуют данные вида ""КурсыВалют""';en='The service manager is no data type ""KursyValyut""'"));
	КонецЕсли;
...
Процедура ОбработатьПоставляемыеКурсыЗаДень(Знач Дескриптор, Знач ПутьКФайлу)
	...
	Если ДатаКурсов = "" Тогда
		ВызватьИсключение НСтр("ka='ტიპ ""ვალუტისკურსებიდღისთვის"" მონაცემები არ შეიცავს მახასიათებლებს ""თარიღი"". კურსების განახლება შეუძლებელია.';ru='Данные вида ""КурсыВалютЗаДень"" не содержат характеристики ""Дата"". Обновление курсов невозможно.';en='These kinds of ""KursyValyutZaDen"" does not contain data ""date"". Updating Conversion impossible.'"); 
	КонецЕсли;
	...
	Для каждого ОбластьДанных Из ОбластиДляОбновления Цикл
		...
		Попытка
			...
			Пока ВыборкаВалюты.Следующий() Цикл
				...
				Иначе
					Комментарий = НСтр("ka='Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.';ru='Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.';en='Download currency exchange rate at the date of 1% 2% canceled due to violations of the prohibition date changes.'"); 
					Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Комментарий, ВыборкаВалюты.Код, ДатаКурсов);
					ИмяСобытия = НСтр("ka='მიწოდებული მონაცემები. ვალუტის კურსის ჩატვირთვის გაუქმება';ru='Поставляемые данные.Отмена загрузки курсов валюты';en='Supplied dannye.Otmena download exchange rates'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ka='მიწოდებული მონაცემები. ვალუტის კურსის გავრცელება მონაცემთა სფეროზე';ru='Поставляемые данные.Распространение курсов валют по областям данных';en='Supplied dannye.Rasprostranenie currency data areas'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
