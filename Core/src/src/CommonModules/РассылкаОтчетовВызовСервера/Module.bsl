...
Функция СформироватьСписокПолучателейРассылки(Знач Параметры) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия   = НСтр("ka='ანგარიშგებების დაგზავნა. მიმღებთა სიის შედგენა';ru='Рассылка отчетов. Формирование списка получателей';en='Reports Newsletter. Formation of the list of recipients'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если РезультатВыполнения.БылиКритичныеОшибки Тогда
		РезультатВыполнения.Текст = НСтр("ka='მიმღებების სიის ჩამოყალიბება ვერ მოხერხდა';ru='Не удалось сформировать список получателей';en='Failed to create a list of recipients'");
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода, Знач УникальныйИдентификатор) Экспорт
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='ანგარიშგებების დაგზავნა: დაგზავნების შესრულება ფონში';ru='Рассылки отчетов: Выполнение рассылок в фоне';en='report distribution: Performing mailings in background'");
