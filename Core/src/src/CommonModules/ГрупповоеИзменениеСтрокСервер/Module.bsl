...
Процедура ОбработатьТаблицу(ЭтаФорма, Таблица, Действие, ОбъектДействия, Значение, ИмяЭлементаНоменклатура) Экспорт
	...
	Если Действие <> Перечисления.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента
		...
		ТекстСообщения = НСтр("ka='არცერთი სტრიქონი არ არის არჩეული.';ru='Не выбрана ни одна строка.';en='Is not selected, no string.'");
	...
	ИначеЕсли Действие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроект") Тогда
		...
		Для каждого Строка Из Таблица Цикл
			...
			Если Строка.Свойство("Проект") Тогда
				Строка.Проект = Значение;
			КонецЕсли;
...
Процедура УстановитьПредставлениеДействия(НаборЭлементов)
	...
	Если ЗначениеЗаполнено(ТипЗначения) Тогда
		...
		//ПодольскийАИ #УчетПроектов 14.06.2021 {
