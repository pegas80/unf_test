...
Функция ПолучитьТаблицуСкидокНаценокДляРозничнойТорговли(Объект, СтруктурнаяЕдиница, ВходныеПараметры)
	...
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	ВидЦен = НЕОПРЕДЕЛЕНО;
	ВходныеПараметры.Свойство("ВидЦен",ВидЦен);
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	...
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	|			ИЛИ (&ПоВидуЦен И АвтоматическиеСкидкиВремяПоДнямНедели.Ссылка.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияАвтоматическихСкидок.ПоВидуЦен) И АвтоматическиеСкидкиВремяПоДнямНедели.Ссылка.ВидЦен = &ВидЦен)
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	...
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	|			ИЛИ (&ПоВидуЦен И АвтоматическиеСкидки.Ссылка.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияАвтоматическихСкидок.ПоВидуЦен) И АвтоматическиеСкидки.Ссылка.ВидЦен = &ВидЦен)
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	...
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	|			ИЛИ (&ПоВидуЦен И АвтоматическиеСкидкиПолучателиСкидки.Ссылка.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияАвтоматическихСкидок.ПоВидуЦен) И АвтоматическиеСкидкиПолучателиСкидки.Ссылка.ВидЦен = &ВидЦен)
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	...
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	Запрос.УстановитьПараметр("ПоВидуЦен", ЗначениеЗаполнено(ВидЦен));
...
Функция ПолучитьТаблицуСкидокНаценокДляОптовойТорговли(Объект, СтруктурнаяЕдиница, ВходныеПараметры, СтруктурнаяЕдиницаСклад = Неопределено) //+++ Дмгг
	...
	|	АвтоматическиеСкидкиПолучателиСкидки.Ссылка,
	|	АвтоматическиеСкидкиПолучателиСкидки.Получатель
	|" + ?(СтруктурнаяЕдиницаСклад <> Неопределено,  
	//+++ Дмгг 
	"
	|ОБЪЕДИНИТЬ ВСЕ 
    |
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СкидкиПослеОтбораПоДнямНедели КАК ВТ_СкидкиПослеОтбораПоДнямНедели
	|		ПО АвтоматическиеСкидкиПолучателиСкидки.Ссылка = ВТ_СкидкиПослеОтбораПоДнямНедели.Ссылка
	|			И (АвтоматическиеСкидкиПолучателиСкидки.Получатель = &СтруктурнаяЕдиница2)
	|ГДЕ
	|	(АвтоматическиеСкидкиПолучателиСкидки.Ссылка.Назначение = &Опт
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	|			ИЛИ (&ПоВидуЦен И АвтоматическиеСкидкиПолучателиСкидки.Ссылка.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияАвтоматическихСкидок.ПоВидуЦен) И АвтоматическиеСкидкиПолучателиСкидки.Ссылка.ВидЦен = &ВидЦен)
	...
	|	И НЕ ВТ_СкидкиПослеОтбораПоДнямНедели.ЕстьОграниченияПоПолучателямСкладам
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СкидкаНаценка
	|",
	...
	Запрос.УстановитьПараметр("ДатаДокумента", Объект.Дата);  
	//+++ Дмгг
	Если СтруктурнаяЕдиницаСклад <> Неопределено Тогда 
		Запрос.УстановитьПараметр("СтруктурнаяЕдиница2", СтруктурнаяЕдиницаСклад);  
	КонецЕсли;
	//--- Дмгг
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	Запрос.УстановитьПараметр("ПоВидуЦен", ЗначениеЗаполнено(ВидЦен));
...
Функция РассчитатьПоЧекуККМ(Объект, ВходныеПараметры)
	...
	//Малышев 2024-10-24 ONESOFT-3539 (--
	ПараметрыРасчета.Вставить("Контрагент", Объект.Контрагент);
	Если не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПараметрыРасчета.Вставить("Контрагент", РегистрыСведений.УчетнаяПолитикаРегл.ЗначениеПоляУчетнойПолитики("РозничныйКонтрагент", Объект.Организация));
	КонецЕсли;	
	//Малышев 2024-10-24 ONESOFT-3539 --)
...
Функция РассчитатьПоРеализацииТоваров(Объект, ВходныеПараметры)
	...
	//+++ Дмгг
	//ПараметрыРасчета.Вставить("СкидкиНаценки", ПолучитьТаблицуСкидокНаценокДляОптовойТорговли(Объект, Объект.Контрагент, ВходныеПараметры));  
	//Если Объект.Свойство("СтруктурнаяЕдиница") Тогда 
	Если ТипЗнч(Объект) = тип("ДанныеФормыСтруктура") И Объект.Свойство("СтруктурнаяЕдиница") Тогда
		ПараметрыРасчета.Вставить("СкидкиНаценки", ПолучитьТаблицуСкидокНаценокДляОптовойТорговли(Объект, Объект.Контрагент, ВходныеПараметры, Объект.СтруктурнаяЕдиница));	 
	Иначе
		ПараметрыРасчета.Вставить("СкидкиНаценки", ПолучитьТаблицуСкидокНаценокДляОптовойТорговли(Объект, Объект.Контрагент, ВходныеПараметры));
	КонецЕсли;     
	//--- Дмгг
