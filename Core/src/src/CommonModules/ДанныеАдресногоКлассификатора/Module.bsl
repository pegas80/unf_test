...
Функция НалоговыйОрганПоАдресу(Знач ЗначенияПолейАдреса) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მისამართი %1:';ru='Адрес %1:';en='Address% 1:'"), ЗначенияПолейАдреса)
	...
	Если РезультатыПроверки.Item.Количество()>0 Тогда
		...
		Если ОшибкиПроверки.Количество() > 0 Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მისამართი %1: %2';ru='Адрес %1: %2';en='Address% 1% 2'"), ЗначенияПолейАдреса, ОшибкиПроверки[0].Text);
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მისამართი %1: მისამართის გარჩევის შეცდომა';ru='Адрес %1: ошибка разбора адреса';en='Address% 1: address parsing error'"), ЗначенияПолейАдреса);
		КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, СтруктураРеквизитов)
	...
	ИмяСобытия = НСтр("ka='საინფორმაციო სამისამართო სისტემის მონაცემთა სერვისი.';ru='Сервис данных ФИАС.';en='Service data FIAS.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
