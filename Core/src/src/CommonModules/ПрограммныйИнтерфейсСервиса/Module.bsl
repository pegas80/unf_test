...
Функция ПрочитанныеДанныеОтвета(Ответ)
	...
	Если ЗначениеЗаполнено(ТелоОтвета) Тогда
		...
		Исключение
			...
			Событие = ИмяСобытия() + "." + НСтр("ka='Ошибка чтения JSON';ru='Ошибка чтения JSON';en='Error reading JSON'", Локаль);
...
Функция ИмяСобытия()
	...
	Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
	Возврат НСтр("ka='Программный интерфейс сервиса';ru='Программный интерфейс сервиса';en='Software service interface'", Локаль);
