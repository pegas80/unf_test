...
Процедура ОткрытьОтчетИлиФорму(ТекущийЭлемент, Пользователь, ТекущийПользователь, ИмяФормыПерсональныхНастроек) Экспорт
	...
	Если Пользователь <> ТекущийПользователь Тогда
		ТекстПредупреждения =
			НСтр("ka='სხვა მომხმარებლის გამართვების სანახავად, აუცილებელია პროგრამის გაშვება მისი სახელით და საჭირო ანგარიშგების ან ფორმის გახსნა.';ru='Для просмотра настроек другого пользователя необходимо"
		...
		Для Каждого КлючОбъекта Из ЭлементДереваЗначений.ТекущиеДанные.Ключи Цикл
			...
			Иначе
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиРабочегоСтола" Тогда
					ПоказатьПредупреждение(,
						НСтр("ka='სამუშაო მაგიდის გამართვების სანახავად გადადით პროგრამის საბრძანებო ინტერფეისში არსებულ ნაწილ ""სამუშაო მაგიდა""-ზე.';ru='Для просмотра настроек рабочего стола перейдите к разделу"
"""Рабочий стол"" в командном интерфейсе программы.'"));
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиКомандногоИнтерфейса" Тогда
					ПоказатьПредупреждение(,
						НСтр("ka='საბრძანებო ინტერფეისის გამართვების სანახავად აირჩიეთ პროგრამის საბრძანებო ინტერფეისის საჭირო განყოფილება.';ru='Для просмотра настроек командного интерфейса"
"выберите нужный раздел командного интерфейса программы.'"));
				...
				Если РодительЭлемента <> Неопределено Тогда
					ТекстПредупреждения =
						НСтр("ka='მოცემული გამართვის სანახავად აუცილებელია გავხსნათ ""%1"" და შემდეგ, გადავიდეთ ფორმა ""%2""-ზე.';ru='Для просмотра данной настройки необходимо открыть ""%1"" "
"и затем перейти к форме ""%2"".'");
		...
		ПоказатьПредупреждение(,НСтр("ka='მოცემული გამართვის ნახვა შეუძლებელია.';ru='Данную настройку невозможно просмотреть.';en='This setting can not be viewed.'"));
		Возврат;
	...
	ПоказатьПредупреждение(,НСтр("ka='აირჩიეთ ნახვის გამართვა.';ru='Выберите настройку для просмотра.';en='Select the setting for viewing.'"));
...
Функция ФормированиеПоясненияПриКопировании(ПредставлениеНастройки, КоличествоНастроек, ПояснениеКомуСкопированыНастройки) Экспорт
	...
	Если КоличествоНастроек = 1 Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='""%1"" скопирована %2';ru='""%1"" скопирована %2';en='""%1"" copied% 2'"),
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Скопировано %1 %2';ru='Скопировано %1 %2';en='Copied% 1% 2'"),
...
Функция ПояснениеПользователи(КоличествоПользователей, Пользователь) Экспорт
	...
	Если КоличествоПользователей = 1 Тогда
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მომხმარებელს ""%1""';ru='пользователю ""%1""';en='User ""%1""'"), Пользователь);
	Иначе
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 მომხმარებლებს';ru='%1 пользователям';en='% 1 people'"), КоличествоПользователей);
	КонецЕсли;
