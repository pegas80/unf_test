		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы[ИмяПоляНоменклатура].ИспользоватьСерийныеНомера Тогда
				...
				Если ОтборСерийныеНомера.Количество() <> СтрокаЗапасыКоличество Тогда
					...
					ТекстСообщения = НСтр("ka='Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.';ru='Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.';en='The number of serial numbers of the table% ImyaTCh% different from the number of units in line% number%.'");
...
Процедура ВыполнитьКонтрольДублей(Отказ, Знач Запасы, Знач СерийныеНомера, Знач вхФорма, ИмяПоляКлючСвязи)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='სტრიქონ %НомерСтроки%-ში მითითებული სერიული ნომერი ""%СерийныйНомер% მითითებულია განმეორებით.';ru='Серийный номер ""%СерийныйНомер%"", указанный в строке %НомерСтроки%, указан повторно.';en='Serial number ""% SeriynyyNomer%"", indicated on line% NomerStroki% Set again.'"
			);
