		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы[ИмяПоляНоменклатура].ИспользоватьСерийныеНомера Тогда
				...
				Если ОтборСерийныеНомера.Количество() <> СтрокаЗапасыКоличество Тогда
					...
					ТекстСообщения = НСтр("ru = 'Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.'; ka = 'სერიული ნომრების რაოდენობა ცხრილურ ნაწილში %ИмяТЧ% განსხვავდება %Номер% ხაზში ერთეულების რაოდენობისგან.'; en = 'The number of serial numbers in the table section %ИмяТЧ% differs from the quantity of units in row %Номер%.'");
					ТекстСообщения = ТекстСообщения + НСтр("ru = ' Серийных номеров - %ЧислоНомеров%, нужно %КоличествоВСтроке%'; ka = ' სერიული ნომერი - %ЧислоНомеров%, საჭიროა %КоличествоВСтроке%'; en = 'Serial numbers – %ЧислоНомеров%, required – %КоличествоВСтроке%.'");
...
Процедура ВыполнитьКонтрольДублей(Отказ, Знач Запасы, Знач СерийныеНомера, Знач вхФорма, ИмяПоляКлючСвязи)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru = 'Серийный номер ""%СерийныйНомер%"", указанный в строке %НомерСтроки%, указан повторно.'; ka = 'სტრიქონ %НомерСтроки%-ში მითითებული სერიული ნომერი ""%СерийныйНомер% მითითებულია განმეორებით.'; en = 'The serial number ""%СерийныйНомер%"", specified in row %НомерСтроки%, is duplicated.'"
			);
	...
	Если ИспользоватьСерийныеНомераОстатки() = Истина Тогда
		...
		Если Объект.Номенклатура.ИспользоватьСерийныеНомера Тогда
			...
			Если Объект.СерийныеНомера.Количество() <> ОбъектКоличество Тогда
				ТекстСообщения = НСтр("ka='სერიული ნომრების რიცხვი განსხვადება დოკუმენტში არსებული რაოდენობისგან.';ru='Число серийных номеров отличается от количества в документе.';en='The number of serial numbers different from the number in the document.'");
				ТекстСообщения = ТекстСообщения + НСтр("ru = ' Серийных номеров - %ЧислоНомеров%, нужно %КоличествоВСтроке%'; ka = ' სერიული ნომერი - %ЧислоНомеров%, საჭიროა %КоличествоВСтроке%'; en = 'Serial numbers - %ЧислоНомеров%, required %КоличествоВСтроке%.'");
