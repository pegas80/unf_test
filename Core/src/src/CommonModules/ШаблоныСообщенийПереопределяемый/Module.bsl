...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
	...
	Для Каждого ПредметШаблона Из Настройки.ПредметыШаблонов Цикл
		...
		Если ПредметШаблона.Имя = Метаданные.Документы.ЗаказПокупателя.ПолноеИмя()
			...
			ПредметШаблона.Представление = НСтр("ka='Заказ покупателя (заказ-наряд)';ru='Заказ покупателя (заказ-наряд)';en=""Buyer's order (work order)""");
		КонецЕсли;
	КонецЦикла;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБиллинг") Тогда
		...
		Предмет.Представление = НСтр("ka='ხელშეკრულება მომსახურება';ru='Договор обслуживания';en='contract service'");
