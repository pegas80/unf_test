	...
	Если СообщенияВМоделиСервиса.ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных()
			...
			Если ОбщегоНазначения.ЗначениеРазделителяСеанса() <> Сообщение.Body.Zone Тогда
				ШаблонСообщения = НСтр("ka='%1 არეალის სახელით %2 არეალიდან შეტყობინების გაგზავნის მცდელობა';ru='Попытка отправить сообщение от имени области %1 из области %2';en='An attempt to send a message on behalf of the region of 1% of the area of ​​2%'");
		...
		Иначе
			...
			Если ПроверкаКлючаОбластиВСообщенияхВозможна() Тогда
				...
				Если ТекущийКлючОбласти <> Сообщение.Body.ZoneKey Тогда
					...
					ВызватьИсключение НСтр("ka='შეტყობინებაში მონაცემთა არეალის არასწორი გასაღები!';ru='Неверный ключ области данных в сообщении!';en='Invalid key field data in a message!'");
