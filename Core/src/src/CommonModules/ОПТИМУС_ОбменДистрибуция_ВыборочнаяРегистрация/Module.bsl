...
Функция ТребуетсяРегистрацияИзмененийОбъекта(СтруктураРеквизитовОбъекта, СтруктураОписанияОбъекта, МассивПолей, ТаблицаДанныхОбъекта, УзелОбмена) Экспорт
	...
	Если ТаблицаДанныхОбъекта.Количество() = 1 Тогда
...
Процедура ЗарегистрироватьИзмененияДанных(УзелОбмена) Экспорт
	...
	Для Каждого ЭлементСоставаПланаОбмена Из СоставПланаОбмена Цикл
		...
		СтруктураОписанияОбъекта = ОПТИМУС_ОбменДистрибуция.ПолучитьСтруктуруОписанияОбъекта(ЭлементСоставаПланаОбмена.Метаданные, Ложь);
...
Функция ПолучитьТекстЗапросаДляРегистра(ИмяОбъекта, ИмяТаблицы, МассивПолей, Изменения, НомерСообщения) Экспорт
	...
	Если Изменения Тогда
		...
		КонецЕсли;	
	Иначе
		МассивПолейОтбора = ОПТИМУС_ОбменДистрибуция.ПолучитьМассивПолейОтбораДляОбъекта("РегистрыСведений", ИмяОбъекта);
	КонецЕсли;
...
Процедура ДобавитьОтборыИзСхемыОбмена(НастройкиКомпоновки, СтруктураНастроекОтбора, ИмяКласса, ИмяОбъекта, 
	...
	Если ИмяКласса = "Справочники" Тогда
		...
		ИначеЕсли ИмяОбъекта = "НоменклатураПрисоединенныеФайлы" Тогда
			ЗаполнитьЭлементОтбораКомпоновкиИзСтруктуры(НастройкиКомпоновки.Отбор, СтруктураНастроекОтбора, "Номенклатура","Ссылка");
		ИначеЕсли ИмяОбъекта = "СтруктурныеЕдиницы" Тогда
			ЗаполнитьЭлементОтбораКомпоновкиИзСтруктуры(НастройкиКомпоновки.Отбор, СтруктураНастроекОтбора, "Склад","Ссылка");
		ИначеЕсли ИмяОбъекта = "Пользователи" Тогда
			ЗаполнитьЭлементОтбораКомпоновкиИзСтруктуры(НастройкиКомпоновки.Отбор, СтруктураНастроекОтбора, "ТорговыйПредставитель","Ссылка");
		КонецЕсли;
...
Функция ПолучитьСтруктуруПараметровОбменаДанными(ПараметрыОбменаДанными) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='შეცდომა მონაცემთა გაცვლის პარამეტრების წაკითხვისას:';ru='Ошибка при чтении параметров обмена данными:';en='Error reading data sharing settings:'");
...
Функция ПолучитьДеревоИспользуемыхМетаданных(ОписаниеМетаданных) Экспорт
	...
	Исключение
		...
		#Если ВнешнееСоединение Тогда
		ВызватьИсключение(НСтр("ka='მობილური დანართის მიერ ხმარებად მეტამონაცემთა ჩამონათვალის წაკითხვის შეცდომა';ru='Ошибка чтения перечня метаданных, используемых мобильным приложением';en='Error reading list of metadata used mobile application'"));
		#КонецЕсли
...
Функция ПолучитьТипПоСтроке(XMLСтрокаТипа)
	...
	Иначе
		#Если ВнешнееСоединение Тогда
		ТекстСообщения = НСтр("ka='მონაცემთა უცნობი ტიპი გაცვლის პაკეტში';ru='Неизвестный тип данных в пакете обмена';en='Unknown type data exchange package'");
...
Функция ПолучитьТекстЗапросаДляВыборкиСсылочныхДанных(СтруктураОписанияОбъекта, СтруктураПараметровНастройкиОбмена)
	...
	МассивПолейОтбора = ОПТИМУС_ОбменДистрибуция.ПолучитьМассивПолейОтбораДляОбъекта(ИмяКласса, ИмяОбъекта);
...
Функция ИспользованиеОбъектаОтключено(ИмяКласса, ИмяОбъекта, УзелОбмена)
	...
	Если ИмяКласса = "Справочники" Тогда
		...
		ИначеЕсли ИмяОбъекта = "ХарактеристикиНоменклатуры" Тогда
			...
			Возврат НЕ ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики");
...
Функция ПолучитьСтруктуруСоставаОбъекта(ОбъектМетаданных, ИмяКласса, ИмяОбъекта, ДеревоИспользуемыхМетаданных)
	...
	Если СтрокаКласса = Неопределено Тогда
		...
		#Если ВнешнееСоединение Тогда
		ТекстСообщения = НСтр("ka='გაცვლის პაკეტში არასწორი მონაცემები არის ჩაწერილი: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%დანართში არ გამოიყენება %ИмяКласса%';ru='В пакет обмена записаны неверные данные: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%В приложении не используются %ИмяКласса%';en='The pack of exchange recorded incorrect data:..%%% ObjectName ClassName%%% PerevodStroki The application does not use% ClassName%'");
	...
	Если СтрокаОбъекта = Неопределено Тогда
		...
		#Если ВнешнееСоединение Тогда
		ТекстСообщения = НСтр("ka='გაცვლის პაკეტში ჩაწერილია არასწორი მონაცემები: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%დანართში არ გამოიყენება მოცემული ობიექტი';ru='В пакет обмена записаны неверные данные: %ИмяКласса%.%ИмяОбъекта%.%ПереводСтроки%В приложении не используется данный объект';en='The pack of exchange recorded incorrect data:.. ClassName%%%%% ObjectName PerevodStroki% do not use the object in the application'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ИмяКласса%",ИмяКласса);
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ИмяОбъекта%",ИмяОбъекта);
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ПереводСтроки%",Символы.ПС);
	...
	Если СтрокаОбъекта <> Неопределено Тогда	
		ВетвьТабличныхЧастей = СтрокаОбъекта.Строки.Найти("TabularParts","Имя");
	Иначе
		ВетвьТабличныхЧастей = Неопределено;
	КонецЕсли;
	...
	Для Каждого СтрокаКласса из КореньДереваМетаданных.Строки Цикл
		...
		Если ИмяКласса = "Константы" ИЛИ ИмяКласса = "Справочники" ИЛИ ИмяКласса = "Документы" ИЛИ ИмяКласса = "РегистрыСведений" Тогда
			...
			Для Каждого СтрокаОбъекта из СтрокаКласса.Строки Цикл
				...
				СтруктураОписанияОбъекта = ОПТИМУС_ОбменДистрибуция.ПолучитьСтруктуруОписанияОбъекта(МетаданныеОбъекта, Ложь);
