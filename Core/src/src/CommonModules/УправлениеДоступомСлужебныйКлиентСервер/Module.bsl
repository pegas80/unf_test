...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа, ДобавитьКоличествоЗначений = Истина) Экспорт
	...
	Если Форма.ЭтоПрофильГруппДоступа Тогда
		Если КоличествоЗначений = 0 Тогда
			ЧислоИПредмет = НСтр("ka='არ არის დანიშნულები';ru='не назначены';en='not assigned'");
		Иначе
			ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
					"Л = ru_RU", НСтр("ka='მნიშვნლობა,მნიშვნელობები,მნიშვნელობებს,,,,,,0';ru='значение,значения,значений,,,,,,0';en='value value value ,,,,,, 0'"));
		КонецЕсли;
	...
	Если КоличествоЗначений = 0 Тогда
		Представление = ?(ОписаниеВидаДоступа.ВсеРазрешены,
			НСтр("ka='ყველა ნებადართულია, გამონაკლისების გარეშე';ru='Все разрешены, без исключений';en='All allowed, no exceptions'"),
			НСтр("ka='ყველა აკრძალულია, გამონაკლისის გარეშე';ru='Все запрещены, без исключений';en='All prohibited without exception'"));
	Иначе
		ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
				"Л = ru_RU", НСтр("ka='значения,значений,значений,,,,,,0';ru='значения,значений,значений,,,,,,0';en='values ​​values ​​values ​​,,,,,, 0'"));
		
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ОписаниеВидаДоступа.ВсеРазрешены,
				НСтр("ka='ყველა ნებადართულია, გარდა %1';ru='Все разрешены, кроме %1';en='All allowed except% 1'"),
				НСтр("ka='ყველა აკრძალულია, გარდა %1';ru='Все запрещены, кроме %1';en='All prohibited, except% 1'")),
			ЧислоИПредмет);
	КонецЕсли;
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ВидДоступа = Форма.ВидДоступаПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ka='აკრძალული მნიშვნელობები (%1) - მიმდინარე მომხმარებელი ყოველთვის ნებადართულია';ru='Запрещенные значения (%1) - текущий пользователь всегда разрешен';en='Prohibited value (% 1) - the current user is always allowed'"),
				НСтр("ka='ნებადართული მნიშვნელობები (%1) - მიმდინარე მომხმარებელი ყოველთვის ნებადართულია';ru='Разрешенные значения (%1) - текущий пользователь всегда разрешен';en='Allowed values ​​(% 1) - the current user is always allowed'") );
		
		ИначеЕсли ТекущиеДанные.ВидДоступа = Форма.ВидДоступаВнешниеПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ka='აკრძალული მნიშVნელობები (%1) - მიმდინარე გარე მომხმარებელი ყოველთვის ნებადართულია';ru='Запрещенные значения (%1) - текущий внешний пользователь всегда разрешен';en='Prohibited value (% 1) - current external user is always allowed'"),
				НСтр("ka='ნებადართული მნიშვნელობები (%1) - მიმდინარე გარე მომხმარებელი ყოველთვის ნებადართულია';ru='Разрешенные значения (%1) - текущий внешний пользователь всегда разрешен';en='Allowed values ​​(% 1) - current external user is always allowed'") );
		Иначе
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ka='აკრძალული მნიშVნელობები (%1)';ru='Запрещенные значения (%1)';en='Prohibited value (1%)'"),
				НСтр("ka='ნებადართული მნიშვნელობები (%1)';ru='Разрешенные значения (%1)';en='Allowed values ​​(1%)'") );
		КонецЕсли;
	...
	Если Форма.ТекущиеТипыВыбираемыхЗначений.Количество() = 0 Тогда
		Форма.ТекущиеТипыВыбираемыхЗначений.Добавить(Неопределено, НСтр("ka='დაუდგენელია';ru='Неопределено';en='Undefined'"));
	КонецЕсли;
	...
	Пока НЕ Отказ И ИндексВидаДоступа >= 0 Цикл
		...
		Если СтрокаВидаДоступа.ВидДоступа = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ka='დაშვების ტიპი არ არის არჩეული.';ru='Вид доступа не выбран.';en='do not select the access kind.'"),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				НСтр("ka='დაშვების ტიპი სტრიქონ %1-ში არჩეული არ არის.';ru='Вид доступа в строке %1 не выбран.';en='View access on line% 1 is not selected.'"),
		...
		Если Параметры.Назначение <> Неопределено
		  И Не ВидДоступаСоответствуетНазначениюПрофиля(СтрокаВидаДоступа.ВидДоступа, НазначениеПрофиля) Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაშვების სახეობა ""%1"" პროფილის დანიშნულებას არ შეესაბამება.';ru='Вид доступа ""%1"" не соответствует назначению профиля.';en='Type of access ""%1"" does not match the profile of the destination.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში დაშვების სახეობა ""%1"" პროფილის დანიშნულებას არ შეესაბამება.';ru='Вид доступа ""%1"" в строке %%1 не соответствует назначению профиля.';en='Type of access ""%1"" in a string %% 1 does not match the profile of the destination.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Если НайденныеВидыДоступа.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაშვების სახეობა ""%1"" მეორდება.';ru='Вид доступа ""%1"" повторяется.';en='Type of access ""%1"" is repeated.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სტრიქონ %%1-ში დაშვების სახეობა ""%1"" მეორდება.';ru='Вид доступа ""%1"" в строке %%1 повторяется.';en='Type of access ""%1"" in a string %% 1 is repeated.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Пока НЕ Отказ И ИндексЗначенияДоступа >= 0 Цикл
			...
			Если СтрокаЗначенияДоступа.ЗначениеДоступа = Неопределено Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					НСтр("ka='მნიშვნელობა არ არის არჩეული.';ru='Значение не выбрано.';en='Value is selected.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ka='მნიშვნელობა სტრიქონ %1-ში არ არის არჩეული.';ru='Значение в строке %1 не выбрано.';en='The value in row 1% is selected.'"),
			...
			Если НайденныеЗначения.Количество() > 1 Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					Параметры.ПутьКТаблицам + "ЗначенияДоступа[%1].ЗначениеДоступа",
					НСтр("ka='მნიშვნელობა მეორდება.';ru='Значение повторяется.';en='Value is repeated.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ka='მნიშვნელობა სტრიქონ %1-ში მეორდება.';ru='Значение в строке %1 повторяется.';en='The value in row 1% repeats.'"),
					Параметры.ЗначенияДоступа.Индекс(СтрокаЗначенияДоступа));
				Отказ = Истина;
				Прервать;
			КонецЕсли;
