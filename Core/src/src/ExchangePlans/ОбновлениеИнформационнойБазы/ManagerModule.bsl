...
Функция УзелПоОчереди(Очередь) Экспорт
	...
	Если ТипЗнч(Очередь) <> Тип("Число")  
		Или Очередь = 0 Тогда
		ТекстИсключения = НСтр("ka='გაცვლის გეგმის კვანძი საინფორმაციობაზისგანახლება-ს მიღება შეუძლებელია, ვინაიდან რიგის ნომერი არ არის გადაცემული.';ru='Не возможно получить узел плана обмена ОбновлениеИнформационнойБазы, т.к. не передан номер очереди.';en='It is not possible to obtain the node plan ObnovlenieInformatsionnoyBazy exchange, since not transmitted sequence number.'");
