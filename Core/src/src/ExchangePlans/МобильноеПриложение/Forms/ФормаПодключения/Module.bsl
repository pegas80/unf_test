...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Найти(АдресДляПодключенияКЦентральнойБазеСМобильногоУстройства, "e1c://") = Истина Тогда
		Элементы.ДекорацияШаг2Заголовок.Заголовок = НСТр("ka='ეწვიეთ მობილურდანართიან გვერდს სინქრონიზაციის მოწყობის აღწერით';ru='Посетите страницу с описанием настройки синхронизации с мобильным приложением';en='Visit the page with a description of the synchronization settings from your mobile app'");
		Элементы.ДекорацияСсылка.Видимость = Истина;
		Элементы.QRКод.Видимость = Ложь;
	Иначе
		Элементы.ДекорацияШаг2Заголовок.Заголовок = НСТр("ka='კავშირის დასამყარებლად მობილური ჩატვირთვის შემდეგ გამოიყენეთ გამოიყენეთ QRკოდი';ru='После загрузки мобильного приложения используйте этот QR-код для установки соединения';en='After downloading the mobile app using the QR-code to establish a connection'");
