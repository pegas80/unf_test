...
Процедура ОчиститьИдентификатор(Команда)
	...
	ЗаголовокВопроса = НСтр("ka='Очистка идентификатора подсистемы';ru='Очистка идентификатора подсистемы';en='Clearing subsystem ID'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ТекстВопроса = НСтр("ru='Очистка идентификатора подсистемы приведет"
"к регистрации новой подсистемы на сервере Штрих-М.';ka='Очистка идентификатора подсистемы приведет"
...
"товаров и документов в новую подсистему из существующей.';ka='Также это приведет к перерегистрации кассовых аппаратов,"
"товаров и документов в новую подсистему из существующей.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	ТекстВопроса = ТекстВопроса + НСтр("ka='Выполнить очистку идентификатора подсистемы (agent_node_id)?';ru='Выполнить очистку идентификатора подсистемы (agent_node_id)?';en='Cleaning the subsystem ID (agent_node_id)?'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
	СписокКнопок = Новый СписокЗначений;
	СписокКнопок.Добавить(КодВозвратаДиалога.Да,
		НСтр("ka='შესრულება';ru='Выполнить';en='Run'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	СписокКнопок.Добавить(КодВозвратаДиалога.Отмена,
		НСтр("ka='გაუქმება';ru='Отменить';en='cancel'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ПроверитьПодключение(Команда)
	...
	Иначе
		СтрокаПредупреждения = НСтр("ka='Соединенение успешно установлено.';ru='Соединенение успешно установлено.';en='Your connection is successfully established.'",
			ОбщегоназначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	ЗаголовокПредупреждения = НСтр("ka='შეერთების შემოწმება';ru='Проверка подключения';en='Check connections'",
			ОбщегоназначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриИзмененииПодписи()
	...
	Если ЗначениеЗаполнено(Подпись) Тогда
		Элементы.ИзменитьПодпись.Заголовок = НСтр("ka='შეცვლა';ru='Изменить';en='Edit'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Элементы.ИзменитьПодпись.Заголовок = НСтр("ka='შევსება';ru='Заполнить';en='Insert'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
