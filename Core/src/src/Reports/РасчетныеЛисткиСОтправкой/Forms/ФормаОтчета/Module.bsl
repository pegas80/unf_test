...
Процедура СформироватьВыполнить()
	...
	Если Константы.ИспользоватьНесколькоОрганизаций.Получить() И НЕ ЗначениеЗаполнено(Организация) Тогда
		ТекстСообщения = НСтр("ka='ორგანიზაცია არ არის არჩეული!';ru='Не выбрана организация!';en='Not selected organization!'");
	...
	Если НЕ ЗначениеЗаполнено(ПериодРегистрации) Тогда
		ТекстСообщения = НСтр("ka='არ არის არჩეული რეგისტრაციის პერიოდი!';ru='Не выбран период регистрации!';en='Not selected registration period!'");
	...
	Если НЕ Константы.ФункциональнаяУчетВалютныхОпераций.Получить() И НЕ ЗначениеЗаполнено(Валюта) Тогда
		ТекстСообщения = НСтр("ka='არ არის არჩეული ვალუტა!';ru='Не выбрана валюта!';en='Not selected currency!'");
	...
	Если РезультатЗапроса[0].Пустой() Тогда
		...
		Сообщение.Текст = НСтр("ka='ანგარიშგების ფორმირებისთვის არ არის მონაცემები!';ru='Нет данных для формирования отчета!';en='No data to generate a report!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
    ...
    ФорматЛокальнойДаты = НСТР ("ru = 'Л=ru_RU; '; ka = 'Л=ka_GE; '; en = 'Л=en_US; '");
	БукваГода = НСТР ("ru = 'г.'; ka = 'წ.'; en = ''");

    ОбластьОтбивка.Параметры.ТекстОтбивки = Формат(ПериодРегистрации , ФорматЛокальнойДаты+"ДФ='ММММ гггг ''"+БукваГода+"'' '");
	ТабличныйДокумент.Вывести(ОбластьОтбивка);

    ОбластьОтбивка.Параметры.ТекстОтбивки = НСТР ("ru = 'Организация: '; ka = 'ორგანიზაცია: '; en = 'Organization: '") + Организация;
	ТабличныйДокумент.Вывести(ОбластьОтбивка);

	ВыборкаПодразделение = РезультатЗапроса[0].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Подразделение");
	Пока ВыборкаПодразделение.Следующий() Цикл

        ОбластьОтбивка.Параметры.ТекстОтбивки = НСТР ("ru = 'Подразделение: '; ka = 'ქვეგანყოფილება: '; en = 'Subdivision: '") + ВыборкаПодразделение.Подразделение;
		ТабличныйДокумент.Вывести(ОбластьОтбивка);
		...
		Пока ВыборкаФизлицо.Следующий() Цикл
			...
			ОбластьШапка.Параметры.Заголовок = НСТР ("ru = 'Расчетный листок за '; ka = 'გაანგარიშების ფურცელი '; en = 'Payslip for '") + Формат(ПериодРегистрации , ФорматЛокальнойДаты+"ДФ='ММММ гггг ''"+БукваГода+"'' '");
			...
			Если ВыборкаФизлицо.СальдоНаНачало < 0 Тогда
				ОбластьПодвал.Параметры.ТекстДолгНаНачалоПериода = НСТР("ru = 'Долг за работником на начало месяца:'; ka = 'თანამშრომლების დავალიანება თვის დასაწყისში'; en = 'Debt due to the employee at the beginning of the month:'");
			Иначе	
				ОбластьПодвал.Параметры.ТекстДолгНаНачалоПериода = НСТР("ru = 'Долг за организацией на начало месяца:'; ka = 'ორგანიზაციის დავალიანება თვის დასაწყისში:'; en = 'Debt due to the organization at the beginning of the month:'");
			КонецЕсли; 
			Если ВыборкаФизлицо.СальдоНаКонец < 0 Тогда
				ОбластьПодвал.Параметры.ТекстДолгНаКонецПериода = НСТР("ru = 'Долг за работником на конец месяца:'; ka = 'თანამშრომლების დავალიანება თვის ბოლოს:'; en = 'Employee''s debt at the end of the month:'");
			Иначе	
				ОбластьПодвал.Параметры.ТекстДолгНаКонецПериода = НСТР("ru = 'Долг за организацией на конец месяца:'; ka = 'ორგანიზაციის დავალიანება თვის ბოლოს '; en = 'Debt due to the organization at the end of the month:'");
			КонецЕсли; 
...
Процедура ОтправитьНаСервере()
	...
	Если НЕ ПроверитьНастройкиПочты() Тогда 
		ТекстСообщения = НСтр("ka='პირველ რიგში აუცილებელია საფოსტო მისამართის აწყობების მითითება!';ru='Сперва требуеться указать настройки почтового ящика!';en='Сперва требуеться указать настройки почтового ящика!'");
	...
	Если Константы.ИспользоватьНесколькоОрганизаций.Получить() И НЕ ЗначениеЗаполнено(Организация) Тогда
		ТекстСообщения = НСтр("ka='არ არის არჩეული ორგანიზაცია!';ru='Не выбрана организация!';en='Not selected organization!'");
		ПолеСообщения = "Организация";
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(Отчет, ТекстСообщения,,,ПолеСообщения);
		Возврат;
	КонецЕсли;
