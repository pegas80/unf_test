...
Процедура МенеджерыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Представление",							НСтр("ka='менеджеры';ru='менеджеры';en='managers'"));
...
Процедура ДиаграммаНовыеОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВРег(Расшифровка) = ВРег("Новые") Тогда
		ПризнакРасшифровки = НСтр("ka='Новые';ru='Новые';en='new'");
		ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Анализ базы контрагентов (расшифровка %1)';ru='Анализ базы контрагентов (расшифровка %1)';en='Analysis base suppliers (transcript 1%)'"),
			НСтр("ka='новых';ru='новых';en='new'"));
	ИначеЕсли ВРег(Расшифровка) = ВРег("Существующие") Тогда
		ПризнакРасшифровки = НСтр("ka='Существующие';ru='Существующие';en='existing'");
		ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Анализ базы контрагентов (расшифровка %1)';ru='Анализ базы контрагентов (расшифровка %1)';en='Analysis base suppliers (transcript 1%)'"),
			НСтр("ka='существующих';ru='существующих';en='existing'"));
	КонецЕсли;
	
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 контрагенты за период: с %2 по %3%4%5%6';ru='%1 контрагенты за период: с %2 по %3%4%5%6';en='% 1 counterparties for the period: of from 2% 3% 4% 5% 6%'"),
		ПризнакРасшифровки,
...
Процедура ДиаграммаНовыеПоПериодамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВРег(ПараметрПериодичность) = ВРег("День") ИЛИ ВРег(ПараметрПериодичность) = "დღე" Тогда
		ПредставлениеПериода = Формат(Расшифровка, "ДФ=dd.MM.yyyy");
	ИначеЕсли ВРег(ПараметрПериодичность) = ВРег("Неделя") ИЛИ ВРег(ПараметрПериодичность) = "კვირა" Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კვირა %1-დან %2-მდე';ru='неделю с %1 по %2';en='week with 1% 2%'"),
			Формат(Расшифровка, "ДФ=dd.MM.yyyy"),
			Формат(КонецНедели(Расшифровка), "ДФ=dd.MM.yyyy"));
	ИначеЕсли ВРег(ПараметрПериодичность) = ВРег("Месяц") ИЛИ ВРег(ПараметрПериодичность) = "თვე" Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თვე %1-დან %2-მდე';ru='месяц с %1 по %2';en='month with% 1% 2'"),
			Формат(Расшифровка, "ДФ=dd.MM.yyyy"),
			Формат(КонецМесяца(Расшифровка), "ДФ=dd.MM.yyyy"));
	КонецЕсли;
	
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ახალი კონტრაგენტები პერიოდისთვის %1 %2%3%4';ru='Новые контрагенты за %1 %2%3%4';en='Counterparties for New% 1% 2% 3% 4'"),
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='კონტრაგენტების ბაზის ანალიზი (ახლების გაშიფვრა პერიოდისთვის)';ru='Анализ базы контрагентов (расшифровка новых за период)';en='Analysis counterparties base (new transcript for the period)'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ДиаграммаПоМенеджерамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Контрагенты менеджера: %1';ru='Контрагенты менеджера: %1';en='Accounts manager:% 1'"),
		?(ТипЗнч(Расшифровка) = Тип("Массив"), "<сводно>", Расшифровка),
		?(ЗначениеЗаполнено(ОтборТег), ", с тегом: " + ОтборТег, ""),
		?(ЗначениеЗаполнено(ОтборСегмент), ", в сегменте: " + ОтборСегмент, ""));
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ka='Анализ базы контрагентов (расшифровка по менеджерам)';ru='Анализ базы контрагентов (расшифровка по менеджерам)';en='Analysis base suppliers (transcript for managers)'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ОбновитьДиаграммуНовые()
	...
	Если ЗначениеЗаполнено(ОтборПериодЗначение.ДатаНачала) И ЗначениеЗаполнено(ОтборПериодЗначение.ДатаОкончания) Тогда
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ru = 'Новых за: '; ka = 'ახლები პერიოდისთვის: '") + Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yy") + " - " + Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yy");
	Иначе
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ka='ახლები: (პერიოდი არ არის მითითებული)';ru='Новых за: (период не указан)';en='New for: (period not specified)'");
	КонецЕсли;
	...
	Подсказка = НСтр("ru = 'Создано новых контрагентов за период: '; ka = 'შექმნილია ახალი კონტრაგენტები პერიოდისთვის: '") + Новых;
	...
	Подсказка = НСтр("ru = 'Существующих контрагентов: '; ka = 'არსებული კონტრაგენტები: '") + Существующих;
...
Процедура ОбновитьДиаграммуНовыеПоПериодам()
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "НЕДЕЛЯ", ВРег(XMLстрока(ПараметрПериодичность)));//Строка(ПараметрПериодичность) Паганини
	...
	Пока ВыборкаДатаСоздания.Следующий() Цикл
		...
		Подсказка = НСтр("ru='Создано ';ka='Создано '") + ВыборкаДатаСоздания.Новый + НСтр("ru=' контрагентов ';ka=' контрагентов '") + Текст;
...
Процедура ОбновитьДиаграммуПоМенеджерам()
	...
	Пока ВыборкаОтветственные.Следующий() Цикл
		...
		Если ПорядковыйНомерСерии < МаксимумСерийДиаграммы Тогда
			...
			ПредставлениеМенеджера = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ka='<Менеджер не указан>';ru='<Менеджер не указан>';en='<Manager Unknown>'"));
			Серия = ДиаграммаПоМенеджерам.Серии.Добавить(ПредставлениеМенеджера);
			Подсказка = ПредставлениеМенеджера + НСтр("ru=', контрагентов: ';ka=', контрагентов: '") + ВыборкаОтветственные.Контрагенты;
		...
		Иначе
			...
			ПредставлениеМенеджера = НСтр("ka='<Остальные>';ru='<Остальные>';en='<Remaining>'");
			...
			Подсказка = ПредставлениеМенеджера + НСтр("ru=', контрагентов: ';ka=', контрагентов: '") + (СводноеЗначение.Значение + ВыборкаОтветственные.Контрагенты);
			...
			ТекстПодробно = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ka='<Менеджер не указан>';ru='<Менеджер не указан>';en='<Manager Unknown>'"))
				+ НСтр("ru=', контрагентов: ';ka=', контрагентов: '") + ВыборкаОтветственные.Контрагенты;
...
Процедура ВыборПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора) Экспорт
	...
	Если ПараметрыВыбора.Вариант = Неопределено Или ПараметрыВыбора.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ka='Относительный...';ru='Относительный...';en='Relative...'"));
