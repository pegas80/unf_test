...
Функция ПервыйВариант()
	...
	СвойстваВарианта.Вставить("Заголовок",
		НСтр("ka='Даты запрета изменения данных по пользователям';ru='Даты запрета изменения данных по пользователям';en='The date of the ban on changing user data'"));
	
	СвойстваВарианта.Вставить("Описание",
		НСтр("ka='Выводит даты запрета изменения, сгруппированные по пользователям.';ru='Выводит даты запрета изменения, сгруппированные по пользователям.';en='Displays the date of the ban changes, grouped by users.'"));
...
Функция ВторойВариант()
	...
	Если Свойства.ПоказыватьРазделы И НЕ Свойства.ВсеРазделыБезОбъектов Тогда
		...
		Заголовок = НСтр("ka='Даты запрета изменения данных по разделам и объектам';ru='Даты запрета изменения данных по разделам и объектам';en='The date of the prohibition of data changes within the divisions and sites'");
		ОписаниеВарианта =
			НСтр("ka='Выводит даты запрета изменения, сгруппированные по разделам с объектами.';ru='Выводит даты запрета изменения, сгруппированные по разделам с объектами.';en='Displays the date of the ban changes grouped by sections of objects.'");
		
	ИначеЕсли Свойства.ВсеРазделыБезОбъектов Тогда
		ИмяВарианта = "ДатыЗапретаИзмененияПоРазделамДляПользователей";
		Заголовок = НСтр("ka='Даты запрета изменения данных по разделам';ru='Даты запрета изменения данных по разделам';en='The date of the ban on changing parts of the data'");
		ОписаниеВарианта =
			НСтр("ka='Выводит даты запрета изменения, сгруппированные по разделам.';ru='Выводит даты запрета изменения, сгруппированные по разделам.';en='Displays the date of the ban changes grouped by section.'");
	Иначе
		ИмяВарианта = "ДатыЗапретаИзмененияПоОбъектамДляПользователей";
		Заголовок = НСтр("ka='Даты запрета изменения данных по объектам';ru='Даты запрета изменения данных по объектам';en='The date of the ban on changing data objects'");
		ОписаниеВарианта =
			НСтр("ka='Выводит даты запрета изменения, сгруппированные по объектам.';ru='Выводит даты запрета изменения, сгруппированные по объектам.';en='Displays the date of the ban changes grouped by objects.'");
	КонецЕсли;
