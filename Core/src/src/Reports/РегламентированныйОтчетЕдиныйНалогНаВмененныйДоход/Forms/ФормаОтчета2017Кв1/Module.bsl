...
Процедура ОбработатьДополнительнуюСрокуДаты(ИмяОбласти)
	...
	Если НомерГиперссылки < МногострочнаяЧасть.Количество() Тогда
		...
		ТекстВопроса = НСтр("ka='Удалить выбранную дату?';ru='Удалить выбранную дату?';en='Remove the selected date?'");
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.ИП Тогда
			...
			Если РегистрацииВНО.Количество() = 0 Тогда
				...
				КодНО = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""';ru='Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""';en='It found the organization registration with the tax authority with numbers on 5-6 positions in PPC organization ""35"" or ""77""'");
				Сообщение.Сообщить();
			...
			Иначе
				...
				Иначе
					...
					КодНО = "";
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ka='Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.';ru='Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.';en='Found a 1st registration of the organization with the tax authority with numbers on 5-6 positions in PPC organization ""35"" or ""77"". Enter the tax authority code on the front page.'");
					Сообщение.Сообщить();
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыРаздела2 Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеТаблПоле.Значение.П000020003000) Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.СообщатьОНезаполненномКодеПоОКТМОВРазделе2 Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Не заполнен код ОКТМО на странице Раздела 2.';ru='Не заполнен код ОКТМО на странице Раздела 2.';en='code is empty OKTMO page Section 2.'");
				Сообщение.Сообщить();
			КонецЕсли;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если СтрНайти(Область.Имя, "П000010001003") > 0 ИЛИ Область.Имя = "П000020003000" Тогда
		...
		Если НЕ (ДлинаОКТМО = 0 ИЛИ ДлинаОКТМО = 8 ИЛИ ДлинаОКТМО = 11) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Код по ОКТМО должен состоять из 8 или 11 цифр.';ru='Код по ОКТМО должен состоять из 8 или 11 цифр.';en='by OKTMO code must consist of 8 or 11 digits.'");
			Сообщение.Сообщить();
	...
	Если Область.Имя = "ОКВЭД" Тогда
		...
		Если НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД.%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';ru='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД.%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';en='. On the cover sheet is set invalid value code type of activity OKVEhD% 1Dopustimy values ​​corresponding to templates as follows: 2% ""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX .XX ""(X - any number).'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
	...
	Если Область.Имя = "П000020010503" Тогда
		...
		Если НЕ (Область.Значение >= 7.5 И Область.Значение <= 15) Тогда
			...
			ПоказатьПредупреждение(,НСтр("ka='Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.';ru='Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.';en='Single tax rate can be set in the range from 7.5 to 15 percent.'"));
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		Если НЕ ЗначениеЗаполнено(КПП) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Для заполнения отчета необходимо значение КПП налогоплательщика. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';ru='Для заполнения отчета необходимо значение КПП налогоплательщика. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';en='To fill in the report, the CPT value of the taxpayer. Fixes in the business directory and update the information in the declaration (menu ""More"" - ""Update"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КПП) <> 9 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Задан КПП налогоплательщика, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';ru='Задан КПП налогоплательщика, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';en='CPR is given taxpayer, not consisting of 9 characters. Fixes in the business directory and update the information in the declaration (menu ""More"" - ""Update"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не задан код налогового органа, в который представляется декларация.';ru='Не задан код налогового органа, в который представляется декларация.';en='Do not set the tax code of the body in which the declaration is submitted.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(КодНО) <> 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Задан код налогового органа, не состоящий из 4 цифр.';ru='Задан код налогового органа, не состоящий из 4 цифр.';en='Given the tax code of the body, not the 4-digit.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ СтатусПроверки Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Заполнение отчета прервано.';ru='Заполнение отчета прервано.';en='Filling the report is interrupted.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.';ru='Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.';en='For the taxpayer to% 1 not found a corresponding entry in the ""Registration with the tax authority."" Filling Declaration interrupted.'"),
...
Процедура ПроверитьТочность()
	...
	Если СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1 Тогда
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 2 Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1: при выводе сумм в рублях точность не может превышать 2 знака.';ru='%1: при выводе сумм в рублях точность не может превышать 2 знака.';en='% 1: the output sums in rubles accuracy can not exceed 2 characters.'"), ЭтаФорма.Заголовок);
			Сообщение.Сообщить();
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если НЕ РегламентированнаяОтчетность.ПринтерДоступен() Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Перед формированием печатных форм необходимо определить в системе принтер и задать его в качестве используемого по умолчанию.';ru='Перед формированием печатных форм необходимо определить в системе принтер и задать его в качестве используемого по умолчанию.';en='Before the formation of printing plates need to define the printer system and set it as the default.'");
		Сообщение.Сообщить();
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 1 (КС 1.1)';ru='Проверка раздела 1 (КС 1.1)';en='Checking section 1 (1.1 KS)'");
...
"всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)';ka='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 Раздел1 по ОКТМО = %1)"
"должна быть равна произведению суммы налога, подлежащей уплате в бюджет (строка 040 Раздела 3) и результата деления суммы строк 110"
"всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)'");
	
	Нарушение      = НСтр("ka='Арифметическая ошибка';ru='Арифметическая ошибка';en='arithmetical mistake'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.2)';ru='Проверка раздела 2 (КС 1.2)';en='Checking section 2 (COP 1.2)'");
	ОписаниеКС = НСтр("ka='Код вида предпринимательской деятельности (строка 010 Раздела 2) должен соответствовать значению из справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';ru='Код вида предпринимательской деятельности (строка 010 Раздела 2) должен соответствовать значению из справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';en='Code business activity (line 010 in Section 2) should match the value of the reference book ""Codes types of business activities and the value of base yields per unit physical indicator""'");
	
	Нарушение      = НСтр("ka='Статья 346.27 НК РФ';ru='Статья 346.27 НК РФ';en='Article 346.27 of the Tax Code'");
	ОписаниеОшибки = НСтр("ka='Неправомерное применение системы налогообложения в виде ЕНВД';ru='Неправомерное применение системы налогообложения в виде ЕНВД';en='Misuse of the system of taxation in the form of UTII'");
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.3)';ru='Проверка раздела 2 (КС 1.3)';en='Checking section 2 (COP 1.3)'");
	ОписаниеКС   = НСтр("ka='Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода';ru='Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода';en='Tax base in the 1st month period (row 070 column 4 of Section 2) must be equal to the product of the values ​​of lines 040, 050, 060, rows 070 columns 2, rows 070 columns 3 divided by the number of calendar days in the first month of the reporting period,'");
	
	Нарушение      = НСтр("ka='Пункт 10 статьи 346.29 НК РФ';ru='Пункт 10 статьи 346.29 НК РФ';en='Paragraph 10 of Article 346.29 of the Tax Code'");
	ОписаниеОшибки = НСтр("ka='Неправильное исчисление налоговой базы за данный месяц';ru='Неправильное исчисление налоговой базы за данный месяц';en='Incorrect calculation of the tax base in a given month'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.4)';ru='Проверка раздела 2 (КС 1.4)';en='Checking section 2 (COP 1.4)'");
	ОписаниеКС   = НСтр("ka='Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода';ru='Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода';en='Tax base in the 2nd month period (row 080 column 4 of Section 2) must be equal to the product of the values ​​of lines 040, 050, 060, rows 080 columns 2, rows 080 columns 3 divided by the number of calendar days in the second month of the reporting period,'");
...
Процедура ПроверитьКС_1_5(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.5)';ru='Проверка раздела 2 (КС 1.5)';en='Checking section 2 (COP 1.5)'");
	ОписаниеКС   = НСтр("ka='Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода';ru='Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода';en='Tax base in the 3rd month period (row 090 column 4 of Section 2) must be equal to the product of the values ​​of rows 040, 050, 060, rows 090 columns 2, rows 090 columns 3 divided by the number of calendar days in the 3rd month of the reporting period,'");
...
Процедура ПроверитьКС_1_6(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.6)';ru='Проверка раздела 2 (КС 1.6)';en='Checking section 2 (COP 1.6)'");
	ОписаниеКС   = НСтр("ka='Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)';ru='Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)';en='The amount of tax base UTII (line 100 of section 2) must be equal to the sum of values ​​in the lines 070, 080, 090 columns 4 of Section 2 on same OKTMO (line 030)'");
	
	Нарушение      = НСтр("ka='п.п. 2, 4 ст.346.29 НК РФ';ru='п.п. 2, 4 ст.346.29 НК РФ';en='pp 2, 4 st.346.29 Tax Code'");
	ОписаниеОшибки = НСтр("ka='Неправильно исчислена налоговая база за налоговый период.';ru='Неправильно исчислена налоговая база за налоговый период.';en='It is calculated incorrectly, the tax base for the tax period.'");
...
Процедура ПроверитьКС_1_7(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 2 (КС 1.7)';ru='Проверка раздела 2 (КС 1.7)';en='Checking section 2 (COP 1.7)'");
	ОписаниеКС   = НСтр("ka='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100';ru='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100';en='Amount calculated single imputed income tax (line 110 of section 2) must be equal to the product of the fiscal base (line 100 of Section 2) and the tax rate on same OKTMO (line 105 in Section 2) divided by 100'");
	
	Нарушение      = НСтр("ka='п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ';ru='п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ';en='pp 2, 4 and st.346.29 st.246.31 Tax Code'");
	ОписаниеОшибки = НСтр("ka='Неправильное исчисление суммы ЕНВД за налоговый период';ru='Неправильное исчисление суммы ЕНВД за налоговый период';en='Incorrect calculation of the amount of the tax period UTII'");
...
Процедура ПроверитьКС_1_8(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 3 (КС 1.8)';ru='Проверка раздела 3 (КС 1.8)';en='Testing Section 3 (COP 1.8)'");
	ОписаниеКС   = НСтр("ka='Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2';ru='Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2';en='Amount calculated single imputed income tax (line 010 Unit 3) must be equal to the sum of all the rows 110 filled Sections 2'");
...
Процедура ПроверитьКС_1_9(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 3 (КС 1.9)';ru='Проверка раздела 3 (КС 1.9)';en='Testing Section 3 (COP 1.9)'");
	ОписаниеКС   = НСтр("ka='Сумма единого налога на вмененный доход, подлежащая к уплате (строка 040 Раздела 3) должна быть равна разнице значений строки 010 Раздела 3 и строк 020, 030 Раздела 3, если результат такого вычитания больше или равен 50% строки 010 Раздела 3 и строка 005 Раздела 3 = ""1""';ru='Сумма единого налога на вмененный доход, подлежащая к уплате (строка 040 Раздела 3) должна быть равна разнице значений строки 010 Раздела 3 и строк 020, 030 Раздела 3, если результат такого вычитания больше или равен 50% строки 010 Раздела 3 и строка 005 Раздела 3 = ""1""';en='Amount single imputed income tax to be paid (line 040 Section 3) must be equal to the difference of line 010, Section 3, and lines 020 values, 030 Section 3, when the result of the subtraction is greater than or equal to 50% of the lines 010, Section 3, and line 005 of Section 3 = ""1""'");
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ka='Проверка раздела 3 (КС 1.10)';ru='Проверка раздела 3 (КС 1.10)';en='Testing Section 3 (COP 1.10)'");
	ОписаниеКС   = НСтр("ka='Сумма единого налога на вмененный доход, подлежащая к уплате (строка 040 Раздела 3) должна быть равна разнице значений по строкам 010 Раздела 3 и по строке 030 Раздела 3, если результат такого вычитания больше или равен нулю и строка 005 Раздела 3 = ""2""';ru='Сумма единого налога на вмененный доход, подлежащая к уплате (строка 040 Раздела 3) должна быть равна разнице значений по строкам 010 Раздела 3 и по строке 030 Раздела 3, если результат такого вычитания больше или равен нулю и строка 005 Раздела 3 = ""2""';en='Amount single imputed income tax to be paid (line 040 Section 3) must be equal to the difference values ​​in the lines 010, Section 3, and line 030, Section 3, when the result of the subtraction is greater than or equal to zero and the row 005, Section 3 = ""2""'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "КодОКТМО" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Код по ОКТМО (строка 010 раздела 1)';ru='Код по ОКТМО (строка 010 раздела 1)';en='OKTMO code (line 010 section 1)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр010" Тогда
		Результат.Вставить("Имя", "П000030001003");
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного за налоговый период единого налога на вмененный доход';ru='Сумма исчисленного за налоговый период единого налога на вмененный доход';en='The sum calculated for the tax period of a single tax on imputed income'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)';ru='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)';en='The sum of the single tax on imputed income, payable for the fiscal period (line 020 section 1)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр040" Тогда
		Результат.Вставить("Имя", "П000030004003");
		Результат.Вставить("Представление", НСтр("ka='Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период';ru='Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период';en='The total amount of the single tax on imputed income, payable to the budget for the tax period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр110" Тогда
		Результат.Вставить("Имя", "П000020011003");
		Результат.Вставить("Представление", НСтр("ka='Сумма исчисленного единого налога на вмененный доход за налоговый период';ru='Сумма исчисленного единого налога на вмененный доход за налоговый период';en='The amount of calculated single tax on imputed income for the tax period'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Налоговая база всего';ru='Налоговая база всего';en='The tax base of all'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070" Тогда
		Результат.Вставить("Имя", "П000020007004");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база в 1 месяце квартала';ru='Налоговая база в 1 месяце квартала';en='The tax base of the quarter in 1 month'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080" Тогда
		Результат.Вставить("Имя", "П000020008004");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база во 2 месяце квартала';ru='Налоговая база во 2 месяце квартала';en='The tax base in the 2nd quarter of the month'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090" Тогда
		Результат.Вставить("Имя", "П000020009004");
		Результат.Вставить("Представление", НСтр("ka='Налоговая база в 3 месяце квартала';ru='Налоговая база в 3 месяце квартала';en='The tax base in the quarter, 3 month'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ka='Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога';ru='Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога';en='The amount of insurance premiums, which can be reduced by the amount calculated for the tax period tax'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр030" Тогда
		Результат.Вставить("Имя", "П000030003003");
		Результат.Вставить("Представление", НСтр("ka='Сумма стаховых взносов, уплаченных индивидуальным предпринимателем в Пенсионный фонд Российской Федерации и в Федеральный фонд обязательного медицинского страхования в фиксированном размере';ru='Сумма стаховых взносов, уплаченных индивидуальным предпринимателем в Пенсионный фонд Российской Федерации и в Федеральный фонд обязательного медицинского страхования в фиксированном размере';en='Stahovs sum of contributions paid by an individual entrepreneur in the Russian Federation Pension Fund and in the Federal Compulsory Medical Insurance Fund a fixed amount'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр005" Тогда
		Результат.Вставить("Имя", "П000030000503");
		Результат.Вставить("Представление", НСтр("ka='Признак налогоплательщика';ru='Признак налогоплательщика';en='taxpayer sign'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр040" Тогда
		Результат.Вставить("Имя", "П000020004003");
		Результат.Вставить("Представление", НСтр("ka='Базовая доходность на единицу физического показателя в месяц';ru='Базовая доходность на единицу физического показателя в месяц';en='Basic yield per unit of physical indicators in the month'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр010" Тогда
		Результат.Вставить("Имя", "П000020001000");
		Результат.Вставить("Представление", НСтр("ka='Код вида предпринимательской деятельности';ru='Код вида предпринимательской деятельности';en='Code of the type of entrepreneurial activity'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр050" Тогда
		Результат.Вставить("Имя", "П000020005003");
		Результат.Вставить("Представление", НСтр("ka='Корректирующий коэффициент К1';ru='Корректирующий коэффициент К1';en='The correction coefficient K1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр060" Тогда
		Результат.Вставить("Имя", "П000020006003");
		Результат.Вставить("Представление", НСтр("ka='Корректирующий коэффициент К2';ru='Корректирующий коэффициент К2';en='The correction coefficient K2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр02" Тогда
		Результат.Вставить("Имя", "П000020007002");
		Результат.Вставить("Представление", НСтр("ka='Величина физического показателя';ru='Величина физического показателя';en='The magnitude of the physical parameter'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр03" Тогда
		Результат.Вставить("Имя", "П000020007003");
		Результат.Вставить("Представление", НСтр("ka='Количество календарных дней';ru='Количество календарных дней';en='Number of calendar days'"));
