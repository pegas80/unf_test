...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ka='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.';ru='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.';en='Generating report ""Counterparty File"" in this configuration is not possible.'");
...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Поле ""ИНН или наименование контрагента"" не заполнено';ru='Поле ""ИНН или наименование контрагента"" не заполнено';en='The ""Tax ID or name of the counterparty"" not filled'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормойНаСервере()
	...
	Если ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Досье контрагента: %1';ru='Досье контрагента: %1';en='counterparty Record:% 1'"),
	...
	ИначеЕсли ЗначениеЗаполнено(Контрагент) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Контрагент);
	Иначе
		Заголовок = НСтр("ka='კონტრაგენტის დოსიე';ru='Досье контрагента';en='counterparty dossier'");
	КонецЕсли;
	Элементы.ДекорацияРазделДанныеГосРеестров.Заголовок = ?(ЭтоЮридическоеЛицо, 
		НСтр("ka='ЕГРЮЛ';ru='ЕГРЮЛ';en='USRLE'"), 
		НСтр("ka='ЕГРИП';ru='ЕГРИП';en='EGRIP'"));
...
Процедура СформироватьОтчетНаКлиенте(ТекстПоиска)
	...
	Если ПоискПоИНН Тогда
		...
		Если СтрДлина(ТекстПоиска) <> 10 
			...
			ТекстОшибки = НСтр("ka='Для поиска по ИНН необходимо ввести 10 или 12 цифр.';ru='Для поиска по ИНН необходимо ввести 10 или 12 цифр.';en='To search for a VAT number you must enter 10 or 12 digits.'");
	...
	Иначе // Поиск по наименованию
		...
		ПараметрыФормы.Вставить("Заголовок",    НСтр("ka='Поиск контрагента';ru='Поиск контрагента';en='Search contractor'"));
...
Функция СформироватьОтчетНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Формирование отчета: Досье контрагента: %1';ru='Формирование отчета: Досье контрагента: %1';en='Report Generation: counterparty Dossier:% 1'"), 
		ИННПоиска);
...
Процедура ОбработатьОшибкиФормированияОтчета()
	...
	Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
		...
		ТекстВопроса = НСтр("ka='Для формирования ""Досье контрагента"""
"необходимо подключиться к интернет-поддержке пользователей."
"Подключиться сейчас?';ru='Для формирования ""Досье контрагента"""
"необходимо подключиться к интернет-поддержке пользователей."
"Подключиться сейчас?'");
