...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Фильтр = НСтр("ka='ფაილი xml';ru='Файл xml';en='XML file'")
		+ "(*.xml)|*.xml";
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ka='Укажите файл, в который надо экспортировать список лент новостей';ru='Укажите файл, в который надо экспортировать список лент новостей';en='Specify the file to which you want to export a list of news feeds'");
...
Процедура КомандаЭкспорт(Команда)
	...
	Если (ВРег(ЭтаФорма.ФорматНовостей) <> ВРег("rss"))
		...
		Сообщение.Текст = НСтр("ka='აირჩიეთ ახალი ამბების სწორი ფორმატი';ru='Выберите правильный формат новостей';en='Select the correct format the news'");
	...
	Если ПустаяСтрока(ЭтаФорма.ИмяФайла) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='მიუთითეთ ფაილის სახელი, სადაც უნდა შევინახოთ ახალი ამბების სია';ru='Укажите имя файла, куда сохранить список новостей';en='Specify the name of the file where to save the list of news'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='ექსპორტი დასრულებულია';ru='Экспорт завершен';en='export finished'"),
		,
		НСтр("ru='Список лент новостей успешно экспортирован в файл."
"Теперь в стороннем rss-агрегаторе можно выбрать из меню [Импортировать список лент новостей из OPML файла] и указать полученный файл (при условии, что rss-агрегатор поддерживает эту возможность).';ka='ახალი ამბების ლენტების სია წარმატებით იქნა ფაილში ექსპორტირებული."
"ეხლა, გარეშე rss-აგრეგატორში შეიძლება, რომ მენიუდან [ახალი ამბების ლენტების სიის იმპორტირება OPML ფაილიდან] ავირჩიოთ და მივუთითოთ მიღებული ფაილი (იმ პირობით, რომ rss-აგრეგატორის მიერ მხარდაჭერილია ეს შესაძლებლობა).'"),
	...
	ЗаписьХМЛ.ЗаписатьАтрибут("version", "1.1");
		ЗаписьХМЛ.ЗаписатьНачалоЭлемента("head");
			ЗаписьХМЛ.ЗаписатьНачалоЭлемента("title");
				ЗаписьХМЛ.ЗаписатьТекст(НСтр("ka='Список лент новостей (1С)';ru='Список лент новостей (1С)';en='The list of news feeds (1C)'"));
		...
		ЗаписьХМЛ.ЗаписатьНачалоЭлемента("body");
			ЗаписьХМЛ.ЗаписатьНачалоЭлемента("outline");
				ЗаписьХМЛ.ЗаписатьАтрибут("text", НСтр("ka='სიახლეები 1С';ru='Новости 1С';en='News 1C'"));
