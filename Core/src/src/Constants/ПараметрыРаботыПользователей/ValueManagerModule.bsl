...
Процедура ПроверитьНазначениеРолей(НазначениеРолей = Неопределено, ПроверитьВсе = Ложь, СписокОшибок = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ka='შეცდომა საერთო მოდულ მომხმარებლებიხელახლაგანსაზღვრებადი-ს პროცედურა როლებისდანიშვნისდადგენისას-ში';ru='Ошибка в процедуре ПриОпределенииНазначенияРолей общего модуля ПользователиПереопределяемый.';en='general module error in procedure PriOpredeleniiNaznacheniyaRoley PolzovateliPereopredelyaemy.'");
		...
		Для Каждого КлючИЗначение Из ОписаниеНазначенияРолей.Значение Цикл
			...
			Если Роль = Неопределено Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='დანიშვნა %2-ში მითითებული როლი ""%1"" მეტამონაცემებში ვერ მოიძებნა.';ru='В метаданных не найдена роль ""%1"","
...
"%2, %3.';ka='როლი ""%1"" ერთზე მეტ დანიშნულებაში არის მითითებული: %2, %3.'"),
	...
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.ТолькоДляВнешнихПользователей, ТекстОшибки,
		НСтр("ka='მხოლოდ გარე მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей только для внешних пользователей найдены ошибки:';en='When checking the roles only for external users found error:'"), СписокОшибок);
	
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.СовместноДляПользователейИВнешнихПользователей, ТекстОшибки,
		НСтр("ka='ერთობლივად მომხმარებლებისთვის და გარე მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей совместно для пользователей и внешних пользователей найдены ошибки:';en='When checking roles together users and external users found error:'"), СписокОшибок);
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ka='დანართის მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей для пользователей приложения найдены ошибки:';en='When checking the roles for the user application errors are found:'"), СписокОшибок, НеразделенныеДанные);
	КонецЕсли;
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ka='მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей для пользователей найдены ошибки:';en='When checking the roles for user errors are found:'"), СписокОшибок);
	...
	Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
		...
		Для Каждого НедоступноеПраво Из НедоступныеПрава Цикл
			Если ПравоДоступа(НедоступноеПраво, Метаданные, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს მიუწვდომელ უფლება %2-ს.';ru='Роль ""%1"" содержит недоступное право %2.';en='Role ""%1"" contains 2% right inaccessible.'"),
		...
		Для Каждого СвойстваДанных Из НеразделенныеДанные Цикл
			...
			Если ПравоДоступа("Изменение", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს უფლება ""განუცალკევებელი ობიექტის ცვლილება""-ს %2.';ru='Роль ""%1"" содержит право Изменение неразделенного объекта %2.';en='Role ""%1"" contains the right Change undivided object 2%.'"),
			...
			Если ПравоДоступа("Добавление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს უფლება ""განუცალკევებელი ობიექტის დამატება""-ს %2.';ru='Роль ""%1"" содержит право Добавление неразделенного объекта %2.';en='Role ""%1"" contains the right undivided object Adding 2%.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
			...
			Если ПравоДоступа("Удаление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს უფლება ""განუცალკევებელი ობიექტის წაშლა""-ს %2.';ru='Роль ""%1"" содержит право Удаление неразделенного объекта %2.';en='Role ""%1"" contains the delete rule undivided object 2%.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
