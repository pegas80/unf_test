...
Процедура СнятьСМониторинга(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ka='Снять выбранных контрагентов с мониторинга?';ru='Снять выбранных контрагентов с мониторинга?';en='Remove the selected contractors to monitor?'"),
...
Процедура ПодборКонтрагентовДляМониторинга(Команда)
	...
	Иначе
		СправочникиКонтрагенты.ПоказатьВыборЭлемента(
			...
			НСтр("ka='Выбор справочника контрагентов';ru='Выбор справочника контрагентов';en='Selection of counterparties directory'"));
	КонецЕсли;
...
Процедура ПриОтветеНаВопросОСнятииСМониторинга(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		...
		Если УдаленоЗаписей = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ka='Среди выбранных контрагентов нет поставленных вручную.';ru='Среди выбранных контрагентов нет поставленных вручную.';en='Among the selected counterparties are no set manually.'"));
		КонецЕсли;
	КонецЕсли;
