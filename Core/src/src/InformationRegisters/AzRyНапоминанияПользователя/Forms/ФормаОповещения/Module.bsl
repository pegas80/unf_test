...
Процедура ОбновитьВремяВТаблицеНапоминаний()
	...
	Для Каждого СтрокаТаблицы Из Напоминания Цикл
		ПредставлениеВремени = НСтр("ka='ვადა არ არის დადგენილი';ru='срок не определен';en='term is not defined'");
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ВремяСобытия) Тогда
			...
			Если СтрокаТаблицы.ВремяСобытия - НачалоДня(СтрокаТаблицы.ВремяСобытия) < 60 // События "на весь день".
				И НачалоДня(СтрокаТаблицы.ВремяСобытия) = НачалоДня(ТекущаяДата) Тогда
					ПредставлениеВремени = НСтр("ka='დღეს';ru='сегодня';en='Today'");
			Иначе
...
Функция ПредставлениеИнтервалаВремени(Знач КоличествоВремени)
	...
	ПредставлениеНедель = НСтр("ka='კვირას';ru='неделю';en='week'") + "," + НСтр("ka='კვირა';ru='недели';en='of the week'") + "," + НСтр("ka='კვირა';ru='недель';en='weeks'");
	ПредставлениеДней = НСтр("ka='დღე';ru='день';en='day'") + "," + НСтр("ka='დღე';ru='дня';en='day'") + "," + НСтр("ka='დღე';ru='дней';en='days'");
	ПредставлениеЧасов = НСтр("ka='სთ';ru='час';en='hour'") + "," + НСтр("ka='საათები';ru='часа';en='hours'") + "," + НСтр("ka='საათი';ru='часов';en='hours'");
	ПредставлениеМинут = НСтр("ka='წუთს';ru='минуту';en='minute'") + "," + НСтр("ka='წუთები';ru='минуты';en='minutes'") + "," + НСтр("ka='წუთი';ru='минут';en='minutes'");
	...
	ШаблонПредставления = НСтр("ka='%1 უკან';ru='%1 назад';en='Ago 1%'");
	Если КоличествоВремени < 0 Тогда
		ШаблонПредставления = НСтр("ka='%1-ის შემდეგ';ru='через %1';en='in 1'");
	...
	Если КоличествоНедель > 4 Тогда
		Если СобытиеНаступило Тогда
			Возврат НСтр("ka='ძალიან დიდი ხნი წინ';ru='очень давно';en='a long time ago'");
		Иначе
			Возврат НСтр("ka='მალე არ იქნება';ru='еще не скоро';en='will not soon'");
		КонецЕсли;
	...
	ИначеЕсли КоличествоНедель > 0 Тогда
		Результат = НСтр("ka='კვირას';ru='неделю';en='week'");
		
	ИначеЕсли КоличествоДней > 1 Тогда
		Если НачалоДня(ТекущаяДата) - НачалоДня(ТекущаяДата - КоличествоВремени) = 60*60*24 * 2 Тогда
			Если СобытиеНаступило Тогда
				Возврат НСтр("ka='გუშინწინ';ru='позавчера';en='the day before yesterday'");
			Иначе
				Возврат НСтр("ka='ზეგ';ru='послезавтра';en='day after tomorrow'");
			КонецЕсли;
		Иначе
	...
	ИначеЕсли КоличествоЧасов + КоличествоДней * 24 > 3 И Не СобытиеСегодня Тогда
			...
			Иначе
				Возврат НСтр("ka='ხვალ';ru='завтра';en='tomorrow'");
			КонецЕсли;
	ИначеЕсли КоличествоДней > 0 Тогда
		Результат = НСтр("ka='დღე';ru='день';en='day'");
	...
	ИначеЕсли КоличествоЧасов > 0 Тогда
		Результат = НСтр("ka='სთ';ru='час';en='hour'");
	...
	ИначеЕсли КоличествоМинут > 0 Тогда
		Результат = НСтр("ka='წუთს';ru='минуту';en='minute'");
		
	Иначе
		Возврат НСтр("ka='ახლა';ru='сейчас';en='now'");
	КонецЕсли;
