...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если КоличествоОбновлений > 0 Тогда
			...
			ИнформацияОбОбновление = НСтр("ka='Доступно обновление от';ru='Доступно обновление от';en='An update is available from the'") + " " + ДатаОбновления;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ka='შევწყვიტოთ სამისამართო კლასიფიკატორის ჩატვირთვა?';ru='Прервать загрузку адресного классификатора?';en='Interrupt the download address of the classifier?'");
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
		НСтр("ka='კატალოგი სამისამართო კლასიფიკატორის ფაილებიანად';ru='Каталог с файлами адресного классификатора';en='Directory with the address of the classifier files'"),
...
Процедура Загрузить(Команда)
	...
	Если КодИсточникаЗагрузки = "КАТАЛОГ" Тогда
		Текст = НСтр("ka='საქაღალდიდან სამისამართო კლასიფიკატორის ჩასატვირთად აუცილებელია გაფართოების დაყენება ფაილებთან სამუშაოდ.';ru='Для загрузки адресного классификатора из папки"
"необходимо установить расширение для работы с файлами.'");
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю( НСтр("ka='Не указан вариант загрузки классификатора.';ru='Не указан вариант загрузки классификатора.';en='Unknown download option classifier.'") );
	КонецЕсли;
...
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Даты.ДатаОбновления > Даты.ДатаСамойСтаройЗагрузки Тогда
			...
			ИнформацияОбОбновление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-ის განახლება ხელმისაწვდომია';ru='Доступно обновление от %1';en='An update is available from the 1%'"), ДатаОбновленияСтрокой);
			Элементы.ИнформацияОбОбновление.Видимость = НЕ Элементы.АвторизацияНаСайтеПоддержкиПользователей.Видимость;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапуститьПроверкуДоступностиОбновления(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Проверка обновления для адресного классификатора';ru='Проверка обновления для адресного классификатора';en='Verify upgrade for the address of the classifier'");
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='არჩეული რეგიონების მიხედვით მიმდინარეობს მონაცემთა ჩატვირთვა (%1)';ru='Загружаются данные по выбранным регионам (%1)';en='Loading data in selected regions (1%)'"), ВыбраноРегионовДляЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач Аутентификация, Знач КодыРегионов)
	...
	Элементы.ШагиЗагрузки.ТекущаяСтраница = Элементы.ОжиданиеЗагрузки;
	ТекстСостоянияЗагрузки = НСтр("ka='ფაილების ჩატვირთვა მომხმარებლების მხარდაჭერის საიტიდან...';ru='Загрузка файлов с сайта поддержки пользователей...';en='Downloading files with customer support site ...'");
...
Процедура ПослеФоновойЗагрузкиССайта(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		Если СтрНайти(Задание.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Задание.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			ТекстОшибки = НСтр("ka='Не удается загрузить адресные сведения.';ru='Не удается загрузить адресные сведения.';en='Unable to load address information.'");
			ТекстОшибки = ТекстОшибки + НСтр("ka='Возможные причины:';ru='Возможные причины:';en='Possible reasons:'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='• Некорректно введен или не введен логин и пароль;';ru='• Некорректно введен или не введен логин и пароль;';en='• Incorrectly entered or not entered your login and password;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* ინტერნეტთან შეერთება არ არის;';ru='• Нет подключения к Интернету;';en='• No Internet connection;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* საიტზე მიმდინარეობს ტექნიკური სამუშაოები. მოგვიანებით სცადეთ ჩატვირთვის გამეორება.';ru='• На сайте ведутся технические работы. Попробуйте повторить загрузку позднее.';en='• On are performing site maintenance. Boot it up again later.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* ბრანდმაუერი ან სხვა შუალედური პროგრამული უზრუნველყოფა (ანტივირუსები და მისთ.) ბლოკავენ ინტერნეტთან შეერთების პროგრამის მცდელობებს;';ru='• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;';en='• Firewall or other middleware (antivirus, etc.) block the program attempts to connect to the Internet;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='* ინტერნეტთან შეერთება ხორციელდება პროქსი-სერვერით, მაგრამ მისი პარამეტრები პროგრამაში არ არის მიცემული.';ru='• Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в программе.';en='• Internet connection is performed via a proxy server, but the parameters are not specified in the program.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='Техническая информация:';ru='Техническая информация:';en='Technical information:'") + " " + Символы.ПС;
			ТекстОшибки = ТекстОшибки + СтрПолучитьСтроку(Задание.КраткоеПредставлениеОшибки, 1);
		Иначе
			ТекстОшибки = НСтр("ka='Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.';ru='Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.';en='Probably at this time on the site are carried out engineering work. Boot it up again later.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ka='Техническая информация:';ru='Техническая информация:';en='Technical information:'") + " " + Символы.ПС;
...
Процедура ЗавершениеЗагрузки()
	...
	// Для сброса признака АдресныйКлассификаторУстарел в параметрах работы клиента.
		...
		ТекстОписанияЗагрузки = НСтр("ka='სამისამართო კლასიფიკატორი წარმატებით ჩაიტვირთა.';ru='Адресный классификатор успешно загружен.';en='The address classifier successfully loaded.'");
		...
		Если ЕстьУстаревшиеАдресныеСведенияКЛАДРДляУдаления() Тогда
			...
			ТекстВопроса = НСтр("ka='Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.';ru='Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.';en='Found outdated information about the regions in KLADR format, which are not used in the program.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ka='თუ ჩატვირთული რაიონები ბევრია, მაშინ გასუფთავების პროცესს შეიძლება ხანგრძლივი დრო დაჭირდეს.';ru='Если загруженных регионов много, то процесс очистки может занять продолжительное время.';en='If you downloaded a lot of regions, the cleaning process can take a long time.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ka='Очистить?';ru='Очистить?';en='Clear?'");
...
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, Аутентификация, УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='სამისამართო კლასიფიკატორის ჩატვირთვა საიტიდან';ru='Загрузка адресного классификатора с сайта';en='Loading address classifier site'");
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиента(Знач ПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстСостоянияЗагрузки = НСтр("ka='Передача файлов на сервер приложения...';ru='Передача файлов на сервер приложения...';en='Transferring files to the application server ...'");
...
Процедура ПослеФоновойЗагрузкиИзКаталога(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		ТекстОшибки = НСтр("ka='Не удается загрузить адресные сведения из файлов.';ru='Не удается загрузить адресные сведения из файлов.';en='Unable to download the address information of the files.'");
		ТекстОшибки = ТекстОшибки + НСтр("ka='Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.';ru='Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.';en='It is necessary to keep the files from the website ""1C» http://its.1c.ru/download/fias to disk and then load into the program.'") + Символы.ПС;
		ТекстОшибки = ТекстОшибки + НСтр("ka='Техническая информация:';ru='Техническая информация:';en='Technical information:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
		ВывестиСообщениеОбОшибке(ТекстОшибки);
...
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='სამისამართო კლასიფიკატორის ჩატვირთვა';ru='Загрузка адресного классификатора';en='Loading address classifier'");
...
Процедура ОчиститьУстаревшиеАдресныеСведенияКЛАДР()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Очистка адресного классификатора в формате КЛАДР';ru='Очистка адресного классификатора в формате КЛАДР';en='Cleaning the address classifier KLADR format'");
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='Очистка адресного классификатора в формате КЛАДР';ru='Очистка адресного классификатора в формате КЛАДР';en='Cleaning the address classifier KLADR format'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,
