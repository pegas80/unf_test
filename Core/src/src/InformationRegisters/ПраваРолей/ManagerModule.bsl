...
Процедура ОбновитьДанныеРегистра(ЕстьИзменения = Неопределено, ТолькоПроверка = Ложь) Экспорт
	...
	Для Каждого ВозможныеПрава Из ВозможныеПраваОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из Метаданные[ВозможныеПрава.Коллекция] Цикл
			// Kuzmich, 05.01.2023, (ONESOFT-2752) Добавление параметров в настройках пользователя
			// Необходимо, чтобы обойти ошибку при обновлении для регистров сведений (и других объектов) в расширениях
			// Объекты из расширений не проверяем
...
"в справочнике ""Идентификаторы объектов метаданных"".';ka='შეცდომა შემდეგი ფუნქციის შესრულებისას საერთოდანიშნულების.მეტამონაცემთაობიექტისიდენტიფიკატორი()."
""
"მეტამონაცემთა ობიექტ ""%1""-ისთვის ცნობარ ""მეტამონაცემთა ობიექტების იდენტიფიკატორებში"" იდენტიფიკატორი ვერ მოიძებნა.'"),
				ПолноеИмя)
