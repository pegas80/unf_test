...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрокаКолонок = СтрокаКолонок + ", ПризнакАванса";
...
Процедура ЗаполнитьАвансыИДолги()
	...
	Запрос.УстановитьПараметр("ГраницаОтчета", новый Граница(Дата, ВидГраницы.Исключая));
	...
	Запрос.УстановитьПараметр("ИспользоватьТолькоРегламентированногоПланаСчетов", ИспользоватьТолькоРегламентированногоПланаСчетов);
	...
	|	ОтобранныеДолги.ПризнакАванса,//Малышев 2025-02-24 ONESOFT-3869
	|	ОтобранныеДолги.Курс,//Малышев 2025-02-24 ONESOFT-3869
	|	ОтобранныеДолги.Кратность,//Малышев 2025-02-24 ONESOFT-3869
	...
	Если ВидПрочегоВзаимозачета = Перечисления.ВидыПрочегоВзаимозачета.ДолгПрочегоКонтрагентаИДолгПередПоставщиком ТОгда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	1 КАК Порядок,
		...
		|	-ЕСТЬNULL(РасчетыСПрочимиКонтрагентамиОстатки.СуммаВалОстаток, 0) КАК СуммаВалОстаток,
		|	-ЕСТЬNULL(РасчетыСПрочимиКонтрагентамиОстатки.СуммаОстаток, 0) КАК СуммаОстаток
		|ПОМЕСТИТЬ СуммыРегистров
		|ИЗ
		...
		|						ТОГДА СчетУчетаРегл = &СчетУчета
		|					ИНАЧЕ СчетУчета = &СчетУчета
		|				КОНЕЦ) КАК РасчетыСПрочимиКонтрагентамиОстатки
		|ГДЕ
		|	РасчетыСПрочимиКонтрагентамиОстатки.СуммаОстаток < 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	|	Т.ПризнакАванса КАК ПризнакАванса,
	|	СУММА(Т.СуммаВалОстаток) КАК СуммаРасчетов,
	|	СУММА(Т.СуммаОстаток) КАК СуммаУчета,
	|	КурсыВалютРасчетов.Курс КАК Курс,
	|	КурсыВалютРасчетов.Кратность КАК Кратность
	|ИЗ
	|	СуммыРегистров КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Период, ) КАК КурсыВалютРасчетов
	|		ПО Т.Договор.ВалютаРасчетов = КурсыВалютРасчетов.Валюта
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Организация,
	|	Т.Договор,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(Т.СуммаВалОстаток) <> 0 И
	|	СУММА(Т.СуммаОстаток) <> 0";
	
	
	РезультатЗапроса = Запрос.Выполнить();	
	//МассивРезультатовЗапроса = Запрос.ВыполнитьПакет();
	//СписокАвансовДолгов.Загрузить(МассивРезультатовЗапроса[1].Выгрузить());
	СписокАвансовДолгов.Загрузить(РезультатЗапроса.Выгрузить());
