...
""
"Запоминание и запись пароля невозможны.';ka='სერთიფიკატზე დაყენებულია დახურული გასაღების გაძლიერებული დაცვა."
"ასეთ შემთხვევაში, ელექტრონული ხელმოწერის და დაშიფრვის პროგრამა პაროლს ითხოვს, ხოლო პროგრამა 1С:საწარმომ უნდა გადასცეს ცარიელი პაროლი იმისთვის, რომ შეცდომას არ ჰქონდეს ადგილი."
""
"პაროლის დამახსოვრება და ჩაწერა შეუძლებელია.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
"Невозможно проверить пароль перед записью.';ka='სერთიფიკატზე არ არის მითითებული პროგრამა დახურული გასაღებისთვის."
"ჩაწერის წინ პაროლის შემოწმება შეუძლებელია.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(СохраненныйПароль) Тогда
				...
				ВызватьИсключение НСтр("ka='განმეორებით შეიყვანეთ პაროლი.';ru='Введите пароль повторно.';en='Enter the password again.'");
			КонецЕсли;
		КонецЕсли;
...
"сертификата ""Пользователь"" и ""Добавил"" или администратор.';ka='პაროლის ჩასაწერად არასაკმარისი უფლებებია."
""
"პაროლის ჩაწერა შეუძლია გააკეთოს მომხმარებელს, რომელიც მითითებულია სერთიფიკატის ველ ""მომხმარებელი""-ში და ""დაამატა""-ში, ან ადმინისტრატორს.'");
...
Процедура УстановитьЗаголовокНадписиИПометкуКомандыДоступенВсем()
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ka='Пароль доступен пользователю:';ru='Пароль доступен пользователю:';en='The password is available to the user:'")
			+ " " + Пользователь;
	Иначе
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ka='Пароль доступен всем пользователям.';ru='Пароль доступен всем пользователям.';en='The password is available to all users.'")
	КонецЕсли;
