...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПлановОбмена.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='მონაცემთა სინქრონიზაციის გამართვის შესაძლებლობა არ არის გათვალისწინებული.';ru='Возможность настройки синхронизации данных не предусмотрена.';en='Ability to customize the synchronization data is not provided.'");
...
Процедура СписокСостоянияУзловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='შევასრულოთ მონაცემთა სინქრონიზაცია?';ru='Выполнить синхронизацию данных?';en='Perform data synchronization?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОбновитьКоличествоСинхронизаций(КоличествоНастроенныхОбменов)
	...
	Если КоличествоСинхронизаций = 0 Тогда
		...
		Заголовок = НСтр("ka='მონაცემთა სინქრონიზაცია';ru='Синхронизация данных';en='data synchronization'");
	...
	Иначе
		...
		Заголовок = НСтр("ka='მონაცემთა გამართული სინქრონიზაციების სია';ru='Список настроенных синхронизаций данных';en='List of configured data synchronization'");
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	Если НастроеннаяСинхронизация.УзелИнформационнойБазы = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ka='მონაცემთა სინქრონიზაციის მონიტორის მუშაობა განუცალკევებელ სეანსში მხარდაჭერილი არ არის';ru='Работа монитора синхронизации данных в неразделенном сеансе не поддерживается';en='Job monitor data synchronization session is not supported undivided'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки <> НастроеннаяСинхронизация.ДатаПоследнейВыгрузки
		...
		ПодсказкаВыгрузки = НСтр("ka='მონაცემები გაიგზავნა: %ДатаОтправки%"
"უკანასკნელი მცდელობა: %ДатаПопытки%';ru='Данные отправлены: %ДатаОтправки%"
"Последняя попытка: %ДатаПопытки%'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки <> НастроеннаяСинхронизация.ДатаПоследнейЗагрузки
		...
		ПодсказкаЗагрузки = НСтр("ka='მონაცემები მიღებულია: %ДатаПолучения%"
"უკანასკნელი მცდელობა: %ДатаПопытки%';ru='Данные получены: %ДатаПолучения%"
"Последняя попытка: %ДатаПопытки%'");
	...
	Иначе
		...
		РасписаниеСинхронизацииДанных = НСтр("ka='სინქრონიზაციის განრიგი არ არის გამართული';ru='Расписание синхронизации не настроено';en='Schedule synchronization is not set'");;
			...
			Если СписокВозможныхНастроекОбмена.Количество() > 0 Тогда
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					...
					Для Каждого ПредопределеннаяНастройка Из СписокВозможныхНастроекОбмена Цикл
						...
						СоздатьКомандуИЭлементФормы(НаименованиеКоманды, ЗаголовокКоманды + " " + НСтр("ka=' (სერვისში)';ru='(в сервисе)';en='(When)'"), СуммирующаяГруппа);
			...
			Иначе
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					СоздатьКомандуИЭлементФормы(НаименованиеКоманды + "КорреспондентВМоделиСервиса", 
					                            ЗаголовокКоманды + " " + НСтр("ka=' (სერვისში)';ru='(в сервисе)';en='(When)'"),
					                            РодительскаяГруппа);
				КонецЕсли;
...
Процедура ПроверитьСовместимостьПравилКонвертации(Знач ИмяПланаОбмена, ОбработкаПродолжения)
	...
	Иначе
		...
		Кнопки.Добавить("ПерейтиКЗагрузкеПравил", НСтр("ka='წესების ჩატვირთვა';ru='Загрузить правила';en='Upload rules'"));
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ka='მონაცემთა სინქრონიზაცია ვერ შესრულდება';ru='Синхронизация данных не может быть выполнена';en='Data synchronization can not be performed'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ka='მონაცემთა სინქრონიზაცია შეიძლება არაკორექტულად შესრულდეს';ru='Синхронизация данных может быть выполнена некорректно';en='Data synchronization can be performed correctly'");
		КонецЕсли;
...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки)
	...
	Если ИмяКонфигурацииИзПравил <> ИмяКонфигурацииИнформационнойБазы Тогда
			...
			ОписаниеОшибки.Вставить("ТекстОшибки", НСтр("ka='მონაცემთა სინქრონიზაცია ვერ შესრულდება, ვინაიდან გამოიყენება პროგრამა ""%1""-ისთვის განკუთვნილი წესები. საჭიროა წესების გამოყენება კონფიგურაციიდან ან ფაილიდან წესების კორექტული კომპლექტის ჩატვირთვა.';ru='Синхронизация данных не может быть выполнена, так как используются правила, предназначенные для программы ""%1"". Следует использовать правила из конфигурации или загрузить корректный комплект правил из файла.';en='Data synchronization can not be performed because the rules intended for the program ""%1"". It is necessary to use the rules of the configuration, or download the correct set of rules from a file.'"));
		...
		Если РезультатСравнения <> 0 Тогда
			...
			Если РезультатСравнения < 0 Тогда
				...
				ТекстОшибки = НСтр("ka='მონაცემთა სინქრონიზაცია შეიძლება არაკორექტულად შესრულდეს, ვინაიდან პროგრამა ""%1""-ის (%2) წინა ვერსიისთვის განკუთვნილი წესები გამოიყენება. რეკომენდებულია წესების გამოყენება კონფიგურაციიდან ან პროგრამის მიმდინარე ვერსიისთვის განკუთვნილი წესების კომპლექტის ჩატვირთვა (%3).';ru='Синхронизация данных может быть выполнена некорректно, так как используются правила, предназначенные для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).';en='Data synchronization can be performed correctly, as the rules are used, designed for a previous version of ""%1"" program (2%). It is recommended to use the rules of the configuration, or download a set of rules designed for the current version of the program (3%).'");
				ВидОшибки = "УстаревшаяВерсияКонфигурации";
				
			Иначе
				
				ТекстОшибки = НСтр("ka='მონაცემთა სინქრონიზაცია შეიძLება არაკორექტულად შესრულდეს, ვინაიდან გამოიყენება პროგრამის უფრო ახალი ვერსიისთვის განკუთვნილი წესები ""%1"" (%2). რეკომენდებულია პროგრამის ვერსიის განახლება ან პროგრამის მიმდინარე ვერსიისთვის განკუთვნილი წესების კომპლექტის გამოყენება (%3).';ru='Синхронизация данных может быть выполнена некорректно, так как используются правила, предназначенные для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).';en='Data synchronization can be performed correctly, since the used rules designed for a newer version ""%1"" program (2%). It is recommended to update the version of the program or use a set of rules designed for the current version of the program (3%).'");
