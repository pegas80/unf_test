...
Процедура ВывестиРодительскиеДокументы(ДокументСсылка)
	...
	Если Выборка.Следующий() Тогда
		...
		//+14.08.2024 ONESOFT-3391
		Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка.СчетФактураОтправленныйГрузия") Тогда 
			СтрокаДерева.СуммаДокумента = Выборка.СуммаДокумента + Выборка.Ссылка.СуммаНДС;
		Иначе 
...
Процедура ВывестиПодчиненныеДокументы(СтрокаДерева)
	...
	Пока Выборка.Следующий() Цикл
		Если мУжеВСписке[Выборка.Ссылка] = Неопределено Тогда
			...
			//+14.08.2024 ONESOFT-339
			Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка.СчетФактураОтправленныйГрузия") Тогда 
				НоваяСтрока.СуммаДокумента = Выборка.СуммаДокумента + Выборка.Ссылка.СуммаНДС;  
			Иначе 
...
Процедура ОткрытьФормуДокумента()
	...
	Попытка
		...
		Если Элементы.ДеревоДокументов.ТекущиеДанные.Ссылка = ДокументСсылка И Форма.Открыта() Тогда
			...
			Сообщение.Текст = НСтр("ka='დოკუმენტი უკვე გახსნილია!';ru='Документ уже открыт!';en='Document is already open!'");
			Сообщение.Сообщить();
		КонецЕсли;
...
Функция ПровестиСервер(ДокументСсылка)
	...
	Исключение
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='დოკუმენტის დაბლოკვა შეუძლებელია.';ru='Невозможно заблокировать документ.';en='Unable to lock the document.'");
		Сообщение.Сообщить();
	КонецПопытки; 
...
Процедура Обновить(Команда)
	...
	Если ОсновнойДокументЕщеДоступен() Тогда
		ВывестиДеревоДокументов(); 
	Иначе
		ЗакрытьФормуСПредупреждением(НСтр("ka='დოკუმენტი, რომლისთვისაც ფორმირებულია ანგარიშგება დაქვემდებარებულობის სტრუქტურის შესახებ წაიშალა, ან მიუწვდომელი გახდა.';ru='Документ, для которого сформирован отчет о структуре"
...
Процедура Провести(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='დოკუმენტის %Документ% გატარება ვერ მოხერხდა!';ru='Не удалось провести документ %Документ%!';en='Unable to hold the document% Document%!'");;
...
Процедура ОтменаПроведения(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='ვერ მოხერხდა დოკუმენტის %Документ% გაუტარებლად გაკეთება!';ru='Не удалось сделать непроведенным документ %Документ%!';en='Failed to make failure to document% Document%!'");;
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Документ%", Элементы.ДеревоДокументов.ТекущиеДанные.ДокументПредставление);
		Сообщение.Сообщить();
...
Процедура УстановитьПометкуУдаления(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='დოკუმენტზე %Документ% ვერ მოხერხდა წასაშლელი ნიშნულის დაყენება!';ru='Не удалось установить пометку удаления на документ %Документ%!';en='Failed to set a mark delete a document% Document%!'");;
