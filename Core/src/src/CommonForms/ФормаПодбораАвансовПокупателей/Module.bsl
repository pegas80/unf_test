...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗаказВШапке И Контрагент.ВестиРасчетыПоЗаказам Тогда // заказ в шапке
		Заказ = ?(Параметры.Заказ = Неопределено,Документы.ЗаказПокупателя.ПустаяСсылка(),Параметры.Заказ);
...
Процедура ЗаполнитьПредоплату()
	...
	// Kuzmich, 04.09.2023, (ONESOFT-2518) Расходная накладная - Зачет авансы - По остатком
	ТЗ_Расчётов = РегистрыНакопления.РасчетыСПокупателями.Остатки(ТекущаяДата(),Новый Структура("Документ", СсылкаРеквизитФормы),"Документ","СуммаВал");
	...
	// Kuzmich, 30.09.2023, (ONESOFT-2518) Расходная накладная - Зачет авансы - По остатком, доделки
	|	ГДЕ
	|	 РасчетыСПокупателямиОстатки.ДокументДата < &Период
	// Kuzmich, 30.09.2023, (ONESOFT-2518) Расходная накладная - Зачет авансы - По остатком, доделки
	|
	|СГРУППИРОВАТЬ ПО
	...
	//|		КОНЕЦ)
	//|	КОНЕЦ) КАК Курс,
	// Kuzmich, 19.09.2023, (ONESOFT-2518) Расходная накладная - Зачет авансы - По остатком
	...
	// Kuzmich, 10.12.2024, (ONESOFT-3559) Lexus - Проблема с авансами, закомментировал
	//Предоплата.Очистить();
	...
	Пока ВыборкаРезультатаЗапроса.Следующий() Цикл
		...
		// Kuzmich, 10.12.2024, (ONESOFT-3559) Lexus - Проблема с авансами
		МассивНайденныхПредоплат = Предоплата.НайтиСтроки(Новый Структура("Документ", ВыборкаРезультатаЗапроса.Документ));
		Если МассивНайденныхПредоплат.Количество()>0 Тогда
			
			Предоплата.Удалить(МассивНайденныхПредоплат[0]);
		...
		КонецЕсли;// Kuzmich, 04.09.2023, (ONESOFT-2518) Расходная накладная - Зачет авансы - По остатком
...
Процедура ЗаполнитьАвансы()
	...
	| ВЫБОР
	|   КОГДА ОтобранныеАвансы.Документ ССЫЛКА Документ.Взаимозачет
	|     ТОГДА ВЫРАЗИТЬ(ОтобранныеАвансы.Документ КАК Документ.Взаимозачет)	
	|   КОГДА ОтобранныеАвансы.Документ ССЫЛКА Документ.ОперацияПоПлатежнымКартам
	|     ТОГДА ВЫРАЗИТЬ(ОтобранныеАвансы.Документ КАК Документ.ОперацияПоПлатежнымКартам)
	|   КОГДА ОтобранныеАвансы.Документ ССЫЛКА Документ.ПоступлениеВКассу
	|     ТОГДА ВЫРАЗИТЬ(ОтобранныеАвансы.Документ КАК Документ.ПоступлениеВКассу)	
	|   КОГДА ОтобранныеАвансы.Документ ССЫЛКА Документ.ПоступлениеНаСчет
	|     ТОГДА ВЫРАЗИТЬ(ОтобранныеАвансы.Документ КАК Документ.ПоступлениеНаСчет)
	|   КОГДА ОтобранныеАвансы.Документ ССЫЛКА Документ.ПриемИПередачаВРемонт
	|     ТОГДА ВЫРАЗИТЬ(ОтобранныеАвансы.Документ КАК Документ.ПриемИПередачаВРемонт)
  	| КОНЕЦ КАК Документ,	
	|	ВЫБОР
	|		КОГДА НЕ &ЕстьЗаказ
	|			ТОГДА &Заказ
	...
	|	ОтобранныеАвансы.СуммаРасчетов *ОтобранныеАвансы.Курс/ ОтобранныеАвансы.Кратность КАК СуммаУчета
	...
	//|		0,                       
	//|		ОтобранныеАвансы.СуммаРасчетов *ОтобранныеАвансы.СуммаРасчетов *ОтобранныеАвансы.Курс/ ОтобранныеАвансы.Кратность,    
	|		ОтобранныеАвансы.СуммаУчета,
	...
	ИначеЕсли ЗаказВШапке
	   ИЛИ НЕ ЕстьЗаказ Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "// ТекстЗаказВШапке", "И Заказ = &Заказ");
		Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьВЗаказНарядеУпрошенныйЗачеАавансов") 
			И ТипЗнч(СсылкаРеквизитФормы) = Тип("ДокументСсылка.ЗаказПокупателя")
			И СсылкаРеквизитФормы.ХозяйственнаяОперация = Справочники.ХозяйственныеОперации.ЗаказНаряд Тогда 
	...
	Для Каждого ТекущаяСтрока Из РезультатТЗ Цикл
		Если ТекущаяСтрока.ВалютаРасчетов <> Константы.НациональнаяВалюта.Получить() Тогда
			...
			ТекущаяСтрока.СуммаУчета = ТекущаяСтрока.СуммаУчета;
			ТекущаяСтрока.Курс = ТекущаяСтрока.СуммаУчета * КурсКратность.Кратность / ТекущаяСтрока.СуммаРасчетов; 
			//ТекущаяСтрока.Курс = ТекущаяСтрока.Курс;//lazare ONESOFT-3089
			// Kuzmich, 12.12.2023, (ONESOFT-2220) Аванс начальных остатков
