...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Или ОбщегоНазначения.ЗначениеСсылочногоТипа(Параметры.ПараметрКоманды),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Недопустимое значение параметра ПараметрКоманды при вызове метода УправлениеПечатьюКлиент.ВыполнитьКомандуПечати.""Ожидалось: Массив, ЛюбаяСсылка.""Передано: %1'; ka = 'პარამეტრ ბრძანებისპარამეტრი-ს დაუშვებელი მნიშვნელობა მეთოდ ამობეჭდვისმართვაკლიენტი.ამობეჭდვისბრძანებისშესრულება-ს მოხმობისას.""მოსალოდნელი იყო: მასივი, ნებისმიერიბმული.""გადაცემულია: %1'; en = 'Invalid value of the parameter Command Parameter when calling the ManagePrintClient.ExecutePrintCommand method.""Expected: Array, AnyReference.""Transferred: %1'"), ТипЗнч(Параметры.ПараметрКоманды)));
	КонецЕсли;
	...
	Если ЭтоПечатьКомплекта() Тогда
		Элементы.Копий.Заголовок = НСтр("ka='კომპლექტის ასლები';ru='Копий комплекта';en='copies of the kit'");
	КонецЕсли;
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ОбщаяФорма",
		Неопределено,
		НСтр("ru = 'Новости: Печать'; ka = 'სიახლე: ბეჭდვა'; en = 'News: Print'"),
		Ложь,
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				Для Каждого ЗаписанныйОбъект Из ЗаписанныеОбъекты Цикл
					...
					ОбсужденияВызовСервера.ДобавитьСообщение(НСтр("ru = 'Присоединен файл '; ka = 'ფაილი მიმაგრებულია'; en = 'File attached'")+ТекстИсточник, ВыбранноеЗначение.ОбъектДляПрикрепления, Истина);
					//УНФ Конец
					
				КонецЦикла;
				Состояние(НСтр("ka='შენხვა წარმატებით დასრულდა.';ru='Сохранение успешно завершено.';en='Saving completed successfully.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ПерейтиКДокументу(Команда)
	...
	СписокВыбора.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ka='ბეჭდურ ფორმაზე გადასვლა';ru='Перейти к печатной форме';en='Go to the printed form'"));
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если НастройкаПечатнойФормы.Количество <> 1 Тогда
			НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление 
				+ " (" + НастройкаПечатнойФормы.Количество + " " + НСтр("ru = 'экз.'; ka = 'ასლი'; en = 'copy'") + ")";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ЕстьРазрешенныйВывод Тогда
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Запретить Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ru = 'вывод не доступен'; ka = 'გამოტანა მიუწვდომელია'; en = 'output not available'") + ")";
			ИначеЕсли ПолеТабличногоДокумента.Защита Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ka='მხოლოდ ბეჭდვა';ru='только печать';en='only print'") + ")";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать(ИмяПринтера = Неопределено)
	...
	Если ПустаяСтрока(ИмяПринтера) Тогда
		ИмяПринтера = НСтр("ru = 'Принтер по умолчанию'; ka = 'ნაგულისხმევი პრინტერი'; en = 'Default Printer'");
	КонецЕсли;
	
	ПодсказкаКнопкиПечать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Напечатать на принтере (%1)'; ka = 'ბეჭდვა პრინტერზე (%1)'; en = 'Printed on the printer (%1)'"),  ИмяПринтера);
...
Процедура УстановитьЗаголовокФормы()
	...
	Иначе
		Если ЭтоПечатьКомплекта() Тогда
			Заголовок = НСтр("ka='კომპლექტის ბეჭდვა';ru='Печать комплекта';en='Print kit'");
		ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) <> Тип("Массив") Или Параметры.ПараметрКоманды.Количество() > 1 Тогда
			Заголовок = НСтр("ka='დოკუმენტის ბეჭდვა';ru='Печать документов';en='Printing documents'");
		Иначе
			Заголовок = НСтр("ka='დოკუმენტის ბეჭდვა';ru='Печать документа';en='Printing a document'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВосстановитьНастройкиПечатныхФорм(СохраненныеНастройкиПечатныхФорм = Неопределено)
	...
	Для Каждого СохраненнаяНастройка Из СохраненныеНастройкиПечатныхФорм Цикл
		...
		Для Каждого НастройкаПечатнойФормы Из НайденныеНастройки Цикл
			...
			НастройкаПечатнойФормы.Количество = ?(СохраненнаяНастройка.Количество<>0,СохраненнаяНастройка.Количество,1);
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		Если НазваниеПечатнойФормы = "Invoice Geo" Или НазваниеПечатнойФормы = "Invoice Eng" Или НазваниеПечатнойФормы = "Invoice Rus" Или НазваниеПечатнойФормы = "Расходная накладная" Тогда
			ПараметрыДляВставки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектПечати, "Дата,Номер,Контрагент");			
		ИначеЕсли НазваниеПечатнойФормы = "სასაქონლო ზედნადების ცხრილი" Тогда 
			ПараметрыДляВставки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектПечати, "Дата,Номер,ГрузоПолучатель");
		Иначе 
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрефиксацияОбъектов") Тогда
			...
			Если НазваниеПечатнойФормы = "Invoice Geo" Или НазваниеПечатнойФормы = "Invoice Eng" Или НазваниеПечатнойФормы = "Invoice Rus" Или НазваниеПечатнойФормы = "Расходная накладная" Тогда
				ПараметрыДляВставки.Контрагент = МодульПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ПараметрыДляВставки.Контрагент);
				ПараметрыДляВставки.Номер =  Строка(Прав(ПараметрыДляВставки.Номер, Найти(ПараметрыДляВставки.Номер, "-")+1));			
			ИначеЕсли НазваниеПечатнойФормы = "სასაქონლო ზედნადების ცხრილი" Тогда
				ПараметрыДляВставки.ГрузоПолучатель = МодульПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ПараметрыДляВставки.ГрузоПолучатель);
			...
			КонецЕсли;		
		КонецЕсли;
		ПараметрыДляВставки.Дата = Формат(ПараметрыДляВставки.Дата, "ДФ=dd.MM.yyyy");
		ПараметрыДляВставки.Вставить("НазваниеПечатнойФормы", НазваниеПечатнойФормы);
		Если НазваниеПечатнойФормы = "Invoice Geo" Или НазваниеПечатнойФормы = "Invoice Eng" Или НазваниеПечатнойФормы = "Invoice Rus" Или НазваниеПечатнойФормы = "Расходная накладная" Тогда 
			Шаблон = НСтр("ka='[Номер] - [Контрагент]   [Дата]';ru='[Номер] - [Контрагент] от [Дата]';en='[Номер] - [Контрагент] of [date]'");
		ИначеЕсли НазваниеПечатнойФормы = "სასაქონლო ზედნადების ცხრილი" Тогда
			Шаблон = НСтр("ka='[Номер] - [ГрузоПолучатель]   [Дата]';ru='[Номер] - [ГрузоПолучатель] от [Дата]';en='[Номер] - [ГрузоПолучатель] of [date]'");
		Иначе 	
			Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] № [Номер] от [Дата]'; ka = '[НазваниеПечатнойФормы] № [Номер] от [Дата]'; en = '[НазваниеПечатнойФормы] № [Номер] от [Дата]'");
		КонецЕсли
	Иначе
		...
		Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]'; ka = '[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]'; en = '[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]'");
	КонецЕсли;
...
Функция ПолучитьИмяФайлаДляАрхива()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Иначе
			Результат = НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	Состояние(НСтр("ka='შენახვა წარმატებით დასრულდა';ru='Сохранение успешно завершено';en='Saving completed successfully'"), , НСтр("ka='საქაღალდეში:';ru='в папку:';en='in folder:'") + " " + Папка);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Для Каждого Файл Из ФайлыВоВременномХранилище Цикл
			...
			Результат.Добавить(МодульПрисоединенныеФайлы.ДобавитьПрисоединенныйФайл(
				ПараметрыФайла, Файл.АдресВоВременномХранилище, , НСтр("ka='საბეჭდი ფორმა';ru='Печатная форма';en='printed form'")));
		КонецЦикла;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОткрытьМакетДляРедактирования()
	...
	ОтобразитьСостояниеТекущейПечатнойФормы(НСтр("ka='მაკეტი რედაქტირდება';ru='Макет редактируется';en='the layout is edited'"));
...
Процедура ПереформироватьПечатнуюФорму(ИмяМакета, ИмяРеквизита)
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ka='ბეჭდური ფორმა ხელახლა არ ჩამოყალიბებულა.';ru='Печатная форма не была переформирована.';en='The printed form has not been reformed.'");
	КонецЕсли;
