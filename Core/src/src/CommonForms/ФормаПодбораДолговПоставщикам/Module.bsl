...
Процедура ЗаписатьПодборВХранилище() 
	...
	Если ТаблицаОтобранныеДолги.Колонки.Найти("СчетНаОплату") = Неопределено Тогда
		ТаблицаОтобранныеДолги.Колонки.Добавить("СчетНаОплату");
	КонецЕсли;	
	Для каждого СтрокаРасшифровки из ТаблицаОтобранныеДолги Цикл
		Если ТипЗнч(СтрокаРасшифровки.Документ) = Тип("ДокументСсылка.ПриходнаяНакладная") 
			и ЗначениеЗаполнено(СтрокаРасшифровки.Документ.ДокументОснование) 
			и ТипЗнч(СтрокаРасшифровки.Документ.ДокументОснование) =	Тип("ДокументСсылка.СчетНаОплатуПоставщика")
			ТОгда 
			СтрокаРасшифровки.СчетНаОплату = СтрокаРасшифровки.Документ.ДокументОснование;
		КонецЕсли;	
	КонецЦикла;	
	//Малышев 2025-07-18 ONESOFT-4227 --)
