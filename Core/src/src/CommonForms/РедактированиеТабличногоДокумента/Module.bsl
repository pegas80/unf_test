...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ცვლილებები შევინახოთ %1-ში?';ru='Сохранить изменения в %1?';en='Save changes to% 1?'"), ИмяДокумента);
...
Процедура НачатьДиалогСохраненияФайла(Знач ОбработчикЗавершения)
	...
	ДиалогСохраненияФайла.Фильтр = НСтр("ka='ცხრილი-დოკუმენტი';ru='Табличный документ';en='spreadsheet document'") + " (*.mxl)|*.mxl";
...
Функция ИмяНовогоДокумента()
	Возврат НСтр("ka='Новый';ru='Новый';en='Новый'");
КонецФункции
...
Процедура УстановитьЗаголовок()
	...
	Если ЭтоНовый() Тогда
		Заголовок = Заголовок + " (" + НСтр("ka='ქმნილება';ru='создание';en='creature'") + ")";
	ИначеЕсли РедактированиеЗапрещено Тогда
		Заголовок = Заголовок + " (" + НСтр("ka='მხოლოდ გადათვალიერება';ru='только просмотр';en='view only'") + ")";
	КонецЕсли;
