...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.ИмяФормыОбъектаПечати) Тогда
		ВызватьИсключение НСтр("ka='ამ ფორმის უშუალო გახსნა არ არის გათვალისწინებული.';ru='Непосредственное открытие этой формы не предусмотрено.';en='The immediate opening of this form is not provided.'");
	КонецЕсли;
...
Процедура СформироватьТабличныеДокументыНаСервере(ВыбранныеКоманды)
	...
	Для Каждого ВыбраннаяКоманда Из ВыбранныеКоманды Цикл
		...
		Если КомандаПечати.ДополнительныеПараметры.Свойство("ВариантЗапуска") Тогда
			//+++ Дмгг
			//Проблема с внешними печатными формами
			//Результат = Новый Структура("КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода");
			Результат = Новый Структура("КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода", Неопределено, Новый СписокЗначений, Неопределено);
			//--- Дмгг
		...
		Если Результат.КоллекцияПечатныхФорм.Количество() > 0 Тогда
			...
			КонецЕсли;
			ИнформацияОтправки.Тема = ?(ЗначениеЗаполнено(ИнформацияОтправки.Тема), НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'"), Результат.ПараметрыВывода.ПараметрыОтправки.Тема);
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище()
	...
	Если ЗаписьZipФайла <> Неопределено Тогда 
		...
		ПутьВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, ЭтотОбъект.УникальныйИдентификатор);
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("Представление", НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'") + ".zip");
		ОписаниеФайла.Вставить("АдресВоВременномХранилище", ПутьВоВременномХранилище);
		Результат.Добавить(ОписаниеФайла);
	КонецЕсли;
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		//Lazare 13.02.2024
		//Шаблон = НСтр("ka='[НазваниеПечатнойФормы] № [Номер] от [Дата]';ru='[НазваниеПечатнойФормы] № [Номер] от [Дата]';en='[NazvaniePechatnoyFormy] number [number] of [date]'"); 
		Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] № [Номер] от [Дата]'; ka = '[Номер] -  [Контрагент] - [Дата] '; en = '[NazvaniePechatnoyFormy] number [number] of [date]'");
		Если ПользователиИнформационнойБазы.ТекущийПользователь() <> Неопределено Тогда 
			Если ПользователиИнформационнойБазы.ТекущийПользователь().Язык.КодЯзыка = Метаданные.Языки.Грузинский.КодЯзыка Тогда 
			ПараметрыДляВставки.Номер = Прав(ОбъектПечати.Номер, 6);	
			КонецЕсли;
		КонецЕсли;  
		//Lazare 13.02.2024
...
Функция ПолучитьПредставлениеФорматаВложений(ВыбранныеФорматыСохранения, УпаковатьВАрхив)
	...
	Если ПустаяСтрока(ФорматВложений) Тогда
		ФорматВложений = НСтр("ka='<დანართების ფორმატი არ არის არჩეული>';ru='<Не выбран формат вложений>';en='<Not selected investments Format>'");
	КонецЕсли;
	
	Если УпаковатьВАрхив Тогда
		ФорматВложений = НСтр("ka='არქივი .zip:';ru='Архив .zip:';en='.Zip archive:'") + " " + ФорматВложений;
	КонецЕсли;
