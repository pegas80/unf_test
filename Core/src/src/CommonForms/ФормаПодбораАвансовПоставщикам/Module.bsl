...
Функция ПолучитьТекстЗапроса_ПриходнаяНкладная()
	...
	|		ЕСТЬNULL(РасчетыСПоставщикамиОстатки.СуммаОстаток, 0) КАК СуммаУчета,
	|		ЕСТЬNULL(РасчетыСПоставщикамиОстатки.СуммаВалОстаток, 0) КАК СуммаРасчетов,
	|		ЕСТЬNULL(РасчетыСПоставщикамиОстатки.СуммаОстаток, 0) * КурсыВалютыУчета.Курс * ЕСТЬNULL(КурсыВалютыДокумента.Кратность, КурсыВалютыУчета.Кратность) / (ЕСТЬNULL(КурсыВалютыДокумента.Курс, КурсыВалютыУчета.Курс) * КурсыВалютыУчета.Кратность) КАК СуммаПлатежа
	|	ИЗ
	|		ВременнаяТаблицаРасчетыСПоставщикамиОстатки КАК РасчетыСПоставщикамиОстатки
