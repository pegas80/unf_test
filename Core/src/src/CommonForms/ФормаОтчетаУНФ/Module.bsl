	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			Если ПериодНачало.АвтоОтметкаНезаполненного
				...
				ТекстОшибки = НСтр("ka='პერიოდი არ არის მითითებული';ru='Не указан период';en='Unknown time'");
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.ТипЗначения.Типы().Количество()>0 Тогда
		...
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Возр, Данные.Поле+НСтр("ka='გაზრდის მიხედვით';ru=' по возрастанию'"),, БиблиотекаКартинок.СортироватьСписокПоВозрастанию); 
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, Данные.Поле+НСтр("ka='დაკლების მიხედვით';ru=' по убыванию'"),, БиблиотекаКартинок.СортироватьСписокПоУбыванию); 
...
Функция СформироватьПодменюОформить(Поле, Значение)
	...
	Для каждого Тип Из ТипЗначения.Типы() Цикл
		...
		НСтр("ka='სწრაფი გამართვების სიის რედაქტირება';ru='Редактировать список быстрых настроек';en='Edit the list of Quick Settings'"),, 
		БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
		СписокКоманд.Добавить(
		"РучнаяНастройкаОформления", 
		НСтр("ka='გაფორმების გამართვა ხელის რეჟიმში';ru='Ручная настройка оформления';en='Manual tuning clearance'"),, 
...
Процедура ОткрытьФормуОтчета()
	...
	Если СтандартнаяОбработка Тогда
		...
		СтруктураОткрытия.Вставить("ПредставлениеВарианта", Строка(НастройкиОтчета.НаименованиеВарианта)+НСтр("ka='(გაშიფვრა)';ru=' (Расшифровка)'"));
...
Процедура Подключаемый_ИерархияФильтраНажатие(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭлементПоле.ВыборГруппИЭлементов=ГруппыИЭлементы.Элементы Тогда
		...
		ЭлементПоле.ПодсказкаВвода = Стр.Заголовок+НСтр("ka='(ჯგუფი)';ru=' (группа)'");
...
Процедура ОбновитьПредставлениеПериода()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		...
		ИначеЕсли Элементы.ПредставлениеПериода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет Тогда
			ПредставлениеПериода = НСтр("ru = 'за '; ka = ' '")+НРег(ПредставлениеПериода(Отчет.Период.ДатаНачала, КонецДня(Период.ДатаОкончания)))+НСтр("ka = 'თვის'");
		Иначе
			ПредставлениеПериода = НРег(ПредставлениеПериода(НачалоДня(Период.ДатаНачала), ?(ЗначениеЗаполнено(Период.ДатаОкончания), КонецДня(Период.ДатаОкончания), Период.ДатаОкончания), НСтр("ka='Л=ka';ru='Л=ru';en='L = ru'")));
		КонецЕсли;
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
...
Процедура Сформировать()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если ЗначениеЗаполнено(Период.ДатаНачала) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru = 'за '; ka = ' '")+ПредставлениеПериода;
		Иначе
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		...
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли;
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НастройкиОтчета.НаименованиеВарианта+НСтр("ru=' на ';ka='ზე'")+ПредставлениеДляЗаголовка);
	Иначе
		...
		Если (ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок) ИЛИ (СравнениеПериод И НЕ ИмяПоля=ИмяСравнения) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ka=' в колонках отчета';ru=' в колонках отчета'");
			...
			Иначе
				...
				ОписаниеПоля = ОписаниеПоля+НСтр("ka=' в строках отчета';ru=' в строках отчета'");
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
			...
			НСтр("ka='სხვადასხვა ველი-პერიოდის სტრიქონებში და სვეტებში გამოყენება აკრძალულია. წინასწარ გამორთეთ ველი %1.';ru='Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1';en='It is forbidden to use in the rows and columns of different fields-periods. Pre disconnect box 1%'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(Элемент)=Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			Если ПустаяСтрока(ЗаголовокГруппировки) Тогда
				ЗаголовокГруппировки = НСтр("ka='<Детальные записи>';ru='<Детальные записи>';en='<Detailed records>'");
			КонецЕсли; 
		...
		Если (ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки) ИЛИ (СерияПериод И НЕ ИмяПоля=ДиаграммаСерии) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ka='დიაგრამაში';ru=' в диаграмме'");
			...
			Иначе
				ОписаниеПоля = """"+?(ПустаяСтрока(Поле.Заголовок), Поле.Поле, Поле.Заголовок)+"""";
				ОписаниеПоля = ОписаниеПоля+НСтр("ka=' в структуре отчета';ru=' в структуре отчета'");
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					ДиаграммаТочки = Неопределено;
				КонецЕсли; 
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Структура") Тогда
			Если НЕ Стр.Значение.ЗначениеОт=Неопределено И НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='от ';ka='от '")+Строка(Стр.Значение.ЗначениеОт) + НСтр("ru=' до ';ka=' до '") + Строка(Стр.Значение.ЗначениеДо);
			ИначеЕсли НЕ Стр.Значение.ЗначениеОт=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='от ';ka='от '")+Строка(Стр.Значение.ЗначениеОт);
			ИначеЕсли НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='до ';ka='до '")+Строка(Стр.Значение.ЗначениеДо);
			КонецЕсли; 
			Префикс = ЗаголовокФильтра;
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Стр.Значение) Тогда
			ПредставлениеЗначения = ЗаголовокФильтра+НСтр("ka=': не указано';ru=': не указано';en=': not specified'");
...
Функция ПредставлениеВидаСравнения(ВидСравнения)
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВИерархии Тогда
		Возврат НСтр("ru=' в группе ';ka=' в группе '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСписке Тогда
		Возврат НСтр("ru=' в списке ';ka=' в списке '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
		Возврат НСтр("ru=' в списке групп ';ka=' в списке групп '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Заполнено Тогда
		Возврат НСтр("ru=' заполнено';ka='სევსებულია'");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеСодержит Тогда
		Возврат НСтр("ru=' не содержит ';ka=' не содержит '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Подобно Тогда
		Возврат НСтр("ru=' подобно ';ka=' подобно '");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Содержит Тогда
		Возврат НСтр("ru=' содержит ';ka=' содержит '");
	Иначе
		Возврат "";
	КонецЕсли; 	
...
Функция СформироватьСписокЭлементовСервер(Текст, МассивТипов, УправляющиеСимволы)
	...
	Пока ВыборкаТип.Следующий() Цикл
		...
		Если Выборка.Количество()>1 ИЛИ (УправляющиеСимволы="""" И Выборка.Количество()>0) Тогда
			...
			Представление = ПредставлениеТипа+НСтр("ru=' содержит ';ka=' содержит '")+Текст;
...
Функция СуммаЯчеек(ТабличныйДокумент, ВыделенныеОбласти)
	...
	Если ВыделенныеОбласти = Неопределено Тогда
		...
		#Иначе
			Возврат НСтр("ka='პარამეტრ ""გამოყოფილიარეალები""-ს მნიშვნელობა არ არის მითითებული.';ru='Не указано значение параметра ""ВыделенныеОбласти"".';en='Unknown value ""VydelennyeOblasti"".'");
		#КонецЕсли
	КонецЕсли;
...
Процедура ВыгрузкаXML(Команда)
	...
	Если ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" Тогда
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';en='All files (*% 1.) |. *% 1'"), "xml", "xml");
	ИначеЕсли ИмяОтчета = "ПростаяПенсионнаяДекларация" Тогда 
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';en='All files (*% 1.) |. *% 1'"), "xls", "xls");
	КонецЕсли;
