...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если СчитывательМагнитныхКарт Тогда
		...
		Если ПрефиксДрайвера > 0 И СуффиксДрайвера > 0 И ПрефиксДрайвера = СуффиксДрайвера Тогда
			ТекстСообщения = НСтр("ka='პირველი გამოყენებული ბილიკის პრეფიქსი უკანასკნელად გამოყენებული ბილიკის სუფიქსს ემთხვევა';ru='Префикс первой используемой дорожки совпадает с суффикс последней используемой дорожки';en='A prefix used by the first track coincides with the last used path suffix'");
		...
		ИначеЕсли НастроеноДорожек = 0 Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია წამკითხველის თუნდაც ერთი ბილიკის გამოყენების მითითება';ru='Необходимо указать использование хотя бы одной дорожки для считывателя';en='You must specify the use of at least one track for the reader'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли ДорожкаСПустымСуффиксом Тогда
			ТекстСообщения = НСтр("ka='ყოველი გამოყენებული ბილიკისთვის მითითებული უნდა იყოს სუფიქსი';ru='Для каждой используемой дорожки должен быть указан суффикс';en='For each track to be used is specified suffix'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
Процедура НачатьЗапускПриложенияЗавершение(Результат, Параметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დაწყებულია დრაივერის ძირითადი მიწოდების დაყენება.';ru='Начата установка основной поставки драйвера.';en='Begun installing the main delivery driver.'")); 
...
Процедура УстановитьДрайвер(Команда)
	...
	Если ИнтеграционныйКомпонент Тогда
		Текст = НСтр("ka='მწარმოებლის საიტზე გადასასვლელად საჭიროა ინტერნეტთან შეერთება. გავაგრძელოთ ოპერაციის შესრულება?';ru='Для перехода на сайт производителя необходимо подключение к Интернету."
...
&НаКлиенте
Процедура ТестУстройстваЗавершение(РезультатВыполнения, Параметры = Неопределено) Экспорт
	...
	ТекстСообщения = ?(РезультатВыполнения.Результат,  НСтр("ka='Тест успешно выполнен. %ДополнительноеОписание%';
																|ru='Тест успешно выполнен. %ДополнительноеОписание%';
																|en='The test was completed successfully. % ДополнительноеОписание%'"),
	                               НСтр("ka='Тест не пройден. %ДополнительноеОписание%';
								   			|ru='Тест не пройден. %ДополнительноеОписание%';
											|en='Fail. % DopolnitelnoeOpisanie%'"));
	...
	Если Параметры<>Неопределено 
		и Параметры.Свойство("P_OpenCashDrawerAfterDocumentCashOnly") и Параметры.P_OpenCashDrawerAfterDocumentCashOnly 
		и Параметры.Свойство("ОткрытьЯщик") = Ложь
		ТОгда    
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Тестируем открытие ящика';
																|ka='უჯრის გახსნის ტესტირება';
																|en='Testing the drawer opening'"));
	
		Параметры.Вставить("ОткрытьЯщик", Истина);
    	Параметры.Вставить("ИДУстройства", Идентификатор);
    	Оповещение = Новый ОписаниеОповещения("ТестУстройстваЗавершение", ЭтотОбъект);
		ВходныеПараметры  = Неопределено;
		ВыходныеПараметры = Неопределено;
		МенеджерОборудованияКлиент.НачатьВыполнениеДополнительнойКоманды(Оповещение, "OpenCashDrawer", ВходныеПараметры, Идентификатор, Параметры);
	КонецЕсли;	
...
Процедура ТестУстройства(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ТестУстройстваЗавершение", ЭтотОбъект, ПараметрыУстройства);
...
Процедура ДополнительноеДействиеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ТекстСообщения = ?(РезультатВыполнения.Результат,  НСтр("ka='Операция выполнена успешно.';ru='Операция выполнена успешно.';en='Operation completed successfully.'"),
	                               НСтр("ka='Ошибка выполнения операции.';ru='Ошибка выполнения операции.';en='Operation failed.'") + Символы.НПП + РезультатВыполнения.ВыходныеПараметры[1]);
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбновитьНастраиваемыйИнтерфейс(ОписаниеИнтерфейса, ДополнительныеДействия, ПервыйЗапуск)
	...
	Если ЧтениеXML.Имя = "Settings" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
		Пока ЧтениеXML.Прочитать() Цикл  
			...
			Если ЧтениеXML.Имя = "Parameter" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
				...
				ИначеЕсли Элементы.Найти(ПараметрИмя) = Неопределено Тогда
					...
					Если БазоваяГруппа = Неопределено Тогда
						...
						БазоваяГруппа.Заголовок = НСтр("ka='პარამეტრები';ru='Параметры';en='Options'");
			...
			Если ЧтениеXML.Имя = "Page" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ЗаголовокСтраницы = ?(ПустаяСтрока(ЗаголовокСтраницы), НСтр("ka='პარამეტრები';ru='Параметры';en='Options'"), ЗаголовокСтраницы);
			...
			Если ЧтениеXML.Имя = "Group" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
				...
				ЗаголовокГруппы = ?(ПустаяСтрока(ЗаголовокГруппы), НСтр("ka='პარამეტრები';ru='Параметры';en='Options'"), ЗаголовокГруппы);
	...
	//Малышев 2025-09-25 ONESOFT-4381
	Если Элементы.Найти("P_OpenCashDrawerAfterDocument")<>Неопределено ТОгда  
		ПараметрИмя = "P_OpenCashDrawerAfterDocumentCashOnly";
		ПараметрТип = "BOOLEAN";
		ПараметрЗначение = Ложь;
		ПараметрЗаголовок = НСтр("ru='Открывать денежный ящик для наличной оплаты';
									|ka='გახსენით ნაღდი ფულის უჯრა ნაღდი ფულის გადახდისთვის';
									|en='Open the cash drawer for cash payment'");
		
		ПараметрОписание = НСтр("ru='Открывать денежный ящик для наличной оплаты';
									|ka='გახსენით ნაღდი ფულის უჯრა ნაღდი ფულის გადახდისთვის';
									|en='Open the cash drawer for cash payment'");
		...
		ЭлементКнопкаТеста = Элементы.Найти("ФормаТестОткрытьДенежныйЯщик");
		Если ЭлементКнопкаТеста<>Неопределено ТОгда
			ЭлементКнопкаТеста.Видимость = Истина;
		КонецЕсли;	
...
Процедура НачатьВыполнениеДополнительнойКомандыЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ИнтеграционныйКомпонент И НЕ ОсновнойДрайверУстановлен Тогда
			ДрайверУстановлен = НСтр("ka='Установлен интеграционный компонент';ru='Установлен интеграционный компонент';en='Installed integrative component'");
			ВерсияДрайвера = НСтр("ka='გაურკვეველია';ru='Не определена';en='Not determined'");
		...
		Элементы.Драйвер.ЦветТекста = ?(ВерсияДрайвера = НСтр("ka='გაურკვეველია';ru='Не определена';en='Not determined'"), ЦветОшибки, ЦветТекста);
		Элементы.Версия.ЦветТекста  = Элементы.Драйвер.ЦветТекста ;
		Элементы.НаименованиеДрайвера.ЦветТекста = ?(НаименованиеДрайвера = НСтр("ka='გაურკვეველია';ru='Не определено';en='Undefined'"), ЦветОшибки, ЦветТекста);
		Элементы.ОписаниеДрайвера.ЦветТекста     = ?(ОписаниеДрайвера     = НСтр("ka='გაურკვეველია';ru='Не определено';en='Undefined'"), ЦветОшибки, ЦветТекста);
	...
	Иначе
		...
		ВерсияДрайвера  = НСтр("ka='გაურკვეველია';ru='Не определена';en='Not determined'");
		Если НЕ ПустаяСтрока(ДрайверСообщение) И ДрайверУстановлен = НСтр("ka='დაყენებულია';ru='Установлен';en='Set'") Тогда
	...
	Если Не Элементы.УстройствоПодключено.Видимость Тогда
		Элементы.УстановитьДрайвер.Видимость = Не (ДрайверУстановлен = НСтр("ka='დაყენებულია';ru='Установлен';en='Set'"));
		Элементы.ТестУстройства.Видимость = (НЕ ДрайверУстановлен = НСтр("ka='არ არის დაყენებული';ru='Не установлен';en='Not installed'")) 
...
#Если Не ВебКлиент Тогда  
	Если ТребуетсяПереустановка Тогда
		...
		Элементы.ДекорацияПереустановитьДрайвер.Заголовок = НСтр("ka='1С:საწარმოს გადატვირთვის შემდეგ, მოხდება დრაივერის ხელახლა დაყენება.';ru='Драйвер будет переустановлен после перезагрузки ""1С:Предприятие"".';en='The driver will be reset after a reboot of ""1C: Enterprise"".'");
	...
	ИначеЕсли Не ПустаяСтрока(ВерсияДрайвераВМакете) И Не ПустаяСтрока(ВерсияДрайвера) 
		И Не ВерсияДрайвераВМакете = ВерсияДрайвера И ДрайверУстановлен = НСтр("ka='დაყენებულია';ru='Установлен';en='Set'")
		И Не Элементы.УстройствоПодключено.Видимость Тогда
		Элементы.Переустановка.Видимость = Истина;
		ТекстСообщения = НСтр("ka='კონფიგურაციაში (%ВКонфигурации%) დრაივერის ვერსია განსხვავდება დაყენებულისგან.';ru='Версия драйвера в конфигурации (%ВКонфигурации%) отличается от установленного.';en='Driver version configurations (% VKonfiguratsii%) different from that set.'");
