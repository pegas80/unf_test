...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		ТекстСообщения = НСтр("ka='ელექტრონული დოკუმენტი არ არის არჩეული';ru='Не выбран электронный документ';en='the electronic document is not selected'");
...
"Продолжить?';ka='შეუნახავი ცვლილებები დაიკარგება. გავაგრძელოთ?'");
			
		ОбработчикОповещения = Новый ОписаниеОповещения("ПрочитатьНоменклатуруОповещение", ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ka='ნომენკლატურის სიის შევსება';ru='Заполнение списка номенклатуры';en='Filling nomenclature list'"));
...
Процедура СохранитьДанныеКаталога(СсылкаНаЭД)
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если ИмяФайла = Неопределено Тогда
			ТекстОшибки = НСтр("ka='ელექტრონული დოკუმენტის ნახვა ვერ მოხერხდა. შეამოწმეთ სამუშაო კატალოგის გამართვა';ru='Не удалось просмотреть электронный документ. Проверьте настройку рабочего каталога';en='Unable to view the electronic document. Check the setting of the working directory'");
...
" Проверьте настройку рабочего каталога';ka='ელექტრონული დოკუმენტის დამატებითი მონაცემების მიღება ვერ მოხერხდა. შეამოწმეთ სამუშაო კატალოგის გამართვა'");
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
						Возврат;
					КонецЕсли;
		...
		Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				КонецЕсли;
				Операция = НСтр("ka='შეფუთვიდან მოხსნა  ЭД';ru='Распаковка ЭД';en='ED Unpacking'");
...
Процедура ПрочитатьНоменклатуруКонтрагентаСервер()
	...
	Если НЕ ЗначениеЗаполнено(Запрос.Текст) Тогда
		ТекстСообщения = НСтр("ka='ნომენკლატურის და მომწოდებლების ნომენკლატურის შეპირისპირება არ არის განსაზღვრული. აუცილებელია პროცედურის შემდეგი შევსება კონტრაგენტებთანგაცვლახელახლაგანსაზღვრებადი.ნომენკლატურისშეპირისპირებისმოთხოვნისტექსტი.';ru='Не определено сопоставление номенклатуры и номенклатуры поставщиков."
