...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВалютаДокумента) Тогда
		...
		Сообщение.Текст = НСтр("ka='არ არის არჩეული ვალუტა შესავსებად!';ru='Не выбрана валюта для заполнения!';en='Not selected currency is required!'");
	...
	Если НЕ ЗначениеЗаполнено(Курс) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='აღმოჩენილია დოკუმენტის ვალუტის ნულოვანი კურსი!';ru='Обнаружен нулевой курс валюты документа!';en='Discovered a zero rate of exchange of the document!'");
		Сообщение.Поле = "Курс";
		Сообщение.Сообщить();
		Отказ = Истина;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Кратность) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='აღმოჩენილია დოკუმენტის ვალუტის კურსის ნულოვანი ჯერადობა!';ru='Обнаружена нулевая кратность курса валюты документа!';en='Detected zero multiplicity of exchange rates for the document!'");
...
Процедура КнОКВыполнить()
	...
	Если НЕ Отказ Тогда
		...
		//Паганини ПФ_
		СтруктураВозврата.Вставить("ПредСуммаВключаетНДС", СуммаВключаетНДСПередИзменением);
		//_Паганини ПФ
		Закрыть(СтруктураВозврата);
	КонецЕсли;
