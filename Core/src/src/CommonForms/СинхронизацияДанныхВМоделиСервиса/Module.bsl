...
Процедура ОтключитьСинхронизациюДанных(Команда)
	...
	Если ТекущиеДанные <> Неопределено
		...
		Если ТекущиеДанные.НастройкаСинхронизацииВМенеджереСервиса Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='მონაცემთა სინქრონიზაციის გამოსართავად გადადით სერვისის მენეჯერში. სერვისის მენეჯერში გამოიყენეთ ბრძანება ""მონაცემთა სინქრონიზაცია"".';ru='Для отключения синхронизации данных перейдите в менеджер сервиса."
...
Процедура ВыполнитьНастройкуСинхронизацииДанных(Знач ТекущиеДанные)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НастройкаСинхронизацииВМенеджереСервиса Тогда
			...
			ПоказатьПредупреждение(, НСтр("ka='მონაცემთა სინქრონიზაციის გასამართად გადადით სერვისის მენეჯერში. სერვისის მენეჯერში გამოიყენეთ ბრძანება ""მონაცემთა სინქრონიზაცია"".';ru='Для настройки синхронизации данных перейдите в менеджер сервиса."
"В менеджере сервиса воспользуйтесь командой ""Синхронизация данных"".'"));
		Иначе
...
Процедура ОбновитьНастройкиСинхронизацииДанных()
	...
	Для Каждого Настройка Из НастройкиСинхронизацииДанных Цикл
		...
		Если Настройка.СинхронизацияНастроена Тогда
			...
			Если СтатусСинхронизации <> Неопределено Тогда
				...
				Если СтатусСинхронизации.Статус = 1 Тогда // Требуется вмешательство администратора сервиса
					...
					Настройка.Состояние = НСтр("ka='Ошибки при синхронизации данных';ru='Ошибки при синхронизации данных';en='Errors in the synchronization'");
					
				ИначеЕсли СтатусСинхронизации.Статус = 2 Тогда // Пользователь может разрешить проблемы самостоятельно
					
					Настройка.Состояние = НСтр("ka='Проблемы при синхронизации данных';ru='Проблемы при синхронизации данных';en='Problems in the synchronization data'");
					
					КоличествоПроблем = КоличествоПроблем + СтатусСинхронизации.КоличествоПроблем;
					
				ИначеЕсли СтатусСинхронизации.Статус = 3 Тогда
					
					Настройка.Состояние = НСтр("ka='Синхронизация данных настроена';ru='Синхронизация данных настроена';en='Synchronizing data set'");
		...
		Иначе
			...
			Настройка.Описание = НСтр("ka='Синхронизация данных не настроена';ru='Синхронизация данных не настроена';en='Data synchronization is not set'");
