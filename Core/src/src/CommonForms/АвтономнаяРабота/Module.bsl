...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не АвтономнаяРаботаСлужебный.ЭтоАвтономноеРабочееМесто() Тогда
		ВызватьИсключение НСтр("ka='ეს საინფორმაციო ბაზა არ წარმოადგენს ავტონომიურ სამუშაო ადგილს.';ru='Эта информационная база не является автономным рабочим местом.';en='This information base is not a stand-alone workstation.'");
	КонецЕсли;
...
"Необходимо установить полученное обновление, после чего синхронизация будет продолжена.';ka='ინტერნეტიდან მიღებულია პროგრამის განახლება. აუცილებელია მიღებული განახლების დაყენება, რომლის შემდგომ, სინქრონიზაცია გაგრძელდება.'");
	Иначе
		Элементы.ПоясняющаяНадписьПолученоОбновление.Заголовок = НСтр("ru='Получено обновление программы из Интернета."
"Обратитесь к администратору информационной базы для установки полученного обновления.';ka='ინტერნეტიდან მიღებულია პროგრამის განახლება. მიღებული განახლების დასაყენებლად მიმართეთ საინფორმაციო ბაზის ადმინისტრატორს.'");
	КонецЕсли;
...
Процедура ПриИзмененииРасписанияСинхронизацииДанных()
	...
	Элементы.ВариантРасписанияСинхронизацииДанных.СписокВыбора.Добавить(1, НСтр("ka='ყოველ 15 წუთში';ru='Каждые 15 минут';en='Every 15 minutes'"));
	Элементы.ВариантРасписанияСинхронизацииДанных.СписокВыбора.Добавить(2, НСтр("ka='ყოველ საათს';ru='Каждый час';en='Every hour'"));
	Элементы.ВариантРасписанияСинхронизацииДанных.СписокВыбора.Добавить(3, НСтр("ka='ყოველდღე 10:00-ზე, შაბათ-კვირის გარდა.';ru='Каждый день в 10:00, кроме сб. и вс.';en='Every day at 10:00, except Sat. and Sun.'"));
