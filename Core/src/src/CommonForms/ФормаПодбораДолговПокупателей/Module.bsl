...
Процедура ЗаполнитьДолги()
	...
	//|		РасчетыСПокупателямиОстатки.Заказ КАК Заказ,
	//|		ЕСТЬNULL(РасчетыСПокупателямиОстатки.СуммаВалОстаток, 0) КАК СуммаВалОстаток
	// Kuzmich, 03.09.2025, (ONESOFT-4324) «Остаток по расчетам покупателей при оплате»
	|		ВЫБОР
	|			КОГДА РасчетыСПокупателямиОстатки.Заказ ЕСТЬ NULL
	|				ТОГДА ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|			КОГДА РасчетыСПокупателямиОстатки.Заказ = НЕОПРЕДЕЛЕНО
	|				ТОГДА ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|			КОГДА РасчетыСПокупателямиОстатки.Заказ = ЗНАЧЕНИЕ(Документ.ПриемИПередачаВРемонт.ПустаяСсылка)
	|				ТОГДА ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
