...
Функция ПроверитьЗаполнениеРеквизитов(МассивОшибок)
	...
	Если Не ЗначениеЗаполнено(Тип) Тогда
		ТекстОшибки = НСтр("ka='რეკვიზიტი ""ველის ტიპი"" არ არის შევსებული.';ru='Не заполнен реквизит Тип поля.';en='Not filled with props field type.'");
		МассивОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	
	// Проверка типа заполнения.
	Если Не ЗначениеЗаполнено(ТипЗаполнения) Тогда
		ТекстОшибки = НСтр("ka='რეკვიზიტი ""შევსების ტიპი"" არ არის შევსებული.';ru='Не заполнен реквизит Тип заполнения.';en='Not filled with props filling type.'");
		МассивОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	
	// Проверка значения.
	Если Не ЗначениеЗаполнено(Значение) И ТипЗаполнения = "Parameter" Тогда
		ТекстОшибки = НСтр("ka='რეკვიზიტი ""მნიშვნელობა"" არ არის შევსებული.';ru='Не заполнен реквизит Значение.';en='Not filled with props value.'");
		МассивОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	...
	Если Тип <> "UserData" Тогда
		...
		Если Лево>Право Тогда
			ТекстОшибки = НСтр("ka='მარცხენა კოორდინატა არ შეიძლება, რომ მარჯვენა კოორდინატაზე მეტი იყოს.';ru='Левая координата не может быть больше правой.';en='Left coordinate can not be more right.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
		
		Если Верх>Низ Тогда
			ТекстОшибки = НСтр("ka='ზემო კოორდინატა არ შეიძლება, რომ ქვემო კოორდინატაზე მეტი იყოს.';ru='Верхней координата не может быть больше нижней.';en='Top coordinate can not be more than the bottom.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
	...
	Если Тип = "Text" Тогда
		...
		Если Не ЗначениеЗаполнено(ПоложениеПоГоризонтали) Тогда
			ТекстОшибки = НСтр("ka='რეკვიზიტი ""თარაზული მდებარეობა"" არ არის შევსებული.';ru='Не заполнен реквизит Горизонтальное положение.';en='Not filled with props horizontal position.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
		
		// Проверка ПоложениеПоВертикали.
		Если Не ЗначениеЗаполнено(ПоложениеПоВертикали) Тогда
			ТекстОшибки = НСтр("ka='რეკვიზიტი ""შვეული მდებარეობა"" არ არის შევსებული.';ru='Не заполнен реквизит Вертикальное положение.';en='Not filled with props upright.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
	...
	ИначеЕсли Тип = "Barcode" Тогда
		...
		Если Не ЗначениеЗаполнено(ТипШтрихкода) Тогда
			ТекстОшибки = НСтр("ka='რეკვიზიტი ""შტრიხკოდის ტიპი"" არ არის შევსებული.';ru='Не заполнен реквизит Тип штрихкода.';en='Not filled with props barcode type.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
		...
		Если ПодписьШтрихкода Тогда
			Если Не ЗначениеЗаполнено(РазмерШрифта) Тогда
				ТекстОшибки = НСтр("ka='რეკვიზიტი ""ხელმოწერის შრიფტის ზომა"" არ არის შევსებული.';ru='Не заполнен реквизит Размер шрифта подписи.';en='Not filled with props signature font size.'");
				МассивОшибок.Добавить(ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
	...
	Если Тип = "Text" ИЛИ Тип = "Image" Тогда
		...
		Если РамкаСверху ИЛИ РамкаСлева ИЛИ РамкаСнизу ИЛИ РамкаСправа Тогда
			...
			Если Не ЗначениеЗаполнено(ТипРамки) Тогда
				ТекстОшибки = НСтр("ka='რეკვიზიტი ""ჩარჩოს ტიპი"" არ არის შევსებული.';ru='Не заполнен реквизит Тип рамки.';en='Not filled with props frame type.'");
				МассивОшибок.Добавить(ТекстОшибки);
			КонецЕсли;
			
			// Проверка ТолщинаРамки.
			Если Не ЗначениеЗаполнено(ТолщинаРамки) Тогда
				ТекстОшибки = НСтр("ka='რეკვიზიტი ""ჩარჩოს სისქე"" არ არის შევსებული.';ru='Не заполнен реквизит Толщина рамки.';en='Not filled with props frame thickness.'");
				МассивОшибок.Добавить(ТекстОшибки);
			КонецЕсли;
