	...
	Если НЕ РольДоступна(Метаданные.Роли.ПолныеПрава)
	   И НЕ(РольДоступна(Метаданные.Роли.ДобавлениеИзменениеПодсистемыПродажи) // Профиль Базовые права.
		...
		ВызватьИсключение(
		  ...
		  + Пользователь
		  + НСтр("ka='"" нет прав на синхронизацию данных с мобильным приложением 1С:Управление нашей фирмой. Необходимо включить профили прав доступа Базовые права и Деньги.';ru='"" нет прав на синхронизацию данных с мобильным приложением 1С:Управление нашей фирмой. Необходимо включить профили прав доступа Базовые права и Деньги.';en='""Do not have permission to synchronize data with the mobile application 1C:. Management of our company is necessary to include profiles of access rights Basic rights and money.'")
		);
...
Процедура ПроверитьДоступУстройства(КодМобильногоКомпьютера)
	...
	Если УзелОбмена.ДоступЗапрещен Тогда
		ВызватьИсключение(
			НСтр("ka='Устройству запрещена синхронизация с центральной базой.';ru='Устройству запрещена синхронизация с центральной базой.';en='Prohibited device synchronization with the central database.'")
		);
	КонецЕсли;
...
Функция ДобавитьПользователя(ПочтаПользователя, ПарольПользователяСервиса, ПрофильПользователя)
	...
	Возврат НСтр("ka='მომხმარებელს ელექტრონულ ფოსტაზე წარმატებით გაეგზავნა მოწვევა.';ru='Пользователю успешно отправлено приглашение на электронный адрес.';en='The user has successfully sent an invitation by e-mail.'");
...
Функция ОбновитьДанныеПользователя(ПочтаПользователя, ПарольПользователяСервиса, ПрофильПользователя, ВходРазрешен)
	...
	Если НЕ ЗначениеЗаполнено(Пользователь) Тогда
		Возврат НСтр("ka='მომხმარებელი ვერ მოიძებნა.';ru='Пользователь не найден.';en='User is not found.'");
	КонецЕсли;
	...
	Возврат НСтр("ka='მომხმარებლის პარამეტრები წარმატებით შეიცვალა.';ru='Параметры пользователя успешно изменены.';en='User Preferences updated successfully.'");
...
Процедура ПроверитьВозможностьДобавленияновогоУзла()
	...
	Если МобильноеПриложениеВызовСервера.НужноОграничитьФункциональность() Тогда
		...
		КонецЦикла;
		Если Сч >= 4 Тогда
			ВызватьИсключение(НСтр("ka='თქვენი სატარიფო გეგმა არ იძლევა სამზე მეტი მობილური დანართის ჩართვის შესაძლებლობას.';ru='Ваш тарифный план не позволяет подключать больше 3-х мобильных приложений.';en='Your plan does not allow you to connect more than 3 mobile applications.'"));
		КонецЕсли;
	КонецЕсли;
