...
Процедура ПерезаполнитьПоТребованию(Команда)
	...
	ТекстВопроса = НСтр("ka='Перезаполнить пояснение без учета ранее отправленных пояснений?"
"Изменения данного документа будут потеряны.';ru='Перезаполнить пояснение без учета ранее отправленных пояснений?"
...
Процедура УправлениеДекларацией(Форма)
	...
	Иначе
		ПредставлениеДекларации = НСтр("ka='Декларация не найдена';ru='Декларация не найдена';en='Declaration not found'");
	КонецЕсли;
...
Процедура УстановитьПредставлениеСчетаФактуры(СтрокаТаблицы, ТипРеквизитов)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы["НомерКорректировочногоСчетаФактуры"+ТипРеквизитов]) Тогда
		Если ЗначениеЗаполнено(СтрокаТаблицы["НомерИсправленияКорректировочногоСчетаФактуры"+ТипРеквизитов]) Тогда
			РеквизитыКоррСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ka='Корр. %1 от %2 (испр.%3 от %4) по с/ф ';ru='Корр. %1 от %2 (испр.%3 от %4) по с/ф '"),
		...
		Иначе
			РеквизитыКоррСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ka='Корр. %1 от %2 по с/ф ';ru='Корр. %1 от %2 по с/ф '"),
									СтрокаТаблицы["НомерКорректировочногоСчетаФактуры"+ТипРеквизитов],
									Формат(СтрокаТаблицы["ДатаКорректировочногоСчетаФактуры"+ТипРеквизитов], "ДЛФ=D"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы["НомерСчетаФактуры"+ТипРеквизитов]) Тогда
		Если ЗначениеЗаполнено(СтрокаТаблицы["НомерИсправленияСчетаФактуры"+ТипРеквизитов]) Тогда
			РеквизитыСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ka='%1 от %2(испр.%3 от %4)';ru='%1 от %2(испр.%3 от %4)';en='From 1% 2% (Corr.% 3% of 4)'"),
		...
		Иначе
			РеквизитыСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ka='%1 %2-დან';ru='%1 от %2';en='% 1% 2 from'"),
									СтрокаТаблицы["НомерСчетаФактуры"+ТипРеквизитов],
									Формат(СтрокаТаблицы["ДатаСчетаФактуры"+ТипРеквизитов], "ДЛФ=D"));
		КонецЕсли;
	КонецЕсли;
	...
	СтрокаТаблицы["РеквизитыСчетаФактуры"+ТипРеквизитов] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ka='%1%2';ru='%1%2';en='% 1% 2'"), РеквизитыКоррСчетаФактуры, РеквизитыСчетаФактуры);
...
Процедура ОткрытьФормуСчетаФактуры(ИмяТабличнойЧасти, ДанныеСтроки, ТипРеквизитов)
	...
	Если ПоискВыполнен Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Счет-фактура не найден';ru='Счет-фактура не найден';en='Invoice not found'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличныхЧастейПередОтправкой(Отказ)
	...
	Если Объект.КнигаПокупок.Количество() = 0
		...
		ТекстСообщения = НСтр("ka='Пояснение не заполнено. Отправка невозможна.';ru='Пояснение не заполнено. Отправка невозможна.';en='Explanation is not filled. Not sent.'");
...
Процедура ПроверитьНетКнигаПрод(Отказ)
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.НетКнигаПрод Цикл
		...
		ПроверитьИНН(СтрокаТабличнойЧасти, "ПокупательИНН", НСтр("ka='ИНН покупателя (по данным ФНС)';ru='ИНН покупателя (по данным ФНС)';en='INN buyer (according FTS)'"), Отказ);
		ПроверитьКПП(СтрокаТабличнойЧасти, "ПокупательКПП", НСтр("ka='КПП покупателя (по данным ФНС)';ru='КПП покупателя (по данным ФНС)';en='Transmission buyer (according FTS)'"), Отказ);
		
		Если СтрокаТабличнойЧасти.РезультатПроверки = Перечисления.РезультатПроверкиСтрокиДекларации.ВыявленыРасхождения Тогда
			
			ПроверитьИНН(СтрокаТабличнойЧасти, "ПокупательИННРасхождение", НСтр("ka='ИНН покупателя (уточненные данные)';ru='ИНН покупателя (уточненные данные)';en='TIN buyer (revised data)'"), Отказ);
			ПроверитьКПП(СтрокаТабличнойЧасти, "ПокупательКППРасхождение", НСтр("ka='КПП покупателя (уточненные данные)';ru='КПП покупателя (уточненные данные)';en='PPC buyer (revised data)'"), Отказ);
			
			ПроверитьИНН(СтрокаТабличнойЧасти, "ПосредникИНН", НСтр("ka='ИНН посредника (по данным ФНС)';ru='ИНН посредника (по данным ФНС)';en='Operation INN (according FTS)'"), Отказ);
			ПроверитьКПП(СтрокаТабличнойЧасти, "ПосредникКПП", НСтр("ka='КПП посредника (по данным ФНС)';ru='КПП посредника (по данным ФНС)';en='Transmission Operation (according FTS)'"), Отказ);
			
			ПроверитьИНН(СтрокаТабличнойЧасти, "ПосредникИННРасхождение", НСтр("ka='ИНН посредника (уточненные данные)';ru='ИНН посредника (уточненные данные)';en='Operation INN (updated data)'"), Отказ);
			ПроверитьКПП(СтрокаТабличнойЧасти, "ПосредникКППРасхождение", НСтр("ka='КПП посредника (уточненные данные)';ru='КПП посредника (уточненные данные)';en='Transmission Operation (updated data)'"), Отказ);
...
Процедура ПроверитьИНН(СтрокаТабличнойЧасти, ИмяПоля, НаименованиеПоля, Отказ)
	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если ЕстьОшибка Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Нет в книге продаж"" %2 заполнен некорректно:';ru='В строке %1 раздела ""Нет в книге продаж"" %2 заполнен некорректно:';en='Line% 1 in ""Not available The book""% 2 is filled correctly:'");
...
Процедура ПроверитьКПП(СтрокаТабличнойЧасти, ИмяПоляКПП, НаименованиеПоляКПП, Отказ)
	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если НЕ ЭтоЮридическоеЛицо И ЗначениеЗаполнено(КПП) Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Нет в книге продаж"" обнаружена ошибка: у ИП не может быть заполнен КПП';ru='В строке %1 раздела ""Нет в книге продаж"" обнаружена ошибка: у ИП не может быть заполнен КПП';en='Line% 1 in ""Out of the sales ledger"" has encountered a problem: in the IP can not be filled with PPC'");
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		...
		Если ЕстьОшибка Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Нет в книге продаж"" %2 заполнено некорректно:';ru='В строке %1 раздела ""Нет в книге продаж"" %2 заполнено некорректно:';en='Line% 1 in ""Not available The book""% 2 filled correctly:'");
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
		Если СтрокаТабличнойЧасти.РезультатПроверки = Перечисления.РезультатПроверкиСтрокиДекларации.ВыявленыРасхождения Тогда
			...
			Если РасхожденийНеНайдено Тогда
				ТекстСообщения = НСтр("ka='В строке %1 %2 указан ответ ""Найдены расхождения"", но уточненные сведения соответствуют данным ФНС';ru='В строке %1 %2 указан ответ ""Найдены расхождения"", но уточненные сведения соответствуют данным ФНС';en='In line% 1% 2 Set the answer is ""found differences"", but updated data are consistent with the Federal Tax Service'");
...
Процедура ПроверитьКонтрольныеСоотношения(Отказ)
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.СведКС Цикл 
		...
		Если НЕ ЗначениеЗаполнено(НомерКС) Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Контрольные соотношения"" не заполнен номер контрольного соотношения';ru='В строке %1 раздела ""Контрольные соотношения"" не заполнен номер контрольного соотношения';en='Line% 1 in ""Control ratio"" is not filled with a control ratio of the number'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаТабличнойЧасти.НомерСтроки);
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ПоясненКС) Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Контрольные соотношения"" не заполнено пояснение расхождений';ru='В строке %1 раздела ""Контрольные соотношения"" не заполнено пояснение расхождений';en='Line 1% of ""Control ratio"" not filled explanation discrepancies'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаТабличнойЧасти.НомерСтроки);
		...
		ИначеЕсли НомерКСПослеЗамены <> "X.X" 
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Контрольные соотношения"" номер контрольного соотношения"
"не соответствует ни одному из шаблонов X.X, X.XX, X.X.X, X.XX.X';ru='В строке %1 раздела ""Контрольные соотношения"" номер контрольного соотношения"
...
Функция ИдДекларации(АдресДанных, ВыбранноеИмяФайла)
	...
	Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиДекларацияНДС(СвойстваФайла.Имя) Тогда
		...
		Если НРег(СвойстваФайла.Расширение) = НРег(".xml") Тогда
			...
			Исключение
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Файл ""%1"" не является файлом выгрузки декларации по НДС в формате XML.';ru='Файл ""%1"" не является файлом выгрузки декларации по НДС в формате XML.';en='File ""%1"" is not a file upload on VAT returns in XML format.'"), СвойстваФайла.Имя);
			...
			Если КНД = Неопределено ИЛИ КНД = "" ИЛИ КНД <> "1151001" Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выбранный файл не является файлом декларации по НДС';ru='Выбранный файл не является файлом декларации по НДС';en='The selected file is not a file VAT returns'"));
				Возврат Неопределено;
			КонецЕсли;
			...
			Если НЕ ДокументооборотСКОВызовСервера.ИспользуетсяРежимТестирования() Тогда
				...
				Если ОрганизацияВФайле <> Объект.Организация Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании';ru='Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании';en='Organization in the selected file the VAT return does not coincide with the organization referred to in the request'"));
					Возврат Неопределено;
				КонецЕсли;
				...
				Если ВТребованииЕстьXMLФайл Тогда
					...
					Если НомКорр <> СвойстваДекларации.ТребованиеНомКорр Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании';ru='Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании';en='Number of adjustments to the VAT return the selected file does not match with the correct number specified in the request'"));
						Возврат Неопределено;
					КонецЕсли;
					
					Период = ПоказателиВыгрузки["Период"];
					Если Период <> СвойстваДекларации.ТребованиеПериод Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании';ru='Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании';en='The period of the selected file VAT returns not coincides with the period specified in the request'"));
						Возврат Неопределено;
					КонецЕсли;
					
					ОтчетГод = ПоказателиВыгрузки["ОтчетГод"];
					Если ОтчетГод <> СвойстваДекларации.ТребованиеОтчетГод Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании';ru='Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании';en='Year in the selected file the VAT return does not coincide with the year in the demand'"));
						Возврат Неопределено;
					КонецЕсли;
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Файл декларации по НДС должен иметь расширение xml';ru='Файл декларации по НДС должен иметь расширение xml';en='VAT returns file must have an xml'"));
			Возврат Неопределено;
		КонецЕсли;
