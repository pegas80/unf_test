...
Процедура УстановитьНадписьЦеныИВалюта()
	...
	Иначе
		...
		ЦеныИВалюта = НСтр("ka='ვალუტა არ არის მითითებული';ru='Валюта не указана';en='Currency is not specified'");
...
Процедура ЗаполнитьНомераГТД(Команда)
	...
	ТекстВопроса = НСтр("ka='ცხრილში გადავსებული იქნება სვეტი ""წარმოშობის ქვეყანა"" და ""საბაჟო დეკლარაციის ნომერი"". გავაგრძელოთ?';ru='В табличной части будет перезаполнена колонка ""Страна происхождения"" и ""Номер ГТД"". Продолжить?';en='In the tables of this will refill the column ""Country of origin"" and ""Number of GTE"". Proceed?'");
