...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++Lazare 19.07.2023
	ЗапретРедактирования = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияПроведенныхДокументовСервиса");		
	...
	ИспользоватьПодсистемуЗарплата = ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуЗарплата")
		И (УправлениеДоступом.ЕстьРоль("ДобавлениеИзменениеПодсистемыЗарплата") ИЛИ УправлениеДоступом.ЕстьРоль("ПолныеПрава") ИЛИ УправлениеДоступом.ЕстьРоль("ДобавлениеИзменениеПодсистемыПродажи"));
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='სერვისის დოკუმენტი (შექმნა)';ru='Заказ-наряд (Создание)';en='Work Order (Creating)'");
		//ПаганиниRS_
		Объект.НомерТТН = "";
		Объект.IDTТН	= "";
		//_ПаганиниRS
	...
	//ПодольскийАИ #ПроверкаОрганизации 22.12.2022 {
	//Убрать
	...
	VINКод = Объект.ТранпсортноеСедство.VINКод; //N
...
Процедура ПриОткрытии(Отказ)
	...
	// Олег Литвиненко 19.10.22 --->
	ПересчитыватьСуммаНачисления = Истина;
	// Олег Литвиненко <---

	//Объект.Финиш = Объект.Старт;//T
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Отказ = ПроверяиятьНаМаксимальныйПроцентСкидки();//T
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен Тогда
		Если Не ЗначениеЗаполнено(ТекущийОбъект.Ссылка) Или ТекущийОбъект.Ссылка.СостояниеЗаказа <> ТекущийОбъект.СостояниеЗаказа Тогда				
			ТекущийОбъект.Финиш = ТекущаяДата();
		КонецЕсли; 	
	КонецЕсли;	
...
Процедура ЗаполнитьТС()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		Иначе 
			Объект.ТранпсортноеСедство = Контрагент.ОсновноеТранпсортноеСедство;
		КонецЕсли;	
	КонецЕсли;
...
Процедура ЗНСтартПриИзменении(Элемент)
	...
	Если Объект.Старт > Объект.Финиш Тогда
		...
		Сообщить(НСтр("ka='სტარტის თარიღი არ შეიძლება, რომ ფინიშის თარიღზე მეტი იყოს.';ru='Дата старта не может быть больше даты финиша.';en='the start date may not be more than the finish date.'"));
	Иначе
		ПриИзмененииСтарт = Объект.Старт;
	КонецЕсли;
...
Процедура ЗНФинишПриИзменении(Элемент)
	...
	Если Объект.Финиш < Объект.Старт Тогда
		...
		Сообщить(НСтр("ka='ფინიშის თარიღი არ შეიძლება, რომ სტარტის თარიღზე ნაკლები იყოს.';ru='Дата финиша не может быть меньше даты старта.';en='finish date may not be less than the start date.'"));
	Иначе
		ПриИзмененииФиниш = Объект.Финиш;
		
		//+++ Дмгг
		ДатаПередИзменением = ДатаДокумента;
		ДатаФинишьПередИзменением = ДатаФинишДокумента;
		ДатаФинишДокумента = Объект.Финиш;
		Если Объект.Финиш <> ДатаФинишьПередИзменением Тогда
			СтруктураДанные = ПолучитьДанныеДатаПриИзменении(ДатаПередИзменением, ВалютаРасчетов);
			//Если СтруктураДанные.РазностьДат <> 0 Тогда
			//	Объект.Номер = "";
			//КонецЕсли;
...
Процедура РаботыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если УчетПоПроектам И НоваяСтрока И Не Копирование Тогда
		...
		ЗаполнитьПроектПоДокументуВТабЧасть("Материалы", Объект.Проект, Элемент.ТекущиеДанные);
	КонецЕсли;	
	//ПодольскийАИ #УчетПроектов 10.10.2020 }
...
Процедура ИсполнителиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	////Паганини ПФ_
	//Если ИзменениеСуммыНалогов Тогда
	//	ИзменениеСуммыНалогов = Ложь;
	//	Возврат;
	//КонецЕсли;
	//_Паганини ПФ	
	//ОбновитьТабличнуюЧастьИсполнители();
	...
	// Олег Литвиненко 19.10.22 --->
	//Если ПересчитыватьСуммаНачисления Тогда
		ОбновитьТабличнуюЧастьИсполнители();
	//КонецЕсли;
...
Процедура ИсполнителиСотрудникПриИзменении(Элемент)
	...
	//Паганини ПФ_
	СтрокаТабличнойЧасти.ВидУдержанияНДФЛ	= СтруктураДанные.ВидУдержанияНДФЛ;
	СтрокаТабличнойЧасти.ВидУдержанияПФ		= СтруктураДанные.ВидУдержанияПФ;
	//_Паганини ПФ	
	
	//+++
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ТипНДФЛ) Тогда 
		СуммаНачисления = СтрокаТабличнойЧасти.СуммаНачисления;
		СуммаНДФЛ = 0;  
		ТипНДФЛ = СтрокаТабличнойЧасти.ТипНДФЛ;
		РасчитатьСуммуНДФЛ(СуммаНачисления, ТипНДФЛ, СуммаНДФЛ);
...
&НаСервере
Процедура РасчитатьСуммуНДФЛ(СуммаНачисления, ТипНДФЛ, СуммаНДФЛ = 0)

	ЗначениеСтавкиНДФЛ = ТипНДФЛ.Ставка;
	СуммаНДФЛ = СуммаНачисления * (ЗначениеСтавкиНДФЛ/100);

КонецПроцедуры

//+++
&НаКлиенте
Процедура ИсполнителиРазмерНачисленияУдержанияПриИзменении(Элемент)
	//Паганини_ПФ
	//СтрокаТабличнойЧасти = Элементы.Исполнители.ТекущиеДанные;
	//
	//Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ТипНДФЛ) Тогда 
	...
	//	СтрокаТабличнойЧасти.СуммаНДФЛ = СуммаНДФЛ;
	//КонецЕсли;
	РасчитатьСуммы_ПФ_НДФЛ(Элементы.Исполнители.ТекущаяСтрока);
	//_Паганини ПФ
КонецПроцедуры

&НаКлиенте
Процедура ИсполнителиТипНДФЛПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.Исполнители.ТекущиеДанные;
...
Процедура ЗаполнитьПоСпецификации(Команда)
	...
	Если ТекущаяСтрокаТЧ = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='არ არის არჩეული ძირითადი ცხრილის სტრიქონი!';ru='Не выбрана строка основной табличной части!';en='Do not select the main line of the table!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект, Новый Структура("РезультатПоиска", РезультатПоиска)),
			НСтр("ka='ცხრილი ""მასალები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""Materials"" will be refilled! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьМатериалыИзВсехСпецификаций(Команда)
	...
	Если НЕ Объект.Работы.Количество() > 0 Тогда
		...
		Сообщение		= Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შეავსეთ ცხრილი ""სამუშაოები"".';ru='Заполните табличную часть ""Работы"".';en='Fill in the tables of this ""work"".'");
	...
	Если Объект.Материалы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьМатериалыИзВсехСпецификацийЗавершение", ЭтотОбъект),
			НСтр("ka='ოპერაციის შესასრულებლად საჭიროა ცხრილ ""მასალები""-ს გასუფთავება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Для выполнения операции требуется очистить табличную часть ""Материалы""! Продолжить выполнение операции?';en='To perform the operation you want to clear a table of the ""Materials""! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьПоРесурсамДляТекущейРаботы(Команда)
	...
	Если Объект.РесурсыПредприятия.Количество() = 0 Тогда
		...
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ka='ცხრილში ""ამოქმედებული რესურსები"" ჩანაწერები არ არის!';ru='В табличной части ""Задействованные ресурсы"" нет записей!';en='In the tables of this ""use of resources"" has no entries!'"));
		Возврат;
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი ""შემსრულებლები"" მიმდინარე სამუშაოსთვოს გადავსებული იქნება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Исполнители"" для текущей работы будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""Artists"" will be refilled for the current job! To continue the operation?'");
...
Процедура ЗаполнитьПоРесурсамДляВсехРабот(Команда)
	...
	Если Объект.Исполнители.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРесурсамДляВсехРаботЗавершение", ЭтотОбъект),
			НСтр("ka='ცხრილი ""შემსრულებლები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Исполнители"" будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""Artists"" will be refilled! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоБригадамДляТекущейРаботыЗавершение1(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоБригадамДляТекущейРаботыЗавершение", ЭтотОбъект, Новый Структура("МассивБригад, РезультатПоиска", МассивБригад, РезультатПоиска)), НСтр("ka='ცხრილი ""შემსრულებლები"" მიმდინარე სამუშაოსთვოს გადავსებული იქნება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Исполнители"" для текущей работы будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""Artists"" will be refilled for the current job! To continue the operation?'"),
	...
	//Малышев 2025-01-23 ONESOFT-3790
	Для Каждого СтрИсполнитель из Объект.Исполнители Цикл
		ОбновитьТабличнуюЧастьИсполнители(СтрИсполнитель);
	КонецЦикла;	
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоПоступлениюВРемонтЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულად გადაივსება საფუძველ-დოკუმენტით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет полностью перезаполнен по документу-основанию! Продолжить выполнение операции?';en='The document will be fully refilled on document-base! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаполнитьОснованиеПечатиНаКлиенте(ДоговорПередИзменением, ДанныеНовогоДоговора)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОснованиеПечатиСсылка)
		...
		Объект.ОснованиеПечати = Нстр("ka='Договор: ';ru='Договор: '") + Строка(ДанныеНовогоДоговора.НаименованиеДоговора);
...
&НаКлиенте
Процедура ОбновитьТабличнуюЧастьИсполнители(ТекущаяСтрокаИсполнители = Неопределено)
	
	//Малышев 202-01-23 ONESOFT-3790
	Если ТекущаяСтрокаИсполнители = Неопределено ТОгда
...
Процедура ПересчитатьСуммуНачисленийПоУказаннойРаботе(КлючСвязи)
	...
	Для каждого СтрокаИсполнителя Из МассивИсполнителей Цикл
		...
		//Малышев 2025-01-24 ONESOFT-3790 (--
		СтруктураСотрудника = Новый Структура;
		СтруктураСотрудника.Вставить("НаДату", Объект.Дата);
		СтруктураСотрудника.Вставить("Организация", объект.Организация);
		СтруктураСотрудника.Вставить("ВалютаДокумента",Объект.ВалютаДокумента);
		СтруктураСотрудника.Вставить("Сотрудник",СтрокаИсполнителя.Сотрудник);
		ДанныеСотрудника = Документы.ЗаказПокупателя.ПолучитьДанныеСотрудникаПриИзменении(СтруктураСотрудника);
		Если не ЗначениеЗаполнено(СтрокаИсполнителя.ВидУдержанияНДФЛ) и ЗначениеЗаполнено(ДанныеСотрудника.ВидУдержанияНДФЛ) Тогда
			СтрокаИсполнителя.ВидУдержанияНДФЛ = ДанныеСотрудника.ВидУдержанияНДФЛ;
		КонецЕсли;	
		Если не ЗначениеЗаполнено(СтрокаИсполнителя.ВидУдержанияПФ) и ЗначениеЗаполнено(ДанныеСотрудника.ВидУдержанияПФ) Тогда
			СтрокаИсполнителя.ВидУдержанияПФ = ДанныеСотрудника.ВидУдержанияПФ;
		КонецЕсли;	
		//Малышев 2025-01-24 ONESOFT-3790 --)
		...
		СтавкаНДФЛ	= 0;
		СтавкаПФ	= 0;
		Если ЗначениеЗаполнено(СтрокаИсполнителя.ВидУдержанияНДФЛ) Тогда
			СтавкаНДФЛ = СтрокаИсполнителя.ВидУдержанияНДФЛ.СтавкаНДФЛ;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаИсполнителя.ВидУдержанияПФ) Тогда
			СтавкаПФ = СтрокаИсполнителя.ВидУдержанияПФ.СтавкаПФ;
		КонецЕсли;
		СтрокаИсполнителя.СуммаПФ		= Окр(СтрокаИсполнителя.СуммаНачисления * СтавкаПФ/100, 2);
		СтрокаИсполнителя.СуммаНДФЛ		= Окр((СтрокаИсполнителя.СуммаНачисления - СтрокаИсполнителя.СуммаПФ) * СтавкаНДФЛ/100, 2);
		//_Паганини ПФ
 	КонецЦикла;
...
Функция ПолучитьДанныеДатаПриИзменении(ДатаПередИзменением, ВалютаРасчетов)
	...
	//ВалютаКурсКратность = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Объект.Дата, Новый Структура("Валюта", ВалютаРасчетов)); //+++ Дмгг
	ВалютаКурсКратность = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Объект.Финиш, Новый Структура("Валюта", ВалютаРасчетов)); //+++ Дмгг
	...
	Если СтруктураДанные.Свойство("ВидЦен") Тогда
		...
		Если СтруктураДанные.Свойство("ВидРабот") Тогда
			...
			Иначе
				...
				//СтруктураДанные.Номенклатура = СтруктураДанные.ВидРабот; //+++ Дмгг
...
Функция ПолучитьДанныеСотрудникаПриИзменении(СтруктураДанные)
	...
	Возврат Документы.ЗаказПокупателя.ПолучитьДанныеСотрудникаПриИзменении(СтруктураДанные);
...
Процедура РассчитатьСуммуНДС(СтрокаТабличнойЧасти)
	...
	Если СтрокаТабличнойЧасти.Гарантия 
		// и (СтрокаТабличнойЧасти.СтавкаНДС = ПредопределенноеЗначение("Справочник.СтавкиНДС.ОсвобожденОтНДС") 
		//	или СтрокаТабличнойЧасти.СтавкаНДС =ПредопределенноеЗначение("Справочник.СтавкиНДС.БезНДС"))
		Тогда
		СтавкаНДС = 0; //УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтавкаНДСНОменклатуры(СтрокаТабличнойЧасти.Номенклатура));
	КонецЕсли;
	//Малышев 2025-11-14 ONESOFT-4452 ---)
...
//Малышев 2025-11-14 ONESOFT-4452  ((---
&НаСервереБезКонтекста
Функция СтавкаНДСНОменклатуры(СпрНоменклатуры)

	Возврат СпрНоменклатуры.СтавкаНДС;

КонецФункции // СтавкаНДСНОменклатуры()
//Малышев 2025-11-14 ONESOFT-4452 ---)
...
Процедура УправлениеФормой()
	...
	// Олег Литвиненко 19.10.22 --->
	Элементы.ИсполнителиСуммаНачисления.Доступность = УправлениеНебольшойФирмойВызовСервера.ПолучитьФункциональнуюОпциюСервер("РедактированиеСуммыНачисленияВЗаказНарядах");
	// Олег Литвиненко <---	
...
&НаСервере
Процедура ЗНЗапасыПроверитьСвободныеОстаткиНаСервере()
	Перем РЕЗ;
...
Процедура ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткам(Команда)
	...
	Если РезультатПоиска.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ცხრილი ""მასალები"" არ არის შევსებული!';ru='Табличная часть ""Материалы"" не заполнена!';en='A table of the ""Materials"" is not full!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткамДляВсех(Команда)
	...
	Если Объект.Работы.Количество() > 1 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткамДляВсехЗавершение", ЭтотОбъект),
			НСтр("ka='ცხრილ ""მასალები""-ში სვეტი ""რეზერვი"" გადავსებული იქნება ყველა სამუშაოსთვის! გავაგრძელოთ ოპერაციის შესრულება?';ru='В табличной части ""Материалы"" колонка ""Резерв"" будет перезаполнена для всех работ! Продолжить выполнение операции?';en='In the tables of the ""Materials"" column ""Reserve"" will refill for all the work! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
...
// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
&НаКлиенте
Процедура ЗНМатериалыПроверитьСвободныеОстатки(Команда)
	
	Если ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаРезерв) И ЗначениеЗаполнено(Объект.Организация) Тогда
		
		ЗНМатериалыПроверитьСвободныеОстаткиНаСервере();
...
&НаСервере
Процедура ЗНМатериалыПроверитьСвободныеОстаткиНаСервере()
	Перем РЕЗ;
	
	ДатаПроверки = ?(ЗначениеЗаполнено(Объект.Дата),Объект.Дата,ТекущаяДата());
	ТЗ_Материалы = Объект.Материалы.Выгрузить(,"НомерСтроки,Номенклатура,Характеристика,Партия,Проект,ЕдиницаИзмерения,Количество,Резерв");
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТЗ_Материалы.НомерСтроки КАК НомерСтроки,
		|	ТЗ_Материалы.Номенклатура КАК Номенклатура,
		|	ТЗ_Материалы.Характеристика КАК Характеристика,
		|	ТЗ_Материалы.Партия КАК Партия,
		|	ТЗ_Материалы.Проект КАК Проект,
		|	ТЗ_Материалы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ТЗ_Материалы.Количество КАК Количество,
		|	ТЗ_Материалы.Резерв КАК Резерв
		|ПОМЕСТИТЬ ТЗ_Материалы
		|ИЗ
		|	&ТЗ_Материалы КАК ТЗ_Материалы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		КОГДА &КонтролироватьОстаткиПриПроведенииПоПроектам
		|			ТОГДА ВЫБОР
		|					КОГДА ТЗ_Материалы.Проект ЕСТЬ NULL
		|							ИЛИ ТЗ_Материалы.Проект = НЕОПРЕДЕЛЕНО
		|							ИЛИ ТЗ_Материалы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|						ТОГДА ВЫБОР
		|								КОГДА &Проект ЕСТЬ NULL
		|										ИЛИ &Проект = НЕОПРЕДЕЛЕНО
		|										ИЛИ &Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		...
		|								ИНАЧЕ &Проект
		|							КОНЕЦ
		|					ИНАЧЕ ТЗ_Материалы.Проект
		|				КОНЕЦ
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ТЗ_Материалы.ЕдиницаИзмерения) = ТИП(Справочник.ЕдиницыИзмерения)
		|			ТОГДА ТЗ_Материалы.Количество * ВЫРАЗИТЬ(ТЗ_Материалы.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения).Коэффициент
		|		ИНАЧЕ ТЗ_Материалы.Количество
		|	КОНЕЦ КАК Количество,
		|	ТЗ_Материалы.Резерв КАК Резерв
		|ПОМЕСТИТЬ ТаблицаДанных
		|ИЗ
	...
	Запрос.УстановитьПараметр("ТЗ_Материалы",ТЗ_Материалы);
	Запрос.УстановитьПараметр("Организация",Объект.Организация);
	Запрос.УстановитьПараметр("СтруктурнаяЕдиница",Объект.СтруктурнаяЕдиницаРезерв);
	Запрос.УстановитьПараметр("Проект",Объект.Проект);
	Запрос.УстановитьПараметр("КонтролироватьОстаткиПриПроведенииПоПроектам",КонтролироватьОстаткиПриПроведенииПоПроектам);
	...
	Для каждого ТекущаяСтрока Из ТаблицаСвободныхОстатоков Цикл
		...
		СтрокаМатериалы = Объект.Материалы[ТекущаяСтрока.НомерСтроки-1];
		СтрокаМатериалы.Резерв = МИН(ТекущаяСтрока.СвободныйОстаток,ТекущаяСтрока.Количество);
		СтрокаМатериалы.СвободныйОстаток = ТекущаяСтрока.СвободныйОстаток;
		СтрокаМатериалы.ОжидаемыеПоставки = ТекущаяСтрока.ОжидаемыеПоставки;
		СтрокаМатериалы.ОжидаемыеСвободныеОстатки = ТекущаяСтрока.ОжидаемыеСвободныеОстатки;			
...
Процедура Фкс_РезервироватьНаСервере(СписокНоменклатур,Склад)
	...
	докРезерв.Комментарий = "Создан из документа " + Объект.Ссылка + " . Ручное изменение документа не разрешено!";
...
Процедура ПроверитьДокРезер(Отказ)
	...
	Если докРезерв = Неопределено ИЛИ Константы.ФункционалРезервированияЗаказов.Получить() = Истина Тогда
		Возврат;
	КонецЕсли;	
...
&НаКлиенте
Процедура ИсполнителиВидУдержанияНДФЛПриИзменении(Элемент)
	РасчитатьСуммы_ПФ_НДФЛ(Элементы.Исполнители.ТекущаяСтрока);
КонецПроцедуры

&НаКлиенте
Процедура ИсполнителиВидУдержанияПФПриИзменении(Элемент)
	РасчитатьСуммы_ПФ_НДФЛ(Элементы.Исполнители.ТекущаяСтрока);
КонецПроцедуры
...
Процедура РасчитатьСуммы_ПФ_НДФЛ(Идентификатор)
	...
	ТекСтрока = Объект.Исполнители.НайтиПоИдентификатору(Идентификатор);
	...
	ТекСтрока.СуммаПФ		= Окр(ТекСтрока.СуммаНачисления * СтавкаПФ/100, 2);
	ТекСтрока.СуммаНДФЛ		= Окр((ТекСтрока.СуммаНачисления - ТекСтрока.СуммаПФ) * СтавкаНДФЛ/100, 2);
	
КонецПроцедуры

&НаКлиенте
Процедура ИсполнителиСуммаНДФЛПриИзменении(Элемент)
	ИзменениеСуммыНалогов = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИсполнителиСуммаПФПриИзменении(Элемент)
	ИзменениеСуммыНалогов = Истина;
КонецПроцедуры
...
Процедура ТранспортныеСредстваТранпсортноеСедствоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Элементы.ТранспортныеСредства.ТекущиеДанные.ТранпсортноеСедство = ВивестиФормуОтбораТС();
	
	ТранспортныеСредстваТранпсортноеСедствоПриИзмененииНаСервере(Элементы.ТранспортныеСредства.ТекущиеДанные.ТранпсортноеСедство);
...
&НаСервере
Процедура ТранспортныеСредстваТранпсортноеСедствоПриИзмененииНаСервере(ТранпсортноеСедство)
	Если ЗначениеЗаполнено(ТранпсортноеСедство) И Контрагент.ТранспортныеСредства.Найти(ТранпсортноеСедство,"ТранпсортноеСедство") = Неопределено  Тогда
		
		ИзмКонтрагент = Объект.Контрагент.ПолучитьОбъект();
		Стр = ИзмКонтрагент.ТранспортныеСредства.Добавить();
		Стр.ТранпсортноеСедство = ТранпсортноеСедство;
		ИзмКонтрагент.Записать();
...
Процедура ЗаполнитьФактурыНаСервере()
	...
	СуммаОсталосьРаспределить = Объект.Запасы.Итог("СуммаНДС") + 
								Объект.Работы.Итог("СуммаНДС");
	Если СуммаОсталосьРаспределить = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаАвансов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьТаблицуАвансовФактур(Объект.Ссылка, "РасходнаяНакладная", Объект.Финиш, Объект.Организация, Объект.Контрагент); //Михаил Бураев 19.04.2024. В параметрах заменил Дата на Финиш
...
&НаКлиенте
Процедура ЗНПроектПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЗаполнитьПроектПоДокументу(ПроектЗначение = Неопределено, ВидТЧ = Неопределено)
	
	Если ПроектЗначение = Неопределено Тогда
		Объект.Проект = ПредопределенноеЗначение("Справочник.Проекты.БезПроекта");
	КонецЕсли;
	
	Если ВидТЧ = Неопределено Тогда
		ЗаполнитьПроектПоДокументуВТабЧасть("Запасы", Объект.Проект);
		ЗаполнитьПроектПоДокументуВТабЧасть("Работы", Объект.Проект);
		ЗаполнитьПроектПоДокументуВТабЧасть("Материалы", Объект.Проект);
	Иначе
		ЗаполнитьПроектПоДокументуВТабЧасть(ВидТЧ, Объект.Проект);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПроектПоДокументуВТабЧасть(ВидТЧ, ПроектЗначение, ТекДанные = Неопределено)
	
	Если ТекДанные = Неопределено Тогда 
		Для Каждого СтрокаТЧ Из Объект[ВидТЧ] Цикл
			СтрокаТЧ.Проект = ПроектЗначение;
		КонецЦикла;
	Иначе
		Для Каждого СтрокаТЧ Из Объект[ВидТЧ] Цикл
...
Процедура РаботыПроектПриИзменении(Элемент)
	...
	КлючСвязи = ТекДанные.КлючСвязи;
	ЗаполнитьПроектПоДокументуВТабЧасть("Материалы", ТекДанные.Проект, ТекДанные);
	
КонецПроцедуры

// Олег Литвиненко 19.10.22 --->
&НаКлиенте
Процедура ИсполнителиСуммаНачисленияПриИзменении(Элемент)
	ПересчитыватьСуммаНачисления = Ложь;
КонецПроцедуры// Олег Литвиненко <---


&НаКлиенте
Процедура ЗапасыГарантияПриИзменении(Элемент)
...
&НаКлиенте
Процедура РаботыГарантияПриИзменении(Элемент)
