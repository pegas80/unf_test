...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ТекстСообщения <> "" Тогда
			...
			Сообщение.Текст = ?(Отказ, НСтр("ru = 'Документ не проведен! '; ka = 'დოკუმენტი არ გატარებულა!'; en = 'Document not posted!'") + ТекстСообщения, ТекстСообщения);
			Сообщение.Сообщить();
		...
		Если Объект.ВидОперации = ВидОперацииВозврат Тогда
			...
			Если НеСоответствуютРеквизитыОплаты Тогда
				...
				ТекстСообщения = НСтр("ka='მიმდინარე დოკუმენტში მითითებულია გადახდის რეკვიზიტები, რომლებიც არ ემთხვევა გადახდის დოკუმენტებში არსებულ რეკვიზიტებს. შეამოწმეთ ეკვაირინგის ტერმინალი, საგადახდო ბარათის სახეობა და ნომერი.';ru='В текущем документы указаны реквизиты оплаты, которые не совпадают с реквзитами в документе оплаты. Проверьте эквайринговый терминал, вид и номер платежной карты.';en='In the current documents including payment details, which do not coincide with rekvzitami in the document of payment. Check acquiring the terminal type and the payment card number.'");
...
Процедура ДополнитьАвансамиИОплатамиПоТаблицеДокумента(ДокументОбъект, ОбщиеПараметры)
	...
	НаименованиеАванса = 
	НСтр("ru = 'Аванс от: '; ka = 'წინასწარი გადახდა:'; en = 'Advance from:'")
	+ ПринятоОт
	+ НСтр("ru = ', Основание: '; ka = ', საფუძველი: '; en = ', Basis:'")
	...
	Для каждого ТекСтрока Из ДокументОбъект.РасшифровкаПлатежа Цикл
		...
		Иначе
			НаименованиеТовара = 
			НСтр("ru = 'Оплата от: '; ka = 'გადახდა დან: '; en = 'Payment from:'")
			+ ПринятоОт
			+ НСтр("ru = ', Основание: '; ka = ', საფუძველი: '; en = ', Basis:'")
			+ ТекСтрока.Документ;
...
Процедура ЗаполнитьПоОперацииОплаты(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОперацияПоПлатежнымКартамШапка) Тогда
		ПоказатьПредупреждение(Неопределено,Нстр("ka='გადახდის დოკუმენტი არ არის არჩეული!';ru='Не выбран документ оплаты!';en='Do not select a document of payment!'"));
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОперацииОплатыЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულად გადაივსება გადახდის დოკუმენტით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет полностью перезаполнен по документу оплаты! Продолжить выполнение операции?';en='The document will be fully refilled through the document of payment! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ДобавитьОплатуКартой(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(ОписаниеОповещенияДобавитьОплатуКартой, НСтр("ru = 'Номер карты уже заполнен. ""Продолжить оформление оплаты картой с использованием эквайрингового терминала?'; ka = 'ბარათის ნომერი უკვე შევსებულია. გავაგრძელოთ ეკვაირინგის ტერმინალით ბარათით გადახდის გაფორმება?'; en = 'The card number has already been filled in. ""Do you want to continue making card payments using the acquiring terminal?'"), РежимДиалогаВопрос.ДаНет,,
...
Процедура ОповещениеОткрытьФормуАвторизацииЭТ(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатОткрытияФормы) = Тип("Структура") Тогда
		...
		Если ПараметрДействия.ПараметрыЭТ.ИспользоватьБезПодключенияОборудования Тогда
			...
			ТекстВопроса = НСтр("ru = 'Требуется выполнить операцию оплаты на эквайринговом терминале.'; ka = 'საჭიროა საგადახდო ტრანზაქციის შესრულება შემსყიდველ ტერმინალში.'; en = 'Payment is required to perform an operation on acquiring terminals.'") + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru = 'Сумма операции:'; ka = 'ოპერაციის თანხა:'; en = 'Transaction Amount:'") + " "+ ПараметрДействия.СуммаОперации  + Символы.ПС;
			ТекстВопроса = ТекстВопроса + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru = 'Операция оплаты на эквайринговом терминале прошла успешно?'; ka = 'იყო თუ არა გადახდის ტრანზაქცია შემსყიდველ ტერმინალში წარმატებული?'; en = 'payment transaction on acquiring terminal is successful?'");
...
Процедура ОповещениеВопросУспешностиОплатыКартой(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если НЕ (РезультатВопроса = КодВозвратаДиалога.Да) Тогда
		СтрокаСлипЧека = "";
		ТекстОписаниеОшибки = НСтр("ru = 'Операция отменена.'; ka = 'ოპერაცია გაუქმდა.'; en = 'Operation canceled.'");
...
Процедура ОповещениеОткрытьФормуАвторизацииЭТЗавершение(РезультатВыполнения, ПараметрДействия) Экспорт 
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ЗавершитьОплатуКартой(ПараметрДействия);
	Иначе
		ТекстСообщения = НСтр("ru = 'При выполнении операции возникла ошибка:""""%ОписаниеОшибки%"".""Оплата по карте не была произведена.'; ka = 'ოპერაციის შესრულებისას წარმოიშვა შეცდომა: ""%ОписаниеОшибки%"".""ოპერაცია ბარათით არ განხორციელდა.'; en = 'An error occurred while performing the operation: ""%ОписаниеОшибки%"".""The operation was not performed with the card.'");
...
Процедура ЗавершитьОплатуКартой(ПараметрДействия)
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა!';ru='Не удалось выполнить проведение документа!';en='Unable to hold the document!'");
		Сообщение.Сообщить();
		Записать();
	КонецПопытки;
		...
		Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента()Тогда // Проверка на определенность рабочего места ВО
			...
			Если Не глПодключаемоеОборудование.Свойство("ПоследнийСлипЧек", СтрокаСлипЧека)
			 ...
			 Или ПустаяСтрока(СтрокаСлипЧека) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Слип-чек отсутствует.""Возможно для данного сеанса еще не выполнялась эквайринговая операция.'; ka = 'სლიპ-ჩეკი არ არის.""შეიძლება მოცემული სეანსისთვის ჯერ არ შესრულებულა ეკვაირინგული ოპერაცია.'; en = 'There is no slip check.""It is possible that an acquiring operation has not yet been performed for this session.'"));
				Возврат;
			КонецЕсли;
			...
			Если ИдентификаторУстройстваФР <> Неопределено Тогда
				...
				Если РезультатФР Тогда
					...
					Если Не РезультатФР Тогда
						ТекстСообщения = НСтр("ru = 'При печати слип-чека возникла ошибка:""""%ОписаниеОшибки%"".'; ka = 'სლიპ-ჩეკის ბეჭდვისას მოხდა შეცდომა:""""%ОписаниеОшибки%"".'; en = 'An error occurred while printing the slip-check:""""%ОписаниеОшибки%"".'");
					...
					МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
																					 ИдентификаторУстройстваФР);
				Иначе
					ТекстСообщения = НСтр("ru = 'При подключении фискального регистратора произошла ошибка:""""%ОписаниеОшибки%"".'; ka = 'ფისკალური რეგისტრატორის შეერთებისას მოხდა შეცდომა:""""%ОписаниеОшибки%"".'; en = 'An error occurred while connecting the fiscal registrar:""""%ОписаниеОшибки%"".'");
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации()
	...
	Элементы.Контрагент.Заголовок = Нстр("ru = 'Покупатель'; ka = 'მყიდველი'; en = 'Client'");
