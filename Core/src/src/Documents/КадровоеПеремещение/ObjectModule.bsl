...
//+++ Дмгг Таск ONESOFT-2766 12.12.22
Процедура ЗаполнитьСотрудниковПоОстаткамОтпусков() Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст = 
		//"ВЫБРАТЬ
		//|	ДниОтпускаСотрудниковОстатки.Организация КАК Организация,
		//|	ДниОтпускаСотрудниковОстатки.Сотрудник КАК Сотрудник,
		//|	ДниОтпускаСотрудниковОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		//|	ДниОтпускаСотрудниковОстатки.ВидНачисленияОтпуска КАК ВидОтпуска,
		//|	ВЫБОР
		//|		КОГДА ДниОтпускаСотрудниковОстатки.ВидНачисленияОтпуска = ЗНАЧЕНИЕ(Перечисление.ВидыВариантовВидовОтпуска.КалендарныйГод)
		//|			ТОГДА ДниОтпускаСотрудниковОстатки.ВидНачисленияОтпуска.КоличествоДней - ДниОтпускаСотрудниковОстатки.КоличествоДнейОстаток
		//|		ИНАЧЕ ДниОтпускаСотрудниковОстатки.КоличествоДнейОстаток
		//|	КОНЕЦ КАК КоличествоДней, 
		////|	ДниОтпускаСотрудниковОстатки.КоличествоДнейОстаток КАК КоличествоДней,
		//|	&ТекДата КАК Период
		//|ИЗ
		//|	РегистрНакопления.ДниОтпускаСотрудников.Остатки(
		//|			&ДатаОстатков,
		//|			Организация = &Организация
		//|				И СтруктурнаяЕдиница = &СтруктурнаяЕдиница) КАК ДниОтпускаСотрудниковОстатки";
		"ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА СотрудникиСрезПоследних.ВидОтпуска.ВидВариантовВидовОтпуска = ЗНАЧЕНИЕ(Перечисление.ВидыВариантовВидовОтпуска.КалендарныйГод)
		|			ТОГДА СотрудникиСрезПоследних.ВидОтпуска.КоличествоДней + ЕСТЬNULL(ДниОтпускаСотрудниковОстатки.КоличествоДнейОстаток, 0)
		|		ИНАЧЕ ЕСТЬNULL(ДниОтпускаСотрудниковОстатки.КоличествоДнейОстаток, 0)
		|	КОНЕЦ КАК КоличествоДней,
		|	&ТекДата КАК Период
		|ИЗ
		...
		|				И ВидОтпуска.ВидВариантовВидовОтпуска <> ЗНАЧЕНИЕ(Перечисление.ВидыВариантовВидовОтпуска.ПустаяСсылка)) КАК СотрудникиСрезПоследних
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ДниОтпускаСотрудников.Остатки(
		|				&ДатаОстатков,
		|				Организация = &Организация
		|					И СтруктурнаяЕдиница = &СтруктурнаяЕдиница) КАК ДниОтпускаСотрудниковОстатки
		|		ПО (ДниОтпускаСотрудниковОстатки.Организация = ДниОтпускаСотрудниковОстатки.Организация)
		|			И СотрудникиСрезПоследних.Сотрудник = ДниОтпускаСотрудниковОстатки.Сотрудник
		|			И СотрудникиСрезПоследних.СтруктурнаяЕдиница = ДниОтпускаСотрудниковОстатки.СтруктурнаяЕдиница
		|			И СотрудникиСрезПоследних.ВидОтпуска = ДниОтпускаСотрудниковОстатки.ВидНачисленияОтпуска";
	
	Запрос.УстановитьПараметр("ДатаОстатков", ?(ЗначениеЗаполнено(Дата), Дата-1, ТекущаяДата()));
...
КонецПроцедуры // ЗаполнитьСотрудниковПоОстаткамОтпусков()
//--- Дмгг Таск ONESOFT-2766 12.12.22
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	РегистрыСведений.ТекущиеКадровыеДанныеСотрудников.ОбновитьТекущиеКадровыеДанныеСотрудников(,"КадровоеПеремещение");
