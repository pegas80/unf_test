...
Функция НачатьВыполнениеДлительнойОперацииНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания	= НСтр("ka='მონაცემთა მიღება გაყიდვების გეგმის შესავსებად';ru='Получение данных для заполнения плана продаж';en='Getting data to fill the sales plan'");
...
Процедура ДлительнаяОперацияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(
			НСтр("ka='შესავსებად მონაცემთა მიღება ვერ მოხერხდა შემდეგი მიზეზით: %1';ru='Не удалось получить данные для заполнения по причине: %1';en='Failed to get the data to populate the following reason:% 1'"),
