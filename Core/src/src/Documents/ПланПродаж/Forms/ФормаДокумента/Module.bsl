		...
		Если РеквизитыСценария.ПланироватьКоличество Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВсегоКоличество) Тогда
				...
				ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка",
					"Заполнение",
					НСтр("ka='რაოდენობა';ru='Количество';en='Количество'"),
					СтрокаТаблицы.НомерСтроки,
					Элементы.СтраницаПлан.Заголовок
				);
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ЕдиницаИзмерения) Тогда
				...
				ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка",
					"Заполнение",
					НСтр("ka='საზომი ერთეული';ru='Единица измерения';en='Unit of Measurement'"),
					СтрокаТаблицы.НомерСтроки,
					Элементы.СтраницаПлан.Заголовок
				);
...
""
"Продолжить?';ka='გეგმა შეიცავს შეყვანილ მონაცემებს."
"გაგრძელებისას, სვეტების შემადგენლობა ახალი სცენარის შესაბამისობაში იქნება მოყვანილი."
""
"გავაგრძელოთ?'");
		ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыВопроса.Заголовок = НСтр("ka='სცენარის ცვლილების დადასტურება';ru='Подтверждение изменения сценария';en='Confirm change scenario'");
...
Процедура ЗаполнитьПоПланируемымПродажам(Команда)
	...
	ОткрытьНастройкиЗаполненияПоСхеме("СКД_ПланПродаж", НСтр("ka='დაგეგმილი გაყიდვების მიხედვით შევსების გამართვები';ru='Настройки заполнения по запланированным продажам';en='Settings filling of the planned sales'"));
...
Процедура ЗаполнитьПоФактическимПродажам(Команда)
	...
	ОткрытьНастройкиЗаполненияПоСхеме("СКД_ФактПродаж", НСтр("ka='ფაქტობრივი გაყიდვების მიხედვით შევსების გამართვები';ru='Настройки заполнения по фактическим продажам';en='Settings filling of actual sales'"));
...
Процедура ПланВыполнитьДействие(Команда)
	...
	Если ПланИзменениеСтрокДействие <> ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента")
		...
		ТекстСообщения = НСтр("ka='არჩეული სტრიქონები არ არის.';ru='Нет выбранных строк.';en='None of the selected rows.'");
...
Процедура УстановитьУсловноеОформлениеХарактеристикНоменклатуры(Форма,
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст",					НСтр("ka='<характеристики не используются>';ru='<характеристики не используются>';en='<характеристики не используются>'"));
...
Процедура ОбновитьСтруктуруТаблицыПланирования()
	...
	Для Индекс = 0 По Периоды.Количество()-1 Цикл
		...
		Если РеквизитыСценария.ПланироватьКоличество Тогда
			...
			РеквизитФормы = Новый РеквизитФормы("КолонкаКоличество_" + Индекс, Число_15_3, "ТаблицаПланирования", НСтр("ka='რაოდენობა';ru='Количество';en='Количество'"), Истина);
			ДобавляемыеРеквизиты.Добавить(РеквизитФормы);
			
		КонецЕсли;
		
		РеквизитФормы = Новый РеквизитФормы("КолонкаСумма_" + Индекс, Число_15_2, "ТаблицаПланирования", НСтр("ka='თანხა';ru='Сумма';en='Сумма'"), Истина);
		ДобавляемыеРеквизиты.Добавить(РеквизитФормы);
...
Процедура ПроверитьКоличествоПериодов(ТаблицаПериодов)
	...
	Если ТаблицаПериодов.Количество() > МаксКоличествоПериодов Тогда
		...
		СохранитьЗначенияРеквизитовОбъекта(ЭтотОбъект);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='არჩეულია საკმაოდ დიდი პერიოდი. დასრულების პერიოდი შეიცვალა. ხანგრძლივი ვადით დასაგეგმად, თქვენ შეგიძლიათ გაყიდვების რამდენიმე გეგმის შეყვანა ან მეტი პერიოდულობის მქონე სცენარის არჩევა.';ru='Выбран слишком большой период. Период окончания был изменен."
...
""
"Продолжить?';ka='გეგმა შეიცავს მონაცემებს. გაგრძელებისას პერიოდების შემადგენლობა შეიცვლება."
""
"გავაგრძელოთ?'");
			ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
			ПараметрыВопроса.Заголовок = НСтр("ka='დაგეგმვის პერიოდის ცვლილების დადასტურება';ru='Подтверждение изменения периода планирования';en='Confirming the changes of the planning period'");
			ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, ПараметрыВопроса);
...
Процедура ИзменитьПометки(Форма, Пометка)
	...
	Форма.Элементы.ПланИзменитьПометки.Заголовок	= ?(Пометка, НСтр("ka='ნიშნულების მოხსნა';ru='Снять пометки';en='Unmark'"), НСтр("ka='ნიშნულების დაყენება';ru='Установить пометки';en='Установить пометки'"));
...
Функция ПредставлениеДействияГрупповогоИзменения(ДействиеПеречисление)
	...
	Если ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьЦеныНаПроцент") Тогда
		Возврат НСтр("ka='თანხების შეცვლა პროცენტზე';ru='Изменить суммы на процент';en='Change in the percentage of the sum of'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		Возврат НСтр("ka='რაოდენობის დადგენა ყველა პერიოდისთვის';ru='Установить количество для всех периодов';en='Set the number for all periods'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
		Возврат НСтр("ka='ჯამის დაყენება ყველა პერიოდისთვის';ru='Установить сумму для всех периодов';en='Set the amount for all periods'");
	КонецЕсли;
...
Функция ПредставлениеЗаголовкаОбъектаДействия(ДействиеПеречисление)
	...
	Если ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		Возврат НСтр("ka='რაოდენობა';ru='Количество';en='Количество'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
		Возврат НСтр("ka='თანხა';ru='Сумма';en='Сумма'");
	КонецЕсли;
...
Процедура УправлениеВидимостьюГрупповогоИзменения(Форма)
	...
	Иначе
		...
		Элементы.ПланИзменениеСтрокЗначение.Заголовок = НСтр("ka='მნიშვნელობა';ru='Значение';en='Value'");
