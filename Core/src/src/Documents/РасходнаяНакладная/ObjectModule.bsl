...
Процедура ЗаполнитьПредоплату() Экспорт
	...
	// Kuzmich, 11.09.2023, (ONESOFT-2547) Зачитывать авансы автоматически - заполнени по остатку задолженности
	ТЗ_Расчётов = РегистрыНакопления.РасчетыСПокупателями.Остатки(ТекущаяДата(),Новый Структура("Документ", ЭтотОбъект.Ссылка),"Документ","СуммаВал");
	...
	//|				КОНЕЦ
	//|		КОНЕЦ) КАК Курс,
	// Kuzmich, 19.09.2023, (ONESOFT-2547) Зачитывать авансы автоматически - заполнени по остатку задолженности
	...
	Пока ВыборкаРезультатаЗапроса.Следующий() Цикл
		...
		КонецЕсли;// Kuzmich, 11.09.2023, (ONESOFT-2547) Зачитывать авансы автоматически - заполнени по остатку задолженности
...
Процедура ЗаполнитьПоПриходнойНакладной(ДанныеЗаполнения, Операция = "") Экспорт
	...
	//Малышев 2025-03-25 ONESOFT-3933
	Если ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику Тогда
		ДанныеКурсов = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Дата, Новый Структура("Валюта", Договор.ВалютаРасчетов));
		Если ТипЗнч(ДанныеКурсов)=Тип("Структура") Тогда
			ДанныеКурсов.Свойство("Курс", Курс);
			ДанныеКурсов.Свойство("Кратность", Кратность);
		КонецЕсли;	
	КонецЕсли;	
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	|		ИЛИ РезервированиеЗапасов_new.Значение
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	...
	|	ЗаказПокупателя.Проект как Проект, //Малышев 2024-06-18  ONESOFT-3236 
	|	ЗаказПокупателя.ВидНалогаНДС как ВидНалогаНДС, //Малышев 2024-12-19 
	// Олег Литвиненко 01.09.22 --->
	|	ЗаказПокупателя.ЭтоПодарок КАК ЭтоПодарок,
	// Олег Литвиненко <---
	|	ВЫБОР
	...
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	|	,Константа.ФункциональнаяОпцияРезервированиеЗапасов_new КАК РезервированиеЗапасов_new
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	|ГДЕ
	...
	|	ЗаказПокупателяЗапасы.Ссылка.Проект как Проект, //Малышев 2024-06-18  ONESOFT-3236 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ИспользоватьАвтоматическиеСкидки Тогда
			//КоличествоВДокументе = Выборка.Количество * Выборка.Коэффициент;
			// Олег Литвиненко 15.09.22 --->
			КоличествоВДокументе = Окр(Выборка.Количество * Выборка.Коэффициент - СмещениеДляОкругления, 3);
			// Олег Литвиненко <---
			ПересчитатьСуммы = КоличествоВДокументе <> КоличествоКСписанию;
			КоэффициентПересчетаСкидки = ?(ПересчитатьСуммы, КоличествоКСписанию / КоличествоВДокументе, 1);
			Если КоэффициентПересчетаСкидки <> 1 Тогда
	...
	Если Запасы.Количество() > 0
		...
		// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
		И (Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() ИЛИ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить())
		// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	...
	Если Запасы.Найти(Истина, "ЭтоПодарок") <> Неопределено Тогда
		Комментарий = "უსასყიდლო რეალიზაცია";
	КонецЕсли;
	// Олег Литвиненко <---
...
Процедура ЗаполнитьПоЗаказПоставщикуНаЗакупку(ДанныеЗаполнения)
	...
	Пока Выборка.Следующий() Цикл
		...
		// Олег Литвиненко 15.09.22 --->
		ИначеЕсли Окр(МассивСтрокОстатков[0].КоличествоОстаток / Выборка.Коэффициент - СмещениеДляОкругления, 3) = Выборка.Количество Тогда
		// Олег Литвиненко <---
		...
		ИначеЕсли Окр(МассивСтрокОстатков[0].КоличествоОстаток / Выборка.Коэффициент - СмещениеДляОкругления, 3) < Выборка.Количество Тогда
			...
			// Олег Литвиненко 15.09.22 --->
			КоличествоКСписанию = -1 * Окр(МассивСтрокОстатков[0].КоличествоОстаток / Выборка.Коэффициент - СмещениеДляОкругления - Выборка.Количество, 3);
			// Олег Литвиненко <---
...
Процедура ЗаполнитьПоСборкеЗапасов(ДанныеЗаполнения) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из ТабЧастьЗаполнения Цикл
		...
		//Малышев 2025-01-31 ONESOFT-3821 ((--
		...
		СтруктураДанные.Вставить("КартаЛояльности", КартаЛояльности); //+++ Дмгг
		СтруктураДанные.Вставить("ПроцентСкидкиПоДисконтнойКарте", ПроцентСкидкиПоДисконтнойКарте);
	    СтруктураДанные = Документы.РасходнаяНакладная.ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные);
		Если НоваяСтрока.Цена = 0 и СтруктураДанные.Свойство("Цена") Тогда
			 НоваяСтрока.Цена = СтруктураДанные.Цена; 
		КонецЕсли;	

		СтруктураДанных = УправлениеНебольшойФирмойСервер.ПолучитьСуммуСтрокиТабличнойЧасти(
					Новый Структура("Количество, Цена, Сумма, ПроцентСкидкиНаценки, СтавкаНДС, СуммаНДС, СуммаВключаетНДС, Всего",
					НоваяСтрока.Количество, НоваяСтрока.Цена, 0, НоваяСтрока.ПроцентСкидкиНаценки, НоваяСтрока.СтавкаНДС, 0, СуммаВключаетНДС, 0));
...
Процедура ЗаполнитьКолонкуРезервПоРезервам() Экспорт
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
		...
		Запрос.Текст = РезервированиеЗапасов_new.ТекстЗапросаПоВременнойТаблицеЗапасов();
	...
	// © Шишка В.В. [6 октября 2021]  НАЧАЛО
	//Запрос.УстановитьПараметр("СтруктурнаяЕдиница", СтруктурнаяЕдиница);
	Запрос.УстановитьПараметр("СтруктурнаяЕдиница", ?(УчетСебестоимостиВРазрезеОрганизации, Неопределено, СтруктурнаяЕдиница));
	// © Шишка В.В. [6 октября 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого СтрокаЗапасы Из МассивСтрокЗапасы Цикл
			...
			ВсегоОстаток = ?(ТипЗнч(СтрокаЗапасы.ЕдиницаИзмерения) = ТИП("СправочникСсылка.КлассификаторЕдиницИзмерения"), ВсегоОстаток, Окр(ВсегоОстаток / СтрокаЗапасы.ЕдиницаИзмерения.Коэффициент - СмещениеДляОкругления,3));
			// Олег Литвиненко <---
			...
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
				СтрокаЗапасы.Проект = Выборка.Проект;
			КонецЕсли;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
		КонецЦикла;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//giga
	Попытка
		ПроверятьОтветственный = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ОбязательноеПолеОтветственный");
		Если ПроверятьОтветственный И ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			Если ЗначениеЗаполнено(ДанныеЗаполнения.Ответственный) Тогда
				ЭтотОбъект.Ответственный = ДанныеЗаполнения.Ответственный;
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("მყიდველის შეკვეთაში არ არის შევსებული 'პასუხისმგებელი'");
			КонецЕсли;
		КонецЕсли;
	Исключение
	КонецПопытки;	
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.КомПредложение") Тогда
		...
		// Олег Литвиненко 01.09.22 --->
		ЭтоПодарок = ДанныеЗаполнения.ЭтоПодарок;
		// Олег Литвиненко <---
	...
	//+++ Lazare 26.10.2021
	ИначеЕсли УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ЗаполнитьОтветственногоИзКонтрагента")
		и ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда 
		ЭтотОбъект.Ответственный  = ДанныеЗаполнения.Контрагент.Ответственный ;     
	//--- Lazare_26.10.2021
	
	КонецЕсли;
	
	//Малышев 2024-10-14 ONESOFT-3508
	Если ЗначениеЗаполнено(ДокументОснование) и ДокументОснование.Метаданные().Реквизиты.Найти("СуммаДокумента") <>Неопределено ТОгда	
		ОбщаяСуммаКОплате = ДокументОснование.СуммаДокумента;
	КонецЕсли;	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Проведен  и РежимЗаписи = РежимЗаписиДокумента.Проведение   Тогда 
		АвтоматическоеПроизводствоСервер.СоздатьДокументПроизводстваБезПроверкиОстатков(ЭтотОбъект);
	КонецЕсли;
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда 
		Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияРасходныхНакладныхПоСтатусеТТН") Тогда    	
			Если ОбщегоНазначения.ЕстьТТН(Ссылка) Тогда
				Сообщить(НСтр("ru = 'Пользователю запрещено отменять проведение документа. На этом документе есть активная ТТН на портале RS.GE'; ka = 'მომხარებლისთვის აკრძალულია გატარებული დოკუმენტის გაუქმება!მოცემულ დოკუმენტზე Rs.Ge-ზე  არსებობს  აქტიური ან დასრულებული  ზედნადები'"));
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	Если Константы.ФункциональнаяОпцияИспользоватьКартыЛояльности.Получить() Тогда
		ВсегоИспользованоБаллов	= Запасы.Итог("БалыРасход");
	Иначе
		ВсегоИспользованоБаллов	= 0;
	КонецЕсли;	
	ОбщаяСуммаКОплате = СуммаДокумента - ВсегоИспользованоБаллов;
	//Малышев 2024-10-14 ONESOFT-3508 --)
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//Малышев 2024-12-10 ONESOFT-3649
	Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС ТОгда
		ПроверяемыеРеквизиты.Удалить(ПроверяемыеРеквизиты.Найти("Запасы.СтавкаНДС"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(КартаЛояльности) И (ВидИспользоыванияБалов = Перечисления.ВидыИспользованияБалов.НакоплениеИРасход ИЛИ ВидИспользоыванияБалов = Перечисления.ВидыИспользованияБалов.Расход) Тогда 
		
		//Малышев 2024-11-05 ONESOFT-3574
		ОстатокДоступноБалов = БаллыЛояльностьСкидки.ПолучитьКоличествоДоступныхБалов_РасходнаяНакладная(Ссылка, КартаЛояльности, Контрагент, Дата, КартаЛояльности.Владелец.КоэфициентПересчета, Ложь);
		//ВсегоРасходованоБалов = Запасы.Итог("БалыРасход");
		//Если ДоступноБалов < ВсегоРасходованоБалов Тогда 
		
		Если ОстатокДоступноБалов < ВсегоИспользованоБаллов Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='Расходовано больше балов чем доступно!';ru='Расходовано больше балов чем доступно!';en='Расходовано больше балов чем доступно!'"),
					КартаЛояльности,
					"КартаЛояльности",,
					Отказ
				);
	...
	Если ЗначениеЗаполнено(КартаЛояльности) И (ВидИспользоыванияБалов = Перечисления.ВидыИспользованияБалов.НакоплениеИРасход ИЛИ ВидИспользоыванияБалов = Перечисления.ВидыИспользованияБалов.Расход) Тогда 
		ВсегоРасходованоБалов = Запасы.Итог("БалыРасход");
		СуммовоеВыражениеБалов = ВсегоРасходованоБалов * КартаЛояльности.Владелец.КоэфициентПересчета;
		СуммаДокумента = Запасы.Итог("Всего");
		Если СуммовоеВыражениеБалов > СуммаДокумента Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='Суммовое выражение балов больше уммы документа!';ru='Суммовое выражение балов больше уммы документа!';en='Суммовое выражение балов больше уммы документа!'"),
					КартаЛояльности,
					"КартаЛояльности",,
					Отказ
				);
	...
	Если ЗначениеЗаполнено(КартаЛояльности) Тогда 
		...
		Запрос.Текст = 
			...
			|	КартыЛояльности.ДатаНачала КАК ДатаОкончания
			|ПОМЕСТИТЬ ВременнаяТаблицаОконсаниеСрокаКрты
			|ИЗ
			...
			|	БалыЛояльностиОбороты.Период
			|ИЗ
			|	РегистрНакопления.БалыЛояльности.Обороты(, , Запись, КартаЛояльности = &КартаЛояльности) КАК БалыЛояльностиОбороты
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(МАКСИМУМ(ВременнаяТаблицаОконсаниеСрокаКрты.ДатаОкончания), ГОД, 1), ДЕНЬ) КАК ДатаОкончания,
			|	ВременнаяТаблицаОконсаниеСрокаКрты.Ссылка КАК Ссылка
			|ИЗ
			|	ВременнаяТаблицаОконсаниеСрокаКрты КАК ВременнаяТаблицаОконсаниеСрокаКрты
			|
			|СГРУППИРОВАТЬ ПО
			|	ВременнаяТаблицаОконсаниеСрокаКрты.Ссылка";
		...
		Если тзРезультатЗапроса.Количество() > 0 Тогда 
			ДатаОкончанияДействия = тзРезультатЗапроса[0].ДатаОкончания;
		КонецЕсли;
	
		Если ДатаОкончанияДействия < Дата Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='ბარათის გამოყენება შეზღუდულია გამოუყენებლობის გამო!';ru='Карта более не действительна!';en='Карта более не действительна!'"),
					КартаЛояльности,
					"КартаЛояльности",,
					Отказ
				);
	...
	//giga
	ПроверятьОтветственный = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ОбязательноеПолеОтветственный");
	Если ПроверятьОтветственный Тогда
	...
	Если ПолучитьФункциональнуюОпцию("РасчетАрендыКвартир") И (Договор.РасчетАрендыКвартир ИЛИ АрендаКвартир) Тогда
	
		ПроверяемыеРеквизиты.Добавить("ВладелецКвартиры");
		ПроверяемыеРеквизиты.Добавить("ДоговорВладельцаКвартиры");
		ПроверяемыеРеквизиты.Добавить("ПроцентАрендыКвартиры");
		ПроверяемыеРеквизиты.Добавить("НоменклатураАрендыКвартиры"); 
		ПроверяемыеРеквизиты.Добавить("ВалютаАрендыКвартир");
		ПроверяемыеРеквизиты.Добавить("СуммаАрендыЗаКвартиру");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Олег Литвиненко 30.12.22 --->
	УправлениеНебольшойФирмойСервер.ОтразитьЗапасыВВалюте(ДополнительныеСвойства, Движения, Отказ);
	// Олег Литвиненко <---
	// Олег Литвиненко 09.01.23 --->
	УправлениеНебольшойФирмойСервер.ОтразитьПродажиВВалюте(ДополнительныеСвойства, Движения, Отказ);
	// Олег Литвиненко <---
	...
	УправлениеНебольшойФирмойСервер.ОтразитьБалыЛояльности(ДополнительныеСвойства, Движения, Отказ); //+++ Дмгг
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВозвратОтПокупателяDG") Тогда 
		УправлениеНебольшойФирмойСервер.ОтразитьТаблицаВозвратиDG(ДополнительныеСвойства, Движения, Отказ,Контрагент);
	КонецЕсли;
	...
	Если  ДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара
		И ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю  Тогда 
		СформироватьТаблицаКонтрольСроковГодностиТовара(Ссылка,ДополнительныеСвойства);//++Lazare  19.10.2022
	...
	//+++ Lazare_15.10.2021
	//Если Константы.ФункционалСбораТоваров.Получить() и не Заказ = ПредопределенноеЗначение("Документ.ЗаказПокупателя.ПустаяСсылка")  
		//и не Заказ.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Доставлен
		//Тогда 
	// Олег Литвиненко 25.11.22 --->
	Если Константы.ФункционалСбораТоваров.Получить() И ЗначениеЗаполнено(Заказ) И ТипЗнч(Заказ)=Тип("ДокументСсылка.ЗаказПокупателя")  Тогда
		ДокОб = Заказ.ПолучитьОбъект();
		Если Не ДокОб.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Доставлен Тогда
	// Олег Литвиненко <---
			//ДокОб = Заказ.ПолучитьОбъект();
			Если ДокОб.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Собран Или ДокОб.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.ВыгруженНаБорт Тогда
				ДокОб.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Доставлен;
	...
	//++Rajak
	  ЗаполнитьЗапасыУКлиента();	
	//--Rajak

	// Kuzmich, 31.03.2024, (ONESOFT-2527) Оплата аренды квартиры и обработка банковских выписок
	Если НЕ Отказ Тогда
		ОбработатьРасчетАрендыКвартир();
	КонецЕсли;
	// Kuzmich, 31.03.2024, (ONESOFT-2527) Оплата аренды квартиры и обработка банковских выписок
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	//Малышев 2024-06-19   ONESOFT-3227 - закоментировал
	// Контроль возникновения отрицательного остатка.
	//Документы.РасходнаяНакладная.ВыполнитьКонтроль(Ссылка, ДополнительныеСвойства, Отказ, Истина);
...
Процедура ЗаполнитьЗапасыУКлиента()
...
Процедура СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	"ВЫБРАТЬ
	 ...
	 |		ЛЕВОЕ СОЕДИНЕНИЕ ВтПродажи КАК ВтПродажи
	 |		ПО КонтрольСроковГодностиТовараОбороты.СтруктурнаяЕдиница = ВтПродажи.СтруктурнаяЕдиница
	 |			И КонтрольСроковГодностиТовараОбороты.Организация = ВтПродажи.Организация
	 |ГДЕ
	 ...
	 Запрос.УстановитьПараметр("Регистратор",ДокументСсылкаРасходнаяНакладная);
...
Процедура ОбработатьРасчетАрендыКвартир()

	РасчетАрендыКвартирСервер.ЗаполнитьПриходПоРасходнойНакладной(ЭтотОбъект.Ссылка);
