...
Процедура СформироватьТаблицаЗапасыПродажа(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если Константы.ПодборЦеныДляОтрицательныхОстатков.Получить() Тогда
		...
		Пока ВыборкаДетальныеЗаписиОтрицательныеОстатки.Следующий() Цикл
			РЕЗ = РозницаHTTP.ПолучитьОстаткиТовараПоБухСчетам(ВыборкаДетальныеЗаписиОтрицательныеОстатки.Номенклатура,ВыборкаДетальныеЗаписиОтрицательныеОстатки.СтруктурнаяЕдиница,ДокументСсылкаРасходнаяНакладная.Дата);
...
//+++ Дмгг
#Область КартыЛояльности

// Формирует таблицу значений, содержащую данные для проведения по регистру БалыЛояльности.
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаБалыЛояльности(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаБалыЛояльности.КартаЛояльности,
	|	ВременнаяТаблицаБалыЛояльности.Контрагент,
	|	СУММА(-ВременнаяТаблицаБалыЛояльности.Сумма),
