...
Процедура ЗаполнитьПоЗаказуПоставщику(ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения.Ссылка)
		...
		ПоДокументу = НСтр("ka='მყიდველის გადასახდელი ანგარიში #';ru='Счет на оплату покупателя №';en='The expense of the buyer payment number'") + СокрЛП(ДанныеЗаполнения.Ссылка.НомерВходящегоДокумента) + НСтр("ru=' от ';ka='ვისგან'") + СокрЛП(ДанныеЗаполнения.Ссылка.ДатаВходящегоДокумента);
