...
Процедура ЗаполнитьРасшифровкуПлатежа() Экспорт
	...
	//Kuzmich, 08.09.2023, (ONESOFT-2544) Расход из кассы - Поставшику
	|	ГДЕ
...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Безналичные Тогда
		ВызватьИсключение НСтр("ka='ფულის ხარჯის შეყვანა არ შეიძლება. განაცხადში გადახდის არასწორი მეთოდი (ფულადი სახსრების ტიპი) არის მითითებული!';ru='Нельзя ввести расход денег. В заявке указан неверный способ оплаты (тип денежных средств)!';en='It is impossible to enter the flow of money. The request has an invalid method of payment (cash type)!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	//Паганини: Скоректированн запрос. Добавлен "ТипНДФЛ". Скоректированны "СуммаПлатежа", "СуммаРасчетов" таблицы расходов;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫБОР
...
Процедура ЗаполнитьПоДополнительныеРасходы(ДокументСсылкаДополнительныеРасходы) Экспорт
	...
	|	КОНЕЦ КАК Документ,
	//Паганини ПФ_
	//|	СУММА(ЕСТЬNULL(ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаДокумента.Всего - ТаблицаДокумента.СуммаНДФЛ, 0) * ВЫБОР
	|	СУММА(ЕСТЬNULL(ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаДокумента.Всего - ТаблицаДокумента.СуммаНДФЛ - ТаблицаДокумента.СуммаПФ, 0) * ВЫБОР
	//_Паганини ПФ
...
Процедура ЗаполнитьПоПлатежнаяВедомость(ДокументСсылкаПлатежнаяВедомость) Экспорт
	...
	Если УчетПоПроектам Тогда
		...
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.Ссылка.ВалютаДокумента = ТаблицаДокумента.Ссылка.Организация.КассаПоУмолчанию.ВалютаПоУмолчанию
		|			ТОГДА ТаблицаДокумента.Ссылка.Организация.КассаПоУмолчанию
		|		ИНАЧЕ ВложенныйЗапрос.Касса
		|	КОНЕЦ КАК Касса,
		...
		|				ПО ТаблицаДокумента.ВалютаДокумента = Кассы.ВалютаПоУмолчанию
		|		ГДЕ
		|			ТаблицаДокумента.Ссылка = &Ссылка
		|			И Кассы.ПометкаУдаления = ЛОЖЬ) КАК ВложенныйЗапрос
		|		ПО ТаблицаДокумента.Ссылка.ВалютаДокумента = ВложенныйЗапрос.ВалютаПоУмолчанию
		|ГДЕ
		|	ТаблицаДокумента.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	&Ссылка КАК Ведомость,
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.ВалютаДокумента = ТаблицаДокумента.Организация.КассаПоУмолчанию.ВалютаПоУмолчанию
		|			ТОГДА ТаблицаДокумента.Организация.КассаПоУмолчанию
		|		ИНАЧЕ ВложенныйЗапрос.Касса
		|	КОНЕЦ КАК Касса,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 1
		|			Кассы.Ссылка КАК Касса,
		|			Кассы.ВалютаПоУмолчанию КАК ВалютаДенежныхСредств
		|		ИЗ
		|			Документ.ПлатежнаяВедомость КАК ТаблицаДокумента
		|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Кассы КАК Кассы
		|				ПО ТаблицаДокумента.ВалютаДокумента = Кассы.ВалютаПоУмолчанию
	...
	//ПодольскийАИ #УчетПроектов 12.02.2021 }
		...
		|	ТаблицаДокумента.ВалютаДокумента<>ТаблицаДокумента.ВалютаРасчетов как НужнаКонвертация,	 //Малышев 2024-11-25 ONESOFT-3523
		|	ТаблицаДокумента.ВалютаДокумента КАК ВалютаДенежныхСредств
		|ИЗ
		|	Документ.ПлатежнаяВедомость КАК ТаблицаДокумента
		|ГДЕ
		...
		//Малышев 2024-11-25 ONESOFT-3523 (--
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	Если ДокументСсылкаНачислениеНалогов.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
		ВызватьИсключение НСтр("ka='სალაროდან გასავლის შეყვანა შეიძლება მხოლოდ გადასახადების დარიცხვის, და არა ანაზღაურების საფუძველზე.';ru='Расход из кассы можно ввести только на основании начисления налогов, а не возмещения.';en='Flow from the cash register, you can enter only on the basis of tax assessment, rather than compensation.'");
	КонецЕсли;
...
Процедура ЗаполнитьПоЗакупкаЛицензии(ДокументСсылкаЗакупкаЛицензии) Экспорт
	...
	|	ЗакупкаЛицензий.Организация.КассаПоУмолчанию КАК Касса,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	...
	|	ЕСТЬNULL(ВложенныйЗапрос.СуммаРасчетов, 0) КАК СуммаРасчетов,
	|	ЕСТЬNULL(ВложенныйЗапрос.СуммаПлатежа, 0) КАК СуммаПлатежа
	|ИЗ
	...
	|	ЗакупкаЛицензий.Организация.КассаПоУмолчанию,
	|	ЗакупкаЛицензий.Контрагент,
	|	ЗакупкаЛицензий.ВалютаДокумента,
	|	ЗакупкаЛицензий.Договор,
	|	ВложенныйЗапрос.Документ,
	|	ЕСТЬNULL(ВложенныйЗапрос.СуммаРасчетов, 0),
	|	ЕСТЬNULL(ВложенныйЗапрос.СуммаПлатежа, 0)";
...
//--------------------------------------------------------------------------
// © Шишка В.В. [28 декабря 2023]  НАЧАЛО
...
// © Шишка В.В. [28 декабря 2023]  КОНЕЦ
//--------------------------------------------------------------------------
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//Паганини_
	Если (ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная") И ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика) ИЛИ 
		  ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ДополнительныеРасходы") Тогда
		Если (ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная") И НЕ ДанныеЗаполнения.Проведен И 
			  ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика) ИЛИ
			 (ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ДополнительныеРасходы") И НЕ ДанныеЗаполнения.Проведен) Тогда
			ТекстОшибки = НСтр("ka='Нельзя создать документ на основании непроведенного документа.';ru='Нельзя создать документ на основании непроведенного документа.';en='You can not create a document based on the failure to document.'");
		...
		Запрос.Текст = 
			...
			|				И Договор = &Договор
			|				И ВЫБОР
			|					КОГДА Контрагент.ВестиРасчетыПоДокументам
			|						ТОГДА Документ = &Документ
			|					ИНАЧЕ 1 = 1
			|				КОНЕЦ) КАК РасчетыСПоставщикамиОстатки";
		...
		РезультатЗапроса = Запрос.Выполнить();
		Если РезультатЗапроса.Пустой() Тогда
			ТекстОшибки = НСтр("ka='На основании текущего документа уже проезведена оплата полностью.';ru='На основании текущего документа уже проезведена оплата полностью.';en='On the basis of the current document already proezvedena payment completely.'");
		  	ВызватьИсключение ТекстОшибки;
		Иначе
			Выборка = РезультатЗапроса.Выбрать();
			Выборка.Следующий();
			Если Выборка.СуммаОстаток < 0 ИЛИ Выборка.СуммаВалОстаток < 0 Тогда
				ТекстОшибки = НСтр("ka='На основании текущего документа уже проезведена оплата полностью.';ru='На основании текущего документа уже проезведена оплата полностью.';en='On the basis of the current document already proezvedena payment completely.'");
			  	ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		КонецЕсли;		
	...
	СтратегияЗаполнения[Тип("ДокументСсылка.ЗакупкаЛицензий")] = "ЗаполнитьПоЗакупкаЛицензии"; //+Lazare 01.11.2022
	//--------------------------------------------------------------------------
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИспользоватьТолькоРегламентированныйПланСчетов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ИспользованиеТолькоРегламентированногоПланаСчетов(Дата); //+++ Дмгг
	...
	Иначе
		Если ПолучитьФункциональнуюОпцию("ИспользоватьБиллинг")
			...
			Если Выборка.Следующий() Тогда
				Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Выборка.Ссылка, КорреспонденцияРегл) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მომსახურების მიმდინრე ხელშეკრულების მიხედვით დაუგეგმავი ხარჯების გატარება აკრძალულია!';ru='Запрещено проводить незапланированные расходы по текущему договору обслуживания!';en='It is forbidden to carry out unplanned maintenance costs of the current contract!'"),,
					"КорреспонденцияРегл",,
					Отказ
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимНаправлениямДеятельности.Получить()
		  И КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
	...
	КонецЕсли;
		...
		Если ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ВзносНаличнымиВБанк 
		 И Не ЗначениеЗаполнено(КорреспонденцияРегл) Тогда
		//Если РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(Дата,УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(Организация)) Тогда
			КорреспонденцияРегл = СчетОрганизации.СчетУчетаРегл;
		//КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ДД
	Если ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Зарплата ИЛИ
		ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику Тогда
...
Процедура ЗаполнитьПоДоговоруКредитаЗайма(ДокументСсылкаДоговорКредитаИЗайма) Экспорт
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту)
