...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФормаСоздана = Истина;
	ПодразделениеОбязательно = ИСТИНА; // Kuzmich, 27.05.2020, Для условного оформления
	...
	Если МобильноеПриложениеВызовСервера.НужноОграничитьФункциональность() Тогда
		...
		КонецЕсли;
		//+++ Дмгг
		тзУчетнаяПолитика = РегистрыСведений.УчетнаяПолитикаРегл.СрезПоследних(ДатаДокумента, Новый Структура("Организация", Объект.Организация));
		СчетПрочиеРасходыРегл = ?(тзУчетнаяПолитика.Количество() <> 0, тзУчетнаяПолитика[0].СчетПрочихРасходов, ПланыСчетов.Хозрасчетный.ПустаяСсылка());
		Объект.КорреспонденцияРегл = СчетПрочиеРасходыРегл;
		//--- Дмгг
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	// Kuzmich, 06.08.2019, Использовать товарный чек (Поступление в кассу, Расход из кассы
	Если ТоварныйЧекНеУникален() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Товарный чек не уникален! ';ka='ჩგდ უნიკალური არ არის!'");
		Сообщение.Поле = "Объект.ТоварныйЧек";
		Сообщение.Сообщить();
		
		Отказ = ИСТИНА;
	Иначе
		Если ДипазонТоварныхЧековОшибка() Тогда
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Товарный чек вне дипазона! ';ka='ჩგდ დიაპაზონის მიღმა!'");
			Сообщение.Поле = "Объект.ТоварныйЧек";
			Сообщение.Сообщить();
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаЗаймаСотруднику") Или
		ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту") Или
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	УстановитьВидимостьИДоступность(ИСТИНА); // Kuzmich, 06.08.2019, Использовать товарный чек (Поступление в кассу, Расход из кассы
	
	//+++ Дмгг
	Объект.ПериодРегистрации = ПолучитьПериодРегистрацииИзВедомости(Объект.ПериодРегистрации, Объект.ДокументОснование, Объект.ВидОперации);
	ЭтотОбъект.Объект.ПериодРегистрации = Объект.ПериодРегистрации;
	ЭтотОбъект.ОтображениеПериодаРегистрации 	= Формат(ЭтотОбъект.Объект.ПериодРегистрации, "ДФ='MMMM yyyy'");
	//--- Дмгг
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.Дата <> ДатаПередИзменением Тогда
		...
		ТекстСообщения = НСтр("ka='შეიცვალა სალაროს ვალუტის კურსი. გადავთვალოთ დოკუმენტის თანხები?';ru='Изменился курс валюты кассы. Пересчитать суммы документа?';en='I changed the exchange rate of the cash register. Calculate the sum of the document?'");
...
Процедура ВалютаДенежныхСредствПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.Зарплата") Тогда
		ТекстСообщения = НСтр("ka='სალაროს ვალუტა შეიცვალა. სია ""საგადახდო უწყისები"" გასუფთავდება.';ru='Изменилась валюта кассы. Список ""Платежные ведомости"" будет очищен.';en='Change currency cash. List of ""Payroll"" will be cleared.'");
...
Процедура ВалютаДенежныхСредствПриИзмененииФрагмент()
	...
	ТекстСообщения = НСтр("ka='გადავთვალოთ დოკუმენტის თანხები?';ru='Пересчитать суммы документа?';en='Calculate the sum of the document?'");
	ПересчитатьСуммыПриИзмененииКурсаВалютыДенежныхСредств(СтруктураДанные, ТекстСообщения);
...
Процедура ПодотчетникПриИзменении(Элемент)
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [30 августа 2022]  НАЧАЛО
	, Объект.Организация	
	// © Шишка В.В. [30 августа 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
	);
	...
	Объект.ПоДокументу =
	  ...
	  //--------------------------------------------------------------------------
	  // © Шишка В.В. [30 августа 2022]  НАЧАЛО
	  ЗаполнитьЗначенияСвойств(Объект, СтруктураДанные, "ВидУдержанияНДФЛ, ВидУдержанияПФ, Подразделение"); 	
	  // © Шишка В.В. [30 августа 2022]  КОНЕЦ
	  //--------------------------------------------------------------------------
...
Процедура КассаПриИзменении(Элемент)
	...
	Объект.ВалютаДенежныхСредств = ПолучитьВалютуПоУмолчаниюКассыНаСервере(Объект.Касса);//++Lazare 10.01.2023
...
Процедура СуммаДокументаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если Объект.ВидОперации = ВидОперацииПоставщику Тогда
		...
		Если ОстатокДенежныхСредств > 0 И (ОстатокДенежныхСредств) <> Объект.СуммаДокумента Тогда
			Элемент.СписокВыбора.Добавить(ОстатокДенежныхСредств, ""+(ОстатокДенежныхСредств)+НСтр("ka='(ნაშთი სალაროში)';ru=' (остаток в кассе)'"));
		КонецЕсли;
...
Процедура Подбор(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='თავდაპირველად, მიუთითეთ ვალუტა!';ru='Укажите вначале валюту!';en='Enter the first currency!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодборЗавершение(Результат1, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.OK Тогда
		...
		Для каждого СтрокаРасшифровкаПлатежа Из Объект.РасшифровкаПлатежа Цикл
			...
			СтрокаРасшифровкаПлатежа.СтавкаНДС = СтруктураСтавок.СтавкаНДС;
			//--- Дмгг
...
Процедура НапечататьЧек(Команда)
	...
	Если Объект.НомерЧекаККМ <> 0 Тогда
		ТекстСообщения = НСтр("ka='ჩეკი უკვე ამორტყმულია მოწყობილობაზე!';ru='Чек уже пробит на оборудовании!';en='Check the equipment already broken!'");
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		МенеджерОборудованияКлиент.ПредложитьВыбратьУстройство(ОписаниеОповещения, МассивОборудования,
				НСтр("ka='აღჭურვილობის აღჩევა';ru='Выберите оборудование';en='Select equipment'"), НСтр("ka='მოწყობილობა არ არის შეერთებული.';ru='Оборудование не подключено.';en='The equipment is not connected.'"));
		
	Иначе
		
		ТекстСообщения = НСтр("ka='წინასწარ აუცილებელია მიმდინარე სეანსის გარე მოწყობილობის სამუშაო ადგილის არჩევა.';ru='Предварительно необходимо выбрать рабочее место внешнего оборудования текущего сеанса.';en='You must first select the workspace external current session equipment.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ДополнитьАвансамиИОплатамиПоТаблицеДокумента(ДокументОбъект, ОбщиеПараметры)
	...
	НаименованиеАванса = 
	НСтр("ka='Выдать: ';ru='Выдать: '")
	+ ДокументОбъект.Выдать
	+ НСтр("ka=', Основание: ';ru=', Основание: '")
...
Функция ПодготовитьДанныеДляПробитияЧека(ИдентификаторУстройстваФР, ЭтоВозврат) 
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		ТекстСообщения = НСтр("ka='Не найдена касса ККМ соответсвующая устройству %устройство%.';ru='Не найдена касса ККМ соответсвующая устройству %устройство%.';en='Found cash register POS Given suitable device% unit%.'");
...
Процедура ПодключенияФискальногоРегистратораЗавершение(ИдентификаторУстройстваФР, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваФР = Неопределено Тогда
		ТекстСообщения = НСтр("ka='ჩეკების ამოსაბეჭდი მოწყობილობა არ არის არჩეული.';ru='Устройство для печати чеков не выбрано.';en='A device for printing checks is selected.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
"%ДополнительноеОписание%';ka='ჩეკის ბეჭდვისას შეცდომა მოხდა."
"ჩეკი არ დაიბეჭდა ფისკალურ რეგისტრატორზე."
"დამატებითი აღწერა:"
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.Зарплата") Тогда
			Для каждого СтрокаТабличнойЧасти Из Объект.ВыплатаЗаработнойПлаты Цикл
				...
				Если ДанныеВедомости.ВалютаДокумента = Объект.ВалютаДенежныхСредств
					...
					НоваяСуммаДокумента = НоваяСуммаДокумента + СтрокаТабличнойЧасти.СуммаПлатежа;
				КонецЕсли; 
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Контрагент, Организация, Дата)
	...
	// Прочие расчеты
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.РасчетыПоКредитам
		Или Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту Тогда
...
Функция ПолучитьДанныеПодотчетникПриИзменении(Подотчетник, Дата
	...
	|		ПО ДокументыФизическихЛицСрезПоследних.Физлицо = СотрудникиСрезПоследних.Сотрудник.Физлицо
	|ГДЕ
	|	ДокументыФизическихЛицСрезПоследних.Физлицо = &Подотчетник
	|	И ДокументыФизическихЛицСрезПоследних.ЯвляетсяДокументомУдостоверяющимЛичность";	
	
	СтруктураДанные.Вставить("ВидУдержанияНДФЛ");
	СтруктураДанные.Вставить("ВидУдержанияПФ");
	СтруктураДанные.Вставить("Подразделение");
	Запрос.УстановитьПараметр("Сотрудник", Подотчетник);
	...
	Если ВыборкаРезультатаЗапроса.Следующий() Тогда
		...
		//--------------------------------------------------------------------------
		// © Шишка В.В. [30 августа 2022]  НАЧАЛО
		ЗаполнитьЗначенияСвойств(СтруктураДанные, ВыборкаРезультатаЗапроса, "ВидУдержанияНДФЛ, ВидУдержанияПФ, Подразделение");	
		// © Шишка В.В. [30 августа 2022]  КОНЕЦ
		//--------------------------------------------------------------------------
...
Функция ПолучитьДанныеРасшифровкаПлатежаДоговорПриИзменении(Дата, Договор, ДокументПланирования = Неопределено, СтатьяДДС = Неопределено)
	...
	Если ЗначениеЗаполнено(Договор) И ТипЗнч(Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		ДанныеДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ВалютаРасчетов,СтатьяДДСПоУмолчанию,Проект");
	Иначе
		ДанныеДоговора = Новый Структура("ВалютаРасчетов,СтатьяДДСПоУмолчанию,Проект",
	...
	СтруктураДанные.Вставить(
		...
		РегистрыСведений.КурсыВалют.ПолучитьПоследнее(
			Дата,
	//ПодольскийАИ #УчетПроектов 08.09.2021 { Оптимизация кода
	//Было---
	...
	//ПодольскийАИ #УчетПроектов 08.09.2021 {
	Если ЗначениеЗаполнено(Договор) И ТипЗнч(Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда 
		СтруктураДанные.Вставить("Проект", ДанныеДоговора.Проект);
	КонецЕсли;
	//ПодольскийАИ #УчетПроектов 08.09.2021 }
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику") Тогда
		...
		Элементы.Подотчетник.Видимость = Истина;
		Элементы.Подотчетник.Заголовок = НСТР("ka='თანამშრომელი';ru='Сотрудник';en='Employee'");
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту") Тогда
...
Процедура ОбработатьИзменениеДоговораКонтрагента(СтрокаТабличнойЧасти = Неопределено)
	...
	ЗаполнитьКолонкуУчитыватьНДФЛ(?(ТекСтрока = Неопределено, СтрокаТабличнойЧасти.ПолучитьИдентификатор(), ТекСтрока), "ИзменениеДоговора");
	// © Шишка В.В. [19 июля 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
...
Процедура ОбновитьКурсыРасчетовРасшифровкиПлатежаНаСервере()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.РасшифровкаПлатежа Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор)  и Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.РасчетыПоКредитам") Тогда
			СтруктураДанные = ПолучитьДанныеРасшифровкаПлатежаДоговорПриИзменении(
				Объект.Дата,
				СтрокаТабличнойЧасти.Договор
			);
...
Процедура ЗаполнитьДолги(ПересчитыватьСлужебныеДанные)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасчетыНДФЛОстаткиИОбороты.Организация КАК Организация,
	|	РасчетыНДФЛОстаткиИОбороты.ВидНачисленияУдержания КАК ВидУдержанияНДФЛ,
	|	РасчетыНДФЛОстаткиИОбороты.Сотрудник КАК Контрагент,
	|	РасчетыНДФЛОстаткиИОбороты.Валюта КАК Валюта,
	|	СУММА(РасчетыНДФЛОстаткиИОбороты.СуммаОборот) КАК СуммаНДФЛ,
	|	СУММА(РасчетыНДФЛОстаткиИОбороты.СуммаВалОборот) КАК СуммаВалНДФЛ,
	|	РасчетыНДФЛОстаткиИОбороты.Регистратор КАК Регистратор,
	|	0 КАК СуммаПФ,
	|	0 КАК СуммаВалПФ
	|ПОМЕСТИТЬ ВременнаяТаблицаНДФЛ_ПФ
	|ИЗ
	|	РегистрНакопления.РасчетыНДФЛ.ОстаткиИОбороты(, , Запись, , ) КАК РасчетыНДФЛОстаткиИОбороты
	|ГДЕ
	|	РасчетыНДФЛОстаткиИОбороты.Регистратор В
	|			(ВЫБРАТЬ
	|				а.Документ
	|			ИЗ
	|				РасчетыСПоставщикамиОстатки КАК а)
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыНДФЛОстаткиИОбороты.Организация,
	|	РасчетыНДФЛОстаткиИОбороты.ВидНачисленияУдержания,
	|	РасчетыНДФЛОстаткиИОбороты.Сотрудник,
	|	РасчетыНДФЛОстаткиИОбороты.Валюта,
	|	РасчетыНДФЛОстаткиИОбороты.Регистратор
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РасчетыНДФЛОстаткиИОбороты.Организация,
	|	ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка),
	|	РасчетыНДФЛОстаткиИОбороты.Сотрудник,
	|	РасчетыНДФЛОстаткиИОбороты.Валюта,
	|	0,
	|	0,
	...
	|
	|ВЫБРАТЬ
	|	РасчетыСПенсионнымФондомОстаткиИОбороты.Организация,
	|	РасчетыСПенсионнымФондомОстаткиИОбороты.ВидНачисленияУдержания,
	|	РасчетыСПенсионнымФондомОстаткиИОбороты.Плательщик,
	|	РасчетыСПенсионнымФондомОстаткиИОбороты.Валюта,
	|	0,
	|	0,
	|	РасчетыСПенсионнымФондомОстаткиИОбороты.Регистратор,
	|	СУММА(РасчетыСПенсионнымФондомОстаткиИОбороты.СуммаОборот),
	|	СУММА(РасчетыСПенсионнымФондомОстаткиИОбороты.СуммаВалОборот)
	|ИЗ
	|	РегистрНакопления.РасчетыСПенсионнымФондом.ОстаткиИОбороты(, , Запись, , ) КАК РасчетыСПенсионнымФондомОстаткиИОбороты
	|ГДЕ
	|	(РасчетыСПенсионнымФондомОстаткиИОбороты.Регистратор, РасчетыСПенсионнымФондомОстаткиИОбороты.Плательщик) В
	|			(ВЫБРАТЬ
	|				а.Документ,
	|				а.Контрагент
	|			ИЗ
	...
	|	ВременнаяТаблицаДолгов.Заказ.Дата КАК ДатаЗаказа,
	|	ВЫБОР
	//Малышев 2024-07-03 ONESOFT-3234  ((--
	//|		КОГДА ВременнаяТаблицаНДФЛ_ПФ.ВидУдержанияНДФЛ = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	|		КОГДА ISNULL(ВременнаяТаблицаНДФЛ_ПФ.ВидУдержанияНДФЛ,ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	//Малышев 2024-07-03 ONESOFT-3234   --))
	|			ТОГДА ВременнаяТаблицаДолгов.ДолгВалДоговора
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДолгВалДоговора,
	|	ВременнаяТаблицаДолгов.ДокументДата КАК ДатаДокумента,
	...
	|		КОГДА ISNULL(ВременнаяТаблицаНДФЛ_ПФ.ВидУдержанияНДФЛ,ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка) 
	//Малышев 2024-07-03 ONESOFT-3234  --)
	|			ТОГДА ВременнаяТаблицаДолгов.ДолгВалДоговора * ВременнаяТаблицаДолгов.Курс * КурсыВалютСрезПоследних.Кратность / (ВременнаяТаблицаДолгов.Кратность * КурсыВалютСрезПоследних.Курс)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДолгВалПлатежа,
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ДатаЗаказаБольшеДатыДокумента,
	|	ВременнаяТаблицаНДФЛ_ПФ.СуммаНДФЛ КАК СуммаНДФЛ,
	|	ВременнаяТаблицаНДФЛ_ПФ.СуммаВалНДФЛ КАК СуммаВалНДФЛ,
	|	ВЫБОР
	|		КОГДА ISNULL(ВременнаяТаблицаНДФЛ_ПФ.СуммаНДФЛ,0) = 0
	|				И ISNULL(ВременнаяТаблицаНДФЛ_ПФ.СуммаВалНДФЛ,0) = 0
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	|		ИНАЧЕ ВременнаяТаблицаНДФЛ_ПФ.ВидУдержанияНДФЛ
	|	КОНЕЦ КАК ВидУдержанияНДФЛ,
	|	ВременнаяТаблицаНДФЛ_ПФ.СуммаПФ,
	|	ВременнаяТаблицаНДФЛ_ПФ.СуммаВалПФ,
	|	ВЫБОР
	|		КОГДА ISNULL(ВременнаяТаблицаНДФЛ_ПФ.СуммаНДФЛ,0) = 0
	|				И ISNULL(ВременнаяТаблицаНДФЛ_ПФ.СуммаВалНДФЛ,0) = 0
	|			ТОГДА ВременнаяТаблицаНДФЛ_ПФ.ВидУдержанияНДФЛ
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	|	КОНЕЦ КАК ВидУдержанияПФ
	//--------------------------------------------------------------------------
	// © Шишка В.В. [19 июля 2021]  НАЧАЛО
	|ПОМЕСТИТЬ ВТ_ТаблицаДолгов
	// © Шишка В.В. [19 июля 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
	|ИЗ
	|	ВременнаяТаблицаДолгов КАК ВременнаяТаблицаДолгов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСчетаНаОплату КАК ВременнаяТаблицаСчетаНаОплату
	|		ПО ВременнаяТаблицаДолгов.Заказ = ВременнаяТаблицаСчетаНаОплату.Заказ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаНДФЛ_ПФ КАК ВременнаяТаблицаНДФЛ_ПФ
	|		ПО ВременнаяТаблицаДолгов.Контрагент = ВременнаяТаблицаНДФЛ_ПФ.Контрагент
	|			И (ВременнаяТаблицаНДФЛ_ПФ.Организация = ВременнаяТаблицаДолгов.Организация)
	|			И ВременнаяТаблицаДолгов.Документ = ВременнаяТаблицаНДФЛ_ПФ.Регистратор,
	|	Константа.ВалютаУчета КАК ВалютаУчета,
	|	РегистрСведений.КурсыВалют.СрезПоследних(КОНЕЦПЕРИОДА(&Период, ДЕНЬ), Валюта = &Валюта) КАК КурсыВалютСрезПоследних
	|
	//--------------------------------------------------------------------------
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаДолгов.Организация КАК Организация,
	|	ВТ_ТаблицаДолгов.Договор КАК Договор,
	|	ВТ_ТаблицаДолгов.Договор.СтатьяДДСПоУмолчанию КАК СтатьяДДС, //+++ Дмгг
	|	ВТ_ТаблицаДолгов.Документ КАК Документ,
	|	ВТ_ТаблицаДолгов.Заказ КАК Заказ,
	|	ВТ_ТаблицаДолгов.ДатаЗаказа КАК ДатаЗаказа,
	|	СУММА(ВТ_ТаблицаДолгов.ДолгВалДоговора) КАК ДолгВалДоговора,
	|	ВТ_ТаблицаДолгов.ДатаДокумента КАК ДатаДокумента,
	|	ВТ_ТаблицаДолгов.Курс КАК Курс,
	|	ВТ_ТаблицаДолгов.Кратность КАК Кратность,
	|	ВТ_ТаблицаДолгов.Вниз КАК Вниз,
	|	ВТ_ТаблицаДолгов.Добавить КАК Добавить,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаДокумента) КАК СуммаДокумента,
	|	ВТ_ТаблицаДолгов.ВалютаДокумента КАК ВалютаДокумента,
	|	СУММА(ВТ_ТаблицаДолгов.ДолгВалПлатежа) КАК ДолгВалПлатежа,
	|	ВТ_ТаблицаДолгов.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаЗаказа) КАК СуммаЗаказа,
	|	ВТ_ТаблицаДолгов.ВалютаЗаказа КАК ВалютаЗаказа,
	|	ВТ_ТаблицаДолгов.ИдентификаторСтрокиРасшифровкиПлатежа КАК ИдентификаторСтрокиРасшифровкиПлатежа,
	|	ВТ_ТаблицаДолгов.ВалютаДоговора КАК ВалютаДоговора,
	|	ВТ_ТаблицаДолгов.ВалютаДоговораСсылка КАК ВалютаДоговораСсылка,
	|	ВТ_ТаблицаДолгов.КурсИсходный КАК КурсИсходный,
	|	ВТ_ТаблицаДолгов.СчетНаОПлату КАК СчетНаОПлату,
	|	ВТ_ТаблицаДолгов.ВалютаСчета КАК ВалютаСчета,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаСчета) КАК СуммаСчета,
	|	ВТ_ТаблицаДолгов.ДатаДокБольшеДатыДокумента КАК ДатаДокБольшеДатыДокумента,
	|	ВТ_ТаблицаДолгов.ДатаЗаказаБольшеДатыДокумента КАК ДатаЗаказаБольшеДатыДокумента,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаНДФЛ) КАК СуммаНДФЛ,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаВалНДФЛ) КАК СуммаВалНДФЛ,
	|	МАКСИМУМ(ВТ_ТаблицаДолгов.ВидУдержанияНДФЛ) КАК ВидУдержанияНДФЛ,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаПФ) КАК СуммаПФ,
	|	СУММА(ВТ_ТаблицаДолгов.СуммаВалПФ) КАК СуммаВалПФ,
	|	МАКСИМУМ(ВТ_ТаблицаДолгов.ВидУдержанияПФ) КАК ВидУдержанияПФ
	|ИЗ
	|	ВТ_ТаблицаДолгов КАК ВТ_ТаблицаДолгов
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ТаблицаДолгов.Организация,
	|	ВТ_ТаблицаДолгов.Договор,
	|	ВТ_ТаблицаДолгов.Договор.СтатьяДДСПоУмолчанию, //+++ Дмгг
	|	ВТ_ТаблицаДолгов.Документ,
	|	ВТ_ТаблицаДолгов.Заказ,
	|	ВТ_ТаблицаДолгов.ДатаЗаказа,
	|	ВТ_ТаблицаДолгов.ДатаДокумента,
	|	ВТ_ТаблицаДолгов.Курс,
	|	ВТ_ТаблицаДолгов.Кратность,
	|	ВТ_ТаблицаДолгов.Вниз,
	|	ВТ_ТаблицаДолгов.Добавить,
	|	ВТ_ТаблицаДолгов.ВалютаДокумента,
	|	ВТ_ТаблицаДолгов.СтавкаНДС,
	|	ВТ_ТаблицаДолгов.ВалютаЗаказа,
	|	ВТ_ТаблицаДолгов.ИдентификаторСтрокиРасшифровкиПлатежа,
	|	ВТ_ТаблицаДолгов.ВалютаДоговора,
	|	ВТ_ТаблицаДолгов.ВалютаДоговораСсылка,
	|	ВТ_ТаблицаДолгов.КурсИсходный,
	|	ВТ_ТаблицаДолгов.СчетНаОПлату,
	|	ВТ_ТаблицаДолгов.ВалютаСчета,
	|	ВТ_ТаблицаДолгов.ДатаДокБольшеДатыДокумента,
	|	ВТ_ТаблицаДолгов.ДатаЗаказаБольшеДатыДокумента
	|УПОРЯДОЧИТЬ ПО
	|	Документ,
	|	Заказ,
	|	Договор
...
//+++ Дмгг
&НаСервереБезКонтекста
Функция ПолучитьПериодРегистрацииИзВедомости(ТекущийПериод, ДокументОснование, ВидОперации)
	
	Если ЗначениеЗаполнено(ДокументОснование) 
		И ТипЗнч(ДокументОснование)	= Тип("ДокументСсылка.ПлатежнаяВедомость") 
		И ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику Тогда 
		
		Возврат НачалоМесяца(ДокументОснование.Дата);
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаЗаймаСотруднику") Тогда
		...
		КонецЦикла;
		Объект.СчетОрганизации = Неопределено;
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту") Тогда
		Объект.Корреспонденция = Неопределено;
		Объект.КорреспонденцияРегл = Неопределено; //+++ Дмгг
		Объект.Контрагент = Неопределено;
		Объект.ВидНалога = Неопределено;
	...
	Корреспонденция = Объект.Корреспонденция;
	КорреспонденцияРегл = Объект.КорреспонденцияРегл;  //+++ Дмгг
...
Процедура УстановитьВидимостьРеквизитовВЗависимостиОтКорреспонденции(ОчиститьАналитику = Ложь)
	...
	Элементы.ДекорацияРазделительПрочиеРасчеты.Видимость = ЛОЖЬ;// Kuzmich, 27.05.2020
	...
	Если Объект.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
		...
		КонецЕсли;
	// Kuzmich, 27.05.2020
		ПодразделениеОбязательно = ЛОЖЬ;// Kuzmich, 27.05.2020
	ИначеЕсли Объект.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.ПрочиеРасходы Тогда
		...
		Элементы.Подразделение.Видимость = ИСТИНА;// Kuzmich, 27.05.2020
		Элементы.Заказ.Видимость = Ложь;
		Элементы.ДекорацияРазделительПрочиеРасчеты.Видимость = ИСТИНА;
	// Kuzmich, 27.05.2020
	Иначе
		Если Объект.ВидОперации <> Перечисления.ВидыОперацийРасходИзКассы.Налоги // для ввода на основании
		   И Объект.ВидОперации <> Перечисления.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику Тогда
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	Элементы.ТоварныйЧек.Видимость = ((Константы.ИспользоватьТоварныйЧекПоступлениеРасходКассы.Получить()) И (Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю)); // Kuzmich, 06.08.2019, Использовать товарный чек (Поступление в кассу, Расход из кассы
...
Процедура НастроитьЭлементДоговораЗайма()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту Тогда
...
Процедура УстановитьВидимостьПроекта()
	...
	ВидимостиПроекта = ВидОперации <> Перечисления.ВидыОперацийРасходИзКассы.Поставщику
...
Процедура ОбработатьИзменениеДоговораКредитаИлиЗайма()
	...
	//Объект.ВалютаДенежныхСредств = ДанныеДоговораЗаймаСотруднику.ВалютаРасчетов;  //++Lazare 11.01.2023
...
Процедура ЗаполнитьПоДоговоруКредита(Команда)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту") Тогда
...
Процедура ЗаполнитьПоДоговоруКредитаПолученного()
	...
	ОткрытьФорму("ОбщаяФорма.ФормаЗаполненияРасшифровкиПлатежаПоКредитамИЗаймам", 
						ПараметрыОтбора,
						ЭтотОбъект,,,,Новый ОписаниеОповещения("ЗаполнитьПоДоговоруКредитаПолученногоЗавершение", ЭтотОбъект));
...
Процедура ЗаполнитьИнформациюПоКредитуЗаймуНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаЗаймаСотруднику")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту") Тогда
...
Функция ПолучитьДоговорКредитаЗаймаПоУмолчанию(Документ, Контрагент, Организация, ВидОперации)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ВыдачаКредитаКонтрагенту Тогда
...
Процедура ЗаполнитьКолонкуУчитыватьНДФЛ(Идентификатор = Неопределено, ИмяСобытия = Неопределено)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику Тогда
		...
		Элементы.ТипСуммаНДФЛ.Видимость = Ложь;
		Элементы.ТипСуммаПФ.Видимость	= Ложь;
		Элементы.ГруппаСуммаЛьгот.Видимость	= Ложь;
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику Тогда
		Элементы.РасшифровкаПлатежаГруппаНДФЛ.Видимость = Ложь;		
		Элементы.ТипСуммаНДФЛ.Видимость = Истина;
		Элементы.ТипСуммаПФ.Видимость 	= Истина;
		Элементы.ГруппаСуммаЛьгот.Видимость	= Истина;
	Иначе
		Элементы.РасшифровкаПлатежаГруппаНДФЛ.Видимость = Ложь;
		Элементы.ТипСуммаНДФЛ.Видимость = Ложь;
		Элементы.ТипСуммаПФ.Видимость	= Ложь;
...
Процедура РасчетыПрочиеКорреспонденцияРеглПриИзменении(Элемент)
	Если КорреспонденцияРегл <> Объект.КорреспонденцияРегл Тогда
		...
		Если Не ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
			...
			КорреспонденцияПриИзменении(Элементы.РасчетыПрочиеКорреспонденция);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ПересчитатьСуммуНДФЛЗарплаты()
	
	//Паганини_ ПФ
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику Тогда
		Объект.СуммаНДФЛ	= 0;
		Объект.СуммаПФ		= 0;
		Для Каждого Строк из ТаблицаДолгов Цикл
			
			ПоискСтрокиРасшифровки = Объект.РасшифровкаПлатежа.НайтиСтроки(Новый Структура("Договор", Строк.Договор));
			
			Если ЗначениеЗаполнено(Строк.ВидУдержанияНДФЛ) Тогда 
				Если ПоискСтрокиРасшифровки.Количество() > 0 Тогда 
					СтрокаЗаполнения = ПоискСтрокиРасшифровки[0];
					СтрокаЗаполнения.ВидУдержанияНДФЛ = Строк.ВидУдержанияНДФЛ;
					СтрокаЗаполнения.СуммаНДФЛ = Строк.СуммаНДФЛ;
				КонецЕсли;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Строк.ВидУдержанияПФ) Тогда 
				
				Если ПоискСтрокиРасшифровки.Количество() > 0 Тогда 
					СтрокаЗаполнения = ПоискСтрокиРасшифровки[0];
					СтрокаЗаполнения.ВидУдержанияПФ = Строк.ВидУдержанияПФ;
					СтрокаЗаполнения.СуммаПФ = Строк.СуммаПФ;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Возврат;
	ИначеЕсли Не Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику Тогда
		Объект.СуммаНДФЛ	= 0;
		Объект.СуммаПФ		= 0;
		Возврат;
	КонецЕсли;
	...
	Объект.СуммаНДФЛ	= Окр(Объект.СуммаДокумента * СтавкаНДФЛ/(100 - СтавкаНДФЛ), 2);
	Объект.СуммаПФ		= Окр((Объект.СуммаДокумента + Объект.СуммаНДФЛ) * СтавкаПФ/(100 - СтавкаПФ), 2);
	//_Паганини ПФ
...
// Kuzmich, 06.08.2019, Использовать товарный чек (Поступление в кассу, Расход из кассы
&НаСервере
Функция ТоварныйЧекНеУникален()
	Перем РЕЗ;
	
	РЕЗ = ЛОЖЬ;
	
	Если Константы.ИспользоватьТоварныйЧекПоступлениеРасходКассы.Получить() И Объект.ТоварныйЧек>0 Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПоступлениеВКассу.ТоварныйЧек
		|ИЗ
		|	Документ.ПоступлениеВКассу КАК ПоступлениеВКассу
		|ГДЕ
		|	НЕ ПоступлениеВКассу.Ссылка = &Ссылка
		|	И ПоступлениеВКассу.ТоварныйЧек = &ТоварныйЧек";
		
		Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
		Запрос.УстановитьПараметр("ТоварныйЧек", Объект.ТоварныйЧек);
...
&НаСервере
Функция ДипазонТоварныхЧековОшибка()
	Перем РЕЗ;
	
	РЕЗ = ЛОЖЬ;
	
	Если Константы.ИспользоватьТоварныйЧекПоступлениеРасходКассы.Получить() Тогда
		
		Если Объект.ТоварныйЧек>0 Тогда
			
			ТекДата = ?(ЗначениеЗаполнено(Объект.Дата),Объект.Дата,ТекущаяДата());
			ДиапазонТоварныхЧеков = РегистрыСведений.ИспользоватьТоварныйЧекПоступлениеРасходКассы.ПолучитьПоследнее(ТекДата);
			
			Если ДиапазонТоварныхЧеков.ВерхнийДиапазон>0  Тогда
			
				Если Объект.ТоварныйЧек>ДиапазонТоварныхЧеков.ВерхнийДиапазон Тогда
			...
			Если ДиапазонТоварныхЧеков.НижнийДиапазон>0 Тогда
			
				Если Объект.ТоварныйЧек<ДиапазонТоварныхЧеков.НижнийДиапазон Тогда
	...
	Возврат РЕЗ;
КонецФункции // ПроверитьДипазонТоварныхЧеков()

&НаКлиенте
Процедура ТоварныйЧекПриИзменении(Элемент)
	Если ТоварныйЧекНеУникален() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Товарный чек не уникален! ';ka='ჩგდ უნიკალური არ არის!'");
		Сообщение.Поле = "Объект.ТоварныйЧек";
...
Процедура РасшифровкаПлатежаРасчетыПоКредитамТипСуммыПриИзмененииНаСервере(Идентификатор = Неопределено)
	Если Идентификатор = Неопределено Тогда
		Для Каждого СтрокаТЧ ИЗ Объект.РасшифровкаПлатежа Цикл
			Идентификатор = СтрокаТЧ.ПолучитьИдентификатор();
			ТекДанные = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(Идентификатор);
			ЗаполнитьКурсИКратност(ТекДанные);//+Lazare 09.01.2023
...
Процедура ЗаполнитьПроектПоВедомости(СтрокаИД)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПерваяСтрока Тогда
			СтрокаТЧ.Проект = Выборка.Проект;
			СтрокаТЧ.СуммаПлатежа = Выборка.СуммаПлатежа;
			ПерваяСтрока = Ложь;
