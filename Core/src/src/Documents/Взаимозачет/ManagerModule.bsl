...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	|				И (ТаблицаДокумента.Ссылка.КорреспонденцияРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	|				ИЛИ ТаблицаДокумента.Ссылка.КорреспонденцияРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеРасходы))
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) //Приход)
	|		КОГДА ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиРасчетыПоДоговорам
	|				И НЕ ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиРасчетыПоДокументам
	|				И НЕ ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиРасчетыПоЗаказам
	|				И НЕ ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиУчетОплатыПоСчетам
	...
	|				И (ТаблицаДокумента.Ссылка.КорреспонденцияРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Доходы)
	|				ИЛИ ТаблицаДокумента.Ссылка.КорреспонденцияРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеДоходы))
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) //Расход)
	|		КОГДА ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиРасчетыПоДоговорам
	|				И НЕ ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиРасчетыПоДокументам
	|				И НЕ ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиРасчетыПоЗаказам
	|				И НЕ ТаблицаДокумента.Ссылка.КонтрагентИсточник.ВестиУчетОплатыПоСчетам
	...
	|						И ТаблицаДокумента.Ссылка.ВидПрочегоВзаимозачета = ЗНАЧЕНИЕ(Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту)
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.РАСХОД)
	|		КОГДА ТаблицаДокумента.Документ ССЫЛКА Документ.ПриходнаяНакладная И ТаблицаДокумента.Ссылка.ДокументРасчетов <> НЕОПРЕДЕЛЕНО
	|				И ((ТаблицаДокумента.Ссылка.ДокументРасчетов ССЫЛКА Документ.РасходСоСчета И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходСоСчета.ПустаяСсылка))
	|					ИЛИ (ТаблицаДокумента.Ссылка.ДокументРасчетов ССЫЛКА Документ.РасходИзКассы И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходИзКассы.ПустаяСсылка)))
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		КОГДА ТаблицаДокумента.Документ ССЫЛКА Документ.ПриходнаяНакладная 
	|				И (ТаблицаДокумента.Ссылка.ДокументРасчетов ССЫЛКА Документ.РасходСоСчета ИЛИ ТаблицаДокумента.Ссылка.ДокументРасчетов ССЫЛКА Документ.РасходИзКассы)
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		КОГДА ТаблицаДокумента.Документ ССЫЛКА Документ.ПриходнаяНакладная
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.РАСХОД)
	|		КОГДА НЕ ТаблицаДокумента.ПризнакАванса И ТаблицаДокумента.Документ ССЫЛКА Документ.Взаимозачет И ТаблицаДокумента.Ссылка.ДокументРасчетов = Неопределено
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.РАСХОД)
	|		КОГДА НЕ ТаблицаДокумента.ПризнакАванса И ТаблицаДокумента.Документ ССЫЛКА Документ.Взаимозачет 
	|				И ТаблицаДокумента.Ссылка.ПризнакАванса 
	|				И ((ТаблицаДокумента.Ссылка.ДокументРасчетов Ссылка Документ.РасходИзКассы И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходИзКассы.ПустаяСсылка))
	|				ИЛИ (ТаблицаДокумента.Ссылка.ДокументРасчетов Ссылка Документ.РасходСоСчета И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходСоСчета.ПустаяСсылка)))
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.РАСХОД)
	|		КОГДА ТаблицаДокумента.ПризнакАванса
	...
	|						ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|					КОГДА ТаблицаДокумента.ПризнакАванса И ТаблицаДокумента.Документ ССЫЛКА Документ.Взаимозачет
	//|						ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|						ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|					ИНАЧЕ ВЫБОР
	|							КОГДА ТаблицаДокумента.Ссылка.КорреспонденцияРегл.Вид = ЗНАЧЕНИЕ(ВидСчета.Активный)
	...
	|							ИЛИ (ТаблицаДокумента.Ссылка.ДокументРасчетов Ссылка Документ.РасходСоСчета И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходСоСчета.ПустаяСсылка)))
	|								ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|						КОГДА ТаблицаДокумента.Ссылка.ПризнакАванса И ТаблицаДокумента.Документ ССЫЛКА Документ.ПриходнаяНакладная И ТаблицаДокумента.Ссылка.ДокументРасчетов <> НЕОПРЕДЕЛЕНО
	|								И ((ТаблицаДокумента.Ссылка.ДокументРасчетов ССЫЛКА Документ.РасходСоСчета И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходСоСчета.ПустаяСсылка))
	|									ИЛИ (ТаблицаДокумента.Ссылка.ДокументРасчетов ССЫЛКА Документ.РасходИзКассы И ТаблицаДокумента.Ссылка.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.РасходИзКассы.ПустаяСсылка)))
	|									ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|						КОГДА ТаблицаДокумента.Ссылка.ПризнакАванса
	...
	//ПодольскийАИ #УчетПроектов 23.05.2021 {
	|	Значение(Справочник.Проекты.БезПроекта) КАК Проект,
	//ПодольскийАИ #УчетПроектов 23.05.2021 }
