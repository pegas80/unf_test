...
Процедура ИнициализироватьДанныеДокументаУправленческий(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("Содержание", НСтр("ka='საწყისი ნაშთების შეყვანა';ru='Ввод начальных остатков';en='Initial Balances'"));
	
	РезультатЗапроса = Запрос.Выполнить();
	
    СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаУправленческий", РезультатЗапроса.Выгрузить());
	
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_ВводНачальныхОстатков(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства);
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПодотчетниками(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.Сумма КАК Сумма,
	|	ТаблицаДокумента.Сотрудник.СчетРасчетовСПодотчетникамиРегл КАК СчетДт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Сотрудник.СчетРасчетовСПодотчетникамиРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Валюта
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаДт,
	|	ВЫБОР
	...
	|	""Валюты"" КАК ВидСубконтоДт1,
	|	""Сотрудники"" КАК ВидСубконтоДт2,
	|	""СтатьиДвиженияДенежныхСредств"" КАК ВидСубконтоДт3,
	|	""Валюты"" КАК ВидСубконтоКт1,
	|	"""" КАК ВидСубконтоКт2,
	|	"""" КАК ВидСубконтоКт3,
	|	&ВозникновениеДолгаПодотчетника КАК Содержание
	|ИЗ
	...
	|	НЕОПРЕДЕЛЕНО,
	|	0,
	|	ТаблицаДокумента.Сотрудник.СчетРасчетовПоПерерасходуРегл,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Сотрудник.СчетРасчетовПоПерерасходуРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Валюта
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеДолгаПодотчетника", НСтр("ka='საქვეანგარიშოს დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности подотчетника';en='Entering receivable balances podotchetnika'"));
	Запрос.УстановитьПараметр("ПогашениеДолгаПодотчетнику", НСтр("ka='საქვეანგარიშოს მიმართ დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности перед подотчетником';en='Entering receivable balances before podotchetnikom'"));
	...
	// Щербаков +++
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаХозрасчетный", МассивРезультатов[2].Выгрузить());
	// Щербаков ---
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПерсоналом(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|	РасчетыСПерсоналом.Сотрудник.СчетРасчетовСПерсоналомРегл КАК СчетУчетаРегл,
	...
	|		КОГДА РасчетыСПерсоналом.Проект <> ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА РасчетыСПерсоналом.Проект
	...
	|	РасчетыСПерсоналом.Сотрудник.СчетРасчетовСПерсоналомРегл
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ТаблицаДокумента.Сотрудник.СчетРасчетовСПерсоналомРегл КАК СчетКт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Сотрудник.СчетРасчетовСПерсоналомРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Валюта
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаКт,
	|	ВЫБОР
	...
	|	ТаблицаДокумента.СтруктурнаяЕдиница КАК СубконтоКт1,
	|	ТаблицаДокумента.Сотрудник КАК СубконтоКт2,
	|	ТаблицаДокумента.Валюта КАК СубконтоКт3,
	|	""Валюты"" КАК ВидСубконтоДт1,
	|	"""" КАК ВидСубконтоДт2,
	|	"""" КАК ВидСубконтоДт3,
	|	""Подразделения"" КАК ВидСубконтоКт1,
	|	""Сотрудники"" КАК ВидСубконтоКт2,
	|	""Валюты"" КАК ВидСубконтоКт3,
	//Малышев 2024-06-13 ONESOFT-3095 (--
	|	ВЫБОР
	|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)  
	|		КОГДА ТаблицаДокумента.Проект <> ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПерсоналом", НСтр("ka='პერსონალის წინაშე ვალდებულებების წარმოშობა';ru='Возникновение обязательств перед персоналом';en='Rise to obligations to employees'"));
	Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам); //Малышев 2024-06-13 ONESOFT-3095
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоНалогам(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ТаблицаДокумента.ВидНалога.СчетУчетаРегл КАК СчетКт,
	|	НЕОПРЕДЕЛЕНО КАК ВалютаКт,
	|	0 КАК СуммаВалКт,
	|	&ВалютаУчета КАК СубконтоДт1,
	|	NULL КАК СубконтоДт2,
	|	NULL КАК СубконтоДт3,
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("НачислениеНалога", НСтр("ka='ბიუჯეტის წინაშე დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности перед бюджетом';en='Entering residues debts to the budget'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПокупателями(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|	КОНЕЦ КАК СчетУчета,
	//ПодольскийАИ #УчетПроектов 05.09.2021 {
	|	ВЫБОР 
	|		КОГДА РасчетыСПокупателями.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|		ИНАЧЕ РасчетыСПокупателями.Проект
	|	КОНЕЦ КАК Проект,
	//ПодольскийАИ #УчетПроектов 05.09.2021 }
	...
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.Сумма КАК Сумма,
	|	ТаблицаДокумента.Контрагент.СчетУчетаРасчетовСПокупателемРегл КАК СчетДт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Контрагент.СчетУчетаРасчетовСПокупателемРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Договор.ВалютаРасчетов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаДт,
	|	ВЫБОР
	...
	//Было---
	//|	ТаблицаДокумента.Договор.ВалютаРасчетов КАК СубконтоДт1,
	//Стало+++
	|	ВЫБОР 
	...
	//ПодольскийАИ #УчетПроектов 06.09.2021 }
	|	ТаблицаДокумента.Контрагент КАК СубконтоДт2,
	|	ТаблицаДокумента.Договор КАК СубконтоДт3,
	|	ТаблицаДокумента.Договор.ВалютаРасчетов КАК СубконтоКт1,
	//ПодольскийАИ #УчетПроектов 06.09.2021 {
	//Было---
	//|	NULL КАК СубконтоКт2,
	//Стало+++
	...
	//ПодольскийАИ #УчетПроектов 06.09.2021 }
	|	""Контрагенты"" КАК ВидСубконтоДт2,
	|	""Договоры"" КАК ВидСубконтоДт3,
	|	""Валюты"" КАК ВидСубконтоКт1,
	//ПодольскийАИ #УчетПроектов 06.09.2021 {
	//Было---
	//|	"""" КАК ВидСубконтоКт2,
	...
	|	НЕОПРЕДЕЛЕНО,
	|	0,
	|	ТаблицаДокумента.Контрагент.СчетУчетаАвансовПокупателяРегл,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Контрагент.СчетУчетаАвансовПокупателяРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Договор.ВалютаРасчетов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателя", НСтр("ka='მყიდველთა დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности покупателей';en='Entering balances receivable'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПокупателя", НСтр("ka='მყიდველებისგან ავანსის ნაშთების შეყვანა';ru='Ввод остатков авансов от покупателей';en='Enter balances of advances from customers'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПоставщиками(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|	КОНЕЦ КАК Заказ,
	|	ВЫБОР
	|		КОГДА РасчетыСПоставщиками.Контрагент.ВестиРасчетыПоДокументам И НЕ РасчетыСПоставщиками.ПроводитьБезДокумента
	...
	//ПодольскийАИ #УчетПроектов 06.09.2021 {
	|	ВЫБОР 
	|		КОГДА РасчетыСПоставщиками.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|		ИНАЧЕ РасчетыСПоставщиками.Проект
	|	КОНЕЦ КАК Проект,
	//ПодольскийАИ #УчетПроектов 06.09.2021 }
	...
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ТаблицаДокумента.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетКт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Контрагент.СчетУчетаРасчетовСПоставщикомРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Договор.ВалютаРасчетов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаКт,
	|	ВЫБОР
	...
	//ПодольскийАИ #УчетПроектов 06.09.2021 }
	|	ТаблицаДокумента.Контрагент КАК СубконтоКт2,
	|	ТаблицаДокумента.Договор КАК СубконтоКт3,
	|	""Валюты"" КАК ВидСубконтоДт1,
	//ПодольскийАИ #УчетПроектов 06.09.2021 {
	//Было---
	//|	"""" КАК ВидСубконтоДт2,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Контрагент.СчетУчетаАвансовПоставщикуРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Договор.ВалютаРасчетов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПоставщиком", НСтр("ka='მომწოდებელებისადმი დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности поставщикам';en='Entering balances payable to suppliers'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПередПоставщиком", НСтр("ka='მომწოდებლებისთვის ავანსის ნაშთების შეყვანა';ru='Ввод остатков авансов поставщикам';en='Enter balances of advances to suppliers'"));
	...
	// Щербаков +++
	Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаХозрасчетный.Количество() > 0 Тогда
...
Процедура ИнициализироватьДанныеДокументаДенежныеСредства(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|		Когда ДенежныеСредства.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|			Тогда Значение(Справочник.Проекты.БезПроекта)
	|		Иначе ДенежныеСредства.Проект
	|	Конец КАК Проект
	//ПодольскийАИ #УчетПроектов 30.09.2021 }
	|ИЗ
	...
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.Сумма КАК Сумма,
	|	ТаблицаДокумента.БанковскийСчетКасса.СчетУчетаРегл КАК СчетДт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.БанковскийСчетКасса.СчетУчетаРегл.Валютный
	|			ТОГДА ТаблицаДокумента.ВалютаДенежныхСредств
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаДт,
	|	ВЫБОР
	...
	//Было---
	//|	ВЫБОР
	//|		КОГДА ФункциональнаяОпцияУчетПоПроектам.Значение
	//|			ТОГДА ТаблицаДокумента.Проект
	...
	|	Конец КАК СубконтоДт1,
	//ПодольскийАИ #УчетПроектов 30.09.2021 }
	|	ТаблицаДокумента.БанковскийСчетКасса КАК СубконтоДт2,
	|	ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ВводНачальныхОстатков) КАК СубконтоДт3,
	|	ТаблицаДокумента.ВалютаДенежныхСредств КАК СубконтоКт1,
	|	NULL КАК СубконтоКт2,
	|	NULL КАК СубконтоКт3,
	//ПодольскийАИ #УчетПроектов 30.09.2021 {
	//Было---
	...
	|	КОНЕЦ КАК ВидСубконтоДт1,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДокумента.БанковскийСчетКасса) = ТИП(Справочник.Кассы)
	|			ТОГДА ""Касса""
	|		ИНАЧЕ ""БанковскиеСчета""
	|	КОНЕЦ КАК ВидСубконтоДт2,
	|	""СтатьиДвиженияДенежныхСредств"" КАК ВидСубконтоДт3,
	|	""Валюты"" КАК ВидСубконтоКт1,
	|	"""" КАК ВидСубконтоКт2,
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("СодержаниеПроводки", НСтр("ka='ფულადი სახსრების ნაშთის შეყვანა';ru='Ввод остатков денежных средств';en='Entering cash balances'"));
...
Процедура ИнициализироватьДанныеДокументаЗапасы(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	//ПодольскийАИ #УчетПроектов 05.09.2021 {
	|	ВводНачальныхОстатковЗапасы.Проект КАК Проект,
	//ПодольскийАИ #УчетПроектов 05.09.2021 }
	...
	|   ВводНачальныхОстатковЗапасы.ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
	|	ВводНачальныхОстатковЗапасы.СодержаниеПроводки КАК СодержаниеПроводки,
	|	ВводНачальныхОстатковЗапасы.СчетУчетаРегл,
	|	ВводНачальныхОстатковЗапасы.СчетУчетаЗатратРегл
	|ИЗ
	|	(ВЫБРАТЬ
	|		0 КАК Порядок,
	|		ВводНачальныхОстатковЗапасы.НомерСтроки КАК НомерСтроки,
	...
	//ПодольскийАИ #УчетПроектов 05.09.2021 {
	|		ВЫБОР
	|			КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	|			//КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА ВводНачальныхОстатковЗапасы.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|				ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|			ИНАЧЕ ВводНачальныхОстатковЗапасы.Проект
	|		КОНЕЦ КАК Проект,
	//ПодольскийАИ #УчетПроектов 05.09.2021 }
	|		ВводНачальныхОстатковЗапасы.Номенклатура КАК Номенклатура,
	|		ВЫБОР
	...
	|  		ВЫБОР 
	|         КОГДА ВводНачальныхОстатковЗапасы.Номенклатура.МетодОценки = ЗНАЧЕНИЕ(Перечисление.МетодОценкиЗапасов.FIFO) Тогда &Ссылка
	|         ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|		ВЫБОР
	//Малышев 2025-04-09 ONESOFT-4000 (--
	|			КОГДА ВводНачальныхОстатковЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|					И ВводНачальныхОстатковЗапасы.СтруктурнаяЕдиница.ТранзитныйСклад 
	|				ТОГДА ВводНачальныхОстатковЗапасы.СтруктурнаяЕдиница.СчетУчетаРегл
	//Малышев 2025-04-09 ONESOFT-4000 --))

	|			КОГДА ВводНачальныхОстатковЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|					ИЛИ ВводНачальныхОстатковЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница)
	|				ТОГДА ВводНачальныхОстатковЗапасы.Номенклатура.СчетУчетаЗапасовРегл
	|			ИНАЧЕ ВводНачальныхОстатковЗапасы.Номенклатура.СчетУчетаЗатратРегл
	|		КОНЕЦ КАК СчетУчетаРегл,
	|		ВводНачальныхОстатковЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл
	|	ИЗ
	...
	// Олег Литвиненко 04.07.23 --->
	// Олег Литвиненко <---
	//ПодольскийАИ #УчетПроектов 05.09.2021 {
	|		ВЫБОР 
	|			КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА ВводНачальныхОстатковПрямыеЗатраты.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|				ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|			ИНАЧЕ ВводНачальныхОстатковПрямыеЗатраты.Проект
	|		КОНЕЦ КАК Проект,
	//ПодольскийАИ #УчетПроектов 05.09.2021 }
	// Олег Литвиненко 04.07.23 --->
	|		ВводНачальныхОстатковПрямыеЗатраты.Номенклатура,	
	// Олег Литвиненко <---
	...
	|		&ОприходованиеЗатрат,
	|		ВводНачальныхОстатковПрямыеЗатраты.СчетЗатратРегл,
	...
	|			КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА ВводНачальныхОстатковЗапасыПринятые.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|				ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|			ИНАЧЕ ВводНачальныхОстатковЗапасыПринятые.Проект
	|		КОНЕЦ КАК Проект,
	//ПодольскийАИ #УчетПроектов 05.09.2021 }
	...
	|		ВводНачальныхОстатковЗапасыПринятые.Номенклатура.СчетУчетаЗапасовРегл,
	|		ВводНачальныхОстатковЗапасыПринятые.Номенклатура.СчетУчетаЗатратРегл
	|	ИЗ
	...
	|			КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	|			КОГДА ВводНачальныхОстатковЗапасыПереданные.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|				ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|			ИНАЧЕ ВводНачальныхОстатковЗапасыПереданные.Проект
	|		КОНЕЦ КАК Проект,
	//ПодольскийАИ #УчетПроектов 05.09.2021 }
	...
	|		ВводНачальныхОстатковЗапасыПереданные.Номенклатура.СчетУчетаЗапасовРегл,
	|		ВводНачальныхОстатковЗапасыПереданные.Номенклатура.СчетУчетаЗатратРегл
	|	ИЗ
	...
	|	ВЫБОР 
	|		КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	//+02.05.2025 ONESOFT-4062
	|		КОГДА &УчетПоПроектам и ВводНачальныхОстатковЗапасы.Проект = Значение(Справочник.Проекты.ПустаяСсылка) Тогда Значение(Справочник.Проекты.БезПроекта)
	//-02.05.2025 ONESOFT-4062
	|		КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	|		КОГДА ВводНачальныхОстатковЗапасы.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА Значение(Справочник.Проекты.БезПроекта)
	...
	|		ИНАЧЕ ВводНачальныхОстатковЗапасы.Количество * ВводНачальныхОстатковЗапасы.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	ВводНачальныхОстатковЗапасы.СрокГодности КАК СрокГодности,
	|	ВводНачальныхОстатковЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр	
	|ИЗ
	|	Документ.ВводНачальныхОстатков.Запасы КАК ВводНачальныхОстатковЗапасы
	|ГДЕ
	|	ВводНачальныхОстатковЗапасы.Ссылка = &Ссылка
	...
	|		КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	//+02.05.2025 ONESOFT-4062
	|		КОГДА &УчетПоПроектам и ВводНачальныхОстатковЗапасыПринятые.Проект = Значение(Справочник.Проекты.ПустаяСсылка) Тогда Значение(Справочник.Проекты.БезПроекта)
	//-02.05.2025 ONESOFT-4062
	|		КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	|		КОГДА ВводНачальныхОстатковЗапасыПринятые.Проект = Значение(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА Значение(Справочник.Проекты.БезПроекта)
	...
	//Малышев 2025-04-09 ONESOFT-4000 (--
	|		КОГДА ТаблицаДокумента.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|				И ТаблицаДокумента.СтруктурнаяЕдиница.ТранзитныйСклад 
	|			ТОГДА ТаблицаДокумента.СтруктурнаяЕдиница.СчетУчетаРегл
	//Малышев 2025-04-09 ONESOFT-4000 --))
	
	|		КОГДА ТаблицаДокумента.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|				ИЛИ ТаблицаДокумента.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница)
	|			ТОГДА ТаблицаДокумента.Номенклатура.СчетУчетаЗапасовРегл
	|		ИНАЧЕ ТаблицаДокумента.Номенклатура.СчетУчетаЗатратРегл
	|	КОНЕЦ КАК СчетДт,
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ВЫБОР
	...
	|						ТОГДА ТаблицаДокумента.Номенклатура.СчетУчетаЗапасовРегл
	|					ИНАЧЕ ТаблицаДокумента.Номенклатура.СчетУчетаЗатратРегл
	|				КОНЕЦ КАК ПланСчетов.Хозрасчетный).Количественный
	|			ТОГДА ВЫБОР
	...
	|	0 КАК СуммаВалКт,
	|	&ОприходованиеЗапасов КАК Содержание,
	|	ТаблицаДокумента.Номенклатура КАК СубконтоДт1,
	|	ТаблицаДокумента.Характеристика КАК СубконтоДт2,
	|	ТаблицаДокумента.СтруктурнаяЕдиница КАК СубконтоДт3,
	//ПодольскийАИ #УчетПроектов 05.09.2021 {
	|	ВЫБОР 
	|		КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	|		КОГДА &СебестоимостьЗапасовПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 	//Малышев 2024-09-25 ONESOFT-3476    
	...
	|	NULL КАК СубконтоКт2,
	|	NULL КАК СубконтоКт3,
	|	""Номенклатура"" КАК ВидСубконтоДт1,
	|	""ХарактеристикиНоменклатуры"" КАК ВидСубконтоДт2,
	|	""Склады"" КАК ВидСубконтоДт3,
	|	"""" КАК ВидСубконтоКт1,
	|	"""" КАК ВидСубконтоКт2,
	|	"""" КАК ВидСубконтоКт3
	|ИЗ
	...
	|	ТаблицаДокумента.Сумма <> 0
	|	И ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	NULL,
	//--------------------------------------------------------------------------
	// © Шишка В.В. [4 октября 2021]  НАЧАЛО
	//|	NULL,
	|	ТаблицаДокумента.СтруктурнаяЕдиница,
	// © Шишка В.В. [4 октября 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
	//ПодольскийАИ #УчетПроектов 05.09.2021 {
	|	ВЫБОР 
	|		КОГДА &УчетПоПроектам = Ложь Тогда Значение(Справочник.Проекты.ПустаяСсылка) 					//Малышев 2024-09-25 ONESOFT-3476    
	...
	|	0,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Номенклатура.СчетУчетаЗапасовРегл.Количественный
	|			ТОГДА ТаблицаДокумента.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Вспомогательный),
	...
	|	ТаблицаДокумента.Сумма <> 0
	|	И ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
	|	НомерСтроки");
	...
	Запрос.УстановитьПараметр("ОприходованиеЗатрат", НСтр("ka='დანახარჯების შემოსავალში გატარება';ru='Оприходование затрат';en='posting costs'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасовПринятых", НСтр("ka='მიღებული მარაგების შემოსავალში გატარება';ru='Оприходование запасов принятых';en='Posting inventory received'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасовПереданных", НСтр("ka='გადაცემული მარაგების შემოსავალში გატარება';ru='Оприходование запасов переданных';en='Posting reserves transferred'"));
	...
	//Малышев 2024-09-25 ONESOFT-3476
	Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
	...
	Если УчетСебестоимостиВРазрезеОрганизации Тогда
		...
		Для каждого ТекущаяСтрока Из ТаблицаЗапасы Цикл
			...
			Если ЗначениеЗаполнено(ТекущаяСтрока.СтруктурнаяЕдиница)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаХозрасчетный", МассивРезультатов[7].Выгрузить());
	...
	СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства );//++Lazare 17.10.2022
...
Процедура ИнициализироватьДанныеВнеоборотныеАктивы(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.КоличествоМесяцеВЭксплуатации КАК КоличествоМесяцеВЭксплуатации,
	//--- Дмгг
	...
	//|	ТаблицаДокумента.ДатаВводаВЭксплуатацию КАК Период,
	|	ВЫБОР
	|		КОГДА &НачислятьАмортизациюОтНачальнойСтоимости
	...
	|	ТаблицаДокумента.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.Проект КАК Проект, //Малышев 2024-06-13 ONESOFT-3095
	...
	|,	0 КАК КоличествоМесяцеВЭксплуатации //+++ Дмгг
	|ИЗ
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаДокумента
	|ГДЕ
	...
	|	ТаблицаДокумента.СчетЗатратРегл КАК СчетУчетаРег
	...
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.ПервоначальнаяСтоимость КАК Сумма,
	|	ТаблицаДокумента.ВнеоборотныйАктив.СчетУчетаРегл КАК СчетДт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВнеоборотныйАктив.СчетУчетаРегл.Валютный
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаДт,
	|	ВЫБОР
	...
	|	НЕОПРЕДЕЛЕНО КАК ВалютаКт,
	|	0 КАК СуммаВалКт,
	|	ТаблицаДокумента.СтруктурнаяЕдиница КАК СубконтоДт1,
	|	ТаблицаДокумента.ВнеоборотныйАктив КАК СубконтоДт2,
	|	NULL КАК СубконтоДт3,
	|	NULL КАК СубконтоКт1,
	|	NULL КАК СубконтоКт2,
	|	NULL КАК СубконтоКт3,
	|	""Подразделения"" КАК ВидСубконтоДт1,
	|	""Имущество"" КАК ВидСубконтоДт2,
	|	"""" КАК ВидСубконтоДт3,
	|	"""" КАК ВидСубконтоКт1,
	|	"""" КАК ВидСубконтоКт2,
	|	"""" КАК ВидСубконтоКт3,
	//Малышев 2024-06-13 ONESOFT-3095 (--
	|	ТаблицаДокумента.Проект КАК ПроектДт,
	|	НЕОПРЕДЕЛЕНО КАК ПроектКт,
	//Малышев 2024-06-13 ONESOFT-3095 --)
	|	&ПринятиеВнеоборотногоАктиваКУчету КАК Содержание
	|ИЗ
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаДокумента
	...
	|	НЕОПРЕДЕЛЕНО,
	|	0,
	|	ТаблицаДокумента.ВнеоборотныйАктив.СчетАмортизацииРегл,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВнеоборотныйАктив.СчетАмортизацииРегл.Валютный
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	...
	//Малышев 2024-06-13 ONESOFT-3095 (--
	|	ТаблицаДокумента.Проект КАК ПроектДт,
	|	ТаблицаДокумента.Проект КАК ПроектКт,
	//Малышев 2024-06-13 ONESOFT-3095 --)
	|	&НачислениеАмортизации
	|ИЗ
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаДокумента
	...
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
	//Малышев 2024-06-13 ONESOFT-3095 --)
	|");
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПринятиеВнеоборотногоАктиваКУчету", НСтр("ka='გრძელვადიანი აქტივების მიხედვით საწყისი ნაშთების შეყვანა';ru='Ввод начальных остатков по внеоборотным активам';en='Entering opening balances of non-current assets'"));
	Запрос.УстановитьПараметр("НачислениеАмортизации", НСтр("ka='ამორტიზაციის მიხედვით საწყისი ნაშთების შეყვანა';ru='Ввод начальных остатков по амортизации';en='Enter opening balances for depreciation'"));
	//Паганини#NexiaGeoLatex_
	Запрос.УстановитьПараметр("НачислятьАмортизациюОтНачальнойСтоимости", РегистрыСведений.УчетнаяПолитикаРегл.ПолучитьПоследнее(ДокументСсылкаВводНачальныхОстатков.Дата, Новый Структура("Организация", ДокументСсылкаВводНачальныхОстатков.Организация)).НачислятьАмортизациюОтНачальнойСтоимости);
	//_Паганини#NexiaGeoLatex
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаУправленческий", МассивРезультатов[5].Выгрузить());
	// Щербаков +++
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаХозрасчетный", МассивРезультатов[6].Выгрузить());
	// Щербаков ---
	
	//Малышев 2024-06-13 ONESOFT-3095 (--
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаУчетПоПроектамВнеоборотныхАктивов", МассивРезультатов[7].Выгрузить());
	//Малышев 2024-06-13 ONESOFT-3095 --)
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоЭквайрингу(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("РасчетыПоЭквайрингу", НСтр("ka='ეკვაირინგის მიხედვით ანგარიშსწორებების ნაშთის შეყვანა';ru='Ввод остатков расчетов по эквайрингу';en='Enter balances of payments on acquiring'"));
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	ИначеЕсли РазделУчета = "Эквайринг" Тогда
		...
		// Конец Эквайринг
	ИначеЕсли РазделУчета = "Кредиты и займи" Тогда
		
		ИнициализироватьДанныеДокументаРасчетыПоКредитамИЗаймам(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства);
...
Процедура ВыполнитьКонтрольЗапасы(ДокументСсылкаВводНачальныхОстатков, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь)
	...
	СтруктураВременныеТаблицы = ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы;
    ИспользоваТолькольРеглПланСчетов = ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов; //+++ Дмгг
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		...
		|ИЗ
		// Kuzmich, 15.01.2020, Отрицательные отстаки в начальных остатках
		...
		// Kuzmich, 15.01.2020, Отрицательные отстаки в начальных остатках
		|	И ВЫБОР КОГДА Константы.ПодборЦеныДляОтрицательныхОстатков ТОГДА НЕ ДвиженияЗапасыНаСкладахИзменение.СтруктурнаяЕдиница.КонтролироватьОстатки		
		|	ИНАЧЕ ИСТИНА КОНЕЦ
		// Kuzmich, 15.01.2020, Отрицательные отстаки в начальных остатках
		|
		|УПОРЯДОЧИТЬ ПО
		...
		// Kuzmich, 15.01.2020, Отрицательные отстаки в начальных остатках
		|	И ВЫБОР КОГДА Константы.ПодборЦеныДляОтрицательныхОстатков ТОГДА НЕ ДвиженияЗапасыИзменение.СтруктурнаяЕдиница.КонтролироватьОстатки		
		|	ИНАЧЕ ИСТИНА КОНЕЦ
		// Kuzmich, 15.01.2020, Отрицательные отстаки в начальных остатках
		|
		|УПОРЯДОЧИТЬ ПО
		...
		|	НомерСтроки
		//+++ Дмгг
		// Проверка запасов по регл плану счетов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Организация) КАК ОрганизацияПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.СтруктурнаяЕдиница) КАК СтруктурнаяЕдиницаПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.СчетУчетаРегл) КАК СчетУчетаПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Номенклатура) КАК НоменклатураПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Характеристика) КАК ХарактеристикаПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Партия) КАК ПартияПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.ЗаказПокупателя) КАК ЗаказПокупателяПредставление,
		...
		Если НЕ МассивРезультатов[0].Пустой()
			...
			ИЛИ НЕ МассивРезультатов[5].Пустой()Тогда //+++ ДмггТогда
		...
		//КонецЕсли;
		Если ИспользоваТолькольРеглПланСчетов Тогда 
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	//ПодольскийАИ #УчетПроектов 06.09.2021 {
	ОписаниеТиповСтрока125 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(125));
	//ПодольскийАИ #УчетПроектов 06.09.2021 }
	...
	//ПредставлениеКолонки = НСтр("ru='';ka='';en=''");
	...
	Если ПолноеИмяОбъектаЗаполнения = "Документ.ВводНачальныхОстатков.ТабличнаяЧасть.Запасы" Тогда
		...
		ПредставлениеКолонки = НСтр("ru='Склад (наименование)';ka='საწყობი (დასახელება)';en='Warehouse (name)'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтруктурнаяЕдиница", ПредставлениеКолонки, ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
		
		Если ПолучитьФункциональнуюОпцию("УчетПоЯчейкам") Тогда
			
			ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Ячейки");
			ПредставлениеКолонки = НСтр("ru='Ячейка (наименование)';ka='უჯრედი (სახელი)';en='Cell (name)'");
			...
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Ячейка", ПредставлениеКолонки, ОписаниеТиповСтрока50, ОписаниеТиповКолонка);
		...
		Если ПолучитьФункциональнуюОпцию("УчетПоПроектам") Тогда
			...
			ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Проекты");
			ПредставлениеКолонки = НСтр("ru='Проект (наименование)';ka='პროექტი (დასახელება)';en='Project (name)'");
			//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Проект", "Проект (наименование)", ОписаниеТиповСтрока125, ОписаниеТиповКолонка);
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Проект", ПредставлениеКолонки, ОписаниеТиповСтрока125, ОписаниеТиповКолонка);
			
		КонецЕсли;
		//ПодольскийАИ #УчетПроектов 06.09.2021 }
		ПредставлениеГруппы = "Номенклатура";
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Штрихкод", ПредставлениеКолонки, ОписаниеТиповСтрока200, ОписаниеТиповКолонка, ПредставлениеГруппы, 1, , Истина);
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Артикул", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы, 2, , Истина);
		
		ПредставлениеКолонки = НСтр("ru='Номенклатура (наименование)';ka='ნომენკალტურა (დასახელება)';en='Nomenclature (name)'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НоменклатураНаименование", ПредставлениеКолонки, ОписаниеТиповСтрока100, ОписаниеТиповКолонка, ПредставлениеГруппы, 3, , Истина);
		
		ПредставлениеКолонки = НСтр("ru='Номенклатура (полное наименование)';ka='ნომენკლატურა (სრული დასახელება)';en='Nomenclature (full name)'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НоменклатураНаименованиеПолное",ПредставлениеКолонки, ОписаниеТиповСтрока1000, ОписаниеТиповКолонка, ПредставлениеГруппы, 4, , Истина);
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
			
			ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры");
			ПредставлениеКолонки = НСтр("ru='Характеристика (наименование)';ka='მახასიათებელი (დასახელება)';en='Characteristics (name)'");
			...
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Характеристика", ПредставлениеКолонки, ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартии") Тогда
			...
			ПредставлениеКолонки = НСтр("ru='Партия (наименование)';ka='პარტია (დასახელება)';en='Party (name)'");
			...
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Партия", ПредставлениеКолонки, ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
			
		КонецЕсли;
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.КлассификаторЕдиницИзмерения, СправочникСсылка.ЕдиницыИзмерения");
		ПредставлениеКолонки = НСтр("ru='Ед. изм.';ka='საზომი ერთეული';en='unit'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ЕдиницаИзмерения", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка);
		
		ПредставлениеКолонки = НСтр("ru='Количество';ka='რაოდენობა';en='Quantity'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Количество", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_3, , , Истина);
		
		ПредставлениеКолонки = НСтр("ru='Цена';ka='ფასი';en='Price'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Цена", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2, , , Истина);
	...
	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ВводНачальныхОстатков.ТабличнаяЧасть.РасчетыСПокупателями" Тогда
		...
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
		ПредставлениеКолонки = НСтр("ru='Контрагент (ИНН или наименование)';ka='კონტრაგენტი (ს/კ ან დასახელება)';en='Contracting partie (INN  or name)'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Контрагент", ПредставлениеКолонки, ОписаниеТиповСтрока100, ОписаниеТиповКолонка, , , Истина);
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов");
		ПредставлениеКолонки = НСтр("ru='Договор контрагента (наименование либо номер)';ka='კონტრაგენტის ხელშეკრულება (დასახელება ან ნომერი)';en='Contracting parties contract (name or number)'");
		//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Договор", "Договор контрагента (наименование либо номер)", ОписаниеТиповСтрока100, ОписаниеТиповКолонка);
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Договор", ПредставлениеКолонки, ОписаниеТиповСтрока100, ОписаниеТиповКолонка);
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("Булево");
		ПредставлениеКолонки = НСтр("ru='Это аванс?';ka='ეს ავანსია?';en='This advance?'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ПризнакАванса", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка);
		
		//ПредставлениеГруппы = НСтр("ru='Заказ';ka='შეკვეთა';en='Order'");
		ПредставлениеГруппы = "Заказ";
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя");
		
		ПредставлениеКолонки = НСтр("ru='Номер заказа покупателя';ka='მყიდველის შეკვეთის ნომერი';en='Sales order number'");
		//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерЗаказаПокупателя", "Номер заказа покупателя", ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Заказ");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерЗаказаПокупателя", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы);
		
		ПредставлениеКолонки = НСтр("ru='Дата заказа покупателя';ka='მყიდველის შეკვეთის თარიღი';en='Sales order data'");
		//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаЗаказаПокупателя", "Дата заказа покупателя", ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Заказ");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаЗаказаПокупателя", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы);
		...
		//ПредставлениеГруппы = НСтр("ru='Документ';ka='დოკუმენტი';en='Document'");
		ПредставлениеГруппы = "Документ";  //Малышев 2024-07-15
		...
		ПредставлениеКолонки = НСтр("ru='Вид расчетного документа';ka='ანგარიშსწორების დოკუმენტის ტიპი';en='Type of settlement document'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ВидРасчетногоДокумента", ПредставлениеКолонки, ОписаниеТиповСтрока50, ОписаниеТиповКолонка, ПредставлениеГруппы);
		
		ПредставлениеКолонки = НСтр("ru='Номер расчетного документа';ka='ანგარიშსწორების დოკუმენტის ნომერი';en='Payment document number'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерРасчетногоДокумента", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы);
		
		ПредставлениеКолонки = НСтр("ru='Дата расчетного документа';ka='ანგარიშსწორების დოკუმენტის თარიღი';en='Date of settlement document'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаРасчетногоДокумента", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы);
		
		ПредставлениеКолонки = НСтр("ru='Сумма (вал.)';ka='თანხა (ვალუტა)';en='Amount (currency)'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СуммаВал", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2, , , Истина);
		
		ПредставлениеКолонки = НСтр("ru='Сумма';ka='თანხა';en='Amount'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Сумма", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2);
		
		//ПредставлениеГруппы = НСтр("ru='Счет';ka='ინვოისი';en='Invoice'");
		ПредставлениеГруппы = "Счет"; //Малышев 2024-07-15
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("ДокументСсылка.СчетНаОплату");
		
		ПредставлениеКолонки = НСтр("ru='Номер счета к оплате';ka='ინვოისის (გადახდის მოთხოვნის) ნომერი';en='Account number for Invoice'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерСчетаПокупателя", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы, , , );
		
		ПредставлениеКолонки = НСтр("ru='Дата счета к оплате';ka='ინვოისის (გადახდის მოთხოვნის) თარიღი';en='Invoice due date'");
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаСчетаПокупателя", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы, , , );
	...
	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ВводНачальныхОстатков.ТабличнаяЧасть.РасчетыСПоставщиками" Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ПризнакАванса", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, , , , Ложь);
		
		
		//ПредставлениеГруппы = НСтр("ru='Заказ';ka='შეკვეთა';en='Order'");
		ПредставлениеГруппы = "Заказ"; //Малышев 2024-07-15
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику");
		ПредставлениеКолонки = НСтр("ru='Номер заказа поставщику';ka='შეკვეთა მომწოდებლის დოკუმენტის ნომერი';en='Order number of the supplier'");
		//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерЗаказаПоставщику", "Номер заказа поставщику", ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Заказ");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерЗаказаПоставщику", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы);
		
		ПредставлениеКолонки = НСтр("ru='Дата заказа поставщику';ka='შეკვეთა მომწოდებლის დოკუმენტის თარიღი';en='Data of the order to the supplier'");
		//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаЗаказаПоставщику", "Дата заказа поставщику", ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Заказ");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаЗаказаПоставщику", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповКолонка, ПредставлениеГруппы);
		...
		//ПредставлениеГруппы = НСтр("ru='Счет';ka='ინვოისი (გადახდის მოთხოვნა)';en='Invoice'");
		//ПредставлениеГруппы = НСтр("ru='Счет';ka='ინვოისი';en='Invoice'");
		ПредставлениеГруппы = "Счет"; //Малышев 2024-08-01 ONESOFT-3315
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерСчетаПокупателя", ПредставлениеКолонки, ОписаниеТиповСтрока25, ОписаниеТиповСтрока25, ПредставлениеГруппы);
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		Если ПолноеИмяОбъектаЗаполнения = "Документ.ВводНачальныхОстатков.ТабличнаяЧасть.Запасы" Тогда
			...
			Если ПолучитьФункциональнуюОпцию("УчетПоПроектам") Тогда
				...
				// Ячейка по наименованию
				ЗначениеПоУмолчанию = Справочники.Проекты.БезПроекта;
				ПриОпределенииЗначенияПоУмолчанию(СтрокаТаблицыФормы.Номенклатура, "Проект", СтрокаТаблицыФормы.Проект_ВходящиеДанные, ЭтаСтрокаСопоставлена, ЗначениеПоУмолчанию);
				ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьПроект(СтрокаТаблицыФормы.Проект, СтрокаТаблицыФормы.Проект_ВходящиеДанные, ЗначениеПоУмолчанию);
...
Процедура СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаЗапасыНаСкладах",ТзЗапасы);
	...
	|	ТаблицаЗапасыНаСкладах.СрокГодности КАК СрокГодности
	|ПОМЕСТИТЬ ТаблицаЗапасыНаСкладах
	|ИЗ
	|	&ТаблицаЗапасыНаСкладах КАК ТаблицаЗапасыНаСкладах
	|ГДЕ
	|	ТаблицаЗапасыНаСкладах.СрокГодности <> ДАТАВРЕМЯ(1, 1, 1)
	|	И ТаблицаЗапасыНаСкладах.ВключенВрегистр = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТТаблицаЗапасыНаСкладах.ВидДвижения КАК ВидДвижения,
	|	ВТТаблицаЗапасыНаСкладах.Документ КАК Документ,
	|	ВТТаблицаЗапасыНаСкладах.Организация КАК Организация,
	|	ВТТаблицаЗапасыНаСкладах.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВТТаблицаЗапасыНаСкладах.Период КАК Период,
	|	ВТТаблицаЗапасыНаСкладах.Номенклатура КАК Номенклатура,
	|	ВТТаблицаЗапасыНаСкладах.Характеристика КАК Характеристика,
	|	ВТТаблицаЗапасыНаСкладах.Количество КАК Количество,
	|	ВТТаблицаЗапасыНаСкладах.СрокГодности КАК СрокГодности
	|ИЗ
	|	ТаблицаЗапасыНаСкладах КАК ВТТаблицаЗапасыНаСкладах";
	
	
	РезултатЗапроса = Запрос.Выполнить().Выгрузить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКонтрольСроковГодностиТовара", РезултатЗапроса);
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоКредитамИЗаймам(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт

	Запрос = Новый Запрос(  
	"
	//Малышев 2024-06-13 ONESOFT-3095
	|ВЫБРАТЬ
	|	ВводНачальныхОстатковКредитыИЗайми.Ссылка КАК Ссылка,
	|	ВводНачальныхОстатковКредитыИЗайми.НомерСтроки КАК НомерСтроки,
	|	ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма КАК ДоговорКредитаЗайма,
	|	ВводНачальныхОстатковКредитыИЗайми.Контрагент КАК Контрагент,
	|	ВводНачальныхОстатковКредитыИЗайми.Валюта КАК Валюта,
	|	ВводНачальныхОстатковКредитыИЗайми.ОсновнойДолг КАК ОсновнойДолг,
	|	ВводНачальныхОстатковКредитыИЗайми.ОсновнойДолгВал КАК ОсновнойДолгВал,
	|	ВводНачальныхОстатковКредитыИЗайми.Проценты КАК Проценты,
	|	ВводНачальныхОстатковКредитыИЗайми.ПроцентыВал КАК ПроцентыВал,
	|	ВЫБОР
	|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)  
	|		КОГДА ВводНачальныхОстатковКредитыИЗайми.Проект <> ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА ВводНачальныхОстатковКредитыИЗайми.Проект
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
	|	КОНЕЦ КАК Проект
	|ПОМЕСТИТЬ ТаблицаДокумента
	|ИЗ
	|	Документ.ВводНачальныхОстатков.КредитыИЗайми КАК ВводНачальныхОстатковКредитыИЗайми
	|ГДЕ
	|	ВводНачальныхОстатковКредитыИЗайми.Ссылка = &Ссылка
	//Малышев 2024-06-13 ONESOFT-3095 --)
	|;
	|ВЫБРАТЬ
	|	МИНИМУМ(ВводНачальныхОстатковКредитыИЗайми.НомерСтроки) КАК НомерСтроки,
	|	ВводНачальныхОстатковКредитыИЗайми.Ссылка.Дата КАК Период,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику)
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		КОГДА ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.КредитВыданный)
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	КОНЕЦ КАК ВидДвижения,
	|	&Организация КАК Организация,
	|	СУММА(ВводНачальныхОстатковКредитыИЗайми.ОсновнойДолг) КАК ОсновнойДолг,
	|	СУММА(ВводНачальныхОстатковКредитыИЗайми.ОсновнойДолгВал) КАК ОсновнойДолгВал,
	|	СУММА(ВводНачальныхОстатковКредитыИЗайми.Проценты) КАК Проценты,
	|	СУММА(ВводНачальныхОстатковКредитыИЗайми.ПроцентыВал) КАК ПроцентыВал,
	|	ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма КАК ДоговорКредитаЗайма,
	|	ВводНачальныхОстатковКредитыИЗайми.Контрагент КАК Контрагент,
	|	ВводНачальныхОстатковКредитыИЗайми.Проект, //Малышев 2024-06-13 ONESOFT-3095 --)
	|	ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма.ВидДоговора КАК ВидДоговора
	|ИЗ
	//|	Документ.ВводНачальныхОстатков.КредитыИЗайми КАК ВводНачальныхОстатковКредитыИЗайми
	|	ТаблицаДокумента КАК ВводНачальныхОстатковКредитыИЗайми  //Малышев 2024-06-13 ONESOFT-3095
	|ГДЕ
	|	ВводНачальныхОстатковКредитыИЗайми.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВводНачальныхОстатковКредитыИЗайми.Ссылка.Дата,
	|	ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма,
	|	ВводНачальныхОстатковКредитыИЗайми.Проект, //Малышев 2024-06-13 ONESOFT-3095 --)
	|	ВводНачальныхОстатковКредитыИЗайми.Контрагент,
	|	ВводНачальныхОстатковКредитыИЗайми.ДоговорКредитаЗайма.ВидДоговора
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический) КАК СценарийПланирования,
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.ОсновнойДолг КАК Сумма,
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Вспомогательный) КАК СчетДт,
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ТаблицаДокумента.ДоговорКредитаЗайма.СчетУчетаРегл КАК СчетКт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ДоговорКредитаЗайма.СчетУчетаРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Валюта
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаКт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ДоговорКредитаЗайма.СчетУчетаРегл.Валютный
	|			ТОГДА ТаблицаДокумента.ОсновнойДолгВал
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВалКт,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	ТаблицаДокумента.Контрагент КАК СубконтоКт1,
	|	ТаблицаДокумента.ДоговорКредитаЗайма КАК СубконтоКт2,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИЗаймов.ОсновнойДолг) КАК СубконтоКт3,
	|	"""" КАК ВидСубконтоДт1,
	|	"""" КАК ВидСубконтоДт2,
	|	"""" КАК ВидСубконтоДт3,
	|	""КонтрагентыСотрудники"" КАК ВидСубконтоКт1,
	|	""ДоговорыКредитовИЗаймов"" КАК ВидСубконтоКт2,
	|	""ТипыСуммГрафикаКредитовИЗаймов"" КАК ВидСубконтоКт3,
	|	ТаблицаДокумента.Проект КАК ПроектДт,  //Малышев 2024-06-13 ONESOFT-3095
	|	ТаблицаДокумента.Проект КАК ПроектКт,  //Малышев 2024-06-13 ONESOFT-3095
	|	&Основной КАК Содержание
	|ИЗ
	//|	Документ.ВводНачальныхОстатков.КредитыИЗайми КАК ТаблицаДокумента
	|	ТаблицаДокумента КАК ТаблицаДокумента  //Малышев 2024-06-13 ONESOFT-3095
	|ГДЕ
	|	ТаблицаДокумента.ОсновнойДолг <> 0
	|	И ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.КредитПолученный)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический),
	|	&Организация,
	|	ТаблицаДокумента.Проценты,
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Вспомогательный),
	|	НЕОПРЕДЕЛЕНО,
	|	0,
	|	ТаблицаДокумента.ДоговорКредитаЗайма.СчетУчетаПроцентовРегл,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ДоговорКредитаЗайма.СчетУчетаПроцентовРегл.Валютный
	|			ТОГДА ТаблицаДокумента.Валюта
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ДоговорКредитаЗайма.СчетУчетаПроцентовРегл.Валютный
	|			ТОГДА ТаблицаДокумента.ПроцентыВал
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО,
	...
	|	""ДоговорыКредитовИЗаймов"",
	|	""ТипыСуммГрафикаКредитовИЗаймов"",
	|	ТаблицаДокумента.Проект КАК ПроектДт,  //Малышев 2024-06-13 ONESOFT-3095
	|	ТаблицаДокумента.Проект КАК ПроектКт,  //Малышев 2024-06-13 ONESOFT-3095
	|	&Процент
	|ИЗ
	...
	|	0,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику)
	|			ТОГДА ТаблицаДокумента.ДоговорКредитаЗайма.Сотрудник
	|		ИНАЧЕ ТаблицаДокумента.ДоговорКредитаЗайма.Контрагент
	|	КОНЕЦ,
	|	ТаблицаДокумента.ДоговорКредитаЗайма,
	...
	|	ТаблицаДокумента.ОсновнойДолг <> 0
	|	И ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.ДоговорКредитаЗайма.ВидДоговора В (ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.КредитВыданный))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("Процент", "პროცენტი");
	Запрос.УстановитьПараметр("Основной", "ძირითადი");
	Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам); //Малышев 2024-06-13 ONESOFT-3095
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	
	//Малышев 2024-06-13 ONESOFT-3095 (--
    //СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыПоКредитамИЗаймам", МассивРезультатов[0].Выгрузить());
    //СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаХозрасчетный", МассивРезультатов[1].Выгрузить());	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыПоКредитамИЗаймам", МассивРезультатов[1].Выгрузить());
    СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаХозрасчетный", МассивРезультатов[2].Выгрузить());	
	//Малышев 2024-06-13 ONESOFT-3095 --)
