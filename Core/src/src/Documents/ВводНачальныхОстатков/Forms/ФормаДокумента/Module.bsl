...
// Олег Литвиненко 05.07.23 --->
&НаКлиенте
Процедура ПрямыеЗатратыНоменклатураПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ПрямыеЗатраты.ТекущиеДанные;
	
	СтрокаТабличнойЧасти.СчетЗатратРегл = ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.Номенклатура, "СчетУчетаЗатратРегл");
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.СрокГодности = ?(СтруктураДанные.СрокГодности >0,Объект.Дата + СтруктураДанные.СрокГодности * 86400,Дата(1,1,1));//+Lazare 17.10.2022
...
#Область ОбработчикиСобытийЭлементовТаблицыКредитыИЗайми
...
Процедура КредитыИЗаймиДоговорКредитаЗаймаПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДоговорКредитаЗайма) Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, ДанныеДоговораКредитаЗайма(СтрокаТабличнойЧасти.ДоговорКредитаЗайма));
...
Процедура КредитыИЗаймиОсновнойДолгПриИзменении(Элемент)
	...
	Если СтрокаТабличнойЧасти.Валюта = НациональнаяВалюта Тогда
		СтрокаТабличнойЧасти.ОсновнойДолгВал = СтрокаТабличнойЧасти.ОсновнойДолг;
		Возврат;
	КонецЕсли;
	
	ДанныеСтроки = Новый Структура("Валюта, ОсновнойДолг, ОсновнойДолгВал");
	ЗаполнитьЗначенияСвойств(ДанныеСтроки, СтрокаТабличнойЧасти);
	
	КредитыИЗаймиПересчитатьВалютнуюСуммуСтроки(ДанныеСтроки, "ОсновнойДолг");
...
&НаКлиенте
Процедура КредитыИЗаймиПроцентыПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.КредитыИЗайми.ТекущиеДанные;
	
	Если СтрокаТабличнойЧасти.Валюта = НациональнаяВалюта Тогда
		СтрокаТабличнойЧасти.ПроцентыВал = СтрокаТабличнойЧасти.Проценты;
		Возврат;
	КонецЕсли;
	
	ДанныеТекущейСтроки = Новый Структура("Валюта, Проценты, ПроцентыВал");
	ЗаполнитьЗначенияСвойств(ДанныеТекущейСтроки, СтрокаТабличнойЧасти);
	
	КредитыИЗаймиПересчитатьВалютнуюСуммуСтроки(ДанныеТекущейСтроки, "Проценты");
	
	ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, ДанныеТекущейСтроки);
...
Процедура СформироватьСписокРазделовУчета()
	...
	Элементы.РазделУчета.СписокВыбора.Добавить("Кредиты и займи", НСтр("ka='კრედიტები და სესხები';ru='Кредиты и займы'"));
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	СтруктураДанные.Вставить("СрокГодности", СтруктураДанные.Номенклатура.СрокГодности); //+++ Lazare 17.10.2022
...
&НаСервереБезКонтекста
Функция ДанныеДоговораКредитаЗайма(ДоговорКредитаЗайма)
	
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ДоговорКредитаЗайма,
		"ВидДоговора, Контрагент, Сотрудник, ВалютаРасчетов");
	
	ДанныеДоговора = Новый Структура;
	
	ДанныеДоговора.Вставить("ВидДоговора", РеквизитыДоговора.ВидДоговора);
	ДанныеДоговора.Вставить("Валюта", РеквизитыДоговора.ВалютаРасчетов);
	Если РеквизитыДоговора.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику Тогда
		ДанныеДоговора.Вставить("Контрагент", РеквизитыДоговора.Сотрудник);
	Иначе
		ДанныеДоговора.Вставить("Контрагент", РеквизитыДоговора.Контрагент);
	КонецЕсли;
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Если Объект.РазделУчета = "Кредиты и займи" И Объект.КредитыИЗайми.Количество() > 0 Тогда
		
		ДоговорыКредитаЗайма = ОбщегоНазначения.ВыгрузитьКолонку(Объект.КредитыИЗайми, "ДоговорКредитаЗайма");
		РеквизитыДоговоров = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ДоговорыКредитаЗайма, "ВидДоговора");
		Для Каждого СтрокаТаблицы Из Объект.КредитыИЗайми Цикл
			СтрокаТаблицы.ВидДоговора = РеквизитыДоговоров[СтрокаТаблицы.ДоговорКредитаЗайма];
		КонецЦикла;
...
&НаСервере
Процедура КредитыИЗаймиПересчитатьВалютнуюСуммуСтроки(СвойстваСтроки, ИмяПоляСумма)
	...
	Запрос.УстановитьПараметр("ВалютаДок", СвойстваСтроки.Валюта);
	...
	СвойстваСтроки[ИмяПоляСумма + "Вал"] = СвойстваСтроки[ИмяПоляСумма + "Вал"] * СтрокаКурсов.КратностьНац * СтрокаКурсов.КурсДок / (СтрокаКурсов.КратностьДок * СтрокаКурсов.КурсНац);
