...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОтчетОтКомиссионера", НСтр("ka='კომისიონერის ანგარიშგება';ru='Отчет комиссионера';en='The report of the commission'"));
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
				...
				СтрокаТаблицаУправленческий.Содержание = НСтр("ka='გაყიდვის თვითღირებულება';ru='Себестоимость продажи';en='Cost of sales'");
				...
				Если СтрокаТаблицаЗапасы.УдержатьКомиссионноеВознаграждение Тогда
					...
					НоваяСтрока.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаСебестоимостьРегл;
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ОтчетКомиссионераЗапасы.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателемРегл КАК СчетУчетаРасчетовСПокупателемРегл,//+++
	...
	|		ИНАЧЕ ОтчетКомиссионераЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаВыручкиОтПродажРегл
	|	КОНЕЦ КАК СчетУчетаПродажиРегл,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаШапка.ВестиУчетРасходовПоДоговорамОбслуживания
	|			ТОГДА ВременнаяТаблицаШапка.НаправлениеДеятельности.СчетУчетаСебестоимостиПродажРегл
	|		ИНАЧЕ ОтчетКомиссионераЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаСебестоимостиПродажРегл
	|	КОНЕЦ КАК СчетУчетаСебестоимостьРегл,
	|	ОтчетКомиссионераЗапасы.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
	//---
	...
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_ОтчетКомиссионера(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства);//+++
