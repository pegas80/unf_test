...
Процедура Отправить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.УчетнаяЗапись)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აირჩიეთ სააღრიცხვო ჩანაწერი ფოსტის გასაგზავნად.';ru='Выберите учетную запись для отправки почты.';en='Select the account to send mail.'"), ,
			"Объект.УчетнаяЗапись");
		Возврат;
	КонецЕсли;
	...
	СписокОтветов.Добавить(КодВозвратаДиалога.ОК, НСтр("ka='გაგზავნის დუბლირება';ru='Продублировать отправку';en='sending duplicate'"));
	СписокОтветов.Добавить(КодВозвратаДиалога.Отмена);
	Если ЗначениеЗаполнено(ИдентификаторСобытия) Тогда
		ПоказатьВопрос(
		ОписаниеОповещения,
		СтрШаблон(НСтр("ka='ეს წერილი უკვე გაიგზავნა %1';ru='Это письмо уже отправлялось %1';en='This letter has already dispatched% 1'"), Объект.НачалоСобытия),
		СписокОтветов, 30,
		КодВозвратаДиалога.Отмена,
		НСтр("ka='ეს წერილი უკვე გაიგზავნა';ru='Это письмо уже отправлялось';en='This letter has already dispatched'"),
		КодВозвратаДиалога.Отмена)
	Иначе
...
Процедура ЗаполнитьНовоеПисьмоПоУмолчанию(Параметры)
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='გამავალი წერილი (შექმნა)';ru='Исходящее письмо  (создание)';en='Outgoing mail (creating)'");
...
Функция ПолучитьТабличныйДокументПоДвоичнымДанным(Знач ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='ცხრილი-დოკუმენტის მიღება';ru='Получение табличного документа';en='Getting a spreadsheet document'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , 
...
Процедура УстановитьЗаголовокГиперссылкиДокументовОснований()
	...
	Если Объект.ДокументыОснования.Количество() = 0 Тогда
		Элементы.ОткрытьДокументыОснования.Заголовок = НСтр("ka='საფუძველი დოკუმენტების სია';ru='Список документов оснований';en='List of Reference Document'");
		Возврат;
	КонецЕсли;
	...
	Элементы.ОткрытьДокументыОснования.Заголовок = СтрШаблон(НСтр("ka='%1 и ещё %2.';ru='%1 и ещё %2.';en='% 1 and% 2.'"),
	ПервыйДокумент,
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	Объект.ДокументыОснования.Количество() - 1,
	НСтр("ka='основание,основания,оснований';ru='основание,основания,оснований';en='base, bases, bases'")));
...
Процедура ЗаполнитьТекстОтвета(ДанныеИсходногоСообщения)
	...
	Если ЗначениеЗаполнено(ДанныеИсходногоСообщения.СодержаниеHTML) Тогда
		...
		ТекстОтвета = СтрШаблон(НСтр("ka='<div>&nbsp;</div>"
"---------- საწყისი შეტყობინება ----------<br>"
"გაგზავნილია: %1<br>"
"მონაწილეები: %2<br>"
"თემა: %3<br>"
"<div>&nbsp;</div>"
...
Процедура ПродолжитьОтправкуПисьма(Результат, Параметры) Экспорт
	...
	Если ПараметрыПисьма = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='საფოსტო შეტყობინების პარამეტრების ფორმირების შეცდომა';ru='Ошибка формирования параметров почтового сообщения';en='Error formation mail message settings'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ka='შეტყობინება წარმატებით გაიგზავნა';ru='Сообщение успешно отправлено';en='Message sent successfully'"), ПолучитьНавигационнуюСсылку(Объект.Ссылка), Строка(Объект.Ссылка), БиблиотекаКартинок.Информация32);
	Закрыть(Успешно);
...
Процедура ЗаполнитьСодержаниеСобытия(ТемаСобытия)
	...
	Если НЕ ПустаяСтрока(ФорматированныйДокумент.ПолучитьТекст()) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСодержаниеСобытияЗавершение", ЭтотОбъект, Новый Структура("ТемаСобытия", ТемаСобытия)),
			НСтр("ka='გადავავსოთ არჩეული თემის შემცველობა?';ru='Перезаполнить содержание по выбранной теме?';en='Refill the contents of the selected topic?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ОткрытьВыборШаблонаПоПредмету(ПредметШаблона = Неопределено)
	...
	Если Не ЕстьДоступныеШаблоны(ПредметШаблона) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Нет доступных шаблонов. Добавить новые шаблоны можно в списке шаблонов: Еще больше возможностей - CRM - Шаблоны сообщений';ru='Нет доступных шаблонов. Добавить новые шаблоны можно в списке шаблонов: Еще больше возможностей - CRM - Шаблоны сообщений';en='No templates available. Add new templates can be in the list of templates: Many more features - CRM - Templates'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьВыборПредметаШаблона(СписокПредметов)
	...
	СписокПредметов.Вставить(0, "Общий", НСтр("ka='<Общие шаблоны>';ru='<Общие шаблоны>';en='<General Templates>'"));
	
	СписокПредметов.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ka='Выбор предмета шаблона';ru='Выбор предмета шаблона';en='Selection template object'"));
