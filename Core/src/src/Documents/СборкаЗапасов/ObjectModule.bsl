...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения) Экспорт
	...
	|	ЗаказыОстатки.Проект КАК Проект,    //Малышев 2024-06-26 ONESOFT-3279
	...
	|		ДвиженияДокументаЗаказыНаПроизводство.Проект,  //Малышев 2024-06-26 ONESOFT-3279
	...
	|	ЗаказыОстатки.Проект,   //Малышев 2024-06-26 ONESOFT-3279
	...
	Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийЗаказНаПроизводство.Разборка Тогда
		...
		|		ЗаказНаПроизводствоПродукция.Проект КАК Проект,  //Малышев 2024-06-26 ONESOFT-3279
		...
		|		СборкаЗапасовПродукция.Проект,  //Малышев 2024-06-26 ONESOFT-3279
	...
	Иначе
		...
		|		ЗаказНаПроизводствоЗапасы.Проект КАК Проект,  //Малышев 2024-06-26 ONESOFT-3279
		...
		|		СборкаЗапасовЗапасы.Проект,   //Малышев 2024-06-26 ONESOFT-3279
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//+++ Дмгг, ONESOFT-2638    
	Если ПолучитьФункциональнуюОпцию("УчитыватьДополнительныеЗатратыВПроизводстве") Тогда 
		Для Каждого СтрокаДопРасходов Из ДополнительныеРасходы Цикл 
			
			Если ЗначениеЗаполнено(СтрокаДопРасходов.Контрагент) И ТипЗнч(СтрокаДопРасходов.Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ ЗначениеЗаполнено(СтрокаДопРасходов.Договор) Тогда 
				
				ТекстСообщения = НСтр("ka='Не указан ДоговорКонтрагента!';ru='Не указан ДоговорКонтрагента!';en='Не указан ДоговорКонтрагента!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "ДополнительныеРасходы", СтрокаДопРасходов.НомерСтроки, "Договор",Отказ);
				
			ИначеЕсли ЗначениеЗаполнено(СтрокаДопРасходов.Контрагент) И ТипЗнч(СтрокаДопРасходов.Контрагент) = Тип("СправочникСсылка.Сотрудники") И НЕ ЗначениеЗаполнено(СтрокаДопРасходов.Подразделение) Тогда 
				
				ТекстСообщения = НСтр("ka='Не указан ""Подразделение""!';ru='Не указан ""Подразделение""!';en='Не указан ""Подразделение""!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "ДополнительныеРасходы", СтрокаДопРасходов.НомерСтроки, "Подразделение",Отказ);
				
			ИначеЕсли ЗначениеЗаполнено(СтрокаДопРасходов.Контрагент) И ТипЗнч(СтрокаДопРасходов.Контрагент) = Тип("СправочникСсылка.Сотрудники") И НЕ ЗначениеЗаполнено(СтрокаДопРасходов.ВидНачисленияУдержания) Тогда 
				
				ТекстСообщения = НСтр("ka='Не указан ""ВидНачисленияУдержания""!';ru='Не указан ""ВидНачисленияУдержания""!';en='Не указан ""ВидНачисленияУдержания""!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "ДополнительныеРасходы", СтрокаДопРасходов.НомерСтроки, "ВидНачисленияУдержания",Отказ);
				
			КонецЕсли;  
			
			Если ЗначениеЗаполнено(СтрокаДопРасходов.Контрагент) И ТипЗнч(СтрокаДопРасходов.Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ ЗначениеЗаполнено(СтрокаДопРасходов.Номенклатура) Тогда 
				
				ТекстСообщения = НСтр("ka='Не указан Номенклатура!';ru='Не указан Номенклатура!';en='Не указан Номенклатура!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "ДополнительныеРасходы", СтрокаДопРасходов.НомерСтроки, "Номенклатура",Отказ);
				
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаДопРасходов.Контрагент) И ТипЗнч(СтрокаДопРасходов.Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ ЗначениеЗаполнено(СтрокаДопРасходов.ЕдиницаИзмерения) Тогда 
				
				ТекстСообщения = НСтр("ka='Не указан Ед.!';ru='Не указан Ед.!';en='Не указан Ед.!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения, "ДополнительныеРасходы", СтрокаДопРасходов.НомерСтроки, "ЕдиницаИзмерения",Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
		ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.ЗаполнитьЗначения(Документы.ЗаказПокупателя.ПустаяСсылка(), "ЗаказПокупателя");
		РезервированиеЗапасов_new.ОтразитьРезервированиеЗапасов(ДополнительныеСвойства, Движения, Отказ);
	КонецЕсли;
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	
	//Малышев 2024-11-11 ONESOFT-3593
	Если ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Колонки.Найти("Подразделение")<>Неопределено ТОгда
		Для каждого стр из ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы Цикл
			стр.Подразделение = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
			// Kuzmich, 14.03.2025, (ONESOFT-3863) Alument - Закрытие месяца, закоментировал
			//стр.КоррЗаказПокупателя = Неопределено;
		КонецЦикла;	
	КонецЕсли;	
	...
	//+++ Дмгг
	Если ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда 
		КдючСвязи = Строка(Ссылка.УникальныйИдентификатор());
		УправлениеНебольшойФирмойСервер.ОчиститьПоСсылкеВариантыПроизводстваПродукция(КдючСвязи);
		УправлениеНебольшойФирмойСервер.ОчиститьПоСсылкеВариантыПроизводстваМатериалы(КдючСвязи);
		УправлениеНебольшойФирмойСервер.ОтразитьВариантыПроизводстваПродукция(ДополнительныеСвойства, Движения, Отказ);
		УправлениеНебольшойФирмойСервер.ОтразитьВариантыПроизводстваМатериалы(ДополнительныеСвойства, Движения, Отказ);
	КонецЕсли;
	//--- Дмгг
	//--------------------------------------------------------------------------
	...
	Если НЕ Отказ И ПолучитьФункциональнуюОпцию("УчитыватьДополнительныеЗатратыВПроизводстве")
		...
		УправлениеНебольшойФирмойСервер.ОтразитьНачисленияИУдержанияВРазрезеПроектов(ДополнительныеСвойства, Движения, Отказ);
	...
	Если  ДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара
		И СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад   Тогда 
