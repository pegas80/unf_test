...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='თანხმობა პერსონალურ მონაცემთა დამუშავებაზე (1C:საწარმოს ცხრილი-დოკუმენტი)';ru='Согласие на обработку ПДн (табличный документ 1С:Предприятия)';en='Consent for treating PD (1C spreadsheet document: Enterprises)'");
	
	// В формате Microsoft Word.
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СогласиеНаОбработкуПерсональныхДанных(MSWord)";
	КомандаПечати.Представление = НСтр("ka='თანხმობა პერსონალურ მონაცემთა დამუშავებაზე (დოკუმენტი Microsoft Word-ი)';ru='Согласие на обработку ПДн (документ Microsoft Word)';en='Consent to the processing of PD (Microsoft Word document)'");
	...
	КомандаПечати.Представление = НСтр("ka='პერსონალური მონაცემების დამუშავების თანხმობა (დოკუმენტი OpenOffice.org Writer)';ru='Согласие на обработку ПДн (документ OpenOffice.org Writer)';en='Consent for treating PD (OpenOffice.org Writer document)'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		Шаблон = НСтр("ka='პერსონალური მონაცემების დამუშავებაზე (#[ნომერი]) [სუბიექტი] - [ორგანიზაცია] თანხმობა მიღებულია [მიღებისთარიღი]';ru='Согласие на обработку ПДн [Субъект] - [Организация] (№[Номер] получено [ДатаПолучения])';en='Consent for treating PD [Subject] - [Organization] (№ [ID] received [DataPolucheniya])'");
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='თანხმობა პერსონალური მონაცემების დამუშავებაზე';ru='Согласие на обработку ПДн';en='Consent to the processing of PD'");
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='თანხმობა პერსონალურ მონაცემთა დამუშავებაზე (დოკუმენტი Microsoft Word-ი)';ru='Согласие на обработку ПДн (документ Microsoft Word)';en='Consent to the processing of PD (Microsoft Word document)'"),
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='პერსონალური მონაცემების დამუშავების თანხმობა (დოკუმენტი OpenOffice.org Writer)';ru='Согласие на обработку ПДн (документ OpenOffice.org Writer)';en='Consent for treating PD (OpenOffice.org Writer document)'"),
			ПечатьСогласияНаОбработкуПДн(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ДанныеДляПечатиСогласий(Согласия)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.СрокДействия) Тогда
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-დან %2-ის ჩათვლით.';ru='с %1 по %2.';en='% 1 to% 2.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"), Формат(Выборка.СрокДействия, "ДЛФ=D"));
		Иначе
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-იდან უვადოდ.';ru='с %1 бессрочно.';en='with 1% indefinitely.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"));
		КонецЕсли;
...
Функция ОписаниеСогласия()
	...
	ОписаниеСогласия.ДатаСогласия = НСтр("ka='«____»_______________20___г.';ru='«____»_______________20___г.';en='""____"" ___ _______________ 20 g.'");
	ОписаниеСогласия.Организация = НСтр("ka='<Организация>';ru='<Организация>';en='<Organization>'");
	ОписаниеСогласия.АдресОрганизации = НСтр("ka='<Адрес организации>';ru='<Адрес организации>';en='<Address of organization>'");
	ОписаниеСогласия.ОтветственныйЗаОбработкуПерсональныхДанных = НСтр("ka='<პასუხისმგებელი პირის გვარ-სახელი>';ru='<ФИО ответственного лица>';en='<Name of the responsible person>'");
