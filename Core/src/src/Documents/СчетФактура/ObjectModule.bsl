...
Процедура ЗаполнитьПоПрочимДокументам(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя")
		...
		ВызватьИсключение НСтр("ka='ანგარიშფაქტურის შეყვანა შეიძლება მხოლოდ შეკვეთა-განწესის საფუძველზე!';ru='Счет-фактуру можно ввести только на основании заказ-наряда!';en='Invoice, you can enter only on the basis of the purchase order!'");
	КонецЕсли;
