...
Процедура ПроверитьУникальностьСчетаФактуры()
	...
	Для каждого ТекСтрока Из ДокументыОснования Цикл
		...
		Если ТипЗнч(ТекСтрока.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			...
			Если УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйСчетФактуру(РеквизитыОснования.ДокументОснование) = Неопределено Тогда
				Если РеквизитыОснования.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки Тогда
					ВызватьИсключение НСтр("ka='ანგარიშფაქტურის გასწორების დამატებამდე საფუძველ-დოკუმენტზე შექმენით ანგარიშფაქტურა';ru='Перед добавлением исправления счета-фактуры создайте счет-фактуру у документа-основания';en='Before adding a correction invoice create invoice from a document-base'");
				Иначе
					ВызватьИсключение НСтр("ka='საკორექტირებო ანგარიშფაქტურის დამატებამდე დოკუმენტ-საფუძველზე შექმენით ანგარიშფაქტურა';ru='Перед добавлением корректировочного счета-фактуры создайте счет-фактуру у документа-основания';en='Before adding a correction invoice create invoice from a document-base'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		НСтр("ka='დოკუმენტ ""%1""-ისთვის უკვე შეყვანილია ანგარიშფაქტურა ""%2""."
"კიდევ ერთი დოკუმენტ ""ანგარიშფაქტურა""-ს ჩაწერა არ არის დასაშვები!';ru='Для документа ""%1"" "
...
"Основание ""%ДокументОснование"".';ka='საავანსოდ ანგარიშფაქტურისთვის მონაცემები არ არის! საფუძველი ""%ДокументОснование"".'");
...
Процедура ЗаполнитьПоПрочимДокументам(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя")
		...
		ВызватьИсключение НСтр("ka='ანგარიშფაქტურის შეყვანა შეიძლება მხოლოდ სერვისის დოკუმენტის საფუძველზე!';ru='Счет-фактуру можно ввести только на основании заказ-наряда!';en='Invoice, you can enter only on the basis of the purchase order!'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
		...
		Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
			...
			Для каждого СтрокаТаблицыЗапасов Из Запасы Цикл
				...
				Если ЗначениеЗаполнено(СтрокаТаблицыЗапасов.НомерГТД)
					...
					ТекстОшибки = НСтр("ka='სტრიქონ [%НомерСтрокиСОшибкой%] -ში სწორად არ არის მითითებული წარმოშობის ქვეყანა';ru='В строке [%НомерСтрокиСОшибкой%] не верно указана страна происхождения';en='In line [% NomerStrokiSOshibkoy%] is not true the country of origin'");
				...
				Если НЕ ВидОперации = Перечисления.ВидыОперацийСчетФактура.НаАвансКомитента
					...
					ТекстОшибки = НСтр("ka='სტრიქონ [%НомерСтрокиСОшибкой%] -ში არ არის მითითებული საბაჟო დეკლარაციის ნომერი';ru='В строке [%НомерСтрокиСОшибкой%] не указан номер ГТД';en='In line [% NomerStrokiSOshibkoy%] Unknown number of GTE'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтрокиСОшибкой%", СокрЛП(СтрокаТаблицыЗапасов.НомерСтроки));
	...
	Для каждого СтрокаТаблицы Из МассивДокументовОснований Цикл
		...
		Если ЗначениеЗаполнено(ДокументОснование)
			...
			ТекстОшибки = НСтр("ka='Документ-основание %ПредставлениеДокументаОснования% не проведен. Проведение счет фактуры невозможно.';ru='Документ-основание %ПредставлениеДокументаОснования% не проведен. Проведение счет фактуры невозможно.';en='Paper base PredstavlenieDokumentaOsnovaniya%% not conducted. Carrying through the invoice is not possible.'");
	...
	Если НеобходимоОтразитьРублевыеСуммыДокумента() Тогда
		Если ДокументыОснования.Количество() > 1 Тогда
			...
			Для Каждого ТекущаяСтрока Из Запасы Цикл
				Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.ДокументОснование) Тогда
					ТекстОшибки = НСтр("ru='В строке [%НомерСтрокиСОшибкой%] не указан документ-основание.';en='On line [% NomerStrokiSOshibkoy%] Unknown-document basis.'");
			...
			Если СтоимостьДоставкиБезНДС > 0 И НЕ ЗначениеЗаполнено(ДокументОснованиеДоставки) Тогда
				ТекстОшибки = НСтр("ru='На закладке ""Доставка"" не указан документ-основание.';en='On the ""Delivery"" Unknown-document basis.'");
		...
		ИначеЕсли ДокументыОснования.Количество() = 1 И НЕ ЗначениеЗаполнено(ДокументыОснования[0].ДокументОснование) Тогда
			...
			ТекстОшибки = НСтр("ka='Не выбран документ-основание.';ru='Не выбран документ-основание.';en='Do not select a document-basis.'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ДокументОснование) И НЕ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
		...
		Если ЗначениеЗаполнено(НайденныйСчетФактура) И НайденныйСчетФактура.Ссылка <> Ссылка Тогда
			...
			ТекстСообщения = НСтр("ka='დოკუმენტ ""%Ссылка%"" უკვე შეყვანილია ანგარიშფაქტურა ""%СчетФактура%"". "
"კიდევ ერთი დოკუმენტ ""ანგარიფაქტურის"" შეყვანა დაუშვებელია!';ru='Для документа ""%Ссылка%"" "
