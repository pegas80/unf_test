...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСчетФактура, СтруктураДополнительныеСвойства) Экспорт
	...
	|			ИНАЧЕ ВЫРАЗИТЬ(СчетФактураЗапасы.Количество * СчетФактураЗапасы.ЕдиницаИзмерения.Коэффициент - &СмещениеДляОкругления КАК ЧИСЛО(15,3))
...
Процедура ВыполнитьКонтроль(ДокументСсылкаСчетФактура, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыВРазрезеГТДИзменение Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			...
			ТекстСообщения = НСтр("ka='%ПредставлениеНоменклатурыТекст% - მარაგების უარყოფითი ნაშთი საბაჟო დეკლარაციების თვალსაზრისით."
"მარაგების ნაშთი საბაჟო დეკლარაციების თვალსაზრისით (რაოდენობა): %КоличествоОстаток%.';ru='%ПредставлениеНоменклатурыТекст% - отрицательный остаток запасов в разрезе ГТД."
...
Функция ПолучитьПараметрыЗаполненияКорректировочногоСчетаФактуры(ДокументОснование) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОснованиеДляВидаОперации) Тогда
		ВызватьИсключение НСтр("ka='გასასწორებელი დოკუმენტი არ არის დადგენილი';ru='Не определен исправляемый документ';en='corrects the document is not defined'");
	КонецЕсли;
...
"Для документа %1 печатная форма не сформирована.'");
		...
		Если Шапка.ВидОперации <> Перечисления.ВидыОперацийСчетФактура.Аванс 
			...
			Если Шапка.ОнЖе Тогда
				...
				ОбластьМакета.Параметры.ПредставлениеГрузоотправителя = ЗаголовокПоля + НСтр("ka='იგივე';ru='он же';en='he is'");
			...
			Для Каждого Строчка Из ТаблицаПоТоварам Цикл
				...
				Если Шапка.ДатаДокумента >= Дата(2016, 07, 01) Тогда
					...
					Если Строчка.СтавкаНДС = СтавкаНДС0
						...
						ОбластьМакета.Параметры.ТоварНаименование = ОбластьМакета.Параметры.ТоварНаименование + НСтр("ru=', код ТН ВЭД ';ka=', код ТН ВЭД '") + Строчка.ТНВЭДКод;
				...
				Количество = Окр(Строчка.Количество * Коэффициент - СмещениеДляОкругления, 3);
				// Олег Литвиненко <---
		...
		Если ЭтоУниверсальныйПередаточныйДокумент Тогда
			...
			Если ЗначениеЗаполнено(Шапка.ДокументОснование) Тогда
				...
				Если ЗначениеЗаполнено(Шапка.Вес) Тогда
					...
					СтрШаблон(НСтр("ru='масса брутто: %1';en='Gross weight: 1%'"), НРег(МассаГрузаПрописью(Шапка.Вес))));
				КонецЕсли; 
				Если ЗначениеЗаполнено(Шапка.Объем) Тогда
					МассаИОбъем.Добавить(
					СтрШаблон(НСтр("ru='объем: %1';en='volume: 1%'"), ОбъемГрузаПрописью(Шапка.Объем)));
				КонецЕсли;
				...
				Тогда
					...
					Тогда
						...
						ОбластьМакета.Параметры.Основание = 
							Шапка.ОснованиеПечати + НСтр("ka='; მინდობილობით #';ru='; по доверенности №';en='; proxy number'") + Шапка.ДокументОснование.ДоверенностьНомер 
							+ НСтр("ru=' от ';ka='ვისგან'") + Формат(Шапка.ДокументОснование.ДоверенностьДата, "ДЛФ=DD") 
							+ НСтр("ru=' выданной ';ka=' выданной '") + Шапка.ДокументОснование.ДоверенностьВыдана + " " 
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйКорректировочныйДокумент = Ложь) Экспорт
	...
	КоличествоЛистов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დოკუმენტი შედგენილია %1%2 %3-ისთვის';ru='Документ составлен на%1%2 %3';en='The document is% 1% 2% 3'"), Символы.ПС, НомерЛиста,
		УправлениеНебольшойФирмойСервер.ФормаМножественногоЧисла(
			НСтр("ka='ფურცელს';ru='листе';en='sheet'"), НСтр("ka='გვერდებზე';ru='листах';en='sheets'"), НСтр("ka='გვერდებზე';ru='листах';en='sheets'"), НомерЛиста));
