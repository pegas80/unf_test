...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	И НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратЗаймаСотрудником") И
			Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") И
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратЗаймаСотрудником") Или
		ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") Или
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ СчетФактураТекст = ""
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗаписиЗавершение", ЭтотОбъект),
			НСтр("ka='დოკუმენტში განხორციელდა ცვლილებები. საჭიროა თუ არა დაქვემდებარებული ანგარიშფაქტურის განმეორებით შევსება?';ru='В документе были произведены изменения. Требуется ли повторно заполнить подчиненный Счет-фактуру?';en='The document changes were made. Do I need to re-fill the subordinate invoice?'"),
...
Процедура СуммаДокументаПриИзменении(Элемент)
	...
	//Паганини ПФ_
	ПоступленнаяСуммаПриИзменении(Неопределено);
	//_Паганини ПФ
...
Процедура ПересчитатьСуммыВСтрокиРасходов(Идентификатор, ИмяПоля)
	Если Идентификатор = Неопределено Тогда
		Для Каждого СтрокаТЧ ИЗ Объект.РасшифровкаПлатежа Цикл
			...
			РС_ОбщегоНазначения.ПересчитатьСуммыСтроки_ПоступлениеВКассу_ПоступлениеНаСчетa(ИмяПоля, ТекДанные);
		КонецЦикла;
	Иначе
		ТекДанные = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(Идентификатор);
		РС_ОбщегоНазначения.ПересчитатьСуммыСтроки_ПоступлениеВКассу_ПоступлениеНаСчетa(ИмяПоля, ТекДанные);
...
Процедура НапечататьЧек(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		МенеджерОборудованияКлиент.ПредложитьВыбратьУстройство(ОписаниеОповещения, МассивОборудования,
				НСтр("ru = 'Выберите оборудование'; ka = 'აღჭურვილობის არჩევა'; en = 'Select equipment'"), НСтр("ka='მოწყობილობა არ არის შეერთებული.';ru='Оборудование не подключено.';en='The equipment is not connected.'"));
...
Процедура ДополнитьАвансамиИОплатамиПоТаблицеДокумента(ДокументОбъект, ОбщиеПараметры)
	...
	НаименованиеАванса = 
	НСтр("ru = 'Аванс от: '; ka = 'ავანის დან: '")
	+ ДокументОбъект.ПринятоОт
	+ НСтр("ru = ', Основание: '; ka = ', საფუძველი: '")
	...
	Для каждого ТекСтрока Из ДокументОбъект.РасшифровкаПлатежа Цикл
		...
		Иначе
			НаименованиеТовара = 
			НСтр("ru = 'Оплата от: '; ka = 'გადახდა დან: '")
			+ ДокументОбъект.ПринятоОт
			+ НСтр("ru = ', Основание: '; ka = ', საფუძველი: '")
			+ ТекСтрока.Документ;
...
Функция ПодготовитьДанныеДляПробитияЧека(ИдентификаторУстройстваФР, ЭтоВозврат) 
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		ТекстСообщения = НСтр("ru = 'Не найдена касса ККМ соответсвующая устройству %устройство%.'; ka = 'მოწყობილობის შესაბამისი სალარო საკასო აპარატი ვერ მოიძებნა %устройство%.'; en = 'Found cash register POS Given suitable device% unit%.'");
...
Процедура УстановитьСтатьюДДСПриСменеВидаОперации()
	...
	ИначеЕсли (Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПодотчетника
		...
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Контрагент, Организация, Дата)
	...
	// Прочие расчеты
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.РасчетыПоКредитам
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом Тогда
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	Элементы.РозничнаяВыручкаРасшифровкаПлатежаСуммаНДС.Видимость = Ложь;
	Элементы.НалогообложениеНДС.Видимость = Ложь;
	Элементы.ДокументыПланирования.Заголовок = НСтр("ka='დაგეგმვა';ru='Планирование';en='Planning'");
	...
	//Паганини_
	Элементы.ГруппаСдача.Видимость = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПоказатьСдачиВДокументеПоступлениеВКассу") И
									 Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ОтПокупателя");
	//_Паганини
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ОтПокупателя") Тогда
		...
		Элементы.Контрагент.Заголовок = НСтр("ru = 'Покупатель'; ka = 'მყიდველი'; en = 'Customer'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.РозничнаяВыручка") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			Элементы.ДокументыПланирования.Заголовок = НСтр("ka='დაგეგმვა, დღგ';ru='Планирование, НДС';en='Planning, VAT'");
		Иначе
			Элементы.ДокументыПланирования.Заголовок = НСтр("ka='დღგ';ru='НДС';en='VAT'");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") Тогда
...
Процедура ЗаполнитьДолги(ПересчитыватьСлужебныеДанные)
	...
	|	РасчетыСПокупателямиОстатки.СтатьяДДС, //+++ Дмгг
	...
	Если РезультатЗакрытия = КодВозвратаДиалога.Да Тогда
		...
		Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
			...
			ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратЗаймаСотрудником")
				...
				Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") Тогда
				ПересчитатьСуммыДокумента(Курс, Кратность, Истина, ДополнительныеПараметры);
			// Конец Прочие расчеты
			Иначе
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратЗаймаСотрудником") Тогда
		...
		КонецЦикла;
		Объект.СчетОрганизации = Неопределено;
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") Тогда
		Объект.Корреспонденция = Неопределено;
		Объект.КорреспонденцияРегл = Неопределено; //+++ Дмгг
		Объект.Контрагент = Неопределено;
		Объект.Подотчетник = Неопределено;
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	Элементы.ТоварныйЧек.Видимость = ((Константы.ИспользоватьТоварныйЧекПоступлениеРасходКассы.Получить()) И (Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя)); // Kuzmich, 06.08.2019, Использовать товарный чек (Поступление в кассу, Расход из кассы
...
Процедура НастроитьЭлементДоговораЗайма()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом Тогда
...
Процедура УстановитьВидимостьПроекта()
	...
	ВидимостиПроекта = ВидОперации <> Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя
...
Процедура ЗаполнитьПоДоговоруКредита(Команда)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") Тогда
...
Процедура ЗаполнитьИнформациюПоКредитуЗаймуНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратЗаймаСотрудником")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом") Тогда
...
Процедура ЗаполнитьИнформациюПоКредитуНаСервере()
	...
	Если Выборка.Следующий() Тогда
		...
		//Объект.ВалютаДенежныхСредств = Выборка.ВалютаРасчетов; //+Lazare. Закоментировал 24.01.2023.
...
Функция ПолучитьДоговорКредитаЗаймаПоУмолчанию(Документ, Контрагент, Организация, ВидОперации)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом Тогда
...
Процедура УстановитьПараметрыВыбораПоМетаданнымДляЭлементаСчета()
	...
	//+++ Дмгг
	ЭлементРегл = Элементы.РасчетыПрочиеКорреспонденция;
...
Функция ДобавитьСтрокуАвансаНаКлиенте()
	...
	ИдентификаторСтрокиАванса = НоваяСтрока.ПолучитьИдентификатор();
	ЗаполнитьРасшифровкаПлатежаДоговорНаСервере(ИдентификаторСтрокиАванса);
...
Процедура ДобавитьДоговорЗачетаДолговНажатиеНаСервере(ТекущаяСтрокаТаблицыДоговоровАвтоЗачета = Неопределено)
	...
	// +giga Добавил проверку повторения элементов
	Если ПоследнийЭлементДоговор <> ("ДоговорАвтоЗачетаДолгов_" + ИндексДоговораУчетаРасчетов) Тогда
		ПоследнийЭлементДоговор = ("ДоговорАвтоЗачетаДолгов_" + ИндексДоговораУчетаРасчетов);
...
Процедура ДобавитьСуммуПлатежаЗачетаДолгов(ИндексДоговораУчетаРасчетов)
	...
	// Поле суммы.
	// +giga Добавил проверку повторения элементов
	Если ПоследнийЭлементСумма <> ("ДоговорАвтоЗачетаДолгов_" + ИндексДоговораУчетаРасчетов) Тогда
		ПоследнийЭлементСумма = ("ДоговорАвтоЗачетаДолгов_" + ИндексДоговораУчетаРасчетов);
...
Процедура ДобавитьДоговорЗачетаДолговСПроектамиНажатиеНаСервере(ТекущаяСтрокаТаблицыДоговоровАвтоЗачета = Неопределено)
	...
	//);
	ИмяЭлемента = "ДоговорАвтоЗачетаДолгов_" + ИндексДоговораУчетаРасчетов;
	ПолеДоговора = Элементы.Найти(ИмяЭлемента);
	Если ПолеДоговора = Неопределено Тогда
	
		ПолеДоговора = Элементы.Добавить(ИмяЭлемента,
		Тип("ПолеФормы"),
		ГруппаДоговоровСПроектами
		);
	...
	// © Шишка В.В. [9 августа 2022]  НАЧАЛО
	//ПолеДоговора.ПутьКДанным = "Объект.ДоговорыАвтоЗачетаДолгов[" + ИндексДоговораУчетаРасчетов + "].Договор";
	Если Не ЗначениеЗаполнено(ПолеДоговора.ПутьКДанным)
		ИЛИ ПолеДоговора.ПутьКДанным <> "Объект.ДоговорыАвтоЗачетаДолгов[" + ИндексДоговораУчетаРасчетов + "].Договор"
		Тогда
...
&НаКлиенте
Процедура ПоступленнаяСуммаПриИзменении(Элемент)
	//Объект.Сдача = ?(Объект.ПоступленнаяСумма > Объект.СуммаДокумента,  Объект.ПоступленнаяСумма - Объект.СуммаДокумента, 0);
	Объект.Сдача = 0;
	Объект.Сдача = Объект.ПоступленнаяСумма - Объект.СуммаДокумента ;
КонецПроцедуры
// Kuzmich, 06.08.2019, Использовать товарный чек (Поступление в кассу, Расход из кассы

//+++ Дсгг
&НаСервере
Процедура ЗаполнитьТаблицаДолговДоговорНаСервере(УИД)
	
	ТекСтрока = ТаблицаДолгов.НайтиПоИдентификатору(УИД);
...
&НаКлиенте
Процедура ТаблицаДолговДоговорПриИзменении(Элемент)
	
	ТекСтрока = Элементы.ТаблицаДолгов.ТекущиеДанные;
	УИД = ТекСтрока.ПолучитьИдентификатор();
	ЗаполнитьТаблицаДолговДоговорНаСервере(УИД); 
