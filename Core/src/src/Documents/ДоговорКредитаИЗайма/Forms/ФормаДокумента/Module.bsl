...
Процедура УстановитьЗначенияПоУмолчаниюДляВидаДоговора()
	...
	//+++ Дмгг
	Если ИспользоватьТолькоРегламентированныйПланСчетов Тогда 
		Объект.СчетУчета = ПланыСчетов.Управленческий.ПустаяСсылка();
		Объект.СчетУчетаКомиссии = ПланыСчетов.Управленческий.ПустаяСсылка();
		Объект.СчетУчетаПроцентов = ПланыСчетов.Управленческий.ПустаяСсылка();
...
Процедура СуммаВключаетНДФЛриИзменении()
	...
	Если Объект.СуммаВключаетНДФЛ И НЕ Объект.ВидДоговора = ВидДоговораДоговорЗайма Тогда
