...
Процедура ИнициализироватьДанныеДокумента(ДокументТабель, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ДокументТабель.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ВцеломЗаПериод Тогда
		...
		Для Счетчик = 1 По 6 Цикл
			...
			|	ТабельОтработанноеВремяЗаПериод.Ссылка.Проект 				КАК Проект,
			//ПодольскийАИ #УчетПроектов 20.10.2020 }
	...
	Иначе        
		...
		Для Счетчик = 1 По 31 Цикл
			...
			//ПодольскийАИ #УчетПроектов 20.10.2020 {
			|	ТабельОтработанноеВремяПоДням.Ссылка.Проект 							КАК Проект,
			//ПодольскийАИ #УчетПроектов 20.10.2020 }
			...
			//ПодольскийАИ #УчетПроектов 20.10.2020 {
			|	ТабельОтработанноеВремяПоДням.Ссылка.Проект,
			//ПодольскийАИ #УчетПроектов 20.10.2020 }
...
Функция ПрибавитьКОтработанномуВремени(ВидВремени)
	...
	Если ВидВремени = Справочники.ВидыРабочегоВремени.Праздники
		...
		ИЛИ ВидВремени = Справочники.ВидыРабочегоВремени.Работа 
		//_lazare
		ИЛИ ВидВремени = Справочники.ВидыРабочегоВремени.РаботаНочныеЧасы
		ИЛИ ВидВремени = Справочники.ВидыРабочегоВремени.Другой
		ИЛИ ВидВремени = Справочники.ВидыРабочегоВремени.ВыходныеДни
		//lazare_
		Тогда
		Возврат Истина;	
	Иначе	
...
Функция ПечатнаяФормаПФ_MXL_Макет1(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	Табель.Организация.ИНН КАК ОрганизацияИНН,
		|	Табель.Организация.ФайлФаксимильнаяПечать КАК ФайлФаксимильнаяПечать
		|ИЗ
		...
		Если Шапка.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням Тогда
			...
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Табель.ПФ_MXL_Макет1");
		...
		Если ЗначениеЗаполнено(Шапка.ФайлФаксимильнаяПечать) Тогда
			ДанныеКартинки = ПрисоединенныеФайлы.ПолучитьДвоичныеДанныеФайла(Шапка.ФайлФаксимильнаяПечать);
		...
		ОбластьШапкаДокумента.Параметры.ИНН     = Шапка.ОрганизацияИНН;
		...
		Если Шапка.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням Тогда
			...
			|	ВЫБОР
			|		КОГДА ТабельОтработанноеВремяПоДням.Сотрудник.Физлицо.ИНН = """"
			|			ТОГДА ТабельОтработанноеВремяПоДням.Сотрудник.Код
			|		ИНАЧЕ ТабельОтработанноеВремяПоДням.Сотрудник.Физлицо.ИНН
			|	КОНЕЦ КАК ТабНомер,
			|	ТабельОтработанноеВремяПоДням.Сотрудник.Физлицо.ИНН
			|ИЗ
			|	Документ.Табель.ОтработанноеВремяПоДням КАК ТабельОтработанноеВремяПоДням
			...
			Пока Выборка.Следующий() Цикл
				...
				ЧасыДругой        = 0;
				ЧасыБолезнь       = 0;
				ЧасыОсновнойОтпуск= 0;
				ЧасыНеоплачиваемыйОтпускПоЗаконодательству = 0;
				//Lazare_
				НПП = НПП + 1;
				...
				Если ЗначениеЗаполнено(Выборка.Фамилия) Тогда
					...
					ФИО = УправлениеНебольшойФирмойСервер.ПолучитьИмяФамилию(Выборка.Фамилия, Выборка.Имя);
				...
				//_Lazare
 					Для Счетчик = 1 По День(КонецМесяца(ТекущийДокумент.ПериодРегистрации)) Цикл
					
					Часы = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Сверхурочные, Выборка["ПервыйЧасы" + Счетчик], 0)
						+  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Сверхурочные, Выборка["ВторойЧасы" + Счетчик], 0)
						+  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Сверхурочные, Выборка["ТретийЧасы" + Счетчик], 0);
					СверхурочныеЧасы   =  СверхурочныеЧасы +Часы ;
					
					Часы1 = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.РаботаНочныеЧасы, Выборка["ПервыйЧасы" + Счетчик], 0)
					 	 +  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.РаботаНочныеЧасы, Выборка["ВторойЧасы" + Счетчик], 0)
						 +  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.РаботаНочныеЧасы, Выборка["ТретийЧасы" + Счетчик], 0);
					РаботаНочныеЧасы   =  РаботаНочныеЧасы +Часы1 ;
					
					Часы2 = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.ВыходныеДни, Выборка["ПервыйЧасы" + Счетчик], 0)
						 +  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.ВыходныеДни, Выборка["ВторойЧасы" + Счетчик], 0)
						 +  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.ВыходныеДни, Выборка["ТретийЧасы" + Счетчик], 0);
					ЧасыВыходныеДни   =  ЧасыВыходныеДни +Часы2 ;
					
					Часы3 = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Другой, Выборка["ПервыйЧасы" + Счетчик], 0)
						 +  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Другой, Выборка["ВторойЧасы" + Счетчик], 0)
						 +  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Другой, Выборка["ТретийЧасы" + Счетчик], 0);
					ЧасыДругой   =  ЧасыДругой +Часы3 ;
					
					Часы4 = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Болезнь, Выборка["ПервыйЧасы" + Счетчик], 0)
						 +  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Болезнь, Выборка["ВторойЧасы" + Счетчик], 0)
						 +  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.Болезнь, Выборка["ТретийЧасы" + Счетчик], 0);
					ЧасыБолезнь  =  ЧасыБолезнь +Часы4 ;
					
					Часы5 = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.ОсновнойОтпуск, Выборка["ПервыйЧасы" + Счетчик], 0)
						 +  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.ОсновнойОтпуск, Выборка["ВторойЧасы" + Счетчик], 0)
						 +  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.ОсновнойОтпуск, Выборка["ТретийЧасы" + Счетчик], 0);
					ЧасыОсновнойОтпуск  =  ЧасыОсновнойОтпуск +Часы5 ;
					
						Часы6 = ?(Выборка["ПервыйВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.НеоплачиваемыйОтпускПоЗаконодательству, Выборка["ПервыйЧасы" + Счетчик], 0)
						 +  ?(Выборка["ВторойВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.НеоплачиваемыйОтпускПоЗаконодательству, Выборка["ВторойЧасы" + Счетчик], 0)
						 +  ?(Выборка["ТретийВидВремени" + Счетчик] = Справочники.ВидыРабочегоВремени.НеоплачиваемыйОтпускПоЗаконодательству, Выборка["ТретийЧасы" + Счетчик], 0);
					ЧасыНеоплачиваемыйОтпускПоЗаконодательству  =  ЧасыНеоплачиваемыйОтпускПоЗаконодательству +Часы6 ;
									
				КонецЦикла;
				
				ОбластьДетали.Параметры.СверхурочныеЧасы  = СверхурочныеЧасы;
				ОбластьДетали.Параметры.РаботаНочныеЧасы  = РаботаНочныеЧасы;
				ОбластьДетали.Параметры.ЧасыВыходныеДни   = ЧасыВыходныеДни;
				ОбластьДетали.Параметры.ЧасыДругой        = ЧасыДругой;
				ОбластьДетали.Параметры.ЧасыБолезнь       = ЧасыБолезнь;
				ОбластьДетали.Параметры.ЧасыОсновнойОтпуск  = ЧасыОсновнойОтпуск;
				ОбластьДетали.Параметры.ЧасыНеоплачиваемыйОтпускПоЗаконодательству  = ЧасыНеоплачиваемыйОтпускПоЗаконодательству;				
				...
				Запрос.УстановитьПараметр("ДатаНачала", Шапка.ПериодРегистрации);
				Запрос.УстановитьПараметр("ДатаОкончания", КонецМесяца(Шапка.ПериодРегистрации));
				РезультатЗапроса = Запрос.Выполнить().Выгрузить();
				
			    КоличествоВыходных = РезультатЗапроса.Количество();
				ОбластьДетали.Параметры.КоличествоВыходных = КоличествоВыходных;			
				//Lazare_
...
//+++ Дмгг, 23.09.24, ONESOFT-3386
Функция ПечатнаяФормаПФ_MXL_БланкРегистрацииРабочегоВремени(МассивОбъектов, ОбъектыПечати)
	Перем ПервыйДокумент, НомерСтрокиНачало, Ошибки;
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_Табель_Макет";
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Табель.ПФ_MXL_БланкРегистрацииРабочегоВремени");
		...
		ОбластьШапка.Параметры.ИмяОрганизации = Шапка.ОрганизацияНаименованиеПолное;
		ОбластьШапка.Параметры.ИНН = Шапка.ОрганизацияИНН;
		ОбластьШапка.Параметры.ИмяПодразделения = Шапка.СтруктурнаяЕдиница;
		ОбластьШапка.Параметры.ДатаЗаполнения = Формат(Шапка.ДатаДокумента, "ДФ=dd/MM/yy");
		ОбластьШапка.Параметры.ДатаНач = Шапка.ПериодРегистрации;
		ОбластьШапка.Параметры.ДатаКон = КонецМесяца(Шапка.ПериодРегистрации);
		...
		Запрос.УстановитьПараметр("ПериодРегистрации",   Шапка.ПериодРегистрации);
			...
			ВидВремениСверхурочные	= Справочники.ВидыРабочегоВремени.Сверхурочные;
			ВидВремениНочныеЧасы	= Справочники.ВидыРабочегоВремени.РаботаНочныеЧасы;
			ВидВремениВыходные 		= Справочники.ВидыРабочегоВремени.ВыходныеДни;
			ВидВремениПраздники 	= Справочники.ВидыРабочегоВремени.Праздники;
			ВидВремениДругое 		= Справочники.ВидыРабочегоВремени.Другой;
			...
			Пока Выборка.Следующий() Цикл
				...
				Иначе
					ФИО = "";
				КонецЕсли;
				ОбластьСтрока.Параметры.ФИО = ?(ЗначениеЗаполнено(ФИО), ФИО, Выборка.Сотрудник);
				
				Для Счетчик = 1 По День(КонецМесяца(ТекущийДокумент.ПериодРегистрации)) Цикл
					
					СтрокаВидВремени = "" + Выборка["ПервыйВидВремени" + Счетчик] + ?(ЗначениеЗаполнено(Выборка["ВторойВидВремени" + Счетчик]), "/" + Выборка["ВторойВидВремени" + Счетчик], "") + ?(ЗначениеЗаполнено(Выборка["ТретийВидВремени" + Счетчик]), "/" + Выборка["ТретийВидВремени" + Счетчик], "");
					
					ОбластьСтрока.Параметры["Символ" + Счетчик] = СтрокаВидВремени;
					
					Часы = ?(ПрибавитьКОтработанномуВремени(Выборка["ПервыйВидВремени" + Счетчик]), Выборка["ПервыйЧасы" + Счетчик], 0)
						+ ?(ПрибавитьКОтработанномуВремени(Выборка["ВторойВидВремени" + Счетчик]), Выборка["ВторойЧасы" + Счетчик], 0)
						+ ?(ПрибавитьКОтработанномуВремени(Выборка["ТретийВидВремени" + Счетчик]), Выборка["ТретийЧасы" + Счетчик], 0);
						
					Если Выборка["ПервыйВидВремени" + Счетчик] = ВидВремениСверхурочные 
						ИЛИ Выборка["ВторойВидВремени" + Счетчик] = ВидВремениСверхурочные
						ИЛИ Выборка["ТретийВидВремени" + Счетчик] = ВидВремениСверхурочные Тогда 
						
							Сверх = Сверх + 1;
							
					КонецЕсли;
						
					Если Выборка["ПервыйВидВремени" + Счетчик] = ВидВремениНочныеЧасы 
						ИЛИ Выборка["ВторойВидВремени" + Счетчик] = ВидВремениНочныеЧасы
						ИЛИ Выборка["ТретийВидВремени" + Счетчик] = ВидВремениНочныеЧасы Тогда 
						
							Ночь = Ночь + 1;
							
					КонецЕсли;
						
					Если Выборка["ПервыйВидВремени" + Счетчик] = ВидВремениВыходные 
						ИЛИ Выборка["ВторойВидВремени" + Счетчик] = ВидВремениВыходные
						ИЛИ Выборка["ТретийВидВремени" + Счетчик] = ВидВремениВыходные
						ИЛИ Выборка["ПервыйВидВремени" + Счетчик] = ВидВремениПраздники
						ИЛИ Выборка["ВторойВидВремени" + Счетчик] = ВидВремениПраздники
						ИЛИ Выборка["ТретийВидВремени" + Счетчик] = ВидВремениПраздники Тогда 
						
							ВыхПразд = ВыхПразд + 1;
							
					КонецЕсли;
						
					Если Выборка["ПервыйВидВремени" + Счетчик] = ВидВремениДругое 
						ИЛИ Выборка["ВторойВидВремени" + Счетчик] = ВидВремениДругое
						ИЛИ Выборка["ТретийВидВремени" + Счетчик] = ВидВремениДругое Тогда 
						
							Другое = Другое + 1;
							
					КонецЕсли;	
						
					ЧасыВсего = ЧасыВсего +  Часы;
					ДниВсего = ДниВсего + ?(Часы > 0, 1, 0);
					
				КонецЦикла;
				
				Для Счетчик = День(КонецМесяца(ТекущийДокумент.ПериодРегистрации)) + 1 По 31 Цикл
					
					ОбластьСтрока.Параметры["Символ" + Счетчик] = "Х";
					ОбластьСтрока.Параметры["ДополнительноеЗначение" + Счетчик] = "Х";
					
				КонецЦикла; 		
				
				ОбластьСтрока.Параметры.Дней = ДниВсего;
				ОбластьСтрока.Параметры.Сверх = Сверх;
				ОбластьСтрока.Параметры.Ночь = Ночь;
				ОбластьСтрока.Параметры.ВыхПразд = ВыхПразд;
				ОбластьСтрока.Параметры.Другое = Другое;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Табель") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Табель", НСтр("ka='სამუშაო დროის აღრიცხვის ტაბელი';ru='Табель учета рабочего времени';en='Timesheets'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
		
	КонецЕсли;
	//_Lazare
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТабельНовый") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТабельНовый", НСтр("ru = 'Форма учета рабочего времени N1.1'; ka = 'სამუშაო დროის აღრიცხვის ფორმა. დანართი N1.1'"), ПечатнаяФормаПФ_MXL_Макет1(МассивОбъектов, ОбъектыПечати));	
	КонецЕсли;
	//Lazare_
	
	//+++ Дмгг, 23.09.24, ONESOFT-3386
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "БланкРегистрацииРабочегоВремени") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "БланкРегистрацииРабочегоВремени", НСтр("ru = 'Бланк регистрации рабочего времени'; ka = 'სამუშაო დროის აღრიცხვის ფორმა'"), ПечатнаяФормаПФ_MXL_БланкРегистрацииРабочегоВремени(МассивОбъектов, ОбъектыПечати));	
	КонецЕсли;
	//--- Дмгг, 23.09.24, ONESOFT-3386
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='სამუშაო დროის აღრიცხვის ტაბელი';ru='Табель учета рабочего времени';en='Timesheets'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ТабельНовый";
	КомандаПечати.Представление = НСтр("ru = 'Форма учета рабочего времени N1.1'; ka = 'სამუშაო დროის აღრიცხვის ფორმა. დანართი N1.1 '");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 2;
	
	//+++ Дмгг, 23.09.24, ONESOFT-3386
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "БланкРегистрацииРабочегоВремени";
	КомандаПечати.Представление = НСтр("ru = 'Бланк регистрации рабочего времени'; ka = 'სამუშაო დროის აღრიცხვის ფორმა'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
