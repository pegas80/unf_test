...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++lazare 13.12.2021
	ЗапретРедактирования = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияПроведенныхДокументовПеремещениеЗапасов");//Lazare-04.11.2021
	...
	ВыводЦеныПоследнейЗакупкиВДокументеПеремещениеЗапасов = ПолучитьФункциональнуюОпцию("ВыводЦеныПоследнейЗакупкиВДокументеПеремещениеЗапасов");
	УстановитьЦенуЗакупкиВСтрокеЗапасы();
	// © Шишка В.В. [18 сентября 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	
	//++ 19.06.2025 ONESOFT-3294
   	Элементы.ЗапасыBackOrder.Видимость = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ИспользоватьBackOrderВПеремещениеЗапасов"); 
	//++ 19.06.2025 ONESOFT-3294
...
Процедура ДобавитьОрганиченияНаРедактированиеДокумента(Пользователь)
	...
	Если НЕ ЗначениеЗаполнено(ОсновнойСкладПользователя) Тогда 
		...
		МассивСкладов = ОсновныеСкладыПользователя.СписокСкладов.ВыгрузитьКолонку("Склад"); //giga
	Иначе 
		МассивСкладов = ОсновнойСкладПользователя; //giga
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьВидимостиСчетовЗатрат();
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	//+++ Lazare 17.11.2021
	Если Не УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПоказыватьБухгалтерскиеПроводки") и не ЗначениеЗаполнено(Объект.СчетЗатратРегл)	
		и  Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы")
		Тогда  Объект.СчетЗатратРегл = РС_ОбщегоНазначения.СчетДляСписанияОприходованияЗапасов(Объект.ссылка);	
	КонецЕсли;
	//--- Lazare_17.11.2021
	
	//+++ Lazare 17.11.2021
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьСкладыВТабличнойЧастиПеремещениеЗапасов") и 
		Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПеремещениеЗапасов.Перемещение") Тогда 	
		Элементы.ЗапасыЗапасыСтруктурнаяЕдиницаОтправитель.Видимость = Истина;
		Элементы.ЗапасыЗапасыСтруктурнаяЕдиницаПолучатель.Видимость  = Истина;	
	Иначе 
		Элементы.ЗапасыЗапасыСтруктурнаяЕдиницаОтправитель.Видимость = Ложь;
		Элементы.ЗапасыЗапасыСтруктурнаяЕдиницаПолучатель.Видимость  = Ложь;	
	КонецЕсли;
	//--- Lazare_17.11.2021
...
// Kuzmich, 16.07.2019, Заполнение Перемещения из заказа покупателя	
	...
	//СтрокаТабличнойЧасти.СрокГодности = ?(СтруктураДанные.КоличествоДни >0,Объект.Дата + СтруктураДанные.КоличествоДни * 86400,Дата(1,1,1));//+Lazare
	
	//+Lazare 14.12.2022
	СкладЗапасов = ?(ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЗапасыСтруктурнаяЕдиницаОтправитель),СтрокаТабличнойЧасти.ЗапасыСтруктурнаяЕдиницаОтправитель,Объект.СтруктурнаяЕдиница);	
	СтрокаТабличнойЧасти.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, СтрокаТабличнойЧасти.Номенклатура,СтрокаТабличнойЧасти.Характеристика,СтрокаТабличнойЧасти.Партия,Объект.Организация,СкладЗапасов);//++Lazare 13.12.2022
	//-Lazare 14.12.2022
	
	//+Lazare 06.01.2023
	ВидЦенПродажи = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВидимостьЦенПродажиВПеремещениеЗапасов");//++Lazare 06.01.2023	
	Если ЗначениеЗаполнено(ВидЦенПродажи) Тогда 			
		 ЗаполнитьЦенуНоменклатурыПоВидуЦен(Элементы.Запасы.ТекущаяСтрока,ВидЦенПродажи);
	КонецЕсли;
	//-Lazare 06.01.2023
	//--------------------------------------------------------------------------
	// © Шишка В.В. [18 сентября 2023]  НАЧАЛО
	УстановитьЦенуЗакупкиВСтрокеЗапасы(СтрокаТабличнойЧасти.ПолучитьИдентификатор());
	// © Шишка В.В. [18 сентября 2023]  КОНЕЦ
	//--------------------------------------------------------------------------   
	
	//++ 19.06.2025 ONESOFT-3294
	ИспользоватьBackOrder = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ИспользоватьBackOrderВПеремещениеЗапасов");	
	Если ИспользоватьBackOrder Тогда 
		СтрокаТабличнойЧасти.BackOrder = СтрокаТабличнойЧасти.КоличествоНаСкладе - СтрокаТабличнойЧасти.Количество;	
	КонецЕсли;   
	//++ 19.06.2025 ONESOFT-3294
КонецПроцедуры // ЗапасыНоменклатураПриИзменении()

&НаСервере
Процедура ЗаполнитьЦенуНоменклатурыПоВидуЦен(Идентификатор,ВидЦенПродажи);
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение Тогда 		
		...
		СтруктураОтбора.Вставить("ВидЦен",ВидЦенПродажи);
		...
		ТекДанные.ЦенаПродажи = РегистрыСведений.ЦеныНоменклатуры.ПолучитьПоследнее(Объект.Дата,СтруктураОтбора).Цена;
...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	КонецЕсли;
	//+Lazare
	Если НоваяСтрока и УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьСкладыВТабличнойЧастиПеремещениеЗапасов") Тогда
		Элемент.ТекущиеДанные.ЗапасыСтруктурнаяЕдиницаОтправитель = Объект.СтруктурнаяЕдиница;
		Элемент.ТекущиеДанные.ЗапасыСтруктурнаяЕдиницаПолучатель = Объект.СтруктурнаяЕдиницаПолучатель;
	КонецЕсли;
	//-Lazare
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	СтруктураДанные.Вставить("КоличествоДни", СтруктураДанные.Номенклатура.СрокГодности);
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если МассивСтрокТЧ.Количество() = 0 Тогда
				...
				СкладЗапасов = ?(ЗначениеЗаполнено(НоваяСтрока.ЗапасыСтруктурнаяЕдиницаОтправитель),НоваяСтрока.ЗапасыСтруктурнаяЕдиницаОтправитель,Объект.СтруктурнаяЕдиница);	
				НоваяСтрока.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, НоваяСтрока.Номенклатура,НоваяСтрока.Характеристика,НоваяСтрока.Партия,Объект.Организация,СкладЗапасов);//++Lazare 13.12.2022
...
Процедура ПолученыШтрихкодыФрагмент(НеизвестныеШтрихкоды) Экспорт
	...
	Для каждого ТекНеизвестныйШтрихкод Из НеизвестныеШтрихкоды Цикл
		...
		СтрокаСообщения = НСтр("ru = 'Данные по штрихкоду не найдены: %1%; количество: %2%'; ka = 'მონაცემები შტრიხკოდის მიხედვით ნაპოვნი არ არის: %1%; რაოდენობა: %2%'; en = 'Data on the barcode is not found: %1%; amount: %2%'");
...
Процедура ПолученыШтрихкодыНекорректногоТипа(ШтрихкодыНекорректногоТипа) Экспорт
	...
	Для каждого ТекНекорректныйШтрихкод Из ШтрихкодыНекорректногоТипа Цикл
		...
		СтрокаСообщения = НСтр("ru = 'Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этой табличной части'; ka = 'შტრიხკოდ %1%-ით მოძებნილ ნომენკლატურას -%2%- აქვს ტიპი %3%, რომელიც ამ ცხრილის შესაფერი არ არის'; en = 'Found by barcode nomenclature%1% -%2% - %3% of type, which is not suitable for this table part'");
...
Процедура УстановитьВидимостьИДоступность(ВидОперацииПриИзменении = Ложь)
	...
	Если ЭтаПеремещение Тогда
		...
		//+Lazare 14.12.2022
		ИспользоватьСкладыВТабличнойЧастиПеремещениеЗапасов =  УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьСкладыВТабличнойЧастиПеремещениеЗапасов");//++Lazare 14.12.2022
		Элементы.ЗапасыЗапасыСтруктурнаяЕдиницаОтправитель.Видимость = ИспользоватьСкладыВТабличнойЧастиПеремещениеЗапасов;
		Элементы.ЗапасыЗапасыСтруктурнаяЕдиницаПолучатель.Видимость  = ИспользоватьСкладыВТабличнойЧастиПеремещениеЗапасов;
		//-Lazare 14.12.2022
	...
	Элементы.ЗапасыКоличествоНаСкладе.Видимость =  УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалВидимостиСвободногоОстаткаЗапасовВВокументах");//++Lazare 14.12.2022
	
	Элементы.ЗапасыЦенаПродажи.Видимость = Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение;//++Lazare 06.01.2023
...
Процедура УстановитьВидимостьОтФОИспользоватьПодсистемуПроизводство()
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуПроизводство.Получить()
		...
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение, НСтр("ru = 'Перемещение'; ka = 'მარაგების გადაადგილება'; en = 'Moving'"));
...
Процедура УстановитьВидимостиСчетовЗатрат()
	...
	ЗначениеВидимости = Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы
						или Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеТаваровНаОС; //Малышев 2024-06-24 ONESOFT-3275
	
	Элементы.СчетЗатрат.Видимость = ЗначениеВидимости;
	Элементы.СчетЗатратРегл.Видимость = ЗначениеВидимости;
	
	//Малыщев 2024-06-24 ONESOFT-3275 (--
	//ПодольскийАИ #УчетПроектов 05.10.2020 {
	...
	Элементы.ЗапасыПроектОтправитель.Видимость = Ложь; 
	Элементы.ПроектОтправитель.Видимость = Ложь;
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение 
		или Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.ПередачаВЭксплуатацию 
		или Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.ВозвратИзЭксплуатации 
		Тогда
		Элементы.ПроектОтправитель.Видимость = Истина;
		Элементы.ПроектОтправитель.Заголовок = НСтр("ka='გამგზავნი პროექტი';ru='Проект отправитель';en='Project from'");
		
		Элементы.Проект.Видимость = Ложь;
		Элементы.ПроектПолучатель.Видимость = Истина;
		
		Элементы.ЗапасыПроектПолучатель.Видимость = Истина;
		Элементы.ЗапасыПроект.Заголовок = НСтр("ka='პროექტი გამგზავნი';ru='Проект отправитель';en='Project from'");
	иначе 
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеТаваровНаОС тогда
			Элементы.ПроектОтправитель.Видимость = Истина;
			Элементы.ПроектОтправитель.Заголовок = НСтр("ka='პროექტი';ru='Проект';en='Project'");
		Иначе     
			Элементы.ПроектОтправитель.Видимость = Ложь;
		КонецЕсли;	
			
		
		Элементы.ЗапасыПроектПолучатель.Видимость = Ложь;                                                  
		Элементы.ЗапасыПроект.Заголовок = НСтр("ka='პროექტი';ru='Проект';en='Project'");
		Элементы.ПроектПолучатель.Видимость = Ложь;
	КонецЕсли;	
	//Малыщев 2024-06-24 ONESOFT-3275 --)
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалВидимостиСвободногоОстаткаЗапасовВВокументах") Тогда 		
		Для Каждого Ст из Объект.Запасы Цикл 	
			СкладЗапасов = ?(ЗначениеЗаполнено(Ст.ЗапасыСтруктурнаяЕдиницаОтправитель),Ст.ЗапасыСтруктурнаяЕдиницаОтправитель,Объект.СтруктурнаяЕдиница);	
			Ст.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, Ст.Номенклатура,Ст.Характеристика,Ст.Партия,Объект.Организация,СкладЗапасов);//++Lazare 13.12.2022	
		КонецЦикла;	
	КонецЕсли;
	//-Lazare 15.12.2022
	//--------------------------------------------------------------------------
	...
	Для Каждого Ст из Объект.Запасы Цикл 				
		Ст.BackOrder = Ст.КоличествоНаСкладе - Ст.Количество;
	КонецЦикла;
...
Процедура ЗаполнитьПроектПоДокументу(ПроектЗначение = Неопределено, мВидОперации)
	...
	Если ПроектЗначение = Неопределено Тогда         
		Объект.Проект = КэшЗначений.ПустойПроект; //Малышев 2024-06-24 ONESOFT-3275
	...
	Для Каждого СтрокаЗапасы Из Объект.Запасы Цикл
		СтрокаЗапасы.Проект = Объект.Проект;  //Малышев 2024-06-24 ONESOFT-3275
		Если мВидОперации = "СписаниеНаРасходы" Тогда
...
//--------------------------------------------------------------------------
// © Шишка В.В. [4 ноября 2021]  НАЧАЛО
&НаСервере
Процедура УстановитьОтборДляНаправлениеДеятельности()
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ПоказатьРеквизитыУправленческогоУчета")
		...
		Элементы.НаправлениеДеятельности.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
	...
	Иначе
		...
		МассивПараметров.Добавить(Новый СвязьПараметраВыбора("СчетЗатрат", "Объект.СчетЗатрат", РежимИзмененияСвязанногоЗначения.Очищать));
		Элементы.НаправлениеДеятельности.СвязиПараметровВыбора = Новый ФиксированныйМассив(МассивПараметров);
...
// © Шишка В.В. [4 ноября 2021]  КОНЕЦ
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
// © Шишка В.В. [18 сентября 2023]  НАЧАЛО
&НаСервере
Процедура УстановитьЦенуЗакупкиВСтрокеЗапасы(ИД_Строки = Неопределено)

	Если НЕ ВыводЦеныПоследнейЗакупкиВДокументеПеремещениеЗапасов Тогда
	
		Возврат;	
	
	КонецЕсли;
	
	Если ИД_Строки <> Неопределено  Тогда
	
		СтрокаЗапасы = Объект.Запасы.НайтиПоИдентификатору(ИД_Строки);
		ТаблицаЗапасы = Объект.Запасы.Выгрузить(Новый Структура("НомерСтроки", СтрокаЗапасы.НомерСтроки)); 
	...
	КонецЕсли;
	ДатаКонтроля =   ?(НачалоДня(Объект.Дата) = НачалоДня(ТекущаяДатаСеанса()), КонецДня(ТекущаяДатаСеанса()), Объект.Дата);
	Запрос = Новый Запрос;
	...
	Пока Выборка.Следующий() Цикл
		...
		Объект.Запасы[Выборка.НомерСтроки - 1].ЦенаЗакупки = Выборка.ЦенаЗакупки;	
...
//++ 19.06.2025 ONESOFT-3294
&НаСервере
Процедура ПересчитатьКолBackOrder()
	Для Каждого Ст из Объект.Запасы Цикл 
		Ст.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, Ст.Номенклатура,Ст.Характеристика,Ст.Партия,Объект.Организация,Объект.СтруктурнаяЕдиница);	
		Ст.BackOrder = Ст.КоличествоНаСкладе - Ст.Количество;
	КонецЦикла;
КонецПроцедуры 
//++ 19.06.2025 ONESOFT-3294
