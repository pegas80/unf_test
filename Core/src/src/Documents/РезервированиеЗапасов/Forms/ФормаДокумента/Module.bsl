...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Редактировать Авторезерв могут только пользоватьели с доп настройкой МенятьРезервПослеПроводки
	Если Объект.Авторезерв Тогда
		ТолькоПросмотр = Не УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(ПараметрыСеанса.ТекущийПользователь, "МенятьРезервПослеПроводки");
	КонецЕсли;
	...
	//Lazare_
	ЗагрузкаДанныхИзВнешнегоИсточника.ПриСозданииНаСервере(Метаданные.Документы.РезервированиеЗапасов.ТабличныеЧасти.Запасы, НастройкиЗагрузкиДанных, ЭтотОбъект);
	//_Lazare
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ТекстВопроса = НСтр("ka='დოკუმენტი სრულიად შეივსება ""მყიდველის შეკვეთით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Заказу покупателя""! Продолжить выполнение операции?';en='The document will be fully refilled by ""Buyer''s order!"" To continue the operation?'");
	Ответ = Неопределено;
...
Процедура ЗагрузитьИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		Если ПолучитьФункциональнуюОпцию("УчетПоПроектам") Тогда
			...
			НоваяСтрокаТовары.Проект = СтрокаТаблицы.Проект;
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				ИменаСвойств = "Номенклатура,Количество,ЕдиницаИзмерения,НовоеМестоРезерва";
				Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
				...
				Если ПолучитьФункциональнуюОпцию("УчетПоПроектам") Тогда
					...
					ИменаСвойств = ИменаСвойств + ", Проект";
...
ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузкаДанныхИзВнешнегоИсточникаОбработкаРезультата", ЭтотОбъект, НастройкиЗагрузкиДанных);
	...
	НастройкиЗагрузкиДанных.Вставить("ПолноеИмяТабличнойЧасти", "РезервированиеЗапасов.Запасы");
