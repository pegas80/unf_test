...
Процедура ПолучательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Организация = Неопределено ИЛИ Объект.Организация = ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Поле ""От кого"" не заполнено';ru='Поле ""От кого"" не заполнено';en='The ""From"" not filled'"));
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Объект.Получатель = Неопределено ИЛИ НЕ ЗначениеЗаполнено(РеквизитыПолучателя.Код) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Код налогового органа необходимо задать в справочнике ""Организации"".';ru='Код налогового органа необходимо задать в справочнике ""Организации"".';en='the tax authority code must be set in the ""Organization"".'"));
		
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ინსპექციის ასარჩევად აუცილებელია შესაბამისი ჩანაწერის შეყვანა ცნობარ ""საგადასახადო ორგანოში რეგისტრაციები""-ში';ru='Для выбора инспекции, необходимо завести соответствующую запись в справочнике ""Регистрации в налоговом органе"".';en='To select the inspection, you must have a corresponding entry in the ""Registration with the tax authority.""'"));
	КонецЕсли;
	...
	Если ЭтоКБК Тогда 
		...
		Если ЗначениеЗаполнено(НаименованиеКБК) Тогда
			ТекущиеДанные.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 (%2)';ru='%1 (%2)';en='%12)'"), ФорматированныйКБК, НаименованиеКБК);
		Иначе
...
Процедура УстановитьНадписьВИнформационнойПанели(Форма)
	...
	Если ЭтоАктСверкиРасчетов Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ka='Акт сверки используется для сопоставления данных с налоговой инспекцией по налогам (пеням, штрафам) и позволяет выявить переплату или задолженность.';ru='Акт сверки используется для сопоставления данных с налоговой инспекцией по налогам (пеням, штрафам) и позволяет выявить переплату или задолженность.';en='Reconciliation Report is used for comparison of the data with tax authorities for taxes (penalties, fines) and reveals an overpayment or the debt.'");
	ИначеЕсли ЭтоВыпискаОперацийИзКарточкиРасчетыСБюджетом Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ka='Выписка операций используется для выяснения отдельных вопросов (неясностей), которые возникают по результатам сверки с налоговым органом.';ru='Выписка операций используется для выяснения отдельных вопросов (неясностей), которые возникают по результатам сверки с налоговым органом.';en='Statement of operations is used to determine the specific issues (unclear) that occur as a result of reconciliation with the tax authority.'");
	ИначеЕсли ЭтоПереченьБухгалтерскойИНалоговойОтчетности Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ka='Список представленной отчетности подтверждает, что вся отчетность принята в полном объеме.';ru='Список представленной отчетности подтверждает, что вся отчетность принята в полном объеме.';en='List of reporting confirms that all statements taken in their entirety.'");
	ИначеЕсли ЭтоСправкаОСостоянииРасчетовСБюджетом Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ru='Справка о состоянии расчетов позволяет узнать, нет ли задолженности или переплаты по налогам на указанную дату. ';ka='Справка о состоянии расчетов позволяет узнать, нет ли задолженности или переплаты по налогам на указанную дату. '");
	ИначеЕсли ЭтоСправкаОбИсполненииОбязанностейПоУплате Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ka='Справка позволяет проверить наличие неисполненной обязанности по уплате налогов (пеней, штрафов). Часто требуется при участии в тендере, получении кредита, оформлении лицензии для подтверждения добросовестности налогоплательщика.';ru='Справка позволяет проверить наличие неисполненной обязанности по уплате налогов (пеней, штрафов). Часто требуется при участии в тендере, получении кредита, оформлении лицензии для подтверждения добросовестности налогоплательщика.';en='Help allows you to check the presence of unfulfilled obligation to pay taxes (fines, penalties). Often it requires the participation in the tender for a loan, take out a license to confirm the integrity of the taxpayer.'");
	КонецЕсли;
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				Если ЗначениеЗаполнено(НайденнаяСтрока.Наименование) Тогда
					...
					СтрокаТаблицы.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='%1 (%2)';ru='%1 (%2)';en='%12)'"), КБК, НайденнаяСтрока.Наименование);
				Иначе
					КБК = ФорматированныйКБК(СтрокаТаблицы.КБК);
...
Процедура ПроверитьПередЗаписью(Отказ, ТаблицаКБК)
	...
	Если Объект.КоличествоНалогов = Перечисления.КоличествоНалоговДляСверкиИОН.Один Тогда
		...
		Если НЕ ЗначениеЗаполнено(Налог) Тогда
			...
			// Не выбран налог
			ТекстСообщения = НСтр("ka='Укажите налог, по которому будет выполняться сверка';ru='Укажите налог, по которому будет выполняться сверка';en='Specify taxes for which verification will be carried out'");
		...
		ИначеЕсли СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Налог) Тогда
			...
			ТекстСообщения = НСтр("ka='В качестве налога не может быть указан КБК. Выберите налог из списка';ru='В качестве налога не может быть указан КБК. Выберите налог из списка';en='As a tax can not be specified CSC. Select from the list of tax'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"Налог");
		...
		Иначе
			...
			Если НЕ ЗначениеЕстьВМакете(ТаблицаКБК, Налог) Тогда
				...
				ТекстСообщения = НСтр("ka='Налог ""%1"" не найден. Выберите налог из списка';ru='Налог ""%1"" не найден. Выберите налог из списка';en='Tax ""%1"" not found. Select from the list of tax'");
...
"Код ОКАТО/ОКТМО должен иметь длину 8 или 11 и состоять только из цифр.';ka='Введено некорректное значение ОКАТО или ОКТМО %1"
"Код ОКАТО/ОКТМО должен иметь длину 8 или 11 и состоять только из цифр.'");
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ОКАТО) Тогда
			...
			// Не заполнено ОКАТО
			ТекстСообщения = НСтр("ka='Не заполнено ОКАТО или ОКТМО';ru='Не заполнено ОКАТО или ОКТМО';en='Not filled OKATO or OKTMO'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"ОКАТО");
...
Функция ЗначениеЕстьВМакете(ТаблицаКБК, Значение)
	...
	Для каждого СтрокаТаблицыКБК Из ТаблицаКБК Цикл
		...
		ЗначениеЕстьВТаблице = 
			...
			ИЛИ Значение = НСтр("ka='სხვები';ru='Прочие';en='other'")
...
Процедура ЗадатьПравилаОтображенияДатыНаДляАктов(Форма)
	...
	Если ЭтоАктСверкиРасчетов Тогда
		Если НачалоДня(Объект.ДатаОкончанияПериода) = НачалоДня(ТекущаяДата()) Тогда
			Элементы.КомментарийКДатеОкончанияПериода.Заголовок = НСтр("ka='(сегодня)';ru='(сегодня)';en='(Today)'");
		Иначе
	...
	Если ЭтоСправкаОСостоянииРасчетовСБюджетом
		...
		Если НачалоДня(Объект.ДатаОкончанияПериода) = НачалоДня(ТекущаяДата()) Тогда
			Если Форма.ЗаявлениеОтправлено Тогда
				...
				Элементы.КомментарийКНаДату.Заголовок = Формат(Объект.ДатаОкончанияПериода, "ДЛФ=D") + " " + НСтр("ka='(сегодня)';ru='(сегодня)';en='(Today)'");
			Иначе
				Элементы.КомментарийКНаДату.Заголовок = НСтр("ka='(сегодня)';ru='(сегодня)';en='(Today)'");
			КонецЕсли;
		Иначе
...
Процедура УправлениеВнешнимВидомОтправленногоЗаявления(Форма)
	...
	Если Форма.ЗаявлениеОтправлено Тогда
		...
		Если Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеАктовСверкиРасчетов") Тогда
			...
			Если НачалоДня(Объект.ДатаОкончанияПериода) = НачалоДня(ТекущаяДата()) Тогда
				Элементы.КомментарийКДатеОкончанияПериода.Заголовок = НСтр("ka='Взаиморасчеты по состоянию на: сегодня';ru='Взаиморасчеты по состоянию на: сегодня';en='Settlements as of today'");
			Иначе
				Элементы.КомментарийКДатеОкончанияПериода.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Взаиморасчеты по состоянию на: %1';ru='Взаиморасчеты по состоянию на: %1';en='Settlements as of:% 1'"), 
					Формат(Объект.ДатаОкончанияПериода, "ДЛФ=D"));
			КонецЕсли;
			
			Элементы.НадписьПредставлениеЗаГод.Заголовок = Формат(Объект.ДатаНачалаПериода, "ДФ='yyyy'") + НСтр("ka='წ.';ru=' г.'");
...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Объект 		= Форма.Объект;
	Заголовок 	= НСтр("ka='Запрос на сверку: %1 (%2)';ru='Запрос на сверку: %1 (%2)';en='The request for verification of: 1% (2%)'");
	
	// Для нового документа в скобках пишем "создание", 
	// для записанного - период.
	Если Форма.ЭтоНовый Тогда
		ДополнительныеСведения = НСтр("ka='ქმნილება';ru='создание';en='creature'");
	Иначе
...
Процедура УправлениеНалогамиИСборами(Форма)
	...
	Если Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеАктовСверкиРасчетов")
		...
		Если Объект.КоличествоНалогов = ПредопределенноеЗначение("Перечисление.КоличествоНалоговДляСверкиИОН.Один") Тогда
			...
			Форма.ВариантВыбораНалогаПредставление = НСтр("ka='Одному налогу';ru='Одному налогу';en='one tax'");
		ИначеЕсли Объект.КоличествоНалогов = ПредопределенноеЗначение("Перечисление.КоличествоНалоговДляСверкиИОН.Несколько")
			...
			Форма.ВариантВыбораНалогаПредставление = НСтр("ka='Нескольким налогам или КБК';ru='Нескольким налогам или КБК';en='Several taxes or BCF'");
		ИначеЕсли Объект.КоличествоНалогов = ПредопределенноеЗначение("Перечисление.КоличествоНалоговДляСверкиИОН.Все")
			ИЛИ (НЕ ЗначениеЗаполнено(Объект.КоличествоНалогов) И Объект.ЗапрашиваемыеНалоги.Количество() = 0) Тогда
			Элементы.ОдинНалог.Видимость 		= Ложь;
			Элементы.НесколькоНалогов.Видимость = Ложь;
			Форма.ВариантВыбораНалогаПредставление = НСтр("ka='Всем налогам';ru='Всем налогам';en='all taxes'");
		КонецЕсли;
...
Функция ПредставлениеДатВЗаголоке(ВидУслуги, ДатаНачалаПериода, ДатаОкончанияПериода)
	...
	Если ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеАктовСверкиРасчетов") Тогда
		...
		ДополнительныеСведения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='за %1 г. на дату %2';ru='за %1 г. на дату %2';en='over 1% on the date of 2%'"), 
	...
	ИначеЕсли ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеВыпискиОперацийИзКарточкиРасчетыСБюджетом") Тогда
		...
		ДополнительныеСведения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='за %1 г.';ru='за %1 г.';en='for 1% of'"), 
	...
	ИначеЕсли ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеПеречняБухгалтерскойИНалоговойОтчетности") Тогда
		...
		ДополнительныеСведения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='за период %1 - %2';ru='за период %1 - %2';en='% for the period of 1 - 2%'"), 
...
Процедура ЗадатьЗначениеПолучателяПоУмолчанию()
	...
	Если Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеСправкиОбИсполненииОбязанностейПоУплате") Тогда
		РеквизитыОрганизации 	= РеквизитыОрганизацииНаСервере(Объект.Организация);
		ПолучательПредставление = НСтр("ka='ФНС';ru='ФНС';en='FTS'") + ?(ЗначениеЗаполнено(Объект.Организация), " " + Строка(РеквизитыОрганизации.КодНалоговогоОргана), "");
...
Процедура УправлениеГруппировкойОперации(Форма)
	...
	Если Объект.ДополнительныйПараметр = 0 Тогда
		Форма.ГруппировкаОперацийПредставление = НСтр("ka='თარიღით';ru='По дате';en='By date'"); 
	ИначеЕсли Объект.ДополнительныйПараметр = 1 Тогда
		Форма.ГруппировкаОперацийПредставление = НСтр("ka='По виду платежа';ru='По виду платежа';en='By type of payment'");
	Иначе
		Форма.ГруппировкаОперацийПредставление = НСтр("ka='არ არის შერცეული';ru='Не выбран';en='not selected'");
	КонецЕсли;
...
Процедура ОбработкаВыбораГруппировкиОперации(НоваяГруппировкаОперации, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(НоваяГруппировкаОперации) Тогда
		Если НоваяГруппировкаОперации = НСтр("ka='თარიღით';ru='По дате';en='By date'") Тогда
			Объект.ДополнительныйПараметр = 0;
		Иначе
			Объект.ДополнительныйПараметр = 1;
		КонецЕсли;
...
Процедура УправлениеВидамиОтчетов(Форма)
	...
	Если Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеПеречняБухгалтерскойИНалоговойОтчетности") Тогда
		...
		ИначеЕсли Объект.ДополнительныйПараметр = 1 Тогда
			Форма.ВидыОтчетовПредставление = НСтр("ka='Первичные и корректирующие';ru='Первичные и корректирующие';en='Primary and corrective'");
		ИначеЕсли Объект.ДополнительныйПараметр = 2 Тогда
			Форма.ВидыОтчетовПредставление = НСтр("ka='პირველადი';ru='Первичные';en='primary'");
		ИначеЕсли Объект.ДополнительныйПараметр = 3 Тогда
			Форма.ВидыОтчетовПредставление = НСтр("ka='მაკორექტირებლები';ru='Корректирующие';en='corrective'");
		КонецЕсли;
...
Процедура УправлениеФорматомОтвета(Форма)
	...
	Если Объект.ФорматОтвета = ПредопределенноеЗначение("Перечисление.ФорматОтветаНаЗапросИОН.RTF") Тогда
		Форма.ФорматОтветаПредставление = НСтр("ka='Текст в формате RTF (.rtf)';ru='Текст в формате RTF (.rtf)';en='Text in RTF format (.rtf)'");
	ИначеЕсли Объект.ФорматОтвета = ПредопределенноеЗначение("Перечисление.ФорматОтветаНаЗапросИОН.XLS") Тогда
		Форма.ФорматОтветаПредставление = НСтр("ka='ფურცელი Microsoft Excel 97-2003 (.xls)';ru='Лист Microsoft Excel 97-2003 (.xls)';en='Microsoft Excel 97-2003 List (.xls)'");
	ИначеЕсли Объект.ФорматОтвета = ПредопределенноеЗначение("Перечисление.ФорматОтветаНаЗапросИОН.XML") Тогда
		Форма.ФорматОтветаПредставление = НСтр("ka='XML-документ (.xml)';ru='XML-документ (.xml)';en='XML-document (.xml)'");
	ИначеЕсли Объект.ФорматОтвета = ПредопределенноеЗначение("Перечисление.ФорматОтветаНаЗапросИОН.PDF") Тогда
		Форма.ФорматОтветаПредставление = НСтр("ka='დოკუმენტი Adobe PDF (.pdf)';ru='Документ Adobe PDF (.pdf)';en='Document Adobe PDF (.pdf)'");
	Иначе
		Форма.ФорматОтветаПредставление = НСтр("ka='არ არის შერცეული';ru='Не выбран';en='not selected'");
	КонецЕсли;
