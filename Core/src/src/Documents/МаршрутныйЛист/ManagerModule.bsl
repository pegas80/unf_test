...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Иначе
		...
		Если МассивДокументов.Количество()=0 Тогда
			ТекстСообщенияОбОшибке = НСтр("ka='По маршрутному листу нет сформированных расходных документов. Данные для печати отсутствуют.';ru='По маршрутному листу нет сформированных расходных документов. Данные для печати отсутствуют.';en='According to the itinerary is not formed expense receipts. Print data is not available.'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='სამარშრუტო ფურცელი';ru='Маршрутный лист';en='itinerary'");
		...
		Пока ВыборкаДокументов.Следующий() Цикл
			...
			Пока ВыборкаСтрокЗаказы.Следующий() Цикл
				...
				Если ПустаяСтрока(ТекстКонтактноеЛицо) Тогда
					ТекстКонтактноеЛицо = НСтр("ka='არ არის მითითებული';ru='Не указано';en='Not specified'");
				КонецЕсли; 
				...
				Если НЕ ЗначениеЗаполнено(ВыборкаСтрокЗаказы.ДатаДоставки)
					...
					ТекстДатаИВремяДоставки = НСтр("ka='Не указаны';ru='Не указаны';en='not specified'");
				Иначе
...
Процедура ВыполнитьОтгрузку(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	Если НЕ Параметры.Свойство("МаршрутныйЛист") ИЛИ НЕ ЗначениеЗаполнено(Параметры.МаршрутныйЛист) Тогда
		Ошибка = НСтр("ka='Не указан маршрутный лист для выполнения отгрузки';ru='Не указан маршрутный лист для выполнения отгрузки';en='Unknown itinerary to perform shipment'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Исключение
				...
				Ошибка = СтрШаблон(НСтр("ka='Не удалось сформировать отгрузку для заказа %1: %2';ru='Не удалось сформировать отгрузку для заказа %1: %2';en='Unable to generate shipping Order% 1% 2'"), Выборка.Заказ, РекурсивноеОписаниеОшибки(Информация));
