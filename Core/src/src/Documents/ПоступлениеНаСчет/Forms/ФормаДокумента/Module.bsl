...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	И НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником") И
			Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.РасчетыПоКредитам") И
			Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
	...
	// Олег Литвиненко 13.10.22 --->
	// СтандартныеПодсистемы.ЗагрузкаДанныхИзВнешнегоИсточника
	ЗагрузкаДанныхИзВнешнегоИсточника.ПриСозданииНаСервере(Метаданные.Документы.ПоступлениеНаСчет.ТабличныеЧасти.РасшифровкаПлатежаОтАгента, НастройкиЗагрузкиДанных, ЭтотОбъект);
	...
	УстановитьВидимостьДляОперацииОтКурьерскойКомпанииПочты();
	// Олег Литвиненко <---	
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВидОперации  = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам") Тогда
		...
		//Малышев 2024-09-29 ONESOFT-3474 - закоентировал (--
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником") Или
		ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Или
	...
	// Закомментировал Олег Литвиненко 20.10.22. По заданию от Giorgi Sigua
	//Если ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты") Тогда
...
Процедура ПометитьНаУдалениеИнтерактивно(Команда)
	...
	Если Модифицированность Тогда
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru = 'Для снятия пометки удаления необходимо записать внесенные вами изменения. Записать данные?'; ka = 'წაშლის ნიშნულის მოხსნისთვის აუცილებელია შეინახოთ შეტანილი ცვლილებები. შეინახოს მონახემები?'; en = 'To deselect removal is necessary to record your changes. Write data?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Для установки отметки удаления необходимо записать внесенные вами изменения. Записать данные?'; ka = 'წაშლაზე მონიშნისთვის აუცილებელია შეინახოთ შეტანილი ცვლილებები. შეინახოს მონახემები?'; en = 'To set the mark removal is necessary to record your changes. Write data?'");
		КонецЕсли;
	Иначе
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru = 'Снять с ""'; ka = 'მოიხსნას ""'; en = 'Remove from the ""'")+Объект.Ссылка + НСтр("ru = '"" пометку на удаление?'; ka = '"" წაშლაზე მონიშვნა?'; en = '""Marked for deletion?'");
		Иначе
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Контрагент, Организация, Дата)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.РасчетыПоКредитам
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом Тогда
...
// Олег Литвиненко 15.09.22 --->
&НаСервере
Процедура УстановитьВидимостьДляОперацииОтКурьерскойКомпанииПочты()
	
	Видимость = (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты"));
	Элементы.РасчетСКурьером.Видимость = Видимость;
	
	Элементы.ПриходнаяНакладная.Видимость = Объект.РасчетСКурьером;
	Элементы.РасшифровкаПлатежаОтАгентаСуммаРасчета.Видимость = Объект.РасчетСКурьером;
...
Функция ПолучитьДанныеДатаПриИзменении(ДатаПередИзменением)
	...
	// Kuzmich, 08.08.2023, ONESOFT-2354, Возврат кредита контрагентом - поступления в кассу и поступления на счет
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом Тогда
		РазностьДат = Объект.Дата - ДатаПередИзменением;
	КонецЕсли;
	// Kuzmich, 08.08.2023, ONESOFT-2354, Возврат кредита контрагентом - поступления в кассу и поступления на счет
...
Процедура УстановитьВидимостьДокументыПланирования()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом Тогда
		Элементы.ДокументыПланирования.Видимость = Ложь;
	Иначе
		Элементы.ДокументыПланирования.Видимость = Истина;
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты") Тогда
		...
		Элементы.ДоговорСАгентом.Заголовок = ?(ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"), НСтр("ru = 'Договор (руб.)'; ka = 'ხელშეკრულება (руб.)'; en = 'Treaty (rub.)'"), НСтр("ka='ხელშეკრულება';ru='Договор';en='Agreement'"));
...
Процедура ОбновитьКурсВРасшифровкеПлатежаИПересчитатьСуммы(КнопкаРезультат, ДополнительныеПараметры) Экспорт
	...
	Если КнопкаРезультат = КодВозвратаДиалога.Да Тогда
		// Kuzmich, 08.08.2023, ONESOFT-2354, Возврат кредита контрагентом - поступления в кассу и поступления на счет
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
			ОбработатьИзменениеДоговораКредитаИлиЗайма();
			Возврат;
		КонецЕсли;
		// Kuzmich, 08.08.2023, ONESOFT-2354, Возврат кредита контрагентом - поступления в кассу и поступления на счет
...
Процедура ЗаполнитьДолги(ПересчитыватьСлужебныеДанные)
	...
	|	РасчетыСПокупателямиОстатки.Договор.СтатьяДДСПоУмолчанию КАК СтатьяДДС, //+++ Дмгг
	...
	|	ДвиженияДокументаРасчетыСПоставщиками.Договор.СтатьяДДСПоУмолчанию, //+++ Дмгг
	...
	Если РезультатЗакрытия = КодВозвратаДиалога.Да Тогда
		...
		Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
			...
			ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты")
				...
				Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником") Тогда
		...
		КонецЦикла;
		Объект.ЭквайринговыеОперации.Очистить();
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
		Объект.Корреспонденция = Неопределено;
		Объект.КорреспонденцияРегл = Неопределено;//+++ Дмгг
		Объект.Контрагент = Неопределено;
		Объект.СчетКонтрагента = Неопределено;
...
Процедура НастроитьЭлементДоговораЗайма()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом Тогда
...
Процедура ДоговорСАгентомНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ВидОперации) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Сначала выберите операцию.'; ka = 'თავდაპირველად აირჩიეთ ოპერაციის ტიპი.'; en = 'First select the operation.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Сначала выберите контрагента.'; ka = 'თავდაპირველად აირჩიეთ კონტრაგენტი.'; en = 'First, select a contractor.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.РасшифровкаПлатежа.Количество() = 0) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Ошибка заполнения документа. Проверьте заполнение организации, операции, контрагента.'; ka = 'დოკუმენტის შევსების შეცდომა. შეამოწმეთ შევსებულია თუ არა ორგანიზაცია, ოპერაციის ტიპი, კონტრაგენტი.'; en = 'Error filling document. Check the filling of the organization, operations, contractor.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоДоговоруКредита(Команда)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
		ЗаполнитьПоДоговоруЗайма(Команда);
	Иначе
...
Процедура ЗаполнитьИнформациюПоКредитуЗаймуНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом") Тогда
...
Процедура ЗаполнитьИнформациюПоЗаймуНаСервере()
	...
	Если Объект.ДоговорКредитаЗайма.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику
		Или Объект.ДоговорКредитаЗайма.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный Тогда
		Множитель = 1;
	Иначе
		Множитель = -1;
	КонецЕсли;
...
Процедура УстановитьВидимостьРеквизитовВЗависимостиОтКорреспонденции(ОчиститьАналитику = Ложь, ЭлементАналитикиНаименование = "")
	...
	// Олег Литвиненко 10.11.22 --->
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.Прочее Тогда
		Если ЗначенияРеквитов.ТипСчета = Перечисления.ТипыСчетов.ПрочиеДоходы Тогда 
			
			Элементы.НаправлениеДеятельности.Видимость = Истина;
			Элементы.Подразделение.Видимость = Истина;
			Элементы.Заказ.Видимость = Истина;
			Если НЕ ЗначениеЗаполнено(Объект.ПодразделениеЗатраты) Тогда
				Пользователь = Пользователи.ТекущийПользователь();
				ЗначениеНастройки = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "ОсновноеПодразделение");
				Объект.ПодразделениеЗатраты = ?(ЗначениеЗаполнено(ЗначениеНастройки), ЗначениеНастройки, Справочники.СтруктурныеЕдиницы.ОсновноеПодразделение);
			КонецЕсли;
...
Функция ПолучитьДоговорКредитаЗаймаПоУмолчанию(Документ, Контрагент, Организация, ВидОперации)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом Тогда
...
Функция ДобавитьСтрокуАвансаНаСервере()
	...
	НоваяСтрока.СтатьяДДС = НоваяСтрока.Договор.СтатьяДДСПоУмолчанию; //+++ Дмгг
...
Процедура УстановитьЗаголовкиКолонокТабличныхЧастей(ВалютаСсылка)
	...
	Элементы.ТаблицаДолговДолгВалПлатежа.Заголовок = НСтр("ru = 'Текущий долг '; ka = 'მიმდინარე ვალდებულება'; en = 'Current liability'")+ПредставлениеВалюты;
	Элементы.ТаблицаДолговЗачесть.Заголовок = НСтр("ru = 'Зачитываем '; ka = 'ჩასათვლელად'")+ПредставлениеВалюты;
	
	Элементы.РасшифровкаПлатежаСуммаПлатежа.Заголовок = НСтр("ru = 'Сумма '; ka = 'თანხა'; en = 'Amount'")+ПредставлениеВалюты;
...
Процедура ПодборРасшифровкиПлатежаОтАгента(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru = 'Укажите вначале контрагента!'; ka = 'თავდაპირველად მიუთითეთ კონტრაგენტი!'; en = 'Specify the first contractor!'"));
		Возврат;
	КонецЕсли;
...
Процедура РасчетыПрочиеКорреспонденцияРеглПриИзменении(Элемент)
	...
	// Олег Литвиненко 10.11.22 --->
	Если ИспользоватьТолькоРегламентированногоПланаСчетов Тогда 
		УстановитьВидимостьРеквизитовВЗависимостиОтКорреспонденцииРегл();
	Иначе
		УстановитьВидимостьРеквизитовВЗависимостиОтКорреспонденции();
	КонецЕсли
	// Олег Литвиненко <---
...
// Олег Литвиненко 15.09.22 --->
&НаКлиенте
Процедура РасчетСКурьеромПриИзменении(Элемент)
...
// Олег Литвиненко 15.09.22 --->
&НаКлиенте
Процедура ПриходнаяНакладнаяПриИзменении(Элемент)
...
&НаКлиенте
Процедура СформироватьСуммуРасчета()
	
	ТекущиеДанные = Элементы.РасшифровкаПлатежаОтАгента.ТекущиеДанные;
	
	ТекущиеДанные.СуммаРасчета = ЗначениеРеквизитаОбъекта(ТекущиеДанные.Документ, "СуммаДокумента") - ТекущиеДанные.СуммаПлатежа;
		
КонецПроцедуры// Олег Литвиненко <---


// Олег Литвиненко 29.09.22 --->
&НаСервереБезКонтекста
...
// Олег Литвиненко 13.10.22 --->
&НаКлиенте
Процедура ЗагрузкаИзФайлаРасшифровкаПлатежаОтАгента(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='მარაგების ჩატვირთვა ფაილიდან';ru='Загрузка из файла';en='Download the file from the reserves'"));
	НастройкиЗагрузкиДанных.Вставить("СодержаниеВидимо", Истина);
	НастройкиЗагрузкиДанных.Вставить("ПоложениеЗаказаВДокументе", ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти"));
...
Процедура ЗагрузитьИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		ЗаполнитьСтрокуРасшифровкаПлатежаОтАгента(НоваяСтрока, СтрокаТаблицы);
...
&НаСервере
Процедура ЗаполнитьСтрокуРасшифровкаПлатежаОтАгента(НоваяСтрока, СтрокаТаблицы)
	ИменаСвойств = "Заказ, СуммаПлатежа, Документ, СуммаРасчета";
				
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы, ИменаСвойств);
	
	РасшифровкаПлатежаОтАгентаЗаказПриИзмененииНаСервере(НоваяСтрока.ПолучитьИдентификатор());
	
	НоваяСтрока.ПолученоОтКлиента = НоваяСтрока.СуммаПлатежа;
	
	Если Не ЗначениеЗаполнено(НоваяСтрока.СуммаРасчета) Тогда
		НоваяСтрока.СуммаРасчета = ?(ЗначениеЗаполнено(НоваяСтрока.Документ), ЗначениеРеквизитаОбъекта(НоваяСтрока.Документ, "СуммаДокумента") - НоваяСтрока.СуммаПлатежа, 0);
	КонецЕсли;
КонецПроцедуры
