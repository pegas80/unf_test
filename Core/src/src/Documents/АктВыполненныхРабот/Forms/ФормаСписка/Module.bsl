...
Процедура СоздатьАктВыполненныхРабот(Команда)
	...
	Иначе
		...
		Если СтруктураДанных.СформироватьНесколькоЗаказов Тогда
			...
			ТекстСообщения = НСтр("ka='შეკვეთები განსხვავდება დოკუმენტების თავსართის მონაცემებით (%ПредставлениеДанных%)! ჩამოვაყალიბოთ შესრულებული სამუშაოების რამდენიმე აქტი?';ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько актов выполненных работ?';en='Orders different data (% PredstavlenieDannyh%) hats documents! Form several acts of work performed?'");
