...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [18 января 2023]  НАЧАЛО
	//ЗаполнитьКолонкуУчитыватьНДФЛ();//Паганини
	// © Шишка В.В. [18 января 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	УстановитьДостуаностьЭлементовПослеОтправкиДокументаВБанк();
	//--- Дмгг, 27.05.2025, ONESOFT-4091
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту")
		Или ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам") Тогда
		ОбработатьИзменениеДоговораКредитаИлиЗайма();
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	Элементы.РасчетыПоКредитам.Заголовок = "ანგარიშსწორება კრედიტების მიხედვით";
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту") Тогда
		
		Элементы.РасчетыПоКредитам.Заголовок = НСтр("ka='კრედიტების მიხედვით ანგარიშსწორება';ru='Расчеты по кредитам';en='loan calculations'");
		Элементы.РасчетыПоКредитам.Видимость = Истина;
		Элементы.РасшифровкаПлатежаРасчетыПоКредитам.Видимость = Ложь;
		...
		Элементы.ВыплатаЗаработнойПлатыГруппаСотрудник.Видимость = Ложь;
		Элементы.ГруппаПлатежныеВедомости.Видимость = Ложь;
		Элементы.ВыплатаЗаработнойПлатыПодборПоВедомости.Видимость = Ложь;
...
Процедура ОбновитьКурсыРасчетовРасшифровкиПлатежаНаСервере()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.РасшифровкаПлатежа Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор) и Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам") Тогда
			СтруктураДанные = ПолучитьДанныеРасшифровкаПлатежаДоговорПриИзменении(
				Объект.Дата,
				СтрокаТабличнойЧасти.Договор
			);
		...
		Иначе 
			ТекДанные = Новый Структура("Курс,Кратность");
			ЗаполнитьКурсИКратност(ТекДанные);	
			СтрокаТабличнойЧасти.Курс = ?(ТекДанные.Курс = 0,1,ТекДанные.Курс);
			СтрокаТабличнойЧасти.Кратность = ?(ТекДанные.Кратность = 0,1,ТекДанные.Кратность);
		КонецЕсли;
...
Процедура ЗаполнитьРасшифровкуПлатежа(ТекущийОбъект = Неопределено)
	...
	Основание = Документ.ДокументОснование; 
	ПоПриходнаяНакладная = (ТипЗнч(Основание) = Тип("ДокументСсылка.ПриходнаяНакладная"));

	//Malyshev 2024-05-23 ONESOFT-3069 -->
	//Закоментировал, т.к. считало, что документ всегда на основании чего то сделан
	//
	//Если Не ПоПриходнаяНакладная Тогда 
	//	Для Каждого ТекСтр Из Документ.РасшифровкаПлатежа Цикл
	//		Если ТипЗнч(ТекСтр.Документ) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
	//			Основание = ТекСтр.Документ;
	//			ПоПриходнаяНакладная = Истина;
	//			Прервать;
	//		КонецЕсли;
	//	КонецЦикла;
	//КонецЕсли;
	...
	Если ПоПриходнаяНакладная Тогда
		//--------------------------------------------------------------------------
		// © Шишка В.В. [21 августа 2023]  НАЧАЛО
		//Документ.ЗаполнитьПоПриходнаяНакладная(Основание);
		Документ.ЗаполнитьРасшифровкуПлатежа(Основание);
		// © Шишка В.В. [21 августа 2023]  КОНЕЦ
		//--------------------------------------------------------------------------
	Иначе
		Документ.ЗаполнитьРасшифровкуПлатежа();
	КонецЕсли;
...
Процедура РасшифровкаПлатежаДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Документ) И
		ТипЗнч(СтрокаТабличнойЧасти.Документ) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
		Возврат;
	КонецЕсли;
	...
	//+++ Дмгг	
	ИначеЕсли ЗначениеЗаполнено(СтрокаТабличнойЧасти.Документ) И
		ТипЗнч(СтрокаТабличнойЧасти.Документ) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда  
...
Процедура ВыплатаЗаработнойПлатыВедомостьПриИзменении(Элемент)
	...
	Иначе
		...
		//ПодольскийАИ #УчетПроектов 06.09.2021 {
		Если ИспользоватьПроекты Тогда
			СтрокаИД = СтрокаТабличнойЧасти.ПолучитьИдентификатор();
			ЗаполнитьПроектПоВедомости(СтрокаИД);
		Иначе
		//ПодольскийАИ #УчетПроектов 06.09.2021 }
		СтрокаТабличнойЧасти.СуммаПлатежа = ПолучитьДанныеВыплатаЗаработнойПлатыВедомостьПриИзменении(СтрокаТабличнойЧасти.Ведомость);
		СтрокаТабличнойЧасти.СуммаЛьгот = ПолучитьДанныеВыплатаНалоговыхЛьготЗаработнойПлатыВедомостьПриИзменении(СтрокаТабличнойЧасти.Ведомость);
		//ПодольскийАИ #УчетПроектов 06.09.2021 {
		КонецЕсли;
		//ПодольскийАИ #УчетПроектов 06.09.2021 }
	КонецЕсли;
...
Процедура ЗаполнитьРасшифровкуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	// Закомментировал Олег Литвиненко 20.04.23
	// Переместил в ЗаполнитьРасшифровкуПлатежа
	//Объект.РасшифровкаПлатежа.Очистить();
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Поставщику") Тогда
		
		ЗаполнитьРасшифровкуПлатежа(); 
		ЗаполнитьКолонкуУчитыватьНДФЛ();//++ 25.09.2025, ONESOFT-4375
...
&НаСервере
Функция РасчетыПоКредитам(СтруктураДанные)
...
Функция ПолучитьДанныеДатаПриИзменении(ДатаПередИзменением)
	...
	//ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС();  //+++ Дмгг, 19.12.2025, ONESOFT-4575
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеВыплатаНалоговыхЛьготЗаработнойПлатыВедомостьПриИзменении(Ведомость)
	
	Возврат Ведомость.Сотрудники.Итог("СуммаЛгот");
		...
		Если Объект.РасшифровкаПлатежа.Количество() > 0
		   ...
		   ИЛИ Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ЗарплатаВРазрезеСотрудников")) Тогда // Kuzmich, 23.02.2019,Зарплата по ведомости в разрезе сотрудников
			...
			Иначе
				...
				Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Зарплата") Тогда
					Для каждого СтрокаТабличнойЧасти Из Объект.ВыплатаЗаработнойПлаты Цикл
						 ДанныеВедомости = ПолучитьДанныеВедомости(СтрокаТабличнойЧасти.Ведомость); 
						 //Объект.ВалютаДенежныхСредств, СтрокаТабличнойЧасти.Ведомость, КурсПередИзменением, Курс, КратностьПередИзменением, Кратность);
						 Если ДанныеВедомости.ВалютаДокумента = Объект.ВалютаДенежныхСредств
							 и ДанныеВедомости.ВалютаДокумента <> ДанныеВедомости.ВалютаРасчетов
							 Тогда
							 
							СтрокаТабличнойЧасти.СуммаПлатежа = ДанныеВедомости.СуммаРасчетов / (Курс * Кратность); 
...
&НаСервере
Функция ПолучитьДанныеВедомости(Ведомость)

	Результат = Новый Структура("ВалютаДокумента, ВалютаРасчетов, СуммаДокумента, СуммаРасчетов");	
	ЗаполнитьЗначенияСвойств(Результат, Ведомость);
	Возврат Результат;
...
Процедура РассчитатьСуммуПлатежаНаКлиенте(СтрокаТабличнойЧасти, ИмяКолонки = "")
	...
	РасчетыПоКредитам(СтруктураДанные);
...
Процедура ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС()
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [31 октября 2023]  НАЧАЛО
	ИначеЕсли ЗначениеЗаполнено(Объект.ДокументОснование) И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
	
		Объект.НалогообложениеНДС = Объект.Контрагент.НалогообложениеНДС;
	// © Шишка В.В. [31 октября 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	Иначе
...
Процедура УстановитьВидимостьДокументыПланирования()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПрочиеРасчеты
		...
		Или Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту Тогда
		Элементы.ДокументыПланирования.Видимость = Ложь;
	// Конец Прочие расчеты
	Иначе
		Элементы.ДокументыПланирования.Видимость = Истина;
...
Процедура ЗаполнитьДоговор(Параметры = Неопределено)
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику")
		И Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту")
		И Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам") Тогда
...
Процедура ОбновитьВидимостьИОстаткиВзаиморасчетов()
	...
	Элементы.ОстатокВзаиморасчетов.Заголовок = ЗаголовокНадписиВзаиморасчетов(Объект.Контрагент, ОстатокВзаиморасчетов);
	// Олег Литвиненко <---
	
КонецПроцедуры

// Олег Литвиненко 22.05.23 --->
// Скопирована из РаботаСКонтрагентамиУНФ
&НаСервере
Функция ЗаголовокНадписиВзаиморасчетов(Контрагент, СуммаДолга = 0) Экспорт
	...
	// TODO: Решить что делать, если запрос вернет две записи
	Запрос.УстановитьПараметр("МоментВремени", ?(ЗначениеЗаполнено(Объект.Ссылка), Новый Граница(
	...
	Если Сумма < 0 Тогда
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='ჩვენი ვალი';ru='Мы должны';en='We must'") + " ", КрупныйШрифт));
		Сумма = -Сумма;
	Иначе
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='დავალიანება ჩვენს მიმართ';ru='Должен нам';en='should we'") + " ", КрупныйШрифт));
	КонецЕсли;
...
Процедура ЗаполнитьДолги(ПересчитыватьСлужебныеДанные)
	...
	|	ВременнаяТаблицаДолгов.Договор.СтатьяДДСПоУмолчанию КАК СтатьяДДС, //+++ Дмгг
	...
	|АВТОУПОРЯДОЧИВАНИЕ";
	//--------------------------------------------------------------------------
	// © Шишка В.В. [1 декабря 2023]  НАЧАЛО
	Запрос.Текст = Запрос.Текст + Символы.ПС + "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаДолгов.Организация КАК Организация,
	|	ВременнаяТаблицаДолгов.Договор КАК Договор,
	|	ВременнаяТаблицаДолгов.Договор.СтатьяДДСПоУмолчанию КАК СтатьяДДС, //+++ Дмгг
	|	ВременнаяТаблицаДолгов.Документ КАК Документ,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Заказ,
	|	ВременнаяТаблицаДолгов.Заказ.Дата КАК ДатаЗаказа,
	|	ВременнаяТаблицаДолгов.ДолгВалДоговора КАК ДолгВалДоговора,
	|	ВременнаяТаблицаДолгов.ДокументДата КАК ДатаДокумента,
	|	ВременнаяТаблицаДолгов.Курс КАК Курс,
	|	ВременнаяТаблицаДолгов.Кратность КАК Кратность,
	|	ВременнаяТаблицаДолгов.Вниз КАК Вниз,
	|	ВременнаяТаблицаДолгов.Добавить КАК Добавить,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ВременнаяТаблицаДолгов.Документ) = ТИП(Документ.ОтчетПереработчика)
	|			ТОГДА ВременнаяТаблицаДолгов.Документ.Сумма
	|		КОГДА ТИПЗНАЧЕНИЯ(ВременнаяТаблицаДолгов.Документ) = ТИП(Документ.Взаимозачет)
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ДатаЗаказаБольшеДатыДокумента,
	|	ВременнаяТаблицаДолгов.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
	|	ВременнаяТаблицаДолгов.ВидУдержанияПФ КАК ВидУдержанияПФ,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаДолгов.ВидУдержанияПФ, ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	...
	|					ТОГДА ВременнаяТаблицаДолгов.Документ.Сумма
	|				ИНАЧЕ ВременнаяТаблицаДолгов.Документ.СуммаДокумента
	|			КОНЕЦ * ВременнаяТаблицаДолгов.ВидУдержанияПФ.СтавкаПФ / 100
	|	КОНЕЦ КАК СуммаПФ,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаДолгов.ВидУдержанияНДФЛ, ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	|			ТОГДА 0
	...
	|							ТОГДА ВременнаяТаблицаДолгов.Документ.Сумма
	|						ИНАЧЕ ВременнаяТаблицаДолгов.Документ.СуммаДокумента
	|					КОНЕЦ * ВременнаяТаблицаДолгов.ВидУдержанияНДФЛ.СтавкаНДФЛ / 100
	|			КОНЕЦ) * ВременнаяТаблицаДолгов.ВидУдержанияНДФЛ.СтавкаНДФЛ / 100
	|	КОНЕЦ КАК СуммаНДФЛ,
	|	ВременнаяТаблицаДолгов.ВидУдержанияНДФЛ.СтавкаНДФЛ КАК СтавкаНДФЛ
	|ИЗ
	|	ВременнаяТаблицаДолгов КАК ВременнаяТаблицаДолгов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСчетаНаОплату КАК ВременнаяТаблицаСчетаНаОплату
	|		ПО ВременнаяТаблицаДолгов.Заказ = ВременнаяТаблицаСчетаНаОплату.Заказ,
	...
	|АВТОУПОРЯДОЧИВАНИЕ
	|";
	// © Шишка В.В. [1 декабря 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДолгов Цикл
		...
		//КонецЕсли;
		Если ТипЗнч(ТекущаяСтрока.Документ) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
		
			ЗаполнитьСуммыНДФЛ_ПФ_СтрокиЗакупкаУслуг(ТекущаяСтрока, КонтрагентЯвляетсяНеРезидентом);	
...
//--------------------------------------------------------------------------
// © Шишка В.В. [1 декабря 2023]  НАЧАЛО
Процедура ЗаполнитьСуммыНДФЛ_ПФ_СтрокиЗакупкаУслуг(ТекДанные, КонтрагентЯвляетсяНеРезидентом)

	КонтрагентЯвляетсяНеРезидентом = Объект.Контрагент.ЯвляетсяНеРезидентом;

	Ставки	= РС_ОбщегоНазначения.ПолучитьЗначенииСтавок(ТекДанные.СтавкаНДС,ТекДанные.ВидУдержанияНДФЛ, ТекДанные.ВидУдержанияПФ);
	СуммаВключаетНДС = ТекДанные.Документ.СуммаВключаетНДС;
	Сумма			= Окр(ТекДанные.ДолгВалДоговора, 2);
	Если СуммаВключаетНДС Тогда
		СуммаНДФЛ		= Окр(Сумма/((100-Ставки.СтавкаНДФЛ)/100)*(Ставки.СтавкаНДФЛ/100), 2);
		СуммаПФ			= Окр(((Сумма+СуммаНДФЛ)/((100-Ставки.СтавкаПФ)/100))*(Ставки.СтавкаПФ/100), 2);
		СуммаДляНДС = Сумма;
	...
	Иначе
		...
		//СуммаПФ			= Окр(((Сумма + СуммаНДС + СуммаНДФЛ)/((100-Ставки.СтавкаПФ)/100)) * (Ставки.СтавкаПФ / 100), 2);
		//++lazare 08.02.2024
...
//+++ Дмгг, 27.05.2025, ONESOFT-4091
&НаСервере
Процедура УстановитьДостуаностьЭлементовПослеОтправкиДокументаВБанк()

	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		//ЭтаФорма.ТолькоПросмотр = Истина;     
		//ЭтаФорма.Элементы.БанковскийСчет.ТолькоПросмотр = Ложь;    
		//ЭтаФорма.Элементы.СчетКонтрагента.Имя Доступность
		//ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма.Элементы.СчетКонтрагента, "ТолькоПросмотр")      
		Если НЕ Документы.РасходСоСчета.ПолучитьДанныеОФормированииПроводок(Объект.Ссылка, Истина) Тогда 
			Для Каждого ЭлемФорми Из ЭтаФорма.Элементы Цикл 
				
				ИмяПодходит = ЛОЖЬ;
				ИмяПодходит = ЭлемФорми.Имя = "РасшифровкаПлатежаДоговор" 
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаПроект" 
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаПризнакАванса" 
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаДокумент" 
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежа"
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаСтатьяДДС"   
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаСуммаРасчетовГруппа"
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаСуммаРасчетов"
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаСуммаПлатежа"   
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаЗаказ"  
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаСчетНаОплату"
				ИЛИ ЭлемФорми.Имя = "СтраницаРучноеЗаполнение"
				ИЛИ ЭлемФорми.Имя = "СтраницыРасчетовСКонтрагентом"
				ИЛИ ЭлемФорми.Имя = "РасчетыСКонтрагентом"
				ИЛИ ЭлемФорми.Имя = "Шапка"
				ИЛИ ЭлемФорми.Имя = "ПраваяКолонка"     
				ИЛИ ЭлемФорми.Имя = "Группа3"   
				ИЛИ ЭлемФорми.Имя = "ГруппаНомерДата"
				ИЛИ ЭлемФорми.Имя = "Дата"     
				ИЛИ ЭлемФорми.Имя = "ГруппаДополнительно"
				ИЛИ ЭлемФорми.Имя = "ГруппаВидОперацииОрганизация"     
				//+++ Дмгг, я был против и проинформировал про это
				ИЛИ ЭлемФорми.Имя = "ВидОперации"                 
				//--- Дмгг, я был против и проинформировал про это
				ИЛИ ЭлемФорми.Имя = "ГруппаНазначениеКомментарийИСтатья"
				ИЛИ ЭлемФорми.Имя = "ГруппаКомментарий" 
				ИЛИ ЭлемФорми.Имя = "ГруппаНазначениеПлатежаИСтатья" 
				ИЛИ ЭлемФорми.Имя = "ГруппаНазначение"     
				ИЛИ ЭлемФорми.Имя = "НазначениеПлатежа"
				ИЛИ ЭлемФорми.Имя = "Комментарий"
				ИЛИ ЭлемФорми.Имя = "ГруппаСтатья" 
				ИЛИ ЭлемФорми.Имя = "Статья"
				ИЛИ ЭлемФорми.Имя = "РасшифровкаПлатежаПризнакАвансаИДокумент";
				
				Если ИмяПодходит Тогда 
					Продолжить;
				КонецЕсли;
				
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлемФорми, "ТолькоПросмотр") Тогда 
					ЭлемФорми.ТолькоПросмотр = Истина;	
				КонецЕсли;
...
Процедура ПолучитьСтатусДокументаВИнтернетБанке()
	...
	Если ИспользуетсяИнтеграцияСБанком 
		И Объект.БанковскийСчет.Банк.СВИФТБИК = "BAGAGE22" 
		ИЛИ Объект.БанковскийСчет.Банк.СВИФТБИК = "MIBGGE22" Тогда 
...
Процедура УстановитьТекущуюСтраницу()
	...
	Если КоличествоСтрок = 0 Тогда
		...
		Если ИспользоватьПроекты И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Зарплата") Тогда
			Для Каждого СтрокаЗарплаты Из Объект.ВыплатаЗаработнойПлаты Цикл 
				СтрокаРасшифровка = Объект.РасшифровкаПлатежа.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРасшифровка, СтрокаЗарплаты);
			КонецЦикла;
			КоличествоСтрок = Объект.РасшифровкаПлатежа.Количество();
		Иначе
		//ПодольскийАИ #УчетПроектов 25.11.2020 }
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику") Тогда
		...
		КонецЦикла;
		Объект.ЭквайринговыеОперации.Очистить();
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту") Тогда
		Объект.Корреспонденция = Неопределено;
		Объект.КорреспонденцияРегл = Неопределено; //+++ Дмгг
		Объект.Контрагент = Неопределено;
		Объект.СчетКонтрагента = Неопределено;
...
Процедура НастроитьЭлементДоговораЗайма()
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту Тогда
		Элементы.ДоговорКредита.ПодсказкаВвода = НСтр("ka='ხელშეკრულების ასარჩევად აირჩიეთ კონტრაგენტი';ru='Чтобы выбрать договор, выберите контрагента';en='To select the contract, select counterparty'");
	Иначе
...
Процедура УстановитьВидимостьСчетаПолучателяДляИнтернетБанка()
	...
	Если ИспользуетсяИнтеграцияСБанком И 
		...
		ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту"))
...
Процедура УстановитьВидимостьПроекта()
	...
	ВидимостиПроекта = ВидОперации <> Перечисления.ВидыОперацийРасходСоСчета.Поставщику
...
Процедура ЗаполнитьПоДоговоруКредита(Команда)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту") Тогда
		ЗаполнитьПоДоговоруКредитаВыданнного();
	Иначе // Расчеты по кредиту полученному
		ЗаполнитьПоДоговоруКредитаПолученного();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоДоговоруКредитаВыданнного()
...
&НаКлиенте
Процедура ЗаполнитьПоДоговоруКредитаПолученного()
	...
	ПараметрыОтбора = Новый Структура("
		...
		Объект.Подотчетник);
	//--------------------------------------------------------------------------
	// © Шишка В.В. [26 ноября 2022]  НАЧАЛО
	ПараметрыОтбора.Вставить("Проект", Объект.ПроектДляЗачетаАвансаСПомощником);	
	// © Шишка В.В. [26 ноября 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
	
	
	ОткрытьФорму("ОбщаяФорма.ФормаЗаполненияРасшифровкиПлатежаПоКредитамИЗаймам", 
						ПараметрыОтбора,
						ЭтаФорма,,,, Новый ОписаниеОповещения("ЗаполнитьПоДоговоруКредитаПолученногоЗавершение", ЭтотОбъект));
...
//
&НаКлиенте
Процедура ЗаполнитьПоДоговоруКредитаПолученногоЗавершение(РезультатЗаполнения, ПараметрыЗавершения) Экспорт
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику") Или
		ТекущийОбъект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту") Или
...
Процедура УстановитьПараметрыВыбораПоУчетуПрочихРасчетовНаСервереДляЭлементаСчетаКорреспонденции()
	...
	Для Каждого Параметр Из ЭлементРегл.ПараметрыВыбора Цикл
		Если Параметр.Имя = "Отбор.ТипСчета" Тогда
			...
			//Малышев 2024-07-05  ONESOFT-3309 ((--
...
Процедура ЗаполнитьИнформациюПоКредитуЗаймуНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту") Тогда
...
Процедура ЗаполнитьИнформациюПоКредитуНаСервере();
	...
	Если Объект.ДоговорКредитаЗайма.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.КредитПолученный Тогда
		Множитель = -1;
	Иначе
		Множитель = 1;
	КонецЕсли;
	...
	Если ВыборкаГрафикБудущиеМесяцы.Следующий() Тогда
		...
		Иначе
			ДатаПлатежа = "" + Формат(ВыборкаГрафикБудущиеМесяцы.Период, "ДФ=dd.MM.yyyy") + " (ამ თვეში არა)";
			НадписьИнформацияПоКредитуЦветТекста = ЦветаСтиля.ЦветТекстаФормы;
		КонецЕсли;
			
		НадписьИнформацияПоКредиту = "გადახდის თარიღი: "+ДатаПлатежа+". სესხის თანხა: "+Формат(ВыборкаГрафикБудущиеМесяцы.СуммаОсновногоДолга, "ЧДЦ=2; ЧН=0")+". თანხა %: "+
			Формат(ВыборкаГрафикБудущиеМесяцы.СуммаПроцентов, "ЧДЦ=2; ЧН=0")+
			". საკომისიო: "+Формат(ВыборкаГрафикБудущиеМесяцы.СуммаКомиссии, "ЧДЦ=2; ЧН=0")+" (" + ВыборкаГрафикБудущиеМесяцы.ВалютаРасчетовПредставление +")";
	...
	ИначеЕсли ВыборкаГрафик.Следующий() Тогда
		...
		Иначе
			ДатаПлатежа = "" + Формат(ВыборкаГрафик.Период, "ДФ=dd.MM.yyyy") + " (ამ თვეში არა)";
			НадписьИнформацияПоКредитуЦветТекста = ЦветаСтиля.ЦветТекстаФормы;
		КонецЕсли;
			
		НадписьИнформацияПоКредиту = "გადახდის თარიღი: "+ДатаПлатежа+". სესხის თანხა: "+Формат(ВыборкаГрафик.СуммаОсновногоДолга, "ЧДЦ=2; ЧН=0")+". თანხა %: "+
			Формат(ВыборкаГрафик.СуммаПроцентов, "ЧДЦ=2; ЧН=0")+
			". საკომისიო: "+Формат(ВыборкаГрафик.СуммаКомиссии, "ЧДЦ=2; ЧН=0")+" (" + ВыборкаГрафик.ВалютаРасчетовПредставление +")";
		
	Иначе
		
		НадписьИнформацияПоКредиту = "გადახდის თარიღი: <არ არის განსაზღვრული>";
	...
	Если ВыборкаОстатки.Следующий() Тогда
		...
		НадписьОстатокДолгаПоКредиту = "სესხის ნაშთი: "+Формат(Множитель * ВыборкаОстатки.ОсновнойДолгВалОстаток, "ЧДЦ=2; ЧН=0")+". თანხა %: "+
			Формат(Множитель * ВыборкаОстатки.ПроцентыВалОстаток, "ЧДЦ=2; ЧН=0")+
			". საკომისიო: "+Формат(Множитель * ВыборкаОстатки.КомиссияВалОстаток, "ЧДЦ=2; ЧН=0")+" (" + ВыборкаОстатки.ВалютаРасчетовПредставление +")";
	...
	Иначе
		...
		НадписьОстатокДолгаПоКредиту = "სესხის ნაშთი: <არ არის განსაზღვრული> "
...
Функция ПолучитьДоговорКредитаЗаймаПоУмолчанию(Документ, Контрагент, Организация, ВидОперации)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту Тогда
		СписокВидовДоговоров.Добавить(Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный);
	Иначе
		СписокВидовДоговоров.Добавить(Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику);
	КонецЕсли;
...
Процедура ПриИзмененииВТаблицеДолгов(ИмяКолонки, ТекущаяСтрокаДолговИдентификатор, УстанавливатьЗачитываемуюСумму = Истина)
	...
	//+++ Дмгг
	Если ЗначениеЗаполнено(ТекущаяСтрокаДолгов.Договор) Тогда 
		ТекущаяСтрокаРасшифровки.СтатьяДДС = ТекущаяСтрокаДолгов.СтатьяДДС; 
	КонецЕсли;
	//--- Дмгг
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		КонтрагентЯвляетсяНеРезидентом = Объект.Контрагент.ЯвляетсяНеРезидентом;
	КонецЕсли;
	Если ТипЗнч(ТекущаяСтрокаДолгов.Документ) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
		
		ЗаполнитьСуммыНДФЛ_ПФ_СтрокиЗакупкаУслуг(ТекущаяСтрокаДолгов, КонтрагентЯвляетсяНеРезидентом);	
...
Процедура РаспределитьРасшифровкуПоТаблицеДолгов()
	...
	Для Каждого ТекущаяСтрокаРасшифровки Из Объект.РасшифровкаПлатежа Цикл
		...
		Если МассивСтрок.Количество() > 0 Тогда
			...
			ЗаполнитьЗначенияСвойств(
				...
				//"СтавкаНДС, СуммаРасчетов, Курс, Кратность, СчетНаОплату"); //+++ Дмгг
				"СтавкаНДС, СуммаРасчетов, Курс, Кратность, СчетНаОплату, СтатьяДДС"); //--- Дмгг
	...
	Если НЕ ЕстьСтрокаАванса Тогда
		...
		СтрокаАванса.СтатьяДДС = СтрокаАванса.Договор.СтатьяДДСПоУмолчанию; //+++ Дмгг
		РасшифровкаПлатежаСуммаПлатежаПриИзмененииЗавершениеНаСервере(СтрокаАванса);
...
Функция ДобавитьСтрокуАвансаНаКлиенте()
	...
	Если ЗначениеЗаполнено(НоваяСтрока.Договор) Тогда 
		НоваяСтрока.СтатьяДДС = ПолучитьСтатьяДДСИзДоговораНаСервере(НоваяСтрока.Договор); //+++ Дмгг
	КонецЕсли;
	//+++ Дмгг
	...
	ИдентификаторСтрокиАванса = НоваяСтрока.ПолучитьИдентификатор();
	ЗаполнитьРасшифровкаПлатежаДоговорНаСервере(ИдентификаторСтрокиАванса);  //+++ Дмгг
...
&НаСервере    
Функция ПолучитьСтатьяДДСИзДоговораНаСервере(Договор)

	Возврат Договор.СтатьяДДСПоУмолчанию;

КонецФункции // ПолучитьСтатьяДДСИзДоговораНаСервере()
...
Процедура УстановитьЗаголовкиКолонокТабличныхЧастей(ВалютаСсылка)
	...
	Элементы.ТаблицаДолговДолгВалПлатежа.Заголовок = НСтр("ka='მიმდინარე დავალიანება ';ru='Текущий долг '")+ПредставлениеВалюты;
	Элементы.ТаблицаДолговЗачесть.Заголовок = НСтр("ka='ჩასათვლელი ';ru='Зачитываем '")+ПредставлениеВалюты;
	
	Элементы.РасшифровкаПлатежаСуммаПлатежа.Заголовок = НСтр("ka='თანხა ';ru='Сумма '")+ПредставлениеВалюты;
	...
	Если НЕ ИмяСобытия = "ПослеУдаленияСтроки" Тогда
		Если Идентификатор = Неопределено Тогда
			Для Каждого СтрокаТЧ Из Объект.РасшифровкаПлатежа Цикл
				...
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 декабря 2022]  НАЧАЛО
				//СтрокаТЧ.ВидУдержанияНДФЛ = СтруктураСтавок.ВидУдержанияНДФЛ;
				//СтрокаТЧ.ВидУдержанияПФ = СтруктураСтавок.ВидУдержанияПФ;
				мОснование = Неопределено;
				Если НЕ Параметры.Свойство("Основание", мОснование) ИЛИ НЕ ТипЗнч(мОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
				
					СтрокаТЧ.ВидУдержанияНДФЛ = СтруктураСтавок.ВидУдержанияНДФЛ;
					СтрокаТЧ.ВидУдержанияПФ = СтруктураСтавок.ВидУдержанияПФ;
				
				КонецЕсли; 	
				// © Шишка В.В. [11 декабря 2022]  КОНЕЦ
				//--------------------------------------------------------------------------
...
Процедура ОбработкаПодбораЗарплатаВРазрезеСотрудниковПоВедомости(Результат, ДополнительныеПараметры) ЭКСПОРТ
	Если ТипЗНЧ(Результат)=Тип("Массив") Тогда
		...
		Если мДобавитьПлатежнуюВедомость Тогда
			
			ДобавитьПлатёжныеВедомости(Результат);	
			
		КонецЕсли;
		мДобавитьПлатежнуюВедомость = Ложь;
...
Функция ОбработкаПодбораЗарплатаВРазрезеСотрудниковПоВедомостиНаСервере(СтрРезультат)
	Если ТипЗНЧ(СтрРезультат)=Тип("Структура") Тогда
		...
		//ПодольскийАИ #УчетПроектов 25.11.2020 {
		Если ИспользоватьПроекты Тогда
			СтруктураПоиска = Новый Структура("Ведомость,Сотрудник,Проект");
		Иначе
		//ПодольскийАИ #УчетПроектов 25.11.2020 }
...
Процедура ПересчитатьСуммыСтроки(ТекДанные)
	...
	Если (ТекДанные.ТипСуммы = Перечисления.ТипыСуммГрафикаКредитовИЗаймов.Проценты ИЛИ ТекДанные.ТипСуммы = Перечисления.ТипыСуммГрафикаКредитовИЗаймов.Комиссия) И Объект.ДоговорКредитаЗайма.СуммаВключаетНДФЛ  Тогда
	...
	//++Lazare
	Если  ТекДанные.СуммаРасчетов = 0 Тогда					
		Если (Курс = ТекДанные.Курс) И (Кратность = ТекДанные.Кратность) Тогда
			ТекДанные.СуммаРасчетов = ТекДанные.СуммаПлатежа;
		ИначеЕсли  Курс = 0 ИЛИ ТекДанные.Курс = 0 ИЛИ Кратность = 0 ИЛИ ТекДанные.Кратность = 0 Тогда
			...
			ТекДанные.СуммаРасчетов = ТекДанные.СуммаПлатежа;
		Иначе
			ТекДанные.СуммаРасчетов = Окр((ТекДанные.СуммаПлатежа * Курс * ТекДанные.Кратность) / (ТекДанные.Курс * Кратность), 2);
		КонецЕсли;	
	КонецЕсли;
	//--Lazare
...
&НаСервере
Процедура ЗаполнитьКурсИКратност(ТекДанные)
	
	 ДанныеДоговора = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Объект.Дата, Новый Структура("Валюта",Объект.ДоговорКредитаЗайма.ВалютаРасчетов));	 
	 ТекДанные.Курс     =  ?(ДанныеДоговора.Курс = 0, 1, ДанныеДоговора.Курс);
	 ТекДанные.Кратность = ?(ДанныеДоговора.Кратность = 0, 1, ДанныеДоговора.Кратность);
...
//ПодольскийАИ #УчетПроектов 06.09.2021 {
&НаСервере
Процедура ЗаполнитьПроектПоВедомости(СтрокаИД)
	
	СтрокаТЧ = Объект.ВыплатаЗаработнойПлаты.НайтиПоИдентификатору(СтрокаИД);
	Если СтрокаТЧ = Неопределено ИЛИ Не ЗначениеЗаполнено(СтрокаТЧ.Ведомость) Тогда
		СтрокаТЧ.СуммаПлатежа = 0;
		СтрокаТЧ.СуммаЛьгот = 0;
		СтрокаТЧ.Проект = Справочники.Проекты.ПустаяСсылка();
		Возврат;
	КонецЕсли;
	
	//Поиск и удаление строк по текущей ведомости
	ОтборСтрок = Новый Структура("Ведомость", СтрокаТЧ.Ведомость);
	СтрокиПоВедомости = Объект.ВыплатаЗаработнойПлаты.НайтиСтроки(ОтборСтрок);
	МассивСтрокДляУдаления = Новый Массив;
	Для Каждого СтрокаВедомости Из СтрокиПоВедомости Цикл 
		Если СтрокаВедомости.ПолучитьИдентификатор() <> СтрокаИД Тогда
			МассивСтрокДляУдаления.Добавить(СтрокаВедомости);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаУдалить Из МассивСтрокДляУдаления Цикл
		Объект.ВыплатаЗаработнойПлаты.Удалить(СтрокаУдалить);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПлатежнаяВедомостьСотрудники.Проект КАК Проект,
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ПлатежнаяВедомостьСотрудники.Проект";
	Запрос.УстановитьПараметр("Ведомость", СтрокаТЧ.Ведомость);
	РезультатЗапроса = Запрос.Выполнить();
	...
	ТекИндекс = Объект.ВыплатаЗаработнойПлаты.Индекс(СтрокаТЧ) + 1;
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			НоваяСтрока = Объект.ВыплатаЗаработнойПлаты.Вставить(ТекИндекс);
...
// © Шишка В.В. [10 августа 2022]  НАЧАЛО
&НаКлиенте
Процедура ПлатежныеВедомостиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	//Отказ = Истина;
	//мДобавитьПлатежнуюВедомость = Истина;
	//ОткрытьФорму("Справочник.Сотрудники.ФормаВыбора",,,,,, Новый ОписаниеОповещения("ПлатежныеВедомостиПередНачаломДобавленияЗавершение", ЭтотОбъект), );
	
КонецПроцедуры

&НаКлиенте
Процедура ПлатежныеВедомостиПередНачаломДобавленияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ПараметрыФормы.Вставить("Организация",Объект.Организация);
		ПараметрыФормы.Вставить("Подразделение",Объект.Подразделение);
		ПараметрыФормы.Вставить("Проект",Объект.ПроектДляЗачетаАвансаСПомощником);
		ПараметрыФормы.Вставить("Сотрудник",Результат);
		ПараметрыФормы.Вставить("МассивВедомостей",ПолучитьМассивВедомостейПоСотруднику(ПараметрыФормы));
		оп = Новый ОписаниеОповещения("ОбработкаПодбораЗарплатаВРазрезеСотрудниковПоВедомости", ЭтотОбъект);
		ОткрытьФорму("ОбщаяФорма.ПодборПоВедомости",ПараметрыФормы,,,,,оп,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервереБезКонтекста
Функция ПолучитьМассивВедомостейПоСотруднику(СтруктураПараметров)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РасчетыСПерсоналомОстатки.Организация КАК Организация,
	|	РасчетыСПерсоналомОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	...
	|					ИЛИ &ПоВсемСтруктурнымИдиницам = ИСТИНА)
	|				И (Проект = &Проект
	|					ИЛИ &ПоВсемПроектам = ИСТИНА)
	|				И Сотрудник = &Сотрудник) КАК РасчетыСПерсоналомОстатки
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПлатежнаяВедомостьСотрудники.Ссылка КАК Ссылка,
	|	ПлатежнаяВедомостьСотрудники.Ссылка.Дата КАК Дата
	|ИЗ
	|	ВТ_Остатки КАК ВТ_Остатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПлатежнаяВедомость.Сотрудники КАК ПлатежнаяВедомостьСотрудники
	|		ПО ВТ_Остатки.Организация = ПлатежнаяВедомостьСотрудники.Ссылка.Организация
	|			И (ВЫБОР
	|				КОГДА &ПоВсемСтруктурнымИдиницам = ИСТИНА
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ВТ_Остатки.СтруктурнаяЕдиница = ПлатежнаяВедомостьСотрудники.Ссылка.СтруктурнаяЕдиница
	|			КОНЕЦ)
	|			И ВТ_Остатки.Сотрудник = ПлатежнаяВедомостьСотрудники.Сотрудник
	|			И (ВЫБОР
	|				КОГДА &ПоВсемПроектам = ИСТИНА
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ВТ_Остатки.Проект = ПлатежнаяВедомостьСотрудники.Проект
	|			КОНЕЦ)
	|ГДЕ
	|	ПлатежнаяВедомостьСотрудники.Ссылка.Организация = &Организация
	|	И (&ПоВсемСтруктурнымИдиницам = ИСТИНА
	|			ИЛИ ПлатежнаяВедомостьСотрудники.Ссылка.СтруктурнаяЕдиница = &СтруктурнаяЕдиница)
	|	И ПлатежнаяВедомостьСотрудники.Сотрудник = &Сотрудник
	|//	И ПлатежнаяВедомостьСотрудники.Ссылка.Проведен
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ
	|";
	...
	Запрос.УстановитьПараметр("СтруктурнаяЕдиница", СтруктураПараметров.Подразделение);
	...
	Запрос.УстановитьПараметр("ПоВсемСтруктурнымИдиницам", НЕ ЗначениеЗаполнено(СтруктураПараметров.Подразделение));
	Запрос.УстановитьПараметр("ПоВсемПроектам", НЕ ПолучитьФункциональнуюОпцию("УчетПоПроектам") ИЛИ Не ЗначениеЗаполнено(СтруктураПараметров.Проект));
	...
	МассивВедомостей = Новый Массив;
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		МассивВедомостей.Добавить(ВыборкаДетальныеЗаписи.Ссылка);
	КонецЦикла;
	Возврат МассивВедомостей;

КонецФункции // ПолучитьМассивВедомостейПоСотруднику()

&НаСервере
Процедура ДобавитьПлатёжныеВедомости(МассивВедомостей)

	Для каждого ТекВедомость Из МассивВедомостей Цикл
	
		Если Объект.ПлатежныеВедомости.НайтиСтроки(Новый Структура("Ведомость", ТекВедомость)).Количество() = 0 Тогда
		
			НоваяСтрока = Объект.ПлатежныеВедомости.Добавить();
			НоваяСтрока.Ведомость = ТекВедомость.Ведомость;
		
		КонецЕсли; 	
	
	КонецЦикла; 	

КонецПроцедуры // ДобавитьПлатёжныеВедомости()
...
//+++ Дмгг
&НаСервере
Процедура ЗаполнитьРасшифровкаПлатежаДоговорНаСервере(УИД)
	
	ТекСтрока = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(УИД);
	ТекСтрока.СтатьяДДС = ТекСтрока.Договор.СтатьяДДСПоУмолчанию;
	
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаДолговСтатьяПриИзменении(Элемент)
	
	ТекущаяСтрокаДолгов = Элементы.ТаблицаДолгов.ТекущиеДанные;
	
	ТекущаяСтрокаДолгов.Выбран = Истина;
	ПриИзмененииВТаблицеДолгов("СтатьяДДС", ТекущаяСтрокаДолгов.ПолучитьИдентификатор(), Ложь);
