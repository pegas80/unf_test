...
Процедура ЗаполнитьРасшифровкуПлатежа(ДокументОприходования = Неопределено) Экспорт
...
//--------------------------------------------------------------------------
	...
	СтавкаНДСПоУмолчанию = РС_ОбщегоНазначения.ПолучитьСтавкуНДСКонтрагента(Контрагент, Организация);     
	//--- Дмгг, 12.02.24, ONESOFT-2924
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [21 августа 2023]  НАЧАЛО
	Запрос.УстановитьПараметр("ДокументОприходования", ДокументОприходования);	
	Запрос.УстановитьПараметр("ПоДокументуОприходования", ДокументОприходования <> Неопределено);	
	// © Шишка В.В. [21 августа 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	//Малышев 2025-07-18 ONESOFT-4227 ((--
	|   ВЫБОР 
	|       КОГДА ТИПЗНАЧЕНИЯ(РасчетыСПоставщикамиОстатки.Документ) = ТИП(Документ.ПриходнаяНакладная) и  ТИПЗНАЧЕНИЯ(РасчетыСПоставщикамиОстатки.Документ.ДокументОснование) = ТИП(Документ.СчетНаОплатуПоставщика)
	|            ТОГДА РасчетыСПоставщикамиОстатки.Документ.ДокументОснование
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [21 августа 2023]  НАЧАЛО
	|					И &ПоДокументуОприходования = Ложь ИЛИ Документ = &ДокументОприходования 	
	// © Шишка В.В. [21 августа 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	//+++ Дмгг
	|	ГДЕ
	|		РасчетыСПоставщикамиОстатки.Документ.Дата < &Период          
	//--- Дмгг
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|   И РасчетыСПоставщикамиОстатки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг) //Малышев 2024-05-24 
	|СГРУППИРОВАТЬ ПО
	...
	Пока СуммаОсталосьРаспределить > 0 Цикл
		...
		Если ВыборкаРезультатаЗапроса.Следующий() Тогда
			...
			//+++ Дмгг, 12.02.24, ONESOFT-2924
			Ставки = РС_ОбщегоНазначения.ПолучитьСтавкиНалогов(Дата, Контрагент, НоваяСтрока.Договор, Организация);
			НоваяСтрока.ВидУдержанияНДФЛ = Ставки.ВидУдержанияНДФЛ;
			НоваяСтрока.ВидУдержанияПФ = Ставки.ВидУдержанияПФ; 
			//--- Дмгг, 12.02.24, ONESOFT-2924
...
Процедура ЗаполнитьРасшифровкуПлатежаАвтоЗачетДолгов() Экспорт
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда 
		...
		ИначеЕсли Контрагент.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
			СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
		ИначеЕсли Контрагент.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
			СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСНоль();
		КонецЕсли;
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Пока СуммаОсталосьРаспределить > 0 Цикл
			...
			Иначе
				...
				//НоваяСтрока.СуммаНДС = НоваяСтрока.СуммаПлатежа - (НоваяСтрока.СуммаПлатежа) / ((СтавкаНДСПоУмолчанию.Ставка + 100) / 100);
				//+++ Дмгг
				Если ДоговорДляЗачетаАвансаСПомощником.УчитыватьНДФЛ Тогда 
					НоваяСтрока.ВидУдержанияНДФЛ = ДоговорДляЗачетаАвансаСПомощником.ВидУдержанияНДФЛ;
				КонецЕсли;
				
				тзСрезПФ = РегистрыСведений.УдержанияДляПенсионногоФонда.СрезПоследних(Дата, Новый Структура("Контрагент, Договор", Контрагент, ДоговорДляЗачетаАвансаСПомощником));
...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.СтатусУтвержденияПлатежа = Перечисления.СтатусыУтвержденияПлатежей.НеУтвержден Тогда
		ВызватьИсключение НСтр("ka='დაუმტკიცებელი განაცხადის საფუძველზე ფულის გახარჯვის შეყვანა არ შეიძლება!';ru='Нельзя ввести расход денег на основании неутвержденной заявки!';en='It is impossible to enter the flow of money on the basis of an unapproved application!'");
	КонецЕсли;
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		ВызватьИсключение НСтр("ka='ფულის გასავალის შეყვანა არ შეიძლება. განაცხადში მითითებულია გადახდის არასწორი მეთოდი (ფულადი სახსრების ტიპი)!';ru='Нельзя ввести расход денег. В заявке указан не верный способ оплаты (тип денежных средств)!';en='It is impossible to enter the flow of money. The application specified is not a sure method of payment (cash type)!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	//Малышев 2025-07-18 ONESOFT-4227 (--
	|	ВЫБОР 
	|		КОГДА ТИПЗНАЧЕНИЯ(ВложенныйЗапрос.ДокументОснование) = ТИП(Документ.СчетНаОплатуПоставщика)
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.БанковскийСчет) Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка, , "БанковскийСчет");
	Иначе
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
	КонецЕсли;
	// Олег Литвиненко <---
...
Процедура ЗаполнитьПоДополнительныеРасходы(ДокументСсылкаДополнительныеРасходы) Экспорт
	...
	|	КОНЕЦ КАК Документ,
	//Паганини ПФ_
	//|	СУММА(ЕСТЬNULL(ВЫРАЗИТЬ((ТаблицаДокумента.Всего - ТаблицаДокумента.СуммаНДФЛ) * ВЫБОР
	|	СУММА(ЕСТЬNULL(ВЫРАЗИТЬ((ТаблицаДокумента.Всего - ТаблицаДокумента.СуммаНДФЛ - ТаблицаДокумента.СуммаПФ) * ВЫБОР
	//_Паганини ПФ
	...
	|	КОНЕЦ КАК Кратность,
	//Паганини ПФ_
	//|	СУММА(ЕСТЬNULL(ТаблицаДокумента.Всего - ТаблицаДокумента.СуммаНДФЛ, 0)) КАК СуммаПлатежа,
	|	СУММА(ЕСТЬNULL(ТаблицаДокумента.Всего - ТаблицаДокумента.СуммаНДФЛ - ТаблицаДокумента.СуммаПФ, 0)) КАК СуммаПлатежа,
	//_Паганини ПФ
	|	ТаблицаДокумента.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ЕСТЬNULL(ТаблицаДокумента.СуммаНДС, 0)) КАК СуммаНДС,
	//Паганини ПФ_
	//|	ЕСТЬNULL(ТаблицаДокумента.ТипНДФЛ, ЗНАЧЕНИЕ(Справочник.ТипыНДФЛ.ПустаяСсылка)) КАК ТипНДФЛ
	|	ТаблицаДокумента.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
	|	СУММА(ЕСТЬNULL(ТаблицаДокумента.СуммаНДФЛ, 0)) КАК СуммаНДФЛ,
	|	ТаблицаДокумента.ВидУдержанияПФ КАК ВидУдержанияПФ,
	|	СУММА(ЕСТЬNULL(ТаблицаДокумента.СуммаПФ, 0)) КАК СуммаПФ
	//_Паганини ПФ
	|ИЗ
	...
	|	КОНЕЦ,
	//Паганини ПФ_
	//|	ЕСТЬNULL(ТаблицаДокумента.ТипНДФЛ, ЗНАЧЕНИЕ(Справочник.ТипыНДФЛ.ПустаяСсылка))
	|	ТаблицаДокумента.ВидУдержанияНДФЛ,
	|	ТаблицаДокумента.ВидУдержанияПФ
	//_Паганини ПФ
	|";
...
Процедура ЗаполнитьПоПлатежнаяВедомость(ДокументСсылкаПлатежнаяВедомость) Экспорт
	...
	ЗапросОпций = Новый Запрос;
	ЗапросОпций.Текст = 
		"ВЫБРАТЬ
		|	ФункциональнаяОпцияУчетПоПроектам.Значение КАК УчетПоПроектам
		|ИЗ
		|	Константа.ФункциональнаяОпцияУчетПоПроектам КАК ФункциональнаяОпцияУчетПоПроектам";
	РезультатЗапроса = ЗапросОпций.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Выборка.Следующий();
	УчетПоПроектам = Выборка.УчетПоПроектам; 
	...
	Если УчетПоПроектам Тогда
		...
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.Ссылка.ВалютаДокумента = ТаблицаДокумента.Ссылка.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств
		|			ТОГДА ТаблицаДокумента.Ссылка.Организация.БанковскийСчетПоУмолчанию
		|		ИНАЧЕ ВложенныйЗапрос.БанковскийСчет
		|	КОНЕЦ КАК БанковскийСчет,
		|	ТаблицаДокумента.Проект КАК Проект,
		|	СУММА(ТаблицаДокумента.СуммаПлатежа) КАК СуммаПлатежа
		|ИЗ
		|	Документ.ПлатежнаяВедомость.Сотрудники КАК ТаблицаДокумента
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 1
		|			БанковскиеСчета.Ссылка КАК БанковскийСчет,
		|			БанковскиеСчета.Владелец КАК Владелец,
		|			БанковскиеСчета.ВалютаДенежныхСредств КАК ВалютаДенежныхСредств
		...
		|			ТаблицаДокумента.Ссылка = &Ссылка
		|			И БанковскиеСчета.ПометкаУдаления = ЛОЖЬ) КАК ВложенныйЗапрос
		|		ПО ТаблицаДокумента.Ссылка.ВалютаДокумента = ВложенныйЗапрос.ВалютаДенежныхСредств
		|			И ТаблицаДокумента.Ссылка.Организация = ВложенныйЗапрос.Владелец
		|ГДЕ
		|	ТаблицаДокумента.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		...
		ВыборкаОбъект = РезультатЗапроса[1].Выбрать();
		ВыборкаОбъект.Следующий();
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаОбъект);
	...
	//ПодольскийАИ #УчетПроектов 25.11.2020 }
		...
		|	ТаблицаДокумента.ВалютаДокумента<>ТаблицаДокумента.ВалютаРасчетов как НужнаКонвертация	
		|ИЗ
		|	Документ.ПлатежнаяВедомость КАК ТаблицаДокумента
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 1
		|			БанковскиеСчета.Ссылка КАК БанковскийСчет,
		...
		//Малышев 2024-11-21 ONESOFT-3523 (--
		НациональнаяВалюта = Константы.НациональнаяВалюта.Получить();
		Если Выборка.НужнаКонвертация и НациональнаяВалюта = Выборка.ВалютаРасчетов Тогда 
			СтруктураПоВалюте = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Дата, Новый Структура("Валюта", ВалютаДенежныхСредств));
			СуммаВНациональнойВалюте = Выборка.Ведомость.Сотрудники.Итог("СуммаРасчетов");
			СуммаДокумента = СуммаВНациональнойВалюте / ?(СтруктураПоВалюте.Курс=0,1,СтруктураПоВалюте.Курс); 
			СуммаРасчетов = СуммаВНациональнойВалюте;
			НоваяСтрока.СуммаПлатежа = СуммаДокумента; 
			
			РасшифровкаПлатежа.Очистить();
			СтрокаРасшифровки = РасшифровкаПлатежа.Добавить();
			СтрокаРасшифровки.СуммаПлатежа = СуммаДокумента;
			СтрокаРасшифровки.СуммаРасчетов = СуммаВНациональнойВалюте;
			СтрокаРасшифровки.Курс = СтруктураПоВалюте.Курс; 
			СтрокаРасшифровки.Кратность = СтруктураПоВалюте.Кратность; 
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	Если ДокументСсылкаНачислениеНалогов.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
		ВызватьИсключение НСтр("ka='ანგარიშიდან გასავლის შეყვანა შეიძლება მხოლოდ გადასახადების დარიცხვის, და არა ანაზღაურების საფუძველზე.';ru='Расход со счета можно ввести только на основании начисления налогов, а не возмещения.';en='Flow from the account can only be entered on the basis of tax assessment, rather than compensation.'");
	КонецЕсли;
...
//	
Процедура ЗаполнитьПоКорректировкеПоступления(ДанныеЗаполнения, НомерСтроки = Неопределено, Сумма = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =                                                                                                            
		"ВЫБРАТЬ
		|	КорректировкаПоступленияРасходы.Ссылка КАК Ссылка,
		|	КорректировкаПоступленияРасходы.Ссылка.Контрагент КАК Контрагент,
		|	КорректировкаПоступленияРасходы.Ссылка.Договор КАК Договор,
		|	СУММА(КорректировкаПоступленияРасходы.Сумма - КорректировкаПоступленияРасходы.СуммаДоИзменения) КАК Сумма,
		|	КорректировкаПоступленияРасходы.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
		|	КорректировкаПоступленияРасходы.Ссылка.Контрагент.БанковскийСчетПоУмолчанию КАК КонтрагентБанковскийСчет,
		|	КорректировкаПоступленияРасходы.Ссылка КАК ДокументОснование,
		|	КорректировкаПоступленияРасходы.Ссылка КАК Документ,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(КорректировкаПоступленияРасходы.Ссылка.Заказ) = ТИП(Документ.ЗаказПоставщику)
		|			ТОГДА КорректировкаПоступленияРасходы.Ссылка.Заказ 
		|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка) 
		|	КОНЕЦ КАК Заказ,
		|	КорректировкаПоступленияРасходы.Ссылка.Курс КАК Курс,
		|	КорректировкаПоступленияРасходы.Ссылка.Кратность КАК Кратность,
		|	КорректировкаПоступленияРасходы.Ссылка.НалогообложениеНДС КАК НалогообложениеНДС,
		|	КорректировкаПоступленияРасходы.Ссылка.Организация КАК Организация,
		|	КорректировкаПоступленияРасходы.СтавкаНДС КАК СтавкаНДС,
		|	СУММА(КорректировкаПоступленияРасходы.Сумма - КорректировкаПоступленияРасходы.СуммаДоИзменения) КАК СуммаПлатежа,
		|	СУММА(КорректировкаПоступленияРасходы.Сумма - КорректировкаПоступленияРасходы.СуммаДоИзменения) КАК СуммаРасчетов,
		|	СУММА(КорректировкаПоступленияРасходы.СуммаНДС - КорректировкаПоступленияРасходы.СуммаНДСДоИзменения) КАК СуммаНДС
		|ИЗ
		...
		|	КорректировкаПоступленияРасходы.Ссылка.Контрагент,
		|	КорректировкаПоступленияРасходы.Ссылка.Договор,
		|	КорректировкаПоступленияРасходы.СтавкаНДС,
		|	КорректировкаПоступленияРасходы.Ссылка.ВалютаДокумента,
		|	КорректировкаПоступленияРасходы.Ссылка.Контрагент.БанковскийСчетПоУмолчанию,
		|	КорректировкаПоступленияРасходы.Ссылка.ДокументОснование,
		|	КорректировкаПоступленияРасходы.Ссылка.Заказ,
		|	КорректировкаПоступленияРасходы.Ссылка.Курс,
		|	КорректировкаПоступленияРасходы.Ссылка.Кратность,
		|	КорректировкаПоступленияРасходы.Ссылка.НалогообложениеНДС,
		|	КорректировкаПоступленияРасходы.Ссылка.Организация,
		|	КорректировкаПоступленияРасходы.Ссылка.ДокументОснование";
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
		ЭтотОбъект.ВидОперации = перечисления.ВидыОперацийРасходСоСчета.Поставщику;
		Если НЕ ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ВалютаДенежныхСредств = Выборка.ВалютаДокумента;
		КонецЕсли;
		НоваяСтрока = РасшифровкаПлатежа.Добавить();
...
КонецПроцедуры // ЗаполнитьПоКорректировкеПоступления()

Процедура ЗаполнитьПоЗакупкаЛицензий(ДокументСсылкаЗакупкаЛицензий) Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаЗакупкаЛицензий);
	Запрос.УстановитьПараметр("Дата", ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
	...
	|	&Ссылка КАК ДокументОснование,
	|	ЗакупкаЛицензий.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ЗакупкаЛицензий.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств = ЗакупкаЛицензий.Договор.ВалютаРасчетов
	|			ТОГДА ЗакупкаЛицензий.Организация.БанковскийСчетПоУмолчанию
	|		ИНАЧЕ ЗапросБанковскиеСчета.БанковскийСчет
	|	КОНЕЦ КАК БанковскийСчет,
	|	ЗакупкаЛицензий.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ЗакупкаЛицензий.Контрагент КАК Контрагент,
	|	ЗакупкаЛицензий.ВалютаДокумента КАК ВалютаДенежныхСредств,
	|	ЗакупкаЛицензий.Договор КАК Договор,
	|	ЛОЖЬ КАК ПризнакАванса,
	|	СУММА(ЕСТЬNULL(ВложенныйЗапрос.СуммаРасчетов, 0)) КАК СуммаРасчетов,
	|	ВложенныйЗапрос.Курс КАК Курс,
	|	ВложенныйЗапрос.Кратность КАК Кратность,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			&Ссылка КАК Документ,
	|			ВЫРАЗИТЬ(ЗакупкаЛицензийЛицензии.Всего * ВЫБОР
	|					КОГДА ЗакупкаЛицензийЛицензии.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|						ТОГДА КурсыРегВалют.Курс * ЗакупкаЛицензийЛицензии.Ссылка.Кратность / (ЗакупкаЛицензийЛицензии.Ссылка.Курс * ЕСТЬNULL(КурсыРегВалют.Кратность, 1))
	|					ИНАЧЕ 1
	|				КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасчетов,
	|			ВЫБОР
	|				КОГДА ЗакупкаЛицензийЛицензии.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|					ТОГДА ЗакупкаЛицензийЛицензии.Ссылка.Курс
	|				ИНАЧЕ КурсыВалютыРасчетов.Курс
	|			КОНЕЦ КАК Курс,
	|			ВЫБОР
	|				КОГДА ЗакупкаЛицензийЛицензии.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|					ТОГДА ЗакупкаЛицензийЛицензии.Ссылка.Кратность
	|				ИНАЧЕ КурсыВалютыРасчетов.Кратность
	|			КОНЕЦ КАК Кратность,
	|			ЗакупкаЛицензийЛицензии.Всего КАК СуммаПлатежа,
	|			ЗакупкаЛицензийЛицензии.СуммаНДС КАК СуммаНДС,
	...
	|				ПО (ИСТИНА)
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютыРасчетов
	|				ПО ЗакупкаЛицензийЛицензии.Ссылка.Договор.ВалютаРасчетов = КурсыВалютыРасчетов.Валюта
	|		ГДЕ
	|			ЗакупкаЛицензийЛицензии.Ссылка = &Ссылка) КАК ВложенныйЗапрос
	|		ПО ЗакупкаЛицензий.Ссылка = ВложенныйЗапрос.Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 1
	|			БанковскиеСчета.Ссылка КАК БанковскийСчет,
	|			БанковскиеСчета.Владелец КАК Владелец,
	|			БанковскиеСчета.ВалютаДенежныхСредств КАК ВалютаДенежныхСредств
	...
	|			И БанковскиеСчета.ПометкаУдаления = ЛОЖЬ) КАК ЗапросБанковскиеСчета
	|		ПО ЗакупкаЛицензий.Организация = ЗапросБанковскиеСчета.Владелец
	|			И ЗакупкаЛицензий.ВалютаДокумента = ЗапросБанковскиеСчета.ВалютаДенежныхСредств
	|ГДЕ
	|	ЗакупкаЛицензий.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Курс,
	|	ВложенныйЗапрос.Кратность,
	|	ЗакупкаЛицензий.Организация,
	|	ВЫБОР
	|		КОГДА ЗакупкаЛицензий.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств = ЗакупкаЛицензий.Договор.ВалютаРасчетов
	|			ТОГДА ЗакупкаЛицензий.Организация.БанковскийСчетПоУмолчанию
	|		ИНАЧЕ ЗапросБанковскиеСчета.БанковскийСчет
	|	КОНЕЦ,
	|	ЗакупкаЛицензий.НалогообложениеНДС,
	|	ЗакупкаЛицензий.Контрагент,
	|	ЗакупкаЛицензий.ВалютаДокумента,
	|	ЗакупкаЛицензий.Договор,
	|	ВложенныйЗапрос.Документ";
...
Процедура ЗаполнитьПоСборкаЗапасов(ДокументСсылкаСборкаЗапасов) Экспорт
	...
	Запрос.УстановитьПараметр("ВалютаРегл", Константы.НациональнаяВалюта.Получить());
	Запрос.УстановитьПараметр("Дата", ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МАКСИМУМ(СборкаЗапасовДополнительныеРасходы.Контрагент) КАК Контрагент,
	|	МАКСИМУМ(СборкаЗапасовДополнительныеРасходы.Договор) КАК Договор,
	|	МАКСИМУМ(СборкаЗапасовДополнительныеРасходы.СтавкаНДС) КАК СтавкаНДС,
	|	МАКСИМУМ(СборкаЗапасовДополнительныеРасходы.ВидУдержанияНДФЛ) КАК ВидУдержанияНДФЛ,
	|	МАКСИМУМ(СборкаЗапасовДополнительныеРасходы.ВидУдержанияПФ) КАК ВидУдержанияПФ,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СборкаЗапасовДополнительныеРасходы.Контрагент) КАК КонтрагентКоличество,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СборкаЗапасовДополнительныеРасходы.Договор) КАК ДоговорКоличество,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СборкаЗапасовДополнительныеРасходы.СтавкаНДС) КАК СтавкаНДСКоличество,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СборкаЗапасовДополнительныеРасходы.ВидУдержанияНДФЛ) КАК ВидУдержанияНДФЛКоличество,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СборкаЗапасовДополнительныеРасходы.ВидУдержанияПФ) КАК ВидУдержанияПФКоличество
	|ПОМЕСТИТЬ ВТ_Шапка
	|ИЗ
	...
	|	ВТ_Шапка.СтавкаНДС КАК СтавкаНДС,
	|	ВТ_Шапка.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
	|	ВТ_Шапка.ВидУдержанияПФ КАК ВидУдержанияПФ,
	|	ВТ_Шапка.КонтрагентКоличество КАК КонтрагентКоличество,
	|	ВТ_Шапка.ДоговорКоличество КАК ДоговорКоличество,
	|	ВТ_Шапка.СтавкаНДСКоличество КАК СтавкаНДСКоличество,
	|	ВТ_Шапка.ВидУдержанияНДФЛКоличество КАК ВидУдержанияНДФЛКоличество,
	|	ВТ_Шапка.ВидУдержанияПФКоличество КАК ВидУдержанияПФКоличество
	|ИЗ
	...
	|	ШапкаДокумента.Организация.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ВТ_Шапка.Контрагент КАК Контрагент,
	|	&ВалютаРегл КАК ВалютаДенежныхСредств,
	...
	|	СУММА(ЕСТЬNULL(ВложенныйЗапрос.СуммаПФ, 0)) КАК СуммаПФ
	|ИЗ
	|	Документ.СборкаЗапасов КАК ШапкаДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			НЕОПРЕДЕЛЕНО КАК ЗаказПоставщику,
	|			ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказПокупателя,
	|			МАКСИМУМ(ТаблицаДокумента.Ссылка.ДокументОснование) КАК ДокументОснование,
	|			&Ссылка КАК Документ,
	|			СУММА(ВЫРАЗИТЬ(ТаблицаДокумента.Всего КАК ЧИСЛО(15, 2))) КАК СуммаРасчетов,
	|			1 КАК Курс,
	|			1 КАК Кратность,
	|			СУММА(ТаблицаДокумента.Всего) КАК СуммаПлатежа,
	|			МАКСИМУМ(ТаблицаДокумента.СтавкаНДС) КАК СтавкаНДС,
	|			СУММА(ТаблицаДокумента.СуммаНДС) КАК СуммаНДС,
	|			МАКСИМУМ(ТаблицаДокумента.ВидУдержанияНДФЛ) КАК ВидУдержанияНДФЛ,
	|			СУММА(ТаблицаДокумента.СуммаНДФЛ) КАК СуммаНДФЛ,
	|			МАКСИМУМ(ТаблицаДокумента.ВидУдержанияПФ) КАК ВидУдержанияПФ,
	|			СУММА(ТаблицаДокумента.СуммаПФ) КАК СуммаПФ
	|		ИЗ
	|			Документ.СборкаЗапасов.ДополнительныеРасходы КАК ТаблицаДокумента
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|						&Дата,
	|						Валюта В
	|							(ВЫБРАТЬ
	...
	|				ПО (ИСТИНА)
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютыРасчетов
	|				ПО (&ВалютаРегл = КурсыВалютыРасчетов.Валюта),
	|			Константа.НациональнаяВалюта КАК КонстантаНациональнаяВалюта
	|		ГДЕ
	|			ТаблицаДокумента.Ссылка = &Ссылка) КАК ВложенныйЗапрос
	|		ПО ШапкаДокумента.Ссылка = ВложенныйЗапрос.Документ
	...
	|			И БанковскиеСчета.ПометкаУдаления = ЛОЖЬ) КАК ЗапросБанковскиеСчета
	|		ПО ШапкаДокумента.Организация = ЗапросБанковскиеСчета.Владелец
	|			И (ЗапросБанковскиеСчета.ВалютаДенежныхСредств = &ВалютаРегл)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Шапка КАК ВТ_Шапка
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ШапкаДокумента.Ссылка = &Ссылка
	|
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ОплатаПоставщикам)
	|	КОНЕЦ,
	|	ШапкаДокумента.Организация.БанковскийСчетПоУмолчанию,
	|	ВложенныйЗапрос.ВидУдержанияНДФЛ,
	|	ВложенныйЗапрос.ВидУдержанияПФ,
	|	ШапкаДокумента.Организация.НалогообложениеНДС,
	|	ВТ_Шапка.Контрагент,
	|	ВТ_Шапка.Договор";
	
	МассивРезультат = Запрос.ВыполнитьПакет();
	ВыборкаПроверка = МассивРезультат[1].Выбрать();
	ТекстСообщения = НСтр("ka='В табличной части документа - основания указаны разные значения реквизитов: _Реквизиты_';ru='В табличной части документа - основания указаны разные значения реквизитов: _Реквизиты_';en='В табличной части документа - основания указаны разные значения реквизитов: _Реквизиты_'");
	Если ВыборкаПроверка.Следующий() Тогда
	
		Если ВыборкаПроверка.КонтрагентКоличество > 1
			ИЛИ ВыборкаПроверка.ДоговорКоличество > 1
			ИЛИ ВыборкаПроверка.СтавкаНДСКоличество > 1
			ИЛИ ВыборкаПроверка.ВидУдержанияНДФЛКоличество > 1
			ИЛИ ВыборкаПроверка.ВидУдержанияПФКоличество > 1
			Тогда
			
			МассивРеквизитов = Новый Массив;
			Если ВыборкаПроверка.КонтрагентКоличество > 1 Тогда
			
				МассивРеквизитов.Добавить(НСтр("ka='Контрагент';ru='Контрагент';en='Контрагент'"));	
			
			КонецЕсли; 
			Если ВыборкаПроверка.ДоговорКоличество > 1 Тогда
			
				МассивРеквизитов.Добавить(НСтр("ka='Договор';ru='Договор';en='Договор'"));	
			
			КонецЕсли; 
			Если ВыборкаПроверка.СтавкаНДСКоличество > 1 Тогда
			
				МассивРеквизитов.Добавить(НСтр("ka='Ставка НДС';ru='Ставка НДС';en='Ставка НДС'"));	
			
			КонецЕсли; 
			Если ВыборкаПроверка.ВидУдержанияНДФЛКоличество > 1 Тогда
			
				МассивРеквизитов.Добавить(НСтр("ka='Вид удержания НДФЛ';ru='Вид удержания НДФЛ';en='Вид удержания НДФЛ'"));	
			
			КонецЕсли; 
			Если ВыборкаПроверка.ВидУдержанияПФКоличество > 1 Тогда
			
				МассивРеквизитов.Добавить(НСтр("ka='Вид удержания ПФ';ru='Вид удержания ПФ';en='Вид удержания ПФ'"));	
			
			КонецЕсли;
			
			СписокРеквизитов = СтрСоединить(МассивРеквизитов, ",");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "_Реквизиты_", СписокРеквизитов);
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='В табличной части ""Дополнительные расходы"" документа - основания отсутствуют данные!';ru='В табличной части ""Дополнительные расходы"" документа - основания отсутствуют данные!';en='В табличной части ""Дополнительные расходы"" документа - основания отсутствуют данные!'");
		ВызватьИсключение ТекстСообщения;
	
	КонецЕсли; 
	РезультатЗапроса = МассивРезультат[МассивРезультат.ВГраница()];
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	СтратегияЗаполнения[Тип("ДокументСсылка.КорректировкаПоступления")] = "ЗаполнитьПоКорректировкеПоступления"; //+++ Дмгг
	СтратегияЗаполнения[Тип("ДокументСсылка.ЗакупкаЛицензий")] = "ЗаполнитьПоЗакупкаЛицензий"; //++Lazare
	//--------------------------------------------------------------------------
	// © Шишка В.В. [26 декабря 2023]  НАЧАЛО
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// операции.
	ИспользоватьТолькоРегламентированногоПланаСчетов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ИспользованиеТолькоРегламентированногоПланаСчетов(?(ЗначениеЗаполнено(Дата),Дата,ТекущаяДата()));
	 ...
	 ИЛИ ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю Тогда
					   ...
					   И НЕ СтрокаРасшифровкаПлатежа.ПризнакАванса)) Тогда
						Если РасшифровкаПлатежа.Количество() = 1 Тогда
							Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику Тогда
								ТекстСообщения = НСтр("ka='მიუთითეთ გადმოტვირთვის დოკუმენტი ან გადახდის ავანსის ნიშანი.';ru='Укажите документ отгрузки или признак аванса платежа.';en='Specify the shipping document or a sign of the advance payment.'");
							Иначе
								ТекстСообщения = НСтр("ka='მიუთითეთ ანგარიშსწორებების დოკუმენტი.';ru='Укажите документ расчетов.';en='Specify paper calculations.'");
							КонецЕсли;
						Иначе
							Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику Тогда
								ТекстСообщения = НСтр("ka='მიუთითეთ გადმოტვირთვის დოკუმენტი ან გადახდის ნიშანი სია ""გადახდის გაშიფრვის"" სტრიქონ %НомерСтроки%-ში.';ru='Укажите документ отгрузки или признак оплаты в строке %НомерСтроки% списка ""Расшифровка платежа"".';en='Specify the shipping document or sign the payment on line% NomerStroki% list ""Transcription of payment.""'");
							Иначе
								ТекстСообщения = НСтр("ka='მიუთითეთ ანგარიშსწორებების დოკუმენტი სია ""გადახდის გაშიფრვის"" სტრიქონ %НомерСтроки%-ში.';ru='Укажите документ расчетов в строке %НомерСтроки% списка ""Расшифровка платежа"".';en='Specify the settlement document at line% NomerStroki% list ""Transcription of payment.""'");
							КонецЕсли;
		...
		Иначе
			...
			Для каждого СтрокаДоговоры Из ДоговорыАвтоЗачетаДолгов Цикл
				...
				Если (СтрокаДоговоры.Договор.Пустая() И СтрокаДоговоры.СуммаПлатежа > 0) ИЛИ
					...
					ТекстСообщения = НСтр("ka='№%НомерСтроки% ხელშეკრულებისთვის უნდა გასუფთავდეს თანხა ან არჩეული იქნას ხელშეკრულება.';ru='Для договора под №%НомерСтроки% нужно очистить сумму или выбрать договор.';en='For the contract under the number NomerStroki%% you need to clear or select the amount of the contract.'");
		...
		Если СуммаПлатежа <> СуммаДокумента Тогда
			ТекстСообщения = ?(ПогашатьАвтоматическиСРазнесениемПоДоговорам,
			    НСтр("ka='დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств%, ხელშეკრულებების მიხედვით გატარებული გადახდების თანხას არ შეესაბამება: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей по договорам: %СуммаПлатежа% %ВалютаДенежныхСредств%!';en='The amount of the document: SummaDokumenta%%%% ValyutaDenezhnyhSredstv does not correspond to the amount of separated payments under contracts: SummaPlatezha%%%% ValyutaDenezhnyhSredstv!'"),
				НСтр("ka='დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств%, არ შეესაბამება განკუთვნებულ გადახდებს ცხრილში: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';en='The amount of the document: SummaDokumenta%%%% ValyutaDenezhnyhSredstv does not correspond to the amount of separated charges in the tabular section: SummaPlatezha%%%% ValyutaDenezhnyhSredstv!'")
			);
		...
		ПроверяемыеРеквизиты.Добавить("НалогообложениеНДС");//Паганини
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата //Тогда
		ИЛИ ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ЗарплатаВРазрезеСотрудников Тогда // Kuzmich, 26.02.2019,Зарплата по ведомости в разрезе сотрудников
		...
		Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ЗарплатаВРазрезеСотрудников Тогда
			ПроверяемыеРеквизиты.Добавить("ВыплатаЗаработнойПлаты.Сотрудник");
		КонецЕсли;
		// Kuzmich, 26.02.2019,Зарплата по ведомости в разрезе сотрудников
		
		СуммаПлатежа = ВыплатаЗаработнойПлаты.Итог("СуммаПлатежа");
		Если СуммаПлатежа <> СуммаДокумента Тогда
			ТекстСообщения = НСтр("ka='დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств%, არ შეესაბამება განკუთვნებულ გადახდებს ცხრილში: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';en='The amount of the document: SummaDokumenta%%%% ValyutaDenezhnyhSredstv does not correspond to the amount of separated charges in the tabular section: SummaPlatezha%%%% ValyutaDenezhnyhSredstv!'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаДокумента%", Строка(СуммаДокумента));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаПлатежа%", Строка(СуммаПлатежа));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВалютаДенежныхСредств%", СокрЛП(Строка(ВалютаДенежныхСредств)));
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
		...
		Для Каждого СтрокаВедомости Из ВыплатаЗаработнойПлаты Цикл
			Если СтрокаВедомости.Ведомость.ВалютаДокумента <> ВалютаДенежныхСредств Тогда
				ТекстСообщения = НСтр("ka='დოკუმენტის ვალუტა: %ВалютаДенежныхСредств%, საგადახდო უწყისის ვალუტას არ შეესაბამება: %ВалютаВедомости%!';ru='Валюта документа: %ВалютаДенежныхСредств%, не соответствует валюте платежной ведомости: %ВалютаВедомости%!';en='paper currency:% ValyutaDenezhnyhSredstv% does not match the currency of the payroll:% ValyutaVedomosti%!'");
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты Тогда
		...
		Если НЕ ИспользоватьТолькоРегламентированногоПланаСчетов Тогда 
			...
			КонецЕсли;
		Иначе
			Если ЭквайринговыйТерминал.СчетЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.Расходы Тогда
				УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ПодразделениеЗатраты");
			КонецЕсли;
		КонецЕсли;
		//--- Дмгг
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику Тогда
		...
		Если Подотчетник.Пустая() Тогда
			ТекстСообщения = НСтр("ka='ველი ""თანამშრომელი"" არ არის შევსებული';ru='Поле ""Сотрудник"" не заполнено';en='The ""employee"" is not filled'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
				,
	...
	Иначе
		...
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ДоговорыАвтоЗачетаДолгов");
		//Паганини ПФ_
		Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПенсионномуФонду Тогда
			УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ВидНалога");
			ПроверяемыеРеквизиты.Добавить("ВидУдержания");
		КонецЕсли;
		//_Паганини ПФ		
	...
	Если ПроверитьДокументыПланирования Тогда
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			...
			Если ЕстьОшибка Тогда
				ТекстСообщения = НСтр("ka='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств% меньше суммы разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств% меньше суммы разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';en='The amount of the document:%%% SummaDokumenta ValyutaDenezhnyhSredstv% less than the separated charges in the dimension tables: SummaPlatezha%%%% ValyutaDenezhnyhSredstv!'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаДокумента%", Строка(СуммаДокумента));
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБиллинг") 
		...
		Если Выборка.Следующий() Тогда
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Выборка.Ссылка, ?(ИспользоватьТолькоРегламентированногоПланаСчетов,Корреспонденция,КорреспонденцияРегл)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მომსახურების მიმდინრე ხელშეკრულების მიხედვით დაუგეგმავი ხარჯების გატარება აკრძალულია!';ru='Запрещено проводить незапланированные расходы по текущему договору обслуживания!';en='It is forbidden to carry out unplanned maintenance costs of the current contract!'"),,
					?(ИспользоватьТолькоРегламентированногоПланаСчетов,"КорреспонденцияРегл","Корреспонденция"),,
					Отказ
				);
			КонецЕсли;
		КонецЕсли;
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьЗагрузкуИзБанка") 
		И БанковскийСчет.Банк.ИспользуетсяДляИнтеграцииСБанком Тогда  
		
		ПроверяемыеРеквизиты.Добавить("НазначениеПлатежа");
		
		Если (ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Покупателю")
			...
			ИЛИ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту"))
			И БанковскийСчет.Банк.ИспользуетсяДляИнтеграцииСБанком Тогда 
		
			ПроверяемыеРеквизиты.Добавить("СчетКонтрагента");
			//ПроверяемыеРеквизиты.Добавить("НазначениеПлатежа");
		ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Налоги")
				И БанковскийСчет.Банк.ИспользуетсяДляИнтеграцииСБанком Тогда 
				
			ПроверяемыеРеквизиты.Добавить("КодКазначейства");
	...
	КонецЕсли;
	// Kuzmich, 22.05.2020
	Если ЗначениеЗаполнено(КорреспонденцияРегл) Тогда
		Если (КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы) ИЛИ (КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.ПрочиеРасходы) Тогда
			УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подразделение");
		КонецЕсли;
	КонецЕсли;
	// Kuzmich, 22.05.2020
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимНаправлениямДеятельности.Получить()
		  И ?(ИспользоватьТолькоРегламентированногоПланаСчетов,Корреспонденция.ТипСчета,КорреспонденцияРегл.ТипСчета) = Перечисления.ТипыСчетов.Расходы Тогда
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПереводНаДругойСчет Тогда
		КорреспонденцияРегл = СчетКонтрагента.СчетУчетаРегл;//+++
		Если Не ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
		...
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.СнятиеНаличных Тогда
		КорреспонденцияРегл = Касса.СчетУчетаРегл;
		Если Не ИспользоватьТолькоРегламентированногоПланаСчетов Тогда 
			Если РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(Дата,УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(Организация)) Тогда
				Корреспонденция = ПланыСчетов.Управленческий.ПереводыВПути;
			Иначе
	...
	Если НЕ ЗначениеЗаполнено(ТипОтправки) Тогда 
		Если СуммаДокумента < 10000 Тогда 
			ТипОтправки = Перечисления.ТипыОтправки.BULK;
		Иначе
			ТипОтправки = Перечисления.ТипыОтправки.MT103;
		КонецЕсли;
	КонецЕсли;
	
	//Малышев 2024-11-22 ONESOFT-3523 - уточнение
	СуммаРасчетов = 0;
	Для каждого СтрокаДокумента из ВыплатаЗаработнойПлаты Цикл
		СуммаРасчетов = СуммаРасчетов + СтрокаДокумента.Ведомость.СуммаРасчетов;
	КонецЦикла;	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СформироватьПроводки = ПолучитьДанныеОФормированииПроводок(Ссылка);  //+++ Дмгг, 27.05.2025, ONESOFT-4091
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ТипОтправки = Перечисления.ТипыОтправки.ПустаяСсылка();
	УникальныйКлючВИнтернетБанке = "";
	ТипКомиссии = Перечисления.ТипыКомиссии.ПустаяСсылка();
	ПромежуточныйБанк = Справочники.Банки.ПустаяСсылка();
	Выгружено = Ложь;
	СтатусВИнтернетБанке = Перечисления.СтатусыДокументовВИнтернетБанке.ПустаяСсылка();
...
Процедура ЗаполнитьПоДоговоруКредитаЗайма(ДокументСсылкаДоговорКредитаИЗайма, Сумма = Неопределено) Экспорт
	...
	ДанныеДокументаОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ДокументСсылкаДоговорКредитаИЗайма,
		"ВидДоговора, ДокументРеализации");
	
	Если ДанныеДокументаОснования.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный
		И ЗначениеЗаполнено(ДанныеДокументаОснования.ДокументРеализации) Тогда
		ТекстСообщения
			= НСтр("ru='Нельзя ввести выдачу денег по кредиту, оформленному по накладной!';
					|ka ='თქვენ არ შეგიძლიათ ფულის გაცემა კრედიტით, თუ სესხი გაცემულია გასავლის ზედდებულით!';
					|en='It is impossible to enter the flow of money on the basis of loan based on invoice!'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Если Сумма <> Неопределено Тогда
		...
		|		КОГДА ТаблицаДокумента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.КредитВыданный)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ВыдачаКредитаКонтрагенту)
	...
	РасшифровкаПлатежа.Очистить();
	Если Выборка.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику
		Или Выборка.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный Тогда
		НоваяСтрока = РасшифровкаПлатежа.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
	Иначе
		СуммаДокумента = 0;
...
Процедура ЗаполнитьПоАвансовыйОтчет(ДанныеЗаполнения) Экспорт
Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Подотчетнику) КАК ВидОперации,
	|	&Ссылка КАК ДокументОснование,
	|	ТаблицаДокумента.ДокументОснование КАК ДокументОснованиеЗаявки,
	|	ТаблицаДокумента.Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.Прочее) КАК Статья,
...
//+++ Дмгг
Функция ПолучитьДанныеОФормированииПроводок(Ссылка)
