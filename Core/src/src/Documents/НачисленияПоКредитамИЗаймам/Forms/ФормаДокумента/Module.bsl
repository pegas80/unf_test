...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидОперацииНачисленияПоЗаймам = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоЗаймамСотрудникам;
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		Если ВыбранноеЗначение.Свойство("АдресНачисленийВХранилище") Тогда
			...
			Если Объект.Начисления.Количество() = 0 Тогда
				СтрокаДляВидаОперации = ?(Объект.ВидОперации = ВидОперацииНачисленияПоЗаймам, "займам", "кредитам");
				ПоказатьПредупреждение(Неопределено, "За указанный период выполнять начисления по "+СтрокаДляВидаОперации+" не требуется");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	ЗаполнитьВидНачисленияУдержанияНДФЛ()
...
Процедура ЗаполнитьНачисления(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВидОперации) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ოპერაცია არ არის მითითებული!';ru='Не указана операция!';en='operation is not available!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ორგანიზაცია არ არის მითითებული!';ru='Не указана организация!';en='Not specified organization!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ДатаНачала) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='დარიცხვების პერიოდის დასაწყისი არ არის მითითებული!';ru='Не указано начало периода начислений!';en='Unknown beginning of the period charge!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ДатаОкончания) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='დარიცხვების პერიოდის დასასრული არ არის მითითებული!';ru='Не указан конец периода начислений!';en='Unknown period-end accruals!'"));
		Возврат;
	КонецЕсли;
...
Процедура ВидОперацииПриИзмененииНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоЗаймамСотрудникам") Тогда
		Элементы.НачисленияКонтрагент.Видимость = Ложь;
		Элементы.НачисленияСотрудник.Видимость = Истина;
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитамВыданным") Тогда
		Элементы.НачисленияКонтрагент.Видимость = Истина;
		Элементы.НачисленияКонтрагент.Заголовок = НСтр("ka='კონტრაგენტი';ru='Контрагент';en='Customer'");
		Элементы.НачисленияСотрудник.Видимость = Ложь;
	Иначе
		Элементы.НачисленияКонтрагент.Видимость = Истина;
		Элементы.НачисленияКонтрагент.Заголовок = НСтр("ka='ბანკი';ru='Банк';en='Bank'");
		Элементы.НачисленияСотрудник.Видимость = Ложь;
	КонецЕсли;
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитамВыданным Тогда
		НовыйПараметр = Новый ПараметрВыбора("Отбор.ВидДоговора", Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный);
	Иначе
...
Процедура НачисленияДоговорКредитаИЗаймаПриИзменении(Элемент)
	...
	ЗаполнитьВидНачисленияУдержанияНДФЛ(Элементы.Начисления.ТекущиеДанные);
КонецПроцедуры

&НаСервереБезКонтекста
Функция НачисленияДоговорКредитаИЗаймаПриИзмененииНаСервере(ДоговорКредитаЗайма, ВидОперации)
	
	ДанныеДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКредитаЗайма,
		"ВалютаРасчетов, ВидДоговора, Контрагент, Сотрудник");
	
	СтруктураРеквизитов = Новый Структура;
	СтруктураРеквизитов.Вставить("ВалютаРасчетов", ДанныеДокумента.ВалютаРасчетов);
	Если ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитам И
		ДанныеДокумента.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.КредитПолученный Тогда
		
		СтруктураРеквизитов.Вставить("Контрагент", ДанныеДокумента.Контрагент);
		
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитамВыданным И
		ДанныеДокумента.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный Тогда
		
		СтруктураРеквизитов.Вставить("Контрагент", ДанныеДокумента.Контрагент);
		
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоЗаймамСотрудникам И
		ДанныеДокумента.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику Тогда
		
		СтруктураРеквизитов.Вставить("Сотрудник", ДанныеДокумента.Сотрудник);
...
&НаКлиенте
Процедура ЗаполнитьВидНачисленияУдержанияНДФЛ(ТекущиеДанные = Неопределено)
		
	Если ТекущиеДанные = Неопределено Тогда
		Для Каждого Стр Из Объект.Начисления Цикл
			Стр.ВидНачисленияУдержанияНДФЛ = ПолучитьВидНачисленияУдержанияНДФЛ(Стр.ДоговорКредитаЗайма);
			Стр.СуммаНДФЛ = Стр.Сумма/(1-ПолучитьСтавкаНДФЛ(Стр.ДоговорКредитаЗайма)/100)-Стр.Сумма ;
		КонецЦикла;
	Иначе
		ТекущиеДанные.ВидНачисленияУдержанияНДФЛ = ПолучитьВидНачисленияУдержанияНДФЛ(ТекущиеДанные.ДоговорКредитаЗайма);
		ТекущиеДанные.СуммаНДФЛ = ТекущиеДанные.Сумма/(1-ПолучитьСтавкаНДФЛ(ТекущиеДанные.ДоговорКредитаЗайма)/100)-ТекущиеДанные.Сумма ;
	КонецЕсли;

КонецПроцедуры                                                                                                                            
&НаСервере
Функция ПолучитьВидНачисленияУдержанияНДФЛ(Договор)
	
Возврат Договор.ВидНачисленияУдержанияНДФЛ
		
КонецФункции

&НаСервере

Функция ПолучитьСтавкаНДФЛ(Договор)
	
Возврат Договор.ВидНачисленияУдержанияНДФЛ.СтавкаНДФЛ;
...
&НаКлиенте
Процедура НачисленияСуммаПриИзменении()
	ЗаполнитьВидНачисленияУдержанияНДФЛ(Элементы.Начисления.ТекущиеДанные);
КонецПроцедуры

&НаКлиенте
Процедура НачисленияВидНачисленияУдержанияНДФЛПриИзменении()
	ТекущиеДанные = Элементы.Начисления.ТекущиеДанные;
	ТекущиеДанные.СуммаНДФЛ = ТекущиеДанные.Сумма/(1-ПолучитьСтавкаНДФЛ(ТекущиеДанные.ДоговорКредитаЗайма)/100)-ТекущиеДанные.Сумма ;	
