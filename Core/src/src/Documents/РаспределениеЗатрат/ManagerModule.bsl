...
Процедура СформироватьТаблицаЗапасыЗапасы(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства)
	...
	//Малышев 2024-06-23 ONESOFT-3264  (--
	//Запрос.Текст = 	
	//"ВЫБРАТЬ
	...
	ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства,,,,,Ложь).Выгрузить(); 
	...
	//СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Колонки.Добавить("ДокументПоступления"); //Малышев 2024-12-09 ONESOFT-3646	
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			//Малышев 2024-12-09 ONESOFT-3646
			...
			Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяРезерв Тогда
				...
				КоличествоКСписанию = КоличествоТребуетсяРезерв; //Малышев 2024-12-09 ONESOFT-3646	
            ...
            ИначеЕсли КоличествоОстаток <= КоличествоТребуетсяРезерв Тогда  //Малышев 2024-12-09 ONESOFT-3646				
			...
			КоличествоТребуетсяРезерв = КоличествоТребуетсяРезерв - КоличествоКСписанию; //Малышев 2024-12-09 ONESOFT-3646
			...
			СтрокаТаблицыРасход.ДокументПоступления = Неопределено; //СтрокаТаблицыОстатков.ДокументПоступления;
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
				...
				СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект; //Малышев 2024-06-23 ONESOFT-3264
				...
				СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект; //Малышев 2024-06-23 ONESOFT-3264
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
				...
				КоличествоКСписанию = КоличествоТребуетсяСвободныйОстаток; //Малышев 2024-12-09 ONESOFT-3646	
            ...
            ИначеЕсли КоличествоОстаток <= КоличествоТребуетсяСвободныйОстаток Тогда  //Малышев 2024-12-09 ONESOFT-3646				
			...
			КоличествоТребуетсяСвободныйОстаток = КоличествоТребуетсяСвободныйОстаток - КоличествоКСписанию; //Малышев 2024-11-30 ONESOFT-3642
			...
			СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
			//СтрокаТаблицыРасход.ФиксированнаяСтоимость = Истина; //Малышев 2025-03-25 ONESOFT-3922 // Kuzmich, 08.05.2025, ONESOFT-4061, ONESOFT-3922, закоментировал
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
				...
				СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
				СтрокаТаблицыПриход.Количество = 0;
				СтрокаТаблицыПриход.КоличествоРаспределенияЗапасов = СтрокаТаблицаЗапасы.КоличествоРаспределенияЗапасов; //+++ Дмгг
				...
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					...
					ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_РаспределениеЗатрат2(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства, СтрокаТаблицыПриход);
				КонецЕсли;
...
Процедура ИнициализироватьДанныеПоЗапасам(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства) Экспорт
	...
	//+++
	//Малышев 2025-03-14 ONESOFT-3922 (--
	//|	Запасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаРегл,
	|	ВЫБОР 
	|		КОГДА Запасы.СчетУчетаРегл = Значение(ПланСчетов.Хозрасчетный.ПустаяССылка)
	|			ТОГДА Запасы.Номенклатура.СчетУчетаЗатратРегл
	|		ИНАЧЕ Запасы.СчетУчетаРегл
	|	КОНЕЦ КАК СчетУчетаРегл,
	//Малышев 2025-03-14 ONESOFT-3922 --)
	
	|	РаспределениеЗапасов.Номенклатура.СчетУчетаЗатратРегл КАК КоррСчетУчетаРегл,
	//---
	...
	//Малышев 2024-06-23 ONESOFT-3264 (--
	|	РаспределениеЗапасов.Проект как ПроектДт,
	|	Запасы.Проект как ПроектКт,
	|	Запасы.Проект как Проект,
	|	РаспределениеЗапасов.Проект как КоррПроект,
	//Малышев 2024-06-23 ONESOFT-3264 --)
	...
	|	КОНЕЦ КАК Резерв,
	|	РаспределениеЗапасов.ДокументПоступления КАК ДокументПоступления, //Малышев 2024-12-09 ONESOFT-3646
	|	0 КАК Сумма,
	|	РаспределениеЗапасов.Номенклатура.СчетУчетаЗатрат КАК СчетДт,
	|	Запасы.Номенклатура.СчетУчетаЗатрат КАК СчетКт,
	//+++
	|	РаспределениеЗапасов.Номенклатура.СчетУчетаЗатратРегл КАК СчетДтРегл,
	
	//Малышев 2025-03-14 ONESOFT-3922 --)
	//|	Запасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетКтРегл,
	|	ВЫБОР 
	|		КОГДА Запасы.СчетУчетаРегл = Значение(ПланСчетов.Хозрасчетный.ПустаяССылка)
	|			ТОГДА Запасы.Номенклатура.СчетУчетаЗатратРегл
	|		ИНАЧЕ Запасы.СчетУчетаРегл
	...
	|	ТаблицаЗапасы.ПроектДт, 
	|	ТаблицаЗапасы.ПроектКт, 
	...
	|	ТаблицаЗапасы.ДокументПоступления КАК ДокументПоступления, //Малышев 2024-12-09 ONESOFT-3646
	|	СУММА(ТаблицаЗапасы.Количество) КАК Количество,
	|	СУММА(ТаблицаЗапасы.Количество) КАК КоличествоРаспределенияЗапасов, //+++ Дмгг
	...
	|	ТаблицаЗапасы.Партия,
	|	ТаблицаЗапасы.ДокументПоступления, //Малышев 2024-12-09 ONESOFT-3646
	...
	|	ТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ТаблицаЗапасы.Партия КАК Партия,
	|	ТаблицаЗапасы.Проект КАК Проект,   //Малышев 2024-06-23 ONESOFT-3264
	|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|
	...
	|	ТаблицаЗапасы.Номенклатура,
	|	ТаблицаЗапасы.Характеристика,
	|	ТаблицаЗапасы.Проект,  //Малышев 2024-06-23 ONESOFT-3264
	|	ТаблицаЗапасы.Партия
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства) Экспорт
	...
	//Малышев 2024-06-23 ONESOFT-3264 (--
	|	РаспределениеЗатрат.Проект как ПроектДт,
	|	Затраты.Проект как ПроектКт,
	|	Затраты.Проект как Проект,
	|	РаспределениеЗатрат.Проект как КоррПроект,
	//Малышев 2024-06-23 ONESOFT-3264 --)
	...
	|	РаспределениеЗатрат.ДокументПоступления КАК ДокументПоступления, //Малышев 2024-12-09 ONESOFT-3646
	...
	Для н = 0 По ТаблицаЗапасы.Количество() - 1 Цикл
		...
		СтрокаТаблицаЗапасыПриход.Проект = СтрокаТаблицаЗапасыРасход.КоррПроект; //Малышев 2024-06-23 ONESOFT-3264
		...
		//Малышев 2024-12-10 ONESOFT-3646 - очистим у расхода документ поступления
		СтрокаТаблицаЗапасыРасход.ДокументПоступления = Неопределено; 
...
Процедура ВыполнитьКонтроль(ДокументСсылкаРаспределениеЗатрат, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если Константы.КонтролироватьОстаткиЗапасовНаСкладахПриПроведении.Получить() Тогда
		...
		Результат = УправлениеНебольшойФирмойСервер.ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(ДокументСсылкаРаспределениеЗатрат);
		
		Если НЕ Результат.Пустой() Тогда
			ДокументСсылкаРаспределениеЗатрат_Объект = ДокументСсылкаРаспределениеЗатрат.ПолучитьОбъект();
			ВыборкаИзРезультатаЗапроса = Результат.Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументСсылкаРаспределениеЗатрат_Объект, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;			
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение Тогда
		...
		//Малышев 2024-06-23 ONESOFT-3094
		+ УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКонтроля_ОстаткиЗапасов(ДополнительныеСвойства, Ложь) //МассивРезультатов[7]
		...
		//МассивРезультатов = Запрос.Выполнить(); //+++ Дмгг
		МассивРезультатов = Запрос.ВыполнитьПакет(); //+++ Дмгг
