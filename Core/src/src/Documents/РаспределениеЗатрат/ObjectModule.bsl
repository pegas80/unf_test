...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		ВызватьИсключение НСтр("ka='ხარჯების განაწილების შეყვანა შეუძლებელია შეკვეთა-განწესის საფუძველზე!';ru='Нельзя ввести Распределение затрат на основании заказ-наряда!';en='You can not enter the distribution of costs based on the purchase order!'");
	КонецЕсли;
