...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.СписаниеВАНаТовары.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	// Контроль возникновения отрицательных остатков.
	Документы.СписаниеВАНаТовары.ВыполнитьКонтроль(Ссылка, ДополнительныеСвойства, Отказ, Истина);
...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	// Не вверное распределение.
	Запрос = Новый Запрос();
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаДокумента.НомерСтроки КАК НомерСтроки,
	|	ТаблицаДокумента.ОстаточнаяСтоимость - ТаблицаДокумента.АмортизацияЗаМесяц КАК ОстаточнаяСтоимость,
	...
	|	СУММА(ТаблицаДокумента.Себестоимость) КАК Себестоимость
	|ПОМЕСТИТЬ ВТГ_Групп
	|ИЗ
	|	ТаблицаДокумента КАК ТаблицаДокумента
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТГ_Групп.НомерСтроки,
	|	ВТГ_Групп.ВнеоборотныйАктив,
	|	ВТГ_Групп.ОстаточнаяСтоимость,
	|	ВТГ_Групп.Себестоимость
	|ИЗ
	|	ВТГ_Групп КАК ВТГ_Групп
	|ГДЕ
	|	ВТГ_Групп.ОстаточнаяСтоимость <> ВТГ_Групп.Себестоимость";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='Остаточная стоимость имущества ""%ВнеоборотныйАктив%"" не равна распределенной себестоимости на товарах.';ru='Остаточная стоимость имущества ""%ВнеоборотныйАктив%"" не равна распределенной себестоимости на товарах.';en='The residual value of the property ""% VneoborotnyyAktiv%"" is not equal to the cost of distributed products.'"
			);
...
Процедура РаспределитьПоКоличеству() Экспорт

	КопияТаблицы = ВнеоборотныеАктивы.Выгрузить().Скопировать();
	
	КопияТаблицы.Свернуть("ВнеоборотныйАктив, ОстаточнаяСтоимость, АмортизацияЗаМесяц", "Количество");
	
	Для Каждого СтрокВА Из КопияТаблицы Цикл 
		
		ИсхСуммаСебестоимость = 0;
		БазаРаспределенияКоличество = СтрокВА.Количество;
		ВсегоСебестоимость = СтрокВА.ОстаточнаяСтоимость - СтрокВА.АмортизацияЗаМесяц;
		
		МассивСтрок = ВнеоборотныеАктивы.НайтиСтроки(Новый Структура("ВнеоборотныйАктив", СтрокВА.ВнеоборотныйАктив));
		Для Каждого НайденнаяСтрока Из МассивСтрок Цикл
			
			НайденнаяСтрока.Себестоимость = ?(БазаРаспределенияКоличество <> 0, Окр((ВсегоСебестоимость - ИсхСуммаСебестоимость) * НайденнаяСтрока.Количество / БазаРаспределенияКоличество, 2, 1),0);
			БазаРаспределенияКоличество = БазаРаспределенияКоличество - НайденнаяСтрока.Количество;
			ИсхСуммаСебестоимость = ИсхСуммаСебестоимость + НайденнаяСтрока.Себестоимость;
...
Процедура РаспределитьПоКоэффициенту() Экспорт

	КопияТаблицы = ВнеоборотныеАктивы.Выгрузить().Скопировать();
	
	КопияТаблицы.Свернуть("ВнеоборотныйАктив, ОстаточнаяСтоимость, АмортизацияЗаМесяц", "Коэффициент");
	
	Для Каждого СтрокВА Из КопияТаблицы Цикл 
		
		ИсхСуммаСебестоимость = 0;
		БазаРаспределенияКоэффициент = СтрокВА.Коэффициент;
		ВсегоСебестоимость = СтрокВА.ОстаточнаяСтоимость - СтрокВА.АмортизацияЗаМесяц;
		
		МассивСтрок = ВнеоборотныеАктивы.НайтиСтроки(Новый Структура("ВнеоборотныйАктив", СтрокВА.ВнеоборотныйАктив));
		Для Каждого НайденнаяСтрока Из МассивСтрок Цикл
			
			НайденнаяСтрока.Себестоимость = ?(БазаРаспределенияКоэффициент <> 0, Окр((ВсегоСебестоимость - ИсхСуммаСебестоимость) * НайденнаяСтрока.Коэффициент / БазаРаспределенияКоэффициент, 2, 1),0);
			БазаРаспределенияКоэффициент = БазаРаспределенияКоэффициент - НайденнаяСтрока.Коэффициент;
			ИсхСуммаСебестоимость = ИсхСуммаСебестоимость + НайденнаяСтрока.Себестоимость;
