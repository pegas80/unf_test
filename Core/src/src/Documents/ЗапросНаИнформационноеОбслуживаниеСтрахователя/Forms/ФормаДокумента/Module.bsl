...
Процедура ЗаполнитьСписокЗастрахованныхЛиц(Команда)
	...
	Если Объект.ЗастрахованныеЛица.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ka='Список будет очищен перед заполнением."
"Продолжить?';ru='Список будет очищен перед заполнением."
"Продолжить?'");
...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Если Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОС.СверкаФИОиСНИЛС") Тогда
		Заголовок = НСтр("ka='Запрос на сверку ""%1""';ru='Запрос на сверку ""%1""';en='Request for verification of ""%1""'");
	...
	Иначе 
		Заголовок = НСтр("ka='Запрос на сверку';ru='Запрос на сверку';en='The request for reconciliation'");
	КонецЕсли;
