...
Процедура ЗаполнитьПоИнвентаризацииЗапасов(ДокументСсылкаИнвентаризацияЗапасов) Экспорт
	...
	Если Не Константы.ФункциональнаяОпцияИспользоватьПодсистемуПроизводство.Получить()
		...
		ВызватьИсключение НСтр("ka='არ შეიძლება მარაგების მიღების შეყვანა მარაგების ინვენტარიზაციის საფუძველზე, ვინაიდან საქმიანობის ტიპი ""წარმოება"" მიუწვდომელია!';ru='Нельзя ввести Оприходование запасов на основании инвентеризации запасов, т.к. недоступен вид деятельности Производство!';en='You can not enter Posting stocks based inventerizatsii stocks because unavailable activity Manufacture!'");
	КонецЕсли;
	...
	Если РезультатЗапроса.Пустой() Тогда
		ВызватьИсключение НСтр("ka='შემოსავალში გატარების გასაფორმებლად მონაცემები არ არის!';ru='Нет данных для оформления оприходования!';en='No data for the design of posting!'");
	КонецЕсли;
