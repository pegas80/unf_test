...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	//++20.02.2024 
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасы.СтруктурнаяЕдиница.ТранзитныйСклад
	|			ТОГДА ТаблицаЗапасы.СтруктурнаяЕдиница.СчетУчетаРегл
	|		ИНАЧЕ ТаблицаЗапасы.СчетУчетаРегл
	...
	|	ТаблицаЗапасы.Проект КАК Проект, //Малышев 2024-08-25
	|	ТаблицаЗапасы.КоррПартия КАК КоррПартия,
	|	ТаблицаЗапасы.КоррПроект КАК КоррПроект, //Малышев 2024-08-25
	...
	|		КОГДА ТаблицаЗапасы.ДокументОснование Ссылка Документ.КорректировкаПоступления
	|		  ТОГДА	
	|			ВЫБОР
	|				КОГДА ТаблицаЗапасы.ДокументОснование.ДокументОснование Ссылка Документ.КорректировкаПоступления
	|					ТОГДА ТаблицаЗапасы.ДокументОснование.ДокументОснование.ДокументОснование
	|				ИНАЧЕ ТаблицаЗапасы.ДокументОснование.ДокументОснование
	|			КОНЕЦ
	|		ИНАЧЕ ТаблицаЗапасы.ДокументОснование
	...
	|						ТОГДА ТаблицаЗапасы.Сумма + ТаблицаЗапасы.СуммаРасходов - ТаблицаЗапасы.СуммаНДС
...
Процедура СформироватьТаблицаЗапасыПоступление(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		//Малышев 20224-08-25 ONESOFT-3414
		Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам 
			и СтруктураДополнительныеСвойства.УчетнаяПолитика.СебестоимостьЗапасовПоПроектам Тогда
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	//+++
	|	СУММА(ТаблицаДоходыИРасходы.Сумма - ТаблицаДоходыИРасходы.СуммаНДС) КАК СуммаРасходов,
...
Процедура СформироватьТаблицаЗакупки(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗакупки.Проект КАК Проект, //Малышев 2024-08-25 ONESOFT-3414
	...
	|			ИЛИ ТаблицаЗакупки.СуммаНДС <> 0 //Малышев 2025-11-10 ONESOFT-4489
	...
	|	ТаблицаЗакупки.Проект, //Малышев 2024-08-25 ONESOFT-3414
...
Процедура СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ТаблицаЗапасы.Партия КАК Партия,
	|	ТаблицаЗапасы.Проект КАК Проект, //Малышев 2024-08-25 ONESOFT-3414
	|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	...
	|	ТаблицаЗапасы.Период,
	|	ТаблицаЗапасы.ВидДвижения,
	|	ТаблицаЗапасы.Проект, //Малышев 2024-08-25 ONESOFT-3414
	|	ТаблицаЗапасы.Партия";
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	//Малышев 2025-11-13 ONESOFT-4489 - отменил комментарий
	...
	//|	СУММА(ВЫБОР
	//|			КОГДА ТаблицаДокумента.ВключатьРасходыВСебестоимость
	//|				ТОГДА ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС + ТаблицаДокумента.СуммаРасходов
	//|			ИНАЧЕ ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС
	//|		КОНЕЦ) КАК Сумма,
	//|	СУММА(ВЫБОР
	//|			КОГДА ТаблицаДокумента.ВключатьРасходыВСебестоимость
	//|				ТОГДА ТаблицаДокумента.СуммаВал - ТаблицаДокумента.СуммаНДСВал + ТаблицаДокумента.СуммаРасходовВал
	...
	//Малышев 2025-11-14 ONESOFT-4489 - убираем нулевые строки ((--
	ТаблицаРезультата2 = ТаблицаРезультата.СкопироватьКолонки();
	Для каждого СтрокаДанных из ТаблицаРезультата Цикл
		Если СтрокаДанных.Сумма <> 0 или СтрокаДанных.СуммаВал <> 0 Тогда
			НовСтрока = ТаблицаРезультата2.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрока, СтрокаДанных );
		КонецЕсли;	
	КонецЦикла;	                                                                                                
	ТаблицаРезультата = ТаблицаРезультата2;
	//Малышев 2025-11-14 ONESOFT-4489 - убираем нулевые строки  --)
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	|	1 КАК Порядок,
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасы.ДокументОснование.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика)
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства) Экспорт
	...
	тзКурсаВалУчета = РегистрыСведений.КурсыВалют.СрезПоследних(ДокументСсылкаКорректировкаПоступления.Дата, СтруктураОтбора);
	...
	тзЗапасы = ДокументСсылкаКорректировкаПоступления.Запасы.Выгрузить();
	...
	Для Каждого Строк Из тзЗапасы Цикл 
		//+++ Дмгг, 20.02.24, ONESOFT-2901
		//Строк.СуммаРегл = Строк.Сумма * ДокументСсылкаКорректировкаПоступления.Курс * ДокументСсылкаКорректировкаПоступления.Кратность;
		//Строк.СуммаДоИзмененияРегл = Строк.СуммаДоИзменения * ДокументСсылкаКорректировкаПоступления.Курс * ДокументСсылкаКорректировкаПоступления.Кратность;
		//Строк.СуммаДоКорректировкиРегл = Строк.СуммаДоКорректировки * ДокументСсылкаКорректировкаПоступления.Курс * ДокументСсылкаКорректировкаПоступления.Кратность;
		//Строк.СуммаНДСРегл = Строк.СуммаНДС * ДокументСсылкаКорректировкаПоступления.Курс * ДокументСсылкаКорректировкаПоступления.Кратность;
		//Строк.СуммаНДСДоИзмененияРегл = Строк.СуммаНДСДоИзменения * ДокументСсылкаКорректировкаПоступления.Курс * ДокументСсылкаКорректировкаПоступления.Кратность;
		//Строк.СуммаНДСДоКорректировкиРегл = Строк.СуммаНДСДоКорректировки * ДокументСсылкаКорректировкаПоступления.Курс * ДокументСсылкаКорректировкаПоступления.Кратность;
		Строк.СуммаРегл = Строк.Сумма * ДокументСсылкаКорректировкаПоступления.Курс * КратностьУчета / (КурсУчета * ДокументСсылкаКорректировкаПоступления.Кратность);
		Строк.СуммаДоИзмененияРегл = Строк.СуммаДоИзменения * ДокументСсылкаКорректировкаПоступления.Курс * КратностьУчета / (КурсУчета * ДокументСсылкаКорректировкаПоступления.Кратность);
		Строк.СуммаДоКорректировкиРегл = Строк.СуммаДоКорректировки * ДокументСсылкаКорректировкаПоступления.Курс * КратностьУчета / (КурсУчета * ДокументСсылкаКорректировкаПоступления.Кратность);
		Строк.СуммаНДСРегл = Строк.СуммаНДС * ДокументСсылкаКорректировкаПоступления.Курс * КратностьУчета / (КурсУчета * ДокументСсылкаКорректировкаПоступления.Кратность);
		Строк.СуммаНДСДоИзмененияРегл = Строк.СуммаНДСДоИзменения *ДокументСсылкаКорректировкаПоступления.Курс * КратностьУчета / (КурсУчета * ДокументСсылкаКорректировкаПоступления.Кратность);
		Строк.СуммаНДСДоКорректировкиРегл = Строк.СуммаНДСДоКорректировки * ДокументСсылкаКорректировкаПоступления.Курс * КратностьУчета / (КурсУчета * ДокументСсылкаКорректировкаПоступления.Кратность);
		//+++ Дмгг, 20.02.24, ONESOFT-2901
		
	КонецЦикла;
	
	Если ДокументСсылкаКорректировкаПоступления.ВалютаДокумента <> Константы.ВалютаУчета.Получить()
		И ДокументСсылкаКорректировкаПоступления.Предоплата.Количество() > 0 Тогда 
		
		ПересчитатьСуммыРегл(тзЗапасы, ДокументСсылкаКорректировкаПоступления);
		ПересчитатьСуммыДоКорРегл(тзЗапасы, ДокументСсылкаКорректировкаПоступления);
		ПересчитатьСуммыДоИзмРегл(тзЗапасы, ДокументСсылкаКорректировкаПоступления);
		
	КонецЕсли;
	
	тзЗапасы.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.КорректировкаПоступления"));
	тзЗапасы.ЗаполнитьЗначения(ДокументСсылкаКорректировкаПоступления, "Ссылка");
	//---
	
	Запрос = Новый Запрос(ПолучитьТекстЗапросаИнициализации());
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц; 
	
	//Малышев 2024-08-24 ONESOFT-3414  (--
	Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
	...
	СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
	
	//Малышев 2025-11-19 ONESOFT-4489
	СформироватьТаблицаАнализПоПодразделениям(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
    СформироватьТаблицаСебестоимостьЗакупки(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);


	СформироватьТаблицаУправленческий(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_КорректировкаПоступления(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
	
КонецПроцедуры // ИнициализироватьДанныеДокумента()

Процедура СформироватьТаблицаАнализПоПодразделениям(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	
	Если не Константы.ИспользоватьПодразделениеВЗапасеПриходнаяНакладнаяИПриходныйОрдер.Получить()	Тогда
		Возврат;
	КонецЕсли;	
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаАнализПоПодразделениям", РезультатЗапроса.Выгрузить());

КонецПроцедуры	

Процедура СформироватьТаблицаСебестоимостьЗакупки(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)

	ТаблицаДанных = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.СкопироватьКолонки();
	ТаблицаДанных.Колонки.Добавить("ДокументЗакупки");
	Для каждого СтрДвижения из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы Цикл
		НовСтрока = ТаблицаДанных.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтрока, СтрДвижения);
		НовСтрока.ДокументЗакупки = ДокументСсылкаКорректировкаПоступления.ДокументОснование;
	КонецЦикла;
	
 	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСебестоимостьЗакупки", ТаблицаДанных);
	
КонецПроцедуры

Функция ПолучитьТекстЗапросаИнициализации()
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Выразить(тзЗапасы.Ссылка как Документ.КорректировкаПоступления) КАК Ссылка,
	...
	//|	тзЗапасы.Ссылка.ОтражатьВУчете КАК ОтражатьВУчете, 
	...
	|	тзЗапасы.СуммаРасходовДоКорректировки КАК СуммаРасходовДоКорректировки,
	|	тзЗапасы.СуммаРасходовДоИзменения КАК СуммаРасходовДоИзменения,
	|	тзЗапасы.СодержаниеДоИзменения КАК СодержаниеДоИзменения,
	|	тзЗапасы.Содержание КАК Содержание,
	|	тзЗапасы.ЕстьВДокументеПоступления КАК ЕстьВДокументеПоступления,
	...
	|	КорректировкаПоступленияЗапасы.Проект, //Малышев 2024-08-24 ONESOFT-3414
	...
	|		ИНАЧЕ КорректировкаПоступленияЗапасы.СуммаРегл - КорректировкаПоступленияЗапасы.СуммаДоИзмененияРегл
	|	КОНЕЦ КАК Сумма,
	...
	|		ИНАЧЕ КорректировкаПоступленияЗапасы.СуммаНДСРегл - КорректировкаПоступленияЗапасы.СуммаНДСДоИзмененияРегл
	|	КОНЕЦ КАК СуммаНДС,      
	|	ВЫБОР
	|		КОГДА НЕ КорректировкаПоступленияЗапасы.ЕстьВДокументеПоступления
	|			ТОГДА КорректировкаПоступленияЗапасы.СуммаНДС
	...
	|	ВТ_Запасы КАК КорректировкаПоступленияЗапасы      
	|ГДЕ
	...
	//Малышев 2025-11-10 ONESOFT-4489 ((-
	|			ИЛИ (КорректировкаПоступленияЗапасы.СуммаНДС - КорректировкаПоступленияЗапасы.СуммаНДСДоИзменения) <> 0
    //Малышев 2025-11-10 ONESOFT-4489 -)
	...
	|		КОГДА &УчетПоПроектам = ЛОЖЬ Тогда Значение(Справочник.Проекты.ПустаяСсылка)
	|		КОГДА &СебестоимостьЗапасовПоПроектам = ЛОЖЬ Тогда Значение(Справочник.Проекты.БезПроекта)
	|		КОГДА КорректировкаПоступленияРасходы.Проект = Значение(Справочник.Проекты.ПустаяСсылка) Тогда Значение(Справочник.Проекты.БезПроекта)
	|		ИНАЧЕ КорректировкаПоступленияРасходы.Проект 
	|	КОНЕЦ КАК Проект, 
	//Малышев 2024-08-24 ONESOFT-3414  --)
	...
	|	ВЫБОР 
	|		КОГДА ИзмененияЗапасов.Ссылка.ДокументОснование ССЫЛКА Документ.ПриходнаяНакладная
	|			ТОГДА ИзмененияЗапасов.Ссылка.ДокументОснование.ВидНалогаНДС 
	//Малышев 2024-06-03 ONESOFT-3200
	|		КОГДА ИзмененияЗапасов.Ссылка.ДокументОснование ССЫЛКА Документ.КорректировкаПоступления
	|			ТОГДА ИзмененияЗапасов.Ссылка.ДокументОснование.ВидНалогаНДС 
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ПустаяСсылка)
	...
	|	ИзмененияЗапасов.Ссылка.СтруктурнаяЕдиница.СчетУчетаНаценкиРегл КАК СчетУчетаНаценкиРегл,
	...
	|	ИзмененияЗапасов.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРозницеРегл КАК СчетУчетаВРозницеРегл,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
	|			ТОГДА ИзмененияЗапасов.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРозницеРегл
	|		ИНАЧЕ ИзмененияЗапасов.Номенклатура.СчетУчетаЗапасовРегл
	|	КОНЕЦ КАК СчетУчетаРегл,
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
	|			ТОГДА ИзмененияЗапасов.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРозницеРегл.Валютный
	|		ИНАЧЕ ИзмененияЗапасов.Номенклатура.СчетУчетаЗапасовРегл.Валютный
	|	КОНЕЦ КАК СчетУчетаВалютныйРегл,
	|	НЕОПРЕДЕЛЕНО КАК КоррСчетУчетаРегл,
	|	ИзмененияЗапасов.Номенклатура КАК Номенклатура,
	|	ИзмененияЗапасов.Проект КАК Проект,  //Малышев 2024-08-25
	...
	|	НЕОПРЕДЕЛЕНО КАК КоррПроект,  //Малышев 2024-08-25
	|	ВЫБОР
	...
	|				ИЛИ ИзмененияЗапасов.СуммаНДС > 0  //Малышев 2025-11-10 ONESOFT-4489
	...
	|	ИзмененияЗапасов.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщикомРегл.Валютный КАК СчетУчетаРасчетовСПоставщикомВалютныйРегл,
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|					КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА ИзмененияЗапасов.СуммаНДСВал * КурсыРегВалюты.Курс * ИзмененияЗапасов.Ссылка.Кратность / (ИзмененияЗапасов.Ссылка.Курс * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ ИзмененияЗапасов.СуммаНДСВал
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСВал,
	|	ВЫБОР
	...
	|	ВЫБОР 
	|		КОГДА ИзмененияРасходов.Ссылка.ДокументОснование ССЫЛКА Документ.ПриходнаяНакладная
	|			ТОГДА ИзмененияРасходов.Ссылка.ДокументОснование.ВидНалогаНДС 
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ПустаяСсылка)
	|	КОНЕЦ КАК ВидНалогаНДС,
	|	&Организация КАК Организация,
	...
	|	ИзмененияРасходов.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаРегл,
	|	ИзмененияРасходов.Номенклатура КАК Номенклатура,
	|	ИзмененияРасходов.Проект КАК Проект,  //Малышев 202-08-25
	...
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСЗакупкиПродажи,
	//+++ Дмгг
	|	ВЫРАЗИТЬ(ИзмененияРасходов.СуммаНДС КАК ЧИСЛО(15, 2)) КАК СуммаНДСЗакупкиПродажиВал,
	//--- Дмгг
	...
	|	КОНЕЦ 
	|		ИНАЧЕ ВЫБОР
	|		КОГДА ИзмененияРасходов.Номенклатура.СчетУчетаЗатратРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	|				И НЕ ИзмененияРасходов.Ссылка.ВключатьРасходыВСебестоимость
	|			ТОГДА ИзмененияРасходов.НаправлениеДеятельности
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВременнаяТаблицаШапка.ВестиУчетРасходовПоДоговорамОбслуживания
	...
	|	ИзмененияРасходов.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРасчетовСПоставщикомРегл,
	|	ВЫБОР 
	|		КОГДА &ИспользованиеТолькоРеглПланаСчетов
	|			ТОГДА ИзмененияРасходов.Номенклатура.СчетУчетаЗатратРегл.ТипСчета
	|		ИНАЧЕ ИзмененияРасходов.Номенклатура.СчетУчетаЗатрат.ТипСчета
	|	КОНЕЦ КАК ТипСчета,
	|	ВЫБОР
	...
	//+++
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателемРегл КАК СчетУчетаРасчетовСПокупателемРегл,
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаАвансовПокупателяРегл КАК СчетУчетаАвансовПокупателяРегл,
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРасчетовСПоставщикомРегл,
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаАвансовПоставщикуРегл КАК СчетУчетаАвансовПоставщикуРегл,
	//---
	...
	|			КОНЕЦ * КорректировкаПоступленияПредоплата.Ссылка.ДокументОснование.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КорректировкаПоступленияПредоплата.Ссылка.ДокументОснование.Кратность) КАК ЧИСЛО(15, 2))) КАК Сумма,
	|	СУММА(ВЫБОР
	|			КОГДА НЕ КорректировкаПоступленияПредоплата.ЕстьВДокументеПоступления
	...
	//+++
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателемРегл,
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаАвансовПокупателяРегл,
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщикомРегл,
	|	КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаАвансовПоставщикуРегл,
	//---
...
//+++
Процедура ПересчитатьСуммыРегл(тзЗапасы, ДокументСсылкаКорректировкаПоступления)

	Если тзЗапасы.Количество() = 0 Тогда 
		Возврат;
	КонецЕсли;
	
	КурсДок = ДокументСсылкаКорректировкаПоступления.Курс;
	КратностьДок = ДокументСсылкаКорректировкаПоступления.Кратность;
	тчПредоплат = ДокументСсылкаКорректировкаПоступления.Предоплата;
	Сч=0;
	ОстатокПерсчета = 0;
	СуммаРегл = 0;
	СуммаНДСРегл = 0;
	...
	Для Каждого СтрокаПредоплаты Из тчПредоплат Цикл
		...
		Пока СуммаПредоплатыВал <> 0 Цикл
			...
			Если Сч > тзЗапасы.Количество() - 1 Тогда 
				...
				СуммаНДСРегл = СуммаНДСРегл + ?(ДокументСсылкаКорректировкаПоступления.СуммаВключаетНДС, 
			...
			Если СуммаПредоплатыВал >= СтрокаТоваров.Сумма И ОстатокПерсчета = 0 Тогда       
					...
					СуммаРегл = СтрокаТоваров.Сумма * КурсПредоплаты * КратностьПредоплаты;   
				СуммаНДСРегл = СуммаНДСРегл + ?(ДокументСсылкаКорректировкаПоступления.СуммаВключаетНДС, 
									  СуммаРегл - (СуммаРегл) / ((СтавкаНДС + 100) / 100),
									  СуммаРегл * СтавкаНДС / 100);
				СуммаПредоплатыВал = СуммаПредоплатыВал - СтрокаТоваров.Сумма;
			...
			ИначеЕсли СуммаПредоплатыВал < ОстатокПерсчета И ОстатокПерсчета <> 0 Тогда 
				...
				СуммаНДСРегл = ?(ДокументСсылкаКорректировкаПоступления.СуммаВключаетНДС, 
									  СуммаРегл - (СуммаРегл) / ((СтавкаНДС + 100) / 100),
									  СуммаРегл * СтавкаНДС / 100);
...
Процедура ПересчитатьСуммыДоКорРегл(тзЗапасы, ДокументСсылкаКорректировкаПоступления)
	...
	Для Каждого СтрокаПредоплаты Из тчПредоплат Цикл
		...
		Пока СуммаПредоплатыВал <> 0 Цикл
			...
			Если СуммаПредоплатыВал >= СтрокаТоваров.Сумма И ОстатокПерсчета = 0 Тогда 
				...
				СуммаРегл = СтрокаТоваров.СуммаДоКорректировки * КурсПредоплаты * КратностьПредоплаты;
				СуммаНДСРегл = СуммаНДСРегл + ?(ДокументСсылкаКорректировкаПоступления.СуммаВключаетНДС, 
									  СуммаРегл - (СуммаРегл) / ((СтавкаНДС + 100) / 100),
									  СуммаРегл * СтавкаНДС / 100);
				СуммаПредоплатыВал = СуммаПредоплатыВал - СтрокаТоваров.СуммаДоКорректировки;
...
Процедура ПересчитатьСуммыДоИзмРегл(тзЗапасы, ДокументСсылкаКорректировкаПоступления)
	...
	Для Каждого СтрокаПредоплаты Из тчПредоплат Цикл
		...
		Пока СуммаПредоплатыВал <> 0 Цикл
			...
			Если СуммаПредоплатыВал >= СтрокаТоваров.Сумма И ОстатокПерсчета = 0 Тогда 
                ...
                СуммаРегл = СтрокаТоваров.СуммаДоИзменения * КурсПредоплаты * КратностьПредоплаты;
				СуммаНДСРегл = СуммаНДСРегл + ?(ДокументСсылкаКорректировкаПоступления.СуммаВключаетНДС, 
									  СуммаРегл - (СуммаРегл) / ((СтавкаНДС + 100) / 100),
									  СуммаРегл * СтавкаНДС / 100);
				СуммаПредоплатыВал = СуммаПредоплатыВал - СтрокаТоваров.СуммаДоИзменения;
...
Процедура ВыполнитьКонтроль(ДокументСсылкаКорректировкаПоступления, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		Если НЕ МассивРезультатов[0].Пустой()
			...
			ИЛИ НЕ МассивРезультатов[7].Пустой() //+++ Дмгг
			Тогда
