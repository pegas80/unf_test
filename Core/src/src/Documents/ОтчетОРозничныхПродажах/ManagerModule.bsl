...
Процедура СформироватьТаблицаЗапасыПродажа(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			МассивСтрокОстатков = ТаблицаЗапасыОстатки.НайтиСтроки(СтруктураДляПоиска);
			//--------------------------------------------------------------------------
			// © Шишка В.В. [25 июля 2022]  НАЧАЛО
			Если СтруктураДополнительныеСвойства.РезервированиеБезСебестоимости Тогда
			
				МассивКоличеств = Новый Массив;
				СуммаСебестоимость = 0;
				Для каждого ТекСтрокаОстаток Из МассивСтрокОстатков Цикл
				
					МассивКоличеств.Добавить(ТекСтрокаОстаток.КоличествоОстаток);
					СуммаСебестоимость = СуммаСебестоимость + ТекСтрокаОстаток.СуммаОстаток;
				
				КонецЦикла;
				Если СуммаСебестоимость > 0 Тогда
					
					МассивСумм = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(СуммаСебестоимость, МассивКоличеств);
					Для Сч = 0 По МассивСумм.Количество() - 1 Цикл
						
						МассивСтрокОстатков[Сч].СуммаОстаток = МассивСумм[Сч];	
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
				...
				СуммаКСписанию = ?(СтруктураДополнительныеСвойства.РезервированиеБезСебестоимости И СебестоимостьЕдиницы <> 0, Окр(СебестоимостьЕдиницы * КоличествоКСписанию, 2, 1)
									,Окр(СуммаОстаток * КоличествоКСписанию / КоличествоОстаток , 2, 1));
			...
			ИначеЕсли КоличествоОстаток <= КоличествоТребуетсяСвободныйОстаток Тогда   //Малышев 2024-12-06 ONESOFT-3670
			...
			КоличествоТребуетсяСвободныйОстаток = КоличествоТребуетсяСвободныйОстаток - КоличествоКСписанию;  //Малышев 2024-12-06 ONESOFT-3670
			...
			СтрокаТаблицыРасход.Количество = КоличествоКСписанию; 									//Малышев 2024-12-06 ONESOFT-3670
			Попытка
				СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления; 	//Малышев 2024-12-06 ONESOFT-3670
			Исключение
			КонецПопытки;
			...
			//ИначеЕсли Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 04.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
			Иначе	
				// Сформируем проводки.
				СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицаЗапасы);
				...
				СтрокаДоходыИРасходы.Аналитика = СтрокаТаблицаЗапасы.Контрагент;  //Малышев 2024-10-28 
		...
		// Kuzmich, 04.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицаЗапасы);
				СтрокаТаблицаУправленческий.СценарийПланирования = Справочники.СценарииПланирования.Фактический;
				СтрокаТаблицаУправленческий.Сумма = 0;
				
				// Щербаков +++
				СтрокаТаблицаХозрасчетный = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаХозрасчетный.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицаХозрасчетный, СтрокаТаблицаЗапасы);
				СтрокаТаблицаХозрасчетный.Сумма = 0;
				СтрокаТаблицаХозрасчетный.СчетДт = СтрокаТаблицаЗапасы.НаправлениеДеятельности.СчетУчетаСебестоимостиПродажРегл;
				Если СтрокаТаблицаХозрасчетный.СчетДт.Количественный Тогда
					СтрокаТаблицаХозрасчетный.КоличествоДт = КоличествоТребуетсяСвободныйОстаток;
				Иначе
					СтрокаТаблицаХозрасчетный.КоличествоДт = 0;
				КонецЕсли;
				СтрокаТаблицаХозрасчетный.СубконтоДт1 = СтрокаТаблицаЗапасы.Подразделение;
				...
				СтрокаТаблицаХозрасчетный.СчетКт = СтрокаТаблицаЗапасы.Номенклатура.СчетУчетаЗапасовРегл;
				Если СтрокаТаблицаХозрасчетный.СчетКт.Количественный Тогда
					СтрокаТаблицаХозрасчетный.КоличествоКт = КоличествоТребуетсяСвободныйОстаток;
				Иначе
					СтрокаТаблицаХозрасчетный.КоличествоКт = 0;
				КонецЕсли;
				...
				//СтрокаДоходыИРасходы.СуммаРасходов = СуммаКСписанию;
				//СтрокаДоходыИРасходы.Сумма = СуммаКСписанию;
				СтрокаДоходыИРасходы.СуммаРасходов = 0;
				СтрокаДоходыИРасходы.Сумма = 0;
...
Процедура СформироватьТаблицаПродажи(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаПродажи.Контрагент, //Малышев 2024-10-23 ONESOFT-3537
...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПриемЗапасовТоварыНаКомиссии", НСтр("ka='საკომისიო საქონლის რელიზაცია';ru='Реализация комиссионных товаров';en='Implementation of commission goods'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаУправленческий.КассаККМСчетУчета.Валютный
	|				ТОГДА ТаблицаУправленческий.СуммаВал - ТаблицаУправленческий.СуммаНДСВал
	|			ИНАЧЕ 0
	...
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаУправленческий.ТоварыНаКомиссии
	|				ТОГДА ТаблицаУправленческий.Сумма - ТаблицаУправленческий.СуммаНДС
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаВалКт,
	|	СУММА(ТаблицаУправленческий.Сумма - ТаблицаУправленческий.СуммаНДС) КАК Сумма,
	|	&ОтражениеВыручки КАК Содержание
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаУправленческий
	|ГДЕ
	|	&ПолноеПроведение
	|	И ТаблицаУправленческий.Сумма - ТаблицаУправленческий.СуммаНДС <> 0
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос1.УстановитьПараметр("Дата",ДокументСсылкаОтчетОРозничныхПродажах.Дата-2);
	Запрос1.УстановитьПараметр("Организация",ДокументСсылкаОтчетОРозничныхПродажах.Организация);
	Запрос1.УстановитьПараметр("СтруктурнаяЕдиница",ДокументСсылкаОтчетОРозничныхПродажах.СтруктурнаяЕдиница);
...
Функция ПолучитьТекстЗапросаБезФильтрЗаказов()
...
Процедура СформироватьТаблицаПродажиПоПредоставленнымАвтоматическимСкидкам(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаАвтоСкидкиНаценки.СуммаБезСкидки КАК СуммаБезСкидки
	|ИЗ
...
Процедура ВыполнитьКонтроль(ДокументСсылкаОтчетОРозничныхПродажах, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если Константы.КонтролироватьОстаткиЗапасовНаСкладахПриПроведении.Получить() Тогда
		...
		Результат = УправлениеНебольшойФирмойСервер.ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(ДокументСсылкаОтчетОРозничныхПродажах);
		
		Если НЕ Результат.Пустой() Тогда
			ДокументСсылкаОтчетОРозничныхПродажах_Объект = ДокументСсылкаОтчетОРозничныхПродажах.ПолучитьОбъект();
			ВыборкаИзРезультатаЗапроса = Результат.Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументСсылкаОтчетОРозничныхПродажах_Объект, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;			
...
"%ДополнительноеОписание%';ka='გადადებული ჩეკების წაშლისას შეცდომა მოხდა."
"დამატებითი აღწერა:"
...
"Дополнительное описание:"
"%ДополнительноеОписание%';ka='საკასო ჩეკების არქივაციისას შეცდომა მოხდა."
"სსა-ის ჩეკების არქივაცია არ შესრულდა."
"დამატებითი აღწერა:"
"%ДополнительноеОписание%'"
		);
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ИнформацияОбОшибке().Описание);
...
Функция СформироватьПечатнуюФормуКМ6(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ОтчетОРозничныхПродажах Из МассивОбъектов Цикл
		...
		ДанныеПечати.Вставить("ПрограммаУчета", НСтр("ka='1С:საწარმო 8';ru='1С:Предприятие 8';en='1C: Enterprise 8'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипКассы = Перечисления.ТипыКассККМ.АвтономнаяККМ Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დოკუმენტ %1-ისთვის სალაროს ტიპით ""ავტონომიური სსა"" გამოუყენებელი ჩეკების (КМ-3) მიხედვით ფულადი თანხების დაბრუნებაზე აქტის ბეჭდვა საჭირო არ არის.';ru='Для документа %1 с типом кассы ""Автономная ККМ"" печать Акта о возврате денежных сумм по неиспользованным чекам (КМ-3) не требуется.';en='To document% 1 to the type of cash ""Autonomous CMC"" Printing Act refund monies for unused checks (KM-3) is not required.'"),
				Выборка.Ссылка
			);
		...
		Если ВыборкаПоЧекамИтоги.НайтиСледующий(Новый Структура("ОтчетОРозничныхПродажах", Выборка.Ссылка)) Тогда
			...
			ОбластьШапки.Параметры.ВидДеятельностиПоОКДП	= "";
			ОбластьШапки.Параметры.ПрограммаУчета 			= НСтр("ka='1С:საწარმო 8';ru='1С:Предприятие 8';en='1C: Enterprise 8'");
		...
		Если Не ДокументНапечатан Тогда
			...
			ТекстОшибки = НСтр("ru='Нет данных для печати Акта о возврате денежных сумм по неиспользованным чекам (КМ-3). Первичный документ %1.';en='No data to print the Act on the return of sums of money on unused checks (KM-3). Primary document 1%.'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОРозничныхПродажах") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='საცალო გაყიდვების ანგარიშგება';ru='Отчет о розничных продажах';en='A report on retail sales'"),
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КМ6") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КМ6",
			НСтр("ru='Отчет за смену';en='Report shift'"),
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='საცალო გაყიდვების ანგარიშგება';ru='Отчет о розничных продажах';en='A report on retail sales'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "КМ6";
	КомандаПечати.Представление = НСтр("ka='КМ6 (ცვლის ანგარიშგება)';ru='КМ6 (Отчет за смену)';en='MC6 (shift report)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 4;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "КМ3";
	КомандаПечати.Представление = НСтр("ka='КМ3 (აქტი ფულადი სახსრების დაბრუნების თაობაზე)';ru='КМ3 (Акт о возврате денежных сумм)';en='KM3 (Act on the return of monies)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Функция ОтчетОРозничныхПродажахXML2_40(ДокументСсылка) Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		НоваяСтрока.doc_name   = НСтр("ka='საცალო გაყიდვების ანგარიშგება';ru='Отчет о розничных продажах';en='A report on retail sales'");
