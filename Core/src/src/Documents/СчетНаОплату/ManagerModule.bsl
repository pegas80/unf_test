...
Процедура СформироватьТаблицаОплатаСчетовИЗаказов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	//Малышев 2024-06-22 ONESOFT-3265 (--
...
Функция НапечататьInvoice(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|	СчетНаОплатуЗапасы.Ссылка.НомерЗаказа КАК НомерЗаказа,
		|	СчетНаОплатуЗапасы.Ссылка.ФиксированныйОбменныйКурс КАК ФиксированныйОбменныйКурс
		|ИЗ
		|	Документ.СчетНаОплату.Запасы КАК СчетНаОплатуЗапасы
		|ГДЕ
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Запасы = ВыборкаДокумент.Документ.Запасы;
		Скидка = ЕстьСкидка(Запасы);
		...
		Иначе 
			ОбластьШапки = МакетПечатнойФормы.ПолучитьОбласть("ШапкаБезСкидки");
			ОбластьСтрока = МакетПечатнойФормы.ПолучитьОбласть("ТабличаяЧастьБезСкидки");
			ОбластьПодвал = МакетПечатнойФормы.ПолучитьОбласть("ПодвалБезСкидки");
		КонецЕсли;
		
		
		ОбластьШапки.Параметры.НомерДоговора = ВыборкаДокумент.Договор.НомерДоговора;
		...
		Пока ВыборкаДанныхДокумента.Следующий() Цикл 
			...
			всего = ВыборкаДанныхДокумента.Количество *  ВыборкаДанныхДокумента.Цена;
			
			//ОбластьСкидка = МакетПечатнойФормы.ПолучитьОбласть("Скидка");
			//
			//Смещать = ТипСмещенияТабличногоДокумента.ПоВертикали;
			//Область = ТабличныйДокумент.Область("R19C20:R19C22");
			//ТабличныйДокумент.УдалитьОбласть(Область, Смещать);
			//
			//ТабличныйДокумент.УдалитьОбласть(Область, ТипСмещенияТабличногоДокумента.БезСмещения);
			//
			//
			//Смещать = ТипСмещенияТабличногоДокумента.ПоВертикали;
			//Область = ТабличныйДокумент.Область("R21C20:R21C22");
			//ТабличныйДокумент.УдалитьОбласть(Область, Смещать);
			...
			СуммаСкидки = СуммаСкидки +  (всего *  ВыборкаДанныхДокумента.ПроцентСкидкиНаценки)/100;
		...
		Если Скидка Тогда 
		ОбластьПодвал.Параметры.ВсегоСкидка = СуммаСкидки;
		КонецЕсли;
		Если ЗначениеЗаполнено(ВыборкаДокумент.ФайлФаксимильнаяПечать) Тогда
			...
			Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
				...
				Исключение
					ОбластьПодвал.Рисунки.Факсимилия1.Картинка = Новый Картинка(ДанныеКартинки);
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(ВыборкаДокумент.ФайлПечати) Тогда
			...
			Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
				...
				Исключение
					ОбластьПодвал.Рисунки.Печать1.Картинка = Новый Картинка(ДанныеКартинки);
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
...
Функция НапечататьInvoiceEng(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|	ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Значение КАК СТРОКА(150)) КАК НаименованиеАнг,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Ссылка КАК Контрагент
		|ПОМЕСТИТЬ ВТ_НаименованиеРу
		|ИЗ
		|	Справочник.Контрагенты.ДополнительныеРеквизиты КАК КонтрагентыДополнительныеРеквизиты
		|ГДЕ
		|	КонтрагентыДополнительныеРеквизиты.Свойство.ПометкаУдаления = ЛОЖЬ
		|	И КонтрагентыДополнительныеРеквизиты.Свойство.Наименование ПОДОБНО ""დასახელება ინგლისურად%""
		|
		|СГРУППИРОВАТЬ ПО
		|	КонтрагентыДополнительныеРеквизиты.Свойство,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Ссылка,
		|	ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Значение КАК СТРОКА(150))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	КОНЕЦ КАК Продавец,
		|	ВЫБОР
		|		КОГДА ВТ_НаименованиеРу.НаименованиеАнг <> """"
		|			ТОГДА ВТ_НаименованиеРу.НаименованиеАнг
		|		ИНАЧЕ ВЫБОР
		|				КОГДА (ВЫРАЗИТЬ(СчетНаОплатуЗапасы.Ссылка.Контрагент.НаименованиеПолное КАК СТРОКА(100))) = """"
		|					ТОГДА СчетНаОплатуЗапасы.Ссылка.Контрагент.Наименование
		|				ИНАЧЕ СчетНаОплатуЗапасы.Ссылка.Контрагент.НаименованиеПолное
		...
		|ИЗ
		|	Документ.СчетНаОплату.Запасы КАК СчетНаОплатуЗапасы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_НаименованиеРу КАК ВТ_НаименованиеРу
		|		ПО СчетНаОплатуЗапасы.Ссылка.Контрагент = ВТ_НаименованиеРу.Контрагент
		|ГДЕ
		|	СчетНаОплатуЗапасы.Ссылка В(&МассивОбъектов)
		|ИТОГИ
		|	МАКСИМУМ(Продавец),
...
Функция НапечататьInvoiceRus(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КонтрагентыДополнительныеРеквизиты.Свойство КАК Свойство,
		|	ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Значение КАК СТРОКА(150)) КАК НаименованиеРу,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Ссылка КАК Контрагент
		|ПОМЕСТИТЬ ВТ_НаименованиеРу
		|ИЗ
		|	Справочник.Контрагенты.ДополнительныеРеквизиты КАК КонтрагентыДополнительныеРеквизиты
		|ГДЕ
		|	КонтрагентыДополнительныеРеквизиты.Свойство.ПометкаУдаления = ЛОЖЬ
		|	И КонтрагентыДополнительныеРеквизиты.Свойство.Наименование ПОДОБНО ""დასახელება რუსულად%""
		|
		|СГРУППИРОВАТЬ ПО
		|	КонтрагентыДополнительныеРеквизиты.Свойство,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Ссылка,
		...
		|	КОНЕЦ КАК Продавец,
		|	ВЫБОР
		|		КОГДА ВТ_НаименованиеРу.НаименованиеРу <> """"
		|			ТОГДА ВТ_НаименованиеРу.НаименованиеРу
		|		ИНАЧЕ ВЫБОР
		|				КОГДА (ВЫРАЗИТЬ(СчетНаОплатуЗапасы.Ссылка.Контрагент.НаименованиеПолное КАК СТРОКА(100))) = """"
		|					ТОГДА СчетНаОплатуЗапасы.Ссылка.Контрагент.Наименование
		|				ИНАЧЕ СчетНаОплатуЗапасы.Ссылка.Контрагент.НаименованиеПолное
...
//+++rajak
Функция ЕстьСкидка(Запасы)
	
		МассивВыгрузки = Новый Массив;
		МассивВыгрузки = Запасы.ВыгрузитьКолонку("ПроцентСкидкиНаценки");
		количество = 0;
		
		Для Каждого стр Из МассивВыгрузки Цикл 
			
			количество = количество + Число(стр); 	
