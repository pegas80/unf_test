...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаСписаниеЗапасов, СтруктураДополнительныеСвойства)
	...
	//|				(Организация, СтруктурнаяЕдиница, СчетУчетаРегл, Номенклатура, Характеристика, Партия, выбор когда &КонтрольПоПроектам тогда Проект иначе неопределено конец, ЗаказПокупателя) В //Малышев 2024-06-04 ONESOFT-3093
	//|					(ВЫБРАТЬ
	//|						ТаблицаЗапасы.Организация,
	//|						ТаблицаЗапасы.СтруктурнаяЕдиница,
	////|						ТаблицаЗапасы.СчетУчета,
	...
	//|						ТаблицаЗапасы.Характеристика,
	//|						ТаблицаЗапасы.Партия,
	//|						выбор когда &КонтрольПоПроектам тогда ТаблицаЗапасы.Проект иначе неопределено конец,   //Малышев 2024-06-04 ONESOFT-3093
	//|						ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказПокупателя
	//|					ИЗ
	...
	//|		ДвиженияДокументаЗапасы.Проект,  //Малышев 2024-06-04 ONESOFT-3093
	//|		ВЫБОР
	...
	//
	////Малышев
	//Запрос.УстановитьПараметр("КонтрольПоПроектам", УправлениеНебольшойФирмойСервер.ВыполнитьКонтрольОстатковПоПроектам());
	...
	НоваяТаблицаЗапасов = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.СкопироватьКолонки();
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		//	и НЕ УчетСебестоимостиВРазрезеОрганизации
		//	и (
		//	(СтрокаТаблицаЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад и СтруктураДополнительныеСвойства.УчетнаяПолитика.СебестоимостьЗапасовПоПроектам)
		//		ИЛИ
		//	(СтрокаТаблицаЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение и СтруктураДополнительныеСвойства.УчетнаяПолитика.СебестоимостьЗатратПоПроектам)
		//	)
		//	Тогда
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуется Тогда
				...
				СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - КоличествоТребуется;
            ...
            ИначеЕсли КоличествоОстаток >0 и КоличествоОстаток <= КоличествоТребуется Тогда //Малышев 2024-11-19 ONESOFT-3602
			...
			КоличествоТребуется = КоличествоТребуется - КоличествоКСписанию; //Малышев 2024-11-19 ONESOFT-3602
			
			//СтрокаТаблицаЗапасы.Сумма = СуммаКСписанию;
			////СтрокаТаблицаЗапасы.Количество = КоличествоТребуется;
			//СтрокаТаблицаЗапасы.Количество = КоличествоКСписанию;
			
			
			СтрокаТаблицыРасход = НоваяТаблицаЗапасов.Добавить();
			...
			Если КоличествоТребуется > 0 Тогда
				...
				СтрокаТаблицыРасходОтрицательныеОстатки = НоваяТаблицаЗапасов.Добавить();
				...
				СтрокаТаблицыРасходОтрицательныеОстатки.Количество = КоличествоТребуется;
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы = НоваяТаблицаЗапасов;
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСписаниеЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(СписаниеЗапасовЗапасы.Количество * СписаниеЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент - &СмещениеДляОкругления КАК ЧИСЛО(15,3))
	...
	//|		КОГДА (СписаниеЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад) и &СебестоимостьЗапасовПоПроектам)
	//|				ИЛИ (СписаниеЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение) и &СебестоимостьЗатратПоПроектам)
	//|            ТОГДА 
	//|				ВЫБОР
	//|					КОГДА СписаниеЗапасовЗапасы.Проект <> ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	//|						ТОГДА СписаниеЗапасовЗапасы.Проект
	...
	|	ТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ТаблицаЗапасы.Партия КАК Партия,
	|	ТаблицаЗапасы.Проект КАК Проект, //Малышев 2024-06-26 ONESOFT-3267
	|	ТаблицаЗапасы.ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
	...
	|	ТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ТаблицаЗапасы.Партия КАК Партия,
	|	ТаблицаЗапасы.Проект КАК Прект,  //Малышев 2024-06-26 ONESOFT-3267
	|	ТаблицаЗапасы.ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
	|	ТаблицаЗапасы.Количество КАК Количество
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	...
	Запрос.УстановитьПараметр("СписаниеЗапасов", НСтр("en='Write-off stocks';ka='მარაგების ჩამოწერა';ru='Списание запасов'"));
	//+++ Lazare 29.11.2021
	...
	|	ТаблицаЗапасы.Период,
	//--------------------------------------------------------------------------
	// © Шишка В.В. [15 июня 2023]  НАЧАЛО
	|	ТаблицаЗапасы.СтруктурнаяЕдиница,
	// © Шишка В.В. [15 июня 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	|	ТаблицаЗапасы.Регистратор.КорреспонденцияРегл КАК СчетУчетаРегл,
	...
	|			ТОГДА ТаблицаЗапасы.Регистратор.КорреспонденцияРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеРасходы)
	|				ИЛИ ТаблицаЗапасы.Регистратор.КорреспонденцияРегл В (&СчетаФинРезРасходы)
	|		ИНАЧЕ ТаблицаЗапасы.Регистратор.Корреспонденция.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеРасходы)
	|				ИЛИ ТаблицаЗапасы.Регистратор.КорреспонденцияРегл В (&СчетаФинРезРасходы)
	|	КОНЕЦ
	//--- Дмгг
	|
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [16 июня 2023]  НАЧАЛО
	Запрос.УстановитьПараметр("Подразделение", ДокументСсылкаСписаниеЗапасов.Подразделение); 
	// © Шишка В.В. [16 июня 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
...
//
Процедура ВыполнитьКонтроль(ДокументСсылкаСписаниеЗапасов, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь, ПроверятьИтогиРегистров = ЛОЖЬ) Экспорт
	
	//	Запрос = Новый Запрос;
	//	Запрос.Текст = 
	//	"ВЫБРАТЬ
	//	|	СписаниеЗапасов.СтруктурнаяЕдиница.КонтролироватьОстатки как КонтролироватьОстатки
	//	|ИЗ
	//	|	Документ.СписаниеЗапасов КАК СписаниеЗапасов";
	//	РезультатЗапроса = Запрос.Выполнить();
	//	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	//	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	//		Если Константы.КонтролироватьОстаткиПриПроведении.Получить() и ВыборкаДетальныеЗаписи.КонтролироватьОстатки = Истина Тогда
	//			Возврат;
	//		КонецЕсли;
	//	КонецЦикла;
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		...
		+ УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКонтроля_ОстаткиЗапасов(ДополнительныеСвойства) //МассивРезультатов[4]
		//Малышев 2024-06-17 ONESOFT-3094 --)
		...
		Если НЕ МассивРезультатов[0].Пустой() Тогда
			...
			Иначе 
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументОбъектСписаниеЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
			КонецЕсли;
		КонецЕсли;
		...
		Если ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда 
			Если НЕ МассивРезультатов[4].Пустой() Тогда
				...
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСписаниеЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
			КонецЕсли;
		Иначе
		...
		Если НЕ МассивРезультатов[2].Пустой() Тогда
			...
			Иначе 
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомера(ДокументОбъектСписаниеЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
			КонецЕсли;
		КонецЕсли;
		...
		Если Константы.КонтролироватьОстаткиПоНомерамГТД.Получить()
			...
			Иначе 
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыВРазрезеГТД(ДокументОбъектСписаниеЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
			КонецЕсли;
...
Функция ДанныеДокументовБланкТоварногоНаполнения(МассивОбъектов)
	...
	|	СписаниеЗапасов.Подразделение КАК Подразделение,
	|	СписаниеЗапасов.Организация КАК  Организация
	|ИЗ
	|	Документ.СписаниеЗапасов КАК СписаниеЗапасов
	|ГДЕ
