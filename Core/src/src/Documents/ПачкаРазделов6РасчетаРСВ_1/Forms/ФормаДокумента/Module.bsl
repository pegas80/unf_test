...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)	
	ЗаписиОСтажеТекст = НСтр("ka='ჩანაწერები სტაჟის თაობაზე';ru='Записи о стаже';en='Records of seniority'");
...
Процедура ДанныеТекущегоДокументаПоСотрудникуВДанныеФормы(АдресВоВременномХранилище)
	...
	Если ДанныеТекущейСтрокиПоСотруднику = Неопределено  Тогда
		...
		ВызватьИсключение НСтр("ka='მიმდინარე დოკუმენტში რედაქტირებადი თანამშრომლის თაობაზე მონაცემები ვერ მოიძებნა.';ru='В текущем документе не найдены данные по редактируемому сотруднику.';en='In the current document data edited employee not found.'");
	КонецЕсли;
...
Процедура ПредупреждениеОНедопустимомОтчетномПериоде()
	Текст = НСтр("ka='ფორმის წარდგენა ხდება 2014 წლიდან მოყოლებული. წინა პერიოდებისთვის ხდება ფორმა СЗВ-1(2, 4)-ის წარდგენა.';ru='Форма предоставляется, начиная с 2014 г. За предыдущие периоды предоставляется формы СЗВ-1(2, 4).';en='Form is provided, starting from 2014 g. the previous period available forms SCB-1 (2, 4).'");
