...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаСписаниеЗапасов, СтруктураДополнительныеСвойства)
	...
	//|	ЗапасыОстатки.Проект КАК Проект, //Малышев 2024-06-06 ONESOFT-3093
	...
	//|				(Организация, СтруктурнаяЕдиница, СчетУчетаРегл, Номенклатура, Характеристика, Партия, Проект, ЗаказПокупателя) В //Малышев 2024-06-06 ONESOFT-3093
	//|					(ВЫБРАТЬ
	//|						ТаблицаЗапасы.Организация,
	//|						ТаблицаЗапасы.СтруктурнаяЕдиница,
	////|						ТаблицаЗапасы.СчетУчета,
	...
	//|						ТаблицаЗапасы.Характеристика,
	//|						ТаблицаЗапасы.Партия,
	//|						ТаблицаЗапасы.Проект,    //Малышев 2024-06-06 ONESOFT-3093
	//|						ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказПокупателя
	//|					ИЗ
	...
	//|		ЗапасыОстатки.Проект, //Малышев 2024-06-06 ONESOFT-3093
	//|		ЗапасыОстатки.СчетУчетаРегл
	//|	
	//|	ОБЪЕДИНИТЬ ВСЕ
	//|	
	...
	//|		ДвиженияДокументаЗапасы.Проект, //Малышев 2024-06-06 ONESOFT-3093
	//|		ВЫБОР
	...
	//Запрос.УстановитьПараметр("МоментКонтроля", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата, ВидГраницы.Исключая));
	...
	ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСписаниеЗапасов, СтруктураДополнительныеСвойства).Выгрузить(); 
	...
	Для н = 0 По КоличествоСтрок Цикл
		...
		//	СтруктураДляПоиска.Вставить("Проект", СтрокаТаблицаЗапасы.Проект); //Малышев 2024-06-06 ONESOFT-3093
		//КонецЕсли;
		//Малышев 2024-07-27 --)
...
Процедура СформироватьТаблицаКонтрольСроковГодностиТовараЗапасыРасход(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	|	И КонтрольСроковГодностиТовара.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаКонтрольСроковГодностиТовараЗапасыПриход(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|	КонтрольСроковГодностиТовара.Период КАК Период,
	|	КонтрольСроковГодностиТовара.Организация КАК Организация,
	|	ЕСТЬNULL(КонтрольСроковГодностиТовара.СтруктурнаяЕдиница, &Склад) КАК СтруктурнаяЕдиница,
	|	КонтрольСроковГодностиТовара.Номенклатура КАК Номенклатура,
	|	КонтрольСроковГодностиТовара.Характеристика КАК Характеристика,
	|	КонтрольСроковГодностиТовара.Ссылка КАК Документ,
	|	КонтрольСроковГодностиТовара.ВидДвижения КАК ВидДвижения
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК КонтрольСроковГодностиТовара
	|ГДЕ
	|	КонтрольСроковГодностиТовара.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И КонтрольСроковГодностиТовара.ВключенВрегистр = ИСТИНА
	|	И КонтрольСроковГодностиТовара.СрокГодности <> ДАТАВРЕМЯ(1, 1, 1)";
	
	Запрос.УстановитьПараметр("Склад",ДокументСсылка.СтруктурнаяЕдиница);
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСписаниеЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	|	Значение(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказПокупателя,  //Малышев 2024-06-17
	|	ВЫБОР
	...
	//ПодольскийАИ #УчетПроектов 07.09.2021 {
	|	ВЫБОР 
	|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА Значение(Справочник.Проекты.ПустаяССылка) //Малыщев 2024-06-14
	...
	//+Lazare
	|	СписаниеЗапасовЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр,
	...
	//+Lazare
	|	СписаниеЗапасовЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр,
	|	СписаниеЗапасовЗапасы.СрокГодности
	...
	Если СтруктураДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара Тогда 
		СформироватьТаблицаКонтрольСроковГодностиТовараЗапасыРасход(ДокументСсылкаСписаниеЗапасов,СтруктураДополнительныеСвойства);
		СформироватьТаблицаКонтрольСроковГодностиТовараЗапасыПриход(ДокументСсылкаСписаниеЗапасов,СтруктураДополнительныеСвойства);	
	КонецЕсли;
	//-Lazare
