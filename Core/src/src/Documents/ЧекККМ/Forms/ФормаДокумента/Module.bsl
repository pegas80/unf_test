	...
	Если Объект.СуммаДокумента < Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		Если Элементы.Зарезервировать.Видимость И НЕ Элементы.РасчитатьСуммуНаличнойОплаты.Доступность Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + 
				НСтр("ka='გადახდის თანხის შეყვანამდე, უნდა დავაჭიროთ ღილაკ %1-ს.';ru='Прежде чем вводить сумму оплаты, требуется нажать кнопку %1.';en='Before you enter the amount of payment, you must press the button% 1.'");
			Если РезервироватьЗапасыИИспользоватьЗаказы Тогда	
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", НСтр("ka='""Провести""';ru='""Провести""';en='""Conduct""'"));
			Иначе
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", НСтр("ka='""Зарезервировать""';ru='""Зарезервировать""';en='""Make a reservation""'"));
			КонецЕсли;
		КонецЕсли;
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		//Если ЛЕВ(ТекШтрихкод.Штрихкод,1) = "2" Тогда // Kuzmich, 05.03.2019, неправильно находит штрихкод НЕ от весов
		//	ПреобразоватьВесовойШтрихкод(ТекШтрихкод);
		//КонецЕсли;
...
Функция ПроверитьВалютуДокументаОснования(БратьКурсНаТекущуюДату = Ложь)
	...
	Если НЕ Объект.Заказ.Пустая() Тогда
		Если Объект.ВалютаДокумента <> Объект.Заказ.ВалютаДокумента Тогда
			...
			ТекстСообщения = НСтр("ka='საფუძველ-დოკუმენტის ვალუტა (%1) განსხვავდება სსა-ის ჩეკის ვალუტისგან (%2)! თანხები გადათვლილი იქნა!';ru='Валюта документа-основания (%1) отличается от валюты чека ККМ (%2)! Суммы были пересчитаны!';en='Currency paper-base (% 1) is different from the currency of the check CMC (2%)! Amounts have been restated!'");
...
//++Lazare 03.08.2021
&НаСервере
Процедура УстановитьВидимостьКонсультант()	
...
Процедура УстановитьРежимТолькоПросмотр()
	...
	Если КассаККМИспользоватьБезПодключенияОборудования Тогда
		Элементы.ПробитьЧек.Заголовок = НСтр("ka='ჩეკის გაუქმება';ru='Отменить пробитие';en='cancel penetration'");
...
Процедура ОтменитьРежимТолькоПросмотр()
	...
	Элементы.ПробитьЧек.Заголовок = НСтр("ka='ჩეკის ამორტყმა';ru='Пробить чек';en='to break check'");
	ТолькоПросмотр = Ложь;
...
Процедура УстановитьВидимостьРезервов(ОчищатьКолонкуРезерв=Ложь, ОчищатьКолонкуЗаказ=Ложь)
	...
	ВидимостьРезервов = НЕ (ВШапке И Объект.Заказ.Пустая()) И
		...
		(ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") ИЛИ ПолучитьФункциональнуюОпцию("РезервированиеЗапасов_new")) И
		// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
	РезервироватьЗапасыИИспользоватьЗаказы = ((ПолучитьФункциональнуюОпцию("РезервированиеЗапасов")ИЛИ ПолучитьФункциональнуюОпцию("РезервированиеЗапасов_new")) И ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыВРозничнойТорговле"));
	// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
	...
	УстановитьВидимостьКонсультант ();
...
Процедура ПриОткрытии(Отказ)
	...
	//ЗапретРедактированияДокументовОператоров();
КонецПроцедуры // ПриОткрытии()

&НаСервере
Процедура ЗапретРедактированияДокументовОператоров()
	
	НовыйGUID = Новый УникальныйИдентификатор("76337576-bff4-11df-9174-e0cb4ed5f4c3");
	...
	|ГДЕ
	|	Профили.ИдентификаторПоставляемыхДанных = &НовыйGUID";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если КОнстанты.ЗапретРедактированияДокументовОператоров.Получить() = Истина
			...
			И ПараметрыСеанса.ТекущийПользователь <> Объект.Автор    Тогда
...
Процедура КассаККМПриИзменении(Элемент)
	...
	Если НЕ СменаОткрыта Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='სალაროს ცვლა არ არის გახსნილი. ცვლის გახსნა შეიძლება ჟურნალ ""სსა-ის ჩეკები""-ში. ცვლის გახსნის შემდეგ, მიმდინარე დოკუმენტში ხელახლა აირჩიეთ სსა სალარო.';ru='Кассовая смена не открыта. Открыть смену можно в журнале ""Чеки ККМ"". После открытия смены перевыберите кассу ККМ в текущем документе.';en='Cash change is not open. Open the change in magazine ""Checks CMC"". After opening the cash change perevyberite CMC in the current document.'");
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если НЕ Объект.Заказ.Пустая() Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещения, 
			НСтр("ka='შევავსოთ დოკუმენტი მყიდველის მიერ არჩეული შეკვეთით?';ru='Заполнить документ по выбранному заказу покупателя?';en='Complete the document on the selected customer request?'"),
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент)
	...
	//+Lazare
	СтрокаТабличнойЧасти.СпособОкругления = СтруктураДанные.СпособОкругления;
	//-Lazare
...
Процедура ОплатаПлатежнымиКартамиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		ПодставитьДанныеКарты(Элементы.ОплатаПлатежнымиКартами.ТекущаяСтрока);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПодставитьДанныеКарты(Идентификатор)
	
	пЭквайринговыйТерминал = Объект.ЭквайринговыйТерминал;
	ТекСтрока = Объект.ОплатаПлатежнымиКартами.НайтиПоИдентификатору(Идентификатор);
	ТекСтрока.ВидПлатежнойКарты		= ?(пЭквайринговыйТерминал.ВидыПлатежныхКарт.Количество()>0, пЭквайринговыйТерминал.ВидыПлатежныхКарт[0].ВидПлатежнойКарты, "");
	ТекСтрока.НомерПлатежнойКарты	= пЭквайринговыйТерминал.Код;
	
КонецПроцедуры

//Kuzmich, 13.02.2019, Проблемы с выгрузкой штучного товара
&НаСервереБезКонтекста
Функция ПреобразоватьВесовойШтрихкод(ТекШтрихкод)
	РегистрыСведений.ШтрихкодыНоменклатуры.ПреобразоватьВесовойШтрихкод(ТекШтрихкод);
КонецФункции
//Kuzmich, 13.02.2019, Проблемы с выгрузкой штучного товара
