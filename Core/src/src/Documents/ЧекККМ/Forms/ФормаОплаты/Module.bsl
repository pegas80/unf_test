...
Процедура ЗаполнитьДанныеКарты(Инд = Неопределено)
	...
	Если Инд = Неопределено Тогда
		Для Каждого СтрокаТЧ2 ИЗ ВременнаяТаблицаКарт Цикл
			Если пЭквайринговыйТерминал.ВидыПлатежныхКарт.Найти(СтрокаТЧ2.ВидПлатежнойКарты,"ВидПлатежнойКарты") = Неопределено Тогда
				СтрокаТЧ2.ВидПлатежнойКарты		= пВидПлатежнойКарты;
				СтрокаТЧ2.НомерПлатежнойКарты	= пЭквайринговыйТерминал.Код;
			КонецЕсли;
		КонецЦикла;
	Иначе
...
Процедура НажатиеКнопкиКомандаТерминал(Команда)
	...
	СтруктураТерминала = ПолучитьСтруктуруТерминала(Команда.Имя);
...
&НаСервере
Функция ПолучитьСтруктуруТерминала(ИмяКоманды)
