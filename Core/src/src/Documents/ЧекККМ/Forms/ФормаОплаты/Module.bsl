...
//
&НаКлиенте
Процедура ПересчитатьСуммы(ТребуетсяСписокВыбора = Истина)
	
	ОплатаКартами = ВременнаяТаблицаКарт.Итог("Сумма");
	//СуммаСдачи = ПолученоНаличными + ОплатаКартами - СуммаДокумента;  //+++ Дмгг
	СуммаСдачи = ПолученоНаличными + ОплатаКартами - СуммаДокумента + СООплатаКартой + ОплатыСуммаНаличные; // Дмгг
	// Сумма сдачи не должна быть больше оплаты наличными.
	// Если это так, то возвращать нужно на карту.
	Если СуммаСдачи > (ПолученоНаличными + ОплатыСуммаНаличные) Тогда
	...
	Для Каждого ТекущаяСтрока Из ВременнаяТаблицаКарт Цикл
		...
		Элементы["ОплатаКартойСО_"+Индекс].СписокВыбора.Очистить();
		Если Дельта > 0 //Тогда
			И ТребуетсяСписокВыбора Тогда
...
&НаСервере
Процедура УстановитьВидимостьСмешаннойОплаты()

	Элементы.ГруппаСмешаннаяОплата.Видимость = ДоступностьСмешаннойОплаты;	

КонецПроцедуры // УстановитьВидимостьСмешаннойОплаты()()

&НаСервере
Процедура УстановитьВидимостьПриновойФормеОплат()

	Элементы.ГруппаНоваяФормаОплаты.Видимость = ИспользоватьНовуюФормуОплат;
	Элементы.ГруппаСмешаннаяОплата.Видимость = НЕ ИспользоватьНовуюФормуОплат;
	Элементы.ГруппаНаличнаяОплата.Видимость = НЕ ИспользоватьНовуюФормуОплат;
	Элементы.ГруппаБезналичнаяОплата.Видимость = НЕ ИспользоватьНовуюФормуОплат;
		...
		Если Параметры.ИспользоватьНовуюФормуОплат Тогда 
			Если Параметры.Свойство("ДоступныеТерминалы") Тогда 
				...
				Для Каждого Элем Из Параметры.ДоступныеТерминалы Цикл 
					...
					//Малышев 2025-05-27 ONESOFT-4097
					СтатусПодсказки = "";
					Если ТерминалОнлайн(Элем) Тогда
						Кмд.Картинка = БиблиотекаКартинок.БольшаяЖелтаяЗвезда;
						СтатусПодсказки = "ONLINE";
					Иначе     
						Кмд.Картинка = БиблиотекаКартинок.БольшаяЖелтаяЗвезда;
						СтатусПодсказки = "OFFLINE";
					КонецЕсли;       
					Кмд.Подсказка = Элем.Наименование + " - " + СтатусПодсказки;
					...
					Элемент2.Ширина = 30;
					Элемент2.Высота = 2;
					Элемент2.ПоложениеКартинки = ПоложениеКартинкиКнопкиФормы.Лево;  //Малышев 2025-05-27 ONESOFT-4097
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИспользоватьНовуюФормуОплат Тогда 
		ТекущееПолеВводаСуммы = Элементы.ОплатыСуммаНаличные;
		ГруппаОплатаПриСменеСтраницы(Элементы.ГруппаОплата, Элементы.ГруппаНоваяФормаОплаты);
	КонецЕсли;
...
Процедура ДобавитьКартуНаСервере(ТекущаяСтрокаИдентификатор = Неопределено)
	...
	Если ТекущаяСтрокаИдентификатор = Неопределено Тогда
		...
		//Паганини_
		НоваяСтрока.ВидПлатежнойКарты	= пВидПлатежнойКарты;
		НоваяСтрока.НомерПлатежнойКарты	= пНомерПлатежнойКарты;
		//_Паганини
	...
	//СмешаннаяОплпта
	///////////////////////////////////////////////////////////////////////////
	НоваяГруппа = Элементы.Добавить("ГруппаОплатаКартойСО_"+Индекс, Тип("ГруппаФормы"), Элементы.ГруппаСОКарты);
	НоваяГруппа.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппа.ОтображатьЗаголовок = Ложь;
	НоваяГруппа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	НоваяГруппа.ЦветФона = ЦветФонаГруппы;
	...
	НоваяГруппаСуммаКарты = Элементы.Добавить("ГруппаОплатаКартойСуммаСО_"+Индекс, Тип("ГруппаФормы"), НоваяГруппа);
	НоваяГруппаСуммаКарты.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппаСуммаКарты.ОтображатьЗаголовок = Ложь;
	НоваяГруппаСуммаКарты.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
	НоваяГруппаСуммаКарты.ЦветФона = ЦветФонаГруппы;
	
	НоваяГруппаГоризонтальная = Элементы.Добавить("ГруппаОплатаКартойГоризонтальнаяСО_"+Индекс, Тип("ГруппаФормы"), Элементы.ГруппаСОКарты);
	НоваяГруппаГоризонтальная.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппаГоризонтальная.ОтображатьЗаголовок = Ложь;
	НоваяГруппаГоризонтальная.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
	НоваяГруппаГоризонтальная.ТолькоПросмотр = ЗначениеЗаполнено(ЭквайринговыйТерминал);
	
	НоваяГруппаРеквизитыКартыЧасть1 = Элементы.Добавить("ГруппаРеквизитыКартыЧасть1СО"+Индекс, Тип("ГруппаФормы"), НоваяГруппаГоризонтальная);
	НоваяГруппаРеквизитыКартыЧасть1.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппаРеквизитыКартыЧасть1.ОтображатьЗаголовок = Ложь;
	НоваяГруппаРеквизитыКартыЧасть1.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	НоваяГруппаРеквизитыКартыЧасть1.ЦветФона = ЦветФонаГруппы;
	
	НоваяГруппаРеквизитыКартыЧасть2 = Элементы.Добавить("ГруппаРеквизитыКартыЧасть2СО"+Индекс, Тип("ГруппаФормы"), НоваяГруппаГоризонтальная);
	НоваяГруппаРеквизитыКартыЧасть2.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппаРеквизитыКартыЧасть2.ОтображатьЗаголовок = Ложь;
	НоваяГруппаРеквизитыКартыЧасть2.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	НоваяГруппаРеквизитыКартыЧасть2.ЦветФона = ЦветФонаГруппы;
	...
	// НадписьУдаленияОплатыКартой
	ЭлементНадписьОбУдалении = Элементы.Добавить("НадписьОбУдаленииОплатыКартойСО_"+Индекс, Тип("ДекорацияФормы"), НоваяГруппаСуммаКарты);
	ЭлементНадписьОбУдалении.Вид = ВидДекорацииФормы.Надпись;
	ЭлементНадписьОбУдалении.Заголовок = "";
	ЭлементНадписьОбУдалении.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	ЭлементНадписьОбУдалении.ЦветТекста = Новый Цвет(255, 0, 0);
	...
	НовыйЭлемент = Элементы.Добавить("ОплатаКартойСО_"+Индекс, Тип("ПолеФормы"), НоваяГруппаСуммаКарты);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "ВременнаяТаблицаКарт["+Индекс+"].Сумма";
	НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	НовыйЭлемент.Заголовок = НСтр("ru = 'Оплата картой'; ka ='ბარათით გადახდა'");
	НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	НовыйЭлемент.Ширина = 35;
	НовыйЭлемент.РастягиватьПоГоризонтали = Ложь;
	НовыйЭлемент.КнопкаВыбора = Истина;
	...
	НовыйЭлемент.Подсказка = "Калькулятор изменяет данные в нижнем поле ""Сумма"" или в последнем изменённом поле ""Сумма"".
		|Если нужно привязать калькулятор к другому полю, то нажмите в этом поле кнопку с изображением калькулятора.";
	НовыйЭлемент.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	//НовыйЭлемент.УстановитьДействие("ПриИзменении", "ОплатаКартойСО_ПриИзменении");
	...
	НовыйЭлемент = Элементы.Добавить("ВалютаДокументаСО_"+Индекс, Тип("ПолеФормы"), НоваяГруппаСуммаКарты);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеНадписи;
	НовыйЭлемент.ПутьКДанным = "ВалютаДокумента";
	НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	...
	// КнопкаУдаленияОплатыКартой
	Если ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		ИмяКнопки = "КнопкаУдаленияОплатыСО_"+Индекс;
		
		НоваяКоманда = ЭтаФорма.Команды.Добавить(ИмяКнопки);
		НоваяКоманда.Действие = "ВыбратьКартуДляУдаленияОплатыСО";
		НоваяКоманда.Заголовок = "";
		НоваяКоманда.Отображение = ОтображениеКнопки.Картинка;
		НоваяКоманда.Картинка = БиблиотекаКартинок.ПометитьНаУдаление;
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
	...
	//НовыйЭлемент = Элементы.Добавить("ВидПлатежнойКартыСО_"+Индекс, Тип("ПолеФормы"), НоваяГруппаРеквизитыКартыЧасть1);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.ПутьКДанным = "ВременнаяТаблицаКарт["+Индекс+"].ВидПлатежнойКарты";
	//НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	//НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	...
	//НовыйЭлемент.УстановитьДействие("ПриИзменении", "РеквизитКартыПриИзменении");
	//
	//НовыйЭлемент = Элементы.Добавить("НомерПлатежнойКартыСО_"+Индекс, Тип("ПолеФормы"), НоваяГруппаРеквизитыКартыЧасть2);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.ПутьКДанным = "ВременнаяТаблицаКарт["+Индекс+"].НомерПлатежнойКарты";
	//НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	//НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	...
	//НовыйЭлемент.УстановитьДействие("ПриИзменении", "РеквизитКартыПриИзменении");
	//
	//НовыйЭлемент = Элементы.Добавить("НомерЧекаЭТСО_"+Индекс, Тип("ПолеФормы"), НоваяГруппаРеквизитыКартыЧасть1);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.ПутьКДанным = "ВременнаяТаблицаКарт["+Индекс+"].НомерЧекаЭТ";
	//НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	//НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	...
	//НовыйЭлемент.УстановитьДействие("ПриИзменении", "РеквизитКартыПриИзменении");
	//
	//НовыйЭлемент = Элементы.Добавить("СсылочныйНомерСО_"+Индекс, Тип("ПолеФормы"), НоваяГруппаРеквизитыКартыЧасть2);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.ПутьКДанным = "ВременнаяТаблицаКарт["+Индекс+"].СсылочныйНомер";
	//НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	//НовыйЭлемент.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	...
	СООплатаКартой = 0;
	Элементы.ГруппаКартыБезТерминала.Видимость = ЛОЖЬ;
...
Процедура ДобавитьЦифру(ВведеннаяЦифраСтрокой)
	...
	//+++ Дмгг
	ИначеЕсли Элементы.ГруппаОплата.ТекущаяСтраница = Элементы.ГруппаНоваяФормаОплаты Тогда
		ВводимоеЧисло = ПолученоНаличнымиСтрока;
		ТекПервыйВвод = ПервыйВвод;
	//--- Дмгг	
	Иначе
	...
	ИначеЕсли Элементы.ГруппаОплата.ТекущаяСтраница = Элементы.ГруппаНоваяФормаОплаты Тогда
		//ПолученоНаличными = ЧислоВвода;
		ОплатыСуммаНаличные = ЧислоВвода;
		ПолученоНаличнымиСтрока = ВводимоеЧисло;
		ПервыйВвод = ТекПервыйВвод; 
	Иначе
...
Процедура ОК(Команда)
	...
	Если Объект.СуммаДокумента > ПолученоНаличными + ОплатыСуммаНаличные + Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
	...
	ПараметрыЗакрытия = Новый Структура("Объект, Наличными, ПлатежнымиКартами, Сдача, СформироватьТоварныйЧек, ОплатаПлатежнымиКартами, ЗаписатьДокумент, Кнопка", 
		Объект,
		//ПолученоНаличными, 
		ПолученоНаличными + ОплатыСуммаНаличные,
	...
	//Малышев 2025-05-27 ONESOFT-4097((--
	СтруктураТерминала = ПолучитьСтруктуруТерминала();
	ИдентификаторУстройстваЭТ = ?(
		ЗначениеЗаполнено(СтруктураТерминала.Терминал),
		СтруктураТерминала.Терминал,
		Неопределено
	);
	
	// Выбор устройства ФР
	ИдентификаторУстройстваФР = ?(
		ЗначениеЗаполнено(СтруктураТерминала.ПринтерЧеков),
		СтруктураТерминала.ПринтерЧеков,
		Неопределено
	);                    
	
	Если ЗначениеЗаполнено(СтруктураТерминала.Терминал) и не СтруктураТерминала.ИспользоватьБезПодключенияОборудования Тогда //Малышев 2025-06-21 ONESOFT-4172 

		Дельта = СуммаДокумента - ОплатыСуммаНаличные;
		НомерКарты = "";
		СтруктураРезультат = Новый Структура();
		СтруктураРезультат.Вставить("Сумма"         , Дельта);
		СтруктураРезультат.Вставить("ДанныеКарты"   , "");
		СтруктураРезультат.Вставить("СсылочныйНомер", "");
		СтруктураРезультат.Вставить("ТипКарты"      , 0);
		СтруктураРезультат.Вставить("НомерКарты"    , "");
		СтруктураРезультат.Вставить("НомерЧека"     , "");
		
		СтруктураДопПараметров = Новый Структура("ИдентификаторУстройстваФР, ИдентификаторУстройстваЭТ, НомерКарты, ЭтоНоваяФормаОплаты", ИдентификаторУстройстваФР, ИдентификаторУстройстваЭТ, НомерКарты, Истина);
		
		Если Дельта>0 Тогда
			ДобавитьОплатуКартойЗавершение(СтруктураРезультат, СтруктураДопПараметров);
		КонецЕсли;	
...
Процедура ДокументЧекККМПолученоНаличнымиПриИзменении(Элемент)
	//+++ Дмгг
	Если Элементы.ГруппаОплата.ТекущаяСтраница = Элементы.ГруппаНоваяФормаОплаты Тогда
		ПолученоНаличнымиСтрока = ОплатыСуммаНаличные;
	КонецЕсли;
	//--- Дмгг
	ПересчитатьСуммы();
...
Процедура РеквизитКартыПриИзменении(Элемент)
	...
	КоличествоКартДляЗаголовка = КоличествоКарт;
	//Паганини_
	Если Лев(Элемент.Имя, 17) = "ВидПлатежнойКарты" Тогда
		ИндексСтроки = Число(Прав(Элемент.Имя, СтрДлина(Элемент.Имя) - 18));
		ЗаполнитьДанныеКарты(ИндексСтроки);
	КонецЕсли;
	//_Паганини
...
Процедура ГруппаОплатаПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.ГруппаНаличнаяОплата Тогда
		...
		Элементы.Сбросить.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.BackSpace, Ложь, Истина, Ложь);
	//Иначе
	ИначеЕсли ТекущаяСтраница = Элементы.ГруппаБезналичнаяОплата Тогда
		Для Сч = 0 По 9 Цикл
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ГруппаБезналичнаяОплата Тогда
		Для Сч = 0 По 9 Цикл
			Элементы["Кнопка"+Сч+"_3"].СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
		КонецЦикла;
		Элементы.РазделительДробнойЧасти_3.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
		Элементы.Сбросить_3.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
		
	ИначеЕсли ТекущаяСтраница = Элементы.ГруппаНоваяФормаОплаты Тогда
		Для Сч = 0 По 9 Цикл
			Элементы["Кнопка"+Сч+"_4"].СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
		КонецЦикла;
		Элементы.РазделительДробнойЧасти_4.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
		Элементы.Сбросить_4.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
	...
	//+++ Дмгг
	Если ДоступностьСмешаннойОплаты Тогда 
		ТребуетсяСписокВыбора = Истина;
		Если ТекущаяСтраница = Элементы.ГруппаНаличнаяОплата Тогда
			Если ВременнаяТаблицаКарт.Количество() > 0 Тогда 
				ВременнаяТаблицаКарт[0].Сумма = 0;
			КонецЕсли;
			СООплатаКартой = 0;
			ПолученоНаличными = СуммаДокумента;
			Элементы.ГруппаПринимаемаяОплатаСО.Видимость = ЛОЖЬ;
			ПринимаемаяСуммаСО = 0;
		ИначеЕсли ТекущаяСтраница = Элементы.ГруппаБезналичнаяОплата Тогда
			ПолученоНаличными = 0;
			СООплатаКартой = 0;
			Если ВременнаяТаблицаКарт.Количество() > 0 Тогда
				ВременнаяТаблицаКарт[0].Сумма = СуммаДокумента;
			КонецЕсли;
			Элементы.ГруппаПринимаемаяОплатаСО.Видимость = ЛОЖЬ;
			ПринимаемаяСуммаСО = 0;
		ИначеЕсли ТекущаяСтраница = Элементы.ГруппаСмешаннаяОплата Тогда
			Если ВременнаяТаблицаКарт.Количество() > 0 Тогда
				ВременнаяТаблицаКарт[0].Сумма = 0;
			КонецЕсли;
			ПолученоНаличными = 0;
			Элементы.ГруппаПринимаемаяОплатаСО.Видимость = ИСТИНА;
			ПринимаемаяСуммаСО = 0;
			ТребуетсяСписокВыбора = ЛОЖЬ;
		ИначеЕсли ТекущаяСтраница = Элементы.ГруппаНоваяФормаОплаты Тогда
			Если ВременнаяТаблицаКарт.Количество() > 0 Тогда
				ВременнаяТаблицаКарт[0].Сумма = 0;
			КонецЕсли;
			...
			ТребуетсяСписокВыбора = ЛОЖЬ;
		КонецЕсли;
		ПересчитатьСуммы(ТребуетсяСписокВыбора);
	КонецЕсли;
	//--- Дмгг
...
Процедура ДобавитьОплатуКартой()
	...
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтаФорма, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='საგადახდო ტერმინალით გადახდა ვერ შესრულდა';ru='Не удалось выполнить оплату через платежный терминал';en='It failed to make a payment through the payment terminal'"));
		Возврат;
	КонецЕсли;
	...
	Если ИдентификаторУстройстваЭТ <> Неопределено 
		...
		//+++ Дмгг
		//Дельта = СуммаДокумента - ПолученоНаличными - ОплатаКартами;
		Если СООплатаКартой <> 0 Тогда 
			Дельта = СООплатаКартой;
		Иначе
			Дельта = СуммаДокумента - ПолученоНаличными - ОплатаКартами;
		КонецЕсли;
		//--- Дмгг
...
Процедура ДобавитьОплатуКартойЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если НЕ ИспользоватьНовуюФормуОплат Тогда 
			Если ВидыПлатежныхКарт.Количество()>0 Тогда
				ВидКарты = ВидыПлатежныхКарт[Результат.ТипКарты].Значение;
			КонецЕсли;	
		Иначе
			СтруктураЭвкайринговогоТерминала = НайтиВидКарты(ИдентификаторУстройстваЭТ);
			ВидКарты = СтруктураЭвкайринговогоТерминала.ВидКарты;
			ЭквайринговыйТерминал = СтруктураЭвкайринговогоТерминала.ЭквайринговыйТерминал;
		КонецЕсли;
		//--- Дмгг
		...
		ДополнительныеПараметры.Вставить("ЭквайринговыйТерминал", ЭквайринговыйТерминал);  //+++ Дмгг
...
Процедура ОперацияПоПлатежнойКартеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если Параметры.Свойство("ЭтоПечатьОшибки") ИЛИ РезультатВыполнения.Свойство("ЭтоПечатьОшибки") Тогда 
			//ПараметрыЗакрытия = Новый Структура("Объект, Наличными, ПлатежнымиКартами, Сдача, СформироватьТоварныйЧек, ОплатаПлатежнымиКартами, ЗаписатьДокумент, Кнопка", 
			//	Объект,
			//	//ПолученоНаличными, 
			//	ПолученоНаличными + ОплатыСуммаНаличные,
		...
		//+++ Дмгг
		Если ИспользоватьНовуюФормуОплат И ЗначениеЗаполнено(Параметры.ЭквайринговыйТерминал) Тогда
			Объект.ЭквайринговыйТерминал = Параметры.ЭквайринговыйТерминал;
			//ИзменитьЭквайринговыйТерминалВЧекеККМНаСервере(Параметры.ЭквайринговыйТерминал);
		ИначеЕсли ИспользоватьНовуюФормуОплат И ЗначениеЗаполнено(Параметры.ВидКарты) Тогда
			Объект.ЭквайринговыйТерминал = НайтиПоНаименованиюНаСервере(Параметры.ВидКарты);
		КонецЕсли; //                                                  
		//--- Дмгг
	...
	Иначе
		//+++ Дмгг
		Если ИспользоватьНовуюФормуОплат 
			//--------------------------------------------------------------------------
			// © Шишка В.В. [13 октября 2021]  НАЧАЛО
			И Параметры.Свойство("ЭквайринговыйТерминал")	
			// © Шишка В.В. [13 октября 2021]  КОНЕЦ
			//--------------------------------------------------------------------------
			И ЗначениеЗаполнено(Параметры.ЭквайринговыйТерминал) Тогда 
			Объект.ЭквайринговыйТерминал = Параметры.ЭквайринговыйТерминал;
			//ИзменитьЭквайринговыйТерминалВЧекеККМНаСервере(Параметры.ЭквайринговыйТерминал);
		КонецЕсли;
		//--- Дмгг
		ТекстСообщения = НСтр("ka='При выполнении операции произошла ошибка:""%ОписаниеОшибки%"".';ru='При выполнении операции произошла ошибка:""%ОписаниеОшибки%"".';en='In operation, an error occurred: ""% OpisanieOshibki%.""'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", РезультатВыполнения.ОписаниеОшибки);
		Сообщить(ТекстСообщения);
		
		//Если ИспользоватьНовуюФормуОплат Тогда
		//	Отменить(Элементы.ФормаОтменить);
		//КонецЕсли;
...
Процедура УдалитьОплатуКартойПослеВыбораКарты(ТекущиеДанные)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ წასაშლელი ბარათით გადახდის სტრიქონი';ru='Выберите строку удаляемой оплаты картой.';en='Select the row to delete a payment card.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтаФорма, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='ბარათით გადახდის გაუქმება ვერ მოხერხდა.';ru='Не удалось выполнить отмену оплаты картой.';en='Failed to execute the cancellation of payment cards.'"));
		Возврат;
	КонецЕсли;
...
Функция НайтиПоНаименованиюНаСервере(Наименование)
	...
	ЭквТерминалВозврат = Неопределено;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЭквайринговыеТерминалыВидыПлатежныхКарт.ВидПлатежнойКарты = &ВидПлатежнойКарты
		|	И ЭквайринговыеТерминалыВидыПлатежныхКарт.Ссылка.Касса = &Касса";
	
	Запрос.УстановитьПараметр("ВидПлатежнойКарты", Наименование);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЭквТерминалВозврат = ВыборкаДетальныеЗаписи.Ссылка;
	КонецЦикла;
	
	Возврат ЭквТерминалВозврат;	

КонецФункции // НайтиПоНаименованиюНаСервере()
...
&НаКлиенте
Процедура ВыбратьКартуДляУдаленияОплатыСО(Команда)
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		ДополнительныеПараметры = Новый Структура("СтрокаТаблицы", НайденныеСтроки[0]);
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыбратьКартуДляУдаленияОплатыЗавершениеСО", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(ОписаниеОповещения, "Отменить оплату картой", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
&НаКлиенте
Процедура ВыбратьКартуДляУдаленияОплатыЗавершениеСО(Результат, ДополнительныеПараметры) Экспорт
...
//Паганини_
&НаСервере
Процедура ЗаполнитьДанныеКарты(Инд = Неопределено)
	
	ЭквайринговыйТерминал = Объект.ЭквайринговыйТерминал;
	Если ЭквайринговыйТерминал.ВидыПлатежныхКарт.Количество() > 0 Тогда
		пВидПлатежнойКарты		= ЭквайринговыйТерминал.ВидыПлатежныхКарт[0].ВидПлатежнойКарты;
		пНомерПлатежнойКарты	= ЭквайринговыйТерминал.Код;
	Иначе
		пВидПлатежнойКарты 		= "";
		пНомерПлатежнойКарты	= ЭквайринговыйТерминал.Код;
	КонецЕсли;
	Если Инд = Неопределено Тогда
		Для Каждого СтрокаТЧ2 ИЗ ВременнаяТаблицаКарт Цикл
			Если ЭквайринговыйТерминал.ВидыПлатежныхКарт.Найти(СтрокаТЧ2.ВидПлатежнойКарты,"ВидПлатежнойКарты") = Неопределено Тогда
				СтрокаТЧ2.ВидПлатежнойКарты		= пВидПлатежнойКарты;
				СтрокаТЧ2.НомерПлатежнойКарты	= ЭквайринговыйТерминал.Код;
			КонецЕсли;
		КонецЦикла;
	Иначе
		ВременнаяТаблицаКарт[Инд].НомерПлатежнойКарты	= пНомерПлатежнойКарты;
	КонецЕсли;
	
	//Малышев 2025-05-27 ONESOFT-4097
	Если ТерминалОнлайн(Объект.ЭквайринговыйТерминал) 
	Тогда
		Элементы.ДекорацияСтатусТерминала.Картинка = БиблиотекаКартинок.БольшаяЖелтаяЗвезда;
		Элементы.ДекорацияСтатусТерминала1.Картинка = БиблиотекаКартинок.БольшаяЖелтаяЗвезда; 
		Элементы.ДекорацияСтатусТерминала.Подсказка = "ONLINE";
		Элементы.ДекорацияСтатусТерминала1.Подсказка = "ONLINE";
		
	Иначе         
		Элементы.ДекорацияСтатусТерминала.Картинка = БиблиотекаКартинок.БольшаяЖелтаяЗвезда;
		Элементы.ДекорацияСтатусТерминала1.Картинка = БиблиотекаКартинок.БольшаяЖелтаяЗвезда;
		Элементы.ДекорацияСтатусТерминала.Подсказка = "OFFLINE";
		Элементы.ДекорацияСтатусТерминала1.Подсказка = "OFFLINE";
	КонецЕсли;
...
Функция ТерминалОнлайн(ЭквайринговыйТерминал)
	
	Если ЗначениеЗаполнено(ЭквайринговыйТерминал) и 
		ЗначениеЗаполнено(ЭквайринговыйТерминал.ПодключаемоеОборудование) и 
		ЭквайринговыйТерминал.ПодключаемоеОборудование.УстройствоИспользуется
	Тогда 
		Возврат Истина;
	Иначе
		Возврат Ложь;  
...
КонецФункции // ТерминалОнлайн()


&НаСервере
Процедура ИзменениеСуммыСмешанойОплатыНаСкрвере()
	
	Если Элементы.ГруппаКартыБезТерминала.Видимость Тогда 
		ПринимаемаяСуммаСО = ПолученоНаличными + СООплатаКартой;
	Иначе
		ПринимаемаяСуммаСО = ПолученоНаличными + ВременнаяТаблицаКарт[0].Сумма;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОплатаКартойСО_ПриИзменении(Элемент)
	ИзменениеСуммыСмешанойОплатыНаСкрвере();
	ПересчитатьСуммы(Ложь);
КонецПроцедуры

&НаКлиенте
Процедура СОНаличныеПриИзменении(Элемент)
	ИзменениеСуммыСмешанойОплатыНаСкрвере();
	ПересчитатьСуммы(Ложь);
КонецПроцедуры

&НаКлиенте
Процедура СООплатаКартойПриИзменении(Элемент)
...
//+++ Дмгг
&НаКлиенте
Процедура НажатиеКнопкиКомандаТерминал(Команда)
	
	СтруктураТерминала = ПолучитьСтруктуруТерминалаПоКоманде(Команда.Имя);
	
	Объект.ЭквайринговыйТерминал = СтруктураТерминала.ЭквайринговыйТерминал;
	...
	Если СтруктураТерминала.ИспользоватьБезПодключенияОборудования И ЗначениеЗаполнено(СтруктураТерминала.ЭквайринговыйТерминал)
		И ВременнаяТаблицаКарт.Количество()
		И ЭтотОбъект.СуммаДокумента > 0
		Тогда
		
		ЭквайринговыйТерминал = СтруктураТерминала.ЭквайринговыйТерминал;
		ВременнаяТаблицаКарт[0].Сумма = ЭтотОбъект.СуммаДокумента;
		СуммаСдачи = 0; //Малышев 2025-10-02 ONESOFT-4381
		ОК(Команда);
		Возврат;
		
	ИначеЕсли СтруктураТерминала.ИспользоватьБезПодключенияОборудования И ЗначениеЗаполнено(СтруктураТерминала.ЭквайринговыйТерминал)
		И НЕ ВременнаяТаблицаКарт.Количество()
		И ЭтотОбъект.СуммаДокумента > 0
		Тогда
		НовСтрок = ВременнаяТаблицаКарт.Добавить();
		ЭквайринговыйТерминал = СтруктураТерминала.ЭквайринговыйТерминал;
		ВременнаяТаблицаКарт[0].Сумма = ЭтотОбъект.СуммаДокумента;
		ВременнаяТаблицаКарт[0].ВидПлатежнойКарты = СтруктураТерминала.ЭквайринговыйТерминал;
		ВременнаяТаблицаКарт[0].НомерПлатежнойКарты = 1234567890123456;
		СуммаСдачи = 0; //Малышев 2025-10-02 ONESOFT-4381
		ОК(Команда);
		Возврат;
	...
	// Выбор устройства ЭТ
	Если СтруктураТерминала.Терминал = Неопределено Тогда 
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Терминал неопределен';ru='Терминал неопределен';en='Терминал неопределен'"));
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьСтруктуруТерминалаПоКоманде(ИмяКоманды)

	//--------------------------------------------------------------------------
	// © Шишка В.В. [12 октября 2021]  НАЧАЛО
	//СтруктураВозвратка = Новый Структура("Терминал, ПринтерЧеков", Неопределено, Неопределено);
	СтруктураВозвратка = Новый Структура("Терминал, ПринтерЧеков, ЭквайринговыйТерминал", Неопределено, Неопределено, Неопределено, Ложь);
	// © Шишка В.В. [12 октября 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
	
	ПоискСтроки = ТаблицаКнопокТерминалов.НайтиСтроки(Новый Структура("ИмяКнопки", ИмяКоманды));
	
	Если ПоискСтроки.Количество() > 0 Тогда 
		
		СтруктураВозвратка.Терминал = ПоискСтроки[0].ЭквайринговыйТерминал.ПодключаемоеОборудование;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [12 октября 2021]  НАЧАЛО
		СтруктураВозвратка.Вставить("ИспользоватьБезПодключенияОборудования", ПоискСтроки[0].ЭквайринговыйТерминал.ИспользоватьБезПодключенияОборудования);	
		СтруктураВозвратка.Вставить("ЭквайринговыйТерминал", ПоискСтроки[0].ЭквайринговыйТерминал);	
		// © Шишка В.В. [12 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		
		Если ПоискСтроки[0].ЭквайринговыйТерминал.ИспользовыатьВнешнееПечатноеОборудование Тогда 
			СтруктураВозвратка.ПринтерЧеков = ПоискСтроки[0].ЭквайринговыйТерминал.ПечатноеУстройство;
		Иначе
			//СтруктураВозвратка.ПринтерЧеков = Неопределено;
			СтруктураВозвратка.ПринтерЧеков = ПоискСтроки[0].ЭквайринговыйТерминал.ПодключаемоеОборудование;
		КонецЕсли;
	КонецЕсли;
	
	Возврат СтруктураВозвратка;

КонецФункции // ПолучитьСтруктуруТерминала(Команда.Имя)()

&НаСервере
Функция ПолучитьСтруктуруТерминала()

	СтруктураВозвратка = Новый Структура("Терминал, ПринтерЧеков, ЭквайринговыйТерминал", Неопределено, Неопределено, Неопределено, Ложь);
	
	Если ЗначениеЗаполнено(Объект.ЭквайринговыйТерминал) Тогда 
		
		СтруктураВозвратка.Терминал = Объект.ЭквайринговыйТерминал.ПодключаемоеОборудование;
		СтруктураВозвратка.Вставить("ИспользоватьБезПодключенияОборудования", Объект.ЭквайринговыйТерминал.ИспользоватьБезПодключенияОборудования);	
		СтруктураВозвратка.Вставить("ЭквайринговыйТерминал", Объект.ЭквайринговыйТерминал);	
		
		Если Объект.ЭквайринговыйТерминал.ИспользовыатьВнешнееПечатноеОборудование Тогда 
			СтруктураВозвратка.ПринтерЧеков = Объект.ЭквайринговыйТерминал.ПечатноеУстройство;
		Иначе
			СтруктураВозвратка.ПринтерЧеков = Объект.ЭквайринговыйТерминал.ПодключаемоеОборудование;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция НайтиВидКарты(ИдентификаторУстройстваЭТ)
	...
	СтруктураВозврата = Новый Структура("ВидКарты, ЭквайринговыйТерминал");
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СтруктураВозврата.ВидКарты = ВыборкаДетальныеЗаписи.ВидПлатежнойКарты;
		СтруктураВозврата.ЭквайринговыйТерминал = ВыборкаДетальныеЗаписи.ЭквайринговыйТерминал;
	КонецЦикла;
...
&НаСервере
Процедура ИзменитьЭквайринговыйТерминалВЧекеККМНаСервере(ЭквТерминал)
	
	ДокЧекККМОбъект = Объект.Ссылка.ПолучитьОбъект();
	
	ДокЧекККМОбъект.ЭквайринговыйТерминал = ЭквТерминал;
	
	ДокЧекККМОбъект.Записать(РежимЗаписиДокумента.Запись);
