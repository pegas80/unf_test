...
Процедура СформироватьЗаголовок(СтруктураСостояниеКассовойСмены)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ka='%ТорговаяТочка%, ცვლა № %НомерСмены%  %ВремяИзменения%';ru='%ТорговаяТочка%, Смена № %НомерСмены%  %ВремяИзменения%';en='TorgovayaTochka%% Change NomerSmeny №%%%% VremyaIzmeneniya'");
	...
	Иначе
		ТекстСообщения = НСтр("ka='%ТорговаяТочка%';ru='%ТорговаяТочка%';en='%A store%'");
...
"Смена не открыта."
"Дополнительное описание:"
"%ДополнительноеОписание%';ka='ცვლის გახსნისას შეცდომა მოხდა."
"ცვლა არ გახსნილა."
"დამატებითი აღწერა:"
"%ДополнительноеОписание%'");
...
"Дополнительное описание: %ДополнительноеОписание%';ka='ცვლის გახსნისას შეცდომა მოხდა."
"ცვლა არ გახსნილა."
"დამატებითი აღწერა:"
"%ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", ОписаниеОшибки);
...
"%ДополнительноеОписание%';ka='ცვლის გახსნისას შეცდომა მოხდა. ფისკალურ რეგისტრატორზე ცვლა არ არის გახსნილი. %ДополнительноеОписание%'");
...
Процедура ВнесениеДенег(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ЗаголовокОкна = НСтр("ka='შეტანის თანხა, %Валюта%';ru='Сумма внесения, %Валюта%';en='making Amount% Currency%'");
...
Процедура ВыемкаДенег(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ЗаголовокОкна = НСтр("ka='ამოღების თანხა, %Валюта%';ru='Сумма выемки, %Валюта%';en='recesses Amount% Currency%'");
		ЗаголовокОкна = СтрЗаменить(
			ЗаголовокОкна,
			"%Валюта%",
			СтруктураСостояниеКассовойСмены.ВалютаДокументаПредставление
...
Процедура ПечатьОтчетаБезгашенияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ТекстСообщения = ?(РезультатВыполнения.Результат, НСтр("ru='Операция успешна завершена.';en='The operation completed successfully.'"), РезультатВыполнения.ОписаниеОшибки);
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
"""%ОписаниеОшибки%"""
"Отчет на фискальном регистраторе не сформирован.';ka='ცვლის დახურვისას ფისკალურ რეგისტრატორზე შეცდომა მოხდა. ""%ОписаниеОшибки%"" ფისკალურ რეგისტრატორზე ანგარიშგება არ არის შედგენილი.'"
		);
		ТекстСообщения = СтрЗаменить(
			ТекстСообщения,
			"%ОписаниеОшибки%",
...
"Дополнительное описание:"
"%ДополнительноеОписание%';ka='ჩეკის ანულირებისას შეცდომა მოხდა. ფისკალურ რეგისტრატორზე ჩეკი არ არის ანულირებული. დამატებითი აღწერილობა: %ДополнительноеОписание%'");
...
Процедура ПоказатьСкрытьСдачуНаСервере(Показать = Истина, Проверять = Ложь)
	...
	Если Не Проверять ИЛИ Не Элементы.СтраницыДанныеПоСтрокеИСдача.ТекущаяСтраница = Элементы.СтраницаДанныеПоСтроке Тогда
		СдачаСтрока = НСТР("ru = 'Сдача: '; ka = 'ხურდა: '")+Сдача+" "+Объект.ВалютаДокумента;
...
Функция ЗарезервироватьНаСервере(ОтменаРезервирования = Ложь)
	...
	Иначе
		...
		Исключение
			...
			Если СтрНайти(ОписаниеОшибки, НСтр("ka='გატარება ვერ მოხერხდა';ru='Не удалось провести';en='could not be held'")) = 0 Тогда
				Сообщение = Новый СообщениеПользователю;
...
Процедура ЗаполнитьСоставЧекаИСсылкиНаДокументыНаСервере(ЧекККМ)
	...
	Если ЧекККМ.КассоваяСмена.КассоваяСмена.Статус = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Элементы.ДекорацияЧекККМНаВозврат.Заголовок = НСТР("ka='დასაბრუნებელი ჩეკი №';ru='Чек ККМ на возврат №';en='Check CMC to return number'") + НомерДокумента + НСТР("ru = ' от '") + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			Элементы.СоздатьЧекККМНаВозврат.ЦветТекста = WebЦвета.Серый;
		Иначе
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда // Чек ККМ на возврат.
			...
			Пока Выборка.Следующий() Цикл
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияЧекККМНаВозврат.Заголовок = Элементы.ДекорацияЧекККМНаВозврат.Заголовок + "; "+" №"+НомерДокумента+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			КонецЦикла;
		...
		Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияПриходнаяНакладная.Заголовок = НСТР("ka='შემოსავლის ზედდებული №';ru='Прих. нак-я №';en='Com. NAC-I number'")+СокрЛП(НомерДокумента)+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				
				Пока Выборка.Следующий() Цикл
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
					Элементы.ДекорацияПриходнаяНакладная.Заголовок = Элементы.ДекорацияПриходнаяНакладная.Заголовок + "; "+" №"+НомерДокумента+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				КонецЦикла;
			Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияРКО.Заголовок = НСТР("ka='სგო №';ru='РКО №';en='RKO number'")+НомерДокумента+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				Элементы.СоздатьРКОНаОснованииПриходнойНакладной.ЦветТекста = WebЦвета.Серый;
				
				Пока Выборка.Следующий() Цикл
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
					Элементы.ДекорацияРКО.Заголовок = Элементы.ДекорацияРКО.Заголовок + "; "+" №"+НомерДокумента+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				КонецЦикла;
			Иначе
...
Процедура РассчитатьСкидкиНаценки(Команда)
	...
	Если СуммаДоРасчетаАвтоСкидок <> Объект.Запасы.Итог("Всего") Тогда
		СообщениеСтрока = НСтр("ru='Автоматические скидки (наценки) рассчитаны';en='Automatic discounts (extra charges) calculated'");
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	ПоказатьОповещениеПользователя(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თვლადი დისკონტური რუკა%1';ru='Считана дисконтная карта %1';en='Read discount card 1%'"), ДисконтнаяКарта),
		БиблиотекаКартинок.Информация32);
...
"Дополнительное описание:"
"%ДополнительноеОписание%';ka='მყიდველის დისპლეის გამოყენებისას შეცდომა მოხდა."
"დამატებითი აღწერა:"
"%ДополнительноеОписание%'"
			);
			ТекстСообщения = СтрЗаменить(
