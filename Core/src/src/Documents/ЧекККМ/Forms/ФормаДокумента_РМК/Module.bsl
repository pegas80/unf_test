...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	//+lazare
	СтруктуруСкидки = ПовторноеИспользованиеСервер.ПолучитьСтруктуруСкидки(СтруктураДанные);
	Если СтруктуруСкидки = Неопределено Тогда
		СтруктураДанные.Вставить("ПроцентСкидкиНаценки", 	0);
		СтруктураДанные.Вставить("СпособОкругления", 		Неопределено);
	//-lazare
	...
	//+lazare
	Иначе
		Если СтруктуруСкидки.Цена = 0 Тогда
			СтруктураДанные.Вставить("ПроцентСкидкиНаценки", 	СтруктуруСкидки.ПроцентСкидки);
			СтруктураДанные.Вставить("СпособОкругления", 		СтруктуруСкидки.СпособОкругления);
			СтруктураДанные.Вставить("Цена", 					?(СтруктураДанные.Свойство("ВидЦен"), УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураДанные), 0));
		Иначе
			СтруктураДанные.Вставить("ПроцентСкидкиНаценки", 	0);
			СтруктураДанные.Вставить("Цена", 					СтруктуруСкидки.Цена);
			СтруктураДанные.Вставить("СпособОкругления", 		Неопределено);
		КонецЕсли;
	КонецЕсли;
	//-lazare
...
Процедура НоменклатураПриИзменении(СтрокаТабличнойЧасти)
	...
	СтрокаТабличнойЧасти.СпособОкругления = СтруктураДанные.СпособОкругления;//+Lazare	
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти = Неопределено, УстановитьОписание = Истина)
	...
	//+Lazare
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.СпособОкругления) Тогда
		СуммаПередРасчетом 	= СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.Цена;
		ЦенаБезОкругления = СтрокаТабличнойЧасти.Цена * (1 - СтрокаТабличнойЧасти.ПроцентСкидкиНаценки / 100);
		Если ЦенаБезОкругления = Цел(ЦенаБезОкругления) Тогда
			ЦенаСоСкидкой 		= ЦенаБезОкругления;
		Иначе
			ЦенаСоСкидкой 		= Окр(ЦенаБезОкругления + ?(СтрокаТабличнойЧасти.СпособОкругления = ПредопределенноеЗначение("Перечисление.СпособыОкругления.Вверх"), 0.5, -0.5), 0);
		КонецЕсли;
		СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Количество * ЦенаСоСкидкой;
		СтрокаТабличнойЧасти.КоличествоСтар = СтрокаТабличнойЧасти.Количество;
		СтрокаТабличнойЧасти.ЦенаСтар  = СтрокаТабличнойЧасти.Цена;
		СтрокаТабличнойЧасти.СуммаСтар = СтрокаТабличнойЧасти.Сумма;
	Иначе
	//-Lazare
		...
		//+++ Lazare_13.05.2021
		СтрокаТабличнойЧасти.ЦенаСтар  = СтрокаТабличнойЧасти.Цена;
		//СтрокаТабличнойЧасти.СуммаСтар = Окр(СтрокаТабличнойЧасти.КоличествоСтар * СтрокаТабличнойЧасти.ЦенаСтар,2 );
		СтрокаТабличнойЧасти.СуммаСтар = СтрокаТабличнойЧасти.Сумма;	
		//--- Lazare_13.05.2021
	...
	ПроверкаАвтоматическиеСкидкиСразу(СтрокаТабличнойЧасти); // Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидЦен) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru = '%ВидЦен%'; ka = '%ВидЦен%'; en = '%ВидЦен%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru = ' • %ВидЦен%'; ka = ' • %ВидЦен%'; en = ' • %ВидЦен%'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидСкидки) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru = '%ВидСкидкиНаценки%'; ka = '%ВидСкидкиНаценки%'; en = '%ВидСкидкиНаценки%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru = ' • %ВидСкидкиНаценки%'; ka = ' • %ВидСкидкиНаценки%'; en = ' • %ВидСкидкиНаценки%'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ДисконтнаяКарта) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru = '%ДисконтнаяКарта%'; ka = '%ДисконтнаяКарта%'; en = '%ДисконтнаяКарта%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru = ' • %ДисконтнаяКарта%'; ka = ' • %ДисконтнаяКарта%'; en = ' • %ДисконтнаяКарта%'");
		КонецЕсли;
...
Процедура СформироватьЗаголовок(СтруктураСостояниеКассовойСмены)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ru = '%ТорговаяТочка%, Смена № %НомерСмены%  %ВремяИзменения%'; ka = '%ТорговаяТочка%, ცვლა %НомерСмены% %ВремяИзменения%'; en = '%ТорговаяТочка%, Shift No. %НомерСмены% %ВремяИзменения%'");
	...
	Иначе
		ТекстСообщения = НСтр("ru = '%ТорговаяТочка%'; ka = '%ТорговаяТочка%'; en = '%ТорговаяТочка%'");
...
Процедура СформироватьЗаголовокНаКлиенте(СтруктураСостояниеКассовойСмены)
	...
	ТекстСообщения = НСтр("ru = '%ТорговаяТочка%, Смена № %НомерСмены%  %ВремяИзменения%'; ka = '%ТорговаяТочка%, ცვლა № %НомерСмены%  %ВремяИзменения%'; en = '%ТорговаяТочка%% Change %НомерСмены%  %ВремяИзменения%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТорговаяТочка%", СокрЛП(СтруктураСостояниеКассовойСмены.СтруктурнаяЕдиница));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСмены%", СокрЛП(СтруктураСостояниеКассовойСмены.НомерКассовойСмены));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВремяИзменения%", Формат(СтруктураСостояниеКассовойСмены.ДатаИзмененияСтатуса,"ДФ=dd.MM.yyyy"));
...
//
&НаКлиенте
Процедура ПробитьЧек(СформироватьТоварныйЧек = Ложь, ДанныеОплаты = Неопределено)
	
	ОписаниеОшибки = "";
	
	Если Объект.НомерЧекаККМ <> 0
	И НЕ КассаККМИспользоватьБезПодключенияОборудования Тогда
		ТекстСообщения = НСтр("ka='ჩეკი უკვე ამორტყმულია ფისკალურ რეგისტრატორზე!';ru='Чек уже пробит на фискальном регистраторе!';en='Check has already broken on Fiscal registrars!'");
	...
	ТекстОшибки = "";
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтаФорма, ПоказыватьПредупреждение, ТекстОшибки) Тогда
		Если ПоказыватьПредупреждение Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა';ru='Не удалось выполнить проведение документа';en='Failed holding a document'" + ":" + ТекстОшибки));
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ИспользоватьПодключаемоеОборудование // Проверка на включенную ФО "Использовать ПО"
		...
		Если СформироватьТоварныйЧек И НЕ Объект.Ссылка.Пустая() Тогда
			...
			ТабличныйДокумент = ПечатьНаПринтере(МассивЧековККМ);
			Попытка
			...
			//+++ Дмгг
			Если ИспользоватьРазныеПринтерыЧековДляОтделов Тогда 
				Кухня = ПредопределенноеЗначение("Перечисление.ВидыЧековДляНоменклатурыККМ.Кухня");
				Напитки = ПредопределенноеЗначение("Перечисление.ВидыЧековДляНоменклатурыККМ.Напитки");
				Десерты = ПредопределенноеЗначение("Перечисление.ВидыЧековДляНоменклатурыККМ.Десерты");
				СтруктураПринтеровВкассеККМ = ПолучитьСтруктуруПринтеровВКассеККМ();
				
				Если СтруктураПринтеровВкассеККМ.Свойство(НСТР("ru = 'Кухня'; en = 'Kitchen'; ka = 'სამზარეულო'")) Тогда 
					ТабличныйДокумент = ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Кухня);
					Если ТабличныйДокумент <> Неопределено Тогда 
						Попытка
							ТабличныйДокумент.ИмяПринтера = СтруктураПринтеровВкассеККМ[НСТР("ru = 'Кухня'; en = 'Kitchen'; ka = 'სამზარეულო'")];
				...
				Если СтруктураПринтеровВкассеККМ.Свойство(НСТР("ru = 'Напитки'; en = 'Drinks'; ka = 'სასმელები'")) Тогда
					ТабличныйДокумент = ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Напитки);
					Если ТабличныйДокумент <> Неопределено Тогда
						Попытка
							ТабличныйДокумент.ИмяПринтера = СтруктураПринтеровВкассеККМ[НСТР("ru = 'Напитки'; en = 'Drinks'; ka = 'სასმელები'")];
							ТабличныйДокумент.АвтоМасштаб = Истина;
							ТабличныйДокумент.Напечатать();
						Исключение
							Сообщить(ОписаниеОшибки());
				...
				Если СтруктураПринтеровВкассеККМ.Свойство(НСТР("ru = 'Десерты'; en = 'Desserts'; ka = 'დესერტები'")) Тогда
					ТабличныйДокумент = ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Десерты);
					Если ТабличныйДокумент <> Неопределено Тогда
						Попытка
							ТабличныйДокумент.ИмяПринтера = СтруктураПринтеровВкассеККМ[НСТР("ru = 'Десерты'; en = 'Desserts'; ka = 'დესერტები'")];
							ТабличныйДокумент.АвтоМасштаб = Истина;
							ТабличныйДокумент.Напечатать();
						Исключение
							Сообщить(ОписаниеОшибки());
				...
				Попытка
					ОтправитьЧекНаВебСервисНаСервере(МассивЧековККМ);
				Исключение
					Сообщить("Не удалось отправить чек на терминал");
				КонецПопытки;
	...
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		...
		Контекст.Вставить("ДанныеОплаты", ДанныеОплаты); //Малышев 2025-09-25 
...
//+++ Дмгг
&НаСервере
Функция ПолучитьСтруктуруПринтеровВКассеККМ()

	СтруктураВозврата = Новый Структура;
	
	Для Каждого Строк из КассаККМ.ПринтерыДляОтделов Цикл 
		Если ЗначениеЗаполнено(Строк.ИмяПринтера) Тогда 
			СтруктураВозврата.Вставить(Строка(Строк.Отдел), Строк.ИмяПринтера);
		КонецЕсли;
	КонецЦикла;
	
	Возврат СтруктураВозврата;

КонецФункции // ПолучитьСтруктуруПринтеровВКассеККМ()

&НаКлиенте
Процедура ОтправитьЧекНаВебСервисНаСервере(МассивЧековККМ)

	//КассаККМ	
	СсылкаНастроек = ПолучитьСсылкуНастроекДляКассыККМ(КассаККМ);
	
	//Если СсылкаНастроек = Неопределено Тогда 
	Если СсылкаНастроек.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	//ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	//Результат = Обработки.ОтправкаЧековККМНаВебСервис.Создать().ОтправитьPOST(СсылкаНастроек, МассивЧековККМ[0]);
	Результат = ОтправитьPOST(СсылкаНастроек, МассивЧековККМ[0]);
...
&НаСервере
Функция ПолучитьСсылкуНастроекДляКассыККМ(КассаККМ)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		//СтруктураВозврата = ВыборкаДетальныеЗаписи.Ссылка;
		СтруктураВозврата.Вставить("ЗащищенноеСоединение", ВыборкаДетальныеЗаписи.Ссылка.ЗащищенноеСоединение);
		СтруктураВозврата.Вставить("Сервер", ВыборкаДетальныеЗаписи.Ссылка.Сервер);
		СтруктураВозврата.Вставить("Порт", ВыборкаДетальныеЗаписи.Ссылка.Порт);
		СтруктураВозврата.Вставить("ОстаткоАдреса", ВыборкаДетальныеЗаписи.Ссылка.ОстаткоАдреса);
		
		Заголовки = Новый Соответствие;
		
		Для Каждого Строк Из ВыборкаДетальныеЗаписи.Ссылка.Заголовки Цикл 
			
			Заголовки.Вставить(СокрЛП(Строк.Заголовок), СокрЛП(Строк.Значение));
			
		КонецЦикла;
		
		СтруктураВозврата.Вставить("Заголовки", Заголовки);
...
Функция ОтправитьPOST(НастройкиОтправки, СсылкаЧека) Экспорт
	...
	Заголовки = НастройкиОтправки.Заголовки;
	
	СтруктураJSON = Новый Структура;
	СтруктураParams = Новый Структура;
	
	СтруктураParams.Вставить("OrderNumber", Объект.НоммерОчереди);
	СтруктураParams.Вставить("OrderDate", Объект.Дата);
...
&НаКлиенте
Функция ФункцияПреобразования(Свойство, Значение, ДополнительныеПараметры, Отказ)
...
Процедура ПробитьЧекЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Исключение
			ЗаполнитьОписаниеДляТЧЗапасыНаКлиенте();
			ПоказатьПредупреждение(Неопределено, НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა';ru='Не удалось выполнить проведение документа';en='Failed holding a document'")); // Асинхронный метод!
			Возврат;
		КонецПопытки;
		
		//Паганини მოვაშორე საბეჭდი ფორმის ბეჭდვისთვის
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'При печати чека произошла ошибка.""Чек не напечатан на фискальном регистраторе.""Дополнительное описание:""%ДополнительноеОписание%'; ka = 'ჩეკის ბეჭდვისას შეცდომა მოხდა.""ჩეკი არ დაიბეჭდა ფისკალურ რეგისტრატორზე.""დამატებითი აღწერა:""%ДополнительноеОписание%'; en = 'An error occurred when printing the check.""The check was not printed on the fiscal registrar.""Additional description:""%ДополнительноеОписание%'"
		);
...
Процедура ОткрытьКассовуюСмену()
	...
	Иначе
		...
		Если НЕ Результат Тогда
			ТекстСообщения = НСтр("ru = 'При открытии смены произошла ошибка.""Смена не открыта.""Дополнительное описание:""%ДополнительноеОписание%'; ka = 'ცვლის გახსნისას შეცდომა მოხდა.""ცვლა არ გახსნილა.""დამატებითი აღწერა:""%ДополнительноеОписание%'; en = 'An error occurred when opening a shift.""Shift is not open.""Additional description:""%ДополнительноеОписание%'");
...
Процедура ОткрытьКассовуюСменуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда 
		...
		Результат = ОткрытьКассовуюСменуНаСервере(КассаККМ, ОписаниеОшибки); 
		Если НЕ Результат Тогда
			ТекстСообщения = НСтр("ru = 'При открытии смены произошла ошибка. Смена не открыта.""Дополнительное описание: %ДополнительноеОписание%'; ka = 'ცვლის გახსნისას შეცდომა მოხდა.""ცვლა არ გახსნილა.""დამატებითი აღწერა:""%ДополнительноеОписание%'; en = 'An error occurred when opening a shift. Shift not open.""Additional description: ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", ОписаниеОшибки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		ОбновитьСостояниеКассовойСмены(КассаККМ);
	Иначе
		ТекстСообщения = НСтр("ru = 'При открытии смены произошла ошибка.""Смена не открыта на фискальном регистраторе.""%ДополнительноеОписание%'; ka = 'ცვლის გახსნისას შეცდომა მოხდა. ფისკალურ რეგისტრატორზე ცვლა არ არის გახსნილი. %ДополнительноеОписание%'; en = 'An error occurred when opening a shift.""The shift was not opened on the fiscal registrar.""%ДополнительноеОписание%'");
...
Процедура ВнесениеДенег(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ЗаголовокОкна = НСтр("ru = 'Сумма внесения, %Валюта%'; ka = 'შეტანის თანხა, %Валюта%'; en = 'making Amount %Валюта%'");
...
Процедура ИнкассацияНаФискальномУстройствеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ru = 'При выполнении операции произошла ошибка.
                               ...
                               |Дополнительное описание: %ДополнительноеОписание%'; ka = 'ოპერაციის შესრულებისას მოხდა შეცდომა.
                               |ქვითარი ფისკალურ მოწყობილობაზე არ იბეჭდება.
                               |დამატებითი აღწერა:: %ДополнительноеОписание%'; en = 'An error occurred while performing the operation.
...
Процедура ВыемкаДенег(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ЗаголовокОкна = НСтр("ru = 'Сумма выемки, %Валюта%'; ka = 'ამოღების თანხა, %Валюта%'; en = 'recesses Amount, %Валюта%'");
		ЗаголовокОкна = СтрЗаменить(
			ЗаголовокОкна,
			"%Валюта%",
			СтруктураСостояниеКассовойСмены.ВалютаДокументаПредставление
...
//
&НаКлиенте
Процедура ПробитьЧекВыполнить(Команда, СформироватьТоварныйЧек = Ложь, ДанныеОплаты = Неопределено)
	...
	Если Объект.ПометкаУдаления Тогда
		...
		ТекстОшибки = НСтр("ka='დოკუმენტი მონიშნულია წასაშლელად';ru='Документ помечен на удаление';en='Document marked for deletion'");
	...
	Если Объект.СуммаДокумента > Объект.ПолученоНаличными + Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		ТекстОшибки = НСтр("ka='გადახდის თანხა ჩეკის თანხაზე ნაკლებია';ru='Сумма оплаты меньше суммы чека';en='The amount of payment is less than the amount of the check'");
	...
	Если Объект.СуммаДокумента < Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		ТекстОшибки = НСтр("ka='გადახდის ბარათებით გადახდის თანხა აღემატება ჩეკის თანხას';ru='Сумма оплаты платежными картами превышает сумму чека';en='payment cards payment amount exceeds the amount of the check'");
	...
	Если НЕ Отказ И ПроверитьЗаполнение() Тогда
		...
		ПробитьЧек(СформироватьТоварныйЧек, ДанныеОплаты);
...
Процедура ПечатьОтчетаБезгашенияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ТекстСообщения = ?(РезультатВыполнения.Результат, НСтр("ru = 'Операция успешна завершена.'; ka = 'ოპერაცია წარმატებით დასრულდა.'; en = 'The operation completed successfully.'"), РезультатВыполнения.ОписаниеОшибки);
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ПечатьОтчетаБезГашенияВыполнить()
	...
	Попытка
		...
		МенеджерОборудованияКлиент.НачатьВыполнениеКраткийОтчетНаЭквайринговомТерминале(ОписаниеОповещения, УникальныйИдентификатор);
	Исключение
	КонецПопытки;
...
Процедура ЗакрытьКассовуюСмену(Команда)
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		ТекстОшибки = НСтр("ka='აირჩიეთ სსა სალარო!';ru='Выберите кассу ККМ!';en='Select POS cash!'");
...
Процедура СформироватьОтчетОРозничныхПродажах(ИспользоватьБезПодключенияОборудования) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ОписаниеОшибки)
		...
		ТекстСообщения = НСтр(
			"ru = 'При формировании отчета о розничных продажах возникли ошибки.
             ...
             |%ДополнительноеОписание%'; ka = 'საცალო გაყიდვების ანგარიშის გენერირებისას წარმოიშვა შეცდომები.
             |დამატებითი აღწერა:
             |%ДополнительноеОписание%'; en = 'Errors occurred when generating the retail sales report.
             |Additional description:
             |%ДополнительноеОписание%'"
		);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", ОписаниеОшибки);
	...
	Если НЕ Результат Тогда
		ТекстСообщения = НСтр("ru = 'При закрытии смены произошла ошибка.
                               |Смена не закрыта.
                               |Дополнительное описание:
                               |%ДополнительноеОписание%'; ka = 'ცვლის დახურვისას მოხდა შეცდომა.
                               |ცვლა არ არის დახურული.
                               |დამატებითი აღწერა:
                               |%ДополнительноеОписание%'; en = 'An error occurred while closing the shift.
...
Процедура ПечатьФискальногоОтчетаЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'При закрытии смены на фискальном регистраторе произошла ошибка.""""%ОписаниеОшибки%""""Отчет на фискальном регистраторе не сформирован.'; ka = 'ცვლის დახურვისას ფისკალურ რეგისტრატორზე შეცდომა მოხდა. """"%ОписаниеОшибки%"""" ფისკალურ რეგისტრატორზე ანგარიშგება არ არის შედგენილი.'; en = 'An error occurred on the fiscal registrar when closing a shift.""""%ОписаниеОшибки%""""The report on the fiscal registrar was not generated.'"
		);
		ТекстСообщения = СтрЗаменить(
			ТекстСообщения,
			"%ОписаниеОшибки%",
...
Процедура АннулироватьЧекККМ(КассаККМ)
	...
	Иначе
		ТекстСообщения = НСтр("ka='ფისკალური რეგისტრატორი არ არის არჩეული.';ru='Не выбран фискальный регистратор.';en='Not selected fiscal register.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура АннулированиеЧекаЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		...
		ТекстСообщения = НСтр("ru = 'При аннулировании чека произошла ошибка. Чек не аннулирован на фискальном регистраторе.""Дополнительное описание:""%ДополнительноеОписание%'; ka = 'ჩეკის ანულირებისას შეცდომა მოხდა. ფისკალურ რეგისტრატორზე ჩეკი არ არის ანულირებული. დამატებითი აღწერილობა: %ДополнительноеОписание%'; en = 'An error occurred while canceling the check. The check was not canceled at the fiscal registrar.""Additional description:""%ДополнительноеОписание%'");
...
Процедура ПечатьКопииНаФискальномРегистраторе(Команда)
	...
	Иначе
		ТекстСообщения = НСтр("ka='უკანასკნელი სლიპ ჩეკი არ არის.';ru='Отсутствует последний слип чек.';en='No slip last check.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПоказатьСкрытьСдачуНаСервере(Показать = Истина, Проверять = Ложь)
	...
	Если Не Проверять ИЛИ Не Элементы.СтраницыДанныеПоСтрокеИСдача.ТекущаяСтраница = Элементы.СтраницаДанныеПоСтроке Тогда
		СдачаСтрока = НСТР("ru = 'Сдача: '; ka = 'ხურდა: '; en = 'Change: '")+Сдача+" "+Объект.ВалютаДокумента;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	//--- Дмгг
	// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
	АвтоматическиеСкидкиСразу = тзУчетнаяПолитика[0].РозничныеПродажиАвтоматическиеСкидкиСразу;
	// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
...
Процедура ПриОткрытии(Отказ)
	...
	ВосстановитьТаблицуИзПамяти(); //+++ Дмгг
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПодборПроизведен" 
		...
		ПересчитатьДокументНаКлиенте();
		ПроверкаАвтоматическиеСкидкиСразу(НЕОПРЕДЕЛЕНО); // Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
...
Процедура ОбновитьВидимостьНадписиИстеклоБолее24Часов(СтруктураСостояниеКассовойСмены = Неопределено)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ka='სალაროს ცვლა გახსნილია';ru='Кассовая смена открыта';en='Cash change open'");
...
Процедура Оплатить(Команда)
	...
	Если Объект.Запасы.Итог("Всего") <= 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОплатитьОбработкаОтветаНаВопрос", ЭтотОбъект, СуммаДоРасчетаАвтоСкидок),
			НСтр("ka='გადასახდელი თანხა = 0! დარწმუნებული ხართ, რომ საჭიროა გაგრძელება?';ru='Сумма к оплате = 0! Уверены, что нужно продолжить?';en='Payment amount = 0! We believe that it is necessary to continue?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			КодВозвратаДиалога.Нет,
			НСтр("ka='მოქმედების დადასტურების მოთხოვნა';ru='Запрос о подтверждении действия';en='Request you to confirm that'"));
	Иначе
...
Процедура ОплатитьФрагмент(СуммаДоРасчетаАвтоСкидок)
	...
	//+++ Дмгг
	СтруктураПараметров.Вставить("ДоступныеТерминалы", ПолучитьДоступныеТерминалыСервер(КассаККМ));
	СтруктураПараметров.Вставить("ИспользоватьНовуюФормуОплат", ИспользоватьНовуюФормуОплат);
	//--- Дмгг
...
Процедура ОплатитьЗавершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Кнопка = "Пробить чек" Тогда
			...
			ПробитьЧекВыполнить(Команды.ПробитьЧек, СформироватьТоварныйЧек, Результат);
...
Функция ЗарезервироватьНаСервере(ОтменаРезервирования = Ложь)
	...
	Иначе
		...
		Исключение
			...
			Если СтрНайти(ОписаниеОшибки, НСтр("ru = 'Не удалось провести :%ОписаниеОшибки%'; ka = 'გატარება ვერ მოხერხდა %ОписаниеОшибки%'; en = 'could not be held %ОписаниеОшибки%'")) = 0 Тогда
				Сообщение = Новый СообщениеПользователю;
...
Процедура ЗначениеПоискаНоменклатурыПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ЗначениеПоискаНоменклатуры) Тогда
		...
		НайденнаяСтрока = НайтиРаниеВведенныеСтрокиНаСервере(ЗначениеПоискаНоменклатуры);
		Если НайденнаяСтрока = Неопределено Тогда 
			НоваяСтрока = Объект.Запасы.Добавить();
			НоваяСтрока.Номенклатура = ЗначениеПоискаНоменклатуры;
			НоваяСтрока.Консультант = Объект.Консультант;//++Lazare 05-08-2021
		Иначе
			НоваяСтрока = объект.Запасы.НайтиПоИдентификатору(НайденнаяСтрока);
...
&НаСервере
Функция НайтиРаниеВведенныеСтрокиНаСервере(ЗначениеПоискаНоменклатуры)

	УИДСтрокаВозврата = Неопределено;
	МассивПоиска = Объект.Запасы.НайтиСтроки(Новый Структура("Номенклатура, Характеристика",ЗначениеПоискаНоменклатуры, Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка()));	

	Если МассивПоиска.Количество() > 0 Тогда 
		
		УИДСтрокаВозврата = МассивПоиска[0].ПолучитьИдентификатор();
		
	КонецЕсли;
	
	Возврат УИДСтрокаВозврата;
		
КонецФункции // НайтиРаниеВведенныеСтрокиНаСервере(ЗначениеПоискаНоменклатуры)()
...
Процедура ЧекНеПредъявленПриИзмененииНаСервере()
	...
	Если ЧекНеПредъявлен Тогда
		...
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСТР("ka='შექმენით შემოსავლის ზედდებული და სგო';ru='Создайте приходную накладную и РКО';en='Create Invoice and RKO'");
	...
	Иначе
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСТР("ka='აირჩიეთ დაბრუნების საფუძველი';ru='Выберите основание для возврата';en='Select a reason to return'");
...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	//++Lazare 03.08.2021		
	ВидимостьКонсультанта = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьКонсультантыВЧекККМ");	
	Если  ВидимостьКонсультанта И НоваяСтрока Тогда 
		Элемент.ТекущиеДанные.Консультант =	Объект.Консультант ;
	КонецЕсли;
	//--Lazare 03.08.2021
...
Процедура ЗапасыПередУдалением(Элемент, Отказ)
	...
	Если ПроверитьПравоРедактирования() Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;	
...
Процедура ЗапасыКоличествоПриИзменении(Элемент)
	...
	Если СтрокаТабличнойЧасти.КоличествоСтар > СтрокаТабличнойЧасти.Количество Тогда
		Если ПроверитьПравоРедактирования() Тогда
			СтрокаТабличнойЧасти.Количество = СтрокаТабличнойЧасти.КоличествоСтар;
		    Возврат;
		КонецЕсли;
	КонецЕсли;					
...
Процедура ЗапасыЦенаПриИзменении(Элемент)
	...
	//+++ Lazare_13.05.2021
	СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;	
	Если СтрокаТабличнойЧасти.ЦенаСтар > СтрокаТабличнойЧасти.Цена Тогда
		Если ПроверитьПравоРедактирования() Тогда
			СтрокаТабличнойЧасти.Цена = СтрокаТабличнойЧасти.ЦенаСтар;
		    Возврат;
		КонецЕсли;
	КонецЕсли;						
	СтрокаТабличнойЧасти.ЦенаСтар = СтрокаТабличнойЧасти.Цена;	
...
Процедура ЗапасыСуммаПриИзменении(Элемент)
	...
	//+++ Lazare_14.05.2021
	//+++ Lazare_13.05.2021СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;	
	Если СтрокаТабличнойЧасти.СуммаСтар > СтрокаТабличнойЧасти.Сумма Тогда
		Если ПроверитьПравоРедактирования() Тогда
			СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.СуммаСтар;
		    Возврат;
		КонецЕсли;
	КонецЕсли;						
	СтрокаТабличнойЧасти.СуммаСтар = СтрокаТабличнойЧасти.Сумма;	
...
Процедура ЗапасыПриИзменении(Элемент)
	...
	ЭтаФорма.ТекущийЭлемент = Элементы.ЗначениеПоискаНоменклатуры; //+++ Дмгг
...
Процедура ЗаполнитьСоставЧекаИСсылкиНаДокументыНаСервере(ЧекККМ)
	...
	Если ЧекККМ.КассоваяСмена.КассоваяСмена.Статус = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Элементы.ДекорацияЧекККМНаВозврат.Заголовок = НСТР("ka='დასაბრუნებელი ჩეკი №';ru='Чек ККМ на возврат №';en='Check CMC to return number'") + НомерДокумента + НСТР("ru = ' от '; ka = 'დან'; en = 'from'") + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			Элементы.СоздатьЧекККМНаВозврат.ЦветТекста = WebЦвета.Серый;
		Иначе
		...
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСТР("ka='აირჩიეთ საფუძველი დასაბრუნებლად';ru='Выберите основание для возврата';en='Select a reason to return'");
	Иначе
		...
		Если Выборка.Следующий() Тогда // Чек ККМ на возврат.
			...
			Пока Выборка.Следующий() Цикл
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияЧекККМНаВозврат.Заголовок = Элементы.ДекорацияЧекККМНаВозврат.Заголовок + "; "+" №"+НомерДокумента+НСТР("ru = ' от '; ka = 'დან'; en = 'from'")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			КонецЦикла;
		...
		Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияПриходнаяНакладная.Заголовок = НСТР("ka='შემოსავლის ზედდებული №';ru='Прих. нак-я №';en='Com. NAC-I number'")+СокрЛП(НомерДокумента)+НСТР("ru = ' от '; ka = 'დან'; en = ' From'")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				
				Пока Выборка.Следующий() Цикл
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
					Элементы.ДекорацияПриходнаяНакладная.Заголовок = Элементы.ДекорацияПриходнаяНакладная.Заголовок + "; "+" №"+НомерДокумента+НСТР("ru = ' от '; ka = ' დან'; en = ' From'")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				КонецЦикла;
			Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияРКО.Заголовок = НСТР("ka='სგო №';ru='РКО №';en='RKO number'")+НомерДокумента+НСТР("ru = ' от '; ka = ' დან'; en = ' from'")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				Элементы.СоздатьРКОНаОснованииПриходнойНакладной.ЦветТекста = WebЦвета.Серый;
				
				Пока Выборка.Следующий() Цикл
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
					Элементы.ДекорацияРКО.Заголовок = Элементы.ДекорацияРКО.Заголовок + "; "+" №"+НомерДокумента+НСТР("ru = ' от '; ka = ' დან'; en = ' from'")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				КонецЦикла;
			Иначе
...
// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
	...
	СуммаДоРасчетаАвтоСкидок = Объект.Запасы.Итог("Всего");
	СуммаАвтоматическойСкидкиДоРасчетаАвтоСкидок = Объект.Запасы.Итог("СуммаАвтоматическойСкидки");// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
	РассчитатьСкидкиНаценкиКлиент();
	
	Если СуммаДоРасчетаАвтоСкидок <> Объект.Запасы.Итог("Всего") //Тогда
		// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
		ИЛИ СуммаАвтоматическойСкидкиДоРасчетаАвтоСкидок <> Объект.Запасы.Итог("СуммаАвтоматическойСкидки")
		// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
		Тогда
		СообщениеСтрока = НСтр("ru = 'Автоматические скидки (наценки) рассчитаны'; ka = 'გამოითვალა ავტომატური ფასდაკლებები (ფასნამატი).'; en = 'Automatic discounts (extra charges) calculated'");
...
Процедура ОткрытьИнформациюОСкидкахКлиент()
	...
	Если НЕ Объект.СкидкиРассчитаны Тогда
		ТекстВопроса = НСтр("ka='ფასდაკლებები არ არის გათვალისწინებული, გავითვალისწინოთ?';ru='Скидки (наценки) не рассчитаны, рассчитать?';en='Discounts (markups) are not designed to calculate?'");
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Считана дисконтная карта'; ka = 'განიხილება ფასდაკლების ბარათი'; en = 'Considered a discount card'"),
		ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Считана дисконтная карта %1'; ka = 'თვლადი დისკონტური რუკა%1'; en = 'Read discount card %1'"), ДисконтнаяКарта),
		БиблиотекаКартинок.Информация32);
...
Процедура ВыбранаДисконтнаяКартаДополнительно(ДисконтнаяКарта)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		Текст = НСтр("ka='ხელახლა შეივსოს ფასდაკლებები ყველა სტრიქონში?';ru='Перезаполнить скидки во всех строках?';en='Refill discounts on all lines?'");
...
Процедура ВыводТекстаНаДисплейПокупателяЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		...
		ТекстСообщения = НСтр("ru = 'При использовании дисплея покупателя произошла ошибка.""Дополнительное описание:""%ДополнительноеОписание%'; ka = 'მყიდველის დისპლეის გამოყენებისას შეცდომა მოხდა.""დამატებითი აღწერა:""%ДополнительноеОписание%'; en = 'An error occurred while using the customer display.""Additional description:%ДополнительноеОписание%'"
			);
			ТекстСообщения = СтрЗаменить(
				ТекстСообщения,
				"%ДополнительноеОписание%",
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если МассивСтрокТЧ.Количество() = 0
				...
				//+Lazare
				СтруктуруСкидки = ПовторноеИспользованиеСервер.ПолучитьСтруктуруСкидкиИзКлиента(НоваяСтрока.Номенклатура, НоваяСтрока.Характеристика, СтруктураДанные.Дата);
				Если СтруктуруСкидки = Неопределено Тогда
					НоваяСтрока.СпособОкругления		= Неопределено;
				//-Lazare	
				...
				Иначе
					Если СтруктуруСкидки.Цена = 0 Тогда
						НоваяСтрока.ПроцентСкидкиНаценки 	= СтруктуруСкидки.ПроцентСкидки;
						НоваяСтрока.СпособОкругления 		= СтруктуруСкидки.СпособОкругления;
						НоваяСтрока.Цена 					= ДанныеШтрихкода.СтруктураДанныеНоменклатуры.Цена;
					Иначе
						НоваяСтрока.ПроцентСкидкиНаценки 	= 0;
						НоваяСтрока.Цена 					= СтруктуруСкидки.Цена;
						НоваяСтрока.СпособОкругления 		= Неопределено;
					КонецЕсли;
				КонецЕсли;
				//-Lazare
...
Процедура ПолученыШтрихкоды(ДанныеШтрикодов) Экспорт
	...
	//++Lazare 29.05.2023 
	ИспользоватьКонсультантыВЧекККМ = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьКонсультантыВЧекККМ");	
	Если ИспользоватьКонсультантыВЧекККМ и не ЗначениеЗаполнено(Объект.Консультант) Тогда 	
		ОчиститьСообщения();
		ЗаполнитьКонсултантаПоШтрихкоду(ДанныеШтрикодов);	
		Возврат;
	КонецЕсли;
	//--Lazare 29.05.2023 
		
	Если ТипЗнч(ДанныеШтрикодов)  = Тип("Структура") Тогда
		
		Если ПроверитьКодДоступа(ДанныеШтрикодов.Штрихкод) Тогда	
			ПравоРедактирования = Истина;
			УстановитьДоступность();
			Возврат;	
		КонецЕсли;
	...
	Иначе
		Если ДанныеШтрикодов.Количество() <> 0 Тогда
			...
			Если ПроверитьКодДоступа(ДанныеШтрикодов[0].Штрихкод) Тогда	
				ПравоРедактирования = Истина;
				УстановитьДоступность();
				Возврат;	
			КонецЕсли;
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ПоказатьВводЗначения(ОписаниеОповещения, ТекШтрихкод, НСтр("ka='შეიყვანეთ შტრიხკოდი';ru='Введите штрихкод';en='enter the barcode'"));
...
Процедура ПоискПоШтрихкодуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	//ДД
	Если ПроверитьКодДоступа(Результат) Тогда
...
&НаСервере
Функция ПроверитьКодДоступа(Код)
	
	Запрос = Новый Запрос;
	
	Запрос.Текст = "ВЫБРАТЬ
	               |	РегистрДляКодовРазблокировкиСрезПоследних.Код,
	               |	РегистрДляКодовРазблокировкиСрезПоследних.Временный
	               |ИЗ
	               |	РегистрСведений.РегистрДляКодовРазблокировки.СрезПоследних КАК РегистрДляКодовРазблокировкиСрезПоследних
	               |ГДЕ
	               |	РегистрДляКодовРазблокировкиСрезПоследних.Код = &Код
	               |	И ВЫБОР
	               |			КОГДА РегистрДляКодовРазблокировкиСрезПоследних.Временный
	               |				ТОГДА РАЗНОСТЬДАТ(РегистрДляКодовРазблокировкиСрезПоследних.Период, &Дата, МИНУТА) < 10
	               |			ИНАЧЕ ИСТИНА
	               |		КОНЕЦ";
	...
	Если Выборка.Следующий() Тогда 
		...
		Если Выборка.Временный Тогда
			НаборЗаписей = РегистрыСведений.РегистрДляКодовРазблокировки.СоздатьНаборЗаписей();		
...
Процедура УстановитьПериодНаСервере(ВидПериодаРМК, ИмяСписка, ПараметрСтандартныйПериод = Неопределено)
	...
	Если ВидПериодаРМК = ЗаТекущуюСмену Тогда
		...
		Если ИмяСписка = "Список" Тогда
			...
			ВидПериодаЖурнала = НСТР("ka='მიმდინარე ცვლის მანძილზე';ru='За текущую смену';en='For the current shift'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='მიმდინარე ცვლის მანძილზე';ru='За текущую смену';en='For the current shift'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='მიმდინარე ცვლის მანძილზე';ru='За текущую смену';en='For the current shift'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаПрошлуюСмену Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
			ВидПериодаЖурнала = НСТР("ka='გასული ცვლის მანძილზე';ru='За прошлую смену';en='Over the last change'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='გასული ცვლის მანძილზე';ru='За прошлую смену';en='Over the last change'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='გასული ცვლის მანძილზე';ru='За прошлую смену';en='Over the last change'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаВчерашнийДень Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
			ВидПериодаЖурнала = НСТР("ka='გუშინდელი დღისთვის';ru='За вчерашний день';en='For the last day'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='გუშინდელი დღისთვის';ru='За вчерашний день';en='For the last day'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='გუშინდელი დღისთვის';ru='За вчерашний день';en='For the last day'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаВесьПериод Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Истина;
			ВидПериодаЖурнала = НСТР("ka='მთელი პერიოდის მანძილზე';ru='За всё время';en='During all this time'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='მთელი პერიოდის მანძილზე';ru='За всё время';en='During all this time'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='მთელი პერიოდის მანძილზე';ru='За всё время';en='During all this time'");
		КонецЕсли;
...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='მყიდველთა შეკვეთები';ru='Заказы покупателей';en='Customer orders'");
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ПометкаУдаления", Ложь);
	
	РаботаСОтборами.СоздатьЭлементыМеток(ЭтотОбъект, "Закрыт", "ГруппаОтборЗакрыт", Ложь, НСтр("ka='დასახურად';ru='Не закрытые';en='not closed'"));
	...
	ИначеЕсли Параметры.Свойство("Новые") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='ახლები';ru='новые';en='new'");
...
Процедура ВыполнитьСверкуИтогов(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Выполнить сверку итогов на эквайринговом терминале?'; ka = 'ტერმინალების ჯამების შედარების შესრულება'; en = 'Perform verification of the outcome of the acquiring terminals?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
...
Процедура ВыполнитьСверкуИтоговЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='დღის დახურვა დასრულდა წარმატებით.';ru='Операция выполнена успешно.';en='Operation completed successfully.'");
	Иначе
...
Функция РазобратьШтрихкод(Штрихкод, ВариантРаспознаванияШтрихкода)
	...
	// Kuzmich, 01.05.2024, (ONESOFT-3106) штрих М - загрузка штучного товара
	Если ВариантРаспознаванияШтрихкода = Перечисления.ВариантыРаспознованияШтрихкодов.Вес Тогда
		
		СтруктураДанныхШтрихкода = Новый Структура("Штрихкод, Количество", Штрихкод, 1);
		РегистрыСведений.ШтрихкодыНоменклатуры.ПреобразоватьВесовойШтрихкод(СтруктураДанныхШтрихкода);
		...
		ДанныеШтрихкода = ДанныеПоШтрихкодам[СтруктураДанныхШтрихкода.Штрихкод];
		
		Если ДанныеШтрихкода <> Неопределено
			И ДанныеШтрихкода.Количество() <> 0 Тогда
			
			СтруктураРезультата = Новый Структура;
			СтруктураРезультата.Вставить("Номенклатура", ДанныеШтрихкода.Номенклатура);
			СтруктураРезультата.Вставить("Характеристика", ДанныеШтрихкода.Характеристика);
			
			Если ЗначениеЗаполнено(ДанныеШтрихкода.ЕдиницаИзмерения) Тогда
				СтруктураРезультата.Вставить("ЕдиницаИзмерения", ДанныеШтрихкода.ЕдиницаИзмерения);
			Иначе
				СтруктураРезультата.Вставить("ЕдиницаИзмерения", ДанныеШтрихкода.Номенклатура.ЕдиницаИзмерения);
			КонецЕсли;
			
			СтруктураРезультата.Вставить("Весовая", ДанныеШтрихкода.Номенклатура.ЕдиницаИзмерения.Весовая);
			СтруктураРезультата.Вставить("Вес", СтруктураДанныхШтрихкода.Количество);
...
Процедура ДобавитьШтрихкодВТабличнуюЧасть(Штрихкод, ВариантРаспознаванияШтрихкода) Экспорт
	...
	Если СтруктураРезультата = Неопределено Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Товар со штрихкодом %1 не найден!'; ka = 'პროდუქტი %1 შტრიხკოდით ვერ მოიძებნა!'; en = 'Product barcode%1 can not be found!'"), Штрихкод);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
...
Функция ПроверитьПравоРедактирования()
	...
	Если  Не Константы.ФункционалКодовРазблокировки.Получить() Тогда
		Возврат Ложь;
	КонецЕсли;	
	
	Возврат (Не ПравоРедактирования) И Не РольДоступна("ПолныеПрава");
	
КонецФункции	

//Паганини_
&НаСервере
Функция ПечатьНаПринтере(МассивЧековККМ)

	Возврат Документы.ЧекККМ.Печать(МассивЧековККМ, Неопределено, Неопределено, Неопределено, Неопределено, Истина);
	
КонецФункции

//+++ Дмгг
Функция ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Отдел)
	
	Возврат Документы.ЧекККМ.Печать(МассивЧековККМ, Неопределено, Неопределено, Неопределено, Неопределено, Истина, Отдел);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	////+++ Дмгг
	тзЗапасыВПамяти.Очистить();
	Если Объект.Запасы.Количество() > 0 Тогда 
		Для Каждого Строк из Объект.Запасы Цикл  
			НовСтрокаПамяти = тзЗапасыВПамяти.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрокаПамяти, Строк);
		КонецЦикла;
	КонецЕсли;
	////--- Дмгг
	...
	//	Если ПроверитьПравоРедактирования() Тогда 
	//		Отказ = Истина;
	//		Сообщить("ფორმის გასათიშად გაასუფთავეთ ცხრილი!");
	//	КонецЕсли;
	//КонецЕсли;		
КонецПроцедуры
...
Процедура УстановитьДоступность()
	...
	Элементы.ЗапасыНоменклатура.Доступность = Не ПроверитьПравоРедактирования(); 
	
КонецПроцедуры	

&НаСервере
Процедура ПокзатьпродажиСменыНаСервере(ТабличныйДокумент)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ДенежныеСредстваВКассахККМОбороты.Регистратор ССЫЛКА Документ.ЧекККМ
		|			ТОГДА ДенежныеСредстваВКассахККМОбороты.СуммаРасход
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Поле1
		|ПОМЕСТИТЬ ВТ_ОплатыКартами
		|ИЗ
		...
		|			Регистратор,
		|			КассаККМ = &КассаККМ
		|				И Организация = &Организация) КАК ДенежныеСредстваВКассахККМОбороты
		|ГДЕ
		|	ДенежныеСредстваВКассахККМОбороты.Регистратор ССЫЛКА Документ.ЧекККМ
		|	И ДенежныеСредстваВКассахККМОбороты.СуммаРасход <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДенежныеСредстваВКассахККМОбороты.СуммаПриход,
		|	ДенежныеСредстваВКассахККМОбороты.Регистратор
		|ПОМЕСТИТЬ ВТ_ОплатыНал
		|ИЗ
		|	РегистрНакопления.ДенежныеСредстваВКассахККМ.Обороты(
		|			&ДатаНач,
		|			&ДатаКон,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВТ_ОплатыКартами.Поле1) КАК ОплатыКредитнымиКартами,
		|	0 КАК ОплатыНаличными
		|ПОМЕСТИТЬ ВТ_Пред
		|ИЗ
		|	ВТ_ОплатыКартами КАК ВТ_ОплатыКартами
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	0,
		|	СУММА(ВТ_ОплатыНал.СуммаПриход)
		|ИЗ
		|	ВТ_ОплатыНал КАК ВТ_ОплатыНал
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВТ_Пред.ОплатыКредитнымиКартами) КАК ОплатыКредитнымиКартами,
		|	СУММА(ВТ_Пред.ОплатыНаличными - ВТ_Пред.ОплатыКредитнымиКартами) КАК ОплатыНаличными,
		|	СУММА(ВТ_Пред.ОплатыКредитнымиКартами + (ВТ_Пред.ОплатыНаличными - ВТ_Пред.ОплатыКредитнымиКартами)) КАК СуммаВсего
		|ИЗ
		|	ВТ_Пред КАК ВТ_Пред";
	...
	МакетОтчета = Документы.ЧекККМ.ПолучитьМакеДокумента("ОтчетИтогиСмены");
	
	ОбластьВывода = МакетОтчета.ПолучитьОбласть("Заголовок");
	
	ТабличныйДокумент.Вывести(ОбластьВывода);
	
	ОбластьВывода = МакетОтчета.ПолучитьОбласть("ПустаяСтрока");
	
	ТабличныйДокумент.Вывести(ОбластьВывода);
	
	ОбластьВывода = МакетОтчета.ПолучитьОбласть("ШапкаТаблицы");
	
	ТабличныйДокумент.Вывести(ОбластьВывода);
	
	Для Каждого Строк Из тзРезультат Цикл 
		ОбластьВывода = МакетОтчета.ПолучитьОбласть("СтрокаТаблицы");
		ЗаполнитьЗначенияСвойств(ОбластьВывода.Параметры, Строк);
		ОбластьВывода.Параметры.КассоваяСмена = Объект.КассоваяСмена;
		ОбластьВывода.Параметры.Ответственный = Объект.Ответственный;
		ТабличныйДокумент.Вывести(ОбластьВывода);
	КонецЦикла;
...
&НаКлиенте
Процедура ПокзатьпродажиСмены(Команда)
	
	//ТабличныйДокумент = Новый ТабличныйДокумент;
	//ПокзатьпродажиСменыНаСервере(ТабличныйДокумент);
	//
	//ТабличныйДокумент.ОтображатьЗаголовки = ЛОЖЬ;
	//ТабличныйДокумент.ОтображатьСетку = ЛОЖЬ;
	//ТабличныйДокумент.Показать();
	
	Вариант = РаботаСФормойДокументаКлиентСервер.ВариантОтчета("ОтчетЗаСмену", "ОтчетЗаСмену");
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("СформироватьПриОткрытии", Истина);
	ОтборРасшифровки = Новый Соответствие;
	ОтборРасшифровки.Вставить("КассаККМ", КассаККМ);
	ОтборРасшифровки.Вставить("Организация", Объект.Организация);
	ОтборРасшифровки.Вставить("Ответственный", Объект.Ответственный);
	ОтборРасшифровки.Вставить("НачалоПериода", НачалоДня(ТекущаяДата()));
	ОтборРасшифровки.Вставить("КонецПериода", КонецДня(ТекущаяДата()));
...
//+++ Дмгг
&НаСервере
Функция ПолучитьДоступныеТерминалыСервер(Касса)
	...
	|	//И ЭквайринговыеТерминалы.ПодключаемоеОборудование.УстройствоИспользуется = ИСТИНА //Малышев 2025-05-23 ONESOFT-4096 - закоментировал
...
КонецФункции // ПолучитьДоступныеТерминалыСервер(Касса)

&НаСервере
Процедура ВосстановитьТаблицуИзПамяти()

	Объект.Запасы.Загрузить(тзЗапасыВПамяти.Выгрузить());

КонецПроцедуры

&НаСервере
Процедура СоздатьОасходнуюНаксладнуюНаСервере()
	
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьРасходнуюНаксладную_Завершение(РезультатВыполнения, Параметры) Экспорт
	
	Если НЕ РезультатВыполнения.Свойство("Контрагент") Тогда 
		Возврат;
	КонецЕсли;
	...
	СоздатьДокументРеализации(Отказ, РезультатВыполнения.Контрагент, ДокументСсылка);
...
&НаСервере
Процедура СоздатьДокументРеализации(Отказ, Контрагент, ДокументСсылка)

	НоваяРасходная = Документы.РасходнаяНакладная.СоздатьДокумент();
	
	НоваяРасходная.Заполнить(Неопределено);
	
	НоваяРасходная.Контрагент = Контрагент;
	НоваяРасходная.Договор = Контрагент.ДоговорПоУмолчанию;
	НоваяРасходная.Дата = ТекущаяДата();
	
	НоваяРасходная.Запасы.Загрузить(Объект.Запасы.Выгрузить());
	
	Попытка 
		НоваяРасходная.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
		ДокументСсылка = НоваяРасходная.Ссылка;
	Исключение
		НоваяРасходная.Записать(РежимЗаписиДокумента.Запись);
		ДокументСсылка = НоваяРасходная.Ссылка;
	КонецПопытки;
...
&НаКлиенте
Процедура СоздатьРасходнуюНаксладную(Команда)
	
	Если Объект.Запасы.Количество() = 0 Тогда 
		Сообщить("Нет данных для переноса в реализацию");
		Возврат
	КонецЕсли;
	
	СоздатьОасходнуюНаксладнуюНаСервере();
	
	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьРасходнуюНаксладную_Завершение", ЭтотОбъект);
	
	ОткрытьФорму("Документ.ЧекККМ.Форма.ФормаВыбораКонтрагентаРасходнойНакладной",, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаСервере
Функция СформироватьСписокЭлементовСервер(Текст, УправляющиеСимволы)
...
Функция СформироватьПредставлениеЭлементаСписка(Выборка, Текст)
	...
	Представление = СтрШаблон("%1 (Артикул: %2, Код: %3)", Выборка.Представление, Выборка.Артикул, Выборка.Код);
...
//++Lazare 29.05.2023 
&НаСервере
Процедура ЗаполнитьКонсултантаПоШтрихкоду(ДанныеШтрикодов);
	
	Если ДанныеШтрикодов.количество() > 0 Тогда 		
		ТекШтрихкод =  ДанныеШтрикодов[0].Штрихкод;	
		Сптрудник = Справочники.Сотрудники.НайтиПоРеквизиту("ОсновнойШтрихКод",СокрЛП(ТекШтрихкод));
		Объект.Консультант = Сптрудник;		
	КонецЕсли;	
	Если не ЗначениеЗаполнено(Объект.Консультант) Тогда 	
		Сообщить(НСтр( "ru = 'Консультант не заполнен. Прежде чем добавлять товар, необходимо обратиться к консультанту!'; ka = 'კონსულტანტი  არაა შევსებული. საქონლის დამატებამდე აუცილებელია კონსულტანტის მითითება!'; en = 'The consultant is not filled. Before adding goods, it is necessary to refer to a consultant!'"));
	КонецЕсли;
...
// Kuzmich, 08.05.2024, (ONESOFT-2893) Изменение логики автоматической скидки в розничных продажах
&НаКлиенте
Процедура ПроверкаАвтоматическиеСкидкиСразу(СтрокаТабличнойЧасти = Неопределено)

	Если АвтоматическиеСкидкиСразу Тогда
		
		Если СтрокаТабличнойЧасти = Неопределено Тогда
			СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
		КонецЕсли;
		
		Если НЕ(СтрокаТабличнойЧасти=НЕОПРЕДЕЛЕНО) И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) И СтрокаТабличнойЧасти.Количество>0 И СтрокаТабличнойЧасти.Цена>0 Тогда
