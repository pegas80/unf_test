...
Процедура ПробитьЧек(СформироватьТоварныйЧек = Ложь)
	...
	Если НЕ ИспользоватьПодключаемоеОборудование // Проверка на включенную ФО "Использовать ПО"
		...
		Если СформироватьТоварныйЧек И НЕ Объект.Ссылка.Пустая() Тогда
			...
			Если СтруктураПринтеровВкассеККМ.Свойство("Кухня") Тогда 
				ТабличныйДокумент = ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Кухня);
				Если ТабличныйДокумент <> Неопределено Тогда 
					Попытка
						ТабличныйДокумент.ИмяПринтера = СтруктураПринтеровВкассеККМ.Кухня;
						ТабличныйДокумент.Напечатать();
					Исключение
						Сообщить(ОписаниеОшибки());
					КонецПопытки;
			...
			Если СтруктураПринтеровВкассеККМ.Свойство("Напитки") Тогда
				ТабличныйДокумент = ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Напитки);
				Если ТабличныйДокумент <> Неопределено Тогда
					Попытка
						ТабличныйДокумент.ИмяПринтера = СтруктураПринтеровВкассеККМ.Напитки;
						ТабличныйДокумент.Напечатать();
					Исключение
						Сообщить(ОписаниеОшибки());
					КонецПопытки;
			...
			Если СтруктураПринтеровВкассеККМ.Свойство("Десерты") Тогда
				ТабличныйДокумент = ПечатьНаПринтереДляРазныхОтделов(МассивЧековККМ, Десерты);
				Если ТабличныйДокумент <> Неопределено Тогда
					Попытка
						ТабличныйДокумент.ИмяПринтера = СтруктураПринтеровВкассеККМ.Десерты;
						ТабличныйДокумент.Напечатать();
					Исключение
						Сообщить(ОписаниеОшибки());
					КонецПопытки;
...
Процедура ВыполнитьСверкуИтогов(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Выполнить сверку итогов на эквайринговом терминале?';en='Perform verification of the outcome of the acquiring terminals?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
