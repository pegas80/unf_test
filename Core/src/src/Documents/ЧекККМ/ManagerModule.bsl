		...
		Для Каждого ЧекККМ Из МассивОбъектов Цикл
			...
			Если ЗначениеЗаполнено(УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ЗначениеIDДляВебСервисаНесколькихЧековДляРозничнойТорговли")) Тогда 
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					pavilion = ВыборкаДетальныеЗаписи.pavilion;//ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаДетальныеЗаписи.pavilion, Истина, Истина);//Лев(СокрЛП(ВыборкаДетальныеЗаписи.pavilion), 1);
					//pavilion = 0;
					//Попытка
					//	pavilion = Число(Номер_pavilion);
					//Исключение
					//	Прервать;
					//КонецПопытки;
