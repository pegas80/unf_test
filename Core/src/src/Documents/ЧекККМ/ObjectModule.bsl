...
Процедура ОбойтиВыборкуКассККМ(Выборка, ЗаказОснование)
	...
	Если Выборка.Количество() = 1 Тогда
		...
		Если НЕ СостояниеКассовойСмены.СменаОткрыта Тогда
			...
			ТекстСообщения = НСтр("ka='სსა სალარო ""%1""-ისთვის სალაროს ცვლა არ არის გახსნილი. ცვლის გახსნა შეიძლება ჟურნალ ""სსა-ის ჩეკები""-ში. ასევე, ჩეკის გაფორმება შეიძლება მოლარის სამუშაო ადგილზე (მსა).';ru='Не открыта кассовая смена для кассы ККМ ""%1"". Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';en='Do not open the cash changeover for cash KKM ""%1"". Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
		...
		Если НЕ ЕстьОткрытаяСмена Тогда
			...
			Если ЗаказОснование.СтруктурнаяЕдиницаРезерв.Пустая() Тогда
				Если ИспользуетсяРезервированиеЗапасов Тогда
					ТекстСообщения = НСтр("ka='Не открыта ни одна кассовая смена для основного склада (склад можно выбрать в заказе - закладка ""Дополнительно""). Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';ru='Не открыта ни одна кассовая смена для основного склада (склад можно выбрать в заказе - закладка ""Дополнительно""). Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';en='Do not open, no change in cash for the main warehouse (storage can be selected in the order - ""Advanced"" tab). Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
				Иначе
					ТекстСообщения = НСтр("ka='Не открыта ни одна кассовая смена для основного склада. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';ru='Не открыта ни одна кассовая смена для основного склада. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';en='Do not open, no change in cash for the main warehouse. Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
				КонецЕсли;
			ИначеЕсли ИспользуетсяРезервированиеЗапасов Тогда
				ТекстСообщения = НСтр("ka='არცერთი სალაროს ცვლა არ არის გახსნილი შეკვეთაში არჩეული საწყობისთვის. ცვლის გახსნა შეიძლება ჟურნალ ""სსა-ის ჩეკები""-ში. ასევე, შეიძლება ჩეკის გაფორმება მოლარის სამუშაო ადგილზე (მსა).';ru='Не открыта ни одна кассовая смена для склада, который выбран в заказе. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';en='Do not open, no change in cash for the warehouse selected in order. Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
			Иначе
				ТекстСообщения = НСтр("ka='Не открыта ни одна кассовая смена для основного склада. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';ru='Не открыта ни одна кассовая смена для основного склада. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';en='Do not open, no change in cash for the main warehouse. Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
			КонецЕсли;
			ВызватьИсключение ТекстСообщения;
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	Если НЕ ЭтоВводНаОснованииЗаказНаПродажу(ЗаказОснование) Тогда
		ВызватьИсключение НСтр("ka='სსა-ის ჩეკი შევიყვანოთ მხოლოდ მყიდველის შეკვეთის (გასაყიდად) საფუძველზე ან დამოუკიდებლად!';ru='Чек ККМ ввести только на основании заказа покупателя (на продажу) или независимо!';en=""Check the CMC to enter only on the basis of the buyer's order (for sale) or whatever!""");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОплатаПлатежнымиКартами.Количество() > 0 И НЕ ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ""ეკვაირინგის ტერმინალი"" შეუვსებელია';ru='Поле ""Эквайринговый терминал"" не заполнено';en='The ""terminal acquiring"" not filled'");
	...
	Если ОплатаПлатежнымиКартами.Итог("Сумма") > СуммаДокумента Тогда
		...
		ТекстСообщения = НСтр("ka='გადახდის ბარათებით გადახდის თანხა აღემატება დოკუმენტის თანხას';ru='Сумма оплаты платежными картами превышает сумму документа';en='payment cards payment amount exceeds the sum of the document'");
	...
	ТекстСообщения = НСтр("ka='სალაროს ცვლა გაუხსნელია';ru='Кассовая смена не открыта';en='Cash change is not open'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Статус = Перечисления.СтатусыЧековККМ.Пробит
	   ...
	   И НЕ КассаККМ.ИспользоватьБезПодключенияОборудования Тогда
		...
		ТекстСообщения = НСтр("ka='სსა-ის ჩეკი ამორტყმულია ფისკალურ რეგისტრატორზე. გატარების გაუქმება შეუძლებელია';ru='Чек ККМ пробит на фискальном регистраторе. Отмена проведения невозможна';en='Check CMC samples for Fiscal registrars. Cancellation of the impossible'");
	...
	Если НЕ ЗначениеЗаполнено(Ссылка) Тогда 
		НомерПП_ПредДокумента = 0;
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	НумераторыЧековККМДляОчереди.НомерПП
		|ИЗ
		|	РегистрСведений.НумераторыЧековККМДляОчереди КАК НумераторыЧековККМДляОчереди
		|ГДЕ
		|	НумераторыЧековККМДляОчереди.КассоваяСмена = &КассоваяСмена";
		
		Запрос.УстановитьПараметр("КассоваяСмена", КассоваяСмена.КассоваяСмена);
		РезультатЗапроса = Запрос.Выполнить();
		
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			НомерПП_ПредДокумента = ВыборкаДетальныеЗаписи.НомерПП;
		КонецЦикла;
		
		НоммерОчереди = НомерПП_ПредДокумента + 1;
	КонецЕсли;
	//--- Дмгг
...
Процедура ПриЗаписи(Отказ)
	...
	Если не Отказ Тогда 
		...
		НаботРаписейНумераторыЧековККМДляОчереди.Отбор.КассоваяСмена.Установить(Ссылка.КассоваяСмена.КассоваяСмена); 
		
		НоваяЗапись = НаботРаписейНумераторыЧековККМДляОчереди.Добавить(); 
		
		НоваяЗапись.КассоваяСмена = Ссылка.КассоваяСмена.КассоваяСмена; 
		НоваяЗапись.НомерПП = НоммерОчереди;
