...
Процедура ЗаполнитьНазначениеПлатежа()
	...
	Если ПустаяСтрока(ТекстПлатежа)
		...
		Если ТекНомер <> "" Тогда
			ТекстПлатежа = НСтр("ka='გადახდა შემდეგ ანგარიშზე N %НомерСчета%';ru='Оплата по счету N %НомерСчета%';en='Payment of the account N% NomerScheta%'");
	...
	ТекстНазначениеПлатежа = НСтр("ru = '%ТекстНазначение% ,Сумма % ТекстСумма% %ЗначениеСтавкиНДС% %ТекстНДС%'; ka = '%ТекстНазначение% თანხა %ТекстСумма% %ЗначениеСтавкиНДС% %ТекстНДС%'"
	);
	...
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%ЗначениеСтавкиНДС%", ?(БезНалогаНДС, НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)';en='Without tax (VAT)'"), НСтр("ka='მათ შორის';ru='В т.ч.';en='Including'")));
...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.СтатусУтвержденияПлатежа = Перечисления.СтатусыУтвержденияПлатежей.НеУтвержден Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести платежное поручение на основании неутвержденной заявки!';en='You can not enter a payment order on the basis of an unapproved application!'");
	КонецЕсли;
	
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести платежное поручение. В заявке указан неверный способ оплаты (тип денежных средств)!';en='You can not enter a payment order. The request has an invalid method of payment (cash type)!'");
	КонецЕсли;
