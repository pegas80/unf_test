...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	//--- Дмгг
	|	,ТаблицаЗапасы.СчетУчетаСебестоимостьРегл КАК СчетУчетаСебестоимостьРегл //Малышев 2025-04-10  ONESOFT-3991
	|ИЗ
	...
	|	,ТаблицаЗапасы.СчетУчетаСебестоимостьРегл //Малышев 2025-04-10  ONESOFT-3991
	
	//Малышев 2024-09-20 ONESOFT-3465 ((--
	|
	|ОБЪЕДИНИТЬ ВСЕ	
	|
	|ВЫБРАТЬ 
	...
	| НЕОПРЕДЕЛЕНО, //Сценарий планирования
	| ТаблицаРасходы.Организация,	
	| ТаблицаРасходы.СтруктурнаяЕдиницаТаб,	
	| ТаблицаРасходы.СтруктурнаяЕдиницаТаб,	
	| ТаблицаРасходы.СчетЗатратПФ,	
	| ТаблицаРасходы.СчетЗатратПФ,	
	| ТаблицаРасходы.СчетЗатратРеглПФ,
	| ТаблицаРасходы.СчетЗатратРеглПФ,
	| неопределено, //Номенклатура	
	| неопределено, //	
	| неопределено, //Характеристика
	| неопределено, //	
	| неопределено, //Партия	
	| неопределено, //	
	| неопределено, //ставка
	| неопределено, //ответственный
	| Ложь как Возврат, 
	...
	| ""Отражение взносов на ПФ"", //Выразить(ТаблицаРасходы.Содержание как строка(1024)), //СодержаниеПроводки
	| 0, //Количество
	| ТаблицаРасходы.СуммаПФ, //как Сумма
	| ТаблицаРасходы.СчетЗатратРеглПФ, //как СчетДтРегл
	| ТаблицаРасходы.СчетУчетаРегл, //как СчетКтРегл
	| ТаблицаРасходы.СуммаПФ, //как Себемтоимость
	| ТаблицаРасходы.Проект,
	| НЕОПРЕДЕЛЕНО //Малышев 2024-11-19 ONESOFT-3602
	| ,НЕОПРЕДЕЛЕНО //Малышев 2025-04-10  ONESOFT-3991
		
	|ИЗ 
	|	ВременнаяТаблицаРасходы как ТаблицаРасходы
	| ГДЕ
	|	ТаблицаРасходы.СуммаПФ>0 
	//29.09.2025 ONESOFT-4383
	|	И (ТаблицаРасходы.СчетЗатратРеглПФ.ТипСчета  = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.КосвенныеЗатраты) 
	|	Или ТаблицаРасходы.СчетЗатратРеглПФ.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.НезавершенноеПроизводство))   
	//29.09.2025 ONESOFT-4383
  	//Малышев 2024-09-20 ONESOFT-3465 --)	
	|";
	...
	Для каждого СтрокаДвижения из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы Цикл
		Если ЗначениеЗаполнено(СтрокаДвижения.Номенклатура) и СтрокаДвижения.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
			СтрокаДвижения.ДокументПоступления = ДокументСсылкаПриходнаяНакладная;
		КонецЕсли;	
	КонецЦикла;	
	...
	//Малышев 2024-12-16 ONESOFT-3650	
	ИначеЕсли ДокументСсылкаПриходнаяНакладная.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя Тогда
		
		Если ЗначениеЗаполнено(ДокументСсылкаПриходнаяНакладная.ДокументОснование) ТОгда
			СформироватьТаблицаЗапасыВозвратОтПокупателя(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);
		КонецЕсли;	
...
Процедура СформироватьТаблицаЗапасыВозвратОтПокупателя(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	...
	//Малышев 2025-04-08 ONESOFT-3991
	Запрос.Текст = 
	"Выбрать 
	|	Т.Организация, Т.СтруктурнаяЕдиница, Т.СчетУчетаРегл, Т.Номенклатура, Т.Характеристика, Т.Партия, Т.Проект, Т.КоррЗаказ как ЗаказПокупателя
	|Поместить ВременнаяТаблицаЗапасы2
	|из ВременнаяТаблицаЗапасы как Т";
	Запрос.Выполнить();
	ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства,,"ВременнаяТаблицаЗапасы2",,,Ложь).Выгрузить(); 
	
	//Малышев 2025-07-08 ONESOFT-4211
	Если ТаблицаЗапасыОстатки.Колонки.Найти("Порядок")=Неопределено Тогда
		ТаблицаЗапасыОстатки.Колонки.Добавить("Порядок");
		ТаблицаЗапасыОстатки.ЗаполнитьЗначения(0,"Порядок");
	КонецЕсли;	
	
	Если СтруктураДополнительныеСвойства.УчетнаяПолитика.ВозвратЗапасовПоСебестоимостиПродаж
		И СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") 
		И ЗначениеЗаполнено(СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.ДокументОснование)
		Тогда 
		Если ТаблицаЗапасыОстатки.Колонки.Найти("Порядок")=Неопределено Тогда //Малышев 2025-08-11 ONESOFT-4270
			ТаблицаЗапасыОстатки.Колонки.Добавить("Порядок");
		КонецЕсли;
		
		//Малышев 2025-06-09 ONESOFT-3828 добавим стоимость из движений отчета о рожничных продажах
		Если ЗначениеЗаполнено(ДокументСсылкаПриходнаяНакладная.ДокументОснование)  тогда
			
			ТаблицаЗапасыОстаткиККМ = УправлениеНебольшойФирмойСервер.ПолучитьДвиженияЗапасовОснования(ДокументСсылкаПриходнаяНакладная.ДокументОснование);
			Для каждого СтрокаДвижения из ТаблицаЗапасыОстаткиККМ Цикл
				НоваяСтрокаОстатка = ТаблицаЗапасыОстатки.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаОстатка, СтрокаДвижения);
				НоваяСтрокаОстатка.Порядок = -1;
			КонецЦикла;
			
			Если ТипЗнч(ДокументСсылкаПриходнаяНакладная.ДокументОснование)=Тип("ДокументСсылка.ЧекККМ")
				и ЗначениеЗаполнено(ДокументСсылкаПриходнаяНакладная.ДокументОснование.КассоваяСмена) Тогда
				ТаблицаДвиженийКассоваяСмена = УправлениеНебольшойФирмойСервер.ПолучитьДвиженияЗапасовОснования(ДокументСсылкаПриходнаяНакладная.ДокументОснование.КассоваяСмена);
				Для каждого СтрокаДвижения из ТаблицаДвиженийКассоваяСмена Цикл
					НоваяСтрокаОстатка = ТаблицаЗапасыОстатки.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаОстатка, СтрокаДвижения);
					НоваяСтрокаОстатка.Порядок = -0.5;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	//Малышев  ONESOFT-3828 Меняем порядок - сначала из остатков, потом из продажи
	ТаблицаЗапасыОстатки.Сортировать("Порядок"); 
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
				...
				Если СтрокаТаблицыОстатков.Порядок>=0 Тогда //Малышев 2025-04-08 ONESOFT-3991
					КоличествоКСписанию = КоличествоТребуется;
					Если КоличествоОстаток<>0 Тогда
						СуммаКСписанию = Окр(КоличествоТребуется * (СуммаОстаток/КоличествоОстаток),2);
					КонецЕсли;	
				
				ИначеЕсли КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуется Тогда
			...
			//если движений у документа основания нет  - то 
			Если КоличествоТребуется>0 ТОгда
				СтрокаТаблицыПриход = ВременнаяТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
				Если КоличествоТребуется <> -СтрокаТаблицыПриход.Количество и СтрокаТаблицыПриход.Количество<>0 Тогда  
					СтрокаТаблицыПриход.Количество = - КоличествоТребуется; 
					Коэффициент = КоличествоТребуется/(-СтрокаТаблицыПриход.Количество);
					СтрокаТаблицыПриход.Сумма = СтрокаТаблицыПриход.Сумма * Коэффициент; 
					СтрокаТаблицыПриход.ДокументПродажи = Неопределено;	
				КонецЕсли;	
					
				Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаПриходнаяНакладная;
				КонецЕсли;	
			КонецЕсли;	
...
Процедура СформироватьТаблицаЗапасыВозврат(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	//Малышев 2024-12-13 
	ТаблицаЗапасыОстатки2 = УправлениеНебольшойФирмойСервер.ПолучитьДвиженияЗапасовОснования(ДокументСсылкаПриходнаяНакладная.ДокументОснование);
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДоходыИРасходы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница, //+++ Дмгг
	|	ВЫБОР
	...
	|	СУММА(
	|	   ТаблицаДоходыИРасходы.СуммаБезНДС + ТаблицаДоходыИРасходы.СуммаПФ
	|	) КАК СуммаРасходов,
	|	СУММА(
	|		ТаблицаДоходыИРасходы.СуммаБезНДС + ТаблицаДоходыИРасходы.СуммаПФ
	|	) КАК Сумма
	...
	|	ТаблицаДоходыИРасходы.СтруктурнаяЕдиница, //+++ Дмгг
	|	ВЫБОР
	|		КОГДА ТаблицаДоходыИРасходы.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеРасходы)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.Прочее)
	|		ИНАЧЕ ТаблицаДоходыИРасходы.НаправлениеДеятельности
	...
	//29.09.2025 ONESOFT-4383
	|	И ТаблицаДоходыИРасходы.СчетЗатратРеглПФ.ТипСчета <> ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.КосвенныеЗатраты) 
	|	И ТаблицаДоходыИРасходы.СчетЗатратРеглПФ.ТипСчета <> ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.НезавершенноеПроизводство)   
	//29.09.2025 ONESOFT-4383
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДоходыИРасходы.Период,
	...
	|	-СУММА(
	|		ТаблицаДоходыИРасходы.СуммаБезНДС
	//Малышев 2024-12-10 ONESOFT-3619 (--
	|      - ВЫБОР
	|		    КОГДА ЕСТЬNULL(ВременнаяТаблицаБалыЛояльности.КоличествоПриход, 0) = 0  ТОГДА 0 
	|			ИНАЧЕ ЕСТЬNULL(ВременнаяТаблицаБалыЛояльности.КоличествоПриход, 0)
	|		 КОНЕЦ
	//Малышев 2024-12-10 ONESOFT-3619 --)
	|	),
	|	0,
	...
	Запрос.УстановитьПараметр("Содержание_ПродажаСБаллами",НСтр("ka='Продажа с баллами';ru='возврат Продажа с баллами';en='Продажа с баллами'"));
...
Процедура СформироватьТаблицаЗакупки(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	//+++ Дмгг, ONESOFT-2811
	//|	СУММА(ТаблицаЗакупки.СуммаНДСЗакупкиПродажи) КАК Количество,
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаЗакупки.СуммаВключаетНДС
	|				ТОГДА ТаблицаЗакупки.СуммаНДСЗакупкиПродажи
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаНДС,
	//--- Дмгг, ONESOFT-2811
	//+++
	...
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗакупки
	|ГДЕ
	|	ТаблицаЗакупки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЗакупки.Период,
	|	ТаблицаЗакупки.СтруктурнаяЕдиницаТаб,
...
Процедура СформироватьТаблицаПродажи(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаПродажи.Проект КАК Проект,  //Малышев 2024-12-16 ONESOFT-3650
	...
	|	ТаблицаПродажи.Проект,  //Малышев 2024-12-16 ONESOFT-3650
	...
	Если СтруктураДополнительныеСвойства.УчетнаяПолитика.ВозвратЗапасовПоСебестоимостиПродаж
		...
		СформироватьТаблицаПродажи_КорректировкаСебестоимости(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);
	КонецЕсли;
	//Малышев 2024-12-16 ONESOFT-3650 --)
	
КонецПроцедуры // СформироватьТаблицаПродажи()

Процедура СформироватьТаблицаПродажи_КорректировкаСебестоимости(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	
	//ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьДвиженияЗапасовОснования(ДокументСсылкаПриходнаяНакладная.ДокументОснование);
	ТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Скопировать();
	//ТаблицаЗапасыОстатки.Свернуть("Номенклатура, Характеристика, Проект", "КоличествоОстаток, СуммаОстаток");

    ВременнаяТаблицаДвиженийПродажи = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПродажи.СкопироватьКолонки();
	
	Для каждого СтрокаДвижения из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПродажи Цикл
		
		КоличествоТребуется = - СтрокаДвижения.Количество;
		
		СуммаТребуется = - СтрокаДвижения.Сумма;
		СуммаНДСТребуется = - СтрокаДвижения.СуммаНДС;
		
		СуммаОстаток = -СтрокаДвижения.Сумма;
		СуммаНДСОстаток = -СтрокаДвижения.СуммаНДС;
				
		
		Если КоличествоТребуется > 0 Тогда
			
			СтруктураДляПоиска = Новый Структура;
			СтруктураДляПоиска.Вставить("Номенклатура", СтрокаДвижения.Номенклатура);
			СтруктураДляПоиска.Вставить("Характеристика", СтрокаДвижения.Характеристика);
			СтруктураДляПоиска.Вставить("Проект", СтрокаДвижения.Проект);
			
			МассивСтрокОстатковЗапасов = ТаблицаЗапасы.НайтиСтроки(СтруктураДляПоиска);
			
			
			
			Для каждого СтрокаТаблицыЗапасыОстатков из МассивСтрокОстатковЗапасов Цикл
				
				Если КоличествоТребуется <= 0 Тогда
					Прервать;
				КонецЕсли;	
				
				КоличествоОстаток = -СтрокаТаблицыЗапасыОстатков.Количество;
				СебестоимостьОстаток = -СтрокаТаблицыЗапасыОстатков.Сумма;
				
				СуммаКСписанию = 0;
				СуммаНДСКСписанию = 0;
				
				КоличествоКСписанию = 0;
				СебестоимостьКСписанию = 0;
				
 				Если КоличествоОстаток > 0 И КоличествоТребуется > КоличествоОстаток  Тогда
					
					КоличествоКСписанию = КоличествоОстаток;
					
					Коэффициент = КоличествоОстаток / КоличествоТребуется;
					
					СуммаКСписанию = Окр(СуммаОстаток * Коэффициент , 2, 1);
					СуммаНДСКСписанию = Окр(СуммаНДСОстаток * Коэффициент , 2, 1);
					СебестоимостьКСписанию = СебестоимостьОстаток;
				...
				ИначеЕсли КоличествоОстаток >0 и КоличествоОстаток <= КоличествоТребуется Тогда
					...
					СуммаКСписанию = СуммаОстаток;
					СуммаНДСКСписанию = СуммаНДСОстаток; 
					СебестоимостьКСписанию = СебестоимостьОстаток;
				...
				СтрокаТаблицыЗапасыОстатков.Количество = СтрокаТаблицыЗапасыОстатков.Количество + КоличествоКСписанию;
				СтрокаТаблицыЗапасыОстатков.Сумма = СтрокаТаблицыЗапасыОстатков.Сумма + СебестоимостьКСписанию;
					
				
				КоличествоТребуется = КоличествоТребуется - КоличествоКСписанию; 
				
				СуммаОстаток = СуммаОстаток - СуммаКСписанию;
				СуммаНДСОстаток = СуммаНДСОстаток - СуммаНДСКСписанию;
				
				
				СтрокаДвижения.Количество = СтрокаДвижения.Количество  + КоличествоКСписанию;
				СтрокаДвижения.Сумма = СтрокаДвижения.Сумма + СуммаКСписанию;
				СтрокаДвижения.СуммаНДС = СтрокаДвижения.СуммаНДС + СуммаНДСКСписанию;
				
				//Движения
				СтрокаТаблицыПродажи = ВременнаяТаблицаДвиженийПродажи.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПродажи, СтрокаДвижения);
				СтрокаТаблицыПродажи.Количество = -КоличествоКСписанию;
				СтрокаТаблицыПродажи.Себестоимость = -СебестоимостьКСписанию;
				СтрокаТаблицыПродажи.Сумма = -СуммаКСписанию;
				СтрокаТаблицыПродажи.СуммаНДС = -СуммаНДСКСписанию;
				Если не ЗначениеЗаполнено(СтрокаТаблицыЗапасыОстатков.ДокументПродажи) ТОгда
					СтрокаТаблицыПродажи.Документ = ДокументСсылкаПриходнаяНакладная; 
				КонецЕсли;	
			КонецЦикла;	
		КонецЕсли;
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПродажи = ВременнаяТаблицаДвиженийПродажи;
...
Процедура СформироватьТаблицаЗаявкиНаВозврат(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаПродажи.Договор КАК Договор,
	|	ТаблицаПродажи.Номенклатура КАК Номенклатура,
	|	СУММА(ТаблицаПродажи.Количество) КАК Количество
	|ПОМЕСТИТЬ ВТ_ПродажиВозврат
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаПродажи
	|ГДЕ
	|	ТаблицаПродажи.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПродажиВозврат.Период КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ВТ_ПродажиВозврат.Организация КАК Организация,
	|	ВТ_ПродажиВозврат.Контрагент КАК Контрагент,
	|	ВТ_ПродажиВозврат.Договор КАК Договор,
	|	ВТ_ПродажиВозврат.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ЗаявкиНаВозвратОстатки.КоличествоОстаток, 0) < ВТ_ПродажиВозврат.Количество
	|			ТОГДА ВТ_ПродажиВозврат.Количество
	|		ИНАЧЕ ЗаявкиНаВозвратОстатки.КоличествоОстаток
	|	КОНЕЦ * -1 КАК Количество
	|ИЗ
	|	ВТ_ПродажиВозврат КАК ВТ_ПродажиВозврат,
	|	РегистрНакопления.ЗаявкиНаВозврат.Остатки(
	|			&МоментВремени,
	|			(Организация, Контрагент, Договор, Номенклатура) В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|					ВТ_ПродажиВозврат.Организация КАК Организация,
	|					ВТ_ПродажиВозврат.Контрагент КАК Контрагент,
	|					ВТ_ПродажиВозврат.Договор КАК Договор,
	|					ВТ_ПродажиВозврат.Номенклатура КАК Номенклатура
	|				ИЗ
	|					ВТ_ПродажиВозврат КАК ВТ_ПродажиВозврат)) КАК ЗаявкиНаВозвратОстатки
	|ГДЕ
	|	ЕСТЬNULL(ЗаявкиНаВозвратОстатки.КоличествоОстаток, 0) > 0";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗаявкиНаВозврат", РезультатЗапроса.Выгрузить());
...
Процедура СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗапасыНаСкладах.Характеристика КАК Характеристика,
	|	ТаблицаЗапасыНаСкладах.Партия КАК Партия,
	|	ТаблицаЗапасыНаСкладах.Проект КАК Проект,  //Малышев 2024-06-23 ONESOFT-3267
	...
	|	ТаблицаЗапасыНаСкладах.Характеристика,
	|	ТаблицаЗапасыНаСкладах.Партия,
	|	ТаблицаЗапасыНаСкладах.Проект, //Малышев 2024-06-23 ONESOFT-3267
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.ВалютаРасчетов КАК ВалютаДенежныхСредств, //Малышев 2025-07-08 
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	//Малышев 2024-1-29 ONESOFT-3619 (--
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	4 как Порядок,
	...
	|	ТаблицаДокумента.ВалютаРасчетов КАК Валюта,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг) КАК ТипРасчетов,
	|	СУММА(ТаблицаДокумента.БалыРасход) КАК Сумма,
	|	СУММА(ТаблицаДокумента.БалыРасход) КАК СуммаВал,
	|	СУММА(ТаблицаДокумента.БалыРасход) КАК СуммаДляОстатка,
	|	СУММА(ТаблицаДокумента.БалыРасход) КАК СуммаВалДляОстатка,
	|	ТаблицаДокумента.Проект КАК Проект
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	|   и ТаблицаДокумента.Документ.КартаЛояльности <> Значение(Справочник.КартыЛояльности.ПустаяССылка)
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Период,
	|	ТаблицаДокумента.Организация,
	|	ТаблицаДокумента.Контрагент,
	...
	|	ТаблицаДокумента.СчетУчетаРасчетовСПокупателемРегл,
	|	ТаблицаДокумента.Проект 
	|ИМЕЮЩИЕ СУММА(ТаблицаДокумента.БалыРасход)<>0
	//Малышев 2024-1-29 ONESOFT-3619 (--
	|";
...
Процедура СформироватьТаблицаЦеныВозвратаНоменклатурDgFIFO(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	//|	ПриходнаяНакладнаяЗапасы.ДокументОснование КАК Документ  ONESOFT-4609
	|	ВЫБОР 
	|		КОГДА ПриходнаяНакладнаяЗапасы.ДокументОснование<>НЕОПРЕДЕЛЕНО 
	|			ТОГДА ПриходнаяНакладнаяЗапасы.ДокументОснование 
	|       ИНАЧЕ ПриходнаяНакладнаяЗапасы.Ссылка.ДокументОснование
	|	КОНЕЦ КАК Документ
	|ИЗ
	|	Документ.ПриходнаяНакладная.Запасы КАК ПриходнаяНакладнаяЗапасы
	|ГДЕ
	...
	//|	ПриходнаяНакладнаяЗапасы.ДокументОснование     ONESOFT-4609
	|	ВЫБОР 
	|		КОГДА ПриходнаяНакладнаяЗапасы.ДокументОснование<>НЕОПРЕДЕЛЕНО 
	|			ТОГДА ПриходнаяНакладнаяЗапасы.ДокументОснование 
	|       ИНАЧЕ ПриходнаяНакладнаяЗапасы.Ссылка.ДокументОснование
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЦеныВозвратаНоменклатурDgFIFO", РезультатЗапроса.Выгрузить()); //Малышев 2025-12-26 ONESOFT-4609
...
Процедура СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|	КонтрольСроковГодностиТовара.Документ КАК Документ
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК КонтрольСроковГодностиТовара
	|ГДЕ
	|	КонтрольСроковГодностиТовара.СрокГодности <> ДАТАВРЕМЯ(1, 1, 1)
	|	И КонтрольСроковГодностиТовара.ВключенВрегистр = ИСТИНА";
...
Процедура СформироватьТаблицаБалыЛояльности(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	|	1 КАК Порядок,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	Таб.Ссылка.Дата КАК Период,
	|	Таб.Ссылка.КартаЛояльности КАК КартаЛояльности,
	|	Таб.Ссылка.Контрагент КАК Контрагент,
	|	Таб.Ссылка.Договор КАК Договор,
	|	СУММА(Таб.КоличествоНакопленныхБаллов) КАК Количество,
	|	СУММА(Таб.Всего) КАК Сумма
	|ИЗ
	|	ВременнаяТаблицаЗапасыПерерасчетПред КАК Таб
	|ГДЕ
	|	Таб.Ссылка.КартаЛояльности <> ЗНАЧЕНИЕ(Справочник.КартыЛояльности.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	Таб.Ссылка.Дата,
	|	Таб.Ссылка.КартаЛояльности,
	|	Таб.Ссылка.Контрагент,
	|	Таб.Ссылка.Договор
	|
	|ИМЕЮЩИЕ
	|	(СУММА(Таб.КоличествоНакопленныхБаллов) <> 0
	|		ИЛИ СУММА(Таб.Всего) <> 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗароса_ИнициализироватьДанныеДокумента()
	
	ТекстЗапроса =
	//Малышев 2024-12-09 ONESOFT-3619 (--
	"ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.ПриходнаяНакладная КАК РасходнаяНакладная
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			СУММА(ПродажиОбороты.СуммаОборот) КАК СуммаОборот,
	|			ПродажиОбороты.Контрагент КАК Контрагент
	|			ИЗ
	|					РегистрНакопления.Продажи.Обороты(, &ДатаСреза, Регистратор, Контрагент = &Контрагент) КАК ПродажиОбороты
	|				ГДЕ
	...
	|	ПриходнаяНакладнаяЗапасы.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР
	...
	|	ВЫБОР
	//+++ Дмгг, Осталась ставка в строке из-за этого был глюк
	//|		КОГДА ЕСТЬNULL(ПриходнаяНакладнаяЗапасы.СтавкаНДС.Ставка, 0) = 0 
	|		КОГДА ЕСТЬNULL(ПриходнаяНакладнаяЗапасы.СтавкаНДС.Ставка, 0) = 0 
	|			ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|			ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НаЭкспорт)
	//--- Дмгг
	|			ТОГДА 0
	...
	|	ВЫБОР
	|		КОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.МетодОценки = ЗНАЧЕНИЕ(Перечисление.МетодОценкиЗапасов.FIFO) ТОГДА &Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	//Малышев 2024-11-19 ONESOFT-3602
	
	//Малышев 2024-11-29 ONESOFT-3619
	|	ПриходнаяНакладнаяЗапасы.БалыРасход КАК БалыРасход,
	|	ПриходнаяНакладнаяЗапасы.КоличествоНакопленныхБаллов КАК КоличествоНакопленныхБаллов,
	//Малышев 2024-11-29 ONESOFT-3619
	
	
	|	ПриходнаяНакладнаяЗапасы.СрокГодности КАК СрокГодности,
	|	ПриходнаяНакладнаяЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр
	...
	|	ВременнаяТаблицаЗапасыПерерасчет.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВременнаяТаблицаЗапасыПерерасчет.НеобходимостьВводаАкцизнойМарки КАК НеобходимостьВводаАкцизнойМарки,
	...
	|	ВременнаяТаблицаЗапасыПерерасчет.СуммаРасходовНДФЛ КАК СуммаРасходовНДФЛ,
	|	ВременнаяТаблицаЗапасыПерерасчет.СтруктурнаяЕдиницаТаб КАК СтруктурнаяЕдиницаТаб,
	|	ВременнаяТаблицаЗапасыПерерасчет.СуммаРасходовПФ КАК СуммаРасходовПФ,
	...
	|	ВременнаяТаблицаЗапасыПерерасчет.ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
	|	ВременнаяТаблицаЗапасыПерерасчет.БалыРасход, //Малышев 2024-11-29 ONESOFT-3619
	|	ВременнаяТаблицаЗапасыПерерасчет.СрокГодности КАК СрокГодности,
	|	ВременнаяТаблицаЗапасыПерерасчет.ВключенВрегистр КАК ВключенВрегистр	
	...
	|	ПриходнаяНакладная.КартаЛояльности КАК КартаЛояльности, 
	//Малышев 2024-12-09 ONESOFT-3619
	|	ВЫБОР
	...
	|	ПриходнаяНакладная.ВидНалогаНДС КАК ВидНалогаНДС,
	|	ВЫБОР
	...
	|	ПриходнаяНакладнаяЗапасы.Всего КАК ВсегоВалютаДокумента, //Малышев 2024-12-12 
	|	ПриходнаяНакладнаяЗапасы.Ссылка.КартаЛояльности КАК КартаЛояльности, //Малышев 2024-12-12 
	...
	|	ПриходнаяНакладнаяЗапасы.СтруктурнаяЕдиницаТаб КАК СтруктурнаяЕдиницаТаб,
	...
	|	ПриходнаяНакладнаяЗапасы.ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
	|	ПриходнаяНакладнаяЗапасы.БалыРасход, //Малышев 2024-11-29 ONESOFT-3619
	|	ПриходнаяНакладнаяЗапасы.СрокГодности КАК СрокГодности,
	|	ПриходнаяНакладнаяЗапасы.ВключенВрегистр КАК ВключенВрегистр	
	...
	|;
	|" + БаллыЛояльностьСкидки.ПолучитьТекстЗапросаИнициализации_ПриходнаяНакладнаяВозврат();
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ТекстЗароса_ИнициализироватьДанныеДокумента();
	...
	Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам); //Малышев 2024-06-11 ONESOFT-3094
    Запрос.УстановитьПараметр("Контрагент", ДокументСсылкаПриходнаяНакладная.Контрагент);
	
	
	//--------------------------------------------------------------------------
	// © Шишка В.В. [August 11, 2022]  НАЧАЛО
	Если ДокументСсылкаПриходнаяНакладная.СуммаДокумента >= 5000000 Тогда
	
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "КАК ЧИСЛО(15, 8)", "КАК ЧИСЛО(15, 2)");
		
	КонецЕсли; 
	// © Шишка В.В. [August 11, 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	СформироватьТаблицаРазмещениеЗаказов(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства); //Малышев 2025-04-14 ONESOFT-4004
	СформироватьТаблицаЗапасы(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);  //Малышев 2025-04-08 ONESOFT-3991
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [3 октября 2023]  НАЧАЛО
	СформироватьТаблицаЗаявкиНаВозврат(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);
	// © Шишка В.В. [3 октября 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВозвратОтПокупателяDG") Тогда 		
		СформироватьТаблицаЦеныВозвратаНоменклатурDgFIFO(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);//++Lazare
	КонецЕсли;
	
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("КонтрольСроковГодностиТовара")
		И ДокументСсылкаПриходнаяНакладная.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика  Тогда 
		СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства); //++Lazare
	КонецЕсли;
	
	//Малышев 2024-11-29 ONESOFT-3619
	СформироватьТаблицаБалыЛояльности(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПриходнаяНакладная, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь, ПроверятьИтогиРегистров = ЛОЖЬ) Экспорт
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВозвратОтПокупателяDG")  
		и ЗначениеЗаполнено(ДокументСсылкаПриходнаяНакладная.ДокументОснование) 		                    //Малышев 2025-12-26 ONESOFT-4609
		и ТипЗнч(ДокументСсылкаПриходнаяНакладная.ДокументОснование)=Тип("ДокументСсылка.РасходнаяНакладная") //Малышев 2025-12-26 ONESOFT-4609
		и СтруктураВременныеТаблицы.свойство("ДвиженияЦеныВозвратаНоменклатурDgFIFOИзменение") Тогда 
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Проект",  НСтр("ru = 'Проект (наименование)'; ka = 'პროექტი (დასახელება)'; en = 'Project (name)'"), ОписаниеТиповСтрока150, Новый ОписаниеТипов("СправочникСсылка.Проекты"));
	// Олег Литвиненко <---
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных) Экспорт
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		Если ПолноеИмяОбъектаЗаполнения = "Документ.ПриходнаяНакладная.ТабличнаяЧасть.Запасы" Тогда
			...
			// Олег Литвиненко 12.07.23 --->
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьПроект(СтрокаТаблицыФормы.Проект, СтрокаТаблицыФормы.Проект_ВходящиеДанные);
			// Олег Литвиненко <---
...
//--------------------------------------------------------------------------
// © Шишка В.В. [14 сентября 2023]  НАЧАЛО
Процедура СформироватьПриходнуюНакладнуюСРасходамиЛадлшафт(Запрос, ТабличныйДокумент)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПриходнаяНакладная_НакладнаяЛандшафт";
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт;
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриходнаяНакладная.ПФ_MXL_НакладнаяГоризонтально");
	СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация, Шапка.ДатаДокумента, ,);
	СведенияОбКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Контрагент, Шапка.ДатаДокумента, ,);
...
// © Шишка В.В. [14 сентября 2023]  КОНЕЦ
//--------------------------------------------------------------------------
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ИначеЕсли ИмяМакета = "НакладнаяСРасходами" Тогда
			...
			СформироватьПриходнуюНакладнуюСРасходамиЛадлшафт(Запрос, ТабличныйДокумент);
			// © Шишка В.В. [14 сентября 2023]  КОНЕЦ
			//--------------------------------------------------------------------------
