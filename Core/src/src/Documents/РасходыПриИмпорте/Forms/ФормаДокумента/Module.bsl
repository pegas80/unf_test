...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ТаможенныйШтраф <> 0
		...
		ТекстОшибки = НСтр("ka='შეამოწმეთ საქმიანობის მიმართულების შევსება და დანახარჯების ანგარიში';ru='Проверьте заполнение направления деятельности и счета затрат';en='Check the filling of the activities and expenditures account'");
...
Процедура ЗапасыВставитьСтроки(Команда)
	...
	Если СтрокаРаздела = Неопределено Тогда
		...
		ТекстОшибки = НСтр("ka='აუცილებელია იმ ნაწილის გამოყოფა, რომლისთვის ხორციელდება მარაგის დამატება';ru='Необходимо выделить раздел, для которого добавляются запасы';en='You need to select the key for which reserves are added'");
...
Процедура РазделыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НЕ ДанныеТекущейСтроки = Неопределено Тогда
		...
		Если Объект.Разделы.Количество() > 1 Тогда
			...
			Элементы.ЗапасыИзменениеСтрокДействие.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьИсходноеМесто"), НСтр("ka='სტრიქონების გადატანა სხვა ნაწილში';ru='Перенести строки в другой раздел';en='Transfer line to another section'"));
...
Процедура ЗаполнитьСведенияОРазделе(ИдентификаторСтроки)
	...
	Если ИдентификаторСтроки <> Неопределено Тогда
		...
		СтрокаСведений = НСтр("ka='ამ ნაწილში საქონლის სტრიქონები: %1';ru='В этом разделе строк товаров: %1';en='In this section, the rows of goods:% 1'");
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	ТекстНадписи = ТекстНадписи 
		...
		+ ?(СтруктураНадписи.СуммаВключаетНДС, НСтр("ka='თანხა შეიცავს დღგ-ს';ru='Сумма включает НДС';en='The sum includes VAT'"), НСтр("ka='თანხა არ შეიცავს დღგ-ს';ru='Сумма не включает НДС';en='The sum does not include VAT'"));
...
"Распределение невозможно.';ka='ნაწილ %1-ის ფაქტურული ღირებულების საერთო თანხა ნულოვანია! განაწილება შეუძლებელია.'");
...
Процедура ОбработатьТаблицу()
	...
	Если СтрокаРаздела = Неопределено Тогда
		...
		ТекстОшибки = НСтр("ka='მიუთითეთ სატვირთო-საბაჟო დეკლარაციის ნაწილი, რომელშიც საჭიროა საქონლების დამატება';ru='Укажите раздел ГТД, в который необходимо добавить товары';en='Specify the section of GTE, which you want to add products'");
...
Процедура ЗаполнитьСписокДействий()
	...
	Если ОбменСБухгалтериейНастроен Тогда
		...
		СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ПодобратьДокументыПартий,		НСтр("ka='დოკუმენტების გადარჩევა პარტიები';ru='Подобрать документы партий';en='Parties pick up the documents'"));
		
	КонецЕсли;
	
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента,			НСтр("ka='სტრიქონების დამატება დოკუმენტიდან';ru='Добавить строки из документа';en='Add a line in the document'"));
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ПодобратьНомераГТД,				НСтр("ka='სატვირთო საბაჟო დეკლარაციის ნომრის მიხედვით სტრიქონების დამატება შემოსვლის ზედნადებებიდან';ru='Добавить строки по номеру ГТД из приходных накладных';en='Add a line of GTD number of Invoice'"));
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам,		НСтр("ka='საბაჟო მოსაკრებლის განაწილების თანხების დაზუსტება';ru='Уточнить суммы распределения таможенного сбора';en='Clarify the distribution of the amount of customs duty'"));
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.УдалитьСтроки, 					НСтр("ka='სტრიქონების წაშლა';ru='Удалить строки';en='Remove row'"));
	
	Если Объект.Разделы.Количество() > 1 Тогда
		
		СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.УстановитьИсходноеМесто,	НСтр("ka='სტრიქონების გადატანა სხვა ნაწილში';ru='Перенести строки в другой раздел';en='Transfer line to another section'"));
...
Процедура ДобавитьСтрокиВРазделИзДокумента(ИдентификаторРаздела, Ошибки)
	...
	Если НЕ ЗначениеЗаполнено(ЗапасыИзменениеСтрокЗначение) Тогда
		...
		ТекстОшибки = НСтр("ka='შესავსებად სავალდებულოა დოკუმენტის არჩევა';ru='Для заполнения необходимо выбрать документ';en='To fill the need to select the document'");
...
Процедура ДобавитьСтрокиВРазделИзДокументовПриходнаяНакладная(ИдентификаторРаздела, Ошибки)
	...
	Если НЕ ЗначениеЗаполнено(Объект.НомерГТД) Тогда
		...
		ТекстОшибки = НСтр("ka='მიუთითეთ სატვირთო-საბაჟო დეკლარაციის ნომერი';ru='Укажите номер ГТД';en='Specify the number of GTE'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Разделы", ТекстОшибки, "");
