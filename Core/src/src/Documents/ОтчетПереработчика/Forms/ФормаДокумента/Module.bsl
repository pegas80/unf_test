...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НайденныйСчетФактура = УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйСчетФактуруГрузия(Объект.Ссылка, Истина);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Документ.СчетФактураПолученныйГрузия.Форма.ФормаДокумента" Тогда
...
Процедура СчетФактураНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	УправлениеНебольшойФирмойКлиент.ОткрытьСчетФактуруГрузия(ЭтаФорма, Истина);
...
Процедура КомандаЗаполнитьПоСпецификации(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Запасы"" будет перезаполнена! Продолжить выполнение операции?';en='Tabular part of the ""reserve"" will be refilled! To continue the operation?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ОбработатьИзменениеВидаЦенИВалютыРасчетов(ПараметрыДокумента)
	...
	Если ОткрытьФормуЦеныИВалюты Тогда
		...
		Если ИзменилсяВидЦен Тогда
			...
			ТекстПредупреждения = НСтр("ka='კონტრაგენტთან ხელშეკრულება ითვალისწინებს დოკუმენტში დადგენილისგან განსხვავებულ ფასების ტიპს! გადავთვალოთ დოკუმენტი ხელშეკრულების შესაბამისად?';ru='Договор с контрагентом предусматривает вид цен,"
...
"Необходимо проверить валюту документа!'");
	...
	ИначеЕсли ВопросВидЦен Тогда
		...
		Если ПересчетНеобходим Тогда
			...
			ТекстВопроса = НСтр("ka='კონტრაგენტთან ხელშეკრულება ითვალისწინებს დოკუმენტში დადგენილისგან განსხვავებულ ფასების ტიპს! გადავთვალოთ დოკუმენტი ხელშეკრულების შესაბამისად?';ru='Договор с контрагентом предусматривает вид цен,"
"отличный от установленного в документе! "
