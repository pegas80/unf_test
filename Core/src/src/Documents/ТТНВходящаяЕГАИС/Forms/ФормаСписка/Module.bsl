...
Процедура ВыгрузкаДокумента_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		Если ДополнительныеПараметры.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросНаОтменуПроведенияТТН") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Запрос на отмену проведения отправлен.';ru='Запрос на отмену проведения отправлен.';en='The request for cancellation of sent.'"));
		Иначе
