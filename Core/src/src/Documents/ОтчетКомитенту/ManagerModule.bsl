...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ЗадолженностьКомитенту", НСтр("ka='კომიტენტისადმი დავალიანება';ru='Задолженность комитенту';en='payable to the principal'"));
...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗапасыПринятые.СчетУчета КАК СчетУчетаРегл,//+++ Дмгг
	...
	|	ТаблицаЗапасыПринятые.СчетУчета,//+++ Дмгг
	...
	Запрос.УстановитьПараметр("ПриемЗапасовОтложеннаяВыручка", НСтр("ka='მარაგების მიღება';ru='Прием запасов';en='Admission stocks'"));
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПоставщиком", НСтр("ka='მომწოდებლის წინაშე ვალდებულებების წარმოშობა';ru='Возникновение обязательств перед поставщикт';en='The emergence of obligations to postavschikt'"));
	...
	|	ТаблицаДокумента.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРегл, //+++Дмгг
	...
	|	ТаблицаДокумента.ВалютаРасчетов,
	|	ТаблицаДокумента.СчетУчетаРасчетовСПоставщиком,
	|	ТаблицаДокумента.СчетУчетаРасчетовСПоставщикомРегл, //+++Дмгг
	//ПодольскийАИ #УчетПроектов 15.02.2021 {
	...
	|	ТаблицаДокумента.ВестиРасчетыПоДокументам,
	|	ТаблицаДокумента.СчетУчетаАвансовПоставщику,
	|	ТаблицаДокумента.СчетУчетаАвансовПоставщикуРегл, //+++ Дмгг
	|	ТаблицаДокумента.Договор,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВестиРасчетыПоДокументам
	|			ТОГДА ТаблицаДокумента.Документ
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ОтчетКомитентуЗапасы.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРасчетовСПоставщикомРегл, //+++ Дмгг
	...
	|		ИНАЧЕ ОтчетКомитентуЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаВыручкиОтПродажРегл
	|	КОНЕЦ КАК СчетУчетаПродажиРегл,
	|	ОтчетКомитентуЗапасы.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
	//--- Дмгг
...
Процедура СформироватьОтчетКомитенту(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Отчет комитенту № ';ka='Отчет комитенту № '") + НомерДокумента + " от " + Формат(Шапка.Дата, "ДЛФ=DD");
		...
		ОбластьМакета.Параметры.СуммаВознаграждения = НСтр("ka='Сумма комиссионного вознаграждения составила ';ru='Сумма комиссионного вознаграждения составила '")
			+ УправлениеНебольшойФирмойСервер.СформироватьСуммуПрописью(Шапка.СуммаВознаграждения, Шапка.ВалютаДокумента);
		ОбластьМакета.Параметры.ИтоговаяСтрока = НСтр("ka='Всего наименований ';ru='Всего наименований '") + ВыборкаСтрокТовары.Количество() + НСтр("ka=', на сумму ';ru=', на сумму '")
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='კომიტენტის ანგარიშგება';ru='Отчет комитенту';en='report to the principal'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='კომიტენტის ანგარიშგება';ru='Отчет комитенту';en='report to the principal'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
