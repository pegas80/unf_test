...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НомераКиЗСтоимость.Заголовок = СтрЗаменить(НСтр("ka='Стоимость (%Валюта%)';ru='Стоимость (%Валюта%)';en='Cost (% Currency%)'"), "%Валюта%", ВалютаРегламентированногоУчета);
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='შეტყობინება მარკირების სისტემის მარკირებული საქონლის შემოსვლის შესახებ შეიცვალა. ჩავწეროთ?';ru='Уведомление о поступлении маркированных товаров ГИСМ было изменено. Записать?';en='Notification to the marked goods GISM was changed. Record?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ka='შეტყობინებები მარკირების სისტემის მარკირებული საქონლის შემოსვლის შესახებ არ არის ჩაწერილი. ჩავწეროთ?';ru='Уведомление о поступлении маркированных товаров ГИСМ не записано. Записать?';en='Notification to the marked goods GISM not recorded. Record?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура СопоставитьПоИННКПП(Команда)
	...
	Если Не КонтрагентНайден Тогда
		Если ЕстьПравоСозданияКонтрагента Тогда
			ТекстВопроса = НСтр("ka='აღნიშნული გსნ-ით და აამკ-ით კონტრაგენტი ვერ მოიძებნა. შევქმნათ ახალი?';ru='Контрагент с указанными ИНН и КПП не найден. Создать нового?';en='Counterparty with the specified VAT and found the CAT. Create new?'");
			ПоказатьВопрос(Новый ОписаниеОповещения("СоздатьНовогоКонтрагента", ЭтотОбъект), ТекстВопроса,РежимДиалогаВопрос.ДаНет);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აღნიშნული გსნ-ით და აამკ-ით კონტრაგენტი ვერ მოიძებნა.';ru='Контрагент с указанными ИНН и КПП не найден.';en='Counterparty with the specified VAT and found the CAT.'"),
			                                                  ,
			                                                  "GLNКонтрагента",
			                                                  "Объект");
		КонецЕсли;
...
Процедура ЗаполнитьПоступления(Команда)
	...
	ТекстВопроса = НСтр("ka='დაუდასტურებელი სსნ-ით შემოსვლის დოკუმენტები გადაივსება. გავაგრძელოთ?';ru='Документы поступления по неподтвержденным КиЗ будут перезаполнены. Продолжить?';en='receipt of documents on unconfirmed Keys will be refilled. Proceed?'");
	ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ПодтвердитьПолучениеВыделенныхСтрок(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='დასადასტურებლად არცერთი სტრიქონი არ არის არჩეული.';ru='Не выбрано ни одной строки для подтверждения.';en='Unspecified single line to confirm.'");
	...
	Если КоличествоУстановленныхСостоянийКПодтверждению > 0 Тогда
		ТекстОповещения = НСтр("ka='მდგომარეობა ""გადასაცემად"" დაყენებულია %2-დან %1 სტრიქონისთვის';ru='Состояние ""К передаче"" установлено для %1 из %2 строк';en='Condition ""To transfer"" is set to 1% of rows 2%'");
	...
	Иначе
		ТекстОповещения = НСтр("ka='Отсутствуют строки с состоянием ""Подтвердить"" или ""Отклонено ГИСМ""';ru='Отсутствуют строки с состоянием ""Подтвердить"" или ""Отклонено ГИСМ""';en='No line with the state of the ""Confirm"" or ""Rejected GISM""'");
	КонецЕсли;
	ТекстЗаголовка  = НСтр("ka='მიღების დადასტურება';ru='Подтверждение получения';en='Delivery confirmation'");
...
Процедура ПодтвердитьПолучение(Команда)
	...
	Если КоличествоСтрок = 0 Тогда
		ТекстСообщения = НСтр("ka='დასადასტურებლად სტრიქონები არ არის.';ru='Нет строк для подтверждения.';en='No lines to confirm.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='<არ შემოსულა>';ru='<Не поступало>';en='<There were no>'"));
