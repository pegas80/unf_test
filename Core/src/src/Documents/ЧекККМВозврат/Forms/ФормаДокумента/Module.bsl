...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти = Неопределено)
	...
	//Малышев 2025-01-21 ONESOFT-3781 (--
	...
	//КонецЕсли;
	СтруктураСкидки = ВернутьСтруктурудляНоменклатуры(СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Характеристика, Объект.Дата);
	СпособОкругления = СтруктураСкидки.СпособОкругления;
	Если ЗначениеЗаполнено(СпособОкругления) ТОгда
		...
		Иначе
			ЦенаСоСкидкой 		= Окр(ЦенаБезОкругления + ?(СпособОкругления = ПредопределенноеЗначение("Перечисление.СпособыОкругления.Вверх"), 0.5, -0.5), 0);
		КонецЕсли;
	...
	//Малышев 2024-08-15
	Если Объект.ОплатаПлатежнымиКартами.Количество()=1 и
		Объект.ОплатаПлатежнымиКартами.Итог("Сумма")<>Объект.Запасы.Итог("Сумма") ТОгда
		Объект.ОплатаПлатежнымиКартами[0].Сумма = Объект.Запасы.Итог("Сумма");
	КонецЕсли;	

КонецПроцедуры // РассчитатьСуммуВСтрокеТабличнойЧасти()

&НаСервереБезКонтекста
Функция ВернутьСтруктурудляНоменклатуры(Номенклатура, Характеристика, ДатаДокумента)
	
	СтруктураДляНоменклатуры = Новый Структура;
	СтруктураСкидки = ПовторноеИспользованиеСервер.ПолучитьСтруктуруСкидкиИзКлиента(Номенклатура, Характеристика, ДатаДокумента);
	
	Если СтруктураСкидки<>Неопределено ТОгда
		СтруктураДляНоменклатуры.Вставить("СпособОкругления", СтруктураСкидки.СпособОкругления);	
		СтруктураДляНоменклатуры.Вставить("ПроцентСкидки", СтруктураСкидки.ПроцентСкидки);	
		СтруктураДляНоменклатуры.Вставить("Цена", СтруктураСкидки.Цена);	
	Иначе
		СтруктураДляНоменклатуры.Вставить("СпособОкругления", Неопределено);	
		СтруктураДляНоменклатуры.Вставить("ПроцентСкидки", 0);	
		СтруктураДляНоменклатуры.Вставить("Цена", 0);	
	КонецЕсли;
	
	Возврат СтруктураДляНоменклатуры;

КонецФункции // ВернутьСтруктурудляНоменклатуры()
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Документ.ОбработкаПроверкиЗаполнения() ;
	...
	Если НЕ ЭТИспользоватьБезПодключенияОборудования Тогда
		Для каждого ТекСтрока Из Объект.ОплатаПлатежнымиКартами Цикл
			Если НЕ ТекСтрока.ОплатаОтменена Тогда
				ТекстСообщения = НСтр("ka='თავდაპირველად, საჭიროა ბარათებით გადახდის დაბრუნების შესრულება!';ru='Вначале нужно выполнить возврат оплаты по картам!';en='First, you need to perform a refund on the cards!'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОтменитьОплату(Команда)
	...
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтотОбъект, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='ბარათით გადახდის დაბრუნება ვერ მოხერხდა.';ru='Не удалось выполнить возврат оплаты картой.';en='Failed refunds card.'"));
		Возврат;
	КонецЕсли;
	...
	Если ИдентификаторУстройстваЭТ <> Неопределено 
		...
		//Малышев 2024-08-15 - частичная отмена
		СуммаДокументаОснования = СуммаНачальногоЧека();
		Если СуммаДокументаОснования > СуммаОперации Тогда 
			СтруктураЧастичнойОтмены = Новый Структура;
			СтруктураЧастичнойОтмены.Вставить("СсылочныйНомер", НомерСсылкиОперации);
			СтруктураЧастичнойОтмены.Вставить("НомерЧека", ПараметрыОперации.НомерЧека);
			СтруктураЧастичнойОтмены.Вставить("ТипТранзакции", "AuthorizeVoidPartial");
			СтруктураЧастичнойОтмены.Вставить("ИзначальнаяСумма", СуммаДокументаОснования);
			
			ПараметрыОперации.СсылочныйНомер = TransLinkRESTAPI.ИзДанных1с_в_JSON(СтруктураЧастичнойОтмены);
		КонецЕсли;	
...
&НаСервере
Функция СуммаНачальногоЧека()
	Возврат Объект.ЧекККМ.СуммаДокумента;
КонецФункции
