	...
	Если ЗначенияРеквизитов.Свойство("ВидОперации") Тогда
		Если ЗначениеЗаполнено(ЗначенияРеквизитов.ВидОперации)
			И НЕ ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
				ТекстИсключения = НСтр("ka='გასავლის ორდერის შეყვანა შეიძლება მხოლოდ შეკვეთა-განწესის საფუძველზე!';ru='Расходный ордер можно ввести только на основании заказ-наряда!';en='Withdrawal slip, you can enter only on the basis of the purchase order!'");
