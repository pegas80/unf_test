...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ka='При создании настройки обмена во второй информационной базе возникли ошибки.';ru='При создании настройки обмена во второй информационной базе возникли ошибки.';en='When creating a setup for the second exchange of information database errors occurred.'");
		КонецЕсли;
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ka='Возникла ошибка при создании настройки синхронизации данных.';ru='Возникла ошибка при создании настройки синхронизации данных.';en='An error occurred when creating the configuration database synchronization.'"));
		КонецЕсли;
...
Процедура ОбновитьНастройкиОбменаДанными(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ka='Возникла ошибка при обновлении настройки синхронизации данных.';ru='Возникла ошибка при обновлении настройки синхронизации данных.';en='An error occurred while updating configuration data synchronization.'"));
		КонецЕсли;
...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ka='При создании настройки синхронизации данных возникли ошибки.';ru='При создании настройки синхронизации данных возникли ошибки.';en='When you create a configuration synchronization errors occur.'"));
		КонецЕсли;
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле, КодЭтогоУзла, КодНовогоУзла)
	...
	Если ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		Если ГлавныйУзел = Неопределено Тогда
			...
			ВызватьИсключение НСтр("ka='Главный узел для текущей информационной базы не определен."
"Возможно, информационная база не является подчиненным узлом в РИБ.';ru='Главный узел для текущей информационной базы не определен."
...
"В системе уже существует синхронизация данных для информационной базы (программы) с указанным префиксом.';ka='Значение префикса первой информационной базы не уникально."
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если СтруктураНастроек.Свойство("ИмяПланаОбмена")
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ka='Файл содержит настройки обмена для другой информационной базы.';ru='Файл содержит настройки обмена для другой информационной базы.';en='This file contains the settings for the exchange of other information base.'");
	...
	Если Не Отказ Тогда
		...
		Если ВерсияФорматаФайлаНастроекОбменаДанными = "1.0" Тогда
			...
			ЭтотОбъект.НаименованиеЭтойБазы = НСтр("ka='Эта информационная база';ru='Эта информационная база';en='This information base'");
	...
	Если Не ПустаяСтрока(ПрефиксИнформационнойБазы)
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ka='На первом этапе настройки обмена был неправильно указан префикс второй информационной базы."
"Настройку обмена требуется начать заново.';ru='На первом этапе настройки обмена был неправильно указан префикс второй информационной базы."
