...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ka='ИФНС приняла отчет 6-НДФЛ?';ru='ИФНС приняла отчет 6-НДФЛ?';en='The IRS has adopted a report 6-PIT?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ka='Вы подтверждаете, что отчет 6-НДФЛ заполнен полностью и корректно?';ru='Вы подтверждаете, что отчет 6-НДФЛ заполнен полностью и корректно?';en='You acknowledge that the report is 6-PIT is filled completely and correctly?'"), РежимДиалогаВопрос.ДаНет);
