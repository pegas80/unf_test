...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияГиперссылкаКабинетКлиента.Подсказка =
		СтрШаблон(
			НСтр("ka='Логин: %1';ru='Логин: %1';en='Username:% 1'"),
			?(ПустаяСтрока(ЭтотОбъект.Логин), НСтр("ka='не введен, будет запрошен в браузере.';ru='не введен, будет запрошен в браузере.';en='is not entered, you are prompted in the browser.'"), ЭтотОбъект.Логин));
	...
	Элементы.ФильтрПоЭтойИБ.Подсказка = СтрШаблон(НСтр("ka='Идентификатор ИБ: %1';ru='Идентификатор ИБ: %1';en='IB identifier:% 1'"), ЭтотОбъект.ИдентификаторИБ);
...
Процедура КомандаУдалитьФайл(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		СписокКнопок.Добавить(НСтр("ka='წაშლა';ru='Удалить';en='Delete'")); // Идентификатор
		СписокКнопок.Добавить(НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'")); // Идентификатор
		...
		ПоказатьВопрос(
			...
			НСтр("ka='Удалить файл с сервера?';ru='Удалить файл с сервера?';en='Delete the file from the server?'"),
			СписокКнопок,
			0,
			НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"), // Идентификатор
			НСтр("ka='Запрос на удаление файла с сервера';ru='Запрос на удаление файла с сервера';en='Request to delete the file from the server'"));
	КонецЕсли;
...
Процедура УдалитьФайл_НаКлиенте(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = НСтр("ka='წაშლა';ru='Удалить';en='Delete'") Тогда // Идентификатор
