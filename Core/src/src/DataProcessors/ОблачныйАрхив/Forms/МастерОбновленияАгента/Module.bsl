...
#Область ДекорацияИнформацияОНовомАгентеРезервногоКопированияТекст
	...
	МассивПодстрок.Добавить(НСтр("ka='Версия дистрибутива:';ru='Версия дистрибутива:';en='distribution Version:'"));
	...
	МассивПодстрок.Добавить(НСтр("ka='კონფიგურაციის ზომა:';ru='Размер дистрибутива:';en='Size:'"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстЧтоНовогоВВерсии) Тогда
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("ka='Новое в версии.';ru='Новое в версии.';en='New in version.'"),
					,
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстПорядокОбновления) Тогда
		МассивПодстрок.Добавить(НСтр("ka='Перед установкой обновления необходимо ознакомиться с';ru='Перед установкой обновления необходимо ознакомиться с';en='Before installing the update, you must familiarize yourself with'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("ka='особенностями перехода';ru='особенностями перехода';en='features of transition'"),
					,
					ЦветаСтиля.ЦветГиперссылки,
					,
					"backup1C:BackupAgentUpdate_HowToUpdate")); // Идентификатор.
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(НСтр("ka='на эту версию Агента резервного копирования.';ru='на эту версию Агента резервного копирования.';en='this version of Backup Agent.'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;

	Если ЭтотОбъект.АгентКопированияУстановлен Тогда
		МассивПодстрок.Добавить(НСтр("ka='Установлен Агент резервного копирования версии:';ru='Установлен Агент резервного копирования версии:';en='Installed backup agent version:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
		...
		МассивПодстрок.Добавить(Символы.ПС);
	Иначе
		МассивПодстрок.Добавить(НСтр("ka='Агент резервного копирования не установлен';ru='Агент резервного копирования не установлен';en='Backup Agent is not installed'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;
...
"';ka='Для обновления Агента резервного копирования необходимо обладать правами администратора Windows."
""
"Во время обновления произошли ошибки."
"Информация об ошибках сохранена в журнал регистрации."
"'"),
					Новый ФорматированнаяСтрока(
						НСтр("ka='Журнал регистрации.';ru='Журнал регистрации.';en='log.'"),
						,
						ЦветаСтиля.ЦветГиперссылки,
						,
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаОбновлениеАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			...
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ka='Загрузка и обновление Агента резервного копирования.';ru='Загрузка и обновление Агента резервного копирования.';en='Downloads and Upgrades Backup Agent.'");
...
#Область Окончание
		...
		Если ЕстьОшибки = Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ka='შეცდომები არის';ru='Есть ошибки';en='There are mistakes'"));
