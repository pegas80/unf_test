...
Процедура ПодробнееОбОтчетеНажатие(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(ТекДеревоОтчетов.ТекущиеДанные.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Для выбранного отчета вывод подробной информации не предусмотрен.';ru='Для выбранного отчета вывод подробной информации не предусмотрен.';en='For the selected report displays detailed information is not provided.'"));
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ДанныеОтчета.ПолноеИмя = "Документ.РегламентированныйОтчет" Тогда
		...
		ИначеЕсли ТипЗнч(РезультатОткрытияФормыНаСервере) = Тип("Структура") Тогда
			...
			Если ИсточникОтчета = "РегламентированныйОтчетРСВ1" Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='Для формирования формы РСВ-1 используйте календарь налогов и отчетности!';ru='Для формирования формы РСВ-1 используйте календарь налогов и отчетности!';en='For forming the shape of the DAM-1 use a calendar tax and reporting!'");
				Сообщение.Сообщить();
