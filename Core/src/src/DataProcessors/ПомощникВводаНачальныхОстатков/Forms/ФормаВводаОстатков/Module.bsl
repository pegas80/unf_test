...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстЗавершения, СтандартнаяОбработка)
	...
	Если НЕ мЗаписьФормыОсуществлена
		...
		Текст = НСтр("ka='Сохранить внесенные изменения?';ru='Сохранить внесенные изменения?';en='Save your changes?'");
...
Процедура ВидЦенСебестоимостиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ka='Обновить цены в таблице товаров?';ru='Обновить цены в таблице товаров?';en='Refresh prices of goods in the table?'"), РежимДиалогаВопрос.ДаНетОтмена,, КодВозвратаДиалога.Да);
	КонецЕсли; 
...
Процедура ВводНачальныхОстатковИмуществоВнеоборотныеАктивыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИменаРеквизитовАвтозаполнения = "СтруктурнаяЕдиница, НаправлениеДеятельности, СчетЗатрат, СчетЗатратРегл";
	...
	Иначе
		...
		ДанныеДляЗаполнения.СчетЗатратРегл = ПолучитьСчетАмортизацииНаСервере(); //+++
	КонецЕсли; 
	...
	ОпределитьТекущийЭлементТаблицы(Стр, "ВводНачальныхОстатковИмуществоВнеоборотныеАктивы", "СтруктурнаяЕдиница, СчетЗатрат, СчетЗатратРегл", "ОбъемПродукцииРаботДляВычисленияАмортизации");
	ПодключитьОбработчикОжидания("Подключаемый_НачатьИзменениеТаблицы", 0.1, Истина);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьСчетАмортизацииНаСервере()

	Возврат ПланыСчетов.Хозрасчетный.НайтиПоКоду("7470");

КонецФункции // ПолучитьСчетАмортизацииНаСервере()
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ПоказатьВводЗначения(Оповещение, ТекШтрихкод, НСтр("ka='შეიყვანეთ შტრიხკოდი';ru='Введите штрихкод';en='enter the barcode'"));
...
Процедура ЗагрузитьВводыОстатков()
	...
	Пока ВыборкаРезультатЗапроса.Следующий() Цикл
		...
		Элементы.ДатаОстатков.ПредупреждениеПриРедактировании = НСтр("ka='Дата именится также для всех остатков, введенных ранее';ru='Дата именится также для всех остатков, введенных ранее';en='Date also called for all residues introduced earlier'");
...
Процедура УстановитьТекстПояснения()
	...
	Если ТекущийНомерСтраницы = 0 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок =НСтр("ka='დააჭირეთ ღილაკს ""შემდეგ"" თანხობრივი ნაშთების შესავსებად';ru='Нажмите кнопку ""Далее"" для заполнения остатков денежных средств';en='Click ""Next"" button to complete your cash balances'"); 
	ИначеЕсли ТекущийНомерСтраницы = 1 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ka='დააჭირეთ ღილაკს ""შემდეგ"" სასაქონლო ნაშთების შესავსებად';ru='Нажмите кнопку ""Далее"" для заполнения остатков товаров';en='Click ""Next"" button to complete the remnants of the goods'");
	ИначеЕсли ТекущийНомерСтраницы = 2 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ka='დააჭირეთ ღილაკს ""შემდეგ"" გაანგარიშების ბალანსის შესავსებად';ru='Нажмите кнопку ""Далее"" для заполнения остатков по расчетам';en='Click ""Next"" to fill balances'");
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ka='დააჭირეთ ღილაკს ""შემდეგ"" სხვა ნაშთების შესავსებად';ru='Нажмите кнопку ""Далее"" для заполнения прочих остатков';en='Click ""Next"" button to fill other residues'");
	ИначеЕсли ТекущийНомерСтраницы = 4 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ru='Для завершения необходимо нажать кнопку ""Закончить""';ka='ოპერაციის დასრულებისთვის დააჭირეთ ღილაკს ""დასრულება""");
	КонецЕсли;
...
Процедура УстановитьАктивнуюСтраницу()
	...
	ЭтаФорма.Заголовок = "საწყისი ნაშთების შეყვანა (ნაბიჯი " + Строка(ТекущийНомерСтраницы)+ "/" + Строка(ПоследняяСтраница) + ")";
...
Процедура УстановитьДоступностьКнопок()
	...
	Если ТекущийНомерСтраницы >= ПоследняяСтраница - 1 Тогда
		Элементы.Далее.Заголовок = НСтр("ka='დასრულება';ru='Закончить';en='complete'");
		Элементы.Далее.Картинка = БиблиотекаКартинок.Пометка;
	Иначе
		Элементы.Далее.Заголовок = НСтр("ka='შემდეგი';ru='Далее';en='Further'");
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	Если ТекущийНомерСтраницы = 1 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВалютаУчета) Тогда
			ТекстСообщения = НСтр("ka='Укажите валюту учета.';ru='Укажите валюту учета.';en='Specify the accounting currency.'");
		...
		Если НЕ ЗначениеЗаполнено(НациональнаяВалюта) Тогда
			ТекстСообщения = НСтр("ka='Укажите национальную валюту.';ru='Укажите национальную валюту.';en='Enter the national currency.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстСообщения, , , ,
				Отказ
			);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковБанкИКасса.ДенежныеСредства Цикл
			...
			ТекСтрока,
			"БанковскийСчетКасса",
			НСтр("ka='Укажите банковский счет или кассу в строке %1.';ru='Укажите банковский счет или кассу в строке %1.';en='Specify a bank account or cash on line% 1.'"),
			Отказ);
			...
			НСтр("ka='Укажите валюту в строке %1.';ru='Укажите валюту в строке %1.';en='Specify the currency in line% 1.'"),
			Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 2 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.Запасы Цикл
			...
			ТекСтрока,
			"СтруктурнаяЕдиница",
			НСтр("ka='Укажите структурную единицу в строке %1.';ru='Укажите структурную единицу в строке %1.';en='Specify structural unit in row 1%.'"),
			Отказ);
			...
			ТекСтрока,
			"Номенклатура",
			НСтр("ka='Укажите номенклатуру в строке %1.';ru='Укажите номенклатуру в строке %1.';en='Specify the range in line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"ЕдиницаИзмерения",
			НСтр("ka='Укажите единицу измерения в строке %1.';ru='Укажите единицу измерения в строке %1.';en='Specify unit in row 1%.'"),
			Отказ);
			...
			ТекСтрока,
			"Количество",
			НСтр("ka='Укажите количество в строке %1.';ru='Укажите количество в строке %1.';en='Specify number in row 1%.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.ЗапасыПринятые Цикл
			...
			ТекСтрока,
			"ВидОперации",
			НСтр("ka='Укажите вид операции в строке %1.';ru='Укажите вид операции в строке %1.';en='Indicate the type of operations in row 1%.'"),
			Отказ);
			...
			ТекСтрока,
			"Контрагент",
			НСтр("ka='Укажите поставщика в строке %1.';ru='Укажите поставщика в строке %1.';en='Specify the vendor on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"Договор",
			НСтр("ka='Укажите договор в строке %1.';ru='Укажите договор в строке %1.';en='Enter the contract on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"Партия",
			НСтр("ka='Укажите партию в строке %1.';ru='Укажите партию в строке %1.';en='Enter the game on line% 1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.ЗапасыПереданные Цикл
			...
			ТекСтрока,
			"Контрагент",
			НСтр("ka='Укажите покупателя в строке %1.';ru='Укажите покупателя в строке %1.';en='Specify the buyer on line% 1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.ЗапасыВРазрезеГТД Цикл
			...
			ТекСтрока,
			"СтранаПроисхождения",
			НСтр("ka='Укажите страну происхождения в строке %1.';ru='Укажите страну происхождения в строке %1.';en='Specify the country of origin in line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"НомерГТД",
			НСтр("ka='Укажите номер ГТД в строке %1.';ru='Укажите номер ГТД в строке %1.';en='Specify the number of GTE on line% 1.'"),
			Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковРасчетыСКонтрагентами.РасчетыСПоставщиками Цикл
			...
			ТекСтрока,
			"СуммаВал",
			НСтр("ka='Укажите сумму в строке %1.';ru='Укажите сумму в строке %1.';en='Enter the amount on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"Сумма",
			НСтр("ka='Укажите сумму в валюте учета в строке %1.';ru='Укажите сумму в валюте учета в строке %1.';en='Specify the amount in the account currency on line% 1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковЗарплата.РасчетыСПерсоналом Цикл
			...
			ТекСтрока,
			"СтруктурнаяЕдиница",
			НСтр("ka='Укажите подразделение в строке %1.';ru='Укажите подразделение в строке %1.';en='Specify the unit on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"Сотрудник",
			НСтр("ka='Укажите сотрудника в строке %1.';ru='Укажите сотрудника в строке %1.';en='Specify the employee on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"ПериодРегистрации",
			НСтр("ka='Укажите период в строке %1.';ru='Укажите период в строке %1.';en='Specify the period in line% 1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковНалоги.РасчетыПоНалогам Цикл
			...
			ТекСтрока,
			"ВидНалога",
			НСтр("ka='Укажите вид налога в строке %1.';ru='Укажите вид налога в строке %1.';en='Specify the type of tax on line% 1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковЭквайринг.РасчетыПоЭквайрингу Цикл
			...
			НСтр("ka='Укажите экваринговый терминал в строке %1.';ru='Укажите экваринговый терминал в строке %1.';en='Specify ekvaringovy terminal in row 1%.'"),
			Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 4 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковИмущество.ВнеоборотныеАктивы Цикл
			...
			ТекСтрока,
			"ВнеоборотныйАктив",
			НСтр("ka='Укажите карточку имущества в строке %1.';ru='Укажите карточку имущества в строке %1.';en='Specify the property booked on line% 1.'"),
			Отказ);
			...
			НСтр("ka='Укажите текущее состояние в строке %1.';ru='Укажите текущее состояние в строке %1.';en='Specify the current status on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"СчетЗатрат",
			НСтр("ka='Укажите счет затрат в строке %1.';ru='Укажите счет затрат в строке %1.';en='Enter account cost in row 1%.'"),
			Отказ);
			//+++
			ПроверитьКолонкуТаблицы(
			"ВводНачальныхОстатковИмущество.ВнеоборотныеАктивы",
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковПрочее.ПрочиеРазделы Цикл
			...
			ТекСтрока,
			"ВидДвижения",
			НСтр("ka='Укажите вид движения в строке %1.';ru='Укажите вид движения в строке %1.';en='Specify the type of traffic on line% 1.'"),
			Отказ);
			...
			ТекСтрока,
			"Счет",
			НСтр("ka='Укажите счет в строке %1.';ru='Укажите счет в строке %1.';en='Specify the account in the line% 1.'"),
			Отказ);
			...
			Если УчетВалютныхОпераций И ТекСтрока.Валютный Тогда
				...
				ТекСтрока,
				"СуммаВал",
				НСтр("ka='Укажите валютную сумму в строке %1.';ru='Укажите валютную сумму в строке %1.';en='Enter the currency amount on line% 1.'"),
				Отказ);
...
Функция СписокВыбораВалют(Валюты)
	...
	КонецЦикла; 
	Результат.Добавить(Неопределено, НСтр("ka='Еще валюты...';ru='Еще валюты...';en='Another currency ...'"));
	Возврат Результат;
	...
	Если ТипЗнч(РезультатЗагрузки) = Тип("Структура") Тогда
		...
		ИначеЕсли РезультатЗагрузки.ОписаниеДействия = "ОбработатьПодготовленныеДанные" Тогда
			...
			Если ПолноеИмяОбъектаЗаполнения = "Документ.ВводНачальныхОстатков.ТабличнаяЧасть.Запасы" Тогда
				...
				ПоказатьПредупреждение(,Нстр("ka='Загрузка данных завершена.';ru='Загрузка данных завершена.';en='Loading data is completed.'"));
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				КонецЕсли; 
				//+++
				Если НЕ ЗначениеЗаполнено(НоваяСтрока.СчетЗатратРегл) Тогда
					НоваяСтрока.СчетЗатрат = ПланыСчетов.Хозрасчетный.НайтиПоКоду("7470");
				КонецЕсли;
				//---
				Если НЕ ЗначениеЗаполнено(НоваяСтрока.СтруктурнаяЕдиница) Тогда
					НоваяСтрока.СтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ОсновноеПодразделение;
