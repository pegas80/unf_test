...
Функция одТипЗначенияСтрокой(ЗначениеИлиТип) Экспорт
	...
	Иначе
		...
		Если Менеджер = Неопределено Тогда
			...
			Текст= НСтр("ka='Неизвестный тип:';ru='Неизвестный тип:';en='Unknown type:'") + Строка(ТипЗнч(ТипЗначения));
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаПользователю = ?(ТолькоПолучитьУзелСсылки, НСтр("ka='Конвертация ссылки на объект: %1';ru='Конвертация ссылки на объект: %1';en='Converting an object reference: 1%'"), НСтр("ka='Конвертация объекта: %1';ru='Конвертация объекта: %1';en='Converting object:% 1'"));
...
Процедура ПередатьИнформациюОЗаписиВПриемник(ИнформацияДляЗаписиВФайл, СтрокаОшибкиВБазеПриемнике = "") Экспорт
	...
	Если Не ПустаяСтрока(СтрокаОшибкиВБазеПриемнике) Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка в приемнике: %1';ru='Загрузка в приемнике: %1';en='Loading receiver:% 1'"), СтрокаОшибкиВБазеПриемнике);
...
Процедура УдалитьВременныеФайлы(ИмяВременногоФайла) Экспорт
	...
	Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ka='Универсальный обмен данными в формате XML';ru='Универсальный обмен данными в формате XML';en='Universal data exchange in XML format'", КодОсновногоЯзыка()),
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		...
		Если ФлагОшибки Тогда
			...
			СообщитьПользователю(НСтр("ka='При выгрузке обработчиков были обнаружены ошибки.';ru='При выгрузке обработчиков были обнаружены ошибки.';en='When unloading handlers errors were found.'"));
			
		Иначе
			
			СообщитьПользователю(НСтр("ka='Обработчики успешно выгружены.';ru='Обработчики успешно выгружены.';en='Handlers successfully unloaded.'"));
...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец, МассивЗапрошенныхЭлементов, Знач СтрокаПрефикса = "")
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ka='{НАЧАЛО АЛГОРИТМА}';ru='{НАЧАЛО АЛГОРИТМА}';en='START} {ALGORITHM'"));
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода, НСтр("ka='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';ru='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';en='{} Recursive call ALGORITHM'"));
					
					СтрокаОператора = НСтр("ka='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';ru='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';en='VyzvatIsklyuchenie ""recursive calls Algorithm:% 1"";'");
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ka='{КОНЕЦ АЛГОРИТМА}';ru='{КОНЕЦ АЛГОРИТМА}';en='{} END ALGORITHM'"));
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если НПП <> 0 Тогда
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка объекта № %1';ru='Загрузка объекта № %1';en='Download object number% 1'"), НПП);
		Иначе
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка объекта № %1';ru='Загрузка объекта № %1';en='Download object number% 1'"), ГНПП);
		КонецЕсли;
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000, ОписаниеОшибки(), НСтр("ka='Ошибка при создании временного файла для выгрузки данных';ru='Ошибка при создании временного файла для выгрузки данных';en='Error creating temporary file for downloading data'"));
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузка объекта: %1';ru='Выгрузка объекта: %1';en='Unloading object:% 1'"), ПрОбъекта);
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Правило выгрузки данных: %1 (%2)';ru='Правило выгрузки данных: %1 (%2)';en='unloading data Rule: 1% (2%)'"), СокрЛП(Правило.Имя), СокрЛП(Правило.Наименование));
			...
			Если ПоследнийОбъектМетаданных <> ТекущийОбъектМетаданных Тогда
				...
				Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если ФлагКомментироватьОбработкуОбъектов Тогда
						...
						СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Правило выгрузки данных: %1 (%2)';ru='Правило выгрузки данных: %1 (%2)';en='unloading data Rule: 1% (2%)'"),
...
Процедура ЗагрузитьЗначенияПараметровОбменаДанными()
	...
	Если СобытияПослеЗагрузкиПараметров.Свойство(Имя, АлгоритмПослеЗагрузкиПараметра)
		...
		Если ФлагРежимОтладкиОбработчиков Тогда
			...
			ВызватьИсключение НСтр("ka='Отладка обработчика ""После загрузки параметра"" не поддерживается.';ru='Отладка обработчика ""После загрузки параметра"" не поддерживается.';en='Processor Debug ""After boot option"" is not supported.'");
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				ВызватьИсключение НСтр("ka='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';en='Processor Debug ""After downloading the rules of exchange"" is not supported.'");
		...
		Исключение
			...
			Текст = НСтр("ka='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %1';ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %1';en='Processor: ""PosleZagruzkiPravilObmena"": 1%'");
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ka='Отладка обработчика ""После загрузки параметров"" не поддерживается.';ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.';en='Processor Debug ""After downloading the parameters"" is not supported.'");
						...
						Если Отказ = Истина Тогда
							...
							Если Не ПустаяСтрока(ПричинаОтказа) Тогда
								СтрокаИсключения = ПодставитьПараметрыВСтроку(НСтр("ka='Загрузка данных отменена по причине: %1';ru='Загрузка данных отменена по причине: %1';en='Loading data is canceled due to:% 1'"), ПричинаОтказа);
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ka='Отладка глобального алгоритма не поддерживается.';ru='Отладка глобального алгоритма не поддерживается.';en='Debugging a global algorithm is not supported.'");
	...
	Исключение
		...
		СтрокаОшибки = ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომა მონაცემების ჩატვირთვისას: %1';ru='Ошибка при загрузке данных: %1';en='Error loading% 1'"), ОписаниеОшибки());
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ka='Начало загрузки: %1';ru='Начало загрузки: %1';en='Start downloading:% 1'"), ТекущаяДатаСеанса());
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ka='ПередЗагрузкойДанных (конвертация)';ru='ПередЗагрузкойДанных (конвертация)';en='PeredZagruzkoyDannyh (conversion)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ka='ПослеЗагрузкиДанных (конвертация)';ru='ПослеЗагрузкиДанных (конвертация)';en='PosleZagruzkiDannyh (conversion)'"));
		КонецПопытки;
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ka='Окончание загрузки: %1';ru='Окончание загрузки: %1';en='End load: 1%'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ka='Загружено объектов: %1';ru='Загружено объектов: %1';en='Uploaded objects: 1%'"), мСчетчикЗагруженныхОбъектов), , Ложь, , , Истина);
	
	ЗавершитьВедениеПротоколаОбмена();
	
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ka='Загрузка данных завершена.';ru='Загрузка данных завершена.';en='Loading data is completed.'"));
	КонецЕсли;
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьZipФайла(ИмяФайлаОбмена, ПарольДляСжатияФайлаОбмена, НСтр("ka='Файл обмена данными';ru='Файл обмена данными';en='data exchange file'"));
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ka='Начало выгрузки: %1';ru='Начало выгрузки: %1';en='Start upload% 1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
			НСтр("ka='Ошибка при выгрузке данных: %1';ru='Ошибка при выгрузке данных: %1';en='Error when uploading data:% 1'"), СтрокаОшибки), Неопределено, Истина, , , Истина);
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ka='Окончание выгрузки: %1';ru='Окончание выгрузки: %1';en='Ending of discharge: 1%'"), ТекущаяДатаСеанса()), , Ложь, , ,Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ka='Выгружено объектов: %1';ru='Выгружено объектов: %1';en='Unloaded objects: 1%'"), мСчетчикВыгруженныхОбъектов), , Ложь, , , Истина);
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ka='Выгрузка данных завершена.';ru='Выгрузка данных завершена.';en='Upload data is completed.'"));
	КонецЕсли;
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(2,  НСтр("ka='Ошибка распаковки файла обмена. Файл заблокирован';ru='Ошибка распаковки файла обмена. Файл заблокирован';en='Error unpacking file sharing. The file is locked'"));
	одСообщения.Вставить(3,  НСтр("ka='Указанный файл правил обмена не существует';ru='Указанный файл правил обмена не существует';en='Said exchange rules file does not exist'"));
	одСообщения.Вставить(4,  НСтр("ka='შეცდომა მოხდა COM-ობიექტის შექმნისას Msxml2.DOMDocument';ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';en='Error creating COM-object Msxml2.DOMDocument'"));
	одСообщения.Вставить(5,  НСтр("ka='გაცვლის ფაილის გახსნის შეცდომა';ru='Ошибка открытия файла обмена';en='Error opening file sharing'"));
	одСообщения.Вставить(6,  НСтр("ka='შეცდომა გაცვლის წესების ჩატვირთვისას';ru='Ошибка при загрузке правил обмена';en='Error loading sharing rules'"));
	одСообщения.Вставить(7,  НСтр("ka='გაცვლის წესების ფორმატის შეცდომა';ru='Ошибка формата правил обмена';en='Format error exchange rules'"));
	одСообщения.Вставить(8,  НСтр("ka='არაკორექტულად არის მითითებული ფაილის სახელი მონაცემთა გადმოსატვირთად';ru='Не корректно указано имя файла для выгрузки данных';en='Incorrectly stated the name of the file to upload data'"));
	одСообщения.Вставить(9,  НСтр("ka='გაცვლის ფაილის ფორმატის შეცდომა';ru='Ошибка формата файла обмена';en='Error exchange file format'"));
	одСообщения.Вставить(10, НСтр("ka='არ არის მითიტებული ფაილის სახელი მონაცემთა გადმოსატვირთად (მონაცემთა ფაილის სახელი)';ru='Не указано имя файла для выгрузки данных (Имя файла данных)';en='Unknown file name to upload data (file name)'"));
	одСообщения.Вставить(11, НСтр("ka='მინიშნება გაცვლის წესების მეტამონაცემთა არარსებულ ობიექტზე';ru='Ссылка на несуществующий объект метаданных в правилах обмена';en='A link to a non-existent metadata object exchange rules'"));
	одСообщения.Вставить(12, НСтр("ka='არ არის მითიტებული ფაილის სახელი გაცვლის წესებით (წესების ფაილის სახელი)';ru='Не указано имя файла с правилами обмена (Имя файла правил)';en='Not specified file name with the exchange rules (rules file name)'"));
	
	одСообщения.Вставить(13, НСтр("ka='ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (წყაროს თვისების სახელით)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)';en='Error getting a property value of the object (called the source properties)'"));
	одСообщения.Вставить(14, НСтр("ka='ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (მიმღების თვისების სახელით)';ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';en='Failed to get the property value of the object (after the receiver properties)'"));
	
	одСообщения.Вставить(15, НСтр("ka='არ არის მითიტებული ფაილის სახელი მონაცემთა ჩასატვირთად (ჩასატვირთი ფაილის სახელი)';ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';en='Unknown file name to download the data (name of file to download)'"));
	
	одСообщения.Вставить(16, НСтр("ka='დაქვემდებარებული ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (წყაროს თვისების სახელით)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';en='Error getting property values ​​slave object (called the source properties)'"));
	одСообщения.Вставить(17, НСтр("ka='დაქვემდებარებული ობიექტის თვისების მნიშვნელობის მიღების შეცდომა (მიმღების თვისების სახელით)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';en='Error retrieving property values ​​subordinate object (called the receiver properties)'"));
	
	одСообщения.Вставить(19, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისჩატვირთვისწინ';ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';en='Error event handler PeredZagruzkoyObekta'"));
	одСообщения.Вставить(20, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისჩატვირთვისას';ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';en='Error event handler PriZagruzkeObekta'"));
	одСообщения.Вставить(21, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისჩატვირთვისშემდეგ';ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';en='Error event handler PosleZagruzkiObekta'"));
	одСообщения.Вставить(22, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაჩატვირთვისწინ (კონვერტაცია)';ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';en='Error event handler PeredZagruzkoyDannyh (conversion)'"));
	одСообщения.Вставить(23, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაჩატვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';en='Error event handler PosleZagruzkiDannyh (conversion)'"));
	одСообщения.Вставить(24, НСтр("ka='შეცდომა მოხდა ობიექტის წაშლისას';ru='Ошибка при удалении объекта';en='Error deleting object'"));
	одСообщения.Вставить(25, НСтр("ka='შეცდომა დოკუმენტის ჩაწერისას';ru='Ошибка при записи документа';en='Error writing instrument'"));
	одСообщения.Вставить(26, НСтр("ka='ობიექტის ჩაწერის შეცდომა';ru='Ошибка записи объекта';en='Object Error writing'"));
	одСообщения.Вставить(27, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გასუფთავებისწესისდამუშავებისწინ';ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';en='Error event handler PeredObrabotkoyPravilaOchistki'"));
	одСообщения.Вставить(28, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გასუფთავებისწესისდამუშავებისშემდეგ';ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';en='Error event handler PosleObrabotkiPravilaOchistki'"));
	одСообщения.Вставить(29, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისწაშლამდე';ru='Ошибка в обработчике события ПередУдалениемОбъекта';en='Error event handler PeredUdaleniemObekta'"));
	
	одСообщения.Вставить(31, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწესისდამუშავებისწინ';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';en='Error event handler PeredObrabotkoyPravilaVygruzki'"));
	одСообщения.Вставить(32, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწესებისდამუშავებისშემდეგ';ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';en='Error event handler PosleObrabotkiPravilaVygruzki'"));
	одСообщения.Вставить(33, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისწინ';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error event handler PeredVygruzkoyObekta'"));
	одСообщения.Вставить(34, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისშემდეგ';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';en='Error event handler PosleVygruzkiObekta'"));
	
	одСообщения.Вставить(39, НСтр("ka='Ошибка при выполнении алгоритма, содержащегося в файле обмена';ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';en='Error during execution of the algorithm contained in the file sharing'"));
	
	одСообщения.Вставить(41, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისწინ';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error event handler PeredVygruzkoyObekta'"));
	одСообщения.Вставить(42, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისას';ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';en='Error event handler PriVygruzkeObekta'"));
	одСообщения.Вставить(43, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ობიექტისგადმოტვირთვისშემდეგ';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';en='Error event handler PosleVygruzkiObekta'"));
	
	одСообщения.Вставить(45, НСтр("ka='ობიექტების კონვერტაციის წესი ნაპოვნი არ არის';ru='Не найдено правило конвертации объектов';en='Typically the conversion objects found'"));
	
	одСообщения.Вставить(48, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისდამუშავებისწინ თვისებათა ჯგუფები';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';en='Error event handler PeredObrabotkoyVygruzki property group'"));
	одСообщения.Вставить(49, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისდამუშავებისშემდეგ თვისებათა ჯგუფები';ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';en='Error event handler PosleObrabotkiVygruzki property group'"));
	одСообщения.Вставить(50, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწინ (კოლექციის ობიექტი)';ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';en='Error event handler PeredVygruzkoy (object collection)'"));
	одСообщения.Вставить(51, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში  გადმოტვირთვისას (კოლექციის ობიექტის)';ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';en='Error event handler PriVygruzke (object collection)'"));
	одСообщения.Вставить(52, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისშემდეგ (კოლექციის ობიექტის)';ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';en='Error event handler PosleVygruzki (object collection)'"));
	одСообщения.Вставить(53, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისჩატვირთვისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';en='Error in the global event handler PeredZagruzkoyObekta (conversion)'"));
	одСообщения.Вставить(54, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისჩატვირთვისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';en='Error in the global event handler PosleZagruzkiObekta (conversion)'"));
	одСообщения.Вставить(55, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისწინ (თვისებები)';ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';en='Error event handler PeredVygruzkoy (properties)'"));
	одСообщения.Вставить(56, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისას (თვიდებები)';ru='Ошибка в обработчике события ПриВыгрузке (свойства)';en='Error event handler PriVygruzke (properties)'"));
	одСообщения.Вставить(57, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში გადმოტვირთვისშემდეგ (თვისებები)';ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';en='Error event handler PosleVygruzki (properties)'"));
	
	одСообщения.Вставить(62, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაგადმოტვირთვისწინ (კონვერტაცია)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';en='Error event handler PeredVygruzkoyDannyh (conversion)'"));
	одСообщения.Вставить(63, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში მონაცემთაგადმოტვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';en='Error event handler PosleVygruzkiDannyh (conversion)'"));
	одСообщения.Вставить(64, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისკონვერტაციისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';en='Error in the global event handler PeredKonvertatsieyObekta (conversion)'"));
	одСообщения.Вставить(65, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისგადმოტვირთვისწინ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';en='Error in the global event handler PeredVygruzkoyObekta (conversion)'"));
	одСообщения.Вставить(66, НСтр("ka='შემომავალი მონაცემებიდან დაქვემდებარებული ობიექტების კოლექციის მიღების შეცდომა';ru='Ошибка получения коллекции подчиненных объектов из входящих данных';en='Error retrieving objects from the collection of the incoming data'"));
	одСообщения.Вставить(67, НСтр("ka='შემომავალი მონაცემებიდან დაქვემდებარებული ობიექტის თვისების მიღების შეცდომა';ru='Ошибка получения свойства подчиненного объекта из входящих данных';en='Failed to get properties of the subordinate object from the incoming data'"));
	одСообщения.Вставить(68, НСтр("ka='შემომავალი მონაცემებიდან ობიექტის თვისების მიღების შეცდომა';ru='Ошибка получения свойства объекта из входящих данных';en='Failed to get object properties of the incoming data'"));
	
	одСообщения.Вставить(69, НСтр("ka='შეცდომა მოვლენის გლობალურ დამამუშავებელში  ობიექტისგადმოტვირთვისშემდეგ (კონვერტაცია)';ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';en='Error in the global event handler PosleVygruzkiObekta (conversion)'"));
	
	одСообщения.Вставить(71, НСтр("ka='მნიშვნელობა წყაროსთვის არ არის ნაპოვნი შესაბამისობა';ru='Не найдено соответствие для значения Источника';en='Not found a match for the source values'"));
	
	одСообщения.Вставить(72, НСтр("ka='შეცდომა მონაცემთა გადმოტვირთვისას გაცვლის გეგმის კვანძისთვის';ru='Ошибка при выгрузке данных для узла плана обмена';en='Error when uploading data to the node sharing plan'"));
	
	одСообщения.Вставить(73, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ძიებისველებისთანმიმდევრულობა';ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';en='Error event handler PosledovatelnostPoleyPoiska'"));
	
	одСообщения.Вставить(74, НСтр("ka='Необходимо перезагрузить правила обмена для выгрузки данных';ru='Необходимо перезагрузить правила обмена для выгрузки данных';en='You must restart the exchange rules for data extraction'"));
	
	одСообщения.Вставить(75, НСтр("ka='შეცდომა ალგორითმის შესრულებისას პარამეტრების მნიშვნელობების ჩატვირთვის შემდეგ';ru='Ошибка при выполнении алгоритма после загрузки значений параметров';en='Error during execution of the algorithm after downloading the parameter values'"));
	
	одСообщения.Вставить(76, НСтр("ka='შეცდომა მოვლენის დამამუშავებელში ფაილშიობიექტისგადმოტვირვისშემდეგ';ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';en='Error event handler PosleVygruzkiObektaVFayl'"));
	
	одСообщения.Вставить(77, НСтр("ka='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';en='Unknown external processing file with event handlers hook procedure'"));
	
	одСообщения.Вставить(78, НСтр("ka='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';en='Error creating external processing of the file with the procedures of the event handlers'"));
	
	одСообщения.Вставить(79, НСтр("ka='Код алгоритмов не может быть интегрирован в обработчик из-за обнаруженного рекурсивного вызова алгоритмов. "
...
"и повторите выгрузку.';ru='Код алгоритмов не может быть интегрирован в обработчик из-за обнаруженного рекурсивного вызова алгоритмов. "
"Если в процессе отладки нет необходимости отлаживать код алгоритмов, то укажите режим ""не отлаживать алгоритмы"""
"Если необходимо выполнять отладку алгоритмов с рекурсивным вызовом, то укажите режим  ""алгоритмы отлаживать как процедуры"" "
"и повторите выгрузку.'"));
	
	одСообщения.Вставить(80, НСтр("ka='Обмен данными можно проводить только под полными правами';ru='Обмен данными можно проводить только под полными правами';en='possible to exchange data under the full rights only'"));
	
	одСообщения.Вставить(1000, НСтр("ka='შეცდომა მონაცემთა გადმოტვირთვის დროებითი ფაილის შექმნისას';ru='Ошибка при создании временного файла выгрузки данных';en='Error creating temporary data upload file'"));
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "")
	...
	Если СтруктураПодключения.ФайловыйРежим  Тогда
		...
		Если ПустаяСтрока(СтруктураПодключения.КаталогИБ) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='Не задан каталог информационной базы-приемника';ru='Не задан каталог информационной базы-приемника';en='It does not set the directory information base receiver'");
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='Не задано имя сервера 1С:Предприятия информационной базы-приемника';ru='Не задано имя сервера 1С:Предприятия информационной базы-приемника';en='Do not set the server name of 1C: Enterprise information base receiver'");
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ka='Не задано имя информационной базы-приемника на сервере 1С:Предприятия';ru='Не задано имя информационной базы-приемника на сервере 1С:Предприятия';en='Not defined information base receiver name on the 1C: Enterprise server'");
...
"%1';ka='При попытке соединения с COM-сервером произошла следующая ошибка:"
"%1'");
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = НСтр("ka='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка: %1';ru='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка: %1';en='When you try to create UniversalnyyObmenDannymiXML processing error has occurred:% 1'");
		Текст = ПодставитьПараметрыВСтроку(Текст, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
	КонецПопытки;
...
//
// Пример:
//  ПодставитьПараметрыВСтроку(НСтр("ka='%1 წავიდა %2 -ში';ru='%1 пошел в %2';en='% 1 to% 2 I went'"), "Вася", "Зоопарк") = "Вася пошел в Зоопарк".
//
