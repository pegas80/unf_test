...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ
		...
		СообщениеЖурнала = НСтр("ka='Обновление программы невозможно. В подчиненном узле информационной базы обновление получается из главного узла.';ru='Обновление программы невозможно. В подчиненном узле информационной базы обновление получается из главного узла.';en='Updating the program impossible. The slave node information database update is obtained from the master node.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ka='<b>Обновление программы невозможно</b><br />В подчиненном узле информационной базы обновление получается из главного узла.';ru='<b>Обновление программы невозможно</b><br />В подчиненном узле информационной базы обновление получается из главного узла.';en='<B> Update program is not possible </ b> <br /> In the slave node information database update is obtained from the master node.'"),
			,
			СообщениеЖурнала);
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
		Возврат;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		СообщениеЖурнала = НСтр("ka='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.';ru='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.';en='Updating the program impossible. Software Update is not available when working in the Web client mode.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />"
"Обновление программы недоступно при работе в режиме веб-клиента.<br />"
"Откройте программу в тонком или толстом клиенте и повторите попытку.';ka='<b>Обновление программы невозможно</b><br />"
"Обновление программы недоступно при работе в режиме веб-клиента.<br />"
"Откройте программу в тонком или толстом клиенте и повторите попытку.'"),
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ka='Переход на новую версию платформы 1С:Предприятие недоступен в текущем режиме работы.';ru='Переход на новую версию платформы 1С:Предприятие недоступен в текущем режиме работы.';en='Transition to the new version of the platform 1C: Enterprise is not available in the current operation.'");
	...
	ИначеЕсли (ЭтоСценарийРабочегоОбновления(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ka='Получение обновлений программы недоступно в текущем режиме работы.';ru='Получение обновлений программы недоступно в текущем режиме работы.';en='Obtaining software updates is not available in the current operation.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Если Не ЭтоАдминистраторСистемы Тогда
			...
			ВызватьИсключение НСтр("ka='Недостаточно прав для перехода на новую версию платформы.';ru='Недостаточно прав для перехода на новую версию платформы.';en='Not have permission to go to the new version of the platform.'");
		КонецЕсли;
		
		ЭтотОбъект.АвтоЗаголовок = Ложь;
		ЭтотОбъект.Заголовок     = НСтр("ka='Переход на новую версию платформы 1С:Предприятие';ru='Переход на новую версию платформы 1С:Предприятие';en='Transition to the new version of the platform 1C: Enterprise'");
	...
	ИначеЕсли Не ЭтоСценарийРабочегоОбновления(ЭтотОбъект) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Неизвестный сценарий работы помощника обновления программы (""%1"").';ru='Неизвестный сценарий работы помощника обновления программы (""%1"").';en='Unknown assistant job script upgrade program ( ""%1"").'"),
			Сценарий);
	...
	Элементы.ДекорацияНаписатьВТехПоддержку.Заголовок =
		ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				НСтр("ka='<body>При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';ru='<body>При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';en='<Body> If you have problems write to <a href=""mailto:webits-info@1c.ru""> technical support </a>. </ Body>'")));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПолучениеИУстановка
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ka='Не завершена установка платформы 1С:Предприятие.';ru='Не завершена установка платформы 1С:Предприятие.';en='Not completed the installation of the platform 1C: Enterprise.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ЗавершениеРаботы Тогда
		Если ЗначениеЗаполнено(ИДФоновогоЗадания) Тогда
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ka='Получение и установка обновлений не завершены.';ru='Получение и установка обновлений не завершены.';en='Obtaining and installing the update to be completed.'");
		ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.ВыборРежимаОбновленияФайловыйРежим
			...
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ka='Не установлено обновление программы.';ru='Не установлено обновление программы.';en='Do not apply the update program.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДекорацияЗагрузитьДистрибутивКомпонентаПлатформыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(URLСтраницыПлатформы) Тогда
		ПоказатьПредупреждение(, НСтр("ka='Адрес страницы новой версии платформы не определен.';ru='Адрес страницы новой версии платформы не определен.';en='Address of the page of the new version of the platform is not defined.'"));
		Возврат;
	КонецЕсли;
...
Процедура КаталогСохраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбора.Заголовок = НСтр("ka='Каталог хранения дистрибутивов платформы 1С:Предприятие';ru='Каталог хранения дистрибутивов платформы 1С:Предприятие';en='Storage directory distribution platform 1C: Enterprise'");
...
Процедура ДекорацияЗарегистрироватьсяНаПортале1СИТСНажатие(Элемент)
	...
	ОткрытьВебСтраницу(
		ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыСервисаLogin(
			"/registration",
			НастройкиСоединения()),
		НСтр("ka='რეგისტრაცია';ru='Регистрация';en='check in'"));
...
Процедура КомандаДалее(Команда)
		...
		Если Элементы.ГруппаКаталогСохраненияПлатформы.Видимость
			...
			Если ПустаяСтрока(КаталогСохраненияПлатформы) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='Не выбран каталог хранения дистрибутивов платформы.';ru='Не выбран каталог хранения дистрибутивов платформы.';en='Not selected directory storage platform distributions.'"),
					,
	...
	ИначеЕсли ТекСтраница = Элементы.СтраницаПодключениеКПорталу Тогда
		...
		Если ПустаяСтрока(Логин) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнен логин.';ru='Не заполнен логин.';en='Is empty login.'"),
				,
				"Логин",
				,
				Отказ);
		...
		Если ПустаяСтрока(Пароль) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнен пароль.';ru='Не заполнен пароль.';en='Password is empty.'"),
				,
				"Пароль",
				,
				Отказ);
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	Если Ссылка = "open:RCInstruction" Тогда
		...
		ОткрытьВнутреннийФайлИнструкции(
			...
			НСтр("ka='Создание резервной копии.htm';ru='Создание резервной копии.htm';en='Backing kopii.htm'"));
		
	ИначеЕсли Ссылка = "open:V8Update" Тогда
		
		ОткрытьВебСтраницу(
			ИнформацияОДоступномОбновлении.Платформа.URLОсобенностиПерехода,
			НСтр("ka='Особенности перехода на новую версию платформы 1С:Предприятие';ru='Особенности перехода на новую версию платформы 1С:Предприятие';en='Features of the transition to the new version of the platform 1C: Enterprise'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(АдресПолучателя)
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		Или ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект) Тогда
		Тема = НСтр("ka='Интернет-поддержка. Переход на новую версию Платформы 1С:Предприятие';ru='Интернет-поддержка. Переход на новую версию Платформы 1С:Предприятие';en='Internet support. Transition to the new version of the platform 1C: Enterprise'");
	Иначе
		Тема = НСтр("ka='Интернет-поддержка. Обновление программы';ru='Интернет-поддержка. Обновление программы';en='Internet support. Program update'");
	КонецЕсли;
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПодключениеКПорталу Тогда
		...
		Сообщение = НСтр("ka='Не удалось подключиться к Порталу 1С:ИТС.';ru='Не удалось подключиться к Порталу 1С:ИТС.';en='Unable to connect to the portal 1C: ITS.'")
			+ ?(ПустаяСтрока(ОписаниеПричины),
				"",
				Символы.ПС + Символы.ПС + НСтр("ka='Описание:';ru='Описание:';en='Description:'") + Символы.ПС + ОписаниеПричины);
		
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНеУдалосьПодключитьсяКСервису Тогда
		
		Сообщение = НСтр("ka='Ошибка при подключении сервиса автоматического обновления программы.';ru='Ошибка при подключении сервиса автоматического обновления программы.';en='Error connecting service to automatically update the program.'")
			+ ?(ПустаяСтрока(ПодробноеОписаниеОшибки),
				"",
				Символы.ПС + Символы.ПС + НСтр("ka='Описание:';ru='Описание:';en='Description:'") + Символы.ПС + ПодробноеОписаниеОшибки);
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСообщениеСервиса Тогда
		...
		Если ПустаяСтрока(ПодробноеОписаниеОшибки) Тогда
			Сообщение = НСтр("ka='Сообщение при подключении сервиса автоматического обновления программы:';ru='Сообщение при подключении сервиса автоматического обновления программы:';en='Message when you connect the service to automatically update the program:'")
				+ Символы.ПС
		...
		Иначе
				...
				Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(КонтекстОбновления.ПутьФайлаПротокола) Тогда
					Вложения.Добавить(
						Новый Структура("Представление, ИмяФайла",
							НСтр("ka='Протокол установки платформы 1С_Предприятие.txt';ru='Протокол установки платформы 1С_Предприятие.txt';en='Protocol 1S_Predpriyatie.txt platform installation'"),
			...
			Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
				...
				Сообщение = НСтр("ka='Ошибка при получении и установке платформы 1С:Предприятие:';ru='Ошибка при получении и установке платформы 1С:Предприятие:';en='Error retrieving and installing the platform 1C: Enterprise:'")
					+ Символы.ПС
					+ ПодробноеОписаниеОшибки;
				
			Иначе
				
				Сообщение = НСтр("ka='Не удалось обновить программу.';ru='Не удалось обновить программу.';en='Failed to update the program.'")
					+ Символы.ПС
					+ НСтр("ka='Причина:';ru='Причина:';en='Cause:'")
					+ Символы.ПС
					+ ПодробноеОписаниеОшибки;
...
Процедура ОткрытьСозданиеРезервнойКопииИБ()
	...
	Иначе
		ТекстСообщения = НСтр("ka='Не встроена подсистема ""Резервное копирование ИБ"".';ru='Не встроена подсистема ""Резервное копирование ИБ"".';en='Not integrated subsystem ""Backup IB"".'");
...
"%2';ka='Ошибка при открытии внутреннего файла инструкции (%1)."
"%2'"),
				ИмяФайла,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		ПоказатьПредупреждение(, НСтр("ka='Ошибка при открытии файла.';ru='Ошибка при открытии файла.';en='Error opening file.'"));
		Возврат;
	КонецПопытки;
...
Функция ПодготовитьФайлМакетаИнструкции(Знач ИмяМакета, Знач ИмяФайла)
	...
	Если Не ПолучениеОбновленийПрограммы.СлужебнаяДоступноИспользованиеПолученияОбновленийПлатформы() Тогда
		ВызватьИсключение НСтр("ka='Использование обновления платформы 1С:Предприятие недоступно в текущем режиме работы.';ru='Использование обновления платформы 1С:Предприятие недоступно в текущем режиме работы.';en='Using the Platform Update 1C: Enterprise is not available in the current operation.'");
	КонецЕсли;
...
"%2';ka='Ошибка при создании каталога для работы с обновлениями платформы 1С:Предприятие (%1)."
"%2'"),
					КаталогДляРаботыСОбновлениями,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
			ВызватьИсключение НСтр("ka='Ошибка при создании временного каталога.';ru='Ошибка при создании временного каталога.';en='Error creating temporary directory.'");
		КонецПопытки;
	КонецЕсли;
...
"%2';ka='Ошибка при записи данных макета инструкции в файл на диске (%1)."
"%2'"),
				ИмяФайла,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		ВызватьИсключение НСтр("ka='Ошибка при создании временного файла.';ru='Ошибка при создании временного файла.';en='Error creating temporary file.'");
	КонецПопытки;
...
Функция УстановитьОбновлениеКонфигурации()
	...
	Для каждого ТекФайл Из ПрараметрыУстановки.ФайлыОбновления Цикл
		СтрОписаниеФайловОбновлений = СтрОписаниеФайловОбновлений + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='  Имя файла: %1, Выполнить обработчики обновления: %2;';ka='  Имя файла: %1, Выполнить обработчики обновления: %2;'"),
...
"Файлы обновления: %1"
"Платформа 1С:Предприятие: %2';ka='Вызов интерфейса установки обновления (СтандартныеПодсистемы.ОбновлениеКонфигурации)."
"Файлы обновления: %1"
...
Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновлении()
	...
	Состояние(, , НСтр("ka='Получение информации о доступном обновлении';ru='Получение информации о доступном обновлении';en='Getting information about the available update'"));
	...
	Если Не ПустаяСтрока(ИнформацияОДоступномОбновлении.ИмяОшибки) Тогда
		...
		Иначе
			ОтобразитьСообщениеСервиса(
				ИнформацияОДоступномОбновлении.Сообщение,
				НСтр("ka='Повторить попытку подключения >';ru='Повторить попытку подключения >';en='Retry connection>'"),
				ИнформацияОДоступномОбновлении.ИнформацияОбОшибке);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ОтобразитьИнформациюОДоступномОбновленииПлатформы(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(ОбновлениеПлатформы.Версия) Тогда
		...
		Элементы.ДекорацияНетВерсии.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Используемая сейчас версия: %1</body>';ru='<body>Используемая сейчас версия: %1</body>';en='<Body> currently used version is:% 1 </ body>'"),
		...
		Элементы.ДекорацияНетВерсии.Заголовок = ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='<body>Рекомендуемая версия платформы 1С:Предприятие: <b>%1</b>"
"<br />Используемая сейчас версия: %2</body>';ru='<body>Рекомендуемая версия платформы 1С:Предприятие: <b>%1</b>"
"<br />Используемая сейчас версия: %2</body>'"),
	...
	Если Не ЕстьОбновление Тогда
		...
		Элементы.КнопкаДалее.Заголовок    = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
		Элементы.КнопкаОтмена.Заголовок   = НСтр("ka='დახურვა';ru='Закрыть';en='close'");
		Возврат;
	...
	Иначе
		...
		Иначе
			...
			Элементы.НадписьВерсия.Заголовок = НСтр("ka='ვერსია';ru='Версия';en='Version'")
				+ " " + ОбновлениеПлатформы.Версия;
			...
			Иначе
				...
				Элементы.КнопкаДалее.Доступность  = Истина;
				Элементы.КнопкаДалее.Заголовок    = НСтр("ka='მზად არის';ru='Готово';en='Done'");
			...
			Элементы.КнопкаОтмена.Доступность = Истина;
			Элементы.ПродолжитьРаботуНаТекущейВерсии.Видимость = Ложь;
			Элементы.КнопкаОтмена.Заголовок   = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
...
Процедура ОтобразитьСтраницуНоваяВерсияПлатформыУжеУстановленаНаКомпьютере()
	...
	Элементы.НадписьВерсияУстановлено.Заголовок =
		НСтр("ka='ვერსია';ru='Версия';en='Version'") + " " + ОбновлениеПлатформы.Версия;
	...
	Если ИнформацияОДоступномОбновлении.Свойство("URLОсобенностиПерехода")
		...
		Элементы.НадписьУстановленоДополнительно.Заголовок = ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='<body>Версия платформы %1 уже установлена на компьютере."
"<br />Перед началом работы на новой версии платформы рекомендуется ознакомиться"
"<br />с <a href=""open:V8Update"">особенностями перехода</a> на эту версию платформы.</body>';ru='<body>Версия платформы %1 уже установлена на компьютере."
"<br />Перед началом работы на новой версии платформы рекомендуется ознакомиться"
"<br />с <a href=""open:V8Update"">особенностями перехода</a> на эту версию платформы.</body>'"),
	...
	Иначе
		...
		Элементы.НадписьУстановленоДополнительно.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Версия платформы %1 уже установлена на компьютере.</body>';ru='<body>Версия платформы %1 уже установлена на компьютере.</body>';en='<Body> Platform Version% 1 is already installed on the computer. </ Body>'"),
					ОбновлениеПлатформы.Версия));
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Если Не ОбновлениеКомКонф.ДоступноОбновление И Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Если ЭтоСценарийРабочегоОбновления(ЭтотОбъект) Или ПустаяСтрока(ЗаголовокНетОбновления) Тогда
			ТекстСообщения = ТекстСообщения
				+ НСтр("ka='Обновление не требуется. Установлена актуальная версия программы.';ru='Обновление не требуется. Установлена актуальная версия программы.';en='The update is not required. Actual installed version of the program.'");
		Иначе
	...
	Иначе
		...
		Если ПрямоеОбновление Тогда
			ТекстСообщения = ТекстСообщения + НСтр("ka='ხელმისაწვდომია პროგრამის განახლება.';ru='Доступно обновление программы.';en='Available update program.'");
		Иначе
			ТекстСообщения = ТекстСообщения
		...
		Если РазмерОбновления <> 0 Тогда
			ТекстСообщения = ТекстСообщения + ?(ПрямоеОбновление, " ", "<br />")
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='კონფიგურაციის ზომა: <b>%1</b>';ru='Размер дистрибутива: <b>%1</b>';en='size distribution: <b>% 1 </ b>'"),
		...
		Если ОбновитьКонфигурацию Тогда
			Если ЭтоАдминистраторСистемы И Не ПустаяСтрока(ОбновлениеКомКонф.URLПорядокОбновления) Тогда
				ТекстСообщения = ТекстСообщения
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='<br /><b>Внимание!</b> Для продолжения необходимо ознакомиться с информацией о <a href=""%1"">порядке обновления</a>.';ru='<br /><b>Внимание!</b> Для продолжения необходимо ознакомиться с информацией о <a href=""%1"">порядке обновления</a>.';en='<br /> <b> Warning! </ b> To continue, you must read the information on <a href=""%1""> update the </a>.'"),
		...
		Если Не ЭтоАдминистраторСистемы Тогда
			ТекстСообщения = ТекстСообщения
				+ НСтр("ka='<br />Для обновления программы обратитесь к администратору.';ru='<br />Для обновления программы обратитесь к администратору.';en='<br /> Contact your administrator to upgrade the program.'");
		КонецЕсли;
...
Процедура НастроитьКнопкиКоманднойПанелиДляКомпонентов()
	...
	ИначеЕсли ОбновитьКонфигурацию Тогда
		...
		Если ОбновитьПлатформу Тогда
			Элементы.КнопкаДалее.Доступность = Ложь;
			Элементы.КнопкаДалее.Заголовок   = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
			Элементы.КнопкаОтмена.Заголовок  = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
		Иначе
			Элементы.КнопкаДалее.Доступность = Истина;
			Элементы.КнопкаДалее.Заголовок   = НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'");
			Элементы.КнопкаОтмена.Заголовок  = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
...
Процедура ОтобразитьКомпонентКонфигурации()
	...
	Если ПустаяСтрока(ОбновлениеКомКонфигурации.Версия) Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеКонфигурации.Заголовок = НСтр("ka='კონფიგურაციის განახლება';ru='Обновление конфигурации';en='update configuration'");
...
"%2</body>';ka='<body>Обновление не требуется. Текущая версия конфигурации: <b>%1</b><br />"
"%2</body>'"),
	...
	Иначе
		...
		Элементы.ДекорацияЗаголовокОбновлениеКонфигурации.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Обновление конфигурации, <b>%1</b></body>';ru='<body>Обновление конфигурации, <b>%1</b></body>';en='<Body> Update configuration, <b>% 1 </ b> </ body>'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
...
"%4</body>';ka='<body><b>Версия %1</b>   <a href=""%2"">Новое в версии</a><br />"
"Текущая версия конфигурации: %3<br />"
"%4</body>'"),
...
"%3</body>';ka='<body><b>Версия %1</b><br />"
"Текущая версия конфигурации: %2<br />"
"%3</body>'"),
						ОбновлениеКомКонфигурации.Версия,
						ИнтернетПоддержкаПользователейКлиентСервер.ВерсияКонфигурации(),
...
Процедура ОтобразитьКомпонентПлатформы()
	...
	Если Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
			НСтр("ka='Обновление платформы 1С:Предприятие';ru='Обновление платформы 1С:Предприятие';en='Updating the 1C: Enterprise'");
		Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='<body>Обновление не требуется. Текущая версия платформы 1С:Предприятие: <b>%1</b></body>';ru='<body>Обновление не требуется. Текущая версия платформы 1С:Предприятие: <b>%1</b></body>';en='<Body> No update required. The current version of 1C: Enterprise: <b>% 1 </ b> </ body>'"),
					ПолучениеОбновленийПрограммыКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие()));
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='<body>Обновление платформы 1С:Предприятие, <b>%1</b></body>';ru='<body>Обновление платформы 1С:Предприятие, <b>%1</b></body>';en='<Body> Update platform 1C: Enterprise, <b>% 1 </ b> </ body>'"),
						ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
							ОбновлениеКомПл.РазмерОбновления)));
		Иначе
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
						НСтр("ka='<body>Обновление платформы 1С:Предприятие</body>';ru='<body>Обновление платформы 1С:Предприятие</body>';en='<Body> Update platform 1C: Enterprise </ body>'"));
		КонецЕсли;
...
"Текущая версия платформы 1С:Предприятие: %3</body>';ka='<body><b>Версия %1</b>   <a href=""%2"">Особенности перехода на новую версию платформы</a><br />"
		...
		Иначе
			...
			Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b><br />"
"Текущая версия платформы 1С:Предприятие: %2</body>';ka='<body><b>Версия %1</b><br />"
"Текущая версия платформы 1С:Предприятие: %2</body>'"),
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Обновление платформы является обязательным для работы с новой версией конфигурации %1.';ru='Обновление платформы является обязательным для работы с новой версией конфигурации %1.';en='Platform Update is required to work with the new configuration version% 1.'"),
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки = 1 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Новая версия %1 платформы 1С:Предприятие рекомендуется для работы с новой версией конфигурации %2.';ru='Новая версия %1 платформы 1С:Предприятие рекомендуется для работы с новой версией конфигурации %2.';en='The new version of% 1 platform 1C: Enterprise is recommended for use with the new version of the configuration% 2.'"),
							ОбновлениеКомПл.Версия,
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				Иначе
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ka='Обновление платформы не является обязательным для работы с новой версией конфигурации %1.';ru='Обновление платформы не является обязательным для работы с новой версией конфигурации %1.';en='platform update is not required for the new configuration version% 1.'"),
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				КонецЕсли;
			...
			ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки < 2 Тогда
				...
				Элементы.ДекорацияПояснениеПлатформа.Заголовок =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Рекомендуется установить новую версию платформы 1С:Предприятие.';ru='Рекомендуется установить новую версию платформы 1С:Предприятие.';en='It is recommended to install the new version of 1C: Enterprise.'"),
						ИнформацияОДоступномОбновлении.Конфигурация.Версия);
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ka='<body>Для работы новой версии конфигурации требуется платформа 1С:Предприятие не ниже версии <b>%1</b>.</body>';ru='<body>Для работы новой версии конфигурации требуется платформа 1С:Предприятие не ниже версии <b>%1</b>.</body>';en='<Body> For the new version of the configuration is required platform 1C: Enterprise version is not lower than <b>% 1 </ b> </ body>.'"),
...
Процедура НачатьПолучениеОбновлений()
	...
	Иначе
		...
		Состояние(, , НСтр("ka='Подключение к сервису обновлений';ru='Подключение к сервису обновлений';en='Connection to the service updates'"));
		...
		Иначе
			...
			Иначе
				...
				ОтобразитьСообщениеСервиса(
					ОписаниеФайловОбновлений.Сообщение,
					НСтр("ka='Повторить попытку >';ru='Повторить попытку >';en='Retry>'"),
					ОписаниеФайловОбновлений.ИнформацияОбОшибке);
...
Процедура СохранитьДанныеАутентификации(Знач ДанныеАутентификации)
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ka='ინტერნეტ-მხარდაჭერის ავთენტიფიკაციის მონაცემთა ჩასაწერად არასაკმარისი უფლებებია.';ru='Недостаточно прав для записи данных аутентификации Интернет-поддержки.';en='Insufficient permissions to write the authentication e-support data.'");
	КонецЕсли;
...
"Имя файла обновления (cfu): %7.';ka='Получение обновления конфигурации."
"URL: %1;"
"Размер: %2;"
"Формат файла обновления: %3;"
"Контрольная сумма файла обновления: %4;"
...
"Имя файла обновления (cfu): %7.'"),
		...
		Если Не ПолучениеОбновленийПрограммыКлиентСервер.ОбновлениеКонфигурацииПолучено(ТекОбновление, КонтекстОбновления) Тогда
			...
			КонтекстОбновления.ТекущееДействие =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Получение файла %1 из %2';ru='Получение файла %1 из %2';en='Preparation file% 1% 2'"),
					Строка(Итератор + 1),
		...
		Иначе
			...
			ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ka='Обновление конфигурации уже было получено ранее.';ru='Обновление конфигурации уже было получено ранее.';en='Update configuration has been received previously.'"));
...
Процедура ЗагрузитьОбновлениеКонфигурации()
	...
	Если ПолучениеНаСервере Тогда
		...
		ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Загрузка файла обновления конфигурации %1 (выполняется на сервере информационной базы).';en='To download the configuration update file% 1 (carried out in the information database server).'"),
	...
	Иначе
		...
		ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Загрузка файла обновления конфигурации %1 (выполняется на компьютере пользователя).';en=""Download the update file configuration% 1 (runs on the user's computer)."""),
				Обновление.ИмяПолученногоФайла));
...
Процедура ПриПолученииФайлаОбновленияКонфигурации(ПолученныеФайлы, РезультатПолучения) Экспорт
	...
	ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru='Файл обновления успешно загружен.';en='The update file is successfully loaded.'"));
	...
	СообщениеЖурнала =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при получении файла дистрибутива конфигурации (%1). %2';en='Error getting configuration file distribution (1%). 2%'"),
			Обновление.URLФайлаОбновления,
	...
	КонтекстОбновления.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка при получении файла дистрибутива конфигурации. %1';en='Error retrieving configuration file distribution. %one'"),
...
Процедура ИнициализироватьСтраницуПолучениеОбновления()
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მიმდინარეობს პლატფორმა 1С:საწარმოს დისტრიბუტივის ფაილის მიღება (%1).';ru='Выполняется получение файла дистрибутива платформы 1С:Предприятие (%1).';en='Retrieves the file distribution platform 1C: Enterprise (1%).'") + " "
					+ НСтр("ru='Получение файла может занять от нескольких минут до нескольких часов в зависимости от размера обновления,';en='file preparation can take anywhere from several minutes to several hours depending on the size of the update,'") + " "
					+ НСтр("ru='скорости подключения к Интернету и производительности компьютера.';en='Internet connection speed and PC performance.'"),
				РазмерОбновленияСтр(ЭтотОбъект));
	Иначе
		// Другие сценарии.
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется получение файлов обновления (%1). Получение файлов может занять от нескольких минут до';en='Retrieves an update file (% 1). Getting files may take a few minutes to'")
					+ " " + НСтр("ru='нескольких часов в зависимости от размера обновления,';en='a few hours depending on the size of the update,'")
					+ " " + НСтр("ru='скорости подключения к Интернету и производительности компьютера.';en='Internet connection speed and PC performance.'"),
				РазмерОбновленияСтр(ЭтотОбъект));
	КонецЕсли;
...
Процедура НачатьПолучениеОбновленийВФоновомРежиме()
	...
	Попытка
		Задание = ФоновыеЗадания.Выполнить(
			...
			,
			НСтр("ru='Получение и установка обновлений программы';en='Obtaining and installing software updates'"));
	Исключение
		
		СообщениеЖурналаРегистрации =
			НСтр("ru='Ошибка запуска задания.';en='Error starting the job.'") + Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурналаРегистрации);
		
		ОтобразитьВнутреннююОшибку(НСтр("ru='Ошибка запуска задания.';en='Error starting the job.'") + Символы.ПС
	...
	Если КонтекстОбновления = Неопределено Тогда
		Прогресс = 0;
		ТекущееДействие = НСтр("ka='Подготовка к получению обновления...';ru='Подготовка к получению обновления...';en='Preparing to Receive updates ...'");
	КонецЕсли;
...
Функция СостояниеЗадания(Знач ИДЗадания)
	...
	Исключение
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при обращении к заданию. %1';en='Error when accessing the job. %one'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
	...
	Если Задание = Неопределено Тогда
		...
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(НСтр("ka='დავალება ვერ მოიძებნა.';ru='Задание не найдено.';en='The job is not found.'"));
		
		Результат.КодСостояния = "Ошибка";
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ka='დავალება ვერ მოიძებნა.';ru='Задание не найдено.';en='The job is not found.'"),
				НСтр("ka='დავალება ვერ მოიძებნა.';ru='Задание не найдено.';en='The job is not found.'")));
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		СообщениеЖурнала = НСтр("ka='დავალება გაუქმებულია სისტემის ადმინისტრატორის მიერ.';ru='Задание отменено администратором системы.';en='Job canceled by the system administrator.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		...
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ka='დავალება გაუქმებულია სისტემის ადმინისტრატორის მიერ.';ru='Задание отменено администратором системы.';en='Job canceled by the system administrator.'"),
				СообщениеЖурнала));
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დავალება დასრულებულია შეცდომით. %1';ru='Задание завершено с ошибкой. %1';en='Job is completed with an error. %one'"),
	...
	Если ВРег(Лев(Сообщение, 6)) <> "<BODY>" Тогда
		...
		ПредставлениеHTML =
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				...
				+ "<br />"
				+ "<br />" + НСтр("ru='Технические подробности см. в <a href=""open:log"" >журнале регистрации</a>.';en='Technical details see. In <a href=""open:log""> log </a>.'")
				+ "<br /><br />" + НСтр("ru='При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.';en='If you have problems write to <a href=""mailto:webits-info@1c.ru""> </a> technical support.'")
				+ "</body>");
...
Процедура ОтобразитьОшибкуПодключения(Сообщение, ОписаниеОшибкиПодключения)
	...
	Элементы.КнопкаНазад.Доступность =
		...
		Или ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект));
	Элементы.КнопкаДалее.Заголовок    = НСтр("ka='Повторить попытку подключения >';ru='Повторить попытку подключения >';en='Retry connection>'");
	Элементы.КнопкаДалее.Доступность  = Истина;
	Элементы.КнопкаОтмена.Доступность = Истина;
	Элементы.ПродолжитьРаботуНаТекущейВерсии.Видимость = Ложь;
...
Процедура ОтобразитьПолучениеОбновленийЗавершено(Инициализировать = Ложь)
	...
	Иначе
		...
		Если ПараметрыАдминистрирования = Неопределено Тогда
			...
			ЗаголовокФормы = Нстр("ka='განახლების დაყენება';ru='Установка обновления';en='Installing the update'");
			Если ЭтоФайловаяИБ Тогда
				ПоясняющаяНадпись = Нстр("ka='Для установки обновления необходимо ввести"
"параметры администрирования информационной базы';ru='Для установки обновления необходимо ввести"
"параметры администрирования информационной базы'");
				ЗапрашиватьПараметрыАдминистрированияКластера = Ложь;
			Иначе
				ПоясняющаяНадпись = Нстр("ka='Для установки обновления необходимо ввести параметры"
"администрирования кластера серверов и информационной базы';ru='Для установки обновления необходимо ввести параметры"
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, Инициализировать) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = Нстр("ka='Для установки обновления необходимо ввести параметры администрирования.';ru='Для установки обновления необходимо ввести параметры администрирования.';en='To install the update, you must enter the administrative settings.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		
		ТекстСообщения = Нстр("ru='Не удалось установить обновление программы, т.к. не были введены"
"корректные параметры администрирования информационной базы.';ka='Не удалось установить обновление программы, т.к. не были введены"
...
"<br />Дистрибутив платформы скопирован в каталог <a href=""open:DistribFolder"">%2</a></body>';ka='<body>დაყენებულია ვერსია: <b>%1</b>"
"<br />პლატფორმის დისტრიბუტივი კოპირებულია კატალოგში <a href=""open:DistribFolder"">%2</a></body>'");
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если Инициализировать Тогда
				...
				Если Элементы.ПорядокОбновленияФайл.Видимость Тогда
					Элементы.НадписьПорядокОбновленияФайл.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='<body>Для продолжения необходимо ознакомиться с <a href=""%1"">порядком обновления</a>.</body>';en='<Body> To continue, you must familiarize yourself with <a href=""%1""> order updates </a>. </ Body>'"),
								ИнформацияОДоступномОбновлении.Конфигурация.URLПорядокОбновления));
				КонецЕсли;
			...
			Элементы.КнопкаДалее.Заголовок = ?(
				РежимОбновления = 0,
				НСтр("ka='შემდეგ >';ru='Далее >';en='Next>'"),
				НСтр("ka='მზად არის';ru='Готово';en='Done'"));
...
Процедура ОтобразитьНерекомендуемаяВерсияПлатформы()
	...
	Иначе
		Элементы.КнопкаНазад.Доступность = Ложь;
		Элементы.КнопкаДалее.Заголовок   = НСтр("ru='Обновить платформу >';en='Update platform>'");
	...
	Элементы.КнопкаОтмена.Заголовок   = НСтр("ka='მუშაობის დასრულება';ru='Завершить работу';en='To finish work'");
...
Функция ЗаголовокСтраницыНоваяВерсияПлатформы()
	...
	Возврат ?(ИнформацияОДоступномОбновлении.Платформа.РекомендуетсяПереход,
		НСтр("ru='Рекомендуется перейти на новую версию платформы';en='It is recommended to upgrade to the new version of the platform'"),
		НСтр("ka='ხელმისაწვდომია პლატფორმის ახალი ვერსია';ru='Доступна новая версия платформы';en='A new version of the platform'"));
...
Функция ТекстИнструкцииПоПереходуНаНовуюВерсиюПлатформы(ПараметрСоздатьРезервнуюКопию)
	...
	Если ВнедренаПодсистемаРезервногоКопирования
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
			Строки.Добавить(
				НСтр("ka='<body>ავტომატურად დასრულდება პროგრამის <a href=""open:ActiveUsers"">აქტიური მომხმარებლების</a> მუშაობა, შექმნილია სარეზერვო ასლი';ru='<body>Будет автоматически завершена работа <a href=""open:ActiveUsers"">активных пользователей</a> программы, создана резервная копия';en='<Body> The work will be completed automatically <a href=""open:ActiveUsers""> </a> active users of the program, the backup created'"));
		Иначе
			Строки.Добавить(
				НСтр("ka='<body>ავტომატურად დასრულდება პროგრამის აქტიური მომხმარებლების მუშაობა, შეიქმნება სარეზერვო ასლი';ru='<body>Будет автоматически завершена работа активных пользователей программы, создана резервная копия';en='<Body> will automatically terminate the program of active users, backup is created'"));
		КонецЕсли;
		
		Если ЭтоБазоваяВерсияКонфигурации Тогда
			Строки.Добавить(НСтр("ka='საინფორმაციო ბაზას და გაშვებულია პროგრამასთან პლატფორმის ახალ ვერსიაზე მუშაობის სეანსი.<body>';ru='информационной базы и запущен сеанс работы с программой на новой версии платформы.<body>';en='information base and started work session with the program on the new version of the platform. <body>'"));
		Иначе
			Строки.Добавить(НСтр("ru='информационной базы и запущен сеанс работы с программой на новой версии платформы."
"<br />На компьютерах других пользователей программы необходимо обновить платформу 1С:Предприятие вручную.</body>';ka='საინფორმაციო ბაზას და გაშვებულია პროგრამასთან პლატფორმის ახალ ვერსიაზე მუშაობის სეანსი."
"<br />პროგრამის სხვა მომხმარებლების კომპიუტერებზე აუცილებელია პლატფორმა 1С:საწარმოს ხელით განახლება.</body>'"));
		КонецЕсли;
		
	Иначе
		
		Строки.Добавить(НСтр("ka='<body>პლატფორმის ახალ ვერსიაზე მუშაობის დასაწყებად:';ru='<body>Чтобы начать работать на новой версии платформы:';en='<Body> In order to start working on a new version of the platform:'"));
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
			Строки.Добавить(НСтр("ka=' - დაასრულეთ პროგრამის ყველა <a href=""open:ActiveUsers"">აქტიური მომხმარებლის</a> მუშაობა;';ru='- завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;';en='- Exit all other <a href=""open:ActiveUsers""> </a> active users of the program;'"));
		Иначе
			Строки.Добавить(НСтр("ka=' - დაასრულეთ პროგრამის ყველა აქტიური მომხმარებლის მუშაობა;';ru='- завершите работу всех активных пользователей программы;';en='- shut down all active users of the program;'"));
		КонецЕсли;
		
		Строки.Добавить(НСтр("ka='- завершите текущий сеанс работы с программой;';ru='- завершите текущий сеанс работы с программой;';en='- complete the current work session with the program;'"));
		
		Если Не ЭтоБазоваяВерсияКонфигурации Тогда
			Строки.Добавить(НСтр("ka=' - პროგრამის სხვა მომხმარებლების კომპიუტერებზე დააყენეთ პლატფორმის ახალი ვერსია;';ru='- установите новую версию платформы на компьютеры других пользователей программы;';en='- install the new version of the platform on the computers of other users of the program;'"));
		КонецЕсли;
		
		Строки.Добавить(НСтр("ka=' - გახსენით პროგრამა პლატფორმის ახალი ვერსიით;';ru='- откройте программу на новой версии платформы;';en='- open the program on the new version of the platform;'"));
		
		Если ВнедренаПодсистемаРезервногоКопирования Тогда
			Строки.Добавить(НСтр("ka='<br />პლატფორმის ახალ ვერსიაზე მუშაობის დაწყებამდე რეკომენდებულია სარეზერვო ასლის შექმნა.</body>';ru='<br />Перед началом работы на новой версии платформы рекомендуется создать резервную копию.</body>';en='<br /> Before starting work on the new version of the platform is recommended to create a backup. </ body>'"));
		Иначе
			Строки.Добавить(НСтр("ka='<br />პლატფორმის ახალ ვერსიაზე მუშაობის დაწყებამდე რეკომენდებულია <a href=""open:RCInstruction"">სარეზერვო ასლის შექმნა</a>.</body>';ru='<br />Перед началом работы на новой версии платформы рекомендуется <a href=""open:RCInstruction"">создать резервную копию</a>.</body>';en='<br /> Before starting work on the new platform version <a href=""open:RCInstruction""> recommended to create a backup </a>. </ body>'"));
		КонецЕсли;
