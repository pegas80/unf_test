...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокЭлемента = НСтр("ka='Для автономной работы на Вашем компьютере должна быть установлена"
"платформа ""1С:Предприятие 8.3"" версии [ВерсияПлатформы]';ru='Для автономной работы на Вашем компьютере должна быть установлена"
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ka='Отменить создание автономного рабочего места?';ru='Отменить создание автономного рабочего места?';en='Cancel the creation of an autonomous workplace?'");
...
Процедура КакУстановитьИлиОбновитьВерсиюПлатформы1СПредприятие(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ka='როგორ დავაყენოთ ან განვაახლოთ პლატფორმა 1С:საწარმოს ვერსია';ru='Как установить или обновить версию платформы 1С:Предприятие';en='How to install or upgrade version of 1C: Enterprise'"));
...
Процедура СохранитьИнструкциюКак(Команда)
	...
	ПолучитьФайл(АдресВоВременномХранилище, НСтр("ka='Инструкция по настройке автономного рабочего места.html';ru='Инструкция по настройке автономного рабочего места.html';en='Instructions for setting up an autonomous working mesta.html'"));
...
Процедура СоздатьНачальныйОбразАвтономногоРабочегоМестаНаСервере(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
						...
						КонтекстПомощника,
						НСтр("ka='Создание начального образа автономного рабочего места';ru='Создание начального образа автономного рабочего места';en='Creating the initial image of the autonomous workplace'"),
						Истина);
...
Функция АдресПриложенияВИнтернете()
	...
	Если Не ПараметрыСоединения.Свойство("ws") Тогда
		ВызватьИсключение НСтр("ka='Создать автономное рабочее место можно только в режиме веб-клиента.';ru='Создать автономное рабочее место можно только в режиме веб-клиента.';en='Create a stand-alone workstation can only Web-client mode.'");
	КонецЕсли;
...
Функция Подключаемый_НастройкаВыгрузки_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеАвтономногоРабочегоМеста) Тогда
		...
		НСтрока = НСтр("ka='Не задано наименование автономного рабочего места.';ru='Не задано наименование автономного рабочего места.';en='Do not set the name of an autonomous workplace.'");
...
Функция Подключаемый_Окончание_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	ЗаголовокЭлемента = СтрЗаменить(ЗаголовокЭлемента, "[ЕдиницаИзмерения]", НСтр("ka='მბ';ru='Мб';en='MB'"));
