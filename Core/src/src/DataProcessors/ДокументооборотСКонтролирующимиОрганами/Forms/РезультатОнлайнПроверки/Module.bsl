...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ПрограммаПроверки) Тогда
		Элементы.ДекорацияПрограммаПроверки.Заголовок = "   " + НСтр("ka='შემოწმებულია';ru='Проверено';en='Checked'") + ": " + ПрограммаПроверки;
	КонецЕсли;
	...
	Если РезультатПроверкиВУниверсальномФормате = 0 ИЛИ РезультатПроверкиВУниверсальномФормате = "0" Тогда
		ЭлементДекорацияЗаголовкаПротокола.Заголовок = НСтр("ka='Ошибок не обнаружено. Отчет готов к отправке.';ru='Ошибок не обнаружено. Отчет готов к отправке.';en='No errors were found. The report is ready for shipment.'");
	...
	ИначеЕсли РезультатПроверкиВУниверсальномФормате = 4 ИЛИ РезультатПроверкиВУниверсальномФормате = "4" Тогда
		ЭлементДекорацияЗаголовкаПротокола.Заголовок = НСтр("ka='Есть предупреждения. При необходимости внесите исправления и повторите попытку.';ru='Есть предупреждения. При необходимости внесите исправления и повторите попытку.';en='There is a warning. If necessary, make corrections and try again.'");
...
Процедура КомандаСохранитьПротоколПроверки(Команда)
	...
	ИмяФайлаПротоколаПроверкиДляСохранения = ?(ЗначениеЗаполнено(ИмяФайлаПротокола), 
		ИмяФайлаПротокола, НСтр("ka='შემოწმების პროტოკოლი';ru='Протокол проверки';en='Authentication protocol'") + ".txt");
