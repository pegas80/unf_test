...
"%1';ka='Ошибка выгрузки сообщения о проверке во временный файл:"
...
"%1';ka='Ошибка чтения XML из файла:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""Файл"".';ru='Некорректная структура XML файла: не обнаружен узел ""Файл"".';en='Incorrect structure of the XML file not found ""file"" node.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""Документ"".';ru='Некорректная структура XML файла: не обнаружен узел ""Документ"".';en='Incorrect structure of the XML file not found ""Document"" node.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвПроверка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""СвПроверка"".';ru='Некорректная структура XML файла: не обнаружен узел ""СвПроверка"".';en='Incorrect structure of the XML file not found ""SvProverka"" node.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ОбщиеСведения.Свойство("Отметка") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен атрибут ""Отметка"".';ru='Некорректная структура XML файла: не обнаружен атрибут ""Отметка"".';en='Incorrect structure of the XML file: attribute not found ""mark.""'"),,,, Отказ);
		Возврат;
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Некорректная структура XML файла: некорректное значение атрибута ""Отметка"".';ru='Некорректная структура XML файла: некорректное значение атрибута ""Отметка"".';en='Incorrect structure of the XML file: invalid attribute value ""mark.""'"),,,, Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
