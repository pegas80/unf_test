...
"%1';ka='Ошибка выгрузки вложения во временный файл:"
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтметка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""СвОтметка"".';ru='Некорректная структура XML файла: не обнаружен узел ""СвОтметка"".';en='Incorrect structure of the XML file not found ""SvOtmetka"" node.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
