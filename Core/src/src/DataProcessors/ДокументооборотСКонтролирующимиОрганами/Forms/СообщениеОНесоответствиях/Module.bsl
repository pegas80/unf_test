...
"%1';ka='Ошибка выгрузки сообщения о несоответствиях во временный файл:"
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвНесоответ) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Некорректная структура XML файла: не обнаружен узел ""СвНесоответ"".';ru='Некорректная структура XML файла: не обнаружен узел ""СвНесоответ"".';en='Incorrect structure of the XML file not found ""SvNesootvet"" node.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
