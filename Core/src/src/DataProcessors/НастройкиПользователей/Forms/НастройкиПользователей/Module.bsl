...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Заголовок = НСтр("ka='მომხმარებლის გასწორებები';ru='Настройки пользователя';en='User Settings'");
...
Процедура ПользовательСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИспользоватьВнешнихПользователей Тогда
		...
		ВыборТипаПользователей.Добавить("ВнешниеПользователи", НСтр("ka='გარე მომხმარებლები';ru='Внешние пользователи';en='external users'"));
		ВыборТипаПользователей.Добавить("Пользователи", НСтр("ka='მომხმარებლები';ru='Пользователи';en='users'"));
...
Процедура НастройкиПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ТекстВопроса = НСтр("ka='Очистить выбранные настройки?';ru='Очистить выбранные настройки?';en='Clear your settings?'");
...
"Устранить проблему можно в карточке данного пользователя.';ka='У выбранного пользователя ""%1"" не указан"
"пользователь информационной базы, просмотреть его настройки невозможно."
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("НастройкиОтчетов", НСтр("ka='Настройки отчетов';ru='Настройки отчетов';en='reporting settings'"));
	КопируемыеНастройки.Добавить("НастройкиВнешнегоВида", НСтр("ka='Настройки внешнего вида';ru='Настройки внешнего вида';en='Appearance Settings'"));
	КопируемыеНастройки.Добавить("ДанныеФорм", НСтр("ka='Данные форм';ru='Данные форм';en='These forms'"));
	КопируемыеНастройки.Добавить("ПерсональныеНастройки", НСтр("ka='პერსონალური პარამეტრები';ru='Персональные настройки';en='personal settings'"));
	КопируемыеНастройки.Добавить("Избранное", НСтр("ka='Избранное';ru='Избранное';en='Избранное'"));
	КопируемыеНастройки.Добавить("НастройкиПечати", НСтр("ka='Настройки печати';ru='Настройки печати';en='Print settings'"));
	КопируемыеНастройки.Добавить(
		"ПрочиеПользовательскиеНастройки", НСтр("ka='Настройки дополнительных отчетов и обработок';ru='Настройки дополнительных отчетов и обработок';en='Set up additional reports and treatments'"));
...
Процедура Очистить(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ka='Необходимо выбрать настройки, которые требуется удалить.';ru='Необходимо выбрать настройки, которые требуется удалить.';en='You need to select the settings that you want to delete.'"));
		Возврат;
		
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ОчиститьЗавершение", ЭтотОбъект, ДеревоНастроек);
	ТекстВопроса = НСтр("ka='Очистить выделенные настройки?';ru='Очистить выделенные настройки?';en='Clear the selected settings?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
"Откроется окно выбора пользователей, которым необходимо очистить настройки.';ka='Очистить выделенные настройки?"
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = НСтр("ka='Очистить все настройки у пользователя ""%1""?';ru='Очистить все настройки у пользователя ""%1""?';en='Clear all settings at the user ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ka='გასუფთავება';ru='Очистить';en='clean'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = НСтр("ka='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';ru='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';en='Clear all reporting settings and appearance of the user ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ka='გასუფთავება';ru='Очистить';en='clean'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
"Продолжить?';ka='Сейчас будут очищены все настройки всех пользователей."
"Продолжить?'");
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("ОчиститьВсе", НСтр("ka='ყველას გასუფთავება';ru='Очистить все';en='clear all'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
...
Функция ОбновлениеСпискаНастроек()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Обновление настроек пользователей';ru='Обновление настроек пользователей';en='Updating user settings'");
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Настройки отчетов (%1)';ru='Настройки отчетов (%1)';en='Reporting Settings (1%)'"), КоличествоНастроек);
	Иначе
		Элементы.НастройкиОтчетовСтраница.Заголовок = НСтр("ka='Настройки отчетов';ru='Настройки отчетов';en='reporting settings'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Внешний вид (%1)';ru='Внешний вид (%1)';en='Appearance (1%)'"), КоличествоНастроек);
	Иначе
		Элементы.ВнешнийВидСтраница.Заголовок = НСтр("ka='გარეგნული იერი';ru='Внешний вид';en='Appearance'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Прочие настройки (%1)';ru='Прочие настройки (%1)';en='Other settings (1%)'"), КоличествоНастроек);
	Иначе
		Элементы.ПрочиеНастройкиСтраница.Заголовок = НСтр("ka='სხვა გამართვები';ru='Прочие настройки';en='Other settings'");
	КонецЕсли;
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		...
		Если Параметр.КопироватьВсе Тогда
			...
			Если МассивНастроек.Количество() = 7 Тогда
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Скопированы все настройки %1';ru='Скопированы все настройки %1';en='Copied all the settings% 1'"), ПояснениеКомуСкопированыНастройки);
			Иначе
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 скопированы %2';ru='%1 скопированы %2';en='% 1% 2 copied'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
			КонецЕсли;
				
			Состояние(НСтр("ka='Выполняется копирование настроек';ru='Выполняется копирование настроек';en='Copying settings'"));
			
			Отчет = Неопределено;
			СкопироватьВсеНастройкиНаСервере(ПользовательИнформационнойБазы, ПользователиПриемник, МассивНастроек, Отчет);
			
			Если Отчет <> Неопределено Тогда
				ТекстВопроса = НСтр("ka='Не все варианты отчетов и настройки были скопированы.';ru='Не все варианты отчетов и настройки были скопированы.';en='Not all versions of the reports and settings are copied.'");
				КнопкиВопроса = Новый СписокЗначений;
				КнопкиВопроса.Добавить("Ок", НСтр("ka='ОК';ru='ОК';en='OK'"));
				КнопкиВопроса.Добавить("ПоказатьОтчет", НСтр("ka='ანგარიშგების ჩვენება';ru='Показать отчет';en='Show report'"));
			...
			ПоказатьОповещениеПользователя(
				НСтр("ka='Копирование настроек';ru='Копирование настроек';en='copy settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ka='Очистка настроек';ru='Очистка настроек';en='Cleanup settings'"), ,
		НСтр("ka='Очищены все настройки всех пользователя';ru='Очищены все настройки всех пользователя';en='Cleared all the settings of this member'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ka='Очищены все настройки пользователя ""%1""';ru='Очищены все настройки пользователя ""%1""';en='User cleared all settings ""%1""'");
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПояснения, ПользовательСсылка);
	ПоказатьОповещениеПользователя(НСтр("ka='Очистка настроек';ru='Очистка настроек';en='Cleanup settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ka='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""';ru='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""';en='Cleaned all the report settings and appearance of the user ""%1""'");
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = Формат(КоличествоНастроек, "ЧДЦ=0") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНастроек,
			"Л = ru_RU", НСтр("ka='გამართვა,გამართვები,გამართვებს,,,,,,0';ru='настройка,настройки,настроек,,,,,,0';en='setup, configuration, settings ,,,,,, 0'"));
	
	Если КоличествоНастроек = 1
		И КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = НСтр("ka='""%1"" очищена пользователю ""%2""';ru='""%1"" очищена пользователю ""%2""';en='""%1"" purified user ""%2""'");
	...
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = НСтр("ka='Очищено %1 пользователю ""%2""';ru='Очищено %1 пользователю ""%2""';en='Cleaned 1% user ""%2""'");
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = НСтр("ka='""%1"" очищена %2';ru='""%1"" очищена %2';en='""%1""% cleared 2'");
		...
		Иначе
			ТекстПояснения = НСтр("ka='Очищено %1 %2';ru='Очищено %1 %2';en='Cleaned% 1% 2'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ka='Очистка настроек';ru='Очистка настроек';en='Cleanup settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Необходимо выбрать настройки, которые требуется скопировать.';ru='Необходимо выбрать настройки, которые требуется скопировать.';en='You need to select the settings you want to copy.'"));
		Возврат;
...
"его пересохранить со снятой пометкой ""Только для автора"".';ka='Невозможно скопировать личный вариант отчета."
"Для того чтобы личный вариант отчета стал доступен другим пользователям, необходимо"
...
"Копирование настроек личных вариантов отчетов не предусмотрено.';ka='Невозможно скопировать настройку личного варианта отчета."
