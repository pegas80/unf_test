...
Процедура ПопробоватьУдалитьСсылку(ПараметрыВыполнения, Ссылка, Информация, Результат)
	...
	Если Объект.ПометкаУдаления <> Истина Тогда
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("ka='Объект не помечен на удаление.';ru='Объект не помечен на удаление.';en='Object is not marked for deletion.'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("ka='Объект используется в других объектах программы.';ru='Объект используется в других объектах программы.';en='The object used in other program objects.'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьПредупреждение(Ссылка, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='მონიშნულების წაშლა';ru='Удаление помеченных';en='Deleting flagged'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если СтрокаТаблицы.ОбнаруженныйСсылка = Неопределено Тогда
		...
		Иначе
			Причина.ОбнаруженныйСсылка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Обнаружены неразрешимые ссылки (%1)';ru='Обнаружены неразрешимые ссылки (%1)';en='Found insoluble links (1%)'"),
...
Процедура КлиентОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Если ИмяКоллекции = "ПередПоискомПомеченныхНаУдаление" Тогда
		...
		Текст = НСтр("ka='Подготовка к поиску объектов, помеченных на удаление.';ru='Подготовка к поиску объектов, помеченных на удаление.';en='Preparing for the search of objects that are marked for deletion.'");
		
	ИначеЕсли ИмяКоллекции = "НайтиПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("ka='Поиск объектов, помеченных на удаление.';ru='Поиск объектов, помеченных на удаление.';en='Find objects that are marked for deletion.'");
		
	ИначеЕсли ИмяКоллекции = "ВсеПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("ka='Анализ помеченных на удаление.';ru='Анализ помеченных на удаление.';en='Analysis marked for deletion.'");
		
	ИначеЕсли ИмяКоллекции = "ТехнологическиеОбъекты" Тогда
		
		Текст = НСтр("ka='Подготовка к удалению.';ru='Подготовка к удалению.';en='Preparing to remove.'");
		
	ИначеЕсли ИмяКоллекции = "МонопольноеУдаление" Тогда
		
		Текст = НСтр("ka='Выполняется удаление объектов.';ru='Выполняется удаление объектов.';en='Removes the objects.'");
	...
	ИначеЕсли ИмяКоллекции = "ПользовательскиеОбъекты" Тогда
		...
		Если НеУдалено = 0 Тогда // Переход на СтрШаблон невозможен.
			Текст = НСтр("ka='Удалено: %1 из %2 объектов.';ru='Удалено: %1 из %2 объектов.';en='Removed% 1% 2 objects.'");
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ПредставлениеНомер, ПредставлениеВсего);
		Иначе
			Текст = НСтр("ka='Обработано: %1 из %2 объектов, из них не удалено: %3.';ru='Обработано: %1 из %2 объектов, из них не удалено: %3.';en='Processed% 1 of% 2 objects are not removed:% 3.'");
	...
	ИначеЕсли ИмяКоллекции = "ПовторноУдаляемые" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Повторная проверка не удаленных объектов: %1 из %2.';ru='Повторная проверка не удаленных объектов: %1 из %2.';en='Retesting is not remote sites:% 1% 2.'"),
	...
	ИначеЕсли ИмяКоллекции = "ПрепятствующиеУдалению" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Анализ объектов, препятствующих удалению: %1 из %2.';ru='Анализ объектов, препятствующих удалению: %1 из %2.';en='Analysis objects hindering removal% 1% 2.'"),
			Формат(ПараметрыВыполнения.Номер, "ЧН=0; ЧГ="),
			Формат(ПараметрыВыполнения.Всего, "ЧН=0; ЧГ="));
