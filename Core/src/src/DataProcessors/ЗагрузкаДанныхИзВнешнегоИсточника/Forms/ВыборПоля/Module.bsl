...
Процедура ОбработкаГруппыПолей(Поле, Отказ)
	...
	Если НЕ ПустаяСтрока(Поле.ИмяГруппыПолей) Тогда
		...
		Отказ = Истина;
		ПоказатьПредупреждение(, Нстр("ka='Выбор групп не предусмотрен...';ru='Выбор групп не предусмотрен...';en='Group selection is not provided ...'"));
...
Процедура ОбработкаПоляДополнительныхРеквизитов(Поле, Отказ)
	...
	Если Поле.ИмяПоля = КэшЗначений.ИмяПоляДополнительныхРеквизитов Тогда
		...
		Если Параметры.НастройкиЗагрузкиДанных.ВыбранныеДополнительныеРеквизиты.Количество() >= МаксимумДополнительныхРеквизитов Тогда
			...
			ТекстПредупреждения = НСтр("ka='Большое количество дополнительных реквизитов в загрузке существенно замедляет процесс. "
"Рекомендуется разделить загрузку на несколько итераций.';ru='Большое количество дополнительных реквизитов в загрузке существенно замедляет процесс. "
"Рекомендуется разделить загрузку на несколько итераций.'");
			
			ТекстЗаголовка = СтрШаблон(НСтр("ka='Выбрано %1 реквизита';ru='Выбрано %1 реквизита';en='Joined 1% props'"), МаксимумДополнительныхРеквизитов);
