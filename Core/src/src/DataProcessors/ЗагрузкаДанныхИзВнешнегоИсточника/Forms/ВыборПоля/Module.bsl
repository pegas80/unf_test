...
Процедура ОбработкаГруппыПолей(Поле, Отказ)
	...
	Если НЕ ПустаяСтрока(Поле.ИмяГруппыПолей) Тогда
		...
		Отказ = Истина;
		ПоказатьПредупреждение(, Нстр("ru = 'Выбор групп не предусмотрен...'; ka = 'ჯგუფის შერჩევა არ არის გათვალისწინებული...'; en = 'Group selection is not provided ...'"));
...
Процедура ОбработкаПоляДополнительныхРеквизитов(Поле, Отказ)
	...
	Если Поле.ИмяПоля = КэшЗначений.ИмяПоляДополнительныхРеквизитов Тогда
		...
		Если Параметры.НастройкиЗагрузкиДанных.ВыбранныеДополнительныеРеквизиты.Количество() >= МаксимумДополнительныхРеквизитов Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Большое количество дополнительных реквизитов в загрузке существенно замедляет процесс. ""Рекомендуется разделить загрузку на несколько итераций.'; ka = 'ჩამოტვირთვისას დამატებითი დეტალების დიდი რაოდენობა მნიშვნელოვნად ანელებს პროცესს. ”რეკომენდებულია ჩამოტვირთვის გაყოფა რამდენიმე გამეორებად.'; en = 'A large number of additional details in the download significantly slows down the process. “It is recommended to divide the download into several iterations.'");
			
			ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Выбрано %1 реквизита'; ka = 'არჩეულია %1 ატრიბუტი'; en = 'Joined 1% props'"), МаксимумДополнительныхРеквизитов);
...
Процедура ЗапомнитьВыборИЗакрытьФорму(Поле, ДополнительныйРеквизит = Неопределено)
	...
	Результат.Вставить("НомерЦвета", 	Поле.НомерЦвета); //Малышав 2024-08-01   
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗаголовокКолонки", ЗаголовокКолонки) Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ka='ველის არჩევა';ru='Выбор поля';en='field Chooser'") + ?(ПустаяСтрока(ЗаголовокКолонки), "", ": " + СокрЛП(ЗаголовокКолонки));
	...
	КэшЗначений.Вставить("ПолеВозвратаСписок", НСтр("ka='უკან,რეკვიზიტების სიაზე დაბრუნება';ru='Назад к списку реквизитов';en='Back to the details of the list'"));
