...
Процедура ОткрытьОбразец(Расширение)
	...
	Если Расширение = "mxl" Тогда
		...
		Если ТипЗнч(ТабличныйДокумент) = Тип("ТабличныйДокумент") Тогда
			...
			ПечатнаяФорма.СинонимМакета = НСтр("ka='მომზადებული მონაცემების ნიმუში mxl ფორმატისთვის';ru='Образец подготовленных данных в формате mxl';en='The sample is prepared in the data format mxl'");
			ПечатнаяФорма.ТабличныйДокумент = ТабличныйДокумент;
			ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ka='მომზადებული მონაცემების ნიმუში mxl ფორმატისთვის';ru='Образец подготовленных данных в формате mxl';en='The sample is prepared in the data format mxl'");
...
Процедура ЗаполнитьСписокВыбораФильтраТаблицыСопоставления(ЭтоЗагрузкаТабличнойЧасти)
	...
	Если Параметры.НастройкиЗагрузкиДанных.ЭтоЗагрузкаТабличнойЧасти Тогда
		...
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='ОтборОшибокНет';ru='ОтборОшибокНет';en='OtborOshibokNet'"),	Нстр("ka='მონაცემები, რომლებიც მზადაა ჩასატვირთად';ru='Данные, готовые к загрузке';en='Data ready for upload'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(2, Нстр("ka='ОтборОшибки';ru='ОтборОшибки';en='OtborOshibki'"), 	Нстр("ka='მონაცემები, რომელთა ჩატვირთვაც არ არის შესაძლებელი';ru='Данные, которые загрузить невозможно';en='Data that can not be loaded'"));
		
	Иначе
		
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='Сопоставленные';ru='Сопоставленные';en='mapped'"), Нстр("ka='მონაცემები, რომლებიც შეესაბამება';ru='Данные, которые удалось сопоставить';en='Data that could be compared'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='БудутСозданы';ru='БудутСозданы';en='BudutSozdany'"), Нстр("ka='მონაცემები, რომლებიც არ მოიძებნა შესაბამისობის პროგრამაში';ru='Данные, которым не найдено соответствие в программе';en='Data that is not found in compliance with the program'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ka='Несогласованные';ru='Несогласованные';en='inconsistent'"), Нстр("ka='მონაცემები, რომლებიც შეიცავენ შეცდომას (შევსება არასრულია)';ru='Данные, которые содержат ошибку (заполнены не полностью)';en='The data that contains an error (not fully filled)'"));
...
Процедура УстановитьТекстЗаголовкаДекорацииНесопоставленныеСтроки()
	...
	Если СоздаватьЕслиНеСопоставлено Тогда
		...
		ТекстЗаголовка = ?(Параметры.НастройкиЗагрузкиДанных.ЭтоЗагрузкаСправочника, 
			Нстр("ka='будет создано новых элементов:';ru='будет создано новых элементов:';en='It will create new elements:'"), 
			Нстр("ka='будет создано новых записей:';ru='будет создано новых записей:';en='It will create new entries:'"));
		
	Иначе
		
		ТекстЗаголовка = Нстр("ka='будет пропущено строк:';ru='будет пропущено строк:';en='will be skipped lines:'");
...
Процедура УстановитьТекстЗаголовкаДекорацииСопоставленыхОбъектов()
	...
	ТекстЗаголовока = ?(ОбновлятьСуществующие,
		Нстр("ka='из них сопоставлены и будут обновлены';ru='из них сопоставлены и будут обновлены';en='They are collated and will be updated'"),
		Нстр("ka='из них сопоставлены';ru='из них сопоставлены';en='are compared'"));
...
Процедура УстановитьТекстЗаголовкаДекорацииОбщееЗначение()
	...
	Если Параметры.НастройкиЗагрузкиДанных.Свойство("ОбщееЗначение")
		...
		Если Параметры.НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Справочник.Контрагенты"
			...
			Элементы.ОбщееЗначениеСправочник.Заголовок = Нстр("ru='< ';ka='< '") + Строка(Параметры.НастройкиЗагрузкиДанных.ОбщееЗначение) + Нстр("ka=' >';ru=' >'");
			
		ИначеЕсли Параметры.НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "РегистрСведений.ЦеныНоменклатуры" Тогда
			
			Элементы.ОбщееЗначениеРС.Заголовок = Нстр("ru='< ';ka='< '") + Строка(Параметры.НастройкиЗагрузкиДанных.ОбщееЗначение) + Нстр("ka=' >';ru=' >'");
...
Процедура СобратьЗаголовокСтраницыПроверкаДанных()
	...
	ТекстОбычный = НСтр("ru = 'Если найдено %1 пустых строк, обработка следующих строк не выполняется.""При проверке заполнения табличного документа '; ka = 'თუ %1 ცარიელი ხაზები მოიძებნება, შემდეგი სტრიქონები არ დამუშავდება.""ცხრილების დოკუმენტის შევსების შემოწმებისას '; en = 'Если найдено %1 пустых строк, обработка следующих строк не выполняется.""При проверке заполнения табличного документа '");
	ТекстОбычный = СтрШаблон(ТекстОбычный, ЗагрузкаДанныхИзВнешнегоИсточника.МаксимальноеКоличествоПустыхСтрок());
	
	ТекстЖирный = НСтр("ka='აღმოჩენილია შეცდომები:';ru='обнаружены следующие ошибки:';en='The following errors are detected:'");
...
Процедура ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(ПерейтиДалее)
	...
	Описание = НСтр("ka='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузка данных из файла';ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузка данных из файла';en='Subsystem ZagruzkaDannyhIzVneshnegoIstochnika: Performing server method of loading data from a file'");
...
Процедура УстановитьТекстДекорацииКоличестваСтрок()
	...
	Если ФильтрТаблицаСопоставления = "БезОтбора" Тогда 
		...
		НовыйЗаголовок = Нстр("ka='Всего строк в таблице .......... %1';ru='Всего строк в таблице .......... %1';en='Number of rows in the table .......... 1%'");
		ЗначениеПараметра = КоличествоСтрокТаблицы;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "ОтборОшибокНет" Тогда 
		
		НовыйЗаголовок = Нстр("ka='Строк с данными, которые возможно загрузить в приложение .......... %1';ru='Строк с данными, которые возможно загрузить в приложение .......... %1';en='Rows of data that may be loaded into the application .......... 1%'");
		ЗначениеПараметра = КоличествоСтрокБезОшибок;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "ОтборОшибки" Тогда 
		
		НовыйЗаголовок = Нстр("ka='Строки, содержащие ошибки и препятствующие загрузке данных .......... %1';ru='Строки, содержащие ошибки и препятствующие загрузке данных .......... %1';en='The rows that contain errors and preventing data loading .......... 1%'");
	...
	ИначеЕсли ФильтрТаблицаСопоставления = "Сопоставленные" Тогда 
		...
		Если ОбновлятьСуществующие Тогда
			...
			НовыйЗаголовок = Нстр("ka='Данные, которые соответствуют элементам программы и будут обновлены .......... %1';ru='Данные, которые соответствуют элементам программы и будут обновлены .......... %1';en='The data that correspond to the elements of the program and will be updated .......... 1%'");
			
		Иначе
			
			НовыйЗаголовок = Нстр("ka='Данные, которые соответствуют элементам программы .......... %1';ru='Данные, которые соответствуют элементам программы .......... %1';en='Data that correspond to the programs ..........% 1'");
	...
	ИначеЕсли ФильтрТаблицаСопоставления = "БудутСозданы" Тогда 
		...
		НовыйЗаголовок = Нстр("ka='Данные, которые не удалось сопоставить .......... %1';ru='Данные, которые не удалось сопоставить .......... %1';en='Data that could not be compared .......... 1%'");
		ЗначениеПараметра = НесопоставленоДанных;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "Несогласованные" Тогда 
		
		НовыйЗаголовок = Нстр("ka='Строки, которые содержат ошибку либо заполнены не полностью .......... %1';ru='Строки, которые содержат ошибку либо заполнены не полностью .......... %1';en='The rows that contain errors or are not filled completely .......... 1%'");
...
Процедура ВыбратьВнешнийФайлОбработкаЗавершение(Результат, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ka='Импорт данных из файлов данного типа не поддерживается.';ru='Импорт данных из файлов данного типа не поддерживается.';en='Import data from this file type is not supported.'");
...
Процедура ОбработкаРасшифровкиЗаголовкаКолонки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Свойство("ОтменитьВыборВКолонке") Тогда
			...
			ОбластьЗаголовка.Текст 					= НСтр("ka='აირჩიეთ ჩასატვირთი ველი';ru='Не загружать';en='Do not load'");
