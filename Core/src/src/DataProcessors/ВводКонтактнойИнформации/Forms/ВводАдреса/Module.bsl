...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидКонтактнойИнформации.ПроверятьПоФИАС Тогда
		Команды.ПроверитьЗаполнениеАдреса.Подсказка = НСтр("ka='Проверить заполнение адреса в формате ФИАС';ru='Проверить заполнение адреса в формате ФИАС';en='Check the filling of addresses in the format of FIAS'");
	Иначе
		Команды.ПроверитьЗаполнениеАдреса.Подсказка  = НСтр("ka='Проверить заполнение адреса в формате КЛАДР';ru='Проверить заполнение адреса в формате КЛАДР';en='Check the filling of addresses in the format KLADR'");
	КонецЕсли;
	...
	Иначе
		...
		Если Параметры.Свойство("Страна") И ЗначениеЗаполнено(Параметры.Страна) Тогда 
			...
			КонецЕсли;
		Иначе
			XDTOКонтактная.Состав.Страна = "საქართველო";//СтранаРоссия.Наименование Паганини
		КонецЕсли;
	КонецЕсли;
	...
	Если АдресТолькоРоссийский  Тогда
		...
		Если Страна <> СтранаРоссия Тогда
			...
			ТекстПредупрежденияПриОткрытии = НСтр("ka='Адрес введен некорректно: допускается ввод только российских адресов. Значение поля ""Страна"" было изменено на ""Россия"", необходимо проверить остальные поля.';ru='Адрес введен некорректно: допускается ввод только российских адресов. Значение поля ""Страна"" было изменено на ""Россия"", необходимо проверить остальные поля.';en='Your address is invalid: You can enter only addresses Russian. The value of the field ""Country"" was changed to ""Russia"", you need to check out the rest of the field.'"); 
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					...
					Если СведенияОРегионе.Загружен = Ложь Тогда
						...
						ДоступностьКлассификатора.КраткоеПредставлениеОшибки = 
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").';ru='Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").';en='You need to download the address information in the region ""%1"" (in the ""%2"" menu).'"),
							XDTOКонтактная.Состав.Состав.СубъектРФ,
							?(ЭтоТакси, НСтр("ka='კიდე';ru='Еще';en='Still'"), НСтр("ka='ყველა მოქმედება';ru='Все действия';en='All actions'")));
	...
	ИначеЕсли ВыбранноеЗначение.Отказ Тогда
		...
		Иначе
			Если НЕ ПодборПоИндексуДоступен Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Индекс %1 не найден в адресном классификаторе';ru='Индекс %1 не найден в адресном классификаторе';en='The index% 1 is not found in the address qualifier'"),
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если ЕстьПравоЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиент.ПредложениеЗагрузкиКлассификатора(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Данные для ""%1"" не загружены.';ru='Данные для ""%1"" не загружены.';en='Data for ""%1"" is not loaded.'"), ВыбранноеЗначение.Представление),
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Улица", Улица);
		ПараметрыФормы.Вставить("Заголовок", НаселенныйПункт + " " + НСтр("ka='(не найден)';ru='(не найден)';en='(not found)'"));
	КонецЕсли;
...
Процедура АдресНаДатуАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если СтрСравнить(Текст, НСтр("ka='начала учета';ru='начала учета';en='start accounting'")) = 0 Или ПустаяСтрока(Текст) Тогда
...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		Если ЗначениеЗаполнено(Результат.ДействуетПо) Тогда
			ТекстИсторическийАдрес = " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='действует по %1';ru='действует по %1';en='It acts on% 1'"), Формат(Результат.ДействуетПо - 10, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = НСтр("ka='действует по настоящее время.';ru='действует по настоящее время.';en='valid to the present.'");
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ПоказатьПредупреждение(, НСтр("ka='Адрес не может быть проверен, так как он введен в свободной форме.';ru='Адрес не может быть проверен, так как он введен в свободной форме.';en='The address can not be verified, since it is introduced into the free form.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ТекстВопроса = НСтр("ka='Изменения, введенные вручную, будут потеряны."
"Исходный адрес будет перемещен в комментарий."
"Продолжить?';ru='Изменения, введенные вручную, будут потеряны."
"Исходный адрес будет перемещен в комментарий."
"Продолжить?'");
	Иначе
		ТекстВопроса = НСтр("ka='Ввести адрес в свободной форме?"
"Адреса, введенные в свободной форме, могут не пройти проверку по адресному классификатору.';ru='Ввести адрес в свободной форме?"
"Адреса, введенные в свободной форме, могут не пройти проверку по адресному классификатору.'");
	...
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ka='დადასტურება';ru='Подтверждение';en='the confirmation'"));
КонецПроцедуры
...
"Рекомендуется выполнить обновление, чтобы проверка адреса выполнялась корректно.';ka='Адресные сведения ФИАС, загруженные в программу, устарели."
"Рекомендуется выполнить обновление, чтобы проверка адреса выполнялась корректно.'");
	...
	Иначе
		СообщениеОПроверкеАдреса = СообщениеОПроверкеАдреса + Символы.ПС
			+ НСтр("ka='Для обновления адресного классификатора необходимо обратиться к администратору.';ru='Для обновления адресного классификатора необходимо обратиться к администратору.';en='To update the address of the classifier need to contact the administrator.'");
...
Процедура ПоказатьСписокДомов(ВключитьОтбор)
	...
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр("ka='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.';ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.';en='Selecting from the list is not available, because in the address qualifier is no information about the numbering of houses for the entered address.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='Для просмотра список домов заполните поля адреса.';ru='Для просмотра список домов заполните поля адреса.';en='To view the list of houses fill the address fields.'");
		КонецЕсли;
...
Процедура ВопросОбОбновлениеАдресногоКлассификатора(Знач СообщениеОПроверкеАдреса)
	...
	Кнопки.Добавить("ОбновитьКлассификатор", НСтр("ka='კლასიფიკატორის განახლება';ru='Обновить классификатор';en='update qualifier'"));
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РазрешитьВводАдресаВСвободнойФорме И ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
		Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ НСтр("ka='Исходный адрес:';ru='Исходный адрес:';en='The original address:'") + " " + ПредставлениеАдреса;
		ПодключитьОбработчикОжидания("УстановитьПиктограммуКомментария", 0.1, Истина);
	КонецЕсли;
...
Функция СписокОшибокЗаполнения(Контекст, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если Результат.Количество() = 0 // Нет ошибок
		...
		Результат.Добавить("/", НСтр("ka='Адрес пуст.';ru='Адрес пуст.';en='Address is empty.'"));
	КонецЕсли;
...
Процедура ОтобразитьКоличествоЗаписейВИсторииИзменений()
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='История изменений (%1)';ru='История изменений (%1)';en='Change History (1%)'"), НайденныеСтроки.Количество());
	...
	Иначе
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = НСтр("ka='ცვლილებების ისტორია';ru='История изменений';en='change History'");
		Элементы.ИсторияИзмененийГиперссылка.Видимость = Истина;
	КонецЕсли;
...
"Рекомендуется выполнить обновление и повторить проверку адреса заново.';ka='Адрес введен корректно, но адресные сведения, загруженные в программу, давно не обновлялись."
...
"Для обновления адресного классификатора необходимо обратиться к администратору.';ka='Адрес введен корректно, но адресные сведения, загруженные в программу,"
"давно не обновлялись. Рекомендуется выполнить обновление и повторить проверку адреса заново."
"Для обновления адресного классификатора необходимо обратиться к администратору.'");
				ПоказатьПредупреждение(, СообщениеОПроверкеАдреса);
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Адрес введен корректно.';ru='Адрес введен корректно.';en='Address is entered correctly.'"));
		КонецЕсли;
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].Пометка = Ложь И НЕ ЭтоВебСервис Тогда
			ТекстВопроса = НСтр("ka='Адрес не может быть проверен.';ru='Адрес не может быть проверен.';en='The address can not be verified.'") + Символы.ПС + СписокОшибок[0].Представление + Символы.ПС + НСтр("ka='Загрузить классификатор для проверки адреса?';ru='Загрузить классификатор для проверки адреса?';en='Upload a classifier for address verification?'");
...
Процедура СостояниеВводаПредставленияАдреса(РедактироватьПредставлениеАдреса, ФормироватьПредставление = Истина)
	...
	Если РедактироватьПредставлениеАдреса Тогда
		...
		Элементы.ПредставлениеАдреса.Заголовок          = НСтр("ka='Адрес в свободной форме';ru='Адрес в свободной форме';en='Address in free form'");
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ДоступностьКлассификатора <> Неопределено Тогда
			...
			Если ДоступностьКлассификатора.Отказ Тогда
				ТекстСообщенияСервиса = НСтр("ka='Автоподбор и проверка адреса недоступны:';ru='Автоподбор и проверка адреса недоступны:';en='Autofit and address verification are not available:'") + Символы.ПС 
				+ ДоступностьКлассификатора.КраткоеПредставлениеОшибки;
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='Проверка доступности сервиса адресного классификатора';ru='Проверка доступности сервиса адресного классификатора';en='Check the availability of the service address of the classifier'");
