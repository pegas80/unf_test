...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ДанныеКлассификатора.Данные.Количество() = 0 Тогда
		...
		КраткоеПредставлениеОшибки = НСтр("ka='Индекс не найден в адресном классификаторе.';ru='Индекс не найден в адресном классификаторе.';en='The index is not found in the address qualifier.'");
		Возврат;
	КонецЕсли;
...
Процедура ПроизвестиВыбор(Знач НомерСтроки)
	...
	ПредупреждениеНеактуальности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Адрес ""%2, %1"" неактуален."
"Продолжить?';ru='Адрес ""%2, %1"" неактуален."
