...
Процедура ПроверитьСписокЗагружаемыхВалют()
	Если Объект.СписокВалют.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ka='В справочнике валют отсутствуют валюты, курсы которых можно загружать из сети Интернет.';ru='В справочнике валют отсутствуют валюты, курсы которых можно загружать из сети Интернет.';en='The reference rates are no currency that can be downloaded from the Internet.'");
...
Функция ВыполнитьЗагрузкуКурсов()
	...
	НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელით გაშვება: %1';ru='Запуск вручную: %1';en='Manual start:% 1'"), РегламентноеЗадание.Метаданные.Синоним);
...
Процедура ОбработатьРезультатЗагрузки(РезультатЗагрузки)
	...
	Иначе
		...
		Если КоличествоОшибок > 1 Тогда
			...
			Кнопки.Добавить("Подробнее", НСтр("ka='დაწვრილებით...';ru='Подробнее...';en='More ...'"));
			Кнопки.Добавить("Продолжить", НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось загрузить курсы валют (%1).';ru='Не удалось загрузить курсы валют (%1).';en='Failed to load exchange rates (1%).'"), КоличествоОшибок);
...
"Подключиться сейчас?';ka='Для загрузки курсов валют из Интернета"
"необходимо подключиться к Интернет-поддержке пользователей."
"Подключиться сейчас?'");
...
Процедура ПродолжитьЗагрузкуКурсовВалют()
	...
	Если Не ЗначениеЗаполнено(Объект.НачалоПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не задана дата начала периода загрузки.';ru='Не задана дата начала периода загрузки.';en='Do not set the date for the download period.'"),
			,
	...
	Если Не ЗначениеЗаполнено(Объект.ОкончаниеПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не задана дата окончания периода загрузки.';ru='Не задана дата окончания периода загрузки.';en='Do not set the date of the end of the loading period.'"),
			,
