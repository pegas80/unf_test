...
Процедура ЗаполнитьГруппыКатегории()
	...
	ИначеЕсли КоличествоГрупп = 0 Тогда
	    ОтборКатегорииГруппыПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры;
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ru = 'Выберите категории номенклатуры для переноса на сайт'; ka = 'აირჩიეთ პროდუქტის კატეგორიები საიტზე გადასატანად'; en = 'Select the range of categories for the transfer of the site'");
	...
	ИначеЕсли КоличествоКатегорий = 0 Тогда
		...
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ru = 'Выберите группы номенклатуры для переноса на сайт'; ka = 'აირჩიეთ ნომენკლატურის ჯგუფები საიტზე გადასატანად'; en = 'Select the range of the group for the transfer of the site'");
...
Процедура ЗаполнитьДеревоГрупп()
	...
	СтрокаВсеГруппы.Представление= НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'");
	СтрокаВсеГруппы.Флаг = Истина;
	
	СтрокаВсеГруппы = Дерево.Строки.Добавить();
	СтрокаВсеГруппы.Значение	 = Справочники.Номенклатура.ПустаяСсылка();
	СтрокаВсеГруппы.Представление= НСтр("ka='<არ არის ჯგუფი>';ru='<Нет группы>';en='<No Group>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Процедура ЗаполнитьДеревоКатегорий()
	...
	СтрокаВсеГруппы.Представление= НСтр("ru = '<Все категории>'; ka = '<ყველა კატეგორია>'; en = '<All categories>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Процедура ПометкаИерархииПриИзменении()
	...
	Если ТекущиеДанные.Представление = НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'") Тогда
		//Все элементы дерева
...
Функция СоздатьСайтВФоне()
	...
	Заголовок = НСтр("ru = 'Создание сайта'; ka = 'საიტის შექმნა'; en = 'Create site'");
...
Функция СоздатьСайтВФонеПродолжить(ПараметрыСозданияСайта)
	...
	Заголовок = НСтр("ru = 'Заполнение сайта'; ka = 'საიტის შევსება'; en = 'filling the site'");
...
Функция ПолучитьТаблицуКаталогов()
	...
	Если НЕ ВсеГруппы Тогда
		...
		Для Каждого ЭлементСЗ Из СписокГрупп Цикл
			...
			Если ЭлементСЗ.Представление = НСтр("ka='<ყველა ჯგუფი>';ru='<Все группы>';en='<All Groups>'") Тогда
	...
	НовСтр.Каталог = НСтр("ka='საქონლის ძირითადი კატალოგი';ru='Основной каталог товаров';en='The main catalog'");
...
Процедура СоздатьСайт(Команда)
	...
	Если ТипСайта=0 Тогда
		Сообщить(НСтр("ru = 'Выберите тип сайта!'; ka = 'აირჩიეთ საიტის ტიპი!'; en = 'Select the type of site!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ПринимаюСоглашение Тогда
		Сообщить(НСтр("ru = 'Чтобы создать сайт, нужно принять лицензионное соглашение!'; ka = 'ვებსაიტის შესაქმნელად, თქვენ უნდა დაეთანხმოთ სალიცენზიო ხელშეკრულებას!'; en = 'To create a website, you need to accept the license agreement!'"));
		Возврат;
	КонецЕсли;
	
	Если СокрЛП(EmailРегистрацииСайта)="" Тогда
		Сообщить(НСтр("ru = 'Укажите e-mail, на который будет зарегистрирован сайт!'; ka = 'შეიყვანეთ ელექტრონული ფოსტა, რომელზეც დარეგისტრირდება საიტი!'; en = 'Enter the e-mail, which will be registered with the site!'"));
		Возврат;
	КонецЕсли;
	...
	Если НЕ ИмяСайтаСвободно Тогда
		Сообщить(НСтр("ru = 'Предложенное имя сайта занято, выберите другое!'; ka = 'შემოთავაზებული საიტის სახელი დაკავებულია, გთხოვთ აირჩიოთ სხვა!'; en = 'The proposed name of the site is occupied, choose another!'"));
		Возврат;
	КонецЕсли;
...
Процедура СоздатьСайтПродолжить()
	...
	ИначеЕсли ДанныеСайтаСтруктура.Свойство("СайтСоздан") И ДанныеСайтаСтруктура.СайтСоздан = Ложь Тогда
		...
		Если (ТекущаяДата() - СчетчикОжиданияСозданияСайта)>300 Тогда // ждем создания сайта не более 5 мин, потом выдаем ошибку
			ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
			Сообщить(НСтр("ru = 'Время ответа от сайта превышено, попробуйте повторить операцию позже'; ka = 'საიტის პასუხის დრო გადააჭარბა, გთხოვთ, სცადოთ მოგვიანებით'; en = 'The response time of the site is exceeded, try again later'"));
			Возврат;
		Иначе	
			ПодключитьОбработчикОжидания("СоздатьСайтПродолжить", 2, Истина);
		КонецЕсли;
...
Процедура ОткрытьСозданныйСайт(ДанныеСозданияСайтаСтруктура)
	...
	ПоказатьОповещениеПользователя(,,НСтр("ru = 'Создан сайт: '; ka = 'შექმნილი საიტი:'; en = 'Website created:'")+ДанныеСозданияСайтаСтруктура.АдресСайта,БиблиотекаКартинок.Информация32);
	ОбновитьИнтерфейс();
	Закрыть();
...
Функция ПолучитьДанныеЗапросом(HTTPМетод, ТелоЗапроса, АдресЗапроса, ЗаголовкиЗапроса, 
	...
	Исключение
		...
		Если СтрНайти(ТекстОшибки, "Couldn't resolve host name")<>0 Тогда
		    ТекстОшибки = НСтр("ru = 'Ошибка: Не могу подключиться к интернету!'; ka = 'შეცდომა: ვერ ვუკავშირდები ინტერნეტს!'; en = 'Error: Could not connect to the internet!'");
		КонецЕсли;
		Возврат "";
	КонецПопытки;
...
Функция ПроверитьИмяСайтаТекст(Текст)
	...
	Пока н <= СтрДлина(Текст) Цикл
		...
		Если РазрешенныеСимволыИмениСайта.Получить(НРег(ПроверСимвол)) = Неопределено Тогда
			НадписьИмяСайтаДоступно = НСтр("ru = 'Разрешены латинские буквы и цифры'; ka = 'დასაშვებია ლათინური ასოები და ციფრები'; en = 'Allowed Latin letters and numbers'");
			Возврат Ложь;
		Иначе
			н=н+1;
		КонецЕсли;
...
Функция ОбновитьНадписьИмяСайтаДоступно(ИмяСайтаПроверено, ИмяСайтаСвободно)
	...
	Если НЕ ИмяСайтаПроверено Тогда
		Возврат НСтр("ru = 'Проверяется ...'; ka = 'მიმდინარეობს შემოწმება...'; en = 'Checked ...'");
	ИначеЕсли ИмяСайтаСвободно Тогда
		Возврат НСтр("ru = 'Имя свободно!'; ka = 'სახელი თავისუფალია!'; en = 'Name is available!'");
	Иначе
		Возврат НСтр("ru = 'Имя занято!'; ka = 'სახელი დაკავებულია!'; en = 'Name Busy!'");
	КонецЕсли;
