...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("ВсеОбъекты", НСтр("ka='Все данные';ru='Все данные';en='All data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, Новый ФиксированнаяСтруктура);
	
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъектыНеутвержденные", НСтр("ka='Изменения';ru='Изменения';en='changes'"));
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъекты", НСтр("ka='Сопоставленные данные';ru='Сопоставленные данные';en='mapped data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 0));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъекты", НСтр("ka='Несопоставленные данные';ru='Несопоставленные данные';en='unmapped data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыПриемника", НСтр("ka='Несопоставленные данные этой базы';ru='Несопоставленные данные этой базы';en='Unmapped this database'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления",  1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыИсточника", НСтр("ka='Несопоставленные данные второй базы';ru='Несопоставленные данные второй базы';en='Unmapped data of the second base'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления", -1));
	...
	КонецЕсли;
	Заголовок = НСтр("ka='Сопоставление данных ""[ПредставлениеДанных]""';ru='Сопоставление данных ""[ПредставлениеДанных]""';en='Comparison of the data ""[PredstavlenieDannyh]""'");
	...
	НаименованиеЭтойПрограммы = ?(ПустаяСтрока(НаименованиеЭтойПрограммы), НСтр("ka='ეს პროგრამა';ru='Эта программа';en='This program'"), НаименованиеЭтойПрограммы);
	
	НаименованиеВторойПрограммы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.УзелИнформационнойБазы, "Наименование");
	НаименованиеВторойПрограммы = ?(ПустаяСтрока(НаименованиеВторойПрограммы), НСтр("ka='Вторая программа';ru='Вторая программа';en='The second program'"), НаименованиеВторойПрограммы);
	...
	Элементы.Пояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Для сопоставления данных ""%1"""
"с данными ""%2"" воспользуйтесь командой ""Сопоставить автоматически...""."
"Оставшиеся несопоставленные данные можно связать друг с другом вручную.';ru='Для сопоставления данных ""%1"""
"с данными ""%2"" воспользуйтесь командой ""Сопоставить автоматически...""."
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	НСтрока = НСтр("ka='Получить данные в информационную базу?';ru='Получить данные в информационную базу?';en='Get the data in the information base?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='Нет соответствия, объект будет скопирован';ru='Нет соответствия, объект будет скопирован';en='No correspondence, the object will be copied'"));
...
Процедура ВыполнитьЗагрузкуДанныхПослеВопросаОПодтвержденииПолученияДанных(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		НСтрока = НСтр("ka='Данные уже были получены. Выполнить получение данных повторно?';ru='Данные уже были получены. Выполнить получение данных повторно?';en='Data were obtained. Perform data acquisition again?'");
...
Процедура ВыполнитьЗагрузкуДанныхПослеПодтвержденияПолученияДанных()
	...
	Состояние(НСтр("ka='Выполняется получение данных. Пожалуйста, подождите...';ru='Выполняется получение данных. Пожалуйста, подождите...';en='Retrieves data. Please wait...'"));
...
"Перейти в журнал регистрации?';ka='При получении данных возникли ошибки."
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	Если ПользовательскиеПоля.Количество() = 0 Тогда
		...
		НСтрока = НСтр("ka='Следует указать хотя бы одно поле для отображения';ru='Следует указать хотя бы одно поле для отображения';en='You must specify at least one field to display'");
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ka='Уменьшите количество полей (можно выбирать не более %1 полей)';ru='Уменьшите количество полей (можно выбирать не более %1 полей)';en='Reduce the number of fields (it is possible to select not more than 1% fields)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления=-1 Или ТекущиеДанные.СтатусСопоставления=+1) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='Объекты уже сопоставлены';ru='Объекты уже сопоставлены';en='Objects already mapped'"), 2);
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,     НСтр("ka='დაყენება';ru='Установить';en='establish'"));
	...
	ТекстВопроса = НСтр("ka='Установить соответствие между выбранными объектами?';ru='Установить соответствие между выбранными объектами?';en='Establish a correspondence between the selected objects?'");
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Если Результат.Количество() = 0 Тогда
		...
		Результат.Добавить(НСтр("ka='<не задан>';ru='<не задан>';en='<Not specified>'"));
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(Отказ)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ka='Сопоставление объектов';ru='Сопоставление объектов';en='Comparison sites'");
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='Помощник сопоставления объектов.Анализ данных';ru='Помощник сопоставления объектов.Анализ данных';en='Assistant comparison obektov.Analiz data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
