...
Функция РезультатВыгрузкиВXML(СтруктураПараметров, АдресДляРазмещенияРезультата = Неопределено) Экспорт
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ТекстСообщения = НСтр("ka='В ходе выполнения операции возникли ошибки';ru='В ходе выполнения операции возникли ошибки';en='During the operation there were errors'") + ": " + Символы.ПС + КомпонентыОбмена.СтрокаСообщенияОбОшибке;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	ИначеЕсли НЕ ЕстьВыгруженныеОбъекты Тогда
		ТекстСообщения = НСтр("ka='Не найдено ни одного объекта к выгрузке.';ru='Не найдено ни одного объекта к выгрузке.';en='We could not find any object to be unloaded.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ЗагрузкаСообщения(СтруктураПараметров, АдресРезультата) Экспорт
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ошибка чтения сообщения загрузки. Неверный формат сообщения.';ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.';en='Read Error loading posts. Invalid message format.'"));
...
Процедура ЗаполнениеПравилВыгрузки() Экспорт
	...
	УзелДереваСправочники.Наименование = НСтр("ka='ცნობარები';ru='Справочники';en='References'");
	
	УзелДереваДокументы = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваДокументы.ЭтоГруппа = Истина;
	УзелДереваДокументы.Наименование = НСтр("ka='დოკუმენტები';ru='Документы';en='Documentation'");

	УзелДереваПВХ = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваПВХ.ЭтоГруппа = Истина;
	УзелДереваПВХ.Наименование = НСтр("ka='მახასიათებლების ტიპების გეგმები';ru='Планы видов характеристик';en='Plans species'");
	...
	Для Каждого СтрокаПОД Из КомпонентыОбмена.ПравилаОбработкиДанных Цикл
		...
		Если Метаданные.Справочники.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ka='Справочник %1';ru='Справочник %1';en='Reference 1%'");
		ИначеЕсли Метаданные.Документы.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ka='Документ %1';ru='Документ %1';en='Document 1%'");
		ИначеЕсли  Метаданные.ПланыВидовХарактеристик.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("ka='План видов характеристик %1';ru='План видов характеристик %1';en='Species characteristics Plan 1%'");
		Иначе
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Некорректное имя метаданных ""%1""';ru='Некорректное имя метаданных ""%1""';en='Incorrect metadata name ""%1""'"),
