...
Процедура Загрузить(Команда)
	...
	Если ЗаполненыОбязательныеРеквизиты() Тогда
		Если ЗначениеЗаполнено(Контрагент) Тогда
			ТекстВопроса = НСтр("ka='Контрагент существует. Перезаполнить реквизиты?';ru='Контрагент существует. Перезаполнить реквизиты?';en='Counterparty exists. Refill details?'");
...
Функция ЗаполненыОбязательныеРеквизиты()
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		...
		ТекстСообщения = НСтр("ka='Не заполнено <Наименование>, загрузка не возможна.';ru='Не заполнено <Наименование>, загрузка не возможна.';en='Not filled <Name>, downloading not possible.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ТекстСообщения = НСтр("ka='Проверьте правильность указания <Файла> с реквизитами контрагента.';ru='Проверьте правильность указания <Файла> с реквизитами контрагента.';en='Check that the indication <file> with the details of the counterparty.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
