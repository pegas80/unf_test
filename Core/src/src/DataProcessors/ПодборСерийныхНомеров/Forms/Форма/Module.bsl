...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Параметры.Запасы.Номенклатура.ИспользоватьСерийныеНомера Тогда
		ТекстСообщения = НСтр("ru = 'Для номенклатуры не ведется учет по серийным номерам!'; ka = 'ნომენკლატურისთვის სერიული ნომრებით აღრიცხვა არ მიმდინარეობს!'; en = 'For the nomenclature do not keep records of the serial numbers!'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("КонтрольОстатковСерийныхНомеров") Тогда
		...
		Если Параметры.Свойство("СтруктурнаяЕдиница") Тогда
			...
			Если ЗначениеЗаполнено(Параметры.СтруктурнаяЕдиница) Тогда
				Элементы.СерииОстаткиСерия.Заголовок = НСтр("ru = 'В наличии на '; ka = 'მარაგშია მდგომარეობით:'; en = 'In stock at'")+Параметры.СтруктурнаяЕдиница;
			КонецЕсли;
		Иначе
	...
	Элементы.СерийныеНомераИзменениеСтрокДействие.СписокВыбора.Добавить(НСтр("ru = 'Сгенерировать номера в порядке очереди'; ka = 'ნომრების გენერაცია რიგის მიხედვით.'; en = 'Number generation by sequence.'"));
	Элементы.СерийныеНомераИзменениеСтрокДействие.СписокВыбора.Добавить(НСтр("ru = 'Заполнить номерами из диапазона'; ka = 'შეავსეთ რიცხვებით მოცემული დიაპაზონიდან.'; en = 'Fill with numbers from the range.'"));
	...
	Если ЭтоМаркировкаТоваровГИСМ Тогда
		...
		Если ЭтоМаркировкаПерсонифицированнымиКиЗ
			...
			ТекстСообщения = НСтр("ru = 'Выбранный КиЗ предназначен для маркировки товаров с GTIN %GTINИзКиЗ%, его нельзя использовать для маркировки товара с GTIN %GTIN%.'; ka = 'არჩეული КиЗ განკუთვნილია საქონლის დასამარკად GTIN %GTINИзКиЗ%-ით, ის არ შეიძლება გამოყენებულ იქნას საქონლის დასამარკად GTIN %GTIN%-ით.'; en = 'Keyes selected for labeling with GTIN% GTINИзКиЗ%, it can not be used for the labeling of goods from the GTIN% GTIN%.'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Новый'; ka = 'ახალი'; en = 'New'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Зарегистрированный'; ka = 'რეგისტრირებული'; en = 'Registered'"));
...
Процедура ЗаполнитьСерийныеНомераПоНаличию(ДобавитьКоличество)
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [10 декабря 2022]  НАЧАЛО
	//Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫБРАТЬ ", "ВЫБРАТЬ Первые "+ДобавитьКоличество+" ");
	Если Найти(ВРЕГ(Запрос.Текст), "ВЫБРАТЬ РАЗЛИЧНЫЕ") > 0 Тогда
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫБРАТЬ РАЗЛИЧНЫЕ", "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ "+ Формат(ДобавитьКоличество, "ЧГ=0")+" ");
	...
	КонецЕсли; 	
	// © Шишка В.В. [10 декабря 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Если ДанныеШтрихкода <> Неопределено
		   И ДанныеШтрихкода.Количество() = 0 Тогда
		    //+++ Дмгг, делаем что бы окно регистра не открывалось
			...
			ЗаполнитьРСШтрихкодыНоменклатурыНаСервере(ТекШтрихкод);
		...
		ИначеЕсли НЕ (ДанныеШтрихкода.Номенклатура = Номенклатура И ДанныеШтрихкода.Характеристика = Характеристика 
			...
			СтрокаСообщения = НСтр("ru = 'Считанный штрихкод привязан к другой номенклатуре: %1% %2% %3% %4%'; ka = 'წაკითხული შტრიხკოდი მიბმულია სხვა ნომენკლატურას: %1% %2% %3% %4%'; en = 'Read the barcode attached to another nomenclature:% 1% 2% 3%%%% 4%'");
...
Процедура ПолученыШтрихкоды(ДанныеШтрикодов) Экспорт
	...
	Если ЛОЖЬ Тогда //НеизвестныеШтрихкоды.Количество() > 0 Тогда //+++ Дмгг
...
Процедура ПолученыШтрихкодыНекорректногоТипа(ШтрихкодыНекорректногоТипа) Экспорт
	...
	Для каждого ТекНекорректныйШтрихкод Из ШтрихкодыНекорректногоТипа Цикл
		...
		СтрокаСообщения = НСтр("ru = 'Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этой табличной части'; ka = 'შტრიხკოდ %1%-ით მოძებნილ ნომენკლატურას -%2%- აქვს ტიპი %3%, რომელიც ამ ცხრილის შესაფერი არ არის'; en = 'Found by barcode nomenclature% 1% -% 2% -  is type of %3%, which is not suitable for this table part'");
...
//+++ Дмгг
&НаСервере
Процедура ЗаполнитьРСШтрихкодыНоменклатурыНаСервере(ТекШтрихкод);
	
	МенеджерЗаписи = РегистрыСведений.ШтрихкодыНоменклатуры.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Номенклатура = Номенклатура;
	МенеджерЗаписи.Характеристика = Характеристика;
	МенеджерЗаписи.Партия = Партия;
	МенеджерЗаписи.Штрихкод = ТекШтрихкод;
...
Процедура НастроитьРежимГИСМ()
	...
	Если РежимГИСМ Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'К рабочему месту подключено несколько считывателей RFID. Работа одновременно с несколькими считывателями не поддерживается. Оставьте только один считываетель.'; ka = 'სამუშაო ადგილს მიერთებულია რამდენიმე RFID-სკანერი. ერთდროული მუშაობა რამდენიმე სკანერთან არ არის მხარდაჭერილი. დატოვეთ მხოლოდ ერთი სკანერი.'; en = 'Multiple RFID readers are connected to the workstation. Simultaneous operation with multiple readers is not supported. Please leave only one reader connected.'");
...
Процедура ПолучитьИнформациюПоКиЗДобавитьСерию(НомерКиЗ, ИдентификаторТекущейСтроки)
	...
	Если Не ИнтеграцияГИСМВызовСервера.ЭтоНомерКиЗ(НомерКиЗ) Тогда
		ТекстСообщения = НСтр("ka='შეყვანილი მონაცემები სსნ-ის ნომერს არ წარმოადგენს.';ru='Введенные данные не являются номером КиЗ.';en='The entered data are not numbered Keyes.'");
		НомерКиЗ = "";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Функция ПроверитьМеткиНаНеобходимостьЗаписать()
	...
	Если ЭтоМаркировкаТоваровГИСМ
		...
		Для Каждого Строка Из СтрокиСТребованиемЗаписи Цикл
			...
			Сообщить(НСтр("ru = 'Необходимо записать данные в КИЗ с номером '; ka = 'აუცილებელია მონაცემების ჩაწერა КИЗ-ში ნომრით'; en = 'It is necessary to write data to the КИЗ with the number.'")+Строка.НомерКИЗГИСМ);
