...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущийОбъект.ЭтоФайловаяБаза() Тогда
		...
		Если ТекЭлемент <> Неопределено Тогда
			ТекЭлемент.Представление = НСтр("ka='В каталоге:';ru='В каталоге:';en='In the catalog:'");
		КонецЕсли;
	КонецЕсли;
...
#Иначе
	ВызватьИсключение(НСтр("ka='В WEB-клиенте данная возможность недоступна.';ru='В WEB-клиенте данная возможность недоступна.';en='In the WEB-client, this option is not available.'"));
#КонецЕсли
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Фильтр = НСтр("ka='Внешние обработки (*.epf)|*.epf';ru='Внешние обработки (*.epf)|*.epf';en='External processing (* .epf) | * .epf'");
