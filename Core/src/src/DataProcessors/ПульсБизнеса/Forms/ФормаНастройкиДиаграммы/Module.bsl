...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИдентификаторСтроки") Тогда
		...
		Заголовок = НСтр("ka='დიაგრამის აწყობა';ru='Настройка диаграммы';en='Customize charts'");
	Иначе
		ИдентификаторСтроки = -1;
		Заголовок = НСтр("ka='დიაგრამის დამატება';ru='Добавление диаграммы';en='Adding charts'");
	КонецЕсли; 
	...
	Если ЗначениеЗаполнено(ОписаниеСерий.ОбязательныеФильтры) Тогда
		...
		Для каждого ОписаниеФильтра Из ОписаниеСерий.ОбязательныеФильтры Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЗначениеФильтра) Тогда
				Состояние(НСтр("ru = 'Не задан обязательный фильтр: '; ka = 'არ არის მითითებული აუცილებელი ფილტრი: '")+?(ДоступноеПоле=Неопределено, ОписаниеФильтра.Ключ, ДоступноеПоле.Заголовок));
				Отказ = Истина;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(Диаграмма) Тогда
		...
		Если ЗначениеЗаполнено(Серия) Тогда
			...
			Если НастройкиСерии.ТипДиаграммы=ТипДиаграммы.Круговая 
				...
				Элементы.Серия.Заголовок = НСтр("ka='მაჩვენებელი';ru='Показатель';en='Index'");
				Элементы.Точка.Заголовок = НСтр("ka='მნიშვნელობა';ru='Значения';en='meaning'");
				Элементы.ПериодСравненияПредставление.Видимость = Ложь;
			ИначеЕсли НастройкиСерии.ТипДиаграммы=ТипДиаграммы.ГистограммаСНакоплением 
				...
				Элементы.Серия.Заголовок = НСтр("ka='ვერტიკალურად';ru='Вертикально';en='upright'");
				Элементы.Точка.Заголовок = НСтр("ka='ჰორიზონტალურად';ru='Горизонтально';en='Horizontally'");
				Элементы.ПериодСравненияПредставление.Видимость = Ложь;
			Иначе
				Элементы.Серия.Заголовок = НСтр("ka='ვერტიკალურად';ru='Вертикально';en='upright'");
				Элементы.Точка.Заголовок = НСтр("ka='ჰორიზონტალურად';ru='Горизонтально';en='Horizontally'");
...
Процедура УстановитьСвостваПолейПериодов()
	...
	Если РежимОстатка Тогда
		...
		КонецЕсли; 
		Элементы.ПериодПредставление.Заголовок = НСтр("ka='თარიღისთვის';ru='На дату';en='On the date'");
	...
	Иначе
		...
		КонецЕсли; 
		Элементы.ПериодПредставление.Заголовок = НСтр("ka='პერიოდი';ru='Период';en='Period'");
...
Процедура ВыборПериодаНачало(Элемент)
	...
	Если РежимОстатка Тогда
		Если Элемент=Элементы.ПериодПредставление Тогда
			...
			ДанныеВыбора.Добавить(Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала.ПроизвольнаяДата), НСтр("ka='თავისუფალი თარიღი';ru='Произвольная дата';en='arbitrary date'"));
			СтруктураПараметров.Вставить("Значение", Период);
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			// Дата сравнения
			ДанныеВыбора.Добавить(Неопределено, НСтр("ka='არ შედარდეს';ru='Не сравнивать';en='not compare'"));
	...
	Иначе
		Если Элемент=Элементы.ПериодПредставление Тогда
			...
			ДанныеВыбора.Добавить(Новый Структура("Вариант", "Последние7ДнейНеСчитаяТекущего"), НСтр("ka='ბოლო 7 დღე, მიმდინარეს გაუთვალისწინებლად';ru='Последние 7 дней, не считая текущего';en='The last 7 days, not including the current'"));
			ДанныеВыбора.Добавить(Новый СтандартныйПериод(ВариантСтандартногоПериода.ПроизвольныйПериод), НСтр("ka='თავისუფალი თარიღი';ru='Произвольный период';en='any period'"));
			СтруктураПараметров.Вставить("Значение", Период);
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			// Период сравнения
			ДанныеВыбора.Добавить(Неопределено, НСтр("ka='არ შედარდეს';ru='Не сравнивать';en='not compare'"));
...
Процедура ВыборПериодаЗавершение(ВыбранноеЗначение, СтруктураПараметров) Экспорт
	...
	Если РежимОстатка Тогда
		Если НЕ ЗначениеЗаполнено(ПериодРасчета) И НЕ ПериодРасчета=Неопределено Тогда
			...
			ПоказатьВводДаты(Оповещение, ТекущееЗначение.Дата, НСтр("ka='მიუთითეთ თარიღი';ru='Укажите дату';en='select dates'"), ЧастиДаты.Дата); 
...
Функция ПредставлениеСтандартногоПериода(Период)
	...
	Если НЕ ЗначениеЗаполнено(Период) Тогда
		Возврат НСтр("ka='არ არის შერჩეული';ru='Не выбран';en='not selected'");
	...
	ИначеЕсли Период.Вариант="ПредыдущийПлавающийПериод" Тогда
		Возврат НСтр("ka='გასული პერიოდი (';ru='Предыдущий период (';en='Previous period ('")+ПредставлениеПериода(НачалоДня(Период.Период.ДатаНачала), КонецДня(Период.Период.ДатаОкончания))+")";
	ИначеЕсли Период.Вариант="ЗаПрошлыйГод" Тогда
		Возврат НСтр("ka='გასული წლისთვის (';ru='За прошлый год (';en='For the last year ('")+ПредставлениеПериода(НачалоДня(Период.Период.ДатаНачала), КонецДня(Период.Период.ДатаОкончания))+")";
	ИначеЕсли Период.Вариант="Последние7ДнейНеСчитаяТекущего" Тогда
		Возврат НСтр("ka='ბოლო 7 დღე, მიმდინარეს გაუთვალისწინებლად';ru='Последние 7 дней, не считая текущего';en='The last 7 days, not including the current'");
	Иначе
...
Функция ПредставлениеСтандартнойДатыНачала(Период)
	...
	ИначеЕсли Период.Вариант="ТакойЖеДеньНаПрошлойНеделе" Тогда
		Возврат НСтр("ka='გასული კვირის იგივე დღე (';ru='Такой же день на прошлой неделе (';en='The same day last week ('")+Формат(Период.Дата, "ДФ='dd.MM.yyyy, ддд'")+")";
	ИначеЕсли Период.Вариант="ТакойЖеДеньВПрошломМесяце" Тогда
		Возврат НСтр("ka='გასული თვის იგივე დღე (';ru='Такой же день прошлого месяца (';en='The same day last month ('")+Формат(Период.Дата, "ДФ='dd.MM.yyyy, ддд'")+")";
	ИначеЕсли Период.Вариант="ТакойЖеДеньВПрошломГоду" Тогда
		Возврат НСтр("ka='გასული წლის იგივე დღე (';ru='Такой же день в прошлом году (';en='The same day last year ('")+Формат(Период.Дата, "ДФ='dd.MM.yyyy'")+")";
	Иначе
		Возврат Строка(Период);
	КонецЕсли; 
