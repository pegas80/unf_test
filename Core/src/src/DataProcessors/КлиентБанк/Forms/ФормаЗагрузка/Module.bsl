...
Функция ПроверитьЗаполнениеРеквизитовФормы()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Кодировка) Тогда
		ТекстСообщения = НСтр("ka='В настройках не указана кодировка (кнопка ""Еще"" - ""Настройка"")!';ru='В настройках не указана кодировка (кнопка ""Еще"" - ""Настройка"")!';en='In the setting of the encoding is not specified (button ""More"" - ""Settings"")!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВерсияФормата) Тогда
		ТекстСообщения = НСтр("ka='В настройках не указана версия формата обмена (кнопка ""Еще"" - ""Настройка"")!';ru='В настройках не указана версия формата обмена (кнопка ""Еще"" - ""Настройка"")!';en='interchange format version is not configured ( ""More"" button - ""Setup"")!'");
...
Функция ПолучитьДатуИзСтроки(Приемник, Источник)
	...
	Если ПозицияТочки = 0 Тогда
		Возврат НСтр("ka='თარიღიანი სტრიქონის არასწორი ფორმატი';ru='Неверный формат строки с датой';en='Invalid string format with the date'");
	КонецЕсли;
	...
	Исключение
		Возврат НСтр("ka='Не удалось преобразовать строку в дату';ru='Не удалось преобразовать строку в дату';en='Could not convert a string to date'");
	КонецПопытки;
...
Функция НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора = Неопределено, СписокВидовДоговора = Неопределено)
	...
	СтруктураВозврат = Новый Структура("Договор, КоличествоДоговоров", НСтр("ka='Не найден договор по умолчанию';ru='Не найден договор по умолчанию';en='The default contract is not found'"), 0);
	...
	Пока Лев(Врег(СокрЛП(СтрокаРазбора)), 14) <> "КОНЕЦДОКУМЕНТА" Цикл
		...
		Если РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Тогда
			...
			Иначе
				...
				ТекстСообщения = НСтр("ka='Неверный реквизит платежного документа, строка %Импорт%: %СтрокаРазбора%';ru='Неверный реквизит платежного документа, строка %Импорт%: %СтрокаРазбора%';en='Invalid props payment document at line% Import%:% StrokaRazbora%'"
				);
		...
		Иначе
			...
			// Неправиьный реквизит заголовка.
			ТекстСообщения = НСтр("ka='Нарушена структура платежного документа, строка %Импорт%: %СтрокаРазбора%';ru='Нарушена структура платежного документа, строка %Импорт%: %СтрокаРазбора%';en='Disrupted structure payment document import line%%:% StrokaRazbora%'"
			);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтрокаРазбора%", СтрокаРазбора);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения);
...
Функция ЗагрузитьСекциюРасчСчета(СтрокаРССчета, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ТегиРасчетногоСчета)
	...
	Пока РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Цикл
		...
		Иначе
			// Неправиьный реквизит заголовка.
			ТекстСообщения = НСтр("ka='Неверный реквизит в секции описания расчетного счета, строка %Импорт%: %СтрокаРазбора%';ru='Неверный реквизит в секции описания расчетного счета, строка %Импорт%: %СтрокаРазбора%';en='Invalid props in the current account of a section line% Import%:% StrokaRazbora%'"
			);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтрокаРазбора%", СтрокаРазбора);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения);
...
Процедура ПроверитьНаПустоеЗначениеИмпорта(СтрокаИмпорта, ИмяСвойства, ПредставлениеСвойства, ИмпортНеПустые)
	...
	Если ИмпортНеПустые[0][ИмяСвойства] = Истина Тогда
		Если НЕ ЗначениеЗаполнено(СтрокаИмпорта[ИмяСвойства]) Тогда
			СтрокаЗамечание = НСтр("ka='Не заполнено ""%ИмяСвойства%""!';ru='Не заполнено ""%ИмяСвойства%""!';en='Not filled ""PropertyName%%!""'");
...
Функция ЗагрузитьСтрокуЗаголовка(ТекстСтрокиЗаголовка, ТегиЗаголовка, ИмпортЗаголовок, ИмпортТекущаяСтрока)
	...
	Иначе
		...
		// Неправиьный реквизит заголовка.
		ТекстСообщения = НСтр("ka='Неверный реквизит заголовка, строка %Импорт%: %ТекстСтрокиЗаголовка%';ru='Неверный реквизит заголовка, строка %Импорт%: %ТекстСтрокиЗаголовка%';en='Invalid header props, line% Import%:% TekstStrokiZagolovka%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
		...
		Если Не НашлиЗаказ И Не НашлиСчет Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Если ВыборкаЗаказовИСчетов.Количество() = 1 Тогда
					...
					//СтрНайти(СтрокаДокумента.НазначениеПлатежа, НСтр("ka='შეკვეთა';ru='Заказ';en='Order'")) = 0 И
					//СтрНайти(СтрокаДокумента.НазначениеПлатежа, НСтр("ka='ანგარიში';ru='Счет';en='Score'")) = 0 Тогда
					Если (СтрНайти(СтрокаДокумента.НазначениеПлатежа, "№"+НомерДляПоиска) > 0 ИЛИ
					СтрНайти(СтрокаДокумента.НазначениеПлатежа, " "+НомерДляПоиска) > 0) Тогда
...
Функция ПроверитьВидОперации(СтрокаДокумента, ПараметрыВыделенияКолонок, СтруктураЗначенийИзСоответствия)
	...
	ЕстьКлючевоеСлово = СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='СЧЕТ';ru='СЧЕТ';en='SCORE'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='ЗАКАЗ';ru='ЗАКАЗ';en='ORDER'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='СЧЁТ';ru='СЧЁТ';en='SCORE'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='НАКЛ';ru='НАКЛ';en='Knuckle'")) > 0;
		
	ЕстьСловаОРасчетномОбслуживании = СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='Р/О';ru='Р/О';en='P / O'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчетное обслуживание';ru='Расчетное обслуживание';en='settlement services'"))) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчетные услуги';ru='Расчетные услуги';en='accounting services'"))) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчётные услуги';ru='Расчётные услуги';en='Settlement services'"))) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='Р\О';ru='Р\О';en='P \ O'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ka='Р\У';ru='Р\У';en='RU'")) > 0 ИЛИ
		СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ka='Расчётное обслуживание';ru='Расчётное обслуживание';en='Estimated service'"))) > 0;
		
	ЕслиСловоВозврат = СтрНайти(СтрокаДокумента.НазначениеПлатежа, НСтр("ka='დაბრუნება';ru='Возврат';en='Return'")) > 0;
	...
	Если Не ЕстьКлючевоеСлово Или ЕстьСловаОРасчетномОбслуживании ИЛИ ЕслиСловоВозврат Тогда
		...
		СтрокаЗамечание = НСтр("ka='Проверьте операцию!';ru='Проверьте операцию!';en='Check the operation!'");
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, ПараметрыВыделенияКолонок, ПослеЗагрузкиВыписокВ1С = Ложь)
	...
	Если ПустаяСтрока(Буфер) Тогда
		...
		СтрокаЗамечание = НСтр("ka='Сумма не указана!';ru='Сумма не указана!';en='The amount is not specified!'");
	...
	Иначе
		СтрокаДокумента.НеВерныйФорматСуммыКартинка = 0;
		СтрокаЗамечание = НСтр("ka='Укажите сумму вручную (%Буфер%)!';ru='Укажите сумму вручную (%Буфер%)!';en='Specify the amount of hand (% Buffer%)!'");
	...
	Если ВсеРеквизитыПоискаЕсть Тогда
			...
			Если ВыборкаНомер = НомерДляПоискаДок
				...
				Если КоличествоДок = 0 Тогда
					...
					Если ДокументПодходит Тогда
						...
						Иначе
							СтрокаДокумента.ДокументНайден = ?(СтрокаДокумента.Исходящий, НСтр("ru='Найден ';ka='Найден '"), НСтр("ru='Найдено ';ka='Найдено '"))+ВыборкаДокументов.Ссылка;
						КонецЕсли;
						...
						Если СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета
							...
							//СтрокаДокумента.Контрагент = НСтр("ka='Наша организация (';ru='Наша организация (';en='Our organization ('") + ВыборкаДокументов.Организация + ")";
		...
		Если КоличествоДок > 1 Тогда
			...
			СтрокаЗамечание = НСтр("ka='В информационной базе найдено несколько (%КоличествоДок%) соответствующих документов!';ru='В информационной базе найдено несколько (%КоличествоДок%) соответствующих документов!';en='The information database found several (% KolichestvoDok%) of the relevant documents!'");
			СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%КоличествоДок%", КоличествоДок);
			
			Если Не ПослеЗагрузкиВыписокВ1С Тогда
				СтрокаДокумента.ДокументНайден = СтрокаДокумента.ДокументНайден + НСтр("ka='. И еще %КоличествоДок% док.';ru='. И еще %КоличествоДок% док.';en='. And KolichestvoDok%% doc.'");
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) Тогда
		...
		Если ВыборкаПоиска.Следующий() Тогда
			...
			Если НЕ ПустаяСтрока(ИННКонтрагента) И ВыборкаПоиска.ПриоритетИНН = 2 И ВыборкаПоиска.ПриоритетНаименования = 2 Тогда
				...
				СтрокаЗамечание = НСтр("ka='Банковский счет найден у контрагента с другим ИНН (%ВладелецИНН%)!';ru='Банковский счет найден у контрагента с другим ИНН (%ВладелецИНН%)!';en='Bank account found at the counterparty with a different INN (% VladeletsINN%)!'");
				...
				Иначе
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%ВладелецИНН%", НСтр("ka='<Не заполнен>';ru='<Не заполнен>';en='<Do not filled>'"));
				КонецЕсли;
				ДобавитьЗамечание(СтрокаДокумента, 1, СтрокаЗамечание);
			...
			Иначе
				...
				Если (ВыборкаПоиска.ПриоритетБИК = 2) Тогда
					...
					СтрокаЗамечание = НСтр("ka='БИК банка счета контрагента в базе (%БИК_из_Базы%) не совпадает с БИК банка в выписке (%БИК_в_Выписке%).';ru='БИК банка счета контрагента в базе (%БИК_из_Базы%) не совпадает с БИК банка в выписке (%БИК_в_Выписке%).';en='BIC bank account of the counterparty in the database (% BIK_iz_Bazy%) does not coincide with the BIC bank statement (% BIK_v_Vypiske%).'");
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%БИК_из_Базы%", ?(ЗначениеЗаполнено(ВыборкаПоиска.БИК_из_Базы), ВыборкаПоиска.БИК_из_Базы, НСтр("ka='<пустой>';ru='<пустой>';en='<Empty>'")));
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%БИК_в_Выписке%", ?(ЗначениеЗаполнено(БИКСчета), БИКСчета, НСтр("ka='<пустой>';ru='<пустой>';en='<Empty>'")));
					ДобавитьЗамечание(СтрокаДокумента, 4, СтрокаЗамечание);
				КонецЕсли;
		...
		Если КонтрагентНеНайден Тогда
			СтрокаЗамечание = НСтр("ka='Не найден счет контрагента (%СчетКонтрагента%)!';ru='Не найден счет контрагента (%СчетКонтрагента%)!';en='Is not found by the counterparty (% SchetKontragenta%)!'");
			...
			СтрокаСчетКонтрагента = НСтр("ka='Не найден (%СчетКонтрагента%).';ru='Не найден (%СчетКонтрагента%).';en='Not found (% SchetKontragenta%).'");
		...
		Если КоличествоЭлементовВВыборке > 1 //И 
			...
			Тогда
			СтрокаЗамечание = НСтр("ka='В информационной базе найдено несколько (%Количество%) одинаковых банковских счетов! Проверьте контрагента и р/счет!';ru='В информационной базе найдено несколько (%Количество%) одинаковых банковских счетов! Проверьте контрагента и р/счет!';en='The information database found several (% number%) of the same bank accounts! Check the contractor and p / account!'");
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Тогда
		...
		ИначеЕсли НЕ ПустаяСтрока(ИННКонтрагента) Тогда
			...
			Если ВыборкаПоиска.Следующий() Тогда
				...
				Если ПустаяСтрока(ВыборкаПоиска.ИНН) Тогда
					СтрокаЗамечание = НСтр("ka='У контрагента не заполнен ИНН!';ru='У контрагента не заполнен ИНН!';en='The counterparty is not filled INN!'");
					ДобавитьЗамечание(СтрокаДокумента, 2, СтрокаЗамечание);
			...
			Если ВыборкаПоиска.Количество() > 1 Тогда
				СтрокаЗамечание = НСтр("ka='В информационной базе найдено несколько (%Количество%) контрагентов с одинаковым ИНН!';ru='В информационной базе найдено несколько (%Количество%) контрагентов с одинаковым ИНН!';en='The information database found several (% Amount%) counterparts with the same INN!'");
			...
			ИначеЕсли ВыборкаПоиска.Количество() = 0 Тогда
				СтрокаЗамечание = НСтр("ka='Не найден контрагент (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!';ru='Не найден контрагент (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!';en='Unable to find a counterparty (ImyaKontragenta%%% VAT INNKontragenta%)!'");
				...
				СтрокаКонтрагент = НСтр("ka='Не найден (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!';ru='Не найден (%ИмяКонтрагента%, ИНН %ИННКонтрагента%)!';en='Not found (% ImyaKontragenta%, VAT% INNKontragenta%)!'");
		...
		Иначе
			ДобавитьЗамечание(СтрокаДокумента, 2, НСтр("ru='Не указан ИНН контрагента! ';ka='Не указан ИНН контрагента! '"), Истина);
			СтрокаКонтрагент = НСтр("ka='Не найден (%ИмяКонтрагента%, не указан ИНН)!';ru='Не найден (%ИмяКонтрагента%, не указан ИНН)!';en='Not found (% ImyaKontragenta% Unknown TIN)!'");
			СтрокаКонтрагент = СтрЗаменить(СтрокаКонтрагент, "%ИмяКонтрагента%", ИмяКонтрагента);
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.Контрагент) И СтрокаДокумента.КонтрагентНеНайденКартинка = -1 Тогда
			Если ИННКонтрагентаВБазе <> ИННКонтрагента
				...
				Если ИННКонтрагентаВБазе <> ИННКонтрагента Тогда
					СтрокаПроверки = НСтр("ru='ИНН, ';ka='ИНН, '");
					КолвоРеквизитовДляПроверки = КолвоРеквизитовДляПроверки + 1;
				КонецЕсли;
				Если КППКонтрагентаВБазе <> КППКонтрагента Тогда
					СтрокаПроверки = СтрокаПроверки + НСтр("ru='КПП, ';ka='КПП, '");
					КолвоРеквизитовДляПроверки = КолвоРеквизитовДляПроверки + 1;
				КонецЕсли;
				Если ВРег(СокрЛП(НаименованиеПолноеКонтрагентаВБазе)) <> ВРег(СокрЛП(ИмяКонтрагента)) Тогда
					СтрокаПроверки = СтрокаПроверки + НСтр("ru='юридическое название, ';ka='юридическое название, '");
					КолвоРеквизитовДляПроверки = КолвоРеквизитовДляПроверки + 1;
				КонецЕсли;
				СтрокаПроверки = Лев(СтрокаПроверки, СтрДлина(СтрокаПроверки) - 2);
				
				Если КолвоРеквизитовДляПроверки = 2 Тогда
					СтрокаПроверки = СтрЗаменить(СтрокаПроверки, ", ", НСтр("ru=' и ';ka=' и '"));
				КонецЕсли;
				
				СтрокаДокумента.КонтрагентНеНайденКартинка = 0;
				ПараметрыВыделенияКолонок.ВыделитьКолонкуКонтрагент = Истина;
				Если КолвоРеквизитовДляПроверки = 1 Тогда
					СтрокаЗамечание = НСтр("ka='Проверьте %СтрокаПроверки% контрагента!';ru='Проверьте %СтрокаПроверки% контрагента!';en='Check% StrokaProverki% counterpart!'");
				Иначе
					СтрокаЗамечание = НСтр("ka='Проверьте реквизиты контрагента: %СтрокаПроверки%!';ru='Проверьте реквизиты контрагента: %СтрокаПроверки%!';en='Check the details of the counterparty:% StrokaProverki%!'");
				КонецЕсли;
	...
	Иначе
		...
		СтрокаДокумента.ОписаниеОшибок = УдалитьДанныеИзОписанияОшибкиНаСервере(НСтр("ka='Не найден счет контрагента';ru='Не найден счет контрагента';en='Is not found by the counterparty'"), СтрокаДокумента.ОписаниеОшибок, СтрокаДокумента);
	КонецЕсли;
	...
	И НЕ ЗначениеЗаполнено(СтрокаДокумента.Договор) Тогда
		...
		Если СтрНайти(СтрокаДокумента.Договор, НСтр("ka='Не найден';ru='Не найден';en='Not found'")) > 0 Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") И
				...
				Если ТипЗнч(СтрокаДокумента.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
					СтрокаЗамечания = НСтр("ru='Организация в договоре отличается от организации счета! ';ka='Организация в договоре отличается от организации счета! '");
		...
		Если СтрНайти(СтрокаДокумента.Договор, НСтр("ka='Не найден';ru='Не найден';en='Not found'")) > 0 И ТипЗнч(СтрокаДокумента.Контрагент) <> Тип("Строка") Тогда
			ДобавитьЗамечание(СтрокаДокумента, 2, НСтр("ru='Не найден договор контрагента! ';ka='Не найден договор контрагента! '"));
			ПараметрыВыделенияКолонок.ВыделитьКолонкуДоговор = Истина;
			СтрокаДокумента.ДоговорНеНайденКартинка = 0;
		КонецЕсли;
		Если СтруктураДанныхПоискаДоговора.КоличествоДоговоров > 1 Тогда
			СтрокаЗамечания = НСтр("ru='У контрагента несколько договоров (%КоличествоДоговоров%)! ';ka='У контрагента несколько договоров (%КоличествоДоговоров%)! '");
	...
	Если //ТипЗнч(СтрокаДокумента.СчетКонтрагента) = Тип("СправочникСсылка.БанковскиеСчета")
		...
		И СтрНайти(""+СтрокаДокумента.Контрагент, НСтр("ka='Наша организация (';ru='Наша организация (';en='Our organization ('")) = 0 Тогда
		//СтрокаДокумента.Контрагент = НСтр("ka='Наша организация (';ru='Наша организация (';en='Our organization ('") + СтрокаДокумента.Контрагент + ")";
	КонецЕсли;
	...
	Пока ИмпортТекущаяСтрока <= ИмпортКоличествоСтрок Цикл
		...
		Если Лев(ВРег(СокрЛП(Стр)), 14) = "СЕКЦИЯДОКУМЕНТ" Тогда
			...
			Если Тег = "СЕКЦИЯДОКУМЕНТ" Тогда
				...
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ka='Нарушена структура файла импорта, строка %Импорт%: %Стр%';ru='Нарушена структура файла импорта, строка %Импорт%: %Стр%';en='Broken the import file structure, line% Import%:%% Page'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
		...
		ИначеЕсли Лев(Врег(СокрЛП(Стр)), 14) = "СЕКЦИЯРАСЧСЧЕТ" Тогда
			...
			Если НЕ ЗагрузитьСекциюРасчСчета(СтрокаРССчетов, ИмпортТекущаяСтрока, ИмпортКоличествоСтрок, ИмпортТекстДляРазбора, ТегиРасчетногоСчета) Тогда
				ТекстСообщения = НСтр("ka='Нарушена структура файла импорта в секции описания расчетного счета! Строка: %Импорт%';ru='Нарушена структура файла импорта в секции описания расчетного счета! Строка: %Импорт%';en='Broken the import file structure in the section describing the current account! Line:% Import%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Импорт%", (ИмпортТекущаяСтрока - 1));
				Возврат ТекстСообщения;
			КонецЕсли;
...
"Для проверки можете использовать гиперссылку ""Посмотреть содержимое файла"" в правом нижнем углу.';ka='В заголовке файла не указан банковский счет. Есть строка ""РасчСчет="", но после символа ""="" нет номера счета или строки ""РасчСчет="" нет совсем."
"Будет использован банковский счет из первой секции ""СЕКЦИЯРАСЧСЧЕТ""!!!"
"Для проверки можете использовать гиперссылку ""Посмотреть содержимое файла"" в правом нижнем углу.'");
					...
					ИначеЕсли ЕстьОтборПоБанковскомуСчету Тогда
						...
						//ТекстСообщения = НСтр("ka='В заголовке файла не указан банковский счет. В файле есть строка ""РасчСчет="", но после символа ""="" нет номера счета.';ru='В заголовке файла не указан банковский счет. В файле есть строка ""РасчСчет="", но после символа ""="" нет номера счета.';en='The header file is specified bank account. In the file there is a line ""RaschSchet ="", but after the ""="" no account number.'");
						//Возврат ТекстСообщения;
				...
				Если Выборка.Следующий() Тогда
					...
					Если НЕ ЕстьОтборПоБанковскомуСчету И ЗначениеЗаполнено(Объект.БанковскийСчет)
					   ...
					   Если Объект.БанковскийСчет.НомерСчета = Значение Тогда
							ТекстСообщения = НСтр("ka='В базе есть несколько банковский счетов организаций с одинаковым номером!';ru='В базе есть несколько банковский счетов организаций с одинаковым номером!';en='The database has several bank accounts of organizations with the same number!'");
						Иначе
							ТекстСообщения = НСтр("ka='В заголовке файла указан счет (%Значение%) отличный от указанного!';ru='В заголовке файла указан счет (%Значение%) отличный от указанного!';en='In the title specified by the file (% Value%) than the specified!'");
						КонецЕсли;
					...
					КонецЕсли;
				Иначе
					ТекстСообщения = НСтр("ka='В заголовке файла указан счет, не принадлежащий организации: %Значение%!';ru='В заголовке файла указан счет, не принадлежащий организации: %Значение%!';en='In the title specified by the file does not belong to the organization:% Value%!'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Значение%", Значение);
					Возврат ТекстСообщения;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли Лев(Врег(СокрЛП(Стр)), 10) = "КОНЕЦФАЙЛА" Тогда
			...
			Если НЕ ИмпортПризнакОбмена Тогда
				ТекстСообщения = НСтр("ka='В файле импорта отсутствует признак обмена ""1CClientBankExchange""!';ru='В файле импорта отсутствует признак обмена ""1CClientBankExchange""!';en='The import file is not a sign of sharing ""1CClientBankExchange""!'");
				Возврат ТекстСообщения;
			КонецЕсли;
	...
	Если ЕстьОтборПоБанковскомуСчету И НЕ НашлиБанковскийСчет Тогда
		...
		ТекстСообщения = НСтр("ka='В заголовке файла не найден указанный банковский счет (%Значение%)!';ru='В заголовке файла не найден указанный банковский счет (%Значение%)!';en='The title specified bank account file is not found (%% value)!'");
	...
	Если НЕ НайденКонецФайла Тогда
		...
		ДокументыКИмпорту.Очистить();
		ТаблицаТаблицДокументов.Очистить();
		ТекстСообщения = НСтр("ka='Файл загрузки не соответствует стандарту (не найдена секция КонецФайла)!';ru='Файл загрузки не соответствует стандарту (не найдена секция КонецФайла)!';en='Download the file does not meet the standard (not found KonetsFayla section)!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтаФорма, ТекстСообщения);
	КонецЕсли;
	...
	Для каждого СтрокаДокумента Из ДокументыКИмпорту Цикл
		...
		Если СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ДатаПоступило) Тогда
				...
				СтрокаЗамечание = НСтр("ka='Это данные по другому счету: %1%!';ru='Это данные по другому счету: %1%!';en='This data on another account:% 1%!'");
		...
		Если СтрокаДокумента.Документ = Неопределено Тогда
			СтрокаДокумента.ДокументНеЗагружен = ?(СтрокаДокумента.Загружать, НСтр("ka='Будет загружен';ru='Будет загружен';en='It is loaded'"), НСтр("ka='Не будет загружен';ru='Не будет загружен';en='Not be loaded'"))
...
Функция ПрочитатьЭлектроннуюВыпискуБанкаНаСервере(ЭлектроннаяВыписка, СтекОповещений)
	...
	Исключение
		ТекстСообщения = НСтр("ka='Файл не прочитан.';ru='Файл не прочитан.';en='The file is not read.'");
		СтекОповещений.Добавить(Новый Структура("Текст", ТекстСообщения));
		Возврат Неопределено;
	КонецПопытки;
	
	Если ПотокЧтения.КоличествоСтрок() < 1 Тогда
		ТекстСообщения = НСтр("ka='В файле нет данных!';ru='В файле нет данных!';en='Your file!'");
		СтекОповещений.Добавить(Новый Структура("Текст", ТекстСообщения));
		Возврат Неопределено;
	КонецЕсли;
	
	Если СокрЛП(ПотокЧтения.ПолучитьСтроку(1)) <> "1CClientBankExchange" Тогда
		ТекстСообщения = НСтр("ka='Указанный файл не является файлом обмена или неверно указана кодировка!';ru='Указанный файл не является файлом обмена или неверно указана кодировка!';en='The specified file is not a file sharing or incorrect encoding is specified!'");
		СтекОповещений.Добавить(Новый Структура("Текст", ТекстСообщения));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПрочитатьЭлектронныеВыпискиБанка()
	...
	Если ИмпортТекстДляРазбора = Неопределено Тогда
		ТекстСообщения = НСтр("ka='Файл загрузки не содержит данных!';ru='Файл загрузки не содержит данных!';en='The download file contains no data!'");
	...
	Если ЗначениеЗаполнено(СоглашениеПрямогоОбменаСБанками) Тогда
		...
		Если ВыпискиБанка.Количество() = 0 Тогда
			УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма,
				Нстр("ka='Для получения электронной выписки банка нажмите кнопку ""Запросить выписку""';ru='Для получения электронной выписки банка нажмите кнопку ""Запросить выписку""';en='For electronic bank statement, click ""Request an extract""'")
...
Функция ПериодЗаполнен()
	...
	Если НЕ ЗначениеЗаполнено(Объект.НачПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Нстр("ka='Не заполнена дата начала периода';ru='Не заполнена дата начала периода';en='the date of commencement of the period is not filled'")
			,, "Объект.НачПериода");
		ПериодЗаполнен = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Нстр("ka='Не заполнена дата окончания периода';ru='Не заполнена дата окончания периода';en='end date of the period is not filled'")
			,, "Объект.КонПериода");
		ПериодЗаполнен = Ложь;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресФайлаКОбработке") Тогда
		...
		Если ТипЗнч(ФайлЗагрузки) = Тип("ДвоичныеДанные") Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ka='Обмен с банком.Временный файл';ru='Обмен с банком.Временный файл';en='Exchange with bankom.Vremenny file'"), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Не удалось сохранение временного файла на диск по причине: %1';ru='Не удалось сохранение временного файла на диск по причине: %1';en='Unable to save temporary file on disk for a reason:% 1'"),
						ОписаниеОшибки()));
	...
	Если Параметры.Свойство("СоглашениеПрямогоОбменаСБанками") Тогда
		...
		Если ЗначениеЗаполнено(СоглашениеПрямогоОбменаСБанками) Тогда
			ТекстШаблона = НСтр("ka='С %1 действует соглашение о прямом обмене: банковская выписка будет загружена в 1С:Управление нашей фирмой напрямую из банка';ru='С %1 действует соглашение о прямом обмене: банковская выписка будет загружена в 1С:Управление нашей фирмой напрямую из банка';en='From% 1 an agreement on direct exchange: bank statement will be loaded in the 1C: our company directly from the bank'");
...
Процедура ЗагрузкаОбновитьВыполнить(Команда)
	...
	Состояние(
		НСтр("ka='მიმდინარეობს ამონაწერის ფაილის წაკითხვა...';ru='Выполняется чтение файла выписки...';en='Reads an extract file ...'"),
		,
		НСтр("ka='გთხოვთ, დაელოდეთ';ru='Пожалуйста, подождите';en='Please wait'"),
	...
	Если Объект.Загрузка.Количество() = 0 Тогда
		//ПоказатьПредупреждение(Неопределено, НСтр("ka='В файле нет данных о платежах или их не удалось прочитать';ru='В файле нет данных о платежах или их не удалось прочитать';en='The file does not contain data on payments or could not read'"));
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтаФорма,
				НСтр("ka='В файле нет данных о платежах или их не удалось прочитать';ru='В файле нет данных о платежах или их не удалось прочитать';en='The file does not contain data on payments or could not read'")
				,, "ЭлектроннаяВыпискаБанка");
	КонецЕсли;
...
Процедура ЗагрузитьВыполнить(Команда)
	...
	Если Объект.Загрузка.Количество() > 0 Тогда
		...
		Если Не ЕстьОтмеченныеСтроки Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Не отмечено ни одной строки! Выписки не будут загружены. Нужно установить отметку в загружаемых строках и нажать кнопку еще раз.';ru='Не отмечено ни одной строки! Выписки не будут загружены. Нужно установить отметку в загружаемых строках и нажать кнопку еще раз.';en='There was no single line! Statements will not be loaded. It is necessary to place a check mark in the loaded rows and press the button again.'");
		...
		Состояние(
			НСтр("ka='Выполняется загрузка выписки из файла в 1С...';ru='Выполняется загрузка выписки из файла в 1С...';en='Loading statements from the file in 1C ...'"),
			,
			НСтр("ka='გთხოვთ, დაელოდეთ';ru='Пожалуйста, подождите';en='Please wait'"),
			БиблиотекаКартинок.ЗагрузкаДанных32
		);
		...
		#Если ВебКлиент Тогда
			...
			ПраваяЧастьПредупреждения = НСтр("ru='Обработано строк: ';ka='Обработано строк: '")+ПараметрыЗавершения.ОбработаноСтрок+НСтр("ru='. Всего строк: ';ka='. Всего строк: '")+ПараметрыЗавершения.ВсегоСтрок+".";
	...
	Иначе
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Список документов для загрузки пуст.';ru='Список документов для загрузки пуст.';en='List of documents to download empty.'"));
	...
	Если ТаблицаПомеченныхНаУдалениеКакДубль.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПротокола Из ТаблицаПомеченныхНаУдалениеКакДубль Цикл
			...
			Если Не СтрокаПротокола.ОперацияВыполнена Тогда
				Область.Параметры.Комментарий = НСтр("ka='Не получилось отменить проведение!';ru='Не получилось отменить проведение!';en='It did not happen to cancel the!'");
			КонецЕсли;
...
Функция ТекстСсылкиНаЭлектронныеВыписки(НачалоПериода, КонецПериода)
	...
	ШаблонСтроки = НСтр("ka='Выписка банка за период с %1 по %2';ru='Выписка банка за период с %1 по %2';en='Extract the bank for the period from% 1 to% 2'");
...
Функция ПодготовитьОднуСтрокуТекстаПредупреждения(ОписаниеОшибок, ПоказыватьЗнакПредупреждения, ДанныеСтроки, ТолькоИнформацияОДублях = Неопределено)
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Не найден контрагент';ru='Не найден контрагент';en='Unable to find a counterparty'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + 
			?(ДанныеСтроки.ДействиеСКонтрагентом.Пустая(), НСтр("ka=' Будет создан новый при загрузке в 1С.';ru=' Будет создан новый при загрузке в 1С.'"), " "+ДанныеСтроки.ДействиеСКонтрагентом+".");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Не найден счет';ru='Не найден счет';en='Not found by'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + 
			?(ДанныеСтроки.ДействиеСоСчетомКонтрагента.Пустая(), НСтр("ka=' Будет создан новый при загрузке в 1С.';ru=' Будет создан новый при загрузке в 1С.'"), " "+ДанныеСтроки.ДействиеСоСчетомКонтрагента+".");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Не найден договор';ru='Не найден договор';en='Unable to find an agreement'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ?(ДанныеСтроки.ДействиеСоСчетомКонтрагента.Пустая(), 
			НСтр("ka='Не найден основной договор. Будет создан новый при загрузке в 1С и установлен основным.';ru='Не найден основной договор. Будет создан новый при загрузке в 1С и установлен основным.';en='Not found the main contract. new boot will be created in the 1C and set basic.'"), 
			НСтр("ru='Не найден договор контрагента. ';ka='Не найден договор контрагента. '")+ДанныеСтроки.ДействиеСоСчетомКонтрагента+".");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Проверьте операцию';ru='Проверьте операцию';en='Check operation'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka=' В назначении не найдены данные о счете, накладной или заказе.';ru=' В назначении не найдены данные о счете, накладной или заказе.'");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Укажите сумму вручную';ru='Укажите сумму вручную';en='Specify the amount of manual'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka=' Сумма в файле содержит недопустимые символы.';ru=' Сумма в файле содержит недопустимые символы.'");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='Сумма не указана';ru='Сумма не указана';en='Amount specified'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka=' В файле из банка не указана сумма операции.';ru=' В файле из банка не указана сумма операции.'");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='соответствующих документов';ru='соответствующих документов';en='the relevant documents'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka=' Дубли документов будут помечены на удаление!!!';ru=' Дубли документов будут помечены на удаление!!!'");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='контрагентов с одинаковым ИНН!';ru='контрагентов с одинаковым ИНН!';en='contractors with the same INN!'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka=' Проверьте контрагента!';ru=' Проверьте контрагента!'");
	...
	ТекС = СтрНайти(ОписаниеОшибок, НСтр("ka='одинаковых банковских счетов!';ru='одинаковых банковских счетов!';en='the same bank accounts!'"));
	Если ТекС > 0 Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka=' Проверьте контрагента и его банковский счет!';ru=' Проверьте контрагента и его банковский счет!'");
...
Функция ПодготовитьТекстПредупреждения(ДанныеСтроки, ПоказыватьЗнакПредупреждения = Ложь)
	...
	ПодготовленноеОписаниеОшибок = НСтр("ru='В текущей строке: ';ka='В текущей строке: '")+ Символы.ПС + Символы.ПС;
	
	Если НЕ ДанныеСтроки.Загружать Тогда
		ПодготовленноеОписаниеОшибок = ПодготовленноеОписаниеОшибок + НСтр("ka='<Флаг загрузки снят. Строка не будет загружена>';ru='<Флаг загрузки снят. Строка не будет загружена>';en='<Flag boot removed. The string will not be loaded>'") + Символы.ПС + Символы.ПС;
	КонецЕсли;
...
Процедура ЗагрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(Элементы.Загрузка.ТекущиеДанные.Документ)
		...
		Если ДанныеСтроки.НесколькоДокументовКартинка = 0 ИЛИ СтрНайти(ДанныеСтроки.ДокументНайден, НСтр("ka='И еще';ru='И еще';en='And further'")) > 0 Тогда
			СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли Поле.Имя = "ЗагрузкаНеВерныйФорматСуммыКартинка" Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Сумма) Тогда
			ПоказатьВводЧисла(Новый ОписаниеОповещения("ПоказатьВводСуммыДокументаЗавершение", ЭтотОбъект, ТекущиеДанные.ПолучитьИдентификатор()),
				...
				НСтр("ka='Введите нужное число!';ru='Введите нужное число!';en='Enter the number!'"), 
				СтрЗаменить(НСтр("ka='Введите нужное число (%1)!';ru='Введите нужное число (%1)!';en='Enter the desired number (1%)!'"), "%1", СокрЛП(ТекущиеДанные.Сумма)))
				, 15, 2);
		Иначе
	...
	ИначеЕсли (Поле.Имя = "ЗагрузкаСуммаДокумента"
		...
		Если НеВерныйФорматСуммы Тогда
			ПоказатьВводЧисла(Новый ОписаниеОповещения("ПоказатьВводСуммыДокументаЗавершение", ЭтотОбъект, ДанныеСтроки.ПолучитьИдентификатор()),
				...
				НСтр("ka='Введите нужное число!';ru='Введите нужное число!';en='Enter the number!'"), 
				СтрЗаменить(НСтр("ka='Введите нужное число (%1)!';ru='Введите нужное число (%1)!';en='Enter the desired number (1%)!'"), "%1", СокрЛП(ДанныеСтроки.Сумма)))
				, 15, 2);
		Иначе
...
Функция ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, СтрокаСНазваниемДокумента)
	...
	Если ТипЗнч(ТекущаяСтрокаТабличнойЧасти.Контрагент) = Тип("Строка") Тогда
		...
		ТекстСообщения = НСтр("ka='Контрагент не идентифицирован, выбор %1 не возможен.';ru='Контрагент не идентифицирован, выбор %1 не возможен.';en='The counterparty is not identified, the choice of 1% is not possible.'");
...
Функция ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, СтрокаСНазваниемДокумента, ИмяРеквизита)
	...
	Если Не ПолучитьРеквизитКонтрагента(ТекущаяСтрокаТабличнойЧасти.Контрагент, ИмяРеквизита) Тогда
		...
		ТекстСообщения = НСтр("ka='С контрагент не ведется учет расчетов по %1. Выбор документа не требуется.';ru='С контрагент не ведется учет расчетов по %1. Выбор документа не требуется.';en='On the counterparty fails to keep records of settlements under 1%. Selecting the document is not required.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", СтрокаСНазваниемДокумента);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ЗагрузкаЗаказНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='заказа';ru='заказа';en='order'"));
	СтандартнаяОбработка = ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='заказам';ru='заказам';en='orders'"), "ВестиРасчетыПоЗаказам");
...
Процедура ЗагрузкаСчетНаОплатуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущаяСтрокаТабличнойЧасти = Элементы.Загрузка.ТекущиеДанные;
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='счета на оплату';ru='счета на оплату';en='invoice'"));
	СтандартнаяОбработка = ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='счетам на оплату';ru='счетам на оплату';en='invoice for payment'"), "ВестиУчетОплатыПоСчетам");
...
Процедура ЗагрузкаДокументРасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущаяСтрокаТабличнойЧасти = Элементы.Загрузка.ТекущиеДанные;
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='документа расчетов';ru='документа расчетов';en='settlement document'"));
	СтандартнаяОбработка = ПроверитьНастройкиРасчетов(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='документам расчетов';ru='документам расчетов';en='settlement documents'"), "ВестиРасчетыПоДокументам");
...
Процедура ЗагрузкаПлатежноеПоручениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущаяСтрокаТабличнойЧасти = Элементы.Загрузка.ТекущиеДанные;
	СтандартнаяОбработка = ПроверитьВозможностьВыбораДокумента(ТекущаяСтрокаТабличнойЧасти, НСтр("ka='платежного поручения';ru='платежного поручения';en='payment order'"));
...
Процедура ЗаполнитьСуммыВыделеныхНаКлиенте(ТекСтрока)
	...
	ОстатокКон = ОстатокНач + Объект.Загрузка.Итог("СуммаПоступилоВыделено") - Объект.Загрузка.Итог("СуммаСписаноВыделено");
	ТекСтрока.ДокументНеЗагружен = ?(ТекСтрока.Загружать, НСтр("ka='Будет загружен';ru='Будет загружен';en='It is loaded'"), НСтр("ka='Не будет загружен';ru='Не будет загружен';en='Not be loaded'"))
...
Функция ПолучитьТекстДокументГотовКЗагрузкеНаКлиенте(ДанныеСтроки, ПодробноеСообщение = Ложь)
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.Документ) Тогда
		ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%1", НСтр("ka=' (был загружен ранее)';ru=' (был загружен ранее)'"));
	Иначе
...
//		ДанныеСтроки.ОписаниеОшибок = СтрЗаменить(НСтр("ka='Проверьте операцию!';ru='Проверьте операцию!';en='Check the operation!'"), "", "");
...
Процедура ОповещениеРедактированиеКонтрагента(Результат, Параметры) Экспорт
	...
	Если (ТипЗнч(Результат.Контрагент) = Тип("СправочникСсылка.Контрагенты")) И ЗначениеЗаполнено(Результат.Контрагент) Тогда
		...
		ИначеЕсли Результат.ДействиеСконтрагентом = ПредопределенноеЗначение("Перечисление.ДействияСКонтрагентомПриЗагрузкеИзКлиентБанка.НеСоздаватьНовогоКонтрагента") Тогда
			ДанныеСтроки.Контрагент = ДанныеСтроки.КонтрагентИсходный;
		КонецЕсли;
		//ДанныеСтроки.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден контрагент';ru='Не найден контрагент';en='Unable to find a counterparty'"), ДанныеСтроки.ОписаниеОшибокПодТЧ, ДанныеСтроки);
				...
				Если ДанныеСтроки <> Строка И ИНН = ?(Строка.Исходящий, Строка.ПолучательИНН, Строка.ПлательщикИНН) И
					...
					Если ЗначениеЗаполнено(Результат.Договор) Тогда
						...
						Если ТипЗнч(Строка.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
							//Строка.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден договор';ru='Не найден договор';en='Unable to find an agreement'"), Строка.ОписаниеОшибокПодТЧ, Строка);
						КонецЕсли;
					КонецЕсли;
					...
					Если ЗначениеЗаполнено(Результат.БанковскийСчетПоУмолчанию) Тогда
						...
						//Строка.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден счет контрагента';ru='Не найден счет контрагента';en='Is not found by the counterparty'"), Строка.ОписаниеОшибокПодТЧ, Строка);
					КонецЕсли;
...
Процедура ОткрытьФормуРедактированияКонтрагентов(ДанныеСтроки, ИмяЗакладкиНаФорме)
	...
	КонецЕсли;
	СтруктураПараметров.Вставить("ЕстьНесколькоДокументов", ?(СтрНайти(ДанныеСтроки.ДокументНайден, НСтр("ka='И еще';ru='И еще';en='And further'")) > 0 ИЛИ ДанныеСтроки.НесколькоДокументовКартинка = 0, Истина, Ложь));
...
Процедура ОткрытьФормуРедактированияСчетаКорреспонденции(ДанныеСтроки)
	...
	Если ДанныеСтроки.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.Прочее") И
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Корреспонденцию нужно устанавливать только для операции ""Прочее"".';ru='Корреспонденцию нужно устанавливать только для операции ""Прочее"".';en=""All correspondence should be installed only for the 'Other'."""));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузкаСчетКонтрагентаПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТипЗнч(ТекущиеДанные.СчетКонтрагента) = Тип("СправочникСсылка.БанковскиеСчета") И ЗначениеЗаполнено(ТекущиеДанные.СчетКонтрагента) Тогда
			...
			ТекущиеДанные.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Не найден счет контрагента';ru='Не найден счет контрагента';en='Is not found by the counterparty'"), ТекущиеДанные.ОписаниеОшибокПодТЧ, ТекущиеДанные);
...
Процедура УбратьПредупреждающийЗнакВТекущейКолонке(Команда)
	...
	Исключение
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Выберите в таблице ячейку с предупреждающим знаком';ru='Выберите в таблице ячейку с предупреждающим знаком';en='Select the table cell with a warning sign'"), 30);
		Возврат;
	КонецПопытки;
	...
	ИначеЕсли ИмяКолонки = "ЗагрузкаКонтрагентНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаКонтрагент" Тогда
		...
		//УдалятьПредупреждение = НСтр("ka='Не найден контрагент';ru='Не найден контрагент';en='Unable to find a counterparty'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаВидОперацииНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаВидОперацииПоступление" ИЛИ 
		...
		УдалятьПредупреждение = НСтр("ka='Проверьте операцию';ru='Проверьте операцию';en='Check operation'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаДоговорНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаДоговор" Тогда
		...
		//УдалятьПредупреждение = НСтр("ka='Не найден договор';ru='Не найден договор';en='Unable to find an agreement'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаСчетКонтрагентаНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаСчетКонтрагента" Тогда
		...
		//УдалятьПредупреждение = НСтр("ka='Не найден счет';ru='Не найден счет';en='Not found by'");
	ИначеЕсли ИмяКолонки = "ЗагрузкаНеВерныйФорматСуммыКартинка" Тогда
		ИмяКолонки = "НеВерныйФорматСуммыКартинка";
		//УдалятьПредупреждение = НСтр("ka='Укажите сумму вручную';ru='Укажите сумму вручную';en='Specify the amount of manual'");
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Действие доступно для колонок с предупреждающим знаком';ru='Действие доступно для колонок с предупреждающим знаком';en='Action is available for columns with a warning sign'"), 30);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидОперации(ДанныеСтроки, ВыбранноеЗначение)
	...
	ДанныеСтроки.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ka='Проверьте операцию';ru='Проверьте операцию';en='Check operation'"), ДанныеСтроки.ОписаниеОшибокПодТЧ, ДанныеСтроки);
	ПоменятьСимволПСНаПробел(ДанныеСтроки.ОписаниеОшибокПодТЧ);
	...
	Если ДанныеСтроки.ВидОперации = ВидОперацииПереводНаДругойСчет ИЛИ ДанныеСтроки.ВидОперации = ВидОперацииПереводСДругогоСчета Тогда
		...
		//ДанныеСтроки.Контрагент = НСтр("ka='Наша организация (';ru='Наша организация (';en='Our organization ('") + Объект.Организация + ")";
