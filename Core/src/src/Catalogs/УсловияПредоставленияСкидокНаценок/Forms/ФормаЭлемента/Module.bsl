...
Функция СформироватьАвтоНаименованиеНаКлиенте()
	...
	Если Объект.УсловиеПредоставления = ПредопределенноеЗначение("Перечисление.УсловияПредоставленияСкидокНаценок.ЗаРазовыйОбъемПродаж") Тогда
		СтрокаНаименования = ""+?(Объект.КритерийОграниченияПримененияЗаОбъемПродаж = ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество"), НСтр("ka='რ-ბა';ru='Кол-во';en='Count'"), Объект.КритерийОграниченияПримененияЗаОбъемПродаж) + " " + 
		?(Объект.ОбластьОграничения = ПредопределенноеЗначение("Перечисление.ВариантыОбластейОграниченияСкидокНаценок.ВДокументе"),НСтр("ka='დოკუმენტში';ru='в документе';en='in the document'"),НСтр("ka='სტრიქონში';ru='в строке';en='in line'")) + " "+Объект.ТипСравнения + " "+Объект.ЗначениеУсловияОграничения + 
		?(Объект.КритерийОграниченияПримененияЗаОбъемПродаж = ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество"), НСтр("ka=' ც.';ru=' ед.'"), " "+Объект.ВалютаОграничения);
	...
	ИначеЕсли Объект.УсловиеПредоставления = ПредопределенноеЗначение("Перечисление.УсловияПредоставленияСкидокНаценок.ЗаКомплектПокупки") Тогда
		СтрокаНаименования = НСтр("ka='კომპლექტი:';ru='Комплект:';en='Contents:'");
...
Функция СформироватьАвтоНаименованиеНаСервере()
	...
	Если Объект.УсловиеПредоставления = Перечисления.УсловияПредоставленияСкидокНаценок.ЗаРазовыйОбъемПродаж Тогда
		...
		?(Объект.ОбластьОграничения = Перечисления.ВариантыОбластейОграниченияСкидокНаценок.ВДокументе,НСтр("ka='დოკუმენტში';ru='в документе';en='in the document'"),НСтр("ka='სტრიქონში';ru='в строке';en='in line'")) + " "+Объект.ТипСравнения + " "+Объект.ЗначениеУсловияОграничения + 
		?(Объект.КритерийОграниченияПримененияЗаОбъемПродаж = Перечисления.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество, НСтр("ka=' ед.';ru=' ед.'"), " "+Объект.ВалютаОграничения);
		Если Объект.ОтборПродажПоНоменклатуре.Количество() > 0 Тогда
...
Процедура ЗаполнитьПоДопРеквизитамХарактеристики(Команда)
	...
	ОткрытьФорму("Обработка.Ценообразование.Форма.ФормаЗаполненияПоДопРеквизитамХарактеристики", , ЭтотОбъект, , , , ОбработкаОписание);
...
Процедура ОбработатьРезультатЗаполненияНаСервере(Результат, ДополнительныеПараметры)
	...
	Если ЗначениеЗаполнено(Результат.АдресВременногоХранилища) Тогда
		...
		Для Каждого СтрокаХ ИЗ ТаблицаХарактеристик Цикл
			НайденныеСтрокиХ = Объект.ОтборПродажПоНоменклатуре.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаХ.Номенклатура, СтрокаХ.Характеристика));
