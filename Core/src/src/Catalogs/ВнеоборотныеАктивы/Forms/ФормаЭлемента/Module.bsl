...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьЭлементаСгенерироватьШтрихкод();// ++ ONESOFT-4150 
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// ONESOFT-4150	
	ИначеЕсли ИмяСобытия = "ШтрихкодыВА" И  Параметр = Объект.Ссылка Тогда
...
Процедура СгенерироватьШтрихкодНаСервере()
	...
	Объект.Штрихкод = РегистрыСведений.ШтрихкодыВА.СформироватьШтрихкод();
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи) 
	...
	Если Константы.ИспользованиеШтрихкодовВВнеоборотныхАктивах.Получить() Тогда 
		ИмеющийсяШтрихкод = "";
		РегистрыСведений.ШтрихкодыВА.ПроверитьКоличествоШтрихкодов(ТекущийОбъект.Ссылка, Объект.Штрихкод, ИмеющийсяШтрихкод, Отказ); 
		РегистрыСведений.ШтрихкодыВА.ПроверитьУникалностьШтрихкода(ТекущийОбъект.Ссылка, Объект.Штрихкод, ИмеющийсяШтрихкод, Отказ);
		
		Если Не Отказ И ЗначениеЗаполнено(Объект.Штрихкод) Тогда 
			МенеджерЗаписи = РегистрыСведений.ШтрихкодыВА.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.ВнеоборотныйАктив = ТекущийОбъект.Ссылка;
			МенеджерЗаписи.Штрихкод = Объект.Штрихкод;	
			МенеджерЗаписи.Записать();  
		Иначе 
			Объект.Штрихкод = ИмеющийсяШтрихкод;	
		КонецЕсли;  
	КонецЕсли;
...
&НаСервере
Процедура ВидимостьЭлементаСгенерироватьШтрихкод(); 
	 Элементы.ГруппаШтрихкод.Видимость = Константы.ИспользованиеШтрихкодовВВнеоборотныхАктивах.Получить();
КонецПроцедуры
//--ONESOFT-4150  
