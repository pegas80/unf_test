...
Процедура ПередЗаписью(Отказ)
	...
	ХранилищеСтруктура = Параметры.Получить();
	Если ХранилищеСтруктура.Свойство("P_OpenCashDrawerAfterDocument") и не ХранилищеСтруктура.Свойство("P_OpenCashDrawerAfterDocumentCashOnly") ТОгда
		ХранилищеСтруктура.Вставить("P_OpenCashDrawerAfterDocumentCashOnly", Ложь);
		Параметры = Новый ХранилищеЗначения(ХранилищеСтруктура);
	КонецЕсли;	
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ПустаяСтрока(Наименование) Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მითითებულია ელემენტის არაუნიკალური სახელწოდება. მიუთითეთ უნიკალური სახელწოდება.';ru='Указано неуникальное наименование элемента. Укажите уникальное наименование.';en='Specified non-unique name of the element. Specify a unique name.'"), ЭтотОбъект, , , Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.WebСервисОборудование Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторWebСервисОборудования) Тогда
			...
			Если Не Запрос.Выполнить().Пустой() Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მითითებულია ელემენტის აქტიური მოწყობილობის არაუნიკალური იდენტიფიკატორი. მიუთითეთ უნიკალური იდენტიფიკატორი.';ru='Указан неуникальный идентификатор активного оборудования элемента. Укажите уникальный идентификатор.';en='Set a non-unique identifier of the active element of equipment. Specify a unique identifier.'"), ЭтотОбъект, , , Отказ);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Наименование = НСтр("ka='%Наименование% (ასლი)';ru='%Наименование% (копия)';en='% Name% (copy)'");
