...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Объект.Наименование = НСтр("ka='Патент';ru='Патент';en='Patent'") + ?(ПустаяСтрока(Объект.НомерПатента),""," №"+ Объект.НомерПатента)+
		 НСтр("ka=', მოქმედების ვადა:';ru=', срок действия:';en=', validity:'") + Формат(Объект.ДатаНачала,"ДЛФ=D") + " - " + Формат(Объект.ДатаОкончания,"ДЛФ=D");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если НЕ ЗначениеЗаполнено(Объект.НомерПатента) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ka='პატენტის ნომერი';ru='Номер патента';en='patent number'"));
	...
	Если СуммаНалога > 0  И НЕ ЗначениеЗаполнено(Объект.КБК) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ka='КБК';ru='КБК';en='CSC'"));
	...
	Если Объект.СуммаВторогоПлатежа > 0 И Объект.СуммаПервогоПлатежа > 0
		...
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "КОРРЕКТНОСТЬ", НСтр("ka='პირველი გადახდის თარიღი';ru='Дата первого платежа';en='First payment date'"));
...
Процедура УстановитьПредставлениеОплаты(Форма);
	...
	Если Форма.ОдинПлатеж Тогда
		...
		Элементы.СуммаПервогоПлатежа.Заголовок  	= Нстр("ka='გადასახადის ოდენობა';ru='Сумма налога';en='The amount of tax'");
	Иначе
		...
		Элементы.СуммаПервогоПлатежа.Заголовок  	= Нстр("ka='გადასახადის 1/3';ru='1/3 налога';en='1/3 tax'");
	КонецЕсли;
...
Процедура СоздатьЗадачиПоОплате() Экспорт
	...
	ОбъектПерваяЗадача.ПредставлениеВКалендаре 			= ?(ОдинПлатеж, НСтр("ru='Уплатить налог с патента ';ka='Уплатить налог с патента '"), НСтр("ru='Уплатить 1/3 налога с патента ';ka='Уплатить 1/3 налога с патента '")) + ?(ПустаяСтрока(Объект.НомерПатента),"","№"+ Объект.НомерПатента)+ " (" + Формат(Объект.ДатаНачала,"ДЛФ=D") + "-" + Формат(Объект.ДатаПервогоПлатежа,"ДЛФ=D")+ ")";
	...
	Иначе
		...
		ОбъектВтораяЗадача.ПредставлениеВКалендаре 			= НСтр("ru='Уплатить 2/3 налога с патента ';ka='Уплатить 2/3 налога с патента '")+ ?(ПустаяСтрока(Объект.НомерПатента),"","№"+ Объект.НомерПатента)+ " (" + Формат(Объект.ДатаНачала,"ДЛФ=D") + "-" + Формат(Объект.ДатаВторогоПлатежа,"ДЛФ=D")+ ")";
