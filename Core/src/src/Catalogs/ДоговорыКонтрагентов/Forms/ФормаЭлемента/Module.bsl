...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Объект.ДопустимыйЛимитЗадолженностиОтПокупателя = Константы.КонтрольДебиторскойЗадолженности.Получить();
		...
		Объект.КатегорияПолучателейДоходов = "26"; //+++ Дмгг
		
		УстановитьТипВладелец(); //+++ Дмгг
		
	КонецЕсли;
	//Paganini_{
    Элементы.ДопустимыйЛимитЗадолженностиОтПокупателя.ТолькоПросмотр = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(, "ЗапретИзмененияЛимитаВДоговореКонтрагента");
	//}_Paganini
	...
	Если ПравоДоступа("Чтение",Метаданные.Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция) Тогда	
		//Сообщить(ПравоДоступа("Чтение",Метаданные.Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция));
		ИспользоватьДистрибуцию = Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция.Получить();
		Элементы.Дистрибуция.Видимость = ИспользоватьДистрибуцию;
		Если ИспользоватьДистрибуцию Тогда
			Элементы.Дистрибуция.Видимость = Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция.Получить();
		...
		КонецЕсли;
	Иначе 
		Элементы.Дистрибуция.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.ПокупательПоставщикПрочие.Видимость = Константы.ИспользоватьВидыДоговоров.Получить();// Kuzmich, 12.08.2019, Использовать виды договоров
...
Процедура ЗаписатьГрафикПосещенияСервер(ТекущийОбъект)
	...
	Для Каждого СтрокаТЧ Из ГрафикПосещения Цикл
		...
		ЗначениеПеречисления = СтрокаТЧ.ДеньНедели;
		ИндексЗначения = Перечисления.ДниНедели.Индекс(СтрокаТЧ.ДеньНедели);
		ИмаПеречисления = Метаданные.Перечисления.ДниНедели.ЗначенияПеречисления[ИндексЗначения].Имя;
		ТекущийОбъект[ИмаПеречисления] = СтрокаТЧ.Посещение;
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьДоступностьНДФЛ();//_Паганини
	
	//УстановитьТипВладелец(); //+++ Дмгг Перенесено в Создание на сервере
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоДоговорОбслуживания И НЕ Объект.ПометкаУдаления Тогда
		...
		Пока Выборка.Следующий() Цикл
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='ხელშეკრულების მოქმედების დაწყების თარიღისთვის (%3) ფასების სახეობა ""%2""-ის მიხედვით ნომენკლატურა ""%1""-ისთვის ფასი არ არის დაყენებული!';ru='Для номенклатуры ""%1"" не установлена цена по виду цен ""%2"" на дату начала действия договора (%3)!';en='For the nomenclature of ""%1"" does not set the price of sight prices ""%2"" at the inception of the contract (3%)!'"),
	...
	Если ПравоДоступа("Чтение",Метаданные.Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция) Тогда	
		Если ПравоДоступа("Чтение",Метаданные.Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция) Тогда	
			Если Константы.ОПТИМУС_ФункциональнаяОпцияДистрибуция.Получить() Тогда
				ЗаписатьГрафикПосещенияСервер(ТекущийОбъект);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если Константы.ОбязательноеПолеТорговыйПредставитель.Получить() Тогда 
		 Элементы.ТорговыйПредставитель.АвтоОтметкаНезаполненного = Истина;
       Если Не ЗначениеЗаполнено(Объект.ТорговыйПредставитель) тогда
			 Сообщить("სავაჭრო წარმომადგენლის ველი არ არის შევსებული");
			 Отказ = Истина;
	   КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьТипВладелец()
	если Объект.Покупатель = ложь и Объект.Поставщик = ложь и Объект.ПрочиеОтношения = ложь тогда
		Объект.Покупатель=Объект.Владелец.Покупатель;
		Объект.Поставщик=Объект.Владелец.Поставщик;
		Объект.ПрочиеОтношения=Объект.Владелец.ПрочиеОтношения;
	конецесли;
...
Процедура ПроверитьКорректностьДатыОкончанияДоговора()
	...
	Если Объект.ДоговорОбслуживанияДатаОкончания < Объект.ДоговорОбслуживанияДатаНачала Тогда
		...
		Сообщение.Текст = НСтр("ka='Дата окончания действия договора обслуживания не может быть раньше даты начала.';ru='Дата окончания действия договора обслуживания не может быть раньше даты начала.';en='End date of service of the contract can not be before start date.'");
		Сообщение.Сообщить();
		Возврат;
...
Функция СформироватьНаименование(НомерДоговора, ДатаДоговора, ВалютаРасчетов)
	...
	ТекстНаименования = НСтр("ka='%ДатаДоговора%№ %НомерДоговора% (%ВалютаРасчетов%)';ru='№ %НомерДоговора% %ДатаДоговора% (%ВалютаРасчетов%)';en='NomerDogovora №%%%% DataDogovora (% ValyutaRaschetov%)'");
	ТекстНаименования = СтрЗаменить(ТекстНаименования, "%НомерДоговора%", СокрЛП(НомерДоговора));
	ТекстНаименования = СтрЗаменить(ТекстНаименования, "%ДатаДоговора%", ?(ЗначениеЗаполнено(ДатаДоговора), СокрЛП(Строка(Формат(ДатаДоговора, "ДФ=dd.MM.yyyy"))) + НСТР("ru = ' от'; ka = '-ის '"), ""));
...
Процедура УправлениеФормой()
	...
	Элементы.ВидОплатыУслуг.Видимость = Объект.УслугаИП; // Kuzmich, 18.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
...
Процедура УстановитьЗаголовокЦенообразование(Форма)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦен) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Вид цен ';ka='ფასის ტიპი  '") + Строка(Объект.ВидЦен));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ВидСкидкиНаценки) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Вид скидки, наценки ';ka='ფასდაკლების, ფასნამატის ტიპი '") + Строка(Объект.ВидСкидкиНаценки));
	КонецЕсли;
...
Процедура УстановитьЗаголовокБиллинг(Форма)
	...
	Если Объект.ЭтоДоговорОбслуживания И ЗначениеЗаполнено(Объект.ДоговорОбслуживанияТарифныйПлан) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Тарифный план ';ka='Тарифный план '") + Строка(Объект.ДоговорОбслуживанияТарифныйПлан));
	КонецЕсли;
...
Процедура УстановитьЗаголовокОбменДанными(Форма)
	...
	Если ЗначениеЗаполнено(Объект.ВидДоговора) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='договор ';ka='договор '") + Строка(Объект.ВидДоговора));
	КонецЕсли;
...
Процедура ПроверитьВозможностьИзменений(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ka='ბაზაში არის ეკვაირინგის ტერმინალები, რომლებშიც არჩეულია მიმდინარე ხელშეკრულება. ანგარიშსწორებების ვალუტის შეცვლა აკრძალულია';ru='В базе присутствуют эквайринговые терминалы, в которых выбран текущий договор. Изменение валюты расчетов запрещено.';en='The database contains acquiring terminals, in which the current contract selected. Change Currency calculations prohibited.'");
...
Процедура СоздатьНаправлениеДеятельностиВопрос()
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ka='მოქმედების მიმართულების შექმნამდე აუცილებელია ობიექტის ჩაწერა. ჩავწეროთ?';ru='Перед созданием направления действия необходимо записать объект. Записать?';en='Before creating the direction of the object to be recorded. Record?'");
...
//Паганини_
&НаКлиенте
Процедура УчитыватьНДФЛПриИзменении(Элемент)
	УстановитьДоступностьНДФЛ(Истина);
КонецПроцедуры

&НаСервере
Процедура УстановитьДоступностьНДФЛ(ПриИзминении = Ложь)
	Элементы.ВидУдержанияНДФЛ.ТолькоПросмотр = НЕ Объект.УчитыватьНДФЛ;
	Элементы.ВидУдержанияНДФЛ.АвтоОтметкаНезаполненного	= Объект.УчитыватьНДФЛ И НЕ ЗначениеЗаполнено(Объект.ВидУдержанияНДФЛ);
	Элементы.ВидУдержанияНДФЛ.ОтметкаНезаполненного		= Объект.УчитыватьНДФЛ И НЕ ЗначениеЗаполнено(Объект.ВидУдержанияНДФЛ);
	Элементы.ВидВыдачиНДФЛ.АвтоОтметкаНезаполненного	= Объект.УчитыватьНДФЛ И НЕ ЗначениеЗаполнено(Объект.ВидУдержанияНДФЛ);
	Элементы.ВидВыдачиНДФЛ.ОтметкаНезаполненного		= Объект.УчитыватьНДФЛ И НЕ ЗначениеЗаполнено(Объект.ВидУдержанияНДФЛ);
	Элементы.КатегорияПолучателейДоходов.АвтоОтметкаНезаполненного	= Объект.УчитыватьНДФЛ И НЕ ЗначениеЗаполнено(Объект.ВидУдержанияНДФЛ);
	Элементы.КатегорияПолучателейДоходов.ОтметкаНезаполненного		= Объект.УчитыватьНДФЛ И НЕ ЗначениеЗаполнено(Объект.ВидУдержанияНДФЛ);
	Элементы.КатегорияПолучателейДоходов.Видимость = Объект.УчитыватьНДФЛ; //+++ Дмгг
	Элементы.ВидВыдачиНДФЛ.Видимость = Объект.УчитыватьНДФЛ; //+++ Дмгг
	Если ПриИзминении Тогда
		Если Объект.УчитыватьНДФЛ Тогда
			Объект.ВидВыдачиНДФЛ				= "7";
			//Объект.КатегорияПолучателейДоходов	= "1.2.1";
			Объект.КатегорияПолучателейДоходов = "26"; //+++ Дмгг
		Иначе
			Объект.ВидУдержанияНДФЛ = Неопределено;
			Объект.ВидВыдачиНДФЛ				= "";
			Объект.КатегорияПолучателейДоходов	= "";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
// Kuzmich, 18.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
#Область ВидОплатыУслуг

&НаКлиенте
Процедура УслугаИППриИзменении(Элемент)
	
	УправлениеФормой();
	
	Если Объект.УслугаИП Тогда
	
		Если НЕ ЗначениеЗаполнено(Объект.ВидОплатыУслуг) Тогда
		
			Объект.ВидОплатыУслуг = ПредопределенноеЗначение("Справочник.ВидыОплатУслуг.ОплатаДругихУслуг")
		
		КонецЕсли;
		
	Иначе
		
		Объект.ВидОплатыУслуг = НЕОПРЕДЕЛЕНО;
