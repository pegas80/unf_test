...
Процедура ПроверитьДублиДисконтныхКарт(Форма)
	...
	Если КоличествоЭлементовДублей > 0 Тогда
		...
		Если КоличествоЭлементовДублей = 1 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("КоличествоЭлементовДублей", НСтр("ka='ერთი';ru='одна';en='one'"));
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ka='რუკა/ბარათი';ru='карта';en='map'"));
		...
		ИначеЕсли КоличествоЭлементовДублей < 5 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ka='ბართები';ru='карты';en='cards'"));
			
		Иначе
			
			СтруктураПараметровСообщенияОДублях.Вставить("КоличествоЭлементовДублей", КоличествоЭлементовДублей);
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ka='карт';ru='карт';en='kart'"));
			
		КонецЕсли;	
		
		ТекстНадписиОДублях = НСтр("ka='ასეთ კოდით არის [КоличествоЭлементовДублей] [Склонение]';ru='С таким кодом есть [КоличествоЭлементовДублей] [Склонение]';en='With such a code has [KolichestvoElementovDubley] [Declination]'");
...
//+++ Дмгг
&НаСервере
Процедура УстановитьВидимостьПериодаДействия()

	ВидимостьПериодаДействия =  ЗначениеЗаполнено(Объект.Владелец); 
	
	Элементы.ГруппаПериодДействия.Видимость = ВидимостьПериодаДействия;
...
Процедура ПриОткрытии(Отказ)
	...
	//+++ Дмгг
	УстановитьВидимостьПериодаДействия();
	//--- Дмгг
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		//+++ Дмгг
		Если ЗначениеЗаполнено(Объект.Владелец) И Объект.Владелец.КартаЛояльности Тогда 
...
Процедура ВидДисконтнойКартыПриИзменении(Элемент)
	...
	//+++ Дмгг
	УстановитьВидимостьПериодаДействия();
	ЭтоКартаЛояльности = ПолучитьОтметкуКартыЛояльностиНаСетвере();
	Если ЭтоКартаЛояльности Тогда 
		Если НЕ ЗначениеЗаполнено(Объект.ДатаНачала) Тогда 
			Объект.ДатаНачала = ТекущаяДата();
			Объект.ДатаОкончания = ДобавитьМесяц(Объект.ДатаНачала, 12);
		КонецЕсли;
...
//+++ Дмгг
&НаСервере
Функция  ПолучитьОтметкуКартыЛояльностиНаСетвере()

	Возврат ЗначениеЗаполнено(Объект.Владелец);
...
Процедура КодКартыМагнитныйПриИзменении(Элемент)
	...
	ПроверитьДублиДисконтныхКарт(ЭтаФорма);
	УправлениеФормой(ЭтаФорма);
	ДисконтныеКартыВызовСервера.ПодготовитьКодКартыПоНастройкамПоУмолчанию(Объект.КодКартыМагнитный);
	...
	Если ПолученныйТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
		...
		Иначе
			...
			ИначеЕсли Результат.Количество() = 0 Тогда
				...
				КонецЕсли;
			Иначе
				ВыбранноеЗначение = Результат.ВыбратьЭлемент(НСтр("ka='მაგნიტური ბარათის კოდის არჩევა';ru='Выбор кода магнитной карты';en='Choosing a magnetic card code'"));
...
&НаКлиенте
Процедура ВладелецКартыСоздание(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
		
	П = Новый Структура();
	П.Вставить("ГруппаКонтрагента", ПолучитьГруппуКонтрагента());
	П.Вставить("РежимВыбора", Истина);
	ОткрытьФорму("Справочник.Контрагенты.ФормаОбъекта", П, Элементы.ВладелецКарты)
	
КонецПроцедуры
// Конец СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки

&НаСервере
Функция ПолучитьГруппуКонтрагента()
	
	Возврат Константы.ГруппаКонтрагентовДисконтыхКарт.Получить();
