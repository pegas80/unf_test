...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РегламентированныйУчетСервер.ПриСозданииНаСервере(ЭтотОбъект);
	
	//Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
	//	Объект.ВидВыдачиНДФЛ = "7";
	//КонецЕсли;//Паганини: Перенес в справочник договоры
	
	//DD Функционал Транспортных Средств 
	Элементы.ТранспортныеСредства.Видимость = Константы.ФункционалТранспортныхСредств.Получить();
	...
	Если Константы.УстановитьВзаиморасчетыКонтрагента.Получить() = true тогда
	...
	Если БезРасчетовПоЗаказам И НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		Объект.ВестиРасчетыПоЗаказам = ЛОЖЬ;
	ИначеЕсли НЕ БезРасчетовПоЗаказам И НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда  
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [21 марта 2023]  НАЧАЛО
	УстановитьПараметрСпискаКартЛояльности();	
	// © Шишка В.В. [21 марта 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	...
	Элементы.Родитель.ТолькоПросмотр = (ЗапретРедактированияГруппыВСправочникеКонтрагенты И НЕ Объект.Ссылка.Пустая());
	//_Paganini
		 	
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ЗапретРредактированияРодителяКонтрагента") Тогда 
		Элементы.Родитель.Доступность = НЕ ЗначениеЗаполнено(Объект.Ссылка);
	КонецЕсли; //++Lazare 05.02.2024
...
// © Шишка В.В. [21 марта 2023]  НАЧАЛО
&НаСервере
Процедура УстановитьПараметрСпискаКартЛояльности()

	КартыЛояльностиКонтрагента.Параметры.УстановитьЗначениеПараметра("ВладелецКарты", Объект.Ссылка);	

КонецПроцедуры // УстановитьПараметрСпискаКартЛояльности()
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ПустаяСтрока(ОписаниеОшибкиЗаполнения) Тогда
		...
		Если ОписаниеОшибкиЗаполнения = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ka='კონტრაგენტების რეკვიზიტების ავტომატურად შესავსებად აუცილებელია მომხმარებლების ინტერნეტ-მხარდაჭერასთან შეერთება. მოვახდინოთ შეერთება ეხლავე?';ru='Для автоматического заполнения реквизитов контрагентов"
...
"Подключиться сейчас?'");
	...
	УстановитьВидимостьКартЛояльности(); //+++ Дмгг
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменилисьСчетаРасчетов" Тогда
		...
		Объект.СчетУчетаРасчетовСПокупателемРегл = Параметр.СчетУчетаРасчетовСПокупателемРегл;
		Объект.СчетУчетаАвансовПокупателяРегл = Параметр.СчетУчетаАвансовПокупателяРегл;
		Объект.СчетУчетаРасчетовСПоставщикомРегл = Параметр.СчетУчетаРасчетовСПоставщикомРегл;
		Объект.СчетУчетаАвансовПоставщикуРегл = Параметр.СчетУчетаАвансовПоставщикуРегл;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ Lazare 11.02.2025 ONESOFT-3834
	// Заполним служебный реквизит номера АдресЭП
	АдресЭП = Новый Массив;
	...
	Для Каждого СтрокаКИ Из ЭтотОбъект["КонтактнаяИнформация"] Цикл
		Если СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты И Не ПустаяСтрока(СтрокаКИ.Представление) Тогда
			АдресЭП.Добавить(СтрокаКИ.Представление);
		КонецЕсли;
	КонецЦикла;
	...
	ТекущийОбъект.АдресЭП = СтрСоединить(АдресЭП, ", ");
    //++ Lazare 11.02.2025 ONESOFT-3834
	...
	Если Не ЗначениеЗаполнено(Объект.ОсновноеТранпсортноеСедство) Тогда
		
		Если Объект.ТранспортныеСредства.Количество() <> 0 Тогда
			Объект.ОсновноеТранпсортноеСедство = Объект.ТранспортныеСредства[0].ТранпсортноеСедство;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//+++ Дмгг
	Если СоздатьКартуЛояльности И ЗначениеЗаполнено(ВидКартыЛояльности) Тогда
		ДобавитьКартуЛояльности(ВидКартыЛояльности);   
		УстановитьПараметрСпискаКартЛояльности();
	КонецЕсли;
	//--- Дмгг
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [21 марта 2023]  НАЧАЛО
	Если ИспользоватьКартыЛояльности Тогда //+++ Дмгг, система не любит обновлять невидимые элементы
		Элементы.КартыЛояльностиКонтрагента.Обновить();	
	КонецЕсли;
	// © Шишка В.В. [21 марта 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
...
Процедура ЗаполнитьРеквизитыПоДаннымЕГР(Команда)
	...
	// Выполняем заполнение по ИНН без открытия вспомогательной формы
		...
		Если КлючевыеРеквизитыЗаполнены(ЭтотОбъект) Тогда
			ТекстВопроса = НСтр("ka='გადავავსოთ მიმდინარე რეკვიზიტები?';ru='Перезаполнить текущие реквизиты?';en='Refill current information?'");
...
Процедура УправлениеФормой()
	...
	Если Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо") Тогда
		...
		Элементы.ИНН.ПодсказкаВвода							= НСтр("ka='11 ციფრი';ru='11 цифр';en='11 digits'");
		Элементы.ИНН.ОграничениеТипа						= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(11));
		Элементы.КодПоОКПО.ПодсказкаВвода					= НСтр("ka='8 ციფრი';ru='8 цифр';en='8 digits'");
		...
		Если КонтрагентЗарегистрированВРФ Тогда
			Элементы.РегистрационныйНомер.Заголовок			= НСтр("ka='ძსსნ';ru='ОГРН';en='BIN'");
			Элементы.РегистрационныйНомер.Подсказка			= НСтр("ka='იურიდიული პირის ძირითადი სახელმწიფო სარეგისტრაციო ნომერი';ru='Основной государственный регистрационный номер юридического лица';en='Primary state registration number of legal entity'");
			Элементы.РегистрационныйНомер.ПодсказкаВвода	= НСтр("ka='13 ციფრი';ru='13 цифр';en='13 digits'");
			Элементы.РегистрационныйНомер.ОграничениеТипа	= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
			Элементы.ИНН.Подсказка							= НСтр("ka='გადამხდელის საიდენტიფიკაციო ნომერი';ru='Идентификационный номер налогоплательщика';en='Taxpayer Identification Number'");
		Иначе
			Элементы.РегистрационныйНомер.Заголовок			= НСтр("ka='სარეგ. ნომერი';ru='Рег. номер';en='Reg. number'");
			Элементы.РегистрационныйНомер.Подсказка			= НСтр("ka='უცხოური ორგანიზაციისთვის რეგისტრაციის (ინკორპორაციის) ქვეყანაში მინიჭებული სარეგისტრაციო ნომერი';ru='Регистрационный номер, присвоенный иностранной организации в стране регистрации (инкорпорации)';en='Registration number assigned by the foreign organization in the country of registration (incorporation)'");
			Элементы.РегистрационныйНомер.ПодсказкаВвода	= НСтр("ka='სარეგისტრაციო ნომერი რეგისტრაციის ქვეყანაში';ru='Рег. номер в стране регистрации';en='Reg. number in the country of registration'");
			Элементы.РегистрационныйНомер.ОграничениеТипа	= Новый ОписаниеТипов("Строка");
			Элементы.ИНН.Подсказка							= НСтр("ka='გადამხდელის საიდენტიფიკაციო ნომერი, ენიჭება უცხოურ ორგანიზაციას საგადასახადო ორგანოში აღრიცხვაზე პირველად აყვანისას';ru='Идентификационный номер налогоплательщика, присваивается иностранной организации "
"при первой постановке на учет в налоговом органе Российской Федерации'");
	...
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ИндивидуальныйПредприниматель") Тогда
		...
		Элементы.РегистрационныйНомер.Заголовок				= НСтр("ka='ОГРН ИП';ru='ОГРН ИП';en='BIN SP'");
...
"указан в Свидетельстве о государственной регистрации физического лица в качестве ИП';ka='Основной государственный регистрационный номер индивидуального предпринимателя, "
"указан в Свидетельстве о государственной регистрации физического лица в качестве ИП'");
		Элементы.РегистрационныйНомер.ПодсказкаВвода		= НСтр("ka='15 ციფრი';ru='15 цифр';en='15 digits'");
		Элементы.РегистрационныйНомер.ОграничениеТипа		= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(15));
		Элементы.ИНН.ПодсказкаВвода							= НСтр("ka='11 ციფრი';ru='11 цифр';en='11 digits'");
		Элементы.ИНН.ОграничениеТипа						= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(11));
		Элементы.КодПоОКПО.ПодсказкаВвода					= НСтр("ka='13 ციფრი';ru='13 цифр';en='13 digits'");
	...
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ГосударственныйОрган") Тогда
		...
		Если ЗначениеЗаполнено(Объект.ВидГосударственногоОргана) Тогда
			...
			Если Объект.ВидГосударственногоОргана = ПредопределенноеЗначение("Перечисление.ВидыГосударственныхОрганов.НалоговыйОрган") Тогда
				...
				Элементы.КодГосударственногоОргана.Заголовок		= НСтр("ka='Код инспекции';ru='Код инспекции';en='code inspection'");
			ИначеЕсли Объект.ВидГосударственногоОргана = ПредопределенноеЗначение("Перечисление.ВидыГосударственныхОрганов.ОрганПФР") Тогда
				...
				Элементы.КодГосударственногоОргана.Заголовок		= НСтр("ka='განყოფილების კოდი';ru='Код отделения';en='branch code'");
			...
			Иначе
				...
				Элементы.КодГосударственногоОргана.Заголовок		= НСтр("ka='კოდი';ru='Код';en='The code'");
			КонецЕсли;
	...
	Если Объект.ЯвляетсяНеРезидентом Тогда
		Элементы.ИНН.ПодсказкаВвода	 = НСтр("ru='20 цифр';ka='20 ციფრი'");
		Элементы.ИНН.ОграничениеТипа = Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(20));
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Объект.Покупатель Тогда
		ВидыОтношений.Добавить(НСтр("ka='მყიდველი';ru='Покупатель';en='Customer'"));
	КонецЕсли;
	
	Если Объект.Поставщик Тогда
		ВидыОтношений.Добавить(НСтр("ka='მომწოდებელი';ru='Поставщик';en='Supplier'"));
	КонецЕсли;
	
	Если Объект.ПрочиеОтношения Тогда
		ВидыОтношений.Добавить(НСтр("ka='სხვა ურთიერთობები';ru='Прочие отношения';en='Other relationship'"));
	КонецЕсли;
	
	Заголовок = Объект.Наименование + " (" + НСтр("ka='კონტრაგენტი';ru='Контрагент';en='Counterparty'");
...
//+++ Дмгг
&НаСервере
Процедура УстановитьВидимостьКартЛояльности()

	Элементы.КартыЛояльности.Видимость = ИспользоватьКартыЛояльности;

КонецПроцедуры

&НаСервере
Процедура ДобавитьКартуЛояльности(ВидКарты)

	НоваяКарта = Справочники.КартыЛояльности.СоздатьЭлемент();
	
	НоваяКарта.Владелец = ВидКарты;
	НоваяКарта.ДатаНачала = НачалоДня(ТекущаяДата());
	НоваяКарта.ДатаОкончания = НачалоДня(ТекущаяДата()) + 86400 * 365;
	
	НоваяКарта.ВладелецКарты = Объект.Ссылка;
	НоваяКарта.Наименование = Объект.Наименование + ", " + Строка(ВидКартинки) + ", " + НоваяКарта.КодКартыШтрихкод + " / " + НоваяКарта.КодКартыМагнитный;
	
	НоваяКарта.ОбменДанными.Загрузка = Истина; // временно пока не понятно как заполнять штрих и магнит код
	НоваяКарта.Записать();
	
	ЭтаФорма.СоздатьКартуЛояльности = ЛОЖЬ;
	...
	Для Каждого ДанныеКонтактногоЛица Из ДанныеКонтактныхЛиц Цикл
		...
		Для Каждого СтрокаТаблицы Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			Если Не ПустаяСтрока(СтрокаТаблицы.ЗначенияПолей) Или Не ПустаяСтрока(СтрокаТаблицы.Представление) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ФИО контакта не заполнено.';ru='ФИО контакта не заполнено.';en='Name of contact is not filled.'"),,,ИмяРеквизита, Отказ);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаТаблицы Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			...
			Если СвойстваВида.ОбязательноеЗаполнение И ПустаяСтрока(СтрокаТаблицы.Представление)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ka='საკონტაქტო ინფორმაციის ტიპი ""%1"" არ არის შევსებული.';ru='Вид контактной информации ""%1"" не заполнен.';en='View contact information ""%1"" is empty.'"), СвойстваВида.ПредставлениеВида),,, ИмяРеквизита);
...
Процедура ДобавитьКонтактнуюИнформациюКонтактаВидВыбран(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если СвойстваВида.ВыводитьВФормеВсегда = Ложь Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ka='""%1""-ის შეყვანის შესაძლებლობა დამატებული იქნება ყველა %2-სთვის."
"გავაგრძელოთ?';ru='Возможность ввода ""%1"" будет добавлена для всех %2."
"Продолжить?'"), ВыбранныйЭлемент.Значение, ДополнительныеПараметры.МножественнаяФормаВладельца);
		ЗаголовокВопроса = НСтр("ka='დამატების დადასტირება';ru='Подтверждение добавления';en='Uploading confirmation'");
...
Процедура Подключаемый_ДействиеКИКонтактаНажатие(Элемент)
	...
	Если ДанныеКИ.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Skype") Тогда
		...
		Список.Добавить("Позвонить", НСтр("ka='ჩატის დაწყება';ru='Начать чат';en='IM'"));
		Список.Добавить("НачатьЧат", НСтр("ka='ჩატის დაწყება';ru='Начать чат';en='IM'"));
...
Процедура СформироватьПредставлениеПроверкиДублей(Форма)
	...
	Если ИННиКППКорректны(Форма) И Не ПустаяСтрока(Объект.ИНН) Тогда
		...
		Если КоличествоДублей > 0 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("ИННиКПП", ?(ЭтоЮрЛицо(Объект.ВидКонтрагента), НСтр("ka='გსნ და აამკ';ru='ИНН и КПП';en='TIN and CRR'"), НСтр("ka='ИНН';ru='ИНН';en='INN'")));
			
			Если КоличествоДублей = 1 Тогда
				СтруктураПараметровСообщенияОДублях.Вставить("КоличествоДублей", НСтр("ka='один';ru='один';en='one'"));
				СтруктураПараметровСообщенияОДублях.Вставить("СклонениеКонтрагентов", НСтр("ka='კონტრაგენტი';ru='контрагент';en='counterpart'"));
			ИначеЕсли КоличествоДублей < 5 Тогда
				СтруктураПараметровСообщенияОДублях.Вставить("КоличествоДублей", КоличествоДублей);
				СтруктураПараметровСообщенияОДублях.Вставить("СклонениеКонтрагентов", НСтр("ka='კონტრაგენტს';ru='контрагента';en='counterparty'"));
			Иначе
				СтруктураПараметровСообщенияОДублях.Вставить("КоличествоДублей", КоличествоДублей);
				СтруктураПараметровСообщенияОДублях.Вставить("СклонениеКонтрагентов", НСтр("ka='კონტარგენტებს';ru='контрагентов';en='counterparties'"));
			КонецЕсли;
			
			ОписаниеОшибки = НСтр("ka='С таким [ИННиКПП] есть [КоличествоДублей] [СклонениеКонтрагентов]';ru='С таким [ИННиКПП] есть [КоличествоДублей] [СклонениеКонтрагентов]';en='With such [INNiKPP] is [KolichestvoDubley] [SklonenieKontragentov]'");
...
Процедура УстановитьЗаголовокЮридическихДанных(Форма)
	...
	Если Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='იურიდ. პირი';ru='Юр.лицо';en='Entity'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ИндивидуальныйПредприниматель") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ИП';ru='ინდ. მეწარმე'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ФизическоеЛицо") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ფიზ. პირი';ru='Физ.лицо';en='person'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ГосударственныйОрган") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='სახ. ორგანო';ru='Гос.орган';en='Gos.organ'"));
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.ИНН) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ს/კ';ru='ИНН';en='INN'") + " " + Объект.ИНН);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='ს/კ...';ru='ИНН...';en='INN ...'"));
	КонецЕсли;
...
Процедура ПрочитатьДанныеПанелиДопИнформации()
	...
	Если ПросмотрПродаж Тогда
		...
		КомпонентыФС = Новый Массив;
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='გაყიდვები';ru='Продажи на';en='sales in the'") + " ", КрупныйШрифт));
		...
		КомпонентыФС = Новый Массив;
		КомпонентыФС.Добавить(НСтр("ka='ბოლო გაყიდვა';ru='Последняя продажа';en='Last sale'") + " ");
	...
	КомпонентыФС = Новый Массив;
	КомпонентыФС.Добавить(НСтр("ka='ბოლო მოვლენა';ru='Последнее событие';en='Last event'") + " ");
	КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(Формат(Дата, "Л=ru_RU; ДЛФ=D; ДП=<нет>"), , , , НавСсылка));
...
Процедура ПолучитьИнфоИзRSСервер()
	...
	ДанныеКонтрагента = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьИнфоИзRSНаСервере(Объект.ИНН);
	...
	Объект.Наименование       = ДанныеКонтрагента.Имя;
	Объект.НаименованиеПолное = ДанныеКонтрагента.Имя;
	...
	ДанныеКонтрагента = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьИнфоИзRSНаСервереНов(Объект.ИНН);
...
&НаСервере
Процедура УстановитьОсновноеТранспортноеСредствоНаСервере()
	Если Элементы.ТранспортныеСредства1.ТекущаяСтрока <> Неопределено Тогда
		СтрокаТаблицы = Объект.ТранспортныеСредства.НайтиПоИдентификатору(Элементы.ТранспортныеСредства1.ТекущаяСтрока);
		Объект.ОсновноеТранпсортноеСедство = СтрокаТаблицы.ТранпсортноеСедство;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура УстановитьОсновноеТранспортноеСредство(Команда)
	УстановитьОсновноеТранспортноеСредствоНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура НеЯвляетсяРезидентомПриИзменении(Элемент)
	УправлениеФормой();
КонецПроцедуры

&НаКлиенте
Процедура СоздатьКартуЛояльностиПриИзменении(Элемент)
	
	Элементы.ВидКартыЛояльности.Видимость = СоздатьКартуЛояльности;
...
Процедура ОбновитьАдресИТелефонВОсновномДоговореКонтрагента()
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПриИзминенииИлиСозданииКонтрагентаОбновлениеАдресаИТелефонаВОсновномДоговоре") Тогда
		Адрес_Контрагента	= "";
		АресаКонтрагента 	= Объект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип, Вид", Перечисления.ТипыКонтактнойИнформации.Адрес, УправлениеНебольшойФирмойПовтИсп.ПолучитьПредопределенноеЗначениеСправочника("ВидыКонтактнойИнформации", "ФактАдресКонтрагента")));
		Если АресаКонтрагента.Количество() > 0 Тогда
			Адрес_Контрагента 	= АресаКонтрагента[0].Представление;
		КонецЕсли;
		Телефон_Контрагента = "";
		ТелефоныКонтрагента = Объект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип", Перечисления.ТипыКонтактнойИнформации.Телефон));
		Если ТелефоныКонтрагента.Количество() > 0 Тогда
			Телефон_Контрагента = ТелефоныКонтрагента[0].Представление;
		КонецЕсли;
		Если НЕ Адрес_Контрагента = Объект.ДоговорПоУмолчанию.Адрес ИЛИ НЕ Телефон_Контрагента = Объект.ДоговорПоУмолчанию.Телефон Тогда
			ДоговорПоУмолчанию_Объект = Объект.ДоговорПоУмолчанию.ПолучитьОбъект();
