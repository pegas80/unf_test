...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მუშაობის გრაფიკში შესაყვანი დღეები არ არის აღნიშნული';ru='Не отмечены дни, включаемые в график работы';en='Not marked the days included in the work schedule'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если МодифицированностьРезультата Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ჩავწეროთ შეცვლილი მონაცემები %1 წლის მანძილზე?';ru='Записать измененные данные за %1 год?';en='Record the changed data for% 1 year?'"), Формат(НомерПредыдущегоГода, "ЧГ=0"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ka='განრიგის შევსება';ru='Заполнить расписание';en='fill in the schedule'"));
...
Процедура СформироватьШаблонЗаполнения(СпособЗаполнения, ШаблонЗаполнения, Знач ДлинаЦикла, Знач ДатаОтсчета = Неопределено)
	...
	Если СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафикаРаботы.ПоНеделям") Тогда
		ШаблонЗаполнения[0].ПредставлениеДня = НСтр("ka='ორშაბათი';ru='Понедельник';en='Monday'");
		ШаблонЗаполнения[1].ПредставлениеДня = НСтр("ka='სამშაბათი';ru='Вторник';en='Tuesday'");
		ШаблонЗаполнения[2].ПредставлениеДня = НСтр("ka='ოთხშაბათი';ru='Среда';en='Wednesday'");
		ШаблонЗаполнения[3].ПредставлениеДня = НСтр("ka='ხუთშაბათი';ru='Четверг';en='Thursday'");
		ШаблонЗаполнения[4].ПредставлениеДня = НСтр("ka='პარასკევი';ru='Пятница';en='Friday'");
		ШаблонЗаполнения[5].ПредставлениеДня = НСтр("ka='შაბათი';ru='Суббота';en='Saturday'");
		ШаблонЗаполнения[6].ПредставлениеДня = НСтр("ka='კვირა დღე';ru='Воскресенье';en='Sunday'");
	Иначе
		ДатаДня = ДатаОтсчета;
...
Функция ПредставлениеРасписанияДня(Форма, НомерДня)
	...
	Если Секунд = 0 Тогда
		Возврат НСтр("ka='განრიგის შევსება';ru='Заполнить расписание';en='fill in the schedule'");
	КонецЕсли;
	
	Часов = Окр(Секунд / 3600, 1);
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 სთ. (%2)';ru='%1 ч. (%2)';en='% 1 hr. (2%)'"), Часов, ПредставлениеИнтервалов);
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.ДатаЗаполненности) Тогда
		Форма.ЗаполненностьИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკი არ არის შევსებული';ru='График работы не заполнен';en='Schedule is empty'");
		Форма.ТребуетЗаполнения = Истина;
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			ИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკი შევსებულია %1-მდე';ru='График работы заполнен до %1';en='Schedule filled to 1%'");
		...
		Иначе											
			...
			ИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკი შევსებულია %1-მდე, დაგეგმვის ჰორიზონტის გათვალისწინებით გრაფიკი უნდა შეივსოს %2-მდე';ru='График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2';en='Schedule filled to 1%, based schedule planning horizon has to be filled up to 2%'");
...
Процедура ЗаполнитьИнформационныйТекстРезультатаЗаполнения(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационныйТекст = НСтр("ka='მიმდინარე წლის სამუშაო გრაფიკი შეცვლილია ხელით. ავტომატურ შევსებაზე დასაბრუნებლად, დააჭირეთ ""შაბლონის მიხედვით შევსებას"".';ru='График работы на текущий год изменен вручную. Нажмите «Заполнить по шаблону» чтобы вернуться к автоматическому заполнению.';en='Schedule for the current year has been manually changed. Click the ""Fill pattern"" to return to the automatic filling.'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				ИнформационныйТекст = НСтр("ka='მუშაობის გრაფიკის განახლება ავტომატურად ხდება, მიმდინარე წლის საწარმოო კალენდრის შეცვლისას.';ru='График работы автоматически обновляется при изменении производственного календаря за текущий год.';en='Work schedule is automatically updated when you change the factory calendar for the current year.'");
				ИнформационнаяКартинка = БиблиотекаКартинок.Информация;
			КонецЕсли;
		Иначе
			ИнформационныйТекст = НСтр("ka='გამოსახული შედეგი შაბლონის გამართვას არ შეესაბამება. დააჭირეთ ""შაბლონის მიხედვით შევსებას"" იმისათვის, რომ ნახოთ, თუ როგორ გამოიყურება მუშაობის გრაფიკი შაბლონის ცვლილებების გათვალისწინებით.';ru='Отображаемый результат не соответствует настройке шаблона. Нажмите «Заполнить по шаблону», чтобы увидеть как выглядит график работы с учетом изменений шаблона.';en='The display result does not meet the setup template. Click the ""Fill pattern"" to see looks like work schedule taking into account the pattern changes.'");
			ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
			ДоступноЗаполнениеПоШаблону = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьИнформационныйТекстРучногоРедактирования(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
		ИнформационныйТекст = НСтр("ka='მიმდინარე წლის მუშაობის გრაფიკი შეცვლილია ხელით. ცვლილებები გამოყოფილია შევსების შედეგებში.';ru='График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.';en='Schedule for the current year has been manually changed. Changes are highlighted in the results of the filling.'");
	КонецЕсли;
