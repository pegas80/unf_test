	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокументов Цикл
		Если Не СтрокаТаблицы.ФайлыВыбраны И ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Добавьте файлы для документа ""%1""';ru='Добавьте файлы для документа ""%1""';en='Add files to document ""%1""'"), СтрокаТаблицы.Документ),, 
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ka='Заполните вид документа в строке %1';ru='Заполните вид документа в строке %1';en='Fill in the form of a document at line% 1'"), ТаблицаДокументов.Индекс(СтрокаТаблицы) + 1),,
...
Функция ПредставлениеФайлов(Файлы)
	...
	Если ЗначениеЗаполнено(Файлы) Тогда
		...
		Если КоличествоСтраниц = 1 Тогда
			ШаблонПредставления = НСтр("ka='Страница № %1 (%2)';ru='Страница № %1 (%2)';en='Page 1 №% (2%)'");
		Иначе
			ШаблонПредставления = НСтр("ka='Страницы № %1 (%2)';ru='Страницы № %1 (%2)';en='Pages №% 1 (2%)'");
		КонецЕсли;
...
Функция ПроверитьЗаполнениеПоляОтправитель()
	...
	Если Не ЗначениеЗаполнено(Объект.Отправитель) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ka='Заполните поле ""От кого""';ru='Заполните поле ""От кого""';en='Fill in the field ""From""'");
	...
	Если ЗначениеЗаполнено(Объект.Отправитель) И Не ЗначениеЗаполнено(УчетнаяЗаписьОтправителя(Объект.Отправитель)) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ka='Организация не подключена к 1С-Отчетности';ru='Организация не подключена к 1С-Отчетности';en='The organization is not connected to 1C-Accounting'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.Отправитель");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
...
Функция ПроверитьЗаполнениеПоляПолучатель()
	...
	Если Не ЗначениеЗаполнено(Объект.Получатель) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ka='Заполните поле ""Кому""."
"Требования к документам определяются выбранным органом ПФР.';ru='Заполните поле ""Кому""."
...
Функция ПроверитьЗаполнениеПоляСотрудник()
	...
	Если Не ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Заполните поле ""Сотрудник""';ru='Заполните поле ""Сотрудник""';en='Fill in the ""Employee""'"),, 
			"Объект.Сотрудник");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
	...
	Если ПроверитьЗаполнениеПоляОтправитель() И ПроверитьЗаполнениеПоляПолучатель() Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Документ) Тогда
				ОчиститьСообщения();
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='ველი ""დოკუმენტი"" არ არის შევსებული';ru='Поле ""Документ"" не заполнено';en='The ""document"" is not filled'"),, 
		...
		Иначе
			...
			ОперацииСФайламиЭДКОКлиент.ДобавитьФайлы(Оповещение, УникальныйИдентификатор,
				СтрШаблон(НСтр("ru='Выберите файлы документа <%1> ';ka='Выберите файлы документа <%1> '"), Документ), ПараметрыФормы);		
		КонецЕсли;
	КонецЕсли;
