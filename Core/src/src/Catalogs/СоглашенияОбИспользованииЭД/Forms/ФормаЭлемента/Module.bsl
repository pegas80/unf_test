...
"Отклонить?';ka='ხელშეკრულების მოქმედების გაუქმებისას, საჭიროა მოწვევის უარყოფა. უარვყოთ?'");
...
"Изменить идентификатор получателя?';ka='Идентификатор получателя заполнен, при изменении контрагента он будет изменен."
...
Процедура ШифроватьДанныеПакетаЭДПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СертификатОрганизацииДляРасшифровки)
		...
		ТекстВопроса = НСтр("ka='დაშიფრვის გამართვები გასუფთავდება. გავაგრძელოთ?';ru='Настройки шифрования будут очищены. Продолжить?';en='Encryption settings will be cleared. Proceed?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ПроверятьСертификатыПодписейПриИзменении(Элемент)
	...
	Если Объект.СертификатыПодписейКонтрагента.Количество() <> 0 Тогда
		ТекстВопроса = НСтр("ka='გამართვები სერთიფიკატების შემოწმება კონტრაგენტების ხელმოწერები გასუფთავდება. გავაგრძელოთ?';ru='Настройки проверка сертификатов подписи контрагентов будут очищены. Продолжить?';en='Settings check counterparties signing certificates will be cleared. Proceed?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ДоговорКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Исключение
		...
		ВидОперации = НСтр("ka='ხელშეკრულების არჩევის ფორმის გახსნა';ru='Открытие формы выбора договора';en='Opening the choice of the form of the contract'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='ხელშეკრულების არჩევის ფორმის გახსნის შეცდომა';ru='Ошибка открытия формы выбора договора';en='Failed to open the contract to choose the form'");
...
Процедура ОтправитьПриглашение(Команда)
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ka='ედბ-ის მიმდინარე გამართვაში შეტანილია ცვლილებები. ჩავწეროთ?';ru='В текущую настройку ЭДО внесены изменения. Записать?';en='In the current setting of the EDC amended. Record?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ТестНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='ედბ-ის მიმდინარე გამართვაში შეტანილია ცვლილებები. გავაგრძელოთ ტესტის შესრულება?';ru='В текущую настройку ЭДО внесены изменения. Продолжить выполнение теста?';en='In the current setting of the EDC amended. Continue the test?'");
...
Процедура ВключитьРасширенныйРежимНастройкиСоглашения(Команда)
	...
	Если Объект.РасширенныйРежимНастройкиСоглашения Тогда
		ТекстВопроса = НСтр("ka='გაფართოებული რეჟიმის ცვლილებები გასუფთავდება. გავაგრძელოთ?';ru='Изменения расширенного режима будут очищены."
"Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ИсходящиеДокументыПередНачаломИзмененияНаСервере(Отказ)
	...
	Если Не Объект.РасширенныйРежимНастройкиСоглашения Тогда
		ТекстСообщения = НСтр("ka='ცხრილ ""ელექტრონული დოკუმენტები""-ს შეცვლა შესაძლებელია:"
"""ჯგუფური"" - ედბ-ის გამართვების პროფილში;"
"""ინდივიდუალური"" - გამართვების გაფართოებულ რეჟიმში.';ru='Изменение табличной части ""Электронные документы"" возможно:"
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		Если Не Объект.ИспользоватьУПД Тогда
			...
			СписокВыбора.Добавить(НСтр("ka='ФНС 5.01 (УПД:Счет-фактура)';ru='ФНС 5.01 (УПД:Счет-фактура)';en='FTS 5.01 (UPD: Invoice)'"), НСтр("ka='ФНС 5.01 (УПД:Счет-фактура)';ru='ФНС 5.01 (УПД:Счет-фактура)';en='FTS 5.01 (UPD: Invoice)'"));
		КонецЕсли;
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.КорректировочныйСчетФактура") Тогда
		...
		КонецЕсли;
		СписокВыбора.Добавить(НСтр("ka='CML 2.08';ru='CML 2.08';en='CML 2.08'"), НСтр("ka='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';ru='ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';en='FTS 5.01 (SACs: Correction invoice and the original document)'"));
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		СписокВыбора.Добавить(НСтр("ka='ФНС 5.01';ru='ФНС 5.01';en='FTS 5.01'"), НСтр("ka='ФНС 5.01';ru='ФНС 5.01';en='FTS 5.01'"));
		Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Заголовок = НСтр("ka='ედბ-ის გამართვების გაფართოებული რეჟიმის ჩართვა';ru='Включить расширенный режим настроек ЭДО';en='Enable advanced settings EDM mode'");
	Если Объект.РасширенныйРежимНастройкиСоглашения Тогда
		Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Заголовок = НСтр("ka='ედბ-ის გამართვების გაფართოებული რეჟიმის გამორთვა';ru='Выключить расширенный режим настроек ЭДО';en='Turn off the advanced settings EDM mode'");
	КонецЕсли;
...
Процедура ОбновитьВходящийКаталога(Форма)
	...
	ШаблонЗаголовка = НСтр("ka='სრული გზა: %1';ru='Полный путь: %1';en='Full path:% 1'");
...
Процедура ОпределитьИспользуемыеСпособыОбменаЭД(ПрофильНастроекЭДО = Неопределено, ИдентификаторКонтрагента = Неопределено)
	...
	Для каждого СтрокаТаблицы Из Объект.ИсходящиеДокументы Цикл
		...
		Если СтрокаТаблицы.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
			...
			ШаблонНаименование = НСтр("ka='%1_%2';ru='%1_%2';en='1_% 2%'");
...
#Если НЕ ВебКлиент Тогда
	...
	ДиалогКаталога.Заголовок = НСтр("ka='მიუთითეთ კატალოგი ედ-ის გასაცვლელად';ru='Укажите каталог для обмена ЭД';en='Specify the directory for ED exchange'");
...
Процедура ДобавитьДанныеПоТабЧасти(ВыбранноеЗначение, АдресВХранилище = Неопределено)
	...
	Если АдресВХранилище <> Неопределено Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ka='სერთიფიკატის ფაილი უნდა იყოს შემდეგ ფორმატში DER X.509,  ოპერაცია შეწყვეტილია.';ru='Файл сертификата должен быть в формате DER X.509, операция прервана.';en='The certificate file must be in DER X.509 format, the operation is aborted.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецПопытки;
...
Процедура ИспользованиеИдентификатораКонтрагентаУникально(Отказ)
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ШаблонСообщения = НСтр("ka='კონტრაგენტ %1-ის იდენტიფიკატორი უკვე გამოიყენება კონტრაგენტ %2-ს და ორგანიზაცია %3-ს შორის ედბ-ის გამართვაში';ru='Идентификатор контрагента %1"
...
"%1';ka='გაცვლის კატალოგებთან დაშვების შემოწმება."
"%1'");
	Попытка
...
Процедура ТестНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ЧерезОператораЭДО Тогда
		Состояние(НСтр("ka='აწყობეების ტესტი.';ru='Тест настроек.';en='Test Settings.'"),
			,
			НСтр("ka='ედბ-ის სერვისით მიმდინარეობს ედ-ის გაცვლის ტესტირება. მოიცადეთ...';ru='Выполняется тестирование обмена ЭД через сервис ЭДО. Подождите...';en='ED test is performed through the exchange of EDI services. Wait...'"));
	КонецЕсли;
...
"В профиле нет доступных сертификатов. Тест не выполнен.';ka='გაცვლის ტესტი პროფილის მიხედვით: %1. პროფილში არ არის ხელმისაწვდომი სერთიფიკატები. ტესტი არ არის შესრულებული.'");
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		...
		Если РезультатВыполнения.Успех = Истина Тогда
			ШаблонСообщения = НСтр("ru='Тест обмена по профилю: %1"
"Пройден успешно.';ka='გაცვლის ტესტი პროფილის მიხედვით: %1 წარმატებით იქნა გავლილი.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Контекст.ПрофильНастроекЭДО);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
"Тест не выполнен.';ka='პროფილ %1-ით გაცვლის ტესტი. პროფილში არ არის ხელმისაწვდომი სერთიფიკატები. ტესტი არ არის შესრულებული.'");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтаФорма.ЧерезКаталог ИЛИ ЭтаФорма.ЧерезЭлектроннуюПочту ИЛИ ЭтаФорма.ЧерезFTP Тогда
		...
		Если (ЗначениеЗаполнено(Объект.СертификатОрганизацииДляРасшифровки) И Не ЗначениеЗаполнено(ПредставлениеСертификатаКонтрагентаДляШифрования))
			...
			ТекстСообщения = НСтр("ka='დაშიფრვის კორექტულად სამუშაოდ აუცილებელია ერთდროულად დაშიფრვის სერთიფიკატების მითითება ორგანიზაციისთვის და კონტრაგენტისთვის.';ru='Для корректной работы шифрования необходимо"
