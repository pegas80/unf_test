...
Процедура ЗаполнитьПредставлениеНоменклатуры()
	...
	ЗаполнитьПредставлениеНоменклатурыНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПредставлениеНоменклатурыНаСервере()
	
	ТаблицаРеквизитов = Новый ТаблицаЗначений;
	Справочники.ПрайсЛисты.ДоступныеПоляНоменклатуры(ТаблицаРеквизитов);	
	
	СоставРеквизитов = "";
	Для каждого Строка Из Объект.ПредставлениеНоменклатуры Цикл
		
		Если Строка.Использование Тогда
			ПредставлениеРеквизита = Строка.РеквизитПредставление;
			СтрокаСоответствия = ТаблицаРеквизитов.Найти(Строка.РеквизитНоменклатуры, "РеквизитНоменклатуры");
			Если СтрокаСоответствия <> Неопределено Тогда
				 ПредставлениеРеквизита = СтрокаСоответствия.РеквизитПредставление; 
			КонецЕсли;	
			
			СоставРеквизитов = СоставРеквизитов + ?(ПустаяСтрока(СоставРеквизитов), "", ", ") + ПредставлениеРеквизита;
	...
	Если Объект.ПечатьПрайсЛиста = КэшЗначений.Диафильм Тогда
		...
		СоставРеквизитов = СоставРеквизитов + НСтр("ru=', Колонок: ';ka=', სვეტი: '") + Объект.КоличествоКолонок + НСтр("ru=', Размер: ';ka=', ზომა: '") + Объект.КартинкаШирина + НСтр("ru=' х ';ka=' х '") + Объект.КартинкаВысота;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(Объект.Наименование) Тогда
		...
		Объект.Наименование = Нстр("ka='პრაის-ლისტი';ru='Прайс-лист';en='Price list'");
		
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ПоложениеТекстаОтносительноМеткиНовинка) Тогда
		
		Объект.ПоложениеТекстаОтносительноМеткиНовинка = Нстр("ru='Под текстом';ka='ტექსტის ქვემოთ'");
	...
	Если НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаИерархияВалюта", "Заголовок", НСтр("ka='პრაის-ლისტის დაჯგუფება';ru='Группировка прайс-листа';en='Grouping pricelist'"));
