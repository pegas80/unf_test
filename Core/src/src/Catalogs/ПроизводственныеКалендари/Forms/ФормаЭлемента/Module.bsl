...
Процедура ПеренестиДень(Команда)
	...
	ПараметрыВыбораДаты.Заголовок = НСтр("ka='გადატანის თარიღის არჩევა';ru='Выбор даты переноса';en='The choice of the date of transfer'");
	
	ТекстСообщения = НСтр("ka='აირჩიეთ თარიღი, რომელზეც განხორციელდება დღის გადატანა %1 (%2)';ru='Выберите дату, на которую будет осуществлен перенос дня %1 (%2)';en='Select the date on which the transfer of the day 1% (2%) will be carried out'");
...
Процедура Печать(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПоказатьВопрос(
			Обработчик,
			НСтр("ka='საწარმოო კალენდრის მონაცემები ჯერ არ არის ჩაწერილი."
"ამობეჭდვა შესაძლებელია მხოლოდ მონაცემთა ჩაწერის შემდეგ."
""
"ჩავწეროთ?';ru='Данные производственного календаря еще не записаны."
...
Процедура ЗаполнитьПредставлениеПереносов(Форма)
	...
	Для Каждого КлючИЗначение Из Форма.ПереносыДней Цикл
		...
		Форма.СписокПереносов.Добавить(ДатаИсточник, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 (%3) გადატანილია %2 (%4)-ისთვის';ru='%1 (%3) перенесен на %2 (%4)';en='1% (3%) transferred to 2% (4%)'"), Формат(ДатаИсточник, "ДФ='д ММММ'"), Формат(ДатаПриемник, "ДФ='д ММММ'"), ВидДняИсточник, ВидДняПриемник));
	КонецЦикла;
