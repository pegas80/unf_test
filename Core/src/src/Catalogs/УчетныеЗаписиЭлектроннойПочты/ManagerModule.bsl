...
Функция ПроверитьПодключениеКСерверуВходящейПочты(Профиль, Протокол)
	...
	?(ПустаяСтрока(ТекстОшибки), НСтр("ka='OK';ru='OK';en='OK'"), ТекстОшибки));
	
	ЗаписьЖурналаРегистрации(НСтр("ka='საფოსტო სერვერთან შეერთების შემოწმება';ru='Проверка подключения к почтовому серверу';en='Verify connectivity to the mail server'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	ИмяОтправителяПисем = НСтр("ka='1С:საწარმო';ru='1С:Предприятие';en='1C: Enterprise'");
