...
Процедура ПометитьНаУдалениеСнятьПометку(Команда)
	...
	Иначе
		Если ЕстьПометкаУдаления Тогда
			ТекстВопроса = НСтр("ka='გამოყოფილ ელემენტებს მოვხსნათ წაშლის ნიშნული?';ru='Снять с выделенных элементов пометку на удаление?';en='Remove from selected items marked for deletion?'");
		Иначе
			ТекстВопроса = НСтр("ka='გამოყოფილი ელემენტები მოვნიშნოთ წასაშლელად?';ru='Пометить выделенные элементы на удаление?';en='Mark selected items for deletion?'");
		КонецЕсли;
	КонецЕсли;
