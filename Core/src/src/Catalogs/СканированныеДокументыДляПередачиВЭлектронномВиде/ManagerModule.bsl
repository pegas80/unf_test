...
Функция ЗагрузитьИзВнешнегоИсточникаПоСтруктуреРезультатаВыбора(СтруктураРезультата) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось извлечь файлы из файла обмена';ru='Не удалось извлечь файлы из файла обмена';en='Unable to extract files from file sharing'"));
	...
	Для каждого СтрокаТЗЗагруженныеДокументы Из ТЗЗагруженныеДокументы Цикл
		...
		Для каждого Участник Из МассивУчастниковДокумента Цикл
			...
			Если СтрокаРеквизитов = Неопределено Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось определить реквизиты одного из участников документа';ru='Не удалось определить реквизиты одного из участников документа';en='Unable to determine the details of one of the participants in the document'") + ":
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось записать элемент справочника';ru='Не удалось записать элемент справочника';en='Unable to write the directory entry'") + ":
			|" + ОписаниеОшибки());
		...
		Для каждого СтрокаТЗФайлыДокументов Из МассивФайловДокумента Цикл
			...
			Если НЕ ЗагружаемыйФайл.Существует() Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='В пакете обмена не обнаружен файл';ru='В пакете обмена не обнаружен файл';en='In the not found file sharing package'") + "
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ka='Электронный документооборот с контролирующими органами. Сканированные документы для передачи в электронном виде';ru='Электронный документооборот с контролирующими органами. Сканированные документы для передачи в электронном виде';en='Electronic document management with regulatory authorities. Scanned documents to be sent in electronic form'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
