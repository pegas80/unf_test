...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ka='В Веб-клиенте экспорт папок не поддерживается.';ru='В Веб-клиенте экспорт папок не поддерживается.';en='The export folders is not supported by the Web client.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
...
Процедура ПапкаДляЭкспортаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Элемент.ТекстРедактирования) Тогда
		...
		Если Не Файл.Существует() Тогда
			ТекстПредупреждения = НСтр("ka='Невозможно открыть папку, так как она не существует.';ru='Невозможно открыть папку, так как она не существует.';en='Unable to open the folder because it does not exist.'");
...
"Выберите другую папку.';ka='Папка ""%1"" не найдена."
"Выберите другую папку.'"),
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Попытка
			...
			Если Не КаталогВыгрузки.Существует() Тогда
				ВызватьИсключение НСтр("ka='წარმატებული მოქმედების შემდეგ ქვესაქაღალდე არ მოიძებნა';ru='После успешного создания подпапка не найдена.';en='After successfully creating a subfolder can not be found.'");
			КонецЕсли;
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось создать подпапку ""%1"" в папке ""%2"" по причине:"
"%3';ka='Не удалось создать подпапку ""%1"" в папке ""%2"" по причине:"
"%3'"),
				Строка(ЧтоСохраняем),
				ПапкаДляЭкспорта,
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ხორციელდება საქაღალდე ""%1""-ის ექსპორტი..."
"გთხოვთ, მოიცადოთ.';ru='Выполняется экспорт папки ""%1""..."
...
Процедура ОбойтиДеревоФайлов4(ПараметрыВыполнения)
	...
	Исключение
		ТекстОшибки = НСтр("ka='საქაღალდე ""%1""-ის შექმნის შეცდომა:';ru='Ошибка создания папки ""%1"":';en='Error creating ""%1"" folder:'");
...
Процедура ОбойтиДеревоФайлов9(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() И ПараметрыВыполнения.ФайлНаДиске.ЭтоКаталог() Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ფაილ ""%1""-ის ნაცვლად არსებობს ამავე სახელწოდების საქაღალდე. გავიმეოროთ ამ ფაილის ექსპორტი?';ru='Вместо файла"
"""%1"""
		...
		Иначе
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='საქაღალდე ""%1""-ში"
"არსებობს ფაილი ""%2"""
"არსებული ფაილის ზომა = %3 ბაიტი, ცვლილების თარიღი %4."
"შესანახი ფაილის ზომა = %5 ბაიტი, ცვლილების თარიღი %6."
""
"შევცვალოთ არსებული ფაილი ფაილით ფაილების საცავიდან?';ru='В папке ""%1"""
"существует файл ""%2"""
...
Процедура ОбойтиДеревоФайлов14(ПараметрыВыполнения)
	...
	СостояниеЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='საქაღალდე ""%1""-ის ექსპორტი';ru='Экспорт папки ""%1""';en='Export folder ""%1""'"),
		ПараметрыВыполнения.ЗаписьФайла.НаименованиеПапки);
	СостояниеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Сохраняется на диск файл"
"""%1"" (%2 Мб)...';ka='დისკზე შეინახება ფაილი ""%1"" (%2 მბ)...'"),
