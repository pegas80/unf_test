...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоГруппа Тогда
		...
		Если ЭтотОбъект.Назначение <> Перечисления.НазначенияАвтоматическихСкидок.ПоВидуЦен Тогда
			ЭтотОбъект.ВидЦен = НЕОПРЕДЕЛЕНО;
		КонецЕсли;
		// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
	Если Назначение <> Перечисления.НазначенияАвтоматическихСкидок.ПоВидуЦен Тогда
...
Процедура ОбновитьИнформациюВСлужебномРегистреСведений(Отказ)
	...
	ЗаписьЖурналаРегистрации(
			НСтр("ka='ავტომატური ფასდაკლებები. სამახურებრივი ინფორმაცია ავტომატური ფასდაკლებების შესახებ';ru='Автоматические скидки. Служебная информация по автоматическим скидкам';en='Automatic discounts. Service information for automatic discounts'",
	...
	Если Отказ Тогда
		ВызватьИсключение
			НСтр("ka='ავტომატური ფასდაკლებების, ფასნამატების თაობაზე სამსახურებრივი ინფორმაციის ჩაწერა ვერ მოხერხდა."
"დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Не удалось записать служебную информацию по автоматическим скидкам, наценкам."
