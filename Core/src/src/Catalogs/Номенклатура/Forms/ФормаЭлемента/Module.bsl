...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		//+++
		Объект.СчетУчетаЗатратРегл = ПланыСчетов.Хозрасчетный.НайтиПоКоду("1630");
		//---
		...
		Если НЕ ЗначениеЗаполнено(Объект.СчетУчетаЗатрат)
			И НЕ ИспользоватьТолькоРегламентированныйПланСчетов Тогда	//+++ Дмгг
		...
		//+++ Дмгг, 05.02.24, ONESOFT-2820
		Если Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас Тогда 
			АвтоматичесткоеУстановлениеИспользоватьПартии = ПолучитьФункциональнуюОпцию("АвтоматическаяМаркировкаИспользованияПартии");
			Объект.ИспользоватьПартии = АвтоматичесткоеУстановлениеИспользоватьПартии;
		КонецЕсли;
		//--- Дмгг, 05.02.24, ONESOFT-2820
	...
	Если РольДоступна("ПолныеПрава") Или Не Константы.ФункционалПраваРедактированияНоменклатуры.Получить() Тогда
		ПравоИзмененияНаименованииНоменклатуры	= Истина;		
		ПравоИзмененияКодаНоменклатуры			= Истина;		
		ПравоИзмененияАртикулаНоменклатуры		= Истина;
		ПравоИзмененияЦеныНоменклатуры			= Истина;
		ПравоДобавленияЦеныНоменклатуры			= Истина;
	Иначе	
		ПравоИзмененияНаименованииНоменклатуры	= УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоИзмененияНаименованииНоменклатуры");		
		ПравоИзмененияКодаНоменклатуры			= УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоИзмененияКодаНоменклатуры");		
		ПравоИзмененияАртикулаНоменклатуры		= УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоИзмененияАртикулаНоменклатуры");
		ПравоИзмененияЦеныНоменклатуры			= УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоИзмененияЦеныНоменклатуры");
		ПравоДобавленияЦеныНоменклатуры			= УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ПравоДобавленияЦеныНоменклатуры");
	КонецЕсли;
	
	//+++ Дмгг
	ЗапретОтображенияЦен = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
					Пользователи.ТекущийПользователь(),
					"ЗапретНаПросмотрЦенВкарточкеНоменклатуры");
	//--- Дмгг
	
	//+ Lazare 09.11.2022
	ИспользуетсяКонтрольСроковГодностиТовара = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("КонтрольСроковГодностиТовара");	
	Элементы.ВключенВрегистр.Видимость       = ИспользуетсяКонтрольСроковГодностиТовара;
	//- Lazare 09.11.2022
	
	//Шрифт Структурной Единици(хранящая в константе)на форме номенклатури в разделе "ТекущиеОстатки" будет полужирный.
	Объект.СтруктурнаяЕдиницаНаКарточкеНоменклатури = Константы.СтруктурныеЕдиницаНаКарточкеНоменклатури.Получить();
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьТолькоПросмотрРеквизитов();
	// Конец СтандартныеПодсистемы.Свойства
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменилисьСчетаНоменклатуры" Тогда
		...
		Объект.СчетУчетаЗапасовРегл = Параметр.СчетУчетаЗапасовРегл;
		Объект.СчетУчетаЗатрат = Параметр.СчетУчетаЗатрат;
		Объект.СчетУчетаЗатратРегл = Параметр.СчетУчетаЗатратРегл;
		Объект.СчетУчетаПереработчикуРегл = Параметр.СчетУчетаПереработчикуРегл;
	...
	ИначеЕсли ИмяСобытия = "ШтрихкодыНоменклатуры" И Параметр = Объект.Ссылка Тогда
...
Процедура ТипНоменклатурыПриИзменении(Элемент)
	...
	//ПодольскийАИ #УчетПроектов 17.12.2020 {
	Объект.ИспользоватьПроект = Ложь;
	//ПодольскийАИ #УчетПроектов 17.12.2020 }
	
	//+++ Дмгг, 05.02.24, ONESOFT-2820
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Запас") Тогда 
		Объект.ИспользоватьПартии = АвтоматичесткоеУстановлениеИспользоватьПартии;
	КонецЕсли;
	//--- Дмгг, 05.02.24, ONESOFT-2820
...
Процедура СпецификацияНачалоВыбора(Элемент,  ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Сообщение.Текст = НСтр("ka='ცნობარის ელემენტი ჯერ არ ჩაწერილა.';ru='Элемент справочника еще не записан.';en='directory entry has not yet been recorded.'");
		Сообщение.Сообщить();
...
Процедура Подключаемый_ЗаголовокЗначенияЦенОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Не ПравоИзмененияЦеныНоменклатуры Тогда
		Возврат;
	КонецЕсли;	
...
&НаКлиенте
Процедура ТоварнаяНоменклатураВЭДПриИзменении(Элемент)// Kuzmich, 08.07.2020, Таможенное оформление с HS-кодами
	УправлениеФормой();
КонецПроцедуры
#КонецОбласти
...
Процедура УправлениеФормой()
	...
	Элементы.НаправлениеДеятельности.Видимость = ЭтоЗапас() Или ЭтоУслуга() Или ЭтоРабота() Или ЭтоЛицензия();
	Элементы.МетодОценки.Видимость = ЭтоЗапас();
	Элементы.СрокИсполненияЗаказа.Видимость = ЭтоЗапас() Или ЭтоУслуга() Или ЭтоРабота();
	Элементы.СтавкаНДС.Видимость = ЭтоЗапас() Или ЭтоУслуга() Или ЭтоРабота() Или ЭтоЛицензия();
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Вес",		"АвтоВыборНезаполненного",	ЗначениеЗаполнено(Объект.ТоварнаяНоменклатураВЭД));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВесНетто",	"АвтоВыборНезаполненного",	ЗначениеЗаполнено(Объект.ТоварнаяНоменклатураВЭД));
	// Kuzmich, 08.07.2020, Таможенное оформление с HS-кодами
	
	//+++ Дмгг
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АктуальныеЦены" 			,"Видимость",	НЕ ЗапретОтображенияЦен);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаЦеныНеУстановлены" 	,"Видимость",	НЕ ЗапретОтображенияЦен);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияЦеныНеУстановлены" ,"Видимость",	НЕ ЗапретОтображенияЦен);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СозданиеНовыхЦен" 			,"Видимость",	НЕ ЗапретОтображенияЦен);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДинамикаЦен" 				,"Видимость",	НЕ ЗапретОтображенияЦен);
	//--- Дмгг
	
	//ПодольскийАИ #УчетПроектов 17.12.2020 {
	УстановитьВидимостьПроектаНаКлиенте();
	//ПодольскийАИ #УчетПроектов 17.12.2020 }
...
Процедура ЗаполнитьСписокТипов()
	...
	Если Не Константы.УчетЛицензий.Получить() Тогда
		НайденнаяОперация = Элементы.ТипНоменклатуры.СписокВыбора.НайтиПоЗначению(Перечисления.ТипыНоменклатуры.Лицензия);
		Если НайденнаяОперация <> Неопределено Тогда
			Элементы.ТипНоменклатуры.СписокВыбора.Удалить(НайденнаяОперация);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьДанныеФормы(Области = Неопределено)
	...
	|	РезервированиеЗапасовОстатки.КоличествоОстаток КАК Резерв
	|ИЗ
	|	РегистрНакопления.РезервированиеЗапасов.Остатки(
	|			,
	|			Номенклатура = &Номенклатура) КАК РезервированиеЗапасовОстатки
	|,Константы КАК Константы
	|ГДЕ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ЗаполнитьДиаграммуПродажи(Данные)
	...
	Иначе
		...
		Если Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас
			...
			СерияСебестоимость.Текст = НСТР("ka='თვითღირებულება';ru='Себестоимость';en='Cost price'");//Паганини
			СерияСебестоимость.Цвет = ЦветаСерий[0];
		КонецЕсли;
		
		СерияПрибыль = ДиаграммаПродажи.Серии.Добавить("Валовая прибыль");
		СерияПрибыль.Текст = НСТР("ka='მთლიანი მოგება';ru='Валовая прибыль';en='Gross profit'");//Паганини
		...
		СерияКоличество.Текст = НСТР("ka='რაოდენობა';ru='Количество';en='Количество'");//Паганини
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ВаловаяПрибыль < 0 Тогда
				...
				Если НЕ ПоказыватьУбыток Тогда
					...
					СерияУбыток.Текст = НСТР("ka='დანაკარგი';ru='Убыток';en='Lesion'");//Паганини
...
Функция ПредставлениеДаты(Дата, Формат)
	...
	Если Формат = "ДФ=""к""" Тогда
		Возврат Формат(Дата, Формат) + " მეოთხედი " + Формат(Дата, "ДФ=""гггг""");
	Иначе
...
Процедура ИнициализироватьДиаграммуЦеныПустую()
	...
	Если ЗначениеЗаполнено(ДиаграммаЦеныПериодЗаголовок) Тогда
		ДиаграммаЗаголовок = НСтр("ka='არჩეული პერიოდის მონაცემები არ არის.';ru='Нет данных за выбранный период.';en='No data for the selected period.'");
	Иначе
		ДиаграммаЗаголовок = НСтр("ka='ფასები არ არის დაყენებული.';ru='Цены не установлены.';en='Prices have not been established.'");
	КонецЕсли;
...
Процедура ИнициализироватьДиаграммуПродажиПустую()
	...
	Если ЗначениеЗаполнено(ДиаграммаПродажиПериодЗаголовок) Тогда
		ДиаграммаЗаголовок = НСтр("ka='არჩეული პერიოდის მონაცემები არ არის.';ru='Нет данных за выбранный период.';en='No data for the selected period.'");
	Иначе
		ДиаграммаЗаголовок = НСтр("ka='გაყიდვების მონაცემები არ არის.';ru='Отсутствуют данные по продажам.';en='There are no data on sales.'");
	КонецЕсли;
...
&НаКлиенте
Функция ЭтоЛицензия()
	Возврат Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Лицензия");
КонецФункции
//+Lazare

//ПодольскийАИ #УчетПроектов 17.12.2020 {
&НаКлиенте
Процедура УстановитьВидимостьПроектаНаКлиенте()
	
	Элементы.ИспользоватьПроект.Видимость = ЭтоЗапас();
	Элементы.Проект.Видимость = Объект.ИспользоватьПроект И ЭтоЗапас();
...
&НаКлиенте
Процедура УстановитьТолькоПросмотрРеквизитов()
	СсылкаПустая = Объект.Ссылка.Пустая();
	Если НЕ СсылкаПустая Тогда
		Элементы.Наименование.ТолькоПросмотр			= НЕ ПравоИзмененияНаименованииНоменклатуры;		
		Элементы.Код.ТолькоПросмотр						= НЕ ПравоИзмененияКодаНоменклатуры;		
		Элементы.Артикул.ТолькоПросмотр					= НЕ ПравоИзмененияАртикулаНоменклатуры;
	КонецЕсли;
	//Если СсылкаПустая Тогда
		Элементы.СоздатьНовуюЦену.Доступность   		= ПравоДобавленияЦеныНоменклатуры;
	//Иначе
	//	Если ЗначениеЗаполнено(ОсновнаяЦенаПродажи) Тогда
	//		Элементы.ОсновнаяЦенаПродажи.ТолькоПросмотр	= НЕ ПравоИзмененияЦеныНоменклатуры;
	//	Иначе
	//		Элементы.ОсновнаяЦенаПродажи.ТолькоПросмотр	= НЕ ПравоДобавленияЦеныНоменклатуры;
	//	КонецЕсли;
	//КонецЕсли;
КонецПроцедуры
// Конец СтандартныеПодсистемы.Свойства
...
//ПодольскийАИ #УчетПроектов 17.12.2020 {
&НаКлиенте
Процедура ИспользоватьПроектПриИзменении(Элемент)
