...
Процедура ОповеститьОбИзмененииСчетов()
	...
	СтруктураПараметры = Новый Структура("СчетЗатрат, СчетЗатратРегл", СчетЗатрат, СчетЗатратРегл);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетЗатрат(ВидНачисленияУдержания) Тогда
		...
		Элементы.ГруппаСчетовУчета.Подсказка	= НСтр("ka='ბაზაში არის მოძრაობები დარიცხვის (დაკავების) ამ ტიპის მიხედვით! აღრიცხვის დათვლის შეცვლა აკრძალულია!';ru='В базе есть движения по этому виду начисления (удержания)! Изменение счета учета запрещено!';en='At the base there is movement on the accrual form (hold)! Change in accounting account banned!'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоНалог Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='დარიცხვის ტიპისთვის სახეობისთვის აღრიცხვის ანგარიშის გადასახადი არ რედაქტირდება!';ru='Для типа вида начисления Налог счета учета не редактируются!';en='To view type accrual accounting tax bill can not be edited!'"));
		Отказ = Истина;
...
Процедура ПоУмолчанию(Команда)
	...
	СчетЗатрат = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПустаяСсылка");
...
Процедура СчетЗатратРеглПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(СчетЗатратРегл) Тогда
		
		СчетЗатрат = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.Вспомогательный");
		
	КонецЕсли;
	РегламентированныйУчетСервер.УстановитьУпрСчетИзПланаСчетовХозрасчетный(СчетЗатратРегл, СчетЗатрат);
	ОповеститьОбИзмененииСчетов();
