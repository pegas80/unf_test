...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если НЕ ЗначениеЗаполнено(НоменклатураВладелец)
			...
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ka='სერიული ნომრები შენახულია მხოლოდ მარაგებისთვის';ru='Серийные номера хранятся только для запасов';en='Serial numbers are only stored for stocks'");
...
Процедура ПолученыШтрихкоды(ДанныеШтрикодов) Экспорт
	...
	Если НЕ ИспользоватьСерийныеНомера Тогда
		...
		Сообщение.Текст = НСтр("ka='ნომენკლატურისთვის სერიული ნომრების მიხედვით აღრიცხვა არ წარმოებს! ნომენკლატურის ბარათში დააყენეთ ალამი ""სერიული ნომრების გამოყენება""';ru='Для номенклатуры не ведется учет по серийным номерам!"
...
Функция СоздатьСерйныйНомер(СерийныйНомерСтрока, НоменклатураВладелец)
	...
	Попытка
		...
		СтрокаСообщения = НСтр("ka='შექმნილია სერიული ნომერი: %1%';ru='Создан серийный номер: %1%';en='Created by serial number:% 1%'");
...
Функция ПолучитьСерийныйНомерПоШтрихкоду(ДанныеШтрихкода, НоменклатураВладелец)
	...
	Иначе	
		СтрокаСообщения = НСтр("ka='შეყვანილი შტრიხკოდი %1% მიბმულია სხვა ნომენკლატურაზე (სერიულ ნომერზე): %2%';ru='Введенный штрихкод %1% привязан к другой номенклатуре (серийному номеру): %2%';en='Introduced barcode% 1% bound to another nomenclature (serial number):% 2%'");
