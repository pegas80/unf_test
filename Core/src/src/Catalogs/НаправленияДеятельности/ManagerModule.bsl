...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Номенклатура") Тогда
		...
		Если НЕ Параметры.Номенклатура.СчетУчетаЗатрат.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
			...
			ТекстСообщения = НСтр("ka='ამ ხარჯისთვის არ შეიძლება საქმიანობის მიმართულების არჩევა!';ru='Для этого расхода нельзя выбрать направление деятельности!';en='For this flow can not choose the direction of activity!'");
	...
	Если Параметры.Свойство("СчетЗатрат") Тогда
		...
		Если ЗначениеЗаполнено(Параметры.СчетЗатрат) Тогда
			...
			Если Параметры.СчетЗатрат.ТипСчета = Перечисления.ТипыСчетов.ПрочиеРасходы Тогда
				...
				ТекстСообщения = НСтр("ka='ისეთი ტიპის ანგარიშისთვის, როგორიც არის ""სხვა დანახარჯები"" საქმიანობის მიმართულება არ მიეთითება!';ru='Для счета типа ""Прочие расходы"" направление деятельности не указывается!';en='For account of ""Other expenses"" line of action is not specified!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(, ТекстСообщения);
			...
			ИначеЕсли Параметры.СчетЗатрат.ТипСчета = Перечисления.ТипыСчетов.НезавершенноеПроизводство
				...
				ТекстСообщения = НСтр("ka='ისეთი ტიპის ანგარიშისთვის, როგორიც არის ""დაუსრულებელი წარმოება"" ან ""ირიბი დანახარჯები"" საქმიანობის მიმართულება არ მიეთითება!';ru='Для счета типа ""Незавершенное производство"" или ""Косвенные затраты"" направление деятельности не указывается!';en='For the type of account ""Work in progress"" or ""indirect costs"" activity is not specified!'");
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(, ТекстСообщения);
