...
Процедура УправлениеСвойствомДоступность()
	...
	// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
	Если Объект.ИмяИсточника = "Справочники.ВидыРабочегоВремени.Сверхурочные" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Источник", "Доступность", ИСТИНА);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроизвольныйЗапрос", "Доступность", ИСТИНА);
	КонецЕсли;
	// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
...
Процедура ЗаполнитьПоШаблону(ДополнительныеПараметры)
	...
	// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
	ИначеЕсли ИмяШаблона = "СверхурочныеЧасы" Тогда
		Объект.Наименование           = "Сверхурочные часы";
		Объект.ПредставлениеИсточника = "Сверхурочные часы";
		Объект.ИмяИсточника           = "Справочники.ВидыРабочегоВремени.Сверхурочные";
		ЗаполнитьКакСправочникиВидыРабочегоВремениСверхурочные();
	// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
	//+++ Дмгг, 26.11.2025, ONESOFT-4497
	ИначеЕсли ИмяШаблона = "НормаЧасовПоГрафикуРаботы" Тогда
 
		// Норма часов по графику работы
		Объект.Наименование				= "Норма часов по графику работы";
		Объект.Идентификатор 			= "НормаЧасовПоГрафикуРаботы";
		Объект.ПроизвольныйЗапрос		= Истина;
		Объект.ЗадаватьЗначениеПриРасчетеЗП = Ложь;
...
Функция ЗапросКорректный()
	...
	Попытка
		...
		Если ПостроительЗапроса.Измерения.Количество() > 0 Тогда
			...
			Сообщение.Текст = НСтр("ka='მოთხოვნა არ უნდა შეიცავდეს ჯამებს!';ru='Запрос не должен содержать итоги!';en='Query can not contain the results!'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли; 
		...
		Если ПостроительЗапроса.ВыбранныеПоля.Количество() > 1 Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='მოთხოვნა უნდა შეიცავდეს არა უმეტეს ერთ მაჩვენებელს!';ru='Запрос должен содержать не более одного показателя!';en='The request must contain no more than one record!'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
...
"Измените наименование параметра';ka='ასეთი სახელწოდების მქონე გამოანგარიშების პარამეტრი უკვე არსებობს!"
"შეცვალეთ პარამეტრის სახელწოდება'");
...
Функция ПроверкаНаВыборПоказателя()
	...
	Сообщение = Новый СообщениеПользователю();
	Сообщение.Текст = НСтр("ka='არ არის არჩეული მაჩვენებელი!';ru='Не выбран показатель!';en='Not figure out!'");
	Сообщение.Сообщить();
...
Процедура ЗаполнитьПоказателиИПериодыОтбораДанныхИсточника()
	...
	// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
	Если СтрНайти(ВидИсточникаПоМетаданным,"Справочник")>0 Тогда
		Возврат;
	КонецЕсли;
	// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
	...
	Если Не Объект.ПроизвольныйЗапрос Тогда
		...
		Для каждого СтрокаПараметров Из Объект.ПараметрыЗапроса Цикл
			...
			Если СтрокаПараметров.Имя <> "МоментВремени"
				...
				Иначе
				    ЭлементОтбора.ВидСравнения = ОбщегоНазначения.ПолучитьВидСравненияГруз(СтрокаПараметров.ВидСравнения);
				КонецЕсли; 
...
Процедура РедактироватьЗапрос(Команда)
	...
	Если Элементы.РедактироватьЗапрос.Пометка Тогда
		...
		Иначе
			...
			ТекстВопроса = НСтр("ka='მოთხოვნა შეიცავს შეცდომას! გავწმინდოთ მოთხოვნა და  საწყისს დავუბრუნდეთ?';ru='Запрос содержит ошибку! "
...
Процедура ЗадаватьЗначениеПриРасчетеЗППриИзменении(Элемент)
	...
	Если Объект.ЗадаватьЗначениеПриРасчетеЗП Тогда
		...
		ТекстСообщения = НСтр("ka='ალმის დაყენების შემდეგ ყველა რეკვიზიტები გაიწმინდება? გავაგრძელოთ?';ru='После установки флага все реквизиты будут очищены! Продолжить?';en='After installing all the details of the flag will be cleared! Proceed?'");
...
//+++ Дмгг, 26.11.2025, ONESOFT-4497
&НаКлиенте
Процедура ШаблонНормаЧасовПрГрафикуРаботы(Команда)
	
	ИнициализироватьЗаполнениеПоШаблону("НормаЧасовПоГрафикуРаботы");
...
Процедура ИнициализироватьЗаполнениеПоШаблону(ИмяШаблона)
	...
	ТекстВопроса = НСтр("ka='გაანგარიშების პარამეტრი სრულად იქნება გადავსებული! გავაგრძელოთ ოპერაციის შესრულება?';ru='Параметр расчета будет полностью перезаполнен! Продолжить выполнение операции?';en='the calculation of the parameter will be fully refilled! To continue the operation?'");
...
// Kuzmich, 12.10.2023, (ONESOFT-2531) Добавление показателья "Сверхурочние часы" в формуле расчета начисление
Процедура ЗаполнитьКакСправочникиВидыРабочегоВремениСверхурочные()
	
	Объект.ПроизвольныйЗапрос = ИСТИНА;
	
	Объект.Идентификатор 	  = "СверхурочныеЧасы";
	Объект.ЗадаватьЗначениеПриРасчетеЗП = Ложь;
	НовыйПараметрЗапроса 						 = Объект.ПараметрыЗапроса.Добавить();
	НовыйПараметрЗапроса.Имя 					 = "Организация";
	НовыйПараметрЗапроса.Представление 			 = "Организация";
	...
	НовыйПараметрЗапроса.Представление 			 = "Сотрудник";
	НовыйПараметрЗапроса 						 = Объект.ПараметрыЗапроса.Добавить();
	НовыйПараметрЗапроса.Имя 					 = "СтруктурнаяЕдиница";
	НовыйПараметрЗапроса.Представление 			 = "Подразделение";
	
	Объект.Запрос 			 = 
	"ВЫБРАТЬ
	|	ТабельОбороты.ЧасовОборот КАК СверхурочныеЧасы
	|ИЗ
	...
	|				И ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыРабочегоВремени.Сверхурочные)) КАК ТабельОбороты
	|ГДЕ
	|	ВЫБОР
	|			КОГДА НЕ &СтруктурнаяЕдиница = ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	|					И НЕ &СтруктурнаяЕдиница = НЕОПРЕДЕЛЕНО
	|				ТОГДА ТабельОбороты.СтруктурнаяЕдиница = &СтруктурнаяЕдиница
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ";
...
&НаКлиенте
Процедура ШаблонСверхурочныеЧасы(Команда)
	
	ИнициализироватьЗаполнениеПоШаблону("СверхурочныеЧасы");
