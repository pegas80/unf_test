...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების დასამატებლად არასაკმარისი დაშვების უფლებებია.';ru='Недостаточно прав доступа для добавления дополнительных отчетов или обработок.';en='It is not enough access rights to add additional reports or treatments.'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение НСтр("ka='დამატებითი ანგარიშგებების და დამუშავებების ჩასატვირთად არასაკმარისი უფლებებია';ru='Недостаточно прав для загрузки дополнительных отчетов и обработок';en='Not have permission to upload additional reports and treatments'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების წინასწარ განსაზღვრული ვარიანტის წასაშლელად მონიშვნა აკრძალულია.';ru='Пометка на удаление предопределенного варианта отчета запрещена.';en='Mark on removal of a predetermined version of the report is prohibited.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მოვნიშნოთ ""%1"" წასაშლელად?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for removal?'"), Вариант.Наименование);
...
Процедура ВариантыДополнительногоОтчетаОткрыть(Команда)
	...
	Если Вариант = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ანგარიშგების ვარიანტი.';ru='Выберите вариант отчета.';en='Select the report.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='ბეჭდვის ბრძანებების ხილვადობის გასამართად, აუცილებელია დამუშავების ჩაწერა. გავაგრძელოთ?';ru='Для настройки видимости команд печати обработку необходимо записать. Продолжить?';en='To configure the print instruction processing appear to be written. Proceed?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Продолжить", НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
...
Процедура ВыполнитьКоманду(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		ТекстВопроса = НСтр("ka='Для выполнения команды необходимо записать данные.';ru='Для выполнения команды необходимо записать данные.';en='To execute the command you want to record data.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ЗаписатьИПродолжить", НСтр("ka='ჩავიწეროთ და გავაგრძელოთ';ru='Записать и продолжить';en='Record and continue'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = НСтр("ka='ღია ფანჯრებში ცვლილებების გამოსაყენებლად აუცილებელია მათი დახურვა და ხელახლა გახსნა.';ru='Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.';en='To apply the changes you need to open windows to close them and reopen.'");
...
Процедура ОбновлениеИзФайлаПослеПодтверждения(Ответ, ПараметрыРегистрации) Экспорт
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыДиалога.Заголовок = НСтр("ka='აირჩიეთ გარე ანგარიშგების ან დამუშავების ფაილი';ru='Выберите файл внешнего отчета или обработки';en='Select external report or file processing'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыДиалога.ИндексФильтра = 1;
		ПараметрыДиалога.Заголовок = НСтр("ka='აირჩიეთ გარე ანგარიშგების ფაილი';ru='Выберите файл внешнего отчета';en='Select an external report file'");
	Иначе
		ПараметрыДиалога.ИндексФильтра = 2;
		ПараметрыДиалога.Заголовок = НСтр("ka='აირჩიეთ გარე დამუშავების ფაილი';ru='Выберите файл внешней обработки';en='Select external processing file'");
	КонецЕсли;
...
Процедура ОбновлениеИзФайлаПослеВыбораФайла(ПомещенныеФайлы, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ka='ფაილის გაფართოება არ შეესაბამება გარე ანგარიშგების (ERF) ან დამუშავების (EPF) გაფართოებას.';ru='Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).';en='Expanding file does not match the expansion of an external report (ERF) or processing (EPF).'");
...
Процедура ОбновлениеИзФайлаМеханикаНаКлиенте(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.Успех Тогда
		ОповещениеЗаголовок = ?(ПараметрыРегистрации.ЭтоОтчет, НСтр("ka='გარე ანგარიშგების ფაილი ჩატვირთულია';ru='Файл внешнего отчета загружен';en='external report file is loaded'"), НСтр("ka='გარე დამუშავების ფაილი ჩატვირთულია';ru='Файл внешней обработки загружен';en='external processing file is loaded'"));
	...
	Иначе
		...
		Иначе
			...
			Если ПараметрыРегистрации.ЭтоОтчет Тогда
				ЗаголовокОшибки = НСтр("ka='ანგარიშგება არ არის შეერთებული';ru='Отчет не подключен';en='The report is not connected'");
			Иначе
				ЗаголовокОшибки = НСтр("ka='დამუშავება არ არის შეერთებული';ru='Обработка не подключена';en='Processing is not connected'");
			КонецЕсли;
...
Процедура ОбновлениеИзФайлаПоказатьКонфликты(ПараметрыРегистрации)
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ka='კონფლიქტები გარე ანგარიშგების ჩატვირთვისას';ru='Конфликты при загрузке внешнего отчета';en='Conflicts when loading external report'");
			ТекстВопроса = НСтр("ka='ანგარიშგების შიდა სახელი ""[Name]"""
"უკვე დაკავებულია არსებული დამატებითი ანგარიშგებებით ([Count]): "
"[List]."
""
"აირჩიეთ:"
"1. ""[Continue]"" - ახალი ანგარიშგების ჩატვირთვა გასწორების რეჟიმში."
"2. ""[Disable]"" - ახალი ანგარიშგების ჩატვირთვა ყველა კონფლიქტური ანგარიშგების გამოქვეყნების გამორთვით."
"3. ""[Open]"" - ჩატვირთვის გაუქმება და კონფლიქტური ანგარიშგებების სიის ჩვენება.';ru='Внутреннее имя отчета ""[Name]"""
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих отчетов.'");
		Иначе
			ЗаголовокВопроса = НСтр("ka='კონფლიქტები გარე ანგარიშგების ჩატვირთვისას';ru='Конфликты при загрузке внешней обработки';en='Conflicts at boot external processing'");
			ТекстВопроса = НСтр("ka='დამუშავების შიდა სახელი ""[Name]"""
"უკვე დაკავებულია არსებული დამატებითი დამუშავებებით ([Count]):"
"[List]."
""
"აირჩიეთ:"
"1. ""[Continue]"" - ახალი დამუშავების ჩატვირთვა გასწორების რეჟიმში."
"2. ""[Disable]"" - ახალი დამუშავების ჩატვირთვა, ყველა კონფლიქტური დამუშავების გამოქვეყნების გამორთვით."
"3. ""[Open]"" - ჩატვირთვის გაუქმება და კონფლიქტური დამუშავებების სიის ჩვენება.';ru='Внутреннее имя обработки ""[Name]"""
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих обработок.'");
		КонецЕсли;
		ПредставлениеКнопкиОтключить = НСтр("ka='კონფლიქტურების გამორთვა';ru='Отключить конфликтующие';en='disable conflicting'");
		ПредставлениеКнопкиОткрыть = НСтр("ka='გაუქმება და სიის ჩვენება';ru='Отменить и показать список';en='Cancel and show the list'");
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ka='კონფლიქტი გარე ანგარიშგების ჩატვირთვისას';ru='Конфликт при загрузке внешнего отчета';en='Conflict when loading external report'");
			ТекстВопроса = НСтр("ka='ანგარიშგების შიდა სახელი ""[Name]"""
"უკვე დაკავებულია არსებული დამატებითი ანგარიშგებით [List]."
""
"აირჩიეთ:"
"1. ""[Continue]"" - ახალი ანგარიშგების ჩატვირთვა გასწორების რეჟიმში."
"2. ""[Disable]"" - ახალი ანგარიშგების ჩატვირთვა კონფლიქტური ანგარიშგების გამოქვეყნების გამორთვით."
"3. ""[Open]"" - კონფლიქტური ანგარიშგების ბარათის გახსნა.';ru='Внутреннее имя отчета ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующего отчета.'");
			ПредставлениеКнопкиОтключить = НСтр("ka='სხვა ანგარიშგების გამორთვა';ru='Отключить другой отчет';en='Disable other report'");
		Иначе
			ЗаголовокВопроса = НСтр("ka='კონფლიქტი გარე დამუშავების ჩატვირთვისას';ru='Конфликт при загрузке внешней обработки';en='Conflict handling external loading'");
			ТекстВопроса = НСтр("ka='დამუშავების შიდა სახელი ""[Name]"""
"უკვე დაკავებულია არსებული დამატებითი დამუშავებით [List]."
""
"აირჩიეთ:"
"1. ""[Continue]"" - ახალი დამუშავების ჩატვირთვა გასწორების რეჟიმში."
"2. ""[Disable]"" - ახალი დამუშავების ჩატვირთვა კონფლიქტური დამუშავების გამოქვეყნების გამორთვით."
"3. ""[Open]"" - კონფლიქტური დამუშავების ბარათის გახსნა.';ru='Внутреннее имя обработки ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующей обработки.'");
			ПредставлениеКнопкиОтключить = НСтр("ka='სხვა დამუშავების გამორთვა';ru='Отключить другую обработку';en='Disable other processing'");
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("ka='გაუქმება და გახსნა';ru='Отменить и открыть';en='Cancel and open'");
	КонецЕсли;
	ПредставлениеКнопкиПродолжить = НСтр("ka='გასწორების რეჟიმში';ru='В режиме отладки';en='In debug mode'");
...
Процедура ОбновлениеИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები შიდა სახელით ""%1""';ru='Дополнительные отчеты и обработки с внутреннем именем ""%1""';en='Additional reports and processing with the internal name ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგების ვარიანტი ""%1"" არ არის რეგისტრირებული.';ru='Вариант отчета ""%1"" не зарегистрирован.';en='report option ""%1"" is not registered.'"), Вариант.Наименование);
...
Процедура ИзменитьРегламентноеЗадание(РежимВыбора = Ложь, ИзмененФлажок = Ложь)
	...
	Если ЭлементКоманда.ВариантЗапуска <> ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		ТекстОшибки = НСтр("ka='ბრძანება გაშვების ვარიანტ ""%1""-ით არ შეიძლება, რომ გამოყენებული იქნას სარეგლამენტო დავალებებში.';ru='Команда с вариантом запуска ""%1"""
...
Процедура ВыполнитьКомандуПослеПодтвержденияЗаписи(Ответ, Контекст) Экспорт
	...
	ИначеЕсли СтрокаТаблицыКоманд.ВариантЗапуска = ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		ЗаголовокСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выполняется команда ""%1""';en='The command ""%1""'"),
...
Процедура ПослеЗавершенияВыполненияСервернойКомандыВФоне(Задание, ВыполняемаяКоманда) Экспорт
	Если Задание.Статус = "Ошибка" Тогда
		...
		ВводныйТекст = НСтр("ka='ბრძანების შესრულება ვერ მოხერხდა.';ru='Не удалось выполнить команду.';en='Failed to execute the command.'");
...
Функция ПредставлениеОтключенногоРасписания()
	Возврат НСтр("ka='განრიგი მოცემული არ არის';ru='Расписание не задано';en='Schedule is not specified'");
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеБыстрогоДоступаПользователей(КоличествоПользователей)
	Если КоличествоПользователей = 0 Тогда
		Возврат НСтр("ka='არა';ru='Нет';en='Not'");
	КонецЕсли;
	
	БыстрыйДоступПредставление = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		КоличествоПользователей,
		НСтр("ka='მომხმარებელი:';ru='пользователь';en='user'") + "," + НСтр("ka='მომხმარებელს';ru='пользователя';en='user'") + "," + НСтр("ka='მომხმარებლებს';ru='пользователей';en='users'"));
...
Функция НачатьВыполнениеСервернойКомандыВФоне(ВыполняемаяКоманда, УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები: დამუშავების სერვერული მეთოდის შესრულება';ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки';en='Additional reports and processing: Perform server processing method'");
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ka='დამატებითი დამუშავების დანიშნულება';ru='Назначение дополнительной обработки';en='Appointment of additional processing'"));
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если Объект.Вид = ВидДополнительныйОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = НСтр("ka='ანგარიშგების ვარიანტები';ru='Варианты отчета';en='options report'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგების ვარიანტები (%1).';ru='Варианты отчета (%1)';en='Embodiments of the report (1%)'"),
	...
	Иначе
		...
		Элементы.СтраницаРазрешения.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ნებართვები (%1)';ru='Разрешения (%1)';en='Resolution (1%)'"),
		...
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
			...
			Элементы.ДекорацияПерсональныйПрофильБезопасностиНадпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დამატებითი ანგარიშგება ან დამუშავება პროგრამაზე შეერთებული იქნება უსაფრთხოების ""პერსონალური"" პროფილ %1-ის გამოყენებით, რომელშიც ნებადართული იქნება მხოლოდ შემდეგი ოპერაციები:';ru='Дополнительный отчет или обработка будет подключаться к программе с использованием ""персонального"""
"профиля безопасности %1, в котором будут разрешены только следующие операции:'"),
				БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='%1 არ წარმოადგენს შეერთების კორექტულ რეჟიმს დამატებითი ანგარიშგებებისთვის და დამუშავებებისთვის, რომლებიც უსაფრთხოების პროფილების გამოყენების ნებართვას ითხოვს!';ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!';en='% 1 is not a valid connection mode for additional reports and treatments requiring the use of safety profiles permission!'"),
				БезопасныйРежим);
		КонецЕсли;
	...
	Если ЭтоГлобальнаяОбработка Тогда
		ПредставлениеНазначения = НСтр("ka='<Не задано>';ru='<Не задано>';en='<Not specified>'");
	...
	Иначе
		Если Объект.Назначение.Количество() = 0 Тогда
			ПредставлениеНазначения = НСтр("ka='გაურკვეველია';ru='Не определено';en='Undefined'");
		Иначе
			ПредставлениеНазначения = "";
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("ka='დამატებითი ანგარიშგება (შექმნა)';ru='Дополнительный отчет (создание)';en='An additional report (creation)'"), НСтр("ka='დამატებითი დამუშავება (შექმნა)';ru='Дополнительная обработка (создание)';en='Additional processing (creation)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, НСтр("ka='(დამატებითი ანგარიშგება)';ru='(Дополнительный отчет)';en='(Additional report)'"), НСтр("ka='(დამატებითი დამუშავება)';ru='(Дополнительная обработка)';en='(General processing)'"));
	КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("ka='ცვლის ბეჭდვის სტანდარტულ ბრძანებას %1';ru='Заменяет стандартную команду печати %1';en='It replaces the standard print command% 1'");
					Иначе
						ШаблонКомментария = НСтр("ka='ელემენტი ამობეჭდვის სტანდარტული გამართვები %1';ru='Заменяет стандартные команды печати: %1';en='It replaces the standard print command% 1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='გაშვების ვარიანტ ""%1""-ის მქონე ბრანებებისთვის გამოუყენებელია';ru='Не применимо для команд с вариантом запуска ""%1""';en='Not applicable for commands to launch one ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = НСтр("ka='ანგარიშგების ვარიანტების სიის მიღება ვერ მოხერხდა ამ ანგარიშგების ჩართვისას წარმოშობილი მიზეზის გამო:';ru='Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:';en='Failed to get a list of options for the report due to an error that occurred when connecting this report:'");
...
Функция НазваниеСтраницыКоманд()
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		Возврат НСтр("ka='ბეჭდვის ბრძანებები';ru='Команды печати';en='Команды печати'");
	Иначе
		Возврат НСтр("ka='ბრძანებები';ru='Команды';en='commands'");
	КонецЕсли;
КонецФункции
