...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ka='ანგარიშგებების დაგზავნა. ელემენტის ფორმის გახსნა';ru='Рассылка отчетов. Открытие формы элемента';en='Reports Newsletter. Opening a form element'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
"Пользователь ""%1"" (%2) не может быть автором.';ka='დაგზავნის ავტორის შევსების შეცდომა:"
"მომხმარებელი ""%1"" (%2) არ შეიძლება, რომ ავტორი იყოს.'"),
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='სახელწოდება არ არის შეყვანილი';ru='Не введено наименование';en='Do not put the name'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='მიმღებების ტიპი არ არის არჩეული';ru='Не выбран тип получателей';en='recipient type is not selected'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='არცერთი ანგარიშგება არ არის არჩეული';ru='Не выбрано ни одного отчета';en='You have no report'");
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ka='გაშვების პერიოდულობა არ არის არჩეული';ru='Не выбрана периодичность запуска';en='Not selected frequency of the launch'");
		...
		Если Объект.ИспользоватьFTPРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.FTPСервер)
				...
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='FTP მისამართი არ არის შეყვანილი';ru='Не введен FTP адрес';en='Do not Enter the FTP address'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='Windows-ის ქსელური კატალოგი არ არის შეყვანილი';ru='Не введен сетевой каталог Windows';en='Do not put the Windows network directory'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='ქსელური კატალოგი Linux-ი არ არის შეყვანილი';ru='Не введен сетевой каталог Linux';en='Do not put Linux network directory'");
		...
		Если Объект.ИспользоватьПапку Тогда
			Если Не ЗначениеЗаполнено(Объект.Папка) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='საქაღალდე არ არის არჩეული';ru='Не выбрана папка';en='Folder is not selected'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ka='საფოსტო მისამართი არ არის არჩეული';ru='Не выбран почтовый адрес';en='mail address is not selected'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ka='მიმღებების საფოსტო მისამართის ტიპი არ არის არჩეული';ru='Не выбран тип почтового адреса получателей';en='type of mail addresses of the recipients are not selected'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ka='გასაგზავნად სააღრიცხვო ჩანაწერი არ არის არჩეული';ru='Не выбрана учетная запись для отправки';en='Do not choose an account to send'");
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka=""ანგარიშგება '%1'-ისთვის სავალდებულოდ შესავსები ყველა გამართვა არ არის შევსებული. საჭიროა ყველა სავალდებულო გამართვის შევსება ან ალამ 'მომზადებულია'-ს მოხსნა."";ru=""Для отчета '%1' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок 'Подготовлена'."";en=""For the record '% 1' is not filled in all the settings required. Please fill in all the required settings and uncheck the 'prepared'."""),
		...
		Если ПроверитьПерсонализированные Тогда
			...
			Если ОтчетПерсонализирован Тогда
				Отказ = Истина;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka=""ანგარიშგება '%1'-ისთვის მიცემულია შერჩევა დაგზავნის მიმღებების მიხედვით. აუცილებელია მიმღებების მიხედვით შერჩევის გამორთვა ან დაგზავნის ტიპის შეცვლა შემდეგით 'საკუთარი ანგარიშგება ყოველი მიმღებისთვის'."";ru=""Для отчета '%1' задан отбор по получателям рассылки. Необходимо отключить отбор по получателям или изменить вид рассылки на 'Свой отчет для каждого получателя'."";en='For the record ''% 1'' is set to selection of recipients lists. You must disable the selection of recipients or change the view of the distribution ''His report for each recipient. ""'"),
					Строка(СтрокаОтчеты.Отчет));
				Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
			КонецЕсли;
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		СтрокаВопроса = НСтр("ka='გასაგრძელებლად საჭიროა მიმღებების სიის გასუფთავება.';ru='Для продолжения необходимо очистить список получателей.';en='To continue, you must clear the list of recipients.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='გასუფთავება';ru='Очистить';en='clean'"));
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ka='<Укажите получателя>';ru='<Укажите получателя>';en='<Specify the recipient>'"));
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = НСтр("ka='საქაღალდე ""%1""-ის ფაილების შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения файлов папки ""%1"".';en='Not allowed to edit files in the folder ""%1"".'");
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='არჩეული ჯგუფი გამოიყენება მხოლოდ პირადი დაგზავნისთვის ელექტრონული ფოსტით';ru='Выбранная группа используется только для личных рассылок по электронной почте';en='The selected group is only used for personal email lists'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = НСтр("ka='შეცდომების გამო ანგარიშგების დამატება ვერ მოხერხდა';ru='Не удалось добавить отчет из-за ошибок';en='Failed to add the report due to errors'");
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = НСтр("ka='შეცდომების გამო ანგარიშგებების დამატება ვერ მოხერხდა';ru='Не удалось добавить отчеты из-за ошибок';en='Failed to add the reports due to errors'");
		ИначеЕсли СтруктураВыбора.Выбрано.Количество >= 6 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დამატებულია ანგარიშგება: %2-დან %1, არ არის დამატებული შეცდომების გამო: %3';ru='Добавлено отчетов: %1 из %2,"
...
"%2';ka='წარმატებით დაემატა ანგარიშგება:"
"%1"
""
"შემდეგი ანგარიშგებების დამატება ვერ მოხერხდა შეცდომების გამო:"
"%2'"),
		...
		ПараметрыПредупреждения.Заголовок = НСтр("ka='დაგზავნაში ანგარიშგებების დამატების შეცდომა';ru='Ошибки добавления отчетов в рассылку';en='adding reports errors in the newsletter'");
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("ka='ყველა არჩეული ანგარიშგება უკვე შეტანილია დაგზავნაში';ru='Все выбранные отчеты уже включены в рассылку';en='All selected reports are already included in the newsletter'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("ka='ანგარიშგება შეყვანილია დაგზავნაში';ru='Отчет включен в рассылку';en='The report is included in the newsletter'");
			Иначе
				ЗаголовокОповещения = НСтр("ka='ანგარიშგებები შეტანილია დაგზავნაში';ru='Отчеты включены в рассылку';en='Reports included in the newsletter'");
			КонецЕсли;
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = НСтр("ka='მიმღებების შესაყვანად აუცილებელია მათი ტიპის არჩევა';ru='Для ввода получателей необходимо выбрать их тип';en='To enter the recipient must select their type'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, НСтр("ka='დაგზავნა არ არის მომზადებული';ru='Рассылка не подготовлена';en='Newsletter not prepared'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	СтрокаОтчеты = Элементы.Отчеты.ТекущиеДанные;
	Если СтрокаОтчеты = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ka='შეარჩიეთ ანგარიშგება';ru='Выберите отчет';en='Select a report'"));
		Возврат;
	КонецЕсли;
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("ka='მიმღების გამოყენება პარამეტრებში შესაძლებელია მხოლოდ დაგზავნის სახეობა ""%1""-სთვის.';ru='Использовать получателя в параметрах возможно только для вида рассылки ""%1""';en='Use recipient parameters is possible only to send the form ""%1""'");
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ველი ""მიმღებები"" არ არის შევსებული';ru='Поле ""Получатели"" не заполнено';en='The ""recipients"" not filled'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеИОМД.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='მიმღებების ტიპი ვერ მოიძებნა';ru='Тип получателей не найден';en='recipient type can not be found'"));
		Возврат;
	КонецЕсли;
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ანგარიშგების გამართვა';ru='Выберите настройку отчета';en='Select the report setting'"));
			Возврат;
		КонецЕсли;
		...
		Если СтрокаНастроек = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების გამართვა ვერ მოიძებნა';ru='Настройка отчета не найдена';en='Setting the report not found'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='მიმღების მითითება შეიძლება მხოლოდ ანგარიშგებების პარამეტრებისთვის და შერჩევებისთვის';ru='Указывать получателя можно только для параметров и отборов отчетов';en='The recipient can specify only the settings and selections reports'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების გასამართად ხელმისაწვდომი ველის აღწერილობა ვერ მოიძებნა';ru='Для настройки отчета не найдено описание доступного поля';en='To customize the report found the description of the available fields'"));
					Возврат;
				КонецЕсли;
...
"Необходимо выбрать другой тип получателей или другую настройку.';ka='ტიპი ""%1"" არჩეულ გამართვას ტიპით არ ესადაგება. აუცილებელია მიმღებების სხვა ტიპის ან სხვა გამართვის არჩევა.'");
...
Процедура ЗаполнитьРасписаниеПоШаблону(Команда)
	...
	СписокВариантов.ПоказатьВыборЭлемента(Обработчик, НСтр("ka='აირჩიეთ განრიგის შაბლონი';ru='Выберите шаблон расписания';en='Select the template schedules'"));
КонецПроцедуры
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='შაბლონის ტექსტში მიმღების გამოყენება შესაძლებელია მხოლოდ დაგზავნის ტიპ ""%1""-ისთვის';ru='Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""';en='Using a recipient template text is only possible to send the form ""%1""'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = НСтр("ka='წერილის თემა უკვე შეესაბამება შაბლონს სტანდარტულად.';ru='Тема письма уже соответствует шаблону по умолчанию.';en='The theme of the letter is already corresponds to the default template.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='წერილის ტექსტი უკვე შეესაბამება შაბლონს სტანდარტულად.';ru='Текст письма уже соответствует шаблону по умолчанию.';en='Text of the letter already corresponds to the default template.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = НСтр("ka='წერილის თემაში სტანდარტულად შაბლონის დამატება';ru='Добавить в тему письма шаблон по умолчанию';en='Add in the subject line of the default template'");
			ТекстВопроса = НСтр("ka='წერილის თემა შევცვალოთ შაბლონით სტანდარტულად?';ru='Заменить тему письма на шаблон по умолчанию?';en='Replace the subject line to the default template?'");
		Иначе
			ЗаголовокВопроса = НСтр("ka='წერილის ტექსტში შაბლონის დამატება სტანდარტულად';ru='Добавить в текст письма шаблон по умолчанию';en='Add in the text of the letter the default template'");
			ТекстВопроса = НСтр("ka='წერილის ტექსტი შევცვალოთ შაბლონით სტანდარტულად?';ru='Заменить текст письма на шаблон по умолчанию?';en='Replace the text of the letter to the default template?'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ka='შეცვლა';ru='Заменить';en='replace the'"));
		Кнопки.Добавить(2, НСтр("ka='დამატება';ru='Добавить';en='Add'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ka='ტესტური შეტყობინება 1С:საწარმო';ru='Тестовое сообщение 1С:Предприятие';en='The test message 1C: Enterprise'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru='Это сообщение отправлено системой рассылок 1С:Предприятие.';en='This message was sent mailings system 1C: Enterprise.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = НСтр("ka='აირჩიეთ ფორმატები სტანდარტულად';ru='Выберите форматы по умолчанию';en='Select the default format'");
	Иначе
		ЗаголовокДиалога = НСтр("ka='ანგარიშგება ""%1""-სთვის აირჩიეთ ფორმატები';ru='Выберите форматы для отчета ""%1""';en='Select the format for the report ""%1""'");
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		ЗаголовокВопроса = НСтр("ka='მიწოდების მეთოდის შემოწმება';ru='Проверка способа доставки';en='Checking the method of delivery'");
		Если Не Объект.Подготовлена Тогда
...
"Нажмите ""Продолжить"", чтобы записать рассылку.';ka='შემოწმებამდე დაგზავნა უნდა ჩაიწეროს. დაგზავნის ჩასაწერად დააჭირეთ ""გაგრძელებას"".'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				НСтр("ka='ჯგუფი დაყენებულია დაგზავნის ტიპის შესაბამისად';ru='Группа установлена в соответствии с видом рассылки';en='Group established in accordance with the distribution view'"));
	...
	Если Изменения = "" Или Изменения = "Отчеты" Тогда
		...
		Если КоличествоОтчетов > 0 Тогда
			Элементы.СтраницаОтчеты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ანგარიშგებები (%1)';ru='Отчеты (%1)';en='Reports (1%)'"), 
				Формат(КоличествоОтчетов, "ЧН=0; ЧГ="));
		Иначе
			Элементы.СтраницаОтчеты.Заголовок = НСтр("ka='ანგარიშგებები';ru='Отчеты';en='Reports'") ;
		КонецЕсли;
	КонецЕсли;
	...
	Если Изменения = ""
		...
		СпособыДоставки = "";
		Если Объект.ИспользоватьПапку Тогда
			СпособыДоставки = НСтр("ka='საქაღალდე';ru='папка';en='folder'");
		КонецЕсли;
		Если Объект.ИспользоватьСетевойКаталог Тогда
			СпособыДоставки = НСтр("ka='ქსელური კატალოგი';ru='сетевой каталог';en='network directory'");
		КонецЕсли;
		Если Объект.ИспользоватьFTPРесурс Тогда
			СпособыДоставки = НСтр("ka='FTP';ru='FTP';en='FTP'");
		КонецЕсли;
		Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.ТолькоУведомить Тогда
			СпособыДоставки = СпособыДоставки + ?(СпособыДоставки = "", НСтр("ka='ელ. ფოსტა';ru='эл. почта';en='e. post office'"), " "+ НСтр("ka='და ელ. ფოსტა';ru='и эл. почта';en='and e. post office'"));
		КонецЕсли;
		
		Элементы.СтраницаДоставка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='მიტანა (%1)';ru='Доставка (%1)';en='Delivery (1%)'"), СпособыДоставки);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ВыполнятьПоРасписанию" Тогда
		Если Объект.ВыполнятьПоРасписанию Тогда
			Элементы.СтраницаРасписание.Заголовок = НСтр("ka='განრიგი (აქტიურად)';ru='Расписание (активно)';en='Schedule (active)'");
		Иначе
			Элементы.СтраницаРасписание.Заголовок = НСтр("ka='განრიგი (არ არის აქტიური)';ru='Расписание (не активно)';en='Schedule (not active)'");
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ka='დასაწყისი';ru='начала';en='beginning'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ka='ბოლოს';ru='конца';en='the end of'");
			КонецЕсли;
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = НСтр("ka='<მიუთითეთ მიმღებები>';ru='<Укажите получателей>';en='<Enter recipients>'");
	...
	Если ПредставлениеОтключенных.Всего > 0 Тогда
		ШаблонРазделения = НСтр("ka='გარდა';ru='Кроме';en='Besides'")+ ": ";
	...
	Если Представление.МаксимумПревышен
		...
		Если ПредставлениеОтключенных.Всего > 0 Тогда
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='(სულ %1, %2-ის გამოკლებით)';ru='(всего %1, исключая %2)';en='(Of 1%, excluding 2%)'"),
				Представление.Всего,
				ПредставлениеОтключенных.Всего);
		Иначе
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='(სულ %1)';ru='(всего %1)';en='(Total 1%)'"),
				Представление.Всего);
		КонецЕсли;
...
Функция ПредставлениеФорматовПоУмолчанию()
	Возврат НСтр("ka='ძირითადი მნიშვნელობა';ru='по умолчанию';en='default'");
КонецФункции
...
Функция ПолучателиУказаны(Получатели)
	Для Каждого СтрокаТаблицы Из Получатели Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТекстСообщения = НСтр("ka='არცერთი მიმღები არ არის არჩეული';ru='Не выбрано ни одного получателя';en='You have no recipient'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПолучателиРассылки");
	Возврат Ложь;
КонецФункции
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ka='ანგარიშგებების დაგზავნა. მიწოდების მეთოდის შემოწმება';ru='Рассылка отчетов. Проверка способа доставки';en='Reports Newsletter. Checking the method of delivery'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = НСтр("ka='მიწოდების მეთოდის ტესტი წარმატებით იქნა გავლილი.';ru='Тест способа доставки успешно пройден.';en='Test method of delivery is successfully passed.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("ka='მიწოდების მეთოდის ტესტი ვერ იქნა გავლილი.';ru='Тест способа доставки не пройден.';en='Test delivery method fails.'");
...
Процедура ЗаполнитьСведенияТаблицыОтчеты()
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если ИнформацияОбОтчете = Неопределено Тогда
			...
			СтрокаОтчеты.Представление = НСтр("ru='<Недостаточно прав для работы с отчетом>';en='<Insufficient permissions to work with the report>'");
		Иначе
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ka='ანგარიშგებების დაგზავნა: ანგარიშგების ინიციალიზაცია';ru='Рассылка отчетов. Инициализация отчета';en='Reports Newsletter. Initialization report'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = НСтр("ka='ანგარიშგება ""%1""-ისთვის ჩატვირთულია ახალი სამომხმარებლო გამართვები.';ru='Для отчета ""%1"" загружены новые пользовательские настройки.';en='For the record, ""%1"" is loaded with the new user settings.'");
			Иначе
				ШаблонСтрокиСообщения = НСтр("ka='დამატებულია ანგარიშგება ""%1"".';ru='Добавлен отчет ""%1"".';en='New report ""%1"".'");
			КонецЕсли;
...
Процедура ЗаписатьНаКлиенте(Результат, ПараметрыЗаписи) Экспорт
	...
	Если ПараметрыЗаписи.Шаг = 3 И ЖелательноОтключитьАрхивацию() Тогда
		...
		// Вопрос.
		ЗаголовокВопроса = НСтр("ka='დაარქივების გამორთვა';ru='Отключить архивацию';en='disable archiving'");
		ТекстВопроса = НСтр("ka='საქაღალდეში ანგარიშგებების გამოქვეყნებისას რეკომენდებულია ZIP-ში დაარქივების გამორთვა.';ru='При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.';en='When you publish a folder of reports recommended to disable archiving in ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ka='ZIP-ში დაარქივების გამორთვა';ru='Отключить архивацию в ZIP';en='Disable archiving in ZIP'"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
