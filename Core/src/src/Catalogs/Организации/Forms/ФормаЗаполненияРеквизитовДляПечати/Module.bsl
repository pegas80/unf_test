...
Функция НеЗаполненЮрАдрес()
	...
	Для Каждого Элемент Из ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если Элемент.Вид = Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации Тогда
			...
			Возврат ЭтотОбъект[Элемент.ИмяРеквизита] = НСтр("ka='შევსება';ru='Заполнить';en='Insert'");
...
Процедура НастроитьЭлементыФормы(ИменаРеквизитовДляЗаполнения)
	...
	//Или Элементы.ГруппаЮрАдрес.Видимость;//T
...
Процедура ДобавитьИзображение(ИмяРеквизитаСКартинкой)
	...
	НСтр("ka='გამოსახულების ასარჩევად საჭიროა ობიექტის ჩაწერა. ჩავწეროთ?';ru='Для выбора изображения необходимо записать объект. Записать?';en='To select the image you want to record the object. Record?'"),
...
Функция НомерСчетаУказанНекорректно()
	...
	НСтр("ka='საჭირო არის ზუსტად 20 ციფრის მითითება.';ru='Требуется указать строго 20 цифр.';en='Must specify a strictly 20 digits.'"),,
	"НомерСчета");
