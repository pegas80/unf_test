...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() И Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='პროგრამაში გამორთულია აღრიცხვა რამდენიმე ორგანიზაციის მიხედვით';ru='В программе отключен учет по нескольким организациям.';en='The program records off for several organizations.'"));
		Отказ = Истина;
	КонецЕсли;
