...
"Выберите другой сертификат.';ka='არჩეული სერთიფიკატის მოქმედების ვადა ამოიწურა."
...
"Выберите другой сертификат.';ka='არჩეულ სერთიფიკატს დახურული გასაღებისთვის პროგრამა არ აქვს მითითებული. აირჩიეთ სხვა სერთიფიკატი.'"));
		ПоказатьОшибку(Контекст.ОшибкаНаКлиенте, Контекст.ОшибкаНаСервере);
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='მონაცემებზე ხელმოწერა ვერ მოხერხდა';ru='Не удалось подписать данные';en='Unable to sign data'"), "",
