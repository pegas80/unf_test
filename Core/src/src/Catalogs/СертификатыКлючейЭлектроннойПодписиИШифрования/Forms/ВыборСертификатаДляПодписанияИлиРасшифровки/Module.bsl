...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДобавлениеВСписок Тогда
		...
		Элементы.Выбрать.Заголовок = НСтр("ka='დამატება';ru='Добавить';en='Add'");
		
		Элементы.ПояснениеУсиленногоПароля.Заголовок =
			НСтр("ka='პაროლის შეყვანაზე გადასასვლელად, დააჭირეთ ""დამატება""-ს.';ru='Нажмите Добавить, чтобы перейти к вводу пароля.';en='Click Add to go to the password.'");
		
		ЛичныйСписокПриДобавлении = Параметры.ЛичныйСписокПриДобавлении;
		Элементы.ПоказыватьВсе.Подсказка =
			НСтр("ka='ყველა სერთიფიკატის ჩვენება შერჩევის გარეშე (მაგალითად, დამატებულების და ვადაგასულების ჩათვლით)';ru='Показать все сертификаты без отбора (например, включая добавленные и просроченные)';en='Show all certificates without selection (eg, including adding and overdue)'");
	КонецЕсли;
	...
	Если ДляШифрованияИРасшифровки = Истина Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემთა დასაშიფრად და გასაშიფრად';ru='Добавление сертификата для шифрования и расшифровки данных';en='Adding a certificate to encrypt and decrypt data'");
		Иначе
			Заголовок = НСтр("ka='სერთიფიკატის არჩევა მონაცემთა დასაშიფრად და გასაშიფრად';ru='Выбор сертификата для шифрования и расшифровки данных';en='Select the certificate for encrypting and decrypting data'");
		КонецЕсли;
	ИначеЕсли ДляШифрованияИРасшифровки = Ложь Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემებზე ხელმოსაწერად';ru='Добавление сертификата для подписания данных';en='Add a certificate for signing data'");
		КонецЕсли;
	ИначеЕсли ЭлектроннаяПодпись.ИспользоватьШифрование() Тогда
		Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემთა ხელმოსაწერად და დასაშიფრად';ru='Добавление сертификата для подписания и шифрования данных';en='Adding a certificate to sign and encrypt data'");
	Иначе
		Заголовок = НСтр("ka='სერთიფიკატის დამატება მონაცემებზე ხელმოსაწერად';ru='Добавление сертификата для подписания данных';en='Add a certificate for signing data'");
	КонецЕсли;
	...
	Если СоздаватьЭлектронныеПодписиНаСервере Тогда
		Элементы.ГруппаСертификаты.Заголовок =
			НСтр("ka='პირადი სერთიფიკატები კომპიუტერზე და სერვერზე';ru='Личные сертификаты на компьютере и сервере';en='Personal certificates on your computer and the server'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Элементы.СертификатОрганизация.Видимость
	   ...
	   И Не ЗначениеЗаполнено(СертификатОрганизация) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ორგანიზაცია არ არის შევსებული.';ru='Поле Организация не заполнено.';en='The Organization field is not filled.'");
...
Процедура СертификатыНедоступныНаКлиентеНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='სერთიფიკატები კომპიუტერზე';ru='Сертификаты на компьютере';en='Certificates on your computer'"), "", ОшибкаПолученияСертификатовНаКлиенте, Новый Структура);
...
Процедура СертификатыНедоступныНаСервереНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='სერთიფიკატები სერვერზე';ru='Сертификаты на сервере';en='Certificates on the server'"), "", ОшибкаПолученияСертификатовНаСервере, Новый Структура);
...
Процедура ВыбратьПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ЗаголовокФормы = НСтр("ka='დაშიფრვის და გაშიფრვის შეცდომა';ru='Проверка шифрования и расшифровки';en='Check the encryption and decryption'");
		Иначе
			ЗаголовокФормы = НСтр("ka='ელექტრონული ხელმოწერის დაყენების შემოწმება';ru='Проверка установки электронной подписи';en='Check the installation of an electronic signature'");
		КонецЕсли;
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
...
Процедура ПерейтиКВыборуТекущегоСертификата(Оповещение)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("ka='გამოჰყავით სერთიფიკატი, რომელიც გამოყენებული იქნება.';ru='Выделите сертификат, который будет использоваться.';en='Highlight the certificate to be used.'");
	...
	Если ТекущиеДанные.ЭтоЗаявление Тогда
		...
		Результат.ОписаниеОшибки =
			НСтр("ka='ამ სერთიფიკატისთვის გამოშვების თაობაზე განცხადება ჯერ არ არის შესრულებული."
"გახსენით სერთიფიკატის გამოშვების თაობაზე განცხადება და შეასრულეთ მოთხოვნილი ეტაპები.';ru='Для этого сертификата заявление на выпуск еще не исполнено."
"Откройте заявление на выпуск сертификата и выполните требуемые шаги.'");
	...
	Если СертификатНаСервере Тогда
		...
		Иначе
			Результат.ОписаниеОшибки = НСтр("ka='სერვერზე სერთიფიკატი ვერ მოიძებნა (შეიძლება წაშლილია).';ru='Сертификат не найден на сервере (возможно удален).';en='The certificate is not found on the server (possibly deleted).'");
			Результат.ОбновитьСписокСертификатов = Истина;
			ВыполнитьОбработкуОповещения(Оповещение, Результат);
		КонецЕсли;
		Возврат;
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеПоискаСертификата(РезультатПоиска, Контекст) Экспорт
	...
	Если ТипЗнч(РезультатПоиска) <> Тип("СертификатКриптографии") Тогда
		Если РезультатПоиска.Свойство("СертификатНеНайден") Тогда
			Контекст.Результат.ОписаниеОшибки = НСтр("ka='კომპიუტერზე სერთიფიკატი ვერ მოიძებნა (შეიძლება წაშლილია).';ru='Сертификат не найден на компьютере (возможно удален).';en='Certificate can not be found on your computer (possibly deleted).'");
		Иначе
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеЗаполненияСвойствСертификата(Контекст)
	...
	Если ДобавлениеВСписок Тогда
		Строка = ?(ЗначениеЗаполнено(Сертификат), НСтр("ka='განახლება';ru='Обновить';en='update'"), НСтр("ka='დამატება';ru='Добавить';en='Add'"));
...
Процедура ПроверитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии")
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			Результат.Вставить("ЗаголовокОшибки", НСтр("ka='დაშიფრვის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку шифрования по причине:';en='It failed to pass the test because of encryption:'"));
		Иначе
			Результат.Вставить("ЗаголовокОшибки", НСтр("ka='ხელმოწერის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку подписания по причине:';en='Could not be tested because of the signing:'"));
		КонецЕсли;
...
Функция ПроверитьСертификатИЗаписатьВСправочник(Знач ЗначениеПароля, ОшибкаНаСервере)
	...
	Если ЭлектроннаяПодписьСлужебный.МенеджерКриптографии("", Ложь, ОшибкаНаСервере) = Неопределено
	   И ОшибкаНаСервере.Общая Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ka='დაშიფრვის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку шифрования по причине:';en='It failed to pass the test because of encryption:'"));
		Иначе
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ka='ხელმოწერის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:';ru='Не удалось пройти проверку подписания по причине:';en='Could not be tested because of the signing:'"));
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
"%2';ka='პროგრამა %1-ის დახმარებით დაშიფრვის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:"
"%2'"),
...
"%2';ka='პროგრამა %1-ის მეშვეობით გაშიფრვის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:"
"%2'"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
...
"%2';ka='პროგრამა %1-ის მეშვეობით ხელმოწერის შემოწმების გავლა ვერ მოხერხდა შემდეგი მიზეზით:"
"%2'"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
