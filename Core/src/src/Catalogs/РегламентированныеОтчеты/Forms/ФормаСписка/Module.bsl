...
Функция ПолучитьОтчет(НаименованиеОтчета = Неопределено)
	...
	Если ПравоДоступаКОтчету = Ложь Тогда
		...
		Сообщение.Текст = НСтр("ka='არასაკმარისი უფლებები.';ru='Недостаточно прав.';en='Enough rights.'");
		Сообщение.Сообщить();
	...
	ИначеЕсли ПравоДоступаКОтчету = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Отчет не найден.';ru='Отчет не найден.';en='The report could not be found.'");
		Сообщение.Сообщить();
	...
	Если ТекОтчет = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось получить отчет.';ru='Не удалось получить отчет.';en='Could not get a report.'");
		Сообщение.Сообщить();
...
Функция УстановленаСортировкаПоКоду()
	...
	Если НЕ СортировкаПоКоду Тогда
		...
		ПоказатьПредупреждение(,НСтр("ka='Для изменения порядка следования необходимо установить сортировку по коду.';ru='Для изменения порядка следования необходимо установить сортировку по коду.';en='To change the sequence of the order is necessary to set the sorting code.'"));
...
Функция СоздатьНовыйОтчет()
	...
	Если Элементы.Список.ТекущаяСтрока = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შეარჩიეთ ანგარიშგება.';ru='Выберите отчет.';en='Select the report.'");
		Сообщение.Сообщить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Объект.ЭтоГруппа Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინფორმაციის რეგისტრში ფარულ ანგარიშგებაზე ბმულის ჩაწერა ვერ მოხერხდა: %1';ru='Не удалось записать ссылку на скрытый отчет в регистр сведений. %1';en='Failed to write a link to a hidden account in the register information. %one'"), ОписаниеОшибки());
...
Процедура ПереместитьЭлементВыполнить(СписокРеквизит, СписокЭлемент, Вверх)
	...
	Иначе
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), СтрОшибка));
	КонецЕсли;
...
Функция ПроверитьСписокПередОперацией(СписокРеквизит, СписокЭлемент, УстановленныеОтборы)
	...
	Для Каждого ЭлементГруппировки Из ЭлементыГруппировки Цикл
		Если ЭлементГруппировки.Использование Тогда
			ПоказатьПредупреждение(,НСтр("ka='Изменение порядка элементов невозможно при установленной группировке.';ru='Изменение порядка элементов невозможно при установленной группировке.';en='Changing the order of the elements is impossible when fixed grouping.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
