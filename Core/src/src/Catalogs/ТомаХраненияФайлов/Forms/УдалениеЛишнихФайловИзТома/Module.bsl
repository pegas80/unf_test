...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	Если КоличествоЛишнихФайлов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='დისკზე არცერთი ზედმეტი ფაილი არ არის';ru='Нет ни одного лишнего файла на диске';en='No extra files on the disk'"));
		Возврат;
	КонецЕсли;
	...
	Если ЧислоУдаленныхФайлов <> 0 Тогда
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='წაშლილია ფაილი: %1';ru='Удалено файлов: %1';en='Deleted files:% 1'"),
			ЧислоУдаленныхФайлов);
		ПоказатьОповещениеПользователя(
			НСтр("ka='ზედმეტი ფაილების წაშლა დასრულებულია.';ru='Завершено удаление лишних файлов.';en='Completed delete unnecessary files.'"),
			,
			ТекстОповещения,
...
Процедура ЗаполнитьТаблицуЛишнихФайлов()
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		НоваяСтрока.СтатусПроверки = НСтр("ka='ზედმეტი ფაილები (არის დისკზე, მაგრამ მათ შესახებ ინფორმაცია არ არის)';ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)';en='Unnecessary files (available on the disc, but no information about them)'");
	...
	МассивЛишнихФайлов = ТаблицаФайловНаДиске.НайтиСтроки(
		Новый Структура("СтатусПроверки", НСтр("ka='ზედმეტი ფაილები (არის დისკზე, მაგრამ მათ შესახებ ინფორმაცია არ არის)';ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)';en='Unnecessary files (available on the disc, but no information about them)'")));
...
""
"%1';ka='კოპირებისთვის კატალოგის გზა არაკორექტულია."
"შეიძლება, რომ სააღრიცხვო ჩანაწერს, რომლის სახელით მუშაობს 1С:საწარმოს სერვერი, აღნიშნულ კატალოგთან დაშვების უფლებები არ გააჩნია."
""
"%1'");
...
Процедура ОбработатьСообщениеОбОшибке(ИмяФайла, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. ზედმეტი ფაილების წაშლის შეცდომა';ru='Файлы.Ошибка удаления лишних файлов';en='Fayly.Oshibka delete unnecessary files'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При удалении файла с диска"
"""%1"""
"возникла ошибка:"
"""%2"".';ka='დისკ ""%1""-იდან ფაილის წაშლისას შეცდომა წარმოიშვა:"
"""%2"".'"),
			ИмяФайла,
			ИнформацияОбОшибке));
...
Процедура СформироватьОтчетОбОшибках(ОтчетОбОшибках, МассивФайловСОшибками)
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("ka='შეცდომებიანი ფაილები:';ru='Файлы с ошибками:';en='Files with errors:'");
