...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = СтрШаблон(
		НСтр("ka='შერჩევების გასწორება ახალი ამბების ლენტ %1-ისთვის';ru='Настройка отборов для ленты новостей %1';en='Setting selections for feeds 1%'"),
		...
		Если (Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.РазрешеноНастраиватьПользователям = Истина) 
			...
			Если Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
				ПоказатьОповещениеПользователя(
					НСтр("ka='შერჩევების ნაწილი არ არის წაშლილი';ru='Часть отборов не удалена';en='Part selection is not deleted'"),
					, // Навигационная ссылка
					НСтр("ka='ადმინისტრატორის მიერ გამართული შერჩევების წაშლა არ შეიძლება.';ru='Отборы, настроенные администратором, удалить нельзя.';en='Selections are configured by the administrator can not be deleted.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
					...
					Если (НайденнаяСтрока.СписокЗначенийОтборов.Количество() = 0) Тогда
						Если (НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() = 0) Тогда
							Если НайденнаяСтрока.ОтбиратьПоКатегории Тогда
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("ka='Введите значения отборов';ru='Введите значения отборов';en='Enter values ​​selections'");
							Иначе
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = ""; // Вывести все значения
							КонецЕсли;
						Иначе
							НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("ka='აწყობილია ადმინისტრატორის მიერ:';ru='Настроено администратором:';en='Configured by the administrator:'") + " ";
	...
	Иначе
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			Если (НайденнаяСтрока.ЗаполняетсяАвтоматически = Истина) Тогда
				НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='შევსება წარმოებს ავტომატურად, გამართვა აკრძალულია';ru='Заполняется автоматически, настройка запрещена';en='It is filled in automatically, setting prohibited'");
			Иначе
				Если (НайденнаяСтрока.РазрешеноНастраиватьПользователям = Истина) Тогда
					Если НайденнаяСтрока.ОтбиратьПоКатегории = Истина Тогда
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='მე მსურს შერჩევის გამართვა';ru='Я хочу настроить отбор';en='I want to set selection'");
					Иначе
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='ამ კატეგორიის ყველა ახალი ამბავის ჩვენება';ru='Показывать все новости по этой категории';en='Show all news of this category'");
					КонецЕсли;
				Иначе
					Если НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
						// Если администратор ввел свои значения отбора и запретил редактировать.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='შერჩევა უკვე გამართულია ადმინისტრატორის მიერ, გამართვა აკრძალულია';ru='Отбор уже настроен администратором, настройка запрещена';en='The selection has already been configured by the administrator, the setting is prohibited'");
					Иначе
						// Если администратор просто запретил редактировать, не вводя доп.ограничений - будут получены все новости.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='შერჩევის ეს გამართვა აკრძალულია ადმინისტრატორის მიერ';ru='Эта настройка отбора запрещена администратором';en='This setting is disabled by the administrator selection'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
