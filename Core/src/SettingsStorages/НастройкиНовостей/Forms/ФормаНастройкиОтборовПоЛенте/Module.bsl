...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = СтрШаблон(
		НСтр("ka='შერჩევების გასწორება ახალი ამბების ლენტ %1-ისთვის';ru='Настройка отборов для ленты новостей %1'"),
		...
		Если (Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.РазрешеноНастраиватьПользователям = Истина) 
			...
			Если Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
				ПоказатьОповещениеПользователя(
					НСтр("ka='შერჩევების ნაწილი არ არის წაშლილი';ru='Часть отборов не удалена'"),
					, // Навигационная ссылка
					НСтр("ka='ადმინისტრატორის მიერ გამართული შერჩევების წაშლა არ შეიძლება.';ru='Отборы, настроенные администратором, удалить нельзя.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
					...
					Если (НайденнаяСтрока.СписокЗначенийОтборов.Количество() = 0) Тогда
						Если (НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() = 0) Тогда
							Если НайденнаяСтрока.ОтбиратьПоКатегории Тогда
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("ka='Введите значения отборов';ru='Введите значения отборов'");
							Иначе
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = ""; // Вывести все значения
							КонецЕсли;
						Иначе
							НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("ka='აწყობილია ადმინისტრატორის მიერ:';ru='Настроено администратором:'") + " ";
	...
	Иначе
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			Если (НайденнаяСтрока.ЗаполняетсяАвтоматически = Истина) Тогда
				НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='შევსება წარმოებს ავტომატურად, გამართვა აკრძალულია';ru='Заполняется автоматически, настройка запрещена'");
			Иначе
				Если (НайденнаяСтрока.РазрешеноНастраиватьПользователям = Истина) Тогда
					Если НайденнаяСтрока.ОтбиратьПоКатегории = Истина Тогда
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='მე მსურს შერჩევის გამართვა';ru='Я хочу настроить отбор'");
					Иначе
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='ამ კატეგორიის ყველა ახალი ამბავის ჩვენება';ru='Показывать все новости по этой категории'");
					КонецЕсли;
				Иначе
					Если НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
						// Если администратор ввел свои значения отбора и запретил редактировать.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='შერჩევა უკვე გამართულია ადმინისტრატორის მიერ, გამართვა აკრძალულია';ru='Отбор уже настроен администратором, настройка запрещена'");
					Иначе
						// Если администратор просто запретил редактировать, не вводя доп.ограничений - будут получены все новости.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ka='შერჩევის ეს გამართვა აკრძალულია ადმინისტრატორის მიერ';ru='Эта настройка отбора запрещена администратором'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
