...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ka='Список новостей обновляется еженедельно';ru='Список новостей обновляется еженедельно'");
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ka='Список новостей обновляется ежедневно';ru='Список новостей обновляется ежедневно'");
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // 1 раз в час
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ka='Список новостей обновляется ежечасно';ru='Список новостей обновляется ежечасно'");
	Иначе
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление =
			СтрШаблон(
				НСтр("ka='Список новостей обновляется каждые %1 минут';ru='Список новостей обновляется каждые %1 минут'"),
	...
	Если ЭтаФорма.ТекущийПользователь <> ПользователиКлиентСервер.ТекущийПользователь() Тогда
		ЭтаФорма.Заголовок =
			СтрШаблон(
				НСтр("ka='Настройки отображения новостей (%1)';ru='Настройки отображения новостей (%1)'"),
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("ka='თავდაპირველად, ჩართეთ ამ ლენტის მიხედვით ახალი ამბების მიღება.';ru='Вначале включите получение новостей по этой ленте.'"),
				0,
				НСтр("ka='გაფრთხილება';ru='Предупреждение'"));
		КонецЕсли;
	КонецЕсли;
...
"'"),
				...
				Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
					ПредставлениеВсехОтборовКратко   = НСтр("ka='შერჩევები არ არის გამართული';ru='Отборы не настроены'");
					ПредставлениеВсехОтборовПодробно = НСтр("ka='შერჩევები არ არის გამართული';ru='Отборы не настроены'");
				Иначе
					ПредставлениеВсехОтборовКратко   = НСтр("ka='გამართულია დეტალური შერჩევა';ru='Настроен детальный отбор'");
				КонецЕсли;
