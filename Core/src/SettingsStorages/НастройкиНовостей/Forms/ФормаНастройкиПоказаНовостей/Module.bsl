...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ka='Список новостей обновляется еженедельно';ru='Список новостей обновляется еженедельно';en='News list is updated weekly'");
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ka='Список новостей обновляется ежедневно';ru='Список новостей обновляется ежедневно';en='News list updated daily'");
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // 1 раз в час
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ka='Список новостей обновляется ежечасно';ru='Список новостей обновляется ежечасно';en='News List updated hourly'");
	Иначе
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление =
			СтрШаблон(
				НСтр("ka='Список новостей обновляется каждые %1 минут';ru='Список новостей обновляется каждые %1 минут';en='News list is updated every% 1 minutes'"),
	...
	Если ЭтаФорма.ТекущийПользователь <> ПользователиКлиентСервер.ТекущийПользователь() Тогда
		ЭтаФорма.Заголовок =
			СтрШаблон(
				НСтр("ka='Настройки отображения новостей (%1)';ru='Настройки отображения новостей (%1)';en='Settings display news (1%)'"),
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("ka='თავდაპირველად, ჩართეთ ამ ლენტის მიხედვით ახალი ამბების მიღება.';ru='Вначале включите получение новостей по этой ленте.';en='Initially, turn the news on this tape.'"),
				0,
				НСтр("ka='გაფრთხილება';ru='Предупреждение';en='A warning'"));
		КонецЕсли;
	КонецЕсли;
...
"';ka='Нет отборов, разрешенных для настройки пользователем"
"'"),
							СтрШаблон(
								НСтр("ru='Отборы, настроенные пользователем (можно изменить):"
"%1"
"';ka='Отборы, настроенные пользователем (можно изменить):"
"%1"
"'"),
								ПредставлениеПользовательскихОтборов));
...
"%1"
"';ka='Отборы, настроенные администратором (нельзя изменить):"
"%1"
"'"),
								ПредставлениеАдминистративныхОтборов));
...
"%1"
"';ka='Автоматически заполняемые отборы (нельзя изменить):"
"%1"
"'"),
								ПредставлениеАвтоматическиЗаполняемыхОтборов));

				Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
					ПредставлениеВсехОтборовКратко   = НСтр("ka='შერჩევები არ არის გამართული';ru='Отборы не настроены';en='Selections are not set'");
					ПредставлениеВсехОтборовПодробно = НСтр("ka='შერჩევები არ არის გამართული';ru='Отборы не настроены';en='Selections are not set'");
				Иначе
					ПредставлениеВсехОтборовКратко   = НСтр("ka='გამართულია დეტალური შერჩევა';ru='Настроен детальный отбор';en='Set detailed selection'");
				КонецЕсли;
...
"%1"
"';ka='По этой ленте нельзя отключить получение новостей (так настроено администратором)"
"%1"
"'"),
						ПредставлениеВсехОтборовПодробно);
			КонецЕсли;
