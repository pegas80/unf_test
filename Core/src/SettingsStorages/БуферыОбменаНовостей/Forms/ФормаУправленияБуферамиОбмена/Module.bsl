...
"но она не записана."
""
"Записать?';ka='Введено наименование настройки (%1),"
"но она не записана."
""
"Записать?'"),
			...
			Если ЭтаФорма.ТекущийБуферОбмена.Количество() > 0 Тогда
				Если НЕ ПустаяСтрока(ЭтаФорма.КлючНастройки) Тогда
					Если ЭтаФорма.СписокКлючейНастроек.НайтиПоЗначению(ЭтаФорма.КлючНастройки) = Неопределено Тогда
						...
						СписокКнопок.Добавить("ЗаписатьИЗакрыть", НСтр("ka='ჩაწერა და დახურვა';ru='Записать и закрыть';en='Record and Close'"));
						СписокКнопок.Добавить("Закрыть", НСтр("ka='Закрыть без записи';ru='Закрыть без записи';en='Close without saving'"));
						СписокКнопок.Добавить("Отмена", НСтр("ka='არ დაიხუროს';ru='Не закрывать';en='Do not close'"));
						ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", НСтр("ka='Текущее значение буфера обмена не сохранено';ru='Текущее значение буфера обмена не сохранено';en='The current value is not stored clipboard'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ka='Выберите файл, куда записать список буферов обмена';ru='Выберите файл, куда записать список буферов обмена';en='Select the file in which to place the list of clipboards'");
	...
	Если Диалог.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='მონაცემები გადმოტვირთულია';ru='Данные выгружены.';en='Data uploaded.'"),
			,
			,
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ka='Выберите файл с сохраненными ранее буферами обмена';ru='Выберите файл с сохраненными ранее буферами обмена';en='Select the file previously saved clipboard'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	...
	Если Диалог.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='მონაცემები ატვირთულია.';ru='Данные загружены.';en='Data is loaded.'"),
			,
			,
...
"Вначале скопируйте в него строки данных.';ka='Пустой буфер обмена."
"Нельзя сохранить пустой буфер обмена."
...
Функция ПолучитьОписаниеВажности(Важность)
	...
	Если Важность = 1 Тогда
		Возврат НСтр("ka='ძალიან მნიშვნელოვანი';ru='Очень важная';en='Very important'");
	ИначеЕсли Важность = 2 Тогда
		Возврат НСтр("ka='მნიშვნელოვანი';ru='Важная';en='important'");
	Иначе
		Возврат НСтр("ka='ჩვეულებრივი';ru='Обычная';en='usual'");
	КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбменаНаСервере(ИмяФайла)
	...
	ЗаписьХМЛ.ОткрытьФайл(
		...
		ЗаписьХМЛ.ЗаписатьКомментарий(
			СтрШаблон(
				НСтр("ka='Выгрузка данных буферов обмена для %1, %2';ru='Выгрузка данных буферов обмена для %1, %2';en='Unloading of data exchange buffers to% 1,% 2'"),
				ЭтаФорма.КлючОбъекта,
