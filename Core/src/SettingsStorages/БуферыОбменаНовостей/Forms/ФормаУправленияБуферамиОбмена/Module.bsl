			...
			Если ЭтаФорма.ТекущийБуферОбмена.Количество() > 0 Тогда
				Если НЕ ПустаяСтрока(ЭтаФорма.КлючНастройки) Тогда
					Если ЭтаФорма.СписокКлючейНастроек.НайтиПоЗначению(ЭтаФорма.КлючНастройки) = Неопределено Тогда
						...
						СписокКнопок.Добавить("ЗаписатьИЗакрыть", НСтр("ka='ჩაწერა და დახურვა';ru='Записать и закрыть'"));
						СписокКнопок.Добавить("Закрыть", НСтр("ka='Закрыть без записи';ru='Закрыть без записи'"));
						СписокКнопок.Добавить("Отмена", НСтр("ka='არ დაიხუროს';ru='Не закрывать'"));
						ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", НСтр("ka='Текущее значение буфера обмена не сохранено';ru='Текущее значение буфера обмена не сохранено'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ka='Выберите файл, куда записать список буферов обмена';ru='Выберите файл, куда записать список буферов обмена'");
	...
	Если Диалог.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='მონაცემები გადმოტვირთულია';ru='Данные выгружены.'"),
			,
			,
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ka='Выберите файл с сохраненными ранее буферами обмена';ru='Выберите файл с сохраненными ранее буферами обмена'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	...
	Если Диалог.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='მონაცემები ატვირთულია.';ru='Данные загружены.'"),
			,
			,
...
Функция ПолучитьОписаниеВажности(Важность)
	...
	Если Важность = 1 Тогда
		Возврат НСтр("ka='ძალიან მნიშვნელოვანი';ru='Очень важная'");
	ИначеЕсли Важность = 2 Тогда
		Возврат НСтр("ka='მნიშვნელოვანი';ru='Важная'");
	Иначе
		Возврат НСтр("ka='ჩვეულებრივი';ru='Обычная'");
	КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбменаНаСервере(ИмяФайла)
	...
	ЗаписьХМЛ.ОткрытьФайл(
		...
		ЗаписьХМЛ.ЗаписатьКомментарий(
			СтрШаблон(
				НСтр("ka='Выгрузка данных буферов обмена для %1, %2';ru='Выгрузка данных буферов обмена для %1, %2'"),
				ЭтаФорма.КлючОбъекта,
