...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Режим", Режим) Тогда
		ВызватьИсключение НСтр("ka='სამსახურებრივი პარამეტრი ""რეჟიმი"" არ არის გადაცემული.';ru='Не передан служебный параметр ""Режим"".'");
	КонецЕсли;
	...
	ИначеЕсли Режим = "Отборы" Или Режим = "ВыбранныеПоля" Или Режим = "Сортировка" Или Режим = "ПоляГруппировки" Тогда
		ИмяТаблицы = Режим;
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრ ""რეჟიმი""-ს არაკორექტული მნიშვნელობა: ""%1"".';ru='Некорректное значение параметра ""Режим"": ""%1"".'"), Строка(Режим));
	КонецЕсли;
	...
	Если ИмяТаблицы = "ПоляГруппировки" Тогда
		...
		Если Режим = "СтруктураВарианта" Тогда
			...
			СтрокаДерева.Представление  = НСтр("ka='<Детальные записи>';ru='<Детальные записи>'");
...
Процедура ВыбратьИЗакрыть()
	...
	Если ТипЗнч(ДоступноеПолеКД) = Тип("ДоступноеПолеКомпоновкиДанных")
		...
		Если ДоступноеПолеКД.Папка Тогда
			ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ელემენტები';ru='Выберите элемент'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
