...
Процедура ВыбратьИЗакрыть()
	...
	Если Вариант.ИндексКартинки = 4 Тогда
		ТекстВопроса = НСтр("ka='ანგარიშგების არჩეული ვარიანტი მონიშნულია წასაშლელად. ავირჩიოთ ანგარიშგების ეს ვარიანტი?';ru='Выбранный вариант отчета помечен на удаление."
...
Процедура ОткрытьВариантДляИзменения()
	...
	Если Не ПравоИзмененияВарианта(Вариант, ПолныеПраваНаВарианты) Тогда
		ТекстПредупреждения = НСтр("ka='ვარიანტ ""%1""-ის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения варианта ""%1"".';en='Not allowed to edit options ""%1"".'");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		Если НаборСтрокДерева = Неопределено Тогда
			...
			Если СтрокаТаблицы.НомерГруппы = 2 Тогда
				ГруппаДерева.Наименование = НСтр("ka='Скрытые в панелях отчетов';ru='Скрытые в панелях отчетов';en='Hidden in the reporting panels'");
				ГруппаДерева.ИндексКартинки = 0;
				ГруппаДерева.КартинкаАвтора = -1;
			ИначеЕсли СтрокаТаблицы.НомерГруппы = 3 Тогда
				ГруппаДерева.Наименование = НСтр("ka='წასაშლელად მონიშნულები';ru='Помеченные на удаление';en='Marked for deletion'");
