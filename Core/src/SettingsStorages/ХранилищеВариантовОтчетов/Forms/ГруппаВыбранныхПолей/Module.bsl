...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Расположение", Расположение) Тогда
		ВызватьИсключение НСтр("ka='სამსახურებრივი პარამეტრი ""განთავსება"" არ არის გადაცემული.';ru='Не передан служебный параметр ""Расположение"".'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='პარამეტრ ""განლაგება""-ს არაკორექტული მნიშვნელობა: ""%1"".';ru='Некорректное значение параметра ""Расположение"": ""%1"".'"), Строка(Расположение));
	КонецЕсли;
КонецПроцедуры
