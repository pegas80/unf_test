...
Процедура ОбновитьСостояниеКассовойСменыНаСервере(КассаККМ)
	...
	Если ЗначениеЗаполнено(СтруктураСостояниеКассовойСмены.СтатусКассовойСмены) Тогда
		...
		ТекстСообщения = НСтр("ka='ცვლა № %НомерСмены%, სტატუსი: %СтатусСмены% %ВремяИзменения%, სალაროში %НаличностьВКассе% %Валюта%';ru='Смена № %НомерСмены%, Статус: %СтатусСмены% %ВремяИзменения%, В кассе %НаличностьВКассе% %Валюта%'");
	...
	Иначе
		...
		СостояниеКассовойСмены = НСтр("ka='ცვლა არ არის გახსნილი.';ru='Смена не открыта.'");
