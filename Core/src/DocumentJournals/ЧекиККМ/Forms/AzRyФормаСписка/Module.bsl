...
Процедура ОбновитьСостояниеКассовойСменыНаСервере(КассаККМ)
	...
	Если ЗначениеЗаполнено(СтруктураСостояниеКассовойСмены.СтатусКассовойСмены) Тогда
		...
		ТекстСообщения = НСтр("ka='ცვლა № %НомерСмены%, სტატუსი: %СтатусСмены% %ВремяИзменения%, სალაროში %НаличностьВКассе% %Валюта%';ru='Смена № %НомерСмены%, Статус: %СтатусСмены% %ВремяИзменения%, В кассе %НаличностьВКассе% %Валюта%';en='Changing the number NomerSmeny%% Title:%%% StatusSmeny VremyaIzmeneniya% in box office NalichnostVKasse%%% Currency%'");
	...
	Иначе
		...
		СостояниеКассовойСмены = НСтр("ka='ცვლა არ არის გახსნილი.';ru='Смена не открыта.';en='Change is not open.'");
