...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Контрагент.Пустая() Тогда
		ВызватьИсключение НСтр("ka='კონტრაგენტის თაობაზე დოკუმენტების ჟურნალის გახსნა ხდება კონტრაგენტის ბარათიდან.';ru='Журнал документов по контрагенту открывается из карточки контрагента.';en='Document Journal counterparty opens from counterparty cards.'");
	КонецЕсли;
	
	Заголовок = НСтр("ka='დოკუმენტი მიხედვით';ru='Документы по';en='documents'") + " """ + Параметры.Контрагент + """";
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	_Ссылка = Элементы.Список.ТекущиеДанные.Ссылка;
	_ИмяДокумента = ПолучитьИмяДокумента(_Ссылка);
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму("Документ."+_ИмяДокумента+".ФормаОбъекта", Новый Структура("Ключ", _Ссылка));
...
Процедура Провести(Команда)
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл 
		...
		//_Ссылка = Элементы.Список.ТекущиеДанные.Ссылка;
		_Ссылка = Элементы.Список.ДанныеСтроки(Строка).Ссылка;
		
		ПараметрыЗаписи   = Новый Структура ;
		ПараметрыЗаписи.Вставить("РежимЗаписи", РежимЗаписиДокумента.Проведение);
		ПараметрыЗаписи.Вставить("РежимПроведения",	РежимПроведенияДокумента.Неоперативный);
		
		Если команда.имя =  "Провести" Тогда 
			ПолучитьОбъектДляЗаписи(_Ссылка);
		КонецЕсли;	
...
&НаСервере
Процедура ПолучитьОбъектДляЗаписи(Ссылка)
...
Процедура ОтменитьПроведение(Команда)
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		ПараметрыЗаписи.Вставить("РежимПроведения",	РежимПроведенияДокумента.Неоперативный);		
		Если команда.имя =  "ОтменитьПроведение" Тогда 
			ПолучитьОбъектДляЗаписи1(_Ссылка);
		КонецЕсли;
	КонецЦикла;		
КонецПроцедуры

&НаСервере
Процедура ПолучитьОбъектДляЗаписи1(Ссылка)
	
	ДокОбъект = Ссылка.ПолучитьОбъект();
	попытка
		ДокОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения, РежимПроведенияДокумента.Неоперативный);
	Исключение
		Сообщить(ОписаниеОшибки());
	КонецПопытки;
...
Процедура Скопировать(Команда)
	...
	_Ссылка = Элементы.Список.ТекущиеДанные.Ссылка;		
	СсылкаНаНовДокумент = ПолучитьОбъектДляСкопирования(_Ссылка);
	ОткрытьЗначение(СсылкаНаНовДокумент);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьОбъектДляСкопирования(Ссылка)
...
Процедура ПометитьНаУдалениеСнятьПометку(Команда)
	...
	Ответ = Вопрос(НСтр("ru = 'Пометить на удаление ?'; ka = 'მოინიშნოს დოკუმენტი წაშლაზე ?'"
	+ " en = ' '"), Режим, 0);
...
Процедура   Тест (Ссылка)
	...
	Если   ДокОбъект.ПометкаУдаления  Тогда  ДокОбъект.ПометкаУдаления = Ложь
			
	ИначеЕсли  Не ДокОбъект.ПометкаУдаления  Тогда ДокОбъект.ПометкаУдаления = Истина 
