...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+Lazare 09.03.2022
	Контекст.Вставить("ФиксацияСверху",Параметры.ФиксацияСверху);
	Контекст.Вставить("ФиксацияСлева",Параметры.ФиксацияСлева);
	//-Lazare 09.03.2022
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если НЕ ЗначениеЗаполнено(ВариантНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ველი ""სახელწოდება"" არ არის შევსებული';ru='Поле ""Наименование"" не заполнено';en='""Name"" field is not filled'"),
			,
			"Наименование");
		Отказ = Истина;
	ИначеЕсли ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, ВариантСсылка, ВариантНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(
				НСтр("ka='""%1"" დაკავებულია, საჭიროა სხვა სახელწოდების მითითება.';ru='""%1"" занято, необходимо указать другое Наименование.';en='""%1"" is occupied, you must specify a different name.'"),
				ВариантНаименование
			),
			,
			"Наименование");
...
Процедура ОписаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Элементы.Описание.ТекстРедактирования,
		НСтр("ka='აღწერა';ru='Описание';en='Description'"));
КонецПроцедуры
...
Процедура ВариантыОтчетаПередУдалением(Элемент, Отказ)
	...
	Если Не Контекст.ПолныеПраваНаВарианты И НЕ Вариант.АвторТекущийПользователь Тогда
		ТекстПредупреждения = НСтр("ka='ანგარიშგება ""%1""-ის ვარიანტის წასაშლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для удаления варианта отчета ""%1"".';en='Not have permission to delete the report option ""%1"".'");
	...
	Если Не Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების ხელახლა განსაზღვრული ვარიანტის წაშლა შეუძლებელია.';ru='Невозможно удалить предопределенный вариант отчета.';en='You can not delete a pre-defined version of the report.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВариантОбразецНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ka='აირჩიეთ გამოსახულების ფაილი';ru='Выберите файл изображения';en='Select an image file'"));
	ПараметрыДиалога.Вставить("ПредварительныйПросмотр", Истина);
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ka='ყველა სურათი (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf';ru='Все картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf';en='All pictures (* .bmp; * gif; * png; * jpeg; * dib; * rle; * tif; * jpg; * ico; * wmf; * emf..........) | *. bmp;. * gif;. * png;. * jpeg;. * dib;. * rle; * tif;. * jpg;. * ico;. * wmf;.. * emf'"));
...
Процедура ЗаполнитьСписокВариантов()
	...
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Порядок,
	|	ВариантыОтчетов.ФиксацияСверху КАК ФиксацияСверху,
	|	ВариантыОтчетов.ФиксацияСлева КАК ФиксацияСлева
	|ИЗ
...
"внешнего отчета ""%1"":';ka='ვერ მოხერხდა გარე ანგარიშგება ""%1""-ის წინასწარ განსაზღვრული ვარიანტების სიის მიღება:'"),
...
Функция СформироватьСвободноеНаименование(Вариант, ВариантыОтчета)
	ШаблонИмениВарианта = СокрЛП(Вариант.Наименование) +" - "+ НСтр("ka='ასლი';ru='копия';en='copy'");
...
Процедура ПрочитатьДанныеВарианта()
	...
	Если Выборка.Следующий() Тогда
		...
		КонецЕсли;
		//+Lazare 09.03.2022
		ФиксацияСверху =  Выборка.ФиксацияСверху;
		ФиксацияСверху =  Выборка.ФиксацияСлева;
		//-Lazare 09.03.2022
...
Процедура ВыполнитьПакет(Результат, Пакет) Экспорт
	...
	Если Пакет.ПроверитьСтраницу1 = Истина Тогда
		...
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ТекстОшибки = НСтр("ka='ველი ""სახელწოდება"" არ არის შევსებული';ru='Поле ""Наименование"" не заполнено';en='""Name"" field is not filled'");
		...
		Если Не Пакет.ВариантЭтоНовый Тогда
			...
			Если НЕ ПравоЗаписиВарианта(Вариант, ПравоНастройкиВарианта(Вариант, Контекст.ПолныеПраваНаВарианты), Контекст.ЭтоПолныеПрава, Контекст.РежимНастройки) Тогда
				ТекстОшибки = НСтр("ka='ვარიანტ ""%1""-ის შესაცვლელად არასაკმარისი უფლებებია. აუცილებელია სხვა ვარიანტის არჩევა ან ""სახელწოდება""-ს შეცვლა.';ru='Недостаточно прав для изменения варианта ""%1"". Необходимо выбрать другой вариант или изменить Наименование.';en='Not allowed to edit options ""%1"". You must select another option or change the name.'");
			...
			Если Не Пакет.Свойство("ВопросНаПерезаписьПройден") Тогда
				Если Вариант.ПометкаУдаления = Истина Тогда
					ТекстВопроса = НСтр("ka='ანგარიშგების ვარიანტი ""%1"" მონიშნულია წასაშლელად. შევცვალოთ წასაშლელად მონიშნული ანგარიშგების ვარიანტი?';ru='Вариант отчета ""%1"" помечен на удаление."
"Заменить помеченный на удаление вариант отчета?'");
					КнопкаПоУмолчанию = КодВозвратаДиалога.Нет;
				Иначе
					ТекстВопроса = НСтр("ka='შევცვალოთ ადრე შენახული ანგარიშგების ვარიანტი ""%1""?';ru='Заменить ранее сохраненный вариант отчета ""%1""?';en='Replace a previously saved version of the report ""%1""?'");
	...
	ИначеЕсли Пакет.ПроверитьИЗаписатьСервер Тогда 
		...
		Если Не Пакет.ВариантЭтоНовый И НЕ Контекст.ЭтоПолныеПрава И Контекст.ПолныеПраваНаВарианты И НЕ ВариантАвтор=Контекст.ТекущийПользователь Тогда 
			ТекстОшибки = НСтр("ka='ვარიანტ ""%1""-ის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения варианта ""%1"".';en='Not allowed to edit options ""%1"".'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, ВариантНаименование);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ВариантНаименование");
			Возврат;
		КонецЕсли; 
...
Процедура СохранитьИЗакрыть()
	...
	Если НЕ Элементы.ДобавитьПривязку.Видимость Тогда
		Состояние(НСтр("ka='გააუქმეთ ან დაასრულეთ ანგარიშგების კონტექსტური გახსნის აღწერილობის რედაქტირება';ru='Отмените или завершите редактирование описания контекстного открытия отчета';en='Cancel or finish editing the description of the context of opening report'"));
		Возврат;
	КонецЕсли; 
...
Процедура ПроверитьИЗаписатьНаСервере(Пакет)
	...
	Если ВариантЭтоНовый И ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, 
		...
		ТекстОшибки = СтрШаблон(НСтр("ka='""%1"" დაკავებულია, საჭიროა სხვა სახელწოდების მითითება.';ru='""%1"" занято, необходимо указать другое Наименование.';en='""%1"" is occupied, you must specify a different name.'"), ВариантНаименование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ВариантНаименование");
		Пакет.Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ВариантЭтоНовый Тогда
		...
		//+Lazare 09.03.2022
		ВариантОбъект.ФиксацияСверху   = Контекст.ФиксацияСверху;
		ВариантОбъект.ФиксацияСлева    = Контекст.ФиксацияСлева;
		//-Lazare 09.03.2022
...
Функция ПредставлениеПривязки(Идентификатор, Синоним = "")
	...
	Если Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
		Представление = НСтр("ru='Из справочника ';ka='ცნობარიდან'")+""""+Синоним+"""";
	ИначеЕсли Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
		Представление = НСтр("ru='Из документа ';ka='Из документа '")+""""+Синоним+"""";
	Иначе
		Представление = НСтр("ru='Из ';ka='Из '")+""""+Синоним+"""";
	КонецЕсли; 
