...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если НЕ ЗначениеЗаполнено(ВариантНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ველი ""სახელწოდება"" არ არის შევსებული';ru='Поле ""Наименование"" не заполнено'"),
			,
			"Наименование");
		Отказ = Истина;
	ИначеЕсли ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, ВариантСсылка, ВариантНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(
				НСтр("ka='""%1"" დაკავებულია, საჭიროა სხვა სახელწოდების მითითება.';ru='""%1"" занято, необходимо указать другое Наименование.'"),
				ВариантНаименование
			),
			,
			"Наименование");
...
Процедура ВариантыОтчетаПередУдалением(Элемент, Отказ)
	...
	Если Не Контекст.ПолныеПраваНаВарианты И НЕ Вариант.АвторТекущийПользователь Тогда
		ТекстПредупреждения = НСтр("ka='ანგარიშგება ""%1""-ის ვარიანტის წასაშლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для удаления варианта отчета ""%1"".'");
	...
	Если Не Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშგების ხელახლა განსაზღვრული ვარიანტის წაშლა შეუძლებელია.';ru='Невозможно удалить предопределенный вариант отчета.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВариантОбразецНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ka='აირჩიეთ გამოსახულების ფაილი';ru='Выберите файл изображения'"));
	ПараметрыДиалога.Вставить("ПредварительныйПросмотр", Истина);
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ka='ყველა სურათი (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf';ru='Все картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf'"));
...
Функция СформироватьСвободноеНаименование(Вариант, ВариантыОтчета)
	ШаблонИмениВарианта = СокрЛП(Вариант.Наименование) +" - "+ НСтр("ka='ასლი';ru='копия'");
...
Процедура ВыполнитьПакет(Результат, Пакет) Экспорт
	...
	Если Пакет.ПроверитьСтраницу1 = Истина Тогда
		...
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ТекстОшибки = НСтр("ka='ველი ""სახელწოდება"" არ არის შევსებული';ru='Поле ""Наименование"" не заполнено'");
		...
		Если Не Пакет.ВариантЭтоНовый Тогда
			...
			Если НЕ ПравоЗаписиВарианта(Вариант, ПравоНастройкиВарианта(Вариант, Контекст.ПолныеПраваНаВарианты), Контекст.ЭтоПолныеПрава, Контекст.РежимНастройки) Тогда
				ТекстОшибки = НСтр("ka='ვარიანტ ""%1""-ის შესაცვლელად არასაკმარისი უფლებებია. აუცილებელია სხვა ვარიანტის არჩევა ან ""სახელწოდება""-ს შეცვლა.';ru='Недостаточно прав для изменения варианта ""%1"". Необходимо выбрать другой вариант или изменить Наименование.'");
...
"Заменить помеченный на удаление вариант отчета?'");
				...
				Иначе
					ТекстВопроса = НСтр("ka='შევცვალოთ ადრე შენახული ანგარიშგების ვარიანტი ""%1""?';ru='Заменить ранее сохраненный вариант отчета ""%1""?'");
	...
	ИначеЕсли Пакет.ПроверитьИЗаписатьСервер Тогда 
		...
		Если Не Пакет.ВариантЭтоНовый И НЕ Контекст.ЭтоПолныеПрава И Контекст.ПолныеПраваНаВарианты И НЕ ВариантАвтор=Контекст.ТекущийПользователь Тогда 
			ТекстОшибки = НСтр("ka='ვარიანტ ""%1""-ის შესაცვლელად არასაკმარისი უფლებებია.';ru='Недостаточно прав для изменения варианта ""%1"".'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, ВариантНаименование);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ВариантНаименование");
			Возврат;
		КонецЕсли; 
...
Процедура СохранитьИЗакрыть()
	...
	Если НЕ Элементы.ДобавитьПривязку.Видимость Тогда
		Состояние(НСтр("ka='გააუქმეთ ან დაასრულეთ ანგარიშგების კონტექსტური გახსნის აღწერილობის რედაქტირება';ru='Отмените или завершите редактирование описания контекстного открытия отчета'"));
		Возврат;
	КонецЕсли; 
...
Процедура ПроверитьИЗаписатьНаСервере(Пакет)
	...
	Если ВариантЭтоНовый И ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, 
		...
		ТекстОшибки = СтрШаблон(НСтр("ka='""%1"" დაკავებულია, საჭიროა სხვა სახელწოდების მითითება.';ru='""%1"" занято, необходимо указать другое Наименование.'"), ВариантНаименование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ВариантНаименование");
		Пакет.Отказ = Истина;
		Возврат;
	КонецЕсли;
