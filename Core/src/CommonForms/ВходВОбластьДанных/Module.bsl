...
Процедура ВойтиВОбластьДанныхПослеВыхода()
	...
	Если НЕ УказаннаяОбластьДанныхЗаполнена(ОбластьДанных) Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='მონაცემთა არჩეული არეალი არ გამოიყენება, გავაგრძელოთ შესვლა?';ru='Выбранная область данных не используется, продолжить вход?';en='The selected area is not used the data input to continue?'"),
