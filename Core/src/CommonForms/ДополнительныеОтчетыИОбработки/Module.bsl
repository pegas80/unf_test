...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ЗаполнениеОбъекта Тогда
		...
		Заголовок = НСтр("ka='ობიექტების შევსების ბრძანებები';ru='Команды заполнения объектов';en='Teams filling objects'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет Тогда
		ЭтоНазначаемыеОбработки = Истина;
		ЭтоОтчеты = Истина;
		Заголовок = НСтр("ka='ანგარიშგებები';ru='Отчеты';en='Reports'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		ЭтоНазначаемыеОбработки = Истина;
		Заголовок = НСтр("ka='დამატებითი ბეჭდური ფორმები';ru='Дополнительные печатные формы';en='Additional plates'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.СозданиеСвязанныхОбъектов Тогда
		ЭтоНазначаемыеОбработки = Истина;
		Заголовок = НСтр("ka='დაკავშირებული ობიექტების შექმნის ბრძანებები';ru='Команды создания связанных объектов';en='create related objects Teams'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ШаблонСообщения Тогда
		ЭтоНазначаемыеОбработки = Истина;
		Заголовок = НСтр("ka='შეტყობინების შაბლონები';ru='Шаблоны сообщений';en='message templates'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительнаяОбработка Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დამატებითი გამართვები (%1)';ru='Дополнительные обработки (%1)';en='Additional treatment (1%)'"), 
	...
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительныйОтчет Тогда
		...
		ЭтоОтчеты = Истина;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დამატებითი ანგარიშგებები (%1)';ru='Дополнительные отчеты (%1)';en='Additional reports (1%)'"), 
			ДополнительныеОтчетыИОбработки.ПредставлениеРаздела(СсылкаРаздела));
	КонецЕсли;
...
Процедура ВыполнитьОбработкуПоПараметрам()
	...
	ИначеЕсли ДанныеОбработки.ВариантЗапуска = ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		Элементы.ПоясняющаяДекорация.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მიმდინარეობს ბრძანება ""%1""-ის შესრულება...';ru='Выполняется команда ""%1""...';en='The command ""%1"" ...'"),
...
Функция ЗапуститьФоновоеЗадание(Знач ВыполняемаяКоманда, Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Доп. отчеты и обработки: Выполнение команды ""%1""';ru='Доп. отчеты и обработки: Выполнение команды ""%1""';en='Extras. reports and processing: Execute Command ""%1""'"),
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='ბრძანება შესრულებულია';ru='Команда выполнена';en='command complete'"),
				,
				ВыполняемаяКоманда.Представление);
		КонецЕсли;
		Если Открыта() Тогда
	...
	Иначе
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Команда ""%1"" не выполнена:';ru='Команда ""%1"" не выполнена:';en='The ""%1"" failed:'"),
			ВыполняемаяКоманда.Представление);
