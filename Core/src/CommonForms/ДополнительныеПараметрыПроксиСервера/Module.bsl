	...
	Для каждого СтрАдрес Из АдресаИсключений Цикл
		Если НЕ ПустаяСтрока(СтрАдрес.АдресСервера) Тогда
			...
			Если НЕ ПустаяСтрока(НедопустимыеСимволы) Тогда
				...
				ТекстСообщения = СтрЗаменить(НСтр("ka='მისამართში მოიძებნა დაუშვებელი სიმბოლოები: %1';ru='В адресе найдены недопустимые символы: %1';en='The address is found invalid characters:% 1'"),
					"%1",
					НедопустимыеСимволы);
