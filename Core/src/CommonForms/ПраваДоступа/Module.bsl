...
Процедура ИсключитьИзГруппы(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяГруппаДоступа) Тогда
		ПоказатьПредупреждение(, НСтр("ka='დაშვების ჯგუფი არჩეული არ არის.';ru='Группа доступа не выбрана.';en='access group is not selected.'"));
		Возврат;
	КонецЕсли;
...
"Редактировать группу доступа могут ответственный за участников группы доступа и администратор.';ka='დაშვების ჯგუფის რედაქტირებისთვის არასაკმარისი უფლებებია. დაშვების ჯგუფის რედაქტირება შეუძლია დაშვების ჯგუფის მონაწილეებზე პასუხისმგებელ პირს და ადმინისტრატორს.'"));
		Возврат;
	КонецЕсли;
...
"не полноправный администратор.';ka='შეუძლებელია მომხმარებლის შეყვანა დაშვების ჯგუფში,"
"ვინაიდან მიმდინარე მომხმარებელი"
"არ არის დაშვების ჯგუფის მონაწილეებზე პასუხისმგებელი და"
"არ არის სრულუფლებიანი ადმინისტრატორი.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Невозможно исключить пользователя из группы доступа,"
"так как текущий пользователь"
"не ответственный за участников группы доступа и"
"не полноправный администратор.';ka='დაშვების ჯგუფიდან მომხმარებლის გარიცხვა შეუძლებელია, ვინაიდან მიმდინარე მომხმარებელი არ არის დაშვების ჯგუფის მონაწილეებზე პასუხისმგებელი და არ არის სრულუფლებიანი ადმინისტრატორი.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ Добавить И НЕ ПользовательВключенВГруппуДоступа(ТекущаяГруппаДоступа) Тогда
		ОписаниеОшибки =
			НСтр("ru='Невозможно исключить пользователя из группы доступа,"
"так как он включен в нее косвенно.';ka='შეუძლებელია დაშვების ჯგუფიდან მომხმარებლის გარიცხვა, ვინაიდან იგი მასში ირიბად არის ჩართული.'");
		Возврат;
	КонецЕсли;
