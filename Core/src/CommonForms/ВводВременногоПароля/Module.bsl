...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ИзменитьПолучателя.Заголовок = НСтр("ka='ნომერის შეცვლა';ru='Изменить номер'");
...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ka='Сервис отправки SMS-сообщений временно недоступен. Повторите попытку позже.';ru='Сервис отправки SMS-сообщений временно недоступен. Повторите попытку позже.'"));
		Отказ = Истина;
	КонецЕсли; 
	...
	Если ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) = 6 Тогда
		...
		Исключение
			...
			Если СтрНайти(ТекстИсключения, "Invalid password") Тогда
				ТекстОшибки = НСтр("ka='Указан неверный пароль';ru='Указан неверный пароль'");
			ИначеЕсли СтрНайти(ТекстИсключения, "MaxAttemptsInputPasswordExceededError") Тогда
				Закрыть(Новый Структура("Состояние, ОписаниеОшибки", "ПарольНеПринят", НСтр("ka='Превышен лимит попыток ввода пароля';ru='Превышен лимит попыток ввода пароля'"))); 
			ИначеЕсли СтрНайти(ТекстИсключения, "PasswordExpiredError") Тогда
				Закрыть(Новый Структура("Состояние, ОписаниеОшибки", "ПарольНеПринят", НСтр("ka='Срок действия пароля истек';ru='Срок действия пароля истек'")));
			Иначе 
				Закрыть(Новый Структура("Состояние, ОписаниеОшибки", "ПарольНеПринят", НСтр("ka='Выполнение операции временно невозможно';ru='Выполнение операции временно невозможно'")));
			КонецЕсли;
		КонецПопытки;		
	ИначеЕсли ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) <> 6 Тогда
		ТекстОшибки = НСтр("ka='Пароль должен состоять из 6 цифр';ru='Пароль должен состоять из 6 цифр'");
	Иначе
		ТекстОшибки = НСтр("ka='Пароль не указан';ru='Пароль не указан'");
	КонецЕсли;
...
Процедура Подключаемый_ОбработчикОбратногоОтсчета()
	...
	Таймер = Таймер - 1;
	Если Таймер >= 0 Тогда
		НадписьОбратногоОтсчета = СтрШаблон(НСтр("ka='Запросить пароль повторно можно будет через %1 сек.';ru='Запросить пароль повторно можно будет через %1 сек.'"), Таймер);
		ПодключитьОбработчикОжидания("Подключаемый_ОбработчикОбратногоОтсчета", 1, Истина);		
	Иначе
		...
		Элементы.ИзменитьПолучателя.Заголовок = НСтр("ka='მისამართის შეცვლა';ru='Изменить адрес'");
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.СпособДоставкиПаролей = "phone" Тогда
		Элементы.Пояснение.Заголовок = НСтр("ka='Пароль отправлен в SMS-сообщении на номер';ru='Пароль отправлен в SMS-сообщении на номер'");
		ТекстКоманды = НСтр("ka='Отправить пароль на %1';ru='Отправить пароль на %1'");
		Элементы.ИзменитьПолучателя.Заголовок = НСтр("ka='ნომერის შეცვლა';ru='Изменить номер'");
		...
		Элементы.Пароль.ПодсказкаВвода = НСтр("ka='Введите пароль из SMS';ru='Введите пароль из SMS'");
	ИначеЕсли Форма.СпособДоставкиПаролей = "email" Тогда
		Элементы.Пояснение.Заголовок = НСтр("ka='Пароль отправлен в письме на адрес';ru='Пароль отправлен в письме на адрес'");
		ТекстКоманды = НСтр("ka='Отправить пароль на %1';ru='Отправить пароль на %1'");
		Элементы.ИзменитьПолучателя.Заголовок = НСтр("ka='მისამართის შეცვლა';ru='Изменить адрес'");
		...
		Элементы.Пароль.ПодсказкаВвода = НСтр("ka='Введите пароль из письма';ru='Введите пароль из письма'");
	КонецЕсли;
