...
Процедура ВесыПосмотретьСписокТоваров(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(Неопределено,НСтр("ka='ბრძანება არ შეიძლება შესრულებული იქნას მითითებული ობიექტისთვის';ru='Команда не может быть выполнена для указанного объекта';en='Command can not be executed for the indicated object'"));
...
Процедура КассыККМВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.КассыККМДатаЗагрузки И ЗначениеЗаполнено(ТекущиеДанные.ДатаЗагрузки) Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ka='საცალო გაყიდვების ანგარიშგება ვერ მოიძებნა.';ru='Отчет о розничных продажах не найден.';en='retail sales report could not be found.'"));
		КонецЕсли;
