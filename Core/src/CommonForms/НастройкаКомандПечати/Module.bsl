...
Процедура ПерейтиКСписку()
	...
	Для Каждого ОкноКлиентскогоПриложения Из ПолучитьОкна() Цикл
		Если ОкноКлиентскогоПриложения.ПолучитьНавигационнуюСсылку() = НавигационнаяСсылка Тогда
			...
			Кнопки.Добавить("Переоткрыть", НСтр("ka='ხელახლად გახსნა';ru='Переоткрыть';en='reopen'"));
			Кнопки.Добавить("Отмена", НСтр("ka='ხელახლად არ გაიხსნას';ru='Не переоткрывать';en='Do not reopen'"));
			ТекстВопроса = 
				НСтр("ru='Список уже открыт. Переоткрыть список, "
"чтобы увидеть изменения меню печать?';ka='სია უკვე გახსნილია. ხელახლა გავხსნათ სია იმისთვის, რომ ვნახოთ მენიუ ""ამობეჭდვა""-ს ცვლილებები?'");
