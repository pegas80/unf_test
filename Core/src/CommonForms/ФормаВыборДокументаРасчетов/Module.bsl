...
Функция ПолучитьТекстЗапросаДокументыРасчетовСПокупателямиПоступление(ОтборПоДоговору)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ПродажаЛицензии) Тогда
		//ТекстЗапроса = ТекстЗапроса + " ОБЪЕДИНИТЬ ВСЕ " +
		ТекстЗапроса = 
		"
		|ВЫБРАТЬ
		...
		|	КОНЕЦ КАК Поле2
		|ИЗ
		|	Документ.ПродажаЛицензии КАК ДанныеДокумента
		|";
		МассивЗапросов.Добавить(ТекстЗапроса);	
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ВозвратДенегОтПФ) Тогда
		//ТекстЗапроса = ТекстЗапроса + " ОБЪЕДИНИТЬ ВСЕ " +
		ТекстЗапроса = 
		" 
		|ВЫБРАТЬ
		...
		|	ДанныеДокумента.Контрагент,
		|	&ДоговорПоУмолчанию,
		|	ДанныеДокумента.Ссылка.Сумма,
		|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка),
		|	ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка),
		|	ВЫБОР
		|		КОГДА ДанныеДокумента.Ссылка.Проведен
		|			ТОГДА 1
		|		КОГДА ДанныеДокумента.Ссылка.ПометкаУдаления
		|			ТОГДА 2
		|		ИНАЧЕ 0
		|	КОНЕЦ
		|ИЗ
		|	Документ.ВозвратДенегОтПФ.Контрагенты КАК ДанныеДокумента
		|";
		МассивЗапросов.Добавить(ТекстЗапроса);	
		
	КонецЕсли;     
	
	Если ПравоДоступа("Чтение", Метаданные.Документы.СборкаЗапасов) Тогда
		//ТекстЗапроса = ТекстЗапроса + " ОБЪЕДИНИТЬ ВСЕ " +
		ТекстЗапроса = 
		"
		|ВЫБРАТЬ
		...
		|	КОНЕЦ
		|ИЗ
		|	Документ.СборкаЗапасов.ДополнительныеРасходы КАК ДанныеДокумента";
		МассивЗапросов.Добавить(ТекстЗапроса);	
	...
	//Малышев 2024-07-04 ONESOFT-3307 - исправление соединения запросов
	ТекстЗапроса = "";
	Если МассивЗапросов.Количество()>=1 Тогда
		ТекстЗапроса = МассивЗапросов[0];
	КонецЕсли;
	
	Для ИндексЗапроса = 1 по МассивЗапросов.Количество()-1 Цикл
		ТекстЗапроса = ТекстЗапроса + " 
		| ОБЪЕДИНИТЬ ВСЕ 
		| "; 
		
		ТекстЗапроса = ТекстЗапроса + МассивЗапросов[ИндексЗапроса];
	КонецЦикла;	
...
Функция ПолучитьТекстЗапросаДокументыРасчетовСПоставщикамиПоступление(ОтборПоДоговору)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.СборкаЗапасов) Тогда
		...
		|	ДанныеДокумента.Договор,
		|	ДанныеДокумента.Сумма,
		|	ДанныеДокумента.Ссылка.ВалютаДокумента,
		|	ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка),
		|	ВЫБОР
		|		КОГДА ДанныеДокумента.Ссылка.Проведен
		|			ТОГДА 1
	...
	//Малышев 2024-07-04 ONESOFT-3307 - универсальнее, как в предыдущей процедуре
	ТекстЗапроса = "";
	Если МассивЗапросов.Количество()>=1 Тогда
		ТекстЗапроса = МассивЗапросов[0];
	КонецЕсли;
