...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение НСтр("ka='დაშვების არასაკმარისი უფლებებია. მიმართეთ ადმინისტრატორს.';ru='Недостаточно прав доступа. Обратитесь к администратору.';en='It is not enough access rights. Contact your administrator.'");
	КонецЕсли;
	...
	Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
		ВызватьИсключение НСтр("ka='პროგრამის საბაზო ვერსიაში გაფართოებები მიუწვდომელია.';ru='Расширения недоступны в базовой версии программы.';en='Extensions are not available in the basic version of the program.'");
	КонецЕсли;
...
Процедура СохранитьКакПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Заголовок = НСтр("ka='კონფიგურაციის გაფართოების შესანახად აირჩიეთ ფაილი';ru='Выберите файл для сохранения расширения конфигурации';en='Select the file to save the extension configuration'");
		Диалог.Фильтр    = НСтр("ka='კონფიგურაციის გაფართოებების ფაილები (*.cfe)|*.cfe|ყველა ფაილი (*.*)|*.*';ru='Файлы расширений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*';en='Extensions configuration files (* .cfe) | * .cfe | All files (* *.) | * *.'");
...
Процедура УдалитьУстаревшиеКэшиРасширений(Команда)
	...
	ПоказатьПредупреждение(, НСтр("ka='გაფართოებების მუშაობის პარამეტრების მოძველებული ვერსიების წაშლა შესრულებულია';ru='Выполнено удаление устаревших версий параметров работы расширений.';en='Done removing outdated versions of operating parameters of extensions.'"));
...
Процедура УдалитьРасширения(ВыделенныеСтроки)
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ka='დარწმუნებული ხართ, რომ გამოყოფილი გაფართოებების წაშლა გსურთ?';ru='Вы уверены, что хотите удалить выделенные расширения?';en='Are you sure you want to delete the selected extension?'", "ru");
	Иначе
		ТекстВопроса = НСтр("ka='დარწმუნებული ხართ, რომ გაფართოების წაშლა გსურთ?';ru='Вы уверены, что хотите удалить расширение?';en='Are you sure you want to delete an extension?'", "ru");
	КонецЕсли;
...
""
"%1';ka='Не удалось удалить по причине:"
""
...
"%1';ka='წაშლილი გაფართოებების აღდგენის მცდელობისას, კიდევ ერთი შეცდომა მოხდა:"
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
		КонецПопытки;
		Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
			ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС
				+ НСтр("ka='წაშლილი გაფართოებები აღდგენილი იქნა.';ru='Удаленные расширения были восстановлены.';en='Remote extensions have been restored.'");
		КонецЕсли;
	КонецПопытки;
...
""
"%1';ka='После удаления, при подготовке оставшихся расширений к работе, произошла ошибка:"
""
"%1'"), ТекстОшибки);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ЗагрузитьРасширениеПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Фильтр = НСтр("ka='კონფიგურაციის გაფართოება';ru='Расширение конфигурации';en='configuration expansion'", "ru")+ " (*.cfe)|*.cfe";
		Диалог.МножественныйВыбор = Контекст.МножественныйВыбор;
		Диалог.ПроверятьСуществованиеФайла = Истина;
		Диалог.Заголовок = НСтр("ka='აირჩიეთ კონფიგურაციის გაფართოების ფაილი';ru='Выберите файл расширения конфигурации';en='Select the configuration file extension'", "ru");
...
"%1';ka='დამატების შემდეგ, სამუშაოდ გაფართოებების მომზადებისას შეცდომა მოხდა:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
		Возврат;
	КонецПопытки;
	СкрытьДлительнуюОперацию();
	...
	Если Контекст.ИдентификаторРасширения = Неопределено Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			ТекстОповещения = НСтр("ka='კონფიგურაციის გაფართოებები დამატებულია';ru='Расширения конфигурации добавлены';en='configuration extensions added'", "ru");
		Иначе
			ТекстОповещения = НСтр("ka='კონფიგურაციის გაფართოება დამატებულია';ru='Расширение конфигурации добавлено';en='Configuration Expanding added'", "ru");
		КонецЕсли;
	Иначе
		ТекстОповещения = НСтр("ka='კონფიგურაციის გაფართოება დამატებულია';ru='Расширение конфигурации обновлено';en='Advanced Configuration updated'", "ru");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(НеподключенныеРасширения) Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			Если СтрНайти(НеподключенныеРасширения, ",") > 0 Тогда
				Пояснение = НСтр("ka='Некоторые расширения не подключаются:';ru='Некоторые расширения не подключаются:';en='Some extensions are not connected:'");
			Иначе
				Пояснение = НСтр("ka='Одно расширение не подключается:';ru='Одно расширение не подключается:';en='One extension is not connected:'");
			КонецЕсли;
			Пояснение = Пояснение + " " + НеподключенныеРасширения;
		Иначе
			Пояснение = НСтр("ka='Расширение не подключается.';ru='Расширение не подключается.';en='The extension is not connected.'");
		КонецЕсли;
	Иначе
		Пояснение = "";
	КонецЕсли;
		...
		Если ИдентификаторРасширения <> Неопределено Тогда
			...
			Если ПрежнееИмяРасширения <> Расширение.Имя Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გაფართოება ""%1""-ის ""%2""-ში ჩატვირთვა არ შეიძლება.';ru='Нельзя загрузить расширение ""%1"" в ""%2"".';en='You can not download the extension ""%1"" to ""%2"".'"),
					Расширение.Имя,
...
"%1';ka='შეცვლილი გაფართოების აღდგენის მცდელობისას კიდევ ერთი შეცდომა მოხდა:"
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
				Иначе
					ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='При попытке удалить добавленные расширения произошла еще одна ошибка:"
"%1';ka='დამატებული გაფართოებების წაშლის მცდელობისას კიდევ ერთი შეცდომა მოხდა:"
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
				КонецЕсли;
			КонецПопытки;
		КонецЕсли;
		ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС
			+ НСтр("ka='დამატებული გაფართოებები წაიშალა.';ru='Добавленные расширения были удалены.';en='Added extensions have been removed.'");
	КонецПопытки;
...
"возможно оно было удалено в другом сеансе.';ka='Текущее расширение не найдено в базе данных,"
...
Процедура ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "")
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("ka='Фоновое задание подготовки расширений не вернуло результат.';ru='Фоновое задание подготовки расширений не вернуло результат.';en='Background job to expand the training did not return results.'");
	КонецЕсли;
...
Процедура ПроверитьДоступностьРазделенныхДанных()
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ka='განუცალკევებელ რეჟიმში გაფართოებები მიუწვდომელია.';ru='Расширения недоступны в неразделенном режиме.';en='Extensions available in undivided mode.'");
	КонецЕсли;
