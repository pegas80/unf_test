...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение НСтр("ka='დაშვების არასაკმარისი უფლებებია. მიმართეთ ადმინისტრატორს.';ru='Недостаточно прав доступа. Обратитесь к администратору.'");
	КонецЕсли;
	...
	Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
		ВызватьИсключение НСтр("ka='პროგრამის საბაზო ვერსიაში გაფართოებები მიუწვდომელია.';ru='Расширения недоступны в базовой версии программы.'");
	КонецЕсли;
...
Процедура СохранитьКакПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Заголовок = НСтр("ka='კონფიგურაციის გაფართოების შესანახად აირჩიეთ ფაილი';ru='Выберите файл для сохранения расширения конфигурации'");
		Диалог.Фильтр    = НСтр("ka='კონფიგურაციის გაფართოებების ფაილები (*.cfe)|*.cfe|ყველა ფაილი (*.*)|*.*';ru='Файлы расширений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*'");
...
Процедура УдалитьУстаревшиеКэшиРасширений(Команда)
	...
	ПоказатьПредупреждение(, НСтр("ka='გაფართოებების მუშაობის პარამეტრების მოძველებული ვერსიების წაშლა შესრულებულია';ru='Выполнено удаление устаревших версий параметров работы расширений.'"));
...
Процедура УдалитьРасширения(ВыделенныеСтроки)
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ka='დარწმუნებული ხართ, რომ გამოყოფილი გაფართოებების წაშლა გსურთ?';ru='Вы уверены, что хотите удалить выделенные расширения?'", "ru");
	Иначе
		ТекстВопроса = НСтр("ka='დარწმუნებული ხართ, რომ გაფართოების წაშლა გსურთ?';ru='Вы уверены, что хотите удалить расширение?'", "ru");
	КонецЕсли;
...
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
		...
		Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
			ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС
				+ НСтр("ka='წაშლილი გაფართოებები აღდგენილი იქნა.';ru='Удаленные расширения были восстановлены.'");
		КонецЕсли;
	КонецПопытки;
...
Процедура ЗагрузитьРасширениеПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Фильтр = НСтр("ka='კონფიგურაციის გაფართოება';ru='Расширение конфигурации'", "ru")+ " (*.cfe)|*.cfe";
		Диалог.МножественныйВыбор = Контекст.МножественныйВыбор;
		Диалог.ПроверятьСуществованиеФайла = Истина;
		Диалог.Заголовок = НСтр("ka='აირჩიეთ კონფიგურაციის გაფართოების ფაილი';ru='Выберите файл расширения конфигурации'", "ru");
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	...
	Если Контекст.ИдентификаторРасширения = Неопределено Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			ТекстОповещения = НСтр("ka='კონფიგურაციის გაფართოებები დამატებულია';ru='Расширения конфигурации добавлены'", "ru");
		Иначе
			ТекстОповещения = НСтр("ka='კონფიგურაციის გაფართოება დამატებულია';ru='Расширение конфигурации добавлено'", "ru");
		КонецЕсли;
	Иначе
		ТекстОповещения = НСтр("ka='კონფიგურაციის გაფართოება დამატებულია';ru='Расширение конфигурации обновлено'", "ru");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(НеподключенныеРасширения) Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			Если СтрНайти(НеподключенныеРасширения, ",") > 0 Тогда
				Пояснение = НСтр("ka='Некоторые расширения не подключаются:';ru='Некоторые расширения не подключаются:'");
			Иначе
				Пояснение = НСтр("ka='Одно расширение не подключается:';ru='Одно расширение не подключается:'");
			КонецЕсли;
			Пояснение = Пояснение + " " + НеподключенныеРасширения;
		Иначе
			Пояснение = НСтр("ka='Расширение не подключается.';ru='Расширение не подключается.'");
		КонецЕсли;
	Иначе
		Пояснение = "";
	КонецЕсли;
		...
		Если ИдентификаторРасширения <> Неопределено Тогда
			...
			Если ПрежнееИмяРасширения <> Расширение.Имя Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='გაფართოება ""%1""-ის ""%2""-ში ჩატვირთვა არ შეიძლება.';ru='Нельзя загрузить расширение ""%1"" в ""%2"".'"),
					Расширение.Имя,
...
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
		...
		ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС
			+ НСтр("ka='დამატებული გაფართოებები წაიშალა.';ru='Добавленные расширения были удалены.'");
	КонецПопытки;
...
Процедура ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "")
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("ka='Фоновое задание подготовки расширений не вернуло результат.';ru='Фоновое задание подготовки расширений не вернуло результат.'");
	КонецЕсли;
...
Процедура ПроверитьДоступностьРазделенныхДанных()
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ka='განუცალკევებელ რეჟიმში გაფართოებები მიუწვდომელია.';ru='Расширения недоступны в неразделенном режиме.'");
	КонецЕсли;
