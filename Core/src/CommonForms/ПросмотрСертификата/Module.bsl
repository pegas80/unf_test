...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Просмотр сертификата: %1';ru='Просмотр сертификата: %1'"), Сертификат.Наименование);
...
Процедура ПроверитьЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено И Результат.Валиден Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატი ძალაშია';ru='Сертификат действителен'"));
	ИначеЕсли Результат.Выполнено И Не Результат.Валиден Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატი უმოქმედოა.';ru='Сертификат не действителен.'"));
	КонецЕсли;
...
Процедура ОткрытьСертификатПослеЭкспортаСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერთიფიკატის გახსნა ვერ მოხერხდა.';ru='Не удалось открыть сертификат.'"));
	КонецЕсли;
