...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Просмотр сертификата: %1';ru='Просмотр сертификата: %1';en='View the certificate% 1'"), Сертификат.Наименование);
...
Процедура ПроверитьЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено И Результат.Валиден Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატი ძალაშია';ru='Сертификат действителен';en='The certificate is valid'"));
	ИначеЕсли Результат.Выполнено И Не Результат.Валиден Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერთიფიკატი უმოქმედოა.';ru='Сертификат не действителен.';en='The certificate is not valid.'"));
	КонецЕсли;
...
Процедура ОткрытьСертификатПослеЭкспортаСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერთიფიკატის გახსნა ვერ მოხერხდა.';ru='Не удалось открыть сертификат.';en='Unable to open the certificate.'"));
	КонецЕсли;
