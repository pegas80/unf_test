...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если ПустаяСтрока(КаталогВыгрузки) Тогда
		...
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='გადმოტვირთვის კატალოგი არ არის მითითებული.';ru='Каталог выгрузки не указан.'"); 
	КонецЕсли;
	...
	Если ПустаяСтрока(ИмяЗагружаемогоФайла) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='ჩატვირთვის ფაილის სახელი არ არის მითითებული.';ru='Имя файла загрузки не указано.'"); 
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ИмяЗагружаемогоФайла) И Не ПустаяСтрока(ИмяФайлаПрайсЛиста) Тогда
		Если Лев(ИмяЗагружаемогоФайла, СтрДлина(ИмяФайлаПрайсЛиста)) = ИмяФайлаПрайсЛиста Тогда
			Результат = Ложь;
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ka='ფაილების სახელები ერთმანეთს არ უნდა კვეთდნენ.';ru='Имена файлов не должны пересекаться.'"); 
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შემოწმებისას აღმოჩენილი იქნა შემდეგი შეცდომები:';ru='При проверке были обнаружены следующие ошибки:'")+ Символы.ПС + ТекстОшибкиОбщий);
	КонецЕсли;
