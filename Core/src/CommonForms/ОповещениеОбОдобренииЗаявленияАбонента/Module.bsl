		...
		Иначе
			...
			Если НЕ РезультатОтветаСервера.УдалосьСоединиться Тогда
				...
				ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПоказатьДиалогОшибкиДоступаВИнтернет(
					НСтр("ka='Не удалось обратиться к серверу регистрации';ru='Не удалось обратиться к серверу регистрации';en='Failed to access the registration server'"));
...
Процедура СформироватьНадпись()
	...
	Если ЭтоВторичноеЗаявление Тогда
		ВидЗаявления = НСтр("ka='Заявление №%1 на изменение подключения';ru='Заявление №%1 на изменение подключения';en='Statement number% 1 in the network connection'");
	Иначе
		ВидЗаявления = НСтр("ka='Заявление №%1 на подключение';ru='Заявление №%1 на подключение';en='Statement number% 1 for connection'");
	КонецЕсли;
	...
	Если Одобрено Тогда
		Результат = НСтр("ka='მოწონებულია';ru='одобрено';en='approved'");
	Иначе
		Результат = НСтр("ka='უარი';ru='отклонено';en='has been rejected'");
	КонецЕсли;
	
	ТекстОповещения = Новый ФорматированнаяСтрока(НСтр("ka=' организации ""%1"" к 1С-Отчетности %2.';ru=' организации ""%1"" к 1С-Отчетности %2.'"));
	...
	Если Одобрено Тогда
		ЧтоНужноСделать = НСтр("ka='Для завершения нужно выполнить несколько простых шагов.';ru='Для завершения нужно выполнить несколько простых шагов.';en='To end the need to perform a few simple steps.'");
	Иначе
		ЧтоНужноСделать = НСтр("ka='Устраните проблему и отправьте новое заявление.';ru='Устраните проблему и отправьте новое заявление.';en='Correct the problem and submit a new application.'");
	КонецЕсли;
...
Процедура СформироватьЗаголовок()
	...
	Если ЭтоВторичноеЗаявление Тогда
		ВидЗаявления = НСтр("ru='Заявление на изменение подключения к 1С-Отчетности ';ka='Заявление на изменение подключения к 1С-Отчетности '");
	Иначе
		ВидЗаявления = НСтр("ru='Заявление на подключение к 1С-Отчетности ';ka='Заявление на подключение к 1С-Отчетности '");
	КонецЕсли;
...
Процедура СформироватьЗаголовокКнопок()
	...
	Если Одобрено Тогда
		Элементы.КнопкаНачатьИспользование.Заголовок = НСтр("ka='გაგრძელდეს ეხლა';ru='Продолжить сейчас';en='continue now!'");
	Иначе
		Элементы.КнопкаНачатьИспользование.Заголовок = НСтр("ka='ახალი განცხადების მომზადება';ru='Подготовить новое заявление';en='Prepare a new application'");
	КонецЕсли;
