		...
		Иначе
			...
			Если НЕ РезультатОтветаСервера.УдалосьСоединиться Тогда
				...
				ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПоказатьДиалогОшибкиДоступаВИнтернет(
					НСтр("ka='Не удалось обратиться к серверу регистрации';ru='Не удалось обратиться к серверу регистрации'"));
...
Процедура СформироватьНадпись()
	...
	Если ЭтоВторичноеЗаявление Тогда
		ВидЗаявления = НСтр("ka='Заявление №%1 на изменение подключения';ru='Заявление №%1 на изменение подключения'");
	Иначе
		ВидЗаявления = НСтр("ka='Заявление №%1 на подключение';ru='Заявление №%1 на подключение'");
	КонецЕсли;
	...
	Если Одобрено Тогда
		Результат = НСтр("ka='მოწონებულია';ru='одобрено'");
	Иначе
		Результат = НСтр("ka='უარი';ru='отклонено'");
	КонецЕсли;
	...
	Если Одобрено Тогда
		ЧтоНужноСделать = НСтр("ka='Для завершения нужно выполнить несколько простых шагов.';ru='Для завершения нужно выполнить несколько простых шагов.'");
	Иначе
		ЧтоНужноСделать = НСтр("ka='Устраните проблему и отправьте новое заявление.';ru='Устраните проблему и отправьте новое заявление.'");
	КонецЕсли;
...
Процедура СформироватьЗаголовокКнопок()
	...
	Если Одобрено Тогда
		Элементы.КнопкаНачатьИспользование.Заголовок = НСтр("ka='გაგრძელდეს ეხლა';ru='Продолжить сейчас'");
	Иначе
		Элементы.КнопкаНачатьИспользование.Заголовок = НСтр("ka='ახალი განცხადების მომზადება';ru='Подготовить новое заявление'");
	КонецЕсли;
