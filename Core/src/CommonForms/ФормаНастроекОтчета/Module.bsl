...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("НастройкиОтчета", НастройкиОтчета) Тогда
		ВызватьИсключение НСтр("ka='სამსახურებრივი პარამეტრი ""ანგარიშგებისგამართვები"" არ არის გადაცემული.';ru='Не передан служебный параметр ""НастройкиОтчета"".'");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение НСтр("ka='სამსახურებრივი პარამეტრი ""ვარიანტისახელწოდება"" არ არის შეყვანილი.';ru='Не передан служебный параметр ""ВариантНаименование"".'");
	КонецЕсли;
	...
	Если ИдентификаторТекущегоУзлаКД <> Неопределено Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение НСтр("ka='სამსახურებრივი პარამეტრი ""სათაური"" არ არის გადაცემული.';ru='Не передан служебный параметр ""Заголовок"".'");
		КонецЕсли;
		Если Не Параметры.Свойство("ТипТекущегоУзлаКД", ТипТекущегоУзлаКД) Тогда
			ВызватьИсключение НСтр("ka='სამსახურებრივი პარამეტრი ""მშმიმდინარეკვანძისტიპი"" არ არის გადაცემული.';ru='Не передан служебный параметр ""ТипТекущегоУзлаКД"".'");
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ВариантНаименование = НастройкиОтчета.Наименование;
		КонецЕсли;
		Заголовок = НСтр("ka='ანგარიშგების გამართვები';ru='Настройки отчета'") + " """ + ВариантНаименование + """";
	КонецЕсли;
	...
	Иначе
		Для Каждого ЭлементСпискаДубльВФорме Из СписокЗначенийВФорме Цикл
			Если ЭлементСпискаДубльВФорме.Значение = Значение И ЭлементСпискаДубльВФорме <> ЭлементСпискаВФорме Тогда
				Состояние(НСтр("ka='აღმოჩენილია დუბლირებადი ჩანაწერები. რედაქტირება გაუქმებულია.';ru='Обнаружены дублирующиеся записи. Редактирование отменено.'"));
...
Процедура Подключаемый_СписокСПодбором_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("ka='ელემენტი დამატებულია სიაში';ru='Элемент добавлен в список'");
		Иначе
			ЗаголовокОповещения = НСтр("ka='ელემენტები დამატებულია სიაში';ru='Элементы добавлены в список'");
		КонецЕсли;
...
Процедура СтруктураВариантаДобавитьГруппировкуПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	Если ДоступноеПолеКД = "<>" Тогда
		...
		Представление = НСтр("ka='<Детальные записи>';ru='<Детальные записи>'");
	Иначе
...
Процедура ДобавитьТаблицуИлиДиаграмму(ТипЭлемента)
	...
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		...
		СтрокаТаблицы.Представление = НСтр("ka='დიაგრამა';ru='Диаграмма'");
		...
		ВложеннаяСтрока.Представление = НСтр("ka='წერტილები';ru='Точки'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ka='სერიები';ru='Серии'");
		
	ИначеЕсли СтрокаТаблицы.Тип = "Таблица" Тогда
		
		СтрокаТаблицы.Представление = НСтр("ka='ცხრილი';ru='Таблица'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ka='Строки';ru='Строки'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ka='სვეტები';ru='Колонки'");
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ka='აწყობები.xml';ru='Настройки.xml'"), Истина);
КонецПроцедуры
...
Функция ВыполнитьШаг(ВыборПользователя, Сценарий)
	...
	Иначе
		Сценарий.ПричинаОтказа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='შეცდომა სცენარში: უცნობი ნაბიჯი ""%1"".';ru='Ошибка в сценарии: Неизвестный шаг ""%1"".'"), Сценарий.ТекущийШаг);
		Возврат Ложь;
...
Функция ШагОпределитьВыделенныеСтроки(ВыборПользователя, Сценарий)
	...
	Если Сценарий.СтрокиДерева.Количество() = 0 Тогда
		Сценарий.ПричинаОтказа = НСтр("ka='შეარჩიეთ ელემენტები.';ru='Выберите элементы.'");
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Сценарий.ТекущаяСтрока = Неопределено Тогда
		Если Сценарий.Действие = "ИзменитьГруппу" Тогда
			Сценарий.ПричинаОтказа = НСтр("ka='აირჩიეთ ჯგუფი.';ru='Выберите группу.'");
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьВыделеннуюСтроку(Сценарий, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр Тогда
		Если Сценарий.Действие = "Переместить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ka='პარამეტრების გადაადგილება ვერ მოხდება.';ru='Параметры не могут быть перемещены.'");
		ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
			Сценарий.ПричинаОтказа = НСтр("ka='პარამეტრები არ შეიძლება, რომ ჯგუფების მონაწილეები იყოს.';ru='Параметры не могут быть участниками групп.'");
		ИначеЕсли Сценарий.Действие = "Удалить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ka='პარამეტრები ვერ წაიშლება.';ru='Параметры не могут быть удалены.'");
		КонецЕсли;
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Сценарий.ТекущийРодитель <> Родитель Тогда
			Если Сценарий.Действие = "Переместить" Тогда
				Сценарий.ПричинаОтказа = НСтр("ka='არჩეული ელემენტები არ შეიძლება გადაადგილებული იქნას, ვინაიდან ისინი სხვდასხვა მშობლებს ეკუთვნის.';ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.'");
			ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
				Сценарий.ПричинаОтказа = НСтр("ka='არჩეული ელემენტები არ შეიძლება დაჯგუფებული იქნას, ვინაიდან ისინი სხვადასხვა მშობლებს ეკუთვნიან.';ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.'");
			КонецЕсли;
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Процедура СтруктураВариантаИзменитьУзел(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы, ИмяСтраницы)
	...
	ШаблонЗаголовка = НСтр("ka='ანგარიშგება ""%2""-ის გამართვა %1';ru='Настройка %1 отчета ""%2""'");
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		ПредставлениеУзла = НСтр("ka='დიაგრამები';ru='диаграммы'");
	Иначе
		ПредставлениеУзла = НСтр("ka='დაჯგუფებები';ru='группировки'");
	КонецЕсли;
...
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(2, НСтр("ka='ანგარიშგების სათაურში';ru='В шапке отчета'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(1, НСтр("ka='მხოლოდ ალამი ანგარიშგების სათაურში';ru='Только флажок в шапке отчета'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(4, НСтр("ka='ანგარიშგების გამართვებში';ru='В настройках отчета'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(3, НСтр("ka='მხოლოდ ალამი ანგარიშგების გამართვებში';ru='Только флажок в настройках отчета'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить(5, НСтр("ka='არ იყოს ნაჩვენები';ru='Не показывать'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОформлениеИзменитьЭлемент(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ანგარიშგება ""%1""-ის პირობითი გაფორმების ელემენტი';ru='Элемент условного оформления отчета ""%1""'"), ВариантНаименование));
...
Процедура ТаблицыПолейРазгруппировать(ТаблицаИмя)
	...
	Если ВыделенныеСтроки.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ერთი ჯგუფი.';ru='Выберите одну группу.'"));
		Возврат;
	КонецЕсли;
	...
	Если ГруппаДерева = Неопределено Или Не ГруппаДерева.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ka='აირჩიეთ ჯგუფი.';ru='Выберите группу.'"));
		Возврат;
	КонецЕсли;
...
Функция ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя, СтрокаТаблицы = Неопределено, ИдентификаторУзлаКД = Неопределено)
	Если ЭтотОбъект.РасширенныйРежим = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ცხრილ ""%1""-ის კვანძების შეცვლას მხარდაჭერა არ გააჩნია.';ru='Изменение узлов таблицы ""%1"" не поддерживается.'"), ТаблицаИмя);
		КонецЕсли;
	Иначе
...
Процедура ВидимостьДоступностьКорректность(ИмяСобытия = "")
	Если ИмяСобытия = "ПриСозданииНаСервере" Тогда
		Если РежимИзмененияУзлаВарианта Тогда
			...
			Элементы.ЗакрытьИСформировать.Заголовок = НСтр("ka='რედაქტირების დასრულება';ru='Завершить редактирование'");
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Если ВидимостьВыбранныхПолей И ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ka='ველები და დახარისხებები';ru='Поля и сортировки'");
	ИначеЕсли ВидимостьВыбранныхПолей Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ka='ველები';ru='Поля'");
	ИначеЕсли ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ka='დახარისხება';ru='Сортировки'");
	КонецЕсли;
...
Функция ЗарегистрироватьЭлементСтруктурыОтчета(СтрокаДерева, СтрокиРодителя)
	...
	Если СтрокаФормы.Тип = "Настройки" Тогда
		...
		СтрокаФормы.Представление = НСтр("ka='ანგარიშგება';ru='Отчет'");
...
Функция ЗарегистрироватьЭлементыСортировки()
	...
	СтрокаРаздел.Представление = НСтр("ka='დახარისხება';ru='Сортировки'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоЭлементПорядка" Тогда
			СтрокаТаблицы.Представление = НСтр("ka='ავტო (მშობლის დახარისხებები)';ru='Авто (сортировки родителя)'");
...
Процедура ЗарегистрироватьЭлементыСоставаГруппировки(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено)
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоПолеГруппировки" Тогда
			СтрокаТаблицы.Представление  = НСтр("ka='ავტო (ყველა ველში)';ru='Авто (по всем полям)'");
...
Функция ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	...
	Если НаборСтрок = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = НСтр("ka='ველები';ru='Поля'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоВыбранноеПоле" Тогда
			Представление = НСтр("ka='ავტო (მშობლის ველები)';ru='Авто (поля родителя)'");
			ИндексКартинки = 6;
		Иначе
...
Процедура ЗарегистрироватьПараметры(Информация)
	...
	СтрокаРаздел.Представление = НСтр("ka='პარამეტრები';ru='Параметры'");
...
Процедура ЗарегистрироватьЭлементыОтборов(Информация, НаборСтрокВарианта = Неопределено, НаборСтрокФормы = Неопределено)
	...
	Если НаборСтрокФормы = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = НСтр("ka='შერჩევები';ru='Отборы'");
