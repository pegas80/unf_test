...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтатусОтправки = НСтр("ka='შეტყობინება იგზავნება...';ru='Сообщение отправляется...';en='The message is sent ...'");
	...
	Заголовок = НСтр("ka='SMS-ის გაგზავნა ტელეფონზე';ru='Отправка SMS на телефон';en='Sending an SMS to your phone'") + " " + НомераПолучателей;
КонецПроцедуры
...
Процедура Отправить(Команда)
	...
	Если СтрДлина(ТекстСообщения) = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='აუცილებელია შეტყობინების ტექსტის შეყვანა';ru='Необходимо ввести текст сообщения';en='You must enter the message'"));
		Возврат;
	КонецЕсли;
...
"%1"
"Подробности см. в журнале регистрации.';ka='Сообщение не было отправлено:"
"%1"
...
"%1.';ka='Отправка сообщения выполнена. Состояние доставки:"
"%1.'");
...
Функция СтатусДоставки(ИдентификаторСообщения)
	...
	СтатусыДоставки.Вставить("Ошибка", НСтр("ka='произошла ошибка при подключении к провайдеру SMS';ru='произошла ошибка при подключении к провайдеру SMS';en='An error occurred while connecting to the SMS provider'"));
	СтатусыДоставки.Вставить("НеОтправлялось", НСтр("ka='сообщение не отправлялось провайдером';ru='сообщение не отправлялось провайдером';en='Do not send a message provider'"));
	СтатусыДоставки.Вставить("Отправляется", НСтр("ka='выполняется отправка провайдером';ru='выполняется отправка провайдером';en='send the same provider'"));
	СтатусыДоставки.Вставить("Отправлено", НСтр("ka='გაგზავნილია პროვაიდერის მიერ';ru='отправлено провайдером';en='sent to the provider'"));
	СтатусыДоставки.Вставить("НеОтправлено", НСтр("ka='сообщение не отправлено провайдером';ru='сообщение не отправлено провайдером';en='Message not sent to the provider'"));
	СтатусыДоставки.Вставить("Доставлено", НСтр("ka='სეტყობინება მიწოდებულია';ru='сообщение доставлено';en='message delivered'"));
	СтатусыДоставки.Вставить("НеДоставлено", НСтр("ka='сообщение не доставлено';ru='сообщение не доставлено';en='Message could not be delivered'"));
