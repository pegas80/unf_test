...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтатусОтправки = НСтр("ka='შეტყობინება იგზავნება...';ru='Сообщение отправляется...'");
	...
	Заголовок = НСтр("ka='SMS-ის გაგზავნა ტელეფონზე';ru='Отправка SMS на телефон'") + " " + НомераПолучателей;
КонецПроцедуры
...
Процедура Отправить(Команда)
	...
	Если СтрДлина(ТекстСообщения) = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='აუცილებელია შეტყობინების ტექსტის შეყვანა';ru='Необходимо ввести текст сообщения'"));
		Возврат;
	КонецЕсли;
...
Функция СтатусДоставки(ИдентификаторСообщения)
	...
	СтатусыДоставки.Вставить("Ошибка", НСтр("ka='произошла ошибка при подключении к провайдеру SMS';ru='произошла ошибка при подключении к провайдеру SMS'"));
	СтатусыДоставки.Вставить("НеОтправлялось", НСтр("ka='сообщение не отправлялось провайдером';ru='сообщение не отправлялось провайдером'"));
	СтатусыДоставки.Вставить("Отправляется", НСтр("ka='выполняется отправка провайдером';ru='выполняется отправка провайдером'"));
	СтатусыДоставки.Вставить("Отправлено", НСтр("ka='გაგზავნილია პროვაიდერის მიერ';ru='отправлено провайдером'"));
	СтатусыДоставки.Вставить("НеОтправлено", НСтр("ka='сообщение не отправлено провайдером';ru='сообщение не отправлено провайдером'"));
	СтатусыДоставки.Вставить("Доставлено", НСтр("ka='სეტყობინება მიწოდებულია';ru='сообщение доставлено'"));
	СтатусыДоставки.Вставить("НеДоставлено", НСтр("ka='сообщение не доставлено';ru='сообщение не доставлено'"));
