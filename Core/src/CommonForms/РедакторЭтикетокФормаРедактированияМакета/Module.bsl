...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Иначе
		...
		Если МакетИзменен Тогда
			Отказ = Истина;
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ka='დავხუროთ შენახვის გარეშე?';ru='Закрыть без сохранения?'"), РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
...
Процедура ШагСеткиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение<>ШагСетки Тогда
		...
		Если УпраздняемыеПоля.Количество()>0 Тогда
			...
			СтрокаВопрос = НСтр("ka='ველები:%Поля%საკმაოდ მცირე ზომები გააჩნიათ.%ПереносСтроки%გავაგრძელოთ ბადის ნაბიჯის ცვლილება აღნიშნული ველების წაშლით?';ru='Поля:%Поля%имеют слишком маленькие размеры.%ПереносСтроки%Продолжить изменение шага сетки, удалив указанные поля?'");
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	Если Поля.Количество()>0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьИзФайлаЗавершение", ЭтотОбъект), НСтр("ka='ეტიკეტის მაკეტი ჩავტვირთოთ ფაილიდან?';ru='Загрузить макет этикетки из файла?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ВыборФайла();
	КонецЕсли;
...
Процедура СохранитьВФайл(Команда)
	...
	ДОФ.Фильтр = НСтр("ka='XML-ფაილი';ru='XML-файл'") + "(*.xml)|*.xml";
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ka='ფაილ-მაკეტის შენახვა';ru='Сохранение файла-макета'");
...
Процедура ОчиститьМакет(Команда)
	...
	Если Поля.Количество()>0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьМакетЗавершение", ЭтотОбъект), НСтр("ka='მაკეტი გასუფთავდეს?';ru='Очистить макет?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ОчисткаМакета();
		МакетИзменен = Истина;
	КонецЕсли;
...
Процедура ВыборФайла()
	...
	ДОФ.Фильтр = НСтр("ka='XML-ფაილი';ru='XML-файл'") + "(*.xml)|*.xml";
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ka='აირჩიეთ მაკეტის ფაილი';ru='Выберите файл макета'");
...
Процедура ПредложитьПользователюПродолжитьРедактирование(ОписаниеПоля, РедактируемоеПоле)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПредложитьПользователюПродолжитьРедактированиеЗавершение", ЭтотОбъект, ПередаваемыеПараметры),
					НСтр("ka='ჩაწერა შეუძლებელია! რეკვიზიტ ""ველის სახელი""-ს მიმდინარე მნიშვნელობა უკვე გამოიყენება. გავაგრძელოთ რედაქტირება?';ru='Запись невозможна! Текущее значение реквизита ""Имя поля"" уже используется. Продолжить редактирование?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОпроситьПользователяОбУдаленииПересекаемыхПолей(ПередаваемыеПараметры)
	...
	Если ПередаваемыеПараметры.Свойство("РедактируемоеПоле") Тогда
		ТекстВопроса = НСтр("ka='რედაქტირებადი ველის ახალი საზღვრები კვეთს ველებს:%СписокПолей%გავაუქმოთ შეტანილი ცვლილებები?';ru='Новые границы редактируемого поля пересекаются с полями:%СписокПолей%Отменить внесенные изменения?'");
	Иначе
		ТекстВопроса = НСтр("ka='შესაქმნელი ველი კვეთს უკვე არსებულ ველებს:%СписокПолей%გავაუქმოთ შეტანილი ცვლილებები?';ru='Создаваемое поле пересекается с уже существующими полями:%СписокПолей%Отменить внесенные изменения?'");
	КонецЕсли;
