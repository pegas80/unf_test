	...
	Иначе
		Если ОжиданиеИмяКнопки <> "" Тогда
			НовыйЗаголовок = (
				...
				+ " ("
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დარჩენილია %1 წამი';ru='осталось %1 сек.';en='It is remaining% 1 seconds.'"), Строка(ОжиданиеСчетчик))
				+ ")");
...
Функция СтандартныйНабор(Кнопки)
	...
	Если Кнопки = "РежимДиалогаВопрос.ДаНет" Тогда
		Результат.Добавить("КодВозвратаДиалога.Да",  НСтр("ka='დიახ';ru='Да';en='Yes'"));
		Результат.Добавить("КодВозвратаДиалога.Нет", НСтр("ka='არა';ru='Нет';en='Not'"));
	ИначеЕсли Кнопки = "РежимДиалогаВопрос.ДаНетОтмена" Тогда
		Результат.Добавить("КодВозвратаДиалога.Да",     НСтр("ka='დიახ';ru='Да';en='Yes'"));
		Результат.Добавить("КодВозвратаДиалога.Нет",    НСтр("ka='არა';ru='Нет';en='Not'"));
		Результат.Добавить("КодВозвратаДиалога.Отмена", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
	ИначеЕсли Кнопки = "РежимДиалогаВопрос.ОК" Тогда
		Результат.Добавить("КодВозвратаДиалога.ОК", НСтр("ka='ОК';ru='ОК';en='OK'"));
	ИначеЕсли Кнопки = "РежимДиалогаВопрос.ОКОтмена" Тогда
		Результат.Добавить("КодВозвратаДиалога.ОК",     НСтр("ka='ОК';ru='ОК';en='OK'"));
		Результат.Добавить("КодВозвратаДиалога.Отмена", НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
	ИначеЕсли Кнопки = "РежимДиалогаВопрос.ПовторитьОтмена" Тогда
		Результат.Добавить("КодВозвратаДиалога.Повторить", НСтр("ka='გამეორება';ru='Повторить';en='repeat'"));
		Результат.Добавить("КодВозвратаДиалога.Отмена",    НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'"));
	ИначеЕсли Кнопки = "РежимДиалогаВопрос.ПрерватьПовторитьПропустить" Тогда
		Результат.Добавить("КодВозвратаДиалога.Прервать",   НСтр("ka='შეწყვეტა';ru='Прервать';en='abort'"));
		Результат.Добавить("КодВозвратаДиалога.Повторить",  НСтр("ka='გამეორება';ru='Повторить';en='repeat'"));
		Результат.Добавить("КодВозвратаДиалога.Пропустить", НСтр("ka='გამოტოვება';ru='Пропустить';en='Skip'"));
	КонецЕсли;
