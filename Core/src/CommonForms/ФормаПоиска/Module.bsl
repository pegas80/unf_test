	...
	Если Не ПустаяСтрока(Параметры.ПереданнаяСтрокаПоиска) Тогда
		...
		Если РезультатыПоиска.Количество() <> 0 Тогда
			...
			ПоказаныРезультатыСПо = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='ნაჩვენებია %1 - %2 %3-დან';ru='Показаны %1 - %2 из %3'"),
		...
		Иначе
			ПоказаныРезультатыСПо = НСтр("ka='ნაპოვნი არ არის';ru='Не найдено'");
			Элементы.Далее.Доступность = Ложь;
			Элементы.Назад.Доступность = Ложь;
		КонецЕсли;
	Иначе
...
Процедура Искать(Направление, Отказ = Неопределено)
	...
	Если ПустаяСтрока(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეიყვანეთ, რაც საპოვნია.';ru='Введите, что нужно найти.'"), , "СтрокаПоиска");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხორციელდება ძებნა ""%1""...';ru='Выполняется поиск ""%1""...'"), СтрокаПоиска));
	...
	Если РезультатыПоиска.Количество() > 0 Тогда
		...
		Если Направление = 0 И Результат.ТекущаяПозиция = 0 И Результат.СлишкомМногоРезультатов Тогда
			ПоказатьПредупреждение(, НСтр("ka='ძალიან ბევრი შედეგია, დააზუსტეთ მოთხოვნა:';ru='Слишком много результатов, уточните запрос.'"));
		КонецЕсли;
...
Процедура СформироватьПредставлениеОбластиПоиска()
	...
	Если Не ИскатьВРазделах Или ОбластиПоиска.Количество() = 0 Тогда
		Элементы.ОбластиПоискаПредставление.Заголовок = НСтр("ka='ყველგან';ru='Везде'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		Элементы.ОбластиПоискаПредставление.Заголовок = НСтр("ka='არჩეულ ნაწილებში';ru='В выбранных разделах'");
	КонецЕсли;
