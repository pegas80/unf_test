...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Элементы.Профили.Заголовок = НСтр("ka='გარე მომხმარებლის პროფილები';ru='Профили внешнего пользователя'");
	Иначе
		Элементы.Профили.Заголовок = НСтр("ka='მომხმარებლის პროფილები';ru='Профили пользователя'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение(НСтр("ka='Не найден профиль ""Рабочее место кассира""';ru='Не найден профиль ""Рабочее место кассира""'"));
	КонецЕсли;
	// Конец УНФ
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ВладелецФормы) <> Тип("УправляемаяФорма")
	 Или ВладелецФормы.Окно <> Окно Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაშვების უფლებები (%1)';ru='Права доступа (%1)'"), Строка(Параметры.Пользователь));
	КонецЕсли;
