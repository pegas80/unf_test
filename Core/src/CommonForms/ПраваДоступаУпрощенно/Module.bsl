...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Элементы.Профили.Заголовок = НСтр("ka='გარე მომხმარებლის პროფილები';ru='Профили внешнего пользователя';en='Profiles external user'");
	Иначе
		Элементы.Профили.Заголовок = НСтр("ka='მომხმარებლის პროფილები';ru='Профили пользователя';en='user profiles'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение(НСтр("ka='Не найден профиль ""Рабочее место кассира""';ru='Не найден профиль ""Рабочее место кассира""';en='Profile could not be found ""Workplace cashier""'"));
	КонецЕсли;
	// Конец УНФ
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ВладелецФормы) <> Тип("УправляемаяФорма")
	 Или ВладелецФормы.Окно <> Окно Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დაშვების უფლებები (%1)';ru='Права доступа (%1)';en='Permissions (1%)'"), Строка(Параметры.Пользователь));
	КонецЕсли;
...
"Очистить другие профили?';ka='Для профиля рабочего места кассира выбор других профилей недоступен."
...
"Снять пометку с профиля рабочего места кассира?';ka='Профиль рабочего места кассира не совместим с другими профилями."
...
Процедура ПриИзмененииТекущегоПрофиля(Знач Форма)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ИмяТекущегоПрофиля = "Продажи" ИЛИ ИмяТекущегоПрофиля = "გაყიდვა" Тогда
	...
	Если Элементы.Найти("ВидыДоступаИзменить") <> Неопределено Тогда
		Элементы.ВидыДоступаИзменить.Доступность                 =    РедактированиеТекущихОграничений; 
	КонецЕсли;
	//++Lazare ONESOFT-3152
