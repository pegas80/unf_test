...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если СчитывательМагнитныхКарт Тогда
		...
		Если ПрефиксДрайвера > 0 И СуффиксДрайвера > 0 И ПрефиксДрайвера = СуффиксДрайвера Тогда
			ТекстСообщения = НСтр("ka='პირველი გამოყენებული ბილიკის პრეფიქსი უკანასკნელად გამოყენებული ბილიკის სუფიქსს ემთხვევა';ru='Префикс первой используемой дорожки совпадает с суффикс последней используемой дорожки'");
		...
		ИначеЕсли НастроеноДорожек = 0 Тогда
			ТекстСообщения = НСтр("ka='აუცილებელია წამკითხველის თუნდაც ერთი ბილიკის გამოყენების მითითება';ru='Необходимо указать использование хотя бы одной дорожки для считывателя'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли ДорожкаСПустымСуффиксом Тогда
			ТекстСообщения = НСтр("ka='ყოველი გამოყენებული ბილიკისთვის მითითებული უნდა იყოს სუფიქსი';ru='Для каждой используемой дорожки должен быть указан суффикс'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
Процедура НачатьЗапускПриложенияЗавершение(Результат, Параметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დაწყებულია დრაივერის ძირითადი მიწოდების დაყენება.';ru='Начата установка основной поставки драйвера.'")); 
...
Процедура ТестУстройстваЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ТекстСообщения = ?(РезультатВыполнения.Результат,  НСтр("ka='Тест успешно выполнен. %ДополнительноеОписание%';ru='Тест успешно выполнен. %ДополнительноеОписание%'"),
	                               НСтр("ka='Тест не пройден. %ДополнительноеОписание%';ru='Тест не пройден. %ДополнительноеОписание%'"));
...
Процедура ДополнительноеДействиеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ТекстСообщения = ?(РезультатВыполнения.Результат,  НСтр("ka='Операция выполнена успешно.';ru='Операция выполнена успешно.'"),
	                               НСтр("ka='Ошибка выполнения операции.';ru='Ошибка выполнения операции.'") + Символы.НПП + РезультатВыполнения.ВыходныеПараметры[1]);
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	Если ЧтениеXML.Имя = "Settings" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
		Пока ЧтениеXML.Прочитать() Цикл  
			...
			Если ЧтениеXML.Имя = "Parameter" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
				...
				ИначеЕсли Элементы.Найти(ПараметрИмя) = Неопределено Тогда
					...
					Если БазоваяГруппа = Неопределено Тогда
						...
						БазоваяГруппа.Заголовок = НСтр("ka='პარამეტრები';ru='Параметры'");
			...
			Если ЧтениеXML.Имя = "Page" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ЗаголовокСтраницы = ?(ПустаяСтрока(ЗаголовокСтраницы), НСтр("ka='პარამეტრები';ru='Параметры'"), ЗаголовокСтраницы);
			...
			Если ЧтениеXML.Имя = "Group" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
				...
				ЗаголовокГруппы = ?(ПустаяСтрока(ЗаголовокГруппы), НСтр("ka='პარამეტრები';ru='Параметры'"), ЗаголовокГруппы);
...
Процедура НачатьВыполнениеДополнительнойКомандыЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ИнтеграционныйКомпонент И НЕ ОсновнойДрайверУстановлен Тогда
			ДрайверУстановлен = НСтр("ka='Установлен интеграционный компонент';ru='Установлен интеграционный компонент'");
			ВерсияДрайвера = НСтр("ka='გაურკვეველია';ru='Не определена'");
		...
		Элементы.Драйвер.ЦветТекста = ?(ВерсияДрайвера = НСтр("ka='გაურკვეველია';ru='Не определена'"), ЦветОшибки, ЦветТекста);
		Элементы.Версия.ЦветТекста  = Элементы.Драйвер.ЦветТекста ;
		Элементы.НаименованиеДрайвера.ЦветТекста = ?(НаименованиеДрайвера = НСтр("ka='გაურკვეველია';ru='Не определено'"), ЦветОшибки, ЦветТекста);
		Элементы.ОписаниеДрайвера.ЦветТекста     = ?(ОписаниеДрайвера     = НСтр("ka='გაურკვეველია';ru='Не определено'"), ЦветОшибки, ЦветТекста);
	...
	Иначе
		...
		ВерсияДрайвера  = НСтр("ka='გაურკვეველია';ru='Не определена'");
		Если НЕ ПустаяСтрока(ДрайверСообщение) И ДрайверУстановлен = НСтр("ka='დაყენებულია';ru='Установлен'") Тогда
	...
	Если Не Элементы.УстройствоПодключено.Видимость Тогда
		Элементы.УстановитьДрайвер.Видимость = Не (ДрайверУстановлен = НСтр("ka='დაყენებულია';ru='Установлен'"));
		Элементы.ТестУстройства.Видимость = (НЕ ДрайверУстановлен = НСтр("ka='არ არის დაყენებული';ru='Не установлен'")) 
...
#Если Не ВебКлиент Тогда  
	Если ТребуетсяПереустановка Тогда
		...
		Элементы.ДекорацияПереустановитьДрайвер.Заголовок = НСтр("ka='1С:საწარმოს გადატვირთვის შემდეგ, მოხდება დრაივერის ხელახლა დაყენება.';ru='Драйвер будет переустановлен после перезагрузки ""1С:Предприятие"".'");
	...
	ИначеЕсли Не ПустаяСтрока(ВерсияДрайвераВМакете) И Не ПустаяСтрока(ВерсияДрайвера) 
		И Не ВерсияДрайвераВМакете = ВерсияДрайвера И ДрайверУстановлен = НСтр("ka='დაყენებულია';ru='Установлен'")
		И Не Элементы.УстройствоПодключено.Видимость Тогда
		Элементы.Переустановка.Видимость = Истина;
		ТекстСообщения = НСтр("ka='კონფიგურაციაში (%ВКонфигурации%) დრაივერის ვერსია განსხვავდება დაყენებულისგან.';ru='Версия драйвера в конфигурации (%ВКонфигурации%) отличается от установленного.'");
