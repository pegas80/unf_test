...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И Параметры.ЗапрашиватьПараметрыАдминистрированияКластера Тогда
		ВызватьИсключение НСтр("ka='სერვერების კლასტერის პარამეტრების გამართვა ხელმისაწვდომია მხოლოდ მუშაობის კლიენტ-სერვერულ რეჟიმში.';ru='Настройка параметров кластера серверов доступна только в клиент-серверном режиме работы.';en='Setting the parameters of the cluster servers is only available in client-server mode.'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.ЗапрашиватьПараметрыАдминистрированияКластера
		И ОбщегоНазначенияКлиентСервер.ЭтоOSXКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='Подключение к кластеру серверов недоступно в клиенте под управлением ОС X.';ru='Подключение к кластеру серверов недоступно в клиенте под управлением ОС X.';en='Connecting to a server cluster is not available in the client running X.'");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если ПользовательИБ = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მითითებულ მომხმარებელს საინფორმაციო ბაზასთან წვდომა არ გააჩნია.';ru='Указанный пользователь не имеет доступа к информационной базе.';en='The specified user does not have access to the database.'"),,
				ИмяПоля,,Отказ);
			Возврат;
		КонецЕсли;
		
		Если Не Пользователи.ЭтоПолноправныйПользователь(ПользовательИБ, Истина) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მომხმარებელს არ გააჩნია ადმინისტრაციული უფლებები.';ru='У пользователя нет административных прав.';en='The user does not have administrative rights.'"),,
				ИмяПоля,,Отказ);
			Возврат;
		КонецЕсли;
