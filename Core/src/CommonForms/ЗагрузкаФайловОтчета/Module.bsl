			...
			Если ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС")
				...
				Если ОпределитьТипФайлаНДС(ВыбранныйФайл.Имя) = Неопределено
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ka='Файл ""%1"" не может быть загружен в составе реестра НДС.';ru='Файл ""%1"" не может быть загружен в составе реестра НДС.'"),
						ВыбранныйФайл.Имя);
...
Процедура УправлениеФормой(Форма)
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС") Тогда
		...
		Пока ИндексФайла >= 1 Цикл
			СвойстваФайла = Форма.СвойстваФайлов[ИндексФайла];
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Файл ""%1"" не может быть загружен в составе комплекта.';ru='Файл ""%1"" не может быть загружен в составе комплекта.'"),
				СвойстваФайла.ИмяФайла);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ОтчетФСГС") Тогда
		...
		Пока ИндексФайла >= 1 Цикл
			СвойстваФайла = Форма.СвойстваФайлов[ИндексФайла];
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Файл ""%1"" не будет загружен. Загрузится только первый выбранный.';ru='Файл ""%1"" не будет загружен. Загрузится только первый выбранный.'"),
				СвойстваФайла.ИмяФайла);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ПолучитьНаименование(ВидОтчета, НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Иначе
		ШаблонНаименования = НСтр("ka='%1 %3 %2';ru='%1 %3 %2'");
	КонецЕсли;
...
Функция ВозможноПередатьФайлыДляЗагрузки(СвойстваВидаОтчета = Неопределено)
	...
	Если СвойстваФайлов.Количество() = 0 Тогда
		...
		Сообщение.Текст =
			НСтр("ka='Не выбраны файлы электронных представлений для загрузки.';ru='Не выбраны файлы электронных представлений для загрузки.'");
		Сообщение.Сообщить();
	...
	Если ПустаяСтрока(ВидОтчета) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не заполнено поле ""Вид"".';ru='Не заполнено поле ""Вид"".'");
	...
	Если Элементы.ПредставлениеПериода.Видимость И ПустаяСтрока(ПредставлениеПериода) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не заполнено поле ""Период"".';ru='Не заполнено поле ""Период"".'");
		...
		Если РегламентированнаяОтчетностьЗагрузкаКлиент.ЭтоФормаРегламентированногоОтчета(ВладелецФормы) Тогда
			...
			Если МаксИндекс > 0 Тогда
				...
				Если ВРег(ИсточникОтчета) <> ВРег(ИсточникФормыОтчета) Тогда
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ka='Данные электронного представления ""%1"" нельзя загрузить в отчет ""%2"".';ru='Данные электронного представления ""%1"" нельзя загрузить в отчет ""%2"".'");
				...
				Если СвойстваВидаОтчета <> Неопределено И СвойстваВидаОтчета.ТипПолучателя <> ТипПолучателя Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ka='Электронное представление отчета %1 не может быть загружено в форму отчета %2.';ru='Электронное представление отчета %1 не может быть загружено в форму отчета %2.'");
				...
				Если Элементы.ПредставлениеПериода.Видимость
				 И НЕ РегламентированнаяОтчетностьВызовСервера.ФормаРеглОтчетаДействуетВОтчетномПериоде(
					...
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ka='Выбранная для загрузки форма отчета ""%2"" не поддерживает отчетный период загружаемого электронного представления ""%1"".';ru='Выбранная для загрузки форма отчета ""%2"" не поддерживает отчетный период загружаемого электронного представления ""%1"".'");
