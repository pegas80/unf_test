...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если СвойстваФайлов.Количество() > 0 Тогда
		...
		Кнопки.Добавить("Закрыть", НСтр("ru=' Закрыть без загрузки ';ka=' Закрыть без загрузки '"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru=' Не закрывать ';ka=' Не закрывать '"));
...
"если у вас слабый компьютер. Продолжить?';ka='Размер файла превышает максимально допустимый (%1 Мб). "
"Открытие данного файла может занять длительное время или "
"привести к ошибкам нехватки оперативной памяти в случае, "
			...
			Если ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС")
				...
				Если ОпределитьТипФайлаНДС(ВыбранныйФайл.Имя) = Неопределено
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ka='Файл ""%1"" не может быть загружен в составе реестра НДС.';ru='Файл ""%1"" не может быть загружен в составе реестра НДС.';en='File ""%1"" can not be loaded as part of the VAT register.'"),
						ВыбранныйФайл.Имя);
...
Процедура УправлениеФормой(Форма)
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС") Тогда
		...
		Пока ИндексФайла >= 1 Цикл
			СвойстваФайла = Форма.СвойстваФайлов[ИндексФайла];
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Файл ""%1"" не может быть загружен в составе комплекта.';ru='Файл ""%1"" не может быть загружен в составе комплекта.';en='File ""%1"" can not be loaded as part of the kit.'"),
				СвойстваФайла.ИмяФайла);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ОтчетФСГС") Тогда
		...
		Пока ИндексФайла >= 1 Цикл
			СвойстваФайла = Форма.СвойстваФайлов[ИндексФайла];
			ТекстСообщения = СтрШаблон(
				НСтр("ka='Файл ""%1"" не будет загружен. Загрузится только первый выбранный.';ru='Файл ""%1"" не будет загружен. Загрузится только первый выбранный.';en='File ""%1"" will not be loaded. Load only the first selected.'"),
				СвойстваФайла.ИмяФайла);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ПолучитьНаименование(ВидОтчета, НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Иначе
		ШаблонНаименования = НСтр("ka='%1 %3 %2';ru='%1 %3 %2';en='1% 2% 3%'");
	КонецЕсли;
...
Функция ВозможноПередатьФайлыДляЗагрузки(СвойстваВидаОтчета = Неопределено)
	...
	Если СвойстваФайлов.Количество() = 0 Тогда
		...
		Сообщение.Текст =
			НСтр("ka='Не выбраны файлы электронных представлений для загрузки.';ru='Не выбраны файлы электронных представлений для загрузки.';en='Not selected files to download electronic submissions.'");
		Сообщение.Сообщить();
	...
	Если ПустаяСтрока(ВидОтчета) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не заполнено поле ""Вид"".';ru='Не заполнено поле ""Вид"".';en='Do not fill in the ""View"".'");
	...
	Если Элементы.ПредставлениеПериода.Видимость И ПустаяСтрока(ПредставлениеПериода) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не заполнено поле ""Период"".';ru='Не заполнено поле ""Период"".';en='Do not fill in the ""Period"".'");
		...
		Если РегламентированнаяОтчетностьЗагрузкаКлиент.ЭтоФормаРегламентированногоОтчета(ВладелецФормы) Тогда
			...
			Если МаксИндекс > 0 Тогда
				...
				Если ВРег(ИсточникОтчета) <> ВРег(ИсточникФормыОтчета) Тогда
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ka='Данные электронного представления ""%1"" нельзя загрузить в отчет ""%2"".';ru='Данные электронного представления ""%1"" нельзя загрузить в отчет ""%2"".';en='These electronic submission of ""%1"" can not be loaded into the report ""2%"".'");
				...
				Если СвойстваВидаОтчета <> Неопределено И СвойстваВидаОтчета.ТипПолучателя <> ТипПолучателя Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ka='Электронное представление отчета %1 не может быть загружено в форму отчета %2.';ru='Электронное представление отчета %1 не может быть загружено в форму отчета %2.';en='The electronic representation of the report% 1 can not be loaded in report form 2%.'");
				...
				Если Элементы.ПредставлениеПериода.Видимость
				 И НЕ РегламентированнаяОтчетностьВызовСервера.ФормаРеглОтчетаДействуетВОтчетномПериоде(
					...
					Сообщение = Новый СообщениеПользователю;
					ШаблонСообщения = НСтр("ka='Выбранная для загрузки форма отчета ""%2"" не поддерживает отчетный период загружаемого электронного представления ""%1"".';ru='Выбранная для загрузки форма отчета ""%2"" не поддерживает отчетный период загружаемого электронного представления ""%1"".';en='Choose to download a report form ""%2"" does not support the reporting period downloadable electronic submission of ""%1"".'");
