...
"для объектов типа ""%1"".';ka='დაშვების უფლებების გამართვა ""%1"" ტიპის ობიექტებისთვის არ ხდება.'"),
...
Процедура Перечитать(Команда)
	...
	Иначе
		ПоказатьВопрос(
			...
			НСтр("ka='მონაცემები შეცვლილია. წავიკითხოთ შენახვის გარეშე?';ru='Данные изменены. Прочитать без сохранения?';en='These modified. Read without saving?'"),
...
Процедура ЗаписатьНачало(Закрыть = Ложь)
	...
	Если ПодтвердитьОтказОтУправленияПравами = Истина Тогда
		...
		Кнопки.Добавить("ЗаписатьИЗакрыть", НСтр("ka='ჩაწერა და დახურვა';ru='Записать и закрыть';en='Record and Close'"));
		...
		ПоказатьВопрос(
			...
			НСтр("ka='После записи настройка прав станет недоступной.';ru='После записи настройка прав станет недоступной.';en='After recording setting rights will become inaccessible.'"),
			Кнопки,, "Отмена");
	Иначе
		Если Закрыть Тогда
			Закрыть();
...
Процедура ДобавитьРеквизитыИлиЭлементыФормы(НовыеРеквизиты = Неопределено)
	...
	Если НовыеРеквизиты = Неопределено И Параметры.СсылкаНаОбъект.Метаданные().Иерархический Тогда
		...
		Элемент.Заголовок = НСтр("ka='ქვესაქაღალდეებისთვის';ru='Для"
"подпапок'");
		Элемент.Подсказка = НСтр("ru='Права не только для текущей папки,"
"но и для ее нижестоящих папок';ka='Права не только для текущей папки,"
"но и для ее нижестоящих папок'");
		...
		Элемент.Заголовок = НСтр("ka='მემკვიდრეობის მიღება   -დან';ru='Наследуется от';en='inherited from'");
		Элемент.Подсказка = НСтр("ka='Папка, от которой наследуются настройка прав';ru='Папка, от которой наследуются настройка прав';en='The folder from which the setting is inherited rights'");
	...
	Пока НЕ Отказ И НомерСтроки >= 0 Цикл
		...
		Если НетЗаполненногоПрава Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнено ни одно право доступа.';ru='Не заполнено ни одно право доступа.';en='Is not filled, no right of access.'"),
				,
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока["Пользователь"]) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнен пользователь или группа.';ru='Не заполнен пользователь или группа.';en='Not filled with the user or group.'"),
				,
		...
		Если ГруппыПрав.НайтиСтроки(Отбор).Количество() > 1 Тогда
			Если ТипЗнч(Отбор.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстСообщения = НСтр("ka='Настройка для пользователя ""%1"" уже есть.';ru='Настройка для пользователя ""%1"" уже есть.';en='Adjusting for the user ""%1"" already exists.'");
			Иначе
				ТекстСообщения = НСтр("ka='Настройка для группы пользователей ""%1"" уже есть.';ru='Настройка для группы пользователей ""%1"" уже есть.';en='Configuring user groups ""%1"" already exists.'");
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ЗаписатьПрава()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='После записи настройка прав станет недоступной.';ru='После записи настройка прав станет недоступной.';en='After recording setting rights will become inaccessible.'");
		КонецЕсли;
...
"вышестоящей папки ""%1"".';ka='Эти права унаследованы, их можно изменить в форме настройки прав"
"вышестоящей папки ""%1"".'"),
		...
		Если ПроверкаУдаления Тогда
			ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მემკვიდრეობით მიღებული ყველა უფლების წასაშლელად უნდა მოვხსნათ ალამი ""%1"".';ru='Для удаления всех унаследованных прав следует"
"снять флажок ""%1"".'"),
...
Процедура ПроверитьРазрешениеНаУправлениеПравами()
	...
	ВызватьИсключение НСтр("ka='Настройка прав недоступна.';ru='Настройка прав недоступна.';en='Setting rights not available.'");
