...
Процедура Перечитать(Команда)
	...
	Иначе
		ПоказатьВопрос(
			...
			НСтр("ka='მონაცემები შეცვლილია. წავიკითხოთ შენახვის გარეშე?';ru='Данные изменены. Прочитать без сохранения?'"),
...
Процедура ЗаписатьНачало(Закрыть = Ложь)
	...
	Если ПодтвердитьОтказОтУправленияПравами = Истина Тогда
		...
		Кнопки.Добавить("ЗаписатьИЗакрыть", НСтр("ka='ჩაწერა და დახურვა';ru='Записать и закрыть'"));
		Кнопки.Добавить("Отмена", НСтр("ka='გაუქმება';ru='Отмена'"));
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаписатьПослеПодтверждения", ЭтотОбъект),
			НСтр("ka='После записи настройка прав станет недоступной.';ru='После записи настройка прав станет недоступной.'"),
			Кнопки,, "Отмена");
	Иначе
		Если Закрыть Тогда
			Закрыть();
...
"но и для ее нижестоящих папок'");
		...
		Элемент.Заголовок = НСтр("ka='მემკვიდრეობის მიღება   -დან';ru='Наследуется от'");
		Элемент.Подсказка = НСтр("ka='Папка, от которой наследуются настройка прав';ru='Папка, от которой наследуются настройка прав'");
	...
	Пока НЕ Отказ И НомерСтроки >= 0 Цикл
		...
		Если НетЗаполненногоПрава Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнено ни одно право доступа.';ru='Не заполнено ни одно право доступа.'"),
				,
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока["Пользователь"]) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Не заполнен пользователь или группа.';ru='Не заполнен пользователь или группа.'"),
				,
		...
		Если ГруппыПрав.НайтиСтроки(Отбор).Количество() > 1 Тогда
			Если ТипЗнч(Отбор.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстСообщения = НСтр("ka='Настройка для пользователя ""%1"" уже есть.';ru='Настройка для пользователя ""%1"" уже есть.'");
			Иначе
				ТекстСообщения = НСтр("ka='Настройка для группы пользователей ""%1"" уже есть.';ru='Настройка для группы пользователей ""%1"" уже есть.'");
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ЗаписатьПрава()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ka='После записи настройка прав станет недоступной.';ru='После записи настройка прав станет недоступной.'");
		КонецЕсли;
...
Процедура ПроверитьРазрешениеНаУправлениеПравами()
	...
	ВызватьИсключение НСтр("ka='Настройка прав недоступна.';ru='Настройка прав недоступна.'");
