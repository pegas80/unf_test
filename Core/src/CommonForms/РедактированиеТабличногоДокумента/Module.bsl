...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ცვლილებები შევინახოთ %1-ში?';ru='Сохранить изменения в %1?'"), ИмяДокумента);
...
Процедура НачатьДиалогСохраненияФайла(Знач ОбработчикЗавершения)
	...
	ДиалогСохраненияФайла.Фильтр = НСтр("ka='ცხრილი-დოკუმენტი';ru='Табличный документ'") + " (*.mxl)|*.mxl";
...
Функция ИмяНовогоДокумента()
	Возврат НСтр("ka='Новый';ru='Новый'");
КонецФункции
...
Процедура УстановитьЗаголовок()
	...
	Если ЭтоНовый() Тогда
		Заголовок = Заголовок + " (" + НСтр("ka='ქმნილება';ru='создание'") + ")";
	ИначеЕсли РедактированиеЗапрещено Тогда
		Заголовок = Заголовок + " (" + НСтр("ka='მხოლოდ გადათვალიერება';ru='только просмотр'") + ")";
	КонецЕсли;
