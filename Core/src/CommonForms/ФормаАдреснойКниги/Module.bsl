...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(АдресВХранилище) И ВыбранныеАдресаты.ПолучитьЭлементы().Количество() > 0 Тогда
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ka='Выбранные адресаты не перенесены.';ru='Выбранные адресаты не перенесены.'");
			Возврат;
		КонецЕсли;
		
		ТекстВопроса = НСтр("ka='გადავიტანოთ აღნიშნული ადრესატები?';ru='Перенести отмеченных адресатов?'");
		
		Оповещение = Новый ОписаниеОповещения("ПередЗакрытиемПереносПредложен", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да, НСтр("ka='სამისამართი წიგნი';ru='Адресная книга'"));
	...
	Если НеПоказыватьВопросВыбораГруппы = Ложь Тогда
		Для Каждого ВыделеннаяСтрока Из Элементы.СписокКонтрагентов.ВыделенныеСтроки Цикл
			...
			Если ВыделеннаяСтрока.Пустая() Или Данные.ЭтоГруппа Тогда
				...
				ПараметрыВопроса.Вставить("Заголовок", НСтр("ka='ჯგუფის არჩევის დადასტურება';ru='Подтверждение выбора группы'"));
				ПараметрыВопроса.Вставить("ТекстСообщения", НСтр("ka='ავირჩიოთ ჯგუფის ყველა კონტრაგენტი?';ru='Выбрать всех контрагентов группы?'"));
