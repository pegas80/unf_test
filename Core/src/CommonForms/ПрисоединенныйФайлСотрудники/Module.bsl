...
Процедура СтандартнаяПеречитать(Команда)
	...
	ТекстВопроса = НСтр("ka='მონაცემები შეცვლილია. გადავიკითხოთ მონაცემები?';ru='Данные изменены. Перечитать данные?'");
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='წავშალოთ გამოყოფილი ხელმოწერები?';ru='Удалить выделенные подписи?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 (მიერთებული ფაილი)';ru='%1 (Присоединенный файл)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = НСтр("ka='მიერთებული ფაილი (შექმნა)';ru='Присоединенный файл (Создание)'")
	КонецЕсли;
...
Процедура ЗаполнитьСписокШифрования()
	...
	ТекстЗаголовка = НСтр("ka='ნებადართულია გაშფვრა';ru='Разрешено расшифровывать'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = НСтр("ka='Электронные подписи';ru='Электронные подписи'");
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='გასაგრძელებლად მიუთითეთ ფაილის სახელი.';ru='Для продолжения укажите имя файла.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		Если ТранзакцияАктивна Тогда
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. მიერთებული ფაილის ჩაწერის შეცდომა';ru='Файлы.Ошибка записи присоединенного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
