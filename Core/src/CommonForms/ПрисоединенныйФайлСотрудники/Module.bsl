...
Процедура СтандартныеУстановитьПометкуУдаления(Команда)
	...
	Если Модифицированность Тогда
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ka='მოქმედების შესასრულებლად საჭიროა ფაილის ცვლილებების ჩაწერა. ჩავწეროთ ცვლილებები და მოვხსნათ ფაილ ""%1""-დან წაშლის ნიშნული?';ru='Для выполнения действия требуется записать изменения файла."
"Записать изменения и снять пометку на удаление с файла"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ka='მოქმედების შესასრულებლად საჭიროა ფაილის ცვლილებების ჩაწერა. ჩავწეროთ ცვლილებები და წასაშლელად მოვნიშნოთ ფაილი ""%1""?';ru='Для выполнения действия требуется записать изменения файла."
...
"""%1""?';ka='ფაილ ""%1""-ს მოვხსნათ წაშლის ნიშნული?'");
		Иначе
			ТекстВопроса = НСтр("ru='Пометить на удаление файл"
"""%1""?';ka='ფაილი ""%1"" მოვნიშნოთ წასაშლელად?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СтандартнаяПеречитать(Команда)
	...
	ТекстВопроса = НСтр("ka='მონაცემები შეცვლილია. გადავიკითხოთ მონაცემები?';ru='Данные изменены. Перечитать данные?';en='These modified. Re-read the data?'");
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='წავშალოთ გამოყოფილი ხელმოწერები?';ru='Удалить выделенные подписи?';en='Delete the selected signatures?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1 (მიერთებული ფაილი)';ru='%1 (Присоединенный файл)';en='% 1 (attached file)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = НСтр("ka='მიერთებული ფაილი (შექმნა)';ru='Присоединенный файл (Создание)';en='The attached file (Create)'")
	КонецЕсли;
...
Процедура ЗаполнитьСписокШифрования()
	...
	ТекстЗаголовка = НСтр("ka='ნებადართულია გაშფვრა';ru='Разрешено расшифровывать';en='allowed to decrypt'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = НСтр("ka='Электронные подписи';ru='Электронные подписи';en='electronic signatures'");
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='გასაგრძელებლად მიუთითეთ ფაილის სახელი.';ru='Для продолжения укажите имя файла.';en='To continue, enter a file name.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		Если ТранзакцияАктивна Тогда
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ka='ფაილები. მიერთებული ფაილის ჩაწერის შეცდომა';ru='Файлы.Ошибка записи присоединенного файла';en='Fayly.Oshibka recording attached file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Для каждого Элемент Из Элементы Цикл
		Если ТипЗнч(Элемент) = Тип("ПолеФормы")
		   ...
		   И СтрЗаканчиваетсяНа(Элемент.Имя, "0") Тогда
			...
			Если Найти(Элемент.Имя,"Одобрен")>0 Тогда
