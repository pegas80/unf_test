...
""
"%1'"), ТекстСнятияБлокировки);
			Элементы.ИнформационнаяБазаПеремещена.Заголовок = НСтр("ka='აპლიკაცია გადაადგილებულია';ru='Приложение перемещено'");
			Элементы.ЭтоКопияИнформационнойБазы.Заголовок = НСтр("ka='ეს არის დანართის ასლი';ru='Это копия приложения'");
			Заголовок = НСтр("ka='დანართი გადაადგილდა ან აღდგა სარეზერვო ასლიდან';ru='Приложение было перемещено или восстановлено из резервной копии'");
...
"%1'"), ТекстСнятияБлокировки);
	...
	Иначе
		...
		Заголовок = НСтр("ka='გარე რესურსებთან მუშაობის ბლოკირების პარამეტრები';ru='Параметры блокировки работы с внешними ресурсами'");
	КонецЕсли;
...
Процедура НадписьПредупреждениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("СобытиеЖурналаРегистрации",
		НСтр("ka='გარე რესურსებთან მუშაობა დაბლოკილია';ru='Работа с внешними ресурсами заблокирована'",
