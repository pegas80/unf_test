	...
	Для каждого СтрокаПредоплата Из Предоплата Цикл
		...
		И НЕ ЗначениеЗаполнено(СтрокаПредоплата.Курс) Тогда
			...
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Курс"" в строке ';ka='სვეტი ""კურსი"" არ არის შევსებული სტრიქონში'")
				+ Строка(НомерСтроки)
				+ НСтр("ka='სია ""ანგარიშსწორებების გაშიფრვა"".';ru=' списка ""Расшифровка расчетов"".'");
		...
		И НЕ ЗначениеЗаполнено(СтрокаПредоплата.СуммаПлатежа) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Сумма платежа"" в строке ';ka='სვეტი ""გადახდის თანხა"" სტრიქონში არ არის შევსებული'")
				+ Строка(НомерСтроки)
				+ НСтр("ka='სია ""ანგარიშსწორებების გაშიფრვა"".';ru=' списка ""Расшифровка расчетов"".'");
			Сообщение.Поле = "Документ";
			Сообщение.Сообщить();
...
Процедура ЗаполнитьПредоплату()
	...
	// Kuzmich, 15.10.2023, (ONESOFT-2593) Приходная накладная - Зачет авансы - По остатком
	ТЗ_Расчётов = РегистрыНакопления.РасчетыСПоставщиками.Остатки(ТекущаяДата(),Новый Структура("Документ", Ссылка),"Документ","СуммаВал");
	ОбщаяСуммаРасчетов = ТЗ_Расчётов.Итог("СуммаВал");
	ПроверятьОбщаяСуммаРасчетов = (ОбщаяСуммаРасчетов>0);
	// Kuzmich, 15.10.2023, (ONESOFT-2593) Приходная накладная - Зачет авансы - По остатком
	...
	// Kuzmich, 15.10.2023, (ONESOFT-2593) Приходная накладная - Зачет авансы - По остатком
	|	ГДЕ
	|	 РасчетыСПоставщикамиОстатки.ДокументДата < &Период
	// Kuzmich, 15.10.2023, (ONESOFT-2593) Приходная накладная - Зачет авансы - По остатком
	|
	|СГРУППИРОВАТЬ ПО
	...
	|;
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|		ЕСТЬNULL(РасчетыСПоставщикамиОстатки.СуммаОстаток, 0) * КурсыВалютыУчета.Курс * РасчетыСПоставщикамиОстатки.Кратность / (РасчетыСПоставщикамиОстатки.Курс * КурсыВалютыУчета.Кратность) КАК СуммаПлатежа,
	|		РасчетыСПоставщикамиОстатки.Курс КАК КурсыВалютыДокументаКурс,
	|		РасчетыСПоставщикамиОстатки.Кратность КАК КурсыВалютыДокументаКратность
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|	ИЗ
	...
	Пока ВыборкаРезультатаЗапроса.Следующий() Цикл
			...
			Если ОбщаяСуммаРасчетов > 0 Тогда
				...
				Если ВыборкаРезультатаЗапроса.СуммаРасчетов <= ОбщаяСуммаРасчетов Тогда
					...
					Если ВыборкаРезультатаЗапроса.СуммаРасчетов <= НайденнаяСтрока.ВсегоРасч Тогда // сумма остатка меньше или равна чем осталось распределить
						...
						//+++ Дмгг   
						Если Контрагент.ВестиРасчетыПоДоговорам И НЕ Контрагент.ВестиРасчетыПоЗаказам Тогда
							
						Иначе
							НоваяСтрока.Курс = ВыборкаРезультатаЗапроса.КурсыВалютыДокументаКурс; 
							НоваяСтрока.Кратность = ВыборкаРезультатаЗапроса.КурсыВалютыДокументаКратность;  
						КонецЕсли;
						//--- Дмгг
		...
		КонецЕсли; // Kuzmich, 15.10.2023, (ONESOFT-2593) Приходная накладная - Зачет авансы - По остатком
...
Процедура ЗаполнитьАвансы()
	...
	//|				ТОГДА РасчетыСПоставщикамиОстатки.СуммаРасчетов
	//|			ИНАЧЕ 1
	//|		КОНЕЦ) * (КурсыВалютыДокумента.Курс / КурсыВалютыДокумента.Кратность) КАК Курс
	//|	1 КАК Кратность
	//|ИЗ
	//|	(ВЫБРАТЬ
	//|		РасчетыСПоставщикамиОстатки.ВалютаРасчетов КАК ВалютаРасчетов,
	...
	//|	КурсыВалютыДокумента.Курс,
	//|	КурсыВалютыДокумента.Кратность
	//|
	//|ИМЕЮЩИЕ
	//|	-СУММА(РасчетыСПоставщикамиОстатки.СуммаРасчетов) > 0
	//|
	...
	ТекстЗапроса = "";
	// не знаю всех тонкостей поэтому выделим ПриходнуюНакладную в отдельный запрос (доработка пока что для неё)
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда 
		ТекстЗапроса = ПолучитьТекстЗапроса_ПриходнаяНкладная();
	Иначе
		ТекстЗапроса = ПолучитьРоднойТекстЗапроса();
	КонецЕсли;
	//--- Дмгг
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [26 июля 2021]  НАЧАЛО
	Если Контрагент.ВестиРасчетыПоДоговорам И НЕ Контрагент.ВестиРасчетыПоЗаказам Тогда
	
		Для каждого ТекСтрока Из СписокАвансов Цикл
			
			Если ТекСтрока.СуммаРасчетов > 0 Тогда
			
				ТекСтрока.Курс = ТекСтрока.СуммаУчета / ТекСтрока.СуммаРасчетов; 	
...
//+++ Дмгг
&НаСервере
Функция ПолучитьРоднойТекстЗапроса()
...
КонецФункции // ПолучитьРоднойТекстЗапроса()()

&НаСервере
Функция ПолучитьТекстЗапроса_ПриходнаяНкладная()
	...
	|	ОтобранныеАвансы.СуммаПлатежа КАК СуммаПлатежа
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|	,ОтобранныеАвансы.Курс КАК Курс
	|	,ОтобранныеАвансы.Кратность КАК Кратность
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|ПОМЕСТИТЬ ТаблицаОтобранныеАвансы
	|ИЗ
	|	&ТаблицаОтобранныеАвансы КАК ОтобранныеАвансы
	...
	|	-СУММА(РасчетыСПоставщикамиОстатки.СуммаРасчетов) КАК СуммаРасчетов,
	|	-СУММА(РасчетыСПоставщикамиОстатки.СуммаПлатежа) КАК СуммаПлатежа,
	//|   ЕСТЬNULL(КурсыВалютыДокумента.Курс, КурсыВалютыУчета.Курс) КАК Курс,
	//|	ЕСТЬNULL(КурсыВалютыДокумента.Кратность, КурсыВалютыУчета.Кратность) КАК Кратность
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	...
	//|			ПО (НАЧАЛОПЕРИОДА(РасчетыСПоставщикамиОстатки.Документ.Дата, ДЕНЬ) = НАЧАЛОПЕРИОДА(КурсыВалютыДокумента.Период, ДЕНЬ))
	//|				И (РасчетыСПоставщикамиОстатки.ВалютаРасчетов = КурсыВалютыДокумента.Валюта)
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|		ВременнаяТаблицаРасчетыСПоставщикамиОстаткиСКурсамиВалют КАК РасчетыСПоставщикамиОстатки
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта = &ВалютаУчета) КАК КурсыВалютыУчета
	...
	|		ОтобранныеАвансы.СуммаПлатежа
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|		,ОтобранныеАвансы.Курс
	|		,ОтобранныеАвансы.Кратность
	// Kuzmich, 17.09.2023, (ONESOFT-2559) предоплата в документ приходная накладная
	|	ИЗ
	|		ТаблицаОтобранныеАвансы КАК ОтобранныеАвансы) КАК РасчетыСПоставщикамиОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта = &ВалютаУчета) КАК КурсыВалютыУчета
