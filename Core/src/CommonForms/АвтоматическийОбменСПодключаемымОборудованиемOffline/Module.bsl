...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.СостояниеВыгрузки      = НСтр("ka='<გადმოტვირთვა არ განხორციელებულა>';ru='<Выгрузка не производилась>'");
		...
		Если НоваяСтрока.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток Тогда
			НоваяСтрока.СостояниеЗагрузки = НСтр("ka='<არ არის საჭირო>';ru='<Не требуется>'");
		Иначе
			НоваяСтрока.СостояниеЗагрузки = НСтр("ka='<ჩატვირთვა არ განხორციელებულა>>';ru='<Загрузка не производилась>'");
		КонецЕсли;
...
Процедура Начать(Команда)
	...
	Если Не ЗначениеЗаполнено(ПериодичностьОбмена) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მოწყობილობასთან გაცვლის პერიოდულობა არ არის მიცემული';ru='Не задана периодичность обмена с оборудованием'"),,"ПериодичностьОбмена");
		Возврат;
	КонецЕсли;
	
	Если Не ЕстьОборудованиеДляОбмена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='გასაცვლელად მოწყობილობა არ არის არჩეული';ru='Не выбрано оборудование для обмена'"),,"Оборудование");
		Возврат;
	КонецЕсли;
	...
	Состояние = НСтр("ka='მიმდინარეობს შესაერთებელ მოწყობილობასთან გაცვლა...';ru='Выполняется обмен с подключенным оборудованием...'");
...
Процедура Завершить(Команда)
	...
	Состояние = НСтр("ka='გაცვლა დასრულებულია';ru='Обмен завершен'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ОбменВыполняется Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='აპარატურასთან გაცვლითი ფორმის დახურვის  შემდეგ არ შესრულდება:';ru='После закрытия формы обмен с оборудованием выполняться не будет.'"));
