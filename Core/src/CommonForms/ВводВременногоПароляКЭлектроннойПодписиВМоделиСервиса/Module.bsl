	...
	Если ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) = 6 Тогда
		...
		Исключение
			...
			Если СтрНайти(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), КриптосервисВМоделиСервисаКлиентСервер.ТекстИсключенияУказанНеВерныйВременныйПароль()) > 0 Тогда
				ТекстОшибки = НСтр("ka='Указан неверный временный пароль.';ru='Указан неверный временный пароль.'");
			Иначе 
				ПоказатьПредупреждение(, НСтр("ka='Выполнение операции временно невозможно.';ru='Выполнение операции временно невозможно.'"));
			КонецЕсли;
		КонецПопытки;		
	ИначеЕсли ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) <> 6 Тогда
		ТекстОшибки = НСтр("ka='Временный пароль должен состоять из 6 цифр.';ru='Временный пароль должен состоять из 6 цифр.'");
	Иначе
		ТекстОшибки = НСтр("ka='Временный пароль не указан.';ru='Временный пароль не указан.'");
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.СпособДоставкиПаролей = "phone" Тогда
		Пояснение = НСтр("ka='На номер %1 отправлено SMS-сообщение с временным паролем.';ru='На номер %1 отправлено SMS-сообщение с временным паролем.'");
		
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Пояснение, Форма.НомерТелефона);
			
		ТекстКоманды = НСтр("ka='Не получили SMS? Попробуйте отправить пароль на %1';ru='Не получили SMS? Попробуйте отправить пароль на %1'");
	...
	ИначеЕсли Форма.СпособДоставкиПаролей = "email" Тогда
		Пояснение = НСтр("ka='На адрес %1 отправлено письмо с временным паролем.';ru='На адрес %1 отправлено письмо с временным паролем.'");
		
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Пояснение, Форма.АдресЭлектроннойПочты);
			
		ТекстКоманды = НСтр("ka='Не получили письмо? Попробуйте отправить пароль на %1';ru='Не получили письмо? Попробуйте отправить пароль на %1'");
		ТекстКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ОбновитьНадписьОбработногоОтчета()
	...
	Если Таймер > 0 Тогда
		...
		НадписьПовторнойОтправкиСМС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='(через %1 сек...)';ru='(через %1 сек...)'"), Таймер);
		ПодключитьОбработчикОжидания("ОбновитьНадписьОбработногоОтчета", 1, Истина);
	Иначе
