...
Процедура ПродолжитьЗагрузкуДанных(ВыборВыполнен, АдресХранилища, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если ВыборВыполнен Тогда
		...
		Состояние(
			НСтр("ka='მიმდინარეობს მონაცემთა ჩატვირთვა სერვისიდან. ოპერაციას შეიძლება ხანგრძლივი დრო დასჭირდეს, გთხოვთ, მოიცადოთ...';ru='Выполняется загрузка данных из сервиса."
...
"-----------------------------------------"
"%1"
"-----------------------------------------';ka='При загрузке данных произошла ошибка:"
""
"-----------------------------------------"
"%1"
"-----------------------------------------'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ka='მონაცემების ჩატვირთვა';ru='Загрузка данных';en='Loading data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
""
"Расширенная информация для службы поддержки записана в журнал регистрации. Если Вам неизвестна причина ошибки - рекомендуется обратиться в службу технической поддержки, предоставив для расследования выгрузку журнала регистрации и файл, из которого предпринималась попытка загрузить данные.';ka='При загрузке данных произошла ошибка: %1."
""
