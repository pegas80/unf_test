...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтандартныеПодсистемыВызовСервера.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая Тогда
		Элементы.ПутьКАрхивуWindows.Заголовок = НСтр("ka='1С:საწარმოს სერვერისთვის Microsoft Windows-ის მართვის ქვეშ';ru='Для сервера 1С:Предприятия под управлением Microsoft Windows'"); 
	Иначе
...
Процедура ПутьКАрхивуWindowsНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Диалог.Фильтр                       = НСтр("ka='Архивы zip(*.zip)|*.zip';ru='Архивы zip(*.zip)|*.zip'");
...
"должен быть в формате UNC (\\servername\resource).';ka='საწყისი იერ-სახის ფაილების მქონე არქივისკენ გზა უნდა იყოს UNC ფორმატში (\\servername\resource).'");
	...
	Состояние(
		НСтр("ka='მონაცემთა გაცვლა';ru='Обмен данными'"),
		,
