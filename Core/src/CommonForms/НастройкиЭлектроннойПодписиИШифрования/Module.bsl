...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.Программы.Заголовок =
			НСтр("ka='Список программ, предусмотренных администратором, которые можно использовать на компьютере';ru='Список программ, предусмотренных администратором, которые можно использовать на компьютере';en='List of programs provided by the administrator, you can use on your computer'");
	КонецЕсли;
...
"Обратитесь к администратору.';ka='Невозможно сохранить путь к программе. Отсутствует право сохранения данных."
...
Процедура ОбновитьТекущуюВидимостьЭлементов()
	...
	Если Константы.ИспользоватьШифрование.Получить()
	 Или ЭлектроннаяПодпись.ОбщиеНастройки().ЗаявлениеНаВыпускСертификатаДоступно Тогда
		...
		Элементы.СертификатыСоздать.Заголовок = НСтр("ka='დამატება...';ru='Добавить...';en='Add ...'");
		Элементы.СертификатыКонтекстноеМенюСоздать.Заголовок = НСтр("ka='დამატება...';ru='Добавить...';en='Add ...'");
	...
	Если Константы.ИспользоватьШифрование.Получить() Тогда
		Если ЭлектроннаяПодписьСлужебный.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("ka='Из установленных в облачном сервисе и на компьютере...';ru='Из установленных в облачном сервисе и на компьютере...';en='The established cloud services and computer ...'");
		Иначе
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("ka='კომპიუტერზე დაყენებულებიდან...';ru='Из установленных на компьютере...';en='Of installed on the computer ...'");
		КонецЕсли;
	Иначе
		Если ЭлектроннаяПодписьСлужебный.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("ka='Из установленных в облачном сервисе и на компьютере';ru='Из установленных в облачном сервисе и на компьютере';en='From established in the cloud service, and a computer'");
		Иначе
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("ka='კომპიუტერზე დაყენებულებიდან';ru='Из установленных на компьютере';en='Of installed on your computer'");
		КонецЕсли;
	КонецЕсли;
	
	Если Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("ka='ხელმოწერების და სერთიფიკატების შემოწმება სერვერზე';ru='Проверять подписи и сертификаты на сервере';en='Checking signatures and certificates on the server'");
		ПодсказкаФлажка =
...
"должна быть установлена хотя бы одна из программ в списке.';ka='საშუალებას იძლევა, რომ მომხმარებლის კომპიუტერზე დაყენებული არ იქნას პროგრამა ელექტრონული ხელმოწერების და სერთიფიკატების შესამოწმებლად."
""
"მნიშვნელოვანია: ყოველ კომპიუტერზე, სადაც 1С:საწარმოს სერვერი ან ფაილური საინფორმაციო ბაზის გამომყენებელი ვებ-სერვერი მუშაობს, დაყენებული უნდა იყოს სიიდან თუნდ ერთი პროგრამა.'");
	...
	Иначе
		ЗаголовокФлажка = НСтр("ka='სერთიფიკატების შემოწმება სერვერზე';ru='Проверять сертификаты на сервере';en='Check the certificates on the server'");
		ПодсказкаФлажка =
			НСтр("ru='Позволяет не устанавливать программу на компьютер пользователя"
...
"Важно: на каждый компьютер, где работает сервер 1С:Предприятия"
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена хотя бы одна из программ в списке.';ka='საშუალებას იძლევა, რომ სერთიფიკატების შესამოწმებლად მომხმარებლის კომპიუტერზე პროგრამა არ იქნას დაყენებული."
"მნიშვნელოვანია: ყოველ კომპიუტერზე, რომელიც ფაილურ საინფორმაციო ბაზას იყენებს და სადაც 1С:საწარმოს სერვერი ან ვებ-სერვერი მუშაობს, რომ დაყენებული იქნას სიაში არსებული თუნდ ერთი პროგრამა.'");
	...
	Если Не Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("ka='სერვერზე დაშიფრვა და გაშიფრვა';ru='Шифровать и расшифровывать на сервере';en='Encrypt and decrypt server'");
		ПодсказкаФлажка =
...
"Важно: на каждый компьютер, где работает сервер 1С:Предприятия"
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';ka='დასაშიფრად და გასაშიფრად იძლევა მომხმარებლის კომპიუტერზე პროგრამის და სერთიფიკატის დაუყენებლობის საშუალებას."
""
"მნიშვნელოვანია: ყოველ კომპიუტერზე, სადაც 1С:საწარმოს სერვერი ან ფაილური საინფორმაციო ბაზის გამომყენებელი ვებ-სერვერი მუშაობს, დაყენებული უნდა იყოს პროგრამა და სერთიფიკატი დახურული გასაღებით.'");
	ИначеЕсли Не Константы.ИспользоватьШифрование.Получить() Тогда
		ЗаголовокФлажка = НСтр("ka='სერვერზე ხელმოწერა';ru='Подписывать на сервере';en='Sign on the server'");
		ПодсказкаФлажка =
			НСтр("ru='Позволяет не устанавливать программу и сертификат"
...
"Важно: на каждый компьютер, где работает сервер 1С:Предприятия"
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';ka='საშუალებას იძლევა, რომ ხელმოსაწერად, მომხმარებლის კომპიუტერზე არ იქნას დაყენებული პროგრამა და სერთიფიკატი."
""
"მნიშვნელოვანია: ყოველ კომპიუტერზე, სადაც 1С:საწარმოს სერვერი ან ფაილური საინფორმაციო ბაზის გამომყენებელი ვებ-სერვერი მუშაობს, პროგრამა და სერთიფიკატი დაყენებული უნდა იყოს დახურული გასაღებით.'");
	Иначе
		ЗаголовокФлажка = НСтр("ka='ხელმოწერა და დაშიფრვა სერვერზე';ru='Подписывать и шифровать на сервере';en='Sign and encrypt on the server'");
		ПодсказкаФлажка =
			НСтр("ru='Позволяет не устанавливать программу и сертификат"
...
"Важно: на каждый компьютер, где работает сервер 1С:Предприятия"
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';ka='საშუალებას იძლევა, რომ პროგრამა და სერთიფიკატი არ იქნას მომხმარებლის კომპიუტერზე დაყენებული ხელმოსაწერად, დასაშიფრად და გასაშიფრად."
""
"მნიშვნელოვანია: ყოველ კომპიუტერზე, სადაც 1С:საწარმოს სერვერი ან ფაილური საინფორმაციო ბაზის გამომყენებელი ვებ-სერვერი მუშაობს, დაყენებული უნდა იყოს პროგრამა და სერთიფიკატი დახურული გასაღებით.'");
	КонецЕсли;
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст)
	...
	ИначеЕсли ОписаниеПрограммы.ЭтоПрограммаОблачногоСервиса Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ka='Доступен.';ru='Доступен.';en='Available.'"));
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Менеджер <> Неопределено Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ka='დაყენებულია კომპიუტერზე.';ru='Установлена на компьютере.';en='Installed on your computer.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, Истина);
		ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ka='პროგრამისკენ გზა არ არის მითითებული.';ru='Не указан путь к программе.';en='Unknown path to the program.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
		ТекстОшибки = НСтр("ka='კომპიუტერზე არ არის დაყენებული.';ru='Не установлена на компьютере.';en='It is not installed on the computer.'") + " " + Ошибка.Описание;
		Если Ошибка.КАдминистратору И Не ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ka='მიმართეთ ადმინისტრატორს';ru='Обратитесь к администратору.';en='Contact your administrator.'");
		КонецЕсли;
