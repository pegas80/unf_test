...
Процедура ПолучитьЗначенияПараметровФормы()
	...
	//+++ Дмгг
	Если Параметры.Свойство("ОсновнаяСтавкаНДС") Тогда
		
		ОсновнаяСтавкаНДС = Параметры.ОсновнаяСтавкаНДС;
		Элементы.ОсновнаяСтавкаНДС.Видимость = Истина;
		ОсновнаяСтавкаНДСЕстьРеквизит = Истина;
		Если Параметры.Свойство("ДисконтнаяКарта") Тогда
			ОсновнаяСтавкаНДСОткрытии = Параметры.ДисконтнаяКарта;
		КонецЕсли;	
	Иначе
		
		Элементы.ОсновнаяСтавкаНДС.Видимость = Ложь;
		ОсновнаяСтавкаНДСЕстьРеквизит = Ложь;
	...
	Параметры.Свойство("ПерезаполнитьЦены", ПерезаполнитьЦены);
	Параметры.Свойство("ПересчитатьЦены", ПересчитатьЦены);
...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ, НеЗаполненТолькоВидЦен = Ложь)
	...
	Если (ПерезаполнитьЦены ИЛИ РегистрироватьЦеныПоставщика) И ВидЦенКонтрагентаЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(ВидЦенКонтрагента) Тогда
			...
			Сообщение.Текст = НСтр("ka='შესავსებად არ არის არჩეული კონტრაგენტის ფასების ტიპი!';ru='Не выбран вид цен контрагента для заполнения!';en='Not selected contractor prices to fill the form!'");
	...
	Если ВалютаДокументаЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(ВалютаДокумента) Тогда
            Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='არ არის შევსებული დოკუმენტის ვალუტა!';ru='Не заполнена валюта документа!';en='Not filled paper currency!'");
	...
	Если НалогообложениеНДСЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(НалогообложениеНДС) Тогда
            Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='გადასახადებით დაბეგვრა შეუვსებელია!';ru='Не заполнено налогообложение!';en='Taxation is not filled!'");
	...
	// основная ставув НДС. //+++ Дмгг
	Если ОсновнаяСтавкаНДСЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(НалогообложениеНДС) Тогда
            Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='გადასახადებით დაბეგვრა შეუვსებელია!';ru='Не заполнено налогообложение!';en='Taxation is not filled!'");
			Сообщение.Поле = "НалогообложениеНДС";
	...
	Если ДоговорЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(КурсРасчетов) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='აღმოჩენილია ანგარიშსწორებათა ვალუტის ნულოვანი კურსი!';ru='Обнаружен нулевой курс валюты расчетов!';en='Discovered a zero rate currency calculations!'");
		...
		Если НЕ ЗначениеЗаполнено(КратностьРасчетов) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='აღმოჩენილია ანგარიშსწორებათა ვალუტის კურსის ნულოვანი ჯერადობა!';ru='Обнаружена нулевая кратность курса валюты расчетов!';en='Detected zero multiplicity of exchange rates for the calculation!'");
	...
	Если ПерезаполнитьЦены И ВидЦенЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(ВидЦен) Тогда
			...
			Иначе
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = НСтр("ka='არ არის ფასის ტიპი არჩეული შესავსებად!';ru='Не выбран вид цены для заполнения!';en='Not selected kind of price are required!'");
...
Процедура ПроверитьМодифицированностьФормы()
	...
	ИзмененияОсновнаяСтавкаНДС      = ?(ОсновнаяСтавкаНДСЕстьРеквизит, ОсновнаяСтавкаНДСПриОткрытии <> ОсновнаяСтавкаНДС, Ложь);
	...
	Если ПерезаполнитьЦены
		...
		ИЛИ ИзмененияВидСкидки 
		ИЛИ ИзмененияОсновнаяСтавкаНДС 
		ИЛИ ПерезаполнитьЦеныПоДиапазону	
		Тогда	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДоступностьСуммаВключаетНДС") Тогда
		Элементы.СуммаВключаетНДС.Доступность = Параметры.ДоступностьСуммаВключаетНДС;
	КонецЕсли;
	// Kuzmich, 03.02.2024, (ONESOFT-2587) Расчет НДС неправильный в документ приходная накладная (услуги)
	
	ТипЦеныНоменклатурыПоКоличеству = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ТипЦеныНоменклатурыПоКоличеству");
	Элементы.РегистрироватьЦеныПоДиапазону.Видимость =  ТипЦеныНоменклатурыПоКоличеству;
...
Процедура КомандаОК(Команда)
	...
	Если НЕ Отказ ИЛИ НеЗаполненТолькоВидЦенИЗаполненаКарта Тогда
		...
		СтруктураРеквизитовФормы.Вставить("ОсновнаяСтавкаНДС",				ОсновнаяСтавкаНДС);//+++ Дмгг
		СтруктураРеквизитовФормы.Вставить("ПредОсновнаяСтавкаНДС", 			ОсновнаяСтавкаНДСПриОткрытии);
		СтруктураРеквизитовФормы.Вставить("ПерезаполнитьЦеныПоДиапазону", 	ПерезаполнитьЦеныПоДиапазону);
