...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ, НеЗаполненТолькоВидЦен = Ложь)
	...
	Если (ПерезаполнитьЦены ИЛИ РегистрироватьЦеныПоставщика) И ВидЦенКонтрагентаЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(ВидЦенКонтрагента) Тогда
			...
			Сообщение.Текст = НСтр("ka='შესავსებად არ არის არჩეული კონტრაგენტის ფასების ტიპი!';ru='Не выбран вид цен контрагента для заполнения!'");
	...
	Если ВалютаДокументаЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(ВалютаДокумента) Тогда
            Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='არ არის შევსებული დოკუმენტის ვალუტა!';ru='Не заполнена валюта документа!'");
	...
	Если НалогообложениеНДСЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(НалогообложениеНДС) Тогда
            Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='გადასახადებით დაბეგვრა შეუვსებელია!';ru='Не заполнено налогообложение!'");
	...
	Если ДоговорЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(КурсРасчетов) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='აღმოჩენილია ანგარიშსწორებათა ვალუტის ნულოვანი კურსი!';ru='Обнаружен нулевой курс валюты расчетов!'");
		...
		Если НЕ ЗначениеЗаполнено(КратностьРасчетов) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ka='აღმოჩენილია ანგარიშსწორებათა ვალუტის კურსის ნულოვანი ჯერადობა!';ru='Обнаружена нулевая кратность курса валюты расчетов!'");
	...
	Если ПерезаполнитьЦены И ВидЦенЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(ВидЦен) Тогда
			...
			Иначе
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = НСтр("ka='არ არის ფასის ტიპი არჩეული შესავსებად!';ru='Не выбран вид цены для заполнения!'");
