...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(АвтономноеРабочееМесто) Тогда
		ВызватьИсключение НСтр("ka='ავტონომიური სამუშაო ადგილი მიცემული არ არის.';ru='Не задано автономное рабочее место.';en='Not defined standalone workstation.'");
	КонецЕсли;
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Исключение
		...
		ДлительнаяОперация = Ложь;
		ПерейтиНазад();
		ПоказатьПредупреждение(,НСтр("ka='მუშაობის პროცესში შეცდომები წარმოიშვა.';ru='В процессе работы возникли ошибки.';en='In the process, any errors.'"));
...
Процедура УдалитьАвтономноеРабочееМесто(Отказ)
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
					...
					НСтр("ka='ავტონომიური სამუშაო ადგილის წაშლა';ru='Удаление автономного рабочего места';en='Removal autonomous workplace'"));
