...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(АвтономноеРабочееМесто) Тогда
		ВызватьИсключение НСтр("ka='ავტონომიური სამუშაო ადგილი მიცემული არ არის.';ru='Не задано автономное рабочее место.'");
	КонецЕсли;
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Исключение
		...
		ДлительнаяОперация = Ложь;
		ПерейтиНазад();
		ПоказатьПредупреждение(,НСтр("ka='მუშაობის პროცესში შეცდომები წარმოიშვა.';ru='В процессе работы возникли ошибки.'"));
...
Процедура УдалитьАвтономноеРабочееМесто(Отказ)
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
					...
					НСтр("ka='ავტონომიური სამუშაო ადგილის წაშლა';ru='Удаление автономного рабочего места'"));
