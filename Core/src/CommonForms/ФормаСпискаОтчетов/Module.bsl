...
Процедура Подключаемый_Избранное(Команда)
	...
	Если Добавлен = Истина Тогда
		Состояние(НСтр("ka='ანგარიშგება დამატებულია რჩეულებში';ru='Отчет добавлен в избранное'"),,, БиблиотекаКартинок.БольшаяЖелтаяЗвезда);
...
Процедура Подключаемый_УдалитьОтчет(Команда)
	...
	ПоказатьВопрос(
	ОписаниеОповещения,
	СтрШаблон(НСтр("ka='წავშალოთ ანგარიშგება ""%1""?';ru='Удалить отчет ""%1""?'"), ДанныеОтчета.Наименование),
...
Процедура ДобавитьКомандуНастройкиОтчета(КонтекстноеМеню, Знач Стр)
	...
	Команда = Команды.Добавить(ИмяКоманды);
	Команда.Заголовок = НСтр("ka='ანგარიშგების გამართვები';ru='Настройки отчета'");
...
Процедура ДобавитьКомандуУдалитьОтчет(КонтекстноеМеню, Стр, ПолныеПраваНаВарианты)
	...
	Команда = Команды.Добавить(ИмяКоманды);
	Команда.Заголовок = НСтр("ka='წაშლა';ru='Удалить'");
...
Процедура ОбновитьКомандуИзбранное(ИмяКоманды, НеВИзбранном)
	...
	Если НеВИзбранном Тогда
		Команда.Заголовок = НСтр("ka='რჩეულში';ru='В избранное'");
		Команда.Картинка = БиблиотекаКартинок.БольшаяСераяЗвезда;
	Иначе
		Команда.Заголовок = НСтр("ka='რჩეულიდან წაშლა';ru='Удалить из избранного'");
...
Функция ТекстПодсказкиДляПустогоСписка()
	...
	Если Раздел = РазделВсеОтчеты()
		...
		Возврат НСтр("ka='შერჩევის შესაფერისი ანგარიშგებები ვერ მოიძებნა.';ru='Не найдены отчеты, отвечающие отбору.'");
		
	ИначеЕсли Раздел = РазделИзбранные() Тогда
		
		Возврат НСтр("ka='ჯერ-ჯერობით, არცერთი ანგარიშგება არ არის რჩეულებში დამატებული.';ru='Пока ни один отчет не добавлен в избранное.'");
		
	ИначеЕсли Раздел = РазделПоследние() Тогда
		
		Возврат НСтр("ka='ამ სიაში ავტომატურად ხდება უკანასკნელად გამოყენებული ანგარიშგებების დამატება. სია, ჯერ-ჯერობით ცარიელია.';ru='В этот список автоматически добавляются последние использованные отчеты. Список пока пуст.'");
...
Процедура УстановитьВидимостьИДостпность()
	...
	Если ПараметрыФормы.ЕстьВнешниеОтчеты Тогда
		...
		Если Элемент = Неопределено Тогда
			Элементы.Раздел.СписокВыбора.Добавить("Внешние", НСтр("ka='გარე';ru='Внешние'"));
		КонецЕсли; 
	Иначе
		Элемент = Элементы.Раздел.СписокВыбора.НайтиПоЗначению("Внешние");
		Если Элемент <> Неопределено Тогда
...
Процедура ОбновитьЗаголовок()
	...
	ИначеЕсли Раздел = РазделВсеОтчеты() Тогда
		...
		Заголовок = НСтр("ka='ყველა ანგარიშგება';ru='Все отчеты'");
		
	ИначеЕсли Раздел = РазделИзбранные() Тогда
		
		Заголовок = НСтр("ka='ამორჩეული ანგარიშგებები';ru='Избранные отчеты'");
		
	ИначеЕсли Раздел = РазделПоследние() Тогда
		
		Заголовок = НСтр("ka='ბოლო ანგარისგებები';ru='Последние отчеты'");
		
	ИначеЕсли Раздел = РазделВнешние() Тогда
		
		Заголовок = НСтр("ka='გარე ანგარიშგებები';ru='Внешние отчеты'");
...
Процедура ОткрытьДополнительныйОтчет(Вариант)
	...
	ИначеЕсли ВыполняемаяКоманда.ВариантЗапуска = ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		Иначе
			Состояние(НСтр("ka='სრულდება ბრძანება...';ru='Выполняется команда...'"),,, БиблиотекаКартинок.ДлительнаяОперация48);
...
Функция ВыполнитьСерверныйМетодОбработкиНаСервере(ПараметрыВызоваСервера)
	...
	НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები: დამუშავების სერვერული მეთოდის შესრულება';ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки'"),
	,
	Ложь);
...
Процедура ПоказатьРезультатВыполненияОбработки(Задание, ЗакрытьЭтуФорму)
	...
	Если Задание.Успешно Тогда
		...
		Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
			ПоказатьОповещениеПользователя(НСтр("ka='ბრძანება შესრულებულია';ru='Команда выполнена'"),, ВыполняемаяКоманда.Представление);
		КонецЕсли;
...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере(ФоновоеЗаданиеИдентификатор, ФоновоеЗаданиеАдресХранилища, Отменить = Ложь)
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение НСтр("ka='ფონური დავალება ვერ მოიძებნა.';ru='Фоновое задание не найдено.'");
