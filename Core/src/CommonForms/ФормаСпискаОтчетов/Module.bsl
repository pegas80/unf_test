...
Процедура Подключаемый_Избранное(Команда)
	...
	Если Добавлен = Истина Тогда
		Состояние(НСтр("ka='ანგარიშგება დამატებულია რჩეულებში';ru='Отчет добавлен в избранное';en='The report added to favorites'"),,, БиблиотекаКартинок.БольшаяЖелтаяЗвезда);
...
Процедура Подключаемый_УдалитьОтчет(Команда)
	...
	ПоказатьВопрос(
	ОписаниеОповещения,
	СтрШаблон(НСтр("ka='წავშალოთ ანგარიშგება ""%1""?';ru='Удалить отчет ""%1""?';en='Delete ""%1"" report?'"), ДанныеОтчета.Наименование),
...
Процедура ДобавитьКомандуНастройкиОтчета(КонтекстноеМеню, Знач Стр)
	...
	Команда = Команды.Добавить(ИмяКоманды);
	Команда.Заголовок = НСтр("ka='ანგარიშგების გამართვები';ru='Настройки отчета';en='report settings'");
...
Процедура ДобавитьКомандуУдалитьОтчет(КонтекстноеМеню, Стр, ПолныеПраваНаВарианты)
	...
	Команда = Команды.Добавить(ИмяКоманды);
	Команда.Заголовок = НСтр("ka='წაშლა';ru='Удалить';en='Delete'");
...
Процедура ОбновитьКомандуИзбранное(ИмяКоманды, НеВИзбранном)
	...
	Если НеВИзбранном Тогда
		Команда.Заголовок = НСтр("ka='რჩეულში';ru='В избранное';en='To favorites'");
		Команда.Картинка = БиблиотекаКартинок.БольшаяСераяЗвезда;
	Иначе
		Команда.Заголовок = НСтр("ka='რჩეულიდან წაშლა';ru='Удалить из избранного';en='Remove from favorites'");
...
Функция ТекстПодсказкиДляПустогоСписка()
	...
	Если Раздел = РазделВсеОтчеты()
		...
		Возврат НСтр("ka='შერჩევის შესაფერისი ანგარიშგებები ვერ მოიძებნა.';ru='Не найдены отчеты, отвечающие отбору.';en='Found records that meet the selection.'");
		
	ИначеЕсли Раздел = РазделИзбранные() Тогда
		
		Возврат НСтр("ka='ჯერ-ჯერობით, არცერთი ანგარიშგება არ არის რჩეულებში დამატებული.';ru='Пока ни один отчет не добавлен в избранное.';en='So far, none of the report was not added to favorites.'");
		
	ИначеЕсли Раздел = РазделПоследние() Тогда
		
		Возврат НСтр("ka='ამ სიაში ავტომატურად ხდება უკანასკნელად გამოყენებული ანგარიშგებების დამატება. სია, ჯერ-ჯერობით ცარიელია.';ru='В этот список автоматически добавляются последние использованные отчеты. Список пока пуст.';en='In this list are automatically added to the recently used reports. The list is empty so far.'");
...
Процедура УстановитьВидимостьИДостпность()
	...
	Если ПараметрыФормы.ЕстьВнешниеОтчеты Тогда
		...
		Если Элемент = Неопределено Тогда
			Элементы.Раздел.СписокВыбора.Добавить("Внешние", НСтр("ka='გარე';ru='Внешние';en='external'"));
		КонецЕсли; 
	Иначе
		Элемент = Элементы.Раздел.СписокВыбора.НайтиПоЗначению("Внешние");
		Если Элемент <> Неопределено Тогда
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ПоОбъекту) Тогда
		...
		Заголовок = НСтр("ru = 'Отчеты по: '; ka = 'ანგარიშგებით: '; en = 'Reports on:'") + ПоОбъекту;
		
	ИначеЕсли Раздел = РазделВсеОтчеты() Тогда
		
		Заголовок = НСтр("ka='ყველა ანგარიშგება';ru='Все отчеты';en='All reports'");
		
	ИначеЕсли Раздел = РазделИзбранные() Тогда
		
		Заголовок = НСтр("ka='ამორჩეული ანგარიშგებები';ru='Избранные отчеты';en='Favorite reports'");
		
	ИначеЕсли Раздел = РазделПоследние() Тогда
		
		Заголовок = НСтр("ka='ბოლო ანგარისგებები';ru='Последние отчеты';en='Recent reports'");
		
	ИначеЕсли Раздел = РазделВнешние() Тогда
		
		Заголовок = НСтр("ka='გარე ანგარიშგებები';ru='Внешние отчеты';en='external reports'");
...
Процедура ОткрытьДополнительныйОтчет(Вариант)
	...
	ИначеЕсли ВыполняемаяКоманда.ВариантЗапуска = ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		Иначе
			Состояние(НСтр("ka='სრულდება ბრძანება...';ru='Выполняется команда...';en='The command ...'"),,, БиблиотекаКартинок.ДлительнаяОперация48);
...
Функция ВыполнитьСерверныйМетодОбработкиНаСервере(ПараметрыВызоваСервера)
	...
	НСтр("ka='დამატებითი ანგარიშგებები და დამუშავებები: დამუშავების სერვერული მეთოდის შესრულება';ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки';en='Additional reports and processing: Perform server processing method'"),
	,
	Ложь);
...
Процедура ПоказатьРезультатВыполненияОбработки(Задание, ЗакрытьЭтуФорму)
	...
	Если Задание.Успешно Тогда
		...
		Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
			ПоказатьОповещениеПользователя(НСтр("ka='ბრძანება შესრულებულია';ru='Команда выполнена';en='command complete'"),, ВыполняемаяКоманда.Представление);
		КонецЕсли;
...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере(ФоновоеЗаданиеИдентификатор, ФоновоеЗаданиеАдресХранилища, Отменить = Ложь)
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение НСтр("ka='ფონური დავალება ვერ მოიძებნა.';ru='Фоновое задание не найдено.';en='Background job is not found.'");
