...
"Передано: %1';ka='პარამეტრ ბრძანებისპარამეტრი-ს დაუშვებელი მნიშვნელობა მეთოდ ამობეჭდვისმართვაკლიენტი.ამობეჭდვისბრძანებისშესრულება-ს მოხმობისას."
"მოსალოდნელი იყო: მასივი, ნებისმიერიბმული."
"გადაცემულია: %1'"), ТипЗнч(Параметры.ПараметрКоманды)));
	КонецЕсли;
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ОбщаяФорма",
		Неопределено,
		НСтр("ru='Новости: Печать';en='News: Print'"),
		Ложь,
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				Для Каждого ЗаписанныйОбъект Из ЗаписанныеОбъекты Цикл
					...
					ОбсужденияВызовСервера.ДобавитьСообщение(НСтр("ru='Присоединен файл ';ka='Присоединен файл '")+ТекстИсточник, ВыбранноеЗначение.ОбъектДляПрикрепления, Истина);
					//УНФ Конец
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если НастройкаПечатнойФормы.Количество <> 1 Тогда
			НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление 
				+ " (" + НастройкаПечатнойФормы.Количество + " " + НСтр("ka='экз.';ru='экз.';en='ind.'") + ")";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ЕстьРазрешенныйВывод Тогда
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Запретить Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ka='გამოტანა მიუწვდომელია';ru='вывод не доступен';en='O is not available'") + ")";
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать(ИмяПринтера = Неопределено)
	...
	Если ПустаяСтрока(ИмяПринтера) Тогда
		ИмяПринтера = НСтр("ru='Принтер по умолчанию';en='Default Printer'");
	КонецЕсли;
	
	ПодсказкаКнопкиПечать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Напечатать на принтере (%1)';en='Printed on the printer (1%)'"),  ИмяПринтера);
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		Иначе 	
			Шаблон = НСтр("en='[NazvaniePechatnoyFormy] number [number] of [date]';ka='[НазваниеПечатнойФормы] № [Номер]  [Дата]';ru='[НазваниеПечатнойФормы] № [Номер] от [Дата]'");
		КонецЕсли
	Иначе
		...
		Шаблон = НСтр("ka='[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]';ru='[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]';en='[NazvaniePechatnoyFormy] - [PredstavlenieObekta] - [TekuschayaData]'");
	КонецЕсли;
