...
"გადაცემულია: %1'"), ТипЗнч(Параметры.ПараметрКоманды)));
	...
	Если ЭтоПечатьКомплекта() Тогда
		Элементы.Копий.Заголовок = НСтр("ka='კომპლექტის ასლები';ru='Копий комплекта'");
	КонецЕсли;
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.СохранениеПечатнойФормы") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				КонецЦикла;
				Состояние(НСтр("ka='შენხვა წარმატებით დასრულდა.';ru='Сохранение успешно завершено.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ПерейтиКДокументу(Команда)
	...
	СписокВыбора.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ka='ბეჭდურ ფორმაზე გადასვლა';ru='Перейти к печатной форме'"));
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если НастройкаПечатнойФормы.Количество <> 1 Тогда
			НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление 
				+ " (" + НастройкаПечатнойФормы.Количество + " " + НСтр("ka='экз.';ru='экз.'") + ")";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ЕстьРазрешенныйВывод Тогда
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Запретить Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ka='გამოტანა მიუწვდომელია';ru='вывод не доступен'") + ")";
			ИначеЕсли ПолеТабличногоДокумента.Защита Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ka='მხოლოდ ბეჭდვა';ru='только печать'") + ")";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьЗаголовокФормы()
	...
	Иначе
		Если ЭтоПечатьКомплекта() Тогда
			Заголовок = НСтр("ka='კომპლექტის ბეჭდვა';ru='Печать комплекта'");
		ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) <> Тип("Массив") Или Параметры.ПараметрКоманды.Количество() > 1 Тогда
			Заголовок = НСтр("ka='დოკუმენტის ბეჭდვა';ru='Печать документов'");
		Иначе
			Заголовок = НСтр("ka='დოკუმენტის ბეჭდვა';ru='Печать документа'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		Иначе 	
			Шаблон = НСтр("ka='[НазваниеПечатнойФормы] № [Номер]  [Дата]';ru='[НазваниеПечатнойФормы] № [Номер] от [Дата]';en='[NazvaniePechatnoyFormy] number [number] of [date]'");
		КонецЕсли
	Иначе
...
Функция ПолучитьИмяФайлаДляАрхива()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Иначе
			Результат = НСтр("ka='დოკუმენტები';ru='Документы'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	Состояние(НСтр("ka='შენახვა წარმატებით დასრულდა';ru='Сохранение успешно завершено'"), , НСтр("ka='საქაღალდეში:';ru='в папку:'") + " " + Папка);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Для Каждого Файл Из ФайлыВоВременномХранилище Цикл
			...
			Результат.Добавить(МодульПрисоединенныеФайлы.ДобавитьПрисоединенныйФайл(
				ПараметрыФайла, Файл.АдресВоВременномХранилище, , НСтр("ka='საბეჭდი ფორმა';ru='Печатная форма'")));
		КонецЦикла;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОткрытьМакетДляРедактирования()
	...
	ОтобразитьСостояниеТекущейПечатнойФормы(НСтр("ka='მაკეტი რედაქტირდება';ru='Макет редактируется'"));
...
Процедура ПереформироватьПечатнуюФорму(ИмяМакета, ИмяРеквизита)
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ka='ბეჭდური ფორმა ხელახლა არ ჩამოყალიბებულა.';ru='Печатная форма не была переформирована.'");
	КонецЕсли;
