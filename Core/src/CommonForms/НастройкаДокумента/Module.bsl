...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Lazare_
	Если Параметры.Свойство("ПоложениеСкладаВЗаказеПокупателя") Тогда 
		ПоложениеСкладаВЗаказеПокупателя = Параметры.ПоложениеСкладаВЗаказеПокупателя ; 
		ПоложениеСкладаВЗаказеПокупателяПриОткрытии = Параметры.ПоложениеСкладаВЗаказеПокупателя ;
		Элементы.ПоложениеСкладаВЗаказеПокупателя.Видимость = Истина ;
	Иначе 
		Элементы.ПоложениеСкладаВЗаказеПокупателя.Видимость = Ложь;
	КонецЕсли;	
	Если Параметры.Свойство("ПоложениеСкладаВДокументахОтгрузки") Тогда 
		ПоложениеСкладаВДокументахОтгрузки = Параметры.ПоложениеСкладаВДокументахОтгрузки ; 
		ПоложениеСкладаВДокументахОтгрузкиПриОткрытии = Параметры.ПоложениеСкладаВДокументахОтгрузки ;
		Элементы.ПоложениеСкладаВДокументахОтгрузки.Видимость = Истина ;
	Иначе 
	...
	Если Параметры.Свойство("ПоложениеЯчейкиВПеремещенииЗапасов") Тогда
		ПоложениеЯчейкиВПеремещенииЗапасов = Параметры.ПоложениеЯчейкиВПеремещенииЗапасов;
		ПоложениеЯчейкиВПеремещенииЗапасовПриОткрытии = Параметры.ПоложениеЯчейкиВПеремещенииЗапасов;
		Элементы.ПоложениеЯчейкиВПеремещенииЗапасов.Видимость = Истина;
	Иначе
		Элементы.ПоложениеЯчейкиВПеремещенииЗапасов.Видимость = Ложь;
	КонецЕсли;
...
Процедура ЗаписатьНовыеНастройки()
	...
	Если ЗначениеЗаполнено(ПоложениеЯчейкиВПеремещенииЗапасов) Тогда
		УстановитьНастройку("ПоложениеЯчейкиВПеремещенииЗапасов");
	КонецЕсли;
	...
	КонецЕсли;	
	//Lazare_
	Если ЗначениеЗаполнено(ПоложениеСкладаВЗаказеПокупателя) Тогда
		УстановитьНастройку("ПоложениеСкладаВЗаказеПокупателя");
	КонецЕсли;
	Если ЗначениеЗаполнено(ПоложениеСкладаВДокументахОтгрузки) Тогда
		УстановитьНастройку("ПоложениеСкладаВДокументахОтгрузки");
	КонецЕсли;
	//_Lazare	
...
Процедура ПроверитьМодифицированностьФормы(СтруктураРеквизитовФормы)
	...
	ИзмененияПоложениеЯчейкиВПеремещенииЗапасов                 = ПоложениеЯчейкиВПеремещенииЗапасовПриОткрытии <> ПоложениеЯчейкиВПеремещенииЗапасов; 
	//ИзмененияПоложениеОтветственныйПриОткрытии		            = ПоложениеОтветственныйПриОткрытииПриОткрытии <> ПоложениеОтветственныйПриОткрытии;
	...
	//Lazare_
	ИзмененияПоложениеСкладаВЗаказеПокупателя					= ПоложениеСкладаВЗаказеПокупателяПриОткрытии <> ПоложениеСкладаВЗаказеПокупателя;
	ИзмененияПоложениеСкладаВДокументахОтгрузки                 = ПоложениеСкладаВДокументахОтгрузкиПриОткрытии <> ПоложениеСкладаВДокументахОтгрузки ;
	//_Lazare	
	...
	Если ИзмененияПоложениеВидаРаботВЗаказНаряде
		...
		ИЛИ ИзмененияПоложениеЯчейкиВПеремещенииЗапасов
		//ИЛИ ИзмененияПоложениеОтветственныйПриОткрытии
		...
		//Lazare_
		ИЛИ ИзмененияПоложениеСкладаВЗаказеПокупателя
		ИЛИ ИзмененияПоложениеСкладаВДокументахОтгрузки
		//_Lazar
		Тогда
	...
	СтруктураРеквизитовФормы.Вставить("ПоложениеЯчейкиВПеремещенииЗапасов",	 		            ПоложениеЯчейкиВПеремещенииЗапасов);
	...
	//Lazare_
	СтруктураРеквизитовФормы.Вставить("ПоложениеСкладаВЗаказеПокупателя",						ПоложениеСкладаВЗаказеПокупателя);
	СтруктураРеквизитовФормы.Вставить("ПоложениеСкладаВДокументахОтгрузки",						ПоложениеСкладаВДокументахОтгрузки);
	//_Lazare	
