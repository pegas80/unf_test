...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ka='სერვისის მოდელში მუშაობისას ინტერნეტ-მხარდაჭერის გამოყენება მიუწვდომელია.';ru='Использование Интернет-поддержки недоступно при работе в модели сервиса.';en='Using e-support is not available when working in the service model.'");
	КонецЕсли;
...
Процедура НадписьПоясненияПодключенияАвторизацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "action:openPortal" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("ka='პორტალი  1С:ИТС';ru='Портал 1С:ИТС';en='Portal 1C: ITS'"));
		
	Иначе
		
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ka='ინტერნეტ-მხარდაჭერა. ინტერნეტ-მხარდაჭერის შეერთება.';ru='Интернет-поддержка. Подключение Интернет-поддержки.';en='Internet support. Internet connection support.'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='მომხმარებლების ინტერნეტ-მხარდაჭერის შეერთება ვერ ხერხდება."
"შესაერთებლად მიეთითება ლოგ-ინი %1.';ru='Не получается подключить Интернет-поддержку пользователей."
...
Процедура НадписьВосстановленияПароляАвторизацияНажатие(Элемент)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ka='პაროლის აღდგენა';ru='Восстановление пароля';en='password reset'"));
...
Процедура КомандаОК(Команда)
	...
	Состояние(, , НСтр("ka='ინტერნეტ-მხარდაწერის ჩართვა...';ru='Подключение Интернет-поддержки...';en='Connecting Online Support ...'"));
...
"%1';ka='Логин и пароль сохранены в программе, но проверка корректности"
"логина и пароля не выполнена из-за ошибки:"
"%1'"),
...
Функция ЗаполнениеЛогинаИПароляКорректно()
	...
	Если ПустаяСтрока(Логин) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ველი ""ლოგ-ინი"" არ არის შევსებული.';ru='Поле ""Логин"" не заполнено.';en='""Login"" field is blank.'"),
			,
			"Логин");
		Возврат Ложь;
	...
	ИначеЕсли ПустаяСтрока(Пароль) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ველი ""პაროლი"" არ არის შევსებული.';ru='Поле ""Пароль"" не заполнено.';en='""Password"" field is blank.'"),
			,
			"Пароль");
		Возврат Ложь;
...
Процедура ВывестиДатуНастройкиНеНапоминатьОбАвторизацииДо()
	...
	ОбщийЗаголовокФлажка = НСтр("ka='შეერთების შესახებ არ შემახსენო შვიდი დღე';ru='Не напоминать о подключении семь дней';en='Do not remind me about connecting seven days'");
	...
	СтрокаФлажка = ОбщийЗаголовокФлажка
		+ ?(ЗначениеНастройки = '00010101',
			"",
			" " + НСтр("ka='(до';ru='(до';en='(before'") + " " + Формат(ЗначениеНастройки, "ДЛФ=D") + ")");
