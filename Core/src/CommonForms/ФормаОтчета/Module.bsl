...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Паганини#Nexia_
	Если Строка(ОтчетОбъект) = "ОтчетОбъект.СравнитьДанные_Документа_И_RS" И НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьНесколькоОрганизаций") Тогда
		Если НЕ ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("Организация") = Неопределено Тогда
			ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("Организация").ОграничениеИспользования = Истина;
	        ...
	        ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьНастройки(ОтчетОбъект.КомпоновщикНастроек.Настройки);
		КонецЕсли;
	КонецЕсли;
	//_Паганини#Nexia 
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Иначе
		...
		ПараметрыЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Выполнение отчета: %1';ru='Выполнение отчета: %1';en='Execution report:% 1'"),
