...
Процедура ПриОткрытии(Отказ)
	...
	Если ПоказатьОповещениеОПодсказках И ПоказыватьПодсказки Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ka='ახალი შესაძლებლობა';ru='Новая возможность'"),
			"e1cib/data/ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ОписаниеНовойВозможностиПоВыводуОписаний",
			НСтр("ka='აღწერილობების გამოტანა ანგარიშგებების პანელში';ru='Вывод описаний в панелях отчетов'"),
...
Функция ВведенаСлишкомКороткаяСтрокаПоиска(Текст)
	...
	Если СтрДлина(Текст) < 2 Тогда
		ПоказатьПредупреждение(, НСтр("ka='შეყვანილია ძებნის საკმაოდ მოკლე სტრიქონი.';ru='Введена слишком короткая строка поиска.'"));
		Возврат Истина;
	КонецЕсли;
	...
	Если Не ЕстьНормальноеСлово Тогда
		ПоказатьПредупреждение(, НСтр("ka='მოსაძებნად შეყვანილია საკმაოდ მოკლე სიტყვები.';ru='Введены слишком короткие слова для поиска.'"));
		Возврат Истина;
	КонецЕсли;
...
Процедура СброситьНастройки(Команда)
	ТекстВопроса = НСтр("ka='ჩამოვაგდოთ ანგარიშგებების განთავსების გამართვები?';ru='Сбросить настройки расположения отчетов?'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = Комментарий + "; " + НСтр("ka='ანგარიშგებების პანელიდან:';ru='Из панели отчетов:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		Комментарий = Комментарий + "; " + НСтр("ka='კარნახი:';ru='Подсказки:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			+ "; " + НСтр("ka='ძებნა:';ru='Поиск:'") + " " + Строка(СтрокаПоиска)
			+ "; " + НСтр("ka='ყველა ნაწილში';ru='Во всех разделах:'") + " " + Строка(ИскатьВоВсехРазделах);
	Иначе
		Комментарий = Комментарий + "; " + НСтр("ka='ძებნის გარეშე';ru='Без поиска'");
	КонецЕсли;
	
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = Комментарий + "; " + НСтр("ka='გამართვის რეჟიმიდან გამოსვლა';ru='Выход из режима настройки'");
	КонецЕсли;
...
Процедура ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	// Заголовок.
	СуффиксРежимаНастройки = " (" + НСтр("ka='აწყობა';ru='настройка'") + ")";
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Иначе
		...
		Если ПараметрыЗаполнения.ДругиеРазделы.Количество() = 0 Тогда
			...
			Надпись.Заголовок = "    " + НСтр("ka='ანგარიშგებები სხვა ნაწილებში ვერ მოიძებნა.';ru='Отчеты в других разделах не найдены.'") + Символы.ПС;
			Надпись.Высота = 2;
		КонецЕсли;
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			ЗаголовокНадписи = НСтр("ka='სხვა ნაწილებიდან გამოტანილია პირველი %1 ანგარიშგება, დააზუსტეთ საძიებო მოთხოვნა.';ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.'");
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Если ПараметрыЗаполнения.ВариантыКоличество = 0 Тогда
		...
		Если ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
				Если ПараметрыЗаполнения.ТолькоТекущийРаздел Тогда
					Надпись.Заголовок = НСтр("ka='ანგარიშგებები ვერ მოიძებნა.';ru='Отчеты не найдены.'");
				Иначе
					Надпись.Заголовок = НСтр("ka='მიმდინარე ნაწილში ანგარიშგებები ვერ მოიძებნა.';ru='Отчеты в текущем разделе не найдены.'");
					Надпись.Высота = 2;
				КонецЕсли;
			Иначе
				Надпись.Заголовок = НСтр("ka='ამ ნაწილის ანგარიშგებების პანელში არცერთი ანგარიშგება არ არის განთავსებული.';ru='В панели отчетов этого раздела не размещено ни одного отчета.'");
			КонецЕсли;
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если ТребуетсяПодсветка Тогда
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			Суффикс = НСтр("ka='(გაგრძელება)';ru='(продолжение)'");
	...
	Иначе
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			ПодсистемаПредставление = ПодсистемаПредставление + " " + НСтр("ka='(გაგრძელება)';ru='(продолжение)'");
		КонецЕсли;
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка + Символы.ПС) + НСтр("ka='ავტორი';ru='Автор:'") + " " + СокрЛП(Строка(Вариант.Автор));
	КонецЕсли;
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		...
		Если ПараметрыПодсветки <> Неопределено Тогда
			...
			Если ПараметрыПодсветки.ПредставлениеАвтора.КоличествоНайденныхСлов > 0 Тогда
				...
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='ავტორი';ru='Автор:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПользовательскихНастроек.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='შენახული აწყობები';ru='Сохраненные настройки:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПолей.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='ველები:';ru='Поля:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПараметровИОтборов.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='აწყობები:';ru='Настройки:'") + " ");
			...
			Если ПараметрыПодсветки.КлючевыеСлова.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='საკვანძო სიტყვები';ru='Ключевые слова:'") + " ");
