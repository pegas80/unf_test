...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТекущийРазделСсылка") Тогда
		ТекущийРазделСсылка = Параметры.ТекущийРазделСсылка;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПоказатьОповещениеОПодсказках И ПоказыватьПодсказки Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ka='ახალი შესაძლებლობა';ru='Новая возможность';en='new feature'"),
			"e1cib/data/ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ОписаниеНовойВозможностиПоВыводуОписаний",
			НСтр("ka='აღწერილობების გამოტანა ანგარიშგებების პანელში';ru='Вывод описаний в панелях отчетов';en='The conclusion in the report descriptions panels'"),
...
Функция ВведенаСлишкомКороткаяСтрокаПоиска(Текст)
	...
	Если СтрДлина(Текст) < 2 Тогда
		ПоказатьПредупреждение(, НСтр("ka='შეყვანილია ძებნის საკმაოდ მოკლე სტრიქონი.';ru='Введена слишком короткая строка поиска.';en='Introduced too short search string.'"));
		Возврат Истина;
	КонецЕсли;
	...
	Если Не ЕстьНормальноеСлово Тогда
		ПоказатьПредупреждение(, НСтр("ka='მოსაძებნად შეყვანილია საკმაოდ მოკლე სიტყვები.';ru='Введены слишком короткие слова для поиска.';en='Introduced too short search words.'"));
		Возврат Истина;
	КонецЕсли;
...
Процедура СброситьНастройки(Команда)
	ТекстВопроса = НСтр("ka='ჩამოვაგდოთ ანგარიშგებების განთავსების გამართვები?';ru='Сбросить настройки расположения отчетов?';en='I reset the report arrangement?'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = Комментарий + "; " + НСтр("ka='ანგარიშგებების პანელიდან:';ru='Из панели отчетов:';en='Reports from the panel:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		Комментарий = Комментарий + "; " + НСтр("ka='კარნახი:';ru='Подсказки:';en='tips:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			+ "; " + НСтр("ka='ძებნა:';ru='Поиск:';en='Search:'") + " " + Строка(СтрокаПоиска)
			+ "; " + НСтр("ka='ყველა ნაწილში';ru='Во всех разделах:';en='In all sections:'") + " " + Строка(ИскатьВоВсехРазделах);
	Иначе
		Комментарий = Комментарий + "; " + НСтр("ka='ძებნის გარეშე';ru='Без поиска';en='without searching'");
	КонецЕсли;
	
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = Комментарий + "; " + НСтр("ka='გამართვის რეჟიმიდან გამოსვლა';ru='Выход из режима настройки';en='Exit setup'");
	КонецЕсли;
...
Процедура ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	// Заголовок.
	СуффиксРежимаНастройки = " (" + НСтр("ka='აწყობა';ru='настройка';en='adjustment'") + ")";
...
"См. модуль ВариантыОтчетовПереопределяемый, процедуру ОпределитьРазделыСВариантамиОтчетов.';ka='ნაწილი ""%1"" ქვესისტემა ""%2""-ზე არ არის შეერთებული."
"იხ. მოდული ანგარიშგებებისვარიანტებიხელახლაგანსაზღვრებადი, პროცედურა ნაწილებისდადგენაანგარიშგებებისვარიანტებით.'"),
	...
	Если ЗначениеЗаполнено(ТекущийРазделСсылка) Тогда
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Иначе
		...
		Если ПараметрыЗаполнения.ДругиеРазделы.Количество() = 0 Тогда
			...
			Надпись.Заголовок = "    " + НСтр("ka='ანგარიშგებები სხვა ნაწილებში ვერ მოიძებნა.';ru='Отчеты в других разделах не найдены.';en='Reports in other sections are not found.'") + Символы.ПС;
			Надпись.Высота = 2;
		КонецЕсли;
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			ЗаголовокНадписи = НСтр("ka='სხვა ნაწილებიდან გამოტანილია პირველი %1 ანგარიშგება, დააზუსტეთ საძიებო მოთხოვნა.';ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';en='1% derived first reports from other sections specify the search query.'");
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения)
	...
	Запрос.УстановитьПараметр("ВариантыНайденныеПоиском", ?(ЗначениеЗаполнено(РезультатПоиска), РезультатПоиска.Ссылки, Неопределено));
	// © Шишка В.В. [5 мая 2022]  КОНЕЦ
	//--------------------------------------------------------------------------
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Если ПараметрыЗаполнения.ВариантыКоличество = 0 Тогда
		...
		Если ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
				Если ПараметрыЗаполнения.ТолькоТекущийРаздел Тогда
					Надпись.Заголовок = НСтр("ka='ანგარიშგებები ვერ მოიძებნა.';ru='Отчеты не найдены.';en='Records found.'");
				Иначе
					Надпись.Заголовок = НСтр("ka='მიმდინარე ნაწილში ანგარიშგებები ვერ მოიძებნა.';ru='Отчеты в текущем разделе не найдены.';en='Reports in this section is not found.'");
					Надпись.Высота = 2;
				КонецЕсли;
			Иначе
				Надпись.Заголовок = НСтр("ka='ამ ნაწილის ანგარიშგებების პანელში არცერთი ანგარიშგება არ არის განთავსებული.';ru='В панели отчетов этого раздела не размещено ни одного отчета.';en='The panel reports in this section is not aligned with any report.'");
			КонецЕсли;
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если ТребуетсяПодсветка Тогда
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			Суффикс = НСтр("ka='(გაგრძელება)';ru='(продолжение)';en='(Continued)'");
	...
	Иначе
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			ПодсистемаПредставление = ПодсистемаПредставление + " " + НСтр("ka='(გაგრძელება)';ru='(продолжение)';en='(Continued)'");
		КонецЕсли;
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка + Символы.ПС) + НСтр("ka='ავტორი';ru='Автор:';en='Author:'") + " " + СокрЛП(Строка(Вариант.Автор));
	КонецЕсли;
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		...
		Если ПараметрыПодсветки <> Неопределено Тогда
			...
			Если ПараметрыПодсветки.ПредставлениеАвтора.КоличествоНайденныхСлов > 0 Тогда
				...
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='ავტორი';ru='Автор:';en='Author:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПользовательскихНастроек.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='შენახული აწყობები';ru='Сохраненные настройки:';en='The saved settings:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПолей.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='ველები:';ru='Поля:';en='fields:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПараметровИОтборов.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='აწყობები:';ru='Настройки:';en='settings:'") + " ");
			...
			Если ПараметрыПодсветки.КлючевыеСлова.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ka='საკვანძო სიტყვები';ru='Ключевые слова:';en='Keywords:'") + " ");
