...
"только в форме объекта-владельца.';ka='მიერთებული ფაილების ნახვა შესაძლებელია მხოლოდ ობიექტ-მფლობელის ფორმაში.'");
	...
	Если Параметры.РежимВыбора Тогда
		...
		Заголовок = НСтр("ka='მიერთებული ფაილის არჩევა';ru='Выбор присоединенного файла'");
...
Процедура Печать(Команда)
	...
	Если СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86 
		И СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86_64 Тогда
			ПоказатьПредупреждение(, НСтр("ka='ფაილების ამობეჭდვა შესაძლებელია მხოლოდ Windows-ში.';ru='Печать файлов возможна только в Windows.'"));
			Возврат;
	КонецЕсли;
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаФайлов)
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებული ფაილების დინამიკური სიის გამართვისას.';ru='Ошибка при настройке динамического списка присоединенных файлов.'");
	ОкончаниеОшибки = НСтр("ka='ამ შემთხვევაში, დინამიკური სიის გამართვა შეუძლებელია.';ru='В этом случае настройка динамического списка невозможна.'");
...
Процедура ОбновитьПредпросмотр()
	...
	Иначе
		...
		Элементы.АдресДанныхФайла.ТекстНевыбраннойКартинки = НСтр("ka='წინასწარ სანახავად მონაცემები არ არის';ru='Нет данных для предварительного просмотра'");
	КонецЕсли;
...
Процедура ОбновитьПояснениеОблачногоСервиса()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЧастиСтроки.Добавить(НСтр("ka='ფაილებთან მუშაობა წარმოებს ღრუბლოვან სერვისში';ru='Работа с файлами ведется в облачном сервисе'"));
...
Процедура СформироватьСписокДокументов(Команда)
	...
	Если ВвестиЗначение(ТекДолжность,НСтр("ka='თანამდებობა';ru='Должность'"), Новый ОписаниеТипов("СправочникСсылка.Должности")) Тогда
