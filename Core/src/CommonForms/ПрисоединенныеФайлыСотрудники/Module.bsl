...
"только в форме объекта-владельца.';ka='მიერთებული ფაილების ნახვა შესაძლებელია მხოლოდ ობიექტ-მფლობელის ფორმაში.'");
	КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Заголовок = НСтр("ka='მიერთებული ფაილის არჩევა';ru='Выбор присоединенного файла';en='Selecting the attached file'");
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если (Поле.Имя = "Одобрен") ИЛИ (Поле.Имя = "მოწონებულია") Тогда
		СтандартнаяОбработка = Ложь;
		
		ТекСотрудникиПрисоединенныеФайлы = Элементы.Список.ТекущиеДанные.Ссылка;
		ИзменитьОдобрен(ТекСотрудникиПрисоединенныеФайлы);
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	ДанныеФайла.Наименование = "*"; // Kuzmich
	ДанныеФайла.Расширение = "*"; // Kuzmich
...
Процедура ОткрытьСвойстваФайла(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ПрисоединенныйФайлСотрудники", ПараметрыФормы);
...
Процедура Печать(Команда)
	...
	Если СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86 
		И СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86_64 Тогда
			ПоказатьПредупреждение(, НСтр("ka='ფაილების ამობეჭდვა შესაძლებელია მხოლოდ Windows-ში.';ru='Печать файлов возможна только в Windows.';en='Print files is only possible in Windows.'"));
			Возврат;
	КонецЕсли;
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаФайлов)
	...
	|	Файлы.НазваниеДокумента КАК НазваниеДокумента,
	|	Файлы.Одобрен КАК Одобрен
	|ИЗ
	...
	ЗаголовокОшибки = НСтр("ka='შეცდომა მიერთებული ფაილების დინამიკური სიის გამართვისას.';ru='Ошибка при настройке динамического списка присоединенных файлов.';en='Error when setting up a dynamic list of attached files.'");
	ОкончаниеОшибки = НСтр("ka='ამ შემთხვევაში, დინამიკური სიის გამართვა შეუძლებელია.';ru='В этом случае настройка динамического списка невозможна.';en='In this case adjust the dynamic list is not possible.'");
...
Процедура ОбновитьПредпросмотр()
	...
	Иначе
		...
		Элементы.АдресДанныхФайла.ТекстНевыбраннойКартинки = НСтр("ka='წინასწარ სანახავად მონაცემები არ არის';ru='Нет данных для предварительного просмотра';en='No data to preview'");
	КонецЕсли;
...
Процедура ОбновитьПояснениеОблачногоСервиса()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЧастиСтроки.Добавить(НСтр("ka='ფაილებთან მუშაობა წარმოებს ღრუბლოვან სერვისში';ru='Работа с файлами ведется в облачном сервисе';en='Working with files is carried out in a cloud service'"));
...
&НаКлиенте
Процедура СформироватьСписокДокументов(Команда)
	ТекДолжность = Неопределено;
	Если ВвестиЗначение(ТекДолжность,НСтр("ka='თანამდებობა';ru='Должность';en='Position'"), Новый ОписаниеТипов("СправочникСсылка.Должности")) Тогда
		СформироватьСписокДокументовНаСервере(ТекДолжность);
...
&НаСервере
Процедура СформироватьСписокДокументовНаСервере(ТекДолжность)
	Если НЕ ЗначениеЗаполнено(ТекДолжность) ИЛИ НЕ ЗначениеЗаполнено(ВладелецФайла) Тогда
		Возврат;
	КонецЕсли; 
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ВЫРАЗИТЬ(СправочникСотрудникиПрисоединенныеФайлы.НазваниеДокумента КАК СТРОКА(1000)) КАК НазваниеДокумента
		|ПОМЕСТИТЬ ТЗ_СуществующиеФайлы
		|ИЗ
		|	Справочник.СотрудникиПрисоединенныеФайлы КАК СправочникСотрудникиПрисоединенныеФайлы
		|ГДЕ
		|	СправочникСотрудникиПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла
		|	И НЕ СправочникСотрудникиПрисоединенныеФайлы.ПометкаУдаления
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СправочникДокументыДляТрудоустройстваСписокДокументов.НазваниеДокумента КАК НазваниеДокумента
		|ИЗ
		|	Справочник.ДокументыДляТрудоустройства.СписокДокументов КАК СправочникДокументыДляТрудоустройстваСписокДокументов
		|ГДЕ
		|	СправочникДокументыДляТрудоустройстваСписокДокументов.Ссылка.Должность = &ТекДолжность
		|	И НЕ ВЫРАЗИТЬ(СправочникДокументыДляТрудоустройстваСписокДокументов.НазваниеДокумента КАК СТРОКА(1000)) В
		|				(ВЫБРАТЬ
		|					ТЗ_СуществующиеФайлы.НазваниеДокумента
		|				ИЗ
		|					ТЗ_СуществующиеФайлы)";
	
	Запрос.УстановитьПараметр("ТекДолжность", ТекДолжность);
	Запрос.УстановитьПараметр("ВладелецФайла", ВладелецФайла);
	...
	Пока Выборка.Следующий() Цикл
		...
		ПрисоединенныйФайл = ПрисоединенныеФайлы.ДобавитьФайл(ВладелецФайла, Выборка.НазваниеДокумента, "txt", , , адресВХранилище);
		ПрисоединенныйФайлОбъект = ПрисоединенныйФайл.ПолучитьОбъект();
		ПрисоединенныйФайлОбъект.НазваниеДокумента = Выборка.НазваниеДокумента;
...
Процедура ИзменитьОдобрен(ТекСотрудникиПрисоединенныеФайлы)

	Если НЕ ЗначениеЗаполнено(ТекСотрудникиПрисоединенныеФайлы) Тогда
	
		Возврат;
	
	КонецЕсли;
	
	ТекСотрудникиПрисоединенныеФайлыОбъект = ТекСотрудникиПрисоединенныеФайлы.ПолучитьОбъект();
	ТекСотрудникиПрисоединенныеФайлыОбъект.Одобрен = НЕ ТекСотрудникиПрисоединенныеФайлыОбъект.Одобрен;
	ТекСотрудникиПрисоединенныеФайлыОбъект.Записать();
