...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ka='Заполнение реквизитов в данной конфигурации невозможно.';ru='Заполнение реквизитов в данной конфигурации невозможно.';en='Filling in the details of this configuration is not possible.'");
	...
	Элементы.Регион.СписокВыбора.Вставить(0, "", НСтр("ka='ყველა რეგიონი';ru='Все регионы';en='All regions'"));
	Регион = "";
...
"Указать сейчас?';ka='Необходимо указать пароль к Интернет-поддержке пользователей"
"и установить флажок ""Запомнить пароль""."
"Указать сейчас?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ПодключитьИнтернетПоддержку", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	...
	ИначеЕсли КоличествоНайденных > 20 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Найдено слишком много похожих контрагентов (%1). Уточните реквизиты для поиска.';ru='Найдено слишком много похожих контрагентов (%1). Уточните реквизиты для поиска.';en='Found too many similar counterparts (1%). Precise details for your search.'"),
			КоличествоНайденных);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	ИначеЕсли НЕ ПустаяСтрока(СтрокаПоиска) И КоличествоНайденных = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='Ничего не найдено. Уточните реквизиты для поиска.';ru='Ничего не найдено. Уточните реквизиты для поиска.';en='Nothing found. Precise details for your search.'"));
	КонецЕсли;
...
Процедура ВыполнитьВыборКонтрагента()
	...
	Если ТекДанные = Неопределено Тогда
		Если Контрагенты.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ka='Ничего не найдено. Уточните реквизиты и нажмите ""Найти"".';ru='Ничего не найдено. Уточните реквизиты и нажмите ""Найти"".';en='Nothing found. Precise details and click ""Search"".'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТекДанные.ИНН) Тогда
		ТекстПредупреждения = НСтр("ka='აირჩიეთ კონტრაგენტი, რომელსაც მითითებული აქვს გსნ.';ru='Выберите контрагента, у которого указан ИНН.';en='Choose a contractor who has indicated INN.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура НайтиРеквизитыНаСервере(ОписаниеОшибки)
	...
	Если КоличествоНайденных > 20 Тогда
		...
		Информация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Показаны первые 20 контрагентов из %1 найденных. Уточните реквизиты для поиска.';ru='Показаны первые 20 контрагентов из %1 найденных. Уточните реквизиты для поиска.';en='Shows the first 20% of the counterparts 1 found. Precise details for your search.'"), 
			КоличествоНайденных);
