...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			...
			"ОбщаяФорма",
			,
			НСтр("ka='Новости: Регламентированная отчетность';ru='Новости: Регламентированная отчетность';en='News: regulatory reporting'"),
			Ложь,
	...
	ТекстОшибкиИнициализацииКонтекстаЭДО = НСтр("ka='Подождите, пожалуйста. Выполняется обновление модуля 1С-Отчетности...';ru='Подождите, пожалуйста. Выполняется обновление модуля 1С-Отчетности...';en='Please wait. It is an upgrade module 1C-Accounting ...'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПриЗакрытии_ЭлектронныеПредставленияРегламентированныхОтчетов" Тогда	
		Если ТипЗнч(Параметр) = Тип("Структура") И Параметр.Свойство("ЭтоУведомление") Тогда
			ШаблонПредупреждения = НСтр("ka='""%1"" отображается в разделе ""%2""';ru='""%1"" отображается в разделе ""%2""';en='""%1"" is displayed in the ""%2""'");
			Если Параметр.ЭтоУведомление И Элементы.Разделы.ТекущаяСтраница = Элементы.СтраницаОтчеты Тогда
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредупреждения, Строка(Источник), НСтр("ka='Уведомления';ru='Уведомления';en='notice'"));
				ПоказатьПредупреждение(, ТекстПредупреждения);
			ИначеЕсли Не Параметр.ЭтоУведомление И Элементы.Разделы.ТекущаяСтраница = Элементы.СтраницаУведомления Тогда
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредупреждения, Строка(Источник), НСтр("ka='ანგარიშგებები';ru='Отчеты';en='Reports'"));
				ПоказатьПредупреждение(, ТекстПредупреждения);	
			КонецЕсли;
		КонецЕсли;
...
Процедура УправлениеЭУ(Форма, ВыполнитьУстановкуСтраницы = Ложь)
	...
	Если Форма.КоличествоНовых > 0 Тогда
		...
		Форма.ГиперссылкаНовое = СтрШаблон(НСтр("ka='Новое (%1)';ru='Новое (%1)';en='New (1%)'"), ?(Форма.КоличествоНовых > 99, "99+", Форма.КоличествоНовых));
	КонецЕсли;
...
"Установка компоненты печати машиночитаемых форм невозможна.';ka='ფაილებთან მუშაობის გაფართოების შეერთება ვერ მოხერხდა! მანქანაკითხვითი ფორმების ბეჭდვის კომპონენტის დაყენება შეუძლებელია.'"));
	Иначе
		ДиалогВыбораКаталога = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
...
Процедура ОбновитьДанные(Команда)
	...
	Если ВыбранныеОрганизации.Количество() = 0 Тогда
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(НСтр("ka='Организация не выбрана';ru='Организация не выбрана';en='The organization is not selected'"), ,"Организация");
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьОтчет(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) = Тип("СправочникСсылка.МакетыПенсионныхДел")
			...
			ТекстСообщения = НСтр("ka='Макет пенсионного дела уже отправлен';ru='Макет пенсионного дела уже отправлен';en='Layout of the pension case has already been sent'");			
		...
		Если СтрНайти(ТекущиеДанныеНаименованиеОтчета, "винограда") > 0 Тогда
			...
			ТекстПредупреждения = НСтр("ka='Проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.';ru='Проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.';en='Check the Internet, sending a version of the electronic form, approved by the PP RF №411 from 05.13.2016, it will be implemented in future versions of the program.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		...
		Если РегламентированнаяОтчетностьКлиент.ВебБраузер() = "MSIE" Тогда
			Если (ТекущиеДанныеНаименованиеОтчета = "РСВ-1 ПФР" И ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) <> Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов"))
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для выгрузки (проверки выгрузки, проверки в интернете, отправки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""';ru='Для выгрузки (проверки выгрузки, проверки в интернете, отправки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""';en='For discharging (unloading check, check on the internet, sending) in the browser ""Internet Explorer"" is necessary to open the regulated Report ""%1""'"),
			...
			Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
				...
				Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
					Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Отправка прервана.';ru='%1 Отправка прервана.';en='% 1 Sending interrupted.'"), ОписаниеДохИно.ТекстСообщения);
						Сообщение.Сообщить();
					КонецЕсли;
					Возврат;
				КонецЕсли;
		...
		Если ТекущиеДанныеНаименованиеОтчета = "РСВ-1 ПФР" И ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) <> Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
			...
			Если ОписаниеРСВ_1.ДатаКонцаПериодаОтчета >= '20140101' И НЕ (ПерсонифицированныйУчетВедетсяВоВнешнейПрограмме И ОписаниеРСВ_1.ОтчетСтарогоОбразца_20141101) Тогда
				ПоказатьПредупреждение(,НСтр("ka='Внимание! Отправлять в электронном виде форму РСВ-1 необходимо"
"из специализированного рабочего места подготовки квартальной"
"отчетности в ПФР. Отправка прервана.';ru='Внимание! Отправлять в электронном виде форму РСВ-1 необходимо"
"из специализированного рабочего места подготовки квартальной"
"отчетности в ПФР. Отправка прервана.'"));
		...
		Если ВыполнитьСтандартнуюОтправкуОтчета Тогда
			...
			Если ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган Тогда
				...
				ПредупредитьОбОграничениях(Закладка, НСтр("ka='Отправка не предусмотрена';ru='Отправка не предусмотрена';en='Sending is not provided'"));
			...
			Иначе
				...
				Если ЗначениеЗаполнено(КонтролирующийОрган) Тогда
					...
					КонецЕсли;
				Иначе
					ПоказатьПредупреждение(, НСтр("ka='Не указан контролирующий орган. Отправка невозможна';ru='Не указан контролирующий орган. Отправка невозможна';en='Unknown supervisory authority. are not sent'"));
				КонецЕсли;
...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
			...
			ПредупредитьОбОграничениях(Закладка, НСтр("ka='Проверка выгрузки не предусмотрена';ru='Проверка выгрузки не предусмотрена';en='discharge check is not provided'"));
			Возврат;
...
"Создайте новый отчет за период 2016 год или позже.';ka='Проверка выгрузки, выгрузка редакции формы, утвержденной ПП РФ №411 от 13.05.2016, "
"реализована начиная с отчетного периода за 2016 год."
			...
			Если ТекущиеДанныеНаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
				...
				Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
					...
					Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
						Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Проверка выгрузки прервана.';ru='%1 Проверка выгрузки прервана.';en='1% discharge check interrupted.'"), ОписаниеДохИно.ТекстСообщения);
							Сообщение.Сообщить();
						КонецЕсли;
						Возврат;
					КонецЕсли;
...
Процедура ПроверитьВыгрузкуЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьКлиент.ПроверитьРегламентированныеОтчеты(СписокОтчетов) Тогда
		ПоказатьПредупреждение(,НСтр("ka='შემოწმება შეწყვეტილია!';ru='Проверка прервана!';en='Check aborted!'"));
	КонецЕсли;
...
Процедура ОтправитьСверку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЕщеНеОтправлялся(ОтправляемыйОбъект) Тогда
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Запрос на сверку уже отправлен';ru='Запрос на сверку уже отправлен';en='Request for verification of already sent'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтправитьЗапросНаВыпискуИзЕГРЮЛ_ЕГРИП(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Запрос уже отправлен';ru='Запрос уже отправлен';en='A request has already been sent'"));
		КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			...
			Если ТипЗнч(ОтправляемыйОбъект) = Тип("СправочникСсылка.ОписиИсходящихДокументовВНалоговыеОрганы") Тогда
				ТекстСообщения = НСтр("ka='Ответ на требование уже отправлен';ru='Ответ на требование уже отправлен';en='Response to a request has already been sent'");
			ИначеЕсли ТипЗнч(ОтправляемыйОбъект) = Тип("ДокументСсылка.ПоясненияКДекларацииПоНДС") Тогда
				ТекстСообщения = НСтр("ka='Пояснение уже отправлено';ru='Пояснение уже отправлено';en='Explanation already sent'");
			Иначе
				ТекстСообщения = НСтр("ka='Пояснение уже отправлено';ru='Пояснение уже отправлено';en='Explanation already sent'");
			КонецЕсли;
...
Процедура ПечатьПисьма(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(ПечатаемыйОбъект) = Тип("СправочникСсылка.ДокументыРеализацииПолномочийНалоговыхОрганов") Тогда
			ПоказатьПредупреждение(, НСтр("ru='Печать требования не предусмотрена';en='Printing requirements are not provided'"));
			Возврат;
		ИначеЕсли ТипЗнч(ПечатаемыйОбъект) = Тип("СправочникСсылка.ОписиИсходящихДокументовВНалоговыеОрганы")
			...
			ПоказатьПредупреждение(, НСтр("ru='Печать ответа на требование не предусмотрена';en='Print answer to the requirement not provided'"));
			Возврат;
		КонецЕсли;
...
Процедура ПечатьУведомлений(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(УведомлениеДляПечати) = Тип("ДокументСсылка.УведомлениеОПолучателеДокументов") Тогда
			...
			ТекстПредупреждения = НСтр("ru='Печать уведомления о получателе документов не предусмотрена.';en='Print notification recipient documents are not available.'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
...
Процедура СоздатьОтвет(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ТранспортноеСообщение") Тогда
			...
			ТекстПредупреждения = НСтр("ka='Создание ответа на это письмо не предусмотрено';ru='Создание ответа на это письмо не предусмотрено';en='Creating a response to this letter is not provided'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
...
Процедура СообщитьОНевозможностиИзменитьПрочтенность()
	...
	Если ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Отчеты") Тогда
		ТекстСообщения = НСтр("ka='Отчет уже прочитан';ru='Отчет уже прочитан';en='The report has already read'");
	...
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Сверки") Тогда
		ТекстСообщения = НСтр("ka='Уведомление уже прочитано';ru='Уведомление уже прочитано';en='The notice has been read'");
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ") Тогда
		ТекстСообщения = НСтр("ka='Запрос выписки из ЕГРЮЛ уже прочитан';ru='Запрос выписки из ЕГРЮЛ уже прочитан';en='Request for Certificate of Incorporation already read'");
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Входящие") Тогда
		ТекстСообщения = НСтр("ka='Письмо уже прочитано';ru='Письмо уже прочитано';en='The letter has been read'");
	КонецЕсли;
...
Процедура ПодтвердитьСкрытие(ВыполняемоеОповещение)
	...
	Если ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Отчеты") Тогда
		...
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ka='отчеты';ru='отчеты';en='reports'");
		Иначе
			ВидОбъекта = НСтр("ka='отчет';ru='отчет';en='report'");
		КонецЕсли;
		
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Уведомления") Тогда
		
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ka='уведомления';ru='уведомления';en='notice'");
		Иначе
			ВидОбъекта = НСтр("ka='уведомление';ru='уведомление';en='notification'");
		КонецЕсли;

	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Сверки") Тогда
		
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ka='запросы на сверку';ru='запросы на сверку';en='requests for reconciliation'");
		Иначе
			ВидОбъекта = НСтр("ka='запросы на сверку';ru='запросы на сверку';en='requests for reconciliation'");
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ") Тогда
		...
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ka='выписки из ЕГРЮЛ';ru='выписки из ЕГРЮЛ';en='Certificate of Incorporation'");
		Иначе
			ВидОбъекта = НСтр("ka='выписки из ЕГРЮЛ';ru='выписки из ЕГРЮЛ';en='Certificate of Incorporation'");
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Входящие")
		...
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ka='письма';ru='письма';en='writing'");
		Иначе
			ВидОбъекта = НСтр("ka='письмо';ru='письмо';en='letter'");
		КонецЕсли;
...
Процедура УправлениеЭУПанельУстановкаКомпонентыПечати(ОперацияСКомпонентойСовершена = Ложь)
	...
	Если РегламентированнаяОтчетностьКлиент.КомпонентаПечатиМашиночитаемыхФормДоступна() Тогда
		Элементы.ПереустановитьКомпонентыПечатиМашиночитаемыхФорм.Заголовок = НСтр("ka='Переустановить компоненты печати машиночитаемых форм';ru='Переустановить компоненты печати машиночитаемых форм';en='Reinstall components printing machine-readable form'");
	Иначе
		Элементы.ПереустановитьКомпонентыПечатиМашиночитаемыхФорм.Заголовок = НСтр("ka='Установка компоненты печати';ru='Установка компоненты печати';en='Installing print components'");
	КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
				...
				Если НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
					...
					Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
						...
						Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
							Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Печать прервана.';ru='%1 Печать прервана.';en='% 1 Printing interrupted.'"), ОписаниеДохИно.ТекстСообщения);
								Сообщение.Сообщить();
							КонецЕсли;
							Возврат;
						КонецЕсли;
		...
		ИначеЕсли ВидПечати = "ПечатьОтчетШтрих" Тогда
			...
			Иначе
				...
				// Печать объекта, не входящего в БРО
				ПредупредитьОбОграничениях(Закладка, НСтр("ka='Печать со штрихкодом PDF417 не предусмотрена';ru='Печать со штрихкодом PDF417 не предусмотрена';en='Printing PDF417 barcode is not provided'"));
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Декларация по УСН" Тогда
		...
		Если ОписаниеУСН.ВыбраннаяФорма = "ФормаОтчета2014Кв1" И ОписаниеУСН.РекомендованнаяФорма Тогда
			ПоказатьПредупреждение(, НСтр("ka='Печать с двухмерным штрихкодом PDF417 формы, рекомендованной письмом ФНС от 20.05.2015 № ГД-4-3/8533@, не предусмотрена.';ru='Печать с двухмерным штрихкодом PDF417 формы, рекомендованной письмом ФНС от 20.05.2015 № ГД-4-3/8533@, не предусмотрена.';en='Printing from a two-dimensional bar code PDF417 form recommended by the Federal Tax Service letter from 5/20/2015 number DG-4-3 / 8533 @ is not provided.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Наименование = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Печать формы с двухмерным штрихкодом PDF417 прервана.';ru='%1 Печать формы с двухмерным штрихкодом PDF417 прервана.';en='% 1 Printing form with dimensional bar code PDF417 interrupted.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
	...
	Если НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Выгрузка прервана.';ru='%1 Выгрузка прервана.';en='% 1 Unloading interrupted.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
	...
	Если НаименованиеОтчета = "РСВ-1 ПФР" Тогда
		...
		Иначе
			...
			Если СокрЛП(ВариантОтчета) = "П"
				...
				ПоказатьПредупреждение(,НСтр("ka='Внимание! Выгружать в электронном виде первичную форму РСВ-1 необходимо"
...
"отчетности в ПФР. Выгрузка прервана.';ru='Внимание! Выгружать в электронном виде первичную форму РСВ-1 необходимо"
"в комплекте с документами квартальной отчетности персонифицированного"
"учета из специализированного рабочего места подготовки квартальной"
"отчетности в ПФР. Выгрузка прервана.'"));
	...
	Если НаименованиеОтчета = "Статистика: Форма П-1" Тогда
		...
		Если НЕ ПроверитьВозможностьВыгрузкиП1ИзФормыОтчетностьНаСервере(Ссылка) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для отчета ""Статистика: Форма П-1 за %1"" не выбрано Приложение №3."
"Необходимо сначала осуществить выгрузку из формы отчета! Выгрузка прервана.';ru='Для отчета ""Статистика: Форма П-1 за %1"" не выбрано Приложение №3."
"Необходимо сначала осуществить выгрузку из формы отчета! Выгрузка прервана.'"), ПредставлениеПериода);
	...
	Если НаименованиеОтчета = "Статистика: Форма П-1 Приложение 3" Тогда 
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';ru='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';en='Unloading report ""Statistics: a form of P-1 (Annex 3) for the 1%"" is made from a form of P-1 for the period.'"), ПредставлениеПериода);
		Сообщение.Сообщить();
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ВыгрузкаРегламентированногоОтчетаПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	НСтр("ka='%1';ru='%1'"), ТекстВопроса), РежимДиалогаВопрос.ДаНет);
...
Процедура ВыгрузкаРегламентированногоОтчетаЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьКлиент.ВыгрузитьОтчеты(
		...
		ПоказатьПредупреждение(,НСтр("ka='Выгрузка прервана!';ru='Выгрузка прервана!';en='Unloading is interrupted!'"));
	КонецЕсли;
...
Процедура НапечататьОбъектНеВходящийВБРО(Ссылка, СведенияПоОбъекту)
	...
	Если СтандартнаяПечатьОбъекта Тогда
		...
		Если МакетПечати <> Неопределено Тогда
			...
			Исключение
				ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(НСтр("ka='При печати возникла ошибка';ru='При печати возникла ошибка';en='An error occurred while printing'"));
			КонецПопытки;
...
Процедура ПечатьПротоколовПоСсылке(
	...
	Если НеОтправляетсяВКонтролирующийОрган Тогда
		ПредупредитьОбОграничениях(Закладка, НСтр("ka='Печать протоколов не предусмотрена';ru='Печать протоколов не предусмотрена';en='Printing protocols are not available'"));
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ПоследнийЦиклИлиОтправка) ИЛИ НЕ ПротоколЕсть Тогда
			...
			Иначе
				ТекстПредупреждения = НСтр("ru='У пользователя недостаточно прав для просмотра и печати протоколов.';en='User not allowed to view and print reports.'");
				ПоказатьПредупреждение(,ТекстПредупреждения);
			КонецЕсли;
			Возврат;
		КонецЕсли;
...
"%1';ka='Ошибка при проверке результата обновления информации механизма онлайн-сервисов регламентированной отчетности"
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ka='Регламентированная отчетность. Проверка результата обновления информации механизма онлайн-сервисов регламентированной отчетности';ru='Регламентированная отчетность. Проверка результата обновления информации механизма онлайн-сервисов регламентированной отчетности';en='Regulatory reporting. Check Result updating mechanism for online services, regulatory reporting'"), 
...
Процедура ПоказатьРезультатПроверкиОбновленияИнформацииМОС(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ka='Обновлены внешние формы отчетов - %1';ru='Обновлены внешние формы отчетов - %1';en='Updated external forms of reporting - 1%'"),
	...
	Для каждого СтрокаСообщения Из РезультатПроверки Цикл
		...
		ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.ПС
					   + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), СтрокаСообщения);
...
"%1';ka='Ошибка при подсчете количества объектов, не отображаемых в форме 1С-Отчетность "
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ka='Регламентированная отчетность. Проверка количества незаписанных объектов';ru='Регламентированная отчетность. Проверка количества незаписанных объектов';en='Regulatory reporting. Checking the number of unrecorded sites'"), 
			"Ошибка", 
			ТекстСообщенияОбОшибке,
			,
			Истина);
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если Представление = "РСВ-1 ПФР" И ТипЗнч(ПроверяемыйОбъект) <> Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
			...
			Если ОписаниеРСВ1.ВыбраннаяФорма = "ФормаОтчета2015Кв1" ИЛИ ОписаниеРСВ1.ВыбраннаяФорма = "ФормаОтчета2014Кв1" Тогда
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Для проверки в интернете необходимо открыть форму регламентированного отчета ""%1""';ru='Для проверки в интернете необходимо открыть форму регламентированного отчета ""%1""';en='To check on the internet you need to open the regulated Report ""%1""'"),
					Представление);
				ПоказатьПредупреждение(, ТекстПредупреждения);
				Возврат;
			КонецЕсли;
		...
		Если Представление = "Доходы, выплаченные иностранным организациям" Тогда
			...
			Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
				...
				Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
					Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 Проверка в интернете прервана.';ru='%1 Проверка в интернете прервана.';en='% 1 Checking the Internet interrupted.'"), ОписаниеДохИно.ТекстСообщения);
						Сообщение.Сообщить();
					КонецЕсли;
					Возврат;
				КонецЕсли;
...
Процедура ПредупредитьОбОграничениях(Закладка, Знач ТекстПредупреждения, Знач ВидОбъекта = Неопределено)
	...
	ТекстПредупреждения = НСтр("ru='Для %1 ""%2"" ';ka='Для %1 ""%2"" '") + ТекстПредупреждения;
	...
	Если ВидОбъекта = Неопределено Тогда
		Если ЭтоОтчеты Тогда
			ВидОбъекта = НСтр("ka='отчета';ru='отчета';en='report'");
		Иначе
			ВидОбъекта = НСтр("ka='уведомления';ru='уведомления';en='notice'"); 
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьНовоеПисьмо()
	...
	СписокВидовПисьма.Добавить(НСтр("ka='Письмо';ru='Письмо';en='Letter'"));
	СписокВидовПисьма.Добавить(НСтр("ka='Ответ на требование';ru='Ответ на требование';en='Response to demand'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеВыбораВидаПисьма", ЭтотОбъект);
	СписокВидовПисьма.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ka='Выберите вид письма';ru='Выберите вид письма';en='Select the type of letter'"));
...
Процедура ЗавершениеВыбораВидаПисьма(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Значение = НСтр("ka='Письмо';ru='Письмо';en='Letter'") Тогда
		 СоздатьНовоеИсходящееПисьмо();
	ИначеЕсли Результат.Значение = НСтр("ka='Ответ на требование';ru='Ответ на требование';en='Response to demand'") Тогда 
...
Процедура СоздатьНовуюСверку()
	...
	СписокВыбораВидаСверки.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ka='Выберите вид сверки';ru='Выберите вид сверки';en='Select the type of reconciliation'"));
