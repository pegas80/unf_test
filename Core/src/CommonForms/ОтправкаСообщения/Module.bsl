...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.УчетнаяЗапись) Тогда
		...
		Если ДоступныеУчетныеЗаписи.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='ელექტრონული ფოსტის ხელმისაწვდომი სააღრიცხვო ჩანაწერები აღმოჩენილი არ არის, მიმართეთ სისტემის ადმინისტრატორს.';ru='Не обнаружены доступные учетные записи электронной почты, обратитесь к администратору системы.'");
	...
	ИначеЕсли ТипЗнч(Параметры.УчетнаяЗапись) = Тип("СписокЗначений") Тогда
		...
		Если НаборУчетныхЗаписей.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='შეტყობინების გასაგზავნად არ არის მითითებული სააღრიცხვო ჩანაწერები, მიმართეთ სისტემის ადმინისტრატორს.';ru='Не указаны учетные записи для отправки сообщения, обратитесь к администратору системы.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			Возврат;
		КонецЕсли;
...
Процедура ОтправитьПисьмо()
	...
	Если ПоляЗаполненыКорректно() И ОтправитьПочтовоеСообщение() Тогда
		...
		Состояние(НСтр("ka='შეტყობინება წარმატებით გაიგზავნა';ru='Сообщение успешно отправлено'"));
...
Функция ПоляЗаполненыКорректно()
	...
	Если ПочтовыйАдресаПолучателей.Количество() = 0 Тогда
		...
		НСтр("ka='აუცილებელია წერილის მიმღების შევსება';ru='Необходимо заполнить получателя письма'"), , "ПочтовыйАдресПолучателя");
		Результат = Ложь;
	КонецЕсли;
	Для каждого Получателей Из ПочтовыйАдресаПолучателей Цикл
		Если ПустаяСтрока(Получателей.Адресат) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='აუცილებელია წერილის მიმღების შევსება';ru='Необходимо заполнить получателя письма'"),, "ПочтовыйАдресаПолучателей[" + Формат(ПочтовыйАдресаПолучателей.Индекс(Получателей), "ЧГ=0") + "].Адресат");
			Результат = Ложь;
		ИначеЕсли СтрНайти(Получателей.Адресат, "@") = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Неверный адрес электронной почты';ru='Неверный адрес электронной почты'"),, "ПочтовыйАдресаПолучателей[" + Формат(ПочтовыйАдресаПолучателей.Индекс(Получателей), "ЧГ=0") + "].Адресат");
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьПараметрыПисьма()
	...
	Для каждого Получатель Из ПочтовыйАдресаПолучателей Цикл
		Если Получатель.ВариантОтправки = НСтр("ka='დამალული ასლი:';ru='Скрытая копия:'") Тогда
			СкрытыеКопии.Добавить(Новый Структура("Адрес, Представление", Получатель.Адресат, Получатель.Представление));
		ИначеЕсли Получатель.ВариантОтправки = НСтр("ka='ასლი:';ru='Копия:'") Тогда
...
Функция ПолучитьПриведенныйПочтовыйАдресВФормате(Текст)
	...
	Если Адреса.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Можно указывать только один адрес для ответа.';ru='Можно указывать только один адрес для ответа.'"), , "АдресОтвета");
		СтандартнаяОбработка = Ложь;
		Возврат Текст;
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("ka='Сообщение еще не отправлено. Закрыть форму?';ru='Сообщение еще не отправлено. Закрыть форму?'");
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ka='შეტყობინების გაგზავნა';ru='Отправка сообщения'"));
КонецПроцедуры
