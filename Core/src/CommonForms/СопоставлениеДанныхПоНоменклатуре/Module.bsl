...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		ТекстСообщения = НСтр("ka='ელექტრონული დოკუმენტი არ არის არჩეული';ru='Не выбран электронный документ'");
...
"Продолжить?';ka='შეუნახავი ცვლილებები დაიკარგება. გავაგრძელოთ?'");
		...
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ka='ნომენკლატურის სიის შევსება';ru='Заполнение списка номенклатуры'"));
...
Процедура СохранитьДанныеКаталога(СсылкаНаЭД)
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если ИмяФайла = Неопределено Тогда
			ТекстОшибки = НСтр("ka='ელექტრონული დოკუმენტის ნახვა ვერ მოხერხდა. შეამოწმეთ სამუშაო კატალოგის გამართვა';ru='Не удалось просмотреть электронный документ. Проверьте настройку рабочего каталога'");
...
" Проверьте настройку рабочего каталога';ka='ელექტრონული დოკუმენტის დამატებითი მონაცემების მიღება ვერ მოხერხდა. შეამოწმეთ სამუშაო კატალოგის გამართვა'");
		...
		Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				КонецЕсли;
				Операция = НСтр("ka='შეფუთვიდან მოხსნა  ЭД';ru='Распаковка ЭД'");
