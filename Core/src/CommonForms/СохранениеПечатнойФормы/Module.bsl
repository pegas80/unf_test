...
Процедура Сохранить(Команда)
	...
	Если ВариантСохранения = "СохранитьВПапку" И ПустаяСтрока(ВыбраннаяПапка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აუცილებელია საქაღალდის მითითება.';ru='Необходимо указать папку.';en='You must specify the folder.'"),,"ВыбраннаяПапка");
		Возврат;
	КонецЕсли;
	#КонецЕсли
	...
	Если ФорматыСохранения.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='აუცილებელია მინიმუმ ერთ-ერთი შემოთავაზებული ფორმატის მითითება.';ru='Необходимо указать как минимум один из предложенных форматов.';en='You must specify at least one of the proposed formats.'"));
		Возврат;
	КонецЕсли;
