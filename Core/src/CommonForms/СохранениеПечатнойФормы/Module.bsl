...
Процедура Сохранить(Команда)
	...
	Если ВариантСохранения = "СохранитьВПапку" И ПустаяСтрока(ВыбраннаяПапка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აუცილებელია საქაღალდის მითითება.';ru='Необходимо указать папку.'"),,"ВыбраннаяПапка");
		Возврат;
	КонецЕсли;
	#КонецЕсли
	...
	Если ФорматыСохранения.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='აუცილებელია მინიმუმ ერთ-ერთი შემოთავაზებული ფორმატის მითითება.';ru='Необходимо указать как минимум один из предложенных форматов.'"));
		Возврат;
	КонецЕсли;
