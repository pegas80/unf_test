...
""
"%1';ka='პაროლის შეცვლის ფორმის გახსნა ვერ მოხერხდა შემდეგი მიზეზით:"
""
"%1'"),
	...
	Если ПриВходеВПрограмму Тогда
		...
		Иначе
			...
			Элементы.ПояснениеПриВходе.Заголовок =
				НСтр("ka='პროგრამაში შესასვლელად საჭიროა პაროლის დაყენება.';ru='Для входа в программу нужно установить пароль.';en='To access the program you need to set a password.'")
		КонецЕсли;
	Иначе
		...
		Элементы.ФормаЗакрытьФорму.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	 Или Не ДополнительныеПараметры.ПарольУстановлен Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='პაროლის დაყენება';ru='Установка пароля';en='Setting a password'");
...
Процедура УстановитьПароль(Команда)
	...
	Если Не ПоказыватьНовыйПароль И НовыйПароль <> Подтверждение Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='პაროლის დადასტურება არაკორექტულად არის მითითებული.';ru='Подтверждение пароля указано некорректно.';en='Confirm password specified correctly.'"));
		Возврат;
	КонецЕсли;
