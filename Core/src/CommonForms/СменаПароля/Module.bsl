...
"%1'"),
	...
	Если ПриВходеВПрограмму Тогда
		...
		Иначе
			...
			Элементы.ПояснениеПриВходе.Заголовок =
				НСтр("ka='პროგრამაში შესასვლელად საჭიროა პაროლის დაყენება.';ru='Для входа в программу нужно установить пароль.'")
		КонецЕсли;
	Иначе
		...
		Элементы.ФормаЗакрытьФорму.Заголовок = НСтр("ka='გაუქმება';ru='Отмена'");
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	 Или Не ДополнительныеПараметры.ПарольУстановлен Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='პაროლის დაყენება';ru='Установка пароля'");
...
Процедура УстановитьПароль(Команда)
	...
	Если Не ПоказыватьНовыйПароль И НовыйПароль <> Подтверждение Тогда
		...
		ПоказатьПредупреждение(, НСтр("ka='პაროლის დადასტურება არაკორექტულად არის მითითებული.';ru='Подтверждение пароля указано некорректно.'"));
		Возврат;
	КонецЕсли;
