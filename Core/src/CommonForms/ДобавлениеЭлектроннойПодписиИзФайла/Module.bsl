...
Процедура ОК(Команда)
	...
	Если Подписи.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ხელმოწერის არცერთი ფაილი არ არის არჩეული';ru='Не выбрано ни одного файла подписи'"));
		Возврат;
	КонецЕсли;
...
Процедура ОКЗавершение(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='ფაილიდან ელექტრონული ხელმოწერის დამატება ვერ მოხერხდა';ru='Не удалось добавить электронную подпись из файла'"), "", Ошибка, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ელექტრონული ხელმოწერის ფაილი';ru='Выберите файл электронной подписи'");
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელმოწერის ფაილები (*.%1)|*.%1|ყველა ფაილი(*.*)|*.*';ru='Файлы подписи (*.%1)|*.%1|Все файлы(*.*)|*.*'"),
...
Процедура ВыбратьФайлПослеПолученияСертификатовИзПодписи(Сертификаты, Контекст) Экспорт
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
			НСтр("ka='ხელმოწერის ფაილში არცერთი სერთიფიკატი არ არის.';ru='В файле подписи нет ни одного сертификата.'"));
...
"%1'"),
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ka='ხელმოწერის ფაილში არცერთი სერთიფიკატი არ არის.';ru='В файле подписи нет ни одного сертификата.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='ფაილიდან ხელმოწერის მიღება ვერ მოხერხდა';ru='Не удалось получить подпись из файла'"), "", ОшибкаНаКлиенте, ОшибкаНаСервере);
