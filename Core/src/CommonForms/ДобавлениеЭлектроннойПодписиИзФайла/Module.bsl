...
Процедура ОК(Команда)
	...
	Если Подписи.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ka='ხელმოწერის არცერთი ფაილი არ არის არჩეული';ru='Не выбрано ни одного файла подписи';en='You have no signature file'"));
		Возврат;
	КонецЕсли;
...
Процедура ОКЗавершение(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ka='ფაილიდან ელექტრონული ხელმოწერის დამატება ვერ მოხერხდა';ru='Не удалось добавить электронную подпись из файла';en='Unable to add an electronic signature from a file'"), "", Ошибка, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ka='აირჩიეთ ელექტრონული ხელმოწერის ფაილი';ru='Выберите файл электронной подписи';en='Select the electronic signature file'");
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელმოწერის ფაილები (*.%1)|*.%1|ყველა ფაილი(*.*)|*.*';ru='Файлы подписи (*.%1)|*.%1|Все файлы(*.*)|*.*';en='Signature files (*% 1.) |. *% 1 | All files (* *.) | * *.'"),
...
"%1';ka='ხელმოწერის ფაილიდან სერთიფიკატების მიღებისას შეცდომა მოხდა:"
"%1'"),
...
Процедура ВыбратьФайлПослеПолученияСертификатовИзПодписи(Сертификаты, Контекст) Экспорт
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
			НСтр("ka='ხელმოწერის ფაილში არცერთი სერთიფიკატი არ არის.';ru='В файле подписи нет ни одного сертификата.';en='In the signature file, there is no certificate.'"));
...
"%1'"),
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ka='ხელმოწერის ფაილში არცერთი სერთიფიკატი არ არის.';ru='В файле подписи нет ни одного сертификата.';en='In the signature file, there is no certificate.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ka='ფაილიდან ხელმოწერის მიღება ვერ მოხერხდა';ru='Не удалось получить подпись из файла';en='Could not get the signature from a file'"), "", ОшибкаНаКлиенте, ОшибкаНаСервере);
