...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Элементы.ФормаЗаписатьИЗакрыть.Заголовок = НСтр("ka='ჩაწერა';ru='Записать';en='record'");
	...
	Если КоличествоПользователей = 0 Тогда
		ВызватьИсключение НСтр("ka='არცერთი მომხმარებელი არ არის არჩეული.';ru='Не выбрано ни одного пользователя.';en='You have no users.'");
	КонецЕсли;
	...
	Для Каждого ПользовательИзМассива Из МассивПользователей Цикл
		...
		Если ТипПользователяИзМассива <> Тип("СправочникСсылка.Пользователи")
		   И ТипПользователяИзМассива <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда
			...
			ВызватьИсключение НСтр("ka='ბრძანება არ შეიძლება შესრულდეს მითითებული ობიექტისთვის.';ru='Команда не может быть выполнена для указанного объекта.';en='Command can not be executed for the indicated object.'");
		КонецЕсли;
		
		Если ТипПользователей <> ТипПользователяИзМассива Тогда
			ВызватьИсключение НСтр("ka='ბრძანება არ შეიძლება, რომ ეგრევე შესრულდეს მომხმარებელთა ორი, სხვადასხვა სახეობისთვის.';ru='Команда не может быть выполнена сразу для двух разных видов пользователей.';en='Command can not be executed at once for two different types of members.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если КоличествоПользователей > 1
	   И Параметры.Пользователь = Неопределено Тогда
		...
		Заголовок = НСтр("ka='მომხმარებელთა ჯგუფები';ru='Группы пользователей';en='Group of users'");
	...
	Если УведомлениеПользователя.ЕстьОшибки = Ложь Тогда
		Если УведомлениеПользователя.Сообщение <> Неопределено Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ka='მომხმარებლების გადაადგილება';ru='Перемещение пользователей';en='Moving users'"), , УведомлениеПользователя.Сообщение, БиблиотекаКартинок.Информация32);
		КонецЕсли;
	Иначе
...
"т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".';ka='არჩეულ ჯგუფებში მომხმარებელ ""%1""-ის შეყვანა ვერ მოხერხდა, ვინაიდან ჯგუფებს განსხვავებული ტიპი გააჩნიათ ან მათ დაყენებული აქვთ ნიშან-თვისება ""მოცემული ტიპის ყველა მომხმარებელი"".'"),
				Предмет);
		Иначе
			Предмет = Формат(КоличествоНеПеремещенныхПользователей, "ЧДЦ=0") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНеПеремещенныхПользователей,
					"Л = ru_RU", НСтр("ka='მომხმარებელს,მომხმარებლებს,მომხმარებლებს,,,,,,0';ru='пользователю,пользователям,пользователям,,,,,,0';en='user, users, users ,,,,,, 0'"));
			СообщениеПользователю =
				НСтр("ka='არჩეულ ჯგუფებში ყველა მომხმარებლის შეყვანა ვერ მოხერხდა, ვინაიდან ჯგუფებს განსხვავებული ტიპი გააჩნიათ ან მათ დაყენებული აქვთ ნიშან-თვისება ""მოცემული ტიპის ყველა მომხმარებელი"".';ru='Не всех пользователей удалось включить в выбранные группы,"
"т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".'");
			...
			УведомлениеПользователя.ПолныйТекстСообщения =
				НСтр("ka='Следующие пользователи не были включены в группы:';ru='Следующие пользователи не были включены в группы:';en='The following users have been included in the group:'")
	...
	ИначеЕсли КоличествоПользователей = 1 Тогда
		...
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='მომხმარებელ ""%1""-სთვის შეცვლილია ჯგუფების შემადგენლობა';ru='Изменен состав групп у пользователя ""%1""';en='Changed the composition of the groups the user ""%1""'"),
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"Л = ru_RU", НСтр("ka='пользователя,пользователей,пользователей,,,,,,0';ru='пользователя,пользователей,пользователей,,,,,,0';en='user, users, user ,,,,,, 0'"));
		
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='%1-ს შეეცვალა ჯგუფების შემადგენლობა';ru='Изменен состав групп у %1';en='Changed the groups have 1%'"), СтрокаОбъект);
	КонецЕсли;
...
Процедура ЗаписатьИЗакрытьОбработкаВопроса(Ответ, Отчет) Экспорт
	...
	Если Ответ = "Ок" Тогда
		Возврат;
	Иначе
		Отчет.Показать(НСтр("ka='Пользователи, не включенные в группы';ru='Пользователи, не включенные в группы';en='Users who are not included in the group'"));
		Возврат;
	КонецЕсли;
