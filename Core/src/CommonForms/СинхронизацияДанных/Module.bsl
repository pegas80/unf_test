...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПлановОбмена.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ka='მონაცემთა სინქრონიზაციის გამართვის შესაძლებლობა არ არის გათვალისწინებული.';ru='Возможность настройки синхронизации данных не предусмотрена.'");
...
Процедура СписокСостоянияУзловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='შევასრულოთ მონაცემთა სინქრონიზაცია?';ru='Выполнить синхронизацию данных?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОбновитьКоличествоСинхронизаций(КоличествоНастроенныхОбменов)
	...
	Если КоличествоСинхронизаций = 0 Тогда
		...
		Заголовок = НСтр("ka='მონაცემთა სინქრონიზაცია';ru='Синхронизация данных'");
	...
	Иначе
		...
		Заголовок = НСтр("ka='მონაცემთა გამართული სინქრონიზაციების სია';ru='Список настроенных синхронизаций данных'");
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	Если НастроеннаяСинхронизация.УзелИнформационнойБазы = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ka='მონაცემთა სინქრონიზაციის მონიტორის მუშაობა განუცალკევებელ სეანსში მხარდაჭერილი არ არის';ru='Работа монитора синхронизации данных в неразделенном сеансе не поддерживается'");
...
"Последняя попытка: %ДатаПопытки%'");
	...
	Иначе
		...
		РасписаниеСинхронизацииДанных = НСтр("ka='სინქრონიზაციის განრიგი არ არის გამართული';ru='Расписание синхронизации не настроено'");;
			...
			Если СписокВозможныхНастроекОбмена.Количество() > 0 Тогда
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					...
					Для Каждого ПредопределеннаяНастройка Из СписокВозможныхНастроекОбмена Цикл
						...
						СоздатьКомандуИЭлементФормы(НаименованиеКоманды, ЗаголовокКоманды + " " + НСтр("ka=' (სერვისში)';ru='(в сервисе)'"), СуммирующаяГруппа);
			...
			Иначе
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					СоздатьКомандуИЭлементФормы(НаименованиеКоманды + "КорреспондентВМоделиСервиса", 
					                            ЗаголовокКоманды + " " + НСтр("ka=' (სერვისში)';ru='(в сервисе)'"),
					                            РодительскаяГруппа);
				КонецЕсли;
...
Процедура ПроверитьСовместимостьПравилКонвертации(Знач ИмяПланаОбмена, ОбработкаПродолжения)
	...
	Иначе
		...
		Кнопки.Добавить("ПерейтиКЗагрузкеПравил", НСтр("ka='წესების ჩატვირთვა';ru='Загрузить правила'"));
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ka='მონაცემთა სინქრონიზაცია ვერ შესრულდება';ru='Синхронизация данных не может быть выполнена'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ka='მონაცემთა სინქრონიზაცია შეიძლება არაკორექტულად შესრულდეს';ru='Синхронизация данных может быть выполнена некорректно'");
		КонецЕсли;
