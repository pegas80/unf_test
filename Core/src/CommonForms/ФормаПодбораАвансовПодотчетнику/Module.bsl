...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ)
	...
	Для каждого СтрокаПредоплата Из ОтобранныеАвансы Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаПредоплата.Документ) Тогда
			...
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Документ"" в строке ';ka='სვეტი ""დოკუმენტი"" სტრიქონში არ არის შევსებული'")
				+ Строка(НомерСтроки)
				+ НСтр("ka='სია ""გადარჩეული ავანსები"".';ru=' списка ""Отобранные авансы"".'");
		...
		Если НЕ ЗначениеЗаполнено(СтрокаПредоплата.Сумма) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Сумма"" в строке ';ka='სვეტი ""თანხა"" სტრიქონში არ არის შევსებული'")
				+ Строка(НомерСтроки)
				+ НСтр("ka='სია ""გადარჩეული ავანსები"".';ru=' списка ""Отобранные авансы"".'");
			Сообщение.Поле = "Документ";
			Сообщение.Сообщить();
