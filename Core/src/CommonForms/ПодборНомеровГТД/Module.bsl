...
Процедура ТаблицаЗапасовНомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ Элементы.ТаблицаЗапасов.ТекущиеДанные.ЭтоЗапас Тогда
		...
		ТекстСообщения = НСтр("ka='საბაჟო დეკლარაციის ნომრის მითითება შესაძლებელია მხოლოდ ტიპ <მარაგი>-ს მქონე ნომენკლატურისთვის.';ru='Указать номер ГТД возможно только для номенклатуры с типом <Запас>.';en='Specify the number of GTD is possible only for the range of the type <Reserve>.'");
...
Процедура ТаблицаЗапасовНомерГТДПриИзменении(Элемент)
	...
	Если ДатаРегистрацииГТД > ДатаДокумента Тогда
		...
		ТекстВопроса = Нстр("ka='არჩეულია სატვირთო საბაჟო დეკლარაცია, რომლის რეგისტრაციის თარიღი დოკუმენტის თარიღზე უფროსია. გავაგრძელოთ?';ru='Выбрана ГТД, у которой дата регистрации старше даты документа. "
"Продолжить?'");
...
Процедура ТаблицаЗапасовСтранаПроисхожденияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ Элементы.ТаблицаЗапасов.ТекущиеДанные.ЭтоЗапас Тогда
		...
		ТекстСообщения = НСтр("ka='Указать страну происхождения возможно только для номенклатуры с типом <Запас>.';ru='Указать страну происхождения возможно только для номенклатуры с типом <Запас>.';en='Indicate the country of origin is possible only for the range of the type <Reserve>.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура УстановитьОтметкуДляСтрокСАналогичнойСтраной(Команда)
	...
	Если ДанныеТекущейCтроки = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='გამოჰყავით ცხრილის სტრიქონი და განმეორებით დააჭირეთ...';ru='Выделите строку табличной части и повторите нажатие...';en='Select the row of the table and repeat the press ...'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
