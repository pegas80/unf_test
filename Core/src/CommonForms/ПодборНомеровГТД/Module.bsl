...
Процедура ТаблицаЗапасовНомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ Элементы.ТаблицаЗапасов.ТекущиеДанные.ЭтоЗапас Тогда
		...
		ТекстСообщения = НСтр("ka='საბაჟო დეკლარაციის ნომრის მითითება შესაძლებელია მხოლოდ ტიპ <მარაგი>-ს მქონე ნომენკლატურისთვის.';ru='Указать номер ГТД возможно только для номенклатуры с типом <Запас>.'");
...
Процедура ТаблицаЗапасовСтранаПроисхожденияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ Элементы.ТаблицаЗапасов.ТекущиеДанные.ЭтоЗапас Тогда
		...
		ТекстСообщения = НСтр("ka='Указать страну происхождения возможно только для номенклатуры с типом <Запас>.';ru='Указать страну происхождения возможно только для номенклатуры с типом <Запас>.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура УстановитьОтметкуДляСтрокСАналогичнойСтраной(Команда)
	...
	Если ДанныеТекущейCтроки = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ka='გამოჰყავით ცხრილის სტრიქონი და განმეორებით დააჭირეთ...';ru='Выделите строку табличной части и повторите нажатие...'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
