...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რა არის ახალი კონფიგურაცია %1-ში';ru='Что нового в конфигурации %1'"), Метаданные.Синоним);
	...
	Если ДокументОписаниеОбновлений.ВысотаТаблицы = 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='კონფიგურაცია წარმატებით განახლდა ვერსიით %1';ru='Конфигурация успешно обновлена на версию %1'"), Метаданные.Версия);
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ЗаголовокСообщения = НСтр("ka='აუცილებელია მონაცემთა დამუშავების დამატებითი პროცედურების შესრულება';ru='Необходимо выполнить дополнительные процедуры обработки данных'");
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Элементы.ОтложенноеОбновлениеДанных.Заголовок =
			НСтр("ka='მონაცემთა დამუშავების დამატებითი პროცედურები არ არის შესრულებული';ru='Не выполнены дополнительные процедуры обработки данных'");
	КонецЕсли;
...
Процедура ОтключеныРегламентныеЗаданияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ka='გადაიტვირთოს პროგრამა?';ru='Перезапустить программу?'");
