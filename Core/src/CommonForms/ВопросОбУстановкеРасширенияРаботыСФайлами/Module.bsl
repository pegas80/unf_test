...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Параметры.ТекстПредложения) Тогда
		Элементы.ДекорацияПояснение.Заголовок = Параметры.ТекстПредложения
			+ Символы.ПС
			+ НСтр("ka='დაყენდეს?';ru='Установить?';en='Set?'");
		
	ИначеЕсли Не Параметры.ВозможноПродолжениеБезУстановки Тогда
		Элементы.ДекорацияПояснение.Заголовок =
			НСтр("ka='მოქმედების შესასრულებლად ვებ-კლიენტ 1С:საწარმოსთვის საჭიროა გაფართოების დაყენება."
"დავაყენოთ?';ru='Для выполнения действия требуется установить расширение для веб-клиента 1С:Предприятие."
"Установить?'");
	КонецЕсли;
	
	Если Не Параметры.ВозможноПродолжениеБезУстановки Тогда
		Элементы.ПродолжитьБезУстановки.Заголовок = НСтр("ka='გაუქმება';ru='Отмена';en='cancellation'");
