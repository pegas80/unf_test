...
"т.к. он не найден в справочнике.';ka='სერთიფიკატ ""%1""-ის გახსნა ვერ მოხერხდა, ვინაიდან იგი ცნობარში ვერ მოიძებნა.'"), Параметры.Ссылка);
		КонецЕсли;
	Иначе // Отпечаток
...
"по отпечатку ""%1"".';ka='სერთიფიკატის გახსნა ვერ მოხერხდა, ვინაიდან ანაბეჭდ ""%1""-ის მიხედვით იგი ვერ მოიძებნა.'"), Параметры.Отпечаток);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗавершение(Результат, Неопределен) Экспорт
	...
	Если Результат = Истина Тогда
		ПоказатьПредупреждение(, НСтр("ka='სერტიფიკატი ძალაშა.';ru='Сертификат действителен.';en='The certificate is valid.'"));
		
	ИначеЕсли Результат <> Неопределено Тогда
		ПараметрыПредупреждения = Новый Структура;
		
		ПараметрыПредупреждения.Вставить("ТекстПредупреждения", Результат);
		ПараметрыПредупреждения.Вставить("ЗаголовокПредупреждения", НСтр("ka='Сертификат недействителен по причине:';ru='Сертификат недействителен по причине:';en='Certificate is invalid by reason of:'"));
...
Процедура ЗаполнитьСвойстваСубъекта(Сертификат)
	...
	ПредставленияСвойств.Добавить("ОбщееИмя",         НСтр("ka='საერთო სახელი';ru='Общее имя';en='Common name'"));
	ПредставленияСвойств.Добавить("Страна",           НСтр("ka='ქვეყანა';ru='Страна';en='A country'"));
	ПредставленияСвойств.Добавить("Регион",           НСтр("ka='რეგიონი';ru='Регион';en='Region'"));
	ПредставленияСвойств.Добавить("НаселенныйПункт",  НСтр("ka='დასახლებული პუნქტი';ru='Населенный пункт';en='Locality'"));
	ПредставленияСвойств.Добавить("Улица",            НСтр("ka='ქუჩა';ru='Улица';en='Street'"));
	ПредставленияСвойств.Добавить("Организация",      НСтр("ka='ორგანიზაცია';ru='Организация';en='Organization'"));
	ПредставленияСвойств.Добавить("Подразделение",    НСтр("ka='ქვეგანყოფილება';ru='Подразделение';en='Subdivision'"));
	ПредставленияСвойств.Добавить("Должность",        НСтр("ka='თანამდებობა';ru='Должность';en='Position'"));
	ПредставленияСвойств.Добавить("ЭлектроннаяПочта", НСтр("ka='ელ.ფოსტა';ru='Электронная почта';en='Email'"));
	ПредставленияСвойств.Добавить("ОГРН",             НСтр("ka='ძსსნ';ru='ОГРН';en='BIN'"));
	ПредставленияСвойств.Добавить("ОГРНИП",           НСтр("ka='ОГРНИП';ru='ОГРНИП';en='OGRN'"));
	ПредставленияСвойств.Добавить("СНИЛС",            НСтр("ka='СНИЛС';ru='СНИЛС';en='SNILS'"));
	ПредставленияСвойств.Добавить("ИНН",              НСтр("ka='ИНН';ru='ИНН';en='INN'"));
	ПредставленияСвойств.Добавить("Фамилия",          НСтр("ka='გვარი';ru='Фамилия';en='Surname'"));
	ПредставленияСвойств.Добавить("Имя",              НСтр("ka='სახელი';ru='Имя';en='Name'"));
	ПредставленияСвойств.Добавить("Отчество",         НСтр("ka='მამის სახელი';ru='Отчество';en='middle name'"));
...
Процедура ЗаполнитьВнутренниеПоляСертификата()
	...
	Если ГруппаВнутреннихПолей = "Общие" Тогда
		...
		ДобавитьСвойство(Сертификат, "Версия",                    НСтр("ka='ვერსია';ru='Версия';en='Version'"));
		ДобавитьСвойство(Сертификат, "ДатаНачала",                НСтр("ka='დაწყების თარიღი';ru='Дата начала';en='the date of the beginning'"));
		ДобавитьСвойство(Сертификат, "ДатаОкончания",             НСтр("ka='დასრულების თარიღი';ru='Дата окончания';en='expiration date'"));
		ДобавитьСвойство(Сертификат, "ИспользоватьДляПодписи",    НСтр("ka='ხელმოწერისთვის გამოყენება';ru='Использовать для подписи';en='Use to sign'"));
		ДобавитьСвойство(Сертификат, "ИспользоватьДляШифрования", НСтр("ka='დასაშიფრად გამოყენება';ru='Использовать для шифрования';en='Used for encryption'"));
		ДобавитьСвойство(Сертификат, "ОткрытыйКлюч",              НСтр("ka='გახსნილი გასაღები';ru='Открытый ключ';en='The public key'"), Истина);
		ДобавитьСвойство(Сертификат, "Отпечаток",                 НСтр("ka='ანაბეჭდი';ru='Отпечаток';en='mark'"), Истина);
		ДобавитьСвойство(Сертификат, "СерийныйНомер",             НСтр("ka='სერიული ნომერი';ru='Серийный номер';en='Serial Numbers'"), Истина);
