...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокПорт.Добавить(0, НСтр("ka='<Клавиатура>';ru='<Клавиатура>'"));
	...
	СписокСтопБит.Добавить(0, НСтр("ka='1 სტოპ-ბიტი';ru='1 стоп-бит'"));
	СписокСтопБит.Добавить(1, НСтр("ka='1.5 სტოპ-ბიტი';ru='1.5 стоп-бита'"));
	СписокСтопБит.Добавить(2, НСтр("ka='2 სტოპ-ბიტი';ru='2 стоп-бита'"));
...
Процедура ЗаписатьИЗакрытьВыполнить()
	...
	ИначеЕсли ДорожкаСПустымСуффиксом Тогда
		ТекстСообщения = НСтр("ka='ყოველი გამოყენებადი ბილიკისთვის უნდა მიეთითოს 0-ისგან განსხვავებული  სუფიქსი';ru='Для каждой используемой дорожки должен быть указан суффикс, отличный от 0'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ОбновитьИнформациюОДрайвере()
	...
	Если МенеджерОборудованияКлиент.ВыполнитьДополнительнуюКоманду("ПолучитьВерсиюДрайвера",
		...
		Если ВерсияИзБПОСтр > ВерсияСтр Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='კომპიუტერზე დაყენებული დრაივერის ვერსია მოძველდა! აუცილებელია განახლება შემდეგ ვერსიამდე:';ru='Установленная на компьютере версия драйвера устарела! Необходимо обновление до версии:'") + Символы.НПП + ВерсияИзБПО);
		КонецЕсли;
	Иначе
	...
	Элементы.ТестУстройства.Доступность = (Драйвер = НСтр("ka='დაყენებულია';ru='Установлен'"));
