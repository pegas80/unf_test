...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокПорт.Добавить(0, НСтр("ka='<Клавиатура>';ru='<Клавиатура>';en='<Keyboard>'"));
	...
	СписокСтопБит.Добавить(1, НСтр("ka='1.5 სტოპ-ბიტი';ru='1.5 стоп-бита';en='1.5 stop bits'"));
...
Процедура ЗаписатьИЗакрытьВыполнить()
	...
	ИначеЕсли ДорожкаСПустымСуффиксом Тогда
		ТекстСообщения = НСтр("ka='ყოველი გამოყენებადი ბილიკისთვის უნდა მიეთითოს 0-ისგან განსხვავებული  სუფიქსი';ru='Для каждой используемой дорожки должен быть указан суффикс, отличный от 0';en='Each track is used must be specified suffix other than 0'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ОбновитьИнформациюОДрайвере()
	...
	Если МенеджерОборудованияКлиент.ВыполнитьДополнительнуюКоманду("ПолучитьВерсиюДрайвера",
		...
		Если ВерсияИзБПОСтр > ВерсияСтр Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='კომპიუტერზე დაყენებული დრაივერის ვერსია მოძველდა! აუცილებელია განახლება შემდეგ ვერსიამდე:';ru='Установленная на компьютере версия драйвера устарела! Необходимо обновление до версии:';en='Installed on your computer is outdated version of the driver! You must upgrade to version:'") + Символы.НПП + ВерсияИзБПО);
		КонецЕсли;
	Иначе
	...
	Элементы.ТестУстройства.Доступность = (Драйвер = НСтр("ka='დაყენებულია';ru='Установлен';en='Set'"));
