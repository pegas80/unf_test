		...
		ИначеЕсли ПроверкаТелефонСтарый.ВыполняетсяПроверка Тогда
			...
			Если Результат.Выполнено Тогда
				...
				Если ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено И ПроверкаЭлектроннаяПочтаДляПаролей.ПодтверждениеВыполнено Тогда
					ТекстПредупреждения = НСтр("ka='Ваш номер телефона и адрес электронной почты успешно изменены';ru='Ваш номер телефона и адрес электронной почты успешно изменены';en='Your phone number and email address was successfully changed'");
				ИначеЕсли ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
					ТекстПредупреждения = НСтр("ka='Ваш номер телефона успешно изменен';ru='Ваш номер телефона успешно изменен';en='Your phone number changed successfully'");
				Иначе
					ТекстПредупреждения = НСтр("ka='Ваш адрес электронной почты успешно изменен';ru='Ваш адрес электронной почты успешно изменен';en='Your email address has been changed successfully'");
				КонецЕсли;				
