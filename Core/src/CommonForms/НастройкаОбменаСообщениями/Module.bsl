...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='გაყოფილ რეჟიმში ქვესისტემის გამართვას მხარდაჭერა არ გააჩნია.';ru='Настройка подсистемы в разделенном режиме не поддерживается.';en='Setting subsystem in a split mode is not supported.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
