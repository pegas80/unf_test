	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			Если ПериодНачало.АвтоОтметкаНезаполненного
				...
				ТекстОшибки = НСтр("ka='პერიოდი არ არის მითითებული';ru='Не указан период'");
				ПутьКДанным = ПериодНачало.ПутьКДанным;
			ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
				ТекстОшибки = НСтр("ka='პერიოდის დასასრული დასაწყისზე მეტი უნდა იყოს';ru='Конец периода должен быть больше начала'");
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		ПоказатьВводДаты(Оповещение, Период.ДатаОкончания, НСтр("ka='თარიღისთვის';ru='На дату'"), ЧастиДаты.Дата);
	КонецЕсли; 
...
Процедура СтрокаОтбораАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если СписокВыбора.Количество()=0 И СтрДлина(Текст)>2 Тогда
		СписокВыбора.Добавить("", Новый ФорматированнаяСтрока(НСтр("ka='თანხვედრები ვერ მოიძებნა';ru='Совпадений не найдено'"),, WebЦвета.НейтральноСерый));
	КонецЕсли; 
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Данные.ЭтоСсылка Тогда
	    ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение, НСтр("ka='გახსნა';ru='Открыть'"));
	КонецЕсли; 
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И НЕ ДоступноеПоле.Ресурс Тогда
		ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Отфильтровать, НСтр("ka='გაფილტრვა""';ru='Отфильтровать ""'")+Данные.Поле+" = "+Данные.Значение+"""");
	КонецЕсли;
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.ТипЗначения.Типы().Количество()>0 Тогда
		ДополнительныеДействия.Добавить(СформироватьПодменюОформить(Данные.Поле, Данные.Значение), НСтр("ka='გაფორმება';ru='Оформить'"));
	    ...
	    ДополнительныеДействия.Добавить(ПодменюСортировка, НСтр("ka='დახარისხება';ru='Сортировать'"));
	КонецЕсли;
...
Функция СформироватьПодменюОформить(Поле, Значение)
	...
	Для каждого Тип Из ТипЗначения.Типы() Цикл
		...
		НСтр("ka='სწრაფი გამართვების სიის რედაქტირება';ru='Редактировать список быстрых настроек'"),, 
		БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
		СписокКоманд.Добавить(
		"РучнаяНастройкаОформления", 
		НСтр("ka='გაფორმების გამართვა ხელის რეჟიმში';ru='Ручная настройка оформления'"),, 
...
Процедура ВидДиаграммыНачалоВыбораЗадержка()
	...
	Меню.Добавить(Неопределено, НСтр("ka='არ გამოიტანოს';ru='Не выводить'"),, БиблиотекаКартинок.Пустая);
	Меню.Добавить(ТипДиаграммы.График, НСтр("ka='გრაფიკები';ru='Графики'"),, КартинкаДиаграммы(ТипДиаграммы.График));
	Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ka='ჩვეულებრივი ჰისტოგრამები';ru='Обычные гистограммы'"),, КартинкаДиаграммы(ТипДиаграммы.Гистограмма));
	Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ka='ჰორიზონტი. ჰისტოგრამა';ru='Горизонт. гистограммы'"),, КартинкаДиаграммы(ТипДиаграммы.ГистограммаГоризонтальная));
	Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ka='წრიული დიაგრამები';ru='Круговые диаграммы'"),, КартинкаДиаграммы(ТипДиаграммы.Круговая));
	Меню.Добавить(ТипДиаграммы.Изометрическая, НСтр("ka='იზომეტრიული დიაგრამები';ru='Изометрические диаграммы'"),, КартинкаДиаграммы(ТипДиаграммы.Изометрическая));
	Меню.Добавить("<Прочие>", НСтр("ka='სხვები';ru='Прочие'"),, КартинкаДиаграммы(ТипДиаграммы.Поверхность));
...
Процедура ВидДиаграммыВыборГруппы(Значение, ДополнительныеДанные) Экспорт
	...
	Если Значение.Значение=ТипДиаграммы.График Тогда
		Меню.Добавить(ТипДиаграммы.График, НСтр("ka='ჩვეულებრივი';ru='Обычный'"));
		Меню.Добавить(ТипДиаграммы.ГрафикПоШагам, НСтр("ka='ნაბიჯების მიხედვით';ru='По шагам'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСНакоплением, НСтр("ka='დაგროვებით';ru='С накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластями, НСтр("ka='ოლქებით';ru='С областями'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиИНакоплением, НСтр("ka='არეალებით და დაგროვებით';ru='С областями и накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиНормированный, НСтр("ka='ნორმირებული';ru='Нормированный'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Гистограмма Тогда
		Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ka='ჩვეულებრივი';ru='Обычная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплением, НСтр("ka='დაგროვებით';ru='С накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированная, НСтр("ka='ნორმირებული';ru='Нормированная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаОбъемная, НСтр("ka='მოცულობითი';ru='Объемная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемОбъемная, НСтр("ka='მოცულობითი დაგროვებით';ru='Объемная с накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяОбъемная, НСтр("ka='მოცულობითი ნორმირებული';ru='Объемная нормированная'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.ГистограммаГоризонтальная Тогда
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ka='ჩვეულებრივი';ru='Обычная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальная, НСтр("ka='დაგროვებით';ru='С накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальная, НСтр("ka='ნორმირებული';ru='Нормированная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальнаяОбъемная, НСтр("ka='მოცულობითი';ru='Объемная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальнаяОбъемная, НСтр("ka='მოცულობითი დაგროვებით';ru='Объемная с накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальнаяОбъемная, НСтр("ka='მოცულობითი ნორმირებული';ru='Объемная нормированная'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Круговая Тогда
		Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ka='ჩვეულებრივი';ru='Обычная'"));
		Меню.Добавить(ТипДиаграммы.КруговаяОбъемная, НСтр("ka='მოცულობითი';ru='Объемная'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Изометрическая Тогда
		Меню.Добавить(ТипДиаграммы.Изометрическая, НСтр("ka='ჩვეულებრივი';ru='Обычная'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяНепрерывная, НСтр("ka='უწყვეტი';ru='Непрерывная'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяЛента, НСтр("ka='ლენტი';ru='Лента'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяПирамида, НСтр("ka='პირამიდა';ru='Пирамида'"));
...
Процедура Подключаемый_ИерархияФильтраНажатие(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭлементПоле.ВыборГруппИЭлементов=ГруппыИЭлементы.Элементы Тогда
		...
		Элемент.Подсказка = НСтр("ka='გამოიყენება ჯგუფების არჩევა. დააჭირეთ ელემენტების ასარჩევად.';ru='Используется выбор групп. Нажмите для выбора элементов.'");
	...
	Иначе
		...
		Элемент.Подсказка = НСтр("ka='გამოიყენება ელემენტების არჩევა. დააჭირეთ ჯგუფების ასარჩევად.';ru='Используется выбор элементов. Нажмите для выбора групп.'");
...
Процедура ОтправитьПоЭлектроннойПочте(Команда)
	...
	Если ОтображениеСостояния.Видимость = Истина
		...
		ТекстВопроса = НСтр("ka='ანგარიშგება არ არის ჩამოყალიბებული. ჩამოვაყალიბოთ?';ru='Отчет не сформирован. Сформировать?'");
...
Процедура ДобавитьСтроку(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокСтрок Тогда
		Список.Добавить(Неопределено, НСтр("ka='სხვა ველები';ru='Прочие поля'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ДобавитьФильтр(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихФильтров Тогда
		Список.Добавить(Неопределено, НСтр("ka='სხვა დანარჩენი';ru='Прочее'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
	Если Список.Количество()=1 Тогда
...
Процедура ЗапомнитьНастройки(Команда)
	...
	Если Форма=Неопределено Тогда
		Состояние(НСтр("ka='არჩეული ანგარიშგების გამართვების რედაქტირების უფლება არ არის';ru='Нет права редактирования настроек выбранного отчета'"));
	КонецЕсли;
...
Процедура ОбновитьПредставлениеПериода()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ka='მთელი პერიოდი';ru='за все время'");
		...
		Иначе
			ПредставлениеПериода = НРег(ПредставлениеПериода(НачалоДня(Период.ДатаНачала), ?(ЗначениеЗаполнено(Период.ДатаОкончания), КонецДня(Период.ДатаОкончания), Период.ДатаОкончания), НСтр("ka='Л=ka';ru='Л=ru'")));
		КонецЕсли;
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ka='აქტუალური მონაცემები';ru='актуальные данные'");
		Иначе
...
Процедура ОбновитьЗаголовокОтчета()
	...
	Если РежимРасшифровки Тогда
		ЗаголовокОтчета = ЗаголовокОтчета + " (" + НСтр("ka='გაშიფრვა';ru='Расшифровка'") + ")";
	КонецЕсли; 
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтрШаблон(НСтр("ka='ანგარიშგება ""%1""-ის გაგზავნა ფოსტით';ru='Отправка отчета ""%1"" по почте'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура УстановитьВидимостьИДоступность(Событие = "")
	...
	Если ПолнаяИнициализация Тогда
		...
		НСтр("ka='თარიღისთვის';ru='На дату'"), 
		НСтр("ka='პერიოდი';ru='Период'"));
	КонецЕсли;
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		ТекстОшибки = НСтр("ka='ფიქსირებული შერჩევის დაყენება ვერ მოხერხდა ""%1"".';ru='Не удалось установить фиксированный отбор ""%1"".'");
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Если ФоновоеЗаданиеВыполнено() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ka='ანგარიშგება ჩამოყალიბებულია';ru='Отчет сформирован'"), , Заголовок);
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Если Отчет.КомпоновщикНастроек.Настройки.Структура.Количество()=0 Тогда
		ПоказатьОшибкиФормирования(НСтр("ka='არცერთი დაჯგუფება არ არის არჩეული';ru='Не выбрано ни одной группировки'"));
		Возврат Ложь;
	КонецЕсли; 
	...
	Попытка
		...
		РезультатФоновогоЗадания = УправлениеНебольшойФирмойСервер.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ka='ანგარიშგების ვარიანტები: ანგარიშგების ჩამოყალიბება';ru='Варианты отчетов: Формирование отчета'"),
			,
			Ложь);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	Иначе
		...
		ОтображениеСостояния.Текст                          = НСтр("ka='მიმდინარეობს ანგარიშგების ფორმირება...';ru='Отчет формируется...'");
...
Процедура ОтметитьНеактуальным()
	...
	ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='ანგარიშგება არ არის შედგენილი. ანგარიშგების მისაღებად დააჭირეთ ""ჩამოყალიბება""-ს (F5).';ru='Отчет не сформирован. Нажмите ""Сформировать"" (F5) для получения отчета.'");
	ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
...
Процедура Сформировать()
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеДляЗаголовка = НСтр("ka='მიმდინარე მომენტი';ru='текущий момент'");
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли;
...
Процедура ЗаполнитьВариантыСерийТочекЗначений()
	...
	ДоступныеСерии.Добавить(Неопределено, НСтр("ka='არ გამოიყენოს';ru='Не использовать'"));
	ДоступныеТочки.Добавить(Неопределено, НСтр("ka='არ გამოიყენოს';ru='Не использовать'"));
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		ДоступныеСерии.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее'"),, БиблиотекаКартинок.Выбрать);
		ДоступныеТочки.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ЗаполнитьВариантыГруппировкиКолонок()
	...
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Очистить();
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("", НСтр("ka='არ დაჯგუფდეს';ru='Не группировать'"));
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Функция ПроверитьСовместимостьГруппировокКолонок(ИмяПоляПроверки = "")
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		Если (ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок) ИЛИ (СравнениеПериод И НЕ ИмяПоля=ИмяСравнения) Тогда
			...
			НСтр("ka='სხვადასხვა ველი-პერიოდის სტრიქონებში და სვეტებში გამოყენება აკრძალულია. წინასწარ გამორთეთ ველი %1.';ru='Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(Элемент)=Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			Если ПустаяСтрока(ЗаголовокГруппировки) Тогда
				ЗаголовокГруппировки = НСтр("ka='<Детальные записи>';ru='<Детальные записи>'");
			КонецЕсли; 
...
Процедура УстановитьПредставлениеКнопкиИерархииГруппировки(Кнопка, Стр)
	...
	Если Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
		...
		Кнопка.Подсказка = НСтр("ka='მხოლოდ ელემენტების გამოსახვა';ru='Отображать только элементы'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.ТолькоИерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.РежимВыбораГрупп;
		Кнопка.Подсказка = НСтр("ka='მხოლოდ იერარქიის გამოსახვა';ru='Отображать только иерархию'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Иерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.РежимВыбораГруппИЭлементов;
		Кнопка.Подсказка = НСтр("ka='ელემენტების გამოსახვა იერარქიაში';ru='Отображать элементы и иерархию'");
	КонецЕсли; 
...
Функция СписокПолейГруппировок(Сравнение = Ложь)
	...
	Если (НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок И НЕ Сравнение) 
		...
		СписокВыбора.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Функция ПроверитьСовместимостьДиаграммы(ИмяПоляПроверки = "")
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		Если (ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки) ИЛИ (СерияПериод И НЕ ИмяПоля=ДиаграммаСерии) Тогда
			...
			КонецЕсли; 
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგებაში და დიაგრამაში სხვადასხვა ველ-პერიოდების გამოყენება აკრძალულია. თავდაპირველად, გამორთეთ ველი %1';ru='Запрещено использование в отчете и диаграмме различных полей-периодов. Предварительно отключите поле %1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ЗаполнитьВариантыСравнения()
	...
	Элементы.ПолеСравнения.СписокВыбора.Очистить();
	Элементы.ПолеСравнения.СписокВыбора.Добавить(Неопределено, НСтр("ka='აე იყოს შედარებულია';ru='Не сравнивать'"));
	
	Если НастройкиОтчета.ИспользоватьПериодичность Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.День, НСтр("ka='დღეები';ru='Дни'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Неделя, НСтр("ka='კვირა';ru='Недели'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Месяц, НСтр("ka='თვეები';ru='Месяцы'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Квартал, НСтр("ka='კვარტლები';ru='Кварталы'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Полугодие, НСтр("ka='ნახევარი წელი';ru='Полугодия'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Год, НСтр("ka='წლები';ru='Годы'"));
	КонецЕсли;
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихПолейСравнения Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Булево")) Тогда
		...
		Значения.Добавить(Перечисления.БулевоТриСостояния.НеИзменять, НСтр("ka='ყველა';ru='Все'"), Значение=Неопределено);
...
Процедура ЗаполнитьТаблицуНаложенныхФильтров()
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Стр.Значение) Тогда
			ПредставлениеЗначения = ЗаголовокФильтра+НСтр("ka=': не указано';ru=': не указано'");
...
Процедура РедактироватьСКД()
	...
	Если Не НастройкиОтчета.РазрешитьРедактироватьСКД Тогда
		Состояние(НСтр("ka='მოცემული ანგარიშგების გამართვების რედაქტირება აკრძალულია';ru='Редактирование настроек данного отчета запрещено'"));
		Возврат;
	КонецЕсли;
...
Функция СуммаЯчеек(ТабличныйДокумент, ВыделенныеОбласти)
	...
	Если ВыделенныеОбласти = Неопределено Тогда
		...
		#Иначе
			Возврат НСтр("ka='პარამეტრ ""გამოყოფილიარეალები""-ს მნიშვნელობა არ არის მითითებული.';ru='Не указано значение параметра ""ВыделенныеОбласти"".'");
		#КонецЕсли
	КонецЕсли;
...
Процедура ВыгрузкаXML(Команда)
	...
	Если ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" Тогда
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1'"), "xml", "xml");
	ИначеЕсли ИмяОтчета = "ПростаяПенсионнаяДекларация" Тогда 
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ყველა ფაილი (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1'"), "xls", "xls");
	КонецЕсли;
