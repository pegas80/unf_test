...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяОтчета = ОтчетОбъект.Метаданные().Имя; //+++ Дмгг  
	...
	Если ФормаПараметры.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
		...
		//+++ Дмгг
		Элементы.ПредставлениеПериода2.Шрифт = Новый Шрифт(Элементы.ПредставлениеПериода.Шрифт,, 13);
		Элементы.ПредставлениеПериода2.ШрифтЗаголовка = Новый Шрифт(Элементы.ПредставлениеПериода.ШрифтЗаголовка,, 13);
		//--- Дмгг
	Иначе
	...
	//++Lazare 05.02.2024
	ЗапретРедактированияОтчетов  = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(,"ЗапретРедактированияОтчетов");
	Элементы.СКД.Доступность 	 = Не ЗапретРедактированияОтчетов;
	Элементы.Вариант.Доступность = Не ЗапретРедактированияОтчетов;
	Элементы.ГруппаСтроки.Доступность = Не ЗапретРедактированияОтчетов;
	Элементы.ГруппаКолонки.Доступность = Не ЗапретРедактированияОтчетов;
	//--Lazare 05.02.2024 
	
	//+++ Дмгг, 02.07.25, ONESOFT-4188
	Если ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" Тогда 
		ДополнитьНастройкиОтчетаПлатежнаяВыпискаУдерживанияИсточникаПлатежа();  
	КонецЕсли;
	//+++ Дмгг, 02.07.25, ONESOFT-4188
	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			Если ПериодНачало.АвтоОтметкаНезаполненного
				...
				ТекстОшибки = НСтр("ru = 'Не указан период'; ka = 'პერიოდი არ არის მითითებული'; en = 'Unknown Period'");
				ПутьКДанным = ПериодНачало.ПутьКДанным;
			ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
				ТекстОшибки = НСтр("ka='პერიოდის დასასრული დასაწყისზე მეტი უნდა იყოს';ru='Конец периода должен быть больше начала';en='End of the period must be greater than start'");
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		ПоказатьВводДаты(Оповещение, Период.ДатаОкончания, НСтр("ka='თარიღისთვის';ru='На дату';en='On the date'"), ЧастиДаты.Дата);
	КонецЕсли; 
...
Процедура ПредставлениеПериодаНажатиеЗавершение(НовыйПериод, ДополнительныеДанные) Экспорт
	...
	//+++ Дмгг, 02.07.25, ONESOFT-4188 
	//Поставил здесь, специапльно что бы замен происходила только от изменения дати
	Если ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" Тогда 
		
		ОбновитьНастройкиНаФорме();
		Если НастройкиОтчета.БылоИзмененияСКД Тогда 
			ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
			ОтображениеСостояния.Видимость = Истина;
			ОтображениеСостояния.Текст = НСтр("ka='Проверте настройки отчете. ანგარიშგება არ არის შედგენილი. ანგარიშგების მისაღებად დააჭირეთ ""ჩამოყალიბება""-ს (F5).';ru='Проверте настройки отчете. Отчет не сформирован. Нажмите ""Сформировать"" (F5) для получения отчета.';en='Проверте настройки отчете. The report has not been formed. Click ""Generate"" (F5) to obtain a report.'");
...
//+++ Дмгг
&НаКлиенте
Процедура ПредставлениеПериода2Нажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПараметрПериод = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Период2");
	Если ПараметрПериод=Неопределено Тогда
		Возврат;
	КонецЕсли;
	Оповещение = Новый ОписаниеОповещения("ПредставлениеПериода2НажатиеЗавершение", ЭтотОбъект);
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Диалог = Новый ДиалогРедактированияСтандартногоПериода;
		Диалог.Период = Период2;
		Диалог.Показать(Оповещение);
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		ПоказатьВводДаты(Оповещение, Период2.ДатаОкончания, НСтр("ka='თარიღისთვის';ru='На дату';en='On the date'"), ЧастиДаты.Дата);
	КонецЕсли; 
...
//+++ Дмгг
&НаКлиенте
Процедура ПредставлениеПериода2НажатиеЗавершение(НовыйПериод, ДополнительныеДанные) Экспорт
	...
	Если ТипЗнч(НовыйПериод)=Тип("СтандартныйПериод") Тогда
		Период2 = НовыйПериод;
	ИначеЕсли ТипЗнч(НовыйПериод)=Тип("Дата") Тогда
		Период2.ДатаНачала = '0001-01-01';
		Период2.ДатаОкончания = НовыйПериод;
		Период2.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
	КонецЕсли;
	ПериодПриИзмененииСервер();
	ОтметитьНеактуальным();
...
Процедура СтрокаОтбораАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если СписокВыбора.Количество()=0 И СтрДлина(Текст)>2 Тогда
		СписокВыбора.Добавить("", Новый ФорматированнаяСтрока(НСтр("ka='თანხვედრები ვერ მოიძებნა';ru='Совпадений не найдено';en='No matches found'"),, WebЦвета.НейтральноСерый));
	КонецЕсли; 
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
		...
		Если НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.Ресурс Тогда
			...
			Для каждого Область Из ОтчетТабличныйДокумент.ВыделенныеОбласти Цикл
				...
				Если НЕ ПустаяСтрока(Представление) Тогда
					ДробныйРазделитель = НСтр("ka=',';ru=',';en='.'");
	...
	Если Данные.ЭтоСсылка Тогда
	    ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение, НСтр("ka='გახსნა';ru='Открыть';en='Open'"));
	КонецЕсли; 
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И НЕ ДоступноеПоле.Ресурс Тогда
		ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Отфильтровать, НСтр("ka='გაფილტრვა""';ru='Отфильтровать ""';en='filter ""'")+Данные.Поле+" = "+Данные.Значение+"""");
	КонецЕсли;
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.ТипЗначения.Типы().Количество()>0 Тогда
		ДополнительныеДействия.Добавить(СформироватьПодменюОформить(Данные.Поле, Данные.Значение), НСтр("ka='გაფორმება';ru='Оформить';en='Contents'"));
		ПодменюСортировка = Новый СписокЗначений;
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Возр, Данные.Поле+НСтр("ru = ' по возрастанию'; ka = ' ზრდადობით'; en = ' ascending'"),, БиблиотекаКартинок.СортироватьСписокПоВозрастанию); 
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, Данные.Поле+НСтр("ru = ' по убыванию'; ka = ' კლებადობით'; en = ' descending'"),, БиблиотекаКартинок.СортироватьСписокПоУбыванию); 
	    ДополнительныеДействия.Добавить(ПодменюСортировка, НСтр("ka='დახარისხება';ru='Сортировать';en='Sort'"));
	КонецЕсли;
...
Функция СформироватьПодменюОформить(Поле, Значение)
	...
	Для каждого Тип Из ТипЗначения.Типы() Цикл
		...
		НСтр("ru = 'Редактировать список быстрых настроек'; ka = 'სწრაფი გამართვების სიის რედაქტირება'; en = 'Edit the list of Quick Settings'"),, 
		БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
		СписокКоманд.Добавить(
		"РучнаяНастройкаОформления", 
		НСтр("ru = 'Ручная настройка оформления'; ka = 'გაფორმების გამართვა ხელის რეჟიმში'; en = 'Manual tuning clearance'"),, 
...
Процедура ОткрытьФормуОтчета()
	...
	Если СтандартнаяОбработка Тогда
		...
		СтруктураОткрытия.Вставить("ПредставлениеВарианта", Строка(НастройкиОтчета.НаименованиеВарианта)+НСтр("ru = ' (Расшифровка)'; ka = '(გაშიფვრა)'; en = ' (Transcript)'"));
...
Процедура ВидДиаграммыНачалоВыбораЗадержка()
	...
	Меню.Добавить(Неопределено, НСтр("ka='არ გამოიტანოს';ru='Не выводить';en='Do not show'"),, БиблиотекаКартинок.Пустая);
	Меню.Добавить(ТипДиаграммы.График, НСтр("ka='გრაფიკები';ru='Графики';en='graphics'"),, КартинкаДиаграммы(ТипДиаграммы.График));
	Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ka='ჩვეულებრივი ჰისტოგრამები';ru='Обычные гистограммы';en='conventional histogram'"),, КартинкаДиаграммы(ТипДиаграммы.Гистограмма));
	Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ka='ჰორიზონტი. ჰისტოგრამა';ru='Горизонт. гистограммы';en='Horizon. histogram'"),, КартинкаДиаграммы(ТипДиаграммы.ГистограммаГоризонтальная));
	Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ka='წრიული დიაგრამები';ru='Круговые диаграммы';en='pie charts'"),, КартинкаДиаграммы(ТипДиаграммы.Круговая));
	Меню.Добавить(ТипДиаграммы.Изометрическая, НСтр("ka='იზომეტრიული დიაგრამები';ru='Изометрические диаграммы';en='Isometric diagrams'"),, КартинкаДиаграммы(ТипДиаграммы.Изометрическая));
	Меню.Добавить("<Прочие>", НСтр("ka='სხვები';ru='Прочие';en='other'"),, КартинкаДиаграммы(ТипДиаграммы.Поверхность));
...
Процедура ВидДиаграммыВыборГруппы(Значение, ДополнительныеДанные) Экспорт
	...
	Если Значение.Значение=ТипДиаграммы.График Тогда
		Меню.Добавить(ТипДиаграммы.График, НСтр("ka='ჩვეულებრივი';ru='Обычный';en='Usual'"));
		Меню.Добавить(ТипДиаграммы.ГрафикПоШагам, НСтр("ka='ნაბიჯების მიხედვით';ru='По шагам';en='step by step'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСНакоплением, НСтр("ka='დაგროვებით';ru='С накоплением';en='Stacked'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластями, НСтр("ka='ოლქებით';ru='С областями';en='With areas'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиИНакоплением, НСтр("ka='არეალებით და დაგროვებით';ru='С областями и накоплением';en='With the stacked area'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиНормированный, НСтр("ka='ნორმირებული';ru='Нормированный';en='normalized'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Гистограмма Тогда
		Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ka='ჩვეულებრივი';ru='Обычная';en='usual'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплением, НСтр("ka='დაგროვებით';ru='С накоплением';en='Stacked'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированная, НСтр("ka='ნორმირებული';ru='Нормированная';en='normalized'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаОбъемная, НСтр("ka='მოცულობითი';ru='Объемная';en='bulk'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемОбъемная, НСтр("ka='მოცულობითი დაგროვებით';ru='Объемная с накоплением';en='With the accumulation volume'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяОбъемная, НСтр("ka='მოცულობითი ნორმირებული';ru='Объемная нормированная';en='volume normalized'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.ГистограммаГоризонтальная Тогда
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ka='ჩვეულებრივი';ru='Обычная';en='usual'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальная, НСтр("ka='დაგროვებით';ru='С накоплением';en='Stacked'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальная, НСтр("ka='ნორმირებული';ru='Нормированная';en='normalized'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальнаяОбъемная, НСтр("ka='მოცულობითი';ru='Объемная';en='bulk'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальнаяОбъемная, НСтр("ka='მოცულობითი დაგროვებით';ru='Объемная с накоплением';en='With the accumulation volume'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальнаяОбъемная, НСтр("ka='მოცულობითი ნორმირებული';ru='Объемная нормированная';en='volume normalized'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Круговая Тогда
		Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ka='ჩვეულებრივი';ru='Обычная';en='usual'"));
		Меню.Добавить(ТипДиаграммы.КруговаяОбъемная, НСтр("ka='მოცულობითი';ru='Объемная';en='bulk'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Изометрическая Тогда
		Меню.Добавить(ТипДиаграммы.Изометрическая, НСтр("ka='ჩვეულებრივი';ru='Обычная';en='usual'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяНепрерывная, НСтр("ka='უწყვეტი';ru='Непрерывная';en='continuous'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяЛента, НСтр("ka='ლენტი';ru='Лента';en='Tape'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяПирамида, НСтр("ka='პირამიდა';ru='Пирамида';en='Pyramid'"));
...
Процедура Подключаемый_ИерархияФильтраНажатие(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭлементПоле.ВыборГруппИЭлементов=ГруппыИЭлементы.Элементы Тогда
		...
		ЭлементПоле.ПодсказкаВвода = Стр.Заголовок+НСтр("ru = ' (группа)'; ka = '(ჯგუფი)'; en = ' (group)'");
		Элемент.Подсказка = НСтр("ka='გამოიყენება ჯგუფების არჩევა. დააჭირეთ ელემენტების ასარჩევად.';ru='Используется выбор групп. Нажмите для выбора элементов.';en='Use the selection groups. Click to select the items.'");
	...
	Иначе
		...
		Элемент.Подсказка = НСтр("ka='გამოიყენება ელემენტების არჩევა. დააჭირეთ ჯგუფების ასარჩევად.';ru='Используется выбор элементов. Нажмите для выбора групп.';en='Used items are selected. Click to select the groups.'");
...
Процедура ОтправитьПоЭлектроннойПочте(Команда)
	...
	Если ОтображениеСостояния.Видимость = Истина
		...
		ТекстВопроса = НСтр("ka='ანგარიშგება არ არის ჩამოყალიბებული. ჩამოვაყალიბოთ?';ru='Отчет не сформирован. Сформировать?';en='The report has not been formed. Form?'");
...
Процедура ДобавитьСтроку(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокСтрок Тогда
		Список.Добавить(Неопределено, НСтр("ka='სხვა ველები';ru='Прочие поля';en='Other fields'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ДобавитьФильтр(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихФильтров Тогда
		Список.Добавить(Неопределено, НСтр("ka='სხვა დანარჩენი';ru='Прочее';en='Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
	Если Список.Количество()=1 Тогда
...
Процедура ЗапомнитьНастройки(Команда)
	...
	ПараметрыОткрытия.Вставить("НастройкиОтчета", НастройкиОтчета);   //Малышев 2024-10-24 ONESOFT-3543
	
	//+Lazare 09.03.2022
	ПараметрыОткрытия.Вставить("ФиксацияСверху", ФиксацияСверху);
	ПараметрыОткрытия.Вставить("ФиксацияСлева", ФиксацияСлева);
	//-Lazare 09.03.2022
	Форма = ОткрытьФорму("ОбщаяФорма.СохранениеВариантаОтчетаУНФ", ПараметрыОткрытия, ЭтаФорма);
	Если Форма=Неопределено Тогда
		Состояние(НСтр("ka='არჩეული ანგარიშგების გამართვების რედაქტირების უფლება არ არის';ru='Нет права редактирования настроек выбранного отчета';en='No right to edit the settings for the selected report'"));
	КонецЕсли;
...
Процедура ПериодПриИзмененииСервер()
	...
	ОбновитьПредставлениеПериода2(); //+++ Дмгг
	...
	//+++ Дмгг
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта) И НЕ НастройкиОтчета.РежимПериода="БезПериода" И НЕ НастройкиОтчета.Контекстный Тогда
		ХранилищеСистемныхНастроек.Сохранить("ПериодОтчета1", НастройкиОтчета.ИдентификаторВарианта, Период2);
	КонецЕсли;
	//--- Дмгг
КонецПроцедуры
...
Процедура ОбновитьПредставлениеПериода()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ka='მთელი პერიოდი';ru='за все время';en='during all this time'");
		ИначеЕсли Элементы.ПредставлениеПериода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет Тогда
			ПредставлениеПериода = НСтр("ru = 'за '; ka = 'განმავლობაში '; en = 'during '")+НРег(ПредставлениеПериода(Отчет.Период.ДатаНачала, КонецДня(Период.ДатаОкончания)))+НСтр("ru = 'для'; ka = 'თვის'; en = 'for'");
		Иначе
			ПредставлениеПериода = НРег(ПредставлениеПериода(НачалоДня(Период.ДатаНачала), ?(ЗначениеЗаполнено(Период.ДатаОкончания), КонецДня(Период.ДатаОкончания), Период.ДатаОкончания), НСтр("ru = 'Л=ru'; ka = 'ენა=ka'; en = 'L = en'")));
		КонецЕсли;
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ka='აქტუალური მონაცემები';ru='актуальные данные';en='the actual data'");
		Иначе
...
//+++ Дмгг
&НаСервере
Процедура ОбновитьПредставлениеПериода2()
	
	Если НЕ Элементы.ПредставлениеПериода2.Видимость Тогда
		Возврат;
	КонецЕсли;                            
	
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НЕ ЗначениеЗаполнено(Период2.ДатаНачала) И НЕ ЗначениеЗаполнено(Период2.ДатаОкончания) Тогда
			ПредставлениеПериода2 = НСтр("ka='მთელი პერიოდი';ru='за все время';en='during all this time'");
		ИначеЕсли Элементы.ПредставлениеПериода2.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет Тогда
			ПредставлениеПериода2 = НСтр("ru = 'за '; ka = 'განმავლობაში '; en = 'during '")+НРег(ПредставлениеПериода(Отчет.Период2.ДатаНачала, КонецДня(Период2.ДатаОкончания)))+НСтр("ru = 'для'; ka = 'თვის'; en = 'for'");
		Иначе
			ПредставлениеПериода2 = НРег(ПредставлениеПериода(НачалоДня(Период2.ДатаНачала), ?(ЗначениеЗаполнено(Период2.ДатаОкончания), КонецДня(Период2.ДатаОкончания), Период2.ДатаОкончания), НСтр("ru = 'Л=ru'; ka = 'ენა=ka'; en = 'L = en'")));
		КонецЕсли;
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период2.ДатаОкончания) Тогда
			ПредставлениеПериода2 = НСтр("ka='აქტუალური მონაცემები';ru='актуальные данные';en='the actual data'");
		Иначе
			ПредставлениеПериода2 = Формат(Период2.ДатаОкончания, "ДФ=dd.MM.yyyy");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокОтчета()
	...
	Если РежимРасшифровки Тогда
		ЗаголовокОтчета = ЗаголовокОтчета + " (" + НСтр("ka='გაშიფრვა';ru='Расшифровка';en='transcript'") + ")";
	КонецЕсли; 
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтрШаблон(НСтр("ka='ანგარიშგება ""%1""-ის გაგზავნა ფოსტით';ru='Отправка отчета ""%1"" по почте';en='Submitting the report ""%1"" in the mail'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура УстановитьВидимостьИДоступность(Событие = "")
	...
	//ВидимостьКнопкиВыгрузкиXML = ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" ИЛИ ИмяОтчета = "ПростаяПенсионнаяДекларация"; //+++ Дмгг
	ВидимостьКнопкиВыгрузкиXML = ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" ИЛИ ИмяОтчета = "ПростаяПенсионнаяДекларация" //+++ Дмгг
	ИЛИ ИмяОтчета = "ДекларацияДляИндивидуальныхПредпринимателей"; // Kuzmich, 01.12.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель) 
	...
	Если ПолнаяИнициализация Тогда
		...
		НСтр("ka='თარიღისთვის';ru='На дату';en='On the date'"), 
		НСтр("ka='პერიოდი';ru='Период';en='Period'"));
	КонецЕсли;
	...
	Элементы.ВыгрузкаXML.Видимость = ВидимостьКнопкиВыгрузкиXML;//+++ Дмгг
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	ЕстьПериод2 = (Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Период2") <> Неопределено);
	Элементы.ПредставлениеПериода2.Видимость = ЕстьПериод2;
	...
	ДополнительныйСвойства = Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства;
	                                                                            
	// Восстановление периода из настроек
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта) 
		И НЕ ПараметрыПериодов.Период=Неопределено 
		И НастройкиОтчета.ПользовательскиеНастройки=Неопределено 
		Тогда
		
		ПериодИзНастроек = ХранилищеСистемныхНастроек.Загрузить("ПериодОтчета", НастройкиОтчета.ИдентификаторВарианта);
		Период2ИзНастроек = ХранилищеСистемныхНастроек.Загрузить("ПериодОтчета1", НастройкиОтчета.ИдентификаторВарианта); //+++ Дмгг
		
		Если ДополнительныйСвойства.Свойство("Период") ТОгда
			ПериодИзНастроек = ДополнительныйСвойства.Период;
		КонецЕсли;	
		Если ДополнительныйСвойства.Свойство("Период2") ТОгда
			Период2ИзНастроек = ДополнительныйСвойства.Период2;
		КонецЕсли;	
		...
		Если ПериодИзНастроек=Неопределено И НЕ ПустаяСтрока(НастройкиОтчета.КлючВарианта) Тогда
			...
			Период2ИзНастроек = ХранилищеСистемныхНастроек.Загрузить("ПериодОтчета1", Строка(НастройкиОтчета.КлючВарианта)); //+++ Дмгг
		КонецЕсли; 
		...
		Если ТипЗнч(ПериодИзНастроек)=Тип("СтандартныйПериод") Тогда
			Если ТипЗнч(ПараметрыПериодов.Период.Значение)=Тип("СтандартныйПериод") Тогда
				...
				Период = ПараметрыПериодов.Период.Значение;  //Малышев 2024-10-24 ONESOFT-3543
			...
			ИначеЕсли ТипЗнч(ПараметрыПериодов.Период.Значение)=Тип("Дата") Тогда
				...
				ПараметрыПериодов.Период.Использование = Истина;     
				Период = ПериодИзНастроек; //Малышев 2024-10-24 ONESOFT-3543
			КонецЕсли; 
		ИначеЕсли ПериодИзНастроек = Неопределено ТОгда
			//Если Период.			Тип(ВариантСтандартногоПериода.ПроизвольныйПериод
			Период.ДатаНачала = Дата(1,1,1);    
			Период.ДатаОкончания = Дата(1,1,1);    
		КонецЕсли; 
		
		//Малышев 2024-10-28 ONESOFT-3543 - закоментировал
		//+++ Дмгг
		//Если НЕ ТипЗнч(Период2ИзНастроек)=Тип("СтандартныйПериод") И ЕстьПериод2
		//	Тогда
		//		Элементы.ПредставлениеПериода2.Видимость = Ложь;
		//КонецЕсли;
		//--- Дмгг
		
		Если ЕстьПериод2 Тогда
			//ПараметрыПериодов.Вставить("Период1);
			//Период1 = Период1ИзНастроек;
			Если ТипЗнч(Период2ИзНастроек)=Тип("СтандартныйПериод") Тогда
				Если ТипЗнч(ПараметрыПериодов.Период2.Значение)=Тип("СтандартныйПериод") Тогда
					ПараметрыПериодов.Период2.Значение = Период2ИзНастроек;
					ПараметрыПериодов.Период2.Использование = Истина;
					Период2 = ПараметрыПериодов.Период2.Значение;  //Малышев 2024-10-24 ONESOFT-3543
				ИначеЕсли ТипЗнч(ПараметрыПериодов.Период2.Значение)=Тип("СтандартнаяДатаНачала") Тогда
					ПараметрыПериодов.Период2.Значение = Новый СтандартнаяДатаНачала(Период2ИзНастроек.ДатаОкончания);
					ПараметрыПериодов.Период2.Использование = Истина;
				ИначеЕсли ТипЗнч(ПараметрыПериодов.Период2.Значение)=Тип("Дата") Тогда
					ПараметрыПериодов.Период2.Значение = Период2ИзНастроек.ДатаОкончания;
					ПараметрыПериодов.Период2.Использование = Истина; 
					Период2 = Период2ИзНастроек; //Малышев 2024-10-24 ONESOFT-3543

				КонецЕсли; 
			ИначеЕсли Период2ИзНастроек = Неопределено ТОгда
				//Если Период.			Тип(ВариантСтандартногоПериода.ПроизвольныйПериод
				Период.ДатаНачала = Дата(1,1,1);    
				Период.ДатаОкончания = Дата(1,1,1);    
			КонецЕсли;	
		...
		КонецЕсли;
	Иначе
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("Период", Период); //Малышев 2024-10-24 ONESOFT-3543
	КонецЕсли; 
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		ТекстОшибки = НСтр("ka='ფიქსირებული შერჩევის დაყენება ვერ მოხერხდა ""%1"".';ru='Не удалось установить фиксированный отбор ""%1"".';en='Unable to establish a fixed selection ""%1"".'");
...
Функция ОпределитьПараметрыПериодовСКД(ОтчетОбъект)
	...
	//Малышев 2024-11-24 ONESOFT-3543 (--
	ПараметрПериод2 = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("СтПериод2");
	Если ПараметрПериод2=Неопределено Тогда
		ПараметрПериод2 = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Период2");
	КонецЕсли;
	ПараметрНачалоПериода2 = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("НачалоПериода2");
	ПараметрКонецПериода2 = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("КонецПериода2");
	
	Результат.Вставить("Период2", ПараметрПериод2);
	Результат.Вставить("НачалоПериода2", ПараметрНачалоПериода2);
	Результат.Вставить("КонецПериода2", ПараметрКонецПериода2);
	//Малышев 2024-11-24 ONESOFT-3543 --)
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Если ФоновоеЗаданиеВыполнено() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ka='ანგარიშგება ჩამოყალიბებულია';ru='Отчет сформирован';en='report generated'"), , Заголовок);
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Если Отчет.КомпоновщикНастроек.Настройки.Структура.Количество()=0 Тогда
		ПоказатьОшибкиФормирования(НСтр("ka='არცერთი დაჯგუფება არ არის არჩეული';ru='Не выбрано ни одной группировки';en='Unspecified any groups'"));
		Возврат Ложь;
	КонецЕсли; 
	...
	Попытка
		...
		РезультатФоновогоЗадания = УправлениеНебольшойФирмойСервер.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ka='ანგარიშგების ვარიანტები: ანგარიშგების ჩამოყალიბება';ru='Варианты отчетов: Формирование отчета';en='reporting options: Report Generation'"),
			,
			Ложь);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	Иначе
		...
		ОтображениеСостояния.Текст                          = НСтр("ka='მიმდინარეობს ანგარიშგების ფორმირება...';ru='Отчет формируется...';en='It generates a report ...'");
...
Процедура ОтметитьНеактуальным()
	...
	ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ka='ანგარიშგება არ არის შედგენილი. ანგარიშგების მისაღებად დააჭირეთ ""ჩამოყალიბება""-ს (F5).';ru='Отчет не сформирован. Нажмите ""Сформировать"" (F5) для получения отчета.';en='The report has not been formed. Click ""Generate"" (F5) to obtain a report.'");
	ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
...
//+++ Дмгг, 02.07.25, ONESOFT-4188
&НаСервере
Процедура ПоменятьСхемукомпановкиДляПлатежнаяВыпискаУдерживанияИсточникаПлатежа(БылоИзмененияСКД)
	...
	Если Период.ДатаОкончания < Дата("20241201000000") И НЕ ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа = 0 Тогда
		
		СхемаКомпоновкиДанных = Отчеты.ПлатежнаяВыпискаУдерживанияИсточникаПлатежа.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
		...
		УровеньГруппировки = ТекущиеНастройки.Структура[0];	
		Для каждого ЭлемГруппировки Из УровеньГруппировки.ПоляГруппировки.Элементы Цикл  
			
			//Обработаываем только первыую
			ЭлементыГрупНовойСКД = ОсновныеНастройки.Структура[0].ПоляГруппировки.Элементы;     
			Для каждого ЭлемГруппировкиНовойСКД Из ЭлементыГрупНовойСКД Цикл               
				
				Если ЭлемГруппировкиНовойСКД.Поле = ЭлемГруппировки.Поле Тогда  
					
					ЭлемГруппировкиНовойСКД.Использование = ЭлемГруппировки.Использование;
					Прервать;
				КонецЕсли;
		...
		Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(ОсновныеНастройки);        
		
		ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа = 0;       
		БылоИзмененияСКД = Истина;
		
	ИначеЕсли Период.ДатаНачала >= Дата("20241201000000") И НЕ ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа = 1  Тогда
		
		СхемаКомпоновкиДанных = Отчеты.ПлатежнаяВыпискаУдерживанияИсточникаПлатежа.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных2025");
		...
		// Перенесем сделаные настройки группировок в новую СКД                              
		УровеньГруппировки = ТекущиеНастройки.Структура[0];	
		Для каждого ЭлемГруппировки Из УровеньГруппировки.ПоляГруппировки.Элементы Цикл  
		...
		ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа = 1;
		БылоИзмененияСКД = Истина;
	...
	//
	//НастройкиОтчета.Вставить("ПредопределенныеВарианты", Новый СписокЗначений);
	//Если НастройкиОтчета.Внешний И СхемаКомпоновкиДанных <> Неопределено Тогда
...
Процедура Сформировать()
	...
	//Малышев 2024-07-14
	НаименованиеВарианта = НастройкиОтчета.НаименованиеВарианта;
	Если НаименованиеВарианта = Неопределено или ТипЗнч(НаименованиеВарианта)<>Тип("Строка")Тогда
		НаименованиеВарианта = НастройкиОтчета.Наименование;
	КонецЕсли;	
	
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если ЗначениеЗаполнено(Период.ДатаНачала) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru = 'за '; ka = 'თვის  '; en = 'for '") + ПредставлениеПериода;
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли; 
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НаименованиеВарианта + " " + ПредставлениеДляЗаголовка);
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеДляЗаголовка = НСтр("ka='მიმდინარე მომენტი';ru='текущий момент';en='this moment'");
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли;
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НаименованиеВарианта + НСтр("ru = ' на '; ka = ' ზე '; en = ' on '") + ПредставлениеДляЗаголовка);
	Иначе
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НаименованиеВарианта);
	КонецЕсли;
...
Процедура ОбновитьПараметрыПериода()
	...
	//+++ Дмгг
	ПараметрПериод2 = Настройки.ПараметрыДанных.Элементы.Найти("Период2");   
	Если ПараметрПериод2=Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Если НЕ ПараметрПериод2=Неопределено Тогда
		Если ТипЗнч(ПараметрПериод2.Значение)=Тип("СтандартныйПериод") Тогда
			ПараметрПериод2.Значение = Период2;
			Если НастройкиОтчета.РежимПериода="НаДату" И ПараметрПериод2.Значение.ДатаНачала>ПараметрПериод2.Значение.ДатаОкончания Тогда
				ПараметрПериод2.Значение.ДатаНачала = '0001-01-01';
			КонецЕсли;
		ИначеЕсли ТипЗнч(ПараметрПериод2.Значение)=Тип("СтандартнаяДатаНачала") Тогда
			ПараметрПериод2.Значение.Вариант = ВариантСтандартнойДатыНачала.ПроизвольнаяДата;
			ПараметрПериод2.Значение.Дата = Период2.ДатаОкончания;
		ИначеЕсли ТипЗнч(ПараметрПериод2.Значение)=Тип("Дата") Тогда
			ПараметрПериод2.Значение = Период2.ДатаОкончания;
		КонецЕсли; 
		ПараметрПериод2.Использование = Истина;
	КонецЕсли;
	ПараметрКонецПериода2 = Настройки.ПараметрыДанных.Элементы.Найти("КонецПериода2");
	Если НЕ ПараметрКонецПериода=Неопределено Тогда
		ПараметрКонецПериода2.Значение = Период2.ДатаОкончания;
		ПараметрКонецПериода2.Использование = Истина;
	КонецЕсли;
	//--- Дмгг
...
Процедура ОбновитьНастройкиНаФорме()
	...
	Если НастройкиОтчета.ПрограммноеИзменениеФормыОтчета Тогда  
		...
		//	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
		//	ОтчетОбъект.ОбновитьНастройкиНаФорме(НастройкиОтчета, Отчет.КомпоновщикНастроек.Настройки, ЭтаФорма);
		//	ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа = НастройкиОтчета.ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа;   
		//Иначе
		//	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
		//	ОтчетОбъект.ОбновитьНастройкиНаФорме(НастройкиОтчета, Отчет.КомпоновщикНастроек.Настройки, ЭтаФорма);
		//КонецЕсли; 
	...
	ОбновитьОтображениеОтметокФильтров();
	ОбновитьЗаголовокОтчета();
	ОбновитьфиксацияСверхуИФиксацияСлева();//+Lazare 09.03.2022
	
КонецПроцедуры

//+Lazare 09.03.2022
&НаСервере
Процедура ОбновитьфиксацияСверхуИФиксацияСлева()
	
	Попытка //+++ Дмгг, ДЛя дополнительных отчетов которым присваеваешь эту форму
		ФиксацияСверху =   НастройкиОтчета.ВариантСсылка.ФиксацияСверху;
		ФиксацияСлева  =   НастройкиОтчета.ВариантСсылка.ФиксацияСлева;
	Исключение
	КонецПопытки;
КонецПроцедуры
//-Lazare 09.03.2022
...
Процедура ЗаполнитьВариантыСерийТочекЗначений()
	...
	ДоступныеСерии.Добавить(Неопределено, НСтр("ka='არ გამოიყენოს';ru='Не использовать';en='Do not use'"));
	ДоступныеТочки.Добавить(Неопределено, НСтр("ka='არ გამოიყენოს';ru='Не использовать';en='Do not use'"));
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		ДоступныеСерии.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее';en='Other'"),, БиблиотекаКартинок.Выбрать);
		ДоступныеТочки.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее';en='Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ОбновитьПериодНаФорме()
	...
	//+++ Дмгг, 11.10.24, ONESOFT-3392
	ПараметрПериод2 = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Период2");
	Если ПараметрПериод2=Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ПараметрПериод2=Неопределено ИЛИ НЕ ПараметрПериод2.Использование Тогда
		Период2 = Новый СтандартныйПериод;
	ИначеЕсли ТипЗнч(ПараметрПериод2.Значение)=Тип("Дата") Тогда
		Период2 = Новый СтандартныйПериод('0001-01-01', ПараметрПериод2.Значение);
	ИначеЕсли ТипЗнч(ПараметрПериод2.Значение)=Тип("СтандартныйПериод") Тогда
		Период2 = ПараметрПериод2.Значение;
	ИначеЕсли ТипЗнч(ПараметрПериод2.Значение)=Тип("СтандартнаяДатаНачала") Тогда
		Период2 = Новый СтандартныйПериод('0001-01-01', ПараметрПериод2.Значение.Дата);
	КонецЕсли;
	//--- Дмгг, 11.10.24, ONESOFT-3392
...
Процедура ЗаполнитьВариантыГруппировкиКолонок()
	...
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Очистить();
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("", НСтр("ka='არ დაჯგუფდეს';ru='Не группировать';en='do not group'"));
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее';en='Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
		...
		Если (ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок) ИЛИ (СравнениеПериод И НЕ ИмяПоля=ИмяСравнения) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в колонках отчета'; ka = ' ანგარიშგების სვეტებში'; en = ' in report columns'");
				ИначеЕсли СравнениеПериод И НЕ ИмяПоля=ИмяСравнения Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в колонках отчета'; ka = ' ანგარიშის სვეტებში'; en = ' in report columns'");
				КонецЕсли; 
			Иначе
				ОписаниеПоля = """"+?(ПустаяСтрока(Поле.Заголовок), Поле.Поле, Поле.Заголовок)+"""";
				ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в строках отчета'; ka = ' ანგარიშგების სტრიქონებში'; en = ' in report lines'");
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
			...
			НСтр("ru = 'Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1'; ka = 'სხვადასხვა ველი-პერიოდის სტრიქონებში და სვეტებში გამოყენება აკრძალულია. წინასწარ გამორთეთ ველი %1.'; en = 'It is forbidden to use in the rows and columns of different fields-periods. Pre disconnect box %1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(Элемент)=Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			Если ПустаяСтрока(ЗаголовокГруппировки) Тогда
				ЗаголовокГруппировки = НСтр("ru = '<Детальные записи>'; ka = '<დეტალური ჩანაწერები>'; en = '<Detailed records>'");
			КонецЕсли; 
...
Процедура УстановитьПредставлениеКнопкиИерархииГруппировки(Кнопка, Стр)
	...
	Если Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
		...
		Кнопка.Подсказка = НСтр("ka='მხოლოდ ელემენტების გამოსახვა';ru='Отображать только элементы';en='Show only items'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.ТолькоИерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.РежимВыбораГрупп;
		Кнопка.Подсказка = НСтр("ka='მხოლოდ იერარქიის გამოსახვა';ru='Отображать только иерархию';en='Show only hierarchy'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Иерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.РежимВыбораГруппИЭлементов;
		Кнопка.Подсказка = НСтр("ka='ელემენტების გამოსახვა იერარქიაში';ru='Отображать элементы и иерархию';en='Display elements and hierarchy'");
	КонецЕсли; 
...
Функция СписокПолейГруппировок(Сравнение = Ложь)
	...
	Если (НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок И НЕ Сравнение) 
		...
		СписокВыбора.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее';en='Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
		...
		Если (ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки) ИЛИ (СерияПериод И НЕ ИмяПоля=ДиаграммаСерии) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в диаграмме'; ka = 'დიაგრამაში'; en = 'in the diagram'");
			...
			Иначе
				ОписаниеПоля = """"+?(ПустаяСтрока(Поле.Заголовок), Поле.Поле, Поле.Заголовок)+"""";
				ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в структуре отчета'; ka = ' ანგარიშგების სტრუქტურაში'; en = ' in the report structure'1");
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					ДиаграммаТочки = Неопределено;
				КонецЕсли; 
			...
			КонецЕсли; 
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ანგარიშგებაში და დიაგრამაში სხვადასხვა ველ-პერიოდების გამოყენება აკრძალულია. თავდაპირველად, გამორთეთ ველი %1';ru='Запрещено использование в отчете и диаграмме различных полей-периодов. Предварительно отключите поле %1';en='It is forbidden to use in reports and charts of various fields-periods. Pre disconnect box 1%'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ЗаполнитьВариантыСравнения()
	...
	Элементы.ПолеСравнения.СписокВыбора.Очистить();
	Элементы.ПолеСравнения.СписокВыбора.Добавить(Неопределено, НСтр("ka='აე იყოს შედარებულია';ru='Не сравнивать';en='not compare'"));
	
	Если НастройкиОтчета.ИспользоватьПериодичность Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.День, НСтр("ka='დღეები';ru='Дни';en='days'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Неделя, НСтр("ka='კვირა';ru='Недели';en='of the week'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Месяц, НСтр("ka='თვეები';ru='Месяцы';en='months'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Квартал, НСтр("ka='კვარტლები';ru='Кварталы';en='districts'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Полугодие, НСтр("ka='ნახევარი წელი';ru='Полугодия';en='half of'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Год, НСтр("ka='წლები';ru='Годы';en='years'"));
	КонецЕсли;
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихПолейСравнения Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить("<Прочие>", НСтр("ka='სხვა დანარჩენი';ru='Прочее';en='Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Булево")) Тогда
		...
		Значения.Добавить(Перечисления.БулевоТриСостояния.НеИзменять, НСтр("ka='ყველა';ru='Все';en='Everything'"), Значение=Неопределено);
...
Процедура ОбновитьРеквизитыФильтровВПанелиПоПредставлению(Представление, Значение)
	...
	Строки = СтрокиДерева(ПоляНастроек, СтруктураОтбора);
	Для каждого Стр Из Строки Цикл
		//Малышев 2025-05-13 ONESOFT-4070
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Стр.ДополнительныеПараметры, "СвязьПоПредставлению", Ложь)=Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуНаложенныхФильтров()
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		Если ТипЗнч(Стр.Значение)=Тип("СтандартныйПериод") Тогда
			...
			ПредставлениеЗначения1 = ПредставлениеПериода(Стр.Значение.ДатаНачала, Стр.Значение.ДатаОкончания);  //+++ Дмгг
			Префикс = ЗаголовокФильтра;
		ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Структура") Тогда
			Если НЕ Стр.Значение.ЗначениеОт=Неопределено И НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru = 'от '; ka = 'დან '; en = 'from '")+Строка(Стр.Значение.ЗначениеОт) + НСтр("ru = ' до '; ka = ' მდე '; en = ' to '") + Строка(Стр.Значение.ЗначениеДо);
			ИначеЕсли НЕ Стр.Значение.ЗначениеОт=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru = 'от '; ka = 'დან '; en = 'from '")+Строка(Стр.Значение.ЗначениеОт);
			ИначеЕсли НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru = 'до '; ka = 'მდე '; en = 'to '")+Строка(Стр.Значение.ЗначениеДо);
			КонецЕсли; 
			Префикс = ЗаголовокФильтра;
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Стр.Значение) Тогда
			ПредставлениеЗначения = ЗаголовокФильтра+НСтр("ru = ': не указано'; ka = ': არ არის მითითებული'; en = ': not specified'");
...
Функция ПредставлениеВидаСравнения(ВидСравнения)
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВИерархии Тогда
		Возврат НСтр("ru = ' в группе '; ka = ' ჯგუფში '; en = ' In group '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСписке Тогда
		Возврат НСтр("ru = ' в списке '; ka = ' სიაში '; en = ' on the list '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
		Возврат НСтр("ru = ' в списке групп '; ka = ' ჯგუფში სიიდან '; en = ' in the list of groups '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Заполнено Тогда
		Возврат НСтр("ru = ' заполнено'; ka = ' შევსებულია '; en = ' filled '");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НачинаетсяС Тогда
		Возврат НСтр("ru = ' начинается с '; ka = ' დაწყება '; en = 'begin with'");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеВИерархии Тогда
		Возврат НСтр("ru = ' не в группе '; ka = ' არ არის ჯგუფში'; en = ' not in the group '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеВСписке Тогда
		Возврат НСтр("ru = ' не в списке '; ka = ' არ არის სიაში '; en = ' Not on the list '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
		Возврат НСтр("ru = ' не в списке групп '; ka = ' არ არის ჯგუფში სიიდან '; en = ' not in the list of groups '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеЗаполнено Тогда
		Возврат НСтр("ru = ' не заполнено'; ka = 'არ არის შევსებული'; en = ' not filled in '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеНачинаетсяС Тогда
		Возврат НСтр("ru = ' не начинается с '; ka = ' არ იწყება '; en = ' doesn''t start with '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеПодобно Тогда
		Возврат НСтр("ru = ' не подобно '; ka = ' არ არის მსგავსი '; en = 'not similar'");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеСодержит Тогда
		Возврат НСтр("ru = ' не содержит '; ka = ' არ შეიცავს '; en = ' does not contain '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Подобно Тогда
		Возврат НСтр("ru = ' подобно '; ka = ' მსგავსი '; en = ' similarly '");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Содержит Тогда
		Возврат НСтр("ru = ' содержит '; ka = ' შეიცავს  '; en = ' contains '");
	Иначе
		Возврат "";
	КонецЕсли; 	
...
Процедура ОбновитьОтображениеОтметокФильтров()
	...
	Если Элементы.ПредставлениеПериода.Видимость Тогда
		Если ФормаПараметры.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
			...
			Элементы.ПредставлениеПериода2.Ширина = Окр(СтрДлина(ПредставлениеПериода2)*0.75+4);  //+++ Дмгг
		Иначе
			Элементы.ПредставлениеПериода.Ширина = Окр(СтрДлина(ПредставлениеПериода)*0.8+4);
			Элементы.ПредставлениеПериода2.Ширина = Окр(СтрДлина(ПредставлениеПериода2)*0.8+4); //+++ Дмгг
		КонецЕсли; 
		ТекущаяДлинаСтрокиОтметок = Элементы.ПредставлениеПериода2.Ширина+СтрДлина(Элементы.ПредставлениеПериода2.Заголовок)+1; //+++ Дмгг
		ТекущаяДлинаСтрокиОтметок1 = Элементы.ПредставлениеПериода2.Ширина+СтрДлина(Элементы.ПредставлениеПериода2.Заголовок)+1; //+++ Дмгг
	Иначе
		ТекущаяДлинаСтрокиОтметок = 0;
		ТекущаяДлинаСтрокиОтметок1 = 0; //+++ Дмгг
	КонецЕсли; 
...
Функция СформироватьСписокЭлементовСервер(Текст, МассивТипов, УправляющиеСимволы)
	...
	Пока ВыборкаТип.Следующий() Цикл
		...
		Если Выборка.Количество()>1 ИЛИ (УправляющиеСимволы="""" И Выборка.Количество()>0) Тогда
			...
			Представление = ПредставлениеТипа+НСтр("ru = ' содержит '; ka = ' შეიცავს '; en = ' contains '")+Текст;
...
Процедура РедактироватьСКД()
	...
	Если Не НастройкиОтчета.РазрешитьРедактироватьСКД Тогда
		Состояние(НСтр("ka='მოცემული ანგარიშგების გამართვების რედაქტირება აკრძალულია';ru='Редактирование настроек данного отчета запрещено';en='Editing the settings of this report is forbidden'"));
		Возврат;
	КонецЕсли;
...
Процедура НаложитьОтборПоДобавленномуПолю(Поле, Значение, УправляющиеСимволы = "")
	...
	ИначеЕсли Поле="ДобавленноеПоле_Сегмент" Тогда
		...
		КонецЕсли; 
		Если ТипЗнч (Значение) = Тип ("Массив") Тогда 
			МассивСегментов = Справочники.Сегменты.ПолучитьСоставСегмента(Значение[0]);
		Иначе
...
Процедура ОбновитьДополнительныйСвойстваСКД()
	...
	Если НастройкиОтчета.РежимПериода="НаДату" Тогда
		...
		ДополнительныеСвойства.Вставить("Период2", Период2.ДатаОкончания); //+++ Дмгг
	ИначеЕсли НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		...
		ДополнительныеСвойства.Вставить("НачалоПериода2", Период2.ДатаНачала);  //+++ Дмгг
		ДополнительныеСвойства.Вставить("КонецПериода2", Период2.ДатаОкончания); //+++ Дмгг
		
		ДополнительныеСвойства.Вставить("Период", Период);
		ДополнительныеСвойства.Вставить("Период2", Период2); 
	...
	ДополнительныеСвойства.Вставить("ТекущийАдресСхемы", НастройкиОтчета.АдресСхемы);
	ДополнительныеСвойства.Вставить("ТекущийУникальныйИдентификатор",УникальныйИдентификатор);
	// Kuzmich, 14.01.2025, (ONESOFT-3767) Новая форма Декларации (Приложение "А")
КонецПроцедуры
...
&НаКлиенте
Процедура СКД(Команда)//Паганини
	РедактироватьСКД();
КонецПроцедуры

//+++ Дмгг
&НаКлиенте
Процедура ВыгрузкаXML(Команда)
	
	ВыборФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	ВыборФайла.МножественныйВыбор = Ложь;
	ВыборФайла.ПолноеИмяФайла = "";
	Если ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" Тогда
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (*.%1)|*.%1'; ka = 'ყველა ფაილი (*.%1)|*.%1'; en = 'All files (*.%1)|*.%1'"), "xml", "xml");
	ИначеЕсли ИмяОтчета = "ПростаяПенсионнаяДекларация" Тогда 
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (*.%1)|*.%1'; ka = 'ყველა ფაილი (*.%1)|*.%1'; en = 'All files (*.%1)|*.%1'"), "xls", "xls");
	// Kuzmich, 01.12.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель) 
	ИначеЕсли ИмяОтчета = "ДекларацияДляИндивидуальныхПредпринимателей" Тогда 
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (*.%1)|*.%1'; ka = 'ყველა ფაილი (*.%1)|*.%1'; en = 'All files (*.%1)|*.%1'"), "xml", "xml");
	// Kuzmich, 01.12.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель) 
	КонецЕсли;
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		Если ИмяОтчета = "ПлатежнаяВыпискаУдерживанияИсточникаПлатежа" Тогда
			...
			СтруктураГруппировки = НастройкиСКД.Структура[0];
			
			ПолеГруппировкиПолучатель = СтруктураГруппировки.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));       
			ПолеГруппировкиПолучатель.Использование      = Истина;
			ПолеГруппировкиПолучатель.Поле               = Новый ПолеКомпоновкиДанных("Получатель");
			ПолеГруппировкиПолучатель.ТипГруппировки 	 = ТипГруппировкиКомпоновкиДанных.Элементы;
			ПолеГруппировкиПолучатель.ТипДополнения      = ТипДополненияПериодаКомпоновкиДанных.БезДополнения;
			
			ПолеГруппировкиКодРС = СтруктураГруппировки.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));       
			ПолеГруппировкиКодРС.Использование      = Истина;
			ПолеГруппировкиКодРС.Поле               = Новый ПолеКомпоновкиДанных("РезидентураКодРС");
			ПолеГруппировкиКодРС.ТипГруппировки 	 = ТипГруппировкиКомпоновкиДанных.Элементы;
			ПолеГруппировкиКодРС.ТипДополнения      = ТипДополненияПериодаКомпоновкиДанных.БезДополнения;
			
			СтрокаХМЛ = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ЗаполнениеХМLЕжемесячныеВыплаты(НастройкиСКД);
			
			МассивУдаляемыхГруппировок = Новый Массив;
			Для Каждого Элем Из СтруктураГруппировки.ПоляГруппировки.Элементы Цикл 
				
				Если Строка(Элем.Поле) = "Получатель"
					ИЛИ Строка(Элем.Поле) = "РезидентураКодРС" Тогда 
					
					МассивУдаляемыхГруппировок.Добавить(Элем)  
					
				КонецЕсли;
				
			КонецЦикла;
			
			Для Каждого Элем Из МассивУдаляемыхГруппировок Цикл 
				
				СтруктураГруппировки.ПоляГруппировки.Элементы.Удалить(Элем);
				
			КонецЦикла;
			
			ТекстДок = Новый ТекстовыйДокумент;
			ТекстДок.ДобавитьСтроку(СтрокаХМЛ);
			ТекстДок.Записать(ВыборФайла.ПолноеИмяФайла);
		...
		ИначеЕсли ИмяОтчета = "ПростаяПенсионнаяДекларация" Тогда 
			...
			//+++ Дмгг
			// из-за преобразования ИНН в числовой формат, будем выгружать в ексель через ком объект что бы указать формат для ячеек
			...
			//Заполним шапку
			ЗаполнитьШапкуНаКлиенте(Лист, НомерСтроки);
			
			// установим высоту первых 2-ох строки и заливку серым
			Лист.Rows(1).RowHeight = 27;
			Лист.Rows(1).Interior.ColorIndex = 15;//заливка серым
			Лист.Rows(2).RowHeight = 15;
			Лист.Rows(1).Interior.ColorIndex = 15;//заливка серым
			
			//Получим результат и преобразуем его в массив структур для обработки на клиенте
			МассивСтруктурВыгрузить = ПолучитьМассиСтруктурИзТаблицыВыгрузкиНаСервере(НастройкиСКД);
			
			// Запись значения в ячейку
			//Лист.Cells(НомерСтроки, НомерКолонки).Value = ЗначениеЯчейки;
			
			Для Каждого ЭлемМасс Из МассивСтруктурВыгрузить Цикл 
				
				Если ЭлемМасс.НПП = Неопределено ИЛИ ЭлемМасс.НПП = 0 Тогда 
					Продолжить;
				КонецЕсли;
				
				НомерСтроки = НомерСтроки + 1;
				
				Лист.Cells(НомерСтроки, 1).NumberFormat = "0";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 1).Value = ЭлемМасс.НПП;
				
				Лист.Cells(НомерСтроки, 2).NumberFormat = "@";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 2).Value = ЭлемМасс.Имя;
				
				Лист.Cells(НомерСтроки, 3).NumberFormat = "@";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 3).Value = ЭлемМасс.Фамилия;
				
				Лист.Cells(НомерСтроки, 4).NumberFormat = "@";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 4).Value = ЭлемМасс.ИдентификационныйНомер;
				
				Лист.Cells(НомерСтроки, 5).NumberFormat = "0,00";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 5).Value = ЭлемМасс.СуммаВыдачиОборот;
				
				Лист.Cells(НомерСтроки, 6).NumberFormat = "0,00";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 6).Value = ЭлемМасс.СуммаПФВсего;
				
				Лист.Cells(НомерСтроки, 7).NumberFormat = "0";
				Лист.Cells(НомерСтроки, 1).Font.Size = 8;
				Лист.Cells(НомерСтроки, 7).Value = ЭлемМасс.УчаствуетДобровольно_;
			...
			Лист.Columns(3).ColumnWidth = 20;
			Лист.Columns(4).ColumnWidth = 20;
			Лист.Columns(5).ColumnWidth = 25;
			Лист.Columns(6).ColumnWidth = 13;
			Лист.Columns(7).ColumnWidth = 23;
			
			// Сохранение файла
			Книга.SaveAs(ВыборФайла.ПолноеИмяФайла);
			...
			//ТабличныйДокумент = ВыполнитВыгрузкуНаСервере(ИмяОтчета, НастройкиСКД);
			//
			//МассивУдаляемыхГруппировок = Новый Массив;
			//Для Каждого Элем Из СтруктураГруппировки.ПоляГруппировки.Элементы Цикл 
			//	
			...
			//ТабличныйДокумент.Записать(ВыборФайла.ПолноеИмяФайла, "XLS");
		...
		ИначеЕсли ИмяОтчета = "ДекларацияДляИндивидуальныхПредпринимателей" Тогда
			...
			СтрокаХМЛ = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ЗаполнениеХМLДекларацияДляИндивидуальныхПредпринимателей(НастройкиСКД);
...
//+++ Дмгг
&НаКлиенте
Процедура ЗаполнитьШапкуНаКлиенте(Лист, НомерСтроки)

	//Создадим шапку в экселе    
	//Лист.Cells(СтрокаЛиста, Колонка).NumberFormat = "@"; укажем формат ячейки "Текст"
	Лист.Cells(НомерСтроки, 1).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 1).Font.Size = 11;
	Лист.Cells(НомерСтроки, 1).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 1).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 1).Value = "N";
	
	Лист.Cells(НомерСтроки, 2).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 2).Font.Size = 11;
	Лист.Cells(НомерСтроки, 2).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 2).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 2).Value = "სახელი";
	
	Лист.Cells(НомерСтроки, 3).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 3).Font.Size = 11;
	Лист.Cells(НомерСтроки, 3).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 3).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 3).Value = "გვარი";
	
	Лист.Cells(НомерСтроки, 4).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 4).Font.Size = 11;
	Лист.Cells(НомерСтроки, 4).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 4).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 4).Value = "პირადი ნომერი";
	
	Лист.Cells(НомерСтроки, 5).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 5).Font.Size = 11;
	Лист.Cells(НомерСтроки, 5).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 5).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 5).WrapText = True; // Перенос текста в ячейке
	Лист.Cells(НомерСтроки, 5).Value = "ხელფასი/შემოსავალი";
	
	Лист.Cells(НомерСтроки, 6).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 6).Font.Size = 11;
	Лист.Cells(НомерСтроки, 6).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 6).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 6).WrapText = True; // Перенос текста в ячейке
	Лист.Cells(НомерСтроки, 6).Value = "შენატანი 2%+2%";
	
	Лист.Cells(НомерСтроки, 7).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 7).Font.Size = 11;
	Лист.Cells(НомерСтроки, 7).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 7).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 7).WrapText = True; // Перенос текста в ячейке
	Лист.Cells(НомерСтроки, 7).Value = "ნებაყოფლობით ჩართული";
	
	//
	// вторая строка простая нумерация колонок
	НомерСтроки = НомерСтроки + 1;
	...
	Лист.Cells(НомерСтроки, 1).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 1).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 1).Value = "1";
	
	Лист.Cells(НомерСтроки, 2).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 2).Font.Size = 11;
	Лист.Cells(НомерСтроки, 2).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 2).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 2).Value = "2";
	
	Лист.Cells(НомерСтроки, 3).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 3).Font.Size = 11;
	Лист.Cells(НомерСтроки, 3).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 3).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 3).Value = "3";
	
	Лист.Cells(НомерСтроки, 4).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 4).Font.Size = 11;
	Лист.Cells(НомерСтроки, 4).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 4).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 4).Value = "4";
	
	Лист.Cells(НомерСтроки, 5).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 5).Font.Size = 11;
	Лист.Cells(НомерСтроки, 5).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 5).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 5).Value = "5";
	
	Лист.Cells(НомерСтроки, 6).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 6).Font.Size = 11;
	Лист.Cells(НомерСтроки, 6).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 6).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 6).Value = "6";
	
	Лист.Cells(НомерСтроки, 7).NumberFormat = "@";
	Лист.Cells(НомерСтроки, 7).Font.Size = 11;
	Лист.Cells(НомерСтроки, 7).HorizontalAlignment=-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 7).VerticalAlignment =-4108;//выравнивание текста по центру
	Лист.Cells(НомерСтроки, 7).Value = "7";	
...
//+++ Дмгг
&НаСервере
Функция ПолучитьМассиСтруктурИзТаблицыВыгрузкиНаСервере(НастройкиСКД);
	
	ТаблицаВыгрузки = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьТаблицуЗначенийОтчета_ПростаяПенсионнаяДекларация(НастройкиСКД);
	
	ПозицияИмениФИО = Константы.ПозицияИмениВФИО.Получить();
	ФозицияФамилииФИО = Константы.ПозицияФамилииФИО.Получить();
	
	МассивСтруктура = Новый Массив;
	//
	//Датее выгрузка таблицы выгрузки
	Для Каждого Строк Из ТаблицаВыгрузки Цикл 
		
		СтруктураСтроки = Новый Структура;
		
		Имя = ОтчетыСервер.ПолучитьСловоИзТекста(Строка(Строк.Получатель), " ", ПозицияИмениФИО);
		Фамилия = ОтчетыСервер.ПолучитьСловоИзТекста(Строка(Строк.Получатель), " ", ФозицияФамилииФИО);
		
		СтруктураСтроки.Вставить("НПП", Строк.НПП);
		СтруктураСтроки.Вставить("Имя", Имя);
		СтруктураСтроки.Вставить("Фамилия", Фамилия);
		СтруктураСтроки.Вставить("ИдентификационныйНомер", Строка(Строк.ИдентификационныйНомер));
		СтруктураСтроки.Вставить("СуммаВыдачиОборот", Строк.СуммаВыдачиОборот);
		СтруктураСтроки.Вставить("СуммаПФВсего", Строк.СуммаПФВсего);
		СтруктураСтроки.Вставить("УчаствуетДобровольно_", Строк.УчаствуетДобровольно_);
		
		МассивСтруктура.Добавить(СтруктураСтроки);
		
	КонецЦикла;
	
	Возврат МассивСтруктура;
	
КонецФункции
//--- Дмгг

&НаСервере
Функция ВыполнитВыгрузкуНаСервере(ИмяОтчета,НастройкиСКД)
	...
	Макет = Отчеты.ПростаяПенсионнаяДекларация.ПолучитьМакет("МакетВыгрузки");
	...
	Для Каждого Строк Из ТаблицаВыгрузки Цикл 
		...
		Если Строк.НПП = Неопределено Тогда 
			Продолжить;
		КонецЕсли;
		
		ОбластьЗаполнения = Макет.ПолучитьОбласть("ОбластьСтрока");
		
		Строк.Имя = ОтчетыСервер.ПолучитьСловоИзТекста(Строка(Строк.Получатель), " ", ПозицияИмениФИО);
		Строк.Фамилия = ОтчетыСервер.ПолучитьСловоИзТекста(Строка(Строк.Получатель), " ", ФозицияФамилииФИО);
		
		ЗаполнитьЗначенияСвойств(ОбластьЗаполнения.Параметры,Строк);
		ТабДокумент.Вывести(ОбластьЗаполнения);
...
//+++ Дмгг, 02.07.25, ONESOFT-4188
&НаСервере
Процедура ДополнитьНастройкиОтчетаПлатежнаяВыпискаУдерживанияИсточникаПлатежа()
	
	Если ТипЗнч(НастройкиОтчета) = Тип("Структура") И НЕ НастройкиОтчета.Свойство("ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа") Тогда 
		Если Период.ДатаОкончания < Дата("20241201000000")  Тогда   
			НастройкиОтчета.Вставить("ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа", 0);
		ИначеЕсли Период.ДатаНачала >= Дата("20241201000000")  Тогда            
			НастройкиОтчета.Вставить("ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа", 1);
		КонецЕсли;   
		НастройкиОтчета.Вставить("БылоИзмененияСКД", ЛОЖЬ);
	ИначеЕсли ТипЗнч(НастройкиОтчета) = Тип("Структура") И НастройкиОтчета.Свойство("ТекущийВариантСКД_ПлатежнаяВыпискаУдерживанияИсточникаПлатежа") Тогда	
		Если Период.ДатаОкончания < Дата("20241201000000")  Тогда     
		...
		КонецЕсли;  
		НастройкиОтчета.БылоИзмененияСКД = ЛОЖЬ;
	КонецЕсли;
