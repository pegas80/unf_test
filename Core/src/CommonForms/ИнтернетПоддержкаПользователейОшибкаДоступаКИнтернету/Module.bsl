	...
	Элементы.НадписьСообщенияВТехПоддержку.Заголовок =
		ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				НСтр("ka='<body>პრობლემების წარმოქმნის შემთხვევაში წერილი გააგზავნეთ შემდეგ მისამართზე <a href=""SendMail"">webits-info@1c.ru</a></body>';ru='<body>При возникновении проблем отправьте письмо по адресу <a href=""SendMail"">webits-info@1c.ru</a></body>'")));
...
Процедура НадписьСообщенияВТехПоддержкуОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "SendMail" Тогда
		...
		Если МестоЗапуска = "handStartNew"
			...
			Тема = НСтр("ka='ინტერნეტ-მხარდაჭერა: ინტერნეტ-მხარდაჭერის მონიტორთან მიმართვა.';ru='Интернет-поддержка. Обращение к монитору Интернет-поддержки.'");
			Тело = НСтр("ka='წარმოიქმნება ინტერნეტ-მხარდაჭერის მონიტორთან მიმართვის შეცდომა.';ru='Возникает ошибка обращения к монитору Интернет-поддержки.'");
			
		ИначеЕсли МестоЗапуска = "taxcomGetID" Тогда
			
			Тема = НСтр("ka='ინტერნეტ-მხარდაჭერა. 1C-ტაქსკომთან მიმართვა';ru='Интернет-поддержка. Обращение к 1С-Такском'");
			Тело = НСтр("ka='1C-ტაქსკომის იდენტიფიკატორის მიღების სერვისთან მიმართვისას შეცდომა წარმოიშვება.';ru='Возникает ошибка при обращении к сервису получения идентификатора 1С-Такском.'");
		...
		Иначе
			...
			Тема = НСтр("ka='ინტერნეტ-მხარდაჭერა. სერვისთან მიმართვის შეცდომა';ru='Интернет-поддержка. Ошибка обращения к сервису'");
			Тело = НСтр("ka='ინტერნეტ-მხარდაჭერის სერვისთან მიმართვისას წარმოიშობა შეცდომა.';ru='Возникает ошибка при обращении к сервису Интернет-поддержки.'");
			
		КонецЕсли;
		
		
		Если Не ПустаяСтрока(ПодробноеОписаниеОшибки) Тогда
			Тело = Тело + Символы.ПС + НСтр("ka='შეცდომის აღწერა:';ru='Описание ошибки:'")
