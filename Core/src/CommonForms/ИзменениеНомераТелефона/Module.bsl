...
Процедура НадписьШаг1ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "#НапечататьЗаявление" И ПроверитьПодтвержденностьТелефона() Тогда
		...
		Если Результат.Выполнено Тогда
			...
			НазваниеПечатнойФормы = НСтр("ka='Заявление на смену абонентского номера подвижной (мобильной) связи';ru='Заявление на смену абонентского номера подвижной (мобильной) связи'");
...
Процедура ОтправитьЗаявление(Команда)
	...
	Если Не ЗначениеЗаполнено(АдресФайлаЗаявления) Тогда
		ТекстСообщения = НСтр("ka='Выберите файл заявления';ru='Выберите файл заявления'");
...
Функция ПроверитьПодтвержденностьТелефона()
	...
	Если Не ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
		ТекстСообщения = НСтр("ka='Сначала необходимо подтвердить новый номер телефона';ru='Сначала необходимо подтвердить новый номер телефона'");
...
Процедура УправлениеФормой(Форма)
	...
	Форма.ФайлЗаявления = НСтр("ka='არჩევა';ru='Выбрать'");
