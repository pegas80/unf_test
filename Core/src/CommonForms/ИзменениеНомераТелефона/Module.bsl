...
Процедура НадписьШаг1ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "#НапечататьЗаявление" И ПроверитьПодтвержденностьТелефона() Тогда
		...
		Если Результат.Выполнено Тогда
			...
			НазваниеПечатнойФормы = НСтр("ka='Заявление на смену абонентского номера подвижной (мобильной) связи';ru='Заявление на смену абонентского номера подвижной (мобильной) связи';en='Application to shift the mobile subscriber number (mobile) communication'");
...
Процедура ОтправитьЗаявление(Команда)
	...
	Если Не ЗначениеЗаполнено(АдресФайлаЗаявления) Тогда
		ТекстСообщения = НСтр("ka='Выберите файл заявления';ru='Выберите файл заявления';en='Select the application file'");
...
Функция ПроверитьПодтвержденностьТелефона()
	...
	Если Не ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
		ТекстСообщения = НСтр("ka='Сначала необходимо подтвердить новый номер телефона';ru='Сначала необходимо подтвердить новый номер телефона';en='First, you need to verify the new phone number'");
...
Процедура УправлениеФормой(Форма)
	...
	Форма.ФайлЗаявления = НСтр("ka='არჩევა';ru='Выбрать';en='Select'");
...
"По результатам рассмотрения заявления вы получите SMS на номер %1';ka='Заявление принято к рассмотрению."
