...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Узел = Менеджер.ЭтотУзел() Тогда
		ВызватьИсключение
			НСтр("ka='მოცემული კვანძისთვის საწყისი სახის შექმნა შეუძლებელია.';ru='Создание начального образа для данного узла невозможно.'");
	Иначе
	...
	Если Не СтандартныеПодсистемыВызовСервера.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая Тогда
		Элементы.ПутьКАрхивуСФайламиТомов.ПодсказкаВвода = НСтр("ka='\\имя сервера\resource\files.zip';ru='\\имя сервера\resource\files.zip'");
...
"на данной платформе не поддерживается.';ka='მოცემულ პლატფორმაზე ფაილური საინფორმაციო ბაზის საწყისი იერ-სახის შექმნას მხარდაჭერა არ გააჩნია.'");
	Иначе
		...
		Если ВидБазы = 0 Тогда
			...
			ПараметрыЗадания.Вставить("НаименованиеЗадания", НСтр("ka='ფაილური დაწყებითი იერ-სახის შექმნა';ru='Создание файлового начального образа'"));
		...
		Иначе
			...
			ПараметрыЗадания.Вставить("НаименованиеЗадания", НСтр("ka='სერვერული საწყისი იერ-სახის შექმნა';ru='Создание серверного начального образа'"));
...
Процедура ОбработчикСохраненияФайлаПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	Диалог.Заголовок                = НСтр("ka='აირჩიეთ ფაილი შესანახად';ru='Выберите файл для сохранения'");
...
Процедура ЗапуститьСозданиеНачальногоОбраза()
	...
	Иначе
		ВызватьИсключение НСтр("ka='Не удалось создать начальный образ по причине:';ru='Не удалось создать начальный образ по причине:'") + " " + Результат.КраткоеПредставлениеОшибки; 
	КонецЕсли;
...
Процедура СоздатьНачальныйОбразНаСервереЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ПроцентВыполнения = 0;
		ДопИнформацияВыполнение = НСтр("ka='Действие отменено администратором.';ru='Действие отменено администратором.'");
	...
	Если Результат.Статус = "Ошибка" Тогда
		ПроцентВыполнения = 0;
		Элементы.СтатусГотово.Заголовок = НСтр("ka='Не удалось создать начальный образ по причине:';ru='Не удалось создать начальный образ по причине:'") + " " + Результат.КраткоеПредставлениеОшибки;
		ЗапуститьПереходРезультат();
		Возврат;
	КонецЕсли;
