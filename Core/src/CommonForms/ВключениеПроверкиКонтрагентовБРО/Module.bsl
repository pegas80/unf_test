...
Функция ИнформацияОСервисе()
	...
	ЧастиСтроки.Добавить(НСтр("ka='გესრ-ში კონტრაგენტების რეგისტრაციის შესამოწმებლად პროგრამაში არსებობს საგადასახადო სამსახურის ვებ-სერვისის გამოყენების შესაძლებლობა';ru='В программе имеется возможность использовать веб-сервис ФНС для проверки регистрации контрагентов в ЕГРН';en='The program has the ability to use the FTS Web service to check the registration of contractors in USRRE'"));
	Если ЭтоЗапускПроверкиПослеЗаполнения Тогда
		ЧастиСтроки.Добавить(НСтр("ka='დეკლარაციის შევსების შემდეგ';ru=' после заполнения декларации'"));
	КонецЕсли;
	ЧастиСтроки.Добавить(НСтр("ka='(საჭიროა ინტერნეტთან წვდომა).';ru=' (требуется доступ в интернет).'"));
