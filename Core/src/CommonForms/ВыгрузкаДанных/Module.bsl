...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ТекстЗаголовкаФормы = НСтр("ka='მონაცემთა გადმოტვირთვა ლოკალურ ვერსიაში';ru='Выгрузить данные в локальную версию';en='Unload data in the local version'");
...
"и использования в локальной версии.'");
	Иначе
		ТекстЗаголовкаФормы = НСтр("ka='მონაცემთა გადმოტვირთვა სერვისში გადასასვლელად';ru='Выгрузить данные для перехода в сервис';en='Unload the data to go into service'");
...
Процедура ПолучитьЧастьФайла(ДополнительныеПараметры)
	...
	Состояние(НСтр("ka='ფაილის მიღება..';ru='Получение файла...';en='Receiving file ...'"), ДополнительныеПараметры.ПолученоЧастейФайла * 100 / ДополнительныеПараметры.ВсегоЧастейФайла);
...
Процедура ПоместитьВоВременноеХранилищеЧастьФайла(Знач ОписаниеЧастиФайла, АдресХранилища)
	...
	Если ХешированиеДанных.ХешСумма <> ОписаниеЧастиФайла.ХешСумма Тогда
		...
		ВызватьИсключение НСтр("ka='Ошибка при получении части файла.';ru='Ошибка при получении части файла.';en='Error retrieving the file.'");
...
Функция ВыгрузкаГотова()
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение(НСтр("ka='გადმოტვირთვის მომზადებისას შეცდომა მოხდა - გადმოტვირთვის მომამზადებელი დავალება ვერ მოიძებნა.';ru='При подготовке выгрузки произошла ошибка - не найдено задание подготавливающее выгрузку.';en='When preparing the discharge error occurred - did not match the task prepares unloading.'"));
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		Иначе
			ВызватьИсключение(НСтр("ka='გადმოტვირთვის მომზადებისას წარმოიშვა შეცდომა - გადმოტვირთვის მომამზადებელი დავალება უცნობი შეცდომით დასრულდა.';ru='При подготовке выгрузки произошла ошибка - задание подготавливающее выгрузку завершилось с неизвестной ошибкой.';en='When preparing the discharge error occurred - a task prepares the unloading was completed with an unknown error.'"));
		КонецЕсли;
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение(НСтр("ka='გადმოტვირთვის მომზადებისას შეცდომა წარმოიშვა - გადმოტვირთვის მომამზადებელი დავალება გაუქმდა ადმინისტრატორის მიერ.';ru='При подготовке выгрузки произошла ошибка - задание подготавливающее выгрузку было отменено администратором.';en='When preparing the discharge error occurred - a task prepares upload was canceled by an administrator.'"));
	Иначе
...
Процедура ЗапуститьВыгрузкуДанныхНаСервере()
	...
	Попытка
		...
		Задание = ФоновыеЗадания.Выполнить("ВыгрузкаЗагрузкаОбластейДанных.ВыгрузитьТекущуюОбластьДанныхВФайлИРазделитьНаЧасти", 
			ПараметрыЗадания,
			,
			НСтр("ka='მონაცემთა არეალის გადმოტვირთვის მომზადება';ru='Подготовка выгрузки области данных';en='Preparing upload data field'"));
...
Процедура ОтменитьЗаданиеПодготовки(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ka='მონაცემთა არეალის გადმოტვირთვის მომზადების დავალების შესრულების გაუქმება';ru='Отмена выполнения задания подготовки выгрузки области данных';en='Canceling the job upload data preparation area'", 
...
"-----------------------------------------'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ka='მონაცემთა გადმოტვირთვა';ru='Выгрузка данных';en='Upload data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		,
		ТекстЗаписиЖР);
