...
Функция ВыполнитьПроверку()
	...
	Если ПредставительЯвляетсяЮЛ Тогда
		...
		Если ЗначениеЗаполнено(ПредставительЮЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПредставительЮЛ_ИНН, Ложь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='წარმომადგენელი ორგანზიაციის გსნ არაკორექტულად არის მიცემული';ru='Некорректно задан ИНН организации-представителя'"), , "ПредставительЮЛ_ИНН", , );
			ВсеОК = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПредставительЮЛ_КПП) И НЕ РегламентированнаяОтчетностьКлиентСервер.КППСоответствуетТребованиям(ПредставительЮЛ_КПП) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректно задан КПП организации-представителя';ru='Некорректно задан КПП организации-представителя'"), , "ПредставительЮЛ_КПП", , );
			ВсеОК = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПредставительЮЛ_ОГРН) И СтрДлина(СокрЛП(ПредставительЮЛ_ОГРН)) <> 13 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Некорректно задан ОГРН организации-представителя';ru='Некорректно задан ОГРН организации-представителя'"), , "ПредставительЮЛ_ОГРН", , );
			ВсеОК = Ложь;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставительФЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПредставительФЛ_ИНН, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ფიზპირის-წარმომადგენლის გსნ არაკორექტულად არის მიცემული';ru='Некорректно задан ИНН физлица-представителя'"), , "ПредставительФЛ_ИНН", , );
		ВсеОК = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставительФЛ_ОГРН) И СтрДлина(СокрЛП(ПредставительФЛ_ОГРН)) <> 15 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ფიზპირის-წარმომადგენლის ძსსნ არაკორექტულად არის მიცემული';ru='Некорректно задан ОГРН физлица-представителя'"), , "ПредставительФЛ_ОГРН", , );
		ВсеОК = Ложь;
	КонецЕсли;
	Возврат ВсеОК;
