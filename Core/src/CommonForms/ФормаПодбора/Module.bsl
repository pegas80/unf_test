...
Процедура ИзменитьДобавитьСтрокуВКорзину(СтрокаСпискаТоваров, СтрокаХарактеристикПартий, СтрокаКорзины, Количество, Цена, СвободныйОстаток, ЕдИзмерения = Неопределено)
	...
	Если Строки.Количество() > 0 Тогда
		...
		Если ЗаполнятьРезерв
			И НЕ(Константы_НеРезервироватьВФормеПодбора_Получить())// Kuzmich, 06.11.2024, (ONESOFT-3530) существующие константа: Не резервировать в форме подбора  - должно быть исправить также для новом функционале резервиорвание
...
Процедура УстановитьВидимостьПолейДинамическогоСписка(СтруктураНастроек)
	...
	Если Элементы.Найти("СписокХарактеристикОстаток") <> Неопределено Тогда 
		...
		ВидимостьСписокХарактеристикОстаток                     = ПоказыватьВТаблице И СтруктураНастроек.ПоказыватьОстатки И СтруктураНастроек.ТребуетсяПоказОстатков;    
	Иначе
		ВидимостьСписокХарактеристикОстаток                     = Ложь;
	КонецЕсли;
	Если Элементы.Найти("СписокХарактеристикРезерв") <> Неопределено Тогда
		...
		ВидимостьСписокХарактеристикРезерв                      = ПоказыватьВТаблице И СтруктураНастроек.ПоказыватьРезерв И НЕ ЗапросПоСкладу И СтруктураНастроек.ТребуетсяПоказОстатков;  
	Иначе
		ВидимостьСписокХарактеристикРезерв                     = Ложь;
	КонецЕсли;
	Если Элементы.Найти("СписокХарактеристикГруппаОстатки") <> Неопределено Тогда
		...
		Элементы.СписокХарактеристикГруппаОстатки.Видимость		= ВидимостьСписокХарактеристикОстаток ИЛИ ВидимостьСписокХарактеристикРезерв;
	КонецЕсли;
	Если Элементы.Найти("СписокХарактеристикСвободныйОстаток") <> Неопределено Тогда
...
Функция ПолучитьНастройкиПодбора()
	...
	СтруктураОтвета = Новый Структура("ЗапрашиватьКоличествоИЦену, ПоказыватьОстатки, ПоказыватьРезерв, ПоказыватьСвободныйОстаток, ПоказыватьЦены, ТребуетсяПоказОстатков, СпособВыводаОстатков, ЗапоминатьТекущуюИерархию, НеПоказыватьНулевыеОстатки",
	//--- Дмгг, 23.01.24, (ONESOFT-2840)
				...
				// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
				УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "ПоказыватьРезерв") И (Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() ИЛИ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить()) И ТребуетсяПоказОстатков,
				УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "ПоказыватьСвободныйОстаток") И (Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() ИЛИ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить()) И ТребуетсяПоказОстатков,
				// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
	...
	//+++ Дмгг, 23.01.24, (ONESOFT-2840)
				...
				УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "ЗапоминатьТекущуюИерархию"),
				УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "НеПоказыватьНулевыеОстатки"));
	//+++ Дмгг, 23.01.24, (ONESOFT-2840)
	
	СтруктураОтвета.Вставить("ПоказыватьИзображение", Истина); //Малышев 2024-10-31 ONESOFT-3567
...
Процедура УстановитьТекстЗапросаСпискаЗапасов(СтруктураНастроек)
	...
	|	НоменклатураСписок.СтавкаНДС КАК СтавкаНДС, //Малышев 2024-10-31 ONESOFT-3567
	|	НоменклатураСписок.СтавкаНДС.Ставка КАК СтавкаНДСЗначение, //Малышев 2024-10-31 ONESOFT-3567
	...
	Если ЗапросСОстатками Тогда
		...
		Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
			...
			|				Организация = &Организация
			|					И (НЕ Номенклатура.ЭтоГруппа)
			|					И &УсловиеОтбораПоСкладуРезервированиеЗапасов
			|					И &УсловиеОтбораПоТипамНоменклатурыРезервированиеЗапасов
			|					И &УсловиеОтбораПоСтатусуПартииРезервированиеЗапасов) КАК РезервированиеЗапасовОстатки
			|		ПО НоменклатураСписок.Ссылка = РезервированиеЗапасовОстатки.Номенклатура";
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
				"&УсловиеОтбораПоСкладуРезервированиеЗапасов",
				?(НЕ (Склад = НЕОПРЕДЕЛЕНО ИЛИ Склад = Справочники.СтруктурныеЕдиницы.ПустаяСсылка()), "СтруктурнаяЕдиница = &СтруктурнаяЕдиница", "ИСТИНА"));
				
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
				"&УсловиеОтбораПоТипамНоменклатурыРезервированиеЗапасов",
				?(ЗапросСТипамиНоменклатуры, "Номенклатура.ТипНоменклатуры В (&ТипНоменклатуры)", "ИСТИНА"));
				
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
				"&УсловиеОтбораПоСтатусуПартииРезервированиеЗапасов",
				?(ЗапросСоСтатусамиПартий, "(Партия = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяССылка) ИЛИ Партия.Статус В (&СтатусПартии))", "ИСТИНА"));
	...
	|	И &УсловиеОтбораПоТипамНоменклатуры
	//+++ Дмгг, 23.01.24, (ONESOFT-2840)
	|	И &УсловиеНеПоказыватьНулевыеОстатки
	//--- Дмгг, 23.01.24, (ONESOFT-2840)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		КОНЕЦ + ЕСТЬNULL(РезервированиеЗапасовОстатки.КоличествоОстаток,0)) ", "0"));
	...
	//+++ Дмгг, 23.01.24, (ONESOFT-2840)  
	Если ЗапросСОстатками И НЕ ЗапросПоСкладу Тогда    
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеНеПоказыватьНулевыеОстатки", ?(ЗапросСОстатками, "	
		|	ВЫБОР
		|		КОГДА &НеПоказыватьНулевыеОстатки
		|			ТОГДА ЗапасыОстатки.КоличествоОстаток <> 0 ИЛИ ВЫБОР
		|				КОГДА ЗапасыОстатки.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяССылка)
		|					ТОГДА 0
		|				ИНАЧЕ ЗапасыОстатки.КоличествоОстаток
		|			КОНЕЦ <> 0
	...
	УстановитьПараметрДинамическогоСписка(СписокЗапасов, "НеПоказыватьНулевыеОстатки", СтруктураНастроек.НеПоказыватьНулевыеОстатки);
...
Процедура УстановитьТекстЗапросаСпискаХарактеристик(СтруктураНастроек)
	...
	Если ЗапросСОстатками Тогда
		...
		Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
			...
			|					И &УсловиеОтбораПоТипамНоменклатурыРезервированиеЗапасов
			|					И &УсловиеОтбораПоСтатусуПартииРезервированиеЗапасов) КАК РезервированиеЗапасовОстатки
			|		ПО ЗапасыОстатки.Номенклатура = РезервированиеЗапасовОстатки.Номенклатура
			|			И ЗапасыОстатки.Характеристика = РезервированиеЗапасовОстатки.Характеристика
			|			И ЗапасыОстатки.Партия = РезервированиеЗапасовОстатки.Партия";
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
				...
				|				ТОГДА 0
				|			ИНАЧЕ ЗапасыОстатки.КоличествоОстаток
				|		КОНЕЦ + ЕСТЬNULL(РезервированиеЗапасовОстатки.КоличествоОстаток,0)"));
...
Процедура ПодборСписокЗапасовПриАктивизацииСтрокиОбработчикОжидания()
	...
	Если (СокрЛП(СтруктураНастроек.СпособВыводаОстатков) = "Отдельно" 
			ИЛИ СокрЛП(СтруктураНастроек.СпособВыводаОстатков) = "ცალკე")
		И 
...
Функция ПолучитьСтруктуруОстатковИЦенуПоТовару(НаДату, Номенклатура, Характеристика, Партия, ВидЦен, СводноПоХарактеристикам)
	...
	| //&УсловиеРезервированиеЗапасов_new
	|ГДЕ
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, 
			...
			?(ИспользуютсяЦеныКонтрагентов, "ВидЦенКонтрагента", "ВидЦен"));
	// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
	Если НЕ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда		
	// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
	Запрос.Текст = СтрЗаменить(Запрос.Текст, 
	...
	// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"//&УсловиеРезервированиеЗапасов_new","");	
	Иначе
		
		ТекстЗапросаРезервированиеЗапасов = "	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РезервированиеЗапасов.Остатки(
		|				,
		|				Номенклатура = &Номенклатура
		|				И Организация = &Организация
		|					И &УсловиеОтбораПоСкладуРезервированиеЗапасов
		|					И &УсловиеОтбораПоХарактеристикамРезервированиеЗапасов
		|					И &УсловиеОтбораПоСтатусамПартийРезервированиеЗапасов
		|					И &УсловиеОтбораПоСтатусуОднойПартииРезервированиеЗапасов) КАК РезервированиеЗапасовОстатки
		|		ПО НоменклатураСписок.Ссылка = РезервированиеЗапасовОстатки.Номенклатура";
		
		ТекстЗапросаРезервированиеЗапасов = СтрЗаменить(ТекстЗапросаРезервированиеЗапасов, 
			"&УсловиеОтбораПоСкладуРезервированиеЗапасов",
			?(НЕ (Склад = НЕОПРЕДЕЛЕНО ИЛИ Склад = Справочники.СтруктурныеЕдиницы.ПустаяСсылка()), "СтруктурнаяЕдиница = &СтруктурнаяЕдиница", "ИСТИНА"));
			
		Если СводноПоХарактеристикам Тогда
		
			ТекстЗапросаРезервированиеЗапасов = СтрЗаменить(ТекстЗапросаРезервированиеЗапасов, "&УсловиеОтбораПоХарактеристикамРезервированиеЗапасов", "Истина");
			
		Иначе
			
			ТекстЗапросаРезервированиеЗапасов = СтрЗаменить(ТекстЗапросаРезервированиеЗапасов, 
					"&УсловиеОтбораПоХарактеристикамРезервированиеЗапасов",
					?(Характеристика = Неопределено, "Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)", "Характеристика = &Характеристика"));
			
		КонецЕсли;
			
		ТекстЗапросаРезервированиеЗапасов = СтрЗаменить(ТекстЗапросаРезервированиеЗапасов, 
			"&УсловиеОтбораПоСтатусамПартийРезервированиеЗапасов", 
			?(СтатусПартии.Количество() > 0, "(Партия = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяССылка) ИЛИ Партия.Статус В (&СтатусПартии))", "ИСТИНА"));
			
		ТекстЗапросаРезервированиеЗапасов = СтрЗаменить(ТекстЗапросаРезервированиеЗапасов, 
			"&УсловиеОтбораПоСтатусуОднойПартииРезервированиеЗапасов", 
			?(НЕ Партия = Неопределено, "Партия = &Партия", "ИСТИНА"));
			
		// Вставляем в запрос
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"//&УсловиеРезервированиеЗапасов_new",ТекстЗапросаРезервированиеЗапасов);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() 
		// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
		ИЛИ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() 
		// Kuzmich, 28.10.2024, (ONESOFT-3520) Подбор номенклатуры в документах реализаций и подбор (две обработки)
		Тогда
	...
	Если ЗначениеЗаполнено(Валюта) Тогда
		...
		СтрокаНовогоЗаголовка 						= Нстр("ka='გადარჩეულია შემდეგი თანხის საქონელი (%ВВалюте%)';ru='Товаров подобрано на сумму (%ВВалюте%)';en='Products matched to the amount (% VValyute%)'");
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Иначе
		...
		Сообщение.Текст 		= НСтр("ka='ფორმის მონაცემთა განსაახლებლად შეავსეთ ორგანიზაცია';ru='Для обновление данных формы заполните организацию';en='To update data forms filled out organization'");
...
Процедура СписокЗапасовПриАктивизацииСтроки(Элемент)
	...
	ПодключитьОбработчикОжидания("ЗаполнитьКартинкуНоменклатуры", 1, Истина); //Малышев 2024-10-29 ONESOFT-3567
	//_Паганини
...
Процедура ЗаполнитьКартинкуНоменклатуры()
	ТекущиеДанные = Элементы.СписокЗапасов.ТекущиеДанные;
	Если НЕ ТекущиеДанные = Неопределено Тогда
		ЗаполнитьКартинкуНоменклатуры_НаСервере(ТекущиеДанные.Номенклатура);
	КонецЕсли;
КонецПроцедуры
...
// Kuzmich, 06.11.2024, (ONESOFT-3530) существующие константа: Не резервировать в форме подбора  - должно быть исправить также для новом функционале резервиорвание
&НаСервере
Функция Константы_НеРезервироватьВФормеПодбора_Получить()

	Возврат Константы.НеРезервироватьВФормеПодбора.Получить(); 
