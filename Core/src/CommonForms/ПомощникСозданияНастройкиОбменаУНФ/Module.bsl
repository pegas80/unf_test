...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = Нстр("ka='ბანკების სიის განახლება, რომელთა მიერ მხარდაჭერილია გაცვლა DirectBank';ru='Обновление списка банков, поддерживающих обмен DirectBank';en='Updating the list of banks that support the exchange of DirectBank'");
...
"Обратитесь к администратору';ka='თქვენ არ გაქვთ ბანკთან პირდაპირი გაცვლის გასწორების უფლებები."
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ КлассификаторЗаполнен Тогда
		Состояние(
		НСтр("ka='ბანკების კლასიფიკატორის განახლება';ru='Обновление классификатора банков';en='Update classifier banks'"),,,
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ЗакрытьФорму 
		...
		Отказ = Истина;
		ТекстВопроса = НСтр("ka='DirectBank-ი არ არის შეერთებული."
"შევწყვიტოთ დამხმარეს მუშაობა?';ru='DirectBank не подключен."
...
Процедура ПутьКФайлуКомпонентыСбербанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		...
		Фильтр = НСтр("ka='გარე მოდული';ru='Внешний модуль';en='external module'") + "(*.zip)|*.zip";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = Нстр("ka='აირჩიეთ ბანკის გარე მოდულის ფაილი';ru='Выберите файл внешнего модуля банка';en='Select external bank module file'");
...
Процедура ПутьКФайлуСертификатаСбербанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='ბანკის სერტიფიკატი';ru='Сертификат банка';en='bank certificate'") + "(*.cer)|*.cer";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ბანკის სერთიფიკატის ფაილი';ru='Выберите файл сертификата банка';en='Select Bank certificate file'");
...
Процедура ПутьКФайлуВнешнегоМодуляНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='გარე მოდული';ru='Внешний модуль';en='external module'") + "(*.epf)|*.epf";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ გარე მოდულის ფაილი';ru='Выберите файл внешнего модуля';en='Select an external module file'");
...
Процедура ПутьКФайлуНастроекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='გაცვლის გამართვების ფაილი';ru='Файл настроек обмена';en='File sharing settings'") + "(*.xml)|*.xml";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = Нстр("ka='აირჩიეთ გაცვლის გამართვების ფაილი';ru='Выберите файл настроек обмена';en='Select the file sharing settings'");
...
Процедура Готово(Команда)
	...
	Если Элементы.Шаги.ТекущаяСтраница=Элементы.СтраницаДобавлениеСчета Тогда
		...
		Если Не ЗначениеЗаполнено(НомерСчета) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მიუთითეთ ანგარიშის ნომერი';ru='Укажите номер счета';en='Enter the account number'"), , "НомерСчета", , Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მიუთითეთ ანგარიშის ვალუტა';ru='Укажите валюту счета';en='Specify the account currency'"), , "ВалютаДенежныхСредств", , Отказ);
		КонецЕсли;
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
...
Процедура СледующийШаг()
	...
	Если Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборОрганизацииИБанка Тогда
		...
		Если Не ЗначениеЗаполнено(Организация) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ ორგანიზაცია';ru='Выберите организацию';en='Select the organization'"), , "Организация", , Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Банк) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ ბანკი';ru='Выберите банк';en='Select Bank'"), , "Банк", , Отказ);
		КонецЕсли;
		...
		Если ИспользуетсяВнешнийМодуль Тогда
			...
			#Если ВебКлиент Тогда
				Если Не ВыбранФайлВнешнегоМодуля Тогда
					ТекстСообщения = НСтр("ka='გარე მოდულის ფაილი არ არის არჩეული';ru='Не выбран файл внешнего модуля';en='Not selected external module file'");
			...
			#Иначе
				Если ЗначениеЗаполнено(ПутьКФайлуВнешнегоМодуля) Тогда
					...
					Исключение
						ТекстСообщения = НСтр("ka='გარე მოდულის ფაილისკენ არსებული გზა არასწორად არის მითითებული';ru='Неверно указан путь к файлу внешнего модуля';en='Invalid path to the external module'");
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВнешнегоМодуля");
						Возврат;
					КонецПопытки;
...
"Файл внешнего модуля должен иметь расширение ""epf""';ka='მითითებულია არასწორი ფაილი."
"გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""epf""'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВнешнегоМодуля");
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаАсинхронныйОбмен Тогда
		...
		Если Не ЗначениеЗаполнено(АдресСервера) Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერის მისამართი არ არის მითითებული';ru='Не указан адрес сервера банка';en='Unknown address of the bank server'");
		...
		Если ЗначениеЗаполнено(АдресСервера) И НЕ ОбменСБанкамиСлужебныйКлиент.ПравильныйФорматАдреса(АдресСервера) Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერის მისამართი შემდეგნაირად უნდა იწყებოდეს ""https://"" или ""http://""';ru='Адрес сервера банка должен начинаться с ""https://"" или ""http://""';en='the bank''s server address should start with ""https: //"" or ""http: //""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "АдресСервера", , Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			ТекстСообщения = НСтр("ka='ორგანიზაციის იდენტიფიკატორი არ არის მითითებული';ru='Не указан идентификатор организации';en='ID Unknown Organization'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСинхронныйОбмен Тогда
		...
		Если Не ЗначениеЗаполнено(РесурсПриемник) Тогда
			ТекстСообщения = НСтр("ka='გასაგზავნად რესურსი არ არის მითითებული';ru='Не указан ресурс для отправки';en='Unknown resource for sending'");
		...
		Если Не ЗначениеЗаполнено(РесурсИсточник) Тогда
			ТекстСообщения = НСтр("ka='მისაღებად რესურსი არ არის მითითებული';ru='Не указан ресурс для получения';en='Unknown resource for'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборСпособаАутентификации Тогда
		...
		Если СпособАутентификации = 1 И НЕ ЗначениеЗаполнено(Логин) Тогда
			ТекстСообщения = НСтр("ka='შეიყვანეთ ლოგინი';ru='Введите логин';en='Enter login'");
		...
		ИначеЕсли СпособАутентификации = 0 И НЕ ЗначениеЗаполнено(СертификатАутентификации) Тогда
			ТекстСообщения = НСтр("ka='აირჩიეთ სერტიფიკატი';ru='Выберите сертификат';en='Select a certificate'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСертификаты Тогда
		Если ИспользуетсяКриптография И НЕ Сертификаты.Количество() Тогда
			ТекстСообщения = НСтр("ka='დაამატეთ ელექტრონული ხელმოწერის სერთიფიკატები';ru='Добавьте сертификаты электронной подписи';en='Add an electronic signature certificates'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСбербанк Тогда
		...
		#Если ВебКлиент Тогда
			Если Не ВыбранФайлСертификатаСбербанка Тогда
				ТекстСообщения = НСтр("ka='ბანკის სერთიფიკატი არ არის არჩეული';ru='Не выбран сертификат банка';en='Do not choose a bank certificate'");
		...
		#Иначе
			Если ЗначениеЗаполнено(ПутьКФайлуСертификатаСбербанка) Тогда
				...
				Исключение
					ТекстСообщения = НСтр("ka='ბანკის სერთიფიკატის ფაილისკენ არსებული გზა არასწორად არის მითითებული';ru='Неверно указан путь к файлу сертификата банка';en=""Invalid path to the bank's certificate""");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуСертификатаСбербанка");
					Возврат;
				КонецПопытки;
				АдресФайлаСертификатаСбербанка = ПоместитьВоВременноеХранилище(ДвоичныеДанныеСертификатаСбербанка, УникальныйИдентификатор);
			Иначе
				ТекстСообщения = НСтр("ka='ბანკის სერთიფიკატის ფაილი არ არის არჩეული';ru='Не выбран файл сертификата банка';en='Do not choose a bank certificate file'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуСертификатаСбербанка");
				Возврат;
			КонецЕсли;
			...
			Иначе
				ТекстСообщения = НСтр("ka='ბანკის გარე მოდულის ფაილი არ არის არჩეული';ru='Не выбран файл внешнего модуля банка';en='Not selected external bank module file'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВКСбербанка");
				Возврат;
			КонецЕсли;
		#КонецЕсли
		...
		Если Не Файл.Расширение = ".cer" Тогда
			ТекстСообщения = НСтр("ru='Указан неверный файл."
"Файл сертификата банка должен иметь расширение ""cer""';ka='მითითებულია არასწორი ფაილი. ბანკის სერთიფიკატის ფაილს უნდა ჰქონდეს გაფართოება ""cer""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,"ПутьКФайлуСертификатаСбербанка");
			Возврат;
		КонецЕсли;
		...
		Если Не Файл.Расширение = ".zip" Тогда
			ТекстСообщения = НСтр("ru='Указан неверный файл."
"Файл внешнего модуля должен иметь расширение ""zip""';ka='მითითებულია არასწორი ფაილი."
"გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""zip""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,"ПутьКФайлуВКСбербанка");
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера И (СпособАутентификации=0 ИЛИ СпособАутентификации=1) Тогда
		...
		Если НЕ ЗначениеЗаполнено(АдресСервераНастроек) Тогда
			ТекстСообщения = НСтр("ka='მიუთითეთ ბანკის სერვერის მისამართი';ru='Укажите адрес сервера банка';en='Enter the address of the bank server'");
		...
		Если СпособАутентификации = 0 И Не ЗначениеЗаполнено(СертификатАутентификации) Тогда  // по сертификату
			ТекстСообщения = НСтр("ka='აირჩიეთ სერთიფიკატი ბანკის სერვერზე ავთენტიფიკაციისთვის';ru='Выберите сертификат для аутентификации на сервере банка';en='Select a certificate to authenticate to the server of the bank'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "СертификатАутентификации", , Отказ);
		КонецЕсли;
		
		Если СпособАутентификации = 1 И Не ЗначениеЗаполнено(Логин) Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერზე ავთენტიფიკაციისთვის შეიყვანეთ ლოგ-ინი';ru='Введите логин для аутентификации на сервере банка';en='Enter your username for authentication on the server of the bank'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера И СпособАутентификации=2 Тогда
		...
		#Если ВебКлиент Тогда
			Если Не ВыбранФайлНастроекОбмена Тогда
				ТекстСообщения = НСтр("ka='გაცვლის გამართვების ფაილი არ არის არჩეული';ru='Не выбран файл настроек обмена';en='Not selected file sharing settings'");
		...
		#Иначе
			Если ЗначениеЗаполнено(ПутьКФайлуНастроекОбмена) Тогда
				...
				Исключение
					ТекстСообщения = НСтр("ka='გაცვლის გამართვების ფაილისკენ არსებული გზა არასწორად არის მითითებული';ru='Неверно указан путь к файлу настроек обмена';en='Invalid path to the file sharing settings'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуНастроекОбмена");
					Возврат;
				КонецПопытки;
		...
		Если Не Файл.Расширение = ".xml" Тогда
			ТекстСообщения = НСтр("ru='Указан неверный файл."
"Файл настроек должен иметь расширение ""xml""';ka='მითითებულია არასწორი ფაილი."
"გამართვების ფაილს უნდა ჰქონდეს გაფართოება ""xml""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуНастроекОбмена");
			Возврат;
		КонецЕсли;
...
Процедура СменитьСтраницуШаги(НоваяСтраница)
	...
	ИначеЕсли НоваяСтраница = Элементы.СтраницаПолучениеССервера Тогда
		...
		Если НЕ ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
			Элементы.СпособАутентификации.СписокВыбора.Добавить(1, НСтр("ka='სახელი და პაროლი';ru='Логин и пароль';en='Login and password'"));
			Элементы.СпособАутентификации.СписокВыбора.Добавить(0, НСтр("ka='სერთიფიკატი';ru='Сертификат';en='Certificate'"));
		КонецЕсли; 
		Элементы.СпособАутентификации.СписокВыбора.Добавить(2, НСтр("ka='გამართვები ფაილიდან';ru='Настройки из файла';en='Settings from the file'"));
...
Процедура ПродолжитьТестНастроекАсинхронныйОбменПослеПолученияОтпечатков(ДанныеОтпечатков, ДополнительныеПараметры) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		...
		Если НастройкиОбмена.ДоступныеСертификаты.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='ხელმოწერის შესაბამისი სერთიფიკატი ვერ მოიძებნა."
"უნდა დარწმუნდეთ იმაში, რომ გაცვლის გამართვაში მითითებული სერთიფიკატები დაყენებულია საოპერაციო სისტემის სერთიფიკატების პირად საცავში';ru='Не найден подходящий сертификат подписи."
...
Функция ЗагрузитьНастройкуОбменаИзФайлаНаСервере(АдресФайлаНастроекОбмена)
	...
	Попытка
		...
		НазваниеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლა: გაცვლის გამართვების ჩატვირთვა ფაილიდან';ru='Прямой обмен с банком: Загрузка настроек обмена из файла';en='Direct exchange with the bank: Loading sharing settings from a file'");
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно = Ложь;
		Текст = НСтр("ka='გაცვლის გამართვების ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось загрузить настройки обмена.';en='Could not load sharing settings.'");
		ВидОперации = НСтр("ka='გაცვლის გამართვების ჩატვირთვა ფაილიდან';ru='Загрузка настроек обмена из файла';en='Loading sharing settings from a file'");
...
Процедура ПолучитьИдентификаторСессииПослеПолученияИнформацииОМенеджереКриптографии(ИнформацияМодуля, ДополнительныеПараметры) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка';en='Authentication on the bank server'"));
...
Функция ЗагрузитьНастройкиОбменаНаСервере(Организация, Банк, ИдентификаторСессии, АдресФайлаНастроекОбмена)
	...
	Попытка
		НазваниеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლა: გაცვლის გამართვების მიღება ბანკის სერვერიდან';ru='Прямой обмен с банком: Получение настроек обмена с сервера банка';en='Direct exchange with the Bank: Getting sharing settings from the bank server'");
	...
	Исключение
		...
		Текст = НСтр("ka='ბანკის სერვერიდან გაცვლის გამართვების ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось загрузить настройки обмена с сервера банка.';en='Could not load sharing settings from the bank server.'");
		ВидОперации = НСтр("ka='გაცვლის გამართვების მიღება ბანკის სერვერიდან';ru='Получение настроек обмена с сервера банка';en='Getting sharing settings from the bank server'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, Текст, 1);
	КонецПопытки;
...
Процедура ВыбратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Если ЗначениеЗаполнено(СтруктураСертификата) Тогда
			ШаблонПредставления = Нстр("ka='%1, %2-მდე';ru='%1, до %2';en='1% to 2%'");
	...
	ЗаголовокФормыВыбора = НСтр("ka='აირჩიეთ ხელმოწერის დამატებადი სერთიფიკატი';ru='Выберите добавляемый сертификат подписи';en='Select the added certificate signature'");
...
"Код ошибки: %1"
"%2';ka='ხელმოწერის დაყენების შეცდომა."
"შეცდომის კოდი: %1"
"%2'");
...
"Код ошибки: %1"
"%2';ka='ხელმოწერის შემოწმების შეცდომა."
"შეცდომის კოდი: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"%2'");
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								ШаблонОшибки, ДеталиОшибки.Код, ДеталиОшибки.Сообщение);
			Операция = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის დადგენა';ru='Определение необходимости расширенной аутентификации';en='Determine if you need advanced authentication'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, , 1);
			ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
			ВыполнитьОбработкуОповещения(Оповещение, Ложь);
		...
		Если ТребуетсяРасширеннаяАутентификация Тогда
			Если ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ka='ბანკთან შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения с банком.';en='Error connection with the bank.'");
				ТекстОшибки = НСтр("ka='გაფართოებული ავთენტიფიკაციის მეთოდები არ არის დადგენილი.';ru='Не определены способы расширенной аутентификации.';en='Not defined Extensible Authentication methods.'");
				Операция = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის დადგენა';ru='Определение необходимости расширенной аутентификации';en='Determine if you need advanced authentication'");
			...
			Если НЕ ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
				ТекстСообщения = НСтр("ka='ბანკთან შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения с банком.';en='Error connection with the bank.'");
				ТекстОшибки =НСтр("ka='გაფართოებული ავთენტიფიკაცია SMS-ით არ არის მხარდაჭერილი.';ru='Расширенная аутентификация по SMS не поддерживается.';en='Extended authentication via SMS is not supported.'");
				Операция = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის დადგენა';ru='Определение необходимости расширенной аутентификации';en='Determine if you need advanced authentication'");
				ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
				ВыполнитьОбработкуОповещения(Оповещение, Ложь);
			...
			Иначе
				ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა';ru='Ошибка установки соединения';en='Connection Setup Error'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				ВыполнитьОбработкуОповещения(Оповещение, Ложь);
			КонецЕсли;
	КонецЕсли;
...
"Код ошибки: %1"
"%2';ka='SMS-ით ავთენტიფიკაციის გაშვების შეცდომა. შეცდომის კოდი: %1 "
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОшибки, ДеталиОшибки.Код, ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='ერთჯერადი პაროლის მოთხოვნა SMS-ით';ru='Запрос одноразового пароля по SMS';en='Request one-time password via SMS'");
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, , 1);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура ПослеВводаОдноразовогоПароляЧерезДополнительнуюОбработку(ОдноразовыйПароль, ДополнительныеПараметры) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='ერთჯერადი პაროლის გაგზავნა SMS-დან ბანკში';ru='Отправка одноразового пароля из SMS в банк';en='Sending of SMS one-time password to the bank'");
		ТекстСообщения = НСтр("ru='Ошибка аутентификации.';en='Authentication failed.'");
...
Функция ПодключитьДополнительнуюОбработку()
	...
	#Иначе
		...
		Исключение
			Операция = НСтр("ka='გარე დამუშავების ფორმის მიღება.';ru='Получение формы внешней обработки.';en='Preparation forms of external processing.'");
			ТекстСообщения = НСтр("ru='Не удалось подключить указанный внешний модуль';en='Failed to connect said external unit'");
			ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
...
Процедура НачатьИнициализациюДополнительнойОбработки(Оповещение)
	...
	Если ЗначениеЗаполнено(ВерсияАктуальнойОбработки) Тогда
		ТекстВопроса = НСтр("ka='საინფორმაციო ბაზაში უკვე არის ვერსიის მოდული %1."
"არჩეულ მოდულს სხვა ვერსია აქვს %2."
"ეხლა, რომელი ვერსიის მოდულის გამოყენება არის საჭირო?';ru='В информационной базе уже есть модуль версии %1."
...
"Модуль какой версии нужно теперь использовать?'");
		...
		ЗаголовокВопроса = НСтр("ka='მოდულის არჩევა გამოსაყენებლად';ru='Выбор модуля для использования';en='Selecting the module to be used'");
...
"Код ошибки: %1"
"%2';ka='გარე მოდულის ინიციალიზაციის შეცდომა."
"შეცდომის კოდი: %1"
"%2'");
			ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Код ошибки: %1"
"%2';ka='გარე მოდულის ინიციალიზაციის დასრულების შეცდომა."
"შეცდომის კოდი: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПослеОшибкиУстановкиВнешнейКомпонентыДополнительногоМодуля(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='დამატებითი დამუშავების გარე კომპონენტის დაყენება';ru='Установка внешней компоненты дополнительной обработки';en='Installing external components further processing'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ТекстСообщения = НСтр("ka='გარე მოდულის შეერთებისას შეცდომა მოხდა.';ru='При подключении внешнего модуля произошла ошибка.';en='When connecting an external module error occurred.'");
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеИнициализацииДополнительнойОбработки, Ложь);
...
Процедура ТестНастройкиОбменаЧерезВнешнююКомпоненту(Обработчик)
	...
	Исключение
		ВидОперации = НСтр("ka='ტექსტური მოთხოვნის გაგზავნა';ru='Отправка текстового запроса';en='Sending a text query'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკში სატესტო შეტყობინების გაგზავნისას.';ru='Ошибка при отправке тестового сообщения в банк.';en='An error occurred while sending a test message to the bank.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
...
Функция ЗагрузитьКомпонентуНаСервере(Знач УникальныйИдентификатор, Знач АдресСервера)
	...
	Попытка
		НазваниеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლა: გარე კომპონენტის მიღება';ru='Прямой обмен с банком: Получение внешней компоненты';en='Direct exchange with the Bank: Getting external components'");
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
	...
	Исключение
		...
		Задание.Успешно = Ложь;
		Задание.Значение = Новый Структура;
		Текст = НСтр("ka='ბანკის სერვერიდან გარე მოდულის ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось загрузить внешний модуль с сервера банка.';en='Failed to load an external module to the bank server.'");
		ВидОперации = НСтр("ka='გარე მოდულის მიღება ბანკის სერვერიდან';ru='Получение внешнего модуля с сервера банка';en=""Obtaining external module with the bank's server""");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, Текст, 1);
	КонецПопытки;
...
Процедура ПолучитьХранилищаСертификатов(ОписаниеОповещения)
	...
	Попытка
		...
		Если НЕ Хранилища.Количество() Тогда
			ТекстСообщения = НСтр("ka='შეერთებული ელექტრონული გასაღებები, რომლებიც არ არის აღმოჩენილი."
"დარწმუნდით იმაში, რომ გასაღები შეერთებულია კომპიუტერზე და გაიმეორეთ მცდელობა';ru='Не обнаруженные подключенные электронные ключи."
"Убедитесь что ключ подключен к компьютеру и повторите попытку'");
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ka='გასაღებების მოწყობილობების სიის მიღება';ru='Получение списка устройств ключей';en='Retrieving key devices'");
		ТекстСообщения = НСтр("ka='შეცდომა გასაღებების მოწყობილობების სიის მიღებისას.';ru='Ошибка при получении списка устройств ключей.';en='Failed to obtain a list of key devices.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Неопределено);
	КонецПопытки;
...
Процедура ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения)
	...
	Исключение
		ВидОперации = НСтр("ka='შეცდომის შესახებ ინფორმაციის მიღება';ru='Получение информации об ошибке';en='Getting error information'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გარე მოდულთან მუშაობისას.';ru='Ошибка при работе с внешним модулем банка.';en='Error when working with an external unit of the bank.'");
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
	...
	ШаблонОшибки = ТекстСообщения + Символы.ПС + НСтр("ka='შეცდომის კოდი: %1"
"%2';ru='Код ошибки: %1"
"%2'");
...
Процедура УстановитьPINКлюча(Обработчик, ИдентификаторХранилища, Пин)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='შეცდომა PIN-კოდის დაყენებისას.';ru='Ошибка при установке PIN-кода.';en='Error when setting PIN-code.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Ложь);
	КонецПопытки;
...
Процедура ПолучитьСертификатыХранилища(Обработчик, ИдентификаторХранилища)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატების მიღება ბანკის გასაღებიდან';ru='Получение сертификатов с банковского ключа';en='Obtaining certificates from a bank key'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გასაღების სერთიფიკატების მიღებისას.';ru='Ошибка при получении сертификатов банковского ключа.';en='It errors getting a bank key certificates.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки;
...
Процедура АутентифицироватьсяНаЭлектронномКлюче(Оповещение, СертификатXML, Пароль)
	...
	Попытка
		...
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Исключение
		ВидОперации = НСтр("ka='აუთენტიფიკაცია';ru='Аутентификация';en='authentication'");
		ТекстСообщения = НСтр("ru='Ошибка аутентификации.';en='Authentication failed.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура УстановитьНастройкиПрокси(Обработчик)
	...
	Если ЗначениеЗаполнено(НастройкиПроксиСервера) Тогда
		...
		Исключение
			ВидОперации = НСтр("ka='პროქსი-სერვერის გამართვების დაყენება';ru='Установка настроек прокси-сервера';en='Setting the proxy server settings'");
			ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.';en='Connection installation failed.'");
			ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Обработчик, Ложь);
			Возврат;
		КонецПопытки;
...
Процедура ДополнитьСертификат(Обработчик, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატის დამატება';ru='Дополнение сертификата';en='certificate supplement'");
		ТекстСообщения = НСтр("ka='სერთიფიკატის ჩატვირთვის შეცდომა.';ru='Ошибка загрузки сертификата.';en='Error loading certificate.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки
...
Процедура УстановитьСоединениеЧерезВнешнююКомпоненту(Обработчик, Идентификатор, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ka='ბანკის სერვერთან შეერთების დაყენება';ru='Установка соединения с сервером банка';en='Establishing a connection with the bank server'");
		ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.';en='Connection installation failed.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Ложь);
		Возврат;
...
Процедура ПроверитьНеобходимостьРасширеннойАутентификации(Обработчик)
	...
	Попытка
		...
		ПараметрыРасширеннойАутентификации = ОбменСБанкамиСлужебныйКлиент.ДеСериализованныеДанные(
			ПараметрыРасширеннойАутентификацииXML);
	Исключение
		ВидОперации = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის შემოწმება';ru='Проверка необходимости расширенной аутентификации';en='Check the need for enhanced authentication'");
		ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.';en='Connection installation failed.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
		Возврат;
...
Процедура ПослеПроверкиРасширеннойАутентификации(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ТребуетсяРасширеннаяАутентификация Тогда
		Если Результат.ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='გაფართოებული ავთენტიფიკაციის მეთოდები არ არის დადგენილი.';ru='Не определены способы расширенной аутентификации.';en='Not defined Extensible Authentication methods.'"));
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
			Возврат;
		КонецЕсли;
		Если НЕ Результат.ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
			ТекстСообщения = НСтр("ka='გაფართოებული ავთენტიფიკაცია SMS-ით არ არის მხარდაჭერილი.';ru='Расширенная аутентификация по SMS не поддерживается.';en='Extended authentication via SMS is not supported.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
			Возврат;
		КонецЕсли;
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეერთების დაყენების შეცდომა';ru='Ошибка установки соединения';en='Connection Setup Error'"));
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
	КонецЕсли
...
Процедура ЗапроситьПарольПоSMSЧерезВнешнююКомпоненту(Оповещение, Сессия)
	...
	Попытка
		...
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Исключение
		ВидОперации = НСтр("ka='ერთჯერადი პაროლის მოთხოვნა SMS-ით';ru='Запрос одноразового пароля по SMS';en='Request one-time password via SMS'");
		ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.';en='Connection installation failed.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура ПолучитьДанныеСертификатаЧерезВнешнююКомпоненту(Обработчик, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატის მონაცემთა მიღება ბანკის გასაღებიდან';ru='Получение данных сертификата с банковского ключа';en='Obtain a certificate from the bank data key'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გასაღების სერთიფიკატის მონაცემთა მიღებისას.';ru='Ошибка при получении данных сертификата банковского ключа.';en='Error getting key certificate data bank.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки;
...
Процедура ПроверитьНеобходимостьУстановкиPIN(Обработчик, ИдентификаторХранилища)
	...
	Исключение
		ВидОперации = НСтр("ka='ბანკის გასაღების PIN-კოდის არსებობის დადგენა';ru='Определение наличия PIN-кода банковского ключа';en='Determining whether a bank key PIN-code'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გასაღებთან მუშაობისას.';ru='Ошибка при работе с банковским ключом.';en='Error when working with key banking.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки
...
Функция ПрограммыКриптографии()
	...
	Настройка.Представление       = НСтр("ka='კრიპტოპრო eToken CSP';ru='КриптоПро eToken CSP';en='Crypto eToken CSP'");
...
Процедура ЗапуститьСозданиеНастройкиОбмена(Знач СтруктураПараметров, Знач АдресХранилища, ИдентификаторЗадания, ЗаданиеВыполнено, ЕстьОшибка)
	...
	НаименованиеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლის გამართვის შექმნა';ru='Создание настройки прямого обмена с банком';en='Creation of the bank set up a direct exchange'");
...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере(ФоновоеЗаданиеИдентификатор)
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно   = Ложь;
		Текст = НСтр("ka='При выполнении операции произошла ошибка.';ru='При выполнении операции произошла ошибка.';en='In an operation error.'");
		ВидОперации = НСтр("ka='ფონური დავალება სერვერზე';ru='Фоновое задание на сервере';en='Background job on the server'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
