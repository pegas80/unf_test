...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = Нстр("ka='ბანკების სიის განახლება, რომელთა მიერ მხარდაჭერილია გაცვლა DirectBank';ru='Обновление списка банков, поддерживающих обмен DirectBank'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ КлассификаторЗаполнен Тогда
		Состояние(
		НСтр("ka='ბანკების კლასიფიკატორის განახლება';ru='Обновление классификатора банков'"),,,
...
Процедура ПутьКФайлуКомпонентыСбербанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		...
		Фильтр = НСтр("ka='გარე მოდული';ru='Внешний модуль'") + "(*.zip)|*.zip";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = Нстр("ka='აირჩიეთ ბანკის გარე მოდულის ფაილი';ru='Выберите файл внешнего модуля банка'");
...
Процедура ПутьКФайлуСертификатаСбербанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='ბანკის სერტიფიკატი';ru='Сертификат банка'") + "(*.cer)|*.cer";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ ბანკის სერთიფიკატის ფაილი';ru='Выберите файл сертификата банка'");
...
Процедура ПутьКФайлуВнешнегоМодуляНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='გარე მოდული';ru='Внешний модуль'") + "(*.epf)|*.epf";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ka='აირჩიეთ გარე მოდულის ფაილი';ru='Выберите файл внешнего модуля'");
...
Процедура ПутьКФайлуНастроекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ka='გაცვლის გამართვების ფაილი';ru='Файл настроек обмена'") + "(*.xml)|*.xml";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = Нстр("ka='აირჩიეთ გაცვლის გამართვების ფაილი';ru='Выберите файл настроек обмена'");
...
Процедура Готово(Команда)
	...
	Если Элементы.Шаги.ТекущаяСтраница=Элементы.СтраницаДобавлениеСчета Тогда
		...
		Если Не ЗначениеЗаполнено(НомерСчета) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მიუთითეთ ანგარიშის ნომერი';ru='Укажите номер счета'"), , "НомерСчета", , Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='მიუთითეთ ანგარიშის ვალუტა';ru='Укажите валюту счета'"), , "ВалютаДенежныхСредств", , Отказ);
		КонецЕсли;
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
...
Процедура СледующийШаг()
	...
	Если Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборОрганизацииИБанка Тогда
		...
		Если Не ЗначениеЗаполнено(Организация) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ ორგანიზაცია';ru='Выберите организацию'"), , "Организация", , Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Банк) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ ბანკი';ru='Выберите банк'"), , "Банк", , Отказ);
		КонецЕсли;
		...
		Если ИспользуетсяВнешнийМодуль Тогда
			...
			#Если ВебКлиент Тогда
				Если Не ВыбранФайлВнешнегоМодуля Тогда
					ТекстСообщения = НСтр("ka='გარე მოდულის ფაილი არ არის არჩეული';ru='Не выбран файл внешнего модуля'");
			...
			#Иначе
				Если ЗначениеЗаполнено(ПутьКФайлуВнешнегоМодуля) Тогда
					...
					Исключение
						ТекстСообщения = НСтр("ka='გარე მოდულის ფაილისკენ არსებული გზა არასწორად არის მითითებული';ru='Неверно указан путь к файлу внешнего модуля'");
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВнешнегоМодуля");
						Возврат;
					КонецПопытки;
...
"გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""epf""'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаАсинхронныйОбмен Тогда
		...
		Если Не ЗначениеЗаполнено(АдресСервера) Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერის მისამართი არ არის მითითებული';ru='Не указан адрес сервера банка'");
		...
		Если ЗначениеЗаполнено(АдресСервера) И НЕ ОбменСБанкамиСлужебныйКлиент.ПравильныйФорматАдреса(АдресСервера) Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერის მისამართი შემდეგნაირად უნდა იწყებოდეს ""https://"" или ""http://""';ru='Адрес сервера банка должен начинаться с ""https://"" или ""http://""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "АдресСервера", , Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			ТекстСообщения = НСтр("ka='ორგანიზაციის იდენტიფიკატორი არ არის მითითებული';ru='Не указан идентификатор организации'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСинхронныйОбмен Тогда
		...
		Если Не ЗначениеЗаполнено(РесурсПриемник) Тогда
			ТекстСообщения = НСтр("ka='გასაგზავნად რესურსი არ არის მითითებული';ru='Не указан ресурс для отправки'");
		...
		Если Не ЗначениеЗаполнено(РесурсИсточник) Тогда
			ТекстСообщения = НСтр("ka='მისაღებად რესურსი არ არის მითითებული';ru='Не указан ресурс для получения'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборСпособаАутентификации Тогда
		...
		Если СпособАутентификации = 1 И НЕ ЗначениеЗаполнено(Логин) Тогда
			ТекстСообщения = НСтр("ka='შეიყვანეთ ლოგინი';ru='Введите логин'");
		...
		ИначеЕсли СпособАутентификации = 0 И НЕ ЗначениеЗаполнено(СертификатАутентификации) Тогда
			ТекстСообщения = НСтр("ka='აირჩიეთ სერტიფიკატი';ru='Выберите сертификат'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСертификаты Тогда
		Если ИспользуетсяКриптография И НЕ Сертификаты.Количество() Тогда
			ТекстСообщения = НСтр("ka='დაამატეთ ელექტრონული ხელმოწერის სერთიფიკატები';ru='Добавьте сертификаты электронной подписи'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСбербанк Тогда
		...
		#Если ВебКлиент Тогда
			Если Не ВыбранФайлСертификатаСбербанка Тогда
				ТекстСообщения = НСтр("ka='ბანკის სერთიფიკატი არ არის არჩეული';ru='Не выбран сертификат банка'");
		...
		#Иначе
			Если ЗначениеЗаполнено(ПутьКФайлуСертификатаСбербанка) Тогда
				...
				Исключение
					ТекстСообщения = НСтр("ka='ბანკის სერთიფიკატის ფაილისკენ არსებული გზა არასწორად არის მითითებული';ru='Неверно указан путь к файлу сертификата банка'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуСертификатаСбербанка");
					Возврат;
				КонецПопытки;
				АдресФайлаСертификатаСбербанка = ПоместитьВоВременноеХранилище(ДвоичныеДанныеСертификатаСбербанка, УникальныйИдентификатор);
			Иначе
				ТекстСообщения = НСтр("ka='ბანკის სერთიფიკატის ფაილი არ არის არჩეული';ru='Не выбран файл сертификата банка'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуСертификатаСбербанка");
				Возврат;
			КонецЕсли;
			...
			Иначе
				ТекстСообщения = НСтр("ka='ბანკის გარე მოდულის ფაილი არ არის არჩეული';ru='Не выбран файл внешнего модуля банка'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВКСбербанка");
				Возврат;
			КонецЕсли;
		#КонецЕсли
...
"გარე მოდულის ფაილს უნდა ჰქონდეს გაფართოება ""zip""'");
		...
		Если НЕ ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
				"Поле", "Заполнение", НСтр("ka='ორგანიზაციის იდენტიფიკატორი';ru='Идентификатор организации'"));
		...
		Исключение
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
				"Поле", "Корректность", НСтр("ka='ორგანიზაციის იდენტიფიკატორი';ru='Идентификатор организации'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ИдентификаторОрганизации");
		КонецПопытки;
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера И (СпособАутентификации=0 ИЛИ СпособАутентификации=1) Тогда
		...
		Если НЕ ЗначениеЗаполнено(АдресСервераНастроек) Тогда
			ТекстСообщения = НСтр("ka='მიუთითეთ ბანკის სერვერის მისამართი';ru='Укажите адрес сервера банка'");
		...
		Если СпособАутентификации = 0 И Не ЗначениеЗаполнено(СертификатАутентификации) Тогда  // по сертификату
			ТекстСообщения = НСтр("ka='აირჩიეთ სერთიფიკატი ბანკის სერვერზე ავთენტიფიკაციისთვის';ru='Выберите сертификат для аутентификации на сервере банка'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "СертификатАутентификации", , Отказ);
		КонецЕсли;
		
		Если СпособАутентификации = 1 И Не ЗначениеЗаполнено(Логин) Тогда
			ТекстСообщения = НСтр("ka='ბანკის სერვერზე ავთენტიფიკაციისთვის შეიყვანეთ ლოგ-ინი';ru='Введите логин для аутентификации на сервере банка'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера И СпособАутентификации=2 Тогда
		...
		#Если ВебКлиент Тогда
			Если Не ВыбранФайлНастроекОбмена Тогда
				ТекстСообщения = НСтр("ka='გაცვლის გამართვების ფაილი არ არის არჩეული';ru='Не выбран файл настроек обмена'");
		...
		#Иначе
			Если ЗначениеЗаполнено(ПутьКФайлуНастроекОбмена) Тогда
				...
				Исключение
					ТекстСообщения = НСтр("ka='გაცვლის გამართვების ფაილისკენ არსებული გზა არასწორად არის მითითებული';ru='Неверно указан путь к файлу настроек обмена'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуНастроекОбмена");
					Возврат;
				КонецПопытки;
...
Процедура СменитьСтраницуШаги(НоваяСтраница)
	...
	ИначеЕсли НоваяСтраница = Элементы.СтраницаПолучениеССервера Тогда
		...
		Если НЕ ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
			Элементы.СпособАутентификации.СписокВыбора.Добавить(1, НСтр("ka='სახელი და პაროლი';ru='Логин и пароль'"));
			Элементы.СпособАутентификации.СписокВыбора.Добавить(0, НСтр("ka='სერთიფიკატი';ru='Сертификат'"));
		КонецЕсли; 
		Элементы.СпособАутентификации.СписокВыбора.Добавить(2, НСтр("ka='გამართვები ფაილიდან';ru='Настройки из файла'"));
...
Функция ЗагрузитьНастройкуОбменаИзФайлаНаСервере(АдресФайлаНастроекОбмена)
	...
	Попытка
		...
		НазваниеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლა: გაცვლის გამართვების ჩატვირთვა ფაილიდან';ru='Прямой обмен с банком: Загрузка настроек обмена из файла'");
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно = Ложь;
		Текст = НСтр("ka='გაცვლის გამართვების ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось загрузить настройки обмена.'");
		ВидОперации = НСтр("ka='გაცვლის გამართვების ჩატვირთვა ფაილიდან';ru='Загрузка настроек обмена из файла'");
...
Процедура ПолучитьИдентификаторСессииПослеПолученияИнформацииОМенеджереКриптографии(ИнформацияМодуля, ДополнительныеПараметры) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных", НСтр("ka='ავთენტიფიკაცია ბანკის სერვერზე';ru='Аутентификация на сервере банка'"));
...
Функция ЗагрузитьНастройкиОбменаНаСервере(Организация, Банк, ИдентификаторСессии, АдресФайлаНастроекОбмена)
	...
	Попытка
		НазваниеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლა: გაცვლის გამართვების მიღება ბანკის სერვერიდან';ru='Прямой обмен с банком: Получение настроек обмена с сервера банка'");
	...
	Исключение
		...
		Текст = НСтр("ka='ბანკის სერვერიდან გაცვლის გამართვების ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось загрузить настройки обмена с сервера банка.'");
		ВидОперации = НСтр("ka='გაცვლის გამართვების მიღება ბანკის სერვერიდან';ru='Получение настроек обмена с сервера банка'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, Текст, 1);
	КонецПопытки;
...
Процедура ВыбратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Если ЗначениеЗаполнено(СтруктураСертификата) Тогда
			ШаблонПредставления = Нстр("ka='%1, %2-მდე';ru='%1, до %2'");
	...
	ЗаголовокФормыВыбора = НСтр("ka='აირჩიეთ ხელმოწერის დამატებადი სერთიფიკატი';ru='Выберите добавляемый сертификат подписи'");
...
"%2'");
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								ШаблонОшибки, ДеталиОшибки.Код, ДеталиОшибки.Сообщение);
			Операция = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის დადგენა';ru='Определение необходимости расширенной аутентификации'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, , 1);
			ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
			ВыполнитьОбработкуОповещения(Оповещение, Ложь);
		...
		Если ТребуетсяРасширеннаяАутентификация Тогда
			Если ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ka='ბანკთან შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения с банком.'");
				ТекстОшибки = НСтр("ka='გაფართოებული ავთენტიფიკაციის მეთოდები არ არის დადგენილი.';ru='Не определены способы расширенной аутентификации.'");
				Операция = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის დადგენა';ru='Определение необходимости расширенной аутентификации'");
			...
			Если НЕ ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
				ТекстСообщения = НСтр("ka='ბანკთან შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения с банком.'");
				ТекстОшибки =НСтр("ka='გაფართოებული ავთენტიფიკაცია SMS-ით არ არის მხარდაჭერილი.';ru='Расширенная аутентификация по SMS не поддерживается.'");
				Операция = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის დადგენა';ru='Определение необходимости расширенной аутентификации'");
				ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
				ВыполнитьОбработкуОповещения(Оповещение, Ложь);
			...
			Иначе
				ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა';ru='Ошибка установки соединения'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				ВыполнитьОбработкуОповещения(Оповещение, Ложь);
			КонецЕсли;
	КонецЕсли;
...
"%2'");
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОшибки, ДеталиОшибки.Код, ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='ერთჯერადი პაროლის მოთხოვნა SMS-ით';ru='Запрос одноразового пароля по SMS'");
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, , 1);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура ПослеВводаОдноразовогоПароляЧерезДополнительнуюОбработку(ОдноразовыйПароль, ДополнительныеПараметры) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ka='ერთჯერადი პაროლის გაგზავნა SMS-დან ბანკში';ru='Отправка одноразового пароля из SMS в банк'");
...
Функция ПодключитьДополнительнуюОбработку()
	...
	#Иначе
		...
		Исключение
			Операция = НСтр("ka='გარე დამუშავების ფორმის მიღება.';ru='Получение формы внешней обработки.'");
...
"Модуль какой версии нужно теперь использовать?'");
		...
		ЗаголовокВопроса = НСтр("ka='მოდულის არჩევა გამოსაყენებლად';ru='Выбор модуля для использования'");
...
Процедура ПослеОшибкиУстановкиВнешнейКомпонентыДополнительногоМодуля(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ka='დამატებითი დამუშავების გარე კომპონენტის დაყენება';ru='Установка внешней компоненты дополнительной обработки'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ТекстСообщения = НСтр("ka='გარე მოდულის შეერთებისას შეცდომა მოხდა.';ru='При подключении внешнего модуля произошла ошибка.'");
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеИнициализацииДополнительнойОбработки, Ложь);
...
Процедура ТестНастройкиОбменаЧерезВнешнююКомпоненту(Обработчик)
	...
	Исключение
		ВидОперации = НСтр("ka='ტექსტური მოთხოვნის გაგზავნა';ru='Отправка текстового запроса'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკში სატესტო შეტყობინების გაგზავნისას.';ru='Ошибка при отправке тестового сообщения в банк.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
...
Функция ЗагрузитьКомпонентуНаСервере(Знач УникальныйИдентификатор, Знач АдресСервера)
	...
	Попытка
		НазваниеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლა: გარე კომპონენტის მიღება';ru='Прямой обмен с банком: Получение внешней компоненты'");
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
	...
	Исключение
		...
		Задание.Успешно = Ложь;
		Задание.Значение = Новый Структура;
		Текст = НСтр("ka='ბანკის სერვერიდან გარე მოდულის ჩატვირთვა ვერ მოხერხდა.';ru='Не удалось загрузить внешний модуль с сервера банка.'");
		ВидОперации = НСтр("ka='გარე მოდულის მიღება ბანკის სერვერიდან';ru='Получение внешнего модуля с сервера банка'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, Текст, 1);
	КонецПопытки;
...
"Убедитесь что ключ подключен к компьютеру и повторите попытку'");
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ka='გასაღებების მოწყობილობების სიის მიღება';ru='Получение списка устройств ключей'");
		ТекстСообщения = НСтр("ka='შეცდომა გასაღებების მოწყობილობების სიის მიღებისას.';ru='Ошибка при получении списка устройств ключей.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Неопределено);
	КонецПопытки;
...
Процедура ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения)
	...
	Исключение
		ВидОперации = НСтр("ka='შეცდომის შესახებ ინფორმაციის მიღება';ru='Получение информации об ошибке'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გარე მოდულთან მუშაობისას.';ru='Ошибка при работе с внешним модулем банка.'");
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
...
Процедура УстановитьPINКлюча(Обработчик, ИдентификаторХранилища, Пин)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ka='შეცდომა PIN-კოდის დაყენებისას.';ru='Ошибка при установке PIN-кода.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Ложь);
	КонецПопытки;
...
Процедура ПолучитьСертификатыХранилища(Обработчик, ИдентификаторХранилища)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატების მიღება ბანკის გასაღებიდან';ru='Получение сертификатов с банковского ключа'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გასაღების სერთიფიკატების მიღებისას.';ru='Ошибка при получении сертификатов банковского ключа.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки;
...
Процедура АутентифицироватьсяНаЭлектронномКлюче(Оповещение, СертификатXML, Пароль)
	...
	Попытка
		...
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Исключение
		ВидОперации = НСтр("ka='აუთენტიფიკაცია';ru='Аутентификация'");
		ТекстСообщения = НСтр("ru='Ошибка аутентификации.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура УстановитьНастройкиПрокси(Обработчик)
	...
	Если ЗначениеЗаполнено(НастройкиПроксиСервера) Тогда
		...
		Исключение
			ВидОперации = НСтр("ka='პროქსი-სერვერის გამართვების დაყენება';ru='Установка настроек прокси-сервера'");
			ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.'");
			ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Обработчик, Ложь);
			Возврат;
...
Процедура ДополнитьСертификат(Обработчик, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატის დამატება';ru='Дополнение сертификата'");
		ТекстСообщения = НСтр("ka='სერთიფიკატის ჩატვირთვის შეცდომა.';ru='Ошибка загрузки сертификата.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки
...
Процедура УстановитьСоединениеЧерезВнешнююКомпоненту(Обработчик, Идентификатор, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ka='ბანკის სერვერთან შეერთების დაყენება';ru='Установка соединения с сервером банка'");
		ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Ложь);
		Возврат;
...
Процедура ПроверитьНеобходимостьРасширеннойАутентификации(Обработчик)
	...
	Попытка
		...
		ПараметрыРасширеннойАутентификации = ОбменСБанкамиСлужебныйКлиент.ДеСериализованныеДанные(
			ПараметрыРасширеннойАутентификацииXML);
	Исключение
		ВидОперации = НСтр("ka='გაფართოებული ავთენტიფიკაციის აუცილებლობის შემოწმება';ru='Проверка необходимости расширенной аутентификации'");
		ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
		Возврат;
...
Процедура ПослеПроверкиРасширеннойАутентификации(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ТребуетсяРасширеннаяАутентификация Тогда
		Если Результат.ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='გაფართოებული ავთენტიფიკაციის მეთოდები არ არის დადგენილი.';ru='Не определены способы расширенной аутентификации.'"));
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
			Возврат;
		КонецЕсли;
		Если НЕ Результат.ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
			ТекстСообщения = НСтр("ka='გაფართოებული ავთენტიფიკაცია SMS-ით არ არის მხარდაჭერილი.';ru='Расширенная аутентификация по SMS не поддерживается.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
			Возврат;
		КонецЕсли;
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='შეერთების დაყენების შეცდომა';ru='Ошибка установки соединения'"));
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
	КонецЕсли
...
Процедура ЗапроситьПарольПоSMSЧерезВнешнююКомпоненту(Оповещение, Сессия)
	...
	Попытка
		...
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Исключение
		ВидОперации = НСтр("ka='ერთჯერადი პაროლის მოთხოვნა SMS-ით';ru='Запрос одноразового пароля по SMS'");
		ТекстСообщения = НСтр("ka='შეერთების დაყენების შეცდომა.';ru='Ошибка установки соединения.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура ПолучитьДанныеСертификатаЧерезВнешнююКомпоненту(Обработчик, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ka='სერთიფიკატის მონაცემთა მიღება ბანკის გასაღებიდან';ru='Получение данных сертификата с банковского ключа'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გასაღების სერთიფიკატის მონაცემთა მიღებისას.';ru='Ошибка при получении данных сертификата банковского ключа.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки;
...
Процедура ПроверитьНеобходимостьУстановкиPIN(Обработчик, ИдентификаторХранилища)
	...
	Исключение
		ВидОперации = НСтр("ka='ბანკის გასაღების PIN-კოდის არსებობის დადგენა';ru='Определение наличия PIN-кода банковского ключа'");
		ТекстСообщения = НСтр("ka='შეცდომა ბანკის გასაღებთან მუშაობისას.';ru='Ошибка при работе с банковским ключом.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки
...
Функция ПрограммыКриптографии()
	...
	Настройка.Представление       = НСтр("ka='კრიპტოპრო eToken CSP';ru='КриптоПро eToken CSP'");
...
Процедура ЗапуститьСозданиеНастройкиОбмена(Знач СтруктураПараметров, Знач АдресХранилища, ИдентификаторЗадания, ЗаданиеВыполнено, ЕстьОшибка)
	...
	НаименованиеЗадания = НСтр("ka='ბანკთან პირდაპირი გაცვლის გამართვის შექმნა';ru='Создание настройки прямого обмена с банком'");
...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере(ФоновоеЗаданиеИдентификатор)
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно   = Ложь;
		Текст = НСтр("ka='При выполнении операции произошла ошибка.';ru='При выполнении операции произошла ошибка.'");
		ВидОперации = НСтр("ka='ფონური დავალება სერვერზე';ru='Фоновое задание на сервере'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
