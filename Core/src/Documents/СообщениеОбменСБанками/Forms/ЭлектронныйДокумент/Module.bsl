...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Объект.НастройкаОбмена, "ПрограммаБанка, АутентификацияПоСертификату, ПредставлениеДокумента");
...
Процедура ОткрытьСертификат(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ТекстОшибки = НСтр("ka='დაყენებული ხელმოწერების სიაში აირჩიეთ სერთიფიკატი.';ru='Выберите сертификат в списке установленных подписей.';en='Select the certificate in the list of installed signatures.'");
...
Процедура ЗапроситьСостояниеЭД(Команда)
	...
	Если НЕ ЕстьПоддержкаВидаЭД(Объект.НастройкаОбмена, ВидЭДЗапросОСостоянии) Тогда
		ТекстСообщения = НСтр("ka='ბანკის მიერ მოცემულ ფუნქციონალს მხარდაჭერა არ გააჩნია';ru='Банк не поддерживает данный функционал';en='The bank does not support this functionality'");
	...
	Если ТребуетсяПодписьЗапроса Тогда
		...
		Иначе // Обмен через ВК
			...
			Иначе
				...
				ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа';en='The signing of the electronic document'");
...
Процедура ОтозватьЭД(Команда)
	...
	Если МожноОтозватьЭД(Объект.Ссылка) Тогда
		...
		ЗаголовокОкна = НСтр("ka='მიუთითეთ ელექტრონული დოკუმენტის გამოხმობის მიზეზი';ru='Укажите причину отзыва электронного документа';en='Enter a reason for withdrawing an electronic document'");
...
Процедура ИзменитьВидимостьДоступность()
	...
	Если НЕ ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписиЭД")
		...
		Элементы.КомандаУтвердитьЭД.Заголовок = НСтр("ka='დამტკიცება და გაგზავნა';ru='Утвердить и отправить';en='Approve and send'");
	КонецЕсли;
	...
	Если ПодготовленКОтправке Тогда
		КомандаПодписиОтправки.Заголовок = НСтр("ka='გაგზავნა';ru='Отправить';en='to send'");
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуЭП()
	...
	ШаблонЗаголовка = НСтр("ka='ხელმოწერები %1 და სტატუსები';ru='Подписи %1и статусы';en='The signature% 1 and Updates'");
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.Представление = ?(ТекСтрока.ПодписьВерна, НСтр("ka='სწორია';ru='Верна';en='Verne'"), НСтр("ka='არ არის სწორი';ru='Неверна';en='incorrectly'"))
			+" (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.Представление = НСтр("ka='არ არის შემოწმებული';ru='Не проверена';en='not tested'");
	КонецЕсли
...
"Код ошибки: ДО-%1"
"%2';ka='საგადახდო დავალების დადასტურების შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
			...
			Операция = НСтр("ka='საგადახდო დავალების დადასტურება';ru='Подтверждение платежного поручения';en='Confirmation of the payment order'");
		...
		Если Результат.Количество() = 0 Тогда
			ПоказатьОповещениеПользователя(НСтр("ka='დასადასტურებლად მონაცემები არ არის';ru='Нет данных для подтверждения';en='There is no data to confirm'"));
		КонецЕсли;
		
		Если Не ПустаяСтрока(Результат[0].ТекстОшибки) Тогда
			ТекстСообщения = НСтр("ka='შეცდომა საგადახდო დავალების დადასტურებისას:';ru='Ошибка при подтверждении платежного поручения:';en='Error when confirming the payment order:'") + " " + Результат[0].ТекстОшибки;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		ПоказатьОповещениеПользователя(НСтр("ka='დოკუმენტი დადასტურებულია';ru='Документ подтвержден';en='document confirmed'"));
	КонецЕсли;
...
"Код ошибки: ДО-%1"
"%2';ka='დადასტურების სესიის ინიციალიზაციის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='დადასტურების სესიის ინიციალიზაცია';ru='Инициализация сессии подтверждения';en='Initialize session confirmation'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, Объект.Ссылка);
		Возврат;
...
"Код ошибки: ДО-%1"
"%2';ka='საგადახდო დავალების დადასტურების კოდის გაგზავნის შეცდომა."
"შეცდომის კოდი: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='საგადახდო დავალების დადასტურების კოდის გაგზავნა';ru='Отправка кода подтверждения платежного поручения';en='Sending a confirmation code of payment order'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, Объект.Ссылка);
		Возврат;
	КонецПопытки;
...
Процедура ПослеПолученияОтпечатковЗапроситьСостояниеЭД(Отпечатки, ПараметрыОбработки) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов.';en='No certificates available.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция СформироватьЗапросНаОтзывЭД(Знач НастройкаОбмена, Знач СообщениеОбменаПлатежноеПоручение, Знач Причина)
	...
	НазваниеЭД = НСтр("ka='ელექტრონული დოკუმენტის გამოხმობის მოთხოვნა';ru='Запрос на отзыв электронного документа';en='Request for Review of an electronic document'");
...
Процедура СформироватьЗапросСостоянияЭД(Знач НастройкаОбмена, Знач СообщениеОбменаПлатежныйДокумент, Знач ТребуетсяПодпись, ЗапросСостоянияЭД, Знач МассивОтпечатковСертификатов = Неопределено, МассивСертификатов = Неопределено)
	...
	Если ТребуетсяПодпись Тогда
		...
		Если ТаблицаСертификатов.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='დოკუმენტ ""ელექტრონული დოკუმენტის მდგომარეობის მოთხოვნა""-სთვის ხელმოწერის შესაბამისი სერთიფიკატი ვერ მოიძებნა. შეამოწმეთ გაცვლის გამართვები სერვის 1C:დირექტბანკით.';ru='Не найден подходящий сертификат подписи для документа Запрос состояния электронного документа."
"Проверьте настройки обмена через сервис 1С:ДиректБанк.'");
		...
		Если МассивСертификатов.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ka='На компьютере не установлен ни один сертификат, указанный в настройке обмена."
"Установите сертификаты или обратитесь к администратору.';ru='На компьютере не установлен ни один сертификат, указанный в настройке обмена."
"Установите сертификаты или обратитесь к администратору.'");
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ШаблонСообщения = НСтр("ka='ბანკთან გაცვლის გამართვა %1 უმოქმედოა';ru='Настройка обмена с банком %1 недействительна';en='with the bank exchange Configuring% 1 is invalid'");
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			Операция = НСтр("ka='ელექტრონული დოკუმენტის ფორმირება';ru='Формирование электронного документа';en='Formation of an electronic document'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	НазваниеЭД = НСтр("ka='ელექტრონული დოკუმენტის მდგომარეობის მოთხოვნა';ru='Запрос состояния электронного документа';en='Query status of an electronic document'");
...
Процедура ОбработатьОшибкуПодтвержденияПлатежногоПорученияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='საგადახდო დავალების დადასტურებისას შეცდომა მოხდა.';ru='При подтверждении платежного поручения произошла ошибка.';en='Upon confirmation of payment order error.'");
	ВидОперации = НСтр("ka='საგადახდო დავალების დადასტურება.';ru='Подтверждение платежного поручения.';en='Confirmation of the payment order.'");
...
Процедура ОбработатьОшибкуОтправкиКодаПодтвержденияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='SMS-ით დადასტურების კოდის გაგზავნისას შეცდომა მოხდა.';ru='При отправке кода подтверждения через SMS произошла ошибка.';en='When sending a verification code via SMS error occurred.'");
	ВидОперации = НСтр("ka='საგადახდო დავალების დადასტურების კოდის გაგზავნა.';ru='Отправка кода подтверждения платежного поручения.';en='Sending a confirmation code of payment order.'");
	Оповещение = Новый ОписаниеОповещения("СообщитьОшибкуПользователю", ЭтотОбъект);
...
Процедура ОбработатьОшибкуНачалаПодтвержденияПлатежногоПоручения(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='გადახდის   დადასტურების დაწყებისას წარმოიშვა შეცდომა.';ru='При начале подтверждения платежного поручения произошла ошибка.';en='At the beginning of the confirmation of the payment order error occurred.'");
	ВидОперации = НСтр("ka='საგადახდო დავალების დადასტურების დასაწყისი.';ru='Начало подтверждения платежного поручения.';en='Start the confirmation of the payment order.'");
	Оповещение = Новый ОписаниеОповещения("СообщитьОшибкуПользователю", ЭтотОбъект);
