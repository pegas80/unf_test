...
Процедура ОткрытьСертификат(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ТекстОшибки = НСтр("ka='დაყენებული ხელმოწერების სიაში აირჩიეთ სერთიფიკატი.';ru='Выберите сертификат в списке установленных подписей.'");
...
Процедура ЗапроситьСостояниеЭД(Команда)
	...
	Если НЕ ЕстьПоддержкаВидаЭД(Объект.НастройкаОбмена, ВидЭДЗапросОСостоянии) Тогда
		ТекстСообщения = НСтр("ka='ბანკის მიერ მოცემულ ფუნქციონალს მხარდაჭერა არ გააჩნია';ru='Банк не поддерживает данный функционал'");
	...
	Если ТребуетсяПодписьЗапроса Тогда
		...
		Иначе // Обмен через ВК
			...
			Иначе
				...
				ВидОперации = НСтр("ka='ელექტრონული დოკუმენტის ხელმოწერა';ru='Подписание электронного документа'");
...
Процедура ОтозватьЭД(Команда)
	...
	Если МожноОтозватьЭД(Объект.Ссылка) Тогда
		...
		ЗаголовокОкна = НСтр("ka='მიუთითეთ ელექტრონული დოკუმენტის გამოხმობის მიზეზი';ru='Укажите причину отзыва электронного документа'");
...
Процедура ИзменитьВидимостьДоступность()
	...
	Если НЕ ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписиЭД")
		...
		Элементы.КомандаУтвердитьЭД.Заголовок = НСтр("ka='დამტკიცება და გაგზავნა';ru='Утвердить и отправить'");
	КонецЕсли;
	...
	Если ПодготовленКОтправке Тогда
		КомандаПодписиОтправки.Заголовок = НСтр("ka='გაგზავნა';ru='Отправить'");
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуЭП()
	...
	ШаблонЗаголовка = НСтр("ka='ხელმოწერები %1 და სტატუსები';ru='Подписи %1и статусы'");
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.Представление = ?(ТекСтрока.ПодписьВерна, НСтр("ka='სწორია';ru='Верна'"), НСтр("ka='არ არის სწორი';ru='Неверна'"))
			+" (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.Представление = НСтр("ka='არ არის შემოწმებული';ru='Не проверена'");
	КонецЕсли
...
"%2'");
			...
			Операция = НСтр("ka='საგადახდო დავალების დადასტურება';ru='Подтверждение платежного поручения'");
		...
		Если Результат.Количество() = 0 Тогда
			ПоказатьОповещениеПользователя(НСтр("ka='დასადასტურებლად მონაცემები არ არის';ru='Нет данных для подтверждения'"));
		КонецЕсли;
		
		Если Не ПустаяСтрока(Результат[0].ТекстОшибки) Тогда
			ТекстСообщения = НСтр("ka='შეცდომა საგადახდო დავალების დადასტურებისას:';ru='Ошибка при подтверждении платежного поручения:'") + " " + Результат[0].ТекстОшибки;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		ПоказатьОповещениеПользователя(НСтр("ka='დოკუმენტი დადასტურებულია';ru='Документ подтвержден'"));
	КонецЕсли;
...
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='დადასტურების სესიის ინიციალიზაცია';ru='Инициализация сессии подтверждения'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, Объект.Ссылка);
		Возврат;
...
"%2'");
		...
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ka='საგადახდო დავალების დადასტურების კოდის გაგზავნა';ru='Отправка кода подтверждения платежного поручения'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, Объект.Ссылка);
		Возврат;
	КонецПопытки;
...
Процедура ПослеПолученияОтпечатковЗапроситьСостояниеЭД(Отпечатки, ПараметрыОбработки) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='ხელმისაწვდომი სერთიფიკატები არ არის.';ru='Нет доступных сертификатов.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция СформироватьЗапросНаОтзывЭД(Знач НастройкаОбмена, Знач СообщениеОбменаПлатежноеПоручение, Знач Причина)
	...
	НазваниеЭД = НСтр("ka='ელექტრონული დოკუმენტის გამოხმობის მოთხოვნა';ru='Запрос на отзыв электронного документа'");
...
"Установите сертификаты или обратитесь к администратору.'");
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ШаблонСообщения = НСтр("ka='ბანკთან გაცვლის გამართვა %1 უმოქმედოა';ru='Настройка обмена с банком %1 недействительна'");
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			Операция = НСтр("ka='ელექტრონული დოკუმენტის ფორმირება';ru='Формирование электронного документа'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	НазваниеЭД = НСтр("ka='ელექტრონული დოკუმენტის მდგომარეობის მოთხოვნა';ru='Запрос состояния электронного документа'");
...
Процедура ОбработатьОшибкуПодтвержденияПлатежногоПорученияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='საგადახდო დავალების დადასტურებისას შეცდომა მოხდა.';ru='При подтверждении платежного поручения произошла ошибка.'");
	ВидОперации = НСтр("ka='საგადახდო დავალების დადასტურება.';ru='Подтверждение платежного поручения.'");
...
Процедура ОбработатьОшибкуОтправкиКодаПодтвержденияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='SMS-ით დადასტურების კოდის გაგზავნისას შეცდომა მოხდა.';ru='При отправке кода подтверждения через SMS произошла ошибка.'");
	ВидОперации = НСтр("ka='საგადახდო დავალების დადასტურების კოდის გაგზავნა.';ru='Отправка кода подтверждения платежного поручения.'");
	Оповещение = Новый ОписаниеОповещения("СообщитьОшибкуПользователю", ЭтотОбъект);
...
Процедура ОбработатьОшибкуНачалаПодтвержденияПлатежногоПоручения(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ka='გადახდის   დადასტურების დაწყებისას წარმოიშვა შეცდომა.';ru='При начале подтверждения платежного поручения произошла ошибка.'");
	ВидОперации = НСтр("ka='საგადახდო დავალების დადასტურების დასაწყისი.';ru='Начало подтверждения платежного поручения.'");
	Оповещение = Новый ОписаниеОповещения("СообщитьОшибкуПользователю", ЭтотОбъект);
