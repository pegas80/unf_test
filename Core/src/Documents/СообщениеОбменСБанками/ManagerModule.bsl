...
"%1.';ka='ცხრილი-დოკუმენტის შედგენის შეცდომა: %1.'");
		...
		Операция = НСтр("ka='ცხრილი-დოკუმენტის ჩამოყალიბება';ru='формирования табличного документа'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечататьКарточкуЭД Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"КарточкаЭД",
			НСтр("ka='ელექტრონული დოკუმენტის ბარათი';ru='Карточка электронного документа'"),
	...
	Если ПечататьЭД Тогда
		...
		СинонимМакета = НСтр("ka='ელექტრონული დოკუმენტი';ru='Электронный документ'");
...
Процедура ЗаполнитьТабличныйДокументПлатежноеТребование_ЭД(ТабличныйДокумент, Реквизиты, ИД)
	...
	Если Реквизиты.УсловиеОплаты = "1" Тогда
		ОбластьМакета.Параметры.УсловиеОплаты = НСтр("ka='1 - გადამხდელის წინასწარ მიცემული აქცეპტი';ru='1 - заранее данный акцепт плательщика'");
	Иначе
		ОбластьМакета.Параметры.УсловиеОплаты = НСтр("ka='1 - საჭიროა გადამხდელის აქცეპტის მიღება';ru='2 - требуется получение акцепта плательщика'");
	КонецЕсли;
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ka='საგადახდო მოთხოვნა #';ru='ПЛАТЕЖНОЕ ТРЕБОВАНИЕ №'") + " " + Реквизиты.Номер;
...
Процедура ЗаполнитьТабличныйДокументПлатежноеПоручениеИсходящее_ЭД(ТабличныйДокумент, Реквизиты, ИД)
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ka='საგადახდო დავალება №:';ru='ПЛАТЕЖНОЕ ПОРУЧЕНИЕ №'") + " " + Реквизиты.Номер;
	ОбластьМакета.Параметры.ДатаДокумента = Формат(Реквизиты.Дата, "ДЛФ=D");
...
Функция ПолучитьДанныеВыпискиДляПечати(СтрокаОбъекта, ДеревоРазбора)
	...
	Иначе	
		ДанныеЗаполненияШапки.Вставить("Организация", НСтр("ka='<не указано>';ru='<не указано>'"));
	КонецЕсли;
	...
	Иначе
		ДанныеЗаполненияШапки.Вставить("Банк", НСтр("ka='<не указано>';ru='<не указано>'"));
	КонецЕсли;
...
Функция СтатусПодписи(ВыборкаЭД)
	...
	Если ВыборкаЭД.ПодписьВерна Тогда
		СтатусПодписи = НСтр("ka='სწორია(';ru='Верна ('") + Формат(ВыборкаЭД.ДатаПроверкиПодписи,"ДЛФ=DT") + ")";
	Иначе
		СтатусПодписи = НСтр("ka='არასწორია(';ru='Неверна ('") + Формат(ВыборкаЭД.ДатаПроверкиПодписи,"ДЛФ=DT") + ")";
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеПодписантов(СтрокаМассива, ТребуемыеПодписи)
	...
	Если СтрокаМассива.ТребуетсяПодпись Тогда
		...
		КонецЕсли;
	Иначе
		ТребуемыеПодписи.ПредставлениеОтправителя = НСтр("ka='არ საჭიროებს';ru='Не требуется'");
		ТребуемыеПодписи.ПредставлениеПолучателя = НСтр("ka='არ საჭიროებს';ru='Не требуется'");
	КонецЕсли;
