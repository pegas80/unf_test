...
"%1.';ka='ცხრილი-დოკუმენტის შედგენის შეცდომა: %1.'");
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения, КраткоеПредставлениеОшибки);
		Операция = НСтр("ka='ცხრილი-დოკუმენტის ჩამოყალიბება';ru='формирования табличного документа';en='the formation of a spreadsheet document'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечататьКарточкуЭД Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"КарточкаЭД",
			НСтр("ka='ელექტრონული დოკუმენტის ბარათი';ru='Карточка электронного документа';en='Card electronic document'"),
	...
	Если ПечататьЭД Тогда
		...
		СинонимМакета = НСтр("ka='ელექტრონული დოკუმენტი';ru='Электронный документ';en='Electronic document'");
...
Процедура ЗаполнитьТабличныйДокументПлатежноеТребование_ЭД(ТабличныйДокумент, Реквизиты, ИД)
	...
	Если Реквизиты.УсловиеОплаты = "1" Тогда
		ОбластьМакета.Параметры.УсловиеОплаты = НСтр("ka='1 - გადამხდელის წინასწარ მიცემული აქცეპტი';ru='1 - заранее данный акцепт плательщика';en='1 - advance the acceptance of the payer'");
	Иначе
		ОбластьМакета.Параметры.УсловиеОплаты = НСтр("ka='1 - საჭიროა გადამხდელის აქცეპტის მიღება';ru='2 - требуется получение акцепта плательщика';en='2 - is required to obtain the acceptance of the payer'");
	КонецЕсли;
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ka='საგადახდო მოთხოვნა #';ru='ПЛАТЕЖНОЕ ТРЕБОВАНИЕ №';en='Payment request number'") + " " + Реквизиты.Номер;
...
Процедура ЗаполнитьТабличныйДокументПлатежноеПоручениеИсходящее_ЭД(ТабличныйДокумент, Реквизиты, ИД)
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ka='საგადახდო დავალება №:';ru='ПЛАТЕЖНОЕ ПОРУЧЕНИЕ №';en='PAYMENT ORDER number'") + " " + Реквизиты.Номер;
	ОбластьМакета.Параметры.ДатаДокумента = Формат(Реквизиты.Дата, "ДЛФ=D");
...
Функция ПолучитьДанныеВыпискиДляПечати(СтрокаОбъекта, ДеревоРазбора)
	...
	Иначе	
		ДанныеЗаполненияШапки.Вставить("Организация", НСтр("ka='<не указано>';ru='<не указано>';en='<Unknown>'"));
	КонецЕсли;
	...
	Иначе
		ДанныеЗаполненияШапки.Вставить("Банк", НСтр("ka='<не указано>';ru='<не указано>';en='<Unknown>'"));
	КонецЕсли;
...
Функция СтатусПодписи(ВыборкаЭД)
	...
	Если ВыборкаЭД.ПодписьВерна Тогда
		СтатусПодписи = НСтр("ka='სწორია(';ru='Верна (';en='Verne ('") + Формат(ВыборкаЭД.ДатаПроверкиПодписи,"ДЛФ=DT") + ")";
	Иначе
		СтатусПодписи = НСтр("ka='არასწორია(';ru='Неверна (';en='is incorrect ('") + Формат(ВыборкаЭД.ДатаПроверкиПодписи,"ДЛФ=DT") + ")";
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеПодписантов(СтрокаМассива, ТребуемыеПодписи)
	...
	Если СтрокаМассива.ТребуетсяПодпись Тогда
		...
		КонецЕсли;
	Иначе
		ТребуемыеПодписи.ПредставлениеОтправителя = НСтр("ka='არ საჭიროებს';ru='Не требуется';en='Not required'");
		ТребуемыеПодписи.ПредставлениеПолучателя = НСтр("ka='არ საჭიროებს';ru='Не требуется';en='Not required'");
	КонецЕсли;
