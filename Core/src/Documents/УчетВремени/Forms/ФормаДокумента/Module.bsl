...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОперацииПнДлительность.Заголовок = НСтр("ru='Пн ';ka='ორშ'") + Формат(Объект.ДатаС, "ДФ=dd.MM");
	Элементы.ОперацииВтДлительность.Заголовок = НСтр("ru='Вт ';ka='სამშ'") + Формат(Объект.ДатаС + 86400, "ДФ=dd.MM");
	Элементы.ОперацииСрДлительность.Заголовок = НСтр("ru='Ср ';ka='ოთხ'") + Формат(Объект.ДатаС + 86400*2, "ДФ=dd.MM");
	Элементы.ОперацииЧтДлительность.Заголовок = НСтр("ru='Чт ';ka='ხუთშ'") + Формат(Объект.ДатаС + 86400*3, "ДФ=dd.MM");
	Элементы.ОперацииПтДлительность.Заголовок = НСтр("ru='Пт ';ka='პარ'") + Формат(Объект.ДатаС + 86400*4, "ДФ=dd.MM");
	Элементы.ОперацииСбДлительность.Заголовок = НСтр("ru='Сб ';ka='შაბ'") + Формат(Объект.ДатаС + 86400*5, "ДФ=dd.MM");
	Элементы.ОперацииВсДлительность.Заголовок = НСтр("ru='Вс ';ka='კვირა'") + Формат(Объект.ДатаС + 86400*6, "ДФ=dd.MM");
...
Процедура ЗаполнитьПоПлану(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Сообщение.Текст = НСтр("ka='არ არის შევსებული ორგანიზაცია! შევსება გაუქმებულია.';ru='Не заполнена организация! Заполнение отменено.';en='Not filled with organization! Filling canceled.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='ქვეგანყოფილება შეუვსებელია! შევსება გაუქმებულია.';ru='Не заполнено подразделение! Заполнение отменено.';en='Not filled unit! Filling canceled.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული თანამშრომელი! შევსება გაუქმებულია.';ru='Не выбран сотрудник! Заполнение отменено.';en='Not an employee is selected! Filling canceled.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаС) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული კვირის დასაწყისი! შევსება გაუქმებულია.';ru='Не выбрано начало недели! Заполнение отменено.';en='Unspecified start to the week! Filling canceled.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаПо) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული კვირის დასასრული! შევსება გაუქმებულია.';ru='Не выбрано окончание недели! Заполнение отменено.';en='Unspecified end of the week! Filling canceled.'");
