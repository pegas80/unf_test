...
Процедура ЗаполнитьПоПлану(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Сообщение.Текст = НСтр("ka='არ არის შევსებული ორგანიზაცია! შევსება გაუქმებულია.';ru='Не заполнена организация! Заполнение отменено.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='ქვეგანყოფილება შეუვსებელია! შევსება გაუქმებულია.';ru='Не заполнено подразделение! Заполнение отменено.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული თანამშრომელი! შევსება გაუქმებულია.';ru='Не выбран сотрудник! Заполнение отменено.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаС) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული კვირის დასაწყისი! შევსება გაუქმებულია.';ru='Не выбрано начало недели! Заполнение отменено.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаПо) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული კვირის დასასრული! შევსება გაუქმებულია.';ru='Не выбрано окончание недели! Заполнение отменено.'");
	...
	Если Объект.Операции.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоПлануЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტის ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть документа будет очищена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
