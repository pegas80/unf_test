		...
		Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") И ТекущийОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты Тогда
			...
			Для Каждого ТекущаяСтрока Из ТекущийОбъект.ЭквайринговыеОперации Цикл
				Если ТекущаяСтрока.Документ <> Неопределено И ТекущаяСтрока.Документ.Дата >= ТекущийОбъект.Дата Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ka='ოპერაციის თარიღი დოკუმენტის თარიღზე მეტი ან მისი ტოლია!';ru='Дата операции больше или равна дате документа!'");
			...
			Если Отказ Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='დოკუმენტის თარიღი ყველა იმ ოპერაციის თარიღზე მეტი უნდა იყოს, რომელიც მითითებულია ცხრილ ""ეკვაირინგის ოპერაციები""-ში. დოკუმენტი არ არის გატარებული!';ru='Дата документа должна быть больше даты всех операций, которые указаны в табличной части ""Эквайринговые операции"". Документ не проведен!'");
				Сообщение.Сообщить();
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.Дата <> ДатаПередИзменением Тогда
		...
		СтруктураДанные.Вставить("ОбновитьКурсыРасчетовТекстВопроса", НСтр("ka='დოკუმენტის თარიღი შეიცვალა. ანგარიშსწორების კურსი დავაწესოთ ხელშეკრულების ვალუტის კურსის შესაბამისად?';ru='Изменилась дата документа. Установить курс расчетов в соответствии с курсом валюты договора?'"));
		// Конец Обновление курсов расчетов
		
		ТекстСообщения = НСтр("ka='შეიცვალა საბანკო ანგარიშის ვალუტის კურსი. გადავთვალოთ დოკუმენტის თანხები?';ru='Изменился курс валюты банковского счета. Пересчитать суммы документа?'");
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	ТекстСообщения = НСтр("ka='შეიცვალა საბანკო ანგარიშის ვალუტა. გადავთვალოთ დოკუმენტის თანხები?';ru='Изменилась валюта банковского счета. Пересчитать суммы документа?'");
	ПересчитатьСуммыПриИзмененииКурсаВалютыДенежныхСредств(СтруктураДанные, ТекстСообщения);
...
Процедура БанковскийСчетПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Зарплата") //Тогда
		...
		ТекстСообщения = НСтр("ka='საბანკო ანგარიშის ვალუტა შეიცვალა. სია ""საგადახდო უწყისები"" გასუფთავდება.';ru='Изменилась валюта банковского счета. Список ""Платежные ведомости"" будет очищен.'");
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	Элементы.СуммаДокумента.Заголовок = НСтр("ka='თანხა';ru='Сумма'");
	// Конец Эквайринг
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Поставщику") Тогда
		...
		Элементы.СуммаПлатежа.Заголовок = НСтр("ka='გავრცელებიულია';ru='Разнесено'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Покупателю") Тогда
		...
		Элементы.СуммаПлатежа.Видимость = Истина;
		Элементы.СуммаПлатежа.Заголовок = НСтр("ka='გადახდის თანხა';ru='Сумма платежа'");
		Элементы.СуммаРасчетов.Видимость = НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Подотчетнику") Тогда
		...
		Элементы.СуммаПлатежа.Заголовок = ?(ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь"), НСтр("ka='თანხა (გეგმ.)';ru='Сумма (план)'"), НСтр("ka='გადახდის თანხა';ru='Сумма платежа'"));
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты") Тогда
		...
		Элементы.ГруппаКонтрагент.Видимость = Истина;
		Элементы.Контрагент.Видимость = Истина;
		Элементы.Контрагент.Заголовок = НСтр("ka='ბანკი';ru='Банк-эквайрер'");
		Элементы.СчетКонтрагента.Видимость = Истина;
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ПрочиеРасчеты") Тогда
		...
		Элементы.ГруппаКонтрагент.Видимость = Истина;
		Элементы.Контрагент.Видимость = Истина;
		Элементы.Контрагент.Заголовок = НСтр("ka='კონტრაგენტი';ru='Контрагент'");
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам") Тогда
		...
		Элементы.ГруппаКонтрагент.Видимость = Истина;
		Элементы.Контрагент.Видимость = Истина;
		Элементы.Контрагент.Заголовок = НСтр("ka='ბანკი';ru='Банк'");
		Элементы.НалогообложениеНДС.Видимость = Истина;
...
Процедура ДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Подотчетник.Пустая() Тогда
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект,НСтр("ka='თავდაპირველად, საჭიროა ანგარიშვალდებული პირის არჩევა';ru='Сначала нужно выбрать подотчетника'"),,,"Объект.Подотчетник");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	...
	Если Объект.ВидОперации = ВидОперацииПоставщику Тогда
		...
		Если СуммаИтог <> 0 И СуммаИтог <> Объект.СуммаДокумента
			...
			НадписьВСкобках = ?(Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиВручную, 
				НСтр("ka='გავრცელებულია მექანიკურად';ru='разнесено вручную'"), 
				НСтр("ka='ჩათვლილია + ავანსი';ru='зачтено + аванс'"));
		...
		Если Элемент.СписокВыбора.Количество() = 0 Тогда
			Элемент.СписокВыбора.Добавить(Объект.СуммаДокумента, НСтр("ka='შესავსებად მონაცემები არ არის';ru='Нет данных для заполнения'"));
		КонецЕсли;
...
Процедура НазначениеПлатежаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияКомментария(Элемент.ТекстРедактирования, ЭтотОбъект, "Объект.НазначениеПлатежа", НСтр("ka='გადახდის დანიშნულება';ru='Назначение платежа'"));
	СтатистикаИспользованияФормКлиент.ПриИнтерактивномДействии(ЭтотОбъект, Элемент, "НачалоВыбора");
...
Процедура РасшифровкаПлатежаПризнакАвансаПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Покупателю")
		...
		Если ТипЗнч(СтрокаТабличнойЧасти.Документ) = Тип("ДокументСсылка.ПоступлениеВКассу")
		 ...
		 ИЛИ ТипЗнч(СтрокаТабличнойЧасти.Документ) = Тип("ДокументСсылка.ОперацияПоПлатежнымКартам") Тогда
			СтрокаТабличнойЧасти.ПризнакАванса = Истина;
			ПоказатьПредупреждение(Неопределено,НСтр("ka='ანგარიშსწორებათა დოკუმენტის მოცემული ტიპისთვის ავანსის თვისება ყოველთვის დაყენებულია!';ru='Для данного типа документа расчетов признак аванса всегда установлен!'"));
		ИначеЕсли ТипЗнч(СтрокаТабличнойЧасти.Документ) <> Тип("ДокументСсылка.Взаимозачет") Тогда
			СтрокаТабличнойЧасти.ПризнакАванса = Ложь;
			ПоказатьПредупреждение(Неопределено,НСтр("ka='ანგარიშსწორებათა დოკუმენტის მოცემული ტიპისთვის არ შეიძლება ავანსის თვისების დაყენება!';ru='Для данного типа документа расчетов нельзя установить признак аванса!'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура РасшифровкаПлатежаДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СтрокаТабличнойЧасти.ПризнакАванса
		...
		ПоказатьПредупреждение(, НСтр("ka='ანგარიშსწორების სახისთვის ნიშნით ""ავანსი"" ანგარიშსწორების დოკუმენტი იქნება მიმდინარე!';ru='Для вида расчета с признаком ""Аванс"" документом расчетов будет текущий!'"));
...
Процедура Подбор(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='დასაწყისში მიუთითეთ კონტრაგენტი!';ru='Укажите вначале контрагента!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет)
	   И НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='თავდაპირველად, მიუთითეთ საბანკო ანგარიში!';ru='Укажите вначале банковский счет!'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПоказатьПредупреждение(Неопределено, Нстр("ka='არ არის არჩეული დოკუმენტი საფუძველი!';ru='Не выбран документ основание!'"));
		Возврат;
	КонецЕсли;
	...
	Если (ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПеремещениеДСПлан")
		...
		ВызватьИсключение НСтр("ka='ფულის გადაადგილების შეყვანა დაუმტკიცებელი გეგმიური დოკუმენტის საფუძველზე არ შეიძლება!';ru='Нельзя ввести перемещение денег на основании неутвержденного планового документа!'");
	КонецЕсли;
	
	Ответ = Неопределено;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულად გადაივსება ""საფუძველი-დოკუმენტით""! გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет полностью перезаполнен по ""Документу-основанию""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаполнитьРасшифровку(Команда)
	...
	Если Объект.СуммаДокумента = 0 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='მიუთითეთ დასაწყისში დოკუმენტის თანხა.';ru='Укажите вначале сумму документа.'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьРасшифровкуЗавершение", ЭтотОбъект), 
		НСтр("ka='გაშიფრვა სრულად გადაივსება. გავაგრძელოთ?';ru='Расшифровка будет полностью перезаполнена. Продолжить?'"),
...
Процедура ОбновитьТаблицаДолгов(Команда)
	...
	Если Объект.Организация.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='თავდაპირველად, მიუთითეთ ორგანიზაცია!';ru='Укажите вначале организацию!'"));
		Возврат;
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
			...
			Если ДанныеДокумента <> Неопределено Тогда 
				...
				Если ДанныеДокумента.Выгружено Тогда 
					Выгружено = Истина;
					Элементы.ДекорацияВыгружено.Заголовок = НСтр("ka='გადატვირთულია';ru='Выгружен'");
					Элементы.ДекорацияВыгружено.ЦветТекста = Новый Цвет(0,255,0);
				Иначе
					Выгружено = Ложь;
					Элементы.ДекорацияВыгружено.Заголовок = НСтр("ka='არ არის გადატვირთული';ru='Не выгружен'");
...
Процедура НастроитьЭлементДоговораЗайма()
	...
	Иначе
		Элементы.ДоговорЗаймаСотруднику.ПодсказкаВвода = НСтр("ka='ხელშეკრულების ასარჩევად აირჩიეთ თანამშრომელი';ru='Чтобы выбрать договор, выберите сотрудника'");
	КонецЕсли;
	...
	Иначе
		Элементы.ДоговорКредита.ПодсказкаВвода = НСтр("ka='ხელშეკრულების ასარჩევად აირჩიეთ ბანკი';ru='Чтобы выбрать договор, выберите банк'");
	КонецЕсли;
...
Процедура РасшифровкаПлатежаПрочиеРасчетыДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='თავდაპირველად, აირჩიეთ კონტრაგენტი';ru='Сначала выберите контрагента'");
...
Процедура ЗаполнитьЭквайринговыеОперации(Команда)
	...
	Если Объект.ЭквайринговыеОперации.Количество() > 0 Тогда
		ПоказатьВопрос(ОписаниеОповещенияОЗавершении, НСтр("ka='ეკვაირინგის ოპერაციები სრულად იქნება გადავსებული. გავაგრძელოთ?';ru='Эквайринговые операции будут полностью перезаполнены. Продолжить?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	Иначе
...
Функция ПроверитьРеквизитыПередЗаполнениемТабличнойЧасти()
	...
	Если Объект.Контрагент.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='დასაწყისში მიუთითეთ კონტრაგენტი!';ru='Укажите вначале контрагента!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет)
	   И НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='თავდაპირველად, მიუთითეთ საბანკო ანგარიში!';ru='Укажите вначале банковский счет!'");
	...
	Если Объект.ЭквайринговыйТерминал.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='თავდაპირველად მიუთითეთ ეკვაირინგის ტერმინალი!';ru='Укажите вначале эквайринговый терминал!'");
...
Процедура ЭквайринговыйТерминалНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='თავდაპირველად, აირჩიეთ ეკვაირერი ბანკი!';ru='Сначала выберите банк-эквайрер!'");
		Сообщение.Поле = "Объект.Контрагент";
		Сообщение.Сообщить();
	...
	Если Объект.БанковскийСчет.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='თავდაპირველად, აირჩიეთ საბანკო ანგარიში!';ru='Сначала выберите банковский счет!'");
...
Процедура ПодборЭквайринговыхОпераций(Команда)
	...
	Если (НЕ ЗначениеЗаполнено(Объект.ЭквайринговыйТерминал) ИЛИ НЕ ЗначениеЗаполнено(Объект.Контрагент) 
		...
		И НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств))) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='არჩეული უნდა იქნას ბანკი-ეკვაირერი, ეკვაირინგის ტერმინალი და საბანკო ანგარიში!';ru='Должнен быть выбран банк-эквайрер, эквайринговый терминал и банковский счет!'"));
		Возврат;
	КонецЕсли;
...
Процедура СформироватьЗаголовокГруппыЗачетаДолгов()
		...
		Иначе
			...
			ИначеЕсли КоличествоСтрок > 1 Тогда
				Если КоличествоСтрок > 4 Тогда
					Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ka='ვალების ჩათვლა: ავტომატურად, რამდენიმე ხელშეკრულების მიხედვით (';ru='Зачет долгов: автоматически по нескольким договорам ('") + КоличествоСтрок + ")";
				Иначе
					ЗаголовокГруппы = НСтр("ka='ვალების ჩათვლა: ავტომატურად ხელშეკრულების %КоличествоСтрок%-ით';ru='Зачет долгов: автоматически по %КоличествоСтрок%-м договорам'");
	...
	ИначеЕсли Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиСПомощником Тогда
		...
		ОпределитьТекущуюСтраницуПомощника();
		Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ka='ვალების ჩათვლა: დამხმარესთან ერთად';ru='Зачет долгов: с помощником'");
		
	// Ручное заполнение.
	ИначеЕсли Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиВручную Тогда
		
		Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ka='ვალების ჩათვლა: ხელით';ru='Зачет долгов: вручную'");
...
Процедура ДобавитьСуммуПлатежаЗачетаДолгов(ИндексДоговораУчетаРасчетов)
	...
	ПолеСуммыПлатежа.Заголовок = НСтр("ka='თანხა';ru='сумма'");
...
Процедура ДоговорДляЗачетаАванса2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='თავდაპირველად, საჭიროა მომწოდებლის არჩევა';ru='Сначала нужно выбрать поставщика'"));
...
Процедура ЗаполнитьПФПоОстаткам(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ВидУдержания) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ველი ""დაკავების ტიპი"" არ არის შევსებული';ru='Поле ""Вид удержания"" не заполнен'");
