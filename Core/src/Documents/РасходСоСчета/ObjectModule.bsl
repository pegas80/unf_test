...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.СтатусУтвержденияПлатежа = Перечисления.СтатусыУтвержденияПлатежей.НеУтвержден Тогда
		ВызватьИсключение НСтр("ka='დაუმტკიცებელი განაცხადის საფუძველზე ფულის გახარჯვის შეყვანა არ შეიძლება!';ru='Нельзя ввести расход денег на основании неутвержденной заявки!'");
	КонецЕсли;
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		ВызватьИсключение НСтр("ka='ფულის გასავალის შეყვანა არ შეიძლება. განაცხადში მითითებულია გადახდის არასწორი მეთოდი (ფულადი სახსრების ტიპი)!';ru='Нельзя ввести расход денег. В заявке указан не верный способ оплаты (тип денежных средств)!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	Если ДокументСсылкаНачислениеНалогов.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
		ВызватьИсключение НСтр("ka='ანგარიშიდან გასავლის შეყვანა შეიძლება მხოლოდ გადასახადების დარიცხვის, და არა ანაზღაურების საფუძველზე.';ru='Расход со счета можно ввести только на основании начисления налогов, а не возмещения.'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	 ...
	 ИЛИ ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю Тогда
					   ...
					   И НЕ СтрокаРасшифровкаПлатежа.ПризнакАванса)) Тогда
						Если РасшифровкаПлатежа.Количество() = 1 Тогда
							Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику Тогда
								ТекстСообщения = НСтр("ka='მიუთითეთ გადმოტვირთვის დოკუმენტი ან გადახდის ავანსის ნიშანი.';ru='Укажите документ отгрузки или признак аванса платежа.'");
							Иначе
								ТекстСообщения = НСтр("ka='მიუთითეთ ანგარიშსწორებების დოკუმენტი.';ru='Укажите документ расчетов.'");
							КонецЕсли;
						Иначе
							Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику Тогда
								ТекстСообщения = НСтр("ka='მიუთითეთ გადმოტვირთვის დოკუმენტი ან გადახდის ნიშანი სია ""გადახდის გაშიფრვის"" სტრიქონ %НомерСтроки%-ში.';ru='Укажите документ отгрузки или признак оплаты в строке %НомерСтроки% списка ""Расшифровка платежа"".'");
							Иначе
								ТекстСообщения = НСтр("ka='მიუთითეთ ანგარიშსწორებების დოკუმენტი სია ""გადახდის გაშიფრვის"" სტრიქონ %НомерСтроки%-ში.';ru='Укажите документ расчетов в строке %НомерСтроки% списка ""Расшифровка платежа"".'");
							КонецЕсли;
		...
		Иначе
			...
			Для каждого СтрокаДоговоры Из ДоговорыАвтоЗачетаДолгов Цикл
				...
				Если (СтрокаДоговоры.Договор.Пустая() И СтрокаДоговоры.СуммаПлатежа > 0) ИЛИ
					...
					ТекстСообщения = НСтр("ka='№%НомерСтроки% ხელშეკრულებისთვის უნდა გასუფთავდეს თანხა ან არჩეული იქნას ხელშეკრულება.';ru='Для договора под №%НомерСтроки% нужно очистить сумму или выбрать договор.'");
		...
		Если СуммаПлатежа <> СуммаДокумента Тогда
			ТекстСообщения = ?(ПогашатьАвтоматическиСРазнесениемПоДоговорам,
			    НСтр("ka='დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств%, ხელშეკრულებების მიხედვით გატარებული გადახდების თანხას არ შეესაბამება: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей по договорам: %СуммаПлатежа% %ВалютаДенежныхСредств%!'"),
				НСтр("ka='დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств%, არ შეესაბამება განკუთვნებულ გადახდებს ცხრილში: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!'")
			);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата //Тогда
		...
		Если СуммаПлатежа <> СуммаДокумента Тогда
			ТекстСообщения = НСтр("ka='დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств%, არ შეესაბამება განკუთვნებულ გადახდებს ცხრილში: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаДокумента%", Строка(СуммаДокумента));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаПлатежа%", Строка(СуммаПлатежа));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВалютаДенежныхСредств%", СокрЛП(Строка(ВалютаДенежныхСредств)));
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
		...
		Для Каждого СтрокаВедомости Из ВыплатаЗаработнойПлаты Цикл
			Если СтрокаВедомости.Ведомость.ВалютаДокумента <> ВалютаДенежныхСредств Тогда
				ТекстСообщения = НСтр("ka='დოკუმენტის ვალუტა: %ВалютаДенежныхСредств%, საგადახდო უწყისის ვალუტას არ შეესაბამება: %ВалютаВедомости%!';ru='Валюта документа: %ВалютаДенежныхСредств%, не соответствует валюте платежной ведомости: %ВалютаВедомости%!'");
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику Тогда
		...
		Если Подотчетник.Пустая() Тогда
			ТекстСообщения = НСтр("ka='ველი ""თანამშრომელი"" არ არის შევსებული';ru='Поле ""Сотрудник"" не заполнено'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
				,
	...
	Если ПроверитьДокументыПланирования Тогда
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			...
			Если ЕстьОшибка Тогда
				ТекстСообщения = НСтр("ka='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств% меньше суммы разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств% меньше суммы разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаДокумента%", Строка(СуммаДокумента));
		...
		Если Выборка.Следующий() Тогда
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Выборка.Ссылка, ?(ИспользоватьТолькоРегламентированногоПланаСчетов,Корреспонденция,КорреспонденцияРегл)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მომსახურების მიმდინრე ხელშეკრულების მიხედვით დაუგეგმავი ხარჯების გატარება აკრძალულია!';ru='Запрещено проводить незапланированные расходы по текущему договору обслуживания!'"),,
