...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	|		ПО (ВыплатаЗаработнойПлаты.Ссылка = ВременнаяТаблицаПроверкиНаСтатус.Ссылка)
	|ГДЕ
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПогашениеОбязательствПередПоставщиком", НСтр("ka='მომწოდებლის წინაშე ვალდებულებების დაფარვა';ru='Погашение обязательств перед поставщиком'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПередПерсоналом", НСтр("ka='პერსონალის წინაშე ვალდებულებების დაფარვა';ru='Погашение обязательств перед персоналом'"));
	//_Паганини#Удержания
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПогашениеАвансаПокупателя", НСтр("ka='მყიდველის ავანსის დაფარვა';ru='Погашение аванса покупателя'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	//Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	//Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
	//Запрос.УстановитьПараметр("Содержание", НСтр("ka='ფულადი სახსრების ჩამოწერა თავისუფალ ანგარიშზე';ru='Списание денежных средств на произвольный счет'"));
	//Запрос.УстановитьПараметр("СодержаниеСнятиеНаличных", НСтр("ka='Оприходование денежных средств в кассу';ru='Оприходование денежных средств в кассу'"));
	//Запрос.УстановитьПараметр("ОплатаНалога", НСтр("ka='გადასახადის გადახდა';ru='Оплата налога'"));
	//Запрос.УстановитьПараметр("СодержаниеОплатаОтБанкаЭквайрера", НСтр("ka='გადახდა ეკვაირერი ბანკისგან';ru='Оплата от банка-эквайрера'"));
	//Запрос.УстановитьПараметр("СодержаниеКомиссияБанкаЭквайрера", НСтр("ka='ბანკი-ექვაირერის კომისია';ru='Комиссия банка-эквайрера'"));
	//Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
	//Запрос.УстановитьПараметр("ИспользуютсяПереводыВПути",РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата,СтруктураДополнительныеСвойства.ДляПроведения.Организация));
	////Паганини
	////Добавлена таблица 18;
	//Запрос.УстановитьПараметр("НДФЛ", НСтр("ka='საშემოსავლო';ru='НДФЛ'"));
	//Запрос.УстановитьПараметр("КурсоваяРазницаНДФЛ", НСтр("ka='Курсовая разница НДФЛ';ru='Курсовая разница НДФЛ'"));
	////_Паганини
	//
	//Запрос.Текст =
	//"ВЫБРАТЬ
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	//_Паганини
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		//+++ Истомин, Работа с Интеграцией банка
		|	ВременнаяТаблицаПроверкиНаСтатус.СформироватьПроводки, 
		//--- Истомин
		...
		//+++ Истомин, Работа с Интеграцией банка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПроверкиНаСтатус КАК ВременнаяТаблицаПроверкиНаСтатус
		|		ПО РасходСоСчета.Ссылка = ВременнаяТаблицаПроверкиНаСтатус.Ссылка
		|ГДЕ
		|	ВременнаяТаблицаПроверкиНаСтатус.СформироватьПроводки
		//--- Истомин
		|;
	...
	ИначеЕсли ДокументСсылкаРасходСоСчета.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Или ДокументСсылкаРасходСоСчета.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.РасчетыПоКредитам Тогда
		...
		Запрос.УстановитьПараметр("ПогашениеНалога",	НСтр("ka='გადასახადის დაფარვა';ru='Погашение налога'"));
	 	//Паганини#Удержания_
		Запрос.УстановитьПараметр("СуммыНалоговУдержаниеДляКонтрагентаКОплатеПриНачислении",	УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("СуммыНалоговУдержаниеДляКонтрагентаКОплатеПриНачислении"));
	 	//_Паганини#Удержания
		Запрос.УстановитьПараметр("ИспользоватьТолькоРегламентированногоПланаСчетов", СтруктураДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов);
		...
		//+++ Истомин, Работа с Интеграцией банка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПроверкиНаСтатус КАК ВременнаяТаблицаПроверкиНаСтатус
		|		ПО РасходСоСчетаВыплатаЗаработнойПлаты.Ссылка = ВременнаяТаблицаПроверкиНаСтатус.Ссылка
		|ГДЕ
		|	ВременнаяТаблицаПроверкиНаСтатус.СформироватьПроводки
		//--- Истомин
		//Паганини#Удержания_
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства) Экспорт
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаПроверкиНаСтатус
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПроверкиНаСтатус КАК ВременнаяТаблицаПроверкиНаСтатус
	|		ПО (ВременнаяТаблицаПроверкиНаСтатус.Ссылка = ТаблицаДокумента.Ссылка)
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	//|	И ВременнаяТаблицаПроверкиНаСтатус.СформироватьПроводки
	|;
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	//+++ Истомин, Интеграци с банком
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПроверкиНаСтатус КАК ВременнаяТаблицаПроверкиНаСтатус
	|		ПО РасходСоСчетаЭквайринговыеОперации.Ссылка = ВременнаяТаблицаПроверкиНаСтатус.Ссылка
	//--- Истомин
	|ГДЕ
...
Процедура СформироватьТаблицаДоходыИРасходыЭквайринг(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Для каждого ТекущаяСтрокаРасчетов Из ТаблицаРасчетыПоЭквайрингу Цикл
		...
		Движение.СодержаниеПроводки = НСтр("ka='ბანკი-ესქვაირერის კომისია';ru='Комиссия банка-эквайера'");
...
Процедура СформироватьТаблицаРасчетыСПрочимиКонтрагентами(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Комментарий", НСтр("ka='კონტრაგენტის ვალის გაზრდა';ru='Увеличение долга контрагента'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходСоСчета);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("УчетРасчетовПоЗаймамСотрудникам", НСтр("ka='თანამშრომლისთვის სესხის გაცემა';ru='Выдача займа сотруднику'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходСоСчета);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаУправленческий_РС(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	//Запрос.УстановитьПараметр("КурсоваяРазницаНДФЛ", НСтр("ka='Курсовая разница НДФЛ';ru='Курсовая разница НДФЛ'"));
	////_Паганини
	//Запрос.УстановитьПараметр("СодержаниеПроводкиПоЗарплате",НСтр("ka='გადასახადის დარიცხვა';ru='Начисление налога'"));
