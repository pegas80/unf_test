...
Процедура НадписьДокументОснованиеОтсутствуетНажатие(Элемент)
	...
	Если СписокТиповДокументовОснований.Количество() > 0 Тогда
		...
		Иначе
			ЗаголовокСпискаВыбора = НСтр("ka='აირჩიეთ საფიძველი დოკუმენტის ტიპი...';ru='Выберите тип документа основания...';en='Select the type of the base document ...'");
...
Процедура ГиперссылкаОчиститьОснованиеНажатие(Элемент)
	...
	ТекстВопроса = НСтр("ka='საფუძველი-დოკუმენტის აღება';ru='Убрать документ-основание?';en='Remove paper-base?'");
...
Процедура ДобавитьВложениеИзФайлаНаДиске(Команда)
	...
	ТекстПредупреждения = НСтр("ka='Для загрузки файла необходимо установить расширение для веб-клиента 1С:Предприятие.';ru='Для загрузки файла необходимо установить расширение для веб-клиента 1С:Предприятие.';en='To download the file you need to install the extension for Web client 1C: Enterprise.'");
...
Функция ПредставлениеОснования(СсылкаНаОснование)
	...
	Если ЗначениеЗаполнено(СсылкаНаОснование) Тогда
		Если Найти(СсылкаНаОснование.Метаданные().ПолноеИмя(), "Справочник") > 0 Тогда
			...
			ТекстДокументОснование = НСтр("ka='%1 %2';ru='%1 %2';en='%12'");
		...
		Иначе
			...
			ТекстДокументОснование = НСтр("ka='%1 %3-ის №%2';ru='%1 №%2 от %3';en='№% 1% 2% 3 from'");
			ТекстДокументОснование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстДокументОснование,
...
Процедура ПоказатьРезультатПодписи(Подписан)
	...
	ТекстСостояния = НСтр("ka='ხელმოწერილია :(0)';ru='Подписано: (0)';en='Signed: (0)'");
	...
	Если Подписан Тогда
		...
		Прочитать();
		УстановитьДоступностьЭлементов();
		ТекстСостояния = НСтр("ka='ხელმოწერილია:(1)';ru='Подписано: (1)';en='Signed: (1)'");
	КонецЕсли;
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.ПодписьВерна = ?(ТекСтрока.ПодписьВерна, НСтр("ka='სწორია';ru='Верна';en='Verne'"), НСтр("ka='არ არის სწორი';ru='Неверна';en='incorrectly'"))
			+ " (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.ПодписьВерна = НСтр("ka='არ არის შემოწმებული';ru='Не проверена';en='not tested'");
	КонецЕсли
...
Процедура ДобавитьСертификатВДоверенные(ДанныеПодписи)
	...
	Если ДанныеПодписи <> Неопределено И ДанныеПодписи.ОтсутствуетВСписке Тогда
		Текст = НСтр("ka='სერთიფიკატი %1 დავამატოთ კონტრაგენტის მოსალოდნელი სერთიფიკატების სიაში?';ru='Добавить сертификат %1 в список ожидаемых сертификатов контрагента?';en='Add the certificate% 1 in the list of expected counterpart certificates?'");
...
Процедура УстановитьДоступностьЭлементов()
	...
	ЗаголовокПодписи = НСтр("ka='ხელმოწერები';ru='Подписи';en='signatures'");
	Если ЗначениеЗаполнено(ЭлектронныйДокументСсылка) Тогда
		КоличествоЭП = ЭлектронныйДокументСсылка.ЭлектронныеПодписи.Количество();
		Если КоличествоЭП > 0 Тогда
			ЗаголовокПодписи = СтрЗаменить(НСтр("ka='ხელმოწერები (%1)';ru='Подписи (%1)';en='Signatures (1%)'"), "%1", КоличествоЭП);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Объект.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ТекстЗаголовка = НСтр("ka='%2-ის გამავალი ნებისმიერი დოკუმენტი %1';ru='Исходящий произвольный документ %1 от %2';en='Outgoing arbitrary document by% 1% 2'");
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстЗаголовка, Объект.Номер, Объект.Дата);
		Иначе
			Заголовок = НСтр("ka='გამავალი ნებისმიერი დოკუმენტი (შექმნა)';ru='Исходящий произвольный документ (создание)';en='Outgoing arbitrary document (creation)'");
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ka='%2-ის შემომავალი თავისუფალი დოკუმენტი %1';ru='Входящий произвольный документ %1 от %2';en='Incoming arbitrary document by% 1% 2'");
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстЗаголовка, Объект.Номер, Объект.Дата);
	КонецЕсли;
...
Процедура ПослеОтправкиПЭД(Результат, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("КолОтправленных", КолОтправленных)
			...
			ТекстСостояния = НСтр("ka='გაგზავნილია: (%1)';ru='Отправлено: (%1)';en='Submitted (1%)'");
		...
		ИначеЕсли Результат.Свойство("КолПодготовленных", КолПодготовленных)
			...
			ТекстСостояния = НСтр("ka='გასაგზავნად მომზადებულია: (%1)';ru='Подготовлено к отправке: (%1)';en='Prepared by sending (1%)'");
...
Процедура ПроверитьЗаписьДокумента(Неопределен, ОбработкаПродолжения) Экспорт
	...
	Если Модифицированность ИЛИ Объект.Ссылка.Пустая() Тогда
		ТекстВопроса = НСтр("ka='ოპერაციის გასაგრძელებლად საჭიროა დოკუმენტის ჩაწერა. ჩავწეროთ დოკუმენტი?';ru='Для продолжения операции необходимо записать документ."
"Записать документ?'");
