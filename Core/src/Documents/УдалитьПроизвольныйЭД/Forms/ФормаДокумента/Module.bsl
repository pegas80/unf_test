...
Процедура НадписьДокументОснованиеОтсутствуетНажатие(Элемент)
	...
	Если СписокТиповДокументовОснований.Количество() > 0 Тогда
		...
		Иначе
			ЗаголовокСпискаВыбора = НСтр("ka='აირჩიეთ საფიძველი დოკუმენტის ტიპი...';ru='Выберите тип документа основания...'");
...
Процедура ГиперссылкаОчиститьОснованиеНажатие(Элемент)
	...
	ТекстВопроса = НСтр("ka='საფუძველი-დოკუმენტის აღება';ru='Убрать документ-основание?'");
...
Процедура ДобавитьВложениеИзФайлаНаДиске(Команда)
	...
	ТекстПредупреждения = НСтр("ka='Для загрузки файла необходимо установить расширение для веб-клиента 1С:Предприятие.';ru='Для загрузки файла необходимо установить расширение для веб-клиента 1С:Предприятие.'");
...
Функция ПредставлениеОснования(СсылкаНаОснование)
	...
	Если ЗначениеЗаполнено(СсылкаНаОснование) Тогда
		Если Найти(СсылкаНаОснование.Метаданные().ПолноеИмя(), "Справочник") > 0 Тогда
			...
			ТекстДокументОснование = НСтр("ka='%1 %2';ru='%1 %2'");
		...
		Иначе
			...
			ТекстДокументОснование = НСтр("ka='%1 %3-ის №%2';ru='%1 №%2 от %3'");
			ТекстДокументОснование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстДокументОснование,
...
Процедура ПоказатьРезультатПодписи(Подписан)
	...
	ТекстСостояния = НСтр("ka='ხელმოწერილია :(0)';ru='Подписано: (0)'");
	...
	Если Подписан Тогда
		...
		Прочитать();
		УстановитьДоступностьЭлементов();
		ТекстСостояния = НСтр("ka='ხელმოწერილია:(1)';ru='Подписано: (1)'");
	КонецЕсли;
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.ПодписьВерна = ?(ТекСтрока.ПодписьВерна, НСтр("ka='სწორია';ru='Верна'"), НСтр("ka='არ არის სწორი';ru='Неверна'"))
			+ " (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.ПодписьВерна = НСтр("ka='არ არის შემოწმებული';ru='Не проверена'");
	КонецЕсли
...
Процедура ДобавитьСертификатВДоверенные(ДанныеПодписи)
	...
	Если ДанныеПодписи <> Неопределено И ДанныеПодписи.ОтсутствуетВСписке Тогда
		Текст = НСтр("ka='სერთიფიკატი %1 დავამატოთ კონტრაგენტის მოსალოდნელი სერთიფიკატების სიაში?';ru='Добавить сертификат %1 в список ожидаемых сертификатов контрагента?'");
...
Процедура УстановитьДоступностьЭлементов()
	...
	ЗаголовокПодписи = НСтр("ka='ხელმოწერები';ru='Подписи'");
	Если ЗначениеЗаполнено(ЭлектронныйДокументСсылка) Тогда
		КоличествоЭП = ЭлектронныйДокументСсылка.ЭлектронныеПодписи.Количество();
		Если КоличествоЭП > 0 Тогда
			ЗаголовокПодписи = СтрЗаменить(НСтр("ka='ხელმოწერები (%1)';ru='Подписи (%1)'"), "%1", КоличествоЭП);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Объект.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			ТекстЗаголовка = НСтр("ka='%2-ის გამავალი ნებისმიერი დოკუმენტი %1';ru='Исходящий произвольный документ %1 от %2'");
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстЗаголовка, Объект.Номер, Объект.Дата);
		Иначе
			Заголовок = НСтр("ka='გამავალი ნებისმიერი დოკუმენტი (შექმნა)';ru='Исходящий произвольный документ (создание)'");
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ka='%2-ის შემომავალი თავისუფალი დოკუმენტი %1';ru='Входящий произвольный документ %1 от %2'");
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстЗаголовка, Объект.Номер, Объект.Дата);
	КонецЕсли;
...
Процедура ПослеОтправкиПЭД(Результат, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("КолОтправленных", КолОтправленных)
			...
			ТекстСостояния = НСтр("ka='გაგზავნილია: (%1)';ru='Отправлено: (%1)'");
		...
		ИначеЕсли Результат.Свойство("КолПодготовленных", КолПодготовленных)
			...
			ТекстСостояния = НСтр("ka='გასაგზავნად მომზადებულია: (%1)';ru='Подготовлено к отправке: (%1)'");
