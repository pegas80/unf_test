...
Процедура ИнициализироватьДанныеДокументаДенежныеСредстваПрогноз(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеПриход", НСтр("ka='ფულადი სახსრების შემოსვლის პროგნოზი';ru='Прогноз поступления денежных средств'"));
	Запрос.УстановитьПараметр("СодержаниеРасход", НСтр("ka='ფულადი სახსრების გამოსვლის პროგნოზი';ru='Прогноз выбытия денежных средств'"));
...
Процедура ИнициализироватьДанныеДокументаДоходыИРасходыПрогноз(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаБюджет);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("СодержаниеПриход", НСтр("ka='შემოსავლების პროგნოზი';ru='Прогноз доходов'"));
	Запрос.УстановитьПараметр("СодержаниеРасход", НСтр("ka='ხარჯების პროგნოზი';ru='Прогноз расходов'"));
...
Процедура ИнициализироватьДанныеДокументаОстатки(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ВалютаУчета", Константы.ВалютаУчета.Получить());
	Запрос.УстановитьПараметр("Содержание", НСтр("ka='საწყისი ნაშთების პროგნოზი';ru='Прогноз начальных остатков'"));
						...
						Если ОбходПоСчетамУчета.СуммаДоходов <> 0 
						 ИЛИ ОбходПоСчетамУчета.СуммаРасходов <> 0 Тогда
							...
							Если СуммаРаспределили = 0 Тогда
								...
								ТекстСообщения = НСтр("ka='ფინანსური შედეგის გაანგარიშება: აღრიცხვის ანგარიშს ""%СчетУчета%"" არ გააჩნია გადანაწილების ბაზა!';ru='Расчет финансового результата: Счет учета ""%СчетУчета%"", не имеет базы распределения!'");
