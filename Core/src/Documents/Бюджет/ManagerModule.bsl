...
Процедура ИнициализироватьДанныеДокументаДоходыИРасходыПрогноз(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеПриход", НСтр("ka='შემოსავლების პროგნოზი';ru='Прогноз доходов';en='earnings forecast'"));
	Запрос.УстановитьПараметр("СодержаниеРасход", НСтр("ka='ხარჯების პროგნოზი';ru='Прогноз расходов';en='Term expenditure'"));
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КоррСчетРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеДоходы)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	...
	|	ТаблицаДокумента.КоррСчетРегл КАК СчетУчета,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КоррСчетРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеДоходы)
	|				ИЛИ ТаблицаДокумента.КоррСчетРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПроцентыПоКредитам)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ТаблицаДокумента.СтруктурнаяЕдиница
	|	КОНЕЦ,
	|	ВЫБОР
...
Процедура ИнициализироватьДанныеДокументаДоходы(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КоррСчетРегл.Валютный
	|			ТОГДА &ВалютаУчета
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаДт,
	|	ВЫБОР
...
Процедура ИнициализироватьДанныеДокументаВыбытия(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Ссылка.СценарийПланирования КАК СценарийПланирования,
	|	ТаблицаДокумента.Сумма КАК Сумма,
	|	ТаблицаДокумента.КоррСчет КАК СчетДт1,
	|	ВЫБОР
	...
	|	ТаблицаДокумента.Счет КАК СчетКт,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СчетРегл.Валютный
	|			ТОГДА &ВалютаУчета
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаКт,
	|	ВЫБОР
...
Процедура ИнициализироватьДанныеДокументаПоступления(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Ссылка.СценарийПланирования КАК СценарийПланирования,
	|	ТаблицаДокумента.Сумма КАК Сумма,
	|	ТаблицаДокумента.Счет КАК СчетДт1,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СчетРегл.Валютный
	|			ТОГДА &ВалютаУчета
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
...
Процедура ИнициализироватьДанныеДокументаОстатки(ДокументСсылкаБюджет, СтруктураДополнительныеСвойства)
	...
	|	0 КАК СуммаВалДт,
	|	ТаблицаДокумента.Счет КАК Счет1,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СчетРегл.Валютный
	|			ТОГДА &ВалютаУчета
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ВалютаУчета", Константы.ВалютаУчета.Получить());
	Запрос.УстановитьПараметр("Содержание", НСтр("ka='საწყისი ნაშთების პროგნოზი';ru='Прогноз начальных остатков';en='Prediction opening balances'"));
...
//	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаБюджет);
//	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
//	Запрос.УстановитьПараметр("СодержаниеПриход", НСтр("ka='ფულადი სახსრების შემოსვლის პროგნოზი';ru='Прогноз поступления денежных средств';en='Forecast cash flows'"));
//	Запрос.УстановитьПараметр("СодержаниеРасход", НСтр("ka='ფულადი სახსრების გამოსვლის პროგნოზი';ru='Прогноз выбытия денежных средств';en='Forecast cash outflows'"));
//	Запрос.УстановитьПараметр("ВалютаУчета", Константы.ВалютаУчета.Получить());
//	
//	Запрос.Текст =
//	"ВЫБРАТЬ
...
//								
//								ТекстСообщения = НСтр("ka='ფინანსური შედეგის გაანგარიშება: აღრიცხვის ანგარიშს ""%СчетУчета%"" არ გააჩნია გადანაწილების ბაზა!';ru='Расчет финансового результата: Счет учета ""%СчетУчета%"", не имеет базы распределения!';en='The calculation of the financial result: accounting bill ""% SchetUcheta%"", has no base distribution!'");
