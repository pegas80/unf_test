...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВыдачаИзРемонта И (НЕ РемонтВыполнен) Тогда
		Если ВариантРемонта = Перечисления.ВариантыРемонта.СервисЦентр Тогда
		    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერვის-ცენტრიდან მიღებულია არ არის დაყენებული!';ru='Не установлен ""Принят из сервисного центра""!'"));
		Иначе	
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='რემონტი შესრულებულია არ არის დაყენებული!';ru='Не установлен ""Ремонт выполнен""!'"));
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
	Если РемонтВыполнен И ВариантРемонта = Перечисления.ВариантыРемонта.СервисЦентр И (НЕ ПередачаВСервисныйЦентр) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='არ არის დაყენებული ""სერვის-ცენტრში გადაცემულია""!';ru='Не установлен ""Передан в сервисный центр""!'"));
		Отказ = Истина;
	КонецЕсли;
	
	Если ПередачаВСервисныйЦентр И РемонтВыполнен И ДатаПередачаВСервисныйЦентр > ДатаРемонтВыполнен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერვის-ცენტრში გადაცემის თარიღი არ შეიძლება, რომ სერვის-ცენტრიდან მიღების თარიღზე მეტი იყოს!';ru='Дата передачи в сервисный центр не может быть больше даты приема из сервисного центра!'"));
		Отказ = Истина;
	КонецЕсли;
	
	Если РемонтВыполнен И ВыдачаИзРемонта И ДатаРемонтВыполнен > ДатаВыдачаИзРемонта Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='რემონტის შესრულების ვადა არ შეიძლება, რომ რემონტიდან გაცემის ვადაზე მეტი იყოს!';ru='Дата выпонения ремонта не может быть больше даты выдачи из ремонта!'"));
		Отказ = Истина;
	КонецЕсли;
	...
	Если ПередачаВСервисныйЦентр И Дата > ДатаПередачаВСервисныйЦентр Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='სერვის-ცენტრში გადაცემის თარიღი არ შეიძლება, რომ სარემონტოდ მიღების თარიღზე (დოკუმენტის თარიღზე) ნაკლები იყოს!';ru='Дата передачи в сервисный центр не может быть меньше даты приема в ремонт (даты документа)!'"));
			Отказ = Истина;
		КонецЕсли; 
	КонецЕсли;
