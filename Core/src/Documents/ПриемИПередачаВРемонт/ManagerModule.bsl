...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПриемИПередачаВРемонт, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДатаРемонтВыполнен, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателя", НСтр("ka='მყიდველის დავალიანება სარემონტო მომსახურებისთვის';ru='Задолженность покупателя за услуги ремонта';en=""Buyer's debts for repair services"""));
...
Процедура СформироватьАктВозврата(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ШаблонЗаголовка = НСтр("ka='Возврат из ремонта № %1 от %2';ru='Возврат из ремонта № %1 от %2';en='Return of repair №% 1% 2 from'");
		...
		Если Шапка.СуммаДокумента > 0 Тогда
			...
			ОбластьМакета.Параметры.ИтоговаяСтрока = Нстр("ka='Всего наименований 1, на сумму ';ru='Всего наименований 1, на сумму '") + УправлениеНебольшойФирмойСервер.ФорматСумм(Шапка.СуммаДокумента, Шапка.ВалютаДокумента);
			...
			Если Шапка.СуммаНДС = 0 Тогда
				ОбластьМакета.Параметры.НДС = Нстр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)';en='Without tax (VAT)'");
				ОбластьМакета.Параметры.СуммаНДС = Нстр("ka='-';ru='-'");
			Иначе
				ОбластьМакета.Параметры.НДС = ?(Шапка.СуммаВключаетНДС, Нстр("ka='მათ შორის დღგ:';ru='В том числе НДС:';en='Including VAT:'"), Нстр("ka='დღგ-ის თანხა';ru='Сумма НДС:';en='VAT:'"));
...
Процедура СформироватьЧек(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
		ШаблонЗаголовка = НСтр("ka='Товарный чек № %1 от %2';ru='Товарный чек № %1 от %2';en='Receipt №% 1% 2 from'");
		ОбластьМакета.Параметры.ТекстЗаголовка = СтрШаблон(ШаблонЗаголовка, НомерДокумента, ПечатьДокументовУНФ.ПредставлениеДатыВДокументах(Шапка.ДатаДокумента));
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		ОбластьМакета.Параметры.Заполнить(Шапка);
		Если СокрЛП(Шапка.Содержание) = "" Тогда
			ОбластьМакета.Параметры.Содержание = НСтр("ka='რემონტი';ru='Ремонт';en='Repairs'");
		КонецЕсли; 
		ТабличныйДокумент.Вывести(ОбластьМакета);
...
Процедура СформироватьТалонПриемки(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ШаблонЗаголовка = НСтр("ka='Поступление в ремонт № %1 от %2';ru='Поступление в ремонт № %1 от %2';en='Release in repair №% 1% 2 from'");
		ОбластьМакета.Параметры.ТекстЗаголовка = СтрШаблон(ШаблонЗаголовка, НомерДокумента, ПечатьДокументовУНФ.ПредставлениеДатыВДокументах(Шапка.ДатаДокумента));
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Гарантия");
		ОбластьМакета.Параметры[?(Шапка.Гарантийный, "Гарантия", "НеГарантия")] = НСтр("ka='X';ru='X';en='X'");
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		ОбластьМакета = Макет.ПолучитьОбласть("ВремяПечати");
		ОбластьМакета.Параметры.ВремяПечати = НСтр("ru='Дата и время печати: ';ka='Дата и время печати: '") + ТекущаяДата();
		ТабличныйДокумент.Вывести(ОбластьМакета);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='მიღების ტალონი';ru='Талон приемки';en='card acceptance'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='დაბრუნების აქტი';ru='Акт возврата';en='return act'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='სასაქონლო ჩეკი';ru='Товарный чек';en='Sales receipt'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='მიღების ტალონი';ru='Талон приемки';en='card acceptance'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_АктВозврата";
	КомандаПечати.Представление = НСтр("ka='დაბრუნების აქტი';ru='Акт возврата';en='return act'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
