...
Процедура ОбновитьДокументПродажи()
	...
	Если ИспользоватьСерийныеНомера И ЗначениеЗаполнено(Объект.СерийныйНомер) И ЗначениеЗаполнено(Объект.Номенклатура)
		...
		Иначе	
			ДокументПродажиСсылка = НСтр("ka='<Не найден>';ru='<Не найден>';en='<Not found>'");
			Элементы.ДокументПродажи.Гиперссылка = Ложь;
		КонецЕсли;
		
		Если ГарантийныйСрок>0 Тогда
			Элементы.ТекстГарантии.Заголовок = НСтр("ka='Гарантийный срок: ';ru='Гарантийный срок: '")+ГарантийныйСрок+НСтр("ka=' мес.';ru=' мес.'");
		Иначе
			Элементы.ТекстГарантии.Заголовок = НСтр("ka='გარანტია არ გაიცემა';ru='Гарантия не предоставляется';en='Warranty is not granted'");
		КонецЕсли;
	Иначе	
...
Процедура ИсторияПродажНажатие(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ ნომენკლატურა!';ru='Выберите номенклатуру!';en='Select the range!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПолученыШтрихкодыНекорректногоТипа(ШтрихкодыНекорректногоТипа) Экспорт
	...
	Для каждого ТекНекорректныйШтрихкод Из ШтрихкодыНекорректногоТипа Цикл
		...
		СтрокаСообщения = НСтр("ka='შტრიხკოდ %1%-ით მოძებნილ ნომენკლატურას-%2%-გააჩნია ტიპი %3%, რომელიც ამ დოკუმენტისთვის გამოუსადეგარია.';ru='Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этого документа';en='Found on the barcode% 1% range -% 2% - is of type% 3%, which is not suitable for this document'");
