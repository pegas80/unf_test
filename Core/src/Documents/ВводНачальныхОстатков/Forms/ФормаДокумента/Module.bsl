...
Процедура ВнеоборотныеАктивыОбъемПродукцииРаботДляВычисленияАмортизацииПриИзменении(Элемент)
	...
	Если НЕ СтруктураДанные.СпособАмортизацииПропорциональноОбъемуПродукции Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='""პროდუქციის (სამუშაოების) მოცულობა ამორტიზაციის გამოსათვლელად"" არ შეიძლება შევსებული იქნას ამორტიზაციის დარიცხვის მითითებული მეთოდისთვის!';ru='""Объем продукции (работ) для исчисления амортизации"" не может быть заполнен для указанного способа начисления амортизации!'"));
...
Процедура ВнеоборотныеАктивыСрокИспользованияДляВычисленияАмортизацииПриИзменении(Элемент)
	...
	Если СтруктураДанные.СпособАмортизацииПропорциональноОбъемуПродукции Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='""ამორტიზაციის გამოსათვლელად გამოყენების ვადა"" არ შეიძლება შევსებული იქნას ამორტიზაციის დარიცხვის მითითებული მეთოდისთვის!';ru='""Срок использования для вычисления амортизации"" не может быть заполнен для указанного способа начисления амортизации!'"));
...
Процедура ДенежныеСредстваВалютаДенежныхСредствНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтруктураДанные.ТипДенежныхСредств)
	   И СтруктураДанные.ТипДенежныхСредств <> Наличные Тогда
		ПоказатьПредупреждение(Неопределено,НСТР("ka='საბანკო ანგარიშისთვის არ შეიძლება ფულადი სახსრების ვალუტის შეცვლა!';ru='Для банковского счета нельзя изменить валюту денежных средств!'"));
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ПрочиеРазделыВалютаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//		ПоказатьПредупреждение(Неопределено,НСтр("ka='არჩეულ ანგარიშზე არ არის დაყენებული სავალუტო თვისება!';ru='У выбранного счета не установлен признак валютный!'"));
	//	Иначе
	//		ПоказатьПредупреждение(Неопределено,НСтр("ka='დასაწყისში მიუთითეთ ანგარიში!';ru='Укажите в начале счет!'"));
	//	КонецЕсли;
	//КонецЕсли;
...
Процедура ЗапасыВРазрезеГТДНомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДанныеТекущейСтроки.СтранаПроисхождения) Тогда
			...
			ТекстСообщения = НСтр("ka='წარმოშობის ქვეყანა არ არის შევსებული!';ru='Не заполнена страна происхождения!'");
		...
		Если ЗначениеЗаполнено(ДанныеТекущейСтроки.СтранаПроисхождения)
		   И ДанныеТекущейСтроки.СтранаПроисхождения = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
			...
			ТекстСообщения = НСтр("ka='Учет ГТД для отечественных товаров не ведется!';ru='Учет ГТД для отечественных товаров не ведется!'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		...
		Если (ЗначениеЗаполнено(ДанныеТекущейСтроки.Номенклатура)
			...
			ТекстСообщения = НСтр("ka='Учет ГТД в программе ведеться только для номенклатуры с типом ""Запас"".';ru='Учет ГТД в программе ведеться только для номенклатуры с типом ""Запас"".'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			СтандартнаяОбработка = Ложь;
...
Процедура СформироватьСписокРазделовУчета()
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуЗарплата.Получить()
		...
		Элементы.РазделУчета.СписокВыбора.Добавить("Расчеты с персоналом", НСтр("ka='ანგარიშსწორებები პერსონალთან';ru='Расчеты с персоналом'"));
	КонецЕсли;
	...
	Если Константы.ФункциональнаяОпцияУчетВнеоборотныхАктивов.Получить()
		...
		Элементы.РазделУчета.СписокВыбора.Добавить("Имущество", НСтр("ka='ქონება';ru='Имущество'"));
	КонецЕсли;
	
	// Прочее.
	//Элементы.РазделУчета.СписокВыбора.Добавить("Прочие разделы", НСтр("ka='სხვა განყოფილებები';ru='Прочие разделы'"));
	...
	Если Не ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") 
		...
		Элементы.РазделУчета.СписокВыбора.Добавить("Эквайринг", НСтр("ka='ეკვაირინგი';ru='Эквайринг'"));
	КонецЕсли;
	// Конец Эквайринг
...
Процедура ОбработатьПодготовленныеДанныеЗапасы(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ka='მონაცემების ჩატვირთვა';ru='Загрузка данных'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ВводНачальныхОстатков, , Нстр("ru='Запасы: ';ka='მარაგები'") + ОписаниеОшибки());
		ОтменитьТранзакцию();
...
Процедура ОбработатьПодготовленныеДанныеРасчетыСПокупателями(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ka='მონაცემების ჩატვირთვა';ru='Загрузка данных'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ВводНачальныхОстатков, , Нстр("ru='Расчеты с покупателями: ';ka='ანგარიშსწორებები მყიდველებთან'") + ОписаниеОшибки());
		ОтменитьТранзакцию();
