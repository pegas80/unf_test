...
Процедура ИнициализироватьДанныеДокументаУправленческий(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("Содержание", НСтр("ka='საწყისი ნაშთების შეყვანა';ru='Ввод начальных остатков'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПодотчетниками(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеДолгаПодотчетника", НСтр("ka='საქვეანგარიშოს დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности подотчетника'"));
	Запрос.УстановитьПараметр("ПогашениеДолгаПодотчетнику", НСтр("ka='საქვეანგარიშოს მიმართ დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности перед подотчетником'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПерсоналом(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПерсоналом", НСтр("ka='პერსონალის წინაშე ვალდებულებების წარმოშობა';ru='Возникновение обязательств перед персоналом'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоНалогам(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("НачислениеНалога", НСтр("ka='ბიუჯეტის წინაშე დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности перед бюджетом'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПокупателями(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателя", НСтр("ka='მყიდველთა დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности покупателей'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПокупателя", НСтр("ka='მყიდველებისგან ავანსის ნაშთების შეყვანა';ru='Ввод остатков авансов от покупателей'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПоставщиками(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПоставщиком", НСтр("ka='მომწოდებელებისადმი დავალიანების ნაშთის შეყვანა';ru='Ввод остатков задолженности поставщикам'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПередПоставщиком", НСтр("ka='მომწოდებლებისთვის ავანსის ნაშთების შეყვანა';ru='Ввод остатков авансов поставщикам'"));
...
Процедура ИнициализироватьДанныеДокументаДенежныеСредства(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("СодержаниеПроводки", НСтр("ka='ფულადი სახსრების ნაშთის შეყვანა';ru='Ввод остатков денежных средств'"));
...
Процедура ИнициализироватьДанныеДокументаЗапасы(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("ОприходованиеЗатрат", НСтр("ka='დანახარჯების შემოსავალში გატარება';ru='Оприходование затрат'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасовПринятых", НСтр("ka='მიღებული მარაგების შემოსავალში გატარება';ru='Оприходование запасов принятых'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасовПереданных", НСтр("ka='გადაცემული მარაგების შემოსავალში გატარება';ru='Оприходование запасов переданных'"));
	Запрос.УстановитьПараметр("ПриемЗапасов", НСтр("ka='მარაგების მიღება';ru='Прием запасов'"));
...
Процедура ИнициализироватьДанныеВнеоборотныеАктивы(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПринятиеВнеоборотногоАктиваКУчету", НСтр("ka='გრძელვადიანი აქტივების მიხედვით საწყისი ნაშთების შეყვანა';ru='Ввод начальных остатков по внеоборотным активам'"));
	Запрос.УстановитьПараметр("НачислениеАмортизации", НСтр("ka='ამორტიზაციის მიხედვით საწყისი ნაშთების შეყვანა';ru='Ввод начальных остатков по амортизации'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоЭквайрингу(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("РасчетыПоЭквайрингу", НСтр("ka='ეკვაირინგის მიხედვით ანგარიშსწორებების ნაშთის შეყვანა';ru='Ввод остатков расчетов по эквайрингу'"));
