  ...
  ИЛИ НЕ Автоформирование Тогда
	...
	Для каждого Строка Из РасчетыСПокупателями Цикл
		Если НЕ ЗначениеЗаполнено(Строка.Документ)
			...
			СтрокаКомментарий = Нстр("ka='ფორმირებულია ავტომატურად დოკუმენტით საწყისი ნაშთების შეყვანა  № %Номер% %Дата%';ru='Сформирован автоматически документом Ввод начальных остатков № %Номер% от %Дата%'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого СтрокаТЧ Из ПрочиеРазделы Цикл
		...
		И НЕ ЗначениеЗаполнено(СтрокаТЧ.Валюта) Тогда
			ТекстСообщения = НСтр("ka='სია ""სხვა დანარჩენი ნაწილები""-ს სტრიქონ %НомерСтроки%-ში არსებული სავალუტო ანგარიშისთვის სვეტი ""ვალუტა"" არ არის შევსებული.';ru='Не заполнена колонка ""Валюта"" для валютного счета в строке %НомерСтроки% списка ""Прочие разделы"".'");
		...
		И НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаВал) Тогда
			ТекстСообщения = НСтр("ka='სია ""სხვა დანარჩენი ნაწილები""-ს სტრიქონ %НомерСтроки%-ში არსებული სავალუტო ანგარიშისთვის სვეტი ""თანხა (ვალ.)"" არ არის შევსებული.';ru='Не заполнена колонка ""Сумма (вал.)"" для валютного счета в строке %НомерСтроки% списка ""Прочие разделы"".'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
	...
	Для каждого СтрокаТЧ Из РасчетыСПокупателями Цикл
		...
		И НЕ ЗначениеЗаполнено(СтрокаТЧ.Документ) Тогда
			ТекстСообщения = НСтр("ka='სია ""მყიდველებთან ანგარიშსწორებების"" სტრიქონ %НомерСтроки%-ში სვეტი ""ანგარიშსწორებების დოკუმენტი"" არ არის შევსებული.';ru='Не заполнена колонка ""Документ расчетов"" в строке %НомерСтроки% списка ""Расчеты с покупателями"".'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
