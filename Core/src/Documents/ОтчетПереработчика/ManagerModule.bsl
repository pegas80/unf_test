...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПоложительнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Хозрасчетный.ПустаяСсылка());
	Запрос.УстановитьПараметр("ОтрицательнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Хозрасчетный.ПустаяСсылка());
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыУслуга.Количество() - 1 Цикл
		...
		Если СтрокаТаблицаЗапасы.Сумма > 0 Тогда
			...
			Если СтрокаТаблицаХозрасчетный.СчетДт.Валютный Тогда 
				СтрокаТаблицаХозрасчетный.ВалютаДт = СтрокаТаблицаЗапасы.ВалютаДт;
			КонецЕсли;
			
			Если СтрокаТаблицаХозрасчетный.СчетКт.Валютный Тогда 
				СтрокаТаблицаХозрасчетный.ВалютаКт = СтрокаТаблицаЗапасы.ВалютаКт;
			КонецЕсли;
			...
			СтрокаТаблицаХозрасчетный.ВидСубконтоКт1 = "Контрагенты";
			СтрокаТаблицаХозрасчетный.ВидСубконтоКт2 = "Договоры";
			
			СтрокаТаблицаХозрасчетный.СубконтоДт1 = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
			...
			СтрокаТаблицаХозрасчетный.СубконтоКт1 = ДокументСсылкаОтчетПереработчика.Контрагент;
			СтрокаТаблицаХозрасчетный.СубконтоКт2 = ДокументСсылкаОтчетПереработчика.Договор;
			
			СтрокаТаблицаХозрасчетный.Сумма = СтрокаТаблицаЗапасы.Сумма - СтрокаТаблицаЗапасы.СуммаНДС;
		...
		Если СтрокаТаблицаЗапасы.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда
			...
			СтрокаТаблицыПриход.КоррСчетУчетаРегл = Неопределено; //+++ Дмгг
			...
			СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
			...
			СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл; //+++ Дмгг
		...
		Иначе
			...
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
			СтрокаТаблицыПриход.Сумма = СтрокаТаблицаЗапасы.Сумма - СтрокаТаблицаЗапасы.СуммаНДС;
			...
			СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;
			СтрокаТаблицыПриход.КоррСчетУчета = Неопределено;
			СтрокаТаблицыПриход.КоррСчетУчетаРегл = Неопределено;
			СтрокаТаблицыПриход.КоррНоменклатура = Неопределено;
			СтрокаТаблицыПриход.КоррХарактеристика = Неопределено;
			СтрокаТаблицыПриход.КоррПартия = Неопределено;
			СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
...
Процедура ИнициализироватьДанныеПоУслуге(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, СуммаУслуги) Экспорт
	...
	|	ОтчетПереработчика.Ссылка.Расход.СчетУчетаЗатратРегл КАК СчетУчетаРегл,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|			ТОГДА ОтчетПереработчика.Номенклатура.СчетУчетаЗапасовРегл
	|		ИНАЧЕ ОтчетПереработчика.Номенклатура.СчетУчетаЗатратРегл
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	|			ТОГДА ОтчетПереработчика.Ссылка.Расход.СчетУчетаЗатратРегл
	|		ИНАЧЕ ОтчетПереработчика.Номенклатура.СчетУчетаЗапасовРегл
	|	КОНЕЦ КАК СчетДтРегл,
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ОтчетПереработчика.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщиком КАК СчетКт,
	|	ОтчетПереработчика.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетКтРегл,
	|	ВЫБОР
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ОтчетПереработчика.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ОтчетПереработчика.Ссылка.СуммаНДС * ОтчетПереработчика.Количество / ВТКоличествоПродукции.КоличествоПродукции * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетПереработчика.Ссылка.СуммаНДС * ОтчетПереработчика.Количество / ВТКоличествоПродукции.КоличествоПродукции * ОтчетПереработчика.Ссылка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ОтчетПереработчика.Ссылка.Кратность)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ОтчетПереработчика.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ОтчетПереработчика.Ссылка.СуммаНДС * ОтчетПереработчика.Количество / ВТКоличествоПродукции.КоличествоПродукции * КурсыРегВалюты.Курс * ОтчетПереработчика.Ссылка.Кратность / (ОтчетПереработчика.Ссылка.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетПереработчика.Ссылка.СуммаНДС * ОтчетПереработчика.Количество / ВТКоличествоПродукции.КоличествоПродукции
	...
	Запрос.УстановитьПараметр("ОтражениеРасходовПоПереработке", НСтр("ka='გადამუშავებაზე ხარჯების ასახვა';ru='Отражение расходов по переработке';en='Reflection on processing costs'"));
...
Процедура ИнициализироватьДанныеПоОтходам(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ОтчетПереработчикаОтходы.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл
	|ИЗ
...
Процедура СформироватьТаблицаЗапасыЗапасы(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, СуммаСборки)
	...
	ТекПродукция = Неопределено;
	ТекХарПродукция = Неопределено;
	ВнестиКолвоПродукции = Ложь;
	ТекНоменклатура = Неопределено;
	ТекХарактеристика = Неопределено; 
	ТребуетсяДобавитьПогрешность = ЛОЖЬ;
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
		...
		Если ТекНоменклатура <> СтрокаТаблицаЗапасы.Номенклатура ИЛИ ТекХарактеристика <> СтрокаТаблицаЗапасы.Характеристика Тогда 
			ТекНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
			ТекХарактеристика = СтрокаТаблицаЗапасы.Характеристика; 
			ТребуетсяДобавитьПогрешность = Истина;
		КонецЕсли;
		
		Если ТекПродукция <> СтрокаТаблицаЗапасы.КоррНоменклатура ИЛИ ТекХарПродукция <> СтрокаТаблицаЗапасы.КоррХарактеристика Тогда 
			ТекПродукция = СтрокаТаблицаЗапасы.КоррНоменклатура;
			ТекХарПродукция = СтрокаТаблицаЗапасы.КоррХарактеристика; 
			ВнестиКолвоПродукции = Истина;
		КонецЕсли;
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
			СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПереработчикуРегл;
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
				...
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					...
					Если ТребуетсяДобавитьПогрешность Тогда 
						ПоискСтрокиПродукции = ДокументСсылкаОтчетПереработчика.Продукция.НайтиСтроки(Новый Структура("Номенклатура,Характеристика,Спецификация",СтрокаТаблицыПриход.Номенклатура,СтрокаТаблицыПриход.Характеристика,СтрокаТаблицыПриход.Спецификация));
						Если ПоискСтрокиПродукции.Количество() <> 0 Тогда 
							КоличествоПродукции = ПоискСтрокиПродукции[0].Количество;
						КонецЕсли;
					КонецЕсли;
					
					СтрокаТаблицаХозрасчетный = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаХозрасчетный.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицаХозрасчетный, СтрокаТаблицыПриход);
					
					СтрокаТаблицаХозрасчетный.СчетДт = СтрокаТаблицыПриход.Номенклатура.СчетУчетаЗапасовРегл;//СтрокаТаблицыПриход.СчетКтРегл;
					СтрокаТаблицаХозрасчетный.СчетКт = СтрокаТаблицыПриход.СчетКтРегл;//СтрокаТаблицыПриход.КоррНоменклатура.СчетУчетаПереработчикуРегл;
					
					Если СтрокаТаблицаХозрасчетный.СчетДт.Количественный Тогда 
						Если ВнестиКолвоПродукции Тогда 
							СтрокаТаблицаХозрасчетный.КоличествоДт = КоличествоПродукции; //СтрокаТаблицыПриход.Количество;
						Иначе
					...
					Если СтрокаТаблицаХозрасчетный.СчетКт.Количественный Тогда 
						Если ТребуетсяДобавитьПогрешность Тогда 
							СтрокаТаблицаХозрасчетный.КоличествоКт = СтрокаТаблицаЗапасы.КоличествоНаЕдиницуПродукции + СтрокаТаблицаЗапасы.ПогрешностьРаспределения;//СтрокаТаблицыПриход.Количество;
						Иначе
							СтрокаТаблицаХозрасчетный.КоличествоКт = СтрокаТаблицаЗапасы.КоличествоНаЕдиницуПродукции;
						КонецЕсли;
					КонецЕсли;
					...
					СтрокаТаблицаХозрасчетный.СубконтоДт1 = СтрокаТаблицыПриход.СтруктурнаяЕдиница;
					СтрокаТаблицаХозрасчетный.СубконтоДт2 = СтрокаТаблицыПриход.Номенклатура;
					СтрокаТаблицаХозрасчетный.СубконтоДт3 = СтрокаТаблицыПриход.Характеристика;
					
					СтрокаТаблицаХозрасчетный.СубконтоКт1 = СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница;
					СтрокаТаблицаХозрасчетный.СубконтоКт2 = СтрокаТаблицыПриход.КоррНоменклатура;
					СтрокаТаблицаХозрасчетный.СубконтоКт3 = СтрокаТаблицыПриход.КоррХарактеристика;
...
Процедура ИнициализироватьДанныеПоЗапасам(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, СуммаСборки) Экспорт
	...
	|	ОтчетПереработчикаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,//+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,//+++ Дмгг
	|	ОтчетПереработчикаЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,//+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,//+++ Дмгг
	...
	|	ОтчетПереработчикаЗапасы.СчетДтРегл КАК СчетДтРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетКтРегл КАК СчетКтРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетУчетаПереработчикуРегл КАК СчетУчетаПереработчикуРегл, //+++ Дмгг 
	...
	|   ОтчетПереработчикаЗапасы.МаксПозицийПродукции КАК МаксПозицийПродукции,
	|   ОтчетПереработчикаЗапасы.КоличествоНаЕдиницуПродукции КАК КоличествоНаЕдиницуПродукции,
	|   ОтчетПереработчикаЗапасы.ПогрешностьРаспределения КАК ПогрешностьРаспределения
	...
	|	ОтчетПереработчикаЗапасы.КоррСчетУчета КАК КоррСчетУчета,
	|	ОтчетПереработчикаЗапасы.СчетУчетаПродукции КАК СчетУчетаПродукции,
	|	ОтчетПереработчикаЗапасы.СчетУчетаПереработчикуРегл КАК СчетУчетаРегл, //СчетУчетаРегл КАК СчетУчетаРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл, //+++ Дмгг
	|	ОтчетПереработчикаЗапасы.Номенклатура КАК Номенклатура,
	...
	|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл, //+++ Дмгг
	|	ТаблицаЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл, //+++ Дмгг
	|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
	...
	|	ТаблицаЗапасы.КоррСчетУчетаРегл КАК СчетДтРегл, //+++ Дмгг
	|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетКтРегл, //+++ Дмгг
	...
	|	0 КАК Сумма,
	|   ТаблицаЗапасы.МаксПозицийПродукции КАК МаксПозицийПродукции,
	|   ТаблицаЗапасы.КоличествоНаЕдиницуПродукции КАК КоличествоНаЕдиницуПродукции,
	|   ТаблицаЗапасы.ПогрешностьРаспределения КАК ПогрешностьРаспределения
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаЗапасы.СчетУчетаРегл, //+++ Дмгг
	|	ТаблицаЗапасы.КоррСчетУчетаРегл, //+++ Дмгг
	|	ТаблицаЗапасы.СчетУчетаПереработчикуРегл, //+++ Дмгг
	|	ТаблицаЗапасы.Партия,
	...
	|	ТаблицаЗапасы.СчетУчета,
	|	ТаблицаЗапасы.СодержаниеПроводки,
	|   ТаблицаЗапасы.МаксПозицийПродукции,
	|   ТаблицаЗапасы.КоличествоНаЕдиницуПродукции,
	|   ТаблицаЗапасы.ПогрешностьРаспределения
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|;
		   ...
		   И ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.ЗаказПоставщику) Тогда
			...
			Если КоличествоВыходаВРезерв > 0 Тогда	
				...
				Для Индекс = 0 по СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРазмещениеЗаказов.Количество() - 1 Цикл
					...
					СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл; //+++ Дмгг
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыСПоставщикамиРегл", МассивРезультатов[НомерЗапроса].Выгрузить());
...
Процедура ИнициализироватьДанныеРасчетыСПоставщиками(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства)
	...
	|	ОтчетПереработчикаЗатраты.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРасчетовСПоставщикомРЕгл,
	|	ОтчетПереработчикаЗатраты.Договор КАК Договор,
	|	ОтчетПереработчикаЗатраты.Расход.СчетУчетаЗатрат КАК СчетУчета,
	|	ОтчетПереработчикаЗатраты.Расход.СчетУчетаЗатратРегл КАК СчетУчетаРегл,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаШапка.ВестиУчетРасходовПоДоговорамОбслуживания
	|		ТОГДА ВременнаяТаблицаШапка.НаправлениеДеятельности
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|			ТОГДА ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасовРегл
	|		ИНАЧЕ ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗатратРегл
	|	КОНЕЦ КАК СчетДтРегл,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|			ТОГДА ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасовРегл
	...
	|	ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,
	|	ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаПереработчикуРегл КАК СчетУчетаПереработчикуРегл
	...
	|	ТаблицаПродукция.СчетУчетаЗапасовРегл,
	|	ТаблицаПродукция.СчетУчетаПереработчикуРегл
	|ИЗ
	|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ОтчетПереработчика.Организация.ВидНалогаНДС.СчетУчетаКВозмещениюРегл КАК СчетДт,
	|	ОтчетПереработчика.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетКт,
	|	ОтчетПереработчика.ВалютаДокумента КАК Валюта,
	...
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_ОтчетПереработчика(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства); //+++ Дмгг
	
	СформироватьТаблицаЗакупкиДляКУДиР(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства);
	
	СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства);
...
Процедура СформироватьТаблицуРаспределенияМатериалов(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, ТаблицаПродукция) Экспорт
	...
	|	ТаблицаПродукция.СчетУчетаПереработчикуРегл КАК СчетУчетаПереработчикуРегл,
	...
	|	ТаблицаСоставПродукции.СчетУчетаПереработчикуРегл КАК СчетУчетаПереработчикуРегл,
	...
	|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
	|	ВЫБОР
	|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
	|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
	|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
	...
	|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетКтПродукцииРегл,
	|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаПереработчикуРегл КАК СчетУчетаПереработчикуРегл,
	|	ЛОЖЬ КАК Распределено,
	|	МАКСИМУМ(СборкаЗапасовПродукция.НомерСтроки) КАК МаксПозицийПродукции,
	|	0 КАК КоличествоНаЕдиницуПродукции,
	|	0 КАК ПогрешностьРаспределения
	|ИЗ
	|	Документ.ОтчетПереработчика.Запасы КАК СборкаЗапасовЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.Продукция КАК СборкаЗапасовПродукция
	|		ПО СборкаЗапасовЗапасы.Ссылка = СборкаЗапасовПродукция.Ссылка
	|ГДЕ
	...
	|	СборкаЗапасовЗапасы.Ссылка.ЗаказПокупателя,
	|	СборкаЗапасовЗапасы.Ссылка.ДокументОснование,
	|	СборкаЗапасовЗапасы.Ссылка.Контрагент,
	|	СборкаЗапасовЗапасы.Ссылка.Договор,
	|	СборкаЗапасовЗапасы.Ссылка.Курс,
	|	СборкаЗапасовЗапасы.Ссылка.Кратность,
	|	СборкаЗапасовЗапасы.Ссылка.ВалютаДокумента,
	|	СборкаЗапасовЗапасы.Ссылка.Дата,
	|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница,
	|	СборкаЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент,
	|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад,
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	|			ТОГДА СборкаЗапасовЗапасы.Ссылка.Ячейка
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
	...
	|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница
	|	КОНЕЦ,
	|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов,
	|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА СборкаЗапасовЗапасы.Количество
	|		ИНАЧЕ СборкаЗапасовЗапасы.Количество * СборкаЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ,
	|	СборкаЗапасовЗапасы.Всего,
	|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаПереработчикуРегл,
	|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница,
	|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад,
	|	СборкаЗапасовЗапасы.Ссылка.ЗаказПокупателя,
	|	ВЫБОР
	...
	Пока Инд < КоличествоПродукции Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТаблицаСоставПродукции.Свернуть("КоррНоменклатура,КоррХарактеристика,КоррПартия,КоррСпецификация,КоррСчетУчета,КоррСчетУчетаРегл,СчетУчетаПродукции,СчетУчетаПродукцииРегл,СчетДт,СчетДтРегл,СчетДтПродукции,СчетДтПродукцииРегл,СчетКтПродукции,СчетКтПродукцииРегл,КоррКоличество,ТМНоменклатура,ТМХарактеристика,Распределено", "ТМКоличество");
	...
	Для каждого стр Из ТаблицаМатериалов Цикл
		...
		стр.КоличествоНаЕдиницуПродукции = стр.ИсходноеКоличество / стр.МаксПозицийПродукции;
		стр.ПогрешностьРаспределения = стр.ИсходноеКоличество - ((стр.ИсходноеКоличество / стр.МаксПозицийПродукции) * стр.МаксПозицийПродукции);
	КонецЦикла;
...
Процедура ВыполнитьКонтроль(ДокументСсылкаОтчетПереработчика, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если Константы.КонтролироватьОстаткиЗапасовНаСкладахПриПроведении.Получить() Тогда
		...
		Результат = УправлениеНебольшойФирмойСервер.ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(ДокументСсылкаОтчетПереработчика);
		
		Если НЕ Результат.Пустой() Тогда
			ДокументСсылкаОтчетПереработчика_Объект = ДокументСсылкаОтчетПереработчика.ПолучитьОбъект();
			ВыборкаИзРезультатаЗапроса = Результат.Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументСсылкаОтчетПереработчика_Объект, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;			
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаРасчетыПоНалогам.СуммаНДС КАК Сумма
	|ИЗ
