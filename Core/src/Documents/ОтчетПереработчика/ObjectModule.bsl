...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная, "ВидОперации")
		...
		ВызватьИсключение НСтр("ka='გადამუშავების თაობაზე ანგარიშგების შეყვანა ხდება მხოლოდ დამუშავებაზე გადაცემის საფუძველზე!';ru='Отчет о переработке вводится только на основании передачи в переработку!';en='Report on processing introduced only on the basis of the transmission for processing!'");;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ОтразитьХозрасчетный(ДополнительныеСвойства, Движения, Отказ);//+++ Дмгг
	УправлениеНебольшойФирмойСервер.ОтразитьРасчетыПоНалогам(ДополнительныеСвойства, Движения, Отказ);//+++ Дмгг
