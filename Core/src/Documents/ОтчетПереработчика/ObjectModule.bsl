...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная, "ВидОперации")
		...
		ВызватьИсключение НСтр("ka='გადამუშავების თაობაზე ანგარიშგების შეყვანა ხდება მხოლოდ დამუშავებაზე გადაცემის საფუძველზე!';ru='Отчет о переработке вводится только на основании передачи в переработку!'");;
	КонецЕсли;
