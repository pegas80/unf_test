...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАмортизацияВА, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Сообщение1",	   НСтр("ka='ამორტიზაცია ამ თვეს უკვე დაირიცხა!';ru='амортизация в этом месяце уже начислялась!'"));
	Запрос.УстановитьПараметр("Сообщение2",	   НСтр("ka='ამორტიზაციის დარიცხვის მეთოდი არ არის მითითებული!';ru='не указан способ начисления амортизации!'"));
	Запрос.УстановитьПараметр("Сообщение3",	   НСтр("ka='ღირებულება არის 0-ის ტოლი!';ru='стоимость равна 0!'"));
	Запрос.УстановитьПараметр("Сообщение4",	   НСтр("ka='გამოყენების ვადა ტოლია 0-ის!';ru='срок использования равен 0!'"));
	Запрос.УстановитьПараметр("Сообщение5",	   НСтр("ka='სამუშაოების პროდუქციის მოცულობა ამორტიზაციის გამოსათვლელად შეუვსებელია!';ru='объем продукции работ для вычисления амортизации не заполнен!'"));
	Запрос.УстановитьПараметр("Сообщение6",	   НСтр("ka='თავდაპირველი ღირებულება ტოლია 0!';ru='первоначальная стоимость равна 0!'"));
...
Процедура СформироватьТаблицаВнеоборотныеАктивы(ДокументСсылкаАмортизацияВА, СтруктураДополнительныеСвойства)
	...
	|	СУММА(ТаблицаДокумента.Амортизация) КАК Амортизация,
	|	СУММА(ТаблицаДокумента.Амортизация) КАК Сумма,
	...
	|	ПараметрыВнеоборотныхАктивовСрезПоследних.СчетЗатратРегл";
...
Процедура СформироватьТаблицаОшибкиЗакрытияМесяца(ДокументСсылкаАмортизацияВА, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("НачислениеАмортизации", НСтр("ka='ამორტიზაციის დარიცხვა';ru='Начисление амортизации'"));
	Запрос.УстановитьПараметр("АмортизацияРавнаНулю", НСтр("ka='გაანგარიშებული ამორტიზაცია არის 0-ის ტოლი!';ru='расчитанная амортизация равна 0!'"));
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	...
	Для каждого ТекСтрока Из ТаблицаРезультата Цикл
		ТекстОшибки = НСтр("ka='ქონება %ВнеоборотныйАктивПредставление% -ისთვის (%Код%) %СообщениеОбОшибкеНачисленияАмортизации%.';ru='Для имущества %ВнеоборотныйАктивПредставление% (%Код%) %СообщениеОбОшибкеНачисленияАмортизации%.'");
