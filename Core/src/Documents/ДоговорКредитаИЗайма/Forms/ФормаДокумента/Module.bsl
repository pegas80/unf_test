...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		Если ЕстьДвиженияПоДоговору(Объект.Ссылка) Тогда
			Элементы.ГруппаСчетаУчетаКолонки.Подсказка = НСтр("ka='ბაზაში არის ამ ხელშეკრულებასთან დაკავშირებული მოძრაობები! აღრიცხვის ანგარიშების შეცვლა არ არის რეკომენდებული!';ru='В базе есть движения по этому договору! Изменение счетов учета не рекомендуется!'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьНапоминанияОбОплате(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='გთხოვთ, ჩაიწეროთ დოკუმენტი!';ru='Пожалуйста, запишите документ!'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.ГрафикПлатежейИНачислений.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='გადახდის გრაფიკი არ არის შევსებული!';ru='Не заполнен график платежей!'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаДоговора()
	...
	Если Объект.ВидДоговора = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКредитаИЗайма.КредитПолученный") Тогда
		...
		Элементы.СтраницаВыдача.Заголовок = НСтр("ka='სესხის მიღება';ru='Получение кредита'");
	...
	Иначе
		...
		Элементы.СтраницаВыдача.Заголовок = НСтр("ka='სესხის გაცემა';ru='Выдача займа'");
