...
Процедура Создать(Команда)
	...
	Если Пользователь.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='აირჩიეთ მომხმარებელი!';ru='Выберите пользователя!'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Время) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='დააყენეთ შეხსენების დრო!';ru='Установите время напоминания!'"));
		Возврат;
	КонецЕсли;
