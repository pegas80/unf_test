...
Процедура Создать(Команда)
	...
	Если Пользователь.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='აირჩიეთ მომხმარებელი!';ru='Выберите пользователя!';en='Select the user!'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Время) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='დააყენეთ შეხსენების დრო!';ru='Установите время напоминания!';en='Set the reminder time!'"));
		Возврат;
	КонецЕсли;
