...
Процедура ЗаполнитьПоРасходуИзКассы(ДокументСсылкаРасходИзКассы) Экспорт
	...
	Если ДокументСсылкаРасходИзКассы.ВидОперации <> Перечисления.ВидыОперацийРасходИзКассы.Подотчетнику Тогда
		ВызватьИсключение НСтр("ka='საავანსო ანგარიშგების შეყვანა შეუძლებელია სალაროდან გასავლის საფუძველზე ოპერაციის ამ ტიპით!';ru='Нельзя ввести Авансовый отчет на основании расхода из кассы с этим видом операции!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходуСоСчета(ДокументСсылкаРасходСоСчета) Экспорт
	...
	Если ДокументСсылкаРасходСоСчета.ВидОперации <> Перечисления.ВидыОперацийРасходСоСчета.Подотчетнику Тогда
		ВызватьИсключение НСтр("ka='საავანსო ანგარიშგების შეყვანა შეუძლებელია სალაროდან გასავლის საფუძველზე ოპერაციის ამ ტიპით!';ru='Нельзя ввести Авансовый отчет на основании расхода со счета с этим видом операции!'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ИзрасходованоИтог > ЗапасыИтог + РасходыИтог + ОплатыИтог + ЗПИтог Тогда
		ТекстСообщения = НСтр("ka='ავანსების გახარჯული თანხა აღემატება თანხას დოკუმენტის მიხედვით!';ru='Израсходованная сумма авансов превышает сумму по документу!'");
	...
	Для каждого СтрокаОплаты Из Оплаты Цикл
		Если СтрокаОплаты.Контрагент.ВестиРасчетыПоДокументам
		   ...
		   И НЕ ЗначениеЗаполнено(СтрокаОплаты.Документ) Тогда
			ТекстСообщения = НСтр("ka='სია ""გადახდების"" სტრიქონ %НомерСтроки% სვეტი ""ანგარიშსწორებათა დოკუმენტი"" შეუვსებელია.';ru='Не заполнена колонка ""Документ расчетов"" в строке %НомерСтроки% списка ""Оплаты"".'");
	...
	Для каждого СтрокаРасходы Из Расходы Цикл
		...
		Если НЕ ИспользоватьТолькоРегламентированныйПланСчетов Тогда //+++ Дмгг
			Если ПолучитьФункциональнуюОпцию("УчетПоНесколькимПодразделениям")
				...
				ТекстСообщения = НСтр("ka='სია ""ხარჯების""  სტრიქონ  %НомерСтроки% მითითებული ნომენკლატურისთვის ""%Номенклатура%"" უნდა შეივსოს რეკვიზიტი ""ქვეგანყოფილება"".';ru='Для номенклатуры ""%Номенклатура%"" указанной в строке %НомерСтроки% списка ""Расходы"", должен быть заполнен реквизит ""Подразделение"".'"
				);
