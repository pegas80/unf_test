...
Процедура СформироватьТаблицаРасчетыСПодотчетниками(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПогашениеДолгаПодотчетника", НСтр("ka='საქვეანგარიშო პირის ვალის დაფარვა';ru='Погашение долга подотчетника'"));
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	КонецЕсли;
	//Паганини ПФ_
	Запрос.УстановитьПараметр("ЗатратаПенсионногоФонда",	НСтр("ka='ორგანიზაციის ხარჯი საპენსიო ფონდისთვის';ru='Удержание организации для пенсионного фонда'"));
	//_Паганини ПФ
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", НСтр("ka='მარაგების შემოსავალში აყვანა';ru='Оприходование запасов'"));
	Запрос.УстановитьПараметр("ОплатаПоставщику", НСтр("ka='მომწოდებლისათვის გადახდა';ru='Оплата поставщику'"));
	Запрос.УстановитьПараметр("ПрочиеРасходы", НСтр("ka='სხვა ხარჯები';ru='Прочие расходы'"));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='საავანსო ანგარიში';ru='Авансовый отчет'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "АвансовыйОтчетВВалюте";
	КомандаПечати.Представление = НСтр("ka='საავანსო ანგარიშგება (მმართვ.)';ru='Авансовый отчет (упр.)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
