...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОтражениеВзносовНаПФ", НСтр("ru = 'Отражение взносов на ПФ'; ka = 'საპენსიო შენატანი '; en = 'საპენსიო შენატანი '"));
	...
	|	ТаблицаДокумента.Проект,   //Малышев 2024-12-03 ONESOFT-3631
	...
	//Малышев 2024-11-29 ONESOFT-3631 (--
	|	ВЫБОР 
	|		КОГДА ТаблицаДокумента.Номенклатура.МетодОценки = Значение(Перечисление.МетодОценкиЗапасов.FIFO) 
	...
	|	ИСТИНА, 
	|	НЕОПРЕДЕЛЕНО, //Малышев 2024-11-29 ONESOFT-3631
	|	&ПрочиеРасходы   
	//++ ONESOFT-4383
	|	,NULL,
	|	NULL   
	...
	|	И (ВременнаяТаблицаРасходы.СчетЗатратРеглПФ.ТипСчета  = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.КосвенныеЗатраты) 
	|	Или ВременнаяТаблицаРасходы.СчетЗатратРеглПФ.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.НезавершенноеПроизводство))
	 //++ ONESOFT-4383
...
Процедура СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	АвансовыйОтчетЗапасы.Проект как Проект, //Малышев 2024-12-03 ONESOFT-3631
...
Процедура СформироватьТаблицаРасчетыСПодотчетниками(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|		СУММА(ТаблицаДокумента.СуммаПоСотрудникуВал)
	|	ИЗ
	|		ВременнаяТаблицаПлатежныеВедомостиДетальный КАК ТаблицаДокумента
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ТаблицаДокумента.Период,
	|		ТаблицаДокумента.Сотрудник,
	...
	|
	|ВЫБРАТЬ
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.СуммаПоСотруднику
	|ИЗ
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный КАК ВременнаяТаблицаПлатежныеВедомостиДетальный
	//+++ Дмгг, 07.05.2025, ONESOFT-4025
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК Проект, //Малышев 2024-12-04 ONESOFT-3631
	...
	|	ТаблицаДокумента.Период,
	|	ТаблицаДокумента.Организация,
	|	ТаблицаДокумента.Проект, //Малышев 2024-12-04 ONESOFT-3631
	|	ТаблицаДокумента.СтруктурнаяЕдиница,
	|	ВЫБОР
	...
	|                                                                                                           
	//08.10.2025 ONESOFT-4383
	|	И ТаблицаДокумента.СчетЗатратРеглПФ.ТипСчета <> ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.КосвенныеЗатраты) 
	|	И ТаблицаДокумента.СчетЗатратРеглПФ.ТипСчета <> ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.НезавершенноеПроизводство)   
	//08.10.2025 ONESOFT-4383
  	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ТаблицаДокумента.Дата,
	|	ТаблицаДокумента.Организация,
	|	НЕОПРЕДЕЛЕНО, //Малышев 2024-12-04 ONESOFT-3631
	|	НЕОПРЕДЕЛЕНО,
...
Процедура СформироватьТаблицаДоходыИРасходыКассовыйМетод(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Статья КАК Статья,
	|	ТаблицаДокумента.Проект КАК Проект, //Малышев 2024-12-03 ONESOFT-3631
	...
	|	Таблица.Статья,
	|	НЕОПРЕДЕЛЕНО, //Малышев 2024-12-03 ONESOFT-3631
...
Процедура СформироватьТаблицаЗакупки(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗакупки.Проект КАК Проект, //Малышев 2024-12-03 ONESOFT-3631
	...
	|	ТаблицаЗакупки.Проект, //Малышев 2024-12-03 ONESOFT-3631
...
Процедура СформироватьТаблицаРасчетыНДФЛ(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаРасчетыПоНалогам.Проект КАК Проект, //Малышев 2024-12-04 ONESOFT-3631
	...
	|	ТаблицаРасчетыПоНалогам.Проект, //Малышев 2024-12-04 ONESOFT-3631
	...
	|	&ПогашениеНалога КАК СодержаниеПроводки
	|ИЗ
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный КАК ТаблицаРасчетыПоНалогам
	|ГДЕ
	|	НЕ ТаблицаРасчетыПоНалогам.ВидУдержанияНДФЛ = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)
	|	И НЕ ТаблицаРасчетыПоНалогам.НДФЛПлатежа = 0
	|
...
Процедура СформироватьТаблицаРасчетыСПерсоналом(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства) //+++ Дмгг
	...
	//ПодольскийАИ #УчетПроектов 14.07.2021 {
	|	Значение(Справочник.Проекты.БезПроекта) КАК Проект,
	//ПодольскийАИ #УчетПроектов 14.07.2021 }
	...
	|	ПлатежнаяВедомостьСотрудники.Ссылка.ВалютаРасчетов,
	|	ПлатежнаяВедомостьСотрудники.Ссылка.ПериодРегистрации,
	|	СУММА(ВЫРАЗИТЬ(ВыплатаЗаработнойПлаты.СуммаПоСотруднику КАК ЧИСЛО(15, 2))),
	|	СУММА(ВыплатаЗаработнойПлаты.СуммаПоСотрудникуВал),
	...
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияНДФЛ,
	|	СУММА(ВЫРАЗИТЬ(ПлатежнаяВедомостьСотрудники.НДФЛПлатежа * КурсыВалютКассы.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))),
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияПФ,
	|	СУММА(ПлатежнаяВедомостьСотрудники.СуммаПФПлатежа),
	|	СУММА(ВЫРАЗИТЬ(ПлатежнаяВедомостьСотрудники.СуммаПФПлатежа * КурсыВалютКассы.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))),
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияПФ.ПенсионныйНалогСотрудника.СчетУчетаРегл,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияПФ.ПенсионныйНалогСотрудника.СчетУчетаКВозмещениюРегл,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияПФ.ПенсионныйНалогОрганизации.СчетУчетаРегл,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияПФ.ПенсионныйНалогОрганизации.СчетУчетаКВозмещениюРегл,
	...
	|ИЗ
	|	Документ.ПлатежнаяВедомость.Сотрудники КАК ПлатежнаяВедомостьСотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПлатежныеВедомостиДетальный КАК ВыплатаЗаработнойПлаты
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, ) КАК КурсыВалютКассы
	|			ПО ВыплатаЗаработнойПлаты.ВалютаДокумента = КурсыВалютКассы.Валюта
	|		ПО ПлатежнаяВедомостьСотрудники.Ссылка = ВыплатаЗаработнойПлаты.ПлатежнаяВедомость
	|			И ПлатежнаяВедомостьСотрудники.Сотрудник = ВыплатаЗаработнойПлаты.СотрудникВедомость
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства) Экспорт
	...
	//Малышев 2024-12-03 ONESOFT-3631
	|	ВЫБОР
	...
	|		ВЫБОР
	|			КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)  
	|			КОГДА АвансовыйОтчетЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|				ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
	|			ИНАЧЕ АвансовыйОтчетЗапасы.Проект
	|		КОНЕЦ КАК Проект,
	//Малышев 2024-12-03 ONESOFT-3631|
	|
	...
	|	АвансовыйОтчетРасходы.Проект КАК Проект, //Малышев 2024-12-03 ONESOFT-3631
	...
	|		ВЫБОР
	|			КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)  
	|			КОГДА АвансовыйОтчетРасходы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|				ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
	...
	|	АвансовыйОтчетПлатежныеВедомости.Ссылка.Проект КАК Проект, //Малышев 2024-12-04 ONESOFT-3631
	...
	|	АвансовыйОтчетПлатежныеВедомости.Ссылка.Проект, //Малышев 2024-12-04 ONESOFT-3631
	|	ПлатежнаяВедомостьСотрудники.Сотрудник,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияНДФЛ,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержанияПФ,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержания,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(АвансовыйОтчетПлатежныеВедомостиДетальный.НомерСтроки) КАК НомерСтроки,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ведомость КАК ПлатежнаяВедомость,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Дата КАК Период,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка КАК Ссылка,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Организация КАК Организация,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник КАК Сотрудник,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПодотчетниками КАК СчетРасчетовСПодотчетниками,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовПоПерерасходу КАК СчетРасчетовПоПерерасходу,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПерсоналом КАК СчетРасчетовСПерсоналом,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПодотчетникамиРегл КАК СчетРасчетовСПодотчетникамиРегл,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовПоПерерасходуРегл КАК СчетРасчетовПоПерерасходуРегл,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПерсоналомРегл КАК СчетРасчетовСПерсоналомРегл,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Проект КАК Проект, 
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Сотрудник КАК СотрудникВедомость,
	|	СУММА(ВЫРАЗИТЬ(АвансовыйОтчетПлатежныеВедомостиДетальный.Сумма * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Кратность) КАК ЧИСЛО(15, 2))) КАК СуммаПоСотруднику,
	|	СУММА(АвансовыйОтчетПлатежныеВедомостиДетальный.Сумма) КАК СуммаПоСотрудникуВал,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.ВидУдержанияПФ КАК ВидУдержанияПФ,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержания КАК ВидУдержания,
	|	ПлатежнаяВедомостьСотрудники.Ссылка.ВидОперации КАК ВедомостьВидОперации,
	|	СУММА(ВЫРАЗИТЬ(АвансовыйОтчетПлатежныеВедомостиДетальный.НДФЛПлатежа * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Кратность) КАК ЧИСЛО(15, 2))) КАК НДФЛПлатежа,
	|	СУММА(АвансовыйОтчетПлатежныеВедомостиДетальный.НДФЛПлатежа) КАК НДФЛПлатежаВал,
	|	СУММА(ВЫРАЗИТЬ(АвансовыйОтчетПлатежныеВедомостиДетальный.СуммаПФПлатежа * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Кратность) КАК ЧИСЛО(15, 2))) КАК СуммаПФПлатежа,
	|	СУММА(АвансовыйОтчетПлатежныеВедомостиДетальный.СуммаПФПлатежа) КАК СуммаПФПлатежаВал,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	СУММА(АвансовыйОтчетПлатежныеВедомостиДетальный.СуммаЛгот) КАК СуммаЛгот,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ведомость.ПериодРегистрации КАК ПериодРегистрации
	|ПОМЕСТИТЬ ВременнаяТаблицаПлатежныеВедомостиДетальный
	|ИЗ
	|	Документ.АвансовыйОтчет.ПлатежныеВедомостиДетальный КАК АвансовыйОтчетПлатежныеВедомостиДетальный
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|				&МоментВремени,
	|				Валюта В
	|					(ВЫБРАТЬ
	...
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПлатежнаяВедомость.Сотрудники КАК ПлатежнаяВедомостьСотрудники
	|		ПО АвансовыйОтчетПлатежныеВедомостиДетальный.Ведомость = ПлатежнаяВедомостьСотрудники.Ссылка
	|ГДЕ
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ведомость,
	|	ПлатежнаяВедомостьСотрудники.Ссылка.ВидОперации,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Дата,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПерсоналом,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.ВалютаДокумента,
	|	ВЫРАЗИТЬ(АвансовыйОтчетПлатежныеВедомостиДетальный.Сумма * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Кратность) КАК ЧИСЛО(15, 2)),
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Сумма,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПодотчетниками,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовПоПерерасходу,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПодотчетникамиРегл,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовПоПерерасходуРегл,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Сотрудник.СчетРасчетовСПерсоналомРегл,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Проект, 
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Сотрудник,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.ВидУдержанияНДФЛ,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.ВидУдержанияПФ,
	|	ПлатежнаяВедомостьСотрудники.ВидУдержания,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.Ссылка.Организация,
	|	АвансовыйОтчетПлатежныеВедомостиДетальный.СтруктурнаяЕдиница";
	//--- Дмгг, 07.05.2025, ONESOFT-4025
	...
	Если СтруктураВременныеТаблицы.ДвиженияРасчетыСПодотчетникамиИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияРасчетыСПоставщикамиИзменение Тогда
		...
		Если НЕ МассивРезультатов[0].Пустой()
			...
			ИЛИ НЕ МассивРезультатов[5].Пустой()//+++ Дмгг Проверка по регл счетам
			Тогда
		...
		Если НЕ МассивРезультатов[5].Пустой() Тогда
			...
			//Если НЕ ПроверятьИтогиРегистров Тогда 
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструОстаткиНалоговыхЛьгот(ДокументОбъектАвансовыйОтчет, ВыборкаИзРезультатаЗапроса, Отказ);
			//Иначе 
...
Процедура СформироватьТаблицаНалоговыеЛготы(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаПлатежныеВедомости.СотрудникВедомость КАК Сотрудник,
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.Период,
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.Организация,
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.СотрудникВедомость,
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.СтруктурнаяЕдиница,
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.СуммаЛгот,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаПлатежныеВедомостиДетальный.ВедомостьВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежнаяВедомость.УдержаниеДляКонтрагента)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипЛгот.УдержаниеДляКонтрагента)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипЛгот.Прочее)
	|	КОНЕЦ
	|ИЗ
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный КАК ВременнаяТаблицаПлатежныеВедомостиДетальный
	|ГДЕ
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.СуммаЛгот <> 0";
	//--- Дмгг, 07.05.2025, ONESOFT-4025
...
Процедура СформироватьТаблицаОстаткиНалоговыхЛьгот(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаПлатежныеВедомости.Организация КАК Организация,
	|	ВременнаяТаблицаПлатежныеВедомости.СотрудникВедомость КАК Сотрудник,
	|	СУММА(ВременнаяТаблицаПлатежныеВедомости.СуммаЛгот) КАК Сумма,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаПлатежныеВедомости.ВедомостьВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежнаяВедомость.УдержаниеДляКонтрагента)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипЛгот.УдержаниеДляКонтрагента)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипЛгот.Прочее)
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаПлатежныеВедомости.Период,
	|	ВременнаяТаблицаПлатежныеВедомости.Организация,
	|	ВременнаяТаблицаПлатежныеВедомости.СотрудникВедомость,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаПлатежныеВедомости.ВедомостьВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежнаяВедомость.УдержаниеДляКонтрагента)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипЛгот.УдержаниеДляКонтрагента)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипЛгот.Прочее)
	...
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.Организация,
	|	ВременнаяТаблицаПлатежныеВедомостиДетальный.СотрудникВедомость,
	|	СУММА(ВременнаяТаблицаПлатежныеВедомостиДетальный.СуммаЛгот),
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаПлатежныеВедомостиДетальный.ВедомостьВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежнаяВедомость.УдержаниеДляКонтрагента)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипЛгот.УдержаниеДляКонтрагента)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипЛгот.Прочее)
