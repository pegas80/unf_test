...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтобранныеДолгиКопия.Загрузить(ПолучитьИзВременногоХранилища(АдресРасшифровкаПлатежаВХранилище));
...
Процедура ЗаполнитьРасшифровкуПлатежа()
	...
	|	РасчетыСПоставщикамиОстатки.Документ.Контрагент КАК Контрагент,
	...
	|		И ДвиженияДокументаРасчетыСПоставщиками.Контрагент В(&Контрагент)
	...
	Запрос.УстановитьПараметр("Контрагент", ОтобранныеДолгиКопия.Выгрузить().ВыгрузитьКолонку("Контрагент"));
...
Процедура ЗаполнитьДолги()
	...
	//|	ОтобранныеДолги.СуммаПлатежа КАК СуммаПлатежа
	...
	//|	РасчетыСПоставщикамиОстатки.СуммаПлатежа КАК СуммаПлатежа,
	//|	РасчетыСПоставщикамиОстатки.Документ.Дата КАК ДокументДата,
	//|	КурсыВалютРасчетов.Курс КАК Курс,
	//|	КурсыВалютРасчетов.Кратность КАК Кратность,
	//|	ВЫБОР
	//|		КОГДА РасчетыСПоставщикамиОстатки.Документ ССЫЛКА Документ.ПриходнаяНакладная
	//|			ТОГДА ВЫРАЗИТЬ(РасчетыСПоставщикамиОстатки.Документ КАК Документ.ПриходнаяНакладная).СтруктурнаяЕдиница
	//|		КОГДА РасчетыСПоставщикамиОстатки.Документ ССЫЛКА Документ.ДополнительныеРасходы
	//|			ТОГДА ВЫРАЗИТЬ(РасчетыСПоставщикамиОстатки.Документ КАК Документ.ДополнительныеРасходы).СтруктурнаяЕдиница
	//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	//|	КОНЕЦ КАК СтруктураняЕдиницаСклад
	//|ИЗ
	//|	(ВЫБРАТЬ
	//|		РасчетыСПоставщикамиОстатки.Контрагент КАК Контрагент,
	//|		РасчетыСПоставщикамиОстатки.Организация КАК Организация,
	...
	//|		РасчетыСПоставщикамиОстатки.Заказ КАК Заказ,
	//|		ЕСТЬNULL(РасчетыСПоставщикамиОстатки.СуммаВалОстаток, 0) КАК СуммаВалОстаток,
	//|		РасчетыСПоставщикамиОстатки.СуммаОстаток КАК СуммаПлатежа
	//|	ИЗ
	//|		РегистрНакопления.РасчетыСПоставщиками.Остатки(
	//|				,
	//|				Организация = &Организация
	...
	//|	РасчетыСПоставщикамиОстатки.СуммаПлатежа
	//|
	//|ИМЕЮЩИЕ
	...
	//|	ДокументДата";
	  ...
	  |			ИНАЧЕ ЕСТЬNULL(ДвиженияДокументаРасчетыСПоставщиками.СуммаВал, 0)
	  |		КОНЕЦ,
	  |		ДвиженияДокументаРасчетыСПоставщиками.Сумма
	  |	ИЗ
	  |		РегистрНакопления.РасчетыСПоставщиками КАК ДвиженияДокументаРасчетыСПоставщиками
	  |	ГДЕ
	  |		ДвиженияДокументаРасчетыСПоставщиками.Регистратор = &Ссылка
