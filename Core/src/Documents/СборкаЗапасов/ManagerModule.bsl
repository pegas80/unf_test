	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ТипСтрокиСостава = Перечисления.ТипыСтрокСоставаСпецификации.Узел Тогда
			Если НЕ СтекСпецификацийУзлов.Найти(Выборка.Спецификация) = Неопределено Тогда
				ТекстСообщения = НСтр("ka='აღმოჩენილია ელემენტის რეკურსული მიმოსვლა';ru='Обнаружено рекурсивное вхождение элемента'")+" "+Выборка.Номенклатура+" "+НСтр("ka='სპეციფიკაციაში';ru='в спецификации'")+" "+СтрокаПродукция.КоррСпецификация+"
...
Процедура СформироватьТаблицаЗапасыОтходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыОтходы.Количество() - 1 Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаОтходов) Тогда
			...
			СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов'");
            СтрокаТаблицыРасход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов'");
			...
			СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов'");
			СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов'");
...
Процедура ИнициализироватьДанныеПоОтходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("ВозвратныеОтходы", НСтр("ka='დაბრუნებადი ნარჩენები';ru='Возвратные отходы'"));
...
Процедура ИнициализироватьДанныеПоЗапасамСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки) Экспорт
	...
	Запрос.УстановитьПараметр("РаспределениеЗапасов", НСтр("ka='მარაგების გადანაწილება';ru='Распределение запасов'"));
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "М11" Тогда
			...
			Если ОбластьМакета <> Неопределено Тогда
				...
				ШаблонЗаголовока = НСтр("ka='ТРЕБОВАНИЕ-НАКЛАДНАЯ № %1 от %2';ru='ТРЕБОВАНИЕ-НАКЛАДНАЯ № %1 от %2'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='Гарантийный талон на продукцию';ru='Гарантийный талон на продукцию'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ka='Гарантийный талон на комплектующие';ru='Гарантийный талон на комплектующие'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.ФункциональныеОпции = "ИспользоватьСерийныеНомера";
