#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	
	///////////////////////////////////////////////////////////////////////////////
	// ОБЩИЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	
	// Процедура формирует состав узлов.
	//
	Процедура ЗаполнитьТаблицуПродукцииСоставомУзлов(СтрокаПродукция, ТаблицаПродукция, СтекСпецификацийУзлов)
		
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаМатериалы.НомерСтроки) КАК НомерСтрокиСостава,
		|	ТаблицаМатериалы.ТипСтрокиСостава КАК ТипСтрокиСостава,
		|	ТаблицаМатериалы.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА ИспользоватьХарактеристики.Значение
		|			ТОГДА ТаблицаМатериалы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	СУММА(ВЫБОР
		|			КОГДА ТИПЗНАЧЕНИЯ(ТаблицаМатериалы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|				ТОГДА ТаблицаМатериалы.Количество / ТаблицаМатериалы.КоличествоПродукции * &КоличествоПродукции
		|			ИНАЧЕ ТаблицаМатериалы.Количество * ТаблицаМатериалы.ЕдиницаИзмерения.Коэффициент / ТаблицаМатериалы.КоличествоПродукции * &КоличествоПродукции
		|		КОНЕЦ) КАК НормаРасхода,
		|	ТаблицаМатериалы.Спецификация КАК Спецификация
		|ИЗ
		|	Справочник.Спецификации.Состав КАК ТаблицаМатериалы,
		|	Константа.ФункциональнаяОпцияИспользоватьХарактеристики КАК ИспользоватьХарактеристики
		|ГДЕ
		|	ТаблицаМатериалы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
		|	И ТаблицаМатериалы.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаМатериалы.ТипСтрокиСостава,
		|	ТаблицаМатериалы.Номенклатура,
		|	ТаблицаМатериалы.ДоляСтоимости,
		|	ТаблицаМатериалы.Спецификация,
		|	ВЫБОР
		|		КОГДА ИспользоватьХарактеристики.Значение
		|			ТОГДА ТаблицаМатериалы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтрокиСостава";
		
		Запрос.УстановитьПараметр("Ссылка", СтрокаПродукция.ТМСпецификация);
		Запрос.УстановитьПараметр("КоличествоПродукции", СтрокаПродукция.ТМКоличество);
		
		СтекСпецификацийУзлов.Добавить(СтрокаПродукция.ТМСпецификация);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.ТипСтрокиСостава = Перечисления.ТипыСтрокСоставаСпецификации.Узел Тогда
				Если НЕ СтекСпецификацийУзлов.Найти(Выборка.Спецификация) = Неопределено Тогда
					ТекстСообщения = НСтр("ka='აღმოჩენილია ელემენტის რეკურსული მიმოსვლა';ru='Обнаружено рекурсивное вхождение элемента';en='Found recursive entry element'")+" "+Выборка.Номенклатура+" "+НСтр("ka='სპეციფიკაციაში';ru='в спецификации';en='specification'")+" "+СтрокаПродукция.КоррСпецификация+"
					|Операция не выполнена!";
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
				СтекСпецификацийУзлов.Добавить(Выборка.Спецификация);
				СтрокаПродукция.ТМКоличество = Выборка.НормаРасхода;
				СтрокаПродукция.ТМСпецификация = Выборка.Спецификация;
				ЗаполнитьТаблицуПродукцииСоставомУзлов(СтрокаПродукция, ТаблицаПродукция, СтекСпецификацийУзлов);
			Иначе
				НоваяСтрока = ТаблицаПродукция.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПродукция);
				НоваяСтрока.ТМТипСтрокиСостава = Выборка.ТипСтрокиСостава;
				НоваяСтрока.ТМНоменклатура = Выборка.Номенклатура;
				НоваяСтрока.ТМХарактеристика = Выборка.Характеристика;
				НоваяСтрока.ТМКоличество = Выборка.НормаРасхода;
				НоваяСтрока.ТМСпецификация = Выборка.Спецификация;
			КонецЕсли;
		КонецЦикла;
		
		СтекСпецификацийУзлов.Очистить();
		
	КонецПроцедуры // ЗаполнитьТаблицуПродукцииСоставомУзлов()
	
	// Процедура распределяет материалы по спецификациям продукции.
	//
	Процедура РаспределитьМатериалыПоНормативам(СтрокаМатериалы, БазоваяТаблица, ТаблицаМатериалов)
		
		СтрокаМатериалы.Распределено = Истина;
		
		БазаРаспределения = 0;
		Для каждого СтрокаБазы Из БазоваяТаблица Цикл
			БазаРаспределения = БазаРаспределения + СтрокаБазы.ТМКоличество;
			СтрокаБазы.Распределено = Истина;
		КонецЦикла;
		
		РаспределитьСтрокуТабличнойЧастиМатериалы(СтрокаМатериалы, БазоваяТаблица, ТаблицаМатериалов, БазаРаспределения, Истина);
		
	КонецПроцедуры // РаспределитьМатериалыПоНормативам()
	
	// Процедура распределяет материалы пропорционально количеству продукции.
	//
	Процедура РаспределитьМатериалыПоКоличеству(БазоваяТаблица, ТаблицаМатериалов, БазаРаспределения = 0)
		
		ИсклРаспределено = Ложь;
		Если БазаРаспределения = 0 Тогда
			ИсклРаспределено = Истина;
			Для каждого СтрокаБазы Из БазоваяТаблица Цикл
				Если НЕ СтрокаБазы.Распределено Тогда
					БазаРаспределения = БазаРаспределения + СтрокаБазы.КоррКоличество;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		Для н = 0 По ТаблицаМатериалов.Количество() - 1 Цикл
			
			СтрокаМатериалы = ТаблицаМатериалов[н];
			
			Если НЕ СтрокаМатериалы.Распределено Тогда
				РаспределитьСтрокуТабличнойЧастиМатериалы(СтрокаМатериалы, БазоваяТаблица, ТаблицаМатериалов, БазаРаспределения, Ложь, ИсклРаспределено);
			КонецЕсли;
			
		КонецЦикла;
		
	КонецПроцедуры // РаспределитьМатериалыПоКоличеству()
	
	// Процедура распределяет строку материалов.
	//
	Процедура РаспределитьСтрокуТабличнойЧастиМатериалы(СтрокаМатериалы, БазоваяТаблица, ТаблицаМатериалов, БазаРаспределения, ПоНормативам, ИсклРаспределено = Ложь)
		
		ИсхКоличество = 0;
		ИсхРезерв = 0;
		КоличествоКСписанию = СтрокаМатериалы.Количество;
		РезервКСписанию = СтрокаМатериалы.Резерв;
		
		БазаРаспределенияКоличество = БазаРаспределения;
		БазаРаспределенияРезерв = БазаРаспределения;
		
		Для каждого СтрокаБазовойТаблицы Из БазоваяТаблица Цикл
			
			Если ИсклРаспределено И СтрокаБазовойТаблицы.Распределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если ИсхКоличество = КоличествоКСписанию Тогда
				Продолжить;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаМатериалы.КоррНоменклатура) Тогда
				НоваяСтрока = ТаблицаМатериалов.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаМатериалы);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаБазовойТаблицы);
				СтрокаМатериалы = НоваяСтрока;
			Иначе
				ЗаполнитьЗначенияСвойств(СтрокаМатериалы, СтрокаБазовойТаблицы);
			КонецЕсли;
			
			Если ПоНормативам Тогда
				КоличествоБазовойТаблицы = СтрокаБазовойТаблицы.ТМКоличество;
			Иначе
				КоличествоБазовойТаблицы = СтрокаБазовойТаблицы.КоррКоличество
			КонецЕсли;
			
			// Количество.
			СтрокаМатериалы.Количество = Окр((КоличествоКСписанию - ИсхКоличество) * КоличествоБазовойТаблицы / БазаРаспределенияКоличество, 3, 1);
			
			Если (ИсхКоличество + СтрокаМатериалы.Количество) > КоличествоКСписанию Тогда
				СтрокаМатериалы.Количество = КоличествоКСписанию - ИсхКоличество;
				ИсхКоличество = КоличествоКСписанию;
			Иначе
				БазаРаспределенияКоличество = БазаРаспределенияКоличество - КоличествоБазовойТаблицы;
				ИсхКоличество = ИсхКоличество + СтрокаМатериалы.Количество;
			КонецЕсли;
			
			// Резерв.
			Если ИсхРезерв = РезервКСписанию Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаМатериалы.Резерв = Окр((РезервКСписанию - ИсхРезерв) * КоличествоБазовойТаблицы / БазаРаспределенияРезерв, 3, 1);
			
			Если (ИсхРезерв + СтрокаМатериалы.Резерв) > РезервКСписанию Тогда
				СтрокаМатериалы.Резерв = РезервКСписанию - ИсхРезерв;
				ИсхРезерв = РезервКСписанию;
			Иначе
				БазаРаспределенияРезерв = БазаРаспределенияРезерв - КоличествоБазовойТаблицы;
				ИсхРезерв = ИсхРезерв + СтрокаМатериалы.Резерв;
			КонецЕсли;
			
		КонецЦикла;
		
		Если ИсхКоличество < КоличествоКСписанию Тогда
			СтрокаМатериалы.Количество = СтрокаМатериалы.Количество + (КоличествоКСписанию - ИсхКоличество);
		КонецЕсли;
		
		Если ИсхРезерв < РезервКСписанию Тогда
			СтрокаМатериалы.Резерв = СтрокаМатериалы.Резерв + (РезервКСписанию - ИсхРезерв);
		КонецЕсли;
		
	КонецПроцедуры // РаспределитьСтрокуТабличнойЧастиМатериалы()
	
	// Процедура распределяет материалы по спецификациям продукции.
	//
	Процедура РаспределитьПродукциюПоНормативам(СтрокаПродукция, БазоваяТаблица, БазаРаспределения)
		
		РаспределитьСтрокуТабличнойЧастиПродукция(СтрокаПродукция, БазоваяТаблица, БазаРаспределения, Истина);
		
	КонецПроцедуры // РаспределитьПродукциюПоНормативам()
	
	// Процедура распределяет материалы пропорционально количеству продукции.
	//
	Процедура РаспределитьПродукциюПоКоличеству(ТаблицаПродукция, БазоваяТаблица, БазаРаспределения = 0, ИсклРаспределено = Истина)
		
		Если ИсклРаспределено Тогда
			Для каждого СтрокаМатериалы Из БазоваяТаблица Цикл
				Если НЕ СтрокаМатериалы.НоваяСтрока
					И НЕ СтрокаМатериалы.Распределено Тогда
					БазаРаспределения = БазаРаспределения + СтрокаМатериалы.ДоляСтоимости;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		Для каждого СтрокаПродукция Из ТаблицаПродукция Цикл
			
			Если НЕ СтрокаПродукция.Распределено Тогда
				РаспределитьСтрокуТабличнойЧастиПродукция(СтрокаПродукция, БазоваяТаблица, БазаРаспределения, Ложь, ИсклРаспределено);
			КонецЕсли;
			
		КонецЦикла;
		
	КонецПроцедуры // РаспределитьПродукциюПоКоличеству()
	
	// Процедура распределяет строку продукции.
	//
	Процедура РаспределитьСтрокуТабличнойЧастиПродукция(СтрокаПродукции, БазоваяТаблица, БазаРаспределения, ПоНормативам, ИсклРапределение = Ложь)
		
		ИсхКоличество = 0;
		ИсхРезерв = 0;
		КоличествоКСписанию = СтрокаПродукции.Количество;
		РезервКСписанию = СтрокаПродукции.Резерв;
		
		БазаРаспределенияКоличество = БазаРаспределения;
		БазаРаспределенияРезерв = БазаРаспределения;
		
		СтрокаРапределения = Неопределено;
		Для н = 0 По БазоваяТаблица.Количество() - 1 Цикл
			
			СтрокаМатериалы = БазоваяТаблица[н];
			
			Если ИсхКоличество = КоличествоКСписанию
				ИЛИ СтрокаМатериалы.НоваяСтрока Тогда
				СтрокаМатериалы.ВыполненРасчет = Ложь;
				Продолжить;
			КонецЕсли;
			
			Если ПоНормативам И НЕ СтрокаМатериалы.ВыполненРасчет Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаМатериалы.ВыполненРасчет = Ложь;
			
			Если НЕ ПоНормативам И ИсклРапределение
				И СтрокаМатериалы.Распределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СтрокаМатериалы.Номенклатура) Тогда
				Распределено = СтрокаМатериалы.Распределено;
				ЗаполнитьЗначенияСвойств(СтрокаМатериалы, СтрокаПродукции);
				СтрокаРапределения = СтрокаМатериалы;
				СтрокаРапределения.Распределено = Распределено;
			Иначе
				СтрокаРапределения = БазоваяТаблица.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРапределения, СтрокаМатериалы);
				ЗаполнитьЗначенияСвойств(СтрокаРапределения, СтрокаПродукции);
				СтрокаРапределения.НоваяСтрока = Истина;
			КонецЕсли;
			
			// Количество.
			СтрокаРапределения.Количество = Окр((КоличествоКСписанию - ИсхКоличество) * СтрокаМатериалы.ДоляСтоимости / ?(БазаРаспределенияКоличество = 0, 1, БазаРаспределенияКоличество),3,1);
			
			Если СтрокаРапределения.Количество = 0 Тогда
				СтрокаРапределения.Количество = КоличествоКСписанию;
				ИсхКоличество = КоличествоКСписанию;
			Иначе
				БазаРаспределенияКоличество = БазаРаспределенияКоличество - СтрокаМатериалы.ДоляСтоимости;
				ИсхКоличество = ИсхКоличество + СтрокаРапределения.Количество;
			КонецЕсли;
			
			Если ИсхКоличество > КоличествоКСписанию Тогда
				СтрокаРапределения.Количество = СтрокаРапределения.Количество - (ИсхКоличество - КоличествоКСписанию);
				ИсхКоличество = КоличествоКСписанию;
			КонецЕсли;
			
			// Резерв.
			Если ИсхРезерв = РезервКСписанию Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаРапределения.Резерв = Окр((РезервКСписанию - ИсхРезерв) * СтрокаМатериалы.ДоляСтоимости / ?(БазаРаспределенияРезерв = 0, 1, БазаРаспределенияРезерв),3,1);
			
			Если СтрокаРапределения.Резерв = 0 Тогда
				СтрокаРапределения.Резерв = РезервКСписанию;
				ИсхРезерв = РезервКСписанию;
			Иначе
				БазаРаспределенияРезерв = БазаРаспределенияРезерв - СтрокаМатериалы.ДоляСтоимости;
				ИсхРезерв = ИсхРезерв + СтрокаРапределения.Резерв;
			КонецЕсли;
			
			Если ИсхРезерв > РезервКСписанию Тогда
				СтрокаРапределения.Резерв = СтрокаРапределения.Резерв - (ИсхРезерв - РезервКСписанию);
				ИсхРезерв = РезервКСписанию;
			КонецЕсли;
			
		КонецЦикла;
		
		Если СтрокаРапределения <> Неопределено Тогда
			
			Если ИсхКоличество < КоличествоКСписанию Тогда
				СтрокаРапределения.Количество = СтрокаРапределения.Количество + (КоличествоКСписанию - ИсхКоличество);
			КонецЕсли;
			
			Если ИсхРезерв < РезервКСписанию Тогда
				СтрокаРапределения.Резерв = СтрокаРапределения.Резерв + (РезервКСписанию - ИсхРезерв);
			КонецЕсли;
			
		КонецЕсли;
		
	КонецПроцедуры // РаспределитьСтрокуТабличнойЧастиПродукция()
	
	&НаСервере
	Функция ПолучитьСебестоимостьНоменклатуры(Номенклатура, Характеристика, документ, видЦен, таблица, структ)
		
		
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПеремещениеЗапасов.Номенклатура КАК Номенклатура,
		|	ХозрасчетныйДвиженияССубконто.Сумма КАК Себестоимость,
		|	ХозрасчетныйДвиженияССубконто.Сумма / ПеремещениеЗапасов.Количество КАК СебестоимостьЕдиницы
		|ИЗ
		|	Документ.СборкаЗапасов."+таблица+" КАК ПеремещениеЗапасов
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
		|				&ДатаДокумента,
		|				Номенклатура = (&СписокНоменклатура)
		|					И Характеристика = (&СписокХарактеристика)
		|					И ВидЦен = &ВидЦен) КАК ЦеныНоменклатурыСрезПоследних
		|		ПО ПеремещениеЗапасов.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
		|			И ПеремещениеЗапасов.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
		|			И ПеремещениеЗапасов.Ссылка."+Структ+".РозничныйВидЦен = ЦеныНоменклатурыСрезПоследних.ВидЦен
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(
		|				,
		|				,
		|				Регистратор = &ТекущийДокумент
		|					И Субконто2 = (&СписокНоменклатура),
		|				,
		|				) КАК ХозрасчетныйДвиженияССубконто
		|		ПО (ПеремещениеЗапасов.Ссылка = (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.СборкаЗапасов)))
		|			И (ПеремещениеЗапасов.Номенклатура = (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт2 КАК Справочник.Номенклатура)))
		|ГДЕ
		|	ПеремещениеЗапасов.Ссылка = &ТекущийДокумент";
		
		
		Запрос.УстановитьПараметр("ВидЦен", видЦен);
		Запрос.УстановитьПараметр("ДатаДокумента", документ.дата);
		Запрос.УстановитьПараметр("ТекущийДокумент", документ);
		Запрос.УстановитьПараметр("СписокНоменклатура", Номенклатура);
		Запрос.УстановитьПараметр("СписокХарактеристика", Характеристика);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		
		Себетоимость = 0;
		
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Попытка
				Себетоимость =  Себетоимость + ВыборкаДетальныеЗаписи.СебестоимостьЕдиницы;
			Исключение
			КонецПопытки;
			
		КонецЦикла;
		
		Возврат Себетоимость;
		
		
		
	КонецФункции
	//Малышев 2024-06-25   --)

	
	////////////////////////////////////////////////////////////////////////////////
	// ОТХОДЫ
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыОтходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыОтходы.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыОтходы[н];
			
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
			
			//+++
			//СтрокаТаблицаХозрасчетныйПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаХозрасчетный.Добавить();
			//ЗаполнитьЗначенияСвойств(СтрокаТаблицаХозрасчетныйПеремещение, СтрокаТаблицаЗапасы);
			////СтрокаТаблицаХозрасчетныйПеремещение.Сумма = СуммаКСписаниюПеремещение;
			//СтрокаТаблицаХозрасчетныйПеремещение.СчетДт 		= СтрокаТаблицыПриход.СчетДтРегл;
			//СтрокаТаблицаХозрасчетныйПеремещение.СчетКт 		= СтрокаТаблицыПриход.СчетКтРегл;
			//
			//ТаблицаЗапасыПродукция = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция;
			//СтруктураПоискаПродукции = Новый Структура("Номенклатура, Характеристиа", СтрокаТаблицыПриход.Номенклатура, СтрокаТаблицыПриход.Характеристика);
			//МассСтрок = ТаблицаЗапасыПродукция.НайтиСтрока(СтруктураПоискаПродукции);
			//Если МассСтрок.Количество()<>0 Тогда 
			//	СтрокаТаблицаХозрасчетныйПеремещение.КоличествоДт	= МассСтрок[0].Количество;
			//	СтрокаТаблицаХозрасчетныйПеремещение.КоличествоКт	= МассСтрок[0].Количество;
			//Иначе
			//	СтрокаТаблицаХозрасчетныйПеремещение.КоличествоДт	= 0;
			//	СтрокаТаблицаХозрасчетныйПеремещение.КоличествоКт	= 0;
			//КонецЕсли;
			//
			//СтрокаТаблицаХозрасчетныйПеремещение.СубконтоДт1 	= СтрокаТаблицыПриход.СтруктурнаяЕдиницаПродукции;
			//СтрокаТаблицаХозрасчетныйПеремещение.СубконтоДт2 	= СтрокаТаблицыПриход.Номенклатура;
			//СтрокаТаблицаХозрасчетныйПеремещение.СубконтоДт3	= СтрокаТаблицыПриход.Характеристика;
			//СтрокаТаблицаХозрасчетныйПеремещение.СубконтоКт1 	= СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница;
			//СтрокаТаблицаХозрасчетныйПеремещение.СубконтоКт2 	= СтрокаТаблицыПриход.КоррНоменклатура;
			//СтрокаТаблицаХозрасчетныйПеремещение.СубконтоКт3 	= СтрокаТаблицыПриход.КоррХарактеристика;
			//Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда 
			//	СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоДт1	= "Подразделения";
			//Иначе
			//	СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоДт1 = "Склады";
			//КонецЕсли;
			//СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоДт2 = "Номенклатура";
			//СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоДт3 = "ХарактеристикиНоменклатуры";
			//Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда 
			//	СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоКт1	= "Подразделения";
			//Иначе
			//	СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоКт1 = "Склады";
			//КонецЕсли;
			//СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоКт2 = "Номенклатура";
			//СтрокаТаблицаХозрасчетныйПеремещение.ВидСубконтоКт3 = "ХарактеристикиНоменклатуры";
			//---
			
			// Автоперемещение возвратных отходов.
			Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаОтходов) Тогда
				
				// Расход.
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
				
				СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаОтходов;
				СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаОтходов;
				
				//+++
				СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаОтходовРегл;
				//---
				
				СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
				СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
				СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
				СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
				
				СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				СтрокаТаблицыРасход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				
				// Приход.
				СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаОтходов;
				СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаОтходов;
				
				СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
				СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
				//+++
				СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаОтходовРегл;
				СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
				//---
				
				СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
				СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
				СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
				СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
				
				СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыОтходы");
		
	КонецПроцедуры // СформироватьТаблицаЗапасыОтходы()
	
	// Инициализирует таблицы значений, содержащие данные табличных частей документа.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура ИнициализироватьДанныеПоОтходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СборкаЗапасовОтходы.НомерСтроки КАК НомерСтроки,
		|	&Организация КАК Организация,
		|	СборкаЗапасовОтходы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовОтходы.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК Ячейка,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов
		|	КОНЕЦ КАК СтруктурнаяЕдиницаОтходов,
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам
		|			ТОГДА СборкаЗапасовОтходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам
		|			ТОГДА СборкаЗапасовОтходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		|	ВЫБОР
		|		КОГДА СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаОтходов,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовОтходы.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаОтходовРегл,
		|	СборкаЗапасовОтходы.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовОтходы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовОтходы.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Партия,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовОтходы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовОтходы.Ссылка.ЗаказПокупателя
		|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	КОНЕЦ КАК ЗаказПокупателя,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	0 КАК Сумма,
		|	ВЫРАЗИТЬ(&ВозвратныеОтходы КАК СТРОКА(100)) КАК СодержаниеПроводки,
		|	ВЫРАЗИТЬ(&ВозвратныеОтходы КАК СТРОКА(100)) КАК Содержание,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК Подразделение
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СборкаЗапасовОтходы.НомерСтроки КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	СборкаЗапасовОтходы.Ссылка.Дата КАК Период,
		|	&Организация КАК Организация,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовОтходы.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК Ячейка,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов КАК СтруктурнаяЕдиницаОтходов,
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам
		|			ТОГДА СборкаЗапасовОтходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам
		|			ТОГДА СборкаЗапасовОтходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСклад,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов.ОрдерныйСклад КАК ОрдерныйСкладОтходов,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовОтходы.Ссылка.ЯчейкаОтходов
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК ЯчейкаОтходов,
		|	СборкаЗапасовОтходы.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовОтходы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовОтходы.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Партия,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК Подразделение
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|	И СборкаЗапасовОтходы.Ссылка.Дата < &ДатаОбновленияНаРелиз_1_2_1
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СборкаЗапасовОтходы.НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	СборкаЗапасовОтходы.Ссылка.Дата,
		|	&Организация,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовОтходы.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов,
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам
		|			ТОГДА СборкаЗапасовОтходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам
		|			ТОГДА СборкаЗапасовОтходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов.ОрдерныйСклад,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовОтходы.Ссылка.ЯчейкаОтходов
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ,
		|	СборкаЗапасовОтходы.Номенклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовОтходы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовОтходы.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|	И НЕ СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
		|	И СборкаЗапасовОтходы.Ссылка.Дата >= &ДатаОбновленияНаРелиз_1_2_1
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СборкаЗапасовОтходы.НомерСтроки КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	СборкаЗапасовОтходы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовОтходы.Ссылка.Организация КАК Организация,
		|	СборкаЗапасовОтходы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовОтходы.Характеристика КАК Характеристика,
		|	СборкаЗапасовОтходы.Партия КАК Партия,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК Подразделение
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
		|	И СборкаЗапасовОтходы.Ссылка.Дата >= &ДатаОбновленияНаРелиз_1_2_1
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СборкаЗапасовОтходы.НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	СборкаЗапасовОтходы.Ссылка.Дата,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов,
		|	СборкаЗапасовОтходы.Ссылка.Организация,
		|	СборкаЗапасовОтходы.Номенклатура,
		|	СборкаЗапасовОтходы.Характеристика,
		|	СборкаЗапасовОтходы.Партия,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов.ОрдерныйСклад
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов <> СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СборкаЗапасовОтходы.НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	СборкаЗапасовОтходы.Ссылка.Дата,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов,
		|	СборкаЗапасовОтходы.Ссылка.Организация,
		|	СборкаЗапасовОтходы.Номенклатура,
		|	СборкаЗапасовОтходы.Характеристика,
		|	СборкаЗапасовОтходы.Партия,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов.ОрдерныйСклад
		|	И СборкаЗапасовОтходы.Ссылка.Дата < &ДатаОбновленияНаРелиз_1_2_1
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов <> СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	СборкаЗапасовОтходы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовОтходы.Ссылка.Организация КАК Организация,
		|	СборкаЗапасовОтходы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовОтходы.Характеристика КАК Характеристика,
		|	СборкаЗапасовОтходы.Партия КАК Партия,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовОтходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовОтходы.Количество
		|		ИНАЧЕ СборкаЗапасовОтходы.Количество * СборкаЗапасовОтходы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница КАК Подразделение
		|ИЗ
		|	Документ.СборкаЗапасов.Отходы КАК СборкаЗапасовОтходы
		|ГДЕ
		|	СборкаЗапасовОтходы.Ссылка = &Ссылка
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
		|	И СборкаЗапасовОтходы.Ссылка.Дата >= &ДатаОбновленияНаРелиз_1_2_1
		|	И СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиницаОтходов <> СборкаЗапасовОтходы.Ссылка.СтруктурнаяЕдиница";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);
		
		//ПодольскийАИ #УчетПроектов 05.10.2020 {
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
		//ПодольскийАИ #УчетПроектов 05.10.2020 }
		
		// Временно: изменение движений по ордерному складу.
		ДатаОбновленияНаРелиз_1_2_1 = Константы.ДатаОбновленияНаРелиз_1_2_1.Получить();
		Запрос.УстановитьПараметр("ДатаОбновленияНаРелиз_1_2_1", ДатаОбновленияНаРелиз_1_2_1);
		
		Запрос.УстановитьПараметр("ВозвратныеОтходы", НСтр("ka='დაბრუნებადი ნარჩენები';ru='Возвратные отходы';en='recyclable waste'"));
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		// Определим таблицу для учета запасов.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыОтходы", МассивРезультатов[0].Выгрузить());
		
		// Сформируем таблицу для учета запасов.
		СформироватьТаблицаЗапасыОтходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Дополним таблицу для запасов.
		ВыборкаРезультатов = МассивРезультатов[1].Выбрать();
		
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, ВыборкаРезультатов);
			
			// Автоперемещение возвратных отходов (дополним таблицу ТаблицаЗапасыНаСкладах).
			Если (ВыборкаРезультатов.СтруктурнаяЕдиницаОтходов = ВыборкаРезультатов.СтруктурнаяЕдиница
				И ВыборкаРезультатов.ЯчейкаОтходов <> ВыборкаРезультатов.Ячейка)
				ИЛИ ВыборкаРезультатов.СтруктурнаяЕдиницаОтходов <> ВыборкаРезультатов.СтруктурнаяЕдиница Тогда
				
				// Расход.			
				Если (ВыборкаРезультатов.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (ВыборкаРезультатов.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ ВыборкаРезультатов.ОрдерныйСклад) Тогда	
					
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, ВыборкаРезультатов);
					
					СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
					
				КонецЕсли;	
				
				// Приход.
				Если (ВыборкаРезультатов.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (ВыборкаРезультатов.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ ВыборкаРезультатов.ОрдерныйСкладОтходов) Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, ВыборкаРезультатов);
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = ВыборкаРезультатов.СтруктурнаяЕдиницаОтходов;
					СтрокаТаблицыПриход.Ячейка = ВыборкаРезультатов.ЯчейкаОтходов;
					
				КонецЕсли;	
				
			КонецЕсли;
			
		КонецЦикла;
		
		// Автоперемещение возвратных отходов (дополним таблицу ТаблицаЗапасыКПоступлениюНаСклады).
		ВыборкаРезультатов = МассивРезультатов[2].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасходПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыКПоступлениюНаСклады.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПриход, ВыборкаРезультатов);		
			
		КонецЦикла;
		
		// Автоперемещение возвратных отходов (дополним таблицу ТаблицаЗапасыКРасходуСоСкладов).
		ВыборкаРезультатов = МассивРезультатов[3].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасходПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыКРасходуСоСкладов.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПриход, ВыборкаРезультатов);		
			
		КонецЦикла;
		
	КонецПроцедуры // ИнициализироватьДанныеПоОтходам()
	
	////////////////////////////////////////////////////////////////////////////////
	// ЗАПАСЫ (СБОРКА)
	
	Процедура СформироватьТаблицаЗапасыВРазрезеГТДСборка(СтруктураДополнительныеСвойства)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасыНаСкладах.НомерСтроки) КАК НомерСтроки
		|	,ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения
		|	,ТаблицаЗапасыНаСкладах.Период КАК Период
		|	,ТаблицаЗапасыНаСкладах.Организация КАК Организация
		|	,ТаблицаЗапасыНаСкладах.Номенклатура КАК Номенклатура
		|	,ТаблицаЗапасыНаСкладах.Характеристика КАК Характеристика
		|	,ТаблицаЗапасыНаСкладах.Партия КАК Партия
		|	,ТаблицаЗапасыНаСкладах.НомерГТД КАК НомерГТД
		|	,ТаблицаЗапасыНаСкладах.СтранаПроисхождения КАК СтранаПроисхождения
		|	,СУММА(ТаблицаЗапасыНаСкладах.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасыНаСкладах
		|ГДЕ
		|	ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> Значение(Справочник.СтраныМира.Россия)
		|	И ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> Значение(Справочник.СтраныМира.ПустаяССылка)
		|	И ТаблицаЗапасыНаСкладах.НомерГТД <> Значение(Справочник.НомераГТД.ПустаяССылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасыНаСкладах.Период
		|	,ТаблицаЗапасыНаСкладах.Организация
		|	,ТаблицаЗапасыНаСкладах.Номенклатура
		|	,ТаблицаЗапасыНаСкладах.Характеристика
		|	,ТаблицаЗапасыНаСкладах.Партия
		|	,ТаблицаЗапасыНаСкладах.НомерГТД
		|	,ТаблицаЗапасыНаСкладах.СтранаПроисхождения";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыВРазрезеГТД", РезультатЗапроса.Выгрузить());
		
	КонецПроцедуры
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыЗапасыСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
		УчетСебестоимостиВРазрезеОрганизации = СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетСебестоимостиВРазрезеОрганизации;
		Запрос.УстановитьПараметр("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации);
		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		
		// Установка исключительной блокировки контролируемых остатков запасов.
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация КАК Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|		ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|		ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|		ТаблицаЗапасы.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|		ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|		ТаблицаЗапасы.Характеристика КАК Характеристика,
		|		ТаблицаЗапасы.Партия КАК Партия,
		|		ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|	ГДЕ
		|		ТаблицаЗапасы.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиница,
		|		ТаблицаЗапасы.СчетУчета,
		|		ТаблицаЗапасы.СчетУчетаРегл,
		|		ТаблицаЗапасы.СчетУчетаЗатратРегл,
		|		ТаблицаЗапасы.Номенклатура,
		|		ТаблицаЗапасы.Характеристика,
		|		ТаблицаЗапасы.Партия,
		|		ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы) КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗатратРегл,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.Запасы");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			Если КолонкаРезультатЗапроса.Имя <> "СчетУчетаЗатратРегл" Тогда
				ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
			КонецЕсли;
		КонецЦикла;
		Блокировка.Заблокировать();      
		
		// Получение остатков запасов по стоимости.  
		
		//Малышев 2024-06-16 ONESOFT-3094 - запрос перенесен в эту процедуру (--
		//ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении_Сборка(ДокументСсылкаСборкаЗапасов,СтруктураДополнительныеСвойства).Выгрузить(); 
		//ТаблицаЗапасыОстатки.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчета,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя");
		
		ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСборкаЗапасов,СтруктураДополнительныеСвойства).Выгрузить(); 
		ТаблицаЗапасыОстатки.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя");
		//Малышев 2024-06-16 ONESOFT-3094  --)
		
		ЗаполнитьКоличествоПоДебету(СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы, СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция);
		
		СуммаКПеремещению = 0;
		СтрокаТаблицыЗапасыКПеремещению = Неопределено;
		ТаблицыПродукцияКПеремещению = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.СкопироватьКолонки();
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы[н];
			
			//Малышев 2024-07-27 ((-
			СтруктураДляПоиска = УправлениеНебольшойФирмойСервер.ПолучитьСтруктуруДляПоиска(СтрокаТаблицаЗапасы, СтруктураДополнительныеСвойства);
			//СтруктураДляПоиска = Новый Структура;
			//СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасы.Организация);
			////--------------------------------------------------------------------------
			//// © Шишка В.В. [17 августа 2021]  НАЧАЛО
			////СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасы.СтруктурнаяЕдиница);
			//Если НЕ УчетСебестоимостиВРазрезеОрганизации Тогда
			//	
			//	СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасы.СтруктурнаяЕдиница);
			//	
			//КонецЕсли; 	
			//// © Шишка В.В. [17 августа 2021]  КОНЕЦ
			////--------------------------------------------------------------------------
			////СтруктураДляПоиска.Вставить("СчетУчета", СтрокаТаблицаЗапасы.СчетУчета);
			//СтруктураДляПоиска.Вставить("СчетУчетаРегл", СтрокаТаблицаЗапасы.СчетУчетаРегл);
			//СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасы.Номенклатура);
			//СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасы.Характеристика);
			//СтруктураДляПоиска.Вставить("Партия", СтрокаТаблицаЗапасы.Партия);
			////Малышев 2024-06-16 ONESOFT-3094
			//Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
			//	СтруктураДляПоиска.Вставить("Проект", СтрокаТаблицаЗапасы.Проект);
			//КонецЕсли;	
			//Малышев 2024-07-27 --)
	

			//Малышев 2024-07-10
			СуммаДопРасходов = 0;
			
			//КоличествоТребуетсяРезерв = СтрокаТаблицаЗапасы.Резерв;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
				КоличествоТребуетсяРезерв = 0;
			Иначе
				КоличествоТребуетсяРезерв = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасы.Резерв), СтрокаТаблицаЗапасы.Резерв, 0);
			КонецЕсли;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			КоличествоТребуетсяСвободныйОстаток = СтрокаТаблицаЗапасы.Количество;
			
			Если КоличествоТребуетсяРезерв > 0 Тогда
				
				КоличествоТребуетсяСвободныйОстаток = КоличествоТребуетсяСвободныйОстаток - КоличествоТребуетсяРезерв;
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
				
				МассивСтрокОстатков = ТаблицаЗапасыОстатки.НайтиСтроки(СтруктураДляПоиска);
				
				//КоличествоОстаток = 0;
				//СуммаОстаток = 0;
				//
				//Если МассивСтрокОстатков.Количество() > 0 Тогда
				//	КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток;
				//	СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток;
				//КонецЕсли;
				//
				//Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяРезерв Тогда
				//	
				//	СуммаКСписанию = Окр(СуммаОстаток * КоличествоТребуетсяРезерв / КоличествоОстаток , 2, 1);
				//	
				//	МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток - КоличествоТребуетсяРезерв;
				//	МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СуммаКСписанию;
				//	
				//ИначеЕсли КоличествоОстаток = КоличествоТребуетсяРезерв Тогда
				//	
				//	СуммаКСписанию = СуммаОстаток;
				//	
				//	МассивСтрокОстатков[0].КоличествоОстаток = 0;
				//	МассивСтрокОстатков[0].СуммаОстаток = 0;
				//	
				//Иначе
				//	СуммаКСписанию = 0;	
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
			
					Если КоличествоТребуетсяРезерв <= 0 Тогда
						Прервать;
					КонецЕсли;
					
					КоличествоОстатокРезерв = СтрокаТаблицыОстатков.КоличествоОстаток;
					СуммаОстатокРезерв = СтрокаТаблицыОстатков.СуммаОстаток;
					
					КоличествоКСписаниюРезерв = 0;
					СуммаКСписаниюРезерв = 0;
					
					Если (КоличествоОстатокРезерв > 0) И (КоличествоОстатокРезерв > КоличествоТребуетсяРезерв) Тогда
						
						КоличествоКСписаниюРезерв = КоличествоОстатокРезерв;
						СуммаКСписанию = Окр(СуммаОстатокРезерв * КоличествоТребуетсяРезерв / КоличествоОстатокРезерв , 2, 1);
						
						СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - КоличествоТребуетсяРезерв;
						СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписанию;
						
					ИначеЕсли (КоличествоОстатокРезерв > 0) И (КоличествоОстатокРезерв <= КоличествоТребуетсяРезерв) Тогда
						
						КоличествоКСписаниюРезерв = КоличествоОстатокРезерв;
						СуммаКСписанию = СуммаОстатокРезерв;
						
						СтрокаТаблицыОстатков.КоличествоОстаток = 0;
						СтрокаТаблицыОстатков.СуммаОстаток = 0;
						
					ИначеЕсли КоличествоОстатокРезерв <= 0 Тогда  
						
						Продолжить;
						
					КонецЕсли;
					
				КоличествоТребуетсяРезерв = КоличествоТребуетсяРезерв - КоличествоКСписаниюРезерв;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Спишем запасы со склада (производственного подразделения).
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				//СтрокаТаблицыРасход.Количество = КоличествоТребуетсяРезерв;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.Количество = КоличествоКСписаниюРезерв;
				СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
    			//Малышев 2024-07-17 ONESOFT-3273--)

				
				// Отнесем списанные запасы либо на стоимость запасов на складе либо на затраты НЗП.
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//--------------------------------------------------------------------------
					// © Шишка В.В. [7 сентября 2021 г.]  НАЧАЛО
					//ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы,, ?(УчетСебестоимостиВРазрезеОрганизации, "СтруктурнаяЕдиница", Неопределено));
					// © Шишка В.В. [7 сентября 2021 г.]  КОНЕЦ
					//--------------------------------------------------------------------------
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
					
					СтрокаТаблицыПриход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости

					//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
    				//Малышев 2024-07-17 ONESOFT-3273--)
			
					// Сформируем проводки.
					Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Сборка1(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					КонецЕсли;
					
					// Автосписание запасов.
					СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
					
				КонецЕсли;
					
				КонецЦикла; // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
			КонецЕсли;
			
			//--------------------------------------------------------------------------
			// © Шишка В.В. [7 сентября 2021 г.]  НАЧАЛО
			Если УчетСебестоимостиВРазрезеОрганизации Тогда
				
				СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.ЗаполнитьЗначения(Неопределено, "СтруктурнаяЕдиница");	
				СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.ЗаполнитьЗначения(Неопределено, "КоррСтруктурнаяЕдиница");	
				ТаблицыПродукцияКПеремещению.Очистить();
				
			КонецЕсли; 	
			// © Шишка В.В. [7 сентября 2021 г.]  КОНЕЦ
			//--------------------------------------------------------------------------
			Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", Документы.ЗаказПокупателя.ПустаяСсылка());
				
				МассивСтрокОстатков = ТаблицаЗапасыОстатки.НайтиСтроки(СтруктураДляПоиска);
				
				//КоличествоОстаток = 0;
				//СуммаОстаток = 0;
				//
				//Если МассивСтрокОстатков.Количество() > 0 Тогда
				//	КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток;
				//	СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток;
				//КонецЕсли;
				//
				//Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
				//	
				//	СуммаКСписанию = Окр(СуммаОстаток * КоличествоТребуетсяСвободныйОстаток / КоличествоОстаток , 2, 1);
				//	
				//	МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток - КоличествоТребуетсяСвободныйОстаток;
				//	МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СуммаКСписанию;
				//	
				//ИначеЕсли КоличествоОстаток = КоличествоТребуетсяСвободныйОстаток Тогда
				//	
				//	СуммаКСписанию = СуммаОстаток;
				//	
				//	МассивСтрокОстатков[0].КоличествоОстаток = 0;
				//	МассивСтрокОстатков[0].СуммаОстаток = 0;
				//	
				//Иначе
				//	СуммаКСписанию = 0;	
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
					
					Если КоличествоТребуетсяСвободныйОстаток <= 0 Тогда
						Прервать;
					КонецЕсли;
					
					КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток;
					СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток;
					
					КоличествоКСписанию = 0;
					СуммаКСписанию = 0;
					
					Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
						
						КоличествоКСписанию = КоличествоТребуетсяСвободныйОстаток;
						СуммаКСписанию = Окр(СуммаОстаток * КоличествоТребуетсяСвободныйОстаток / КоличествоОстаток , 2, 1);
						
						СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - КоличествоТребуетсяСвободныйОстаток;
						СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписанию;
						
					ИначеЕсли (КоличествоОстаток>0) И (КоличествоОстаток<=КоличествоТребуетсяСвободныйОстаток) Тогда
						
						КоличествоКСписанию = КоличествоОстаток;
						СуммаКСписанию = СуммаОстаток;
						
						СтрокаТаблицыОстатков.КоличествоОстаток = 0;
						СтрокаТаблицыОстатков.СуммаОстаток = 0;
						
					Иначе
						СуммаКСписанию = 0;	
					КонецЕсли;
					
				КоличествоТребуетсяСвободныйОстаток = КоличествоТребуетсяСвободныйОстаток - КоличествоКСписанию;	
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Расход.
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				//СтрокаТаблицыРасход.Количество = КоличествоТребуетсяСвободныйОстаток;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.Количество = КоличествоКСписанию;
				Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				КонецЕсли;
				Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.ЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
				
				//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
    			//Малышев 2024-07-17 ONESOFT-3273--)
				
                // Приход
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
					КонецЕсли;
					Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриход.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
    				//Малышев 2024-07-17 ONESOFT-3273--)
				
					// Сформируем проводки.
					Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Сборка2(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					КонецЕсли;
					
					// Автосписание запасов.
					СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
					
				КонецЕсли;
				
				КонецЦикла; // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// делаем отрицательный остаток
				Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
					
					// Расход.
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыРасход.Сумма = 0;
					
					СтрокаТаблицыРасход.Количество = КоличествоТребуетсяСвободныйОстаток;				
					
					СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
					СтрокаТаблицыРасход.ЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					
					//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
					//Малышев 2024-07-17 ONESOFT-3273--)
					
					//// Приход
					//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					//	
					//	СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//	ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//	
					//	СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					//	
					//	СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//	СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//	СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//	СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//	СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//	СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					//	СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//	
					//	СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					//	СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//	//+++
					//	СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//	СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//	//---
					//	СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					//	СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					//	СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					//	СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					//	СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					//	
					//	//ПодольскийАИ #УчетПроектов 13.10.2020 {
					//	СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//	СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//	//ПодольскийАИ #УчетПроектов 13.10.2020 }
					//	
					//	СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					//	СтрокаТаблицыПриход.Количество = 0;
					//	
					//	СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//	СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
					//	
					//	// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
					//	//СтрокаТаблицыПриход.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					//	// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	
					//	//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
					//	ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
					//	//Малышев 2024-07-17 ONESOFT-3273--)
					//	
					//	// Сформируем проводки.
					//	Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					//		СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					//		ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
					//		
					//		ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Сборка2(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//	КонецЕсли;
					//	
					//	// Автосписание запасов.
					//	СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
					//КонецЕсли;
				КонецЕсли;
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости	
					
			КонецЕсли;
			
			// Автосписание запасов.
			СтрокаТаблицыЗапасыКПеремещению = СтрокаТаблицаЗапасы;
			
			Если СуммаКПеремещению > 0 
				И СтрокаТаблицыЗапасыКПеремещению <> Неопределено 
				И ЗначениеЗаполнено(СтрокаТаблицыЗапасыКПеремещению.СтруктурнаяЕдиницаПродукции) Тогда
				
				НоваяСтрока = ТаблицыПродукцияКПеремещению.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицыЗапасыКПеремещению);
				НоваяСтрока.Сумма = СуммаКПеремещению;
				
			КонецЕсли;
			
			СуммаКПеремещению = 0;
			
		КонецЦикла;
		
		//Малышев 2024-07-09 ONESOFT-3310
		//сделаем записи по допрасходам, если они есть
		Если СтруктураДополнительныеСвойства.ДляПроведения.Свойство("СтруктураТаблицПоДопРасходам") 
			и СтрокаТаблицыПриход <> Неопределено //исправлено 2024-10-21 ONESOFT-3531
			и ЗначениеЗаполнено(СтрокаТаблицыПриход.СтруктурнаяЕдиница) //исправлено 2024-10-21 ONESOFT-3531
			и СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
			Тогда
			//СтруктураОтбораДопРасходов = Новый Структура("Организация, Номенклатура, Характеристика,СтруктурнаяЕдиница,Проект");
			//ЗаполнитьЗначенияСвойств(СтруктураОтбораДопРасходов, СтрокаТаблицыПриход);
			///МассивСтрокДопРасходов = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы.НайтиСтроки(СтруктураОтбораДопРасходов);
			
			//Если МассивСтрокДопРасходов.Количество()>0 тогда   
				Для каждого строкаДопРасходов из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
				СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, строкаДопРасходов);	
				СуммаДопРасходов = строкаДопРасходов.Сумма;
				СтрокаТаблицыПриход_ДопРасходы.Сумма = СуммаДопРасходов; 
				
				//Малышев 2024-07-17 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход_ДопРасходы);		
				//Малышев 2024-07-17 ONESOFT-3273--)
				КонецЦикла;
				
			//КонецЕсли;
		КонецЕсли;
		
		Если ТаблицыПродукцияКПеремещению.Количество() > 1 Тогда
			//ТаблицыПродукцияКПеремещению.Свернуть("Организация,Период,СценарийПланирования,СтруктурнаяЕдиницаПродукции,ЗатратыНаВыпуск,КоррЗаказПокупателя,КоррНоменклатура,КоррПартия,КоррСтруктурнаяЕдиница,КоррСчетУчета,КоррХарактеристика,СчетДтПродукции,СчетКтПродукции,СчетУчетаПродукции","Сумма");
			//ПодольскийАИ #УчетПроектов 15.10.2020 {
			// Добавлены поля "Проект", "КоррПроект"
			ТаблицыПродукцияКПеремещению.Свернуть("Организация,Период,СценарийПланирования,СтруктурнаяЕдиницаПродукции,ЗатратыНаВыпуск,КоррЗаказПокупателя,КоррНоменклатура,КоррПартия,КоррСтруктурнаяЕдиница,Проект,КоррПроект,КоррСчетУчета,КоррСчетУчетаРегл,СчетУчетаЗатратРегл,КоррХарактеристика,СчетДтПродукции,СчетКтПродукции,СчетУчетаПродукции,СчетДтПродукцииРегл,СчетКтПродукцииРегл,СчетУчетаПродукцииРегл","Сумма");
		КонецЕсли;
		
		// Автосписание запасов.
		Для каждого СтрокаПродукцияКПеремещению Из ТаблицыПродукцияКПеремещению Цикл
			
			// Расход.
			СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаПродукцияКПеремещению);
			
			СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
			
			СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.КоррСтруктурнаяЕдиница;
			СтрокаТаблицыРасход.СчетУчета = СтрокаПродукцияКПеремещению.КоррСчетУчета;
			СтрокаТаблицыРасход.Номенклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыРасход.Характеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыРасход.Партия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыРасход.Спецификация = Неопределено;
			СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.СтруктурнаяЕдиницаПродукции;
			СтрокаТаблицыРасход.КоррСчетУчета = СтрокаПродукцияКПеремещению.СчетУчетаПродукции;
			//+++
			СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаПродукцияКПеремещению.КоррСчетУчетаРегл;
			СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаПродукцияКПеремещению.СчетУчетаПродукцииРегл;
			//---
			СтрокаТаблицыРасход.КоррНоменклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыРасход.КоррХарактеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыРасход.КоррПартия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
			СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			//ПодольскийАИ #УчетПроектов 13.10.2020 {
			СтрокаТаблицыРасход.Проект = СтрокаПродукцияКПеремещению.КоррПроект;
			СтрокаТаблицыРасход.КоррПроект = СтрокаПродукцияКПеремещению.Проект;
			//ПодольскийАИ #УчетПроектов 13.10.2020 }
			
			СтрокаТаблицыРасход.Сумма = СтрокаПродукцияКПеремещению.Сумма;
			СтрокаТаблицыРасход.Количество = 0;
			
			// Приход.
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаПродукцияКПеремещению);
			
			СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
			
			СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.СтруктурнаяЕдиницаПродукции;
			СтрокаТаблицыПриход.СчетУчета = СтрокаПродукцияКПеремещению.СчетУчетаПродукции;
			СтрокаТаблицыПриход.Номенклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыПриход.Характеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыПриход.Партия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыПриход.Спецификация = Неопределено;
			СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			СтрокаТаблицыПриход.СчетДт = СтрокаПродукцияКПеремещению.СчетДтПродукции;
			СтрокаТаблицыПриход.СчетКт = СтрокаПродукцияКПеремещению.СчетКтПродукции;
			
			СтрокаТаблицыПриход.СчетДтРегл = СтрокаПродукцияКПеремещению.СчетДтПродукцииРегл;
			СтрокаТаблицыПриход.СчетКтРегл = СтрокаПродукцияКПеремещению.СчетКтПродукцииРегл;
			
			СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.КоррСтруктурнаяЕдиница;
			СтрокаТаблицыПриход.КоррСчетУчета = СтрокаПродукцияКПеремещению.КоррСчетУчета;
			//+++
			СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаПродукцияКПеремещению.СчетУчетаПродукцииРегл;
			СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаПродукцияКПеремещению.КоррСчетУчетаРегл;
			//---
			СтрокаТаблицыПриход.КоррНоменклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыПриход.КоррХарактеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыПриход.КоррПартия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
			СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			//ПодольскийАИ #УчетПроектов 13.10.2020 {
			СтрокаТаблицыПриход.Проект = СтрокаПродукцияКПеремещению.КоррПроект;
			СтрокаТаблицыПриход.КоррПроект = СтрокаПродукцияКПеремещению.Проект;
			//ПодольскийАИ #УчетПроектов 13.10.2020 }
			
			СтрокаТаблицыПриход.Сумма = СтрокаПродукцияКПеремещению.Сумма;
			СтрокаТаблицыПриход.Количество = 0;
			
			СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
			СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
			
			// Сформируем проводки.
			СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
			
			ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Сборка3(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход);
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыЗапасы");
		ТаблицыПродукцияКПеремещению = Неопределено;
		
	КонецПроцедуры // СформироватьТаблицаЗапасовСборка()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыЗапасыСборкаПеремещение(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
		УчетСебестоимостиВРазрезеОрганизации = СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетСебестоимостиВРазрезеОрганизации;
		Запрос.УстановитьПараметр("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации);
		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.СценарийПланирования КАК СценарийПланирования,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетУчета,
		|	ТаблицаЗапасы.СчетУчета КАК КоррСчетУчета,
		//+++
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаРегл КАК КоррСчетУчетаРегл,
		
		//Малышев 2024-06-24 ONESOFT-3273 (--
		//|	ТаблицаЗапасы.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	выбор 
		|		когда ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			тогда значение(ПланСчетов.Хозрасчетный.ПустаяСсылка) 
		|		иначе  ТаблицаЗапасы.СчетУчетаЗатратРегл 
		|	конец как СчетУчетаЗатратРегл,
		//Малышев 2024-06-24  ONESOFT-3273 --)
		
		//---
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Номенклатура КАК КоррНоменклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Характеристика КАК КоррХарактеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Партия КАК КоррПартия,
		|	ТаблицаЗапасы.Спецификация КАК Спецификация,
		|	ТаблицаЗапасы.КоррСпецификация КАК КоррСпецификация,
		
		//Малышев 2024-06-17 вернул обратно
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ТаблицаЗапасы.КоррПроект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_
		
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК КоррЗаказПокупателя,
		|	НЕОПРЕДЕЛЕНО КАК ДокументПродажи,
		|	НЕОПРЕДЕЛЕНО КАК ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
		|	НЕОПРЕДЕЛЕНО КАК КоррДокументПоступления, // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	НЕОПРЕДЕЛЕНО КАК ЗаказПродажи,
		//|	НЕОПРЕДЕЛЕНО КАК Подразделение,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК Подразделение,
		|	НЕОПРЕДЕЛЕНО КАК Ответственный,
		|	ТаблицаЗапасы.СчетУчета КАК СчетДт,
		|	ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетКт,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетДтРегл,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетКтРегл,
		//---
		|	&ПеремещениеЗапасов КАК Содержание,
		|	&ПеремещениеЗапасов КАК СодержаниеПроводки,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество,
		|	СУММА(ТаблицаЗапасы.Резерв) КАК Резерв,
		|	СУММА(ТаблицаЗапасы.Сумма) КАК Сумма,
		|	ЛОЖЬ КАК ФиксированнаяСтоимость
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СценарийПланирования,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаЗапасов,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		
		//Малышев 2024-06-24  ONESOFT-3273 (--
		//|	ТаблицаЗапасы.СчетУчетаЗатратРегл,
		|	выбор 
		|		когда ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			тогда значение(ПланСчетов.Хозрасчетный.ПустаяСсылка) 
		|		иначе  ТаблицаЗапасы.СчетУчетаЗатратРегл 
		|	конец,
        //Малышев 2024-06-24 ONESOFT-3273 --)
		
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		//---
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		
		//Малышев 2024-06-17 вернул обратно
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_   
		
		|	ТаблицаЗапасы.Спецификация,
		|	ТаблицаЗапасы.КоррСпецификация,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.КоррСчетУчета,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаЗапасов,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл";
		//---
		
		Запрос.УстановитьПараметр("ПеремещениеЗапасов", НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'"));
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПеремещение", Запрос.Выполнить().Выгрузить());
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
		Запрос.УстановитьПараметр("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации);
		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		
		// Установка исключительной блокировки контролируемых остатков запасов.
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл, //+++
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация КАК Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
		|		ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетУчета,
		|		ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл, //+++
		|		ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|		ТаблицаЗапасы.Характеристика КАК Характеристика,
		|		ТаблицаЗапасы.Партия КАК Партия,
		|		ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|	ГДЕ
		|		ТаблицаЗапасы.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
		|		ТаблицаЗапасы.СчетУчетаЗапасов,
		|		ТаблицаЗапасы.СчетУчетаЗапасовРегл, //+++
		|		ТаблицаЗапасы.Номенклатура,
		|		ТаблицаЗапасы.Характеристика,
		|		ТаблицаЗапасы.Партия,
		|		ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы) КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл, //+++
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.Запасы");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			Если КолонкаРезультатЗапроса.Имя <> "СчетУчетаРегл" Тогда 
				ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
			КонецЕсли;
		КонецЦикла;
		Блокировка.Заблокировать();
		
		
		//Малышев 2025-01-10 ONESOFT-3758 формируем таблицу, на какие товары вывести остатки (--
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказПокупателя
		|ПОМЕСТИТЬ ТаблицаПеремещаемыхТоваров
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	(ТаблицаЗапасы.Количество - ТаблицаЗапасы.Резерв) <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.ЗаказПокупателя
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	И ТаблицаЗапасы.Резерв <> 0";
		Запрос.Выполнить();
		//Малышев Малышев 2025-01-10 ONESOFT-3758 формируем таблицу, на какие товары вывести остатки --)
	
		
		
		
		// Получение остатков запасов по стоимости.
		//Малышев 2024-06-13 ONESOFT-3092 - запрос перенесен в отдельный модуль (--
		//ТаблицаЗапасыОстаткиПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении_СборкаПеремещение(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства).Выгрузить();
		//ТаблицаЗапасыОстаткиПеремещение.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчета,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя");

		//ТаблицаЗапасыОстаткиПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, Истина).Выгрузить();
		ТаблицаЗапасыОстаткиПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, Ложь, "ТаблицаПеремещаемыхТоваров",,,Ложь).Выгрузить(); //Малышев 2025-01-10 ONESOFT-3758
		
		ТаблицаЗапасыОстаткиПеремещение.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя"); 
		//Малышев 2024-06-13 ONESOFT-3092 --)
		
		ВременнаяТаблицаЗапасыПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.СкопироватьКолонки();
		
		ПустаяСтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
		ПустойСчетУчета = ПланыСчетов.Управленческий.ПустаяСсылка();
		ПустойСчетУчетаРегл = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
		ПустаяНоменклатура = Справочники.Номенклатура.ПустаяСсылка();
		ПустаяХарактеристика = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
		ПустаяПартия = Справочники.ПартииНоменклатуры.ПустаяСсылка();
		ПустойЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасыПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение[н];
			
			//Малышев 2024-07-27 ((-
			СтруктураДляПоискаПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьСтруктуруДляПоиска(СтрокаТаблицаЗапасыПеремещение, СтруктураДополнительныеСвойства);
			//СтруктураДляПоискаПеремещение = Новый Структура;
			//СтруктураДляПоискаПеремещение.Вставить("Организация", СтрокаТаблицаЗапасыПеремещение.Организация);
			////--------------------------------------------------------------------------
			//// © Шишка В.В. [11 октября 2021]  НАЧАЛО
			////СтруктураДляПоискаПеремещение.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница);
			//СтруктураДляПоискаПеремещение.Вставить("СтруктурнаяЕдиница", ?(УчетСебестоимостиВРазрезеОрганизации И 
			//СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад, Неопределено, СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница));
			//// © Шишка В.В. [11 октября 2021]  КОНЕЦ
			////--------------------------------------------------------------------------
			////Если СтруктураДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда 
			////	СтруктураДляПоискаПеремещение.Вставить("СчетУчета", ПустойСчетУчета);
			////Иначе
			////	СтруктураДляПоискаПеремещение.Вставить("СчетУчета", СтрокаТаблицаЗапасыПеремещение.СчетУчета);
			////КонецЕсли;
			//СтруктураДляПоискаПеремещение.Вставить("СчетУчетаРегл", СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл);
			//СтруктураДляПоискаПеремещение.Вставить("Номенклатура", СтрокаТаблицаЗапасыПеремещение.Номенклатура);
			//СтруктураДляПоискаПеремещение.Вставить("Характеристика", СтрокаТаблицаЗапасыПеремещение.Характеристика);
			//СтруктураДляПоискаПеремещение.Вставить("Партия", СтрокаТаблицаЗапасыПеремещение.Партия);
			////Малышев 2024-06-16 ONESOFT-3094
			//Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
			//	СтруктураДляПоискаПеремещение.Вставить("Проект", СтрокаТаблицаЗапасыПеремещение.Проект);
			//КонецЕсли;	
            //Малышев 2024-07-27 --)
	


			
			//КоличествоТребуетсяРезервПеремещение = СтрокаТаблицаЗапасыПеремещение.Резерв;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
				КоличествоТребуетсяРезервПеремещение = 0;
			Иначе
				КоличествоТребуетсяРезервПеремещение = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасыПеремещение.Резерв), СтрокаТаблицаЗапасыПеремещение.Резерв, 0);
			КонецЕсли;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			КоличествоТребуетсяСвободныйОстатокПеремещение = СтрокаТаблицаЗапасыПеремещение.Количество;
			
			Если КоличествоТребуетсяРезервПеремещение > 0 Тогда
				
				КоличествоТребуетсяСвободныйОстатокПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение - КоличествоТребуетсяРезервПеремещение;
				
				СтруктураДляПоискаПеремещение.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя);
				
				МассивСтрокОстатковПеремещение = ТаблицаЗапасыОстаткиПеремещение.НайтиСтроки(СтруктураДляПоискаПеремещение);
				
				//Малышев 2024-11-19 ONESOFT-3602
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатковПеремещение Цикл
			
					Если КоличествоТребуетсяРезервПеремещение <= 0 Тогда
						Прервать;
					КонецЕсли;	
 

				
				//КоличествоОстатокПеремещение = 0;
				//СуммаОстатокПеремещение = 0;
				//
				//Если МассивСтрокОстатковПеремещение.Количество() > 0 Тогда
				//	КоличествоОстатокПеремещение = МассивСтрокОстатковПеремещение[0].КоличествоОстаток;
				//	СуммаОстатокПеремещение = МассивСтрокОстатковПеремещение[0].СуммаОстаток;
				//КонецЕсли;
				
				КоличествоОстатокПеремещение = СтрокаТаблицыОстатков.КоличествоОстаток;
				СуммаОстатокПеремещение = СтрокаТаблицыОстатков.СуммаОстаток;
				
				КоличествоКСписаниюПеремещение = 0;
				СуммаКСписаниюПеремещение = 0;
				
				
				Если КоличествоОстатокПеремещение > 0 И КоличествоОстатокПеремещение > КоличествоТребуетсяРезервПеремещение Тогда
					
					КоличествоКСписаниюПеремещение = КоличествоТребуетсяРезервПеремещение;
					СуммаКСписаниюПеремещение = Окр(СуммаОстатокПеремещение * КоличествоТребуетсяРезервПеремещение / КоличествоОстатокПеремещение , 2, 1);
					
					//МассивСтрокОстатковПеремещение[0].КоличествоОстаток = МассивСтрокОстатковПеремещение[0].КоличествоОстаток - КоличествоТребуетсяРезервПеремещение;
					СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - КоличествоКСписаниюПеремещение;
					
					//МассивСтрокОстатковПеремещение[0].СуммаОстаток = МассивСтрокОстатковПеремещение[0].СуммаОстаток - СуммаКСписаниюПеремещение;
					СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписаниюПеремещение;
					
				//ИначеЕсли КоличествоОстатокПеремещение = КоличествоТребуетсяРезервПеремещение Тогда
				ИначеЕсли КоличествоОстатокПеремещение >0 и КоличествоОстатокПеремещение <= КоличествоТребуетсяРезервПеремещение Тогда //Малышев 2024-11-19 ONESOFT-3602

	
					КоличествоКСписаниюПеремещение = КоличествоОстатокПеремещение;
					СуммаКСписаниюПеремещение = СуммаОстатокПеремещение;
					
					//МассивСтрокОстатковПеремещение[0].КоличествоОстаток = 0;
					СтрокаТаблицыОстатков.КоличествоОстаток = 0;
					//МассивСтрокОстатковПеремещение[0].СуммаОстаток = 0;
					СтрокаТаблицыОстатков.СуммаОстаток = 0;
					
				//Малышев 2024-11-19 ONESOFT-3602 (--	
				
				ИначеЕсли КоличествоОстатокПеремещение <= 0 Тогда  
					Продолжить;
				//Малышев 2024-11-19 ONESOFT-3602 --)
		
	
					
				Иначе
					СуммаКСписаниюПеремещение = 0;	
				КонецЕсли;
				
				КоличествоТребуетсяРезервПеремещение = КоличествоТребуетсяРезервПеремещение - КоличествоКСписаниюПеремещение; //Малышев 2024-11-19 ONESOFT-3602
			
				
				// Расход.
				СтрокаТаблицыРасходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПеремещение, СтрокаТаблицаЗапасыПеремещение);
				
				СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено;
				СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено;
				
				СтрокаТаблицыРасходПеремещение.Сумма = СуммаКСписаниюПеремещение;
				//СтрокаТаблицыРасходПеремещение.Количество = КоличествоТребуетсяРезервПеремещение;
				СтрокаТаблицыРасходПеремещение.Количество = КоличествоКСписаниюПеремещение;  //Малышев 2024-11-19 ONESOFT-3602
				СтрокаТаблицыРасходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления; //Малышев 2024-11-19 ONESOFT-3602
				
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 	
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Сформируем проводки.
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 Тогда
					СтрокаТаблицаУправленческийПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческийПеремещение, СтрокаТаблицаЗапасыПеремещение);
					СтрокаТаблицаУправленческийПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение1(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СуммаКСписаниюПеремещение);
				КонецЕсли;
				
				// Приход.
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоТребуетсяРезервПеремещение > 0 Тогда
					
					СтрокаТаблицыПриходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриходПеремещение, СтрокаТаблицаЗапасыПеремещение);
					
					СтрокаТаблицыПриходПеремещение.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриходПеремещение.Организация = СтрокаТаблицаЗапасыПеремещение.Организация;
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					//СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница =  ?(УчетСебестоимостиВРазрезеОрганизации И (СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница = Неопределено ИЛИ
					СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад), Неопределено, СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница);
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.Номенклатура = СтрокаТаблицаЗапасыПеремещение.КоррНоменклатура;
					СтрокаТаблицыПриходПеремещение.Характеристика = СтрокаТаблицаЗапасыПеремещение.КоррХарактеристика;
					СтрокаТаблицыПриходПеремещение.Партия = СтрокаТаблицаЗапасыПеремещение.КоррПартия;
					СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено;
					
					СтрокаТаблицыПриходПеремещение.ЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.КоррЗаказПокупателя;
					
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					//СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница =  ?(УчетСебестоимостиВРазрезеОрганизации И (СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница = Неопределено ИЛИ
					СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад), Неопределено, СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница);
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					СтрокаТаблицыПриходПеремещение.КоррСчетУчета = СтрокаТаблицаЗапасыПеремещение.СчетУчета;
					//+++
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриходПеремещение.КоррНоменклатура = СтрокаТаблицаЗапасыПеремещение.Номенклатура;
					СтрокаТаблицыПриходПеремещение.КоррХарактеристика = СтрокаТаблицаЗапасыПеремещение.Характеристика;
					СтрокаТаблицыПриходПеремещение.КоррПартия = СтрокаТаблицаЗапасыПеремещение.Партия;
					СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено;
					
					СтрокаТаблицыПриходПеремещение.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя;
					
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриходПеремещение.Проект = СтрокаТаблицаЗапасыПеремещение.КоррПроект;
					СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриходПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					//СтрокаТаблицыПриходПеремещение.Количество = КоличествоТребуетсяРезервПеремещение;
					СтрокаТаблицыПриходПеремещение.Количество = КоличествоКСписаниюПеремещение; // Kuzmich, 27.03.2025, (ONESOFT-3962) Документ Производство - списание материалов с резервами
					
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл; //+++ Дмгг
					
					СтрокаТаблицыПриходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					//Малышев 2024-06-24 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриходПеремещение);		
					//Малышев 2024-06-24 ONESOFT-3273 --)
					
					
				КонецЕсли;
				КонецЦикла; //Малышев 2024-11-19 ONESOFT-3602
			КонецЕсли;
			
			Если КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
				
				СтруктураДляПоискаПеремещение.Вставить("ЗаказПокупателя", ПустойЗаказПокупателя);
				
				МассивСтрокОстатковПеремещение = ТаблицаЗапасыОстаткиПеремещение.НайтиСтроки(СтруктураДляПоискаПеремещение);
				
				//Малышев 2024-11-19 ONESOFT-3602
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатковПеремещение Цикл
				
					Если КоличествоТребуетсяСвободныйОстатокПеремещение <= 0 Тогда
						Прервать;
					КонецЕсли;	
 

				
				//КоличествоОстатокПеремещение = 0;
				//СуммаОстатокПеремещение = 0;
				//
				//Если МассивСтрокОстатковПеремещение.Количество() > 0 Тогда
				//	КоличествоОстатокПеремещение = МассивСтрокОстатковПеремещение[0].КоличествоОстаток;
				//	СуммаОстатокПеремещение = МассивСтрокОстатковПеремещение[0].СуммаОстаток;
				//КонецЕсли;
				
				КоличествоОстатокПеремещение = СтрокаТаблицыОстатков.КоличествоОстаток;
				СуммаОстатокПеремещение = СтрокаТаблицыОстатков.СуммаОстаток;
				
				СуммаКСписаниюПеремещение = 0;
				КоличествоКСписаниюПеремещение = 0;
				
				Если КоличествоОстатокПеремещение > 0 И КоличествоОстатокПеремещение > КоличествоТребуетсяСвободныйОстатокПеремещение Тогда
					
					КоличествоКСписаниюПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение;
					СуммаКСписаниюПеремещение = Окр(СуммаОстатокПеремещение * КоличествоТребуетсяСвободныйОстатокПеремещение / КоличествоОстатокПеремещение , 2, 1);
					
					//МассивСтрокОстатковПеремещение[0].КоличествоОстаток = МассивСтрокОстатковПеремещение[0].КоличествоОстаток - КоличествоТребуетсяСвободныйОстатокПеремещение;
					СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - КоличествоКСписаниюПеремещение;
					//МассивСтрокОстатковПеремещение[0].СуммаОстаток = МассивСтрокОстатковПеремещение[0].СуммаОстаток - СуммаКСписаниюПеремещение;
					СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписаниюПеремещение;
					
				//ИначеЕсли КоличествоОстатокПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение Тогда
				ИначеЕсли КоличествоОстатокПеремещение >0 и КоличествоОстатокПеремещение <= КоличествоТребуетсяСвободныйОстатокПеремещение Тогда //Малышев 2024-11-19 ONESOFT-3602

	
					КоличествоКСписаниюПеремещение = КоличествоОстатокПеремещение; 
					СуммаКСписаниюПеремещение = СуммаОстатокПеремещение;
					
					//МассивСтрокОстатковПеремещение[0].КоличествоОстаток = 0;
					СтрокаТаблицыОстатков.КоличествоОстаток = 0;
					//МассивСтрокОстатковПеремещение[0].СуммаОстаток = 0;
                    СтрокаТаблицыОстатков.СуммаОстаток = 0;


				//Малышев 2024-11-19 ONESOFT-3602 (--	
				
				ИначеЕсли КоличествоОстатокПеремещение <= 0 Тогда  
					Продолжить;
				//Малышев 2024-11-19 ONESOFT-3602 --)
		
	
				Иначе
					СуммаКСписаниюПеремещение = 0;	
				КонецЕсли;
				
				КоличествоТребуетсяСвободныйОстатокПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение - КоличествоКСписаниюПеремещение; //Малышев 2024-11-19 ONESOFT-3602
				
				
				// Расход.    расход материалов для сборки (со склада или из подразделения)
				СтрокаТаблицыРасходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПеремещение, СтрокаТаблицаЗапасыПеремещение);
				
				СтрокаТаблицыРасходПеремещение.Сумма = СуммаКСписаниюПеремещение;
				
				//СтрокаТаблицыРасходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
				СтрокаТаблицыРасходПеремещение.Количество = КоличествоКСписаниюПеремещение; //Малышев 2024-11-19 ONESOFT-3602
				//СтрокаТаблицыРасходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.Документпоступления; //Малышев 2024-11-19 ONESOFT-3602
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыРасходПеремещение.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				КонецЕсли;
				Если СтрокаТаблицыРасходПеремещение.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасходПеремещение.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СтрокаТаблицыРасходПеремещение.ЗаказПокупателя = ПустойЗаказПокупателя;
				СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено;
				СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено;
				СтрокаТаблицыРасходПеремещение.КоррПроект = СтрокаТаблицыРасходПеремещение.Проект; //Малышев 2024-08-20
					
				// Сформируем проводки.
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 Тогда
					СтрокаТаблицаУправленческийПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческийПеремещение, СтрокаТаблицаЗапасыПеремещение);
					СтрокаТаблицаУправленческийПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение2(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СуммаКСписаниюПеремещение, КоличествоКСписаниюПеремещение);
				КонецЕсли;
				
				// Приход.  Поступление материала 
				//Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоКСписаниюПеремещение > 0 Тогда
					
					СтрокаТаблицыПриходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриходПеремещение, СтрокаТаблицаЗапасыПеремещение);
					
					СтрокаТаблицыПриходПеремещение.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриходПеремещение.Организация = СтрокаТаблицаЗапасыПеремещение.Организация;
					СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.Номенклатура = СтрокаТаблицаЗапасыПеремещение.КоррНоменклатура;
					СтрокаТаблицыПриходПеремещение.Характеристика = СтрокаТаблицаЗапасыПеремещение.КоррХарактеристика;
					СтрокаТаблицыПриходПеремещение.Партия = СтрокаТаблицаЗапасыПеремещение.КоррПартия;
					СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено;
					
					СтрокаТаблицыПриходПеремещение.ЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя;
					
					СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчета = СтрокаТаблицаЗапасыПеремещение.СчетУчета;
					//+++
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриходПеремещение.КоррНоменклатура = СтрокаТаблицаЗапасыПеремещение.Номенклатура;
					СтрокаТаблицыПриходПеремещение.КоррХарактеристика = СтрокаТаблицаЗапасыПеремещение.Характеристика;
					СтрокаТаблицыПриходПеремещение.КоррПартия = СтрокаТаблицаЗапасыПеремещение.Партия;
					СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено;
					
					СтрокаТаблицыПриходПеремещение.КоррЗаказПокупателя = ПустойЗаказПокупателя;
					
					
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					//СтрокаТаблицыПриходПеремещение.Проект = СтрокаТаблицаЗапасыПеремещение.КоррПроект;
					//СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриходПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					//СтрокаТаблицыПриходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
					СтрокаТаблицыПриходПеремещение.Количество = КоличествоКСписаниюПеремещение;  //Малышев 2024-11-19 ONESOFT-3602
					
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;//+++ Дмгг
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					Если СтрокаТаблицыПриходПеремещение.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					Если СтрокаТаблицыПриходПеремещение.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриходПеремещение.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					//Малышев 2024-06-24 ONESOFT-3273 (--
					//Если СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение
					//	и не ЗначениеЗаполнено(СтрокаТаблицыПриходПеремещение.СчетУчетаЗатратРегл)
					//	Тогда 
					//	СтрокаТаблицыПриходПеремещение.СчетУчетаЗатратРегл = СтрокаТаблицаЗапасыПеремещение.СчетДтРегл;
					//КонецЕсли;
					СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицыПриходПеремещение.Проект; //Малышев 2024-08-20
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриходПеремещение);		
					//Малышев 2024-06-24 ONESOFT-3273 --)
					
				КонецЕсли;
				КонецЦикла; //Малышев 2024-11-19 ONESOFT-3602
				
				// Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// делаем отрицательный остаток
				Если КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
					
					// Расход.    расход материалов для сборки (со склада или из подразделения)
					СтрокаТаблицыРасходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПеремещение, СтрокаТаблицаЗапасыПеремещение);
					
					СтрокаТаблицыРасходПеремещение.Сумма = 0;
					
					СтрокаТаблицыРасходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					СтрокаТаблицыРасходПеремещение.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					СтрокаТаблицыРасходПеремещение.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					СтрокаТаблицыРасходПеремещение.ЗаказПокупателя = ПустойЗаказПокупателя;
					СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено;
					СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено;
					СтрокаТаблицыРасходПеремещение.КоррПроект = СтрокаТаблицыРасходПеремещение.Проект; //Малышев 2024-08-20
					
					// Сформируем проводки.
					//Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческийПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческийПеремещение, СтрокаТаблицаЗапасыПеремещение);
						СтрокаТаблицаУправленческийПеремещение.Сумма = 0;
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение2(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, 0, КоличествоТребуетсяСвободныйОстатокПеремещение);		
					//КонецЕсли;
					
					// Приход.  Поступление материала 
					//Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоКСписаниюПеремещение > 0 Тогда // Kuzmich, 23.04.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
						
						СтрокаТаблицыПриходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриходПеремещение, СтрокаТаблицаЗапасыПеремещение);
						
						СтрокаТаблицыПриходПеремещение.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриходПеремещение.Организация = СтрокаТаблицаЗапасыПеремещение.Организация;
						СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
						СтрокаТаблицыПриходПеремещение.Номенклатура = СтрокаТаблицаЗапасыПеремещение.КоррНоменклатура;
						СтрокаТаблицыПриходПеремещение.Характеристика = СтрокаТаблицаЗапасыПеремещение.КоррХарактеристика;
						СтрокаТаблицыПриходПеремещение.Партия = СтрокаТаблицаЗапасыПеремещение.КоррПартия;
						СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						СтрокаТаблицыПриходПеремещение.ДокументПоступления = НЕОПРЕДЕЛЕНО;
						СтрокаТаблицыПриходПеремещение.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						СтрокаТаблицыПриходПеремещение.ЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя;
						
						СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница;
						СтрокаТаблицыПриходПеремещение.КоррСчетУчета = СтрокаТаблицаЗапасыПеремещение.СчетУчета;
						//+++
						СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;
						СтрокаТаблицыПриходПеремещение.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл;
						//---
						СтрокаТаблицыПриходПеремещение.КоррНоменклатура = СтрокаТаблицаЗапасыПеремещение.Номенклатура;
						СтрокаТаблицыПриходПеремещение.КоррХарактеристика = СтрокаТаблицаЗапасыПеремещение.Характеристика;
						СтрокаТаблицыПриходПеремещение.КоррПартия = СтрокаТаблицаЗапасыПеремещение.Партия;
						СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено;
						
						СтрокаТаблицыПриходПеремещение.КоррЗаказПокупателя = ПустойЗаказПокупателя;
						
						СтрокаТаблицыПриходПеремещение.Сумма = 0;
						
						СтрокаТаблицыПриходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
						
						СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
						
						СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;//+++ Дмгг
						
						СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицыПриходПеремещение.Проект; //Малышев 2024-08-20
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриходПеремещение);		
						
					//КонецЕсли;
				КонецЕсли;
				// Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение = ВременнаяТаблицаЗапасыПеремещение;
		
		//Перенесем все перемещения в таблицу движений по регистру Запасы
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение[н];
			
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
			
		КонецЦикла;	
		
		ВременнаяТаблицаЗапасыПеремещение.Индексы.Добавить("ВидДвижения,Организация,СтруктурнаяЕдиница,СчетУчета,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя");

		
		СуммаКПеремещению = 0;
		СтрокаТаблицыЗапасыКПеремещению = Неопределено;
		ТаблицыПродукцияКПеремещению = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.СкопироватьКолонки();
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы[н];
			
			//СтруктураДляПоиска = Новый Структура;
			//СтруктураДляПоиска.Вставить("ВидДвижения", ВидДвиженияНакопления.Приход);
			//СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасы.Организация);
			////--------------------------------------------------------------------------
			//// © Шишка В.В. [11 октября 2021]  НАЧАЛО
			////СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасы.СтруктурнаяЕдиница);
			//СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", ?(УчетСебестоимостиВРазрезеОрганизации 
			//И( СтрокаТаблицаЗапасы.СтруктурнаяЕдиница = Неопределено ИЛИ СтрокаТаблицаЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад), Неопределено, СтрокаТаблицаЗапасы.СтруктурнаяЕдиница));
			//// © Шишка В.В. [11 октября 2021]  КОНЕЦ
			////--------------------------------------------------------------------------
			//СтруктураДляПоиска.Вставить("СчетУчета", СтрокаТаблицаЗапасы.СчетУчета);
			//СтруктураДляПоиска.Вставить("СчетУчетаРегл", СтрокаТаблицаЗапасы.СчетУчетаРегл);
			//СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасы.Номенклатура);
			//СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасы.Характеристика);
			//СтруктураДляПоиска.Вставить("Партия", СтрокаТаблицаЗапасы.Партия);
			////Малышев 2024-06-25   (--
			//Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
			//	СтруктураДляПоиска.Вставить("Проект", СтрокаТаблицаЗапасы.Проект);
			//КонецЕсли;	
			////Малышев 2024-06-25 --)
			
			
			
			//КоличествоТребуетсяРезерв = СтрокаТаблицаЗапасы.Резерв;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			СтруктураДляПоиска = УправлениеНебольшойФирмойСервер.ПолучитьСтруктуруДляПоиска(СтрокаТаблицаЗапасы, СтруктураДополнительныеСвойства);
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
				КоличествоТребуетсяРезерв = 0;
			Иначе
				КоличествоТребуетсяРезерв = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасы.Резерв), СтрокаТаблицаЗапасы.Резерв, 0);
			КонецЕсли;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			КоличествоТребуетсяСвободныйОстаток = СтрокаТаблицаЗапасы.Количество;
			
			//Малышев 2024-07-09
			СуммаДопРасходов = 0;
			
			Если КоличествоТребуетсяРезерв > 0 Тогда
				
				КоличествоТребуетсяСвободныйОстаток = КоличествоТребуетсяСвободныйОстаток - КоличествоТребуетсяРезерв;
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
				
				МассивКоличествоОстаток = 0;
				МассивСуммаОстаток = 0;
				МассивСтрокОстатков = ВременнаяТаблицаЗапасыПеремещение.НайтиСтроки(СтруктураДляПоиска);
				Для каждого СтрокаОстатков Из МассивСтрокОстатков Цикл
					МассивКоличествоОстаток = МассивКоличествоОстаток + СтрокаОстатков.Количество;
					МассивСуммаОстаток = МассивСуммаОстаток + СтрокаОстатков.Сумма;
				КонецЦикла;
				
				КоличествоОстаток = 0;
				СуммаОстаток = 0;
				Если МассивСтрокОстатков.Количество() > 0 Тогда
					КоличествоОстаток = МассивСтрокОстатков[0].Количество;
					СуммаОстаток = МассивСтрокОстатков[0].Сумма;
					МассивСтрокОстатков[0].Количество = МассивКоличествоОстаток;
					МассивСтрокОстатков[0].Сумма = МассивСуммаОстаток;
				КонецЕсли;
				
				Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяРезерв Тогда
					
					СуммаКСписанию = Окр(СуммаОстаток * КоличествоТребуетсяРезерв / КоличествоОстаток , 2, 1);
					
					МассивСтрокОстатков[0].Количество = МассивСтрокОстатков[0].Количество - КоличествоТребуетсяРезерв;
					МассивСтрокОстатков[0].Сумма = МассивСтрокОстатков[0].Сумма - СуммаКСписанию;
					
				ИначеЕсли КоличествоОстаток = КоличествоТребуетсяРезерв Тогда
					
					СуммаКСписанию = СуммаОстаток;
					
					МассивСтрокОстатков[0].Количество = 0;
					МассивСтрокОстатков[0].Сумма = 0;
					
				Иначе
					СуммаКСписанию = 0;	
				КонецЕсли;
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Спишем запасы со склада (производственного подразделения).
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				СтрокаТаблицыРасход.Количество = КоличествоТребуетсяРезерв;
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Отнесем списанные запасы либо на стоимость запасов на складе либо на затраты НЗП.
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
					СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг

					//Малышев 2024-06-24 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
    				//Малышев 2024-06-24 ONESOFT-3273 --)

					// Сформируем проводки.
					Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение3(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					КонецЕсли;
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					// Австосписание запасов.
					СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
					
				КонецЕсли;
				
			КонецЕсли;
			
			Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
				//Малышев 2025-01-10 ONESOFT-3758 закоментировал
				//СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если НЕ СтрокаТаблицаЗапасы.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				МассивКоличествоОстаток = 0;
				МассивСуммаОстаток = 0;
				МассивСтрокОстатков = ВременнаяТаблицаЗапасыПеремещение.НайтиСтроки(СтруктураДляПоиска);
				Для каждого СтрокаОстатков Из МассивСтрокОстатков Цикл
					МассивКоличествоОстаток = МассивКоличествоОстаток + СтрокаОстатков.Количество;
					МассивСуммаОстаток = МассивСуммаОстаток + СтрокаОстатков.Сумма;
				КонецЦикла;
				
				КоличествоОстаток = 0;
				СуммаОстаток = 0;
				Если МассивСтрокОстатков.Количество() > 0 Тогда
					МассивСтрокОстатков[0].Количество = МассивКоличествоОстаток;
					МассивСтрокОстатков[0].Сумма = МассивСуммаОстаток;
					КоличествоОстаток = МассивСтрокОстатков[0].Количество;
					СуммаОстаток = МассивСтрокОстатков[0].Сумма;
				КонецЕсли;
				
				Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
					
					СуммаКСписанию = Окр(СуммаОстаток * КоличествоТребуетсяСвободныйОстаток / КоличествоОстаток , 2, 1);
					
					//// Изменения
					МассивСтрокОстатков[0].Количество = МассивСтрокОстатков[0].Количество - КоличествоТребуетсяСвободныйОстаток;
					МассивСтрокОстатков[0].Сумма = МассивСтрокОстатков[0].Сумма - СуммаКСписанию;
					//// Изменения
					
				ИначеЕсли КоличествоОстаток = КоличествоТребуетсяСвободныйОстаток Тогда
					
					СуммаКСписанию = СуммаОстаток;
					
					МассивСтрокОстатков[0].Количество = 0;
					МассивСтрокОстатков[0].Сумма = 0;
					
				Иначе
					СуммаКСписанию = 0;	
				КонецЕсли;
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;

				// Расход.  затраты из основного производственно подразделения
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				СтрокаТаблицыРасход.Количество = КоличествоТребуетсяСвободныйОстаток;
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
				СтрокаТаблицыРасход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
				
				//СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура; //Малышев 2024-07-08
				//СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
				//СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
				//СтрокаТаблицыРасход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
				//СтрокаТаблицыРасход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
				//Малышев 2024-06-25 ONESOFT-3273 --)


				
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Приход.    в затраты основного производящего подразделения
				//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, Закоментировал
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					
					//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO И МассивСтрокОстатков.Количество() > 0 Тогда
					Если МассивСтрокОстатков.Количество() > 0 Тогда
						СтрокаТаблицыПриход.ДокументПоступления = СтрокаТаблицаЗапасы.ДокументПоступления;
					КонецЕсли;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости

					//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
    				//Малышев 2024-06-25 ONESOFT-3273 --)


					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					// Сформируем проводки.
					//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение4(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//КонецЕсли;
					
					// Автосписание запасов.
					СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
					
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Иначе
				
					КоличествоТребуетсяСвободныйОстатокFIFO = КоличествоТребуетсяСвободныйОстаток;
					
					МассивСтрокОстатков = ВременнаяТаблицаЗапасыПеремещение.НайтиСтроки(СтруктураДляПоиска);
					Для каждого СтрокаОстатков Из МассивСтрокОстатков Цикл
						
						КоличествоОстаток = СтрокаОстатков.Количество;
						СуммаОстаток = СтрокаОстатков.Сумма;
						
						Если (КоличествоОстаток > 0) И (КоличествоОстаток > КоличествоТребуетсяСвободныйОстатокFIFO) И (КоличествоТребуетсяСвободныйОстатокFIFO > 0) Тогда
							
							СуммаКСписанию = Окр(СуммаОстаток * КоличествоТребуетсяСвободныйОстаток / КоличествоОстаток , 2, 1);
							
						ИначеЕсли (КоличествоОстаток > 0) И (КоличествоТребуетсяСвободныйОстатокFIFO > 0) Тогда
							
							СуммаКСписанию = СуммаОстаток;
							
						Иначе
							
							СуммаКСписанию = 0;
							
						КонецЕсли;
						
						СуммаСборки = СуммаСборки + СуммаКСписанию;
						
						КоличествоТребуетсяСвободныйОстатокFIFO = КоличествоТребуетсяСвободныйОстатокFIFO - КоличествоОстаток;
						
						// Расход.  затраты из основного производственно подразделения
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
						СтрокаТаблицыРасход.Количество = КоличествоОстаток;
						СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
						СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
						
						Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.ДокументПоступления = СтрокаОстатков.ДокументПоступления;
						КонецЕсли;
						Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
						
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						
						// Приход.
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
						
						СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
						СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
						//+++
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
						//---
						
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
						СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
						
						//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
						СтрокаТаблицыПриход.Количество = 0;
						
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
						
						Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.КоррДокументПоступления = СтрокаОстатков.ДокументПоступления;
						КонецЕсли;

						//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
	    				//Малышев 2024-06-25 ONESOFT-3273 --)
						
						//--------------------------------------------------------------------------
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						// Сформируем проводки.
						//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
							СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
							
							ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение4(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						//КонецЕсли;
						
						// Автосписание запасов.
						СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
						
					КонецЦикла;
					
					// Kuzmich, 12.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
					// делаем отрицательный остаток
					Если МассивСтрокОстатков.Количество()=0 ИЛИ КоличествоТребуетсяСвободныйОстатокFIFO > 0 Тогда
					
						// Расход.  затраты из основного производственно подразделения
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыРасход.Сумма = 0;
						СтрокаТаблицыРасход.Количество = КоличествоТребуетсяСвободныйОстатокFIFO;
						СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
						СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
						
						СтрокаТаблицыРасход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
						
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
						
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						
						// Приход.
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
						
						СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
						СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
						//+++
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
						//---
						
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
						СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
						
						//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыПриход.Сумма = 0;
						СтрокаТаблицыПриход.Количество = 0;
						
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
						
						СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;

						//Малышев 2024-06-25 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
	    				//Малышев 2024-06-25 ONESOFT-3273 --)
						
						//--------------------------------------------------------------------------
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						// Сформируем проводки.
						//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
							СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
							
							ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение4(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						//КонецЕсли;
						
						// Автосписание запасов.
						СуммаКПеремещению = СуммаКПеремещению + СуммаКСписанию;
					
					КонецЕсли;
					// Kuzmich, 12.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში, закоментировал
					
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
			КонецЕсли;
			
			// Автосписание запасов.    - расход - затраты основного производителя спишем
			СтрокаТаблицыЗапасыКПеремещению = СтрокаТаблицаЗапасы;
			
			//Если СуммаКПеремещению > 0 
			//	И СтрокаТаблицыЗапасыКПеремещению <> Неопределено 
			//	И ЗначениеЗаполнено(СтрокаТаблицыЗапасыКПеремещению.СтруктурнаяЕдиницаПродукции) Тогда
			// Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
			// делаем отрицательный остаток
			Если СтрокаТаблицыЗапасыКПеремещению <> Неопределено 
				И ЗначениеЗаполнено(СтрокаТаблицыЗапасыКПеремещению.СтруктурнаяЕдиницаПродукции) Тогда
			// Kuzmich, 19.05.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				
				НоваяСтрока = ТаблицыПродукцияКПеремещению.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицыЗапасыКПеремещению);
				НоваяСтрока.Сумма = СуммаКПеремещению;
				
			КонецЕсли;
			
			СуммаКПеремещению = 0;
			
		КонецЦикла;
		
		//Малышев 2024-07-11 ONESOFT-3310
		//сделаем записи по допрасходам, если они есть только по Подразделениям
		//Если СтруктураДополнительныеСвойства.ДляПроведения.Свойство("СтруктураТаблицПоДопРасходам") Тогда
		//	
		//	Если СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение
		//		Тогда
		//		МассивДопЗатратПроизводства = Новый Массив;
		//		
		//		//приход допрасходов в производство
		//		Для каждого СтрДопРасхода из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
		//			СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
		//			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
		//			
		//			СтрокаТаблицыПриход_ДопРасходы.СчетУчетаРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗатратРегл;
		//			СтрокаТаблицыПриход_ДопРасходы.СчетУчетаЗатратРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗатратРегл;
		//			СтрокаТаблицыПриход_ДопРасходы.КоррПроект = СтрДопРасхода.Проект;
		//			
		//			МассивДопЗатратПроизводства.Добавить(СтрокаТаблицыПриход_ДопРасходы);
		//		КонецЦикла;
		//		
		//		//расход допрасходов в производство
		//		Для каждого СтрДопРасхода из МассивДопЗатратПроизводства Цикл
		//			СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
		//			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
		//			СтрокаТаблицыПриход_ДопРасходы.ВидДвижения = ВидДвиженияНакопления.Расход;
		//		КонецЦикла;	
		//		
		//	Иначе
		//		////приход допрасходов в производство
		//		//Для каждого СтрДопРасхода из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
		//		//	СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
		//		//	ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
		//		//	
		//		//	//СтрокаТаблицыПриход_ДопРасходы.СчетУчетаРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗатратРегл;
		//		//	//СтрокаТаблицыПриход_ДопРасходы.СчетУчетаЗатратРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗатратРегл;
		//		//	
		//		//	//МассивДопЗатратПроизводства.Добавить(СтрокаТаблицыПриход_ДопРасходы);
		//		//КонецЦикла;
		//		
		//	КонецЕсли;
		//	
		//КонецЕсли;	
		
		
		
		Если ТаблицыПродукцияКПеремещению.Количество() > 1 Тогда
			//ТаблицыПродукцияКПеремещению.Свернуть("Организация,Период,СценарийПланирования,СтруктурнаяЕдиницаПродукции,ЗатратыНаВыпуск,КоррЗаказПокупателя,КоррНоменклатура,КоррПартия,КоррСтруктурнаяЕдиница,КоррСчетУчета,КоррХарактеристика,СчетДтПродукции,СчетКтПродукции,СчетУчетаПродукции","Сумма");
			//ПодольскийАИ #УчетПроектов 13.10.2020 {
			//Добавлены поля "Проект", "КоррПрокет"
			ТаблицыПродукцияКПеремещению.Свернуть("Организация,Период,СценарийПланирования,СтруктурнаяЕдиницаПродукции,ЗатратыНаВыпуск,КоррЗаказПокупателя,КоррНоменклатура,КоррПартия,КоррСтруктурнаяЕдиница,Проект,КоррПроект,КоррСчетУчета,КоррСчетУчетаРегл,СчетУчетаЗатратРегл,КоррХарактеристика,СчетДтПродукции,СчетКтПродукции,СчетУчетаПродукции,СчетДтПродукцииРегл,СчетКтПродукцииРегл,СчетУчетаПродукцииРегл,Подразделение","Сумма");
		КонецЕсли;
		
		// Автосписание запасов.   - спишем готовую продукцию из основного производственного подразделения
		Для каждого СтрокаПродукцияКПеремещению Из ТаблицыПродукцияКПеремещению Цикл
			
			// Расход.
			СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаПродукцияКПеремещению);
			
			СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
			
			СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.КоррСтруктурнаяЕдиница;
			СтрокаТаблицыРасход.СчетУчета = СтрокаПродукцияКПеремещению.КоррСчетУчета;
			СтрокаТаблицыРасход.Номенклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыРасход.Характеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыРасход.Партия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыРасход.Спецификация = Неопределено;
			СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.СтруктурнаяЕдиницаПродукции;
			СтрокаТаблицыРасход.КоррСчетУчета = СтрокаПродукцияКПеремещению.СчетУчетаПродукции;
			//+++
			СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаПродукцияКПеремещению.КоррСчетУчетаРегл;
			СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаПродукцияКПеремещению.СчетУчетаПродукцииРегл;
			//----
			СтрокаТаблицыРасход.КоррНоменклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыРасход.КоррХарактеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыРасход.КоррПартия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
			СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			//ПодольскийАИ #УчетПроектов 13.10.2020 {
			СтрокаТаблицыРасход.Проект = СтрокаПродукцияКПеремещению.КоррПроект;
			//СтрокаТаблицыРасход.КоррПроект = СтрокаПродукцияКПеремещению.Проект;  //Малышев 2024-06-30 - закоментировал, т.к. закрытие не работало корректно
			//ПодольскийАИ #УчетПроектов 13.10.2020 }
			
			//Малышев 2024-06-24 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
			ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
    		//Малышев 2024-06-24 ONESOFT-3273--)
				
			
			СтрокаТаблицыРасход.Сумма = СтрокаПродукцияКПеремещению.Сумма;
			СтрокаТаблицыРасход.Количество = 0;
			
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				СтрокаТаблицыРасход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
			КонецЕсли;
			Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
			КонецЕсли;
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
			//--------------------------------------------------------------------------
			// © Шишка В.В. [11 октября 2021]  НАЧАЛО
			Если УчетСебестоимостиВРазрезеОрганизации Тогда
				
				Если СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					
					СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
					
				КонецЕсли; 	
				
				Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					
					СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
					
				КонецЕсли; 	
				
			КонецЕсли; 
			// © Шишка В.В. [11 октября 2021]  КОНЕЦ
			//--------------------------------------------------------------------------
			
			// Приход.       - приход продукции на СтруктурнаяЕдиницаПродукции - подразделение выпуска продукции
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаПродукцияКПеремещению);
			
			СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
			
			СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.СтруктурнаяЕдиницаПродукции;
			СтрокаТаблицыПриход.СчетУчета = СтрокаПродукцияКПеремещению.СчетУчетаПродукции;
			СтрокаТаблицыПриход.Номенклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыПриход.Характеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыПриход.Партия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыПриход.Спецификация = Неопределено;
			СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			СтрокаТаблицыПриход.СчетДт = СтрокаПродукцияКПеремещению.СчетДтПродукции;
			СтрокаТаблицыПриход.СчетКт = СтрокаПродукцияКПеремещению.СчетКтПродукции;
			
			СтрокаТаблицыПриход.СчетДтРегл = СтрокаПродукцияКПеремещению.СчетДтПродукцииРегл;
			СтрокаТаблицыПриход.СчетКтРегл = СтрокаПродукцияКПеремещению.СчетКтПродукцииРегл;
			
			СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаПродукцияКПеремещению.КоррСтруктурнаяЕдиница;
			СтрокаТаблицыПриход.КоррСчетУчета = СтрокаПродукцияКПеремещению.КоррСчетУчета;
			//+++
			СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаПродукцияКПеремещению.СчетУчетаПродукцииРегл;
			СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаПродукцияКПеремещению.КоррСчетУчетаРегл;
			//---
			СтрокаТаблицыПриход.КоррНоменклатура = СтрокаПродукцияКПеремещению.КоррНоменклатура;
			СтрокаТаблицыПриход.КоррХарактеристика = СтрокаПродукцияКПеремещению.КоррХарактеристика;
			СтрокаТаблицыПриход.КоррПартия = СтрокаПродукцияКПеремещению.КоррПартия;
			СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
			СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаПродукцияКПеремещению.КоррЗаказПокупателя;
			
			//ПодольскийАИ #УчетПроектов 13.10.2020 {
			СтрокаТаблицыПриход.Проект = СтрокаПродукцияКПеремещению.КоррПроект;
			//СтрокаТаблицыПриход.КоррПроект = СтрокаПродукцияКПеремещению.Проект;    //Малышев 2024-06-24 ONESOFT-3273
			//ПодольскийАИ #УчетПроектов 13.10.2020 }
			
			СтрокаТаблицыПриход.Сумма = СтрокаПродукцияКПеремещению.Сумма;
			СтрокаТаблицыПриход.Количество = 0;
			
			////Малышев 2024-11-19 ONESOFT-3602
			//Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO 
			//	и СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
			//	Тогда
			//	СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
			//КонецЕсли;	
			
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
			КонецЕсли;
			Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				СтрокаТаблицыПриход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
			КонецЕсли;
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
			//Малышев 2024-06-24 ONESOFT-3273 - чистим СчетУчетаЗатратРегл для склада  (--  
			ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
    		//Малышев 2024-06-24 ONESOFT-3273 --)
			
			СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
			СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
			//--------------------------------------------------------------------------
			// © Шишка В.В. [11 октября 2021]  НАЧАЛО
			Если УчетСебестоимостиВРазрезеОрганизации Тогда
				
				Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
					
				КонецЕсли; 	
				
				Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
					
				КонецЕсли; 	
				
			КонецЕсли; 
			// © Шишка В.В. [11 октября 2021]  КОНЕЦ
			//--------------------------------------------------------------------------
			
			
			// Сформируем проводки.
			СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
			
			// Kuzmich, 31.10.2025, (ONESOFT-4474) Документ производство - Двойное количество в бухгалтерских проводках - готовая продукция.
			СтруктураПоискаДляКоэффициентаКоличества = Новый Структура("КоррСтруктурнаяЕдиница, КоррНоменклатура,КоррХарактеристика
			|,КоррСчетУчета,КоррСчетУчетаРегл
			|,КоррЗаказПокупателя,КоррПартия,КоррПроект",
			СтрокаПродукцияКПеремещению.КоррСтруктурнаяЕдиница,СтрокаПродукцияКПеремещению.КоррНоменклатура,СтрокаПродукцияКПеремещению.КоррХарактеристика,
			СтрокаПродукцияКПеремещению.КоррСчетУчета,СтрокаПродукцияКПеремещению.КоррСчетУчетаРегл,
			СтрокаПродукцияКПеремещению.КоррЗаказПокупателя,СтрокаПродукцияКПеремещению.КоррПартия,СтрокаПродукцияКПеремещению.КоррПроект);
			
			ОбщаяСуммаДляКоэффициентаКоличества = ТаблицыПродукцияКПеремещению.Скопировать(СтруктураПоискаДляКоэффициентаКоличества).Итог("Сумма");
			
			Если ОбщаяСуммаДляКоэффициентаКоличества<=0 Тогда
				КоэффициентДляКоличества = 1;
			Иначе
				КоэффициентДляКоличества = СтрокаПродукцияКПеремещению.Сумма / ОбщаяСуммаДляКоэффициентаКоличества;
			КонецЕсли;
			// Kuzmich, 31.10.2025, (ONESOFT-4474) Документ производство - Двойное количество в бухгалтерских проводках - готовая продукция.
			
			ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_СборкаПеремещение5(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы
			,КоэффициентДляКоличества // Kuzmich, 31.10.2025, (ONESOFT-4474) Документ производство - Двойное количество в бухгалтерских проводках - готовая продукция.
			);
			
		КонецЦикла;
		
		
		//Малышев 2024-07-11 ONESOFT-3310
		//сделаем записи по допрасходам, приход допзатрат в продукции
		Если СтруктураДополнительныеСвойства.ДляПроведения.Свойство("СтруктураТаблицПоДопРасходам") Тогда
			//приход допрасходов в производство
			Для каждого СтрДопРасхода из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
				СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
				
				СтрокаТаблицыПриход_ДопРасходы.ВидДвижения = ВидДвиженияНакопления.Приход; 
				СтрокаТаблицыПриход_ДопРасходы.КоррПроект = СтрДопРасхода.Проект;
				СтрокаТаблицыПриход_ДопРасходы.СтруктурнаяЕдиница = СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.СтруктурнаяЕдиницаПродукции; 
				СтрокаТаблицыПриход_ДопРасходы.ФиксированнаяСтоимость = Истина; //2024-07-18
			КонецЦикла;
		КонецЕсли;		
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыЗапасы");
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыПеремещение");
		ТаблицыПродукцияКПеремещению = Неопределено;
		
	КонецПроцедуры // СформироватьТаблицаЗапасыЗапасыСборкаПеремещение()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаПотребностьВЗапасахСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаПотребностьВЗапасах.Организация КАК Организация,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка) КАК ТипДвижения,
		|	ТаблицаПотребностьВЗапасах.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПотребностьВЗапасах.Номенклатура КАК Номенклатура,
		|	ТаблицаПотребностьВЗапасах.Характеристика КАК Характеристика
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаПотребностьВЗапасах";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.ПотребностьВЗапасах");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
		КонецЦикла;
		Блокировка.Заблокировать();
		
		// Получение остатков
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ПотребностьВЗапасахОстатки.Организация КАК Организация,
		|	ПотребностьВЗапасахОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ПотребностьВЗапасахОстатки.Номенклатура КАК Номенклатура,
		|	ПотребностьВЗапасахОстатки.Характеристика КАК Характеристика,
		|	СУММА(ПотребностьВЗапасахОстатки.Количество) КАК КоличествоОстаток
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПотребностьВЗапасахОстатки.Организация КАК Организация,
		|		ПотребностьВЗапасахОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|		ПотребностьВЗапасахОстатки.Номенклатура КАК Номенклатура,
		|		ПотребностьВЗапасахОстатки.Характеристика КАК Характеристика,
		|		СУММА(ПотребностьВЗапасахОстатки.КоличествоОстаток) КАК Количество
		|	ИЗ
		|		РегистрНакопления.ПотребностьВЗапасах.Остатки(
		|				&МоментКонтроля,
		|				(Организация, ТипДвижения, ЗаказПокупателя, Номенклатура, Характеристика) В
		|					(ВЫБРАТЬ
		|						ВременнаяТаблицаЗапасы.Организация КАК Организация,
		|						ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка) КАК ТипДвижения,
		|						ВременнаяТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|						ВременнаяТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|						ВременнаяТаблицаЗапасы.Характеристика КАК Характеристика
		|					ИЗ
		|						ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы)) КАК ПотребностьВЗапасахОстатки
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ПотребностьВЗапасахОстатки.Организация,
		|		ПотребностьВЗапасахОстатки.ЗаказПокупателя,
		|		ПотребностьВЗапасахОстатки.Номенклатура,
		|		ПотребностьВЗапасахОстатки.Характеристика
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ДвиженияДокументаПотребностьВЗапасах.Организация,
		|		ДвиженияДокументаПотребностьВЗапасах.ЗаказПокупателя,
		|		ДвиженияДокументаПотребностьВЗапасах.Номенклатура,
		|		ДвиженияДокументаПотребностьВЗапасах.Характеристика,
		|		ВЫБОР
		|			КОГДА ДвиженияДокументаПотребностьВЗапасах.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|				ТОГДА ЕСТЬNULL(ДвиженияДокументаПотребностьВЗапасах.Количество, 0)
		|			ИНАЧЕ -ЕСТЬNULL(ДвиженияДокументаПотребностьВЗапасах.Количество, 0)
		|		КОНЕЦ
		|	ИЗ
		|		РегистрНакопления.ПотребностьВЗапасах КАК ДвиженияДокументаПотребностьВЗапасах
		|	ГДЕ
		|		ДвиженияДокументаПотребностьВЗапасах.Регистратор = &Ссылка
		|		И ДвиженияДокументаПотребностьВЗапасах.Период <= &ПериодКонтроля) КАК ПотребностьВЗапасахОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	ПотребностьВЗапасахОстатки.Организация,
		|	ПотребностьВЗапасахОстатки.ЗаказПокупателя,
		|	ПотребностьВЗапасахОстатки.Номенклатура,
		|	ПотребностьВЗапасахОстатки.Характеристика";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		
		Если ЗначениеЗаполнено(ДокументСсылкаСборкаЗапасов.ЗаказПокупателя) Тогда
			Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Иначе
			Запрос.УстановитьПараметр("МоментКонтроля", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
		КонецЕсли;
		
		Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.ПериодКонтроля);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		ТаблицаПотребностьВЗапасахОстатки = РезультатЗапроса.Выгрузить();
		ТаблицаПотребностьВЗапасахОстатки.Индексы.Добавить("Организация,ЗаказПокупателя,Номенклатура,Характеристика");
		
		ВременнаяТаблицаПотребностьВЗапасах = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПотребностьВЗапасах.СкопироватьКолонки();
		
		Для каждого СтрокаТаблицыПотребностьВЗапасах Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПотребностьВЗапасах Цикл
			
			СтруктураДляПоиска = Новый Структура;
			СтруктураДляПоиска.Вставить("Организация", 		СтрокаТаблицыПотребностьВЗапасах.Организация);
			СтруктураДляПоиска.Вставить("ЗаказПокупателя", 	СтрокаТаблицыПотребностьВЗапасах.ЗаказПокупателя);
			СтруктураДляПоиска.Вставить("Номенклатура", 	СтрокаТаблицыПотребностьВЗапасах.Номенклатура);
			СтруктураДляПоиска.Вставить("Характеристика", 	СтрокаТаблицыПотребностьВЗапасах.Характеристика);
			
			МассивСтрокОстатков = ТаблицаПотребностьВЗапасахОстатки.НайтиСтроки(СтруктураДляПоиска);
			Если МассивСтрокОстатков.Количество() > 0 И МассивСтрокОстатков[0].КоличествоОстаток > 0 Тогда
				
				Если СтрокаТаблицыПотребностьВЗапасах.Количество > МассивСтрокОстатков[0].КоличествоОстаток Тогда
					СтрокаТаблицыПотребностьВЗапасах.Количество = МассивСтрокОстатков[0].КоличествоОстаток;
				КонецЕсли;
				
				СтрокаТаблицыРасход = ВременнаяТаблицаПотребностьВЗапасах.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицыПотребностьВЗапасах);
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПотребностьВЗапасах = ВременнаяТаблицаПотребностьВЗапасах;
		
	КонецПроцедуры // СформироватьТаблицаПотребностьВЗапасахСборка()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицуРаспределенияМатериаловСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, ТаблицаПродукция) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаПродукция.КоррНомерСтроки КАК КоррНомерСтроки,
		|	ТаблицаПродукция.КоррНоменклатура КАК КоррНоменклатура,
		|	ТаблицаПродукция.КоррХарактеристика КАК КоррХарактеристика,
		|	ТаблицаПродукция.КоррПартия КАК КоррПартия,
		|	ТаблицаПродукция.КоррСпецификация КАК КоррСпецификация,
		
		//Малышев 2024-06-17 вернул обратно
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.КоррПроект КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_
		
		|	ТаблицаПродукция.КоррСчетУчета КАК КоррСчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции КАК СчетУчетаПродукции,
		|	ТаблицаПродукция.СчетДт КАК СчетДт,
		|	ТаблицаПродукция.СчетДтПродукции КАК СчетДтПродукции,
		|	ТаблицаПродукция.СчетКтПродукции КАК СчетКтПродукции,
		//+++
		|	ТаблицаПродукция.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		|	ТаблицаПродукция.СчетДтРегл КАК СчетДтРегл,
		|	ТаблицаПродукция.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	ТаблицаПродукция.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		//---
		|	ТаблицаПродукция.КоррКоличество КАК КоррКоличество
		|ПОМЕСТИТЬ ВременнаяТаблицаТЗ
		|ИЗ
		|	&ТаблицаПродукция КАК ТаблицаПродукция
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаСоставПродукции.КоррНомерСтроки КАК КоррНомерСтроки,
		|	ТаблицаСоставПродукции.КоррНоменклатура КАК КоррНоменклатура,
		|	ТаблицаСоставПродукции.КоррХарактеристика КАК КоррХарактеристика,
		|	ТаблицаСоставПродукции.КоррПартия КАК КоррПартия,
		|	ТаблицаСоставПродукции.КоррСпецификация КАК КоррСпецификация,
		
		//Малышев 2024-06-17 вернул обратно
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаСоставПродукции.КоррПроект КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		//_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаСоставПродукции.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаСоставПродукции.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_  
		
		|	ТаблицаСоставПродукции.КоррСчетУчета КАК КоррСчетУчета,
		|	ТаблицаСоставПродукции.СчетУчетаПродукции КАК СчетУчетаПродукции,
		|	ТаблицаСоставПродукции.СчетДт КАК СчетДт,
		|	ТаблицаСоставПродукции.СчетДтПродукции КАК СчетДтПродукции,
		|	ТаблицаСоставПродукции.СчетКтПродукции КАК СчетКтПродукции,
		//+++
		|	ТаблицаСоставПродукции.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ТаблицаСоставПродукции.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаСоставПродукции.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		|	ТаблицаСоставПродукции.СчетДтРегл КАК СчетДтРегл,
		|	ТаблицаСоставПродукции.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	ТаблицаСоставПродукции.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		//---
		|	ТаблицаСоставПродукции.КоррКоличество КАК КоррКоличество,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаМатериалы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА ВЫБОР
		|					КОГДА ТаблицаМатериалы.Количество = 0
		|						ТОГДА 1
		|					ИНАЧЕ ТаблицаМатериалы.Количество
		|				КОНЕЦ / ТаблицаМатериалы.КоличествоПродукции * ТаблицаСоставПродукции.КоррКоличество
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ТаблицаМатериалы.Количество = 0
		|					ТОГДА 1
		|				ИНАЧЕ ТаблицаМатериалы.Количество
		|			КОНЕЦ * ТаблицаМатериалы.ЕдиницаИзмерения.Коэффициент / ТаблицаМатериалы.КоличествоПродукции * ТаблицаСоставПродукции.КоррКоличество
		|	КОНЕЦ КАК ТМКоличество,
		|	ТаблицаМатериалы.ТипСтрокиСостава КАК ТМТипСтрокиСостава,
		|	ТаблицаМатериалы.Номенклатура КАК ТМНоменклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА ТаблицаМатериалы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК ТМХарактеристика,
		|	ТаблицаМатериалы.Спецификация КАК ТМСпецификация,
		|	ЛОЖЬ КАК Распределено
		|ИЗ
		|	ВременнаяТаблицаТЗ КАК ТаблицаСоставПродукции
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Спецификации.Состав КАК ТаблицаМатериалы
		|		ПО ТаблицаСоставПродукции.КоррСпецификация = ТаблицаМатериалы.Ссылка
		|			И (ТаблицаМатериалы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас))
		|
		|УПОРЯДОЧИТЬ ПО
		|	КоррНомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовЗапасы.КлючСвязи КАК КлючСвязи,
		|	СборкаЗапасовЗапасы.Ссылка КАК Ссылка,
		|	СборкаЗапасовЗапасы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовЗапасы.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК Ячейка,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов
		|	КОНЕЦ КАК СтруктурнаяЕдиницаЗапасов,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиницаЗапасовНаСклад,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов.ОрдерныйСклад КАК ОрдерныйСкладЗапасов,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции
		|	КОНЕЦ КАК СтруктурнаяЕдиницаПродукции,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовЗапасы.Ссылка.ЯчейкаЗапасов
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК ЯчейкаЗапасов,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаЗапасов,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК КоррСчетУчета,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетУчетаПродукции,
		//+++
		
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаРегл,
		
		//Малышев 2024-06-24 ONESOFT-3273 (--
		//|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ВЫБОР 
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|          ТОГДА Значение(ПланСчетов.Хозрасчетный.ПустаяСсылка)
		|	  	ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл 
		|	КОНЕЦ КАК СчетУчетаЗатратРегл,	  
		//Малышев 2024-06-24 ONESOFT-3273 --)
		
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаЗапасовРегл,
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК КоррСчетУчетаРегл,
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетУчетаПродукцииРегл,
		//---
		|	СборкаЗапасовЗапасы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовЗапасы.СтранаПроисхождения КАК СтранаПроисхождения,
		|	СборкаЗапасовЗапасы.НомерГТД КАК НомерГТД,
		|	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК КоррНоменклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовЗапасы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК КоррХарактеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовЗапасы.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Партия,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовЗапасы.Партия.Статус
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ПустаяСсылка)
		|	КОНЕЦ КАК ПартияСтатус,
		|	ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка) КАК КоррПартия,
		|	ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка) КАК КоррСпецификация,
		|	СборкаЗапасовЗапасы.Спецификация КАК Спецификация,
		
		//Малышев 2024-06-17 - ONESOFT-3094 (-- 
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		//|	ВЫБОР
		//|		КОГДА &УчетПоПроектам
		//|			ТОГДА СборкаЗапасовЗапасы.Проект
		//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА &УчетПоПроектам
		//|			ТОГДА СборкаЗапасовЗапасы.Проект
		//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|	КОНЕЦ КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		|	ВЫБОР 
		|		КОГДА &УчетПоПроектам = Ложь тогда ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка) 
		|		КОГДА СборкаЗапасовЗапасы.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовЗапасы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР 
		|		КОГДА &УчетПоПроектам = Ложь тогда ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка) 
		|		КОГДА СборкаЗапасовЗапасы.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовЗапасы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		|	КОНЕЦ КАК КоррПроект,
		//Малышев 2024-06-17 - ONESOFT-3094 --) 
		
		
		|	СборкаЗапасовЗапасы.Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовЗапасы.Количество
		|		ИНАЧЕ СборкаЗапасовЗапасы.Количество * СборкаЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовЗапасы.Резерв
		|		ИНАЧЕ СборкаЗапасовЗапасы.Резерв * СборкаЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Резерв,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетДт,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетДтПродукции,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКт,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетКтПродукции,
		//+++
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДтРегл,
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДтПродукцииРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКтРегл,
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКтПродукцииРегл,
		//---
		|	ЛОЖЬ КАК Распределено,
		|	СборкаЗапасовЗапасы.Номенклатура.СрокГодности КАК СрокГодности,
		|	СборкаЗапасовЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Ссылка = &Ссылка
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
		
		Запрос.УстановитьПараметр("ТаблицаПродукция", ТаблицаПродукция);
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);
		
		//ПодольскийАИ #УчетПроектов 05.10.2020 {
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
		//ПодольскийАИ #УчетПроектов 05.10.2020 }
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		ТаблицаСоставПродукции = МассивРезультатов[1].Выгрузить();
		ТаблицаМатериалов = МассивРезультатов[2].Выгрузить();
		
		КоличествоПродукции = ТаблицаСоставПродукции.Количество();
		Инд = 0;
		Пока Инд < КоличествоПродукции Цикл
			СтрокаПродукции = ТаблицаСоставПродукции[Инд];
			Если СтрокаПродукции.ТМТипСтрокиСостава = Перечисления.ТипыСтрокСоставаСпецификации.Узел Тогда
				СтекСпецификацийУзлов = Новый Массив();
				ЗаполнитьТаблицуПродукцииСоставомУзлов(СтрокаПродукции, ТаблицаСоставПродукции, СтекСпецификацийУзлов);
				ТаблицаСоставПродукции.Удалить(СтрокаПродукции);
				Если Инд + 1 = ТаблицаСоставПродукции.Количество() Тогда
					Прервать;
				КонецЕсли;
			Иначе
				Инд = Инд + 1;
			КонецЕсли;
		КонецЦикла;


		//ТаблицаСоставПродукции.Свернуть("КоррНоменклатура,КоррХарактеристика,КоррПартия,КоррСпецификация,КоррСчетУчета,СчетУчетаПродукции,СчетДт,СчетДтПродукции,СчетКтПродукции,КоррКоличество,ТМНоменклатура,ТМХарактеристика,Распределено", "ТМКоличество");
		//ПодольскийАИ #УчетПроектов добавлена колонка КоррПроект
		ТаблицаСоставПродукции.Свернуть("КоррНоменклатура,КоррХарактеристика,КоррПартия,КоррСпецификация,КоррПроект,КоррСчетУчета,СчетУчетаПродукции,СчетДт,СчетДтПродукции,СчетКтПродукции,КоррСчетУчетаРегл,СчетУчетаЗатратРегл,СчетУчетаПродукцииРегл,СчетДтРегл,СчетДтПродукцииРегл,СчетКтПродукцииРегл,КоррКоличество,ТМНоменклатура,ТМХарактеристика,Распределено", "ТМКоличество");


		ТаблицаСоставПродукции.Индексы.Добавить("ТМНоменклатура,ТМХарактеристика");
		
		РаспределеноМатериалов = 0;
		КоличествоПродукции = ТаблицаСоставПродукции.Количество();
		КоличествоМатериалов = ТаблицаМатериалов.Количество();
		Для н = 0 По КоличествоМатериалов - 1 Цикл
			
			СтрокаМатериалы = ТаблицаМатериалов[н];
			
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("ТМНоменклатура", СтрокаМатериалы.Номенклатура);
			СтруктураПоиска.Вставить("ТМХарактеристика", СтрокаМатериалы.Характеристика);
			
			РезультатПоиска = ТаблицаСоставПродукции.НайтиСтроки(СтруктураПоиска);
			Если РезультатПоиска.Количество() <> 0 Тогда
				РаспределитьМатериалыПоНормативам(СтрокаМатериалы, РезультатПоиска, ТаблицаМатериалов);
				РаспределеноМатериалов = РаспределеноМатериалов + 1;
			КонецЕсли;
			
		КонецЦикла;
		
		РаспределеноПродукции = 0;
		Для каждого СтрокаСоставПродукции Из ТаблицаСоставПродукции Цикл
			Если СтрокаСоставПродукции.Распределено Тогда
				РаспределеноПродукции = РаспределеноПродукции + 1;
			КонецЕсли;
		КонецЦикла;
		
		Если РаспределеноМатериалов < КоличествоМатериалов Тогда
			Если РаспределеноПродукции = КоличествоПродукции Тогда
				БазаРаспределения = ТаблицаПродукция.Итог("КоррКоличество");
				РаспределитьМатериалыПоКоличеству(ТаблицаПродукция, ТаблицаМатериалов, БазаРаспределения);
			Иначе
				РаспределитьМатериалыПоКоличеству(ТаблицаСоставПродукции, ТаблицаМатериалов);
			КонецЕсли;
		КонецЕсли;
		
		ТаблицаПродукция = Неопределено;
		ТаблицаСоставПродукции = Неопределено;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРаспределенияМатериаловСборка", ТаблицаМатериалов);
		ТаблицаМатериалов = Неопределено;
		
	КонецПроцедуры // СформироватьТаблицуРаспределенияМатериаловСборка()
	
	// Инициализирует таблицы значений, содержащие данные табличных частей документа.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура ИнициализироватьДанныеПоЗапасамСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовЗапасы.Период КАК Период,
		|	&Организация КАК Организация,
		|	ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический) КАК СценарийПланирования,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовЗапасы.ОрдерныйСклад КАК ОрдерныйСклад,
		|	СборкаЗапасовЗапасы.Ячейка КАК Ячейка,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиницаЗапасов,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад КАК СтруктурнаяЕдиницаЗапасовНаСклад,
		|	СборкаЗапасовЗапасы.ОрдерныйСкладЗапасов КАК ОрдерныйСкладЗапасов,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукции,
		|	СборкаЗапасовЗапасы.ЯчейкаЗапасов КАК ЯчейкаЗапасов,
		|	СборкаЗапасовЗапасы.СчетУчета КАК СчетУчета,
		|	СборкаЗапасовЗапасы.СчетУчетаЗапасов КАК СчетУчетаЗапасов,
		|	СборкаЗапасовЗапасы.КоррСчетУчета КАК КоррСчетУчета,
		|	СборкаЗапасовЗапасы.СчетУчетаПродукции КАК СчетУчетаПродукции,
		//+++
		|	СборкаЗапасовЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|	СборкаЗапасовЗапасы.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	СборкаЗапасовЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,
		|	СборкаЗапасовЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	СборкаЗапасовЗапасы.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		//---
		|	СборкаЗапасовЗапасы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовЗапасы.СтранаПроисхождения КАК СтранаПроисхождения,
		|	СборкаЗапасовЗапасы.НомерГТД КАК НомерГТД,
		|	СборкаЗапасовЗапасы.КоррНоменклатура КАК КоррНоменклатура,
		|	СборкаЗапасовЗапасы.Характеристика КАК Характеристика,
		|	СборкаЗапасовЗапасы.КоррХарактеристика КАК КоррХарактеристика,
		|	СборкаЗапасовЗапасы.Партия КАК Партия,
		|	СборкаЗапасовЗапасы.ПартияСтатус КАК ПартияСтатус,
		|	СборкаЗапасовЗапасы.КоррПартия КАК КоррПартия,
		|	СборкаЗапасовЗапасы.Спецификация КАК Спецификация,
		|	СборкаЗапасовЗапасы.КоррСпецификация КАК КоррСпецификация,
		
		//Малышев 2024-06-17 - ONESOFT-3094 - вернул обратно
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	СборкаЗапасовЗапасы.Проект КАК Проект,
		|	СборкаЗапасовЗапасы.КоррПроект КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		//|		КОГДА СборкаЗапасовЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ СборкаЗапасовЗапасы.Проект 
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		//|		КОГДА СборкаЗапасовЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ СборкаЗапасовЗапасы.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_05.02.2021_ 
		
		|	СборкаЗапасовЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	СборкаЗапасовЗапасы.Количество КАК Количество,
		|	СборкаЗапасовЗапасы.Резерв КАК Резерв,
		|	0 КАК Сумма,
		|	СборкаЗапасовЗапасы.СчетДт КАК СчетДт,
		|	СборкаЗапасовЗапасы.СчетДтПродукции КАК СчетДтПродукции,
		|	СборкаЗапасовЗапасы.СчетКт КАК СчетКт,
		|	СборкаЗапасовЗапасы.СчетКтПродукции КАК СчетКтПродукции,
		//+++
		|	СборкаЗапасовЗапасы.СчетДтРегл КАК СчетДтРегл,
		|	СборкаЗапасовЗапасы.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	СборкаЗапасовЗапасы.СчетКтРегл КАК СчетКтРегл,
		|	СборкаЗапасовЗапасы.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиница КАК Подразделение,
		//---
		|	ВЫРАЗИТЬ(&РаспределениеЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки,
		|	ВЫРАЗИТЬ(&РаспределениеЗапасов КАК СТРОКА(100)) КАК Содержание,
		|	&ДатаОбновленияНаРелиз_1_2_1 КАК ДатаОбновленияНаРелиз_1_2_1,
		//+Lazare
		|	СборкаЗапасовЗапасы.СрокГодности КАК СрокГодности,
		|	СборкаЗапасовЗапасы.ВключенВрегистр КАК ВключенВрегистр
		//-Lazare
		|ПОМЕСТИТЬ ВременнаяТаблицаЗапасы
		|ИЗ
		|	&ТаблицаРаспределенияМатериаловСборка КАК СборкаЗапасовЗапасы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СценарийПланирования КАК СценарийПланирования,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукции,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|	ТаблицаЗапасы.КоррСчетУчета КАК КоррСчетУчета,
		|	ТаблицаЗапасы.СчетУчетаПродукции КАК СчетУчетаПродукции,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		//---
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.КоррНоменклатура КАК КоррНоменклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.КоррХарактеристика КАК КоррХарактеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.КоррПартия КАК КоррПартия,
		|	ТаблицаЗапасы.Спецификация КАК Спецификация,
		|	ТаблицаЗапасы.КоррСпецификация КАК КоррСпецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ТаблицаЗапасы.КоррПроект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект 
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_05.02.2021_
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК КоррЗаказПокупателя,
		|	ТаблицаЗапасы.СчетДт КАК СчетДт,
		|	ТаблицаЗапасы.СчетКт КАК СчетКт,
		|	ТаблицаЗапасы.СчетДтПродукции КАК СчетДтПродукции,
		|	ТаблицаЗапасы.СчетКтПродукции КАК СчетКтПродукции,
		//+++
		|	ТаблицаЗапасы.СчетДтРегл КАК СчетДтРегл,
		|	ТаблицаЗапасы.СчетКтРегл КАК СчетКтРегл,
		|	ТаблицаЗапасы.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	ТаблицаЗапасы.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		|	ТаблицаЗапасы.Подразделение КАК Подразделение,
		//---
		|	ТаблицаЗапасы.СодержаниеПроводки КАК Содержание,
		|	ТаблицаЗапасы.СодержаниеПроводки КАК СодержаниеПроводки,
		|	ЛОЖЬ КАК ЗатратыНаВыпуск,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество,
		|	СУММА(ТаблицаЗапасы.Резерв) КАК Резерв,
		|	0 КАК Сумма
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СценарийПланирования,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаПродукции,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.КоррСчетУчета,
		|	ТаблицаЗапасы.СчетУчетаПродукции,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗатратРегл,
		|	ТаблицаЗапасы.КоррСчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаПродукцииРегл,
		|	ТаблицаЗапасы.Подразделение,
		//---
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.КоррНоменклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.КоррХарактеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.КоррПартия,
		|	ТаблицаЗапасы.Спецификация,
		|	ТаблицаЗапасы.КоррСпецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект 
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_05.02.2021_
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.СчетДт,
		|	ТаблицаЗапасы.СчетКт,
		|	ТаблицаЗапасы.СчетДтПродукции,
		|	ТаблицаЗапасы.СчетКтПродукции,
		//+++
		|	ТаблицаЗапасы.СчетДтРегл,
		|	ТаблицаЗапасы.СчетКтРегл,
		|	ТаблицаЗапасы.СчетДтПродукцииРегл,
		|	ТаблицаЗапасы.СчетКтПродукцииРегл,
		//---
		|	ТаблицаЗапасы.СодержаниеПроводки,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.СодержаниеПроводки
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад КАК СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.ЯчейкаЗапасов КАК ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов КАК ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Ячейка КАК Ячейка,
		
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект КАК Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект 
		//|	КОНЕЦ КАК Проект,
		////Lazare_05.02.2021_ 
		
		|	ТаблицаЗапасы.Подразделение КАК Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.Период < ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект 
		//|	КОНЕЦ,
		////Lazare_05.02.2021_ 
		
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Ячейка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки),
		|	ТаблицаЗапасы.Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Ячейка,
		
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект 
		//|	КОНЕЦ,
		////Lazare_05.02.2021_
		
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество)
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	НЕ ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.Период >= ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Ячейка,
		
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare _05.02.2021
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект 
		//|	КОНЕЦ
		////Lazare_05.02.2021_
		
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ВЫБОР
		|		КОГДА ТаблицаЗапасы.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ТаблицаЗапасы.ЗаказПокупателя
		|	КОНЕЦ КАК Заказ,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыПриемаПередачиТоваров.ОтчетПоПереработке) КАК ТипПриемаПередачи,
		|	ТаблицаЗапасы.Подразделение КАК Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ПартияСтатус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ВЫБОР
		|		КОГДА ТаблицаЗапасы.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ТаблицаЗапасы.ЗаказПокупателя
		|	КОНЕЦ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка) КАК ТипДвижения,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Подразделение КАК Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Характеристика
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Подразделение КАК Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов
		|	И ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад <> ТаблицаЗапасы.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Партия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки),
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество)
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.Период >= ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Партия
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Подразделение КАК Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад <> ТаблицаЗапасы.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Партия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки),
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Подразделение КАК Подразделение, //+++ Истомин
		|	СУММА(ТаблицаЗапасы.Количество)
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.Период < ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|	И ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад <> ТаблицаЗапасы.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Подразделение, //+++ Истомин
		|	ТаблицаЗапасы.Партия";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);
		Запрос.УстановитьПараметр("ТаблицаРаспределенияМатериаловСборка", СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРаспределенияМатериаловСборка);
		
		// Временно: изменение движений по ордерному складу.
		ДатаОбновленияНаРелиз_1_2_1 = Константы.ДатаОбновленияНаРелиз_1_2_1.Получить();
		Запрос.УстановитьПараметр("ДатаОбновленияНаРелиз_1_2_1", ДатаОбновленияНаРелиз_1_2_1);
		
		Запрос.УстановитьПараметр("РаспределениеЗапасов", НСтр("ka='მარაგების გადანაწილება';ru='Распределение запасов';en='distribution of reserves'"));
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
				
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		// Определим таблицу для учета запасов.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыЗапасы", МассивРезультатов[1].Выгрузить());
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Колонки.Добавить("ДокументПоступления");
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		
		// Сформируем таблицу для учета запасов.
		Если ЗначениеЗаполнено(ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаЗапасов) 
			И ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаЗапасов <> ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиница Тогда
			
			// Автоперемещение запасов.
			СформироватьТаблицаЗапасыЗапасыСборкаПеремещение(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
			
		Иначе
			
			СформироватьТаблицаЗапасыЗапасыСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
			
		КонецЕсли;
		
		СформироватьТаблицаЗапасыВРазрезеГТДСборка(СтруктураДополнительныеСвойства);
		
		// Дополним таблицу для запасов.
		ВыборкаРезультатов = МассивРезультатов[2].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			// Автоперемещение запасов.
			Если (ВыборкаРезультатов.СтруктурнаяЕдиницаЗапасов = ВыборкаРезультатов.СтруктурнаяЕдиница
				И ВыборкаРезультатов.ЯчейкаЗапасов <> ВыборкаРезультатов.Ячейка)
				ИЛИ ВыборкаРезультатов.СтруктурнаяЕдиницаЗапасов <> ВыборкаРезультатов.СтруктурнаяЕдиница Тогда
				
				// Расход.
				Если (ВыборкаРезультатов.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (ВыборкаРезультатов.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ ВыборкаРезультатов.ОрдерныйСкладЗапасов) Тогда
					
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, ВыборкаРезультатов);
					
					СтрокаТаблицыРасход.СтруктурнаяЕдиница = ВыборкаРезультатов.СтруктурнаяЕдиницаЗапасов;
					СтрокаТаблицыРасход.Ячейка = ВыборкаРезультатов.ЯчейкаЗапасов;
					
				КонецЕсли;
				
				// Приход.
				Если (ВыборкаРезультатов.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (ВыборкаРезультатов.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ ВыборкаРезультатов.ОрдерныйСклад) Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, ВыборкаРезультатов);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
				КонецЕсли;
				
			КонецЕсли;
			
			СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, ВыборкаРезультатов);
			
		КонецЦикла;
		
		// Определим таблицу израсходованного давальческого сырья, принятого в переработку, за которые будет необходимо отчитаться перед давальцем.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПринятые", МассивРезультатов[3].Выгрузить());
		
		// Определим таблицу для движения по потребности позиций зависимого спроса.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПотребностьВЗапасах", МассивРезультатов[4].Выгрузить());
		СформироватьТаблицаПотребностьВЗапасахСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Автоперемещение запасов (дополним таблицу ТаблицаЗапасыКРасходуСоСкладов).
		ВыборкаРезультатов = МассивРезультатов[5].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасходПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыКРасходуСоСкладов.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПриход, ВыборкаРезультатов);
			
		КонецЦикла;
		
		// Автоперемещение запасов (дополним таблицу ТаблицаЗапасыКПоступлениюНаСклады).
		ВыборкаРезультатов = МассивРезультатов[6].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасходПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыКПоступлениюНаСклады.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПриход, ВыборкаРезультатов);
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаРаспределенияМатериаловСборка");
		
	КонецПроцедуры // ИнициализироватьДанныеПоЗапасамСборка()
	
	////////////////////////////////////////////////////////////////////////////////
	// ПРОДУКЦИЯ (СБОРКА)
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыПродукцияСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки)
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРазмещениеЗаказов.Индексы.Добавить("ВидДвижения,Организация,Номенклатура,Характеристика");;
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Индексы.Добавить("ВидДвижения,Организация,Номенклатура,Характеристика,Партия,КоррНоменклатура,КоррХарактеристика,КоррПартия,ЗатратыНаВыпуск");;
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасыПродукция = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция[н];
			
			//Малышев 2024-07-12 ONESOFT-3273 - закоментировал
			// Сформируем выпуск продукции в количественном выражении. Если заказ покупателя указан - под заказ
			// покупателя, если нет - то под пустой заказ.
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);
			
			//Малышев 2024-06-24 ONESOFT-3273(--
			Если СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда
				СтрокаТаблицыПриход.СчетУчетаЗатратРегл = СтрокаТаблицыПриход.Номенклатура.СчетУчетаЗатратРегл;
			КонецЕсли;	
			//ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);		
			//Если не ЗначениеЗаполнено(СтрокаТаблицыПриход.КоррСчетУчетаРегл) Тогда
			//	 СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл;  //Малышев 2024-06-08       
			//	 СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.Подразделение;
			//	 СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицыПриход.Номенклатура;
			//КонецЕсли;	
    		//Малышев 2024-06-24 ONESOFT-3273--)
			
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
			КонецЕсли;
			СтрокаТаблицыПриход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
			// Автоперемещение продукции.
			ПеремещениеСчетУчета = Неопределено;
			ПеремещениеСчетУчетаРегл = Неопределено; //+++
			Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
				
				// Расход.
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыПродукция);
				
				СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
				СтрокаТаблицыРасход.Спецификация = Неопределено;
				
				СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
				СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукции;
				
				//+++
				СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл;
				//СтрокаТаблицыРасход.СчетУчетаЗатратРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл; //Малышев 2024-06-24 ONESOFT-3273
				//---
				
				СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
				СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
				СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
				СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
				СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя;
				
				//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
				//ПодольскийАИ #УчетПроектов 13.10.2020 {
				СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасыПродукция.КоррПроект;
				СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасыПродукция.Проект;
				//СтрокаТаблицыРасход.Проект = ?(СтрокаТаблицаЗапасыПродукция.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПродукция.КоррПроект);//+++Lazare_05.02.2021
				//СтрокаТаблицыРасход.КоррПроект =?(СтрокаТаблицаЗапасыПродукция.Проект  = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПродукция.Проект);//+++Lazare_05.02.2021
				//ПодольскийАИ #УчетПроектов 13.10.2020 }
				
				//Малышев 2024-06-24 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыПродукция);		
    			//Малышев 2024-06-24 ONESOFT-3273--)
					
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				СтрокаТаблицыРасход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				
				// Приход.
				СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);
				
				СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
				СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукции;
				СтрокаТаблицыПриход.Спецификация = Неопределено;
				
				ПеремещениеСчетУчета = СтрокаТаблицыПриход.СчетУчета;
				ПеремещениеСчетУчетаРегл = СтрокаТаблицыПриход.СчетУчета;
				
				СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
				СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
				
				//+++
				СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл;
				СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
				//---
				
				СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
				СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
				СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
				СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
				СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя;
				
				//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
				//ПодольскийАИ #УчетПроектов 13.10.2020 {
				СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасыПродукция.КоррПроект;
				СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасыПродукция.Проект;
				//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасыПродукция.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПродукция.КоррПроект);//+++Lazare_05.02.2021
				//СтрокаТаблицыПриход.КоррПроект =?(СтрокаТаблицаЗапасыПродукция.Проект  = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПродукция.Проект);//+++Lazare_05.02.2021
				//ПодольскийАИ #УчетПроектов 13.10.2020 }
				
				//Малышев 2024-06-24 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);		
    			//Малышев 2024-06-24 ONESOFT-3273--)
				
				////Малышев 2024-11-19 ONESOFT-3602
				//Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO 
				//	и СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
				//	Тогда
				//	СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыПриход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				
			КонецЕсли;
			
			// Если заказ на производство заполнен, а заказ покупателя нет, то необходимо проверить,
			// есть ли размещенные заказы покупателей в заказе на производство.
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя)
				И ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.ЗаказНаПроизводство) Тогда
				
				СтруктураДляПоиска = Новый Структура;
				СтруктураДляПоиска.Вставить("ВидДвижения", ВидДвиженияНакопления.Расход);
				СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасыПродукция.Организация);
				СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасыПродукция.Номенклатура);
				СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасыПродукция.Характеристика);
				
				Индекс = 0;
				КоличествоВыхода = СтрокаТаблицаЗапасыПродукция.Количество;
				МассивРазмещенияПродукции = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРазмещениеЗаказов.НайтиСтроки(СтруктураДляПоиска);
				
				Если МассивРазмещенияПродукции.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				СтруктураДляПоиска = Новый Структура;
				СтруктураДляПоиска.Вставить("ВидДвижения", ВидДвиженияНакопления.Приход);
				СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасыПродукция.Организация);
				СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасыПродукция.Номенклатура);
				СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасыПродукция.Характеристика);
				СтруктураДляПоиска.Вставить("Партия", СтрокаТаблицаЗапасыПродукция.Партия);
				СтруктураДляПоиска.Вставить("ЗатратыНаВыпуск", Ложь);
				
				Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
					СтруктураДляПоиска.Вставить("КоррНоменклатура", СтрокаТаблицаЗапасыПродукция.Номенклатура);
					СтруктураДляПоиска.Вставить("КоррХарактеристика", СтрокаТаблицаЗапасыПродукция.Характеристика);
					СтруктураДляПоиска.Вставить("КоррПартия", СтрокаТаблицаЗапасыПродукция.Партия);
				КонецЕсли;
				
				СтоимостьВыхода = 0;
				МассивСтоимостиВыхода = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.НайтиСтроки(СтруктураДляПоиска);
				Для каждого СтрокаВыхода Из МассивСтоимостиВыхода Цикл
					СтоимостьВыхода = СтоимостьВыхода + СтрокаВыхода.Сумма;
				КонецЦикла;
				
				Для каждого СтрокаМассивРазмещения Из МассивРазмещенияПродукции Цикл
					
					КоличествоВыходаВРезерв = СтрокаМассивРазмещения.Количество;
					
					Если КоличествоВыхода = КоличествоВыходаВРезерв Тогда
						СтоимостьВыходаВРезерв = СтоимостьВыхода;
					Иначе
						СтоимостьВыходаВРезерв = Окр(СтоимостьВыхода * КоличествоВыходаВРезерв / КоличествоВыхода, 2, 1);
					КонецЕсли;
					
					Если КоличествоВыходаВРезерв > 0 Тогда
						
						ВсегоСуммаКСписаниюПоЗаказу = 0;
						
						СуммаКСписаниюПоЗаказу = Окр(СтоимостьВыходаВРезерв * СтрокаМассивРазмещения.Количество / КоличествоВыходаВРезерв, 2, 1);
						ВсегоСуммаКСписаниюПоЗаказу = ВсегоСуммаКСписаниюПоЗаказу + СуммаКСписаниюПоЗаказу;
						
						Если Индекс = МассивРазмещенияПродукции.Количество() - 1 Тогда // Это последняя строка, необходимо скорректировать сумму.
							СуммаКСписаниюПоЗаказу = СуммаКСписаниюПоЗаказу + (СтоимостьВыходаВРезерв - ВсегоСуммаКСписаниюПоЗаказу);
						КонецЕсли;
						
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыПродукция);
						
						СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
						
						Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
							СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыРасход.СчетУчета = ПеремещениеСчетУчета;
							СтрокаТаблицыРасход.КоррСчетУчета = ПеремещениеСчетУчета;
							//+++
							СтрокаТаблицыРасход.СчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							СтрокаТаблицыРасход.КоррСчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							//---
						Иначе
							СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							//+++
							СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							//---
						КонецЕсли;
						СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
						СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
						СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
						СтрокаТаблицыРасход.КоррСпецификация = СтрокаТаблицаЗапасыПродукция.Спецификация;
						СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаМассивРазмещения.ЗаказПокупателя;
						СтрокаТаблицыРасход.Количество = СтрокаМассивРазмещения.Количество;
						СтрокаТаблицыРасход.Сумма = СуммаКСписаниюПоЗаказу;
						
						//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыРасход.КоррПроект = СтрокаМассивРазмещения.Проект;
						//СтрокаТаблицыРасход.КоррПроект = ?(СтрокаМассивРазмещения.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаМассивРазмещения.Проект);//++Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаМассивРазмещения.ЗаказПокупателя;
						
						Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыПриход.СчетУчета = ПеремещениеСчетУчета;
							СтрокаТаблицыПриход.КоррСчетУчета = ПеремещениеСчетУчета;
							//+++
							СтрокаТаблицыПриход.СчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							СтрокаТаблицыПриход.КоррСчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							//---
						Иначе
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							//+++
							СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							//---
						КонецЕсли;
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
						СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасыПродукция.Спецификация;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя;
						СтрокаТаблицыПриход.Количество = СтрокаМассивРазмещения.Количество;
						СтрокаТаблицыПриход.Сумма = СуммаКСписаниюПоЗаказу;
						
						//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасыПродукция.Проект;
						//СтрокаТаблицыПриход.КоррПроект =?(СтрокаТаблицаЗапасыПродукция.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПродукция.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						
						Индекс = Индекс + 1;
						
					КонецЕсли;
					
				КонецЦикла;
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыПродукция");
		ТаблицаРазмещенияПродукции = Неопределено;
		
	КонецПроцедуры // СформироватьТаблицаЗапасыПродукцияСборка()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаРазмещениеЗаказовСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		
		// Установка исключительной блокировки контролируемых размещений заказов.
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаПродукция.Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.ИсточникОбеспечения КАК ИсточникОбеспечения
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ИсточникОбеспечения <> НЕОПРЕДЕЛЕНО
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.ИсточникОбеспечения";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.РазмещениеЗаказов");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
		КонецЦикла;
		Блокировка.Заблокировать();
		
		// Получение остатков.
		Запрос.Текст = 	
		"ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	РазмещениеЗаказовОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.ИсточникОбеспечения КАК ИсточникОбеспечения,
		|	ВЫБОР
		|		КОГДА ТаблицаПродукция.Количество > ЕСТЬNULL(РазмещениеЗаказовОстатки.Количество, 0)
		|			ТОГДА ЕСТЬNULL(РазмещениеЗаказовОстатки.Количество, 0)
		|		КОГДА ТаблицаПродукция.Количество <= ЕСТЬNULL(РазмещениеЗаказовОстатки.Количество, 0)
		|			ТОГДА ТаблицаПродукция.Количество
		|	КОНЕЦ КАК Количество  
		|	,ТаблицаПродукция.Проект КАК Проект //+++ Дмгг Таск ONESOFT-2774 14,12,223
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			РазмещениеЗаказовОстатки.Организация КАК Организация,
		|			РазмещениеЗаказовОстатки.Номенклатура КАК Номенклатура,
		|			РазмещениеЗаказовОстатки.Характеристика КАК Характеристика,
		|			РазмещениеЗаказовОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|			РазмещениеЗаказовОстатки.ИсточникОбеспечения КАК ИсточникОбеспечения,
		|			СУММА(РазмещениеЗаказовОстатки.КоличествоОстаток) КАК Количество
		|		ИЗ
		|			(ВЫБРАТЬ
		|				РазмещениеЗаказовОстатки.Организация КАК Организация,
		|				РазмещениеЗаказовОстатки.Номенклатура КАК Номенклатура,
		|				РазмещениеЗаказовОстатки.Характеристика КАК Характеристика,
		|				РазмещениеЗаказовОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|				РазмещениеЗаказовОстатки.ИсточникОбеспечения КАК ИсточникОбеспечения,
		|				РазмещениеЗаказовОстатки.КоличествоОстаток КАК КоличествоОстаток
		|			ИЗ
		|				РегистрНакопления.РазмещениеЗаказов.Остатки(
		|						&МоментКонтроля,
		|						(Организация, Номенклатура, Характеристика, ИсточникОбеспечения) В
		|							(ВЫБРАТЬ
		|								ТаблицаПродукция.Организация КАК Организация,
		|								ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|								ТаблицаПродукция.Характеристика КАК Характеристика,
		|								ТаблицаПродукция.ИсточникОбеспечения КАК ИсточникОбеспечения
		|							ИЗ
		|								ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|							ГДЕ
		|								ТаблицаПродукция.ИсточникОбеспечения <> НЕОПРЕДЕЛЕНО)) КАК РазмещениеЗаказовОстатки
		|			
		|			ОБЪЕДИНИТЬ ВСЕ
		|			
		|			ВЫБРАТЬ
		|				ДвиженияДокументаРазмещениеЗаказов.Организация,
		|				ДвиженияДокументаРазмещениеЗаказов.Номенклатура,
		|				ДвиженияДокументаРазмещениеЗаказов.Характеристика,
		|				ДвиженияДокументаРазмещениеЗаказов.ЗаказПокупателя,
		|				ДвиженияДокументаРазмещениеЗаказов.ИсточникОбеспечения,
		|				ВЫБОР
		|					КОГДА ДвиженияДокументаРазмещениеЗаказов.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|						ТОГДА ЕСТЬNULL(ДвиженияДокументаРазмещениеЗаказов.Количество, 0)
		|					ИНАЧЕ -ЕСТЬNULL(ДвиженияДокументаРазмещениеЗаказов.Количество, 0)
		|				КОНЕЦ
		|			ИЗ
		|				РегистрНакопления.РазмещениеЗаказов КАК ДвиженияДокументаРазмещениеЗаказов
		|			ГДЕ
		|				ДвиженияДокументаРазмещениеЗаказов.Регистратор = &Ссылка
		|				И ДвиженияДокументаРазмещениеЗаказов.Период <= &ПериодКонтроля) КАК РазмещениеЗаказовОстатки
		|		
		|		СГРУППИРОВАТЬ ПО
		|			РазмещениеЗаказовОстатки.Организация,
		|			РазмещениеЗаказовОстатки.Номенклатура,
		|			РазмещениеЗаказовОстатки.Характеристика,
		|			РазмещениеЗаказовОстатки.ЗаказПокупателя,
		|			РазмещениеЗаказовОстатки.ИсточникОбеспечения) КАК РазмещениеЗаказовОстатки
		|		ПО ТаблицаПродукция.Организация = РазмещениеЗаказовОстатки.Организация
		|			И ТаблицаПродукция.Номенклатура = РазмещениеЗаказовОстатки.Номенклатура
		|			И ТаблицаПродукция.Характеристика = РазмещениеЗаказовОстатки.Характеристика
		|			И ТаблицаПродукция.ИсточникОбеспечения = РазмещениеЗаказовОстатки.ИсточникОбеспечения
		|ГДЕ
		|	ТаблицаПродукция.ИсточникОбеспечения <> НЕОПРЕДЕЛЕНО
		|	И РазмещениеЗаказовОстатки.ЗаказПокупателя ЕСТЬ НЕ NULL ";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.ПериодКонтроля);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРазмещениеЗаказов", РезультатЗапроса.Выгрузить());
		
	КонецПроцедуры // СформироватьТаблицаРазмещениеЗаказовСборка()
	
	Процедура СформироватьТаблицаКонтрольСроковГодностиТовараПродукция(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Если ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы <>  Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
			Возврат;
		КонецЕсли;
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	КонтрольСроковГодностиТовара.СрокГодности КАК СрокГодности,
		|	КонтрольСроковГодностиТовара.Количество КАК Количество,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	КонтрольСроковГодностиТовара.Период КАК Период,
		|	КонтрольСроковГодностиТовара.Организация КАК Организация,
		|	ЕСТЬNULL(КонтрольСроковГодностиТовара.СтруктурнаяЕдиницаПродукции, &Склад) КАК СтруктурнаяЕдиница,
		|	КонтрольСроковГодностиТовара.Номенклатура КАК Номенклатура,
		|	КонтрольСроковГодностиТовара.Характеристика КАК Характеристика,
		|	КонтрольСроковГодностиТовара.Ссылка КАК Документ
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК КонтрольСроковГодностиТовара
		|ГДЕ
		|	КонтрольСроковГодностиТовара.ВключенВрегистр = ИСТИНА
		|	И КонтрольСроковГодностиТовара.СрокГодности <> ДАТАВРЕМЯ(1, 1, 1)";
		
		Запрос.УстановитьПараметр("Склад",ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаПродукции);
		РезультатЗапроса = Запрос.Выполнить().Выгрузить();
		
		//СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКонтрольСроковГодностиТовара", РезультатЗапроса.Выгрузить());
		
		Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.свойство("ТаблицаКонтрольСроковГодностиТовара") Тогда 
			Для Каждого Ст из РезультатЗапроса Цикл 
				НовСтрока = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаКонтрольСроковГодностиТовара.добавить();
				ЗаполнитьЗначенияСвойств(НовСтрока,Ст);
			КонецЦикла;	
		Иначе 		
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКонтрольСроковГодностиТовара", РезультатЗапроса);	
		КонецЕсли;
		
		
	КонецПроцедуры 
	
	Процедура СформироватьТаблицаКонтрольСроковГодностиТовараЗапасы(ДокументСсылка, СтруктураДополнительныеСвойства) 
		
		Если ДокументСсылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы <>  Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
			Возврат;
		КонецЕсли;
		
		КопияТаблицаЗапасы = Новый ТаблицаЗначений;
		КопияТаблицаЗапасы.Колонки.Добавить("Организация");
		КопияТаблицаЗапасы.Колонки.Добавить("СтруктурнаяЕдиница");
		КопияТаблицаЗапасы.Колонки.Добавить("Документ");
		КопияТаблицаЗапасы.Колонки.Добавить("Номенклатура");
		КопияТаблицаЗапасы.Колонки.Добавить("Характеристика");
		КопияТаблицаЗапасы.Колонки.Добавить("Количество");
		КопияТаблицаЗапасы.Колонки.Добавить("Период");
		КопияТаблицаЗапасы.Колонки.Добавить("ВидДвижения");	
		КопияТаблицаЗапасы.Колонки.Добавить("СрокГодности");
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	КонтрольСроковГодностиТовара.Количество КАК Количество,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	КонтрольСроковГодностиТовара.Период КАК Период,
		|	КонтрольСроковГодностиТовара.Организация КАК Организация,
		|	КонтрольСроковГодностиТовара.Номенклатура КАК Номенклатура,
		|	КонтрольСроковГодностиТовара.Характеристика КАК Характеристика,
		|	КонтрольСроковГодностиТовара.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиницаЗапасов
		|ПОМЕСТИТЬ ВтПродажи
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК КонтрольСроковГодностиТовара
		|ГДЕ
		|	КонтрольСроковГодностиТовара.ВключенВрегистр = ИСТИНА
		|
		|СГРУППИРОВАТЬ ПО
		|	КонтрольСроковГодностиТовара.Номенклатура,
		|	КонтрольСроковГодностиТовара.Характеристика,
		|	КонтрольСроковГодностиТовара.Количество,
		|	КонтрольСроковГодностиТовара.Период,
		|	КонтрольСроковГодностиТовара.Организация,
		|	КонтрольСроковГодностиТовара.СтруктурнаяЕдиницаЗапасов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 15000
		|	ВтПродажи.Номенклатура КАК Номенклатура,
		|	СУММА(ВтПродажи.Количество) КАК Количество,
		|	КонтрольСроковГодностиТовараОстатки.Документ КАК Документ,
		|	СУММА(КонтрольСроковГодностиТовараОстатки.КоличествоОстаток) КАК КоличествоОстаток,
		|	КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВтПродажи.Организация КАК Организация,
		|	ВтПродажи.Период КАК Период,
		|	ВтПродажи.ВидДвижения КАК ВидДвижения,
		|	КонтрольСроковГодностиТовараОстатки.СрокГодности КАК СрокГодности
		|ПОМЕСТИТЬ ВтОстатки
		|ИЗ
		|	РегистрНакопления.КонтрольСроковГодностиТовара.Остатки(
		|			&Дата,
		|			Организация = &Организация
		|				И СтруктурнаяЕдиница = &СтруктурнаяЕдиница
		|				И (Номенклатура, Характеристика) В
		|					(ВЫБРАТЬ
		|						ВтПродажи.Номенклатура КАК Номенклатура,
		|						ВтПродажи.Характеристика КАК Характеристика
		|					ИЗ
		|						ВтПродажи КАК ВтПродажи)) КАК КонтрольСроковГодностиТовараОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПродажи КАК ВтПродажи
		|		ПО КонтрольСроковГодностиТовараОстатки.Организация = ВтПродажи.Организация
		|			И КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница = ВтПродажи.СтруктурнаяЕдиницаЗапасов
		|
		|СГРУППИРОВАТЬ ПО
		|	КонтрольСроковГодностиТовараОстатки.Документ,
		|	ВтПродажи.Номенклатура,
		|	КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница,
		|	ВтПродажи.Организация,
		|	ВтПродажи.Период,
		|	ВтПродажи.ВидДвижения,
		|	КонтрольСроковГодностиТовараОстатки.СрокГодности
		|
		|УПОРЯДОЧИТЬ ПО
		|	СрокГодности
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтОстатки.Номенклатура КАК Номенклатура,
		|	СУММА(ВтОстатки.Количество) КАК Количество,
		|	ВтОстатки.Документ КАК Документ,
		|	СУММА(ВтОстатки.КоличествоОстаток) КАК КоличествоОстаток,
		|	ВтОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВтОстатки.Организация КАК Организация,
		|	ВтОстатки.Период КАК Период,
		|	ВтОстатки.ВидДвижения КАК ВидДвижения,
		|	ДАТАВРЕМЯ(1, 1, 1) КАК СрокГодности
		|ИЗ
		|	ВтОстатки КАК ВтОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	ВтОстатки.Организация,
		|	ВтОстатки.ВидДвижения,
		|	ВтОстатки.СтруктурнаяЕдиница,
		|	ВтОстатки.Документ,
		|	ВтОстатки.Номенклатура,
		|	ВтОстатки.Период
		|ИТОГИ
		|	МАКСИМУМ(Количество),
		|	СУММА(КоличествоОстаток)
		|ПО
		|	Номенклатура";
		
		
		Запрос.УстановитьПараметр("Дата",ДокументСсылка.Дата-2);
		Запрос.УстановитьПараметр("Организация",ДокументСсылка.Организация);
		Запрос.УстановитьПараметр("СтруктурнаяЕдиница",ДокументСсылка.СтруктурнаяЕдиницаЗапасов);
		
		ВыборкаНоменклатура = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		
		Пока ВыборкаНоменклатура.Следующий() Цикл
			КоличествоПогасить=0;
			КоличествоПогасить = ВыборкаНоменклатура.Количество;
			
			ВыборкаПродажи = ВыборкаНоменклатура.Выбрать(); 		
			Пока ВыборкаПродажи.Следующий() Цикл
				Если Не ВыборкаПродажи.КоличествоОстаток > 0 Тогда
					Продолжить;
				КонецЕсли;
				Если   КоличествоПогасить <> Null Тогда 
					Если КоличествоПогасить >0 Тогда  
						стрЗапасы = КопияТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(стрЗапасы,ВыборкаПродажи);
						стрЗапасы.Количество = Мин(ВыборкаПродажи.КоличествоОстаток,КоличествоПогасить);
						КоличествоПогасить 	 = КоличествоПогасить - ВыборкаПродажи.КоличествоОстаток;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКонтрольСроковГодностиТовара", КопияТаблицаЗапасы);	
		
	КонецПроцедуры
	
	Процедура СформироватьТаблицаКонтрольСроковГодностиТовараЗапасыРазборка(ДокументСсылка, СтруктураДополнительныеСвойства);
		
		Если ДокументСсылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы <>  Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
			Возврат;
		КонецЕсли;
		
		КопияТаблицаЗапасы = Новый ТаблицаЗначений;
		КопияТаблицаЗапасы.Колонки.Добавить("Организация");
		КопияТаблицаЗапасы.Колонки.Добавить("СтруктурнаяЕдиница");
		КопияТаблицаЗапасы.Колонки.Добавить("Документ");
		КопияТаблицаЗапасы.Колонки.Добавить("Номенклатура");
		КопияТаблицаЗапасы.Колонки.Добавить("Характеристика");
		КопияТаблицаЗапасы.Колонки.Добавить("Количество");
		КопияТаблицаЗапасы.Колонки.Добавить("Период");
		КопияТаблицаЗапасы.Колонки.Добавить("ВидДвижения");	
		КопияТаблицаЗапасы.Колонки.Добавить("СрокГодности");
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СборкаЗапасовПродукция.Количество КАК Количество,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	СборкаЗапасовПродукция.Ссылка.Дата КАК Период,
		|	СборкаЗапасовПродукция.Ссылка.Организация КАК Организация,
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовПродукция.Характеристика КАК Характеристика,
		|	&СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаЗапасов
		|ПОМЕСТИТЬ ВтЗапасы
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|ГДЕ
		|	СборкаЗапасовПродукция.Ссылка = &Ссылка
		|	И СборкаЗапасовПродукция.Номенклатура.ВключенВрегистр = ИСТИНА
		|
		|СГРУППИРОВАТЬ ПО
		|	СборкаЗапасовПродукция.Количество,
		|	СборкаЗапасовПродукция.Ссылка.Дата,
		|	СборкаЗапасовПродукция.Ссылка.Организация,
		|	СборкаЗапасовПродукция.Номенклатура,
		|	СборкаЗапасовПродукция.Характеристика
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 15000
		|	ВтЗапасы.Номенклатура КАК Номенклатура,
		|	СУММА(ВтЗапасы.Количество) КАК Количество,
		|	КонтрольСроковГодностиТовараОстатки.Документ КАК Документ,
		|	СУММА(КонтрольСроковГодностиТовараОстатки.КоличествоОстаток) КАК КоличествоОстаток,
		|	КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВтЗапасы.Организация КАК Организация,
		|	ВтЗапасы.Период КАК Период,
		|	ВтЗапасы.ВидДвижения КАК ВидДвижения,
		|	КонтрольСроковГодностиТовараОстатки.СрокГодности КАК СрокГодности
		|ПОМЕСТИТЬ ВтРасходЗапасы
		|ИЗ
		|	РегистрНакопления.КонтрольСроковГодностиТовара.Остатки(
		|			&Дата,
		|			Организация = &Организация
		|				И СтруктурнаяЕдиница = &СтруктурнаяЕдиница
		|				И (Номенклатура, Характеристика) В
		|					(ВЫБРАТЬ
		|						ВтЗапасы.Номенклатура КАК Номенклатура,
		|						ВтЗапасы.Характеристика КАК Характеристика
		|					ИЗ
		|						ВтЗапасы КАК ВтЗапасы)) КАК КонтрольСроковГодностиТовараОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЗапасы КАК ВтЗапасы
		|		ПО КонтрольСроковГодностиТовараОстатки.Организация = ВтЗапасы.Организация
		|			И КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница = ВтЗапасы.СтруктурнаяЕдиницаЗапасов
		|
		|СГРУППИРОВАТЬ ПО
		|	КонтрольСроковГодностиТовараОстатки.Документ,
		|	ВтЗапасы.Номенклатура,
		|	КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница,
		|	ВтЗапасы.Организация,
		|	ВтЗапасы.Период,
		|	ВтЗапасы.ВидДвижения,
		|	КонтрольСроковГодностиТовараОстатки.СрокГодности
		|
		|УПОРЯДОЧИТЬ ПО
		|	СрокГодности
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтРасходЗапасы.Номенклатура КАК Номенклатура,
		|	СУММА(ВтРасходЗапасы.Количество) КАК Количество,
		|	ВтРасходЗапасы.Документ КАК Документ,
		|	СУММА(ВтРасходЗапасы.КоличествоОстаток) КАК КоличествоОстаток,
		|	ВтРасходЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВтРасходЗапасы.Организация КАК Организация,
		|	ВтРасходЗапасы.Период КАК Период,
		|	ВтРасходЗапасы.ВидДвижения КАК ВидДвижения,
		|	ДАТАВРЕМЯ(1, 1, 1) КАК СрокГодности
		|ИЗ
		|	ВтРасходЗапасы КАК ВтРасходЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ВтРасходЗапасы.Организация,
		|	ВтРасходЗапасы.ВидДвижения,
		|	ВтРасходЗапасы.СтруктурнаяЕдиница,
		|	ВтРасходЗапасы.Документ,
		|	ВтРасходЗапасы.Номенклатура,
		|	ВтРасходЗапасы.Период
		|ИТОГИ
		|	МАКСИМУМ(Количество),
		|	СУММА(КоличествоОстаток)
		|ПО
		|	Номенклатура";
		
		
		Запрос.УстановитьПараметр("Дата",ДокументСсылка.Дата-2);
		Запрос.УстановитьПараметр("Организация",ДокументСсылка.Организация);
		Запрос.УстановитьПараметр("СтруктурнаяЕдиница",ДокументСсылка.СтруктурнаяЕдиницаЗапасов);
		Запрос.УстановитьПараметр("Ссылка",ДокументСсылка);
		
		ВыборкаНоменклатура = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		
		Пока ВыборкаНоменклатура.Следующий() Цикл
			КоличествоПогасить=0;
			КоличествоПогасить = ВыборкаНоменклатура.Количество;
			
			ВыборкаПродажи = ВыборкаНоменклатура.Выбрать(); 		
			Пока ВыборкаПродажи.Следующий() Цикл
				Если Не ВыборкаПродажи.КоличествоОстаток > 0 Тогда
					Продолжить;
				КонецЕсли;
				Если   КоличествоПогасить <> Null Тогда 
					Если КоличествоПогасить >0 Тогда  
						стрЗапасы = КопияТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(стрЗапасы,ВыборкаПродажи);
						стрЗапасы.Количество = Мин(ВыборкаПродажи.КоличествоОстаток,КоличествоПогасить);
						КоличествоПогасить 	 = КоличествоПогасить - ВыборкаПродажи.КоличествоОстаток;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКонтрольСроковГодностиТовара", КопияТаблицаЗапасы);	
		
		
	КонецПроцедуры
	////////////////////////////////////////////////////////////////////////////////
	// ЗАПАСЫ (РАЗБОРКА)
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыЗапасыРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		
		// Установка исключительной блокировки контролируемых остатков запасов.
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация КАК Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|		ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|		ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|		ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|		ТаблицаЗапасы.Характеристика КАК Характеристика,
		|		ТаблицаЗапасы.Партия КАК Партия,
		|		ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|	ГДЕ
		|		ТаблицаЗапасы.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиница,
		|		ТаблицаЗапасы.СчетУчета,
		|		ТаблицаЗапасы.СчетУчетаРегл,
		|		ТаблицаЗапасы.Номенклатура,
		|		ТаблицаЗапасы.Характеристика,
		|		ТаблицаЗапасы.Партия,
		|		ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы) КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.Запасы");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			Если КолонкаРезультатЗапроса.Имя <> "СчетУчетаРегл" Тогда 
				ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
			КонецЕсли;
		КонецЦикла;
		Блокировка.Заблокировать();
		
		// Получение остатков запасов по стоимости.
		
		//Малышев 2024-06-13 ONESOFT-3092 - перенесено в отдельный модуль (--
		//ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении_Разборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства).Выгрузить();
		//ТаблицаЗапасыОстатки.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчета,Номенклатура,Характеристика,Партия,ЗаказПокупателя");
		
		ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства).Выгрузить();
		ТаблицаЗапасыОстатки.Индексы.Добавить("Организация,СтруктурнаяЕдиница,Номенклатура,Характеристика,Партия,ЗаказПокупателя");	
		//Малышев 2024-06-13 ONESOFT-3092 --)
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы[н];
			
			//Малышев 2024-07-27 ((-
			СтруктураДляПоиска = УправлениеНебольшойФирмойСервер.ПолучитьСтруктуруДляПоиска(СтрокаТаблицаЗапасы, СтруктураДополнительныеСвойства);
			//СтруктураДляПоиска = Новый Структура;
			//СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасы.Организация);
			//СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасы.СтруктурнаяЕдиница);
			////СтруктураДляПоиска.Вставить("СчетУчета", СтрокаТаблицаЗапасы.СчетУчета);
			//СтруктураДляПоиска.Вставить("СчетУчетаРегл", СтрокаТаблицаЗапасы.СчетУчетаРегл);
			//СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасы.Номенклатура);
			//СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасы.Характеристика);
			//СтруктураДляПоиска.Вставить("Партия", СтрокаТаблицаЗапасы.Партия);
			////Малышев 2024-06-16 ONESOFT-3094
			//Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
			//	СтруктураДляПоиска.Вставить("Проект", СтрокаТаблицаЗапасы.Проект);
			//КонецЕсли;	
            //Малышев 2024-07-27 --)

			
			ТребуетсяРезерв = СтрокаТаблицаЗапасы.Резерв;
			ТребуетсяКоличество = СтрокаТаблицаЗапасы.Количество;
			
			Если ТребуетсяРезерв > 0 Тогда
				
				ТребуетсяКоличество = ТребуетсяКоличество - ТребуетсяРезерв;
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
				
				МассивСтрокОстатков = ТаблицаЗапасыОстатки.НайтиСтроки(СтруктураДляПоиска);
				
				//КоличествоОстаток = 0;
				//СуммаОстаток = 0;
				//
				//Если МассивСтрокОстатков.Количество() > 0 Тогда
				//	
				//	КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток;
				//	СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток;
				//	
				//	ТребуетсяСумма = Окр(МассивСтрокОстатков[0].СуммаОстаток * ТребуетсяРезерв / МассивСтрокОстатков[0].КоличествоОстаток,2,1);
				//	
				//КонецЕсли;
				//
				//Если КоличествоОстаток > 0 И КоличествоОстаток > ТребуетсяРезерв Тогда
				//	
				//	СуммаКСписанию = Окр(СуммаОстаток * ТребуетсяРезерв / КоличествоОстаток , 2, 1);
				//	
				//	МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток - ТребуетсяРезерв;
				//	МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СуммаКСписанию;
				//	
				//ИначеЕсли КоличествоОстаток = ТребуетсяРезерв Тогда
				//	
				//	СуммаКСписанию = СуммаОстаток;
				//	
				//Иначе
				//	СуммаКСписанию = 0;
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
			
					Если ТребуетсяРезерв <= 0 Тогда
						Прервать;
					КонецЕсли;
					
					КоличествоОстатокРезерв = СтрокаТаблицыОстатков.КоличествоОстаток;
					СуммаОстатокРезерв = СтрокаТаблицыОстатков.СуммаОстаток;
					
					КоличествоКСписаниюРезерв = 0;
					СуммаКСписаниюРезерв = 0;
					
					Если (КоличествоОстатокРезерв > 0) И (КоличествоОстатокРезерв > ТребуетсяРезерв) Тогда
						
						КоличествоКСписаниюРезерв = ТребуетсяРезерв;
						СуммаКСписанию = Окр(СуммаОстатокРезерв * ТребуетсяРезерв / КоличествоОстатокРезерв , 2, 1);
						
						СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - ТребуетсяРезерв;
						СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписанию;
						
					ИначеЕсли (КоличествоОстатокРезерв > 0) И (КоличествоОстатокРезерв <= ТребуетсяРезерв) Тогда
						
						КоличествоКСписаниюРезерв = КоличествоОстатокРезерв;
						СуммаКСписанию = СуммаОстатокРезерв;
						
						СтрокаТаблицыОстатков.КоличествоОстаток = 0;
						СтрокаТаблицыОстатков.СуммаОстаток = 0;
						
					ИначеЕсли КоличествоОстатокРезерв <= 0 Тогда  
						
						Продолжить;
						
					КонецЕсли;
					
				ТребуетсяРезерв = ТребуетсяРезерв - КоличествоКСписаниюРезерв;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Спишем запасы со склада (производственного подразделения).
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				//СтрокаТаблицыРасход.Количество = ТребуетсяРезерв;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.Количество = КоличествоКСписаниюРезерв;
				СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				// Отнесем списанные запасы либо на стоимость запасов на складе либо на затраты НЗП.
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
					
					//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
					//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;//+++ Дмгг
					
					СтрокаТаблицыПриход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					// Сформируем проводки.
					Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Разборка1(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					КонецЕсли;
					
					// Автосписание запасов.
					Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции) Тогда
						
						// Расход.
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыРасход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыРасход.Спецификация = Неопределено;
						СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						//+++
						СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						//---
						СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
						СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыРасход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыРасход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
						СтрокаТаблицыРасход.Количество = 0;
						
						// Приход.
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.Спецификация = Неопределено;
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыПриход.СчетДт = СтрокаТаблицаЗапасы.СчетДтПродукции;
						
						СтрокаТаблицыПриход.СчетДтРегл = СтрокаТаблицаЗапасы.СчетДтПродукцииРегл;
						
						СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						//+++
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						//---
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.202
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
						СтрокаТаблицыПриход.Количество = 0;
						
						СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						
						// Сформируем проводки.
						Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
							СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
							
							ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Разборка2(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						КонецЕсли;
						
					КонецЕсли;
					
				КонецЕсли;
				
				КонецЦикла; // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости	
				
			КонецЕсли;
			
			Если ТребуетсяКоличество > 0 Тогда
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", Документы.ЗаказПокупателя.ПустаяСсылка());
				
				МассивСтрокОстатков = ТаблицаЗапасыОстатки.НайтиСтроки(СтруктураДляПоиска);
				
				//КоличествоОстаток = 0;
				//СуммаОстаток = 0;
				//
				//Если МассивСтрокОстатков.Количество() > 0 Тогда
				//	
				//	КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток;
				//	СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток;
				//	
				//	ТребуетсяСумма = Окр(МассивСтрокОстатков[0].СуммаОстаток * ТребуетсяКоличество / МассивСтрокОстатков[0].КоличествоОстаток,2,1);
				//	
				//КонецЕсли;
				//
				//Если КоличествоОстаток > 0 И КоличествоОстаток > ТребуетсяКоличество Тогда
				//	
				//	СуммаКСписанию = Окр(СуммаОстаток * ТребуетсяКоличество / КоличествоОстаток , 2, 1);
				//	
				//	МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток - ТребуетсяКоличество;
				//	МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СуммаКСписанию;
				//	
				//ИначеЕсли КоличествоОстаток = ТребуетсяКоличество Тогда
				//	
				//	СуммаКСписанию = СуммаОстаток;
				//	
				//Иначе
				//	СуммаКСписанию = 0;
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
					
					Если ТребуетсяКоличество <= 0 Тогда
						Прервать;
					КонецЕсли;
					
					КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток;
					СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток;
					
					КоличествоКСписанию = 0;
					СуммаКСписанию = 0;
					
					Если КоличествоОстаток > 0 И КоличествоОстаток > ТребуетсяКоличество Тогда
						
						КоличествоКСписанию = ТребуетсяКоличество;
						СуммаКСписанию = Окр(СуммаОстаток * ТребуетсяКоличество / КоличествоОстаток , 2, 1);
						
						СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - ТребуетсяКоличество;
						СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписанию;
						
					ИначеЕсли (КоличествоОстаток>0) И (КоличествоОстаток<=ТребуетсяКоличество) Тогда
						
						КоличествоКСписанию = КоличествоОстаток;
						СуммаКСписанию = СуммаОстаток;
						
						СтрокаТаблицыОстатков.КоличествоОстаток = 0;
						СтрокаТаблицыОстатков.СуммаОстаток = 0;
						
					Иначе
						СуммаКСписанию = 0;	
					КонецЕсли;
					
				ТребуетсяКоличество = ТребуетсяКоличество - КоличествоКСписанию;	
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Расход.
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				//СтрокаТаблицыРасход.Количество = ТребуетсяКоличество;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.Количество = КоличествоКСписанию;
				Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				КонецЕсли;
				Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.ЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
				
				// Приход
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					
					//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
					//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
					КонецЕсли;
					Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриход.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости

					// Сформируем проводки.
					Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Разборка3(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					КонецЕсли;
					
					// Автосписание запасов.
					Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции) Тогда
						
						// Расход.
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						
						СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыРасход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыРасход.Спецификация = Неопределено;
						СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						//+++
						СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						
						СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						//---
						СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
						СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыРасход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыРасход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
						СтрокаТаблицыРасход.Количество = 0;
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						СтрокаТаблицыРасход.Количество = КоличествоКСписанию;
						Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
						КонецЕсли;
						Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						
						// Приход.
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;

                        СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.Спецификация = Неопределено;
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыПриход.СчетДт = СтрокаТаблицаЗапасы.СчетДтПродукции;

						СтрокаТаблицыПриход.СчетДтРегл = СтрокаТаблицаЗапасы.СчетДтПродукцииРегл;

						СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;

                        СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;

						//+++
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						//---
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
						СтрокаТаблицыПриход.Количество = 0;
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						
						СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						
                        // Сформируем проводки.
						Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
							СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
							
							ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Разборка4(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						КонецЕсли;
						
					КонецЕсли;
					
				КонецЕсли;
				
				КонецЦикла;// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// делаем отрицательный остаток
				Если ТребуетсяКоличество > 0 Тогда
					
					// Расход.
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыРасход.Сумма = 0;
					СтрокаТаблицыРасход.Количество = ТребуетсяКоличество;
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					СтрокаТаблицыРасход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					СтрокаТаблицыРасход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
					СтрокаТаблицыРасход.ЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					
					//// Приход
					////Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					//	
					//	СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//	ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//	
					//	СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					//	
					//	СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//	СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//	СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//	СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//	СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//	СтрокаТаблицыПриход.Спецификация = СтрокаТаблицаЗапасы.КоррСпецификация;
					//	СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//	
					//	СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					//	СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//	//+++
					//	СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//	СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//	//---
					//	СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					//	СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					//	СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					//	СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;
					//	СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					//	
					//	СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//	СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//	
					//	СтрокаТаблицыПриход.Сумма = 0;
					//	СтрокаТаблицыПриход.Количество = 0;
					//	
					//	СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//	СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл; //+++ Дмгг
					//	
					//	// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	СтрокаТаблицыПриход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					//	СтрокаТаблицыПриход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					//	// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	
					//	// Сформируем проводки.
					//	Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					//		СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					//		ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
					//		
					//		ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Разборка3(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//	КонецЕсли;
					//	
					//	// Автосписание запасов.
					//	Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции) Тогда
					//		
					//		// Расход.
					//		СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//		ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
					//		
					//		СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//		
					//		СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//		СтрокаТаблицыРасход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыРасход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыРасход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//		СтрокаТаблицыРасход.Спецификация = Неопределено;
					//		СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
					//		СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
					//		//+++
					//		СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//		
					//		СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
					//		//---
					//		СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
					//		СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
					//		СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//		СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//		
					//		СтрокаТаблицыРасход.Сумма = 0;
					//		СтрокаТаблицыРасход.Количество = 0;
					//		
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		СтрокаТаблицыРасход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		СтрокаТаблицыРасход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		
					//		// Приход.
					//		СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//		ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//		
					//		СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					//		
					//		СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
					//		
					//		СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
					//		СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//		СтрокаТаблицыПриход.Спецификация = Неопределено;
					//		СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыПриход.СчетДт = СтрокаТаблицаЗапасы.СчетДтПродукции;
					//		
					//		СтрокаТаблицыПриход.СчетДтРегл = СтрокаТаблицаЗапасы.СчетДтПродукцииРегл;
					//		
					//		СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//		
					//		СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//		
					//		//+++
					//		СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
					//		СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//		//---
					//		СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
					//		СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
					//		СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//		СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//		
					//		СтрокаТаблицыПриход.Сумма = 0;
					//		СтрокаТаблицыПриход.Количество = 0;
					//		
					//		СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
					//		СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
					//		
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		СтрокаТаблицыПриход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		СтрокаТаблицыПриход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		
					//		// Сформируем проводки.
					//		//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					//			СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					//			ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
					//			
					//			ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_Разборка4(СтруктураДополнительныеСвойства, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//		//КонецЕсли;
					//		
					//	КонецЕсли;
					//	
					////КонецЕсли;
					
				КонецЕсли;
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыЗапасы");
		
	КонецПроцедуры // СформироватьТаблицаЗапасыЗапасыРазборка()
	
	Процедура СформироватьТаблицаЗапасыВРазрезеГТДРазборка(СтруктураДополнительныеСвойства)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасыНаСкладах.НомерСтроки) КАК НомерСтроки
		|	,ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения
		|	,ТаблицаЗапасыНаСкладах.Период КАК Период
		|	,ТаблицаЗапасыНаСкладах.Организация КАК Организация
		|	,ТаблицаЗапасыНаСкладах.Номенклатура КАК Номенклатура
		|	,ТаблицаЗапасыНаСкладах.Характеристика КАК Характеристика
		|	,ТаблицаЗапасыНаСкладах.Партия КАК Партия
		|	,ТаблицаЗапасыНаСкладах.НомерГТД КАК НомерГТД
		|	,ТаблицаЗапасыНаСкладах.СтранаПроисхождения КАК СтранаПроисхождения
		|	,СУММА(ТаблицаЗапасыНаСкладах.Количество) КАК Количество
		|ИЗ
		|	СписаниеПоНомерамГТД КАК ТаблицаЗапасыНаСкладах
		|ГДЕ
		|	ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> Значение(Справочник.СтраныМира.Россия)
		|	И ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> Значение(Справочник.СтраныМира.ПустаяССылка)
		|	И ТаблицаЗапасыНаСкладах.НомерГТД <> Значение(Справочник.НомераГТД.ПустаяССылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасыНаСкладах.Период
		|	,ТаблицаЗапасыНаСкладах.Организация
		|	,ТаблицаЗапасыНаСкладах.Номенклатура
		|	,ТаблицаЗапасыНаСкладах.Характеристика
		|	,ТаблицаЗапасыНаСкладах.Партия
		|	,ТаблицаЗапасыНаСкладах.НомерГТД
		|	,ТаблицаЗапасыНаСкладах.СтранаПроисхождения";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыВРазрезеГТД", РезультатЗапроса.Выгрузить());
		
	КонецПроцедуры
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыЗапасыРазборкаПеремещение(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
		УчетСебестоимостиВРазрезеОрганизации = СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетСебестоимостиВРазрезеОрганизации;
		Запрос.УстановитьПараметр("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации);
		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.СценарийПланирования КАК СценарийПланирования,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетУчета,
		|	ТаблицаЗапасы.СчетУчета КАК КоррСчетУчета,
		//+++
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаЗатратРегл,

		//---
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Номенклатура КАК КоррНоменклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Характеристика КАК КоррХарактеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Партия КАК КоррПартия,
		|	ТаблицаЗапасы.Спецификация КАК Спецификация,
		|	ТаблицаЗапасы.Спецификация КАК КоррСпецификация,
		
		//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ТаблицаЗапасы.КоррПроект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_
		
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК КоррЗаказПокупателя,
		|	НЕОПРЕДЕЛЕНО КАК ДокументПродажи,
		|	НЕОПРЕДЕЛЕНО КАК ДокументПоступления,  //Малышев 2024-11-19 ONESOFT-3602
		|	НЕОПРЕДЕЛЕНО КАК КоррДокументПоступления, // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	НЕОПРЕДЕЛЕНО КАК ЗаказПродажи,
		|	НЕОПРЕДЕЛЕНО КАК Подразделение,
		|	НЕОПРЕДЕЛЕНО КАК Ответственный,
		|	ТаблицаЗапасы.СчетУчета КАК СчетДт,
		|	ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетКт,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетДтРегл,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетКтРегл,
		//---
		|	&ПеремещениеЗапасов КАК Содержание,
		|	&ПеремещениеЗапасов КАК СодержаниеПроводки,
		|	ЛОЖЬ КАК ФиксированнаяСтоимость,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество,
		|	СУММА(ТаблицаЗапасы.Резерв) КАК Резерв,
		|	ТаблицаЗапасы.Сумма КАК Сумма
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.СценарийПланирования,
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчетаЗапасов,
		|	ТаблицаЗапасы.СчетУчета,
		//+++
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		|	ТаблицаЗапасы.СчетУчетаРегл,

		//---
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Спецификация,
		|	ТаблицаЗапасы.Спецификация,
		
		//Малышев 2024-06-17 - ONESOFT-3094 вернул обратно
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_
		
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаЗапасов,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		//---
		|	ТаблицаЗапасы.Сумма";
		
		Запрос.УстановитьПараметр("ПеремещениеЗапасов", НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'"));
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПеремещение", Запрос.Выполнить().Выгрузить());
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
		Запрос.УстановитьПараметр("УчетСебестоимостиВРазрезеОрганизации", УчетСебестоимостиВРазрезеОрганизации);
		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
		//--------------------------------------------------------------------------
		
		// Установка исключительной блокировки контролируемых остатков запасов.
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,

		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация КАК Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
		|		ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетУчета,
		|		ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|		ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|		ТаблицаЗапасы.Характеристика КАК Характеристика,
		|		ТаблицаЗапасы.Партия КАК Партия,

		|		ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|	ГДЕ
		|		ТаблицаЗапасы.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ТаблицаЗапасы.Организация,
		|		ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
		|		ТаблицаЗапасы.СчетУчетаЗапасов,
		|		ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		|		ТаблицаЗапасы.Номенклатура,
		|		ТаблицаЗапасы.Характеристика,
		|		ТаблицаЗапасы.Партия,

		|		ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	ИЗ
		|		ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы) КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.СчетУчетаРегл,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,

		|	ТаблицаЗапасы.ЗаказПокупателя";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.Запасы");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			Если КолонкаРезультатЗапроса.Имя <> "СчетУчетаРегл" Тогда
				ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
			КонецЕсли;
		КонецЦикла;
		Блокировка.Заблокировать();
		
		
		//Малышев 2025-01-10 ONESOFT-3758 формируем таблицу, на какие товары вывести остатки (--
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказПокупателя
		|ПОМЕСТИТЬ ТаблицаПеремещаемыхТоваров
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	(ТаблицаЗапасы.Количество - ТаблицаЗапасы.Резерв) <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.ЗаказПокупателя
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	И ТаблицаЗапасы.Резерв <> 0";
		Запрос.Выполнить();
		//Малышев Малышев 2025-01-10 ONESOFT-3758 формируем таблицу, на какие товары вывести остатки --)

		
		// Получение остатков запасов по стоимости.
		//Малышев 2024-06-13 ONESOFT-3092 - перенесено в отдельный модуль (--
		//ТаблицаЗапасыОстаткиПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении_РазборкаПеремещение(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства).Выгрузить();
		//ТаблицаЗапасыОстаткиПеремещение.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчета,Номенклатура,Характеристика,Партия,ЗаказПокупателя");  //+++ Дмгг
		//ТаблицаЗапасыОстаткиПеремещение.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчета,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя");  //+++ Дмгг
		
		//ТаблицаЗапасыОстаткиПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, Истина).Выгрузить();
		ТаблицаЗапасыОстаткиПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, Ложь, "ТаблицаПеремещаемыхТоваров").Выгрузить();
		ТаблицаЗапасыОстаткиПеремещение.Индексы.Добавить("Организация,СтруктурнаяЕдиница,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя");  //+++ Дмгг
		//Малышев 2024-06-13 ONESOFT-3092 --)
		
		ВременнаяТаблицаЗапасыПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.СкопироватьКолонки();
		
		ПустаяСтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
		ПустойСчетУчета = ПланыСчетов.Управленческий.ПустаяСсылка();
		ПустойСчетУчетаРегл = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
		ПустаяНоменклатура = Справочники.Номенклатура.ПустаяСсылка();
		ПустаяХарактеристика = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
		ПустаяПартия = Справочники.ПартииНоменклатуры.ПустаяСсылка();
		ПустойЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасыПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение[н];
			
			//Малышев 2024-07-27 ((-
			СтруктураДляПоискаПеремещение = УправлениеНебольшойФирмойСервер.ПолучитьСтруктуруДляПоиска(СтрокаТаблицаЗапасыПеремещение, СтруктураДополнительныеСвойства);
			//СтруктураДляПоискаПеремещение = Новый Структура;
			//СтруктураДляПоискаПеремещение.Вставить("Организация", СтрокаТаблицаЗапасыПеремещение.Организация);
			////--------------------------------------------------------------------------
			//// © Шишка В.В. [11 октября 2021]  НАЧАЛО
			////СтруктураДляПоискаПеремещение.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница);
			//СтруктураДляПоискаПеремещение.Вставить("СтруктурнаяЕдиница", ?(УчетСебестоимостиВРазрезеОрганизации И (СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница = Неопределено ИЛИ
			//СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад), Неопределено, СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница));
			//// © Шишка В.В. [11 октября 2021]  КОНЕЦ
			////--------------------------------------------------------------------------
			////СтруктураДляПоискаПеремещение.Вставить("СчетУчета", СтрокаТаблицаЗапасыПеремещение.СчетУчета);
			//СтруктураДляПоискаПеремещение.Вставить("СчетУчетаРегл", СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл); //+++ Дмгг
			//СтруктураДляПоискаПеремещение.Вставить("Номенклатура", СтрокаТаблицаЗапасыПеремещение.Номенклатура);
			//СтруктураДляПоискаПеремещение.Вставить("Характеристика", СтрокаТаблицаЗапасыПеремещение.Характеристика);
			//СтруктураДляПоискаПеремещение.Вставить("Партия", СтрокаТаблицаЗапасыПеремещение.Партия);
			////Малышев 2024-06-16 ONESOFT-3094
			//Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
			//	СтруктураДляПоискаПеремещение.Вставить("Проект", СтрокаТаблицаЗапасыПеремещение.Проект);
			//КонецЕсли;	
			//Малышев 2024-07-27 --)
	

			//Малышев 2024-07-09
            СуммаДопРасходов = 0;
			
			//КоличествоТребуетсяРезервПеремещение = СтрокаТаблицаЗапасыПеремещение.Резерв;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
				КоличествоТребуетсяРезервПеремещение = 0;
			Иначе
				КоличествоТребуетсяРезервПеремещение = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасыПеремещение.Резерв), СтрокаТаблицаЗапасыПеремещение.Резерв, 0);
			КонецЕсли;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			КоличествоТребуетсяСвободныйОстатокПеремещение = СтрокаТаблицаЗапасыПеремещение.Количество;
			
			Если КоличествоТребуетсяРезервПеремещение > 0 Тогда
				
				КоличествоТребуетсяСвободныйОстатокПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение - КоличествоТребуетсяРезервПеремещение;
				
				СтруктураДляПоискаПеремещение.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя);
				
				МассивСтрокОстатковПеремещение = ТаблицаЗапасыОстаткиПеремещение.НайтиСтроки(СтруктураДляПоискаПеремещение);
				
				КоличествоОстатокПеремещение = 0;
				СуммаОстатокПеремещение = 0;
				
				Если МассивСтрокОстатковПеремещение.Количество() > 0 Тогда
					КоличествоОстатокПеремещение = МассивСтрокОстатковПеремещение[0].КоличествоОстаток;
					СуммаОстатокПеремещение = МассивСтрокОстатковПеремещение[0].СуммаОстаток;
				КонецЕсли;
				
				Если КоличествоОстатокПеремещение > 0 И КоличествоОстатокПеремещение > КоличествоТребуетсяРезервПеремещение Тогда
					
					СуммаКСписаниюПеремещение = Окр(СуммаОстатокПеремещение * КоличествоТребуетсяРезервПеремещение / КоличествоОстатокПеремещение , 2, 1);
					
					МассивСтрокОстатковПеремещение[0].КоличествоОстаток = МассивСтрокОстатковПеремещение[0].КоличествоОстаток - КоличествоТребуетсяРезервПеремещение;
					МассивСтрокОстатковПеремещение[0].СуммаОстаток = МассивСтрокОстатковПеремещение[0].СуммаОстаток - СуммаКСписаниюПеремещение;
					
				ИначеЕсли КоличествоОстатокПеремещение = КоличествоТребуетсяРезервПеремещение Тогда
					
					СуммаКСписаниюПеремещение = СуммаОстатокПеремещение;
					
					МассивСтрокОстатковПеремещение[0].КоличествоОстаток = 0;
					МассивСтрокОстатковПеремещение[0].СуммаОстаток = 0;
					
				Иначе
					СуммаКСписаниюПеремещение = 0;
				КонецЕсли;
				
				// Расход.
				СтрокаТаблицыРасходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПеремещение, СтрокаТаблицаЗапасыПеремещение);
				
				//--------------------------------------------------------------------------
				// © Шишка В.В. [23 октября 2023]  НАЧАЛО
				//СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено;
				//СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено;
				// © Шишка В.В. [23 октября 2023]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				СтрокаТаблицыРасходПеремещение.Сумма = СуммаКСписаниюПеремещение;
				СтрокаТаблицыРасходПеремещение.Количество = КоличествоТребуетсяРезервПеремещение;
				СтрокаТаблицыРасходПеремещение.Подразделение = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница; //+++ Дмгг 
				
				//Малышев 2024-06-24 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасходПеремещение);		
    			//Малышев 2024-06-24 ONESOFT-3273--)
								
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено ИЛИ СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено ИЛИ СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 	
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Сформируем проводки.
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 Тогда
					СтрокаТаблицаУправленческийПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческийПеремещение, СтрокаТаблицаЗапасыПеремещение);
					СтрокаТаблицаУправленческийПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение1(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СуммаКСписаниюПеремещение);
				КонецЕсли;
				
				// Приход.
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоТребуетсяРезервПеремещение > 0 Тогда
					
					СтрокаТаблицыПриходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриходПеремещение, СтрокаТаблицаЗапасыПеремещение);
					
					СтрокаТаблицыПриходПеремещение.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриходПеремещение.Организация = СтрокаТаблицаЗапасыПеремещение.Организация;
					СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.Номенклатура = СтрокаТаблицаЗапасыПеремещение.КоррНоменклатура;
					СтрокаТаблицыПриходПеремещение.Характеристика = СтрокаТаблицаЗапасыПеремещение.КоррХарактеристика;
					СтрокаТаблицыПриходПеремещение.Партия = СтрокаТаблицаЗапасыПеремещение.КоррПартия;
					//--------------------------------------------------------------------------
					// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено;
					СтрокаТаблицыПриходПеремещение.Спецификация = СтрокаТаблицаЗапасыПеремещение.КоррСпецификация;
					// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					СтрокаТаблицыПриходПеремещение.ЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчета = СтрокаТаблицаЗапасыПеремещение.СчетУчета;
					//+++
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриходПеремещение.КоррНоменклатура = СтрокаТаблицаЗапасыПеремещение.Номенклатура;
					СтрокаТаблицыПриходПеремещение.КоррХарактеристика = СтрокаТаблицаЗапасыПеремещение.Характеристика;
					СтрокаТаблицыПриходПеремещение.КоррПартия = СтрокаТаблицаЗапасыПеремещение.Партия;
					//--------------------------------------------------------------------------
					// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено;
					СтрокаТаблицыПриходПеремещение.КоррСпецификация = СтрокаТаблицаЗапасыПеремещение.Спецификация;
					// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//--------------------------------------------------------------------------
					СтрокаТаблицыПриходПеремещение.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя;
					
					//Малышев 2024-06-17 вернул обратно.
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриходПеремещение.Проект = СтрокаТаблицаЗапасыПеремещение.КоррПроект;
					СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
					//СтрокаТаблицыПриходПеремещение.Проект = ?(СтрокаТаблицаЗапасыПеремещение.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПеремещение.КоррПроект);//Lazare_05.02.2021
					//СтрокаТаблицыПриходПеремещение.КоррПроект = ?(СтрокаТаблицаЗапасыПеремещение.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПеремещение.Проект);//Lazare_05.02.2021
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриходПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					СтрокаТаблицыПриходПеремещение.Количество = КоличествоТребуетсяРезервПеремещение;
					
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл; //+++ Дмгг
					СтрокаТаблицыПриходПеремещение.Подразделение = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница; //+++ Дмгг  
					
					//Малышев 2024-06-24 ONESOFT-3273- чистим СчетУчетаЗатратРегл для склада  (--  
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриходПеремещение);		
    				//Малышев 2024-06-24 ONESOFT-3273--)
					
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = Неопределено
							ИЛИ СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = Неопределено
							ИЛИ СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 	
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
				КонецЕсли;
				
			КонецЕсли;
			
			Если КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
				
				СтруктураДляПоискаПеремещение.Вставить("ЗаказПокупателя", ПустойЗаказПокупателя);
				
				МассивСтрокОстатковПеремещение = ТаблицаЗапасыОстаткиПеремещение.НайтиСтроки(СтруктураДляПоискаПеремещение);
				
				
				//Малышев 2024-11-19 ONESOFT-3602
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатковПеремещение Цикл
			
					Если КоличествоТребуетсяСвободныйОстатокПеремещение <= 0 Тогда
						Прервать;
					КонецЕсли;	
 

				
				//КоличествоОстатокПеремещение = 0;
				//СуммаОстатокПеремещение = 0;
				//
				//Если МассивСтрокОстатковПеремещение.Количество() > 0 Тогда
				//	КоличествоОстатокПеремещение = МассивСтрокОстатковПеремещение[0].КоличествоОстаток;
				//	СуммаОстатокПеремещение = МассивСтрокОстатковПеремещение[0].СуммаОстаток;
				//КонецЕсли;
				
				КоличествоОстатокПеремещение = СтрокаТаблицыОстатков.КоличествоОстаток;
				СуммаОстатокПеремещение = СтрокаТаблицыОстатков.СуммаОстаток;
				
				СуммаКСписаниюПеремещение = 0;
				КоличествоКСписаниюПеремещение = 0;
				
				Если КоличествоОстатокПеремещение > 0 И КоличествоОстатокПеремещение > КоличествоТребуетсяСвободныйОстатокПеремещение Тогда
					
					КоличествоКСписанию = КоличествоТребуетсяСвободныйОстатокПеремещение; 
					СуммаКСписаниюПеремещение = Окр(СуммаОстатокПеремещение * КоличествоТребуетсяСвободныйОстатокПеремещение / КоличествоОстатокПеремещение , 2, 1);
					
					//МассивСтрокОстатковПеремещение[0].КоличествоОстаток = МассивСтрокОстатковПеремещение[0].КоличествоОстаток - КоличествоТребуетсяСвободныйОстатокПеремещение;
					СтрокаТаблицыОстатков.КоличествоОстаток = СтрокаТаблицыОстатков.КоличествоОстаток - СуммаКСписаниюПеремещение;
					//МассивСтрокОстатковПеремещение[0].СуммаОстаток = МассивСтрокОстатковПеремещение[0].СуммаОстаток - СуммаКСписаниюПеремещение;
					СтрокаТаблицыОстатков.СуммаОстаток = СтрокаТаблицыОстатков.СуммаОстаток - СуммаКСписаниюПеремещение;
					
					
				ИначеЕсли КоличествоОстатокПеремещение <= 0 Тогда  
					Продолжить;
				
				//ИначеЕсли КоличествоОстатокПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение Тогда
				ИначеЕсли КоличествоОстатокПеремещение >0 и КоличествоОстатокПеремещение <= КоличествоТребуетсяСвободныйОстатокПеремещение Тогда //Малышев 2024-11-19 ONESOFT-3602

	
					КоличествоКСписанию = КоличествоОстатокПеремещение; 
					СуммаКСписаниюПеремещение = СуммаОстатокПеремещение;
					
					//МассивСтрокОстатковПеремещение[0].КоличествоОстаток = 0;
					СтрокаТаблицыОстатков.КоличествоОстаток = 0;
					//МассивСтрокОстатковПеремещение[0].СуммаОстаток = 0;
					СтрокаТаблицыОстатков.СуммаОстаток = 0;
					
				Иначе
					СуммаКСписаниюПеремещение = 0;
				КонецЕсли;
				
				КоличествоТребуетсяСвободныйОстатокПеремещение = КоличествоТребуетсяСвободныйОстатокПеремещение - КоличествоКСписанию; //Малышев 2024-11-19 ONESOFT-3602
			
				
				// Расход.
				СтрокаТаблицыРасходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПеремещение, СтрокаТаблицаЗапасыПеремещение);
				
				//--------------------------------------------------------------------------
				// © Шишка В.В. [21 октября 2023]  НАЧАЛО
				//СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено;
				//СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено;
				// © Шишка В.В. [21 октября 2023]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				СтрокаТаблицыРасходПеремещение.Сумма = СуммаКСписаниюПеремещение;
				//СтрокаТаблицыРасходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
				СтрокаТаблицыРасходПеремещение.Количество = КоличествоКСписанию; //Малышев 2024-11-19 ONESOFT-3602
				
				////Малышев 2024-11-19 ONESOFT-3602
				//Если СтрокаТаблицыРасходПеремещение.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO 
				//	и СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
				//	Тогда
				//	СтрокаТаблицыРасходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыРасходПеремещение.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления; 
				КонецЕсли;
				Если СтрокаТаблицыРасходПеремещение.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасходПеремещение.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
				СтрокаТаблицыРасходПеремещение.ЗаказПокупателя = ПустойЗаказПокупателя;
				СтрокаТаблицыРасходПеремещение.Подразделение = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница; //+++ Дмгг
				
				//Малышев 2024-06-25    (-- 
				СтрокаТаблицыРасходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
				СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено; //2024-07-19
				СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено; //2024-07-19
				
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасходПеремещение);		
				//Малышев 2024-06-25   --)
				
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено
						ИЛИ СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено
						ИЛИ СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 	
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Сформируем проводки.
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 Тогда
					СтрокаТаблицаУправленческийПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческийПеремещение, СтрокаТаблицаЗапасыПеремещение);
					СтрокаТаблицаУправленческийПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					//Малышев 2024-06-25    (-- 
					//ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение2(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СуммаКСписаниюПеремещение);
					ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение2(СтруктураДополнительныеСвойства, СтрокаТаблицыРасходПеремещение, СуммаКСписаниюПеремещение);
					//Малышев 2024-06-25    --) 
					
				КонецЕсли;

				// Приход
				Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
					
					СтрокаТаблицыПриходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриходПеремещение, СтрокаТаблицаЗапасыПеремещение);
					
					СтрокаТаблицыПриходПеремещение.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриходПеремещение.Организация = СтрокаТаблицаЗапасыПеремещение.Организация;
					СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.Номенклатура = СтрокаТаблицаЗапасыПеремещение.КоррНоменклатура;
					СтрокаТаблицыПриходПеремещение.Характеристика = СтрокаТаблицаЗапасыПеремещение.КоррХарактеристика;
					СтрокаТаблицыПриходПеремещение.Партия = СтрокаТаблицаЗапасыПеремещение.КоррПартия;
					//--------------------------------------------------------------------------
					// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено;
					СтрокаТаблицыПриходПеремещение.Спецификация = СтрокаТаблицаЗапасыПеремещение.КоррСпецификация;
					// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					СтрокаТаблицыПриходПеремещение.ЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя;
					
					//СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчета = СтрокаТаблицаЗапасыПеремещение.СчетУчета;
					//+++
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;
					СтрокаТаблицыПриходПеремещение.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриходПеремещение.КоррНоменклатура = СтрокаТаблицаЗапасыПеремещение.Номенклатура;
					СтрокаТаблицыПриходПеремещение.КоррХарактеристика = СтрокаТаблицаЗапасыПеремещение.Характеристика;
					СтрокаТаблицыПриходПеремещение.КоррПартия = СтрокаТаблицаЗапасыПеремещение.Партия;
					//--------------------------------------------------------------------------
					// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено;
					СтрокаТаблицыПриходПеремещение.КоррСпецификация = СтрокаТаблицаЗапасыПеремещение.Спецификация;
					// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//--------------------------------------------------------------------------
					СтрокаТаблицыПриходПеремещение.КоррЗаказПокупателя = ПустойЗаказПокупателя;
					
					//Мадыщев 2024-06-17 вернул обратно
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					//СтрокаТаблицыПриходПеремещение.Проект = СтрокаТаблицаЗапасыПеремещение.КоррПроект;
					//СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
					//СтрокаТаблицыПриходПеремещение.Проект = ?(СтрокаТаблицаЗапасыПеремещение.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПеремещение.КоррПроект);//Lazare_05.02.2021
					//СтрокаТаблицыПриходПеремещение.КоррПроект = ?(СтрокаТаблицаЗапасыПеремещение.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПеремещение.Проект);//Lazare_05.02.2021
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриходПеремещение.Сумма = СуммаКСписаниюПеремещение;
					
					//СтрокаТаблицыПриходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
					СтрокаТаблицыПриходПеремещение.Количество = КоличествоКСписанию; //Малышев 202-11-19 ONESOFT-3602
					
					СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
					СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл; //+++ Дмгг
					СтрокаТаблицыПриходПеремещение.Подразделение = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница; //+++ Дмгг 

					//Малышев 2024-06-25    (--      
					СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено; //2046-07-19
					СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено; //2046-07-19
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриходПеремещение);		
					//Малышев 2024-06-25   --)
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					Если СтрокаТаблицыПриходПеремещение.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					Если СтрокаТаблицыПриходПеремещение.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриходПеремещение.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости

					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = Неопределено
							ИЛИ СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = Неопределено
							ИЛИ СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 	
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
				КонецЕсли;
				КонецЦикла; //по остаткам
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// делаем отрицательный остаток
				Если КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
					
					// Расход.
					СтрокаТаблицыРасходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПеремещение, СтрокаТаблицаЗапасыПеремещение);
					
					СтрокаТаблицыРасходПеремещение.Сумма = 0;
					
					СтрокаТаблицыРасходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					СтрокаТаблицыРасходПеремещение.ДокументПоступления = НЕОПРЕДЕЛЕНО; 
					СтрокаТаблицыРасходПеремещение.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					СтрокаТаблицыРасходПеремещение.ЗаказПокупателя = ПустойЗаказПокупателя;
					СтрокаТаблицыРасходПеремещение.Подразделение = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница; //+++ Дмгг
					
					//Малышев 2024-06-25    (-- 
					СтрокаТаблицыРасходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
					СтрокаТаблицыРасходПеремещение.Спецификация = Неопределено; //2024-07-19
					СтрокаТаблицыРасходПеремещение.КоррСпецификация = Неопределено; //2024-07-19
					
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасходПеремещение);		
					//Малышев 2024-06-25   --)
					
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено
							ИЛИ СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыРасходПеремещение.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено
							ИЛИ СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыРасходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 	
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					// Сформируем проводки.
					//Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческийПеремещение = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческийПеремещение, СтрокаТаблицаЗапасыПеремещение);
						СтрокаТаблицаУправленческийПеремещение.Сумма = 0;
						
						//Малышев 2024-06-25    (-- 
						//ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение2(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СуммаКСписаниюПеремещение);
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение2(СтруктураДополнительныеСвойства, СтрокаТаблицыРасходПеремещение, СуммаКСписаниюПеремещение);
						//Малышев 2024-06-25    --) 
						
					//КонецЕсли;
					
					// Приход
					//Если Окр(СуммаКСписаниюПеремещение, 2, 1) <> 0 ИЛИ КоличествоТребуетсяСвободныйОстатокПеремещение > 0 Тогда
						
						СтрокаТаблицыПриходПеремещение = ВременнаяТаблицаЗапасыПеремещение.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриходПеремещение, СтрокаТаблицаЗапасыПеремещение);
						
						СтрокаТаблицыПриходПеремещение.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриходПеремещение.Организация = СтрокаТаблицаЗапасыПеремещение.Организация;
						СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
						СтрокаТаблицыПриходПеремещение.Номенклатура = СтрокаТаблицаЗапасыПеремещение.КоррНоменклатура;
						СтрокаТаблицыПриходПеремещение.Характеристика = СтрокаТаблицаЗапасыПеремещение.КоррХарактеристика;
						СтрокаТаблицыПриходПеремещение.Партия = СтрокаТаблицаЗапасыПеремещение.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено;
						СтрокаТаблицыПриходПеремещение.Спецификация = СтрокаТаблицаЗапасыПеремещение.КоррСпецификация;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						СтрокаТаблицыПриходПеремещение.ДокументПоступления = НЕОПРЕДЕЛЕНО;
						СтрокаТаблицыПриходПеремещение.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						
						СтрокаТаблицыПриходПеремещение.ЗаказПокупателя = СтрокаТаблицаЗапасыПеремещение.ЗаказПокупателя;
						
						//СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПеремещение.СтруктурнаяЕдиница;
						СтрокаТаблицыПриходПеремещение.КоррСчетУчета = СтрокаТаблицаЗапасыПеремещение.СчетУчета;
						//+++
						СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл;
						СтрокаТаблицыПриходПеремещение.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.СчетУчетаРегл;
						//---
						СтрокаТаблицыПриходПеремещение.КоррНоменклатура = СтрокаТаблицаЗапасыПеремещение.Номенклатура;
						СтрокаТаблицыПриходПеремещение.КоррХарактеристика = СтрокаТаблицаЗапасыПеремещение.Характеристика;
						СтрокаТаблицыПриходПеремещение.КоррПартия = СтрокаТаблицаЗапасыПеремещение.Партия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено;
						СтрокаТаблицыПриходПеремещение.КоррСпецификация = СтрокаТаблицаЗапасыПеремещение.Спецификация;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыПриходПеремещение.КоррЗаказПокупателя = ПустойЗаказПокупателя;
						
						//Мадыщев 2024-06-17 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						//СтрокаТаблицыПриходПеремещение.Проект = СтрокаТаблицаЗапасыПеремещение.КоррПроект;
						//СтрокаТаблицыПриходПеремещение.КоррПроект = СтрокаТаблицаЗапасыПеремещение.Проект;
						//СтрокаТаблицыПриходПеремещение.Проект = ?(СтрокаТаблицаЗапасыПеремещение.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПеремещение.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыПриходПеремещение.КоррПроект = ?(СтрокаТаблицаЗапасыПеремещение.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасыПеремещение.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыПриходПеремещение.Сумма = 0;
						
						СтрокаТаблицыПриходПеремещение.Количество = КоличествоТребуетсяСвободныйОстатокПеремещение;
						
						СтрокаТаблицыПриходПеремещение.СчетУчета = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчета;
						СтрокаТаблицыПриходПеремещение.СчетУчетаРегл = СтрокаТаблицаЗапасыПеремещение.КоррСчетУчетаРегл; //+++ Дмгг
						СтрокаТаблицыПриходПеремещение.Подразделение = СтрокаТаблицаЗапасыПеремещение.КоррСтруктурнаяЕдиница; //+++ Дмгг 
						
						//Малышев 2024-06-25    (--      
						СтрокаТаблицыПриходПеремещение.Спецификация = Неопределено; //2046-07-19
						СтрокаТаблицыПриходПеремещение.КоррСпецификация = Неопределено; //2046-07-19
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриходПеремещение);		
						//Малышев 2024-06-25   --)
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						Если СтрокаТаблицыПриходПеремещение.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриходПеремещение.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
						КонецЕсли;
						Если СтрокаТаблицыПриходПеремещение.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриходПеремещение.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						
						//--------------------------------------------------------------------------
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = Неопределено
								ИЛИ СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриходПеремещение.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = Неопределено
								ИЛИ СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриходПеремещение.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 	
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						
					//КонецЕсли;				
				КонецЕсли;
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение = ВременнаяТаблицаЗапасыПеремещение;
		//Перенесем данные в таблицу для движения по регистру Запасы
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение[н];
			
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
			
			//Малышев 2024-06-25 
			ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
			
		КонецЦикла;
		
		//ВременнаяТаблицаЗапасыПеремещение.Индексы.Добавить("ВидДвижения,Организация,СтруктурнаяЕдиница,СчетУчета,Номенклатура,Характеристика,Партия,ЗаказПокупателя"); //+++ Дмгг
		ВременнаяТаблицаЗапасыПеремещение.Индексы.Добавить("ВидДвижения,Организация,СтруктурнаяЕдиница,СчетУчета,СчетУчетаРегл,Номенклатура,Характеристика,Партия,ЗаказПокупателя"); //+++ Дмгг
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы[н];
			//--------------------------------------------------------------------------
			// © Шишка В.В. [23 октября 2023]  НАЧАЛО
			Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Свойство("ТаблицаСоставПродукции") Тогда
				
				СтруктураПоскаКомплектующие = Новый Структура("Номенклатура, Характеристика, ТМНоменклатура, ТМХарактеристика, Спецификация"
											, СтрокаТаблицаЗапасы.Номенклатура
											, СтрокаТаблицаЗапасы.Характеристика
											, СтрокаТаблицаЗапасы.КоррНоменклатура
											, СтрокаТаблицаЗапасы.КоррХарактеристика
											, СтрокаТаблицаЗапасы.Спецификация);
				СтрокиСоставПродукции = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСоставПродукции.НайтиСтроки(СтруктураПоскаКомплектующие);
				Если СтрокиСоставПродукции.Количество() Тогда
				
					СтрокаТаблицаЗапасы.КоррСпецификация = СтрокаТаблицаЗапасы.Спецификация;	
				
				КонецЕсли;
				
			КонецЕсли;	
			// © Шишка В.В. [23 октября 2023]  КОНЕЦ
			//--------------------------------------------------------------------------
			
			СтруктураДляПоиска = Новый Структура;
			СтруктураДляПоиска.Вставить("ВидДвижения", ВидДвиженияНакопления.Приход);
			СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасы.Организация);
			//--------------------------------------------------------------------------
			// © Шишка В.В. [11 октября 2021]  НАЧАЛО
			//СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", СтрокаТаблицаЗапасы.СтруктурнаяЕдиница);
			СтруктураДляПоиска.Вставить("СтруктурнаяЕдиница", ?(УчетСебестоимостиВРазрезеОрганизации И (СтрокаТаблицаЗапасы.СтруктурнаяЕдиница = Неопределено ИЛИ
			СтрокаТаблицаЗапасы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад), Неопределено, СтрокаТаблицаЗапасы.СтруктурнаяЕдиница));
			// © Шишка В.В. [11 октября 2021]  КОНЕЦ
			//--------------------------------------------------------------------------
			СтруктураДляПоиска.Вставить("СчетУчета", СтрокаТаблицаЗапасы.СчетУчета);
			СтруктураДляПоиска.Вставить("СчетУчетаРегл", СтрокаТаблицаЗапасы.СчетУчетаРегл); //+++ Дмгг
			СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасы.Номенклатура);
			СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасы.Характеристика);
			СтруктураДляПоиска.Вставить("Партия", СтрокаТаблицаЗапасы.Партия);
			
			//Малышев 2024-06-25    (--
			Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда
				СтруктураДляПоиска.Вставить("Проект", СтрокаТаблицаЗапасы.Проект);
			КонецЕсли;	
			//Малышев 2024-06-25   --)
			
			//Малышев 2024-07-10
			СуммаДопРасходов = 0;
			
			ТребуетсяРезерв = СтрокаТаблицаЗапасы.Резерв;
			ТребуетсяКоличество = СтрокаТаблицаЗапасы.Количество;
			
			Если ТребуетсяРезерв > 0 Тогда
				
				ТребуетсяКоличество = ТребуетсяКоличество - ТребуетсяРезерв;
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
				
				МассивКоличествоОстаток = 0;
				МассивСуммаОстаток = 0;
				МассивСтрокОстатков = ВременнаяТаблицаЗапасыПеремещение.НайтиСтроки(СтруктураДляПоиска);
				Для каждого СтрокаОстатков Из МассивСтрокОстатков Цикл
					МассивКоличествоОстаток = МассивКоличествоОстаток + СтрокаОстатков.Количество;
					МассивСуммаОстаток = МассивСуммаОстаток + СтрокаОстатков.Сумма;
				КонецЦикла;
				
				КоличествоОстаток = 0;
				СуммаОстаток = 0;
				Если МассивСтрокОстатков.Количество() > 0 Тогда
					
					МассивСтрокОстатков[0].Количество = МассивКоличествоОстаток;
					МассивСтрокОстатков[0].Сумма = МассивСуммаОстаток;
					КоличествоОстаток = МассивСтрокОстатков[0].Количество;
					СуммаОстаток = МассивСтрокОстатков[0].Сумма;
					
					ТребуетсяСумма = Окр(МассивСтрокОстатков[0].Сумма * ТребуетсяРезерв / МассивСтрокОстатков[0].Количество,2,1);
					
				КонецЕсли;
				
				Если КоличествоОстаток > 0 И КоличествоОстаток > ТребуетсяРезерв Тогда
					
					СуммаКСписанию = Окр(СуммаОстаток * ТребуетсяРезерв / КоличествоОстаток , 2, 1);
					
					МассивСтрокОстатков[0].Количество = МассивСтрокОстатков[0].Количество - ТребуетсяРезерв;
					МассивСтрокОстатков[0].Сумма = МассивСтрокОстатков[0].Сумма - СуммаКСписанию;
					
				ИначеЕсли КоличествоОстаток = ТребуетсяРезерв Тогда
					
					СуммаКСписанию = СуммаОстаток;
					
				Иначе
					СуммаКСписанию = 0;
				КонецЕсли;
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Спишем запасы со склада (производственного подразделения).
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				СтрокаТаблицыРасход.Количество = ТребуетсяРезерв;
				СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 	
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Отнесем списанные запасы либо на стоимость запасов на складе либо на затраты НЗП.
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
					
					//Мадыщев 2024-06-17 вернул обратно
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
					//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;  //+++ Дмгг
					СтрокаТаблицыПриход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
					
					//Малышев 2024-06-25    (--
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
					//Малышев 2024-06-25   --)
					
					
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыПриход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 	
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					// Сформируем проводки.
					Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение3(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					КонецЕсли;
					
					// Автосписание запасов.
					Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции) Тогда
						
						// Расход.
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыРасход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыРасход.Спецификация = Неопределено;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						//+++
						СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						//---
						СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
						СтрокаТаблицыРасход.КоррСпецификация = СтрокаТаблицыРасход.Спецификация;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Мадыщев 2024-06-17 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыРасход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыРасход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						//Малышев 2024-06-25    (--
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
						//Малышев 2024-06-25   --)

						СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
						СтрокаТаблицыРасход.Количество = 0;
						СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
						
						// Приход.
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.Спецификация = Неопределено;
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыПриход.СчетДт = СтрокаТаблицаЗапасы.СчетДтПродукции;
						СтрокаТаблицыПриход.СчетДтРегл = СтрокаТаблицаЗапасы.СчетДтПродукцииРегл;
						
						СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						//+++
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						//---
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Мадыщев 2024-06-17 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						
						СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
						СтрокаТаблицыПриход.Количество = 0;
						
						СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						СтрокаТаблицыПриход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
						
						//Малышев 2024-06-25    (--
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
						//Малышев 2024-06-25   --)
						
						//--------------------------------------------------------------------------
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыПриход.СтруктурнаяЕдиница <> Неопределено
								И СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 	
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						
						// Сформируем проводки.
						Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
							СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
							
							ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение4(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						КонецЕсли;
						
					КонецЕсли;
					
				КонецЕсли;
				
			КонецЕсли;
			
			Если ТребуетсяКоличество > 0 Тогда
				
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
				
				//МассивКоличествоОстаток = 0;
				//МассивСуммаОстаток = 0;
				//МассивСтрокОстатков = ВременнаяТаблицаЗапасыПеремещение.НайтиСтроки(СтруктураДляПоиска);
				//Для каждого СтрокаОстатков Из МассивСтрокОстатков Цикл
				//	МассивКоличествоОстаток = МассивКоличествоОстаток + СтрокаОстатков.Количество;
				//	МассивСуммаОстаток = МассивСуммаОстаток + СтрокаОстатков.Сумма;
				//КонецЦикла;
				//
				//КоличествоОстаток = 0;
				//СуммаОстаток = 0;
				//Если МассивСтрокОстатков.Количество() > 0 Тогда
				//	
				//	МассивСтрокОстатков[0].Количество = МассивКоличествоОстаток;
				//	МассивСтрокОстатков[0].Сумма = МассивСуммаОстаток;
				//	КоличествоОстаток = МассивСтрокОстатков[0].Количество;
				//	СуммаОстаток = МассивСтрокОстатков[0].Сумма;
				//	
				//	ТребуетсяСумма = Окр(МассивСтрокОстатков[0].Сумма * ТребуетсяКоличество / МассивСтрокОстатков[0].Количество,2,1);
				//	
				//КонецЕсли;
				//
				//Если КоличествоОстаток > 0 И КоличествоОстаток > ТребуетсяКоличество Тогда
				//	
				//	СуммаКСписанию = Окр(СуммаОстаток * ТребуетсяКоличество / КоличествоОстаток , 2, 1);
				//	
				//	МассивСтрокОстатков[0].Количество = МассивСтрокОстатков[0].Количество - ТребуетсяКоличество;
				//	МассивСтрокОстатков[0].Сумма = МассивСтрокОстатков[0].Сумма - СуммаКСписанию;
				//	
				//ИначеЕсли КоличествоОстаток = ТребуетсяКоличество Тогда
				//	
				//	СуммаКСписанию = СуммаОстаток;
				//	
				//Иначе
				//	СуммаКСписанию = 0;
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				МассивСтрокОстатков = ВременнаяТаблицаЗапасыПеремещение.НайтиСтроки(СтруктураДляПоиска);
				
				Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
					
					КоличествоОстаток = СтрокаТаблицыОстатков.Количество;
					СуммаОстаток = СтрокаТаблицыОстатков.Сумма;
					
					СуммаКСписанию = 0;
					КоличествоКСписанию = 0;
					
					Если (КоличествоОстаток > 0) И (КоличествоОстаток > ТребуетсяКоличество) Тогда
						
						КоличествоКСписанию = ТребуетсяКоличество;
						СуммаКСписанию = Окр(СуммаОстаток * ТребуетсяКоличество / КоличествоОстаток , 2, 1);
						
						СтрокаТаблицыОстатков.Количество = СтрокаТаблицыОстатков.Количество - КоличествоКСписанию;
						СтрокаТаблицыОстатков.Сумма = СтрокаТаблицыОстатков.Сумма - СуммаКСписанию;
						
					ИначеЕсли (КоличествоОстаток > 0) И (КоличествоОстаток <= ТребуетсяКоличество) Тогда

		
						КоличествоКСписанию = КоличествоОстаток; 
						СуммаКСписанию = СуммаОстаток;
						
						СтрокаТаблицыОстатков.Количество = 0;
						СтрокаТаблицыОстатков.Сумма = 0;

					ИначеЕсли КоличествоОстаток <= 0 Тогда  
						
						Продолжить;	
						
					КонецЕсли;
				
					ТребуетсяКоличество = ТребуетсяКоличество - КоличествоКСписанию;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				СуммаСборки = СуммаСборки + СуммаКСписанию;
				
				// Расход.
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
				
				СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
				//СтрокаТаблицыРасход.Количество = ТребуетсяКоличество;
				СтрокаТаблицыРасход.Количество = КоличествоКСписанию; // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
				СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
				СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
				
				//СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект; //Малышев 2024-06-25  
				
				//Малышев 2024-06-25    (--
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);
				СтрокаТаблицыРасход.КоррСпецификация = Неопределено; //2024-07-19 
				//Малышев 2024-06-25   --)
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				КонецЕсли;
				Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
				//--------------------------------------------------------------------------
				// © Шишка В.В. [11 октября 2021]  НАЧАЛО
				Если УчетСебестоимостиВРазрезеОрганизации Тогда
					
					Если СтрокаТаблицыРасход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
					Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
						
					КонецЕсли; 	
					
				КонецЕсли; 	
				// © Шишка В.В. [11 октября 2021]  КОНЕЦ
				//--------------------------------------------------------------------------
				
				// Приход
				//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
				Если Окр(СуммаКСписанию, 2, 1) <> 0 ИЛИ (КоличествоКСписанию > 0) Тогда // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					
					СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//+++
					СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//---
					СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					
					//Мадыщев 2024-06-17 вернул обратно
					//ПодольскийАИ #УчетПроектов 13.10.2020 {
					//СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
					//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
					//ПодольскийАИ #УчетПроектов 13.10.2020 }
					СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					
					СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
					СтрокаТаблицыПриход.Количество = 0;
					
					////Малышев 2024-11-19 ONESOFT-3602
					//Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO 
					//	и СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
					//	Тогда
					//	СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
					//КонецЕсли;	
					
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
					КонецЕсли;
					Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
						СтрокаТаблицыПриход.КоррДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
					КонецЕсли;
					// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					
					СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					
					СтрокаТаблицыПриход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг 
					
					//Малышев 2024-06-25    (--
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
					СтрокаТаблицыПриход.КоррСпецификация = неопределено; //2024-07-19
					//Малышев 2024-06-25   --)

					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыПриход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 	
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					// Сформируем проводки.
					//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости, закоментировал
						СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
						
						ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение5(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//КонецЕсли;
					
				    // Автосписание запасов.
					Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции) Тогда
						
						// Расход.
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						СтрокаТаблицыРасход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыРасход.Спецификация = Неопределено;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						//+++
						СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						//----
						СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Мадыщев 2024-06-17 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						//СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						//СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыРасход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыРасход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.КоррПроект;
				
						СтрокаТаблицыРасход.Сумма = СуммаКСписанию;
						СтрокаТаблицыРасход.Количество = 0;
						СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг

						//Малышев 2024-06-25    (--
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);
						СтрокаТаблицыРасход.Спецификация = Неопределено; //2024-07-19
						СтрокаТаблицыРасход.КоррСпецификация = Неопределено;  //2024-07-19
						//Малышев 2024-06-25   --)
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости

						//--------------------------------------------------------------------------
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыРасход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 	
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						
						//Малышев 2024-07-10 ONESOFT-3310
						//сделаем записи по допрасходам, если они есть
						Если СуммаДопРасходов>0 Тогда
							//СтрокаТаблицыРасход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
							//ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход_ДопРасходы, СтрокаТаблицыРасход); 
							//СтрокаТаблицыРасход_ДопРасходы.Сумма = СуммаДопРасходов;  
							////СтрокаТаблицыРасход.Сумма = СтрокаТаблицыРасход.Сумма + СуммаДопРасходов;  
						КонецЕсли;

						// Приход.
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
						СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
						СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыПриход.Спецификация = Неопределено;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						СтрокаТаблицыПриход.СчетДт = СтрокаТаблицаЗапасы.СчетДтПродукции;
						СтрокаТаблицыПриход.СчетКт = СтрокаТаблицаЗапасы.СчетКтПродукции;
						
						СтрокаТаблицыПриход.СчетДтРегл = СтрокаТаблицаЗапасы.СчетДтПродукцииРегл;
						СтрокаТаблицыПриход.СчетКтРегл = СтрокаТаблицаЗапасы.СчетКтПродукцииРегл;
						
						СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
						СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
						//+++
						СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
						СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
						//---
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
						//--------------------------------------------------------------------------
						// © Шишка В.В. [23 октября 2023]  НАЧАЛО
						//СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
						// © Шишка В.В. [23 октября 2023]  КОНЕЦ
						//--------------------------------------------------------------------------
						СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
						
						//Малышев 2024-06-17 вернул обратно
						//ПодольскийАИ #УчетПроектов 13.10.2020 {
						//СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
						//СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
						//СтрокаТаблицыПриход.Проект = ?(СтрокаТаблицаЗапасы.КоррПроект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.КоррПроект);//Lazare_05.02.2021
						//СтрокаТаблицыПриход.КоррПроект = ?(СтрокаТаблицаЗапасы.Проект = Справочники.Проекты.ПустаяСсылка(),Справочники.Проекты.БезПроекта,СтрокаТаблицаЗапасы.Проект);//Lazare_05.02.2021
						//ПодольскийАИ #УчетПроектов 13.10.2020 }
						СтрокаТаблицыПриход.Проект = СтрокаТаблицыРасход.Проект;
						
						СтрокаТаблицыПриход.Сумма = СуммаКСписанию;
						СтрокаТаблицыПриход.Количество = 0;
						
						////Малышев 2024-11-19 ONESOFT-3602
						//Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO 
						//	и СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
						//	Тогда
						//	СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						//КонецЕсли;
						
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
						Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
							СтрокаТаблицыПриход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
						КонецЕсли;
						// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
						СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
						//СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
                        СтрокаТаблицыПриход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //Малышев  2024-07-10


						//Малышев 2024-06-25    (--
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);
						СтрокаТаблицыПриход.Спецификация = Неопределено;    //2024-07-19
						СтрокаТаблицыПриход.КоррСпецификация = Неопределено; //2024-07-19
						//Малышев 2024-06-25   --)

						//--------------------------------------------------------------------------
						// © Шишка В.В. [11 октября 2021]  НАЧАЛО
						Если УчетСебестоимостиВРазрезеОрганизации Тогда
							
							Если СтрокаТаблицыПриход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
							Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
								
								СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
								
							КонецЕсли; 	
							
						КонецЕсли; 	
						// © Шишка В.В. [11 октября 2021]  КОНЕЦ
						//--------------------------------------------------------------------------
						
						
						// Сформируем проводки.
						Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
							СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
							
							ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение6(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
						КонецЕсли; 
						
					КонецЕсли;
					
				КонецЕсли;
				
				КонецЦикла; // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// делаем отрицательный остаток
				Если ТребуетсяКоличество > 0 Тогда
					
					// Расход.
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
					
					СтрокаТаблицыРасход.Сумма = 0;
					СтрокаТаблицыРасход.Количество = ТребуетсяКоличество;
					СтрокаТаблицыРасход.ЗатратыНаВыпуск = Истина;
					СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПокупателя;
					СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
					
					//Малышев 2024-06-25    (--
					ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);
					СтрокаТаблицыРасход.КоррСпецификация = Неопределено; //2024-07-19 
					//Малышев 2024-06-25   --)
					
					СтрокаТаблицыРасход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					СтрокаТаблицыРасход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					
					//--------------------------------------------------------------------------
					// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					Если УчетСебестоимостиВРазрезеОрганизации Тогда
						
						Если СтрокаТаблицыРасход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
						Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
							
							СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
							
						КонецЕсли; 	
						
					КонецЕсли; 	
					// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//--------------------------------------------------------------------------
					
					//// Приход
					//Если Окр(СуммаКСписанию, 2, 1) <> 0 ИЛИ (КоличествоКСписанию > 0) Тогда // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	
					//	СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//	ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//	
					//	СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					//	
					//	СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//	СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//	СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//	СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//	СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//	СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//	
					//	СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					//	СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчета;
					//	//+++
					//	СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//	СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаРегл;
					//	//---
					//	СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.Номенклатура;
					//	СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.Характеристика;
					//	СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.Партия;
					//	СтрокаТаблицыПриход.КоррЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка();
					//	
					//	СтрокаТаблицыПриход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//	СтрокаТаблицыПриход.КоррПроект = СтрокаТаблицаЗапасы.Проект;
					//	
					//	СтрокаТаблицыПриход.Сумма = 0;
					//	СтрокаТаблицыПриход.Количество = 0;
					//	
					//	// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	СтрокаТаблицыПриход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					//	СтрокаТаблицыПриход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					//	// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	
					//	СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//	
					//	СтрокаТаблицыПриход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг 
					//	
					//	//Малышев 2024-06-25    (--
					//	ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
					//	СтрокаТаблицыПриход.КоррСпецификация = неопределено; //2024-07-19
					//	//Малышев 2024-06-25   --)
					//	
					//	//--------------------------------------------------------------------------
					//	// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					//	Если УчетСебестоимостиВРазрезеОрганизации Тогда
					//		
					//		Если СтрокаТаблицыПриход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					//			
					//			СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
					//			
					//		КонецЕсли; 	
					//		
					//		Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					//			
					//			СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
					//			
					//		КонецЕсли; 	
					//		
					//	КонецЕсли; 	
					//	// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//	//--------------------------------------------------------------------------
					//	
					//	// Сформируем проводки.
					//	//Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//	СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					//	ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
					//	
					//	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение5(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//	//КонецЕсли;
					//	
					//	// Автосписание запасов.
					//	Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции) Тогда
					//		
					//		// Расход.
					//		СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//		ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасы);
					//		
					//		СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//		СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//		СтрокаТаблицыРасход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыРасход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыРасход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//		//--------------------------------------------------------------------------
					//		// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//		//СтрокаТаблицыРасход.Спецификация = Неопределено;
					//		// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//		//--------------------------------------------------------------------------
					//		СтрокаТаблицыРасход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
					//		СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
					//		//+++
					//		СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//		СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
					//		//----
					//		СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
					//		//--------------------------------------------------------------------------
					//		// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//		//СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
					//		// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//		//--------------------------------------------------------------------------
					//		СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыРасход.Проект = СтрокаТаблицаЗапасы.КоррПроект;
					//		СтрокаТаблицыРасход.КоррПроект = СтрокаТаблицаЗапасы.КоррПроект;
					//		
					//		СтрокаТаблицыРасход.Сумма = 0;
					//		СтрокаТаблицыРасход.Количество = 0;
					//		СтрокаТаблицыРасход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //+++ Дмгг
					//		
					//		//Малышев 2024-06-25    (--
					//		ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);
					//		СтрокаТаблицыРасход.Спецификация = Неопределено; //2024-07-19
					//		СтрокаТаблицыРасход.КоррСпецификация = Неопределено;  //2024-07-19
					//		//Малышев 2024-06-25   --)
					//		
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		СтрокаТаблицыРасход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		СтрокаТаблицыРасход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		
					//		//--------------------------------------------------------------------------
					//		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					//		Если УчетСебестоимостиВРазрезеОрганизации Тогда
					//			
					//			Если СтрокаТаблицыРасход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					//				
					//				СтрокаТаблицыРасход.СтруктурнаяЕдиница = Неопределено;	
					//				
					//			КонецЕсли; 	
					//			
					//			Если СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					//				
					//				СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = Неопределено;	
					//				
					//			КонецЕсли; 	
					//			
					//		КонецЕсли; 	
					//		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//		//--------------------------------------------------------------------------
					//		
					//		// Приход.
					//		СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					//		ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//		
					//		СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					//		
					//		СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаПродукции;
					//		СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаПродукции;
					//		СтрокаТаблицыПриход.Номенклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыПриход.Характеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыПриход.Партия = СтрокаТаблицаЗапасы.КоррПартия;
					//		//--------------------------------------------------------------------------
					//		// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//		//СтрокаТаблицыПриход.Спецификация = Неопределено;
					//		// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//		//--------------------------------------------------------------------------
					//		СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыПриход.СчетДт = СтрокаТаблицаЗапасы.СчетДтПродукции;
					//		СтрокаТаблицыПриход.СчетКт = СтрокаТаблицаЗапасы.СчетКтПродукции;
					//		
					//		СтрокаТаблицыПриход.СчетДтРегл = СтрокаТаблицаЗапасы.СчетДтПродукцииРегл;
					//		СтрокаТаблицыПриход.СчетКтРегл = СтрокаТаблицаЗапасы.СчетКтПродукцииРегл;
					//		
					//		СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница;
					//		СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасы.КоррСчетУчета;
					//		//+++
					//		СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасы.СчетУчетаПродукцииРегл;
					//		СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасы.КоррСчетУчетаРегл;
					//		//---
					//		СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасы.КоррНоменклатура;
					//		СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасы.КоррХарактеристика;
					//		СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасы.КоррПартия;
					//		//--------------------------------------------------------------------------
					//		// © Шишка В.В. [23 октября 2023]  НАЧАЛО
					//		//СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
					//		// © Шишка В.В. [23 октября 2023]  КОНЕЦ
					//		//--------------------------------------------------------------------------
					//		СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасы.КоррЗаказПокупателя;
					//		
					//		СтрокаТаблицыПриход.Проект = СтрокаТаблицыРасход.Проект;
					//		
					//		СтрокаТаблицыПриход.Сумма = 0;
					//		СтрокаТаблицыПриход.Количество = 0;
					//		
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		СтрокаТаблицыПриход.ДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		СтрокаТаблицыПриход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
					//		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
					//		
					//		СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
					//		СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
					//		СтрокаТаблицыПриход.Подразделение = СтрокаТаблицаЗапасы.КоррСтруктурнаяЕдиница; //Малышев  2024-07-10
					//		
					//		
					//		//Малышев 2024-06-25    (--
					//		ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);
					//		СтрокаТаблицыПриход.Спецификация = Неопределено;    //2024-07-19
					//		СтрокаТаблицыПриход.КоррСпецификация = Неопределено; //2024-07-19
					//		//Малышев 2024-06-25   --)
					//		
					//		//--------------------------------------------------------------------------
					//		// © Шишка В.В. [11 октября 2021]  НАЧАЛО
					//		Если УчетСебестоимостиВРазрезеОрганизации Тогда
					//			
					//			Если СтрокаТаблицыПриход.СтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					//				
					//				СтрокаТаблицыПриход.СтруктурнаяЕдиница = Неопределено;	
					//				
					//			КонецЕсли; 	
					//			
					//			Если СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница <> Неопределено И СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
					//				
					//				СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = Неопределено;	
					//				
					//			КонецЕсли; 	
					//			
					//		КонецЕсли; 	
					//		// © Шишка В.В. [11 октября 2021]  КОНЕЦ
					//		//--------------------------------------------------------------------------
					//		
					//		
					//		// Сформируем проводки.
					//		Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					//			СтрокаТаблицаУправленческий = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
					//			ЗаполнитьЗначенияСвойств(СтрокаТаблицаУправленческий, СтрокаТаблицыПриход);
					//			
					//			ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_СборкаЗапасов_РазборкаПеремещение6(СтруктураДополнительныеСвойства, СтрокаТаблицаЗапасыПеремещение, СтрокаТаблицыПриход, СтрокаТаблицаЗапасы);
					//		КонецЕсли; 
					//		
					//	КонецЕсли;
					//	
					//КонецЕсли;
					
				КонецЕсли;	
				// Kuzmich, 22.06.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыЗапасы");
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыПеремещение");
		
	КонецПроцедуры // СформироватьТаблицаЗапасыЗапасыРазборкаПеремещение()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаПотребностьВЗапасахРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаПотребностьВЗапасах.Организация КАК Организация,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка) КАК ТипДвижения,
		|	ТаблицаПотребностьВЗапасах.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПотребностьВЗапасах.Номенклатура КАК Номенклатура,
		|	ТаблицаПотребностьВЗапасах.Характеристика КАК Характеристика
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаПотребностьВЗапасах";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.ПотребностьВЗапасах");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
		КонецЦикла;
		Блокировка.Заблокировать();
		
		// Получение остатков.
		Запрос.Текст = 	
		"ВЫБРАТЬ
		|	ПотребностьВЗапасахОстатки.Организация КАК Организация,
		|	ПотребностьВЗапасахОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ПотребностьВЗапасахОстатки.Номенклатура КАК Номенклатура,
		|	ПотребностьВЗапасахОстатки.Характеристика КАК Характеристика,
		|	СУММА(ПотребностьВЗапасахОстатки.Количество) КАК КоличествоОстаток
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПотребностьВЗапасахОстатки.Организация КАК Организация,
		|		ПотребностьВЗапасахОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|		ПотребностьВЗапасахОстатки.Номенклатура КАК Номенклатура,
		|		ПотребностьВЗапасахОстатки.Характеристика КАК Характеристика,
		|		СУММА(ПотребностьВЗапасахОстатки.КоличествоОстаток) КАК Количество
		|	ИЗ
		|		РегистрНакопления.ПотребностьВЗапасах.Остатки(
		|				&МоментКонтроля,
		|				(Организация, ТипДвижения, ЗаказПокупателя, Номенклатура, Характеристика) В
		|					(ВЫБРАТЬ
		|						ВременнаяТаблицаЗапасы.Организация КАК Организация,
		|						ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка) КАК ТипДвижения,
		|						ВременнаяТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|						ВременнаяТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|						ВременнаяТаблицаЗапасы.Характеристика КАК Характеристика
		|					ИЗ
		|						ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы)) КАК ПотребностьВЗапасахОстатки
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ПотребностьВЗапасахОстатки.Организация,
		|		ПотребностьВЗапасахОстатки.ЗаказПокупателя,
		|		ПотребностьВЗапасахОстатки.Номенклатура,
		|		ПотребностьВЗапасахОстатки.Характеристика
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ДвиженияДокументаПотребностьВЗапасах.Организация,
		|		ДвиженияДокументаПотребностьВЗапасах.ЗаказПокупателя,
		|		ДвиженияДокументаПотребностьВЗапасах.Номенклатура,
		|		ДвиженияДокументаПотребностьВЗапасах.Характеристика,
		|		ВЫБОР
		|			КОГДА ДвиженияДокументаПотребностьВЗапасах.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|				ТОГДА ЕСТЬNULL(ДвиженияДокументаПотребностьВЗапасах.Количество, 0)
		|			ИНАЧЕ -ЕСТЬNULL(ДвиженияДокументаПотребностьВЗапасах.Количество, 0)
		|		КОНЕЦ
		|	ИЗ
		|		РегистрНакопления.ПотребностьВЗапасах КАК ДвиженияДокументаПотребностьВЗапасах
		|	ГДЕ
		|		ДвиженияДокументаПотребностьВЗапасах.Регистратор = &Ссылка
		|		И ДвиженияДокументаПотребностьВЗапасах.Период <= &ПериодКонтроля) КАК ПотребностьВЗапасахОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	ПотребностьВЗапасахОстатки.Организация,
		|	ПотребностьВЗапасахОстатки.ЗаказПокупателя,
		|	ПотребностьВЗапасахОстатки.Номенклатура,
		|	ПотребностьВЗапасахОстатки.Характеристика";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		
		Если ЗначениеЗаполнено(ДокументСсылкаСборкаЗапасов.ЗаказПокупателя) Тогда
			Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Иначе
			Запрос.УстановитьПараметр("МоментКонтроля", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
		КонецЕсли;
		
		Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.ПериодКонтроля);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		ТаблицаПотребностьВЗапасахОстатки = РезультатЗапроса.Выгрузить();
		ТаблицаПотребностьВЗапасахОстатки.Индексы.Добавить("Организация,ЗаказПокупателя,Номенклатура,Характеристика");
		
		ВременнаяТаблицаПотребностьВЗапасах = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПотребностьВЗапасах.СкопироватьКолонки();
		
		Для каждого СтрокаТаблицыПотребностьВЗапасах Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПотребностьВЗапасах Цикл
			
			СтруктураДляПоиска = Новый Структура;
			СтруктураДляПоиска.Вставить("Организация", 		СтрокаТаблицыПотребностьВЗапасах.Организация);
			СтруктураДляПоиска.Вставить("ЗаказПокупателя", 	СтрокаТаблицыПотребностьВЗапасах.ЗаказПокупателя);
			СтруктураДляПоиска.Вставить("Номенклатура", 	СтрокаТаблицыПотребностьВЗапасах.Номенклатура);
			СтруктураДляПоиска.Вставить("Характеристика", 	СтрокаТаблицыПотребностьВЗапасах.Характеристика);
			
			МассивСтрокОстатков = ТаблицаПотребностьВЗапасахОстатки.НайтиСтроки(СтруктураДляПоиска);
			Если МассивСтрокОстатков.Количество() > 0 И МассивСтрокОстатков[0].КоличествоОстаток > 0 Тогда
				
				Если СтрокаТаблицыПотребностьВЗапасах.Количество > МассивСтрокОстатков[0].КоличествоОстаток Тогда
					СтрокаТаблицыПотребностьВЗапасах.Количество = МассивСтрокОстатков[0].КоличествоОстаток;
				КонецЕсли;
				
				СтрокаТаблицыРасход = ВременнаяТаблицаПотребностьВЗапасах.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицыПотребностьВЗапасах);
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПотребностьВЗапасах = ВременнаяТаблицаПотребностьВЗапасах;
		
	КонецПроцедуры // СформироватьТаблицаПотребностьВЗапасахРазборка()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицуРаспределенияМатериаловРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, ТаблицаПродукция) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.ПартияСтатус КАК ПартияСтатус,
		|	ТаблицаПродукция.Спецификация КАК Спецификация,
		
		//Малышев 2024-06-17 вернул обратно
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект КАК Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ КАК Проект,
		////Lazare_    
		
		|	ТаблицаПродукция.СчетУчета КАК СчетУчета,
		|	ТаблицаПродукция.СчетУчетаЗапасов КАК СчетУчетаЗапасов,
		|	ТаблицаПродукция.СчетКт КАК СчетКт,
		//+++
		|	ТаблицаПродукция.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,
		|	ТаблицаПродукция.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.СчетКтРегл КАК СчетКтРегл,
		//---
		|	ТаблицаПродукция.Количество КАК Количество,
		|	ТаблицаПродукция.Резерв КАК Резерв
		|ПОМЕСТИТЬ ВременнаяТаблицаТЗ
		|ИЗ
		|	&ТаблицаПродукция КАК ТаблицаПродукция
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаСоставПродукции.Номенклатура КАК Номенклатура,
		|	ТаблицаСоставПродукции.Характеристика КАК Характеристика,
		|	ТаблицаСоставПродукции.Партия КАК Партия,
		|	ТаблицаСоставПродукции.ПартияСтатус КАК ПартияСтатус,
		|	ТаблицаСоставПродукции.Спецификация КАК Спецификация,
		
		//Малышев 2024-06-17 вернул обратно
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаСоставПродукции.Проект КАК Проект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаСоставПродукции.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаСоставПродукции.Проект
		//|	КОНЕЦ КАК Проект,
		////Lazare_
		|	ТаблицаСоставПродукции.СчетУчета КАК СчетУчета,
		|	ТаблицаСоставПродукции.СчетУчетаЗапасов КАК СчетУчетаЗапасов,
		|	ТаблицаСоставПродукции.СчетКт КАК СчетКт,
		//+++
		|	ТаблицаСоставПродукции.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаСоставПродукции.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,
		|	ТаблицаСоставПродукции.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаСоставПродукции.СчетКтРегл КАК СчетКтРегл,
		//---
		|	ТаблицаСоставПродукции.Количество КАК Количество,
		|	ТаблицаСоставПродукции.Резерв КАК Резерв,
		|	ТаблицаМатериалы.ТипСтрокиСостава КАК ТМТипСтрокиСостава,
		//--------------------------------------------------------------------------
		// © Шишка В.В. [21 октября 2023]  НАЧАЛО
		//|	1 КАК КоррКоличество,
		//|	1 КАК ТМКоличество,
		|	ТаблицаМатериалы.Количество / ТаблицаМатериалы.КоличествоПродукции * ТаблицаСоставПродукции.Количество КАК КоррКоличество,
		|	ТаблицаМатериалы.Количество / ТаблицаМатериалы.КоличествоПродукции * ТаблицаСоставПродукции.Количество КАК ТМКоличество,
		// © Шишка В.В. [21 октября 2023]  КОНЕЦ
		//--------------------------------------------------------------------------
		|	ТаблицаМатериалы.Номенклатура КАК ТМНоменклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА ТаблицаМатериалы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК ТМХарактеристика,
		|	ТаблицаМатериалы.Спецификация КАК ТМСпецификация
		|ИЗ
		|	ВременнаяТаблицаТЗ КАК ТаблицаСоставПродукции
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Спецификации.Состав КАК ТаблицаМатериалы
		|		ПО ТаблицаСоставПродукции.Спецификация = ТаблицаМатериалы.Ссылка
		|			И (ТаблицаМатериалы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовЗапасы.КлючСвязи КАК КлючСвязи,
		|	СборкаЗапасовЗапасы.Ссылка КАК Ссылка,
		|	СборкаЗапасовЗапасы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовЗапасы.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК Ячейка,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов
		|	КОНЕЦ КАК СтруктурнаяЕдиницаЗапасов,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиницаЗапасовНаСклад,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов.ОрдерныйСклад КАК ОрдерныйСкладЗапасов,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции
		|	КОНЕЦ КАК СтруктурнаяЕдиницаПродукции,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ОрдерныйСклад КАК ОрдерныйСкладПродукции,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовЗапасы.Ссылка.ЯчейкаЗапасов
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК ЯчейкаЗапасов,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетУчета,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетУчетаЗапасов,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК КоррСчетУчета,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетУчетаПродукции,
		//+++
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетУчетаРегл,
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетУчетаЗапасовРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК КоррСчетУчетаРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетУчетаЗатратРегл,

		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетУчетаПродукцииРегл,
		//|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		//---
		|	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК Номенклатура,
		|	СборкаЗапасовЗапасы.Номенклатура КАК КоррНоменклатура,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовЗапасы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК КоррХарактеристика,
		|	ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка) КАК Партия,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ПустаяСсылка) КАК ПартияСтатус,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовЗапасы.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК КоррПартия,
		|	ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка) КАК Спецификация,
		|	СборкаЗапасовЗапасы.Спецификация КАК КоррСпецификация,
		
		//Малышев 2024-06-17 вернул обратно
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка) КАК Проект,
		//|	ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) КАК Проект, //++Lazare_05.02.2021
		|	СборкаЗапасовЗапасы.Проект КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА СборкаЗапасовЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ СборкаЗапасовЗапасы.Проект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_

		|	СборкаЗапасовЗапасы.Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
		|	0 КАК Количество,
		|	0 КАК Резерв,
		|	0 КАК Сумма,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетДт,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетДтПродукции,
		|	ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка) КАК СчетКт,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКтПродукции,
		//+++
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетДтРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетДтПродукцииРегл,
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКтРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКтПродукцииРегл,
		//---
		|	СборкаЗапасовЗапасы.ДоляСтоимости КАК ДоляСтоимости,
		|	ЛОЖЬ КАК НоваяСтрока,
		|	ЛОЖЬ КАК ВыполненРасчет,
		|	ЛОЖЬ КАК Распределено,
		//+Lazare 10.11.2022
		|   СборкаЗапасовЗапасы.СрокГодности КАК СрокГодности,
		|	СборкаЗапасовЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр
		//-Lazare 10.11.2022
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Ссылка = &Ссылка
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
		
		Запрос.УстановитьПараметр("ТаблицаПродукция", ТаблицаПродукция);
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);



		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		ТаблицаСоставПродукции = МассивРезультатов[1].Выгрузить();
		ТаблицаМатериалов = МассивРезультатов[2].Выгрузить();
		
		КоличествоПродукции = ТаблицаСоставПродукции.Количество();
		Инд = 0;
		Пока Инд < КоличествоПродукции Цикл
			СтрокаПродукции = ТаблицаСоставПродукции[Инд];
			Если СтрокаПродукции.ТМТипСтрокиСостава = Перечисления.ТипыСтрокСоставаСпецификации.Узел Тогда
				СтекСпецификацийУзлов = Новый Массив();
				ЗаполнитьТаблицуПродукцииСоставомУзлов(СтрокаПродукции, ТаблицаСоставПродукции, СтекСпецификацийУзлов);
				ТаблицаСоставПродукции.Удалить(СтрокаПродукции);
				Если Инд + 1 = ТаблицаСоставПродукции.Количество() Тогда
					Прервать;
				КонецЕсли;
			Иначе
				Инд = Инд + 1;
			КонецЕсли;
		КонецЦикла;
		
		//ТаблицаСоставПродукции.Свернуть("Номенклатура,Характеристика,Партия,ПартияСтатус,Спецификация,СчетУчета,СчетУчетаЗапасов,СчетКт,Количество,Резерв,ТМНоменклатура,ТМХарактеристика");
		//ПодольскийАИ #УчетПроектов добалено поле "Проект"
		//--------------------------------------------------------------------------
		// © Шишка В.В. [21 октября 2023]  НАЧАЛО
		//ТаблицаСоставПродукции.Свернуть("Номенклатура,Характеристика,Партия,ПартияСтатус,Спецификация,Проект,СчетУчета,СчетУчетаЗапасов,СчетКт,СчетУчетаРегл,СчетУчетаЗапасовРегл,СчетКтРегл,Количество,Резерв,ТМНоменклатура,ТМХарактеристика");
		ТаблицаСоставПродукции.Свернуть("Номенклатура,Характеристика,Партия,ПартияСтатус,Спецификация,Проект,СчетУчета,СчетУчетаЗапасов,СчетКт,СчетУчетаРегл,СчетУчетаЗапасовРегл,СчетКтРегл,Количество,Резерв,ТМНоменклатура,ТМХарактеристика", "КоррКоличество, ТМКоличество");


		// © Шишка В.В. [21 октября 2023]  КОНЕЦ
		//--------------------------------------------------------------------------
		ТаблицаСоставПродукции.Индексы.Добавить("Номенклатура,Характеристика,Партия,Спецификация");
		
		ТаблицаМатериалов.Индексы.Добавить("КоррНоменклатура,КоррХарактеристика");
		
		РаспределеноПродукции = 0;
		КоличествоМатериалов = ТаблицаМатериалов.Количество();
		КоличествоПродукции = ТаблицаСоставПродукции.Количество();
		Для каждого СтрокаПродукция Из ТаблицаПродукция Цикл
			
			СтруктураПоискаПродукция = Новый Структура;
			СтруктураПоискаПродукция.Вставить("Номенклатура", СтрокаПродукция.Номенклатура);
			СтруктураПоискаПродукция.Вставить("Характеристика", СтрокаПродукция.Характеристика);
			СтруктураПоискаПродукция.Вставить("Партия", СтрокаПродукция.Партия);
			СтруктураПоискаПродукция.Вставить("Спецификация", СтрокаПродукция.Спецификация);
			
			БазаДоляСтоимости = 0;
			РезультатПоискаПродукция = ТаблицаСоставПродукции.НайтиСтроки(СтруктураПоискаПродукция);
			Для каждого СтрокаПоискаПродукция Из РезультатПоискаПродукция Цикл
				
				СтруктураПоискаМатериалы = Новый Структура;
				СтруктураПоискаМатериалы.Вставить("НоваяСтрока", Ложь);
				СтруктураПоискаМатериалы.Вставить("КоррНоменклатура", СтрокаПоискаПродукция.ТМНоменклатура);
				СтруктураПоискаМатериалы.Вставить("КоррХарактеристика", СтрокаПоискаПродукция.ТМХарактеристика);
				
				РезультатПоискаМатериалы = ТаблицаМатериалов.НайтиСтроки(СтруктураПоискаМатериалы);
				КоличествоСоставМатериалы = РезультатПоискаМатериалы.Количество();
				Для каждого СтрокаПоискаМатериалы Из РезультатПоискаМатериалы Цикл
					СтрокаПродукция.Распределено = Истина;
					СтрокаПоискаМатериалы.Распределено = Истина;
					СтрокаПоискаМатериалы.ВыполненРасчет = Истина;
					БазаДоляСтоимости = БазаДоляСтоимости + СтрокаПоискаМатериалы.ДоляСтоимости;
				КонецЦикла;
				
			КонецЦикла;
			
			Если БазаДоляСтоимости > 0 Тогда
				РаспределитьПродукциюПоНормативам(СтрокаПродукция, ТаблицаМатериалов, БазаДоляСтоимости);
			КонецЕсли;
			
			Если СтрокаПродукция.Распределено Тогда
				РаспределеноПродукции = РаспределеноПродукции + 1;
			КонецЕсли;
			
		КонецЦикла;
		
		РаспределеноМатериалов = 0;
		Для каждого СтрокаМатериалы Из ТаблицаМатериалов Цикл
			Если СтрокаМатериалы.Распределено И НЕ СтрокаМатериалы.НоваяСтрока Тогда
				РаспределеноМатериалов = РаспределеноМатериалов + 1;
			КонецЕсли;
		КонецЦикла;
		
		Если РаспределеноПродукции < ТаблицаПродукция.Количество() Тогда
			Если РаспределеноМатериалов = КоличествоМатериалов Тогда
				БазаДоляСтоимости = ТаблицаМатериалов.Итог("ДоляСтоимости");
				РаспределитьПродукциюПоКоличеству(ТаблицаПродукция, ТаблицаМатериалов, БазаДоляСтоимости, Ложь);
			Иначе
				РаспределитьПродукциюПоКоличеству(ТаблицаПродукция, ТаблицаМатериалов);
			КонецЕсли;
		КонецЕсли;
		//--------------------------------------------------------------------------
		// © Шишка В.В. [21 октября 2023]  НАЧАЛО
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСоставПродукции", ТаблицаСоставПродукции);	
		// © Шишка В.В. [21 октября 2023]  КОНЕЦ
		//--------------------------------------------------------------------------
			
		ТаблицаПродукция = Неопределено;
		ТаблицаСоставПродукции = Неопределено;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРаспределенияМатериаловРазборка", ТаблицаМатериалов);
		ТаблицаМатериалов = Неопределено;
		
	КонецПроцедуры // СформироватьТаблицуРаспределенияМатериаловРазборка()
	
	// Инициализирует таблицы значений, содержащие данные табличных частей документа.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура ИнициализироватьДанныеПоЗапасамРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовЗапасы.Период КАК Период,
		|	&Организация КАК Организация,
		|	ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический) КАК СценарийПланирования,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовЗапасы.ОрдерныйСклад КАК ОрдерныйСклад,
		|	СборкаЗапасовЗапасы.Ячейка КАК Ячейка,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиницаЗапасов,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад КАК СтруктурнаяЕдиницаЗапасовНаСклад,
		|	СборкаЗапасовЗапасы.ОрдерныйСкладЗапасов КАК ОрдерныйСкладЗапасов,
		|	СборкаЗапасовЗапасы.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукции,
		|	СборкаЗапасовЗапасы.ОрдерныйСкладПродукции КАК ОрдерныйСкладПродукции,
		|	СборкаЗапасовЗапасы.ЯчейкаЗапасов КАК ЯчейкаЗапасов,
		|	СборкаЗапасовЗапасы.СчетУчета КАК СчетУчета,
		|	СборкаЗапасовЗапасы.СчетУчетаЗапасов КАК СчетУчетаЗапасов,
		|	СборкаЗапасовЗапасы.КоррСчетУчета КАК КоррСчетУчета,
		|	СборкаЗапасовЗапасы.СчетУчетаПродукции КАК СчетУчетаПродукции,
		//+++
		|	СборкаЗапасовЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|	СборкаЗапасовЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,
		|	СборкаЗапасовЗапасы.КоррСчетУчетаРегл КАК СчетУчетаЗатратРегл,
		|	СборкаЗапасовЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	СборкаЗапасовЗапасы.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		//---
		|	СборкаЗапасовЗапасы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовЗапасы.КоррНоменклатура КАК КоррНоменклатура,
		|	СборкаЗапасовЗапасы.Характеристика КАК Характеристика,
		|	СборкаЗапасовЗапасы.КоррХарактеристика КАК КоррХарактеристика,
		|	СборкаЗапасовЗапасы.Партия КАК Партия,
		|	СборкаЗапасовЗапасы.ПартияСтатус КАК ПартияСтатус,
		|	СборкаЗапасовЗапасы.КоррПартия КАК КоррПартия,
		|	СборкаЗапасовЗапасы.Спецификация КАК Спецификация,
		|	СборкаЗапасовЗапасы.КоррСпецификация КАК КоррСпецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		//|	СборкаЗапасовЗапасы.Проект КАК Проект,
		//|	СборкаЗапасовЗапасы.КоррПроект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		//_lazare
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		|		ИНАЧЕ СборкаЗапасовЗапасы.Проект
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		|		ИНАЧЕ СборкаЗапасовЗапасы.КоррПроект
		|	КОНЕЦ КАК КоррПроект,
		//Lazare_
		|	СборкаЗапасовЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	СборкаЗапасовЗапасы.Количество КАК Количество,
		|	СборкаЗапасовЗапасы.Резерв КАК Резерв,
		|	0 КАК Сумма,
		|	СборкаЗапасовЗапасы.СчетДт КАК СчетДт,
		|	СборкаЗапасовЗапасы.СчетДтПродукции КАК СчетДтПродукции,
		|	СборкаЗапасовЗапасы.СчетКт КАК СчетКт,
		|	СборкаЗапасовЗапасы.СчетКтПродукции КАК СчетКтПродукции,
		//+++
		|	СборкаЗапасовЗапасы.СчетДтРегл КАК СчетДтРегл,
		|	СборкаЗапасовЗапасы.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	СборкаЗапасовЗапасы.СчетКтРегл КАК СчетКтРегл,
		|	СборкаЗапасовЗапасы.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		//---
		|	СборкаЗапасовЗапасы.ДоляСтоимости КАК ДоляСтоимости,
		|	ВЫРАЗИТЬ(&РаспределениеЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки,
		|	ВЫРАЗИТЬ(&РаспределениеЗапасов КАК СТРОКА(100)) КАК Содержание,
		|	&ДатаОбновленияНаРелиз_1_2_1 КАК ДатаОбновленияНаРелиз_1_2_1,
		//+Lazare 10.11.2022
		|	СборкаЗапасовЗапасы.СрокГодности КАК СрокГодности,
		|	СборкаЗапасовЗапасы.ВключенВрегистр КАК ВключенВрегистр		
		//-Lazare 10.11.2022
		|ПОМЕСТИТЬ ВременнаяТаблицаЗапасы
		|ИЗ
		|	&ТаблицаРаспределенияМатериаловРазборка КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СценарийПланирования КАК СценарийПланирования,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукции,
		|	ТаблицаЗапасы.СчетУчета КАК СчетУчета,
		|	ТаблицаЗапасы.КоррСчетУчета КАК КоррСчетУчета,
		|	ТаблицаЗапасы.СчетУчетаПродукции КАК СчетУчетаПродукции,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаЗапасы.КоррСчетУчетаРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаЗапасы.КоррСчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		//---
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.КоррНоменклатура КАК КоррНоменклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.КоррХарактеристика КАК КоррХарактеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.КоррПартия КАК КоррПартия,
		|	ТаблицаЗапасы.Спецификация КАК Спецификация,
		|	ТаблицаЗапасы.КоррСпецификация КАК КоррСпецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ТаблицаЗапасы.КоррПроект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК КоррЗаказПокупателя,
		|	ТаблицаЗапасы.СодержаниеПроводки КАК СодержаниеПроводки,
		|	ЛОЖЬ КАК ЗатратыНаВыпуск,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество,
		|	СУММА(ТаблицаЗапасы.Резерв) КАК Резерв,
		|	0 КАК Сумма,
		|	ТаблицаЗапасы.СчетДт КАК СчетДт,
		|	ТаблицаЗапасы.СчетКт КАК СчетКт,
		|	ТаблицаЗапасы.СчетДтПродукции КАК СчетДтПродукции,
		|	ТаблицаЗапасы.СчетКтПродукции КАК СчетКтПродукции,
		//+++
		|	ТаблицаЗапасы.СчетДтРегл КАК СчетДтРегл,
		|	ТаблицаЗапасы.СчетКтРегл КАК СчетКтРегл,
		|	ТаблицаЗапасы.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	ТаблицаЗапасы.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		//---
		|	ТаблицаЗапасы.СодержаниеПроводки КАК Содержание,
		|	ТаблицаЗапасы.ДоляСтоимости КАК ДоляСтоимости
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СценарийПланирования,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаПродукции,
		|	ТаблицаЗапасы.СчетУчета,
		|	ТаблицаЗапасы.КоррСчетУчета,
		|	ТаблицаЗапасы.СчетУчетаПродукции,
		//+++
		|	ТаблицаЗапасы.СчетУчетаРегл,
		//|	ТаблицаЗапасы.СчетУчетаЗатратРегл,
		|	ТаблицаЗапасы.КоррСчетУчетаРегл,
		|	ТаблицаЗапасы.СчетУчетаПродукцииРегл,
		//---
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.КоррНоменклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.КоррХарактеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.КоррПартия,
		|	ТаблицаЗапасы.Спецификация,
		|	ТаблицаЗапасы.КоррСпецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.СчетДт,
		|	ТаблицаЗапасы.СчетКт,
		|	ТаблицаЗапасы.СчетДтПродукции,
		|	ТаблицаЗапасы.СчетКтПродукции,
		//+++
		|	ТаблицаЗапасы.СчетДтРегл,
		|	ТаблицаЗапасы.СчетКтРегл,
		|	ТаблицаЗапасы.СчетДтПродукцииРегл,
		|	ТаблицаЗапасы.СчетКтПродукцииРегл,
		//---
		|	ТаблицаЗапасы.СодержаниеПроводки,
		|	ТаблицаЗапасы.ДоляСтоимости,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.СодержаниеПроводки
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад КАК СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.ЯчейкаЗапасов КАК ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСкладПродукции КАК ОрдерныйСкладПродукции,
		|	ТаблицаЗапасы.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов КАК ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект КАК Проект,
		|	ТаблицаЗапасы.КоррПроект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_
		|	ТаблицаЗапасы.Ячейка КАК Ячейка,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.Период < ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСкладПродукции,
		|	ТаблицаЗапасы.ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаЗапасы.Ячейка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки),
		|	ТаблицаЗапасы.Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСкладПродукции,
		|	ТаблицаЗапасы.ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаЗапасы.Ячейка,
		|	СУММА(ТаблицаЗапасы.Количество)
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	НЕ ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.Период >= ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.ЯчейкаЗапасов,
		|	ТаблицаЗапасы.ОрдерныйСкладПродукции,
		|	ТаблицаЗапасы.ОрдерныйСклад,
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаЗапасы.Проект,
		|	ТаблицаЗапасы.КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаЗапасы.КоррПроект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаЗапасы.КоррПроект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаЗапасы.Ячейка
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ВЫБОР
		|		КОГДА ТаблицаЗапасы.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ТаблицаЗапасы.ЗаказПокупателя
		|	КОНЕЦ КАК Заказ,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыПриемаПередачиТоваров.ОтчетПоПереработке) КАК ТипПриемаПередачи,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ПартияСтатус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ВЫБОР
		|		КОГДА ТаблицаЗапасы.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ТаблицаЗапасы.ЗаказПокупателя
		|	КОНЕЦ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка) КАК ТипДвижения,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСкладЗапасов
		|	И ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад <> ТаблицаЗапасы.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	СУММА(ТаблицаЗапасы.Количество)
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.Период >= ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаЗапасы.Период КАК Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация КАК Организация,
		|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	СУММА(ТаблицаЗапасы.Количество) КАК Количество
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад <> ТаблицаЗапасы.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	СУММА(ТаблицаЗапасы.Количество)
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
		|ГДЕ
		|	ТаблицаЗапасы.ОрдерныйСклад
		|	И ТаблицаЗапасы.Период < ТаблицаЗапасы.ДатаОбновленияНаРелиз_1_2_1
		|	И ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасовНаСклад <> ТаблицаЗапасы.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаЗапасы.Период,
		|	ТаблицаЗапасы.СтруктурнаяЕдиница,
		|	ТаблицаЗапасы.Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);
		Запрос.УстановитьПараметр("ТаблицаРаспределенияМатериаловРазборка", СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРаспределенияМатериаловРазборка);
		
		// Временно: изменение движений по ордерному складу.
		ДатаОбновленияНаРелиз_1_2_1 = Константы.ДатаОбновленияНаРелиз_1_2_1.Получить();
		Запрос.УстановитьПараметр("ДатаОбновленияНаРелиз_1_2_1", ДатаОбновленияНаРелиз_1_2_1);
		
		Запрос.УстановитьПараметр("РаспределениеЗапасов", НСтр("ka='მარაგების გადანაწილება';ru='Распределение запасов';en='distribution of reserves'"));
		Запрос.УстановитьПараметр("ПеремещениеЗапасов", НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'"));
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		// Определим таблицу для учета запасов.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыЗапасы", МассивРезультатов[1].Выгрузить());
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Колонки.Добавить("ДокументПоступления");
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		
		// Сформируем таблицу для учета запасов.
		Если ЗначениеЗаполнено(ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаЗапасов) 
			И ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаЗапасов <> ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиница Тогда
			
			// Автоперемещение запасов.
			СформироватьТаблицаЗапасыЗапасыРазборкаПеремещение(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
			
		Иначе
			
			СформироватьТаблицаЗапасыЗапасыРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
			
		КонецЕсли;
		
		//Малышев 2024-07-12 ONESOFT-3310
		//сделаем записи по допрасходам, если они есть только по Подразделениям
		Если СтруктураДополнительныеСвойства.ДляПроведения.Свойство("СтруктураТаблицПоДопРасходам") Тогда 
			Если СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда
				МассивДопЗатратПроизводства = Новый Массив;
				
				//приход допрасходов в производство
				Для каждого СтрДопРасхода из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
					СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
					
					СтрокаТаблицыПриход_ДопРасходы.СчетУчетаРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗатратРегл;
					СтрокаТаблицыПриход_ДопРасходы.СчетУчетаЗатратРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗатратРегл;
					СтрокаТаблицыПриход_ДопРасходы.КоррПроект = СтрДопРасхода.Проект;
				
					МассивДопЗатратПроизводства.Добавить(СтрокаТаблицыПриход_ДопРасходы);
				КонецЦикла;
				
				//расход допрасходов в производство
				Для каждого СтрДопРасхода из МассивДопЗатратПроизводства Цикл
					СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
					СтрокаТаблицыПриход_ДопРасходы.ВидДвижения = ВидДвиженияНакопления.Расход;   
					СтрокаТаблицыПриход_ДопРасходы.КоррструктурнаяЕдиница = ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаПродукции;
					СтрокаТаблицыПриход_ДопРасходы.КоррНоменклатура = СтрокаТаблицыПриход_ДопРасходы.Номенклатура;
					СтрокаТаблицыПриход_ДопРасходы.КоррСчетУчетаРегл = СтрокаТаблицыПриход_ДопРасходы.Номенклатура.СчетУчетаЗапасовРегл;
				КонецЦикла;	
				
				//Приход допраасходов в стоимости итоговой продукции  2024-07-19
				Для каждого СтрДопРасхода из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
					СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	
					СтрокаТаблицыПриход_ДопРасходы.КоррПроект = СтрДопРасхода.Проект;
				    СтрокаТаблицыПриход_ДопРасходы.СтруктурнаяЕдиница = ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаПродукции;
					СтрокаТаблицыПриход_ДопРасходы.КоррструктурнаяЕдиница = СтрДопРасхода.СтруктурнаяЕдиница;
					СтрокаТаблицыПриход_ДопРасходы.КоррНоменклатура = СтрокаТаблицыПриход_ДопРасходы.Номенклатура;
				
					//МассивДопЗатратПроизводства.Добавить(СтрокаТаблицыПриход_ДопРасходы);
				КонецЦикла;
				
				
			Иначе
				//приход допрасходов в производство
				Для каждого СтрДопРасхода из СтруктураДополнительныеСвойства.ДляПроведения.СтруктураТаблицПоДопРасходам.ТаблицаЗапасы Цикл
					СтрокаТаблицыПриход_ДопРасходы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход_ДопРасходы, СтрДопРасхода);	

					СтрокаТаблицыПриход_ДопРасходы.КоррПроект = СтрДопРасхода.Проект;
				КонецЦикла;
				
			КонецЕсли;
			
		КонецЕсли;	
		
		
		
		// Дополним таблицу для запасов.
		ВыборкаРезультатов = МассивРезультатов[2].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			// Автоперемещение запасов.
			Если (ВыборкаРезультатов.СтруктурнаяЕдиницаЗапасов = ВыборкаРезультатов.СтруктурнаяЕдиница
				И ВыборкаРезультатов.ЯчейкаЗапасов <> ВыборкаРезультатов.Ячейка)
				ИЛИ ВыборкаРезультатов.СтруктурнаяЕдиницаЗапасов <> ВыборкаРезультатов.СтруктурнаяЕдиница Тогда
				
				// Расход.
				Если (ВыборкаРезультатов.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (ВыборкаРезультатов.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ ВыборкаРезультатов.ОрдерныйСкладЗапасов) Тогда
					
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, ВыборкаРезультатов);
					
					СтрокаТаблицыРасход.СтруктурнаяЕдиница = ВыборкаРезультатов.СтруктурнаяЕдиницаЗапасов;
					СтрокаТаблицыРасход.Ячейка = ВыборкаРезультатов.ЯчейкаЗапасов;
					
				КонецЕсли;
				
				// Приход.
				Если (ВыборкаРезультатов.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (ВыборкаРезультатов.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ ВыборкаРезультатов.ОрдерныйСклад) Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, ВыборкаРезультатов);
					
					СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
					
				КонецЕсли;
				
			КонецЕсли;
			
			СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, ВыборкаРезультатов);
			
		КонецЦикла;
		
		// Определим таблицу израсходованного давальческого сырья, принятого в переработку, за которые будет необходимо отчитаться перед давальцем.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПринятые", МассивРезультатов[3].Выгрузить());
		
		// Определим таблицу для движения по потребности позиций зависимого спроса.
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПотребностьВЗапасах", МассивРезультатов[4].Выгрузить());
		СформироватьТаблицаПотребностьВЗапасахРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Автоперемещение запасов (дополним таблицу ТаблицаЗапасыКРасходуСоСкладов).
		ВыборкаРезультатов = МассивРезультатов[5].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасходПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыКРасходуСоСкладов.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПриход, ВыборкаРезультатов);
			
		КонецЦикла;
		
		// Автоперемещение запасов (дополним таблицу ТаблицаЗапасыКПоступлениюНаСклады).
		ВыборкаРезультатов = МассивРезультатов[6].Выбрать();
		Пока ВыборкаРезультатов.Следующий() Цикл
			
			СтрокаТаблицыРасходПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыКПоступлениюНаСклады.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасходПриход, ВыборкаРезультатов);
			
		КонецЦикла;
		
	КонецПроцедуры // ИнициализироватьДанныеПоЗапасамРазборка()
	
	////////////////////////////////////////////////////////////////////////////////
	// ПРОДУКЦИЯ (РАЗБОРКА)
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаЗапасыПродукцияРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки)
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРазмещениеЗаказов.Индексы.Добавить("ВидДвижения,Организация,Номенклатура,Характеристика");;
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Индексы.Добавить("ВидДвижения,Организация,Номенклатура,Характеристика,Партия,КоррНоменклатура,КоррХарактеристика,КоррПартия,ЗатратыНаВыпуск");;
		
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасыПродукция = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция[н];
			
			// Сформируем выпуск продукции в количественном выражении. Если заказ покупателя указан - под заказ
			// покупателя, если нет - то под пустой заказ.
			СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);
			
			//Малышев 2024-06-25    (--
			ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
			//Малышев 2024-06-25   --)
			
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
				СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
			КонецЕсли;
			СтрокаТаблицыПриход.КоррДокументПоступления = НЕОПРЕДЕЛЕНО;
			// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
			
			// Автоперемещение продукции.
			ПеремещениеСчетУчета = Неопределено;
			ПеремещениеСчетУчетаРегл = Неопределено;
			Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
				
				// Расход.
				СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыПродукция);
				
				СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
				СтрокаТаблицыРасход.Спецификация = Неопределено;
				
				СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
				СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукции;
				
				//+++
				СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл;
				//---				

				СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
				СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
				СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
				СтрокаТаблицыРасход.КоррСпецификация = Неопределено;
				СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя;
				
				//Малышев 2024-06-25    (--
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
				//Малышев 2024-06-25   --)
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыРасход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				Если СтрокаТаблицыРасход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыРасход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		
				СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				СтрокаТаблицыРасход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");

				
				// Приход.
				СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);
				
				СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
				СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукции;
				СтрокаТаблицыПриход.Спецификация = Неопределено;
				
				ПеремещениеСчетУчета = СтрокаТаблицыПриход.СчетУчета;
				
				СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
				СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
				//+++
				СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл;
				СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
				//---
				
				СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
				СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
				СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
				СтрокаТаблицыПриход.КоррСпецификация = Неопределено;
				СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя;
				
				//Малышев 2024-06-25    (--
				ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
				//Малышев 2024-06-25   --)
				
				////Малышев 2024-11-19 ONESOFT-3602
				//Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO 
				//	и СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
				//	Тогда
				//	СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
				//КонецЕсли;
				
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
				Если СтрокаТаблицыПриход.Номенклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыПриход.ДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				Если СтрокаТаблицыПриход.КоррНоменклатура.МетодОценки = Перечисления.МетодОценкиЗапасов.FIFO Тогда
					СтрокаТаблицыПриход.КоррДокументПоступления = ДокументСсылкаСборкаЗапасов;
				КонецЕсли;
				// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости

				СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				СтрокаТаблицыПриход.Содержание = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов';en='Stock Transfer'");
				
 			КонецЕсли;
			
			// Если заказ на производство заполнен, а заказ покупателя нет, то необходимо проверить,
			// есть ли размещенные заказы покупателей в заказе на производство.
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя)
				И ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.ЗаказНаПроизводство) Тогда
				
				СтруктураДляПоиска = Новый Структура;
				СтруктураДляПоиска.Вставить("ВидДвижения", ВидДвиженияНакопления.Расход);
				СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасыПродукция.Организация);
				СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасыПродукция.Номенклатура);
				СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасыПродукция.Характеристика);
				
				Индекс = 0;
				КоличествоВыхода = СтрокаТаблицаЗапасыПродукция.Количество;
				МассивРазмещенияПродукции = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРазмещениеЗаказов.НайтиСтроки(СтруктураДляПоиска);
				
				Если МассивРазмещенияПродукции.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				СтруктураДляПоиска = Новый Структура;
				СтруктураДляПоиска.Вставить("ВидДвижения", ВидДвиженияНакопления.Приход);
				СтруктураДляПоиска.Вставить("Организация", СтрокаТаблицаЗапасыПродукция.Организация);
				СтруктураДляПоиска.Вставить("Номенклатура", СтрокаТаблицаЗапасыПродукция.Номенклатура);
				СтруктураДляПоиска.Вставить("Характеристика", СтрокаТаблицаЗапасыПродукция.Характеристика);
				СтруктураДляПоиска.Вставить("Партия", СтрокаТаблицаЗапасыПродукция.Партия);
				СтруктураДляПоиска.Вставить("ЗатратыНаВыпуск", Ложь);
				
				Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
					СтруктураДляПоиска.Вставить("КоррНоменклатура", СтрокаТаблицаЗапасыПродукция.Номенклатура);
					СтруктураДляПоиска.Вставить("КоррХарактеристика", СтрокаТаблицаЗапасыПродукция.Характеристика);
					СтруктураДляПоиска.Вставить("КоррПартия", СтрокаТаблицаЗапасыПродукция.Партия);
				КонецЕсли;
				
				СтоимостьВыхода = 0;
				МассивСтоимостиВыхода = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.НайтиСтроки(СтруктураДляПоиска);
				Для каждого СтрокаВыхода Из МассивСтоимостиВыхода Цикл
					СтоимостьВыхода = СтоимостьВыхода + СтрокаВыхода.Сумма;
				КонецЦикла;
				
				Для каждого СтрокаМассивРазмещения Из МассивРазмещенияПродукции Цикл
					
					КоличествоВыходаВРезерв = СтрокаМассивРазмещения.Количество;
					
					Если КоличествоВыхода = КоличествоВыходаВРезерв Тогда
						СтоимостьВыходаВРезерв = СтоимостьВыхода;
					Иначе
						СтоимостьВыходаВРезерв = Окр(СтоимостьВыхода * КоличествоВыходаВРезерв / КоличествоВыхода, 2, 1);
					КонецЕсли;
					
					Если КоличествоВыходаВРезерв > 0 Тогда
						
						ВсегоСуммаКСписаниюПоЗаказу = 0;
						
						СуммаКСписаниюПоЗаказу = Окр(СтоимостьВыходаВРезерв * СтрокаМассивРазмещения.Количество / КоличествоВыходаВРезерв, 2, 1);
						ВсегоСуммаКСписаниюПоЗаказу = ВсегоСуммаКСписаниюПоЗаказу + СуммаКСписаниюПоЗаказу;
						
						Если Индекс = МассивРазмещенияПродукции.Количество() - 1 Тогда // Это последняя строка, необходимо скорректировать сумму.
							СуммаКСписаниюПоЗаказу = СуммаКСписаниюПоЗаказу + (СтоимостьВыходаВРезерв - ВсегоСуммаКСписаниюПоЗаказу);
						КонецЕсли;
						
						//Расход -
						СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыПродукция);
						
						СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
						
						Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
							СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыРасход.СчетУчета = ПеремещениеСчетУчета;
							СтрокаТаблицыРасход.КоррСчетУчета = ПеремещениеСчетУчета;
							//+++
							СтрокаТаблицыРасход.СчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							СтрокаТаблицыРасход.КоррСчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							//---
						Иначе
							СтрокаТаблицыРасход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыРасход.СчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							СтрокаТаблицыРасход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							//+++
							СтрокаТаблицыРасход.СчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							СтрокаТаблицыРасход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							//---
						КонецЕсли;
						СтрокаТаблицыРасход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
						СтрокаТаблицыРасход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
						СтрокаТаблицыРасход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
						СтрокаТаблицыРасход.КоррСпецификация = СтрокаТаблицаЗапасыПродукция.Спецификация;
						СтрокаТаблицыРасход.КоррЗаказПокупателя = СтрокаМассивРазмещения.ЗаказПокупателя;
						СтрокаТаблицыРасход.Количество = СтрокаМассивРазмещения.Количество;
						СтрокаТаблицыРасход.Сумма = СуммаКСписаниюПоЗаказу;
						
						//Малышев 2024-06-25    (--
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыРасход);		
						//Малышев 2024-06-25   --)
		
						
						//Приход - 
						СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция);
						
						СтрокаТаблицыПриход.ВидДвижения = ВидДвиженияНакопления.Приход;
						
						СтрокаТаблицыПриход.ЗаказПокупателя = СтрокаМассивРазмещения.ЗаказПокупателя;
						
						Если ЗначениеЗаполнено(СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции) Тогда
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиницаПродукции;
							СтрокаТаблицыПриход.СчетУчета = ПеремещениеСчетУчета;
							СтрокаТаблицыПриход.КоррСчетУчета = ПеремещениеСчетУчета;
							//+++
							СтрокаТаблицыПриход.СчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							СтрокаТаблицыПриход.КоррСчетУчетаРегл = ПеремещениеСчетУчетаРегл;
							//---
						Иначе
							СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыПриход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасыПродукция.СтруктурнаяЕдиница;
							СтрокаТаблицыПриход.СчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							СтрокаТаблицыПриход.КоррСчетУчета = СтрокаТаблицаЗапасыПродукция.СчетУчета;
							//+++
							СтрокаТаблицыПриход.СчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							СтрокаТаблицыПриход.КоррСчетУчетаРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаРегл;
							//---
						КонецЕсли;
						СтрокаТаблицыПриход.КоррНоменклатура = СтрокаТаблицаЗапасыПродукция.Номенклатура;
						СтрокаТаблицыПриход.КоррХарактеристика = СтрокаТаблицаЗапасыПродукция.Характеристика;
						СтрокаТаблицыПриход.КоррПартия = СтрокаТаблицаЗапасыПродукция.Партия;
						СтрокаТаблицыПриход.КоррСпецификация = СтрокаТаблицаЗапасыПродукция.Спецификация;
						СтрокаТаблицыПриход.КоррЗаказПокупателя = СтрокаТаблицаЗапасыПродукция.ЗаказПокупателя;
						СтрокаТаблицыПриход.Количество = СтрокаМассивРазмещения.Количество;
						СтрокаТаблицыПриход.Сумма = СуммаКСписаниюПоЗаказу;
						
						//Малышев 2024-06-25    (--
						ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход);		
						//Малышев 2024-06-25   --)
						
                        Индекс = Индекс + 1;
						
					КонецЕсли;
					
				КонецЦикла;
				
			КонецЕсли;
			
		КонецЦикла;
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Удалить("ТаблицаЗапасыПродукция");
		ТаблицаРазмещенияПродукции = Неопределено;
		
	КонецПроцедуры // СформироватьТаблицаЗапасыПродукцияРазборка()
	
	// Формирует таблицу значений, содержащую данные для проведения по регистру.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура СформироватьТаблицаРазмещениеЗаказовРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		
		// Установка исключительной блокировки контролируемых размещений заказов.
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаПродукция.Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.ИсточникОбеспечения КАК ИсточникОбеспечения
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ИсточникОбеспечения <> НЕОПРЕДЕЛЕНО
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.ИсточникОбеспечения";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.РазмещениеЗаказов");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		
		Для каждого КолонкаРезультатЗапроса из РезультатЗапроса.Колонки Цикл
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных(КолонкаРезультатЗапроса.Имя, КолонкаРезультатЗапроса.Имя);
		КонецЦикла;
		Блокировка.Заблокировать();
		
		// Получение остатков.
		Запрос.Текст = 	
		"ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	РазмещениеЗаказовОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.ИсточникОбеспечения КАК ИсточникОбеспечения,
		|	ВЫБОР
		|		КОГДА ТаблицаПродукция.Количество > ЕСТЬNULL(РазмещениеЗаказовОстатки.Количество, 0)
		|			ТОГДА ЕСТЬNULL(РазмещениеЗаказовОстатки.Количество, 0)
		|		КОГДА ТаблицаПродукция.Количество <= ЕСТЬNULL(РазмещениеЗаказовОстатки.Количество, 0)
		|			ТОГДА ТаблицаПродукция.Количество
		|	КОНЕЦ КАК Количество
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			РазмещениеЗаказовОстатки.Организация КАК Организация,
		|			РазмещениеЗаказовОстатки.Номенклатура КАК Номенклатура,
		|			РазмещениеЗаказовОстатки.Характеристика КАК Характеристика,
		|			РазмещениеЗаказовОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|			РазмещениеЗаказовОстатки.ИсточникОбеспечения КАК ИсточникОбеспечения,
		|			СУММА(РазмещениеЗаказовОстатки.КоличествоОстаток) КАК Количество
		|		ИЗ
		|			(ВЫБРАТЬ
		|				РазмещениеЗаказовОстатки.Организация КАК Организация,
		|				РазмещениеЗаказовОстатки.Номенклатура КАК Номенклатура,
		|				РазмещениеЗаказовОстатки.Характеристика КАК Характеристика,
		|				РазмещениеЗаказовОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		|				РазмещениеЗаказовОстатки.ИсточникОбеспечения КАК ИсточникОбеспечения,
		|				РазмещениеЗаказовОстатки.КоличествоОстаток КАК КоличествоОстаток
		|			ИЗ
		|				РегистрНакопления.РазмещениеЗаказов.Остатки(
		|						&МоментКонтроля,
		|						(Организация, Номенклатура, Характеристика, ИсточникОбеспечения) В
		|							(ВЫБРАТЬ
		|								ТаблицаПродукция.Организация КАК Организация,
		|								ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|								ТаблицаПродукция.Характеристика КАК Характеристика,
		|								ТаблицаПродукция.ИсточникОбеспечения КАК ИсточникОбеспечения
		|							ИЗ
		|								ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|							ГДЕ
		|								ТаблицаПродукция.ИсточникОбеспечения <> НЕОПРЕДЕЛЕНО)) КАК РазмещениеЗаказовОстатки
		|			
		|			ОБЪЕДИНИТЬ ВСЕ
		|			
		|			ВЫБРАТЬ
		|				ДвиженияДокументаРазмещениеЗаказов.Организация,
		|				ДвиженияДокументаРазмещениеЗаказов.Номенклатура,
		|				ДвиженияДокументаРазмещениеЗаказов.Характеристика,
		|				ДвиженияДокументаРазмещениеЗаказов.ЗаказПокупателя,
		|				ДвиженияДокументаРазмещениеЗаказов.ИсточникОбеспечения,
		|				ВЫБОР
		|					КОГДА ДвиженияДокументаРазмещениеЗаказов.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|						ТОГДА ЕСТЬNULL(ДвиженияДокументаРазмещениеЗаказов.Количество, 0)
		|					ИНАЧЕ -ЕСТЬNULL(ДвиженияДокументаРазмещениеЗаказов.Количество, 0)
		|				КОНЕЦ
		|			ИЗ
		|				РегистрНакопления.РазмещениеЗаказов КАК ДвиженияДокументаРазмещениеЗаказов
		|			ГДЕ
		|				ДвиженияДокументаРазмещениеЗаказов.Регистратор = &Ссылка
		|				И ДвиженияДокументаРазмещениеЗаказов.Период <= &ПериодКонтроля) КАК РазмещениеЗаказовОстатки
		|		
		|		СГРУППИРОВАТЬ ПО
		|			РазмещениеЗаказовОстатки.Организация,
		|			РазмещениеЗаказовОстатки.Номенклатура,
		|			РазмещениеЗаказовОстатки.Характеристика,
		|			РазмещениеЗаказовОстатки.ЗаказПокупателя,
		|			РазмещениеЗаказовОстатки.ИсточникОбеспечения) КАК РазмещениеЗаказовОстатки
		|		ПО ТаблицаПродукция.Организация = РазмещениеЗаказовОстатки.Организация
		|			И ТаблицаПродукция.Номенклатура = РазмещениеЗаказовОстатки.Номенклатура
		|			И ТаблицаПродукция.Характеристика = РазмещениеЗаказовОстатки.Характеристика
		|			И ТаблицаПродукция.ИсточникОбеспечения = РазмещениеЗаказовОстатки.ИсточникОбеспечения
		|ГДЕ
		|	ТаблицаПродукция.ИсточникОбеспечения <> НЕОПРЕДЕЛЕНО
		|	И РазмещениеЗаказовОстатки.ЗаказПокупателя ЕСТЬ НЕ NULL ";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("МоментКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.ПериодКонтроля);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРазмещениеЗаказов", РезультатЗапроса.Выгрузить());
		
	КонецПроцедуры // СформироватьТаблицаРазмещениеЗаказовРазборка()
	
	////////////////////////////////////////////////////////////////////////////////
	// ИНИЦИАЛИЗАЦИЯ ДАННЫХ
	
	// Инициализирует таблицы значений, содержащие данные табличных частей документа.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура ИнициализироватьДанныеДокументаСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СборкаЗапасовПродукция.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовПродукция.Ссылка.Дата КАК Период,
		|	СборкаЗапасовПродукция.КлючСвязи КАК КлючСвязи,
		|	СборкаЗапасовПродукция.Ссылка КАК Ссылка,
		|	&Организация КАК Организация,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовПродукция.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК Ячейка,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции
		|	КОНЕЦ КАК СтруктурнаяЕдиницаПродукции,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукцииНаСклад,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции.ОрдерныйСклад КАК ОрдерныйСкладПродукции,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовПродукция.Ссылка.ЯчейкаПродукции
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК ЯчейкаПродукции,
		//|	НЕОПРЕДЕЛЕНО КАК ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	СборкаЗапасовПродукция.Ссылка КАК ДокументПоступления,
		|	СборкаЗапасовПродукция.Ссылка КАК КоррДокументПоступления,
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовПродукция.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовПродукция.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Партия,
		|	СборкаЗапасовПродукция.Спецификация КАК Спецификация,
   		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовПродукция.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовПродукция.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
		|	КОНЕЦ КАК Проект,
   		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовПродукция.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовПродукция.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
		|	КОНЕЦ КАК КоррПроект,
   		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетУчетаПродукции,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетДт,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетДтПродукции,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКтПродукции,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетУчетаРегл,
		|	СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетУчетаПродукцииРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетДтРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетДтПродукцииРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКтПродукцииРегл,
		|	СборкаЗапасовПродукция.Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК КоррЗаказПокупателя,
		|	СборкаЗапасовПродукция.Ссылка.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Ссылка.ЗаказНаПроизводство = ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ СборкаЗапасовПродукция.Ссылка.ЗаказНаПроизводство
		|	КОНЕЦ КАК ИсточникОбеспечения,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовПродукция.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовПродукция.Количество
		|		ИНАЧЕ СборкаЗапасовПродукция.Количество * СборкаЗапасовПродукция.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	0 КАК Сумма,
		|	ВЫРАЗИТЬ(&СборкаЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки,
		|	ВЫРАЗИТЬ(&СборкаЗапасов КАК СТРОКА(100)) КАК Содержание,
		|	&ДатаОбновленияНаРелиз_1_2_1 КАК ДатаОбновленияНаРелиз_1_2_1,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница КАК Подразделение,
		|	СборкаЗапасовПродукция.ДоляСтоимости КАК ДоляСтоимости,
		|	СборкаЗапасовПродукция.НомерСтроки КАК НомерСтроки1,
		|	СборкаЗапасовПродукция.СрокГодности КАК СрокГодности,
		|	СборкаЗапасовПродукция.Номенклатура.ВключенВрегистр КАК ВключенВрегистр
		|ПОМЕСТИТЬ ВременнаяТаблицаПродукция
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|ГДЕ
		|	СборкаЗапасовПродукция.Ссылка = &Ссылка
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 1
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаПродукция.НомерСтроки) КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	НЕОПРЕДЕЛЕНО КАК СценарийПланирования,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	НЕОПРЕДЕЛЕНО КАК КоррСтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукции,
		|	ТаблицаПродукция.СчетУчета КАК СчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции КАК СчетУчетаПродукции,
		|	НЕОПРЕДЕЛЕНО КАК КоррСчетУчета,
		|	ТаблицаПродукция.СчетУчетаРегл КАК СчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		|	НЕОПРЕДЕЛЕНО КАК КоррСчетУчетаРегл,
		|	НЕОПРЕДЕЛЕНО КАК СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	НЕОПРЕДЕЛЕНО КАК ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
		|	НЕОПРЕДЕЛЕНО КАК КоррДокументПоступления, // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	НЕОПРЕДЕЛЕНО КАК КоррНоменклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	НЕОПРЕДЕЛЕНО КАК КоррХарактеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	НЕОПРЕДЕЛЕНО КАК КоррПартия,
		|	ТаблицаПродукция.Спецификация КАК Спецификация,
		|	НЕОПРЕДЕЛЕНО КАК КоррСпецификация,
		|	ТаблицаПродукция.Проект КАК Проект,
		|	ТаблицаПродукция.КоррПроект КАК КоррПроект,
		|	ТаблицаПродукция.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПродукция.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ТаблицаПродукция.КоррЗаказПокупателя КАК КоррЗаказПокупателя,
		|	НЕОПРЕДЕЛЕНО КАК СчетДт,
		|	НЕОПРЕДЕЛЕНО КАК СчетКт,
		|	НЕОПРЕДЕЛЕНО КАК СчетДтРегл,
		|	НЕОПРЕДЕЛЕНО КАК СчетКтРегл,
		|	НЕОПРЕДЕЛЕНО КАК Содержание,
		|	ТаблицаПродукция.СодержаниеПроводки КАК СодержаниеПроводки,
		|	ЛОЖЬ КАК ЗатратыНаВыпуск,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	СУММА(ТаблицаПродукция.Сумма) КАК Сумма,
		|	ЛОЖЬ КАК ФиксированнаяСтоимость,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукции,
		|	ТаблицаПродукция.СчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции,
		|	ТаблицаПродукция.СчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Спецификация,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект,
		|	ТаблицаПродукция.ЗаказПокупателя,
		|	ТаблицаПродукция.ЗаказНаПроизводство,
		|	ТаблицаПродукция.КоррЗаказПокупателя,
		|	ТаблицаПродукция.СодержаниеПроводки,
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 2
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Ячейка КАК Ячейка,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад КАК СтруктурнаяЕдиницаПродукции,
		|	ТаблицаПродукция.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции КАК ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.ЯчейкаПродукции КАК ЯчейкаПродукции,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.Проект КАК Проект,
		|	ТаблицаПродукция.КоррПроект КАК КоррПроект,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления КАК ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.Период < ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект,
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаПродукция.Период,
		|	&Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект,
		|	СУММА(ТаблицаПродукция.Количество),
		|	NULL
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления КАК ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	НЕ ТаблицаПродукция.ОрдерныйСклад
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаПродукция.Период,
		|	&Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект,
		|	СУММА(ТаблицаПродукция.Количество),
		|	NULL
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления КАК ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСклад
		|	И НЕ ТаблицаПродукция.ОрдерныйСкладПродукции
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 3
		|ВЫБРАТЬ
		|	ТаблицаПродукция.Период КАК Период,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.Спецификация КАК Спецификация,
		|	ТаблицаПродукция.Проект КАК Проект,
		|	ТаблицаПродукция.КоррПроект КАК КоррПроект,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.ЗаказПокупателя,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Спецификация,
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.КоррПроект,
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 4
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.ЗаказНаПроизводство,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 5
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.Проект КАК Проект,  //Малышев 2024-06-23 
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСклад
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,  //Малышев 2024-06-23 
		|	ТаблицаПродукция.Подразделение
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект КАК Проект,  //Малышев 2024-06-23 
		|	СУММА(ТаблицаПродукция.Количество),
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСкладПродукции
		|	И ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад <> ТаблицаПродукция.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,  //Малышев 2024-06-23 
		|	ТаблицаПродукция.Подразделение
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект КАК Проект,  //Малышев 2024-06-23 
		|	СУММА(ТаблицаПродукция.Количество),
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСкладПродукции
		|	И ТаблицаПродукция.Период < ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|	И ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад <> ТаблицаПродукция.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,  //Малышев 2024-06-23 
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 6
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.Проект КАК Проект,  //Малышев 2024-06-23 
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСклад
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|	И ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад <> ТаблицаПродукция.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,  //Малышев 2024-06-23 
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 7
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаПродукция.НомерСтроки) КАК КоррНомерСтроки,
		|	ТаблицаПродукция.Номенклатура КАК КоррНоменклатура,
		|	ТаблицаПродукция.Характеристика КАК КоррХарактеристика,
		|	ТаблицаПродукция.Партия КАК КоррПартия,
		|	ТаблицаПродукция.Спецификация КАК КоррСпецификация,
		|	ТаблицаПродукция.КоррПроект КАК КоррПроект,
		|	ТаблицаПродукция.СчетУчета КАК КоррСчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции КАК СчетУчетаПродукции,
		|	ТаблицаПродукция.СчетДт КАК СчетДт,
		|	ТаблицаПродукция.СчетДтПродукции КАК СчетДтПродукции,
		|	ТаблицаПродукция.СчетКтПродукции КАК СчетКтПродукции,
		|	ТаблицаПродукция.СчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаРегл КАК СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		|	ТаблицаПродукция.СчетДтРегл КАК СчетДтРегл,
		|	ТаблицаПродукция.СчетДтПродукцииРегл КАК СчетДтПродукцииРегл,
		|	ТаблицаПродукция.СчетКтПродукцииРегл КАК СчетКтПродукцииРегл,
		|	СУММА(ВЫБОР
		|			КОГДА &РаспределитьПоДолеСтоимости
		|				ТОГДА ТаблицаПродукция.ДоляСтоимости
		|			ИНАЧЕ ТаблицаПродукция.Количество
		|		КОНЕЦ) КАК КоррКоличество,
		|	ЛОЖЬ КАК Распределено,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.Ссылка КАК ДокументПоступления
		|	,ТаблицаПродукция.Ссылка КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Спецификация,
		|	ТаблицаПродукция.КоррПроект,
		|	ТаблицаПродукция.СчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции,
		|	ТаблицаПродукция.СчетДт,
		|	ТаблицаПродукция.СчетДтПродукции,
		|	ТаблицаПродукция.СчетКтПродукции,
		|	ТаблицаПродукция.СчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл,
		|	ТаблицаПродукция.СчетДтРегл,
		|	ТаблицаПродукция.СчетДтПродукцииРегл,
		|	ТаблицаПродукция.СчетКтПродукцииРегл,
		|	ТаблицаПродукция.Подразделение,
		|	ТаблицаПродукция.СчетУчетаРегл
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.Ссылка
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|
		|УПОРЯДОЧИТЬ ПО
		|	КоррНомерСтроки
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 8
		|ВЫБРАТЬ
		|	ТаблицаПродукция.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК ДатаСобытия,
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииСерийныхНомеров.Приход) КАК Операция,
		|	ТаблицаСерийныеНомераПродукция.СерийныйНомер КАК СерийныйНомер,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.Проект КАК Проект,  //Малышев 2024-06-23 
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.ЯчейкаПродукции КАК Ячейка,
		|	1 КАК Количество,
		|	ТаблицаПродукция.Подразделение КАК Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.СерийныеНомераПродукция КАК ТаблицаСерийныеНомераПродукция
		|		ПО ТаблицаПродукция.Ссылка = ТаблицаСерийныеНомераПродукция.Ссылка
		|			И ТаблицаПродукция.КлючСвязи = ТаблицаСерийныеНомераПродукция.КлючСвязи
		|ГДЕ
		|	ТаблицаСерийныеНомераПродукция.Ссылка = &Ссылка
		|	И &ИспользоватьСерийныеНомера
		|	И НЕ ТаблицаПродукция.ОрдерныйСкладПродукции
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТаблицаЗапасы.Ссылка.Дата,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаЗапасы.Ссылка.Дата,
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииСерийныхНомеров.Расход),
		|	ТаблицаСерийныеНомера.СерийныйНомер,
		|	&Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Проект,  //Малышев 2024-06-23 
		|	ТаблицаЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.Ссылка.ЯчейкаЗапасов,
		|	1,
		|	ТаблицаЗапасы.Ссылка.СтруктурнаяЕдиница
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК ТаблицаЗапасы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.СерийныеНомера КАК ТаблицаСерийныеНомера
		|		ПО ТаблицаЗапасы.Ссылка = ТаблицаСерийныеНомера.Ссылка
		|			И ТаблицаЗапасы.КлючСвязи = ТаблицаСерийныеНомера.КлючСвязи
		|ГДЕ
		|	ТаблицаСерийныеНомера.Ссылка = &Ссылка
		|	И &ИспользоватьСерийныеНомера
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 9
		|ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.Ссылка КАК Ссылка,
		|	СборкаЗапасовЗапасы.Номенклатура КАК СоответствующаяНоменклатура,
		|	СборкаЗапасовЗапасы.Характеристика КАК ХарактеристикаСоответствующийНоменклатури,
		|	СборкаЗапасовЗапасы.Партия КАК ПартияСоответствующийНоменклатури,
		|	СУММА(СборкаЗапасовЗапасы.Количество) КАК КоличестваПотраченныхЗапасов,
		|	СборкаЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовЗапасы.Ссылка.Метка КАК Метка
		|ПОМЕСТИТЬ Вт_Запасы
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	СборкаЗапасовЗапасы.Характеристика,
		|	СборкаЗапасовЗапасы.Партия,
		|	СборкаЗапасовЗапасы.Номенклатура,
		|	СборкаЗапасовЗапасы.Количество,
		|	СборкаЗапасовЗапасы.Ссылка,
		|	СборкаЗапасовЗапасы.НомерСтроки,
		|	СборкаЗапасовЗапасы.Ссылка.Метка
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 10
		|ВЫБРАТЬ
		|	СУММА(СборкаЗапасовЗапасы.Количество) КАК КоличестваПотраченныхЗапасов
		|ПОМЕСТИТЬ Вт_Коэфициенты
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Ссылка = &Ссылка
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 11
		|ВЫБРАТЬ
		|	Вт_Запасы.Ссылка КАК Ссылка,
		|	ВременнаяТаблицаПродукция.Организация КАК Организация,
		|	Вт_Запасы.СоответствующаяНоменклатура КАК СоответствующаяНоменклатура,
		|	Вт_Запасы.ХарактеристикаСоответствующийНоменклатури КАК ХарактеристикаСоответствующийНоменклатури,
		|	Вт_Запасы.ПартияСоответствующийНоменклатури КАК ПартияСоответствующийНоменклатури,
		|	СУММА(Вт_Запасы.КоличестваПотраченныхЗапасов) КАК КоличестваПотраченныхЗапасов,
		|	ВременнаяТаблицаПродукция.Ссылка.ТыпОперации КАК ТыпОперации,
		|	ВременнаяТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ВременнаяТаблицаПродукция.Характеристика КАК Характеристика,
		|	ВременнаяТаблицаПродукция.Партия КАК Партия,
		|	СУММА(ВременнаяТаблицаПродукция.Количество) КАК ПолученноеКоличествоЗапасов,
		|	Вт_Запасы.КоличестваПотраченныхЗапасов / Вт_Коэфициенты.КоличестваПотраченныхЗапасов КАК Коэфициент,
		|	Вт_Запасы.КоличестваПотраченныхЗапасов - ВременнаяТаблицаПродукция.Количество КАК КоличествоПотерь,
		|	Вт_Запасы.Ссылка.Дата КАК Период,
		|	Вт_Запасы.Метка КАК Метка
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ВременнаяТаблицаПродукция
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Вт_Запасы КАК Вт_Запасы
		|		ПО ВременнаяТаблицаПродукция.НомерСтроки = Вт_Запасы.НомерСтроки,
		|	Вт_Коэфициенты КАК Вт_Коэфициенты
		|
		|СГРУППИРОВАТЬ ПО
		|	Вт_Запасы.Ссылка,
		|	Вт_Запасы.СоответствующаяНоменклатура,
		|	Вт_Запасы.ХарактеристикаСоответствующийНоменклатури,
		|	Вт_Запасы.ПартияСоответствующийНоменклатури,
		|	ВременнаяТаблицаПродукция.Номенклатура,
		|	ВременнаяТаблицаПродукция.Партия,
		|	ВременнаяТаблицаПродукция.Характеристика,
		|	ВременнаяТаблицаПродукция.Ссылка.ТыпОперации,
		|	ВременнаяТаблицаПродукция.Организация,
		|	Вт_Запасы.КоличестваПотраченныхЗапасов / Вт_Коэфициенты.КоличестваПотраченныхЗапасов,
		|	Вт_Запасы.КоличестваПотраченныхЗапасов - ВременнаяТаблицаПродукция.Количество,
		|	Вт_Запасы.Ссылка.Дата,
		|	Вт_Запасы.Метка";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии",  СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам",  СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);
		Запрос.УстановитьПараметр("ИспользоватьТехоперации",  СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьТехоперации);
		
		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
		//ПодольскийАИ #УчетПроектов 13.10.2020 }
		
		Запрос.УстановитьПараметр("ИспользоватьСерийныеНомера", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьСерийныеНомера);
		Запрос.УстановитьПараметр("РаспределитьПоДолеСтоимости",Константы.РаспределитьМатериалыПоДолеСтоимости.Получить());
		
		// Временно: изменение движений по ордерному складу.
		ДатаОбновленияНаРелиз_1_2_1 = Константы.ДатаОбновленияНаРелиз_1_2_1.Получить();
		Запрос.УстановитьПараметр("ДатаОбновленияНаРелиз_1_2_1", ДатаОбновленияНаРелиз_1_2_1);
		
		Запрос.УстановитьПараметр("СборкаЗапасов", НСтр("ka='მარაგების შეკრება';ru='Сборка запасов';en='stock assembly'"));
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПродукция", МассивРезультатов[1].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасы", СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция.СкопироватьКолонки());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыНаСкладах", МассивРезультатов[2].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВыпускПродукции", МассивРезультатов[3].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗаказыНаПроизводство", МассивРезультатов[4].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ПроизводственныеПотери", МассивРезультатов[11].Выгрузить()); //++Lazare 20.09.2021
		
		//+++ Дмгг
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВариантыПроизводстваПродукция", Новый ТаблицаЗначений);
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВариантыПроизводстваМатериалы", Новый ТаблицаЗначений);
		Если ДокументСсылкаСборкаЗапасов.Продукция.Количество() = 1 Тогда 
			СформироватьТаблицыВариантовПродукции(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		КонецЕсли;
		//--- Дмгг
		
		//Малышев 2024-07-15
		СформироватьТаблицаЗапасы_ДопРасходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);	
		
		// Автоперемещение продукции (дополним таблицу ТаблицаЗапасыНаСкладах).
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасыНаСкладах = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах[н];
			
			Если (СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиницаПродукции = СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиница
				И СтрокаТаблицаЗапасыНаСкладах.ЯчейкаПродукции <> СтрокаТаблицаЗапасыНаСкладах.Ячейка)
				ИЛИ СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиницаПродукции <> СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиница Тогда
				
				// Расход.
				Если (СтрокаТаблицаЗапасыНаСкладах.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (СтрокаТаблицаЗапасыНаСкладах.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ СтрокаТаблицаЗапасыНаСкладах.ОрдерныйСклад) Тогда
					
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыНаСкладах);
					
					СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
					
				КонецЕсли;
				
				// Приход.
				Если (СтрокаТаблицаЗапасыНаСкладах.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (СтрокаТаблицаЗапасыНаСкладах.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ СтрокаТаблицаЗапасыНаСкладах.ОрдерныйСкладПродукции
					И НЕ СтрокаТаблицаЗапасыНаСкладах.ОрдерныйСклад) Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыНаСкладах);
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиницаПродукции;
					СтрокаТаблицыПриход.Ячейка = СтрокаТаблицаЗапасыНаСкладах.ЯчейкаПродукции;
					
				КонецЕсли;
				
			КонецЕсли;
			
		КонецЦикла;
		
		// Автоперемещение продукции (сформируем таблицу ТаблицаЗапасыКПоступлениюНаСклады).
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыКПоступлениюНаСклады", МассивРезультатов[5].Выгрузить());
		
		// Автоперемещение продукции (сформируем таблицу ТаблицаЗапасыКРасходуСоСкладов).
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыКРасходуСоСкладов", МассивРезультатов[6].Выгрузить());
		
		// Формирование структуры таблицы проводок документа.
		УправлениеНебольшойФирмойСервер.СформироватьТаблицуПроводок(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Сформируем таблицу по размещению заказов.
		СформироватьТаблицаРазмещениеЗаказовСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Сформируем таблицу распределения материалов.
		ТаблицаПродукция = МассивРезультатов[7].Выгрузить();
		СформироватьТаблицуРаспределенияМатериаловСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, ТаблицаПродукция);
		
		// Запасы.
		СуммаСборки = 0;
		ИнициализироватьДанныеПоЗапасамСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
		
		// Продукция.
		СформироватьТаблицаЗапасыПродукцияСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
		
		// Отходы.
		ИнициализироватьДанныеПоОтходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Серийные номера
		РезультатЗапроса8 = МассивРезультатов[8].Выгрузить();
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераГарантии", РезультатЗапроса8);
		Если СтруктураДополнительныеСвойства.УчетнаяПолитика.ОстаткиСерийныхНомеров Тогда
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераОстатки", РезультатЗапроса8);
		Иначе
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераОстатки", Новый ТаблицаЗначений);
		КонецЕсли; 
		
		Если СтруктураДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара Тогда 		
			СформироватьТаблицаКонтрольСроковГодностиТовараЗапасы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства); //++Lazare
			СформироватьТаблицаКонтрольСроковГодностиТовараПродукция(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства); //++Lazare
		КонецЕсли;
		
		//--------------------------------------------------------------------------
		// © Шишка В.В. [26 декабря 2023]  НАЧАЛО
		СформироватьТаблицу_ДополнитьТаблицыДвиженияПоДопРасходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);	
		// © Шишка В.В. [26 декабря 2023]  КОНЕЦ
		//--------------------------------------------------------------------------
		
		
		
	КонецПроцедуры // ИнициализироватьДанныеДокументаСборка()
	
	//--------------------------------------------------------------------------
	// © Шишка В.В. [26 декабря 2023]  НАЧАЛО
	Процедура СформироватьТаблицу_ДополнитьТаблицыДвиженияПоДопРасходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		Если НЕ ПолучитьФункциональнуюОпцию("УчитыватьДополнительныеЗатратыВПроизводстве") Тогда
		
			Возврат;	
		
		КонецЕсли;
		
		СтруктураТаблицПоДопРасходам = Новый Структура;
		Если не СтруктураДополнительныеСвойства.ДляПроведения.Свойство("СтруктураТаблицПоДопРасходам", СтруктураТаблицПоДопРасходам) Тогда
			СтруктураТаблицПоДопРасходам = СформироватьТаблицу_ДополнитьТаблицыДвиженияПоДопРасходам_ПолучитьСтруктуруТаблиц(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		КонецЕсли;	
		
		Для каждого КлючИЗначение Из СтруктураТаблицПоДопРасходам Цикл
		
			//ТекущаяТаблица = МассивРезультаты[КлючИЗначение.Значение].Выгрузить();  
			ТекущаяТаблица = КлючИЗначение.Значение;
			
			Если НЕ СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Свойство(КлючИЗначение.Ключ) Тогда
			
				СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить(КлючИЗначение.Ключ, ТекущаяТаблица);	
			
			Иначе
				
				Если КлючИЗначение.Ключ = "ТаблицаЗапасы" Тогда 
					
					Продолжить;
					
					ТаблицаДополненияДвижений = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.СкопироватьКолонки();
					
					Если ДокументСсылкаСборкаЗапасов.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда
						//Мы сделали это при формировании запасов 
						Продолжить;
					КонецЕсли;	
						
					
					Для Каждого ТекСтрока Из ТекущаяТаблица Цикл        
						
						СтруктураФильтра = Новый Структура("ВидДвижения,Организация,СтруктурнаяЕдиница,Номенклатура", ТекСтрока.ВидДвижения,ТекСтрока.Организация,ТекСтрока.СтруктурнаяЕдиница,ТекСтрока.Номенклатура);
						СтрокиЗапасов = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.НайтиСтроки(СтруктураФильтра);
						
							
						Если СтрокиЗапасов.Количество() > 0 Тогда 
							СтрокаЗапасов = СтрокиЗапасов[0];
							//СтрокаЗапасов.Сумма = СтрокаЗапасов.Сумма + ТекСтрока.Сумма;  
							
							СтрокаТаблицыПриход = ТаблицаДополненияДвижений.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход,СтрокаЗапасов); 
					        СтрокаТаблицыПриход.Сумма = ТекСтрока.Сумма;
						КонецЕсли;
						
					КонецЦикла;
					
					Для каждого СтрокаТаблицыПриход из ТаблицаДополненияДвижений Цикл
						НоваяСтрокаДвижения = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрокаДвижения,СтрокаТаблицыПриход);
					КонецЦикла;
					
				Иначе
					
					Для каждого ТекСтрока Из ТекущаяТаблица Цикл
						
						СтрокаДвижения = СтруктураДополнительныеСвойства.ТаблицыДляДвижений[КлючИЗначение.Ключ].Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаДвижения, ТекСтрока);
						
					КонецЦикла; 
					
				КонецЕсли;
			
			КонецЕсли;	
		
		КонецЦикла; 
	
	КонецПроцедуры
	
	
	Функция СформироватьТаблицу_ДополнитьТаблицыДвиженияПоДопРасходам_ПолучитьСтруктуруТаблиц(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		СтруктураТаблиц = Новый Структура;
		СтруктураТаблиц.Вставить("ТаблицаРасчетыНДФЛ", 7);
		СтруктураТаблиц.Вставить("ТаблицаЗакупки", 8);
		СтруктураТаблиц.Вставить("ТаблицаРасчетыПоНалогам", 9);
		СтруктураТаблиц.Вставить("ТаблицаРасчетыСПенсионнымФондом", 10);
		СтруктураТаблиц.Вставить("ТаблицаДоходыИРасходы", 11);
		СтруктураТаблиц.Вставить("ТаблицаРасчетыСПоставщиками", 12);    
		СтруктураТаблиц.Вставить("ТаблицаРасчетыСПерсоналом", 13);
		СтруктураТаблиц.Вставить("ТаблицаНачисленияИУдержания", 14); 
		СтруктураТаблиц.Вставить("ТаблицаНачисленияИУдержанияВРазрезеПроекта", 15);
		СтруктураТаблиц.Вставить("ТаблицаЗапасы", 16);
		СтруктураТаблиц.Вставить("ТаблицаХозрасчетный", 17);   
		
		ВалютаУчета = УправлениеНебольшойФирмойПовтИсп.ПолучитьВалютуУчета();     
		ВалютаНациональная = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("НациональнаяВалюта");
		МассивВалютДокумента = ДокументСсылкаСборкаЗапасов.ДополнительныеРасходы.Выгрузить().ВыгрузитьКолонку("Валюта");   
		МассивВалютДокумента.Добавить(ВалютаУчета);
		МассивВалютДокумента.Добавить(ВалютаНациональная);
		
		Запрос = Новый Запрос;
		//Малышев 2024-07-09 ONESOFT-3310 (--
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц; 
		Запрос.Параметры.Вставить("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам); 
		//Малышев 2024-07-09 ONESOFT-3310 --)
		Запрос.Параметры.Вставить("Ссылка", СтруктураДополнительныеСвойства.ДляПроведения.Ссылка);
		Запрос.Параметры.Вставить("ВалютаНациональная", ВалютаНациональная);
		Запрос.Параметры.Вставить("ВалютаУчета", ВалютаУчета); 
		Запрос.Параметры.Вставить("МассивВалютДополнительныхРасходов", МассивВалютДокумента);
		Запрос.Параметры.Вставить("ДатаСреза", СтруктураДополнительныеСвойства.ДляПроведения.МоментКонтроля);
		Запрос.Параметры.Вставить("ИспользованиеТолькоРеглПланаСчетов", Истина);
		Запрос.Параметры.Вставить("МоментВремени", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени);
		Запрос.Параметры.Вставить("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.Параметры.Вставить("СмещениеДляОкругления", 0);
		Запрос.Параметры.Вставить("СписаниеЗапасов", НСтр("ka='Списание запасов';ru='Списание запасов';en='Списание запасов'"));
		Запрос.Параметры.Вставить("ТранзитныйСклад", ЛОЖЬ);
		Запрос.Параметры.Вставить("СодержаниеРасчетыНДФЛ", НСтр("ka='დარიცხულია გადასახადი';ru='Начислен налог';en='accrued tax'"));
		Запрос.Параметры.Вставить("СодержаниеПроводкиДоходыРасходы", НСтр("ka='Удержание организации для пенсионного фонта';ru='Удержание организации для пенсионного фонта';en='Удержание организации для пенсионного фонта'"));
		Запрос.Параметры.Вставить("СодержаниеПроводкиРасчетыСПоставщиками", НСтр("ka='Обязательство перед поставщиком';ru='Обязательство перед поставщиком';en='Обязательство перед поставщиком'"));
		Запрос.Параметры.Вставить("СодержаниеРасчетиСПерсоналомЗП", НСтр("ka='ხელფასის დარიცხვა';ru='Начисление зарплаты';en='Payroll'"));
		Запрос.Параметры.Вставить("СодержаниеРасчетиСПерсоналомНДФЛ", НСтр("ka='დარიცხულია გადასახადი';ru='Начислен налог';en='accrued tax'"));
		Запрос.Параметры.Вставить("СодержаниеРасчетиСПерсоналомПФ", НСтр("ka='საპენსიო ფონდის გადასახადი';ru='Налог пенсионного фонда';en='Tax pension fund'"));    
		Запрос.Параметры.Вставить("СодержаниеПроводкиДоходыРасходыСотрудник", НСтр("ka='ორგანიზაციის ხარჯი საპენსიო ფონდისთვის';ru='Удержание организации для пенсионного фонда';en='Hold the organization to the Pension Fund'"));
		
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СборкаЗапасовПродукция.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовПродукция.Характеристика КАК Характеристика,
		|	СборкаЗапасовПродукция.Количество КАК Количество,
		//Малышев 2024-07-09 ONESOFT-3310 (--
		|	ВЫБОР 
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА Значение(Справочник.проекты.ПустаяСсылка)
		|		КОГДА СборкаЗапасовПродукция.Проект <> Значение(Справочник.проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовПродукция.Проект
		|       ИНАЧЕ Значение(Справочник.проекты.БезПроекта)
		|   КОНЕЦ КАК Проект,
		//Малышев 2024-07-09 ONESOFT-3310 --)
		|	СборкаЗапасовПродукция.ДоляДопРасходов КАК ДоляДопРасходов
		|ПОМЕСТИТЬ ВТ_Продукция
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|ГДЕ
		|	СборкаЗапасовПродукция.Ссылка = &Ссылка 
		//Малышев 2024-07-09 ONESOFT-3310 (--
		|	И СборкаЗапасовПродукция.Ссылка.ВидОперации =  Значение(Перечисление.ВидыОперацийСборкаЗапасов.Сборка)
		|
		|  ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СборкаЗапасовПродукция.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовПродукция.Характеристика КАК Характеристика,
		|	СборкаЗапасовПродукция.Количество КАК Количество,
		|	ВЫБОР 
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА Значение(Справочник.проекты.ПустаяСсылка)
		|		КОГДА СборкаЗапасовПродукция.Проект <> Значение(Справочник.проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовПродукция.Проект
		|       ИНАЧЕ Значение(Справочник.проекты.БезПроекта)
		|   КОНЕЦ КАК Проект,
		|	1 КАК ДоляДопРасходов
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовПродукция
		|ГДЕ
		|	СборкаЗапасовПродукция.Ссылка = &Ссылка 
		|	И СборкаЗапасовПродукция.Ссылка.ВидОперации =  Значение(Перечисление.ВидыОперацийСборкаЗапасов.Разборка)
		//Малышев 2024-07-09 ONESOFT-3310 --)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_Продукция.НомерСтроки КАК НомерСтроки,
		|	ВТ_Продукция.Номенклатура КАК Номенклатура,
		|	ВТ_Продукция.Характеристика КАК Характеристика,
		|	ВТ_Продукция.Проект КАК Проект, //Малышев 2024-07-09 ONESOFT-3310
		|	ВТ_Продукция.Количество КАК Количество,
		|	ВЫБОР
		|		КОГДА ИтоговоеКоличество.Количество = 0
		|			ТОГДА 0
		//|		ИНАЧЕ ВТ_Продукция.ДоляДопРасходов / ИтоговоеКоличество.Количество 
		|		ИНАЧЕ ВТ_Продукция.Количество / ИтоговоеКоличество.Количество
		|	КОНЕЦ КАК ДоляСтоимости,
		|	ВТ_Продукция.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасов
		|ПОМЕСТИТЬ ВТ_ПродукцияДоляСтоимости
		|ИЗ
		|	ВТ_Продукция КАК ВТ_Продукция,
		|	(ВЫБРАТЬ
		//|		СУММА(ВТ_Продукция.ДоляДопРасходов) КАК Количество
		|		СУММА(ВТ_Продукция.Количество) КАК Количество
		|	ИЗ
		|		ВТ_Продукция КАК ВТ_Продукция) КАК ИтоговоеКоличество
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|   СборкаЗапасов.Организация как Организация,
		|   СборкаЗапасов.Дата как Период,
		|   СборкаЗапасов.ВидОперации как ВидОперации,
		|   СборкаЗапасов.СтруктурнаяЕдиница как СтруктурнаяЕдиница,
		|   СборкаЗапасов.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы как СтруктурнаяЕдиница_ТипСтруктурнойЕдиницы,
		|   СборкаЗапасов.СтруктурнаяЕдиницаЗапасов как СтруктурнаяЕдиницаЗапасов,
		|   СборкаЗапасов.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы как СтруктурнаяЕдиницаЗапасов_ТипСтруктурнойЕдиницы,
		|   СборкаЗапасов.СтруктурнаяЕдиницаОтходов как СтруктурнаяЕдиницаОтходов,
		|   СборкаЗапасов.СтруктурнаяЕдиницаОтходов.ТипСтруктурнойЕдиницы как СтруктурнаяЕдиницаОтходов_ТипСтруктурнойЕдиницы,
		|   СборкаЗапасов.СтруктурнаяЕдиницаПродукции как СтруктурнаяЕдиницаПродукции,
		|   СборкаЗапасов.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы как СтруктурнаяЕдиницаПродукции_ТипСтруктурнойЕдиницы,
		|	ЛОЖЬ КАК ВестиУчетРасходовПоДоговорамОбслуживания,
		|	НЕОПРЕДЕЛЕНО КАК НаправлениеДеятельности,
		|	ЛОЖЬ КАК СуммаВключаетНДС,
		|	ЗНАЧЕНИЕ(Справочник.ВидыНалогов.Пустаяссылка) КАК ВидНалогаНДС,
		|	НЕОПРЕДЕЛЕНО КАК Проект
		|ПОМЕСТИТЬ ВременнаяТаблицаШапка
		|ИЗ
		|	Документ.СборкаЗапасов КАК СборкаЗапасов
		|ГДЕ
		|	СборкаЗапасов.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КурсыВалютСрезПоследних.Валюта КАК Валюта,
		|	КурсыВалютСрезПоследних.Курс КАК Курс,
		|	КурсыВалютСрезПоследних.Кратность КАК Кратность
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсыВалютСрезПоследних
		|ИЗ
		//|	РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, Валюта В (&ВалютаУчета, &ВалютаНациональная)) КАК КурсыВалютСрезПоследних
		|	РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, Валюта В (&МассивВалютДополнительныхРасходов)) КАК КурсыВалютСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПриходнаяНакладнаяРасходы.Ссылка КАК Ссылка,
		|	ПриходнаяНакладнаяРасходы.Контрагент КАК Контрагент,  
		|	ПриходнаяНакладнаяРасходы.Подразделение КАК Подразделение,
		|	ПриходнаяНакладнаяРасходы.Договор КАК Договор,
		|	ПриходнаяНакладнаяРасходы.НомерСтроки КАК НомерСтроки,
		|	ПриходнаяНакладнаяРасходы.Номенклатура КАК Номенклатура,
		|	ПриходнаяНакладнаяРасходы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ПриходнаяНакладнаяРасходы.Количество КАК Количество,
		|	ПриходнаяНакладнаяРасходы.Цена КАК Цена,
		|	0 КАК ПроцентСкидкиНаценки,
		|	ПриходнаяНакладнаяРасходы.СтавкаНДС КАК СтавкаНДС,
		|	НЕОПРЕДЕЛЕНО КАК ЗаказПоставщику,
		|	ПриходнаяНакладнаяРасходы.Всего КАК Всего,
		|	НЕОПРЕДЕЛЕНО КАК Заказ,
		|	ПриходнаяНакладнаяРасходы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,  
		|	ПриходнаяНакладнаяРасходы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
		|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ОсновноеНаправление) КАК НаправлениеДеятельности,
		|	"""" КАК Содержание,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
		|	ПриходнаяНакладнаяРасходы.СуммаНДФЛ КАК СуммаНДФЛ,
		|	ПриходнаяНакладнаяРасходы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаТаб,
		|	ПриходнаяНакладнаяРасходы.СуммаНДС КАК СуммаНДС,
		|	ПриходнаяНакладнаяРасходы.Всего - ПриходнаяНакладнаяРасходы.СуммаНДС КАК Сумма,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ КАК ВидУдержанияПФ,
		|	ПриходнаяНакладнаяРасходы.СуммаПФ КАК СуммаПФ,
		|	ПриходнаяНакладнаяРасходы.СуммаБезНалогов КАК СуммаБезНалогов,
		|	ПриходнаяНакладнаяРасходы.ВидНалогаНДС КАК ВидНалогаНДС, 
		|	ПриходнаяНакладнаяРасходы.Ссылка.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукция,
		//Малышев 2024-07-09 ONESOFT-3310 (--
		//|	ПриходнаяНакладнаяРасходы.Проект КАК Проект, 
		|	ВЫБОР 
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА Значение(Справочник.проекты.ПустаяСсылка)
		|		КОГДА ПриходнаяНакладнаяРасходы.Проект <> Значение(Справочник.проекты.ПустаяСсылка)
		|			ТОГДА ПриходнаяНакладнаяРасходы.Проект
		|       ИНАЧЕ Значение(Справочник.проекты.БезПроекта)
		|   КОНЕЦ КАК Проект,
		//Малышев 2024-07-09 ONESOFT-3310 --)
		
		|	ПриходнаяНакладнаяРасходы.Ссылка.Организация.ВидНалогаНДС.СчетУчетаРегл КАК СчетУчетаНДСРегл,
		|	ПриходнаяНакладнаяРасходы.Валюта КАК Валюта,
		|	ПриходнаяНакладнаяРасходы.Кратность КАК Кратность,
		|	ПриходнаяНакладнаяРасходы.Курс КАК Курс,   
		|	ПриходнаяНакладнаяРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания, 
		|	ПриходнаяНакладнаяРасходы.Ссылка.Организация.ВидНалогаНДС.СчетУчетаКВозмещениюРегл КАК СчетУчетаНДСКВозмещениюРегл
		|ПОМЕСТИТЬ ВременнаяТаблицаРасходыПерерасчет
		|ИЗ
		|	Документ.СборкаЗапасов.ДополнительныеРасходы КАК ПриходнаяНакладнаяРасходы
		|ГДЕ
		|	ПриходнаяНакладнаяРасходы.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПриходнаяНакладнаяЗапасы.НомерСтроки КАК НомерСтроки,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации КАК ВидОперации,
		|	ЛОЖЬ КАК СуммаВключаетНДС,
		|	ПриходнаяНакладнаяЗапасы.Ссылка КАК Документ,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.ДокументОснование КАК ДокументОснование,
		|	ПриходнаяНакладнаяЗапасы.Контрагент КАК Контрагент,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.ВестиРасчетыПоДоговорам КАК ВестиРасчетыПоДоговорам,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.ВестиРасчетыПоЗаказам КАК ВестиРасчетыПоЗаказам,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.ВестиУчетОплатыПоСчетам КАК ВестиУчетОплатыПоСчетам,
		|	ПриходнаяНакладнаяЗапасы.Договор КАК Договор,
		|	НЕОПРЕДЕЛЕНО КАК Ответственный,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаНаценки КАК СчетУчетаНаценки,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.ДокументОснование КАК ДокументОснования,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаНаценкиРегл КАК СчетУчетаНаценкиРегл,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.РозничныйВидЦен КАК РозничныйВидЦен,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.РозничныйВидЦен.ВалютаЦены КАК ВалютаЦены,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРознице КАК СчетУчетаВРознице,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРозницеРегл КАК СчетУчетаВРозницеРегл,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемВПереработку)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемНаОтветхранение)
		|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|		ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	КОНЕЦ КАК ВидДвижения,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ПеремещениеВРозницуСуммовойУчет,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.Дата КАК Период,
		|	&Организация КАК Организация,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветхранения)
		|			ТОГДА &Организация
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК КоррОрганизация,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ПриходнаяНакладнаяЗапасы.Контрагент КАК КоррСтруктурнаяЕдиница,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ОрдерныйСклад,
		|	НЕОПРЕДЕЛЕНО КАК Ячейка,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТранзитныйСклад = &ТранзитныйСклад
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРознице
		|		ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасов
		|	КОНЕЦ КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТранзитныйСклад
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаРегл
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТранзитныйСклад = &ТранзитныйСклад
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРозницеРегл
		|		ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|	КОНЕЦ КАК СчетУчетаРегл,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТранзитныйСклад = &ТранзитныйСклад
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРознице.Валютный
		|		ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасов.Валютный
		|	КОНЕЦ КАК СчетУчетаВалютный,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТранзитныйСклад
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаРегл.Валютный
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.ТранзитныйСклад = &ТранзитныйСклад
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница.СчетУчетаВРозницеРегл.Валютный
		|		ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		|	КОНЕЦ КАК СчетУчетаВалютныйРегл,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветхранения)
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК КоррСчетУчета,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветхранения)
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК КоррСчетУчетаРегл,
//		|	ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		//Малышев 2024-06-24 ONESOFT-3273 (--
		//|	ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл, 
		|	ВЫБОР 
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|        	ТОГДА Значение(ПланСчетов.Хозрасчетный.ПустаяСсылка)
		|		ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗатратРегл
 		|	КОНЕЦ КАК СчетУчетаЗатратРегл,		
		//Малышев 2024-06-24 ONESOFT-3273 --)
		
		|	ПриходнаяНакладнаяЗапасы.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика)
		|				ИЛИ ПриходнаяНакладнаяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветхранения)
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Номенклатура
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК КоррНоменклатура,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК КоррХарактеристика,
		|	ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка) КАК Партия,
		|	ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка) КАК КоррПартия,
		|	НЕОПРЕДЕЛЕНО КАК Заказ,
		|	ЛОЖЬ КАК ЗаказНаряд,
		|	НЕОПРЕДЕЛЕНО КАК КоррЗаказ,
		|	ЛОЖЬ КАК ТоварыНаКомиссии,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница КАК ПодразделениеПродажи,
		|	ПриходнаяНакладнаяЗапасы.Номенклатура.НаправлениеДеятельности КАК НаправлениеДеятельностиПродажи,
		|	ПриходнаяНакладнаяЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаВыручкиОтПродаж КАК СчетУчетаПродажи,
		|	ПриходнаяНакладнаяЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаВыручкиОтПродажРегл КАК СчетУчетаПродажиРегл,
		|	ПриходнаяНакладнаяЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаВыручкиОтПродажРегл КАК СчетУчетаПродажиВозвратРегл,
		|	ПриходнаяНакладнаяЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаСебестоимостиПродаж КАК СчетУчетаСебестоимость,
		|	ПриходнаяНакладнаяЗапасы.Номенклатура.НаправлениеДеятельности.СчетУчетаСебестоимостиПродажРегл КАК СчетУчетаСебестоимостьРегл,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ПриходнаяНакладнаяЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА ПриходнаяНакладнаяЗапасы.Количество
		|		ИНАЧЕ ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Количество * ПриходнаяНакладнаяЗапасы.ЕдиницаИзмерения.Коэффициент - &СмещениеДляОкругления КАК ЧИСЛО(15, 3))
		|	КОНЕЦ КАК Количество,
		|	ПриходнаяНакладнаяЗапасы.СтавкаНДС КАК СтавкаНДС,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДС,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДСЗакупкиПродажи,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК Сумма,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)), 0) КАК СуммаРасходов,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс / КурсыРегВалюты.Кратность КАК ЧИСЛО(15, 8)) КАК СуммаРасходовНДС,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаРасходовНДСВал,
		|	0 КАК Себестоимость,
		|	ПриходнаяНакладнаяЗапасы.Всего КАК СуммаРасчетовПринятыеПереданные,
		|	ИСТИНА КАК ВключатьРасходыВСебестоимость,
		|	ЛОЖЬ КАК ФиксированнаяСтоимость,
		|	ВЫРАЗИТЬ(&СписаниеЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.СчетУчетаРасчетовСПокупателем КАК СчетУчетаРасчетовСПокупателем,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.СчетУчетаРасчетовСПокупателемРегл КАК СчетУчетаРасчетовСПокупателемРегл,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.СчетУчетаРасчетовСПоставщиком КАК СчетУчетаРасчетовСПоставщиком,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРасчетовСПоставщикомРегл,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.СчетУчетаРасчетовСПоставщиком.Валютный КАК СчетУчетаРасчетовСПоставщикомВалютный,
		|	ПриходнаяНакладнаяЗапасы.Контрагент.СчетУчетаРасчетовСПоставщикомРегл.Валютный КАК СчетУчетаРасчетовСПоставщикомВалютныйРегл,
		|	НЕОПРЕДЕЛЕНО КАК СтранаПроисхождения,
		|	НЕОПРЕДЕЛЕНО КАК НомерГТД,
		|	ПриходнаяНакладнаяЗапасы.Договор.ВалютаРасчетов КАК ВалютаРасчетов,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаВал,
		|	ПриходнаяНакладнаяЗапасы.СуммаНДС КАК СуммаНДСВал,
		|	0 КАК КлючСвязи,
		|	ВЫРАЗИТЬ((ПриходнаяНакладнаяЗапасы.Всего - ПриходнаяНакладнаяЗапасы.СуммаНДС) * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаБезНДС,
		|	ВЫРАЗИТЬ((ПриходнаяНакладнаяЗапасы.Всего - ПриходнаяНакладнаяЗапасы.СуммаНДС) * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаБезНДСВал,
		|	ВременнаяТаблицаШапка.ВидНалогаНДС КАК ВидНалогаНДС,
		|	ПриходнаяНакладнаяЗапасы.Договор.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
		|	ПриходнаяНакладнаяЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаТаб,
		|	ПриходнаяНакладнаяЗапасы.Договор.ВидУдержанияНДФЛ.ВидНалога.СчетУчетаРегл КАК СчетНачисленияНДФЛ,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаПФ * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаРасходовПФ,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаПФ * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаРасходовПФВал,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаНДФЛ * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаРасходовНДФЛ,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаНДФЛ * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаРасходовНДФЛВал,
		|	ПриходнаяНакладнаяЗапасы.Содержание КАК Содержание,
		|	ПриходнаяНакладнаяЗапасы.Проект КАК Проект,
		|	НЕОПРЕДЕЛЕНО КАК СрокГодности,
		|	ЛОЖЬ КАК ВключенВрегистр
		//|ПОМЕСТИТЬ ВременнаяТаблицаЗапасы
		|ПОМЕСТИТЬ ВременнаяТаблицаЗапасы_ДляДопРасходов
		|ИЗ
		|	ВременнаяТаблицаРасходыПерерасчет КАК ПриходнаяНакладнаяЗапасы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыУпрВалюты
		|		ПО (КурсыУпрВалюты.Валюта = &ВалютаУчета)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыРегВалюты
		|		ПО (КурсыРегВалюты.Валюта = &ВалютаНациональная) 
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаРегл.СрезПоследних(&ДатаСреза, ) КАК УчетнаяПолитикаРегл
		|		ПО ПриходнаяНакладнаяЗапасы.Ссылка.Организация = УчетнаяПолитикаРегл.Организация,
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|ГДЕ
		|	ПриходнаяНакладнаяЗапасы.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПриходнаяНакладнаяРасходы.НомерСтроки КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ПриходнаяНакладнаяРасходы.Ссылка.Дата КАК Период,
		|	ПриходнаяНакладнаяРасходы.Ссылка КАК Документ,
		|	ПриходнаяНакладнаяРасходы.Ссылка.ВидОперации КАК ВидОперации,
		|	&Организация КАК Организация,
		|	ПриходнаяНакладнаяРасходы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница, 
		|	ПриходнаяНакладнаяРасходы.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
		|	&ВалютаНациональная КАК Валюта,
		|	ПриходнаяНакладнаяРасходы.Номенклатура.СчетУчетаЗатрат КАК СчетУчета,
		|	ПриходнаяНакладнаяРасходы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаРегл,
		|	ПриходнаяНакладнаяРасходы.Номенклатура КАК Номенклатура,
		|	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК ЗапасыНоменклатура,
		|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика,
		|	ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка) КАК Партия,
		|	ПриходнаяНакладнаяРасходы.Заказ КАК Заказ,
		|	ПриходнаяНакладнаяРасходы.ЗаказПоставщику КАК ЗаказПоставщику,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ПриходнаяНакладнаяРасходы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА ПриходнаяНакладнаяРасходы.Количество
		|		ИНАЧЕ ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Количество * ПриходнаяНакладнаяРасходы.ЕдиницаИзмерения.Коэффициент - &СмещениеДляОкругления КАК ЧИСЛО(15, 3))
		|	КОНЕЦ КАК Количество,
		|	ПриходнаяНакладнаяРасходы.СтавкаНДС КАК СтавкаНДС,                                                                                                                  
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК Сумма,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Всего * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * ЕСТЬNULL(КурсыУпрВалюты.Кратность, 1) / (ЕСТЬNULL(КурсыУпрВалюты.Курс, 1) * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК Сумма,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Всего КАК ЧИСЛО(15, 8)) КАК СуммаВал,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДС,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДСЗакупкиПродажи,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * ЕСТЬNULL(КурсыУпрВалюты.Кратность, 1) / (ЕСТЬNULL(КурсыУпрВалюты.Курс, 1) * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаНДС,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * ЕСТЬNULL(КурсыУпрВалюты.Кратность, 1) / (ЕСТЬNULL(КурсыУпрВалюты.Курс, 1) * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаНДСЗакупкиПродажи,
		|	ИСТИНА КАК ВключатьРасходыВСебестоимость,
		|	ВЫБОР
		|		КОГДА НЕ &ИспользованиеТолькоРеглПланаСчетов
		|			ТОГДА ПриходнаяНакладнаяРасходы.Номенклатура.НаправлениеДеятельности
		|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Номенклатура.НаправлениеДеятельности
		|	КОНЕЦ КАК НаправлениеДеятельности,
		|	ПриходнаяНакладнаяРасходы.Контрагент КАК Контрагент,
		//|	ПриходнаяНакладнаяРасходы.Контрагент.ВестиРасчетыПоДоговорам КАК ВестиРасчетыПоДоговорам,
		//|	ПриходнаяНакладнаяРасходы.Контрагент.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
		//|	ПриходнаяНакладнаяРасходы.Контрагент.ВестиРасчетыПоЗаказам КАК ВестиРасчетыПоЗаказам,
		//|	ПриходнаяНакладнаяРасходы.Контрагент.ВестиУчетОплатыПоСчетам КАК ВестиУчетОплатыПоСчетам, 
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ПриходнаяНакладнаяРасходы.Контрагент.ВестиРасчетыПоДоговорам
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ВестиРасчетыПоДоговорам, 
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ПриходнаяНакладнаяРасходы.Контрагент.ВестиРасчетыПоДокументам
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ВестиРасчетыПоДокументам,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ПриходнаяНакладнаяРасходы.Контрагент.ВестиРасчетыПоЗаказам
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ВестиРасчетыПоЗаказам,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ПриходнаяНакладнаяРасходы.Контрагент.ВестиУчетОплатыПоСчетам
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ВестиУчетОплатыПоСчетам,
		|	ПриходнаяНакладнаяРасходы.Договор КАК Договор,   
		|	ПриходнаяНакладнаяРасходы.Подразделение КАК Подразделение,
		//|	ПриходнаяНакладнаяРасходы.Договор.ВалютаРасчетов КАК ВалютаРасчетов,   
		|	ПриходнаяНакладнаяРасходы.Валюта КАК ВалютаРасчетов,
		//|	ПриходнаяНакладнаяРасходы.Контрагент.СчетУчетаРасчетовСПоставщиком КАК СчетУчетаРасчетовСПоставщиком,
		//|	ПриходнаяНакладнаяРасходы.Контрагент.СчетУчетаРасчетовСПоставщикомРегл КАК СчетУчетаРасчетовСПоставщикомРегл, 
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ПриходнаяНакладнаяРасходы.Контрагент.СчетУчетаРасчетовСПоставщиком               
		|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Контрагент.СчетРасчетовСПерсоналом
		|	КОНЕЦ КАК СчетУчетаРасчетовСПоставщиком,
		|	ВЫБОР
		|		КОГДА ПриходнаяНакладнаяРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ПриходнаяНакладнаяРасходы.Контрагент.СчетУчетаРасчетовСПоставщикомРегл
		|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Контрагент.СчетРасчетовСПерсоналомРегл
		|	КОНЕЦ КАК СчетУчетаРасчетовСПоставщикомРегл,
		|	ВЫБОР
		|		КОГДА &ИспользованиеТолькоРеглПланаСчетов
		|			ТОГДА ПриходнаяНакладнаяРасходы.Номенклатура.СчетУчетаЗатратРегл.ТипСчета
		|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Номенклатура.СчетУчетаЗатрат.ТипСчета
		|	КОНЕЦ КАК ТипСчета,
		|	ПриходнаяНакладнаяРасходы.СтруктурнаяЕдиницаТаб КАК СтруктурнаяЕдиницаТаб,
		//|	ВЫРАЗИТЬ((ПриходнаяНакладнаяРасходы.Всего - ПриходнаяНакладнаяРасходы.СуммаНДС - ПриходнаяНакладнаяРасходы.СуммаПФ) * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаБезНДС,
		|	ВЫРАЗИТЬ((ПриходнаяНакладнаяРасходы.Всего - ПриходнаяНакладнаяРасходы.СуммаНДС - ПриходнаяНакладнаяРасходы.СуммаПФ) * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаБезНДСВал,
		|	ВЫРАЗИТЬ((ПриходнаяНакладнаяРасходы.Всего - ПриходнаяНакладнаяРасходы.СуммаНДС - ПриходнаяНакладнаяРасходы.СуммаПФ) * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * ЕСТЬNULL(КурсыУпрВалюты.Кратность, 1) / (ЕСТЬNULL(КурсыУпрВалюты.Курс, 1) * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаБезНДС,
		//|	ВЫРАЗИТЬ((ПриходнаяНакладнаяРасходы.Всего - ПриходнаяНакладнаяРасходы.СуммаНДС - ПриходнаяНакладнаяРасходы.СуммаПФ) * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * 1 / (1 * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаБезНДСВал,
		|	ПриходнаяНакладнаяРасходы.ВидНалогаНДС КАК ВидНалогаНДС,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДСВал,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * 1 / (1 * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаНДСВал,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияНДФЛ.ВидНалога.СчетУчетаРегл КАК СчетНачисленияНДФЛ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ КАК ВидУдержанияПФ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.СчетЗатратРегл КАК СчетЗатратРеглПФ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.СчетЗатрат КАК СчетЗатратПФ,
		//|	ПриходнаяНакладнаяРасходы.Ссылка.СтруктурнаяЕдиница КАК Подразделение,
		|	ВЫБОР
		|		КОГДА &ИспользованиеТолькоРеглПланаСчетов
		|			ТОГДА ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.СчетЗатратРегл.ТипСчета
		|		ИНАЧЕ ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.СчетЗатрат.ТипСчета
		|	КОНЕЦ КАК ТипСчетаЗатратПФ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.Контрагент КАК КонтрагентПФ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.Договор КАК ДоговорПФ,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.ПенсионныйНалогСотрудника.СчетУчетаРегл КАК СчетНачисленииПФСотрудника,
		|	ПриходнаяНакладнаяРасходы.ВидУдержанияПФ.ПенсионныйНалогОрганизации.СчетУчетаРегл КАК СчетНачисленииПФОрганизации,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаПФ * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаПФ,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаПФ * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаПФВал,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДФЛ * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДФЛ,
		//|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДФЛ * КурсыРегВалюты.Курс * 1 / (1 * КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 8)) КАК СуммаНДФЛВал,   
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаПФ * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * ЕСТЬNULL(КурсыУпрВалюты.Кратность, 1) / (ЕСТЬNULL(КурсыУпрВалюты.Курс, 1) * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаПФ,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаПФ * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * 1 / (1 * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаПФВал,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДФЛ * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * ЕСТЬNULL(КурсыУпрВалюты.Кратность, 1) / (ЕСТЬNULL(КурсыУпрВалюты.Курс, 1) * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаНДФЛ,
		|	ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДФЛ * ЕСТЬNULL(КурсыВалютыСтроки.Курс, 1) * 1 / (1 * ЕСТЬNULL(КурсыВалютыСтроки.Кратность, 1)) КАК ЧИСЛО(15, 8)) КАК СуммаНДФЛВал,
		|	ПриходнаяНакладнаяРасходы.Содержание КАК Содержание,
		
		//Малышев 2024-06-17
		|	ПриходнаяНакладнаяРасходы.Проект как Проект,
		//|	ВЫБОР
		//|		КОГДА ПриходнаяНакладнаяРасходы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Проект
		//|	КОНЕЦ КАК Проект,
		
		|	ПриходнаяНакладнаяРасходы.СчетУчетаНДСРегл КАК СчетУчетаНДСРегл,  
		|	ПриходнаяНакладнаяРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	ПриходнаяНакладнаяРасходы.СчетУчетаНДСКВозмещениюРегл КАК СчетУчетаНДСКВозмещениюРегл,  
		|	ПриходнаяНакладнаяРасходы.СтруктурнаяЕдиницаПродукция КАК СтруктурнаяЕдиницаПродукция
		|ПОМЕСТИТЬ ВременнаяТаблицаРасходы
		|ИЗ
		|	ВременнаяТаблицаРасходыПерерасчет КАК ПриходнаяНакладнаяРасходы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыУпрВалюты
		|		ПО (КурсыУпрВалюты.Валюта = &ВалютаУчета)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыРегВалюты
		|		ПО (КурсыРегВалюты.Валюта = &ВалютаНациональная)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютыСтроки
		|		ПО (КурсыВалютыСтроки.Валюта = ПриходнаяНакладнаяРасходы.Валюта)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.ВидУдержанияНДФЛ КАК ВидНачисленияУдержания,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта, 
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииПФ.Начисленный) КАК Операция,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛВал КАК СуммаВал,
		|	&СодержаниеРасчетыНДФЛ КАК СодержаниеПроводки,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	""РасчетыНДФЛ"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Номенклатура КАК Номенклатура,
		|	ВременнаяТаблицаРасходы.Характеристика КАК Характеристика,
		|	ВременнаяТаблицаРасходы.Документ КАК Документ,
		|	ВременнаяТаблицаРасходы.СтавкаНДС КАК СтавкаНДС,
		|	ВременнаяТаблицаРасходы.Сумма КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаНДС КАК СуммаНДС, 
		|	ВременнаяТаблицаРасходы.Количество КАК Количество,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	""Закупки"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ  
		|	ВременнаяТаблицаРасходы.Контрагент Ссылка Справочник.Контрагенты     
		|	И ВременнаяТаблицаРасходы.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.ВидУдержанияНДФЛ.ВидНалога КАК ВидНалога,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ КАК Сумма,     
		|	&СодержаниеРасчетыНДФЛ КАК СодержаниеПроводки,
		|	""РасчетыПоНалогам"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.ВидУдержанияПФ КАК ВидНачисленияУдержания,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Плательщик,
		|	ВременнаяТаблицаРасходы.Контрагент КАК СотрудникКонтрагент,
		|	ВременнаяТаблицаРасходы.Договор КАК ДоговорКонтрагента,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ПериодРегистрации,
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииПФ.Начисленный) КАК Операция,
		|	ВременнаяТаблицаРасходы.СуммаПФ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаПФВал КАК СуммаВал,
		|	""РасчетыСПенсионнымФондом"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаПФ > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения,
		|	ВременнаяТаблицаРасходы.ВидУдержанияПФ,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВременнаяТаблицаРасходы.Контрагент,
		|	ВременнаяТаблицаРасходы.Договор,
		|	ВременнаяТаблицаРасходы.Валюта,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ),
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииПФ.Начисленный),
		|	ВременнаяТаблицаРасходы.СуммаПФ,
		|	ВременнаяТаблицаРасходы.СуммаПФВал,
		|	""РасчетыСПенсионнымФондом""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаПФ > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	ВременнаяТаблицаРасходы.СчетЗатратРеглПФ КАК СчетУчетаРегл,
		|	ВременнаяТаблицаРасходы.СуммаПФ КАК СуммаРасходов,
		//|	&СодержаниеПроводкиДоходыРасходы КАК СодержаниеПроводки,
		//|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПоступлениеОтПоставщика) КАК ХозяйственнаяОперация,  
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА &СодержаниеПроводкиДоходыРасходы
		|		ИНАЧЕ &СодержаниеПроводкиДоходыРасходыСотрудник
		|	КОНЕЦ КАК СодержаниеПроводки,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА 	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПоступлениеОтПоставщика)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.НачислениеЗарплаты)
		|	КОНЕЦ КАК ХозяйственнаяОперация,
		|	""ДоходыИРасходы"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаПФ > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг) КАК ТипРасчетов,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Контрагент,
		|	ВременнаяТаблицаРасходы.Договор КАК Договор,
		|	ВременнаяТаблицаРасходы.Документ КАК Документ,
		|	ВременнаяТаблицаРасходы.Сумма - ВременнаяТаблицаРасходы.СуммаНДФЛ - ВременнаяТаблицаРасходы.СуммаПФ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаВал - ВременнаяТаблицаРасходы.СуммаНДФЛВал - ВременнаяТаблицаРасходы.СуммаПФВал КАК СуммаВал,
		|	&СодержаниеПроводкиРасчетыСПоставщиками КАК СодержаниеПроводки,
		|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПоступлениеОтПоставщика) КАК ХозяйственнаяОперация,
		|	""РасчетыСПоставщиками"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.Сумма КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПоступлениеОтПоставщика) КАК ХозяйственнаяОперация,
		|	&СодержаниеРасчетиСПерсоналомЗП КАК СодержаниеПроводки,
		|	""РасчетыСПерсоналом"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники  
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|  
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ПериодРегистрации,
		|	- ВременнаяТаблицаРасходы.СуммаНДФЛ КАК Сумма,
		|	- ВременнаяТаблицаРасходы.СуммаНДФЛВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПоступлениеОтПоставщика) КАК ХозяйственнаяОперация,     
		|	&СодержаниеРасчетиСПерсоналомНДФЛ КАК СодержаниеПроводки,
		|	""РасчетыСПерсоналом"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники 
		|	И ВременнаяТаблицаРасходы.СуммаНДФЛ <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|  
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		//|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ПериодРегистрации,
		|	- ВременнаяТаблицаРасходы.СуммаПФ КАК Сумма,
		|	- ВременнаяТаблицаРасходы.СуммаПФВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПоступлениеОтПоставщика) КАК ХозяйственнаяОперация,  
		|	&СодержаниеРасчетиСПерсоналомПФ КАК СодержаниеПроводки,
		|	""РасчетыСПерсоналом"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники 
		|	И ВременнаяТаблицаРасходы.СуммаПФ <> 0
		|;
		|        
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаНачала,  
		|	КОНЕЦПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаОкончания,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.Сумма КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ВременнаяТаблицаРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	""НачисленияИУдержания"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники 
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|     
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,   
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаНачала,
		|	КОНЕЦПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаОкончания,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.СуммаПФ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаПФВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ВременнаяТаблицаРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	""НачисленияИУдержания"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники 
		|	И ВременнаяТаблицаРасходы.СуммаПФ <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|     
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,  
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаНачала, 
		|	КОНЕЦПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаОкончания,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ВременнаяТаблицаРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	""НачисленияИУдержания"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники
		|	И ВременнаяТаблицаРасходы.СуммаНДФЛ <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаНачала,  
		|	КОНЕЦПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаОкончания,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.Сумма КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ВременнаяТаблицаРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	""НачисленияИУдержанияВРазрезеПроекта"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники 
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|     
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,   
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаНачала,
		|	КОНЕЦПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаОкончания,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.СуммаПФ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаПФВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ВременнаяТаблицаРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	""НачисленияИУдержанияВРазрезеПроекта"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники 
		|	И ВременнаяТаблицаРасходы.СуммаПФ <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|     
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,  
		|	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаНачала, 
		|	КОНЕЦПЕРИОДА(ВременнаяТаблицаРасходы.Период, МЕСЯЦ) КАК ДатаОкончания,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.Контрагент КАК Сотрудник,
		|	ВременнаяТаблицаРасходы.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасходы.Период КАК ПериодРегистрации,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ КАК Сумма,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛВал КАК СуммаВал,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	ВременнаяТаблицаРасходы.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	""НачисленияИУдержанияВРазрезеПроекта"" КАК ИмяТаблицы
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники
		|	И ВременнаяТаблицаРасходы.СуммаНДФЛ <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		//|ВЫБРАТЬ
		//|	ВременнаяТаблицаРасходы.Период КАК Период,
		//|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		//|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		//|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		//|	ВременнаяТаблицаРасходы.Номенклатура КАК Номенклатура,
		//|	ВременнаяТаблицаРасходы.Характеристика КАК Характеристика,
		//|	ВременнаяТаблицаРасходы.СчетУчетаРегл КАК СчетУчетаРегл,
		//|	ВременнаяТаблицаРасходы.Количество КАК Количество,
		////|	ВременнаяТаблицаРасходы.Сумма КАК Сумма,  
		//|	ВременнаяТаблицаРасходы.СуммаБезНДС КАК Сумма, 	
		//|	NULL КАК КоррСтруктурнаяЕдиница,
		//|	NULL КАК КоррСчетУчетаРегл,
		//|	NULL КАК КоррНоменклатура,
		//|	NULL КАК КоррХарактеристика,  
		//|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		//|	""Запасы"" КАК ИмяТаблицы
		//|ИЗ
		//|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		//|
		//|ОБЪЕДИНИТЬ ВСЕ
		//|
		//|ВЫБРАТЬ
		//|	ВременнаяТаблицаРасходы.Период,
		//|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		//|	ВременнаяТаблицаРасходы.Организация,
		//|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница,
		//|	ВременнаяТаблицаРасходы.Номенклатура,
		//|	ВременнаяТаблицаРасходы.Характеристика,
		//|	ВременнаяТаблицаРасходы.СчетУчетаРегл,
		//|	ВременнаяТаблицаРасходы.Количество,
		////|	ВременнаяТаблицаРасходы.Сумма * ВТ_Продукция.ДоляДопРасходов,  
		//|	ВременнаяТаблицаРасходы.СуммаБезНДС * ВТ_Продукция.ДоляДопРасходов, 
		//|	ВременнаяТаблицаРасходы.Подразделение,
		//|	ВТ_Продукция.Номенклатура.СчетУчетаЗапасовРегл,
		//|	ВТ_Продукция.Номенклатура,
		//|	ВТ_Продукция.Характеристика,   
		//|	ВременнаяТаблицаРасходы.Проект,
		//|	""Запасы""
		//|ИЗ
		//|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Продукция КАК ВТ_Продукция
		//|		ПО (ИСТИНА)
		//|
		//|ОБЪЕДИНИТЬ ВСЕ
		//|
		//|ВЫБРАТЬ
		//|	ВременнаяТаблицаРасходы.Период,
		//|	ВременнаяТаблицаРасходы.ВидДвижения,
		//|	ВременнаяТаблицаРасходы.Организация,
		//|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница,
		//|	ВТ_Продукция.Номенклатура,
		//|	ВТ_Продукция.Характеристика,
		//|	ВТ_Продукция.Номенклатура.СчетУчетаЗапасовРегл,
		//|	0,
		////|	ВременнаяТаблицаРасходы.Сумма * ВТ_Продукция.ДоляДопРасходов,   
		//|	ВременнаяТаблицаРасходы.СуммаБезНДС * ВТ_Продукция.ДоляДопРасходов,
		//|	ВременнаяТаблицаРасходы.Подразделение,
		//|	ВременнаяТаблицаРасходы.СчетУчетаРегл,
		//|	ВременнаяТаблицаРасходы.Номенклатура,
		//|	ВТ_Продукция.Характеристика,  
		//|	ВременнаяТаблицаРасходы.Проект,
		//|	""Запасы""
		//|ИЗ
		//|	ВТ_Продукция КАК ВТ_Продукция
		//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		//|		ПО (ИСТИНА)       
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ВТ_Продукция.Номенклатура КАК Номенклатура,
		|	ВТ_Продукция.Характеристика КАК Характеристика,
		|	ВТ_Продукция.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	0 КАК Количество,
		|	ВременнаяТаблицаРасходы.СуммаБезНДС * ВТ_Продукция.ДоляСтоимости КАК Сумма,
		|	ВременнаяТаблицаРасходы.Подразделение КАК КоррСтруктурнаяЕдиница,
		|	ВременнаяТаблицаРасходы.СчетУчетаРегл КАК КоррСчетУчетаРегл,
		|	ВременнаяТаблицаРасходы.Номенклатура КАК КоррНоменклатура,
		|	ВТ_Продукция.Характеристика КАК КоррХарактеристика,
		|	ВременнаяТаблицаРасходы.Проект КАК Проект,
		|	""Запасы"" КАК ИмяТаблицы
		|ИЗ
		|	ВТ_ПродукцияДоляСтоимости КАК ВТ_Продукция
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|		ПО (ИСТИНА)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ 
		|	1 КАК Порядок,
		|	ВременнаяТаблицаРасходы.Период КАК Период,
		|	ВременнаяТаблицаРасходы.Организация КАК Организация,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.СтруктурнаяЕдиница
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ПодразделениеДт,
		|	ВременнаяТаблицаРасходы.СчетУчетаРегл КАК СчетДт,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		//|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|			ТОГДА ВременнаяТаблицаРасходы.ВалютаРасчетов
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ВалютаДт,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаБезНДСВал + ВременнаяТаблицаРасходы.СуммаПФВал
		|		ИНАЧЕ 0
		//|	КОНЕЦ КАК ВалютнаяСуммаДт,    
		|	КОНЕЦ КАК СуммаВалДт,
		|	""Подразделения"" КАК ВидСубконтоДт1,
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница КАК СубконтоДт1,
		|	""Номенклатура"" КАК ВидСубконтоДт2,
		|	ВременнаяТаблицаРасходы.Номенклатура КАК СубконтоДт2,
		//|	"""" КАК ВидСубконтоДт3,
		//|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,     
		|	""Проекты"" КАК ВидСубконтоДт3,
		|	ВременнаяТаблицаРасходы.Проект КАК СубконтоДт3,
		|	ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл КАК СчетКт, 
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		//|			ТОГДА ВременнаяТаблицаРасходы.Валюта   
		|			ТОГДА ВременнаяТаблицаРасходы.ВалютаРасчетов
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ВалютаКт,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаБезНДСВал + ВременнаяТаблицаРасходы.СуммаПФВал
		|		ИНАЧЕ 0
		//|	КОНЕЦ КАК ВалютнаяСуммаКт, 
		|	КОНЕЦ КАК СуммаВалКт, 
		//|	""Контрагенты"" КАК ВидСубконтоКт1,           
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Контрагенты""
		|		ИНАЧЕ ""Сотрудники""
		|	КОНЕЦ КАК ВидСубконтоКт1,
		|	ВременнаяТаблицаРасходы.Контрагент КАК СубконтоКт1,
		//|	""Договоры"" КАК ВидСубконтоКт2,    
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Договоры""
		|		ИНАЧЕ ""Подразделения""
		|	КОНЕЦ КАК ВидСубконтоКт2,
		//|	ВременнаяТаблицаРасходы.Договор КАК СубконтоКт2,    
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Договор
		|		ИНАЧЕ ВременнаяТаблицаРасходы.Подразделение
		|	КОНЕЦ КАК СубконтоКт2,
		//|	"""" КАК ВидСубконтоКт3,
		//|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,      
		|	""Проекты"" КАК ВидСубконтоКт3,
		|	ВременнаяТаблицаРасходы.Проект КАК СубконтоКт3,
		|	ВременнаяТаблицаРасходы.СуммаБезНДС + ВременнаяТаблицаРасходы.СуммаПФ КАК Сумма,            
		//|	ВременнаяТаблицаРасходы.Сумма КАК Сумма,
		|	""Отражение затрат"" КАК Содержание
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ   
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты       
		|	И ВременнаяТаблицаРасходы.ТипСтруктурнойединицы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
		//|	ВременнаяТаблицаРасходы.ТипСтруктурнойединицы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)   
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ 
		|	2,
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВЫБОР
		|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.Подразделение
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл,
		|	ВЫБОР
		|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаВал * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		//|	""Подразделения"",
		|	""Склады"",
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиницаПродукция,
		|	""Номенклатура"",
		|	ВТ_ПродукцияДоляСтоимости.Номенклатура,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СчетУчетаРегл,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаВал * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		//|	""Склады"",     
		|	""Подразделения"",
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница,
		|	""Номенклатура"",
		|	ВременнаяТаблицаРасходы.Номенклатура,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО, 
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		//|	ВременнаяТаблицаРасходы.Сумма * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,
		|	ВременнаяТаблицаРасходы.СуммаБезНДС * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости + ВременнаяТаблицаРасходы.СуммаПФ * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,  
		//|	ВременнаяТаблицаРасходы.Сумма * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,
		|	""Распределение затрат""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПродукцияДоляСтоимости КАК ВТ_ПродукцияДоляСтоимости
		|		ПО (ИСТИНА)   
		|ГДЕ   
		|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты 
		|	И ВременнаяТаблицаРасходы.ТипСтруктурнойединицы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
		|
		|ОБЪЕДИНИТЬ ВСЕ  
		|
		|ВЫБРАТЬ 
		|	3 КАК Порядок,
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВЫБОР
		|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.Подразделение
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл,
		|	ВЫБОР
		|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаВал * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		//|	""Подразделения"",
		|	""Склады"",
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиницаПродукция,
		|	""Номенклатура"",
		|	ВТ_ПродукцияДоляСтоимости.Номенклатура,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		//|	ВременнаяТаблицаРасходы.СчетУчетаРегл,
		|	ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаВал * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		////|	""Склады"",     
		//|	""Подразделения"",
		//|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница,
		//|	""Номенклатура"",
		//|	ВременнаяТаблицаРасходы.Номенклатура,
		////|	"""",
		////|	НЕОПРЕДЕЛЕНО, 
		//|	""Проекты"",
		//|	ВременнаяТаблицаРасходы.Проект,  
		//|	""Контрагенты"" КАК ВидСубконтоКт1,           
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Контрагенты""
		|		ИНАЧЕ ""Сотрудники""
		|	КОНЕЦ КАК ВидСубконтоКт1,
		|	ВременнаяТаблицаРасходы.Контрагент КАК СубконтоКт1,
		//|	""Договоры"" КАК ВидСубконтоКт2,    
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Договоры""
		|		ИНАЧЕ ""Подразделения""
		|	КОНЕЦ КАК ВидСубконтоКт2,
		//|	ВременнаяТаблицаРасходы.Договор КАК СубконтоКт2,    
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Договор
		|		ИНАЧЕ ВременнаяТаблицаРасходы.Подразделение
		|	КОНЕЦ КАК СубконтоКт2,
		//|	"""" КАК ВидСубконтоКт3,
		//|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,      
		|	""Проекты"" КАК ВидСубконтоКт3,
		|	ВременнаяТаблицаРасходы.Проект КАК СубконтоКт3,
		//|	ВременнаяТаблицаРасходы.Сумма * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,
		|	ВременнаяТаблицаРасходы.СуммаБезНДС * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости + ВременнаяТаблицаРасходы.СуммаПФ * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,
		|	""Распределение затрат""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПродукцияДоляСтоимости КАК ВТ_ПродукцияДоляСтоимости
		|		ПО (ИСТИНА)  
		|ГДЕ   
		//|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники    
		|	ВременнаяТаблицаРасходы.ТипСтруктурнойединицы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад) 
		|	ИЛИ (ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Сотрудники
		|		И ВременнаяТаблицаРасходы.ТипСтруктурнойединицы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)) 
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ   
		|	4,
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.СтруктурнаяЕдиница
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасходы.СчетУчетаНДСКВозмещениюРегл,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаНДСКВозмещениюРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаНДСКВозмещениюРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаНДСВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	"""",
		|	НЕОПРЕДЕЛЕНО,
		|	"""",
		|	НЕОПРЕДЕЛЕНО,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,   
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаНДСВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	""Контрагенты"",
		|	ВременнаяТаблицаРасходы.Контрагент,
		|	""Договоры"",
		|	ВременнаяТаблицаРасходы.Договор,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,    
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СуммаНДС,
		|	""Начисление НДС""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы  
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаНДС <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ     
		|	5,
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.СтруктурнаяЕдиница
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаНДФЛВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		//|	""Контрагенты"",  
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Контрагенты""
		|		ИНАЧЕ ""Сотрудники""
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасходы.Контрагент,
		//|	""Договоры"", 
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Договоры""
		|		ИНАЧЕ ""Подразделения""
		|	КОНЕЦ,
		//|	ВременнаяТаблицаРасходы.Договор,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Договор
		|		ИНАЧЕ ВременнаяТаблицаРасходы.Подразделение
		|	КОНЕЦ,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,                        
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СчетНачисленияНДФЛ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетНачисленияНДФЛ.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетНачисленияНДФЛ.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаНДФЛВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	""КонтрагентыСотрудники"",
		|	ВременнаяТаблицаРасходы.Контрагент,
		|	"""",
		|	НЕОПРЕДЕЛЕНО,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО, 
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ,
		|	""Начисление НДФЛ""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаНДФЛ > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ    
		|	6,
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.СтруктурнаяЕдиница
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРасчетовСПоставщикомРегл.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаПФВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		//|	""Контрагенты"",   
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Контрагенты""
		|		ИНАЧЕ ""Сотрудники""
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасходы.Контрагент,
		//|	""Договоры"",
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ""Договоры""
		|		ИНАЧЕ ""Подразделения""
		|	КОНЕЦ,
		//|	ВременнаяТаблицаРасходы.Договор,         
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Договор
		|		ИНАЧЕ ВременнаяТаблицаРасходы.Подразделение
		|	КОНЕЦ,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,   
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СчетНачисленииПФСотрудника,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетНачисленииПФСотрудника.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетНачисленииПФСотрудника.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаПФВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	""Контрагенты"",
		//|	ВременнаяТаблицаРасходы.Контрагент,   
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Контрагент
		|		ИНАЧЕ ВременнаяТаблицаРасходы.ВидУдержанияПФ.Контрагент
		|	КОНЕЦ,
		|	""Договоры"",
		//|	ВременнаяТаблицаРасходы.Договор,       
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Договор
		|		ИНАЧЕ ВременнаяТаблицаРасходы.ВидУдержанияПФ.Договор
		|	КОНЕЦ,
		|	""КонтрагентыСотрудники"",
		|	ВременнаяТаблицаРасходы.Контрагент,
		|	ВременнаяТаблицаРасходы.СуммаПФ,
		|	""Налог пенсионного фонда""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаПФ > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ 
		|	7,
		|	ВременнаяТаблицаРасходы.Период,
		|	ВременнаяТаблицаРасходы.Организация,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетЗатратРеглПФ.УчетПоПодразделениям
		|			ТОГДА ВременнаяТаблицаРасходы.СтруктурнаяЕдиница
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасходы.СчетЗатратРеглПФ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетЗатратРеглПФ.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетЗатратРеглПФ.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаПФВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	""Подразделения"",
		|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница,
		|	""КонтрагентыСотрудники"",
		|	ВременнаяТаблицаРасходы.КонтрагентПФ,
		//|	"""",
		//|	НЕОПРЕДЕЛЕНО,                      
		|	""Проекты"",
		|	ВременнаяТаблицаРасходы.Проект,
		|	ВременнаяТаблицаРасходы.СчетНачисленииПФОрганизации,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетНачисленииПФОрганизации.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.СчетНачисленииПФОрганизации.Валютный
		|			ТОГДА ВременнаяТаблицаРасходы.СуммаПФВал
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	""Контрагенты"",
		//|	ВременнаяТаблицаРасходы.Контрагент,   
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Контрагент
		|		ИНАЧЕ ВременнаяТаблицаРасходы.ВидУдержанияПФ.Контрагент
		|	КОНЕЦ,
		|	""Договоры"",
		//|	ВременнаяТаблицаРасходы.Договор,  
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВременнаяТаблицаРасходы.Договор
		|		ИНАЧЕ ВременнаяТаблицаРасходы.ВидУдержанияПФ.Договор
		|	КОНЕЦ,
		|	""КонтрагентыСотрудники"",
		|	ВременнаяТаблицаРасходы.Контрагент,
		|	ВременнаяТаблицаРасходы.СуммаПФ,
		|	""Удержание организации для пенсионного фонда""
		|ИЗ
		|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		|ГДЕ
		|	ВременнаяТаблицаРасходы.СуммаПФ > 0
		//|
		//|ОБЪЕДИНИТЬ ВСЕ
		//|
		//|ВЫБРАТЬ 
		//|	7,
		//|	ВременнаяТаблицаРасходы.Период,
		//|	ВременнаяТаблицаРасходы.Организация,
		//|	ВЫБОР
		//|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.УчетПоПодразделениям
		//|			ТОГДА ВременнаяТаблицаРасходы.Подразделение
		//|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		//|	КОНЕЦ,
		//|	ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл,
		//|	ВЫБОР
		//|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		//|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		//|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ВТ_ПродукцияДоляСтоимости.Номенклатура.СчетУчетаЗапасовРегл.Валютный
		//|			ТОГДА ВременнаяТаблицаРасходы.СуммаВал * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости
		//|		ИНАЧЕ 0
		//|	КОНЕЦ,
		////|	""Подразделения"",
		//|	""Склады"",
		//|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиницаПродукция,
		//|	""Номенклатура"",
		//|	ВТ_ПродукцияДоляСтоимости.Номенклатура,
		////|	"""",
		////|	НЕОПРЕДЕЛЕНО,
		//|	""Проекты"",
		//|	ВременнаяТаблицаРасходы.Проект,
		//|	ВременнаяТаблицаРасходы.СчетУчетаРегл,
		//|	ВЫБОР
		//|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		//|			ТОГДА ВременнаяТаблицаРасходы.Валюта
		//|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ВременнаяТаблицаРасходы.СчетУчетаРегл.Валютный
		//|			ТОГДА ВременнаяТаблицаРасходы.СуммаВал * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости
		//|		ИНАЧЕ 0
		//|	КОНЕЦ,
		////|	""Склады"",     
		//|	""Подразделения"",
		//|	ВременнаяТаблицаРасходы.СтруктурнаяЕдиница,
		//|	""Номенклатура"",
		//|	ВременнаяТаблицаРасходы.Номенклатура,
		////|	"""",
		////|	НЕОПРЕДЕЛЕНО, 
		//|	""Проекты"",
		//|	ВременнаяТаблицаРасходы.Проект,
		////|	ВременнаяТаблицаРасходы.Сумма * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,
		//|	ВременнаяТаблицаРасходы.СуммаБезНДС * ВТ_ПродукцияДоляСтоимости.ДоляСтоимости,
		//|	""Распределение затрат""
		//|ИЗ
		//|	ВременнаяТаблицаРасходы КАК ВременнаяТаблицаРасходы
		//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПродукцияДоляСтоимости КАК ВТ_ПродукцияДоляСтоимости
		//|		ПО (ИСТИНА)   
		//|ГДЕ   
		//|	ВременнаяТаблицаРасходы.Контрагент ССЫЛКА Справочник.Контрагенты 
		//|	И ВременнаяТаблицаРасходы.ТипСтруктурнойединицы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
		|";
		
		МассивРезультаты = Запрос.ВыполнитьПакет();
		
		Для каждого КлючИЗначение из СтруктураТаблиц цикл
			 СтруктураТаблиц.Вставить(КлючИЗначение.Ключ, МассивРезультаты[КлючИЗначение.Значение].Выгрузить());
		КонецЦикла;	
		
		Возврат СтруктураТаблиц;
		
	КонецФункции
	
	Процедура СформироватьТаблицаЗапасы_ДопРасходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	1 КАК ПорядокДанных,
		|	СборкаЗапасовДополнительныеРасходы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовДополнительныеРасходы.Ссылка КАК Ссылка,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	СборкаЗапасовДополнительныеРасходы.Ссылка.Организация КАК Организация,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовДополнительныеРасходы.Подразделение КАК СтруктурнаяЕдиница,
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|		КОГДА СборкаЗапасовДополнительныеРасходы.Проект <> ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовДополнительныеРасходы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		|	КОНЕЦ КАК Проект,
		|	СборкаЗапасовДополнительныеРасходы.Количество КАК Количество,
		|	СборкаЗапасовДополнительныеРасходы.Сумма КАК Сумма,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл
		|ПОМЕСТИТЬ ВременнаяТаблицаДополнительныеРасходы
		|ИЗ
		|	Документ.СборкаЗапасов.ДополнительныеРасходы КАК СборкаЗапасовДополнительныеРасходы
		|ГДЕ
		|	СборкаЗапасовДополнительныеРасходы.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	1 КАК ПорядокДанных,
		|	СборкаЗапасовДополнительныеРасходы.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	СборкаЗапасовДополнительныеРасходы.Организация КАК Организация,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовДополнительныеРасходы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовДополнительныеРасходы.Количество КАК Количество,
		|	СборкаЗапасовДополнительныеРасходы.Сумма КАК Сумма,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	СборкаЗапасовДополнительныеРасходы.Проект КАК Проект,
		|	СборкаЗапасовДополнительныеРасходы.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница
		|ИЗ
		|	ВременнаяТаблицаДополнительныеРасходы КАК СборкаЗапасовДополнительныеРасходы
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	2,
		|	СборкаЗапасовДополнительныеРасходы.Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	СборкаЗапасовДополнительныеРасходы.Организация,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура,
		|	СборкаЗапасовДополнительныеРасходы.СтруктурнаяЕдиница,
		|	СборкаЗапасовДополнительныеРасходы.Количество,
		|	СборкаЗапасовДополнительныеРасходы.Сумма,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура.СчетУчетаЗапасовРегл,
		|	СборкаЗапасовДополнительныеРасходы.Номенклатура.СчетУчетаЗатратРегл,
		|	СборкаЗапасовДополнительныеРасходы.Проект,
		|	ВременнаяТаблицаШапка.СтруктурнаяЕдиница
		|ИЗ
		|	ВременнаяТаблицаДополнительныеРасходы КАК СборкаЗапасовДополнительныеРасходы,
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка";
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
		
		Выборка = Запрос.Выполнить().Выбрать();
		//Пока Выборка.Следующий() Цикл
		//	СтрокаПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Добавить(); 
		//	ЗаполнитьЗначенияСвойств(СтрокаПриход, Выборка);
		//	
		//КонецЦикла;
		
	КонецПроцедуры	
		
	
	// © Шишка В.В. [26 декабря 2023]  КОНЕЦ
	//--------------------------------------------------------------------------
	
	
	// Инициализирует таблицы значений, содержащие данные табличных частей документа.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура ИнициализироватьДанныеДокументаРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства) Экспорт
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
		|	СборкаЗапасовЗапасы.Ссылка.Дата КАК Период,
		|	СборкаЗапасовЗапасы.КлючСвязи КАК КлючСвязи,
		|	СборкаЗапасовЗапасы.Ссылка КАК Ссылка,
		|	&Организация КАК Организация,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовЗапасы.Ссылка.Ячейка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК Ячейка,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница = СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции
		|	КОНЕЦ КАК СтруктурнаяЕдиницаПродукции,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукцииНаСклад,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ОрдерныйСклад КАК ОрдерныйСкладПродукции,
		|	ВЫБОР
		|		КОГДА &УчетПоЯчейкам
		|			ТОГДА СборкаЗапасовЗапасы.Ссылка.ЯчейкаПродукции
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
		|	КОНЕЦ КАК ЯчейкаПродукции,
		|	СборкаЗапасовЗапасы.Спецификация КАК Спецификация,

		//ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовЗапасы.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовЗапасы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовЗапасы.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовЗапасы.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
		|	КОНЕЦ КАК КоррПроект,
		//ПодольскийАИ #УчетПроектов 13.10.2020 }

		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатрат
		|	КОНЕЦ КАК СчетУчетаПродукции,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетУчетаРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиницаПродукции.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|			ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
		|	КОНЕЦ КАК СчетУчетаПродукцииРегл,
		|	СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл,
		|	СборкаЗапасовЗапасы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовЗапасы.СтранаПроисхождения КАК СтранаПроисхождения,
		|	СборкаЗапасовЗапасы.НомерГТД КАК НомерГТД,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовЗапасы.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовЗапасы.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Партия,
		|	СборкаЗапасовЗапасы.Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК КоррЗаказПокупателя,
		|	СборкаЗапасовЗапасы.Ссылка.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовЗапасы.Ссылка.ЗаказНаПроизводство = ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ СборкаЗапасовЗапасы.Ссылка.ЗаказНаПроизводство
		|	КОНЕЦ КАК ИсточникОбеспечения,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|			ТОГДА СборкаЗапасовЗапасы.Количество
		|		ИНАЧЕ СборкаЗапасовЗапасы.Количество * СборкаЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент
		|	КОНЕЦ КАК Количество,
		|	0 КАК Сумма,
		|	ВЫРАЗИТЬ(&СборкаЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки,
		|	ВЫРАЗИТЬ(&СборкаЗапасов КАК СТРОКА(100)) КАК Содержание,
		|	&ДатаОбновленияНаРелиз_1_2_1 КАК ДатаОбновленияНаРелиз_1_2_1,
		|	СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница КАК Подразделение,
		//+Lazare 10.11.2022
		|	СборкаЗапасовЗапасы.СрокГодности КАК СрокГодности,
		|	СборкаЗапасовЗапасы.Номенклатура.ВключенВрегистр КАК ВключенВрегистр
		//-Lazare 10.11.2022
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,СборкаЗапасовЗапасы.Ссылка КАК ДокументПоступления
		|	,СборкаЗапасовЗапасы.Ссылка КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ПОМЕСТИТЬ ВременнаяТаблицаПродукция
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Ссылка = &Ссылка
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 1
		|ВЫБРАТЬ
		|	МИНИМУМ(ТаблицаПродукция.НомерСтроки) КАК НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	НЕОПРЕДЕЛЕНО КАК СценарийПланирования,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	НЕОПРЕДЕЛЕНО КАК КоррСтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиницаПродукции,
		|	ТаблицаПродукция.СчетУчета КАК СчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции КАК СчетУчетаПродукции,
		|	НЕОПРЕДЕЛЕНО КАК КоррСчетУчета,
		|	ТаблицаПродукция.СчетУчетаРегл КАК СчетУчетаРегл,
		|	НЕОПРЕДЕЛЕНО КАК СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл КАК СчетУчетаПродукцииРегл,
		|	НЕОПРЕДЕЛЕНО КАК КоррСчетУчетаРегл,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	НЕОПРЕДЕЛЕНО КАК ДокументПоступления, //Малышев 2024-11-19 ONESOFT-3602
		|	НЕОПРЕДЕЛЕНО КАК КоррДокументПоступления, // Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	НЕОПРЕДЕЛЕНО КАК КоррНоменклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	НЕОПРЕДЕЛЕНО КАК КоррХарактеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	НЕОПРЕДЕЛЕНО КАК КоррПартия,
		|	ТаблицаПродукция.Спецификация КАК Спецификация,
		|	НЕОПРЕДЕЛЕНО КАК КоррСпецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект КАК Проект,
		|	ТаблицаПродукция.Проект КАК КоррПроект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ КАК Проект,
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ КАК КоррПроект,
		////Lazare_
		|	ТаблицаПродукция.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПродукция.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ТаблицаПродукция.КоррЗаказПокупателя КАК КоррЗаказПокупателя,
		|	НЕОПРЕДЕЛЕНО КАК СчетДт,
		|	НЕОПРЕДЕЛЕНО КАК СчетКт,
		|	НЕОПРЕДЕЛЕНО КАК СчетДтРегл,
		|	НЕОПРЕДЕЛЕНО КАК СчетКтРегл,
		|	НЕОПРЕДЕЛЕНО КАК Содержание,
		|	ТаблицаПродукция.СодержаниеПроводки КАК СодержаниеПроводки,
		|	ЛОЖЬ КАК ЗатратыНаВыпуск,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	СУММА(ТаблицаПродукция.Сумма) КАК Сумма,
		|	ЛОЖЬ КАК ФиксированнаяСтоимость,
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукции,
		|	ТаблицаПродукция.СчетУчета,
		|	ТаблицаПродукция.СчетУчетаПродукции,
		|	ТаблицаПродукция.СчетУчетаРегл,
		|	ТаблицаПродукция.СчетУчетаПродукцииРегл,
		|	ТаблицаПродукция.СчетУчетаЗатратРегл,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Спецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект,
		|	ТаблицаПродукция.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ,
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаПродукция.ЗаказПокупателя,
		|	ТаблицаПродукция.ЗаказНаПроизводство,
		|	ТаблицаПродукция.КоррЗаказПокупателя,
		|	ТаблицаПродукция.СодержаниеПроводки,
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 2
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад КАК СтруктурнаяЕдиницаПродукции,
		|	ТаблицаПродукция.Ячейка КАК Ячейка,
		|	ТаблицаПродукция.ЯчейкаПродукции КАК ЯчейкаПродукции,
		|	ТаблицаПродукция.ОрдерныйСклад КАК ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции КАК ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект КАК Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ КАК Проект,
		////Lazare_
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления КАК ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.Период < ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаПродукция.Период,
		|	&Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ,
		////Lazare_
		|	СУММА(ТаблицаПродукция.Количество),
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления КАК ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	НЕ ТаблицаПродукция.ОрдерныйСклад
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ, 
		////Lazare_
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаПродукция.Период,
		|	&Организация,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ,
		////Lazare_
		|	СУММА(ТаблицаПродукция.Количество),
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления КАК ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления КАК КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСклад
		|	И НЕ ТаблицаПродукция.ОрдерныйСкладПродукции
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.ЯчейкаПродукции,
		|	ТаблицаПродукция.Ячейка,
		|	ТаблицаПродукция.ОрдерныйСклад,
		|	ТаблицаПродукция.ОрдерныйСкладПродукции,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		|	ТаблицаПродукция.Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		////_lazare
		//|	ВЫБОР
		//|		КОГДА ТаблицаПродукция.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|			ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		//|		ИНАЧЕ ТаблицаПродукция.Проект
		//|	КОНЕЦ,
		////Lazare_
		|	ТаблицаПродукция.Подразделение
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|	,ТаблицаПродукция.ДокументПоступления
		|	,ТаблицаПродукция.КоррДокументПоступления
		// Kuzmich, 26.01.2025, (ONESOFT-3713) Закрытие Месяца - Расчет фактической себестоимости
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 3
		|ВЫБРАТЬ
		|	ТаблицаПродукция.Период КАК Период,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.Проект КАК Проект,  //Малышев 2024-06-23
		|	ТаблицаПродукция.Спецификация КАК Спецификация,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.ЗаказПокупателя,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Проект,  //Малышев 2024-06-23
		|	ТаблицаПродукция.Спецификация,
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 4
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.ЗаказНаПроизводство,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 5
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСклад
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Подразделение
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	СУММА(ТаблицаПродукция.Количество),
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСкладПродукции
		|	И ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад <> ТаблицаПродукция.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Подразделение
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	СУММА(ТаблицаПродукция.Количество),
		|	NULL
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСкладПродукции
		|	И ТаблицаПродукция.Период < ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|	И ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад <> ТаблицаПродукция.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 6
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	СУММА(ТаблицаПродукция.Количество) КАК Количество,
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|ГДЕ
		|	ТаблицаПродукция.ОрдерныйСклад
		|	И ТаблицаПродукция.Период >= ТаблицаПродукция.ДатаОбновленияНаРелиз_1_2_1
		|	И ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад <> ТаблицаПродукция.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаПродукция.Период,
		|	ТаблицаПродукция.СтруктурнаяЕдиница,
		|	ТаблицаПродукция.Организация,
		|	ТаблицаПродукция.Номенклатура,
		|	ТаблицаПродукция.Характеристика,
		|	ТаблицаПродукция.Партия,
		|	ТаблицаПродукция.Подразделение
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 7
		|ВЫБРАТЬ
		|	МИНИМУМ(СборкаЗапасовПродукция.НомерСтроки) КАК НомерСтроки,
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовПродукция.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Характеристика,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовПродукция.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ КАК Партия,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовПродукция.Партия.Статус
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ПустаяСсылка)
		|	КОНЕЦ КАК ПартияСтатус,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаЗапасов,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаРегл,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетУчетаЗапасовРегл,
		|	СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат КАК СчетУчетаЗатратРегл,
		|	СборкаЗапасовПродукция.Спецификация КАК Спецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		//|	ВЫБОР
		//|		КОГДА &УчетПоПроектам
		//|			ТОГДА СборкаЗапасовПродукция.Проект
		//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|	КОНЕЦ КАК Проект,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		//_Lazare
 		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовПродукция.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовПродукция.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
		|	КОНЕЦ КАК Проект,
		//Lazare_

		|	СУММА(ВЫБОР
		|			КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовПродукция.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|				ТОГДА СборкаЗапасовПродукция.Количество
		|			ИНАЧЕ СборкаЗапасовПродукция.Количество * СборкаЗапасовПродукция.ЕдиницаИзмерения.Коэффициент
		|		КОНЕЦ) КАК Количество,
		|	СУММА(ВЫБОР
		|			КОГДА ТИПЗНАЧЕНИЯ(СборкаЗапасовПродукция.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
		|				ТОГДА СборкаЗапасовПродукция.Резерв
		|			ИНАЧЕ СборкаЗапасовПродукция.Резерв * СборкаЗапасовПродукция.ЕдиницаИзмерения.Коэффициент
		|		КОНЕЦ) КАК Резерв,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКт,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ КАК СчетКтРегл,
		|	ЛОЖЬ КАК Распределено,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница КАК Подразделение
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|ГДЕ
		|	СборкаЗапасовПродукция.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	СборкаЗапасовПродукция.Номенклатура,
		|	СборкаЗапасовПродукция.СтранаПроисхождения,
		|	СборкаЗапасовПродукция.НомерГТД,
		|	СборкаЗапасовПродукция.Спецификация,
		////ПодольскийАИ #УчетПроектов 13.10.2020 {
		//|	ВЫБОР
		//|		КОГДА &УчетПоПроектам
		//|			ТОГДА СборкаЗапасовПродукция.Проект
		//|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		//|	КОНЕЦ,
		////ПодольскийАИ #УчетПроектов 13.10.2020 }
		//_Lazare
 		|	ВЫБОР
		|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)   //Малышев 2024-06-10 ONESOFT-3093 
		|		КОГДА СборкаЗапасовПродукция.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
		|			ТОГДА СборкаЗапасовПродукция.Проект
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
		|	КОНЕЦ,
		//Lazare_
		|	ВЫБОР
		|		КОГДА &ИспользоватьХарактеристики
		|			ТОГДА СборкаЗапасовПродукция.Характеристика
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовПродукция.Партия
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА &ИспользоватьПартии
		|			ТОГДА СборкаЗапасовПродукция.Партия.Статус
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ПустаяСсылка)
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиницаЗапасов.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ,
		|	СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат,
		|	СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасов
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатрат
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
		|				ИЛИ СборкаЗапасовПродукция.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
		|			ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СборкаЗапасовПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|					ТОГДА СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл
		|				ИНАЧЕ СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл
		|			КОНЕЦ
		|	КОНЕЦ
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 8
		|ВЫБРАТЬ
		|	ТаблицаПродукция.Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
		|	ТаблицаПродукция.Период КАК ДатаСобытия,
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииСерийныхНомеров.Приход) КАК Операция,
		|	ТаблицаСерийныеНомера.СерийныйНомер КАК СерийныйНомер,
		|	&Организация КАК Организация,
		|	ТаблицаПродукция.Номенклатура КАК Номенклатура,
		|	ТаблицаПродукция.Характеристика КАК Характеристика,
		|	ТаблицаПродукция.Партия КАК Партия,
		|	ТаблицаПродукция.СтруктурнаяЕдиницаПродукцииНаСклад КАК СтруктурнаяЕдиница,
		|	ТаблицаПродукция.ЯчейкаПродукции КАК Ячейка,
		|	1 КАК Количество,
		|	ТаблицаПродукция.Подразделение
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ТаблицаПродукция
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.СерийныеНомера КАК ТаблицаСерийныеНомера
		|		ПО ТаблицаПродукция.Ссылка = ТаблицаСерийныеНомера.Ссылка
		|			И ТаблицаПродукция.КлючСвязи = ТаблицаСерийныеНомера.КлючСвязи
		|ГДЕ
		|	ТаблицаПродукция.Ссылка = &Ссылка
		|	И &ИспользоватьСерийныеНомера
		|	И НЕ ТаблицаПродукция.ОрдерныйСклад
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТаблицаЗапасы.Ссылка.Дата,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаЗапасы.Ссылка.Дата,
		|	ЗНАЧЕНИЕ(Перечисление.ОперацииСерийныхНомеров.Расход),
		|	ТаблицаСерийныеНомера.СерийныйНомер,
		|	&Организация,
		|	ТаблицаЗапасы.Номенклатура,
		|	ТаблицаЗапасы.Характеристика,
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.Ссылка.СтруктурнаяЕдиницаЗапасов,
		|	ТаблицаЗапасы.Ссылка.ЯчейкаЗапасов,
		|	1,
		|	ТаблицаЗапасы.Ссылка.СтруктурнаяЕдиница
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК ТаблицаЗапасы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.СерийныеНомераПродукция КАК ТаблицаСерийныеНомера
		|		ПО ТаблицаЗапасы.Ссылка = ТаблицаСерийныеНомера.Ссылка
		|			И ТаблицаЗапасы.КлючСвязи = ТаблицаСерийныеНомера.КлючСвязи
		|ГДЕ
		|	ТаблицаСерийныеНомера.Ссылка = &Ссылка
		|	И &ИспользоватьСерийныеНомера
		|;
		|
		|//////////////////////////////////////////////////////////////////////////////// 9
		|ВЫБРАТЬ
		|	ТаблицаЗапасыНаСкладах.НомерСтроки КАК НомерСтроки,
		|	ТаблицаЗапасыНаСкладах.Ссылка.Дата КАК Период,
		|	ТаблицаЗапасыНаСкладах.Ссылка.Организация КАК Организация,
		|	ТаблицаЗапасыНаСкладах.Номенклатура КАК Номенклатура,
		|	ТаблицаЗапасыНаСкладах.Характеристика КАК Характеристика,
		|	ТаблицаЗапасыНаСкладах.Партия КАК Партия,
		|	ТаблицаЗапасыНаСкладах.НомерГТД КАК НомерГТД,
		|	ТаблицаЗапасыНаСкладах.СтранаПроисхождения КАК СтранаПроисхождения,
		|	ТаблицаЗапасыНаСкладах.Количество КАК Количество,
		|	ТаблицаЗапасыНаСкладах.Ссылка.СтруктурнаяЕдиница КАК Подразделение
		|ПОМЕСТИТЬ СписаниеПоНомерамГТД
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК ТаблицаЗапасыНаСкладах
		|ГДЕ
		|	ТаблицаЗапасыНаСкладах.Ссылка = &Ссылка
		|	И ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
		|	И ТаблицаЗапасыНаСкладах.СтранаПроисхождения <> ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяССылка)
		|	И ТаблицаЗапасыНаСкладах.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяССылка)";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
		Запрос.УстановитьПараметр("ИспользоватьПартии",  СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
		Запрос.УстановитьПараметр("УчетПоЯчейкам",  СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоЯчейкам);
		Запрос.УстановитьПараметр("ИспользоватьТехоперации",  СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьТехоперации);
		
		//ПодольскийАИ #УчетПроектов 05.10.2020 {
		Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
		//ПодольскийАИ #УчетПроектов 05.10.2020 }
		
		Запрос.УстановитьПараметр("ИспользоватьСерийныеНомера", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьСерийныеНомера);
		
		// Временно: изменение движений по ордерному складу.
		ДатаОбновленияНаРелиз_1_2_1 = Константы.ДатаОбновленияНаРелиз_1_2_1.Получить();
		Запрос.УстановитьПараметр("ДатаОбновленияНаРелиз_1_2_1", ДатаОбновленияНаРелиз_1_2_1);
		
		Запрос.УстановитьПараметр("СборкаЗапасов", НСтр("ka='მარაგების შეკრება';ru='Сборка запасов';en='stock assembly'"));
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПродукция", МассивРезультатов[1].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасы", СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция.СкопироватьКолонки());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыНаСкладах", МассивРезультатов[2].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВыпускПродукции", МассивРезультатов[3].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗаказыНаПроизводство", МассивРезультатов[4].Выгрузить());
		
		СформироватьТаблицаЗапасыВРазрезеГТДРазборка(СтруктураДополнительныеСвойства);
		
		//Малышев 2024-07-15
		СформироватьТаблицаЗапасы_ДопРасходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);	
	
		
		// Автоперемещение продукции (дополним таблицу ТаблицаЗапасыНаСкладах).
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Количество() - 1 Цикл
			
			СтрокаТаблицаЗапасыНаСкладах = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах[н];
			
			Если (СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиницаПродукции = СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиница
				И СтрокаТаблицаЗапасыНаСкладах.ЯчейкаПродукции <> СтрокаТаблицаЗапасыНаСкладах.Ячейка)
				ИЛИ СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиницаПродукции <> СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиница Тогда
				
				// Расход.
				Если (СтрокаТаблицаЗапасыНаСкладах.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (СтрокаТаблицаЗапасыНаСкладах.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ СтрокаТаблицаЗапасыНаСкладах.ОрдерныйСклад) Тогда
					
					СтрокаТаблицыРасход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыРасход, СтрокаТаблицаЗапасыНаСкладах);
					
					СтрокаТаблицыРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
					
				КонецЕсли;
				
				// Приход.
				Если (СтрокаТаблицаЗапасыНаСкладах.Период < ДатаОбновленияНаРелиз_1_2_1)
					ИЛИ (СтрокаТаблицаЗапасыНаСкладах.Период >= ДатаОбновленияНаРелиз_1_2_1
					И НЕ СтрокаТаблицаЗапасыНаСкладах.ОрдерныйСкладПродукции
					И НЕ СтрокаТаблицаЗапасыНаСкладах.ОрдерныйСклад) Тогда
					
					СтрокаТаблицыПриход = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыНаСкладах);
					
					СтрокаТаблицыПриход.СтруктурнаяЕдиница = СтрокаТаблицаЗапасыНаСкладах.СтруктурнаяЕдиницаПродукции;
					СтрокаТаблицыПриход.Ячейка = СтрокаТаблицаЗапасыНаСкладах.ЯчейкаПродукции;
					
				КонецЕсли;
				
			КонецЕсли;
			
		КонецЦикла;
		
		// Автоперемещение продукции (сформируем таблицу ТаблицаЗапасыКПоступлениюНаСклады).
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыКПоступлениюНаСклады", МассивРезультатов[5].Выгрузить());
		
		// Автоперемещение продукции (сформируем таблицу ТаблицаЗапасыКРасходуСоСкладов).
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыКРасходуСоСкладов", МассивРезультатов[6].Выгрузить());
		
		// Формирование проводок документа.
		УправлениеНебольшойФирмойСервер.СформироватьТаблицуПроводок(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Сформируем таблицу по размещению заказов.
		СформироватьТаблицаРазмещениеЗаказовРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Сформируем таблицу распределения материалов.
		ТаблицаПродукция = МассивРезультатов[7].Выгрузить();
		СформироватьТаблицуРаспределенияМатериаловРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, ТаблицаПродукция);
		
		// Запасы.
		СуммаСборки = 0;
		ИнициализироватьДанныеПоЗапасамРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
		
		// Продукция.
		СформироватьТаблицаЗапасыПродукцияРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства, СуммаСборки);
		
		// Отходы.
		ИнициализироватьДанныеПоОтходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		
		// Серийные номера
		Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах.Количество()>0 Тогда
			РезультатЗапроса8 = МассивРезультатов[8].Выгрузить();
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераГарантии", РезультатЗапроса8);
			Если СтруктураДополнительныеСвойства.УчетнаяПолитика.ОстаткиСерийныхНомеров Тогда
				СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераОстатки", РезультатЗапроса8);
			Иначе
				СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераОстатки", Новый ТаблицаЗначений);
			КонецЕсли;
		Иначе
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераГарантии", Новый ТаблицаЗначений);
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерийныеНомераОстатки", Новый ТаблицаЗначений);
		КонецЕсли; 
		
		//+Lazare 10.11.2022
		Если СтруктураДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара Тогда 			
			СформироватьТаблицаКонтрольСроковГодностиТовараЗапасыРазборка(ДокументСсылкаСборкаЗапасов,СтруктураДополнительныеСвойства);
			СформироватьТаблицаКонтрольСроковГодностиТовараПродукция(ДокументСсылкаСборкаЗапасов,СтруктураДополнительныеСвойства);
		КонецЕсли;
		//+Lazare 10.11.2022
		
		//--------------------------------------------------------------------------
		// © Шишка В.В. [26 декабря 2023]  НАЧАЛО
		СформироватьТаблицу_ДополнитьТаблицыДвиженияПоДопРасходам(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);	
		// © Шишка В.В. [26 декабря 2023]  КОНЕЦ
		//--------------------------------------------------------------------------

	КонецПроцедуры // ИнициализироватьДанныеДокументаРазборка()
	
	// Инициализирует таблицы значений, содержащие данные табличных частей документа.
	// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
	//
	Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства) Экспорт
		
		//Малышев 2024-07-09  ONESOFT-3310 (--
		СтруктураТаблицПоДопРасходам = СформироватьТаблицу_ДополнитьТаблицыДвиженияПоДопРасходам_ПолучитьСтруктуруТаблиц(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
		Если СтруктураТаблицПоДопРасходам.Количество()>0 тогда
			СтруктураДополнительныеСвойства.ДляПроведения.Вставить("СтруктураТаблицПоДопРасходам",СтруктураТаблицПоДопРасходам);	
		КонецЕсли;	
		//Малышев 2024-07-09  ONESOFT-3310 --)
		
		Если ДокументСсылкаСборкаЗапасов.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда
			
			ИнициализироватьДанныеДокументаСборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
			
		Иначе
			
			ИнициализироватьДанныеДокументаРазборка(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства);
			
		КонецЕсли;
		
		СформироватьТаблицаРезервированиеЗапасов(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства); // Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
		
	КонецПроцедуры // ИнициализироватьДанныеДокумента()
	
	////////////////////////////////////////////////////////////////////////////////
	// КОНТРОЛЬ ОСТАТКОВ
	
	// Выполняет контроль возникновения отрицательных остатков.
	//
	Процедура ВыполнитьКонтроль(ДокументСсылкаСборкаЗапасов, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь, ПроверятьИтогиРегистров = ЛОЖЬ) Экспорт
		// Kuzmich, 25.07.2019, Контролировать остатки запасов на складах при проведении документов
		Если Константы.КонтролироватьОстаткиЗапасовНаСкладахПриПроведении.Получить() Тогда
			
			Результат = УправлениеНебольшойФирмойСервер.ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(ДокументСсылкаСборкаЗапасов,ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиницаЗапасов);
			
			Если НЕ Результат.Пустой() Тогда
				ДокументСсылкаСборкаЗапасов_Объект = ДокументСсылкаСборкаЗапасов.ПолучитьОбъект();
				ВыборкаИзРезультатаЗапроса = Результат.Выбрать();
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументСсылкаСборкаЗапасов_Объект, ВыборкаИзРезультатаЗапроса, Отказ);
			КонецЕсли;			
			
		КонецЕсли;
		// Kuzmich, 25.07.2019, Контролировать остатки запасов на складах при проведении документов
		
		Если Не УправлениеНебольшойФирмойСервер.ВыполнитьКонтрольОстатков() Тогда
			Возврат;
		КонецЕсли;
		
		СтруктураВременныеТаблицы = ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы;
		
		// Если временные таблицы "ДвиженияЗаказыНаПроизводствоИзменение", 
		// "ДвиженияРазмещениеЗаказовИзменение", "ДвиженияЗапасыИзменение"
		// содержат записи, необходимо выполнить контроль реализации товаров.
		
		Если СтруктураВременныеТаблицы.ДвиженияЗаказыНаПроизводствоИзменение
			ИЛИ СтруктураВременныеТаблицы.ДвиженияРазмещениеЗаказовИзменение
			ИЛИ СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
			ИЛИ СтруктураВременныеТаблицы.ДвиженияЗапасыВРазрезеГТДИзменение
			ИЛИ СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
			ИЛИ СтруктураВременныеТаблицы.ДвиженияЗапасыПринятыеИзменение
			ИЛИ СтруктураВременныеТаблицы.ДвиженияСерийныеНомераИзменение
			ИЛИ (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И СтруктураВременныеТаблицы.ДвиженияРезервированиеЗапасовИзменение) // Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Тогда
			
			Запрос = Новый Запрос;
			Запрос.Текст = 
			//Малышев 2024-12-03 ONESOFT-3670 (--
			УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКонтроля_ОстаткиЗапасовНаСкладе(ДополнительныеСвойства)  //[0]
			//"ВЫБРАТЬ
			//|	ДвиженияЗапасыНаСкладахИзменение.НомерСтроки КАК НомерСтроки,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыНаСкладахИзменение.Организация) КАК ОрганизацияПредставление,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыНаСкладахИзменение.СтруктурнаяЕдиница) КАК СтруктурнаяЕдиницаПредставление,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыНаСкладахИзменение.Номенклатура) КАК НоменклатураПредставление,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыНаСкладахИзменение.Характеристика) КАК ХарактеристикаПредставление,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыНаСкладахИзменение.Партия) КАК ПартияПредставление,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыНаСкладахИзменение.Ячейка) КАК ЯчейкаПредставление,
			//|	ЗапасыНаСкладахОстатки.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
			//|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ЗапасыНаСкладахОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
			//|	ЕСТЬNULL(ДвиженияЗапасыНаСкладахИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0) КАК ОстатокЗапасыНаСкладах,
			//|	ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокЗапасыНаСкладах
			//|ИЗ
			//|	ДвиженияЗапасыНаСкладахИзменение КАК ДвиженияЗапасыНаСкладахИзменение
			//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(
			//|				&МоментКонтроля,
			//|				(Организация, СтруктурнаяЕдиница, Номенклатура, Характеристика, Партия, Ячейка) В
			//|					(ВЫБРАТЬ
			//|						ДвиженияЗапасыНаСкладахИзменение.Организация КАК Организация,
			//|						ДвиженияЗапасыНаСкладахИзменение.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
			//|						ДвиженияЗапасыНаСкладахИзменение.Номенклатура КАК Номенклатура,
			//|						ДвиженияЗапасыНаСкладахИзменение.Характеристика КАК Характеристика,
			//|						ДвиженияЗапасыНаСкладахИзменение.Партия КАК Партия,
			//|						ДвиженияЗапасыНаСкладахИзменение.Ячейка КАК Ячейка
			//|					ИЗ
			//|						ДвиженияЗапасыНаСкладахИзменение КАК ДвиженияЗапасыНаСкладахИзменение)) КАК ЗапасыНаСкладахОстатки
			//|		ПО ДвиженияЗапасыНаСкладахИзменение.Организация = ЗапасыНаСкладахОстатки.Организация
			//|			И ДвиженияЗапасыНаСкладахИзменение.СтруктурнаяЕдиница = ЗапасыНаСкладахОстатки.СтруктурнаяЕдиница
			//|			И ДвиженияЗапасыНаСкладахИзменение.Номенклатура = ЗапасыНаСкладахОстатки.Номенклатура
			//|			И ДвиженияЗапасыНаСкладахИзменение.Характеристика = ЗапасыНаСкладахОстатки.Характеристика
			//|			И ДвиженияЗапасыНаСкладахИзменение.Партия = ЗапасыНаСкладахОстатки.Партия
			//|			И ДвиженияЗапасыНаСкладахИзменение.Ячейка = ЗапасыНаСкладахОстатки.Ячейка
			//|ГДЕ
			//|	ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0) < 0
			////+++ Дмгг, Исключение по реквизиту справочника
			//|	И НЕ ДвиженияЗапасыНаСкладахИзменение.СтруктурнаяЕдиница.КонтролироватьОстатки
			////--- Дмгг
			//|
			//|УПОРЯДОЧИТЬ ПО
			//|	НомерСтроки
			//Малышев 2024-12-03 ONESOFT-3670 --)
			+ "
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДвиженияЗапасыИзменение.НомерСтроки КАК НомерСтроки,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Организация) КАК ОрганизацияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.СтруктурнаяЕдиница) КАК СтруктурнаяЕдиницаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.СчетУчета) КАК СчетУчетаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Номенклатура) КАК НоменклатураПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Характеристика) КАК ХарактеристикаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.Партия) КАК ПартияПредставление,

			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыИзменение.ЗаказПокупателя) КАК ЗаказПокупателяПредставление,
			|	ЗапасыОстатки.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ЗапасыОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
			|	ЕСТЬNULL(ДвиженияЗапасыИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) КАК ОстатокЗапасы,
			|	ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокЗапасы,
			|	ЕСТЬNULL(ЗапасыОстатки.СуммаОстаток, 0) КАК СуммаОстатокЗапасы
			|ИЗ
			|	ДвиженияЗапасыИзменение КАК ДвиженияЗапасыИзменение
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы.Остатки(
			|				&МоментКонтроля,
			|				(Организация, СтруктурнаяЕдиница, СчетУчета, Номенклатура, Характеристика, Партия, ЗаказПокупателя) В

			|					(ВЫБРАТЬ
			|						ДвиженияЗапасыИзменение.Организация КАК Организация,
			|						ДвиженияЗапасыИзменение.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
			|						ДвиженияЗапасыИзменение.СчетУчета КАК СчетУчета,

			|						ДвиженияЗапасыИзменение.Номенклатура КАК Номенклатура,
			|						ДвиженияЗапасыИзменение.Характеристика КАК Характеристика,
			|						ДвиженияЗапасыИзменение.Партия КАК Партия,

			|						ДвиженияЗапасыИзменение.ЗаказПокупателя КАК ЗаказПокупателя
			|					ИЗ
			|						ДвиженияЗапасыИзменение КАК ДвиженияЗапасыИзменение)) КАК ЗапасыОстатки
			|		ПО ДвиженияЗапасыИзменение.Организация = ЗапасыОстатки.Организация
			|			И ДвиженияЗапасыИзменение.СтруктурнаяЕдиница = ЗапасыОстатки.СтруктурнаяЕдиница
			|			И ДвиженияЗапасыИзменение.СчетУчета = ЗапасыОстатки.СчетУчета

			|			И ДвиженияЗапасыИзменение.Номенклатура = ЗапасыОстатки.Номенклатура
			|			И ДвиженияЗапасыИзменение.Характеристика = ЗапасыОстатки.Характеристика
			|			И ДвиженияЗапасыИзменение.Партия = ЗапасыОстатки.Партия

			|			И ДвиженияЗапасыИзменение.ЗаказПокупателя = ЗапасыОстатки.ЗаказПокупателя
			|ГДЕ
			|	ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) < 0
			//+++ Дмгг, Исключение по реквизиту справочника
			|	И НЕ ДвиженияЗапасыИзменение.СтруктурнаяЕдиница.КонтролироватьОстатки
			//--- Дмгг
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДвиженияЗапасыПринятыеИзменение.НомерСтроки КАК НомерСтроки,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыПринятыеИзменение.Организация) КАК ОрганизацияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыПринятыеИзменение.ТипПриемаПередачи) КАК ТипПриемаПередачиПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыПринятыеИзменение.Номенклатура) КАК НоменклатураПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыПринятыеИзменение.Характеристика) КАК ХарактеристикаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыПринятыеИзменение.Партия) КАК ПартияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыПринятыеИзменение.Заказ) КАК ЗаказПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ЗапасыПринятыеОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
			|	ЕСТЬNULL(ДвиженияЗапасыПринятыеИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(ЗапасыПринятыеОстатки.КоличествоОстаток, 0) КАК ОстатокЗапасыПринятые,
			|	ЕСТЬNULL(ЗапасыПринятыеОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокЗапасыПринятые,
			|	0 КАК СуммаРасчетовОстатокЗапасыПринятые
			|ИЗ
			|	ДвиженияЗапасыПринятыеИзменение КАК ДвиженияЗапасыПринятыеИзменение
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыПринятые.Остатки(
			|				&МоментКонтроля,
			|				(Организация, ТипПриемаПередачи, Номенклатура, Характеристика, Партия, Заказ) В
			|					(ВЫБРАТЬ
			|						ДвиженияЗапасыПринятыеИзменение.Организация КАК Организация,
			|						ЗНАЧЕНИЕ(Перечисление.ТипыПриемаПередачиТоваров.ОтчетПоПереработке) КАК ТипПриемаПередачи,
			|						ДвиженияЗапасыПринятыеИзменение.Номенклатура КАК Номенклатура,
			|						ДвиженияЗапасыПринятыеИзменение.Характеристика КАК Характеристика,
			|						ДвиженияЗапасыПринятыеИзменение.Партия КАК Партия,
			|						НЕОПРЕДЕЛЕНО КАК Заказ
			|					ИЗ
			|						ДвиженияЗапасыПринятыеИзменение КАК ДвиженияЗапасыПринятыеИзменение)) КАК ЗапасыПринятыеОстатки
			|		ПО ДвиженияЗапасыПринятыеИзменение.Организация = ЗапасыПринятыеОстатки.Организация
			|			И ДвиженияЗапасыПринятыеИзменение.ТипПриемаПередачи = ЗапасыПринятыеОстатки.ТипПриемаПередачи
			|			И ДвиженияЗапасыПринятыеИзменение.Номенклатура = ЗапасыПринятыеОстатки.Номенклатура
			|			И ДвиженияЗапасыПринятыеИзменение.Характеристика = ЗапасыПринятыеОстатки.Характеристика
			|			И ДвиженияЗапасыПринятыеИзменение.Партия = ЗапасыПринятыеОстатки.Партия
			|			И ДвиженияЗапасыПринятыеИзменение.Заказ = ЗапасыПринятыеОстатки.Заказ
			|ГДЕ
			|	ЕСТЬNULL(ЗапасыПринятыеОстатки.КоличествоОстаток, 0) < 0
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДвиженияЗаказыНаПроизводствоИзменение.НомерСтроки КАК НомерСтроки,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗаказыНаПроизводствоИзменение.Организация) КАК ОрганизацияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗаказыНаПроизводствоИзменение.ЗаказНаПроизводство) КАК ЗаказНаПроизводствоПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗаказыНаПроизводствоИзменение.Номенклатура) КАК НоменклатураПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗаказыНаПроизводствоИзменение.Характеристика) КАК ХарактеристикаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ЗаказыНаПроизводствоОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
			|	ЕСТЬNULL(ДвиженияЗаказыНаПроизводствоИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(ЗаказыНаПроизводствоОстатки.КоличествоОстаток, 0) КАК ОстатокЗаказыНаПроизводство,
			|	ЕСТЬNULL(ЗаказыНаПроизводствоОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокЗаказыНаПроизводство
			|ИЗ
			|	ДвиженияЗаказыНаПроизводствоИзменение КАК ДвиженияЗаказыНаПроизводствоИзменение
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыНаПроизводство.Остатки(
			|				&МоментКонтроля,
			|				(Организация, ЗаказНаПроизводство, Номенклатура, Характеристика) В
			|					(ВЫБРАТЬ
			|						ДвиженияЗаказыНаПроизводствоИзменение.Организация КАК Организация,
			|						ДвиженияЗаказыНаПроизводствоИзменение.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
			|						ДвиженияЗаказыНаПроизводствоИзменение.Номенклатура КАК Номенклатура,
			|						ДвиженияЗаказыНаПроизводствоИзменение.Характеристика КАК Характеристика
			|					ИЗ
			|						ДвиженияЗаказыНаПроизводствоИзменение КАК ДвиженияЗаказыНаПроизводствоИзменение)) КАК ЗаказыНаПроизводствоОстатки
			|		ПО ДвиженияЗаказыНаПроизводствоИзменение.Организация = ЗаказыНаПроизводствоОстатки.Организация
			|			И ДвиженияЗаказыНаПроизводствоИзменение.ЗаказНаПроизводство = ЗаказыНаПроизводствоОстатки.ЗаказНаПроизводство
			|			И ДвиженияЗаказыНаПроизводствоИзменение.Номенклатура = ЗаказыНаПроизводствоОстатки.Номенклатура
			|			И ДвиженияЗаказыНаПроизводствоИзменение.Характеристика = ЗаказыНаПроизводствоОстатки.Характеристика
			|ГДЕ
			|	ЕСТЬNULL(ЗаказыНаПроизводствоОстатки.КоличествоОстаток, 0) < 0
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДвиженияРазмещениеЗаказовИзменение.НомерСтроки КАК НомерСтроки,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияРазмещениеЗаказовИзменение.Организация) КАК ОрганизацияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияРазмещениеЗаказовИзменение.ЗаказПокупателя) КАК ЗаказПокупателяПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияРазмещениеЗаказовИзменение.Номенклатура) КАК НоменклатураПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияРазмещениеЗаказовИзменение.Характеристика) КАК ХарактеристикаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияРазмещениеЗаказовИзменение.ИсточникОбеспечения) КАК ИсточникОбеспеченияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(РазмещениеЗаказовОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
			|	ЕСТЬNULL(ДвиженияРазмещениеЗаказовИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(РазмещениеЗаказовОстатки.КоличествоОстаток, 0) КАК ОстатокРазмещениеЗаказов,
			|	ЕСТЬNULL(РазмещениеЗаказовОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокРазмещениеЗаказов
			|ИЗ
			|	ДвиженияРазмещениеЗаказовИзменение КАК ДвиженияРазмещениеЗаказовИзменение
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РазмещениеЗаказов.Остатки(
			|				&МоментКонтроля,
			|				(Организация, ЗаказПокупателя, Номенклатура, Характеристика, ИсточникОбеспечения) В
			|					(ВЫБРАТЬ
			|						ДвиженияРазмещениеЗаказовИзменение.Организация КАК Организация,
			|						ДвиженияРазмещениеЗаказовИзменение.ЗаказПокупателя КАК ЗаказПокупателя,
			|						ДвиженияРазмещениеЗаказовИзменение.Номенклатура КАК Номенклатура,
			|						ДвиженияРазмещениеЗаказовИзменение.Характеристика КАК Характеристика,
			|						ДвиженияРазмещениеЗаказовИзменение.ИсточникОбеспечения КАК ИсточникОбеспечения
			|					ИЗ
			|						ДвиженияРазмещениеЗаказовИзменение КАК ДвиженияРазмещениеЗаказовИзменение)) КАК РазмещениеЗаказовОстатки
			|		ПО ДвиженияРазмещениеЗаказовИзменение.Организация = РазмещениеЗаказовОстатки.Организация
			|			И ДвиженияРазмещениеЗаказовИзменение.ЗаказПокупателя = РазмещениеЗаказовОстатки.ЗаказПокупателя
			|			И ДвиженияРазмещениеЗаказовИзменение.Номенклатура = РазмещениеЗаказовОстатки.Номенклатура
			|			И ДвиженияРазмещениеЗаказовИзменение.Характеристика = РазмещениеЗаказовОстатки.Характеристика
			|			И ДвиженияРазмещениеЗаказовИзменение.ИсточникОбеспечения = РазмещениеЗаказовОстатки.ИсточникОбеспечения
			|ГДЕ
			|	ЕСТЬNULL(РазмещениеЗаказовОстатки.КоличествоОстаток, 0) < 0
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДвиженияСерийныеНомераИзменение.НомерСтроки КАК НомерСтроки,
			|	ДвиженияСерийныеНомераИзменение.СерийныйНомер КАК СерийныйНомерПредставление,
			|	ДвиженияСерийныеНомераИзменение.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаПредставление,
			|	ДвиженияСерийныеНомераИзменение.Номенклатура КАК НоменклатураПредставление,
			|	ДвиженияСерийныеНомераИзменение.Характеристика КАК ХарактеристикаПредставление,
			|	ДвиженияСерийныеНомераИзменение.Партия КАК ПартияПредставление,
			|	ДвиженияСерийныеНомераИзменение.Ячейка КАК ЯчейкаПредставление,
			|	СерийныеНомераОстатки.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
			|	СерийныеНомераОстатки.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмеренияПредставление,
			|	ЕСТЬNULL(ДвиженияСерийныеНомераИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(СерийныеНомераОстатки.КоличествоОстаток, 0) КАК ОстатокСерийныеНомера,
			|	ЕСТЬNULL(СерийныеНомераОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокСерийныеНомера
			|ИЗ
			|	ДвиженияСерийныеНомераИзменение КАК ДвиженияСерийныеНомераИзменение
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СерийныеНомера.Остатки(&МоментКонтроля, ) КАК СерийныеНомераОстатки
			|		ПО ДвиженияСерийныеНомераИзменение.СтруктурнаяЕдиница = СерийныеНомераОстатки.СтруктурнаяЕдиница
			|			И ДвиженияСерийныеНомераИзменение.Номенклатура = СерийныеНомераОстатки.Номенклатура
			|			И ДвиженияСерийныеНомераИзменение.Характеристика = СерийныеНомераОстатки.Характеристика
			|			И ДвиженияСерийныеНомераИзменение.Партия = СерийныеНомераОстатки.Партия
			|			И ДвиженияСерийныеНомераИзменение.СерийныйНомер = СерийныеНомераОстатки.СерийныйНомер
			|			И ДвиженияСерийныеНомераИзменение.Ячейка = СерийныеНомераОстатки.Ячейка
			|			И (ЕСТЬNULL(СерийныеНомераОстатки.КоличествоОстаток, 0) < 0)
			|
			//+++ Дмгг, Исключение по реквизиту справочника
			|ГДЕ
			|	НЕ ДвиженияСерийныеНомераИзменение.СтруктурнаяЕдиница.КонтролироватьОстатки
			//--- Дмгг
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДвиженияЗапасыВРазрезеГТДИзменение.НомерСтроки КАК НомерСтроки,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыВРазрезеГТДИзменение.Организация) КАК ОрганизацияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыВРазрезеГТДИзменение.НомерГТД) КАК НомерГТДПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыВРазрезеГТДИзменение.Номенклатура) КАК НоменклатураПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыВРазрезеГТДИзменение.Характеристика) КАК ХарактеристикаПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыВРазрезеГТДИзменение.Партия) КАК ПартияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияЗапасыВРазрезеГТДИзменение.СтранаПроисхождения) КАК СтранаПроисхожденияПредставление,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ЗапасыВРазрезеГТДОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
			|	ЕСТЬNULL(ДвиженияЗапасыВРазрезеГТДИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(ЗапасыВРазрезеГТДОстатки.КоличествоОстаток, 0) КАК ОстатокЗапасыВРазрезеГТД,
			|	ЕСТЬNULL(ЗапасыВРазрезеГТДОстатки.КоличествоОстаток, 0) КАК КоличествоОстатокЗапасыВРазрезеГТД
			|ИЗ
			|	ДвиженияЗапасыВРазрезеГТДИзменение КАК ДвиженияЗапасыВРазрезеГТДИзменение
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыВРазрезеГТД.Остатки(
			|				&МоментКонтроля,
			|				(Организация, НомерГТД, Номенклатура, Характеристика, Партия, СтранаПроисхождения) В
			|					(ВЫБРАТЬ
			|						ДвиженияЗапасыВРазрезеГТДИзменение.Организация КАК Организация,
			|						ДвиженияЗапасыВРазрезеГТДИзменение.НомерГТД КАК НомерГТД,
			|						ДвиженияЗапасыВРазрезеГТДИзменение.Номенклатура КАК Номенклатура,
			|						ДвиженияЗапасыВРазрезеГТДИзменение.Характеристика КАК Характеристика,
			|						ДвиженияЗапасыВРазрезеГТДИзменение.Партия КАК Партия,
			|						ДвиженияЗапасыВРазрезеГТДИзменение.СтранаПроисхождения КАК СтранаПроисхождения
			|					ИЗ
			|						ДвиженияЗапасыВРазрезеГТДИзменение КАК ДвиженияЗапасыВРазрезеГТДИзменение)) КАК ЗапасыВРазрезеГТДОстатки
			|		ПО ДвиженияЗапасыВРазрезеГТДИзменение.Организация = ЗапасыВРазрезеГТДОстатки.Организация
			|			И ДвиженияЗапасыВРазрезеГТДИзменение.НомерГТД = ЗапасыВРазрезеГТДОстатки.НомерГТД
			|			И ДвиженияЗапасыВРазрезеГТДИзменение.Номенклатура = ЗапасыВРазрезеГТДОстатки.Номенклатура
			|			И ДвиженияЗапасыВРазрезеГТДИзменение.Характеристика = ЗапасыВРазрезеГТДОстатки.Характеристика
			|			И ДвиженияЗапасыВРазрезеГТДИзменение.Партия = ЗапасыВРазрезеГТДОстатки.Партия
			|			И ДвиженияЗапасыВРазрезеГТДИзменение.СтранаПроисхождения = ЗапасыВРазрезеГТДОстатки.СтранаПроисхождения
			|ГДЕ
			|	ЕСТЬNULL(ЗапасыВРазрезеГТДОстатки.КоличествоОстаток, 0) < 0
			|
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;"
			
			//Малышев 2024-06-17 ONESOFT-3094
			+ УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКонтроля_ОстаткиЗапасов(ДополнительныеСвойства, Истина) //МассивРезультатов[7]
			//Малышев 2024-06-17 ONESOFT-3094 --)
			; 
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
				
				Если ПроверятьИтогиРегистров Тогда
					
					Запрос.Текст = Запрос.Текст + РезервированиеЗапасов_new.ПолучитьТекстЗапросаКонтроля_РезервированиеЗапасов(ДополнительныеСвойства, ЛОЖЬ); //[11]
					
				Иначе
					
					Запрос.Текст = Запрос.Текст + РезервированиеЗапасов_new.ПолучитьТекстЗапросаКонтроля_РезервированиеЗапасов(ДополнительныеСвойства, ИСТИНА) //[11]
					+ РезервированиеЗапасов_new.ПолучитьТекстЗапросаКонтроля_СвободныеОстатки(ДополнительныеСвойства); //[17] 
					
				КонецЕсли;			 
				
				Запрос.УстановитьПараметр("КонтролироватьОстаткиПриПроведенииПоПроектам",  ДополнительныеСвойства.УчетнаяПолитика.КонтролироватьОстаткиПриПроведенииПоПроектам);
				Запрос.УстановитьПараметр("МоментКонтроляРезерва", ДокументСсылкаСборкаЗапасов.МоментВремени());
				
			КонецЕсли;
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			
			Запрос.МенеджерВременныхТаблиц = СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
			Если НЕ ПроверятьИтогиРегистров Тогда 
				Запрос.УстановитьПараметр("МоментКонтроля", ДополнительныеСвойства.ДляПроведения.МоментКонтроля);
			Иначе 
				Запрос.УстановитьПараметр("МоментКонтроля", Дата(2999,12,31));
			КонецЕсли;
			
			МассивРезультатов = Запрос.ВыполнитьПакет();
			
			Если НЕ МассивРезультатов[0].Пустой()
				ИЛИ НЕ МассивРезультатов[1].Пустой()
				ИЛИ НЕ МассивРезультатов[2].Пустой()
				ИЛИ НЕ МассивРезультатов[3].Пустой()
				ИЛИ НЕ МассивРезультатов[4].Пустой()
				ИЛИ НЕ МассивРезультатов[5].Пустой()
				ИЛИ НЕ МассивРезультатов[6].Пустой()
				ИЛИ НЕ МассивРезультатов[7].Пустой()
				// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
				ИЛИ (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(МассивРезультатов[11].Пустой()))
				ИЛИ (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(ПроверятьИтогиРегистров) И НЕ(МассивРезультатов[17].Пустой()))
				// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
				Тогда
				
				ДокументОбъектСборкаЗапасов = ДокументСсылкаСборкаЗапасов.ПолучитьОбъект()
				
			КонецЕсли;
			
			// Отрицательный остаток запасов на складе.
			Если НЕ МассивРезультатов[0].Пустой() Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[0].Выбрать();
				Если НЕ ПроверятьИтогиРегистров Тогда 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
			КонецЕсли;
			
			// Отрицательный остаток запасов.
			Если НЕ ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда 
				Если НЕ МассивРезультатов[1].Пустой() Тогда
					ВыборкаИзРезультатаЗапроса = МассивРезультатов[1].Выбрать();
					Если НЕ ПроверятьИтогиРегистров Тогда 
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
					Иначе 
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
					КонецЕсли;
				КонецЕсли;
				// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			ИначеЕсли Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(МассивРезультатов[11].Пустой()) Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[11].Выбрать();
				Если НЕ(ПроверятьИтогиРегистров) Тогда 
					РезервированиеЗапасов_new.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ,,,1);
				Иначе 
					РезервированиеЗапасов_new.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ,,ИСТИНА,1);
				КонецЕсли;
			ИначеЕсли Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(ПроверятьИтогиРегистров) И НЕ(МассивРезультатов[17].Пустой()) Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[17].Выбрать();
				РезервированиеЗапасов_new.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ,,,2);
				// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			Иначе
				Если НЕ МассивРезультатов[7].Пустой() Тогда

					ВыборкаИзРезультатаЗапроса = МассивРезультатов[7].Выбрать();
					Если НЕ ПроверятьИтогиРегистров Тогда 
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
					Иначе 
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			// Отрицательный остаток запасов принятых.
			Если НЕ МассивРезультатов[2].Пустой() Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[2].Выбрать();
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Если НЕ ПроверятьИтогиРегистров Тогда 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
			КонецЕсли;
			
			// Отрицательный остаток по заказам на производство.
			Если НЕ МассивРезультатов[3].Пустой() Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
				Если НЕ ПроверятьИтогиРегистров Тогда 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗаказыНаПроизводство(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗаказыНаПроизводство(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
			КонецЕсли;
			
			// Отрицательный остаток по размещению запасов.
			Если НЕ МассивРезультатов[4].Пустой() Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[4].Выбрать();
				Если НЕ ПроверятьИтогиРегистров Тогда 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструРазмещениеЗаказов(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструРазмещениеЗаказов(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
			КонецЕсли;
			
			// Отрицательный остаток учета серийных номеров.
			Если НЕ МассивРезультатов[5].Пустой() Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[5].Выбрать();
				Если НЕ ПроверятьИтогиРегистров Тогда 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомера(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомера(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
			КонецЕсли;
			
			// Отрицательный остаток по остаткам запасов в разрезе номеров ГТД.
			Если Константы.КонтролироватьОстаткиПоНомерамГТД.Получить()
				И НЕ МассивРезультатов[6].Пустой() Тогда
				
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[6].Выбрать();
				Если НЕ ПроверятьИтогиРегистров Тогда 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыВРазрезеГТД(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыВРазрезеГТД(ДокументОбъектСборкаЗапасов, ВыборкаИзРезультатаЗапроса, Отказ, , Истина);
				КонецЕсли;
				
			КонецЕсли;
			
		КонецЕсли;
		
	КонецПроцедуры // ВыполнитьКонтроль()
	
	#Область ИнтерфейсПечати
	
	Функция ДанныеДокументовБланкТоварногоНаполнения(МассивОбъектов)
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
		
		Запрос.Текст =
		"
		//:::Шапка
		|Выбрать
		|	СборкаЗапасов.Ссылка КАК Ссылка
		|	,Выбор когда СборкаЗапасов.ВидОперации = Значение(Перечисление.ВидыОперацийСборкаЗапасов.Сборка)
		|		Тогда ""Производство (сборка)""
		|		Иначе ""Производство (разборка)"" Конец КАК ПредставлениеРегистратора
		|	,СборкаЗапасов.ВидОперации КАК ВидОперации
		|	,СборкаЗапасов.Номер КАК Номер
		|	,СборкаЗапасов.Дата КАК ДатаДокумента
		|	,СборкаЗапасов.СтруктурнаяЕдиница КАК ПредставлениеСклада
		|	,СборкаЗапасов.Ячейка КАК ПредставлениеЯчейки
		|	,СборкаЗапасов.Организация.Префикс КАК Префикс
		|
		//:::Табличная часть "Запасы"
		|	,СборкаЗапасов.Запасы.(
		|		НомерСтроки КАК НомерСтроки
		|		,Неопределено КАК Содержание
		|		,Выбор когда(Выразить(СборкаЗапасов.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))) = """"
		|			Тогда СборкаЗапасов.Запасы.Номенклатура.Наименование
		|			Иначе Выразить(СборкаЗапасов.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000)) Конец КАК ПредставлениеНоменклатуры
		|		,Характеристика
		|		,Номенклатура.Артикул КАК Артикул
		|		,Номенклатура.Код КАК Код
		|		,Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры
		|		,Номенклатура.Склад КАК Склад
		|		,Номенклатура.Ячейка КАК Ячейка
		|		,ЕдиницаИзмерения КАК ЕдиницаИзмерения
		|		,Количество КАК Количество
		|		,КлючСвязи
		|	) КАК ТаблицаЗапасы
		|
		//:::Табличная часть "Продукция"
		|	,СборкаЗапасов.Продукция.(
		|		НомерСтроки КАК НомерСтроки
		|		,Неопределено КАК Содержание
		|		,Выбор когда (Выразить(СборкаЗапасов.Продукция.Номенклатура.НаименованиеПолное КАК СТРОКА(100))) = """"
		|			Тогда СборкаЗапасов.Продукция.Номенклатура.Наименование
		|			Иначе СборкаЗапасов.Продукция.Номенклатура.НаименованиеПолное Конец КАК ПредставлениеНоменклатуры
		|		,Характеристика КАК Характеристика
		|		,Номенклатура.Артикул КАК Артикул
		|		,Номенклатура.Код КАК Код
		|		,Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры
		|		,Номенклатура.Склад КАК Склад
		|		,Номенклатура.Ячейка КАК Ячейка
		|		,ЕдиницаИзмерения КАК ЕдиницаИзмерения
		|		,Количество КАК Количество
		|		,КлючСвязи
		|	) КАК ТаблицаПродукция
		|
		//:::Табличная часть "СерийныеНомера"
		|	,СборкаЗапасов.СерийныеНомера.(
		|		СерийныйНомер
		|		,КлючСвязи
		|	) КАК ТаблицаСерийныеНомера
		|
		//:::Табличная часть "СерийныеНомераПродукции"
		|	,СборкаЗапасов.СерийныеНомераПродукция.(
		|		СерийныйНомер,
		|		КлючСвязи
		|	) КАК ТаблицаСерийныеНомераПродукции
		|
		|Из Документ.СборкаЗапасов КАК СборкаЗапасов
		|Где СборкаЗапасов.Ссылка В(&МассивОбъектов)
		|Упорядочить по Ссылка, НомерСтроки";
		
		ТаблицаДанных = Запрос.Выполнить().Выгрузить();
		
		Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			
			Если СтрокаТаблицы.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Разборка Тогда
				
				СтрокаТаблицы.ТаблицаЗапасы = СтрокаТаблицы.ТаблицаПродукция;
				СтрокаТаблицы.ТаблицаСерийныеНомера = СтрокаТаблицы.ТаблицаСерийныеНомераПродукции;
				
			КонецЕсли;
			
		КонецЦикла;
		
		ТаблицаДанных.Колонки.Удалить("ТаблицаПродукция");
		ТаблицаДанных.Колонки.Удалить("ТаблицаСерийныеНомераПродукции");
		
		Возврат ТаблицаДанных;
		
	КонецФункции
	
	// Функция проверяет, проведен документ или нет, и вызывает процедуру печати документа.
	//
	Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
		Перем ПервыйДокумент, НомерСтрокиНачало, Ошибки;
		
		ТабличныйДокумент = Новый ТабличныйДокумент;
		ДанныеПечати = Новый Структура;
		
		Для каждого ТекущийДокумент Из МассивОбъектов Цикл
			
			ПечатьДокументовУНФ.ПередНачаломФормированияДокумента(ТабличныйДокумент, ПервыйДокумент, НомерСтрокиНачало, ДанныеПечати);
			
			Если ИмяМакета = "М11" Тогда
				
				Макет									= УправлениеПечатью.МакетПечатнойФормы("Документ.СборкаЗапасов.ПФ_MXL_М11");
				ТабличныйДокумент.КлючПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_СборкаЗапасов_М11";
				ЭтоСборкаЗапасов 						= (ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка);
				
				ТекстЗапроса =
				"
				|//::::: Шапка документа :::::
				|
				|ВЫБРАТЬ
				|	СборкаЗапасов.Дата КАК ДатаДокумента
				|	,СборкаЗапасов.Организация КАК Организация
				|	,СборкаЗапасов.Номер КАК Номер
				|	,СборкаЗапасов.Организация.Префикс КАК Префикс
				|	,СборкаЗапасов.СтруктурнаяЕдиницаЗапасов КАК Отправитель
				|	,СборкаЗапасов.СтруктурнаяЕдиница КАК Получатель
				|	,СборкаЗапасов.ПодписьРуководителя.РасшифровкаПодписи КАК РасшифровкаПодписиРуководителя
				|	,СборкаЗапасов.ПодписьКладовщикаОтправил.Должность КАК ДолжностьКладовщикаОтпустил
				|	,СборкаЗапасов.ПодписьКладовщикаОтправил.РасшифровкаПодписи КАК РасшифровкаПодписиКладовщикаОтпустил
				|	,СборкаЗапасов.ПодписьКладовщикаПолучил.Должность КАК ДолжностьКладовщикаПринял
				|	,СборкаЗапасов.ПодписьКладовщикаПолучил.РасшифровкаПодписи КАК РасшифровкаПодписиКладовщикаПринял
				|Из Документ.СборкаЗапасов КАК СборкаЗапасов
				|Где СборкаЗапасов.Ссылка = &ТекущийДокумент
				|
				|//::::: Таблица документа :::::
				|
				|;Выбрать
				|	ТабличнаяЧасть.НомерСтроки КАК НомерСтроки
				|	,ТабличнаяЧасть.Номенклатура.НаименованиеПолное КАК Запас
				|	,ТабличнаяЧасть.Номенклатура.Артикул КАК Артикул
				|	,ТабличнаяЧасть.Номенклатура.СчетУчетаЗапасов.Код КАК Счет
				|	,ТабличнаяЧасть.Номенклатура.Код КАК НоменклатурныйНомер
				|	,ТабличнаяЧасть.Количество КАК Количество
				|	,ТабличнаяЧасть.Резерв КАК Резерв
				|	,ТабличнаяЧасть.Ссылка.ЗаказПокупателя КАК ЗаказПокупателя
				|	,ТабличнаяЧасть.Характеристика КАК Характеристика
				|	,ТабличнаяЧасть.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияНаименование
				|	,ТабличнаяЧасть.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияКод
				|	,Выбор когда &ВалютаВидЦен = НациональнаяВалюта.Значение
				|		Тогда ЕСТЬNULL(УчетныеЦены.Цена, 0) / ЕСТЬNULL(УчетныеЦены.ЕдиницаИзмерения.Коэффициент, 1)
				|		Иначе (ЕСТЬNULL(УчетныеЦены.Цена, 0) * КурсыВалютыВидаЦен.Курс / КурсыВалютыВидаЦен.Кратность) / ЕСТЬNULL(УчетныеЦены.ЕдиницаИзмерения.Коэффициент, 1)
				|	Конец КАК Цена
				|	,Выбор когда &ВалютаВидЦен = НациональнаяВалюта.Значение
				|		Тогда ЕСТЬNULL(УчетныеЦены.Цена * ТабличнаяЧасть.Количество, 0) / ЕСТЬNULL(УчетныеЦены.ЕдиницаИзмерения.Коэффициент, 1)
				|		Иначе (ЕСТЬNULL(УчетныеЦены.Цена * ТабличнаяЧасть.Количество, 0) * КурсыВалютыВидаЦен.Курс / КурсыВалютыВидаЦен.Кратность) / ЕСТЬNULL(УчетныеЦены.ЕдиницаИзмерения.Коэффициент, 1)
				|	Конец КАК Сумма
				|ИЗ
				|	ИмяТабличнойЧасти КАК ТабличнаяЧасть
				|	Левое Соединение РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаДокумента, ВидЦен = &ВидЦенПлановойОценки) КАК УчетныеЦены
				|	По ТабличнаяЧасть.Номенклатура = УчетныеЦены.Номенклатура
				|		И ТабличнаяЧасть.Характеристика = УчетныеЦены.Характеристика
				|	,РегистрСведений.КурсыВалют.СрезПоследних(&ДатаДокумента, Валюта = &ВалютаВидЦен) КАК КурсыВалютыВидаЦен
				|	,Константа.НациональнаяВалюта КАК НациональнаяВалюта
				|Где ТабличнаяЧасть.Ссылка = &ТекущийДокумент
				|Упорядочить по ТабличнаяЧасть.НомерСтроки";
				
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяТабличнойЧасти", СтрШаблон("Документ.СборкаЗапасов.%1", ?(ЭтоСборкаЗапасов, "Запасы", "Продукция")));
				
				Запрос = Новый Запрос(ТекстЗапроса);
				Запрос.УстановитьПараметр("ТекущийДокумент",		ТекущийДокумент);
				Запрос.УстановитьПараметр("ДатаДокумента",			ТекущийДокумент.Дата);
				Запрос.УстановитьПараметр("ВидЦенПлановойОценки",	ТекущийДокумент.ПлановаяОценка);
				Запрос.УстановитьПараметр("ВалютаВидЦен",			?(ЗначениеЗаполнено(ТекущийДокумент.ПлановаяОценка), ТекущийДокумент.ПлановаяОценка.ВалютаЦены, Константы.НациональнаяВалюта.Получить()));
				РезультатЗапроса = Запрос.ВыполнитьПакет();
				Шапка = РезультатЗапроса[0].Выбрать();
				Шапка.Следующий();
				
				СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация, Шапка.ДатаДокумента);
				
				ОбластьМакета = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "Шапка", "", Ошибки);
				Если ОбластьМакета <> Неопределено Тогда
					
					ШаблонЗаголовока = НСтр("ka='ТРЕБОВАНИЕ-НАКЛАДНАЯ № %1 от %2';ru='ТРЕБОВАНИЕ-НАКЛАДНАЯ № %1 от %2';en='CLAIM-WAYBILL №% 1% 2 from'");
					НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс);
					ДатаДокумента = ПечатьДокументовУНФ.ПредставлениеДатыВДокументах(Шапка.ДатаДокумента);
					
					ДанныеПечати.Вставить("Заголовок", СтрШаблон(ШаблонЗаголовока, НомерДокумента, ДатаДокумента));
					ДанныеПечати.Вставить("КодОКПО", СведенияОбОрганизации.КодПоОКПО);
					ДанныеПечати.Вставить("ПредставлениеОрганизации", УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации));
					ДанныеПечати.Вставить("ДатаСоставления", Формат(Шапка.ДатаДокумента, "ДФ=dd.MM.yy"));
					
					ОбластьМакета.Параметры.Заполнить(Шапка);
					ОбластьМакета.Параметры.Заполнить(ДанныеПечати);
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
				КонецЕсли;
				
				ОбластьМакета = Макет.ПолучитьОбласть("Строка");
				
				ВыборкаСтрокЗапасы = РезультатЗапроса[1].Выбрать();
				Пока ВыборкаСтрокЗапасы.Следующий() Цикл
					
					ДанныеПечати.Очистить();
					
					ОписаниеЗапаса = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокЗапасы.Запас, ВыборкаСтрокЗапасы.Характеристика, ВыборкаСтрокЗапасы.Артикул);
					ДанныеПечати.Вставить("Запас", ОписаниеЗапаса);
					
					ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокЗапасы);
					ОбластьМакета.Параметры.Заполнить(ДанныеПечати);
					
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
				КонецЦикла;
				
				ОбластьМакета = Макет.ПолучитьОбласть("Подвал");
				ОбластьМакета.Параметры.Заполнить(Шапка);
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
			ИначеЕсли ИмяМакета = "МХ18" Тогда
				
				Макет									= УправлениеПечатью.МакетПечатнойФормы("Документ.СборкаЗапасов.ПФ_MXL_МХ18");
				ТабличныйДокумент.КлючПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_СборкаЗапасов_МХ18";
				ЭтоСборкаЗапасов 						= (ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка);
				
				Запрос = Новый Запрос();
				Запрос.УстановитьПараметр("ТекущийДокумент", ТекущийДокумент);
				
				Запрос.Текст =
				"ВЫБРАТЬ
				|	СборкаЗапасов.Дата КАК ДатаДокумента
				|	,СборкаЗапасов.Организация КАК Организация
				|	,СборкаЗапасов.Номер КАК Номер
				|	,СборкаЗапасов.Организация.Префикс КАК Префикс
				|	,СборкаЗапасов.СтруктурнаяЕдиница КАК Отправитель
				|	,СборкаЗапасов.СтруктурнаяЕдиницаПродукции КАК Получатель
				|	,СборкаЗапасов.ПодписьКладовщикаОтправил.Должность КАК ДолжностьКладовщикаОтпустил
				|	,СборкаЗапасов.ПодписьКладовщикаОтправил.РасшифровкаПодписи КАК РасшифровкаПодписиКладовщикаОтпустил
				|	,СборкаЗапасов.ПодписьКонтролера.Должность КАК ДолжностьКонтролера
				|	,СборкаЗапасов.ПодписьКонтролера.РасшифровкаПодписи КАК РасшифровкаПодписиКонтролера
				|	,СборкаЗапасов.ПодписьКладовщикаПолучил.Должность КАК ДолжностьКладовщикаПринял
				|	,СборкаЗапасов.ПодписьКладовщикаПолучил.РасшифровкаПодписи КАК РасшифровкаПодписиКладовщикаПринял
				|	,СборкаЗапасов.Продукция.(
				|		НомерСтроки,
				|		Номенклатура.НаименованиеПолное КАК Запас,
				|		Номенклатура.Артикул КАК Артикул,
				|		Количество КАК КоличествоМест,
				|		Резерв КАК Резерв,
				|		Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
				|		Характеристика,
				|		ЕдиницаИзмерения.Наименование КАК ВидУпаковки,
				|		Номенклатура.СчетУчетаЗапасов.Код КАК Счет,
				|		Номенклатура.Код КАК ТоварКод,
				|		Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмеренияНаименование,
				|		Номенклатура.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияКодПоОКЕИ,
				|		ВЫБОР
				|			КОГДА СборкаЗапасов.Продукция.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
				|				ТОГДА СборкаЗапасов.Продукция.ЕдиницаИзмерения.Коэффициент
				|			ИНАЧЕ 1
				|		КОНЕЦ КАК КоличествоВОдномМесте
				|	)
				|	,СборкаЗапасов.Запасы.(
				|		НомерСтроки,
				|		Номенклатура.НаименованиеПолное КАК Запас,
				|		Номенклатура.Артикул КАК Артикул,
				|		Количество КАК КоличествоМест,
				|		Резерв КАК Резерв,
				|		Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
				|		Характеристика,
				|		ЕдиницаИзмерения.Наименование КАК ВидУпаковки,
				|		Номенклатура.СчетУчетаЗапасов.Код КАК Счет,
				|		Номенклатура.Код КАК ТоварКод,
				|		Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмеренияНаименование,
				|		Номенклатура.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияКодПоОКЕИ,
				|		ВЫБОР
				|			КОГДА СборкаЗапасов.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
				|				ТОГДА СборкаЗапасов.Запасы.ЕдиницаИзмерения.Коэффициент
				|			ИНАЧЕ 1
				|		КОНЕЦ КАК КоличествоВОдномМесте
				|	)
				|Из Документ.СборкаЗапасов КАК СборкаЗапасов
				|Где СборкаЗапасов.Ссылка = &ТекущийДокумент";
				
				Шапка = Запрос.Выполнить().Выбрать();
				Шапка.Следующий();
				
				ВыборкаСтрокЗапасы = ?(ЭтоСборкаЗапасов, Шапка.Продукция.Выгрузить(), Шапка.Запасы.Выгрузить());
				
				Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СборкаЗапасов.ПФ_MXL_МХ18");
				
				НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс);
				
				// Выводим общие реквизиты шапки
				СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация, Шапка.ДатаДокумента);
				ОбластьМакета         = Макет.ПолучитьОбласть("Шапка");
				ОбластьМакета.Параметры.Заполнить(Шапка);
				ОбластьМакета.Параметры.ПредставлениеОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации);
				ОбластьМакета.Параметры.НомерДокумента           = НомерДокумента;
				ОбластьМакета.Параметры.ДатаДокумента            = Шапка.ДатаДокумента;
				ОбластьМакета.Параметры.ОрганизацияПоОКПО        = СведенияОбОрганизации.КодПоОКПО;
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
				СтрокНаСтранице = 23;
				СтрокШапки      = 10;
				СтрокПодвала    = 9;
				НомерСтраницы   = 1;
				
				// Выводим заголовок таблицы
				ЗаголовокТаблицы = Макет.ПолучитьОбласть("ЗаголовокТаблицы");
				ЗаголовокТаблицы.Параметры.НомерСтраницы = "Страница " + НомерСтраницы; 
				ТабличныйДокумент.Вывести(ЗаголовокТаблицы);
				
				КоличествоСтрок = ВыборкаСтрокЗапасы.Количество();
				
				Если КоличествоСтрок = 1 Тогда
					ПереноситьПоследнююСтроку = 0;
				Иначе
					ЦелыхСтраницСПодвалом     = Цел((СтрокШапки + КоличествоСтрок + СтрокПодвала) / СтрокНаСтранице);
					ЦелыхСтраницБезПодвала    = Цел((СтрокШапки + КоличествоСтрок - 1) / СтрокНаСтранице);
					ПереноситьПоследнююСтроку = ЦелыхСтраницСПодвалом - ЦелыхСтраницБезПодвала;
				КонецЕсли;
				
				// инициализация итогов по странице
				ИтогКоличествоМестПоСтранице = 0;
				
				// инициализация итогов по документу
				ИтогоКоличество  = 0;
				
				Ном = 0;
				
				// Выводим многострочную часть докмента
				ОбластьИтоговПоСтранице = Макет.ПолучитьОбласть("ИтогиПоСтранице");
				
				ОбластьМакета = Макет.ПолучитьОбласть("Строка");
				Для каждого СтрокаЗапасы Из ВыборкаСтрокЗапасы Цикл
					
					Ном = Ном + 1;
					//Начинаем новую страницу, если предыдущая строка была последней на странице
					//или пора переносить последнюю строку на последнюю страницу с подвалом.
					ЦелаяСтраница = (СтрокШапки + Ном - 1) / СтрокНаСтранице;
					
					Если (ЦелаяСтраница = Цел(ЦелаяСтраница))
						ИЛИ ((ПереноситьПоследнююСтроку = 1) и (Ном = КоличествоСтрок)) Тогда
						
						ОбластьИтоговПоСтранице.Параметры.ИтогМестНаСтранице = ИтогКоличествоМестПоСтранице;
						
						ТабличныйДокумент.Вывести(ОбластьИтоговПоСтранице);
						
						// инициализация итогов по странице
						ИтогКоличествоМестПоСтранице = 0;
						
						НомерСтраницы = НомерСтраницы + 1;
						ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
						ЗаголовокТаблицы.Параметры.НомерСтраницы = "Страница " + НомерСтраницы;
						ТабличныйДокумент.Вывести(ЗаголовокТаблицы);
						
					КонецЕсли;
					
					ОбластьМакета.Параметры.Заполнить(СтрокаЗапасы);
					КоличествоМест = СтрокаЗапасы.КоличествоМест;
					ОбластьМакета.Параметры.ТоварНаименование = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(СтрокаЗапасы.Запас, СтрокаЗапасы.Характеристика, СтрокаЗапасы.Артикул);
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					// Обновим итоги по странице
					ИтогКоличествоМестПоСтранице = ИтогКоличествоМестПоСтранице + КоличествоМест;
					
					// Обновим итогов по документу
					ИтогоКоличество  = ИтогоКоличество  + КоличествоМест;
					
				КонецЦикла;
				
				ОбластьИтоговПоСтранице.Параметры.ИтогМестНаСтранице = ИтогКоличествоМестПоСтранице;
				
				ТабличныйДокумент.Вывести(ОбластьИтоговПоСтранице);
				
				// Выводим итоги по документу в целом
				ОбластьМакета = Макет.ПолучитьОбласть("Всего");
				ОбластьМакета.Параметры.ИтогМест = ИтогоКоличество;
				
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
				// Выводим подвал документа
				ОбластьМакета = Макет.ПолучитьОбласть("Подвал");
				ОбластьМакета.Параметры.Заполнить(Шапка);
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
			ИначеЕсли ИмяМакета = "ГарантийныйТалонПродукция" Тогда
				
				РаботаСНоменклатуройСервер.СформироватьГарантийныеТалоныПроизводство(ТабличныйДокумент, ТекущийДокумент, Ошибки, "Продукция");
				
			ИначеЕсли ИмяМакета = "ГарантийныйТалонКомплектующие" Тогда
				
				РаботаСНоменклатуройСервер.СформироватьГарантийныеТалоныПроизводство(ТабличныйДокумент, ТекущийДокумент, Ошибки, "Запасы");
				
				
			ИначеЕсли ИмяМакета = "ПФ_MXL_МакетТребованиеНаСкладПроизводство" Тогда	
				
				
				ТабличныйДокумент.КлючПараметровПечати = "ПараметрыПечати_ЗаказНаПроизводство";
				
				Запрос = Новый Запрос();
				Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
				Запрос.УстановитьПараметр("Характеристика", НСтр("ka='მახასიათებელი:';ru='Характеристика:';en='Characteristic:'"));
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	СборкаЗапасов.Ссылка КАК Ссылка,
				|	СборкаЗапасов.Номер КАК Номер,
				|	СборкаЗапасов.Дата КАК ДатаДокумента,
				|	СборкаЗапасов.Организация КАК Организация,
				|	СборкаЗапасов.Автор КАК Исполнитель,
				|	СборкаЗапасов.ЗаказПокупателя КАК Заказ,
				|	СборкаЗапасов.СтруктурнаяЕдиница КАК Подразделение,
				|	СборкаЗапасов.Организация.Префикс КАК Префикс,
				|	СборкаЗапасов.Продукция.(
				|		НомерСтроки КАК НомерСтроки,
				|		Номенклатура.НаименованиеПолное КАК Продукция,
				|		Номенклатура.Артикул КАК Артикул,
				|		Характеристика КАК Характеристика,
				|		ЕдиницаИзмерения КАК ЕдиницаИзмерения,
				|		Количество КАК Количество
				|	),
				|	СборкаЗапасов.Запасы.(
				|		НомерСтроки КАК НомерСтроки,
				|		Номенклатура.НаименованиеПолное КАК Материал,
				|		Номенклатура.Артикул КАК Артикул,
				|		Характеристика КАК Характеристика,
				|		ЕдиницаИзмерения КАК ЕдиницаИзмерения,
				|		Количество КАК Количество
				|	)
				|ИЗ
				|	Документ.СборкаЗапасов КАК СборкаЗапасов,
				|	Константы КАК Константы
				|ГДЕ
				|	СборкаЗапасов.Ссылка В(&МассивОбъектов)
				|
				|УПОРЯДОЧИТЬ ПО
				|	Ссылка,
				|	НомерСтроки";
				
				Шапка = Запрос.Выполнить().Выбрать();
				
				ПервыйДокумент = Истина;
				
				
				Пока Шапка.Следующий() Цикл
					
					Если Не ПервыйДокумент Тогда
						ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
					КонецЕсли;
					ПервыйДокумент = Ложь;
					
					НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
					
					ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаПроизводство_МакетТребованиеНаСклад";
					
					//Малышев 2025-02-20
					//Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаказНаПроизводство.ПФ_MXL_МакетТребованиеНаСклад");
					Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СборкаЗапасов.ПФ_MXL_МакетТребованиеНаСкладПроизводство");
					
					НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс);
					
					ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
					ОбластьМакета.Параметры.ТекстЗаголовка = "წარმოება № "
					+ НомерДокумента
					+ " , "
					+ Формат(Шапка.ДатаДокумента, "Л=ka; ДФ='dd MMMM yyyy'");
					
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					// Шапка.
					ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
					ОбластьМакета.Параметры.Заполнить(Шапка);
					ОбластьМакета.Параметры.ДатаЗапуска = Формат(Шапка.ДатаДокумента, "ДФ=dd.MM.yyyy");
					
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					// ТЧ Продукция.
					ВыборкаСтрокТовары = Шапка.Продукция.Выбрать();
					
					ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицыПродукция");
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					ОбластьМакета = Макет.ПолучитьОбласть("СтрокаПродукция");
					
					Пока ВыборкаСтрокТовары.Следующий() Цикл
						ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
						
						ОбластьМакета.Параметры.Продукция = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокТовары.Продукция,
						ВыборкаСтрокТовары.Характеристика, ВыборкаСтрокТовары.Артикул);
						ТабличныйДокумент.Вывести(ОбластьМакета);
					КонецЦикла;
					
					ОбластьМакета = Макет.ПолучитьОбласть("ИтогоПродукция");
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					// ТЧ Запасы.
					ВыборкаСтрокТовары = Шапка.Запасы.Выбрать();
					
					ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицы");
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					ОбластьМакета = Макет.ПолучитьОбласть("Строка");
					
					Пока ВыборкаСтрокТовары.Следующий() Цикл
						ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
						
						ОбластьМакета.Параметры.Материал = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокТовары.Материал, 
						ВыборкаСтрокТовары.Характеристика, ВыборкаСтрокТовары.Артикул);
						ТабличныйДокумент.Вывести(ОбластьМакета);
					КонецЦикла;
					
					ОбластьМакета = Макет.ПолучитьОбласть("Итого");
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					// Подпись.
					ОбластьМакета = Макет.ПолучитьОбласть("Подписи");
					ОбластьМакета.Параметры.Заполнить(Шапка);
					ТабличныйДокумент.Вывести(ОбластьМакета);
					
					УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
					
				КонецЦикла;
				
				
				
			ИначеЕсли ИмяМакета = "ПФ_MXL_МакетТребованиеНаСкладПроизводствоПроекты" Тогда	
					
				ПечатьМакетТребованиеНаСкладПроизводствоПроекты(ТабличныйДокумент, МассивОбъектов, ОбъектыПечати)	
				
			КонецЕсли;
			
			УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, ТекущийДокумент);
			
		КонецЦикла;
		
		ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(Ошибки);
		ТабличныйДокумент.АвтоМасштаб = Истина;
		
		Возврат ТабличныйДокумент;
		
	КонецФункции // ПечатнаяФорма()
	
	// Олег Литвиненко 23.05.23 --->
	// Выделил старый код в отдельную процедуру
	Процедура ПечатьМакетТребованиеНаСкладПроизводствоПроектыСтарая(ТабличныйДокумент, МассивОбъектов, ОбъектыПечати)
		ТабличныйДокумент.КлючПараметровПечати = "ПФ_MXL_МакетТребованиеНаСкладПроизводствоПроекты";
				
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
		Запрос.УстановитьПараметр("Характеристика", НСтр("ka='მახასიათებელი:';ru='Характеристика:';en='Characteristic:'"));
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СборкаЗапасов.Ссылка КАК Ссылка,
		|	СборкаЗапасов.Номер КАК Номер,
		|	СборкаЗапасов.Дата КАК ДатаДокумента,
		|	СборкаЗапасов.Организация КАК Организация,
		|	СборкаЗапасов.Автор КАК Исполнитель,
		|	СборкаЗапасов.ЗаказПокупателя КАК Заказ,
		|	СборкаЗапасов.СтруктурнаяЕдиница КАК Подразделение,
		|	СборкаЗапасов.СтруктурнаяЕдиницаЗапасов КАК СписатьИз,
		|	СборкаЗапасов.Проект КАК Проект,
		|	СборкаЗапасов.СтруктурнаяЕдиницаПродукции КАК Получатель,
		|	СборкаЗапасов.Организация.Префикс КАК Префикс,
		|	СборкаЗапасов.Продукция.(
		|		НомерСтроки КАК НомерСтроки,
		|		Номенклатура.НаименованиеПолное КАК Продукция,
		|		Номенклатура.Артикул КАК Артикул,
		|		Характеристика КАК Характеристика,
		|		Номенклатура КАК Номенклатура,
		|		ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|		Количество КАК Количество
		|	),
		|	СборкаЗапасов.Запасы.(
		|		НомерСтроки КАК НомерСтроки,
		|		Номенклатура.НаименованиеПолное КАК Материал,
		|		Номенклатура.Артикул КАК Артикул,
		|		Характеристика КАК Характеристика,
		|		Номенклатура КАК Номенклатура,
		|		ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|		Количество КАК Количество
		|	)
		|ИЗ
		|	Документ.СборкаЗапасов КАК СборкаЗапасов,
		|	Константы КАК Константы
		|ГДЕ
		|	СборкаЗапасов.Ссылка В(&МассивОбъектов)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	НомерСтроки";
		
		Шапка = Запрос.Выполнить().Выбрать();
		
		ПервыйДокумент = Истина;
		
		
		Пока Шапка.Следующий() Цикл
			
			Если Не ПервыйДокумент Тогда
				ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
			КонецЕсли;
			ПервыйДокумент = Ложь;
			
			НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
			
			ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаПроизводство_МакетТребованиеНаСклад";
			
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СборкаЗапасов.ПФ_MXL_МакетТребованиеНаСкладПроизводствоПроекты");
			
			НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс);
			
			ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
			ОбластьМакета.Параметры.ТекстЗаголовка = "წარმოება № "
			+ НомерДокумента
			+ " , "
			+ Формат(Шапка.ДатаДокумента, "Л=ka; ДФ='dd MMMM yyyy'");
			
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// Шапка.
			ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
			ОбластьМакета.Параметры.Заполнить(Шапка);
			ОбластьМакета.Параметры.ДатаЗапуска = Формат(Шапка.ДатаДокумента, "ДФ=dd.MM.yyyy");
			
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// ТЧ Продукция.
			ВыборкаСтрокТовары = Шапка.Продукция.Выбрать();
			
			ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицыПродукция");
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			ОбластьМакета = Макет.ПолучитьОбласть("СтрокаПродукция");
			
			К = 0;
			кол = 0;
			
			Пока ВыборкаСтрокТовары.Следующий() Цикл
				
				ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
				
				ОбластьМакета.Параметры.Продукция = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокТовары.Продукция,
				ВыборкаСтрокТовары.Характеристика, ВыборкаСтрокТовары.Артикул);
				
				Себест = ПолучитьСебестоимостьНоменклатуры(ВыборкаСтрокТовары.Номенклатура, ВыборкаСтрокТовары.Характеристика, Шапка.Ссылка, Шапка.Ссылка.СтруктурнаяЕдиницаПродукции.РозничныйВидЦен, "Продукция", "СтруктурнаяЕдиницаПродукции");
				ОбластьМакета.Параметры.Себест = Себест;
				ОбластьМакета.Параметры.СебестСумм = Себест * ВыборкаСтрокТовары.Количество;
				
				Если Шапка.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда
					ОбластьМакета.Параметры.Себест = Себест / 2;
					ОбластьМакета.Параметры.СебестСумм = (Себест * ВыборкаСтрокТовары.Количество)/ 2;
				КонецЕсли;
				
				кол = кол + ОбластьМакета.Параметры.СебестСумм;
				
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
			КонецЦикла;
			
								
			
			ОбластьМакета = Макет.ПолучитьОбласть("ИтогоПродукция");
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// ТЧ Запасы.
			ВыборкаСтрокТовары = Шапка.Запасы.Выбрать();
			
			ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицы");
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			ОбластьМакета = Макет.ПолучитьОбласть("Строка");
			
			Пока ВыборкаСтрокТовары.Следующий() Цикл
				ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
				
				ОбластьМакета.Параметры.Материал = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокТовары.Материал, 
				ВыборкаСтрокТовары.Характеристика, ВыборкаСтрокТовары.Артикул);
				
				Себест = ПолучитьСебестоимостьНоменклатуры(ВыборкаСтрокТовары.Номенклатура, ВыборкаСтрокТовары.Характеристика, Шапка.Ссылка, Шапка.Ссылка.СтруктурнаяЕдиницаПродукции.РозничныйВидЦен, "Продукция", "СтруктурнаяЕдиницаПродукции");
				ОбластьМакета.Параметры.Себест = Себест / ВыборкаСтрокТовары.Количество;
				ОбластьМакета.Параметры.СебестСумм = Себест;
				
				К = К + Себест;
				//ТабличныйДокумент.Вывести(ОбластьМакета);
			КонецЦикла;
			
			//кэф = Кол / к;
			// Олег Литвиненко 22.05.23 --->
			кэф = ?(к=0, 0, Кол / к);
			// Олег Литвиненко <---
			
			ВыборкаСтрокТовары = Шапка.Запасы.Выбрать();
								
			ОбластьМакета = Макет.ПолучитьОбласть("Строка");
			

			Пока ВыборкаСтрокТовары.Следующий() Цикл
				ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
				
				ОбластьМакета.Параметры.Материал = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокТовары.Материал, 
				ВыборкаСтрокТовары.Характеристика, ВыборкаСтрокТовары.Артикул);
				
				Себест = ПолучитьСебестоимостьНоменклатуры(ВыборкаСтрокТовары.Номенклатура, ВыборкаСтрокТовары.Характеристика, Шапка.Ссылка, Шапка.Ссылка.СтруктурнаяЕдиницаПродукции.РозничныйВидЦен, "Продукция", "СтруктурнаяЕдиницаПродукции");
				ОбластьМакета.Параметры.СебестСумм = Себест * кэф;
				ОбластьМакета.Параметры.Себест = ОбластьМакета.Параметры.СебестСумм / ВыборкаСтрокТовары.Количество;
				
				ТабличныйДокумент.Вывести(ОбластьМакета);
			КонецЦикла;
			
			ОбластьМакета = Макет.ПолучитьОбласть("Итого");
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// Подпись.
			ОбластьМакета = Макет.ПолучитьОбласть("Подписи");
			ОбластьМакета.Параметры.Заполнить(Шапка);
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
			
		КонецЦикла;
	КонецПроцедуры// Олег Литвиненко <---
	
	// Олег Литвиненко 23.05.23 --->
	Процедура ПечатьМакетТребованиеНаСкладПроизводствоПроекты(ТабличныйДокумент, МассивОбъектов, ОбъектыПечати)
		
		ТабличныйДокумент.КлючПараметровПечати = "ПФ_MXL_МакетТребованиеНаСкладПроизводствоПроекты";
				
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
		Запрос.УстановитьПараметр("Характеристика", НСтр("ka='მახასიათებელი:';ru='Характеристика:';en='Characteristic:'"));
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СборкаЗапасов.Ссылка КАК Ссылка,
		|	СборкаЗапасов.Номер КАК Номер,
		|	СборкаЗапасов.Дата КАК ДатаДокумента,
		|	СборкаЗапасов.Организация КАК Организация,
		|	СборкаЗапасов.Автор КАК Исполнитель,
		|	СборкаЗапасов.ЗаказПокупателя КАК Заказ,
		|	СборкаЗапасов.СтруктурнаяЕдиница КАК Подразделение,
		|	СборкаЗапасов.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
		|	СборкаЗапасов.СтруктурнаяЕдиницаЗапасов КАК СписатьИз,
		|	СборкаЗапасов.Проект КАК Проект,
		|	СборкаЗапасов.СтруктурнаяЕдиницаПродукции КАК Получатель,
		|	СборкаЗапасов.Организация.Префикс КАК Префикс
		|ПОМЕСТИТЬ втШапка
		|ИЗ
		|	Документ.СборкаЗапасов КАК СборкаЗапасов
		|ГДЕ
		|	СборкаЗапасов.Ссылка В(&МассивОбъектов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втШапка.Ссылка КАК Ссылка,
		|	втШапка.Номер КАК Номер,
		|	втШапка.ДатаДокумента КАК ДатаДокумента,
		|	втШапка.Организация КАК Организация,
		|	втШапка.Исполнитель КАК Исполнитель,
		|	втШапка.Заказ КАК Заказ,
		|	втШапка.Подразделение КАК Подразделение,
		|	втШапка.СписатьИз КАК СписатьИз,
		|	втШапка.Проект КАК Проект,
		|	втШапка.Получатель КАК Получатель,
		|	втШапка.Префикс КАК Префикс
		|ИЗ
		|	втШапка КАК втШапка
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СборкаЗапасовПродукция.Ссылка КАК Ссылка,
		|	СборкаЗапасовПродукция.НомерСтроки КАК НомерСтроки,
		|	ПОДСТРОКА(СборкаЗапасовПродукция.Номенклатура.НаименованиеПолное, 1, 150) КАК Продукция,
		|	СборкаЗапасовПродукция.Номенклатура.Артикул КАК Артикул,
		|	СборкаЗапасовПродукция.Характеристика КАК Характеристика,
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовПродукция.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	СборкаЗапасовПродукция.Количество КАК Количество,
		|	ХозрасчетныйДвиженияССубконто.Сумма КАК Себестоимость,
		|	ХозрасчетныйДвиженияССубконто.Сумма / СборкаЗапасовПродукция.Количество КАК СебестоимостьЕдиницы
		|ИЗ
		|	втШапка КАК втШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(, , ВЫРАЗИТЬ(Регистратор КАК Документ.СборкаЗапасов) В (&МассивОбъектов), , ) КАК ХозрасчетныйДвиженияССубконто
		|			ПО (СборкаЗапасовПродукция.Ссылка = (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.СборкаЗапасов)))
		|				И (СборкаЗапасовПродукция.Номенклатура В (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт1 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт2 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт3 КАК Справочник.Номенклатура)))
		|				И СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл = ХозрасчетныйДвиженияССубконто.СчетДт
		|				И СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗатратРегл = ХозрасчетныйДвиженияССубконто.СчетКт
		|		ПО втШапка.Ссылка = СборкаЗапасовПродукция.Ссылка
		|ГДЕ
		|	втШапка.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
		|	И СборкаЗапасовПродукция.Ссылка В(&МассивОбъектов)
		|	И НЕ СборкаЗапасовПродукция.Количество = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СборкаЗапасовПродукция.Ссылка,
		|	СборкаЗапасовПродукция.НомерСтроки,
		|	ПОДСТРОКА(СборкаЗапасовПродукция.Номенклатура.НаименованиеПолное, 1, 150),
		|	СборкаЗапасовПродукция.Номенклатура.Артикул,
		|	СборкаЗапасовПродукция.Характеристика,
		|	СборкаЗапасовПродукция.Номенклатура,
		|	СборкаЗапасовПродукция.ЕдиницаИзмерения,
		|	СборкаЗапасовПродукция.Количество,
		|	ХозрасчетныйДвиженияССубконто.Сумма * СборкаЗапасовПродукция.Количество / ХозрасчетныйДвиженияССубконто.КоличествоДт,
		|	ХозрасчетныйДвиженияССубконто.Сумма / ХозрасчетныйДвиженияССубконто.КоличествоДт
		|ИЗ
		|	втШапка КАК втШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(, , ВЫРАЗИТЬ(Регистратор КАК Документ.СборкаЗапасов) В (&МассивОбъектов), , ) КАК ХозрасчетныйДвиженияССубконто
		|			ПО (СборкаЗапасовПродукция.Ссылка = (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.СборкаЗапасов)))
		|				И (СборкаЗапасовПродукция.Номенклатура В (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт1 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт2 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт3 КАК Справочник.Номенклатура)))
		|				И СборкаЗапасовПродукция.Номенклатура.СчетУчетаЗапасовРегл = ХозрасчетныйДвиженияССубконто.СчетДт
		|		ПО втШапка.Ссылка = СборкаЗапасовПродукция.Ссылка
		|ГДЕ
		|	втШапка.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|	И СборкаЗапасовПродукция.Ссылка В(&МассивОбъектов)
		|	И НЕ СборкаЗапасовПродукция.Количество = 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	НомерСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.Ссылка КАК Ссылка,
		|	ПОДСТРОКА(СборкаЗапасовЗапасы.Номенклатура.НаименованиеПолное, 1, 150) КАК Материал,
		|	СборкаЗапасовЗапасы.Номенклатура.Артикул КАК Артикул,
		|	СборкаЗапасовЗапасы.Характеристика КАК Характеристика,
		|	СборкаЗапасовЗапасы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	СборкаЗапасовЗапасы.Количество КАК Количество,
		|	ХозрасчетныйДвиженияССубконто.Сумма КАК Себестоимость,
		|	ХозрасчетныйДвиженияССубконто.Сумма / СборкаЗапасовЗапасы.Количество КАК СебестоимостьЕдиницы
		|ИЗ
		|	втШапка КАК втШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(, , ВЫРАЗИТЬ(Регистратор КАК Документ.СборкаЗапасов) В (&МассивОбъектов), , ) КАК ХозрасчетныйДвиженияССубконто
		|			ПО (СборкаЗапасовЗапасы.Ссылка = (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.СборкаЗапасов)))
		|				И (СборкаЗапасовЗапасы.Номенклатура В (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт1 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт2 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоДт3 КАК Справочник.Номенклатура)))
		|				И СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл = ХозрасчетныйДвиженияССубконто.СчетДт
		|				И СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл = ХозрасчетныйДвиженияССубконто.СчетКт
		|		ПО втШапка.Ссылка = СборкаЗапасовЗапасы.Ссылка
		|ГДЕ
		|	втШапка.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
		|	И СборкаЗапасовЗапасы.Ссылка В(&МассивОбъектов)
		|	И НЕ СборкаЗапасовЗапасы.Количество = 0
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		|	СборкаЗапасовЗапасы.Ссылка,
		|	ПОДСТРОКА(СборкаЗапасовЗапасы.Номенклатура.НаименованиеПолное, 1, 150),
		|	СборкаЗапасовЗапасы.Номенклатура.Артикул,
		|	СборкаЗапасовЗапасы.Характеристика,
		|	СборкаЗапасовЗапасы.Номенклатура,
		|	СборкаЗапасовЗапасы.ЕдиницаИзмерения,
		|	СУММА(СборкаЗапасовЗапасы.Количество),
		|	СУММА(ХозрасчетныйДвиженияССубконто.Сумма * СборкаЗапасовЗапасы.Количество / ХозрасчетныйДвиженияССубконто.КоличествоКт),
		|	СУММА(ХозрасчетныйДвиженияССубконто.Сумма) / СУММА(ХозрасчетныйДвиженияССубконто.КоличествоКт)
		|ИЗ
		|	втШапка КАК втШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(, , ВЫРАЗИТЬ(Регистратор КАК Документ.СборкаЗапасов) В (&МассивОбъектов), , ) КАК ХозрасчетныйДвиженияССубконто
		|			ПО (СборкаЗапасовЗапасы.Ссылка = (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.СборкаЗапасов)))
		|				И (СборкаЗапасовЗапасы.Номенклатура В (ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоКт1 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоКт2 КАК Справочник.Номенклатура), ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.СубконтоКт3 КАК Справочник.Номенклатура)))
		|				И СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл = ХозрасчетныйДвиженияССубконто.СчетКт
		|		ПО втШапка.Ссылка = СборкаЗапасовЗапасы.Ссылка
		|ГДЕ
		|	втШапка.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
		|	И СборкаЗапасовЗапасы.Ссылка В(&МассивОбъектов)
		|	И НЕ СборкаЗапасовЗапасы.Количество = 0
		|
		|СГРУППИРОВАТЬ ПО
		|	СборкаЗапасовЗапасы.ЕдиницаИзмерения,
		|	СборкаЗапасовЗапасы.Ссылка,
		|	СборкаЗапасовЗапасы.Характеристика,
		|	СборкаЗапасовЗапасы.Номенклатура,
		|	СборкаЗапасовЗапасы.Номенклатура.Артикул,
		|	ПОДСТРОКА(СборкаЗапасовЗапасы.Номенклатура.НаименованиеПолное, 1, 150)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка";
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		Шапка 		= МассивРезультатов[1].Выбрать();
		Продукция 	= МассивРезультатов[2].Выгрузить();
		Запасы 		= МассивРезультатов[3].Выгрузить();
		
		ПервыйДокумент = Истина;
		
		Пока Шапка.Следующий() Цикл
			
			Если Не ПервыйДокумент Тогда
				ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
			КонецЕсли;
			ПервыйДокумент = Ложь;
			
			НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
			
			ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаПроизводство_МакетТребованиеНаСклад";
			
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СборкаЗапасов.ПФ_MXL_МакетТребованиеНаСкладПроизводствоПроекты");
			
			НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс);
			
			ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
			ОбластьМакета.Параметры.ТекстЗаголовка = "წარმოება № "
			+ НомерДокумента
			+ " , "
			+ Формат(Шапка.ДатаДокумента, "Л=ka; ДФ='dd MMMM yyyy'");
			
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// Олег Литвиненко 12.07.23 --->
			ОбластьИтоговаяСумма = Макет.ПолучитьОбласть("ИтоговаяСумма");
			// Олег Литвиненко <---

			// Шапка.
			ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
			ОбластьМакета.Параметры.Заполнить(Шапка);
			ОбластьМакета.Параметры.ДатаЗапуска = Формат(Шапка.ДатаДокумента, "ДФ=dd.MM.yyyy");
			
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// ТЧ Продукция.
			ТаблицаПродукция = Продукция.Скопировать(Новый Структура("Ссылка", Шапка.Ссылка));
			
			ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицыПродукция");
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			ОбластьМакета = Макет.ПолучитьОбласть("СтрокаПродукция");
			
			НомерСтроки = 1;
			Для Каждого ТекСтр Из ТаблицаПродукция Цикл
				
				ОбластьМакета.Параметры.Заполнить(ТекСтр);
				
				ОбластьМакета.Параметры.Продукция = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ТекСтр.Продукция,
				ТекСтр.Характеристика, ТекСтр.Артикул);
				
				ОбластьМакета.Параметры.Себест = ТекСтр.СебестоимостьЕдиницы;
				ОбластьМакета.Параметры.СебестСумм = ТекСтр.Себестоимость;
				
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
				НомерСтроки = НомерСтроки + 1;
				
			КонецЦикла;
			
			//ОбластьМакета = Макет.ПолучитьОбласть("ИтогоПродукция");
			//ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// Олег Литвиненко 12.07.23 --->
			ОбластьИтоговаяСумма.Параметры.СуммаИтог = ТаблицаПродукция.Итог("Себестоимость"); 
			ТабличныйДокумент.Вывести(ОбластьИтоговаяСумма);
			// Олег Литвиненко <---
			
			// ТЧ Запасы.
			ТаблицаЗапасы = Запасы.Скопировать(Новый Структура("Ссылка", Шапка.Ссылка));

			ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицы");
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			ОбластьМакета = Макет.ПолучитьОбласть("Строка");
			
			НомерСтроки = 1;
			Для Каждого ТекСтр Из ТаблицаЗапасы Цикл
				ОбластьМакета.Параметры.Заполнить(ТекСтр);
				
				ОбластьМакета.Параметры.Материал = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ТекСтр.Материал, 
				ТекСтр.Характеристика, ТекСтр.Артикул);
				
				ОбластьМакета.Параметры.Себест = ТекСтр.СебестоимостьЕдиницы;
				ОбластьМакета.Параметры.СебестСумм = ТекСтр.Себестоимость;
				ОбластьМакета.Параметры.НомерСтроки = НомерСтроки;
				
				ТабличныйДокумент.Вывести(ОбластьМакета);
				
				НомерСтроки = НомерСтроки + 1;
				
			КонецЦикла;
			
			//ОбластьМакета = Макет.ПолучитьОбласть("Итого");
			//ТабличныйДокумент.Вывести(ОбластьМакета);
			
			// Олег Литвиненко 12.07.23 --->
			ОбластьИтоговаяСумма.Параметры.СуммаИтог = ТаблицаЗапасы.Итог("Себестоимость"); 
			ТабличныйДокумент.Вывести(ОбластьИтоговаяСумма);
			// Олег Литвиненко <---
			
			// Подпись.
			ОбластьМакета = Макет.ПолучитьОбласть("Подписи");
			ОбластьМакета.Параметры.Заполнить(Шапка);
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Шапка.Ссылка);
			
		КонецЦикла;
	КонецПроцедуры// Олег Литвиненко <---
	
	// Сформировать печатные формы объектов.
	//
	// ВХОДЯЩИЕ:
	//   ИменаМакетов    - Строка    - Имена макетов, перечисленные через запятую
	//   МассивОбъектов  - Массив    - Массив ссылок на объекты которые нужно распечатать
	//   ПараметрыПечати - Структура - Структура дополнительных параметров печати
	//
	// ИСХОДЯЩИЕ:
	//   КоллекцияПечатныхФорм - Таблица значений - Сформированные табличные документы
	//   ПараметрыВывода       - Структура        - Параметры сформированных табличных документов
	//
	Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
		
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "М11") Тогда
			
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "М11", "М-11", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "М11"));
			
		КонецЕсли;
		
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МХ18") Тогда
			
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МХ18", "МХ-18", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "МХ18"));
			
		КонецЕсли;
		
		ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "БланкТоварногоНаполнения");
		Если ПечатнаяФорма <> Неопределено Тогда
			
			ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
			ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = Обработки.ПечатьБланкТоварногоНаполнения.КлючПараметровПечати();
			ПечатнаяФорма.ПолныйПутьКМакету = Обработки.ПечатьБланкТоварногоНаполнения.ПолныйПутьКМакету();
			ПечатнаяФорма.СинонимМакета = Обработки.ПечатьБланкТоварногоНаполнения.ПредставлениеПФ();
			
			ДанныеОбъектовПечати = ДанныеДокументовБланкТоварногоНаполнения(МассивОбъектов);
			Обработки.ПечатьБланкТоварногоНаполнения.СформироватьПФ(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати);
			
		КонецЕсли;
		
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ГарантийныйТалонПродукция") Тогда
			
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ГарантийныйТалонПродукция", "Гарантийный талон (продукция)", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ГарантийныйТалонПродукция"));
			
		КонецЕсли;
		
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ГарантийныйТалонКомплектующие") Тогда
			
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ГарантийныйТалонКомплектующие", "Гарантийный талон (комплектующие)", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ГарантийныйТалонКомплектующие"));
			
		КонецЕсли;
		
		
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТребованиеНаСклад") Тогда
			
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТребованиеНаСклад", "Требование на склад", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ПФ_MXL_МакетТребованиеНаСкладПроизводство"));
			
		КонецЕсли;
		
		
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТребованиеНаСкладПроекты") Тогда
			
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТребованиеНаСкладПроекты", "Требование на склад (Проекты)", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ПФ_MXL_МакетТребованиеНаСкладПроизводствоПроекты"));
			
		КонецЕсли;
		
		
		// параметры отправки печатных форм по электронной почте
		УправлениеНебольшойФирмойСервер.ЗаполнитьПараметрыОтправки(ПараметрыВывода.ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм);
		
	КонецПроцедуры // Печать()
	
	// Заполняет список команд печати.
	// 
	// Параметры:
	//   КомандыПечати - ТаблицаЗначений - состав полей см. в функции УправлениеПечатью.СоздатьКоллекциюКомандПечати.
	//
	Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "М11,МХ18";
		КомандаПечати.Представление = НСтр("ka='Настраиваемый комплект документов';ru='Настраиваемый комплект документов';en='Customizable set of documents'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.Порядок = 1;
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "М11";
		КомандаПечати.Представление = НСтр("ka='М11 (მოთხოვნა-ზედნადები)';ru='М11 (Требование-накладная)';en='M11 (requirement-waybill)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.Порядок = 4;
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "МХ18";
		КомандаПечати.Представление = НСтр("ka='МХ18 (მზა პროდუქციის გადაცემის ზედნადები)';ru='МХ18 (Накладная на передачу готовой продукции)';en='MH18 (Invoice to transfer the finished product)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.Порядок = 7;
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "ГарантийныйТалонПродукция";
		КомандаПечати.Представление = НСтр("ka='Гарантийный талон на продукцию';ru='Гарантийный талон на продукцию';en='Warranty card product'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.ФункциональныеОпции = "ИспользоватьСерийныеНомера";
		КомандаПечати.Порядок = 10;
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "ГарантийныйТалонКомплектующие";
		КомандаПечати.Представление = НСтр("ka='Гарантийный талон на комплектующие';ru='Гарантийный талон на комплектующие';en='Warranty card for accessories'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.ФункциональныеОпции = "ИспользоватьСерийныеНомера";
		КомандаПечати.Порядок = 14;
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "БланкТоварногоНаполнения";
		КомандаПечати.Представление = Обработки.ПечатьБланкТоварногоНаполнения.ПредставлениеПФ();
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.Порядок = 17;
		
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "ТребованиеНаСклад";
		КомандаПечати.Представление = НСтр("ka='მოთხოვნა საწყობში';ru='Требование на склад';en='Demand for warehouse'");
		КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.Порядок = 2;
		
		
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "ТребованиеНаСкладПроекты";
		КомандаПечати.Представление = НСтр("ka='მოთხოვნა საწყობში (პროექტები)';ru='Требование на склад (Проекты)';en='Demand for warehouse (Project)'");
		КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		КомандаПечати.Порядок = 99;
		
		
		
	КонецПроцедуры
	
	Процедура ЗаполнитьКоличествоПоДебету(ТаблицаЗапасыЗапасы, тзПродукции)
		
		ТаблицаЗапасыЗапасы.Колонки.Добавить("КоличествоДт");	
		
		Для Каждого Продукция Из тзПродукции Цикл 
			
			СтруктураОтбора = Новый Структура("КоррНоменклатура, КоррХарактеристика", Продукция.Номенклатура, Продукция.Характеристика); 
			МассивСтрокЗапасыЗапасы = ТаблицаЗапасыЗапасы.НайтиСтроки(СтруктураОтбора);
			
			КоличествоПродукцииПоСтроке = Продукция.Количество;
			Если МассивСтрокЗапасыЗапасы.Количество() = 0 Тогда 
				Продолжить;
			КонецЕсли;
			
			КолДт = Окр(Продукция.Количество / МассивСтрокЗапасыЗапасы.Количество(), 3);
			Для Каждого ЗапасаЗапаса Из МассивСтрокЗапасыЗапасы Цикл 
				
				ЗапасаЗапаса.КоличествоДт = КолДт;
				КоличествоПродукцииПоСтроке = КоличествоПродукцииПоСтроке - КолДт;
				
			КонецЦикла;
			
			Если КоличествоПродукцииПоСтроке <> 0 Тогда
				
				ЗапасаЗапаса.КоличествоДт = ЗапасаЗапаса.КоличествоДт + КоличествоПродукцииПоСтроке;
				
			КонецЕсли;
			
		КонецЦикла;
		
	КонецПроцедуры
	
	//+++ Дмгг
	Процедура СформироватьТаблицыВариантовПродукции(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
		
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	&КлючСвязи КАК Ссылка,
		|	СборкаЗапасовПродукция.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовПродукция.Характеристика КАК Характеристика,
		|	СборкаЗапасовПродукция.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	СборкаЗапасовПродукция.Количество КАК Количество
		|ПОМЕСТИТЬ ВременнаяТаблицаПродукция
		|ИЗ
		|	Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
		|ГДЕ
		|	СборкаЗапасовПродукция.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	&КлючСвязи КАК Ссылка,
		|	СборкаЗапасовЗапасы.Номенклатура КАК Номенклатура,
		|	СборкаЗапасовЗапасы.Характеристика КАК Характеристика,
		|	СборкаЗапасовЗапасы.Количество КАК Количество,
		|	СборкаЗапасовЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения
		|ПОМЕСТИТЬ ВременнаяТаблицаМатериалы
		|ИЗ
		|	Документ.СборкаЗапасов.Запасы КАК СборкаЗапасовЗапасы
		|ГДЕ
		|	СборкаЗапасовЗапасы.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаПродукция.Ссылка КАК Ссылка,
		|	ВЫБОР
		|		КОГДА ВариантыПроизводстваПродукция.Продукция ЕСТЬ NULL
		|			ТОГДА ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|		ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|	КОНЕЦ КАК ЕстьНоменклатура,
		|	ВЫБОР
		|		КОГДА ВариантыПроизводстваПродукция.Характеристика ЕСТЬ NULL
		|			ТОГДА ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|		ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|	КОНЕЦ КАК ЕстьХарактеристика,
		|	ВЫБОР
		|		КОГДА ВариантыПроизводстваПродукция.ЕдиницаИзмерений ЕСТЬ NULL
		|			ТОГДА ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|		ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|	КОНЕЦ КАК ЕстьЕдиницаИзмерения,
		|	ВЫБОР
		|		КОГДА ВариантыПроизводстваПродукция.Количество ЕСТЬ NULL
		|			ТОГДА ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|		ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваПродукция.КлючСвязи, """") = ВременнаяТаблицаПродукция.Ссылка
		|	КОНЕЦ КАК ЕстьКоличество,
		|	ВариантыПроизводстваПродукция.КлючСвязи КАК КлючСвязи
		|ПОМЕСТИТЬ ВременнаяТаблицаДубльПродукции
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ВременнаяТаблицаПродукция
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВариантыПроизводстваПродукция КАК ВариантыПроизводстваПродукция
		|		ПО ВременнаяТаблицаПродукция.Номенклатура = ВариантыПроизводстваПродукция.Продукция
		|			И ВременнаяТаблицаПродукция.Характеристика = ВариантыПроизводстваПродукция.Характеристика
		|			И ВременнаяТаблицаПродукция.ЕдиницаИзмерения = ВариантыПроизводстваПродукция.ЕдиницаИзмерений
		|			И ВременнаяТаблицаПродукция.Количество = ВариантыПроизводстваПродукция.Количество
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаМатериалы.Ссылка КАК Ссылка,
		|	МИНИМУМ(ВЫБОР
		|			КОГДА ЕСТЬNULL(ВариантыПроизводстваМатериалы.Характеристика, ЛОЖЬ) = ЛОЖЬ
		|				ТОГДА ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|			ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|		КОНЕЦ) КАК ЕстьХарактеристика,
		|	МИНИМУМ(ВЫБОР
		|			КОГДА ВариантыПроизводстваМатериалы.ЕдиницаИзмерений ЕСТЬ NULL
		|				ТОГДА ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|			ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|		КОНЕЦ) КАК ЕстьЕдиницаИзмерения,
		|	МИНИМУМ(ВЫБОР
		|			КОГДА ВариантыПроизводстваМатериалы.Материал ЕСТЬ NULL
		|				ТОГДА ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|			ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|		КОНЕЦ) КАК ЕстьНоменклатура,
		|	МИНИМУМ(ВЫБОР
		|			КОГДА ВариантыПроизводстваМатериалы.Количество ЕСТЬ NULL
		|				ТОГДА ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|			ИНАЧЕ НЕ ЕСТЬNULL(ВариантыПроизводстваМатериалы.КлючСвязи, """") = ВременнаяТаблицаМатериалы.Ссылка
		|		КОНЕЦ) КАК ЕстьКоличество,
		|	ВариантыПроизводстваМатериалы.КлючСвязи КАК КлючСвязи
		|ПОМЕСТИТЬ ВременнаяТаблицаДубльМатериалов
		|ИЗ
		|	ВременнаяТаблицаМатериалы КАК ВременнаяТаблицаМатериалы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВариантыПроизводстваМатериалы КАК ВариантыПроизводстваМатериалы
		|		ПО ВременнаяТаблицаМатериалы.Номенклатура = ВариантыПроизводстваМатериалы.Материал
		|			И ВременнаяТаблицаМатериалы.Характеристика = ВариантыПроизводстваМатериалы.Характеристика
		|			И ВременнаяТаблицаМатериалы.ЕдиницаИзмерения = ВариантыПроизводстваМатериалы.ЕдиницаИзмерений
		|			И ВременнаяТаблицаМатериалы.Количество = ВариантыПроизводстваМатериалы.Количество
		|
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблицаМатериалы.Ссылка,
		|	ВариантыПроизводстваМатериалы.КлючСвязи
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаДубльПродукции.Ссылка КАК Ссылка,
		|	ВременнаяТаблицаДубльПродукции.ЕстьНоменклатура КАК ЕстьНоменклатура,
		|	ВременнаяТаблицаДубльПродукции.ЕстьХарактеристика КАК ЕстьХарактеристика,
		|	ВременнаяТаблицаДубльПродукции.ЕстьЕдиницаИзмерения КАК ЕстьЕдиницаИзмерения,
		|	ВременнаяТаблицаДубльПродукции.ЕстьКоличество КАК ЕстьКоличество
		|ПОМЕСТИТЬ ВременнаяТаблицаМатериалыПродукция
		|ИЗ
		|	ВременнаяТаблицаДубльПродукции КАК ВременнаяТаблицаДубльПродукции
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВременнаяТаблицаДубльМатериалов.Ссылка,
		|	ВременнаяТаблицаДубльМатериалов.ЕстьНоменклатура,
		|	ВременнаяТаблицаДубльМатериалов.ЕстьХарактеристика,
		|	ВременнаяТаблицаДубльМатериалов.ЕстьЕдиницаИзмерения,
		|	ВременнаяТаблицаДубльМатериалов.ЕстьКоличество
		|ИЗ
		|	ВременнаяТаблицаДубльМатериалов КАК ВременнаяТаблицаДубльМатериалов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаМатериалыПродукция.Ссылка КАК Ссылка,
		|	МИНИМУМ(ВременнаяТаблицаМатериалыПродукция.ЕстьНоменклатура) КАК ЕстьНоменклатура,
		|	МИНИМУМ(ВременнаяТаблицаМатериалыПродукция.ЕстьХарактеристика) КАК ЕстьХарактеристика,
		|	МИНИМУМ(ВременнаяТаблицаМатериалыПродукция.ЕстьЕдиницаИзмерения) КАК ЕстьЕдиницаИзмерения,
		|	МИНИМУМ(ВременнаяТаблицаМатериалыПродукция.ЕстьКоличество) КАК ЕстьКоличество
		|ПОМЕСТИТЬ ВременнаяТаблицаТредуетьсяЗаписьВРегистр
		|ИЗ
		|	ВременнаяТаблицаМатериалыПродукция КАК ВременнаяТаблицаМатериалыПродукция
		|
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблицаМатериалыПродукция.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаПродукция.Ссылка КАК КлючСвязи,
		|	ВременнаяТаблицаПродукция.Номенклатура КАК Продукция,
		|	ВременнаяТаблицаПродукция.Характеристика КАК Характеристика,
		|	ВременнаяТаблицаПродукция.Количество КАК Количество,
		|	ВременнаяТаблицаПродукция.ЕдиницаИзмерения КАК ЕдиницаИзмерений
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ВременнаяТаблицаПродукция
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр КАК ВременнаяТаблицаТредуетьсяЗаписьВРегистр
		|		ПО ВременнаяТаблицаПродукция.Ссылка = ВременнаяТаблицаТредуетьсяЗаписьВРегистр.Ссылка
		|ГДЕ
		|	(НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьНоменклатура
		|			ИЛИ НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьХарактеристика
		|			ИЛИ НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьЕдиницаИзмерения
		|			ИЛИ НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьКоличество)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаМатериалы.Ссылка КАК КлючСвязи,
		|	ВременнаяТаблицаМатериалы.Номенклатура КАК Материал,
		|	ВременнаяТаблицаМатериалы.Характеристика КАК Характеристика,
		|	ВременнаяТаблицаМатериалы.Количество КАК Количество,
		|	ВременнаяТаблицаМатериалы.ЕдиницаИзмерения КАК ЕдиницаИзмерений
		|ИЗ
		|	ВременнаяТаблицаМатериалы КАК ВременнаяТаблицаМатериалы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр КАК ВременнаяТаблицаТредуетьсяЗаписьВРегистр
		|		ПО ВременнаяТаблицаМатериалы.Ссылка = ВременнаяТаблицаТредуетьсяЗаписьВРегистр.Ссылка
		|ГДЕ
		|	(НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьНоменклатура
		|			ИЛИ НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьХарактеристика
		|			ИЛИ НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьЕдиницаИзмерения
		|			ИЛИ НЕ ВременнаяТаблицаТредуетьсяЗаписьВРегистр.ЕстьКоличество)";
		
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаСборкаЗапасов);
		Запрос.УстановитьПараметр("КлючСвязи", Строка(ДокументСсылкаСборкаЗапасов.УникальныйИдентификатор()));
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВариантыПроизводстваПродукция", МассивРезультатов[6].Выгрузить());
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВариантыПроизводстваМатериалы", МассивРезультатов[7].Выгрузить());
		
	КонецПроцедуры
	//--- Дмгг
	
	#КонецОбласти
	
	//Малышев 2024-06-25    (--
	Процедура ПроверитьСчетУчетаЗатратРегл_ДвиженияЗапасов(СтрокаТаблицыПриход, СтрокаТаблицаЗапасыПродукция = Неопределено)
		
		СчетУчетаЗатратРегл = СтрокаТаблицыПриход.Номенклатура.СчетУчетаЗатратРегл; 
		Если СтрокаТаблицаЗапасыПродукция<>Неопределено Тогда
			//СчетУчетаЗатратРегл = СтрокаТаблицаЗапасыПродукция.СчетУчетаПродукцииРегл;
		КонецЕсли;	
				
		Если СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение 
			и не ЗначениеЗаполнено(СтрокаТаблицыПриход.СчетУчетаЗатратРегл) Тогда 
			  СтрокаТаблицыПриход.СчетУчетаЗатратРегл = СчетУчетаЗатратРегл;
		ИначеЕсли СтрокаТаблицыПриход.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад 
			и ЗначениеЗаполнено(СтрокаТаблицыПриход.СчетУчетаЗатратРегл) Тогда 
			  СтрокаТаблицыПриход.СчетУчетаЗатратРегл = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
		КонецЕсли;	
    КонецПроцедуры
	
#КонецЕсли


#Область ЗагрузкаДанныхИзВнешнегоИсточника

Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	
	//
	// Для группы полей действует правило: хотя бы одно поле в группе должно быть выбрано в колонках
	//
	
	ОписаниеТиповСтрока25 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(25));
	ОписаниеТиповСтрока30 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(30));
	ОписаниеТиповСтрока50 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(50));
	ОписаниеТиповСтрока100 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(100));
	ОписаниеТиповСтрока150 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(150));
	ОписаниеТиповСтрока200 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(200));
	ОписаниеТиповСтрока1000 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(1000));
	ОписаниеТиповЧисло15_2 = Новый ОписаниеТипов("Число", , , , Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный));
	ОписаниеТиповЧисло15_3 = Новый ОписаниеТипов("Число", , , , Новый КвалификаторыЧисла(15, 3, ДопустимыйЗнак.Неотрицательный));
	ОписаниеТиповДата = Новый ОписаниеТипов("Дата", , , , Новый КвалификаторыДаты(ЧастиДаты.Дата));
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Номенклатура");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Штрихкод", НСтр("en='Barcode';ka='შტრიხ კოდი';ru='Штрихкод'"), ОписаниеТиповСтрока200, ОписаниеТиповКолонка, "Номенклатура", 1, , Истина);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Артикул", НСтр("en='OptionCode';ka='არტიკული';ru='Артикул'"), ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Номенклатура", 2, , Истина);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НоменклатураНаименование", НСтр("ka='ნომენკლატურა (დასახელება)';ru='Номенклатура (наименование)';en='Nomenclature (name)'"), ОписаниеТиповСтрока100, ОписаниеТиповКолонка, "Номенклатура", 3, , Истина);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НоменклатураНаименованиеПолное",НСтр("ka='ნომენკლატურა (სრული დასახელება)';ru='Номенклатура (полное наименование';en='Nomenclature (full name'"), ОписаниеТиповСтрока1000, ОписаниеТиповКолонка, "Номенклатура", 5, , Истина);
	
	//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Содержание",  НСтр("en='Content';ka='შინაარსი';ru='Содержание'"), ОписаниеТиповСтрока1000, ОписаниеТиповСтрока1000, , , , , НастройкиЗагрузкиДанных.СодержаниеВидимо);
	
	// Олег Литвиненко 12.07.23 --->
	//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Проект",  НСтр("ru = 'Проект'; ka = 'პროექტი'; en = 'Project'"), ОписаниеТиповСтрока150, Новый ОписаниеТипов("СправочникСсылка.Проекты"));
	// Олег Литвиненко <---
	
	//НСтр("ru='';ka=''");
	Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Характеристика",  НСтр("ru = 'Характеристика (наименование)'; ka = 'მახასიათებელი  (დასახელება)'; en = 'Characteristics (name)'"), ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
		
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартии") Тогда
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.ПартииНоменклатуры");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Партия", НСтр("ka='პარტია (დასახელება)';ru='Партия (наименование)';en='Party (name)'"), ОписаниеТиповСтрока100, ОписаниеТиповКолонка);
		
	КонецЕсли;
	
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Количество",  НСтр("en='Quantity';ka='რაოდენობა';ru='Количество'"), ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_3, , , Истина);
	
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Резерв", НСтр("ka='რეზერვი';ru='Резерв';en='Резерв'"), ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_3, , , , , ПолучитьФункциональнуюОпцию("РезервированиеЗапасов"));
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.КлассификаторЕдиницИзмерения, СправочникСсылка.ЕдиницыИзмерения");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ЕдиницаИзмерения", НСтр("en='Unit Measurement';ka='საზომი ერთეული';ru='Ед. изм.'"), ОписаниеТиповСтрока25, ОписаниеТиповКолонка, , , , , ПолучитьФункциональнуюОпцию("УчетВРазличныхЕдиницахИзмерения"));
	
	//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Цена", НСтр("en='Price';ka='ფასი';ru='Цена'"), ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2, , , Ложь);
	
	//ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС");
	//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтавкаНДС", НСтр("ka='დღგ';ru='Ставка НДС';en='VAT Rate'"), ОписаниеТиповСтрока50, ОписаниеТиповКолонка);
	//
	//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СуммаНДС",  НСтр("en='VAT Amount';ka='დღგ-ს თანხა';ru='Сумма НДС'"), ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2);
	
	//МассивТипов = Новый Массив;
	//МассивТипов.Добавить(Тип("ДокументСсылка.ЗаказПокупателя"));
	//МассивТипов.Добавить(Тип("ДокументСсылка.ЗаказПоставщику"));
	//
	//ВидимостьЗаказаПокупателя = (НастройкиЗагрузкиДанных.ПоложениеЗаказаВДокументе = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти);
	//ОписаниеТиповКолонка = Новый ОписаниеТипов(МассивТипов);
	//ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Заказ", НСтр("ka='შეკვეთა (მყიდველი/მომწოდებელი)';ru='Заказ (покупатель/поставщик)';en='Check (customer / supplier)'"), ОписаниеТиповСтрока50, ОписаниеТиповКолонка, , , , , ВидимостьЗаказаПокупателя);
	
	//Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
	//	
	//	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.СтраныМира");
	//	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтранаПроисхождения", НСтр("en='Country of Origin';ka='მწარმოებელი ქვეყანა';ru='Страна происхождения'"), ОписаниеТиповСтрока100, ОписаниеТиповКолонка);
	//	
	//	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.НомераГТД");
	//	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерГТД", НСтр("ka='საბაჭო დეკლარაციის ნომერი';ru='Номер ГТД';en='Номер ГТД'"), ОписаниеТиповСтрока30, ОписаниеТиповКолонка);
	//	
	//КонецЕсли;
	
КонецПроцедуры

Процедура ПриОпределенииОбразцовЗагрузкиДанных(НастройкиЗагрузкиДанных, УникальныйИдентификатор) Экспорт
	
	Образец_xlsx = ПолучитьМакет("ОбразецЗагрузкиДанных_xlsx");
	ОбразецЗагрузкиДанных_xlsx = ПоместитьВоВременноеХранилище(Образец_xlsx, УникальныйИдентификатор);
	НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_xlsx", ОбразецЗагрузкиДанных_xlsx);
	
	НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_mxl", "ОбразецЗагрузкиДанных_mxl");
	
	Образец_csv = ПолучитьМакет("ОбразецЗагрузкиДанных_csv");
	ОбразецЗагрузкиДанных_csv = ПоместитьВоВременноеХранилище(Образец_csv, УникальныйИдентификатор);
	НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_csv", ОбразецЗагрузкиДанных_csv);
	
КонецПроцедуры

Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных) Экспорт
	
	ПолноеИмяОбъектаЗаполнения = НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения;
	
	// ТаблицаСопоставленияДанных - Тип ДанныеФормыКоллекция
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		
		// Номенклатура по ШтрихКоду, Артикулу, Наименованию, НаименованиеПолное
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНоменклатуру(СтрокаТаблицыФормы.Номенклатура,СтрокаТаблицыФормы.Штрихкод,СтрокаТаблицыФормы.Артикул, СтрокаТаблицыФормы.НоменклатураНаименование, СтрокаТаблицыФормы.НоменклатураНаименованиеПолное);
		
		//// Содержание
		//ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СкопироватьСтрокуВЗначениеСтроковогоТипа(СтрокаТаблицыФормы.Содержание, СтрокаТаблицыФормы.Содержание_ВходящиеДанные);
		
		// Характеристика по Владельцу и Наименованию
		Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
			
			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) Тогда
				
				ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьХарактеристику(СтрокаТаблицыФормы.Характеристика, СтрокаТаблицыФормы.Номенклатура, СтрокаТаблицыФормы.Штрихкод= Неопределено , СтрокаТаблицыФормы.Характеристика_ВходящиеДанные);
				
			КонецЕсли;
			
		КонецЕсли;
		
		// Партия по Владельцу и Наименованию
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартии") Тогда
			
			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) Тогда
				
				ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьПартию(СтрокаТаблицыФормы.Партия, СтрокаТаблицыФормы.Номенклатура, СтрокаТаблицыФормы.Штрихкод, СтрокаТаблицыФормы.Партия_ВходящиеДанные);
				
			КонецЕсли;
			
		КонецЕсли;
		
		// Количество
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Количество, СтрокаТаблицыФормы.Количество_ВходящиеДанные, 1);
		
		// Резерв
		Если ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") Тогда
			
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Резерв, СтрокаТаблицыФормы.Резерв_ВходящиеДанные, 0);
			
		КонецЕсли;
		
		// ЕдиницыИзмерения по Наименованию (так же рассмотреть возможность прикрутить пользовательские ЕИ)
		ЗначениеПоУмолчанию = ?(ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура), СтрокаТаблицыФормы.Номенклатура.ЕдиницаИзмерения, Справочники.КлассификаторЕдиницИзмерения.шт);
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьЕдиницыИзмерения(СтрокаТаблицыФормы.Номенклатура, СтрокаТаблицыФормы.ЕдиницаИзмерения, СтрокаТаблицыФормы.ЕдиницаИзмерения_ВходящиеДанные, ЗначениеПоУмолчанию);
		
		//// Цена
		//ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Цена, СтрокаТаблицыФормы.Цена_ВходящиеДанные, 0);
		//
		//// СтавкаНДС по наименованию
		//ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСтавкуНДС(СтрокаТаблицыФормы.СтавкаНДС, СтрокаТаблицыФормы.СтавкаНДС_ВходящиеДанные, Неопределено);
		//
		//// СуммаНДС
		//ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.СуммаНДС, СтрокаТаблицыФормы.СуммаНДС_ВходящиеДанные, 0);
		//
		//// Заказ
		//ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьЗаказ(СтрокаТаблицыФормы.Заказ, СтрокаТаблицыФормы.Заказ_ВходящиеДанные);
		
		// Страна и ГТД
		//Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
		//	
		//	ЗначениеПоУмолчанию = Неопределено;
		//	Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) Тогда
		//		
		//		ЗначениеПоУмолчанию = СтрокаТаблицыФормы.Номенклатура.СтранаПроисхождения;
		//		
		//	КонецЕсли;
		
		//ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСтрануПроисхождения(,, );
		//
		//Если ЗначениеЗаполнено(СтрокаТаблицыФормы.СтранаПроисхождения)
		//	И СтрокаТаблицыФормы.СтранаПроисхождения <> Справочники.СтраныМира.Россия Тогда
		//	
		//	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНомерГТД(СтрокаТаблицыФормы.НомерГТД, СтрокаТаблицыФормы.НомерГТД_ВходящиеДанные);
		//	
		//КонецЕсли;
		
		//КонецЕсли;
		
		ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения = "") Экспорт
	
	ИмяСлужебногоПоля = ЗагрузкаДанныхИзВнешнегоИсточника.ИмяСлужебногоПоляЗагрузкаВПриложениеВозможна();
	
	СтрокаТаблицыФормы[ИмяСлужебногоПоля] = ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура)
	И (СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас 
	//ИЛИ СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга)
	И СтрокаТаблицыФормы.Количество <> 0);
	
КонецПроцедуры

#КонецОбласти

// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
Процедура СформироватьТаблицаРезервированиеЗапасов(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МИНИМУМ(ТаблицаЗапасы.НомерСтроки) КАК НомерСтроки,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ТаблицаЗапасы.Период КАК Период,
	|	ТаблицаЗапасы.Организация КАК Организация,
	|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов КАК СтруктурнаяЕдиница,
	|	ТаблицаЗапасы.ЗаказПокупателя КАК Документ,
	|	ТаблицаЗапасы.Номенклатура КАК Номенклатура,
	|	ТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ТаблицаЗапасы.Партия КАК Партия,
	|	ВЫБОР
	|		КОГДА &УчетПоПроектам
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаЗапасы.Проект ЕСТЬ NULL
	|							ИЛИ ТаблицаЗапасы.Проект = НЕОПРЕДЕЛЕНО
	|							ИЛИ ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|						ТОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
	|					ИНАЧЕ ТаблицаЗапасы.Проект
	|				КОНЕЦ
	|	КОНЕЦ КАК Проект,
	|	СУММА(ТаблицаЗапасы.Резерв) КАК Количество
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы,
	|	Константы КАК Константы
	|ГДЕ
	|	Константы.ФункциональнаяОпцияРезервированиеЗапасов_new
	|	И ТаблицаЗапасы.Резерв > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЗапасы.Период,
	|	ТаблицаЗапасы.Организация,
	|	ТаблицаЗапасы.СтруктурнаяЕдиницаЗапасов,
	|	ТаблицаЗапасы.ЗаказПокупателя,
	|	ТаблицаЗапасы.Номенклатура,
	|	ТаблицаЗапасы.Характеристика,
	|	ТаблицаЗапасы.Партия,
	|	ВЫБОР
	|		КОГДА &УчетПоПроектам
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаЗапасы.Проект ЕСТЬ NULL
	|							ИЛИ ТаблицаЗапасы.Проект = НЕОПРЕДЕЛЕНО
	|							ИЛИ ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|						ТОГДА ТаблицаЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
	|					ИНАЧЕ ТаблицаЗапасы.Проект
	|				КОНЕЦ
	|	КОНЕЦ";
	
	Запрос.УстановитьПараметр("УчетПоПроектам",  СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРезервированиеЗапасов", РезультатЗапроса.Выгрузить());
	
КонецПроцедуры
// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления





