...
Процедура ЗаполнитьПоЗаказуПокупателя(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуПокупателяЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულიად შეივსება ""მყიდველის შეკვეთით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Заказу покупателя""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура КомандаЗаполнитьПоСпецификации(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მასალები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить выполнение операции?'"), 
		РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоОстаткамЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მასალები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить выполнение операции?'"), 
		РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьПоОстаткамПродукция(Команда)
	...
	Если Объект.Продукция.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоОстаткамПродукцияЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""პროდუქცია"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Продукция"" будет перезаполнена! Продолжить выполнение операции?'"), 
		РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
