...
"обнаружено рекурсивное вхождение элемента';ka='ცხრილ მასალები სპეციფიკაციის მიხედვით შევსების მცდელობისას, აღმოჩენილია ელემენტის რეკურსული შესვლა'")+" "+Выборка.Номенклатура+" "+НСтр("ka='სპეციფიკაციაში';ru='в спецификации'")+" "+Выборка.СпецификацияПродукции+"
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("Резерв") > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЗаказПокупателя) Тогда
			...
			ТекстСообщения = НСтр("ka='არ არის მითითებული მყიდველის შეკვეთა-რეზერვის წყარო!';ru='Не указан заказ покупателя- источник резерва!'");
		...
		Если ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда
			...
			Для каждого СтрокаЗапасы Из Запасы Цикл
				...
				Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
					...
					ТекстСообщения = НСтр("ka='ცხრილ ""მარაგების"" სტრიქონ №%Номер% რეზერვიდან ჩამოსაწერი პოზიციების რაოდენობა აღემატება მარაგების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Запасы"" количество позиций к списанию из резерва превышает общее количество запасов.'");
		...
		Иначе
			...
			Для каждого СтрокаПродукция Из Продукция Цикл
				...
				Если СтрокаПродукция.Резерв > СтрокаПродукция.Количество Тогда
					...
					ТекстСообщения = НСтр("ka='ცხრილ ""პროდუქცია""-ს სტრიქონ №%Номер%-ში რეზერვიდან ჩამოსაწერი პოზიციების რაოდენობა აღემატება პროდუქციის საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Продукция"" количество позиций к списанию из резерва превышает общее количество продукции.'");
