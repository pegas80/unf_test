...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоНовый() И ЗначениеЗаполнено(Основание) Тогда
		...
		Если Не Результат.Пустой() Тогда
			...
			ТекстОшибки = НСтр("ka='დოკუმენტ %1-ისთვის უკვე არსებობს მიმდინარე %2.';ru='Для документа %1 уже существует текущая %2.'");
	...
	Для каждого СтрокаНомераКиЗ Из НомераКиЗ Цикл
		...
		Если СтрокаНомераКиЗ.Индивидуализирован Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.НомерКиЗ)
				...
				ТекстОшибки = НСтр("ka='სია ""საქონლები""-ს სტრიქონ %НомерСтроки%-ში არ არის მითითებული არც ""სსნ ნომერი"", არც ""RFID TID"", არც ""RFID EPC""';ru='В строке %НомерСтроки% списка ""Товары"" не указан ни ""Номер КиЗ"", ни ""RFID TID"", ни ""RFID EPC""'");
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.НомерКиЗ) Тогда
				...
				ТекстОшибки = НСтр("ka='В строке %НомерСтроки% списка ""Товары"" не указан ""Номер КиЗ""';ru='В строке %НомерСтроки% списка ""Товары"" не указан ""Номер КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.ПричинаСписания) Тогда
				...
				ТекстОшибки = НСтр("ka='В строке %НомерСтроки% списка ""Товары"" не указана ""Причина списания""';ru='В строке %НомерСтроки% списка ""Товары"" не указана ""Причина списания""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.ЗаявкаНаВыпускКиЗ) Тогда
				...
				ТекстОшибки = НСтр("ka='В строке %НомерСтроки% списка ""Товары"" не указана ""Заявка на выпуск КиЗ""';ru='В строке %НомерСтроки% списка ""Товары"" не указана ""Заявка на выпуск КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
