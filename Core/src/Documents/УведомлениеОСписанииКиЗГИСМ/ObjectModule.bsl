	...
	Для каждого СтрокаНомераКиЗ Из НомераКиЗ Цикл
		...
		Если СтрокаНомераКиЗ.Индивидуализирован Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.НомерКиЗ)
				...
				ТекстОшибки = НСтр("ka='სია ""საქონლები""-ს სტრიქონ %НомерСтроки%-ში არ არის მითითებული არც ""სსნ ნომერი"", არც ""RFID TID"", არც ""RFID EPC""';ru='В строке %НомерСтроки% списка ""Товары"" не указан ни ""Номер КиЗ"", ни ""RFID TID"", ни ""RFID EPC""';en='In line%% NomerStroki list of ""goods"" is not listed in either ""Number Keys"" or ""RFID TID"", or ""RFID EPC""'");
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.НомерКиЗ) Тогда
				...
				ТекстОшибки = НСтр("ka='В строке %НомерСтроки% списка ""Товары"" не указан ""Номер КиЗ""';ru='В строке %НомерСтроки% списка ""Товары"" не указан ""Номер КиЗ""';en='In line%% NomerStroki list of ""Goods"" Unknown ""Number Keys""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.ПричинаСписания) Тогда
				...
				ТекстОшибки = НСтр("ka='В строке %НомерСтроки% списка ""Товары"" не указана ""Причина списания""';ru='В строке %НомерСтроки% списка ""Товары"" не указана ""Причина списания""';en='In line%% NomerStroki list of ""goods"" is not specified, ""The reason write-off""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.ЗаявкаНаВыпускКиЗ) Тогда
				...
				ТекстОшибки = НСтр("ka='В строке %НомерСтроки% списка ""Товары"" не указана ""Заявка на выпуск КиЗ""';ru='В строке %НомерСтроки% списка ""Товары"" не указана ""Заявка на выпуск КиЗ""';en='In line%% NomerStroki list of ""goods"" is not specified ""Application for issue of Keys""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
