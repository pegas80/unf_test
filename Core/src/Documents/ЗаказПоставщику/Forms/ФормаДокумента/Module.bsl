...
Процедура ПриОткрытии(Отказ)
	...
	//giga
	// Привет, когда работаешь с формой, не забывай делать проверку, на то что это уже существующий документ, и в нем не нужно делать изменений
	// Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда Это условие на то что данный документ еще не записан в базе.
	// - Спасибо, учту
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Для каждого Строка из Объект.Запасы Цикл
			ЗапасыНоменклатураПриИзменении("", Строка)
		КонецЦикла;
	КонецЕсли;
	//giga
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.ВариантЗавершенияРасширеннаяПодсказка.Заголовок = СтрШаблон(НСтр("ka='წინა მდგომარეობა: %1';ru='Предыдущее состояние: %1';en='Previous state: %1'"),  //chilo translate
...
Процедура ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС(ЭтоОткрытие = Ложь)
	...
	Объект.НалогообложениеНДС = УправлениеНебольшойФирмойСервер.НалогообложениеНДС(Объект.Организация,, Объект.Дата, Объект.Контрагент, Неопределено);
...
Процедура СостояниеЗаказаПриИзменении(Элемент)
	...
	Если Объект.СостояниеЗаказа = ПредопределенноеЗначение("Справочник.СостоянияЗаказовПоставщикам.Завершен") 
		//Паганини_
		И НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВидимостьПодсистемыВнутренниеЗаказы") Тогда
		//_Паганини
...
&НаКлиенте
Процедура ЗапасыНоменклатураПриИзменении(Элемент, Строка = Неопределено)
	...
	Если ГруппыПользователя.Количество() Тогда 		
		...
		ГруппаНоменклатуры = РС_ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаТабличнойЧасти.Номенклатура, "Родитель");
...
Процедура ОбработатьИзменениеВидаЦенИВалютыРасчетов(ПараметрыДокумента)
	...
	Если ОткрытьФормуЦеныИВалюты Тогда
		...
		Если ИзменилсяВидЦен Тогда
			...
			ТекстПредупреждения = НСтр("ka='კონტრაგენტთან ხელშეკრულება ითვალისწინებს დოკუმენტში დაყენებულისგან განსხვავებულ კონტრაგენტის ფასების სახეობას! შეიძლება, საჭიროა ფასების გადავსება.';ru='Договор с контрагентом предусматривает вид цен контрагента,"
...
"Необходимо проверить валюту документа!'");
	...
	ИначеЕсли ВопросВидЦен Тогда
		...
		Если ПересчетНеобходим Тогда
			...
			ТекстВопроса = НСтр("ka='კონტრაგენტთან ხელშეკრულება ითვალისწინებს დოკუმენტში დადგენილისგან განსხვავებულ ფასების ტიპს! გადავთვალოთ დოკუმენტი ხელშეკრულების შესაბამისად?';ru='Договор с контрагентом предусматривает вид цен, отличный от установленного в документе! "
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидЦенКонтрагента) Тогда
		...
		Иначе	
			ТекстНадписи = ТекстНадписи + НСтр("ru = ' • %ВидЦенКонтрагента%'; ka = ' • %ВидЦенКонтрагента%'");
		КонецЕсли;	
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.СтраницаЗавершениеЗаказа.Видимость = Объект.СостояниеЗаказа = ПредопределенноеЗначение("Справочник.СостоянияЗаказовПоставщикам.Завершен")
												//Паганини_
												И НЕ УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВидимостьПодсистемыВнутренниеЗаказы");
												//_Паганини
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПоставщику.ЗаказНаЗакупку") Тогда
		...
		Элементы.ГруппаЗапасы.Заголовок = Нстр ("ru = 'Товары и услуги'; ka = 'საქონელი და მომსახურება'; en = 'Goods and services'");  //chilo translate
	...
	Иначе
		...
		Элементы.ГруппаЗапасы.Заголовок = Нстр ("ru = 'Товары'; ka = 'საქონელი'; en = 'Goods'"); //chilo translate
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='მარაგების ჩატვირთვა ფაილიდან';ru='Загрузка запасов из файла';en='Download the file from the reserves'")); //chilo translate
...
&НаКлиенте
Процедура УстановитьСостояниеПодтвержденGIGA(Команда)
	
	УстановитьСостояниеЗаказа(ПредопределенноеЗначение("Перечисление.СтатусыВнутреннихЗаказов.ПодтвержденGIGA"));
...
Процедура УстановитьСостояниеЗаказа(НовыйСтатус)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) ИЛИ Модифицированность Тогда
		ТекстОшибки = НСтр("ka='სტატუსის ცვლილებამდე აუცილებელია დოკუმენტის ჩაერა!';ru='Перед изменением статуса необходимо записать документ!';en='Before changing the status of the document to be written!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Объект.Ссылка);
		Возврат;
	КонецЕсли; 
	ТекстВопроса = НСтр("ru='У заказа будет установлено состояние " + НовыйСтатус + ". Продолжить?';
						|ka = 'შეკვეთისთვის დადგენილი იქნება მდგომარეობა " + НовыйСтатус + ". გაგრძელდეს?'");
	...
	СостояниеИзменено = УстановитьСостояниеДокументаНаСервере(НовыйСтатус);
	
	Если СостояниеИзменено Тогда
		ОповеститьПользователяОбУстановкеСостояния(СостояниеИзменено, "" + НовыйСтатус);
	КонецЕсли; 

КонецПроцедуры	

&НаСервере
Функция УстановитьСостояниеДокументаНаСервере(Знач СтатусЗаказа)

	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	СостояниеИзменено = Документы.ЗаказПоставщику.ИзменитьСостояниеДокумента(ДокументОбъект, СтатусЗаказа);
	Если СостояниеИзменено Тогда
...
&НаКлиенте
Процедура ОповеститьПользователяОбУстановкеСостояния(СостояниеИзменено, Состояние)
	...
	Если СостояниеИзменено Тогда
		...
		УстановитьВидимостьИДоступность();
		ТекстСообщения = НСтр("ka='შეკვეთისთვის დადგენილია მდგომარეობა ""%Состояние%""';ru='Для заказа установлено состояние ""%Состояние%""';en='To order status is set to ""% status%""'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='მდგომარეობა ""%Состояние%"" არ არის დადგენილი.';ru='Состояние ""%Состояние%"" не установлено.';en='Status ""% status%"" is not set.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Состояние%", Состояние);
...
&НаКлиенте
Процедура ЗагрузиьИзВнешнегоФайла(Команда)
	ПараметрыФормы = Новый Структура();
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище, КэшированныеЗначения)
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		 нСтрока = Объект.Запасы.Добавить();
		 ЗаполнитьЗначенияСвойств(нСтрока,СтрокаТоваров);
		 нСтрока.ЕдиницаИзмерения = нСтрока.Номенклатура.ЕдиницаИзмерения;
		 нСтрока.СтавкаНДС = нСтрока.Номенклатура.СтавкаНДС;
		 нСтрока.Сумма = нСтрока.Количество * нСтрока.Цена;
		 
		 // Сумма НДС.
		 СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(нСтрока.СтавкаНДС);

		 Если НЕ Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
			 нСтрока.СуммаНДС = 0;
		 Иначе
			 нСтрока.СуммаНДС = ?(Объект.СуммаВключаетНДС, 
			 нСтрока.Сумма - (нСтрока.Сумма) / ((СтавкаНДС + 100) / 100),
			 нСтрока.Сумма * СтавкаНДС / 100);
		 КонецЕсли;
		 
		 // Всего.
		 нСтрока.Всего = нСтрока.Сумма + ?(Объект.СуммаВключаетНДС, 0, нСтрока.СуммаНДС);
