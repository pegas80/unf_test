...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულიად შეივსება ""მყიდველის შეკვეთით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Заказу покупателя""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура УстановитьСостояниеЗаказа(НовыйСтатус)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) ИЛИ Модифицированность Тогда
		ТекстОшибки = НСтр("ka='სტატუსის ცვლილებამდე აუცილებელია დოკუმენტის ჩაერა!';ru='Перед изменением статуса необходимо записать документ!'");
...
Процедура ОповеститьПользователяОбУстановкеСостояния(СостояниеИзменено, Состояние)
	...
	Если СостояниеИзменено Тогда
		...
		УстановитьВидимостьИДоступность();
		ТекстСообщения = НСтр("ka='შეკვეთისთვის დადგენილია მდგომარეობა ""%Состояние%""';ru='Для заказа установлено состояние ""%Состояние%""'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Состояние%",              Состояние);
		ТекстЗаголовка = НСтр("ka='მდგომარეობა ""%Состояние%"" დადგენილია';ru='Состояние ""%Состояние%"" установлено'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%Состояние%", Состояние);
		ПоказатьОповещениеПользователя(ТекстЗаголовка,, ТекстСообщения, БиблиотекаКартинок.Информация32);
		
	Иначе
		
		ТекстСообщения = НСтр("ka='მდგომარეობა ""%Состояние%"" არ არის დადგენილი.';ru='Состояние ""%Состояние%"" не установлено.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Состояние%", Состояние);
		ТекстЗаголовка = НСтр("ka='მდგომარეობა ""%Состояние%"" არ არის დადგენილი';ru='Состояние ""%Состояние%"" не установлено'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%Состояние%", Состояние);
		ПоказатьОповещениеПользователя(ТекстЗаголовка,, ТекстСообщения, БиблиотекаКартинок.Информация32);
