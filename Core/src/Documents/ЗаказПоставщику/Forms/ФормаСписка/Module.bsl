...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаСписка",
		Неопределено,
		НСтр("ka='Новости: Заказы поставщикам';ru='Новости: Заказы поставщикам'"),
		Ложь,
...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='შეკვეთები მომწოდებლებისადმი';ru='Заказы поставщикам'");
...
Процедура ОповеститьПользователяОбУстановкеСостояния(СписокДокументов, КоличествоОбработанных, КоличествоВсего, Состояние)
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		ТекстСообщения = НСтр("ka=' %КоличествоВсего% გამოყოფილი სტრიქონიდან %КоличествоОбработанных% დოკუმენტისთვის დადგინდა მდგომარეობა""%Состояние%""';ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке заказов поставщикам установлено состояние ""%Состояние%""'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ka='მდგომარეობა ""%Состояние%"" არ არის დადგენილი არცერთი შეკვეთისთვის.';ru='Состояние ""%Состояние%"" не установлено ни для одного заказа поставщику.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Состояние%", Состояние);
...
Функция ПроверитьПолучитьВыделенныеВСпискеСсылки(Список) Экспорт
	...
	Если МассивСсылок.Количество() = 0 Тогда
		Предупреждение( НСтр("ka='ბრძანება არ შეიძლება შესრულდეს მითითებული ობიექტისთვის!';ru='Команда не может быть выполнена для указанного объекта!'"));
	КонецЕсли;
