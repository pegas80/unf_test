	...
	Если Материалы.Итог("Резерв") > 0 Тогда
		...
		Для каждого СтрокаМатериалы из Материалы Цикл
			...
			Если СтрокаМатериалы.Резерв > 0 И НЕ ЗначениеЗаполнено(СтруктурнаяЕдиницаРезерв) Тогда
				...
				ТекстСообщения = НСтр("ka='არ არის შევსებული რეზერვის საწყობი.';ru='Не заполнен склад резерва.'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить()
		...
		Для каждого СтрокаМатериалы Из Материалы Цикл
			...
			Если СтрокаМатериалы.Резерв > СтрокаМатериалы.Количество Тогда
				...
				ТекстСообщения = НСтр("ka='ცხრილ ""გადასამუშავებელი მასალების"" სტრიქონ №%Номер% რეზერვიდან ჩამოსაწერი პოზიციების რაოდენობა აღემატება მასალების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Материалы в переработку"" количество позиций к списанию из резерва превышает общее количество материалов.'");
	...
	Если ЕстьРучныеСкидки Тогда
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.ПроцентСкидкиНаценки <> 100 И ТекСуммаРучнойСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ka='არ არის შევსებული სვეტი ""თანხა"" სია ""საქონლები, სამუშაოები, მომსახურებები""-ს სტრიქონ %Номер%-ში.';ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Товары, работы, услуги"".'");
