...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Материалы.Итог("Резерв") > 0 Тогда
		...
		Для каждого СтрокаМатериалы из Материалы Цикл
			...
			Если СтрокаМатериалы.Резерв > 0 И НЕ ЗначениеЗаполнено(СтруктурнаяЕдиницаРезерв) Тогда
				...
				ТекстСообщения = НСтр("ka='არ არის შევსებული რეზერვის საწყობი.';ru='Не заполнен склад резерва.';en='Not filled warehouse reserve.'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить()
		...
		Для каждого СтрокаМатериалы Из Материалы Цикл
			...
			Если СтрокаМатериалы.Резерв > СтрокаМатериалы.Количество Тогда
				...
				ТекстСообщения = НСтр("ka='ცხრილ ""გადასამუშავებელი მასალების"" სტრიქონ №%Номер% რეზერვიდან ჩამოსაწერი პოზიციების რაოდენობა აღემატება მასალების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Материалы в переработку"" количество позиций к списанию из резерва превышает общее количество материалов.';en='In line №% number% Table. part of ""Materials in the processing of the"" number of items to be deducted from the reserve exceeds the total amount of materials.'");
	...
	Если ЕстьРучныеСкидки Тогда
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.ПроцентСкидкиНаценки <> 100 И ТекСуммаРучнойСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ka='არ არის შევსებული სვეტი ""თანხა"" სია ""საქონლები, სამუშაოები, მომსახურებები""-ს სტრიქონ %Номер%-ში.';ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Товары, работы, услуги"".';en='Not filled column ""Amount"" on line% number% list ""of goods, works and services.""'");
	...
	Если Не СостояниеЗаказа = Справочники.СостоянияЗаказовПоставщикам.Завершен Тогда
...
Процедура ЗаполнитьПоДокументЗапроса (ДанныеЗаполнения) Экспорт
	...
	СтруктурнаяЕдиница = ДанныеЗаполнения.ДепартаментИспольнитель;
	Комментарий = ДанныеЗаполнения.Комментарий; 
	Про = ДанныеЗаполнения.Комментарий;
...
Процедура ЗаполнитьПоВнутреннийЗаказ(ДанныеЗаполнения) Экспорт
	...
	ДатаПоступления = ДанныеЗаполнения.Дата;
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДокументСсылкаЗаказНаПроизводство) Экспорт
	...
	|		Номенклатура,
	|		Характеристика,
	|		Проект,  //Малышев 2024-06-26 ONESOFT-3279
	|		ЕдиницаИзмерения,
	|		Количество,
...
Процедура ЗаполнитьКолонкуРезервПоОстаткам() Экспорт
	...
	|						ТаблицаЗапасы.Номенклатура.СчетУчетаЗапасовРегл,
