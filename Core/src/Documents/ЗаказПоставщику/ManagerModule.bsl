...
Процедура ПроверитьВозможностьВводаНаОснованииЗаказаПоставщику(ДанныеЗаполнения, ЗначенияРеквизитов) Экспорт
	...
	Если ЗначенияРеквизитов.Свойство("СостояниеЗаказа")
		...
		ТекстОшибки = НСтр("ka='დოკუმენტი %Документ% არის სტატუსში დასრულებული. დასრულებული შეკვეთის საფუძველზე შექმნა აკრძალულია!';ru='Документ %Документ% в состоянии Завершен. Ввод на основании завершенного заказа запрещен.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаПоступления",  НСтр("ka='თარიღი';ru='Дата Поступления'"), ОписаниеТиповСтрока25, ОписаниеТиповДата, , , , , ПолеВидимо);
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета = "")
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = НСТР("ru = 'Заказ поставщику № '; ka = 'შეკვეთა მომწოდებელს № '")
												+ НомерДокумента
												+ НСТР("ru = ' от '; ka = '    '")
												+ Формат(Шапка.ДатаДокумента, НСТР("ka=""Л=ka; ДФ='dd MMMM yyyy წ.'"";ru='ДЛФ=DD'"));
		...
		Иначе
			ОбластьМакета.Параметры.НДС = ?(Шапка.СуммаВключаетНДС, НСТР("ka='მათ შორის დღგ:';ru='В том числе НДС:'"), НСТР("ka='დღგ-ს თანხა:';ru='Сумма НДС:'"));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='შეკვეთა მომწოდებელს';ru='Заказ поставщику'");
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВидимостьПодсистемыВнутренниеЗаказы") Тогда
		...
		КомандаПечати.Представление = НСтр("ka='შიდა შეკვეთა';ru='Внутренный заказ'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ГруппаДел	= НСтр("ka='შეკვეთები მომწოდებლებისადმი';ru='Заказы поставщикам'");
	...
	Дело.Важное			= Истина;
	Дело.Представление	= НСтр("ka='გადავადებულია შესრულება';ru='Просрочено выполнение'");
	...
	Дело.Важное			= Ложь;
	Дело.Представление	= НСтр("ka='დღეისთვის';ru='На сегодня'");
...
Функция УстановитьСостояниеДокументов(Знач МассивДокументов, Знач СтатусЗаказа) Экспорт
	...
	ШаблонОшибкиСостояниеСовпадает	= НСтр("ka='დოკუმენტს %Документ% უკვე მინიჭებული აქვს მდგომარეობა ""%Состояние%""';ru='Документу %Документ% уже присвоено состояние ""%Состояние%""'");
	ШаблонОшибкиНеПроведен        	= НСтр("ka='დოკუმენტი %Документ% არ არის გატარებული. შეუძლებელია მდგომარეობის შეცვლა';ru='Документ %Документ% не проведен. Невозможно изменить состояние'");
	ШаблонОшибкиПомеченНаУдаление 	= НСтр("ka='დოკუმენტი %Документ% მონიშნულია წაშლაზე. შეუძლებელია მდგომარეობის შეცვლა';ru='Документ %Документ% помечен на удаление. Невозможно изменить состояние'");
	ШаблонОшибкиПредыдущееСостояние	= НСтр("ka='დოკუმენტი %Документ%: შეუძლებელია გადავიდეს მდგომარეობაში ""%Состояние%"" მდგომარეობიდან ""%ПредыдущееСостояние%""';ru='Документ %Документ%: невозможно перейти в состояние ""%Состояние%"" из состояния ""%ПредыдущееСостояние%""'");
	ШаблонОшибкиЗаблокировать     	= НСтр("ka='ვერ მოხედხდა დაბლოკვა %Документ%. %ОписаниеОшибки%';ru='Не удалось заблокировать %Документ%. %ОписаниеОшибки%'");
	ШаблонОшибкиЗаписать          	= НСтр("ka='ვერ მოხერხდა დოკუმენტის ჩაწერა %Документ%. %ОписаниеОшибки%';ru='Не удалось записать %Документ%. %ОписаниеОшибки%'");
...
Функция ИзменитьСостояниеДокумента(ДокументОбъект, Знач СтатусЗаказа) Экспорт
	...
	ШаблонОшибкиНеПроведен        	= НСтр("ka='დოკუმენტი %Документ% არ არის გატარებული. შეუძლებელია მდგომარეობის შეცვლა';ru='Документ %Документ% не проведен. Невозможно изменить состояние'");
	ШаблонОшибкиПомеченНаУдаление 	= НСтр("ka='დოკუმენტი %Документ% მონიშნულია წაშლაზე. შეუძლებელია მდგომარეობის შეცვლა';ru='Документ %Документ% помечен на удаление. Невозможно изменить состояние'");
	ШаблонОшибкиПредыдущееСостояние	= НСтр("ka='დოკუმენტი %Документ%: შეუძლებელია მდგომარეობიდან ""%ПредыдущееСостояние%"" მდგომარეობაში ""%Состояние%"" გადაყვანა';ru='Документ %Документ%: невозможно перейти в состояние ""%Состояние%"" из состояния ""%ПредыдущееСостояние%""'");
	ШаблонОшибкиЗаблокировать     	= НСтр("ka='ვერ მოხერხდა დაბლოკვა %Документ%. %ОписаниеОшибки%';ru='Не удалось заблокировать %Документ%. %ОписаниеОшибки%'");
	ШаблонОшибкиЗаписать          	= НСтр("ka='ვერ მოხერხდა ჩაწერა %Документ%. %ОписаниеОшибки%';ru='Не удалось записать %Документ%. %ОписаниеОшибки%'");
