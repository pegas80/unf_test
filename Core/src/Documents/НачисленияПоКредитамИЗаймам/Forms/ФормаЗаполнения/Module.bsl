...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоЗаймамСотрудникам Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитамВыданным Тогда
...
Функция УстановитьПараметрыВыбораДоговора()
	...
	Если ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитам Тогда
		...
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитамВыданным Тогда
		МассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.ВидДоговора", Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный));
		Если НЕ Контрагент.Пустая() Тогда
			МассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.Контрагент", Контрагент));
		КонецЕсли;
	Иначе
...
Функция ТекстЗапросаПоНачислениям()
	...
	|	СУММА(ВЫБОР
	|			КОГДА ГрафикПогашенияКредитовИЗаймов.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.КредитВыданный)
	|					И ГрафикПогашенияКредитовИЗаймов.ДоговорКредитаЗайма.НачислитьПроценты
	|				ТОГДА 0
	|			ИНАЧЕ ГрафикПогашенияКредитовИЗаймов.СуммаПроцентов
	|		КОНЕЦ) КАК СуммаПроцентов,
	|	СУММА(ГрафикПогашенияКредитовИЗаймов.СуммаКомиссии) КАК СуммаКомиссии,
	|	ВЫБОР
	|		КОГДА ГрафикПогашенияКредитовИЗаймов.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику)
	|			ТОГДА ГрафикПогашенияКредитовИЗаймов.ДоговорКредитаЗайма.Сотрудник
	|		ИНАЧЕ ГрафикПогашенияКредитовИЗаймов.ДоговорКредитаЗайма.Контрагент
	...
	|	ГрафикПогашенияКредитовИЗаймов.Проект КАК Проект,
	...
	|	ГрафикПогашенияКредитовИЗаймов.Проект,
	...
	|	РасчетыПоКредитамИЗаймам.Проект КАК Проект,
	...
	|{ГДЕ
	|	(ВЫРАЗИТЬ(РасчетыПоКредитамИЗаймам.Контрагент КАК Справочник.Контрагенты)).* КАК Контрагент,
	|	(ВЫРАЗИТЬ(РасчетыПоКредитамИЗаймам.Контрагент КАК Справочник.Сотрудники)).* КАК Сотрудник,
	...
	|	РасчетыПоКредитамИЗаймам.Проект,
	...
	|			И ТекущиеНачисления.Проект = ПредыдущиеНачисления.Проект
...
Процедура НачисленияСервер()
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНачисленияПоКредитамИЗаймам.НачисленияПоКредитамВыданным Тогда
		ПостроительЗапроса.Параметры.Вставить("ВидДоговора", Перечисления.ВидыДоговоровКредитаИЗайма.КредитВыданный);
	Иначе
