	...
	Если НомераКиЗ.Количество() > 0 Тогда
		...
		Пока ВыборкаНомераКиЗ.Следующий() Цикл
			...
			Если Не ТекущийДокументПоступленияПравильный Тогда
				...
				ТекстОшибки = НСтр("ka='სია ""სსნ ნომრები""-ს სტრიქონ %НомерСтроки%-ში მითითებულია შემოსვლის არაკორექტული დოკუმენტი';ru='Указан некорректный документ поступления в строке %НомерСтроки% списка ""Номера КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ВызватьИсключение НСтр("ka='კოპირებას მხარდაჭერა არ გააჩნია.';ru='Копирование не поддерживается.'")
