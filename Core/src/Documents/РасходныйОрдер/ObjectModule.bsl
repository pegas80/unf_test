	...
	Если ЗначенияРеквизитов.Свойство("ВидОперации") Тогда
		Если ЗначениеЗаполнено(ЗначенияРеквизитов.ВидОперации)
			И НЕ ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
				ТекстИсключения = НСтр("ka='გასავლის ორდერის შეყვანა შეიძლება მხოლოდ სერვისის დოკუმენტის საფუძველზე!';ru='Расходный ордер можно ввести только на основании заказ-наряда!';en='Withdrawal slip, you can enter only on the basis of the purchase order!'");
...
"Документ ""%ДокументСсылка"" не имеет ордерного склада!';ka='შეუძლებელია ოპერაცია ""საორდერო საწყობიდან გასავალის"" შეყვანა. "
"დოკუმენტ ""%ДокументСсылка""-ს არ გააჩნია საორდერო საწყობი!'");
