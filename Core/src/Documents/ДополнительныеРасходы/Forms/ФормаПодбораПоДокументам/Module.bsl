...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Для каждого ДокументПоступления Из МассивВыбранныхДокументов Цикл
		...
		Если МассивНайденныхДокументов.Количество() > 0 Тогда
			...
			ТекстСообщения = НСтр("ka='დოკუმენტი %ПредставлениеДокумента% უკვე არის არჩეული დოკუმენტების სიაში.';ru='Документ %ПредставлениеДокумента% уже присутствует в списке выбранных документов.'");
...
Процедура ЗаполнитьПоОтобраннымДокументам(Команда)
	...
	Если ОтобранныеЗапасы.Количество() > 0 Тогда
		...
		ТекстВопроса = Нстр("ka='ცხრილი გასუფთავდება და ხელმეორედ შეივსება. გავაგრძელოთ?';ru='Табличная часть будит очищена и повторно заполнена. Продолжить?'");
...
Процедура ЗаполнитьПоТекущемуДокументу(Команда)
	...
	Если ТекущаяСтрокаДокументовПоступлений = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Нстр("ka='გამოჰყავით სტრიქონი დოკუმენტიანად და გაიმეორეთ შევსების მცდელობა';ru='Выделите строку с документом и повторите попытку заполнить'"),
			,
			"ОтобранныеДокументы",
			,
			);
