...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Для каждого ДокументПоступления Из МассивВыбранныхДокументов Цикл
		...
		Если МассивНайденныхДокументов.Количество() > 0 Тогда
			...
			ТекстСообщения = НСтр("ka='დოკუმენტი %ПредставлениеДокумента% უკვე არის არჩეული დოკუმენტების სიაში.';ru='Документ %ПредставлениеДокумента% уже присутствует в списке выбранных документов.';en='Document% PredstavlenieDokumenta% is already present in the list of selected documents.'");
...
Процедура ЗаполнитьПоОтобраннымДокументам(Команда)
	...
	Если ОтобранныеЗапасы.Количество() > 0 Тогда
		...
		ТекстВопроса = Нстр("ka='ცხრილი გასუფთავდება და ხელმეორედ შეივსება. გავაგრძელოთ?';ru='Табличная часть будит очищена и повторно заполнена. Продолжить?';en='Tabular part wakes cleaned and re-filled. Proceed?'");
...
Процедура ЗаполнитьПоТекущемуДокументу(Команда)
	...
	Если ТекущаяСтрокаДокументовПоступлений = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Нстр("ka='გამოჰყავით სტრიქონი დოკუმენტიანად და გაიმეორეთ შევსების მცდელობა';ru='Выделите строку с документом и повторите попытку заполнить';en='Highlight the line with the document, and then try again to fill'"),
			,
			"ОтобранныеДокументы",
			,
			);
