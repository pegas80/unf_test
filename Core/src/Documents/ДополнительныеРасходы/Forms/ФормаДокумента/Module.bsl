...
Процедура ЗаказПоставщикуПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И КонтрагентВестиРасчетыПоЗаказам Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПоставщикуПриИзмененииЗавершение", ЭтотОбъект), НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?'"), Режим, 0);
		Возврат;
	КонецЕсли;
...
Процедура РаспределитьРасходыПоКоличеству(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ka='ცხრილ ""მარაგები""-ში ჩანაწერები არ არის!';ru='В табличной части ""Запасы"" нет записей!'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Расходы.Количество() = 0 И Объект.ДопРасходыИмпорт.Количество() = 0 Тогда
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ka='ცხრილ ""ხარჯები""-ში ჩანაწერები არ არის!';ru='В табличной части ""Расходы"" нет записей!'"));
		Возврат;
	КонецЕсли;
