...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого СтрокаВнеоборотныхАктивов Из ВнеоборотныеАктивы Цикл
		...
		Если СтрокаВнеоборотныхАктивов.ВнеоборотныйАктив.СпособАмортизации <> Перечисления.СпособыНачисленияАмортизацииВнеоборотныхАктивов.ПропорциональноОбъемуПродукции Тогда
			ТекстСообщения = НСтр("ka='სიის ""ქონება"" სტრიქონ %НомерСтроки% მითითებულ ქონება %ВнеоборотныйАктив% გამოიყენება ამორტიზაციის დარიცხვის მეთოდი, რომელიც განსხვავებულია ""პროდუქციის (სამუშაოების) მოცულობის პროპორციულისგან"".';ru='Для имущества %ВнеоборотныйАктив% указанного в строке %НомерСтроки% списка ""Имущество"", используется способ начисления амортизации отличный от ""Пропорционально объему продукции (работ)"".';en='For property% VneoborotnyyAktiv% of the line%% NomerStroki list ""Property"", a method of calculating depreciation than ""proportional to the volume of production (work).""'");
