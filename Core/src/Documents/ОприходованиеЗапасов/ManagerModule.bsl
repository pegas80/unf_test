...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОприходованиеЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	|		КОГДА ОприходованиеЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|				ИЛИ ОприходованиеЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница)
	|			ТОГДА ОприходованиеЗапасовЗапасы.Номенклатура.СчетУчетаЗапасовРегл
	|		ИНАЧЕ ОприходованиеЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл
	|	КОНЕЦ КАК СчетУчетаРегл,
	|	ОприходованиеЗапасовЗапасы.Номенклатура КАК Номенклатура,
	|	ОприходованиеЗапасовЗапасы.Номенклатура.СчетУчетаЗатратРегл КАК СчетУчетаЗатратРегл, //Малышев 2024-06-06 ONESOFT-3093
	|	ВЫБОР
	...
	//|			ТОГДА ВЫБОР
	//|					КОГДА ОприходованиеЗапасовЗапасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	//|						ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
	//|					ИНАЧЕ ОприходованиеЗапасовЗапасы.Проект
	//|				КОНЕЦ
	...
	|		КОГДА ОприходованиеЗапасовЗапасы.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА ОприходованиеЗапасовЗапасы.Проект
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта) //++Lazare_05.02.2021
	|	КОНЕЦ КАК Проект,
   	//Малышев 2024-06-10 ONESOFT-3093 --)	
	
	//Малышев 2024-12-13 ONESOFT-3632 (--
	|	ВЫБОР 
	|		КОГДА ОприходованиеЗапасовЗапасы.Номенклатура.МетодОценки = Значение(Перечисление.МетодОценкиЗапасов.FIFO) 
	|			ТОГДА &Ссылка
	|		ИНАЧЕ Неопределено
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ОприходованиеЗапасовЗапасы.Количество * ОприходованиеЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент - &СмещениеДляОкругления КАК ЧИСЛО(15, 3))
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Партия,
	//Малышев 2024-06-23 ONESOFT-3267 (--
	|	ВЫБОР
	|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)    
	|		КОГДА ОприходованиеЗапасовЗапасы.Проект <> Значение(Справочник.Проекты.ПустаяСсылка)
	|			ТОГДА ОприходованиеЗапасовЗапасы.Проект
	...
	|	ОприходованиеЗапасовЗапасы.Ссылка.Подразделение КАК СтруктурнаяЕдиница,
	// Олег Литвиненко <---
	...
	|	ОприходованиеЗапасовЗапасы.Ссылка.КорреспонденцияРегл КАК СчетУчетаРегл,
	...
	|					ТОГДА ОприходованиеЗапасовЗапасы.Ссылка.КорреспонденцияРегл.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеДоходы)
	|				ИНАЧЕ ОприходованиеЗапасовЗапасы.Ссылка.Корреспонденция.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеДоходы)
	|			КОНЕЦ
	|			ИЛИ ОприходованиеЗапасовЗапасы.Ссылка.КорреспонденцияРегл В (&СчетаФинРезДоходы))
	...
	|	ОприходованиеЗапасовЗапасы.Ссылка.КорреспонденцияРегл,
	...
	|	ОприходованиеЗапасовЗапасы.СрокГодности КАК СрокГодности,
	|	&Организация КАК Организация,
	|	ОприходованиеЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	...
	|	Документ.ОприходованиеЗапасов.Запасы КАК ОприходованиеЗапасовЗапасы
	|ГДЕ
	|	&ИспользоватьКонтрольСроковГодностиТовара
	|	И ОприходованиеЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад = ЛОЖЬ
	|	И ОприходованиеЗапасовЗапасы.Ссылка = &Ссылка
	|	И ОприходованиеЗапасовЗапасы.СрокГодности <> ДАТАВРЕМЯ(1, 1, 1)
	|	И ОприходованиеЗапасовЗапасы.Номенклатура.ВключенВрегистр = ИСТИНА");
	...
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", НСтр("en='Posting of Goods';ka='მარაგების შემოსავალში აყვანა';ru='Оприходование запасов'"));
	...
	Запрос.УстановитьПараметр("ПрочееОприходование", НСтр("ka='მარაგების სხვაგვარი შემოსავალში გატარება';ru='Прочее оприходование запасов';en='Other postings stocks'"));
	...
	Запрос.УстановитьПараметр("ИспользоватьКонтрольСроковГодностиТовара", СтруктураДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара);//+++ Lazare
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКонтрольСроковГодностиТовара", МассивРезультатов[7].Выгрузить());
...
Функция СформироватьАктОприходованияЗапасов(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = Нстр("ru = 'Акт об оприходовании запасов № '; ka = 'მარაგების შემოსავალზე აყვანის აქტი № '")
			+ НомерДокумента                       
			+ Нстр("ru = ' от '; ka = ' დან '")
			+ Формат(Шапка.ДатаДокумента, "Л="+Нстр("ru = 'ru_RU'; ka = 'ka_GE'")+"; ДЛФ=DD");
		...
		ОбластьМакета.Параметры.ВремяПечати =
			Нстр("ru = 'Дата и время печати: '; ka = 'ბეჭდვის თარიღი და დრო: '")
			+ ТекущаяДата()
			+ Нстр("ru = '. Пользователь: '; ka = '. მომხმარებელი: '")
		...
		ТекстИтоговойСтроки = НСтр("ka='სულ, %СуммаВсего% %ВалютаУпр%-ის ოდენობის %ВсегоНаименований% დასახელება';ru='Всего наименований %ВсегоНаименований%, на сумму %СуммаВсего% %ВалютаУпр%';en='Total items% VsegoNaimenovany%, amounting to SummaVsego%%%% ValyutaUpr'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("en='Act on Posting of Inventory';ka='მარაგების შესავალში აყვანის აქტი';ru='Акт об оприходовании запасов'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("en='Act on Posting of Inventory';ka='მარაგების შესავალში აყვანის აქტი';ru='Акт об оприходовании запасов'");
