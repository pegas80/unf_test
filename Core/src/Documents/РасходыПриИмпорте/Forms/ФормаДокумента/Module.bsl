...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ТаможенныйШтраф <> 0
		...
		ТекстОшибки = НСтр("ka='შეამოწმეთ საქმიანობის მიმართულების შევსება და დანახარჯების ანგარიში';ru='Проверьте заполнение направления деятельности и счета затрат'");
...
Процедура ЗапасыВставитьСтроки(Команда)
	...
	Если СтрокаРаздела = Неопределено Тогда
		...
		ТекстОшибки = НСтр("ka='აუცილებელია იმ ნაწილის გამოყოფა, რომლისთვის ხორციელდება მარაგის დამატება';ru='Необходимо выделить раздел, для которого добавляются запасы'");
...
Процедура РазделыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НЕ ДанныеТекущейСтроки = Неопределено Тогда
		...
		Если Объект.Разделы.Количество() > 1 Тогда
			...
			Элементы.ЗапасыИзменениеСтрокДействие.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьИсходноеМесто"), НСтр("ka='სტრიქონების გადატანა სხვა ნაწილში';ru='Перенести строки в другой раздел'"));
...
Процедура ЗаполнитьСведенияОРазделе(ИдентификаторСтроки)
	...
	Если ИдентификаторСтроки <> Неопределено Тогда
		...
		СтрокаСведений = НСтр("ka='ამ ნაწილში საქონლის სტრიქონები: %1';ru='В этом разделе строк товаров: %1'");
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	ТекстНадписи = ТекстНадписи 
		...
		+ ?(СтруктураНадписи.СуммаВключаетНДС, НСтр("ka='თანხა შეიცავს დღგ-ს';ru='Сумма включает НДС'"), НСтр("ka='თანხა არ შეიცავს დღგ-ს';ru='Сумма не включает НДС'"));
...
Процедура ОбработатьТаблицу()
	...
	Если СтрокаРаздела = Неопределено Тогда
		...
		ТекстОшибки = НСтр("ka='მიუთითეთ სატვირთო-საბაჟო დეკლარაციის ნაწილი, რომელშიც საჭიროა საქონლების დამატება';ru='Укажите раздел ГТД, в который необходимо добавить товары'");
...
Процедура ЗаполнитьСписокДействий()
	...
	Если ОбменСБухгалтериейНастроен Тогда
		...
		СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ПодобратьДокументыПартий,		НСтр("ka='დოკუმენტების გადარჩევა პარტიები';ru='Подобрать документы партий'"));
		
	КонецЕсли;
	
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента,			НСтр("ka='სტრიქონების დამატება დოკუმენტიდან';ru='Добавить строки из документа'"));
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ПодобратьНомераГТД,				НСтр("ka='სატვირთო საბაჟო დეკლარაციის ნომრის მიხედვით სტრიქონების დამატება შემოსვლის ზედნადებებიდან';ru='Добавить строки по номеру ГТД из приходных накладных'"));
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам,		НСтр("ka='საბაჟო მოსაკრებლის განაწილების თანხების დაზუსტება';ru='Уточнить суммы распределения таможенного сбора'"));
	СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.УдалитьСтроки, 					НСтр("ka='სტრიქონების წაშლა';ru='Удалить строки'"));
	
	Если Объект.Разделы.Количество() > 1 Тогда
		
		СписокВыбораДействий.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.УстановитьИсходноеМесто,	НСтр("ka='სტრიქონების გადატანა სხვა ნაწილში';ru='Перенести строки в другой раздел'"));
...
Процедура ДобавитьСтрокиВРазделИзДокумента(ИдентификаторРаздела, Ошибки)
	...
	Если НЕ ЗначениеЗаполнено(ЗапасыИзменениеСтрокЗначение) Тогда
		...
		ТекстОшибки = НСтр("ka='შესავსებად სავალდებულოა დოკუმენტის არჩევა';ru='Для заполнения необходимо выбрать документ'");
...
Процедура ДобавитьСтрокиВРазделИзДокументовПриходнаяНакладная(ИдентификаторРаздела, Ошибки)
	...
	Если НЕ ЗначениеЗаполнено(Объект.НомерГТД) Тогда
		...
		ТекстОшибки = НСтр("ka='მიუთითეთ სატვირთო-საბაჟო დეკლარაციის ნომერი';ru='Укажите номер ГТД'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Разделы", ТекстОшибки, "");
