...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Заказы.Количество()=0 Тогда
		...
		НСтр("ka='Укажите отгружаемые заказы';ru='Укажите отгружаемые заказы';en='Specify shipped orders'"),
		Ссылка,
		"Заказы",,
		Отказ);
	КонецЕсли; 
...
Процедура ОбновитьДанныеЗаказов(Отказ, Проведение = Истина)
	...
	Если НЕ Ошибки.Количество()=0 Тогда
		Пока Ошибки.Следующий() Цикл
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='%1 уже отгружен документом %2';ru='%1 уже отгружен документом %2';en='% 1% already shipped document 2'"), Ошибки.Заказ, Ошибки.МаршрутныйЛист),
			...
			Если НЕ ЗначениеЗаполнено(Выборка.НовоеСостояние) Тогда
				...
				Если НесколькоВидовЗаказов Тогда
					Если ВыведенныеВидыЗаказов.Найти(ЗаказОбъект.ВидЗаказа)=Неопределено Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(НСтр("ka='Не задано состояние %1 заказа для вида %2';ru='Не задано состояние %1 заказа для вида %2';en='Not defined state order for% 1% 2 species'"),
						?(Проведение, НСтр("ka='отгрузки';ru='отгрузки';en='shipment'"), НСтр("ka='ожидания отгрузки';ru='ожидания отгрузки';en='shipment expectations'")),
				...
				Иначе
					Если ВыведенныеВидыЗаказов.Найти(Справочники.ВидыЗаказовПокупателей.Основной)=Неопределено Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(НСтр("ka='Не задано состояние %1 заказа';ru='Не задано состояние %1 заказа';en='Do not set the state 1% of the order'"),
						?(Проведение, НСтр("ka='отгрузки';ru='отгрузки';en='shipment'"), НСтр("ka='ожидания отгрузки';ru='ожидания отгрузки';en='shipment expectations'"))),
