...
Процедура ПриОткрытииЗадержка()
	...
	Если ФормаПараметры.НесколькоВидовЗаказов Тогда
		ПодсказкаНастройки = НСтр("ka='Укажите фиксированные состояния для разных видов заказов. Данные требуются для корректной работы механизма доставки';ru='Укажите фиксированные состояния для разных видов заказов. Данные требуются для корректной работы механизма доставки';en='Specify a fixed state for different types of orders. Data is required for correct operation of the delivery mechanism'");
	Иначе
		ПодсказкаНастройки = НСтр("ka='Укажите фиксированные состояния заказов. Данные требуются для корректной работы механизма доставки';ru='Укажите фиксированные состояния заказов. Данные требуются для корректной работы механизма доставки';en='Specify fixed state orders. Data is required for correct operation of the delivery mechanism'");
	КонецЕсли; 
	ОткрытьФормуНастройкиСостояний(
	НСтр("ka='Состояния доставки заказов';ru='Состояния доставки заказов';en='delivery status of orders'"),
	ПодсказкаНастройки);
...
Процедура СостояниеЗаказовРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФормуНастройкиСостояний(
	НСтр("ka='Состояния доступных к отгрузке заказов';ru='Состояния доступных к отгрузке заказов';en='State available to ship orders'"),
	НСтр("ka='Укажите состояние заказов, которые будут заполняться в маршрутный лист';ru='Укажите состояние заказов, которые будут заполняться в маршрутный лист';en='Specify the status of orders, which will be filled in the itinerary'"),
...
Процедура НовоеСостояниеЗаказовРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФормуНастройкиСостояний(
	НСтр("ka='Состояния отгруженных заказов';ru='Состояния отгруженных заказов';en='shipped order status'"),
	НСтр("ka='Укажите состояние, в которое будут переведены заказы покупателя после проведения маршрутного листа';ru='Укажите состояние, в которое будут переведены заказы покупателя после проведения маршрутного листа';en='Specify the state that will be transferred to the buyer orders after the route sheet'"),
	"СостояниеОтгружен");
...
Процедура СформироватьДокументыОтгрузки(Команда)
	...
	ПараметрыСовета.Вставить("Заголовок", НСтр("ka='Формирование документов отгрузки';ru='Формирование документов отгрузки';en='Formation of the shipping documents'"));
	ПараметрыСовета.Вставить("ТекстСообщения", НСтр("ka='Сейчас будут сформированы расходные накладные по всем заказам из маршрутного листа (если они еще не были созданы). Отгрузка будет произведена со склада, указанного в заказе.';ru='Сейчас будут сформированы расходные накладные по всем заказам из маршрутного листа (если они еще не были созданы). Отгрузка будет произведена со склада, указанного в заказе.';en='Who will be formed expendable overhead on all orders from the route sheet (if they have not yet been established). Shipment will be made from stock, specified in the order.'"));
	ПараметрыСовета.Вставить("ВидимостьБольшеНеПоказывать", Истина);
	ПараметрыСовета.Вставить("ЗаголовокКнопкиОк", НСтр("ka='გაგრძელება';ru='Продолжить';en='Proceed'"));
...
Процедура НастроитьНовоеСостояниеЗаказов(Команда)
	...
	ОткрытьФормуНастройкиСостояний(
	НСтр("ka='Состояния отгруженных заказов';ru='Состояния отгруженных заказов';en='shipped order status'"),
	НСтр("ka='Укажите состояния, в которые будут переведены заказы покупателя после проведения маршрутного листа';ru='Укажите состояния, в которые будут переведены заказы покупателя после проведения маршрутного листа';en='Specify the states to which will be transferred to the buyer orders after the route sheet'"),
	"СостояниеОтгружен");
...
Процедура НастроитьСостояниеЗаказов(Команда)
	...
	ОткрытьФормуНастройкиСостояний(
	НСтр("ka='Состояния доступных к отгрузке заказов';ru='Состояния доступных к отгрузке заказов';en='State available to ship orders'"),
	НСтр("ka='Укажите состояния для разных видов заказов, которые будут заполняться в маршрутный лист';ru='Укажите состояния для разных видов заказов, которые будут заполняться в маршрутный лист';en='Specify the status for the different types of orders that will be filled in the itinerary'"),
	"СостояниеОжидаетОтгрузки");
	...
	Если НЕ ФормаПараметры.НесколькоВидовЗаказов Тогда
		...
		Если ЗначениеЗаполнено(СостояниеЗаказов) Тогда
			Элементы.СостояниеЗаказовРасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(
				...
				Новый ФорматированнаяСтрока(НСтр("ka='აქ';ru='здесь';en='here'"),,,,"Переход"));
		Иначе
...
Процедура ДополнитьПредставления(ДокументыДляПолученияПредставлений)
	...
	Пока Выборка.Следующий() Цикл
		...
		НСтр("ka='%2-ის #%1';ru='№%1 от %2';en='№% 1% 2 from'"),
...
Процедура ОбновитьЗаголовокФильтров()
	...
	КонецЕсли;
	Если ФормаПараметры.НесколькоВидовЗаказов Тогда
		ЗаголовокГруппы = ЗаголовокГруппы+?(ПустаяСтрока(ЗаголовокГруппы), "", ", ")+НСтр("ka='только готовые к отгрузке';ru='только готовые к отгрузке';en='just ready for shipment'");
	КонецЕсли; 
	
	Если ПустаяСтрока(ЗаголовокГруппы) Тогда
		Элементы.ГруппаФильтры.ЗаголовокСвернутогоОтображения = НСтр("ka='ფილტრები';ru='Фильтры';en='filters'");
	Иначе
...
Функция СостояниеОжидаетОтгрузки(Заказ, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(СостояниеЗаказа) Тогда
		Если ФормаПараметры.НесколькоВидовЗаказов Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Не задано состояние готовых к отгрузке заказов для вида %1';ru='Не задано состояние готовых к отгрузке заказов для вида %1';en='Do not set the state ready for shipment orders for the type of% 1'"),
			ВидЗаказа),
			ВидЗаказа,
			"СостояниеОжидаетОтгрузки",,
			Отказ);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Не задано состояние готовых к отгрузке заказов';ru='Не задано состояние готовых к отгрузке заказов';en='the state is not set ready for shipment orders'"),
			ВидЗаказа,
			"СостояниеОжидаетОтгрузки",,
			Отказ);
		КонецЕсли;
