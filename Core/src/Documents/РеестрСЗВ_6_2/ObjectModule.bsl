...
Процедура ПроверкаЗаполненияДокумента(ПроверяемыеРеквизиты, НеПроверяемыеРеквизиты, Отказ)Экспорт 
	...
	Если ЗаписиОСтаже.Количество() > 200 Тогда
		ТекстОшибки = Нстр("ka='დოკუმენტში 200 თანამშრომელზე მეტი არ უნდა იყოს!';ru='В документе должно быть не более 200 сотрудников !'");
	...
	Пока ВыборкаЗаписиОСтаже.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаЗаписиОСтаже.Сотрудник) Тогда 
			Если ВыборкаЗаписиОСтаже.КонфликтующаяСтрока <> Null Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინფორმაცია თანამშრომელ %2-ის შესახებ დოკუმენტში უწინ იქნა შეყვანილი!';ru='Bнформация о сотруднике %2 была введена в документе ранее!'"), ВыборкаЗаписиОСтаже.НомерСтрокиСотрудник, ВыборкаЗаписиОСтаже.СотрудникНаименование);
				...
				Пока ВыборкаЗаписиОСтаже.СледующийПоЗначениюПоля("НомерСтрокиСтаж") Цикл
					...
					Если ЗначениеЗаполнено(ВыборкаЗаписиОСтаже.ДатаНачалаПериода) И ЗначениеЗаполнено(ВыборкаЗаписиОСтаже.ДатаОкончанияПериода) Тогда
						...
						Если ВыборкаЗаписиОСтаже.ДатаНачалаПериода < ?(ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ, ОтчетныйПериод, КорректируемыйПериод) Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თანამშრომელი %2 პერიოდის დაწყების თარიღი პერიოდის დაწყებაზე ადრე არ უნდა იყოს!';ru='Сотрудника %2: дата начала периода должна быть не ранее начала периода!'"), НомерСтроки, ВыборкаЗаписиОСтаже.СотрудникНаименование);     
						...
						Если ВыборкаЗаписиОСтаже.ДатаОкончанияПериода > ОкончаниеПериода Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თანამშრომელი %2: პერიოდის დასრულების თარიღი პერიოდის დასრულებაზე გვიანი არ უნდა იყოს!';ru='Сотрудник %2: дата окончания периода должна быть не позднее окончания периода!'"), НомерСтроки, ВыборкаЗаписиОСтаже.СотрудникНаименование);     
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаЗаписиОСтаже.НомерСтрокиСотрудник - 1, "ЧН=0; ЧГ=0") + "].ПериодыСтажаСтрока",,Отказ);
						КонецЕсли;	 
						...
						Если ВыборкаЗаписиОСтаже.ДатаНачалаПериода > ВыборкаЗаписиОСтаже.ДатаОкончанияПериода Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თანამშრომელ %2-ს: პერიოდის დაწყების თარიღი პერიოდის დასრულების თარიღს არ უნდა აღემატებოდეს!';ru='Сотрудника %2: дата начала периода не должна превышать дату окончания периода!'"), НомерСтроки, ВыборкаЗаписиОСтаже.СотрудникНаименование);     
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаЗаписиОСтаже.НомерСтрокиСотрудник - 1, "ЧН=0; ЧГ=0") + "].ПериодыСтажаСтрока",,Отказ);
						КонецЕсли;
