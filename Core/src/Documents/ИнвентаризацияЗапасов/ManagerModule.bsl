	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Пока ВыборкаКомиссия.Следующий() Цикл
			...
			Если ВыборкаКомиссия.ЭтоПодписьПредседателяКомиссии Тогда
				...
				ЗаголовокПодписи = НСтр("ka='კომისიის თავმჯდომარე:';ru='Председатель комиссии:'");
				
			ИначеЕсли ЭтоПервыйЧленКомиссии Тогда
				
				ЗаголовокПодписи = НСтр("ka='კომისიის წევრები:';ru='Члены комиссии:'");
...
Процедура СформироватьИнвентаризациюЗапасов(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ШаблонЗаголовка = Нстр("ka='Инвентаризация запасов № %1 от %2';ru='Инвентаризация запасов № %1 от %2'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='ИНВ-3 (საქონლის საინვენტარიზაციო აღწერა)';ru='ИНВ-3 (Инвентаризационная опись товаров)'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ИнвентаризацияЗапасов_ИНВ3";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ИнвентаризацияЗапасов.ПФ_MXL_ИНВ3";
		ПечатнаяФорма.СинонимМакета = НСтр("ka='ИНВ-3 (საინვენტარიზაციო აღწერა ცარიელი ფაქტობრივი მონაცემებით)';ru='ИНВ-3 (Инвентаризационная опись с пустыми фактическими данными)'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='ИНВ-19 (შეჯერებითი უწყისი)';ru='ИНВ-19 (Сличительная ведомость)'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ИнвентаризацияЗапасов_ИНВ19";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ИнвентаризацияЗапасов.ПФ_MXL_ИНВ19";
		ПечатнаяФорма.СинонимМакета = НСтр("ka='ИНВ-19 (Сличительная ведомость, суммы по данным учета)';ru='ИНВ-19 (Сличительная ведомость, суммы по данным учета)'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='მარაგების ინვენტარიზაცია';ru='Инвентаризация запасов'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='მარაგების ინვენტარიზაცია';ru='Инвентаризация запасов'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 4;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ИНВ3";
	КомандаПечати.Представление = НСтр("ka='ИНВ-3 (საქონლის საინვენტარიზაციო აღწერა)';ru='ИНВ-3 (Инвентаризационная опись товаров)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 7;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ИНВ3БезФактическихДанных";
	КомандаПечати.Представление = НСтр("ka='ИНВ-3 (საინვენტარიზაციო აღწერა ცარიელი ფაქტობრივი მონაცემებით)';ru='ИНВ-3 (Инвентаризационная опись с пустыми фактическими данными)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 10;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ИНВ19";
	КомандаПечати.Представление = НСтр("ka='ИНВ-19 (შეჯერებითი უწყისი)';ru='ИНВ-19 (Сличительная ведомость)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 14;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ИНВ19_УчетныеЦены";
	КомандаПечати.Представление = НСтр("ka='ИНВ-19 (შეჯერებითი უწყისი). თანხები აღრიცხვის მონაცემთა მიხედვით';ru='ИНВ-19 (Сличительная ведомость). Суммы по данным учета'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
