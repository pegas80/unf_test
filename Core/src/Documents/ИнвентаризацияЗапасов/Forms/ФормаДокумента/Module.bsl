...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		УстановитьФлагУчетСебестоимостиВРазрезеОрганизации();
...
//	
//	СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
// Kuzmich, 28.04.2020, Подбор цены и сумм для отрицательных и нулевых остатков
Процедура ЗапасыЦенаПриИзменении(Элемент, ТекСтрокаЗапасы=НЕОПРЕДЕЛЕНО)
...
Процедура КомандаЗаполнитьПоОстаткамНаСкладе()
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоОстаткамНаСкладеЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена! Продолжить выполнение операции?';en='Tabular section will be cleared! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
//+++СДА
&НаКлиенте
Процедура КомандаЗаполнитьПоОстаткамНаСкладеБезКоличства(Команда)
	
	Если Объект.Запасы.Количество() > 0 Тогда
		Ответ = Неопределено;

		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоОстаткамНаСкладеЗавершениеБезКоличества", ЭтотОбъект), НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена! Продолжить выполнение операции?';en='Tabular section will be cleared! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;

	КомандаЗаполнитьПоОстаткамНаСкладеФрагментБезКоличества();
КонецПроцедуры
//---СДА
...
&НаКлиенте
Процедура КомандаЗаполнитьПоОстаткамНаСкладеЗавершениеБезКоличества(Результат, ДополнительныеПараметры) Экспорт
...
//+++СДА
&НаКлиенте
Процедура КомандаЗаполнитьПоОстаткамНаСкладеФрагментБезКоличества() Экспорт
    
    ЗаполнитьПоОстаткамНаСкладеБезКоличества();

КонецПроцедуры // КомандаЗаполнитьПоОстаткамНаСкладеФрагментБезКоличества()
//---СДА
...
Процедура КомандаЗаполнитьТолькоУчетныеДанные()
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьТолькоУчетныеДанныеЗавершение", ЭтотОбъект), НСтр("ka='სააღრიცხვო მონაცემები გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Учетные данные будут очищены! Продолжить выполнение операции?';en='The credentials are cleared! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура КомандаЗаполнитьПоВидуЦен(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоВидуЦенЗавершение1", ЭтотОбъект), НСтр("ka='ფასები გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Цены будут перезаполнены! Продолжить выполнение операции?';en='Prices will be refilled! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура КомандаОбнулитьКоличествоИСумму(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаОбнулитьКоличествоИСуммуЗавершение", ЭтотОбъект), НСтр("ka='სვეტი ""რაოდენობა"" და ""ჯამი"" გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Колонки ""Количество"" и ""Сумма"" будут очищены! Продолжить выполнение операции?';en='Column ""Number"" and ""Amount"" will be cleared! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
	Иначе
		Возврат;
	КонецЕсли;
...
Функция CформироватьТекстЗапросаПоОстаткамНаСкладе()
	...
	Если УчетСебестоимостиВРазрезеОрганизации Тогда
		...
		|	СУММА(ЗапасыОстатки.СуммаОстаток) / СУММА(ЗапасыОстатки.КоличествоОстаток) КАК СтоимостьЕдиницы,
		|	ЗапасыОстатки.Организация КАК Организация
		|ПОМЕСТИТЬ ИтоговыеЗапасыОстаткиСебестоимости
		|ИЗ
		...
		|	СУММА(ЕСТЬNULL(ИтоговыеЗапасыОстаткиСебестоимости.СтоимостьЕдиницы, 0) * ЗапасыНаСкладахОстатки.КоличествоОстаток) КАК СуммаУчет,
		|	СУММА(ЕСТЬNULL(ИтоговыеЗапасыОстаткиСебестоимости.СтоимостьЕдиницы, 0) * ЗапасыНаСкладахОстатки.КоличествоОстаток) КАК Сумма
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИтоговыеЗапасыОстаткиСебестоимости КАК ИтоговыеЗапасыОстаткиСебестоимости
		|		ПО ЗапасыНаСкладахОстатки.Организация = ИтоговыеЗапасыОстаткиСебестоимости.Организация
		|			И ЗапасыНаСкладахОстатки.Номенклатура = ИтоговыеЗапасыОстаткиСебестоимости.Номенклатура
		|			И ЗапасыНаСкладахОстатки.Характеристика = ИтоговыеЗапасыОстаткиСебестоимости.Характеристика
		|			И ЗапасыНаСкладахОстатки.Партия = ИтоговыеЗапасыОстаткиСебестоимости.Партия
		|ГДЕ
		|	ЗапасыНаСкладахОстатки.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
...
//+++СДА
&НаСервере
Процедура ЗаполнитьПоОстаткамНаСкладеБезКоличества()
	
	тзНоменклатуры = Объект.Запасы.Выгрузить(,"Номенклатура,Характеристика,Партия,Количество");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(тзНоменклатуры.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	тзНоменклатуры.Характеристика КАК Характеристика,
	|	тзНоменклатуры.Партия КАК Партия,
	|	ВЫРАЗИТЬ(тзНоменклатуры.Количество КАК ЧИСЛО(15,3)) КАК Количество
	...
	|ВЫБРАТЬ
	|	1 КАК Порядок,
	|	ВТ_СтарыеДанные.Номенклатура.Наименование КАК Наименование,
	|	ВТ_СтарыеДанные.Номенклатура КАК Номенклатура,
	|	ЗапасыОстаткиИнвентаризация.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ВТ_СтарыеДанные.Характеристика КАК Характеристика,
	|	ВТ_СтарыеДанные.Партия КАК Партия,
	|	ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.КоличествоУчет, 0) КАК КоличествоУчет,
	|	ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.СуммаУчет, 0) КАК СуммаУчет,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.КоличествоУчет, 0) <= 0
	...
	|		ИНАЧЕ ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.Сумма, 0) / ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.КоличествоУчет, 0)
	|	КОНЕЦ КАК ЦенаУчет,
	|	ВТ_СтарыеДанные.Количество КАК Количество,
	|	ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.Сумма, 0) КАК Сумма,
	|	ВЫБОР
	|		КОГДА ЗапасыОстаткиИнвентаризация.ПризнакВЯчейке
	|			ТОГДА ВТ_СтарыеДанные.Количество - ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.Количество, 0)
	|		ИНАЧЕ ВТ_СтарыеДанные.Количество - ЕСТЬNULL(ЗапасыОстаткиИнвентаризация.КоличествоУчет, 0)
	|	КОНЕЦ КАК Отклонение,
	|	ВТ_СтарыеДанные.Номенклатура КАК Ссылка
	|ИЗ
	|	ВТ_СтарыеДанные КАК ВТ_СтарыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗапасыОстаткиИнвентаризация КАК ЗапасыОстаткиИнвентаризация
	|		ПО ВТ_СтарыеДанные.Номенклатура = ЗапасыОстаткиИнвентаризация.Номенклатура
	|			И ВТ_СтарыеДанные.Характеристика = ЗапасыОстаткиИнвентаризация.Характеристика
	|			И ВТ_СтарыеДанные.Партия = ЗапасыОстаткиИнвентаризация.Партия
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок УБЫВ,
	|	Ссылка ИЕРАРХИЯ,
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.КоличествоУчет <> 0 Тогда
			...
			//Если Выборка.КоличествоУчет < 0 Тогда 
			//	НоваяСтрока.Отклонение = НоваяСтрока.Отклонение * (-1);
			//КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Элементы.ЗапасыПодобратьЦены.Видимость = ПодборЦеныДляОтрицательныхОстатков(); // Kuzmich, 28.04.2020, Подбор цены и сумм для отрицательных и нулевых остатков
...
Процедура ОчиститьУсловиеОтбораПоНоменклатуре()
	...
	Элементы.УстановитьОтборПоНоменклатуре.Заголовок = НСтр("en='The Selection is Not Set';ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
...
Процедура ОчиститьУсловиеОтбораПоГруппамНоменклатуры()
	...
	Элементы.УстановитьОтборПоГруппамНоменклатуры.Заголовок = НСтр("en='The Selection is Not Set';ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
...
Процедура ОчиститьУсловиеОтбораПоКатегориямНоменклатуры()
	...
	Элементы.УстановитьОтборПоКатегориямНоменклатуры.Заголовок = НСтр("en='The Selection is Not Set';ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
...
Процедура ДобавитьПоКатегориям(Команда)
	...
	СтруктураВыбора = Новый Структура("МножественныйВыбор", Истина);
	ВыбранныеКатегории = ПолучитьФорму("Справочник.КатегорииНоменклатуры.Форма.ФормаВыбора", СтруктураВыбора);
...
Процедура ДобавитьПоКатегориямНаСервере(ВыбранныеКатегории)
	...
	Запрос.Текст = 
		...
		|	НЕ Номенклатура.ПометкаУдаления
		|	И НЕ Номенклатура.ЭтоГруппа
		|	И Номенклатура.КатегорияНоменклатуры В ИЕРАРХИИ(&ВыбранныеКатегории)
		|	И НЕ Номенклатура.Ссылка В
		|				(ВЫБРАТЬ
		|					Запасы.Номенклатура
		|				ИЗ
		|					Запасы КАК Запасы)";
	Запрос.УстановитьПараметр("ВыбранныеКатегории",	ВыбранныеКатегории);
	Запрос.УстановитьПараметр("Запасы", 			Объект.Запасы.Выгрузить());
	ТаблицаНоменклатур = Запрос.Выполнить().Выгрузить();
...
&НаКлиенте
Процедура ЗаполнитьКоличествоПоУчетномуКоличеству(Команда)
	
	Если Объект.Запасы.Количество() > 0 Тогда
		Ответ = Неопределено;

		ПоказатьВопрос(Новый ОписаниеОповещения("Команда_ЗаполнитьКоличествоПоУчетномуКоличеству_Завершение", ЭтотОбъект), НСтр("ka='სვეტი ""რაოდენობა"" გადაივსება სვეტით ""სააღრიცხვო რაოდენობა""! გავაგრძელოთ ოპერაციის შესრულება?';ru='Колонка ""Количество"" перезаполнится по колонкой ""учетное количество""! Продолжить выполнение операции?';en='The column ""Number"" column refill on ""Accounting amount""! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
	Иначе
		Возврат;
	КонецЕсли;
	
	Команда_ЗаполнитьКоличествоПоУчетномуКоличеству_Фрагмент();
КонецПроцедуры

&НаКлиенте
Процедура Команда_ЗаполнитьКоличествоПоУчетномуКоличеству_Фрагмент()
...
&НаКлиенте
Процедура Команда_ЗаполнитьКоличествоПоУчетномуКоличеству_Завершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьСуммыИКоличестваПоОстаткам(Команда)
	
	Если Объект.Запасы.Количество() > 0 Тогда
		Ответ = Неопределено;

		ПоказатьВопрос(Новый ОписаниеОповещения("Команда_ЗаполнитьСуммыИКоличестваПоОстаткам_Завершение", ЭтотОбъект), НСтр("ka='რაოდენობი და თანხები გადაივსება ნაშთის მიხედვით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Количества и суммы заполнятся по остаткам! Продолжить выполнение операции?';en='The number and amount will be filled from the remains! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
	Иначе
		Возврат;
	КонецЕсли;
	
	Команда_ЗаполнитьСуммыИКоличестваПоОстаткам_Фрагмент();
КонецПроцедуры

&НаСервере
Процедура Команда_ЗаполнитьСуммыИКоличестваПоОстаткам_Фрагмент()
	...
	|	ИнвентаризацияЗапасовЗапасы.Партия КАК Партия,
	|	ИнвентаризацияЗапасовЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ВТЗапасыОстатки.Количество КАК Количество,
	|	ВТЗапасыОстатки.СуммаУчет КАК Сумма,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВТЗапасыОстатки.КоличествоУчет, 0) <= 0
	|				ИЛИ ЕСТЬNULL(ВТЗапасыОстатки.СуммаУчет, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(ВТЗапасыОстатки.СуммаУчет, 0) / ЕСТЬNULL(ВТЗапасыОстатки.КоличествоУчет, 0)
	|	КОНЕЦ КАК ЦенаУчет,
	|	ЕСТЬNULL(ВТЗапасыОстатки.Количество, 0) / ЕСТЬNULL(ИнвентаризацияЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент, 1) КАК КоличествоВЯчейке,
	|	ЕСТЬNULL(ВТЗапасыОстатки.КоличествоУчет, 0) / ЕСТЬNULL(ИнвентаризацияЗапасовЗапасы.ЕдиницаИзмерения.Коэффициент, 1) КАК КоличествоУчет,
	|	ЕСТЬNULL(ВТЗапасыОстатки.СуммаУчет, 0) КАК СуммаУчет,
	...
	|		КОГДА ВТЗапасыОстатки.ПризнакВЯчейке
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ВТЗапасыОстатки.Количество, 0) < 0
	|						ТОГДА -ЕСТЬNULL(ВТЗапасыОстатки.Количество, 0)
	|					ИНАЧЕ 0
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЕСТЬNULL(ВТЗапасыОстатки.КоличествоУчет, 0) < 0
	|					ТОГДА -ЕСТЬNULL(ВТЗапасыОстатки.КоличествоУчет, 0)
	|				ИНАЧЕ 0
	|			КОНЕЦ
	|	КОНЕЦ КАК Отклонение
	|ИЗ
...
&НаКлиенте
Процедура Команда_ЗаполнитьСуммыИКоличестваПоОстаткам_Завершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура  ИтогСумма()
	 Отклонение = 0;
	 Объект.ИтогСумма = Объект.Запасы.Итог("Сумма");
	 Объект.ИтогСуммаУчет = Объект.Запасы.Итог("СуммаУчет");
	 Отклонение = Объект.ИтогСумма - Объект.ИтогСуммаУчет;
	 Объект.ИтогСуммаОтклонение = Отклонение;
КонецПроцедуры

// Kuzmich, 28.04.2020, Подбор цены и сумм для отрицательных и нулевых остатков
#Область ПодборЦенДляТоваровКоторыхНетНаСкладе 
&НаСервере
Функция ПодборЦеныДляОтрицательныхОстатков()
	Возврат Константы.ПодборЦеныДляОтрицательныхОстатков.Получить();
КонецФункции // ()

&НаКлиенте
Процедура ПодобратьЦены(Команда)
	
	Если ПодборЦеныДляОтрицательныхОстатков() Тогда
		...
		Для каждого СтрЗапасы Из Объект.Запасы Цикл
			...
			Если СтрЗапасы.Цена <= 0 Тогда
			
				РЕЗ = РозницаHTTP.ПолучитьОстаткиТовараПоБухСчетам(СтрЗапасы.Номенклатура,Объект.СтруктурнаяЕдиница,ТекДата);
				Если РЕЗ.Цена>0 Тогда
					СтрЗапасы.Цена = РЕЗ.Цена;
					ЗапасыЦенаПриИзменении(Элементы.ЗапасыЦена, СтрЗапасы);
				КонецЕсли;
...
// © Шишка В.В. [21 октября 2021]  НАЧАЛО
&НаСервере
Процедура УстановитьФлагУчетСебестоимостиВРазрезеОрганизации()
...
КонецПроцедуры // УстановитьФлагУчетСебестоимостиВРазрезеОрганизации()
...
Процедура ЗаполнитьПоДаннымExcel(Команда)
	...
	ОткрытьФорму("Документ.ИнвентаризацияЗапасов.Форма.ФормаВыбораПути", ,ЭтотОбъект, , , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
	//ЗаполнитьПоДаннымExcelНаСервере();
КонецПроцедуры
...
Процедура ВариантЗагрузкиЗавершение(СтруктураВыбора, Параметры) Экспорт
	...
	ЗаполнитьПоДаннымExcelНаСервере(ТабДок, ПутьКФайлу);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПоДаннымExcelНаСервере(ТабДок, ПутьКФайлу)
	
	ТабДок.Прочитать(ПутьКФайлу);
	
	ТаблицаЗагрузки = Новый ТаблицаЗначений;
	//КоллекцияКолонок = Метаданные.Документы.ИнвентаризацияЗапасов.ТабличныеЧасти.Запасы.Реквизиты;
	//Для Каждого Рекв Из КоллекцияКолонок Цикл 
	//	ТаблицаЗагрузки.Колонки.Добавить(Рекв.Имя);
	//КонецЦикла;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда 
		НомерКолонкиКоличество = НомерКолонкиКоличество + 1;
	КонецЕсли;
	...
	Для Сч = 2 По ТабДок.ВысотаТаблицы Цикл
		...
		//СтрокаТаблицы.ПолучательТекст = СокрЛП(ТабДок.Область(Сч, 4, Сч, 4).Текст); 
		//СтрокаТаблицы.ВалютаТекст = СокрЛП(ТабДок.Область(Сч, 5, Сч, 5).Текст);
		//СтрокаТаблицы.СуммаТекст = СокрЛП(ТабДок.Область(Сч, 6, Сч, 6).Текст);
		
	КонецЦикла;
	
	Для Каждого Строк Из ТаблицаЗагрузки Цикл 
		
		Объект.Запасы[Строк.НомерСтроки - 1].Количество = Строк.Количество;
		
		Объект.Запасы[Строк.НомерСтроки - 1].Отклонение = Объект.Запасы[Строк.НомерСтроки - 1].Количество - Объект.Запасы[Строк.НомерСтроки - 1].КоличествоУчет;
		Объект.Запасы[Строк.НомерСтроки - 1].Сумма = Объект.Запасы[Строк.НомерСтроки - 1].Количество * Объект.Запасы[Строк.НомерСтроки - 1].Цена;
		
		// Серийные номера
		Если ИспользоватьСерийныеНомераОстатки<>Неопределено Тогда
			РаботаССерийнымиНомерамиКлиентСервер.ОбновитьСерийныеНомераКоличество(Объект, Объект.Запасы[Строк.НомерСтроки - 1]);
		КонецЕсли;
