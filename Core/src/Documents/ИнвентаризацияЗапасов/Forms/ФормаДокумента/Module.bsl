...
Процедура КомандаЗаполнитьПоОстаткамНаСкладе()
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоОстаткамНаСкладеЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура КомандаЗаполнитьТолькоУчетныеДанные()
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьТолькоУчетныеДанныеЗавершение", ЭтотОбъект), НСтр("ka='სააღრიცხვო მონაცემები გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Учетные данные будут очищены! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура КомандаЗаполнитьПоВидуЦен(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоВидуЦенЗавершение1", ЭтотОбъект), НСтр("ka='ფასები გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Цены будут перезаполнены! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура КомандаОбнулитьКоличествоИСумму(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаОбнулитьКоличествоИСуммуЗавершение", ЭтотОбъект), НСтр("ka='სვეტი ""რაოდენობა"" და ""ჯამი"" გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Колонки ""Количество"" и ""Сумма"" будут очищены! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
	Иначе
		Возврат;
	КонецЕсли;
...
Функция СформироватьЗаголовокОтбораИзСписка(СписокЭлементов)
	...
	Если ЗаголовокОтбораСтрока = "" Тогда
		ЗаголовокОтбораСтрока = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
	КонецЕсли;
...
Процедура ОчиститьУсловиеОтбораПоНоменклатуре()
	...
	Элементы.УстановитьОтборПоНоменклатуре.Заголовок = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
...
Процедура ОчиститьУсловиеОтбораПоГруппамНоменклатуры()
	...
	Элементы.УстановитьОтборПоГруппамНоменклатуры.Заголовок = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
...
Процедура ОчиститьУсловиеОтбораПоКатегориямНоменклатуры()
	...
	Элементы.УстановитьОтборПоКатегориямНоменклатуры.Заголовок = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен'");
...
Процедура ЗаполнитьСуммыИКоличестваПоОстаткам(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("Команда_ЗаполнитьСуммыИКоличестваПоОстаткам_Завершение", ЭтотОбъект), НСтр("ka='რაოდენობი და თანხები გადაივსება ნაშთის მიხედვით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Количества и суммы заполнятся по остаткам! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
	Иначе
		Возврат;
	КонецЕсли;
