		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			Если СтрокаЗапасы.Номенклатура.ИспользоватьСерийныеНомера Тогда
				...
				Если ОтборСерийныеНомера.Количество() <> СтрокаЗапасыКоличество Тогда
					ТекстСообщения = НСтр("ka='სერიული ნომრების რიცხოვნება სტრიქონ %Номер%.-ში არსებული ერთეულების რაოდენობისგან განსხვავდება.';ru='Число серийных номеров отличается от количества единиц в строке %Номер%.';en='The number of serial numbers is different from the number of units in line% number%.'");
