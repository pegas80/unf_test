...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", НСтр("ka='მარაგების შემოსავალში აყვანა';ru='Оприходование запасов'"));
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			//	
				...
				СтрокаТаблицыРасходДляСебестоимости.Период = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасы.ЗаказПокупателя), СтрокаТаблицаЗапасы.ЗаказПокупателя.Дата, СтрокаТаблицыРасходДляСебестоимости.Период);
				...
				СтрокаТаблицыРасходДляСебестоимости.СодержаниеПроводки = НСтр("ka='მარაგების რეზერვირება';ru='Резервирование запасов'");
			...
			////	
				...
				СтрокаТаблицыПриход.Период = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасы.ЗаказПокупателя), СтрокаТаблицаЗапасы.ЗаказПокупателя.Дата, СтрокаТаблицыПриход.Период);
			...
			Если Передача Тогда
				...
				СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ka='მარაგების გადაცემა';ru='Передача запасов'"); 
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Доставка", НСтр("ka='Выручка от доставки';ru='Выручка от доставки';en='Revenue from delivery'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница';en='Exchange difference'"));
	Запрос.УстановитьПараметр("БезвозмезднаяПродажа", НСтр("ka='Безвозмезная продажа';ru='Безвозмезная продажа';en='Free sale'"));
	Запрос.УстановитьПараметр("СписаниеСебестоимости", НСтр("ka='Списание себестоимости';ru='Списание себестоимости';en='Write-off of cost'"));
	Запрос.УстановитьПараметр("НДС", НСтр("ka='НДС';ru='НДС';en='VAT'"));
	Запрос.УстановитьПараметр("НДФЛ", НСтр("ka='НДФЛ';ru='НДФЛ';en='PIT'"));
	Запрос.УстановитьПараметр("ПФ", НСтр("ka='ПФ';ru='ПФ';en='PF'"));
	Запрос.УстановитьПараметр("СписаниеСуммыБезНалогов", НСтр("ka='Списание суммы без налогов';ru='Списание суммы без налогов';en='Write-off amounts without taxes'"));
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателяПоДоставке", НСтр("ka='Возникновение обязательств покупателя по доставке';ru='Возникновение обязательств покупателя по доставке'"));
	...
	Запрос.УстановитьПараметр("БезвозмезднаяПродажа", НСтр("ka='Безвозмездная продажа';ru='Безвозмездная продажа'"));
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("СторнированиеДолга", НСтр("ka='დავალიანების სტორნირება';ru='Сторнирование долга'"));
	Запрос.УстановитьПараметр("ВосстановлениеПредоплаты", НСтр("ka='წინასწარი გადახდის აღდგენა';ru='Восстановление предоплаты'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаРасчетыПоНДФЛ(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) //+++ Дмгг
	...
	Запрос.УстановитьПараметр("Содержание", Нстр("ka='Начислен налог';ru='Начислен налог'"));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ТаблицаЗапасы.Период,
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СторнированиеПредоплаты", НСтр("ka='წინასწარ გადახდის სტორნირება';ru='Сторнирование предоплаты'"));
	Запрос.УстановитьПараметр("НачислениеНДС", НСтр("ka='Начисление НДС';ru='Начисление НДС'"));
	Запрос.УстановитьПараметр("СторнированиеПоставки", НСтр("ka='მიწოდების სტორნირება';ru='Сторнирование поставки'"));
	Запрос.УстановитьПараметр("ОтражениеВыручки", НСтр("ka='შემოსავალი გაყიდვიდან';ru='Выручка от продажи'"));
	Запрос.УстановитьПараметр("ОтражениеДоставки", НСтр("ka='Выручка от доставки';ru='Выручка от доставки'"));
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Штрихкод", НСтр("ka='შტრიხ კოდი';ru='Штрихкод'"), ОписаниеТиповСтрока200, ОписаниеТиповКолонка, "Номенклатура", 1, , Истина);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Содержание",  НСтр("ka='შინაარსი';ru='Содержание'"), ОписаниеТиповСтрока1000, ОписаниеТиповСтрока1000, , , , , НастройкиЗагрузкиДанных.СодержаниеВидимо);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Характеристика",  НСтр("ka='თვისება (დასახელება)';ru='Характеристика (наименование)'"), ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ЕдиницаИзмерения", НСтр("ka='საზომი ერთეული';ru='Ед. изм.'"), ОписаниеТиповСтрока25, ОписаниеТиповКолонка, , , , , ПолучитьФункциональнуюОпцию("УчетВРазличныхЕдиницахИзмерения"));
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтавкаНДС", НСтр("ka='დღგ';ru='Ставка НДС'"), ОписаниеТиповСтрока50, ОписаниеТиповКолонка);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Заказ", НСтр("ka='შეკვეთა (მყიდველი/მომწოდებელი)';ru='Заказ (покупатель/поставщик)'"), ОписаниеТиповСтрока50, ОписаниеТиповКолонка, , , , , ВидимостьЗаказаПокупателя);
	
	Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.СтраныМира");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтранаПроисхождения", НСтр("ka='მწარმოებელი ქვეყანა';ru='Страна происхождения'"), ОписаниеТиповСтрока100, ОписаниеТиповКолонка);
		
		ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.НомераГТД");
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерГТД", НСтр("ka='საბაჭო დეკლარაციის ნომერი';ru='Номер ГТД'"), ОписаниеТиповСтрока30, ОписаниеТиповКолонка);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|	СУММА(ЕСТЬNULL(РасходнаяНакладнаяПредоплата.Курс, 0)) КАК Курс,
	|	СУММА(ЕСТЬNULL(РасходнаяНакладнаяПредоплата.Кратность, 0)) КАК Кратность,
...
Функция ОтчетОРозничныхПродажахXML(ДокументСсылка) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ka='Формат 2.40 для расходной накладной в розницу не поддерживается';ru='Формат 2.40 для расходной накладной в розницу не поддерживается'");
	КонецЕсли;
...
Функция ОтчетОРозничныхПродажахXML2_41(ДокументСсылка) Экспорт
	...
	Если Не Шапка.Следующий()
		...
		СообщениеXML.ТекстОшибки = НСтр("ka='გადმოსატვირთად მონაცემები არ არის.';ru='Нет данных для выгрузки.'");
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		doc.doc_type   = 1;
		doc.doc_name   = НСтр("ka='ჩეკი';ru='Чек'");
		doc.doc_number = НСтр("ka='б/н';ru='б/н'");
...
Функция СформироватьУниверсальныйПередаточныйДокумент(ТабличныйДокумент, ТекущийДокумент)
	...
	Если ЗначениеЗаполнено(Шапка.ДоверенностьВыдана)
		...
		ТекстОснования = Шапка.Основание + НСтр("ka='; მინდობილობით #';ru='; по доверенности №'") + Шапка.ДоверенностьНомер 
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='M15 (ზედნადები მასალების გასაცემად)';ru='М15 (Накладная на отпуск материалов)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 17;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ТТН";
	КомандаПечати.Представление = НСтр("ka='1-Т (სატრანსპორტო-სასაქონლო ზედნადები)';ru='1-Т (Товарно-транспортная накладная)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ka='დანართი #4 (სასაქონლო ზედანდები';ru='Приложение №4 (Товарная накладная)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ka='Счет-фактура';ru='Счет-фактура'");
	...
	КомандаПечати.Представление = НСтр("ka='გასავლის ზედდებული';ru='Расходная накладная'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 32;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "НакладнаяСУслугами";
	КомандаПечати.Представление = НСтр("ka='გასავლის ზედნადები (მომსახურებებთან ერთად)';ru='Расходная накладная (с услугами)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ka='სასაქონლო ჩეკი';ru='Товарный чек'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление 					= НСтр("ka='გასავლის ზედდებული(შტრიხ კოდით)';ru='Расходная Накладная (ШтрихКод)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью	= Ложь;
