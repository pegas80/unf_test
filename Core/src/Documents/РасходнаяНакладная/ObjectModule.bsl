...
Процедура ЗаполнитьПоСтруктуре(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("Основание")
		...
		Тогда
		ВызватьИсключение НСтр("ka='გასავლის ზედნადების შეყვანა შეუძლებელია შეკვეთა-განწესის საფუძველზე!';ru='Нельзя ввести Расходную накладную на основании заказ-наряда!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнойНакладной(ДанныеЗаполнения, Операция = "") Экспорт
	...
	Если Операция = "Продажа" Тогда
		...
		Иначе
			...
			НСтр("ka='ოპერაცია ""მყიდველისთვის მიყიდვა""-ს შეყვანა შეუძლებელია ოპერაციის საფუძველზე - ""%1""!';ru='Невозможен ввод операции ""Продажа покупателю"" на основании операции - ""%1""!'"),
	...
	ИначеЕсли Операция = "Возврат" Тогда
		...
		Иначе
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='ოპერაცია ""დაბრუნება""-ს შეყვანა შეუძლებელია ოპერაციის საფუძველზე - ""%1""!';ru='Невозможен ввод операции ""Возврат"" на основании операции - ""%1""!'"),
			ДанныеЗаполнения.ВидОперации);
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если НайденнаяСтрокаЗапасы = Неопределено
		   ...
		   И Контрагент.ВестиРасчетыПоЗаказам Тогда
			ТекстСообщения = НСтр("ka='ავანსის ჩათვლა შეუძლებელია ცხრილის ნაწილ ""მარაგებში"" ან ""ხარჯებში"" მითითებული შეკევთისგან განსხვავებულის მიხედვით!';ru='Нельзя зачесть аванс по заказу отличному от указанных в табличных частях ""Запасы"" или ""Расходы""!'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить()
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
				...
				ТекстСообщения = НСтр("ka='ცხრილ ""მარაგების და მომსახურებების"" სტრიქონ №%Номер% რეზერვიდან გადმოტვირთული პოზიციების რაოდენობა აღემატება მარაგების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Запасы и услуги"" количество отгружаемых позиций из резерва превышает общее количество запасов.'");
		...
		Для Каждого Стр Из Запасы Цикл
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Договор, Стр.Номенклатура, Стр.Характеристика) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='მომსახურების მიმდინარე ხელშეკრულების მიხედვით დაუგეგმავი საქონლების/მომსახურებების გატარება აკრძალულია!';ru='Запрещено проводить незапланированные товары/услуги по текущему договору обслуживания!'"),
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Тогда 
		Если Запасы.Количество() = 0 Тогда 
			Отказ = Истина;
			Сообщить(НСтр("ka='ცხრილურ ნაწილში ""მარაგები"" არ არის შევსებული არცერთი სტრიქონი';ru='В табличной части ""Запасы"" нет ни одной записи'"));
		КонецЕсли;
	КонецЕсли;
