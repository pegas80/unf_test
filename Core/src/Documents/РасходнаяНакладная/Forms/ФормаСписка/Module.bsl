...
Процедура СоздатьРасходнуюНакладную(Команда)
	...
	Иначе
		...
		Если СтруктураДанных.СформироватьНесколькоЗаказов Тогда
			...
			ТекстСообщения = НСтр("ka='შეკვეთები განსხვავდება დოკუმენტების თავსართის მონაცემებით (%ПредставлениеДанных%)! ჩამოვაყალიბოთ რამდენიმე გასავლის ზედნადები?';ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько расходных накладных?';en='Orders different data (% PredstavlenieDannyh%) hats documents! Form several outgoing invoices?'");
