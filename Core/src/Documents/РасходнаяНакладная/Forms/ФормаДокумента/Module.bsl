...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ СчетФактураТекст = ""
		...
		ТекстВопроса = НСтр("ka='დოკუმენტში განხორციელდა ცვლილებები. საჭიროა თუ არა დაქვემდებარებული ანგარიშფაქტურის განმეორებით შევსება?';ru='В документе были произведены изменения. Требуется ли повторно заполнить подчиненный Счет-фактуру?'");
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентПередИзменением <> Объект.Контрагент Тогда
		...
		Если Объект.Предоплата.Количество() > 0
			...
			ТекстВопроса = НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?'");
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПриИзмененииЗавершение", ЭтотОбъект), НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?'"), Режим, 0);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованиюНачало() Экспорт
	...
	ПоказатьВопрос(
		ОписаниеОповещения, 
		НСтр("ka='შევავსოთ დოკუმენტი არჩეული საფუძვლით?';ru='Заполнить документ по выбранному основанию?'"), 
...
Процедура ЗапасыНомерГТДПриИзменении(Элемент)
	...
	Если СтрокаТабличнойЧасти <> Неопределено
		...
		Если ДатаГТД > КонецДня(Объект.Дата) Тогда
			...
			ТекстСообщения = НСтр("ka='ყურადღება! არჩეული სატვირთო-საბაჟო დეკლარაცია დათარიღებულია უფრო გვიანი თარიღით, ვიდრე მიმდინარე დოკუმენტი';ru='Внимание! Выбранная ГТД датирована более поздней датой, чем текущий документ'");
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულიად შეივსება ""საფუძვლით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულიად შეივსება ""შეკვეთით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Заказу""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(, НСтр("ka='დასაწყისში მიუთითეთ კონტრაგენტი!';ru='Укажите вначале контрагента!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
		ПоказатьПредупреждение(, НСтр("ka='დასაწყისში მიუთითეთ კონტრაგენტის ხელშეკრულება!';ru='Укажите вначале договор контрагента!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ТекШтрихкод = "";
	ПоказатьВводЗначения(Новый ОписаниеОповещения("ПоискПоШтрихкодуЗавершение", ЭтотОбъект, Новый Структура("ТекШтрихкод", ТекШтрихкод)), ТекШтрихкод, НСтр("ka='შეიყვანეთ შტრიხკოდი';ru='Введите штрихкод'"));
...
Процедура ИзменитьРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ცხრილი ""მარაგები და მომსახურებები"" შევსებული არ არის!';ru='Табличная часть ""Запасы и услуги"" не заполнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ПередатьДанныеВЕГАИС(Команда)
	...
	Если Модифицированность ИЛИ НЕ Объект.Проведен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Перед выполнение операции необходимо провести документ.';ru='Перед выполнение операции необходимо провести документ.'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ესასს-ის ზედნადების გადაცემა მხარდაჭერილია მხოლოდ ოპერაცია ""მომწოდებლისთვის დაბრუნება""-სთვის.';ru='Передача накладной ЕГАИС поддерживается только для операции ""Возврат поставщику"".'"));
		Возврат;
	КонецЕсли;
...
Процедура ОповещениеПередачаДанныхВЕГАИС(ДанныеОтЕГАИС, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ДанныеОтЕГАИС = Неопределено Тогда
		Если НЕ ДанныеОтЕГАИС.Результат Тогда
			СтрокаСообщения = НСтр("ka='Ошибка передачи данных в ЕГАИС';ru='Ошибка передачи данных в ЕГАИС'") 
...
Процедура ПодготовитьТТН(ДокументСсылка, Отказ)
	...
	Если ВыборкаДокумент.Следующий() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДокумент.Грузополучатель) Тогда
			Отказ = Истина;
			СтрокаОшибки = НСтр("ka='ესასს-ის კლასიფიკატორთან კონტრაგენტის შესაბამისობის დადგენა ვერ მოხერხდა.';ru='Не удалось определить соответствие контрагента классификатору ЕГАИС.'");
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДокумент.ОрганизацияЕГАИС) Тогда
			Отказ = Истина;
			СтрокаОшибки = НСтр("ka='ესასს-ის კლასიფიკატორთან ორგანიზაციის შესაბამისობის დადგენა ვერ მოხერხდა.';ru='Не удалось определить соответствие организации классификатору ЕГАИС.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрокаОшибки,
				ДокументСсылка,
				"Объект.Организация");
		...
		Если КоличествоСтрок = 0 Тогда
			Отказ = Истина;
			СтрокаОшибки = НСтр("ka='ესასს-ის კლასიფიკატორის მიხედვით ალკოჰოლური პროდუქციის შესაბამისობის დადგენა ვერ მოხერხდა.';ru='Не удалось определить соответствие алкогольной продукции по классификатору ЕГАИС.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрокаОшибки,
				ДокументСсылка,
				"Объект.Запасы");
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаТовары.НомерСправкиА) Тогда
				Отказ = Истина;
				СтрокаОшибки = НСтр("ka='სტრიქონ %-ში ცნობა ბ-ში ცნობა ა-ს ნომერი არ არის შევსებული.';ru='В строке %1 в справке Б не заполнен номер справки А.'");
			...
			Если ЗначениеЗаполнено(ВыборкаТовары.СправкаБ) Тогда
				Если НЕ ЗначениеЗаполнено(ВыборкаТовары.АлкогольнаяПродукцияСправкиБ)
					...
					Отказ = Истина;
					СтрокаОшибки = НСтр("ka='სტრიქონ %1-ში არ არის შევსებული ალკოჰოლური პროდუქცია ცნობა ბ-ში.';ru='В строке %1 не заполнена алкогольная продукция в справке Б.'");
					СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ВыборкаТовары.НомерСтроки);
					ИмяПоляТоваров = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Запасы", ВыборкаТовары.НомерСтроки, "СправкаБ");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрокаОшибки,
			...
			Иначе
				Отказ = Истина;
				СтрокаОшибки = НСтр("ka='სტრიქონ %1-ში ცნობა ბ არ არის შევსებული.';ru='В строке %1 не заполнена справка Б.'");
				СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ВыборкаТовары.НомерСтроки);
				ИмяПоляТоваров = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Запасы", ВыборкаТовары.НомерСтроки, "СправкаБ");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрокаОшибки,
	...
	Если ТипЗнч(СтруктураЦеныИВалюта) = Тип("Структура") И СтруктураЦеныИВалюта.БылиВнесеныИзменения Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗакрытия.ДисконтнаяКарта) И ЗначениеЗаполнено(РезультатЗакрытия.Контрагент) И Не Объект.Контрагент.Пустая() Тогда
			...
			Иначе // Выдадим сообщение и не будем менять данные о дисконтной карте.
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='დისკონტური ბარათი არ არის წაკითხული. დისკონტური ბარათის მფლობელი დოკუმენტში არსებულ კონტრაგენტს არ ემთხვევა.';ru='Дисконтная карта не считана. Владелец дисконтной карты не совпадает с контрагентом в документе.'"),
				,
				"Контрагент",
				"Объект");
			КонецЕсли;
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	Если СтруктураНадписи.УчетВалютныхОпераций Тогда
		Если ЗначениеЗаполнено(СтруктураНадписи.ВалютаДокумента) Тогда
			ТекстНадписи = НСтр("ka='%Валюта%';ru='%Валюта%'");
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ДисконтнаяКарта) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ka='%ДисконтнаяКарта%';ru='%ДисконтнаяКарта%'");
		Иначе
	...
	Если ПустаяСтрока(ТекстНадписи) Тогда	
		Если СтруктураНадписи.СуммаВключаетНДС Тогда
			ТекстНадписи = НСтр("ka='თანხა შეიცავს დღგ-ს';ru='Сумма включает НДС'");
		Иначе
			ТекстНадписи = НСтр("ka='თანხა არ შეიცავს დღგ-ს';ru='Сумма не включает НДС'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолученыШтрихкодыФрагмент(НеизвестныеШтрихкоды) Экспорт
	...
	Для каждого ТекНеизвестныйШтрихкод Из НеизвестныеШтрихкоды Цикл
		...
		СтрокаСообщения = НСтр("ka='მონაცემები შტრიხკოდის მიხედვით ნაპოვნი არ არის: %1%; რაოდენობა: %2%';ru='Данные по штрихкоду не найдены: %1%; количество: %2%'");
...
Процедура ПолученыШтрихкодыНекорректногоТипа(ШтрихкодыНекорректногоТипа) Экспорт
	...
	Для каждого ТекНекорректныйШтрихкод Из ШтрихкодыНекорректногоТипа Цикл
		...
		СтрокаСообщения = НСтр("ka='შტრიხკოდ %1%-ით მოძებნილ ნომენკლატურას -%2%- აქვს ტიპი %3%, რომელიც ამ ცხრილის შესაფერი არ არის';ru='Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этой табличной части'");
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю")
		...
		Элементы.Контрагент.Заголовок = НСТР("ka='მყიდველი';ru='Покупатель'");
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаОтветхранение") Тогда
		Элементы.Контрагент.Заголовок = НСТР("ka='მყიდველი';ru='Покупатель'");
	Иначе	
		Элементы.Контрагент.Заголовок = НСТР("ka='მომწოდებელი';ru='Поставщик'");
	КонецЕсли;
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	Если Объект.Контрагент.Пустая() И Не ВладелецДисконтнойКарты.Пустая() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='შევსებულია კონტრაგენტი და წაკითხულია დისკონტური ბარათი';ru='Заполнен контрагент и считана дисконтная карта'"),
			ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტში შევსებულია კონტრაგენტი და წაკითხულია დისკონტური ბარათი %1';ru='В документе заполнен контрагент и считана дисконтная карта %1'"), ДисконтнаяКарта),
			БиблиотекаКартинок.Информация32);
	...
	Иначе
		ПоказатьОповещениеПользователя(
			НСтр("ka='Считана дисконтная карта';ru='Считана дисконтная карта'"),
			ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='მარაგების ჩატვირთვა ფაილიდან';ru='Загрузка запасов из файла'"));
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ka='მონაცემების ჩატვირთვა';ru='Загрузка данных'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Номенклатура, , ОписаниеОшибки());
		ОтменитьТранзакцию();
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='Отчет о розничных продажах был изменен. Записать?';ru='Отчет о розничных продажах был изменен. Записать?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ka='Отчет о розничных продажах не записан. Записать?';ru='Отчет о розничных продажах не записан. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Функция ПроверитьВозможностьСозданияЭЛНакладной()
	...
	Если Модифицированность Или НЕ ПроверитьЗаполнение() Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='დოკუმენტი შეიცვალა. თავდაპირველად, საჭიროა დოკუმენტის ჩაწერა!';ru='Документ был изменен. Сначала следует записать документ!'");	
		Сообщение.Сообщить();
		Результат = Ложь;
	Иначе
		Результат = Истина;
	...
	Если Результат Тогда	
		...
		Если Не Объект.Проведен Тогда
			...
			Ответ = Вопрос(НСтр("ka='დოკუმენტი გაუტარებელია. გნებავთ გაგრძელება?';ru='Документ не проведен. хотите продолжить?'"), Режим, 0);						
			Если Ответ <> КодВозвратаДиалога.Да Тогда				
				Результат = Ложь;							
			КонецЕсли; 		
		КонецЕсли;
...
Процедура ВыгрузитьВШаблонRS(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Или Модифицированность Тогда
		ТекстСообщение = НСтр("ka='აუცილებელია დოკუმენტის ჩაწერა. ჩაიწეროს?';ru='Документ должен быть записан. Записать?'");
...
Процедура ДобавитьШтрихкодВТабличнуюЧасть(Штрихкод, ВариантРаспознаванияШтрихкода) Экспорт
	...
	Если СтруктураРезультата = Неопределено Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Товар со штрихкодом %1 не найден!';ru='Товар со штрихкодом %1 не найден!'"), Штрихкод);
...
Процедура ЗаполнитьФактуры(Команда)
	Если Объект.ФактурыАвансов.Количество() > 0 Тогда
		Ответ = Вопрос(НСТР("ka='გაგზავნილი ავანსების ა/ფ-ის ცხრილი არ არის ცარიელი. გასუფთავდეს და გაგრძელდეს?';ru='Таблица СФ Отправленных авансов не пустая. Очистить и продолжить?'"), РежимДиалогаВопрос.ОКОтмена);
