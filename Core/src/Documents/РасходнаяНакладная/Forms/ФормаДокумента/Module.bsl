	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю")
		...
		//Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
			...
			Если ИспользоватьИсториюНалогообложенияКонтрагента Тогда 
				СтруктураНалогообложения = УправлениеНебольшойФирмойСервер.НалогообложениеНДС_СтруктураСНДС(Объект.Организация,, Объект.Дата);
...
Процедура РассчитатьСкидкиНаценки(Команда)
	...
	Если Объект.СкидкиНаценки.Количество() > 0 Тогда
		ОчиститьСообщения();
		сообщить(НСтр("ru = 'Пожалуйста идентифицируйте покупателья'; 
	   				| ka = 'გთხოვთ მოახდინოთ მყიდველის იდენტიფიცირება!'"));
	КонецЕсли;
	//--- Lazare_24.05.2021
