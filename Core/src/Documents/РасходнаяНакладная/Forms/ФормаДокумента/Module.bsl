...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+++ Дмгг
	КолвоБалов = 0;
	КоэффициентЛояльности = БаллыЛояльностьСкидки.ПолучитьКоэффициентЛояльности(Объект.КартаЛояльности, Объект.Дата); //Малышев 2024-12-05
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.КартаЛояльности) Тогда 
			//Элементы.ДоступноБалов.Заголовок = НСтр("ru = 'Достуно балов: '; ka = 'ხელმისაწვდომი ქულები: '; en = 'Достуно балов: '") + (Объект.ДоступноБалов - Объект.Запасы.Итог("БалыРасход"));
			ДоступноБалов = Объект.ДоступноБалов - Объект.Запасы.Итог("БалыРасход");
		КонецЕсли;
	КонецЕсли;
	...
	//++Lazare_30.08.2023 
	УстановитьФорматаВКоличестве(); //  "ЧДЦ=4" 
	//--Lazare_30.08.2023
...
Процедура ПриОткрытии(Отказ)
	...
	//+++ Дмгг
	УстановитьВидимостьИспользованияБалов();
	//--- Дмгг
	...
	ЗапретРедактированияРасходныхНакладныхПоСтатусеТТН();  
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВыбранноеЗначение<>Неопределено и ТипЗнч(ВыбранноеЗначение)=Тип("СправочникСсылка.Сотрудники") Тогда
		ТекДанные.Сотрудник = ВыбранноеЗначение;	
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗапретРедактированияРасходныхНакладныхПоСтатусеТТН" Тогда	
		ЗапретРедактированияРасходныхНакладныхПоСтатусеТТН(); 
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПоказатСебестоимостиВТаблице") Тогда
			Для Каждого Стр Из Объект.Запасы Цикл
				...
				ЦенаПродажи = Стр.Цена * Объект.Курс / Объект.Кратность;//++Lazare 22.12.2022				
				
				Если ЦенаПродажи < Стр.Себестоимость Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ""+ Стр.Номенклатура +"-ის თვითღირებულება აღემატება ფასს";
					Сообщение.КлючДанных = Объект.Ссылка;
					Сообщение.Сообщить();
					Отказ = Истина;//++Lazare 22.12.2022
				КонецЕсли;
			КонецЦикла;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВалютаАрендыКвартир) Тогда
		Объект.ВалютаАрендыКвартир = УправлениеНебольшойФирмойПовтИсп.ПолучитьНациональнуюВалюту(); 
	КонецЕсли;
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентПередИзменением <> Объект.Контрагент Тогда
		...
		ЗаполнениеДанныхПоРасчетуАрендыКвартир();// Kuzmich, 29.03.2024, (ONESOFT-2527) Оплата аренды квартиры и обработка банковских выписок
...
Процедура ОбновитьСуммуНДСДоставки()
	...
	РассчитатьСуммуКОплате(); //Малышев 2024-10-14 ONESOFT-3508
	ОбновитьПодвалФормы();
...
Процедура КартаЛояльностиПриИзменении(Элемент)
	...
	//	//Объект.ПроцентСкидкиПоКартеЛояльности = УправлениеНебольшойФирмойСервер.ВычислитьПроцентСкидкиПоКартеЛояльности(Объект.Ссылка, ДатаДокумента, Объект.КартаЛояльности);
	//	//Объект.ПроцентСкидкиПоКартеЛояльности = ПолучитьПроцентСкидкиЛояльности(); 
	//Иначе
	//	КоэффициентЛояльности = 0;
	//	Объект.ПроцентСкидкиПоКартеЛояльности = 0;
	//КонецЕсли;
	
	СброситьФлагСкидкиРассчитаныКлиент("ПерезаполнениеПоДаннымФормыЦеныИВалюта");
	
	РассчитатьПроцентыСкидокИПересчитатьСтроки();
	
	
	//Малышев 2024-10-29 ONESOFT-3544 -
	//Если ЗначениеЗаполнено(объект.КартаЛояльности) Тогда 
	//	Объект.ПроцентСкидкиПоКартеЛояльности = ПолучитьПроцентСкидкиЛояльности(); 
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент)
	...
	//Малышев 2024-11-05 ONESOFT-3574
	СтрокаТабличнойЧасти.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
	////+++ Дмгг
	//Если ЗначениеЗаполнено(Объект.КартаЛояльности) Тогда   
	...
	Если ЗначениеЗаполнено(Объект.КартаЛояльности) Тогда 
		Для Каждого СтрокаЗапаса из Объект.Запасы Цикл 
			СтрокаЗапаса.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
	...
	СтрокаТабличнойЧасти.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, СтрокаТабличнойЧасти.Номенклатура,СтрокаТабличнойЧасти.Характеристика,СтрокаТабличнойЧасти.Партия,Объект.Организация, Объект.СтруктурнаяЕдиница);//++Lazare 13.12.2022
	// Олег Литвиненко <---
...
&НаКлиенте
Процедура РассчитатьПроцентыСкидокИПересчитатьСтроки()
...
Процедура ЗапасыПроцентСкидкаЛояльностиПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
	//Малышев 2024-10-29 ONESOFT-3544 - закоментровал чтобы скидки могли работать одновременно
	...
	Иначе
		...
		Если Объект.ЦенаВключаетНалоги = ПредопределенноеЗначение("Перечисление.ВидВлключениеНалоговВЦену.ЦенаБезНалогов") Тогда 
			...
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ВидНалогаНДФЛ) Тогда
				//+++ Дмгг, 24.04.25, ONESOFT-4029
				//СуммаНДФЛ = (СтрокаТабличнойЧасти.Сумма - УдержаниеПенсии) * 0.2;
				СтавкаНДФЛ = ПолуччитьСтавкуИзВидаНалогаНДФЛ(СтрокаТабличнойЧасти.ВидНалогаНДФЛ);
				СуммаНДФЛ = (СтрокаТабличнойЧасти.Сумма - УдержаниеПенсии) * (СтавкаНДФЛ/100);
				//--- Дмгг, 24.04.25, ONESOFT-4029
			Иначе
				СуммаНДФЛ = 0;
			КонецЕсли;
		...
		ИначеЕсли Объект.ЦенаВключаетНалоги = ПредопределенноеЗначение("Перечисление.ВидВлключениеНалоговВЦену.ЦенаВключаетНалоги") Тогда 
			...
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ВидНалогаНДФЛ) Тогда
				...
				СтавкаНДФЛ = ПолуччитьСтавкуИзВидаНалогаНДФЛ(СтрокаТабличнойЧасти.ВидНалогаНДФЛ);
				СтрокаТабличнойЧасти.СуммаНДФЛ = (СтрокаТабличнойЧасти.Сумма - СтрокаТабличнойЧасти.УдержаниеПенсии) * (СтавкаНДФЛ/100);
				//--- Дмгг, 24.04.25, ONESOFT-4029
			КонецЕсли;
...
&НаСервере
Функция РассчитатьРезервы()
	
	Если Константы.ОбновлениеРезервовПоСкладу.Получить() Тогда 
		...
		Для Каждого Запас Из Объект.Запасы Цикл
			...
			Запрос.Текст = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	РезервированиеЗапасовЗапасы.Количество КАК Количество,
			|	РезервированиеЗапасов.Ссылка КАК Ссылка
			|ИЗ
			|	Документ.РезервированиеЗапасов.Запасы КАК РезервированиеЗапасовЗапасы
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РезервированиеЗапасов КАК РезервированиеЗапасов
			|		ПО РезервированиеЗапасовЗапасы.Ссылка = РезервированиеЗапасов.Ссылка
			|ГДЕ
			|	РезервированиеЗапасов.ДокументОснование = &ЗаказПокупателя
			|	И РезервированиеЗапасовЗапасы.Номенклатура.Ссылка = &Номенклатура
			|	И РезервированиеЗапасовЗапасы.НовоеМестоРезерва = &РасходнаяНакладнаяСтруктурнаяЕдиница";
			
			Запрос.УстановитьПараметр("ЗаказПокупателя", ЗаказПокупателя);
			Запрос.УстановитьПараметр("Номенклатура", Запас.Номенклатура);
			Запрос.УстановитьПараметр("РасходнаяНакладнаяСтруктурнаяЕдиница", Объект.СтруктурнаяЕдиница);
			...
			Объект.Запасы[Индекс].Резерв = Итог;
			Индекс = Индекс + 1;
		КонецЦикла;
	КонецЕсли;                                    
КонецФункции
...
Процедура ПоискПоШтрихкодуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ((ВариантРаспознаванияШтрихкода = ПредопределенноеЗначение("Перечисление.ВариантыРаспознованияШтрихкодов.Вес") И СтрДлина(ТекШтрихкод) = 13)
				ИЛИ ВариантРаспознаванияШтрихкода = ПредопределенноеЗначение("Перечисление.ВариантыРаспознованияШтрихкодов.Цена"))
				И Лев(ТекШтрихкод,3) = "990" 
				ИЛИ Лев(ТекШтрихкод,3) = "200" ИЛИ Лев(ТекШтрихкод,3) = "220" Тогда
...
Функция ПолучитьДанныеОрганизацияПриИзменении()
	...
	СтруктураДанные.Вставить("ВидНалогаНДС", Объект.Организация.ВидНалогаНДС);
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные) Экспорт
	...
	Возврат Документы.РасходнаяНакладная.ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные);
...
Процедура ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС(ЭтоОткрытие = Ложь)
	...
	//Малышев 2024-12-19
	Если ЗначениеЗаполнено(Объект.Организация) и не ЗначениеЗаполнено(Объект.ВидНалогаНДС) Тогда
		Объект.ВидНалогаНДС = Объект.Организация.ВидНалогаНДС;
	КонецЕсли;	
...
Процедура РассчитатьСуммуНДС(СтрокаТабличнойЧасти)
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.БезвозмезднаяПродажаСотруднику") Тогда 
		...
		//Малышев 2024-10-28 ONESOFT-3556								  
		СтрокаТабличнойЧасти.СуммаНДСБаллов = СтрокаТабличнойЧасти.БалыРасход * СтавкаНДС / (100+СтавкаНДС); 
	Иначе 
		Если Объект.ЦенаВключаетНалоги = ПредопределенноеЗначение("Перечисление.ВидВлключениеНалоговВЦену.ЦенаБезНалогов") Тогда 
	...
	Объект.ВсегоИспользованоБаллов = Объект.Запасы.Итог("БалыРасход"); //2024-11-05 ONESOFT-3574
	Объект.СуммаНДСБаллов = Объект.Запасы.Итог("СуммаНДСБаллов"); 

	РассчитатьСуммуКОплате(); //Малышев 2024-11-06 ONESOFT-3575
	
КонецПроцедуры // РассчитатьСуммуНДС()

&НаСервере
Процедура ПосчитатьСуммуСтрокиСоСкидками(ИдСтроки)             
	//СкидкиНаценкиСервер.ПосчитатьСуммуСтрокиСоСкидками(СтрокаТабличнойЧасти);
КонецПроцедуры
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти = Неопределено, СброситьФлагСкидкиРассчитаны = Истина) Экспорт 
	...
	Если ЗначениеЗаполнено(объект.КартаЛояльности) Тогда 
		Объект.ПроцентСкидкиПоКартеЛояльности = ПолучитьПроцентСкидкиЛояльности();
		Объект.ПроцентНакопленияБалов = ПолучитьПроцентНакопленияБаллов();
		Элементы.КартаЛояльности.Подсказка = "ПроцентСкидкиПоКартеЛояльности=" + Объект.ПроцентСкидкиПоКартеЛояльности + "
		|ПроцентНакопленияБалов =" + Объект.ПроцентНакопленияБалов;
	Иначе
		Объект.ПроцентСкидкиПоКартеЛояльности = 0;
		Объект.ПроцентНакопленияБалов = 0;
		Элементы.КартаЛояльности.Подсказка = "";
	КонецЕсли;
	Для каждого СтрДокумента из Объект.Запасы Цикл
		СтрДокумента.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
	КонецЦикла;	
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.БезвозмезднаяПродажаСотруднику") Тогда 
		...
		СкидкиНаценкиКлиентСервер.ПосчитатьСуммуСтрокиСоСкидками(СтрокаТабличнойЧасти);		
		...
		//		И НЕ СтрокаТабличнойЧасти.Количество = 0 Тогда
		//		
		//		//Малышев 2025-01-06 ONESOFT-3748
		...
		//		СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма * (1 - (СтрокаТабличнойЧасти.ПроцентАвтоматическойСкидки + СтрокаТабличнойЧасти.ПроцентСкидкиНаценки) / 100);
		//		
		...
		//	
		////++Lazare 13.12.2022
		//ИначеЕсли СтрокаТабличнойЧасти.ПроцентСкидкиПосСрокуГодности <> 0 Тогда 				
		//	
		//	Если СтрокаТабличнойЧасти.ПроцентСкидкиПосСрокуГодности = 100 Тогда				
		//		СтрокаТабличнойЧасти.Сумма = 0;	
		//	ИначеЕсли НЕ СтрокаТабличнойЧасти.Количество = 0 Тогда		
		//		СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма * (1 - СтрокаТабличнойЧасти.ПроцентСкидкиПосСрокуГодности / 100) * (1 - СтрокаТабличнойЧасти.ПроцентАвтоматическойСкидки / 100) * (1 - СтрокаТабличнойЧасти.ПроцентСкидкиНаценки / 100);				
		//	КонецЕсли;	
		////--Lazare 13.12.2022	
		//
		//Иначе
		...
		//		И НЕ СтрокаТабличнойЧасти.Количество = 0 Тогда
		//		
		//		//Малышев 2025-01-07 ONESOFT-3749
		//		СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма * (1 - (СтрокаТабличнойЧасти.ПроцентСкидкиНаценки+СтрокаТабличнойЧасти.ПроцентСкидкаЛояльности)/ 100); 
		//		////+++ Дмгг
		//		////СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма * (1 - СтрокаТабличнойЧасти.ПроцентСкидкиНаценки / 100);
		//		////Если СтрокаТабличнойЧасти.ПроцентСкидкаЛояльности = 0 Тогда 
		//		//Если СтрокаТабличнойЧасти.ПроцентСкидкиНаценки<>0 Тогда  //Малышев 2024-12-25
		//		//	СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма * (1 - СтрокаТабличнойЧасти.ПроцентСкидкиНаценки / 100);
		//		//Иначе
	...
	Иначе 
		...
		Если Объект.ЦенаВключаетНалоги = ПредопределенноеЗначение("Перечисление.ВидВлключениеНалоговВЦену.ЦенаБезНалогов") Тогда 
			...
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ВидНалогаНДФЛ) Тогда 
				...
				СтавкаНДФЛ = ПолуччитьСтавкуИзВидаНалогаНДФЛ(СтрокаТабличнойЧасти.ВидНалогаНДФЛ);
				СтрокаТабличнойЧасти.СуммаНДФЛ = (Сумма + СтрокаТабличнойЧасти.СуммаНДС) * (1 / (1- (СтавкаНДФЛ/100))) - (Сумма + СтрокаТабличнойЧасти.СуммаНДС);
				//--- Дмгг, 24.04.25, ONESOFT-4029
			Иначе 
...
Процедура ОбработатьИзмененияПоКнопкеЦеныИВалюты(Знач ВалютаРасчетовПередИзменением, ПерезаполнитьЦены = Ложь, ПересчитатьЦены = Ложь, ТекстПредупреждения = "")
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю") Тогда
		...
		СтруктураПараметров.Вставить("ПерезаполнитьЦены",	 ПерезаполнитьЦены или ЗначениеЗаполнено(Объект.ДисконтнаяКарта));	//Малышев 2025-02-05 ONESOFT-3821
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ОбновитьПодвалФормы(); //+++ Дмгг, 08.01.24, ONESOFT-2842
...
Процедура УстановитьВидимостьСотрудниковвТабЧастиЗапасы()
	...
	Элементы.ЭтоПодарок.Видимость 						= ЭтоПродажа;
	// Олег Литвиненко <---
...
Процедура ОбработатьИзменениеУсловийДоговора(ДанныеДоговора, ДоговорПередИзменением)
	...
	//Малышев 2025-02-05 ONESOFT-3821	
	Если ЗначениеЗаполнено(Объект.ВидЦен) и Объект.Запасы.НайтиСтроки(новый Структура("Цена",0)).Количество()>0 Тогда
		ОткрытьФормуЦеныИВалюты = истина;
		ИзменилсяВидЦен = Истина;
	КонецЕсли;		
...
Процедура ОбновитьПодвалФормы()
	...
	КолвоДоступныхБалов = ПолучитьКоличествоДоступныхБалов(Объект.Ссылка, Объект.КартаЛояльности, Объект.Контрагент, Объект.Дата, КоэффициентЛояльности);
	//Малышев 2024-11-05 ONESOFT-3574
	//Элементы.ДоступноБалов.Заголовок = НСтр("ru = 'Доступно балов: '; ka = 'ხელმისაწვდომი ქულები: '; en = 'Доступно балов: '") + (КолвоДоступныхБалов - Объект.Запасы.Итог("БалыРасход"));
	ДоступноБалов = КолвоДоступныхБалов - Объект.Запасы.Итог("БалыРасход");
	...
	//+++ Дмгг
	//КоличествоНакопленныхБаллов = (Объект.Запасы.Итог("Сумма") - (Объект.Запасы.Итог("БалыРасход") * КоэффициентЛояльности)) * (Объект.ПроцентНакопленияБалов / 100); //+++ Дмгг
	//Элементы.НакопленоБалов.Заголовок = НСтр("ru = 'Накопленные баллы: '; ka = 'აგროვებული ქულა: '; en = 'Accumulated points: '") + КоличествоНакопленныхБаллов; //+++ Дмгг
	
	//ИтогДоступноБалов = ?(КоэффициентЛояльности = 0, 0, КоличествоНакопленныхБаллов * КоэффициентЛояльности);     
	ИтогДоступноБалов = ?(КоэффициентЛояльности = 0, 0, Объект.КоличествоНакопленныхБаллов * КоэффициентЛояльности); //Малышев 2024-11-12    
	
	Если Объект.ВидИспользоыванияБалов <> ПредопределенноеЗначение("Перечисление.ВидыИспользованияБалов.Расход") Тогда 
		//ИтогБалов = Объект.ДоступноБалов - Объект.Запасы.Итог("БалыРасход") + КоличествоНакопленныхБаллов;       
		//ИтогБалов = КолвоДоступныхБалов - Объект.Запасы.Итог("БалыРасход") + КоличествоНакопленныхБаллов;       
		ИтогБалов = КолвоДоступныхБалов - Объект.Запасы.Итог("БалыРасход") + Объект.КоличествоНакопленныхБаллов;  //Малышев 2024-11-12     
		
	Иначе
		//ИтогБалов = Объект.ДоступноБалов - Объект.Запасы.Итог("БалыРасход"); 
		ИтогБалов = КолвоДоступныхБалов - Объект.Запасы.Итог("БалыРасход"); 
	КонецЕсли;
	
	ИтогБалов = Окр(ИтогБалов, 2);
	//--- Дмгг
	
	РассчитатьСуммуАрендыЗаКвартиру();// Kuzmich, 29.03.2024, (ONESOFT-2527) Оплата аренды квартиры и обработка банковских выписок
	
	//ОбновитьСуммунакопленныхБаллов(СуммаНДСНакопленныхБаллов = 
...
Процедура РассчитатьОбъемИВесВДокументе()
	...
	Объект.СуммаНДСБаллов = Объект.Запасы.Итог("СуммаНДСБаллов"); //Малышев 2024-11-05 ONESOFT-3574
...
//+++ Дмгг
&НаСервере
Функция ПолучитьКоличествоДоступныхБалов(ДокСсылка = Неопределено, КартаЛояльности, Контрагент, ДатаДокумента, КоэффициентЛояльности)
	
	//КоэффициентЛояльности = КартаЛояльности.Владелец.КоэфициентПересчета;
	КоэффициентЛояльности = БаллыЛояльностьСкидки.ПолучитьКоэффициентЛояльности(Объект.КартаЛояльности, Объект.Дата); //Малышев 2024-12-05
	
	//Малышев 2024-10-11 ONESOFT-3509
	Результат = БаллыЛояльностьСкидки.ПолучитьКоличествоДоступныхБалов_РасходнаяНакладная(ДокСсылка, КартаЛояльности, Контрагент, ДатаДокумента, КоэффициентЛояльности);
	Возврат Результат;
...
&НаСервере 
Процедура ЗапретРедактированияРасходныхНакладныхПоСтатусеТТН()
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияРасходныхНакладныхПоСтатусеТТН") Тогда    	
		Если ОбщегоНазначения.ЕстьТТН(Объект.Ссылка) Тогда
			Элементы.Шапка.Доступность = Ложь;
			Элементы.Страницы.Доступность = Ложь;
			Элементы.Подвал.Доступность = Ложь;
			КоманднаяПанель.ПодчиненныеЭлементы.ФормаЗаписать.Доступность = Ложь;      
			КоманднаяПанель.ПодчиненныеЭлементы.ФормаОтменаПроведения.Доступность = Ложь;
            КоманднаяПанель.ПодчиненныеЭлементы.ФормаСтандартныеКоманды.Доступность = Ложь;   
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
//+++ Дмгг, ONESOFT-2840
&НаКлиенте
Процедура ЗапасыПодборНовый(Команда)
...
Процедура ПолучитьЗапасыИзХранилища(АдресЗапасовВХранилище, ИмяТабличнойЧасти, ЕстьХарактеристики, ЕстьПартии)
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ИмяТабличнойЧасти = "Запасы" Тогда 
			НоваяСтрока.Себестоимость = ПолучитьСебестоимостьНоменклатуры(НоваяСтрока.Номенклатура , НоваяСтрока.Характеристика);
		КонецЕсли;
		//--- Дмгг
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	// Олег Литвиненко 21.04.23 --->
	Если ЗапасыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьНаправлениеДеятельности") Тогда
		Элементы.ЗапасыИзменениеСтрокЗначение.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.НаправленияДеятельности");
	КонецЕсли;	
	// Олег Литвиненко <---
	// Конец ГрупповоеИзменениеСтрок
	...
	Элементы.ЗапасыКоличествоНаСкладе.Видимость =  УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалВидимостиСвободногоОстаткаЗапасовВВокументах");//++Lazare 13.12.2022
	Элементы.РассчитатьСкидкиПоСрокуГодностиТовара.Видимость = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("КонтрольСроковГодностиТовара");//++Lazare 13.12.2022
	
	УправлениеЭлементамиДляРасчетАрендыКвартир();// Kuzmich, 29.03.2024, (ONESOFT-2527) Оплата аренды квартиры и обработка банковских выписок
...
//+++ Дмгг
&НаСервере
Процедура УстановитьВидимостьИспользованияБалов()
	...
	//Малышев 2024-10-28 ONESOFT-3556
	Элементы.ЗапасыСуммаНДСБаллов.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности) и Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС; 
	Элементы.СуммаНДСБаллов.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности) и Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС;
	Элементы.СуммаНДСЛояльности.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности) и Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС;
	Элементы.СуммаНДСНакопленныхБаллов.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности) и Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС;
	
	Элементы.ГруппаОплатаИтого.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности); //Малышев 2024-12-09 
	Элементы.ОбщаяСуммаКОплате.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности);
...
Процедура РассчитатьСкидкиНаценки(Команда)
	//Малышев 2024-10-29 ONESOFT-3544 - закоментировал
...
Процедура РассчитатьСкидкиНаценкиПоЗаказуСервер()
	...
	Для Каждого ТекущаяСтрокаДокумента Из ОбъектЗапасы Цикл
		...
		ТекущаяСтрокаДокумента.ОсталосьРаспределить = Окр(ТекущаяСтрокаДокумента.Количество *
			?(ТипЗнч(ТекущаяСтрокаДокумента.ЕдиницаИзмерения) = Тип("СправочникСсылка.КлассификаторЕдиницИзмерения"), 1, ТекущаяСтрокаДокумента.ЕдиницаИзмерения.Коэффициент) - СмещениеДляОкругления, 3);
		// Олег Литвиненко <---
	КонецЦикла;
	...
	Для Каждого ТекущаяСтрокаЗаказа Из СкидкиНаценкиЗаказа Цикл
		...
		// Олег Литвиненко 18.10.22 --->
		КоличествоВЗаказе = Окр(ТекущаяСтрокаЗаказа.Количество * ТекущаяСтрокаЗаказа.Коэффициент - СмещениеДляОкругления, 3);
		// Олег Литвиненко <---
		Распределено = 0;
...
Функция СкидкиИзменились()
	...
	СтруктураПараметры.Вставить("ТолькоПредварительныйРасчет",      Ложь);
	СтруктураПараметры.Вставить("ВидЦен",Объект.ВидЦен);// Kuzmich, 05.09.2023,(ONESOFT-2458) ომატურ ფასდაკლებაში ფასის ტიპის დამატება
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//Закомментировал Олег Литвиненко 02.11.22
	//Перенес в ЗапасыЦенаНачалоВыбора
	//+++ Lazare_21.12.2021		
...
Процедура ОбработатьТаблицу()
	...
	Для каждого Строка Из ИзменяемыеСтроки Цикл
		...
		// Олег Литвиненко 21.04.23 --->
		ИначеЕсли ЗапасыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьНаправлениеДеятельности") Тогда
			
			Строка.НаправлениеДеятельности = ЗапасыИзменениеСтрокЗначение;
...
Процедура ОпределитьОбъектИзменений()
	...
	ИначеЕсли ЗапасыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьНаправлениеДеятельности") Тогда
		...
		ЗапасыИзменениеСтрокОбъектИзмененийРеквизит = "НаправлениеДеятельности";
		ЗапасыИзменениеСтрокОбъектИзмененийЭлемент = "ЗапасыНаправлениеДеятельности";
	// Олег Литвиненко <---
...
Процедура ЗаполнитьСписокДействий()
	...
	// Олег Литвиненко 21.04.23 --->
	Действия.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.ИзменитьНаправлениеДеятельности);
	// Олег Литвиненко <---
...
Процедура ОткрытьПодборСерийныеНомера()
	...
	ПараметрыСерийныхНомеров.Вставить("СтруктурнаяЕдиница", Объект.СтруктурнаяЕдиница);//Малышев 2024-08-30 ONESOFT-3417
...
Функция РазобратьШтрихкод(Штрихкод, ВариантРаспознаванияШтрихкода)
	...
	Если Лев(Штрихкод, 3) = "200" ИЛИ Лев(Штрихкод, 3) = "220" Тогда  
		...
		|	И ПОДСТРОКА(ШтрихкодыНоменклатуры.Штрихкод, 4, 4) = &Штрихкод";
...
Процедура ЗаполнитьСчетаБезвозмезднойПродажи()
	...
	БезвозмезднаяПродажа.ЗаполнитьСчетаБезвозмезднойПродажи(Объект);
	// Олег Литвиненко <---
...
Процедура ВключитьЭтоПодарок(Команда)
	...
	// Олег Литвиненко 21.09.22 --->
	Объект.ЭтоПодарок = Истина;
	// Олег Литвиненко <---	
...
Процедура ЗапасыСотрудникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Сотрудники.ФормаВыбора",Пар,ЭтаФорма);
КонецПроцедуры
//_Lazare
...
Процедура ПересчитатьПолеКоличествоЗапасовНаСкладеВТчЗапасы(СтруктурнаяЕдиница);
	...
	Для Каждого Ст Из Объект.Запасы Цикл 	
		  //Ст.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата,Ст.Номенклатура,Организация,СтруктурнаяЕдиница);
		  Ст.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата,Ст.Номенклатура,Ст.Характеристика,Ст.Партия,Объект.Организация,СтруктурнаяЕдиница); //++Lazare 13.12.2022
	КонецЦикла;
...
// Олег Литвиненко 20.09.22 --->
&НаКлиенте
Процедура ЭтоПодарокПриИзменении(Элемент)
	
	Для Каждого ТекСтр Из Объект.Запасы Цикл
		ТекСтр.ЭтоПодарок = Объект.ЭтоПодарок;			
	КонецЦикла;
...
// Олег Литвиненко 20.09.22 --->
&НаКлиенте
Процедура ЗапасыЭтоПодарокПриИзменении(Элемент)
	
	Установлено = Истина;
	
	Для Каждого ТекСтр Из Объект.Запасы Цикл
		Если Не ТекСтр.ЭтоПодарок Тогда
			Установлено = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Объект.ЭтоПодарок = Установлено;
		
КонецПроцедуры// Олег Литвиненко <---

// Олег Литвиненко 20.09.22 --->
&НаСервере
Процедура УстановитьВидимостьЭтоПодарок()
	
	СрезУчетнаяПолитикаРегл = РегистрыСведений.УчетнаяПолитикаРегл.СрезПоследних(ТекущаяДата(), Новый Структура("Организация", Объект.Организация));
	Если СрезУчетнаяПолитикаРегл.Количество() Тогда
		
		ИспользоватьПодарокВТабличнойЧасти = СрезУчетнаяПолитикаРегл[0].ИспользоватьПодарокВТабличнойЧасти;
		
		Элементы.ЭтоПодарок.Видимость = Не ИспользоватьПодарокВТабличнойЧасти;
		Элементы.Группа4.Видимость = ИспользоватьПодарокВТабличнойЧасти;
		Элементы.ЗаполнитьВсеНаЭтоПодарок.Видимость = ИспользоватьПодарокВТабличнойЧасти;
		Элементы.ЗапасыЭтоПодарок.Видимость = ИспользоватьПодарокВТабличнойЧасти;
		
	КонецЕсли;

КонецПроцедуры// Олег Литвиненко <---

// Олег Литвиненко 02.11.22 --->
&НаКлиенте
Процедура ЗапасыЦенаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	//+++ Lazare_21.12.2021		
	Если ПравоИзмененияЦен() Тогда 
		 ПравоИзмененияЦены =   ПравоИзмененияЦеныНоменклатурыВРасходнойНакл(Элемент.ТекущиеДанные.Номенклатура);
		Если ПравоИзмененияЦены  Тогда 
...
&НаСервере
Процедура УстановитьФорматаВКоличестве()
	
	ЧетыреЗнаковПослеЗапятойВКоличестве = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ЧетыреЗнаковПослеЗапятойВКоличествеВРасхНаклад");	
	Если ЧетыреЗнаковПослеЗапятойВКоличестве Тогда 
		Элементы.ЗапасыКоличество.Формат = "ЧДЦ=4";
		Элементы.ЗапасыКоличество.ФорматРедактирования = "ЧДЦ=4";			
	Иначе 
		Элементы.ЗапасыКоличество.Формат = "ЧДЦ=3";
		Элементы.ЗапасыКоличество.ФорматРедактирования = "ЧДЦ=3";	
	КонецЕсли;
		
КонецПроцедуры


#область Рассчитать_Скидки_По_Сроку_Годности_товара

&НаКлиенте
Процедура РассчитатьСкидкиПоСрокуГодностиТовара(Команда)
	
	РассчитатьСкидкиПоСрокуГодностиТовараНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура РассчитатьСкидкиПоСрокуГодностиТовараНаСервере()	
	
	СрокиГодности = СформироватьТаблицаКонтрольСроковГодностиТовара();	
	
КонецПроцедуры
 
 &НаСервере
 Функция СформироватьТаблицаКонтрольСроковГодностиТовара()
	...
	КопияТаблицаЗапасы.Колонки.Добавить("Номенклатура",Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	КопияТаблицаЗапасы.Колонки.Добавить("НоменклатураСсылка",Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	КопияТаблицаЗапасы.Колонки.Добавить("Характеристика",Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	КопияТаблицаЗапасы.Колонки.Добавить("Количество",Новый ОписаниеТипов("Число"));
	КопияТаблицаЗапасы.Колонки.Добавить("Период",Новый ОписаниеТипов("Дата"));
	КопияТаблицаЗапасы.Колонки.Добавить("СрокГодности",Новый ОписаниеТипов("Дата"));
	...
	"ВЫБРАТЬ
	|	ТзЗапасы.Номенклатура КАК Номенклатура,
	//|	ТзЗапасы.Номенклатура.Ссылка КАК НоменклатураСсылка,
	|	ТзЗапасы.Характеристика КАК Характеристика,
	|	&Период КАК Период,
	...
	|	ТзЗапасы.Номенклатура КАК Номенклатура,
	|	СУММА(ТзЗапасы.Количество) КАК Количество,
	...
	|	ТзЗапасы.Организация КАК Организация,
	|	ТзЗапасы.Период КАК Период,
	...
	|						ТзЗапасы.Характеристика КАК Характеристика
	|					ИЗ
	|						ТзЗапасы КАК ТзЗапасы)) КАК КонтрольСроковГодностиТовараОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТзЗапасы КАК ТзЗапасы
	|		ПО КонтрольСроковГодностиТовараОстатки.Организация = ТзЗапасы.Организация
	|			И КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница = ТзЗапасы.СтруктурнаяЕдиница
	|			И КонтрольСроковГодностиТовараОстатки.Номенклатура = ТзЗапасы.Номенклатура
	|			И КонтрольСроковГодностиТовараОстатки.Характеристика = ТзЗапасы.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТВТ.Номенклатура КАК Номенклатура,
	|	СУММА(ВТВТ.Количество) КАК Количество,
	|	ВТВТ.Документ КАК Документ,
	|	СУММА(ВТВТ.КоличествоОстаток) КАК КоличествоОстаток,
	|	ВТВТ.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВТВТ.Организация КАК Организация,
	|	ВТВТ.Период КАК Период,
	|	ВТВТ.СрокГодности КАК СрокГодности
	|ИЗ
	|	ВТВТ КАК ВТВТ
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТВТ.Организация,
	|	ВТВТ.СтруктурнаяЕдиница,
	|	ВТВТ.Документ,
	|	ВТВТ.Номенклатура,
	|	ВТВТ.Период,
	|	ВТВТ.СрокГодности
	...
	Запрос.УстановитьПараметр("Дата",Объект.Дата-60);
	...
	Если КопияТаблицаЗапасы.количество() > 0 Тогда 
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТзСрокиГодности.Номенклатура КАК Номенклатура,
		|	ТзСрокиГодности.Характеристика КАК Характеристика,
		|	ТзСрокиГодности.Количество КАК Количество,
		|	ТзСрокиГодности.СрокГодности КАК СрокГодности,
		|	РАЗНОСТЬДАТ(ВЫРАЗИТЬ(&ТекДата КАК ДАТА), ТзСрокиГодности.СрокГодности, ДЕНЬ) КАК ДниДоИстеченияСрока
		|ПОМЕСТИТЬ ТовариИСрокиГодности
		|ИЗ
		|	&ТзСрокиГодности КАК ТзСрокиГодности
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ ПЕРВЫЕ 1
		|	СкидкиПоСрокуГодностиТовара.Ссылка КАК Ссылка,
		|	СкидкиПоСрокуГодностиТовара.ТипСравнения КАК ТипСравнения,
		|	СкидкиПоСрокуГодностиТовара.ЗначениеУсловияОграничения КАК ЗначениеУсловияОграничения,
		|	СкидкиПоСрокуГодностиТовара.ОбластьОграничения КАК ОбластьОграничения,
		|	СкидкиПоСрокуГодностиТовара.КритерийОграничения КАК КритерийОграничения,
		|	СкидкиПоСрокуГодностиТовара.СпособПредоставления КАК СпособПредоставления,
		|	СкидкиПоСрокуГодностиТовара.ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки,
		|	СкидкиПоСрокуГодностиТовара.ЕстьУточненияПоНоменклатуре КАК ЕстьУточненияПоНоменклатуре
		|ПОМЕСТИТЬ ВременнаяТаблицаСкидки
		|ИЗ
		|	Справочник.СкидкиПоСрокуГодностиТовара КАК СкидкиПоСрокуГодностиТовара
		|ГДЕ
		|	СкидкиПоСрокуГодностиТовара.ПометкаУдаления = ЛОЖЬ
		|	И СкидкиПоСрокуГодностиТовара.Действует = ИСТИНА
		|
		|УПОРЯДОЧИТЬ ПО
		|	СкидкиПоСрокуГодностиТовара.РеквизитДопУпорядочивания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СкидкиПоСрокуГодностиТовараСкиди.Номенклатура КАК Номенклатура,
		|	СкидкиПоСрокуГодностиТовараСкиди.Характеристика КАК Характеристика,
		|	СкидкиПоСрокуГодностиТовараСкиди.Ссылка КАК Ссылка,
		|	СкидкиПоСрокуГодностиТовараСкиди.ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки
		|ПОМЕСТИТЬ ОтборПродажПоНоменклатуре
		|ИЗ
		|	ВременнаяТаблицаСкидки КАК ВременнаяТаблицаСкидки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СкидкиПоСрокуГодностиТовара.Скиди КАК СкидкиПоСрокуГодностиТовараСкиди
		|		ПО ВременнаяТаблицаСкидки.Ссылка = СкидкиПоСрокуГодностиТовараСкиди.Ссылка
		|ГДЕ
		|	СкидкиПоСрокуГодностиТовараСкиди.Ссылка.ЕстьУточненияПоНоменклатуре = ИСТИНА
		|	И ВременнаяТаблицаСкидки.Ссылка.Естьруппа = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	Справочник.СкидкиПоСрокуГодностиТовара.Скиди КАК СкидкиПоСрокуГодностиТовараСкиди
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаСкидки КАК ВременнаяТаблицаСкидки
		|		ПО СкидкиПоСрокуГодностиТовараСкиди.Ссылка = ВременнаяТаблицаСкидки.Ссылка
		|ГДЕ
		|	СкидкиПоСрокуГодностиТовараСкиди.Ссылка.ЕстьУточненияПоНоменклатуре = ИСТИНА
		|	И ВременнаяТаблицаСкидки.Ссылка.Естьруппа = ИСТИНА
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаСкидки.Ссылка КАК Ссылка,
		|	ВременнаяТаблицаСкидки.ТипСравнения КАК ТипСравнения,
		|	ВременнаяТаблицаСкидки.ЗначениеУсловияОграничения КАК ЗначениеУсловияОграничения,
		|	ВременнаяТаблицаСкидки.ОбластьОграничения КАК ОбластьОграничения,
		|	ВременнаяТаблицаСкидки.КритерийОграничения КАК КритерийОграничения,
		|	ВременнаяТаблицаСкидки.СпособПредоставления КАК СпособПредоставления,
		|	ВременнаяТаблицаСкидки.ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки,
		|	ВременнаяТаблицаСкидки.ЕстьУточненияПоНоменклатуре КАК ЕстьУточненияПоНоменклатуре
		|ПОМЕСТИТЬ ПродажБезОтбора
		|ИЗ
		|	ВременнаяТаблицаСкидки КАК ВременнаяТаблицаСкидки
		|ГДЕ
		|	ВременнаяТаблицаСкидки.Ссылка.ЕстьУточненияПоНоменклатуре = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТовариИСрокиГодности.Номенклатура КАК Номенклатура,
		|	ТовариИСрокиГодности.Характеристика КАК Характеристика,
		|	ТовариИСрокиГодности.Количество КАК Количество,
		|	ТовариИСрокиГодности.СрокГодности КАК СрокГодности,
		|	ТовариИСрокиГодности.ДниДоИстеченияСрока КАК ДниДоИстеченияСрока,
		|	ОтборПродажПоНоменклатуре.Ссылка.ЗначениеУсловияОграничения КАК ЗначениеУсловияОграничения,
		|	ОтборПродажПоНоменклатуре.ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки
		|ИЗ
		|	ТовариИСрокиГодности КАК ТовариИСрокиГодности
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтборПродажПоНоменклатуре КАК ОтборПродажПоНоменклатуре
		|		ПО ТовариИСрокиГодности.Номенклатура = ОтборПродажПоНоменклатуре.Номенклатура
		|			И ТовариИСрокиГодности.Характеристика = ОтборПродажПоНоменклатуре.Характеристика
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ОтборПродажПоНоменклатуре.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.БольшеИлиРавно)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока >= ОтборПродажПоНоменклатуре.Ссылка.ЗначениеУсловияОграничения
		|			КОГДА ОтборПродажПоНоменклатуре.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.Больше)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока > ОтборПродажПоНоменклатуре.Ссылка.ЗначениеУсловияОграничения
		|			КОГДА ОтборПродажПоНоменклатуре.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.МеньшеИлиРавно)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока <= ОтборПродажПоНоменклатуре.Ссылка.ЗначениеУсловияОграничения
		|			ИНАЧЕ ТовариИСрокиГодности.ДниДоИстеченияСрока < ОтборПродажПоНоменклатуре.Ссылка.ЗначениеУсловияОграничения
		|		КОНЕЦ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	ТовариИСрокиГодности.СрокГодности КАК СрокГодности,
		|	ТовариИСрокиГодности.ДниДоИстеченияСрока КАК ДниДоИстеченияСрока,
		|	ОтборПродажПоНоменклатуреГруппы.Ссылка.ЗначениеУсловияОграничения КАК ЗначениеУсловияОграничения,
		|	ОтборПродажПоНоменклатуреГруппы.ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки
		|ИЗ
		|	ТовариИСрокиГодности КАК ТовариИСрокиГодности,
		|	ОтборПродажПоНоменклатуреГруппы КАК ОтборПродажПоНоменклатуреГруппы
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ОтборПродажПоНоменклатуреГруппы.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.БольшеИлиРавно)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока >= ОтборПродажПоНоменклатуреГруппы.Ссылка.ЗначениеУсловияОграничения
		|			КОГДА ОтборПродажПоНоменклатуреГруппы.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.Больше)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока > ОтборПродажПоНоменклатуреГруппы.Ссылка.ЗначениеУсловияОграничения
		|			КОГДА ОтборПродажПоНоменклатуреГруппы.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.МеньшеИлиРавно)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока <= ОтборПродажПоНоменклатуреГруппы.Ссылка.ЗначениеУсловияОграничения
		|			ИНАЧЕ ТовариИСрокиГодности.ДниДоИстеченияСрока < ОтборПродажПоНоменклатуреГруппы.Ссылка.ЗначениеУсловияОграничения
		|		КОНЕЦ
		|	И ТовариИСрокиГодности.Номенклатура В ИЕРАРХИИ
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ТовариИСрокиГодности.СрокГодности КАК СрокГодности,
		|	ТовариИСрокиГодности.ДниДоИстеченияСрока КАК ДниДоИстеченияСрока,
		|	ПродажБезОтбора.ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки,
		|	ПродажБезОтбора.Ссылка.ЗначениеУсловияОграничения КАК ЗначениеУсловияОграничения
		|ИЗ
		|	ПродажБезОтбора КАК ПродажБезОтбора,
		|	ТовариИСрокиГодности КАК ТовариИСрокиГодности
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ПродажБезОтбора.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.БольшеИлиРавно)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока >= ПродажБезОтбора.Ссылка.ЗначениеУсловияОграничения
		|			КОГДА ПродажБезОтбора.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.Больше)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока > ПродажБезОтбора.Ссылка.ЗначениеУсловияОграничения
		|			КОГДА ПродажБезОтбора.Ссылка.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.МеньшеИлиРавно)
		|				ТОГДА ТовариИСрокиГодности.ДниДоИстеченияСрока <= ПродажБезОтбора.Ссылка.ЗначениеУсловияОграничения
		|			ИНАЧЕ ТовариИСрокиГодности.ДниДоИстеченияСрока < ПродажБезОтбора.Ссылка.ЗначениеУсловияОграничения
		|		КОНЕЦ";
		
		Запрос.УстановитьПараметр("ТзСрокиГодности",КопияТаблицаЗапасы);
		Запрос.УстановитьПараметр("ТекДата",Объект.Дата);
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		РезултатПоНоменклатуре 		 = МассивРезультатов[5].Выгрузить();
		РезултатПоНоменклатуреГруппы = МассивРезультатов[6].Выгрузить();
		РезултатПродажБезОтбора 	 = МассивРезультатов[7].Выгрузить();
		
		Если РезултатПоНоменклатуре.количество() > 0 Тогда 
			ПерезаполнитьТзЗапасы(ТзЗапасы,РезултатПоНоменклатуре);			
		ИначеЕсли РезултатПоНоменклатуреГруппы.количество() > 0 Тогда
			ПерезаполнитьТзЗапасы(ТзЗапасы,РезултатПоНоменклатуреГруппы);
		ИначеЕсли РезултатПродажБезОтбора.количество() > 0 Тогда
			ПерезаполнитьТзЗапасы(ТзЗапасы,РезултатПродажБезОтбора);
		КонецЕсли;
...
&НаСервере
Процедура ПерезаполнитьТзЗапасы(ТзЗапасы,ВтСкидки)
	
	ЗапасыТЧ =  ТзЗапасы;
	
	ДобавленныеСтрокиТЧ = ЗапасыТЧ.Скопировать();
	ДобавленныеСтрокиТЧ.Очистить();
	СтруктураПоиска = Новый Структура("Номенклатура",);
	МассивСтрокЗамена = Новый Массив;
	Для каждого СтрокаЗапас Из ЗапасыТЧ Цикл
	
			СтруктураПоиска.Номенклатура = СтрокаЗапас.Номенклатура;
			КоличествоРаспределить = СтрокаЗапас.Количество;
			СтрокиСкидки = ВтСкидки.НайтиСтроки(СтруктураПоиска);		
			ПерваяСтрока = Истина;
			
			Для каждого ТекущаяСтрокаСкидки Из СтрокиСкидки Цикл
			
				Если КоличествоРаспределить <= 0 Тогда			
					Прервать;		
				КонецЕсли; 
				
				ТекущееРаспределение = Мин(ТекущаяСтрокаСкидки.Количество, КоличествоРаспределить);
				СтрокаДобавить = ДобавленныеСтрокиТЧ.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаДобавить, СтрокаЗапас, , "Количество,Сумма,СуммаНДС"); 
				СтрокаДобавить.Количество = ТекущееРаспределение;
				СтрокаДобавить.Сумма = СтрокаДобавить.Количество * СтрокаДобавить.Цена;
				СтрокаДобавить.ПроцентСкидкиПосСрокуГодности = ТекущаяСтрокаСкидки.ЗначениеСкидкиНаценки;
						
				Если СтрокаДобавить.ПроцентСкидкиПосСрокуГодности <> 0 Тогда 
					Если СтрокаДобавить.Количество <> 0 Тогда 	
					СтрокаДобавить.Сумма = СтрокаДобавить.Сумма * (1 - СтрокаДобавить.ПроцентСкидкиПосСрокуГодности / 100) ;	
					КонецЕсли;	
				КонецЕсли;	
				
				СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтрокаДобавить.СтавкаНДС);	
				СтрокаДобавить.СуммаНДС = ?(Объект.СуммаВключаетНДС, 
				СтрокаДобавить.Сумма - (СтрокаДобавить.Сумма) / ((СтавкаНДС + 100) / 100),
				СтрокаДобавить.Сумма * СтавкаНДС / 100);	
				СтрокаДобавить.Всего = СтрокаДобавить.Сумма + ?(Объект.СуммаВключаетНДС, 0, СтрокаДобавить.СуммаНДС);

				КоличествоРаспределить = КоличествоРаспределить - ТекущееРаспределение;
				
			КонецЦикла;
									
		КонецЦикла;
		
		Если ДобавленныеСтрокиТЧ.Количество()  Тогда
			
			КоличествоДобавленныхСтрок = 0;
			КоличествоДобавленоПоТекущейСтроке = 0;
			ДобавленныеСтрокиТЧ.Сортировать("НомерСтроки");
			ТекущийНомерТЧ = ДобавленныеСтрокиТЧ[0].НомерСтроки; 
			Для каждого ТекущаяСтрокаДобавить Из ДобавленныеСтрокиТЧ Цикл
				
				Если ТекущийНомерТЧ < ТекущаяСтрокаДобавить.НомерСтроки Тогда			
					КоличествоДобавленоПоТекущейСтроке = 0;
					ТекущийНомерТЧ = ТекущаяСтрокаДобавить.НомерСтроки;			
				КонецЕсли;
				
				Если КоличествоДобавленоПоТекущейСтроке = 0 Тогда						
					СтрокаТЧ = ЗапасыТЧ[ТекущийНомерТЧ - 1 + КоличествоДобавленныхСтрок];
				Иначе				
					КоличествоДобавленныхСтрок = КоличествоДобавленныхСтрок + 1;
					СтрокаТЧ = ЗапасыТЧ.Вставить(ТекущийНомерТЧ - 1 + КоличествоДобавленныхСтрок);		
				КонецЕсли; 
				
				КоличествоДобавленоПоТекущейСтроке = КоличествоДобавленоПоТекущейСтроке + 1;
				ЗаполнитьЗначенияСвойств(СтрокаТЧ, ТекущаяСтрокаДобавить);
				
			КонецЦикла; 
				
		КонецЕсли; 
		
		Объект["Запасы"].Загрузить(ЗапасыТЧ);
...
// Kuzmich, 29.03.2024, (ONESOFT-2527) Оплата аренды квартиры и обработка банковских выписок
#Область РасчетАрендыКвартир

&НаСервере
Процедура УправлениеЭлементамиДляРасчетАрендыКвартир()

	Если ПолучитьФункциональнуюОпцию("РасчетАрендыКвартир") И Объект.АрендаКвартир Тогда
		
		Элементы.ГруппаРасчетАрендыКвартир.Видимость = Объект.АрендаКвартир;
		
		Элементы.ВладелецКвартиры.АвтоОтметкаНезаполненного = Объект.АрендаКвартир;
		Элементы.ДоговорВладельцаКвартиры.АвтоОтметкаНезаполненного = Объект.АрендаКвартир;
		Элементы.ПроцентАрендыКвартиры.АвтоОтметкаНезаполненного = Объект.АрендаКвартир;
		Элементы.НоменклатураАрендыКвартиры.АвтоОтметкаНезаполненного = Объект.АрендаКвартир;
		Элементы.СуммаАрендыЗаКвартиру.АвтоОтметкаНезаполненного = Объект.АрендаКвартир;
		Элементы.ВалютаАрендыКвартир.АвтоОтметкаНезаполненного = Объект.АрендаКвартир;
	Иначе
		
		Элементы.ГруппаРасчетАрендыКвартир.Видимость = ЛОЖЬ;
...
&НаСервере
Процедура ЗаполнениеДанныхПоРасчетуАрендыКвартир(ПерезаполнятьВладельца=ИСТИНА)
	
	Если ПолучитьФункциональнуюОпцию("РасчетАрендыКвартир") И ЗначениеЗаполнено(Объект.Договор) И ПерезаполнятьВладельца Тогда
	
		Объект.АрендаКвартир = Объект.Договор.РасчетАрендыКвартир;
	
	КонецЕсли;
	
	Если Объект.АрендаКвартир Тогда
		
		Если ПерезаполнятьВладельца Тогда
			
			Объект.ВладелецКвартиры = Объект.Договор.ВладелецКвартиры;
			Объект.ДоговорВладельцаКвартиры = Объект.Договор.ДоговорВладельцаКвартиры;
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Объект.ДоговорВладельцаКвартиры) Тогда
		
			Объект.ПроцентАрендыКвартиры = Объект.ДоговорВладельцаКвартиры.ПроцентАрендыКвартиры;
			Объект.НоменклатураАрендыКвартиры = Объект.ДоговорВладельцаКвартиры.НоменклатураАрендыКвартиры;
			Объект.ВалютаАрендыКвартир = Объект.ДоговорВладельцаКвартиры.ВалютаРасчетов;
		...
		Если НЕ ЗначениеЗаполнено(Объект.ВалютаАрендыКвартир) Тогда
			...
			Если ЗначениеЗаполнено(Объект.ДоговорВладельцаКвартиры) Тогда
				Объект.ВалютаАрендыКвартир = Объект.ДоговорВладельцаКвартиры.ВалютаРасчетов;
			Иначе	
				Объект.ВалютаАрендыКвартир = Объект.Договор.ВалютаРасчетов;
			КонецЕсли;
		...
		РассчитатьСуммуАрендыЗаКвартиру();
		
	Иначе
		
		Если ПерезаполнятьВладельца Тогда
			
			Объект.ВладелецКвартиры = НЕОПРЕДЕЛЕНО;
			Объект.ДоговорВладельцаКвартиры = НЕОПРЕДЕЛЕНО;
			Объект.ВалютаАрендыКвартир = НЕОПРЕДЕЛЕНО;
			
		Конецесли;
	
		Объект.ПроцентАрендыКвартиры = 0;
		Объект.НоменклатураАрендыКвартиры = НЕОПРЕДЕЛЕНО;
		Объект.СуммаАрендыЗаКвартиру = 0;
		
	КонецЕсли;

	УправлениеЭлементамиДляРасчетАрендыКвартир();

КонецПроцедуры

&НаСервере
Процедура РассчитатьСуммуАрендыЗаКвартиру()
	
	Если ПолучитьФункциональнуюОпцию("РасчетАрендыКвартир") И ЗначениеЗаполнено(Объект.ПроцентАрендыКвартиры) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяВалютаДокумента) Тогда
		
			ТекущаяВалютаДокумента = УправлениеНебольшойФирмойПовтИсп.ПолучитьНациональнуюВалюту(); 
		
		КонецЕсли;
		
		Если ТекущаяВалютаДокумента = Объект.ВалютаАрендыКвартир Тогда
			Объект.СуммаАрендыЗаКвартиру = ИтогВсего * Объект.ПроцентАрендыКвартиры / 100;
		Иначе
			
			СуммаАрендыЗаКвартиру = ИтогВсего * Объект.ПроцентАрендыКвартиры / 100;
			Объект.СуммаАрендыЗаКвартиру = РаботаСКурсамиВалют.ПересчитатьВВалюту(СуммаАрендыЗаКвартиру,ТекущаяВалютаДокумента,Объект.ВалютаАрендыКвартир,?(ЗначениеЗаполнено(Объект.Дата),Объект.Дата,ТекущаяДата()));	
...
функция ПолучитьПроцентСкидкиЛояльности()
	Возврат БаллыЛояльностьСкидки.ПолучитьПроцентСкидкиЛояльности_РасходнаяНакладная(Объект);
КонецФункции	

&НаСервере
функция ПолучитьПроцентНакопленияБаллов()
	Возврат БаллыЛояльностьСкидки.ПолучитьПроцентНакопленияБаллов_РасходнаяНакладная(Объект);
КонецФункции	

&НаКлиенте
Процедура РассчитатьСуммуКОплате()
	...
	Для Каждого СтрокаЗапасов из Объект.Запасы Цикл 
		СтрокаЗапасов.ПроцентСкидкаЛояльности = Объект.ПроцентСкидкиПоКартеЛояльности;
		//РассчитатьСуммуВСтрокеТабличнойЧасти(Строк);
		Если СтрокаЗапасов.ПроцентАвтоматическойСкидки<>0 Тогда
			СтрокаЗапасов.ПроцентСкидкаЛояльности = 0;
		КонецЕсли;	
		
	КонецЦикла;
	
	КоэффициентЛояльности = БаллыЛояльностьСкидки.ПолучитьКоэффициентЛояльности(Объект.КартаЛояльности, Объект.Дата); //Малышев 2024-12-05
	КартаДействительна = БаллыЛояльностьСкидки.КартаДействительна(Объект.КартаЛояльности, Объект.Дата);
	//Малышев 2024-10-28 ONESOFT-3556 (--
	Для каждого СтрокаДокумента из Объект.Запасы Цикл 
		Если не ЗначениеЗаполнено(Объект.КартаЛояльности) тогда
			 СтрокаДокумента.БалыРасход = 0;
		КонецЕсли;	
		
		
		ВсегоСуммаСтроки = СтрокаДокумента.Всего;
		Если КартаДействительна ТОгда
			//СтрокаДокумента.КоличествоНакопленныхБаллов = (СтрокаДокумента.Сумма - СтрокаДокумента.БалыРасход * КоэффициентЛояльности)*(Объект.ПроцентНакопленияБалов / 100); 
			//СтрокаДокумента.КоличествоНакопленныхБаллов = (СтрокаДокумента.Сумма - СтрокаДокумента.БалыРасход * КоэффициентЛояльности)*(Объект.ПроцентНакопленияБалов / 100); 
			СтрокаДокумента.КоличествоНакопленныхБаллов = (ВсегоСуммаСтроки - СтрокаДокумента.БалыРасход * КоэффициентЛояльности)*(Объект.ПроцентНакопленияБалов / 100); //Малышев 2024-12-24
			
		Иначе
			СтрокаДокумента.КоличествоНакопленныхБаллов = 0;
		КонецЕсли;	
		
		СтрокаДокумента.СуммаНДСНакопленныхБаллов = СтрокаДокумента.КоличествоНакопленныхБаллов * 18/118;
		//СтрокаДокумента.СуммаНДСЛояльности = (СтрокаДокумента.Всего - СтрокаДокумента.КоличествоНакопленныхБаллов - СтрокаДокумента.БалыРасход)*18/118;
		СтрокаДокумента.СуммаНДСЛояльности = (ВсегоСуммаСтроки - СтрокаДокумента.КоличествоНакопленныхБаллов - СтрокаДокумента.БалыРасход)*18/118;
		
		
		//СуммаНДСЛояльности = СуммаНДСЛояльности + СуммаНДСЛояльностиСтроки; 
	КонецЦикла;
	Объект.СуммаНДСЛояльности =	Объект.Запасы.Итог("СуммаНДСЛояльности");
	Объект.СуммаНДСНакопленныхБаллов =	Объект.Запасы.Итог("СуммаНДСНакопленныхБаллов");
	Объект.КоличествоНакопленныхБаллов =	Объект.Запасы.Итог("КоличествоНакопленныхБаллов");
	
	//Малышев 2024-10-28 ONESOFT-3556 --)                                

	СуммаДокумента = Объект.Запасы.Итог("Всего") + Объект.СтоимостьДоставки + ?(Объект.СуммаВключаетНДС, 0, Объект.СуммаНДСДоставки);	
	Объект.ВсегоИспользованоБаллов	= Объект.Запасы.Итог("БалыРасход");
	Объект.ОбщаяСуммаКОплате = СуммаДокумента - Объект.ВсегоИспользованоБаллов;   
	
КонецПроцедуры

&НаКлиенте
Процедура ПроцентАрендыКвартирыПриИзменении(Элемент)
	РассчитатьСуммуАрендыЗаКвартиру();
КонецПроцедуры

&НаСервере
Процедура УстановитьСвязиПараметровВыбораДляДоговораВладельцаКвартиры()

	Если ЗначениеЗаполнено(Объект.ВладелецКвартиры) Тогда
		
		ЭлементСвязиПараметровВыбора = Новый СвязьПараметраВыбора("Отбор.Владелец", "Объект.ВладелецКвартиры", РежимИзмененияСвязанногоЗначения.Очищать);
		
		МассивСвязиПараметровВыбора = Новый Массив();
		МассивСвязиПараметровВыбора.Добавить(ЭлементСвязиПараметровВыбора);
		
		Элементы.ДоговорВладельцаКвартиры.СвязиПараметровВыбора = Новый ФиксированныйМассив(МассивСвязиПараметровВыбора);
	
	Иначе
		
		Элементы.ДоговорВладельцаКвартиры.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив());
...
&НаКлиенте
Процедура ВладелецКвартирыПриИзменении(Элемент)
	
	УстановитьСвязиПараметровВыбораДляДоговораВладельцаКвартиры();
	
КонецПроцедуры

&НаКлиенте
Процедура ВладелецКвартирыОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДоговорВладельцаКвартирыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаСервере
Процедура ДоговорВладельцаКвартирыПриИзмененииНаСервере()
	
	Если ЗначениеЗаполнено(Объект.ДоговорВладельцаКвартиры) Тогда
		
		Объект.ВладелецКвартиры = Объект.ДоговорВладельцаКвартиры.Владелец;
		Объект.ВалютаАрендыКвартир = Объект.ДоговорВладельцаКвартиры.ВалютаРасчетов;
		ЗаполнениеДанныхПоРасчетуАрендыКвартир(ЛОЖЬ);
...
&НаКлиенте
Процедура ДоговорВладельцаКвартирыПриИзменении(Элемент)
	
	ДоговорВладельцаКвартирыПриИзмененииНаСервере();
	
КонецПроцедуры

&НаКлиенте
Процедура ВалютаАрендыКвартирПриИзменении(Элемент)
...
&НаКлиенте
Процедура АрендаКвартирПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЗапасыБалыРасходПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПоказатьГруппуБаллов(Команда)
	Элементы.ЗапасыГруппаДляБаллов.Видимость = не Элементы.ЗапасыГруппаДляБаллов.Видимость;
КонецПроцедуры
...
//+++ Дмгг, 24.04.25, ONESOFT-4029
&НаСервере
Функция ПолуччитьСтавкуИзВидаНалогаНДФЛ(ВидНалогаНДФЛ);
	           
	Возврат ВидНалогаНДФЛ.СтавкаНДФЛ;	

КонецФункции //ПолуччитьСтавкуИзВидаНалогаНДФЛ ()
//--- Дмгг, 24.04.25, ONESOFT-4029
