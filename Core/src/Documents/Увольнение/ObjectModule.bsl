...
Процедура ВыполнитьКонтроль(ДополнительныеСвойства, Отказ) 
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		ТекстСообщения = НСтр("ka='ცხრილ ""თანამშრომლების"" სტრიქონ №%Номер% თანამშრომლისთვის  %Сотрудник% არსებობს საკადრო მოძრაობები %Период% გათავისუფლების თარიღის შემდეგ.';ru='В строке №%Номер% табл. части ""Сотрудники"" по сотруднику %Сотрудник% есть кадровые движения %Период% после даты увольнения.';en='In line №% number% Table. part of the ""Employees"" on the employee% Employee% have staffing movements Period%% after the date of dismissal.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сотрудник%", ВыборкаИзРезультатаЗапроса.Сотрудник); 
