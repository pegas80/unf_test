		...
		Если РеквизитыСценария.ПланироватьКоличество Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВсегоКоличество) Тогда
				...
				ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка",
					"Заполнение",
					НСтр("ka='რაოდენობა';ru='Количество'"),
					СтрокаТаблицы.НомерСтроки,
					Элементы.СтраницаПлан.Заголовок
				);
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ЕдиницаИзмерения) Тогда
				...
				ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка",
					"Заполнение",
					НСтр("ka='საზომი ერთეული';ru='Единица измерения'"),
					СтрокаТаблицы.НомерСтроки,
					Элементы.СтраницаПлан.Заголовок
				);
...
"გავაგრძელოთ?'");
		...
		ПараметрыВопроса.Заголовок = НСтр("ka='სცენარის ცვლილების დადასტურება';ru='Подтверждение изменения сценария'");
...
Процедура ЗаполнитьПоПланируемымПродажам(Команда)
	...
	ОткрытьНастройкиЗаполненияПоСхеме("СКД_ПланПродаж", НСтр("ka='დაგეგმილი გაყიდვების მიხედვით შევსების გამართვები';ru='Настройки заполнения по запланированным продажам'"));
...
Процедура ЗаполнитьПоФактическимПродажам(Команда)
	...
	ОткрытьНастройкиЗаполненияПоСхеме("СКД_ФактПродаж", НСтр("ka='ფაქტობრივი გაყიდვების მიხედვით შევსების გამართვები';ru='Настройки заполнения по фактическим продажам'"));
...
Процедура ПланВыполнитьДействие(Команда)
	...
	Если ПланИзменениеСтрокДействие <> ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента")
		...
		ТекстСообщения = НСтр("ka='არჩეული სტრიქონები არ არის.';ru='Нет выбранных строк.'");
...
Процедура УстановитьУсловноеОформлениеХарактеристикНоменклатуры(Форма,
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст",					НСтр("ka='<характеристики не используются>';ru='<характеристики не используются>'"));
...
Процедура ОбновитьСтруктуруТаблицыПланирования()
	...
	Для Индекс = 0 По Периоды.Количество()-1 Цикл
		...
		Если РеквизитыСценария.ПланироватьКоличество Тогда
			...
			РеквизитФормы = Новый РеквизитФормы("КолонкаКоличество_" + Индекс, Число_15_3, "ТаблицаПланирования", НСтр("ka='რაოდენობა';ru='Количество'"), Истина);
			ДобавляемыеРеквизиты.Добавить(РеквизитФормы);
			
		КонецЕсли;
		
		РеквизитФормы = Новый РеквизитФормы("КолонкаСумма_" + Индекс, Число_15_2, "ТаблицаПланирования", НСтр("ka='თანხა';ru='Сумма'"), Истина);
		ДобавляемыеРеквизиты.Добавить(РеквизитФормы);
...
"გავაგრძელოთ?'");
			ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
			ПараметрыВопроса.Заголовок = НСтр("ka='დაგეგმვის პერიოდის ცვლილების დადასტურება';ru='Подтверждение изменения периода планирования'");
			ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, ПараметрыВопроса);
...
Процедура ИзменитьПометки(Форма, Пометка)
	...
	Форма.Элементы.ПланИзменитьПометки.Заголовок	= ?(Пометка, НСтр("ka='ნიშნულების მოხსნა';ru='Снять пометки'"), НСтр("ka='ნიშნულების დაყენება';ru='Установить пометки'"));
...
Функция ПредставлениеДействияГрупповогоИзменения(ДействиеПеречисление)
	...
	Если ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьЦеныНаПроцент") Тогда
		Возврат НСтр("ka='თანხების შეცვლა პროცენტზე';ru='Изменить суммы на процент'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		Возврат НСтр("ka='რაოდენობის დადგენა ყველა პერიოდისთვის';ru='Установить количество для всех периодов'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
		Возврат НСтр("ka='ჯამის დაყენება ყველა პერიოდისთვის';ru='Установить сумму для всех периодов'");
	КонецЕсли;
...
Функция ПредставлениеЗаголовкаОбъектаДействия(ДействиеПеречисление)
	...
	Если ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		Возврат НСтр("ka='რაოდენობა';ru='Количество'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
		Возврат НСтр("ka='თანხა';ru='Сумма'");
	КонецЕсли;
...
Процедура УправлениеВидимостьюГрупповогоИзменения(Форма)
	...
	Иначе
		...
		Элементы.ПланИзменениеСтрокЗначение.Заголовок = НСтр("ka='მნიშვნელობა';ru='Значение'");
