...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
	Запрос.УстановитьПараметр("КорректировкаДолга", НСтр("ka='ვალის კორექტირება';ru='Корректировка долга'"));
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВзаимозачет);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Взаимозачет", НСтр("ka='ურთიერთჩათვლა';ru='Взаимозачет'"));
	Запрос.УстановитьПараметр("ПереуступкаДолга", НСтр("ka='ვალის გადაპირება';ru='Переуступка долга'"));
	Запрос.УстановитьПараметр("КорректировкаДолга", НСтр("ka='ვალის კორექტირება';ru='Корректировка долга'"));
	...
	Запрос.УстановитьПараметр("Комментарий", НСтр("ka='ურთიერთჩათვლა';ru='Взаимозачет'"));
...
"Остаток задолженности покупателя: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
				...
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
					Если ВыборкаИзРезультатаЗапроса.СуммаНепогашенныхАвансов = 0 Тогда
						ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - შესაძლებელია, მყიდველისგან ავანსები არ ყოფილა ან იგი უკვე სრულად ჩათვლილია სასაქონლო დოკუმენტებში.';ru='%ПредставлениеКонтрагента% - возможно, авансов от покупателя не было или они уже полностью зачтены в товарных документах.'"
						);
					Иначе
...
"Остаток задолженности перед поставщиком: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
				...
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
					Если ВыборкаИзРезультатаЗапроса.СуммаНепогашенныхАвансов = 0 Тогда
						ТекстСообщения = НСтр("ka='%ПредставлениеКонтрагента% - შესაძლებელია, მომწოდებლისთვის ავანსები არ ყოფილა ან იგი უკვე სრულად ჩათვლილია სასაქონლო დოკუმენტებში.';ru='%ПредставлениеКонтрагента% - возможно, авансов поставщику не было или они уже полностью зачтены в товарных документах.'"
						);
					Иначе
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ТипДокумента)
	...
	Пока Шапка.Следующий() Цикл
		...
		СтрокаКредиторки = НСтр("ka='1. %ПредставлениеКредитора%-ის წინაშე %ПредставлениеОрганизации%-ის დავალიანება შეადგენს';ru='1. Задолженность %ПредставлениеОрганизации% перед %ПредставлениеКредитора% составляет'")+ " ";
		...
		СтрокаШапки = НСтр("ka='%Дата%-ის ურთიერთჩათვლის აქტი № %Номер%';ru='Акт взаимозачета № %Номер% от %Дата%'"); 
		...
		СтрокаКредиторки  = СтрокаКредиторки + Формат(КредиторскаяЗадолженность.Итог(КолонкаСуммы), "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты + " " +НСтр("ka='შემდეგი ხელშეკრულებების მიხედვით:';ru='по следующим договорам:'");
		...
		СтрокаВзаимозачета = НСтр("ka='3. ურთიერთჩათვლა ხორციელდება შემდეგ თანხაზე';ru='3. Взаимозачет производится на сумму'")+ " " + Формат(ДебиторскаяЗадолженность.Итог(КолонкаСуммы), "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='ურთიეთჩათვლის აქტი';ru='Акт взаимозачета'");
