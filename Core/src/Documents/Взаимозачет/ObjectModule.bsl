...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации = Перечисления.ВидыОперацийВзаимозачет.КорректировкаДолгаПоставщику Тогда
		...
		//КонецЕсли;
	КонецЕсли;
	Если ВидОперации = Перечисления.ВидыОперацийВзаимозачет.КорректировкаДолгаПокупателя Тогда
		ВозвратСуммаУчета();
	ИначеЕсли 
		 ВидОперации = Перечисления.ВидыОперацийВзаимозачет.Взаимозачет Тогда
		ВозвратСуммаУчета();
	КонецЕсли;
...
&НаСервере
Процедура ВозвратСуммаУчета()
	Сумма = 0;
	для Каждого стр из Дебитор Цикл 
	    Сумма = Сумма  +  стр.СуммаУчета;
	КонецЦикла;
	СуммаУчета = Сумма;  
КонецПроцедуры
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	УправлениеНебольшойФирмойСервер.ОтразитьРасчетыПоНалогам(ДополнительныеСвойства, Движения, Отказ);//Lazare 20.02.2024
	УправлениеНебольшойФирмойСервер.ОтразитьРасчетыСПерсоналом(ДополнительныеСвойства, Движения, Отказ);//Lazare 20.02.2024
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ОтразитьРасчетыСПенсионнымФондом(ДополнительныеСвойства, Движения, Отказ);//Lazare 20.02.2024
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ОтразитьДекларация(ДополнительныеСвойства, Движения, Отказ);//Lazare 20.02.2024
	УправлениеНебольшойФирмойСервер.ОтразитьРасчетыНДФЛ(ДополнительныеСвойства, Движения, Отказ);//Lazare 20.02.2024
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	//Малышев 2025-10-31 ONESOFT-4460 - закоментировал
	// Контроль возникновения отрицательного остатка.
	//Документы.Взаимозачет.ВыполнитьКонтроль(Ссылка, ДополнительныеСвойства, Отказ, Истина);
