...
Процедура ПодборРасчетыСПокупателем(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонтрагентИсточник) Тогда
		ПоказатьПредупреждение(, НСтр("ka='მიუთითეთ დასაწყისში მომხმარებელი!';ru='Укажите вначале покупателя!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодборРасчетыСПоставщиком(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(, НСтр("ka='მიუთითეთ დასაწყისში მომწოდებელი!';ru='Укажите вначале поставщика!'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоДокументуОснованию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Сообщение.Текст			= НСтр("ka='აირჩიეთ დოკუმენტი საფუძველი.';ru='Выберите документ основание.'");
	...
	ТекстВопроса 	= НСтр("ka='დოკუმენტი გასუფთავდება და შეივსება საფუძველ-დოკუმენტის მიხედვით. გავაგრძელოთ?';ru='Документ будит очищен и заполнен по документу-основанию. Продолжить?'");
	Ответ = Неопределено;
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.РасчетыСДебитором.Заголовок = НСтр("ka='ანგარიშსწორებები მყიდველთან';ru='Расчеты с покупателем'");
	Элементы.РасчетыСКредитором.Заголовок = НСтр("ka='ანგარიშსწორებები მომწოდებელთან';ru='Расчеты с поставщиком'");
	...
	Элементы.КредиторДокумент.Заголовок = НСтр("ka='დოკუმენტი';ru='Документ'");
	Элементы.ДебиторДокумент.Заголовок = НСтр("ka='დოკუმენტი';ru='Документ'");
...
Процедура УстановитьВидимостьИДоступностьДляВидаПрочегоВзаимозачета()
	...
	Элементы.Контрагент.Заголовок = ?(Объект.ВидПрочегоВзаимозачета = ПредопределенноеЗначение("Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту"),
		НСтр("ka='მყიდველი';ru='Покупатель'"), НСтр("ka='კონტრაგენტი';ru='Контрагент'"));
	Элементы.КонтрагентПолучатель.Заголовок = ?(Объект.ВидПрочегоВзаимозачета = ПредопределенноеЗначение("Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту"),
		НСтр("ka='კონტრაგენტი';ru='Контрагент'"),
		НСтр("ka='მომწოდებელი';ru='Поставщик'"));
	...
	Элементы.РасчетыСДебитором.Заголовок = ?(Объект.ВидПрочегоВзаимозачета = ПредопределенноеЗначение("Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту"),
		НСтр("ka='ანგარიშსწორებები მყიდველთან';ru='Расчеты с покупателем'"), НСтр("ka='Расчеты с прочим контрагентом';ru='Расчеты с прочим контрагентом'"));
	Элементы.РасчетыСКредитором.Заголовок = ?(Объект.ВидПрочегоВзаимозачета = ПредопределенноеЗначение("Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту"),
		НСтр("ka='Расчеты с прочим контрагентом';ru='Расчеты с прочим контрагентом'"),
		НСтр("ka='ანგარიშსწორებები მომწოდებელთან';ru='Расчеты с поставщиком'"));
	...
	Если Объект.ВидПрочегоВзаимозачета = ПредопределенноеЗначение("Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту") Тогда
		...
		Элементы.ДебиторДокумент.Заголовок = НСтр("ka='დოკუმენტი';ru='Документ'");
		Элементы.КредиторДокумент.Заголовок = ?(ПолучитьФункциональнуюОпцию("РаботаВМоделиСервиса"), НСтр("ka='Документ (для обмена с Бухгалтерией)';ru='Документ (для обмена с Бухгалтерией)'"), НСтр("ka='Документ (справочно)';ru='Документ (справочно)'"));
	...
	Иначе
		...
		Элементы.ДебиторДокумент.Заголовок = ?(ПолучитьФункциональнуюОпцию("РаботаВМоделиСервиса"), НСтр("ka='Документ (для обмена с Бухгалтерией)';ru='Документ (для обмена с Бухгалтерией)'"), НСтр("ka='Документ (справочно)';ru='Документ (справочно)'"));
		Элементы.КредиторДокумент.Заголовок = НСтр("ka='დოკუმენტი';ru='Документ'");
