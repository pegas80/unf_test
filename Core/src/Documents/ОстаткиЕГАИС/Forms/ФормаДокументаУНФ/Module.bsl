...
Процедура ЗаполнитьПоРасхождениям(Команда)
	...
	Если Объект.СтатусОбработки <> ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОстатковЕГАИС.ПолученыОстатки") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для заполнения по расхождениям требуется получение остатков из ЕГАИС.';ru='Для заполнения по расхождениям требуется получение остатков из ЕГАИС.';en='To fill the need for discrepancies resulting residue from EGAIS.'"));
		Возврат;
	КонецЕсли;
