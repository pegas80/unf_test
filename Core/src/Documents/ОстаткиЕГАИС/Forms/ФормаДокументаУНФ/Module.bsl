...
Процедура ЗаполнитьПоРасхождениям(Команда)
	...
	Если Объект.СтатусОбработки <> ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОстатковЕГАИС.ПолученыОстатки") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для заполнения по расхождениям требуется получение остатков из ЕГАИС.';ru='Для заполнения по расхождениям требуется получение остатков из ЕГАИС.'"));
		Возврат;
	КонецЕсли;
	
	Если Модифицированность ИЛИ НЕ Объект.Проведен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტი არ არის გატარებული.';ru='Документ не проведен.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузкаВЕГАИС_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Запрос на загрузку остатков сформирован.';ru='Запрос на загрузку остатков сформирован.'"));
