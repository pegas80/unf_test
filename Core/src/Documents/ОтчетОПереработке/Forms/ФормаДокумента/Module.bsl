...
Процедура ИзменитьРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Продукция.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ცხრილი ""პროდუქცია"" არ არის შევსებული!';ru='Табличная часть ""Продукция"" не заполнена!';en='A table of the ""Products"" is not full!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОснованиеПечатиНаКлиенте(ДоговорПередИзменением, ДанныеНовогоДоговора)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОснованиеПечатиСсылка)
		...
		Объект.ОснованиеПечати = Нстр("ru='Договор: ';ka='Договор: '") + Строка(ДанныеНовогоДоговора.НаименованиеДоговора);
