...
Процедура ЗаполнитьПоЗаказПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Если ДокументСсылкаЗаказПокупателя.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		ВызватьИсключение НСтр("ka='არ შეიძლება გადამუშავების თაობაზე ანგარიშგების შეყვანა შეკვეთა-განწესის საფუძველზე!';ru='Нельзя ввести Отчет о переработке на основании заказ-наряда!';en='You can not enter report processing based on the purchase order!'");
	КонецЕсли;
