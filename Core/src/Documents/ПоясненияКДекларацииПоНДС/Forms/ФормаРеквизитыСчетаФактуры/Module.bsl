...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='Реквизиты счета-фактуры (строка декларации %1)';ru='Реквизиты счета-фактуры (строка декларации %1)'"),
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если СтандартныйСчетФактура Тогда 
		Если Не ЗначениеЗаполнено(НомерСчетаФактуры) Тогда 
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='ანგარიშფაქტურის ნომერი';ru='Номер счета-фактуры'"));
			Поле = "НомерСчетаФактуры";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
		КонецЕсли;
			
		Если Не ЗначениеЗаполнено(ДатаСчетаФактуры) Тогда 
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='ანგარიშფაქტურის თარიღი';ru='Дата счета-фактуры'"));
		...
		Если Исправление Тогда 
			Если Не ЗначениеЗаполнено(НомерИсправленияСчетаФактуры) Тогда 
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='Номер исправления счета-фактуры';ru='Номер исправления счета-фактуры'"));
				Поле = "НомерИсправленияСчетаФактуры";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);		
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ДатаИсправленияСчетаФактуры) Тогда 
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='Дата исправления счета-фактуры';ru='Дата исправления счета-фактуры'"));
	...
	Иначе
		Если Не ЗначениеЗаполнено(НомерКорректировочногоСчетаФактуры) Тогда 
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='საკორექტირებო ანგარიშფაქტურის ნომერი';ru='Номер корректировочного счета-фактуры'"));
			Поле = "НомерКорректировочногоСчетаФактуры";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);		
		КонецЕсли;
			
		Если Не ЗначениеЗаполнено(ДатаКорректировочногоСчетаФактуры) Тогда 
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='Дата корректировочного счета-фактуры';ru='Дата корректировочного счета-фактуры'"));
		...
		Если ИсправлениеКорректировочногоСчетаФактуры Тогда 
			Если Не ЗначениеЗаполнено(НомерИсправленияКорректировочногоСчетаФактуры) Тогда 
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='Номер исправления корректировочного счета-фактуры';ru='Номер исправления корректировочного счета-фактуры'"));
				Поле = "НомерИсправленияКорректировочного";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);		
			КонецЕсли;
				
			Если Не ЗначениеЗаполнено(ДатаИсправленияКорректировочногоСчетаФактуры) Тогда 
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='Дата исправления корректировочного счета-фактуры';ru='Дата исправления корректировочного счета-фактуры'"));
		...
		Если Не ЗначениеЗаполнено(НомерСчетаФактуры) Тогда 
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='კორექტირებადი ანგარიშფაქტურის ნომერი';ru='Номер корректируемого счета-фактуры'"));
			Поле = "НомерИсходногоДокумента";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);				
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДатаСчетаФактуры) Тогда 
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='კორექტირებადი ანგარიშფაქტურის თარიღი';ru='Дата корректируемого счета-фактуры'"));
