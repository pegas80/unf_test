...
Процедура УправлениеДекларацией(Форма)
	...
	Иначе
		ПредставлениеДекларации = НСтр("ka='Декларация не найдена';ru='Декларация не найдена'");
	КонецЕсли;
...
Процедура УстановитьПредставлениеСчетаФактуры(СтрокаТаблицы, ТипРеквизитов)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы["НомерСчетаФактуры"+ТипРеквизитов]) Тогда
		Если ЗначениеЗаполнено(СтрокаТаблицы["НомерИсправленияСчетаФактуры"+ТипРеквизитов]) Тогда
			РеквизитыСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ka='%1 от %2(испр.%3 от %4)';ru='%1 от %2(испр.%3 от %4)'"),
	...
	СтрокаТаблицы["РеквизитыСчетаФактуры"+ТипРеквизитов] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ka='%1%2';ru='%1%2'"), РеквизитыКоррСчетаФактуры, РеквизитыСчетаФактуры);
...
Процедура ОткрытьФормуСчетаФактуры(ИмяТабличнойЧасти, ДанныеСтроки, ТипРеквизитов)
	...
	Если ПоискВыполнен Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ka='Счет-фактура не найден';ru='Счет-фактура не найден'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличныхЧастейПередОтправкой(Отказ)
	...
	Если Объект.КнигаПокупок.Количество() = 0
		...
		ТекстСообщения = НСтр("ka='Пояснение не заполнено. Отправка невозможна.';ru='Пояснение не заполнено. Отправка невозможна.'");
...
Процедура ПроверитьНетКнигаПрод(Отказ)
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.НетКнигаПрод Цикл
		...
		ПроверитьИНН(СтрокаТабличнойЧасти, "ПокупательИНН", НСтр("ka='ИНН покупателя (по данным ФНС)';ru='ИНН покупателя (по данным ФНС)'"), Отказ);
		ПроверитьКПП(СтрокаТабличнойЧасти, "ПокупательКПП", НСтр("ka='КПП покупателя (по данным ФНС)';ru='КПП покупателя (по данным ФНС)'"), Отказ);
		
		Если СтрокаТабличнойЧасти.РезультатПроверки = Перечисления.РезультатПроверкиСтрокиДекларации.ВыявленыРасхождения Тогда
			
			ПроверитьИНН(СтрокаТабличнойЧасти, "ПокупательИННРасхождение", НСтр("ka='ИНН покупателя (уточненные данные)';ru='ИНН покупателя (уточненные данные)'"), Отказ);
			ПроверитьКПП(СтрокаТабличнойЧасти, "ПокупательКППРасхождение", НСтр("ka='КПП покупателя (уточненные данные)';ru='КПП покупателя (уточненные данные)'"), Отказ);
			
			ПроверитьИНН(СтрокаТабличнойЧасти, "ПосредникИНН", НСтр("ka='ИНН посредника (по данным ФНС)';ru='ИНН посредника (по данным ФНС)'"), Отказ);
			ПроверитьКПП(СтрокаТабличнойЧасти, "ПосредникКПП", НСтр("ka='КПП посредника (по данным ФНС)';ru='КПП посредника (по данным ФНС)'"), Отказ);
			
			ПроверитьИНН(СтрокаТабличнойЧасти, "ПосредникИННРасхождение", НСтр("ka='ИНН посредника (уточненные данные)';ru='ИНН посредника (уточненные данные)'"), Отказ);
			ПроверитьКПП(СтрокаТабличнойЧасти, "ПосредникКППРасхождение", НСтр("ka='КПП посредника (уточненные данные)';ru='КПП посредника (уточненные данные)'"), Отказ);
...
Процедура ПроверитьИНН(СтрокаТабличнойЧасти, ИмяПоля, НаименованиеПоля, Отказ)
	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если ЕстьОшибка Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Нет в книге продаж"" %2 заполнен некорректно:';ru='В строке %1 раздела ""Нет в книге продаж"" %2 заполнен некорректно:'");
...
Процедура ПроверитьКПП(СтрокаТабличнойЧасти, ИмяПоляКПП, НаименованиеПоляКПП, Отказ)
	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если НЕ ЭтоЮридическоеЛицо И ЗначениеЗаполнено(КПП) Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Нет в книге продаж"" обнаружена ошибка: у ИП не может быть заполнен КПП';ru='В строке %1 раздела ""Нет в книге продаж"" обнаружена ошибка: у ИП не может быть заполнен КПП'");
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		...
		Если ЕстьОшибка Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Нет в книге продаж"" %2 заполнено некорректно:';ru='В строке %1 раздела ""Нет в книге продаж"" %2 заполнено некорректно:'");
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
		Если СтрокаТабличнойЧасти.РезультатПроверки = Перечисления.РезультатПроверкиСтрокиДекларации.ВыявленыРасхождения Тогда
			...
			Если РасхожденийНеНайдено Тогда
				ТекстСообщения = НСтр("ka='В строке %1 %2 указан ответ ""Найдены расхождения"", но уточненные сведения соответствуют данным ФНС';ru='В строке %1 %2 указан ответ ""Найдены расхождения"", но уточненные сведения соответствуют данным ФНС'");
...
Процедура ПроверитьКонтрольныеСоотношения(Отказ)
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.СведКС Цикл 
		...
		Если НЕ ЗначениеЗаполнено(НомерКС) Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Контрольные соотношения"" не заполнен номер контрольного соотношения';ru='В строке %1 раздела ""Контрольные соотношения"" не заполнен номер контрольного соотношения'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаТабличнойЧасти.НомерСтроки);
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ПоясненКС) Тогда
			...
			ТекстСообщения = НСтр("ka='В строке %1 раздела ""Контрольные соотношения"" не заполнено пояснение расхождений';ru='В строке %1 раздела ""Контрольные соотношения"" не заполнено пояснение расхождений'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаТабличнойЧасти.НомерСтроки);
...
Функция ИдДекларации(АдресДанных, ВыбранноеИмяФайла)
	...
	Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиДекларацияНДС(СвойстваФайла.Имя) Тогда
		...
		Если НРег(СвойстваФайла.Расширение) = НРег(".xml") Тогда
			...
			Исключение
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Файл ""%1"" не является файлом выгрузки декларации по НДС в формате XML.';ru='Файл ""%1"" не является файлом выгрузки декларации по НДС в формате XML.'"), СвойстваФайла.Имя);
			...
			Если КНД = Неопределено ИЛИ КНД = "" ИЛИ КНД <> "1151001" Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Выбранный файл не является файлом декларации по НДС';ru='Выбранный файл не является файлом декларации по НДС'"));
				Возврат Неопределено;
			КонецЕсли;
			...
			Если НЕ ДокументооборотСКОВызовСервера.ИспользуетсяРежимТестирования() Тогда
				...
				Если ОрганизацияВФайле <> Объект.Организация Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании';ru='Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании'"));
					Возврат Неопределено;
				КонецЕсли;
				...
				Если ВТребованииЕстьXMLФайл Тогда
					...
					Если НомКорр <> СвойстваДекларации.ТребованиеНомКорр Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании';ru='Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании'"));
						Возврат Неопределено;
					КонецЕсли;
					
					Период = ПоказателиВыгрузки["Период"];
					Если Период <> СвойстваДекларации.ТребованиеПериод Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании';ru='Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании'"));
						Возврат Неопределено;
					КонецЕсли;
					
					ОтчетГод = ПоказателиВыгрузки["ОтчетГод"];
					Если ОтчетГод <> СвойстваДекларации.ТребованиеОтчетГод Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании';ru='Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании'"));
						Возврат Неопределено;
					КонецЕсли;
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Файл декларации по НДС должен иметь расширение xml';ru='Файл декларации по НДС должен иметь расширение xml'"));
			Возврат Неопределено;
		КонецЕсли;
