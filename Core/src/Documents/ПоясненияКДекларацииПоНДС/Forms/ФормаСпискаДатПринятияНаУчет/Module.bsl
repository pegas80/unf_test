...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого ДатаПринятияНаУчет Из ДатыПринятияНаУчет Цикл
		Если НЕ ЗначениеЗаполнено(ДатаПринятияНаУчет.ДатаПринятияНаУчет) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='Дата принятия на учет';ru='Дата принятия на учет';en='Date of adoption registration'"));
