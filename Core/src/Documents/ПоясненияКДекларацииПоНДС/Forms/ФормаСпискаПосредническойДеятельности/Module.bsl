...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого Элемент Из ПосредническаяДеятельность Цикл
		Если ЗначениеЗаполнено(Элемент.ПродавецИНН) 
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ka='გამყიდველის ИНН';ru='ИНН продавца';en='INN Seller'"), , , НСтр("ka='Длина ИНН должна быть 10 или 12 символов';ru='Длина ИНН должна быть 10 или 12 символов';en='INN length should be 10 or 12 characters'"));
		...
		Если НЕ ЗначениеЗаполнено(Элемент.НомерСчетаФактуры) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='ნომერი с/ф';ru='Номер с/ф';en='Room w / f'"));
		...
		Если НЕ ЗначениеЗаполнено(Элемент.ДатаСчетаФактуры) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='თარიღი с/ф';ru='Дата с/ф';en='S / F date'"));
...
Процедура УстановитьНадписиСтроки(Строка)
	Строка.НадписьСумма = НСтр("ka='თანხა';ru='Сумма';en='Сумма'");
	Строка.НадписьНДС   = НСтр("ka='მ.შ. დღგ';ru='в т.ч. НДС';en='including VAT'");	
КонецПроцедуры
