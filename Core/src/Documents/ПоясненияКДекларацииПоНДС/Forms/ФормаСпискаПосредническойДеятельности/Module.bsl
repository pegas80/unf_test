...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого Элемент Из ПосредническаяДеятельность Цикл
		Если ЗначениеЗаполнено(Элемент.ПродавецИНН) 
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ka='გამყიდველის ИНН';ru='ИНН продавца'"), , , НСтр("ka='Длина ИНН должна быть 10 или 12 символов';ru='Длина ИНН должна быть 10 или 12 символов'"));
		...
		Если НЕ ЗначениеЗаполнено(Элемент.НомерСчетаФактуры) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='ნომერი с/ф';ru='Номер с/ф'"));
		...
		Если НЕ ЗначениеЗаполнено(Элемент.ДатаСчетаФактуры) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='თარიღი с/ф';ru='Дата с/ф'"));
...
Процедура УстановитьНадписиСтроки(Строка)
	Строка.НадписьСумма = НСтр("ka='თანხა';ru='Сумма'");
	Строка.НадписьНДС   = НСтр("ka='მ.შ. დღგ';ru='в т.ч. НДС'");	
КонецПроцедуры
