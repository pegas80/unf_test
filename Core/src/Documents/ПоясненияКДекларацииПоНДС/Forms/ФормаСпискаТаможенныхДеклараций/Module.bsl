...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого ТаможеннаяДекларация Из ТаможенныеДекларации Цикл
		Если НЕ ЗначениеЗаполнено(ТаможеннаяДекларация.Номер) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ka='საბაჟო დეკლარაციის ნომერი';ru='Номер таможенной декларации';en='Number of customs declaration'"));
