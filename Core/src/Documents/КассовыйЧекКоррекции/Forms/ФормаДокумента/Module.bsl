...
Процедура ОповещениеВопросПроведениеПередПечатьюЧека(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
			ПоказатьПредупреждение(,НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა';ru='Не удалось выполнить проведение документа';en='Failed holding a document'"));
		КонецПопытки;
	КонецЕсли;
...
Процедура НапечататьЧекКоррекции(Команда)
	...
	Если Объект.ПробитЧек Тогда
		ТекстСообщения = НСтр("ka='Чек уже пробит на фискальном устройстве!';ru='Чек уже пробит на фискальном устройстве!';en='Check has already broken on the fiscal device!'");
...
Процедура НапечататьЧекКоррекцииЗавершение(РезультатВыполнения, ПараметрыОперации) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='При печати чека коррекции произошла ошибка."
"Чек коррекции не напечатан на фискальном устройстве."
"Дополнительное описание: %ДополнительноеОписание%';ru='При печати чека коррекции произошла ошибка."
"Чек коррекции не напечатан на фискальном устройстве."
