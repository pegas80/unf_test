...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='შეტყობინება მარკირებული საქონლების იმპორტის შესახებ შეიცვალა. ჩავწეროთ?';ru='Уведомление об импорте маркированных товаров было изменено. Записать?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ka='შეტყობინება მარკირებული საქონლის იმპორტის შესახებ არ არის ჩაწერილი. ჩავწეროთ?';ru='Уведомление об импорте маркированных товаров не записано. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура НомераКиЗПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстПредупреждения = НСтр("ka='საქონლიანი სტრიქონი არ არის არჩეული';ru='Не выбрана строка с товаром'");
...
Процедура ОформитьМаркировкуТоваров(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='შეტყობინება მარკირების სისტემის მარკირებული საქონლის იმპორტის შესახებ შეიცვალა. ჩავწეროთ?';ru='Уведомление об импорте маркированных товаров ГИСМ было изменено. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ОформитьМаркировкуТоваровОбработкаНавигационнойСсылкиЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ka='მარკირების სისტემის მარკირებული საქონლის იმპორტის შესახებ შეტყობინება არ არის ჩაწერილი. ჩავწეროთ?';ru='Уведомление об импорте маркированных товаров ГИСМ не записано. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ЗаполнитьНомераКиЗ(Команда)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.НоменклатураКиЗ) Тогда
		ТекстПредупреждения = НСтр("ka='სტრიქონში არ არის შევსებული ნომენკლატურა სსნ';ru='В строке не заполнена номенклатура КиЗ'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	Если ТекущиеДанные.ХарактеристикиИспользуются И Не ЗначениеЗаполнено(ТекущиеДанные.ХарактеристикаКиЗ) Тогда
		ТекстПредупреждения = НСтр("ka='სტრიქონში სსნ-ის მახასიათებელი არ არის შევსებული';ru='В строке не заполнена характеристика КиЗ'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьВидМехаЗавершение(ВидМеха, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(ВидМеха) Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='ბეწვის სახეობა შევსებულია';ru='Вид меха заполнен'"),
			,
			СтрЗаменить(НСтр("ka='გამოყოფილი სტრიქონებისთვის შევსებულია ბეწვის სახეობა ""%ВидМеха%""';ru='Для выделенных строк заполнен вид меха ""%ВидМеха%""'"), "%ВидМеха%", ВидМеха),
	...
	Если ШтрихкодыПоТипам.КиЗ.Количество() > 0 Тогда
		...
		Для Каждого Штрихкод из ШтрихкодыПоТипам.КиЗ Цикл
			...
			Если ТекущаяСтрока.Количество <= СтрокиКиЗ.Количество() Тогда
				ТекстСообщения = НСтр("ka='მიმდინარე სტრიქონისთვის შეყვანილია სსნ-ის ყველა ნომერი.';ru='Для текущей строки введены все номера КиЗ.'");
