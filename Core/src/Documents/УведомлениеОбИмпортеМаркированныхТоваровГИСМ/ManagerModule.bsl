	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ka='სტრიქონ %2-ში ნომენკლატურა %1-ისთვის საგარეო ეკონომიკური საქმიანობის სასაქონლო ნომენკლატურის კოდი არ არის შევსებული.';ru='Не заполнен Код ТНВЭД для номенклатуры %1 в строке %2.';en='Not filled HS Code nomenclature for% 1 at line% 2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.КодВидМеха) Тогда
			...
			Исключение
				ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
					СообщениеXML,
					СтрШаблон(НСтр("ka='Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.';ru='Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.';en='Not correctly filled type of fur for the nomenclature of% 1 at line% 2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
			КонецПопытки;
		КонецЕсли;
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ka='სტრიქონ %2-ში ნომენკლატურა %1-ისთვის ""საქონლის აღწერილობა"" არ არის შევსებული.';ru='Не заполнено Описание товара для номенклатуры %1 в строке %2.';en='Not filled Product description for an item% 1 at line% 2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ka='Не удалось заблокировать документ: %Объект% по причине: %Причина%';ru='Не удалось заблокировать документ: %Объект% по причине: %Причина%';en='Unable to lock the document:% object% due to:% reason%'");
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ka='Не удалось обработать документ: %Объект% по причине: %Причина%';ru='Не удалось обработать документ: %Объект% по причине: %Причина%';en='Unable to process the document:% object% due to:% reason%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Объект%", ВыборкаИтоги.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			                         УровеньЖурналаРегистрации.Предупреждение,
