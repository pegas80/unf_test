	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ka='სტრიქონ %2-ში ნომენკლატურა %1-ისთვის საგარეო ეკონომიკური საქმიანობის სასაქონლო ნომენკლატურის კოდი არ არის შევსებული.';ru='Не заполнен Код ТНВЭД для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.КодВидМеха) Тогда
			...
			Исключение
				ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
					СообщениеXML,
					СтрШаблон(НСтр("ka='Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.';ru='Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
			КонецПопытки;
		КонецЕсли;
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ka='სტრიქონ %2-ში ნომენკლატურა %1-ისთვის ""საქონლის აღწერილობა"" არ არის შევსებული.';ru='Не заполнено Описание товара для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ka='Не удалось заблокировать документ: %Объект% по причине: %Причина%';ru='Не удалось заблокировать документ: %Объект% по причине: %Причина%'");
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ka='Не удалось обработать документ: %Объект% по причине: %Причина%';ru='Не удалось обработать документ: %Объект% по причине: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Объект%", ВыборкаИтоги.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			                         УровеньЖурналаРегистрации.Предупреждение,
