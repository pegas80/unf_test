...
Процедура ПроверитьВыпущенныеНомераКиз(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ka='Номер КиЗ %1 не выпущен в строке %2 списка ""Товары""';ru='Номер КиЗ %1 не выпущен в строке %2 списка ""Товары""'"), Выборка.НомерКиЗ, Выборка.НомерСтроки),
			ЭтотОбъект,
