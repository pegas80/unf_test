...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если (Запасы.Итог("Резерв") > 0 ИЛИ Продукция.Итог("Резерв") > 0)
		...
		ТекстСообщения = НСтр("ka='არ არის მითითებული რეზერვის საწყობი!';ru='Не указан склад резерва!'");
		...
		Если ВидОперации = Перечисления.ВидыОперацийЗаказНаПроизводство.Сборка Тогда
			...
			Для каждого СтрокаЗапасы Из Запасы Цикл
				...
				Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
					...
					ТекстСообщения = НСтр("ka='ცხრილ ""მასალების"" სტრიქონ №%Номер%-ში რეზერვირებადი პოზიციების რაოდენობა აღემატება მასალების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Материалы"" количество резервируемых позиций превышает общее количество материалов.'");
		...
		Иначе
			...
			Для каждого СтрокаПродукция Из Продукция Цикл
				...
				Если СтрокаПродукция.Резерв > СтрокаПродукция.Количество Тогда
					...
					ТекстСообщения = НСтр("ka='ცხრილ ""პროდუქცია""-ს სტრიქონ №%Номер%-ში რეზერვირებადი პოზიციების რაოდენობა აღემატება პროდუქციის საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Продукция"" количество резервируемых позиций превышает общее количество продукции.'");
