...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если (Запасы.Итог("Резерв") > 0 ИЛИ Продукция.Итог("Резерв") > 0)
		...
		ТекстСообщения = НСтр("ka='არ არის მითითებული რეზერვის საწყობი!';ru='Не указан склад резерва!';en='Unknown storage reserve!'");
		...
		Если ВидОперации = Перечисления.ВидыОперацийЗаказНаПроизводство.Сборка Тогда
			...
			Для каждого СтрокаЗапасы Из Запасы Цикл
				...
				Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
					...
					ТекстСообщения = НСтр("ka='ცხრილ ""მასალების"" სტრიქონ №%Номер%-ში რეზერვირებადი პოზიციების რაოდენობა აღემატება მასალების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Материалы"" количество резервируемых позиций превышает общее количество материалов.';en='In line №% number% Table. part of the ""Materials"" number of redundant positions exceeds the total amount of materials.'");
		...
		Иначе
			...
			Для каждого СтрокаПродукция Из Продукция Цикл
				...
				Если СтрокаПродукция.Резерв > СтрокаПродукция.Количество Тогда
					...
					ТекстСообщения = НСтр("ka='ცხრილ ""პროდუქცია""-ს სტრიქონ №%Номер%-ში რეზერვირებადი პოზიციების რაოდენობა აღემატება პროდუქციის საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Продукция"" количество резервируемых позиций превышает общее количество продукции.';en='In line №% number% Table. part of the ""Products"" the number of redundant positions exceeds the total number of products.'");
...
"В табличной части ""Продукция"" указаны только услуги. Необходимо очистить табличную часть ""Материалы"".';ka='მასალებში მოთხოვნილების დაგეგმვა არ ხორციელდება მომსახურებებისთვის! ცხრილ ""პროდუქციაში"" მითითებულია მხოლოდ მომსახურებები. აუცილებელია ცხრილ ""მასალების"" გასუფთავება.'");
...
Процедура ЗаполнитьКолонкуРезервПоОстаткам() Экспорт
	...
	// Kuzmich, 06.11.2024, (ONESOFT-3572) При создании "заказа на производства" с основания документа "заказа покупателя" в заказе на производство не заполняется табличная часть
	...
	// Kuzmich, 06.11.2024, (ONESOFT-3572) При создании "заказа на производства" с основания документа "заказа покупателя" в заказе на производство не заполняется табличная часть
	|	СУММА(ВЫБОР 
	|		КОГДА Константы.ФункциональнаяОпцияРезервированиеЗапасов_new
	|			ТОГДА ЗапасыОстатки.КоличествоОстаток - ЕСТЬNULL(РезервированиеЗапасовОстатки.КоличествоОстаток, 0)
	...
	|						ВЫБОР
	|							КОГДА &ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	|								ТОГДА ТаблицаЗапасы.Номенклатура.СчетУчетаЗатратРегл
	|							ИНАЧЕ ТаблицаЗапасы.Номенклатура.СчетУчетаЗапасовРегл
	|						КОНЕЦ,
	|						ТаблицаЗапасы.Номенклатура,
	|						ТаблицаЗапасы.Характеристика,
	|						ТаблицаЗапасы.Партия,
	...
	|			ПО ЗапасыОстатки.Организация = РезервированиеЗапасовОстатки.Организация
	|			И ЗапасыОстатки.СтруктурнаяЕдиница = РезервированиеЗапасовОстатки.СтруктурнаяЕдиница
	|			И ЗапасыОстатки.Номенклатура = РезервированиеЗапасовОстатки.Номенклатура
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	|	ЗаказПокупателя.Проект КАК Проект,  //Малышев 2024-06-26 ONESOFT-3279
	|	ВЫБОР
	|		КОГДА ФункциональнаяОпцияРезервированиеЗапасов.Значение
	// Kuzmich, 15.10.2024, ONESOFT-3500, Заполнение "заказа покупателя" в шапке документов "перемещение запасов" и "заказ на производство"
	|		ИЛИ ФункциональнаяОпцияРезервированиеЗапасов_new.Значение
	// Kuzmich, 15.10.2024, ONESOFT-3500, Заполнение "заказа покупателя" в шапке документов "перемещение запасов" и "заказ на производство"
	...
	|	Константа.ФункциональнаяОпцияРезервированиеЗапасов КАК ФункциональнаяОпцияРезервированиеЗапасов
	// Kuzmich, 15.10.2024, ONESOFT-3500, Заполнение "заказа покупателя" в шапке документов "перемещение запасов" и "заказ на производство"
	|	,Константа.ФункциональнаяОпцияРезервированиеЗапасов_new КАК ФункциональнаяОпцияРезервированиеЗапасов_new
	// Kuzmich, 15.10.2024, ONESOFT-3500, Заполнение "заказа покупателя" в шапке документов "перемещение запасов" и "заказ на производство"
	|ГДЕ
	...
	|	,ЗаказПокупателяЗапасы.Партия КАК Партия  //+++ Дмгг
	|ИЗ
	...
	|	,ЗаказПокупателяЗапасы.Партия //+++ Дмгг
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	Если ОснованиеВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		...
		|	ЗаказПокупателяМатериалы.Проект КАК Проект, //Малышев 2024-06-26 ONESOFT-3279
		|	ВЫБОР
		...
		|	ЗаказПокупателяМатериалы.Проект, //Малышев 2024-06-26 ONESOFT-3279
