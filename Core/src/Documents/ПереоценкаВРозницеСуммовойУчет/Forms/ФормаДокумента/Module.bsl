...
Процедура УстановитьВозможностьРедактированияСписком()
	...
	Если НЕ Элементы.РедактироватьСписком.Пометка
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("УстановитьВозможностьРедактированияСпискомЗавершение", ЭтотОбъект), 
			НСтр("ka='ყველა სტრიქონი შეიკვეცება. გავაგრძელოთ?';ru='Все строки будут свернуты. Продолжить?';en='All lines will be phased out. Proceed?'"),
...
Процедура СтруктурнаяЕдиницаПриИзменении(Элемент)
	...
	Объект.КорреспонденцияРегл = СтруктураДанные.СчетУчетаНаценкиРегл; //+++ Дмгг
...
Функция ПолучитьДанныеСтруктурнаяЕдиницаПриИзменении(СтруктурнаяЕдиница)
	...
	СтруктураДанные.Вставить("СчетУчетаНаценкиРегл", СтруктурнаяЕдиница.СчетУчетаНаценкиРегл); //+++ Дмгг
