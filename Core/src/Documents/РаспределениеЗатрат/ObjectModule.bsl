...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		ВызватьИсключение НСтр("ka='ხარჯების განაწილების შეყვანა შეუძლებელია შეკვეთა-განწესის საფუძველზე!';ru='Нельзя ввести Распределение затрат на основании заказ-наряда!'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("Количество") <> РаспределениеЗапасов.Итог("Количество") Тогда
		...
		ТекстСообщения = НСтр("ka='მარაგების რაოდენობა არ შეესაბამება განაწილების რაოდენობას!';ru='Количество запасов не соответствует количеству распределения!'");
	...
	Если Затраты.Итог("Сумма") <> РаспределениеЗатрат.Итог("Сумма") Тогда
		...
		ТекстСообщения = НСтр("ka='ხარჯების ოდენობა არ შეესაბამება განაწილების ოდენობას!';ru='Сумма расходов не соответствует сумме распределения!'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() Тогда
		...
		Для каждого СтрокаРаспределениеЗапасов Из РаспределениеЗапасов Цикл
			...
			Если СтрокаРаспределениеЗапасов.Резерв > СтрокаРаспределениеЗапасов.Количество Тогда
				...
				ТекстСообщения = НСтр("ka='ცხრილ ""მარაგების განაწილების"" სტრიქონ №%Номер% რეზერვიდან ჩამოსაწერი პოზიციების რაოდენობა აღემატება მარაგების საერთო რაოდენობას.';ru='В строке №%Номер% табл. части ""Распределение запасов"" количество позиций к списанию из резерва превышает общее количество запасов.'");
