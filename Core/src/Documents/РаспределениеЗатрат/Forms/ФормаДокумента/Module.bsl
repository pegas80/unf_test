...
//+++
Процедура ЗаполнитьТабличнуюЧастьЗатратыПоОстаткамРегл()
	
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.ВыполнитьЗаполнениеРасходовПоОстаткамРегл();
	ЗначениеВРеквизитФормы(Документ, "Объект");
	
КонецПроцедуры // ЗаполнитьТабличнуюЧастьЗатратыПоОстаткамРегл()
//---
...
// Процедура заполняет табличну часть РаспределениеЗатрат.
//
Процедура ЗаполнитьТабличнуюЧастьРаспределениеЗатратПоСебестоимости()
	
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.ВыполнитьРаспределениеЗатратПоСебестоимости();
	ЗначениеВРеквизитФормы(Документ, "Объект");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗаполнитьПоОстаткам.Видимость = НЕ ИспользоватьТолькоРегламентированныйПланСчетов; //+++ Дмгг
...
Процедура ЗапасыЗаполнитьПоНормативам(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი ""მარაგები"" გადაივსება!';ru='Табличная часть ""Запасы"" будет перезаполнена!';en='Tabular part of the ""reserve"" will be refilled!'") + Символы.ПС;
		Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
			ТекстВопроса = ТекстВопроса + НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გასუფთავდება!';ru='Табличная часть ""Распределение запасов"" будет очищена!';en='Tabular part of the ""stock distribution"" will be cleared!'") + Символы.ПС;
		КонецЕсли;	
		ТекстВопроса = ТекстВопроса + НСтр("ka='გავაგრძელოთ ოპერაციის შესრულება?';ru='Продолжить выполнение операции?';en='To continue the operation?'");
		Ответ = Неопределено;
...
Процедура ЗапасыРаспределитьПоНормативам(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоНормативамЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?';en='Tabular part of the ""stock distribution"" will be refilled! To continue the operation?'"), 
...
&НаКлиенте
Процедура ЗапасыРаспределитьПоСебестоимостиЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    ЗапасыРаспределитьПоСебестоимостиФрагмент();
...
Процедура ЗапасыРаспределитьПоКоличеству(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоКоличествуЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?';en='Tabular part of the ""stock distribution"" will be refilled! To continue the operation?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ЗатратыЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Проведен Тогда 
		...
		ТекстВопроса = НСтр("ru='Перед заполнением табличной части ""Затраты"", требуется расспровести документ!';ka='ცხრულური ნაწილის  ""დანახარჯების"" შევსებამდე აუცილებელია გატარების გაუქმება !'") + Символы.ПС;
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыЗаполнитьПоОстаткамЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ОК, 0);
        Возврат;
	...
	Если Объект.Затраты.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი ""დანახარჯები"" გადაივსება!';ru='Табличная часть ""Затраты"" будет перезаполнена!';en='A table of the ""costs"" will be refilled!'") + Символы.ПС;
		ТекстВопроса = ТекстВопроса + НСтр("ka='ცხრილი ""ხარჯების გადანაწილება"" გასუფთავდება!';ru='Табличная часть ""Распределение расходов"" будет очищена!';en='A table of the ""distribution costs"" will be cleared!'") + Символы.ПС;
		ТекстВопроса = ТекстВопроса + НСтр("ka='გავაგრძელოთ ოპერაციის შესრულება?';ru='Продолжить выполнение операции?';en='To continue the operation?'");
		Ответ = Неопределено;
...
//+++
&НаКлиенте
Процедура ЗатратыЗаполнитьПоОстаткамРегл(Команда)
	
	//+++ Дмгг
	Если Объект.Проведен Тогда 
		
		ТекстВопроса = НСтр("ru='Перед заполнением табличной части ""Затраты"", требуется расспровести документ!';ka='ცხრულური ნაწილის  ""დანახარჯების"" შევსებამდე აუცილებელია გატარების გაუქმება !'") + Символы.ПС;
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыЗаполнитьПоОстаткамЗавершениеРегл", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ОК, 0);
        Возврат;
	...
	Если Объект.Затраты.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыЗаполнитьПоОстаткамЗавершениеРегл", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
 
	КонецЕсли;
	
	ЗатратыЗаполнитьПоОстаткамФрагментРегл();
КонецПроцедуры
///---
...
&НаКлиенте
Процедура ЗатратыЗаполнитьПоОстаткамЗавершениеРегл(Результат, ДополнительныеПараметры) Экспорт
...
//+++
&НаКлиенте
Процедура ЗатратыЗаполнитьПоОстаткамФрагментРегл()
    
    ЗаполнитьТабличнуюЧастьЗатратыПоОстаткамРегл();

КонецПроцедуры // ЗатратыЗаполнитьПоОстаткамРегл()
//---
...
Процедура ЗатратыРаспределитьПоКоличеству(Команда)
	...
	Если Объект.РаспределениеЗатрат.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыРаспределитьПоКоличествуЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""ხარჯების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение расходов"" будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""distribution costs"" will be refilled! To continue the operation?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыРаспределитьПоСебестоимости(Команда)
	...
	Если Объект.РаспределениеЗатрат.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыРаспределитьПоСебестоимостиЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""ხარჯების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение расходов"" будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""distribution costs"" will be refilled! To continue the operation?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
 
	КонецЕсли;
	
	ЗатратыРаспределитьПоСебестоимостиФрагмент();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗатратыРаспределитьПоСебестоимостиФрагмент()
    
    ЗаполнитьТабличнуюЧастьРаспределениеЗатратПоСебестоимости();
...
&НаКлиенте
Процедура ЗатратыРаспределитьПоСебестоимостиЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПродукцияЗаполнитьПоВыпуску(Команда)
	...
	Если Объект.Продукция.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПродукцияЗаполнитьПоВыпускуЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""პროდუქცია"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Продукция"" будет перезаполнена! Продолжить выполнение операции?';en='A table of the ""Products"" will be refilled! To continue the operation?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
&НаКлиенте
Процедура ЗатратыСчетЗатратРеглПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЗатратыВнеоборотныйАктивНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ТекСтрока = Элементы.Затраты.ТекущиеДанные;
	
	СтруктураПараметров = Новый Структура("НаДату, СчетЗатратРегл", Объект.Дата, ТекСтрока.СчетЗатратРегл);
	ФормаВыбораИмущества = ПолучитьФорму("Справочник.ВнеоборотныеАктивы.Форма.ФормаВыбораОтборПоСчету", СтруктураПараметров, Элемент);
	
	ФормаВыбораИмущества.Открыть();
...
&НаКлиенте
Процедура ЗапасыРаспределитьПоСебестоимостиФрагмент()
	
	ЗаполнитьТабличнуюЧастьРаспределениеЗапасовПоСебестоимости();
...
&НаКлиенте
Процедура ЗапасыРаспределитьПоСебестоимости(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоСебестоимостиЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?';en='Tabular part of the ""stock distribution"" will be refilled! To continue the operation?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ЗаполнитьТабличнуюЧастьРаспределениеЗапасовПоСебестоимости()
	
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.ВыполнитьРаспределениеЗапасовПоСебестоимости();
	ЗначениеВРеквизитФормы(Документ, "Объект");
