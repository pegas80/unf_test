...
Процедура ЗапасыЗаполнитьПоНормативам(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი ""მარაგები"" გადაივსება!';ru='Табличная часть ""Запасы"" будет перезаполнена!'") + Символы.ПС;
		Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
			ТекстВопроса = ТекстВопроса + НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გასუფთავდება!';ru='Табличная часть ""Распределение запасов"" будет очищена!'") + Символы.ПС;
		КонецЕсли;	
		ТекстВопроса = ТекстВопроса + НСтр("ka='გავაგრძელოთ ოპერაციის შესრულება?';ru='Продолжить выполнение операции?'");
		Ответ = Неопределено;
...
Процедура ЗапасыРаспределитьПоНормативам(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоНормативамЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?'"), 
...
Процедура ЗапасыРаспределитьПоКоличеству(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоКоличествуЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ЗатратыЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Затраты.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი ""დანახარჯები"" გადაივსება!';ru='Табличная часть ""Затраты"" будет перезаполнена!'") + Символы.ПС;
		ТекстВопроса = ТекстВопроса + НСтр("ka='ცხრილი ""ხარჯების გადანაწილება"" გასუფთავდება!';ru='Табличная часть ""Распределение расходов"" будет очищена!'") + Символы.ПС;
		ТекстВопроса = ТекстВопроса + НСтр("ka='გავაგრძელოთ ოპერაციის შესრულება?';ru='Продолжить выполнение операции?'");
		Ответ = Неопределено;
...
Процедура ЗатратыРаспределитьПоКоличеству(Команда)
	...
	Если Объект.РаспределениеЗатрат.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыРаспределитьПоКоличествуЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""ხარჯების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение расходов"" будет перезаполнена! Продолжить выполнение операции?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ЗатратыРаспределитьПоСебестоимости(Команда)
	...
	Если Объект.РаспределениеЗатрат.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыРаспределитьПоСебестоимостиЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""ხარჯების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение расходов"" будет перезаполнена! Продолжить выполнение операции?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ПродукцияЗаполнитьПоВыпуску(Команда)
	...
	Если Объект.Продукция.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПродукцияЗаполнитьПоВыпускуЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""პროდუქცია"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Продукция"" будет перезаполнена! Продолжить выполнение операции?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ЗапасыРаспределитьПоСебестоимости(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоСебестоимостиЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი ""მარაგების გადანაწილება"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
