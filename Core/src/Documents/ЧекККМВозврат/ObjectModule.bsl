...
Процедура ЗаполнитьПоЧекуККМ(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("ДокументСсылка.ЧекККМ") Тогда
		...
		ВызватьИсключение НСтр("ka='დაბრუნების ჩეკების შეყვანა უნდა მოხდეს საკასო ჩეკების საფუძველზე';ru='Чеки ККМ на возврат должны вводиться на основании чеков ККМ'");
	...
	Если НЕ РозничныеПродажиСервер.СменаОткрыта(Выборка.КассоваяСмена, ТекущаяДата(), ТекстОшибки) Тогда
		...
		ТекстОшибки = ТекстОшибки + НСтр("ka='. საფუძველზე შეყვანა შეუძლებელია';ru='. Ввод на основании невозможен'");
	...
	Если НЕ Выборка.Проведен Тогда
		...
		ТекстОшибки = НСтр("ka='სსა-ის ჩეკი არ არის გატარებული. საფუძველზე შეყვანა შეუძლებელია';ru='Чек ККМ не проведен. Ввод на основании невозможен'");
	...
	Если НЕ ЗначениеЗаполнено(Выборка.НомерЧекаККМ) Тогда
		...
		ТекстОшибки = НСтр("ka='სსა-ის ჩეკი არ არის ამორტყმული. საფუძვლის მიხედვით შეყვანა შეუძლებელია';ru='Чек ККМ не пробит. Ввод на основании невозможен'");
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ВызватьИсключение НСтр("ka='დაბრუნების ჩეკი შეიყვანება მხოლოდ საფუძვლის მიხედვით';ru='Чек на возврат вводится только на основании'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ka='მოცემული ჩეკისთვის უკვე შეყვანილია დასაბრუნებელი ჩეკი';ru='Для данного чека уже введен чек на возврат'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НачалоДня(Выборка.Дата) <> НачалоДня(Дата) Тогда
			...
			ТекстОшибки = НСтр("ka='დასაბრუნებელი ჩეკის თარიღი უნდა შეესაბამებოდეს გაყიდვის ჩეკის თარიღს';ru='Дата чека на возврат должна соответствовать дате чека продажи'");
		...
		Если КассоваяСмена <> Выборка.КассоваяСмена Тогда
			...
			ТекстОшибки = НСтр("ka='ჩეკის სალაროს ცვლა უნდა შეესაბამებოდეს გაყიდვის ჩეკის სალაროს ცვლას';ru='Кассовая смена Чека на возврат должна соответствовать кассовой смене чека продажи'");
		...
		Если НЕ Выборка.Проведен Тогда
			...
			ТекстОшибки = НСтр("ka='სსა-ის ჩეკი არ არის გატარებული';ru='Чек ККМ не проведен'");
		...
		Если НЕ ЗначениеЗаполнено(Выборка.НомерЧекаККМ) Тогда
			...
			ТекстОшибки = НСтр("ka='გაყიდვის თაობაზე სსა-ის ჩეკი ამოურტყმელია';ru='Чек ККМ продажи не пробит'");
		...
		ТекстОшибки = НСтр("ka='სალაროს ცვლა გაუხსნელია';ru='Кассовая смена не открыта'");
	...
	Если ОплатаПлатежнымиКартами.Количество() > 0 И НЕ ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		...
		ТекстОшибки = НСтр("ka='ველი ""ეკვაირინგის ტერმინალი"" შეუვსებელია';ru='Поле ""Эквайринговый терминал"" не заполнено'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(НомерЧекаККМ)
	   ...
	   И НЕ КассаККМ.ИспользоватьБезПодключенияОборудования Тогда
		...
		ТекстОшибки = НСтр("ka='დასაბრუნებლად სსა-ის ჩეკი ამორტყმულია ფისკალურ რეგისტრატორზე. გატარების გაუქმება შეუძლებელია';ru='Чек ККМ на возврат пробит на фискальном регистраторе. Отмена проведения невозможна'");
