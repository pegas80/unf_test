...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьДвиженияЗапасовОснования(ДокументСсылкаЧекККМВозврат.ЧекККМ); 
...
Процедура СформироватьТаблицаПродажи(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаПродажи.Проект КАК Проект, //Малышев 2024-12-06 ONESOFT-3670
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства) Экспорт
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ЧекККМЗапасы.Заказ = Значение(Документ.ЗаказПокупателя.ПустаяССылка)  
	...
	// Щербаков +++
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаХозрасчетный_ЧекККМВозврат(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства);
	// Щербакво ---
...
Функция СформироватьПечатнуюФормуТоварногоЧека(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ЧекККМ Из МассивОбъектов Цикл
		...
		Пока ВыборкаСтрокЗапасы.Следующий() Цикл
			...
			ОбластьДанных.Параметры.Запас = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(?(Константы.НаименованиеДляПечатиНоменклатуриВпечатныхФормах.Получить(),ВыборкаСтрокЗапасы.ЗапасПолноеНаименование,ВыборкаСтрокЗапасы.Запас), 
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='КМ3 (აქტი მყიდველებისთვის ფულადი თანხების დაბრუნების თაობაზე)';ru='КМ3 (Акт о возврате денежных сумм покупателям)';en='KM3 (Act monies returning customers)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
