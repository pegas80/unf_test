...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ЗначениеЗаполнено(ТекущийОбъект.ОрганизацияЕГАИС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не удалось идентифицировать организацию по классификатору ЕГАИС. Проверьте наличие настроек ЕГАИС для выбранной организации.';ru='Не удалось идентифицировать организацию по классификатору ЕГАИС. Проверьте наличие настроек ЕГАИС для выбранной организации.';en='Unable to identify the organization Classifier EGAIS. Check for EGAIS settings for the selected organization.'"),,"Объект.Организация",, Отказ);
	КонецЕсли;
КонецПроцедуры
...
Процедура УказатьАкцизнуюМарку(Команда)
	...
	Если Объект.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.АктПостановкиНаБалансВТорговомЗале") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для постановки на баланс в торговом зале, ввод акцизной марки не требуется.';ru='Для постановки на баланс в торговом зале, ввод акцизной марки не требуется.';en='For setting the balance of the trading floor, the input tax stamps are required.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ МаркируемаяПродукция(ТекущиеДанные.АлкогольнаяПродукция) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Ввод акцизной марки возможен только для маркируемой алкогольной продукции.';ru='Ввод акцизной марки возможен только для маркируемой алкогольной продукции.';en='Input tax stamps is only possible to be marked alcohol.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗапросНаОтменуПроведения_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		ЭтотОбъект.Прочитать();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Запрос на отмену проведения успешно выгружен.';ru='Запрос на отмену проведения успешно выгружен.';en='The request for cancellation of successfully unloaded.'"));
	КонецЕсли;
