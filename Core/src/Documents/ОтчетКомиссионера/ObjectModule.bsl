...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	Если ДокументСсылкаПриходнаяНакладная.ВидОперации <> Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию Тогда
		...
		НСтр("ka='ოპერაცია ""%1""-ის საფუძველზე კომისიონერის ანგარიშგების შეყვანა არ შეიძლება';ru='Нельзя ввести Отчет комиссионера на основании операции ""%1""'"),
...
Процедура СформироватьСчетаФактурыПокупатели(СсылкаНового)
	...
	Для каждого СтрокаТабличнойЧасти Из Покупатели Цикл
		...
		Если СформированаНоваяСчФ Тогда
			ТекстСообщения = НСтр("ka='ჩამოყალიბებულია დოკუმენტი %ПредставлениеСчетФактуры%.';ru='Сформирован документ %ПредставлениеСчетФактуры%.'");
		...
		Если РезультатПоиска.Количество() = 0 Тогда
			...
			Если ТекущаяСчФ.ДокументыОснования.Количество() > 0 Тогда
				...
				Если НЕ СтрокаОснование = Неопределено Тогда
					...
					ТекстСообщения = НСтр("ka='დოკუმენტ %ПредставлениеСчетФактуры% -დან წაშლილია ბმული მიმდინარე დოკუმენტზე.';ru='Из документа %ПредставлениеСчетФактуры% удалена ссылка на текщий документ.'");
			...
			Иначе
				...
				ТекстСообщения = НСтр("ka='დოკუმენტი %ПредставлениеСчетФактуры% მონიშნულია წასაშლელად.';ru='Документа %ПредставлениеСчетФактуры% помечен на удаление.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеСчетФактуры%", """Счет фактура (выданная) № " + ТекущаяСчФ.Номер + " от " + ТекущаяСчФ.Дата + """");
...
Процедура ОбработкаПодчиненныхСчетФактур(Провести = Истина)
	...
	Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Если СостояниеПроведен И НЕ Провести Тогда
			...
			ТекстСообщения = НСтр("ka='გატარებულია ანგარიშფაქტურა %ПредставлениеСчетФактуры%.';ru='Распроведена счет-фактура %ПредставлениеСчетФактуры%.'");
		...
		ИначеЕсли Не СостояниеПроведен И Провести Тогда
			...
			ТекстСообщения = НСтр("ka='მოყვანილია ანგარიშფაქტურა %ПредставлениеСчетФактуры%.';ru='Проведена счет-фактура %ПредставлениеСчетФактуры%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеСчетФактуры%", """Счет фактура (выданная) № " + ТекущийДокумент.Номер + " от " + ТекущийДокумент.Дата + """");
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если НайденнаяСтрокаЗапасы = Неопределено
		   ...
		   И Контрагент.ВестиРасчетыПоЗаказам Тогда
			ТекстСообщения = НСтр("ka='ავანსის ჩათვლა შეუძლებელია ცხრილის ნაწილ ""მარაგებში"" მითითებული შეკევთისგან განსხვავებულის მიხედვით!';ru='Нельзя зачесть аванс по заказу отличному от указанных в табличной части ""Запасы""!'");
