...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)	
	ЗаписиОСтажеТекст = НСтр("ka='ჩანაწერები სტაჟის თაობაზე';ru='Записи о стаже'");
...
Процедура ДанныеТекущегоДокументаПоСотрудникуВДанныеФормы(АдресВоВременномХранилище)
	...
	Если ДанныеТекущейСтрокиПоСотруднику = Неопределено  Тогда
		...
		ВызватьИсключение НСтр("ka='მიმდინარე დოკუმენტში რედაქტირებადი თანამშრომლის თაობაზე მონაცემები ვერ მოიძებნა.';ru='В текущем документе не найдены данные по редактируемому сотруднику.'");
	КонецЕсли;
...
Процедура ПредупреждениеОНедопустимомОтчетномПериоде()
	Текст = НСтр("ka='ფორმის წარდგენა ხდება 2014 წლიდან მოყოლებული. წინა პერიოდებისთვის ხდება ფორმა СЗВ-1(2, 4)-ის წარდგენა.';ru='Форма предоставляется, начиная с 2014 г. За предыдущие периоды предоставляется формы СЗВ-1(2, 4).'");
