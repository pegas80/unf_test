	...
	Пока ВыборкаПоСтрокамДокумента.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда 
			...
			Если ВыборкаПоСтрокамДокумента.КонфликтующаяСтрока <> Null Тогда  
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ინფორმაცია თანამშრომელ %1-ის შესახებ დოკუმენტში შეყვანილი იქნა ადრე.';ru='Информация о сотруднике %1 была введена в документе ранее.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
...
Процедура ПроверитьДанныеШапкиДокумента(Ошибки, Отказ = Ложь, НеПроверяемыеРеквизиты = Неопределено)
	Если Сотрудники.Количество() > 200 Тогда
		ТекстОшибки = НСтр("ka='დოკუმენტში 200 ფორმაზე მეტი არ უნდა იყოს.';ru='В документе должно быть не более 200 форм.'");
	...
	Если ОтчетныйПериод < '20140101' Тогда
		ТекстОшибки = НСтр("ka='РСВ-1-ის ანგარიშსწორების 6 ნაწილების წარდგენა ხდება 2014 წლის 1 კვარტლიდან დაწყებული! 2010-2012 წლების ინფორმაციის შესადგენად გამოიყენეთ დოკუმენტი ""დოკუმენტების დასტა СЗВ-6-1"", ""რეესტრი СЗВ-6-2"" , 2013 წლისთვის გამოიყენეთ დოკუმენტი """"დოკუმენტების დასტა СЗВ-6-4"""".';ru='Разделы 6 Расчета РСВ-1 представляются начиная с 1 квартала 2014 года! Для формирования сведений за 2010-2012 годы используйте документ ""Пачка документов СЗВ-6-1"", ""Реестр СЗВ-6-2"" , за 2013 год используйте документ """"Пачка документов СЗВ-6-4"""".'");
	...
	Если ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ Тогда
		Если Не ЗначениеЗаполнено(КорректируемыйПериод) Тогда
			ТекстОшибки = НСтр("ka='კორექტირებადი საანგარიშგებო პერიოდი არ არის მითითებული.';ru='Не указан корректируемый отчетный период.'");
		...
		ИначеЕсли ОтчетныйПериод <= КорректируемыйПериод Тогда
			ТекстОшибки = НСтр("ka='კორექტირებადი პერიოდი უნდა უსწრებდეს საანგარიშგებო პერიოდს.';ru='Корректируемый период должен предшествовать отчетному.'");
		
			ПерсонифицированныйУчетКлиентСервер.ДобавитьОшибкуЗаполненияЭлементаДокумента(Ошибки, Ссылка, ТекстОшибки, "КорректируемыйПериод", Отказ);
		ИначеЕсли КорректируемыйПериод < '20140101' Тогда
			ТекстОшибки = НСтр("ka='ანგარიშსწორება РСВ-1-ის კორექტირებადი (გამაუქმებელი) ნაწილების 6 წარდგენა ხდება მხოლოდ 2014 წლის 1 კვარტლიდან მოყოლებული საანგარიშგებო პერიოდებისთვის.';ru='Корректирующие (отменяющие) Разделы 6 Расчета РСВ-1 представляются только за отчетные периоды c 1 квартала 2014 года.'");
...
Процедура ПроверитьСтрокиЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТарифыПоКатегориям, Отказ = Ложь)
		...
		Пока ВыборкаПоДоходам.СледующийПоЗначениюПоля("КатегорияЗастрахованныхЛиц") Цикл
			...
			Пока ВыборкаПоДоходам.Следующий() Цикл
				Если ЗначениеЗаполнено(ВыборкаПоДоходам.Месяц) 
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ხელფასის სტრიქონებში მითითებულია თვე (%1), რომელიც საანგარიშგებო პერიოდს არ განეკუთვნება';ru='в строках заработка указан месяц (%1), не принадлежащий отчетному периоду'"), ВыборкаПоДоходам.Месяц);
				...
				Если Не ЗначениеЗаполнено(ВыборкаПоДоходам.КатегорияЗастрахованныхЛиц) Тогда
					ТекстОшибки = НСтр("ka='დაზღვეული პირების კატეგორია არ არის მითითებული';ru='не указана категория застрахованных лиц.'");
				...
				Если ВыборкаПоДоходам.Заработок = 0 И ВыборкаПоДоходам.ОблагаетсяВзносамиДоПредельнойВеличины = 0 И ВыборкаПоДоходам.ОблагаетсяВзносамиСвышеПредельнойВеличины = 0  Тогда
					ТекстОшибки = НСтр("ka='მითითებულია ნულოვანი მონაცემები.';ru='указаны нулевые данные.'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.Заработок < 0 Тогда
					ТекстОшибки = НСтр("ka='მითითებულია უარყოფითი ხელფასი.';ru='указан отрицательный заработок.'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.ОблагаетсяВзносамиДоПредельнойВеличины < 0 Тогда
					ТекстОшибки = НСтр("ka='მითითებულია უარყოფითი ბაზა, რომელიც უკიდურეს სიდიდეს არ აღემატება.';ru='указана отрицательная база, не превышающая предельную величину.'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.ПоДоговорамГПХДоПредельнойВеличины < 0 Тогда
					ТекстОшибки = НСтр("ka='სამოქალაქო-სამართლებრივი ხასიათის ხელშეკრულებების მიხედვით მითითებულია უარყოფითი ბაზა, რომელიც უკიდურეს სიდიდეს არ აღემატება';ru='указана отрицательная база по договорам ГПХ, не превышающая предельную величину'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.ОблагаетсяВзносамиСвышеПредельнойВеличины < 0 Тогда
					ТекстОшибки = НСтр("ka='მითითებულია უარყოფითი ბაზა, რომელიც უკიდურეს სიდიდეს აღემატება.';ru='указана отрицательная база, превышающая предельную величину.'");
				...
				Если ВыборкаПоДоходам.Заработок < ВыборкаПоДоходам.ОблагаетсяВзносамиДоПредельнойВеличины + ВыборкаПоДоходам.ОблагаетсяВзносамиСвышеПредельнойВеличины Тогда
					ТекстОшибки = НСтр("ka='თანხა, რომელიც შენატანებით იბეგრება, დარიცხული ხელფასის ოდენობას აღემატება.';ru='сумма, облагаемая взносами, превышает сумму начисленного заработка.'");
		...
		Если ЗначениеЗаполнено(ПериодДанных) Тогда
			Для каждого СтрокаТаблицы Из МесяцыДохода Цикл
				Если СтрокаТаблицы.КоличествоСтрок > 1 Тогда
					ШаблонТекстаОшибки = НСтр("ka='თანამშრომელი %1: ნაწილ 6.4-ში (სახელფასო ინფორმაცია) კატეგორია %2-ით თვე #%3 მითითებულია ერთჯერ მეტად';ru='Сотрудник %1: в разделе 6.4 (сведения о заработке) по категории %2 месяц №%3 указан более одного раза'");
	...
	Если Разность > 0.03 Тогда
		ШаблонТекстаОшибки = НСтр("ka='თანამშრომელი %1: %2';ru='Сотрудник %1: %2'");	
...
Функция ТекстОшибкиЗаработка(НаименованиеСотрудника, Месяц = Неопределено, КатегорияЗастрахованныхЛиц = Неопределено, ТекcтОшибки)
	...
	ШаблонТекстаОшибки = НСтр("ka='თანამშრომელი %1: ნაწილ 6.4-ში (ინფორმაცია ხელფასის შესახებ) %2%3%4';ru='Сотрудник %1: в разделе 6.4 (сведения о заработке) %2%3%4'");
...
Процедура ПроверитьСтрокиВредногоЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах, Отказ = Ложь)
	...
	Если ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.СведенияЗаполнены Тогда
		Пока ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.СледующийПоЗначениюПоля("КлассУсловийТруда") Цикл
			Пока ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.Следующий() Цикл 		
				Если ЗначениеЗаполнено(ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.Месяц) 
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ka='ხელფასის სტრიქონებში მითითებულია თვე (%1), რომელიც საანგარიშგებო პერიოდს არ განეკუთვნება';ru='в строках заработка указан месяц (%1), не принадлежащий отчетному периоду'"), 
				...
				Если ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах < 0 Тогда
					ТекстОшибки = НСтр("ka='მითითებულია მიწისქვეშა და მავნე სამუშაოებზე დაკავებულების შენატანების უარყოფითი ბაზა.';ru='указана отрицательная база взносов за занятых на подземных и вредных работах.'");
				...
				Если ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах < 0 Тогда
					ТекстОшибки = НСтр("ka='მძიმე სამუშაოებზე დაკავებულებზე მითითებულია შენატანების უარყოფითი ბაზა.';ru='указана отрицательная база взносов за занятых на тяжелых работах.'");
		...
		Если ЗначениеЗаполнено(ПериодДанных) Тогда 
			Для каждого СтрокаТаблицы Из МесяцыДохода Цикл
				Если СтрокаТаблицы.КоличествоСтрок > 1 Тогда
					ШаблонТекстаОшибки = НСтр("ka='ქვეთავ 6.7-ში თანამშრომელი %1 (ინფორმაცია ვადამდელი პენსიის უფლების მქონე სამუშაოებზე დაკავებულებისთვის გადახდების შესახებ) შრომის პირობების კლასით %2 თვე #%3 ერთჯერ მეტად არის მითითებული';ru='Сотрудник %1: в разделе 6.7 (Сведения о выплатах занятых на работах дающих право на досрочную пенсию) по классу условий труда %2 месяц №%3 указан более одного раза'");
	...
	Если ВыборкаДанныхСотрудника.Ст27_1ВПервомМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВПервомМесяце = 0 Тогда
			ТекстОшибки = НСтр("ka='საანგარიშო პერიოდის პირველი თვის შრომის განსაკუთრებული პირობების მე-6.7 ქვეგანყოფილების სტრიქონებში 27-1 კოდის არსებობის შემთხვევაში  აუცილებელია მნიშვნელობათა არსებობა მე-4 გრაფის მე-6.7 ქვეგანყოფილების სტრიქონებში 701,711...';ru='При наличии кода 27-1 особых условий труда в строках подраздела 6.8 за первый месяц отчетного периода обязательно наличие значений в графе 4 подраздела 6.7 в строках 701,711 ...'");
	...
	Если ВыборкаДанныхСотрудника.Ст27_1ВоВторомМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВоВторомМесяце = 0 Тогда
			ТекстОшибки = НСтр("ka=' საანგარიშო პერიოდის მეორე თვის შრომის განსაკუთრებული პირობების მე-6.8 ქვეგანყოფილების სტრიქონებში  27-1 კოდის არსებობის შემთხვევაში  აუცილებელია მნიშვნელობათა არსებობა მე-4 გრაფის მე-6.7 ქვეგანყოფილების სტრიქონებში 702,712...';ru='При наличии кода 27-1 особых условий труда в строках подраздела 6.8 за второй месяц отчетного периода обязательно наличие значений в графе 4 подраздела 6.7 в строках 702,712 ...'");
	...
	Если ВыборкаДанныхСотрудника.Ст27_1ВТретьемМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВТретьемМесяце = 0 Тогда
			ТекстОшибки = НСтр("ka=' საანგარიშო პერიოდის მესამე თვის შრომის განსაკუთრებული პირობების მე-6.8 ქვეგანყოფილების სტრიქონებში  27-1 კოდის არსებობის შემთხვევაში  აუცილებელია მნიშვნელობათა არსებობა მე-4 გრაფის მე-6.7 ქვეგანყოფილების სტრიქონებში 703,713...';ru='При наличии кода 27-1 особых условий труда в строках подраздела 6.8 за третий месяц отчетного периода обязательно наличие значений в графе 4 подраздела 6.7 в строках 703,713 ...'");
...
Функция ТекстОшибкиВредногоЗаработка(НаименованиеСотрудника, Месяц = Неопределено, КлассУсловийТруда = Неопределено, ТекcтОшибки)
	...
	ШаблонТекстаОшибки = НСтр("ka='თანამშრომელი %1: ნაწილ 6.7-ში (ინფორმაცია ვადამდელი პენსიის უფლების მქონე სამუშაოებზე დაკავებულებისთვის გადახდების შესახებ) %2%3%4';ru='Сотрудник %1: в разделе 6.7 (Сведения о выплатах занятых на работах дающих право на досрочную пенсию) %2%3%4'");
