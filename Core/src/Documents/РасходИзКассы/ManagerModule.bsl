...
Процедура СформироватьТаблицаДенежныеСредстваВКассахККМ(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("ПоступлениеДенежныхСредств", НСтр("ka='ფულადი სახსრების შემოსვლა სსა-ის სალაროში';ru='Поступление денежных средств в кассу ККМ'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	//Запрос.УстановитьПараметр("СодержаниеВыдачаЗаймаСотруднику", НСтр("ka='თანამშრომლისთვის სესხის გაცემა';ru='Выдача займа сотруднику'"));
	//Запрос.УстановитьПараметр("СодержаниеПеремещениеВКассуККМ", НСтр("ka='ფულის გადაადგილება სსა-ის სალაროში';ru='Перемещение денежных средств в кассу ККМ'"));
...
Процедура СформироватьРКО_ЛОК(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ОбъектаИзМассива Из МассивОбъектов Цикл
		...
		Область.Параметры.Кредит		= ОбъектаИзМассива.Касса.СчетУчета.Код;
		Область.Параметры.Дебет			= СтрокаДебетСубСчет;
		Область.Параметры.Цена			= ОбъектаИзМассива.СуммаДокумента;	
		//Область.Параметры.Выдать		= Строка(ОбъектаИзМассива.Контрагент) +  "(" + Строка(ОбъектаИзМассива.Контрагент.ИНН)  +")";
		
		Если ЗначениеЗаполнено(ОбъектаИзМассива.Выдать) = Истина Тогда 
			 Область.Параметры.Выдать = ОбъектаИзМассива.Выдать;
		Иначе 	 
			 Область.Параметры.Выдать = ОбъектаИзМассива.Подотчетник;
		КонецЕсли;;	
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		...
		ОписаниеПечатнойФормы.СинонимМакета = НСтр("ka='კო-2 (გასავლის სალაროს ორდერი)';ru='КО-2 (Расходный кассовый ордер)'");
	...
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		...
		ОписаниеПечатнойФормы.СинонимМакета = НСтр("ka='სალაროს გასავლის ორდერი';ru='Расходный кассовый ордер'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='კო-2 (გასავლის სალაროს ორდერი)';ru='КО-2 (Расходный кассовый ордер)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "РКО_ЛОК";
	КомандаПечати.Представление = НСтр("ka='სალაროს გასავლის ორდერი';ru='Расходный кассовый ордер'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
