...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Безналичные Тогда
		ВызватьИсключение НСтр("ka='ფულის ხარჯის შეყვანა არ შეიძლება. განაცხადში გადახდის არასწორი მეთოდი (ფულადი სახსრების ტიპი) არის მითითებული!';ru='Нельзя ввести расход денег. В заявке указан неверный способ оплаты (тип денежных средств)!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	Если ДокументСсылкаНачислениеНалогов.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
		ВызватьИсключение НСтр("ka='სალაროდან გასავლის შეყვანა შეიძლება მხოლოდ გადასახადების დარიცხვის, და არა ანაზღაურების საფუძველზე.';ru='Расход из кассы можно ввести только на основании начисления налогов, а не возмещения.'");
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если (ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная") И ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика) ИЛИ 
		...
		Если (ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная") И НЕ ДанныеЗаполнения.Проведен И 
			...
			ТекстОшибки = НСтр("ka='Нельзя создать документ на основании непроведенного документа.';ru='Нельзя создать документ на основании непроведенного документа.'");
		...
		РезультатЗапроса = Запрос.Выполнить();
		Если РезультатЗапроса.Пустой() Тогда
			ТекстОшибки = НСтр("ka='На основании текущего документа уже проезведена оплата полностью.';ru='На основании текущего документа уже проезведена оплата полностью.'");
		  	ВызватьИсключение ТекстОшибки;
		Иначе
			Выборка = РезультатЗапроса.Выбрать();
			Выборка.Следующий();
