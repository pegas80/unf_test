...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.Дата <> ДатаПередИзменением Тогда
		...
		ТекстСообщения = НСтр("ka='შეიცვალა სალაროს ვალუტის კურსი. გადავთვალოთ დოკუმენტის თანხები?';ru='Изменился курс валюты кассы. Пересчитать суммы документа?'");
...
Процедура ВалютаДенежныхСредствПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.Зарплата") Тогда
		ТекстСообщения = НСтр("ka='სალაროს ვალუტა შეიცვალა. სია ""საგადახდო უწყისები"" გასუფთავდება.';ru='Изменилась валюта кассы. Список ""Платежные ведомости"" будет очищен.'");
...
Процедура ВалютаДенежныхСредствПриИзмененииФрагмент()
	...
	ТекстСообщения = НСтр("ka='გადავთვალოთ დოკუმენტის თანხები?';ru='Пересчитать суммы документа?'");
	ПересчитатьСуммыПриИзмененииКурсаВалютыДенежныхСредств(СтруктураДанные, ТекстСообщения);
...
Процедура Подбор(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='თავდაპირველად, მიუთითეთ ვალუტა!';ru='Укажите вначале валюту!'"));
		Возврат;
	КонецЕсли;
...
Процедура НапечататьЧек(Команда)
	...
	Если Объект.НомерЧекаККМ <> 0 Тогда
		ТекстСообщения = НСтр("ka='ჩეკი უკვე ამორტყმულია მოწყობილობაზე!';ru='Чек уже пробит на оборудовании!'");
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		МенеджерОборудованияКлиент.ПредложитьВыбратьУстройство(ОписаниеОповещения, МассивОборудования,
				НСтр("ka='აღჭურვილობის აღჩევა';ru='Выберите оборудование'"), НСтр("ka='მოწყობილობა არ არის შეერთებული.';ru='Оборудование не подключено.'"));
		
	Иначе
		
		ТекстСообщения = НСтр("ka='წინასწარ აუცილებელია მიმდინარე სეანსის გარე მოწყობილობის სამუშაო ადგილის არჩევა.';ru='Предварительно необходимо выбрать рабочее место внешнего оборудования текущего сеанса.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ПодготовитьДанныеДляПробитияЧека(ИдентификаторУстройстваФР, ЭтоВозврат) 
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		ТекстСообщения = НСтр("ka='Не найдена касса ККМ соответсвующая устройству %устройство%.';ru='Не найдена касса ККМ соответсвующая устройству %устройство%.'");
...
Процедура ПодключенияФискальногоРегистратораЗавершение(ИдентификаторУстройстваФР, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваФР = Неопределено Тогда
		ТекстСообщения = НСтр("ka='ჩეკების ამოსაბეჭდი მოწყობილობა არ არის არჩეული.';ru='Устройство для печати чеков не выбрано.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику") Тогда
		...
		Элементы.Подотчетник.Видимость = Истина;
		Элементы.Подотчетник.Заголовок = НСТР("ka='თანამშრომელი';ru='Сотрудник'");
