	...
	Для каждого СтрокаНалоги Из Налоги Цикл
		...
		Если НЕ ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
			...
			Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
				...
				ТекстСообщения = НСтр("ka='სია ""გადასახადების"" სტრიქონ  %НомерСтроки% მითითებული ანგარიშისთვის ""%Корреспонденция%"" უნდა შეივსოს რეკვიზიტი ""ქვეგანყოფილება"".';ru='Для счета затрат ""%Корреспонденция%"" указанного в строке %НомерСтроки% списка ""Налоги"", должен быть заполнен реквизит ""Подразделение"".'"
				);
