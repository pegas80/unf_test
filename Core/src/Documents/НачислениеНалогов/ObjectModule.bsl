...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить() Тогда
		...
		Для каждого СтрокаНалоги Из Налоги Цикл
			...
			Если ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
				Если СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.НезавершенноеПроизводство
					ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.КосвенныеЗатраты
					ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Доходы
					ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимНаправлениямДеятельности.Получить() Тогда
		...
		Для каждого СтрокаНалоги Из Налоги Цикл
			Если ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
				Если СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Доходы
					ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийНачислениеНалогов.Взаиморасчет Тогда
		ПроверяемыеРеквизиты.Добавить("Налоги.ВидНалогаКор");
	Иначе
		Если ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
			ПроверяемыеРеквизиты.Добавить("Налоги.КорреспонденцияРегл");
		Иначе
			ПроверяемыеРеквизиты.Добавить("Налоги.Корреспонденция");
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого СтрокаНалоги Из Налоги Цикл
		...
		Если НЕ ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
			...
			Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
				И Не ВидОперации = Перечисления.ВидыОперацийНачислениеНалогов.Взаиморасчет
				...
				ТекстСообщения = НСтр("ka='სია ""გადასახადების"" სტრიქონ  %НомерСтроки% მითითებული ანგარიშისთვის ""%Корреспонденция%"" უნდა შეივსოს რეკვიზიტი ""ქვეგანყოფილება"".';ru='Для счета затрат ""%Корреспонденция%"" указанного в строке %НомерСтроки% списка ""Налоги"", должен быть заполнен реквизит ""Подразделение"".';en='For costs account ""% Correspondence%"" of the line%% NomerStroki list of ""Taxes"" should be filled with props ""Division"".'"
				);
		...
		Иначе	
			...
			Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
				И Не ВидОперации = Перечисления.ВидыОперацийНачислениеНалогов.Взаиморасчет
				И (СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.НезавершенноеПроизводство
				ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.КосвенныеЗатраты
				ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Доходы
				ИЛИ СтрокаНалоги.КорреспонденцияРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы)
				И НЕ ЗначениеЗаполнено(СтрокаНалоги.Подразделение) Тогда
				ТекстСообщения = НСтр("ka='სია ""გადასახადების"" სტრიქონ  %НомерСтроки% მითითებული ანგარიშისთვის ""%Корреспонденция%"" უნდა შეივსოს რეკვიზიტი ""ქვეგანყოფილება"".';ru='Для счета затрат ""%Корреспонденция%"" указанного в строке %НомерСтроки% списка ""Налоги"", должен быть заполнен реквизит ""Подразделение"".';en='For costs account ""% Correspondence%"" of the line%% NomerStroki list of ""Taxes"" should be filled with props ""Division"".'"
				);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Корреспонденция%", СокрЛП(Строка(СтрокаНалоги.КорреспонденцияРегл))); 
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%",Строка(СтрокаНалоги.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
