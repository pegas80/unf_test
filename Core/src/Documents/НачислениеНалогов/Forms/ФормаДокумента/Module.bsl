...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	ЕстьВзаиморасчет =  Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНачислениеНалогов.Взаиморасчет");
	Элементы.НалогиВидНалогаКор.Видимость    = ЕстьВзаиморасчет; 
	Элементы.НалогиКорреспонденция.Видимость = Не ЕстьВзаиморасчет;
	
КонецПроцедуры

&НаКлиенте
Процедура НалогиВидНалогаКорПриИзменении(Элемент)
	
	Если Не ИспользоватьТолькоРегламентированногоПланаСчетов Тогда 
		СтрокаТабличнойЧасти = Элементы.Налоги.ТекущиеДанные;
		СтрокаТабличнойЧасти.Корреспонденция = ПолучитьСчетУчетаВидНалогаКор(СтрокаТабличнойЧасти.ВидНалогаКор);
		СтрокаТабличнойЧасти.КорреспонденцияРегл = ПолучитьСчетУчетаВидНалогаКорРегл(СтрокаТабличнойЧасти.ВидНалогаКор);
	Иначе 
		СтрокаТабличнойЧасти = Элементы.Налоги.ТекущиеДанные;
		СтрокаТабличнойЧасти.КорреспонденцияРегл = ПолучитьСчетУчетаВидНалогаКорРегл(СтрокаТабличнойЧасти.ВидНалогаКор);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьСчетУчетаВидНалогаКор(СсылкаВидНалогаКор)
	
	Возврат СсылкаВидНалогаКор.СчетУчетаКВозмещению;
	
КонецФункции 

&НаСервереБезКонтекста
Функция ПолучитьСчетУчетаВидНалогаКорРегл(СсылкаВидНалогаКор) //+++ Дмгг
	
	Возврат СсылкаВидНалогаКор.СчетУчетаКВозмещениюРегл;
	
КонецФункции

&НаКлиенте
Процедура НалогиКорреспонденцияРеглПриИзменении(Элемент)
	ТекСтрока = Элементы.Налоги.ТекущиеДанные;
	Если Не ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
		РегламентированныйУчетСервер.УстановитьУпрСчетИзПланаСчетовХозрасчетный(ТекСтрока.КорреспонденцияРегл, ТекСтрока.Корреспонденция);
	КонецЕсли;
КонецПроцедуры
