...
Процедура ЗаказПокупателяПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И КонтрагентВестиРасчетыПоЗаказам Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПокупателяПриИзмененииЗавершение", ЭтотОбъект), НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?';en='Passed prepayment will be cleared to continue?'"), Режим, 0);
        Возврат;
	КонецЕсли;
