...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			...
			НСтр("ka='შესრულებული სამუშაოების შესახებ აქტის შეყვანა შეკვეთა-განწესის %1 საფუძველზე არ შეიძლება!';ru='Нельзя ввести Акт выполненных работ на основании заказ-наряда %1!'"),
...
Процедура ЗаполнитьПоСчетуНаОплату(ДокументСсылкаСчетНаОплату) Экспорт
	...
	Если ТипЗнч(ОснованиеСчетаНаОплату) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Если ОснованиеСчетаНаОплату.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			ВызватьИсключение НСтр("ka='შესრულებული სამუშაოების აქტის შეყვანა შეკვეთა-განწესისთვის გამოწერილი გადასახდელი ანგარიშის საფუძველზე არ შეიძლება!';ru='Нельзя ввести Акт выполненных работ на основании Счета на оплату, выписанного на Заказ-наряд!'");;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьРучныеСкидки ИЛИ ЕстьАвтоматическиеСкидки Тогда
		Для каждого СтрокаРаботыИУслуги Из РаботыИУслуги Цикл
			...
			Если СтрокаРаботыИУслуги.ПроцентСкидкиНаценки <> 100 И ТекСуммаСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ka='არ არის შევსებული სვეტი ""თანხა"" სია ""სამუშაოები და მომსახურებები""-ს სტრიქონ %Номер%-ში.';ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Работы и услуги"".'");
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если НайденнаяСтрокаРаботыИУслуги = Неопределено
		   ...
		   И Контрагент.ВестиРасчетыПоЗаказам Тогда
			ТекстСообщения = НСтр("ka='ავანსის ჩათვლა შეუძლებელია ცხრილის ნაწილ ""სამუშაოები და მომსახურებებში"" მითითებული შეკევთისგან განსხვავებულის მიხედვით!';ru='Нельзя зачесть аванс по заказу отличному от указанных в табличной части ""Работы и услуги""!'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				,
				ТекстСообщения,
				Неопределено,
		...
		Для Каждого Стр Из РаботыИУслуги Цикл
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Договор, Стр.Номенклатура, Стр.Характеристика) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ka='Запрещено проводить незапланированные услуги/работы по текущему договору обслуживания!';ru='Запрещено проводить незапланированные услуги/работы по текущему договору обслуживания!'"),
