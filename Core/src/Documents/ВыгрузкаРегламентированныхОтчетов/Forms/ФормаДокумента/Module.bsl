...
Процедура Заполнить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не указана организация.';ru='Не указана организация.'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Выгрузки.Количество() <> 0 Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект);
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Тексты выгрузки будут очищены.%1Продолжить?';ru='Тексты выгрузки будут очищены.%1Продолжить?'"), Символы.ПС);
...
Процедура СформироватьТекстыВыгрузки(Команда)
	...
	Если ОшибкаВыгрузки <> Истина Тогда
		ПоказатьПредупреждение(,НСтр("ka='Формирование текстов выгрузки успешно завершено.';ru='Формирование текстов выгрузки успешно завершено.'"));
	КонецЕсли;
		...
		Для Каждого Стр Из ОтборФСС Цикл
			...
			Исключение
				...
				Если СообщенияОбОшибках = Неопределено Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось выгрузить %1.';ru='Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
					Сообщение.Сообщить();
	...
	Если Отбор300.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда	
		...
		Для Каждого Стр Из Отбор300 Цикл
			...
			Если НЕ РегламентированнаяОтчетностьВызовСервера.ВывестиСведенияСлужебнойЧастиИОбОтправителе30(Текст, Объект.Организация, КПП, СокрЛП(Объект.КодНалоговогоОргана)) Тогда 
				...
				Сообщение.Текст = НСтр("ka='Не удалось сформировать общую информацию для файла выгрузки формата 3.00.';ru='Не удалось сформировать общую информацию для файла выгрузки формата 3.00.'");
			...
			Если Стр.Отчет = "Статистика: Форма П-1" И Год(Стр.ДатаНачала) <= 2016 Тогда
				...
				Если ТекФормаП1Пр3 = Неопределено Тогда
					...
					Если НазваниеИсточникОтчета <> "Без приложения" Тогда
						ТекстСообщения = НСтр("ka='ვერ მოიძებნა შენახული ანგარიშგება ""სტატისტიკა: ფორმა პ-1 დანართი 3"" XML ფაილში გადმოსატვირთად - შესაბამისი ნაწილი ცარიელი იქნება.';ru='Не найден сохраненный отчет ""Статистика: Форма П-1 Приложение 3"" для выгрузки в файл XML - соответствующий раздел будет пустым.'");
				...
				Иначе
					...
					Исключение
						...
						Если СообщенияОбОшибках = Неопределено Тогда
							...
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось выгрузить %1.';ru='Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(СтрДокП1Пр3));
							Сообщение.Сообщить();
...
Процедура ОбработатьСобытие1СОтчетностиНаСервере(ПервыйДок)
	...
	РегламентированнаяОтчетность.ОбработатьСобытие1СОтчетности(НСтр("ka='Регламентированный отчет. Выгрузка';ru='Регламентированный отчет. Выгрузка'"), ПервыйДок);
...
Процедура СформироватьТекстыВыгрузкиСервер(ПервыйДок, ДляОтправкиВФНС, КПП, ОшибкаВыгрузки)
	...
	Если КоличествоДокументовДляВыгрузки = 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Не выбраны документы.';ru='Не выбраны документы.'");
	...
	Для Каждого Стр Из Основания Цикл
		Если Стр.Организация <> Основания.Получить(0).Организация Тогда
			...
			Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна.';ru='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна.'");
		...
		Если КонецДня(Стр.ДатаОкончания) <> КонецДня(Основания.Получить(0).ДатаОкончания) Тогда
			...
			Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна.';ru='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна.'");
	...
	Если НалоговыеОрганы.Количество() > 0 Тогда
		...
		Если НЕ НалоговыеОрганы.Количество() = НайденныеСтроки.Количество() Тогда
			...
			Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются коды ИМНС, в которую они должны представляться. Процесс прерван.';ru='У выбранных отчетов различаются коды ИМНС, в которую они должны представляться. Процесс прерван.'");
	...
	Если ОтчетыВыгружаемыеТолькоИзФормы.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Выгрузка следующих отчетов возможна только из формы отчета:';ru='Выгрузка следующих отчетов возможна только из формы отчета:'");
	...
	Если ЕстьНеопределенныеФорматы Тогда
		...
		Сообщение.Текст = НСтр("ka='შემდეგი ანგარიშგებებისთვის გადმოტვირთვა არ არის გათვალისწინებული:';ru='Для следующих отчетов выгрузка не предусмотрена:'");
		...
		Сообщение.Текст = НСтр("ka='Продолжение невозможно. Формирование текстов выгрузки прервано.';ru='Продолжение невозможно. Формирование текстов выгрузки прервано.'");
	...
	Если ДляОтправкиВФНС И ОтчетыНеотправляемыеВФНС.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Следующие отчеты не предназначены для отправки в ФНС по каналам связи:';ru='Следующие отчеты не предназначены для отправки в ФНС по каналам связи:'");
	...
	Если ОтчетыСохраненныеСНевернымОкруглением.Количество() <> 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='შემდეგი ანგარიშგებები შენახულია საზომი ერთეულით და/ან იმ სიზუსტით, რომელიც გადმოტვირთვის ფორმატის მოთხოვნებს არ შეესაბამება:';ru='Следующие отчеты сохранены с единицей измерения и/или точностью, не соответствующими требованиям формата выгрузки:'");
		...
		Сообщение.Текст = НСтр("ka='შეინახეთ ანგარიშგებები საჭირო საზომი ერთეულით და ხელახლა გაუშვით ჩამოყალიბების პროცესი.';ru='Сохраните отчеты с нужной единицей измерения и перезапустите процесс формирования.'");
...
Функция ВывестиОписаниеФормыОтчетности(Текст, Организация, КодНалоговогоОргана)
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ВывестиСведенияСлужебнойЧастиИОбОтправителе30(Текст, Организация, , СокрЛП(КодНалоговогоОргана), "2.01") Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось сформировать общую информацию для файла выгрузки формата 2.01.';ru='Не удалось сформировать общую информацию для файла выгрузки формата 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ВывестиСведенияИнформационнойЧасти201ИСведенияОбОрганизации(Текст, Организация) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось выгрузить сведения об организации для файла выгрузки формата 2.01.';ru='Не удалось выгрузить сведения об организации для файла выгрузки формата 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ВывестиСведенияОРуководствеОрганизации201(Текст, Организация) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось выгрузить сведения о руководстве организации для файла выгрузки формата 2.01.';ru='Не удалось выгрузить сведения о руководстве организации для файла выгрузки формата 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Процедура СохранитьТексты(НЕЗаписыватьДокумент,
	...
	Если Объект.Выгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='შესანახად არ არის გადმოტვირთვის ტექსტები!';ru='Нет текстов выгрузки для сохранения!'"));
		ОшибкаВыгрузки = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЧислоВыгружаемыхОтчетов = 0 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შესანახად ინფორმაცია არ არის!';ru='Нет информации для сохранения!'");
		Сообщение.Сообщить();
			...
			Если ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
							...
							Если СтрНайти(ТекСтр.ИмяФайла, "NO_BUHOTCH_") = 1
								...
								Если ТекСтр.Тип <> ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСГС Тогда
									...
									Если СписокДополнительныхФайлов.Количество() > 0 Тогда
										...
										Сообщение = Новый СообщениеПользователю;
										Сообщение.Текст = НСтр("ka='საბუღალტრო ანგარიშგებულობის გადმოტვირთვის ფაილს დაურთეთ:';ru='К файлу выгрузки бухгалтерской отчетности приложите:'");
										Сообщение.Сообщить();
				...
				Исключение
					...
					Если ФайлВыгрузкиВоВременномХранилище = Неопределено Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось записать файл ""%1"". Возможно, недостаточно места на диске или диск защищен от записи.';ru='Не удалось записать файл ""%1"". Возможно, недостаточно места на диске или диск защищен от записи.'"), ИмяКонечногоФайла);
					...
					Иначе
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დროებით საცავში გადმოტვირთვის ფაილ ""%1""-ის მოთავსება ვერ მოხერხდა!';ru='Не удалось поместить файл выгрузки ""%1"" во временное хранилище!'"), ИмяКонечногоФайла);
	...
	Если ВыгруженХотяБыОдинФайл Тогда
		...
		Если БылаОшибкаЗаписи = Истина Тогда
			Если НЕ РежимВызоваЭкспортируемогоМетодаФормы Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='В процессе записи отчетов произошла ошибка!';ru='В процессе записи отчетов произошла ошибка!'");
				Сообщение.Сообщить();
...
Функция ОпределитьИмяФайлаВыгрузкиФССПоТекстуВыгрузки(ТекстВыгрузки)
	...
	Если ДеревоXML = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить имя файла выгрузки ФСС по тексту выгрузки.';ru='Не удалось определить имя файла выгрузки ФСС по тексту выгрузки.'");
...
Функция ВывестиСведенияИнформационнойЧасти201ИСведенияОбОрганизации(Текст, Орг)
	...
	Если Св.ТипНП = 1 Тогда//российская организация
		...
		Если НЕ ЗначениеЗаполнено(НаимЮЛПол) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для организации не задано полное наименование!';ru='Для организации не задано полное наименование!'");
		...
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ИННЮЛ, Ложь) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციისთვის არასწორად არის მიცემული ს.კ / პ.ნ!';ru='Для организации неверно задан ИНН!'");
		...
		Если НЕ ПустаяСтрока(ОГРН) И НЕ РегламентированнаяОтчетность.ОГРНСоответствуетТребованиям(ОГРН) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციისთთვის არასწორად არის მიცემული ძსსნ!';ru='Для организации неверно задан ОГРН!'");
	...
	ИначеЕсли Св.ТипНП = 2 Тогда//обособленное подразделение
		...
		Если НЕ ЗначениеЗаполнено(НаимЮЛПол) Тогда
			...
			Сообщение.Текст = НСтр("ka='სათაო ორგანიზაციისთვის სრული სახელწოდება არ არის მიცემული!';ru='Для головной организации не задано полное наименование!'");
		...
		Если НЕ ЗначениеЗаполнено(НаимОППол) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для обособленного подразделения не задано полное наименование!';ru='Для обособленного подразделения не задано полное наименование!'");
	...
	ИначеЕсли Св.ТипНП = 3 Тогда//ИП
		...
		Если НЕ ЗначениеЗаполнено(Св.ФИО) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для индивидуального предпринимателя не задано ФИО!';ru='Для индивидуального предпринимателя не задано ФИО!'");
		...
		Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ИННФЛ) Тогда
			...
			Сообщение.Текст = НСтр("ka='ინდმეწარმისთვის არასწორად არის მიცემული ს.კ / პ.ნ!';ru='Для индивидуального предпринимателя неверно задан ИНН!'");
	...
	ИначеЕсли Св.ТипНП = 5 Тогда//иностранная организация
		...
		Если НЕ ЗначениеЗаполнено(НаимОИОПол) Тогда
			...
			Сообщение.Текст = НСтр("ka='უცხოური ორგანიზაციის განყოფილებისთვის არ არის მიცემული სრული სახელწოდება!';ru='Для отделения иностранной организации не задано полное наименование!'");
		...
		Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ИННИО) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для иностранной организации неверно задан ИНН!';ru='Для иностранной организации неверно задан ИНН!'");
...
Функция ПроверитьИОбработатьАдрЮр(Сведения)
	...
	Если Сведения.АдрЮр = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить юридический адрес организации!';ru='Не удалось определить юридический адрес организации!'");
	...
	Иначе
		Если СтрЧислоВхождений(Сведения.АдрЮр, ",") <> 9 Тогда
			...
			Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში მითითებულია ორგანიზაციის არაკორექტული იურიდიული მისამართი (მისამართში მძიმის (,) რაოდენობა უნდა უდრიდეს ცხრას)!';ru='В информационной базе указан некорректный юридический адрес организации (число запятых в адресе должно быть равно девяти)!'");
		...
		Иначе
			...
			Если ПустаяСтрока(СтрАдрес.Индекс) Тогда
				...
				Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში შენახულ ორგანიზაციის იურიდიულ მისამართში არ არის მითითებული ინდექსი!';ru='В юридическом адресе организации, хранящемся в информационной базе, не указан индекс!'");
			...
			Если ПустаяСтрока(СтрАдрес.Регион) Тогда
				...
				Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში შენახულ ორგანიზაციის იურიდიულ მისამართში არ არის მითითებული რეგიონი!';ru='В юридическом адресе организации, хранящемся в информационной базе, не указан регион!'");
...
Функция ПроверитьИОбработатьАдрГолОрг(Сведения)
	...
	Если Сведения.АдрЮр = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ka='სათაო ორგანიზაციის იურიდიული მისამართის დადგნა ვერ მოხერხდა!';ru='Не удалось определить юридический адрес головной организации!'");
	...
	Иначе
		Если СтрЧислоВхождений(Сведения.АдрЮр, ",") <> 9 Тогда
			...
			Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში მითითებულია სათაო ორგანიზაციის არაკორექტული იურიდიული მისამართი (მისამართში სასვენი მძიმის (,) რაოდენობა ცხრის ტოლი უნდა იყოს)!';ru='В информационной базе указан некорректный юридический адрес головной организации (число запятых в адресе должно быть равно девяти)!'");
		...
		Иначе
			...
			Если ПустаяСтрока(СтрАдрес.Индекс) Тогда
				...
				Сообщение.Текст = НСтр("ka='В юридическом адресе головной организации, хранящемся в информационной базе, не указан индекс!';ru='В юридическом адресе головной организации, хранящемся в информационной базе, не указан индекс!'");
			...
			Если ПустаяСтрока(СтрАдрес.Регион) Тогда
				...
				Сообщение.Текст = НСтр("ka='В юридическом адресе головной организации, хранящемся в информационной базе, не указан регион!';ru='В юридическом адресе головной организации, хранящемся в информационной базе, не указан регион!'");
...
Функция ВывестиСведенияОРуководствеОрганизации201(Текст, Орг)
	...
	Если ПустаяСтрока(Св.ФИОРук) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить ФИО руководителя организации!';ru='Не удалось определить ФИО руководителя организации!'");
	...
	И (Св.АдрМЖРукРф = "643,,,,,,,,,") Тогда
		...
		Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის გსნ-ის დადგენა ვერ მოხერხდა!';ru='Не удалось определить ИНН руководителя организации!'");
	...
	ИначеЕсли ЗначениеЗаполнено(Св.ИННРук) Тогда
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(Св.ИННРук, Истина) Тогда
			...
			Сообщение.Текст = НСтр("ka='ИНН руководителя организации не соответствует требованиям!';ru='ИНН руководителя организации не соответствует требованиям!'");
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Св.КодУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის პირადობის დამადასტურებელი დოკუმენტის ტიპის კოდის დადგენა ვერ მოხერხდა!';ru='Не удалось определить код вида документа, удостоверяющего личность руководителя организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.СерияИНомерУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ka='Не удалось определить серию и номер документа, удостоверяющего личность руководителя организации!';ru='Не удалось определить серию и номер документа, удостоверяющего личность руководителя организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.ДатаУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის პირადობის დამადასტურებელი მოწმობის გაცემის თარიღის დადგენა ვერ მოხერხდა!';ru='Не удалось определить дату выдачи удостоверения личности руководителя организации!'");
		...
		Если Св.АдрМЖРукРФ = "643,,,,,,,,," Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის საცხოვრებელი ადგილის დადგენა ვერ მოხერხდა!';ru='Не удалось определить адрес места жительства руководителя организации!'");
		...
		Иначе	
			...
			Иначе
				...
				Если ПустаяСтрока(СтрАдресРук.Индекс) Тогда
					...
					Сообщение.Текст = НСтр("ka='Не указан индекс в адресе места жительства руководителя организации!';ru='Не указан индекс в адресе места жительства руководителя организации!'");
				...
				Если ПустаяСтрока(СтрАдресРук.Регион) Тогда
					...
					Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის საცხოვრებელი ადგილის მისამართში რეგიონი არ არის მითითებული!';ru='Не указан регион в адресе места жительства руководителя организации!'");
	...
	Если ПустаяСтрока(Св.ФИОБух) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить ФИО главного бухгалтера организации!';ru='Не удалось определить ФИО главного бухгалтера организации!'");
	...
	И Св.АдрМЖБухРф = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить ИНН главного бухгалтера организации!';ru='Не удалось определить ИНН главного бухгалтера организации!'");
	...
	ИначеЕсли ЗначениеЗаполнено(Св.ИННБух) Тогда
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(Св.ИННБух, Истина) Тогда
			...
			Сообщение.Текст = НСтр("ka='ИНН главного бухгалтера организации не соответствует требованиям!';ru='ИНН главного бухгалтера организации не соответствует требованиям!'");
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Св.КодУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის პირადობის დამადასტურებელი დოკუმენტის სახეობის კოდის დადგენა ვერ მოხერხდა!';ru='Не удалось определить код вида документа, удостоверяющего личность главного бухгалтера организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.СерияИНомерУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის პირადობის დამადასტურებელი დოკუმენტის სერიის და ნომრის დადგენა ვერ მოხერხდა!';ru='Не удалось определить серию и номер документа, удостоверяющего личность главного бухгалтера организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.ДатаУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის პირადობის დამადასტურებელი მოწმობის გაცემის თარიღის დადგენა ვერ მოხერხდა!';ru='Не удалось определить дату выдачи удостоверения личности главного бухгалтера организации!'");
		...
		Если Св.АдрМЖБухРФ = "643,,,,,,,,," Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართის დადგენა ვერ მოხერხდა!';ru='Не удалось определить адрес места жительства главного бухгалтера организации!'");
		...
		Иначе	
			...
			Иначе
				...
				Если ПустаяСтрока(СтрАдресБух.Индекс) Тогда
					...
					Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართში ინდექსი არ არის მითითებული!';ru='Не указан индекс в адресе места жительства главного бухгалтера организации!'");
				...
				Если ПустаяСтрока(СтрАдресБух.Регион) Тогда
					...
					Сообщение.Текст = НСтр("ka='მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართში არ არის მითითებული რეგიონი!';ru='Не указан регион в адресе места жительства главного бухгалтера организации!'");
...
Процедура СообщитьОбОбработкеОтчета(Отчет, НомерОтчета, КоличествоОтчетов)
	...
	Текст 				= НСтр("ka='ანგარიშგების დამუშავება %1 (%3-დან %2 ) ...';ru='Обработка отчета %1 (%2 из %3) ...'");
...
Процедура ПроверитьОтчетыСервер(ОшибкаВыгрузки)
	...
	Если РежимВызоваЭкспортируемогоМетодаФормы <> Истина Тогда
		...
		Если НалоговыеОрганы.Количество() > 0 Тогда
			...
			Если НЕ НалоговыеОрганы.Количество() = НайденныеСтроки.Количество() Тогда
				...
				Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются коды ИФНС, в которую они должны представляться. Процесс прерван.';ru='У выбранных отчетов различаются коды ИФНС, в которую они должны представляться. Процесс прерван.'");
	...
	Если ЕстьНеопределенныеФорматы Тогда
		...
		Сообщение.Текст = НСтр("ka='შემდეგი ანგარიშგებებისთვის გადმოტვირთვა და შემოწმება არ არის გათვალისწინებული.';ru='Для следующих отчетов выгрузка и проверка не предусмотрены:'");
	...
	Если ОтчетыСохраненныеСНевернымОкруглением.Количество() <> 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='შეინახეთ ანგარიშგებები საჭირო საზომი ერთეულით და შემოწმება გაუშვით განმეორებით.';ru='Сохраните отчеты с нужной единицей измерения и запустите проверку повторно.'");
...
Процедура ПоказатьТекстВыгрузки(Команда)
	...
	Если Элементы.Основания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не задана текущая строка!';ru='Не задана текущая строка!'"));
		Возврат;
	КонецЕсли;
	...
	Если НепривязанныеТекстыВыгрузки.Количество() = 0 И Элементы.Основания.ТекущиеДанные.НомерСтрокиТекстаВыгрузки = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Для выбранного отчета текст выгрузки не сформирован!';ru='Для выбранного отчета текст выгрузки не сформирован!'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменениеПараметровОтбора(ВыполняемоеОповещение) 
	...
	Иначе
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეკვიზიტის შეცვლისას გასუფთავდება დოკუმენტების სია და გადმოტვირთვების ტექსტი.%1გავაგრძელოთ?';ru='При изменении реквизита будут очищены список документов и тексты выгрузок.%1Продолжить?'"), Символы.ПС);
...
Процедура ОснованияПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элементы.Основания.ТекущийЭлемент.Имя = "ОснованияПометка"
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';ru='При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?'"), Символы.ПС);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
