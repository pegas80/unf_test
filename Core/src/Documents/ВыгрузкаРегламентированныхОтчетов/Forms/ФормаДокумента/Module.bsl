...
Процедура СформироватьИЗаписать(ДокументыОснования, ТекстВыгрузки = Неопределено, ПечатаемыеЛисты = Неопределено, ФайлДляСохранения = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, КаталогСохранения = Неопределено, СообщенияОбОшибках = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина, ВыгружатьСОшибками = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, УникальныйИДФормы = Неопределено, ВыполняемоеОповещение = Неопределено, СтруктураДополнительныхПараметров = Неопределено) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), РегламентированнаяОтчетностьКлиент.СформироватьТекстСообщения(ОписаниеОшибки()));
		Сообщение.Сообщить();
...
Процедура Заполнить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не указана организация.';ru='Не указана организация.';en='Not specified organization.'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Выгрузки.Количество() <> 0 Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект);
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Тексты выгрузки будут очищены.%1Продолжить?';ru='Тексты выгрузки будут очищены.%1Продолжить?';en='upload texts will be cleared.% 1Prodolzhit?'"), Символы.ПС);
...
Процедура УстановитьСнятьФлажки(Пометка)
	...
	Если Объект.Выгрузки.Количество() <> 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("УстановитьСнятьФлажкиЗавершение", ЭтотОбъект, Новый Структура("Пометка", Пометка)), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';ru='При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';en='When you change the document list will be cleared unloading texts.% 1Prodolzhit?'"), Символы.ПС), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура СформироватьТекстыВыгрузки(Команда)
	...
	Если ОшибкаВыгрузки <> Истина Тогда
		ПоказатьПредупреждение(,НСтр("ka='Формирование текстов выгрузки успешно завершено.';ru='Формирование текстов выгрузки успешно завершено.';en='Formation of unloading texts successfully completed.'"));
	КонецЕсли;
...
Процедура СформироватьТекстыВыгрузкиКлиент(ПечатаемыеЛисты = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, ДляОтправкиВФНС = Ложь, ПервыйДок = Неопределено, СообщенияОбОшибках = Неопределено, ВыгружатьСОшибками = Ложь, СтруктураДополнительныхПараметров = Неопределено)
		...
		Для Каждого Стр Из ОтборФСС Цикл
			...
			Исключение
				...
				Если СообщенияОбОшибках = Неопределено Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось выгрузить %1.';ru='Не удалось выгрузить %1.';en='Failed to upload% 1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
					Сообщение.Сообщить();
	...
	Если Отбор300.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда	
		...
		Для Каждого Стр Из Отбор300 Цикл
			...
			Если НЕ РегламентированнаяОтчетностьВызовСервера.ВывестиСведенияСлужебнойЧастиИОбОтправителе30(Текст, Объект.Организация, КПП, СокрЛП(Объект.КодНалоговогоОргана)) Тогда 
				...
				Сообщение.Текст = НСтр("ka='Не удалось сформировать общую информацию для файла выгрузки формата 3.00.';ru='Не удалось сформировать общую информацию для файла выгрузки формата 3.00.';en='It failed to generate general information for uploading file format 3.00.'");
			...
			Если Стр.Отчет = "Статистика: Форма П-1" И Год(Стр.ДатаНачала) <= 2016 Тогда
				...
				Если ТекФормаП1Пр3 = Неопределено Тогда
					...
					Если НазваниеИсточникОтчета <> "Без приложения" Тогда
						ТекстСообщения = НСтр("ka='ვერ მოიძებნა შენახული ანგარიშგება ""სტატისტიკა: ფორმა პ-1 დანართი 3"" XML ფაილში გადმოსატვირთად - შესაბამისი ნაწილი ცარიელი იქნება.';ru='Не найден сохраненный отчет ""Статистика: Форма П-1 Приложение 3"" для выгрузки в файл XML - соответствующий раздел будет пустым.';en='Unable to find a saved report ""Statistics: Form F-1 Appendix 3"" to upload an XML file - the corresponding section will be empty.'");
				...
				Иначе
					...
					Исключение
						...
						Если СообщенияОбОшибках = Неопределено Тогда
							...
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось выгрузить %1.';ru='Не удалось выгрузить %1.';en='Failed to upload% 1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(СтрДокП1Пр3));
							Сообщение.Сообщить();
	...
	Если ВыгрузкаПрервана И СообщенияОбОшибках = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось сформировать тексты выгрузки.';ru='Не удалось сформировать тексты выгрузки.';en='Failed to generate discharge texts.'");
		Сообщение.Сообщить();
		
		Если ТаблицаСообщений.Количество() <> 0 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='შეცდომების გასასწორებლად გამოიყენეთ შეცდომებში ნავიგაციის ფანჯარა.';ru='Для исправления ошибок используйте окно навигации по ошибкам.';en='To correct the error, use the navigation window by mistake.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьСобытие1СОтчетностиНаСервере(ПервыйДок)
	...
	РегламентированнаяОтчетность.ОбработатьСобытие1СОтчетности(НСтр("ka='Регламентированный отчет. Выгрузка';ru='Регламентированный отчет. Выгрузка';en='Regulated report. Unloading'"), ПервыйДок);
...
Процедура СформироватьТекстыВыгрузкиСервер(ПервыйДок, ДляОтправкиВФНС, КПП, ОшибкаВыгрузки)
	...
	Если КоличествоДокументовДляВыгрузки = 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Не выбраны документы.';ru='Не выбраны документы.';en='Not selected documents.'");
	...
	Для Каждого Стр Из Основания Цикл
		Если Стр.Организация <> Основания.Получить(0).Организация Тогда
			...
			Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна.';ru='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна.';en='We selected different details of the report ""Organization"". Batch upload is not possible.'");
		...
		Если КонецДня(Стр.ДатаОкончания) <> КонецДня(Основания.Получить(0).ДатаОкончания) Тогда
			...
			Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна.';ru='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна.';en='We selected reports vary the date of the drawing up periods. Batch upload is not possible.'");
	...
	Если НалоговыеОрганы.Количество() > 0 Тогда
		...
		Если НЕ НалоговыеОрганы.Количество() = НайденныеСтроки.Количество() Тогда
			...
			Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются коды ИМНС, в которую они должны представляться. Процесс прерван.';ru='У выбранных отчетов различаются коды ИМНС, в которую они должны представляться. Процесс прерван.';en='We selected reports differ IMNS codes, which must be submitted. The process is interrupted.'");
	...
	Если ОтчетыВыгружаемыеТолькоИзФормы.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Выгрузка следующих отчетов возможна только из формы отчета:';ru='Выгрузка следующих отчетов возможна только из формы отчета:';en='Unloading of the following statements is possible only from the form of the report:'");
		...
		Для Каждого Док Из ОтчетыВыгружаемыеТолькоИзФормы Цикл
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' - %1';ka=' - %1'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Док));
	...
	Если ЕстьНеопределенныеФорматы Тогда
		...
		Сообщение.Текст = НСтр("ka='შემდეგი ანგარიშგებებისთვის გადმოტვირთვა არ არის გათვალისწინებული:';ru='Для следующих отчетов выгрузка не предусмотрена:';en='Unloading is not available for the following reports:'");
		...
		Для Каждого Стр Из ВерсииИОтчеты Цикл
			Если НЕ ЗначениеЗаполнено(Стр.Версия) Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' - %1';ka=' - %1'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
		...
		Сообщение.Текст = НСтр("ka='Продолжение невозможно. Формирование текстов выгрузки прервано.';ru='Продолжение невозможно. Формирование текстов выгрузки прервано.';en='It can not continue. Formation of unloading texts interrupted.'");
	...
	Если ДляОтправкиВФНС И ОтчетыНеотправляемыеВФНС.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='Следующие отчеты не предназначены для отправки в ФНС по каналам связи:';ru='Следующие отчеты не предназначены для отправки в ФНС по каналам связи:';en='The following reports are not intended to be sent to the Federal Tax Service of communication channels:'");
		...
		Для Каждого ОтчетНеотправляемыйВФНС Из ОтчетыНеотправляемыеВФНС Цикл
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' - %1';ka=' - %1'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(ОтчетНеотправляемыйВФНС));
	...
	Если ОтчетыСохраненныеСНевернымОкруглением.Количество() <> 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='შემდეგი ანგარიშგებები შენახულია საზომი ერთეულით და/ან იმ სიზუსტით, რომელიც გადმოტვირთვის ფორმატის მოთხოვნებს არ შეესაბამება:';ru='Следующие отчеты сохранены с единицей измерения и/или точностью, не соответствующими требованиям формата выгрузки:';en='The following reports are saved to the unit of measure and / or precision is not appropriate discharge format requirements:'");
		...
		Для Каждого Сч Из ОтчетыСохраненныеСНевернымОкруглением Цикл
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1';ru='%1'"), ТекстСообщения);
		...
		Сообщение.Текст = НСтр("ka='შეინახეთ ანგარიშგებები საჭირო საზომი ერთეულით და ხელახლა გაუშვით ჩამოყალიბების პროცესი.';ru='Сохраните отчеты с нужной единицей измерения и перезапустите процесс формирования.';en='Save the report with the desired unit of measure, and then restart the process of formation.'");
...
Функция ВывестиОписаниеФормыОтчетности(Текст, Организация, КодНалоговогоОргана)
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ВывестиСведенияСлужебнойЧастиИОбОтправителе30(Текст, Организация, , СокрЛП(КодНалоговогоОргана), "2.01") Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось сформировать общую информацию для файла выгрузки формата 2.01.';ru='Не удалось сформировать общую информацию для файла выгрузки формата 2.01.';en='It failed to generate general information for uploading file format 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ВывестиСведенияИнформационнойЧасти201ИСведенияОбОрганизации(Текст, Организация) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось выгрузить сведения об организации для файла выгрузки формата 2.01.';ru='Не удалось выгрузить сведения об организации для файла выгрузки формата 2.01.';en='Unable to upload information about your organization to upload the file format 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ВывестиСведенияОРуководствеОрганизации201(Текст, Организация) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='Не удалось выгрузить сведения о руководстве организации для файла выгрузки формата 2.01.';ru='Не удалось выгрузить сведения о руководстве организации для файла выгрузки формата 2.01.';en='Unable to upload information about the management organization to upload the file format 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Процедура СохранитьТексты(НЕЗаписыватьДокумент,
	...
	Если Объект.Выгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='შესანახად არ არის გადმოტვირთვის ტექსტები!';ru='Нет текстов выгрузки для сохранения!';en='No upload texts to save!'"));
		ОшибкаВыгрузки = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЧислоВыгружаемыхОтчетов = 0 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შესანახად ინფორმაცია არ არის!';ru='Нет информации для сохранения!';en='There is no information to save!'");
		Сообщение.Сообщить();
			...
			Если ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
							...
							Если СтрНайти(ТекСтр.ИмяФайла, "NO_BUHOTCH_") = 1
								...
								Если ТекСтр.Тип <> ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСГС Тогда
									...
									Если СписокДополнительныхФайлов.Количество() > 0 Тогда
										...
										Сообщение = Новый СообщениеПользователю;
										Сообщение.Текст = НСтр("ka='საბუღალტრო ანგარიშგებულობის გადმოტვირთვის ფაილს დაურთეთ:';ru='К файлу выгрузки бухгалтерской отчетности приложите:';en='To file upload financial statements apply:'");
										Сообщение.Сообщить();
										...
										Для Каждого ДополнительныйФайл Из СписокДополнительныхФайлов Цикл
											...
											НСтр("ka='%1';ru='%1'"), ДополнительныйФайл.Представление);
											Сообщение.Сообщить();
											
											Сообщение = Новый СообщениеПользователю;
											Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ka='%1';ru='%1'"), ДополнительныйФайл.Значение);
											Сообщение.Сообщить();
				...
				Исключение
					...
					Если ФайлВыгрузкиВоВременномХранилище = Неопределено Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось записать файл ""%1"". Возможно, недостаточно места на диске или диск защищен от записи.';ru='Не удалось записать файл ""%1"". Возможно, недостаточно места на диске или диск защищен от записи.';en='Unable to write file ""%1"". Perhaps not enough space on the disk or the disk is write-protected.'"), ИмяКонечногоФайла);
						
						#Если ВебКлиент Тогда
							Если НЕ ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно Тогда
								Сообщение.Текст = Сообщение.Текст + НСтр("ka=' Также, возможно, не подключено расширение для работы с файлами.';ru=' Также, возможно, не подключено расширение для работы с файлами.'");
							КонецЕсли;
						#КонецЕсли
						
					Иначе
						
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დროებით საცავში გადმოტვირთვის ფაილ ""%1""-ის მოთავსება ვერ მოხერხდა!';ru='Не удалось поместить файл выгрузки ""%1"" во временное хранилище!';en='Unable to place the unloading file ""%1"" for temporary storage!'"), ИмяКонечногоФайла);
	...
	Если ВыгруженХотяБыОдинФайл Тогда
		...
		Если БылаОшибкаЗаписи = Истина Тогда
			Если НЕ РежимВызоваЭкспортируемогоМетодаФормы Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='В процессе записи отчетов произошла ошибка!';ru='В процессе записи отчетов произошла ошибка!';en='The error occurred during the recording of reports!'");
				Сообщение.Сообщить();
					...
					#Иначе
						...
						Если МассивПередаваемыхФайловКомплекта.Количество() > 0 Тогда
							...
							Иначе
								...
								ТекстПояснения = НСтр("ka='მიმდინარეობს კომპლექტის ფაილების შენახვა. გთხოვთ, მოიცადოთ.';ru='Выполняется сохранение файлов комплекта."
...
Функция ОпределитьИмяФайлаВыгрузкиФССПоТекстуВыгрузки(ТекстВыгрузки)
	...
	Если ДеревоXML = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить имя файла выгрузки ФСС по тексту выгрузки.';ru='Не удалось определить имя файла выгрузки ФСС по тексту выгрузки.';en='Unable to determine the file name of the FSS discharge for discharging the text.'");
...
Функция ВывестиСведенияИнформационнойЧасти201ИСведенияОбОрганизации(Текст, Орг)
	...
	Если Св.ТипНП = 1 Тогда//российская организация
		...
		Если НЕ ЗначениеЗаполнено(НаимЮЛПол) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для организации не задано полное наименование!';ru='Для организации не задано полное наименование!';en='For organizations not specified full name!'");
		...
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ИННЮЛ, Ложь) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციისთვის არასწორად არის მიცემული ს.კ / პ.ნ!';ru='Для организации неверно задан ИНН!';en='For the organization is set incorrectly INN!'");
		...
		Если НЕ ПустаяСтрока(ОГРН) И НЕ РегламентированнаяОтчетность.ОГРНСоответствуетТребованиям(ОГРН) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციისთთვის არასწორად არის მიცემული ძსსნ!';ru='Для организации неверно задан ОГРН!';en='For the organization is set incorrectly BIN!'");
	...
	ИначеЕсли Св.ТипНП = 2 Тогда//обособленное подразделение
		...
		Если НЕ ЗначениеЗаполнено(НаимЮЛПол) Тогда
			...
			Сообщение.Текст = НСтр("ka='სათაო ორგანიზაციისთვის სრული სახელწოდება არ არის მიცემული!';ru='Для головной организации не задано полное наименование!';en='For the parent company is not given the full name!'");
		...
		Если НЕ ЗначениеЗаполнено(НаимОППол) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для обособленного подразделения не задано полное наименование!';ru='Для обособленного подразделения не задано полное наименование!';en='To separate division is not specified full name!'");
	...
	ИначеЕсли Св.ТипНП = 3 Тогда//ИП
		...
		Если НЕ ЗначениеЗаполнено(Св.ФИО) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для индивидуального предпринимателя не задано ФИО!';ru='Для индивидуального предпринимателя не задано ФИО!';en='For an individual entrepreneur is not defined Name!'");
		...
		Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ИННФЛ) Тогда
			...
			Сообщение.Текст = НСтр("ka='ინდმეწარმისთვის არასწორად არის მიცემული ს.კ / პ.ნ!';ru='Для индивидуального предпринимателя неверно задан ИНН!';en='For an individual entrepreneur is set incorrectly INN!'");
	...
	ИначеЕсли Св.ТипНП = 5 Тогда//иностранная организация
		...
		Если НЕ ЗначениеЗаполнено(НаимОИОПол) Тогда
			...
			Сообщение.Текст = НСтр("ka='უცხოური ორგანიზაციის განყოფილებისთვის არ არის მიცემული სრული სახელწოდება!';ru='Для отделения иностранной организации не задано полное наименование!';en='For a branch of a foreign company is not given the full name!'");
		...
		Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ИННИО) Тогда
			...
			Сообщение.Текст = НСтр("ka='Для иностранной организации неверно задан ИНН!';ru='Для иностранной организации неверно задан ИНН!';en='For a foreign company is set incorrectly INN!'");
...
Функция ПроверитьИОбработатьАдрЮр(Сведения)
	...
	Если Сведения.АдрЮр = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить юридический адрес организации!';ru='Не удалось определить юридический адрес организации!';en='Unable to determine the legal address of the organization!'");
	...
	Иначе
		Если СтрЧислоВхождений(Сведения.АдрЮр, ",") <> 9 Тогда
			...
			Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში მითითებულია ორგანიზაციის არაკორექტული იურიდიული მისამართი (მისამართში მძიმის (,) რაოდენობა უნდა უდრიდეს ცხრას)!';ru='В информационной базе указан некорректный юридический адрес организации (число запятых в адресе должно быть равно девяти)!';en='The informational basis of these flawed legal address of the organization (number of commas in the address must be equal to nine)!'");
		...
		Иначе
			...
			Если ПустаяСтрока(СтрАдрес.Индекс) Тогда
				...
				Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში შენახულ ორგანიზაციის იურიდიულ მისამართში არ არის მითითებული ინდექსი!';ru='В юридическом адресе организации, хранящемся в информационной базе, не указан индекс!';en='The legal address of the organization that is stored in the information database, Unknown Index!'");
			...
			Если ПустаяСтрока(СтрАдрес.Регион) Тогда
				...
				Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში შენახულ ორგანიზაციის იურიდიულ მისამართში არ არის მითითებული რეგიონი!';ru='В юридическом адресе организации, хранящемся в информационной базе, не указан регион!';en='The legal address of the organization, the information stored in the base region is not included!'");
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(КодРегиона) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ორგანიზაციის იურიდიული მისამართიდან გამოყოფილი სახელწოდებით რეგიონის კოდის დადგენა ვერ მოხერხდა.%1შეამოწმეთ რეგიონის მითითების სისწორე იურიდიულ მისამართში!';ru='Не удалось определить код региона по его названию, выделенному из юридического адреса организации.%1Проверьте правильность указания названия региона в юридическом адресе!';en='Unable to determine the region code on its name, selected out of the legal address of the organization. 1Make% correctly indicating the name of the region in the legal address!'"), Символы.ПС);
...
Функция ПроверитьИОбработатьАдрГолОрг(Сведения)
	...
	Если Сведения.АдрЮр = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ka='სათაო ორგანიზაციის იურიდიული მისამართის დადგნა ვერ მოხერხდა!';ru='Не удалось определить юридический адрес головной организации!';en='Unable to determine the legal address of the parent organization!'");
	...
	Иначе
		Если СтрЧислоВхождений(Сведения.АдрЮр, ",") <> 9 Тогда
			...
			Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში მითითებულია სათაო ორგანიზაციის არაკორექტული იურიდიული მისამართი (მისამართში სასვენი მძიმის (,) რაოდენობა ცხრის ტოლი უნდა იყოს)!';ru='В информационной базе указан некорректный юридический адрес головной организации (число запятых в адресе должно быть равно девяти)!';en='The informational basis of these flawed legal address of the parent company (the number of commas in the address must be equal to nine)!'");
		...
		Иначе
			...
			Если ПустаяСтрока(СтрАдрес.Индекс) Тогда
				...
				Сообщение.Текст = НСтр("ka='В юридическом адресе головной организации, хранящемся в информационной базе, не указан индекс!';ru='В юридическом адресе головной организации, хранящемся в информационной базе, не указан индекс!';en='The legal address of the parent organization, stored in the information database, Unknown Index!'");
			...
			Если ПустаяСтрока(СтрАдрес.Регион) Тогда
				...
				Сообщение.Текст = НСтр("ka='В юридическом адресе головной организации, хранящемся в информационной базе, не указан регион!';ru='В юридическом адресе головной организации, хранящемся в информационной базе, не указан регион!';en='The legal address of the parent organization, the information stored in the base region is not included!'");
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(КодРегиона) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='Не удалось определить код региона по его названию, выделенному из юридического адреса головной организации.%1Проверьте правильность указания названия региона в юридическом адресе головной организации!';ru='Не удалось определить код региона по его названию, выделенному из юридического адреса головной организации.%1Проверьте правильность указания названия региона в юридическом адресе головной организации!';en='Unable to determine the region code on its name, legal address of the selected parent organization. 1Make% correctly indicating the name of the region in the legal address of the parent organization!'"), Символы.ПС);
...
Функция ВывестиСведенияОРуководствеОрганизации201(Текст, Орг)
	...
	Если ПустаяСтрока(Св.ФИОРук) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить ФИО руководителя организации!';ru='Не удалось определить ФИО руководителя организации!';en='Unable to determine the full name of the organization!'");
	...
	И (Св.АдрМЖРукРф = "643,,,,,,,,,") Тогда
		...
		Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის გსნ-ის დადგენა ვერ მოხერხდა!';ru='Не удалось определить ИНН руководителя организации!';en='Unable to determine INN head of the organization!'");
	...
	ИначеЕсли ЗначениеЗаполнено(Св.ИННРук) Тогда
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(Св.ИННРук, Истина) Тогда
			...
			Сообщение.Текст = НСтр("ka='ИНН руководителя организации не соответствует требованиям!';ru='ИНН руководителя организации не соответствует требованиям!';en='INN head of the organization does not meet the requirements!'");
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Св.КодУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის პირადობის დამადასტურებელი დოკუმენტის ტიპის კოდის დადგენა ვერ მოხერხდა!';ru='Не удалось определить код вида документа, удостоверяющего личность руководителя организации!';en='Unable to determine the code type of the document certifying the identity of the head of organization!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.СерияИНомерУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ka='Не удалось определить серию и номер документа, удостоверяющего личность руководителя организации!';ru='Не удалось определить серию и номер документа, удостоверяющего личность руководителя организации!';en='Unable to determine the series and number of the document certifying the identity of the head of organization!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.ДатаУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის პირადობის დამადასტურებელი მოწმობის გაცემის თარიღის დადგენა ვერ მოხერხდა!';ru='Не удалось определить дату выдачи удостоверения личности руководителя организации!';en='Unable to determine the date of issue of the head of the organization identity card!'");
		...
		Если Св.АдрМЖРукРФ = "643,,,,,,,,," Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის საცხოვრებელი ადგილის დადგენა ვერ მოხერხდა!';ru='Не удалось определить адрес места жительства руководителя организации!';en='Unable to determine the address of residence head of the organization!'");
		...
		Иначе	
			Если СтрЧислоВхождений(Св.АдрМЖРукРФ, ",") <> 9 Тогда
				...
				Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში მითითებულია ორგანიზაციის ხელმძღვანელის საცხოვრებელი ადგილის არაკორექტული მისამართი (სასვენი მძიმის რაოდენობა მისამართში ცხრის ტოლი უნდა იყოს)!';ru='В информационной базе указан некорректный адрес места жительства руководителя организации (число запятых в адресе должно быть равно девяти)!';en='The informational basis of the place of residence specified an incorrect address head of the organization (number of commas in the address must be equal to nine)!'");
			...
			Иначе
				...
				Если ПустаяСтрока(СтрАдресРук.Индекс) Тогда
					...
					Сообщение.Текст = НСтр("ka='Не указан индекс в адресе места жительства руководителя организации!';ru='Не указан индекс в адресе места жительства руководителя организации!';en='Unknown code in the address of residence head of the organization!'");
				...
				Если ПустаяСтрока(СтрАдресРук.Регион) Тогда
					...
					Сообщение.Текст = НСтр("ka='ორგანიზაციის ხელმძღვანელის საცხოვრებელი ადგილის მისამართში რეგიონი არ არის მითითებული!';ru='Не указан регион в адресе места жительства руководителя организации!';en='Unknown region residence address head of the organization!'");
				...
				Иначе
					...
					Если Не ЗначениеЗаполнено(КодРегионаРук) Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეგიონის კოდის დადგენა ორგანიზაციის ხელმძღვანელის საცხოვრებელი ადგილის მისამართიდან გამოყოფილი მისი სახელწოდების მიხედვით ვერ მოხერხდა.%1შეამოწმეთ ხელმძღვანელის საცხოვრებელი ადგილის მისამართში რეგიონის სახელწოდების მითითების სისწორე!';ru='Не удалось определить код региона по его названию, выделенному из адреса места жительства руководителя организации.%1Проверьте правильность указания названия региона в адресе места жительства руководителя!';en='Unable to determine the region its name code of the selected place of residence of the head of the organization. 1Make% correctly indicating the name of the region in the address of residence head!'"), Символы.ПС);
	...
	Если ПустаяСтрока(Св.ФИОБух) Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить ФИО главного бухгалтера организации!';ru='Не удалось определить ФИО главного бухгалтера организации!';en='Unable to determine the chief accountant of the company name!'");
	...
	И Св.АдрМЖБухРф = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ka='Не удалось определить ИНН главного бухгалтера организации!';ru='Не удалось определить ИНН главного бухгалтера организации!';en='Unable to determine INN chief accountant!'");
	...
	ИначеЕсли ЗначениеЗаполнено(Св.ИННБух) Тогда
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(Св.ИННБух, Истина) Тогда
			...
			Сообщение.Текст = НСтр("ka='ИНН главного бухгалтера организации не соответствует требованиям!';ru='ИНН главного бухгалтера организации не соответствует требованиям!';en='INN organization does not comply with the chief accountant!'");
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Св.КодУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის პირადობის დამადასტურებელი დოკუმენტის სახეობის კოდის დადგენა ვერ მოხერხდა!';ru='Не удалось определить код вида документа, удостоверяющего личность главного бухгалтера организации!';en='Unable to determine the type code of the document certifying the identity of the chief accountant of the organization!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.СерияИНомерУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის პირადობის დამადასტურებელი დოკუმენტის სერიის და ნომრის დადგენა ვერ მოხერხდა!';ru='Не удалось определить серию и номер документа, удостоверяющего личность главного бухгалтера организации!';en='Unable to determine the series and number of the document certifying the identity of the chief accountant of the organization!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.ДатаУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის პირადობის დამადასტურებელი მოწმობის გაცემის თარიღის დადგენა ვერ მოხერხდა!';ru='Не удалось определить дату выдачи удостоверения личности главного бухгалтера организации!';en='Unable to determine the date of the issuance of identity cards chief accountant!'");
		...
		Если Св.АдрМЖБухРФ = "643,,,,,,,,," Тогда
			...
			Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართის დადგენა ვერ მოხერხდა!';ru='Не удалось определить адрес места жительства главного бухгалтера организации!';en='Unable to determine the address of residence chief accountant!'");
		...
		Иначе	
			Если СтрЧислоВхождений(Св.АдрМЖБухРФ, ",") <> 9 Тогда
				...
				Сообщение.Текст = НСтр("ka='საინფორმაციო ბაზაში მითითებულია ორგანიზაციის მთავარი ბუღალტერის საცხოვრებელი ადგილის არასწორი მისამართი (მისამართში მძიმის (,) რაოდენობა ცხრას უნდა უდრიდეს)!';ru='В информационной базе указан некорректный адрес места жительства главного бухгалтера организации (число запятых в адресе должно быть равно девяти)!';en='The informational basis of the chief accountant of the organization of the incorrect address of residence (the number of commas in the address must be equal to nine)!'");
			...
			Иначе
				...
				Если ПустаяСтрока(СтрАдресБух.Индекс) Тогда
					...
					Сообщение.Текст = НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართში ინდექსი არ არის მითითებული!';ru='Не указан индекс в адресе места жительства главного бухгалтера организации!';en='Unknown code in the address of residence chief accountant!'");
				...
				Если ПустаяСтрока(СтрАдресБух.Регион) Тогда
					...
					Сообщение.Текст = НСтр("ka='მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართში არ არის მითითებული რეგიონი!';ru='Не указан регион в адресе места жительства главного бухгалтера организации!';en='Unknown region in the address of residence chief accountant!'");
				...
				Иначе
					...
					Если Не ЗначениеЗаполнено(КодРегионаБух) Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ორგანიზაციის მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართიდან გამოყოფილი რეგიონის კოდის დადგენა მისი სახელწოდებით ვერ მოხერხდა.%1შეამოწმეთ რეგიონის სახელწოდების მითითების სისწორე მთავარი ბუღალტრის საცხოვრებელი ადგილის მისამართში!';ru='Не удалось определить код региона по его названию, выделенному из адреса места жительства главного бухгалтера организации.%1Проверьте правильность указания названия региона в адресе места жительства главного бухгалтера!';en='Unable to determine the region code by its name, the address of the selected place of residence of the chief accountant of the organization. 1Make% correctly indicating the name of the region as a place of residence of the chief accountant!'"), Символы.ПС);
...
Процедура СообщитьОбОбработкеОтчета(Отчет, НомерОтчета, КоличествоОтчетов)
	...
	Текст 				= НСтр("ka='ანგარიშგების დამუშავება %1 (%3-დან %2 ) ...';ru='Обработка отчета %1 (%2 из %3) ...';en='Report Processing 1% (% 2% 3), ...'");
...
Процедура ПроверитьОтчетыСервер(ОшибкаВыгрузки)
	...
	Если РежимВызоваЭкспортируемогоМетодаФормы <> Истина Тогда
		...
		Если НалоговыеОрганы.Количество() > 0 Тогда
			...
			Если НЕ НалоговыеОрганы.Количество() = НайденныеСтроки.Количество() Тогда
				...
				Сообщение.Текст = НСтр("ka='У выбранных отчетов различаются коды ИФНС, в которую они должны представляться. Процесс прерван.';ru='У выбранных отчетов различаются коды ИФНС, в которую они должны представляться. Процесс прерван.';en='We selected reports differ in the IRS code, which must be submitted. The process is interrupted.'");
	...
	Если ЕстьНеопределенныеФорматы Тогда
		...
		Сообщение.Текст = НСтр("ka='შემდეგი ანგარიშგებებისთვის გადმოტვირთვა და შემოწმება არ არის გათვალისწინებული.';ru='Для следующих отчетов выгрузка и проверка не предусмотрены:';en='For the following statements is not unloading and checking are provided:'");
	...
	Если ОтчетыСохраненныеСНевернымОкруглением.Количество() <> 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='შეინახეთ ანგარიშგებები საჭირო საზომი ერთეულით და შემოწმება გაუშვით განმეორებით.';ru='Сохраните отчеты с нужной единицей измерения и запустите проверку повторно.';en='Save the report with the desired unit of measure and run the test again.'");
...
Процедура ПоказатьТекстВыгрузки(Команда)
	...
	Если Элементы.Основания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ka='Не задана текущая строка!';ru='Не задана текущая строка!';en='Do not set the current line!'"));
		Возврат;
	КонецЕсли;
	...
	Если НепривязанныеТекстыВыгрузки.Количество() = 0 И Элементы.Основания.ТекущиеДанные.НомерСтрокиТекстаВыгрузки = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Для выбранного отчета текст выгрузки не сформирован!';ru='Для выбранного отчета текст выгрузки не сформирован!';en='For the selected text of the report is not generated discharge!'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменениеПараметровОтбора(ВыполняемоеОповещение) 
	...
	Иначе
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='რეკვიზიტის შეცვლისას გასუფთავდება დოკუმენტების სია და გადმოტვირთვების ტექსტი.%1გავაგრძელოთ?';ru='При изменении реквизита будут очищены список документов и тексты выгрузок.%1Продолжить?';en='If you change the props will be cleared list of documents and texts landings.% 1Prodolzhit?'"), Символы.ПС);
...
Процедура ОснованияПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элементы.Основания.ТекущийЭлемент.Имя = "ОснованияПометка"
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';ru='При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';en='When you change the document list will be cleared unloading texts.% 1Prodolzhit?'"), Символы.ПС);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
