...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	ЗаголовокПредставление = НСтр("ka='დაგზავნა';ru='Рассылка';en='delivery'");
	
	Если Данные.ПометкаУдаления Тогда
		Состояние = НСтр("ka='(წაშლილია)';ru='(удалена)';en='(Deleted)'");
	Иначе
		Состояние = "(" + НРег(Данные.Состояние) + ")";
	КонецЕсли;
	
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1 %2: %3 %4 %5';ru='%1 %2: %3 %4 %5';en='% 1% 2% 3% 4% 5'"),
