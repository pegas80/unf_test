...
"Продолжить?';ka='დაგზავნა შესრულდება %1-ისთვის. გავაგრძელოთ?'"),
					СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
						Объект.Получатели.Количество(), НСтр("ka='მიმღებს,მიმღებებს,მიმღებებს';ru='получателя,получателей,получателей';en='the recipient, the recipient, the recipient'")));
	
	ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.Заголовок = НСтр("ka='დაგზავნის დადასტურება';ru='Подтверждение рассылки';en='sending confirmation'");
	...
	Если Записать() Тогда
		...
		Иначе
			...
			ИначеЕсли ДоступноПравоНастройкиSMS Тогда
				ТекстСообщения = НСтр("ka='SMS-ის გასაგზავნად საჭიროა გაგზავნის პარამეტრების გამართვა."
"გამართვა ხორციელდება შემდეგნაირად: უფრო მეტი შესაძლებლობა - ადმინისტრირება - ფოსტა, SMS, შეხსენებები - SMS-ის გაგზავნის გამართვა.';ru='Для отправки SMS требуется настройка параметров отправки."
"Настройка осуществляется в Еще больше возможностей - Администрирование - Почта, SMS, напоминания - Настройка отправки SMS.'");
				ПоказатьПредупреждение(, ТекстСообщения);
				Возврат;
			Иначе
				ТекстСообщения = НСтр("ka='SMS-ს გასაგზავნად საჭიროა გაგზავნის პარამეტრების გამართვა. გამართვების შესასრულებლად მიმართეთ ადმინისტრატორს.';ru='Для отправки SMS требуется настройка параметров отправки."
"Для выполнения настроек обратитесь к администратору.'");
		...
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='წარმატებით გაიგზავნა: %1 შეტყობინება';ru='Успешно отправлено: %1 сообщений';en='Successfully sent:% 1 posts'"), УспешноОтправленных);
...
Функция ОтправитьРассылкуEmail()
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ელფოსტის გაგზავნა ვერ მოხერხდა მიმღებისთვის: %1, შემდეგი მიზეზით: %2';ru='Не удалось отправить E-mail получателю: %1, по причине: %2';en='Failed to send E-mail recipient: 1%, due to:% 2'"),
...
Функция ОтправитьРассылкуSMS()
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		...
		Если ПустаяСтрока(РезультатОтправки.ОписаниеОшибки) Тогда
			...
			УспешноОтправленных = УспешноОтправленных + 1;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='SMS-ის გაგზავნა ვერ მოხერხდა მიმღებისთვის: %1, შემდეგი მიზეზით: %2';ru='Не удалось отправить SMS получателю: %1, по причине: %2';en='Failed to send a SMS to the recipient: 1%, due to:% 2'"),
				СтрокаПолучатели.Контакт,
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ru='Загрузка получателей из файла';en='Loading recipients from file'"));
