...
"Продолжить?';ka='დაგზავნა შესრულდება %1-ისთვის. გავაგრძელოთ?'"),
					СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
						Объект.Получатели.Количество(), НСтр("ka='მიმღებს,მიმღებებს,მიმღებებს';ru='получателя,получателей,получателей'")));
	
	ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.Заголовок = НСтр("ka='დაგზავნის დადასტურება';ru='Подтверждение рассылки'");
...
"Для выполнения настроек обратитесь к администратору.'");
		...
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='წარმატებით გაიგზავნა: %1 შეტყობინება';ru='Успешно отправлено: %1 сообщений'"), УспешноОтправленных);
...
Функция ОтправитьРассылкуEmail()
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='ელფოსტის გაგზავნა ვერ მოხერხდა მიმღებისთვის: %1, შემდეგი მიზეზით: %2';ru='Не удалось отправить E-mail получателю: %1, по причине: %2'"),
...
Функция ОтправитьРассылкуSMS()
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		...
		Если ПустаяСтрока(РезультатОтправки.ОписаниеОшибки) Тогда
			...
			УспешноОтправленных = УспешноОтправленных + 1;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='SMS-ის გაგზავნა ვერ მოხერხდა მიმღებისთვის: %1, შემდეგი მიზეზით: %2';ru='Не удалось отправить SMS получателю: %1, по причине: %2'"),
				СтрокаПолучатели.Контакт,
