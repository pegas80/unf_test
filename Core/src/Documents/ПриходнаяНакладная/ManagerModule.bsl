...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("УдержаниеПенсионногоФонда",	НСтр("ka='ორგანიზაციის ხარჯი საპენსიო ფონდისთვის';ru='Удержание организации для пенсионного фонда'"));//Паганини ПФ
...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПриемЗапасовВозврат", НСтр("ka='მარაგების მიღება';ru='Прием запасов'"));
...
Процедура СформироватьТаблицаСуммовойУчетВРознице(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПоступлениеВРозницу", НСтр("ka='შემოსვლა საცალო გაყიდვაში';ru='Поступление в розницу'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	//Запрос.УстановитьПараметр("СторнированиеЗапасов", НСтр("ka='თვითღირებულების სტორნირება';ru='Сторнирование себестоимости'"));
	//Запрос.УстановитьПараметр("СторнированиеВыручки", НСтр("ka='მოგების სტორნირება';ru='Сторнирование выручки'"));
	//Запрос.УстановитьПараметр("Наценка", НСтр("ka='ფასნამატი';ru='Наценка'"));
	//Запрос.УстановитьПараметр("ПоложительнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Управленческий.ПрочиеДоходы);
	//Запрос.УстановитьПараметр("ОтрицательнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Управленческий.ПрочиеРасходы);
	//Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
	//Запрос.УстановитьПараметр("ВидОперации", ДокументСсылкаПриходнаяНакладная.ВидОперации);
	//Запрос.УстановитьПараметр("НачислениеНДФЛ", НСтр("ka='Начисление НДФЛ';ru='Начисление НДФЛ'"));
	//
	//РезультатЗапроса = Запрос.Выполнить();
	//Выборка = РезультатЗапроса.Выбрать();
	//
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Штрихкод", НСтр("ka='შტრიხ კოდი';ru='Штрихкод'"), ОписаниеТиповСтрока200, ОписаниеТиповКолонка, "Номенклатура", 1, , Истина, ЭтоЗапасы);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартии") Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Партия",  НСтр("ka='პარტია (დასახელება)';ru='Партия (наименование)'"), ОписаниеТиповСтрока150, ОписаниеТиповКолонка);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Заказ", НСтр("ka='შეკვეთა (მყიდველი/მომწოდებელი)';ru='Заказ (покупатель/поставщик)'"), ОписаниеТиповСтрока50, ОписаниеТиповКолонка);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='შემოსვლის ზედნადები (მომსახურებებით)';ru='Приходная накладная (с услугами)'");
	...
	КомандаПечати.Представление = НСтр("ka='შემოსავლის ზედდებული';ru='Приходная накладная'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "НакладнаяВРозничныхЦенах";
	КомандаПечати.Представление = НСтр("ka='შემოსავლის ზედდებული (საცალო ფასებში)';ru='Приходная накладная (в розничных ценах)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	Если ПравоДоступа("Просмотр", Метаданные.Обработки.ПечатьЭтикетокИЦенников)
		...
		КомандаПечати.Представление = НСтр("ka='ეტიკეტების ბეჭდვა';ru='Печать этикеток'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		...
		КомандаПечати.Представление = НСтр("ka='ფასდებულების ბეჭდვა';ru='Печать ценников'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.EPC) Тогда
			...
			Исключение
				ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
					СообщениеXML,
					СтрШаблон(НСтр("ka='ნომენკლატურა %1-ისთვის მითითებულია არაკორექტული EPC ""%2"".';ru='Для номенклатуры %1 указан некорректный EPC ""%2"".'"),
