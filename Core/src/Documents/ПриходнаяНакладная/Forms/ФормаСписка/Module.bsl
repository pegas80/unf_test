...
Процедура СоздатьПриходнуюНакладную(Команда)
	...
	Если Элементы.СписокЗаказыПоставщикам.ТекущиеДанные = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ka='ბრძანება არ შეიძლება შესრულდეს მითითებული ობიექტისთვის!';ru='Команда не может быть выполнена для указанного объекта!'");
	...
	Иначе
		...
		Если СтруктураДанных.СформироватьНесколькоЗаказов Тогда
			...
			ТекстСообщения = НСтр("ka='შეკვეთები განსხვავდება დოკუმენტების თავსართის მონაცემებით (%ПредставлениеДанных%)! ჩამოვაყალიბოთ რამდენიმე შემოსვლის ზედნადები?';ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько приходных накладных?'");
...
Процедура СоздатьПриходнуюНакладнуюЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если Ответ = КодВозвратаДиалога.Да Тогда
        ...
        Текст = НСтр("ka='შექმნა:';ru='Создание:'");
