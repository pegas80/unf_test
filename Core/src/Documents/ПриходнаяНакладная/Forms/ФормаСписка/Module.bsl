...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФормаПроцентМаржи.Видимость = Константы.ИнформацияОПрибылиСогласноНалоговомуДокументу.Получить();// Kuzmich, 09.03.2019, Информация о прибыли в форму списка Приходная накладная, исправление
	//Паганини_
...
Процедура СоздатьПриходнуюНакладную(Команда)
	...
	Иначе
		...
		Если СтруктураДанных.СформироватьНесколькоЗаказов Тогда
			...
			ТекстСообщения = НСтр("ka='შეკვეთები განსხვავდება დოკუმენტების თავსართის მონაცემებით (%ПредставлениеДанных%)! ჩამოვაყალიბოთ რამდენიმე შემოსვლის ზედნადები?';ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько приходных накладных?';en='Orders different data (% PredstavlenieDannyh%) hats documents! Form several Invoice?'");
...
Процедура СоздатьПриходнуюНакладнуюЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если Ответ = КодВозвратаДиалога.Да Тогда
        ...
        Текст = НСтр("ka='შექმნა:';ru='Создание:';en='Creature:'");
...
// Kuzmich, 07.03.2019, Информация о прибыли в форму списка Приходная накладная
#Область ИнформацияОПрибыли
&НаКлиенте
Процедура ПроцентМаржи(Команда)
	ПараметрыФормы = Новый Структура("МассивДокументов",Элементы.Список.ВыделенныеСтроки);
	ОткрытьФормуМодально("Документ.ПриходнаяНакладная.Форма.ИнформацияОПрибыли", ПараметрыФормы);
КонецПроцедуры
#КонецОбласти
// Kuzmich, 07.03.2019, Информация о прибыли в форму списка Приходная накладная