...
&НаКлиенте
Перем КопированиеСтрокиЗатратыНаИмущество1;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда 
		ПересчитатьСуммыСтрокиРасхода(,"Всего");
	...
	ВидЦен = Объект.ВидЦен ; //++Lazare_13.04.2021
	...
	//--------------------------------------------------------------------------
	// © Шишка В.В. [17 октября 2023]  НАЧАЛО
	ПравоРедактированияВозвратаВПриходнойНакладной = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(, "ПравоРедактированияВозвратаВПриходнойНакладной");	
	...
	//ЕстьПравоРедактирования = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ПравоРедактированияПроведенныхДокументовРеализацияПеремещенияПроизводство");//Lazare-04.11.2021
	
	//+++ Дмгг
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		ОсновноеПодразделение = объект.Подразделение;
		////+++Lazare 04.11.2021
	...
	УстановитьВидимостьПоНДФЛ();//_Паганини
	
	УстановитьВидимостьДляРозничнойЦены(); //_Истомин
	...
	УстановитьВидимостьПоВидуОперации_ЗакупкаКапитальныхЗатрат(); //+++ Дмгг
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда 
		ПроектИзДоговора = Объект.Договор.Проект;
	КонецЕсли;
	//--- Дмгг
...
Процедура РассчитатьСуммуИПроцентСкидкиРасходыЗапасы()
	...
	Для каждого стр Из Объект.Расходы Цикл
		ОбщаяСуммаБезСкидок = ОбщаяСуммаБезСкидок + Окр(стр.Цена * стр.Количество, 2);
		ОбщаяСуммаРучнойСкидки = ОбщаяСуммаРучнойСкидки + Окр(стр.Цена * стр.Количество, 2) - стр.Сумма;
	КонецЦикла;
	...
	КонецЦикла;
	//+++ Дмгг
	Для каждого стр Из Объект.ЗатратыНаИмущество Цикл
		ОбщаяСуммаБезСкидок = ОбщаяСуммаБезСкидок + Окр(стр.Цена * стр.Количество, 2);
		ОбщаяСуммаРучнойСкидки = ОбщаяСуммаРучнойСкидки + Окр(стр.Цена * стр.Количество , 2)- стр.Сумма;
	КонецЦикла;
	//Паганини: Добавили Округление("Окр") суммы до двых чисел после запятой, чтобы не остался погрешность
	//--- Дмгг
...
Процедура ПриОткрытии(Отказ)
	//giga
	Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ВнутреннийЗаказ") И НЕ Объект.Проведен Тогда
		Для Каждого Строка Из Объект.Запасы Цикл
			ЗапасыНоменклатураПриИзменении("", Истина, Строка);
		КонецЦикла;		
	КонецЕсли;
	//giga
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		//+++ Дмгг, перенесено в модуль объекта, по причине того что не идет заполнение при проведении из формы списка,
		// а так же по просьбе что бы перезаполнение было всегда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//	и ВозвратЗапасовПоСебестоимостиПродаж() 
	//	Тогда	
	//	РассчитатьСтомостьВозвратовНаСервере();	
	//КонецЕсли;
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	ОчиститьНеИспользуемыеТабличныеЧасти(); //+++ Дмгг, 17.12.2025, ES4001-114
	ПересчитатьСуммыТЧ(Объект.НалогообложениеНДС, Объект.СуммаВключаетНДС);//_Паганини
	ОбработатьИзменениеВидаОперации();
	ОбработатьИзменениеДоговора();
	
	УстановитьВидимостьПоВидуОперации_ЗакупкаКапитальныхЗатрат(); //+++ Дмгг
	УстановитьВидимостьДляЗапасыНаправленияДеятельности(); //+++ Дмгг
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") 
		И Объект.Запасы.Количество() > 0 Тогда 
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработатьПерезаполнениеСебестоимостиЗавершение", ЭтотОбъект),
				"შეივსოს ""თვითღირებულების"" სვეტი?",//НСтр("ka=Заполнить колонку ""Себестоимость""?';ru='Заполнить колонку ""Себестоимость""?'"),
...
//+++ Дмгг, 17.12.2025, ES4001-114
&НаКлиенте
Процедура ОчиститьНеИспользуемыеТабличныеЧасти()

	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ЗакупкаКапитальныхЗатрат") Тогда 
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика")
		...
		Объект.ЗатратыНаИмущество.Очистить(); 
...
&НаКлиенте
Процедура ОбработатьПерезаполнениеСебестоимостиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл 
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") 
			...
			Цена = 0;
			ПолучитьСреднееЗначениеСебестоимости(СтруктураДанные, Цена);
			СтрокаТабличнойЧасти.Себестоимость = Цена * СтрокаТабличнойЧасти.Количество;
		КонецЕсли;
...
Процедура ВключатьРасходыВСебестоимостьПриИзменении(Элемент)
	...
	Если Объект.ВключатьРасходыВСебестоимость Тогда
		...
		Элементы.Запасы.ПодчиненныеЭлементы.ЗапасыСуммаРасходовПФ.Видимость = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика");//Паганини
	...
	Иначе
		...
		Элементы.Запасы.ПодчиненныеЭлементы.ЗапасыСуммаРасходовПФ.Видимость = Ложь;//Паганини
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентПередИзменением <> Объект.Контрагент Тогда
		...
		ПроектИзДоговора = СтруктураДанные.ПроектИзДоговора; //+++ Дмгг
	...
	Установить_СуммаВключаетНДС_ДляНЕрезидента();
	// Kuzmich, 03.02.2024, (ONESOFT-2587) Расчет НДС неправильный в документ приходная накладная (услуги)
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика") Тогда
		...
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("ЗаказПередИзменением", ЗаказПередИзменением)), НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?';en='Passed prepayment will be cleared to continue?'"), Режим, 0);
	КонецЕсли;
...
Процедура СкидкаПроцентПриИзменении(Элемент)
	...
	Если Объект.Расходы.Количество()>0 Тогда
		ОбработатьТаблицу("Расходы", "СкидкаПроцент");//Паганини ПФ: + "СкидкаПроцент"
	КонецЕсли;
	Если Объект.Запасы.Количество()>0 Тогда
...
Процедура СкидкаСуммаПриИзменении(Элемент)
	...
	ОбработатьТаблицу("Расходы", "СкидкаСумма");//Паганини: + "СкидкаСумма"
	ОбработатьТаблицу("Запасы");
	РассчитатьСуммуИПроцентСкидкиРасходыЗапасы();//Паганини
...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Копирование Тогда
		...
		//ЗаполнитьИтогКоличество(); //+++ Дмгг;
		СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные; //+++ Дмгг
		СтрокаТабличнойЧасти.Проект = ?(НоваяСтрока, ПроектИзДоговора, СтрокаТабличнойЧасти.Проект); //+++ Дмгг
	КонецЕсли;	
...
&НаКлиенте
Процедура ЗапасыНоменклатураПриИзменении(Элемент, ОбновитьПроизвольнуюСтроку = Ложь, ПроизвольнаяСтрока = "")
	//giga	
	Если ОбновитьПроизвольнуюСтроку Тогда
		СтрокаТабличнойЧасти = ПроизвольнаяСтрока;	
	Иначе
	...
	Иначе 
		СтрокаТабличнойЧасти.Цена = СтруктураДанные.Цена;//Было
	КонецЕсли;	
	//--- Lazare_13.04.2021	
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") 
		...
		Цена = 0;
		ПолучитьСреднееЗначениеСебестоимости(СтруктураДанные, Цена);
		СтрокаТабличнойЧасти.СебестоимостьЕдиницы = Цена;
		СтрокаТабличнойЧасти.Себестоимость = Цена * СтрокаТабличнойЧасти.Количество;
	КонецЕсли;
	//-- Дмгг
	
	//giga
	РассчитатьСуммуВСтрокеТабличнойЧасти("Запасы", ?(ОбновитьПроизвольнуюСтроку, СтрокаТабличнойЧасти, Неопределено));
	
	ОткрытьВводАкцизнойМарки(СтрокаТабличнойЧасти);
	
	//+ Истомин
	Если ВидимостьКолонокРозницаМаржа Тогда 
		РозничнаяЦена = ПолучитьРозничнуЦену(Объект.Дата, СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Характеристика); 
		
		СтрокаТабличнойЧасти.РозничнаяЦена = РозничнаяЦена;
		РасчетМаржи(СтрокаТабличнойЧасти);
	КонецЕсли;
	//- Истомин
	...
	СтрокаТабличнойЧасти.СрокГодности = ?(СтруктураДанные.КоличествоДни >0,Объект.Дата + СтруктураДанные.КоличествоДни * 86400,Дата(1,1,1));//+Lazare 17.10.2022
...
Процедура РасходыПроцентСкидкиНаценкиПриИзменении(Элемент)
	...
	РассчитатьСуммуВСтрокеТабличнойЧасти("Расходы", ,"ПроцентСкидкиНаценки");

КонецПроцедуры

//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ПроцентСкидкиНаценкиПриИзменении(Элемент)
	
	РассчитатьСуммуВСтрокеТабличнойЧасти("ЗатратыНаИмущество1", ,"ПроцентСкидкиНаценки");
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ПриИзменении(Элемент)
	
	Если КопированиеСтрокиЗатратыНаИмущество1 = Неопределено ИЛИ НЕ КопированиеСтрокиЗатратыНаИмущество1 Тогда
		ОбновитьПодвалФормы();
	Иначе
		КопированиеСтрокиЗатратыНаИмущество1 = Ложь;
	КонецЕсли;
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	
	Если Копирование Тогда
		ИтогВсего = Объект.Запасы.Итог("Всего") + Объект.Расходы.Итог("Всего") + Объект.ЗатратыНаИмущество.Итог("Всего") + Элемент.ТекущиеДанные.Всего;
		ИтогСуммаНДС = Объект.Запасы.Итог("СуммаНДС") + Объект.Расходы.Итог("СуммаНДС") + Объект.ЗатратыНаИмущество.Итог("СуммаНДС") + Элемент.ТекущиеДанные.СуммаНДС;
		КопированиеСтрокиРасходы = Истина;
    КонецЕсли;
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НоваяСтрока Тогда
		
		СтрокаТабличнойЧасти = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
		СтрокаТабличнойЧасти.СтруктурнаяЕдиница = ОсновноеПодразделение;
		//Паганини_ ПФ
		Если НЕ Копирование Тогда
			СтрокаТабличнойЧасти.ВидУдержанияНДФЛ = УправлениеНебольшойФирмойСервер.ЗначениеРеквизитаОбъекта(Объект.Договор, "ВидУдержанияНДФЛ");
...
Процедура РасходыНоменклатураПриИзменении(Элемент)
	...
	Если ГруппыПользователя.Количество() Тогда 		
		...
		ГруппаНоменклатуры = РС_ОбщегоНазначения.ПолучитьЗначениеРеквизита(Элементы.Расходы.ТекущиеДанные.Номенклатура, "Родитель");
		НайденнаяГруппа    = ГруппыПользователя.НайтиПоЗначению(ГруппаНоменклатуры) ;		
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1НоменклатураПриИзменении(Элемент)
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СодержаниеАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1КоличествоПриИзменении(Элемент)
	
	РассчитатьСуммуВСтрокеТабличнойЧасти("ЗатратыНаИмущество1", , "Количество");
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ЕдиницаИзмеренияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	РассчитатьСуммуВСтрокеТабличнойЧасти("ЗатратыНаИмущество1");
	
КонецПроцедуры // ЗатратыНаИмущество1ЕдиницаИзмеренияОбработкаВыбора()
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ЦенаПриИзменении(Элемент)
	
	РассчитатьСуммуВСтрокеТабличнойЧасти("ЗатратыНаИмущество1", ,"Цена");//Паганини Добавленн параметр #3
	
КонецПроцедуры // ЗатратыНаИмущество1ЦенаПриИзменении()
//--- Дмгг
...
&НаКлиенте
Процедура ЗатратыНаИмущество1СуммаНДСПриИзменении(Элемент)
...
КонецПроцедуры // ЗатратыНаИмущество1СуммаНДСПриИзменении()
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1НаправлениеДеятельностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
КонецПроцедуры // ЗатратыНаИмущество1НаправлениеДеятельностиНачалоВыбора()
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СтруктурнаяЕдиницаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
КонецПроцедуры // ЗатратыНаИмущество1СтруктурнаяЕдиницаНачалоВыбора()
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ЗаказНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СодержаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
		
	ТекСтрока = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
	СтуктураРеквизита = Новый Структура("Объект,ТабЧасть,НомерСтроки,ИмяРеквизита", "Объект","Расходы",ТекСтрока.НомерСтроки-1,"Содержание");
	ОбщегоНазначенияКлиентПереопределяемый.ПоказатьФормуРедактированияРеквизита(Элемент.ТекстРедактирования, ЭтотОбъект, СтуктураРеквизита, Строка(ТекСтрока.Номенклатура)+": содержание");
...
Процедура ПредоплатаСуммаРасчетовПриИзменении(Элемент)
	...
	//+++ Дмгг
	СтуктураЗаполнения = ПолучитьСтавкиНаСервере(СтрокаТабличнойЧасти.СуммаРасчетов);
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	ПараметрыПодбора = Новый Структура(
		...
		?(ЗначениеПодбор, Истина, Ложь), // Подбор
		Истина, // ЕстьЗаказ
		...
		Объект.Запасы.Итог("Всего") + Объект.Расходы.Итог("Всего") + Объект.ЗатратыНаИмущество.Итог("Всего")
		//--------------------------------------------------------------------------
...
Процедура ЗаполнитьТоварыПоФактуПоДаннымПоставщика(Команда)
	...
	Если ЕстьОшибкиСоответствияЕГАИСКлиент() Тогда
		ПоказатьПредупреждение(,НСтр("ka='შეავსეთ ნომენკლატურის შესაბამისობა ესასს';ru='Заполните соответствие номенлатуры ЕГАИС';en='Fill in the appropriate nomenlatury EGAIS'"));
		Возврат;
	КонецЕсли;
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Табличная часть ""Товары"" будет перезаполнена. Продолжить?';ru='Табличная часть ""Товары"" будет перезаполнена. Продолжить?';en='A table of the ""Goods"" will be refilled. Proceed?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОтправитьАктЕГАИС(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для выполнения операции товары должны быть заполнены.';ru='Для выполнения операции товары должны быть заполнены.';en='To execute items need to be filled.'"),, "Запасы", "Объект");
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.ПодписьЧекаЕГАИС) Тогда
			ТекстСообщения = НСтр("ka='მონაცემები უკვე გადაცემულია ესასს-ში';ru='Данные уже переданы в ЕГАИС';en='These have already been transferred to EGAIS'");
		...
		Если (КонтрольНаСкладеОтключен
			...
			НапечататьЧекКлиент();
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა';ru='Не удалось выполнить проведение документа';en='Failed holding a document'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОчиститьОснованиеПриИзменениеКонтрагентаДоговора()
	...
	Если НЕ ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПриходныйОрдер") 
		...
		И НЕ ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ВнутреннийЗаказ")
		И НЕ ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ДокументЗапроса") Тогда
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные, ИспользоватьТолькоРегламентированныйПланСчетов)
	...
	Если ИспользоватьТолькоРегламентированныйПланСчетов Тогда //+++ Дмгг
	...
	СтруктураДанные.Вставить("КоличествоДни", СтруктураДанные.Номенклатура.СрокГодности); //+++ Lazare 17.10.2022
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Дата, ВалютаДокумента, Контрагент, Организация)
	...
	//+++ Lazare_13.04.2021
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
		СтруктураДанные.ВидЦен = Неопределено ;	
	КонецЕсли ;	
	Если Объект.ВидОперации =  ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
		СтруктураДанные.ВидЦенКонтрагента = Неопределено ;	
	КонецЕсли ;
	//--- Lazare_13.04.2021
...
Функция ПолучитьДанныеДоговорПриИзменении(Дата, ВалютаДокумента, Договор)
	...
	Если ЗначениеЗаполнено(Договор) Тогда 
		СтруктураДанные.Вставить("ПроектИзДоговора", Договор.Проект);
	КонецЕсли;
	//--- Дмгг
...
Процедура ЗаполнитьСтавкуНДСПоНалогообложениеНДС(ПредНалогообложениеНДС, ПредСуммаВключаетНДС)//Паганини "Добавленны параметры"
	...
	Если Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		...
		//+++ Дмгг
		Элементы.ЗатратыНаИмущество1СтавкаНДС.Видимость = Истина;
		Элементы.ЗатратыНаИмущество1СуммаНДС.Видимость = Истина;
		//--- Дмгг
		//Элементы.РасходыВсего.Видимость = Истина;
	...
	Иначе
		...
		//+++ Дмгг
		Элементы.ЗатратыНаИмущество1СтавкаНДС.Видимость = Ложь;
		Элементы.ЗатратыНаИмущество1СуммаНДС.Видимость = Ложь;
		//--- Дмгг
		//Элементы.РасходыВсего.Видимость = Ложь;
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(ИмяТабличнойЧасти, СтрокаТабличнойЧасти = Неопределено, ИмяПоля = "")//Паганини Добавленн параметр #3
	...
	//Если ИмяТабличнойЧасти = "Запасы" Тогда
		...
		//+++ Дмгг
		//Проерка на наличее автоматической скидке в документе расходная накладная
		Если (ЗначениеЗаполнено(Объект.ДокументОснование) И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РасходнаяНакладная"))
			ИЛИ (ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументОснование) И ТипЗнч(СтрокаТабличнойЧасти.ДокументОснование) = Тип("ДокументСсылка.РасходнаяНакладная")) Тогда 
			
			СкидкаВРасходной = ПроверитьНаличееАвтоматическойСкидкиВДокументеОснования(СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Характеристика, СтрокаТабличнойЧасти.ДокументОснование, ЗначениеЗаполнено(Объект.ДокументОснование));
			СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма * (1 - СкидкаВРасходной / 100);
	...
	//+++ Дмгг
	ИначеЕсли ИмяТабличнойЧасти="ЗатратыНаИмущество1" Тогда
		ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, ИмяПоля);
	Иначе
	//--- Дмгг
	//_Паганини
		РассчитатьСуммуНДС(СтрокаТабличнойЧасти);
	...
	//Ручная скидка - заполнение полей ввода на форме
	Если НЕ ИмяПоля = "СкидкаСумма" Тогда
		РассчитатьСуммуИПроцентСкидкиРасходыЗапасы();
	КонецЕсли;
	//Конец Ручная скидка
	...
	//+ Истомин
	Если ВидимостьКолонокРозницаМаржа И ИмяТабличнойЧасти="Запасы" Тогда 
		РасчетМаржи(СтрокаТабличнойЧасти); 
	КонецЕсли;
	//- Истомин
	...
	Если ИмяТабличнойЧасти = "Запасы" Тогда 
		...
		ОбновитьПроцентыСкидокИПересчитатьВсехСтрок(); //Малышев 2024-12-13 ONESOFT-3619
...
Процедура ОбновитьПодвалФормы()
	...
	КоличествоДоступныхБалов = ПолучитьКоличествоДоступныхБалов(Объект.Ссылка, Объект.КартаЛояльности, Объект.Контрагент, Объект.Дата, КоэффициентЛояльности);
	...
	Если Объект.ВидИспользованияБалов <> ПредопределенноеЗначение("Перечисление.ВидыИспользованияБалов.Расход") Тогда 
		//ИтогБаллов = КоличествоДоступныхБалов - Объект.Запасы.Итог("БалыРасход") - Объект.КоличествоНакопленныхБаллов;  //Малышев 2024-11-12     
		ИтогБаллов = КоличествоДоступныхБалов - Объект.Запасы.Итог("КоличествоНакопленныхБаллов") + Объект.Запасы.Итог("БалыРасход") ;  //Малышев 2024-11-12     
		
	Иначе
		ИтогБаллов = КоличествоДоступныхБалов + Объект.Запасы.Итог("БалыРасход"); 
	КонецЕсли;
...
Процедура ОбработатьИзменениеВидаОперации()
	...
	//+++ Lazare_13.04.2021
	Если Объект.ВидОперации =  ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
		СтруктураНадписи.ВидЦенКонтрагента = Неопределено ;	
	КонецЕсли ;
	
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
		СтруктураНадписи.ВидЦен = Неопределено ;	
	КонецЕсли ;
	//--- Lazare_13.04.2021
...
Процедура ОбработатьИзменениеДоговораФрагмент(ДоговорПередИзменением)
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
		СтруктураДанные.ВидЦен = Неопределено ;	
	ИначеЕсли   
		Объект.ВидОперации  =  ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
		СтруктураДанные.ВидЦенКонтрагента = Неопределено ;
		Объект.ВидЦенКонтрагента = Неопределено;
	КонецЕсли ;
	//Если Объект.ВидОперации =  ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда 
	//	СтруктураДанные.ВидЦенКонтрагента = Неопределено ;
	//	Объект.ВидЦенКонтрагента = Неопределено;
...
"Пересчитать документ в соответствии с договором?'");
	...
	Если СтруктураДанные.Свойство("ПроектИзДоговора") Тогда 
		ПроектИзДоговора = СтруктураДанные.ПроектИзДоговора;
	КонецЕсли;
	//--- Дмгг
...
//+++ Дмгг
&НаСервере
Процедура УстановитьВидимостьПоВидуОперации_ЗакупкаКапитальныхЗатрат()

	ВидимостьИмпорт = Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика;
	ЗначениеВидимости = Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ЗакупкаКапитальныхЗатрат;
	Элементы.ГруппаЗапасы.Видимость = НЕ ЗначениеВидимости;
	Элементы.Заказ.Видимость = НЕ ЗначениеВидимости;
	Элементы.СтруктурнаяЕдиница.Видимость = НЕ ЗначениеВидимости;
	Элементы.ДокументОснование.Видимость = НЕ ЗначениеВидимости;
	Элементы.Импорт.Видимость = ВидимостьИмпорт;
	Элементы.Расходы.Видимость = НЕ ЗначениеВидимости;
	Элементы.ЗатратыНаИмущество.Видимость = ЗначениеВидимости;
	Элементы.ГруппаПредоплата.Видимость = ?(ЗначениеВидимости, ЗначениеВидимости, Элементы.ГруппаПредоплата.Видимость);
	Элементы.Предоплата.Видимость = ?(ЗначениеВидимости, ЗначениеВидимости, Элементы.ГруппаПредоплата.Видимость);
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ИмяТабличнойЧасти = "Запасы" Тогда
			...
			Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") 
				И НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
				Цена = 0;
				ПолучитьСреднееЗначениеСебестоимости(СтрокаЗагрузки, Цена);
				НоваяСтрока.СебестоимостьЕдиницы = Цена;
				НоваяСтрока.Себестоимость = Цена * НоваяСтрока.Количество;
			КонецЕсли;
			//-- Дмгг
		...
		ИначеЕсли ИмяТабличнойЧасти = "Расходы" Тогда
			...
			Если ИспользоватьТолькоРегламентированныйПланСчетов Тогда 
				...
				Если СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.Расходы
					И СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.Доходы
					И СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.НезавершенноеПроизводство
					И СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.КосвенныеЗатраты Тогда
				...
				Если СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.Расходы
					И СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.СебестоимостьПродаж
					И СтрокаЗагрузки.Номенклатура.СчетУчетаЗатратРегл.ТипСчета <> Перечисления.ТипыСчетов.Доходы Тогда
		...
		//+++ Дмгг
		НоваяСтрока.Проект = ПроектИзДоговора;
		//--- Дмгг
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	//--Lazare_13.04.2021
	 //++Lazare
	 ИспользоватьКонтрольСроковГодностиТовара = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("КонтрольСроковГодностиТовара"); 
	 Элементы.ЗапасыСрокГодности.Видимость  = ИспользоватьКонтрольСроковГодностиТовара;
	 Элементы.ЗаполнитьЦеныПоРасхаднойНакладной.Видимость =  Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя");
	 //--Lazare
	 //--------------------------------------------------------------------------
	 // © Шишка В.В. [13 октября 2023]  НАЧАЛО
	 Элементы.ОтключитьКонтрольНоменклатурыПриВозврате.Видимость = Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя
	 														И ПравоРедактированияВозвратаВПриходнойНакладной;
	 Элементы.ОтключитьКонтрольНоменклатурыПриВозврате.Доступность = ПравоРедактированияВозвратаВПриходнойНакладной;
	 // © Шишка В.В. [13 октября 2023]  КОНЕЦ
	 //--------------------------------------------------------------------------
	 
	 //Малышев 2024-11-27 ONESOFT-3619
	 Если Объект.ВидОперации=Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя и Константы.ФункциональнаяОпцияИспользоватьКартыЛояльности.Получить() Тогда
		 Элементы.КартаЛояльности.Видимость = Истина;
		 Элементы.ВидИспользованияБалов.Видимость = Истина;
		 Элементы.ГруппаНДСЛояльности.Видимость = истина;
		 Элементы.ГруппаОплатаИтого.Видимость = истина;
	 Иначе         
		 Элементы.КартаЛояльности.Видимость = Ложь;		 
		 Элементы.ВидИспользованияБалов.Видимость = Ложь;
		 Элементы.ГруппаНДСЛояльности.Видимость = Ложь;
		 Элементы.ГруппаОплатаИтого.Видимость = Ложь;
	 КонецЕсли;
	 
	 //Малышев 2024-12-10 ONESOFT-3619
	 Элементы.ГруппаНДСЛояльности.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности);
	 Элементы.ГруппаОплатаИтого.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности);
	 Элементы.ЗапасыГруппаБаллы.Видимость = ЗначениеЗаполнено(Объект.КартаЛояльности);
...
Процедура ПолучитьСтруктуруВидовОпераций()
	...
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПриходнаяНакладная.ЗакупкаКапитальныхЗатрат);
...
&НаКлиенте
Процедура ОбработатьТаблицу(ИмяТЧ, ИмяПоля = "")
	...
	Для каждого Строка Из ИзменяемыеСтроки Цикл
		...
		Если ВыполненноеДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьЦеныНаПроцент")
			...
			РассчитатьСуммуВСтрокеТабличнойЧасти(ИмяТЧ, Строка, ИмяПоля);//Паганини ПФ: + "ИмяПоля"
		...
		ИначеЕсли ВыполненноеДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьСкидкуСуммой") Тогда
			...
			Если ИмяТЧ = "Запасы" Тогда//Паганини ПФ
			...
			////Конец Ручные скидки			
			Если НЕ ИмяПоля = "" И ИмяТЧ = "Расходы" Тогда
				РассчитатьСуммуВСтрокеТабличнойЧасти(ИмяТЧ, Строка, ИмяПоля);
			КонецЕсли;
			//_Паганини
		...
		//+++ Дмгг
		ИначеЕсли ВыполненноеДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроект") Тогда
			
			Если ИмяТЧ = "Запасы" Тогда
				Строка.Проект = ЗапасыИзменениеСтрокЗначение;
			КонецЕсли;
		//--- Дмгг
...
Процедура ОпределитьОбъектИзменений(ИмяТЧ)
	...
	Если ИмяТЧ = "Запасы" Тогда
		...
		//ПодольскийАИ #УчетПроектовБаг 14.06.2021 {
		//Было---
		////+++ Дмгг
		//ИначеЕсли ЗапасыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьСтавкуНДС") Тогда
		//Стало+++
		ИначеЕсли ЗапасыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроект") Тогда
		//ПодольскийАИ #УчетПроектовБаг 14.06.2021 }
		
			ЗапасыИзменениеСтрокОбъектИзмененийРеквизит = "Проект";
			ЗапасыИзменениеСтрокОбъектИзмененийЭлемент = "ЗапасыПроект";
		//--- Дмгг
	...
	ИначеЕсли ИмяТЧ = "Расходы" Тогда
		...
		//ИначеЕсли ЗапасыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроект") Тогда
		//Стало+++
		ИначеЕсли РасходыИзменениеСтрокДействие = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроект") Тогда
		//ПодольскийАИ #УчетПроектовБаг 14.06.2021 }
		
			РасходыИзменениеСтрокОбъектИзмененийРеквизит = "Проект";
			РасходыИзменениеСтрокОбъектИзмененийЭлемент = "РасходыПроект";
		//--- Дмгг
...
Процедура ЗаполнитьСписокДействий(ИмяТЧ)
	...
	Если ИмяТЧ = "Запасы" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетПоПроектам") Тогда 
			Действия.Добавить(Перечисления.ДействияГрупповогоИзмененияСтрок.УстановитьПроект);
		КонецЕсли;
		//--- Дмгг
...
Функция ПодготовкаДанныхДляПередачиДанныеВЕГАИС(ТранспортныйМодуль, ДанныеЕГАИСДостаточны)
	...
	ОбщиеПараметры.Добавить(НСтр("ka='კეთილი იყოს თქვენი მობრძანება';ru='ДОБРО ПОЖАЛОВАТЬ';en='WELCOME'"));   // 7 - Текст шапки (Для чека ЕНВД)
	ОбщиеПараметры.Добавить(НСтр("ka='СПАСИБО ЗА ПОКУПКУ';ru='СПАСИБО ЗА ПОКУПКУ';en='THANK YOU FOR YOUR PURCHASE'")); // 8 - Текст подвала (Для чека ЕНВД)
...
"Дополнительное описание: %ДополнительноеОписание%';ka='При печати чека произошла ошибка."
...
&НаСервере
Процедура УстановитьВидимостьДляЗапасыНаправленияДеятельности() //+++ Дмгг

	ЗначениеВидимости = Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя;	

	Элементы.ЗапасыНаправлениеДеятельности.Видимость = ЗначениеВидимости;
...
Процедура ЗагрузкаИзФайлаУслуги(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='მომსახურებების ჩატვირთვა ფაილიდან';ru='Загрузка услуг из файла';en='Loading services from a file'"));
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				ИменаСвойств = "Номенклатура, Содержание, Количество, ЕдиницаИзмерения, СтавкаНДС, Проект";
...
Процедура СформироватьНадписьМаркировкаПродукции()
	...
	Если ПравоДобавления Тогда
		ТекстНадписи = НСтр("ka='Оформить маркировку товаров';ru='Оформить маркировку товаров';en='Check out the labeling of products'");
		ИмяКоманды   = "ОформитьМаркировкуТоваров";
	Иначе
		ТекстНадписи = НСтр("ka='საქონლის მარკირება არ არის შექმნილი';ru='Маркировка товаров не создана';en='Marking of goods has not been created'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ka='საქონლის მარკირება: %1';ru='Маркировка товаров: %1';en='Marking of goods:% 1'"), НРег (Строка(Выборка.Статус)));
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='კლიენტისგან საქონლის დაბრუნება შეიცვალა. ჩავწეროთ?';ru='Возврат товаров от клиента был изменен. Записать?';en='Return of goods by the client has been changed. Record?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ka='კლიენტისგან საქონლის დაბრუნება არ არის ჩაწერილი. ჩავწეროთ?';ru='Возврат товаров от клиента не записан. Записать?';en='Return of goods by the customer is not recorded. Record?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ПересчитатьСуммыВСтрокиРасходов(СтрокаРасход, ИмяПоля = "")
	...
	ОбновитьПодвалФормы();//Паганини
		
КонецПроцедуры

//+++ Дмгг
&НаКлиенте
Процедура ПересчитатьСуммыВСтрокиЗатратыНаИмущество(СтрокаРасход, ИмяПоля = "")
	
	ПересчитатьСуммыСтрокиЗатратыНаИмущество(СтрокаРасход, ИмяПоля);
	ОбновитьПодвалФормы();//Паганини
...
Процедура ПересчитатьСуммыТЧ(ПредНалогообложениеНДС, ПредСуммаВключаетНДС, пСтавкаНДСПоУмолчанию = Неопределено)
	...
	УчитыватьНДФЛ			= Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика И Объект.Договор.УчитыватьНДФЛ;
	...
	Для Каждого СтрокаРасходы ИЗ Объект.Расходы Цикл
		...
		Если ИзмениласьНалогообложениеНДС Тогда
			...
			КонецЕсли;
		КонецЕсли;
		//РС_ОбщегоНазначения.ПересчитатьСуммыСтрокиРасхода_ПриходнаяНакладная("СуммаБезНалогов", СтрокаРасходы, Объект.СуммаВключаетНДС);
		...
		КонецЕсли;
		РС_ОбщегоНазначения.ПересчитатьСуммыСтрокиРасхода_ПриходнаяНакладная("СуммаБезНалогов", СтрокаРасходы, Объект.СуммаВключаетНДС, КонтрагентЯвляетсяНеРезидентом);
		// Kuzmich, 13.10.2023, (ONESOFT-2587) Расчет НДС неправильный в документ приходная накладная (услуги)
	КонецЦикла;
	
	//+++ Дмгг
	//ЗатратыНаИмущество
	Для Каждого СтрокаРасходы ИЗ Объект.ЗатратыНаИмущество Цикл
		Если ИзмениласьУчитыватьНДФЛ Тогда
			СтрокаРасходы.ВидУдержанияНДФЛ = ВидУдержанияНДФЛ;
		КонецЕсли;
		Если ИзмениласьНалогообложениеНДС Тогда
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СтавкаНДСОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ТекДанные = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
	ПредВидУдержанияНДФЛ	= ТекДанные.ВидУдержанияНДФЛ;
	ПредСтавкаНДС			= ТекДанные.СтавкаНДС;
	ПредВидУдержанияПФ		= ТекДанные.ВидУдержанияПФ;
КонецПроцедуры
...
Процедура УстановитьВидимостьПоНДФЛ()
	...
	ПредУчитыватьНДФЛ = УчитыватьНДФЛ;
	УчитыватьНДФЛ = Объект.Договор.УчитыватьНДФЛ И (Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика
						ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ЗакупкаКапитальныхЗатрат); //+++ Дмгг
	Элементы.РасходыВидУдержанияНДФЛ.Видимость	= УчитыватьНДФЛ;//РасходыТипНДФЛ
	Элементы.РасходыСуммаНДФЛ.Видимость			= УчитыватьНДФЛ;
	Элементы.РасходыИтогСуммаНДФЛ.Видимость 	= УчитыватьНДФЛ;
	Элементы.ЗапасыСуммаРасходовНДФЛ.Видимость	= УчитыватьНДФЛ И Объект.ВключатьРасходыВСебестоимость И Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика;
	...
	//+++ Дмгг
	//Заполним данными удержаниями НДФЛ и ПФ
	...
	Элементы.ЗатратыНаИмущество1ВидУдержанияНДФЛ.Видимость	= УчитыватьНДФЛ;//РасходыТипНДФЛ
	Элементы.ЗатратыНаИмущество1СуммаНДФЛ.Видимость			= УчитыватьНДФЛ;
	
	ВидУдержанияПФ = Неопределено;
	Если тзСрезПФ.Количество() > 0 Тогда 
		ВидУдержанияПФ = тзСрезПФ[0].ВидУдержания;
	Элементы.ЗатратыНаИмущество1СтавкаНДС.Видимость	= НЕ УчитыватьНДФЛ ИЛИ Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС;
	Элементы.ЗатратыНаИмущество1СуммаНДС.Видимость	= НЕ УчитыватьНДФЛ ИЛИ Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС;
	//--- Дмгг
	КонецЕсли;
...
Процедура ЗаполнитьФактурыНаСервере()
	...
	ТаблицаАвансов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьТаблицуАвансовФактур(Объект.Ссылка, "ПриходнаяНакладная", Объект.Дата, Объект.Организация, Объект.Контрагент);
...
Функция ПолучитьАдресТаблицыОстатков()
	ТаблицаАвансов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ПолучитьТаблицуАвансовФактур(Объект.Ссылка, "ПриходнаяНакладная", Объект.Дата, Объект.Организация, Объект.Контрагент, Объект.ФактурыАвансов);
	Возврат Новый Структура("АдресТаблицы", ПоместитьВоВременноеХранилище(ТаблицаАвансов));
КонецФункции

&НаСервере
Процедура УстановитьВидимостьДляРозничнойЦены()

	ВидимостьКолонокРозницаМаржа = Константы.ПоказыватьРозничныеЦеныВПриходныхНакладных.Получить();
	
	Элементы.ЗапасыМаржа.Видимость = ВидимостьКолонокРозницаМаржа;
	Элементы.ЗапасыРозничнаяЦена.Видимость = ВидимостьКолонокРозницаМаржа;
	
	Если ВидимостьКолонокРозницаМаржа Тогда 
		
		ВидРозничнойЦеныДляРасчета = Константы.ВидРозничнойЦеныПриходныхНакладных.Получить();
		
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
			ЗаполнитьКолонкиРозницыМаржи();
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьКолонкиРозницыМаржи()
	...
	Запрос.Текст = 
		...
		|					ИЗ
		|						вт_Запасы КАК а)
		|				И ВидЦен = &ВидРозничнойЦеныДляРасчета) КАК ЦеныНоменклатурыСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ПО вт_Запасы.Номенклатура = вт_СрезЦен.Номенклатура
		|			И вт_Запасы.Характеристика = вт_СрезЦен.Характеристика
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
	...
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.УстановитьПараметр("ВидРозничнойЦеныДляРасчета", ВидРозничнойЦеныДляРасчета);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтрокаЗапасов = Объект.Запасы[ВыборкаДетальныеЗаписи.НомерСтроки-1];
		
		СтрокаЗапасов.РозничнаяЦена = ВыборкаДетальныеЗаписи.Цена;
		Если СтрокаЗапасов.РозничнаяЦена = 0 Тогда 
			СтрокаЗапасов.Маржа = 0;
		Иначе
			СтрокаЗапасов.Маржа = 100 - СтрокаЗапасов.Цена / СтрокаЗапасов.РозничнаяЦена * 100;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПолучитьРозничнуЦену(Дата, Номенклатура, Характеристика)

	ДатаСреза = ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДата());
	
	СтруктураОтбора = Новый Структура("Номенклатура, Характеристика, ВидЦен", Номенклатура, Характеристика, ВидРозничнойЦеныДляРасчета);
	
	тзСрезЦены = РегистрыСведений.ЦеныНоменклатуры.СрезПоследних(ДатаСреза, СтруктураОтбора);
	
	Если тзСрезЦены.Количество() > 0 Тогда 
		Возврат тзСрезЦены[0].Цена;
	КонецЕсли;
...
&НаКлиенте
Процедура РасчетМаржи(СтрокаТабличнойЧасти)
	
	Если СтрокаТабличнойЧасти.РозничнаяЦена = 0 Тогда 
		СтрокаТабличнойЧасти.Маржа = 0;
	Иначе
		СтрокаТабличнойЧасти.Маржа = 100 - СтрокаТабличнойЧасти.Цена / СтрокаТабличнойЧасти.РозничнаяЦена * 100;
	КонецЕсли;	
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ВидУдержанияНДФЛОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ТекДанные = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
	ПредВидУдержанияНДФЛ	= ТекДанные.ВидУдержанияНДФЛ;
	ПредСтавкаНДС			= ТекДанные.СтавкаНДС;
	ПредВидУдержанияПФ		= ТекДанные.ВидУдержанияПФ;
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ВидУдержанияПФОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ТекДанные = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
	ПредВидУдержанияНДФЛ	= ТекДанные.ВидУдержанияНДФЛ;
	ПредСтавкаНДС			= ТекДанные.СтавкаНДС;
	ПредВидУдержанияПФ		= ТекДанные.ВидУдержанияПФ;
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СуммаПриИзменении(Элемент)
	ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, "Сумма");
КонецПроцедуры // ЗатратыНаИмущество1СуммаПриИзменении()
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СуммаБезНалоговПриИзменении(Элемент)
	ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, "СуммаБезНалогов");
КонецПроцедуры
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ВсегоПриИзменении(Элемент)
	ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, "Всего");
КонецПроцедуры
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СтавкаНДСПриИзменении(Элемент)
	ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, "СтавкаНДС");
КонецПроцедуры // ЗатратыНаИмущество1СтавкаНДСПриИзменении()
///--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ВидУдержанияНДФЛПриИзменении(Элемент)
	ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, "ВидУдержанияНДФЛ");
КонецПроцедуры
//--- Дмгг
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1ВидУдержанияПФПриИзменении(Элемент)
	ПересчитатьСуммыВСтрокиЗатратыНаИмущество(Элементы.ЗатратыНаИмущество1.ТекущаяСтрока, "ВидУдержанияПФ");
КонецПроцедуры
//--- Дмгг
...
//+++ Дмгг
&НаСервере
Процедура ПересчитатьСуммыСтрокиЗатратыНаИмущество(Идентификатор, ИмяПоля)
	Если Идентификатор = Неопределено Тогда
		Для Каждого СтрокаТЧ ИЗ Объект.ЗатратыНаИмущество Цикл
			Идентификатор = СтрокаТЧ.ПолучитьИдентификатор();
			ТекДанные = Объект.ЗатратыНаИмущество.НайтиПоИдентификатору(Идентификатор);
			РС_ОбщегоНазначения.ПересчитатьСуммыСтрокиРасхода_ПриходнаяНакладная(ИмяПоля, ТекДанные, Объект.СуммаВключаетНДС);
		КонецЦикла;
	Иначе
		ТекДанные = Объект.ЗатратыНаИмущество.НайтиПоИдентификатору(Идентификатор);
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СуммаНДФЛПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
	СтрокаТабличнойЧасти.Всего = СтрокаТабличнойЧасти.Сумма + ?(Объект.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС) + СтрокаТабличнойЧасти.СуммаНДФЛ + СтрокаТабличнойЧасти.СуммаПФ;
КонецПроцедуры
//--- Дмгг 
...
//+++ Дмгг
&НаКлиенте
Процедура ЗатратыНаИмущество1СуммаПФПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ЗатратыНаИмущество1.ТекущиеДанные;
	СтрокаТабличнойЧасти.Всего = СтрокаТабличнойЧасти.Сумма + ?(Объект.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС) + СтрокаТабличнойЧасти.СуммаНДФЛ + СтрокаТабличнойЧасти.СуммаПФ;
КонецПроцедуры
//--- Дмгг

// Kuzmich, 05.03.2019, распределение услуг по весу/объему
...
Процедура УстановкаФорматовЦен()
	...
	Элементы.РасходыЦена.Формат = НСтр("ru='"+СтрокаФормата+"';ka='"+СтрокаФормата+"'");
	Элементы.РасходыЦена.ФорматРедактирования = НСтр("ru='"+СтрокаФормата+"';ka='"+СтрокаФормата+"'");
КонецПроцедуры
// Kuzmich, 05.06.2019, Количество знаков после запятой в цене

&НаКлиенте
Процедура ЗатратыНаИмущество1ВнеоборотныйАктивНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
	Элементы.ЗатратыНаИмущество1ВнеоборотныйАктив.ПараметрыВыбора = НовыеПараметры;
...
&НаСервере
Процедура ПолучитьСреднееЗначениеСебестоимости(СтруктураДанные, Цена) //+++ Дмгг
	...
	Запрос.УстановитьПараметр("МоментКонтроля", ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата-1, ТекущаяДата()-1));
...
&НаКлиенте
Процедура ПодразделениеПриИзменении(Элемент)  //+++ Дмгг
...
//+++ Lazare_07.09.2021
&НаСервере
Процедура РассчитатьСтомостьВозвратовНаСервере() ;	
	ДокОбъект =  РеквизитФормыВЗначение("Объект");
	ДокОбъект.РассчитатьСтомостьВозвратов();
КонецПроцедуры

&НаСервере
Функция ВозвратЗапасовПоСебестоимостиПродаж() 
	
	Запрос = Новый Запрос ;
	Запрос.Текст =
	
	"ВЫБРАТЬ
	|	УчетнаяПолитикаРеглСрезПоследних.ВозвратЗапасовПоСебестоимостиПродаж КАК ВозвратЗапасовПоСебестоимостиПродаж
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаРегл.СрезПоследних КАК УчетнаяПолитикаРеглСрезПоследних
	|ГДЕ
	|	УчетнаяПолитикаРеглСрезПоследних.ВозвратЗапасовПоСебестоимостиПродаж = ИСТИНА";
...
Процедура ЗаполнитьПоПродажамНаСервере()
	...
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.ЗаполнитьПоПродажамиНаСервере();
	ЗначениеВРеквизитФормы(Документ, "Объект");
	// Обновим подвал формы.
	ИтогВсего = Объект.Запасы.Итог("Всего") + Объект.Расходы.Итог("Всего");
	ИтогСуммаНДС = Объект.Запасы.Итог("СуммаНДС") + Объект.Расходы.Итог("СуммаНДС");
...
//+++ Дмгг
&НаСервере
Функция ПроверитьНаличееАвтоматическойСкидкиВДокументеОснования(Номенклатура, Характеристика, ДокументОснованиеТаблицы, ДокументОснованияВШапке = Ложь)

	ПроцентВозврата = 0;
	
	Если ДокументОснованияВШапке Тогда 
		ДокОснования = Объект.ДокументОснование;
	Иначе
		ДокОснования = ДокументОснованиеТаблицы;
	КонецЕсли;
	
	тзЗапасы = ДокОснования.Запасы.Выгрузить();
	
	МассивНайденныхСтрок = тзЗапасы.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", Номенклатура, Характеристика));
	
	Для Каждого Строк Из МассивНайденныхСтрок Цикл 
		
		Если Строк.ПроцентАвтоматическойСкидки = 0 Тогда 
			Продолжить;
		КонецЕсли;
		
		ПроцентВозврата = Строк.ПроцентАвтоматическойСкидки;
		Прервать;
		
	КонецЦикла;
	
	Возврат ПроцентВозврата;
	
КонецФункции // ПроверитьНаличееАвтоматическойСкидкиВДокументеОснования()
//--- Дмгг

//+Lazare 17.02.2023
&НаКлиенте
Процедура ЗаполнитьЦеныПоРасхаднойНакладной(Команда)
	
	ЗаполнитьЦеныПоРасхаднойНакладнойПоВидЦеныНаСервере();
...
&НаСервере
Процедура ЗаполнитьЦеныПоРасхаднойНакладнойПоВидЦеныНаСервере()
	
	Если ЗначениеЗаполнено(Объект.ДокументОснование) и ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РасходнаяНакладная") Тогда 		
		ЗаполнитьЦеныВозвратаПоРасхаднойНакладной(Объект.ДокументОснование);
	Иначе 		
		ЗаполнитьЦеныВозвратаПоВидЦены(Объект.Дата,Объект.ВидЦен);	
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЦеныВозвратаПоРасхаднойНакладной(ДокументОснование)
	
	ТаблицаЗапасыЦени = ДокументОснование.Запасы; 
	ТаблицаЗапасы = Объект.Запасы;
	
	для Каждого Ст из ТаблицаЗапасы Цикл 		
		
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("Номенклатура",Ст.Номенклатура);
		ПараметрыОтбора.Вставить("Характеристика",Ст.Характеристика);
		ПараметрыОтбора.Вставить("Партия",Ст.Партия);
		ПараметрыОтбора.Вставить("ЕдиницаИзмерения",Ст.ЕдиницаИзмерения);		
		
		НайденнаяСтрока = ТаблицаЗапасыЦени.НайтиСтроки(ПараметрыОтбора);
		
		Если НайденнаяСтрока.количество()>0 Тогда 		
			Ст.Цена = НайденнаяСтрока[0].Цена;	
...
&НаСервере
Процедура ЗаполнитьЦеныВозвратаПоВидЦены(Дата,ВидЦен);
	...
	Если ЗначениеЗаполнено(ВидЦен) Тогда 
		...
		для Каждого Ст из ТаблицаЗапасы Цикл 		
			...
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("ДатаОбработки",Объект.Дата);			
			ПараметрыОтбора.Вставить("Номенклатура",Ст.Номенклатура);
			ПараметрыОтбора.Вставить("Характеристика",Ст.Характеристика);
			ПараметрыОтбора.Вставить("ВидЦен",ВидЦен);
			ПараметрыОтбора.Вставить("ВалютаДокумента",Объект.ВалютаДокумента);
	
			Если ЗначениеЗаполнено(Ст.ЕдиницаИзмерения) И ТипЗнч(Ст.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				ПараметрыОтбора.Вставить("Коэффициент",Ст.ЕдиницаИзмерения.Коэффициент);
			Иначе
				ПараметрыОтбора.Вставить("Коэффициент",1);
			КонецЕсли;
			
			//Цена = УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦенКонтрагента(СтруктураДанные);
			Цена = УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(ПараметрыОтбора);
			Ст.Цена = Цена;	
			Ст.Сумма = Ст.Цена * Ст.Количество ;
...
// © Шишка В.В. [13 октября 2023]  НАЧАЛО
&НаКлиенте
Процедура ПроверитьДанныеПоВозврату(Команда)
	ПроверитьДанныеПоВозвратуНаСервере();
КонецПроцедуры

&НаСервере
Процедура ПроверитьДанныеПоВозвратуНаСервере()
	...
	Для каждого ТекСтрока Из Объект.Запасы Цикл
		...
		СтруктураДанныеСтроки = Новый Структура("НомерСтроки, ID, Количество, Договор"
								...
								, Объект.Договор.УникальныйИдентификатор());
		ПроверяемыеДанные.Добавить(СтруктураДанныеСтроки);
	
	КонецЦикла;
	МассивОшибок = Новый Массив;
	ОПТИМУС_ОбменДистрибуция.ВыполнитьПрорверкуПоЗаявкамНаВозврат(, Новый ХранилищеЗначения(ПроверяемыеДанные, Новый СжатиеДанных(9)), МассивОшибок, Объект.Дата);
...
// Kuzmich, 03.02.2024, (ONESOFT-2587) Расчет НДС неправильный в документ приходная накладная (услуги)
&НаСервере
Функция Установить_СуммаВключаетНДС_ДляНЕрезидента()
	ПЕРЕМ РЕЗ;
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) И Объект.Контрагент.ЯвляетсяНеРезидентом Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор) И Объект.Договор.УчитыватьНДФЛ Тогда
...
Процедура КартаЛояльностиПриИзменении(Элемент)
	...
	УстановитьВидимостьИДоступность(); 
	ОбновитьПроцентыСкидокИПересчитатьВсехСтрок();
	
КонецПроцедуры

&НаКлиенте
Процедура ДокументОснованиеПриИзменении1(Элемент)
	Если ЗначениеЗаполнено(Объект.ДокументОснование) 
		//Малышев 2025-04-03 ONESOFT-3979 - заполнение для всех, кроме возврата
		и Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") 
		
		ТОгда
		
		ЗаполнитьТабличнуюЧастьПоОснованию();
...
&НаСервере
Процедура ЗаполнитьТабличнуюЧастьПоОснованию()
	
	Если ЗначениеЗаполнено(Объект.ДокументОснование) и Объект.ДокументОснование.Метаданные().Реквизиты.Найти("КартаЛояльности")<>Неопределено  Тогда
		Объект.КартаЛояльности = Объект.ДокументОснование.КартаЛояльности;
		Объект.ПроцентНакопленияБалов = Объект.ДокументОснование.ПроцентНакопленияБалов;
		
		Объект.Запасы.Загрузить(Объект.ДокументОснование.Запасы.Выгрузить());
		
	Иначе     
		Объект.КартаЛояльности = 0;
		Объект.ВидИспользованияБалов = 0;
		Объект.ВсегоИспользованоБаллов = 0;
		Объект.ОбщаяСуммаКОплате = 0;
		Объект.ПроцентНакопленияБалов = 0;
		Объект.КоличествоНакопленныхБаллов = 0;
	КонецЕсли;	
...
&НаКлиенте
Процедура ОбновитьПроцентыСкидокИПересчитатьВсехСтрок()
	
	ПроцентСкидкиПоКартеЛояльности = 0;
	ПроцентНакопленияБалов = 0;
		
	Если ЗначениеЗаполнено(объект.КартаЛояльности) Тогда 
		ПроцентСкидкиПоКартеЛояльности = ПолучитьПроцентСкидкиЛояльности(); 
		ПроцентНакопленияБалов = ПолучитьПроцентНакопления();
	КонецЕсли;

	Для Каждого СтрокаДокумента из Объект.Запасы Цикл 
		ОбновитьПроцентыСкидокИПересчитатьСтроки(СтрокаДокумента);
	КонецЦикла;
...
&НаКлиенте
Процедура ОбновитьПроцентыСкидокИПересчитатьСтроки(СтрокаДокумента)
	
	СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтрокаДокумента.СтавкаНДС);

	СтрокаДокумента.ПроцентСкидкаЛояльности = ПроцентСкидкиПоКартеЛояльности;
	//РассчитатьСуммуВСтрокеТабличнойЧасти("Запасы", СтрокаДокумента);
	...
	СтрокаДокумента.СуммаНДСБаллов = СтрокаДокумента.БалыРасход * СтавкаНДС / (100+СтавкаНДС); 
...
Функция ПолучитьПроцентСкидкиЛояльности()
	...
	Результат = БаллыЛояльностьСкидки.ПолучитьПроцентСкидкиЛояльности_РасходнаяНакладная(Объект);
	Возврат Результат;
...
Функция ПолучитьПроцентНакопления()
	...
	//Малышев 2024-10-11 ONESOFT-3509
	Результат = БаллыЛояльностьСкидки.ПолучитьПроцентНакопленияБаллов_РасходнаяНакладная(Объект);
	Возврат Результат;
...
Процедура РассчитатьСуммуКОплате()
	...
	Объект.ОбщаяСуммаКОплате = СуммаДокумента - Объект.Запасы.Итог("БалыРасход");
...
&НаКлиенте
Процедура ЗаполнитьТоварыПоОснованию(Команда)
	Если ЗначениеЗаполнено(Объект.ДокументОснование) 
		ТОгда
		ЗаполнитьТабличнуюЧастьПоОснованию();
		ОбновитьПодвалФормы();
