		...
		Если Объект.Предоплата.Количество() > 0
		   И Объект.Договор <> ДоговорПередИзменением Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("КонтрагентПередИзменением, ДоговорПередИзменением, КонтрагентВестиРасчетыПоЗаказамПередИзменением, ВидимостьДоговораПередИзменением, СтруктураДанные", КонтрагентПередИзменением, ДоговорПередИзменением, КонтрагентВестиРасчетыПоЗаказамПередИзменением, ВидимостьДоговораПередИзменением, СтруктураДанные)),
				НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?'"),
				РежимДиалогаВопрос.ДаНет
			);
			Возврат;
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика") Тогда
		...
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("ЗаказПередИзменением", ЗаказПередИзменением)), НСтр("ka='წინასწარი გადახდის ჩათვლა გაიწმინდება, გავაგრძელოთ?';ru='Зачет предоплаты будет очищен, продолжить?'"), Режим, 0);
	КонецЕсли;
...
Процедура РасходыНаправлениеДеятельностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.ВозможностьУказанияНаправленияДеятельности Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='მოცემული ხარისთვის საქმიანობის მიმართულება არ მიეთითება!';ru='Для данного расхода направление деятельности не указывается!'"));
	КонецЕсли;
...
Процедура РасходыСтруктурнаяЕдиницаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.ВозможностьУказанияПодразделения Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='ამ ხარჯისთვის ქვეგანყოფილება არ მიეთითება!';ru='Для этого расхода подразделение не указывается!'"));
	КонецЕсли;
...
Процедура РасходыЗаказНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.ВозможностьУказанияЗаказа Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ka='ამ ხარჯისთვის შეკვეთა არ მიეთითება!';ru='Для этого расхода заказ не указывается!'"));
	КонецЕсли;
...
Процедура ЗаполнитьТоварыПоФактуПоДаннымПоставщика(Команда)
	...
	Если ЕстьОшибкиСоответствияЕГАИСКлиент() Тогда
		ПоказатьПредупреждение(,НСтр("ka='შეავსეთ ნომენკლატურის შესაბამისობა ესასს';ru='Заполните соответствие номенлатуры ЕГАИС'"));
		Возврат;
	КонецЕсли;
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ka='Табличная часть ""Товары"" будет перезаполнена. Продолжить?';ru='Табличная часть ""Товары"" будет перезаполнена. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОтправитьАктЕГАИС(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Для выполнения операции товары должны быть заполнены.';ru='Для выполнения операции товары должны быть заполнены.'"),, "Запасы", "Объект");
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.ПодписьЧекаЕГАИС) Тогда
			ТекстСообщения = НСтр("ka='მონაცემები უკვე გადაცემულია ესასს-ში';ru='Данные уже переданы в ЕГАИС'");
		...
		Если (КонтрольНаСкладеОтключен
			...
			НапечататьЧекКлиент();
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა';ru='Не удалось выполнить проведение документа'"));
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидЦенКонтрагента) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ka='%ВидЦенКонтрагента%';ru='%ВидЦенКонтрагента%'");
		Иначе	
...
"Пересчитать документ в соответствии с договором?'");
	...
	Если СтруктураДанные.Свойсто("ПроектИзДоговора") Тогда 
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветхранения") Тогда
		Элементы.Контрагент.Заголовок = НСтр("ka='მყიდველი';ru='Получатель'");
	Иначе	
		Элементы.Контрагент.Заголовок = НСтр("ka='მყიდველი';ru='Получатель'");
	КонецЕсли;
...
Функция ПодготовкаДанныхДляПередачиДанныеВЕГАИС(ТранспортныйМодуль, ДанныеЕГАИСДостаточны)
	...
	ОбщиеПараметры.Добавить(НСтр("ka='კეთილი იყოს თქვენი მობრძანება';ru='ДОБРО ПОЖАЛОВАТЬ'"));   // 7 - Текст шапки (Для чека ЕНВД)
	ОбщиеПараметры.Добавить(НСтр("ka='СПАСИБО ЗА ПОКУПКУ';ru='СПАСИБО ЗА ПОКУПКУ'")); // 8 - Текст подвала (Для чека ЕНВД)
...
Процедура ЗагрузкаИзФайлаУслуги(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='მომსახურებების ჩატვირთვა ფაილიდან';ru='Загрузка услуг из файла'"));
...
Процедура СформироватьНадписьМаркировкаПродукции()
	...
	Если ПравоДобавления Тогда
		ТекстНадписи = НСтр("ka='Оформить маркировку товаров';ru='Оформить маркировку товаров'");
		ИмяКоманды   = "ОформитьМаркировкуТоваров";
	Иначе
		ТекстНадписи = НСтр("ka='საქონლის მარკირება არ არის შექმნილი';ru='Маркировка товаров не создана'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ka='საქონლის მარკირება: %1';ru='Маркировка товаров: %1'"), НРег (Строка(Выборка.Статус)));
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='კლიენტისგან საქონლის დაბრუნება შეიცვალა. ჩავწეროთ?';ru='Возврат товаров от клиента был изменен. Записать?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ka='კლიენტისგან საქონლის დაბრუნება არ არის ჩაწერილი. ჩავწეროთ?';ru='Возврат товаров от клиента не записан. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
