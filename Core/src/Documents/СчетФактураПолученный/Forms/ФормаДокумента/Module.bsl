...
Процедура ЗаполнитьПоДокументуОснованию()
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоДокументуОснованиюЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულიად შეივსება ""საფუძვლით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура УстановитьНадписьЦеныИВалюта()
	...
	Иначе
		...
		ЦеныИВалюта = НСтр("ka='ვალუტა არ არის მითითებული';ru='Валюта не указана'");
...
Процедура ЗаполнитьНомераГТД(Команда)
	...
	ТекстВопроса = НСтр("ka='ცხრილში გადავსებული იქნება სვეტი ""წარმოშობის ქვეყანა"" და ""საბაჟო დეკლარაციის ნომერი"". გავაგრძელოთ?';ru='В табличной части будет перезаполнена колонка ""Страна происхождения"" и ""Номер ГТД"". Продолжить?'");
