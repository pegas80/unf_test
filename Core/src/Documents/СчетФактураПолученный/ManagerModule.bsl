...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		Если Шапка.ДатаВходящегоДокумента >= '20150101'
			...
			ОбластьРазъяснение.Параметры.Заполнить(Новый Структура("Разъяснение", Нстр("ka='მიღებული (შემომავალი) ანგარიშფაქტურის ჩამოყალიბებას კრებსითი მონაცემებით (რამდენიმე მყიდველი) კომიტენტისთვის ანგარიშგების საფუძველზე მხარდაჭერა არ გააჩნია.';ru='Формирование полученного (входящего) счета-фактуры на основании отчета комитенту по сводным данным (несколько покупателей) не поддерживается.';en='Formation obtained (entering) Invoice based committent report summary data (multiple buyers) is not supported.'")));
		...
		Если Шапка.ДатаДокумента >= НачалоИспользованияСФ1137 Тогда
			...
			Если Шапка.Исправление Тогда
				ТекстИсправление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='Исправление № %1 от %2';ru='Исправление № %1 от %2';en='Correction №% 1% 2 from'"),
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактураВходящий") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"СчетФактураВходящий",
			НСтр("ka='ანგარიშ-ფაქტურა (მიღებული)';ru='Счет-фактура (полученный)';en='Invoice (obtained)'"),
			ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
		);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='ანგარიშ-ფაქტურა (მიღებული)';ru='Счет-фактура (полученный)';en='Invoice (obtained)'");
