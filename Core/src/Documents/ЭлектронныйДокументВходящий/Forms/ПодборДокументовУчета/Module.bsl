...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЭлектронныйДокумент",СсылкаНаЭД) Тогда
		...
		Если ЭлектронныйДокумент.ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
			Элементы.ФормаПерезаполнитьТекущий.Заголовок = НСтр("ka='ნმენკლატურის შედარება';ru='Сопоставить номенклатуру';en='Сопоставить номенклатуру'");
		КонецЕсли;
...
Процедура ПерезаполнитьТекущий(Команда)
	...
	Если ТекСтрока <> Неопределено Тогда
		ТекстВопроса = НСтр("ka='Документ учета будет заполнены данными электронного документа. Продолжить?';ru='Документ учета будет заполнены данными электронного документа. Продолжить?';en='accounting document will be filled in the electronic document data. Proceed?'");
...
Процедура УдалитьСвязьСДокументом(Команда)
	...
	Если ТекСтрока <> Неопределено Тогда
		ТекстВопроса = НСтр("ka='Связь между документами разорвется. Повторно связать документы возможно только в ручном режиме. Продолжить?';ru='Связь между документами разорвется. Повторно связать документы возможно только в ручном режиме. Продолжить?';en='Communication between documents burst. Relink the documents is only possible in manual mode. Proceed?'");
