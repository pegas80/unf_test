...
Процедура СоздатьПоступление(Команда)
	...
	Если ПоступлениеТоваров = Неопределено И ТолькоПоиск Тогда
		Если ДополнительныеПараметры.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения")
			...
			СтрокаСообщения = НСтр("ka='ზედნადები უკვე დამტკიცებულია აქტით, შექმნა შეუძლებელია';ru='Накладная уже подветждена актом, создание не возможно'");
		...
		ИначеЕсли ДополнительныеПараметры.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктОтказа") Тогда
			СтрокаСообщения = НСтр("ka='ესასს-ს უკვე გადაეცა ზადნადებზე უარი, შექმნა შეუძლებელია';ru='В ЕГАИС уже передан отказ от накладной, , создание не возможно'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
			Возврат;
		КонецЕсли;
...
Процедура ЗагрузитьТТН(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТранспортныйМодуль) Тогда
		ПоказатьПредупреждение(, НСтр("ka='შეავსეთ სატრანსპორტო მოდული';ru='Заполните транспортный модуль'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтказатьсяОтТТН(Команда)
	...
	Если ДанныеТТН.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения") Тогда
		СтрокаСообщения = НСтр("ka='ზედნადები უკვე დამოწმებულია აქტით, უარყოფა შეუძლებელია';ru='Накладная уже подветждена актом, отказ не возможен'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат;
	ИначеЕсли ДанныеТТН.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктОтказа") Тогда
		СтрокаСообщения = НСтр("ka='მონაცემებზე უარი უკვე გადაცემული იქნა ესასს-ში';ru='Отказ от данных уже передавался в ЕГАИС'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат;
	Иначе
		ТекстВопроса = НСтр("ka='დარწმუნებული ხართ, რომ გსურთ ზედნადებზე უარის გადაცემა ესასს-ში?';ru='Вы уверены что хотите передать отказ от накладной в ЕГАИС?'");
...
Процедура ЗапроситьОтменуПроведения(Команда)
	...
	ТекстВопроса = НСтр("ka='Отправить грузоотправителю запрос на отмену проведения ТТН?';ru='Отправить грузоотправителю запрос на отмену проведения ТТН?'");
...
Процедура ВыгрузкаДокумента_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		Элементы.Список.Обновить();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტი წარმატებით დადასტურდა.';ru='Документ успешно подтвержден.'"));
	КонецЕсли;
