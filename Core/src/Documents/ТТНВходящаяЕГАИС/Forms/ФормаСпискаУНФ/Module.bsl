...
Процедура СоздатьПоступление(Команда)
	...
	Если ПоступлениеТоваров = Неопределено И ТолькоПоиск Тогда
		Если ДополнительныеПараметры.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения")
			...
			СтрокаСообщения = НСтр("ka='ზედნადები უკვე დამტკიცებულია აქტით, შექმნა შეუძლებელია';ru='Накладная уже подветждена актом, создание не возможно';en='The consignment note has podvetzhdena act of creation is not possible'");
		...
		ИначеЕсли ДополнительныеПараметры.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктОтказа") Тогда
			СтрокаСообщения = НСтр("ka='ესასს-ს უკვე გადაეცა ზადნადებზე უარი, შექმნა შეუძლებელია';ru='В ЕГАИС уже передан отказ от накладной, , создание не возможно';en='In EGAIS already passed the rejection of the consignment note, the creation is not possible'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
			Возврат;
		КонецЕсли;
...
Процедура ЗагрузитьТТН(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТранспортныйМодуль) Тогда
		ПоказатьПредупреждение(, НСтр("ka='შეავსეთ სატრანსპორტო მოდული';ru='Заполните транспортный модуль';en='Fill in the transport module'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗапроситьОтменуПроведения(Команда)
	...
	ТекстВопроса = НСтр("ka='Отправить грузоотправителю запрос на отмену проведения ТТН?';ru='Отправить грузоотправителю запрос на отмену проведения ТТН?';en='Send a request to cancel the shipper of the consignment note?'");
...
Процедура ЗагрузкаДокументов_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ka='ჩატვირთვა დასრულებულია."
"ჩაიტვირთა დოკუმენტი: %ЗагруженоДокументов%';ru='Загрузка завершена."
