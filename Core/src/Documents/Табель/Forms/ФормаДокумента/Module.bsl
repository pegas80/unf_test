...
Процедура Заполнить(Команда)
	...
	Если Объект.ОтработанноеВремяПоДням.Количество() > 0
	 ИЛИ Объект.ОтработанноеВремяЗаПериод.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ?';ru='Табличная часть будет очищена! Продолжить?'"), Режим, 0);
	Иначе
		ЗаполнитьТабель();
	КонецЕсли;
