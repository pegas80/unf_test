...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Табель") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Табель", НСтр("ka='სამუშაო დროის აღრიცხვის ტაბელი';ru='Табель учета рабочего времени'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='სამუშაო დროის აღრიცხვის ტაბელი';ru='Табель учета рабочего времени'");
...
Процедура ПриОпределенииЗначенияПоУмолчанию(ДокументСсылка, ИмяРеквизита, ВходящиеДанные, СтрокаСопоставлена, ОбновлятьДанные, ЗначениеПоУмолчанию)
	
	Если СтрокаСопоставлена 
		И НЕ ЗначениеЗаполнено(ВходящиеДанные)
		И ЗначениеЗаполнено(ДокументСсылка[ИмяРеквизита])
	Тогда
		
		ЗначениеПоУмолчанию = ДокументСсылка[ИмяРеквизита];
...
Процедура ПриОпределенииОбразцовЗагрузкиДанных(НастройкиЗагрузкиДанных, УникальныйИдентификатор) Экспорт
	...
	Если НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Документ.Табель.ТабличнаяЧасть.ОтработанноеВремяПоДням" Тогда
		
		Образец_xlsx = ПолучитьМакет("ОбразецЗагрузкиДанных_ПоДням_xlsx");
		...
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_mxl", "ОбразецЗагрузкиДанных_ПоДням_mxl");
		
		Образец_csv = ПолучитьМакет("ОбразецЗагрузкиДанных_ПоДням_csv");
	...
	ИначеЕсли НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Документ.Табель.ТабличнаяЧасть.ОтработанноеВремяЗаПериод" Тогда
		
		Образец_xlsx = ПолучитьМакет("ОбразецЗагрузкиДанных_ЗаПериод_xlsx");
		ОбразецЗагрузкиДанных_xlsx = ПоместитьВоВременноеХранилище(Образец_xlsx, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_xlsx", ОбразецЗагрузкиДанных_xlsx);
		
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_mxl", "ОбразецЗагрузкиДанных_ЗаПериод_mxl");
		
		Образец_csv = ПолучитьМакет("ОбразецЗагрузкиДанных_ЗаПериод_csv");
		ОбразецЗагрузкиДанных_csv = ПоместитьВоВременноеХранилище(Образец_csv, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_csv", ОбразецЗагрузкиДанных_csv);
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Сотрудник", НСтр("ru = 'Сотрудник (ФИО или таб. номер)'; ka = 'Сотрудник (ФИО или таб. номер)'"), ОписаниеТиповСтрока150, ОписаниеТиповКолонка, , , Истина);
	
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Должности");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Должность", НСтр("ru = 'Должность'; ka = 'Должность'"), ОписаниеТиповСтрока100, ОписаниеТиповКолонка, , , Истина);
	
	Если НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Документ.Табель.ТабличнаяЧасть.ОтработанноеВремяПоДням" Тогда
		Для НомСтр = 1 По 31 Цикл
			ИмяПоля = "День" + Строка(НомСтр);
			ЗаголовокПоля = НСтр("ru = 'День '; ka = 'День '") + Строка(НомСтр);
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, ИмяПоля, ЗаголовокПоля, ОписаниеТиповСтрока8, ОписаниеТиповСтрока8);
		КонецЦикла;
		
	ИначеЕсли НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Документ.Табель.ТабличнаяЧасть.ОтработанноеВремяЗаПериод" Тогда
		Для НомСтр = 1 По 6 Цикл
			ИмяПоля = "ВидВремени" + Строка(НомСтр);
			ЗаголовокПоля = Нстр("ru = 'ВидВремени ('; ka = 'ВидВремени ('") + Строка(НомСтр) 
							+ НСтр("ru = ') / Дней ('; ka = ') / Дней ('") + Строка(НомСтр) 
							+ НСтр("ru = ') / Часов ('; ka = ') / Часов ('") + Строка(НомСтр) + Нстр("ru=')';ka=')'");
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, ИмяПоля, ЗаголовокПоля, ОписаниеТиповСтрока3, ОписаниеТиповСтрока3);
		КонецЦикла;
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных) Экспорт
	...
	Если НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Документ.Табель.ТабличнаяЧасть.ОтработанноеВремяПоДням" Тогда
		Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
			...
			ТекСотрудник = СтрокаТаблицыФормы.Сотрудник;
			// Сотрудники
			...
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьДолжность(СтрокаТаблицыФормы.Должность, СтрокаТаблицыФормы.Должность_ВходящиеДанные);
			
			Для НомСтр = 1 По 31 Цикл
				ИмяПоля = "День" + Строка(НомСтр);
				ИмяПояВхДанные = ИмяПоля + "_ВходящиеДанные";
				СтрокаТаблицыФормы[ИмяПоля] = СтрокаТаблицыФормы[ИмяПояВхДанные];
			КонецЦикла;
