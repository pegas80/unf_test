...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СписаниеЗапасов", НСтр("ru='Списание запасов';ka='მარაგების ჩამოწერა'"));
...
Процедура СформироватьТаблицаЗапасыРаботы(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СборкаЗапасов", НСтр("ru='Сборка запасов';ka='მარაგების შეკრება'"));
	РезультатЗапроса = Запрос.Выполнить();
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуется <> 0 Тогда
			...
			СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ru='Списание запасов';ka='მარაგების ჩამოწერა'");
				...
				Если КоличествоТребуетсяРезерв > 0 Тогда // Нужно сделать резерв.
					...
					Если СтрокаТаблицаЗапасы.Выполнен Тогда // Если заказ выполнен - надо и продать.
						...
						Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
							...
							СтрокаДоходыИРасходы.СодержаниеПроводки = НСтр("ru='Отражение расходов';ka='ხარჯების ასახვა'");
...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПриемЗапасовТоварыНаКомиссии", НСтр("ru='Прием запасов';ka='მარაგების მიღება'"));
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Выручка", НСтр("ka='შემოსავალი გაყიდვიდან';ru='Выручка от продажи'"));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='1-Т (Товарно-транспортная накладная)';ka='1-Т (სატრანსპორტო-სასაქონლო ზედნადები)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Универсальный передаточный документ';ka='უნივერსალური გადასაცემი დოკუმენტი'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Универсальный корректировочный документ';ka='უნივერსალური საკორექტირებო დოკუმენტი'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 23;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СчетФактура";
	КомандаПечати.Представление = НСтр("ru='Счет-фактура';ka='Счет-фактура'");
