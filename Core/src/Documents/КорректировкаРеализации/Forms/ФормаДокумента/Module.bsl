...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Укажите вначале корректируемый документ.';ka='თავდაპირველად, მიუთითეთ კორექტირებადი დოკუმენტი.'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Табличная часть ""Запасы и услуги"" не заполнена!';ka='ცხრილი ""მარაგები და მომსახურებები"" შევსებული არ არის!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
