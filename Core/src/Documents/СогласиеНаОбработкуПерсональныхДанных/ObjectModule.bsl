...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(СрокДействия) И СрокДействия < ДатаПолучения Тогда
		ТекстСообщения = НСтр("ka='თანხმობის მოქმედების ვადა მიღების თარიღს ეწინააღმდეგება.';ru='Срок действия согласия противоречит дате получения.'");
	...
	Если Согласие <> Неопределено Тогда
		Если ДатаПолучения = Согласие.ДатаПолучения Тогда
			ТекстСообщения = НСтр("ka='აღნიშნული თარიღისთვის უკვე რეგისტრირებულია თანხმობა.';ru='На указанную дату уже зарегистрировано согласие.'");
