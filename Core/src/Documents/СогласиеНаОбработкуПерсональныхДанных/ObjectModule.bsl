...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(СрокДействия) И СрокДействия < ДатаПолучения Тогда
		ТекстСообщения = НСтр("ka='თანხმობის მოქმედების ვადა მიღების თარიღს ეწინააღმდეგება.';ru='Срок действия согласия противоречит дате получения.';en='Term of agreement action contradicts the date of receipt.'");
	...
	Если Согласие <> Неопределено Тогда
		Если ДатаПолучения = Согласие.ДатаПолучения Тогда
			ТекстСообщения = НСтр("ka='აღნიშნული თარიღისთვის უკვე რეგისტრირებულია თანხმობა.';ru='На указанную дату уже зарегистрировано согласие.';en='At that date already registered consent.'");
