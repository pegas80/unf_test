...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='თანხმობა პერსონალურ მონაცემთა დამუშავებაზე (1C:საწარმოს ცხრილი-დოკუმენტი)';ru='Согласие на обработку ПДн (табличный документ 1С:Предприятия)'");
	
	// В формате Microsoft Word.
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СогласиеНаОбработкуПерсональныхДанных(MSWord)";
	КомандаПечати.Представление = НСтр("ka='თანხმობა პერსონალურ მონაცემთა დამუშავებაზე (დოკუმენტი Microsoft Word-ი)';ru='Согласие на обработку ПДн (документ Microsoft Word)'");
	...
	КомандаПечати.Представление = НСтр("ka='პერსონალური მონაცემების დამუშავების თანხმობა (დოკუმენტი OpenOffice.org Writer)';ru='Согласие на обработку ПДн (документ OpenOffice.org Writer)'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		Шаблон = НСтр("ka='პერსონალური მონაცემების დამუშავებაზე (#[ნომერი]) [სუბიექტი] - [ორგანიზაცია] თანხმობა მიღებულია [მიღებისთარიღი]';ru='Согласие на обработку ПДн [Субъект] - [Организация] (№[Номер] получено [ДатаПолучения])'");
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='თანხმობა პერსონალური მონაცემების დამუშავებაზე';ru='Согласие на обработку ПДн'");
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='თანხმობა პერსონალურ მონაცემთა დამუშავებაზე (დოკუმენტი Microsoft Word-ი)';ru='Согласие на обработку ПДн (документ Microsoft Word)'"),
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='პერსონალური მონაცემების დამუშავების თანხმობა (დოკუმენტი OpenOffice.org Writer)';ru='Согласие на обработку ПДн (документ OpenOffice.org Writer)'"),
			ПечатьСогласияНаОбработкуПДн(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ДанныеДляПечатиСогласий(Согласия)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.СрокДействия) Тогда
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-დან %2-ის ჩათვლით.';ru='с %1 по %2.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"), Формат(Выборка.СрокДействия, "ДЛФ=D"));
		Иначе
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1-იდან უვადოდ.';ru='с %1 бессрочно.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"));
		КонецЕсли;
...
Функция ОписаниеСогласия()
	...
	ОписаниеСогласия.ДатаСогласия = НСтр("ka='«____»_______________20___г.';ru='«____»_______________20___г.'");
	ОписаниеСогласия.Организация = НСтр("ka='<Организация>';ru='<Организация>'");
	ОписаниеСогласия.АдресОрганизации = НСтр("ka='<Адрес организации>';ru='<Адрес организации>'");
	ОписаниеСогласия.ОтветственныйЗаОбработкуПерсональныхДанных = НСтр("ka='<პასუხისმგებელი პირის გვარ-სახელი>';ru='<ФИО ответственного лица>'");
