...
Процедура УстановитьСведенияДействующегоСогласия()
	...
	Если Согласие = Неопределено Тогда
		...
		Элементы.СведенияТекст.Заголовок = НСтр("ka='სუბიექტის პერსონალურ მონაცემთა დამუშავებაზე მოქმედი თანხმობა ვერ მოიძებნა.';ru='Не обнаружено действующего согласия на обработку персональных данных субъекта.';en='Not found the current consent to the processing of personal data subject.'");
		Возврат;
	КонецЕсли;
	...
	ЧастиСообщения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='სუბიექტ %1-ს მიღებული აქვს';ru='У субъекта %1 было получено';en='At 1% of the subject was obtained'"), Формат(Согласие.ДатаПолучения, "ДЛФ=D")) + " ");
	ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(НСтр("ka='თანხმობა';ru='согласие';en='agreement'"), , , , ПолучитьНавигационнуюСсылку(Согласие.ДокументОснование)));
	Если ЗначениеЗаполнено(Согласие.СрокДействия) Тогда
		ЧастиСообщения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka=', которое действует до %1.';ru=', которое действует до %1.';en='Which operates to 1%.'"), Формат(Согласие.СрокДействия, "ДЛФ=D")));
	Иначе	
		ЧастиСообщения.Добавить(НСтр("ka=', რომელიც უვადოდ მოქმედებს.';ru=', которое действует бессрочно.';en='Which is valid indefinitely.'"));
	КонецЕсли;
