...
Процедура ОтобразитьДанныеСотрудника()
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда 
		Фамилия  = ?(ЗначениеЗаполнено(ДанныеТекущейСтроки.Фамилия), ВРег(ДанныеТекущейСтроки.Фамилия), НСтр("ka='<არ არის შევსებული>';ru='<Не заполнено>'"));
		Имя      = ?(ЗначениеЗаполнено(ДанныеТекущейСтроки.Имя), ВРег(ДанныеТекущейСтроки.Имя), НСтр("ka='<არ არის შევსებული>';ru='<Не заполнено>'"));
		Отчество = ВРег(ДанныеТекущейСтроки.Отчество);
		СтраховойНомерПФР =  ?(ЗначениеЗаполнено(ДанныеТекущейСтроки.СтраховойНомерПФР), ВРег(ДанныеТекущейСтроки.СтраховойНомерПФР), НСтр("ka='<არ არის შევსებული>';ru='<Не заполнено>'")); 
		ДанныеСотрудникаТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომლის მონაცემები: %1';ru='Данные сотрудника: %1'"), Строка(ДанныеТекущейСтроки.Сотрудник));
	Иначе
		Фамилия  ="";
		Имя      = "";
		Отчество = "";
		СтраховойНомерПФР = "";
		ДанныеСотрудникаТекст = Нстр("ka='თანამშრომლის მონაცემები:';ru='Данные сотрудника:'");
	КонецЕсли;	
...
Процедура ЗаполнитьСведенияОЗаработке(Команда)
	...
	Если Объект.СведенияОЗаработке.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='ხელფასის შესახებ ინფორმაციის შესავსებად მონაცემები არ არის აღმოჩენილი.';ru='Не обнаружены данные для заполнения сведений о заработке.'"));
	КонецЕсли;	
КонецПроцедуры
...
Процедура СотрудникиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если ТекущиеДанные <> Неопределено И Не ОтменаРедактирования Тогда
		...
		Если Объект.Сотрудники.НайтиСтроки(СтруктураПоиска).Количество() > 1 Тогда
			ПоказатьПредупреждение(,НСтр("ka='დასტაში არ შეიძლება, რომ თანამშრომლები მეორდებოდეს!';ru='В пачке сотрудники не могут повторяться!'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
