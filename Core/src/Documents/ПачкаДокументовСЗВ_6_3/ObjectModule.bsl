...
Процедура ПроверкаЗаполненияДокумента(ПроверяемыеРеквизиты, НеПроверяемыеРеквизиты, Отказ)Экспорт
	...
	Если КоличествоДокументов = 0 Тогда
		ТекстОшибки = Нстр("ka='დაზღვეული პირების სია ცარიელია!';ru='Список застрахованных лиц пуст!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, , ,Отказ);
	КонецЕсли;
	Если КоличествоДокументов > 200 Тогда
		ТекстОшибки = ТекстОшибки = Нстр("ka='დოკუმენტში 200 ფორმაზე მეტი არ უნდა იყოს!';ru='В документе должно быть не более 200 форм!'"); 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, , ,Отказ);
	КонецЕсли;
	...
	Пока ВыборкаПоСтрокамДокумента.Следующий() цикл
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда
			...
			Если Не ВыборкаПоСтрокамДокумента.СотрудникРаботаетВОрганизации тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელ %1-ს საანგარიშგებო პერიოდის განმავლობაში არ უმუშავია ორგანიზაცია %2-ში';ru='Сотрудник %1 в течение отчетного периода не работал в организации %2'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование, НаименованиеОрганизации); 
			...
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПовторяющейсяСтроки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1 დოკუმენტში ორჯერ არის მითითებული';ru='Сотрудник %1 указан в документе дважды'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			ИначеЕсли ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПовторяющейсяСтрокиСтраховойНомер) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: ინფორმაცია ამავე სადაზღვევო ნომრის მქონე თანამშრომლის შესახებ ადრე უკვე შეყვანილი იქნა დოკუმენტში!';ru='Сотрудник %1: информация о сотруднике с таким же страховым номером была введена в документе ранее!'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВсегоЗаработка) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: სახელფასო მონაცემები არ არის';ru='Сотрудник %1: отсутствуют данные о заработке'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			Иначе
				Если ВыборкаПоСтрокамДокумента.НаименьшийМесяц < 1 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: ერთ-ერთი თვე არასწორად არის მითითებული - 1-ზე ნაკლები';ru='Сотрудник %1: неверно указан один из месяцев - меньше 1'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
				КонецЕсли;
				Если ВыборкаПоСтрокамДокумента.МинимальныйЗаработок < 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: ერთი ან რამდენიმე თვისთვის მითითებულია უარყოფითი ხელფასი';ru='Сотрудник %1: за один или несколько месяцев указан отрицательный заработок'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
				КонецЕсли;
				Если ВыборкаПоСтрокамДокумента.МинимальныйОблагаетсяВзносами < 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: ერთი ან რამდენიმე თვისთვის მითითებულია უარყოფითი დაბეგვრადი თანხა';ru='Сотрудник %1: за один или несколько месяцев указана отрицательная облагаемая сумма'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
				КонецЕсли;
			КонецЕсли;
			Если ВыборкаПоСтрокамДокумента.НаибольшийМесяц > 12 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: არასწორად არის მითითებული ერთ-ერთი თვე, აღემატება 12-ს';ru='Сотрудник %1: неверно указан один из месяцев, превышает 12'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ФизЛицоСДвойнымиМесяцами) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: ხელფასის შესახებ მონაცემებში ერთ-ერთი თვე რამდენიმეჯერ მეორდება!';ru='Сотрудник %1: в данных о заработке один из месяцев повторяется несколько раз!'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ФизЛицоСНевернымиОблагаемымиСуммами) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ka='თანამშრომელი %1: სახელფასო მონაცემებში შენატანებით დაბეგვრადი თანხა დარიცხული ხელფასის თანხას აღემატება!';ru='Сотрудник %1: в данных о заработке сумма, облагаемая взносами, превышает сумму начисленного заработка!'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
