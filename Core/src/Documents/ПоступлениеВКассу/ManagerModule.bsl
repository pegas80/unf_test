...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПоступлениеРозничнойВыручки", НСтр("ka='საცალო გაყიდვის ამონაგების მიღება';ru='Поступление розничной выручки'"));
...
Процедура СформироватьТаблицаДенежныеСредстваВКассахККМ(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("РасходДенежныхСредств", НСтр("ka='ფულადი სახსრების დანახარჯი სსა-ის სალაროდან';ru='Расход денежных средств из кассы ККМ'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеРозничнаяВыручка", НСтр("ka='საცალო გაყიდვიდან მიღებული შემოსავალი';ru='Розничная выручка'"));
	Запрос.УстановитьПараметр("СодержаниеСебестоимость", НСтр("ka='თვითღირებულება';ru='Себестоимость'"));
	Запрос.УстановитьПараметр("СодержаниеНаценка", НСтр("ka='ფასნამატი';ru='Наценка'"));
...
Процедура СформироватьТаблицаСуммовойУчетВРознице(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("РозничнаяВыручка", НСтр("ka='საცალო გაყიდვიდან მიღებული შემოსავალი';ru='Розничная выручка'"));
	Запрос.УстановитьПараметр("Себестоимость", НСтр("ka='თვითღირებულება';ru='Себестоимость'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПКО") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПКО", НСтр("ka='КО-1 (შემოსვლის სალაროს ორდერი)';ru='КО-1 (Приходный кассовый ордер)'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='КО-1 (შემოსვლის სალაროს ორდერი)';ru='КО-1 (Приходный кассовый ордер)'");
