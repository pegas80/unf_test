...
Процедура ЗаполнитьРасшифровкуПлатежа(ОстаткиПолучатьНаДатуДокумента = Ложь) Экспорт
	...
	//Kuzmich, 06.09.2023, ONESOFT-2502) Поступление в кассу / операция по платежным картам - кнопку заполнить
	|	ГДЕ
...
Процедура ЗаполнитьПоРасходнойНакладной(ДанныеЗаполнения) Экспорт
	...
	КАССА= ПолучитьКассуТекПользователя(ТекПользователь) ;
	Если ЗначениеЗаполнено(КАССА) Тогда 	
		ЭтотОбъект.Касса = КАССА ;
	КонецЕсли;
	//Lazare_
...
//+Lazare 
Процедура ЗаполнитьПоПродажаЛицензии(ДанныеЗаполнения) Экспорт
	...
	|	ВЫРАЗИТЬ(ПродажаЛицензииЗапасы.Ссылка.Контрагент.НаименованиеПолное КАК СТРОКА(1024)) КАК ПринятоОт,
	|	ПРЕДСТАВЛЕНИЕ(ПродажаЛицензииЗапасы.Ссылка.Контрагент) КАК ПредставлениеКонтрагента,
	|	ПродажаЛицензииЗапасы.Ссылка.Организация.КассаПоУмолчанию КАК Касса,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	...
	|	ПродажаЛицензииЗапасы.Ссылка.Организация.КассаПоУмолчанию,
	...
	|	ВЫРАЗИТЬ(ПродажаЛицензииЗапасы.Ссылка.Контрагент.НаименованиеПолное КАК СТРОКА(1024))";
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	СтратегияЗаполнения[Тип("ДокументСсылка.ПродажаЛицензии")] = "ЗаполнитьПоПродажаЛицензии"; //++Lazare 
...
Процедура ЗаполнитьПоДоговоруКредитаЗайма(ДокументОснование, Сумма = Неопределено) Экспорт
	...
	Если Сумма <> Неопределено Тогда
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ВозвратКредитаКонтрагентом)
...
//Lazare_
&НаСервере
Функция ПолучитьКассуТекПользователя(ТекПользователь)	
	Перем КАССА;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НастройкиПользователей.Настройка.ИмяПредопределенныхДанных КАК Касса,
	|	НастройкиПользователей.Значение.Ссылка
	|ИЗ
	|	РегистрСведений.НастройкиПользователей КАК НастройкиПользователей
	|ГДЕ
	|	НастройкиПользователей.Пользователь = &Пользователь
	|	И НастройкиПользователей.Настройка.ИмяПредопределенныхДанных = ""ОсновнаяКасса""";
	
	Запрос.УстановитьПараметр("Пользователь", ТекПользователь);	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	Для Каждого ТЧ Из РезультатЗапроса Цикл
		КАССА = ТЧ.ЗначениеСсылка ;			
	КонецЦикла;
