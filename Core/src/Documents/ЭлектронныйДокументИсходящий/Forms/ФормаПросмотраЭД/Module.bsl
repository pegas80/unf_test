...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Заголовок = НСтр("ka='Произвольный документ (создание)';ru='Произвольный документ (создание)';en='An arbitrary document (creation)'");
...
Процедура ВыбратьДокумент(Команда)
	...
	ТекстВопроса = НСтр("ka='Не рекомендуется выбирать документ отражения в учете вручную. Продолжить?';ru='Не рекомендуется выбирать документ отражения в учете вручную. Продолжить?';en='Do not select the document reflected in the account manually. Proceed?'");
...
Процедура Завершить(Команда)
	...
	ВводСтрокиЗаголовок = НСтр("ka='მიუთითეთ დოკუმენტბრუნვის დასრულების მიზეზები';ru='Укажите причины завершения документооборота';en='Identify the reasons for the completion of document'");
...
Процедура ВывестиДокументыУчета()
	...
	Элементы.ТекстДокументИБ.Заголовок = НСтр("ka='აღრიცხვის დოკუმენტი';ru='Документ учета';en='accounting document'");
	
	Если КоличествоДокументов = 0 Тогда
		
		Если Объект.ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
			ПредставлениеДокументов = НСтр("ka='ნმენკლატურის შედარება';ru='Сопоставить номенклатуру';en='Сопоставить номенклатуру'");
			Элементы.ТекстДокументИБ.Заголовок = НСтр("ka='ЭДО აწყობა';ru='Настройка ЭДО';en='Setting up the EDI'");
		ИначеЕсли Объект.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			ПредставлениеДокументов = НСтр("ka='<Подбор>';ru='<Подбор>';en='<Selection>'");
		Иначе
			ПредставлениеДокументов = НСтр("ka='Отразить в учете';ru='Отразить в учете';en='Record a'");
		КонецЕсли;
	...
	Иначе
		...
		Элементы.ТекстДокументИБ.Заголовок = НСтр("ka='აღრიცხვის დოკუმენტები';ru='Документы учета';en='accounting documents'");
		ШаблонТекста = НСтр("ka='Список документов (%1)';ru='Список документов (%1)';en='The list of documents (% 1)'");  
	...
	Если ОтображатьПодбор Тогда
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
			НСтр("ka='<Подбор>';ru='<Подбор>';en='<Selection>'"), ,
			, ,
			"ОткрытьФормуПодбора"));
	КонецЕсли;
...
Процедура ПерезаполнитьКомментарии()
	...
	ШаблонКомментария = НСтр("ka='%1, %2 (სტატუსი - %3, პასუხისმგებელი - %4):"
"%5';ru='%1, %2 (статус - %3, ответственный - %4):"
"%5'");
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписиЭД")
		...
		Элементы.КомандаУтвердить.Заголовок = НСтр("ka='დამტკიცება და გაგზავნა';ru='Утвердить и отправить';en='Approve and send'");
	КонецЕсли;
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Если НЕ ЭтоСлужебный И СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.СчетФактура
			...
			Элементы.КомандаОтклонить.Заголовок = НСтр("ka='ელექტრონული დოკუმენტის თაობაზე დაზუსტების მოთხოვნა';ru='Запросить уточнение по электронному документу';en='Ask for clarification of the electronic document'");
	...
	Если СтатусОтклонен Тогда
		...
		Если СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.ОшибкаПередачи Тогда
			Элементы.ГруппаОтклонение.Заголовок = НСтр("ka='გაცვლის შეცდომა';ru='Ошибка обмена';en='error sharing'");
		КонецЕсли;
	...
	ИначеЕсли ЭДОАннулированИлиВПроцессе() Тогда
		...
		НовСтрока.ПричинаОтклонения = СвойстваЭД.ПричинаОтклонения;
		Элементы.СтраницаОтклонение.Видимость = Истина;
		Элементы.ГруппаОтклонение.Заголовок = НСтр("ka='ანულირების მიზეზი:';ru='Причина аннулирования:';en='Reason for cancellation:'");
	...
	Элементы.ГруппаШапки.ЗаголовокСвернутогоОтображения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Состояние: %1';en='Status:% 1'"),
	...
	Если НаправлениеИсходящий Тогда
		...
		Если ЕстьВложения Тогда
			...
			Если ДанныеФайла.ФайлРедактируется Тогда
				ТекстРедактируетФайл = НСтр("ka='ფაილი რედაქტირებისთვის შეპყრობილია მომხმარებელ %1-ის მიერ';ru='Файл захвачен на редактирование пользователем %1';en='Image captured by a user editing% 1'");
			...
			ГруппаКомандДобавитьЗаменить.Заголовок = СтрЗаменить(ГруппаКомандДобавитьЗаменить.Заголовок,
				НСтр("ka='დამატება';ru='Добавить';en='Add'"), НСтр("ka='გამოცვლა (რით)';ru='Заменить на';en='Replaced by'"));
			...
			КонецЕсли;
		Иначе
			ГруппаКомандДобавитьЗаменить.Заголовок = СтрЗаменить(ГруппаКомандДобавитьЗаменить.Заголовок, НСтр("ka='გამოცვლა (რით)';ru='Заменить на';en='Replaced by'"), НСтр("ka='დამატება';ru='Добавить';en='Add'"));
...
Процедура ЗаполнитьТаблицуЭП()
	...
	Элементы.СтраницаПодписиИСтатусы.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ხელმოწერები %1';ru='Подписи %1';en='Signature 1%'"), 
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.ПодписьВернаПредставление = ?(ТекСтрока.ПодписьВерна, НСтр("ka='სწორია';ru='Верна';en='Verne'"), НСтр("ka='არ არის სწორი';ru='Неверна';en='incorrectly'"))
			+" (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.ПодписьВернаПредставление = НСтр("ka='არ არის შემოწმებული';ru='Не проверена';en='not tested'");
	КонецЕсли
...
Процедура ДобавитьСертификатВДоверенные(ДанныеПодписи)
	...
	Если ДанныеПодписи <> Неопределено И ДанныеПодписи.ОтсутствуетВСписке Тогда 
		ТекстВопроса = НСтр("ka='სერთიფიკატი %1 დავამატოთ კონტრაგენტის მოსალოდნელი სერთიფიკატების სიაში?';ru='Добавить сертификат %1 в список ожидаемых сертификатов контрагента?';en='Add the certificate% 1 in the list of expected counterpart certificates?'");
...
Процедура ПередЗаписьюПродолжитьПослеОсвобожденияВложения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		Записать();
	Иначе
		ТекстСообщения = НСтр("ka='ოპერაციის შესრულებისას შეცდომა მოხდა. მოქმედება გაუქმებულია. დაწვრილებით, იხ. რეგისტრაციის ჟურნალში.';ru='Во время выполнения операции произошла ошибка. Действие отменено."
...
Процедура ПроверитьЗахватНаРедактирование(ОбработкаПродолжения, ДанныеФайла)
	...
	Если ДанныеФайла.ФайлРедактируется Тогда
		Если ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда
			...
			ТекстВопроса = НСтр("ka='დანართი ""%1"" შეპყრობილია რედაქტირებისთვის."
"გასაგრძელებლად საჭიროა რედაქტირების დასრულება.';ru='Вложение ""%1"" захвачено для редактирования."
"Для продолжения надо завершить редактирование.'");
			ТекстВопроса = СтрЗаменить(ТекстВопроса, "%1", ПрисоединенныйФайлИмяФайла);
			СписокКнопок = Новый СписокЗначений;
			СписокКнопок.Добавить("ЗакончитьРедактирование", НСтр("ka='რედაქტირების დასრულება';ru='Закончить редактирование';en='finish editing'"));
			СписокКнопок.Добавить("Освободить",              НСтр("ka='რედაქტირების გაუქმება';ru='Отменить редактирование';en='cancel editing'"));
			СписокКнопок.Добавить("Отменить",                НСтр("ka='გაუქმება';ru='Отменить';en='cancel'"));
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок);
		Иначе
			ТекстПредупреждения = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი დაკავებულია სხვა მომხმარებლის მიერ რედაქტირებისთვის.';ru='Действие недоступно, так как файл занят для редактирования"
...
Функция ПроверитьДействиеРазрешено(ДанныеФайла, Знач ТекущееДействие = "")
	...
	Если ТекущееДействие = "ПометкаУдаления" И ДанныеФайла.ФайлРедактируется Тогда
		...
		Если ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда
			ТекстПредупреждения = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი დაკავებულია რედაქტირებისთვის.';ru='Действие недоступно, так как файл занят для редактирования.';en='Action is not available because the file is not available for editing.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი დაკავებულია სხვა მომხმარებლის მიერ რედაქტირებისთვის.';ru='Действие недоступно, так как файл занят для редактирования"
"другим пользователем.'");
		КонецЕсли;
...
Процедура ВыбратьДокументПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Подсказка = НСтр("ka='მიუთითეთ აღრიცხვაში ასახვის დოკუმენტი';ru='Укажите документ отражения в учете';en='Specify the document in accounting'");
...
Процедура ДобавитьСертификатВДоверенныеЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		...
		Если НЕ СертификатДобавлен Тогда 
			ТекстСообщения = НСтр("ru='Ошибка добавления сертификата подписи в список ожидаемых сертификатов.';en='Failed to add the signing certificate in the list of expected certificates.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Иначе
			ЗаполнитьТаблицуЭП();
		КонецЕсли;
