...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Заголовок = НСтр("ka='Произвольный документ (создание)';ru='Произвольный документ (создание)'");
...
Процедура ВыбратьДокумент(Команда)
	...
	ТекстВопроса = НСтр("ka='Не рекомендуется выбирать документ отражения в учете вручную. Продолжить?';ru='Не рекомендуется выбирать документ отражения в учете вручную. Продолжить?'");
...
Процедура Завершить(Команда)
	...
	ВводСтрокиЗаголовок = НСтр("ka='მიუთითეთ დოკუმენტბრუნვის დასრულების მიზეზები';ru='Укажите причины завершения документооборота'");
...
Процедура ВывестиДокументыУчета()
	...
	Элементы.ТекстДокументИБ.Заголовок = НСтр("ka='აღრიცხვის დოკუმენტი';ru='Документ учета'");
	
	Если КоличествоДокументов = 0 Тогда
		
		Если Объект.ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
			ПредставлениеДокументов = НСтр("ka='ნმენკლატურის შედარება';ru='Сопоставить номенклатуру'");
			Элементы.ТекстДокументИБ.Заголовок = НСтр("ka='ЭДО აწყობა';ru='Настройка ЭДО'");
		ИначеЕсли Объект.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			ПредставлениеДокументов = НСтр("ka='<Подбор>';ru='<Подбор>'");
		Иначе
			ПредставлениеДокументов = НСтр("ka='Отразить в учете';ru='Отразить в учете'");
		КонецЕсли;
	...
	Иначе
		...
		Элементы.ТекстДокументИБ.Заголовок = НСтр("ka='აღრიცხვის დოკუმენტები';ru='Документы учета'");
		ШаблонТекста = НСтр("ka='Список документов (%1)';ru='Список документов (%1)'");  
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписиЭД")
		...
		Элементы.КомандаУтвердить.Заголовок = НСтр("ka='დამტკიცება და გაგზავნა';ru='Утвердить и отправить'");
	КонецЕсли;
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Если НЕ ЭтоСлужебный И СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.СчетФактура
			...
			Элементы.КомандаОтклонить.Заголовок = НСтр("ka='ელექტრონული დოკუმენტის თაობაზე დაზუსტების მოთხოვნა';ru='Запросить уточнение по электронному документу'");
	...
	Если СтатусОтклонен Тогда
		...
		Если СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.ОшибкаПередачи Тогда
			Элементы.ГруппаОтклонение.Заголовок = НСтр("ka='გაცვლის შეცდომა';ru='Ошибка обмена'");
		КонецЕсли;
	...
	ИначеЕсли ЭДОАннулированИлиВПроцессе() Тогда
		...
		НовСтрока.ПричинаОтклонения = СвойстваЭД.ПричинаОтклонения;
		Элементы.СтраницаОтклонение.Видимость = Истина;
		Элементы.ГруппаОтклонение.Заголовок = НСтр("ka='ანულირების მიზეზი:';ru='Причина аннулирования:'");
	...
	Если НаправлениеИсходящий Тогда
		...
		Если ЕстьВложения Тогда
			...
			Если ДанныеФайла.ФайлРедактируется Тогда
				ТекстРедактируетФайл = НСтр("ka='ფაილი რედაქტირებისთვის შეპყრობილია მომხმარებელ %1-ის მიერ';ru='Файл захвачен на редактирование пользователем %1'");
			...
			ГруппаКомандДобавитьЗаменить.Заголовок = СтрЗаменить(ГруппаКомандДобавитьЗаменить.Заголовок,
				НСтр("ka='დამატება';ru='Добавить'"), НСтр("ka='გამოცვლა (რით)';ru='Заменить на'"));
			...
			КонецЕсли;
		Иначе
			ГруппаКомандДобавитьЗаменить.Заголовок = СтрЗаменить(ГруппаКомандДобавитьЗаменить.Заголовок, НСтр("ka='გამოცვლა (რით)';ru='Заменить на'"), НСтр("ka='დამატება';ru='Добавить'"));
...
Процедура ЗаполнитьТаблицуЭП()
	...
	Элементы.СтраницаПодписиИСтатусы.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='ხელმოწერები %1';ru='Подписи %1'"), 
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.ПодписьВернаПредставление = ?(ТекСтрока.ПодписьВерна, НСтр("ka='სწორია';ru='Верна'"), НСтр("ka='არ არის სწორი';ru='Неверна'"))
			+" (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.ПодписьВернаПредставление = НСтр("ka='არ არის შემოწმებული';ru='Не проверена'");
	КонецЕсли
...
Процедура ДобавитьСертификатВДоверенные(ДанныеПодписи)
	...
	Если ДанныеПодписи <> Неопределено И ДанныеПодписи.ОтсутствуетВСписке Тогда 
		ТекстВопроса = НСтр("ka='სერთიფიკატი %1 დავამატოთ კონტრაგენტის მოსალოდნელი სერთიფიკატების სიაში?';ru='Добавить сертификат %1 в список ожидаемых сертификатов контрагента?'");
...
"Для продолжения надо завершить редактирование.'");
			...
			СписокКнопок.Добавить("ЗакончитьРедактирование", НСтр("ka='რედაქტირების დასრულება';ru='Закончить редактирование'"));
			СписокКнопок.Добавить("Освободить",              НСтр("ka='რედაქტირების გაუქმება';ru='Отменить редактирование'"));
			СписокКнопок.Добавить("Отменить",                НСтр("ka='გაუქმება';ru='Отменить'"));
...
Функция ПроверитьДействиеРазрешено(ДанныеФайла, Знач ТекущееДействие = "")
	...
	Если ТекущееДействие = "ПометкаУдаления" И ДанныеФайла.ФайлРедактируется Тогда
		...
		Если ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда
			ТекстПредупреждения = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი დაკავებულია რედაქტირებისთვის.';ru='Действие недоступно, так как файл занят для редактирования.'");
		Иначе
			ТекстПредупреждения = НСтр("ka='მოქმედება მიუწვდომელია, ვინაიდან ფაილი დაკავებულია სხვა მომხმარებლის მიერ რედაქტირებისთვის.';ru='Действие недоступно, так как файл занят для редактирования"
"другим пользователем.'");
		КонецЕсли;
...
Процедура ВыбратьДокументПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Подсказка = НСтр("ka='მიუთითეთ აღრიცხვაში ასახვის დოკუმენტი';ru='Укажите документ отражения в учете'");
