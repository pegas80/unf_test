	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
				...
				Если СписаниеНаОС Тогда 
					СтрокаТаблицаХозрасчетный.Содержание = НСтр("ka='კაპიტალური მოდერნიზაცია';ru='Капитальная модернизация'");
				КонецЕсли;
	 ...
	 ИЛИ ВозвратИзРозницыСуммовойУчет Тогда
		...
		Если СуммаНаценка <> 0 Тогда
			...
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицаУправленческий.Содержание = НСтр("ka='სავაჭრო ფასნამატი';ru='Торговая наценка'");
				...
				СтрокаТаблицаХозрасчетный.Содержание = НСтр("ka='სავაჭრო ფასნამატი';ru='Торговая наценка'");
...
Процедура СформироватьТаблицаСуммовойУчетВРознице(ДокументСсылкаПеремещениеЗапасов, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПеремещениеВРозницу", НСтр("ka='საცალო ვაჭრობაში გადაადგილება';ru='Перемещение в розницу'"));
	Запрос.УстановитьПараметр("ПеремещениеВРознице", НСтр("ka='საცალო ვაჭრობაში გადაადგილება';ru='Перемещение в рознице'"));
	Запрос.УстановитьПараметр("ВозвратИзРозницы", НСтр("ka='საცალო გაყიდვიდან დაბრუნება';ru='Возврат из розницы'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПеремещениеЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	|		ИЗ
	|			РегистрСведений.ПараметрыВнеоборотныхАктивов.СрезПоследних(&ДатаДокумента, ) КАК ПараметрыВнеоборотныхАктивовСрезПоследних) КАК ВложенныйЗапрос
	...
	Запрос.УстановитьПараметр("ПеремещениеЗапасов", НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ka='სასაქონლო შევსების ბლანკი (გამგზავნი)';ru='Бланк товарного наполнения (Отправитель)'");
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = Обработки.ПечатьБланкТоварногоНаполнения.КлючПараметровПечати();
		ПечатнаяФорма.ПолныйПутьКМакету = Обработки.ПечатьБланкТоварногоНаполнения.ПолныйПутьКМакету();
		ПечатнаяФорма.СинонимМакета = НСтр("ka='სასაქონლო გავსების ბლანკი (მიმღები)';ru='Бланк товарного наполнения (Получатель)'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='М11 (მოთხოვნა-ზედნადები)';ru='М11 (Требование-накладная)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 4;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ТОРГ13";
	КомандаПечати.Представление = НСтр("ka='ТОРГ13 (შიდა გადაადგილების ზედნადები)';ru='ТОРГ13 (Накладная на внутреннее перемещение)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 7;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "МХ18";
	КомандаПечати.Представление = НСтр("ka='МХ18 (მზა პროდუქციის გადაცემის ზედნადები)';ru='МХ18 (Накладная на передачу готовой продукции)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ka='დანართი #4 (სატრანსპორტო ზედნადები)';ru='Приложение №4 (Транспортная накладная)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 17;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПеремещениеЗапасов";
	КомандаПечати.Представление = НСтр("ka='მარაგების გადაადგილება';ru='Перемещение запасов'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 20;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "БланкТоварногоНаполненияОтправитель";
	КомандаПечати.Представление = НСтр("ka='სასაქონლო შევსების ბლანკი (გამგზავნი)';ru='Бланк товарного наполнения (Отправитель)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 23;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "БланкТоварногоНаполненияПолучатель";
	КомандаПечати.Представление = НСтр("ka='სასაქონლო გავსების ბლანკი (მიმღები)';ru='Бланк товарного наполнения (Получатель)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ka='მძღოლის ფორმა';ru='Форма водителя'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
