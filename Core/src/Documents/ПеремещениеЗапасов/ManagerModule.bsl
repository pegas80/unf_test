...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаПеремещениеЗапасов, СтруктураДополнительныеСвойства)
	...
	Если ДокументСсылкаПеремещениеЗапасов.ЧерезТранзитныйСклад Тогда
		...
		//	Сообщить(НСТР("ka='აღრიცხვის პოლიტიკაში დოკუმენტის თარიღისთვის არ არის მითითებული ""ტრანზიტული საწყობის ანგარიში""';ru='В учетной политике не указанн ""Счет транзитного склада"" на дату документа.';en='The accounting policies are not an indication of ""transit account warehouse"" at the date of the document.'"));
		//КонецЕсли;
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			//	СтрокаТаблицыРасходДоначисление.СодержаниеПроводки = НСтр("ka='მარაგების რეზერვირება';ru='Резервирование запасов';en='inventory reservations'");
