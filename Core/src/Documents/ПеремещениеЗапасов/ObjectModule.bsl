...
Процедура ЗаполнитьЗапасыПоОстаткамНаСкладе() Экспорт
	...
	|	ЗапасыНаСкладахОстатки.Проект КАК Проект,  				//Малышев 2024-06-23 ONESOFT-3264
	|	ЗапасыНаСкладахОстатки.Проект КАК ПроектОтправитель,  	//Малышев 2024-06-23 ONESOFT-3264
	...
	|		ЗапасыНаСкладах.Проект КАК Проект,   //Малышев 2024-06-23 ONESOFT-3264
	...
	|	ИЗ
	|		РегистрНакопления.ЗапасыНаСкладах.Остатки(&Период
	|				,
	...
	|		ДвиженияДокументаЗапасыНаСкладах.Проект,   //Малышев 2024-06-23 ONESOFT-3264
	...
	|	ЗапасыНаСкладахОстатки.Проект,   //Малышев 2024-06-23 ONESOFT-3264
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//Малышев 2024-06-12 ONESOFT-3094 - закоментируем - если поле не заполнено, то берем счет из номенклатуры
	//+++ Дмгг
	//Если (ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.ПередачаВЭксплуатацию
	// ИЛИ ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы) 
	// И ИспользоватьТолькоРегламентированногоПланаСчетов Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++Mira 19.09.2022 Lazare
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказПокупателя")  Тогда
		УправлениеНебольшойФирмойСервер.ОтразитьЗаказыИПеремешения(ДополнительныеСвойства, Движения, Отказ);
	КонецЕсли;
	//--Mira 19.09.2022 Lazare
	
	Если ДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара
		И ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение или ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы Тогда 
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	//Малышев 2024-06-25 отключил контроль
	...
	// Олег Литвиненко <---
	// Контроль
	//Документы.ПеремещениеЗапасов.ВыполнитьКонтроль(Ссылка, ДополнительныеСвойства, Отказ, Истина, Истина);   
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказ) Экспорт
	...
	|				И ТабличнаяЧастьЗапасы.КоличествоСобр > 0
	|			ТОГДА ТабличнаяЧастьЗапасы.КоличествоСобр
	|		ИНАЧЕ ТабличнаяЧастьЗапасы.Количество
	|	КОНЕЦ КАК Количество,
	|	ТабличнаяЧастьЗапасы.Резерв,
	|	ТабличнаяЧастьЗапасы.Содержание,
	|	ТабличнаяЧастьЗапасы.Сумма КАК СуммаЗаказа,
	|	ТабличнаяЧастьЗапасы.КоличествоСобр
	|ИЗ
	...
	Если ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") //Тогда
		// Kuzmich, 15.10.2024, ONESOFT-3500, Заполнение "заказа покупателя" в шапке документов "перемещение запасов" и "заказ на производство"
		ИЛИ ПолучитьФункциональнуюОпцию("РезервированиеЗапасов_new")
		Тогда
		// Kuzmich, 15.10.2024, ONESOFT-3500, Заполнение "заказа покупателя" в шапке документов "перемещение запасов" и "заказ на производство"
...
Процедура ЗаполнитьПоДокументЗапроса(ДанныеЗаполнения) Экспорт
	...
	ЭтотОбъект.ДокументОснование = ДанныеЗаполнения.Ссылка;
	СтруктурнаяЕдиницаПолучатель = ДанныеЗаполнения.СтруктурнаяЕдиница;
	ПроектОтправитель =   ДанныеЗаполнения.Проект;
	Проект =   ДанныеЗаполнения.Проект;
	//ЯчейкаПолучатель = ДанныеЗаполнения.Ячейка;
	
	Запасы.Очистить();
	Для каждого ТекСтрокаЗапасы Из ДанныеЗаполнения.Запасы Цикл
		Если ТекСтрокаЗапасы.ПередатьВЗаказ Тогда 
			НоваяСтрока = Запасы.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрокаЗапасы);
			НоваяСтрока.Количество = ТекСтрокаЗапасы.Количество;
			
			НоваяСтрока.ПроектОтправитель = ТекСтрокаЗапасы.Проект;
			НоваяСтрока.Проект = ТекСтрокаЗапасы.Проект; //Малыщев 2024-06-24 ONESOFT-3275 
