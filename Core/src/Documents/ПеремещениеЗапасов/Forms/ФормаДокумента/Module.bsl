...
Процедура ЗаполнитьПоОстаткамНаСкладе(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОстаткамНаСкладеЗавершение", ЭтотОбъект), НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура ИзменитьРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='ცხრილი ""მარაგები"" შევსებული არ არის!';ru='Табличная часть ""Запасы"" не заполнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьОтФОИспользоватьПодсистемуПроизводство()
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуПроизводство.Получить()
		...
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение, НСтр("ka='მარაგების გადაადგილება';ru='Перемещение'"));
	...
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы, НСтр("ka='ხარჯებზე ჩამოწერა';ru='Списание на расходы'"));
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.ПередачаВЭксплуатацию, НСтр("ka='ექსპლუატაციაში გადაცემა';ru='Передача в эксплуатацию'"));
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.ВозвратИзЭксплуатации, НСтр("ka='ექსპლუატაციიდან დაბრუნება';ru='Возврат из эксплуатации'"));
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеТаваровНаОС, НСтр("ka='მარაგების ძ/ს–ზე ჩამოწერა';ru='Списание таваров на ОС'"));//+++ Дмгг
...
Процедура ЗаполнитьДоМинимальногоОстатка(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьДоМинимальногоОстатка", ЭтотОбъект), НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура ЗаполнитьДоМинимальногоОстаткаНаСервере()
	...
	Для Каждого СтрокаТЧ ИЗ ТаблицаНеХватаетОстатков Цикл
		Сообщить(НСтр("ka='ნომენკლატურა ""';ru='Номенклатура ""'") + СтрокаТЧ.Номенклатура + НСтр("ka='"" საწყობში ""';ru='"" на складу ""'") + СтрокаТЧ.Отправитель +
				 НСтр("ka='"" თარიღზე ""';ru='"" на дату ""'") + Строка(Объект.Дата) +
				 НСтр("ka='"" არასაკმარისია ლიმიტის შესავსებად. დანაკლისი:';ru='"" не хватает для заполнения лимитов. Не хватает:'") +
				 Строка(СтрокаТЧ.НеХватает) + НСтр("ka=', ლიმიტი:';ru=', Лимит:'") + Строка(СтрокаТЧ.Лимит));
	КонецЦикла;
...
Процедура ЗапасыКоличествоВДорогеПриИзменении(Элемент)
	...
	Если ТекДанные.КоличествоВДороге > ТекДанные.Количество Тогда
		Предупреждение(НСТР("ka='""რაოდენობა გზაში"" არ უნდა იყოს მეტი ""რაოდენობაზე""';ru='""Количество в дороге"" не должно быть больше количества'"), 20);
...
Процедура ДатаПеремещенияИзТранзитаПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.ДатаПеремещенияИзТранзита) И Объект.ДатаПеремещенияИзТранзита <= Объект.Дата Тогда
		Предупреждение(НСТР("ka='ტრანზიტიდან გადაადგილების თარიღი უნდა იყოს დოკუმენტის თარიღზე მეტი.';ru='Дата перемещения из транзита должно быть больше даты документа.'"), 20);
		Объект.ДатаПеремещенияИзТранзита = Объект.Дата + 1;
	КонецЕсли;
КонецПроцедуры
