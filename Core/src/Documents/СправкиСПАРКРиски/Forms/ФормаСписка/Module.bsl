...
"ПриОпределенииСвойствСправочниковКонтрагентов() общего модуля СПАРКРискиПереопределяемый.'"),
		...
		Если ОписаниеСправочника.Иерархический
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='Получение справок 1СПАРК Риски недоступно для группы. Выберите элемент справочника.';ru='Получение справок 1СПАРК Риски недоступно для группы. Выберите элемент справочника.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если Параметры.ПоказыватьОтбор Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='Справки 1СПАРК Риски: %1';ru='Справки 1СПАРК Риски: %1'"),
...
Процедура ЗапроситьСправку(Команда)
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Поле ""Контрагент"" не заполнено.';ru='Поле ""Контрагент"" не заполнено.'"),
			,
			"Контрагент");
		Возврат;
	КонецЕсли;
...
Процедура ВсеПолученныеСправки(Команда)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ka='Справки 1СПАРК Риски';ru='Справки 1СПАРК Риски'"));
...
Процедура ОткрытьСправкуСПАРКРиски(Результат, СправкаСсылка) Экспорт
	...
	Состояние(, , НСтр("ka='Открытие справки 1СПАРК Риски...';ru='Открытие справки 1СПАРК Риски...'"));
...
Процедура ЗапроситьНовуюСправку(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Состояние(, , НСтр("ka='Запрос новой справки 1СПАРК Риски';ru='Запрос новой справки 1СПАРК Риски'"));
	...
	Если Не РезультатЗапросаСправки.СправкаСсылка.Пустая() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='Справки 1СПАРК Риски';ru='Справки 1СПАРК Риски'"),
			,
			НСтр("ka='Подготовка новой справки 1СПАРК Риски.';ru='Подготовка новой справки 1СПАРК Риски.'"),
...
"Повторите попытку подключения позже.'"));
	...
	ИначеЕсли РезультатДействия.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено") Тогда
		...
		ПоказатьПредупреждение(
			,
			НСтр("ka='Использование сервиса 1СПАРК Риски недоступно в текущем режиме работы.';ru='Использование сервиса 1СПАРК Риски недоступно в текущем режиме работы.'"));
...
Функция ЗапроситьНовуюСправкуВСервисеСПАРКРиски()
	...
	Если Не СПАРКРиски.ИспользованиеРазрешено() Тогда
		...
		ВызватьИсключение НСтр("ka='Использование недоступно в текущем режиме работы.';ru='Использование недоступно в текущем режиме работы.'");
	КонецЕсли;
	...
	Если Не ИнтернетПоддержкаПользователей.УслугаПодключена(ИдентификаторУслуги) Тогда
		...
		РезультатВызоваОперации.Вставить("СообщениеОбОшибке",
			НСтр("ka='Услуга не подключена.';ru='Услуга не подключена.'"));
...
"ПриОпределенииСвойствСправочниковКонтрагентов() общего модуля СПАРКРискиПереопределяемый.'"),
	...
	Если ЗначенияРеквизитов.Количество() = 0 Тогда
		...
		Результат.Сообщение = НСтр("ka='Не удалось получить данные контрагента. Данные контрагента были удалены или недостаточно прав.';ru='Не удалось получить данные контрагента. Данные контрагента были удалены или недостаточно прав.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Не ДанныеКонтрагента.ПодлежитПроверке Тогда
		...
		Результат.Сообщение = НСтр("ka='Выбранный контрагент не может быть проверен в сервисе 1СПАРК Риски.';ru='Выбранный контрагент не может быть проверен в сервисе 1СПАРК Риски.'");
		Возврат Результат;
...
Процедура ЗагрузитьФайлСправки(ДокументСсылка, ИдентификаторСправки, ИНН, ФайлСсылка, Результат)
	...
	ПараметрыФайла.Вставить("ИмяБезРасширения",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='Справка 1СПАРК Риски %1';ru='Справка 1СПАРК Риски %1'"),
			ИНН));
