...
Процедура ПриИзмененииВидаВыписки(Форма)
	...
	Если Форма.ВидВыписки = "себе" Тогда
		...
		Если ЗначениеЗаполнено(Объект.Организация) Тогда
			Подсказка = НСтр("ka='%1 (ИНН %2)';ru='%1 (ИНН %2)'");
...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Если ЗначениеЗаполнено(СтрокаПодсказки) Тогда
		Заголовок = НСтр("ka='Запрос на выписку из ЕГРЮЛ/ЕГРИП по %1';ru='Запрос на выписку из ЕГРЮЛ/ЕГРИП по %1'");
	Иначе
		Заголовок = НСтр("ka='Запрос на выписку из ЕГРЮЛ/ЕГРИП';ru='Запрос на выписку из ЕГРЮЛ/ЕГРИП'");
	КонецЕсли;
...
Процедура УправлениеЭУОтправка()
	...
	Если ЗаявлениеОтправлено Тогда
		...
		Элементы.ЗаголовокПараметрЗапроса.Заголовок  		= НСтр("ka='Выписка по:';ru='Выписка по:'");
...
Процедура ПрисвоитьЗначениеПараметруЗапроса(РезультатВыбора, Параметры) Экспорт
	...
	Если ПустаяСтрока(ИННКонтрагента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='У контрагента не заполнен ИНН.';ru='У контрагента не заполнен ИНН.'"));
		Объект.Контрагент = Неопределено;
