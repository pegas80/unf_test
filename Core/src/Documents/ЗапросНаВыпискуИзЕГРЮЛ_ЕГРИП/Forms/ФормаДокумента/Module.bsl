...
Процедура ПриИзмененииВидаВыписки(Форма)
	...
	Если Форма.ВидВыписки = "себе" Тогда
		...
		Если ЗначениеЗаполнено(Объект.Организация) Тогда
			Подсказка = НСтр("ka='%1 (ИНН %2)';ru='%1 (ИНН %2)';en='1% (2% INN)'");
...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Если ЗначениеЗаполнено(СтрокаПодсказки) Тогда
		Заголовок = НСтр("ka='Запрос на выписку из ЕГРЮЛ/ЕГРИП по %1';ru='Запрос на выписку из ЕГРЮЛ/ЕГРИП по %1';en='The request for an extract from the register / EGRIP by 1%'");
	Иначе
		Заголовок = НСтр("ka='Запрос на выписку из ЕГРЮЛ/ЕГРИП';ru='Запрос на выписку из ЕГРЮЛ/ЕГРИП';en='Request for Certificate of Incorporation / EGRIP'");
	КонецЕсли;
...
Процедура УправлениеЭУОтправка()
	...
	Если ЗаявлениеОтправлено Тогда
		...
		Элементы.ЗаголовокПараметрЗапроса.Заголовок  		= НСтр("ka='Выписка по:';ru='Выписка по:';en='Statement by:'");
...
Процедура ПрисвоитьЗначениеПараметруЗапроса(РезультатВыбора, Параметры) Экспорт
	...
	Если ПустаяСтрока(ИННКонтрагента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='У контрагента не заполнен ИНН.';ru='У контрагента не заполнен ИНН.';en='The counterparty is empty TIN.'"));
		Объект.Контрагент = Неопределено;
