...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ПоследнийДокументИсправления) Тогда
		ТекстВопроса = НСтр("ka='აღნიშნული დოკუმენტისთვის არსებობს უფრო გვიანი კორექტირებები. გამოვიყენოთ ბოლოს შეყვანილი კორექტირება?';ru='Для указанного документа существуют более поздние корректировки."
...
Процедура ЗапасыПередУдалением(Элемент, Отказ)
	...
	Если СтрокаТаблицы.ЕстьВДокументеПоступления И Объект.ОтражатьВУчете Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='საწყისი დოკუმენტის სტრიქონის წაშლის ნაცვლად, წაშალეთ თანხობრივი და რაოდენობრივი მაჩვენებლები.';ru='Вместо удаления строки исходного документа очистите суммовые и количественные показатели.';en='Instead of deleting a line of the original document, clean the accrual and quantitative indicators.'");
...
Процедура ПредоплатаПередУдалением(Элемент, Отказ)
	...
	Если СтрокаТаблицы.ЕстьВДокументеПоступления И Объект.ОтражатьВУчете Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='საწყისი დოკუმენტის სტრიქონის წაშლის ნაცვლად, გაასუფთავეთ ჯამი.';ru='Вместо удаления строки исходного документа очистите сумму.';en='Instead of deleting the original document line worth clean.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПоказатьПредупреждение(, НСтр("ka='თავდაპირველად, მიუთითეთ კორექტირებადი დოკუმენტი.';ru='Укажите вначале корректируемый документ.';en='Specify first corrected document.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованиюКлиент()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		Если Объект.Запасы.Количество() > 0 ИЛИ Объект.Запасы.Количество() > 0 Тогда 
			ТекстВопроса = НСтр("ka='დოკუმენტი სრულად შეივსება ""საფუძვლის"" მიხედვით! გავაგრძლოთ ოპერაციის შესრულება?';ru='Документ будет полностью заполнен по ""Основанию""! Продолжить выполнение операции?';en='The document will be completely filled by ""base""! To continue the operation?'");
			Оповещение = Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект);
			ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ka='დოკუმენტის შევსება';ru='Заполнить документ';en='Complete the document'"));
		Иначе
...
Процедура ЗаполнитьЗаголовкиСтрок(СтрокаТаблицы)
	...
	СтрокаТаблицы.НадписьДоКорректировки =  НСтр("ka='კორექტირებამდე:';ru='до корректировки:';en='before adjustments:'");
	СтрокаТаблицы.НадписьДоИзменения     =  НСтр("ka='შეცვლამდე:';ru='до изменения:';en='before the change:'");
	СтрокаТаблицы.НадписьПослеИзменения  =  НСтр("ka='შეცვლის შემდეგ:';ru='после изменения:';en='after the change:'");
