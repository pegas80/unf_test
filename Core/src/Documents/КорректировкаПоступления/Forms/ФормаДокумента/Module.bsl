...
Процедура ЗапасыПередУдалением(Элемент, Отказ)
	...
	Если СтрокаТаблицы.ЕстьВДокументеПоступления И Объект.ОтражатьВУчете Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='საწყისი დოკუმენტის სტრიქონის წაშლის ნაცვლად, წაშალეთ თანხობრივი და რაოდენობრივი მაჩვენებლები.';ru='Вместо удаления строки исходного документа очистите суммовые и количественные показатели.'");
...
Процедура ПредоплатаПередУдалением(Элемент, Отказ)
	...
	Если СтрокаТаблицы.ЕстьВДокументеПоступления И Объект.ОтражатьВУчете Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ka='საწყისი დოკუმენტის სტრიქონის წაშლის ნაცვლად, გაასუფთავეთ ჯამი.';ru='Вместо удаления строки исходного документа очистите сумму.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПоказатьПредупреждение(, НСтр("ka='თავდაპირველად, მიუთითეთ კორექტირებადი დოკუმენტი.';ru='Укажите вначале корректируемый документ.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованиюКлиент()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		Если Объект.Запасы.Количество() > 0 ИЛИ Объект.Запасы.Количество() > 0 Тогда 
			ТекстВопроса = НСтр("ka='დოკუმენტი სრულად შეივსება ""საფუძვლის"" მიხედვით! გავაგრძლოთ ოპერაციის შესრულება?';ru='Документ будет полностью заполнен по ""Основанию""! Продолжить выполнение операции?'");
			Оповещение = Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект);
			ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ka='დოკუმენტის შევსება';ru='Заполнить документ'"));
		Иначе
...
Процедура ЗаполнитьЗаголовкиСтрок(СтрокаТаблицы)
	...
	СтрокаТаблицы.НадписьДоКорректировки =  НСтр("ka='კორექტირებამდე:';ru='до корректировки:'");
	СтрокаТаблицы.НадписьДоИзменения     =  НСтр("ka='შეცვლამდე:';ru='до изменения:'");
	СтрокаТаблицы.НадписьПослеИзменения  =  НСтр("ka='შეცვლის შემდეგ:';ru='после изменения:'");
