...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	// Kuzmich, 19.12.2024, (ONESOFT-3719) Заполнение "Вид налога НДС" в документе "корректировка посступления"
	Если (ТипЗНЧ(ДанныеЗаполнения) = Тип("Структура") ИЛИ ТипЗНЧ(ДанныеЗаполнения) = Тип("ФиксированнаяСтруктура")) И ДанныеЗаполнения.Свойство("ВидНалогаНДС") Тогда
	
		ЭтотОбъект.ВидНалогаНДС = ДанныеЗаполнения.ВидНалогаНДС;
	
	ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипЗНЧ(ДанныеЗаполнения)) И ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидНалогаНДС", ДанныеЗаполнения.Метаданные()) Тогда
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимНаправлениямДеятельности.Получить() Тогда
		...
		Если НЕ ВключатьРасходыВСебестоимость Тогда
			...
			Для каждого СтрокаРасходы из Расходы Цикл
				...
				//Если СтрокаРасходы.Номенклатура.СчетУчетаЗатрат.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда //+++ Дмгг
				Если НЕ ИспользоватьТолькоРегламентированныйПланСчетов 
					И СтрокаРасходы.Номенклатура.СчетУчетаЗатрат.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда //+++ Дмгг
					
					СтрокаРасходы.НаправлениеДеятельности = Справочники.НаправленияДеятельности.ОсновноеНаправление;
					
				ИначеЕсли НЕ ИспользоватьТолькоРегламентированныйПланСчетов 
						И СтрокаРасходы.Номенклатура.СчетУчетаЗатратРегл.ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда //+++ Дмгг
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	УправлениеНебольшойФирмойСервер.ОтразитьАнализПоПодразделениям(ДополнительныеСвойства, Движения, Отказ); 
...
Процедура ЗаполнитьПоПоступлению(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная")
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დოკუმენტ %1-ის საფუძველზე შემოსვლის კორექტირების შეყვანა საჭირო არ არის.';ru='Не требуется вводить корректировку поступления на основании документа %1.';en='It does not require a correction based on the receipt document 1%.'"),
			ДанныеЗаполнения);
	...
	|	ПриходнаяНакладнаяЗапасы.Проект, //Малышев 2024-08-24 ONESOFT-3414
	...
	|	ПриходнаяНакладнаяЗапасы.СтавкаНДС как СтавкаНДСДоКорректировки, //Малышев 2025-11-10 ONESOFT-4489
	...
	|	ПриходнаяНакладнаяРасходы.Проект, //Малышев 2024-08-24 ONESOFT-3414
	...
	|	ПриходнаяНакладнаяРасходы.СтавкаНДС как СтавкаНДСДоКорректировки, //Малышев 2025-11-10 ONESOFT-4489
...
Процедура ЗаполнитьПоДопРасходам(ДанныеЗаполнения) Экспорт
	...
	|	ДополнительныеРасходыЗапасы.Проект, //Малышев 2024-08-24
	...
	|	ДополнительныеРасходыРасходы.Проект,  //Малышев 2024-08-24
