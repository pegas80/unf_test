...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСписаниеВАНаТовары, СтруктураДополнительныеСвойства) Экспорт
	...
	//Малышев 2024-06-28 ONESOFT-3288 (--
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаСписаниеВАНаТовары, СтруктураДополнительныеСвойства)
	...
	|	NULL КАК Проект,  //Малышев 2024-06-28 ONESOFT-3288
	|	0 КАК Количество
	|ИЗ
	...
	|	ТаблицаДокумента.ДокументПоступления, //Малышев 2024-11-30 ONESOFT-3645
	|	ТаблицаДокумента.Номенклатура,
	|	ТаблицаДокумента.Характеристика,
	|	ТаблицаДокумента.Проект,  //Малышев 2024-06-28 ONESOFT-3288
	|	ТаблицаДокумента.Количество
	|ИЗ
	|	ВременнаяТаблицаВнеоборотныеАктивы КАК ТаблицаДокумента
	|ГДЕ
...
Процедура СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаСписаниеВАНаТовары, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", НСтр("ka='აქციების კაპიტალიზაცია';ru='Оприходование запасов';en='posting inventory'"));
	Запрос.УстановитьПараметр("ПрочиеРасходы", НСтр("ka='სხვა ხარჯები';ru='Прочие расходы';en='other expenses'"));
	...
	|	ТаблицаДокумента.Проект КАК Проект, //Малышев 2024-06-28 ONESOFT-3288
