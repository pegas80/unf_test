...
Процедура РаспределитьПоКоличествуНаСервере()
	...
	Документ.РаспределитьПоКоличеству();
...
&НаСервере
Процедура РаспределитьПоКоэффициентуНаСервере()
	
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.РаспределитьПоКоэффициенту();
	ЗначениеВРеквизитФормы(Документ, "Объект");
	
КонецПроцедуры

&НаКлиенте
Процедура РаспределитьПоКоэффициенту(Команда)
	РаспределитьПоКоэффициентуНаСервере();
КонецПроцедуры
#КонецОбласти
...
Процедура РассчитатьАмортизацию(АдресВнеоборотныеАктивыВХранилище, Дата, Организация)
	...
	|	ТаблицаВнеоборотныеАктивы.Номенклатура КАК Номенклатура,
	|	ТаблицаВнеоборотныеАктивы.Характеристика КАК Характеристика,
	|	ТаблицаВнеоборотныеАктивы.Количество КАК Количество,
	|	ТаблицаВнеоборотныеАктивы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ТаблицаВнеоборотныеАктивы.Себестоимость КАК Себестоимость,
	|	ТаблицаВнеоборотныеАктивы.Коэффициент КАК Коэффициент,
	|	ТаблицаВнеоборотныеАктивы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница
	...
	|	ТаблицаВнеоборотныеАктивы.Номенклатура,
	|	ТаблицаВнеоборотныеАктивы.Характеристика,
	|	ТаблицаВнеоборотныеАктивы.Количество,
	|	ТаблицаВнеоборотныеАктивы.ЕдиницаИзмерения,
	|	ТаблицаВнеоборотныеАктивы.Себестоимость,
	|	ТаблицаВнеоборотныеАктивы.СтруктурнаяЕдиница,
	|	ТаблицаВнеоборотныеАктивы.Коэффициент
	|ИЗ
	|	ТаблицаВнеоборотныеАктивы КАК ТаблицаВнеоборотныеАктивы
...
Процедура ВнеоборотныеАктивыВнеоборотныйАктивПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.ВнеоборотныеАктивы.ТекущиеДанные;
	ТекущаяСтрока.Коэффициент = 1;
	
	ТекущаяСтрока.Стоимость = ПолучитьПервоначальнуюСтоимостьВА(ТекущаяСтрока.ВнеоборотныйАктив)
	
КонецПроцедуры

&НаСервере
Функция ПолучитьПервоначальнуюСтоимостьВА(ВА)
	Возврат ВА.НачальнаяСтоимость;
КонецФункции // ПолучитьПервоначальнуюСтоимостьВА()


&НаКлиенте
Процедура ВнеоборотныеАктивыНоменклатураПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.ВнеоборотныеАктивы.ТекущиеДанные;
	УидСтроки = СтрокаТабличнойЧасти.ПолучитьИдентификатор();
	ВнеоборотныеАктивыНоменклатураПриИзмененииНаСервере(УидСтроки);
	
КонецПроцедуры

&НаСервере
Процедура ВнеоборотныеАктивыНоменклатураПриИзмененииНаСервере(УидСтроки)

	СтрокаТабличнойЧасти = Объект.ВнеоборотныеАктивы.НайтиПоИдентификатору(УидСтроки);
	СтрокаТабличнойЧасти.ЕдиницаИзмерения = СтрокаТабличнойЧасти.Номенклатура.ЕдиницаИзмерения;
	
КонецПроцедуры

&НаКлиенте
Процедура ВнеоборотныеАктивыНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
