...
Процедура ЗаполнитьТабличнуюЧастьЗапасыПоИнвентаризацииТоваровНаСкладе(ДанныеЗаполнения)
	...
	Если Запасы.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ka='ინვენტარიზაციის მიხედვით შესავსებად მონაცემები არ არის!';ru='Нет данных для заполнения по инвентаризации!'");
		Сообщение.Сообщить();
...
Процедура ЗаполнитьПоИнвентаризацииЗапасовНаСкладе(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы <> Перечисления.ТипыСтруктурныхЕдиниц.Розница Тогда
		ВызватьИсключение(НСтр("ka='დოკუმენტი შეიძლება შეივსოს მხოლოდ საცალო საწყობის მიხედვით ინვენტარიზაციის საფუძველზე!';ru='Документ может быть заполнен только на основании инвентаризации по розничному складу!'"));
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОплатаПлатежнымиКартами.Итог("Сумма") > СуммаДокумента Тогда
		...
		ТекстОшибки = НСтр("ka='გადახდის ბარათებით გადახდის თანხა აღემატება დოკუმენტის თანხას';ru='Сумма оплаты платежными картами превышает сумму документа'");
	...
	Если ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор
		...
		Если ОткрытаяКассоваяСмена <> Неопределено
		   И ОткрытаяКассоваяСмена <> Ссылка Тогда
			...
			ТекстОшибки = НСтр("ka='მოცემული სალაროთი თარიღზე %Дата% უკვე დარეგისტრირებულია %КассоваяСмена%';ru='По данной кассе на дату %Дата% уже зарегистрирован %КассоваяСмена%'");
		...
		Если НЕ ЗначениеЗаполнено(ОкончаниеКассовойСмены)
			...
			ТекстОшибки = НСтр("ka='ველი ""ცვლის დასრულება"" შეუვსებელია';ru='Поле ""Окончание смены"" не заполнено'");
		...
		Если ЗначениеЗаполнено(ОкончаниеКассовойСмены)
			...
			ТекстОшибки = НСтр("ka='სალაროს ცვლის დაწყების დრო მეტია სალაროს ცვლის დასრულების დროზე';ru='Время начала кассовой смены больше времени окончания кассовой смены'");
		...
		Если ЗначениеЗаполнено(СтатусКассовойСмены)
			...
			ТекстОшибки = НСтр("ka='სალაროს ცვლის დაწყების დრო განსხვავდება დოკუმენტის თარიღისგან';ru='Время начала кассовой смены отличается от даты документа'");
		...
		Если ЗначениеЗаполнено(СтатусКассовойСмены)
			...
			ТекстОшибки = НСтр("ka='სალაროს ცვლის დასრულების დრო განსხვავდება დოკუმენტის თარიღისგან';ru='Время окончания кассовой смены отличается от даты документа'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыВРозничнойТорговле") И ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") Тогда
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			Если СтрокаЗапасы.Количество = 0 И СтрокаЗапасы.Резерв = 0 Тогда
				ТекстСообщения = НСтр("ka='სია ""მარაგები""-ს სტრიქონ %Номер%-ში სვეტი ""რაოდენობა"" არ არის შევსებული.';ru='Не заполнена колонка ""Количество"" в строке %Номер% списка ""Запасы"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
	...
	Иначе
		Для каждого СтрокаЗапасы Из Запасы Цикл
			Если СтрокаЗапасы.Количество <= 0 Тогда
				ТекстСообщения = НСтр("ka='სია ""მარაგები""-ს სტრიქონ %Номер%-ში სვეტი ""რაოდენობა"" არასწორად არის შევსებული.';ru='Не верно заполнена колонка ""Количество"" в строке %Номер% списка ""Запасы"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
