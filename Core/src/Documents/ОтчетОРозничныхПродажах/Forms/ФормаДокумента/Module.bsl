...
//ПМ	Если так не сделать, то по кнопке проведения не попадает в процедуру ПриЗаписи в модуле объекта
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// Конец Обсуждения
//ПМ
  Если ИмяСобытия="ПМ_ЗаполнениеДокумента" Тогда
	  ПМ_ЗаполнитьДокументНаКлиенте(Параметр);
	КонецЕсли; 
...
Процедура ЗаполнитьПоЗаказуПокупателяНаСервере(Заказ)
	...
	Если НЕ (ВидОперацииЗаказаПокупателя = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу) Тогда
		ВызватьИсключение НСтр("ka='შესავსებად საჭიროა მყიდველის შეკვეთის არჩევა (გასაყიდად)!';ru='Для заполнения нужно выбрать заказа покупателя (на продажу)!';en='To fill the need to choose the order of the buyer (for sale)!'");
	КонецЕсли;
	...
	Если Объект.СуммаВключаетНДС <> Заказ.СуммаВключаетНДС Тогда
		Если Объект.СуммаВключаетНДС Тогда
			СтрокаПроНДС = НСтр("ka=' В заказе сумма не включает НДС.';ru=' В заказе сумма не включает НДС.'");
		Иначе
			СтрокаПроНДС = НСтр("ka=' В заказе сумма включает НДС.';ru=' В заказе сумма включает НДС.'");
		КонецЕсли;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შეკვეთაში დღგ-ის აღრიცხვის მეთოდი განსხვავდება მიმდინარე დოკუმენტისგან!';ru='Способ учета НДС в заказе отличен от текущего документа!';en='VAT accounting method in the order different from the current document!'") + СтрокаПроНДС;
		Сообщение.Сообщить();
	КонецЕсли;
...
#Область ПМ_ЗаполнениеДокумента
	
&НаКлиенте
Процедура ПМ_ЗаполнитьДаннымиИзОбмена(Команда)
	Если Не ЗначениеЗаполнено(Объект.КассаККМ) Тогда
	  ПоказатьПредупреждение(,"Не указана касса ККМ");
		Возврат;	
	КонецЕсли; 
	Если Не ЗначениеЗаполнено(Объект.Дата) Тогда
	  ПоказатьПредупреждение(,"Не заполнена дата документа");
		Возврат;	
	КонецЕсли;
	Если Объект.Запасы.Количество()>0 Тогда
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ПМ_ПослеВопросаЗаполнитьДокумент", ЭтотОбъект);
		ТекстВопроса = НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена. Продолжить?';en='Tabular section is cleared. Proceed?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ПМ_ПослеВопросаЗаполнитьДокумент(КодВозвратаДиалога.Да, Неопределено);
	КонецЕсли; 
КонецПроцедуры

&НаКлиенте
Процедура ПМ_ПослеВопросаЗаполнитьДокумент(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ПараметрыФормы=Новый Структура("Организация,КассаККМ,Дата, Адрес",Объект.Организация,Объект.КассаККМ,Объект.Дата, ЭтаФорма.УникальныйИдентификатор);
	Оповещение=Новый ОписаниеОповещения("ПМ_ПослеЗаполненияДанных",ЭтаФорма,ДопПараметры);
	ОткрытьФОрму("Обработка.ПМ_ЗаполнениеДокумента_ОтчетОРозничныхПродажах.Форма",ПараметрыФормы,ЭтаФорма,ЭтаФорма.УникальныйИдентификатор,,,Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура ПМ_ПослеЗаполненияДанных(Результат,ДопПараметры) Экспорт
...
&НаКлиенте
Процедура ПМ_ЗаполнитьДокументНаКлиенте(Результат)
	ПМ_ЗаполнитьДокумент(Результат);
	Для каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
		РассчитатьСуммуНДС(СтрокаТабличнойЧасти);
		СтрокаТабличнойЧасти.Всего = СтрокаТабличнойЧасти.Сумма + ?(Объект.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС);
	КонецЦикла;
	ПересчитатьДокументНаКлиенте();
КонецПроцедуры // ПМ_ЗаполнитьДокументНаКлиенте()
 
&НаСервере
Процедура ПМ_ЗаполнитьДокумент(Результат)
	Для каждого стр Из Результат.Сообщения Цикл
	  Сообщить(стр.Значение);
	КонецЦикла;
	Структ=ПолучитьИзВременногоХранилища(Результат.АдресВоВременномХранилище);
	Объект.Запасы.Очистить();
	Объект.ОплатаПлатежнымиКартами.Очистить();
	фл=Ложь;
	Для каждого стр Из Структ.Продажи Цикл
	...
	Для каждого стр Из Структ.Оплата Цикл
	  СтрокаТабличнойЧасти=Объект.ОплатаПлатежнымиКартами.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти,стр);
		фл=Истина;
	КонецЦикла; 
	
	Модифицированность=Модифицированность ИЛИ фл;
КонецПроцедуры // ПМ_ЗаполнитьДокумент()
#КонецОбласти 
// Конец СтандартныеПодсистемы.Печать
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не Объект.Проведен Тогда 
		Ответ = Вопрос("დოკუმენტი არ არის გატარებილი, დაიხუროს დოკუმენტი?",РежимДиалогаВопрос.ДаНет,0,КодВозвратаДиалога.Нет) ;
