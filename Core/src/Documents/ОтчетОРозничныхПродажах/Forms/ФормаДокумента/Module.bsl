...
Процедура ЗаполнитьПоЗаказуПокупателяНаСервере(Заказ)
	...
	Если НЕ (ВидОперацииЗаказаПокупателя = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу) Тогда
		ВызватьИсключение НСтр("ka='შესავსებად საჭიროა მყიდველის შეკვეთის არჩევა (გასაყიდად)!';ru='Для заполнения нужно выбрать заказа покупателя (на продажу)!'");
	КонецЕсли;
	...
	Если Объект.СуммаВключаетНДС <> Заказ.СуммаВключаетНДС Тогда
		...
		КонецЕсли;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შეკვეთაში დღგ-ის აღრიცხვის მეთოდი განსხვავდება მიმდინარე დოკუმენტისგან!';ru='Способ учета НДС в заказе отличен от текущего документа!'") + СтрокаПроНДС;
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ПМ_ЗаполнитьДаннымиИзОбмена(Команда)
	...
	Если Объект.Запасы.Количество()>0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть будет очищена. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
