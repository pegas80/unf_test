...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПриемЗапасовТоварыНаКомиссии", НСтр("ka='საკომისიო საქონლის რელიზაცია';ru='Реализация комиссионных товаров'"));
...
Процедура СформироватьТаблицаДенежныеСредстваВКассахККМ(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОплатаПлатежнымиКартами", НСтр("ka='გადახდის ბარათებით გადახდა';ru='Оплата платежными картами'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОтражениеОплатыПлатежнымиКартами", НСтр("ka='გადახდის ბარათებით გადახდა';ru='Оплата платежными картами'"));
...
Функция СформироватьПечатнуюФормуКМ6(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ОтчетОРозничныхПродажах Из МассивОбъектов Цикл
		...
		ДанныеПечати.Вставить("ПрограммаУчета", НСтр("ka='1С:საწარმო 8';ru='1С:Предприятие 8'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипКассы = Перечисления.ТипыКассККМ.АвтономнаяККМ Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დოკუმენტ %1-ისთვის სალაროს ტიპით ""ავტონომიური სსა"" გამოუყენებელი ჩეკების (КМ-3) მიხედვით ფულადი თანხების დაბრუნებაზე აქტის ბეჭდვა საჭირო არ არის.';ru='Для документа %1 с типом кассы ""Автономная ККМ"" печать Акта о возврате денежных сумм по неиспользованным чекам (КМ-3) не требуется.'"),
				Выборка.Ссылка
			);
		...
		Если ВыборкаПоЧекамИтоги.НайтиСледующий(Новый Структура("ОтчетОРозничныхПродажах", Выборка.Ссылка)) Тогда
			...
			ОбластьШапки.Параметры.ВидДеятельностиПоОКДП	= "";
			ОбластьШапки.Параметры.ПрограммаУчета 			= НСтр("ka='1С:საწარმო 8';ru='1С:Предприятие 8'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОРозничныхПродажах") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ka='საცალო გაყიდვების ანგარიშგება';ru='Отчет о розничных продажах'"),
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='საცალო გაყიდვების ანგარიშგება';ru='Отчет о розничных продажах'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "КМ6";
	КомандаПечати.Представление = НСтр("ka='КМ6 (ცვლის ანგარიშგება)';ru='КМ6 (Отчет за смену)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 4;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "КМ3";
	КомандаПечати.Представление = НСтр("ka='КМ3 (აქტი ფულადი სახსრების დაბრუნების თაობაზე)';ru='КМ3 (Акт о возврате денежных сумм)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Функция ОтчетОРозничныхПродажахXML2_40(ДокументСсылка) Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		НоваяСтрока.doc_name   = НСтр("ka='საცალო გაყიდვების ანგარიშგება';ru='Отчет о розничных продажах'");
