...
Процедура Рассчитать(Команда)
	...
	Если Объект.Проведен Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='ამორტიზაციის გაანგარიშება შესაძლებელია მხოლოდ გაუტარებელ დოკუმენტში';ru='Рассчет амортизации возможен только в непроведенном документе.'"));
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("РассчитатьЗавершение", ЭтотОбъект), НСтр("ka='შეტანილი მონაცემები გადაითვლება! გავაგრძელოთ?';ru='Введенные данные будут пересчитаны! Продолжить?'"),РежимДиалогаВопрос.ДаНет);
...
Процедура УстановитьЗаголовки()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийКомплектацииВА.Комплектация Тогда
		...
		Элементы.ГруппаКомплектуемоеВА.Заголовок = НСтр("ka='კომპლექტი';ru='Комплектуемое имущество'");
		
		Элементы.ГруппаИмуществоИнвНомер.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийКомплектацииВА.Разукомплектация Тогда
		
		Элементы.ГруппаКомплектуемоеВА.Заголовок = НСтр("ka='კომპლექტი';ru='Комплект'");
...
Процедура ДоляСтоимостиПриИзменении(Элемент)
	...
	Если Объект.ДоляСтоимости >= 1 Тогда 
		Сообщить(НСтр("ka='განაწილების კოეფიციენტის ჯამი არ შეიძლება იყოს 1-ზე მეტი';ru='Доля распределения не может быть равна больше ""1""'"));
		Возврат;
	КонецЕсли;
