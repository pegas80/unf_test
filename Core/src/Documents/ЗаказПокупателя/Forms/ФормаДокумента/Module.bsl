...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.ВариантЗавершенияРасширеннаяПодсказка.Заголовок = СтрШаблон(НСтр("ka='Предыдущее состояние: %1';ru='Предыдущее состояние: %1'"),
...
Процедура ЗапланироватьОплатуПриИзменении(Элемент)
	...
	Если Объект.ЗапланироватьОплату И ОжидаетсяВыборВариантаКП() Тогда
		ПоказатьПредупреждение(, НСтр("ka='Не выбран основной вариант коммерческого предложения.';ru='Не выбран основной вариант коммерческого предложения.'"));
...
Процедура РедактироватьСписком(Команда)
	...
	Если Элементы.РедактироватьСписком.Пометка И Объект.ПлатежныйКалендарь.Количество() > 1 Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ka='პირველის გარდა ყველა სტრიქონი წაიშლება. გავაგრძელოთ?';ru='Все строки кроме первой будут удалены. Продолжить?'"),
			РежимДиалогаВопрос.ДаНет
		);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьРезервЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ცხრილი ""საქონელი, მომსახურება"" არ არის შევსებული!';ru='Табличная часть ""Товары, услуги"" не заполнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура СгруппироватьПозиции(Команда)
	...
	Строка.Номенклатура = СтрШаблон(НСтр("ka='Группа %1';ru='Группа %1'"), Строки.Количество() + 1);
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") 
		...
		ИначеЕсли ЗначениеЗаполнено(РезультатЗакрытия.ДисконтнаяКарта) И ЗначениеЗаполнено(РезультатЗакрытия.Контрагент) И Объект.Контрагент.Пустая() Тогда
			...
			ПоказатьОповещениеПользователя(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტში შევსებულია კონტრაგენტი და წაკითხულია დისკონტური ბარათი %1';ru='В документе заполнен контрагент и считана дисконтная карта %1'"), Объект.ДисконтнаяКарта),
				БиблиотекаКартинок.Информация32);
		Иначе
			Объект.ДисконтнаяКарта = РезультатЗакрытия.ДисконтнаяКарта;
			Объект.ПроцентСкидкиПоДисконтнойКарте = РезультатЗакрытия.ПроцентСкидкиПоДисконтнойКарте;
...
Процедура ЗаполнитьВариантыКП(Операция = Неопределено, УстановитьОтбор = Истина)
	...
	Если ЕстьНесколькоВариантовКП Тогда
		...
		КонецЕсли;
	Иначе
		ИмяВарианта = НСтр("ka='ძირითადი ვარიანტი';ru='Основной вариант'");
...
Функция СписокВыбораСлужбыДоставки(Текст, СпособДоставки)
	...
	Если ПустаяСтрока(Текст) ИЛИ Найти(НСтр("ru='яндекс.доставка'"), НРег(Текст))>0 Тогда
		...
		ЭлементЯндексДоставка.Представление = НСтр("ka='Яндекс.Доставка';ru='Яндекс.Доставка'");
...
Процедура ДополнительнаяИнформацияПоДоставкеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НСтр("ka='Комментарий для службы доставки';ru='Комментарий для службы доставки'"));
...
Процедура НастроитьЭлементыПоСпособуДоставки(Форма)
	...
	Если СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.Самовывоз") 
		...
		Элементы.АдресСамовывоза.Заголовок = НСтр("ka='გამოძახების მისამართი';ru='Адрес самовывоза'");
	...
	ИначеЕсли СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.Курьер") Тогда
		...
		Элементы.АдресСамовывоза.Заголовок = НСтр("ka='გამოძახების მისამართი';ru='Адрес самовывоза'");
		Элементы.АдресДоставкиПолучателя.Видимость = Истина;
		Элементы.АдресДоставкиПолучателя.Заголовок = НСтр("ka='ტრანსპორტირების დასრულების ადგილი';ru='Адрес доставки'");
		Элементы.ДополнительнаяИнформацияПоДоставке.Видимость = Истина;
		Элементы.ПунктВыдачиЗаказа.Видимость = Ложь;
		Элементы.ТрекНомер.Доступность = Истина;
...
Процедура ПередатьВЯндексДоставку(Команда)
	...
	Если Модифицированность Или Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		КнопкиДиалога.Добавить(КодВозвратаДиалога.ОК, НСтр("ka='ჩაწერა';ru='Записать'"));
...
Процедура ПередатьЗаказВЯндексДоставку()
	...
	Если Объект.СостояниеЗаказа = ПредопределенноеЗначение("Справочник.СостоянияЗаказовПокупателей.Завершен") Тогда
		ПоказатьПредупреждение(, НСтр("ka='Завершенный заказ не может быть передан в Яндекс.Доставку.';ru='Завершенный заказ не может быть передан в Яндекс.Доставку.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ka='მონაცემების ჩატვირთვა';ru='Загрузка данных'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ЗаказПокупателя, , ОписаниеОшибки());
		ОтменитьТранзакцию();
...
Процедура Фкс_Резервировать(Команда)
	...
	Если Модифицированность Тогда
		ТекстСообщения = Нстр("ka='დოკუმენტი შეცვლილია! დასარეზერვებლად აუცილებელია დოკუმენტის გადატარება!';ru='Вначале нужно записать документ!'");
...
Процедура Фкс_РезервироватьНаСервере(СписокНоменклатур,Склад)
	...
	Если Не (ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаПродажи) И ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаРезерв)) Тогда
		ТекстСообщения = НСтр("ka='ქვეგანყოფილება და საწყობი შევსებული უნდა იყოს!';ru='Подразделение и склад должны быть заполнены!'");
	...
	Если докРезерв.Запасы.Количество() = 0 Тогда
		ТекстСообщения = Нстр("ka='დასარეზერვებელი არაფერია!';ru='Резервировать нечего!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ОтменитьТранзакцию();
		Возврат;
	КонецЕсли;
...
Процедура СнатьРезервы()
	...
	Пока Выборка2.Следующий() Цикл
		...
		//	ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокПеремещения + НСтр("ka='"" მოიხსნა რეზერვიდან.';ru='"" снята с резерва.'");
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		//Исключение
		//	ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокПеремещения + НСтр("ka='"" ვერ მოიხსნა რეზერვიდან.';ru='"" не удалось снять с резерва.'");
		...
		////	ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокПеремещения + НСтр("ka='"" შეცვლილია.';ru='"" изменен.'");
		////	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		////Исключение
		////	ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокПеремещения + НСтр("ka='"" ვერ მოხერხდა შეცვლა.';ru='"" не удалось изменить.'");
		////	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения + Символы.ПС + ОписаниеОшибки());
		////КонецПопытки;
		...
		Попытка
			ДокПеремещения.Записать(РежимЗаписиДокумента.ОтменаПроведения);
			ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокПеремещения + НСтр("ka='"" მონიშნულია წაშლაზე.';ru='"" помечен на удаление.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Исключение
			ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокПеремещения + НСтр("ka='"" ვერ მოინიშნა წაშლაზე.';ru='"" не удалоць пометить на удаления.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения + Символы.ПС + ОписаниеОшибки());
		КонецПопытки;
	...
	Пока Выборка1.Следующий() Цикл
		...
		Попытка
			...
			ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокРезерва + НСтр("ka='"" მონიშნულია წაშლაზე.';ru='"" помечен на удаление.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Исключение
			ТекстСообщения = НСтр("ka='დოკუმენტი ""';ru='Документ ""'") + ДокРезерва + НСтр("ka='"" ვერ მოინიშნა წაშლაზე.';ru='"" не удалоць пометить на удаления.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения + Символы.ПС + ОписаниеОшибки());
		КонецПопытки;
...
Процедура ЗНИзменитьТоварыРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ცხრილი ""საქონელი"" არ არის შევსებული!';ru='Табличная часть ""Товары"" не заполнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
