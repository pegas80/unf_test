...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
	    ЗаполнитьПолеОформлениеРезерва();
	КонецЕсли;
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	...
	ЭтотОбъект.РезервированиеЗапасов	= ПолучитьФункциональнуюОпцию("РезервированиеЗапасов")
										ИЛИ ПолучитьФункциональнуюОпцию("РезервированиеЗапасов_new"); // Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	...
	УстановитьВидимостьКнопкиСчетНаОплату(); //+++ Дмгг
	...
	//+++ Дмгг
	ИспользоватьУпрошенныйПоискПоШтрихкоду = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПоискШтрихкодаВТабличнойЧастиДокументаЗаказПокупателя");
	Если ИспользоватьУпрошенныйПоискПоШтрихкоду Тогда 
	...
	//+++ Дмгг
	ИспользоватьФункционалСбораТоваров = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалСбораТоваров");
	ИспользоватьФункциональнаяОпцияРезервированиеЗапасов = ПолучитьФункциональнуюОпцию("РезервированиеЗапасов")
													ИЛИ ПолучитьФункциональнуюОпцию("РезервированиеЗапасов_new"); // Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления;
	ИспользоватьОтключениеАвтоматическогоРезервированияПриСбореТовара = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ОтключениеАвтоматическогоРезервированияПриСбореТовара");
	//--- Дмгг
	...
	//++Mira
	Если ИспользоватьФункционалМира() Тогда 	
		ЗначениеНастройки = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователи.ТекущийПользователь(), "МногосСкладныйРежимЗаказа");		
		Если ЗначениеНастройки  Тогда 
			Объект.МногосСкладныйРежим = Истина;
			ОбновитьнадписьПеремешений();
		Иначе 
			Объект.МногосСкладныйРежим = Ложь;
		КонецЕсли;
	КонецЕсли;
	//++Mira
...
Процедура ПриОткрытии(Отказ)
	...
	//+++ Дмгг
	Попытка
		ПроигровательСОМОбъект = Новый COMОбъект("WScript.Shell");
		
		ЗвукГотово = ПолучитьИмяФаила("ЗвукУспеха");
		ЗвукОшибки = ПолучитьИмяФаила("ЗвукОшыбки");
		ЗвукДобавить = ПолучитьИмяФаила("ЗвукДобавки");
		
	Исключение
		
		Сообщить("Нет звукового сигала");
		ПроигровательСОМОбъект = Неопределено;
	КонецПопытки;
	//--- Дмгг
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование"
		...
		Если ИмяСобытия = "ScanData" Тогда
			...
			Если ИспользоватьУпрошенныйПоискПоШтрихкоду Тогда 
				ЗаполнитьПоДаннымШтрихкодовПоискВТаблицеДокумета(Данные);
			Иначе
				ПолученыШтрихкоды(Данные);
			КонецЕсли;
			//--- Дмгг
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.ВариантЗавершенияРасширеннаяПодсказка.Заголовок = СтрШаблон(НСтр("ru = 'Предыдущее состояние: %1'; ka = 'Предыдущее состояние: %1'; en = 'Previous state: %1'"),
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если Не Отказ Тогда
			...
			Если Отказ Тогда 
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='შეკვეთის ნაშთები არ ემთხვევა რეზერვის ნაშთებს!';
										|ru='Остатки заказов не соответствуют резервным остаткам!';
										|en='Order balances do not match reserve balances!'"); 
				Сообщение.КлючДанных = Объект.Ссылка;
				Сообщение.Сообщить();
		...
		Если  ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			Ответ = Вопрос(НСтр("ka='საწყობში არ არის საკმარისი ნაშთი. გნებავთ გაგრძელება?';
								|ru='На складе недостаточно товара. Хотите продолжить?';
								|en='There is not enough stock in the warehouse. Do you want to continue?'"), Режим, 0);				
	...
	Если Объект.ОптоваяЦенаВДок = ПредопределенноеЗначение("Справочник.ВидыЦен.Оптовая")и 
		...
		Вопр = Вопрос(НСтр("ka='საქონელი გამოიწერა საბითუმო ფასში, გთხოვთ მოახდინოთ მყიდველის იდენტიფიცირება';
								|ru='Товар был приобретен по оптовой цене, пожалуйста, укажите покупателя.';
								|en='The item was purchased at a wholesale price, please identify the buyer.'"),
	...
	Иначе
		Если ПолучитьФункционалТипЦеныНоменклатурыПоКоличеству() и  ЗначениеЗаполнено(Объект.ОптоваяЦенаВДок) Тогда 
			...
			Если Объект.ОптоваяЦенаВДок <> ПредопределенноеЗначение("Справочник.ВидыЦен.Учетная") Тогда 		
				Вопр = Вопрос(НСтр("ka='საქონელი გამოიწერა საცალო ფასის ტიპისგან განსხვავებული ფასის ტიპით, გთხოვთ მოახდინოთ მყიდველის იდენტიფიცირება.';
								|ru='Товар был приобретен по цене, отличной от розничной. Укажите, пожалуйста, покупателя.';
								|en='The item was purchased at a price type different from the retail price type, please identify the buyer.'"),
				РежимДиалогаВопрос.ДаНет,
				0, 			
				КодВозвратаДиалога.Да, 
				"" 
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
 ...
 ////++Lazare 20-08-2021
 //    Если Объект.ОптоваяЦенаВДок = ПредопределенноеЗначение("Справочник.ВидыЦен.Оптовая")и 
 //   	 ПолучитьФункционалТипЦеныНоменклатурыПоКоличеству()
 //   	 Тогда 
 //   	
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Объект.ВидНалогаНДС = СтруктураДанные.ВидНалогаНДС; //Малышев 2024-12-17
...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Объект.ПоложениеСклада =  ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти")Тогда 
		Если НоваяСтрока И Не Копирование Тогда 	
			Элемент.ТекущиеДанные.СтруктурнаяЕдиницаЗапасы = Объект.СтруктурнаяЕдиницаРезерв ;
		КонецЕсли;
	КонецЕсли;
	
	Если ИспользоватьФункционалМира() Тогда 	
		Если НоваяСтрока И Не Копирование Тогда		
			Элемент.ТекущиеДанные.СтруктурнаяЕдиницаПеремешения = Объект.СтруктурнаяЕдиницаРезерв; 
			ЭтаФорма.ТекущийЭлемент = Элементы.ЗапасыНоменклатура;
		КонецЕсли;
		//_Lazare
	КонецЕсли;
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент, СтрокаТабличнойЧасти = Неопределено)
	...
	СтрокаТабличнойЧасти.Усушка_Потери = Номенклатура_Усушка_Потери(СтрокаТабличнойЧасти.Номенклатура);
	СтрокаТабличнойЧасти.Усушка_СписаниеНаПотери = Номенклатура_Усушка_СписаниеНаПотери(СтрокаТабличнойЧасти.Номенклатура);
	// Kuzmich, 24.09.2021, Потери при сушке, GermanButcher 2010
	...
	Если ЗначениеЗаполнено(Объект.ВидЦен) Тогда
		...
		Иначе 
		//СтруктураДанные.Вставить("ВидЦен", ?(ПолучитьФункционалТипЦеныНоменклатурыПоКоличеству(),ВидЦен,Объект.ВидЦен));//Lazare
		СтруктураДанные.Вставить("ВидЦен",ВидЦен);//Lazare
		КонецЕсли;
...
Процедура ЗапасыКоличествоПриИзменении(Элемент)
	...
	//Lazare_
	Если ПолучитьФункционалТипЦеныНоменклатурыПоКоличеству() Тогда 
		//ОптоваяЦенаВДок = ПредопределенноеЗначение("Справочник.ВидыЦен.ПустаяСсылка");
		СтруктураДанные = Новый Структура ;
		СтруктураДанные.Вставить("Организация", Объект.Организация);
		СтруктураДанные.Вставить("Номенклатура", СтрокаТабличнойЧасти.Номенклатура);
		СтруктураДанные.Вставить("Характеристика", СтрокаТабличнойЧасти.Характеристика);
...
Процедура ПровестиИЗакрыть(Команда)
	...
	Если Не Собран Тогда
		...
		Ответ = Вопрос("შეკვეთილი რაოდენობა არ ედრება აგროვებულ რაოდენობას! გნებავთ დოკუმენტის გატარება?",
	...
	// //++Lazare 20-08-2021
	// Если ОптоваяЦена = ПредопределенноеЗначение("Справочник.ВидыЦен.Оптовая")и ОбъектМодифицирован()и
	//	 ПолучитьФункционалТипЦеныНоменклатурыПоКоличеству()
	//	 Тогда 
	//	
	//	Вопр = Вопрос("საქონელი გამოიწერა საბითუმო ფასში, გთხოვთ მოახდინოთ მყიდველის იდენტიფიცირება",
...
Процедура Провести(Команда)
	...
	Собран = Истина;
	//ЕСЛИ НЕ (ИспользоватьФункционалСбораТоваров
	//		И ИспользоватьФункциональнаяОпцияРезервированиеЗапасов 
	//		И ИспользоватьОтключениеАвтоматическогоРезервированияПриСбореТовара) Тогда 
	...
	Если Не Собран Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да и УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты
		("ПеренестиСобранноеКоличествоЗапасовИзЗаказаПокупателяВРасходнойНакладной") Тогда
			
			Вопр = Вопрос("გნებავთ დოკუმენტის სტატუსის გადყავანა  აგროვებულია-ზე ?",
			РежимДиалогаВопрос.ДаНет,
			0, 			
			КодВозвратаДиалога.Нет, 
			"აგროვება" 
			);		
			Если Вопр = КодВозвратаДиалога.Да Тогда 				
				 ИзменитьСтатусДокументаНаСобран();				 
			КонецЕсли;
...
Процедура ПроигратьЗвук(Тип)
	...
	Если ПроигровательСОМОбъект = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	ИмяФайла = "";
	Если Тип = "ЗвукОшыбки" Тогда 
		ИмяФайла = ЗвукОшибки;
	ИначеЕсли Тип = "ЗвукУспеха" Тогда
		ИмяФайла = ЗвукГотово;
	ИначеЕсли Тип = "ЗвукДобавки" Тогда
		ИмяФайла = ЗвукДобавить;
	Иначе 
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ПроигровательСОМОбъект.Run("taskkill /f /im Cscript.exe", , 0);
		ПроигровательСОМОбъект.Run("taskkill /f /im wscript.exe", , 0);

		ПроигровательСОМОбъект.Run("powershell -Command (New-Object Media.SoundPlayer '" + ИмяФайла + "').PlaySync()", 0, 0);
...
Процедура ИзменитьСтатусПроведение(Собран)
	...
	Если ИспользоватьФункционалСбораТоваров Тогда
		...
		Для Каждого Стр Из Объект.Запасы Цикл 
			...
			Иначе       
				Если НЕ Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда // Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
					Стр.Резерв = 0;
				КонецЕсли; // Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			КонецЕсли;
...
&НаСервере
Процедура ИзменитьСтатусДокументаНаСобран()
		Объект.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Собран;		
КонецПроцедуры	
...
Процедура ПоискПоШтрихкодуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ВариантРаспознаванияШтрихкода = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ВариантРаспознованияШтрихкода");
...
Процедура НастройкаДокумента(Команда)
	...
	СтруктураПараметров.Вставить ("ПоложениеСкладаВЗаказеПокупателя", Объект.ПоложениеСклада );
...
Процедура НастройкаДокументаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(СтруктураНастройкаДокумента) = Тип("Структура") И СтруктураНастройкаДокумента.БылиВнесеныИзменения Тогда
		...
		//Lazare_
		Объект.ПоложениеСклада       = СтруктураНастройкаДокумента.ПоложениеСкладаВЗаказеПокупателя	;
		ПредВидимостьПоложениеСклада = Элементы.СтруктурнаяЕдиницаРезерв.Видимость;
		//_Lazare
		...
		Если ПредВидимостьПоложениеСклада = Ложь // Было в ТЧ.
			И Элементы.СтруктурнаяЕдиницаРезерв.Видимость = Истина Тогда // Стало в шапке.
			
			Для каждого Строка Из Объект.Запасы Цикл
				Если НЕ ЗначениеЗаполнено(Строка.СтруктурнаяЕдиницаЗапасы) Тогда
					Продолжить;
				КонецЕсли;
				СтруктурнаяЕдиницаРезерв = Строка.СтруктурнаяЕдиницаЗапасы;
				Прервать;
			КонецЦикла;
		КонецЕсли;
		//_Lazare
...
// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
&НаСервере
Процедура ЗаполнитьПолеОформлениеРезерва()

	Для каждого ТекущаяСтрока Из Объект.Запасы Цикл
		
		ТекущееКоличество = ТекущаяСтрока.Количество;
		Если ТипЗНЧ(ТекущаяСтрока) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
		
			ТекущееКоличество = ТекущееКоличество * ТекущаяСтрока.ЕдиницаИзмерения.Коэффициент;
		
		КонецЕсли;
	
		Если ТекущаяСтрока.СвободныйОстаток <= 0 Тогда
		
			ТекущаяСтрока.ОформлениеРезерва = 1;
			
		ИначеЕсли ТекущаяСтрока.СвободныйОстаток < ТекущееКоличество Тогда
			
			ТекущаяСтрока.ОформлениеРезерва = 2;
			
		Иначе
			
			ТекущаяСтрока.ОформлениеРезерва = 3;
...
&НаКлиенте
Процедура ЗапасыПроверитьСвободныеОстатки(Команда)
	
	Если ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаРезерв) И ЗначениеЗаполнено(Объект.Организация) Тогда
		
		ЗапасыПроверитьСвободныеОстаткиНаСервере();
	
	Иначе
		
		ТекстСообщения = НСТР("ka='Не заполнено поле ""საწყობი (რეზერვი)"" или ""Организация""';ru='Не заполнено поле ""Склад (резерв)"" или ""Организация""';en='Не заполнено поле ""Warehouse (reserve)"" или ""Организация""'");
...
&НаСервере
Процедура ЗапасыПроверитьСвободныеОстаткиНаСервере()
	Перем РЕЗ;
	
	ДатаПроверки = ?(ЗначениеЗаполнено(Объект.Дата),Объект.Дата,ТекущаяДата());
	ТЗ_Запасы = Объект.Запасы.Выгрузить(,"НомерСтроки,Номенклатура,Характеристика,Партия,Проект,ЕдиницаИзмерения,Количество,Резерв");
	...
	Запрос.Текст = 
		...
		|	ТЗ_Запасы.Номенклатура КАК Номенклатура,
		|	ТЗ_Запасы.Характеристика КАК Характеристика,
		|	ТЗ_Запасы.Партия КАК Партия,
		|	ТЗ_Запасы.Проект КАК Проект,
		|	ТЗ_Запасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ТЗ_Запасы.Количество КАК Количество,
		|	ТЗ_Запасы.Резерв КАК Резерв
		|ПОМЕСТИТЬ ТЗ_Запасы
		|ИЗ
		...
		|			ТОГДА ВЫБОР
		|					КОГДА ТЗ_Запасы.Проект ЕСТЬ NULL
		|							ИЛИ ТЗ_Запасы.Проект = НЕОПРЕДЕЛЕНО
		|							ИЛИ ТЗ_Запасы.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|						ТОГДА ВЫБОР
		|								КОГДА &Проект ЕСТЬ NULL
		|										ИЛИ &Проект = НЕОПРЕДЕЛЕНО
		|										ИЛИ &Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|									ТОГДА ЗНАЧЕНИЕ(Справочник.Проекты.БезПроекта)
		|								ИНАЧЕ &Проект
		|							КОНЕЦ
		|					ИНАЧЕ ТЗ_Запасы.Проект
		|				КОНЕЦ
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|	КОНЕЦ КАК Проект,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ТЗ_Запасы.ЕдиницаИзмерения) = ТИП(Справочник.ЕдиницыИзмерения)
		|			ТОГДА ТЗ_Запасы.Количество * ВЫРАЗИТЬ(ТЗ_Запасы.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения).Коэффициент
		|		ИНАЧЕ ТЗ_Запасы.Количество
	...
	Запрос.УстановитьПараметр("ТЗ_Запасы",ТЗ_Запасы);
	...
	ТаблицаСвободныхОстатоков = РезервированиеЗапасов_new.ПолучитьТаблицуСвободныхОстатков(МенеджерВременныхТаблиц,ДатаПроверки);
	
	Для каждого ТекущаяСтрока Из ТаблицаСвободныхОстатоков Цикл
		
		СтрокаЗапасы = Объект.Запасы[ТекущаяСтрока.НомерСтроки-1];
		СтрокаЗапасы.Резерв = МИН(ТекущаяСтрока.СвободныйОстаток,ТекущаяСтрока.Количество);
		СтрокаЗапасы.СвободныйОстаток = ТекущаяСтрока.СвободныйОстаток;
		СтрокаЗапасы.ОжидаемыеПоставки = ТекущаяСтрока.ОжидаемыеПоставки;
		СтрокаЗапасы.ОжидаемыеСвободныеОстатки = ТекущаяСтрока.ОжидаемыеСвободныеОстатки;			
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() Тогда
		...
		НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
		ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Запасы.ОформлениеРезерва",1);
		ДобавитьОформляемоеПоле(НовоеУсловноеОформление, Элементы.ЗапасыРезерв.Имя);
		ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветФона", WebЦвета.СветлоРозовый);
		
		НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
		ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Запасы.ОформлениеРезерва",2);
		ДобавитьОформляемоеПоле(НовоеУсловноеОформление, Элементы.ЗапасыРезерв.Имя);
		ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветФона", WebЦвета.СветлоЖелтый);
		
		НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
		ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Запасы.ОформлениеРезерва",3);
		ДобавитьОформляемоеПоле(НовоеУсловноеОформление, Элементы.ЗапасыРезерв.Имя);
		ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветФона", WebЦвета.СветлоЗеленый);
...
Функция ПолучитьДанныеОрганизацияПриИзменении()
	...
	СтруктураДанные.Вставить("ВидНалогаНДС", Объект.Организация.ВидНалогаНДС); //Малышев 2024-12-17
...
Процедура ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС(ЭтоОткрытие = Ложь)
	...
	//+++ Дмгг, попросили сделать так же как и Расходной накладной
	//Объект.НалогообложениеНДС = УправлениеНебольшойФирмойСервер.НалогообложениеНДС(Объект.Организация,, Объект.Дата, Объект.Контрагент); 
	Если ИспользоватьИсториюНалогообложенияКонтрагента Тогда 
		СтруктураНалогообложения = УправлениеНебольшойФирмойСервер.НалогообложениеНДС_СтруктураСНДС(Объект.Организация,, Объект.Дата, Объект.Контрагент,Объект.Договор);
	...
	Если НЕ НалогообложениеПередИзменением = Объект.НалогообложениеНДС ИЛИ ЭтоОткрытие
		ИЛИ ИспользоватьИсториюНалогообложенияКонтрагента Тогда
...
&НаКлиенте
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(ИмяТабличнойЧасти = "Запасы", СтрокаТабличнойЧасти = Неопределено) Экспорт
	
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		СтрокаТабличнойЧасти = Элементы[ИмяТабличнойЧасти].ТекущиеДанные;
	КонецЕсли;
	// Kuzmich, 24.09.2021, Потери при сушке, GermanButcher 2010
	Если (ИмяТабличнойЧасти = "Запасы" И ПолучитьФункционалРезервированияЗаказов() И Константы_German_Butcher_Получить() И СтрокаТабличнойЧасти.КоличествоСобр > 0 И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Усушка_Потери)) Тогда
		Усушка_РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти);	
	Иначе
	// Kuzmich, 24.09.2021, Потери при сушке, GermanButcher 2010
	...
	// Kuzmich, 24.09.2021, Потери при сушке, GermanButcher 2010
	КонецЕсли;
	ЗавершитьРасчетСуммВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти);
	// Kuzmich, 24.09.2021, Потери при сушке, GermanButcher 2010
...
Процедура ПересчитатьПлатежныйКалендарь()
	...
	///++Lazare
	Если Объект.НеПересчитатьПлатежныйКалендарь Тогда
		
	СуммаПроцентКалендаря   = 0;
	СуммаЗапасов			= Объект.Запасы.Итог("Всего");
	СуммаПлатежныйКалендарь = Объект.ПлатежныйКалендарь.Итог("СуммаОплаты");
	Если СуммаЗапасов = 0 Тогда
		Возврат;
	КонецЕсли;
	Для Каждого СтрокаТЧ Из Объект.ПлатежныйКалендарь Цикл
		СтрокаТЧ.ПроцентОплаты	= СтрокаТЧ.СуммаОплаты / СуммаЗапасов * 100;
		СуммаПроцентКалендаря	= СуммаПроцентКалендаря + СтрокаТЧ.ПроцентОплаты;
	КонецЦикла;
	
	Если СуммаПлатежныйКалендарь = СуммаЗапасов И СуммаПроцентКалендаря <> 100 Тогда
		ПроцентОтклонения = 100 - СуммаПроцентКалендаря;
		СтрокаТЧ.ПроцентОплаты = СтрокаТЧ.ПроцентОплаты + ПроцентОтклонения;
	КонецЕсли;
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)  
	...
	НеРезервироватьТОвар = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ОтключениеАвтоматическогоРезервированияПриСбореТовара") И УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункциональнаяОпцияРезервированиеЗапасов") И УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалСбораТоваров");
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если МассивСтрокТЧ.Количество() = 0 Тогда
				...
				Если ОбщегоНазначенияВызовСервера.ПолучитьВариантРаспознаванияШтрихкода() = ПредопределенноеЗначение("Перечисление.ВариантыРаспознованияШтрихкодов.Вес") Тогда
					...
					Если ОбщегоНазначенияВызовСервера.ПолучитьКодЕдиницыИзмерения(ДанныеШтрихкода.Номенклатура) = "166" Тогда
						Если ТекШтрихкод.Свойство("КоличествоИзШтрихкода") Тогда 
			...
			Иначе
				...
				Если ПолучитьФункционалРезервированияЗаказов() Тогда
					Если НайденнаяСтрока.КоличествоСобр + Окр(ТекШтрихкод.Количество*ПолучитьКоэффициент(ДанныеШтрихкода.ЕдиницаИзмерения)-СмещениеДляОкругления,3) <= НайденнаяСтрока.Количество Тогда  			 
						Если ПолучитьПустойЗаказСобр() Тогда
							Если ПолучитьПоСостояние() = "ВПроцесеСбора" Тогда
								НайденнаяСтрока.КоличествоСобр = НайденнаяСтрока.КоличествоСобр + Окр(ТекШтрихкод.Количество*ПолучитьКоэффициент(ДанныеШтрихкода.ЕдиницаИзмерения)-СмещениеДляОкругления,3);
								НайденнаяСтрока.КоличествоКлик = НайденнаяСтрока.КоличествоКлик + 1;
							ИначеЕсли ПолучитьПоСостояние() = "Открито" Тогда
								НайденнаяСтрока.Количество =  НайденнаяСтрока.Количество + Окр(ТекШтрихкод.Количество*ПолучитьКоэффициент(ДанныеШтрихкода.ЕдиницаИзмерения)-СмещениеДляОкругления,3);
							КонецЕсли;
						Иначе
								НайденнаяСтрока.КоличествоСобр = НайденнаяСтрока.КоличествоСобр + Окр(ТекШтрихкод.Количество*ПолучитьКоэффициент(ДанныеШтрихкода.ЕдиницаИзмерения)-СмещениеДляОкругления,3);
								НайденнаяСтрока.КоличествоКлик = НайденнаяСтрока.КоличествоКлик + 1;
						...
						Для Каждого Стр Из Объект.Запасы Цикл 
							...
							Если Стр.КоличествоСобр <> Стр.Количество Тогда
								...
								Прервать;
							//+++ Дмгг, значение переменной принимается перед циклом
							ИначеЕсли НеРезервироватьТОвар Тогда 
								Стр.Резерв = 0;
							//--- Дмгг
							Иначе
								Если не УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПеренестиСобранноеКоличествоЗапасовИзЗаказаПокупателяВРасходнойНакладной") Тогда
									Стр.Резерв = Стр.КоличествоСобр;
								КонецЕсли;
							КонецЕсли;	
						...
						КонецЕсли;
					//++Lazare	
					ИначеЕсли УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПеренестиСобранноеКоличествоЗапасовИзЗаказаПокупателяВРасходнойНакладной") 
							Тогда
						 		НайденнаяСтрока.КоличествоСобр = НайденнаяСтрока.КоличествоСобр + Окр(ТекШтрихкод.Количество*ПолучитьКоэффициент(ДанныеШтрихкода.ЕдиницаИзмерения)-СмещениеДляОкругления,3);
								НайденнаяСтрока.КоличествоКлик = НайденнаяСтрока.КоличествоКлик + 1;
					//--Lazare
				...
				Если ПолучитьФункционалТипЦеныНоменклатурыПоКоличеству() Тогда 
					...
					Если ВидЦен <>  ПредопределенноеЗначение("Справочник.ВидыЦен.ПустаяСсылка")или ВидЦен <> Неопределено Тогда 
						...
						СтруктураДанные.Вставить("ВидСкидкиНаценки", Объект.ВидСкидкиНаценки);
						СтруктураДанные  = ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные);
						НайденнаяСтрока.Цена = СтруктураДанные.Цена;
						ОптоваяЦена = ВидЦен;
					КонецЕсли;			
				КонецЕсли;
				//Lazare_
...
&НаКлиенте
Процедура ЗаполнитьПоДаннымШтрихкодовПоискВТаблицеДокумета(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из МассивШтрихкодов Цикл
		ДанныеШтрихкода = ТекШтрихкод.Штрихкод;
		
		ПоискШтрихкода = тзПоискаУпрощенногоСканирования.НайтиСтроки(Новый Структура("ШтрихКод",ТекШтрихкод.Штрихкод));
		
		Если ПоискШтрихкода.Количество() = 0 Тогда
			
			ПроигратьЗвук("ЗвукОшыбки");
			//ПоказатьПредупреждение(,"ნომენკლატურა: " + ДанныеШтрихкода.Номенклатура + " შეკვეთილი არ არის!");
			
			Если НайтиШтрихКодВБазе(ДанныеШтрихкода) Тогда 
				ПоказатьПредупреждение(,"მოცემულ დოკუმენტში არ მოიძებნა დასკანირებული ნომენკლატურა: """+ДанныеШтрихкода+"""");
			Иначе
				ПоказатьПредупреждение(,"არსებული შტრიხკოდი ბაზაში არ მოიძებნა: """+ДанныеШтрихкода+"""");
			КонецЕсли;
		Иначе
			
			ДанныеШтрихкода = ПоискШтрихкода[0];
			
			ДобавляемоеКоличество = ?(ДанныеШтрихкода.КоэффЕдиницыИзмерения = "", 1, ДанныеШтрихкода.КоэффЕдиницыИзмерения);
			
			НайденнаяСтрока = Объект.Запасы[ДанныеШтрихкода.НомерСтроки - 1];
			Если НайденнаяСтрока.КоличествоСобр + ДобавляемоеКоличество <= НайденнаяСтрока.Количество Тогда  			 
				
				НайденнаяСтрока.КоличествоСобр = НайденнаяСтрока.КоличествоСобр + ДобавляемоеКоличество;
				НайденнаяСтрока.КоличествоКлик = НайденнаяСтрока.КоличествоКлик + 1;
				НайденнаяСтрока.Резерв = НайденнаяСтрока.КоличествоСобр;
				
				Если ПолучитьФункционалРезервированияЗаказов() И Не ПолучитьПустойЗаказСобр() Тогда
					ИзменитьСтатусБезЗаписи();
				КонецЕсли;
				
				Собран = Истина;
				
				Если Объект.Запасы.Итог("КоличествоСобр") <> Объект.Запасы.Итог("Количество") Тогда
...
КонецПроцедуры // ЗаполнитьПоДаннымШтрихкодов()
...
&НаСервере
Процедура ИзменитьСтатусБезЗаписи()
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") 
		...
		// 28.06.2022 Soniva 
		Если РезультатЗакрытия.ПерезаполнитьЦеныПоДиапазону Тогда 	
...
Процедура УправлениеФормой()
	...
	//_Lazare	
	ПоложениеСкладаВШапке	= Объект.ПоложениеСклада = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке");
    //Lazare_
	...
	//Lazare_
	Элементы.СтруктурнаяЕдиницаЗапасы.Видимость = Не ПоложениеСкладаВШапке;
	//_Lazare	
	...
	Элементы.ЗапасыПартия.Видимость					= Не ЭтоЗаказНаПереработку; // И ЭтотОбъект.РезервированиеЗапасов; //+++ Дмгг
...
&НаСервере
Процедура ОбновитьИтоги(ИтогВсего, ИтогСуммаНДС, Знач ДанныеДляРасчетаИтогов)
	...
	Для Каждого Стр Из Объект.Запасы.НайтиСтроки(Новый Структура("НомерВариантаКП", ДанныеДляРасчетаИтогов.ТекущийВариантКП)) Цикл
		...
		Иначе
			...
			//Паганини_
			Если НЕ (Стр.ПроцентСкидкиНаценки = 0 И Стр.ПроцентАвтоматическойСкидки = 0) Тогда
				Если Объект.СуммаВключаетНДС Тогда
					ИтогСкидка = ИтогСкидка + Окр(Стр.Цена * Стр.Количество) - Стр.Сумма;
				Иначе
			    	ИтогСкидка = ИтогСкидка + Окр(Стр.Цена * Стр.Количество * (100 + УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(Стр.СтавкаНДС))/100, 2) - Стр.Всего;
				КонецЕсли;
			КонецЕсли;
			//_Паганини
		КонецЕсли;
...
Процедура НастроитьЭлементыПоВариантуУчетаДоставки(Форма)
	...
	//Lazare_
	ПравоРедактированиеСтоимостьДоставки = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ПравоРедактированиеСтоимостьДоставки");
	Если Форма.ВариантУчетаДоставки = ПредопределенноеЗначение("Перечисление.ВариантыУчетаДоставки.ДоставкаСОплатой")
		Или Форма.ВариантУчетаДоставки = ПредопределенноеЗначение("Перечисление.ВариантыУчетаДоставки.ВозмещениеСтоимости") 
		Или  ПравоРедактированиеСтоимостьДоставки = Истина
		Тогда
		Элементы.СтоимостьДоставки.ТолькоПросмотр =  Не ПравоРедактированиеСтоимостьДоставки;
		Элементы.РассчитатьСтоимостьДоставки.Доступность = Форма.ЗаданаФормулаРасчета;
		Элементы.СтавкаНДСДоставки.ТолькоПросмотр = Ложь;
...
Процедура ПроизводствоНаСервере(ДанныеФормы, МассивВыделеныхСтрок)
	...
	//+++ Дмгг
	//НаВсякСлучай добавим док Основание если он не совпадает
	Если НЕ ЗначениеЗаполнено(Док.ДокументОснование)
		ИЛИ ЗначениеЗаполнено(Док.ДокументОснование) И Док.ДокументОснование <> Объект.Ссылка Тогда 
	...
	Для Каждого СтрокПроизводства Из Док.Продукция Цикл 
		...
		МассивНайденныхСтрок = тзОставляемыхСтрок.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, Партия, ЕдиницаИзмерения", СтрокПроизводства.Номенклатура, СтрокПроизводства.Характеристика, СтрокПроизводства.Партия, СтрокПроизводства.ЕдиницаИзмерения));
...
Процедура УстановитьВидимостьКнопкиСчетНаОплату()

	ЗначениеВидивости = ПолучитьФункциональнуюОпцию("КонтрольОтстатковВДокументеСчётНаОплату");
	
	Элементы.СчетНаОплату.Видимость = ЗначениеВидивости;
...
Функция ПроверитьОстатки()
	...
	|	ВТЗапасы.Организация,
	|	ВТЗапасы.СтруктурнаяЕдиница,
	|	ЕСТЬNULL(ВТЗапасы.Количество, 0) как Количество, 
	|	ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0) КАК ОстатокЗапасыНаСкладах,
	|	-(ЕСТЬNULL(ВТЗапасы.Количество, 0) - ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0)) КАК КоличествоОстатокЗапасыНаСкладах
	|ИЗ
	|	ВТЗапасы КАК ВТЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(&МоментКонтроля, Организация=&Организация и СтруктурнаяЕдиница = &СтруктурнаяЕдиница) КАК ЗапасыНаСкладахОстатки
	|		ПО //ВТЗапасы.Организация = ЗапасыНаСкладахОстатки.Организация
	|			//И ВТЗапасы.СтруктурнаяЕдиница = ЗапасыНаСкладахОстатки.СтруктурнаяЕдиница
	|			ВТЗапасы.Номенклатура = ЗапасыНаСкладахОстатки.Номенклатура
	|			И ВТЗапасы.Характеристика = ЗапасыНаСкладахОстатки.Характеристика
	|ГДЕ
	|	ВТЗапасы.Номенклатура.ТипНоменклатуры = Значение(Перечисление.ТипыНоменклатуры.Запас) И  
	|	ЕСТЬNULL(ЗапасыНаСкладахОстатки.КоличествоОстаток, 0) - ЕСТЬNULL(ВТЗапасы.Количество, 0) < 0";
	//Малышев 2025-11-30 ONESOFT-4552 --))
	...
	Запрос.УстановитьПараметр("СтруктурнаяЕдиница",Объект.СтруктурнаяЕдиницаРезерв); //Малышев 2025-11-30 ONESOFT-4552
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл 
		
		ТекстОшыбки = ТекстОшыбки + СтрШаблон(НСтр("ka='ნომენკლატურა: %1/%2   ნაშთი არის: %3, მოთხოვნილება: %4 ';
											|ru='Номенклатура: %1/%2   Баланс такой: %3, затребовано: %4';
											|en='Nomenclature: %1/%2   The balance is: %3, requested: %4'"), Выборка.Номенклатура, Выборка.Характеристика, Выборка.ОстатокЗапасыНаСкладах, Выборка.Количество)	+ Символы.ПС;
		ОстаткиЕсть = Ложь;
...
Процедура ЗаполнитьТаблицуПоиска()
	...
	тчЗапасы = Объект.Запасы;
	Если тчЗапасы.Количество() = 0 Тогда 
		Возврат;
	КонецЕсли;
	
	тзЗапасы = тчЗапасы.Выгрузить();
	...
	Запрос.Текст = 
		...
		|	ВЫРАЗИТЬ(ВТ_Запасы.Номенклатура КАК Справочник.Номенклатура).ОсновнойШтрихКод КАК ОсновнойШтрихКод,
		|	ШтрихкодыНоменклатуры.ЕдиницаИзмерения КАК ЕдиницаИзмеренияШтрихКода,
		|	ШтрихкодыНоменклатуры.ЕдиницаИзмерения.Коэффициент КАК КоэффЕдиницыИзмерения,
		|	ШтрихкодыНоменклатуры.Штрихкод,
		|	ВЫРАЗИТЬ(ВТ_Запасы.Номенклатура КАК Справочник.Номенклатура).ЕдиницаИзмерения КАК ОсновнаяЕдиницаИзмерения
		|ИЗ
	...
	тзПоискаУпрощенногоСканирования.Загрузить(Запрос.Выполнить().Выгрузить());
	
КонецПроцедуры

Функция НайтиШтрихКодВБазе(ШтрихКод)
...
&НаСервере
Процедура УпорядочитьПоАртикулуВозрастаниеНаСервере(ЗначениеУпорядочивания)
	
	тзЗапасы = Объект.Запасы.Выгрузить();
	
	тзЗапасы.Колонки.Добавить("Артикул");
	
	Для Каждого Строк Из тзЗапасы Цикл 
		Строк.Артикул = Строк.Номенклатура.Артикул;
	КонецЦикла;
	
	тзЗапасы.Сортировать("Артикул "+ЗначениеУпорядочивания);
...
&НаКлиенте
Процедура УпорядочитьПоАртикулуВозрастание(Команда)
	УпорядочитьПоАртикулуВозрастаниеНаСервере("Возр");
КонецПроцедуры


&НаКлиенте
Процедура УпорядочитьПоАртикулуУбывание(Команда)
	УпорядочитьПоАртикулуВозрастаниеНаСервере("Убыв")
КонецПроцедуры
//--- Дмгг

// Kuzmich, 24.09.2021, Потери при сушке, GermanButcher 2010
&НаКлиенте
Процедура ЗавершитьРасчетСуммВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти)
	
	//Малышев 2025-03-18 ONESOFT-3748
	// Скидки.
	//Если СтрокаТабличнойЧасти.ПроцентСкидкиНаценки = 100 Тогда
	//	СтрокаТабличнойЧасти.Сумма = 0;
	//ИначеЕсли СтрокаТабличнойЧасти.ПроцентСкидкиНаценки <> 0 И СтрокаТабличнойЧасти.Количество <> 0 Тогда
...
&НаСервере
Функция Номенклатура_Усушка_Потери(Номенклатура)
	Возврат Номенклатура.Усушка_Потери; 
КонецФункции

&НаСервере
Функция Номенклатура_Усушка_СписаниеНаПотери(Номенклатура)
	Возврат Номенклатура.Усушка_СписаниеНаПотери; 
КонецФункции

&НаКлиенте
Процедура Усушка_РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти)
	
	Если ПолучитьФункционалРезервированияЗаказов() И Константы_German_Butcher_Получить() И СтрокаТабличнойЧасти.КоличествоСобр > 0 И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Усушка_Потери) Тогда
		
		Если СтрокаТабличнойЧасти = Неопределено Тогда
			СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Усушка_Потери) Тогда
			
			ПроцентУсушки = СтрокаТабличнойЧасти_Усушка_Потери_ПроцентУсушки(СтрокаТабличнойЧасти.ПолучитьИдентификатор());
			СтрокаТабличнойЧасти.Усушка_КоличествоСобрПотери = СтрокаТабличнойЧасти.КоличествоСобр * ПроцентУсушки / 100;
			
		Иначе
			
			СтрокаТабличнойЧасти.Усушка_КоличествоСобрПотери = 0;
			
		КонецЕсли;
		
		СтрокаТабличнойЧасти.Усушка_КоличествоСобрNET = СтрокаТабличнойЧасти.КоличествоСобр - СтрокаТабличнойЧасти.Усушка_КоличествоСобрПотери;
		
		СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Усушка_КоличествоСобрNET * СтрокаТабличнойЧасти.Цена; 
...
&НаСервере
Функция СтрокаТабличнойЧасти_Усушка_Потери_ПроцентУсушки(ИдентификаторСтроки)
	
	СтрокаТабличнойЧасти = Объект.Запасы.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	Возврат СтрокаТабличнойЧасти.Усушка_Потери.ПроцентУсушки;
	
КонецФункции

&НаКлиенте
Процедура ЗапасыУсушка_ПотериПриИзменении(Элемент)
...
//++Mira
&НаКлиенте
Процедура СоздатьПеремешения(Команда)
	Если Модифицированность Тогда
		Сообщить(НСтр("ru='Данные изменени необходимо провести документ';ka='მონაცემები შეცვლილია, აუცილებელია დოკუმენტის გატარება'"));
		Возврат;
	КонецЕсли;
	СоздатьПеремешенияНаСервере();
	ОбновитьнадписьПеремешений();
КонецПроцедуры

&НаСервере
Функция СоздатьПеремешенияНаСервере() Экспорт 
	...
	Если Не  Объект.Проведен Тогда
		...
		Сообщить("დოკუმენტი აუცილებლად უნდა იყოს გატარებული",СтатусСообщения.Важное);
		
	КонецЕсли;
	Если ПроверитьРезервы() Тогда 
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЗаказыИПеремешенияОстатки.Номенклатура КАК Номенклатура,
		               |	ЗаказыИПеремешенияОстатки.Характеристика КАК Характеристика,
		               |	ЗаказыИПеремешенияОстатки.КоличествоОстаток КАК Количество,
		               |	ЗаказыИПеремешенияОстатки.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		               |	ЗаказыИПеремешенияОстатки.СтруктурнаяЕдиницаПолучатель КАК СтруктурнаяЕдиницаПолучатель,
		               |	ЗаказыИПеремешенияОстатки.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		               |	ЗаказыИПеремешенияОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		               |	ЗаказыИПеремешенияОстатки.Организация КАК Организация
		               |ИЗ
		               |	РегистрНакопления.ЗаказыИПеремешения.Остатки(, ЗаказПокупателя = &ЗаказПокупателя) КАК ЗаказыИПеремешенияОстатки
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ЗаказыИПеремешенияОстатки.Номенклатура,
		               |	ЗаказыИПеремешенияОстатки.Характеристика,
		               |	ЗаказыИПеремешенияОстатки.СтруктурнаяЕдиница,
		               |	ЗаказыИПеремешенияОстатки.ЗаказПокупателя,
		               |	ЗаказыИПеремешенияОстатки.СтруктурнаяЕдиницаПолучатель,
		               |	ЗаказыИПеремешенияОстатки.Номенклатура.ЕдиницаИзмерения,
		               |	ЗаказыИПеремешенияОстатки.КоличествоОстаток,
		               |	ЗаказыИПеремешенияОстатки.Организация
		               |ИТОГИ ПО
		               |	СтруктурнаяЕдиница";
		...
		Пока ВыборкаПоГруппировкам.Следующий() Цикл
			новыыДокументОбновитьнадписьПеремешений = Документы.ПеремещениеЗапасов.СоздатьДокумент();
			//УправлениеНебольшойФирмойСервер.ЗаполнитьШапкуДокумента(новыыДокументОбновитьнадписьПеремешений,,,,"",0,Ложь,Новый Структура);
			новыыДокументОбновитьнадписьПеремешений.Заполнить(Объект.Ссылка);
			//новыыДокументОбновитьнадписьПеремешений.Автор
			//новыыДокументОбновитьнадписьПеремешений.Организация = ВыборкаПоГруппировкам.Организация;
			новыыДокументОбновитьнадписьПеремешений.ВидОперации = Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение;
			новыыДокументОбновитьнадписьПеремешений.Дата = Объект.Дата - 1 ;
			новыыДокументОбновитьнадписьПеремешений.ДокументОснование = Объект.ссылка;
			//новыыДокументОбновитьнадписьПеремешений.ЗаказПокупателя = Объект.ссылка;
			новыыДокументОбновитьнадписьПеремешений.СтруктурнаяЕдиница = ВыборкаПоГруппировкам.СтруктурнаяЕдиница;
			новыыДокументОбновитьнадписьПеремешений.СтруктурнаяЕдиницаПолучатель = Объект.СтруктурнаяЕдиницаРезерв;
			Выборка = ВыборкаПоГруппировкам.Выбрать();
			
			новыыДокументОбновитьнадписьПеремешений.Запасы.Очистить();
			
			Пока Выборка.Следующий() Цикл
				
				нСтрокаЗапасов = новыыДокументОбновитьнадписьПеремешений.Запасы.Добавить();
				ЗаполнитьЗначенияСвойств(нСтрокаЗапасов,Выборка);
				
			КонецЦикла;
			
			новыыДокументОбновитьнадписьПеремешений.КоличествоТоваров = новыыДокументОбновитьнадписьПеремешений.Запасы.Итог("Количество");
			
			Попытка
				новыыДокументОбновитьнадписьПеремешений.Записать(РежимЗаписиДокумента.Проведение,РежимПроведенияДокумента.Неоперативный);
				//новыыДокументОбновитьнадписьПеремешений.Записать(РежимЗаписиДокумента.Запись);
				МассивДок.Добавить(новыыДокументОбновитьнадписьПеремешений.Ссылка);
			Исключение
				Сообщить(ОписаниеОшибки());
			КонецПопытки;
		КонецЦикла;
...
&НаСервере
Процедура ОбновитьСписокПеремещений(МассивДок)
	
	СписокПеремещений.ЗагрузитьЗначения(МассивДок);

КонецПроцедуры


&НаСервере
Функция ПолучитьДокументыПеремещения()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗаказыИПеремешенияОбороты.Регистратор
		|ИЗ
		|	РегистрНакопления.ЗаказыИПеремешения.Обороты(
		|			,
		|			,
		|			Регистратор,
		|			ВидОпераций = ЗНАЧЕНИЕ(Перечисление.рс_ВидыОперацийЗаказыИПеремешения.Перемешение)
		|				И ЗаказПокупателя = &ЗаказПокупателя) КАК ЗаказыИПеремешенияОбороты";
...
&НаСервере
Процедура ОбновитьнадписьПеремешений()
	
	МассивДок = ПолучитьДокументыПеремещения();
	СписокПеремещений.ЗагрузитьЗначения(МассивДок);

КонецПроцедуры

&НаСервере
Функция  ПроверитьРезервы()
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	врЗапасы.Номенклатура,
		|	врЗапасы.НомерСтроки,
		|	врЗапасы.Характеристика,
		|	врЗапасы.Количество,
		|	врЗапасы.Резерв
		|ПОМЕСТИТЬ врТабЗапасы
		|ИЗ
		|	&врЗапасы КАК врЗапасы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	врТабЗапасы.Номенклатура,
		|	врТабЗапасы.НомерСтроки,
		|	врТабЗапасы.Характеристика,
		|	врТабЗапасы.Количество,
		|	врТабЗапасы.Резерв
		|ИЗ
		|	врТабЗапасы КАК врТабЗапасы
		|ГДЕ
		|	врТабЗапасы.Количество <> врТабЗапасы.Резерв";
	
		
	Запрос.УстановитьПараметр("врЗапасы",Объект.Запасы.Выгрузить());
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ЕстьОткланения = РезультатЗапроса.Пустой();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Сообщить(ВыборкаДетальныеЗаписи.Номенклатура.Наименование + "-თვის რეზერვი არ შეესაბამება რაოდენობას");	
	КонецЦикла;
	
	Возврат ЕстьОткланения;
...
Процедура ЗапасыПередНачаломИзменения(Элемент, Отказ)
	...
	Если МассивДок.Количество()  Тогда
		Сообщить(НСтр("ru='Данные изменени необходимо провести документ';ka='დოკუმენტის საფუძველზე შექმნილია გადაადგილებები, ცვლილების ოპერაციის განსახორციელებლად გააუქმეთ გადაადგილებები'"));
		Отказ = Истина;
	КонецЕсли;
...
&НаСервере
Функция ИспользоватьФункционалМира()
	
	Возврат УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("СоздатьПеремещениеЗапасовИзЗаказаПокупателя");	
...
Процедура УстановитьВидимостьЭтоПодарок()
	...
	Если СрезУчетнаяПолитикаРегл.Количество() Тогда
		...
		Элементы.ЗапасыКомандыВклВыкл.Видимость = ИспользоватьПодарокВТабличнойЧасти;
...
Процедура СчетНаОплату(Команда)
	...
	Если Объект.Ссылка.Пустая() ИЛИ ЭтаФорма.Модифицированность Тогда
		...
		Обработчик = Новый ОписаниеОповещения("ПродолжитьВыполнениеСчетНаОплату", ЭтотОбъект, Контекст);
	...
	Форма = ПолучитьФорму("Документ.СчетНаОплату.ФормаОбъекта");
	ДанныеФормы = Форма.Объект; 
	СчетНаОплатуНаСервере(ДанныеФормы, МассивВыделеныхСтрок); 
	КопироватьДанныеФормы(ДанныеФормы, Форма.Объект); 
	Форма.Открыть();
...
&НаКлиенте
Процедура ПродолжитьВыполнениеСчетНаОплату(Ответ, Контекст) Экспорт
	...
	ИначеЕсли Ответ = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
	СчетНаОплату(ЭтаФорма.Команды.Найти("СчетНаОплату"));
	
КонецПроцедуры

&НаСервере
Процедура СчетНаОплатуНаСервере(ДанныеФормы, МассивВыделеныхСтрок)
	
	ТЗ_ТоваровСчетНаОплатуСозданныхНаОсновании = ПолучитьТаблицуТоваровСчетНаОплатуСозданныхНаОсновании(); // Kuzmich, 17.12.2019
	...
	Док = ДанныеФормыВЗначение(ДанныеФормы, Тип("ДокументОбъект.СчетНаОплату")); // Получаем объект из данных формы ИЛИ
	...
	МассивУдаляемыхСтрок = Новый Массив;
	Для Каждого СтрокСчета Из Док.Запасы Цикл 
		
		МассивНайденныхСтрок = тзОставляемыхСтрок.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, Партия, ЕдиницаИзмерения", СтрокСчета.Номенклатура, СтрокСчета.Характеристика, СтрокСчета.Партия, СтрокСчета.ЕдиницаИзмерения));
		
		Если МассивНайденныхСтрок.Количество() = 0 Тогда 
			МассивУдаляемыхСтрок.Добавить(СтрокСчета);
		Иначе
			КоличествоУжеСозданных=0;
			МассивТоваровПеремещенийЗапасовСозданныхНаОсновании = ТЗ_ТоваровСчетНаОплатуСозданныхНаОсновании.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, Партия, ЕдиницаИзмерения", СтрокСчета.Номенклатура, СтрокСчета.Характеристика, СтрокСчета.Партия, СтрокСчета.ЕдиницаИзмерения));
			Если МассивТоваровПеремещенийЗапасовСозданныхНаОсновании.Количество()>0 Тогда
				Для каждого СтрМассивТоваровПеремещенийЗапасовСозданныхНаОсновании Из МассивТоваровПеремещенийЗапасовСозданныхНаОсновании Цикл
					КоличествоУжеСозданных = КоличествоУжеСозданных + СтрМассивТоваровПеремещенийЗапасовСозданныхНаОсновании.Количество;
				КонецЦикла; 
			КонецЕсли;
			Если (СтрокСчета.Количество - КоличествоУжеСозданных)<=0 Тогда
				МассивУдаляемыхСтрок.Добавить(СтрокСчета);
			Иначе
				СтрокСчета.Количество = СтрокСчета.Количество - КоличествоУжеСозданных;
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция ПолучитьТаблицуТоваровСчетНаОплатуСозданныхНаОсновании()
	ПЕРЕМ РЕЗ;
	...
	Запрос.Текст = 
		...
		|	СУММА(СчетНаОплатуЗапасы.Количество) КАК Количество
		|ИЗ
		|	Документ.СчетНаОплату.Запасы КАК СчетНаОплатуЗапасы
		|ГДЕ
		|	СчетНаОплатуЗапасы.Ссылка.ДокументОснование = &ДокументОснование
		|	И СчетНаОплатуЗапасы.Ссылка.Проведен
		|
		|СГРУППИРОВАТЬ ПО
		|	СчетНаОплатуЗапасы.Номенклатура,
		|	СчетНаОплатуЗапасы.Характеристика,
		|	СчетНаОплатуЗапасы.Партия,
		|	СчетНаОплатуЗапасы.ЕдиницаИзмерения";
...
Процедура ДобавитьШтрихкодВТабличнуюЧасть(Штрихкод, ВариантРаспознаванияШтрихкода) Экспорт
	...
	Если ВариантРаспознаванияШтрихкода = ПредопределенноеЗначение("Перечисление.ВариантыРаспознованияШтрихкодов.Вес") Тогда
		...
		Иначе
			...
			ЗапасыНоменклатураПриИзменении(Неопределено, НоваяСтрока);
...
Функция РазобратьШтрихкод(Штрихкод, ВариантРаспознаванияШтрихкода)
	...
	ИначеЕсли Лев(Штрихкод, 3) = "200" ИЛИ Лев(Штрихкод, 3) = "220" Тогда  
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			...
			СтруктураРезультата.Вставить("Номенклатура", ВыборкаДетальныеЗаписи.Номенклатура);
			СтруктураРезультата.Вставить("Характеристика", ВыборкаДетальныеЗаписи.Характеристика);
			СтруктураРезультата.Вставить("ЕдиницаИзмерения", ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ЕдиницаИзмерения),ВыборкаДетальныеЗаписи.ЕдиницаИзмерения,ВыборкаДетальныеЗаписи.Номенклатура.ЕдиницаИзмерения));
			СтруктураРезультата.Вставить("Весовая", ВыборкаДетальныеЗаписи.Весовая);
			СтруктураРезультата.Вставить("Вес", Вес);
			СтруктураРезультата.Вставить("Цена", Цена);
