...
Процедура КонтрагентПриИзмененииФрагмент1(Знач ВалютаРасчетовПередИзменением, Знач ДоговорПередИзменением, Знач СтруктураДанные)
    ...
    Если (ЗначениеЗаполнено(Объект.Договор)
        ...
        ПоказатьПредупреждение(Новый ОписаниеОповещения("КонтрагентПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("ВалютаРасчетовПередИзменением, СтруктураДанные", ВалютаРасчетовПередИзменением, СтруктураДанные)), НСтр("ka='შეიცვალა კონტრაგენტთან ხელშეკრულებით გათვალისწინებული ანგარიშსწორების ვალუტა! აუცილებელია დოკუმენტის ვალუტის შემოწმება!';ru='Изменилась валюта расчетов по договору с контрагентом! Необходимо проверить валюту документа!';en='Changed calculation currency of the contract with the contractor! You need to check the currency of the document!'"));
        Возврат;
...
Процедура ДоговорПриИзмененииФрагмент(Знач ВалютаРасчетовПередИзменением, Знач ДоговорПередИзменением, Знач СтруктураДанные)
    ...
    Если (ЗначениеЗаполнено(Объект.Договор)
        ...
        Объект.ВалютаДокумента = СтруктураДанные.ВалютаРасчетов;
        ПоказатьПредупреждение(Новый ОписаниеОповещения("ДоговорПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("ВалютаРасчетовПередИзменением", ВалютаРасчетовПередИзменением)), НСтр("ka='შეიცვალა კონტრაგენტთან ხელშეკრულებით გათვალისწინებული ანგარიშსწორების ვალუტა! აუცილებელია დოკუმენტის ვალუტის შემოწმება!';ru='Изменилась валюта расчетов по договору с контрагентом! Необходимо проверить валюту документа!';en='Changed calculation currency of the contract with the contractor! You need to check the currency of the document!'"));
