...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаСписка",
		Неопределено,
		НСтр("ka='Новости: Заказы покупателей';ru='Новости: Заказы покупателей';en='News: customer orders'"),
		Ложь,
	...
	ОбщегоНазначения.УстановитьФильтрСпискаДокументовПоОсновномуСкладу(Список, "ЗаказПокупателя");
	//--- Дмгг
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ШаблоныПечатиОфисныхДокументов" И НЕ ЗначениеЗаполнено(Элементы.ПодменюКоммерческоеПредложение.Картинка) Тогда
		...
		Элементы.ПодменюКоммерческоеПредложение.Заголовок = Элементы.ПодменюКоммерческоеПредложение.Заголовок + НСтр("ru=' (требуется переоткрыть форму)';ka=' (требуется переоткрыть форму)'");
	КонецЕсли;
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		СтруктураОтвета = УправлениеНебольшойФирмойСобытия.ОбновитьДолгиКонтрагента(Элемент.ТекущиеДанные.Контрагент);
		
		ИнформацияДолгКонтрагента = СтруктураОтвета.ИнформацияДолгКонтрагента;
		ИнформацияНашДолг = СтруктураОтвета.ИнформацияНашДолг;
...
&НаКлиенте
Процедура ОтборРегионОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если Не ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьМеткуИОтборСписка("Регион", Элемент.Родитель.Имя, ВыбранноеЗначение);
	ВыбранноеЗначение = Неопределено;
...
Процедура УстановитьОтборТекущиеДела()
	...
	ИначеЕсли Параметры.Свойство("НеЗавершенные") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='დაუსრულებულები';ru='не завершенные';en='unfinished'");
...
Процедура ЗаписатьДокумент(ДокСсылка)
	
	ДокОбъект = ДокСсылка.ПолучитьОбъект();
	
	ДокОбъект.СостояниеЗаказа = Статус;
	
	Попытка
		ДокОбъект.Записать();
	Исключение
		Сообщить(НСтр("ka='ვერ ჩაიწერა დოკუმენტი!';ru='Документ не записан!';en='The documnet have not been saved!'"));
	КонецПопытки;
