...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаСписка",
		Неопределено,
		НСтр("ka='Новости: Заказы покупателей';ru='Новости: Заказы покупателей'"),
		Ложь,
...
Процедура УстановитьОтборТекущиеДела()
	...
	ИначеЕсли Параметры.Свойство("НеЗавершенные") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='დაუსრულებულები';ru='не завершенные'");
...
Процедура ЗаписатьДокумент(ДокСсылка)
	...
	Попытка
		ДокОбъект.Записать();
	Исключение
		Сообщить(НСтр("ka='ვერ ჩაიწერა დოკუმენტი!';ru='Документ не записан!'"));
	КонецПопытки;
