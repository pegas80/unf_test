...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ka='შეკვეთა განწესი (შექმნა)';ru='Заказ-наряд (Создание)'");
		//ПаганиниRS_
		Объект.НомерТТН = "";
		Объект.IDTТН	= "";
		//_ПаганиниRS
...
Процедура ЗНСтартПриИзменении(Элемент)
	...
	Если Объект.Старт > Объект.Финиш Тогда
		...
		Сообщить(НСтр("ka='სტარტის თარიღი არ შეიძლება, რომ ფინიშის თარიღზე მეტი იყოს.';ru='Дата старта не может быть больше даты финиша.'"));
	Иначе
		ПриИзмененииСтарт = Объект.Старт;
	КонецЕсли;
...
Процедура ЗНФинишПриИзменении(Элемент)
	...
	Если Объект.Финиш < Объект.Старт Тогда
		...
		Сообщить(НСтр("ka='ფინიშის თარიღი არ შეიძლება, რომ სტარტის თარიღზე ნაკლები იყოს.';ru='Дата финиша не может быть меньше даты старта.'"));
	Иначе
		ПриИзмененииФиниш = Объект.Финиш;
	КонецЕсли;
...
Процедура ЗаполнитьПоСпецификации(Команда)
	...
	Если ТекущаяСтрокаТЧ = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='არ არის არჩეული ძირითადი ცხრილის სტრიქონი!';ru='Не выбрана строка основной табличной части!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект, Новый Структура("РезультатПоиска", РезультатПоиска)),
			НСтр("ka='ცხრილი ""მასალები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьМатериалыИзВсехСпецификаций(Команда)
	...
	Если НЕ Объект.Работы.Количество() > 0 Тогда
		...
		Сообщение		= Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='შეავსეთ ცხრილი ""სამუშაოები"".';ru='Заполните табличную часть ""Работы"".'");
	...
	Если Объект.Материалы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьМатериалыИзВсехСпецификацийЗавершение", ЭтотОбъект),
			НСтр("ka='ოპერაციის შესასრულებლად საჭიროა ცხრილ ""მასალები""-ს გასუფთავება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Для выполнения операции требуется очистить табличную часть ""Материалы""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьПоРесурсамДляТекущейРаботы(Команда)
	...
	Если Объект.РесурсыПредприятия.Количество() = 0 Тогда
		...
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ka='ცხრილში ""ამოქმედებული რესურსები"" ჩანაწერები არ არის!';ru='В табличной части ""Задействованные ресурсы"" нет записей!'"));
		Возврат;
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ka='ცხრილი ""შემსრულებლები"" მიმდინარე სამუშაოსთვოს გადავსებული იქნება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Исполнители"" для текущей работы будет перезаполнена! Продолжить выполнение операции?'");
...
Процедура ЗаполнитьПоРесурсамДляВсехРабот(Команда)
	...
	Если Объект.Исполнители.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРесурсамДляВсехРаботЗавершение", ЭтотОбъект),
			НСтр("ka='ცხრილი ""შემსრულებლები"" გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть ""Исполнители"" будет перезаполнена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоПоступлениюВРемонтЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულად გადაივსება საფუძველ-დოკუმენტით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет полностью перезаполнен по документу-основанию! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткам(Команда)
	...
	Если РезультатПоиска.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='ცხრილი ""მასალები"" არ არის შევსებული!';ru='Табличная часть ""Материалы"" не заполнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткамДляВсех(Команда)
	...
	Если Объект.Работы.Количество() > 1 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткамДляВсехЗавершение", ЭтотОбъект),
			НСтр("ka='ცხრილ ""მასალები""-ში სვეტი ""რეზერვი"" გადავსებული იქნება ყველა სამუშაოსთვის! გავაგრძელოთ ოპერაციის შესრულება?';ru='В табличной части ""Материалы"" колонка ""Резерв"" будет перезаполнена для всех работ! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
