...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И ЗавершениеРаботы Тогда
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ka='При закрытии программы все несохраненные изменения будут утеряны.';ru='При закрытии программы все несохраненные изменения будут утеряны.';en='When closing the program, any unsaved changes will be lost.'");
	ИначеЕсли Модифицированность Тогда
		Отказ = Истина;
		Оповещение = Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект);
		ПоказатьВопрос(
		Оповещение, 
		НСтр("ka='Сохранить измерения калькуляции?';ru='Сохранить измерения калькуляции?';en='Save measurement calculation?'"), 
		РежимДиалогаВопрос.ДаНетОтмена, , 
		КодВозвратаДиалога.Да, НСтр("ka='კალკულაცია';ru='Калькуляция';en='calculation'"));
	КонецЕсли; 
...
Процедура КалькуляцияНаФормеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение)=Тип("СправочникСсылка.Номенклатура") Тогда
		...
		КонецЕсли; 
	ИначеЕсли ТипЗнч(ВыбранноеЗначение)=Тип("ПланСчетовСсылка.Управленческий")
		ИЛИ ТипЗнч(ВыбранноеЗначение)=Тип("ПланСчетовСсылка.Хозрасчетный") Тогда // Kuzmich, 07.03.2021, Доработка калькуляции заказа и расчета потребностей
		СтрокаТабличнойЧасти = КалькуляцияНаФорме.Добавить();
		СтрокаТабличнойЧасти.Номенклатура = ВыбранноеЗначение;
		ВнестиИзмененияВКалькуляцию();
...
Процедура КалькуляцияНаФормеСебестоимостьПриИзменении(Элемент)
	...
	КонецЕсли;
	// Kuzmich, 09.02.2021, Доработка калькуляции заказа и расчета потребностей
	Если ДанныеЗаказа.СпособРасчетаСебестоимостиКалькуляции = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСебестоимостиКалькуляции.ЗаполнитьВручную") Тогда
		СтрокаТабличнойЧасти.РучноеИзменение = ИСТИНА;
		
		СтруктураПоиска = Новый Структура("КлючСвязи, Номенклатура, Характеристика, Спецификация, ЕдиницаИзмерения");
		ЗаполнитьЗначенияСвойств(СтруктураПоиска,СтрокаТабличнойЧасти);
		МассивСтрокКалькуляции = ДанныеЗаказа.Калькуляция.НайтиСтроки(СтруктураПоиска);
		Если МассивСтрокКалькуляции.Количество()>0 Тогда
			ЗаполнитьЗначенияСвойств(МассивСтрокКалькуляции[0],СтрокаТабличнойЧасти,"Количество, Себестоимость, СебестоимостьЕдиницы, РучноеИзменение");
		КонецЕсли;
	КонецЕсли;
	// Kuzmich, 09.02.2021, Доработка калькуляции заказа и расчета потребностей
КонецПроцедуры
...
Процедура ДобавитьРасход(Команда)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("РежимВыбора", Истина);
	Если НЕ Получить_Константы_НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа() Тогда // Kuzmich, 07.03.2021, Доработка калькуляции заказа и расчета потребностей
	...
	ОткрытьФорму("ПланСчетов.Хозрасчетный.ФормаВыбора", ПараметрыОткрытия, Элементы.КалькуляцияНаФорме,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);// Kuzmich, 07.03.2021, Доработка калькуляции заказа и расчета потребностей
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	// Kuzmich, 04.02.2021, Доработка калькуляции заказа и расчета потребностей
	ГруппаОтбораПолеСебестоимостьГруппаИ = НовоеУсловноеОформление.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтбораПолеСебестоимостьГруппаИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
	
	Отбор = ГруппаОтбораПолеСебестоимостьГруппаИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("КалькуляцияНаФорме.ТипНоменклатуры");
	Отбор.ПравоеЗначение 	= Перечисления.ТипыНоменклатуры.Запас;
	
	ГруппаОтбораПолеСебестоимостьГруппаНЕ = ГруппаОтбораПолеСебестоимостьГруппаИ.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтбораПолеСебестоимостьГруппаНЕ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаНЕ;
	
	Отбор = ГруппаОтбораПолеСебестоимостьГруппаНЕ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("ДанныеЗаказа.СпособРасчетаСебестоимостиКалькуляции");
	Отбор.ПравоеЗначение 	= Перечисления.СпособыРасчетаСебестоимостиКалькуляции.ЗаполнитьВручную;
	// Kuzmich, 04.02.2021, Доработка калькуляции заказа и расчета потребностей
	...
	// Строки запасов
	Если НЕ Константы.НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа.Получить() Тогда // Kuzmich, 07.03.2021, Доработка калькуляции заказа и расчета потребностей
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	...
	Поле = НовоеУсловноеОформление.Поля.Элементы.Добавить();
	Поле.Использование = Истина;
	// Kuzmich, 04.02.2021, Доработка калькуляции заказа и расчета потребностей, закоментировал
	//Поле.Поле = Новый ПолеКомпоновкиДанных("КалькуляцияНаФормеСебестоимость");
	//Поле = НовоеУсловноеОформление.Поля.Элементы.Добавить();
	//Поле.Использование = Истина;
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.СохранитьИзменения.Заголовок = ?(ТолькоПросмотр, НСтр("ka='დახურვა';ru='Закрыть';en='close'"), НСтр("ka='ჩაწერა და დახურვა';ru='Записать и закрыть';en='Record and Close'"));
	...
	НСтр("ka='Использовать другой шаблон';ru='Использовать другой шаблон';en='Use a different template'"),
	НСтр("ka='Добавить по шаблону';ru='Добавить по шаблону';en='Add a template'"));
...
Процедура ЗаполнитьВидыЦенКалькуляции()
	...
	Элементы.СпособРасчетаСебестоимостиКалькуляции.СписокВыбора.Добавить(Перечисления.СпособыРасчетаСебестоимостиКалькуляции.ПоПоследнейЦенеЗакупки, НСтр("ka='შესყიდვის ბოლო ფასით';ru='последней цене закупки';en='final purchase price'"));
	Элементы.СпособРасчетаСебестоимостиКалькуляции.СписокВыбора.Добавить(Перечисления.СпособыРасчетаСебестоимостиКалькуляции.ЗаполнитьВручную, НСтр("ka='ხელით შევსება';ru='заполнить вручную';en='manual filling'"));// Kuzmich, 04.02.2021, Доработка калькуляции заказа и расчета потребностей
	...
	Пока Выборка.Следующий() Цикл
		Элементы.СпособРасчетаСебестоимостиКалькуляции.СписокВыбора.Добавить(Выборка.Ссылка, НСтр("ka='виду цен ';ru='виду цен '")+""""+СокрЛП(Выборка.Наименование)+"""");	
	КонецЦикла; 
...
Функция ПредставлениеЭлементаВидЦенКонтрагента(ВидыЦенКонтрагентов)
	...
	Результат = НСтр("ka='მომწოდებლების ფასს';ru='цене поставщиков';en='price providers'")+?(ВидыЦенКонтрагентов.Количество()=0, "", " ("+Строка(ВидыЦенКонтрагентов)+")");	
...
Процедура ЗаполнитьСписокИзделий()
	...
	Элементы.ТекущаяСтрокаСостава.СписокВыбора.Добавить(-1, НСтр("ka='Калькуляция всего заказа';ru='Калькуляция всего заказа';en='Calculation of total order'"));
...
Процедура РассчитатьСебестоимость(ТаблицаМатериалов, ВидыЦен, Курс, Кратность, НалогообложениеНДС, Дата, Ссылка, ВыводитьОшибки = Истина)
	...
	Если ВыводитьОшибки Тогда
		Для каждого Стр Из ТаблицаОшибок Цикл
			...
			НСтр("ka='Не удалось определить себестоимость запаса %1';ru='Не удалось определить себестоимость запаса %1';en='Unable to determine the cost of the stock 1%'"),
...
Процедура ВывестиКалькуляциюКлиент(Идентификатор = Неопределено)
	...
	Если НЕ ТолькоПросмотр И Идентификатор<>Неопределено И НЕ СравнитьКалькуляциюИСпецификацию(Идентификатор) Тогда
		...
		ПоказатьВопрос(
		Оповещение, 
		НСтр("ka='Состав спецификации был изменен. Обновить калькуляцию заказа?';ru='Состав спецификации был изменен. Обновить калькуляцию заказа?';en='specification composition was changed. Update the calculation of the order?'"), 
...
Процедура ВывестиКалькуляциюКлиентЗавершение(Ответ, Идентификатор) Экспорт
	...
	Если Ответ=КодВозвратаДиалога.Нет Тогда
		...
		ЗапрещеноИзменениеКалькуляции = ?(Получить_Константы_НоваяСхемаРасчетаПотребностейИКалькуляцииЗаказа(),ЛОЖЬ,ИСТИНА);// Kuzmich, 07.03.2021, Доработка калькуляции заказа и расчета потребностей
	...
	Для каждого СтрМатериала Из СтрокиСпецификаций Цикл
		...
		Для каждого СтрСостава Из СтрокиСостава Цикл
			Если ЗначениеЗаполнено(СтрСостава.Спецификация) И СтрМатериала.ПроверкаЗацикливания.Найти(СтрСостава.Спецификация)<>Неопределено Тогда
				...
				НСтр("ka='Зацикливание сецификации %1 при изготовлении изделия %2';ru='Зацикливание сецификации %1 при изготовлении изделия %2';en='Looping setsifikatsii% 1% in the manufacture of articles 2'"),
...
Процедура ОбновитьСпецификации()
	...
	Для каждого СтрИдентификатор Из ТаблицаИдентификаторов Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрЗапас.Спецификация) 
			...
			ТекстСообщения = НСтр("ka='Создана автоматически из калькуляции';ru='Создана автоматически из калькуляции';en='Created automatically from costing'");
			Если ЗначениеЗаполнено(СтрЗапас.Спецификация) Тогда
				...
				ТекстСообщения = ТекстСообщения+СтрШаблон(НСтр("ka=' на основании спецификации %1';ru=' на основании спецификации %1'"), ТекстСсылки);
			КонецЕсли; 
		...
		Иначе
			...
			ОбсужденияСервер.ДобавитьСообщение(
			НСтр("ka='Обновлен состав по данным калькуляции';ru='Обновлен состав по данным калькуляции';en='Updated composition according costing'"),
...
Функция ПредставлениеЗаказа(Номер, Дата)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1 %2 %3-ის';ru='%1 %2 от %3';en='% 1% 2% 3 from'"),
		НСтр("ka='მყიდველის შეკვეთა';ru='Заказ покупателя';en='Заказ покупателя'"),
...
Процедура ЗагрузитьИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		// Kuzmich, 28.02.2021, Доработка калькуляции заказа и расчета потребностей
		Иначе
			НоваяСтрока.Количество = ?(НоваяСтрока.Количество=0, НоваяСтрока.Количество, 1);
			НоваяСтрока.СебестоимостьЕдиницы = НоваяСтрока.Себестоимость/НоваяСтрока.Количество;
		КонецЕсли;
		// Kuzmich, 28.02.2021, Доработка калькуляции заказа и расчета потребностей
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	Если ЗагрузкаКалькуляции Тогда
		НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='Загрузка калькуляции из файла';ru='Загрузка калькуляции из файла';en='Loading calculation of file'"));
	Иначе
		НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='Загрузка спецификации из файла';ru='Загрузка спецификации из файла';en='Download specification from file'"));
	КонецЕсли; 
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				ИменаСвойств = "Номенклатура, Количество, ЕдиницаИзмерения, Себестоимость";// Kuzmich, 28.02.2021, Доработка калькуляции заказа и расчета потребностей
				Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
				...
				Иначе
					...
					НоваяСтрока.Себестоимость = НоваяСтрока.СебестоимостьЕдиницы*НоваяСтрока.Количество;
				КонецЕсли; 
				КонецЕсли;// Kuzmich, 28.02.2021, Доработка калькуляции заказа и расчета потребностей 
			КонецЕсли;
