...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И ЗавершениеРаботы Тогда
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ka='При закрытии программы все несохраненные изменения будут утеряны.';ru='При закрытии программы все несохраненные изменения будут утеряны.'");
	ИначеЕсли Модифицированность Тогда
		Отказ = Истина;
		Оповещение = Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект);
		ПоказатьВопрос(
		Оповещение, 
		НСтр("ka='Сохранить измерения калькуляции?';ru='Сохранить измерения калькуляции?'"), 
		РежимДиалогаВопрос.ДаНетОтмена, , 
		КодВозвратаДиалога.Да, НСтр("ka='კალკულაცია';ru='Калькуляция'"));
	КонецЕсли; 
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.СохранитьИзменения.Заголовок = ?(ТолькоПросмотр, НСтр("ka='დახურვა';ru='Закрыть'"), НСтр("ka='ჩაწერა და დახურვა';ru='Записать и закрыть'"));
	...
	НСтр("ka='Использовать другой шаблон';ru='Использовать другой шаблон'"),
	НСтр("ka='Добавить по шаблону';ru='Добавить по шаблону'"));
...
Процедура ЗаполнитьВидыЦенКалькуляции()
	...
	Элементы.СпособРасчетаСебестоимостиКалькуляции.СписокВыбора.Добавить(Перечисления.СпособыРасчетаСебестоимостиКалькуляции.ПоПоследнейЦенеЗакупки, НСтр("ka='შესყიდვის ბოლო ფასით';ru='последней цене закупки'"));
...
Функция ПредставлениеЭлементаВидЦенКонтрагента(ВидыЦенКонтрагентов)
	...
	Результат = НСтр("ka='მომწოდებლების ფასს';ru='цене поставщиков'")+?(ВидыЦенКонтрагентов.Количество()=0, "", " ("+Строка(ВидыЦенКонтрагентов)+")");	
...
Процедура ЗаполнитьСписокИзделий()
	...
	Элементы.ТекущаяСтрокаСостава.СписокВыбора.Добавить(-1, НСтр("ka='Калькуляция всего заказа';ru='Калькуляция всего заказа'"));
...
Процедура РассчитатьСебестоимость(ТаблицаМатериалов, ВидыЦен, Курс, Кратность, НалогообложениеНДС, Дата, Ссылка, ВыводитьОшибки = Истина)
	...
	Если ВыводитьОшибки Тогда
		Для каждого Стр Из ТаблицаОшибок Цикл
			...
			НСтр("ka='Не удалось определить себестоимость запаса %1';ru='Не удалось определить себестоимость запаса %1'"),
...
Процедура ВывестиКалькуляциюКлиент(Идентификатор = Неопределено)
	...
	Если НЕ ТолькоПросмотр И Идентификатор<>Неопределено И НЕ СравнитьКалькуляциюИСпецификацию(Идентификатор) Тогда
		...
		ПоказатьВопрос(
		Оповещение, 
		НСтр("ka='Состав спецификации был изменен. Обновить калькуляцию заказа?';ru='Состав спецификации был изменен. Обновить калькуляцию заказа?'"), 
	...
	Для каждого СтрМатериала Из СтрокиСпецификаций Цикл
		...
		Для каждого СтрСостава Из СтрокиСостава Цикл
			Если ЗначениеЗаполнено(СтрСостава.Спецификация) И СтрМатериала.ПроверкаЗацикливания.Найти(СтрСостава.Спецификация)<>Неопределено Тогда
				...
				НСтр("ka='Зацикливание сецификации %1 при изготовлении изделия %2';ru='Зацикливание сецификации %1 при изготовлении изделия %2'"),
...
Процедура ОбновитьСпецификации()
	...
	Для каждого СтрИдентификатор Из ТаблицаИдентификаторов Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрЗапас.Спецификация) 
			...
			ТекстСообщения = НСтр("ka='Создана автоматически из калькуляции';ru='Создана автоматически из калькуляции'");
		...
		Иначе
			...
			ОбсужденияСервер.ДобавитьСообщение(
			НСтр("ka='Обновлен состав по данным калькуляции';ru='Обновлен состав по данным калькуляции'"),
...
Функция ПредставлениеЗаказа(Номер, Дата)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='%1 %2 %3-ის';ru='%1 %2 от %3'"),
		НСтр("ka='მყიდველის შეკვეთა';ru='Заказ покупателя'"),
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	Если ЗагрузкаКалькуляции Тогда
		НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='Загрузка калькуляции из файла';ru='Загрузка калькуляции из файла'"));
	Иначе
		НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ka='Загрузка спецификации из файла';ru='Загрузка спецификации из файла'"));
	КонецЕсли; 
