	...
	Если Параметры.Свойство("СчетЗатрат") Тогда
		...
		Если ЗначениеЗаполнено(Параметры.СчетЗатрат) Тогда
			...
			Если Параметры.СчетЗатрат.ТипСчета <> Перечисления.ТипыСчетов.Расходы
				...
				ТекстСообщения = НСтр("ka='ანგარიშის მოცემული ტიპისთვის მყიდველის შეკვეთა არ მიეთითება!';ru='Для данного типа счета заказ покупателя не указывается!';en='For this type of account the customer order is not specified!'");
