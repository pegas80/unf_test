...
//				НоваяСтрока.ЦенаСкидком = НоваяСтрока.Цена* ТекСтрока.Количество*(1-ТекСтрока.ПроцентСкидки/100);
	...
	//Малышев 2024-12-17
	Если ЗначениеЗаполнено(Организация) и не ЗначениеЗаполнено(ВидНалогаНДС) ТОгда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		...
		Для Каждого СтрокаНоменклатура Из Работы Цикл
			...
			// Олег Литвиненко 18.10.22 --->
			КоличествоНоменклатуры = Окр(СтрокаНоменклатура.Количество * СтрокаНоменклатура.Кратность * СтрокаНоменклатура.Коэффициент - СмещениеДляОкругления, 3);
			// Олег Литвиненко <---
	...
	//++Lazare 19.07.2023
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд и ЗначениеЗаполнено(ссылка) и Проведен Тогда
		Отказ = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ЗапретРедактированияПроведенныхДокументовСервиса");
		Если Отказ Тогда 
			Сообщить(НСтр("ru = 'редактирование документа запрещено!'; ka = 'აკრძალულია გატარებული დოკუმენტის რედაქტირება!'")) ;
		КонецЕсли;
	КонецЕсли;
	//--Lazare 19.07.2023
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если РегистрыСведений.УчетнаяПолитикаРегл.ЗначениеПоляУчетнойПолитики("ВестиУчетВУпрВалюте", Организация,Дата) Тогда
...
//ПМ Для отладки		
		...
		// Закомментировал Олег Литвиненко 17.11.22. Перенесено ниже
		//// BAN+
	...
	УправлениеНебольшойФирмойСервер.ОтразитьАкционныеНоменклатуры(ДополнительныеСвойства, Движения, Отказ);
	...
	Если МногосСкладныйРежим Тогда
	...
	//+++
	Если СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен Или СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.НайтиПоНаименованию("უარი შეკეთებაზე") Тогда //Михаил Бураев 19.04.2024. Бух.проводки только при состоянии документа Завершен
...
//ПМ Для отладки
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд
		...
		И СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен 
		// Олег Литвиненко <---
		И НЕ ОтключитьКонтрольЛимитаЗадолжностиПокупателя Тогда
		РС_ОбщегоНазначения.ПроверкаСуммиИДниЗадолженности(Ссылка, Отказ);
	КонецЕсли;
	
	Если не ОтключитьКонтрольЗадолжностиПокупателя 
		// Олег Литвиненко 16.11.22 --->
		И СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен 
		// Олег Литвиненко <---
	...
	//Если Не ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
	...
	Если ДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара и СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен Тогда 	
...
Процедура ЗаполнитьКолонкуРезервПоОстаткам(ПараметрыОтбора = Неопределено) Экспорт
	...
	Для каждого Строка Из Строки Цикл
		Строка.Резерв = 0;
		Строка.ТипНоменклатурыЗапас = Строка.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас;// + Lazare 16.02.2023
	КонецЦикла;
	...
	|	ТаблицаЗапасы.СтруктурнаяЕдиницаПеремешения КАК СтруктурнаяЕдиницаПеремешения
	...
	|				ВЫБОР
	|					КОГДА &МногоСкладныйРежим
	|						ТОГДА ТаблицаЗапасы.СтруктурнаяЕдиницаПеремешения
	...
	Запрос.УстановитьПараметр("МногоСкладныйРежим", МногосСкладныйРежим);
