...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПустойРеквизитСтатьяВДокументах") Тогда 
		Объект.СтатьяРасходов = Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка();
	КонецЕсли;
	//--- Дмгг
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ka='მარკირების სისტემის საქონლის მარკირება შეიცვალა. ჩავწეროთ?';ru='Маркировка товаров ГИСМ была изменена. Записать?';en='Marking GISM goods has been changed. Record?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ka='მარკირების სისტემით საქონლის მარკირება არ არის ჩაწერილი. ჩავწეროთ?';ru='Маркировка товаров ГИСМ не записана. Записать?';en='Marking of goods GISM not recorded. Record?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ОткрытьПодборСерийныеНомера()
	...
	Если НЕ ЗначениеЗаполнено(Элементы.Товары.ТекущиеДанные.GTIN) Тогда
		ПоказатьПредупреждение(,НСтр("ka='სერიების არჩევამდე მიუთითეთ GTIN';ru='Перед выбором серий укажите GTIN';en='Before selecting a series of select GTIN'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Элементы.Товары.ТекущиеДанные.НоменклатураКиЗ) Тогда
		ПоказатьПредупреждение(,НСтр("ka='სერიების არჩევამდე მიუთითეთ სსნ';ru='Перед выбором серий укажите КИЗ';en='Before selecting a series of select CIC'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
