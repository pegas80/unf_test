...
Процедура ЗаполнитьНазначениеПлатежа()
	...
	Если ПустаяСтрока(ТекстПлатежа)
		...
		Если ТекНомер <> "" Тогда
			ТекстПлатежа = НСтр("ka='გადახდა შემდეგ ანგარიშზე N %НомерСчета%';ru='Оплата по счету N %НомерСчета%'");
...
"%ЗначениеСтавкиНДС% %ТекстНДС%'"
	...
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%ЗначениеСтавкиНДС%", ?(БезНалогаНДС, НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)'"), НСтр("ka='მათ შორის';ru='В т.ч.'")));
