...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Оплата") Тогда
		...
		Элементы.ИдентификаторПлатежа.ПредупреждениеПриРедактировании = НСтр("ka='31.03.2014-მდე გადახდის იდენტიფიკატორი გამოიყენება მხოლოდ ბიუჯეტში გადასახდელად';ru='До 31.03.2014 идентификатор платежа используется только для платежей в бюджет'");
	КонецЕсли;
...
Процедура УстановитьЗаголовокГруппыБюджет(Форма)
	...
	Если Не ПустаяСтрока(Объект.ВидПеречисленияВБюджет) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='გადარიცხვის  სახეობა';ru='Вид перечисления'") + " " + Объект.ВидПеречисленияВБюджет);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='Вид перечисления ...';ru='Вид перечисления ...'"));
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КодОКАТО) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ОКТМО';ru='ОКТМО'") + " " + Объект.КодОКАТО);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='ОКТМО ...';ru='ОКТМО ...'"));
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КодБК) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='КБК';ru='КБК'") + " " + Объект.КодБК);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='КБК ...';ru='КБК ...'"));
	КонецЕсли;
...
Процедура СформироватьНазначениеПлатежа(ИзменениеСуммы = Ложь)
	...
	Иначе
		...
		Иначе
			Если ПустаяСтрока(НазначениеПлатежа)
				...
				НазначениеПлатежа = НСтр("ka='გადახდა შემდეგ ანგარიშზე N %НомерСчета%';ru='Оплата по счету N %НомерСчета%'");
	...
	И НЕ ЭтоПеречислениеВБюджет Тогда
		ТекстНДС = НСтр("ka='დღგ (%СтавкаНДС%) %СуммаНДС%';ru='НДС(%СтавкаНДС%) %СуммаНДС%'");
	...
	И НЕ ЭтоПеречислениеВБюджет Тогда
		ТекстНДС = НСтр("ka='НДС %СуммаНДС%';ru='НДС %СуммаНДС%'");
		ТекстНДС = СтрЗаменить(ТекстНДС, "%СуммаНДС%", Строка(Формат(Объект.СуммаНДС, "ЧЦ=15; ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ=")));
	КонецЕсли;
...
"%ЗначениеСтавкиНДС% %ТекстНДС%';ka='%ტექსტი დანიშნულება% %ტექსტი თანხა% %დღგ-ს განაკვეთის მნიშვნელობა% %დღგ ტექსტი%'");
	...
	"%ТекстСумма%", "
	|Сумма "+ТекстСумма);
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%ЗначениеСтавкиНДС%", ?(БезНалогаНДС ИЛИ (НЕ ЗначениеЗаполнено(Объект.СуммаНДС) И НЕ ЗначениеЗаполнено(Объект.СтавкаНДС)), НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)'"), НСтр("ka='მათ შორის';ru='В т.ч.'")));
