...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Оплата") Тогда
		...
		Элементы.ИдентификаторПлатежа.ПредупреждениеПриРедактировании = НСтр("ka='31.03.2014-მდე გადახდის იდენტიფიკატორი გამოიყენება მხოლოდ ბიუჯეტში გადასახდელად';ru='До 31.03.2014 идентификатор платежа используется только для платежей в бюджет';en='Until 31.03.2014 payment identifier is used only for payments to the budget'");
	КонецЕсли;
...
Процедура УстановитьЗаголовокГруппыБюджет(Форма)
	...
	Если Не ПустаяСтрока(Объект.ВидПеречисленияВБюджет) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='გადარიცხვის  სახეობა';ru='Вид перечисления';en='View listing'") + " " + Объект.ВидПеречисленияВБюджет);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='Вид перечисления ...';ru='Вид перечисления ...';en='View listing ...'"));
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КодОКАТО) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='ОКТМО';ru='ОКТМО';en='OKTMO'") + " " + Объект.КодОКАТО);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='ОКТМО ...';ru='ОКТМО ...';en='OKTMO ...'"));
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КодБК) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ka='КБК';ru='КБК';en='CSC'") + " " + Объект.КодБК);
	Иначе
		ДинамическиеПараметры.Добавить(НСтр("ka='КБК ...';ru='КБК ...';en='CSC ...'"));
	КонецЕсли;
...
Процедура ЗаполнитьПоДокументуИУстановитьДоступность()
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоДокументуИУстановитьДоступностьЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი გასუფთავდება და შეივსება ""საფუძველი""-ს მიხედვით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет очищен и заполнен по ""Основанию""! Продолжить выполнение операции?';en='The document will be cleaned and filled by ""base""! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура СформироватьНазначениеПлатежа(ИзменениеСуммы = Ложь)
	...
	Иначе
		...
		Иначе
			Если ПустаяСтрока(НазначениеПлатежа)
				...
				НазначениеПлатежа = НСтр("ka='გადახდა შემდეგ ანგარიშზე N %НомерСчета%';ru='Оплата по счету N %НомерСчета%';en='Payment of the account N% NomerScheta%'");
	...
	И НЕ ЭтоПеречислениеВБюджет Тогда
		ТекстНДС = НСтр("ka='დღგ (%СтавкаНДС%) %СуммаНДС%';ru='НДС(%СтавкаНДС%) %СуммаНДС%';en='VAT (% StavkaNDS%)%% SummaNDS'");
	...
	И НЕ ЭтоПеречислениеВБюджет Тогда
		ТекстНДС = НСтр("ka='НДС %СуммаНДС%';ru='НДС %СуммаНДС%';en='VAT% SummaNDS%'");
		ТекстНДС = СтрЗаменить(ТекстНДС, "%СуммаНДС%", Строка(Формат(Объект.СуммаНДС, "ЧЦ=15; ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ=")));
	КонецЕсли;
	
	ТекстНазначениеПлатежа = НСтр("ru='%ТекстНазначение% %ТекстСумма%"
"%ЗначениеСтавкиНДС% %ТекстНДС%';ka='%ტექსტი დანიშნულება% %ტექსტი თანხა% %დღგ-ს განაკვეთის მნიშვნელობა% %დღგ ტექსტი%'");
	...
	"%ТекстСумма%", "
	|Сумма "+ТекстСумма);
	ТекстНазначениеПлатежа = СтрЗаменить(ТекстНазначениеПлатежа, "%ЗначениеСтавкиНДС%", ?(БезНалогаНДС ИЛИ (НЕ ЗначениеЗаполнено(Объект.СуммаНДС) И НЕ ЗначениеЗаполнено(Объект.СтавкаНДС)), НСтр("ka='გადასახადის (დღგ) გარეშე';ru='Без налога (НДС)';en='Without tax (VAT)'"), НСтр("ka='მათ შორის';ru='В т.ч.';en='Including'")));
