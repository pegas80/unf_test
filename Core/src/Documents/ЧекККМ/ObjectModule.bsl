...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	Если НЕ ЭтоВводНаОснованииЗаказНаПродажу(ЗаказОснование) Тогда
		ВызватьИсключение НСтр("ka='სსა-ის ჩეკი შევიყვანოთ მხოლოდ მყიდველის შეკვეთის (გასაყიდად) საფუძველზე ან დამოუკიდებლად!';ru='Чек ККМ ввести только на основании заказа покупателя (на продажу) или независимо!'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОплатаПлатежнымиКартами.Количество() > 0 И НЕ ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		...
		ТекстСообщения = НСтр("ka='ველი ""ეკვაირინგის ტერმინალი"" შეუვსებელია';ru='Поле ""Эквайринговый терминал"" не заполнено'");
	...
	Если ОплатаПлатежнымиКартами.Итог("Сумма") > СуммаДокумента Тогда
		...
		ТекстСообщения = НСтр("ka='გადახდის ბარათებით გადახდის თანხა აღემატება დოკუმენტის თანხას';ru='Сумма оплаты платежными картами превышает сумму документа'");
	...
	ТекстСообщения = НСтр("ka='სალაროს ცვლა გაუხსნელია';ru='Кассовая смена не открыта'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Статус = Перечисления.СтатусыЧековККМ.Пробит
	   ...
	   И НЕ КассаККМ.ИспользоватьБезПодключенияОборудования Тогда
		...
		ТекстСообщения = НСтр("ka='სსა-ის ჩეკი ამორტყმულია ფისკალურ რეგისტრატორზე. გატარების გაუქმება შეუძლებელია';ru='Чек ККМ пробит на фискальном регистраторе. Отмена проведения невозможна'");
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения
	   ...
	   Тогда
		...
		ТекстСообщения = НСтр("ka='სალაროს ცვლა დახურულია. გატარების გაუქმება შეუძლებელია';ru='Кассовая смена закрыта. Отмена проведения невозможна'");
