		...
		Если НЕ ЕстьОткрытаяСмена Тогда
			...
			Если ЗаказОснование.СтруктурнаяЕдиницаРезерв.Пустая() Тогда
				Если ИспользуетсяРезервированиеЗапасов Тогда
					ТекстСообщения = НСтр("ru = 'Не открыта ни одна кассовая смена для основного склада (склад можно выбрать в заказе - закладка ""Дополнительно""). Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).'; ka = 'სალარო აპარატის არც ერთი ცვლა არ არის გახსნილი ძირითადი საწყობისთვის (საწყობის შერჩევა შესაძლებელია შეკვეთით - ჩანართი ""დამატებითი""). შეგიძლიათ გახსნათ ცვლა ჟურნალში ""სსა ჩეკები"". ასევე შეგიძლიათ ჩეკის გაცემა მოლარის სამუშაო ადგილზე (მსა).'; en = 'Do not open, no change in cash for the main warehouse (storage can be selected in the order - ""Advanced"" tab). Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Не открыта ни одна кассовая смена для основного склада. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).'; ka = 'სალარო აპარატის არც ერთი ცვლა არ არის გახსნილი მთავარი საწყობისთვის. შეგიძლიათ გახსნათ ცვლა ჟურნალში ""სსა ჩეკები"". ასევე შეგიძლიათ ჩეკის გაცემა მოლარის სამუშაო ადგილზე (მსა).'; en = 'Do not open, no change in cash for the main warehouse. Open the change in magazine ""Checks CMC"". You can also draw a check in the workplace cashier (RCC).'");
				КонецЕсли;
...
Процедура ЗаполнитьПоСборкеЗапасов(ДанныеЗаполнения) Экспорт   
	...
	Если НЕ Константы.СозданиеДокументаЧекКкмНаОснованииДокументаСборкаЗапасов.Получить() Тогда   
	...
	Иначе 
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СборкаЗапасов.Ссылка КАК ОснованиеСсылка,
		|	СборкаЗапасов.Проведен КАК ОснованиеПроведен,
		|	СборкаЗапасов.Организация КАК Организация,
		|	СборкаЗапасов.СтруктурнаяЕдиницаПродукции КАК СтруктурнаяЕдиница,
		|	СборкаЗапасов.СтруктурнаяЕдиницаПродукции.РозничныйВидЦен КАК ВидЦен,
		|	СборкаЗапасов.ВалютаДокумента КАК ВалютаДокумента,
		|	СборкаЗапасов.НалогообложениеНДС КАК НалогообложениеНДС,
		|	СборкаЗапасов.СуммаВключаетНДС КАК СуммаВключаетНДС,
		|	СборкаЗапасов.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
		|	ВЫБОР
		|		КОГДА СборкаЗапасов.ВалютаДокумента = НациональнаяВалюта.Значение
		|			ТОГДА СборкаЗапасов.Курс
		|		ИНАЧЕ КурсыВалютСрезПоследних.Курс
		|	КОНЕЦ КАК Курс,
		|	ВЫБОР
		|		КОГДА СборкаЗапасов.ВалютаДокумента = НациональнаяВалюта.Значение
		|			ТОГДА СборкаЗапасов.Кратность
		|		ИНАЧЕ КурсыВалютСрезПоследних.Кратность
		|	КОНЕЦ КАК Кратность
		|ИЗ
		|	Константа.НациональнаяВалюта КАК НациональнаяВалюта,
		...
		|ГДЕ
		|	СборкаЗапасов.Ссылка = &ДанныеЗаполнения";
		...
		|	СборкаЗапасовПродукция.СерийныеНомера КАК СерийныеНомера,
		|	СУММА(СборкаЗапасовПродукция.Количество) КАК Количество,
		|	СУММА(СборкаЗапасовПродукция.Резерв) КАК Резерв,
		...
		|	СборкаЗапасовПродукция.СерийныеНомера"; 
		...
		Пока Выборка.Следующий() Цикл 	
			...
			СтруктураДанные.Вставить("Номенклатура", Выборка.Номенклатура);
			СтруктураДанные.Вставить("Характеристика", Выборка.Характеристика);
			СтруктураДанные.Вставить("НалогообложениеНДС", НалогообложениеНДС);
			
			Если ЗначениеЗаполнено(ВидЦен) Тогда		
				СтруктураДанные.Вставить("ДатаОбработки", ТекущаяДата());
				СтруктураДанные.Вставить("ВалютаДокумента", ВидЦен.ВалютаЦены);
				...
				СтруктураДанные.Вставить("СтавкаНДС", Выборка.Номенклатура.СтавкаНДС);
			КонецЕсли;
			Цена = УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураДанные); 
			НоваяСтрока.Цена = Цена;   
			НоваяСтрока.Сумма = НоваяСтрока.Цена * НоваяСтрока.Количество;
			СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(Выборка.Номенклатура.СтавкаНДС);	
			НоваяСтрока.СуммаНДС = ?(СуммаВключаетНДС,НоваяСтрока.Сумма
			- (НоваяСтрока.Сумма) / ((СтавкаНДС + 100) / 100),
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("Структура") И
		ТипЗнч(ДанныеЗаполнения) <> Тип("ДокументСсылка.ЗаказПокупателя") и
		ТипЗнч(ДанныеЗаполнения) <> Тип("ДокументСсылка.СборкаЗапасов") Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ЗначениеЗаполнено(Номер) И РежимЗаписи = РежимЗаписиДокумента.Проведение   Тогда 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
	Движения.РезервированиеЗапасов.Очистить();
	Движения.РезервированиеЗапасов.Записать();
	// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
	...
	Если ДополнительныеСвойства.ИспользоватьКонтрольСроковГодностиТовара Тогда 	
		//РС_ОбщегоНазначения.СформироватьТаблицаКонтрольСроковГодностиТовара(Ссылка,ДополнительныеСвойства);
