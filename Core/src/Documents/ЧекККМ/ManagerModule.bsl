#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура СформироватьТаблицаЗапасыПродажа(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
		...
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ISNULL(ТаблицаЗапасы.Проект,Значение(Справочник.Проекты.ПустаяСсылка)) КАК Проект, // Kuzmich, 31.10.2024
		...
		|		ТаблицаЗапасы.Характеристика,
		|		ТаблицаЗапасы.Партия,
		|		ТаблицаЗапасы.Проект, // Kuzmich, 31.10.2024
		...
		|	ТаблицаЗапасы.Партия,
		|	ТаблицаЗапасы.ЗаказПокупателя,
		|	ISNULL(ТаблицаЗапасы.Проект,Значение(Справочник.Проекты.ПустаяСсылка)), // Kuzmich, 31.10.2024
		|	ТаблицаЗапасы.СчетУчетаРегл";
		...
		//|	ЗапасыОстатки.Проект, // Kuzmich, 31.10.2024
		...
		//|		ЗапасыОстатки.Партия КАК Партия,
		//|		ЗапасыОстатки.ЗаказПокупателя КАК ЗаказПокупателя,
		//|		ЗапасыОстатки.Проект КАК Проект, // Kuzmich, 31.10.2024
		...
		//|				(Организация, СтруктурнаяЕдиница, СчетУчетаРегл, Номенклатура, Характеристика, Партия, ЗаказПокупателя, Проект) В // Kuzmich, 31.10.2024
		//|					(ВЫБРАТЬ
		//|						ТаблицаЗапасы.Организация КАК Организация,
		////--------------------------------------------------------------------------
		//// © Шишка В.В. [14 октября 2021]  НАЧАЛО
		////|						ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		//|						ВЫБОР КОГДА &УчетСебестоимостиВРазрезеОрганизации = ИСТИНА ТОГДА НЕОПРЕДЕЛЕНО 	
		//|                       ИНАЧЕ ТаблицаЗапасы.СтруктурнаяЕдиница КОНЕЦ КАК СтруктурнаяЕдиница,
		//// © Шишка В.В. [14 октября 2021]  КОНЕЦ
		////--------------------------------------------------------------------------
		////|						ТаблицаЗапасы.СчетУчетаЗапасов КАК СчетУчета,
		//|						ТаблицаЗапасы.СчетУчетаЗапасовРегл КАК СчетУчетаРегл,
		...
		////|						ТаблицаЗапасы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		//|						ВЫБОР КОГДА &УчетСебестоимостиВРазрезеОрганизации = ИСТИНА ТОГДА НЕОПРЕДЕЛЕНО 	
		//|                       ИНАЧЕ ТаблицаЗапасы.СтруктурнаяЕдиница КОНЕЦ,
		//// © Шишка В.В. [14 октября 2021]  КОНЕЦ
		////--------------------------------------------------------------------------
		////|						ТаблицаЗапасы.СчетУчетаЗапасов,
		//|						ТаблицаЗапасы.СчетУчетаЗапасовРегл,
		...
		//|		ДвиженияДокументаЗапасы.Проект, // Kuzmich, 31.10.2024
		...
		Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
			...
			//	СтруктураДляПоиска.Вставить("Проект", СтрокаТаблицаЗапасы.Проект);  //Малышев 2024-06-04 ONESOFT-3093
			//КонецЕсли;	
			//// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
			СтруктураДляПоиска = УправлениеНебольшойФирмойСервер.ПолучитьСтруктуруДляПоиска(СтрокаТаблицаЗапасы, СтруктураДополнительныеСвойства);
		    //Малышев 2024-12-06 ONESOFT-3670 --)
			...
			Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
				...
				Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
					...
					КоличествоКСписанию = КоличествоТребуетсяСвободныйОстаток; //Малышев 2024-12-06 ONESOFT-3670
					...
					СуммаКСписанию = ?(СтруктураДополнительныеСвойства.РезервированиеБезСебестоимости И СебестоимостьЕдиницы <> 0, Окр(СебестоимостьЕдиницы * КоличествоКСписанию, 2, 1)
					,Окр(СуммаОстаток * КоличествоКСписанию / КоличествоОстаток , 2, 1));//Окр(СуммаОстаток * КоличествоТребуетсяСвободныйОстаток / КоличествоОстаток , 2, 1);
				...
				//ИначеЕсли КоличествоОстаток = КоличествоТребуетсяСвободныйОстаток Тогда
				ИначеЕсли КоличествоОстаток <= КоличествоТребуетсяСвободныйОстаток Тогда  //Малышев 2024-11-19 ONESOFT-3602
				
					КоличествоКСписанию = КоличествоОстаток; //Малышев 2024-12-06 ONESOFT-3670
					//+++ Дмгг, 18.03.25, ONESOFT-3934
					// На одну единицу ложиться вся себестоимость всего товара, а это не правильно так что разделим на пколичество остаток
					//СуммаКСписанию = СуммаОстаток;   
					СуммаКСписанию = СуммаОстаток / СтрокаТаблицыОстатков.КоличествоОстаток * КоличествоКСписанию;      
					//--- Дмгг, 18.03.25, ONESOFT-3934
	...
	//++lazare
	Процедура СформироватьТаблицаКонтрольСроковГодностиТовара(ДокументСсылкаЧекККМ, ТзЗапасы)
		...
		|			И КонтрольСроковГодностиТовараОстатки.СтруктурнаяЕдиница = ВтПродажи.СтруктурнаяЕдиница
		|
		|СГРУППИРОВАТЬ ПО
		...
		Запрос.УстановитьПараметр("Дата",ДокументСсылкаЧекККМ.Дата-1);
		Запрос.УстановитьПараметр("Организация",ДокументСсылкаЧекККМ.Организация);
		Запрос.УстановитьПараметр("СтруктурнаяЕдиница",ДокументСсылкаЧекККМ.СтруктурнаяЕдиница);
...
КонецПроцедуры
	...
	// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
	// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
	Процедура СформироватьТаблицаРезервированиеЗапасов(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
		...
		|						ВременнаяТаблицаЗапасы.ЗаказПокупателя КАК Документ
		|					ИЗ
		|						ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы)) КАК РезервированиеЗапасовОстатки
		|		ПО ТаблицаЗапасы.Организация = РезервированиеЗапасовОстатки.Организация
		|			И ТаблицаЗапасы.СтруктурнаяЕдиница = РезервированиеЗапасовОстатки.СтруктурнаяЕдиница
		|			И ТаблицаЗапасы.ЗаказПокупателя = РезервированиеЗапасовОстатки.Документ
		|			И ТаблицаЗапасы.Номенклатура = РезервированиеЗапасовОстатки.Номенклатура
		|			И ТаблицаЗапасы.Характеристика = РезервированиеЗапасовОстатки.Характеристика
		|			И ТаблицаЗапасы.Партия = РезервированиеЗапасовОстатки.Партия,
		...
		Запрос.УстановитьПараметр("ДатаПроверкиРезервов",  ДокументСсылкаЧекККМ.МоментВремени());
	...
	Процедура ВыполнитьКонтроль(ДокументСсылкаЧекККМ, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
		...
		Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
			...
			ИЛИ (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И СтруктураВременныеТаблицы.ДвиженияРезервированиеЗапасовИзменение И НЕ(УдалениеПроведения)) // Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
			...
			//// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
			Если Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(УдалениеПроведения) Тогда
				
				Запрос.Текст = Запрос.Текст + РезервированиеЗапасов_new.ПолучитьТекстЗапросаКонтроля_РезервированиеЗапасов(ДополнительныеСвойства); //[10]
				//Запрос.Текст = Запрос.Текст + ОбщегоНазначения.РазделительПакетаЗапросов() + РезервированиеЗапасов_new.ПолучитьТекстЗапросаКонтроля_СвободныеОстатки(ДополнительныеСвойства); //[16]			 
				...
				Запрос.УстановитьПараметр("МоментКонтроляРезерва", ДокументСсылкаЧекККМ.МоментВремени());
			...
			Если НЕ МассивРезультатов[0].Пустой()
				// Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
				ИЛИ (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(МассивРезультатов[10].Пустой()) И НЕ(УдалениеПроведения))
				//ИЛИ (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(МассивРезультатов[13].Пустой()) И НЕ(УдалениеПроведения))
                // Kuzmich, 31.10.2024, (ONESOFT-3535) Чек ККМ и РМК - добавить движение расход по новом регистре
			...
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			ИначеЕсли (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(МассивРезультатов[10].Пустой()) И НЕ(УдалениеПроведения)) Тогда
				ВыборкаИзРезультатаЗапроса = МассивРезультатов[10].Выбрать();
				РезервированиеЗапасов_new.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументСсылкаЧекККМ, ВыборкаИзРезультатаЗапроса, Отказ,,,1);
			//ИначеЕсли (Константы.ФункциональнаяОпцияРезервированиеЗапасов_new.Получить() И НЕ(МассивРезультатов[16].Пустой()) И НЕ(УдалениеПроведения)) Тогда
			//	ВыборкаИзРезультатаЗапроса = МассивРезультатов[16].Выбрать();
			//	РезервированиеЗапасов_new.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументСсылкаЧекККМ, ВыборкаИзРезультатаЗапроса, Отказ,,,2);
			// Kuzmich, ONESOFT-2440, Использование резервов - Переместить в хранилище - Исправления
			КонецЕсли;
	...
	Процедура СформироватьТаблицаКонтрольСроковГодностиТовара1(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
	...
	Запрос1.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос1.Текст =
	"ВЫБРАТЬ
	|	КонтрольСроковГодностиТовара.Количество КАК Количество,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	КонтрольСроковГодностиТовара.Дата КАК Период,
	...
	|	КонтрольСроковГодностиТовара.Дата,
	...
	|	ВТВТ.Количество КАК Количество,
	...
	|	ВТВТ.ВидДвижения КАК ВидДвижения
	|ИЗ
	|	ВТВТ КАК ВТВТ
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос1.УстановитьПараметр("Дата",ДокументСсылкаЧекККМ.Дата-2);
	Запрос1.УстановитьПараметр("Организация",ДокументСсылкаЧекККМ.Организация);
	Запрос1.УстановитьПараметр("СтруктурнаяЕдиница",ДокументСсылкаЧекККМ.СтруктурнаяЕдиница);
	Запрос1.УстановитьПараметр("ЧекПробит", СтруктураДополнительныеСвойства.ДляПроведения.ЧекПробит);
	Запрос1.УстановитьПараметр("ТоварЗарезервирован", СтруктураДополнительныеСвойства.ДляПроведения.ТоварЗарезервирован);
	Запрос1.УстановитьПараметр("Архивный", СтруктураДополнительныеСвойства.ДляПроведения.Архивный);
	Запрос1.УстановитьПараметр("ДвиженияПоЗапасамУдалять", СтруктураДополнительныеСвойства.ДляПроведения.ДвиженияПоЗапасамУдалять);
...
//Функция МойЗапрос(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
//	Запрос = Новый Запрос; //ВременнаяТаблицаЗапроса  "ВременнаяТаблицаЗапасы"
//	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
//	Запрос.Текст =
//	"ВЫБРАТЬ
//	|	КонтрольСроковГодностиТовара.Количество КАК Количество,
...
//	
//	Запрос.УстановитьПараметр("Дата",ДокументСсылкаРасходнаяНакладная.Дата-60);
//	Запрос.УстановитьПараметр("Организация",ДокументСсылкаРасходнаяНакладная.Организация);
//	Запрос.УстановитьПараметр("СтруктурнаяЕдиница",ДокументСсылкаРасходнаяНакладная.СтруктурнаяЕдиница);
...
//КонецФункции
	...
	Процедура СформироватьТаблицаПродажиПоПредоставленнымАвтоматическимСкидкам(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
		...
		|	ВременнаяТаблицаЗапасы.СуммаБезСкидки
		|ИЗ
	...
	//
	Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета, ИзРМК = Ложь) Экспорт 
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЧекККМ.ПФ_MXL_Чек1");
		...
		ОбластьQR			= Макет.ПолучитьОбласть("ОбластьQRКод"); //+++ Дмгг
		
		
		Для Каждого ЧекККМ Из МассивОбъектов Цикл
			Если ЗначениеЗаполнено(УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ЗначениеIDДляВебСервисаНесколькихЧековДляРозничнойТорговли")) Тогда 
				
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	""http://queue.ge/"" КАК URL,
				|	ЧекККМ.Дата КАК date,
				|	ЧекККМ.КассаККМ.Код КАК pavilion,
				|	ЧекККМ.НоммерОчереди КАК order,
				|	ЗначениеIDДляВебСервисаНесколькихЧековДляРозничнойТорговли.Значение КАК id
				|ИЗ
				|	Документ.ЧекККМ КАК ЧекККМ,
				|	Константа.ЗначениеIDДляВебСервисаНесколькихЧековДляРозничнойТорговли КАК ЗначениеIDДляВебСервисаНесколькихЧековДляРозничнойТорговли
				|ГДЕ
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					URL = ВыборкаДетальныеЗаписи.URL;
					date = Формат(ВыборкаДетальныеЗаписи.date, "ДФ=dd.MM.yyyy");
					pavilion = ВыборкаДетальныеЗаписи.pavilion;					
					order = ВыборкаДетальныеЗаписи.order;
					id = ВыборкаДетальныеЗаписи.id;
					
					СтрокаДляMD5 = date + pavilion + order + id;
					
				КонецЦикла;
				
				СтрокаMD5 = ВычислитьMD5(СтрокаДляMD5);
				
				ТекстСсылки = URL + СтрокаMD5;
				
				ВывестиQRКод(ТекстСсылки ,ОбластьQR);
				ТабличныйДокумент.Вывести(ОбластьQR);
				
			КонецЕсли;
			//--- Дмгг

			
			ЯвляетсяЧеком = ЕстьЧекККМ(ЧекККМ);
			
			ОчередьНаКассе = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ОчередьНаКассе");
			Если ОчередьНаКассе И ЯвляетсяЧеком Тогда
				НомерДокумента = ЧекККМ.НоммерОчереди;
			Иначе
			...
			ОбластьШапка.Параметры.ДатаЗаголовок	= Формат(ЧекККМ.Дата, "Л=ka; ДФ=' dd MMMM yyyy წ.'");
			ОбластьШапка.Параметры.Заголовок	= "ჩეკი № " +НомерДокумента;	
			...
			Если ЯвляетсяЧеком Тогда 
				...
				|ВЫБРАТЬ
				|	ВЫБОР
				|		КОГДА НаименованиеДляПечатиНоменклатуриВпечатныхФормах.Значение
				|			ТОГДА ВТ_Запасы.Номенклатура.НаименованиеПолное
				|		ИНАЧЕ ВТ_Запасы.Номенклатура.Наименование
				...
				|	Константа.НаименованиеДляПечатиНоменклатуриВпечатныхФормах КАК НаименованиеДляПечатиНоменклатуриВпечатныхФормах";
			...
			Иначе 
				...
				Для Каждого стр Из ЧекККМ.Запасы Цикл 
		...
		////			
		////			ТекстОшибки = НСтр("ru = 'В одной из строк не заполнено значение номенклатуры - строка при печати пропущена!'; ka = 'ერთ-ერთი ხაზი არ შეიცავს ნივთის მნიშვნელობას - ხაზი გამოტოვებულია დაბეჭდვისას!'; en = 'In one of the lines are not filled with the value of the nomenclature - line printing missed!'");
		...
		////КонецЕсли;
		//+++ Дмгг
		Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ДублироватьПечатьЧекаККМ") Тогда 
			
			ТекТабличныйДокумент = ТабличныйДокумент;
	...
	Функция ЕстьЧекККМ(Документ)
		
		Возврат ТипЗнч(Документ) = Тип("ДокументСсылка.ЧекККМ")
	...
	// Функция формирует табличный документ обложки кассовой книги
	//
	Функция НапечатьтатьФормыДляКухниНапитковДесертов(МассивОбъектов, ОбъектыПечати, ИмяМакета, ИзРМК = Ложь, Отдел)
		Перем ПервыйДокумент, НомерСтрокиНачало, Ошибки;
		
		ТабличныйДокумент = Новый ТабличныйДокумент;
		ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЧекККМ_ТоварныйЧек_"+Отдел;
		////Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЧекККМ.ПФ_MXL_ТоварныйЧек");
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЧекККМ.ПФ_MXL_Чек_Отделы");
		//ДанныеПечати = Новый Структура;
		
		ОбластьШапка				= Макет.ПолучитьОбласть("Шапка");
		ОбластьШапкаТаблицы 		= Макет.ПолучитьОбласть("ШапкаТаблицыТовара");
		//ОбластьСтрока				= Макет.ПолучитьОбласть("Строка");
		//ОбластьПодвал				= Макет.ПолучитьОбласть("Подвал");
		ОбластьШапкаТаблицыСостава	= Макет.ПолучитьОбласть("ШапкаТаблицыСостава");
		...
		Для Каждого ЧекККМ Из МассивОбъектов Цикл
			...
			|ГДЕ
			|	ЧекККМЗапасы.Ссылка = &Ссылка
			|	И ЧекККМЗапасы.Номенклатура.НаправлениеДляЧекаККМ = &Отдел
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|		ПО ЧекККМЗапасы.Номенклатура = СпецификацииСостав.Ссылка.Владелец
			|ГДЕ
			|	ЧекККМЗапасы.Ссылка = &Ссылка
			|	И ЧекККМЗапасы.Номенклатура.НаправлениеДляЧекаККМ = &Отдел";
			...
			тзЗапасы = МассивРезультатов[0].Выгрузить();
			тзШапка = МассивРезультатов[1].Выгрузить();
			тзСостав = МассивРезультатов[2].Выгрузить();
			
			Если тзЗапасы.Количество() = 0 Тогда 
				ТабличныйДокумент = Неопределено;
				Прервать; ///В идеале из РМК приходит только один документ
			КонецЕсли;
			
			СтрокаШапки = тзШапка[0];
			Если Отдел = Перечисления.ВидыЧековДляНоменклатурыККМ.Кухня Тогда 
				ОбластьШапка.Параметры.НомерПечатнойФормы = НСтр("ru = 'Печатная форма #1'; en = 'Printable form #1'; ka = 'საბეჭდი ფორმა #1'");
			Иначе
				ОбластьШапка.Параметры.НомерПечатнойФормы = НСтр("ru = 'Печатная форма #2'; en = 'Printable form #2'; ka = 'საბეჭდი ფორმა #2'");
			КонецЕсли;
			
			ОбластьШапка.Параметры.НаправлениеПечатнойФормы = СтрокаШапки.Отдел;
			ОбластьШапка.Параметры.ДатаВремя 				= Формат(СтрокаШапки.Дата, "ДФ='MM/dd/yy hh:mm'");;
			ОбластьШапка.Параметры.НомерЧека 				= СтрокаШапки.Номер;
			...
			Для Каждого Строк Из тзЗапасы Цикл 
				...
				ОбластьСтрока						= Макет.ПолучитьОбласть("Строка");
				ОбластьСтрока.Параметры.Номенклатура= Строк.Номенклатура;
				ОбластьСтрока.Параметры.Колво   	= Строк.Количество;
				ТабличныйДокумент.Вывести(ОбластьСтрока);
				ИтогПоТоварам = ИтогПоТоварам + Строк.Количество;
				
			КонецЦикла;
			
			ОбластьСтрока						= Макет.ПолучитьОбласть("Строка");
			ОбластьСтрока.Параметры.Номенклатура= НСтр("ru = 'Итого'; en = 'Total'; ka = 'სულ'");
			ОбластьСтрока.Параметры.Колво   	= ИтогПоТоварам;
			ТабличныйДокумент.Вывести(ОбластьСтрока);
			
			ТабличныйДокумент.Вывести(ОбластьПустаяСтрока);
			
			ТабличныйДокумент.Вывести(ОбластьШапкаТаблицыСостава);
			
			Для Каждого СтрокСотсав Из тзСостав Цикл 
				
				ОбластьШапкаТаблицыСостава							= Макет.ПолучитьОбласть("Строка");
				ОбластьШапкаТаблицыСостава.Параметры.Номенклатура	= СтрокСотсав.Номенклатура;
				ОбластьШапкаТаблицыСостава.Параметры.Колво   		= Строка(СтрокСотсав.Количество) + " " + Строка(СтрокСотсав.ЕдиницаИзмерения);
				ТабличныйДокумент.Вывести(ОбластьШапкаТаблицыСостава);
				
			КонецЦикла;
			
			ОбластьЗакрытиеТаблицы = Макет.ПолучитьОбласть("ЗакрытиеТаблицы");
			ТабличныйДокумент.Вывести(ОбластьЗакрытиеТаблицы);
	...
	//
	Функция Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода, ИзРМК = Ложь, Отдел = Неопределено) Экспорт
		
		//Паганини_
		//Если ИзРМК Тогда //+++ Дмгг
		Если ИзРМК И Отдел = Неопределено Тогда
			Возврат ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "Товарный чек", ИзРМК);
			
			//+++ Дмгг	 
		ИначеЕсли ИзРМК И НЕ Отдел = Неопределено Тогда 
			
			Возврат НапечатьтатьФормыДляКухниНапитковДесертов(МассивОбъектов, ОбъектыПечати, "Товарный чек", ИзРМК, Отдел);
			//--- Дмгг
		КонецЕсли;//_Паганини
