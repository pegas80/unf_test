...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если ВводДоступен() Тогда
		...
		Если Результат = Неопределено Тогда
			ТекстСообщения = НСтр("ka='გარე მოვლენის დამუშავებისას მოწყობილობიდან შეცდომა მოხდა: ""%ОписаниеОшибки%"".';ru='При обработке внешнего события от устройства произошла ошибка:""%ОписаниеОшибки%"".'");
...
Процедура ЗакрытьФормуПриСканировании()
	...
	Если НЕ ШтрихкодСоответствуетШаблону(КодАкцизнойМарки) Тогда
		...
		ТекстСообщения = НСтр("ka='შტრიხკოდი არ შეესაბამება ფორმატს.';ru='Штрихкод не соответствует формату.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	Если ШтрихкодМаркиПрисутствуетВЧеке() Тогда
		ТекстСообщения = НСтр("ka='მარკის შტრიხკოდი უკვე არის ჩეკში.';ru='Штрихкод марки уже присутствует в чеке.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
