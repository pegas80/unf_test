...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если ВводДоступен() Тогда
		...
		Если Результат = Неопределено Тогда
			ТекстСообщения = НСтр("ka='გარე მოვლენის დამუშავებისას მოწყობილობიდან შეცდომა მოხდა: ""%ОписаниеОшибки%"".';ru='При обработке внешнего события от устройства произошла ошибка:""%ОписаниеОшибки%"".';en='In the treatment of external events on the device error occurred: ""% OpisanieOshibki%.""'");
...
Процедура ЗакрытьФормуПриСканировании()
	...
	Если НЕ ШтрихкодСоответствуетШаблону(КодАкцизнойМарки) Тогда
		...
		ТекстСообщения = НСтр("ka='შტრიხკოდი არ შეესაბამება ფორმატს.';ru='Штрихкод не соответствует формату.';en='The barcode does not match the format.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	Если ШтрихкодМаркиПрисутствуетВЧеке() Тогда
		ТекстСообщения = НСтр("ka='მარკის შტრიხკოდი უკვე არის ჩეკში.';ru='Штрихкод марки уже присутствует в чеке.';en='Barcode brand is already present in the check.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
