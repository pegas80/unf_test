...
Процедура СформироватьПодсказкиКРеквизитам()
	...
	ЗаголовокСуммаЧека = НСтр("ka='ჩეკის თანხა (%Валюта%)';ru='Сумма чека (%Валюта%)'");
	...
	ЗаголовокПолученоНаличными = НСтр("ka='მიღებულია ნაღდი ფულით (%Валюта%)';ru='Получено наличными (%Валюта%)'");
	...
	ЗаголовокОплатаПлатежнымиКартами = НСтр("ka='საგადახდო ბარათებით (%Валюта%)';ru='Платежными картами (%Валюта%)'");
	...
	ЗаголовокСуммаСдачи = НСтр("ka='ხურდა (%Валюта%)';ru='Сдача (%Валюта%)'");
	...
	Если Объект.СуммаДокумента < Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		Если Элементы.Зарезервировать.Видимость И НЕ Элементы.РасчитатьСуммуНаличнойОплаты.Доступность Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + 
				НСтр("ka='გადახდის თანხის შეყვანამდე, უნდა დავაჭიროთ ღილაკ %1-ს.';ru='Прежде чем вводить сумму оплаты, требуется нажать кнопку %1.'");
			Если РезервироватьЗапасыИИспользоватьЗаказы Тогда	
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", НСтр("ka='""Провести""';ru='""Провести""'"));
			Иначе
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", НСтр("ka='""Зарезервировать""';ru='""Зарезервировать""'"));
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьВалютуДокументаОснования(БратьКурсНаТекущуюДату = Ложь)
	...
	Если НЕ Объект.Заказ.Пустая() Тогда
		Если Объект.ВалютаДокумента <> Объект.Заказ.ВалютаДокумента Тогда
			...
			ТекстСообщения = НСтр("ka='საფუძველ-დოკუმენტის ვალუტა (%1) განსხვავდება სსა-ის ჩეკის ვალუტისგან (%2)! თანხები გადათვლილი იქნა!';ru='Валюта документа-основания (%1) отличается от валюты чека ККМ (%2)! Суммы были пересчитаны!'");
...
Процедура УстановитьРежимТолькоПросмотр()
	...
	Если КассаККМИспользоватьБезПодключенияОборудования Тогда
		Элементы.ПробитьЧек.Заголовок = НСтр("ka='ჩეკის გაუქმება';ru='Отменить пробитие'");
...
Процедура ОтменитьРежимТолькоПросмотр()
	...
	Элементы.ПробитьЧек.Заголовок = НСтр("ka='ჩეკის ამორტყმა';ru='Пробить чек'");
	ТолькоПросмотр = Ложь;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РезервироватьЗапасыИИспользоватьЗаказы Тогда
		Элементы.Зарезервировать.Заголовок = НСтр("ka='გატარება';ru='Провести'");
		Элементы.СнятьРезервирование.Заголовок = НСтр("ka='გატარების გაუქმება';ru='Отмена проведения'");
		Элементы.СнятьРезервирование.ТолькоВоВсехДействиях = Истина;
	Иначе
		Элементы.Зарезервировать.Заголовок = НСтр("ka='დარეზერვება';ru='Зарезервировать'");
		Элементы.СнятьРезервирование.Заголовок = НСтр("ka='რეზერვის მოხსნა';ru='Снять резерв'");
...
Процедура ДобавитьОплатуКартой(Команда)
	...
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтотОбъект, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='საგადახდო ტერმინალით გადახდა ვერ შესრულდა';ru='Не удалось выполнить оплату через платежный терминал'"));
		Возврат;
	КонецЕсли;
...
Процедура ОперацияПоПлатежнойКартеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ka='При выполнении операции произошла ошибка:""%ОписаниеОшибки%"".';ru='При выполнении операции произошла ошибка:""%ОписаниеОшибки%"".'");
...
Процедура УдалитьОплатуКартой(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აირჩიეთ წასაშლელი ბარათით გადახდის სტრიქონი';ru='Выберите строку удаляемой оплаты картой.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтотОбъект, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ka='ბარათით გადახდის გაუქმება ვერ მოხერხდა.';ru='Не удалось выполнить отмену оплаты картой.'"));
		Возврат;
	КонецЕсли;
...
Процедура КассаККМПриИзменении(Элемент)
	...
	Если НЕ СменаОткрыта Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ka='სალაროს ცვლა არ არის გახსნილი. ცვლის გახსნა შეიძლება ჟურნალ ""სსა-ის ჩეკები""-ში. ცვლის გახსნის შემდეგ, მიმდინარე დოკუმენტში ხელახლა აირჩიეთ სსა სალარო.';ru='Кассовая смена не открыта. Открыть смену можно в журнале ""Чеки ККМ"". После открытия смены перевыберите кассу ККМ в текущем документе.'");
...
Функция ФормаОплатыПриИзмененииНаСервере(ОписаниеОшибки = "")
	...
	Если Объект.ФормаОплаты = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		...
		Если Объект.ОплатаПлатежнымиКартами.Количество() > 0 Тогда
			...
			ОписаниеОшибки = НСтр("ka='ჩატარებულია გადახდა საგადახდო ბარათებით! გადახდის ფორმად ""ნაღდი ფულით"" დაყენება შეუძლებელია';ru='Проведена оплата платежными картами! Установить форму оплаты ""Наличными"" невозможно'");
			Возврат Ложь;
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если НЕ Объект.Заказ.Пустая() Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещения, 
			НСтр("ka='შევავსოთ დოკუმენტი მყიდველის მიერ არჩეული შეკვეთით?';ru='Заполнить документ по выбранному заказу покупателя?'"),
