...
Процедура СформироватьЗаголовок(СтруктураСостояниеКассовойСмены)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ka='%ТорговаяТочка%, ცვლა № %НомерСмены%  %ВремяИзменения%';ru='%ТорговаяТочка%, Смена № %НомерСмены%  %ВремяИзменения%'");
	...
	Иначе
		ТекстСообщения = НСтр("ka='%ТорговаяТочка%';ru='%ТорговаяТочка%'");
...
Процедура ПробитьЧек(СформироватьТоварныйЧек = Ложь)
	...
	И НЕ КассаККМИспользоватьБезПодключенияОборудования Тогда
		ТекстСообщения = НСтр("ka='ჩეკი უკვე ამორტყმულია ფისკალურ რეგისტრატორზე!';ru='Чек уже пробит на фискальном регистраторе!'");
...
Процедура ПробитьЧекЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Исключение
			ЗаполнитьОписаниеДляТЧЗапасыНаКлиенте();
			ПоказатьПредупреждение(Неопределено, НСтр("ka='დოკუმენტის გატარება ვერ მოხერხდა';ru='Не удалось выполнить проведение документа'")); // Асинхронный метод!
			Возврат;
		КонецПопытки;
...
Процедура ВнесениеДенег(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ЗаголовокОкна = НСтр("ka='შეტანის თანხა, %Валюта%';ru='Сумма внесения, %Валюта%'");
...
Процедура ВыемкаДенег(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ЗаголовокОкна = НСтр("ka='ამოღების თანხა, %Валюта%';ru='Сумма выемки, %Валюта%'");
		ЗаголовокОкна = СтрЗаменить(
			ЗаголовокОкна,
			"%Валюта%",
			СтруктураСостояниеКассовойСмены.ВалютаДокументаПредставление
...
Процедура ПробитьЧекВыполнить(Команда, СформироватьТоварныйЧек = Ложь)
	...
	Если Объект.ПометкаУдаления Тогда
		...
		ТекстОшибки = НСтр("ka='დოკუმენტი მონიშნულია წასაშლელად';ru='Документ помечен на удаление'");
	...
	Если Объект.СуммаДокумента > Объект.ПолученоНаличными + Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		ТекстОшибки = НСтр("ka='გადახდის თანხა ჩეკის თანხაზე ნაკლებია';ru='Сумма оплаты меньше суммы чека'");
	...
	Если Объект.СуммаДокумента < Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		ТекстОшибки = НСтр("ka='გადახდის ბარათებით გადახდის თანხა აღემატება ჩეკის თანხას';ru='Сумма оплаты платежными картами превышает сумму чека'");
...
Процедура ЗакрытьКассовуюСмену(Команда)
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		ТекстОшибки = НСтр("ka='აირჩიეთ სსა სალარო!';ru='Выберите кассу ККМ!'");
...
Процедура АннулироватьЧекККМ(КассаККМ)
	...
	Иначе
		ТекстСообщения = НСтр("ka='ფისკალური რეგისტრატორი არ არის არჩეული.';ru='Не выбран фискальный регистратор.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПечатьКопииНаФискальномРегистраторе(Команда)
	...
	Иначе
		ТекстСообщения = НСтр("ka='უკანასკნელი სლიპ ჩეკი არ არის.';ru='Отсутствует последний слип чек.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='მომხმარებლისთვის არ არის განსაზღვრული სსა სალარო!';ru='Для пользователя не определена Касса ККМ!'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьВидимостьНадписиИстеклоБолее24Часов(СтруктураСостояниеКассовойСмены = Неопределено)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ka='სალაროს ცვლა გახსნილია';ru='Кассовая смена открыта'");
...
Процедура Оплатить(Команда)
	...
	Если Объект.Запасы.Итог("Всего") <= 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОплатитьОбработкаОтветаНаВопрос", ЭтотОбъект, СуммаДоРасчетаАвтоСкидок),
			НСтр("ka='გადასახდელი თანხა = 0! დარწმუნებული ხართ, რომ საჭიროა გაგრძელება?';ru='Сумма к оплате = 0! Уверены, что нужно продолжить?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			КодВозвратаДиалога.Нет,
			НСтр("ka='მოქმედების დადასტურების მოთხოვნა';ru='Запрос о подтверждении действия'"));
	Иначе
...
Функция ЗарезервироватьНаСервере(ОтменаРезервирования = Ложь)
	...
	Иначе
		...
		Исключение
			...
			Если СтрНайти(ОписаниеОшибки, НСтр("ka='გატარება ვერ მოხერხდა';ru='Не удалось провести'")) = 0 Тогда
				Сообщение = Новый СообщениеПользователю;
...
Процедура ЧекНеПредъявленПриИзмененииНаСервере()
	...
	Если ЧекНеПредъявлен Тогда
		...
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСТР("ka='შექმენით შემოსავლის ზედდებული და სგო';ru='Создайте приходную накладную и РКО'");
	...
	Иначе
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСТР("ka='აირჩიეთ დაბრუნების საფუძველი';ru='Выберите основание для возврата'");
...
Процедура ЗаполнитьСоставЧекаИСсылкиНаДокументыНаСервере(ЧекККМ)
	...
	Если ЧекККМ.КассоваяСмена.КассоваяСмена.Статус = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Элементы.ДекорацияЧекККМНаВозврат.Заголовок = НСТР("ka='დასაბრუნებელი ჩეკი №';ru='Чек ККМ на возврат №'") + НомерДокумента + НСТР("ru = ' от '") + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			Элементы.СоздатьЧекККМНаВозврат.ЦветТекста = WebЦвета.Серый;
		Иначе
		...
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСТР("ka='აირჩიეთ საფუძველი დასაბრუნებლად';ru='Выберите основание для возврата'");
	Иначе
		...
		Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияПриходнаяНакладная.Заголовок = НСТР("ka='შემოსავლის ზედდებული №';ru='Прих. нак-я №'")+СокрЛП(НомерДокумента)+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияРКО.Заголовок = НСТР("ka='სგო №';ru='РКО №'")+НомерДокумента+НСТР("ru = ' от '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				Элементы.СоздатьРКОНаОснованииПриходнойНакладной.ЦветТекста = WebЦвета.Серый;
...
Процедура ОткрытьИнформациюОСкидкахКлиент()
	...
	Если НЕ Объект.СкидкиРассчитаны Тогда
		ТекстВопроса = НСтр("ka='ფასდაკლებები არ არის გათვალისწინებული, გავითვალისწინოთ?';ru='Скидки (наценки) не рассчитаны, рассчитать?'");
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	ПоказатьОповещениеПользователя(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='თვლადი დისკონტური რუკა%1';ru='Считана дисконтная карта %1'"), ДисконтнаяКарта),
		БиблиотекаКартинок.Информация32);
...
Процедура ВыбранаДисконтнаяКартаДополнительно(ДисконтнаяКарта)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		Текст = НСтр("ka='ხელახლა შეივსოს ფასდაკლებები ყველა სტრიქონში?';ru='Перезаполнить скидки во всех строках?'");
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ПоказатьВводЗначения(ОписаниеОповещения, ТекШтрихкод, НСтр("ka='შეიყვანეთ შტრიხკოდი';ru='Введите штрихкод'"));
...
Процедура УстановитьПериодНаСервере(ВидПериодаРМК, ИмяСписка, ПараметрСтандартныйПериод = Неопределено)
	...
	Если ВидПериодаРМК = ЗаТекущуюСмену Тогда
		...
		Если ИмяСписка = "Список" Тогда
			...
			ВидПериодаЖурнала = НСТР("ka='მიმდინარე ცვლის მანძილზე';ru='За текущую смену'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='მიმდინარე ცვლის მანძილზე';ru='За текущую смену'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='მიმდინარე ცვლის მანძილზე';ru='За текущую смену'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаПрошлуюСмену Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
			ВидПериодаЖурнала = НСТР("ka='გასული ცვლის მანძილზე';ru='За прошлую смену'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='გასული ცვლის მანძილზე';ru='За прошлую смену'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='გასული ცვლის მანძილზე';ru='За прошлую смену'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаВчерашнийДень Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
			ВидПериодаЖурнала = НСТР("ka='გუშინდელი დღისთვის';ru='За вчерашний день'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='გუშინდელი დღისთვის';ru='За вчерашний день'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='გუშინდელი დღისთვის';ru='За вчерашний день'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаВесьПериод Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Истина;
			ВидПериодаЖурнала = НСТР("ka='მთელი პერიოდის მანძილზე';ru='За всё время'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСТР("ka='მთელი პერიოდის მანძილზე';ru='За всё время'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСТР("ka='მთელი პერიოდის მანძილზე';ru='За всё время'");
		КонецЕсли;
...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='მყიდველთა შეკვეთები';ru='Заказы покупателей'");
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ПометкаУдаления", Ложь);
	
	РаботаСОтборами.СоздатьЭлементыМеток(ЭтотОбъект, "Закрыт", "ГруппаОтборЗакрыт", Ложь, НСтр("ka='დასახურად';ru='Не закрытые'"));
	...
	Если Параметры.Свойство("ПросроченоВыполнение") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='შესრულება ვადაგადაცილებულია';ru='просрочено выполнение'");
	...
	ИначеЕсли Параметры.Свойство("ПросроченаОплата") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='გადახდა ვადაგადაცილებულია';ru='просрочена оплата'");
	...
	ИначеЕсли Параметры.Свойство("НаСегодня") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='დღეისთვის';ru='на сегодня'");
	...
	ИначеЕсли Параметры.Свойство("ВРаботе") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='სამუშაოში';ru='в работе'");
	...
	ИначеЕсли Параметры.Свойство("Новые") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ka='ახლები';ru='новые'");
...
Процедура ВыполнитьСверкуИтоговЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ka='Операция выполнена успешно.';ru='Операция выполнена успешно.'");
	Иначе
