...
Функция ПолучитьПараметрыЗаполненияКорректировочногоСчетаФактуры(ДокументОснование) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОснованиеДляВидаОперации) Тогда
		ВызватьИсключение НСтр("ka='გასასწორებელი დოკუმენტი არ არის დადგენილი';ru='Не определен исправляемый документ'");
	КонецЕсли;
...
"Для документа %1 печатная форма не сформирована.'");
		...
		Если Шапка.ВидОперации <> Перечисления.ВидыОперацийСчетФактура.Аванс 
			...
			Если Шапка.ОнЖе Тогда
				...
				ОбластьМакета.Параметры.ПредставлениеГрузоотправителя = ЗаголовокПоля + НСтр("ka='იგივე';ru='он же'");
				...
				Тогда
					...
					Тогда
						...
						ОбластьМакета.Параметры.Основание = 
							Шапка.ОснованиеПечати + НСтр("ka='; მინდობილობით #';ru='; по доверенности №'") + Шапка.ДокументОснование.ДоверенностьНомер 
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйКорректировочныйДокумент = Ложь) Экспорт
	...
	КоличествоЛистов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ka='დოკუმენტი შედგენილია %1%2 %3-ისთვის';ru='Документ составлен на%1%2 %3'"), Символы.ПС, НомерЛиста,
		УправлениеНебольшойФирмойСервер.ФормаМножественногоЧисла(
			НСтр("ka='ფურცელს';ru='листе'"), НСтр("ka='გვერდებზე';ru='листах'"), НСтр("ka='გვერდებზე';ru='листах'"), НомерЛиста));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='უნივერსალური საკორექტირებო დოკუმენტი';ru='Универсальный корректировочный документ'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
