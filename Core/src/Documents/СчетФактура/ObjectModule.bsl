		...
		Если ТипЗнч(ТекСтрока.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			...
			Если УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйСчетФактуру(РеквизитыОснования.ДокументОснование) = Неопределено Тогда
				Если РеквизитыОснования.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки Тогда
					ВызватьИсключение НСтр("ka='ანგარიშფაქტურის გასწორების დამატებამდე საფუძველ-დოკუმენტზე შექმენით ანგარიშფაქტურა';ru='Перед добавлением исправления счета-фактуры создайте счет-фактуру у документа-основания'");
				Иначе
					ВызватьИсключение НСтр("ka='საკორექტირებო ანგარიშფაქტურის დამატებამდე დოკუმენტ-საფუძველზე შექმენით ანგარიშფაქტურა';ru='Перед добавлением корректировочного счета-фактуры создайте счет-фактуру у документа-основания'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьПоПрочимДокументам(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя")
		...
		ВызватьИсключение НСтр("ka='ანგარიშფაქტურის შეყვანა შეიძლება მხოლოდ შეკვეთა-განწესის საფუძველზე!';ru='Счет-фактуру можно ввести только на основании заказ-наряда!'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
		...
		Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
			...
			Для каждого СтрокаТаблицыЗапасов Из Запасы Цикл
				...
				Если ЗначениеЗаполнено(СтрокаТаблицыЗапасов.НомерГТД)
					...
					ТекстОшибки = НСтр("ka='სტრიქონ [%НомерСтрокиСОшибкой%] -ში სწორად არ არის მითითებული წარმოშობის ქვეყანა';ru='В строке [%НомерСтрокиСОшибкой%] не верно указана страна происхождения'");
				...
				Если НЕ ВидОперации = Перечисления.ВидыОперацийСчетФактура.НаАвансКомитента
					...
					ТекстОшибки = НСтр("ka='სტრიქონ [%НомерСтрокиСОшибкой%] -ში არ არის მითითებული საბაჟო დეკლარაციის ნომერი';ru='В строке [%НомерСтрокиСОшибкой%] не указан номер ГТД'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтрокиСОшибкой%", СокрЛП(СтрокаТаблицыЗапасов.НомерСтроки));
	...
	Для каждого СтрокаТаблицы Из МассивДокументовОснований Цикл
		...
		Если ЗначениеЗаполнено(ДокументОснование)
			...
			ТекстОшибки = НСтр("ka='Документ-основание %ПредставлениеДокументаОснования% не проведен. Проведение счет фактуры невозможно.';ru='Документ-основание %ПредставлениеДокументаОснования% не проведен. Проведение счет фактуры невозможно.'");
	...
	Если НеобходимоОтразитьРублевыеСуммыДокумента() Тогда
		...
		ИначеЕсли ДокументыОснования.Количество() = 1 И НЕ ЗначениеЗаполнено(ДокументыОснования[0].ДокументОснование) Тогда
			...
			ТекстОшибки = НСтр("ka='Не выбран документ-основание.';ru='Не выбран документ-основание.'");
