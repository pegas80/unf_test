...
Процедура ЗаполнитьПоДокументуОснованию()
	...
	ТекстВопроса = НСтр("ka='დოკუმენტი სრულად გადაივსება ""საფუძვლის"" მიხედვით. გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет полностью перезаполнен по ""Основанию""."
...
Процедура УстановитьВозможностьРедактированияСписком()
	...
	Если НЕ Элементы.ДокументыОснованияРедактироватьСписком.Пометка
		...
		ТекстВопроса = НСтр("ka='ყველა მითითებული საფუძვლის დოკუმენტი პირველის გარდა სიიდან  წაიშლება. გავაგრძელოთ?';ru='Все указанные документы основания кроме первого будут удалены из списка. Продолжить?';en='All of these documents except the first base will be deleted from the list. Proceed?'");
...
Процедура УстановитьВидимостьРеквизитов()
	...
	Если ЗначениеЗаполнено(Объект.НомерВходящегоДокумента) Тогда
		...
		ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='%1 %2-დან';ru='%1 от %2';en='% 1% 2 from'"),
...
Процедура ОбработатьИзменениеДоговора()
	...
	Если ДоговорПередИзменением <> Объект.Договор Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор)
			...
			ТекстПредупреждения = НСтр("ka='შეიცვალა კონტრაგენტთან ხელშეკრულებით გათვალისწინებული ანგარიშსწორების ვალუტა! აუცილებელია დოკუმენტის ვალუტის შემოწმება!';ru='Изменилась валюта расчетов по договору с контрагентом! "
...
Процедура УстановитьНадписьЦеныИВалюта()
	...
	Если ЗначениеЗаполнено(Объект.ВалютаДокумента) Тогда
		...
		ЦеныИВалюта = НСтр("ka='%Валюта%';ru='%Валюта%';en='%Currency%'");
		ЦеныИВалюта = СтрЗаменить(ЦеныИВалюта, "%Валюта%", СокрЛП(Строка(Объект.ВалютаДокумента)));
		
	Иначе
		
		ЦеныИВалюта = НСтр("ka='<Валюта не указана>';ru='<Валюта не указана>';en='<Currency not specified>'");
...
Процедура УстановитьНадписьДокументыОплаты()
	...
	Если Объект.ДатаНомерДокументовОплаты.Количество() = 0 Тогда
		ДокументыОплаты = НСтр("ka='რედაქტირება...';ru='Редактировать...';en='Edit...'");
	КонецЕсли;
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДанныеТекущейСтроки.СтранаПроисхождения)
			...
			ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС) + 
				НСтр("ka='Учет ГТД для отечественных товаров не ведется!';ru='Учет ГТД для отечественных товаров не ведется!';en='Accounting for GTE domestic goods is not conducted!'");
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ДанныеТекущейСтроки.НомерГТД) Тогда
			...
			Если ДатаГТД > Объект.Дата Тогда
				...
				ТекстВопроса = Нстр("ka='არჩეულია სატვირთო-საბაჟო დეკლარაცია, რომლის რეგისტრაციის თარიღი დოკუმენტის თარიღზე ძველია. გავაგრძელოთ?';ru='Выбрана ГТД с датой регистрации старше, чем дата документа."
"Продолжить?'");
...
Процедура ЗаполнитьНомераГТД(Команда)
	...
	Если Объект.Запасы.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ka='ცხრილი მარაგებით შეუვსებელია. შესრულება შეუძლებელია.';ru='Незаполнена табличная часть с запасами. Выполнение не возможно.';en='Unfilled tabular section with stocks. Execution is not possible.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	ТекстВопроса= НСтр("ka='ცხრილში გადაიტვირთება სვეტები ""საბაჟო დეკლარაციის ნობერი"" და ""მწარმოებელი ქვეყანა""! გავაგრძელოთ?';ru='В табличной части будут перезаполнены колонки ""Номер ГТД"" и ""Страна происхождения""! Продолжить?';en='In the tables of this column will refill ""GTD Number"" and ""Country of origin""! Proceed?'");
...
Процедура УстановитьНомерГТД(Команда)
	...
	СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		ТекстСообщения = НСтр("ka='ცხრილის სტრიქონი არ არის არჩეული. შესრულება შეუძლებელია.';ru='Не выбрана строка табличной части. Выполнение не возможно.';en='Not selected row of the table. Execution is not possible.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
