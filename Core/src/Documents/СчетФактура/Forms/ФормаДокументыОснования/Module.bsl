...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ)
	...
	Для каждого СтрокаДокументыОснования Из ДокументыОснования Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаДокументыОснования.ДокументОснование) Тогда
			...
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Документ основание"" в строке ';ka='სვეტი ""დოკუმენტი საფუძველი"" არ არის შევსებული სტრიქონში'")
				+ Строка(НомерСтроки)
				+ НСтр("ka='სია ""საფუძვლის დოკუმენტები"".';ru=' списка ""Документы основания"".'");
...
Процедура ДокументыОснованияДокументОснованиеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ka='ასარჩევი დოკუმენტის კონტრაგენტი, ხელშეკრულება, ვალუტა ან ოპერაცია განსხვავდება ადრე არჩეული დოკუმენტებისგან.';ru='Контрагент, договор, валюта или операция выбираемого документа отличается от выбранных ранее документов.';en='Counterparty, contract, or transaction currency selected document is different from the previously selected documents.'");
		ОчиститьСообщения();
