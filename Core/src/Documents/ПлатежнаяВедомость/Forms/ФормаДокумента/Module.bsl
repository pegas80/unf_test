...
Процедура ПересчитатьСуммуПоВалюте(СтрокаТабличнойЧасти, ИзмененоНачисление, ИзмененоКВыдаче, ДополнительныеПараметры = Неопределено)
	...
	Если ИзмененоНачисление Тогда
		...
		Если ДополнительныеПараметры <> Неопределено Тогда
			...
			СтрокаТабличнойЧасти.НДФЛРасчетов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.НДФЛРасчетов, ДополнительныеПараметры.ВалютаРасчетовПередИзменением, Объект.ВалютаРасчетов, ДополнительныеПараметры.КурсПередИзменением, Объект.Курс, ДополнительныеПараметры.КратностьПередИзменением, Объект.Кратность);
			СтрокаТабличнойЧасти.СуммаПФРасчетов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаПФРасчетов, ДополнительныеПараметры.ВалютаРасчетовПередИзменением, Объект.ВалютаРасчетов, ДополнительныеПараметры.КурсПередИзменением, Объект.Курс, ДополнительныеПараметры.КратностьПередИзменением, Объект.Кратность);
			//-- 10.09 2025 ONESOFT-4331	
		КонецЕсли;
		...
		//++ 10.09 2025 ONESOFT-4331
		СтрокаТабличнойЧасти.НДФЛПлатежа = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.НДФЛРасчетов, Объект.ВалютаРасчетов, Объект.ВалютаДокумента, Объект.Курс, КурсВалютаДокумента, Объект.Кратность, КратностьВалютаДокумента);
        СтрокаТабличнойЧасти.СуммаПФПлатежа = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаПФРасчетов, Объект.ВалютаРасчетов, Объект.ВалютаДокумента, Объект.Курс, КурсВалютаДокумента, Объект.Кратность, КратностьВалютаДокумента);
        //-- 10.09 2025 ONESOFT-4331
	ИначеЕсли ИзмененоКВыдаче Тогда
		...
		Если ДополнительныеПараметры <> Неопределено Тогда
			...
			//++ 10.09 2025 ONESOFT-4331
			СтрокаТабличнойЧасти.НДФЛПлатежа = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.НДФЛПлатежа, ДополнительныеПараметры.ВалютаДокументаПередИзменением, Объект.ВалютаДокумента, ДополнительныеПараметры.КурсВалютаДокументаПередИзменением, КурсВалютаДокумента, ДополнительныеПараметры.КратностьВалютаДокументаПередИзменением, КратностьВалютаДокумента);
			СтрокаТабличнойЧасти.СуммаПФПлатежа = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаПФПлатежа, ДополнительныеПараметры.ВалютаДокументаПередИзменением, Объект.ВалютаДокумента, ДополнительныеПараметры.КурсВалютаДокументаПередИзменением, КурсВалютаДокумента, ДополнительныеПараметры.КратностьВалютаДокументаПередИзменением, КратностьВалютаДокумента);
	        //-- 10.09 2025 ONESOFT-4331
		КонецЕсли;
		...
		//++ 10.09 2025 ONESOFT-4331
		СтрокаТабличнойЧасти.НДФЛРасчетов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.НДФЛПлатежа, Объект.ВалютаДокумента, Объект.ВалютаРасчетов, КурсВалютаДокумента, Объект.Курс, КратностьВалютаДокумента, Объект.Кратность);
		СтрокаТабличнойЧасти.СуммаПФРасчетов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаТабличнойЧасти.СуммаПФПлатежа, Объект.ВалютаДокумента, Объект.ВалютаРасчетов, КурсВалютаДокумента, Объект.Курс, КратностьВалютаДокумента, Объект.Кратность);
		//-- 10.09 2025 ONESOFT-4331
	КонецЕсли;
...
Процедура РасчитатьСуммы_ПФ_НДФЛ(Идентификатор)
	...
	//Если ЗначениеЗаполнено(ТекСтрока.ВидУдержанияНДФЛ) Тогда
	Если ТекСтрока <> Неопределено Тогда	 //++ 11.09.2025 ONESOFT-4331
		СтавкаНДФЛ = ТекСтрока.ВидУдержанияНДФЛ.СтавкаНДФЛ;  
		СтавкаПФ = ТекСтрока.ВидУдержанияПФ.СтавкаПФ;
	КонецЕсли;   
