...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВалютаРасчетов) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='არ არის არჩეული ანგარიშსწორების ვალუტა შესავსებად!';ru='Не выбрана валюта расчетов для заполнения!';en='Not selected currency calculations are required!'");
