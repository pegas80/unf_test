...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		ТекстСообщения	= НСтр("ka='საჭიროა დოკუმენტ-საფუძვლის შევსება';ru='Требуется заполнить документ-основание.';en='Required to complete the document-basis.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.ДокументОснование");
		Возврат;
		
	КонецЕсли;
	
	ТекстВопроса 	= НСтр("ka='დოკუმენტი სრულიად შეივსება ""საფუძვლით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить выполнение операции?';en='The document will be fully refilled by ""base""! To continue the operation?'");
