...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") 
			...
			И ТипЗнч(ДанныеЗаполнения.ОснованиеЗаполнения) = Тип("ДокументСсылка.ДокументЗапроса") Тогда
			
		ЗаполнитьПоДокументуЗапроса(ДанныеЗаполнения.ОснованиеЗаполнения);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо
		...
		ВызватьИсключение НСтр("ka='შემომავალი წერილის კოპირება შეუძლებელია.';ru='Копирование входящего письма невозможно.';en='Copy incoming e-mail is not possible.'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОкончаниеСобытия < НачалоСобытия Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='დასრულების თარიღი არ შეიძლება დაწყების თარიღზე ნაკლები იყოს.';ru='Дата окончания не может быть меньше даты начала.';en='The end date can not be less than the start date.'"),
			ЭтотОбъект,
			"ДатаОкончания",
			,
			Отказ
...
Процедура ОтправитьЭлектронноеПисьмо(Записывать = Истина, Знач ВложенияПисьма = Неопределено) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		ВызватьИсключение НСтр("ka='გასაგზავნი წერილის მოვლენის ტიპი უნდა იყოს ""ელექტრონული წერილი""';ru='Отправляемое письмо должно иметь тип события ""Электронное письмо""';en='Sent a letter should have the type of event, ""The e-mail""'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение НСтр("ka='მიმღების არაკორექტული მისამართი გასაგზავნი წერილისთვის';ru='Некорректный адрес получателя для отправляемого письма';en='Invalid destination address to send emails'") + ":" + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	...
	Исключение
		ВызватьИсключение НСтр("ka='წერილის გაგზავნა ვერ მოხერხდა შემდეგი მიზეზით';ru='Не удалось выполнить отправку письма по причине';en='Failed to send a letter because of'") + ":" + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Процедура ОтправитьSMS(Записывать = Истина) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.СообщениеSMS Тогда
		ВызватьИсключение НСтр("ka='გასაგზავნ SMS შეტყობინებას უნდა ჰქონდეს მოვლენის ტიპი ""SMS""';ru='Отправляемое сообщение SMS должно иметь тип события ""SMS""';en='Sent SMS message must have an event type ""SMS""'");
	КонецЕсли;
	...
	Для Каждого Получатель Из ЭтотОбъект.Участники Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ka='ნომრის არასწორი ფორმატი მიმღებების სიის სტრიქონ %1-ში';ru='Неверный формат номера в строке %1 списка получателей';en='Invalid number format on line% 1 the list of recipients'") + ":" + Символы.ПС + РезультатПроверки.СообщениеОбОшибке;
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ПустаяСтрока(РезультатОтправки.ОписаниеОшибки) Тогда
		ВызватьИсключение НСтр("ka='SMS-ის გაგზავნის შესრულება ვერ მოხერხდა შემდეგი მიზეზით';ru='Не удалось выполнить отправку SMS по причине';en='Failed sending SMS due'") + ":" + Символы.ПС + РезультатОтправки.ОписаниеОшибки;
	КонецЕсли;
...
Процедура ОбработчикЗаполнения(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок
		...
		ЭтотОбъект.Содержание = СтрШаблон(НСтр("ka='Звонок с номера: %1.';ru='Звонок с номера: %1.';en='Up to the room:% 1.'"), ДанныеЗаполнения.НомерТелефона);
	КонецЕсли;
...
Процедура ЗаполнитьПоДокументуЗапроса(Основание)
	
	ДокументОснование = Основание;
	Проект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Проект");
	НачалоСобытия = ТекущаяДата();
	ОкончаниеСобытия = КонецДня(ТекущаяДата());
