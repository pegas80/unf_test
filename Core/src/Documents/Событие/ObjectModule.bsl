...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо
		...
		ВызватьИсключение НСтр("ka='შემომავალი წერილის კოპირება შეუძლებელია.';ru='Копирование входящего письма невозможно.'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОкончаниеСобытия < НачалоСобытия Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='დასრულების თარიღი არ შეიძლება დაწყების თარიღზე ნაკლები იყოს.';ru='Дата окончания не может быть меньше даты начала.'"),
			ЭтотОбъект,
			"ДатаОкончания",
			,
			Отказ
...
Процедура ОтправитьЭлектронноеПисьмо(Записывать = Истина, Знач ВложенияПисьма = Неопределено) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		ВызватьИсключение НСтр("ka='გასაგზავნი წერილის მოვლენის ტიპი უნდა იყოს ""ელექტრონული წერილი""';ru='Отправляемое письмо должно иметь тип события ""Электронное письмо""'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение НСтр("ka='მიმღების არაკორექტული მისამართი გასაგზავნი წერილისთვის';ru='Некорректный адрес получателя для отправляемого письма'") + ":" + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	...
	Исключение
		ВызватьИсключение НСтр("ka='წერილის გაგზავნა ვერ მოხერხდა შემდეგი მიზეზით';ru='Не удалось выполнить отправку письма по причине'") + ":" + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Процедура ОтправитьSMS(Записывать = Истина) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.СообщениеSMS Тогда
		ВызватьИсключение НСтр("ka='გასაგზავნ SMS შეტყობინებას უნდა ჰქონდეს მოვლენის ტიპი ""SMS""';ru='Отправляемое сообщение SMS должно иметь тип события ""SMS""'");
	КонецЕсли;
	...
	Для Каждого Получатель Из ЭтотОбъект.Участники Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ka='ნომრის არასწორი ფორმატი მიმღებების სიის სტრიქონ %1-ში';ru='Неверный формат номера в строке %1 списка получателей'") + ":" + Символы.ПС + РезультатПроверки.СообщениеОбОшибке;
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ПустаяСтрока(РезультатОтправки.ОписаниеОшибки) Тогда
		ВызватьИсключение НСтр("ka='SMS-ის გაგზავნის შესრულება ვერ მოხერხდა შემდეგი მიზეზით';ru='Не удалось выполнить отправку SMS по причине'") + ":" + Символы.ПС + РезультатОтправки.ОписаниеОшибки;
	КонецЕсли;
...
Процедура ОбработчикЗаполнения(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок
		...
		ЭтотОбъект.Содержание = СтрШаблон(НСтр("ka='Звонок с номера: %1.';ru='Звонок с номера: %1.'"), ДанныеЗаполнения.НомерТелефона);
	КонецЕсли;
