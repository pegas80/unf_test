...
Процедура УстановитьОтборТипСобытия()
	...
	Если Параметры.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок Тогда
		Заголовок = НСтр("ka='События: телефонные звонки';ru='События: телефонные звонки';en='Events: phone calls'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		Заголовок = НСтр("ka='События: электронные письма';ru='События: электронные письма';en='Events: emails'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.СообщениеSMS Тогда
		Заголовок = НСтр("ka='События: сообщения SMS';ru='События: сообщения SMS';en='Events: SMS messages'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЛичнаяВстреча Тогда
		Заголовок = НСтр("ka='События: личные встречи';ru='События: личные встречи';en='Events: personal meetings'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.Прочее Тогда
		Заголовок = НСтр("ka='მოვლენები: სხვა';ru='События: прочие';en='Events: Other'");
	КонецЕсли;
...
Процедура ОбработатьРезультатВыполненияЗаданияНаСервере()
	...
	НСтр("ka='ატვირთულია %1';ru='Загружено %1';en='Uploaded 1%'"),
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	ЗагруженоПисем,
	НСтр("ka='письмо,письма,писем';ru='письмо,письма,писем';en='letter, letters, emails'")));
