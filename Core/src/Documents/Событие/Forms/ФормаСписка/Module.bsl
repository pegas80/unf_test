...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='მოვლენები';ru='События'");
...
Процедура УстановитьОтборТипСобытия()
	...
	Если Параметры.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок Тогда
		Заголовок = НСтр("ka='События: телефонные звонки';ru='События: телефонные звонки'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		Заголовок = НСтр("ka='События: электронные письма';ru='События: электронные письма'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.СообщениеSMS Тогда
		Заголовок = НСтр("ka='События: сообщения SMS';ru='События: сообщения SMS'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЛичнаяВстреча Тогда
		Заголовок = НСтр("ka='События: личные встречи';ru='События: личные встречи'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.Прочее Тогда
		Заголовок = НСтр("ka='მოვლენები: სხვა';ru='События: прочие'");
	КонецЕсли;
...
Процедура НастроитьГруппуПочтоваяСлужба()
	...
	Если ПочтовыеСлужбыПодключены Тогда
		...
		Если ЗначениеЗаполнено(КонтекстКонтакт) Тогда
			Команды.ПроверитьПочту.Заголовок = НСтр("ka='მიმოწერის ატვირთვა';ru='Загрузить переписку'");
			Команды.ПроверитьПочту.Подсказка = СтрШаблон(НСтр("ka='%1-თან მიმოწერის ჩატვირთვა';ru='Загрузить переписку с ""%1""'"), КонтекстКонтакт);
...
Функция ЗаголовокПрогресса(Режим, Контакт)
	...
	Если ЗначениеЗаполнено(Контакт) Тогда
		Возврат НСтр("ka='მიმოწერის ატვირთვა';ru='Загрузка переписки'");
	КонецЕсли;
	
	Если Режим = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
		Возврат НСтр("ka='ფოსტის შემოწმება';ru='Проверка почты'");
	КонецЕсли;
	
	Если Режим = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
		Возврат НСтр("ka='წინა შეტყობინებების ჩატვირთვა';ru='Загрузка предыдущих сообщений'");
	КонецЕсли;
...
Процедура ОбработатьРезультатВыполненияЗаданияНаСервере()
	...
	НСтр("ka='ატვირთულია %1';ru='Загружено %1'"),
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	ЗагруженоПисем,
	НСтр("ka='письмо,письма,писем';ru='письмо,письма,писем'")));
