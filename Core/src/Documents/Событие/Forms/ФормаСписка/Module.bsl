...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДопустимыеУчтенныеЗаписи = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
	Пользователи.ТекущийПользователь(),
	"ДопустимыеУчтенныеЗаписи");
	Список.Параметры.УстановитьЗначениеПараметра("УчетнаяЗапись", ДопустимыеУчтенныеЗаписи.УчтенныеЗаписи.ВыгрузитьКолонку("УчетнаяЗапись"));
	Список.Параметры.УстановитьЗначениеПараметра("НеФилтровать",Не Константы.ФилтрСобитииПоОтветственному.Получить());
...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='მოვლენები';ru='События';en='Events'");
...
Процедура УстановитьОтборТипСобытия()
	...
	Если Параметры.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок Тогда
		Заголовок = НСтр("ru = 'События: телефонные звонки'; ka = 'მოვლენები: სატელეფონო ზარები'; en = 'Events: phone calls'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		Заголовок = НСтр("ru = 'События: электронные письма'; ka = 'მოვლენები: ელ.წერილი'; en = 'Events: emails'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.СообщениеSMS Тогда
		Заголовок = НСтр("ru = 'События: сообщения SMS'; ka = 'მოვლენები: SMS შეტყობინებები'; en = 'Events: SMS messages'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЛичнаяВстреча Тогда
		Заголовок = НСтр("ru = 'События: личные встречи'; ka = 'მოვლენები: პირადი შეხვედრები'; en = 'Events: personal meetings'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.Прочее Тогда
		Заголовок = НСтр("ru = 'События: прочие'; ka = 'მოვლენები: სხვა'; en = 'Events: Other'");
	КонецЕсли;
...
Процедура НастроитьГруппуПочтоваяСлужба()
	...
	Если ПочтовыеСлужбыПодключены Тогда
		...
		Если ЗначениеЗаполнено(КонтекстКонтакт) Тогда
			Команды.ПроверитьПочту.Заголовок = НСтр("ka='მიმოწერის ატვირთვა';ru='Загрузить переписку';en='Upload correspondence'");
			Команды.ПроверитьПочту.Подсказка = СтрШаблон(НСтр("ka='%1-თან მიმოწერის ჩატვირთვა';ru='Загрузить переписку с ""%1""';en='Upload a conversation with ""%1""'"), КонтекстКонтакт);
...
Функция ЗаголовокПрогресса(Режим, Контакт)
	...
	Если ЗначениеЗаполнено(Контакт) Тогда
		Возврат НСтр("ka='მიმოწერის ატვირთვა';ru='Загрузка переписки';en='Loading correspondence'");
	КонецЕсли;
	
	Если Режим = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
		Возврат НСтр("ka='ფოსტის შემოწმება';ru='Проверка почты';en='Checking mail'");
	КонецЕсли;
	
	Если Режим = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
		Возврат НСтр("ka='წინა შეტყობინებების ჩატვირთვა';ru='Загрузка предыдущих сообщений';en='Loading older posts'");
	КонецЕсли;
...
Процедура ОбработатьРезультатВыполненияЗаданияНаСервере()
	...
	НСтр("ru = 'Загружено %1'; ka = 'ატვირთულია %1'; en = 'Uploaded %1'"),
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	ЗагруженоПисем,
	НСтр("ru = 'письмо,письма,писем'; ka = 'წერილი, წერილები, წერილები'; en = 'letter, letters, emails'")));
...
&НаСервере
Функция  ПолучитьДопустимыеУчтенныеЗаписи()
	
	ДопустимыеУчтенныеЗаписи = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
	Пользователи.ТекущийПользователь(),
	"ДопустимыеУчтенныеЗаписи");
	
	Возврат  ДопустимыеУчтенныеЗаписи.УчтенныеЗаписи.ВыгрузитьКолонку("УчетнаяЗапись");	
	
КонецФункции

&НаКлиенте
Процедура ОтборУчетнаяЗаписьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка =Ложь;
	СписокЗаписей = ПолучитьДопустимыеУчтенныеЗаписи();
	Элемент.СписокВыбора.Очистить();
	Для каждого Запись Из СписокЗаписей Цикл
		Элемент.СписокВыбора.Добавить(Запись);
	КонецЦикла;
КонецПроцедуры
