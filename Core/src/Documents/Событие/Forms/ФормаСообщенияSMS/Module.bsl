...
Процедура Отправить(Команда)
	...
	Если Объект.Участники.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მიმღებების სია არ არის შევსებული.';ru='Список получателей не заполнен.'"),
			,
			"Объект.Участники",
			,
			ЕстьОшибки);
	...
	Если ПустаяСтрока(Объект.Содержание) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='ველი ""შემცველობა"" არ არის შევსებული.';ru='Поле ""Содержание"" не заполнено.'"),
			,
			"Объект.Содержание",
			,
			ЕстьОшибки);
...
Функция СформироватьНадписьКоличествоСимволов(ОтправлятьВТранслите, знач ТекстСообщения)
	...
	ШаблонТекстаСообщения = НСтр("ka='შეტყობინება - %1, დარჩენილია სიმბოლო - %2';ru='Сообщение - %1, осталось символов - %2'");
...
Процедура ПроверитьИПреобразоватьНомераПолучателей(Отказ)
	...
	Для Каждого Получатель Из Объект.Участники Цикл
		...
		Если ПустаяСтрока(Получатель.КакСвязаться) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='ველი ""ტელეფონის ნომერი"" არ არის შევსებული.';ru='Поле ""Номер телефона"" не заполнено.'"),
				,
		...
		Если СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Получатель.КакСвязаться, ";", Истина).Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='მითითებული უნდა იქნას მხოლოდ ერთი ტელეფონის ნომერი.';ru='Должен быть указан только один номер телефона.'"),
				,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Участники", Получатель.НомерСтроки, "КакСвязаться"),
				,
				Отказ);
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ka='ტელეფონის ნომრის არასწორი ფორმატი.';ru='Неверный формат номера телефона.'"),
				,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Участники", Получатель.НомерСтроки, "КакСвязаться"),
				,
				Отказ);
...
Процедура ОтправкаSMSНастройкиВыполнены()
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		...
		Записать();
		ПоказатьОповещениеПользователя(НСтр("ka='SMS წარმატებით გაიგზავნა';ru='SMS успешно отправлено'"), ПолучитьНавигационнуюСсылку(Объект.Ссылка), Строка(Объект.Ссылка), БиблиотекаКартинок.Информация32);
		Закрыть();
	Иначе
