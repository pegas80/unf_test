...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтрШаблон(
		НСтр("ka='მოვლენა: %1 (შექმნა)';ru='Событие: %1 (создание)'"),
		Объект.ТипСобытия);
...
Процедура КонтактыКонтактНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აუცილებელია კონტრაგენტის არჩევა.';ru='Необходимо выбрать контрагента.'"), , "Контрагент");
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Не указано основание для заполнения.';ru='Не указано основание для заполнения.'"));
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект),
		НСтр("ka='დოკუმენტი სრულიად შეივსება ""საფუძვლით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаполнитьПоКонтрагенту(Команда)
	...
	Если Контакты.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоКонтрагентуЗавершение", ЭтотОбъект),
			НСтр("ka='კონტაქტები სრულად გადაივსება კონტრაგენტის მიხედვით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Контакты будут полностью перезаполнены по контрагенту! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
	Иначе
...
Процедура НабратьНомерКлиент(Номер)
	...
	Если ПустаяСтрока(Номер) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнен номер телефона для связи.';ru='Не заполнен номер телефона для связи.'"));
		Возврат;
	КонецЕсли;
	...
	Если Номера.Количество() > 1 Тогда
		...
		ПараметрыФормы.Вставить("ЗаголовокКнопки", НСтр("ka='Позвонить';ru='Позвонить'"));
...
"с устройства: %2'");
		...
		ПоказатьОповещениеПользователя(
			НСтр("ka='ნომტის აკრეფა';ru='Набор номера'"),
...
Процедура СформироватьПредставлениеПродолжительности(Форма)
	...
	ПодписьЧасов = УправлениеНебольшойФирмойКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ka='სთ';ru='час'"),
		НСтр("ka='საათები';ru='часа'"),
		НСтр("ka='საათი';ru='часов'"),
		Часов
	);
	...
	ПодписьМинут = УправлениеНебольшойФирмойКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ka='წუთი';ru='минута'"),
		НСтр("ka='წუთები';ru='минуты'"),
		НСтр("ka='წუთი';ru='минут'"),
		Минут
	);
