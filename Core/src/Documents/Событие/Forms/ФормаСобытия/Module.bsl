...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтрШаблон(
		НСтр("ka='მოვლენა: %1 (შექმნა)';ru='Событие: %1 (создание)';en='Event: 1% (creation)'"),
		Объект.ТипСобытия);
...
Процедура КонтактыКонтактНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='აუცილებელია კონტრაგენტის არჩევა.';ru='Необходимо выбрать контрагента.';en='You must select a contractor.'"), , "Контрагент");
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Не указано основание для заполнения.';ru='Не указано основание для заполнения.';en='Not Specified base required.'"));
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект),
		НСтр("ka='დოკუმენტი სრულიად შეივსება ""საფუძვლით""! გავაგრძელოთ ოპერაცია?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить выполнение операции?';en='The document will be fully refilled by ""base""! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаполнитьПоКонтрагенту(Команда)
	...
	Если Контакты.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоКонтрагентуЗавершение", ЭтотОбъект),
			НСтр("ka='კონტაქტები სრულად გადაივსება კონტრაგენტის მიხედვით! გავაგრძელოთ ოპერაციის შესრულება?';ru='Контакты будут полностью перезаполнены по контрагенту! Продолжить выполнение операции?';en='Contacts will be fully refilled by counterpart! To continue the operation?'"), РежимДиалогаВопрос.ДаНет, 0);
	Иначе
...
Процедура НабратьНомерКлиент(Номер)
	...
	Если ПустаяСтрока(Номер) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='Не заполнен номер телефона для связи.';ru='Не заполнен номер телефона для связи.';en='Not full phone number to contact.'"));
		Возврат;
	КонецЕсли;
	...
	Если Номера.Количество() > 1 Тогда
		...
		ПараметрыФормы.Вставить("ЗаголовокКнопки", НСтр("ka='Позвонить';ru='Позвонить';en='call'"));
...
Процедура ПоказатьОповещениеОНабореНомера(Номер)
	...
	Если ИспользоватьМобильнуюТелефонию Тогда
		...
		ШаблонПояснения = НСтр("ka='Вызов: %1,"
"с устройства: %2';ru='Вызов: %1,"
"с устройства: %2'");
		
		ПоказатьОповещениеПользователя(
			НСтр("ka='ნომტის აკრეფა';ru='Набор номера';en='Dialing a number'"),
	...
	ИначеЕсли ИспользоватьОблачнуюТелефонию Тогда
		...
		ШаблонПояснения = НСтр("ru='Вызов: %1';en='Calling% 1'");
...
Процедура СформироватьПредставлениеПродолжительности(Форма)
	...
	ПодписьДней = УправлениеНебольшойФирмойКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ka='დღე';ru='день';en='day'"),
		НСтр("ka='დღე';ru='дня';en='day'"),
		НСтр("ka='დღე';ru='дней';en='days'"),
		Дней
	);
	
	Часов = Цел((ДлительностьСек - Дней * 86400) / 3600);
	ПодписьЧасов = УправлениеНебольшойФирмойКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ka='სთ';ru='час';en='hour'"),
		НСтр("ka='საათები';ru='часа';en='hours'"),
		НСтр("ka='საათი';ru='часов';en='hours'"),
		Часов
	);
	...
	ПодписьМинут = УправлениеНебольшойФирмойКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ka='წუთი';ru='минута';en='minute'"),
		НСтр("ka='წუთები';ru='минуты';en='minutes'"),
		НСтр("ka='წუთი';ru='минут';en='minutes'"),
		Минут
	);
