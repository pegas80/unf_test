...
Процедура Отправить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.УчетнаяЗапись)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='აირჩიეთ სააღრიცხვო ჩანაწერი ფოსტის გასაგზავნად.';ru='Выберите учетную запись для отправки почты.'"), ,
			"Объект.УчетнаяЗапись");
		Возврат;
	КонецЕсли;
	...
	СписокОтветов.Добавить(КодВозвратаДиалога.ОК, НСтр("ka='გაგზავნის დუბლირება';ru='Продублировать отправку'"));
	СписокОтветов.Добавить(КодВозвратаДиалога.Отмена);
	Если ЗначениеЗаполнено(ИдентификаторСобытия) Тогда
		ПоказатьВопрос(
		ОписаниеОповещения,
		СтрШаблон(НСтр("ka='ეს წერილი უკვე გაიგზავნა %1';ru='Это письмо уже отправлялось %1'"), Объект.НачалоСобытия),
		СписокОтветов, 30,
		КодВозвратаДиалога.Отмена,
		НСтр("ka='ეს წერილი უკვე გაიგზავნა';ru='Это письмо уже отправлялось'"),
		КодВозвратаДиалога.Отмена)
	Иначе
...
Процедура ЗаполнитьНовоеПисьмоПоУмолчанию(Параметры)
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ka='გამავალი წერილი (შექმნა)';ru='Исходящее письмо  (создание)'");
...
Функция ПолучитьТабличныйДокументПоДвоичнымДанным(Знач ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ka='ცხრილი-დოკუმენტის მიღება';ru='Получение табличного документа'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , 
...
Процедура УстановитьЗаголовокГиперссылкиДокументовОснований()
	...
	Если Объект.ДокументыОснования.Количество() = 0 Тогда
		Элементы.ОткрытьДокументыОснования.Заголовок = НСтр("ka='საფუძველი დოკუმენტების სია';ru='Список документов оснований'");
		Возврат;
	КонецЕсли;
	...
	Элементы.ОткрытьДокументыОснования.Заголовок = СтрШаблон(НСтр("ka='%1 и ещё %2.';ru='%1 и ещё %2.'"),
	ПервыйДокумент,
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	Объект.ДокументыОснования.Количество() - 1,
	НСтр("ka='основание,основания,оснований';ru='основание,основания,оснований'")));
...
Процедура ПродолжитьОтправкуПисьма(Результат, Параметры) Экспорт
	...
	Если ПараметрыПисьма = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='საფოსტო შეტყობინების პარამეტრების ფორმირების შეცდომა';ru='Ошибка формирования параметров почтового сообщения'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ka='შეტყობინება წარმატებით გაიგზავნა';ru='Сообщение успешно отправлено'"), ПолучитьНавигационнуюСсылку(Объект.Ссылка), Строка(Объект.Ссылка), БиблиотекаКартинок.Информация32);
	Закрыть(Успешно);
...
Процедура ЗаполнитьСодержаниеСобытия(ТемаСобытия)
	...
	Если НЕ ПустаяСтрока(ФорматированныйДокумент.ПолучитьТекст()) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСодержаниеСобытияЗавершение", ЭтотОбъект, Новый Структура("ТемаСобытия", ТемаСобытия)),
			НСтр("ka='გადავავსოთ არჩეული თემის შემცველობა?';ru='Перезаполнить содержание по выбранной теме?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ОткрытьВыборШаблонаПоПредмету(ПредметШаблона = Неопределено)
	...
	Если Не ЕстьДоступныеШаблоны(ПредметШаблона) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='Нет доступных шаблонов. Добавить новые шаблоны можно в списке шаблонов: Еще больше возможностей - CRM - Шаблоны сообщений';ru='Нет доступных шаблонов. Добавить новые шаблоны можно в списке шаблонов: Еще больше возможностей - CRM - Шаблоны сообщений'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьВыборПредметаШаблона(СписокПредметов)
	...
	СписокПредметов.Вставить(0, "Общий", НСтр("ka='<Общие шаблоны>';ru='<Общие шаблоны>'"));
	
	СписокПредметов.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ka='Выбор предмета шаблона';ru='Выбор предмета шаблона'"));
