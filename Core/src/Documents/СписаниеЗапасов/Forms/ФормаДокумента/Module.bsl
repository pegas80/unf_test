...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+++ Lazare 17.11.2021
	Если Не УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ПоказыватьБухгалтерскиеПроводки") и не ЗначениеЗаполнено(Объект.КорреспонденцияРегл)	
		Тогда  Объект.КорреспонденцияРегл = РС_ОбщегоНазначения.СчетДляСписанияОприходованияЗапасов(Объект.ссылка);	
	КонецЕсли;
	//--- Lazare_17.11.2021	
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеКонстанты("ФункционалВидимостиСвободногоОстаткаЗапасовВВокументах") Тогда 
		Для Каждого Ст из Объект.Запасы Цикл 	
			Ст.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, Ст.Номенклатура,Ст.Характеристика,Ст.Партия,Объект.Организация, Объект.СтруктурнаяЕдиница);//++Lazare 13.12.2022	
		КонецЦикла;	
	КонецЕсли;
	//-Lazare 15.12.2022
...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ИначеЕсли НоваяСтрока или Копирование Тогда 
		Элемент.ТекущиеДанные.Проект = Объект.Проект;//++Lazare 29-11-2021
	КонецЕсли;	
...
Процедура ПередатьДанныеВЕГАИС(Команда)
	...
	Если Модифицированность ИЛИ НЕ Объект.Проведен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Перед выполнение операции необходимо провести документ.'; ka = 'ოპერაციის შესრულებამდე უნდა გაატაროთ დოკუმენტი.'; en = 'Before the operation it is necessary to hold the paper.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОповещениеПередачаДанныхВЕГАИС(ДанныеОтЕГАИС, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ДанныеОтЕГАИС = Неопределено Тогда
		Если НЕ ДанныеОтЕГАИС.Результат Тогда
			СтрокаСообщения = НСтр("ru = 'Ошибка передачи данных в ЕГАИС'; ka = 'შეცდომა მონაცემთა EGAIS-ში გადაცემისას'; en = 'Error transferring data EGAIS'") 
	...
	Если ВыборкаДокумент.Следующий() Тогда
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаТовары.НомерСправкиА) Тогда
				Отказ = Истина;
				СтрокаОшибки = НСтр("ru = 'В строке %1 в справке Б не заполнен номер справки А.'; ka = 'სტრიქონ %1-ში ცნობა ბ-ში ცნობა ა-ს ნომერი არ არის შევსებული.'; en = 'Line %1 in the certificate B is empty reference number A.'");
			...
			Если ЗначениеЗаполнено(ВыборкаТовары.СправкаБ) Тогда
				Если НЕ ЗначениеЗаполнено(ВыборкаТовары.АлкогольнаяПродукция)
					...
					Отказ = Истина;
					СтрокаОшибки = НСтр("ru = 'В строке %1 не заполнена алкогольная продукция в справке Б.'; ka = 'სტრიქონ %1-ში არ არის შევსებული ალკოჰოლური პროდუქცია ცნობა ბ-ში.'; en = 'Line %1 is not filled with alcoholic beverages in B. Help'");
					СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ВыборкаТовары.НомерСтроки);
					ИмяПоляТоваров = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Запасы", ВыборкаТовары.НомерСтроки, "СправкаБ");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрокаОшибки,
			...
			Иначе
				Отказ = Истина;
				СтрокаОшибки = НСтр("ru = 'В строке %1 не заполнена справка Б.'; ka = 'სტრიქონ %1-ში ცნობა ბ არ არის შევსებული.'; en = 'On line %1 is not filled certificate B.'");
				СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ВыборкаТовары.НомерСтроки);
				ИмяПоляТоваров = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Запасы", ВыборкаТовары.НомерСтроки, "СправкаБ");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрокаОшибки,
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если МассивСтрокТЧ.Количество() = 0 Тогда
				...
				НоваяСтрока.КоличествоНаСкладе = УправлениеНебольшойФирмойСервер.ПолучитьКоличествоЗапасовНаСкладе(Объект.Дата, НоваяСтрока.Номенклатура,НоваяСтрока.Характеристика,НоваяСтрока.Партия,Объект.Организация, Объект.СтруктурнаяЕдиница);//++Lazare 13.12.2022
			Иначе
...
Процедура УправлениеФормой()
	...
	Если ТипСтруктурнойЕдиницы = ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.Подразделение") Тогда
		...
		Элементы.СписаниеИз.Подсказка  = НСтр("ru = '(списание из эксплуатации)'; ka = '(ექსპლუატაციიდან ჩამოწერა)'");
		Элементы.ЗапасыПодбор.Видимость = Ложь;
		
	Иначе
		
		Объект.СписатьЗапасыИзЭксплуатации = Ложь;
		Элементы.СписаниеИз.Подсказка  =  НСтр("ru = '(списание со склада)'; ka = '(ჩამოწერა საწყობიდან)'");		
