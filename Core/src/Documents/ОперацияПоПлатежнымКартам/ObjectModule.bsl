...
Процедура ЗаполнитьПоОперацияПоПлатежнымКартам(ДокументСсылкаОперацияПоПлатежнымКартам) Экспорт
	...
	Если ДокументСсылкаОперацияПоПлатежнымКартамВидОперации = Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю Тогда
		ВызватьИсключение НСтр("ka='ეკვაირინგის ოპერაცია არ შეიძლება, რომ შეყვანილი იქნას დასაბრუნებლად ეკვაირინგის ოპერაციის საფუძველზე';ru='Эквайринговая операция не может вводиться на основании эквайринговой операции на возврат'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		...
		ВызватьИсключение НСтр("ka='ეკვაირინგის ოპერაციის შეყვანა შეიძლება მხოლოდ მყიდველისგან დაბრუნების თაობაზე შემოსვლის ზედნადების საფუძველზე';ru='Эквайринговую операцию можно вводить только на основании приходной накладной на возврат от покупателя'");
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	Если ДокументСсылкаРасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку Тогда
		...
		НСтр("ka='დოკუმენტის შეყვანა შეუძლებელია ოპერაციის საფუძველზე - ""%1""!';ru='Невозможен ввод документа на основании операции - ""%1""!'"),
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		ТекстСообщения = НСтр("ka='ეკვაირინგის ტერმინალი არჩეული უნდა იქნას.';ru='Эквайринговый терминал должен быть выбран.'");
