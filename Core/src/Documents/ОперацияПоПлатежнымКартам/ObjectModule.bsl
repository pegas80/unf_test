...
Процедура ЗаполнитьПоОперацияПоПлатежнымКартам(ДокументСсылкаОперацияПоПлатежнымКартам) Экспорт
	...
	Если ДокументСсылкаОперацияПоПлатежнымКартамВидОперации = Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю Тогда
		ВызватьИсключение НСтр("ka='ეკვაირინგის ოპერაცია არ შეიძლება, რომ შეყვანილი იქნას დასაბრუნებლად ეკვაირინგის ოპერაციის საფუძველზე';ru='Эквайринговая операция не может вводиться на основании эквайринговой операции на возврат';en='Acquiring operation can not be administered on the basis of the acquiring operation to return'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		...
		ВызватьИсключение НСтр("ka='ეკვაირინგის ოპერაციის შეყვანა შეიძლება მხოლოდ მყიდველისგან დაბრუნების თაობაზე შემოსვლის ზედნადების საფუძველზე';ru='Эквайринговую операцию можно вводить только на основании приходной накладной на возврат от покупателя';en='Acquiring operation can be entered only on the basis of an incoming bill to a refund from a buyer'");
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	Если ДокументСсылкаРасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку Тогда
		...
		НСтр("ka='დოკუმენტის შეყვანა შეუძლებელია ოპერაციის საფუძველზე - ""%1""!';ru='Невозможен ввод документа на основании операции - ""%1""!';en='You can not enter a document on the basis of the operation - ""%1""!'"),
	...
	|   - СУММА(ШапкаДокумента.ВсегоИспользованоБаллов) //Малышев 2024-10-22 ONESOFT-3508
	|   КАК СуммаРасчетов,
	|	ВЫБОР
	|		КОГДА ШапкаДокумента.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	...
	//Малышев 2024-10-22 ONESOFT-3508 --)
	//|	СУММА(ЕСТЬNULL(ТаблицаДокумента.Всего, 0)) - СУММА(ШапкаДокумента.ВсегоИспользованоБаллов) КАК СуммаПлатежа,
	|	СУММА(ШапкаДокумента.ВсегоИспользованоБаллов) как ВсегоИспользованоБаллов, 
	//Малышев 2024-10-22 ONESOFT-3508 --)
	|	ТаблицаДокумента.СтавкаНДС КАК СтавкаНДС,
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		ТекстСообщения = НСтр("ka='ეკვაირინგის ტერმინალი არჩეული უნდა იქნას.';ru='Эквайринговый терминал должен быть выбран.';en='Acquiring the terminal must be selected.'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(ЭквайринговыйТерминал.Касса) ИЛИ
		...
		(Не ЗначениеЗаполнено(ЭквайринговыйТерминал.СчетЗатрат) И НЕ ИспользоватьТолькоРегламентированныйПланСчетов)
		ИЛИ (Не ЗначениеЗаполнено(ЭквайринговыйТерминал.СчетЗатратРегл) И ИспользоватьТолькоРегламентированныйПланСчетов) Тогда
		ТекстСообщения = НСтр("ka='В карточке эквайрингового терминала заполнены не все реквизиты. "
...
"Далее очистите поле эквайрингового терминала, а потом выберите его еще раз.';ru='В карточке эквайрингового терминала заполнены не все реквизиты. "
"Откройте карточку эквайрингового терминала и проверьте реквизиты ""Эквайрер"", ""Касса"", ""Банковский счет"" и ""Счет затрат"" (команда ""Счета учета""). "
