...
"Продолжить оформление оплаты картой с использованием эквайрингового терминала?';ka='ბარათის ნომერი უკვე შევსებულია. გავაგრძელოთ ეკვაირინგის ტერმინალით ბარათით გადახდის გაფორმება?'"), РежимДиалогаВопрос.ДаНет,,
...
Процедура ОповещениеОткрытьФормуАвторизацииЭТ(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатОткрытияФормы) = Тип("Структура") Тогда
		...
		Если ПараметрДействия.ПараметрыЭТ.ИспользоватьБезПодключенияОборудования Тогда
			...
			ТекстВопроса = НСтр("ru='Требуется выполнить операцию оплаты на эквайринговом терминале.';en='Payment is required to perform an operation on acquiring terminals.'") + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru='Сумма операции:';en='Transaction Amount:'") + " "+ ПараметрДействия.СуммаОперации  + Символы.ПС;
			ТекстВопроса = ТекстВопроса + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ka='Операция оплаты на эквайринговом терминале прошла успешно?';ru='Операция оплаты на эквайринговом терминале прошла успешно?';en='payment transaction on acquiring terminal is successful?'");
...
Процедура ОповещениеВопросУспешностиОплатыКартой(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если НЕ (РезультатВопроса = КодВозвратаДиалога.Да) Тогда
		СтрокаСлипЧека = "";
		ТекстОписаниеОшибки = НСтр("ru='Операция отменена.';en='Operation canceled.'");
...
"""%ОписаниеОшибки%""."
"Оплата по карте не была произведена.';ka='ოპერაციის შესრულებისას წარმოიშვა შეცდომა: ""%ОписаниеОшибки%""."
"ოპერაცია ბარათით არ განხორციელდა.'");
...
"Возможно для данного сеанса еще не выполнялась эквайринговая операция.';ka='სლიპ-ჩეკი არ არის."
"შეიძლება მოცემული სეანსისთვის ჯერ არ შესრულებულა ეკვაირინგული ოპერაცია.'"));
				Возврат;
			КонецЕсли;
...
"""%ОписаниеОшибки%"".';ka='სლიპ-ჩეკის ბეჭდვისას მოხდა შეცდომა:"
"""%ОписаниеОшибки%"".'");
...
"""%ОписаниеОшибки%"".';ka='ფისკალური რეგისტრატორის შეერთებისას მოხდა შეცდომა:"
"""%ОписаниеОшибки%"".'");
