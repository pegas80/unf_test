...
Процедура ЗаполнитьЗаголовокПоВыбранымДоговорам()
	...
	Если Объект.ДоговорыКонтрагентов.Количество() = 0 Тогда
		...
		ТекстНадписи = НСтр("ka='ხელშეკრულებების სია ცარიელია';ru='Список договоров пуст'");
	...
	ИначеЕсли МассивОтмеченных.Количество() = 0 Тогда
		...
		ТекстНадписи = НСтр("ka='ხელშეკრულებები არ არის არჩეული';ru='Договоры не выбраны'");
...
Процедура ПолучитьДанныеКонтрагента(ДанныеКонтрагента)
	...
	Если ЗаполнитьДанные Тогда
		...
		Объект.ОписаниеВыбораДоговоров = НСтр("ka='არჩეულია კონტრაგენტის ყველა ხელშეკრულება';ru='Выбраны все договоры контрагента'");
...
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыСверокВзаиморасчетов.Сверена") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПодписьОтветственного) Тогда
			...
			ТекстСообщения = Нстр("ka='ორგანიზაციის პასუხისმგებელი პირი არასწორად არის შევსებული.';ru='Неверно заполнено ответственное лицо организации.'");
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПредставительКонтрагента) Тогда
			...
			ТекстСообщения = Нстр("ka='კონტრაგენტის წარმომადგენელი არასწორად არის შევსებული.';ru='Неверно заполнен представитель контрагента.'");
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Иначе
		...
		Объект.ДоговорыКонтрагентов.Очистить();
		Объект.ОписаниеВыбораДоговоров = НСтр("ka='ხელშეკრულებების სია ცარიელია';ru='Список договоров пуст'");
...
Процедура ЗаполнитьПоДатамРасчетныхДокументов(Команда)
	...
	Если Объект.ДанныеКонтрагента.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ka='ურთიერთანგარიშსწორებების ცხრილი კონტრაგენტის მონაცემებით ცარიელია.';ru='Табличная часть взаиморасчетов по данным контрагента пуста.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
...
Процедура ЗаполнитьПоДаннымУчета(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		...
		ТекстСообщения	= НСтр("ka='პერიოდის დასრულების თარიღი არასწორად არის შევსებული.';ru='Неверно заполнена дата окончания периода.'");
	...
	Если ВызватьПроцедуруЗаполненияТабличнойЧасти Тогда
		...
		Если Объект.ДанныеОрганизации.Количество() > 0 Тогда
			...
			ТестВопроса	= Нстр("ka='ცხრილი გასუფთავდება და განმეორებით შეივსება. გავაგრძელოთ?';ru='Табличная часть будет очищена и заполнена повторно. Продолжить?'");
...
Процедура ПеренестиИзДанныхОрганизации(Команда)
	...
	Если Объект.ДанныеОрганизации.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ka='ცხრილი ორგანიზაციის მონაცემებით არ არის შევსებული.';ru='Не заполнена табличная часть с данными организации.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
