...
Процедура ЗаполнитьЗаголовокПоВыбранымДоговорам()
	...
	Если Объект.ДоговорыКонтрагентов.Количество() = 0 Тогда
		...
		ТекстНадписи = НСтр("ka='ხელშეკრულებების სია ცარიელია';ru='Список договоров пуст';en='The list is empty contracts'");
	...
	ИначеЕсли МассивОтмеченных.Количество() = 0 Тогда
		...
		ТекстНадписи = НСтр("ka='ხელშეკრულებები არ არის არჩეული';ru='Договоры не выбраны';en='Contracts not selected'");
...
Процедура ПолучитьДанныеКонтрагента(ДанныеКонтрагента)
	...
	Если ЗаполнитьДанные Тогда
		...
		Объект.ОписаниеВыбораДоговоров = НСтр("ka='არჩეულია კონტრაგენტის ყველა ხელშეკრულება';ru='Выбраны все договоры контрагента';en='Select all counterparty agreements'");
...
"Для привязки необходимо включить видимость соответствующей колонки и указать документ самостоятельно.';ka='სტრიქონი არ არის მიბმული საანგარიშსწორებო დოკუმენტზე. მისაბმელად აუცილებელია შესაბამისი სვეტის ხილვადობის ჩართვა და დოკუმენტის მითითება დამოუკიდებლად.'");
...
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыСверокВзаиморасчетов.Сверена") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПодписьОтветственного) Тогда
			...
			ТекстСообщения = Нстр("ka='ორგანიზაციის პასუხისმგებელი პირი არასწორად არის შევსებული.';ru='Неверно заполнено ответственное лицо организации.';en='Incorrectly filled person in charge of the organization.'");
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПредставительКонтрагента) Тогда
			...
			ТекстСообщения = Нстр("ka='კონტრაგენტის წარმომადგენელი არასწორად არის შევსებული.';ru='Неверно заполнен представитель контрагента.';en='Incorrectly filled representative counterparty.'");
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Иначе
		...
		Объект.ДоговорыКонтрагентов.Очистить();
		Объект.ОписаниеВыбораДоговоров = НСтр("ka='ხელშეკრულებების სია ცარიელია';ru='Список договоров пуст';en='The list is empty contracts'");
...
Процедура ЗаполнитьПоДатамРасчетныхДокументов(Команда)
	...
	Если Объект.ДанныеКонтрагента.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ka='ურთიერთანგარიშსწორებების ცხრილი კონტრაგენტის მონაცემებით ცარიელია.';ru='Табличная часть взаиморасчетов по данным контрагента пуста.';en='Tabular part according mutual counterpart empty.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
...
Процедура ЗаполнитьПоДаннымУчета(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		...
		ТекстСообщения	= НСтр("ka='პერიოდის დასრულების თარიღი არასწორად არის შევსებული.';ru='Неверно заполнена дата окончания периода.';en='Incorrectly filled expiry date of the period.'");
	...
	Если ВызватьПроцедуруЗаполненияТабличнойЧасти Тогда
		...
		Если Объект.ДанныеОрганизации.Количество() > 0 Тогда
			...
			ТестВопроса	= Нстр("ka='ცხრილი გასუფთავდება და განმეორებით შეივსება. გავაგრძელოთ?';ru='Табличная часть будет очищена и заполнена повторно. Продолжить?';en='Tabular part will be cleaned and re-filled. Proceed?'");
...
Процедура ПеренестиИзДанныхОрганизации(Команда)
	...
	Если Объект.ДанныеОрганизации.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ka='ცხრილი ორგანიზაციის მონაცემებით არ არის შევსებული.';ru='Не заполнена табличная часть с данными организации.';en='Not filled with tabular data part of the organization.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
