...
Функция УстановитьСтатус(МассивДокументов, ЗначениеСтатуса) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстОшибки = НСтр("ka='ვერ მოხერხდა დაბლოკვა %Документ%. %ОписаниеОшибки%';ru='Не удалось заблокировать %Документ%. %ОписаниеОшибки%'");
		...
		Исключение
			...
			ТекстОшибки = НСтр("ka='ვერ მოხერხდა ჩაწერა %Документ%. %ОписаниеОшибки%';ru='Не удалось записать %Документ%. %ОписаниеОшибки%'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%",       Выборка.Ссылка);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ОписаниеОшибки%", КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение ТекстОшибки;
...
Функция ПолучитьПредставленияДокументовДляКонтрагентов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",					НСтр("ka='სალაროს შემოსავლის ორდერი';ru='Приходный кассовый ордер'"));
	ВидыДокументовКонтрагента.Вставить("АктВыполненныхРабот",				НСтр("ka='მიღება ( საქონლის, მომსახურების).';ru='Поступление (товаров, услуг)'"));
	ВидыДокументовКонтрагента.Вставить("Взаимозачет",						НСтр("ka='დავალიანების ურთიერთჩათვლა';ru='Взаимозачет задолженности'"));
	ВидыДокументовКонтрагента.Вставить("ЗаказПокупателя",					НСтр("ka='შეკვეთა';ru='Заказ'"));
	ВидыДокументовКонтрагента.Вставить("ЗаказПоставщику", 					НСтр("ka='შეკვეთა';ru='Заказ'"));
	ВидыДокументовКонтрагента.Вставить("КорректировкаРегистров",			НСтр("ka='დავალიანების კორექტირება';ru='Корректировка задолженности'"));
	ВидыДокументовКонтрагента.Вставить("ОтчетКомиссионера",					НСтр("ka='კომიტენტის ანგარიშგება';ru='Отчет комитенту'"));
	ВидыДокументовКонтрагента.Вставить("ОтчетКомитенту",					НСтр("ka='კომისიონერის ანგარიშგება';ru='Отчет комиссионера'"));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеНаСчет", 				НСтр("ka='დანახარჯი ანგარიშიდან';ru='Расход со счета'"));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеВКассу",					НСтр("ka='სალაროს გასავლის ორდერი';ru='Расходный кассовый ордер'"));
	ВидыДокументовКонтрагента.Вставить("РасходСоСчета",						НСтр("ka='ანგარიშზე თანხის მიღება';ru='Поступление на счет'"));
	ВидыДокументовКонтрагента.Вставить("РасходИзКассы",						НСтр("ka='სალაროს შემოსავლის ორდერი';ru='Приходный кассовый ордер'"));
	ВидыДокументовКонтрагента.Вставить("ПриходнаяНакладная",				НСтр("ka='რეალიზაცია (საქონლის, მომსახურების)';ru='Реализация (товаров, услуг)'"));
	ВидыДокументовКонтрагента.Вставить("РасходнаяНакладная", 				НСтр("ka='მიღება ( საქონლის, მომსახურების).';ru='Поступление (товаров, услуг)'"));
	ВидыДокументовКонтрагента.Вставить("СчетНаОплату", 						НСтр("ka='გადახდის მოთხოვნა';ru='Счет на оплату'"));
	ВидыДокументовКонтрагента.Вставить("СчетФактура", 						НСтр("ka='ანგარიშ-ფაქტურა (მიღებული)';ru='Счет-фактура (полученный)'"));
	ВидыДокументовКонтрагента.Вставить("СчетФактураПолученный", 			НСтр("ka='ანგარიშ-ფაქტურა (გაცემული)';ru='Счет-фактура (выданный)'"));
...
Функция ОписаниеРасчетногоДокументаОрганизации(ДокументСсылка, НомерДокумента = "" , Знач ДатаДокумента, ОрганизациюВПредставление = Ложь) Экспорт
	...
	Если ДокументСсылка = Неопределено Тогда
		...
		СтрокаОписания =  НСтр("ka='საანგარიშსწორებო დოკუმენტი %1 %2 წ.';ru='Расчетный документ № %1 от %2 г.'");
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		ОписаниеДокумента = Нстр("ka='შეკვეთა-განწესი';ru='Заказ-наряд'");
	...
	Если ОрганизациюВПредставление Тогда
		...
		УказатьПредставлениеОрганизации = НСтр("ka='(';ru=' ('") + ДокументСсылка.Организация.НаименованиеПолное + НСтр("ka=')';ru=')'");
...
Функция ОписаниеРасчетногоДокументаКонтрагента(ДокументСсылка, Знач НомерДокумента, Знач ДатаДокумента, ОрганизациюВПредставление = Ложь) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		ОписаниеДокумента = Нстр("ka='მიღება ( საქონლის, მომსახურების).';ru='Поступление (товаров, услуг)'");
	...
	Если ПустаяСтрока(ОписаниеДокумента) Тогда
		...
		ОписаниеДокумента = НСтр("ka='საანგარიშსწორებო დოკუმენტი';ru='Расчетный документ'");
...
Функция СформироватьПечатнуюФормуАктБезРасхождений(ДокументПечати, ОбъектыПечати, ВывестиФаксимиле = Ложь)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		ПредставлениеДатыСоставления = НСтр("ka='დოკუმენტი ჩამოყალიბებულია %1';ru='Документ сформирован %1'");
		...
		Если ЗначениеЗаполнено(Шапка.НачалоПериода) Тогда
			...
			ПредставлениеПериода = НСтр("ka='%1-დან %2-ის ჩათვლით არსებულ პერიოდში';ru='в период с %1 по %2'");
		...
		Иначе
			...
			ПредставлениеПериода = НСтр("ka='%1-სთვის არსებული მდგომარეობით';ru='по состоянию на %1'");
		...
		Если ВывестиФаксимиле Тогда
			...
			Если Макет.Области.Найти("ПодвалСчетаСФаксимильнойПечатью") <> Неопределено Тогда
				...
				Иначе
					...
					ТекстСообщения = НСтр("ka='ორგანიზაციისთვის ფაქსიმილე არ არის დაყენებული. ფაქსიმილეს დაყენება ხორციელდება ორგანიზაციის ბარათში, ნაწილი ""ბეჭდვის გამართვა"".';ru='Факсимиле для организации не установлена. Установка факсимиле выполняется в карточке организации, раздел ""Настройка печати"".'");
			...
			Иначе
				...
				ТекстСообщения = НСтр("ka='ყურადღება! შესაძლებელია, რომ გამოიყენებოდეს სამომხმარებლო მაკეტი. ანგარიშების ბეჭდვის საშტატო მექანიზმი შეიძლება არაკორექტულად მუშაობდეს.';ru='ВНИМАНИЕ! Возможно используется пользовательский макет. Штатный механизм печати счетов может работать некоректно.'");
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстСообщения, Неопределено);
				ВывестиПоляПодОригинальнуюПодпись = Истина;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ka='ურთიერთანგარიშსწორებების შედარების აქტი (სხვაობის გარეშე)';ru='Акт сверки взаиморасчетов (без расхождений)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 4;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "АктСДаннымиКонтрагента";
	КомандаПечати.Представление = НСтр("ka='ურთიერთანგარიშსწორებების (კონტრაგენტის მონაცემებთან) შედარების აქტი';ru='Акт сверки взаиморасчетов (с данными контрагента)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
