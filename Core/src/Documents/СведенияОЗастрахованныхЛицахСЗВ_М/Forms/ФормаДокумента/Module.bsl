...
"Продолжить (не рекомендуется)?'");
		...
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ka='გაფრთხილება.';ru='Предупреждение.'"));
	Иначе 
...
Процедура ПроверкаСтороннимиПрограммами(Отказ)
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ka='ჩაშენებული შემოწმებით შემოწმებისას აღმოჩენილია შეცდომები.';ru='При проверке встроенной проверкой обнаружены ошибки.'")
	Иначе	
		ТекстСообщения = НСтр("ka='ჩაშენებული შემოწმების მიერ შემოწმებისას შეცდომები არ არის აღმოჩენილი.';ru='При проверке встроенной проверкой ошибок не обнаружено.'");
	КонецЕсли;
