...
Процедура ПроверитьСЗапросомДальнейшегоДействия(ОповещениеЗавершения = Неопределено)
	...
	Если Отказ Тогда 
		ТекстВопроса = НСтр("ka='კომპლექტში აღმოჩენილია შეცდომები. გავაგრძელოთ (არ არის რეკომენდებული)?';ru='В комплекте обнаружены ошибки."
"Продолжить (не рекомендуется)?'");
		...
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ka='გაფრთხილება.';ru='Предупреждение.';en='A warning.'"));
	Иначе 
...
Процедура ПроверкаСтороннимиПрограммами(Отказ)
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ka='ჩაშენებული შემოწმებით შემოწმებისას აღმოჩენილია შეცდომები.';ru='При проверке встроенной проверкой обнаружены ошибки.';en='When checking the built-in test detected an error.'")
	Иначе	
		ТекстСообщения = НСтр("ka='ჩაშენებული შემოწმების მიერ შემოწმებისას შეცდომები არ არის აღმოჩენილი.';ru='При проверке встроенной проверкой ошибок не обнаружено.';en='When checking the built-in test not found error.'");
	КонецЕсли;
