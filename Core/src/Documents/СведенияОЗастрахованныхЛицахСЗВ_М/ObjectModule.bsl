	...
	Пока ВыборкаДляПроверки.Следующий() Цикл
		...
		Если Не РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(ВыборкаДляПроверки.СтраховойНомерПФР, ТекстСообщения) Тогда		
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელი %1: %2';ru='Сотрудник %1: %2';en='Employee% 1% 2'"),
		...
		Если ВыборкаДляПроверки.ЕстьДубль Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელი %1 დოკუმენტში შეყვანილი იქნა უწინ';ru='Сотрудник %1 был введен в документе ранее';en='Employee% 1 was introduced in document previously'"),
				ВыборкаДляПроверки.Наименование);
		...
		Если Не ВыборкаДляПроверки.РаботаетВОрганизации И Не ТипФормы = Перечисления.ТипыСведенийСЗВ_М.Отменяющая Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელი %1 ორგანიზაციაში არ მუშაობს';ru='Сотрудник %1 не работает в организации';en='Employee% 1 does not work in the organization'"),
				ВыборкаДляПроверки.Наименование);	
		...
		Если ВыборкаДляПроверки.ЯвляетсяВоеннослужащим И Не ТипФормы = Перечисления.ТипыСведенийСЗВ_М.Отменяющая Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელი %1 არ უნდა იქნას შეყვანილი СВЗ-М-ში, ვინაიდან იგი სამხედრო მოსამსახურეა';ru='Сотрудник %1 не должен включаться в СВЗ-М, т.к. является военнослужащим';en='Employee 1% should not be included in the M-BCR, since a military'"),
				ВыборкаДляПроверки.Наименование);
		...
		Если ВыборкаДляПроверки.РаботаетВСтуденческомОтряде И Не ТипФормы = Перечисления.ТипыСведенийСЗВ_М.Отменяющая Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='თანამშრომელი %1 არ უნდა იქნას შეყვანილი СВЗ-М-ში, ვინაიდან იგი სტუდენტურ რაზმში მუშაობს';ru='Сотрудник %1 не должен включаться в СВЗ-М, т.к. работает в студенческом отряде';en='Employee 1% should not be included in the M-BCR, since He is working in student teams'"),
				ВыборкаДляПроверки.Наименование);
