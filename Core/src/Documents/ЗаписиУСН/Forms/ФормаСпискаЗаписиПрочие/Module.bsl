...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = ЭтаФорма.Заголовок + ?(ПустаяСтрока(ЭтаФорма.Заголовок),"",": ")+НСтр("ka='ჩანაწერები შემოსავლების და ხარჯების აღრიცხვის წიგნი';ru='Записи Книги учета доходов и расходов';en='Records Book of income and expense'");
...
Процедура ПересчитатьСДаты(Команда)
	...
	ПоказатьВводДаты(Оповещение, ТекущаяДата(), НСтр("ka='Пересчитать с даты';ru='Пересчитать с даты';en='Counted from the date of'"), ЧастиДаты.Дата);
...
Функция ЗапуститьОбновлениеДанныхНаСервере()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыФункции, 
		НСтр("ka='შემოსავლების და ხარჯების აღრიცხვის წიგნის ჩანაწერების სიის განახლება';ru='Обновление списка записей КУДиР';en='Updating Records KUDiR list'"));
