	...
	Если УчетВалютныхОпераций Тогда
		Для каждого СтрокаТЧ Из Проводки Цикл
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.ВалютаДт) Тогда
				ТекстСообщения = НСтр("ka='სია ""გატარებები""-ს სტრიქონ %НомерСтроки%-ში არსებული სავალუტო ანგარიშისთვის სვეტი ""ვალუტა დტ"" არ არის შევსებული.';ru='Не заполнена колонка ""Валюта Дт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';en='Not filled column ""Currency Debit"" for a foreign currency account in the line%% NomerStroki List ""Transactions.""'");
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаВалДт) Тогда
				ТекстСообщения = НСтр("ka='სია ""გატარებები""-ს სტრიქონ %НомерСтроки%-ში არსებული სავალუტო ანგარიშისთვის სვეტი ""თანხა (ვალ.) დტ"" არ არის შევსებული.';ru='Не заполнена колонка ""Сумма (вал.) Дт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';en='Not filled column ""Amount (shaft). Am"" for a foreign currency account in the line%% NomerStroki List ""Transactions.""'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.ВалютаКт) Тогда
				ТекстСообщения = НСтр("ka='სია ""გატარებები""-ს სტრიქონ %НомерСтроки%-ში არსებული სავალუტო ანგარიშისთვის სვეტი ""ვალუტა კტ"" არ არის შევსებული.';ru='Не заполнена колонка ""Валюта Кт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';en='Not filled column ""Currency Rm"" for a foreign currency account in the line%% NomerStroki List ""Transactions.""'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаВалКт) Тогда
				ТекстСообщения = НСтр("ka='სია ""გატარებები""-ს სტრიქონ %НомерСтроки%-ში არსებული სავალუტო ანგარიშისთვის სვეტი ""თანხა (ვალ.) კტ"" არ არის შევსებული.';ru='Не заполнена колонка ""Сумма (вал.) Кт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';en='Not filled column ""Amount (shaft). Rm"" for a foreign currency account in the line%% NomerStroki List ""Transactions.""'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
