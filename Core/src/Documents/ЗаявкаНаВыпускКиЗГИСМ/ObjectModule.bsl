	...
	Если ВыпущенныеКиЗ.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТЧ Из ВыпущенныеКиЗ Цикл
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ДокументПоступления) 
				...
				ТекстОшибки = НСтр("ka='სია ""გამოშვებული სსნ""-ის სტრიქონ %НомерСтроки%-ში შემოსვლის დოკუმენტი არ არის მითითებული';ru='Не указан документ поступления в строке %НомерСтроки% списка ""Выпущенные КиЗ""';en='Unknown admission document at line% NomerStroki% list ""issued Keyes""'");
		...
		Пока ВыборкаНомераКиЗ.Следующий() Цикл
			...
			Если Не ТекущийДокументПоступленияПравильный Тогда
				...
				ТекстОшибки = НСтр("ka='სია ""გამოშვებული სსნ""-ის სტრიქონ %НомерСтроки%-ში მითითებულია შემოსვლის არაკორექტული დოკუმენტი';ru='Указан некорректный документ поступления в строке %НомерСтроки% списка ""Выпущенные КиЗ""';en='Set an incorrect paper receipts on line% NomerStroki% list ""issued Keyes""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
