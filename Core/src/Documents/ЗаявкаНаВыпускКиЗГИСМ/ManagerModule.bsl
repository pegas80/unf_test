...
Функция ПодтверждениеПоступленияТоваровПоЗаявкеНаВыпускКиЗXML(ДокументСсылка)
	...
	Пока ВыборкаПоДокументамПоступления.Следующий() Цикл
		...
		Иначе
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='დოკუმენტ %1-ის მარკირების სისტემის ნომერი არ არის შევსებული.';ru='Не заполнен номер ГИСМ документа %1.';en='Not filled room GISM document% 1.'"), ДокументСсылка));
		КонецЕсли;
		...
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ka='დოკუმენტ %1-ში შემომავალი დოკუმენტის ნომერი არ არის შევსებული.';ru='В документе %1 не заполнен номер входящего документа.';en='The document% 1 is not filled with the number of the incoming document.'"), ВыборкаПоДокументамПоступления.ДокументПоступления);
		КонецПопытки;
		
		Попытка
			ПодтверждениеПоступления.commit_doc_date = ВыборкаПоДокументамПоступления.ДатаВходящегоДокумента;
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ka='დოკუმენტ %1-ში შემომავალი დოკუმენტის თარიღი არ არის შევსებული.';ru='В документе %1 не заполнена дата входящего документа.';en='% Document 1 is not filled with data incoming document.'"), , ВыборкаПоДокументамПоступления.ДокументПоступления);
		КонецПопытки;
...
Функция ЗакрытиеЗаявкиНаВыпускКиЗXML(ДокументСсылка)
	...
	ОписаниеОперации = СтрШаблон(НСтр("ka='დოკუმენტ %1-ის დახურვა';ru='Закрытие документа %1';en='Closing a document% 1'"), ДокументСсылка);
...
Функция ЗаявкаНаВыпускКиЗXML2_40(ДокументСсылка)
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ka='ემიტენტთან ხელშეკრულების ნომერი არ არის შევსებული.';ru='Не заполнен номер договора с эмитентом.';en='Not full number of the agreement with the issuer.'");
	КонецПопытки;
	
	Попытка
		ЗаявкаНаВыпускКиЗ.contract_date = Шапка.ДатаДоговора;
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ka='ემიტენტთან ხელშეკრულების თარიღი არ არის შევსებული.';ru='Не заполнена дата договора с эмитентом.';en='Not filled with the date of the contract with the issuer.'");
	КонецПопытки;
