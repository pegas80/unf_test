...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДокументСсылкаЗаказНаПроизводство) Экспорт
	...
	Если ДокументСсылкаЗаказНаПроизводство.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
		ВызватьИсключение НСтр("ka='სანარდო განწესი არ შეიძლება შეყვანილი იქნას საწარმოო შეკვეთის საფუძველზე საწყობის მიხედვით!';ru='Сдельный наряд не может быть введен на основании заказа на производство по складу!';en='Piecework outfit can not be entered on the basis of an order for the production of the warehouse!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоСборкеЗапасов(ДокументСсылкаСборкаЗапасов) Экспорт
	...
	Если ДокументСсылкаСборкаЗапасов.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад Тогда
		ВызватьИсключение НСтр("ru = 'Сдельный наряд не может быть введен на основании производства по складу!'; ka = 'სანარდო განწესის შექმნა ვერ მოხერხდა. წარმოების დოკუნმენტში  დამამზადებლად მითითებულია საწყობი!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоСборкеЗапасовСборка(ДокументСсылкаСборкаЗапасов)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылкаСборкаЗапасов,
			"Организация, СтруктурнаяЕдиница, ЗаказНаПроизводство, Дата,");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СборкаЗапасовПродукция.Ссылка.Комментарий,
	...
	|	СборкаЗапасовПродукция.Количество КАК КоличествоПлан,
	...
	|		ПО (СборкаЗапасовПродукция.Спецификация = СпецификацииОперации.Ссылка)
	|ГДЕ
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		НоваяСтрока = Операции.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаИзРезультатаЗапроса);
		НоваяСтрока.Нормочасы = ВыборкаИзРезультатаЗапроса.НормаВремени *ВыборкаИзРезультатаЗапроса.КоличествоФакт ;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ОтразитьРасчетыСПенсионнымФондом(ДополнительныеСвойства, Движения, Отказ);//lazare_ПФ
	УправлениеНебольшойФирмойСервер.ОтразитьРасчетыНДФЛ(ДополнительныеСвойства, Движения, Отказ);//lazare_НДФЛ
	УправлениеНебольшойФирмойСервер.ОтразитьДоходыИРасходы(ДополнительныеСвойства, Движения, Отказ);//lazare
	УправлениеНебольшойФирмойСервер.ОтразитьРасчетыПоНалогам(ДополнительныеСвойства, Движения, Отказ);//lazare
