...
Процедура ПриОткрытии(Отказ)
	...
	Если НеОткрыватьФорму Тогда
		ВызватьИсключение НСтр("ka='Нет данных для оформления пересортицы!';ru='Нет данных для оформления пересортицы!';en='No data for the design of regrading!'");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьПоИнвентаризацииВсеРасходжения(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(ОповещениеПослеОтветаНаВопрос, НСтр("ka='Использовать документ-основание или выбрать другую инвентаризацию?';ru='Использовать документ-основание или выбрать другую инвентаризацию?';en='Use document-base or to choose another inventory?'"), КнопкиВопрос);
...
Процедура ЗаполнитьВсемиОтрицательнымиОстаткамиНаСкладе(Команда)
	...
	Если НЕ ДанныеДобавлены Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Нет данных для заполнения.';ru='Нет данных для заполнения.';en='No information required.'"));
	КонецЕсли;
...
Процедура ОчиститьСписываемуюНоменклатуру(Команда)
	...
	ПоказатьВопрос(
		ОписаниеОповещенияВопрос, 
		НСтр("ka='Очистить списываемую номенклатуру?';ru='Очистить списываемую номенклатуру?';en='Clear written off range?'"), 
...
Процедура ОчиститьПриходуемуюНоменклатуру(Команда)
	...
	ПоказатьВопрос(
		ОписаниеОповещенияВопрос, 
		НСтр("ka='Очистить приходуемую номенклатуру?';ru='Очистить приходуемую номенклатуру?';en='Clear range of credited?'"), 
		РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
...
Процедура ПоменятьМестамиДанныеОСписанииИОприходовании(Команда)
	...
	ПоказатьВопрос(
		ОписаниеОповещенияВопрос, 
		НСтр("ka='Поменять данные о списании и оприходовании?';ru='Поменять данные о списании и оприходовании?';en='Change the data and posting of write-off?'"), 
		РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
...
Процедура ОчиститьТабличнуюЧасть(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Табличная часть не заполнена.';ru='Табличная часть не заполнена.';en='Tabular section is not filled.'"));
	Иначе
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ОчиститьТабличнуюЧастьЗавершение", ЭтотОбъект);
		ПоказатьВопрос(
			ОписаниеОповещенияВопрос, 
			НСтр("ka='Табличная часть будет очищена, продолжить?';ru='Табличная часть будет очищена, продолжить?';en='Tabular section is cleared, continue?'"),
			РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
Процедура СопоставитьАвтоматически(Команда)
	...
	Если Результат = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ka='Среди списываемых и приходуемых товаров нет товаров, по которым возможно провести автоматический зачет.';ru='Среди списываемых и приходуемых товаров нет товаров, по которым возможно провести автоматический зачет.';en='Among the written off and no items of credited products, which may carry out an automatic set-off.'"));
	КонецЕсли;
...
Процедура Сопоставить2Выделенных(Команда)
	...
	Если Элементы.Запасы.ВыделенныеСтроки.Количество() <> 2 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Выберите, удерживая клавишу ""Ctrl"", в табличной части 2 строки и нажмите эту кнопку';ru='Выберите, удерживая клавишу ""Ctrl"", в табличной части 2 строки и нажмите эту кнопку';en='Select by holding ""Ctrl"" key, in the tables of 2 lines, and then press the button'"));
		Возврат;
	КонецЕсли;
	...
	Если (ЗначениеЗаполнено(Строка1.Номенклатура) И ЗначениеЗаполнено(Строка1.НоменклатураОприходование))
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ka='В одной строке должна быть выбрана только списываемая номенклатура, а в другой только приходуемая номенклатура.';ru='В одной строке должна быть выбрана только списываемая номенклатура, а в другой только приходуемая номенклатура.';en='One line should be selected only debited nomenclature, and the other only of credited nomenclature.'"));
		Возврат;
