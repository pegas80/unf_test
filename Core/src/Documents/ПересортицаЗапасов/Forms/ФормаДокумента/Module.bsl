...
Процедура ПриОткрытии(Отказ)
	...
	Если НеОткрыватьФорму Тогда
		ВызватьИсключение НСтр("ru = 'Нет данных для оформления пересортицы!'; ka = 'ხელახალი შეფასების მონაცემები არ არის!'; en = 'No data for the design of regrading!'");
		Отказ = Истина;
	КонецЕсли;
...
Функция СформироватьНадписьЦены(СтруктураНадписи)
	...
	Иначе
		ТекстНадписи = ТекстНадписи + НСтр("ka='ფასების სახეობის მითითება';ru='указать вид цен';en='specify the kind of price'");
	КонецЕсли;
...
Процедура ЗаполнитьПоИнвентаризацииВсеРасходжения(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(ОповещениеПослеОтветаНаВопрос, НСтр("ru = 'Использовать документ-основание или выбрать другую инвентаризацию?'; ka = 'გამოვიყენო საფუძველი დოკუმენტი თუ სხვა ინვენტარიზაცია ავირჩიო?'; en = 'Use document-base or to choose another inventory?'"), КнопкиВопрос);
...
Процедура ЗаполнитьВсемиОтрицательнымиОстаткамиНаСкладе(Команда)
	...
	Если НЕ ДанныеДобавлены Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Нет данных для заполнения.'; ka = 'არ არსებობს ინფორმაცია, რომ შეავსოთ.'; en = 'No information required.'"));
	КонецЕсли;
...
Процедура ОчиститьУсловиеОтбораПоНоменклатуре()
	...
	Элементы.УстановитьОтборПоНоменклатуре.Заголовок = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен';en='The Selection is Not Set'");
...
Процедура ОчиститьУсловиеОтбораПоГруппамНоменклатуры()
	...
	Элементы.УстановитьОтборПоГруппамНоменклатуры.Заголовок = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен';en='The Selection is Not Set'");
...
Процедура ОчиститьУсловиеОтбораПоКатегориямНоменклатуры()
	...
	Элементы.УстановитьОтборПоКатегориямНоменклатуры.Заголовок = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен';en='The Selection is Not Set'");
...
Функция СформироватьЗаголовокОтбораИзСписка(СписокЭлементов)
	...
	Если ЗаголовокОтбораСтрока = "" Тогда
		ЗаголовокОтбораСтрока = НСтр("ka='შერჩევა არ არის დაყენებული';ru='Отбор не установлен';en='The Selection is Not Set'");
	КонецЕсли;
...
Процедура ОчиститьСписываемуюНоменклатуру(Команда)
	...
	ПоказатьВопрос(
		ОписаниеОповещенияВопрос, 
		НСтр("ru = 'Очистить списываемую номенклатуру?'; ka = 'წაიშალოს ნომენკლატურები?'; en = 'Clear written off range?'"), 
...
Процедура ОчиститьПриходуемуюНоменклатуру(Команда)
	...
	ПоказатьВопрос(
		ОписаниеОповещенияВопрос, 
		НСтр("ru = 'Очистить приходуемую номенклатуру?'; ka = 'გასუფთავდეს შემომავალი ნომენკლატურები?'; en = 'Clear range of credited?'"), 
		РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
...
Процедура ПоменятьМестамиДанныеОСписанииИОприходовании(Команда)
	...
	ПоказатьВопрос(
		ОписаниеОповещенияВопрос, 
		НСтр("ru = 'Поменять данные о списании и оприходовании?'; ka = 'შეცვალოთ მონაცემები ჩამოწერისა და შემოასავალში აყვანის შესახებ?'; en = 'Change the data and posting of write-off?'"), 
		РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
...
Процедура ОчиститьТабличнуюЧасть(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Табличная часть не заполнена.'; ka = 'ცხრილური ნაწილი არ არის შევსებული.'; en = 'Tabular section is not filled.'"));
	Иначе
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ОчиститьТабличнуюЧастьЗавершение", ЭтотОбъект);
		ПоказатьВопрос(
			ОписаниеОповещенияВопрос, 
			НСтр("ru = 'Табличная часть будет очищена, продолжить?'; ka = 'ცხრილი გასუფთავდება, გაგრძელდეს?'; en = 'Tabular section is cleared, continue?'"),
			РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
Процедура СопоставитьАвтоматически(Команда)
	...
	Если Результат = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Среди списываемых и приходуемых товаров нет товаров, по которым возможно провести автоматический зачет.'; ka = 'ჩამოწერილ და მიღებულ საქონელს შორის არ არის ისეთი საქონელი, რომლის ავტომატური ჩათვლის გატარება შეიძლება.'; en = 'Among the written off and no items of credited products, which may carry out an automatic set-off.'"));
	КонецЕсли;
...
Процедура Сопоставить2Выделенных(Команда)
	...
	Если Элементы.Запасы.ВыделенныеСтроки.Количество() <> 2 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Выберите, удерживая клавишу ""Ctrl"", в табличной части 2 строки и нажмите эту кнопку'; ka = '""Ctrl"" კლავიშის დაჭერისას აირჩიეთ ცხრილის ნაწილის 2 სტრიქონი და დააჭირეთ ამ ღილაკს'; en = 'Select by holding ""Ctrl"" key, in the tables of 2 lines, and then press the button'"));
		Возврат;
	КонецЕсли;
	...
	Если (ЗначениеЗаполнено(Строка1.Номенклатура) И ЗначениеЗаполнено(Строка1.НоменклатураОприходование))
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'В одной строке должна быть выбрана только списываемая номенклатура, а в другой только приходуемая номенклатура.'; ka = 'ერთ სტრიქონში არჩეული უნდა იყოს მხოლოდ ჩამოწერილი ნივთი, მეორეში კი მხოლოდ მიღებული.'; en = 'One line should be selected only debited nomenclature, and the other only of credited nomenclature.'"));
		Возврат;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные, ЭтоОприходование = Ложь)
	...
	СтруктураДанные.Вставить("КоличествоДни", СтруктураДанные.Номенклатура.СрокГодности);//+Lazare
...
Процедура УправлениеВидимостью()
	...
	Элементы.Корреспонденция.Видимость = Не Объект.ПриходоватьТоварыПоСебестоимостиСписания И НЕ ИспользоватьТолькоРегламентированныйПланСчетов;
	Элементы.КорреспонденцияОприходование.Видимость = Не Объект.ПриходоватьТоварыПоСебестоимостиСписания И НЕ ИспользоватьТолькоРегламентированныйПланСчетов;
	Элементы.КорреспонденцияРегл.Видимость = Не Объект.ПриходоватьТоварыПоСебестоимостиСписания;
	Элементы.КорреспонденцияОприходованиеРегл.Видимость = Не Объект.ПриходоватьТоварыПоСебестоимостиСписания;
	//--- Дмгг
...
&НаКлиенте
Процедура КорреспонденцияОприходованиеРеглПриИзменении(Элемент)
	Если Не ИспользоватьТолькоРегламентированныйПланСчетов Тогда
		РегламентированныйУчетСервер.УстановитьУпрСчетИзПланаСчетовХозрасчетный(Объект.КорреспонденцияОприходованиеРегл, Объект.КорреспонденцияОприходование);
	КонецЕсли;	
КонецПроцедуры
