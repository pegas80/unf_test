		...
		Для Каждого СтрокаТЧИтогПоGTIN Из Товары.Строки Цикл
			...
			Для Каждого СтрокаТЧ Из СтрокаТЧИтогПоGTIN.Строки Цикл
				...
				Иначе
					...
					ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
						СообщениеXML,
						СтрШаблон(НСтр("ka='Для номенклатуры %1 найдены строки в которых не указан номер КиЗ или TID.';ru='Для номенклатуры %1 найдены строки в которых не указан номер КиЗ или TID.';en='For the range of 1% line are found that do not indicate the number of Keys or TID.'"),
							ИнтеграцияГИСМПереопределяемый.ПредставлениеНоменклатуры(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика)));
