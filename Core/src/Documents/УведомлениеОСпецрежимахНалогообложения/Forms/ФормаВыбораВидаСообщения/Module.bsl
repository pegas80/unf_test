...
Процедура ВидыСообщенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Организация) И СозданиеСообщения Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ka='მიუთითეთ ორგანიზაცია';ru='Укажите организацию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),, "Организация");
		Возврат;
	КонецЕсли;
		...
		Если СозданиеСообщения Тогда
			...
			Если ЭтоЮридическоеЛицо Тогда
				...
				Если НЕ ДанноеСообщениеДоступноДляОрганизации(ВидУведомления) Тогда
					...
					ТекстПредупреждения = НСтр("ka='Уведомление ""%1"" можно создавать только для ИП';ru='Уведомление ""%1"" можно создавать только для ИП'");
			...
			Иначе
				...
				Если НЕ ДанноеСообщениеДоступноДляИП(ВидУведомления) Тогда
					...
					ТекстПредупреждения = НСтр("ka='Уведомление ""%1"" можно создавать только для организаций';ru='Уведомление ""%1"" можно создавать только для организаций'");
					ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%1", Строка(ВидУведомления));
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = ТекстПредупреждения;
					Сообщение.Сообщить();
