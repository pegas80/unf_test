...
Функция ВыгрузитьДокумент(УникальныйИдентификатор = Неопределено) Экспорт
	...
	Попытка
		...
		Если СодержаниеВыгрузки = Неопределено Тогда 
			...
			Сообщение.Текст = НСтр("ka='Выгрузка данного уведомления не предусмотрена';ru='Выгрузка данного уведомления не предусмотрена';en='Uploading this notice is not provided'");
			Сообщение.Сообщить();
