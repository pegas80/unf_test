...
Процедура ПереименоватьОтправкиНаПолучениеПатентаВЖурнале() Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Электронный документооборот с контролирующими органами. Обработка обновления';ru='Электронный документооборот с контролирующими органами. Обработка обновления'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьИменаОтчетов() Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ka='Обновление уведомлений завершено с ошибкой';ru='Обновление уведомлений завершено с ошибкой'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
...
Процедура ЗаполнитьДаннымиТС1(ДокОбъект, Выборка)
	...
	Исключение
		...
		СтрОш = НСтр("ka='Не удалось получить ФИО подписанта';ru='Не удалось получить ФИО подписанта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл 
			...
			Исключение
				ОтменитьТранзакцию();
				Ошибка = ИнформацияОбОшибке();
				СтрОш = НСтр("ka='Создание уведомлений ТС-1, ТС-2';ru='Создание уведомлений ТС-1, ТС-2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЦикла;
	Исключение
		Ошибка = ИнформацияОбОшибке();
		СтрОш = НСтр("ka='Ошибка при выполнении УведомлениеОСпецрежимахНалогообложения.СоздатьУведомления()';ru='Ошибка при выполнении УведомлениеОСпецрежимахНалогообложения.СоздатьУведомления()'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
#КонецОбласти
