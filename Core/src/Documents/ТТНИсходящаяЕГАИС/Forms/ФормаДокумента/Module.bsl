...
Процедура ПодобратьСправкиБ(Команда)
	...
	Если Объект.Грузоотправитель.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='ტვირთის გამგზავნი არ არის არჩეული';ru='Не выбран грузоотправитель'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузкаДокумента_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ka='დოკუმენტი წარმატებით გადმოიტვირთა უნივერსალურ სატრანსპორტო მოდულში.';ru='Документ успешно выгружен в УТМ.'"));
