...
Процедура УстановитьУсловноеОформлениеСтатусаОбработки(Форма, ОформляемоеПоле = "Список", ПутьКДанным = "Список.СтатусОбработки") Экспорт
	...
	ПредставлениеЭлемента = НСтр("ka='ხორციელდება დოკუმენტის გადაცემა ესასს-ში';ru='Документ передается в ЕГАИС';en='The document is transmitted to EGAIS'");
	...
	ПредставлениеЭлемента = НСтр("ka='მიღებულია ესასს-ში გადაცემის შეცდომა';ru='Получена ошибка передачи в ЕГАИС';en='Received a transmission error in EGAIS'");
...
Функция ИнициализироватьДанныеДокументаДляВыгрузки(ДокументСсылка, ВидДокумента) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ka='Неподдерживаемый вид документа %1 для акта постановки на баланс';ru='Неподдерживаемый вид документа %1 для акта постановки на баланс';en='Unsupported document type% 1 for the production act on the balance sheet'");
