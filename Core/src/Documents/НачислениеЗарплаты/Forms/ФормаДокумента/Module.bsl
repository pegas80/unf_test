...
Процедура ЗаполнитьПоПодразделениюРегл()
	...
	|	ДокументНачисление.СчетЗатрат КАК СчетЗатрат,
	|	ДокументНачисление.СчетЗатратРегл КАК СчетЗатратРегл,
	|	ЕСТЬNULL(ДанныеТабеля.ОтработаноДней, 0) КАК ОтработаноДней,
	|	ЕСТЬNULL(ДанныеТабеля.ОтработаноЧасов, 0) КАК ОтработаноЧасов,
	|	ДокументНачисление.ВидУдержанияПФ,
	|	ЕСТЬNULL(ДанныеТабеля.ВЦеломЗаПериод, ЛОЖЬ) КАК ВЦеломЗаПериод
	|ИЗ
	|	ТаблицаНачисленияУдержания КАК ДокументНачисление
...
Процедура РассчитатьПоФормулам()
	...
	Для каждого СтрокаНачислений Из Объект.НачисленияУдержания Цикл
		...
		Исключение
			ТекстСообщения = НСтр("ka='ვერ მოხერხდა დარიცხვის თანხის გაანგარისება სტრიქონ №%НомерСтроки%. შეიძლება, ფორმულა შეიცავს შეცდომებს არნ შეუვსებელია მაჩვენებლები.';ru='Не удалось рассчитать сумму начисления  в строке №%НомерСтроки%. Возможно, формула содержит ошибку или не заполнены показатели.'");
	...
	//ПодольскийАИ #УчетПроектов 10.11.2020 {
	//При обновлении подвала теряются реквизиты объекта, добавленные в форме
	//Закомментировал, так как после вызова текущей процедуры следует обновление подвала формы
	//ОбновитьПодвалФормы();
	//ПодольскийАИ #УчетПроектов 10.11.2020 }
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			Если Не ЗаполненТарифНаТравматизм И Не ЗаполненТарифСтраховыхВзносов Тогда
				ТекстВопроса = Нстр("ka='ორგანიზაციაში არ არის შევსებული ტრავმატიზმის ტარიფი და სადაზღვევო შენატანების ტარიფი. შევავსოთ?';ru='В организации не заполнены тариф на травматизм и тариф страховых взносов. Заполнить?'");
				ПоляСОшибками.Вставить("КлючТарифаСтраховыхВзносов.ВидТарифа", НСтр("ka='სადაზღვევო შენატანების ტარიფი არ არის მითითებული';ru='Не указан тариф страховых взносов'"));
				ПоляСОшибками.Вставить("Объект.ТарифНаТравматизм",НСтр("ka='ტრავმატიზმის გრაფიკი არ არის მითითებული';ru='Не указан тариф на травматизм'"));
				Расшифровка.Вставить("ПараметрыРасшифровки",ПоляСОшибками);
				
				Возврат Ложь;
			ИначеЕсли Не ЗаполненТарифНаТравматизм Тогда
				ТекстВопроса = Нстр("ka='ორგანიზაციაში არ არის შევსებული ტრავმატიზმის ტარიფი. შევავსოთ?';ru='В организации не заполнены тариф на травматизм. Заполнить?'");
				ПоляСОшибками.Вставить("Объект.ТарифНаТравматизм",НСтр("ka='ტრავმატიზმის გრაფიკი არ არის მითითებული';ru='Не указан тариф на травматизм'"));
				Расшифровка.Вставить("ПараметрыРасшифровки",ПоляСОшибками);
				
				Возврат Ложь;
			ИначеЕсли Не ЗаполненТарифСтраховыхВзносов Тогда
				ТекстВопроса = Нстр("ka='ორგანიზაციაში არ არის შევსებული სადაზღვევო შენატანების ტარიფი. შევავსოთ?';ru='В организации не заполнены тариф страховых взносов. Заполнить?'");
				ПоляСОшибками.Вставить("КлючТарифаСтраховыхВзносов.ВидТарифа", НСтр("ka='სადაზღვევო შენატანების ტარიფი არ არის მითითებული';ru='Не указан тариф страховых взносов'"));
				Расшифровка.Вставить("ПараметрыРасшифровки",ПоляСОшибками);
...
Процедура Рассчитать(Команда)
	...
	Если Не ПроверитьЗаполнениеРеквизитовОрганизацииДляРасчетаВзносов(Объект.Организация, Объект.ПериодРегистрации, ТекстВопроса, РассчитыватьБезВзносов, Расшифровка) Тогда
			...
			СписокКнопок.Добавить(КодВозвратаДиалога.Да, Нстр("ka='შევსება';ru='Заполнить'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, Нстр("ka='შენატანების გარეშე გამოთვლა';ru='Рассчитать без взносов'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Отмена, Нстр("ka='გაუქმება';ru='Отмена'"));
...
Процедура Заполнить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ka='ქვეგანყოფილება შეუვსებელია! დოკუმენტის შევსება გაუქმებულია.';ru='Не заполнено подразделение! Заполнение документа отменено.'");
	...
	Если Объект.НачисленияУдержания.Количество() > 0 И Объект.НалогиНаДоходы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение1", ЭтотОбъект), НСтр("ka='დოკუმენტის ცხრილები გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличные части документа будут очищены! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	...
	ИначеЕсли Объект.НачисленияУдержания.Количество() > 0 ИЛИ Объект.НалогиНаДоходы.Количество() > 0 ИЛИ
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტის ცხრილი გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличная часть документа будет очищена! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат; 
...
Процедура ВалютаДокументаПриИзменении(Элемент)
	...
	Если Объект.НачисленияУдержания.Количество() > 0 
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВалютаДокументаПриИзмененииЗавершение", ЭтотОбъект), НСтр("ka='ცხრილები გასუფთავდება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Табличные части будут очищены! Продолжить выполнение операции?'"), Режим, 0);
		Возврат;
