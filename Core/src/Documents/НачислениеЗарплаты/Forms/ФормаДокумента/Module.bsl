...
Процедура ЗаполнитьПоПодразделениюРегл()
	...
	// Олег Литвиненко 26.08.22 --->
	|		МАКСИМУМ(ВЫБОР
	|				КОГДА ПлановыеНачисленияИУдержания.Период < &НачалоМесяца
	|					ТОГДА &НачалоМесяца
	|				ИНАЧЕ ПлановыеНачисленияИУдержания.Период
	...
	// Олег Литвиненко 26.08.22 --->
	ПредыдущийВидНачисления = Справочники.ВидыНачисленийИУдержаний.ПустаяСсылка(); 
	Сутки = 60*60*24;
	// Олег Литвиненко <---
	Пока ВыборкаСотрудник.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			// Тут ли должен быть этот фрагмет? Может его в ту ветку, где добавдяется таблица НачисленияУдержания?
			Если Выборка.ДругоеПодрУвольнение Тогда
				ЗаменитьДатуМассив = Объект.НачисленияУдержания.НайтиСтроки(Новый Структура("ДатаОкончания, Сотрудник", КонецМесяца, Выборка.Сотрудник));
				Для каждого ЭлементМассива Из ЗаменитьДатуМассив Цикл
					ЭлементМассива.ДатаОкончания = Выборка.ДатаДействияНачало - 60*60*24;
			...
			// Олег Литвиненко 26.08.22. ЗАКОММЕНТИРОВАЛ
			// Перенес в ту ветку, где добавдяется таблица НачисленияУдержания
			//ЗаменитьДатуМассив = Объект.НачисленияУдержания.НайтиСтроки(Новый Структура("ДатаОкончания, Сотрудник, ВидНачисленияУдержания", КонецМесяца, Выборка.Сотрудник, Выборка.ВидНачисленияУдержания));
			//Для каждого ЭлементМассива Из ЗаменитьДатуМассив Цикл
			//	ЭлементМассива.ДатаОкончания = Выборка.ДатаДействияНачало - 60*60*24;
			//КонецЦикла;
			...
			Иначе
					...
					// Олег Литвиненко 26.08.22 --->
					// ++lyskov 05.03.2023 Исправление логики: ограничиваем период действия предыдущих начислений,
					//                     если следующее действует не с начала месяца.
					Если (Выборка.ДатаДействияНачало - Объект.ПериодРегистрации) > Сутки Тогда
						ЗаменитьДатуМассив = Объект.НачисленияУдержания.НайтиСтроки(Новый Структура("ДатаОкончания, Сотрудник, ВидНачисленияУдержания", КонецМесяца, Выборка.Сотрудник, ПредыдущийВидНачисления));
						Для каждого ЭлементМассива Из ЗаменитьДатуМассив Цикл
							ПредыдущаяДата = Выборка.ДатаДействияНачало - Сутки;
							ЭлементМассива.ДатаОкончания = ?(ЭлементМассива.ДатаОкончания > ПредыдущаяДата, ПредыдущаяДата, ЭлементМассива.ДатаОкончания);
						КонецЦикла;
					КонецЕсли;
					// --lyskov 05.03.2023
					// Олег Литвиненко <---
					...
					// Олег Литвиненко 26.08.22 --->
					ПредыдущийВидНачисления				= Выборка.ВидНачисленияУдержания;
					// Олег Литвиненко <---
					...
					Если Объект.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение
						...
						//НоваяСтрока.СчетЗатратРегл = ПланыСчетов.Хозрасчетный.ПустаяСсылка(); //+++ Дмгг
	...
	ЗаполнитьЗаймыСотрудникам(); // Прочие расчеты. Займы сотрудникам. 
	ДополнитьНачисленияУдержанияПоОтпускам();
	РассчитатьПоФормулам();
...
&НаСервере 
Процедура ДополнитьНачисленияУдержанияПоОтпускам();        
	
	//МассивСотрудников = Объект.НачисленияУдержания.Выгрузить().ВыгрузитьКолонку("Сотрудник");  
	
	МассивТиповУдержаний = Новый Массив;
	МассивТиповУдержаний.Добавить(Перечисления.ТипыНачисленийИУдержаний.Удержание);
	МассивТиповУдержаний.Добавить(Перечисления.ТипыНачисленийИУдержаний.УдержаниеПФ);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.Организация КАК Организация,
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.Сотрудник КАК Сотрудник,
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.СуммаВалОборот КАК Размер,
		|	СотрудникиСрезПоследних.Должность КАК Должность
		|ИЗ
		|	РегистрНакопления.ОдноразовыеНачисленияУдержанияОтпуска.Обороты(
		|			&НачалоПериода,
		|			&КонецПериода,
		|			Регистратор,
		|			Организация = &Организация
		...
		|				И ВидНачисленияУдержания.Тип = &ТипНачисления) КАК ОдноразовыеНачисленияУдержанияОтпускаОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Сотрудники.СрезПоследних(&КонецПериода, ) КАК СотрудникиСрезПоследних
		|		ПО ОдноразовыеНачисленияУдержанияОтпускаОбороты.Сотрудник = СотрудникиСрезПоследних.Сотрудник
		|			И ОдноразовыеНачисленияУдержанияОтпускаОбороты.Организация = СотрудникиСрезПоследних.Организация
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.ВидНачисленияУдержания КАК ВидНачисленияУдержания,
		|	ОдноразовыеНачисленияУдержанияОтпускаОбороты.СуммаВалОборот КАК СуммаОборот,
		|	СотрудникиСрезПоследних.Должность КАК Должность
		|ИЗ
		|	РегистрНакопления.ОдноразовыеНачисленияУдержанияОтпуска.Обороты(
		|			&НачалоПериода,
		...
		|				И СтруктурнаяЕдиница = &СтруктурнаяЕдиница
		|				И Валюта = &ВалютаДокумента
		|				И ВидНачисленияУдержания.Тип В (&ТипУдержания)) КАК ОдноразовыеНачисленияУдержанияОтпускаОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Сотрудники.СрезПоследних КАК СотрудникиСрезПоследних
		|		ПО ОдноразовыеНачисленияУдержанияОтпускаОбороты.Организация = СотрудникиСрезПоследних.Организация
		|			И ОдноразовыеНачисленияУдержанияОтпускаОбороты.Сотрудник = СотрудникиСрезПоследних.Сотрудник";
	...
	Запрос.УстановитьПараметр("ТипУдержания", МассивТиповУдержаний);
	
	МассивРезультатов = Запрос.ВыполнитьПакет(); 
	тзНачислений = МассивРезультатов[0].Выгрузить();
	ТзУдержаний = МассивРезультатов[1].Выгрузить(); 
	
	Для Каждого Строк Из тзНачислений Цикл                      
		
		НоваяСтрока							= Объект.НачисленияУдержания.Добавить();   
		
		НоваяСтрока.Сотрудник 				= Строк.Сотрудник;
		НоваяСтрока.Должность 				= Строк.Должность;
		НоваяСтрока.ВидНачисленияУдержания 	= Строк.ВидНачисленияУдержания;
		НоваяСтрока.СчетЗатратРегл		 	= Строк.ВидНачисленияУдержания.СчетЗатратРегл;
		...
		НоваяСтрока.Размер 					= Строк.Размер;

	КонецЦикла;    
	
	Для Каждого Строк Из ТзУдержаний Цикл                      
		
		НоваяСтрока							= Объект.Удержания.Добавить();   
		
		НоваяСтрока.Сотрудник 				= Строк.Сотрудник;
		//НоваяСтрока.Должность 				= Строк.Должность;
		НоваяСтрока.ВидУдержания 			= Строк.ВидНачисленияУдержания;
		//НоваяСтрока.СчетЗатратРегл		 	= Строк.ВидНачисленияУдержания.СчетЗатратРегл;
		//НоваяСтрока.ДатаНачала 				= НачалоМесяца(Объект.ПериодРегистрации);
		//НоваяСтрока.ДатаОкончания 			= КонецМесяца(Объект.ПериодРегистрации);
		НоваяСтрока.Сумма 					= Строк.СуммаОборот;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+++ Дмгг, 01.05.2025, ONESOFT-4039
	//ОтображениеПериодаРегистрации = Формат(Объект.ПериодРегистрации, "ДФ='MMMM yyyy'");
	ОтображениеПериодаРегистрации = Формат(Объект.ПериодРегистрации, НСтр("ru = 'Л=ru_RU'; ka = 'Л=ka_GE'; en = 'Л=en_GB'") + "; ДФ='MMMM yyyy'");
    //--- Дмгг, 01.05.2025, ONESOFT-4039
...
//&НаСервере
//Процедура УстановитьОтборДляНаправлениеДеятельности()

//	Если НЕ ПолучитьФункциональнуюОпцию("ПоказатьРеквизитыУправленческогоУчета")
//		И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы")
//		Тогда
//	
//		Элементы.НачисленияУдержанияНаправлениеДеятельности.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
//	
//	Иначе
//	
//		МассивПараметров = Новый Массив;
//		//МассивПараметров.Добавить(Новый СвязьПараметраВыбора("ВидОперации", "Объект.ВидОперации", РежимИзмененияСвязанногоЗначения.Очищать));
//		МассивПараметров.Добавить(Новый СвязьПараметраВыбора("СчетЗатрат", "Объект.НачисленияУдержанияСчетЗатрат", РежимИзмененияСвязанногоЗначения.Очищать));
//		Элементы.НачисленияУдержанияНаправлениеДеятельности.СвязиПараметровВыбора = Новый ФиксированныйМассив(МассивПараметров);
//	
//	КонецЕсли; 	

//КонецПроцедуры // УстановитьОтборДляНаправлениеДеятельности()

//&НаКлиенте
//Процедура НачисленияУдержанияНаправлениеДеятельностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
//	
//	УстановитьОтборДляНаправлениеДеятельности();
//	
//КонецПроцедуры
...
Процедура ЗаполнитьРассчитатьОтработаноеВремяПоПроектам(ТолькоРасчет = Ложь)
	...
	Пока ВыборкаСотрудник.Следующий() Цикл
		...
		Пока ВыборкаВидНачисления.Следующий() Цикл
			...
			// Размер начисления (условия оплаты) может изменяться в течение месяца, необходимо разделить расчет по периодам действия условий оплаты.
			ВыборкаРазмерНачисления = ВыборкаВидНачисления.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаРазмерНачисления.Следующий() Цикл
			
				ОсталосьСумма = ВыборкаРазмерНачисления.Сумма;
				ОсталосьСуммаПФ = ВыборкаРазмерНачисления.СуммаПФ;
				ОтработаноЧасовПоПроектуВсего = ВыборкаРазмерНачисления.ОтработаноЧасовПоПроекту;
				ОтработаноДнейПоПроектуВсего  = ВыборкаРазмерНачисления.ОтработаноДнейПоПроекту;
				
				ВыборкаДет = ВыборкаРазмерНачисления.Выбрать();
				Пока ВыборкаДет.Следующий() Цикл
					...
					Если СтрокаНачисления.ЗависитОтЧасов Тогда
						// ++lyskov 05.03.2023 Исправлено: коэффициент должен рассчитываться от итогового времени по проекту, а не от времени работы по графику.
						СтрокаНачисления.КоэффициентПоПроекту = ?(ОтработаноЧасовПоПроектуВсего = 0, 0, Окр(ВыборкаДет.ОтработаноЧасовПоПроекту / ОтработаноЧасовПоПроектуВсего, 4));
					Иначе
						// ++lyskov 05.03.2023 Исправлено: коэффициент должен рассчитываться от итогового времени по проекту, а не от времени работы по графику.
						СтрокаНачисления.КоэффициентПоПроекту = ?(ОтработаноДнейПоПроектуВсего = 0, 0, Окр(ВыборкаДет.ОтработаноДнейПоПроекту / ОтработаноДнейПоПроектуВсего, 4));
					КонецЕсли;
			...
			КонецЦикла; // По размеру начислений
		КонецЦикла; // По виду начислений
		
	КонецЦикла; // По сотруднику
...
Функция ПолучитьТекстЗапросаДляЗаполненияПоОтработанномуВремени()
	...
	// ++lyskov 04.03.2023 Исправлен расчет отработанного времени для случаев, когда начисления действуют только часть месяца.
	НовыйТекст =
	"ВЫБРАТЬ
	...
	|	ТаблицаНачисленияУдержания.ОтработаноДней КАК ОтработаноДней,
	|	ТаблицаНачисленияУдержания.ОтработаноЧасов КАК ОтработаноЧасов,
	...
	|	ТаблицаНачисленияУдержания.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ТаблицаНачисленияУдержания.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ТаблицаНачисленияУдержания.РучнаяКорректировка КАК РучнаяКорректировка,
	|	ТаблицаНачисленияУдержания.Показатель1 КАК Показатель1,
	|	ТаблицаНачисленияУдержания.Представление1 КАК Представление1,
	|	ТаблицаНачисленияУдержания.Значение1 КАК Значение1,
	|	ТаблицаНачисленияУдержания.Показатель2 КАК Показатель2,
	|	ТаблицаНачисленияУдержания.Представление2 КАК Представление2,
	|	ТаблицаНачисленияУдержания.Значение2 КАК Значение2,
	|	ТаблицаНачисленияУдержания.Показатель3 КАК Показатель3,
	|	ТаблицаНачисленияУдержания.Представление3 КАК Представление3,
	|	ТаблицаНачисленияУдержания.Значение3 КАК Значение3,
	|	ТаблицаНачисленияУдержания.ТипСчета КАК ТипСчета,
	|	ТаблицаНачисленияУдержания.СчетЗатратРегл КАК СчетЗатратРегл,
	|	ТаблицаНачисленияУдержания.ВидУдержанияПФ КАК ВидУдержанияПФ,
	|	ТаблицаНачисленияУдержания.СуммаПФ КАК СуммаПФ,
	|	ТаблицаНачисленияУдержания.ЗависитОтДней КАК ЗависитОтДней,
	|	ТаблицаНачисленияУдержания.ЗависитОтЧасов КАК ЗависитОтЧасов
	|ПОМЕСТИТЬ ТаблицаНачисленияУдержания
	|ИЗ
	|	&ТаблицаНачисленияУдержания КАК ТаблицаНачисленияУдержания
	|;
	...
	|	ПериодыНачислений.ДатаНачала КАК НачислениеДатаНачала,
	|	ПериодыНачислений.ДатаОкончания КАК НачислениеДатаОкончания,
	|	ВЫБОР
	...
	|				ТОГДА ПериодыНачислений.ДатаНачала
	|			ИНАЧЕ Табель.Период
	|		КОНЕЦ) КАК ДатаНачала,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ЕСТЬNULL(Табель.ВЦеломЗаПериод, ИСТИНА)
	|				ТОГДА ПериодыНачислений.ДатаОкончания
	|			ИНАЧЕ Табель.Период
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Табель КАК Табель
	|		ПО ПериодыНачислений.Сотрудник = Табель.Сотрудник
	|			И (Табель.Организация = &Организация)
	|			И (Табель.СтруктурнаяЕдиница = &СтруктурнаяЕдиница)
	|			И (ВЫБОР
	|				КОГДА &УчетПоПроектам
	|					ТОГДА ЕСТЬNULL(Табель.ВидВремени.РабочееВремя, ЛОЖЬ) = ИСТИНА
	|				ИНАЧЕ Табель.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыРабочегоВремени.Работа)
	|			КОНЕЦ)
	|			И (НЕ Табель.ВЦеломЗаПериод
	|					И ПериодыНачислений.ДатаНачала <= Табель.Период
	|					И ПериодыНачислений.ДатаОкончания >= Табель.Период
	|				ИЛИ Табель.ВЦеломЗаПериод
	|					И Табель.Период = НАЧАЛОПЕРИОДА(ПериодыНачислений.ДатаНачала, МЕСЯЦ))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
	|	КОНЕЦ,
	|	ПериодыНачислений.ДатаНачала,
	|	ПериодыНачислений.ДатаОкончания
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДокументНачисление.СчетЗатрат КАК СчетЗатрат,
	|	ДокументНачисление.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ДокументНачисление.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ДокументНачисление.РучнаяКорректировка КАК РучнаяКорректировка,
	|	ДокументНачисление.Показатель1 КАК Показатель1,
	|	ДокументНачисление.Представление1 КАК Представление1,
	|	ДокументНачисление.Значение1 КАК Значение1,
	|	ДокументНачисление.Показатель2 КАК Показатель2,
	|	ДокументНачисление.Представление2 КАК Представление2,
	|	ДокументНачисление.Значение2 КАК Значение2,
	|	ДокументНачисление.Показатель3 КАК Показатель3,
	|	ДокументНачисление.Представление3 КАК Представление3,
	|	ДокументНачисление.Значение3 КАК Значение3,
	|	ДокументНачисление.ТипСчета КАК ТипСчета,
	|	ДокументНачисление.СчетЗатратРегл КАК СчетЗатратРегл,
	|	ДокументНачисление.ВидУдержанияПФ КАК ВидУдержанияПФ,
	|	ДокументНачисление.СуммаПФ КАК СуммаПФ,
	|	ДокументНачисление.ЗависитОтДней КАК ЗависитОтДней,
	|	ДокументНачисление.ЗависитОтЧасов КАК ЗависитОтЧасов,
	...
	|ИЗ
	|	ТаблицаНачисленияУдержания КАК ДокументНачисление
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеТабеля КАК ДанныеТабеля
	|		ПО ДокументНачисление.Сотрудник = ДанныеТабеля.Сотрудник
	|			И ДокументНачисление.ДатаНачала = ДанныеТабеля.НачислениеДатаНачала
	|			И ДокументНачисление.ДатаОкончания = ДанныеТабеля.НачислениеДатаОкончания
	|ИТОГИ
	...
	СтарыйТекст =
			...
			|		ПО ДокументНачисление.Сотрудник = ДанныеТабеля.Сотрудник  
			| 
			//+++ Дмгг задвоение в Начислениях ONESOFT-1898
			|СГРУППИРОВАТЬ ПО
			|	ДокументНачисление.Значение3,
			|	ДокументНачисление.ТипСчета,
			|	ДокументНачисление.СчетЗатратРегл,
			|	ДокументНачисление.ВидУдержанияПФ,
			|	ДокументНачисление.СуммаПФ,
			|	ДокументНачисление.ЗависитОтДней,
			|	ДокументНачисление.ЗависитОтЧасов,
			|	ЕСТЬNULL(ДанныеТабеля.ВЦеломЗаПериод, ЛОЖЬ),
			|	ДокументНачисление.СчетЗатрат,
			|	ДокументНачисление.НаправлениеДеятельности,
			|	ДокументНачисление.ЗаказПокупателя,
			|	ДокументНачисление.РучнаяКорректировка,
			...
			|	ДокументНачисление.Показатель3,
			|	ДокументНачисление.Представление3,
			|	ЕСТЬNULL(ДанныеТабеля.ДатаНачала, ДокументНачисление.ДатаНачала),
			|	ДокументНачисление.Должность,
			|	ДокументНачисление.ВидНачисленияУдержания,
			|	ЕСТЬNULL(ДанныеТабеля.Проект, ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)),
			|	ДокументНачисление.ОтработаноДней,
			|	ДокументНачисление.Сотрудник,
			|	ДокументНачисление.ОтработаноЧасов,
			|	ДокументНачисление.Размер,
			|	ДокументНачисление.Сумма,
			|	ЕСТЬNULL(ДанныеТабеля.ОтработаноДней, 0),
			|	ЕСТЬNULL(ДанныеТабеля.ОтработаноЧасов, 0)  
			//--- Дмгг
			|ИТОГИ
			|	МАКСИМУМ(ОтработаноДней),
			|	МАКСИМУМ(ОтработаноЧасов),
...
Процедура ЗагрузкаИзФайла(Команда)
	...
	ОткрытьФорму("Документ.НачислениеЗарплаты.Форма.ФормаЗагрузкеИзТабличногоДокумента",ПараметрыЗаполнения , ЭтаФорма, , , ,Новый ОписаниеОповещения("ЗаполнениеЗарплатыЗавершение", ЭтотОбъект,Новый Структура("АдресЗарплатыВХранилище", АдресЗарплатыВХранилище)) , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПолучитьЗарплатыИзХранилища(АдресЗарплатыВХранилище)
	...
	Если  ТаблицаДляЗагрузки <> ""  Тогда 
		...
		Для Каждого Строка Из ТаблицаДляЗагрузки Цикл 
			...
			НовСтрокаНачУдер = Объект.НачисленияУдержания.Добавить();
			//НовСтрокаНалоги  = Объект.НалогиНаДоходы.Добавить();
			//НовСтрокаУдержания  = Объект.Удержания.Добавить();
			
			
			//Таблица  НачисленияУдержания
			ЗаполнитьЗначенияСвойств(НовСтрокаНачУдер, Строка);
			Если ЗначениеЗаполнено(Строка.ВидУдержанияПФ) Тогда
				СтавкаПФ = Строка.ВидУдержанияПФ.СтавкаПФ;
				НовСтрокаНачУдер.СуммаПФ = Окр(Строка.Сумма * СтавкаПФ/100, 2);
			Иначе
				СтавкаПФ = 0;
				НовСтрокаНачУдер.СуммаПФ = 0;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(НовСтрокаНачУдер.ВидНачисленияУдержания) Тогда
				НовСтрокаНачУдер.СчетЗатратРегл = НовСтрокаНачУдер.ВидНачисленияУдержания.СчетЗатратРегл;					
			КонецЕсли;
			
					
				
			//ТаблицаНалоги	
			Если ЗначениеЗаполнено(Строка.ВидНачисленияУдержания) Тогда 
				НовСтрокаНалоги  = Объект.НалогиНаДоходы.Добавить();
				НовСтрокаНалоги.Сотрудник = Строка.Сотрудник;
				НовСтрокаНалоги.ВидНачисленияУдержания = Строка.ВидНачисленияУдержанияНалоги;
				СтавкаНДФЛ	= НовСтрокаНалоги.ВидНачисленияУдержания.СтавкаНДФЛ;			
				НовСтрокаНалоги.Сумма = Окр((НовСтрокаНачУдер.Сумма - НовСтрокаНачУдер.СуммаПФ ) * СтавкаНДФЛ/100, 2);
			КонецЕсли;
			
			//ТаблицаУдержания
			Если  ЗначениеЗаполнено(Строка.ВидУдержания) Тогда 
				НовСтрокаУдержания  = Объект.Удержания.Добавить();
				НовСтрокаУдержания.Сотрудник = Строка.Сотрудник;
				НовСтрокаУдержания.ВидУдержания = Строка.ВидУдержания;
				НовСтрокаУдержания.ВидУдержанияНДФЛ = Строка.ВидУдержанияНДФЛ;
				НовСтрокаУдержания.ВидУдержанияПФ = Строка.ВидУдержанияПФТзУдержаний;
				НовСтрокаУдержания.Сумма = Строка.СуммаБезНалогов;
			КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьПрокет(Команда)
	
	оп = Новый ОписаниеОповещения("ЗаполнитьПроектЗавершение", ЭтотОбъект);
	ОткрытьФорму("Документ.НачислениеЗарплаты.Форма.ФормаВыбораПроекта",,,,,,оп,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте                    
Процедура ЗаполнитьПроектЗавершение(Результат, ДополнительныеПараметры) ЭКСПОРТ
	
	Если ТипЗнч(Результат) = Тип("СправочникСсылка.Проекты") Тогда
		Для Каждого Строк Из Объект.НачисленияУдержания Цикл 
			Строк.Проект = Результат;
		КонецЦикла;  
		Для Каждого Строк Из Объект.Удержания Цикл 
			Строк.Проект = Результат;
		КонецЦикла;
		Для Каждого Строк Из Объект.НалогиНаДоходы Цикл 
			Строк.Проект = Результат;
		КонецЦикла;
	КонецЕсли;
