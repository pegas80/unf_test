...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Для каждого СтрокаНачисленияУдержания Из ТаблицаНачисленияУдержания Цикл
		...
		Иначе
			...
			ТекстСообщения = НСтр("ka='კავშირის არასწორი პირობა ცხრილ ""დარიცხვები და დაკავებების"" სტრიქონ №%Номер%-ში.';ru='Не верное условие связи в строке №%Номер% табл. части ""Начислений и удержаний"".'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='ცხრილ ""თანამშრომლების"" სტრიქონ №%Номер% ბრძანების მოქმედების პერიოდი ეწინააღმდეგება საკადრო ბრძანებას ""%КадровыйПриказ%"".';ru='В строке №%Номер% табл. части ""Сотрудники"" период действия приказа противоречит кадровому приказу ""%КадровыйПриказ%"".'");
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='ცხრილ ""დარიცხვები და დაკავებების"" სტრიქონ №%Номер% ბრძანების მოქმედების პერიოდი ეწინააღმდეგება საკადრო ბრძანებას ""%КадровыйПриказ%"".';ru='В строке №%Номер% табл. части ""Начисления и удержания"" период действия приказа противоречит кадровому приказу ""%КадровыйПриказ%"".'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КадровыйПриказ%", ВыборкаИзРезультатаЗапроса.Регистратор);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='ცხრილ ""თანამშრომლების"" სტრიქონ №%Номер% თანამშრომელი განმეორებით არის მითითებული.';ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник указывается повторно.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
...
Процедура ВыполнитьКонтроль(ДополнительныеСвойства, Отказ)
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='ცხრილ ""თანამშრომლების"" სტრიქონ №%Номер% თანამშრომელი %Сотрудник% უკვე მუშაობს ქვედანაყოფ %СтруктурнаяЕдиница%.';ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник %Сотрудник% уже работает в подразделении %СтруктурнаяЕдиница%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ka='ცხრილ ""თანამშრომლების"" სტრიქონ №%Номер% თანამშრომელი %Сотрудник% უკვე მუშაობდა კომპანიაში. განმეორებით სამუშაოზე მისაღებად აუცილებელია ახალი თანამშრომლის შექმნა';ru='В строке №%Номер% табл. части ""Сотрудники"": сотрудник %Сотрудник% уже работал в компании. Для повторного приема на работу необходимо создать нового сотрудника.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сотрудник%", ВыборкаИзРезультатаЗапроса.Сотрудник);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
...
Процедура ВыполнитьКонтрольШтатногоРасписания(ДополнительныеСвойства, Отказ) 
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ПротиворечиеШР Тогда
			ТекстСообщения = НСтр("ka='ცხრილ ""თანამშრომლების"" სტრიქონი №%Номер% საშტატო განრიგში განაკვეთები გათვალისწინებული არ არის თანამშრომლის მისაღებად!';ru='Строка №%Номер% табл. части ""Сотрудники"": в штатном расписании не предусмотрены ставки для приема сотрудника!'");
