...
Функция УведомлениеОбОтгрузкеМаркированныхТоваровXML2_40(ДокументСсылка)
	...
	Если Шапка.ВидОборотаТовара <> Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС Тогда
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ka='Не заполнен номер документа-основания.';ru='Не заполнен номер документа-основания.';en='Not filled document number-base.'");
		КонецПопытки;
		
		Попытка
			УведомлениеОбОтгрузкеМаркированныхТоваров.move_doc_date = Шапка.ДатаДокументаОснования;
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ka='Не заполнена дата документа-основания.';ru='Не заполнена дата документа-основания.';en='Not filled date of the document-base.'");
		КонецПопытки;
	...
	Иначе
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='კონტრაგენტ %1-ისთვის რეგისტრაციის ქვეყანა არ არის შევსებული.';ru='Для контрагента %1 не заполнена страна регистрации.';en='For counterparty% 1 is not filled with the country of registration.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_name = Шапка.НаименованиеПолное;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='კონტრაგენტ %1-ისთვის სრული სახელწოდება არ არის შევსებული.';ru='Для контрагента %1 не заполнено полное наименование.';en='For counterparty% 1 is not filled with the full name.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_number = Шапка.РегистрационныйНомер;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='კონტრაგენტ %1-ისთვის სარეგისტრაციო ნომერი არ არის შევსებული.';ru='Для контрагента %1 не заполнен регистрационный номер.';en='For counterparty% 1 is not filled with the registration number.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_address = ЮридическийАдрес;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='კონტრაგენტ %1-ისთვის იურიდიული მისამართი არ არის შევსებული.';ru='Для контрагента %1 не заполнен юридический адрес.';en='For counterparty% 1 is not filled with legal address.'"), Шапка.Контрагент));
		КонецПопытки;
		...
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ka='დოკუმენტ %1-ში დოკუმენტის ნომერი არ არის შევსებული.';ru='В документе %1 не заполнен номер документа.';en='The document% 1 is not filled with the document number.'"), Шапка.Основание);
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.export_doc_date = Шапка.ДатаДокументаОснования;
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ka='დოკუმენტ %1-ში დოკუმენტის თარიღი არ არის შევსებული.';ru='В документе %1 не заполнена дата документа.';en='The document% 1 is not filled with the document date.'"), , Шапка.Основание);
		КонецПопытки;
...
Функция УведомлениеОбОтгрузкеМаркированныхТоваровXML2_41(ДокументСсылка)
			...
			Для Каждого СтрокаТЧ Из Товары Цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаТЧ.НомерКиЗ)
					...
					ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
						СообщениеXML,
						СтрШаблон(НСтр("ka='В строке %1 не указаны данные о КиЗ.';ru='В строке %1 не указаны данные о КиЗ.';en='Line% 1 does not specify the data on the Keys.'"),
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.НомерКиЗ) Тогда
					...
					Исключение
						ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
							СообщениеXML,
							СтрШаблон(НСтр("ka='სტრიქონ %1-ში მითითებულია სსნ ""%2""-ის არაკორექტული ნომერი.';ru='В строке %1 указан некорректный номер КиЗ ""%2"".';en=""Line% 1 contains invalid number Keyes '% 2'."""),
								СтрокаТЧ.НомерСтроки,
								СтрокаТЧ.НомерКиЗ));
					КонецПопытки;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.TID) Тогда
					...
					Исключение
						ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
							СообщениеXML,
							СтрШаблон(НСтр("ka='სტრიქონ %1-ში მითითებულია არაკორექტული TID ""%2"".';ru='В строке %1 указан некорректный TID ""%2"".';en='Line% 1 is incorrect Set the TID ""%2"".'"),
								СтрокаТЧ.НомерСтроки,
								СтрокаТЧ.TID));
					КонецПопытки;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.EPC) Тогда
					...
					Исключение
						ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
							СообщениеXML,
							СтрШаблон(НСтр("ka='სტრიქონ %1-ში მითითებულია არაკორექტული EPC ""%2"".';ru='В строке %1 указан некорректный EPC ""%2"".';en='Line% 1 is incorrect Set the EPC ""%2"".'"),
								СтрокаТЧ.НомерСтроки,
								СтрокаТЧ.EPC));
					КонецПопытки;
				КонецЕсли;
		...
		Иначе
			...
			Иначе
				...
				Если РеквизитыКонтрагента.ЭтоФизическоеЛицо Тогда
					...
					Исключение
						СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ka='У контрагента %1 не правильно заполнено ИНН.';ru='У контрагента %1 не правильно заполнено ИНН.';en='At 1% of the counterparty is not correctly filled inn.'"), Шапка.Контрагент);
					КонецПопытки;
				...
				Иначе
					...
					Попытка
						...
						КонецЕсли;
					Исключение
						СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ka='У контрагента %1 не правильно заполнено КПП.';ru='У контрагента %1 не правильно заполнено КПП.';en='At 1% of the counterparty is not properly filled PPC.'"), Шапка.Контрагент);
					КонецПопытки;
	...
	ИначеЕсли Шапка.ВидОборотаТовара = Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС Тогда
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Для контрагента %1 указана неверно или не заполнена страна регистрации.';ru='Для контрагента %1 указана неверно или не заполнена страна регистрации.';en='For counterparty% 1 is invalid or is not filled with the country of registration.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_name = РеквизитыКонтрагента.НаименованиеПолное;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Для контрагента %1 указан неверно или не заполнено полное наименование.';ru='Для контрагента %1 указан неверно или не заполнено полное наименование.';en='For counterparty% 1 incorrect or not filled full name.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_number = РеквизитыКонтрагента.РегистрационныйНомер;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Для контрагента %1 указан неверно или не заполнен регистрационный номер.';ru='Для контрагента %1 указан неверно или не заполнен регистрационный номер.';en='For counterparty% 1 is not valid or is not filled with the registration number.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_address = РеквизитыКонтрагента.ЮридическийАдрес;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ka='Для контрагента %1 указан неверно или не заполнен юридический адрес.';ru='Для контрагента %1 указан неверно или не заполнен юридический адрес.';en='For counterparty% 1 is not valid or is not filled with legal address.'"), Шапка.Контрагент));
		КонецПопытки;
