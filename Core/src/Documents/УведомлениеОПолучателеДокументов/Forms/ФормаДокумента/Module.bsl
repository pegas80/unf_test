...
Процедура ПослеОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИмяВидаОбъекта = НСтр("ka='уведомление';ru='уведомление';en='notification'");
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.УведомлениеОтправлено Тогда
		Форма.ЗаданиеЯвноРеквизитовПолучателяДокументовПредставление = ?(НЕ Объект.ЗаданиеЯвноРеквизитовПолучателяДокументов,
			НСтр("ka='На учетную запись отправителя уведомления';ru='На учетную запись отправителя уведомления';en=""Alert the sender's account"""),
			НСтр("ka='Задать вручную реквизиты получателя документов';ru='Задать вручную реквизиты получателя документов';en='Manually set details of the recipient of documents'"));
		Форма.ПолучательДокументовЯвляетсяФизическимЛицомПредставление = ?(Объект.ПолучательДокументовЯвляетсяФизическимЛицом,
			НСтр("ka='იურიდიული პირი';ru='Юридическое лицо';en='Entity'"),
			НСтр("ka='ფიზიკური პირი';ru='Физическое лицо';en='Individual'"));
	...
	Если НЕ Объект.ЗаданиеЯвноРеквизитовПолучателяДокументов Тогда
		...
		Если ЭтоЮрЛицо Тогда
			Если Объект.ДляВсехПодразделений Тогда
				ТекстСообщения =
					НСтр("ka='После отправки уведомления все входящие сообщения, в том числе требования, от налогового органа"
"будут приходить на учетную запись организации - отправителя уведомления.';ru='После отправки уведомления все входящие сообщения, в том числе требования, от налогового органа"
"будут приходить на учетную запись организации - отправителя уведомления.'");
			Иначе
				ТекстСообщения =
					НСтр("ka='После отправки уведомления все входящие сообщения, в том числе требования, по указанной регистрации"
"будут приходить на учетную запись организации - отправителя уведомления.';ru='После отправки уведомления все входящие сообщения, в том числе требования, по указанной регистрации"
"будут приходить на учетную запись организации - отправителя уведомления.'");
			КонецЕсли;
		Иначе
			ТекстСообщения =
				НСтр("ka='После отправки уведомления все входящие сообщения, в том числе требования, от налогового органа"
"будут приходить на учетную запись индивидуального предпринимателя - отправителя уведомления.';ru='После отправки уведомления все входящие сообщения, в том числе требования, от налогового органа"
"будут приходить на учетную запись индивидуального предпринимателя - отправителя уведомления.'");
	...
	Иначе
		Если Объект.ДляВсехПодразделений Тогда
			ТекстСообщения =
				НСтр("ka='После отправки уведомления все входящие сообщения, в том числе требования, от налогового органа"
"будут приходить на заданную учетную запись абонента.';ru='После отправки уведомления все входящие сообщения, в том числе требования, от налогового органа"
"будут приходить на заданную учетную запись абонента.'");
		Иначе
			ТекстСообщения =
				НСтр("ka='После отправки уведомления все входящие сообщения, в том числе требования, по указанной регистрации"
"будут приходить на заданную учетную запись абонента.';ru='После отправки уведомления все входящие сообщения, в том числе требования, по указанной регистрации"
"будут приходить на заданную учетную запись абонента.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ Объект.ЗаданиеЯвноРеквизитовПолучателяДокументов Тогда
		Если НЕ Форма.УведомлениеОтправлено Тогда
			ДолжностноеЛицоПредставление = ?(ЗначениеЗаполнено(Форма.ФамилияИмяОтчествоДолжностногоЛицаПредставление),
				СокрЛП(Форма.ФамилияИмяОтчествоДолжностногоЛицаПредставление), НСтр("ka='(არ არის განსაზღვრული)';ru='(не определено)';en='(undefined)'"));
			ТекстДолжностноеЛицо = НСтр("ru='Должностное лицо получателя: %1."
"';ka='Должностное лицо получателя: %1."
"'");
		...
		ТекстСообщения = ТекстДолжностноеЛицо
			+ НСтр("ka='Обратите внимание, при смене должностного лица необходимо отправить уведомление заново.';ru='Обратите внимание, при смене должностного лица необходимо отправить уведомление заново.';en='Please note that if you change officer must send a notice again.'");
