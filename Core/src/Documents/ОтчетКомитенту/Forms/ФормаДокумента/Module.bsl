...
Процедура ЗаполнитьПоКонтрагенту(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоКонтрагентуЗавершение", ЭтотОбъект), НСтр("ka='დოკუმენტი სრულიად გადაივსება! გავაგრძელოთ ოპერაციის შესრულება?';ru='Документ будет полностью перезаполнен! Продолжить выполнение операции?'"), РежимДиалогаВопрос.ДаНет, 0);
КонецПроцедуры
...
Процедура ПодборПоПродажам(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ka='ველი ""ორგანიზაცია"" შეუვსებელია';ru='Поле ""Организация"" не заполнено'");
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения,,, "Организация", Отказ);
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ТекстСообщения = НСтр("ka='ველი ""კონტრაგენტი"" შეუვსებელია';ru='Поле ""Контрагент"" не заполнено'");
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения,,, "Контрагент", Отказ);
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
		ТекстСообщения = НСтр("ka='ველი ""ხელშეკრულება"" არ არის შევსებული';ru='Поле ""Договор"" не заполнено'");
