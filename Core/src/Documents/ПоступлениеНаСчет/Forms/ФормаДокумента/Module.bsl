...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам") Тогда
		...
		Если НЕ СуммаПлатежа = Объект.СуммаДокумента И НЕ СуммаКомиссии = Объект.СуммаКомиссииДокумента Тогда
			Если Вопрос(НСтр("ka='არ ემთხვევა დოკუმენტის თანხა და საკომისიოს თანხა მისი გაშიფვრებთან. გადაითვალოს თანხები?';ru='Не совпадают сумма документа и сумма коммисионных с ее рассшифровками. Пересчитать суммы?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		ИначеЕсли НЕ СуммаПлатежа = Объект.СуммаДокумента Тогда
			Если Вопрос(НСтр("ka='არ ემთხვევა დოკუმენტის თანხა და მისი გაშიფვრა. გადაითვალოს დოკუმენტის თანხა?';ru='Не совпадают сумма документа и ее расшифровка. Пересчитать сумму документа?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
				Объект.СуммаДокумента 			= СуммаПлатежа;
			КонецЕсли;
		ИначеЕсли НЕ СуммаКомиссии = Объект.СуммаКомиссииДокумента Тогда
			Если Вопрос(НСтр("ka='არ ემთხვევა დოკუმენტის საკომისიოს თანხა და მისი გაშიფვრა. გადაითვალოს დოკუმენტის საკომისიოს თანხა?';ru='Не совпадают сумма коммисионных документа и ее расшифровка. Пересчитать сумму коммисионных документа?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
				Объект.СуммаКомиссииДокумента 	= СуммаКомиссии;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПометитьНаУдалениеИнтерактивно(Команда)
	...
	Если Модифицированность Тогда
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ka='Для снятия пометки удаления необходимо записать внесенные вами изменения. Записать данные?';ru='Для снятия пометки удаления необходимо записать внесенные вами изменения. Записать данные?'");
		Иначе
			ТекстВопроса = НСтр("ka='Для установки отметки удаления необходимо записать внесенные вами изменения. Записать данные?';ru='Для установки отметки удаления необходимо записать внесенные вами изменения. Записать данные?'");
		КонецЕсли;
	Иначе
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ka='Снять с ""';ru='Снять с ""'")+Объект.Ссылка + НСтр("ka='"" пометку на удаление?';ru='"" пометку на удаление?'");
		Иначе
			ТекстВопроса = НСтр("ka='მონიშვნა ""';ru='Пометить ""'")+Объект.Ссылка + НСтр("ka='"" на удаление?';ru='"" на удаление?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СуммаДокументаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ВидОперации = ВидОперацииОтКурьерскойКомпанииПочты Тогда
		...
		Если СуммаИтог <> 0 И СуммаИтог <> Объект.СуммаДокумента
			И Объект.ВариантЗаполненияРасшифровки <> ВариантЗаполненияРасшифровкиАвтоматически
			Тогда
			НадписьВСкобках = НСтр("ka='გავრცელებულია მექანიკურად';ru='разнесено вручную'");
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	Элементы.РасчетыПоКредитам.Заголовок = НСТР("ka='ანგარიშსწორება კრედიტების მიხედვით';ru='Расчеты по кредитам'");
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтПокупателя") Тогда
		...
		Элементы.СуммаРазнесеноВручную.Заголовок = НСТР("ka='სულ გადანაწილებულია ხელით';ru='Итого разнесено вручную'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтПоставщика") Тогда
		...
		Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = Элементы.СтраницаРучноеЗаполнение;
		Элементы.РасчетыСКонтрагентом.Заголовок = НСТР("ka='დავალიანებების ჩათვლა';ru='Зачет долгов'");
		
		// Подвал
		Элементы.Подвал.Видимость = Ложь;
		Элементы.СтраницыРасчетовСКонтрагентомПодвал.Видимость = Истина;
		Элементы.СуммаРазнесеноВручную.Заголовок = НСТР("ka='სულ გადანაწილებულია';ru='Итого разнесено'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам") Тогда
		...
		Элементы.ГруппаКонтрагент.Видимость = Истина;
		Элементы.Контрагент.Заголовок = НСТР("ka='ბანკი-ეკვაირერი';ru='Банк-эквайрер'");
		Элементы.СчетКонтрагента.Видимость = Истина;
		...
		Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") Тогда
			...
			Если НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
				Элементы.СуммаПлатежаЭквайринг.Заголовок = НСТР("ka='ანგარიშსწორების თანხა';ru='Сумма расчетов'");
			Иначе
				Элементы.СуммаПлатежаЭквайринг.Заголовок = НСТР("ka='გადახდის თანხა';ru='Сумма платежа'");
			КонецЕсли;
			...
			Элементы.СуммаДокумента.Заголовок = НСТР("ka='თანხა/საკომისიო';ru='Сумма/комиссия'");
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником") Тогда
		...
		Элементы.РасчетыПоКредитам.Заголовок = НСтр("ka='ანგარიშსწორება სესხის მიხედვით';ru='Расчеты по займам'");
		Элементы.РасчетыПоКредитам.Видимость = Истина;
		Элементы.НалогообложениеНДС.Видимость = Истина;
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты") Тогда
		...
		Элементы.ДоговорСАгентом.Заголовок = ?(ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"), НСтр("ka='Договор (руб.)';ru='Договор (руб.)'"), НСтр("ka='ხელშეკრულება';ru='Договор'"));
...
Процедура УстановитьЗагруженИзИнтернетБанка()
	...
	Если РезультатЗапроса.Количество() > 0 Тогда 
		...
		Элементы.ДекорацияЗагруженИзБанка.Заголовок = НСтр("ka='ჩატვირთული ონლაინ საბანკოდან';ru='Загружен из интернет банка'");
...
Функция ПолучитьПараметрыЭквайринговогоТерминалаПриИзменении(ЭквайринговыйТерминал, ИспользоватьТолькоРегламентированногоПланаСчетов, Дата)
	...
	Иначе
		...
		Иначе
			...
			ПараметрыЭквайринговогоТерминала.Вставить("ТипСчетаЗатрат", ПараметрыЭквайринговогоТерминала.СчетЗатратРегл.ТипСчета);
...
Процедура ДоговорСАгентомНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ВидОперации) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Сначала выберите операцию.';ru='Сначала выберите операцию.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Сначала выберите контрагента.';ru='Сначала выберите контрагента.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.РасшифровкаПлатежа.Количество() = 0) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ka='Ошибка заполнения документа. Проверьте заполнение организации, операции, контрагента.';ru='Ошибка заполнения документа. Проверьте заполнение организации, операции, контрагента.'"));
		Возврат;
	КонецЕсли;
...
Процедура РасшифровкаПлатежаОтАгентаЗаказПриИзмененииНаСервере(ТекущиеДанныеИдентификатор)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Заказ) И ТекущиеДанные.Заказ.Договор.ВалютаРасчетов <> Константы.НациональнаяВалюта.Получить() Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Не поддерживается выбор заказов, которые оформлены по договорам в валюте или у.е.';ru='Не поддерживается выбор заказов, которые оформлены по договорам в валюте или у.е.'");
		...
		ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.Заказ)
			...
			Если НЕ НашлиЗаказ Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ka='В расходной накладной не выбран заказ, который выбран в текущей строке. Выберите другой документ или другой заказ.';ru='В расходной накладной не выбран заказ, который выбран в текущей строке. Выберите другой документ или другой заказ.'");
				Сообщение.Поле = "РасшифровкаПлатежаОтАгента["+Объект.РасшифровкаПлатежаОтАгента.Индекс(ТекущиеДанные)+"].Документ";
				Сообщение.Сообщить();
...
Процедура РасшифровкаПлатежаОтАгентаДокументПриИзмененииНаСервере(ТекущиеДанныеИдентификатор)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Документ) И ТекущиеДанные.Документ.Договор.ВалютаРасчетов <> Константы.НациональнаяВалюта.Получить() Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ka='Не поддерживается выбор документов, которые оформлены по договорам в валюте или у.е.';ru='Не поддерживается выбор документов, которые оформлены по договорам в валюте или у.е.'");
			Сообщение.Поле = "РасшифровкаПлатежаОтАгента["+Объект.РасшифровкаПлатежаОтАгента.Индекс(ТекущиеДанные)+"].Документ";
			Сообщение.Сообщить();
