...
Процедура ЗаполнитьБанковскийСчетКонтрагента()
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если УправлениеНебольшойФирмойСервер.ЕстьРеквизитДокумента("СчетКонтрагента", ДокументОснование.Метаданные()) Тогда
			...
			//БанковскийСчет = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "СчетКонтрагента"); //+++ Дмгг
			СчетКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "СчетКонтрагента"); //+++ Дмгг
		...
		ИначеЕсли УправлениеНебольшойФирмойСервер.ЕстьРеквизитДокумента("БанковскийСчетКонтрагента", ДокументОснование.Метаданные()) Тогда
			...
			//БанковскийСчет = ДокументОснование.БанковскийСчетКонтрагента; //+++ Дмгг
			СчетКонтрагента = ДокументОснование.БанковскийСчетКонтрагента; //+++ Дмгг
...
Процедура ЗаполнитьПоНачислениюНалогов(ДокументОснование) Экспорт
	...
	Если ДокументОснование.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Возмещение Тогда
		ВызватьИсключение НСтр("ka='ანგარიშზე შემოსვლის შეყვანა შეიძლება მხოლოდ გადასახადების ანაზღაურების, და არა დარიცხვის საფუძველზე.';ru='Поступление на счет можно ввести только на основании возмещения налогов, а не начисления.';en='Admission to the account, you can enter only on the basis of tax refunds, rather than charging.'");
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходнойНакладной(ДокументОснование) Экспорт
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Документ,
	|	СУММА(ЕСТЬNULL(ВЫРАЗИТЬ(ТаблицаДокумента.Всего * 
	|				ВЫБОР
	...
	|	КОНЕЦ КАК Кратность,
	|	СУММА(ЕСТЬNULL(ТаблицаДокумента.Всего, 0)
	|) КАК СуммаПлатежа,
	|	ТаблицаДокумента.СтавкаНДС КАК СтавкаНДС,
...
Процедура ЗаполнитьПоПриемуВРемонт(ДокументСсылкаПриемИПередачаВРемонт) Экспорт
	...
	Если ДокументСсылкаПриемИПередачаВРемонт.ВариантРемонта = Перечисления.ВариантыРемонта.НашаМастерскаяМногоэтапныйРемонт Тогда
		ВызватьИсключение НСтр("ka='Для выбранного варианта ремонта вводите оплаты на основании подчиненных документов - Заказ-наряд, Расходная, Акт';ru='Для выбранного варианта ремонта вводите оплаты на основании подчиненных документов - Заказ-наряд, Расходная, Акт';en='To repair the selected option type of payment on the basis of the documents subordinate - Work Order, consumables, Act'");
	КонецЕсли;
...
Процедура ЗаполнитьПоКорректировкеПоступления(ДанныеЗаполнения, НомерСтроки = Неопределено, Сумма = Неопределено) Экспорт
	...
	Запрос.Текст =                                                                                                            
		...
		|	СУММА(КорректировкаПоступленияРасходы.СуммаДоИзменения - КорректировкаПоступленияРасходы.Сумма) КАК Сумма,
		...
		|	КорректировкаПоступленияРасходы.Ссылка.ДокументОснование КАК ДокументОснование,
		|	КорректировкаПоступленияРасходы.Ссылка КАК Документ,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(КорректировкаПоступленияРасходы.Ссылка.Заказ) = ТИП(Документ.ЗаказПокупателя) 
		...
		|	СУММА(КорректировкаПоступленияРасходы.СуммаДоИзменения - КорректировкаПоступленияРасходы.Сумма) КАК СуммаПлатежа,
		|	СУММА(КорректировкаПоступленияРасходы.СуммаДоИзменения - КорректировкаПоступленияРасходы.Сумма) КАК СуммаРасчетов,
		|	СУММА(КорректировкаПоступленияРасходы.СуммаНДСДоИзменения - КорректировкаПоступленияРасходы.СуммаНДС) КАК СуммаНДС
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
		ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика;
		Если НЕ ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ВалютаДенежныхСредств = Выборка.ВалютаДокумента;
		КонецЕсли;
		НоваяСтрока = РасшифровкаПлатежа.Добавить();
...
Процедура ЗаполнитьПоПродажаЛицензии(ДокументОснование) Экспорт
	...
	|	&Ссылка КАК ДокументОснование,
	|	ВЫБОР
	|		КОГДА ПродажаЛицензииЗапасы.Ссылка.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств = ПродажаЛицензииЗапасы.Ссылка.Договор.ВалютаРасчетов
	|			ТОГДА ПродажаЛицензииЗапасы.Ссылка.Организация.БанковскийСчетПоУмолчанию
	|		ИНАЧЕ ВложенныйЗапрос.БанковскийСчет
	|	КОНЕЦ КАК БанковскийСчет,
	|	ПродажаЛицензииЗапасы.Ссылка.Организация КАК Организация,
	|	ПродажаЛицензииЗапасы.Ссылка.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ПродажаЛицензииЗапасы.Ссылка.Контрагент КАК Контрагент,
	|	ПродажаЛицензииЗапасы.Ссылка.ВалютаДокумента КАК ВалютаДенежныхСредств,
	|	ПродажаЛицензииЗапасы.Ссылка.Договор КАК Договор,
	|	ЛОЖЬ КАК ПризнакАванса,
	|	ВЫБОР
	|		КОГДА ПродажаЛицензииЗапасы.Ссылка.Контрагент.ВестиРасчетыПоДокументам
	|			ТОГДА &Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Документ,
	|	СУММА(ЕСТЬNULL(ВЫРАЗИТЬ(ПродажаЛицензииЗапасы.Всего * ВЫБОР
	|					КОГДА ПродажаЛицензииЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|						ТОГДА КурсыРегВалют.Курс * ПродажаЛицензииЗапасы.Ссылка.Кратность / (ПродажаЛицензииЗапасы.Ссылка.Курс * ЕСТЬNULL(КурсыРегВалют.Кратность, 1))
	|					ИНАЧЕ 1
	|				КОНЕЦ КАК ЧИСЛО(15, 2)), 0)) КАК СуммаРасчетов,
	|	ВЫБОР
	|		КОГДА ПродажаЛицензииЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|			ТОГДА ПродажаЛицензииЗапасы.Ссылка.Курс
	|		ИНАЧЕ КурсыВалютыРасчетов.Курс
	|	КОНЕЦ КАК Курс,
	|	ВЫБОР
	|		КОГДА ПродажаЛицензииЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|			ТОГДА ПродажаЛицензииЗапасы.Ссылка.Кратность
	|		ИНАЧЕ КурсыВалютыРасчетов.Кратность
	|	КОНЕЦ КАК Кратность,
	|	СУММА(ЕСТЬNULL(ПродажаЛицензииЗапасы.Всего, 0)) КАК СуммаПлатежа,
	|	ПродажаЛицензииЗапасы.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ЕСТЬNULL(ПродажаЛицензииЗапасы.СуммаНДС, 0)) КАК СуммаНДС
	|ИЗ
	|	Константа.НациональнаяВалюта КАК КонстантаНациональнаяВалюта,
	|	Документ.ПродажаЛицензии.Запасы КАК ПродажаЛицензииЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|				&Дата,
	|				Валюта В
	|					(ВЫБРАТЬ
	...
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютыРасчетов
	|		ПО ПродажаЛицензииЗапасы.Ссылка.Договор.ВалютаРасчетов = КурсыВалютыРасчетов.Валюта
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 1
	|			БанковскиеСчета.Ссылка КАК БанковскийСчет,
	|			БанковскиеСчета.Владелец КАК Владелец,
	|			БанковскиеСчета.ВалютаДенежныхСредств КАК ВалютаДенежныхСредств
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.БанковскиеСчета КАК БанковскиеСчета
	|				ПО ПродажаЛицензии.Организация = БанковскиеСчета.Владелец
	|					И ПродажаЛицензии.ВалютаДокумента = БанковскиеСчета.ВалютаДенежныхСредств
	|		ГДЕ
	|			ПродажаЛицензии.Ссылка = &Ссылка
	|			И БанковскиеСчета.ПометкаУдаления = ЛОЖЬ) КАК ВложенныйЗапрос
	|		ПО ПродажаЛицензииЗапасы.Ссылка.ВалютаДокумента = ВложенныйЗапрос.ВалютаДенежныхСредств
	|			И ПродажаЛицензииЗапасы.Ссылка.Организация = ВложенныйЗапрос.Владелец
	|ГДЕ
	|	ПродажаЛицензииЗапасы.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА ПродажаЛицензииЗапасы.Ссылка.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств = ПродажаЛицензииЗапасы.Ссылка.Договор.ВалютаРасчетов
	...
	|		ИНАЧЕ ВложенныйЗапрос.БанковскийСчет
	|	КОНЕЦ,
	|	ПродажаЛицензииЗапасы.Ссылка.Организация,
	|	ПродажаЛицензииЗапасы.Ссылка.НалогообложениеНДС,
	|	ПродажаЛицензииЗапасы.Ссылка.Контрагент,
	|	ПродажаЛицензииЗапасы.Ссылка.ВалютаДокумента,
	|	ПродажаЛицензииЗапасы.Ссылка.Договор,
	|	ВЫБОР
	|		КОГДА ПродажаЛицензииЗапасы.Ссылка.Контрагент.ВестиРасчетыПоДокументам
	|			ТОГДА &Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|		ИНАЧЕ КурсыВалютыРасчетов.Кратность
	|	КОНЕЦ,
	|	ПродажаЛицензииЗапасы.СтавкаНДС";
...
Процедура ЗаполнитьРасшифровкуПоОперациямЭквайрингаНаСервере() Экспорт
	...
	Для Каждого ТекущаяСтрока Из ТаблицаЭквайринговыхОпераций Цикл
		...
		НоваяСтрока = РасшифровкаПлатежа.Добавить();
		ТекущаяСтрока.СуммаРасчетовКомиссии = СуммаКомиссииДокумента;
	...
	КонецЦикла;
	    СуммаДокумента = ТекущаяСуммаРасчетов;
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИспользованиеТолькоРегламентированногоПланаСчетов = ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ИспользованиеТолькоРегламентированногоПланаСчетов(?(ЗначениеЗаполнено(Дата),Дата,ТекущаяДата()));
	...
	Для каждого СтрокаТЧ Из РасшифровкаПлатежа Цикл
		...
		Если (ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты
			...
			Или ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
		...
		Если НЕ ЗначениеЗаполнено(СуммаКомиссииДокумента) Тогда
			СуммаКомиссииДокумента = РасшифровкаПлатежа.Итог("СуммаРасчетовКомиссии");
		КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВзносНаличными 
		...
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВзносНаличными Тогда 
		...
		Если Не ИспользованиеТолькоРегламентированногоПланаСчетов Тогда
			
			Если РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(Дата,УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(Организация)) И Не ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.ИспользованиеТолькоРегламентированногоПланаСчетов(?(ЗначениеЗаполнено(Дата),Дата,ТекущаяДата())) Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	СтратегияЗаполнения[Тип("ДокументСсылка.ПродажаЛицензии")] = "ЗаполнитьПоПродажаЛицензии";//++Lazare 17.10.2022
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПодотчетника Тогда
		...
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ДоговорДляЗачетаАвансаСПомощником");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ДоговорыАвтоЗачетаДолгов");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "НалогообложениеНДС"); //+++ Истомин
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "СтруктурнаяЕдиница");
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
		...
		Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") Тогда
			...
			//СуммаКомиссии = СуммаКомиссииДокумента;
			//Если СуммаКомиссии <> СуммаКомиссииДокумента Тогда
			//	ТекстСообщения = НСтр("ka='კომისიის თანხა: %СуммаКомиссииДокумента% %ВалютаДенежныхСредств%, ცხრილში გატარებული გადახდების ჯამს არ შეესაბამება: %СуммаКомиссии% %ВалютаДенежныхСредств%!';ru='Сумма комиссии: %СуммаКомиссииДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаКомиссии% %ВалютаДенежныхСредств%!';en='The amount of commission: SummaKomissiiDokumenta%%%% ValyutaDenezhnyhSredstv does not correspond to the amount of separated charges in the tabular section: SummaKomissii%%%% ValyutaDenezhnyhSredstv!'");
	...
	//+++ Дмгг
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратДенегОтСотрудника Тогда
		//УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подотчетник");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Контрагент");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Документ");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "СуммаУчета");
	...
	Если ПроверитьДокументыПланирования Тогда
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			...
			Если ЕстьОшибка Тогда
				ТекстСообщения = НСтр("ru = 'Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств% меньше суммы разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!'; ka = 'დოკუმენტის თანხა: %СуммаДокумента% %ВалютаДенежныхСредств% გადახდების ცრილში არჩეულ თანხების ჯამზე ნაკლებია: %СуммаПлатежа% %ВалютаДенежныхСредств%!'; en = 'The amount of the document:%%% SummaDokumenta ValyutaDenezhnyhSredstv% less than the separated charges in the dimension tables: SummaPlatezha%%%% ValyutaDenezhnyhSredstv!'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаДокумента%", Строка(СуммаДокумента));
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//РегламентированныйУчетСервер.ОтразитьДвиженияДокументаПоступлениеНаСчет(ДополнительныеСвойства, Движения, Отказ);
	...
	УправлениеНебольшойФирмойСервер.ОтразитьОтчетКурсовыхРазниц(ДополнительныеСвойства, Движения, Отказ);
	//--- Дмгг
...
Процедура ОтразитьОтчетКурсовыхРазниц(ДополнительныеСвойства, Движения, Отказ)
	...
	//ТаблицаОтчетКурсовыхРазниц = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаОтчетПоКурсовымРазницам;
	//
	//Если Отказ
...
Процедура ЗаполнитьПоДоговоруКредитаИЗайма(ДокументСсылкаДоговорКредитаИЗайма, Сумма = Неопределено) Экспорт
	...
	Если Сумма <> Неопределено Тогда
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом)
...
Процедура ЗаполнитьПоНачисленияПоКредитамИЗаймам(ДанныеЗаполнения) Экспорт
	...
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ДоговорКредитаЗайма.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКредитаИЗайма.КредитВыданный)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником)
	|	КОНЕЦ КАК ВидОперации,
	|	&Ссылка КАК ДокументОснование,
