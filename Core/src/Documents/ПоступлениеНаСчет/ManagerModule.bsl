...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	|			ИЛИ ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом)
	...
	|	И &ОстатокСуммы > 0
	// Олег Литвиненко 22.09.22 --->
	|	И НЕ (ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты) 
	|		И ТаблицаДокумента.РасчетСКурьером)
	// Олег Литвиненко <---
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаДокумента.АналитикаДДС КАК Аналитика,
	|	ТаблицаДокумента.ВалютаДенежныхСредств КАК Валюта,
	|	СУММА(ВЫРАЗИТЬ(РасшифровкаПлатежаОтАгента.СуммаПлатежа * КурсыВалютКассы.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))) КАК Сумма,
	|	СУММА(РасшифровкаПлатежаОтАгента.СуммаПлатежа) КАК СуммаВал,
	|	СУММА(ВЫРАЗИТЬ(РасшифровкаПлатежаОтАгента.СуммаПлатежа * КурсыВалютКассы.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))) КАК СуммаДляОстатка,
	|	СУММА(РасшифровкаПлатежаОтАгента.СуммаПлатежа) КАК СуммаВалДляОстатка,
	//+++ Дмгг
	|	ТаблицаДокумента.Проект КАК Проект
	//--- Дмгг
	|ИЗ
	|	ВременнаяТаблицаШапка КАК ТаблицаДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеНаСчет.РасшифровкаПлатежаОтАгента КАК РасшифровкаПлатежаОтАгента
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютУчетаСрезПоследних
	|			ПО (КурсыВалютУчетаСрезПоследних.Валюта = &ВалютаУчета)
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютКассы
	|			ПО (КурсыВалютКассы.Валюта = &ВалютаДС)
	|		ПО ТаблицаДокумента.Ссылка = РасшифровкаПлатежаОтАгента.Ссылка
	|ГДЕ
	|	РасшифровкаПлатежаОтАгента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты) 
	|		И РасшифровкаПлатежаОтАгента.Ссылка.РасчетСКурьером
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Дата,
	|	ТаблицаДокумента.Статья,
	...
	|	СУММА(ВременнаяТаблицаРасшифровкаПлатежа.СуммаПлатежа * КурсыВалютКассы.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютКассы.Кратность)),
	|	СУММА(ВременнаяТаблицаРасшифровкаПлатежа.СуммаПлатежа),
	//+++ Дмгг, 09.10.2025, ONESOFT-4418
	//|	ВременнаяТаблицаРасшифровкаПлатежа.Проект
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.РасчетыПоКредитам)
	|			ТОГДА ВЫБОР
	|					КОГДА ВременнаяТаблицаРасшифровкаПлатежа.Ссылка.ПроектДляЗачетаАвансаСПомощником = Значение(Справочник.Проекты.ПустаяСсылка)
	|						ТОГДА Значение(Справочник.Проекты.БезПроекта)
	|					ИНАЧЕ ВременнаяТаблицаРасшифровкаПлатежа.Ссылка.ПроектДляЗачетаАвансаСПомощником
	|				КОНЕЦ
	...
	|			ИЛИ ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыДенежныхСредств.БезНаличные),
	|	ВременнаяТаблицаРасшифровкаПлатежа.Статья,
	|	Значение(Перечисление.ВременноеХранилищеДенежныхСредств.ДеньгиВПути), //ВременнаяТаблицаРасшифровкаПлатежа.БанковскийСчет,
	|	ВременнаяТаблицаРасшифровкаПлатежа.БанковскийСчетКассаСчетУчета,
	|	ВременнаяТаблицаРасшифровкаПлатежа.БанковскийСчетКассаСчетУчетаРегл,
	|	ВременнаяТаблицаРасшифровкаПлатежа.АналитикаДДС,
	|	ВременнаяТаблицаРасшифровкаПлатежа.ВалютаДенежныхСредств,
	...
	|		ПО (КурсыВалютКассы.Валюта = &ВалютаДС)
	|ГДЕ
	|	&ИспользуютсяПереводыВПути
	|	И (ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета)
	|		ИЛИ ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВзносНаличными))
	|	//И &ПлатежныйКалендарь
	|	И ВременнаяТаблицаРасшифровкаПлатежа.СуммаПлатежа > 0
	|
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	|   ВременнаяТаблицаРасшифровкаПлатежа.РасчетСКурьером,
	|   ВременнаяТаблицаРасшифровкаПлатежа.ПриходнаяНакладная,
	// Олег Литвиненко <---
	...
	|
	|ВЫБРАТЬ
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.НомерСтроки,
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Документ,
	|	NULL,
	|	&Организация,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.БанковскийСчет,
	// Олег Литвиненко 22.09.22 --->
	|   ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ВидОперации,
	|   ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.РасчетСКурьером,
	|   ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ПриходнаяНакладная,
	//|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Контрагент,
	|	ВЫБОР
	|   	КОГДА ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты) 
	|   		И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.РасчетСКурьером
	|           ТОГДА ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Документ.Контрагент
	|		ИНАЧЕ ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Контрагент
	|	КОНЕЦ,
	// Олег Литвиненко <---
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Статья,
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателем,
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателемРегл,
	//|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ДоговорДляЗачетаАвансаСПомощником,
	// Олег Литвиненко 26.09.22 --->
	|	ВЫБОР
	|   	КОГДА ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты) 
	|   		И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.РасчетСКурьером
	|           ТОГДА ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Документ.Договор
	|		ИНАЧЕ ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ДоговорДляЗачетаАвансаСПомощником
	|	КОНЕЦ,
	// Олег Литвиненко <---
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ДоговорДляЗачетаАвансаСПомощником.ВалютаРасчетов,
	...
	|	СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаПлатежа + ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаПлатежа + ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаПлатежа + ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	СУММА(-(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаПлатежа + ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета)),
	|	СУММА(-(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаПлатежа + ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета)),
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница';en='Exchange difference'")); 
	Запрос.УстановитьПараметр("ПереоценкаВозвратаАванса", Константы.ПереоценкаВозвратаАванса.Получить());
	...
	|						ТОГДА ВременнаяТаблицаРасшифровкаПлатежа.Документ.ДокументОснование
	|					ИНАЧЕ ВременнаяТаблицаРасшифровкаПлатежа.Документ
	|				КОНЕЦ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Документ,
	...
	|	,ВременнаяТаблицаРасшифровкаПлатежа.Ссылка.ВидУдержанияНДФЛ КАК ВидУдержанияНДФЛ
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	...
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	|	,ВременнаяТаблицаРасшифровкаПлатежа.Ссылка.ВидУдержанияНДФЛ
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	|
	// Олег Литвиненко 22.09.22 --->
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Дата,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг),
	|	СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	-СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	-СУММА(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.СуммаРасчета),
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ДоговорДляЗачетаАвансаСПомощником.ВалютаРасчетов,
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ВалютаДенежныхСредств,
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.Контрагент.ВестиРасчетыПоДокументам,
	|	NULL
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	|	,NULL
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Если ДокументСсылкаПоступлениеНаСчет.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратДенегОтСотрудника Тогда 
		...
		|	ТаблицаДокумента.Проект, //Малышев 2025-10-23 ONESOFT-4418
...
Процедура СформироватьТаблицаОплатаСчетовИЗаказов(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	// Олег Литвиненко 22.09.22 --->
	|			И НЕ (ТаблицаДокумента.РасчетСКурьером 
	|			И ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты))
	// Олег Литвиненко <---
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ,
	|	0,
	|	СУММА(СуммаПлатежа+СуммаРасчета)
	|ИЗ
	|	Документ.ПоступлениеНаСчет.РасшифровкаПлатежаОтАгента КАК ПоступлениеНаСчетРасшифровкаПлатежаОтАгента
	|ГДЕ
	|	ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка = &Ссылка
	|	И (ТИПЗНАЧЕНИЯ(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ) = ТИП(Документ.ЗаказПокупателя)
	|				И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ) = ТИП(Документ.ЗаказПоставщику)
	|				И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ <> ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ) = ТИП(Документ.ПриемИПередачаВРемонт)
	|				И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Заказ <> ЗНАЧЕНИЕ(Документ.ПриемИПередачаВРемонт.ПустаяСсылка))
	|			И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.РасчетСКурьером 
	|			И ПоступлениеНаСчетРасшифровкаПлатежаОтАгента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты)
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаРасчетыСПерсоналом(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)//+++ Дмгг
	...
	|	ВременнаяТаблицаШапка.Проект,  //Малышев 2025-10-23 ONESOFT-4418
	...
	|	ВременнаяТаблицаРасчетыСПерсоналом.Проект,  //Малышев 2025-10-23 ONESOFT-4418
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("УчетПоПроектам", СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам); //+++ Дмгг, 09.10.2025, ONESOFT-4418
	
	// Олег Литвиненко 16.08.23 --->
	// ONESOFT-2425	
	...
	Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") Тогда
		...
		// Олег Литвиненко 16.08.23 --->
		// ONESOFT-2425
		|	ПараметрыУчетаЭквайринговыхОпераций.СуммаДокумента КАК СуммаДокумента,
		|	ПараметрыУчетаЭквайринговыхОпераций.СуммаКомиссииДокумента КАК СуммаКомиссииДокумента,
		// Олег Литвиненко <---
		|	ПараметрыУчетаЭквайринговыхОпераций.ПодразделениеЗатраты КАК ПодразделениеЗатраты   
		//+++ Дмгг, 15.10.2025, ONESOFT-4418
		//|,	ВЫБОР
		//|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА Значение(Справочник.Проекты.ПустаяСсылка) 
		//|		ИНАЧЕ ВЫБОР
		//|				КОГДА ПараметрыУчетаЭквайринговыхОпераций.ПроектДляЗачетаАвансаСПомощником = Значение(Справочник.Проекты.ПустаяСсылка)
		//|					ТОГДА Значение(Справочник.Проекты.БезПроекта)
		//|				ИНАЧЕ ПараметрыУчетаЭквайринговыхОпераций.ПроектДляЗачетаАвансаСПомощником 
		//|			КОНЕЦ
		//|	КОНЕЦ КАК Проект
		//--- Дмгг, 15.10.2025, ONESOFT-4418 
		//+++ Дмгг, 09.10.2025, ONESOFT-4495
		...
		//Малышев 2024-10-01 ONESOFT-3487 - вернул старый вариант
		//// Олег Литвиненко 16.08.23 --->
		//// ONESOFT-2425	
		//|	ВЫБОР
		//|		КОГДА УчетнаяПолитика.ПоступлениеНаСчетСуммаКомиссияИзШапки
		//|			ТОГДА ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.СуммаДокумента 
		//|		ИНАЧЕ ПоступлениеНаСчетЭквайринговыеОперации.СуммаРасчетов + ПоступлениеНаСчетЭквайринговыеОперации.СуммаРасчетовВозврата 
		//|	КОНЕЦ КАК СуммаВал,
		//|	ВЫБОР
		//|		КОГДА УчетнаяПолитика.ПоступлениеНаСчетСуммаКомиссияИзШапки
		//|			ТОГДА ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.СуммаКомиссииДокумента
		...
		//|	ВЫБОР
		//|		КОГДА УчетнаяПолитика.ПоступлениеНаСчетСуммаКомиссияИзШапки
		//|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.СуммаДокумента * КурсыВалютБанковскогоСчета.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютБанковскогоСчета.Кратность) КАК ЧИСЛО(15, 2))
		//|	    ИНАЧЕ ВЫРАЗИТЬ((ПоступлениеНаСчетЭквайринговыеОперации.СуммаРасчетов + ПоступлениеНаСчетЭквайринговыеОперации.СуммаРасчетовВозврата) * КурсыВалютБанковскогоСчета.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютБанковскогоСчета.Кратность) КАК ЧИСЛО(15, 2)) 
		//|	КОНЕЦ КАК Сумма,
		//|	ВЫБОР
		//|		КОГДА УчетнаяПолитика.ПоступлениеНаСчетСуммаКомиссияИзШапки
		//|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.СуммаКомиссииДокумента * КурсыВалютБанковскогоСчета.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютБанковскогоСчета.Кратность) КАК ЧИСЛО(15, 2))
		...
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК Заказ,
		|	ПоступлениеНаСчетЭквайринговыеОперации.Ссылка.ЭквайринговыйТерминал КАК ЭквайринговыйТерминал,
		|	ВЫБОР
		|		КОГДА &РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям
		...
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.СчетУчетаЭквайрингРегл КАК СчетУчетаЭквайрингРегл,
		|	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.СчетЗатратЭквайрингРегл КАК СчетЗатратЭквайрингРегл,
		...
		//+++ Дмгг, 09.10.2025, ONESOFT-4418
		//|,	ВременнаяТаблицаПараметрыУчетаЭквайринговыхОпераций.Проект КАК Проект
		//--- Дмгг, 09.10.2025, ONESOFT-4418 
		//+++ Дмгг, 09.10.2025, ONESOFT-4495
		|,	ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка) КАК Проект
		//--- Дмгг, 09.10.2025, ONESOFT-4495
	...
	//Малышев 2024-10-01 ONESOFT-3487  (--
	ПоступлениеНаСчетСуммаКомиссияИзШапки = РегистрыСведений.УчетнаяПолитикаРегл.ЗначениеПоляУчетнойПолитики("ПоступлениеНаСчетСуммаКомиссияИзШапки", СтруктураДополнительныеСвойства.ДляПроведения.Организация, СтруктураДополнительныеСвойства.ДляПроведения.Дата);
	Если ПоступлениеНаСчетСуммаКомиссияИзШапки ТОгда
		МассивРаспределения = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.СуммаКомиссииДокумента, ТаблицаРезультат.ВыгрузитьКолонку("Сумма"));
		ТаблицаРезультат.ЗагрузитьКолонку(МассивРаспределения,"Комиссия"); 
		
		СуммаКомиссииДокументаВал = СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.СуммаКомиссииДокумента/ (СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.Курс * СтруктураДополнительныеСвойства.ДляПроведения.Ссылка.Кратность);
		
		МассивРаспределения = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(СуммаКомиссииДокументаВал, ТаблицаРезультат.ВыгрузитьКолонку("СуммаВал"));
		ТаблицаРезультат.ЗагрузитьКолонку(МассивРаспределения,"КомиссияВал"); 
		
	КонецЕсли;	
	
	//Малышев 2024-10-01 ONESOFT-3487  --)
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыПоЭквайрингу", ТаблицаРезультат);
...
Процедура СформироватьТаблицаДоходыИРасходыЭквайринг(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Для каждого ТекущаяСтрокаРасчетов Из ТаблицаРасчетыПоЭквайрингу Цикл
		...
		Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетПоПроектам Тогда    
			Если ДокументСсылкаПоступлениеНаСчет.ПроектДляЗачетаАвансаСПомощником = Справочники.Проекты.ПустаяСсылка() Тогда 
				Движение.Проект = Справочники.Проекты.БезПроекта;
			Иначе
				Движение.Проект = ДокументСсылкаПоступлениеНаСчет.ПроектДляЗачетаАвансаСПомощником;
			КонецЕсли;
		КонецЕсли;
		//--- Дмгг, 09.10.2025, ONESOFT-4418
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.Свойство("ПроектДокумента") Тогда
		СтруктураДополнительныеСвойства.ДляПроведения.ПроектДокумента = ДокументСсылкаПоступлениеНаСчет.ПроектДляЗачетаАвансаСПомощником;
	Иначе
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ПроектДокумента", ДокументСсылкаПоступлениеНаСчет.ПроектДляЗачетаАвансаСПомощником);
	КонецЕсли;
	//ПодольскийАИ #УчетПроектов 11.02.2021 }
	...
	// Олег Литвиненко 15.09.22 --->
	|	ТаблицаДокумента.Ссылка.РасчетСКурьером КАК РасчетСКурьером,
	//|	ТаблицаДокумента.Ссылка.ВидОперации КАК ВидОперации,
	|	ТаблицаДокумента.Ссылка.ПриходнаяНакладная КАК ПриходнаяНакладная,
	// Олег Литвиненко <---	
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаДокумента.Ссылка.ПроектДляЗачетаАвансаСПомощником <> Значение(Справочник.Проекты.ПустаяСсылка)
	|						ТОГДА ТаблицаДокумента.Ссылка.ПроектДляЗачетаАвансаСПомощником
	...
	|	КОНЕЦ КАК Проект
	//ПодольскийАИ #БезПроекта 06.09.2021 }     
	|,	ТаблицаДокумента.Ссылка.ПодразделениеЗатраты КАК ПодразделениеЗатраты //+++ Дмгг ONESOFT-2771
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	|,	МАКСИМУМ(ТаблицаДокумента.Ссылка) КАК Ссылка
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	...
	|	ТаблицаДокумента.РасчетСКурьером КАК РасчетСКурьером,
	|	ТаблицаДокумента.ПриходнаяНакладная КАК ПриходнаяНакладная,
	// Олег Литвиненко <---
	...
	//ПодольскийАИ #БезПроекта 07.09.2021 {
	//Было---
	////+++ Дмгг
	...
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	ИначеЕсли ДокументСсылкаПоступлениеНаСчет.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика Тогда
		ИнтеграцияСНалоговойСлужбой_И_БухгалтерскийУчет.СформироватьТаблицаДекларация_ПоступлениеНаСчет_ОтПоставщика(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства); //+++
	// Kuzmich, 24.11.2024, (ONESOFT-3522) Декларация для ИП (Индивидуальный предприниматель)
	Иначе
	...
	//Если ДокументСсылкаПоступлениеНаСчет.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика Тогда 
		СформироватьТаблицаОтчетПоКурсовымРазницам(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства);
	//КонецЕсли;
...
//+++ Дмгг
Процедура СформироватьТаблицаОтчетПоКурсовымРазницам(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПоступлениеНаСчет, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияРасчетыСПодотчетникамиИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияРасчетыСПокупателямиИзменение Тогда
		...
		|		ПО (ПоступлениеНаСчет.Ссылка = &Ссылка)
		|ГДЕ
		|	ВЫБОР
		...
		|		КОНЕЦ
		|	И НЕ(ПоступлениеНаСчет.РасчетСКурьером
		...
		// Олег Литвиненко 16.08.23 --->
		// ONESOFT-
		|	ЕСТЬNULL(УчетнаяПолитика.ПоступлениеНаСчетСуммаКомиссияИзШапки, ЛОЖЬ) КАК ПоступлениеНаСчетСуммаКомиссияИзШапки,
		// Олег Литвиненко <---
		...
		// Олег Литвиненко 16.08.23 --->
		// ONESOFT-2425
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаРегл.СрезПоследних(&МоментКонтроля, ) КАК УчетнаяПолитика
		|		ПО ДвиженияРасчетыПоЭквайрингуИзменение.Организация = УчетнаяПолитика.Организация
		// Олег Литвиненко <---
		|ГДЕ
		|	(&УдалениеПроведения
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ОписаниеТиповЗаказПокупателя = Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя");
	ОписаниеТиповРасходнаяНакладная = Новый ОписаниеТипов("ДокументСсылка.РасходнаяНакладная");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Заказ", НСтр("en='Buyers order';ka='მყიდველის შეკვეთა';ru='Заказ покупателя'"), ОписаниеТиповСтрока25, ОписаниеТиповЗаказПокупателя, , 1, Истина);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СуммаПлатежа", НСтр("en='Amount of Payment';ka='გადახდის თანხა';ru='Сумма платежа'"), ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2, , 2, Истина);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Документ", НСтр("en='Sales Invoice';ka='გასავლის ზედდებული';ru='Расходная накладная'"), ОписаниеТиповСтрока25, ОписаниеТиповРасходнаяНакладная, , 3, Ложь);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СуммаРасчета", НСтр("ka='ანგარიშსწორების თანხა';ru='Сумма расчета'"), ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2, , 2, Ложь);
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных) Экспорт
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.СуммаПлатежа, СтрокаТаблицыФормы.СуммаПлатежа_ВходящиеДанные, 0);

		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьРасходнуюНакладную(СтрокаТаблицыФормы.Документ, СтрокаТаблицыФормы.Документ_ВходящиеДанные);
		
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.СуммаРасчета, СтрокаТаблицыФормы.СуммаРасчета_ВходящиеДанные, 0);
...
Процедура ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения = "") Экспорт
	...
	СтрокаТаблицыФормы[ИмяСлужебногоПоля] = ЗначениеЗаполнено(СтрокаТаблицыФормы.Заказ)
		И ЗначениеЗаполнено(СтрокаТаблицыФормы.СуммаПлатежа);
...
Процедура СформироватьТаблицаРасчетыСПрочимиКонтрагентами(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаРасшифровкаПлатежа.Договор КАК Договор,
	// Олег Литвиненко 26.09.22 --->
	|	ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации КАК ВидОперации,
	|	ВременнаяТаблицаРасшифровкаПлатежа.РасчетСКурьером КАК РасчетСКурьером,
	// Олег Литвиненко <---
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаРасшифровкаПлатежа.ВестиРасчетыПоДокументам
	|			ТОГДА ВЫБОР
	...
	|	ИЛИ ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты)
	// Олег Литвиненко 27.09.22 --->
	|	И НЕ ВременнаяТаблицаРасшифровкаПлатежа.РасчетСКурьером
	// Олег Литвиненко <---
	|
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	|	ПоступлениеНаСчет.РасчетСКурьером КАК РасчетСКурьером,
	// Олег Литвиненко <---
	...
	//|	ВЫБОР
	//|		КОГДА ПоступлениеНаСчет.ПроектДляЗачетаАвансаСПомощником = Значение(Справочник.Проекты.ПустаяСсылка)
	//|			ТОГДА Значение(Справочник.Проекты.БезПроекта)
	//|		ИНАЧЕ ПоступлениеНаСчет.ПроектДляЗачетаАвансаСПомощником
	//|	КОНЕЦ КАК Проект
	|	ВЫБОР   
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ВозвратКредитаКонтрагентом)
