...
Процедура ПроверкаЗаполненияДокумента(ПриверяемыеРеквизиты, НепроверяемыеРеквизиты, Отказ)Экспорт
	...
	Пока ВыборкаДокументовДляПроверки.Следующий() Цикл
		...
		Если Не ВыборкаДокументовДляПроверки.ФайлСформирован  Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტს არ აქვს ფაილი ჩამოყალიბებული!';ru='У документа не сформирован файл!'"), ВыборкаДокументовДляПроверки.НомерСтроки);
		...
		Если ВыборкаДокументовДляПроверки.ОтчетныйПериод <> ОтчетныйПериод Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასტა-დოკუმენტის საანგარიშგებო პერიოდი აღნიშნულისგან განსხვავდება!';ru='Отчетный период документа-пачки отличается от указанного!'"), ВыборкаДокументовДляПроверки.НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.ПачкиДокументов[" + Формат(ВыборкаДокументовДляПроверки.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].ПачкаДокументов",,Отказ);
		КонецЕсли;
		
		Если ВыборкаДокументовДляПроверки.Организация <> Организация Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტი-დასტა გაფორმებულია სხვა ორგანიზაციაზე!';ru='Документ-пачка оформлен на другую организацию!'"), ВыборкаДокументовДляПроверки.НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.ПачкиДокументов[" + Формат(ВыборкаДокументовДляПроверки.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].ПачкаДокументов",,Отказ);
		КонецЕсли;
		
		Если ВыборкаДокументовДляПроверки.НомерПачки = НомерПачки Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დასტა-დოკუმენტს იგივე ნომერი გააჩნია, რაც აღწერის დოკუმენტს, რეკომენდებულია აღწერის დოკუმენტში დასტის ნომრის შეცვლა!';ru='Документ-пачка имеет такой же номер как и документ описи, рекомендуется изменить номер пачки в документе описи!'"), ВыборкаДокументовДляПроверки.НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.ПачкиДокументов[" + Формат(ВыборкаДокументовДляПроверки.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].ПачкаДокументов",,Отказ);
		КонецЕсли;
		
		Если ВыборкаДокументовДляПроверки.НомерСтрокиДубль <> Null Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ka='დოკუმენტი-დასტა ორჯერ არის მითითებული!';ru='Документ-пачка указан дважды!'"), ВыборкаДокументовДляПроверки.НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.ПачкиДокументов[" + Формат(ВыборкаДокументовДляПроверки.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].ПачкаДокументов",,Отказ);
		КонецЕсли;
