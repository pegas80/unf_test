...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ТаблицаДокумента.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаЗапасов,
	|	ТаблицаДокумента.Номенклатура.СчетУчетаЗапасовРегл КАК СчетУчетаЗапасовРегл,
	...
	//ПодольскийАИ #УчетПроектов 29.07.2021 {
	|	ВЫБОР
	|		КОГДА &УчетПоПроектам = ЛОЖЬ ТОГДА Значение(Справочник.Проекты.ПустаяСсылка) //Малышев 2024-06-26 ONESOFT-3286
	...
	|	НЕОПРЕДЕЛЕНО КАК ДокументПоступления,  //Малышев 2024-11-30 ONESOFT-3645
...
//
Процедура ВыполнитьКонтроль(ДокументСсылкаПринятиеКУчетуВА, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь,ПроверятьИтогиРегистров = ЛОЖЬ) Экспорт
	// Kuzmich, 02.08.2019, Контролировать остатки запасов на складах при проведении документов
	Если Константы.КонтролироватьОстаткиЗапасовНаСкладахПриПроведении.Получить() Тогда
		
		Результат = УправлениеНебольшойФирмойСервер.ЗапросОтрицательныхОстатковЗапасовНаСкладахПриПроведенииДокумента(ДокументСсылкаПринятиеКУчетуВА);
		
		Если НЕ Результат.Пустой() Тогда
			ДокументСсылкаПринятиеКУчетуВА_Объект = ДокументСсылкаПринятиеКУчетуВА.ПолучитьОбъект();
			ВыборкаИзРезультатаЗапроса = Результат.Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументСсылкаПринятиеКУчетуВА_Объект, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;			
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияВнеоборотныеАктивыИзменение Тогда
		...
		Если НЕ МассивРезультатов[0].Пустой()
			...
			ИЛИ НЕ МассивРезультатов[3].Пустой() //+++ Дмгг
			Тогда
		...
		Если НЕ МассивРезультатов[0].Пустой() Тогда
			...
			Иначе 
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументОбъектПринятиеКУчетуВА, ВыборкаИзРезультатаЗапроса, Отказ,,Истина);
			КонецЕсли;
		...
		Если НЕ ДополнительныеСвойства.ИспользованиеТолькоРеглПланаСчетов Тогда 
			Если НЕ МассивРезультатов[1].Пустой() Тогда
				...
				Иначе 
					УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОбъектПринятиеКУчетуВА, ВыборкаИзРезультатаЗапроса, Отказ,,Истина);
				КонецЕсли;	
			КонецЕсли;
		Иначе
		...
		Если НЕ МассивРезультатов[2].Пустой() Тогда
			...
			Иначе
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструВнеоборотныеАктивы(ДокументОбъектПринятиеКУчетуВА, ВыборкаИзРезультатаЗапроса, Отказ,,Истина);
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗапасы.ДокументПоступления КАК ДокументПоступления, //Малышев 2024-11-30 ONESOFT-3645
	...
	|	ТаблицаЗапасы.ДокументПоступления, //Малышев 2024-11-30 ONESOFT-3645
	...
	Если СтруктураДополнительныеСвойства.УчетнаяПолитика.УчетСебестоимостиВРазрезеОрганизации Тогда
	
		ТаблицаРезультата.ЗаполнитьЗначения(Неопределено, "СтруктурнаяЕдиница");	
	
	КонецЕсли; 
	// © Шишка В.В. [17 августа 2021]  КОНЕЦ
	//--------------------------------------------------------------------------
	
	//Малышев 2024-11-30 ONESOFT-3645
	ТаблицаЗапасыОстатки = УправлениеНебольшойФирмойСервер.ПолучитьОстаткиЗапасовПоСтоимостиПриПроведении(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства, ИСТИНА).Выгрузить();
	
	ВременнаяТаблицаТоваров = ТаблицаРезультата.СкопироватьКолонки();
	
	Для Каждого СтрокаТаблицаЗапасы из ТаблицаРезультата Цикл
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Для каждого СтрокаТаблицыОстатков из МассивСтрокОстатков Цикл
				...
				НоваяСтрокаРасход = ВременнаяТаблицаТоваров.Добавить();
				...
				НоваяСтрокаРасход.ДокументПоступления = СтрокаТаблицыОстатков.ДокументПоступления;
				
			КонецЦикла;
			
			// Kuzmich, 24.04.2025, (ONESOFT-3975) ნიკა პლასტი - წარმოების დოკუმენტის დროს, უარყოფითი ნაშთები არ აისახება მარაგებში და დანახარჯებში
			// делаем отрицательный остаток
			Если КоличествоТребуется > 0 Тогда
				
				НоваяСтрокаРасходОтрицательныеОстатки = ВременнаяТаблицаТоваров.Добавить();
				...
				Иначе	
					ЗаполнитьЗначенияСвойств(НоваяСтрокаРасходОтрицательныеОстатки, НоваяСтрокаРасход);
				КонецЕсли;
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасы", ВременнаяТаблицаТоваров);
...
Процедура СформироватьТаблицаЗапасыКРасходуСоСкладов(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства)
	...
	КонецЕсли; 
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыКРасходуСоСкладов", ТаблицаРезультата);
