...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Если ИтогПервоначальнаяСтоимость <> Сумма Тогда
		ТекстСообщения = НСтр("ka='ნომენკლატურის ღირებულება: %Сумма% , არ შეესაბამება ქონების თავდაპირველი ღირებულებების თანხას: %ИтогПервоначальнаяСтоимость%';ru='Стоимость номенклатуры: %Сумма% , не соответствует сумме первоначальных стоимостей имущества: %ИтогПервоначальнаяСтоимость%'"
		);
	...
	Для каждого СтрокаВнеоборотныхАктивов Из ВнеоборотныеАктивы Цикл
		...
		Если МассивВАСостояния.Найти(СтрокаВнеоборотныхАктивов.ВнеоборотныйАктив) <> Неопределено Тогда
			ТекстСообщения = НСтр("ka='სიის ""ქონება"" სტრიქონ %НомерСтроки% მითითებულ ქონება ""%ВнеоборотныйАктив%"" მიმდინარე მდგომარეობაა ""სააღრიცხვოდ მიღებულია"".';ru='Для имущества ""%ВнеоборотныйАктив%"" указанного в строке %НомерСтроки% списка ""Имущество"", текущее состояние ""Принят к учету"".'"
			);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВнеоборотныйАктив%", СокрЛП(Строка(СтрокаВнеоборотныхАктивов.ВнеоборотныйАктив))); 
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%",Строка(СтрокаВнеоборотныхАктивов.НомерСтроки));
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
...
Процедура ПерезаписатьНачалнуюСтоймость()
	...
	Для Каждого Стр Из ВнеоборотныеАктивы Цикл 
		...
		ОбВнеоборотныйАктив.НачальнаяСтоимость = Стр.НачальнаяСтоимость;
