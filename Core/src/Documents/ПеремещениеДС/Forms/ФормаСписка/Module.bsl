...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		...
		"ФормаСписка",
		Неопределено,
		НСтр("ka='Новости: Перемещение денег';ru='Новости: Перемещение денег';en='News: Moving money'"),
		Ложь,
...
Процедура ОтборОперацияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыВызова.Вставить("ПоказыватьПриход", Ложь);
	ПараметрыВызова.Вставить("ПоказыватьРасход", Ложь);
	ОткрытьФорму("Обработка.БанкИКасса.Форма.ФормаВыбораВидаОперации",ПараметрыВызова,,,,,
...
Процедура ОтборХозОперацияНачалоВыбораЗавершение(РезультатЗавершения, ПараметрыЗавершения) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатЗавершения) Тогда
		Если ТипЗнч(РезультатЗавершения) = Тип("Строка") Тогда
			...
			УстановитьМеткуИОтборСписка("ВидОперации", "ГруппаОтборОперация", РезультатЗавершения);
