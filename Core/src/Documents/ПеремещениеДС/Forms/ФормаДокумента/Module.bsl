...
Процедура ВалютаДенежныхСредствПриИзменении(Элемент)
	...
	Если Валюта <> Объект.ВалютаДенежныхСредств Тогда
		...
		Если Объект.СуммаДокумента <> 0 Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ВалютаДенежныхСредствПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("СтруктураДанные", СтруктураДанные)), НСтр("ka='შეიცვალა დოკუმენტის ვალუტა. გადავთვალოთ დოკუმენტის თანხა?';ru='Изменилась валюта документа. Пересчитать сумму документа?'"), Режим);
            Возврат;
		КонецЕсли;
