...
Процедура СформироватьДвиженияПоРегиструУчетаЗатрат(НаборЗаписей, НаборЗаписейУправленческий, НаборЗаписейХозрасчетный, СтрокаДвижения, Сумма, ФиксированнаяСтоимость, СодержаниеПроводки = Неопределено)
	...
	Если СодержаниеПроводки = Неопределено Тогда
		Если СтрокаДвижения.СчетУчетаТипСчета = Перечисления.ТипыСчетов.Запасы Тогда
			СодержаниеПроводки = НСтр("ka='მარაგების ჩამოწერა საწყობიდან';ru='Списание запасов со склада'");
		Иначе
			Если ЗначениеЗаполнено(СтрокаДвижения.Номенклатура) Тогда
				СодержаниеПроводки = НСтр("ka='ხარჯების ჩამოწერა';ru='Списание расходов'");
			Иначе
				СодержаниеПроводки = НСтр("ka='საწარმოდან მარაგების ჩამოწერა';ru='Списание запасов из производства'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрокаДвижения.КоррСчетУчетаТипСчета = Перечисления.ТипыСчетов.Запасы Тогда
		СодержаниеПроводки = НСтр("ka='საწყობში მარაგების შემოსავალში გატარება';ru='Оприходование запасов на склад'");
	Иначе
		Если ЗначениеЗаполнено(СтрокаДвижения.Номенклатура) Тогда
			СодержаниеПроводки = НСтр("ka='ხარჯების შემოსვლა';ru='Поступление расходов'");
		Иначе
			СодержаниеПроводки = НСтр("ka='წარმოებაში მარაგების შემოსავალში აყვანა';ru='Оприходование запасов в производство'");
		КонецЕсли;
	КонецЕсли;
	...
	//+++ ДМГГ
	Если СодержаниеПроводки = НСтр("ka='წარმოებაში მარაგების შემოსავალში აყვანა';ru='Оприходование запасов в производство'") 
			...
			Если ВыборкаДетальныеЗаписи.КоррСчетУчета = ДополнительныеСвойства.ДляПроведения.ПустойСчет
				...
				Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
				  ...
				  ИЛИ ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.ОтчетОПереработке")) Тогда
					...
					НоваяСтрока.СодержаниеПроводки = НСтр("ka='ხარჯების ასახვა';ru='Отражение расходов'");
					...
					НоваяСтрока.Содержание = НСтр("ka='ხარჯების ასახვა';ru='Отражение расходов'");
				...
				ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СуммаРасходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ka='სხვა ხარჯები';ru='Прочие расходы'");
					Иначе
						...
						НоваяСтрока.СодержаниеПроводки = НСтр("ka='სხვა შემოსავლები';ru='Прочие доходы'");
					Конецесли;
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.Сумма = - СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ka='სხვა ხარჯები';ru='Прочие расходы'");
					Иначе
						НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.СчетКт = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.Сумма = СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ka='სხვა შემოსავლები';ru='Прочие доходы'");
					КонецЕсли;
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.ПеремещениеВРозницуСуммовойУчет Тогда
					...
					НоваяСтрока.СодержаниеПроводки = НСтр("ka='საცალო ვაჭრობაში გადაადგილება';ru='Перемещение в розницу'");
					НоваяСтрока.Себестоимость = СуммаКорректировки;
					...
					НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					НоваяСтрока.Содержание = НСтр("ka='საცალო ვაჭრობაში გადაადგილება';ru='Перемещение в розницу'");
					НоваяСтрока.Сумма = СуммаКорректировки;
				...
				Иначе
					...
					//НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					//НоваяСтрока.Содержание = НСтр("ka='მარაგების ჩამოწერა თავისუფალ ანგარიშზე';ru='Списание запасов на произвольный счет'");
					//НоваяСтрока.Сумма = СуммаКорректировки;
					//
					//// Движения по регистру Хозрасчетный.
					//НоваяСтрока = НаборЗаписейХозрасчетный.Добавить();
...
Процедура РаспределитьСуммыБезКоличества(ВидОперации, ТаблицаОшибок)
	...
	Пока НЕ Результат.Пустой() Цикл
		...
		Если КоличествоИтераций > 60 Тогда
			ОписаниеОшибки = НСтр("ka='ხარჯების მიხედვით თანხობრივი ნაშთები კორექტირება ვერ მოხერხდა.';ru='Не удалось скорректировать суммовые остатки по затратам.'");
			...
			Если ВыборкаДетальныеЗаписи.КоррСчетУчета = ДополнительныеСвойства.ДляПроведения.ПустойСчет
				...
				Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
				  ...
				  ИЛИ ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.ОтчетОПереработке")) Тогда
					...
					НоваяСтрока.СчетУчетаРегл = ВыборкаДетальныеЗаписи.НаправлениеДеятельностиПродажиСчетУчетаСебестоимостиПродажРегл;
					НоваяСтрока.СуммаРасходов = СуммаКорректировки;
					НоваяСтрока.СодержаниеПроводки = НСтр("ka='რეალიზაციისას ხარჯების ასახვა';ru='Отражение расходов по продаже'");
					...
					НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.НаправлениеДеятельностиПродажиСчетУчетаСебестоимостиПродаж;
					НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					НоваяСтрока.Содержание = НСтр("ka='რეალიზაციისას ხარჯების ასახვა';ru='Отражение расходов по продаже'");
					НоваяСтрока.Сумма = СуммаКорректировки;
				...
				ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СчетУчетаРегл = СчетПрочиеРасходыРегл;
						НоваяСтрока.СуммаРасходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ka='სხვა ხარჯები მომწოდებლისთვის დაბრუნებისას';ru='Прочие расходы по возврату поставщику'");
					Иначе
						НоваяСтрока.СчетУчета = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.СчетУчетаРегл = СчетПрочиеДоходыРегл;
						НоваяСтрока.СуммаДоходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ka='სხვა შემოსავლები მომწოდებლისთვის დაბრუნებისას';ru='Прочие доходы по возврату поставщику'");
					Конецесли;
					НоваяСтрока.Аналитика = ВыборкаДетальныеЗаписи.Номенклатура;
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.Сумма = - СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ka='სხვა ხარჯები მომწოდებლისთვის დაბრუნებისას';ru='Прочие расходы по возврату поставщику'");
					Иначе
						НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.СчетКт = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.Сумма = СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ka='სხვა შემოსავლები მომწოდებლისთვის დაბრუნებისას';ru='Прочие доходы по возврату поставщику'");
					КонецЕсли;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если (НЕ Окр(СуммаКорректировки, 2) = 0) Тогда
			...
			СформироватьДвиженияПоРегиструУчетаЗатрат(
				...
				НСтр("ka='მყიდველისგან დაბრუნების თვითღირებულება';ru='Себестоимость возврата от покупателя'")
			);
...
Процедура РассчитатьСебестоимостьВРозницеСуммовойУчет(Отказ, ТаблицаОшибок)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрока.СодержаниеПроводки = НСтр("ka='თვითღირებულება';ru='Себестоимость'");
		...
		НоваяСтрока.Содержание = НСтр("ka='თვითღირებულება';ru='Себестоимость'");
		...
		НоваяСтрока.Содержание = НСтр("ka='ფასნამატი';ru='Наценка'");
		НоваяСтрока.Сумма = - ВыборкаДетальныеЗаписи.СуммаКорректировки;
	...
	Если _РасчетКурсовыхРазницКредиты Тогда
			...
			Если СтрокиПоступления.Количество() > 0 Тогда
				...
				Пока ВыборкаЗакрытиеКредита.Следующий() Цикл
					...
					Если Итератор = 1 И СтрокиПоступления[0].ОсновнойДолгВалОстаток = 0 Тогда 
						...
						НоваяСтрока.Содержание = НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница кредиты'");
						...
						НоваяСтрокаРасчетыПоКредитамИЗаймам.СодержаниеПроводки = НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница кредиты остаток'");
					...
					НоваяСтрокаДоходыИРасходы.СодержаниеПроводки = НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница кредиты'");
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтрокиПоступления.Количество() > 0 Тогда
				...
				НоваяСтрока.Содержание = НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница кредиты остаток'");
	...
	Если _РасчетКурсовыхРазницПроценты Тогда
			...
			Если СтрокиПоступления.Количество() > 0 Тогда
				...
				Пока ВыборкаЗакрытиеКредита.Следующий() Цикл
					...
					Если Итератор = 1 И СтрокиПоступления[0].ПроцентыВалОстаток = 0 Тогда 
						...
						НоваяСтрока.Период = КонецМесяца(Дата);
						НоваяСтрока.Сумма = КурсоваяРазница;
						НоваяСтрока.Содержание = НСтр("ka='დარიცხული პროცენტების საკურსო სხვაობა';ru='Курсовая разница проценты кредиты'");
						...
						НоваяСтрокаРасчетыПоКредитамИЗаймам.Период = КонецМесяца(Дата);
						НоваяСтрокаРасчетыПоКредитамИЗаймам.СодержаниеПроводки = НСтр("ka='სხვაობა კურსებს შორის';ru='Курсовая разница проценты кредиты'");
						НоваяСтрокаРасчетыПоКредитамИЗаймам.ХозяйственнаяОперация = Справочники.ХозяйственныеОперации.ЗакрытиеМесяца;
					ИначеЕсли  Итератор <> 1 И СтрокиПоступления[0].ПроцентыВалОстаток <> 0 Тогда//И СтрокиПоступления[0].ОсновнойДолгВалОстаток <> 0 Тогда //+++ Дмгг
						...
						НоваяСтрокаРасчетыПоКредитамИЗаймам.ПроцентыВал = 0;
						НоваяСтрокаРасчетыПоКредитамИЗаймам.Период = КонецМесяца(Дата);
						НоваяСтрокаРасчетыПоКредитамИЗаймам.СодержаниеПроводки = НСтр("ka='დარიცხული პროცენტების საკურსო სხვაობა';ru='Курсовая разница проценты кредиты остаток'");
						НоваяСтрокаРасчетыПоКредитамИЗаймам.ХозяйственнаяОперация = Справочники.ХозяйственныеОперации.ЗакрытиеМесяца;
					КонецЕсли;
					...
					НоваяСтрокаДоходыИРасходы.Организация = Организация;
					НоваяСтрокаДоходыИРасходы.НаправлениеДеятельности = Справочники.НаправленияДеятельности.Прочее;
					НоваяСтрокаДоходыИРасходы.СодержаниеПроводки = НСтр("ka='დარიცხული პროცენტების საკურსო სხვაობა';ru='Курсовая разница проценты кредиты'");
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтрокиПоступления.Количество() > 0 Тогда
				...
				НоваяСтрока.Содержание = НСтр("ka='დარიცხული პროცენტების საკურსო სხვაობა';ru='Курсовая разница кредиты остаток'");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ka='კალენდარული თვის დახურვისას ფორმირებული იქნა გაფრთხილებები! დაწვრილებით, იხ. ანგარიშგებაში კალენდარული თვის დახურვის თაობაზე.';ru='При закрытии месяца были сформированы предупреждения! Подробнее см. в отчете о закрытии месяца.'");
