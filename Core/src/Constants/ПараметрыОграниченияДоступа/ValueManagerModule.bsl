...
""
"';ka='შეცდომა საერთო მოდულის დაშვებისმართვახელახლაგანსაზღვრული პროცედურაში დაშვებისტიპისთვისებებისშევსება.'");
	...
	Для каждого ВидДоступа Из ВидыДоступа Цикл
		...
		Если ВсеИменаВидовДоступа[ВРег(ВидДоступа.Имя)] <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ka='დაშვების სახეობა ""%1""-ის სახელი უკვე დადგენილია.';ru='Имя вида доступа ""%1"" уже определено.';en='type of access name ""%1"" has already been defined.'"),
				ВидДоступа.Имя);
		КонецЕсли;
...
"не указан в определяемом типе ""Значение доступа"".';ka='პროგრამის ახალ ვერსიებზე გადასასვლელად საჭირო ტიპი მახასიათებლებისსახეობებისგეგმაბმული.დაშვებისსახეობებისწაშლა განსაზღვრებად ტიპ ""დაშვების მნიშვნელობა""-ში არ არის მითითებული.'");
...
"в определяемом типе ""Значение доступа с группами значений доступа объект"".';ka='დაშვების მნიშვნელობის ტიპი ""%1"", რომელიც მნიშვნელობების ჯგუფს იყენებს, შემდეგ განსაზღვრებად ტიპში არ არის მითითებული ""დაშვების მნიშვნელობა დაშვების მნიშვნელობების ჯგუფებით ობიექტი"".'"),
			Строка(ТипОбъекта));
	КонецЕсли;
...
Процедура ПроверитьТип(ВидДоступа, Тип, ВсеТипы, Параметры, ПроверкаТиповГрупп = Ложь, ПроверкаПересечения = Ложь)
	...
	Если Тип = Тип("Неопределено") Тогда
		...
		КонецЕсли;
		ВызватьИсключение Параметры.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ka='დაშვების სახეობა ""%1""-სთვის დაშვების მნიშვნელობების ტიპი არ არის მითითებული.';ru='Для вида доступа ""%1"" не указан тип значений доступа.';en='For the type of access ""%1"" does not indicate the type of access values.'"),
			ВидДоступа.Имя);
	КонецЕсли;
...
"Однако это не тип ссылки.';ka='ტიპი ""%1"" მითითებულია, როგორც მნიშვნელობების ჯგუფების ტიპი, წვდომის სახეობა ""%2""-სთვის. თუმცა, ეს არ არის ბმულის ტიპი.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип значений, для вида доступа ""%2""."
"Однако это не тип ссылки.';ka='ტიპი ""%1"" მითითებულია როგორც მნიშვნელობების ტიპი, დაშვების სახეობა ""%2""-სთვის. თუმცა, ეს არ არის ბმულის ტიპი.'");
		КонецЕсли;
		ВызватьИсключение Параметры.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ПроверкаТиповГрупп Тогда
		Если ПроверкаПересечения Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип значений, для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать, как тип групп значений.';ka='ტიპი ""%1"" მითითებულია, როგორც მნიშვნელობების ტიპი, დაშვების სახეობა ""%2""-სთვის."
"დაშვების სახეობა ""%3""-სთვის მისი მითითება, როგორც მნიშვნელობების ჯგუფის ტიპი არ შეიძლება.'");
		Иначе
			ДляТогоЖеВидаДоступаОшибкиНет = Истина;
			ОписаниеОшибки =
				НСтр("ru='Тип групп значений ""%1"" уже указан для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать.';ka='მნიშვნელობების ჯგუფების ტიპი ""%1"" უკვე მითითებულია დაშვების სახეობა ""%2""-სთვის. დაშვების სახეობა ""%3""-სთვის მისი მითითება არ შეიძლება.'");
		КонецЕсли;
	Иначе
		Если ПроверкаПересечения Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип групп значений, для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать, как тип значений.';ka='ტიპი ""%1"" მითითებულია, როგორც მნიშვნელობების ჯგუფების ტიპი, დაშვების სახეობა ""%2""-სთვის."
"დაშვების სახეობა ""%3""-სთვის მისი მითითება, როგორც მნიშვნელობების ტიპი არ შეიძლება.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип значений ""%1"" уже указан для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать.';ka='მნიშვნელობების ტიპი ""%1"" დაშვების სახეობა ""%2""-სთვის უკვე მითითებულია. დაშვების სახეობა ""%3""-სთვის მისი მითითება არ შეიძლება.'");
		КонецЕсли;
	КонецЕсли;
...
"не указан в определяемом типе ""Значение доступа"".';ka='დაშვების სახეობა ""%2""-ის დაშვების მნიშვნელობების ჯგუფების ტიპი ""%1"" განსაზღვრებად ტიპ ""დაშვების მნიშვნელობა""-ში არ არის მითითებული.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип значений доступа ""%1"" вида доступа ""%2"""
"не указан в определяемом типе ""Значение доступа"".';ka='წვდომის სახეობა ""%2""-ის წვდომის მნიშვნელობების ტიპი ""%1"" დასადგენ ტიპ ""წვდომის მნიშვნელობა""-ში არ არის მითითებული.'");
		КонецЕсли;
	КонецЕсли;
