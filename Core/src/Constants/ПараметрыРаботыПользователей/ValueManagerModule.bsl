...
Процедура ПроверитьНазначениеРолей(НазначениеРолей = Неопределено, ПроверитьВсе = Ложь, СписокОшибок = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ka='შეცდომა საერთო მოდულ მომხმარებლებიხელახლაგანსაზღვრებადი-ს პროცედურა როლებისდანიშვნისდადგენისას-ში';ru='Ошибка в процедуре ПриОпределенииНазначенияРолей общего модуля ПользователиПереопределяемый.'");
...
"%2, %3.';ka='როლი ""%1"" ერთზე მეტ დანიშნულებაში არის მითითებული: %2, %3.'"),
	...
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.ТолькоДляВнешнихПользователей, ТекстОшибки,
		НСтр("ka='მხოლოდ გარე მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей только для внешних пользователей найдены ошибки:'"), СписокОшибок);
	
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.СовместноДляПользователейИВнешнихПользователей, ТекстОшибки,
		НСтр("ka='ერთობლივად მომხმარებლებისთვის და გარე მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей совместно для пользователей и внешних пользователей найдены ошибки:'"), СписокОшибок);
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ka='დანართის მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей для пользователей приложения найдены ошибки:'"), СписокОшибок, НеразделенныеДанные);
	КонецЕсли;
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ka='მომხმარებლებისთვის როლების შემოწმებისას შეცდომები მოიძებნა:';ru='При проверке ролей для пользователей найдены ошибки:'"), СписокОшибок);
	...
	Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
		...
		Для Каждого НедоступноеПраво Из НедоступныеПрава Цикл
			Если ПравоДоступа(НедоступноеПраво, Метаданные, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს მიუწვდომელ უფლება %2-ს.';ru='Роль ""%1"" содержит недоступное право %2.'"),
		...
		Для Каждого СвойстваДанных Из НеразделенныеДанные Цикл
			...
			Если ПравоДоступа("Изменение", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს უფლება ""განუცალკევებელი ობიექტის ცვლილება""-ს %2.';ru='Роль ""%1"" содержит право Изменение неразделенного объекта %2.'"),
			...
			Если ПравоДоступа("Добавление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს უფლება ""განუცალკევებელი ობიექტის დამატება""-ს %2.';ru='Роль ""%1"" содержит право Добавление неразделенного объекта %2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
			...
			Если ПравоДоступа("Удаление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ka='როლი ""%1"" შეიცავს უფლება ""განუცალკევებელი ობიექტის წაშლა""-ს %2.';ru='Роль ""%1"" содержит право Удаление неразделенного объекта %2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
