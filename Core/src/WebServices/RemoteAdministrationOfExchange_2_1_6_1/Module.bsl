...
Функция ПолучитьПланыОбменаКонфигурации()
	...
	Для Каждого ИмяПланаОбмена Из ОбменДаннымиВМоделиСервисаПовтИсп.ПланыОбменаСинхронизацииДанных() Цикл
		...
		Если ПустаяСтрока(ExchangeFeature.ExchangeRole) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ka='გაცვლის გეგმის მენეჯერის მოდულში %1 ფუნქცია წყაროსკონფიგურაციისსახელი() -ის დაბრუნებადი მნიშვნელობა არ არის მიცემული';ru='Не задано возвращаемое значение функции ИмяКонфигурацииИсточника() в модуле менеджера плана обмена %1'"),
				ИмяПланаОбмена
			);
		КонецЕсли;
...
Функция ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДаннымиXDTO)
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ka='მონაცემთა გაცვლის უცნობი რეჟიმი %1';ru='Неизвестный режим обмена данными %1'"), Строка(РежимОбмена)
		);
	КонецЕсли;
