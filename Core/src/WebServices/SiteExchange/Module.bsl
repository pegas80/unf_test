...
Функция ПолучитьСсылкуПоИдентификатору(МенеджерОбъекта, Знач СтрокаGUID)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСТр("ka='GetPicture: იდენტიფიკატორის მიხედვით ობიექტის მიღება ვერ მოხერხდა.';ru='GetPicture: не удалось получить объект по идентификатору.';en='GetPicture: failed to get object by ID.'"), УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки.Описание);
		ВызватьИсключение;
	КонецПопытки;
...
Функция ЗагрузитьЗаказы(ДанныеЗаказовXDTO)
	...
	Если Не ОбменССайтом.ЗагрузитьЗаказы(ДанныеЗаказовXDTO, СтруктураСтатистики, Параметры, ОписаниеОшибки) Тогда 
		...
		ОбменССайтом.ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ka='სერვერიდან ჩატვირთული დოკუმენტების დამუშავება ვერ მოხერხდა.';ru='Не удалось обработать документы, загруженные с сервера.';en='Unable to process the documents downloaded from the server.'"));
		ЕстьОшибки = Истина;
		
	КонецЕсли;
	
	ЗаписатьРезультатВыполненияОперацииВЖурналРегистрации(НСтр("ka='საიტთანგაცვლა.შეკვეთებისჩატვირთვა';ru='ОбменССайтом.ЗагрузкаЗаказов';en='ObmenSSaytom.ZagruzkaZakazov'"), ОписаниеОшибки, ЕстьОшибки);
...
Функция ПолучитьДвоичныеДанныеФайла(ДанныеФайла)
	...
	Если ФайлВХранилище Тогда
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			ОписаниеОшибки = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСТр("ka='GetPicture: ვერ მოხერხდა ნომენკლატურა %2-ის ფაილ %1-ის მონაცემთა მიღება.';ru='GetPicture: не удалось получить данные файла %1 номенклатуры %2.';en='GetPicture: failed to retrieve data file% 1% 2 nomenclature.'"),
				ДанныеФайла.Файл,
