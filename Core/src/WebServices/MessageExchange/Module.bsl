...
Функция ДоставитьСообщения(КодОтправителя, ПотокХранилище)
	...
	Если Отправитель.Пустая() Тогда
		...
		ВызватьИсключение НСтр("ka='მიცემულია ბოლო წერტილთან მიერთების არასწორი გამართვები.';ru='Заданы неправильные настройки подключения к конечной точке.'");
...
Функция ПроверитьПодключениеНаСторонеОтправителя(КодОтправителя)
	...
	Если ОбменСообщениямиВнутренний.КодЭтогоУзла() <> КодОтправителя Тогда
		...
		ВызватьИсключение НСтр("ka='მიმღების ბაზის ჩართვის გამართვები მიუთითებენ სხვა გამგზავნზე.';ru='Настройки подключения базы получателя указывают на другого отправителя.'");
