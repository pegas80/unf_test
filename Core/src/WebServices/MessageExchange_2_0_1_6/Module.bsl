...
Функция ДоставитьСообщения(КодОтправителя, ПотокХранилище)
	...
	Если Отправитель.Пустая() Тогда
		...
		ВызватьИсключение НСтр("ka='მიცემულია ბოლო წერტილთან მიერთების არასწორი გამართვები.';ru='Заданы неправильные настройки подключения к конечной точке.';en='Given incorrect settings to connect to the endpoint.'");
...
""
"Эти сообщения будут обработаны в рамках очереди обработки сообщений системы.';ka='სწრაფი შეტყობინებების მიწოდებისას შეცდომა მოხდა - ზოგიერთი შეტყობინება არ იქნა მიწოდებული მონაცემთა არეალების დაყენებული ბლოკირების გამო!"
""
"ეს შეტყობინებები დამუშავდება სისტემის შეტყობინებების დამუშავების რიგის ფარგლებში.'");
...
Функция ПроверитьПодключениеНаСторонеОтправителя(КодОтправителя)
	...
	Если ОбменСообщениямиВнутренний.КодЭтогоУзла() <> КодОтправителя Тогда
		...
		ВызватьИсключение НСтр("ka='მიმღების ბაზის ჩართვის გამართვები მიუთითებენ სხვა გამგზავნზე.';ru='Настройки подключения базы получателя указывают на другого отправителя.';en='Settings database connection point to another recipient of the sender.'");
