...
Функция СоздатьОбменДанными(ИмяПланаОбмена, СтрокаПараметров, НастройкаОтборовXDTO, ЗначенияПоУмолчаниюXDTO)
	...
	Если Отказ Тогда
		Сообщение = НСтр("ka='გაცვლის გამართვის შექმნისას მეორე საინფორმაციო ბაზაში შეცდომები წარმოიშვა: %1';ru='При создании настройки обмена во второй информационной базе возникли ошибки: %1'");
...
Функция GetFilePart(TransferId, PartNumber, PartData)
	...
	Если ИменаФайлов.Количество() = 0 Тогда
		...
		ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %2-თან გადაცემის სესიის ფრაგმენტი %1 ვერ მოიძებნა';ru='Не найден фрагмент %1 сессии передачи с идентификатором %2'");
	...
	ИначеЕсли ИменаФайлов.Количество() > 1 Тогда
		...
		ШаблонСообщения = НСтр("ka='მოიძებნა გადაცემის სესიის რამდენიმე ფრაგმენტი %1 იდენტიფიკატორით %2';ru='Найдено несколько фрагментов %1 сессии передачи с идентификатором %2'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Строка(PartNumber), Строка(TransferId));
		ВызватьИсключение(ТекстСообщения);
...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзла).Пустая() Тогда
		Результат = НСтр("ka='მონაცემთა სინქრონიზაციის შესაძლებლობა გამორთულია ინტერნეტში დანართის ადმინისტრატორის მიერ.';ru='Возможность синхронизации данных отключена администратором приложения в Интернете.'");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления()
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ka='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.';ru='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.'");
...
Процедура ВыполнитьВыгрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновыеЗадания.ПолучитьФоновыеЗадания (Отбор).Количество() = 1 Тогда
		ВызватьИсключение НСтр("ka='Синхронизация данных уже выполняется.';ru='Синхронизация данных уже выполняется.'");
	КонецЕсли;
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов",
										Параметры,
										КлючФоновогоЗадания,
										НСтр("ka='მონაცემთა გაცვლის შესრულება ვებ-სერვისით.';ru='Выполнение обмена данными через веб-сервис.'"));
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ka='Ошибка при выгрузке данных через веб-сервис.';ru='Ошибка при выгрузке данных через веб-сервис.'");
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ka='Ошибка при загрузке данных через веб-сервис.';ru='Ошибка при загрузке данных через веб-сервис.'");
	КонецЕсли;
...
Функция КлючФоновогоЗаданияВыгрузкиЗагрузкиДанных(ПланОбмена, КодУзла)
	...
	Ключ = НСтр("ka='ПланОбмена:[ПланОбмена] КодУзла:[КодУзла]';ru='ПланОбмена:[ПланОбмена] КодУзла:[КодУзла]'");
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить(ИмяМетода, Параметры,, НСтр("ka='Создание обмена данными.';ru='Создание обмена данными.'"));
