...
Функция СоздатьОбменДанными(ИмяПланаОбмена, СтрокаПараметров, НастройкаОтборовXDTO, ЗначенияПоУмолчаниюXDTO)
	...
	Если Отказ Тогда
		Сообщение = НСтр("ka='გაცვლის გამართვის შექმნისას მეორე საინფორმაციო ბაზაში შეცდომები წარმოიშვა: %1';ru='При создании настройки обмена во второй информационной базе возникли ошибки: %1';en='When creating a setup for the second exchange of information database errors occurred:% 1'");
...
Функция GetFilePart(TransferId, PartNumber, PartData)
	...
	Если ИменаФайлов.Количество() = 0 Тогда
		...
		ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %2-თან გადაცემის სესიის ფრაგმენტი %1 ვერ მოიძებნა';ru='Не найден фрагмент %1 сессии передачи с идентификатором %2';en='Not found a fragment of 1% transmission of session ID% 2'");
	...
	ИначеЕсли ИменаФайлов.Количество() > 1 Тогда
		...
		ШаблонСообщения = НСтр("ka='მოიძებნა გადაცემის სესიის რამდენიმე ფრაგმენტი %1 იდენტიფიკატორით %2';ru='Найдено несколько фрагментов %1 сессии передачи с идентификатором %2';en='Found several fragments 1% transmission session identifier% 2'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Строка(PartNumber), Строка(TransferId));
		ВызватьИсключение(ТекстСообщения);
...
Функция SaveFileFromParts(TransferId, PartQuantity, FileId)
	...
	Для НомерЧасти = 1 По PartQuantity Цикл
		...
		Если НайтиФайлы(ИмяФайла).Количество() = 0 Тогда
			ШаблонСообщения = НСтр("ka='იდენტიფიკატორ %2-ით გადაცემის სესიის ფრაგმენტი %1 ვერ მოიძებნა. აუცილებლად უნდა დარწმუნდეთ იმაში, რომ პროგრამის გამართვებში მიცემულია პარამეტრი ""დროებითი ფაილების კატალოგი Linux-ისთვის"" და ""დროებითი ფაილების კატალოგი Windows-ისთვის"".';ru='Не найден фрагмент %1 сессии передачи с идентификатором %2."
...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзла).Пустая() Тогда
		Результат = НСтр("ka='მონაცემთა სინქრონიზაციის შესაძლებლობა გამორთულია ინტერნეტში დანართის ადმინისტრატორის მიერ.';ru='Возможность синхронизации данных отключена администратором приложения в Интернете.';en='The ability to synchronize data off the application administrator on the Internet.'");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления()
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ka='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.';ru='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.';en='Data synchronization is temporarily unavailable due to update the application on the Internet.'");
...
Процедура ВыполнитьВыгрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновыеЗадания.ПолучитьФоновыеЗадания (Отбор).Количество() = 1 Тогда
		ВызватьИсключение НСтр("ka='Синхронизация данных уже выполняется.';ru='Синхронизация данных уже выполняется.';en='Data synchronization is already in progress.'");
	КонецЕсли;
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов",
										Параметры,
										КлючФоновогоЗадания,
										НСтр("ka='მონაცემთა გაცვლის შესრულება ვებ-სერვისით.';ru='Выполнение обмена данными через веб-сервис.';en='Performing data exchange through a web service.'"));
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ka='Ошибка при выгрузке данных через веб-сервис.';ru='Ошибка при выгрузке данных через веб-сервис.';en='Error when uploading data via a web service.'");
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ka='Ошибка при загрузке данных через веб-сервис.';ru='Ошибка при загрузке данных через веб-сервис.';en='Error loading data through a web service.'");
	КонецЕсли;
...
Функция КлючФоновогоЗаданияВыгрузкиЗагрузкиДанных(ПланОбмена, КодУзла)
	...
	Ключ = НСтр("ka='ПланОбмена:[ПланОбмена] КодУзла:[КодУзла]';ru='ПланОбмена:[ПланОбмена] КодУзла:[КодУзла]';en='PlanObmena: [PlanObmena] KodUzla: [KodUzla]'");
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить(ИмяМетода, Параметры,, НСтр("ka='Создание обмена данными.';ru='Создание обмена данными.';en='Creation of data exchange.'"));
