...
Процедура ПроверитьДоступУстройства(КодМобильногоКомпьютера)
	...
	Если УзелОбмена.ДоступЗапрещен Тогда
		ВызватьИсключение(
			НСтр("ka='Устройству запрещена синхронизация с центральной базой.';ru='Устройству запрещена синхронизация с центральной базой.'")
		);
	КонецЕсли;
...
Функция ДобавитьПользователя(ПочтаПользователя, ПарольПользователяСервиса, ПрофильПользователя)
	...
	Возврат НСтр("ka='მომხმარებელს ელექტრონულ ფოსტაზე წარმატებით გაეგზავნა მოწვევა.';ru='Пользователю успешно отправлено приглашение на электронный адрес.'");
...
Функция ОбновитьДанныеПользователя(ПочтаПользователя, ПарольПользователяСервиса, ПрофильПользователя, ВходРазрешен)
	...
	Если НЕ ЗначениеЗаполнено(Пользователь) Тогда
		Возврат НСтр("ka='მომხმარებელი ვერ მოიძებნა.';ru='Пользователь не найден.'");
	КонецЕсли;
	...
	Возврат НСтр("ka='მომხმარებლის პარამეტრები წარმატებით შეიცვალა.';ru='Параметры пользователя успешно изменены.'");
...
Процедура ПроверитьВозможностьДобавленияновогоУзла()
	...
	Если МобильноеПриложениеВызовСервера.НужноОграничитьФункциональность() Тогда
		...
		КонецЦикла;
		Если Сч >= 4 Тогда
			ВызватьИсключение(НСтр("ka='თქვენი სატარიფო გეგმა არ იძლევა სამზე მეტი მობილური დანართის ჩართვის შესაძლებლობას.';ru='Ваш тарифный план не позволяет подключать больше 3-х мобильных приложений.'"));
		КонецЕсли;
	КонецЕсли;
