	...
	Попытка
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				ВызватьИсключение НСтр("ka='ოპერაციის მოხმობა არ შეიძლება გამყოფების მითითებით გაშვებულ სეანსში. შეამოწმეთ ვებ-სერვისის პუბლიკაციის სისწორე!';ru='Операция не может быть вызвана в сеансе, который запущен с указанием разделителей. Проверьте правильность публикации веб-сервиса!'");
			...
			Иначе
				...
				Если КлючОбластиДанных <> Константы.КлючОбластиДанных.Получить() Тогда
					ВызватьИсключение НСтр("ka='მონაცემთა არეალის არაკორექტული გასაღები!';ru='Некорректный ключ области данных!'");
				КонецЕсли;
		...
		Иначе
			ВызватьИсключение НСтр("ka='ოპერაცია არ შეიძლება შესრულდეს იმ საინფორმაციო ბაზისთვის, რომელშიც გამორთულია მონაცემთა არეალებით დაყოფა!';ru='Операция не может быть выполнена для информационной базы, в которой отключено разделение по областям данных!'");
		КонецЕсли;
