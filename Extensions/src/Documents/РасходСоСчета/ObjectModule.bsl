...
&После("ЗаполнитьПоАвансовыйОтчет")
Процедура РасшAzRy_ЗаполнитьПоАвансовыйОтчет(ДанныеЗаполнения)
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента КАК ЧИСЛО(15, 2)) КАК СуммаДокумента
	|ИЗ
	...
	Выборка.Следующий();
	//ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
	ЭтотОбъект.СуммаДокумента = Выборка.СуммаДокумента;
	
	Для каждого ТекСтрока Из ЭтотОбъект.РасшифровкаПлатежа Цикл
		ТекСтрока.AzRyВалюта = ДанныеЗаполнения.ВалютаДокумента;
		ТекСтрока.СуммаПлатежа = ТекСтрока.СуммаРасчетов;
	КонецЦикла;
