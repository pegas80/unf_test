...
Процедура РасшAzRy_ПриСозданииНаСервереВместо(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		Объект.Контрагент = AzRyУправлениеНебольшойФирмойСервер_Deprecated.УчетнаяПолитика(ДатаДокумента, Объект.Организация).РозничныйКонтрагент;
	КонецЕсли;
	// +++ } AzRy - [01.12.2020] - Jira:ERPM-804 - Developer:ადმინი - Comment:
...
Процедура СчетФактураОтправленныйНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Не AzRyРаботаСДокументамыКлиентСервер_Deprecated.КонтрагентНалогообложениеНДС(Объект.Контрагент) Тогда
...
Процедура ОбновитьСчетФактураОтправленныйНадпись()
	...
	НайденныйСчетФактура = AzRyУправлениеНебольшойФирмойСервер_Deprecated.ПолучитьПодчиненныйСчетФактура(Объект.Ссылка);
	Элементы.СчетФактураОтправленныйНадпись.Заголовок = AzRyРаботаСДокументамыКлиентСервер_Deprecated.СформироватьНадписьСчетФактураОтправленный(НайденныйСчетФактура, , Объект.Контрагент);
...
Процедура ОбновлениеРеквизитаФормаоплаты()
	...
	ИначеЕсли Объект.ФормаОплаты = Перечисления.ТипыДенежныхСредств.Безналичные Тогда 	
		Объект.Контрагент = AzRyУправлениеНебольшойФирмойСервер_Deprecated.УчетнаяПолитика(ДатаДокумента, Объект.Организация).AzRyРозничныйКонтрагентАналитика;
		Объект.AzRyЭлектроннаяНакладная = Ложь;
