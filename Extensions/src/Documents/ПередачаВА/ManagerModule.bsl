...
Процедура РасшAzRy_СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПередачаВА, СтруктураДополнительныеСвойства)
	...
	AzRyУправлениеНебольшойФирмойСервер.ПриходнаяНакладная_ИнициализироватьДанныеДокумента_ДопТекстЗапроса("ПередачаВА", Запрос, ДокументСсылкаПередачаВА.AzRyДокументЗагруженИзДругойБазыРедактированиеЗапрешено);	
	// +++ } AzRy - [29.10.2021] - Developer:გიორგი მამულაშვილი - Comment:
	...
	НомерЗапроса = 0;
	Запрос.Текст = AzRyУправлениеНебольшойФирмойСервер.AzRy_ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателями(Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, ДокументСсылкаПередачаВА);
	// Щербаков +++
	Запрос.Текст = Запрос.Текст + ";" + AzRyУправлениеНебольшойФирмойСервер.AzRy_ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателямиРегл(Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, ДокументСсылкаПередачаВА);
	// Щербаков ---
	МассивРезультатов = Запрос.ВыполнитьПакет();
	// +++ } AzRy - [16.12.2021] - Developer:გიორგი მამულაშვილი - Comment:	
