...
Процедура РасшAzRy_СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	AzRyУправлениеНебольшойФирмойСервер.ПриходнаяНакладная_ИнициализироватьДанныеДокумента_ДопТекстЗапроса("АктВыполненныхРабот", Запрос, ДокументСсылкаАктВыполненныхРабот.AzRyДокументЗагруженИзДругойБазыРедактированиеЗапрешено);	
	// +++ } AzRy - [24.11.2021] - Developer:გიორგი მამულაშვილი - Comment:	
	...
	Запрос.Текст = AzRyУправлениеНебольшойФирмойСервер.AzRy_ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателями(Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, ДокументСсылкаАктВыполненныхРабот);
	Запрос.Текст = Запрос.Текст + ";" + AzRyУправлениеНебольшойФирмойСервер.AzRy_ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателямиРегл(Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, ДокументСсылкаАктВыполненныхРабот);	
	// +++ } AzRy - [24.11.2021] - Developer:გიორგი მამულაშვილი - Comment: ძველი დოკუმენტების გადმოტანის გამო - საკურსო სხვაობის წამოღებისთვის		
